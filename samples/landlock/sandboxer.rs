...
//! Source: samples/landlock/sandboxer.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn str2num(numstr: *const c_char, num_dst: *mut __u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if('\0'): *mut *mut *mut (numstr == '\0') || (endptr !=) -> else {
...
#[no_mangle]
unsafe extern "C" fn parse_path(env_path: *mut c_char, path_list: *const *const *const *const c_char) -> c_int {
...
// clang-format on
    ...
    char *env_path_name;
    const char **path_list = core::ptr::null_mut();
...
// Returns true on error, false otherwise.
    ...
    let mut abstract_scoping: bool = false;
    let mut signal_scoping: bool = false;
...
//
    ...
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "all") ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "write") ==) -> else {
    else if (strcmp(str_access, "write") == 0)
    ruleset_attr.quiet_access_fs |=
    ACCESS_FS_ROUGHLY_WRITE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "tcp_bind") ==) -> else {
    else if (strcmp(str_access, "tcp_bind") == 0)
    ruleset_attr.quiet_access_net |=
    LANDLOCK_ACCESS_NET_BIND_TCP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "tcp_connect") ==) -> else {
    else if (strcmp(str_access, "tcp_connect") == 0)
    ruleset_attr.quiet_access_net |=
    LANDLOCK_ACCESS_NET_CONNECT_TCP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "udp_bind") ==) -> else {
    else if (strcmp(str_access, "udp_bind") == 0)
    ruleset_attr.quiet_access_net |=
    LANDLOCK_ACCESS_NET_BIND_UDP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "udp_connect") ==) -> else {
    else if (strcmp(str_access, "udp_connect") == 0)
    ruleset_attr.quiet_access_net |=
    LANDLOCK_ACCESS_NET_CONNECT_SEND_UDP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "abstract_unix_socket") ==) -> else {
    else if (strcmp(str_access, "abstract_unix_socket") == 0)
    ruleset_attr.quiet_scoped |=
    LANDLOCK_SCOPE_ABSTRACT_UNIX_SOCKET;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str_access, 0: "signal") ==) -> else {
...
pub const LANDLOCK_ABI_LAST: c_int = 11;
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *const *const c_char, envp: *const *const *const c_char) -> c_int {
    ...
    let mut quiet_supported: bool = true;
    int supported_restrict_flags = LANDLOCK_RESTRICT_SELF_LOG_NEW_EXEC_ON |
    LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS;
    let mut set_restrict_flags: c_int = LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS;
    if (argc < 2) {
    fprintf(stderr, help, argv[0]);
    return 1;
    }
    abi = landlock_create_ruleset(core::ptr::null_mut(), 0, LANDLOCK_CREATE_RULESET_VERSION);
    if (abi < 0) {
