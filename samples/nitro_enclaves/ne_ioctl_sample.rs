...
//! Source: samples/nitro_enclaves/ne_ioctl_sample.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ne_user_mem_region {
    pub userspace_addr: *mut c_void,
...
//
#[no_mangle]
unsafe extern "C" fn ne_create_vm(ne_dev_fd: c_int, slot_uid: *mut c_ulong, enclave_fd: *mut c_int) -> c_int {
...
//
    ...
    {
    let mut enclave_fd: c_int = *(int *)data;
    struct pollfd fds[1] = {};
    let mut i: c_int = 0;
    let mut rc: c_int = -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn ne_alloc_user_mem_region(ne_user_mem_region: *mut ne_user_mem_region) -> c_int {
...
//
    ne_user_mem_region.userspace_addr = mmap(core::ptr::null_mut(), ne_user_mem_region.memory_size,
...
//
    ...
    {
    unsigned char *enclave_image = core::ptr::null_mut();
    let mut enclave_image_fd: c_int = -1;
    let mut enclave_image_size: usize = 0;
    let mut enclave_memory_size: usize = 0;
    let mut i: c_ulong = 0;
    let mut image_written_bytes: usize = 0;
    struct ne_image_load_info image_load_info = {
    .flags = NE_EIF_IMAGE,
    };
    let mut image_stat_buf: stat = {};
    let mut rc: c_int = -EINVAL;
    let mut temp_image_offset: usize = 0;
    ...
    enclave_image = mmap(core::ptr::null_mut(), enclave_image_size, PROT_READ,
    ...
    for (i = 0; i < NE_DEFAULT_NR_MEM_REGIONS; i++) {
    let mut bytes_to_write: usize = 0;
    let mut memory_offset: usize = 0;
    let mut memory_size: usize = ne_user_mem_regions[i].memory_size;
...
//
#[no_mangle]
unsafe extern "C" fn ne_set_user_mem_region(enclave_fd: c_int, ne_user_mem_region: ne_user_mem_region) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ne_free_mem_regions(ne_user_mem_regions[]: ne_user_mem_region) {
...
//
#[no_mangle]
unsafe extern "C" fn ne_add_vcpu(enclave_fd: c_int, vcpu_id: *mut c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ne_start_enclave(enclave_fd: c_int, enclave_start_info: *mut ne_enclave_start_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ne_start_enclave_check_booted(enclave_fd: c_int) -> c_int {
    static int ne_start_enclave_check_booted(int enclave_fd)
    {
    let mut client_vsock_addr: sockaddr_vm = {};
    let mut client_vsock_fd: c_int = -1;
    let mut client_vsock_len: socklen_t = sizeof(client_vsock_addr);
    let mut enclave_start_info: ne_enclave_start_info = {};
    struct pollfd fds[1] = {};
    let mut rc: c_int = -EINVAL;
    let mut recv_buf: c_uchar = 0;
    struct sockaddr_vm server_vsock_addr = {
    .svm_family = AF_VSOCK,
    .svm_cid = NE_IMAGE_LOAD_HEARTBEAT_CID,
    .svm_port = NE_IMAGE_LOAD_HEARTBEAT_PORT,
    };
    let mut server_vsock_fd: c_int = -1;
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    {
    let mut enclave_fd: c_int = -1;
    let mut i: c_uint = 0;
    let mut ne_dev_fd: c_int = -1;
    ...
    exit(EXIT_FAILURE);
    printf("Enclave fd %d\n", enclave_fd);
    rc = pthread_create(&thread_id, core::ptr::null_mut(), ne_poll_enclave_fd, (void *)&enclave_fd);
    if (rc < 0) {
