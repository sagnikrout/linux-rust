...
//! Source: samples/hid/hid_mouse.bpf.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn hid_y_event(hctx: *mut hid_bpf_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_x_event(hctx: *mut hid_bpf_ctx) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: hid_event, hctx: *mut hid_bpf_ctx, type: enum hid_report_type) -> c_int {
    int BPF_PROG(hid_event, struct hid_bpf_ctx *hctx, enum hid_report_type type)
    {
    let mut ret: c_int = hid_y_event(hctx);
    if (ret)
    return ret;
