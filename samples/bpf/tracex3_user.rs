...
//! Source: samples/bpf/tracex3_user.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn clear_stats(fd: c_int) {
    ...
    let mut num_colors: c_int = ARRAY_SIZE(color);
...
#[no_mangle]
unsafe extern "C" fn print_hist(fd: c_int) {
    static void print_hist(int fd)
    {
    let mut nr_cpus: c_uint = bpf_num_possible_cpus();
    let mut total_events: __u64 = 0;
    long values[nr_cpus];
    let mut max_cnt: __u64 = 0;
    __u64 cnt[SLOTS];
    __u64 value;
    __u32 key;
    int i;
    ...
    let mut c: c_int = num_colors * cnt[key] / (max_cnt + 1);
    if (text_only)
    printf("%s", sym[c]);
    else
