...
//! Source: samples/bpf/cookie_uid_helper_example.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn maps_create() {
    static void maps_create(void)
    {
    map_fd = bpf_map_create(BPF_MAP_TYPE_HASH, core::ptr::null_mut(), sizeof(uint32_t),
    sizeof(struct stats), 100, core::ptr::null_mut());
    if (map_fd < 0)
    error(1, errno, "map create failed!\n");
    }
#[no_mangle]
unsafe extern "C" fn prog_load() {
...
//
    ...
    prog_fd = bpf_prog_load(BPF_PROG_TYPE_SOCKET_FILTER, core::ptr::null_mut(), "GPL",
    prog, ARRAY_SIZE(prog), &opts);
    if (prog_fd < 0)
    error(1, errno, "failed to load prog\n%s\n", log_buf);
    }
#[no_mangle]
unsafe extern "C" fn prog_attach_iptables(file: *mut c_char) {
...
#[no_mangle]
unsafe extern "C" fn udp_client() {
    static void udp_client(void)
    {
    let mut si_other: sockaddr_in = {0};
    let mut si_me: sockaddr_in = {0};
    struct stats dataEntry;
    int s_rcv, s_send, i, recv_len;
    let mut message: c_char = 'a';
    char buf;
    uint64_t cookie;
    int res;
    let mut cookie_len: socklen_t = sizeof(cookie);
    let mut slen: socklen_t = sizeof(si_other);
...
#[no_mangle]
unsafe extern "C" fn finish(ret: c_int) {
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    {
    int opt;
    let mut cfg_test_traffic: bool = false;
    let mut cfg_test_cookie: bool = false;
    if (argc != 3)
    return usage();
