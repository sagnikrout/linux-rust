...
//! Source: samples/bpf/task_fd_query_user.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ptr_to_u64(ptr: *mut c_void) -> __u64 {
...
#[no_mangle]
unsafe extern "C" fn bpf_find_probe_type(event_type: *const c_char) -> c_int {
    ...
    CHECK_PERROR_RET(ret < 0 || ret >= sizeof(buf));
    errno = 0;
    ret = (int)strtol(buf, core::ptr::null_mut(), 10);
    CHECK_PERROR_RET(errno);
    return ret;
    }

#[no_mangle]
unsafe extern "C" fn bpf_get_retprobe_bit(event_type: *const c_char) -> c_int {
    ...
    errno = 0;
    ret = (int)strtol(buf + strlen("config:"), core::ptr::null_mut(), 10);
    CHECK_PERROR_RET(errno);
    return ret;
    }
    ...
    {
    let mut is_return_bit: c_int = bpf_get_retprobe_bit(event_type);
    let mut type: c_int = bpf_find_probe_type(event_type);
...
#[no_mangle]
unsafe extern "C" fn test_debug_fs_uprobe(binary_path: *mut c_char, offset: c_long, is_return: bool) -> c_int {
    ...
    __u64 probe_offset, probe_addr;
    __u32 len, prog_id, fd_type;
    let mut err: c_int = -1, res, kfd, efd;
    struct bpf_link *link;
    ssize_t bytes;
...
// set a kprobe on "bpf_check + 0x5", which is x64 specific
    ...
    BPF_FD_TYPE_KRETPROBE,
    buf, sizeof(buf)));
    CHECK_AND_RET(test_nondebug_fs_probe("kprobe", core::ptr::null_mut(), 0x0,
