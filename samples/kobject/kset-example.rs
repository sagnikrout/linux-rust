...
//! Source: samples/kobject/kset-example.c
#![no_std]
...
pub struct foo_attribute {
    pub attr: attribute,
    pub buf): *const *const *const *const ssize_t (show)(struct foo_obj foo, struct foo_attribute attr, char,
...
//
    retval = kobject_init_and_add(&foo.kobj, &foo_ktype, core::ptr::null_mut(), "%s", name);
    if (retval) {
...
#[no_mangle]
unsafe extern "C" fn destroy_foo_obj(foo: *mut foo_obj) {
...
//
    example_kset = kset_create_and_add("kset_example", core::ptr::null_mut(), kernel_kobj);
    if (!example_kset)
    return -ENOMEM;
//
