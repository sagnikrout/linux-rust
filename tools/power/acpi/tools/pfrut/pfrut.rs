...
//! Source: tools/power/acpi/tools/pfrut/pfrut.c
#![no_std]
...
pub const LOG_INFO: c_int = 2;
pub const LOG_VERB: c_int = 4;
pub const LOG_EXEC_IDX: c_int = 0;
pub const LOG_HISTORY_IDX: c_int = 1;
pub const REVID_1: c_int = 1;
pub const REVID_2: c_int = 2;
#[no_mangle]
unsafe extern "C" fn valid_log_level(level: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn valid_log_type(type: c_int) -> c_int {
    static int valid_log_type(int type)
    {
    let mut type: return = = LOG_EXEC_IDX || type == LOG_HISTORY_IDX;
    }
#[no_mangle]
pub unsafe extern "C" fn valid_log_revid(id: c_int) -> c_int {
    static inline int valid_log_revid(int id)
    {
    let mut id: return = = REVID_1 || id == REVID_2;
    }
#[no_mangle]
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    addr_map_capsule = mmap(core::ptr::null_mut(), st.st_size, PROT_READ, MAP_SHARED,
    fd_capsule, 0);
    ...
    if (action) {
    if (action == 1) {
    ret = ioctl(fd_update, PFRU_IOC_STAGE, core::ptr::null_mut());
    } else if (action == 2) {
    ret = ioctl(fd_update, PFRU_IOC_ACTIVATE, core::ptr::null_mut());
    } else if (action == 3) {
    ret = ioctl(fd_update, PFRU_IOC_STAGE_ACTIVATE, core::ptr::null_mut());
    } else {
    close(fd_update);
    close(fd_update_log);
    return 1;
    ...
    p_mmap = mmap(core::ptr::null_mut(), max_data_sz, PROT_READ, MAP_SHARED, fd_update_log, 0);
