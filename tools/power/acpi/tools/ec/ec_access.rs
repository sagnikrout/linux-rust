...
//! Source: tools/power/acpi/tools/ec/ec_access.c
#![no_std]
...
//
    let mut read_mode: static int = -1;
    static int sleep_time;
    let mut write_byte_offset: static int = -1;
    let mut read_byte_offset: static int = -1;
    let mut write_value: static uint8_t = -1;
#[no_mangle]
pub unsafe extern "C" fn usage(progname[]: c_char, exit_status: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn parse_opts(argc: c_int, argv[]: *mut c_char) {
    ...
    usage(argv[0], EXIT_FAILURE);
    read_mode = 1;
    read_byte_offset = strtoul(optarg, core::ptr::null_mut(), 16);
    break;
    case 'w':
    if (read_mode != -1)
    usage(argv[0], EXIT_FAILURE);
    read_mode = 0;
    write_byte_offset = strtoul(optarg, core::ptr::null_mut(), 16);
    break;
    case 'v':
    write_value = strtoul(optarg, core::ptr::null_mut(), 16);
    break;
    case 'h':
...
#[no_mangle]
pub unsafe extern "C" fn dump_ec(fd: c_int) {
    void dump_ec(int fd)
    {
...
#[no_mangle]
pub unsafe extern "C" fn read_ec_val(fd: c_int, byte_offset: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn write_ec_val(fd: c_int, byte_offset: c_int, value: u8) {
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    file_mode = O_WRONLY;
#[no_mangle]
pub unsafe extern "C" fn if(1: read_mode ==) -> else {
    else if (read_mode == 1)
    file_mode = O_RDONLY;
    else
    usage(argv[0], EXIT_FAILURE);
