...
//! Source: tools/power/acpi/tools/acpidump/apmain.c
#![no_std]
...
//
// Macro flag: #define _DECLARE_GLOBALS
...
//
#[no_mangle]
unsafe extern "C" fn ap_display_usage() {
    static void ap_display_usage(void)
    {
...
//
#[no_mangle]
unsafe extern "C" fn ap_insert_action(argument: *mut c_char, to_be_done: u32) -> c_int {
    static int ap_insert_action(char *argument, u32 to_be_done)
    {
...
//
#[no_mangle]
unsafe extern "C" fn ap_do_options(argc: c_int, argv: *mut c_char) -> c_int {
    static int ap_do_options(int argc, char **argv)
    {
    int j;
    acpi_status status;
...
// If there are no actions, this means "get/dump all tables"
    if (current_action == 0) {
    if (ap_insert_action(core::ptr::null_mut(), AP_DUMP_ALL_TABLES)) {
    return (-1);
    }
    }
    return (0);
...
#[no_mangle]
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> int ACPI_SYSTEM_XFACE {
    int ACPI_SYSTEM_XFACE main(int argc, char *argv[])

#[no_mangle]
pub unsafe extern "C" fn acpi_main(argc: c_int, argv[]: *mut c_char) -> int ACPI_SYSTEM_XFACE {
