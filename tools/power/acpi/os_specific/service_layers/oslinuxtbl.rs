...
//! Source: tools/power/acpi/os_specific/service_layers/oslinuxtbl.c
#![no_std]
...
// Should we get dynamically loaded SSDTs from DYNAMIC_TABLE_DIR?
    let mut gbl_dump_dynamic_tables: u8 = TRUE;
// Initialization flags
    let mut gbl_table_list_initialized: u8 = FALSE;
// Local copies of main ACPI tables
    struct acpi_table_rsdp gbl_rsdp;
    struct acpi_table_fadt *gbl_fadt = core::ptr::null_mut();
    struct acpi_table_rsdt *gbl_rsdt = core::ptr::null_mut();
    struct acpi_table_xsdt *gbl_xsdt = core::ptr::null_mut();
// Table addresses
    let mut gbl_fadt_address: acpi_physical_address = 0;
    let mut gbl_rsdp_address: acpi_physical_address = 0;
// Revision of RSD PTR
    let mut gbl_revision: u8 = 0;
    struct osl_table_info *gbl_table_list_head = core::ptr::null_mut();
    let mut gbl_table_count: u32 = 0;
//
...
//
#[no_mangle]
unsafe extern "C" fn osl_get_last_status(default_status: acpi_status) -> acpi_status {
    static acpi_status osl_get_last_status(acpi_status default_status)
    {
    switch (errno) {
    case EACCES:
...
//
    ...
    struct acpi_table_header *local_table = core::ptr::null_mut();
...
// Map the table and validate it
    status = osl_map_table(address, core::ptr::null_mut(), &mapped_table);
    if (ACPI_FAILURE(status)) {
    return (status);
    }
...
//
#[no_mangle]
unsafe extern "C" fn osl_add_table_to_list(signature: *mut c_char, instance: u32) -> acpi_status {
    static acpi_status osl_add_table_to_list(char *signature, u32 instance)
    {
...
//
#[no_mangle]
unsafe extern "C" fn osl_find_rsdp_via_efi() -> acpi_physical_address {
    static acpi_physical_address osl_find_rsdp_via_efi(void)
    {
    FILE *file;
...
//
#[no_mangle]
unsafe extern "C" fn osl_load_rsdp() -> acpi_status {
    static acpi_status osl_load_rsdp(void)
    {
    struct acpi_table_header *mapped_table;
    u8 *rsdp_address;
...
//
#[no_mangle]
unsafe extern "C" fn osl_can_use_xsdt() -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn osl_table_initialize() -> acpi_status {
    static acpi_status osl_table_initialize(void)
    {
    acpi_status status;
    acpi_physical_address address;
...
//
#[no_mangle]
unsafe extern "C" fn osl_list_bios_tables() -> acpi_status {
    static acpi_status osl_list_bios_tables(void)
    {
    struct acpi_table_header *mapped_table = core::ptr::null_mut();
    u8 *table_data;
    u8 number_of_tables;
    u8 item_size;
...
// Skip NULL entries in RSDT/XSDT
    ...
    status = osl_map_table(table_address, core::ptr::null_mut(), &mapped_table);
    if (ACPI_FAILURE(status)) {
    return (status);
    }
...
// Match table instance
    ...
    osl_unmap_table(mapped_table);
    mapped_table = core::ptr::null_mut();
    current_instance++;
...
// Skip NULL entries in RSDT/XSDT
    ...
    status =
    osl_map_table(table_address, core::ptr::null_mut(), &mapped_table);
    if (ACPI_FAILURE(status)) {
    return (status);
    }
...
//
#[no_mangle]
unsafe extern "C" fn osl_list_customized_tables(directory: *mut c_char) -> acpi_status {
    static acpi_status osl_list_customized_tables(char *directory)
    {
    void *table_dir;
    u32 instance;
...
//
#[no_mangle]
unsafe extern "C" fn osl_unmap_table(table: *mut acpi_table_header) {
    static void osl_unmap_table(struct acpi_table_header *table)
    {
    if (table) {
...
// Open the file
    ...
    if (table_file == core::ptr::null_mut()) {
