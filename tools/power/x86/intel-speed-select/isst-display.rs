...
//! Source: tools/power/x86/intel-speed-select/isst-display.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn format_and_print(outf: *mut FILE, level: c_int, header: *mut c_char, value: *mut c_char) {
...
#[no_mangle]
unsafe extern "C" fn print_package_info(id: *mut isst_id, outf: *mut FILE) -> c_int {
    ...
    id.pkg, id.punit);
#[no_mangle]
pub unsafe extern "C" fn if(0: id->cpu <) -> else {
    else if (id.cpu < 0)
    snprintf(header, sizeof(header),
    ...
    format_and_print(outf, level, header, core::ptr::null_mut());
    return 1;
    }
    ...
    format_and_print(outf, disp_level, header, core::ptr::null_mut());
...
// core count must be valid for CPU power domain
    ...
    format_and_print(outf, base_level, header, core::ptr::null_mut());
    for (j = 0; j < ISST_FACT_MAX_BUCKETS; ++j) {
    int i;
    if (fact_bucket != 0xff && fact_bucket != j)
    continue;
    ...
    format_and_print(outf, base_level + 1, header, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(val: str1 &&) -> else {
    else if (str1 && val)
    snprintf(value, sizeof(value), "%s", str1);
    else
    snprintf(value, sizeof(value), "%u", val);
    format_and_print(outf, level, prefix, value);
    format_and_print(outf, 1, core::ptr::null_mut(), core::ptr::null_mut());
    }
    ...
    ctdp_level.level);
    format_and_print(outf, level + 1, header, core::ptr::null_mut());
    if (id.cpu >= 0) {
    ...
    format_and_print(outf, level + 2, header, core::ptr::null_mut());
    for (j = 0; j < 8; ++j) {
    snprintf(header, sizeof(header), "bucket-%d", j);
    format_and_print(outf, level + 3, header, core::ptr::null_mut());
...
pub unsafe extern "C" fn isst_ctdp_display_information_start(outf: *mut FILE) {
    ...
    {
    last_level = 0;
    format_and_print(outf, 0, "start", core::ptr::null_mut());
    start = 1;
    }
#[no_mangle]
pub unsafe extern "C" fn isst_ctdp_display_information_end(outf: *mut FILE) {
    ...
    {
    format_and_print(outf, 0, core::ptr::null_mut(), core::ptr::null_mut());
    start = 0;
    }
    ...
    format_and_print(outf, level + 2, header, value);
    format_and_print(outf, level, core::ptr::null_mut(), core::ptr::null_mut());
    }
...
pub unsafe extern "C" fn isst_display_error_info_message(error: c_int, msg: *mut c_char, arg_valid: c_int, arg: c_int) {
    ...
    format_and_print(outf, 1, header, core::ptr::null_mut());
