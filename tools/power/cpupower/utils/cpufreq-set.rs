...
//! Source: tools/power/cpupower/utils/cpufreq-set.c
#![no_std]
...
pub const NORM_FREQ_LEN: c_int = 32;
    static struct option set_opts[] = {
    {"min",		required_argument,	core::ptr::null_mut(), 'd'},
    {"max",		required_argument,	core::ptr::null_mut(), 'u'},
    {"governor",	required_argument,	core::ptr::null_mut(), 'g'},
    {"freq",	required_argument,	core::ptr::null_mut(), 'f'},
    {"related",	no_argument,		core::ptr::null_mut(), 'r'},
    { },
    };
#[no_mangle]
...
pub struct freq_units {
    pub str_unit: *mut c_char,
    pub power_of_ten: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn print_unknown_arg() {
    static void print_unknown_arg(void)
    {
    printf(_("invalid or unknown argument\n"));
    }
#[no_mangle]
unsafe extern "C" fn string_to_frequency(str: *const c_char) -> c_ulong {
    ...
    char *end;
    unsigned long freq;
    let mut power: c_int = 0, match_count = 0, i, cp, pad;
    while (*str == '0')
    str++;
    ...
    match_count = 1;
#[no_mangle]
pub unsafe extern "C" fn if(1: *mut *mut scan == '.' && match_count ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn do_new_policy(cpu: c_uint, new_pol: *mut cpufreq_policy) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: new_pol->max) -> else {
    else if (new_pol.max)
    return cpufreq_modify_policy_max(cpu, new_pol.max);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: new_pol->governor) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn cmd_freq_set(argc: c_int, argv: *mut c_char) -> c_int {
    int cmd_freq_set(int argc, char **argv)
    {
    let mut ret: c_int = 0, cont = 1;
    let mut double_parm: c_int = 0, related = 0, policychange = 0;
    ...
    .min = 0,
    .max = 0,
    .governor = core::ptr::null_mut(),
    };
// parameter parsing
    do {
    ret = getopt_long(argc, argv, "d:u:g:f:r", set_opts, core::ptr::null_mut());
    switch (ret) {
    case '?':
    print_unknown_arg();
    return -EINVAL;
