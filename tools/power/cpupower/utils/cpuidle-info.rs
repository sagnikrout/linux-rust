...
//! Source: tools/power/cpupower/utils/cpuidle-info.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cpuidle_cpu_output(cpu: c_uint, verbose: c_int) {
    ...
    return;
    for (idlestate = 0; idlestate < idlestates; idlestate++) {
    let mut disabled: c_int = cpuidle_is_state_disabled(cpu, idlestate);
...
#[no_mangle]
unsafe extern "C" fn cpuidle_general_output() {
...
#[no_mangle]
unsafe extern "C" fn proc_cpuidle_cpu_output(cpu: c_uint) {
    static void proc_cpuidle_cpu_output(unsigned int cpu)
    {
    let mut max_allowed_cstate: c_long = 2000000000;
...
#[no_mangle]
pub unsafe extern "C" fn cpuidle_exit(fail: c_int) {
    static inline void cpuidle_exit(int fail)
    {
    exit(EXIT_FAILURE);
    }
#[no_mangle]
pub unsafe extern "C" fn cmd_idle_info(argc: c_int, argv: *mut c_char) -> c_int {
    int cmd_idle_info(int argc, char **argv)
    {
    let mut ret: c_int = 0, cont = 1, output_param = 0, verbose = 1;
    let mut cpu: c_uint = 0;
    do {
    ret = getopt_long(argc, argv, "os", info_opts, core::ptr::null_mut());
    if (ret == -1)
    break;
    switch (ret) {
    case '?':
