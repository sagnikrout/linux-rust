...
//! Source: tools/testing/cxl/test/cxl.c
#![no_std]
...
pub const FAKE_QTG_ID: c_int = 42;
pub const NR_CXL_HOST_BRIDGES: c_int = 2;
pub const NR_CXL_SINGLE_HOST: c_int = 1;
pub const NR_CXL_RCH: c_int = 1;
pub const NR_CXL_ROOT_PORTS: c_int = 2;
pub const NR_CXL_SWITCH_PORTS: c_int = 2;
pub const NR_CXL_PORT_DECODERS: c_int = 8;

pub const NR_CXL_TYPE2_ACCEL: c_int = 1;

    let mut mock_auto_region_size: static int = MOCK_AUTO_REGION_SIZE_DEFAULT;
...
#[no_mangle]
pub unsafe extern "C" fn is_multi_bridge(dev: *mut device) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn is_single_bridge(dev: *mut device) -> bool {
...
pub const CFMWS_MOD_ARRAY_START: c_int = 0;
pub const CFMWS_MOD_ARRAY_END: c_int = 5;
pub const CFMWS_XOR_ARRAY_START: c_int = 6;
pub const CFMWS_XOR_ARRAY_END: c_int = 8;
    struct acpi_cedt_cxims *mock_cxims[1] = {
    [0] = &mock_cedt.cxims0.cxims,
    };
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxl_mock_res {
    pub list: list_head,
    pub range: range,
}
...
#[no_mangle]
unsafe extern "C" fn host_bridge_index(adev: *mut acpi_device) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct target_map_ctx {
    pub target_map: *mut u32,
    pub index: c_int,
...
//
#[no_mangle]
unsafe extern "C" fn cxld_registry_index(cxld: *mut cxl_decoder) -> c_ulong {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxl_test_decoder {
    union {
    pub cxlsd: cxl_switch_decoder,
    pub cxled: cxl_endpoint_decoder,
}
...
#[no_mangle]
unsafe extern "C" fn cxld_copy(a: *mut cxl_decoder, b: *mut cxl_decoder) {
...
// Restore saved target intent; live dport binding happens later
    ...
    cxlsd.target[i] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn cxld_registry_update(cxld: *mut cxl_decoder) {
...
unsafe extern "C" fn mock_decoder_reset(cxld: *mut cxl_decoder) {
    ...
    let mut key: c_ulong = cxld_registry_index(cxld);
    if (!td)
    return core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn init_disabled_mock_decoder(cxld: *mut cxl_decoder) {
    static void init_disabled_mock_decoder(struct cxl_decoder *cxld)
    {
...
#[no_mangle]
unsafe extern "C" fn default_mock_decoder(cxld: *mut cxl_decoder) {
...
#[no_mangle]
unsafe extern "C" fn first_decoder(dev: *mut device, data: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mock_decoder_handle_saved(cxld: *mut cxl_decoder, td: *mut cxl_test_decoder) -> bool {
...
//
    ...
    dev = device_find_child(&root_port.dev, core::ptr::null_mut(), first_decoder);
//
...
//
    ...
    iter = dport.port;
    dev = device_find_child(&iter.dev, core::ptr::null_mut(), first_decoder);
//
// Ancestor ports are guaranteed to be enumerated before
// @port, and all ports have at least one decoder.
//
...
//
#[no_mangle]
unsafe extern "C" fn mock_init_hdm_decoder(cxld: *mut cxl_decoder) -> bool {
...
//
    ...
    target_count = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_cxl_root(parent_port)) -> else {
...
#[no_mangle]
unsafe extern "C" fn __mock_cxl_decoders_setup(port: *mut cxl_port) -> c_int {
    static int __mock_cxl_decoders_setup(struct cxl_port *port)
    {
    struct cxl_hdm *cxlhdm;
    cxlhdm = mock_cxl_setup_hdm(port, core::ptr::null_mut());
    if (IS_ERR(cxlhdm)) {
    if (PTR_ERR(cxlhdm) != -ENODEV)
    dev_err(&port.dev, "Failed to map HDM decoder capability\n");
    return PTR_ERR(cxlhdm);
    }
    return mock_cxl_enumerate_decoders(cxlhdm, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn mock_cxl_switch_port_decoders_setup(port: *mut cxl_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mock_cxl_endpoint_decoders_setup(port: *mut cxl_port) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mock_walk_hmem_resources(host: *mut device, fn: walk_hmem_fn) -> c_int {
...
//
    ...
    {
    let mut res: resource = DEFINE_RES_MEM(start, size);
    struct acpi_cedt_cfmws *cfmws = mock_cfmws[0];
    struct resource window =
    DEFINE_RES_MEM(cfmws.base_hpa, cfmws.window_size / 2);
...
#[no_mangle]
unsafe extern "C" fn mock_companion(adev: *mut acpi_device, dev: *mut device) {
    static void mock_companion(struct acpi_device *adev, struct device *dev)
    {
    device_initialize(&adev.dev);
    fwnode_init(&adev.fwnode, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn cxl_rch_topo_init() -> __init int {
    static __init int cxl_rch_topo_init(void)
    {
    int rc, i;
    for (i = 0; i < ARRAY_SIZE(cxl_rch); i++) {
    let mut idx: c_int = NR_CXL_HOST_BRIDGES + NR_CXL_SINGLE_HOST + i;
...
#[no_mangle]
unsafe extern "C" fn cxl_rch_topo_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_single_topo_init() -> __init int {
...
#[no_mangle]
unsafe extern "C" fn cxl_single_topo_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_type3_mem_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_type2_mem_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_mem_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_type2_mem_init() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_type3_mem_init() -> c_int {
    ...
    for (i = 0; i < ARRAY_SIZE(cxl_rcd); i++) {
    let mut idx: c_int = NR_MEM_MULTI + NR_MEM_SINGLE + i;
...
#[no_mangle]
unsafe extern "C" fn cxl_mem_init() -> c_int {
    ...
    &dev_attr_decoder_reset_preserve_registry.attr, core::ptr::null_mut()
    };
    ATTRIBUTE_GROUPS(cxl_acpi);
#[no_mangle]
unsafe extern "C" fn have_multiple_modparms() -> bool __init {
...
#[no_mangle]
unsafe extern "C" fn host_bridges_remove() {
...
#[no_mangle]
unsafe extern "C" fn host_bridges_populate() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_rootports_remove() {
...
#[no_mangle]
unsafe extern "C" fn cxl_rootports_populate() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_usps_remove() {
...
#[no_mangle]
unsafe extern "C" fn cxl_usps_populate() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_dsps_remove() {
...
#[no_mangle]
unsafe extern "C" fn cxl_dsps_populate() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_switches_remove() {
...
#[no_mangle]
unsafe extern "C" fn cxl_switches_populate() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_type2_topo_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_type2_topo_init() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_type3_topo_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_type3_topo_init() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_topo_exit() {
...
#[no_mangle]
unsafe extern "C" fn cxl_topo_init() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_test_init() -> __init int {
...
// Enforce a single module param active at a time
    ...
    rc = cxl_mock_platform_device_add(cxl_acpi, core::ptr::null_mut());
    if (rc)
    goto err_topo;
    rc = cxl_mem_init();
    if (rc)
...
#[no_mangle]
unsafe extern "C" fn free_decoder_registry() {
...
#[no_mangle]
unsafe extern "C" fn cxl_test_exit() -> __exit void {
