...
//! Source: tools/testing/nvdimm/test/nfit.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfit_test_dcr {
    pub bdw_addr: __le64,
    pub bdw_status: __le32,
    pub aperature: [__u8; BDW_SIZE],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfit_test_sec {
    pub state: u8,
    pub ext_state: u8,
    pub old_state: u8,
    pub passphrase: [u8; 32],
    pub master_passphrase: [u8; 32],
    pub overwrite_end_time: u64,
    pub dimm_sec_info: [}; NUM_DCR],
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfit_test_fw {
    pub state: enum intel_fw_update_state,
    pub context: u32,
    pub version: u64,
    pub size_received: u32,
    pub end_time: u64,
    pub armed: bool,
    pub missed_activate: bool,
    pub last_activate: c_ulong,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfit_test {
    pub acpi_desc: acpi_nfit_desc,
    pub pdev: platform_device,
    pub resources: list_head,
    pub nfit_buf: *mut c_void,
    pub nfit_dma: dma_addr_t,
    pub nfit_size: usize,
    pub nfit_filled: usize,
    pub dcr_idx: c_int,
    pub num_dcr: c_int,
    pub num_pm: c_int,
    pub dimm: *mut c_void,
    ...
    pub flush_dma: *mut dma_addr_t,
    pub label: *mut c_void,
    pub label_dma: *mut dma_addr_t,
    pub spa_set: *mut c_void,
    pub spa_set_dma: *mut dma_addr_t,
    pub dcr: *mut nfit_test_dcr,
    pub dcr_dma: *mut dma_addr_t,
    pub t): *mut *mut int (alloc)(struct nfit_test,
    pub t): *mut *mut void (setup)(struct nfit_test,
    pub setup_hotplug: c_int,
    pub _fit: *mut union acpi_object,
    pub _fit_dma: dma_addr_t,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ars_state {
    pub ars_status: *mut nd_cmd_ars_status,
    pub deadline: c_ulong,
    pub lock: spinlock_t,
    pub ars_state: },
    pub dimm_dev: [*mut device; ARRAY_SIZE(handle)],
    pub smart: *mut nd_intel_smart,
    pub smart_threshold: *mut nd_intel_smart_threshold,
    pub badrange: badrange,
    pub work: work_struct,
    pub fw: *mut nfit_test_fw,
}
...
pub const NFIT_TEST_CLEAR_ERR_UNIT: c_int = 256;
...
// for testing, only store up to n records that fit within 4k
    ...
    let mut be_end: u64 = be.start + be.length - 1;
    u64 rstart, rend;
...
// cmd_rc = 0;
    ...
    {
    let mut mask: u64 = NFIT_TEST_CLEAR_ERR_UNIT - 1;
...
unsafe extern "C" fn nfit_test_search_region_spa(dev: *mut device, data: *mut c_void) -> c_int {
    ...
    struct nd_mapping *nd_mapping = core::ptr::null_mut();
...
unsafe extern "C" fn uc_error_notify(work: *mut work_struct) {
    ...
    {
    struct badrange_entry *be;
    let mut max: c_int = SZ_4K / sizeof(struct nd_error_stat_query_record);
    let mut i: c_int = 0;
    err_stat.status = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(time_after(jiffies: !last_activate ||, HZ): *mut *mut last_activate + 3) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(info.max_quiesce_tmo: info.activate_tmo >) -> else {
    else if (info.activate_tmo > info.max_quiesce_tmo)
    status = ND_INTEL_BUS_FWA_STATUS_TMO;
#[no_mangle]
pub unsafe extern "C" fn if(ND_INTEL_FWA_IDLE: info.state ==) -> else {
...
pub unsafe extern "C" fn if(ND_INTEL_FWA_IDLE: info.state ==) -> else {
    ...
    state = ND_INTEL_FWA_IDLE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: fw->armed) -> else {
    else if (fw.armed)
    state = ND_INTEL_FWA_ARMED;
    else
    state = ND_INTEL_FWA_IDLE;
...
#[no_mangle]
unsafe extern "C" fn get_dimm(nfit_mem: *mut nfit_mem, func: c_uint) -> c_int {
...
// dma = (unsigned long) buf;
    ...
    struct nfit_test_resource *n, *nfit_res = core::ptr::null_mut();
...
unsafe extern "C" fn dimm_name_to_id(dev: *mut device) -> c_int {
    ...
    char *buf)
    {
    let mut dimm: c_int = dimm_name_to_id(dev);
    if (dimm < 0)
    return dimm;
...
unsafe extern "C" fn nfit_test_dimm_init(t: *mut nfit_test) -> c_int {
    ...
    &t.pdev.dev, 0, core::ptr::null_mut(),
...
unsafe extern "C" fn nfit_test0_alloc(t: *mut nfit_test) -> c_int {
    ...
    {
    struct acpi_nfit_system_address *spa = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn setup_result(buf: *mut c_void, size: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfit_ctl_test(dev: *mut device) -> c_int {
    ...
    struct acpi_nfit_desc *acpi_desc;
    let mut test_val: u64 = 0x0123456789abcdefULL;
    unsigned long mask, cmd_size, offset;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfit_ctl_test_cmd {
    pub pkg: nd_cmd_pkg,
    union {
    pub cfg_size: nd_cmd_get_config_size,
    pub clear_err: nd_cmd_clear_error,
    pub ars_stat: nd_cmd_ars_status,
    pub ars_cap: nd_cmd_ars_cap,
    pub fwa_info: nd_intel_bus_fw_activate_businfo,
    char buf[sizeof(struct nd_cmd_ars_status)
    pub nd_ars_record)]: + sizeof(struct,
}
...
// test ars_status with zero output
    ...
    if (rc)
    return rc;
    rc = acpi_nfit_ctl(&acpi_desc.nd_desc, core::ptr::null_mut(), ND_CMD_ARS_STATUS,
    cmd.buf, cmd_size, &cmd_rc);
    if (rc < 0 || cmd_rc) {
    dev_dbg(dev, "%s: failed at: %d rc: %d cmd_rc: %d\n",
    __func__, __LINE__, rc, cmd_rc);
...
// test ars_cap with benign extended status
    ...
    if (rc)
    return rc;
    rc = acpi_nfit_ctl(&acpi_desc.nd_desc, core::ptr::null_mut(), ND_CMD_ARS_CAP,
    cmd.buf, cmd_size, &cmd_rc);
    if (rc < 0 || cmd_rc) {
    dev_dbg(dev, "%s: failed at: %d rc: %d cmd_rc: %d\n",
    __func__, __LINE__, rc, cmd_rc);
...
// test clear error
    ...
    if (rc)
    return rc;
    rc = acpi_nfit_ctl(&acpi_desc.nd_desc, core::ptr::null_mut(), ND_CMD_CLEAR_ERROR,
    cmd.buf, cmd_size, &cmd_rc);
    if (rc < 0 || cmd_rc) {
    dev_dbg(dev, "%s: failed at: %d rc: %d cmd_rc: %d\n",
    __func__, __LINE__, rc, cmd_rc);
...
// test firmware activate bus info
    ...
    if (rc)
    return rc;
    rc = acpi_nfit_ctl(&acpi_desc.nd_desc, core::ptr::null_mut(), ND_CMD_CALL,
...
// common alloc
    ...
    let mut num: c_int = nfit_test.num_dcr;
...
// pass */;
    ...
    if (nfit_test.num_pm) {
    let mut num: c_int = nfit_test.num_pm;
...
// per-nfit specific alloc
    ...
    nd_desc.provider_name = core::ptr::null_mut();
...
// associate dimm devices with nfit_mem data for notification testing
    ...
    let mut nfit_handle: u32 = __to_nfit_memdev(nfit_mem).device_handle;
    int i;
    for (i = 0; i < ARRAY_SIZE(handle); i++)
...
#[no_mangle]
unsafe extern "C" fn nfit_test_init() -> __init int {
...
#[no_mangle]
unsafe extern "C" fn nfit_test_exit() -> __exit void {
