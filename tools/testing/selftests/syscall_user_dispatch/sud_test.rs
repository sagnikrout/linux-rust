...
//! Source: tools/testing/selftests/syscall_user_dispatch/sud_test.c
#![no_std]
...
//
    ...
    {
    let mut sel: c_char = SYSCALL_DISPATCH_FILTER_ALLOW;
    struct sysinfo info;
    int ret;
    ret = sysinfo(&info);
    ASSERT_EQ(0, ret);
...
#[no_mangle]
pub unsafe extern "C" fn setup_sigsys_handler() -> c_int {
    ...
    act.sa_mask = mask;
    return sigaction(SIGSYS, &act, core::ptr::null_mut());
    }
