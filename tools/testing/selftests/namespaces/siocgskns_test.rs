...
//! Source: tools/testing/selftests/namespaces/siocgskns_test.c
#![no_std]
...
pub const SIOCGSKNS: c_uint = 0x894C;
...
// Parent: receive socket FD
    ...
    msg.msg_control = cmsg_buf;
    msg.msg_controllen = sizeof(cmsg_buf);
    let mut n: isize = recvmsg(ipc_sockets[0], &msg, 0);
    close(ipc_sockets[0]);
    ASSERT_EQ(n, 1);
    struct cmsghdr *cmsg = CMSG_FIRSTHDR(&msg);
    ASSERT_NE(cmsg, core::ptr::null_mut());
...
// Remaining netns FDs should still be valid
    ...
    let mut test_fd: c_int = open(path, O_RDONLY);
    ASSERT_GE(test_fd, 0);
    close(test_fd);
    }
// Cleanup
...
//
    let mut test_fd: c_int = ioctl(sock_fd, SIOCGSKNS);
    ASSERT_GE(test_fd, 0);
    struct stat st_test;
...
//
    ...
    __u64 ns_ids[256];
    int ret, i;
    let mut found_netns: bool = false;
    EXPECT_EQ(socketpair(AF_LOCAL, SOCK_STREAM | SOCK_CLOEXEC, 0, ipc_sockets), 0);
    pid = fork();
    ASSERT_GE(pid, 0);
    if (pid == 0) {
...
//
    ...
    __u64 ns_ids[256];
    int ret, i;
    let mut found_netns: bool = false, found_userns = false;
    struct file_handle *handle;
    struct nsfs_file_handle *nsfs_fh;
// Allocate file_handle structure for nsfs
    handle = malloc(sizeof(struct file_handle) + sizeof(struct nsfs_file_handle));
...
// Level 2 child - receive from L3 and forward to L1
    ...
    msg.msg_control = cmsg_buf;
    msg.msg_controllen = sizeof(cmsg_buf);
    let mut n: isize = recvmsg(ipc_L3[0], &msg, 0);
    close(ipc_L3[0]);
    if (n != 1) {
    close(ipc_L2[1]);
...
// Level 1 child - receive from L2 and forward to parent
    ...
    msg.msg_control = cmsg_buf;
    msg.msg_controllen = sizeof(cmsg_buf);
    let mut n: isize = recvmsg(ipc_L2[0], &msg, 0);
    close(ipc_L2[0]);
    if (n != 1) {
    close(ipc_sockets[1]);
...
// Get owner user namespace chain: userns_L3 -> userns_L2 -> userns_L1
    let mut userns_L3_fd: c_int = ioctl(netns_L3A_fd, NS_GET_USERNS);
    if (userns_L3_fd < 0) {
    free(handle);
    close(sock_L3A_fd);
    close(netns_L3A_fd);
    ...
    ASSERT_EQ(ret, 0);
    let mut userns_L2_fd: c_int = ioctl(userns_L3_fd, NS_GET_USERNS);
    ASSERT_GE(userns_L2_fd, 0);
    ret = ioctl(userns_L2_fd, NS_GET_ID, &userns_L2_id);
    ASSERT_EQ(ret, 0);
    let mut userns_L1_fd: c_int = ioctl(userns_L2_fd, NS_GET_USERNS);
...
//
    ...
    let mut found_netns_L3A: bool = false;
    let mut found_userns_L1: bool = false;
    let mut found_userns_L2: bool = false;
    let mut found_userns_L3: bool = false;
    for (i = 0; i < ret; i++) {
