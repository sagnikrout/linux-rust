...
//! Source: tools/testing/selftests/namespaces/ns_active_ref_test.c
#![no_std]
...
pub const FILEID_NSFS: c_uint = 0xf1;
...
//
    ...
    let mut fd2: c_int = open_by_handle_at(FD_NSFS_ROOT, handle, O_RDONLY);
    ASSERT_GE(fd2, 0);
...
// Get namespace ID for parent user namespace
    let mut parent_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (parent_fd < 0) {
...
// Get namespace ID for child user namespace
    let mut child_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (child_fd < 0) {
...
//
    ...
    let mut parent_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, parent_handle, O_RDONLY);
...
// Verify we can also get parent via NS_GET_USERNS
    ...
    let mut parent_fd2: c_int = ioctl(child_nsfd, NS_GET_USERNS);
    if (parent_fd2 < 0) {
    close(parent_fd);
    close(child_nsfd);
...
// Parent should still be openable
    ...
    let mut parent_fd3: c_int = open_by_handle_at(FD_NSFS_ROOT, parent_handle, O_RDONLY);
...
// Unshare mount namespace and make mounts private to avoid propagation
    ...
    ret = mount(core::ptr::null_mut(), "/", core::ptr::null_mut(), MS_PRIVATE | MS_REC, core::ptr::null_mut());
    if (ret < 0) {
    close(pipefd[1]);
    unlink(tmpfile);
    exit(1);
...
// Bind mount the namespace
    ret = mount("/proc/self/ns/net", tmpfile, core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    if (ret < 0) {
    close(pipefd[1]);
    unlink(tmpfile);
    exit(1);
...
// Create grandparent user namespace
    ...
    let mut gp_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (gp_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Create parent user namespace
    ...
    let mut p_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (p_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Create child user namespace
    ...
    let mut c_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (c_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Open child before process exits
    let mut c_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, c_handle, O_RDONLY);
    if (c_fd < 0) {
    waitpid(pid, core::ptr::null_mut(), 0);
    SKIP(return, "Failed to open child namespace");
    }
...
//
    ...
    let mut p_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, p_handle, O_RDONLY);
    ASSERT_GE(p_fd, 0);
    TH_LOG("Testing grandparent active when child is active");
    let mut gp_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, gp_handle, O_RDONLY);
    ASSERT_GE(gp_fd, 0);
    close(c_fd);
    close(p_fd);
    close(gp_fd);
...
// Create first child user namespace
    ...
    let mut c1_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (c1_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// by creating a network namespace instead
    ...
    let mut c2_fd: c_int = open("/proc/self/ns/net", O_RDONLY);
    if (c2_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Open both children before process exits
    let mut c1_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, c1_handle, O_RDONLY);
    let mut c2_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, c2_handle, O_RDONLY);
    if (c1_fd < 0 || c2_fd < 0) {
...
// Create user namespace
    ...
    let mut u_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (u_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Create network namespace (owned by user namespace)
    ...
    let mut n_fd: c_int = open("/proc/self/ns/net", O_RDONLY);
    if (n_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Create UTS namespace (also owned by user namespace)
    ...
    let mut ut_fd: c_int = open("/proc/self/ns/uts", O_RDONLY);
    if (ut_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Open both non-user namespaces before process exits
    let mut n_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, n_handle, O_RDONLY);
    let mut ut_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, ut_handle, O_RDONLY);
    if (n_fd < 0 || ut_fd < 0) {
...
//
    ...
    let mut u_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, u_handle, O_RDONLY);
    ASSERT_GE(u_fd, 0);
    close(u_fd);
...
// Create user_A -> user_B -> net hierarchy
    ...
    let mut ua_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (ua_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
    ...
    let mut ub_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (ub_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
    ...
    let mut net_fd: c_int = open("/proc/self/ns/net", O_RDONLY);
    if (net_fd < 0) {
    close(pipefd[1]);
    exit(1);
    }
...
// Open net_ns before child exits to keep it active
    let mut net_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, net_handle, O_RDONLY);
    if (net_fd < 0) {
    waitpid(pid, core::ptr::null_mut(), 0);
...
// With net_ns active, both user_A and user_B should be active
    ...
    let mut ub_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, ub_handle, O_RDONLY);
    ASSERT_GE(ub_fd, 0);
    TH_LOG("Testing user_A active (propagated through user_B)");
    let mut ua_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, ua_handle, O_RDONLY);
    ASSERT_GE(ua_fd, 0);
// Close net_ns - user_B should stay active (we hold direct ref)
    ...
    close(net_fd);
    let mut ub_fd2: c_int = open_by_handle_at(FD_NSFS_ROOT, ub_handle, O_RDONLY);
    ASSERT_GE(ub_fd2, 0);
    close(ub_fd2);
// Close user_B - user_A should stay active (we hold direct ref)
    ...
    close(ub_fd);
    let mut ua_fd2: c_int = open_by_handle_at(FD_NSFS_ROOT, ua_handle, O_RDONLY);
    ASSERT_GE(ua_fd2, 0);
    close(ua_fd2);
...
// Create first network namespace
    ...
    let mut n1_fd: c_int = open("/proc/self/ns/net", O_RDONLY);
    if (n1_fd < 0) {
    close(pipefd[1]);
    close(syncpipe[0]);
    exit(1);
...
// Create second network namespace
    ...
    let mut n2_fd: c_int = open("/proc/self/ns/net", O_RDONLY);
    if (n2_fd < 0) {
    close(n1_fd);
    close(pipefd[1]);
    close(syncpipe[0]);
...
// Open both net namespaces while child is still alive
    let mut n1_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, net1_handle, O_RDONLY);
    let mut n2_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, net2_handle, O_RDONLY);
    if (n1_fd < 0 || n2_fd < 0) {
...
// Parent should be active (has 2 active children)
    ...
    let mut p_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, parent_handle, O_RDONLY);
    ASSERT_GE(p_fd, 0);
    close(p_fd);
...
// Open both non-user namespaces
    let mut n_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, net_handle, O_RDONLY);
    let mut ut_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, uts_handle, O_RDONLY);
    if (n_fd < 0 || ut_fd < 0) {
    if (n_fd >= 0) close(n_fd);
    if (ut_fd >= 0) close(ut_fd);
    waitpid(pid, core::ptr::null_mut(), 0);
...
// User namespace should be active (2 active children)
    ...
    let mut u_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, user_handle, O_RDONLY);
    ASSERT_GE(u_fd, 0);
    close(u_fd);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thread_ns_info {
    pub ns_id: __u64,
    pub pipefd: c_int,
    pub syncfd_read: c_int,
    pub syncfd_write: c_int,
    pub exit_code: c_int,
}
...
// Get namespace ID
    let mut fd: c_int = open("/proc/thread-self/ns/net", O_RDONLY);
    if (fd < 0) {
    info.exit_code = 2;
    return core::ptr::null_mut();
    }
...
// Create thread that will create a namespace
    ret = pthread_create(&thread, core::ptr::null_mut(), thread_create_namespace, &info);
    ASSERT_EQ(ret, 0);
...
// Namespace should be active while thread is alive
    ...
    let mut nsfd: c_int = open_by_handle_at(FD_NSFS_ROOT, handle, O_RDONLY);
    ASSERT_GE(nsfd, 0);
    close(nsfd);
...
// Namespace should still be active because we hold an fd
    let mut nsfd2: c_int = open_by_handle_at(FD_NSFS_ROOT, handle, O_RDONLY);
    ASSERT_GE(nsfd2, 0);
// Verify it's the same namespace
    struct stat st1, st2;
    ASSERT_EQ(fstat(nsfd, &st1), 0);
...
// Get user namespace ID
    let mut user_fd: c_int = open("/proc/self/ns/user", O_RDONLY);
    if (user_fd < 0) {
...
// Create two threads that share the namespaces
    ...
    let mut ret_thread: c_int = pthread_create(&thread1, core::ptr::null_mut(), thread_sleep_and_wait, &data);
    if (ret_thread != 0) {
    ...
    ret_thread = pthread_create(&thread2, core::ptr::null_mut(), thread_sleep_and_wait, &data);
    if (ret_thread != 0) {
...
// Wait for threads to complete - they will unblock when parent writes
    ...
    pthread_join(thread1, core::ptr::null_mut());
    fprintf(stderr, "Child: thread1 exited\n");
    pthread_join(thread2, core::ptr::null_mut());
...
// Verify namespaces are active while subprocess and threads are alive
    ...
    let mut user_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, user_handle, O_RDONLY);
    ASSERT_GE(user_fd, 0);
    let mut net_fd: c_int = open_by_handle_at(FD_NSFS_ROOT, net_handle, O_RDONLY);
    ASSERT_GE(net_fd, 0);
    close(user_fd);
