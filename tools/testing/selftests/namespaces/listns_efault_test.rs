...
//! Source: tools/testing/selftests/namespaces/listns_efault_test.c
#![no_std]
...
//
    map = mmap(core::ptr::null_mut(), page_size * 2, PROT_READ | PROT_WRITE,
...
// Child: create a couple of tmpfs mounts
    ...
    _exit(1);
    if (mount("tmpfs", "/tmp/test_mnt1", "tmpfs", 0, core::ptr::null_mut()) == -1)
    _exit(1);
    if (mount("tmpfs", "/tmp/test_mnt2", "tmpfs", 0, core::ptr::null_mut()) == -1)
    _exit(1);
...
// Wait for all mount namespace children to exit and cleanup
    for (i = 0; i < 5; i++) {
    waitpid(ns_pids[i], core::ptr::null_mut(), 0);
    close(sv[i][0]);
    close(pidfds[i]);
    }
// Kill iterator and wait for it
    sys_pidfd_send_signal(iter_pidfd, SIGKILL, core::ptr::null_mut(), 0);
...
// NULL buffer with non-zero count should fail
    ret = sys_listns(&req, core::ptr::null_mut(), 10, 0);
    if (ret == -1 && errno == ENOSYS)
    SKIP(return, "listns() not supported");
// Should fail with EFAULT
    ASSERT_EQ(ret, -1);
