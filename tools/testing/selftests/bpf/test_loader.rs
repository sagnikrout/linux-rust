...
//! Source: tools/testing/selftests/bpf/test_loader.c
#![no_std]
...
// Copyright (c) 2022 Meta Platforms, Inc. and affiliates.
    ...
    {
    let mut len: usize = strlen(pfx);
    return strncmp(str, pfx, len) == 0 ? str + len : core::ptr::null_mut();
    }
pub const TEST_LOADER_LOG_BUF_SZ: c_int = 2097152;
// Warning: duplicated in bpf_misc.h
pub const POINTER_VALUE: c_uint = 0xbadcafe;
pub const TEST_DATA_LEN: c_int = 64;

pub const EFFICIENT_UNALIGNED_ACCESS: c_int = 1;

pub const EFFICIENT_UNALIGNED_ACCESS: c_int = 0;

    let mut sysctl_unpriv_disabled: static int = -1;
    enum mode {
    PRIV = 1,
    UNPRIV = 2
    };
...
#[no_mangle]
unsafe extern "C" fn tester_init(tester: *mut test_loader) -> c_int {
...
pub unsafe extern "C" fn free_msgs(msgs: *mut expected_msgs) {
    ...
    free(msgs.patterns);
    msgs.patterns = core::ptr::null_mut();
    msgs.cnt = 0;
    }
#[no_mangle]
unsafe extern "C" fn free_test_spec(spec: *mut test_spec) {
...
// Deallocate expect_msgs arrays.
    ...
    spec.priv.name = core::ptr::null_mut();
    spec.priv.description = core::ptr::null_mut();
    spec.unpriv.name = core::ptr::null_mut();
    spec.unpriv.description = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn compile_regex(pattern: *const c_char, regex: *mut regex_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn clone_msgs(from: *mut expected_msgs, to: *mut expected_msgs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn push_msg(substr: *const c_char, negative: bool, msgs: *mut expected_msgs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn push_disasm_msg(regex_str: *const c_char, on_next_line: *mut bool, msgs: *mut expected_msgs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parse_int(str: *const c_char, val: *mut c_int, name: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parse_caps(str: *const c_char, val: *mut __u64, name: *const c_char) -> c_int {
    static int parse_caps(const char *str, __u64 *val, const char *name)
    {
    let mut cap_flag: c_int = 0;
    char *token = core::ptr::null_mut(), *saveptr = core::ptr::null_mut();
    char *str_cpy = strdup(str);
    if (str_cpy == core::ptr::null_mut()) {
    PRINT_FAIL("Memory allocation failed\n");
    return -EINVAL;
    }
    token = strtok_r(str_cpy, "|", &saveptr);
    while (token != core::ptr::null_mut()) {
    errno = 0;
    if (!strncmp("CAP_", token, sizeof("CAP_") - 1)) {
    PRINT_FAIL("define %s constant in bpf_misc.h, failed to parse caps\n", token);
    return -EINVAL;
    }
    cap_flag = strtol(token, core::ptr::null_mut(), 10);
    if (!cap_flag || errno) {
    PRINT_FAIL("failed to parse caps %s\n", name);
    return -EINVAL;
    }
// val |= (1ULL << cap_flag);
    token = strtok_r(core::ptr::null_mut(), "|", &saveptr);
    }
    free(str_cpy);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn parse_retval(str: *const c_char, val: *mut c_int, name: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn update_flags(flags: *mut c_int, flag: c_int, clear: bool) {
...
//
    ...
    {
    const char **tmp, **tags = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn get_current_arch() -> c_int {
...
//
    ...
    {
    const char *description = core::ptr::null_mut();
    let mut has_unpriv_result: bool = false;
    let mut has_unpriv_retval: bool = false;
    let mut unpriv_xlated_on_next_line: bool = true;
    let mut xlated_on_next_line: bool = true;
    bool unpriv_jit_on_next_line;
    bool jit_on_next_line;
    let mut stderr_on_next_line: bool = true;
    let mut unpriv_stderr_on_next_line: bool = true;
    let mut stdout_on_next_line: bool = true;
    let mut unpriv_stdout_on_next_line: bool = true;
    let mut collect_jit: bool = false;
    const char **tags = core::ptr::null_mut();
    int func_id, i, nr_tags;
    let mut err: c_int = 0;
    let mut arch_mask: u32 = 0;
    let mut load_mask: u32 = 0;
    struct btf *btf;
    enum arch arch;
    ...
    let mut name_len: c_int = strlen(spec.prog_name);
    const char *suffix = " @unpriv";
    let mut suffix_len: c_int = strlen(suffix);
    char *name;
    ...
    if (description) {
    let mut descr_len: c_int = strlen(description);
    char *descr;
    ...
    struct bpf_program *prog)
    {
    let mut min_log_level: c_int = 0, prog_flags;
...
#[no_mangle]
unsafe extern "C" fn emit_verifier_log(log_buf: *const c_char, force: bool) {
...
#[no_mangle]
unsafe extern "C" fn emit_xlated(xlated: *const c_char, force: bool) {
...
#[no_mangle]
unsafe extern "C" fn emit_jited(jited: *const c_char, force: bool) {
...
#[no_mangle]
unsafe extern "C" fn emit_stderr(stderr: *const c_char, force: bool) {
...
#[no_mangle]
unsafe extern "C" fn verify_stderr(prog_fd: c_int, msgs: *mut expected_msgs) {
    ...
    if (!ASSERT_NEQ(buf, core::ptr::null_mut(), "malloc"))
    return;
...
pub unsafe extern "C" fn verify_test_stderr(obj: *mut bpf_object, prog: *mut bpf_program) {
    ...
    if (parse_test_spec(core::ptr::null_mut(), obj, prog, &spec))
    return;
    verify_stderr(bpf_program__fd(prog), &spec.priv.stderr);
    free_test_spec(&spec);
    }
#[no_mangle]
unsafe extern "C" fn emit_stdout(bpf_stdout: *const c_char, force: bool) {
    ...
    {
    const char *match = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn count_lines(start: *const c_char, end: *const c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn match_positive_msgs(log: *const c_char, msgs: *mut expected_msgs, matches: *mut match) {
...
//
#[no_mangle]
unsafe extern "C" fn match_negative_msgs(log: *const c_char, msgs: *mut expected_msgs, matches: *mut match) {
...
// -1 to account for i++
    ...
    struct match *prev_match = core::ptr::null_mut();
    int i, j;
    ...
    pat_status = "MATCHED   ";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: wrong_line) -> else {
    else if (wrong_line)
    pat_status = "WRONG LINE";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: no_match) -> else {
    else if (no_match)
    pat_status = "EXPECTED  ";
    else
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cap_state {
    pub old_caps: __u64,
    pub initialized: bool,
}

#[no_mangle]
unsafe extern "C" fn drop_capabilities(caps: *mut cap_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn restore_capabilities(caps: *mut cap_state) -> c_int {
    static int restore_capabilities(struct cap_state *caps)
    {
    int err;
    if (!caps.initialized)
    return 0;
    err = cap_enable_effective(caps.old_caps, core::ptr::null_mut());
    if (err)
...
#[no_mangle]
unsafe extern "C" fn can_execute_unpriv(tester: *mut test_loader, spec: *mut test_spec) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_unpriv_capable_map(map: *mut bpf_map) -> bool {
...
#[no_mangle]
unsafe extern "C" fn do_prog_test_run(fd_prog: c_int, retval: *mut c_int, empty_opts: bool, linear_sz: c_int) -> c_int {
    static int do_prog_test_run(int fd_prog, int *retval, bool empty_opts, int linear_sz)
    {
    __u8 tmp_out[TEST_DATA_LEN << 2] = {};
    __u8 tmp_in[TEST_DATA_LEN] = {};
    let mut ctx: __sk_buff = {};
    int err, saved_errno;
...
#[no_mangle]
unsafe extern "C" fn should_do_test_run(spec: *mut test_spec, subspec: *mut test_subspec) -> bool {
...
#[no_mangle]
unsafe extern "C" fn get_xlated_program_text(prog_fd: c_int, text: *mut c_char, text_sz: usize) -> c_int {
    static int get_xlated_program_text(int prog_fd, char *text, size_t text_sz)
    {
    struct bpf_insn *insn_start = core::ptr::null_mut(), *insn, *insn_end;
    let mut insns_cnt: __u32 = 0, i;
    char buf[64];
    FILE *out = core::ptr::null_mut();
    int err;
...
#[no_mangle]
unsafe extern "C" fn get_stream(stream_id: c_int, prog_fd: c_int, text: *mut c_char, text_sz: usize) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn fixup_btf_from_path(obj: *mut bpf_object, path: *const c_char) -> c_int {
    ...
    if (!prog_btf)
    return 0;
    custom_btf = btf__parse(path, core::ptr::null_mut());
...
//
    ...
    __u32 *prog_param_types = core::ptr::null_mut();
    const char *name;
    int new_proto_id;
    if (!btf_is_func(t))
    continue;
...
//
    ...
    let mut current_runtime: c_int = is_jit_enabled() ? JITED : NO_JITED;
    struct bpf_program *tprog = core::ptr::null_mut(), *tprog_iter;
    ...
    if (subspec.caps) {
    err = cap_enable_effective(subspec.caps, core::ptr::null_mut());
    if (err) {
...
// redo bpf_map__attach_struct_ops for each test
    ...
    struct test_spec *specs = core::ptr::null_mut();
