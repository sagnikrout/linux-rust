...
//! Source: tools/testing/selftests/bpf/map_tests/sk_storage_map.c
#![no_std]
...
// Copyright (c) 2019 Facebook
    ...
    let mut nr_sk_per_thread: static unsigned int = 4096;
    let mut nr_sk_threads: static unsigned int = 4;
    let mut sk_storage_map: static int = -1;
    static unsigned int stop;
    let mut runtime_s: static int = 5;
#[no_mangle]
unsafe extern "C" fn is_stopped() -> bool {
    static bool is_stopped(void)
    {
    return READ_ONCE(stop);
    }
#[no_mangle]
unsafe extern "C" fn threads_err() -> c_uint {
    static unsigned int threads_err(void)
    {
    return READ_ONCE(nr_sk_threads_err);
    }
#[no_mangle]
unsafe extern "C" fn notify_thread_err() {
    static void notify_thread_err(void)
    {
    __sync_add_and_fetch(&nr_sk_threads_err, 1);
    }
#[no_mangle]
unsafe extern "C" fn wait_for_threads_err() -> bool {
...
#[no_mangle]
unsafe extern "C" fn wait_for_threads_redo() -> bool {
...
#[no_mangle]
unsafe extern "C" fn wait_for_map() -> bool {
...
#[no_mangle]
unsafe extern "C" fn wait_for_map_close() -> bool {
...
#[no_mangle]
unsafe extern "C" fn do_sk_storage_map_stress_free() -> c_int {
    ...
    if (!sk_thread_ids) {
    fprintf(stderr, "malloc(sk_threads): core::ptr::null_mut()\n");
    return -ENOMEM;
    }
    for (i = 0; i < nr_sk_threads; i++) {
    err = pthread_create(&sk_thread_ids[i], core::ptr::null_mut(),
    insert_close_thread, core::ptr::null_mut());
    if (err) {
    err = -errno;
    goto done;
    }
    ...
    let mut map_fd: c_int = READ_ONCE(sk_storage_map);
    let mut sk_fd: c_int = *(int *)arg;
...
#[no_mangle]
unsafe extern "C" fn do_sk_storage_map_stress_change() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_sk_storage_map_stress_free() {
...
#[no_mangle]
unsafe extern "C" fn test_sk_storage_map_stress_change() {
...
#[no_mangle]
unsafe extern "C" fn test_sk_storage_map_basic() {
