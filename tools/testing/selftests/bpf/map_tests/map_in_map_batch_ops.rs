...
//! Source: tools/testing/selftests/bpf/map_tests/map_in_map_batch_ops.c
#![no_std]
...
pub const OUTER_MAP_ENTRIES: c_int = 10;
#[no_mangle]
unsafe extern "C" fn get_map_id_from_fd(map_fd: c_int) -> __u32 {
    static __u32 get_map_id_from_fd(int map_fd)
    {
    let mut map_info: bpf_map_info = {};
    let mut info_len: u32 = sizeof(map_info);
    int ret;
...
//
    ...
    let mut map_key: __u32 = 0, map_id;
    char map_name[16];
    ...
    sizeof(__u32), 1, core::ptr::null_mut());
    CHECK(map_fd < 0,
...
#[no_mangle]
unsafe extern "C" fn create_outer_map(map_type: enum bpf_map_type, inner_map_fd: __u32) -> c_int {
    ...
    err = bpf_map_get_next_key(inner_map_fd, core::ptr::null_mut(), &inner_map_key);
    CHECK(err != 0,
...
// Actual value validation
    ...
    let mut batch_key: __u32 = 0, fetch_count, step_size = batch_size;
    let mut value_size: __u32 = sizeof(__u32);
...
// hash map may not always return full batch
    ...
    total_fetched ? &batch_key : core::ptr::null_mut(),
    &batch_key,
...
// validate the fetched entries
    ...
    bool has_holes)
    {
    let mut max_entries: __u32 = OUTER_MAP_ENTRIES - !!has_holes;
