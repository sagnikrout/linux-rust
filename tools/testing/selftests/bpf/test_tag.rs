...
//! Source: tools/testing/selftests/bpf/test_tag.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn bpf_gen_imm_prog(insns: c_uint, fd_map: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bpf_gen_map_prog(insns: c_uint, fd_map: c_int) {
...
#[no_mangle]
unsafe extern "C" fn __hex2bin(ch: c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hex2bin(dst: *mut u8, src: *const c_char, count: usize) -> c_int {
    static int hex2bin(uint8_t *dst, const char *src, size_t count)
    {
    while (count--) {
    let mut hi: c_int = __hex2bin(*src++);
    let mut lo: c_int = __hex2bin(*src++);
    if ((hi < 0) || (lo < 0))
    return -1;
// dst++ = (hi << 4) | lo;
    }
...
#[no_mangle]
unsafe extern "C" fn tag_from_fdinfo(fd_prog: c_int, tag: *mut u8, len: u32) {
    static void tag_from_fdinfo(int fd_prog, uint8_t *tag, uint32_t len)
    {
    let mut prefix_len: c_int = sizeof("prog_tag:\t") - 1;
    char buff[256];
    let mut ret: c_int = -1;
    FILE *fp;
...
#[no_mangle]
unsafe extern "C" fn tag_from_alg(insns: c_int, tag: *mut u8, len: u32) {
    ...
    assert(!ret);
    fd_alg = accept(fd_base, core::ptr::null_mut(), 0);
    assert(fd_alg > 0);
...
#[no_mangle]
unsafe extern "C" fn tag_dump(prefix: *const c_char, tag: *mut u8, len: u32) {
...
// Use libbpf 1.0 API mode
    ...
    fd_map = bpf_map_create(BPF_MAP_TYPE_HASH, core::ptr::null_mut(), sizeof(int),
    sizeof(int), 1, &opts);
    assert(fd_map > 0);
    for (i = 0; i < 5; i++) {
