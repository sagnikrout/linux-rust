...
//! Source: tools/testing/selftests/bpf/prog_tests/lsm_bdev.c
#![no_std]
...
pub struct verity_info {
    pub has_roothash: __u8,
    pub sig_valid: __u8,
    pub setintegrity_cnt: __u32,
}

pub const DATA_SIZE_MB: c_int = 8;
pub const HASH_SIZE_MB: c_int = 1;

// Run a command and optionally capture the first line of stdout.
#[no_mangle]
unsafe extern "C" fn run_cmd(cmd: *const c_char, out: *mut c_char, out_sz: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn has_prerequisites() -> bool {
    ...
    if (run_cmd("modprobe loop 2>/dev/null", core::ptr::null_mut(), 0) &&
    run_cmd("ls /dev/loop-control 2>/dev/null", core::ptr::null_mut(), 0)) {
    printf("SKIP: no loop device support\n");
    return false;
    }
    if (run_cmd("modprobe dm-verity 2>/dev/null", core::ptr::null_mut(), 0) &&
    run_cmd("dmsetup targets 2>/dev/null | grep -q verity", core::ptr::null_mut(), 0)) {
    printf("SKIP: dm-verity module not available\n");
    return false;
    }
    if (run_cmd("which veritysetup >/dev/null 2>&1", core::ptr::null_mut(), 0)) {
...
pub unsafe extern "C" fn test_lsm_bdev() {
    ...
    char roothash[256] = {};
    char cmd[512];
    let mut data_fd: c_int = -1, hash_fd = -1;
    struct lsm_bdev *skel = core::ptr::null_mut();
...
// Clean up any stale device from a previous crashed run.
    ...
    run_cmd(cmd, core::ptr::null_mut(), 0);
...
// Activate dm-verity — triggers verity_preresume() hooks.
    ...
    if (!ASSERT_OK(run_cmd(cmd, core::ptr::null_mut(), 0), "veritysetup open"))
    goto teardown;
