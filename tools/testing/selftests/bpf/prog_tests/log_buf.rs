...
//! Source: tools/testing/selftests/bpf/prog_tests/log_buf.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn libbpf_print_cb(level: enum libbpf_print_level, fmt: *const c_char, args: va_list) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn obj_load_log_buf() {
    static void obj_load_log_buf(void)
    {
    let mut old_print_cb: libbpf_print_fn_t = libbpf_set_print(libbpf_print_cb);
    LIBBPF_OPTS(bpf_object_open_opts, opts);
    let mut log_buf_sz: usize = 1024 * 1024;
...
#[no_mangle]
unsafe extern "C" fn bpf_prog_load_log_buf() {
    ...
    let mut good_prog_insn_cnt: usize = ARRAY_SIZE(good_prog_insns);
    const struct bpf_insn bad_prog_insns[] = {
    BPF_EXIT_INSN(),
    };
    let mut bad_prog_insn_cnt: usize = ARRAY_SIZE(bad_prog_insns);
...
#[no_mangle]
unsafe extern "C" fn bpf_btf_load_log_buf() {
