...
//! Source: tools/testing/selftests/bpf/prog_tests/hashmap.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn hash_fn(k: c_long, ctx: *mut c_void) -> usize {
    static size_t hash_fn(long k, void *ctx)
    {
    return k;
    }
#[no_mangle]
unsafe extern "C" fn equal_fn(a: c_long, b: c_long, ctx: *mut c_void) -> bool {
    static bool equal_fn(long a, long b, void *ctx)
    {
    let mut a: return = = b;
    }
#[no_mangle]
pub unsafe extern "C" fn next_pow_2(n: usize) -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn exp_cap(sz: usize) -> usize {
    static inline size_t exp_cap(size_t sz)
    {
    let mut r: usize = next_pow_2(sz);
    if (sz * 4 / 3 > r)
    r <<= 1;
    return r;
    }
pub const ELEM_CNT: c_int = 62;
#[no_mangle]
unsafe extern "C" fn test_hashmap_generic() {
    ...
    long long found_msk;
    struct hashmap *map;
    map = hashmap__new(hash_fn, equal_fn, core::ptr::null_mut());
    ...
    let mut k: c_long = entry.key;
    let mut v: c_long = entry.value;
    found_msk |= 1ULL << k;
...
#[no_mangle]
unsafe extern "C" fn str_hash_fn(a: c_long, ctx: *mut c_void) -> usize {
...
#[no_mangle]
unsafe extern "C" fn test_hashmap_ptr_iface() {
    ...
    struct hashmap *map;
    int err, i, bkt;
    map = hashmap__new(str_hash_fn, str_equal_fn, core::ptr::null_mut());
    if (CHECK(!map, "hashmap__new", "can't allocate hashmap\n"))
    goto cleanup;

    CHECK(strcmp(var, (expected)), (fn),				\
    "wrong value of " #var ": '%s' instead of '%s'\n", var, (expected))
    err = hashmap__insert(map, "a", "apricot", HASHMAP_ADD, core::ptr::null_mut(), core::ptr::null_mut());
    ...
    if (CHECK(!hashmap__delete(map, "b", core::ptr::null_mut(), core::ptr::null_mut()),
...
#[no_mangle]
unsafe extern "C" fn collision_hash_fn(k: c_long, ctx: *mut c_void) -> usize {
    static size_t collision_hash_fn(long k, void *ctx)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn test_hashmap_multimap() {
    static void test_hashmap_multimap(void)
    {
    let mut k1: c_long = 0, k2 = 1;
    struct hashmap_entry *entry;
    struct hashmap *map;
    long found_msk;
    int err, bkt;
// force collisions
    map = hashmap__new(collision_hash_fn, equal_fn, core::ptr::null_mut());
    if (!ASSERT_OK_PTR(map, "hashmap__new"))
    return;
// set up multimap:
// [0] -> 1, 2, 4;
...
#[no_mangle]
unsafe extern "C" fn test_hashmap_empty() {
...
// force collisions
    ...
    goto cleanup;
    if (CHECK(hashmap__find(map, k, core::ptr::null_mut()), "elem_find",
    "unexpected find\n"))
    goto cleanup;
    if (CHECK(hashmap__delete(map, k, core::ptr::null_mut(), core::ptr::null_mut()), "elem_del",
    "unexpected delete\n"))
    goto cleanup;
    hashmap__for_each_entry(map, entry, bkt) {
