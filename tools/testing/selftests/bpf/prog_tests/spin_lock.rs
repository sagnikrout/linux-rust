...
//! Source: tools/testing/selftests/bpf/prog_tests/spin_lock.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn match_regex(pattern: *const c_char, string: *const c_char) -> c_int {
    ...
    rc = regexec(&re, string, 0, core::ptr::null_mut(), 0);
    regfree(&re);
    let mut rc: return = = 0 ? 1 : 0;
    }
#[no_mangle]
unsafe extern "C" fn test_spin_lock_fail_prog(prog_name: *const c_char, err_msg: *const c_char) {
...
pub unsafe extern "C" fn test_spin_lock_success() {
    ...
    for (i = 0; i < 4; i++) {
    int err;
    err = pthread_create(&thread_id[i], core::ptr::null_mut(), &spin_lock_thread, &prog_fd);
