...
//! Source: tools/testing/selftests/bpf/prog_tests/xdp_devmap_attach.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn test_xdp_with_devmap_helpers() {
    static void test_xdp_with_devmap_helpers(void)
    {
    struct test_xdp_with_devmap_helpers *skel = core::ptr::null_mut();
    ...
    err = bpf_xdp_attach(IFINDEX_LO, dm_fd_redir, XDP_FLAGS_SKB_MODE, core::ptr::null_mut());
...
// can not attach BPF_XDP_DEVMAP program to a device
    err = bpf_xdp_attach(IFINDEX_LO, dm_fd, XDP_FLAGS_SKB_MODE, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn test_xdp_with_devmap_frags_helpers() {
...
#[no_mangle]
unsafe extern "C" fn test_xdp_with_devmap_helpers_veth() {
    ...
    goto out_close;
    dm_fd_redir = bpf_program__fd(skel.progs.xdp_redir_prog);
    err = bpf_xdp_attach(val.ifindex, dm_fd_redir, XDP_FLAGS_DRV_MODE, core::ptr::null_mut());
...
// attach dummy to other side to enable reception
    ...
    err = bpf_xdp_attach(ifindex_dst, dm_fd, XDP_FLAGS_DRV_MODE, core::ptr::null_mut());
...
// wait for the packets to be flushed
    kern_sync_rcu();
    err = bpf_xdp_detach(val.ifindex, XDP_FLAGS_DRV_MODE, core::ptr::null_mut());
    ASSERT_OK(err, "XDP program detach");
    err = bpf_xdp_detach(ifindex_dst, XDP_FLAGS_DRV_MODE, core::ptr::null_mut());
    ASSERT_OK(err, "XDP program detach");
    out_close:
    close_netns(nstoken);
    SYS_NOFAIL("ip netns del %s", TEST_NS);
