...
pub const NPACKETS: c_int = 100;
    let mut root_netns_fd: static int = -1;
#[no_mangle]
unsafe extern "C" fn restore_root_netns() {
    static void restore_root_netns(void)
    {
    ASSERT_OK(setns(root_netns_fd, CLONE_NEWNET), "restore_root_netns");
    }
#[no_mangle]
unsafe extern "C" fn setns_by_name(name: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_rx_packets(iface: *const c_char) -> c_int {
    static int get_rx_packets(const char *iface)
    {
    FILE *f;
    char line[512];
    let mut iface_len: c_int = strlen(iface);
...
pub const MAX_BPF_LINKS: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct skeletons {
    pub xdp_dummy: *mut xdp_dummy,
    pub xdp_tx: *mut xdp_tx,
    pub xdp_redirect_multi_kern: *mut xdp_redirect_multi_kern,
    pub nlinks: c_int,
    pub links: [*mut bpf_link; MAX_BPF_LINKS],
}

#[no_mangle]
unsafe extern "C" fn xdp_attach(skeletons: *mut skeletons, prog: *mut bpf_program, iface: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn link_cleanup(skeletons: *mut skeletons) {
...
#[no_mangle]
unsafe extern "C" fn bonding_cleanup(skeletons: *mut skeletons) {
...
#[no_mangle]
unsafe extern "C" fn send_udp_packets(vary_dst_ip: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_xdp_bonding_with_mode(skeletons: *mut skeletons, mode: c_int, xmit_policy: c_int) {
    ...
    case BOND_MODE_XOR: {
    let mut veth1_rx: c_int = get_rx_packets("veth1_1");
    let mut veth2_rx: c_int = get_rx_packets("veth1_2");
    let mut diff: c_int = abs(veth1_rx - veth2_rx);
...
//
#[no_mangle]
unsafe extern "C" fn test_xdp_bonding_redirect_multi(skeletons: *mut skeletons) {
...
// populate the devmap with the relevant interfaces
    ...
    let mut ifindex: c_int = if_nametoindex(ifaces[i]);
    let mut map_fd: c_int = bpf_map__fd(skeletons.xdp_redirect_multi_kern.maps.map_all);
...
#[no_mangle]
unsafe extern "C" fn test_xdp_bonding_attach(skeletons: *mut skeletons) {
    static void test_xdp_bonding_attach(struct skeletons *skeletons)
    {
    struct bpf_link *link = core::ptr::null_mut();
    struct bpf_link *link2 = core::ptr::null_mut();
    int veth, bond, err;
...
#[no_mangle]
unsafe extern "C" fn test_xdp_bonding_nested(skeletons: *mut skeletons) {
...
//
#[no_mangle]
unsafe extern "C" fn test_xdp_bonding_redirect_no_up(skeletons: *mut skeletons) {
...
//
    ...
    XDP_FLAGS_SKB_MODE, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn test_xdp_bonding_features(skeletons: *mut skeletons) {
...
//
#[no_mangle]
unsafe extern "C" fn test_xdp_bonding_xmit_policy_compat(skeletons: *mut skeletons) {
...
// 802.3ad with layer2+3 policy: native XDP is supported
    ...
    goto out;
    err = bpf_xdp_attach(bond_ifindex, xdp_fd, XDP_FLAGS_DRV_MODE, core::ptr::null_mut());
...
// Detach XDP first, then the same change must succeed
    ASSERT_OK(bpf_xdp_detach(bond_ifindex, XDP_FLAGS_DRV_MODE, core::ptr::null_mut()),
    ...
    out:
    if (bond_ifindex > 0)
    bpf_xdp_detach(bond_ifindex, XDP_FLAGS_DRV_MODE, core::ptr::null_mut());
    close_netns(nstoken);
