...
//! Source: tools/testing/selftests/bpf/prog_tests/task_kfunc.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn run_syscall_success_test(prog_name: *const c_char) {
...
unsafe extern "C" fn run_vpid_test(prog_name: *mut c_void) -> c_int {
    ...
    if (bpf_prog_test_run_opts(prog_fd, core::ptr::null_mut())) {
    err = 6;
    goto cleanup;
    }
    if (skel.bss.err)
...
#[no_mangle]
unsafe extern "C" fn run_vpid_success_test(prog_name: *const c_char) {
    static void run_vpid_success_test(const char *prog_name)
    {
    let mut stack_size: c_int = 1024 * 1024;
