...
//! Source: tools/testing/selftests/bpf/prog_tests/user_ringbuf.c
#![no_std]
...
// Macro flag: #define _GNU_SOURCE
    ...
    let mut c_sample_size: static long = sizeof(struct sample) + BPF_RINGBUF_HDR_SZ;
    static long c_ringbuf_size, c_max_entries;
#[no_mangle]
unsafe extern "C" fn drain_current_samples() {
    static void drain_current_samples(void)
    {
    syscall(__NR_getpgid);
    }
#[no_mangle]
unsafe extern "C" fn write_samples(ringbuf: *mut user_ring_buffer, num_samples: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_mappings() {
...
// cons_pos can be mapped R/O, can't add +X with mprotect.
    mmap_ptr = mmap(core::ptr::null_mut(), page_size, PROT_READ, MAP_SHARED, rb_fd, 0);
...
// prod_pos can be mapped RW, can't add +X with mprotect.
    mmap_ptr = mmap(core::ptr::null_mut(), page_size, PROT_READ | PROT_WRITE, MAP_SHARED,
    rb_fd, page_size);
...
// data pages can be mapped RW, can't add +X with mprotect.
    mmap_ptr = mmap(core::ptr::null_mut(), page_size, PROT_WRITE, MAP_SHARED, rb_fd,
    2 * page_size);
    ...
    {
    struct user_ringbuf_success *skel;
    struct ring_buffer *kern_ringbuf = core::ptr::null_mut();
    struct user_ring_buffer *user_ringbuf = core::ptr::null_mut();
    let mut err: c_int = -ENOMEM, rb_fd;
    skel = open_load_ringbuf_skel();
    if (!skel)
    return err;
// only trigger BPF program for current process
    ...
    kern_ringbuf = ring_buffer__new(rb_fd, callback, skel, core::ptr::null_mut());
...
// kern_ringbuf_out = kern_ringbuf;
    ...
    if (user_ringbuf_out) {
    rb_fd = bpf_map__fd(skel.maps.user_ringbuf);
    user_ringbuf = user_ring_buffer__new(rb_fd, core::ptr::null_mut());
...
// user_ringbuf_out = NULL;
    ...
    {
    return load_skel_create_ringbufs(skel_out, core::ptr::null_mut(), core::ptr::null_mut(), ringbuf_out);
    }
...
// Map the producer_pos as RW.
    producer_pos_ptr = mmap(core::ptr::null_mut(), page_size, PROT_READ | PROT_WRITE,
    MAP_SHARED, rb_fd, page_size);
    ASSERT_OK_PTR(producer_pos_ptr, "producer_pos_ptr");
// Map the data pages as RW.
    data_ptr = mmap(core::ptr::null_mut(), page_size, PROT_WRITE, MAP_SHARED, rb_fd, 2 * page_size);
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_post_producer_wrong_offset() {
    static void test_user_ringbuf_post_producer_wrong_offset(void)
    {
    struct user_ringbuf_success *skel;
    struct user_ring_buffer *ringbuf;
    int err;
    let mut size: __u32 = (1 << 5);
    err = load_skel_create_user_ringbuf(&skel, &ringbuf);
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_post_larger_than_ringbuf_sz() {
    static void test_user_ringbuf_post_larger_than_ringbuf_sz(void)
    {
    struct user_ringbuf_success *skel;
    struct user_ring_buffer *ringbuf;
    int err;
    let mut size: __u32 = c_ringbuf_size;
    err = load_skel_create_user_ringbuf(&skel, &ringbuf);
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_basic() {
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_sample_full_ring_buffer() {
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_post_alignment_autoadjust() {
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_overfill() {
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_discards_properly_ignored() {
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_loop() {
    static void test_user_ringbuf_loop(void)
    {
    struct user_ringbuf_success *skel;
    struct user_ring_buffer *ringbuf;
    let mut total_samples: u32 = 8192;
...
#[no_mangle]
unsafe extern "C" fn kick_kernel_read_messages() {
    static void kick_kernel_read_messages(void)
    {
    syscall(__NR_prctl);
    }
#[no_mangle]
unsafe extern "C" fn handle_kernel_msg(ctx: *mut c_void, data: *mut c_void, len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_msg_protocol() {
    ...
    return;
    for (i = 0; i < 64; i++) {
    let mut op: enum test_msg_op = i % TEST_MSG_OP_NUM_OPS;
    let mut operand_64: __u64 = TEST_OP_64;
    let mut operand_32: __u32 = TEST_OP_32;
...
#[no_mangle]
unsafe extern "C" fn spawn_kick_thread_for_poll() -> c_int {
    static int spawn_kick_thread_for_poll(void)
    {
    pthread_t thread;
    return pthread_create(&thread, core::ptr::null_mut(), kick_kernel_cb, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn test_user_ringbuf_blocking_reserve() {
...
// We correctly time out after 1 second, without a sample.
    ...
    if (!ASSERT_EQ(token, core::ptr::null_mut(), "pre_kick_timeout_token"))
    goto cleanup;
