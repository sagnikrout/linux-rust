...
//! Source: tools/testing/selftests/bpf/prog_tests/sockmap_strp.c
#![no_std]
...
pub const STRP_PKT_HEAD_LEN: c_int = 4;
pub const STRP_PKT_BODY_LEN: c_int = 6;

    static const char packet[STRP_PKT_FULL_LEN] = "head+body\0";
    let mut test_packet_num: static int = 100;
...
//
#[no_mangle]
unsafe extern "C" fn sockmap_strp_consume_pre_data(p: c_int) -> c_int {
...
//
    ...
    bool need_parser)
    {
    struct test_sockmap_strp *strp = core::ptr::null_mut();
    int verdict, parser;
    int err;
...
//
#[no_mangle]
unsafe extern "C" fn test_sockmap_strp_dispatch_pkt(family: c_int, sotype: c_int) {
...
// deliver data with data size greater than 5 to local
    ...
    let mut expected: c_int = send_dir[i].data_len;
...
//
#[no_mangle]
unsafe extern "C" fn test_sockmap_strp_multiple_pkt(family: c_int, sotype: c_int) {
    static void test_sockmap_strp_multiple_pkt(int family, int sotype)
    {
    int i, zero = 0;
    int sent, recvd, total;
    int err, map;
    let mut c: c_int = -1, p = -1;
    struct test_sockmap_strp *strp = core::ptr::null_mut();
    char *snd = core::ptr::null_mut(), *rcv = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn test_sockmap_strp_partial_read(family: c_int, sotype: c_int) {
    static void test_sockmap_strp_partial_read(int family, int sotype)
    {
    let mut zero: c_int = 0, recvd, off;
    int err, map;
    let mut c: c_int = -1, p = -1;
    struct test_sockmap_strp *strp = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn test_sockmap_strp_pass(family: c_int, sotype: c_int, fionread: bool) {
    static void test_sockmap_strp_pass(int family, int sotype, bool fionread)
    {
    let mut zero: c_int = 0, pkt_size = STRP_PKT_FULL_LEN, sent, recvd, avail;
    int err, map;
    let mut c: c_int = -1, p = -1;
    let mut test_cnt: c_int = 10, i;
    struct test_sockmap_strp *strp = core::ptr::null_mut();
    char rcv[STRP_PKT_FULL_LEN + 1] = "0";
    strp = sockmap_strp_init(&map, true, true);
    if (!ASSERT_TRUE(strp, "sockmap_strp_init"))
...
#[no_mangle]
unsafe extern "C" fn test_sockmap_strp_verdict(family: c_int, sotype: c_int) {
    static void test_sockmap_strp_verdict(int family, int sotype)
    {
    let mut zero: c_int = 0, one = 1, sent, recvd, off;
    int err, map;
    let mut c0: c_int = -1, p0 = -1, c1 = -1, p1 = -1;
    struct test_sockmap_strp *strp = core::ptr::null_mut();
    char rcv[STRP_PKT_FULL_LEN + 1] = "0";
...
#[no_mangle]
unsafe extern "C" fn test_sockmap_strp_parser_reject() {
    ...
    link = bpf_link_create(parser_ro, map, BPF_SK_SKB_STREAM_PARSER, core::ptr::null_mut());
    if (!ASSERT_GE(link, 0, "bpf_link_create parser_ro"))
    goto out;
    err = bpf_link_update(link, parser_mod, core::ptr::null_mut());
