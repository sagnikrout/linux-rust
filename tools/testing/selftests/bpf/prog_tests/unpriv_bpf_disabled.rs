...
//! Source: tools/testing/selftests/bpf/prog_tests/unpriv_bpf_disabled.c
#![no_std]
...
pub const NUM_MAPS: c_int = 7;
    static __u32 got_perfbuf_val;
    static __u32 got_ringbuf_val;
#[no_mangle]
unsafe extern "C" fn process_ringbuf(ctx: *mut c_void, data: *mut c_void, len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn process_perfbuf(ctx: *mut c_void, cpu: c_int, data: *mut c_void, len: __u32) {
    ...
    {
    struct perf_buffer *perfbuf = core::ptr::null_mut();
...
// - create a link
//
    perfbuf = perf_buffer__new(bpf_map__fd(skel.maps.perfbuf), 8, process_perfbuf, core::ptr::null_mut(), core::ptr::null_mut(),
    core::ptr::null_mut());
    if (!ASSERT_OK_PTR(perfbuf, "perf_buffer__new"))
    goto cleanup;
    ringbuf = ring_buffer__new(bpf_map__fd(skel.maps.ringbuf), process_ringbuf, core::ptr::null_mut(), core::ptr::null_mut());
...
// trigger & validate perf event, ringbuf output
    ...
    for (i = 0; i < NUM_MAPS; i++) {
    let mut prog_array: bool = strstr(map_paths[i], "prog_array") != core::ptr::null_mut();
    let mut array: bool = strstr(map_paths[i], "array") != core::ptr::null_mut();
    let mut buf: bool = strstr(map_paths[i], "buf") != core::ptr::null_mut();
    let mut key: __u32 = 0, vals[nr_cpus], lookup_vals[nr_cpus];
...
// prog array lookup returns prog id, not fd
    ...
    let mut map_info_len: __u32 = sizeof(map_info);
    let mut link_info: bpf_link_info = {};
...
//
    ...
    ASSERT_EQ(bpf_map_create(i, core::ptr::null_mut(), sizeof(int), sizeof(int), 1, core::ptr::null_mut()),
    ...
    ASSERT_EQ(bpf_btf_load(raw_btf_data, raw_btf_size, core::ptr::null_mut()), -EPERM,
    "bpf_btf_load_fails");
    }
    btf__free(btf);
    }
...
// if unprivileged_bpf_disabled=1, we get -EPERM back; that's okay.
    ...
    sysctl_set("/proc/sys/kernel/perf_event_paranoid", core::ptr::null_mut(), perf_event_paranoid_orig);
    if (strlen(unprivileged_bpf_disabled_orig) > 0)
    sysctl_set("/proc/sys/kernel/unprivileged_bpf_disabled", core::ptr::null_mut(),
