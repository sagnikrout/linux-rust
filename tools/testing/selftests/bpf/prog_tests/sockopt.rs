...
//! Source: tools/testing/selftests/bpf/prog_tests/sockopt.c
#![no_std]
...
//
    ...
    if (!ASSERT_NEQ(sqe, core::ptr::null_mut(), "Get an SQE")) {
    err = -1;
    goto fail;
    }
...
// Execute the getsocktopt operation
    ...
    bool use_link)
    {
    let mut prog_type: c_int = BPF_PROG_TYPE_CGROUP_SOCKOPT;
    int sock_fd, err, prog_fd, link_fd = -1;
    void *optval = core::ptr::null_mut();
    let mut ret: c_int = 0;
    if (test.prog_type)
    ...
    if (use_link) {
    err = bpf_link_create(prog_fd, cgroup_fd, test.attach_type, core::ptr::null_mut());
    link_fd = err;
    } else {
    ...
    let mut num_pages: c_int = test.set_optlen / PAGE_SIZE;
    let mut remainder: c_int = test.set_optlen % PAGE_SIZE;
    ...
    let mut num_pages: c_int = test.get_optlen / PAGE_SIZE;
    let mut remainder: c_int = test.get_optlen % PAGE_SIZE;
