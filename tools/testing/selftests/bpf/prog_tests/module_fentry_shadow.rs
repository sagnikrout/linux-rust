...
//! Source: tools/testing/selftests/bpf/prog_tests/module_fentry_shadow.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn get_bpf_testmod_btf_fd() -> c_int {
    static int get_bpf_testmod_btf_fd(void)
    {
    struct bpf_btf_info info;
    char name[64];
    let mut id: __u32 = 0, len;
    int err, fd;
    while (true) {
...
pub unsafe extern "C" fn test_module_fentry_shadow() {
    ...
    {
    struct btf *vmlinux_btf = core::ptr::null_mut(), *mod_btf = core::ptr::null_mut();
    int err, i;
    int btf_fd[2] = {};
    int prog_fd[2] = {};
    int link_fd[2] = {};
    ...
    prog_fd[i] = bpf_prog_load(BPF_PROG_TYPE_TRACING, core::ptr::null_mut(), "GPL",
    trace_program,
...
// which is forbidden.
//
    link_fd[i] = bpf_link_create(prog_fd[i], 0, BPF_TRACE_FENTRY, core::ptr::null_mut());
    if (!ASSERT_GE(link_fd[i], 0, "bpf_link_create"))
    goto out;
    }
    err = bpf_prog_test_run_opts(prog_fd[0], core::ptr::null_mut());
