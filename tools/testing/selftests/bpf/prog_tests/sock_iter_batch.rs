...
//! Source: tools/testing/selftests/bpf/prog_tests/sock_iter_batch.c
#![no_std]
...
// Copyright (c) 2024 Meta
    ...
    let mut init_batch_size: static int = 16;
    let mut nr_soreuse: static int = 4;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iter_out {
    pub idx: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sock_count {
    pub cookie: __u64,
    pub count: c_int,
}

#[no_mangle]
unsafe extern "C" fn insert(cookie: __u64, counts[]: sock_count, counts_len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn was_seen(fd: c_int, counts[]: sock_count, counts_len: c_int) -> bool {
    static bool was_seen(int fd, struct sock_count counts[], int counts_len)
    {
    let mut cookie: __u64 = socket_cookie(fd);
    let mut i: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn get_seen_socket(fds: *mut c_int, counts[]: sock_count, n: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_nth_socket(fds: *mut c_int, fds_len: c_int, link: *mut bpf_link, n: c_int) -> c_int {
    static int get_nth_socket(int *fds, int fds_len, struct bpf_link *link, int n)
    {
    int i, nread, iter_fd;
    let mut nth_sock_idx: c_int = -1;
    struct iter_out out;
...
#[no_mangle]
unsafe extern "C" fn destroy(fd: c_int) {
    static void destroy(int fd)
    {
    struct sock_iter_batch *skel = core::ptr::null_mut();
    let mut cookie: __u64 = socket_cookie(fd);
    ...
    goto done;
    link = bpf_program__attach_iter(skel.progs.iter_tcp_destroy, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn get_seen_count(fd: c_int, counts[]: sock_count, n: c_int) -> c_int {
...
//
    ...
    return accept(server_poll_fds[i].fd, core::ptr::null_mut(), core::ptr::null_mut());
    return -1;
    }
    ...
    int server_fds_len)
    {
    struct pollfd *server_poll_fds = core::ptr::null_mut();
    int *established_socks = core::ptr::null_mut();
    int i;
...
// Make sure there are no more sockets returned
    ...
    struct bpf_link *link, int iter_fd)
    {
    int *close_idx = core::ptr::null_mut();
    int i;
// Iterate through all listening sockets.
    read_n(iter_fd, listen_socks_len, counts, counts_len);
// Make sure we saw all listening sockets exactly once.
...
// Make sure there are no more sockets returned
    ...
    int counts_len, struct bpf_link *link, int iter_fd)
    {
    int *new_socks = core::ptr::null_mut();
// Iterate through the first socks_len - 1 sockets.
    read_n(iter_fd, socks_len - 1, counts, counts_len);
// Make sure we saw socks_len - 1 sockets exactly once.
    check_n_were_seen_once(socks, socks_len, socks_len - 1, counts,
...
pub struct test_case {
    ...
    int *socks, int socks_len, int *established_socks,
    int established_socks_len, struct sock_count *counts,
    pub iter_fd): *mut *mut int counts_len, struct bpf_link link, int,
    pub description: *const c_char,
    pub ehash_buckets: c_int,
    pub connections: c_int,
    pub init_socks: c_int,
    pub max_socks: c_int,
    pub sock_type: c_int,
    pub family: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn do_resume_test(tc: *mut test_case) {
    static void do_resume_test(struct test_case *tc)
    {
    struct sock_iter_batch *skel = core::ptr::null_mut();
    struct sock_count *counts = core::ptr::null_mut();
    let mut port: static __u16 = 10001;
    struct nstoken *nstoken = core::ptr::null_mut();
    struct bpf_link *link = core::ptr::null_mut();
    int *established_fds = core::ptr::null_mut();
    int err, iter_fd = -1;
    const char *addr;
...
#[no_mangle]
unsafe extern "C" fn do_resume_tests() {
...
#[no_mangle]
unsafe extern "C" fn do_test(sock_type: c_int, onebyone: bool) {
...
//
    ...
    fds[first_idx] = core::ptr::null_mut();
