...
//! Source: tools/testing/selftests/bpf/prog_tests/sock_create.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn run_test(cgroup_fd: c_int, test: *mut sock_create_test) -> c_int {
    static int run_test(int cgroup_fd, struct sock_create_test *test)
    {
    int sock_fd, err, prog_fd, optval, ret = -1;
    let mut optlen: socklen_t = sizeof(optval);
