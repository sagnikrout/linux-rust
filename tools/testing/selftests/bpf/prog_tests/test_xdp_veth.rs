...
//! Source: tools/testing/selftests/bpf/prog_tests/test_xdp_veth.c
#![no_std]
...
pub const VETH_PAIRS_COUNT: c_int = 3;
pub const VETH_NAME_MAX_LEN: c_int = 32;
pub const IP_MAX_LEN: c_int = 16;

pub const PROG_NAME_MAX_LEN: c_int = 128;
pub const NS_NAME_MAX_LEN: c_int = 32;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct veth_configuration {
    pub /: *mut *mut char local_veth[VETH_NAME_MAX_LEN]; / Interface in main namespace,
    pub namespace*/: *mut *mut char remote_veth[VETH_NAME_MAX_LEN]; / Peer interface in dedicated,
    pub /: *mut *mut char namespace[NS_NAME_MAX_LEN]; / Namespace for the remote veth,
    pub /: *mut *mut int next_veth; / Local interface to redirect traffic to,
    pub /: *mut *mut char remote_addr[IP_MAX_LEN]; / IP address of the remote veth,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct net_configuration {
    pub ns0_name: [c_char; NS_NAME_MAX_LEN],
    pub veth_cfg: [veth_configuration; VETH_PAIRS_COUNT],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct prog_configuration {
    pub /: *mut *mut char local_name[PROG_NAME_MAX_LEN]; / BPF prog to attach to local_veth,
    pub /: *mut *mut char remote_name[PROG_NAME_MAX_LEN]; / BPF prog to attach to remote_veth,
    pub /: *mut *mut u32 local_flags; / XDP flags to use on local_veth,
    pub /: *mut *mut u32 remote_flags; / XDP flags to use on remote_veth,
}
    ...
    prog[index].local_flags, core::ptr::null_mut());
    ...
    prog[index].remote_flags, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn create_network(net_config: *mut net_configuration) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cleanup_network(net_config: *mut net_configuration) {
...
pub const VETH_REDIRECT_SKEL_NB: c_int = 3;
#[no_mangle]
unsafe extern "C" fn xdp_veth_redirect(flags: u32) {
...
// Then configure the redirect map and attach programs to interfaces
    ...
    goto destroy_xdp_redirect_map;
    for (i = 0; i < VETH_PAIRS_COUNT; i++) {
    let mut next_veth: c_int = net_config.veth_cfg[i].next_veth;
    int interface_id;
    int err;
...
pub const BROADCAST_REDIRECT_SKEL_NB: c_int = 2;
#[no_mangle]
unsafe extern "C" fn xdp_veth_broadcast_redirect(attach_flags: u32, redirect_flags: u64) {
    ...
    struct xdp_redirect_map *xdp_redirect_map;
    let mut devmap_val: bpf_devmap_val = {};
    struct net_configuration net_config;
    struct nstoken *nstoken = core::ptr::null_mut();
    let mut protocol: u16 = ETH_P_IP;
    int group_map;
    int flags_map;
    int cnt_map;
    let mut cnt: u64 = 0;
    ...
    goto destroy_xdp_redirect_map;
    for (i = 0; i < VETH_PAIRS_COUNT; i++) {
    let mut ifindex: c_int = if_nametoindex(net_config.veth_cfg[i].local_veth);
...
pub const VETH_EGRESS_SKEL_NB: c_int = 3;
#[no_mangle]
unsafe extern "C" fn xdp_veth_egress(flags: u32) {
...
#[no_mangle]
unsafe extern "C" fn xdp_veth_egress_last_dst(flags: u32) {
    ...
    struct bpf_object *bpf_objs[VETH_EGRESS_SKEL_NB];
    struct xdp_redirect_map *xdp_redirect_map;
    let mut net_config: net_configuration = {};
    int mac_map, egress_map, res_map;
    struct nstoken *nstoken = core::ptr::null_mut();
    struct xdp_dummy *xdp_dummy;
