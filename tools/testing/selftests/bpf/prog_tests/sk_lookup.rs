...
//! Source: tools/testing/selftests/bpf/prog_tests/sk_lookup.c
#![no_std]
...
pub const EXT_PORT: c_int = 7007;
// Internal (address, port) pairs the server listens/receives at.

pub const INT_PORT: c_int = 8008;
    enum server {
    SERVER_A = 0,
    SERVER_B = 1,
    MAX_SERVERS,
...
pub struct test {
    ...
    pub lookup_prog: *mut bpf_program,
    pub reuseport_prog: *mut bpf_program,
    pub sock_map: *mut bpf_map,
    pub sotype: c_int,
    pub connect_to: inet_addr,
    pub listen_at: inet_addr,
    pub accept_on: enum server,
    pub /: *mut *mut bool reuseport_has_conns; / Add a connected socket to reuseport group,
}
...
#[no_mangle]
unsafe extern "C" fn is_ipv6(ip: *const c_char) -> bool {
    static bool is_ipv6(const char *ip)
    {
    return !!strchr(ip, ':');
    }
#[no_mangle]
unsafe extern "C" fn attach_reuseport(sock_fd: c_int, reuseport_prog: *mut bpf_program) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn socket_cookie(fd: c_int) -> __u64 {
    static __u64 socket_cookie(int fd)
    {
    __u64 cookie;
    let mut cookie_len: socklen_t = sizeof(cookie);
...
#[no_mangle]
unsafe extern "C" fn send_byte(fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_byte(fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tcp_recv_send(server_fd: c_int) -> c_int {
    static int tcp_recv_send(int server_fd)
    {
    char buf[1];
    int ret, fd;
    ssize_t n;
    fd = accept(server_fd, core::ptr::null_mut(), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn v4_to_v6(ss: *mut sockaddr_storage) {
    static void v4_to_v6(struct sockaddr_storage *ss)
    {
    struct sockaddr_in6 *v6 = (struct sockaddr_in6 *)ss;
    let mut v4: sockaddr_in = *(struct sockaddr_in *)ss;
...
#[no_mangle]
unsafe extern "C" fn udp_recv_send(server_fd: c_int) -> c_int {
    static int udp_recv_send(int server_fd)
    {
    char cmsg_buf[CMSG_SPACE(sizeof(struct sockaddr_storage))];
    let mut _src_addr: sockaddr_storage = { 0 };
    struct sockaddr_storage *src_addr = &_src_addr;
    struct sockaddr_storage *dst_addr = core::ptr::null_mut();
...
// Reply from original destination address.
    fd = start_server_addr(SOCK_DGRAM, dst_addr, sizeof(*dst_addr), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn tcp_echo_test(client_fd: c_int, server_fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn udp_echo_test(client_fd: c_int, server_fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn update_lookup_map(map: *mut bpf_map, index: c_int, sock_fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn query_lookup_prog(skel: *mut test_sk_lookup) {
...
#[no_mangle]
unsafe extern "C" fn run_lookup_prog(t: *const test) {
...
// Connect the extra socket to itself
    ...
    t.sotype, t.connect_to.ip, t.connect_to.port, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn test_redirect_lookup(skel: *mut test_sk_lookup) {
...
#[no_mangle]
unsafe extern "C" fn drop_on_lookup(t: *const test) {
    static void drop_on_lookup(const struct test *t)
    {
    let mut family: c_int = is_ipv6(t.connect_to.ip) ? AF_INET6 : AF_INET;
    ...
    if (server_fd < 0)
    goto detach;
    client_fd = client_socket(family, t.sotype, core::ptr::null_mut());
...
// Read out asynchronous error
    n = recv(client_fd, core::ptr::null_mut(), 0, 0);
    err = n == -1;
    }
...
#[no_mangle]
unsafe extern "C" fn test_drop_on_lookup(skel: *mut test_sk_lookup) {
...
#[no_mangle]
unsafe extern "C" fn drop_on_reuseport(t: *const test) {
    static void drop_on_reuseport(const struct test *t)
    {
    let mut family: c_int = is_ipv6(t.connect_to.ip) ? AF_INET6 : AF_INET;
    let mut dst: sockaddr_storage = { 0 };
...
// second server on destination address we should never reach
    ...
    core::ptr::null_mut() /* reuseport prog */);
    if (server2 < 0)
    goto close_srv1;
    client = client_socket(family, t.sotype, core::ptr::null_mut());
    ...
    goto close_all;
// Read out asynchronous error
    n = recv(client, core::ptr::null_mut(), 0, 0);
    err = n == -1;
    }
    if (CHECK(!err || errno != ECONNREFUSED, "connect",
    "unexpected success or error\n"))
...
#[no_mangle]
unsafe extern "C" fn test_drop_on_reuseport(skel: *mut test_sk_lookup) {
    ...
    for (i = 0; i < ARRAY_SIZE(server_fds); i++) {
    server_fds[i] = make_server(SOCK_STREAM, local_ip, 0, core::ptr::null_mut());
    if (server_fds[i] < 0)
    goto close_servers;
    ...
    struct bpf_link *lookup_link;
    server_fd = make_server(sotype, EXT_IP4, EXT_PORT, core::ptr::null_mut());
    if (server_fd < 0)
    return;
    connected_fd = connect_to_addr_str(AF_INET, sotype, EXT_IP4, EXT_PORT, core::ptr::null_mut());
...
// Try to redirect TCP SYN / UDP packet to a connected socket
    client_fd = connect_to_addr_str(AF_INET, sotype, EXT_IP4, EXT_PORT, core::ptr::null_mut());
    if (!ASSERT_OK_FD(client_fd, "connect_to_addr_str"))
...
#[no_mangle]
unsafe extern "C" fn test_sk_assign_helper(skel: *mut test_sk_lookup) {
    ...
    if (test__start_subtest("sk_assign accepts core::ptr::null_mut() socket"))
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_multi_prog {
    pub desc: *const c_char,
    pub prog1: *mut bpf_program,
    pub prog2: *mut bpf_program,
    pub redir_map: *mut bpf_map,
    pub run_map: *mut bpf_map,
    pub expect_errno: c_int,
    pub listen_at: inet_addr,
}

#[no_mangle]
unsafe extern "C" fn run_multi_prog_lookup(t: *const test_multi_prog) {
    ...
    t.listen_at.port, core::ptr::null_mut());
    if (server_fd < 0)
    goto out_unlink2;
    err = update_lookup_map(t.redir_map, SERVER_A, server_fd);
    if (err)
    goto out_close_server;
    client_fd = client_socket(AF_INET, SOCK_STREAM, core::ptr::null_mut());
    if (!ASSERT_OK_FD(client_fd, "client_socket"))
    goto out_close_server;
...
#[no_mangle]
unsafe extern "C" fn test_multi_prog_lookup(skel: *mut test_sk_lookup) {
...
#[no_mangle]
unsafe extern "C" fn run_tests(skel: *mut test_sk_lookup) {
...
#[no_mangle]
unsafe extern "C" fn switch_netns() -> c_int {
