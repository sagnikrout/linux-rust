...
//! Source: tools/testing/selftests/bpf/prog_tests/global_data_init.c
#![no_std]
...
pub unsafe extern "C" fn test_global_data_init() {
    ...
    let mut err: c_int = -ENOMEM, map_fd, zero = 0;
    __u8 *buff = core::ptr::null_mut(), *newval = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn test_percpu_data_on_cpus(map: *mut bpf_map, map_fd: c_int, prog_fd: c_int, runp: *mut c_int) {
    static void test_percpu_data_on_cpus(struct bpf_map *map, int map_fd, int prog_fd, int *runp)
    {
    struct test_global_percpu_data__percpu *data = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn test_global_percpu_data_init() {
    static void test_global_percpu_data_init(void)
    {
    let mut init_value: test_global_percpu_data__percpu = {};
    struct test_global_percpu_data__percpu *init_data;
    let mut desired_sz: __u32 = sysconf(_SC_PAGE_SIZE);
    struct test_global_percpu_data *skel = core::ptr::null_mut();
    size_t init_data_sz;
    struct bpf_map *map;
    int prog_fd, err;
...
#[no_mangle]
unsafe extern "C" fn test_global_percpu_data_lskel() {
    static void test_global_percpu_data_lskel(void)
    {
    struct test_global_percpu_data_lskel *lskel = core::ptr::null_mut();
    int prog_fd, map_fd;
    ...
    test_percpu_data_on_cpus(core::ptr::null_mut(), map_fd, prog_fd, &lskel.bss.run);
    out:
    test_global_percpu_data_lskel__destroy(lskel);
    }
#[no_mangle]
unsafe extern "C" fn create_rdonly_percpu_array() -> c_int {
    static int create_rdonly_percpu_array(void)
    {
    LIBBPF_OPTS(bpf_map_create_opts, map_opts,
    .map_flags = BPF_F_RDONLY_PROG,
    );
    let mut key: c_int = 0, map_fd, err;
...
#[no_mangle]
unsafe extern "C" fn test_global_percpu_data_rdonly_direct_read() {
...
//
    ...
    ARRAY_SIZE(insns), core::ptr::null_mut());
    if (ASSERT_GE(prog_fd, 0, "bpf_prog_load"))
    close(prog_fd);
    close(map_fd);
    }
#[no_mangle]
unsafe extern "C" fn test_global_percpu_data_rdonly_direct_write() {
...
#[no_mangle]
unsafe extern "C" fn test_global_percpu_data_verifier_log() {
    static void test_global_percpu_data_verifier_log(void)
    {
    RUN_TESTS(test_global_percpu_data);
    }
#[no_mangle]
unsafe extern "C" fn test_global_percpu_data_iter() {
...
pub unsafe extern "C" fn test_global_percpu_data() {
    ...
    {
    if (!feat_supported(core::ptr::null_mut(), FEAT_PERCPU_DATA)) {
    test__skip();
    return;
    }
