...
//! Source: tools/testing/selftests/bpf/prog_tests/cgroup_link.c
#![no_std]
...
// SPDX-License-Identifier: GPL-2.0
    ...
    let mut duration: static __u32 = 0;

    static struct test_cgroup_link *skel = core::ptr::null_mut();
#[no_mangle]
pub unsafe extern "C" fn ping_and_check(exp_calls: c_int, exp_alt_calls: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn serial_test_cgroup_link() {
    ...
    let mut last_cg: c_int = ARRAY_SIZE(cgs) - 1, cg_nr = ARRAY_SIZE(cgs);
...
// query the number of attached progs and attach flags in root cg
    ...
    0, &attach_flags, core::ptr::null_mut(), &prog_cnt);
    CHECK_FAIL(err);
...
// query the number of effective progs in last cg
    ...
    BPF_F_QUERY_EFFECTIVE, core::ptr::null_mut(), core::ptr::null_mut(),
    &prog_cnt);
    CHECK_FAIL(err);
...
// query the effective prog IDs in last cg
    err = bpf_prog_query(cgs[last_cg].fd, BPF_CGROUP_INET_EGRESS,
    BPF_F_QUERY_EFFECTIVE, core::ptr::null_mut(), prog_ids,
    &prog_cnt);
    CHECK_FAIL(err);
    if (CHECK(prog_cnt != cg_nr, "effect_cnt", "exp %d, got %d\n",
    cg_nr, prog_cnt))
...
// detach bottom program and ping again
    ...
    links[last_cg] = core::ptr::null_mut();
