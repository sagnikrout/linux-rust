...
//! Source: tools/testing/selftests/bpf/prog_tests/check_mtu.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn read_mtu_device_lo() -> c_int {
    ...
    if (n == -1)
    return -2;
    value = strtoimax(buf, core::ptr::null_mut(), 10);
    if (errno == ERANGE)
    return -3;
    return value;
    }
#[no_mangle]
unsafe extern "C" fn test_check_mtu_xdp_attach() {
    ...
    __u32 mtu_expect)
    {
    let mut retval_expect: c_int = XDP_PASS;
    let mut mtu_result: __u32 = 0;
    char buf[256] = {};
...
#[no_mangle]
unsafe extern "C" fn test_check_mtu_xdp(mtu: __u32, ifindex: __u32) {
...
// Update "constants" in BPF-prog *BEFORE* libbpf load
    ...
    __u32 mtu_expect)
    {
    let mut retval_expect: c_int = BPF_OK;
    let mut mtu_result: __u32 = 0;
    char buf[256] = {};
    int err, prog_fd = bpf_program__fd(prog);
    LIBBPF_OPTS(bpf_test_run_opts, topts,
...
#[no_mangle]
unsafe extern "C" fn test_chk_segs_flag(skel: *mut test_check_mtu, mtu: __u32) {
...
#[no_mangle]
unsafe extern "C" fn test_check_mtu_tc(mtu: __u32, ifindex: __u32) {
