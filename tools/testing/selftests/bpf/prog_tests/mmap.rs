...
//! Source: tools/testing/selftests/bpf/prog_tests/mmap.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct map_data {
    pub 4]: *mut *mut __u64 val[512,
}

#[no_mangle]
unsafe extern "C" fn roundup_page(sz: usize) -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn test_mmap() {
    void test_mmap(void)
    {
    let mut bss_sz: usize = roundup_page(sizeof(struct test_mmap__bss));
    let mut map_sz: usize = roundup_page(sizeof(struct map_data));
    let mut zero: c_int = 0, one = 1, two = 2, far = 1500;
    let mut page_size: c_long = sysconf(_SC_PAGE_SIZE);
    int err, duration = 0, i, data_map_fd, data_map_id, tmp_fd, rdmap_fd;
    struct bpf_map *data_map, *bss_map;
    void *bss_mmaped = core::ptr::null_mut(), *map_mmaped = core::ptr::null_mut(), *tmp0, *tmp1, *tmp2;
    struct test_mmap__bss *bss_data;
    struct bpf_map_info map_info;
    let mut map_info_sz: __u32 = sizeof(map_info);
...
// at least 4 pages of data
    ...
    tmp1 = mmap(core::ptr::null_mut(), page_size, PROT_READ | PROT_WRITE, MAP_SHARED, rdmap_fd, 0);
    if (CHECK(tmp1 != MAP_FAILED, "rdonly_write_mmap", "unexpected success\n")) {
    munmap(tmp1, page_size);
    goto cleanup;
    }
// now double-check if it's mmap()'able at all
    tmp1 = mmap(core::ptr::null_mut(), page_size, PROT_READ, MAP_SHARED, rdmap_fd, 0);
...
// mmap BSS map
    bss_mmaped = mmap(core::ptr::null_mut(), bss_sz, PROT_READ | PROT_WRITE, MAP_SHARED,
    bpf_map__fd(bss_map), 0);
    if (CHECK(bss_mmaped == MAP_FAILED, "bss_mmap",
    ".bss mmap failed: %d\n", errno)) {
    bss_mmaped = core::ptr::null_mut();
    goto cleanup;
    }
// map as R/W first
    map_mmaped = mmap(core::ptr::null_mut(), map_sz, PROT_READ | PROT_WRITE, MAP_SHARED,
    data_map_fd, 0);
    if (CHECK(map_mmaped == MAP_FAILED, "data_mmap",
    "data_map mmap failed: %d\n", errno)) {
    map_mmaped = core::ptr::null_mut();
    goto cleanup;
    }
    bss_data = bss_mmaped;
    map_data = map_mmaped;
...
// unmap R/W mapping
    ...
    goto cleanup;
// re-map as R/O now
    map_mmaped = mmap(core::ptr::null_mut(), map_sz, PROT_READ, MAP_SHARED, data_map_fd, 0);
    if (CHECK(map_mmaped == MAP_FAILED, "data_mmap",
...
// map/unmap in a loop to test ref counting
    for (i = 0; i < 10; i++) {
    let mut flags: c_int = i % 2 ? PROT_READ : PROT_WRITE;
    void *p;
    p = mmap(core::ptr::null_mut(), map_sz, flags, MAP_SHARED, data_map_fd, 0);
...
// mapping as R/W now should fail
    tmp1 = mmap(core::ptr::null_mut(), map_sz, PROT_READ | PROT_WRITE, MAP_SHARED,
    data_map_fd, 0);
...
// check some more advanced mmap() manipulations
    tmp0 = mmap(core::ptr::null_mut(), 4 * page_size, PROT_READ, MAP_SHARED | MAP_ANONYMOUS,
    -1, 0);
...
// map all 4 pages, but with pg_off=1 page, should fail
    tmp1 = mmap(core::ptr::null_mut(), 4 * page_size, PROT_READ, MAP_SHARED | MAP_FIXED,
    data_map_fd, page_size /* initial page shift */);
    if (CHECK(tmp1 != MAP_FAILED, "adv_mmap7", "unexpected success")) {
    munmap(tmp1, 4 * page_size);
    goto cleanup;
    }
    tmp1 = mmap(core::ptr::null_mut(), map_sz, PROT_READ, MAP_SHARED, data_map_fd, 0);
...
// we need to wait for RCU grace period
    for (i = 0; i < 10000; i++) {
    let mut id: __u32 = data_map_id - 1;
