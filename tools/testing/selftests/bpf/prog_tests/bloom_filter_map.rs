...
//! Source: tools/testing/selftests/bpf/prog_tests/bloom_filter_map.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn test_fail_cases() {
    ...
    int fd, err;
// Invalid key size
    fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, core::ptr::null_mut(), 4, sizeof(value), 100, core::ptr::null_mut());
    if (!ASSERT_LT(fd, 0, "bpf_map_create bloom filter invalid key size"))
    close(fd);
// Invalid value size
    fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, core::ptr::null_mut(), 0, 0, 100, core::ptr::null_mut());
    if (!ASSERT_LT(fd, 0, "bpf_map_create bloom filter invalid value size 0"))
    close(fd);
// Invalid value size: too big
    fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, core::ptr::null_mut(), 0, INT32_MAX, 100, core::ptr::null_mut());
    if (!ASSERT_LT(fd, 0, "bpf_map_create bloom filter invalid value too large"))
    close(fd);
// Invalid max entries size
    fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, core::ptr::null_mut(), 0, sizeof(value), 0, core::ptr::null_mut());
    if (!ASSERT_LT(fd, 0, "bpf_map_create bloom filter invalid max entries size"))
    close(fd);
// Bloom filter maps do not support BPF_F_NO_PREALLOC
    opts.map_flags = BPF_F_NO_PREALLOC;
    fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, core::ptr::null_mut(), 0, sizeof(value), 100, &opts);
    if (!ASSERT_LT(fd, 0, "bpf_map_create bloom filter invalid flags"))
    close(fd);
    fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, core::ptr::null_mut(), 0, sizeof(value), 100, core::ptr::null_mut());
    if (!ASSERT_GE(fd, 0, "bpf_map_create bloom filter"))
    return;
// Test invalid flags
    err = bpf_map_update_elem(fd, core::ptr::null_mut(), &value, -1);
    ASSERT_EQ(err, -EINVAL, "bpf_map_update_elem bloom filter invalid flags");
    err = bpf_map_update_elem(fd, core::ptr::null_mut(), &value, BPF_EXIST);
    ASSERT_EQ(err, -EINVAL, "bpf_map_update_elem bloom filter invalid flags");
    err = bpf_map_update_elem(fd, core::ptr::null_mut(), &value, BPF_F_LOCK);
    ASSERT_EQ(err, -EINVAL, "bpf_map_update_elem bloom filter invalid flags");
    err = bpf_map_update_elem(fd, core::ptr::null_mut(), &value, BPF_NOEXIST);
    ASSERT_EQ(err, -EINVAL, "bpf_map_update_elem bloom filter invalid flags");
    err = bpf_map_update_elem(fd, core::ptr::null_mut(), &value, 10000);
    ASSERT_EQ(err, -EINVAL, "bpf_map_update_elem bloom filter invalid flags");
    close(fd);
    }
#[no_mangle]
unsafe extern "C" fn test_success_cases() {
...
// Add a value to the bloom filter
    err = bpf_map_update_elem(fd, core::ptr::null_mut(), &value, 0);
    if (!ASSERT_OK(err, "bpf_map_update_elem bloom filter success case"))
    goto done;
// Lookup a value in the bloom filter
    err = bpf_map_lookup_elem(fd, core::ptr::null_mut(), &value);
    ASSERT_OK(err, "bpf_map_update_elem bloom filter success case");
    done:
    close(fd);
    }
#[no_mangle]
unsafe extern "C" fn check_bloom(skel: *mut bloom_filter_map) {
...
// Create a bloom filter map that will be used as the inner map
    inner_map_fd = bpf_map_create(BPF_MAP_TYPE_BLOOM_FILTER, core::ptr::null_mut(), 0, sizeof(*rand_vals),
    nr_rand_vals, core::ptr::null_mut());
    if (!ASSERT_GE(inner_map_fd, 0, "bpf_map_create bloom filter inner map"))
    return;
    for (i = 0; i < nr_rand_vals; i++) {
    err = bpf_map_update_elem(inner_map_fd, core::ptr::null_mut(), rand_vals + i, BPF_ANY);
...
// Ensure the inner bloom filter map can be deleted
    ...
    __u32 *rand_vals = core::ptr::null_mut();
    __u32 map_size, val;
    int err, i;
...
// Generate random values and populate both skeletons
    ...
    goto error;
    err = bpf_map_update_elem(bloom_fd, core::ptr::null_mut(), &val, BPF_ANY);
...
pub unsafe extern "C" fn test_bloom_filter_map() {
    ...
    {
    __u32 *rand_vals = core::ptr::null_mut(), nr_rand_vals = 0;
    struct bloom_filter_map *skel = core::ptr::null_mut();
    int err;
    test_fail_cases();
    test_success_cases();
