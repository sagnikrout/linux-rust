...
//! Source: tools/testing/selftests/bpf/prog_tests/sockopt_sk.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn getsetsockopt() -> c_int {
    ...
    socklen_t optlen;
    char *big_buf = core::ptr::null_mut();
...
// optval=NULL case is handled correctly
    ...
    optlen = 0;
    err = getsockopt(fd, SOL_NETLINK, NETLINK_LIST_MEMBERSHIPS, core::ptr::null_mut(), &optlen);
    if (err) {
