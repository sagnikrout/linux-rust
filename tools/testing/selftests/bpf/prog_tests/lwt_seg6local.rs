...
//! Source: tools/testing/selftests/bpf/prog_tests/lwt_seg6local.c
#![no_std]
...
unsafe extern "C" fn setup() -> c_int {
    ...
    for (ns = 1; ns < 6; ns++) {
    let mut local_id: c_int = ns * 2 - 1;
    let mut peer_id: c_int = ns * 2;
    let mut next_ns: c_int = ns + 1;
...
pub const SERVER_PORT: c_int = 7330;
pub const CLIENT_PORT: c_int = 2121;
#[no_mangle]
pub unsafe extern "C" fn test_lwt_seg6local() {
    void test_lwt_seg6local(void)
    {
    let mut server_addr: sockaddr_in6 = {};
    ...
    goto out;
    sfd = start_server_str(AF_INET6, SOCK_DGRAM, "fb00::6", SERVER_PORT, core::ptr::null_mut());
    ...
    goto close_server;
    cfd = start_server_str(AF_INET6, SOCK_DGRAM, "fb00::1", CLIENT_PORT, core::ptr::null_mut());
