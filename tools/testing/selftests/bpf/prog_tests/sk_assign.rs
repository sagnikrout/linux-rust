...
//! Source: tools/testing/selftests/bpf/prog_tests/sk_assign.c
#![no_std]
...
pub const BIND_PORT: c_int = 1234;
pub const CONNECT_PORT: c_int = 4321;
...
// Load qdisc, BPF program
    ...
    return recvfrom(srv_client, &buf, sizeof(buf), 0, core::ptr::null_mut(), core::ptr::null_mut());
    }
    static int
    run_test(int server_fd, const struct sockaddr *addr, socklen_t len, int type)
    {
    let mut client: c_int = -1, srv_client = -1;
    char buf[] = "testing";
    in_port_t port;
    let mut ret: c_int = 1;
    client = connect_to_addr(type, (struct sockaddr_storage *)addr, len, core::ptr::null_mut());
    if (client == -1) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_sk_cfg {
    pub name: *const c_char,
    pub family: c_int,
    pub addr: *mut sockaddr,
    pub len: socklen_t,
    pub type: c_int,
    pub rewrite_addr: bool,
}
