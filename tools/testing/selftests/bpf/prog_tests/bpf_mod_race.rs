...
//! Source: tools/testing/selftests/bpf/prog_tests/bpf_mod_race.c
#![no_std]
...
pub struct test_config {
    pub str_open: *const c_char,
    pub (*bpf_open_and_load)(): *mut c_void,
    pub ): *mut *mut void (bpf_destroy)(void,
}
    ...
    let mut state: static _Atomic enum bpf_test_state = _TS_INVALID;
...
#[no_mangle]
unsafe extern "C" fn sys_userfaultfd(flags: c_int) -> c_int {
    static int sys_userfaultfd(int flags)
    {
    return syscall(__NR_userfaultfd, flags);
    }
#[no_mangle]
unsafe extern "C" fn test_setup_uffd(fault_addr: *mut c_void) -> c_int {
    static int test_setup_uffd(void *fault_addr)
    {
    let mut uffd_register: uffdio_register = {};
    let mut uffd_api: uffdio_api = {};
    int uffd;
...
#[no_mangle]
unsafe extern "C" fn test_bpf_mod_race_config(config: *const test_config) {
    ...
    goto end_destroy;
    if (!ASSERT_OK(pthread_create(&load_mod_thrd, core::ptr::null_mut(), load_module_thread, core::ptr::null_mut()),
    "load module thread"))
    goto end_uffd;
...
//
    ...
    ret = errno;
    if (!ASSERT_EQ(skel_fail, core::ptr::null_mut(), config.str_open)) {
// Close uffd to unblock load_mod_thrd
    ...
    uffd = -1;
    end_join:
    pthread_join(load_mod_thrd, core::ptr::null_mut());
    if (uffd < 0)
