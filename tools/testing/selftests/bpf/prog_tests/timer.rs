...
//! Source: tools/testing/selftests/bpf/prog_tests/timer.c
#![no_std]
...
pub const NUM_THR: c_int = 8;
#[no_mangle]
unsafe extern "C" fn perf_event_open(type: __u32, config: __u64, pid: c_int, cpu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn timer_stress_runner(timer_skel: *mut timer, async_cancel: bool) -> c_int {
    ...
    for (i = 0; i < NUM_THR; i++) {
    err = pthread_create(&thread_id[i], core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn timer_stress(timer_skel: *mut timer) -> c_int {
    static int timer_stress(struct timer *timer_skel)
    {
    return timer_stress_runner(timer_skel, false);
    }
#[no_mangle]
unsafe extern "C" fn timer_stress_async_cancel(timer_skel: *mut timer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn run_nmi_test(timer_skel: *mut timer, prog: *mut bpf_program) -> c_int {
    static int run_nmi_test(struct timer *timer_skel, struct bpf_program *prog)
    {
    struct bpf_link *link = core::ptr::null_mut();
    let mut pe_fd: c_int = -1, pipefd[2] = {-1, -1}, pid = 0, status;
...
// Child: spawn multiple threads to consume multiple CPUs
    ...
    close(pipefd[0]);
    for (i = 0; i < NUM_THR; i++)
    pthread_create(&threads[i], core::ptr::null_mut(), nmi_cpu_worker, core::ptr::null_mut());
    for (i = 0; i < NUM_THR; i++)
...
#[no_mangle]
unsafe extern "C" fn timer_stress_nmi_race(timer_skel: *mut timer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn timer_stress_nmi_update(timer_skel: *mut timer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn timer_stress_nmi_cancel(timer_skel: *mut timer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn timer(timer_skel: *mut timer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn timer_cancel_async(timer_skel: *mut timer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_timer(timer_skel): *mut *mut int (timer_test_fn)(struct timer) {
    static void test_timer(int (*timer_test_fn)(struct timer *timer_skel))
    {
    struct timer *timer_skel = core::ptr::null_mut();
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn serial_test_timer() {
