...
//! Source: tools/testing/selftests/bpf/prog_tests/global_map_resize.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn run_prog_bss_array_sum() {
    static void run_prog_bss_array_sum(void)
    {
    (void)syscall(__NR_getpid);
    }
#[no_mangle]
unsafe extern "C" fn run_prog_data_array_sum() {
    static void run_prog_data_array_sum(void)
    {
    (void)syscall(__NR_getuid);
    }
#[no_mangle]
unsafe extern "C" fn global_map_resize_bss_subtest() {
    static void global_map_resize_bss_subtest(void)
    {
    int err;
    struct test_global_map_resize *skel;
    struct bpf_map *map;
    let mut desired_sz: __u32 = sizeof(skel.bss.sum) + sysconf(_SC_PAGE_SIZE) * 2;
...
#[no_mangle]
unsafe extern "C" fn global_map_resize_data_subtest() {
    static void global_map_resize_data_subtest(void)
    {
    struct test_global_map_resize *skel;
    struct bpf_map *map;
    let mut desired_sz: __u32 = sysconf(_SC_PAGE_SIZE) * 2;
    size_t array_len, actual_sz, new_sz;
    int err;
    skel = test_global_map_resize__open();
    if (!ASSERT_OK_PTR(skel, "test_global_map_resize__open"))
...
#[no_mangle]
unsafe extern "C" fn global_map_resize_invalid_subtest() {
