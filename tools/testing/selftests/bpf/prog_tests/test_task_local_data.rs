...
//! Source: tools/testing/selftests/bpf/prog_tests/test_task_local_data.c
#![no_std]
...
// Macro flag: #define TLD_FREE_DATA_ON_THREAD_EXIT
...
//
#[no_mangle]
unsafe extern "C" fn reset_tld(dyn_data_size: __u16) {
...
pub const TEST_BASIC_THREAD_NUM: c_int = 32;
...
#[no_mangle]
unsafe extern "C" fn test_task_local_data_basic() {
    ...
    ASSERT_OK(pthread_mutex_init(&global_mutex, core::ptr::null_mut()), "pthread_mutex_init");
...
// Access TLDs from multiple threads and check if they are thread-specific
    ...
    err = pthread_create(&thread[i], core::ptr::null_mut(), test_task_local_data_basic_thread, skel);
    ...
    {
    let mut err: c_int = 0, id = (intptr_t)arg;
    char key_name[32];
    tld_key_t key;
    key = tld_create_key("value_not_exist", TLD_PAGE_SIZE + 1);
...
#[no_mangle]
unsafe extern "C" fn test_task_local_data_race() {
...
//
    err = pthread_create(&thread[i], core::ptr::null_mut(), test_task_local_data_race_thread,
    (void *)(intptr_t)(i + 3));
    if (CHECK_FAIL(err))
    break;
    }
...
#[no_mangle]
unsafe extern "C" fn test_task_local_data_dyn_size(dyn_data_size: __u16) {
