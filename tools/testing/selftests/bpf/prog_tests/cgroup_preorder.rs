...
//! Source: tools/testing/selftests/bpf/prog_tests/cgroup_preorder.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn run_getsockopt_test(cg_parent: c_int, cg_child: c_int, sock_fd: c_int, all_preorder: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn run_link_replace_test(cgroup_fd: c_int, sock_fd: c_int) -> c_int {
    static int run_link_replace_test(int cgroup_fd, int sock_fd)
    {
    LIBBPF_OPTS(bpf_link_create_opts, create_opts);
    let mut err: c_int = 0, normal_link = -1, preorder_link = -1;
    struct cgroup_preorder *skel = core::ptr::null_mut();
...
// Replace the normal link's program child(1) -> parent(3).
    err = bpf_link_update(normal_link, bpf_program__fd(skel.progs.parent), core::ptr::null_mut());
...
pub unsafe extern "C" fn test_cgroup_preorder() {
    ...
    {
    let mut cg_parent: c_int = -1, cg_child = -1, sock_fd = -1;
