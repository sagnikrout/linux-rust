...
//! Source: tools/testing/selftests/bpf/prog_tests/ctx_rewrite.c
#![no_std]
...
// This is a corner case in filter.c:bpf_convert_ctx_access()
    ...
    {
    char *next;
    let mut snum: c_int = strtol(str, &next, 10);
    if (next - str == 0 || num != snum)
    return core::ptr::null_mut();
    return next;
    }
#[no_mangle]
unsafe extern "C" fn find_field_offset_aux(btf: *mut btf, btf_id: c_int, field_name: *mut c_char, off: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn find_field_offset(btf: *mut btf, pattern: *mut c_char, matches: *mut regmatch_t) -> c_int {
    static int find_field_offset(struct btf *btf, char *pattern, regmatch_t *matches)
    {
    let mut type_sz: c_int = matches[1].rm_eo - matches[1].rm_so;
    let mut field_sz: c_int = matches[2].rm_eo - matches[2].rm_so;
...
#[no_mangle]
unsafe extern "C" fn free_regex(re: *mut regex_t) {
...
#[no_mangle]
unsafe extern "C" fn max_line_len(str: *mut c_char) -> u32 {
...
//
    ...
    let mut middle: c_int = max_line_len(text) + 2;
...
//
#[no_mangle]
unsafe extern "C" fn match_pattern(btf: *mut btf, pattern: *mut c_char, text: *mut c_char, reg_map[][2]: *mut c_char) -> bool {
...
pub struct prog_info {
    pub prog_kind: *mut c_char,
    pub prog_type: enum bpf_prog_type,
    pub expected_attach_type: enum bpf_attach_type,
    pub prog: *mut bpf_insn,
    pub prog_len: u32,
}
    ...
    {
    struct bpf_insn *buf = core::ptr::null_mut(), *insn, *insn_end;
    let mut err: c_int = 0, prog_fd = 0;
    FILE *prog_out = core::ptr::null_mut();
    char insn_buf[64];
    char *text = core::ptr::null_mut();
    let mut cnt: __u32 = 0;
...
// TODO: log level
    ...
    prog_fd = bpf_prog_load(pinfo.prog_type, core::ptr::null_mut(), "GPL",
...
#[no_mangle]
unsafe extern "C" fn run_one_testcase(btf: *mut btf, test: *mut test_case) {
    static void run_one_testcase(struct btf *btf, struct test_case *test)
    {
    let mut pinfo: prog_info = {};
    int bpf_sz;
