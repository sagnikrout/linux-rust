...
//! Source: tools/testing/selftests/bpf/prog_tests/libarena.c
#![no_std]
...
// Max suffix is ceil((lg 2^32) / (lg 10)) + sizeof("__") = 10 + 2 = 12.
    ...
    {
    pthread_t *threads = core::ptr::null_mut(), *tmp_threads;
    char tdname[MAX_PARTEST_NAME];
...
// Bump the alloc array to accommodate the new thread.
    ...
    threads = tmp_threads;
    ret = pthread_create(&threads[nthreads], core::ptr::null_mut(),
