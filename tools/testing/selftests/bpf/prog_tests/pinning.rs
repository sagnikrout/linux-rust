...
//! Source: tools/testing/selftests/bpf/prog_tests/pinning.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn get_map_id(obj: *mut bpf_object, name: *const c_char) -> __u32 {
    ...
    if (CHECK(!map, "find map", "core::ptr::null_mut() map"))
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn test_pinning() {
...
// check that opening fails with invalid pinning value in map def
    obj = bpf_object__open_file(file_invalid, core::ptr::null_mut());
...
// should be no-op to re-pin same map
    ...
    if (CHECK(!map, "find map", "core::ptr::null_mut() map"))
    goto out;
    err = bpf_map__pin(map, core::ptr::null_mut());
...
// unpin maps with a pin_path set
    err = bpf_object__unpin_maps(obj, core::ptr::null_mut());
    if (CHECK(err, "unpin maps", "err %d errno %d\n", err, errno))
    goto out;
// and re-pin them...
    err = bpf_object__pin_maps(obj, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(bpf_map__name(map), _arg: "nopinmap2")) -> else {
...
// test pinning at custom path with reuse fd
    ...
    map_fd = bpf_map_create(BPF_MAP_TYPE_ARRAY, core::ptr::null_mut(), sizeof(__u32),
    sizeof(__u64), 1, core::ptr::null_mut());
