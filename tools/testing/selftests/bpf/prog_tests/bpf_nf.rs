...
//! Source: tools/testing/selftests/bpf/prog_tests/bpf_nf.c
#![no_std]
...
pub const CT_OPTS_ERROR_GUARD: c_uint = 0x12345678;
    ...
    { "lookup_null_bpf_tuple", "Possibly core::ptr::null_mut() pointer passed to trusted R2" },
    { "lookup_null_bpf_opts", "Possibly core::ptr::null_mut() pointer passed to trusted R4" },
    { "xdp_lookup_null_bpf_tuple", "Possibly core::ptr::null_mut() pointer passed to trusted R2" },
    { "xdp_lookup_null_bpf_opts", "Possibly core::ptr::null_mut() pointer passed to trusted R4" },
    };
    enum {
    TEST_XDP,
    TEST_TC_BPF,
...
#[no_mangle]
unsafe extern "C" fn connect_to_server(srv_fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_bpf_nf_ct(mode: c_int) {
    static void test_bpf_nf_ct(int mode)
    {
    const char *iptables = "iptables-legacy -t raw %s PREROUTING -j CONNMARK --set-mark 42/0";
    let mut srv_fd: c_int = -1, client_fd = -1, srv_client_fd = -1;
    let mut peer_addr: sockaddr_in = {};
...
#[no_mangle]
unsafe extern "C" fn test_bpf_nf_ct_fail(prog_name: *const c_char, err_msg: *const c_char) {
