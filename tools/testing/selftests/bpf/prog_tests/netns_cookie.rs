...
//! Source: tools/testing/selftests/bpf/prog_tests/netns_cookie.c
#![no_std]
...
pub const loopback: c_int = 1;
    static int duration;
#[no_mangle]
pub unsafe extern "C" fn test_netns_cookie() {
    void test_netns_cookie(void)
    {
    LIBBPF_OPTS(bpf_prog_attach_opts, opta);
    LIBBPF_OPTS(bpf_prog_detach_opts, optd);
    let mut server_fd: c_int = -1, client_fd = -1, cgroup_fd = -1;
    int err, val, ret, map, verdict, tc_fd;
    struct netns_cookie_prog *skel;
    uint64_t cookie_expected_value;
    let mut vallen: socklen_t = sizeof(cookie_expected_value);
