...
//! Source: tools/testing/selftests/bpf/prog_tests/sockmap_listen.c
#![no_std]
...
// SOCKMAP and SOCKHASH return different error codes
    ...
    if (s < 0)
    return;
    mapfd = bpf_map_create(BPF_MAP_TYPE_SOCKMAP, core::ptr::null_mut(), sizeof(key),
    sizeof(value32), 1, core::ptr::null_mut());
    if (mapfd < 0) {
...
//
#[no_mangle]
unsafe extern "C" fn do_destroy_orphan_child(family: c_int, sotype: c_int, mapfd: c_int) {
    ...
    int family, int sotype, int mapfd)
    {
    let mut msg_verdict: c_int = bpf_program__fd(skel.progs.prog_msg_verdict);
    let mut skb_verdict: c_int = bpf_program__fd(skel.progs.prog_skb_verdict);
    const struct test {
    int progfd;
...
#[no_mangle]
unsafe extern "C" fn is_thread_done(ctx: *mut connect_accept_ctx) -> bool {
    ...
    int family, int sotype, int mapfd)
    {
    let mut ctx: connect_accept_ctx = { 0 };
    struct sockaddr_storage addr;
    socklen_t len;
    let mut zero: u32 = 0;
    pthread_t t;
    ...
    ctx.sockfd = s;
    ctx.nr_iter = 1000;
    err = xpthread_create(&t, core::ptr::null_mut(), connect_accept_thread, &ctx);
    if (err)
    goto close;
    value = s;
    ...
    xpthread_join(t, core::ptr::null_mut());
    close:
    xclose(s);
    }
    static void *listen_thread(void *arg)
    {
    let mut unspec: sockaddr = { AF_UNSPEC };
    struct connect_accept_ctx *ctx = arg;
    int err, i, s;
    s = ctx.sockfd;
    for (i = 0; i < ctx.nr_iter; i++) {
    ...
    ctx.sockfd = s;
    ctx.nr_iter = 10000;
    err = pthread_create(&t, core::ptr::null_mut(), listen_thread, &ctx);
    if (err)
    goto close;
    value = s;
    while (!is_thread_done(&ctx)) {
...
#[no_mangle]
unsafe extern "C" fn zero_verdict_count(mapfd: c_int) {
    ...
    int sotype)
    {
    let mut verdict: c_int = bpf_program__fd(skel.progs.prog_stream_verdict);
    let mut parser: c_int = bpf_program__fd(skel.progs.prog_stream_parser);
    let mut verdict_map: c_int = bpf_map__fd(skel.maps.verdict_map);
    let mut sock_map: c_int = bpf_map__fd(inner_map);
    int err;
    ...
    int sotype)
    {
    let mut verdict: c_int = bpf_program__fd(skel.progs.prog_msg_verdict);
    let mut verdict_map: c_int = bpf_map__fd(skel.maps.verdict_map);
    let mut sock_map: c_int = bpf_map__fd(inner_map);
    int err;
    ...
    int sotype)
    {
    let mut prog_msg_verdict: c_int = bpf_program__fd(skel.progs.prog_msg_verdict);
    let mut verdict_map: c_int = bpf_map__fd(skel.maps.verdict_map);
    let mut sock_map: c_int = bpf_map__fd(inner_map);
    int link_fd;
    link_fd = bpf_link_create(prog_msg_verdict, sock_map, BPF_SK_MSG_VERDICT, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn redir_partial(family: c_int, sotype: c_int, sock_map: c_int, parser_map: c_int) {
    ...
    let mut verdict: c_int = bpf_program__fd(skel.progs.prog_stream_verdict);
    let mut parser: c_int = bpf_program__fd(skel.progs.prog_stream_parser);
    let mut parser_map: c_int = bpf_map__fd(skel.maps.parser_map);
    let mut sock_map: c_int = bpf_map__fd(inner_map);
    int err;
    err = xbpf_prog_attach(parser, sock_map, BPF_SK_SKB_STREAM_PARSER, 0);
    if (err)
...
// Populate sock_map[0] to avoid ENOENT on first connection
    ...
    goto close_cli0;
    if (sotype == SOCK_STREAM) {
    p0 = xaccept_nonblock(s, core::ptr::null_mut(), core::ptr::null_mut());
    if (p0 < 0)
    goto close_cli0;
    } else {
...
#[no_mangle]
unsafe extern "C" fn test_ops_cleanup(map: *const bpf_map) {
