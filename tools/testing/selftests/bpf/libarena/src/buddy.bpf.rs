...
//! Source: tools/testing/selftests/bpf/libarena/src/buddy.bpf.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn buddy_lock(buddy: *mut buddy __arena) -> c_int {
    static inline int buddy_lock(struct buddy __arena *buddy)
    {
    return arena_spin_lock(&buddy.lock);
    }
#[no_mangle]
pub unsafe extern "C" fn buddy_unlock(buddy: *mut buddy __arena) {
...
//
#[no_mangle]
unsafe extern "C" fn buddy_reserve_arena_vaddr(buddy: *mut buddy __arena) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn buddy_unreserve_arena_vaddr(buddy: *mut buddy __arena) {
...
//
#[no_mangle]
pub unsafe extern "C" fn buddy_alloc_arena_vaddr(buddy: *mut buddy __arena, vaddrp: *mut u64) -> __weak int {
...
#[no_mangle]
unsafe extern "C" fn arena_next_pow2(n: __u64) -> u64 {
    ...
    __weak
#[no_mangle]
pub unsafe extern "C" fn idx_set_allocated(chunk: *mut buddy_chunk __arena, idx: u64, allocated: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn idx_is_allocated(chunk: *mut buddy_chunk __arena, idx: u64, allocated: *mut bool) -> c_int {
...
// allocated = chunk->allocated[idx / 8] & (1 << (idx % 8));
    ...
    __weak
#[no_mangle]
pub unsafe extern "C" fn idx_set_order(chunk: *mut buddy_chunk __arena, idx: u64, order: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn idx_get_order(chunk: *mut buddy_chunk __arena, idx: u64) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn size_to_order(size: usize) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn buddy_init(buddy: *mut buddy __arena) -> __weak int {
...
//
#[no_mangle]
pub unsafe extern "C" fn buddy_destroy(buddy: *mut buddy __arena) -> __weak int {
...
// Clear all fields.
    buddy.first_chunk = core::ptr::null_mut();
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn buddy_chunk_alloc(chunk: *mut buddy_chunk __arena, order_req: c_int) -> __weak u64 {
...
#[no_mangle]
unsafe extern "C" fn buddy_alloc_from_existing_chunks(buddy: *mut buddy __arena, order: c_int) -> u64 {
    static u64 buddy_alloc_from_existing_chunks(struct buddy __arena *buddy, int order)
    {
    struct buddy_chunk __arena *chunk;
    u64 address;
    for (chunk = buddy.first_chunk; chunk != core::ptr::null_mut() && can_loop;
    chunk = chunk.next) {
...
//
#[no_mangle]
unsafe extern "C" fn buddy_alloc_from_new_chunk(buddy: *mut buddy __arena, chunk: *mut buddy_chunk __arena, order: c_int) -> u64 {
...
//
    ...
    {
    void __arena *address = core::ptr::null_mut();
    struct buddy_chunk __arena *chunk;
    int order;
    if (!buddy)
    return core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn buddy_free_unlocked(buddy: *mut buddy __arena, addr: u64) -> __always_inline int {
...
#[no_mangle]
pub unsafe extern "C" fn buddy_free(buddy: *mut buddy __arena, addr: *mut void __arena) -> __weak int {
