...
//! Source: tools/testing/selftests/bpf/libarena/src/asan.bpf.c
#![no_std]
...
//
    let mut asan_violated: volatile u64 = 0;
//
// Shadow map occupancy map.
//
    volatile u64 __asan_shadow_memory_dynamic_address;
    let mut asan_reported: volatile u32 = false;
    let mut asan_inited: volatile bool = false;
//
// Set during program load.
//
    let mut asan_report_once: volatile bool = false;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn asan_memset(dst: *mut s8 __arena, val: i8, size: usize) -> __weak int {
...
#[no_mangle]
unsafe extern "C" fn memory_is_poisoned_1(addr: *mut s8 __arena) -> __always_inline bool {
    static __always_inline bool memory_is_poisoned_1(s8 __arena *addr)
    {
    let mut shadow_value: i8 = *(s8 __arena *)mem_to_shadow(addr);
...
#[no_mangle]
unsafe extern "C" fn memory_is_poisoned_2_4_8(addr: *mut s8 __arena, size: u64) -> __always_inline bool {
    static __always_inline bool memory_is_poisoned_2_4_8(s8 __arena *addr, u64 size)
    {
    let mut end: u64 = (u64)addr + size - 1;
//
...
#[no_mangle]
pub unsafe extern "C" fn asan_shadow_set(addr: *mut void __arena) -> __weak bool {
    __weak bool asan_shadow_set(void __arena *addr)
    {
    return memory_is_poisoned_1(addr);
    }
#[no_mangle]
unsafe extern "C" fn first_nonzero_byte(addr: u64, size: usize) -> __always_inline u64 {
...
#[no_mangle]
unsafe extern "C" fn memory_is_poisoned_n(addr: *mut s8 __arena, size: u64) -> __always_inline bool {
...
#[no_mangle]
pub unsafe extern "C" fn asan_report(addr: *mut s8 __arena, sz: usize, flags: u32) -> __weak int {
    __weak int asan_report(s8 __arena *addr, size_t sz, u32 flags)
    {
    let mut reported: u32 = __sync_val_compare_and_swap(&asan_reported, false, true);
...
#[no_mangle]
pub unsafe extern "C" fn __asan_storeN(addr: intptr_t, size: isize) {
    void __asan_storeN(intptr_t addr, ssize_t size)
    {
    check_region_inline(addr, size, ASAN_WRITE);
    }
#[no_mangle]
pub unsafe extern "C" fn __asan_storeN_noabort(addr: intptr_t, size: isize) {
    void __asan_storeN_noabort(intptr_t addr, ssize_t size)
    {
    check_region_inline(addr, size, ASAN_WRITE);
    }
#[no_mangle]
pub unsafe extern "C" fn __asan_loadN(addr: intptr_t, size: isize) {
    void __asan_loadN(intptr_t addr, ssize_t size)
    {
    check_region_inline(addr, size, ASAN_READ);
    }
#[no_mangle]
pub unsafe extern "C" fn __asan_loadN_noabort(addr: intptr_t, size: isize) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __asan_register_globals(globals: intptr_t, n: usize) {
    void __asan_register_globals(intptr_t globals, size_t n)
    {
    }
#[no_mangle]
pub unsafe extern "C" fn __asan_unregister_globals(globals: intptr_t, n: usize) {
...
//
#[no_mangle]
pub unsafe extern "C" fn asan_poison(addr: *mut void __arena, val: i8, size: usize) -> __hidden __noasan int {
...
//
#[no_mangle]
pub unsafe extern "C" fn asan_unpoison(addr: *mut void __arena, size: usize) -> __hidden __noasan int {
    __hidden __noasan int asan_unpoison(void __arena *addr, size_t size)
    {
    let mut partial: usize = size & ASAN_GRANULE_MASK;
    s8 __arena *shadow;
    size_t len;
//
...
//
    SEC("syscall")
#[no_mangle]
pub unsafe extern "C" fn asan_init(args: *mut asan_init_args) -> __weak __noasan int {
    __weak __noasan int asan_init(struct asan_init_args *args)
    {
    let mut globals_pages: u64 = args.arena_globals_pages;
    let mut all_pages: u64 = args.arena_all_pages;
