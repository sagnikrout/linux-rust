...
//! Source: tools/testing/selftests/bpf/progs/test_cls_redirect_dynptr.c
#![no_std]
...
//
    let mut CONTINUE_PROCESSING: static ret_t = -1;
...
#[no_mangle]
unsafe extern "C" fn ipv4_is_fragment(ip: *const iphdr) -> bool {
    static bool ipv4_is_fragment(const struct iphdr *ip)
    {
    let mut frag_off: u16 = ip.frag_off & bpf_htons(IP_OFFSET_MASK);
    return (ip.frag_off & bpf_htons(IP_MF)) != 0 || frag_off > 0;
    }
#[no_mangle]
unsafe extern "C" fn pkt_parse_ipv4(dynptr: *mut bpf_dynptr, offset: *mut __u64, iphdr: *mut iphdr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pkt_parse_icmp_l4_ports(dynptr: *mut bpf_dynptr, offset: *mut __u64, ports: *mut flow_ports_t) -> bool {
...
//
    let mut dst: u16 = ports.src;
    ports.src = ports.dst;
    ports.dst = dst;
    return true;
    }
#[no_mangle]
unsafe extern "C" fn pkt_checksum_fold(csum: u32) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn pkt_ipv4_checksum(iph: *mut iphdr) {
...
#[no_mangle]
unsafe extern "C" fn accept_locally(skb: *mut __sk_buff, encap: *mut encap_headers_t) -> ret_t {
    static ret_t accept_locally(struct __sk_buff *skb, encap_headers_t *encap)
    {
    const int payload_off =
    sizeof(*encap) +
    sizeof(struct in_addr) * encap.unigue.hop_count;
    let mut encap_overhead: i32 = payload_off - sizeof(struct ethhdr);
// Changing the ethertype if the encapsulated packet is ipv6
    ...
    let mut delta: i32 = sizeof(struct gre_base_hdr) - encap_overhead;
...
//
    ...
    let mut old_saddr: u32 = encap.ip.saddr;
...
// Remove ip->saddr, add next_hop->s_addr
    let mut off: u64 = offsetof(typeof(*encap), ip.check);
    let mut ret: c_int = bpf_l3_csum_replace(skb, off, old_saddr, next_hop.s_addr, 4);
    if (ret < 0) {
    return TC_ACT_SHOT;
    }
    return bpf_redirect(skb.ifindex, 0);
...
#[no_mangle]
unsafe extern "C" fn skip_next_hops(offset: *mut __u64, n: c_int) -> ret_t {
...
//
    ...
    if (iph != core::ptr::null_mut() && tcp != core::ptr::null_mut()) {
// Kludge: we've run out of arguments, but need the length of the ip header.
    let mut iphlen: u64 = sizeof(struct iphdr);
...
#[no_mangle]
unsafe extern "C" fn classify_udp(skb: *mut __sk_buff, tuple: *mut bpf_sock_tuple, tuplen: u64) -> verdict_t {
    ...
    switch (proto) {
    case IPPROTO_TCP:
    return classify_tcp(skb, tuple, tuplen, core::ptr::null_mut(), core::ptr::null_mut());
    case IPPROTO_UDP:
...
// offset += sizeof(udph);
    ...
    SEC("tc")
#[no_mangle]
pub unsafe extern "C" fn cls_redirect(skb: *mut __sk_buff) -> c_int {
