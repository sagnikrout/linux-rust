...
//! Source: tools/testing/selftests/bpf/progs/file_reader.c
#![no_std]
...
pub unsafe extern "C" fn on_open_expect_fault(c: *mut c_void) -> c_int {
    ...
    let mut user_buf_sz: __u32 = sizeof(user_buf);
...
#[no_mangle]
unsafe extern "C" fn task_work_callback(map: *mut bpf_map, key: *mut c_void, value: *mut c_void) -> c_int {
    static int task_work_callback(struct bpf_map *map, void *key, void *value)
    {
    struct task_struct *task = bpf_get_current_task_btf();
...
#[no_mangle]
unsafe extern "C" fn verify_dynptr_read(ptr: *mut bpf_dynptr, off: u32, user_buf: *mut c_char, len: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn validate_file_read(file: *mut file) -> c_int {
    static int validate_file_read(struct file *file)
    {
    struct bpf_dynptr dynptr;
    let mut loc_err: c_int = 1, off;
    let mut user_buf_sz: __u32 = sizeof(user_buf);
    if (bpf_dynptr_from_file(file, 0, &dynptr))
    goto cleanup;
