...
//! Source: tools/testing/selftests/bpf/progs/net_timestamping.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct sk_stg {
    pub /: *mut *mut __u64 sendmsg_ns; / record ts when sendmsg is called,
}
...
pub struct delay_info {
    pub /: *mut *mut u64 sendmsg_ns; / record ts when sendmsg is called,
    pub /: *mut *mut u32 sched_delay; / SCHED_CB - sendmsg_ns,
    pub /: *mut *mut u32 snd_sw_delay; / SND_SW_CB - SCHED_CB,
    pub /: *mut *mut u32 ack_delay; / ACK_CB - SND_SW_CB,
}
...
#[no_mangle]
unsafe extern "C" fn bpf_test_sockopt(ctx: *mut c_void, sk: *const sock, expected: c_int) -> c_int {
    static int bpf_test_sockopt(void *ctx, const struct sock *sk, int expected)
    {
    int tmp, new = SK_BPF_CB_TX_TIMESTAMPING;
    let mut opt: c_int = SK_BPF_CB_FLAGS;
    let mut level: c_int = SOL_SOCKET;
...
#[no_mangle]
unsafe extern "C" fn bpf_test_access_sockopt(ctx: *mut c_void, sk: *const sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn bpf_test_access_load_hdr_opt(skops: *mut bpf_sock_ops) -> bool {
    static bool bpf_test_access_load_hdr_opt(struct bpf_sock_ops *skops)
    {
    u8 opt[3] = {0};
    let mut load_flags: c_int = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn bpf_test_access_cb_flags_set(skops: *mut bpf_sock_ops) -> bool {
...
#[no_mangle]
unsafe extern "C" fn bpf_test_delay(skops: *mut bpf_sock_ops, sk: *const sock) -> bool {
    static bool bpf_test_delay(struct bpf_sock_ops *skops, const struct sock *sk)
    {
    struct bpf_sock_ops_kern *skops_kern;
    let mut timestamp: u64 = bpf_ktime_get_ns();
    struct skb_shared_info *shinfo;
    let mut dinfo: delay_info = {0};
    let mut key: sk_tskey = {0};
...
// Since it's the last one, remove from the map after latency check
    ...
    let mut timestamp: u64 = bpf_ktime_get_ns();
    let mut flag: u32 = sk.sk_bpf_cb_flags;
    struct sk_stg *stg;
    ...
    SEC("sockops")
#[no_mangle]
pub unsafe extern "C" fn skops_sockopt(skops: *mut bpf_sock_ops) -> c_int {
