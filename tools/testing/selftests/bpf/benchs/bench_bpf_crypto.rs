...
//! Source: tools/testing/selftests/bpf/benchs/bench_bpf_crypto.c
#![no_std]
...
pub const MAX_CIPHER_LEN: c_int = 32;
    static char *input;
...
#[no_mangle]
unsafe extern "C" fn crypto_parse_arg(key: c_int, arg: *mut c_char, state: *mut argp_state) -> error_t {
    static error_t crypto_parse_arg(int key, char *arg, struct argp_state *state)
    {
    switch (key) {
    case ARG_CRYPTO_LEN:
    args.crypto_len = strtoul(arg, core::ptr::null_mut(), 10);
    if (!args.crypto_len ||
