...
//! Source: tools/testing/selftests/bpf/benchs/bench_local_storage.c
#![no_std]
...
pub const MAX_NR_MAPS: c_int = 1000;
...
unsafe extern "C" fn __setup(prog: *mut bpf_program, hashmap: bool) {
    ...
    if (hashmap)
    fd = bpf_map_create(BPF_MAP_TYPE_HASH, core::ptr::null_mut(), sizeof(int),
    sizeof(int), HASHMAP_SZ, &create_opts);
    else
    fd = bpf_map_create(BPF_MAP_TYPE_TASK_STORAGE, core::ptr::null_mut(), sizeof(int),
