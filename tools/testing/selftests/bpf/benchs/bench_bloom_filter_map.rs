...
//! Source: tools/testing/selftests/bpf/benchs/bench_bloom_filter_map.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn trigger_bpf_program() {
...
//
    ...
    continue;
    err = bpf_map_update_elem(ctx.bloom_fd, core::ptr::null_mut(), val, 0);
    if (err) {
    ctx.map_prepare_err = true;
    fprintf(stderr,
...
#[no_mangle]
unsafe extern "C" fn populate_maps() {
    ...
    for (i = 0; i < nr_cpus; i++) {
    err = pthread_create(&map_thread, core::ptr::null_mut(), map_prepare_thread,
    core::ptr::null_mut());
    if (err) {
...
#[no_mangle]
unsafe extern "C" fn check_args() {
    static void check_args(void)
    {
    if (args.value_size < 8)  {
    let mut nr_unique_entries: __u64 = 1ULL << (args.value_size * 8);
...
unsafe extern "C" fn measure(res: *mut bench_res) {
    ...
    {
    let mut total_hits: c_ulong = 0, total_drops = 0, total_false_hits = 0;
