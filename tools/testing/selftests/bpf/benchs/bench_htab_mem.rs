...
//! Source: tools/testing/selftests/bpf/benchs/bench_htab_mem.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct htab_mem_use_case {
    pub name: *const c_char,
    pub progs: *const c_char,
// Do synchronization between addition thread and deletion thread
    ...
    const char *ow_progs[] = {"overwrite", core::ptr::null_mut()};
    const char *batch_progs[] = {"batch_add_batch_del", core::ptr::null_mut()};
    const char *add_del_progs[] = {"add_only", "del_only", core::ptr::null_mut()};
    ...
    { "preallocated", ARG_PREALLOCATED, core::ptr::null_mut(), 0, "use preallocated hash map" },
    {},
    };
#[no_mangle]
unsafe extern "C" fn htab_mem_parse_arg(key: c_int, arg: *mut c_char, state: *mut argp_state) -> error_t {
    static error_t htab_mem_parse_arg(int key, char *arg, struct argp_state *state)
    {
    switch (key) {
    case ARG_VALUE_SIZE:
    args.value_size = strtoul(arg, core::ptr::null_mut(), 10);
...
#[no_mangle]
unsafe extern "C" fn htab_mem_bench_init_barriers() -> c_int {
...
// Used for synchronization between two threads
    for (i = 0; i < nr; i++)
    pthread_barrier_init(&barriers[i], core::ptr::null_mut(), 2);
    ctx.notify = barriers;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn htab_mem_bench_exit_barriers() {
...
#[no_mangle]
unsafe extern "C" fn htab_mem_setup_impl(map_type: enum bpf_map_type) {
...
#[no_mangle]
unsafe extern "C" fn htab_mem_add_fn(notify: *mut pthread_barrier_t) {
...
#[no_mangle]
unsafe extern "C" fn htab_mem_delete_fn(notify: *mut pthread_barrier_t) {
...
#[no_mangle]
unsafe extern "C" fn htab_mem_read_mem_cgrp_file(name: *const c_char, value: *mut c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn htab_mem_report_progress(iter: c_int, res: *mut bench_res, delta_ns: c_long) {
...
#[no_mangle]
unsafe extern "C" fn htab_mem_report_final(res[]: bench_res, res_cnt: c_int) {
    static void htab_mem_report_final(struct bench_res res[], int res_cnt)
    {
    let mut mem_mean: double = 0.0, mem_stddev = 0.0;
    let mut loop_mean: double = 0.0, loop_stddev = 0.0;
