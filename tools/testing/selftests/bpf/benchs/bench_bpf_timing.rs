...
//! Source: tools/testing/selftests/bpf/benchs/bench_bpf_timing.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct timing_stats {
    pub max: double min,,
    pub p99: double median,,
    pub stddev: double mean,,
    pub count: c_int,
}
...
unsafe extern "C" fn cmp_double(a: *const c_void, b: *const c_void) -> c_int {
    ...
    {
    let mut da: double = *(const double *)a;
    let mut db: double = *(const double *)b;
    if (da < db)
    return -1;
    if (da > db)
    return 1;
...
#[no_mangle]
unsafe extern "C" fn percentile(sorted: *const double, n: c_int, pct: double) -> double {
    static double percentile(const double *sorted, int n, double pct)
    {
    let mut idx: c_int = (int)(n * pct / 100.0);
    if (idx >= n)
    idx = n - 1;
    return sorted[idx];
    }
...
#[no_mangle]
unsafe extern "C" fn filter_outliers_iqr(sorted: *mut double, n: c_int) -> c_int {
    static int filter_outliers_iqr(double *sorted, int n)
    {
    double q1, q3, iqr, lo, hi;
    let mut start: c_int = 0, end = n;
    if (n < 8)
    return n;
    q1 = sorted[n / 4];
    q3 = sorted[3 * n / 4];
    ...
    {
    let mut sum: double = 0, var_sum = 0;
    memset(s, 0, sizeof(*s));
    s.count = n;
    if (n == 0)
    return;
    ...
    s.mean = sum / n;
    for (int i = 0; i < n; i++) {
    let mut d: double = sorted[i] - s.mean;
    var_sum += d * d;
    }
    s.stddev = n > 1 ? sqrt(var_sum / (n - 1)) : 0;
    }
#[no_mangle]
pub unsafe extern "C" fn bpf_bench_timing_measure(t: *mut bpf_bench_timing, res: *mut bench_res) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_bench_timing_report(t: *mut bpf_bench_timing, name: *const c_char, description: *const c_char) {
    void bpf_bench_timing_report(struct bpf_bench_timing *t, const char *name, const char *description)
    {
    let mut max_out: c_int = BENCH_NR_CPUS * BENCH_NR_SAMPLES;
    struct timing_stats s;
    double *all;
    int total;
...
pub const CALIBRATE_SEED_BATCH: c_int = 100;
pub const CALIBRATE_MIN_BATCH: c_int = 100;
pub const CALIBRATE_MAX_BATCH: c_int = 10000000;
pub const CALIBRATE_TARGET_MS: c_int = 10;
pub const CALIBRATE_RUNS: c_int = 5;

#[no_mangle]
unsafe extern "C" fn reset_timing(t: *mut bpf_bench_timing) {
...
#[no_mangle]
unsafe extern "C" fn compute_batch_iters(per_op_ns: __u64) -> __u32 {
    static __u32 compute_batch_iters(__u64 per_op_ns)
    {
    let mut target_ns: __u64 = (__u64)CALIBRATE_TARGET_MS * 1000000ULL;
    __u32 iters;
    if (per_op_ns == 0)
...
#[no_mangle]
pub unsafe extern "C" fn bpf_bench_calibrate(t: *mut bpf_bench_timing, run_fn: bpf_bench_run_fn, run_ctx: *mut c_void) {
