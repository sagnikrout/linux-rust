...
//! Source: tools/testing/selftests/bpf/benchs/bench_xdp_lb.c
#![no_std]
...
pub const REAL_INDEX: c_int = 1;
pub const REAL_INDEX_V6: c_int = 2;
pub const MAX_PKT_SIZE: c_int = 256;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_scenario {
    pub name: *const c_char,
    pub description: *const c_char,
    pub expected_retval: c_int,
    pub expect_encap: bool,
    pub is_v6: bool,
    pub vip_addr: __u32,
    pub src_addr: __u32,
    pub tunnel_dst: __u32,
    pub vip_addr_v6: [__u32; 4],
    pub src_addr_v6: [__u32; 4],
    pub tunnel_dst_v6: [__u32; 4],
    pub dst_port: __u16,
    pub src_port: __u16,
    pub ip_proto: __u8,
    pub vip_flags: __u32,
    pub vip_num: __u32,
    pub prepopulate_lru: bool,
    pub set_frag: bool,
    pub eth_proto: __u16,
    pub encap_v6_outer: bool,
    pub flow_mask: __u32,
    pub cold_lru: bool,
    pub set_syn: bool,
    pub set_rst: bool,
    pub set_ip_options: bool,
    pub /: *mut *mut __u32 fixed_batch_iters; / 0 = auto-calibrate, >0 = use this value,
    pub /: *mut *mut enum lru_miss_type lru_miss; / expected LRU miss pattern,
}
...
#[no_mangle]
unsafe extern "C" fn ip_checksum(hdr: *const c_void, len: c_int) -> __u16 {
...
#[no_mangle]
unsafe extern "C" fn htonl_v6(dst[4]: __be32, src[4]: __u32) {
...
#[no_mangle]
unsafe extern "C" fn build_flow_key(fk: *mut flow_key, sc: *const test_scenario) {
...
#[no_mangle]
unsafe extern "C" fn build_l4(sc: *const test_scenario, p: *mut __u8, off: *mut __u32) {
...
#[no_mangle]
unsafe extern "C" fn build_packet(idx: c_int) {
    ...
    proto = sc.eth_proto;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sc->is_v6) -> else {
    else if (sc.is_v6)
    proto = ETH_P_IPV6;
    else
    proto = ETH_P_IP;
    ...
    if (sc.is_v6) {
    let mut ip6h: ipv6hdr = {};
    let mut ip6_off: __u32 = off;
    ip6h.version  = 6;
...
// NOP option padding (4 bytes = 1 word)
    let mut nop: __u32 = htonl(0x01010101);
...
#[no_mangle]
unsafe extern "C" fn populate_vip(skel: *mut xdp_lb_bench, sc: *const test_scenario) {
    static void populate_vip(struct xdp_lb_bench *skel, const struct test_scenario *sc)
    {
    let mut key: vip_definition = {};
    let mut val: vip_meta = {};
    int err;
    if (sc.is_v6)
...
#[no_mangle]
unsafe extern "C" fn create_per_cpu_lru_maps(skel: *mut xdp_lb_bench) {
    static void create_per_cpu_lru_maps(struct xdp_lb_bench *skel)
    {
    let mut outer_fd: c_int = bpf_map__fd(skel.maps.lru_mapping);
...
#[no_mangle]
unsafe extern "C" fn ktime_get_ns() -> __u64 {
...
#[no_mangle]
unsafe extern "C" fn populate_lru(sc: *const test_scenario, real_idx: __u32) {
    static void populate_lru(const struct test_scenario *sc, __u32 real_idx)
    {
    let mut lru: real_pos_lru = { .pos = real_idx };
    struct flow_key fk;
    int i, err;
...
#[no_mangle]
unsafe extern "C" fn populate_maps(skel: *mut xdp_lb_bench) {
    static void populate_maps(struct xdp_lb_bench *skel)
    {
    let mut real_v4: real_definition = {};
    let mut real_v6: real_definition = {};
    let mut cval: ctl_value = {};
    ...
    let mut ridx: __u32 = sc.encap_v6_outer ? REAL_INDEX_V6 : REAL_INDEX;
    populate_lru(sc, ridx);
    }
    if (scenarios[args.scenario].expect_encap) {
    const struct test_scenario *sc = &scenarios[args.scenario];
    let mut miss_vip: vip_definition = {};
    if (sc.is_v6)
...
#[no_mangle]
unsafe extern "C" fn build_expected_packet(idx: c_int) {
    ...
    let mut in_len: __u32 = pkt_len[idx];
    let mut off: __u32 = 0;
    let mut inner_len: __u32 = in_len - sizeof(struct ethhdr);
    ...
    let mut ip6h: ipv6hdr = {};
    let mut nexthdr: __u8 = sc.is_v6 ? IPPROTO_IPV6 : IPPROTO_IPIP;
    ip6h.version     = 6;
    ...
    __u32 exp_len)
    {
    let mut max_len: __u32 = got_len > exp_len ? got_len : exp_len;
    __u32 i, ndiffs = 0;
    fprintf(stderr, "  [%s] got %u bytes, expected %u bytes\n",
    name, got_len, exp_len);
    for (i = 0; i < max_len && ndiffs < 8; i++) {
    let mut g: __u8 = i < got_len ? got[i] : 0;
    let mut e: __u8 = i < exp_len ? exp[i] : 0;
...
#[no_mangle]
unsafe extern "C" fn read_stat(stats_fd: c_int, key: __u32, v1_out: *mut __u64, v2_out: *mut __u64) {
    static void read_stat(int stats_fd, __u32 key, __u64 *v1_out, __u64 *v2_out)
    {
    struct lb_stats values[BENCH_NR_CPUS];
    let mut nr_cpus: c_uint = bpf_num_possible_cpus();
    let mut v1: __u64 = 0, v2 = 0;
    unsigned int i;
    if (nr_cpus > BENCH_NR_CPUS)
    nr_cpus = BENCH_NR_CPUS;
...
#[no_mangle]
unsafe extern "C" fn reset_stats(stats_fd: c_int) {
...
#[no_mangle]
unsafe extern "C" fn validate_counters(idx: c_int) -> bool {
    static bool validate_counters(int idx)
    {
    const struct test_scenario *sc = &scenarios[idx];
    let mut stats_fd: c_int = bpf_map__fd(ctx.skel.maps.stats);
...
//
    let mut n: __u64 = ctx.timing.batch_iters + 1;
...
// LRU_MISS_AUTO: compute from scenario flags
    ...
    expected_misses = n;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sc->cold_lru) -> else {
    else if (sc.cold_lru)
    expected_misses = 1;
    else
    expected_misses = n;
...
#[no_mangle]
unsafe extern "C" fn validate_scenario(idx: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn find_scenario(name: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xdp_lb_run_once(__always_unused: *mut *mut void unused) {
    static void xdp_lb_run_once(void *unused __always_unused)
    {
    let mut idx: c_int = args.scenario;
    LIBBPF_OPTS(bpf_test_run_opts, topts,
    .data_in      = pkt_buf[idx],
...
unsafe extern "C" fn xdp_lb_setup() {
    ...
    bpf_bench_calibrate(&ctx.timing, xdp_lb_run_once, core::ptr::null_mut());
    }
...
//
    ...
    skel.bss.cold_lru = 1;
    xdp_lb_run_once(core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn xdp_lb_report_final(res[]: bench_res, res_cnt: c_int) {
    ...
    { "list-scenarios", ARG_LIST_SCENARIOS, core::ptr::null_mut(), 0,
    "List available scenarios and exit" },
    { "machine-readable", ARG_MACHINE_READABLE, core::ptr::null_mut(), 0,
