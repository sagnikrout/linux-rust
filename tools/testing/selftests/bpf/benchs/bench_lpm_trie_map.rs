...
//! Source: tools/testing/selftests/bpf/benchs/bench_lpm_trie_map.c
#![no_std]
...
//
    ...
    { "random", ARG_RANDOM, core::ptr::null_mut(), 0, "Access random keys during op" },
    {},
    };
#[no_mangle]
unsafe extern "C" fn lpm_parse_arg(key: c_int, arg: *mut c_char, state: *mut argp_state) -> error_t {
...
unsafe extern "C" fn lpm_free_validate() {
    ...
    static __u32 *vals;
#[no_mangle]
unsafe extern "C" fn fill_map(map_fd: c_int) {
...
#[no_mangle]
unsafe extern "C" fn empty_map(map_fd: c_int) {
...
#[no_mangle]
unsafe extern "C" fn attach_prog_and_fill_map() {
...
#[no_mangle]
unsafe extern "C" fn bench_reinit_map() {
    static void bench_reinit_map(void)
    {
    let mut fd: c_int = bpf_map__fd(ctx.bench.maps.trie_map);
...
// For NOOP, BASELINE, LOOKUP, INSERT, UPDATE, and DELETE
    ...
    while (true) {
    let mut fd: c_int = bpf_program__fd(ctx.bench.progs.run_bench);
...
//
    ...
    {
    let mut hits_mean: double = 0.0, hits_stddev = 0.0;
    let mut latency: double = 0.0;
    int i;
    for (i = 0; i < res_cnt; i++) {
    ...
    long delta_ns)
    {
    let mut rate_divisor: double = 1000000.0;
    let mut rate: c_char = 'M';
    ...
    long delta_ns)
    {
    let mut rate_divisor: double = 1000.0;
    let mut rate: c_char = 'K';
    frac_second_report_progress(iter, res, delta_ns, rate_divisor, rate);
    }
#[no_mangle]
unsafe extern "C" fn insert_ops_report_final(res[]: bench_res, res_cnt: c_int) {
    static void insert_ops_report_final(struct bench_res res[], int res_cnt)
    {
    let mut lat_divisor: double = 1.0;
    let mut rate_divisor: double = 1000000.0;
    const char *unit = "ns";
    let mut rate: c_char = 'M';
...
#[no_mangle]
unsafe extern "C" fn delete_ops_report_final(res[]: bench_res, res_cnt: c_int) {
...
#[no_mangle]
unsafe extern "C" fn free_ops_report_final(res[]: bench_res, res_cnt: c_int) {
    static void free_ops_report_final(struct bench_res res[], int res_cnt)
    {
    let mut lat_divisor: double = 1000000.0;
    let mut rate_divisor: double = 1000.0;
    const char *unit = "ms";
    let mut rate: c_char = 'K';
    frac_second_report_final(res, res_cnt, lat_divisor, rate_divisor, rate,
