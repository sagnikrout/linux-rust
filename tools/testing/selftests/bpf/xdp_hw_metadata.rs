...
//! Source: tools/testing/selftests/bpf/xdp_hw_metadata.c
#![no_std]
...
pub struct xsk {
    ...
    let mut bind_flags: __u16 = XDP_USE_NEED_WAKEUP | XDP_ZEROCOPY;
    struct xsk *rx_xsk;
    const char *ifname;
    int ifindex;
    int rxq;
...
#[no_mangle]
unsafe extern "C" fn open_xsk(ifindex: c_int, xsk: *mut xsk, queue_id: __u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn kick_tx(xsk: *mut xsk) -> c_int {
    static int kick_tx(struct xsk *xsk)
    {
    return sendto(xsk_socket__fd(xsk.socket), core::ptr::null_mut(), 0, MSG_DONTWAIT, core::ptr::null_mut(), 0);
    }
#[no_mangle]
unsafe extern "C" fn kick_rx(xsk: *mut xsk) -> c_int {
    static int kick_rx(struct xsk *xsk)
    {
    return recvfrom(xsk_socket__fd(xsk.socket), core::ptr::null_mut(), 0, MSG_DONTWAIT, core::ptr::null_mut(), core::ptr::null_mut());
    }

#[no_mangle]
unsafe extern "C" fn gettime(clock_id: clockid_t) -> __u64 {
...
// See man clock_gettime(2) for type of clock_id's
    ...
    {
    let mut delta: __s64 = (__s64)reference - (__s64)tstamp;
...
#[no_mangle]
unsafe extern "C" fn print_vlan_tci(tag: __u16) {
    static void print_vlan_tci(__u16 tag)
    {
    let mut vlan_id: __u16 = FIELD_GET(VLAN_VID_MASK, tag);
    let mut pcp: __u8 = FIELD_GET(VLAN_PRIO_MASK, tag);
    let mut dei: bool = FIELD_GET(VLAN_DEI_MASK, tag);
    printf("PCP=%u, DEI=%d, VID=0x%X\n", pcp, dei, vlan_id);
    }
#[no_mangle]
unsafe extern "C" fn verify_xdp_metadata(data: *mut c_void, clock_id: clockid_t) {
    ...
    let mut ref_tstamp: __u64 = gettime(clock_id);
...
#[no_mangle]
unsafe extern "C" fn verify_skb_metadata(fd: c_int) {
...
#[no_mangle]
unsafe extern "C" fn complete_tx(xsk: *mut xsk, clock_id: clockid_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ping_pong(xsk: *mut xsk, rx_packet: *mut c_void, clock_id: clockid_t) {
...
#[no_mangle]
unsafe extern "C" fn verify_metadata(rx_xsk: *mut xsk, rxq: c_int, server_fd: c_int, clock_id: clockid_t) -> c_int {
    ...
    for (i = 0; i < rxq; i++) {
    let mut first_seg: bool = true;
    let mut is_eop: bool = true;
    if (fds[i].revents == 0)
    continue;
...
#[no_mangle]
unsafe extern "C" fn rxq_num(ifname: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hwtstamp_ioctl(op: c_int, ifname: *const c_char, cfg: *mut hwtstamp_config) {
...
#[no_mangle]
unsafe extern "C" fn hwtstamp_enable(ifname: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn timestamping_enable(fd: c_int, val: c_int) {
...
#[no_mangle]
unsafe extern "C" fn read_args(argc: c_int, argv[]: *mut c_char) {
...
pub const MAX_TC: c_int = 16;
#[no_mangle]
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    int main(int argc, char *argv[])
    {
    let mut clock_id: clockid_t = CLOCK_TAI;
    struct bpf_program *prog;
    let mut server_fd: c_int = -1;
    let mut map_len: usize = 0;
    let mut que_len: usize = 0;
    char *buf = core::ptr::null_mut();
    char *map = core::ptr::null_mut();
    char *que = core::ptr::null_mut();
...
// Enable VLAN tag stripping offload
    ...
    server_fd = start_server(AF_INET6, SOCK_DGRAM, core::ptr::null_mut(), 9092, 1000);
    if (server_fd < 0)
    ...
    for (i = 0; i < rxq; i++) {
    let mut sock_fd: c_int = xsk_socket__fd(rx_xsk[i].socket);
    let mut queue_id: __u32 = i;
    printf("map[%d] = %d\n", queue_id, sock_fd);
