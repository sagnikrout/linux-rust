...
//! Source: tools/testing/selftests/bpf/test_verifier.c
#![no_std]
...
pub const MAX_EXPECTED_INSNS: c_int = 32;
pub const MAX_UNEXPECTED_INSNS: c_int = 32;
pub const MAX_TEST_INSNS: c_int = 1000000;
pub const MAX_FIXUPS: c_int = 8;
pub const MAX_NR_MAPS: c_int = 23;
pub const MAX_TEST_RUNS: c_int = 8;
pub const POINTER_VALUE: c_uint = 0xcafe4all;
pub const TEST_DATA_LEN: c_int = 64;
pub const MAX_FUNC_INFOS: c_int = 8;
pub const MAX_BTF_STRINGS: c_int = 256;
pub const MAX_BTF_TYPES: c_int = 256;

pub const DEFAULT_LIBBPF_LOG_LEVEL: c_int = 4;
...
// need CAP_BPF, CAP_NET_ADMIN, CAP_PERFMON to load progs
    ...
    let mut unpriv_disabled: static bool = false;
    static bool jit_disabled;
    static int skips;
    let mut verbose: static bool = false;
    let mut verif_log_level: static int = 0;
#[repr(C)]
#[derive(Copy, Clone)]
...
//
    ...
    REJECT,
    VERBOSE_ACCEPT,
    pub result_unpriv: } result,,
    pub prog_type: enum bpf_prog_type,
    pub flags: u8,
    pub self): *mut *mut void (fill_helper)(struct bpf_test,
    pub runs: c_int,

    struct {						\
    pub \: uint32_t retval, retval_unpriv;,
    union {						\
    pub \: __u8 data[TEST_DATA_LEN];,
    pub \: __u64 data64[TEST_DATA_LEN / 8];,
}
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_ld_abs_vlan_push_pop(self: *mut bpf_test) {
    static void bpf_fill_ld_abs_vlan_push_pop(struct bpf_test *self)
    {
// test: {skb->data[0], vlan_push} x 51 + {skb->data[0], vlan_pop} x 51
pub const PUSH_CNT: c_int = 51;
// jump range is limited to 16 bit. PUSH_CNT of ld_abs needs room
    let mut len: c_uint = (1 << 15) - PUSH_CNT * 2 * 5 * 6;
    struct bpf_insn *insn = self.fill_insns;
    let mut i: c_int = 0, j, k = 0;
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_jump_around_ld_abs(self: *mut bpf_test) {
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_rand_ld_dw(self: *mut bpf_test) {
    ...
    while (i < self.retval) {
    let mut val: u64 = bpf_semi_rand_get();
...
pub const MAX_JMP_SEQ: c_int = 8192;
// test the sequence of 8k jumps
#[no_mangle]
unsafe extern "C" fn bpf_fill_scale1(self: *mut bpf_test) {
    static void bpf_fill_scale1(struct bpf_test *self)
    {
    struct bpf_insn *insn = self.fill_insns;
    let mut i: c_int = 0, k = 0;
    insn[i++] = BPF_MOV64_REG(BPF_REG_6, BPF_REG_1);
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_scale2(self: *mut bpf_test) {
    static void bpf_fill_scale2(struct bpf_test *self)
    {
    struct bpf_insn *insn = self.fill_insns;
    let mut i: c_int = 0, k = 0;
pub const FUNC_NEST: c_int = 7;
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_scale(self: *mut bpf_test) {
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_torturous_jumps_insn_1(insn: *mut bpf_insn) -> c_int {
    static int bpf_fill_torturous_jumps_insn_1(struct bpf_insn *insn)
    {
    let mut len: c_uint = 259, hlen = 128;
    int i;
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_torturous_jumps_insn_2(insn: *mut bpf_insn) -> c_int {
    static int bpf_fill_torturous_jumps_insn_2(struct bpf_insn *insn)
    {
    let mut len: c_uint = 4100, jmp_off = 2048;
    int i, j;
    insn[0] = BPF_EMIT_CALL(BPF_FUNC_get_prandom_u32);
    for (i = 1; i <= jmp_off; i++) {
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_torturous_jumps(self: *mut bpf_test) {
...
#[no_mangle]
unsafe extern "C" fn bpf_fill_big_prog_with_loop_1(self: *mut bpf_test) {
...
// patching.
//
    let mut len: c_int = getpagesize() - 25;
...
// Macro flag: #define FILL_ARRAY

    };
#[no_mangle]
unsafe extern "C" fn probe_filter_length(fp: *const bpf_insn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn skip_unsupported_map(map_type: enum bpf_map_type) -> bool {
    static bool skip_unsupported_map(enum bpf_map_type map_type)
    {
    if (!libbpf_probe_bpf_map_type(map_type, core::ptr::null_mut())) {
    ...
    fd = bpf_map_create(type, core::ptr::null_mut(), size_key, size_value, max_elem, &opts);
    if (fd < 0) {
...
#[no_mangle]
unsafe extern "C" fn update_map(fd: c_int, index: c_int) {
...
#[no_mangle]
unsafe extern "C" fn create_prog_dummy_simple(prog_type: enum bpf_prog_type, ret: c_int) -> c_int {
    ...
    return bpf_prog_load(prog_type, core::ptr::null_mut(), "GPL", prog, ARRAY_SIZE(prog), core::ptr::null_mut());
    }
    ...
    mfd = bpf_map_create(BPF_MAP_TYPE_PROG_ARRAY, core::ptr::null_mut(), sizeof(int),
    sizeof(int), max_elem, core::ptr::null_mut());
    if (mfd < 0) {
...
#[no_mangle]
unsafe extern "C" fn create_map_in_map() -> c_int {
    static int create_map_in_map(void)
    {
    LIBBPF_OPTS(bpf_map_create_opts, opts);
    int inner_map_fd, outer_map_fd;
    inner_map_fd = bpf_map_create(BPF_MAP_TYPE_ARRAY, core::ptr::null_mut(), sizeof(int),
    sizeof(int), 1, core::ptr::null_mut());
    if (inner_map_fd < 0) {
    ...
    outer_map_fd = bpf_map_create(BPF_MAP_TYPE_ARRAY_OF_MAPS, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn create_cgroup_storage(percpu: bool) -> c_int {
    static int create_cgroup_storage(bool percpu)
    {
    enum bpf_map_type type = percpu ? BPF_MAP_TYPE_PERCPU_CGROUP_STORAGE :
    BPF_MAP_TYPE_CGROUP_STORAGE;
    int fd;
    fd = bpf_map_create(type, core::ptr::null_mut(), sizeof(struct bpf_cgroup_storage_key),
    TEST_DATA_LEN, 0, core::ptr::null_mut());
    if (fd < 0) {
    if (skip_unsupported_map(type))
    return -1;
...
#[no_mangle]
unsafe extern "C" fn load_btf() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn create_sk_storage_map() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn create_map_timer() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn create_map_kptr() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_root(set: bool) {
...
#[no_mangle]
unsafe extern "C" fn ptr_to_u64(ptr: *const c_void) -> __u64 {
...
#[no_mangle]
unsafe extern "C" fn kfuncs_cleanup() {
...
#[no_mangle]
unsafe extern "C" fn set_admin(admin: bool) -> c_int {
    static int set_admin(bool admin)
    {
    int err;
    if (admin) {
    err = cap_enable_effective(ADMIN_CAPS, core::ptr::null_mut());
    if (err)
    perror("cap_enable_effective(ADMIN_CAPS)");
    } else {
    err = cap_disable_effective(ADMIN_CAPS, core::ptr::null_mut());
    if (err)
    ...
    let mut size_tmp: __u32 = sizeof(tmp);
    int err, saved_errno;
...
//
#[no_mangle]
unsafe extern "C" fn cmp_str_seq(log: *const c_char, exp: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_null_insn(insn: *mut bpf_insn) -> bool {
    static bool is_null_insn(struct bpf_insn *insn)
    {
    let mut null_insn: bpf_insn = {};
    return memcmp(insn, &null_insn, sizeof(null_insn)) == 0;
    }
#[no_mangle]
unsafe extern "C" fn is_skip_insn(insn: *mut bpf_insn) -> bool {
    static bool is_skip_insn(struct bpf_insn *insn)
    {
    let mut skip_insn: bpf_insn = SKIP_INSNS();
    return memcmp(insn, &skip_insn, sizeof(skip_insn)) == 0;
    }
#[no_mangle]
unsafe extern "C" fn null_terminated_insn_len(seq: *mut bpf_insn, max_len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compare_masked_insn(orig: *mut bpf_insn, masked: *mut bpf_insn) -> bool {
...
#[no_mangle]
unsafe extern "C" fn find_skip_insn_marker(seq: *mut bpf_insn, len: c_int) -> c_int {
...
//
    ...
    {
    let mut subseqs_len: c_int = null_terminated_insn_len(subseqs, max_subseqs_len);
    while (subseqs_len > 0) {
    let mut skip_idx: c_int = find_skip_insn_marker(subseqs, subseqs_len);
    let mut cur_subseq_len: c_int = skip_idx < 0 ? subseqs_len : skip_idx;
...
#[no_mangle]
unsafe extern "C" fn print_insn(buf: *mut bpf_insn, cnt: c_int) {
...
#[no_mangle]
unsafe extern "C" fn check_xlated_program(test: *mut bpf_test, fd_prog: c_int) -> bool {
    static bool check_xlated_program(struct bpf_test *test, int fd_prog)
    {
    struct bpf_insn *buf;
    unsigned int cnt;
    let mut result: bool = true;
    let mut check_expected: bool = !is_null_insn(test.expected_insns);
    let mut check_unexpected: bool = !is_null_insn(test.unexpected_insns);
...
//
    ...
    opts.log_level = 2 | 4;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: verbose) -> else {
    else if (verbose)
    ...
    fd_prog = bpf_prog_load(prog_type, core::ptr::null_mut(), "GPL", prog, prog_len, &opts);
    saved_errno = errno;
// BPF_PROG_TYPE_TRACING requires more setup and
// bpf_probe_prog_type won't give correct answer
//
    if (fd_prog < 0 && prog_type != BPF_PROG_TYPE_TRACING &&
    !libbpf_probe_bpf_prog_type(prog_type, core::ptr::null_mut())) {
...
#[no_mangle]
unsafe extern "C" fn is_admin() -> bool {
...
#[no_mangle]
unsafe extern "C" fn test_as_unpriv(test: *mut bpf_test) -> bool {
...
#[no_mangle]
unsafe extern "C" fn do_test(unpriv: bool, from: c_uint, to: c_uint) -> c_int {
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    {
    let mut from: c_uint = 0, to = ARRAY_SIZE(tests);
    let mut unpriv: bool = !is_admin();
    let mut arg: c_int = 1;
    ...
    if (argc == 3) {
    let mut l: c_uint = atoi(argv[arg]);
    let mut u: c_uint = atoi(argv[arg + 1]);
    if (l < to && u < to) {
    from = l;
    to   = u + 1;
    }
    } else if (argc == 2) {
    let mut t: c_uint = atoi(argv[arg]);
    if (t < to) {
    from = t;
    to   = t + 1;
    }
