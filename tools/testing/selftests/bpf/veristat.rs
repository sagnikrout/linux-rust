...
//! Source: tools/testing/selftests/bpf/veristat.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct verif_stats {
    pub file_name: *mut c_char,
    pub prog_name: *mut c_char,
    pub stats: [c_long; NUM_STATS_CNT],
}

// joined comparison mode stats
#[repr(C)]
#[derive(Copy, Clone)]
pub struct verif_stats_join {
    pub file_name: *mut c_char,
    pub prog_name: *mut c_char,
    pub stats_a: *const verif_stats,
    pub stats_b: *const verif_stats,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct stat_specs {
    pub spec_cnt: c_int,
    pub ids: [enum stat_id; ALL_STATS_CNT],
    pub variants: [enum stat_variant; ALL_STATS_CNT],
    pub asc: [bool; ALL_STATS_CNT],
    pub abs: [bool; ALL_STATS_CNT],
    pub /: *mut *mut *mut int lens[ALL_STATS_CNT  3]; / 3x for comparison mode,
}
...
#[derive(Copy, Clone)]
pub struct filter {
    pub kind: enum filter_kind,
// FILTER_NAME
    pub any_glob: *mut c_char,
    pub file_glob: *mut c_char,
    pub prog_glob: *mut c_char,
// FILTER_STAT
    pub op: enum operator_kind,
    pub stat_id: c_int,
    pub stat_var: enum stat_variant,
    pub value: c_long,
    pub abs: bool,
}
...
#[derive(Copy, Clone)]
pub struct rvalue {
    pub type: enum { INTEGRAL, ENUMERATOR },
    union {
    pub ivalue: c_longlong,
    pub svalue: *mut c_char,
}

    };
#[repr(C)]
#[derive(Copy, Clone)]
pub struct field_access {
    pub type: enum { FIELD_NAME, ARRAY_INDEX },
    union {
    pub name: *mut c_char,
    pub index: rvalue,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct var_preset {
    pub atoms: *mut field_access,
    pub atom_count: c_int,
...
#[no_mangle]
unsafe extern "C" fn log_errno_aux(file: *const c_char, line: c_int, fmt: *const c_char, ...) -> c_int {
    ...
    { core::ptr::null_mut(), 'h', core::ptr::null_mut(), OPTION_HIDDEN, "Show the full help" },
    { "version", 'V', core::ptr::null_mut(), 0, "Print version" },
    { "verbose", 'v', core::ptr::null_mut(), 0, "Verbose mode" },
    { "debug", 'd', core::ptr::null_mut(), 0, "Debug mode (turns on libbpf debug logging)" },
    { "log-level", 'l', "LEVEL", 0, "Verifier log level (default 0 for normal mode, 1 for verbose mode, 2 for full verification log)" },
    { "log-fixed", OPT_LOG_FIXED, core::ptr::null_mut(), 0, "Disable verifier log rotation" },
    { "log-size", OPT_LOG_SIZE, "BYTES", 0, "Customize verifier log size (default to 16MB)" },
    { "top-n", 'n', "N", 0, "Emit only up to first N results." },
    { "quiet", 'q', core::ptr::null_mut(), 0, "Quiet mode" },
    { "emit", 'e', "SPEC", 0, "Specify stats to be emitted" },
    { "sort", 's', "SPEC", 0, "Specify sort order" },
    { "output-format", 'o', "FMT", 0, "Result output format (table, csv), default is table." },
    { "compare", 'C', core::ptr::null_mut(), 0, "Comparison mode" },
    { "replay", 'R', core::ptr::null_mut(), 0, "Replay mode" },
    { "filter", 'f', "FILTER", 0, "Filter expressions (or @filename for file with expressions)." },
    { "test-states", 't', core::ptr::null_mut(), 0,
    "Force frequent BPF verifier state checkpointing (set BPF_F_TEST_STATE_FREQ program flag)" },
    { "test-reg-invariants", 'r', core::ptr::null_mut(), 0,
...
unsafe extern "C" fn parse_arg(key: c_int, arg: *mut c_char, state: *mut argp_state) -> error_t {
    ...
    case 'l':
    errno = 0;
    env.log_level = strtol(arg, core::ptr::null_mut(), 10);
    if (errno) {
    ...
    case OPT_LOG_SIZE:
    errno = 0;
    env.log_size = strtol(arg, core::ptr::null_mut(), 10);
    if (errno) {
    ...
    case 'n':
    errno = 0;
    env.top_n = strtol(arg, core::ptr::null_mut(), 10);
    if (errno) {
...
#[no_mangle]
pub unsafe extern "C" fn if('!': arg[0] ==) -> else {
    else if (arg[0] == '!')
    ...
    case 'S':
    errno = 0;
    env.top_src_lines = strtol(arg, core::ptr::null_mut(), 10);
    if (errno) {
...
#[no_mangle]
unsafe extern "C" fn glob_matches(str: *const c_char, pat: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_bpf_obj_file(path: *const c_char) -> bool {
    Elf64_Ehdr *ehdr;
    int fd, err = -EINVAL;
    Elf *elf = core::ptr::null_mut();
    fd = open(path, O_RDONLY | O_CLOEXEC);
    if (fd < 0)
    return true; /* we'll fail later and propagate error */
// ensure libelf is initialized
    (void)elf_version(EV_CURRENT);
    elf = elf_begin(fd, ELF_C_READ, core::ptr::null_mut());
    if (!elf)
    goto cleanup;
...
// Exact filter match
#[no_mangle]
unsafe extern "C" fn name_filter_matches(f: *mut filter, filename: *const c_char, prog_name: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn name_filter_may_match(f: *mut filter, filename: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn should_process_file_prog(filename: *const c_char, prog_name: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn append_filter(filters: *mut filter, cnt: *mut c_int, str: *const c_char) -> c_int {
    static int append_filter(struct filter **filters, int *cnt, const char *str)
    {
    struct filter *f;
    void *tmp;
...
#[no_mangle]
unsafe extern "C" fn append_filter_file(path: *const c_char) -> c_int {
    static int append_filter_file(const char *path)
    {
    char buf[1024];
    FILE *f;
...
#[no_mangle]
unsafe extern "C" fn append_file(path: *const c_char) -> c_int {
    static int append_file(const char *path)
    {
    void *tmp;
...
#[no_mangle]
unsafe extern "C" fn append_file_from_file(path: *const c_char) -> c_int {
    static int append_file_from_file(const char *path)
    {
    char buf[1024];
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn is_asc_sym(c: c_char) -> bool {
    static bool is_asc_sym(char c)
    {
    let mut c: return = = '^';
    }
#[no_mangle]
unsafe extern "C" fn is_desc_sym(c: c_char) -> bool {
    static bool is_desc_sym(char c)
    {
    let mut c: return = = 'v' || c == 'V' || c == '.' || c == '!' || c == '_';
    }
...
#[no_mangle]
unsafe extern "C" fn parse_stat(stat_name: *const c_char, specs: *mut stat_specs) -> c_int {
    static int parse_stat(const char *stat_name, struct stat_specs *specs)
    {
    int id;
    let mut has_order: bool = false, is_asc = false, is_abs = false;
    let mut len: usize = strlen(stat_name);
    enum stat_variant var;
...
#[no_mangle]
unsafe extern "C" fn parse_stats(stats_str: *const c_char, specs: *mut stat_specs) -> c_int {
    static int parse_stats(const char *stats_str, struct stat_specs *specs)
    {
    char *input, *state = core::ptr::null_mut(), *next;
    int err, cnt = 0;
    input = strdup(stats_str);
    if (!input)
    return -ENOMEM;
    while ((next = strtok_r(cnt++ ? core::ptr::null_mut() : input, ",", &state))) {
...
#[no_mangle]
unsafe extern "C" fn free_verif_stats(stats: *mut verif_stats, stat_cnt: usize) {
...
pub const MAX_PARSED_LOG_LINES: c_int = 300;
#[no_mangle]
unsafe extern "C" fn parse_verif_log(buf: *const *const c_char, buf_sz: usize, s: *mut verif_stats) -> c_int {
    static int parse_verif_log(char * const buf, size_t buf_sz, struct verif_stats *s)
    {
    const char *cur;
    long sub_stack;
    int pos, lines, cnt = 0;
    char *state = core::ptr::null_mut(), *token, stack[512] = {};
    buf[buf_sz - 1] = '\0';
...
//
    ...
    while ((token = strtok_r(cnt++ ? core::ptr::null_mut() : stack, "+", &state))) {
...
#[no_mangle]
unsafe extern "C" fn print_top_src_lines(buf: *const *const c_char, buf_sz: usize, prog_name: *const c_char) -> c_int {
    static int print_top_src_lines(char * const buf, size_t buf_sz, const char *prog_name)
    {
    let mut lines_cap: c_int = 0;
    let mut lines_size: c_int = 0;
    char **lines = core::ptr::null_mut();
    char *line = core::ptr::null_mut();
    char *state;
    struct line_cnt *freq = core::ptr::null_mut();
    struct line_cnt *cur;
    int unique_lines;
    let mut err: c_int = 0;
    int i;
    while ((line = strtok_r(line ? core::ptr::null_mut() : buf, "\n", &state))) {
    if (strncmp(line, "; ", 2) != 0)
    continue;
    line += 2;
    ...
    const char *src_line = core::ptr::null_mut();
...
//
    { "bpf_raw_tracepoint_args", core::ptr::null_mut(), BPF_PROG_TYPE_RAW_TRACEPOINT },
    };
    int i;
    if (!ctx_name)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn fixup_obj_maps(obj: *mut bpf_object) {
    static void fixup_obj_maps(struct bpf_object *obj)
    {
    struct bpf_map *map;
    bpf_object__for_each_map(map, obj) {
// disable pinning
    bpf_map__set_pin_path(map, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn fixup_obj(obj: *mut bpf_object, prog: *mut bpf_program, filename: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn max_verifier_log_size() -> c_int {
    static int max_verifier_log_size(void)
    {
    let mut SMALL_LOG_SIZE: c_int = UINT_MAX >> 8;
    let mut BIG_LOG_SIZE: c_int = UINT_MAX >> 2;
...
#[no_mangle]
unsafe extern "C" fn output_stat_enabled(id: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn write_one_line(file: *const c_char, fmt: *const c_char, ...) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scanf_one_line(file: *const c_char, fields_expected: c_int, fmt: *const c_char, ...) -> c_int {
    static int scanf_one_line(const char *file, int fields_expected, const char *fmt, ...)
    {
    let mut res: c_int = 0, saved_errno = 0;
    char *line = core::ptr::null_mut();
    size_t line_len;
    va_list ap;
    FILE *f;
...
#[no_mangle]
unsafe extern "C" fn destroy_stat_cgroup() {
...
//
#[no_mangle]
unsafe extern "C" fn create_stat_cgroup() {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !env.quiet) -> else {
    else if (!env.quiet)
...
#[no_mangle]
unsafe extern "C" fn cgroup_memory_peak() -> c_long {
    ...
    buf[err] = 0;
    errno = 0;
    memory_peak = strtoll(buf, core::ptr::null_mut(), 10);
    if (errno) {
...
#[no_mangle]
unsafe extern "C" fn reset_stat_cgroup() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parse_rvalue(val: *const c_char, rvalue: *mut rvalue) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dump(prog_id: __u32, mode: enum dump_mode, file_name: *const c_char, prog_name: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn process_prog(filename: *const c_char, obj: *mut bpf_object, prog: *mut bpf_program) -> c_int {
    ...
    let mut err: c_int = 0, cgroup_err;
    void *tmp;
    int fd;
...
#[no_mangle]
unsafe extern "C" fn append_preset_atom(preset: *mut var_preset, value: *mut c_char, is_index: bool) -> c_int {
    static int append_preset_atom(struct var_preset *preset, char *value, bool is_index)
    {
    struct field_access *tmp;
    let mut i: c_int = preset.atom_count;
    int err;
...
#[no_mangle]
unsafe extern "C" fn parse_var_atoms(full_var: *const c_char, preset: *mut var_preset) -> c_int {
    ...
    preset.atom_count = 0;
    while ((name = strtok_r(preset.atom_count ? core::ptr::null_mut() : expr, ".", &saveptr))) {
    len = strlen(name);
// parse variable name
...
#[no_mangle]
unsafe extern "C" fn append_var_preset(presets: *mut var_preset, cnt: *mut c_int, expr: *const c_char) -> c_int {
    static int append_var_preset(struct var_preset **presets, int *cnt, const char *expr)
    {
    void *tmp;
...
#[no_mangle]
unsafe extern "C" fn append_var_preset_file(filename: *const c_char) -> c_int {
    static int append_var_preset_file(const char *filename)
    {
    char buf[1024];
    FILE *f;
...
#[no_mangle]
unsafe extern "C" fn is_signed_type(t: *const btf_type) -> bool {
...
// retval = e->val;
    ...
    for (i = 0; i < n; ++i, ++e) {
    const char *cur_name = btf__name_by_offset(btf, e.name_off);
    let mut value: __u64 = btf_enum64_value(e);
    if (strcmp(cur_name, evalue) == 0) {
// retval = value;
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn is_preset_supported(t: *const btf_type) -> bool {
    static bool is_preset_supported(const struct btf_type *t)
    {
    return btf_is_int(t) || btf_is_enum(t) || btf_is_enum64(t);
    }
#[no_mangle]
unsafe extern "C" fn find_enum_value(btf: *const btf, name: *const c_char, value: *mut c_longlong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn resolve_rvalue(btf: *mut btf, rvalue: *const rvalue, result: *mut c_longlong) -> c_int {
...
// result = rvalue->ivalue;
    ...
    const struct btf_type *base_type;
    void *ptr;
    let mut value: c_longlong = preset.value.ivalue;
    size_t size;
...
// Check if value fits into the target variable size
    ...
    let mut is_signed: bool = is_signed_type(base_type);
    let mut unsigned_bits: __u32 = sinfo.size * 8 - (is_signed ? 1 : 0);
    let mut max_val: c_longlong = 1ll << unsigned_bits;
    ...
    let mut src_offset: __u8 = sizeof(value) - sinfo.size;
    memcpy(ptr + sinfo.offset, (void *)&value + src_offset, sinfo.size);
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn set_global_vars(obj: *mut bpf_object, presets: *mut var_preset, npresets: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn process_obj(filename: *const c_char) -> c_int {
    ...
    let mut err: c_int = 0, prog_cnt = 0;
    if (!should_process_file_prog(base_filename, core::ptr::null_mut())) {
    if (env.verbose)
...
//
    ...
    let mut v1: c_long = s1.stats[id];
    let mut v2: c_long = s2.stats[id];
    if (abs) {
    v1 = v1 < 0 ? -v1 : v1;
    v2 = v2 < 0 ? -v2 : v2;
    }
...
// num_val = -DBL_MAX;
#[no_mangle]
pub unsafe extern "C" fn if(VERDICT: id ==) -> else {
    else if (id == VERDICT)
...
// num_val = (v2 - v1) * 100.0 / v1;
    ...
    bool asc, bool abs)
    {
    const char *str1 = core::ptr::null_mut(), *str2 = core::ptr::null_mut();
    let mut v1: double = 0.0, v2 = 0.0;
    let mut cmp: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(v2: v1 !=) -> else {
    else if (v1 != v2)
    cmp = v1 < v2 ? -1 : 1;
    return asc ? cmp : -cmp;
    }
#[no_mangle]
unsafe extern "C" fn cmp_join_stats(v1: *const c_void, v2: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn output_header_underlines() {
...
#[no_mangle]
unsafe extern "C" fn output_headers(fmt: enum resfmt) {
    static void output_headers(enum resfmt fmt)
    {
    const char *fmt_str;
    int i, len;
    for (i = 0; i < env.output_spec.spec_cnt; i++) {
    let mut id: c_int = env.output_spec.ids[i];
    int *max_len = &env.output_spec.lens[i];
    switch (fmt) {
    case RESFMT_TABLE_CALCLEN:
    len = snprintf(core::ptr::null_mut(), 0, "%s", stat_defs[id].header);
    if (len > *max_len)
// max_len = len;
    break;
    case RESFMT_TABLE:
...
#[no_mangle]
unsafe extern "C" fn output_stats(s: *const verif_stats, fmt: enum resfmt, last: bool) {
    ...
    case RESFMT_TABLE_CALCLEN:
    if (str)
    len = snprintf(core::ptr::null_mut(), 0, "%s", str);
    else
    len = snprintf(core::ptr::null_mut(), 0, "%ld", val);
    if (len > *max_len)
// max_len = len;
    break;
    case RESFMT_TABLE:
...
#[no_mangle]
unsafe extern "C" fn parse_stat_value(str: *const c_char, id: enum stat_id, st: *mut verif_stats) -> c_int {
    ...
    case PROG_TYPE: {
    let mut prog_type: enum bpf_prog_type = 0;
    const char *type;
    ...
    case ATTACH_TYPE: {
    let mut attach_type: enum bpf_attach_type = 0;
    const char *type;
...
// stat_cntp = 0;
    ...
    char *input = line, *state = core::ptr::null_mut(), *next;
    struct verif_stats *st = core::ptr::null_mut();
    let mut col: c_int = 0, cnt = 0;
    if (!header) {
    void *tmp;
...
// stat_cntp += 1;
    ...
    while ((next = strtok_r(cnt++ ? core::ptr::null_mut() : input, ",\n", &state))) {
    if (header) {
...
// empty/zero stats for mismatched rows
    let mut fallback_stats: static struct verif_stats = { .file_name = "", .prog_name = "" };
#[no_mangle]
unsafe extern "C" fn is_key_stat(id: enum stat_id) -> bool {
    static bool is_key_stat(enum stat_id id)
    {
    let mut id: return = = FILE_NAME || id == PROG_NAME;
    }
#[no_mangle]
unsafe extern "C" fn output_comp_header_underlines() {
    static void output_comp_header_underlines(void)
    {
    int i, j, k;
    for (i = 0; i < env.output_spec.spec_cnt; i++) {
    let mut id: c_int = env.output_spec.ids[i];
    let mut max_j: c_int = is_key_stat(id) ? 1 : 3;
    for (j = 0; j < max_j; j++) {
    let mut len: c_int = env.output_spec.lens[3 * i + j];
...
#[no_mangle]
unsafe extern "C" fn output_comp_headers(fmt: enum resfmt) {
...
// key stats don't have A/B/DIFF columns, they are common for both data sets
    ...
    let mut last: bool = (i == env.output_spec.spec_cnt - 1) && (j == max_j - 1);
    const char *sfx;
    switch (fmt) {
    case RESFMT_TABLE_CALCLEN:
    sfx = is_key_stat(id) ? "" : table_sfxs[j];
    len = snprintf(core::ptr::null_mut(), 0, "%s%s", stat_defs[id].header, sfx);
    if (len > *max_len)
// max_len = len;
    break;
    case RESFMT_TABLE:
...
// max_len - (int)strlen(sfx), stat_defs[id].header, sfx);
    ...
    int i;
    for (i = 0; i < env.output_spec.spec_cnt; i++) {
    let mut id: c_int = env.output_spec.ids[i], len;
    ...
    const char *base_str = core::ptr::null_mut(), *comp_str = core::ptr::null_mut();
    let mut base_val: c_long = 0, comp_val = 0, diff_val = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(base_str, 0: comp_str) ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn cmp_stats_key(base: *const verif_stats, comp: *const verif_stats) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_join_stat_filter_matched(f: *mut filter, stats: *const verif_stats_join) -> bool {
    static bool is_join_stat_filter_matched(struct filter *f, const struct verif_stats_join *stats)
    {
    let mut eps: static double = 1e-9;
    const char *str = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn should_output_join_stats(stats: *const verif_stats_join) -> bool {
...
#[no_mangle]
unsafe extern "C" fn handle_comparison_mode() -> c_int {
    static int handle_comparison_mode(void)
    {
    let mut base_specs: stat_specs = {}, comp_specs = {};
...
#[no_mangle]
unsafe extern "C" fn is_stat_filter_matched(f: *mut filter, stats: *const verif_stats) -> bool {
    static bool is_stat_filter_matched(struct filter *f, const struct verif_stats *stats)
    {
    let mut value: c_long = stats.stats[f.stat_id];
    if (f.abs)
...
#[no_mangle]
unsafe extern "C" fn should_output_stats(stats: *const verif_stats) -> bool {
...
#[no_mangle]
unsafe extern "C" fn output_prog_stats() {
...
#[no_mangle]
unsafe extern "C" fn handle_verif_mode() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn handle_replay_mode() -> c_int {
    static int handle_replay_mode(void)
    {
    let mut specs: stat_specs = {};
    int err;
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    if (argp_parse(&argp, argc, argv, 0, core::ptr::null_mut(), core::ptr::null_mut()))
    return 1;
    if (env.show_version) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: env.replay_mode) -> else {
