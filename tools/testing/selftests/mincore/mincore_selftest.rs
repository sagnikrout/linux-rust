...
//! Source: tools/testing/selftests/mincore/mincore_selftest.c
#![no_std]
...
// Addresses in the specified range are invalid or unmapped
    errno = 0;
    retval = mincore(core::ptr::null_mut(), page_size, vec);
    EXPECT_EQ(-1, retval);
    EXPECT_EQ(ENOMEM, errno);
    errno = 0;
    addr = mmap(core::ptr::null_mut(), page_size, PROT_READ | PROT_WRITE,
...
// <vec> argument points to an illegal address
    errno = 0;
    retval = mincore(addr, page_size, core::ptr::null_mut());
    EXPECT_EQ(-1, retval);
...
//
    errno = 0;
    addr = mmap(core::ptr::null_mut(), FILE_SIZE, PROT_READ | PROT_WRITE,
    MAP_SHARED, fd, 0);
    ASSERT_NE(MAP_FAILED, addr) {
    TH_LOG("mmap error: %s", strerror(errno));
    }
