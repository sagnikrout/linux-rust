...
//! Source: tools/testing/selftests/exec/check-exec.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn drop_privileges(_metadata: *const *const __test_metadata) {
    static void drop_privileges(struct __test_metadata *const _metadata)
    {
    let mut noroot: c_uint = SECBIT_NOROOT | SECBIT_NOROOT_LOCKED;
    cap_t cap_p;
    if ((cap_get_secbits() & noroot) != noroot)
    EXPECT_EQ(0, cap_set_secbits(noroot));
    cap_p = cap_get_proc();
    EXPECT_NE(core::ptr::null_mut(), cap_p);
...
#[no_mangle]
unsafe extern "C" fn test_secbits_set(secbits: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fill_exec_fd(_metadata: *mut __test_metadata, fd_out: c_int) {
...
//
    access_ret = sys_execveat(fd, "", argv, core::ptr::null_mut(),
...
// Tests that we don't get ENOEXEC.
    ...
    {
    let mut exec: c_int = variant.mount_exec && variant.file_exec;
...
// Tests that we don't get ENOEXEC.
    ...
    {
    let mut exec: c_int = variant.file_exec;
...
// Makes sure no exec bits are set.
    ...
    TEST_F(secbits, exec)
    {
    let mut secbits: c_uint = prctl(PR_GET_SECUREBITS);
