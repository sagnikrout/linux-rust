...
//! Source: tools/testing/selftests/exec/nix_origin.bpf.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct scratch {
    pub /: *mut *mut char interp[PATH_MAX]; / PT_INTERP as embedded in the binary,
    pub /: *mut *mut char path[PATH_MAX]; / d_path of the binary, becomes the result,
}
...
#[no_mangle]
unsafe extern "C" fn is_elf64(bprm: *mut linux_binprm) -> bool {
...
#[no_mangle]
unsafe extern "C" fn find_pt_interp(dp: *mut bpf_dynptr, phdr: *mut elf64_phdr) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: nix_origin_match, bprm: *mut linux_binprm) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: nix_origin_load, bprm: *mut linux_binprm) -> c_int {
    ...
    struct scratch *sc;
    __u64 id;
    let mut ret: c_int = -ENOEXEC, len, i;
    if (bpf_dynptr_from_file(bprm.file, 0, &dp))
    goto out;
    if (!find_pt_interp(&dp, &phdr))
    goto out;
...
// Splice the suffix (leading '/' and NUL included) onto the dir.
    ...
    bpf_for(i, 0, sfx) {
    let mut s: __u32 = ORIGIN_LEN + i, d = slash + i;
