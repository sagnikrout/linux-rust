...
//! Source: tools/testing/selftests/exec/binfmt_misc_loader.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn run_memfd(path: *const c_char) -> c_int {
    ...
    pid = fork();
    if (pid == 0) {
    char *argv[] = { PAYLOAD_ARGV0, PAYLOAD_ARG1, PAYLOAD_ARG2, core::ptr::null_mut() };
    char buf[4096];
    int in, mfd;
    ssize_t n;
...
//
#[no_mangle]
unsafe extern "C" fn ptrace_probe(target: *const c_char) -> c_int {
    static int ptrace_probe(const char *target)
    {
    unsigned long auxv[2 * 64], base = 0, entry = 0, at_flags = 0;
    let mut start_code: c_ulong = 0, end_code = 0;
    ...
    raise(SIGSTOP);
    execl(target, PAYLOAD_ARGV0, PAYLOAD_ARG1, PAYLOAD_ARG2, (char *)core::ptr::null_mut());
    _exit(126);
    }
    if (pid < 0)
    return -1;
    ...
    goto fail_kill;
    if (ptrace(PTRACE_SETOPTIONS, pid, core::ptr::null_mut(), (void *)PTRACE_O_TRACEEXEC))
    goto fail_kill;
    if (ptrace(PTRACE_CONT, pid, core::ptr::null_mut(), core::ptr::null_mut()))
    goto fail_kill;
    if (waitpid(pid, &status, 0) != pid || !WIFSTOPPED(status) ||
    ...
    {
    let mut foreign_machine: c_ushort = 0xdead;
