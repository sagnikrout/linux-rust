...
//! Source: tools/testing/selftests/exec/binfmt_misc_bpf.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn check_output(cmd: *const c_char, expected: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn have_binfmt_misc_ops() -> bool {
...
//
    ...
    {
    struct bpf_map *map;
    c.obj = core::ptr::null_mut();
    c.link = core::ptr::null_mut();
    c.entry = entry;
    c.obj = bpf_object__open_file(objfile, core::ptr::null_mut());
    ...
    const char *handler, const char *entry)
    {
    return bpf_case_start_flags(c, objfile, handler, entry, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn bpf_case_stop(c: *mut bpf_case) {
...
// A "$ORIGIN/..." PT_INTERP resolved to an interpreter next to the binary.
    ...
    ASSERT_NE(mkdtemp(dir), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn exec_errno(path: *const c_char) -> c_int {
    ...
    pid = fork();
    if (pid == 0) {
    execl(path, path, (char *)core::ptr::null_mut());
    _exit(errno);
    }
...
#[no_mangle]
unsafe extern "C" fn install_interp(path: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn entry_bind(entry: *const c_char, name: *const c_char, path: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn write_interp_limit(val: *const c_char) -> c_int {
...
// A namespace of its own, with nothing left in it to spend.
    ...
    let mut retval: return = = BIND_NO_BUDGET ? -1 : retval;
    }
...
#[no_mangle]
unsafe extern "C" fn activate(entry: *const c_char) -> c_int {
...
// A second entry for the handler the fixture already published.
    ASSERT_EQ(register_entry("test_born_sealed", "interp_bind", core::ptr::null_mut()), 0);
...
// A binding pins a file: it is charged, and refused once the budget is out.
    ...
    {
    let mut err: c_int = bind_out_of_budget("test_interp_bind", "third", BIND_FIRST);
    if (err < 0)
