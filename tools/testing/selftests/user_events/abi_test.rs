...
//! Source: tools/testing/selftests/user_events/abi_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn wait_for_delete(dir: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn find_multi_event_dir(unique_field: *mut c_char, out_dir: *mut c_char, dir_len: c_int) -> c_int {
    static int find_multi_event_dir(char *unique_field, char *out_dir, int dir_len)
    {
    char path[256];
    glob_t buf;
    int i, ret;
    ret = glob(multi_dir_glob, GLOB_ONLYDIR, core::ptr::null_mut(), &buf);
    if (ret)
    return -1;
    ret = -1;
    ...
    if (!fp)
    continue;
    while (fgets(path, sizeof(path), fp) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn event_exists() -> bool {
    static bool event_exists(void)
    {
    let mut fd: c_int = open(enable_file, O_RDWR);
    if (fd < 0)
    return false;
    close(fd);
    return true;
...
#[no_mangle]
unsafe extern "C" fn change_event(enable: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn wait_for_event_delete() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn reg_enable_flags(enable: *mut c_void, size: c_int, bit: c_int, flags: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn reg_enable(enable: *mut c_void, size: c_int, bit: c_int) -> c_int {
    static int reg_enable(void *enable, int size, int bit)
    {
    return reg_enable_flags(enable, size, bit, 0);
    }
#[no_mangle]
unsafe extern "C" fn reg_disable(enable: *mut c_void, bit: c_int) -> c_int {
    static int reg_disable(void *enable, int bit)
    {
    let mut reg: user_unreg = {0};
    let mut fd: c_int = open(data_file, O_RDWR);
    int ret;
    if (fd < 0)
    return -1;
...
#[no_mangle]
unsafe extern "C" fn clone_check(check: *mut c_void) -> c_int {
