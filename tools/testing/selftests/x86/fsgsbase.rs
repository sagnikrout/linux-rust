...
//! Source: tools/testing/selftests/x86/fsgsbase.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn sigill(sig: c_int, si: *mut siginfo_t, ctx_void: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn rdgsbase() -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn rdfsbase() -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn wrgsbase(gsbase: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn read_base(which: enum which_base) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn check_gs_value(value: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn mov_0_gs(initial_base: c_ulong, schedule: bool) {
...
//
pub const HARD_ZERO: c_uint = 0xa1fa5f343cb85fa4;
#[no_mangle]
unsafe extern "C" fn do_remote_base() {
    static void do_remote_base()
    {
    let mut to_set: c_ulong = remote_base;
    let mut hard_zero: bool = false;
    if (to_set == HARD_ZERO) {
    to_set = 0;
    hard_zero = true;
    }
    ...
    let mut set_thread_area_entry_number: static __thread int = -1;
#[no_mangle]
unsafe extern "C" fn load_gs() -> c_ushort {
...
// 32-bit set_thread_area
    ...
    let mut gs: c_ushort = (unsigned short)((desc.entry_number << 3) | 0x3);
    asm volatile ("mov %0, %%gs" : : "rm" (gs));
    return gs;
    }
    }
#[no_mangle]
pub unsafe extern "C" fn test_wrbase(index: c_ushort, base: c_ulong) {
    ...
    while (ftx != 0)
    syscall(SYS_futex, &ftx, FUTEX_WAIT, 1, core::ptr::null_mut(), core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn test_unexpected_base() {
...
#[no_mangle]
unsafe extern "C" fn test_ptrace_write_gs_read_base() {
    ...
    unsigned long base;
    let mut gs_offset: c_ulong = USER_REGS_OFFSET(gs);
    let mut base_offset: c_ulong = USER_REGS_OFFSET(gs_base);
// Read the initial base.  It should be 1.
    base = ptrace(PTRACE_PEEKUSER, child, base_offset, core::ptr::null_mut());
    if (base == 1) {
...
#[no_mangle]
unsafe extern "C" fn test_ptrace_write_gsbase() {
...
// shared_scratch = load_gs();
    ...
    let mut gs_offset: c_ulong = USER_REGS_OFFSET(gs);
    let mut base_offset: c_ulong = USER_REGS_OFFSET(gs_base);
    gs = ptrace(PTRACE_PEEKUSER, child, gs_offset, core::ptr::null_mut());
...
pub unsafe extern "C" fn main() -> c_int {
    ...
    {
    pthread_t thread;
    shared_scratch = mmap(core::ptr::null_mut(), 4096, PROT_READ | PROT_WRITE,
