...
//! Source: tools/testing/selftests/futex/functional/futex_lock_pi_exiting.c
#![no_std]
...
//
    ...
    {
    let mut park: c_long = (long)arg;
    if (futex_lock_pi(&pi_futex, core::ptr::null_mut(), 0, FUTEX_PRIVATE_FLAG) != 0)
...
//
#[no_mangle]
unsafe extern "C" fn waiter_lock_pi() -> c_int {
    static int waiter_lock_pi(void)
    {
    let mut ret: c_int = futex_lock_pi(&pi_futex, core::ptr::null_mut(), 0, FUTEX_PRIVATE_FLAG);
    let mut ret: return = = 0 ? 0 : errno;
    }
#[no_mangle]
unsafe extern "C" fn outcome_ok(outcome: c_int) -> c_int {
    static int outcome_ok(int outcome)
    {
    let mut outcome: return = = 0 || outcome == EOWNERDEAD || outcome == ESRCH;
    }
...
// Acquire: we become the owner, our TID lands in the futex word.
    ret = futex_lock_pi(&pi_futex, core::ptr::null_mut(), 0, FUTEX_PRIVATE_FLAG);
    ASSERT_EQ(ret, 0)
...
//
    ...
    pthread_barrier_init(&locked_barrier, core::ptr::null_mut(), 2);
    pthread_barrier_init(&release_barrier, core::ptr::null_mut(), 2);
    pi_futex = FUTEX_INITIALIZER;
    waiter_outcome = -1;
    waiter_owns = 0;
    ASSERT_EQ(pthread_create(&owner, core::ptr::null_mut(), owner_thread, (void *)1), 0);
// Wait until the owner actually holds the lock.
    pthread_barrier_wait(&locked_barrier);
// Start the waiter and give it time to block as a real PI waiter.
    ASSERT_EQ(pthread_create(&waiter, core::ptr::null_mut(), waiter_thread, core::ptr::null_mut()), 0);
    usleep(1000);
// Release the owner so it dies while the waiter is queued on it.
    pthread_barrier_wait(&release_barrier);
    pthread_join(waiter, core::ptr::null_mut());
    pthread_join(owner, core::ptr::null_mut());
...
//
    ...
    pthread_barrier_init(&locked_barrier, core::ptr::null_mut(), 2);
    pi_futex = FUTEX_INITIALIZER;
    ASSERT_EQ(pthread_create(&owner, core::ptr::null_mut(), owner_thread, (void *)0), 0);
