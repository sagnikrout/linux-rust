...
//! Source: tools/testing/selftests/futex/functional/robust_list.c
#![no_std]
...
pub const FUTEX_TIMEOUT: c_int = 3;
pub const SLEEP_US: c_int = 100;

    static pthread_barrier_t barrier, barrier2;
#[no_mangle]
unsafe extern "C" fn set_robust_list(head: *mut robust_list_head, len: usize) -> c_int {
    static int set_robust_list(struct robust_list_head *head, size_t len)
    {
    return syscall(SYS_set_robust_list, head, len);
    }
#[no_mangle]
unsafe extern "C" fn get_robust_list(pid: c_int, head: *mut robust_list_head, len_ptr: *mut usize) -> c_int {
    ...
    {
    return syscall(SYS_futex, uaddr, op, val, core::ptr::null_mut(), list_op_pending, val3, 0);
    }
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lock_struct {
    pub futex: atomic_futex_t,
...
//
#[no_mangle]
unsafe extern "C" fn create_child(_metadata: *mut __test_metadata, arg): *mut *mut int (fn)(void, arg: *mut c_void) -> c_int {
    ...
    cargs.arg = arg;
    stack = mmap(core::ptr::null_mut(), STACK_SIZE, PROT_READ | PROT_WRITE,
...
//
#[no_mangle]
unsafe extern "C" fn set_list(head: *mut robust_list_head) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mutex_lock(lock: *mut lock_struct, head: *mut robust_list_head, error_inject: bool) -> c_int {
...
//
    ...
    {
    let mut lock: lock_struct = { .futex = 0 };
    atomic_futex_t *futex = &lock.futex;
    struct robust_list_head head;
    int ret, pid, wstatus;
    ret = set_list(&head);
...
// before the parent
//
    ret = pthread_barrier_init(&barrier, core::ptr::null_mut(), 2);
    ASSERT_EQ(ret, 0);
...
//
    ...
    {
    struct robust_list_head head;
    let mut head_size: usize = sizeof(head);
    int ret;
...
//
    ...
    let mut head_size: usize = sizeof(head), len_ptr;
    int ret;
    ret = set_robust_list(&head, head_size);
    ASSERT_EQ(ret, 0);
...
//
    ...
    pid_t tid;
    ret = pthread_barrier_init(&barrier, core::ptr::null_mut(), 2);
    ret = pthread_barrier_init(&barrier2, core::ptr::null_mut(), 2);
    ASSERT_EQ(ret, 0);
...
//
    ...
    ret = pthread_barrier_init(&barrier, core::ptr::null_mut(), 2);
    ASSERT_EQ(ret, 0);
    ret = pthread_barrier_init(&barrier2, core::ptr::null_mut(), CHILD_NR + 1);
    ASSERT_EQ(ret, 0);
...
//
    ...
    int wstatus;
    pid_t pid;
    pid = create_child(_metadata, child_circular_list, core::ptr::null_mut());
    ASSERT_NE(pid, -1);
    wait(&wstatus);
    EXPECT_EQ(WEXITSTATUS(wstatus), 0)
    TH_LOG("child failed");
...
//
    ...
    struct robust_list_head head;
    let mut exp: uintptr_t = (uintptr_t) core::ptr::null_mut();
    let mut tid: pid_t = gettid();
    int ret;
    if (!self.vdso) {
