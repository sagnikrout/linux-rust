...
//! Source: tools/testing/selftests/futex/functional/futex_priv_hash.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn futex_hash_slots_set_must_fail(_metadata: *mut __test_metadata, slots: c_int) {
...
#[no_mangle]
unsafe extern "C" fn create_max_threads(_metadata: *mut __test_metadata, ): *mut *mut *mut void (thread_fn)(void) {
    static void create_max_threads(struct __test_metadata *_metadata, void *(*thread_fn)(void *))
    {
    int i, ret;
    for (i = 0; i < MAX_THREADS; i++) {
    ret = pthread_create(&threads[i], core::ptr::null_mut(), thread_fn, core::ptr::null_mut());
    ASSERT_EQ(ret, 0)
    TH_LOG("pthread_create failed: %s", strerror(errno));
    }
    }
#[no_mangle]
unsafe extern "C" fn join_max_threads(_metadata: *mut __test_metadata) {
...
pub const SEC_IN_NSEC: c_int = 1000000000;
pub const MSEC_IN_NSEC: c_int = 1000000;
#[no_mangle]
unsafe extern "C" fn futex_dummy_op(_metadata: *mut __test_metadata) {
    static void futex_dummy_op(struct __test_metadata *_metadata)
    {
    let mut lock: pthread_mutex_t = PTHREAD_MUTEX_INITIALIZER;
...
// First thread, expect to be 0, not yet initialized
    ...
    ret = pthread_create(&threads[0], core::ptr::null_mut(), thread_return_fn, core::ptr::null_mut());
    ASSERT_EQ(ret, 0)
    TH_LOG("pthread_create() failed: %d, %s", ret, strerror(errno));
    ret = pthread_join(threads[0], core::ptr::null_mut());
    ASSERT_EQ(ret, 0)
...
// First thread, has to initialize private hash
    ...
    ret = pthread_barrier_init(&barrier_main, core::ptr::null_mut(), MAX_THREADS + 1);
    ASSERT_EQ(ret, 0)
...
// Once the user set something, auto resize must be disabled
    ret = pthread_barrier_init(&barrier_main, core::ptr::null_mut(), MAX_THREADS);
    ASSERT_EQ(ret, 0)
    TH_LOG("pthread_barrier_init failed: %s", strerror(errno));
    create_max_threads(_metadata, thread_lock_fn);
    join_max_threads(_metadata);
