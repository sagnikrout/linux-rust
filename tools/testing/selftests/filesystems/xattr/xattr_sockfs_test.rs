...
//! Source: tools/testing/selftests/filesystems/xattr/xattr_sockfs_test.c
#![no_std]
...
pub const XATTR_SIZE_MAX: c_int = 65536;
...
//
    ...
    let mut found_proto: bool = false;
    ret = fsetxattr(self.sockfd, TEST_XATTR_NAME,
    TEST_XATTR_VALUE, strlen(TEST_XATTR_VALUE), 0);
    ASSERT_EQ(ret, 0) {
    TH_LOG("fsetxattr failed: %s", strerror(errno));
    ...
    ASSERT_EQ(ret, 0);
    ret = fgetxattr(self.sockfd, TEST_XATTR_NAME, core::ptr::null_mut(), 0);
    ASSERT_EQ(ret, 0);
    }
...
//
    ...
    ASSERT_NE(value, core::ptr::null_mut());
