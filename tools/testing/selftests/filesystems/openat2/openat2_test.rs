...
//! Source: tools/testing/selftests/filesystems/openat2/openat2_test.c
#![no_std]
...
pub const O_LARGEFILE: c_uint = 0x20000;

pub const O_LARGEFILE: c_uint = 0x8000;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct open_how_ext {
    pub inner: open_how,
    pub extra1: u32,
    pub pad1: [c_char; 128],
    pub extra2: u32,
...
// Bigger struct, with non-zero trailing bytes.
    ...
    let mut how_ext: open_how_ext = test.arg;
    for (int j = 0; j < ARRAY_SIZE(misalignments); j++) {
    int fd, misalign = misalignments[j];
    void *copy = core::ptr::null_mut(), *how_copy = &how_ext;
    char *fdpath = core::ptr::null_mut();
    if (misalign) {
//
...
// currently unknown upper 32 bit rejected.
    ...
    for (int i = 0; i < ARRAY_SIZE(tests); i++) {
    int fd, fdflags = -1;
    char *path, *fdpath = core::ptr::null_mut();
