...
//! Source: tools/testing/selftests/filesystems/empty_mntns/clone3_empty_mntns_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn clone3_empty_mntns(extra_flags: u64) -> pid_t {
...
#[no_mangle]
unsafe extern "C" fn clone3_empty_mntns_supported() -> bool {
...
//
    ...
    _exit(2);
    if (mount(core::ptr::null_mut(), "/", core::ptr::null_mut(), MS_REC | MS_PRIVATE, core::ptr::null_mut()))
    _exit(3);
    if (!mkdtemp(tmpdir))
    _exit(4);
    ...
    _exit(6);
    if (mount(subdir, subdir, core::ptr::null_mut(), MS_BIND, core::ptr::null_mut()))
    _exit(7);
    }
    if (count_mounts() < 5)
    _exit(8);
