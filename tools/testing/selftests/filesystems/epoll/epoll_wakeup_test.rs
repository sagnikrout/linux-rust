...
//! Source: tools/testing/selftests/filesystems/epoll/epoll_wakeup_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn kill_timeout(ctx: *mut epoll_mtcontext) {
...
//
    ...
    ASSERT_EQ(pthread_create(&ctx.waiter, core::ptr::null_mut(), waiter_entry1a, &ctx), 0);
    ASSERT_EQ(pthread_create(&emitter, core::ptr::null_mut(), emitter_entry1, &ctx), 0);
    if (epoll_wait(ctx.efd[0], &e, 1, -1) > 0)
    __sync_fetch_and_add(&ctx.count, 1);
    ASSERT_EQ(pthread_join(ctx.waiter, core::ptr::null_mut()), 0);
    EXPECT_EQ(ctx.count, 2);
    if (pthread_tryjoin_np(emitter, core::ptr::null_mut()) < 0) {
    pthread_kill(emitter, SIGUSR1);
    pthread_join(emitter, core::ptr::null_mut());
    }
    close(ctx.efd[0]);
    close(ctx.sfd[0]);
    close(ctx.sfd[1]);
...
//
    ...
    ctx.main = pthread_self();
    ASSERT_EQ(pthread_create(&ctx.waiter, core::ptr::null_mut(), waiter_entry2a, &ctx), 0);
    ASSERT_EQ(pthread_create(&emitter, core::ptr::null_mut(), emitter_entry2, &ctx), 0);
    if (epoll_wait(ctx.efd[0], events, 2, -1) > 0)
    __sync_fetch_and_add(&ctx.count, 1);
    ASSERT_EQ(pthread_join(ctx.waiter, core::ptr::null_mut()), 0);
    EXPECT_EQ(ctx.count, 2);
...
//
    ...
    ASSERT_EQ(epoll_ctl(ctx.efd[0], EPOLL_CTL_ADD, ctx.sfd[0], &e), 0);
    ctx.main = pthread_self();
    ASSERT_EQ(pthread_create(&ctx.waiter, core::ptr::null_mut(), waiter_entry1ap, &ctx), 0);
    ASSERT_EQ(pthread_create(&emitter, core::ptr::null_mut(), emitter_entry1, &ctx), 0);
    if (epoll_wait(ctx.efd[0], &e, 1, -1) > 0)
    __sync_fetch_and_add(&ctx.count, 1);
    ASSERT_EQ(pthread_join(ctx.waiter, core::ptr::null_mut()), 0);
...
//
    ...
    ASSERT_EQ(epoll_ctl(ctx.efd[0], EPOLL_CTL_ADD, ctx.sfd[2], events), 0);
    ctx.main = pthread_self();
    ASSERT_EQ(pthread_create(&ctx.waiter, core::ptr::null_mut(), waiter_entry2ap, &ctx), 0);
    ASSERT_EQ(pthread_create(&emitter, core::ptr::null_mut(), emitter_entry2, &ctx), 0);
    if (epoll_wait(ctx.efd[0], events, 2, -1) > 0)
    __sync_fetch_and_add(&ctx.count, 1);
    ASSERT_EQ(pthread_join(ctx.waiter, core::ptr::null_mut()), 0);
...
//
    ...
    ASSERT_EQ(epoll_ctl(ctx.efd[0], EPOLL_CTL_ADD, ctx.efd[1], &e), 0);
    ctx.main = pthread_self();
    ASSERT_EQ(pthread_create(&ctx.waiter, core::ptr::null_mut(), waiter_entry1o, &ctx), 0);
    ASSERT_EQ(pthread_create(&emitter, core::ptr::null_mut(), emitter_entry1, &ctx), 0);
    if (epoll_wait(ctx.efd[1], &e, 1, -1) > 0)
...
//
    ...
    ASSERT_EQ(epoll_ctl(ctx.efd[0], EPOLL_CTL_ADD, ctx.efd[1], &e), 0);
    ctx.main = pthread_self();
    ASSERT_EQ(pthread_create(&ctx.waiter, core::ptr::null_mut(), waiter_entry1op, &ctx), 0);
    ASSERT_EQ(pthread_create(&emitter, core::ptr::null_mut(), emitter_entry1, &ctx), 0);
    if (epoll_wait(ctx.efd[1], &e, 1, -1) > 0)
    __sync_fetch_and_or(&ctx.count, 2);
    ASSERT_EQ(pthread_join(ctx.waiter, core::ptr::null_mut()), 0);
...
//
    ...
    e.events = EPOLLIN | EPOLLERR | EPOLLET;
    ASSERT_EQ(epoll_ctl(ctx.efd[0], EPOLL_CTL_ADD, ctx.sfd[0], &e), 0);
    ASSERT_EQ(pthread_create(&emitter, core::ptr::null_mut(), epoll59_thread, &ctx), 0);
    for (i = 0; i < 100000; i++) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct epoll60_ctx {
    pub stopped: volatile int,
    pub ready: c_int,
    pub waiters: c_int,
    pub epfd: c_int,
    pub evfd: [c_int; EPOLL60_EVENTS_NR],
}
...
// Block SIGUSR1
    ...
    sigprocmask(SIG_SETMASK, &sigmask, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn msecs() -> c_ulonglong {
...
#[no_mangle]
pub unsafe extern "C" fn count_waiters(ctx: *mut epoll60_ctx) -> c_int {
...
// Create waiter threads
    ...
    ASSERT_EQ(pthread_create(&waiters[i], core::ptr::null_mut(),
...
// Stop waiters
    ...
    for (i = 0; i < ARRAY_SIZE(waiters); i++)
    pthread_join(waiters[i], core::ptr::null_mut());
    for (i = 0; i < ARRAY_SIZE(waiters); i++)
    close(ctx.evfd[i]);
    close(ctx.epfd);
    }
...
// be joinable.
//
    ASSERT_EQ(pthread_create(&threads[0], core::ptr::null_mut(),
    epoll61_write_eventfd, &ctx), 0);
    ASSERT_EQ(pthread_create(&threads[1], core::ptr::null_mut(),
    epoll61_epoll_with_timeout, &ctx), 0);
    ASSERT_EQ(pthread_create(&threads[2], core::ptr::null_mut(),
    epoll61_blocking_epoll, &ctx), 0);
    for (n = 0; n < ARRAY_SIZE(threads); ++n)
    ASSERT_EQ(pthread_join(threads[n], core::ptr::null_mut()), 0);
    }
    close(ctx.epfd);
    close(ctx.evfd);
    }
// Equivalent to basic test epoll1, but exercising epoll_pwait2.
    ...
    ASSERT_EQ(epoll_ctl(efd, EPOLL_CTL_ADD, sfd[0], &e), 0);
    ASSERT_EQ(write(sfd[1], "w", 1), 1);
    EXPECT_EQ(sys_epoll_pwait2(efd, &e, 1, core::ptr::null_mut(), core::ptr::null_mut(), 0), 1);
    EXPECT_EQ(sys_epoll_pwait2(efd, &e, 1, core::ptr::null_mut(), core::ptr::null_mut(), 0), 1);
    close(efd);
    close(sfd[0]);
    close(sfd[1]);
...
// Epoll_pwait2 basic timeout test.
    ...
    tdiff = msecs();
    EXPECT_EQ(sys_epoll_pwait2(efd, &e, 1, &ts, core::ptr::null_mut(), 0), 0);
    tdiff = msecs() - tdiff;
...
//
    ctx.main = pthread_self();
    ASSERT_EQ(pthread_create(&waiter[0], core::ptr::null_mut(), waiter_entry1a, &ctx), 0);
    ASSERT_EQ(pthread_create(&waiter[1], core::ptr::null_mut(), waiter_entry1a, &ctx), 0);
    usleep(100000);
    ASSERT_EQ(write(ctx.sfd[1], "w", 1), 1);
    ASSERT_EQ(pthread_join(waiter[0], core::ptr::null_mut()), 0);
    ASSERT_EQ(pthread_join(waiter[1], core::ptr::null_mut()), 0);
    EXPECT_EQ(ctx.count, 2);
    close(ctx.efd[0]);
    close(ctx.sfd[0]);
    close(ctx.sfd[1]);
    ...
    ASSERT_EQ(pthread_create(&threads[i], core::ptr::null_mut(), epoll65_wait, &ctx), 0);
    for (i = 0; i < ARRAY_SIZE(threads); ++i) {
