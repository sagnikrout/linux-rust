...
//! Source: tools/testing/selftests/filesystems/fsmount_ns/fsmount_ns_test.c
#![no_std]
...
pub const FSMOUNT_NAMESPACE: c_uint = 0x00000002;
...
#[no_mangle]
unsafe extern "C" fn get_mnt_ns_id(fd: c_int, mnt_ns_id: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_mnt_ns_id_from_path(path: *const c_char, mnt_ns_id: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn log_mount(_metadata: *mut __test_metadata, sm: *mut statmount) {
...
#[no_mangle]
unsafe extern "C" fn dump_mounts(_metadata: *mut __test_metadata, mnt_ns_id: u64) {
...
#[no_mangle]
unsafe extern "C" fn create_tmpfs_fd() -> c_int {
    ...
    if (fs_fd < 0)
    return -errno;
    ret = sys_fsconfig(fs_fd, FSCONFIG_CMD_CREATE, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (ret < 0) {
    close(fs_fd);
    return -errno;
    }
...
//
    fs_fd = sys_fsopen("tmpfs", FSOPEN_CLOEXEC);
    ASSERT_GE(fs_fd, 0);
    ASSERT_EQ(sys_fsconfig(fs_fd, FSCONFIG_CMD_CREATE, core::ptr::null_mut(), core::ptr::null_mut(), 0), 0);
    pid = fork();
    ASSERT_GE(pid, 0);
    if (pid == 0) {
    int fd;
...
// Now we have CAP_SYS_ADMIN in the user namespace
    ...
    if (fs_fd < 0)
    _exit(3);
    if (sys_fsconfig(fs_fd, FSCONFIG_CMD_CREATE, core::ptr::null_mut(), core::ptr::null_mut(), 0) < 0) {
    close(fs_fd);
    _exit(4);
    }
    fd = sys_fsmount(fs_fd, FSMOUNT_NAMESPACE | FSMOUNT_CLOEXEC, 0);
...
// Wrong error
    ...
    _exit(1);
    if (sys_mount(core::ptr::null_mut(), "/", core::ptr::null_mut(), MS_SLAVE | MS_REC, core::ptr::null_mut()) != 0)
    _exit(1);
    fs_fd = sys_fsopen("tmpfs", FSOPEN_CLOEXEC);
    if (fs_fd < 0)
    _exit(3);
