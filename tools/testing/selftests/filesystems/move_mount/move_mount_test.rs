...
//! Source: tools/testing/selftests/filesystems/move_mount/move_mount_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn get_unique_mnt_id_fd(fd: c_int) -> u64 {
...
//
#[no_mangle]
unsafe extern "C" fn setup_locked_overmount() -> c_int {
    static int setup_locked_overmount(void)
    {
// Isolate so mounts don't leak.
    if (unshare(CLONE_NEWNS))
    return 1;
    if (mount("", "/", core::ptr::null_mut(), MS_REC | MS_PRIVATE, core::ptr::null_mut()))
    return 2;
//
...
//
    ...
    return 3;
// Mount tmpfs A
    if (mount("tmpfs", "/mnt_dir", "tmpfs", 0, core::ptr::null_mut()))
    return 4;
...
//
#[no_mangle]
unsafe extern "C" fn create_detached_tmpfs() -> c_int {
    ...
    if (fs_fd < 0)
    return -1;
    if (sys_fsconfig(fs_fd, FSCONFIG_CMD_CREATE, core::ptr::null_mut(), core::ptr::null_mut(), 0)) {
    close(fs_fd);
    return -1;
    }
...
//
    ...
    rmdir("/chroot_dir");
    ASSERT_EQ(mkdir("/chroot_dir", 0755), 0);
    ASSERT_EQ(mount("tmpfs", "/chroot_dir", "tmpfs", 0, core::ptr::null_mut()), 0);
    chroot_id = get_unique_mnt_id("/chroot_dir");
    ASSERT_NE(chroot_id, 0);
    ASSERT_EQ(chdir("/chroot_dir"), 0);
    ASSERT_EQ(chroot("."), 0);
