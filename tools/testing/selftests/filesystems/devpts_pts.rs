...
//! Source: tools/testing/selftests/filesystems/devpts_pts.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn terminal_dup2(duplicate: c_int, original: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn terminal_set_stdfds(fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn login_pty(fd: c_int) -> c_int {
    static int login_pty(int fd)
    {
    int ret;
    setsid();
    ret = ioctl(fd, TIOCSCTTY, core::ptr::null_mut());
    if (ret < 0)
    return -1;
...
#[no_mangle]
unsafe extern "C" fn resolve_procfd_symlink(fd: c_int, buf: *mut c_char, buflen: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_tiocgptpeer(ptmx: *mut c_char, expected_procfd_contents: *mut c_char) -> c_int {
    static int do_tiocgptpeer(char *ptmx, char *expected_procfd_contents)
    {
    int ret;
    let mut master: c_int = -1, slave = -1, fret = -1;
...
#[no_mangle]
unsafe extern "C" fn verify_non_standard_devpts_mount() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn verify_ptmx_bind_mount() -> c_int {
    static int verify_ptmx_bind_mount(void)
    {
    int ret;
    ret = mount("/dev/pts/ptmx", "/dev/ptmx", core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn verify_invalid_ptmx_bind_mount() -> c_int {
    ...
    ret = mount("/dev/pts/ptmx", ptmx, core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    close(mntpoint_fd);
    if (ret < 0) {
    fprintf(stderr, "Failed to bind mount \"/dev/pts/ptmx\" to "
    "\"%s\" mount namespace\n", ptmx);
