...
//! Source: tools/testing/selftests/filesystems/fscontext_ns/fscontext_ns_test.c
#![no_std]
...
pub const TAINT_WARN_BIT: c_int = 9;
#[no_mangle]
unsafe extern "C" fn taint_warn_set() -> bool {
...
unsafe extern "C" fn send_fd(sock: c_int, fd: c_int) -> c_int {
    ...
    char b[1] = { 'x' };
    let mut iov: iovec = { .iov_base = b, .iov_len = sizeof(b) };
    struct msghdr msg = {
    .msg_iov	= &iov,
    .msg_iovlen	= 1,
    .msg_control	= cmsgbuf,
...
//
#[no_mangle]
unsafe extern "C" fn create_from_child(fsname: *const c_char, nest: bool) -> c_int {
    ...
    return -ENODATA;
    errno = 0;
    ret = sys_fsconfig(fd, FSCONFIG_CMD_CREATE, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    status = errno;
    close(fd);
    errno = status;
    return ret;
