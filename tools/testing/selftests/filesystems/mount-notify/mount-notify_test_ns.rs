...
//! Source: tools/testing/selftests/filesystems/mount-notify/mount-notify_test_ns.c
#![no_std]
...
// Needed for linux/fanotify.h
    ...
    ASSERT_NE(mkdtemp(self.root_mntpoint), core::ptr::null_mut());
    self.orig_root = open("/", O_PATH | O_CLOEXEC);
    ASSERT_GE(self.orig_root, 0);
    ASSERT_EQ(mount("tmpfs", self.root_mntpoint, "tmpfs", 0, core::ptr::null_mut()), 0);
    ...
    let mut fan_fd: c_int = fanotify_init(FAN_REPORT_FID, 0);
...
// Verify that watching mntns where group was created is allowed
    ...
    self.ns_fd, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
// ...but watching orig mntns is not allowed
    ret = fanotify_mark(self.fan_fd[i], FAN_MARK_ADD |
    FAN_MARK_MNTNS,
    FAN_MNT_ATTACH | FAN_MNT_DETACH,
    self.orig_ns_fd, core::ptr::null_mut());
    ASSERT_NE(ret, 0);
...
// the above it means that the list1 and list2 represent the same sets.
    ...
    ASSERT_NE(list, core::ptr::null_mut());
    ...
    {
    int ret, i;
    ret = mount("", "/", core::ptr::null_mut(), MS_SHARED, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
...
// Cleanup
    ...
    uint64_t mnts[2] = { self.root_id };
    uint64_t move_id;
    ret = mount("/", "/a", core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
    mnts[1] = expect_notify_mask(_metadata, self, FAN_MNT_ATTACH);
    ASSERT_NE(mnts[0], mnts[1]);
    check_mounted(_metadata, mnts, 2);
...
// Cleanup
    ...
    ASSERT_EQ(ret, 0);
    ret = mount("", "/", core::ptr::null_mut(), MS_PRIVATE, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
...
// Create setup with a[1] -> b[2] propagation
    ret = mount("/", "/a", core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
    ret = mount("", "/a", core::ptr::null_mut(), MS_SHARED, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
    ret = mount("/a", "/b", core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
    ret = mount("", "/b", core::ptr::null_mut(), MS_SLAVE, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
...
// Mount on b[5], not propagated
    ret = mount("/", "/b", core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
...
// Cleanup
    ...
    chdir("/");
    unshare(CLONE_NEWNS);
    mount("", "/", core::ptr::null_mut(), MS_REC|MS_PRIVATE, core::ptr::null_mut());
...
// Cleanup
    ...
    uint64_t mnts2[3];
    int ret;
    ret = mount("tmpfs", "/a", "tmpfs", 0, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
    ...
    ASSERT_EQ(ret, 0);
    ret = mount("/a", "/a/new", core::ptr::null_mut(), MS_BIND, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
    mnts[1] = expect_notify_mask(_metadata, self, FAN_MNT_ATTACH);
    check_mounted(_metadata, mnts, 3);
