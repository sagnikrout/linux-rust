...
//! Source: tools/testing/selftests/filesystems/open_tree_ns/open_tree_ns_test.c
#![no_std]
...
// Check if open_tree syscall is supported
    ret = sys_open_tree(-1, core::ptr::null_mut(), 0);
...
// Create a temporary directory for the test mount
    ...
    ASSERT_NE(mkdtemp(self.tmpdir), core::ptr::null_mut());
// Mount tmpfs there
    ret = mount("tmpfs", self.tmpdir, "tmpfs", 0, core::ptr::null_mut());
    if (ret < 0) {
    rmdir(self.tmpdir);
    SKIP(return, "Failed to mount tmpfs");
    }
    self.mounted = true;
    ret = mount(core::ptr::null_mut(), self.tmpdir, core::ptr::null_mut(), MS_UNBINDABLE, core::ptr::null_mut());
    if (ret < 0) {
    rmdir(self.tmpdir);
    ...
    int fd;
    ssize_t i;
    let mut found_unbindable: bool = false;
    fd = sys_open_tree(AT_FDCWD, "/",
    OPEN_TREE_NAMESPACE | AT_RECURSIVE | OPEN_TREE_CLOEXEC);
    ASSERT_GT(fd, 0);
