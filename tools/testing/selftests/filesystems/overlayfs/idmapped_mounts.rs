...
//! Source: tools/testing/selftests/filesystems/overlayfs/idmapped_mounts.c
#![no_std]
...
//
pub const ID_NS: c_int = 0;
pub const ID_HOST: c_int = 10000;
pub const ID_RANGE: c_int = 10000;
//
...
//
pub const LAYER_HOST: c_int = 20000;
...
#[no_mangle]
unsafe extern "C" fn ovl_supported() -> bool {
    static bool ovl_supported(void)
    {
    let mut fd: c_int = sys_fsopen("overlay", 0);
    if (fd < 0)
    return false;
    close(fd);
    return true;
...
#[no_mangle]
unsafe extern "C" fn setup_layers(base: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_mount(base: *const c_char, nfs_export: bool) -> c_int {
    ...
    goto err;
    if (sys_fsconfig(fsfd, FSCONFIG_CMD_CREATE, core::ptr::null_mut(), core::ptr::null_mut(), 0))
    goto err;
...
#[no_mangle]
unsafe extern "C" fn ovl_idmap(mfd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn idmapped_layer_fd(path: *const c_char, nsid: c_int, hostid: c_int, range: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_mount_lower_fd(upper: *const c_char, work: *const c_char, fd_lower: c_int) -> c_int {
    ...
    sys_fsconfig(fsfd, FSCONFIG_SET_STRING, "upperdir", upper, 0) ||
    sys_fsconfig(fsfd, FSCONFIG_SET_STRING, "workdir", work, 0) ||
    sys_fsconfig(fsfd, FSCONFIG_SET_FD, "lowerdir+", core::ptr::null_mut(), fd_lower) ||
    sys_fsconfig(fsfd, FSCONFIG_CMD_CREATE, core::ptr::null_mut(), core::ptr::null_mut(), 0))
    goto err;
    ovl = sys_fsmount(fsfd, 0, 0);
    close(fsfd);
    return ovl;
...
//
#[no_mangle]
unsafe extern "C" fn userns_overlay_child(u1: c_int) -> c_int {
...
// Become root in the overlay sb's user namespace u1.
    ...
    sys_mount(core::ptr::null_mut(), "/", core::ptr::null_mut(), MS_SLAVE | MS_REC, core::ptr::null_mut()))
    return fprintf(stderr, "userns: unshare/slave: %m\n"), -1;
    if (sys_mount("tmpfs", "/tmp", "tmpfs", 0, core::ptr::null_mut()))
...
// u1 backs the overlay sb: identity-mapped, but not the init ns.
    ...
    ASSERT_GE(pid, 0);
    if (pid == 0) {
    let mut ret: c_int = userns_overlay_child(u1);
