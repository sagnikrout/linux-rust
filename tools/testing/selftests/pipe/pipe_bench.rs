...
//! Source: tools/testing/selftests/pipe/pipe_bench.c
#![no_std]
...
pub const HIST_BUCKETS: c_int = 32;
    let mut g_msgsize: static size_t = 16 * 4096;
    let mut g_duration: static int = 3;
    let mut g_pipe_size: static int = 1024 * 1024;
...
#[no_mangle]
pub unsafe extern "C" fn now_ns() -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn log2_bucket(v: u64) -> c_int {
    ...
    let mut t0: u64 = now_ns();
    let mut n: isize = write(g_pipe[1], s.buf, g_msgsize);
    let mut dt: u64 = now_ns() - t0;
    if (n > 0) {
    s.writes++;
    s.bytes += (uint64_t)n;
    s.lat_sum_ns += dt;
...
//
    for (;;) {
    let mut n: isize = read(g_pipe[0], s.buf, g_msgsize);
    if (n <= 0)
    break;
    }
    return core::ptr::null_mut();
...
// p99 = bucket_at(agg, p99_target);
    ...
    uint64_t p50, uint64_t p99)
    {
    let mut sec: double = g_duration;
    let mut avg_ns: u64 = t.writes ? t.lat_sum / t.writes : 0;
...
#[no_mangle]
unsafe extern "C" fn summarize(all: *mut wstats, nw: c_int, nr: c_int) {
...
#[no_mangle]
unsafe extern "C" fn stress_ng_child(hs_wr: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn stress_ng_wait_handshake(hs_rd: c_int, pid: pid_t) -> c_int {
    static int stress_ng_wait_handshake(int hs_rd, pid_t pid)
    {
    let mut pfd: pollfd = { .fd = hs_rd, .events = POLLIN };
    char errbuf[256];
    int status;
    int ret;
    ret = poll(&pfd, 1, 500);
    if (ret <= 0)
    return 0;
    let mut n: isize = read(hs_rd, errbuf, sizeof(errbuf) - 1);
    if (n > 0) {
    errbuf[n] = '\0';
    fputs(errbuf, stderr);
...
#[no_mangle]
unsafe extern "C" fn spawn_stress_ng() -> pid_t {
...
#[no_mangle]
unsafe extern "C" fn kill_stress_ng(pid: pid_t) {
...
//
    ...
    {
    for (int i = 0; i < nr; i++) {
    let mut err: c_int = pthread_create(&rt[i], core::ptr::null_mut(), reader, &rs[i]);
    if (err) {
    ...
    {
    for (int i = 0; i < nw; i++) {
    let mut err: c_int = pthread_create(&wt[i], core::ptr::null_mut(), writer, &ws[i]);
    if (err) {
...
#[no_mangle]
unsafe extern "C" fn open_bench_pipe() -> c_int {
...
//
    ...
    pthread_join(wt[i], core::ptr::null_mut());
    for (int i = 0; i < nr_created; i++)
    pthread_join(rt[i], core::ptr::null_mut());
    if (rc == 0)
    close(g_pipe[0]);
    }
#[no_mangle]
unsafe extern "C" fn run_one(nw: c_int, nr: c_int) -> c_int {
    static int run_one(int nw, int nr)
    {
    pthread_t *wt = core::ptr::null_mut(), *rt = core::ptr::null_mut();
    struct wstats *ws = core::ptr::null_mut();
    struct rstats *rs = core::ptr::null_mut();
    let mut nw_created: c_int = 0, nr_created = 0;
    let mut rc: c_int = 0;
...
unsafe extern "C" fn usage(prog: *const c_char) {
    ...
    {"memory-pressure", no_argument, core::ptr::null_mut(), 'M'},
    {0, 0, 0, 0},
    };
    int opt;
    while ((opt = getopt_long(argc, argv, "w:r:s:d:p:",
    long_opts, core::ptr::null_mut())) != -1) {
    switch (opt) {
    case 'w':
...
//
#[no_mangle]
unsafe extern "C" fn validate_args() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn run_sweep() -> c_int {
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    {
    let mut writers_override: c_int = 0, readers_override = 0;
    let mut stress_pid: pid_t = -1;
    let mut rc: c_int = 0;
    ...
    setvbuf(stderr, core::ptr::null_mut(), _IOLBF, 0);
    ...
    let mut nw: c_int = writers_override > 0 ? writers_override : 1;
    let mut nr: c_int = readers_override > 0 ? readers_override : 1;
