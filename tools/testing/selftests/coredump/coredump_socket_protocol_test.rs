...
//! Source: tools/testing/selftests/coredump/coredump_socket_protocol_test.c
#![no_std]
...
pub const NUM_CRASHING_COREDUMPS: c_int = 5;
...
// This should never happen
    ...
    let mut req: coredump_req = {};
    let mut fd_server: c_int = -1, fd_coredump = -1, fd_core_file = -1, fd_peer_pidfd = -1;
    ...
    if (pid_coredump_server == 0) {
    let mut req: coredump_req = {};
    let mut fd_server: c_int = -1, fd_coredump = -1, fd_peer_pidfd = -1;
    let mut exit_code: c_int = EXIT_FAILURE;
    close(ipc_sockets[0]);
    fd_server = create_and_listen_unix_socket("/tmp/coredump.socket");
    if (fd_server < 0) {
...
// Verify coredump_signal is available and correct
    ...
    ASSERT_GE(pid_coredump_server, 0);
    if (pid_coredump_server == 0) {
    let mut fd_server: c_int = -1, fd_coredump = -1, fd_peer_pidfd = -1, fd_core_file = -1;
    let mut exit_code: c_int = EXIT_FAILURE;
    let mut req: coredump_req = {};
    close(ipc_sockets[0]);
    fd_server = create_and_listen_unix_socket("/tmp/coredump.socket");
    ...
    ASSERT_GE(pid_coredump_server, 0);
    if (pid_coredump_server == 0) {
    let mut fd_server: c_int = -1, exit_code = EXIT_FAILURE, n_conns = 0;
    fd_server = -1;
    ...
    let mut fd_coredump: c_int = -1, fd_peer_pidfd = -1, fd_core_file = -1;
    let mut req: coredump_req = {};
    fd_coredump = accept4(fd_server, core::ptr::null_mut(), core::ptr::null_mut(), SOCK_CLOEXEC);
    if (fd_coredump < 0) {
