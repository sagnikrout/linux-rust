...
//! Source: tools/testing/selftests/alsa/pcm-test.c
#![no_std]
...
// Lock while reporting results since kselftest doesn't
    let mut results_lock: pthread_mutex_t = PTHREAD_MUTEX_INITIALIZER;
    enum test_class {
    TEST_CLASS_DEFAULT,
    TEST_CLASS_SYSTEM,
    };
#[no_mangle]
pub unsafe extern "C" fn timestamp_now(tstamp: *mut timestamp_t) {
...
#[no_mangle]
pub unsafe extern "C" fn timestamp_diff_ms(tstamp: *mut timestamp_t) -> c_longlong {
...
#[no_mangle]
unsafe extern "C" fn device_from_id(node: *mut snd_config_t) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn missing_device(card: c_int, device: c_int, subdevice: c_int, stream: snd_pcm_stream_t) {
    static void missing_device(int card, int device, int subdevice, snd_pcm_stream_t stream)
    {
    struct pcm_data *pcm_data;
    for (pcm_data = pcm_list; pcm_data != core::ptr::null_mut(); pcm_data = pcm_data.next) {
...
#[no_mangle]
unsafe extern "C" fn missing_devices(card: c_int, card_config: *mut snd_config_t) {
    ...
    pcm_config = conf_get_subtree(card_config, "pcm", core::ptr::null_mut());
    if (!pcm_config)
    return;
    ...
    if (subdevice < 0)
    continue;
    if (conf_get_subtree(node2, "PLAYBACK", core::ptr::null_mut()))
    missing_device(card, device, subdevice, SND_PCM_STREAM_PLAYBACK);
    if (conf_get_subtree(node2, "CAPTURE", core::ptr::null_mut()))
    missing_device(card, device, subdevice, SND_PCM_STREAM_CAPTURE);
    }
    }
    }
#[no_mangle]
unsafe extern "C" fn find_pcms() {
    ...
    pcm_config = conf_get_subtree(card_config, key, core::ptr::null_mut());
    ...
    pcm_data.stream = stream;
    pcm_data.pcm_config = conf_get_subtree(card_config, key, core::ptr::null_mut());
...
// check for missing devices
    ...
    let mut duration_s: c_int = 2, margin_ms = 100;
    let mut duration_ms: c_int = duration_s * 1000;
    const char *cs;
    int i, err;
    snd_pcm_t *handle = core::ptr::null_mut();
    let mut access: snd_pcm_access_t = SND_PCM_ACCESS_RW_INTERLEAVED;
    snd_pcm_format_t format, old_format;
    const char *alt_formats[8];
    unsigned char *samples = core::ptr::null_mut();
    ...
    desc = conf_get_string(pcm_cfg, "description", core::ptr::null_mut(), core::ptr::null_mut());
    if (desc)
    ...
    cs = conf_get_string(pcm_cfg, "format", core::ptr::null_mut(), "S16_LE");
    format = snd_pcm_format_value(cs);
    if (format == SND_PCM_FORMAT_UNKNOWN)
    ksft_exit_fail_msg("Wrong format '%s'\n", cs);
    conf_get_string_array(pcm_cfg, "alt_formats", core::ptr::null_mut(),
    alt_formats, ARRAY_SIZE(alt_formats), core::ptr::null_mut());
    rate = conf_get_long(pcm_cfg, "rate", core::ptr::null_mut(), 48000);
    channels = conf_get_long(pcm_cfg, "channels", core::ptr::null_mut(), 2);
    period_size = conf_get_long(pcm_cfg, "period_size", core::ptr::null_mut(), 4096);
    buffer_size = conf_get_long(pcm_cfg, "buffer_size", core::ptr::null_mut(), 16384);
...
//
    ...
    if (!cfg)
    return;
    cfg = conf_get_subtree(cfg, "test", core::ptr::null_mut());
    ...
    ksft_exit_fail_msg("snd_config_get_id\n");
    test_type = conf_get_string(pcm_cfg, "type", core::ptr::null_mut(), "time");
    ...
    struct pcm_data *pcm;
    for (pcm = pcm_list; pcm != core::ptr::null_mut(); pcm = pcm.next) {
...
pub unsafe extern "C" fn main() -> c_int {
    ...
    let mut num_pcm_tests: c_int = 0, num_tests, num_std_pcm_tests;
    int ret;
    void *thread_ret;
    ksft_print_header();
    global_config = conf_load_from_file("pcm-test.conf");
    default_pcm_config = conf_get_subtree(global_config, "pcm", core::ptr::null_mut());
    if (default_pcm_config == core::ptr::null_mut())
    ...
    if (conf.card < 0)
    num_missing++;
    num_std_pcm_tests = conf_get_count(default_pcm_config, "test", core::ptr::null_mut());
    for (pcm = pcm_list; pcm != core::ptr::null_mut(); pcm = pcm.next) {
...
// Setting params is reported as a separate test
    num_tests = conf_get_count(cfg, "test", core::ptr::null_mut()) * 2;
    if (num_tests > 0)
    ...
    for (pcm = pcm_missing; pcm != core::ptr::null_mut(); pcm = pcm.next) {
    ksft_test_result(false, "test.missing.%s.%d.%d.%s\n",
    pcm.card_name, pcm.device, pcm.subdevice,
    snd_pcm_stream_name(pcm.stream));
    }
    for (card = card_list; card != core::ptr::null_mut(); card = card.next) {
    ret = pthread_create(&card.thread, core::ptr::null_mut(), card_thread, card);
    if (ret != 0) {
