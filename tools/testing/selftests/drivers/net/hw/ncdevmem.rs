...
//! Source: tools/testing/selftests/drivers/net/hw/ncdevmem.c
#![no_std]
...
// Macro flag: #define __iovec_defined

pub const PAGE_SHIFT: c_int = 12;

pub const NUM_PAGES: c_int = 16000;

pub const MSG_SOCK_DEVMEM: c_uint = 0x2000000;

pub const MAX_IOV: c_int = 1024;
    static size_t max_chunk;
    static char *server_ip;
    static char *client_ip;
    static char *port;
    static size_t do_validation;
    let mut start_queue: static int = -1;
    let mut num_queues: static int = -1;
    static int skip_config;
    static char *ifname;
    static unsigned int ifindex;
    static unsigned int dmabuf_id;
    static uint32_t tx_dmabuf_id;
    let mut waittime_ms: static int = 500;
    static bool fail_on_linear;
    static uint32_t rx_page_size;
// System state loaded by current_config_load()
pub const MAX_FLOWS: c_int = 8;
    static int ntuple_ids[MAX_FLOWS] = { -1, -1, -1, -1, -1, -1, -1, -1, };
#[repr(C)]
#[derive(Copy, Clone)]
pub struct memory_buffer {
    pub fd: c_int,
    pub size: usize,
    pub devfd: c_int,
    pub memfd: c_int,
    pub buf_mem: *mut c_char,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct memory_provider {
    pub size): *mut *mut *mut memory_buffer (alloc)(size_t,
    pub ctx): *mut *mut void (free)(struct memory_buffer,
    void (*memcpy_to_device)(struct memory_buffer *dst, size_t off,
    pub n): *mut *mut void src, int,
    void (*memcpy_from_device)(void *dst, struct memory_buffer *src,
    pub n): size_t off, int,
}
...
unsafe extern "C" fn pr_err(fmt: *const c_char, ...) {
    ...
    ctx.buf_mem = mmap(core::ptr::null_mut(), size, PROT_READ | PROT_WRITE, MAP_SHARED,
    ctx.fd, 0);
...
#[no_mangle]
unsafe extern "C" fn print_nonzero_bytes(ptr: *mut c_void, size: usize) {
...
#[no_mangle]
pub unsafe extern "C" fn validate_buffer(line: *mut c_void, size: usize) -> c_int {
    int validate_buffer(void *line, size_t size)
    {
    let mut seed: static unsigned char = 1;
...
#[no_mangle]
unsafe extern "C" fn run_command(cmd: *const c_char, ...) -> c_int {
    static int run_command(const char *cmd, ...)
    {
    va_list args;
    int ret;
    va_start(args, cmd);
    ret = __run_command(core::ptr::null_mut(), 0, cmd, args);
    va_end(args);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn ethtool_add_flow(format: *const c_char, ...) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rxq_num(ifindex: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn reset_flow_steering() {
...
#[no_mangle]
unsafe extern "C" fn restore_ring_config(config: *const ethtool_rings_get_rsp) {
...
#[no_mangle]
unsafe extern "C" fn configure_flow_steering(server_sin: *mut sockaddr_in6) -> c_int {
...
// If that fails, return error
    ...
    {
    struct netdev_bind_rx_req *req = core::ptr::null_mut();
    struct netdev_bind_rx_rsp *rsp = core::ptr::null_mut();
    struct ynl_error yerr;
// ys = ynl_sock_create(&ynl_netdev_family, &yerr);
    ...
    struct ynl_sock **ys)
    {
    struct netdev_bind_tx_req *req = core::ptr::null_mut();
    struct netdev_bind_tx_rsp *rsp = core::ptr::null_mut();
    struct ynl_error yerr;
// ys = ynl_sock_create(&ynl_netdev_family, &yerr);
    if (!*ys) {
    fprintf(stderr, "YNL: %s\n", yerr.msg);
...
#[no_mangle]
unsafe extern "C" fn enable_reuseaddr(fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_server(mem: *mut memory_buffer) -> c_int {
    static int do_server(struct memory_buffer *mem)
    {
    struct ethtool_rings_get_rsp *ring_config = core::ptr::null_mut();
    char ctrl_data[sizeof(int) * 20000];
    let mut non_page_aligned_frags: usize = 0;
    struct sockaddr_in6 client_addr;
    struct sockaddr_in6 server_sin;
    let mut page_aligned_frags: usize = 0;
    let mut total_received: usize = 0;
    socklen_t client_addr_len;
    let mut is_devmem: bool = false;
    char *tmp_mem = core::ptr::null_mut();
    struct ynl_sock *ys;
    char iobuf[819200];
    int ret, err = -1;
    char buffer[256];
...
// Flow steer our devmem flows to start_queue
    ...
    struct dmabuf_cmsg *dmabuf_cmsg = core::ptr::null_mut();
    struct cmsghdr *cm = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn gettimeofday_ms() -> u64 {
...
#[no_mangle]
unsafe extern "C" fn do_poll(fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wait_compl(fd: c_int) -> c_int {
    static int wait_compl(int fd)
    {
    let mut tstop: i64 = gettimeofday_ms() + waittime_ms;
...
#[no_mangle]
unsafe extern "C" fn do_client(mem: *mut memory_buffer) -> c_int {
    static int do_client(struct memory_buffer *mem)
    {
    char ctrl_data[CMSG_SPACE(sizeof(__u32))];
    struct sockaddr_in6 server_sin;
    struct sockaddr_in6 client_sin;
    struct ynl_sock *ys = core::ptr::null_mut();
    struct iovec iov[MAX_IOV];
    let mut msg: msghdr = {};
    let mut line_size: isize = 0;
    struct cmsghdr *cmsg;
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    unsigned long val;
    errno = 0;
    val = strtoul(optarg, core::ptr::null_mut(), 0);
