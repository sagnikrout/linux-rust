...
//! Source: tools/testing/selftests/seccomp/seccomp_benchmark.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn timing(clk_id: clockid_t, samples: c_ulonglong) -> c_ulonglong {
...
#[no_mangle]
pub unsafe extern "C" fn calibrate() -> c_ulonglong {
    unsigned long long calibrate(void)
    {
    struct timespec start, finish;
    unsigned long long i, samples, step = 9973;
    pid_t pid, ret;
    let mut seconds: c_int = 15;
...
#[no_mangle]
pub unsafe extern "C" fn approx(i_one: c_int, i_two: c_int) -> bool {
...
// go with 10%.
//
    let mut one: double = i_one, one_bump = one * 0.1;
    let mut two: double = i_two, two_bump = two * 0.1;
...
#[no_mangle]
pub unsafe extern "C" fn le(i_one: c_int, i_two: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn affinity() {
    void affinity(void)
    {
    long cpu;
    let mut ncores: c_ulong = sysconf(_SC_NPROCESSORS_CONF);
    cpu_set_t *setp = CPU_ALLOC(ncores);
    let mut setsz: c_ulong = CPU_ALLOC_SIZE(ncores);
//
...
// Avoid using "sysctl" which may not be installed.
    ...
    affinity();
    if (argc > 1)
    samples = strtoull(argv[1], core::ptr::null_mut(), 0);
    else
    samples = calibrate();
