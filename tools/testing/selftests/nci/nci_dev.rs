...
//! Source: tools/testing/selftests/nci/nci_dev.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn send_get_nfc_family(sd: c_int, pid: __u32) -> c_int {
    static int send_get_nfc_family(int sd, __u32 pid)
    {
    let mut nla_get_family_type: __u16 = CTRL_ATTR_FAMILY_NAME;
...
#[no_mangle]
unsafe extern "C" fn get_family_id(sd: c_int, pid: __u32, event_group: *mut __u32) -> c_int {
...
// event_group = *(__u32 *)((char *)group_na +
    ...
    {
    let mut nla_type: __u16 = NFC_ATTR_DEVICE_INDEX;
...
#[no_mangle]
unsafe extern "C" fn get_nci_devid(sd: c_int, fid: __u16, pid: __u32, dev_id: c_int, msg: *mut msgtemplate) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_dev_enable_state(msg: *mut msgtemplate) -> __u8 {
    ...
    if (self.isNCI2)
    rc = pthread_create(&thread_t, core::ptr::null_mut(), virtual_dev_open_v2,
    (void *)&self.virtual_nci_fd);
    else
    rc = pthread_create(&thread_t, core::ptr::null_mut(), virtual_dev_open,
    (void *)&self.virtual_nci_fd);
    ASSERT_GT(rc, -1);
    ...
    if (self.open_state) {
    if (self.isNCI2)
    rc = pthread_create(&thread_t, core::ptr::null_mut(),
    virtual_deinit_v2,
    (void *)&self.virtual_nci_fd);
    else
    rc = pthread_create(&thread_t, core::ptr::null_mut(), virtual_deinit,
    (void *)&self.virtual_nci_fd);
    ASSERT_GT(rc, -1);
    rc = send_cmd_with_idx(self.sd, self.fid, self.pid,
...
#[no_mangle]
pub unsafe extern "C" fn start_polling(dev_idx: c_int, proto: c_int, virtual_fd: c_int, sd: c_int, fid: c_int, pid: c_int) -> c_int {
    ...
    int status;
    int rc;
    rc = pthread_create(&thread_t, core::ptr::null_mut(), virtual_poll_start,
    (void *)&virtual_fd);
    if (rc < 0)
    return rc;
...
#[no_mangle]
pub unsafe extern "C" fn stop_polling(dev_idx: c_int, virtual_fd: c_int, sd: c_int, fid: c_int, pid: c_int) -> c_int {
    int stop_polling(int dev_idx, int virtual_fd, int sd, int fid, int pid)
    {
    pthread_t thread_t;
    int status;
    int rc;
    rc = pthread_create(&thread_t, core::ptr::null_mut(), virtual_poll_stop,
    (void *)&virtual_fd);
    if (rc < 0)
    return rc;
...
#[no_mangle]
pub unsafe extern "C" fn get_taginfo(dev_idx: c_int, sd: c_int, fid: c_int, pid: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(NFC_ATTR_TARGET_SEL_RES: na->nla_type ==) -> else {
    else if (na.nla_type == NFC_ATTR_TARGET_SEL_RES)
    sel_res = *(__u8 *)((char *)na + NLA_HDRLEN);
#[no_mangle]
pub unsafe extern "C" fn if(NFC_ATTR_PROTOCOLS: na->nla_type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn connect_socket(dev_idx: c_int, target_idx: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn connect_tag(dev_idx: c_int, virtual_fd: c_int, sd: c_int, fid: c_int, pid: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn read_tag(nfc_sock: c_int, virtual_fd: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn disconnect_tag(nfc_sock: c_int, virtual_fd: c_int) -> c_int {
    ...
    if (len != -1)
    return -1;
    status = pthread_create(&thread_t, core::ptr::null_mut(), virtual_deactivate_proc,
    (void *)&virtual_fd);
    close(nfc_sock);
    pthread_join(thread_t, (void **)&status);
    return status;
    ...
    EXPECT_EQ(get_dev_enable_state(&msg), 1);
    if (self.isNCI2)
    rc = pthread_create(&thread_t, core::ptr::null_mut(), virtual_deinit_v2,
    (void *)&self.virtual_nci_fd);
    else
    rc = pthread_create(&thread_t, core::ptr::null_mut(), virtual_deinit,
    (void *)&self.virtual_nci_fd);
