...
//! Source: tools/testing/selftests/net/busy_poller.c
#![no_std]
...
pub const EPOLL_IOC_TYPE: c_uint = 0x8A;
    ...
    let mut cfg_bind_addr: static struct in_addr = { .s_addr = INADDR_ANY };
    static char *cfg_outfile;
    let mut cfg_max_events: static int = 8;
...
// NAPI params
    ...
    let mut cfg_napi_threaded_poll: static enum netdev_napi_threaded = NETDEV_NAPI_THREADED_DISABLED;
#[no_mangle]
...
//
    if (c != 'o' && c != 'b')
    tmp = strtoull(optarg, core::ptr::null_mut(), 0);
    switch (c) {
    case 'u':
...
#[no_mangle]
unsafe extern "C" fn epoll_ctl_add(epfd: c_int, fd: c_int, events: u32) {
...
#[no_mangle]
unsafe extern "C" fn setnonblock(sockfd: c_int) {
...
#[no_mangle]
unsafe extern "C" fn write_chunk(fd: c_int, buf: *mut c_char, buflen: isize) {
    static void write_chunk(int fd, char *buf, ssize_t buflen)
    {
    let mut remaining: isize = buflen;
    char *buf_offset = buf;
    let mut writelen: isize = 0;
...
#[no_mangle]
unsafe extern "C" fn setup_queue() {
    static void setup_queue(void)
    {
    struct netdev_napi_get_list *napi_list = core::ptr::null_mut();
    struct netdev_napi_get_req_dump *req = core::ptr::null_mut();
    struct netdev_napi_set_req *set_req = core::ptr::null_mut();
    struct ynl_sock *ys;
...
//
    ...
    conn = accept(sockfd, core::ptr::null_mut(), core::ptr::null_mut());
    if (conn == -1)
    error(1, errno,
...
// spurious event ?
    ...
    events[i].data.fd, core::ptr::null_mut());
