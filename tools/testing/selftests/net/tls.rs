...
//! Source: tools/testing/selftests/net/tls.c
#![no_std]
...
pub const TLS_PAYLOAD_MAX_LEN: c_int = 16384;
pub const TLS_HDR_LEN: c_int = 5;
...
pub struct tls_crypto_info_keys {
    ...
    pub aes128: tls12_crypto_info_aes_gcm_128,
    pub chacha20: tls12_crypto_info_chacha20_poly1305,
    pub sm4gcm: tls12_crypto_info_sm4_gcm,
    pub sm4ccm: tls12_crypto_info_sm4_ccm,
    pub aesccm128: tls12_crypto_info_aes_ccm_128,
    pub aesgcm256: tls12_crypto_info_aes_gcm_256,
    pub ariagcm128: tls12_crypto_info_aria_gcm_128,
    pub ariagcm256: tls12_crypto_info_aria_gcm_256,
}
...
#[no_mangle]
unsafe extern "C" fn memrnd(s: *mut c_void, n: usize) {
...
// Produce a basic cmsg
    ...
    let mut cmsg_len: c_int = sizeof(char);
    struct cmsghdr *cmsg;
    struct msghdr msg;
    struct iovec vec;
    vec.iov_base = data;
...
// CMSG_DATA(cmsg) = record_type;
    ...
    cmsg = CMSG_FIRSTHDR(&msg);
    EXPECT_NE(cmsg, core::ptr::null_mut());
...
// Send some data through with ULP but no keys
    ...
    TEST_F(tls, sendfile)
    {
    let mut filefd: c_int = open("/proc/self/exe", O_RDONLY);
    struct stat st;
    EXPECT_GE(filefd, 0);
    fstat(filefd, &st);
    ...
    let mut to_send: c_int = strlen(test_str) + 1;
    char recv_buf[10];
    struct stat st;
    char *buf;
    EXPECT_GE(filefd, 0);
    ...
    let mut fd: c_int = mkstemp(filename);
    ...
    TEST_F(tls, recv_max)
    {
    let mut send_len: c_uint = TLS_PAYLOAD_MAX_LEN;
    ...
    let mut record_type: c_char = 100;
    let mut send_len: c_int = 10;
...
pub const MAX_FRAGS: c_int = 64;
pub const SEND_LEN: c_int = 13;
    ...
    let mut send_len: usize = 16384;
    let mut sends: usize = 128;
    struct msghdr msg;
    let mut recvs: usize = 0;
    let mut sent: usize = 0;
    memset(&msg, 0, sizeof(struct msghdr));
    while (sent++ < sends) {
    let mut vec: iovec = { (void *)mem, send_len };
    msg.msg_iov = &vec;
    msg.msg_iovlen = 1;
    EXPECT_EQ(sendmsg(self.fd, &msg, 0), send_len);
    }
    ...
    let mut len_cmp: c_int = 0;
    let mut iov_len: c_int = 5;
    char *buf;
    int i;
    memset(&msg, 0, sizeof(struct msghdr));
    for (i = 0; i < iov_len; i++) {
    ...
    {
    let mut send_len: c_int = TLS_PAYLOAD_MAX_LEN;
    char mem_send[TLS_PAYLOAD_MAX_LEN];
    char mem_recv[TLS_PAYLOAD_MAX_LEN];
    int p[2];
    ASSERT_GE(pipe(p), 0);
    EXPECT_GE(write(p[1], mem_send, send_len), 0);
    EXPECT_GE(splice(p[0], core::ptr::null_mut(), self.fd, core::ptr::null_mut(), send_len, 0), 0);
    EXPECT_EQ(recv(self.cfd, mem_recv, send_len, MSG_WAITALL), send_len);
    EXPECT_EQ(memcmp(mem_send, mem_recv, send_len), 0);
    }
    TEST_F(tls, splice_more)
    {
    let mut f: c_uint = SPLICE_F_NONBLOCK | SPLICE_F_MORE | SPLICE_F_GIFT;
    let mut send_len: c_int = TLS_PAYLOAD_MAX_LEN;
    char mem_send[TLS_PAYLOAD_MAX_LEN];
    int i, send_pipe = 1;
    int p[2];
    ...
    EXPECT_GE(write(p[1], mem_send, send_len), 0);
    for (i = 0; i < 32; i++)
    EXPECT_EQ(splice(p[0], core::ptr::null_mut(), self.fd, core::ptr::null_mut(), send_pipe, f), 1);
    }
    TEST_F(tls, splice_from_pipe2)
    {
    let mut send_len: c_int = 16000;
    char mem_send[16000];
    char mem_recv[16000];
    int p2[2];
    int p[2];
    ...
    EXPECT_EQ(splice(p[0], core::ptr::null_mut(), self.fd, core::ptr::null_mut(), 8000, 0), 8000);
    EXPECT_EQ(write(p2[1], mem_send + 8000, 8000), 8000);
    EXPECT_EQ(splice(p2[0], core::ptr::null_mut(), self.fd, core::ptr::null_mut(), 8000, 0), 8000);
    EXPECT_EQ(recv(self.cfd, mem_recv, send_len, MSG_WAITALL), send_len);
    EXPECT_EQ(memcmp(mem_send, mem_recv, send_len), 0);
    }
    ...
    EXPECT_GE(splice(p[0], core::ptr::null_mut(), self.fd, core::ptr::null_mut(), send_len, 0), send_len);
    EXPECT_EQ(recv(self.cfd, mem_recv, send_len, MSG_WAITALL), send_len);
    EXPECT_EQ(memcmp(mem_send, mem_recv, send_len), 0);
    }
    ...
    EXPECT_EQ(splice(p[0], core::ptr::null_mut(), self.fd, core::ptr::null_mut(), frag_len,
    ...
    EXPECT_EQ(splice(self.cfd, core::ptr::null_mut(), p[1], core::ptr::null_mut(), send_len, 0), send_len);
    ...
    EXPECT_EQ(splice(self.cfd, core::ptr::null_mut(), p[1], core::ptr::null_mut(), send_len, 0), -1);
    EXPECT_EQ(errno, EINVAL);
    ...
    char mem_send[TLS_PAYLOAD_MAX_LEN];
    char mem_recv[TLS_PAYLOAD_MAX_LEN];
    let mut half: c_int = send_len / 2;
    int p[2];
    ASSERT_GE(pipe(p), 0);
    EXPECT_EQ(send(self.fd, mem_send, send_len, 0), send_len);
// Recv hald of the record, splice the other half
    EXPECT_EQ(recv(self.cfd, mem_recv, half, MSG_WAITALL), half);
    EXPECT_EQ(splice(self.cfd, core::ptr::null_mut(), p[1], core::ptr::null_mut(), half, SPLICE_F_NONBLOCK),
    half);
    ...
    char mem_send[TLS_PAYLOAD_MAX_LEN];
    char mem_recv[TLS_PAYLOAD_MAX_LEN];
    let mut chunk: c_int = TLS_PAYLOAD_MAX_LEN / 4;
    int n, i, p[2];
    memrnd(mem_send, sizeof(mem_send));
    ASSERT_GE(pipe(p), 0);
    for (i = 0; i < 4; i++)
    ...
    n = 0;
    while (n < send_len) {
    i = splice(self.cfd, core::ptr::null_mut(), p[1], core::ptr::null_mut(), send_len - n, 0);
    EXPECT_GT(i, 0);
    n += i;
    }
...
//
    ...
    spliced = splice(self.cfd, core::ptr::null_mut(), p[1], core::ptr::null_mut(),
    send_len - total, 0);
    EXPECT_GT(spliced, 0);
    if (spliced <= 0)
    break;
...
pub const MAX_FRAGS: c_int = 48;
    ...
    EXPECT_EQ(splice(pipefds[0], core::ptr::null_mut(), self.fd, core::ptr::null_mut(), MAX_FRAGS + 0x1000, 0),
    ...
    let mut send_len: c_int = strlen(test_str) + 1;
    char buf[20];
    struct msghdr hdr;
    struct iovec vec;
    ...
    {
    let mut msg_iovlen: c_uint = 1024;
    struct iovec vec[1024];
    char *iov_base[1024];
    let mut iov_len: c_uint = 16;
    let mut send_len: c_int = 1 << 14;
    char buf[1 << 14];
    struct msghdr hdr;
    int i;
    memrnd(buf, sizeof(buf));
    ...
    {
    let mut total_len: c_uint = TLS_PAYLOAD_MAX_LEN * 2;
    let mut send_len: c_uint = TLS_PAYLOAD_MAX_LEN;
    ...
    {
    let mut total_len: c_uint = 2 * 10;
    let mut send_len: c_uint = 10;
    char recv_mem[2 * 10];
    char send_mem[10];
    memrnd(send_mem, sizeof(send_mem));
    ...
    {
    let mut total_len: c_uint = 15;
    let mut recv_len: c_uint = 10;
...
// Now the remaining 5 bytes of record data are in TLS ULP
    ...
    TEST_F(tls, blocking)
    {
    let mut data: usize = 100000;
    let mut res: c_int = fork();
    EXPECT_NE(res, -1);
    if (res) {
// parent
    let mut left: usize = data;
    char buf[16384];
    int status;
    int pid2;
    while (left) {
...
// child
    ...
    {
    let mut n_children: c_uint = n_readers + n_writers;
    let mut data: usize = 6 * 1000 * 1000;
    let mut file_sz: usize = data / 100;
...
// Split threads for reading and writing
    ...
    let mut left: usize = data * read_bias;
    char rb[8001];
    while (left) {
    int res;
    ...
    let mut left: usize = data * write_bias;
    while (left) {
    int res;
    ASSERT_EQ(lseek(fd, 0, SEEK_SET), 0);
    if (sendpg)
    res = sendfile(self.fd, fd, core::ptr::null_mut(),
...
// partial get of the cipher data should fail
    ...
    iov[1].iov_base = core::ptr::null_mut(); /* broken iov to make process_rx_list fail */
    iov[1].iov_len = 1;
    hdr.msg_iovlen = 2;
    hdr.msg_iov = iov;
...
pub const TLS_RECORD_TYPE_HANDSHAKE: c_uint = 0x16;
...
#[no_mangle]
unsafe extern "C" fn tls_send_keyupdate(_metadata: *mut __test_metadata, fd: c_int) {
    static void tls_send_keyupdate(struct __test_metadata *_metadata, int fd)
    {
    let mut len: usize = sizeof(key_update_msg);
    EXPECT_EQ(tls_send_cmsg(fd, TLS_RECORD_TYPE_HANDSHAKE,
    (char *)key_update_msg, len, 0),
    len);
    }
#[no_mangle]
unsafe extern "C" fn tls_recv_keyupdate(_metadata: *mut __test_metadata, fd: c_int, flags: c_int) {
...
// update RX key
    ...
    {
    let mut send_len: c_int = TLS_PAYLOAD_MAX_LEN / 2;
    char mem_send[TLS_PAYLOAD_MAX_LEN];
    char mem_recv[TLS_PAYLOAD_MAX_LEN];
    struct tls_crypto_info_keys tls12;
    int p[2];
...
// update TX key
    ...
    EXPECT_EQ(setsockopt(self.fd, SOL_TLS, TLS_TX, &tls12, tls12.len), 0);
    EXPECT_EQ(send(self.fd, mem_send, send_len, 0), send_len);
    EXPECT_EQ(splice(self.cfd, core::ptr::null_mut(), p[1], core::ptr::null_mut(), TLS_PAYLOAD_MAX_LEN, 0), send_len);
    EXPECT_EQ(read(p[0], mem_recv, send_len), send_len);
    EXPECT_EQ(memcmp(mem_send, mem_recv, send_len), 0);
// can't splice the KeyUpdate
    EXPECT_EQ(splice(self.cfd, core::ptr::null_mut(), p[1], core::ptr::null_mut(), TLS_PAYLOAD_MAX_LEN, 0), -1);
    EXPECT_EQ(errno, EINVAL);
// peek KeyUpdate
    tls_recv_keyupdate(_metadata, self.cfd, MSG_PEEK);
// get KeyUpdate
...
#[derive(Copy, Clone)]
pub struct raw_rec {
    pub plain_len: c_uint,
    pub plain_data: [c_uchar; 100],
    pub cipher_len: c_uint,
    pub cipher_data: [c_uchar; 128],
}
...
// Don't install keys on fd, we'll send raw records
    ...
    EXPECT_EQ(__tls_recv_cmsg(_metadata, self.cfd, core::ptr::null_mut(),
...
//
    ...
    EXPECT_EQ(errno, EBADMSG);
    ASSERT_GE(pipe(p), 0);
    ret = splice(self.cfd2, core::ptr::null_mut(), p[1], core::ptr::null_mut(), sizeof(buf),
    SPLICE_F_NONBLOCK);
    EXPECT_EQ(ret, -1);
    EXPECT_EQ(errno, EBADMSG);
    close(p[0]);
...
// Queue up one data record
    ...
    {
    let mut tv: timeval = { .tv_usec = 10000, };
    char buf[128];
    int ret;
    if (self.notls)
    SKIP(return, "no TLS support");
...
//
    ...
    let mut total_plaintext_rx: usize = 0;
...
// Plaintext must not exceed the specified limit
    ...
    let mut limit: __u16 = 128;
    let mut opt: __u16 = 0;
    let mut optlen: c_uint = sizeof(opt);
    bool notls;
    tls_crypto_info_init(TLS_1_2_VERSION, TLS_CIPHER_AES_CCM_128,
    &tls12, 0);
...
//
    ...
    let mut total_plaintext_rx: usize = 0;
    __u8 tx[1024], rx[2000];
    let mut tx_partial: __u16 = 256;
    let mut og_limit: __u16 = 512, limit = 128;
    bool notls;
    tls_crypto_info_init(TLS_1_2_VERSION, TLS_CIPHER_AES_CCM_128,
    &tls12, 0);
    ulp_sock_pair(_metadata, &fd, &cfd, &notls);
...
#[no_mangle]
unsafe extern "C" fn __attribute__(fips_check(void: (constructor))) {
    int res;
    FILE *f;
