...
//! Source: tools/testing/selftests/net/tun.c
#![no_std]
...
pub const VN_ID: c_int = 1;
pub const VN_PORT: c_int = 4789;
pub const UDP_SRC_PORT: c_int = 22;
pub const UDP_DST_PORT: c_int = 48878;
pub const IPPREFIX_LEN: c_int = 24;
pub const IP6PREFIX_LEN: c_int = 64;
pub const TIMEOUT_SEC: c_int = 10;
pub const TIMEOUT_USEC: c_int = 100000;
pub const MAX_RETRIES: c_int = 20;
pub const UDP_TUNNEL_GENEVE_4IN4: c_uint = 0x01;
pub const UDP_TUNNEL_GENEVE_6IN4: c_uint = 0x02;
pub const UDP_TUNNEL_GENEVE_4IN6: c_uint = 0x04;
pub const UDP_TUNNEL_GENEVE_6IN6: c_uint = 0x08;

    (ETH_HLEN + 2 * sizeof(struct iphdr) + GENEVE_HLEN + \
#[no_mangle]
pub unsafe extern "C" fn sizeof(udphdr): struct) -> *mut 2 {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_setup_config {
    pub family: c_int,
    union {
    pub r4: in_addr,
    pub r6: in6_addr,
    ...
    pub vnid: __be32,
    pub vnport: __be16,
    pub hwaddr: [c_uchar; 6],
    pub csum: u8,
}

#[no_mangle]
unsafe extern "C" fn tun_attach(fd: c_int, dev: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tun_detach(fd: c_int, dev: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tun_alloc(dev: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tun_delete(dev: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sockaddr_len(family: c_int) -> usize {
...
#[no_mangle]
unsafe extern "C" fn geneve_fill_newlink(req: *mut rt_link_newlink_req, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_pmtu_discover(fd: c_int, is_ipv4: bool) -> c_int {
    ...
    {
    let mut to: timeval = { .tv_sec = TIMEOUT_SEC };
    int fd, family = ssa.ss_family;
    let mut salen: c_int = sockaddr_len(family);
...
unsafe extern "C" fn ip_route_check(intf: *const c_char, family: c_int, addr: *mut c_void) -> c_int {
    ...
    let mut alen: c_int = sockaddr_len(addr.ss_family);
...
#[no_mangle]
unsafe extern "C" fn validate_hdrlen(cur: *mut u8, len: *mut c_int, x: c_int) -> c_int {
...
// dstmac = param_hwaddr_inner_dst;
    ...
    let mut tunnel_type: c_int = variant.tunnel_type;
    ...
    let mut tunnel_type: c_int = variant.tunnel_type;
    let mut payload_len: c_int = variant.data_size;
    let mut gso_size: c_int = variant.gso_size;
    uint8_t *outer_udph, *cur = buf;
    void *sip, *dip, *smac, *dmac;
    let mut is_tap: bool = variant.is_tap;
    ...
    {
    let mut timeout: timeval = { .tv_sec = TIMEOUT_SEC };
    uint8_t buf[MAX_VNET_TUNNEL_PACKET_SZ];
    let mut tunnel_type: c_int = variant.tunnel_type;
    let mut payload_len: c_int = variant.data_size;
    let mut is_tap: bool = variant.is_tap;
    int ret, len, total_len = 0;
    let mut tun_fd: c_int = self.fd;
    fd_set fdset;
    while (total_len < payload_len) {
    FD_ZERO(&fdset);
    FD_SET(tun_fd, &fdset);
    ret = select(tun_fd + 1, &fdset, core::ptr::null_mut(), core::ptr::null_mut(), &timeout);
    if (ret <= 0) {
    perror("select");
    break;
    }
    ...
    {
    let mut vnet_hdr: virtio_net_hdr_v1_hash_tunnel = { 0 };
