...
//! Source: tools/testing/selftests/net/mptcp/mptcp_connect.c
#![no_std]
...
pub const TCP_ULP: c_int = 31;
    ...
    let mut poll_timeout: static int = 10 * 1000;
    ...
    let mut cfg_mode: static enum cfg_mode = CFG_MODE_POLL;
    let mut cfg_peek: static enum cfg_peek = CFG_NONE_PEEK;
    static const char *cfg_host;
    static const char *cfg_port	= "12000";
    let mut cfg_sock_proto: static int = IPPROTO_MPTCP;
    ...
    let mut cfg_repeat: static int = 1;
    static int cfg_truncate;
    static int cfg_rcv_trunc;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cfg_cmsg_types {
    pub cmsg_enabled:1: c_uint,
    pub timestampns:1: c_uint,
    pub tcp_inq:1: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct cfg_sockopt_types {
    pub transparent:1: c_uint,
    pub mptfo:1: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcp_inq_state {
    pub last: c_uint,
    pub expect_eof: bool,
}
...
#[no_mangle]
unsafe extern "C" fn handle_signal(nr: c_int) {
    ...
    {
    let mut flags: c_int = NI_NUMERICHOST | NI_NUMERICSERV;
...
#[no_mangle]
unsafe extern "C" fn set_rcvbuf(fd: c_int, size: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn set_sndbuf(fd: c_int, size: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn set_mark(fd: c_int, mark: u32) {
...
#[no_mangle]
unsafe extern "C" fn set_transparent(fd: c_int, pf: c_int) {
...
#[no_mangle]
unsafe extern "C" fn set_mptfo(fd: c_int) {
    static void set_mptfo(int fd)
    {
    let mut qlen: c_int = 25;
    if (setsockopt(fd, IPPROTO_TCP, TCP_FASTOPEN, &qlen, sizeof(qlen)) == -1)
    perror("TCP_FASTOPEN");
    }
#[no_mangle]
unsafe extern "C" fn do_ulp_so(sock: c_int, name: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sock_test_tcpulp(sock: c_int, proto: c_int, line: c_uint) {
    static void sock_test_tcpulp(int sock, int proto, unsigned int line)
    {
    let mut buflen: socklen_t = 8;
    char buf[8] = "";
    let mut ret: c_int = getsockopt(sock, IPPROTO_TCP, TCP_ULP, buf, &buflen);
    if (ret != 0)
    X("getsockopt");
    if (buflen > 0) {
...
#[no_mangle]
unsafe extern "C" fn do_rnd_write(fd: c_int, buf: *mut c_char, len: usize) -> usize {
...
#[no_mangle]
unsafe extern "C" fn do_write(fd: c_int, buf: *mut c_char, len: usize) -> usize {
...
#[no_mangle]
unsafe extern "C" fn process_cmsg(msgh: *mut msghdr) {
    static void process_cmsg(struct msghdr *msgh)
    {
    struct __kernel_timespec ts;
    let mut inq_found: bool = false;
    let mut ts_found: bool = false;
...
#[no_mangle]
unsafe extern "C" fn do_recvmsg_cmsg(fd: c_int, buf: *mut c_char, len: usize) -> isize {
    ...
    let mut last_hint: c_uint = tcp_inq.last;
    let mut ret: c_int = recvmsg(fd, &msg, flags);
    if (ret <= 0) {
    ...
    let mut next: c_int = read(fd, msg_buf, sizeof(msg_buf));
...
#[no_mangle]
unsafe extern "C" fn do_rnd_read(fd: c_int, buf: *mut c_char, len: usize) -> isize {
    static ssize_t do_rnd_read(const int fd, char *buf, const size_t len)
    {
    let mut ret: c_int = 0;
    char tmp[16384];
    let mut cap: usize = rand();
    cap &= 0xffff;
    if (cap == 0)
    cap = 1;
#[no_mangle]
pub unsafe extern "C" fn if(len: cap >) -> else {
    else if (cap > len)
    cap = len;
...
#[no_mangle]
unsafe extern "C" fn set_nonblock(fd: c_int, nonblock: bool) {
    static void set_nonblock(int fd, bool nonblock)
    {
    let mut flags: c_int = fcntl(fd, F_GETFL);
    if (flags == -1)
    return;
    if (nonblock)
...
#[no_mangle]
unsafe extern "C" fn shut_wr(fd: c_int) {
...
//
    ...
    let mut total_wlen: c_uint = 0, total_rlen = 0;
...
#[no_mangle]
unsafe extern "C" fn do_recvfile(infd: c_int, outfd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spool_buf(fd: c_int, winfo: *mut wstate) -> c_int {
    static int spool_buf(int fd, struct wstate *winfo)
    {
    while (winfo.len) {
    let mut ret: c_int = write(fd, winfo.buf + winfo.off, winfo.len);
    if (ret < 0) {
    perror("write");
    return 4;
    }
    ...
    struct wstate *winfo)
    {
    char *inbuf = mmap(core::ptr::null_mut(), size, PROT_READ, MAP_SHARED, infd, 0);
    let mut ret: isize = 0, off = winfo.total_len;
    size_t rem;
    if (inbuf == MAP_FAILED) {
    perror("mmap");
    return 1;
...
#[no_mangle]
unsafe extern "C" fn get_infd_size(fd: c_int) -> c_int {
    ...
    struct wstate *winfo)
    {
    let mut ret: c_int = spool_buf(outfd, winfo);
    if (ret < 0)
    return ret;
    count -= winfo.total_len;
    while (count > 0) {
    ssize_t r;
    r = sendfile(outfd, infd, core::ptr::null_mut(), count);
    if (r < 0) {
    perror("sendfile");
    return 3;
    }
...
// in_closed_after_out = true;
    ...
    again:
    in_bytes = splice(infd, core::ptr::null_mut(), pipefd[1], core::ptr::null_mut(), len - winfo.total_len,
    SPLICE_F_MOVE | SPLICE_F_MORE);
    if (in_bytes < 0) {
    perror("splice in");
    err = 3;
    } else if (in_bytes > 0) {
    out_bytes = splice(pipefd[0], core::ptr::null_mut(), outfd, core::ptr::null_mut(), in_bytes,
    SPLICE_F_MOVE | SPLICE_F_MORE);
    if (out_bytes < 0) {
    perror("splice out");
    err = 4;
...
#[no_mangle]
unsafe extern "C" fn copyfd_io(infd: c_int, peerfd: c_int, outfd: c_int, close_peerfd: bool, winfo: *mut wstate) -> c_int {
    static int copyfd_io(int infd, int peerfd, int outfd, bool close_peerfd, struct wstate *winfo)
    {
    let mut in_closed_after_out: bool = false;
...
//
    ...
    let mut wanted_size: socklen_t = 0;
    switch (pf) {
    case AF_INET:
...
#[no_mangle]
unsafe extern "C" fn check_getpeername(fd: c_int, ss: *mut sockaddr_storage, salen: socklen_t) {
    static void check_getpeername(int fd, struct sockaddr_storage *ss, socklen_t salen)
    {
    struct sockaddr_storage peerss;
    let mut peersalen: socklen_t = sizeof(peerss);
...
#[no_mangle]
unsafe extern "C" fn check_getpeername_connect(fd: c_int) {
    static void check_getpeername_connect(int fd)
    {
    struct sockaddr_storage ss;
    let mut salen: socklen_t = sizeof(ss);
    char a[INET6_ADDRSTRLEN];
    char b[INET6_ADDRSTRLEN];
    const char *iface;
    size_t len;
...
#[no_mangle]
unsafe extern "C" fn maybe_close(fd: c_int) {
    static void maybe_close(int fd)
    {
    let mut r: c_uint = rand();
...
#[no_mangle]
unsafe extern "C" fn xsetsockopt(fd: c_int, level: c_int, optname: c_int, optval: *const c_void, optlen: socklen_t) {
...
#[no_mangle]
unsafe extern "C" fn apply_cmsg_types(fd: c_int, cmsg: *const cfg_cmsg_types) {
    static void apply_cmsg_types(int fd, const struct cfg_cmsg_types *cmsg)
    {
    let mut on: static unsigned int = 1;
    if (cmsg.timestampns)
...
#[no_mangle]
unsafe extern "C" fn parse_cmsg_types(type: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn parse_setsock_options(name: *const c_char) {
...
pub unsafe extern "C" fn xdisconnect(fd: c_int) {
    ...
    {
    let mut addrlen: socklen_t = sizeof(struct sockaddr_storage);
    struct sockaddr_storage addr, empty;
    let mut msec_sleep: c_int = 10;
    void *raw_addr;
    int i, cmdlen;
    char cmd[128];
...
pub unsafe extern "C" fn main_loop() -> c_int {
    int main_loop(void)
    {
    struct addrinfo *peer = core::ptr::null_mut();
    let mut fd: c_int = 0, ret, fd_in = 0;
    struct wstate winfo;
...
#[no_mangle]
unsafe extern "C" fn parse_int(size: *const c_char) -> c_int {
    static int parse_int(const char *size)
    {
    unsigned long s;
    errno = 0;
    s = strtoul(size, core::ptr::null_mut(), 0);
    if (errno) {
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    if (listen_mode) {
    let mut fd: c_int = sock_listen_mptcp(cfg_host, cfg_port);
    if (fd < 0)
    return 1;
    if (cfg_rcvbuf)
    set_rcvbuf(fd, cfg_rcvbuf);
