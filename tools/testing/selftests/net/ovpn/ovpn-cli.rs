...
//! Source: tools/testing/selftests/net/ovpn/ovpn-cli.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn ovpn_nla_get_uint(attr: *mut nlattr) -> u64 {
...
pub const NONCE_LEN: c_int = 8;
pub const PEER_ID_UNDEF: c_uint = 0x00FFFFFF;
pub const MAX_PEERS: c_int = 10;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nl_ctx {
    pub nl_sock: *mut nl_sock,
    pub nl_msg: *mut nl_msg,
    pub nl_cb: *mut nl_cb,
    pub ovpn_dco_id: c_int,
}
...
#[derive(Copy, Clone)]
pub struct ovpn_ctx {
    pub cmd: enum ovpn_cmd,
    pub key_enc: [__u8; KEY_LEN],
    pub key_dec: [__u8; KEY_LEN],
    pub nonce: [__u8; NONCE_LEN],
    pub cipher: enum ovpn_cipher_alg,
    pub sa_family: sa_family_t,
    pub tx_id: unsigned long peer_id,,
    pub lport: c_ulong,
    union {
    pub in4: sockaddr_in,
    ...
    pub in6: sockaddr_in6,
    pub peer_ip: },
    pub peer_ip_set: bool,
    pub ifindex: c_uint,
    pub ifname: [c_char; IFNAMSIZ],
    pub mode: enum ovpn_mode,
    pub mode_set: bool,
    pub socket: c_int,
    pub cli_sockets: [c_int; MAX_PEERS],
    pub keepalive_interval: __u32,
    pub keepalive_timeout: __u32,
    pub key_dir: enum ovpn_key_direction,
    pub key_slot: enum ovpn_key_slot,
    pub key_id: c_int,
    pub mark: u32,
    pub asymm_id: bool,
    pub peers_file: *const c_char,
}

#[no_mangle]
unsafe extern "C" fn ovpn_nl_recvmsgs(ctx: *mut nl_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nl_ctx_free(ctx: *mut nl_ctx) {
    ...
    struct nlattr *attrs;
    int *ret = arg;
    let mut ack_len: c_int = sizeof(*nlh) + sizeof(int) + sizeof(*nlh);
// ret = err->error;
    ...
    len -= ack_len;
    nla_parse(tb_msg, NLMSGERR_ATTR_MAX, attrs, len, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn ovpn_nl_msg_send(ctx: *mut nl_ctx, cb: ovpn_nl_cb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_parse_key(file: *const c_char, ctx: *mut ovpn_ctx) -> c_int {
    static int ovpn_parse_key(const char *file, struct ovpn_ctx *ctx)
    {
    int idx_enc, idx_dec, ret = -1;
    unsigned char *ckey = core::ptr::null_mut();
    __u8 *bkey = core::ptr::null_mut();
...
// if the file is longer, let's just read a portion
    ...
    olen = 0;
    ret = mbedtls_base64_decode(core::ptr::null_mut(), 0, &olen, ckey, ckey_len);
...
#[no_mangle]
unsafe extern "C" fn ovpn_parse_cipher(cipher: *const c_char, ctx: *mut ovpn_ctx) -> c_int {
    static int ovpn_parse_cipher(const char *cipher, struct ovpn_ctx *ctx)
    {
    if (strcmp(cipher, "aes") == 0)
    ctx.cipher = OVPN_CIPHER_ALG_AES_GCM;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(cipher, 0: "chachapoly") ==) -> else {
    else if (strcmp(cipher, "chachapoly") == 0)
    ctx.cipher = OVPN_CIPHER_ALG_CHACHA20_POLY1305;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(cipher, 0: "none") ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn ovpn_parse_key_direction(dir: *const c_char, ctx: *mut ovpn_ctx) -> c_int {
    static int ovpn_parse_key_direction(const char *dir, struct ovpn_ctx *ctx)
    {
    int in_dir;
    in_dir = strtoll(dir, core::ptr::null_mut(), 10);
    switch (in_dir) {
    case KEY_DIR_IN:
    case KEY_DIR_OUT:
    ctx.key_dir = in_dir;
...
#[no_mangle]
unsafe extern "C" fn ovpn_socket(ctx: *mut ovpn_ctx, family: sa_family_t, proto: c_int) -> c_int {
    static int ovpn_socket(struct ovpn_ctx *ctx, sa_family_t family, int proto)
    {
    let mut local_sock: sockaddr_storage = { 0 };
...
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_TCP: proto ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn ovpn_udp_socket(ctx: *mut ovpn_ctx, family: sa_family_t) -> c_int {
    static int ovpn_udp_socket(struct ovpn_ctx *ctx, sa_family_t family)
    {
    return ovpn_socket(ctx, family, IPPROTO_UDP);
    }
#[no_mangle]
unsafe extern "C" fn ovpn_listen(ctx: *mut ovpn_ctx, family: sa_family_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_accept(ctx: *mut ovpn_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_connect(ovpn: *mut ovpn_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_new_peer(ovpn: *mut ovpn_ctx, is_tcp: bool) -> c_int {
    ...
    ret = ovpn_nl_msg_send(ctx, core::ptr::null_mut());
    nla_put_failure:
    nl_ctx_free(ctx);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn ovpn_set_peer(ovpn: *mut ovpn_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_del_peer(ovpn: *mut ovpn_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_handle_peer(msg: *mut nl_msg, (*arg)__always_unused: *mut c_void) -> c_int {
    ...
    nla_len(attrs[OVPN_A_PEER]), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn ovpn_get_peer(ovpn: *mut ovpn_ctx) -> c_int {
    static int ovpn_get_peer(struct ovpn_ctx *ovpn)
    {
    let mut flags: c_int = 0, ret = -1;
    struct nlattr *attr;
    struct nl_ctx *ctx;
...
#[no_mangle]
unsafe extern "C" fn ovpn_new_key(ovpn: *mut ovpn_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_del_key(ovpn: *mut ovpn_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_handle_key(msg: *mut nl_msg, (*arg)__always_unused: *mut c_void) -> c_int {
    ...
    nla_len(attrs[OVPN_A_KEYCONF]), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn ovpn_get_key(ovpn: *mut ovpn_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_swap_keys(ovpn: *mut ovpn_ctx) -> c_int {
...
// Helper function used to easily add attributes to a rtnl message
    ...
    {
    let mut len: c_int = RTA_LENGTH(alen);
    struct rtattr *rta;
    ...
    if (ovpn_addattr(msg, max_size, attr, core::ptr::null_mut(), 0) < 0)
    return core::ptr::null_mut();
    return nest;
    }
#[no_mangle]
unsafe extern "C" fn ovpn_nest_end(msg: *mut nlmsghdr, nest: *mut rtattr) {
...
// Open RTNL socket
#[no_mangle]
unsafe extern "C" fn ovpn_rt_socket() -> c_int {
    static int ovpn_rt_socket(void)
    {
    let mut sndbuf: c_int = RT_SNDBUF_SIZE, rcvbuf = RT_RCVBUF_SIZE, fd;
...
#[no_mangle]
unsafe extern "C" fn ovpn_rt_bind(fd: c_int, groups: u32) -> c_int {
    static int ovpn_rt_bind(int fd, uint32_t groups)
    {
    let mut local: sockaddr_nl = { 0 };
    socklen_t addr_len;
...
// Send Netlink message and run callback on reply (if specified)
    ...
    payload.nlmsg_seq = time(core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn ovpn_new_iface(ovpn: *mut ovpn_ctx) -> c_int {
    static int ovpn_new_iface(struct ovpn_ctx *ovpn)
    {
    struct rtattr *linkinfo, *data;
    let mut req: ovpn_link_req = { 0 };
    let mut ret: c_int = -1;
    ...
    ret = ovpn_rt_send(&req.n, 0, 0, core::ptr::null_mut(), core::ptr::null_mut());
    err:
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn ovpn_del_iface(ovpn: *mut ovpn_ctx) -> c_int {
    ...
    return ovpn_rt_send(&req.n, 0, 0, core::ptr::null_mut(), core::ptr::null_mut());
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mcast_handler_args {
    pub group: *const c_char,
    pub id: c_int,
}

#[no_mangle]
unsafe extern "C" fn mcast_family_handler(msg: *mut nl_msg, arg: *mut c_void) -> c_int {
    ...
    nla_data(mcgrp), nla_len(mcgrp), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn mcast_ack_handler((*msg)__always_unused: *mut nl_msg, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_handle_msg(msg: *mut nl_msg, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_listen_mcast() -> c_int {
    ...
    ret = 1;
    cb = nl_cb_alloc(NL_CB_DEFAULT);
    nl_cb_set(cb, NL_CB_SEQ_CHECK, NL_CB_CUSTOM, nl_seq_check, core::ptr::null_mut());
    nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, ovpn_handle_msg, &ret);
    nl_cb_err(cb, NL_CB_CUSTOM, ovpn_nl_cb_error, &ret);
    while (ret == 1) {
    let mut err: c_int = nl_recvmsgs(sock, cb);
    if (err < 0) {
    fprintf(stderr,
...
unsafe extern "C" fn usage(cmd: *const c_char) {
    ...
    {
    int ret;
    struct addrinfo *result = core::ptr::null_mut();
    ...
    if (vpnip) {
    ret = getaddrinfo(vpnip, core::ptr::null_mut(), &hints, &result);
    if (ret) {
    ...
    {
    ovpn.peer_id = strtoul(peer_id, core::ptr::null_mut(), 10);
    if (errno == ERANGE || ovpn.peer_id > PEER_ID_UNDEF) {
    fprintf(stderr, "rx peer ID value out of range\n");
    return -1;
    }
    if (ovpn.asymm_id) {
    ovpn.tx_id = strtoul(tx_id, core::ptr::null_mut(), 10);
...
#[no_mangle]
unsafe extern "C" fn ovpn_parse_key_slot(arg: *const c_char, ovpn: *mut ovpn_ctx) -> c_int {
    static int ovpn_parse_key_slot(const char *arg, struct ovpn_ctx *ovpn)
    {
    let mut slot: c_int = strtoul(arg, core::ptr::null_mut(), 10);
...
#[no_mangle]
unsafe extern "C" fn ovpn_send_tcp_data(socket: c_int) -> c_int {
    static int ovpn_send_tcp_data(int socket)
    {
    let mut len: u16 = htons(1000);
    uint8_t buf[1002];
    int ret;
...
#[no_mangle]
unsafe extern "C" fn ovpn_recv_tcp_data(socket: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovpn_parse_cmd(cmd: *const c_char) -> enum ovpn_cmd {
...
//
#[no_mangle]
unsafe extern "C" fn ovpn_waitbg() {
...
#[no_mangle]
unsafe extern "C" fn ovpn_run_cmd(ovpn: *mut ovpn_ctx) -> c_int {
...
// store peer sockets to test TCP I/O
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), vpnip);
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn ovpn_parse_cmd_args(ovpn: *mut ovpn_ctx, argc: c_int, argv[]: *mut c_char) -> c_int {
...
// all commands, except NEW_IFNAME, needs an ifindex
    ...
    if (argc < 6)
    return -EINVAL;
    ovpn.lport = strtoul(argv[3], core::ptr::null_mut(), 10);
    ...
    const char *vpnip = (argc > 8) ? argv[8] : core::ptr::null_mut();
    ...
    ovpn.mark = 0;
    if (argc > 6) {
    ovpn.mark = strtoul(argv[6], core::ptr::null_mut(), 10);
    ...
    if (argc < 6)
    return -EINVAL;
    ovpn.peer_id = strtoul(argv[3], core::ptr::null_mut(), 10);
    if (errno == ERANGE || ovpn.peer_id > PEER_ID_UNDEF) {
    fprintf(stderr, "peer ID value out of range\n");
    return -1;
    }
    ovpn.keepalive_interval = strtoul(argv[4], core::ptr::null_mut(), 10);
    if (errno == ERANGE) {
    fprintf(stderr,
    "keepalive interval value out of range\n");
    return -1;
    }
    ovpn.keepalive_timeout = strtoul(argv[5], core::ptr::null_mut(), 10);
    if (errno == ERANGE) {
    fprintf(stderr,
    "keepalive interval value out of range\n");
    return -1;
    ...
    if (ret)
    return -1;
    ovpn.key_id = strtoul(argv[5], core::ptr::null_mut(), 10);
