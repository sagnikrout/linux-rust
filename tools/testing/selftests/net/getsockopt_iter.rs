...
//! Source: tools/testing/selftests/net/getsockopt_iter.c
#![no_std]
...
// ---------- netlink ----------
    ...
    {
    let mut group: c_int = RTNLGRP_LINK;
...
//
    ...
    {
    let mut tv: __kernel_sock_timeval = {};
    socklen_t optlen;
    optlen = sizeof(tv);
    ASSERT_EQ(0, getsockopt(self.fd, AF_VSOCK,
    ...
    {
    let mut tv: __kernel_old_timeval = {};
    socklen_t optlen;
    optlen = sizeof(tv);
    ASSERT_EQ(0, getsockopt(self.fd, AF_VSOCK,
...
// ---------- raw (ipv4) ----------
    ...
    FIXTURE_SETUP(raw)
    {
    let mut filt: icmp_filter = { .data = 0xdeadbeef };
    ...
    {
    let mut filt: icmp_filter = {};
    let mut optlen: socklen_t = sizeof(filt);
...
// ---------- tls ----------
    ...
    let mut alen: socklen_t = sizeof(a);
    int lfd;
    self.fd = -1;
    self.sfd = -1;
    ...
    self.sfd = accept(lfd, core::ptr::null_mut(), core::ptr::null_mut());
    close(lfd);
...
// do_tls_getsockopt_tx_zc(): fixed-size int, exact length required.
    ...
    {
    let mut optlen: socklen_t = sizeof(int);
    let mut val: c_int = -1;
...
//
    ...
    ASSERT_EQ(-1, getsockopt(self.fd, SOL_TLS, TLS_TX, core::ptr::null_mut(), &optlen));
    ASSERT_EQ(EINVAL, errno);
    }
...
// TLS_TX before crypto is set reports not-ready.
    ...
    let mut optlen: socklen_t = sizeof(info);
