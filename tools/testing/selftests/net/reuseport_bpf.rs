...
//! Source: tools/testing/selftests/net/reuseport_bpf.c
#![no_std]
...
pub struct test_params {
    pub recv_family: c_int,
    pub send_family: c_int,
    pub protocol: c_int,
    pub recv_socks: usize,
    pub recv_port: u16,
    pub send_port_min: u16,
}

#[no_mangle]
unsafe extern "C" fn sockaddr_size() -> usize {
...
#[no_mangle]
unsafe extern "C" fn attach_ebpf(fd: c_int, mod: u16) {
...
#[no_mangle]
unsafe extern "C" fn attach_cbpf(fd: c_int, mod: u16) {
...
// return A
    ...
    size_t len)
    {
    let mut saddr: *mut sockaddr  const = new_any_sockaddr(p.send_family, sport);
    struct sockaddr * const daddr =
    new_loopback_sockaddr(p.send_family, p.recv_port);
    let mut fd: c_int = socket(p.send_family, p.protocol, 0), one = 1;
    if (fd < 0)
...
#[no_mangle]
unsafe extern "C" fn test_recv_order(p: test_params, fd[]: c_int, mod: c_int) {
    static void test_recv_order(const struct test_params p, int fd[], int mod)
    {
    char recv_buf[8], send_buf[8];
    struct msghdr msg;
    let mut recv_io: iovec = { recv_buf, 8 };
    ...
    if (p.protocol == SOCK_STREAM) {
    conn = accept(ev.data.fd, core::ptr::null_mut(), core::ptr::null_mut());
    if (conn < 0)
...
#[no_mangle]
unsafe extern "C" fn test_reuseport_ebpf(p: test_params) {
...
#[no_mangle]
unsafe extern "C" fn test_reuseport_cbpf(p: test_params) {
...
#[no_mangle]
unsafe extern "C" fn test_extra_filter(p: test_params) {
...
#[no_mangle]
unsafe extern "C" fn test_filter_no_reuseport(p: test_params) {
...
#[no_mangle]
unsafe extern "C" fn test_filter_without_bind() {
...
#[no_mangle]
pub unsafe extern "C" fn enable_fastopen() {
    void enable_fastopen(void)
    {
    let mut fd: c_int = open("/proc/sys/net/ipv4/tcp_fastopen", 0);
...
#[no_mangle]
pub unsafe extern "C" fn __attribute__(main_ctor(void: (constructor)) void) -> static {
...
#[no_mangle]
pub unsafe extern "C" fn __attribute__(main_dtor(void: (destructor)) void) -> static {
