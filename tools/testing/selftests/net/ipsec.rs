...
//! Source: tools/testing/selftests/net/ipsec.c
#![no_std]
...
pub const XFRM_ALGO_KEY_BUF_SIZE: c_int = 512;

// /30 mask for one veth connection
pub const PREFIX_LEN: c_int = 30;

pub const VETH_LEN: c_int = 12;
    let mut nsfd_parent: static int = -1;
    let mut nsfd_childa: static int = -1;
    let mut nsfd_childb: static int = -1;
...
//
    ...
    let mut ping_delay_nsec: c_uint = 50 * 1000 * 1000;
    let mut ping_timeout: c_uint = 300;
    let mut ping_count: c_uint = 100;
    let mut ping_success: c_uint = 80;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xfrm_key_entry {
    pub algo_name: [c_char; 35],
    pub key_len: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn randomize_buffer(buf: *mut c_void, buflen: usize) {
...
#[no_mangle]
unsafe extern "C" fn unshare_open() -> c_int {
...
// NLMSG_ALIGNTO == RTA_ALIGNTO, nlmsg_len already aligned
    ...
    let mut nl_size: usize = RTA_ALIGN(nh.nlmsg_len) + RTA_LENGTH(size);
    if (req_sz < nl_size) {
...
#[no_mangle]
unsafe extern "C" fn netlink_check_answer(sock: c_int) -> c_int {
...
pub struct nlmsgerror {
    ...
    pub (int)answer.hdr.nlmsg_type): printk("expected NLMSG_ERROR, got %d",,
    pub -1: return,
    ...
    pub netlink_check_answer(sock): return,
    }
    ...
    pub flags: req.nh.nlmsg_flags =,
    pub seq: req.nh.nlmsg_seq =,
    pub AF_INET: req.info.ifa_family =,
    pub prefix: req.info.ifa_prefixlen =,
    pub if_nametoindex(intf): req.info.ifa_index =,

    {
    pub {}: char addr_str[IPV4_STR_SZ] =,
    pub 1): strncpy(addr_str, inet_ntoa(addr), IPV4_STR_SZ -,
    pub addr_str): printk("ip addr set %s",,
    }
...
#[no_mangle]
unsafe extern "C" fn link_set_up(sock: c_int, seq: u32, intf: *const c_char) -> c_int {
    ...
    pub seq: req.nh.nlmsg_seq =,
    pub AF_INET: req.rt.rtm_family =,
    pub 32: req.rt.rtm_dst_len =,
    pub RT_TABLE_MAIN: req.rt.rtm_table =,
    pub RTPROT_BOOT: req.rt.rtm_protocol =,
    pub RT_SCOPE_LINK: req.rt.rtm_scope =,
    ...
    tunsrc, PREFIX_LEN)) {
    pub addr"): printk("Failed to set ipv4,
    pub -1: return,
    }
    if (ip4_route_set(route_sock, (*route_seq)++, veth, tunsrc, tundst)) {
    pub route"): printk("Failed to set ipv4,
    pub -1: return,
    }
    pub 0: return,
    }
#[no_mangle]
unsafe extern "C" fn init_child(nsfd: c_int, veth: *mut c_char, src: c_uint, dst: c_uint) -> c_int {
    static int init_child(int nsfd, char *veth, unsigned int src, unsigned int dst)
    {
    pub src): in_addr intsrc = inet_makeaddr(INADDR_B,,
    pub src): in_addr tunsrc = inet_makeaddr(INADDR_A,,
    pub dst): in_addr tundst = inet_makeaddr(INADDR_A,,
    pub -1: int route_sock = -1, ret =,
    pub route_seq: u32,
    if (switch_ns(nsfd))
    pub -1: return,
    if (netlink_sock(&route_sock, &route_seq, NETLINK_ROUTE)) {
    pub child"): printk("Failed to open netlink route socket in,
    pub -1: return,
    }
    ...
    pub veth): printk("Failed to bring up %s",,
    pub err: goto,
    }
    if (tunnel_set_route(route_sock, &route_seq, veth, tunsrc, tundst)) {
    pub veth): printk("Failed to add tunnel route on %s",,
    pub err: goto,
    }
    pub 0: ret =,
    err:
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xfrm_desc {
    pub type: enum desc_type,
    pub proto: u8,
    pub a_algo: [c_char; ALGO_LEN],
    pub e_algo: [c_char; ALGO_LEN],
    pub c_algo: [c_char; ALGO_LEN],
    pub ae_algo: [c_char; ALGO_LEN],
    pub icv_len: c_uint,
// unsigned key_len;
}
...
pub struct test_desc {
    pub type: enum msg_type,
    union {
    struct {
    pub reply_ip: in_addr_t,
    pub port: c_uint,
    pub ping: },
    pub xfrm_desc: xfrm_desc,
    pub body: },
}
...
#[no_mangle]
unsafe extern "C" fn write_test_result(res: c_uint, d: *mut xfrm_desc) {
    static void write_test_result(unsigned int res, struct xfrm_desc *d)
    {
    let mut tr: test_result = {};
    ssize_t ret;
    tr.desc = *d;
    tr.res = res;
...
#[no_mangle]
unsafe extern "C" fn write_msg(fd: c_int, msg: *mut test_desc, exit_of_fail: bool) {
    static void write_msg(int fd, struct test_desc *msg, bool exit_of_fail)
    {
    let mut bytes: isize = write(fd, msg, sizeof(*msg));
...
#[no_mangle]
unsafe extern "C" fn read_msg(fd: c_int, msg: *mut test_desc, exit_of_fail: bool) {
    static void read_msg(int fd, struct test_desc *msg, bool exit_of_fail)
    {
    let mut bytes: isize = read(fd, msg, sizeof(*msg));
    if (bytes < 0) {
    pr_err("read()");
    if (exit_of_fail)
    exit(KSFT_FAIL);
    ...
    let mut t: timeval = { .tv_sec = 0, .tv_usec = u_timeout };
    let mut s_len: socklen_t = sizeof(server);
...
#[no_mangle]
pub unsafe extern "C" fn gen_spi(src: in_addr) -> u32 {
...
// XXX: check xfrm algo, see xfrm_state_pack_algo().
    ...
    let mut filter: xfrm_address_filter = {};
...
// Note: zero-spi cannot be deleted
    ...
    let mut new_spi: u32 = htonl(answer.info.id.spi);
    if (new_spi != spi) {
...
#[no_mangle]
unsafe extern "C" fn netlink_sock_bind(sock: *mut c_int, seq: *mut u32, proto: c_int, groups: u32) -> c_int {
    static int netlink_sock_bind(int *sock, uint32_t *seq, int proto, uint32_t groups)
    {
    let mut snl: sockaddr_nl = {};
    socklen_t addr_len;
    let mut ret: c_int = -1;
...
#[no_mangle]
unsafe extern "C" fn xfrm_monitor_acquire(xfrm_sock: c_int, seq: *mut u32, nr: c_uint) -> c_int {
    ...
    let mut xfrm_tmpl: xfrm_user_tmpl = {};
    let mut xfrm_listen: c_int = -1, ret = KSFT_FAIL;
    uint32_t seq_listen;
...
// Fill selector.
    ...
    if (rtattr_pack(&req.nh, sizeof(req), XFRMA_IF_ID, core::ptr::null_mut(), 0)) {
...
#[no_mangle]
unsafe extern "C" fn xfrm_spdinfo_attrs(xfrm_sock: c_int, seq: *mut u32) -> c_int {
    ...
    let mut len: usize = NLMSG_PAYLOAD(&req.nh, sizeof(req.unused));
    struct rtattr *attr = (void *)req.attrbuf;
    let mut got_thresh: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn child_f(nr: c_uint, test_desc_fd: c_int, cmd_fd: c_int, buf: *mut c_void) -> c_int {
    static int child_f(unsigned int nr, int test_desc_fd, int cmd_fd, void *buf)
    {
    struct xfrm_desc desc;
    struct test_desc msg;
    let mut xfrm_sock: c_int = -1;
    uint32_t seq;
...
// Check that seq sock is ready, just for sure.
    ...
    for (;;) {
    let mut received: isize = read(test_desc_fd, &desc, sizeof(desc));
    int ret;
    if (received == 0) /* EOF */
    break;
...
#[no_mangle]
unsafe extern "C" fn grand_child_f(nr: c_uint, cmd_fd: c_int, buf: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn start_child(nr: c_uint, veth: *mut c_char, test_desc_fd[2]: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn exit_usage(argv: *mut c_char) {
...
#[no_mangle]
unsafe extern "C" fn __write_desc(test_desc_fd: c_int, desc: *mut xfrm_desc) -> c_int {
...
// not implemented
    ...
    + ARRAY_SIZE(ae_list);
#[no_mangle]
unsafe extern "C" fn write_proto_plan(fd: c_int, proto: c_int) -> c_int {
...
//
    let mut compat_plan: c_uint = 5;
#[no_mangle]
unsafe extern "C" fn write_compat_struct_tests(test_desc_fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn write_test_plan(test_desc_fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn children_cleanup() -> c_int {
    static int children_cleanup(void)
    {
    let mut ret: unsigned = KSFT_PASS;
    while (1) {
    int status;
    let mut p: pid_t = wait(&status);
    if ((p < 0) && errno == ECHILD)
    break;
    if (p < 0) {
    pr_err("wait()");
...
unsafe extern "C" fn check_results() -> c_int {
    ...
    while (1) {
    let mut received: isize = read(results_fd[0], &tr, sizeof(tr));
    print_res result;
    if (received == 0) /* EOF */
    break;
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    let mut route_sock: c_int = -1, ret = KSFT_SKIP;
    int test_desc_fd[2];
    uint32_t route_seq;
    unsigned int i;
    if (argc > 2)
