...
//! Source: tools/testing/selftests/net/lib/csum.c
#![no_std]
...
// Macro flag: #define _GNU_SOURCE
    ...
    let mut cfg_num_pkt: static int = 4;
    let mut cfg_do_rx: static bool = true;
    let mut cfg_do_tx: static bool = true;
    static bool cfg_encap;
    static char *cfg_ifname = "eth0";
    static char *cfg_mac_dst;
    static char *cfg_mac_src;
    let mut cfg_proto: static int = IPPROTO_UDP;
    let mut cfg_payload_char: static int = 'a';
    let mut cfg_payload_len: static int = 100;
    let mut cfg_port_dst: static uint16_t = 34000;
    let mut cfg_port_src: static uint16_t = 33000;
    let mut cfg_port_src_encap: static uint16_t = 33001;
    ...
    let mut cfg_timeout_ms: static int = 2000;
    ...
    let mut cfg_daddr4: static struct sockaddr_in = {.sin_family = AF_INET};
    let mut cfg_saddr4: static struct sockaddr_in = {.sin_family = AF_INET};
    let mut cfg_daddr6: static struct sockaddr_in6 = {.sin6_family = AF_INET6};
    let mut cfg_saddr6: static struct sockaddr_in6 = {.sin6_family = AF_INET6};

pub const MAX_PAYLOAD_LEN: c_int = 1024;
// Trivial demo encap. Stand-in for transport layer protocols like ESP or PSP
#[repr(C)]
#[derive(Copy, Clone)]
pub struct udp_encap_hdr {
    pub nexthdr: u8,
    pub padding: [u8; 3],
}
...
#[no_mangle]
unsafe extern "C" fn checksum_nofold(data: *mut c_char, len: usize, sum: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn checksum(th: *mut c_void, proto: u16, len: usize) -> u16 {
...
// With CHECKSUM_PARTIAL kernel expects non-inverted pseudo csum
    ...
    return ~checksum_fold(core::ptr::null_mut(), 0, sum);
    else
...
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_UDP: cfg_proto ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn open_inet(ipproto: c_int, protocol: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn open_packet() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn send_inet(fd: c_int, buf: *const c_char, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn eth_str_to_addr(str: *const c_char, eth: *mut c_uchar) {
...
#[no_mangle]
unsafe extern "C" fn send_packet(fd: c_int, buf: *const c_char, len: c_int) {
    static void send_packet(int fd, const char *buf, int len)
    {
    let mut vh: virtio_net_hdr = {0};
    let mut addr: sockaddr_ll = {0};
...
#[no_mangle]
unsafe extern "C" fn recv_prepare_udp() -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __recv_prepare_packet_filter(fd: c_int, off_nexthdr: c_int, off_dport: c_int) {
...
#[no_mangle]
unsafe extern "C" fn recv_prepare_packet_filter(fd: c_int) {
...
#[no_mangle]
unsafe extern "C" fn recv_prepare_packet_bind(fd: c_int) {
    static void recv_prepare_packet_bind(int fd)
    {
    let mut laddr: sockaddr_ll = {0};
...
#[no_mangle]
unsafe extern "C" fn recv_prepare_packet() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_udp(fd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_verify_csum(th: *mut c_void, len: c_int, sport: u16, csum_field: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_verify_packet_tcp(th: *mut c_void, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_verify_packet_udp_encap(th: *mut c_void, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_verify_packet_udp(th: *mut c_void, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_verify_packet_ipv4(nh: *mut c_void, len: c_int) -> c_int {
    static int recv_verify_packet_ipv4(void *nh, int len)
    {
    struct iphdr *iph = nh;
    let mut proto: u16 = cfg_encap ? IPPROTO_UDP : cfg_proto;
    uint16_t ip_len;
...
#[no_mangle]
unsafe extern "C" fn recv_verify_packet_ipv6(nh: *mut c_void, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn recv_get_packet_csum_status(msg: *mut msghdr) -> u32 {
    static uint32_t recv_get_packet_csum_status(struct msghdr *msg)
    {
    struct tpacket_auxdata *aux = core::ptr::null_mut();
    struct cmsghdr *cm;
...
#[no_mangle]
unsafe extern "C" fn recv_packet(fd: c_int) -> c_int {
    static int recv_packet(int fd)
    {
    static char _buf[MAX_HEADER_LEN + MAX_PAYLOAD_LEN];
    let mut total: c_ulong = 0, bad_csums = 0, bad_validations = 0;
...
#[no_mangle]
unsafe extern "C" fn parse_args(argc: c_int, argv[]: *const *const c_char) {
    static void parse_args(int argc, char *const argv[])
    {
    const char *daddr = core::ptr::null_mut(), *saddr = core::ptr::null_mut();
    int c;
    ...
    break;
    case 'l':
    cfg_payload_len = strtol(optarg, core::ptr::null_mut(), 0);
    break;
    case 'L':
    cfg_timeout_ms = strtol(optarg, core::ptr::null_mut(), 0) * 1000;
    break;
    case 'n':
    cfg_num_pkt = strtol(optarg, core::ptr::null_mut(), 0);
    break;
    case 'r':
    cfg_random_seed = strtol(optarg, core::ptr::null_mut(), 0);
    break;
    case 'P':
...
unsafe extern "C" fn do_tx() {
    ...
    fd = open_packet();
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cfg_send_udp) -> else {
...
#[no_mangle]
unsafe extern "C" fn do_rx(fdp: c_int, fdr: c_int) {
    static void do_rx(int fdp, int fdr)
    {
    let mut count_udp: c_ulong = 0, count_pkt = 0;
    long tleft, tstop;
    struct pollfd pfd;
