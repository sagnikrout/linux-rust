...
//! Source: tools/testing/selftests/net/tcp_port_share.c
#![no_std]
...
pub const DST_PORT: c_int = 30000;
pub const SRC_PORT: c_int = 40000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sockaddr_inet {
    ...
    let mut one: c_int = 1;
#[no_mangle]
unsafe extern "C" fn disconnect(fd: c_int) -> c_int {
    static int disconnect(int fd)
    {
    return connect(fd, &(struct sockaddr){ AF_UNSPEC }, sizeof(struct sockaddr));
    }
#[no_mangle]
unsafe extern "C" fn getsockname_port(fd: c_int) -> c_int {
    static int getsockname_port(int fd)
    {
    let mut addr: sockaddr_inet = {};
    int err;
