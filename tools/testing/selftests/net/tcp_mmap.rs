...
//! Source: tools/testing/selftests/net/tcp_mmap.c
#![no_std]
...
pub const MSG_ZEROCOPY: c_uint = 0x4000000;
    ...
    let mut cfg_alen: static socklen_t = sizeof(struct sockaddr_in6);
    let mut cfg_port: static int = 8787;
    ...
    let mut chunk_size: static size_t = 512*1024;
    static size_t map_align;
    unsigned long htotal;
    unsigned int digest_len;
#[no_mangle]
pub unsafe extern "C" fn prefetch(x: *const c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn hash_zone(zone: *mut c_void, length: c_uint) {
    void hash_zone(void *zone, unsigned int length)
    {
    let mut temp: c_ulong = htotal;
...
// Attempt to use huge pages if possible.
    ...
    buffer = mmap(core::ptr::null_mut(), sz, PROT_READ | PROT_WRITE,
...
#[no_mangle]
unsafe extern "C" fn tcp_info_get_rcv_mss(fd: c_int) -> u32 {
    static uint32_t tcp_info_get_rcv_mss(int fd)
    {
    let mut sz: socklen_t = sizeof(struct tcp_info);
    ...
    let mut total_mmap: c_ulong = 0, total = 0;
    struct tcp_zerocopy_receive zc;
    unsigned char *buffer = core::ptr::null_mut();
    unsigned long delta_usec;
    EVP_MD_CTX *ctx = core::ptr::null_mut();
    let mut flags: c_int = MAP_SHARED;
    struct timeval t0, t1;
    void *raddr = core::ptr::null_mut();
    ...
    gettimeofday(&t0, core::ptr::null_mut());
    ...
    if (zflg) {
    raddr = mmap(core::ptr::null_mut(), chunk_size + map_align, PROT_READ, flags, fd, 0);
    if (raddr == MAP_FAILED) {
    perror("mmap");
    zflg = 0;
    } else {
    ...
    EVP_DigestInit_ex(ctx, EVP_sha256(), core::ptr::null_mut());
    }
    while (1) {
    let mut pfd: pollfd = { .fd = fd, .events = POLLIN, };
    int sub;
    poll(&pfd, 1, 10000);
    if (zflg) {
...
//
    ...
    gettimeofday(&t1, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn apply_rcvsnd_buf(fd: c_int) {
...
#[no_mangle]
unsafe extern "C" fn do_accept(fdlisten: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn default_huge_page_size() -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn randomize(target: *mut c_void, count: usize) {
    static void randomize(void *target, size_t count)
    {
    let mut urandom: static int = -1;
    ssize_t got;
...
//
    ...
    if (sflg) {
    let mut fdlisten: c_int = socket(cfg_family, SOCK_STREAM, 0);
    if (fdlisten == -1) {
    perror("socket");
    exit(1);
    }
    ...
    while (total < FILE_SZ) {
    let mut offset: usize = total % chunk_size;
    let mut wr: i64 = FILE_SZ - total;
