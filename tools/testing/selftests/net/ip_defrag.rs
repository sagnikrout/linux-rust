...
//! Source: tools/testing/selftests/net/ip_defrag.c
#![no_std]
...
// Macro flag: #define _GNU_SOURCE
    ...
    let mut addr4: in_addr = { .s_addr = __constant_htonl(INADDR_LOOPBACK + 2) };
    let mut addr6: in6_addr = IN6ADDR_LOOPBACK_INIT;

// IPv6 fragment header lenth.
pub const FRAG_HLEN: c_int = 8;
    static int payload_len;
    static int max_frag_len;

    static uint8_t udp_payload[MSG_LEN_MAX];
    static uint8_t ip_frame[IP_MAXPACKET];
    let mut ip_id: static uint32_t = 0xabcd;
    static int msg_counter;
    static int frag_counter;
    static unsigned int seed;
// Receive a UDP packet. Validate it matches udp_payload.
#[no_mangle]
unsafe extern "C" fn recv_validate_udp(fd_udp: c_int) {
...
#[no_mangle]
unsafe extern "C" fn raw_checksum(buf: *mut u8, len: c_int, sum: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn udp_checksum(iphdr: *mut ip, udphdr: *mut udphdr) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn udp6_checksum(iphdr: *mut ip6_hdr, udphdr: *mut udphdr) -> u16 {
    ...
    int frag_len;
    int res;
    let mut payload_offset: c_int = offset > 0 ? offset - UDP_HLEN : 0;
...
#[no_mangle]
unsafe extern "C" fn run_test(addr: *mut sockaddr, alen: socklen_t, ipv6: bool) {
...
// with each other.
//
    let mut tv: timeval = { .tv_sec = 1, .tv_usec = 10 };
    int idx;
    let mut min_frag_len: c_int = 8;
