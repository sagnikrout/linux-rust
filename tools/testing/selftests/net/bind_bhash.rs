...
//! Source: tools/testing/selftests/net/bind_bhash.c
#![no_std]
...
pub const MAX_THREADS: c_int = 600;
pub const MAX_CONNECTIONS: c_int = 40;
...
#[no_mangle]
unsafe extern "C" fn bind_socket(opt: c_int, addr: *const c_char) -> c_int {
    static int bind_socket(int opt, const char *addr)
    {
    struct addrinfo *res, hint = {};
    int sock_fd, reuse = 1, err;
    let mut domain: c_int = use_v6 ? AF_INET6 : AF_INET;
...
// Set up threads to populate the bhash table entry for the port
    ...
    pthread_create(&tid[i], core::ptr::null_mut(), setup, fd_array[i]);
    for (i = 0; i < MAX_THREADS; i++)
