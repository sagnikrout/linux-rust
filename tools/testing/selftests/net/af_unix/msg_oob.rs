...
//! Source: tools/testing/selftests/net/af_unix/msg_oob.c
#![no_std]
...
//
    ...
    ASSERT_EQ(ret, 0);
    ret = sigprocmask(SIG_BLOCK, &mask, core::ptr::null_mut());
    ASSERT_EQ(ret, 0);
...
#[no_mangle]
unsafe extern "C" fn close_sockets(self: *mut FIXTURE_DATA(msg_oob)) {
    ...
    errno = 0;
    for (i = 0; i < 2; i++) {
    let mut index: c_int = is_sender ? i * 2 : i * 2 + 1;
