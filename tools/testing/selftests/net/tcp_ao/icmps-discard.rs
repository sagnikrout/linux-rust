...
//! Source: tools/testing/selftests/net/tcp_ao/icmps-discard.c
#![no_std]
...
//
    ...
    let mut packets_nr: usize = 20;
    let mut packet_size: usize = 100;
    const char *tcpao_icmps	= "TCPAODroppedIcmps";

    const char *dst_unreach	= "Icmp6InDestUnreachs";
    let mut sk_ip_level: c_int = SOL_IPV6;
    let mut sk_recverr: c_int = IPV6_RECVERR;

    const char *dst_unreach	= "InDestUnreachs";
    let mut sk_ip_level: c_int = SOL_IP;
    let mut sk_recverr: c_int = IP_RECVERR;

// Server is expected to fail with hard error if ::accept_icmp is set

#[no_mangle]
unsafe extern "C" fn serve_interfered(sk: c_int) {
    static void serve_interfered(int sk)
    {
    let mut test_quota: isize = packet_size * packets_nr * 10;
    ...
    dest_unreach_a = netstat_get(ns_before, dst_unreach, core::ptr::null_mut());
    icmp_ignored_a = netstat_get(ns_before, tcpao_icmps, core::ptr::null_mut());
    if (test_get_tcp_counters(sk, &cnt1))
    test_error("test_get_tcp_counters()");
    bytes = test_server_run(sk, test_quota, 0);
    ns_after = netstat_read();
    netstat_print_diff(ns_before, ns_after);
    dest_unreach_b = netstat_get(ns_after, dst_unreach, core::ptr::null_mut());
    ...
    test_assert_counters(core::ptr::null_mut(), &cnt1, &cnt2, TEST_CNT_GOOD);

    test_assert_counters(core::ptr::null_mut(), &cnt1, &cnt2, TEST_CNT_GOOD | TEST_CNT_AO_DROPPED_ICMP);
...
#[no_mangle]
unsafe extern "C" fn checksum4_nofold(data: *mut c_void, len: usize, sum: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn checksum4_fold(data: *mut c_void, len: usize, sum: u32) -> u16 {
    ...
    struct sockaddr_in *src, struct sockaddr_in *dst)
    {
    let mut sk: c_int = socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
    struct {
    struct iphdr iph;
...
pub unsafe extern "C" fn csum_fold(csum: u32) -> u16 {
    ...
    {
    let mut sum: u32 = csum;
    sum = (sum & 0xffff) + (sum >> 16);
    sum = (sum & 0xffff) + (sum >> 16);
    return (uint16_t)~sum;
    }
#[no_mangle]
pub unsafe extern "C" fn csum_add(csum: u32, addend: u32) -> u32 {
    static inline uint32_t csum_add(uint32_t csum, uint32_t addend)
    {
    let mut res: u32 = csum;
    res += addend;
    return res + (res < addend);
    }
#[no_mangle]
pub unsafe extern "C" fn checksum6_nofold(data: *mut c_void, len: usize, sum: u32) -> noinline uint32_t {
    ...
    struct sockaddr_in6 *src, struct sockaddr_in6 *dst)
    {
    let mut sk: c_int = socket(AF_INET6, SOCK_RAW, IPPROTO_RAW);
...
#[no_mangle]
unsafe extern "C" fn get_rcv_nxt(sk: c_int) -> u32 {
    static uint32_t get_rcv_nxt(int sk)
    {
    let mut val: c_int = TCP_REPAIR_ON;
    uint32_t ret;
    let mut sz: socklen_t = sizeof(ret);
...
#[no_mangle]
unsafe extern "C" fn icmp_interfere(nr: usize, rcv_nxt: u32, src: *mut c_void, dst: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn send_interfered(sk: c_int) {
