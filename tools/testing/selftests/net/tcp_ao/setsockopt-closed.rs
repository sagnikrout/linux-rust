...
//! Source: tools/testing/selftests/net/tcp_ao/setsockopt-closed.c
#![no_std]
...
pub const FILTER_TEST_NKEYS: c_int = 16;
    let mut test_port: static int = 7788;
#[no_mangle]
unsafe extern "C" fn make_listen(sk: c_int) {
    ...
    const char *tst)
    {
    let mut tmp: tcp_ao_info_opt = {};
    let mut len: socklen_t = sizeof(tmp);
    ...
    {
    const char *cmd = core::ptr::null_mut();
    socklen_t len;
    switch (optname) {
    case TCP_AO_ADD_KEY:
    cmd = "key add: ";
...
#[no_mangle]
unsafe extern "C" fn prepare_defs(cmd: c_int, optval: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_extend(cmd: c_int, get: bool, tst: *const c_char, under_size: socklen_t) {
    ...
    let mut extended_size: socklen_t = sizeof(tmp_opt);
    int sk;
    ...
    memset(&tmp_opt, 0, sizeof(tmp_opt));
    sk = prepare_defs(cmd, &tmp_opt);
    __setsockopt_checked(sk, cmd, get, core::ptr::null_mut(), &extended_size,
    EFAULT, tst, ": null optval");
    if (get) {
    memset(&tmp_opt, 0, sizeof(tmp_opt));
    sk = prepare_defs(cmd, &tmp_opt);
    __setsockopt_checked(sk, cmd, get, &tmp_opt, core::ptr::null_mut(),
    EFAULT, tst, ": null optlen");
    }
    }
#[no_mangle]
unsafe extern "C" fn extend_tests() {
...
#[no_mangle]
unsafe extern "C" fn test_einval_add_key() {
...
#[no_mangle]
unsafe extern "C" fn test_einval_del_key() {
...
#[no_mangle]
unsafe extern "C" fn test_einval_ao_info() {
...
#[no_mangle]
unsafe extern "C" fn test_einval_get_keys() {
...
#[no_mangle]
unsafe extern "C" fn einval_tests() {
...
#[no_mangle]
unsafe extern "C" fn duplicate_tests() {
...
#[no_mangle]
unsafe extern "C" fn fetch_all_keys(sk: c_int, keys: *mut tcp_ao_getsockopt) {
    static void fetch_all_keys(int sk, struct tcp_ao_getsockopt *keys)
    {
    let mut optlen: socklen_t = sizeof(struct tcp_ao_getsockopt);
...
#[no_mangle]
unsafe extern "C" fn prepare_test_keys(keys: *mut tcp_ao_getsockopt) -> c_int {
    ...
    let mut rcvid: u8 = 100, sndid = 100;
    int sk;
    sk = socket(test_family, SOCK_STREAM, IPPROTO_TCP);
    if (sk < 0)
    test_error("socket()");
...
// Add keys in a random order
    for (int i = 0; i < FILTER_TEST_NKEYS; i++) {
    let mut randidx: c_int = rand() % (FILTER_TEST_NKEYS - i);
...
#[no_mangle]
unsafe extern "C" fn filter_tests() {
    ...
    let mut filter: tcp_ao_getsockopt = {};
    int sk, f, nmatches;
    socklen_t len;
    f = 2;
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    {
    test_init(126, client_fn, core::ptr::null_mut());
    return 0;
    }
