...
//! Source: tools/testing/selftests/net/tcp_ao/rst.c
#![no_std]
...
//
    ...
    let mut quota: usize = 1000;
    let mut packet_sz: usize = 100;
//
...
//
    ...
    before_cnt = netstat_get(before, "TCPAORequired", core::ptr::null_mut());
    after_cnt = netstat_get(after, "TCPAORequired", core::ptr::null_mut());
    if (after_cnt > before_cnt)
    test_fail("Segments without AO sign (%s): %" PRIu64 " => %" PRIu64,
    msg, before_cnt, after_cnt);
    else
    test_ok("No segments without AO sign (%s)", msg);
    before_cnt = netstat_get(before, "TCPAOGood", core::ptr::null_mut());
    after_cnt = netstat_get(after, "TCPAOGood", core::ptr::null_mut());
    if (after_cnt <= before_cnt)
    ...
    msg, before_cnt, after_cnt);
    before_cnt = netstat_get(before, "TCPAOBad", core::ptr::null_mut());
    after_cnt = netstat_get(after, "TCPAOBad", core::ptr::null_mut());
    if (after_cnt > before_cnt)
...
//
#[no_mangle]
unsafe extern "C" fn close_forced(sk: c_int) {
...
#[no_mangle]
unsafe extern "C" fn test_server_active_rst(port: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn test_server_passive_rst(port: c_uint) {
    ...
    struct timeval *ptv = core::ptr::null_mut();
    fd_set left;
    size_t i;
    int ret;
    FD_ZERO(&left);
    ...
    return -ENOENT;
    errno = 0;
    ret = select(nfd + 1, core::ptr::null_mut(), &fds, &efds, ptv);
    if (ret < 0)
    return -errno;
    if (!ret)
    return -ETIMEDOUT;
...
#[no_mangle]
unsafe extern "C" fn test_client_active_rst(port: c_uint) {
    static void test_client_active_rst(unsigned int port)
    {
    int i, sk[3], err;
    bool is_writable[ARRAY_SIZE(sk)] = {false};
    let mut last: c_uint = ARRAY_SIZE(sk) - 1;
...
//
    err = test_wait_fds(sk, last, core::ptr::null_mut(), last, TEST_TIMEOUT_SEC);
    if (err < 0)
    test_error("select(): %d", err);
    for (i = 0; i < ARRAY_SIZE(sk); i++) {
    let mut slen: socklen_t = sizeof(err);
...
#[no_mangle]
unsafe extern "C" fn test_client_passive_rst(port: c_uint) {
