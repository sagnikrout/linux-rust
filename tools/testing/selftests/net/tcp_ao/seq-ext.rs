...
//! Source: tools/testing/selftests/net/tcp_ao/seq-ext.c
#![no_std]
...
//
    ...
    let mut nr_packets: c_uint = 1000;
    let mut msg_len: c_uint = 1000;
    let mut quota: c_uint = nr_packets * msg_len;
...
// make them roll-over during quota, but on different segments
    ...
    before_good = netstat_get_one("TCPAOGood", core::ptr::null_mut());
    ...
    test_error("test_get_tcp_counters()");
    after_good = netstat_get_one("TCPAOGood", core::ptr::null_mut());
    ...
    after_bad = netstat_get_one("TCPAOBad", core::ptr::null_mut());
    if (after_bad)
