...
//! Source: tools/testing/selftests/net/tcp_ao/lib/proc.c
#![no_std]
...
pub struct netstat {
    ...
    let mut cmp: usize = max(len, strlen(ns.header_name));
    ...
    if (ret.header_name == core::ptr::null_mut())
    test_error("strndup()");
    ret.next = ns;
    ret.counters_nr = 0;
...
#[no_mangle]
unsafe extern "C" fn netstat_read_type(fnetstat: *mut FILE, dest: *mut netstat, line: *mut c_char) {
...
// dest = type;
    ...
    if (new_pos == core::ptr::null_mut())
    new_pos = strchr(pos, '\n');
    nc.name = strndup(pos, new_pos - pos);
    if (nc.name == core::ptr::null_mut())
    test_error("strndup()");
...
#[no_mangle]
unsafe extern "C" fn snmp6_read(fnetstat: *mut FILE, dest: *mut netstat) {
...
#[no_mangle]
pub unsafe extern "C" fn netstat_free(ns: *mut netstat) {
...
#[no_mangle]
pub unsafe extern "C" fn netstat_print_diff(nsa: *mut netstat, nsb: *mut netstat) {
    void netstat_print_diff(struct netstat *nsa, struct netstat *nsb)
    {
    size_t i, j;
    while (nsb != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn netstat_get(ns: *mut netstat, name: *const c_char, not_found: *mut bool) -> u64 {
