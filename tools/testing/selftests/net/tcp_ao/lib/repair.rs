...
//! Source: tools/testing/selftests/net/tcp_ao/lib/repair.c
#![no_std]
...
//
    ...
    {
    let mut len: socklen_t = sizeof(state.info);
    int ret;
...
pub unsafe extern "C" fn test_ao_checkpoint(sk: c_int, state: *mut tcp_ao_repair) {
    ...
    {
    let mut len: socklen_t = sizeof(*state);
    int ret;
    memset(state, 0, sizeof(*state));
...
#[no_mangle]
unsafe extern "C" fn test_sock_restore_seq(sk: c_int, queue: c_int, seq: u32) {
...
#[no_mangle]
unsafe extern "C" fn test_sock_restore_queue(sk: c_int, queue: c_int, buf: *mut c_void, len: c_int) {
    ...
    if (device != core::ptr::null_mut() && setsockopt(sk, SOL_SOCKET, SO_BINDTODEVICE,
...
pub unsafe extern "C" fn test_disable_repair(sk: c_int) {
    ...
    {
    let mut val: c_int = TCP_REPAIR_OFF_NO_WP;
    if (setsockopt(sk, SOL_TCP, TCP_REPAIR, &val, sizeof(val)))
    test_error("setsockopt(TCP_REPAIR)");
    }
#[no_mangle]
