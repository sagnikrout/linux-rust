...
//! Source: tools/testing/selftests/net/tcp_ao/unsigned-md5.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn server_add_routes() {
    static void server_add_routes(void)
    {
    let mut family: c_int = TEST_FAMILY;
...
#[no_mangle]
unsafe extern "C" fn server_add_fail_tests(port: *mut c_uint) {
    static void server_add_fail_tests(unsigned int *port)
    {
    let mut addr_any: union tcp_addr = {};
    try_accept("TCP-AO established: add TCP-MD5 key", (*port)++, core::ptr::null_mut(), 0,
    &addr_any, 0, 0, 100, 100, 0, "TCPAOGood", TEST_CNT_GOOD,
    1, 0);
    try_accept("TCP-MD5 established: add TCP-AO key", (*port)++, &addr_any,
    0, core::ptr::null_mut(), 0, 0, 0, 0, 0, core::ptr::null_mut(), 0, 1, 0);
    try_accept("non-signed established: add TCP-AO key", (*port)++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 0, 0, 0, 0, "CurrEstab", 0, 0, 0);
    }
#[no_mangle]
unsafe extern "C" fn server_vrf_tests(port: *mut c_uint) {
    ...
    let mut addr_any: union tcp_addr = {};
    server_add_routes();
    try_accept("[server] AO server (INADDR_ANY): AO client", port++, core::ptr::null_mut(), 0,
    &addr_any, 0, 0, 100, 100, 0, "TCPAOGood",
    TEST_CNT_GOOD, 0, 0);
    try_accept("[server] AO server (INADDR_ANY): MD5 client", port++, core::ptr::null_mut(), 0,
    &addr_any, 0, 0, 100, 100, 0, "TCPMD5Unexpected",
    TEST_CNT_NS_MD5_UNEXPECTED, 1, FAULT_TIMEOUT);
    try_accept("[server] AO server (INADDR_ANY): no sign client", port++, core::ptr::null_mut(), 0,
    &addr_any, 0, 0, 100, 100, 0, "TCPAORequired",
    TEST_CNT_AO_REQUIRED, 0, FAULT_TIMEOUT);
    try_accept("[server] AO server (AO_REQUIRED): AO client", port++, core::ptr::null_mut(), 0,
    &this_ip_dest, TEST_PREFIX, true,
    100, 100, 0, "TCPAOGood", TEST_CNT_GOOD, 0, 0);
    try_accept("[server] AO server (AO_REQUIRED): unsigned client", port++, core::ptr::null_mut(), 0,
    &this_ip_dest, TEST_PREFIX, true,
    100, 100, 0, "TCPAORequired",
    TEST_CNT_AO_REQUIRED, 0, FAULT_TIMEOUT);
    try_accept("[server] MD5 server (INADDR_ANY): AO client", port++, &addr_any, 0,
    core::ptr::null_mut(), 0, 0, 0, 0, 0, "TCPAOKeyNotFound",
    TEST_CNT_NS_KEY_NOT_FOUND, 1, FAULT_TIMEOUT);
    try_accept("[server] MD5 server (INADDR_ANY): MD5 client", port++, &addr_any, 0,
    core::ptr::null_mut(), 0, 0, 0, 0, 0, core::ptr::null_mut(), 0, 1, 0);
    try_accept("[server] MD5 server (INADDR_ANY): no sign client", port++, &addr_any,
    0, core::ptr::null_mut(), 0, 0, 0, 0, 0, "TCPMD5NotFound",
    TEST_CNT_NS_MD5_NOT_FOUND, 1, FAULT_TIMEOUT);
    try_accept("[server] no sign server: AO client", port++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 0, 0, 0, 0, "TCPAOKeyNotFound",
    TEST_CNT_NS_KEY_NOT_FOUND, 0, FAULT_TIMEOUT);
    try_accept("[server] no sign server: MD5 client", port++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 0, 0, 0, 0, "TCPMD5Unexpected",
    TEST_CNT_NS_MD5_UNEXPECTED, 1, FAULT_TIMEOUT);
    try_accept("[server] no sign server: no sign client", port++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 0, 0, 0, 0, "CurrEstab", 0, 0, 0);
    ...
    &this_ip_dest, TEST_PREFIX, &client2, TEST_PREFIX, 0,
    100, 100, 0, core::ptr::null_mut(), 0, 1, 0);
...
// Key rejected by the other side, failing short through skpair
    ...
    port++, &this_ip_dest, TEST_PREFIX, &client2, TEST_PREFIX, 0,
    100, 100, 0, core::ptr::null_mut(), 0, 1, FAULT_KEYREJECT);
    try_accept("[server] AO+MD5 server: client with both TCP-MD5 and [TCP-AO] keys",
    port++, &this_ip_dest, TEST_PREFIX, &client2, TEST_PREFIX, 0,
    100, 100, 0, core::ptr::null_mut(), 0, 1, FAULT_KEYREJECT);
...
#[no_mangle]
unsafe extern "C" fn client_bind(sk: c_int, bind_addr: union tcp_addr) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(fault(TIMEOUT): ret == -ETIMEDOUT &&) -> else {
    ...
    vrf = 0;
    if (set_ao_required) {
    let mut err: c_int = test_set_ao_flags(sk, true, 0);
    if (err)
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn client_add_ip(client: *mut union tcp_addr, ip: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn client_add_ips() {
...
#[no_mangle]
unsafe extern "C" fn client_add_fail_tests(port: *mut c_uint) {
...
#[no_mangle]
unsafe extern "C" fn client_vrf_tests(port: *mut c_uint) {
...
//
    ...
    let mut addr_any: union tcp_addr = {};
    client_add_ips();
    try_connect("AO server (INADDR_ANY): AO client", port++, core::ptr::null_mut(), 0,
    ...
    core::ptr::null_mut(), 0, 100, 100, 0, FAULT_TIMEOUT, 1, &this_ip_addr);
    trace_hash_event_expect(TCP_HASH_AO_REQUIRED, this_ip_addr,
    this_ip_dest, -1, port, 0, 0, 1, 0, 0, 0);
    try_connect("AO server (INADDR_ANY): unsigned client", port++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 100, 100, 0, FAULT_TIMEOUT, 0, &this_ip_addr);
    try_connect("AO server (AO_REQUIRED): AO client", port++, core::ptr::null_mut(), 0,
    &addr_any, 0, 100, 100, 0, 0, 0, &this_ip_addr);
    trace_hash_event_expect(TCP_HASH_AO_REQUIRED, client2,
    this_ip_dest, -1, port, 0, 0, 1, 0, 0, 0);
    try_connect("AO server (AO_REQUIRED): unsigned client", port++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 100, 100, 0, FAULT_TIMEOUT, 0, &client2);
    trace_ao_event_expect(TCP_AO_KEY_NOT_FOUND, this_ip_addr, this_ip_dest,
    -1, port, 0, 0, 1, 0, 0, 0, 100, 100, -1);
    try_connect("MD5 server (INADDR_ANY): AO client", port++, core::ptr::null_mut(), 0,
    &addr_any, 0, 100, 100, 0, FAULT_TIMEOUT, 1, &this_ip_addr);
    try_connect("MD5 server (INADDR_ANY): MD5 client", port++, &addr_any, 0,
    core::ptr::null_mut(), 0, 100, 100, 0, 0, 1, &this_ip_addr);
    trace_hash_event_expect(TCP_HASH_MD5_REQUIRED, this_ip_addr,
    this_ip_dest, -1, port, 0, 0, 1, 0, 0, 0);
    try_connect("MD5 server (INADDR_ANY): no sign client", port++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 100, 100, 0, FAULT_TIMEOUT, 1, &this_ip_addr);
    trace_ao_event_expect(TCP_AO_KEY_NOT_FOUND, this_ip_addr, this_ip_dest,
    -1, port, 0, 0, 1, 0, 0, 0, 100, 100, -1);
    try_connect("no sign server: AO client", port++, core::ptr::null_mut(), 0,
    &addr_any, 0, 100, 100, 0, FAULT_TIMEOUT, 0, &this_ip_addr);
    trace_hash_event_expect(TCP_HASH_MD5_UNEXPECTED, this_ip_addr,
    this_ip_dest, -1, port, 0, 0, 1, 0, 0, 0);
    try_connect("no sign server: MD5 client", port++, &addr_any, 0,
    core::ptr::null_mut(), 0, 100, 100, 0, FAULT_TIMEOUT, 1, &this_ip_addr);
    try_connect("no sign server: no sign client", port++, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0, 100, 100, 0, 0, 0, &this_ip_addr);
    try_connect("AO+MD5 server: AO client (matching)", port++, core::ptr::null_mut(), 0,
    &addr_any, 0, 100, 100, 0, 0, 1, &client2);
    trace_ao_event_expect(TCP_AO_KEY_NOT_FOUND, this_ip_addr, this_ip_dest,
    -1, port, 0, 0, 1, 0, 0, 0, 100, 100, -1);
    try_connect("AO+MD5 server: AO client (misconfig, matching MD5)",
    port++, core::ptr::null_mut(), 0, &addr_any, 0, 100, 100, 0,
    ...
    port++, &addr_any, 0, core::ptr::null_mut(), 0, 100, 100, 0, FAULT_TIMEOUT,
    1, &client2);
    ...
    port++, core::ptr::null_mut(), 0, core::ptr::null_mut(), 0, 100, 100, 0, 0, 1, &client3);
    trace_hash_event_expect(TCP_HASH_AO_REQUIRED, client2,
    this_ip_dest, -1, port, 0, 0, 1, 0, 0, 0);
    try_connect("AO+MD5 server: no sign client (misconfig, matching AO)",
    port++, core::ptr::null_mut(), 0, core::ptr::null_mut(), 0, 100, 100, 0, FAULT_TIMEOUT,
    1, &client2);
    trace_hash_event_expect(TCP_HASH_MD5_REQUIRED, this_ip_addr,
    this_ip_dest, -1, port, 0, 0, 1, 0, 0, 0);
