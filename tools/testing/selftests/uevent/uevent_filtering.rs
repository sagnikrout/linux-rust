...
//! Source: tools/testing/selftests/uevent/uevent_filtering.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn read_nointr(fd: c_int, buf: *mut c_void, count: usize) -> isize {
...
#[no_mangle]
pub unsafe extern "C" fn write_nointr(fd: c_int, buf: *const c_void, count: usize) -> isize {
...
pub unsafe extern "C" fn wait_for_pid(pid: pid_t) -> c_int {
    ...
    let mut rcv_buf_sz: c_int = __UEVENT_BUFFER_SIZE;
    let mut sync_add: u64 = 1;
    let mut sk_addr: sockaddr_nl = { 0 }, rcv_addr = { 0 };
    char buf[__UEVENT_BUFFER_SIZE] = { 0 };
    let mut iov: iovec = { buf, __UEVENT_BUFFER_SIZE };
...
#[no_mangle]
pub unsafe extern "C" fn trigger_uevent(times: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn set_death_signal() -> c_int {
...
//
    ...
    timeout.tv_nsec = 0;
    again:
    ret = sigtimedwait(&mask, core::ptr::null_mut(), &timeout);
    if (ret < 0) {
    if (errno == EINTR)
    goto again;
    if (!expect_uevent)
...
unsafe extern "C" fn signal_handler(sig: c_int) {
    ...
    ret = sigaction(SIGTERM, &act, core::ptr::null_mut());
    ASSERT_EQ(0, ret);
