...
//! Source: tools/testing/selftests/powerpc/syscalls/rtas_filter.c
#![no_std]
...
pub const BLOCK_SIZE: c_int = 4096;
pub const PAGE_SIZE: c_int = 4096;
pub const MAX_PAGES: c_int = 64;
...
pub struct rtas_args {
    ...
    pub nret: __be32,
    pub args: [__be32; 16],
    pub /: *mut *mut *mut __be32 rets; / Pointer to return values in args[].,
}
...
pub struct region {
    ...
    {
    char path[PATH_MAX];
    let mut len: c_int = snprintf(path, sizeof(path), "%s/%s", prop_path, prop_name);
    if (len < 0 || len >= sizeof(path))
    return -ENOMEM;
    return read_file_alloc(path, prop_val, prop_len);
    }
#[no_mangle]
pub unsafe extern "C" fn rtas_token(call_name: *const c_char) -> c_int {
    int rtas_token(const char *call_name)
    {
    char *prop_buf = core::ptr::null_mut();
    size_t len;
    int rc;
...
#[no_mangle]
unsafe extern "C" fn read_kregion_bounds(kregion: *mut region) -> c_int {
    static int read_kregion_bounds(struct region *kregion)
    {
    char *buf;
    int err;
    err = read_file_alloc("/proc/ppc64/rtas/rmo_buffer", &buf, core::ptr::null_mut());
    if (err) {
