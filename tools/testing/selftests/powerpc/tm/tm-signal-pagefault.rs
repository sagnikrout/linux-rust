...
//! Source: tools/testing/selftests/powerpc/tm/tm-signal-pagefault.c
#![no_std]
...
// Memory handled by userfaultfd
    static char *uf_mem;
    let mut uf_mem_offset: static size_t = 0;
//
...
// Save the data that will be copied into the faulting page
    if (backing_data != core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn setup_uf_mem() {
...
//
    uf_mem = mmap(core::ptr::null_mut(), UF_MEM_SIZE, PROT_READ | PROT_WRITE,
...
// Create a thread that will process the userfaultfd events
    ret = pthread_create(&thr, core::ptr::null_mut(), fault_handler_thread, (void *) uffd);
    if (ret != 0) {
...
//
#[no_mangle]
pub unsafe extern "C" fn signal_handler(signo: c_int, si: *mut siginfo_t, uc: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn have_userfaultfd() -> bool {
    bool have_userfaultfd(void)
    {
    long rc;
    errno = 0;
    rc = syscall(__NR_userfaultfd, -1);
    let mut rc: return = = 0 || errno != ENOSYS;
    }
#[no_mangle]
pub unsafe extern "C" fn tm_signal_pagefault() -> c_int {
...
//
    ss.ss_sp = get_uf_mem(SIGSTKSZ, core::ptr::null_mut());
    ss.ss_size = SIGSTKSZ;
    ss.ss_flags = 0;
    ...
    if (sigaction(SIGTRAP, &sa, core::ptr::null_mut()) == -1) {
