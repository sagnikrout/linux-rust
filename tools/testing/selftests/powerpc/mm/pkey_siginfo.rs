...
//! Source: tools/testing/selftests/powerpc/mm/pkey_siginfo.c
#![no_std]
...
pub const NUM_ITERATIONS: c_int = 1000000;
...
// Allocate the region
    r.size = getpagesize();
    r.base = mmap(core::ptr::null_mut(), r.size, PROT_RWX,
...
// Setup barrier for protect and protect-and-access threads
    ...
    FAIL_IF(pthread_barrier_init(&iteration_barrier, core::ptr::null_mut(), 2) != 0);
// Setup and start protect and protect-and-read threads
    ...
    FAIL_IF(pthread_join(prot_thread, core::ptr::null_mut()) != 0);
    FAIL_IF(pthread_join(pacc_thread, core::ptr::null_mut()) != 0);
