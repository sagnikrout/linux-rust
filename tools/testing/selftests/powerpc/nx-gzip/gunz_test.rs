...
//! Source: tools/testing/selftests/powerpc/nx-gzip/gunz_test.c
#![no_std]
...
pub const FNAME_MAX: c_int = 1024;
...
// first and last used parts start here
    ...
    let mut fifo_in_len: c_int = 1<<24;
    let mut fifo_out_len: c_int = 1<<24;
    let mut page_sz: c_int = 1<<16;
    let mut line_sz: c_int = 1<<7;
    let mut window_max: c_int = 1<<15;
//
...
//
    ...
    uint32_t ddecnt;
    uint32_t bytes;
    if (addr == core::ptr::null_mut() && len == 0) {
    clearp_dde(ddl);
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn decompress_file(argc: c_int, argv: *mut c_char, devhandle: *mut c_void) -> c_int {
    int decompress_file(int argc, char **argv, void *devhandle)
    {
    FILE *inpf = core::ptr::null_mut();
    FILE *outf = core::ptr::null_mut();
...
// when using mmap'ed files
    ...
    char *wp;
    inpf = fopen(argv[1], "r");
    if (inpf == core::ptr::null_mut()) {
    perror(argv[1]);
    return -1;
    }
// Make a new file name to write to.  Ignoring '.gz'
    wp = (core::ptr::null_mut() != (wp = strrchr(argv[1], '/'))) ? (wp+1) : argv[1];
    strcpy(w, wp);
    strcat(w, ".nx.gunzip");
    outf = fopen(w, "w");
    if (outf == core::ptr::null_mut()) {
    perror(w);
    return -1;
    }
    }
...
//
    ...
    fifo_in_len + page_sz)) != core::ptr::null_mut());
    assert((fifo_out = (char *)(uintptr_t)aligned_alloc(line_sz,
    fifo_out_len + page_sz + line_sz)) != core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(8: used_in <) -> else {
