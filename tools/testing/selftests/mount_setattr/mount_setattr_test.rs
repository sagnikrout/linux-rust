...
//! Source: tools/testing/selftests/mount_setattr/mount_setattr_test.c
#![no_std]
...
pub const __NR_mount_setattr: c_int = 552;

pub const __NR_mount_setattr: c_int = 442;

pub const __NR_open_tree_attr: c_int = 577;

pub const __NR_open_tree_attr: c_int = 467;

pub const MOUNT_ATTR_IDMAP: c_uint = 0x00100000;

pub const MOUNT_ATTR_NOSYMFOLLOW: c_uint = 0x00200000;
...
#[no_mangle]
unsafe extern "C" fn mount_setattr_supported() -> bool {
    static bool mount_setattr_supported(void)
    {
    int ret;
    ret = sys_mount_setattr(-EBADF, "", AT_EMPTY_PATH, core::ptr::null_mut(), 0);
    ...
    ASSERT_EQ(mount("/tmp", "/mnt/A/AA", core::ptr::null_mut(), MS_BIND | MS_REC, core::ptr::null_mut()), 0);
    ...
    {
    let mut old_flags: c_uint = 0, new_flags = 0, expected_flags = 0;
    char *s = "dummy";
    let mut invalid_attr: mount_attr = {};
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mount_attr_large {
    pub attr1: mount_attr,
    pub attr2: mount_attr,
    pub attr3: mount_attr,
    pub {}: } large_attr =,
    if (!mount_setattr_supported())
    pub supported"): SKIP(return, "mount_setattr syscall not,
    pub read_mnt_flags("/mnt/A"): old_flags =,
    pub 0): ASSERT_GT(old_flags,,
    ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, core::ptr::null_mut(),
    pub 0): sizeof(invalid_attr)),,
    pub EFAULT): ASSERT_EQ(errno,,
    ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, (void *)s,
    pub 0): sizeof(invalid_attr)),,
    pub EINVAL): ASSERT_EQ(errno,,
    pub 0): ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &invalid_attr, 0),,
    pub EINVAL): ASSERT_EQ(errno,,
    ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &invalid_attr,
    pub 0): sizeof(invalid_attr) / 2),,
    pub EINVAL): ASSERT_EQ(errno,,
    ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE, &invalid_attr,
    pub 0): sizeof(invalid_attr) / 2),,
    pub EINVAL): ASSERT_EQ(errno,,
    ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE,
    pub 0): *mut *mut (void )&large_attr, sizeof(large_attr)),,
    pub MOUNT_ATTR_RDONLY: large_attr.attr3.attr_set =,
    ASSERT_NE(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE,
    pub 0): *mut *mut (void )&large_attr, sizeof(large_attr)),,
    pub 0: large_attr.attr3.attr_set =,
    pub MOUNT_ATTR_RDONLY: large_attr.attr1.attr_set =,
    ASSERT_EQ(sys_mount_setattr(-1, "/mnt/A", AT_RECURSIVE,
    pub 0): *mut *mut (void )&large_attr, sizeof(large_attr)),,
    pub old_flags: expected_flags =,
    pub MS_RDONLY: expected_flags |=,
    pub read_mnt_flags("/mnt/A"): new_flags =,
    pub expected_flags): ASSERT_EQ(new_flags,,
    pub read_mnt_flags("/mnt/A/AA"): new_flags =,
    pub expected_flags): ASSERT_EQ(new_flags,,
    pub read_mnt_flags("/mnt/A/AA/B"): new_flags =,
    pub expected_flags): ASSERT_EQ(new_flags,,
    pub read_mnt_flags("/mnt/A/AA/B/BB"): new_flags =,
    pub expected_flags): ASSERT_EQ(new_flags,,
    }
    TEST_F(mount_setattr, basic)
    {
    pub 0: unsigned int old_flags = 0, new_flags = 0, expected_flags =,
    struct mount_attr attr = {
...
//
    ...
    {
    int fd, dfd;
    let mut old_flags: c_uint = 0, new_flags = 0;
    struct mount_attr attr = {
    .attr_set	= MOUNT_ATTR_RDONLY | MOUNT_ATTR_NOEXEC | MOUNT_ATTR_RELATIME,
    .attr_clr	= MOUNT_ATTR__ATIME,
    .propagation	= MS_SHARED,
...
// All writers are gone so this should succeed.
    ...
    {
    let mut old_flags1: c_uint = 0, old_flags2 = 0, new_flags = 0, expected_flags = 0;
    struct mount_attr attr = {
...
// Try to change mount options from multiple threads.
    ...
    ASSERT_EQ(pthread_create(&threads[i], &pattr, mount_setattr_thread, core::ptr::null_mut()), 0);
    for (j = 0; j < i; j++) {
    void *retptr = core::ptr::null_mut();
    ...
    {
    let mut img_fd: c_int = -EBADF;
    ASSERT_EQ(unshare(CLONE_NEWNS), 0);
    ASSERT_EQ(mount(core::ptr::null_mut(), "/", core::ptr::null_mut(), MS_REC | MS_PRIVATE, 0), 0);
    (void)umount2("/mnt", MNT_DETACH);
    (void)umount2("/tmp", MNT_DETACH);
    ASSERT_EQ(mount("testing", "/tmp", "tmpfs", MS_NOATIME | MS_NODEV,
    "size=100000,mode=700"), 0);
...
//
    ...
    {
    let mut open_tree_fd: c_int = -EBADF;
    struct mount_attr attr = {
    .attr_set = MOUNT_ATTR_IDMAP,
    };
    if (!mount_setattr_supported())
...
#[no_mangle]
unsafe extern "C" fn get_userns_fd_cb(data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_userns_fd(nsid: c_ulong, hostid: c_ulong, range: c_ulong) -> c_int {
    static int get_userns_fd(unsigned long nsid, unsigned long hostid, unsigned long range)
    {
    int ret;
    pid_t pid;
    char path[256];
    pid = do_clone(get_userns_fd_cb, core::ptr::null_mut(), CLONE_NEWUSER);
    if (pid < 0)
    return -errno;
...
//
    ...
    {
    let mut fd_tree_base: c_int = -EBADF, fd_tree_subdir = -EBADF;
    struct statx stx;
...
//
    ...
    {
    let mut fd_tree1: c_int = -EBADF, fd_tree2 = -EBADF;
//
// |-/mnt/A               testing tmpfs
// `-/mnt/A/AA          testing tmpfs
// `-/mnt/A/AA/B      testing tmpfs
...
//
    ...
    ASSERT_EQ(unshare(CLONE_NEWNS), 0);
    ASSERT_EQ(mount(core::ptr::null_mut(), "/mnt", core::ptr::null_mut(), MS_REC | MS_SHARED, core::ptr::null_mut()), 0);
//
// Copy the following mount tree:
//
// /mnt                   testing tmpfs
...
// Mount a tmpfs filesystem over /mnt/A.
    ASSERT_EQ(mount(core::ptr::null_mut(), "/mnt/A", "tmpfs", 0, core::ptr::null_mut()), 0);
