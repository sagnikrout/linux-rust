...
//! Source: tools/testing/selftests/pidfd/pidfd_setns_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn switch_timens() -> bool {
...
// Create task that exits right away.
    ...
    ASSERT_EQ(sys_waitid(P_PID, self.child_pid_exited, core::ptr::null_mut(), WEXITED | WNOWAIT), 0);
    self.pidfd = sys_pidfd_open(self.pid, 0);
...
#[no_mangle]
pub unsafe extern "C" fn if(0: self->nsfds[PIDFD_NS_PID] >=) -> else {
    else if (self.nsfds[PIDFD_NS_PID] >= 0)
    self.child_pid1 = create_child(&self.child_pidfd1, CLONE_NEWPID);
#[no_mangle]
pub unsafe extern "C" fn if(0: self->nsfds[PIDFD_NS_USER] >=) -> else {
...
pub unsafe extern "C" fn if(0: self->nsfds[PIDFD_NS_USER] >=) -> else {
    ...
    ASSERT_EQ(sys_waitid(P_PID, self.child_pid_exited, core::ptr::null_mut(), WEXITED), 0);
    ASSERT_EQ(sys_waitid(P_PID, self.child_pid1, core::ptr::null_mut(), WEXITED), 0);
    ASSERT_EQ(sys_waitid(P_PID, self.child_pid2, core::ptr::null_mut(), WEXITED), 0);
    }
#[no_mangle]
unsafe extern "C" fn preserve_ns(pid: c_int, ns: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn in_same_namespace(ns_fd1: c_int, pid2: pid_t, ns: *const c_char) -> c_int {
