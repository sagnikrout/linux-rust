...
//! Source: tools/testing/selftests/pidfd/pidfd_info_test.c
#![no_std]
...
// SIGKILL and reap.
    ...
    _exit(EXIT_SUCCESS);
    EXPECT_EQ(sys_waitid(P_PID, self.child_pid4, core::ptr::null_mut(), WEXITED), 0);
    }
    FIXTURE_TEARDOWN(pidfd_info)
    {
    sys_pidfd_send_signal(self.child_pidfd1, SIGKILL, core::ptr::null_mut(), 0);
    if (self.child_pidfd1 >= 0)
    EXPECT_EQ(0, close(self.child_pidfd1));
    sys_waitid(P_PID, self.child_pid1, core::ptr::null_mut(), WEXITED);
    sys_pidfd_send_signal(self.child_pidfd2, SIGKILL, core::ptr::null_mut(), 0);
    if (self.child_pidfd2 >= 0)
    EXPECT_EQ(0, close(self.child_pidfd2));
    sys_waitid(P_PID, self.child_pid2, core::ptr::null_mut(), WEXITED);
    sys_waitid(P_PID, self.child_pid3, core::ptr::null_mut(), WEXITED);
    sys_waitid(P_PID, self.child_pid4, core::ptr::null_mut(), WEXITED);
    }
...
// Process has already been reaped and PIDFD_INFO_EXIT hasn't been set.
    ...
    let mut ipc_socket: c_int = *(int *)arg;
...
// The thread will outlive the thread-group leader.
    if (pthread_create(&thread, core::ptr::null_mut(), pidfd_info_pause_thread, &ipc_sockets[1]))
...
//
    EXPECT_EQ(sys_pidfd_send_signal(pidfd_leader, SIGKILL, core::ptr::null_mut(), 0), 0);
    EXPECT_EQ(sys_waitid(P_PIDFD, pidfd_leader, core::ptr::null_mut(), WEXITED), 0);
    fds.events = POLLIN;
    fds.fd = pidfd_leader;
    nevents = poll(&fds, 1, -1);
    ASSERT_EQ(nevents, 1);
...
// Inform the grand-parent what the tid of this thread is.
    ...
    return core::ptr::null_mut();
    close(ipc_socket);
    sys_execveat(AT_FDCWD, "pidfd_exec_helper", core::ptr::null_mut(), core::ptr::null_mut(), 0);
    return core::ptr::null_mut();
    }
    ...
    close(ipc_sockets[0]);
// The thread will outlive the thread-group leader.
    if (pthread_create(&thread, core::ptr::null_mut(), pidfd_info_thread_exec, &ipc_sockets[1]))
    syscall(__NR_exit, EXIT_FAILURE);
// Make the thread-group leader exit prematurely.
    syscall(__NR_exit, EXIT_SUCCESS);
    }
...
// Inform the grand-parent what the tid of this thread is.
    ...
    close(ipc_sockets[0]);
// The thread will outlive the thread-group leader.
    if (pthread_create(&thread, core::ptr::null_mut(), pidfd_info_thread_exec_sane, &ipc_sockets[1]))
    syscall(__NR_exit, EXIT_FAILURE);
//
...
// Clean up
    sys_pidfd_send_signal(pidfd, SIGKILL, core::ptr::null_mut(), 0);
    sys_waitid(P_PIDFD, pidfd, core::ptr::null_mut(), WEXITED);
    close(pidfd);
    }
//
