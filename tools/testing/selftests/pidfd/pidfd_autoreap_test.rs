...
//! Source: tools/testing/selftests/pidfd/pidfd_autoreap_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn drop_all_caps() -> c_int {
    static int drop_all_caps(void)
    {
    let mut hdr: cap_header = { .version = _LINUX_CAPABILITY_VERSION_3 };
    struct cap_data data[2] = {};
    return syscall(__NR_capset, &hdr, data);
    }
#[no_mangle]
unsafe extern "C" fn create_autoreap_child(pidfd: *mut c_int) -> pid_t {
...
//
    usleep(200000);
    ret = waitpid(pid, core::ptr::null_mut(), WNOHANG);
    ASSERT_EQ(ret, -1);
...
//
    TEST(autoreap_basic)
    {
    let mut info: pidfd_info = { .mask = PIDFD_INFO_EXIT };
    let mut pidfd: c_int = -1, ret;
    struct pollfd pfd;
    pid_t pid;
...
//
    ...
    ASSERT_GE(pidfd, 0);
// Kill the child.
    ret = sys_pidfd_send_signal(pidfd, SIGKILL, core::ptr::null_mut(), 0);
    ASSERT_EQ(ret, 0);
...
// Read grandchild's PID.
    ...
    ASSERT_GT(ret, 0);
    if (buf[0] == 'E') {
    waitpid(mid_pid, core::ptr::null_mut(), 0);
...
// Wait for the middle child to exit.
    ret = waitpid(mid_pid, core::ptr::null_mut(), 0);
...
//
    ret = waitpid(grandchild_pid, core::ptr::null_mut(), WNOHANG);
    EXPECT_EQ(ret, -1);
...
//
    ...
    pthread_create(&thread, core::ptr::null_mut(), thread_func, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn create_autokill_child(pidfd: *mut c_int) -> pid_t {
...
//
    TEST(autokill_basic)
    {
    let mut pidfd: c_int = -1, pollfd_fd = -1, ret;
    struct pollfd pfd;
    pid_t pid;
...
//
    ...
    {
    let mut pidfd: c_int = -1, open_fd = -1, ret;
    struct pollfd pfd;
    pid_t pid;
    pid = create_autokill_child(&pidfd);
    if (pid < 0 && errno == EINVAL)
