...
//! Source: tools/testing/selftests/cgroup/test_memcontrol.c
#![no_std]
...
pub const MEMCG_SOCKSTAT_WAIT_RETRIES: c_int = 30;
    static bool has_localevents;
    static bool has_recursiveprot;
    static int page_size;
#[no_mangle]
pub unsafe extern "C" fn get_temp_fd() -> c_int {
    int get_temp_fd(void)
    {
    return open(".", O_TMPFILE | O_RDWR | O_EXCL);
    }
#[no_mangle]
pub unsafe extern "C" fn alloc_pagecache(fd: c_int, size: usize) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn alloc_anon(cgroup: *const c_char, arg: *mut c_void) -> c_int {
    int alloc_anon(const char *cgroup, void *arg)
    {
    let mut size: usize = (unsigned long)arg;
    char *buf;
...
#[no_mangle]
pub unsafe extern "C" fn is_swap_enabled() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn set_oom_adj_score(pid: c_int, score: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_subtree_control(root: *const c_char) -> c_int {
    static int test_memcg_subtree_control(const char *root)
    {
    char *parent, *child, *parent2 = core::ptr::null_mut(), *child2 = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn alloc_anon_50M_check(cgroup: *const c_char, arg: *mut c_void) -> c_int {
    static int alloc_anon_50M_check(const char *cgroup, void *arg)
    {
    let mut size: usize = MB(50);
    char *buf;
    long anon, current;
    let mut ret: c_int = -1;
    buf = alloc_and_populate_anon(size);
...
#[no_mangle]
unsafe extern "C" fn alloc_pagecache_50M_check(cgroup: *const c_char, arg: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_current_peak(root: *const c_char) -> c_int {
    static int test_memcg_current_peak(const char *root)
    {
    let mut ret: c_int = KSFT_FAIL;
    long current, peak, peak_reset;
    char *memcg;
    let mut fd2_closed: bool = false, fd3_closed = false, fd4_closed = false;
    let mut peak_fd: c_int = -1, peak_fd2 = -1, peak_fd3 = -1, peak_fd4 = -1;
    struct stat ss;
    ...
    if (peak != 0)
    goto cleanup;
    if (cg_run(memcg, alloc_anon_50M_check, core::ptr::null_mut()))
    goto cleanup;
    peak = cg_read_long(memcg, "memory.peak");
    if (peak < MB(50))
    goto cleanup;
...
// Make sure a completely independent read isn't affected by our  FD-local reset above
    ...
    goto cleanup;
    if (cg_run(memcg, alloc_pagecache_50M_check, core::ptr::null_mut()))
    goto cleanup;
    peak = cg_read_long(memcg, "memory.peak");
    if (peak < MB(50))
    goto cleanup;
...
#[no_mangle]
unsafe extern "C" fn alloc_pagecache_50M_noexit(cgroup: *const c_char, arg: *mut c_void) -> c_int {
    static int alloc_pagecache_50M_noexit(const char *cgroup, void *arg)
    {
    let mut fd: c_int = (long)arg;
...
#[no_mangle]
unsafe extern "C" fn alloc_anon_noexit(cgroup: *const c_char, arg: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn cg_test_proc_killed(cgroup: *const c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_protection(root: *const c_char, min: bool) -> c_int {
    static int test_memcg_protection(const char *root, bool min)
    {
    let mut ret: c_int = KSFT_FAIL, rc;
    char *parent[3] = {core::ptr::null_mut()};
    char *children[4] = {core::ptr::null_mut()};
...
// No memory.min on older kernels is fine
    ...
    goto cleanup;
#[no_mangle]
pub unsafe extern "C" fn if(rc: !min &&) -> else {
    fprintf(stderr,
...
//
    for (i = 0; i < ARRAY_SIZE(children); i++) {
    let mut ignore_low_events_index: c_int = has_recursiveprot ? 2 : -1;
    let mut no_low_events_index: c_int = 1;
    long low, oom;
...
#[no_mangle]
unsafe extern "C" fn test_memcg_min(root: *const c_char) -> c_int {
    static int test_memcg_min(const char *root)
    {
    return test_memcg_protection(root, true);
    }
#[no_mangle]
unsafe extern "C" fn test_memcg_low(root: *const c_char) -> c_int {
    static int test_memcg_low(const char *root)
    {
    return test_memcg_protection(root, false);
    }
#[no_mangle]
unsafe extern "C" fn alloc_pagecache_max_30M(cgroup: *const c_char, arg: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_high(root: *const c_char) -> c_int {
    ...
    if (!cg_run(memcg, alloc_pagecache_50M_check, core::ptr::null_mut()))
    goto cleanup;
    if (cg_run(memcg, alloc_pagecache_max_30M, core::ptr::null_mut()))
    goto cleanup;
...
#[no_mangle]
unsafe extern "C" fn alloc_anon_mlock(cgroup: *const c_char, arg: *mut c_void) -> c_int {
    static int alloc_anon_mlock(const char *cgroup, void *arg)
    {
    let mut size: usize = (size_t)arg;
    void *buf;
    buf = mmap(core::ptr::null_mut(), size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON,
    0, 0);
    if (buf == MAP_FAILED)
    return -1;
    mlock(buf, size);
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_high_sync(root: *const c_char) -> c_int {
    static int test_memcg_high_sync(const char *root)
    {
    let mut ret: c_int = KSFT_FAIL, pid, fd = -1;
    char *memcg;
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_max(root: *const c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn reclaim_until(memcg: *const c_char, goal: c_long) -> bool {
    static bool reclaim_until(const char *memcg, long goal)
    {
    char buf[64];
    int retries, err;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: reclaimed) -> else {
    else if (reclaimed)
    return false;
    ...
    reclaimed = true;
#[no_mangle]
pub unsafe extern "C" fn if(-EAGAIN: err !=) -> else {
    else if (err != -EAGAIN)
    return false;
    }
    return reclaimed;
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_reclaim(root: *const c_char) -> c_int {
...
unsafe extern "C" fn alloc_anon_50M_check_swap(cgroup: *const c_char, arg: *mut c_void) -> c_int {
    ...
    {
    let mut mem_max: c_long = (long)arg;
    let mut size: usize = MB(50);
    char *buf;
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_swap_max_peak(root: *const c_char) -> c_int {
    ...
    long max, peak;
    struct stat ss;
    let mut swap_peak_fd: c_int = -1, mem_peak_fd = -1;
...
// switch the swap and mem fds into local-peak tracking mode
    let mut peak_reset: c_int = write(swap_peak_fd, reset_string, sizeof(reset_string));
    if (peak_reset != sizeof(reset_string))
    goto cleanup;
    if (cg_read_long_fd(swap_peak_fd))
    goto cleanup;
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_oom_events(root: *const c_char) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcp_server_args {
    pub port: c_ushort,
    pub ctl: [c_int; 2],
}

#[no_mangle]
unsafe extern "C" fn tcp_server(cgroup: *const c_char, arg: *mut c_void) -> c_int {
    static int tcp_server(const char *cgroup, void *arg)
    {
    struct tcp_server_args *srv_args = arg;
    let mut saddr: sockaddr_in6 = { 0 };
    let mut slen: socklen_t = sizeof(saddr);
...
// Pass back errno to the ctl_fd
    ...
    client_sk = accept(sk, core::ptr::null_mut(), core::ptr::null_mut());
    if (client_sk < 0)
    goto cleanup;
    ret = -1;
    for (;;) {
...
#[no_mangle]
unsafe extern "C" fn tcp_client(cgroup: *const c_char, port: c_ushort) -> c_int {
    ...
    ret = getaddrinfo(server, servport, core::ptr::null_mut(), &ai);
    if (ret)
    return ret;
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_sock(root: *const c_char) -> c_int {
    static int test_memcg_sock(const char *root)
    {
    let mut bind_retries: c_int = 5, ret = KSFT_FAIL, pid, err;
    unsigned short port;
    char *memcg;
    let mut sock_post: c_long = -1;
    memcg = cg_name(root, "memcg_test");
    if (!memcg)
    goto cleanup;
    if (cg_create(memcg))
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_oom_group_leaf_events(root: *const c_char) -> c_int {
...
//
    ...
    goto cleanup;
#[no_mangle]
pub unsafe extern "C" fn if(0: !has_localevents && parent_oom_events <=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_oom_group_parent_events(root: *const c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn test_memcg_oom_group_score_events(root: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn read_event(inotify_fd: c_int, expected_event: c_int, expected_wd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_memcg_inotify_delete_file(root: *const c_char) -> c_int {
    static int test_memcg_inotify_delete_file(const char *root)
    {
    let mut ret: c_int = KSFT_FAIL;
    char *memcg = core::ptr::null_mut();
    int fd, wd;
    memcg = cg_name(root, "memcg_test_0");
    if (!memcg)
    goto cleanup;
...
#[no_mangle]
unsafe extern "C" fn test_memcg_inotify_delete_dir(root: *const c_char) -> c_int {
