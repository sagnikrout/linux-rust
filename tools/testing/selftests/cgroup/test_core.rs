...
//! Source: tools/testing/selftests/cgroup/test_core.c
#![no_std]
...
pub const CLONE_NEWCGROUP: c_int = 0;

#[no_mangle]
unsafe extern "C" fn touch_anon(buf: *mut c_char, size: usize) -> c_int {
    ...
    return -1;
    while (size > 0) {
    let mut ret: isize = read(fd, pos, size);
    if (ret < 0) {
    if (errno != EINTR) {
    close(fd);
    return -1;
...
#[no_mangle]
unsafe extern "C" fn alloc_and_touch_anon_noexit(cgroup: *const c_char, arg: *mut c_void) -> c_int {
...
unsafe extern "C" fn test_cgcore_destroy(root: *const c_char) -> c_int {
    ...
    char *cg_test = core::ptr::null_mut();
    int child_pid;
    char buf[BUF_SIZE];
...
unsafe extern "C" fn test_cgcore_populated(root: *const c_char) -> c_int {
    ...
    let mut ret: c_int = KSFT_FAIL;
    int err;
    char *cg_test_a = core::ptr::null_mut(), *cg_test_b = core::ptr::null_mut();
    char *cg_test_c = core::ptr::null_mut(), *cg_test_d = core::ptr::null_mut();
...
unsafe extern "C" fn test_cgcore_invalid_domain(root: *const c_char) -> c_int {
    ...
    {
    let mut ret: c_int = KSFT_FAIL;
    char *grandparent = core::ptr::null_mut(), *parent = core::ptr::null_mut(), *child = core::ptr::null_mut();
    if (cg_test_v1_named)
    return KSFT_SKIP;
...
unsafe extern "C" fn test_cgcore_proc_migration(root: *const c_char) -> c_int {
    ...
    char *src = core::ptr::null_mut(), *dst = core::ptr::null_mut();
    ...
    if (pthread_create(&threads[c_threads], core::ptr::null_mut(), dummy_thread_fn, core::ptr::null_mut()))
    goto cleanup;
    }
    ...
    for (t = 0; t < c_threads; ++t) {
    pthread_join(threads[t], core::ptr::null_mut());
    }
    cg_enter_current(root);
    if (dst)
    cg_destroy(dst);
...
unsafe extern "C" fn test_cgcore_thread_migration(root: *const c_char) -> c_int {
    ...
    {
    let mut ret: c_int = KSFT_FAIL;
    char *dom = core::ptr::null_mut();
    char line[PATH_MAX];
    char *grps[3] = { (char *)root, core::ptr::null_mut(), core::ptr::null_mut() };
    pthread_t thr;
    void *retval;
    ...
    goto cleanup;
    if (pthread_create(&thr, core::ptr::null_mut(), migrating_thread_fn, grps))
    goto cleanup;
...
unsafe extern "C" fn test_cgcore_lesser_euid_open(root: *const c_char) -> c_int {
    ...
    let mut ret: c_int = KSFT_FAIL;
    char *cg_test_a = core::ptr::null_mut(), *cg_test_b = core::ptr::null_mut();
    char *cg_test_a_procs = core::ptr::null_mut(), *cg_test_b_procs = core::ptr::null_mut();
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lesser_ns_open_thread_arg {
...
unsafe extern "C" fn test_cgcore_lesser_ns_open(root: *const c_char) -> c_int {
    ...
    let mut ret: c_int = KSFT_FAIL;
    char *cg_test_a = core::ptr::null_mut(), *cg_test_b = core::ptr::null_mut();
    char *cg_test_b_procs = core::ptr::null_mut();
    let mut cg_test_b_procs_fd: c_int = -1;
    let mut targ: lesser_ns_open_thread_arg = { .fd = -1 };
    pid_t pid;
    int status;
    if (!nsdelegate)
    return KSFT_SKIP;
...
#[no_mangle]
unsafe extern "C" fn setup_named_v1_root(root: *mut c_char, len: usize, name: *const c_char) -> c_int {
