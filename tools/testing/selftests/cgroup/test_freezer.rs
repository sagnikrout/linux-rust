...
//! Source: tools/testing/selftests/cgroup/test_freezer.c
#![no_std]
...
unsafe extern "C" fn test_cgfreezer_simple(root: *const c_char) -> c_int {
    ...
    goto cleanup;
    for (i = 0; i < 100; i++)
    cg_run_nowait(cgroup, child_fn, core::ptr::null_mut());
...
unsafe extern "C" fn test_cgfreezer_tree(root: *const c_char) -> c_int {
    ...
    goto cleanup;
    cg_run_nowait(cgroup[2], child_fn, core::ptr::null_mut());
    cg_run_nowait(cgroup[7], child_fn, core::ptr::null_mut());
    cg_run_nowait(cgroup[9], child_fn, core::ptr::null_mut());
    cg_run_nowait(cgroup[9], child_fn, core::ptr::null_mut());
    cg_run_nowait(cgroup[9], child_fn, core::ptr::null_mut());
//
...
unsafe extern "C" fn test_cgfreezer_forkbomb(root: *const c_char) -> c_int {
    ...
    if (cg_create(cgroup))
    goto cleanup;
    cg_run_nowait(cgroup, forkbomb_fn, core::ptr::null_mut());
    usleep(100000);
    if (cg_freeze_wait(cgroup, true))
    goto cleanup;
...
unsafe extern "C" fn test_cgfreezer_mkdir(root: *const c_char) -> c_int {
    ...
    {
    let mut ret: c_int = KSFT_FAIL;
    char *parent, *child = core::ptr::null_mut();
    int pid;
    ...
    if (cg_create(child))
    goto cleanup;
    pid = cg_run_nowait(child, child_fn, core::ptr::null_mut());
    if (pid < 0)
    goto cleanup;
...
unsafe extern "C" fn test_cgfreezer_migrate(root: *const c_char) -> c_int {
    ...
    goto cleanup;
    pid = cg_run_nowait(cgroup[0], child_fn, core::ptr::null_mut());
    if (pid < 0)
    goto cleanup;
...
unsafe extern "C" fn test_cgfreezer_ptrace(root: *const c_char) -> c_int {
    ...
    if (cg_create(cgroup))
    goto cleanup;
    pid = cg_run_nowait(cgroup, child_fn, core::ptr::null_mut());
    if (pid < 0)
    goto cleanup;
    ...
    if (cg_freeze_wait(cgroup, true))
    goto cleanup;
    if (ptrace(PTRACE_SEIZE, pid, core::ptr::null_mut(), core::ptr::null_mut()))
    goto cleanup;
    if (ptrace(PTRACE_INTERRUPT, pid, core::ptr::null_mut(), core::ptr::null_mut()))
    goto cleanup;
...
//
    ...
    goto cleanup;
    if (ptrace(PTRACE_GETSIGINFO, pid, core::ptr::null_mut(), &siginfo))
    goto cleanup;
...
unsafe extern "C" fn proc_check_stopped(pid: c_int) -> c_int {
    ...
    if (strstr(buf, "(test_freezer) T ") == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn vfork_fn(cgroup: *const c_char, arg: *mut c_void) -> c_int {
    static int vfork_fn(const char *cgroup, void *arg)
    {
    let mut pid: c_int = vfork();
    if (pid == 0)
    while (true)
    sleep(1);
    return pid;
...
unsafe extern "C" fn test_cgfreezer_vfork(root: *const c_char) -> c_int {
    ...
    if (cg_create(cgroup))
    goto cleanup;
    cg_run_nowait(cgroup, vfork_fn, core::ptr::null_mut());
