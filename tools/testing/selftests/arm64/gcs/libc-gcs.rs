...
//! Source: tools/testing/selftests/arm64/gcs/libc-gcs.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn gcs_recurse(depth: c_int) -> noinline void {
...
// Verify that if we start a new thread it has GCS enabled
    ...
    void *thread_ret;
    int ret;
    ret = pthread_create(&thread, core::ptr::null_mut(), gcs_test_thread, core::ptr::null_mut());
    ASSERT_TRUE(ret == 0);
    if (ret != 0)
    return;
    ...
    if (ret != 0)
    return;
    ASSERT_TRUE(thread_ret != core::ptr::null_mut());
    }
...
// Ideally we'd cross check with the child memory map
    errno = 0;
    val = ptrace(PTRACE_PEEKDATA, child, (void *)gcspr, core::ptr::null_mut());
    ret = errno;
    if (ret != 0)
...
// Write data via a peek
    ret = ptrace(PTRACE_POKEDATA, child, (void *)gcspr, core::ptr::null_mut());
    if (ret == -1)
    ksft_print_msg("PTRACE_POKEDATA failed: %s (%d)\n",
    strerror(errno), errno);
    EXPECT_EQ(ret, 0);
    EXPECT_EQ(0, ptrace(PTRACE_PEEKDATA, child, (void *)gcspr, core::ptr::null_mut()));
// Restore what we had before
    ...
    strerror(errno), errno);
    EXPECT_EQ(ret, 0);
    EXPECT_EQ(val, ptrace(PTRACE_PEEKDATA, child, (void *)gcspr, core::ptr::null_mut()));
// That's all, folks
    kill(child, SIGKILL);
    return;
    error:
