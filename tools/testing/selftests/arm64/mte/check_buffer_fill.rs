...
//! Source: tools/testing/selftests/arm64/mte/check_buffer_fill.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn check_buffer_by_byte(mem_type: c_int, mode: c_int) -> c_int {
...
// Check the buffer whether it is filled.
    ...
    bool err;
    char *und_ptr = core::ptr::null_mut();
    mte_switch_mode(mode, MTE_ALLOW_NON_ZERO_TAG, false);
    item = ARRAY_SIZE(sizes);
    for (i = 0; i < item; i++) {
...
// Underflow area should not be filled
    ...
    char *over_ptr = core::ptr::null_mut();
    mte_switch_mode(mode, MTE_ALLOW_NON_ZERO_TAG, false);
    item = ARRAY_SIZE(sizes);
    for (i = 0; i < item; i++) {
    ptr = (char *)mte_allocate_memory_tag_range(sizes[i], mem_type, 0,
...
#[no_mangle]
unsafe extern "C" fn check_buffer_by_block_iterate(mem_type: c_int, mode: c_int, size: usize) -> c_int {
    static int check_buffer_by_block_iterate(int mem_type, int mode, size_t size)
    {
    char *src, *dst;
    int j, result = KSFT_PASS;
    let mut alloc_type: enum mte_block_test_alloc = UNTAGGED_TAGGED;
...
#[no_mangle]
unsafe extern "C" fn check_buffer_by_block(mem_type: c_int, mode: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compare_memory_tags(ptr: *mut c_char, size: usize, tag: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn check_memory_initial_tags(mem_type: c_int, mode: c_int, mapping: c_int) -> c_int {
