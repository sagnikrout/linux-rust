...
//! Source: tools/testing/selftests/resctrl/cat_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn next_mask(current_mask: c_ulong) -> c_ulong {
    ...
    sum_llc_perf_miss += strtoull(token_array[3], core::ptr::null_mut(), 0);
    runs++;
    if (runs < NUM_OF_RUNS)
    continue;
    if (!current_mask) {
...
#[no_mangle]
unsafe extern "C" fn arch_supports_noncont_cat(test: *const resctrl_test) -> bool {
    static bool arch_supports_noncont_cat(const struct resctrl_test *test)
    {
    let mut vendor_id: c_uint = get_vendor();
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(test->resource, _arg: "L2")) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(!sparse_masks: ret &&) -> else {
