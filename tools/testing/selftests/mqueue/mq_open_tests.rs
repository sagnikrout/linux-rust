...
//! Source: tools/testing/selftests/mqueue/mq_open_tests.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn validate_current_settings() {
    void validate_current_settings()
    {
    int rlim_needed;
...
//
#[no_mangle]
pub unsafe extern "C" fn test_queue(attr: *mut mq_attr, result: *mut mq_attr) {
    static inline void test_queue(struct mq_attr *attr, struct mq_attr *result)
    {
    let mut flags: c_int = O_RDWR | O_EXCL | O_CREAT;
...
//
#[no_mangle]
pub unsafe extern "C" fn test_queue_fail(attr: *mut mq_attr, result: *mut mq_attr) -> c_int {
    static inline int test_queue_fail(struct mq_attr *attr, struct mq_attr *result)
    {
    let mut flags: c_int = O_RDWR | O_EXCL | O_CREAT;
    let mut perms: c_int = DEFFILEMODE;
...
// Tell the user our initial state
    ...
    "passed to mq_open:\n");
    if (!default_settings) {
    test_queue(core::ptr::null_mut(), &result);
...
// In case max was the same as the default
    ...
    if (test_queue_fail(core::ptr::null_mut(), &result)) {
    if (result.mq_maxmsg == cur_max_msgs &&
    result.mq_msgsize == cur_max_msgsize)
