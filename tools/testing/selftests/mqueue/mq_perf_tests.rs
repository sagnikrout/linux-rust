...
//! Source: tools/testing/selftests/mqueue/mq_perf_tests.c
#![no_std]
...
pub const MAX_CPUS: c_int = 64;
    ...
    int cpu_set_size;
    int cpus_online;
pub const MSG_SIZE: c_int = 16;
pub const TEST1_LOOPS: c_int = 10000000;
pub const TEST2_LOOPS: c_int = 100000;
    int continuous_mode;
...
#[no_mangle]
pub unsafe extern "C" fn __set(stream: *mut FILE, value: c_int, err_msg: *mut c_char) {
    static inline void __set(FILE *stream, int value, char *err_msg)
    {
    rewind(stream);
...
#[no_mangle]
pub unsafe extern "C" fn shutdown(exit_val: c_int, err_cause: *mut c_char, line_no: c_int) {
    void shutdown(int exit_val, char *err_cause, int line_no)
    {
    let mut in_shutdown: static int = 0;
    let mut errno_at_shutdown: c_int = errno;
    int i;
...
// Free the cpu_set allocated using CPU_ALLOC in main function
    ...
    pthread_join(cpu_threads[i], core::ptr::null_mut());
    }
    if (queue != -1)
    if (mq_close(queue))
...
#[no_mangle]
pub unsafe extern "C" fn sig_action_SIGUSR1(signum: c_int, info: *mut siginfo_t, context: *mut c_void) {
    void sig_action_SIGUSR1(int signum, siginfo_t *info, void *context)
    {
...
#[no_mangle]
pub unsafe extern "C" fn sig_action(signum: c_int, info: *mut siginfo_t, context: *mut c_void) {
    void sig_action(int signum, siginfo_t *info, void *context)
    {
    if (pthread_self() != main_thread)
...
#[no_mangle]
pub unsafe extern "C" fn get(stream: *mut FILE) -> c_int {
    static inline int get(FILE *stream)
    {
    int value;
    rewind(stream);
...
#[no_mangle]
pub unsafe extern "C" fn set(stream: *mut FILE, value: c_int) {
    static inline void set(FILE *stream, int value)
    {
    int new_value;
    rewind(stream);
...
#[no_mangle]
pub unsafe extern "C" fn try_set(stream: *mut FILE, value: c_int) -> c_int {
    static inline int try_set(FILE *stream, int value)
    {
    int new_value;
    rewind(stream);
    ...
    new_value = get(stream);
    let mut new_value: return = = value;
    }
#[no_mangle]
pub unsafe extern "C" fn getr(type: c_int, rlim: *mut rlimit) {
    static inline void getr(int type, struct rlimit *rlim)
    {
...
#[no_mangle]
pub unsafe extern "C" fn setr(type: c_int, rlim: *mut rlimit) {
    static inline void setr(int type, struct rlimit *rlim)
    {
...
//
#[no_mangle]
pub unsafe extern "C" fn open_queue(attr: *mut mq_attr) {
    static inline void open_queue(struct mq_attr *attr)
    {
    let mut flags: c_int = O_RDWR | O_EXCL | O_CREAT | O_NONBLOCK;
    let mut perms: c_int = DEFFILEMODE;
...
#[no_mangle]
pub unsafe extern "C" fn const_prio(prio: *mut c_int) {
    void const_prio(int *prio)
    {
    return;
    }
#[no_mangle]
pub unsafe extern "C" fn inc_prio(prio: *mut c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn dec_prio(prio: *mut c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn random_prio(prio: *mut c_int) {
...
//
    ...
    printf("\t\t\t\t\t\t%lld nsec/msg\n", nsec);
    for (cur_test = test2; cur_test.desc != core::ptr::null_mut(); cur_test++) {
    printf("%s:\n", cur_test.desc);
    printf("\t\t(%d iterations)\n", TEST2_LOOPS);
    prio_out = 0;
    send_total.tv_sec = 0;
...
#[no_mangle]
pub unsafe extern "C" fn increase_limits() {
    void increase_limits(void)
    {
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    if (cpu_set == core::ptr::null_mut()) {
    perror("CPU_ALLOC()");
    exit(1);
    }
    cpu_set_size = CPU_ALLOC_SIZE(cpus_online);
    CPU_ZERO_S(cpu_set_size, cpu_set);
    popt_context = poptGetContext(core::ptr::null_mut(), argc, (const char **)argv,
    options, 0);
...
// Tell the user our initial state
    ...
    if (sigaction(SIGUSR1, &sa, core::ptr::null_mut()) == -1)
    shutdown(1, "sigaction(SIGUSR1)", __LINE__);
    sa.sa_sigaction = sig_action;
    if (sigaction(SIGHUP, &sa, core::ptr::null_mut()) == -1)
    shutdown(1, "sigaction(SIGHUP)", __LINE__);
    if (sigaction(SIGINT, &sa, core::ptr::null_mut()) == -1)
    shutdown(1, "sigaction(SIGINT)", __LINE__);
    if (sigaction(SIGQUIT, &sa, core::ptr::null_mut()) == -1)
    shutdown(1, "sigaction(SIGQUIT)", __LINE__);
    if (sigaction(SIGTERM, &sa, core::ptr::null_mut()) == -1)
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: continuous_mode) -> else {
    else if (continuous_mode)
