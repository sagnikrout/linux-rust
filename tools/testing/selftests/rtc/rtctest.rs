...
//! Source: tools/testing/selftests/rtc/rtctest.c
#![no_std]
...
pub const NUM_UIE: c_int = 3;
pub const ALARM_DELTA: c_int = 3;
pub const READ_LOOP_DURATION_SEC: c_int = 30;
pub const READ_LOOP_SLEEP_MS: c_int = 11;
...
#[no_mangle]
unsafe extern "C" fn rtc_time_to_timestamp(rtc_time: *mut rtc_time) -> time_t {
...
#[no_mangle]
unsafe extern "C" fn nanosleep_with_retries(ns: c_long) {
...
#[no_mangle]
unsafe extern "C" fn get_rtc_alarm_state(fd: c_int, need_seconds: c_int) -> enum rtc_alarm_state {
    static enum rtc_alarm_state get_rtc_alarm_state(int fd, int need_seconds)
    {
    let mut param: rtc_param = { 0 };
    int rc;
...
// Turn on update interrupts
    ...
    for (i = 0; i < NUM_UIE; i++) {
    let mut tv: timeval = { .tv_sec = 2 };
    fd_set readfds;
    FD_ZERO(&readfds);
    FD_SET(self.fd, &readfds);
// The select will wait until an RTC interrupt happens.
    rc = select(self.fd + 1, &readfds, core::ptr::null_mut(), core::ptr::null_mut(), &tv);
    ASSERT_NE(-1, rc);
    ASSERT_NE(0, rc);
// This read won't block
    ...
    let mut tv: timeval = { .tv_sec = ALARM_DELTA + 2 };
    unsigned long data;
    struct rtc_time tm;
    fd_set readfds;
    time_t secs, new;
    int rc;
    let mut alarm_state: enum rtc_alarm_state = RTC_ALARM_UNKNOWN;
    if (self.fd == -1 && errno == ENOENT)
    SKIP(return, "Skipping test since %s does not exist", rtc_file);
    ASSERT_NE(-1, self.fd);
