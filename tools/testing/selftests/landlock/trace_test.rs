...
//! Source: tools/testing/selftests/landlock/trace_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn build_enforce_ruleset() -> c_int {
...
//
    ...
    ASSERT_NE(core::ptr::null_mut(), after_first);
...
//
#[no_mangle]
unsafe extern "C" fn child_enforce(nthreads: c_int, flags: __u32) -> c_int {
    ...
    int ruleset_fd, i;
    if (nthreads > 0) {
    if (pthread_barrier_init(&barrier, core::ptr::null_mut(), nthreads + 1))
    return 1;
    for (i = 0; i < nthreads; i++)
    if (pthread_create(&threads[i], core::ptr::null_mut(), enforce_idle,
    &barrier))
    return 1;
    pthread_barrier_wait(&barrier);
    }
...
//
    ...
    EXPECT_EQ(1,
    tracefs_count_matches(buf, REGEX_CREATE_DOMAIN(TRACE_TASK)));
    EXPECT_EQ(variant.total, count_enforce_matches(buf, core::ptr::null_mut(), -1, -1, -1))
    {
    TH_LOG("Expected %d enforce_domain events\n%s", variant.total,
    buf);
    }
    EXPECT_EQ(variant.complete,
    count_enforce_matches(buf, core::ptr::null_mut(), 1, -1, -1));
    EXPECT_EQ(variant.total - variant.complete,
    count_enforce_matches(buf, core::ptr::null_mut(), 0, -1, -1));
    EXPECT_EQ(variant.process_wide,
    count_enforce_matches(buf, core::ptr::null_mut(), -1, 1, -1));
    EXPECT_EQ(variant.total - variant.process_wide,
    count_enforce_matches(buf, core::ptr::null_mut(), -1, 0, -1));
...
//
    ...
    if (pid == 0) {
    pthread_t worker;
    if (pthread_create(&worker, core::ptr::null_mut(), enforce_nonleader, core::ptr::null_mut()))
    _exit(1);
// Leader leaves; the worker enforces as a non-leader.
    ...
    EXPECT_EQ(1,
    tracefs_count_matches(buf, REGEX_CREATE_DOMAIN(TRACE_TASK)));
    EXPECT_EQ(1, count_enforce_matches(buf, core::ptr::null_mut(), 1, 0, -1))
    {
...
//
    ...
    EXPECT_EQ(0,
    tracefs_count_matches(buf, REGEX_CREATE_DOMAIN(TRACE_TASK)));
    EXPECT_EQ(0, count_enforce_matches(buf, core::ptr::null_mut(), -1, -1, -1))
    {
...
#[no_mangle]
unsafe extern "C" fn enforce_nop_handler(sig: c_int) {
    static void enforce_nop_handler(int sig)
    {
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct abort_signaler_data {
...
//
#[no_mangle]
unsafe extern "C" fn child_abort(nsiblings: c_int, attempts: c_int) -> c_int {
    ...
    return 1;
    if (pthread_barrier_init(&barrier, core::ptr::null_mut(), nsiblings + 1))
    return 1;
    for (i = 0; i < nsiblings; i++)
    if (pthread_create(&threads[i], core::ptr::null_mut(), enforce_idle, &barrier))
    return 1;
    pthread_barrier_wait(&barrier);
    data.target = pthread_self();
    if (pthread_create(&signaler, core::ptr::null_mut(), abort_signaler, &data))
    return 1;
    prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0);
    for (i = 0; i < attempts; i++) {
    let mut ruleset_fd: c_int = build_enforce_ruleset();
    if (ruleset_fd < 0)
    break;
//
...
//
    ...
    data.stop = true;
    pthread_join(signaler, core::ptr::null_mut());
    return 0;
    }
//
...
//
    ...
    const char *cursor;
    char domain[64];
    let mut abort_found: bool = false;
    ASSERT_EQ(0, tracefs_clear_buf());
