...
//! Source: tools/testing/selftests/landlock/base_test.c
#![no_std]
...
pub const O_PATH: c_int = 010000000;
    ...
    let mut buf: *mut char const = malloc(page_size + 1);
    let mut ruleset_attr: *mut landlock_ruleset_attr const = (void *)buf;
...
// The size if less than sizeof(struct landlock_attr_enforce).
    ...
    ASSERT_EQ(EINVAL, errno);
    ASSERT_EQ(-1, landlock_create_ruleset(core::ptr::null_mut(), 1, 0));
// The size if less than sizeof(struct landlock_attr_enforce).
    ASSERT_EQ(EFAULT, errno);
    ASSERT_EQ(-1, landlock_create_ruleset(
    core::ptr::null_mut(), sizeof(struct landlock_ruleset_attr), 0));
    ASSERT_EQ(EFAULT, errno);
...
// Checks non-zero value.
    ...
    ASSERT_EQ(11, landlock_create_ruleset(core::ptr::null_mut(), 0,
    LANDLOCK_CREATE_RULESET_VERSION));
    ASSERT_EQ(-1, landlock_create_ruleset(&ruleset_attr, 0,
    LANDLOCK_CREATE_RULESET_VERSION));
    ASSERT_EQ(EINVAL, errno);
    ASSERT_EQ(-1, landlock_create_ruleset(core::ptr::null_mut(), sizeof(ruleset_attr),
    LANDLOCK_CREATE_RULESET_VERSION));
    ASSERT_EQ(EINVAL, errno);
    ASSERT_EQ(-1,
    ...
    LANDLOCK_CREATE_RULESET_VERSION));
    ASSERT_EQ(EINVAL, errno);
    ASSERT_EQ(-1, landlock_create_ruleset(core::ptr::null_mut(), 0,
    LANDLOCK_CREATE_RULESET_VERSION |
    1 << 31));
    ASSERT_EQ(EINVAL, errno);
    }
...
//
    ...
    int errata;
    errata = landlock_create_ruleset(core::ptr::null_mut(), 0,
...
// Tests ordering of syscall argument checks.
    ...
    {
    let mut last_flag: c_int = LANDLOCK_CREATE_RULESET_ERRATA;
    let mut invalid_flag: c_int = last_flag << 1;
    int ruleset_fd;
    const struct landlock_ruleset_attr ruleset_attr = {
    .handled_access_fs = LANDLOCK_ACCESS_FS_READ_FILE,
    };
// Checks priority for invalid flags.
    ASSERT_EQ(-1, landlock_create_ruleset(core::ptr::null_mut(), 0, invalid_flag));
    ASSERT_EQ(EINVAL, errno);
...
// Checks invalid flags.
    ASSERT_EQ(-1, landlock_add_rule(-1, 0, core::ptr::null_mut(), 100));
    ASSERT_EQ(EINVAL, errno);
// Checks invalid ruleset FD.
    ASSERT_EQ(-1, landlock_add_rule(-1, 0, core::ptr::null_mut(), 0));
    ASSERT_EQ(EBADF, errno);
// Checks invalid rule type.
    ASSERT_EQ(-1, landlock_add_rule(ruleset_fd, 0, core::ptr::null_mut(), 0));
    ASSERT_EQ(EINVAL, errno);
...
// LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS requires a ruleset FD.
    ...
    {
    let mut last_flag: __u32 = LANDLOCK_RESTRICT_SELF_NO_NEW_PRIVS;
...
// Sends the ruleset FD over a socketpair and then close it.
    ...
    ASSERT_LE(0, child);
    if (child == 0) {
    let mut ruleset_fd_rx: c_int = recv_fd(socket_fds[1]);
...
// Needed for KEYCTL_SESSION_TO_PARENT permission checks
    EXPECT_NE(-1, syscall(__NR_keyctl, KEYCTL_JOIN_SESSION_KEYRING, core::ptr::null_mut(), 0,
    0, 0))
    {
