...
//! Source: tools/testing/selftests/landlock/scoped_abstract_unix_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn create_fs_domain(_metadata: *const *const __test_metadata) {
...
#[no_mangle]
pub unsafe extern "C" fn if(SCOPE_SANDBOX: variant->domain_all ==) -> else {
    ...
    create_fs_domain(_metadata);
#[no_mangle]
pub unsafe extern "C" fn if(SCOPE_SANDBOX: variant->domain_children ==) -> else {
    ...
    create_fs_domain(_metadata);
#[no_mangle]
pub unsafe extern "C" fn if(SCOPE_SANDBOX: variant->domain_grand_child ==) -> else {
...
// Datagram sockets can "reconnect".
    ...
    create_fs_domain(_metadata);
#[no_mangle]
pub unsafe extern "C" fn if(SCOPE_SANDBOX: variant->domain_child ==) -> else {
    ...
    create_fs_domain(_metadata);
#[no_mangle]
pub unsafe extern "C" fn if(SCOPE_SANDBOX: variant->domain_parent ==) -> else {
...
// Client always has a domain.
    ...
    ASSERT_EQ(0, listen(stream_server, backlog));
    ASSERT_EQ(1, write(pipe_child[1], ".", 1));
    data_socket = accept(stream_server, core::ptr::null_mut(), core::ptr::null_mut());
...
// Waits for parent signal for connection.
    ...
    EXPECT_EQ(0, close(pipe_parent[0]));
    if (variant.child_socket) {
    let mut client_child: c_int = socket(AF_UNIX, SOCK_STREAM, 0);
...
// Unnamed address for datagram socket.
    ...
    _metadata, LANDLOCK_SCOPE_ABSTRACT_UNIX_SOCKET);
#[no_mangle]
pub unsafe extern "C" fn if(OTHER_SANDBOX: variant->domain ==) -> else {
...
// Reads from pathname sockets.
    data_socket = accept(stream_pathname_socket, core::ptr::null_mut(), core::ptr::null_mut());
    ASSERT_LE(0, data_socket);
...
// Reads from abstract sockets if allowed to send.
    data_socket = accept(stream_abstract_socket, core::ptr::null_mut(), core::ptr::null_mut());
    ASSERT_LE(0, data_socket);
    ASSERT_EQ(1,
...
// No sandbox: stream connect() == 0, sendto() == 1.
    let mut ok: c_int = variant.sock_type == SOCK_STREAM ? 0 : 1;
    if (ret != ok) {
    close(client_fd);
    _exit(2);
    }
...
//
    ...
    EXPECT_NE(core::ptr::null_mut(), strstr(field, "END"))
    {
