...
//! Source: tools/testing/selftests/landlock/fs_test.c
#![no_std]
...
// Macro flag: #define _ASM_GENERIC_FCNTL_H
...
#[no_mangle]
pub unsafe extern "C" fn open_tree(dfd: c_int, filename: *const c_char, flags: c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn fgrep(inf: *const *const FILE, str: *const *const c_char) -> bool {
    static bool fgrep(FILE *const inf, const char *const str)
    {
    char line[32];
    let mut slen: c_int = strlen(str);
    while (!feof(inf)) {
...
#[no_mangle]
unsafe extern "C" fn supports_filesystem(filesystem: *const *const c_char) -> bool {
    static bool supports_filesystem(const char *const filesystem)
    {
    char str[32];
    int len;
    let mut res: bool = true;
    let mut inf: *mut FILE const = fopen("/proc/filesystems", "r");
//
...
#[no_mangle]
unsafe extern "C" fn cwd_matches_fs(fs_magic: c_uint) -> bool {
    ...
    ASSERT_NE(path[0], '\0');
    walker = strdup(path);
    ASSERT_NE(core::ptr::null_mut(), walker);
    parent = walker;
    for (i = 1; walker[i]; i++) {
    if (walker[i] != '/')
    continue;
...
#[no_mangle]
unsafe extern "C" fn remove_path(path: *const *const c_char) -> c_int {
...
#[derive(Copy, Clone)]
pub struct mnt_opt {
    pub source: *const *const c_char,
    pub type: *const *const c_char,
    pub flags: c_ulong,
...
#[no_mangle]
unsafe extern "C" fn mount_opt(mnt: *const *const mnt_opt, target: *const *const c_char) -> c_int {
...
//
    ...
    ASSERT_EQ(0, mount(core::ptr::null_mut(), TMP_DIR, core::ptr::null_mut(), MS_PRIVATE | MS_REC, core::ptr::null_mut()));
    clear_cap(_metadata, CAP_SYS_ADMIN);
    }
#[no_mangle]
unsafe extern "C" fn prepare_layout(_metadata: *const *const __test_metadata) {
    static void prepare_layout(struct __test_metadata *const _metadata)
    {
    prepare_layout_opt(_metadata, &mnt_tmp);
    }
#[no_mangle]
unsafe extern "C" fn cleanup_layout(_metadata: *const *const __test_metadata) {
...
#[no_mangle]
unsafe extern "C" fn create_layout1(_metadata: *const *const __test_metadata) {
...
#[no_mangle]
unsafe extern "C" fn remove_layout1(_metadata: *const *const __test_metadata) {
...
#[no_mangle]
unsafe extern "C" fn test_open(path: *const *const c_char, flags: c_int) -> c_int {
...
// clang-format on
    ...
    {
    let mut ruleset_fd: c_int = create_ruleset(_metadata, access_fs, rules);
...
// Checks read access is now denied with layer 7.
    ...
    let mut ruleset_fd: c_int = create_ruleset(_metadata, ACCESS_RW, rules);
    enforce_ruleset(_metadata, ruleset_fd);
    ASSERT_EQ(EACCES, test_open(file1_s1d1, O_WRONLY));
    ASSERT_EQ(EACCES, test_open(dir_s1d1, O_RDONLY | O_DIRECTORY));
...
// Enforces policy which denies read access to all files.
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_READ_FILE, core::ptr::null_mut());
    ASSERT_EQ(EACCES, test_open(file1_s1d1, O_RDONLY));
    ASSERT_EQ(0, test_open(dir_s1d1, O_RDONLY));
// Nests a policy which denies read access to all directories.
    ruleset_fd =
    create_ruleset(_metadata, LANDLOCK_ACCESS_FS_READ_DIR, core::ptr::null_mut());
    enforce_ruleset(_metadata, ruleset_fd);
    ASSERT_EQ(EACCES, test_open(file1_s1d1, O_RDONLY));
    ASSERT_EQ(EACCES, test_open(dir_s1d1, O_RDONLY));
...
// Checks denied access (on a directory).
    ...
    enforce_fs(_metadata, ACCESS_RW, rules);
    set_cap(_metadata, CAP_SYS_ADMIN);
    ASSERT_EQ(-1, mount(core::ptr::null_mut(), dir_s3d2, core::ptr::null_mut(), MS_RDONLY, core::ptr::null_mut()));
    ASSERT_EQ(EPERM, errno);
...
// Mount, remount, move_mount, umount, and pivot_root checks.
    ...
    ASSERT_EQ(0, mount(core::ptr::null_mut(), dir_s1d2, core::ptr::null_mut(), MS_PRIVATE | MS_REC, core::ptr::null_mut()));
    ASSERT_EQ(0, syscall(__NR_move_mount, AT_FDCWD, dir_s1d2, AT_FDCWD,
    dir_s2d2, 0));
...
// Mount, remount, move_mount, umount, and pivot_root checks.
    ...
    ASSERT_EQ(EPERM, errno);
    ASSERT_EQ(-1, mount(core::ptr::null_mut(), dir_s3d2, core::ptr::null_mut(), MS_PRIVATE | MS_REC, core::ptr::null_mut()));
    ASSERT_EQ(EPERM, errno);
    ASSERT_EQ(-1, syscall(__NR_move_mount, AT_FDCWD, dir_s3d2, AT_FDCWD,
    dir_s2d2, 0));
    ASSERT_EQ(EPERM, errno);
...
// Checks the root dir_s1d2.
    ...
    {
    int status;
    char *const argv[] = { (char *)path, core::ptr::null_mut() };
    let mut child: pid_t = fork();
    ASSERT_LE(0, child);
    if (child == 0) {
    ASSERT_EQ(err ? -1 : 0, execve(path, argv, core::ptr::null_mut()))
    {
    ...
    int ret;
    char *const argv[] = { (char *)path, core::ptr::null_mut() };
    ret = sys_execveat(AT_FDCWD, path, argv, core::ptr::null_mut(),
...
// Checks before file1_s1d1 being denied.
    ...
    pipe_parent_str, core::ptr::null_mut() };
...
#[no_mangle]
unsafe extern "C" fn test_rename(oldpath: *const *const c_char, newpath: *const *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_exchange(oldpath: *const *const c_char, newpath: *const *const c_char) -> c_int {
...
// Deny MAKE_REG, but allow MAKE_FIFO.
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_MAKE_REG, core::ptr::null_mut());
//
...
#[no_mangle]
unsafe extern "C" fn is_whiteout(path: *const *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_fifo(path: *const *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_missing(path: *const *const c_char) -> bool {
...
// A whiteout object took the place of the moved FIFO.
    ...
    {
    let mut whiteout_s3d3: *const char const = TMP_DIR "/s3d1/s3d2/s3d3/f2";
    const struct rule rules[] = {
    {
    .path = dir_s3d2,
    .access = LANDLOCK_ACCESS_FS_REFER,
...
// Creates a mount object from a non-mount point.
    ...
    clear_cap(_metadata, CAP_SYS_ADMIN);
    ASSERT_LE(0, root_fd);
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_MAKE_DIR, core::ptr::null_mut());
...
// See layout1.reparent_exdev_layers_exchange2 for complement.
    ...
    {
    let mut dir_file2_s2d3: *const char const = file2_s2d3;
    ASSERT_EQ(0, unlink(file2_s2d3));
    ASSERT_EQ(0, mkdir(file2_s2d3, 0700));
    reparent_exdev_layers_enforce1(_metadata);
    reparent_exdev_layers_enforce2(_metadata);
...
#[no_mangle]
unsafe extern "C" fn test_truncate(path: *const *const c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn test_creat(path: *const *const c_char) -> c_int {
    static int test_creat(const char *const path)
    {
    let mut fd: c_int = creat(path, 0600);
    if (fd < 0)
    return errno;
//
...
//
    ...
    {
    let mut file_r: *const char const = file1_s1d1;
    let mut file_w: *const char const = file2_s1d1;
    let mut file_none: *const char const = file1_s1d2;
    const struct rule rules[] = {
    {
    .path = file_r,
    .access = LANDLOCK_ACCESS_FS_READ_FILE,
...
//
    ...
    {
    let mut file_rwt: *const char const = file1_s1d1;
    let mut file_rw: *const char const = file2_s1d1;
    let mut file_rt: *const char const = file1_s1d2;
    let mut file_t: *const char const = file2_s1d2;
    let mut file_none: *const char const = file1_s1d3;
    let mut dir_t: *const char const = dir_s2d1;
    let mut file_in_dir_t: *const char const = file1_s2d1;
    let mut dir_w: *const char const = dir_s3d1;
    let mut file_in_dir_w: *const char const = file1_s3d1;
    const struct rule rules[] = {
    {
    .path = file_rwt,
    .access = LANDLOCK_ACCESS_FS_READ_FILE |
...
#[no_mangle]
unsafe extern "C" fn test_ftruncate(fd: c_int) -> c_int {
...
// called.
//
    let mut path: *const char const = file1_s1d1;
    ...
    let mut handled2: __u64 = LANDLOCK_ACCESS_FS_TRUNCATE;
    const struct rule layer2[] = {
    {
    .path = path,
    .access = LANDLOCK_ACCESS_FS_TRUNCATE,
...
#[no_mangle]
unsafe extern "C" fn test_fs_ioc_getflags_ioctl(fd: c_int) -> c_int {
...
//
    ...
    ASSERT_EQ(0, close(fd));
// Enables Landlock.
    enforce_fs(_metadata, ACCESS_ALL, core::ptr::null_mut());
    }
    }
#[no_mangle]
unsafe extern "C" fn test_fionread_ioctl(fd: c_int) -> c_int {
...
//
    ...
    int fd;
// Enables Landlock.
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_IOCTL_DEV, core::ptr::null_mut());
...
// FIONREAD is implemented by pipefifo_fops.
    ...
    ASSERT_EQ(0, close(fd));
    ASSERT_EQ(0, unlink(path));
    ASSERT_EQ(child_pid, waitpid(child_pid, core::ptr::null_mut(), 0));
    }
//
...
//
    ...
    {
    let mut path: *const char const = "sock";
    const struct rule rules[] = {
    {
    .path = ".",
    ...
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_RESOLVE_UNIX, core::ptr::null_mut());
#[no_mangle]
pub unsafe extern "C" fn if(ENFORCE_ALL: flags &) -> else {
...
// Wait for child.
    ...
    {
    int fd;
    let mut len: usize = strlen(pattern);
//
...
//
    ...
    let mut sock_path: *const char const = core_pattern + 1;
    int srv_fd, conn_fd, status;
    pid_t child_pid;
    struct ucred cred;
    let mut cred_len: socklen_t = sizeof(cred);
    char buf[4096];
...
// would time out.
//
    conn_fd = accept(srv_fd, core::ptr::null_mut(), core::ptr::null_mut());
    ASSERT_LE(0, conn_fd);
...
// Move targets for disconnected path tests.
    ...
    create_layout1(_metadata);
    set_cap(_metadata, CAP_SYS_ADMIN);
    ASSERT_EQ(0, mount(dir_s1d2, dir_s2d2, core::ptr::null_mut(), MS_BIND, core::ptr::null_mut()));
    clear_cap(_metadata, CAP_SYS_ADMIN);
    }
...
// clang-format on
    ...
    set_cap(_metadata, CAP_SYS_ADMIN);
    ASSERT_EQ(0, mount(TMP_DIR "/s1d1/s1d2", TMP_DIR "/s2d1/s2d2", core::ptr::null_mut(),
    MS_BIND, core::ptr::null_mut()));
    clear_cap(_metadata, CAP_SYS_ADMIN);
    }
...
// clang-format on
    ...
    core::ptr::null_mut(), MS_BIND, core::ptr::null_mut()));
    ASSERT_EQ(0, mount(TMP_DIR "/s2d1/s2d2", TMP_DIR "/s3d1/s3d2", core::ptr::null_mut(),
    MS_BIND | MS_REC, core::ptr::null_mut()));
    clear_cap(_metadata, CAP_SYS_ADMIN);
    }
...
// merge_pl1 is a FIFO and is deliberately not in the lists below.
    ...
    &merge_do1_fl3, &merge_do1_fu3, core::ptr::null_mut(),
    };
//
...
// Checks all merge accesses.
    ...
    {
    let mut merge_pl1_renamed: *const char const = MERGE_DATA "/pl1_renamed";
    if (self.skip_test)
    SKIP(return, "overlayfs is not supported (test)");
//
...
//
    ...
    let mut dev_null_path: *const char const = "/dev/null";
    if (self.skip_test)
...
// Checks that access to the new mount point is denied.
    ...
    char *absolute_path = core::ptr::null_mut();
...
//
    absolute_path = realpath(path, core::ptr::null_mut());
    if (!absolute_path)
    return -errno;
    ...
    {
    return matches_log_fs_extra(_metadata, audit_fd, blockers, path, core::ptr::null_mut());
    }
    ...
    remove_layout1(_metadata);
    cleanup_layout(_metadata);
    EXPECT_EQ(0, audit_cleanup(-1, core::ptr::null_mut()));
    }
    ...
    test_execute(_metadata, 0, file1_s1d1);
    test_check_exec(_metadata, 0, file1_s1d1);
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_EXECUTE, core::ptr::null_mut());
    test_execute(_metadata, EACCES, file1_s1d1);
...
// Whiteout creation is denied and logged as fs.make_reg.
    ...
    struct audit_records records;
    EXPECT_EQ(0, unlink(file1_s1d3));
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_REFER, core::ptr::null_mut());
    EXPECT_EQ(-1, link(file1_s1d1, file1_s1d3));
    EXPECT_EQ(EXDEV, errno);
    EXPECT_EQ(0, matches_log_fs(_metadata, self.audit_fd, "fs\\.refer",
    dir_s1d1));
    EXPECT_EQ(0,
    matches_log_domain_allocated(self.audit_fd, getpid(), core::ptr::null_mut()));
    EXPECT_EQ(0, matches_log_fs(_metadata, self.audit_fd, "fs\\.refer",
    dir_s1d3));
    EXPECT_EQ(0, audit_count_records(self.audit_fd, &records));
    EXPECT_EQ(0, records.access);
...
// Only half of the request is denied.
    ...
    struct audit_records records;
    int fd;
    enforce_fs(_metadata, ACCESS_ALL & ~LANDLOCK_ACCESS_FS_READ_FILE, core::ptr::null_mut());
    ...
    EXPECT_EQ(EXIT_SUCCESS, WEXITSTATUS(status));
    EXPECT_EQ(0, matches_log_fs_extra(_metadata, self.audit_fd,
    "fs\\.resolve_unix", path, core::ptr::null_mut()));
    EXPECT_EQ(0, audit_count_records(self.audit_fd, &records));
    EXPECT_EQ(0, records.access);
    EXPECT_EQ(1, records.domain);
    EXPECT_EQ(0, close(srv_fd));
    ...
    enforce_fs(_metadata, LANDLOCK_ACCESS_FS_EXECUTE, core::ptr::null_mut());
    set_cap(_metadata, CAP_SYS_ADMIN);
    EXPECT_EQ(-1, mount(core::ptr::null_mut(), dir_s3d2, core::ptr::null_mut(), MS_RDONLY, core::ptr::null_mut()));
...
#[no_mangle]
unsafe extern "C" fn sprint_access_bits(buf: *mut c_char, buflen: usize, access: __u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: target->audit_read_blocked) -> else {
    else if (target.audit_read_blocked)
    blocker = "fs\\.read_file";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: target->audit_write_blocked) -> else {
...
//
    ...
    int i;
    FIXTURE_VARIANT(audit_quiet_layout1) variant_2 = {};
    let mut test_has_subdomains_off: bool = false;
    for (i = 0; i < ARRAY_SIZE(variant.layers); i++) {
...
// Skip landlock_add_rule for useless rules.
    ...
    layer.rules[0].path = core::ptr::null_mut();
    if (!dest_allow && !dest_quiet)
    layer.rules[1].path = core::ptr::null_mut();
...
//
    ...
    false, core::ptr::null_mut(), core::ptr::null_mut());
    }
    ...
    LANDLOCK_ACCESS_FS_REFER;
    simple_quiet_rename(_metadata, self, access, access, false, false,
    false, true, "fs\\.remove_file,fs\\.refer", core::ptr::null_mut());
    }
    ...
    LANDLOCK_ACCESS_FS_REFER;
    simple_quiet_rename(_metadata, self, access, access, false, false, true,
    false, core::ptr::null_mut(), "fs\\.make_reg,fs\\.refer");
    }
...
// Isolate tracefs state (PID filter, event enables).
    ...
    ASSERT_EQ(0, mount(core::ptr::null_mut(), "/", core::ptr::null_mut(), MS_REC | MS_PRIVATE, core::ptr::null_mut()));
    clear_cap(_metadata, CAP_SYS_ADMIN);
    prepare_layout(_metadata);
    create_layout1(_metadata);
    set_cap(_metadata, CAP_DAC_OVERRIDE);
