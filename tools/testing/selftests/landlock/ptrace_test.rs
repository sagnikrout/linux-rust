...
//! Source: tools/testing/selftests/landlock/ptrace_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn create_domain(_metadata: *const *const __test_metadata) {
...
#[no_mangle]
unsafe extern "C" fn test_ptrace_read(pid: pid_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_yama_ptrace_scope() -> c_int {
    static int get_yama_ptrace_scope(void)
    {
    int ret;
    char buf[2] = {};
    let mut fd: c_int = open("/proc/sys/kernel/yama/ptrace_scope", O_RDONLY);
    if (fd < 0)
    return 0;
    if (read(fd, buf, 1) < 0) {
    close(fd);
...
// Tests PTRACE_ATTACH on the parent.
    ret = ptrace(PTRACE_ATTACH, parent, core::ptr::null_mut(), 0);
    if (can_trace_parent) {
    EXPECT_EQ(0, ret);
    } else {
    EXPECT_EQ(-1, ret);
    ...
    ASSERT_EQ(0, ptrace(PTRACE_DETACH, parent, core::ptr::null_mut(), 0));
    }
...
// Tests child PTRACE_TRACEME.
    ...
    ASSERT_EQ(1, WIFSTOPPED(status));
    ASSERT_EQ(0, ptrace(PTRACE_DETACH, child, core::ptr::null_mut(), 0));
    } else {
// The child should not be traced by the parent.
    EXPECT_EQ(-1, ptrace(PTRACE_DETACH, child, core::ptr::null_mut(), 0));
...
// Tests PTRACE_ATTACH on the child.
    ret = ptrace(PTRACE_ATTACH, child, core::ptr::null_mut(), 0);
    if (can_trace_child) {
    EXPECT_EQ(0, ret);
    } else {
    EXPECT_EQ(-1, ret);
...
// The child should not be traced by the parent.
    ...
    EXPECT_EQ(ESRCH, errno);
// Tests PTRACE_ATTACH on the child.
    EXPECT_EQ(-1, ptrace(PTRACE_ATTACH, child, core::ptr::null_mut(), 0));
    EXPECT_EQ(EPERM, errno);
...
// PTRACE_ATTACH on unsandboxed parent: denied.
    ...
    ptrace(PTRACE_DETACH, parent, core::ptr::null_mut(), core::ptr::null_mut());
    _exit(2);
    }
    if (errno != EPERM)
    _exit(3);
