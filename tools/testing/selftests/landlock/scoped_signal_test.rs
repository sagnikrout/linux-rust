...
//! Source: tools/testing/selftests/landlock/scoped_signal_test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn scope_signal_handler(sig: c_int, info: *mut siginfo_t, ucontext: *mut c_void) {
...
// clang-format on
    ...
    {
    let mut pipe_read: c_int = *(int *)arg;
    char buf;
    ...
    ASSERT_EQ(0, pthread_create(&no_sandbox_thread, core::ptr::null_mut(), thread_sync,
    &thread_pipe[0]));
...
// Enforces restriction before creating the thread.
    create_scoped_domain(_metadata, LANDLOCK_SCOPE_SIGNAL);
    ASSERT_EQ(0, pthread_create(&scoped_thread, core::ptr::null_mut(), thread_sync,
    &thread_pipe[0]));
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thread_setuid_args {
    pub new_uid: int pipe_read,,
}
...
// Capabilities must be set before creating a new thread.
    ...
    ASSERT_EQ(0, pthread_create(&no_sandbox_thread, core::ptr::null_mut(), thread_setuid,
    &arg));
// Enforces restriction after creating the thread.
    create_scoped_domain(_metadata, LANDLOCK_SCOPE_SIGNAL);
...
#[no_mangle]
unsafe extern "C" fn setup_signal_handler(signal: c_int) -> c_int {
    ...
    return sigaction(SIGURG, &sa, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn has_af_unix_oob() -> bool {
...
//
    ASSERT_EQ(0, pthread_create(&thread, core::ptr::null_mut(), thread_setown_scoped,
    &trigger[0]));
