...
//! Source: tools/testing/selftests/landlock/net_test.c
#![no_std]
...
// Macro flag: #define _GNU_SOURCE
    ...
    let mut sock_port_start: c_short = (1 << 10);
    static const char loopback_ipv4[] = "127.0.0.1";
    static const char loopback_ipv6[] = "::1";
// Number pending connections queue to be hold.
    let mut backlog: c_short = 10;
    enum sandbox_type {
    NO_SANDBOX,
...
#[no_mangle]
unsafe extern "C" fn setup_loopback(_metadata: *const *const __test_metadata) {
...
#[no_mangle]
unsafe extern "C" fn prot_is_tcp(prot: *const *const protocol_variant) -> bool {
...
#[no_mangle]
unsafe extern "C" fn prot_is_udp(prot: *const *const protocol_variant) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(UDP_SANDBOX: sandbox ==) -> else {
    else if (sandbox == UDP_SANDBOX)
    return prot_is_udp(prot);
    return false;
    }
#[no_mangle]
unsafe extern "C" fn socket_variant(srv: *const *const service_fixture) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_port(srv: *const *const service_fixture, port: u16) {
...
// errno is not set in cases of partial writes.
    ...
    int sock_type;
    int addr_family;
    let mut peer_addr: sockaddr_storage = { 0 };
...
// With or without explicit destination address (srv can be NULL).
    ...
    } else if ((send_denied && srv != core::ptr::null_mut()) ||
    (bind_denied && needs_autobind)) {
    ASSERT_EQ(-EACCES, ret)
    {
    return -1;
    }
    } else if (srv == core::ptr::null_mut() && !has_remote_port) {
    ...
    {
    let mut prot_unspec: protocol_variant = variant.prot;
...
// Accepts connection from the child.
    ...
    if (srv.protocol.type == SOCK_STREAM) {
    client_fd = accept(bind_fd, core::ptr::null_mut(), 0);
...
// Fast Open with no address.
    ret = sendto_variant(connect_fd, core::ptr::null_mut(), core::ptr::null_mut(), 0, MSG_FASTOPEN);
    if (self.srv0.protocol.domain == AF_UNIX) {
...
// Send on a non-connected socket.
    res = sendto_variant(client_fd, core::ptr::null_mut(), "A", 1, 0);
...
// Connect.
    ...
    tmp_fd = accept(srv0_fd, core::ptr::null_mut(), 0);
    ASSERT_LE(0, tmp_fd);
    EXPECT_EQ(0, close(srv0_fd));
    srv0_fd = tmp_fd;
// Send without an explicit address.
    EXPECT_EQ(0, sendto_variant(client_fd, core::ptr::null_mut(), "C", 1, 0));
...
// Send to a valid but different address.
    ...
    {
    let mut restricted: bool = is_restricted(&variant.prot, variant.sandbox);
...
// Deny all connect()/send(explicit addr)/bind().
    ...
    srv0_fd, core::ptr::null_mut(), restricted, restricted));
...
//
    ...
    ASSERT_LE(0, client_fd = socket_variant(&self.srv0));
    EXPECT_EQ(0, test_sendmsg(_metadata, &variant.prot, client_fd,
    -1, core::ptr::null_mut(), false, false));
    EXPECT_EQ(0, test_sendmsg(_metadata, &variant.prot, client_fd,
...
// Restrict bind() to explicit calls with an arbitrary (non-0) port.
    ...
    ASSERT_LE(0, child);
    if (child == 0) {
    let mut allowed_src_port: u16 = 42424;
...
// Host port format.
    ...
    let mut little_endian: c_char = *(const char *)&one;
    int ruleset_fd;
    ruleset_fd =
    landlock_create_ruleset(&ruleset_attr, sizeof(ruleset_attr), 0);
    ASSERT_LE(0, ruleset_fd);
...
// Bind to srv2 (with quiet rule): no new audit logs.
    ...
    LANDLOCK_ACCESS_NET_CONNECT_TCP :
    LANDLOCK_ACCESS_NET_CONNECT_SEND_UDP);
    let mut access_rights: __u64 = bind_right | conn_right;
    const struct landlock_ruleset_attr ruleset_attr = {
    .handled_access_net = access_rights,
    .quiet_access_net = access_rights,
    };
...
// Quieting bind access has no effect on connect.
    ...
    LANDLOCK_ACCESS_NET_CONNECT_TCP :
    LANDLOCK_ACCESS_NET_CONNECT_SEND_UDP);
    let mut access_rights: c_int = bind_right | conn_right;
    const struct landlock_ruleset_attr ruleset_attr = {
    .handled_access_net = access_rights,
...
// Check that autobind generates a denied bind event.
    ...
    EXPECT_EQ(0, matches_auditlog(self.audit_fd, "net\\.bind_udp", core::ptr::null_mut(),
    ...
    EXPECT_EQ(0, matches_auditlog(self.audit_fd, "net\\.connect_send_udp",
    "daddr", core::ptr::null_mut(), "dest", 0));
    EXPECT_EQ(0, audit_count_records(self.audit_fd, &records));
    EXPECT_EQ(0, records.access);
    EXPECT_EQ(0, records.domain);
    EXPECT_EQ(0, close(sock_fd));
