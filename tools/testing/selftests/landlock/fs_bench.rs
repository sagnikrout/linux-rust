...
//! Source: tools/testing/selftests/landlock/fs_bench.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn usage(argv0: *const *const c_char) {
...
//
#[no_mangle]
unsafe extern "C" fn build_directory(depth: usize, use_landlock: bool) -> c_int {
    ...
    if (use_landlock) {
    abi = landlock_create_ruleset(core::ptr::null_mut(), 0,
...
#[no_mangle]
unsafe extern "C" fn remove_recursively(depth: usize) {
    static void remove_recursively(const size_t depth)
    {
    const char *path = "d"; /* directory name */
    let mut fd: c_int = openat(AT_FDCWD, ".", O_PATH);
    if (fd < 0)
    err(1, "openat(.)");
    for (size_t i = 0; i < depth - 1; i++) {
    let mut oldfd: c_int = fd;
    ...
    let mut newfd: c_int = openat(fd, "..", O_PATH);
    close(fd);
    fd = newfd;
    }
    close(fd);
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    {
    let mut use_landlock: bool = true;
    let mut num_iterations: usize = 100000;
    let mut num_subdirs: usize = 10000;
    int c, curr, fd;
