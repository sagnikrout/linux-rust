...
//! Source: tools/testing/selftests/riscv/hwprobe/which-cpus.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn print_cpulist(cpus: *mut cpu_set_t) {
    static void print_cpulist(cpu_set_t *cpus)
    {
    let mut start: c_int = 0, end = 0;
    if (!CPU_COUNT(cpus)) {
    printf("cpus: None\n");
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn do_which_cpus(argc: c_int, argv: *mut c_char, cpus: *mut cpu_set_t) {
    static void do_which_cpus(int argc, char **argv, cpu_set_t *cpus)
    {
    struct riscv_hwprobe *pairs;
    let mut nr_pairs: c_int = argc - 1;
    char *start, *end;
    int rc;
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    rc = riscv_hwprobe(pairs, 1, 0, core::ptr::null_mut(), 0);
    ...
    pairs[0] = (struct riscv_hwprobe){ .key = RISCV_HWPROBE_KEY_BASE_BEHAVIOR, .value = RISCV_HWPROBE_BASE_BEHAVIOR_IMA, };
    rc = riscv_hwprobe(pairs, 1, sizeof(cpu_set_t), core::ptr::null_mut(), RISCV_HWPROBE_WHICH_CPUS);
    ksft_test_result(rc == -EINVAL, "core::ptr::null_mut() cpus\n");
