...
//! Source: tools/testing/selftests/riscv/vector/validate_v_ptrace.c
#![no_std]
...
pub const SR_FS_DIRTY: c_uint = 0x00006000UL;
pub const CSR_VXRM_SHIFT: c_int = 1;
    ...
    chld_lock = 1;
    pid = (pid_t)syscall(SYS_clone, SIGCHLD, 0, core::ptr::null_mut(), 0, core::ptr::null_mut());
    ASSERT_LE(0, pid)
    TH_LOG("clone: %m");
    if (pid == 0) {
    while (chld_lock == 1)
    ...
    let mut vlenb: c_ulong = get_vr_len();
    size_t regset_size;
    struct iovec iov;
    int status;
    int ret;
// attach
    ASSERT_EQ(0, ptrace(PTRACE_ATTACH, pid, core::ptr::null_mut(), core::ptr::null_mut()));
    ASSERT_EQ(pid, waitpid(pid, &status, 0));
    ASSERT_TRUE(WIFSTOPPED(status));
// unlock
    ASSERT_EQ(0, ptrace(PTRACE_POKEDATA, pid, &chld_lock, 0));
// resume and wait for ebreak
    ASSERT_EQ(0, ptrace(PTRACE_CONT, pid, core::ptr::null_mut(), core::ptr::null_mut()));
    ASSERT_EQ(pid, waitpid(pid, &status, 0));
    ASSERT_TRUE(WIFSTOPPED(status));
...
// read tracee vector csr regs using ptrace PEEKDATA
    errno = 0;
    vstart_csr = ptrace(PTRACE_PEEKDATA, pid, &vstart, core::ptr::null_mut());
    ASSERT_FALSE((errno != 0) && (vstart_csr == -1));
    errno = 0;
    vl_csr = ptrace(PTRACE_PEEKDATA, pid, &vl, core::ptr::null_mut());
    ASSERT_FALSE((errno != 0) && (vl_csr == -1));
    errno = 0;
    vtype_csr = ptrace(PTRACE_PEEKDATA, pid, &vtype, core::ptr::null_mut());
    ASSERT_FALSE((errno != 0) && (vtype_csr == -1));
    errno = 0;
    vcsr_csr = ptrace(PTRACE_PEEKDATA, pid, &vcsr, core::ptr::null_mut());
    ASSERT_FALSE((errno != 0) && (vcsr_csr == -1));
    errno = 0;
    vlenb_csr = ptrace(PTRACE_PEEKDATA, pid, &vlenb, core::ptr::null_mut());
