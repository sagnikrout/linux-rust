//! Automatically rewritten from C to Rust
//! Source: tools/testing/selftests/gpio/gpio-cdev-uaf.c
#![no_std]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(dead_code)]
#![allow(unused_variables)]
#![allow(unused_mut)]

use core::ffi::*;

// --- Linux Kernel Primitives Prelude ---
pub type uid_t = u32;
pub type gid_t = u32;
pub type uid16_t = u16;
pub type gid16_t = u16;
pub type pid_t = i32;
pub type mode_t = u32;
pub type umode_t = u16;
pub type nlink_t = u32;
pub type off_t = i64;
pub type loff_t = i64;
pub type dev_t = u32;
pub type ino_t = u64;
pub type size_t = usize;
pub type ssize_t = isize;
pub type uintptr_t = usize;
pub type intptr_t = isize;
pub type ptrdiff_t = isize;
pub type clockid_t = i32;
pub type timer_t = i32;
pub type time64_t = i64;
pub type atomic_t = core::sync::atomic::AtomicI32;
pub type atomic64_t = core::sync::atomic::AtomicI64;
// ---------------------------------------


// SPDX-License-Identifier: GPL-2.0-or-later
//
// GPIO character device helper for UAF tests.
//
// Copyright 2026 Google LLC
//

#[no_mangle]
unsafe extern "C" fn print_usage() {
    static void print_usage(void)
    {
    printf("usage:\n");
    printf("  gpio-cdev-uaf [chip|handle|event|req] [poll|read|ioctl]\n");
    }
#[no_mangle]
unsafe extern "C" fn _create_chip(name: *const c_char, create: c_int) -> c_int {
    static int _create_chip(const char *name, int create)
    {
    char path[64];
    snprintf(path, sizeof(path), CONFIGFS_DIR "/%s", name);
    if (create)
    return mkdir(path, 0755);
    else
    return rmdir(path);
    }
#[no_mangle]
unsafe extern "C" fn create_chip(name: *const c_char) -> c_int {
    static int create_chip(const char *name)
    {
    return _create_chip(name, 1);
    }
#[no_mangle]
unsafe extern "C" fn remove_chip(name: *const c_char) {
    static void remove_chip(const char *name)
    {
    _create_chip(name, 0);
    }
#[no_mangle]
unsafe extern "C" fn _create_bank(chip_name: *const c_char, name: *const c_char, create: c_int) -> c_int {
    static int _create_bank(const char *chip_name, const char *name, int create)
    {
    char path[64];
    snprintf(path, sizeof(path), CONFIGFS_DIR "/%s/%s", chip_name, name);
    if (create)
    return mkdir(path, 0755);
    else
    return rmdir(path);
    }
#[no_mangle]
unsafe extern "C" fn create_bank(chip_name: *const c_char, name: *const c_char) -> c_int {
    static int create_bank(const char *chip_name, const char *name)
    {
    return _create_bank(chip_name, name, 1);
    }
#[no_mangle]
unsafe extern "C" fn remove_bank(chip_name: *const c_char, name: *const c_char) {
    static void remove_bank(const char *chip_name, const char *name)
    {
    _create_bank(chip_name, name, 0);
    }
#[no_mangle]
unsafe extern "C" fn _enable_chip(name: *const c_char, enable: c_int) -> c_int {
    static int _enable_chip(const char *name, int enable)
    {
    char path[64];
    int fd, ret;
    snprintf(path, sizeof(path), CONFIGFS_DIR "/%s/live", name);
    fd = open(path, O_WRONLY);
    if (fd == -1)
    return fd;
    if (enable)
    ret = write(fd, "1", 1);
    else
    ret = write(fd, "0", 1);
    close(fd);
    let mut ret: return = = 1 ? 0 : -1;
    }
#[no_mangle]
unsafe extern "C" fn enable_chip(name: *const c_char) -> c_int {
    static int enable_chip(const char *name)
    {
    return _enable_chip(name, 1);
    }
#[no_mangle]
unsafe extern "C" fn disable_chip(name: *const c_char) {
    static void disable_chip(const char *name)
    {
    _enable_chip(name, 0);
    }
#[no_mangle]
unsafe extern "C" fn open_chip(chip_name: *const c_char, bank_name: *const c_char) -> c_int {
    static int open_chip(const char *chip_name, const char *bank_name)
    {
    char path[64], dev_name[32];
    int ret, fd;
    ret = create_chip(chip_name);
    if (ret) {
    fprintf(stderr, "failed to create chip\n");
    return ret;
    }
    ret = create_bank(chip_name, bank_name);
    if (ret) {
    fprintf(stderr, "failed to create bank\n");
    goto err_remove_chip;
    }
    ret = enable_chip(chip_name);
    if (ret) {
    fprintf(stderr, "failed to enable chip\n");
    goto err_remove_bank;
    }
    snprintf(path, sizeof(path), CONFIGFS_DIR "/%s/%s/chip_name",
    chip_name, bank_name);
    fd = open(path, O_RDONLY);
    if (fd == -1) {
    ret = fd;
    fprintf(stderr, "failed to open %s\n", path);
    goto err_disable_chip;
    }
    ret = read(fd, dev_name, sizeof(dev_name) - 1);
    close(fd);
    if (ret == -1) {
    fprintf(stderr, "failed to read %s\n", path);
    goto err_disable_chip;
    }
    dev_name[ret] = '\0';
    if (ret && dev_name[ret - 1] == '\n')
    dev_name[ret - 1] = '\0';
    snprintf(path, sizeof(path), "/dev/%s", dev_name);
    fd = open(path, O_RDWR);
    if (fd == -1) {
    ret = fd;
    fprintf(stderr, "failed to open %s\n", path);
    goto err_disable_chip;
    }
    return fd;
    err_disable_chip:
    disable_chip(chip_name);
    err_remove_bank:
    remove_bank(chip_name, bank_name);
    err_remove_chip:
    remove_chip(chip_name);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn close_chip(chip_name: *const c_char, bank_name: *const c_char) {
    static void close_chip(const char *chip_name, const char *bank_name)
    {
    disable_chip(chip_name);
    remove_bank(chip_name, bank_name);
    remove_chip(chip_name);
    }
#[no_mangle]
unsafe extern "C" fn test_poll(fd: c_int) -> c_int {
    static int test_poll(int fd)
    {
    struct pollfd pfds;
    pfds.fd = fd;
    pfds.events = POLLIN;
    pfds.revents = 0;
    if (poll(&pfds, 1, 0) == -1)
    return -1;
    return (pfds.revents & ~(POLLHUP | POLLERR)) ? -1 : 0;
    }
#[no_mangle]
unsafe extern "C" fn test_read(fd: c_int) -> c_int {
    static int test_read(int fd)
    {
    char data;
    if (read(fd, &data, 1) == -1 && errno == ENODEV)
    return 0;
    return -1;
    }
#[no_mangle]
unsafe extern "C" fn test_ioctl(fd: c_int) -> c_int {
    static int test_ioctl(int fd)
    {
    if (ioctl(fd, 0, core::ptr::null_mut()) == -1 && errno == ENODEV)
    return 0;
    return -1;
    }
#[no_mangle]
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    int main(int argc, char **argv)
    {
    int cfd, fd, ret;
    int (*test_func)(int);
    if (argc != 3) {
    print_usage();
    return EXIT_FAILURE;
    }
    if (strcmp(argv[1], "chip") == 0 ||
    strcmp(argv[1], "event") == 0 ||
    strcmp(argv[1], "req") == 0) {
    if (strcmp(argv[2], "poll") &&
    strcmp(argv[2], "read") &&
    strcmp(argv[2], "ioctl")) {
    fprintf(stderr, "unknown command: %s\n", argv[2]);
    return EXIT_FAILURE;
    }
    } else if (strcmp(argv[1], "handle") == 0) {
    if (strcmp(argv[2], "ioctl")) {
    fprintf(stderr, "unknown command: %s\n", argv[2]);
    return EXIT_FAILURE;
    }
    } else {
    fprintf(stderr, "unknown command: %s\n", argv[1]);
    return EXIT_FAILURE;
    }
    if (strcmp(argv[2], "poll") == 0)
    test_func = test_poll;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[2], 0: "read") ==) -> else {
    else if (strcmp(argv[2], "read") == 0)
    test_func = test_read;
    else	/* strcmp(argv[2], "ioctl") == 0 */
    test_func = test_ioctl;
    cfd = open_chip("chip", "bank");
    if (cfd == -1) {
    fprintf(stderr, "failed to open chip\n");
    return EXIT_FAILURE;
    }
// Step 1: Hold a FD to the test target.
    if (strcmp(argv[1], "chip") == 0) {
    fd = cfd;
    } else if (strcmp(argv[1], "handle") == 0) {
    let mut req: gpiohandle_request = {0};
    req.lines = 1;
    if (ioctl(cfd, GPIO_GET_LINEHANDLE_IOCTL, &req) == -1) {
    fprintf(stderr, "failed to get handle FD\n");
    goto err_close_chip;
    }
    close(cfd);
    fd = req.fd;
    } else if (strcmp(argv[1], "event") == 0) {
    let mut req: gpioevent_request = {0};
    if (ioctl(cfd, GPIO_GET_LINEEVENT_IOCTL, &req) == -1) {
    fprintf(stderr, "failed to get event FD\n");
    goto err_close_chip;
    }
    close(cfd);
    fd = req.fd;
    } else {	/* strcmp(argv[1], "req") == 0 */
    let mut req: gpio_v2_line_request = {0};
    req.num_lines = 1;
    if (ioctl(cfd, GPIO_V2_GET_LINE_IOCTL, &req) == -1) {
    fprintf(stderr, "failed to get req FD\n");
    goto err_close_chip;
    }
    close(cfd);
    fd = req.fd;
    }
// Step 2: Free the chip.
    close_chip("chip", "bank");
// Step 3: Access the dangling FD to trigger UAF.
    ret = test_func(fd);
    close(fd);
    return ret ? EXIT_FAILURE : EXIT_SUCCESS;
    err_close_chip:
    close(cfd);
    close_chip("chip", "bank");
    return EXIT_FAILURE;
    }
