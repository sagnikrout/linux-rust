...
//! Source: tools/testing/selftests/ublk/batch.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn ublk_alloc_commit_buf(t: *mut ublk_thread) -> c_ushort {
    static inline unsigned short ublk_alloc_commit_buf(struct ublk_thread *t)
    {
    let mut idx: c_int = allocator_get(&t.commit_buf_alloc);
    if (idx >= 0)
...
//
    ...
    unsigned short i)
    {
    let mut idx: c_ushort = i - t.commit_buf_start;
...
#[no_mangle]
unsafe extern "C" fn ublk_commit_elem_buf_size(dev: *mut ublk_dev) -> c_uchar {
...
#[no_mangle]
unsafe extern "C" fn ublk_commit_buf_size(t: *mut ublk_thread) -> unsigned {
    static unsigned ublk_commit_buf_size(struct ublk_thread *t)
    {
    struct ublk_dev *dev = t.dev;
    let mut elem_size: unsigned = ublk_commit_elem_buf_size(dev);
    let mut total: c_uint = elem_size * dev.dev_info.queue_depth;
    let mut page_sz: c_uint = getpagesize();
    return round_up(total, page_sz);
    }
#[no_mangle]
unsafe extern "C" fn free_batch_commit_buf(t: *mut ublk_thread) {
    static void free_batch_commit_buf(struct ublk_thread *t)
    {
    if (t.commit_buf) {
    let mut buf_size: unsigned = ublk_commit_buf_size(t);
    let mut total: c_uint = buf_size * t.nr_commit_buf;
...
#[no_mangle]
unsafe extern "C" fn alloc_batch_commit_buf(t: *mut ublk_thread) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ublk_thread_nr_queues(t: *const ublk_thread) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn ublk_batch_prepare(t: *mut ublk_thread) {
...
#[no_mangle]
unsafe extern "C" fn free_batch_fetch_buf(t: *mut ublk_thread) {
...
#[no_mangle]
unsafe extern "C" fn alloc_batch_fetch_buf(t: *mut ublk_thread) -> c_int {
    static int alloc_batch_fetch_buf(struct ublk_thread *t)
    {
// page aligned fetch buffer, and it is mlocked for speedup delivery
    let mut pg_sz: unsigned = getpagesize();
    let mut buf_size: unsigned = round_up(t.dev.dev_info.queue_depth * 2, pg_sz);
    int ret;
    let mut i: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn ublk_batch_alloc_buf(t: *mut ublk_thread) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ublk_batch_free_buf(t: *mut ublk_thread) {
...
// Use plain user buffer instead of fixed buffer
    ...
    unsigned short buf_idx)
    {
    let mut nr_elem: c_ushort = t.fetch[buf_idx].fetch_buf_size / 2;
    struct io_uring_sqe *sqe;
...
#[no_mangle]
pub unsafe extern "C" fn ublk_batch_start_fetch(t: *mut ublk_thread) {
...
// submit two fetch commands for each queue
    ...
    {
    let mut buf_idx: c_ushort = user_data_to_tag(cqe.user_data);
    let mut start: unsigned = t.fetch[buf_idx].fetch_buf_off;
    let mut end: unsigned = start + cqe.res;
    ...
    let mut tag: c_ushort = *(unsigned short *)(buf + i);
    ublk_err("%u ", tag);
    }
    ublk_err("\n");
    }
...
#[no_mangle]
unsafe extern "C" fn __ublk_batch_queue_prep_io_cmds(t: *mut ublk_thread, q: *mut ublk_queue) -> c_int {
    static int __ublk_batch_queue_prep_io_cmds(struct ublk_thread *t, struct ublk_queue *q)
    {
    let mut nr_elem: c_ushort = q.q_depth;
    let mut buf_idx: c_ushort = ublk_alloc_commit_buf(t);
    struct io_uring_sqe *sqe;
    void *buf;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !ublk_queue_no_buf(q)) -> else {
    else if (!ublk_queue_no_buf(q))
...
#[no_mangle]
pub unsafe extern "C" fn ublk_batch_queue_prep_io_cmds(t: *mut ublk_thread, q: *mut ublk_queue) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_IOC_NR(UBLK_U_IO_COMMIT_IO_CMDS): op ==) -> else {
    let mut nr_elem: c_int = user_data_to_tgt_data(cqe.user_data);
...
// FETCH command is per queue
    ...
    struct io_uring_sqe *sqe;
    unsigned short buf_idx;
    let mut nr_elem: c_ushort = cb.done;
// nothing to commit
    if (!nr_elem) {
...
#[no_mangle]
pub unsafe extern "C" fn ublk_batch_commit_io_cmds(t: *mut ublk_thread) {
...
#[no_mangle]
pub unsafe extern "C" fn ublk_batch_prep_commit(t: *mut ublk_thread) {
    ...
    {
    let mut q_t_idx: unsigned = ublk_queue_idx_in_thread(t, q);
