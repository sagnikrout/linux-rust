...
//! Source: tools/testing/selftests/ublk/file_backed.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ublk_to_uring_op(iod: *const ublksrv_io_desc, zc: c_int) -> enum io_uring_op {
...
//
    ...
    {
    let mut ublk_op: unsigned = ublksrv_get_op(iod);
    let mut op: enum io_uring_op = ublk_to_uring_op(iod, 0);
    let mut file_offset: __u64 = iod.start_sector << 9;
    let mut len: __u32 = iod.nr_sectors << 9;
    let mut shmem_idx: __u32 = ublk_shmem_zc_index(iod.addr);
    let mut shmem_off: __u32 = ublk_shmem_zc_offset(iod.addr);
    struct io_uring_sqe *sqe[1];
    void *addr;
    ...
    {
    let mut ublk_op: unsigned = ublksrv_get_op(iod);
    let mut zc: unsigned = ublk_queue_use_zc(q);
    let mut auto_zc: unsigned = ublk_queue_use_auto_zc(q);
    let mut op: enum io_uring_op = ublk_to_uring_op(iod, zc | auto_zc);
    struct ublk_io *io = ublk_get_io(q, tag);
    let mut offset: __u64 = iod.start_sector << 9;
    let mut len: __u32 = iod.nr_sectors << 9;
    struct io_uring_sqe *sqe[3];
    void *addr = io.buf_addr;
    let mut buf_index: c_ushort = ublk_io_buf_idx(t, q, tag);
...
#[no_mangle]
unsafe extern "C" fn loop_queue_tgt_io(t: *mut ublk_thread, q: *mut ublk_queue, tag: c_int) -> c_int {
    ...
    int tag)
    {
    let mut queued: c_int = loop_queue_tgt_io(t, q, tag);
...
#[no_mangle]
unsafe extern "C" fn ublk_loop_memset_file(fd: c_int, byte: __u8, len: usize) -> c_int {
    ...
    while (len) {
    let mut ret: c_int = pwrite(fd, buf, min(len, sizeof(buf)), offset);
    if (ret < 0)
    return -errno;
    if (!ret)
    return -EIO;
