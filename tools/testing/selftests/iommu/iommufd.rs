...
//! Source: tools/testing/selftests/iommu/iommufd.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn get_huge_page_size() -> c_ulong {
    ...
    return 2 * 1024 * 1024;
    buf[ret] = 0;
    return strtoul(buf, core::ptr::null_mut(), 10);
    }
#[no_mangle]
pub unsafe extern "C" fn __attribute__(setup_sizes(void: (constructor)) void) -> static {
    ...
    {
    let mut cmd: iommu_destroy = { .size = sizeof(cmd), .id = 0 };
// object id is invalid
    EXPECT_ERRNO(ENOENT, _test_ioctl_destroy(self.fd, 0));
// Bad pointer
    EXPECT_ERRNO(EFAULT, ioctl(self.fd, IOMMU_DESTROY, core::ptr::null_mut()));
// Unknown ioctl
    EXPECT_ERRNO(ENOTTY,
...
#[no_mangle]
unsafe extern "C" fn drop_cap_ipc_lock(_metadata: *mut __test_metadata) {
    static void drop_cap_ipc_lock(struct __test_metadata *_metadata)
    {
    cap_t caps;
    cap_value_t cap_list[1] = { CAP_IPC_LOCK };
    caps = cap_get_proc();
    ASSERT_NE(caps, core::ptr::null_mut());
    ASSERT_NE(-1,
...
#[no_mangle]
unsafe extern "C" fn get_proc_status_value(pid: pid_t, var: *const c_char) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn get_vm_pinned(pid: pid_t) -> c_long {
    static long get_vm_pinned(pid_t pid)
    {
    return get_proc_status_value(pid, "VmPin:");
    }
#[no_mangle]
unsafe extern "C" fn get_vm_locked(pid: pid_t) -> c_long {
    ...
    test_cmd_mock_domain(self.ioas_id, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    }
...
// Change process works in another process
    child = fork();
    if (!child) {
    let mut nlock: c_int = PAGE_SIZE / 1024;
...
// Child accounts for locked memory after
    ...
    ASSERT_NE(-1, child);
    ASSERT_EQ(child, waitpid(child, core::ptr::null_mut(), 0));
    }
...
// Can allocate and manually free an IOAS table
    ...
    uint32_t num_inv;
    let mut parent_hwpt_id: u32 = 0;
    let mut parent_hwpt_id_not_work: u32 = 0;
...
// Negative nested tests
    ...
    parent_hwpt_id, 0,
    &nested_hwpt_id[0],
    IOMMU_HWPT_DATA_SELFTEST, core::ptr::null_mut(),
    sizeof(data));
...
// Negative test: invalid data_uptr when array is not empty
    num_inv = 1;
    inv_reqs[0].flags = 0;
    test_err_hwpt_invalidate(EINVAL, nested_hwpt_id[0], core::ptr::null_mut(),
    IOMMU_HWPT_INVALIDATE_DATA_SELFTEST,
    sizeof(*inv_reqs), &num_inv);
    assert(!num_inv);
...
// Create a device attached directly to a hwpt
    if (self.stdev_id) {
    test_cmd_mock_domain(self.hwpt_id, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    } else {
    test_err_mock_domain(ENOENT, self.hwpt_id, core::ptr::null_mut(), core::ptr::null_mut());
    }
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iommu_test_hw_info_buffer_larger {
    pub info: iommu_test_hw_info,
    pub trailing_bytes: u64,
    pub buffer_larger: },
    if (self.device_id) {
    pub 0: uint8_t max_pasid =,
// Provide a zero-size user_buffer
    test_cmd_get_hw_info(self.device_id,
    pub 0): IOMMU_HW_INFO_TYPE_DEFAULT, NULL,,
...
// Request for a wrong data_type, and a correct one
    ...
    pub sizeof(buffer_exact)): &buffer_exact,,
    test_cmd_get_hw_info(self.device_id,
...
//
    ...
    pub &max_pasid): test_cmd_get_hw_info_pasid(self->device_id,,
    pub max_pasid): ASSERT_EQ(0,,
    if (variant.pasid_capable) {
    test_cmd_get_hw_info_pasid(self.device_pasid_id,
    pub max_pasid): ASSERT_EQ(MOCK_PASID_WIDTH,,
    }
    } else {
...
// Unmap fails if nothing is mapped
    pub i++): for (i = 0; i != 10;,
    pub PAGE_SIZE): *mut *mut test_err_ioctl_ioas_unmap(ENOENT, i  PAGE_SIZE,,
// Unmap works
    pub i++): for (i = 0; i != 10;,
    test_ioctl_ioas_map_fixed(buffer, PAGE_SIZE,
    pub PAGE_SIZE): *mut *mut self->base_iova + i,
    pub i++): for (i = 0; i != 10;,
    test_ioctl_ioas_unmap(self.base_iova + i * PAGE_SIZE,
// Split fails
    test_ioctl_ioas_map_fixed(buffer, PAGE_SIZE * 2,
    pub PAGE_SIZE): *mut *mut self->base_iova + 16,
...
// Over map fails
    ...
    pub PAGE_SIZE): *mut *mut self->base_iova + 16,
    test_err_ioctl_ioas_map_fixed(EEXIST, buffer, PAGE_SIZE,
    pub PAGE_SIZE): *mut *mut self->base_iova + 17,
    test_err_ioctl_ioas_map_fixed(EEXIST, buffer, PAGE_SIZE * 2,
    pub PAGE_SIZE): *mut *mut self->base_iova + 15,
    test_err_ioctl_ioas_map_fixed(EEXIST, buffer, PAGE_SIZE * 3,
    pub PAGE_SIZE): *mut *mut self->base_iova + 15,
// unmap all works
    pub UINT64_MAX): test_ioctl_ioas_unmap(0,,
...
// Unmap fully contained areas works
    ...
    4 * PAGE_SIZE,
    pub unmap_len): *mut *mut ASSERT_EQ(32  PAGE_SIZE,,
    }
...
// Kernel automatically aligns IOVAs properly
    for (i = 0; i != 10; i++) {
    let mut length: usize = PAGE_SIZE * (i + 1);
    if (self.stdev_id) {
...
// Buffer too small
    ...
    let mut buf_size: usize = 2 * HUGEPAGE_SIZE;
    uint8_t *buf;
...
// Add/remove a domain with a user
    ...
    &mock_hwpt_id, core::ptr::null_mut());
...
// Multi-page test
    ...
    ASSERT_NE(core::ptr::null_mut(), tmp2);
...
//
    test_err_mock_domain(EFAULT, self.ioas_id, core::ptr::null_mut(), core::ptr::null_mut());
    }
    test_cmd_destroy_access(access_id);
    }
...
// Confirm misalignment is rejected during alignment upgrade
    ...
    {
    let mut buf_size: usize = PAGE_SIZE*4;
    __u64 iova;
    int dfd;
...
// Have the kernel check that the user pages made it to the iommu_domain
    ...
    {
    let mut buf_size: usize = self.mmap_buf_size;
    uint8_t *buf;
    __u64 iova;
// Simple one page map
    test_ioctl_ioas_map(buffer, PAGE_SIZE, &iova);
...
//
    ...
    let mut length: usize = end - start;
    __u64 iova;
    if (variant.file) {
...
// Add and destroy a domain while the area exists
    ...
    test_cmd_mock_domain(self.ioas_id, &mock_stdev_id,
    &self.hwpt_ids[1], core::ptr::null_mut());
    check_mock_iova(buf + start, iova, length);
    check_refs(buf + start / PAGE_SIZE * PAGE_SIZE,
    end / PAGE_SIZE * PAGE_SIZE -
    start / PAGE_SIZE * PAGE_SIZE,
...
// Do a hw_pagetable rotation test
    ...
    test_cmd_mock_domain(hwpt_id[0], &stddev_id, core::ptr::null_mut(), core::ptr::null_mut());
...
// Regular case
    ...
    test_cmd_mock_domain(hwpt_id, &stddev_id, core::ptr::null_mut(), core::ptr::null_mut());
...
// IOMMU device does not support dirty tracking
    ...
    let mut dev_flags: u32 = MOCK_FLAGS_DEVICE_PASID;
// Regular case
    test_cmd_hwpt_alloc(self.idev_id, self.ioas_id,
    IOMMU_HWPT_ALLOC_PASID | IOMMU_HWPT_ALLOC_DIRTY_TRACKING,
    &hwpt_id);
    test_cmd_mock_domain_flags(hwpt_id, dev_flags, &stddev_id, core::ptr::null_mut(), core::ptr::null_mut());
...
// IOMMU device does not support dirty tracking
    dev_flags |= MOCK_FLAGS_DEVICE_NO_DIRTY;
    test_ioctl_ioas_alloc(&ioas_id);
    test_cmd_mock_domain_flags(ioas_id, dev_flags, &stddev_id, core::ptr::null_mut(), core::ptr::null_mut());
    ...
    {
    let mut page_size: u32 = MOCK_PAGE_SIZE;
    let mut ioas_id: u32 = self.ioas_id;
    uint32_t hwpt_id;
    if (variant.hugepages)
...
// Requires a domain to be attached
    ...
    {
    let mut info_cmd: vfio_iommu_type1_info = {};
// Invalid argsz
    info_cmd.argsz = 1;
...
//
#[no_mangle]
unsafe extern "C" fn is_filled(buf: *const c_void, c: u8, len: usize) -> bool {
...
#[no_mangle]
unsafe extern "C" fn shuffle_array(array: *mut c_ulong, nelms: usize) {
    static void shuffle_array(unsigned long *array, size_t nelms)
    {
    unsigned int i;
// Shuffle
    for (i = 0; i != nelms; i++) {
    let mut tmp: c_ulong = array[i];
    let mut other: c_uint = rand() % (nelms - i);
    array[i] = array[other];
    array[other] = tmp;
    }
    }
...
// Small pages
    ...
    {
    let mut buf_size: usize = HUGEPAGE_SIZE * 2;
    struct vfio_iommu_type1_dma_map map_cmd = {
    .argsz = sizeof(map_cmd),
    .flags = VFIO_DMA_MAP_FLAG_READ | VFIO_DMA_MAP_FLAG_WRITE,
    .size = buf_size,
...
// type1 mode can cut up larger mappings, type1v2 always fails
    ...
    test_cmd_mock_domain(self.ioas_id, &self.stdev_id, core::ptr::null_mut(),
    &self.device_id);
...
// Allocate a vIOMMU taking refcount of the parent hwpt
    ...
    IOMMU_VIOMMU_TYPE_SELFTEST, core::ptr::null_mut(), 0,
    &self.viommu_id);
// Allocate a regular nested hwpt
    ...
    {
    let mut device_id: u32 = self.device_id;
    let mut ioas_id: u32 = self.ioas_id;
    uint32_t hwpt_id;
    if (self.device_id) {
...
// Negative test -- unsupported viommu type
    ...
    0xdead, core::ptr::null_mut(), 0, core::ptr::null_mut());
    EXPECT_ERRNO(EBUSY,
    ...
    let mut viommu_id: u32 = self.viommu_id;
    let mut dev_id: u32 = self.device_id;
    uint32_t iopf_hwpt_id;
    uint32_t fault_id;
    uint32_t fault_fd;
    uint32_t vdev_id;
...
// Now do a correct mmap for a loopback test
    test = mmap(core::ptr::null_mut(), data.out_mmap_length, PROT_READ | PROT_WRITE,
...
// The owner of the mmap region should be blocked
    ...
    uint32_t veventq_id;
    uint32_t veventq_fd;
    let mut prev_seq: c_int = -1;
    let mut hdr_size: usize = sizeof(struct iommufd_vevent_header);
    char vbuf[64];
    if (dev_id) {
// Must allocate vdevice before attaching to a nested hwpt
    test_err_mock_domain_replace(ENOENT, self.stdev_id,
...
// Test depth lower and upper bounds (mirrors kernel cap)
    ...
    IOMMU_VEVENTQ_TYPE_SELFTEST, 0, core::ptr::null_mut(),
    core::ptr::null_mut());
    test_err_veventq_alloc(EINVAL, viommu_id,
    IOMMU_VEVENTQ_TYPE_SELFTEST,
    VEVENTQ_MAX_DEPTH + 1, core::ptr::null_mut(), core::ptr::null_mut());
...
// Allocate a vEVENTQ with veventq_depth=2
    ...
    IOMMU_VEVENTQ_TYPE_SELFTEST, 2, core::ptr::null_mut(),
    core::ptr::null_mut());
...
// Overflow must be gone after the previous reads
    ...
    test_err_vdevice_alloc(ENOENT, viommu_id, dev_id, 0x99, core::ptr::null_mut());
    }
    }
...
// Negative test: invalid data_uptr when array is not empty
    ...
    inv_reqs[0].flags = 0;
    inv_reqs[0].vdev_id = 0x99;
    test_err_viommu_invalidate(EINVAL, viommu_id, core::ptr::null_mut(),
    IOMMU_VIOMMU_INVALIDATE_DATA_SELFTEST,
    sizeof(*inv_reqs), &num_inv);
    assert(!num_inv);
// Negative test: invalid entry_len when array is not empty
...
// Invalidate all cache entries for nested_dev_id[1] and verify
    ...
    {
    let mut iova: __u64 = MOCK_APERTURE_START, iova2;
    let mut viommu_id: u32 = self.viommu_id;
...
// Now it can unmap the first page
    ...
    &self.no_pasid_stdev_id, core::ptr::null_mut(),
    ...
    let mut s2_hwpt_id: u32 = 0;
    uint32_t iopf_hwpt_id;
    let mut pasid: u32 = 100;
    uint32_t viommu_id;
//
...
// Allocate a regular nested hwpt based on viommu
    ...
    IOMMU_VIOMMU_TYPE_SELFTEST, core::ptr::null_mut(), 0, &viommu_id);
