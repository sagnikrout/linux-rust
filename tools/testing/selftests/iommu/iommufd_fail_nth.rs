...
//! Source: tools/testing/selftests/iommu/iommufd_fail_nth.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn writeat(dfd: c_int, fn: *const c_char, val: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __attribute__(setup_buffer(void: (constructor)) void) -> static {
...
//
#[no_mangle]
pub unsafe extern "C" fn __attribute__(setup_fault_injection(void: (constructor)) void) -> static {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fail_nth_state {
    pub proc_fd: c_int,
    pub iteration: c_uint,
}
...
// Failure path of no IOVA to unmap
    _test_ioctl_ioas_unmap(self.fd, ioas_id, iova, BUFFER_SIZE, core::ptr::null_mut());
    return 0;
    }
// iopt_area_fill_domains() and iopt_area_fill_domain()
    ...
    return -1;
    fail_nth_enable();
    if (_test_cmd_mock_domain(self.fd, ioas_id, &stdev_id, &hwpt_id, core::ptr::null_mut()))
    return -1;
...
// device.c
    ...
    &self.stdev_id, core::ptr::null_mut(), &idev_id))
    return -1;
    if (_test_cmd_get_hw_info(self.fd, idev_id, IOMMU_HW_INFO_TYPE_DEFAULT,
    &info, sizeof(info), core::ptr::null_mut(), core::ptr::null_mut()))
    return -1;
    if (_test_cmd_hwpt_alloc(self.fd, idev_id, ioas_id, 0,
    IOMMU_HWPT_ALLOC_PASID, &hwpt_id,
    IOMMU_HWPT_DATA_NONE, 0, 0))
    return -1;
    if (_test_cmd_mock_domain_replace(self.fd, self.stdev_id, ioas_id2, core::ptr::null_mut()))
    return -1;
    if (_test_cmd_mock_domain_replace(self.fd, self.stdev_id, hwpt_id, core::ptr::null_mut()))
    return -1;
    if (_test_cmd_hwpt_alloc(self.fd, idev_id, ioas_id, 0,
