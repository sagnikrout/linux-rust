...
//! Source: tools/objtool/klp-diff.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct elfs {
    pub out: *mut *mut *mut elf orig, patched,,
...
// c = '_';
#[no_mangle]
pub unsafe extern "C" fn if('.': *mut *mut c ==) -> else {
// c = '\0';
    break;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn read_exports() -> c_int {
    static int read_exports(void)
    {
    const char *symvers = "Module.symvers";
    char line[1024], *path = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn read_sym_checksums(elf: *mut elf) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn is_uncorrelated_static_local(sym: *mut symbol) -> bool {
...
// WARN_ONCE, etc
    ...
    let mut len: usize = strlen(vars[i]);
// GCC: <var>.<id>
...
//
#[no_mangle]
unsafe extern "C" fn is_local_label(sym: *mut symbol) -> bool {
    static bool is_local_label(struct symbol *sym)
    {
    return strstarts(sym.name, ".L");
    }
#[no_mangle]
unsafe extern "C" fn is_special_section(sec: *mut section) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_special_section_aux(sec: *mut section) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_addressable_sym(sym: *mut symbol) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_abs_sym(sym: *mut symbol) -> bool {
    static bool is_abs_sym(struct symbol *sym)
    {
    return sym.sym.st_shndx == SHN_ABS && !is_file_sym(sym);
    }
#[no_mangle]
unsafe extern "C" fn is_initcall_sym(sym: *mut symbol) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_anonymous_rodata(sym: *mut symbol) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn dont_correlate(sym: *mut symbol) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_llvm_sym(sym: *mut symbol) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn maybe_same_file(sym1: *mut symbol, sym2: *mut symbol) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn same_file(sym1: *mut symbol, sym2: *mut symbol) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_tu_local_sym(sym: *mut symbol) -> bool {
...
//
    ...
    {
    struct symbol *name_last = core::ptr::null_mut(), *scope_last = core::ptr::null_mut(),
// file_last = NULL, *csum_last = NULL;
    let mut name_orig: c_uint = 0, name_patched = 0;
    let mut scope_orig: c_uint = 0, scope_patched = 0;
    let mut file_orig: c_uint = 0, file_patched = 0;
    let mut csum_orig: c_uint = 0, csum_patched = 0;
    struct symbol *sym2, *match = core::ptr::null_mut();
...
// Return the widest level that yields a unique (1:1) match
    ...
    match = name_last;
#[no_mangle]
pub unsafe extern "C" fn if(1: scope_orig == 1 && scope_patched ==) -> else {
    else if (scope_orig == 1 && scope_patched == 1)
    match = scope_last;
#[no_mangle]
pub unsafe extern "C" fn if(1: file_orig == 1 && file_patched ==) -> else {
    else if (file_orig == 1 && file_patched == 1)
    match = file_last;
#[no_mangle]
pub unsafe extern "C" fn if(1: csum_orig == 1 && csum_patched ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn update_suffix_map(elf: *mut elf) -> c_int {
...
//
    ...
    {
    const char *suffix, *patched_suffix = core::ptr::null_mut();
    struct symbol *sym2, *match = core::ptr::null_mut();
...
//
    ...
    {
    let mut idx_orig: c_uint = 0, idx_patched = 0;
    let mut sym1_pos: c_uint = 0;
    struct symbol *sym2, *match = core::ptr::null_mut();
    for_each_sym_by_demangled_name(e.orig, sym1.demangled_name, sym2) {
    if (sym2.twin || sym1.type != sym2.type || sym2.dont_correlate ||
    !maybe_same_file(sym1, sym2))
...
//
#[no_mangle]
unsafe extern "C" fn correlate_symbols(e: *mut elfs) -> c_int {
...
// Correlate FILE symbols
    ...
    ERROR("FILE symbol mismatch: core::ptr::null_mut() != %s", file2_sym.name);
    return -1;
    }
    if (file1_sym && !file2_sym) {
    ERROR("FILE symbol mismatch: %s != core::ptr::null_mut()", file1_sym.name);
    return -1;
    }
    if (!file1_sym)
    break;
...
//
    ...
    bool data_too)
    {
    struct section *out_sec = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn mark_included_function(func: *mut symbol) {
...
//
#[no_mangle]
unsafe extern "C" fn mark_changed_functions(e: *mut elfs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn clone_included_functions(e: *mut elfs) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn klp_reloc_needed(patched_reloc: *mut reloc) -> bool {
...
#[no_mangle]
unsafe extern "C" fn convert_reloc_sym_to_secsym(elf: *mut elf, reloc: *mut reloc) -> c_int {
...
// Return -1 error, 0 success, 1 skip
#[no_mangle]
unsafe extern "C" fn convert_reloc_secsym_to_sym(elf: *mut elf, reloc: *mut reloc) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn is_uncorrelated_section(sec: *mut section) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn convert_reloc_sym(elf: *mut elf, reloc: *mut reloc) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn has_module_dep(e: *mut elfs, dep_mod: *const c_char) -> bool {
...
//
    ...
    struct symbol *patched_sym = patched_reloc.sym;
    let mut addend: i64 = reloc_addend(patched_reloc);
...
//
    ...
    return -1;
    sym = elf_create_symbol(e.out, tombstone_name, core::ptr::null_mut(),
...
// STB_WEAK: avoid modpost undefined symbol warnings
    klp_sym = elf_create_symbol(e.out, sym_name, core::ptr::null_mut(),
    STB_WEAK, patched_sym.type, 0, 0);
    if (!klp_sym)
    return -1;
    }
...
// Copy a reloc and its symbol to the output object
    ...
    let mut addend: c_long = reloc_addend(patched_reloc);
    struct symbol *out_sym;
    bool klp;
...
#[no_mangle]
unsafe extern "C" fn clone_sym_relocs(e: *mut elfs, patched_sym: *mut symbol) -> c_int {
    static int clone_sym_relocs(struct elfs *e, struct symbol *patched_sym)
    {
...
#[no_mangle]
unsafe extern "C" fn has_fake_symbols(sec: *mut section) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn create_fake_symbols(elf: *mut elf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn should_keep_special_sym(elf: *mut elf, sym: *mut symbol) -> bool {
    static bool should_keep_special_sym(struct elf *elf, struct symbol *sym)
    {
    let mut annotate_insn: bool = !strcmp(sym.sec.name, ".discard.annotate_insn");
    struct reloc *reloc;
...
//
#[no_mangle]
unsafe extern "C" fn validate_special_section_klp_reloc(e: *mut elfs, sym: *mut symbol) -> c_int {
    static int validate_special_section_klp_reloc(struct elfs *e, struct symbol *sym)
    {
    let mut static_branch: bool = !strcmp(sym.sec.name, "__jump_table");
    let mut static_call: bool = !strcmp(sym.sec.name, ".static_call_sites");
    const char *code_sym = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn clone_special_section(e: *mut elfs, patched_sec: *mut section) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn clone_special_sections(e: *mut elfs) -> c_int {
    static int clone_special_sections(struct elfs *e)
    {
    struct section *sec, *annotate_insn = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn create_klp_sections(e: *mut elfs) -> c_int {
    static int create_klp_sections(struct elfs *e)
    {
    let mut obj_size: usize = sizeof(struct klp_object_ext);
    let mut func_size: usize = sizeof(struct klp_func_ext);
...
// allocate klp_object_ext
    obj_data = elf_add_data(e.out, obj_sec, core::ptr::null_mut(), obj_size);
    if (!obj_data)
    return -1;
...
// klp_object_ext.funcs
    ...
    let mut offset: c_ulong = nr_funcs * func_size;
    unsigned long sympos;
    void *func_data;
...
// allocate klp_func_ext
    func_data = elf_add_data(e.out, funcs_sec, core::ptr::null_mut(), func_size);
    if (!func_data)
    return -1;
...
//
#[no_mangle]
unsafe extern "C" fn copy_import_ns(e: *mut elfs) -> c_int {
    static int copy_import_ns(struct elfs *e)
    {
    struct section *patched_sec, *out_sec = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn cmd_klp_diff(argc: c_int, argv: *const c_char) -> c_int {
