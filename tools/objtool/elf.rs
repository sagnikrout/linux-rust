...
#[no_mangle]
pub unsafe extern "C" fn str_hash_demangled(str: *const c_char) -> u32 {
...
// head = node->next;
    ...
    for (prev = core::ptr::null_mut(), cur = *head; cur; prev = cur, cur = cur.next) {
    if (cur == node) {
    prev.next = cur.next;
    break;
    }
    ...
    __elf_table(elf, name) = mmap(core::ptr::null_mut(),				\
    sizeof(struct elf_hash_node *) << __elf_bits(elf, name), \
    PROT_READ|PROT_WRITE,			\
    MAP_PRIVATE|MAP_ANON, -1, 0);		\
    if (__elf_table(elf, name) == (void *)-1L) {			\
    ERROR_GLIBC("mmap fail " #name);			\
    __elf_table(elf, name) = core::ptr::null_mut();				\
    }								\
    __elf_table(elf, name);						\
    })
#[no_mangle]
pub unsafe extern "C" fn __sym_start(s: *mut symbol) -> c_ulong {
    static inline unsigned long __sym_start(struct symbol *s)
    {
    return s.offset;
    }
#[no_mangle]
pub unsafe extern "C" fn __sym_last(s: *mut symbol) -> c_ulong {
...
unsafe extern "C" fn symbol_hole_by_offset(key: *const c_void, node: *const rb_node) -> c_int {
    ...
    {
    struct rb_root_cached *tree = (struct rb_root_cached *)&sec.symbol_tree;
    struct symbol *sym = core::ptr::null_mut(), *tmp;
...
//
    ...
    return sym ? sym.alias : core::ptr::null_mut();
    }
//
...
// If there are multiple matches, return the first one in the range
    ...
    {
    struct reloc *reloc, *r = core::ptr::null_mut();
    struct section *rsec;
    unsigned long o;
    rsec = sec.rsec;
    if (!rsec)
...
#[no_mangle]
unsafe extern "C" fn is_dwarf_section(sec: *mut section) -> bool {
    static bool is_dwarf_section(struct section *sec)
    {
    return !strncmp(sec.name, ".debug_", 7);
    }
#[no_mangle]
unsafe extern "C" fn read_sections(elf: *mut elf) -> c_int {
    static int read_sections(struct elf *elf)
    {
    Elf_Scn *s = core::ptr::null_mut();
    struct section *sec;
    ...
    sec.data = elf_getdata(s, core::ptr::null_mut());
    if (!sec.data) {
...
//
#[no_mangle]
unsafe extern "C" fn demangled_name_len(name: *const c_char) -> isize {
    static ssize_t demangled_name_len(const char *name)
    {
    ssize_t idx;
    const char *p;
...
#[no_mangle]
unsafe extern "C" fn elf_add_symbol(elf: *mut elf, sym: *mut symbol) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn read_symbols(elf: *mut elf) -> c_int {
    static int read_symbols(struct elf *elf)
    {
    struct section *symtab, *symtab_shndx, *sec;
    struct symbol *sym, *pfunc, *file = core::ptr::null_mut();
    int symbols_nr, i;
    char *coldstr;
    Elf_Data *shndx_data = core::ptr::null_mut();
    Elf32_Word shndx;
...
//
    ...
    file = sym;
#[no_mangle]
pub unsafe extern "C" fn if(!is_sec_sym(sym): sym->bind == STB_LOCAL &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn mark_group_syms(elf: *mut elf) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn elf_update_sym_relocs(elf: *mut elf, sym: *mut symbol) -> c_int {
...
//
    ...
    {
    Elf32_Word shndx;
    Elf_Data *symtab_data = core::ptr::null_mut(), *shndx_data = core::ptr::null_mut();
    let mut entsize: Elf64_Xword = symtab.sh.sh_entsize;
    int max_idx, idx = sym.idx;
    Elf_Scn *s, *t = core::ptr::null_mut();
...
//
    let mut num: c_int = max(1U, sym.idx/3);
    void *buf;
    if (idx) {
...
// setup extended section index magic and write the symbol
    ...
    if (type != STT_SECTION) {
    sym.sym.st_name = elf_add_string(elf, core::ptr::null_mut(), sym.name);
    ...
    sym.sec = find_section_by_index(elf, 0);
    if (!sym.sec) {
    ERROR("no core::ptr::null_mut() section");
    return core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn read_relocs(elf: *mut elf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mark_rodata(elf: *mut elf) {
...
pub unsafe extern "C" fn if(O_RDWR: (flags & O_ACCMODE) ==) -> else {
    ...
    cmd = ELF_C_WRITE;
    elf.elf = elf_begin(elf.fd, cmd, core::ptr::null_mut());
    if (!elf.elf) {
    ERROR_ELF("elf_begin");
    goto err;
    }
    ...
    elf.elf = elf_begin(elf.fd, ELF_C_WRITE, core::ptr::null_mut());
    if (!elf.elf) {
    ERROR_ELF("elf_begin");
    return core::ptr::null_mut();
    }
    ...
    return core::ptr::null_mut();
    null		= elf_create_section(elf, core::ptr::null_mut(), 0, 0, SHT_NULL, 0, 0);
    shstrtab	= elf_create_section(elf, core::ptr::null_mut(), 0, 0, SHT_STRTAB, 1, 0);
    strtab		= elf_create_section(elf, core::ptr::null_mut(), 0, 0, SHT_STRTAB, 1, 0);
...
#[no_mangle]
pub unsafe extern "C" fn elf_add_string(elf: *mut elf, strtab: *mut section, str: *const c_char) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn elf_alloc_reloc(elf: *mut elf, rsec: *mut section) -> c_int {
    static int elf_alloc_reloc(struct elf *elf, struct section *rsec)
    {
    struct reloc *old_relocs, *old_relocs_end, *new_relocs;
    let mut nr_relocs_old: c_uint = sec_num_entries(rsec);
    let mut nr_relocs_new: c_uint = nr_relocs_old + 1;
    ...
    rsec.data.d_buf = core::ptr::null_mut();
    }
...
//
    ...
    let mut key: u32 = reloc_hash(old);
...
//
#[no_mangle]
unsafe extern "C" fn elf_truncate_section(elf: *mut elf, sec: *mut section) -> c_int {
    static int elf_truncate_section(struct elf *elf, struct section *sec)
    {
    let mut size: u64 = sec_size(sec);
    let mut truncated: bool = false;
    Elf_Data *data = core::ptr::null_mut();
    Elf_Scn *s;
    s = elf_getscn(elf.elf, sec.idx);
    if (!s) {
    ERROR_ELF("elf_getscn");
