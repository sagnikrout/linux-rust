...
//! Source: tools/perf/bench/evlist-open-close.c
#![no_std]
...
pub const MMAP_FLUSH_DEFAULT: c_int = 1;
    let mut iterations: static int = 100;
    let mut nr_events: static int = 1;
    static const char *event_string = "dummy";
#[no_mangle]
pub unsafe extern "C" fn timeval2usec(tv: *mut timeval) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn evlist__count_evsel_fds(evlist: *mut evlist) -> c_int {
    ...
    parse_events_error__exit(&err);
    if (uid_str) {
    let mut uid: uid_t = parse_uid(uid_str);
    if (uid == UINT_MAX) {
...
#[no_mangle]
unsafe extern "C" fn bench__do_evlist_open_close(evlist: *mut evlist) -> c_int {
    static int bench__do_evlist_open_close(struct evlist *evlist)
    {
    char sbuf[STRERR_BUFSIZE];
    let mut err: c_int = evlist__open(evlist);
    if (err < 0) {
...
#[no_mangle]
unsafe extern "C" fn bench_evlist_open_close__run(evstr: *mut c_char, uid_str: *const c_char) -> c_int {
...
// used to print statistics only
    ...
    return strbuf_detach(&buf, core::ptr::null_mut());
    out_error:
    strbuf_release(&buf);
    return core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn bench_evlist_open_close(argc: c_int, argv: *const c_char) -> c_int {
    int bench_evlist_open_close(int argc, const char **argv)
    {
    const char *uid_str = core::ptr::null_mut();
