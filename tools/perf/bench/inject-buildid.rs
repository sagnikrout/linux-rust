...
//! Source: tools/perf/bench/inject-buildid.c
#![no_std]
...
pub const MMAP_DEV_MAJOR: c_int = 8;
pub const DSO_MMAP_RATIO: c_int = 4;
    let mut iterations: static unsigned int = 100;
    let mut nr_mmaps: static unsigned int = 100;
    static unsigned int nr_samples = 100;  /* samples per mmap */
    static u64 bench_sample_type;
    static u16 bench_id_hdr_size;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bench_data {
    pub pid: c_int,
    pub input_pipe: [c_int; 2],
    pub output_pipe: [c_int; 2],
    pub th: pthread_t,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct bench_dso {
    pub list: list_head,
    pub name: *mut c_char,
    pub ino: c_int,
}
...
//
    ...
    return 0;
    dso.name = realpath(fpath, core::ptr::null_mut());
    if (dso.name == core::ptr::null_mut())
    return -1;
    dso.ino = nr_dsos++;
...
#[no_mangle]
unsafe extern "C" fn release_dso() {
...
#[no_mangle]
unsafe extern "C" fn dso_map_addr(dso: *mut bench_dso) -> u64 {
    static u64 dso_map_addr(struct bench_dso *dso)
    {
    return 0x400000ULL + dso.ino * 8192ULL;
    }
#[no_mangle]
unsafe extern "C" fn synthesize_attr(data: *mut bench_data) -> isize {
...
#[no_mangle]
unsafe extern "C" fn synthesize_fork(data: *mut bench_data) -> isize {
...
#[no_mangle]
unsafe extern "C" fn synthesize_mmap(data: *mut bench_data, dso: *mut bench_dso, timestamp: u64) -> isize {
    static ssize_t synthesize_mmap(struct bench_data *data, struct bench_dso *dso, u64 timestamp)
    {
    union perf_event event;
    let mut len: usize = offsetof(struct perf_record_mmap2, filename);
...
#[no_mangle]
unsafe extern "C" fn synthesize_sample(data: *mut bench_data, dso: *mut bench_dso, timestamp: u64) -> isize {
...
#[no_mangle]
unsafe extern "C" fn synthesize_flush(data: *mut bench_data) -> isize {
...
#[no_mangle]
unsafe extern "C" fn setup_injection(data: *mut bench_data, build_id_all: bool) -> c_int {
    ...
    if (inject_argv == core::ptr::null_mut())
    exit(1);
...
// signal that we're ready to go
    ...
    pthread_create(&data.th, core::ptr::null_mut(), data_reader, data);
    close(ready_pipe[1]);
    close(data.input_pipe[0]);
    close(data.output_pipe[1]);
...
#[no_mangle]
unsafe extern "C" fn inject_build_id(data: *mut bench_data, max_rss: *mut u64) -> c_int {
...
// this makes the child to run
    ...
    let mut idx: c_int = rand() % nr_dsos;
    struct bench_dso *dso = &dsos[idx];
    let mut timestamp: u64 = rand() % 1000000;
...
#[no_mangle]
unsafe extern "C" fn do_inject_loop(data: *mut bench_data, build_id_all: bool) {
    ...
    pthread_join(data.th, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn do_inject_loops(data: *mut bench_data) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn bench_inject_build_id(argc: c_int, argv: *const c_char) -> c_int {
