...
//! Source: tools/perf/ui/gtk/hists.c
#![no_std]
...
pub const MAX_COLUMNS: c_int = 32;
#[no_mangle]
unsafe extern "C" fn __percent_color_snprintf(hpp: *mut perf_hpp, fmt: *const c_char, ...) -> c_int {
...
pub unsafe extern "C" fn perf_gtk__init_hpp() {
    ...
    {
    struct rb_node *nd;
    let mut has_single_node: bool = (rb_first(root) == rb_last(root));
...
//
    ...
    GtkTreeIter iter;
    char buf[64];
    char *str, *str_alloc = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(CHAIN_FOLDED: callchain_param.mode ==) -> else {
    ...
    {
    let mut expanded: bool = gtk_tree_view_row_expanded(view, path);
    if (expanded)
...
//
    ...
    GtkTreeIter iter;
    let mut total: u64 = hists__total_period(h.hists);
    float percent;
    if (h.filtered)
    continue;
    ...
    if (percent < min_pcnt)
    continue;
    gtk_tree_store_append(store, &iter, core::ptr::null_mut());
    col_idx = 0;
    hists__for_each_format(hists, fmt) {
    ...
    G_CALLBACK(on_row_activated), core::ptr::null_mut());
...
// construct merged column header since sort keys share single column
    ...
    strcat(buf, "+");
    first_col = false;
    fmt.header(fmt, &hpp, hists, 0, core::ptr::null_mut());
    ...
    core::ptr::null_mut(), &hpp, min_pcnt);
    gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(view), TRUE);
    g_signal_connect(view, "row-activated",
    G_CALLBACK(on_row_activated), core::ptr::null_mut());
    gtk_container_add(GTK_CONTAINER(window), view);
    ...
    g_signal_connect(window, "delete_event", gtk_main_quit, core::ptr::null_mut());
