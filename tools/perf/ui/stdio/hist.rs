...
//! Source: tools/perf/ui/stdio/hist.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn callchain__fprintf_left_margin(fp: *mut FILE, left_margin: c_int) -> usize {
    static size_t callchain__fprintf_left_margin(FILE *fp, int left_margin)
    {
    int i;
    let mut ret: c_int = fprintf(fp, "            ");
    ...
    {
    int i;
    let mut ret: usize = callchain__fprintf_left_margin(fp, left_margin);
    for (i = 0; i < depth; i++)
    if (depth_mask & (1 << i))
    ret += fprintf(fp, "|          ");
    else
    ...
    int i;
    let mut ret: usize = 0;
    char bf[1024], *alloc_str = core::ptr::null_mut();
    char buf[64];
    const char *str;
    ...
    callchain_list_counts__printf_value(chain, core::ptr::null_mut(),
    buf, sizeof(buf));
...
#[no_mangle]
unsafe extern "C" fn init_rem_hits() {
    ...
    struct callchain_node *child = core::ptr::null_mut();
    struct callchain_list *chain;
    let mut new_depth_mask: c_int = depth_mask;
    u64 remaining;
    let mut ret: usize = 0;
    int i;
...
//
#[no_mangle]
unsafe extern "C" fn need_percent_display(node: *mut rb_node, parent_samples: u64) -> bool {
...
// displayed twice.
//
    if (!i++ && field_order == core::ptr::null_mut() &&
...
#[no_mangle]
unsafe extern "C" fn __callchain__fprintf_folded(fp: *mut FILE, node: *mut callchain_node) -> usize {
    ...
    FILE *fp)
    {
    let mut parent_samples: u64 = he.stat.period;
...
#[no_mangle]
unsafe extern "C" fn hist_entry__snprintf(he: *mut hist_entry, hpp: *mut perf_hpp) -> c_int {
...
//
    ...
    let mut total_period: u64 = hists.stats.total_period;
    if (size == 0 || size > bfsz)
    size = hpp.size = bfsz;
    ...
    {
    int width;
    if (sep != core::ptr::null_mut() || indent < 2)
    return 0;
    ...
    int indent;
    int depth;
    let mut width: unsigned = 0;
    let mut header_width: unsigned = 0;
    struct perf_hpp_fmt *fmt;
    struct perf_hpp_list_node *fmt_node;
...
// preserve max indent depth for column headers
    ...
    perf_hpp_list__for_each_format(&fmt_node.hpp, fmt) {
    fmt.header(fmt, hpp, hists, line, core::ptr::null_mut());
...
//
    ...
    let mut depth: c_int = hists.nr_hpp_node + h.depth + 1;
    print_hierarchy_indent(sep, depth, " ", fp);
    fprintf(fp, "%*sno entry >= %.2f%%\n", indent, "", min_pcnt);
    if (max_rows && ++nr_rows >= max_rows)
    break;
    }
    if (h.ms.map == core::ptr::null_mut() && verbose > 1) {
...
#[no_mangle]
pub unsafe extern "C" fn events_stats__fprintf(stats: *mut events_stats, fp: *mut FILE) -> usize {
    size_t events_stats__fprintf(struct events_stats *stats, FILE *fp)
    {
    int i;
    let mut ret: usize = 0;
    let mut total: u32 = stats.nr_events[0];
