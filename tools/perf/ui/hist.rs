...
// hist period print (hpp) functions
    ...
    let mut avg: double = nr_samples ? (1.0 * val / nr_samples) : 0;
...
pub struct hpp_fmt_value {
    ...
    struct evsel *pos;
    char *buf = hpp.buf;
    let mut size: usize = hpp.size;
    let mut i: c_int = 0, nr_members = 1;
...
//
    ...
    {
    let mut len: c_int = max(fmt.user_len ?: fmt.len, (int)strlen(fmt.name));
    ...
    let mut len: c_int = fmt.user_len ?: fmt.len;
    ...
    assert(total != 0);
    for (int i = 0; i < MEM_STAT_LEN; i++) {
    let mut val: u64 = he.mem_stat[mem_stat_idx].entries[i];
...
//
    ...
    if (ret)
    return ret;
    if ((a.thread == core::ptr::null_mut() ? core::ptr::null_mut() : RC_CHK_ACCESS(a.thread)) !=
    (b.thread == core::ptr::null_mut() ? core::ptr::null_mut() : RC_CHK_ACCESS(b.thread)) ||
...
#[no_mangle]
unsafe extern "C" fn hpp__mem_stat_type(fmt: *mut perf_hpp_fmt) -> enum mem_stat_type {
    ...
    {
    let mut len: c_int = hpp__width_fn(fmt, hpp, hists);
    const char *hdr = "";
    ...
    let mut ret: c_int = 0;
    int len;
    let mut mst: enum mem_stat_type = hpp__mem_stat_type(fmt);
    let mut mem_stat_idx: c_int = -1;
    for (int i = 0; i < hists.nr_mem_stats; i++) {
    if (hists.mem_stat_types[i] == mst) {
    mem_stat_idx = i;
...
#[no_mangle]
pub unsafe extern "C" fn hpp_color_scnprintf(hpp: *mut perf_hpp, fmt: *const c_char, ...) -> c_int {
    int hpp_color_scnprintf(struct perf_hpp *hpp, const char *fmt, ...)
    {
    va_list args;
    let mut ssize: isize = hpp.size;
    double percent;
    int ret, len;
    va_start(args, fmt);
    len = va_arg(args, int);
...
#[no_mangle]
unsafe extern "C" fn hpp_entry_scnprintf(hpp: *mut perf_hpp, fmt: *const c_char, ...) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_hpp__is_mem_stat_entry(fmt: *mut perf_hpp_fmt) -> bool {
    static bool perf_hpp__is_mem_stat_entry(struct perf_hpp_fmt *fmt)
    {
    return fmt.sort == hpp__sort_mem_stat;
    }
#[no_mangle]
unsafe extern "C" fn perf_hpp__is_hpp_entry(a: *mut perf_hpp_fmt) -> bool {
    static bool perf_hpp__is_hpp_entry(struct perf_hpp_fmt *a)
    {
    return a.header == hpp__header_fn;
    }
#[no_mangle]
unsafe extern "C" fn hpp__equal(a: *mut perf_hpp_fmt, b: *mut perf_hpp_fmt) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hpp__equal_mem_stat(a: *mut perf_hpp_fmt, b: *mut perf_hpp_fmt) -> bool {
...
// sort_list may be linked by setup_sorting()
    if (fmt.sort_list.next == core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn perf_hpp__cancel_cumulate(evlist: *mut evlist) {
...
#[no_mangle]
pub unsafe extern "C" fn perf_hpp__cancel_latency(evlist: *mut evlist) {
...
#[no_mangle]
pub unsafe extern "C" fn perf_hpp__setup_output_field(list: *mut perf_hpp_list) {
...
#[no_mangle]
pub unsafe extern "C" fn perf_hpp__append_sort_keys(list: *mut perf_hpp_list) {
...
#[no_mangle]
pub unsafe extern "C" fn perf_hpp__reset_output_field(list: *mut perf_hpp_list) {
...
#[no_mangle]
pub unsafe extern "C" fn hists__overhead_width(hists: *mut hists) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn hists__reset_column_width(hists: *mut hists) {
...
pub unsafe extern "C" fn perf_hpp__set_user_width(width_list_str: *const c_char) {
    ...
    char *p;
    let mut len: c_int = strtol(ptr, &p, 10);
    fmt.user_len = len;
    if (*p == ',')
    ptr = p + 1;
    else
...
unsafe extern "C" fn add_hierarchy_fmt(hists: *mut hists, fmt: *mut perf_hpp_fmt) -> c_int {
    ...
    {
    struct perf_hpp_list_node *node = core::ptr::null_mut();
    struct perf_hpp_fmt *fmt_copy;
    let mut found: bool = false;
    let mut skip: bool = perf_hpp__should_skip(fmt, hists);
    list_for_each_entry(node, &hists.hpp_formats, list) {
...
#[no_mangle]
pub unsafe extern "C" fn perf_hpp__alloc_mem_stats(list: *mut perf_hpp_list, evlist: *mut evlist) -> c_int {
    int perf_hpp__alloc_mem_stats(struct perf_hpp_list *list, struct evlist *evlist)
    {
    struct perf_hpp_fmt *fmt;
    struct evsel *evsel;
    enum mem_stat_type mst[16];
    let mut nr_mem_stats: unsigned = 0;
    perf_hpp_list__for_each_format(list, fmt) {
    if (!perf_hpp__is_mem_stat_entry(fmt))
    continue;
    ...
    if (hists.mem_stat_types == core::ptr::null_mut())
    return -ENOMEM;
    hists.mem_stat_total = calloc(nr_mem_stats,
    sizeof(*hists.mem_stat_total));
    if (hists.mem_stat_total == core::ptr::null_mut())
    return -ENOMEM;
