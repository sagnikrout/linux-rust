...
//! Source: tools/perf/ui/browsers/annotate.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn type_hash(key: c_long, __maybe_unused: *mut *mut void ctx) -> usize {
    static size_t type_hash(long key, void *ctx __maybe_unused)
    {
    return key;
    }
#[no_mangle]
unsafe extern "C" fn type_equal(key1: c_long, key2: c_long, __maybe_unused: *mut *mut void ctx) -> bool {
...
#[no_mangle]
unsafe extern "C" fn disasm_line__filter(__maybe_unused: *mut *mut ui_browser browser, entry: *mut c_void) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ui_browser__jumps_percent_color(browser: *mut ui_browser, nr: c_int, current: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ui_browser__set_jumps_percent_color(browser: *mut c_void, nr: c_int, current: bool) -> c_int {
    static int ui_browser__set_jumps_percent_color(void *browser, int nr, bool current)
    {
    let mut color: c_int = ui_browser__jumps_percent_color(browser, nr, current);
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__set_percent_color(browser: *mut c_void, percent: double, current: bool) {
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__printf(browser: *mut c_void, fmt: *const c_char, ...) {
...
unsafe extern "C" fn annotate_browser__write(browser: *mut ui_browser, entry: *mut c_void, row: c_int) {
    ...
    struct annotation *notes = browser__annotation(browser);
    struct annotation_line *al = list_entry(entry, struct annotation_line, node);
    let mut is_current_entry: bool = ui_browser__is_current_entry(browser, row);
...
#[no_mangle]
unsafe extern "C" fn is_fused(ab: *mut annotate_browser, cursor: *mut disasm_line) -> c_int {
    static int is_fused(struct annotate_browser *ab, struct disasm_line *cursor)
    {
    struct disasm_line *pos = list_prev_entry(cursor, al.node);
    const char *name;
    let mut diff: c_int = 1;
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__draw_current_jump(browser: *mut ui_browser) {
    ...
    let mut pcnt_width: u8 = annotation__pcnt_width(notes);
    let mut cntr_width: u8 = annotation__br_cntr_width();
    int width;
...
//
    ...
    ui_helpline__printf("WARN: jump target inconsistency, press 'o', notes.offsets[%#x] = core::ptr::null_mut()\n",
...
unsafe extern "C" fn annotate_browser__refresh(browser: *mut ui_browser) -> c_uint {
    ...
    {
    struct annotation *notes = browser__annotation(browser);
    let mut ret: c_int = ui_browser__list_head_refresh(browser);
    let mut pcnt_width: c_int = annotation__pcnt_width(notes);
    ...
    struct rb_node *nd)
    {
    let mut pos: *mut annotation_line = rb_entry(nd, struct annotation_line, rb_node);
    let mut idx: u32 = pos.idx;
    if (annotate_opts.hide_src_code)
    idx = pos.idx_asm;
    ...
    let mut max_percent: double = 0.0;
    int i;
    if (pos.al.offset == -1) {
...
#[no_mangle]
unsafe extern "C" fn annotation__has_source(notes: *mut annotation) -> bool {
    static bool annotation__has_source(struct annotation *notes)
    {
    struct annotation_line *al;
    let mut found_asm: bool = false;
...
// After assembly lines, any line without offset means source.
    ...
    struct annotation_line *al;
    let mut offset: off_t = browser.b.index - browser.b.top_idx;
...
//
    ...
    struct map_symbol *ms = browser.b.priv;
    let mut orig_idx_asm: c_int = al.idx_asm;
...
// should be after annotated_source__purge()
    ...
    if (unlikely(al == core::ptr::null_mut())) {
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__show_full_location(browser: *mut ui_browser) {
...
#[no_mangle]
pub unsafe extern "C" fn if(NULL: al->fileloc ==) -> else {
    else if (al.fileloc == core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn ui_browser__init_asm_mode(browser: *mut ui_browser) {
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__show_function_title(browser: *mut annotate_browser) {
...
// idx = browser->b.index;
    ...
    continue;
    ++*idx;
    if (al.line && strstr(al.line, s) != core::ptr::null_mut())
    return al;
    }
    return core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn __annotate_browser__search(browser: *mut annotate_browser) -> bool {
    static bool __annotate_browser__search(struct annotate_browser *browser)
    {
    struct annotation_line *al;
    s64 idx;
    al = annotate_browser__find_string(browser, browser.search_bf, &idx);
    if (al == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn __annotate_browser__search_reverse(browser: *mut annotate_browser) -> bool {
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__search(browser: *mut annotate_browser, delay_secs: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__show(browser: *mut annotate_browser, title: *mut c_char, help: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn annotate__scnprintf_title(hists: *mut hists, bf: *mut c_char, size: usize) -> c_int {
    static int annotate__scnprintf_title(struct hists *hists, char *bf, size_t size)
    {
    let mut printed: c_int = hists__scnprintf_title(hists, bf, size);
    if (!annotate_opts.hide_src_code_on_title) {
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__debuginfo_warning(browser: *mut annotate_browser) {
    static void annotate_browser__debuginfo_warning(struct annotate_browser *browser)
    {
    struct map_symbol *ms = browser.b.priv;
    struct dso *dso = map__dso(ms.map);
    if (browser.dbg == core::ptr::null_mut() && annotate_opts.code_with_type &&
...
#[no_mangle]
unsafe extern "C" fn annotate_browser__curr_hot_offset(browser: *mut annotate_browser) -> i64 {
    static s64 annotate_browser__curr_hot_offset(struct annotate_browser *browser)
    {
    struct annotation_line *al = core::ptr::null_mut();
    if (browser.curr_hot)
    al = rb_entry(browser.curr_hot, struct annotation_line, rb_node);
    return al ? al.offset : 0;
    }
#[no_mangle]
unsafe extern "C" fn annotate_browser__symbol_annotate_error(browser: *mut annotate_browser, err: c_int) {
    ...
    struct hist_browser_timer *hbt)
    {
    struct rb_node *nd = core::ptr::null_mut();
    ...
    char *br_cntr_text = core::ptr::null_mut();
    char title[256];
    int key;
    ...
    if (browser.selection != core::ptr::null_mut()) {
...
//
    if (nd != core::ptr::null_mut() && RB_EMPTY_NODE(nd))
    nd = core::ptr::null_mut();
    }
    switch (key) {
    case K_TIMER:
    if (hbt)
    ...
    continue;
    case 'r':
    script_browse(core::ptr::null_mut(), core::ptr::null_mut());
    annotate_browser__show(browser, title, help);
    continue;
    case 'k':
    ...
    case 's': {
    struct annotation_line *al = core::ptr::null_mut();
    let mut offset: i64 = annotate_browser__curr_hot_offset(browser);
...
// Try to find the same asm line as before
    ...
    browser.curr_hot = al ? &al.rb_node : core::ptr::null_mut();
    nd = browser.curr_hot;
    annotate__scnprintf_title(hists, title, sizeof(title));
    annotate_browser__show(browser, title, help);
    continue;
...
#[no_mangle]
pub unsafe extern "C" fn if(-1: browser->selection->offset ==) -> else {
    else if (browser.selection.offset == -1)
    ui_helpline__puts("Actions are only available for assembly lines.");
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !dl->ins.ops) -> else {
    else if (!dl.ins.ops)
    goto show_sup_ins;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ins__is_ret(&dl->ins)) -> else {
    ...
    if (browser.dbg == core::ptr::null_mut())
    browser.dbg = dso__debuginfo(map__dso(ms.map));
    if (browser.type_hash == core::ptr::null_mut()) {
...
// reset abort key so that it can get Ctrl-C as a key
    ...
    let mut not_annotated: c_int = list_empty(&notes.src.source);
...
// Copy necessary information when it's called from perf top
    if (hbt != core::ptr::null_mut() && he != &annotate_he) {
    ...
    browser.type_hash = hashmap__new(type_hash, type_equal, /*ctx=*/core::ptr::null_mut());
    }
