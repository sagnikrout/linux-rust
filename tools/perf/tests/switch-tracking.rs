...
//! Source: tools/perf/tests/switch-tracking.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn spin_sleep() -> c_int {
    ...
    err = gettimeofday(&start, core::ptr::null_mut());
    if (err)
    return err;
// Spin for 50ms
    while (1) {
    for (i = 0; i < 1000; i++)
    barrier();
    err = gettimeofday(&now, core::ptr::null_mut());
    if (err)
    return err;
    ...
    ts.tv_sec = 0;
// Sleep for 50ms
    err = nanosleep(&ts, core::ptr::null_mut());
    if (err == EINTR)
    err = 0;
    return err;
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct switch_tracking {
    pub switch_evsel: *mut evsel,
    pub cycles_evsel: *mut evsel,
    pub tids: *mut pid_t,
    pub nr_tids: c_int,
    pub comm_seen: [c_int; 4],
    pub cycles_before_comm_1: c_int,
    pub cycles_between_comm_2_and_comm_3: c_int,
    pub cycles_after_comm_4: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn check_cpu(switch_tracking: *mut switch_tracking, cpu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_event_nodes(events: *mut list_head) {
...
unsafe extern "C" fn compar(a: *const c_void, b: *const c_void) -> c_int {
    ...
    let mut cmp: i64 = nodea.event_time - nodeb.event_time;
...
//
#[no_mangle]
unsafe extern "C" fn test__switch_tracking(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
    static int test__switch_tracking(struct test_suite *test __maybe_unused, int subtest __maybe_unused)
    {
    const char *sched_switch = "sched:sched_switch";
    const char *cycles = "cpu-cycles:u";
    let mut switch_tracking: switch_tracking = { .tids = core::ptr::null_mut(), };
