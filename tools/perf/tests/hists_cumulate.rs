...
//! Source: tools/perf/tests/hists_cumulate.c
#![no_std]
...
unsafe extern "C" fn add_hist_entries(hists: *mut hists, machine: *mut machine) -> c_int {
    ...
    let mut sample: perf_sample = { .evsel = evsel, .period = 1000, };
    size_t i;
...
//
    ...
    evsel__output_resort(hists_to_evsel(hists), core::ptr::null_mut());
    if (verbose > 2) {
...
//
    ...
    if (err < 0)
    goto out;
    err = do_test(hists, expected, ARRAY_SIZE(expected), core::ptr::null_mut(), 0);
    out:
...
#[no_mangle]
unsafe extern "C" fn test__hists_cumulate(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
