...
//! Source: tools/perf/tests/sample-parsing.c
#![no_std]
...
//
pub const BS_EXPECTED_BE: c_uint = 0xa000d00000000000;
pub const BS_EXPECTED_LE: c_uint = 0x1aa00000000;
...
// PERF_FORMAT_ID is forced for PERF_SAMPLE_READ
    ...
    let mut sz: usize = hweight_long(s1_regs.mask) * sizeof(u64);
    COMP(user_regs.mask);
    COMP(user_regs.abi);
    if (s1_regs.abi &&
...
#[no_mangle]
unsafe extern "C" fn do_test(sample_type: u64, sample_regs: u64, read_format: u64) -> c_int {
...
// (i + (u8 *)regs) = i & 0xfe;
    if (read_format & PERF_FORMAT_GROUP) {
    let mut vsz: usize = sample_read_value_size(read_format);
//
...
//
#[no_mangle]
unsafe extern "C" fn test__sample_parsing(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
