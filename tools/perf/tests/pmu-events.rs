...
//! Source: tools/perf/tests/pmu-events.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_pmu_test_event {
...
// be set in the alias.
//
    pub alias_long_desc: *const c_char,
// PMU which we should match against
    pub matching_pmu: *const c_char,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_pmu_test_pmu {
    pub pmu_name: *const c_char,
    pub pmu_is_uncore: bool,
    pub pmu_id: *const c_char,
    pub aliases: [*const perf_pmu_test_event; 10],
}
...
#[no_mangle]
unsafe extern "C" fn is_same(reference: *const c_char, test: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn compare_pmu_events(e1: *const pmu_event, e2: *const pmu_event) -> c_int {
...
// Verify generated events from pmu-events.c are as expected
    ...
    let mut map_events: c_int = 0, expected_events, err;
// ignore 3x sentinels
    ...
    return -1;
    err = pmu_events_table__for_each_event(table, /*pmu=*/ core::ptr::null_mut(),
    test__pmu_event_table_core_callback,
    &map_events);
    if (err)
    return err;
    err = pmu_events_table__for_each_event(sys_event_table, /*pmu=*/ core::ptr::null_mut(),
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_core_pmu_event_aliases_cb_args {
    pub test_event: *const perf_pmu_test_event,
    pub count: *mut c_int,
}
...
#[no_mangle]
unsafe extern "C" fn __test_core_pmu_event_aliases(pmu_name: *const c_char, count: *mut c_int) -> c_int {
    ...
    res = pmu_events_table__find_event(table, pmu, "bp_l1_btb_correct", core::ptr::null_mut(), core::ptr::null_mut());
    if (res != 0) {
    pr_debug("Missing test event in test architecture");
    return res;
    }
    for (; *test_event_table; test_event_table++) {
    let mut test_event: perf_pmu_test_event = **test_event_table;
    struct pmu_event const *event = &test_event.event;
...
#[no_mangle]
unsafe extern "C" fn __test_uncore_pmu_event_aliases(test_pmu: *mut perf_pmu_test_pmu) -> c_int {
    static int __test_uncore_pmu_event_aliases(struct perf_pmu_test_pmu *test_pmu)
    {
    let mut alias_count: c_int = 0, to_match_count = 0, matched_count = 0;
...
// Count how many aliases we expect from the known table
    ...
    for (table = &test_pmu.aliases[0]; *table; table++) {
    let mut test_event: perf_pmu_test_event = **table;
    struct pmu_event const *event = &test_event.event;
    int err;
    struct test_core_pmu_event_aliases_cb_args args = {
    .test_event = &test_event,
...
#[no_mangle]
unsafe extern "C" fn is_number(str: *const c_char) -> bool {
    ...
    let mut errno: return = = 0 && end_ptr != str;
    }
#[no_mangle]
unsafe extern "C" fn check_parse_id(id: *const c_char, error: *mut parse_events_error) -> c_int {
...
// cur = '/';
    ret = __parse_events(evlist, dup, /*pmu_filter=*/core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn check_parse_fake(id: *const c_char) -> c_int {
...
pub struct metric {
    pub list: list_head,
    pub metric_ref: metric_ref,
}

#[no_mangle]
unsafe extern "C" fn is_expected_broken_metric(pm: *const pmu_metric) -> bool {
...
// cputype_filter=*/false);
    ...
    err = evlist__alloc_stats(/*config=*/core::ptr::null_mut(), evlist, /*alloc_raw=*/false);
    if (err)
    goto out_err;
//
...
unsafe extern "C" fn test__parsing(test: *mut test_suite, subtest: c_int) -> c_int {
    ...
    const struct pmu_metrics_table *table = core::ptr::null_mut();
    if (test.test_cases)
    ...
    let mut failures: return = = 0 ? TEST_OK : TEST_FAIL;
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_metric {
...
#[no_mangle]
unsafe extern "C" fn metric_parse_fake(metric_name: *const c_char, str: *const c_char) -> c_int {
    ...
    if (expr__find_ids(str, core::ptr::null_mut(), ctx) < 0) {
...
unsafe extern "C" fn test__parsing_fake(test: *mut test_suite, subtest: c_int) -> c_int {
    ...
    err = pmu_for_each_core_metric(test__parsing_fake_callback, core::ptr::null_mut());
    if (err)
    return err;
    return pmu_for_each_sys_metric(test__parsing_fake_callback, core::ptr::null_mut());
    }
    ...
    {
    let mut err: c_int = 0;
    err = pmu_for_each_core_metric(test__parsing_threshold_callback, core::ptr::null_mut());
    if (err)
    return err;
    return pmu_for_each_sys_metric(test__parsing_threshold_callback, core::ptr::null_mut());
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct populate_cb_data {
    pub test_cases: *mut test_case,
    pub curr: usize,
}

#[no_mangle]
unsafe extern "C" fn count_metrics_tables_cb(__maybe_unused: *const *const pmu_metrics_table table, data: *mut c_void) -> c_int {
...
unsafe extern "C" fn setup_pmu_events_suite(suite: *mut test_suite) -> c_int {
    ...
    let mut num_fixed_tests: usize = 4;
    let mut tests_per_table: usize = 2;
    size_t total_tests;
