...
//! Source: tools/perf/tests/code-reading.c
#![no_std]
...
pub const BUFSZ: c_int = 1024;
pub const READLEN: c_int = 128;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tested_section {
    pub rb_node: rb_node,
    pub addr: u64,
    pub path: *mut c_char,
}
    ...
    cmp = -1;
#[no_mangle]
pub unsafe extern "C" fn if(data->addr: addr >) -> else {
...
#[no_mangle]
unsafe extern "C" fn tested_sections__free(root: *mut rb_root) {
...
#[no_mangle]
unsafe extern "C" fn read_objdump_output(f: *mut FILE, buf: *mut c_void, len: *mut usize, start_addr: u64) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn objdump_version() -> c_int {
...
// Get first line of objdump --version output
    ...
    token = strsep(&line, " ");
    if (token != core::ptr::null_mut() && !strcmp(token, "GNU")) {
...
// Convert version into a format we can compare with
    ...
    size_t len)
    {
    let mut stop_address: u64 = addr + len;
    ...
    let mut version: c_int = objdump_version();
...
#[no_mangle]
unsafe extern "C" fn dump_buf(buf: *mut c_uchar, len: usize) {
...
// The results should be identical
    ...
    int ret;
    ret = machine__process_event(machine, event, core::ptr::null_mut());
    if (ret < 0)
...
#[no_mangle]
unsafe extern "C" fn do_sort_something() {
...
#[no_mangle]
unsafe extern "C" fn sort_something() {
...
#[no_mangle]
unsafe extern "C" fn syscall_something() {
...
#[no_mangle]
unsafe extern "C" fn fs_something() {
...
#[no_mangle]
unsafe extern "C" fn do_test_code_reading(try_kcore: bool) -> c_int {
    ...
    let mut tested_sections: rb_root = RB_ROOT;
    ...
    struct dso *dso;
    const char *events[] = { "cpu-cycles", "cpu-cycles:u", "cpu-clock", "cpu-clock:u", core::ptr::null_mut() };
    let mut evidx: c_int = 0;
...
// No point getting kernel events if there is no kernel object
    ...
    ret = perf_event__synthesize_thread_map(core::ptr::null_mut(), threads,
    ...
    if (ret < 0) {
    evidx++;
    if (events[evidx] == core::ptr::null_mut() && verbose > 0) {
    char errbuf[512];
    evlist__strerror_open(evlist, errno, errbuf, sizeof(errbuf));
    pr_debug("perf_evlist__open() failed!\n%s\n", errbuf);
    }
    perf_evlist__set_maps(evlist__core(evlist), core::ptr::null_mut(), core::ptr::null_mut());
    evlist__put(evlist);
    ...
    if (events[evidx] == core::ptr::null_mut())
    goto out_put;
...
#[no_mangle]
pub unsafe extern "C" fn if(!try_kcore: !have_vmlinux &&) -> else {
    else if (!have_vmlinux && !try_kcore)
    err = TEST_CODE_READING_NO_VMLINUX;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strstr(events[evidx], _arg: ":u")) -> else {
...
#[no_mangle]
unsafe extern "C" fn test__code_reading(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
