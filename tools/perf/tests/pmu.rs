...
#[no_mangle]
unsafe extern "C" fn test_pmu_put(dir: *const c_char, pmu: *mut perf_pmu) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test__pmu_format(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn permitted_event_name(name: *const c_char) -> bool {
    static bool permitted_event_name(const char *name)
    {
    let mut has_lower: bool = false, has_upper = false;
    __u64 config;
    for (size_t i = 0; i < strlen(name); i++) {
    let mut c: c_char = name[i];
    if (islower(c)) {
    if (has_upper)
    goto check_legacy;
    has_lower = true;
...
#[no_mangle]
unsafe extern "C" fn test__name_len(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test__name_cmp(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn test__pmu_match(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
