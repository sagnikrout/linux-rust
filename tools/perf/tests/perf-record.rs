...
//! Source: tools/perf/tests/perf-record.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn sched__get_first_possible_cpu(pid: pid_t, maskp: *mut cpu_set_t) -> c_int {
    static int sched__get_first_possible_cpu(pid_t pid, cpu_set_t *maskp)
    {
    int i, cpu = -1;
    let mut nrcpus: c_int = cpu__max_cpu().cpu;
    let mut size: usize = CPU_ALLOC_SIZE(nrcpus);
    realloc:
...
#[no_mangle]
unsafe extern "C" fn test__PERF_RECORD(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
    ...
    const char *argv[] = { cmd, "1", core::ptr::null_mut(), };
    ...
    found_ld_mmap = false;
    let mut err: c_int = -1, errs = 0, i, wakeups = 0;
    u32 cpu;
    let mut total_events: c_int = 0, nr_events[PERF_RECORD_MAX] = { 0, };
    ...
    if (evlist == core::ptr::null_mut()) { /* Fallback for kernels lacking PERF_COUNT_SW_DUMMY */
...
//
    err = evlist__prepare_workload(evlist, &opts.target, argv, false, core::ptr::null_mut());
    if (err < 0) {
...
//
    ...
    while (1) {
    let mut before: c_int = total_events;
