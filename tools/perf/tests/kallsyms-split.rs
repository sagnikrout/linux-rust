...
//! Source: tools/perf/tests/kallsyms-split.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn remove_proc_dir(__maybe_unused: int sig) {
    static void remove_proc_dir(int sig __maybe_unused)
    {
    char buf[128];
    if (root_dir == core::ptr::null_mut())
    return;
...
#[no_mangle]
unsafe extern "C" fn create_proc_dir() -> c_int {
...
// force to use /proc/kallsyms
    ...
    if (machine__find_kernel_symbol_by_name(&m, "main_symbol3", &map) == core::ptr::null_mut()) {
