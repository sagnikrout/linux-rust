...
//! Source: tools/perf/tests/mmap-basic.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn test__basic_mmap(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_user_read(pmu: *mut perf_pmu, enabled: enum user_read_state) -> enum user_read_state {
    static enum user_read_state set_user_read(struct perf_pmu *pmu, enum user_read_state enabled)
    {
    char buf[2] = {0, '\n'};
    ssize_t len;
    int events_fd, rdpmc_fd;
    let mut old_user_read: enum user_read_state = USER_READ_UNKNOWN;
...
#[no_mangle]
unsafe extern "C" fn test_stat_user_read(event: u64, enabled: enum user_read_state) -> c_int {
    ...
    let mut saved_user_read_state: enum user_read_state = set_user_read(pmu, enabled);
    ...
    struct perf_evsel *evsel = core::ptr::null_mut();
    int err;
    struct perf_event_mmap_page *pc;
    let mut mapped: bool = false, opened = false, rdpmc_supported;
