...
//! Source: tools/perf/tests/vmlinux-kallsyms.c
#![no_std]
...
// Symbol names that contain the following are ignored.
    ...
    let mut l: c_int = strlen(name) - strlen(*p);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test__vmlinux_matches_kallsyms_cb_args {
    pub kallsyms: machine,
    pub vmlinux_map: *mut map,
    pub header_printed: bool,
}

#[no_mangle]
unsafe extern "C" fn test__vmlinux_matches_kallsyms_cb1(map: *mut map, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test__vmlinux_matches_kallsyms_cb2(map: *mut map, data: *mut c_void) -> c_int {
    static int test__vmlinux_matches_kallsyms_cb2(struct map *map, void *data)
    {
    struct test__vmlinux_matches_kallsyms_cb_args *args = data;
    struct map *pair;
    let mut mem_start: u64 = map__unmap_ip(args.vmlinux_map, map__start(map));
    let mut mem_end: u64 = map__unmap_ip(args.vmlinux_map, map__end(map));
    pair = maps__find(args.kallsyms.kmaps, mem_start);
    if (pair != core::ptr::null_mut() && !map__priv(pair) && map__start(pair) == mem_start) {
    struct dso *dso = map__dso(map);
    if (!args.header_printed) {
...
#[no_mangle]
unsafe extern "C" fn test__vmlinux_matches_kallsyms_cb3(map: *mut map, data: *mut c_void) -> c_int {
...
//
    ...
    first_pair = machine__find_kernel_symbol(&args.kallsyms, mem_start, core::ptr::null_mut());
    pair = first_pair;
...
//
    let mut skew: i64 = mem_end - UM(pair.end);
