...
//! Source: tools/perf/builtin-c2c.c
#![no_std]
...
pub struct perf_c2c {
    pub tool: perf_tool,
    pub hists: c2c_hists,
    pub mem2node: mem2node,
    pub nodes: *mut c_ulong,
    pub nodes_cnt: c_int,
    pub cpus_cnt: c_int,
    pub cpu2node: *mut c_int,
    pub node_info: c_int,
    pub show_src: bool,
    pub show_all: bool,
    pub use_stdio: bool,
...
// Shared cache line stats
    pub shared_clines_stats: c2c_stats,
    pub shared_clines: c_int,
    pub display: c_int,
    pub coalesce: *const c_char,
    pub cl_sort: *mut c_char,
    pub cl_resort: *mut c_char,
    pub cl_output: *mut c_char,
}
...
#[no_mangle]
unsafe extern "C" fn c2c_he_free(he: *mut c_void) {
    ...
    if (ret) {
    c2c_he.hists = core::ptr::null_mut();
    free(hists);
    return core::ptr::null_mut();
    }
    return hists;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: stats->lcl_hitm) -> else {
    else if (stats.lcl_hitm)
    update_stats(&cstats.lcl_hitm, weight);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: stats->rmt_peer) -> else {
    else if (stats.rmt_peer)
    update_stats(&cstats.rmt_peer, weight);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: stats->lcl_peer) -> else {
    else if (stats.lcl_peer)
    update_stats(&cstats.lcl_peer, weight);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: stats->load) -> else {
    else if (stats.load)
...
//
#[no_mangle]
unsafe extern "C" fn perf_c2c__has_annotation(list: *mut perf_hpp_list) -> bool {
    ...
    struct c2c_hist_entry *c2c_he;
    let mut stats: c2c_stats = { .nr_entries = 0, };
    struct hist_entry *he;
    struct addr_location al;
    struct mem_info *mi = core::ptr::null_mut();
    ...
    ret = sample__resolve_callchain(sample, cursor, core::ptr::null_mut(),
    &al, sysctl_perf_event_max_stack);
    if (ret)
    goto out;
    mi = sample__resolve_mem(sample, &al);
    if (mi == core::ptr::null_mut()) {
    ret = -ENOMEM;
    goto out;
    }
    c2c_decode_stats(&stats, mi);
    he = hists__add_entry_ops(&c2c_hists.hists, &c2c_entry_ops,
    &al, core::ptr::null_mut(), core::ptr::null_mut(), mi, core::ptr::null_mut(),
    sample, true);
    ...
    if (perf_c2c__has_annotation(core::ptr::null_mut())) {
...
//
    let mut cpu: c_int = sample.cpu == (unsigned int) -1 ? 0 : sample.cpu;
    int node;
...
unsafe extern "C" fn symbol_width(hists: *mut hists, se: *mut sort_entry) -> c_int {
    ...
    {
    let mut width: c_int = hists__col_len(hists, se.se_width_idx);
    if (!c2c.symbol_full)
    ...
    struct c2c_fmt *c2c_fmt;
    struct c2c_dimension *dim;
    const char *text = core::ptr::null_mut();
    let mut width: c_int = c2c_width(fmt, hpp, hists);
    c2c_fmt = container_of(fmt, struct c2c_fmt, fmt);
    dim = c2c_fmt.dim;
    if (dim.se) {
...
// span = dim->header.line[line].span;
    ...
    {
    let mut addr: u64 = 0;
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    char buf[20];
    if (he.mem_info)
    ...
    STAT_FN(rmt_hit)
#[no_mangle]
unsafe extern "C" fn get_load_llc_misses(stats: *mut c2c_stats) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn get_load_cache_hits(stats: *mut c2c_stats) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn get_stores(stats: *mut c2c_stats) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn total_records(stats: *mut c2c_stats) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn total_loads(stats: *mut c2c_stats) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn percent_costly_snoop(c2c_he: *mut c2c_hist_entry) -> double {
    static double percent_costly_snoop(struct c2c_hist_entry *c2c_he)
    {
    struct c2c_hists *hists;
    struct c2c_stats *stats;
    struct c2c_stats *total;
    let mut tot: c_int = 0, st = 0;
    double p;
...
#[no_mangle]
unsafe extern "C" fn percent(st: u32, tot: u32) -> double {
    ...
    {
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    let mut per: double = PERCENT(he, rmt_hitm);
    char buf[10];
    return scnprintf(hpp.buf, hpp.size, "%*s", width, PERC_STR(buf, per));
    }
    static int
    ...
    {
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    let mut per: double = PERCENT(he, lcl_hitm);
    char buf[10];
    return scnprintf(hpp.buf, hpp.size, "%*s", width, PERC_STR(buf, per));
    }
    static int
    ...
    {
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    let mut per: double = PERCENT(he, lcl_peer);
    char buf[10];
    return scnprintf(hpp.buf, hpp.size, "%*s", width, PERC_STR(buf, per));
    }
    static int
    ...
    {
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    let mut per: double = PERCENT(he, rmt_peer);
    char buf[10];
    return scnprintf(hpp.buf, hpp.size, "%*s", width, PERC_STR(buf, per));
    }
    static int
    ...
    {
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    let mut per: double = PERCENT(he, st_l1hit);
    char buf[10];
    return scnprintf(hpp.buf, hpp.size, "%*s", width, PERC_STR(buf, per));
    }
    static int
    ...
    {
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    let mut per: double = PERCENT(he, st_l1miss);
    char buf[10];
    return scnprintf(hpp.buf, hpp.size, "%*s", width, PERC_STR(buf, per));
    }
    static int
    ...
    {
    let mut width: c_int = c2c_width(fmt, hpp, he.hists);
    let mut per: double = PERCENT(he, st_na);
    char buf[10];
    return scnprintf(hpp.buf, hpp.size, "%*s", width, PERC_STR(buf, per));
    }
    static int
...
#[no_mangle]
unsafe extern "C" fn display_metrics(hpp: *mut perf_hpp, val: u32, sum: u32) -> c_int {
    ...
    case 1:
    {
    let mut num: c_int = bitmap_weight(set, c2c.cpus_cnt);
    ...
    let mut header_offset_tui: static struct c2c_header = HEADER_SPAN("-----", "Off", 2);
    ...
    let mut len: usize = fmt.user_len;
    if (!len) {
    ...
    fmt.color	= dim.se ? core::ptr::null_mut()	 : dim.color;
...
#[no_mangle]
unsafe extern "C" fn c2c_hists__init_sort(hpp_list: *mut perf_hpp_list, name: *mut c_char, env: *mut perf_env) -> c_int {
    ...
    if (!c2c_fmt) {
    reset_dimensions();
    return sort_dimension__add(hpp_list, name, /*evlist=*/core::ptr::null_mut(), env, /*level=*/0);
    }
    dim = c2c_fmt.dim;
    if (dim == &dim_dso)
    hpp_list.dso = 1;
    ...
    if (!_list)
    return 0;
    for (tok = strtok_r(_list, ", ", &tmp); tok; tok = strtok_r(core::ptr::null_mut(), ", ", &tmp)) {
    ...
    struct perf_env *env)
    {
    char *output = output_ ? strdup(output_) : core::ptr::null_mut();
    char *sort   = sort_   ? strdup(sort_) : core::ptr::null_mut();
    int ret;
...
// Overload number of header lines.
    ...
    ret = hpp_list__parse(&hists.list, /*output=*/core::ptr::null_mut(), sort, env);
...
#[no_mangle]
unsafe extern "C" fn filter_display(val: u32, sum: u32) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn he__display(he: *mut hist_entry, stats: *mut c2c_stats) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn is_valid_hist_entry(he: *mut hist_entry) -> bool {
    static inline bool is_valid_hist_entry(struct hist_entry *he)
    {
    struct c2c_hist_entry *c2c_he;
    let mut has_record: bool = false;
    c2c_he = container_of(he, struct c2c_hist_entry, he);
...
#[no_mangle]
unsafe extern "C" fn set_node_width(c2c_he: *mut c2c_hist_entry, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn set_nodestr(c2c_he: *mut c2c_hist_entry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn calc_width(c2c_he: *mut c2c_hist_entry) {
...
#[no_mangle]
unsafe extern "C" fn filter_cb(he: *mut hist_entry, __maybe_unused: *mut *mut void arg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn resort_cl_cb(he: *mut hist_entry, arg: *mut c_void) -> c_int {
    static int resort_cl_cb(struct hist_entry *he, void *arg)
    {
    struct perf_env *env = arg;
    struct c2c_hist_entry *c2c_he;
    struct c2c_hists *c2c_hists;
    let mut display: bool = he__display(he, &c2c.shared_clines_stats);
    int ret;
    c2c_he = container_of(he, struct c2c_hist_entry, he);
    ...
    if (ret)
    return ret;
    hists__collapse_resort(&c2c_hists.hists, core::ptr::null_mut());
    hists__output_resort_cb(&c2c_hists.hists, core::ptr::null_mut(), filter_cb);
    }
    return 0;
    }
    let mut header_node_0: static struct c2c_header = HEADER_LOW("Node");
    ...
    let mut header_node_2: static struct c2c_header = HEADER_LOW("Node{cpu list}");
#[no_mangle]
unsafe extern "C" fn setup_nodes_header() {
...
#[no_mangle]
unsafe extern "C" fn setup_nodes(session: *mut perf_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn resort_shared_cl_cb(he: *mut hist_entry, __maybe_unused: *mut *mut void arg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hists__iterate_cb(hists: *mut hists, cb: hists__resort_cb_t, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn print_c2c__display_stats(out: *mut FILE) {
...
#[no_mangle]
unsafe extern "C" fn print_shared_cacheline_info(out: *mut FILE) {
    static void print_shared_cacheline_info(FILE *out)
    {
    struct c2c_stats *stats = &c2c.shared_clines_stats;
    let mut hitm_cnt: c_int = stats.lcl_hitm + stats.rmt_hitm;
    fprintf(out, "=================================================\n");
...
#[no_mangle]
unsafe extern "C" fn print_pareto(out: *mut FILE, env: *mut perf_env) {
    ...
    ret = hpp_list__parse(&hpp_list, cl_output, /*evlist=*/core::ptr::null_mut(), env);
...
#[no_mangle]
unsafe extern "C" fn print_c2c_info(out: *mut FILE, session: *mut perf_session) {
...
#[no_mangle]
unsafe extern "C" fn perf_c2c__hists_fprintf(out: *mut FILE, session: *mut perf_session) {
...
#[no_mangle]
unsafe extern "C" fn perf_c2c__toggle_annotation(browser: *mut hist_browser) -> c_int {
    static int perf_c2c__toggle_annotation(struct hist_browser *browser)
    {
    struct hist_entry *he = browser.he_selection;
    struct symbol *sym = core::ptr::null_mut();
    struct annotated_source *src = core::ptr::null_mut();
    struct c2c_hist_entry *c2c_he = core::ptr::null_mut();
    let mut al_addr: u64 = NO_ADDR;
    ...
    c2c_he = container_of(he, struct c2c_hist_entry, he);
    return hist_entry__tui_annotate(he, c2c_he.evsel, core::ptr::null_mut(), al_addr);
    }
#[no_mangle]
unsafe extern "C" fn c2c_browser__update_nr_entries(hb: *mut hist_browser) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct c2c_cacheline_browser {
    pub hb: hist_browser,
...
#[no_mangle]
unsafe extern "C" fn perf_c2c__browse_cacheline(he: *mut hist_entry) -> c_int {
...
// Display compact version first.
    ...
    if (cl_browser == core::ptr::null_mut())
    return -1;
...
#[no_mangle]
unsafe extern "C" fn perf_c2c__hists_browse(hists: *mut hists) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_c2c_display(session: *mut perf_session) {
...
#[no_mangle]
unsafe extern "C" fn ui_quirks() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setup_callchain(evlist: *mut evlist) -> c_int {
    static int setup_callchain(struct evlist *evlist)
    {
    let mut sample_type: u64 = evlist__combined_sample_type(evlist);
    let mut mode: enum perf_call_graph_mode = CALLCHAIN_NONE;
    ...
    mode = CALLCHAIN_LBR;
#[no_mangle]
pub unsafe extern "C" fn if(PERF_SAMPLE_CALLCHAIN: sample_type &) -> else {
...
#[no_mangle]
unsafe extern "C" fn setup_display(str: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(display, _arg: "rmt")) -> else {
    else if (!strcmp(display, "rmt"))
    c2c.display = DISPLAY_RMT_HITM;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(display, _arg: "lcl")) -> else {
    else if (!strcmp(display, "lcl"))
    c2c.display = DISPLAY_LCL_HITM;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(display, _arg: "peer")) -> else {
    ...
    __tok = strtok_r(core::ptr::null_mut(),  __sep, &__tmp))
#[no_mangle]
unsafe extern "C" fn build_cl_output(cl_sort: *mut c_char, no_source: bool) -> c_int {
    static int build_cl_output(char *cl_sort, bool no_source)
    {
    char *tok, *tmp, *buf = strdup(cl_sort);
    let mut add_pid: bool = false;
    let mut add_tid: bool = false;
    let mut add_iaddr: bool = false;
    let mut add_sym: bool = false;
    let mut add_dso: bool = false;
    let mut add_src: bool = false;
    let mut ret: c_int = 0;
    if (!buf)
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn setup_coalesce(coalesce: *const c_char, no_source: bool) -> c_int {
    static int setup_coalesce(const char *coalesce, bool no_source)
    {
    const char *c = coalesce ?: coalesce_default;
    const char *sort_str = core::ptr::null_mut();
    ...
    sort_str = "tot_hitm";
#[no_mangle]
pub unsafe extern "C" fn if(DISPLAY_RMT_HITM: c2c.display ==) -> else {
    else if (c2c.display == DISPLAY_RMT_HITM)
    sort_str = "rmt_hitm,lcl_hitm";
#[no_mangle]
pub unsafe extern "C" fn if(DISPLAY_LCL_HITM: c2c.display ==) -> else {
    else if (c2c.display == DISPLAY_LCL_HITM)
    sort_str = "lcl_hitm,rmt_hitm";
#[no_mangle]
pub unsafe extern "C" fn if(DISPLAY_SNP_PEER: c2c.display ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn perf_c2c__report(argc: c_int, argv: *const c_char) -> c_int {
    ...
    const char *display = core::ptr::null_mut();
    const char *coalesce = core::ptr::null_mut();
    let mut no_source: bool = false;
    const char *disassembler_style = core::ptr::null_mut(), *objdump_path = core::ptr::null_mut();
    ...
    let mut err: c_int = 0;
    const char *output_str, *sort_str = core::ptr::null_mut();
    struct perf_env *env;
...
//
    if (!display) {
    if (perf_env__e_machine(env, /*e_flags=*/core::ptr::null_mut()) == EM_AARCH64)
    display = "peer";
    else
    display = "tot";
    }
...
//
    if (perf_c2c__has_annotation(core::ptr::null_mut())) {
    let mut ret: c_int = symbol__annotation_init();
    if (ret < 0)
    goto out_mem2node;
//
...
pub unsafe extern "C" fn if(DISPLAY_SNP_PEER: c2c.display ==) -> else {
    ...
    hists__collapse_resort(&c2c.hists.hists, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn perf_c2c__record(argc: c_int, argv: *const c_char) -> c_int {
    static int perf_c2c__record(int argc, const char **argv)
    {
    int rec_argc, i = 0, j;
    const char **rec_argv;
    char *event_name_storage = core::ptr::null_mut();
    int ret;
    let mut all_user: bool = false, all_kernel = false;
    let mut event_set: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn cmd_c2c(argc: c_int, argv: *const c_char) -> c_int {
