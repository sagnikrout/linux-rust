...
//! Source: tools/perf/builtin-inject.c
#![no_std]
...
pub struct guest_session {
    ...
    pub copy_kcore_dir: bool,
    pub have_tc: bool,
    pub fetched: bool,
    pub ready: bool,
    pub dflt_id_hdr_size: u16,
    pub dflt_id: u64,
    pub highest_id: u64,
// Array of guest_vcpu
...
pub struct perf_inject {
    pub tool: perf_tool,
    pub session: *mut perf_session,
    pub build_id_style: enum build_id_rewrite_style,
    pub sched_stat: bool,
    pub have_auxtrace: bool,
    pub strip: bool,
    pub jit_mode: bool,
    pub in_place_update: bool,
    pub in_place_update_dry_run: bool,
    pub copy_kcore_dir: bool,
    pub convert_callchain: bool,
    pub aslr: bool,
...
pub struct event_entry {
    ...
    const char *filename);
#[no_mangle]
unsafe extern "C" fn output_bytes(inject: *mut perf_inject, buf: *mut c_void, sz: usize) -> c_int {
    ...
    u64 *ids;
    int ret;
    union perf_event *aslr_event = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn copy_bytes(inject: *mut perf_inject, data: *mut perf_data, size: off_t) -> c_int {
    ...
    if (!inject.output.is_pipe) {
    let mut offset: off_t = perf_data__seek(&inject.output, 0, SEEK_CUR);
    if (offset == -1)
    return -errno;
    ...
    struct perf_sample *sample)
    {
    let mut sz1: usize = sample.aux_sample.data - (void *)event - sizeof(u64);
    let mut sz2: usize = event.header.size - sample.aux_sample.size - (sz1 + sizeof(u64));
    union perf_event *ev;
    if (inject.event_copy == core::ptr::null_mut()) {
    ...
    return perf_event__repipe_synth(tool, event);
    if (evsel.handler) {
    let mut f: inject_handler = evsel.handler;
    ...
    int err;
    size_t sz;
    let mut orig_type: u64 = evsel.core.attr.sample_type;
    let mut orig_branch_type: u64 = evsel.core.attr.branch_sample_type;
    struct branch_stack *orig_bs = sample.branch_stack;
    if (event_copy == core::ptr::null_mut()) {
    inject.event_copy = malloc(PERF_SAMPLE_MAX_SIZE);
    if (!inject.event_copy)
    return -ENOMEM;
...
// Temporarily add in type bits for synthesis.
    ...
    let mut sample_type: u64 = evsel.core.attr.sample_type;
    size_t sz;
    u64 i, k;
    int ret;
    if (event_copy == core::ptr::null_mut()) {
...
// remove sample_type {STACK,REGS}_USER for synthesize
    ...
    {
    struct thread *thread;
    struct nsinfo *nsi = core::ptr::null_mut();
    struct nsinfo *nnsi;
    struct dso *dso;
    bool vdso;
...
// flags=*/0, PROT_EXEC,
    event.mmap.filename, /*dso_id=*/core::ptr::null_mut(),
    ...
    struct machine *machine)
    {
    let mut id: dso_id = dso_id_empty;
    if (event.header.misc & PERF_RECORD_MISC_MMAP_BUILD_ID) {
    ...
    struct machine *machine)
    {
    let mut err: c_int = perf_event__process_namespaces(tool, event, sample, machine);
    perf_event__repipe(tool, event, sample, machine);
    return err;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: dso__nsinfo(dso)) -> else {
    ...
    int bid_len;
    known_build_ids = strlist__new(known_build_ids_string, core::ptr::null_mut());
    if (known_build_ids == core::ptr::null_mut())
    return core::ptr::null_mut();
    strlist__for_each_entry_safe(pos, tmp, known_build_ids) {
    const char *build_id, *dso_name;
    build_id = skip_spaces(pos.s);
    dso_name = strchr(build_id, ' ');
    if (dso_name == core::ptr::null_mut()) {
    ...
    return 0;
    if (inject.known_build_ids != core::ptr::null_mut() &&
...
// Return to repipe anonymous maps.
    ...
    {
    struct dso *dso;
    let mut misc: u16 = sample.cpumode;
    if (!map)
    return 0;
    if (!sample_in_dso) {
...
//
    ...
    if (!map__hit(map)) {
    let mut null_bid: build_id = { .size = 0 };
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mark_dso_hit_args {
    pub inject: *const perf_inject,
...
// symbols=*/false, mark_dso_hit_callback, &args);
    ...
    {
    if (realloc_array_as_needed(gs.vcpu, gs.vcpu_cnt, vcpu, core::ptr::null_mut()))
    return core::ptr::null_mut();
    return &gs.vcpu[vcpu];
    }
#[no_mangle]
unsafe extern "C" fn guest_session__output_bytes(gs: *mut guest_session, buf: *mut c_void, sz: usize) -> c_int {
    static int guest_session__output_bytes(struct guest_session *gs, void *buf, size_t sz)
    {
    let mut ret: isize = writen(gs.tmp_fd, buf, sz);
    return ret < 0 ? ret : 0;
    }
...
#[no_mangle]
unsafe extern "C" fn guest_session__map_tid(gs: *mut guest_session, tid: u32, vcpu: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn evlist__is_id_used(evlist: *mut evlist, id: u64) -> bool {
    static bool evlist__is_id_used(struct evlist *evlist, u64 id)
    {
    return evlist__id2sid(evlist, id);
    }
#[no_mangle]
unsafe extern "C" fn guest_session__allocate_new_id(gs: *mut guest_session, host_evlist: *mut evlist) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn guest_session__map_id(gs: *mut guest_session, id: u64, host_id: u64, vcpu: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn evlist__find_highest_id(evlist: *mut evlist) -> u64 {
    static u64 evlist__find_highest_id(struct evlist *evlist)
    {
    struct evsel *evsel;
    let mut highest_id: u64 = 1;
    evlist__for_each_entry(evlist, evsel) {
    u32 j;
    for (j = 0; j < evsel.core.ids; j++) {
    let mut id: u64 = evsel.core.id[j];
    if (id > highest_id)
    highest_id = id;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn guest_session__map_ids(gs: *mut guest_session, host_evlist: *mut evlist) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn guest_session__add_attr(gs: *mut guest_session, evsel: *mut evsel) -> c_int {
    static int guest_session__add_attr(struct guest_session *gs, struct evsel *evsel)
    {
    struct perf_inject *inject = container_of(gs, struct perf_inject, guest_session);
    let mut attr: perf_event_attr = evsel.core.attr;
    u64 *id_array;
    u32 *vcpu_array;
    ...
    let mut id: u64 = evsel.core.id[i];
...
#[no_mangle]
unsafe extern "C" fn guest_session__add_attrs(gs: *mut guest_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn synthesize_id_index(inject: *mut perf_inject, new_cnt: usize) -> c_int {
    static int synthesize_id_index(struct perf_inject *inject, size_t new_cnt)
    {
    struct perf_session *session = inject.session;
    struct evlist *evlist = session.evlist;
    struct machine *machine = &session.machines.host;
    let mut from: usize = evlist__nr_entries(evlist) - new_cnt;
...
#[no_mangle]
unsafe extern "C" fn dso__is_in_kernel_space(dso: *mut dso) -> bool {
...
#[no_mangle]
unsafe extern "C" fn evlist__first_id(evlist: *mut evlist) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn synthesize_build_id(inject: *mut perf_inject, dso: *mut dso, machine_pid: pid_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn guest_session__add_build_ids_cb(dso: *mut dso, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn guest_session__add_build_ids(gs: *mut guest_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn guest_session__start(gs: *mut guest_session, name: *const c_char, force: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_hlist(heads: *mut hlist_head, hlist_sz: usize) {
...
#[no_mangle]
unsafe extern "C" fn guest_session__convert_time(gs: *mut guest_session, guest_time: u64, host_time: *mut u64) {
...
#[no_mangle]
unsafe extern "C" fn guest_session__fetch(gs: *mut guest_session) -> c_int {
    static int guest_session__fetch(struct guest_session *gs)
    {
    void *buf;
    struct perf_event_header *hdr;
    let mut hdr_sz: usize = sizeof(*hdr);
    ssize_t ret;
...
#[no_mangle]
unsafe extern "C" fn guest_session__inject_events(gs: *mut guest_session, timestamp: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn guest_session__flush_events(gs: *mut guest_session) -> c_int {
...
//
    ...
    {
    struct perf_inject *inject = container_of(tool, struct perf_inject, tool);
    let mut ret: c_int = perf_event__process_finished_round(tool, event, oe);
    let mut timestamp: u64 = ordered_events__last_flush_time(oe);
    if (ret)
    return ret;
    ...
    {
    struct perf_inject *inject = container_of(tool, struct perf_inject, tool);
    let mut out: bool = event.header.misc & PERF_RECORD_MISC_SWITCH_OUT;
    struct guest_session *gs = &inject.guest_session;
    let mut pid: u32 = event.context_switch.next_prev_pid;
    let mut tid: u32 = event.context_switch.next_prev_tid;
    struct guest_tid *guest_tid;
    u32 vcpu;
...
#[no_mangle]
unsafe extern "C" fn evsel__check_stype(evsel: *mut evsel, sample_type: u64, sample_msg: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parse_vm_time_correlation(opt: *const option, str: *const c_char, unset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parse_guest_data(opt: *const option, str: *const c_char, unset: c_int) -> c_int {
    ...
    if (!tok)
    goto bad_args;
    gs.machine_pid = strtoul(tok, core::ptr::null_mut(), 0);
    ...
    if (!tok)
    goto out;
    gs.time_offset = strtoull(tok, core::ptr::null_mut(), 0);
    tok = strsep(&s, ",");
    if (!tok)
    goto out;
    gs.time_scale = strtod(tok, core::ptr::null_mut());
    if (!gs.time_scale)
    goto bad_args;
    out:
    return 0;
...
unsafe extern "C" fn save_section_info(inject: *mut perf_inject) -> c_int {
    ...
    let mut fd: c_int = perf_data__fd(inject.session.data);
    return perf_header__process_sections(header, fd, inject, save_section_info_cb);
    }
#[no_mangle]
unsafe extern "C" fn keep_feat(inject: *mut perf_inject, feat: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn read_file(fd: c_int, offs: u64, buf: *mut c_void, sz: usize) -> c_int {
    static int read_file(int fd, u64 offs, void *buf, size_t sz)
    {
    let mut ret: isize = preadn(fd, buf, sz, offs);
    if (ret < 0)
    return -errno;
    if ((size_t)ret != sz)
    return -EINVAL;
...
unsafe extern "C" fn feat_copy(inject: *mut perf_inject, feat: c_int, fw: *mut feat_writer) -> c_int {
    ...
    {
    let mut fd: c_int = perf_data__fd(inject.session.data);
    let mut offs: u64 = inject.secs[feat].offset;
    let mut sz: usize = inject.secs[feat].size;
    void *buf = malloc(sz);
    int ret;
    if (!buf)
    return -ENOMEM;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct inject_fc {
    pub fc: feat_copier,
    pub inject: *mut perf_inject,
}
...
#[no_mangle]
unsafe extern "C" fn guest_session__copy_kcore_dir(gs: *mut guest_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn output_fd(inject: *mut perf_inject) -> c_int {
...
unsafe extern "C" fn __cmd_inject(inject: *mut perf_inject) -> c_int {
    ...
    struct guest_session *gs = &inject.guest_session;
    struct perf_session *session = inject.session;
    let mut fd: c_int = output_fd(inject);
    let mut output_data_offset: u64 = perf_session__data_offset(session.evlist);
//
...
//
    let mut write_attrs_after_data: bool = !inject.output.is_pipe && inject.session.data.is_pipe;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(name, _arg: "sched:sched_stat_", _arg: 17)) -> else {
...
#[no_mangle]
unsafe extern "C" fn evsel__has_dwarf_callchain(evsel: *mut evsel) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn cmd_inject(argc: c_int, argv: *const c_char) -> c_int {
    ...
    int ret;
    const char *known_build_ids = core::ptr::null_mut();
    let mut build_ids: bool = false;
    let mut build_id_all: bool = false;
    let mut mmap2_build_ids: bool = false;
    let mut mmap2_build_id_all: bool = false;
    ...
    core::ptr::null_mut(), "opts", "Instruction Tracing options\n"
    ITRACE_HELP,
    itrace_parse_synth_opts),
    OPT_BOOLEAN(0, "strip", &inject.strip,
    "strip non-synthesized events (use with --itrace)"),
    OPT_CALLBACK_OPTARG(0, "vm-time-correlation", &inject, core::ptr::null_mut(), "opts",
    "correlate time between VM guests and the host",
    parse_vm_time_correlation),
    OPT_CALLBACK_OPTARG(0, "guest-data", &inject, core::ptr::null_mut(), "opts",
...
//
    ...
    if (inject.build_id_style != BID_RWS__NONE && known_build_ids != core::ptr::null_mut()) {
    ...
    if (inject.raw_callchain == core::ptr::null_mut()) {
