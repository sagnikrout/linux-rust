...
//! Source: tools/perf/builtin-daemon.c
#![no_std]
...
pub struct daemon_session {
    ...
    pub pid: c_int,
    pub list: list_head,
    pub state: enum daemon_session_state,
    pub start: time_t,
}
...
#[no_mangle]
unsafe extern "C" fn get_session_name(var: *const c_char, session: *mut c_char, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn session_config(daemon: *mut daemon, var: *const c_char, value: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn check_base(daemon: *mut daemon) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setup_client_config(daemon: *mut daemon) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setup_server_config(daemon: *mut daemon) -> c_int {
...
//
    ...
    session.start = time(core::ptr::null_mut());
    session.pid = fork();
    if (session.pid < 0)
    return -1;
    if (session.pid > 0) {
    ...
    if (!argv)
    exit(-1);
    exit(execve(daemon.perf, argv, core::ptr::null_mut()));
    return -1;
    }
#[no_mangle]
unsafe extern "C" fn handle_signalfd(daemon: *mut daemon) -> pid_t {
...
//
    ...
    do {
    let mut err: c_int = poll(&pollfd, 1, 1000);
    if (err > 0) {
    ...
    if (start + secs < time(core::ptr::null_mut()))
    return -1;
    } while (session.pid != -1);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn daemon__has_alive_session(daemon: *mut daemon) -> bool {
...
#[no_mangle]
unsafe extern "C" fn daemon__wait(daemon: *mut daemon, secs: c_int) -> c_int {
    ...
    {
    let mut pollfd: pollfd = { .events = POLLIN, };
...
#[no_mangle]
unsafe extern "C" fn setup_server_socket(daemon: *mut daemon) -> c_int {
    static int setup_server_socket(struct daemon *daemon)
    {
    struct sockaddr_un addr;
    char path[PATH_MAX];
    let mut fd: c_int = socket(AF_UNIX, SOCK_STREAM, 0);
    if (fd < 0) {
...
pub const SESSION_MAX: c_int = 64;
    union cmd {
    int cmd;
// CMD_LIST
    struct {
...
#[no_mangle]
unsafe extern "C" fn daemon_session__ping(session: *mut daemon_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cmd_session_list(daemon: *mut daemon, cmd: *mut union cmd, out: *mut FILE) -> c_int {
    static int cmd_session_list(struct daemon *daemon, union cmd *cmd, FILE *out)
    {
    let mut csv_sep: c_char = cmd.list.csv_sep;
    struct daemon_session *session;
    let mut curr: time_t = time(core::ptr::null_mut());
    if (csv_sep) {
...
#[no_mangle]
unsafe extern "C" fn daemon_session__signal(session: *mut daemon_session, sig: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cmd_session_kill(daemon: *mut daemon, cmd: *mut union cmd, out: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cmd_session_ping(daemon: *mut daemon, cmd: *mut union cmd, out: *mut FILE) -> c_int {
    static int cmd_session_ping(struct daemon *daemon, union cmd *cmd, FILE *out)
    {
    struct daemon_session *session;
    let mut all: bool = false, found = false;
    all = !strcmp(cmd.ping.name, "all");
    list_for_each_entry(session, &daemon.sessions, list) {
    if (all || !strcmp(cmd.ping.name, session.name)) {
    let mut state: c_int = daemon_session__ping(session);
...
#[no_mangle]
unsafe extern "C" fn handle_server_socket(daemon: *mut daemon, sock_fd: c_int) -> c_int {
    static int handle_server_socket(struct daemon *daemon, int sock_fd)
    {
    let mut ret: c_int = -1, fd;
    FILE *out = core::ptr::null_mut();
    union cmd cmd;
    fd = accept(sock_fd, core::ptr::null_mut(), core::ptr::null_mut());
    if (fd < 0) {
...
#[no_mangle]
unsafe extern "C" fn setup_client_socket(daemon: *mut daemon) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn daemon__signal(daemon: *mut daemon, sig: c_int) {
...
#[no_mangle]
unsafe extern "C" fn daemon_session__delete(session: *mut daemon_session) {
...
#[no_mangle]
unsafe extern "C" fn daemon_session__remove(session: *mut daemon_session) {
...
#[no_mangle]
unsafe extern "C" fn daemon__stop(daemon: *mut daemon) {
...
#[no_mangle]
unsafe extern "C" fn daemon__kill(daemon: *mut daemon) {
...
#[no_mangle]
unsafe extern "C" fn daemon__exit(daemon: *mut daemon) {
...
#[no_mangle]
unsafe extern "C" fn daemon__reconfig(daemon: *mut daemon) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setup_config_changes(daemon: *mut daemon) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn process_inotify_event(daemon: *mut daemon, buf: *mut c_char, len: isize) -> bool {
...
#[no_mangle]
unsafe extern "C" fn setup_config(daemon: *mut daemon) -> c_int {
    ...
    if (daemon.config) {
    char *real = realpath(daemon.config, core::ptr::null_mut());
    if (!real) {
...
#[no_mangle]
pub unsafe extern "C" fn if(perf_home_perfconfig(): perf_config_global() &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn lockf(fd: c_int, cmd: c_int, len: off_t) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn check_lock(daemon: *mut daemon) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn go_background(daemon: *mut daemon) -> c_int {
    ...
    setbuf(daemon.out, core::ptr::null_mut());
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn setup_signalfd(daemon: *mut daemon) -> c_int {
    static int setup_signalfd(struct daemon *daemon)
    {
    sigset_t mask;
    sigemptyset(&mask);
    sigaddset(&mask, SIGCHLD);
    if (sigprocmask(SIG_BLOCK, &mask, core::ptr::null_mut()) == -1)
    return -1;
    ...
    {
    let mut foreground: bool = false;
    struct option start_options[] = {
    OPT_BOOLEAN('f', "foreground", &foreground, "stay on console"),
    OPT_PARENT(parent_options),
    OPT_END()
    };
    let mut sock_fd: c_int = -1, conf_fd = -1, signal_fd = -1;
    ...
    daemon.start = time(core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn send_cmd(daemon: *mut daemon, cmd: *mut union cmd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn send_cmd_list(daemon: *mut daemon) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cmd_daemon(argc: c_int, argv: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(argv[0], _arg: "signal")) -> else {
    else if (!strcmp(argv[0], "signal"))
    ret = __cmd_signal(&__daemon, daemon_options, argc, argv);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(argv[0], _arg: "stop")) -> else {
    else if (!strcmp(argv[0], "stop"))
    ret = __cmd_stop(&__daemon, daemon_options, argc, argv);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(argv[0], _arg: "ping")) -> else {
