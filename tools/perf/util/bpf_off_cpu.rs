...
//! Source: tools/perf/util/bpf_off_cpu.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn off_cpu_config(evlist: *mut evlist) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn off_cpu_finish(__maybe_unused: *mut *mut void arg) {
...
#[no_mangle]
unsafe extern "C" fn check_sched_switch_args() {
    ...
    if (t1 == core::ptr::null_mut())
    goto cleanup;
    t2 = btf__type_by_id(btf, t1.type);
    if (t2 == core::ptr::null_mut() || !btf_is_ptr(t2))
    goto cleanup;
...
// new format: pass prev_state as 4th arg
    ...
    {
    int err, fd, i;
    let mut ncpus: c_int = 1, ntasks = 1, ncgrps = 1;
    struct strlist *pid_slist = core::ptr::null_mut();
    struct str_node *pos;
...
// don't need to set cpu filter for system-wide mode
    ...
    if (target.pid) {
    pid_slist = strlist__new(target.pid, core::ptr::null_mut());
    if (!pid_slist) {
    ...
    char *end_ptr;
    let mut pid: c_int = strtol(pos.s, &end_ptr, 10);
...
pub unsafe extern "C" fn off_cpu_write(session: *mut perf_session) -> c_int {
    ...
    let mut tstamp: u64 = OFF_CPU_TIMESTAMP;
    skel.bss.enabled = 0;
