...
//! Source: tools/perf/util/string.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn perf_atoll(str: *const c_char) -> i64 {
...
#[no_mangle]
unsafe extern "C" fn __match_charclass(pat: *const c_char, c: c_char, npat: *const c_char) -> bool {
    static bool __match_charclass(const char *pat, char c, const char **npat)
    {
    let mut complement: bool = false, ret = true;
    if (*pat == '!') {
    complement = true;
    pat++;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn strglobmatch(str: *const c_char, pat: *const c_char) -> bool {
    bool strglobmatch(const char *str, const char *pat)
    {
    return __match_glob(str, pat, false, false);
    }
#[no_mangle]
pub unsafe extern "C" fn strglobmatch_nocase(str: *const c_char, pat: *const c_char) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn strlazymatch(str: *const c_char, pat: *const c_char) -> bool {
...
pub unsafe extern "C" fn strtailcmp(s1: *const c_char, s2: *const c_char) -> c_int {
    ...
    {
    let mut i1: c_int = strlen(s1);
    let mut i2: c_int = strlen(s2);
    while (--i1 >= 0 && --i2 >= 0) {
    if (s1[i1] != s2[i2])
    return s1[i1] - s2[i2];
    }
...
// Like strpbrk_esc(), but not break if it is quoted with single/double quotes
    ...
    {
    char *_stopset = core::ptr::null_mut();
    char *ptr;
    const char *squote = "'";
    const char *dquote = "\"";
...
#[no_mangle]
pub unsafe extern "C" fn if(dquote: *mut *mut ptr ==) -> else {
    else if (*ptr == *dquote)
...
// now *ptr is `quote`.
    ...
    return *ptr == quote ? ptr : core::ptr::null_mut();
    }
//
...
// This is non-escaped quote
    let mut quote: c_int = *d;
    let mut len: c_int = strlen(d + 1) + 1;
//
...
//
    ...
    {
    let mut replace_len: c_int = strlen(replace);
    char *new_s, *to;
