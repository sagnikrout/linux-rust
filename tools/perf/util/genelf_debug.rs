...
//! Source: tools/perf/util/genelf_debug.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct debug_line_header {
// Not counting this field
    pub total_length: uword,
// version number (2 currently)
    pub version: uhalf,
// relative offset from next field to
// program statement
    pub prolog_length: uword,
    pub minimum_instruction_length: ubyte,
    pub default_is_stmt: ubyte,
// line_base - see DWARF 2 specs
    pub line_base: sbyte,
// line_range - see DWARF 2 specs
    pub line_range: ubyte,
// number of opcode + 1
    pub opcode_base: ubyte,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compilation_unit_header {
    pub total_length: uword,
    pub version: uhalf,
    pub debug_abbrev_offset: uword,
    pub pointer_size: ubyte,
    pub __packed: },
...
#[no_mangle]
unsafe extern "C" fn emit_uword(be: *mut buffer_ext, data: uword) {
    static void emit_uword(struct buffer_ext *be, uword data)
    {
    buffer_ext_add(be, &data, sizeof(uword));
    }
#[no_mangle]
unsafe extern "C" fn emit_string(be: *mut buffer_ext, s: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn emit_signed_LEB128(be: *mut buffer_ext, data: c_long) {
    static void emit_signed_LEB128(struct buffer_ext *be, long data)
    {
    let mut more: c_int = 1;
    let mut negative: c_int = data < 0;
    let mut size: c_int = sizeof(long) * CHAR_BIT;
    while (more) {
    let mut cur: ubyte = data & 0x7F;
    data >>= 7;
    if (negative)
...
#[no_mangle]
unsafe extern "C" fn emit_opcode(be: *mut buffer_ext, opcode: ubyte) {
...
#[no_mangle]
unsafe extern "C" fn emit_advance_pc(be: *mut buffer_ext, delta_pc: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn emit_advance_lineno(be: *mut buffer_ext, delta_lineno: c_long) {
...
#[no_mangle]
unsafe extern "C" fn emit_lne_end_of_sequence(be: *mut buffer_ext) {
    static void emit_lne_end_of_sequence(struct buffer_ext *be)
    {
    emit_extended_opcode(be, DW_LNE_end_sequence, core::ptr::null_mut(), 0);
    }
#[no_mangle]
unsafe extern "C" fn emit_set_file(be: *mut buffer_ext, idx: c_ulong) {
...
// start state of the state machine we take care of
    ...
    char const  *cur_filename = core::ptr::null_mut();
...
// empty filename directory
    ...
    let mut old_size: usize = buffer_ext_size(be);
