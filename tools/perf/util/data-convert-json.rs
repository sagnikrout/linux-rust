...
//! Source: tools/perf/util/data-convert-json.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn output_json_string(out: *mut FILE, s: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn output_json_delimiters(out: *mut FILE, comma: bool, depth: c_int) {
...
// Outputs a printf format string (with delimiter) as a JSON value.
    __printf(4, 5)
#[no_mangle]
unsafe extern "C" fn output_json_format(out: *mut FILE, comma: bool, depth: c_int, format: *const c_char, ...) {
...
// Outputs a JSON key-value pair where the value is a printf format string.
    ...
    struct dso *dso = al.map ? map__dso(al.map) : core::ptr::null_mut();
    fputc(',', out);
    ...
    let mut sample_type: u64 = __evlist__combined_sample_type(sample.evsel.evlist);
...
#[no_mangle]
pub unsafe extern "C" fn if(0: thread__cpu(al.thread) >=) -> else {
    ...
    unsigned int i;
    bool ok;
    let mut first_callchain: bool = true;
    ...
    output_sample_callchain_entry(tool, ip, ok ? &tal : core::ptr::null_mut());
    ...
    struct tep_format_field **fields = tp_format ? tep_event_fields(tp_format) : core::ptr::null_mut();
    if (fields) {
    let mut i: c_int = 0;
    while (fields[i]) {
    struct trace_seq s;
...
#[no_mangle]
unsafe extern "C" fn output_headers(session: *mut perf_session, c: *mut convert_json) {
    ...
    int ret;
    let mut fd: c_int = perf_data__fd(session.data);
    int i;
    FILE *out = c.out;
    output_json_key_format(out, false, 2, "header-version", "%u", header.version);
    ret = fstat(fd, &st);
    if (ret >= 0) {
    let mut stctime: time_t = st.st_mtime;
    char buf[256];
