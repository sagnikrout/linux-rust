...
//! Source: tools/perf/util/data-convert-bt.c
#![no_std]
...
pub struct evsel_priv {
    pub event_class: *mut bt_ctf_event_class,
}
...
// writer primitives
    pub writer: *mut bt_ctf_writer,
    pub stream: *mut ctf_stream,
    pub stream_cnt: c_int,
    pub stream_class: *mut bt_ctf_stream_class,
    pub clock: *mut bt_ctf_clock,
// data types
    union {
    struct {
    pub s64: *mut bt_ctf_field_type,
    pub u64: *mut bt_ctf_field_type,
    pub s32: *mut bt_ctf_field_type,
    pub u32: *mut bt_ctf_field_type,
    pub string: *mut bt_ctf_field_type,
    pub u32_hex: *mut bt_ctf_field_type,
    pub u64_hex: *mut bt_ctf_field_type,
}
...
// Ordered events configured queue size.
    ...
    let mut sign: bool = bt_ctf_field_type_integer_get_signed(type);
    int ret;
    ...
    {
    let mut flags: c_ulong = field.flags;
...
#[no_mangle]
unsafe extern "C" fn adjust_signedness(value_int: c_ulonglong, size: c_int) -> c_ulonglong {
...
#[no_mangle]
unsafe extern "C" fn string_set_value(field: *mut bt_ctf_field, string: *const c_char) -> c_int {
    static int string_set_value(struct bt_ctf_field *field, const char *string)
    {
    char *buffer = core::ptr::null_mut();
    let mut len: usize = strlen(string), i, p;
    int err;
    for (i = p = 0; i < len; i++, p++) {
    if (isprint(string[i])) {
    if (!buffer)
    ...
    let mut flags: c_ulong = fmtf.flags;
    ...
    let mut raw_size: c_uint = sample.raw_size;
    let mut nr_elements: c_uint = raw_size / sizeof(u32);
    unsigned int i;
    int ret;
    ...
    struct bt_ctf_field_type *len_type, *seq_type;
    struct bt_ctf_field *len_field, *seq_field;
    let mut nr_elements: c_uint = callchain.nr;
    unsigned int i;
    int ret;
    ...
    struct perf_sample *sample)
    {
    let mut type: u64 = evsel.core.attr.sample_type;
    int ret;
//
// missing:
...
#[no_mangle]
unsafe extern "C" fn ctf_stream__flush(cs: *mut ctf_stream) -> c_int {
    ...
    struct bt_ctf_field *pkt_ctx   = core::ptr::null_mut();
    struct bt_ctf_field *cpu_field = core::ptr::null_mut();
    struct bt_ctf_stream *stream   = core::ptr::null_mut();
    int ret;
    cs = zalloc(sizeof(*cs));
    if (!cs) {
...
#[no_mangle]
unsafe extern "C" fn ctf_stream__delete(cs: *mut ctf_stream) {
...
pub const STREAM_FLUSH_COUNT: c_int = 100000;
//
...
//
#[no_mangle]
unsafe extern "C" fn is_flush_needed(cs: *mut ctf_stream) -> bool {
    ...
    struct bt_ctf_event *event;
    int ret;
    let mut type: c_ulong = evsel.core.attr.sample_type;
...
// If dup < 0, add a prefix. Else, add _dupl_X suffix.
    ...
    {
    char *new_name = core::ptr::null_mut();
    size_t len;
    if (!name)
    name = orig_name;
    if (dup >= 10)
...
//
    ...
    struct tep_format_field *field)
    {
    struct bt_ctf_field_type *t = core::ptr::null_mut();
    char *name;
    let mut dup: c_int = 1;
    int ret;
...
//
    ...
    {
    const struct tep_event *tp_format = evsel__tp_format(evsel);
    struct tep_format_field *common_fields = tp_format ? tp_format.format.common_fields : core::ptr::null_mut();
    struct tep_format_field *fields        = tp_format ? tp_format.format.fields : core::ptr::null_mut();
    int ret;
...
#[no_mangle]
unsafe extern "C" fn add_event(cw: *mut ctf_writer, evsel: *mut evsel) -> c_int {
    ...
    let mut is_tracepoint: bool = evsel.core.attr.type == PERF_TYPE_TRACEPOINT;
...
#[no_mangle]
unsafe extern "C" fn cleanup_events(session: *mut perf_session) {
    ...
    evlist__put(evlist);
    session.evlist = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn setup_streams(cw: *mut ctf_writer, session: *mut perf_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_streams(cw: *mut ctf_writer) {
...
#[no_mangle]
unsafe extern "C" fn ctf_writer__cleanup_data(cw: *mut ctf_writer) {
...
#[no_mangle]
unsafe extern "C" fn ctf_writer__init_data(cw: *mut ctf_writer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctf_writer__cleanup(cw: *mut ctf_writer) {
...
#[no_mangle]
unsafe extern "C" fn ctf_writer__flush_streams(cw: *mut ctf_writer) -> c_int {
