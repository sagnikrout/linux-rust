...
//! Source: tools/perf/util/bpf_lock_contention.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn slab_cache_hash(key: c_long, __maybe_unused: *mut *mut void ctx) -> usize {
    static size_t slab_cache_hash(long key, void *ctx __maybe_unused)
    {
    return key;
    }
#[no_mangle]
unsafe extern "C" fn slab_cache_equal(key1: c_long, key2: c_long, __maybe_unused: *mut *mut void ctx) -> bool {
    static bool slab_cache_equal(long key1, long key2, void *ctx __maybe_unused)
    {
    let mut key1: return = = key2;
    }
#[no_mangle]
unsafe extern "C" fn check_slab_cache_iter(con: *mut lock_contention) {
    static void check_slab_cache_iter(struct lock_contention *con)
    {
    s32 ret;
    hashmap__init(&slab_hash, slab_cache_hash, slab_cache_equal, /*ctx=*/core::ptr::null_mut());
    con.btf = btf__load_vmlinux_btf();
    if (con.btf == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn run_slab_cache_iter() {
...
#[no_mangle]
unsafe extern "C" fn exit_slab_cache_iter() {
...
#[no_mangle]
unsafe extern "C" fn init_numa_data(con: *mut lock_contention) {
...
pub unsafe extern "C" fn lock_contention_prepare(con: *mut lock_contention) -> c_int {
    ...
    {
    int i, fd;
    let mut ncpus: c_int = 1, ntasks = 1, ntypes = 1, naddrs = 1, ncgrps = 1, nslabs = 1;
    struct evlist *evlist = con.evlist;
    struct target *target = con.target;
    let mut has_mmap_lock: bool = false;
...
// resolve lock name in delays
    ...
    let mut pid: u32 = evlist__workload_pid(evlist);
    let mut val: u8 = 1;
    fd = bpf_map__fd(skel.maps.task_filter);
    bpf_map_update_elem(fd, &pid, &val, BPF_ANY);
    }
...
//
#[no_mangle]
unsafe extern "C" fn mark_end_timestamp() {
    static void mark_end_timestamp(void)
    {
    DECLARE_LIBBPF_OPTS(bpf_test_run_opts, opts,
    .flags = BPF_F_TEST_RUN_ON_CPU,
    );
    let mut prog_fd: c_int = bpf_program__fd(skel.progs.end_timestamp);
    ...
    {
    u64 delta;
    let mut stat_key: contention_key = {};
...
//
#[no_mangle]
unsafe extern "C" fn account_end_timestamp(con: *mut lock_contention) {
    static void account_end_timestamp(struct lock_contention *con)
    {
    int ts_fd, stat_fd;
    int *prev_key, key;
    let mut end_ts: u64 = skel.bss.end_ts;
    int total_cpus;
    let mut aggr_mode: enum lock_aggr_mode = con.aggr_mode;
...
pub unsafe extern "C" fn lock_contention_stop() -> c_int {
    ...
    let mut pid: c_int = key.pid;
    let mut task_fd: c_int = bpf_map__fd(skel.maps.task_data);
// do not update idle comm which contains CPU number
    ...
    let mut lock_fd: c_int = bpf_map__fd(skel.maps.lock_syms);
...
// look slab_hash for dynamic locks in a slab object
    ...
    if (con.aggr_mode == LOCK_AGGR_CGROUP) {
    let mut cgrp_id: u64 = key.lock_addr_or_cgroup;
...
// LOCK_AGGR_CALLER: skip lock internal functions
    ...
    u64 *stack_trace = core::ptr::null_mut();
    s32 stack_id;
    let mut ckey: contention_key = {};
    let mut cdata: contention_data = {};
    let mut stack_size: usize = con.max_stack * sizeof(*stack_trace);
    struct lock_stat *st = core::ptr::null_mut();
    stacks_fd = bpf_map__fd(skel.maps.owner_stacks);
    stat_fd = bpf_map__fd(skel.maps.owner_stat);
    if (!stacks_fd || !stat_fd)
    goto out_err;
    stack_trace = zalloc(stack_size);
    if (stack_trace == core::ptr::null_mut())
    goto out_err;
    if (bpf_map_get_next_key(stacks_fd, core::ptr::null_mut(), stack_trace))
    goto out_err;
    ...
    if (!st)
    goto out_err;
    st.name = strdup(stack_trace[0] ? lock_contention_get_name(con, core::ptr::null_mut(), stack_trace, 0) :
    "unknown");
    if (!st.name)
    goto out_err;
...
// tid=*/0);
    ...
    let mut prog_fd: c_int = bpf_program__fd(skel.progs.collect_lock_syms);
    bpf_prog_test_run_opts(prog_fd, &opts);
    }
    prev_key = core::ptr::null_mut();
    while (!bpf_map_get_next_key(fd, prev_key, &key)) {
...
// to handle errors in the loop body
    ...
    if (st.callstack == core::ptr::null_mut())
    break;
    }
    next:
    prev_key = &key;
