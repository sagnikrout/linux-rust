...
//! Source: tools/perf/util/bpf_skel/sample_filter.bpf.c
#![no_std]
...
pub struct filters {
    ...
    pub int): __type(key,,
    pub perf_bpf_filter_entry[MAX_FILTERS]): __type(value, struct,
    pub 1): __uint(max_entries,,
    pub SEC(".maps"): } filters,
//
...
pub struct event_hash {
    pub BPF_MAP_TYPE_HASH): __uint(type,,
    pub __u64): __type(key,,
    pub __u64): __type(value,,
    pub 1): __uint(max_entries,,
    pub SEC(".maps"): } event_hash,
// tgid/evtid to filter index
#[repr(C)]
#[derive(Copy, Clone)]
pub struct idx_hash {
    pub BPF_MAP_TYPE_HASH): __uint(type,,
    pub idx_hash_key): __type(key, struct,
    pub int): __type(value,,
    pub 1): __uint(max_entries,,
    pub SEC(".maps"): } idx_hash,
...
pub struct lost_count {
    ...
    pub int): __type(value,,
    pub 1): __uint(max_entries,,
    pub SEC(".maps"): } dropped,
    pub use_idx_hash: volatile int,
    pub __ksym: *mut *mut *mut void bpf_cast_to_kern_ctx(void ),
...
// fall through
    ...
    if (entry.part == 3) {
    let mut snoop: __u32 = kctx.data.data_src.mem_snoop;
    let mut snoopx: __u32 = kctx.data.data_src.mem_snoopx;
...
// BPF program to be called from perf event overflow handler
    SEC("perf_event")
#[no_mangle]
pub unsafe extern "C" fn perf_sample_filter(ctx: *mut c_void) -> c_int {
    int perf_sample_filter(void *ctx)
    {
    struct bpf_perf_event_data_kern *kctx;
    struct perf_bpf_filter_entry *entry;
    __u64 sample_data;
    let mut in_group: c_int = 0;
    let mut group_result: c_int = 0;
    int i, k;
    int *losts;
