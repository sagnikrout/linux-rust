...
//! Source: tools/perf/util/c2c-function.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn c2c_hitm_count(stats: *const c2c_stats) -> u64 {
    ...
    {
    const struct dso *left_dso = left.map ? map__dso(left.map) : core::ptr::null_mut();
    const struct dso *right_dso = right.map ? map__dso(right.map) : core::ptr::null_mut();
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn hist_entry__iaddr(he: *mut hist_entry) -> u64 {
...
//
#[no_mangle]
pub unsafe extern "C" fn hist_entry__is_cacheline(he: *mut hist_entry) -> bool {
...
pub const C2C_FUNC_INDENT: c_int = 2;
// Width of the folded-sign prefix ("%c ") each identity cell emits.
pub const C2C_FUNC_FOLD_WIDTH: c_int = 2;
//
...
//
#[no_mangle]
unsafe extern "C" fn hist_entry__indent(he: *mut hist_entry, buf: *mut c_char, size: usize) -> c_int {
    static int hist_entry__indent(struct hist_entry *he, char *buf, size_t size)
    {
    let mut indent: c_int = he.depth * C2C_FUNC_INDENT;
...
// Level-2/3 entries already aggregate the stores they represent.
    ...
    let mut total: u64 = hist_entry__displayed_stores(he);
...
//
    ...
    struct hist_entry *left, struct hist_entry *right)
    {
    let mut left_store: u64 = hist_entry__displayed_stores(left);
    let mut right_store: u64 = hist_entry__displayed_stores(right);
...
//
#[no_mangle]
unsafe extern "C" fn get_function_format(name: *const c_char, fmtp: *mut c2c_fmt) -> c_int {
...
// Mark symbol-backed sort keys so hists__has(hists, sym) is correct.
    ...
    if (!list)
    return 0;
    for (tok = strtok_r(list, ", ", &tmp); tok; tok = strtok_r(core::ptr::null_mut(), ", ", &tmp)) {
...
//
#[no_mangle]
unsafe extern "C" fn c2c_function_hists__setup_output_field(list: *mut perf_hpp_list) {
    ...
    struct perf_env *env)
    {
    char *output = output_str ? strdup(output_str) : core::ptr::null_mut();
    char *sort   = sort_str   ? strdup(sort_str)   : core::ptr::null_mut();
    let mut ret: c_int = 0;
    ...
    return function_hpp_list__parse(&hists.list, /*output=*/core::ptr::null_mut(), sort, env);
    }
    static int
    ...
    struct perf_env *env)
    {
    let mut nr_header_lines: c_int = c2c_hists.list.nr_header_lines;
...
// Merge compute_stats during function aggregation.
    ...
    {
    let mut nr_events: u64 = c2c_hitm_count(stats) + stats.rmt_peer + stats.lcl_peer;
    let mut weight1: u64 = c2c_hitm_count(stats);
//
...
// Add entry - histogram handles dedup
    ...
    &al, core::ptr::null_mut(), core::ptr::null_mut(), mi,
    core::ptr::null_mut(), &sample, true);
...
//
    ...
    let mut key: map_symbol = detail_he.ms;
    int64_t cmp;
    key.sym = sym;
    parent = *p;
...
unsafe extern "C" fn c2c_he__resort_by_stores(parent_he: *mut hist_entry) {
    ...
    {
    let mut new_root: rb_root_cached = RB_ROOT_CACHED;
    struct rb_node *nd;
...
// Extract all nodes and re-insert sorted by displayed store count
    ...
    struct hist_entry *he = rb_entry(nd, struct hist_entry, rb_node);
    let mut he_store: u64 = hist_entry__displayed_stores(he);
...
//
    while (*p) {
    struct hist_entry *iter = rb_entry(*p, struct hist_entry, rb_node);
    let mut iter_store: u64 = hist_entry__displayed_stores(iter);
    parent = *p;
    if (he_store != iter_store) {
    cmp = he_store > iter_store ? -1 : 1;
    } else {
// Stable tie-break: instruction address, name, then cacheline.
    let mut a: u64 = hist_entry__iaddr(he), b = hist_entry__iaddr(iter);
    if (a != b)
    cmp = a < b ? -1 : 1;
#[no_mangle]
pub unsafe extern "C" fn if(iter->ms.sym: he->ms.sym &&) -> else {
...
pub const DEFAULT_SYMBOLS_PER_CL: c_int = 64;
#[repr(C)]
#[derive(Copy, Clone)]
...
//
    ...
    struct function_seen *seen = core::ptr::null_mut();
    let mut nr_seen: c_int = 0, nr_alloc = 0;
    let mut ret: c_int = 0;
...
// Build this function's subtree for this cacheline only once.
    ...
    let mut new_alloc: c_int = nr_alloc ? nr_alloc * 2 : DEFAULT_SYMBOLS_PER_CL;
...
//
    ...
    c2c_ext.cl_hists = core::ptr::null_mut();
    c2c_ext.cl_sort = core::ptr::null_mut();
...
unsafe extern "C" fn c2c_function__has_iaddr(cl_sort: *const c_char) -> bool {
    ...
    let mut len: usize = end ? (size_t)(end - field) : strlen(field);
    if (len == sizeof("iaddr") - 1 && !strncmp(field, "iaddr", len))
    return true;
    field = end ? end + 1 : core::ptr::null_mut();
    }
    return false;
    }
//
...
//
    ...
    "symbol_view", 2, core::ptr::null_mut());
...
//
    ...
    "cycles_percent", core::ptr::null_mut());
    if (ret)
    goto out_err;
    hists__collapse_resort(&c2c_ext.function_hists.hists, core::ptr::null_mut());
    hists__output_resort(&c2c_ext.function_hists.hists, core::ptr::null_mut());
