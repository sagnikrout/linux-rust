...
//! Source: tools/perf/util/annotate-arch/annotate-x86.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn x86__cpuid_parse(arch: *mut arch, cpuid: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn invalidate_reg_state(reg: *mut type_state_reg) {
    ...
    let mut fbreg: c_int = dloc.fbreg;
...
// Update register with the return type (if any)
    ...
    let mut imm_value: u64 = -1ULL;
    int offset;
    const char *var_name = core::ptr::null_mut();
    struct map_symbol *ms = dloc.ms;
    let mut ip: u64 = ms.sym.start + dl.al.offset;
...
#[no_mangle]
pub unsafe extern "C" fn if(DWARF_REG_PC: src->reg1 ==) -> else {
...
// Case 1: Based on stack pointer or frame pointer
    ...
    let mut offset: c_int = src.offset - fboff;
...
// Case 2: Based on a register holding a typed pointer
    ...
    __die_get_real_type(&state.regs[sreg].type, &type_die) == core::ptr::null_mut())
    return;
...
// Check if the target type has a member at the new offset
    ...
    src.offset + src_tsr.offset, &type_die) == core::ptr::null_mut())
    return;
...
// xor reg, reg clears the register
    ...
    return;
    if (dloc.fb_cfa) {
    let mut ip: u64 = dloc.ms.sym.start + dl.al.offset;
    let mut pc: u64 = map__rip_2objdump(dloc.ms.map, ip);
...
#[no_mangle]
pub unsafe extern "C" fn if(DWARF_REG_PC: sreg ==) -> else {
    struct map_symbol *ms = dloc.ms;
    let mut ip: u64 = ms.sym.start + dl.al.offset;
    u64 addr;
    int offset;
...
// And check percpu access with base register
    ...
    let mut ip: u64 = dloc.ms.sym.start + dl.al.offset;
    let mut var_addr: u64 = src.offset;
    int offset;
    if (src.multi_regs) {
    let mut reg2: c_int = (sreg == src.reg1) ? src.reg2 : src.reg1;
...
// Check stack variables with offset
    ...
    struct type_state_stack *stack;
    let mut offset: c_int = dst.offset - fboff;
