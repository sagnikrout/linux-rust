...
//! Source: tools/perf/builtin-kvm.c
#![no_std]
...
// Return opposite number for sorting in alphabetical order
    ...
    {
    let mut width: c_int = fmt_width(fmt, hpp, he.hists);
...
#[no_mangle]
unsafe extern "C" fn percent(st: u64, tot: u64) -> double {
    ...
    let mut width: c_int = fmt_width(fmt, hpp, hists);
    kvm_fmt = container_of(fmt, struct kvm_fmt, fmt);
    dim = kvm_fmt.dim;
...
unsafe extern "C" fn fmt_free(fmt: *mut perf_hpp_fmt) {
    ...
    {
    unsigned int i;
    for (i = 0; dimensions[i] != core::ptr::null_mut(); i++) {
...
unsafe extern "C" fn kvm_hists__init_sort(hpp_list: *mut perf_hpp_list, name: *mut c_char) -> c_int {
    ...
    tok = strtok_r(core::ptr::null_mut(), ", ", &tmp)) {
    ret = fn(hpp_list, tok);
    if (!ret)
    continue;
// Handle errors
...
#[no_mangle]
pub unsafe extern "C" fn if(-ESRCH: ret ==) -> else {
    else if (ret == -ESRCH)
...
unsafe extern "C" fn kvm_hists__init() -> c_int {
    ...
    return kvm_hpp_list__parse(&kvm_hists.list, core::ptr::null_mut(), "ev_name");
    }
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn kvm__hists_browse(hists: *mut hists) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(perf_guest: !perf_host &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn register_kvm_events_ops(kvm: *mut perf_kvm_stat, e_machine: u16) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vcpu_event_record {
    pub vcpu_id: c_int,
    pub start_time: u64,
    pub last_event: *mut kvm_event,
}
...
unsafe extern "C" fn kvm_event_expand(event: *mut kvm_event, vcpu_id: c_int) -> bool {
    ...
    {
    let mut old_max_vcpu: c_int = event.max_vcpu;
    void *prev;
...
unsafe extern "C" fn kvm_he_free(he: *mut c_void) {
    ...
    &kvm.al, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), ki, sample, true);
    ...
    struct perf_sample *sample)
    {
    struct kvm_event *event = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn kvm_event_rel_stddev(vcpu_id: c_int, event: *mut kvm_event) -> double {
...
#[no_mangle]
unsafe extern "C" fn skip_event(e_machine: u16, event: *const c_char) -> bool {
...
// Both begin and end events did not get the key.
    ...
    if (!event)
    return false;
    vcpu_record.last_event = core::ptr::null_mut();
...
// seems to happen once in a while during live mode
    ...
    let mut e_machine: u16 = perf_session__e_machine(kvm.session, /*e_flags=*/core::ptr::null_mut());
...
// Only kvm_entry records vcpu id.
    ...
    let mut e_machine: u16 = thread__e_machine(thread, machine, /*e_flags=*/core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn event_is_valid(event: *mut kvm_event, vcpu: c_int) -> bool {
...
unsafe extern "C" fn sort_result(kvm: *mut perf_kvm_stat) {
    ...
    hists__collapse_resort(&kvm_hists.hists, core::ptr::null_mut());
    hists__output_resort_cb(&kvm_hists.hists, core::ptr::null_mut(), filter_cb);
    ui_progress__finish();
    }
#[no_mangle]
unsafe extern "C" fn print_vcpu_info(kvm: *mut perf_kvm_stat) {
    ...
    {
    let mut vcpu: c_int = kvm.trace_vcpu;
    pr_info("Analyze events for ");
    if (kvm.opts.target.system_wide)
    pr_info("all VMs, ");
#[no_mangle]
pub unsafe extern "C" fn if(_arg: kvm->opts.target.pid) -> else {
...
unsafe extern "C" fn print_result(kvm: *mut perf_kvm_stat) {
    ...
    struct perf_sample *sample)
    {
    if (kvm.pid_list && intlist__find(kvm.pid_list, sample.pid) == core::ptr::null_mut())
    return true;
    return false;
    }
...
unsafe extern "C" fn cpu_isa_config(kvm: *mut perf_kvm_stat) -> c_int {
    ...
    e_machine = perf_session__e_machine(kvm.session, /*e_flags=*/core::ptr::null_mut());
...
//
pub const PERF_KVM__MAX_EVENTS_PER_MMAP: c_int = 25;
...
// mmap_time = ULLONG_MAX;
    ...
    err = perf_session__queue_event(kvm.session, event, timestamp, 0, core::ptr::null_mut());
//
...
unsafe extern "C" fn perf_kvm__mmap_read(kvm: *mut perf_kvm_stat) -> c_int {
    ...
    s64 n, ntotal = 0;
    let mut flush_time: u64 = ULLONG_MAX, mmap_time;
...
unsafe extern "C" fn perf_kvm__timerfd_create(kvm: *mut perf_kvm_stat) -> c_int {
    ...
    if (timerfd_settime(kvm.timerfd, 0, &new_value, core::ptr::null_mut()) != 0) {
...
// Reset sort list to "ev_name"
    kvm_hists__reinit(core::ptr::null_mut(), "ev_name");
// reset counts
...
unsafe extern "C" fn kvm_live_open_events(kvm: *mut perf_kvm_stat) -> c_int {
    ...
    evlist__config(evlist, &kvm.opts, core::ptr::null_mut());
//
...
unsafe extern "C" fn parse_target_str(kvm: *mut perf_kvm_stat) -> c_int {
    ...
    if (kvm.pid_list == core::ptr::null_mut()) {
...
unsafe extern "C" fn kvm_events_report_vcpu(kvm: *mut perf_kvm_stat) -> c_int {
    ...
    set_option_flag(record_options, 'g', core::ptr::null_mut(), PARSE_OPT_DISABLED);
...
// name = '\0';
    name++;
    if (evlist__add_newtp(evlist, sys, name, core::ptr::null_mut())) {
...
//
    ...
    if (kvm.evlist == core::ptr::null_mut()) {
    err = -1;
    goto out;
    }
...
#[no_mangle]
unsafe extern "C" fn kvm_cmd_stat(file_name: *const c_char, argc: c_int, argv: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __cmd_record(file_name: *const c_char, argc: c_int, argv: *const c_char) -> c_int {
    static int __cmd_record(const char *file_name, int argc, const char **argv)
    {
    int rec_argc, i = 0, j, ret;
    const char **rec_argv;
    let mut need_arch_event: c_int = !!kvm_need_default_arch_event(EM_HOST, argc, argv);
//
...
#[no_mangle]
unsafe extern "C" fn __cmd_report(file_name: *const c_char, argc: c_int, argv: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __cmd_top(argc: c_int, argv: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cmd_kvm(argc: c_int, argv: *const c_char) -> c_int {
    int cmd_kvm(int argc, const char **argv)
    {
    const char *file_name = core::ptr::null_mut();
    ...
    "buildid-list", "stat", core::ptr::null_mut() };
    const char *kvm_usage[] = { core::ptr::null_mut(), core::ptr::null_mut() };
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(strstarts("report": strlen(argv[0]) > 2 &&, _arg: argv[0])) -> else {
    else if (strlen(argv[0]) > 2 && strstarts("report", argv[0]))
    ret = __cmd_report(file_name, argc, argv);
#[no_mangle]
pub unsafe extern "C" fn if(strstarts("diff": strlen(argv[0]) > 2 &&, _arg: argv[0])) -> else {
    else if (strlen(argv[0]) > 2 && strstarts("diff", argv[0]))
    ret = cmd_diff(argc, argv);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(argv[0], _arg: "top")) -> else {
    else if (!strcmp(argv[0], "top"))
    ret = __cmd_top(argc, argv);
#[no_mangle]
pub unsafe extern "C" fn if(strstarts("buildid-list": strlen(argv[0]) > 2 &&, _arg: argv[0])) -> else {
    else if (strlen(argv[0]) > 2 && strstarts("buildid-list", argv[0]))
    ret = __cmd_buildid_list(file_name, argc, argv);

#[no_mangle]
pub unsafe extern "C" fn if(strstarts("stat": strlen(argv[0]) > 2 &&, _arg: argv[0])) -> else {
