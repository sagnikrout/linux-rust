...
//! Source: tools/perf/arch/x86/util/pmu.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn x86__is_intel_graniterapids() -> bool {
...
#[no_mangle]
unsafe extern "C" fn snc_nodes_per_l3_cache() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn num_chas() -> c_int {
    static int num_chas(void)
    {
    static bool checked_chas;
    static int num_chas;
    if (!checked_chas) {
    let mut fd: c_int = perf_pmu__event_source_devices_fd();
    struct io_dir dir;
    struct io_dirent64 *dent;
    if (fd < 0)
    return -1;
    io_dir__init(&dir, fd);
    while ((dent = io_dir__readdir(&dir)) != core::ptr::null_mut()) {
...
pub const MAX_SNCS: c_int = 6;
#[no_mangle]
unsafe extern "C" fn uncore_cha_snc(pmu: *mut perf_pmu) -> c_int {
...
// CHA SNC numbers are ordered correspond to the CHAs number.
    ...
    let mut snc_nodes: c_int = snc_nodes_per_l3_cache();
    if (snc_nodes <= 1)
    return 0;
    num_cha = num_chas();
    if (num_cha <= 0) {
...
#[no_mangle]
unsafe extern "C" fn uncore_imc_snc(pmu: *mut perf_pmu) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uncore_cha_imc_compute_cpu_adjust(pmu_snc: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gnr_uncore_cha_imc_adjust_cpumask_for_snc(pmu: *mut perf_pmu, cha: bool) {
...
// No adjustment necessary for the first SNC.
    ...
    alloc = adjusted[pmu_snc] == core::ptr::null_mut();
    if (alloc) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strstarts(pmu->name, _arg: "uncore_imc_")) -> else {
