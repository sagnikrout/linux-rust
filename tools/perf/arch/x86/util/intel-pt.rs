...
//! Source: tools/perf/arch/x86/util/intel-pt.c
#![no_std]
...
pub const INTEL_PT_PSB_PERIOD_NEAR: c_int = 256;
#[repr(C)]
#[derive(Copy, Clone)]
...
pub struct intel_pt_recording {
    ...
    pub snapshot_ref_buf_size: usize,
...
#[no_mangle]
unsafe extern "C" fn intel_pt_parse_terms(pmu: *const perf_pmu, str: *const c_char, config: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_pt_masked_bits(mask: u64, bits: u64) -> u64 {
    static u64 intel_pt_masked_bits(u64 mask, u64 bits)
    {
    let mut top_bit: u64 = 1ULL << 63;
    let mut res: u64 = 0;
    int i;
    for (i = 0; i < 64; i++) {
    if (mask & top_bit) {
...
#[no_mangle]
unsafe extern "C" fn intel_pt_pick_bit(bits: c_int, target: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_pt_filter_bytes(filter: *const c_char) -> usize {
    static size_t intel_pt_filter_bytes(const char *filter)
    {
    let mut len: usize = filter ? strlen(filter) : 0;
...
// d = eax;
    ...
    let mut cap_user_time_zero: bool = false, per_cpu_mmaps;
    ...
    if (filter_str_len) {
    let mut len: usize = intel_pt_filter_bytes(filter);
...
unsafe extern "C" fn intel_pt_exclude_guest() -> bool {
    ...
    pt_mode = 0;
    let mut pt_mode: return = = 1;
    }
#[no_mangle]
unsafe extern "C" fn intel_pt_valid_str(str: *mut c_char, len: usize, valid: u64) {
...
//
    ...
    struct evsel *evsel;
    evlist__for_each_entry(evlist, evsel) {
    let mut sz: usize = evsel.core.attr.aux_sample_size;
    if (!sz)
    continue;
...
unsafe extern "C" fn intel_pt_too_many_aux_output(evlist: *mut evlist) -> bool {
    ...
    struct evsel *evsel, *intel_pt_evsel = core::ptr::null_mut();
...
// Set default sizes for snapshot mode
    ...
    let mut psb_period: usize = intel_pt_psb_period(intel_pt_pmu, evlist);
...
// Validate auxtrace_mmap_pages
    ...
    if (!opts.auxtrace_snapshot_mode && !opts.auxtrace_sample_mode) {
    let mut aw: usize = opts.auxtrace_mmap_pages * (size_t)page_size / 4;
    let mut aux_watermark: u32 = aw > UINT_MAX ? UINT_MAX : aw;
...
unsafe extern "C" fn intel_pt_alloc_snapshot_refs(ptr: *mut intel_pt_recording, idx: c_int) -> c_int {
    ...
    {
    let mut sz: usize = sizeof(struct intel_pt_snapshot_ref);
    let mut cnt: c_int = ptr.snapshot_ref_cnt, new_cnt = cnt * 2;
...
unsafe extern "C" fn intel_pt_recording_free(itr: *mut auxtrace_record) {
    ...
    {
    let mut ref_buf_size: usize = ptr.snapshot_ref_buf_size;
    void *ref_buf;
    ...
    size_t snapshot_buf_size)
    {
    let mut max_size: usize = 256 * 1024;
    let mut buf_size: usize = 0, psb_period;
...
//
    ...
    {
    let mut end2: usize = offs2 + compare_size, part_size;
    if (end2 <= buf2_size)
    ...
    {
    let mut ref_end: usize = ref_offset + ref_size;
    if (ref_end > buf_size) {
...
#[no_mangle]
unsafe extern "C" fn intel_pt_first_wrap(data: *mut u64, buf_size: usize) -> bool {
...
#[no_mangle]
unsafe extern "C" fn intel_pt_reference(__maybe_unused: *mut *mut auxtrace_record itr) -> u64 {
