...
//! Source: tools/perf/arch/x86/tests/intel-pt-test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dump_packet(packet: *const intel_pt_pkt, bytes: *const u8, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn decoding_failed(d: *const test_data) {
    ...
    enum intel_pt_pkt_ctx ctx)
    {
    let mut old_ctx: enum intel_pt_pkt_ctx = ctx;
...
#[no_mangle]
unsafe extern "C" fn test_one(d: *const test_data) -> c_int {
    static int test_one(const struct test_data *d)
    {
    struct intel_pt_pkt packet;
    let mut ctx: enum intel_pt_pkt_ctx = d.ctx;
    int ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn test__intel_pt_pkt_decoder(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setaffinity(cpu: c_int) -> c_int {
...
pub const INTEL_PT_SUBLEAF_CNT: c_int = 2;
pub const CPUID_REG_CNT: c_int = 4;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cpuid_result {
    union {
    struct {
    pub eax: c_uint,
    pub ebx: c_uint,
...
#[derive(Copy, Clone)]
pub struct pt_caps {
    pub subleaf: [cpuid_result; INTEL_PT_SUBLEAF_CNT],
}

#[no_mangle]
unsafe extern "C" fn get_pt_caps(cpu: c_int, caps: *mut pt_caps) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_hybrid() -> bool {
...
#[no_mangle]
unsafe extern "C" fn compare_caps(cpu: c_int, caps: *mut pt_caps, caps0: *mut pt_caps) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn test__intel_pt_hybrid_compat(test: *mut test_suite, subtest: c_int) -> c_int {
    int test__intel_pt_hybrid_compat(struct test_suite *test, int subtest)
    {
    let mut max_cpu: c_int = cpu__max_cpu().cpu;
