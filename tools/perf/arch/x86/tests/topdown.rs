...
//! Source: tools/perf/arch/x86/tests/topdown.c
#![no_std]
...
// ret = TEST_FAIL;
    ...
    let mut p_core_pmu: bool = evsel.pmu.type == PERF_TYPE_RAW;
...
#[no_mangle]
unsafe extern "C" fn test__x86_topdown(__maybe_unused: *mut *mut test_suite test, __maybe_unused: int subtest) -> c_int {
...
//
    ...
    CHECK_COND(strstr(evsel__name(next), "instructions") != core::ptr::null_mut(),
    "slots second event is instructions");
    CHECK_COND(strstr(evsel__name(next2), "topdown-retiring") != core::ptr::null_mut(),
    "slots third event is topdown-retiring");
    } else if (expected_slots_group_size == 2) {
    struct evsel *next = evsel__next(evsel);
    CHECK_COND(strstr(evsel__name(next), "topdown-retiring") != core::ptr::null_mut(),
