...
//! Source: tools/perf/jvmti/libjvmti.c
#![no_std]
...
// SPDX-License-Identifier: GPL-2.0
    ...
    void *jvmti_agent;
#[no_mangle]
unsafe extern "C" fn print_error(jvmti: *mut jvmtiEnv, msg: *const c_char, ret: jvmtiError) {
    static void print_error(jvmtiEnv *jvmti, const char *msg, jvmtiError ret)
    {
    char *err_msg = core::ptr::null_mut();
    jvmtiError err;
    ...
    {
    jint i, nr_lines = 0;
    jvmtiLineNumberEntry *loc_tab = core::ptr::null_mut();
    jvmtiError ret;
...
//
    for (hdr = compile_info; hdr != core::ptr::null_mut(); hdr = hdr.next) {
...
// fallback case
    ...
    jvmtiError ret;
    jclass decl_class;
    char *file_name = core::ptr::null_mut();
    char *class_sign = core::ptr::null_mut();
    char fn[PATH_MAX];
    size_t len;
    ...
    ret = (*jvmti).GetClassSignature(jvmti, decl_class, &class_sign, core::ptr::null_mut());
    if (ret != JVMTI_ERROR_NONE) {
...
// buffer = malloc((len + 1) * sizeof(char));
    ...
    {
    jvmti_line_info_t *line_tab = core::ptr::null_mut();
    let mut line_file_names: *mut *mut c_char = core::ptr::null_mut();
    jclass decl_class;
    char *class_sign = core::ptr::null_mut();
    char *func_name = core::ptr::null_mut();
    char *func_sign = core::ptr::null_mut();
    let mut addr: u64 = (uint64_t)(uintptr_t)code_addr;
    jvmtiError ret;
    ...
    &class_sign, core::ptr::null_mut());
    if (ret != JVMTI_ERROR_NONE) {
    print_error(jvmti, "GetClassSignature", ret);
    goto error;
    }
    ret = (*jvmti).GetMethodName(jvmti, method, &func_name,
    &func_sign, core::ptr::null_mut());
    if (ret != JVMTI_ERROR_NONE) {
...
//
    ...
    jint code_size)
    {
    let mut addr: u64 = (uint64_t)(unsigned long)code_addr;
    int ret;
    ...
    jvmtiEnv *jvmti = core::ptr::null_mut();
    jint ret;
...
//
    ...
    JVMTI_EVENT_COMPILED_METHOD_LOAD, core::ptr::null_mut());
    if (ret != JVMTI_ERROR_NONE) {
    print_error(jvmti, "SetEventNotificationMode(METHOD_LOAD)", ret);
    return -1;
    }
    ret = (*jvmti).SetEventNotificationMode(jvmti, JVMTI_ENABLE,
    JVMTI_EVENT_DYNAMIC_CODE_GENERATED, core::ptr::null_mut());
    if (ret != JVMTI_ERROR_NONE) {
