...
//! Source: tools/perf/builtin-script.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_script {
    pub tool: perf_tool,
    pub session: *mut perf_session,
    pub show_task_events: bool,
    pub show_mmap_events: bool,
    pub show_switch_events: bool,
    pub show_namespace_events: bool,
    pub show_lost_events: bool,
    pub show_round_events: bool,
    pub show_bpf_events: bool,
    pub show_cgroup_events: bool,
    pub show_text_poke_events: bool,
    pub allocated: bool,
    pub per_event_dump: bool,
    pub stitch_lbr: bool,
...
#[no_mangle]
unsafe extern "C" fn evsel_script__delete(es: *mut evsel_script) {
    static void evsel_script__delete(struct evsel_script *es)
    {
    zfree(&es.filename);
    fclose(es.fp);
    es.fp = core::ptr::null_mut();
    free(es);
    }
#[no_mangle]
unsafe extern "C" fn evsel_script__fprintf(es: *mut evsel_script, fp: *mut FILE) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn output_type(type: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn evsel__output_type(evsel: *mut evsel) -> c_int {
    static inline int evsel__output_type(struct evsel *evsel)
    {
    let mut type: c_int = evsel.script_output_type;
...
#[no_mangle]
unsafe extern "C" fn output_set_by_user() -> bool {
    ...
    let mut type: c_int = evsel__output_type(evsel);
    const char *evname;
...
#[no_mangle]
unsafe extern "C" fn evsel__check_attr(evsel: *mut evsel, session: *mut perf_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn evsel__set_print_ip_opts(evsel: *mut evsel) {
    static void evsel__set_print_ip_opts(struct evsel *evsel)
    {
    let mut type: c_uint = evsel__output_type(evsel);
...
//
#[no_mangle]
unsafe extern "C" fn perf_session__check_output_opt(session: *mut perf_session) -> c_int {
    static int perf_session__check_output_opt(struct perf_session *session)
    {
    let mut tod: bool = false;
    unsigned int j;
    struct evsel *evsel;
    for (j = 0; j < OUTPUT_TYPE_MAX; ++j) {
...
// 'dsoff' implys 'dso' field
    ...
    if (!no_callchain) {
    let mut use_callchain: bool = false;
    let mut not_pipe: bool = false;
...
//
    ...
    FILE *fp)
    {
    let mut i: unsigned = 0, r;
    let mut printed: c_int = 0;
    if (!regs || !regs.regs)
    return 0;
    printed += fprintf(fp, " ABI:%" PRIu64 " ", regs.abi);
    for_each_set_bit(r, (unsigned long *) &mask, sizeof(mask) * 8) {
    let mut val: u64 = regs.regs[i++];
    ...
    if (localtime_r(&sec, &ltime) == core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn if(symbol_conf.use_callchain: PRINT_FIELD(IP) && evsel__has_callchain(evsel) &&) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: PRINT_FIELD(PID)) -> else {
    else if (PRINT_FIELD(PID))
    printed += fprintf(fp, "%7d ", sample.pid);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: PRINT_FIELD(TID)) -> else {
...
#[no_mangle]
unsafe extern "C" fn print_bstack_flags(fp: *mut FILE, br: *mut branch_entry) -> c_int {
...
//
    ...
    if (!thread__find_map(thread, *cpumode, start, &al) || (dso = map__dso(al.map)) == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn map__fprintf_srccode(map: *mut map, addr: u64, fp: *mut FILE, state: *mut srccode_state) -> c_int {
    ...
    char *srccode;
    struct dso *dso;
    if (!map || (dso = map__dso(map)) == core::ptr::null_mut())
    return 0;
...
#[no_mangle]
unsafe extern "C" fn print_srccode(thread: *mut thread, cpumode: u8, addr: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn add_padding(fp: *mut FILE, printed: c_int, padding: c_int) -> c_int {
    ...
    let mut printed: c_int = fprintf(fp, "\t%016" PRIx64 "\t", ip);
    ...
    let mut i: c_uint = 0, j, num, mask, width, numprinted = 0;
    perf_env__find_br_cntr_info(evsel__env(evsel), core::ptr::null_mut(), &width);
    mask = (1L << width) - 1;
...
// lastsym = al.sym;
    ...
    u8 buffer[MAXBB];
    unsigned off;
    struct symbol *lastsym = core::ptr::null_mut();
    let mut total_cycles: c_int = 0;
    let mut br_cntr: u64 = 0;
    if (!(br && br.nr))
    return 0;
    nr = br.nr;
...
//
    ...
    {
    struct addr_location al;
    let mut printed: c_int = fprintf(fp, "%16" PRIx64, sample.addr);
    addr_location__init(&al);
...
// ip = sample->ip;
    ...
    FILE *fp)
    {
    let mut depth: usize = thread_stack__depth(thread, sample.cpu);
    const char *name = core::ptr::null_mut();
    static int spacing;
    let mut len: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ip) -> else {
    else if (ip)
...
//
    ...
    struct perf_event_attr *attr = &evsel.core.attr;
    let mut type: c_uint = evsel__output_type(evsel);
    let mut print_srcline_last: bool = false;
    let mut printed: c_int = 0;
    if (PRINT_FIELD(CALLINDENT))
    printed += perf_sample__fprintf_callindent(sample, evsel, thread, al, addr_al, fp);
// print branch_from information
    if (PRINT_FIELD(IP)) {
    let mut print_opts: c_uint = output[type].print_ip_opts;
    struct callchain_cursor *cursor = core::ptr::null_mut();
    if (symbol_conf.use_callchain && sample.callchain) {
    cursor = get_tls_callchain_cursor();
    if (thread__resolve_callchain(al.thread, cursor,
    sample, core::ptr::null_mut(), core::ptr::null_mut(),
    scripting_max_stack))
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_flags(flags: u32, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_bpf_output(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
    static int perf_sample__fprintf_bpf_output(struct perf_sample *sample, FILE *fp)
    {
    let mut nr_bytes: c_uint = sample.raw_size;
    let mut printer_data: printer_data = {0, false, true};
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_spacing(len: c_int, spacing: c_int, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_pt_spacing(len: c_int, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ptw_is_prt(val: u64) -> bool {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_ptwrite(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_mwait(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_pwre(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_exstop(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_pwrx(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_cbr(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_psb(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_evt(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
    ...
    const char *cfe[32] = {core::ptr::null_mut(), "INTR", "IRET", "SMI", "RSM", "SIPI",
    "INIT", "VMENTRY", "VMEXIT", "VMEXIT_INTR",
    "SHUTDOWN", core::ptr::null_mut(), "UINTR", "UIRET"};
    ...
    let mut et: c_uint = data.evd[i].evd_type & 0x3f;
    s = evd[et];
    if (s) {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_iflag_chg(sample: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sample__fprintf_synth_vpadtl(data: *mut perf_sample, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn evlist__max_name_len(evlist: *mut evlist) -> c_int {
    static int evlist__max_name_len(struct evlist *evlist)
    {
    struct evsel *evsel;
    let mut max: c_int = 0;
    evlist__for_each_entry(evlist, evsel) {
    let mut len: c_int = strlen(evsel__name(evsel));
    max = MAX(len, max);
    }
    return max;
    }
#[no_mangle]
unsafe extern "C" fn data_src__fprintf(data_src: u64, fp: *mut FILE) -> c_int {
...
pub struct metric_ctx {
    ...
    if (!fmt)
    return;
    perf_sample__fprintf_start(core::ptr::null_mut(), mctx.sample, mctx.thread, mctx.evsel,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct script_find_metrics_args {
...
//
    ...
    if (script_evsel.metric_leader == core::ptr::null_mut())
    continue;
...
//
    ...
    metric_evsel,
// create=*/false);
    if (script_evsel.metric_id == core::ptr::null_mut()) {
    script_evsel.metric_id = metric_evsel.metric_id;
    metric_evsel.metric_id = core::ptr::null_mut();
    }
    if (script_evsel.metric_leader == core::ptr::null_mut()) {
    if (metric_evsel.metric_leader == metric_evsel) {
...
//
    ...
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__global(cpu, /*data=*/core::ptr::null_mut());
    }
...
// TODO: currently only global aggregation is supported.
    ...
    aggr_cpu_id__global, /*data=*/core::ptr::null_mut(),
// needs_sort=*/false);
    }
...
// Display all metrics.
    ...
    struct addr_location *addr_al)
    {
    let mut depth: c_int = thread_stack__depth(thread, sample.cpu);
    ...
    nlen = strlen(name);
    while (*s) {
    let mut len: unsigned = strcspn(s, ",");
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: PRINT_FIELD(BRSTACKSYM)) -> else {
    else if (PRINT_FIELD(BRSTACKSYM))
    perf_sample__fprintf_brstacksym(sample, thread, evsel, fp);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: PRINT_FIELD(BRSTACKOFF)) -> else {
...
#[no_mangle]
unsafe extern "C" fn __process_stat(counter: *mut evsel, tstamp: u64) {
    static void __process_stat(struct evsel *counter, u64 tstamp)
    {
    let mut nthreads: c_int = perf_thread_map__nr(counter.core.threads);
    struct perf_cpu cpu;
...
#[no_mangle]
unsafe extern "C" fn process_stat(counter: *mut evsel, tstamp: u64) {
...
#[no_mangle]
unsafe extern "C" fn process_stat_interval(tstamp: u64) {
...
#[no_mangle]
unsafe extern "C" fn setup_scripting() {
...
#[no_mangle]
unsafe extern "C" fn flush_scripting() -> c_int {
    static int flush_scripting(void)
    {
    return scripting_ops ? scripting_ops.flush_script() : 0;
    }
#[no_mangle]
unsafe extern "C" fn cleanup_scripting() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn filter_cpu(sample: *mut perf_sample) -> bool {
...
// Set thread to NULL to indicate addr_al and al are not initialized
    ...
    if (scripting_ops) {
    struct addr_location *addr_al_ptr = core::ptr::null_mut();
    ...
    struct perf_event_attr *attr = &evsel.core.attr;
    struct evsel_script *es = evsel.priv;
    let mut type: c_uint = output_type(attr.type);
    struct addr_location al;
    FILE *fp = es.fp;
    let mut ret: c_int = 0;
    if (output[type].fields == 0)
...
// Set thread to NULL to indicate addr_al and al are not initialized
    ...
    if (al.filtered)
    goto out_put;
    if (!show_event(sample, evsel, al.thread, &al, core::ptr::null_mut()))
    goto out_put;
    if (evswitch__discard(&scr.evswitch, evsel))
    goto out_put;
...
//
    ...
    e_machine = perf_session__e_machine(evsel__session(evsel), /*e_flags=*/core::ptr::null_mut());
...
// Print before 'exit' deletes anything
    ...
    {
    perf_event__fprintf(event, core::ptr::null_mut(), stdout);
    return 0;
    }
    static int
...
#[no_mangle]
unsafe extern "C" fn perf_script__fclose_per_event_dump(script: *mut perf_script) {
...
#[no_mangle]
unsafe extern "C" fn perf_script__fopen_per_event_dump(script: *mut perf_script) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_script__setup_per_event_dump(script: *mut perf_script) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_script__exit_per_event_dump_stats(script: *mut perf_script) {
...
#[no_mangle]
unsafe extern "C" fn perf_script__exit(script: *mut perf_script) {
...
#[no_mangle]
unsafe extern "C" fn __cmd_script(script: *mut perf_script) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn list_available_languages_cb(ops: *mut scripting_ops, spec: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn list_available_languages() {
...
// Find script file relative to current directory or exec path
    ...
    {
    char *tok, *strtok_saveptr = core::ptr::null_mut();
    int i, imax = ARRAY_SIZE(all_output_options);
    int j;
    let mut rc: c_int = 0;
    char *str = strdup(arg);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(str, _arg: "sw")) -> else {
    else if (!strcmp(str, "sw"))
    type = PERF_TYPE_SOFTWARE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(str, _arg: "trace")) -> else {
    else if (!strcmp(str, "trace"))
    type = PERF_TYPE_TRACEPOINT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(str, _arg: "raw")) -> else {
    else if (!strcmp(str, "raw"))
    type = PERF_TYPE_RAW;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(str, _arg: "break")) -> else {
    else if (!strcmp(str, "break"))
    type = PERF_TYPE_BREAKPOINT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(str, _arg: "synth")) -> else {
...
// Don't override defaults for +-
    ...
    parse:
    for (tok = strtok_r(tok, ",", &strtok_saveptr); tok; tok = strtok_r(core::ptr::null_mut(), ",", &strtok_saveptr)) {
    if (*tok == '+') {
    if (change == SET)
    goto out_badmix;
    change = ADD;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct script_desc {
    pub node: list_head,
    pub name: *mut c_char,
    pub half_liner: *mut c_char,
    pub args: *mut c_char,
}
    ...
    if (s != core::ptr::null_mut() && name)
    s.name = strdup(name);
    return s;
    }
#[no_mangle]
unsafe extern "C" fn script_desc__delete(s: *mut script_desc) {
...
#[no_mangle]
unsafe extern "C" fn script_desc__add(s: *mut script_desc) {
...
#[no_mangle]
unsafe extern "C" fn read_script_info(desc: *mut script_desc, filename: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_top_script(script_path: *const c_char) -> bool {
    static bool is_top_script(const char *script_path)
    {
    return ends_with(script_path, "top") != core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn has_required_arg(script_path: *mut c_char) -> c_int {
    static int has_required_arg(char *script_path)
    {
    struct script_desc *desc;
    let mut n_args: c_int = 0;
    char *p;
    desc = script_desc__new(core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn have_cmd(argc: c_int, argv: *const c_char) -> c_int {
    ...
    core::ptr::null_mut(), PARSE_OPT_STOP_AT_NON_OPTION);
    free(__argv);
    system_wide = (argc == 0);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn script__setup_sample_type(script: *mut perf_script) {
    static void script__setup_sample_type(struct perf_script *script)
    {
    struct perf_session *session = script.session;
    let mut sample_type: u64 = evlist__combined_sample_type(session.evlist);
    callchain_param_setup(sample_type, perf_session__e_machine(session, /*e_flags=*/core::ptr::null_mut()));
...
#[no_mangle]
unsafe extern "C" fn set_maps(script: *mut perf_script) -> c_int {
    ...
    union perf_event *event)
    {
    let mut ret: c_int = perf_event__process_auxtrace_info(tool, session, event);
    if (ret == 0) {
    struct perf_script *script = container_of(tool, struct perf_script, tool);
...
#[no_mangle]
pub unsafe extern "C" fn if(strcmp(str: strlen(str) != 0 &&, 0: "raw") !=) -> else {
    fprintf(stderr, "Only accept raw|disasm\n");
    return -EINVAL;
    }
    }
    ret = parse_output_fields(core::ptr::null_mut(), fields, 0);
    if (ret < 0)
    return ret;
    ...
    int unset __maybe_unused)
    {
    parse_output_fields(core::ptr::null_mut(), "-ip,-addr,-event,-period,+callindent", 0);
    ...
    int unset __maybe_unused)
    {
    parse_output_fields(core::ptr::null_mut(), "-ip,-addr,-event,-period,+callindent,+flags", 0);
    itrace_parse_synth_opts(opt, "crewp", 0);
    symbol_conf.nanosecs = true;
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn cmd_script(argc: c_int, argv: *const c_char) -> c_int {
    int cmd_script(int argc, const char **argv)
    {
    let mut show_full_info: bool = false;
    let mut header: bool = false;
    let mut header_only: bool = false;
    let mut script_started: bool = false;
    let mut unsorted_dump: bool = false;
    let mut merge_deferred_callchains: bool = true;
    char *rec_script_path = core::ptr::null_mut();
    char *rep_script_path = core::ptr::null_mut();
    struct perf_session *session;
    struct itrace_synth_opts itrace_synth_opts = {
    .set = false,
    .default_no_sample = true,
    };
    char *script_path = core::ptr::null_mut();
    const char *dlfilter_file = core::ptr::null_mut();
    const char **__argv;
    int i, j, err = 0;
    let mut script: perf_script = {};
    ...
    OPT_CALLBACK_NOOPT('l', "list", core::ptr::null_mut(), core::ptr::null_mut(), "list available scripts",
    list_available_scripts),
    OPT_CALLBACK_NOOPT(0, "list-dlfilters", core::ptr::null_mut(), core::ptr::null_mut(), "list available dlfilters",
    list_available_dlfilters),
    OPT_CALLBACK('s', "script", core::ptr::null_mut(), "name",
    "script file name (lang:script name, script name, or *)",
    parse_scriptname),
    OPT_STRING('g', "gen-script", &generate_script_lang, "lang",
    "generate perf-script.xx script in specified language"),
    OPT_STRING(0, "dlfilter", &dlfilter_file, "file", "filter .so file name"),
    OPT_CALLBACK(0, "dlarg", core::ptr::null_mut(), "argument", "filter argument",
    add_dlarg),
    ...
    OPT_CALLBACK('F', "fields", core::ptr::null_mut(), "str",
    ...
    OPT_CALLBACK_OPTARG(0, "insn-trace", &itrace_synth_opts, core::ptr::null_mut(), "raw|disasm",
    "Decode instructions from itrace", parse_insn_trace),
    OPT_CALLBACK_OPTARG(0, "xed", core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(),
    "Run xed disassembler on output", parse_xed),
    OPT_CALLBACK_OPTARG(0, "call-trace", &itrace_synth_opts, core::ptr::null_mut(), core::ptr::null_mut(),
    "Decode calls from itrace", parse_call_trace),
    OPT_CALLBACK_OPTARG(0, "call-ret-trace", &itrace_synth_opts, core::ptr::null_mut(), core::ptr::null_mut(),
    ...
    OPT_CALLBACK(0, "unwind-style", core::ptr::null_mut(), "unwind style",
    ...
    const char * const script_subcommands[] = { "record", "report", core::ptr::null_mut() };
...
// make sure PERF_EXEC_PATH is set for scripts
    ...
    system_wide = false;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !system_wide) -> else {
    ...
    err = scripting_ops.generate_script(core::ptr::null_mut(), filename);
