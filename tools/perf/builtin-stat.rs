...
//! Source: tools/perf/builtin-stat.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rusage_stats {
    pub ru_utime_usec_stat: stats,
    pub ru_stime_usec_stat: stats,
}
    ...
    let mut all_counters_use_bpf: static bool = true;
    static struct target target;
    let mut child_pid: static volatile sig_atomic_t = -1;
    let mut detailed_run: static int = 0;
    static bool			transaction_run;
    let mut topdown_run: static bool = false;
    let mut smi_cost: static bool = false;
    let mut smi_reset: static bool = false;
    let mut big_num_opt: static int = -1;
    static const char		*pre_cmd			= core::ptr::null_mut();
    static const char		*post_cmd			= core::ptr::null_mut();
    let mut sync_run: static bool = false;
    let mut forever: static bool = false;
    let mut force_metric_only: static bool = false;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_stat {
    pub record: bool,
    pub data: perf_data,
    pub session: *mut perf_session,
    pub bytes_written: u64,
    pub tool: perf_tool,
    pub maps_allocated: bool,
    pub cpus: *mut perf_cpu_map,
    pub threads: *mut perf_thread_map,
    pub aggr_mode: enum aggr_mode,
    pub aggr_level: u32,
}

    static struct perf_stat		perf_stat;

    let mut done: static volatile sig_atomic_t = 0;
// Options set from the command line.
#[repr(C)]
#[derive(Copy, Clone)]
pub struct opt_aggr_mode {
    pub no_aggr: bool node, socket, die, cluster, cache, core, thread,,
}

// Turn command line option into most generic aggregation mode setting.
#[no_mangle]
unsafe extern "C" fn opt_aggr_mode_to_aggr_mode(opt_mode: *const opt_aggr_mode) -> enum aggr_mode {
    static enum aggr_mode opt_aggr_mode_to_aggr_mode(const struct opt_aggr_mode *opt_mode)
    {
    let mut mode: enum aggr_mode = AGGR_GLOBAL;
    if (opt_mode.node)
    mode = AGGR_NODE;
    if (opt_mode.socket)
    mode = AGGR_SOCKET;
...
#[no_mangle]
unsafe extern "C" fn evlist__check_cpu_maps(evlist: *mut evlist) {
    static void evlist__check_cpu_maps(struct evlist *evlist)
    {
    struct evsel *evsel, *warned_leader = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn write_stat_round_event(tm: u64, type: u64) -> c_int {
    static int write_stat_round_event(u64 tm, u64 type)
    {
    return perf_event__synthesize_stat_round(core::ptr::null_mut(), tm, type,
    ...
    let mut cpu: perf_cpu = perf_cpu_map__cpu(evsel__cpus(counter), cpu_map_idx);
    return perf_event__synthesize_stat(core::ptr::null_mut(), cpu, thread, sid.id, count,
...
unsafe extern "C" fn read_single_counter(counter: *mut evsel, cpu_map_idx: c_int, thread: c_int) -> c_int {
    ...
    {
    let mut err: c_int = evsel__read_counter(counter, cpu_map_idx, thread);
//
...
//
    ...
    struct perf_counts_values *old_count = core::ptr::null_mut();
    u64 val;
...
unsafe extern "C" fn read_counter_cpu(counter: *mut evsel, cpu_map_idx: c_int) -> c_int {
    ...
    {
    let mut nthreads: c_int = perf_thread_map__nr(evlist__core(evsel_list).threads);
    int thread;
...
unsafe extern "C" fn process_interval() {
    ...
    print_counters(&rs, 0, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn handle_interval(interval: c_uint, times: *mut c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn evsel__should_store_id(counter: *mut evsel) -> bool {
...
unsafe extern "C" fn process_evlist(evlist: *mut evlist, interval: c_uint) {
    ...
    int *time_to_sleep)
    {
    let mut tts: c_int = *time_to_sleep;
...
#[no_mangle]
unsafe extern "C" fn dispatch_events(forks: bool, timeout: c_int, interval: c_int, times: *mut c_int) -> c_int {
    static int dispatch_events(bool forks, int timeout, int interval, int *times)
    {
    let mut child_exited: c_int = 0, status = 0;
    int time_to_sleep, sleep_time;
    struct timespec time_start, time_stop;
    if (interval)
    sleep_time = interval;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: timeout) -> else {
    else if (timeout)
    sleep_time = timeout;
    else
    sleep_time = 1000;
...
#[no_mangle]
unsafe extern "C" fn stat_handle_error(counter: *mut evsel, err: c_int) -> enum counter_recovery {
...
unsafe extern "C" fn update_rusage_stats(rusage: *const rusage) {
    ...
    {
    let mut us_to_ns: u64 = 1000;
    let mut s_to_ns: u64 = 1000000000;
...
#[no_mangle]
unsafe extern "C" fn __run_perf_stat(argc: c_int, argv: *const c_char, run_idx: c_int) -> c_int {
    static int __run_perf_stat(int argc, const char **argv, int run_idx)
    {
    let mut interval: c_int = stat_config.interval;
    let mut times: c_int = stat_config.times;
    let mut timeout: c_int = stat_config.timeout;
    char msg[BUFSIZ];
    ...
    let mut is_pipe: bool = STAT_RECORD ? perf_stat.data.is_pipe : false;
    struct evlist_cpu_iterator evlist_cpu_itr;
    int err, open_err = 0;
    let mut second_pass: bool = false, has_supported_counters;
    if (forks) {
...
// Now reopen weak
    ...
    if (STAT_RECORD) {
    let mut fd: c_int = perf_data__fd(&perf_stat.data);
    if (is_pipe) {
    ...
    if (err < 0)
    goto err_out;
    err = perf_event__synthesize_stat_events(&stat_config, core::ptr::null_mut(), evsel_list,
...
//
#[no_mangle]
unsafe extern "C" fn run_perf_stat(argc: c_int, argv: *const c_char, run_idx: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn print_counters(ts: *mut timespec, argc: c_int, argv: *const c_char) {
    static void print_counters(struct timespec *ts, int argc, const char **argv)
    {
...
//
    ...
    if (child_pid != -1)
    kill(child_pid, SIGTERM);
    sigprocmask(SIG_SETMASK, &oset, core::ptr::null_mut());
    if (signr == -1)
    return;
...
unsafe extern "C" fn cpu__get_cache_details(cpu: perf_cpu, cache: *mut perf_cache) -> c_int {
    ...
    {
    let mut ret: c_int = 0;
    let mut cache_level: u32 = stat_config.aggr_level;
    struct cpu_cache_level caches[MAX_CACHE_LVL];
    let mut i: u32 = 0, caches_cnt = 0;
...
//
    ...
    let mut max_level_index: c_int = 0;
...
unsafe extern "C" fn aggr_cpu_id__cache(cpu: perf_cpu, data: *mut c_void) -> aggr_cpu_id {
    ...
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__socket(cpu, /*data=*/core::ptr::null_mut());
    }
    static struct aggr_cpu_id perf_stat__get_die(struct perf_stat_config *config __maybe_unused,
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__die(cpu, /*data=*/core::ptr::null_mut());
    }
    static struct aggr_cpu_id perf_stat__get_cache_id(struct perf_stat_config *config __maybe_unused,
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__cache(cpu, /*data=*/core::ptr::null_mut());
    }
    static struct aggr_cpu_id perf_stat__get_cluster(struct perf_stat_config *config __maybe_unused,
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__cluster(cpu, /*data=*/core::ptr::null_mut());
    }
    static struct aggr_cpu_id perf_stat__get_core(struct perf_stat_config *config __maybe_unused,
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__core(cpu, /*data=*/core::ptr::null_mut());
    }
    static struct aggr_cpu_id perf_stat__get_node(struct perf_stat_config *config __maybe_unused,
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__node(cpu, /*data=*/core::ptr::null_mut());
    }
    ...
    struct perf_cpu cpu)
    {
    return aggr_cpu_id__cpu(cpu, /*data=*/core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn aggr_mode__get_aggr(aggr_mode: enum aggr_mode) -> aggr_cpu_id_get_t {
...
#[no_mangle]
unsafe extern "C" fn aggr_mode__get_id(aggr_mode: enum aggr_mode) -> aggr_get_id_t {
...
unsafe extern "C" fn perf_stat_init_aggr_mode() -> c_int {
    ...
    {
    int nr;
    let mut get_id: aggr_cpu_id_get_t = aggr_mode__get_aggr(stat_config.aggr_mode);
    if (get_id) {
    let mut needs_sort: bool = stat_config.aggr_mode != AGGR_NONE;
    stat_config.aggr_map = cpu_aggr_map__new(
    evlist__core(evsel_list).user_requested_cpus,
    get_id, /*data=*/core::ptr::null_mut(), needs_sort);
    ...
    if (stat_config.aggr_map == core::ptr::null_mut())
    return -ENOMEM;
    for (int s = 0; s < nr; s++) {
    let mut id: aggr_cpu_id = aggr_cpu_id__empty();
    id.thread_idx = s;
...
unsafe extern "C" fn perf_stat__exit_aggr_mode() {
    ...
    stat_config.aggr_map = core::ptr::null_mut();
    stat_config.cpus_aggr_map = core::ptr::null_mut();
    }
#[no_mangle]
...
//
    ...
    {
    int i;
    let mut caches_cnt: c_int = env.caches_cnt;
...
//
    ...
    struct cpu_topology_map *topo = perf_env__get_cpu_topology(env, cpu);
    if (topo) {
    let mut cache_level: u32 = (perf_stat.aggr_level) ?: stat_config.aggr_level;
    id.socket = topo.socket_id;
    id.die = topo.die_id;
...
#[no_mangle]
unsafe extern "C" fn aggr_mode__get_aggr_file(aggr_mode: enum aggr_mode) -> aggr_cpu_id_get_t {
...
#[no_mangle]
unsafe extern "C" fn aggr_mode__get_id_file(aggr_mode: enum aggr_mode) -> aggr_get_id_t {
...
unsafe extern "C" fn perf_stat_init_aggr_mode_file(st: *mut perf_stat) -> c_int {
    ...
    let mut get_id: aggr_cpu_id_get_t = aggr_mode__get_aggr_file(stat_config.aggr_mode);
    let mut needs_sort: bool = stat_config.aggr_mode != AGGR_NONE;
    if (stat_config.aggr_mode == AGGR_THREAD) {
    let mut nr: c_int = perf_thread_map__nr(evlist__core(evsel_list).threads);
    stat_config.aggr_map = cpu_aggr_map__empty_new(nr);
    if (stat_config.aggr_map == core::ptr::null_mut())
    return -ENOMEM;
    for (int s = 0; s < nr; s++) {
...
//
    ...
    if (topdown_run) {
    let mut max_level: c_uint = metricgroups__topdown_max_level();
    char str[] = "TopdownL1";
    if (!force_metric_only)
    stat_config.metric_only = true;
    if (!max_level) {
...
// metric_no_threshold=*/true,
    ...
    metricgroup__copy_metric_events(evlist, /*cgrp=*/core::ptr::null_mut(),
    evlist__metric_events(evlist),
    evlist__metric_events(metric_evlist));
    evlist__put(metric_evlist);
    }
    list_sort(/*priv=*/core::ptr::null_mut(), &evlist__core(evlist).entries, default_evlist_evsel_cmp);
    }
    out:
    if (!ret) {
    evlist__for_each_entry(evlist, evsel) {
...
//
    ...
    metricgroup__copy_metric_events(evsel_list, /*cgrp=*/core::ptr::null_mut(),
...
unsafe extern "C" fn init_features(session: *mut perf_session) {
    ...
    struct timespec tsh, *ts = core::ptr::null_mut();
    struct perf_env *env = perf_session__env(session);
    const char **argv = env.cmdline_argv;
    let mut argc: c_int = env.nr_cmdline;
    process_counters();
    ...
    if (stat_config.aggr_map) {
    let mut nr_aggr: c_int = stat_config.aggr_map.nr;
...
#[no_mangle]
unsafe extern "C" fn __cmd_report(argc: c_int, argv: *const c_char) -> c_int {
    static int __cmd_report(int argc, const char **argv)
    {
    struct perf_session *session;
    let mut opt_mode: opt_aggr_mode = {};
...
#[no_mangle]
pub unsafe extern "C" fn cmd_stat(argc: c_int, argv: *const c_char) -> c_int {
    int cmd_stat(int argc, const char **argv)
    {
    let mut opt_mode: opt_aggr_mode = {};
    let mut affinity: bool = true, affinity_set = false;
    ...
    OPT_CALLBACK_NOOPT('B', "big-num", core::ptr::null_mut(), core::ptr::null_mut(),
    ...
    OPT_CALLBACK_NOOPT(0, "metric-only", &stat_config.metric_only, core::ptr::null_mut(),
    ...
    let mut status: c_int = -EINVAL, run_idx, err;
    const char *mode;
    FILE *output = stderr;
    ...
    if (evsel_list == core::ptr::null_mut())
    return -ENOMEM;
...
// String-parsing callback-based options would segfault when negated
    ...
    parse_options_usage(core::ptr::null_mut(), stat_options, "j", 1);
    goto out;
    }
...
//
    ...
    parse_options_usage(core::ptr::null_mut(), stat_options, "log-fd", 0);
    goto out;
    }
...
//
    ...
    parse_options_usage(core::ptr::null_mut(), stat_options, "table", 0);
    goto out;
    }
    if (output_fd < 0) {
    ...
    parse_options_usage(stat_usage, stat_options, "o", 1);
    parse_options_usage(core::ptr::null_mut(), stat_options, "log-fd", 0);
    parse_options_usage(core::ptr::null_mut(), stat_options, "interval-clear", 0);
    status = -1;
    goto out;
    }
...
// User explicitly passed -B?
    ...
    parse_options_usage(core::ptr::null_mut(), stat_options, "x", 1);
    goto out;
...
//
    ...
    parse_options_usage(core::ptr::null_mut(), stat_options, "p", 1);
    parse_options_usage(core::ptr::null_mut(), stat_options, "t", 1);
    goto out;
    }
    }
//
...
//
    ...
    parse_options_usage(core::ptr::null_mut(), stat_options, "A", 1);
    parse_options_usage(core::ptr::null_mut(), stat_options, "a", 1);
    parse_options_usage(core::ptr::null_mut(), stat_options, "for-each-cgroup", 0);
    goto out;
    }
...
//
    ...
    interval_count = true;
#[no_mangle]
pub unsafe extern "C" fn if(!interval: stat_config.times &&) -> else {
...
// Enable ignoring missing threads when -p option is defined.
    ...
    break;
    if (forever && !interval) {
    print_counters(core::ptr::null_mut(), argc, argv);
