...
//! Source: tools/perf/builtin-kwork.c
#![no_std]
...
//
pub const PRINT_CPU_WIDTH: c_int = 4;
pub const PRINT_COUNT_WIDTH: c_int = 9;
pub const PRINT_RUNTIME_WIDTH: c_int = 10;
pub const PRINT_LATENCY_WIDTH: c_int = 10;
pub const PRINT_TIMESTAMP_WIDTH: c_int = 17;
pub const PRINT_KWORK_NAME_WIDTH: c_int = 30;
pub const RPINT_DECIMAL_WIDTH: c_int = 3;
pub const PRINT_BRACKETPAIR_WIDTH: c_int = 2;
pub const PRINT_TIME_UNIT_SEC_WIDTH: c_int = 2;
pub const PRINT_TIME_UNIT_MESC_WIDTH: c_int = 3;
pub const PRINT_PID_WIDTH: c_int = 7;
pub const PRINT_TASK_NAME_WIDTH: c_int = 16;
pub const PRINT_CPU_USAGE_WIDTH: c_int = 6;
pub const PRINT_CPU_USAGE_DECIMAL_WIDTH: c_int = 2;
pub const PRINT_CPU_USAGE_HIST_WIDTH: c_int = 30;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sort_dimension {
    pub name: *const c_char,
    pub r): *mut *mut *mut int (cmp)(struct kwork_work l, struct kwork_work,
    pub list: list_head,
}
...
unsafe extern "C" fn id_or_cpu_r_cmp(l: *mut kwork_work, r: *mut kwork_work) -> c_int {
    ...
    struct kwork_atom *atom = core::ptr::null_mut();
...
unsafe extern "C" fn atom_free(atom: *mut kwork_atom) {
    ...
    {
    if (atom.prev != core::ptr::null_mut())
    atom_free(atom.prev);
...
pub unsafe extern "C" fn if(0: cmp <) -> else {
    ...
    if (work.name == core::ptr::null_mut() && key.name != core::ptr::null_mut())
    ...
    work.id = key.id;
    work.cpu = key.cpu;
    work.name = key.name ? strdup(key.name) : core::ptr::null_mut();
...
unsafe extern "C" fn perf_kwork__exit(kwork: *mut perf_kwork) {
    ...
    struct perf_sample *sample)
    {
    let mut cpu: c_int = work.cpu;
    let mut time: u64 = sample.time;
...
//
    ...
    bool overwrite)
    {
    struct kwork_atom *atom = core::ptr::null_mut(), *dst_atom, *last_atom;
    struct kwork_work *work, key;
    let mut ret: c_int = 0;
    BUG_ON(class.work_init == core::ptr::null_mut());
    class.work_init(kwork, class, &key, src_type, sample, machine);
    atom = atom_new(kwork, sample);
    if (atom == core::ptr::null_mut()) {
    ret = -1;
    goto out;
    }
    ...
    if (dst_atom != core::ptr::null_mut()) {
    atom.prev = dst_atom;
    list_del(&dst_atom.list);
    }
    }
    if (ret_work != core::ptr::null_mut())
// ret_work = work;
    if (overwrite) {
    ...
    struct kwork_work **ret_work)
    {
    struct kwork_atom *atom = core::ptr::null_mut(), *src_atom;
    struct kwork_work *work, key;
    BUG_ON(class.work_init == core::ptr::null_mut());
    class.work_init(kwork, class, &key, src_type, sample, machine);
    work = work_findnew(&class.work_root, &key, &kwork.cmp_id);
...
// ret_work = NULL;
    ...
    {
    u64 delta;
    let mut exit_time: u64 = sample.time;
    let mut entry_time: u64 = atom.time;
    ...
    machine, core::ptr::null_mut(), true);
    }
    ...
    {
    struct kwork_atom *atom = core::ptr::null_mut();
    struct kwork_work *work = core::ptr::null_mut();
    ...
    {
    u64 delta;
    let mut entry_time: u64 = sample.time;
    let mut raise_time: u64 = atom.time;
    ...
    if (!kwork.show_callchain || sample.callchain == core::ptr::null_mut())
    return;
// want main thread for process - has maps
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), kwork.max_stack + 2) != 0) {
...
unsafe extern "C" fn irq_work_name(work: *mut kwork_work, buf: *mut c_char, len: c_int) {
    ...
    {
    if (work.name != core::ptr::null_mut())
    ...
    struct tep_print_arg *args = tp_format ? tp_format.print_fmt.args : core::ptr::null_mut();
    if ((args == core::ptr::null_mut()) || (args.next == core::ptr::null_mut()))
    return core::ptr::null_mut();
// skip softirq field: "REC->vec"
    for (sym = args.next.symbol.symbols; sym != core::ptr::null_mut(); sym = sym.next) {
...
unsafe extern "C" fn softirq_work_name(work: *mut kwork_work, buf: *mut c_char, len: c_int) {
    ...
    struct machine *machine)
    {
    char *modp = core::ptr::null_mut();
    let mut function_addr: c_ulonglong = perf_sample__intval(sample, "function");
    work.class = class;
    work.cpu = sample.cpu;
...
#[no_mangle]
unsafe extern "C" fn workqueue_work_name(work: *mut kwork_work, buf: *mut c_char, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn sched_work_name(work: *mut kwork_work, buf: *mut c_char, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn print_separator(len: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(KWORK_REPORT_LATENCY: kwork->report ==) -> else {
...
unsafe extern "C" fn print_summary(kwork: *mut perf_kwork) {
    ...
    {
    let mut time: u64 = kwork.timeend - kwork.timestart;
...
#[no_mangle]
unsafe extern "C" fn nr_list_entry(head: *mut list_head) -> c_ulonglong {
    static unsigned long long nr_list_entry(struct list_head *head)
    {
    struct list_head *pos;
    let mut n: c_ulonglong = 0;
...
unsafe extern "C" fn top_print_cpu_usage(kwork: *mut perf_kwork) {
    ...
    {
    struct kwork_top_stat *stat = &kwork.top_stat;
    let mut idle_time: u64 = stat.cpus_runtime[MAX_NR_CPUS].idle;
    let mut hardirq_time: u64 = stat.cpus_runtime[MAX_NR_CPUS].irq;
    let mut softirq_time: u64 = stat.cpus_runtime[MAX_NR_CPUS].softirq;
    let mut cpus_nr: c_int = bitmap_weight(stat.all_cpus_bitmap, MAX_NR_CPUS);
    let mut cpus_total_time: u64 = stat.cpus_runtime[MAX_NR_CPUS].total;
...
#[no_mangle]
unsafe extern "C" fn top_print_header(__maybe_unused: *mut *mut perf_kwork kwork) {
...
#[no_mangle]
unsafe extern "C" fn top_print_work(__maybe_unused: *mut *mut perf_kwork kwork, work: *mut kwork_work) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_kwork__sort(kwork: *mut perf_kwork) {
    ...
    list_for_each_entry(class, &kwork.class_list, list)
    if ((class.class_init != core::ptr::null_mut()) &&
    ...
    if (kwork.time_str != core::ptr::null_mut()) {
...
unsafe extern "C" fn perf_kwork__read_events(kwork: *mut perf_kwork) -> c_int {
    ...
    {
    let mut ret: c_int = -1;
    struct perf_session *session = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn perf_kwork__report_bpf(kwork: *mut perf_kwork) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn top_calc_total_runtime(kwork: *mut perf_kwork) {
...
#[no_mangle]
unsafe extern "C" fn top_calc_cpu_usage(kwork: *mut perf_kwork) {
...
unsafe extern "C" fn top_merge_tasks(kwork: *mut perf_kwork) {
    ...
    struct rb_node *node;
    int cpu;
    let mut merged_root: rb_root_cached = RB_ROOT_CACHED;
    class = get_kwork_class(kwork, KWORK_CLASS_SCHED);
    if (!class)
    return;
    for (;;) {
    ...
    if (merged_work.name == core::ptr::null_mut() && data.name != core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn perf_kwork__top_report(kwork: *mut perf_kwork) {
...
#[no_mangle]
unsafe extern "C" fn perf_kwork__top_bpf(kwork: *mut perf_kwork) -> c_int {
...
//
    if (kwork.event_list_str == core::ptr::null_mut())
    ...
    {
    const char **rec_argv, **to_free = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn cmd_kwork(argc: c_int, argv: *const c_char) -> c_int {
    ...
    .tp_handler          = core::ptr::null_mut(),
    .profile_name        = core::ptr::null_mut(),
    .cpu_list            = core::ptr::null_mut(),
    .time_str            = core::ptr::null_mut(),
    .force               = false,
    .event_list_str      = core::ptr::null_mut(),
    .summary             = false,
    ...
    "record", "report", "latency", "timehist", "top", core::ptr::null_mut()
    };
    let mut ret: c_int = 0;
