...
//! Source: tools/perf/builtin-mem.c
#![no_std]
...
pub const MEM_OPERATION_LOAD: c_uint = 0x1;
pub const MEM_OPERATION_STORE: c_uint = 0x2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_mem {
    pub tool: perf_tool,
...
//
    ...
    if (al.filtered || (mem.hide_unresolved && al.sym == core::ptr::null_mut()))
    goto out_put;
...
#[no_mangle]
unsafe extern "C" fn report_raw_events(mem: *mut perf_mem) -> c_int {
    ...
    {
    let mut has_extra_options: bool = (mem.phys_addr | mem.data_page_size) ? true : false;
    char sort[128];
    if (mem.sort_key)
    scnprintf(sort, sizeof(sort), "--sort=%s", mem.sort_key);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mem->data_type) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(MEM_OPERATION_LOAD): !(mem->operation &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn cmd_mem(argc: c_int, argv: *const c_char) -> c_int {
...
//
    ...
    const char *const mem_subcommands[] = { "record", "report", core::ptr::null_mut() };
