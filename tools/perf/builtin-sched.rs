...
//! Source: tools/perf/builtin-sched.c
#![no_std]
...
pub const COMM_LEN: c_int = 20;
pub const SYM_LEN: c_int = 129;
pub const MAX_PID: c_int = 1024000;

pub const MAX_PRIO: c_int = 140;
pub const SEP_LEN: c_int = 100;
pub const NUM_LAT_BUCKETS: c_int = 22;
    enum hist_mode {
    HIST_MODE_LOG = 0,
    HIST_MODE_LINEAR,
    };
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct task_desc {
    pub nr: c_ulong,
    pub pid: c_ulong,
    pub comm: [c_char; COMM_LEN],
    pub nr_events: c_ulong,
    pub curr_event: c_ulong,
    pub atoms: *mut sched_atom,
    pub thread: pthread_t,
    pub ready_for_work: sem_t,
    pub work_done_sem: sem_t,
    pub cpu_usage: u64,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sched_atom {
    pub type: enum sched_event_type,
    pub timestamp: u64,
    pub duration: u64,
    pub nr: c_ulong,
    pub wait_sem: *mut sem_t,
    pub wakee: *mut task_desc,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct work_atom {
    pub list: list_head,
    pub state: enum thread_state,
    pub sched_out_time: u64,
    pub wake_up_time: u64,
    pub sched_in_time: u64,
    pub runtime: u64,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct work_atoms {
    pub work_list: list_head,
    pub thread: *mut thread,
    pub node: rb_node,
    pub max_lat: u64,
    pub max_lat_start: u64,
    pub max_lat_end: u64,
    pub total_lat: u64,
    pub nb_atoms: u64,
    pub total_runtime: u64,
    pub num_merged: c_int,
    pub hist: [u64; NUM_LAT_BUCKETS],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct trace_sched_handler {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_sched_map {
    pub MAX_CPUS): DECLARE_BITMAP(comp_cpus_mask,,
    pub comp_cpus: *mut perf_cpu,
    pub comp: bool,
    pub color_pids: *mut perf_thread_map,
    pub color_pids_str: *const c_char,
    pub color_cpus: *mut perf_cpu_map,
    pub color_cpus_str: *const c_char,
    pub task_name: *const c_char,
    pub task_names: *mut strlist,
    pub fuzzy: bool,
    pub cpus: *mut perf_cpu_map,
    pub cpus_str: *const c_char,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_sched {
    pub tool: perf_tool,
    pub sort_order: *const c_char,
    pub nr_tasks: c_ulong,
    pub pid_to_task: *mut task_desc,
    pub tasks: *mut task_desc,
    pub tp_handler: *const trace_sched_handler,
    pub start_work_mutex: mutex,
    pub work_done_wait_mutex: mutex,
    pub profile_cpu: c_int,
//
// Track the current task - that way we can know whether there's any
// weird events, such as a task being switched away that is not current.
//
    pub max_cpu: perf_cpu,
    pub curr_pid: *mut u32,
    pub curr_thread: *mut thread,
    pub curr_out_thread: *mut thread,
    pub next_shortname1: c_char,
    pub next_shortname2: c_char,
    pub replay_repeat: c_uint,
    pub nr_run_events: c_ulong,
    pub nr_sleep_events: c_ulong,
    pub nr_wakeup_events: c_ulong,
    pub nr_sleep_corrections: c_ulong,
    pub nr_run_events_optimized: c_ulong,
    pub targetless_wakeups: c_ulong,
    pub multitarget_wakeups: c_ulong,
    pub nr_runs: c_ulong,
    pub nr_timestamps: c_ulong,
    pub nr_unordered_timestamps: c_ulong,
    pub nr_context_switch_bugs: c_ulong,
    pub nr_events: c_ulong,
    pub nr_lost_chunks: c_ulong,
    pub nr_lost_events: c_ulong,
    pub run_measurement_overhead: u64,
    pub sleep_measurement_overhead: u64,
    pub start_time: u64,
    pub cpu_usage: u64,
    pub runavg_cpu_usage: u64,
    pub parent_cpu_usage: u64,
    pub runavg_parent_cpu_usage: u64,
    pub sum_runtime: u64,
    pub sum_fluct: u64,
    pub run_avg: u64,
    pub all_runtime: u64,
    pub all_count: u64,
    pub cpu_last_switched: *mut u64,
    pub merged_atom_root: rb_root_cached atom_root, sorted_atom_root,,
    pub cmp_pid: list_head sort_list,,
    pub force: bool,
    pub skip_merge: bool,
    pub show_histogram: bool,
    pub hist_mode: enum hist_mode,
    pub hist_mode_str: *const c_char,
    pub global_hist: [u64; NUM_LAT_BUCKETS],
    pub map: perf_sched_map,
// options for timehist command
    ...
    pub show_cpu_visual: bool,
    pub show_wakeups: bool,
    pub show_next: bool,
    pub show_migrations: bool,
    pub pre_migrations: bool,
    pub show_state: bool,
    pub show_prio: bool,
    pub skipped_samples: u64,
    pub time_str: *const c_char,
    pub ptime: perf_time_interval,
    pub hist_time: perf_time_interval,
    pub thread_funcs_exit: volatile bool,
    pub prio_str: *const c_char,
    pub MAX_PRIO): DECLARE_BITMAP(prio_bitmap,,
    pub session: *mut perf_session,
    pub data: *mut perf_data,
}

#[no_mangle]
unsafe extern "C" fn scnprintf_latency_unit(buf: *mut c_char, size: usize, nsecs: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn latency_bucket(sched: *mut perf_sched, delta_ns: u64) -> c_int {
    static int latency_bucket(struct perf_sched *sched, u64 delta_ns)
    {
    let mut delta_us: u64 = delta_ns / NSEC_PER_USEC;
    u64 b;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thread_runtime {
    pub /: *mut *mut u64 last_time; / time of previous sched in/out event,
    pub /: *mut *mut u64 dt_run; / run time,
    pub /: *mut *mut u64 dt_sleep; / time between CPU access by sleep (off cpu),
    pub /: *mut *mut u64 dt_iowait; / time between CPU access by iowait (off cpu),
    pub /: *mut *mut u64 dt_preempt; / time between CPU access by preempt (off cpu),
    pub /: *mut *mut u64 dt_delay; / time between wakeup and sched-in,
    pub /: *mut *mut u64 dt_pre_mig; / time between migration and wakeup,
    pub /: *mut *mut u64 ready_to_run; / time of wakeup,
    pub /: *mut *mut u64 migrated; / time when a thread is migrated,
    pub run_stats: stats,
    pub total_run_time: u64,
    pub total_sleep_time: u64,
    pub total_iowait_time: u64,
    pub total_preempt_time: u64,
    pub total_delay_time: u64,
    pub total_pre_mig_time: u64,
    pub last_state: c_char,
    pub shortname: [c_char; 3],
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct evsel_runtime {
    pub /: *mut *mut *mut u64 last_time; / time this event was last seen per cpu,
    pub /: *mut *mut u32 ncpu; / highest cpu slot allocated,
}

// per cpu idle time data
#[repr(C)]
#[derive(Copy, Clone)]
pub struct idle_thread_runtime {
    pub tr: thread_runtime,
    pub last_thread: *mut thread,
    pub sorted_root: rb_root_cached,
    pub callchain: callchain_root,
    pub cursor: callchain_cursor,
}
...
#[no_mangle]
unsafe extern "C" fn get_nsecs() -> u64 {
...
#[no_mangle]
unsafe extern "C" fn burn_nsecs(sched: *mut perf_sched, nsecs: u64) {
    static void burn_nsecs(struct perf_sched *sched, u64 nsecs)
    {
    let mut T0: u64 = get_nsecs(), T1;
    do {
    T1 = get_nsecs();
    } while (T1 + sched.run_measurement_overhead < T0 + nsecs);
    }
#[no_mangle]
unsafe extern "C" fn sleep_nsecs(nsecs: u64) {
...
#[no_mangle]
unsafe extern "C" fn calibrate_run_measurement_overhead(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn calibrate_sleep_measurement_overhead(sched: *mut perf_sched) {
    ...
    let mut idx: c_ulong = task.nr_events;
    size_t size;
...
// perf.data is untrusted — cap pid to prevent overflow in size calculations
    ...
    if (sched.pid_to_task == core::ptr::null_mut()) {
    ...
    sched.pid_to_task[pid_max++] = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn print_task_traces(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn add_cross_task_wakeups(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn get_cpu_usage_nsec_parent() -> u64 {
...
#[no_mangle]
unsafe extern "C" fn self_open_counters(sched: *mut perf_sched, cur_task: c_ulong) -> c_int {
    ...
    int fd;
    struct rlimit limit;
    let mut need_privilege: bool = false;
...
#[no_mangle]
unsafe extern "C" fn get_cpu_usage_nsec_self(fd: c_int) -> u64 {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sched_thread_parms {
    pub task: *mut task_desc,
    pub sched: *mut perf_sched,
    pub fd: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn create_tasks(sched: *mut perf_sched) {
    ...
    BUG_ON(parms == core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn destroy_tasks(sched: *mut perf_sched) {
...
// Get rid of threads so they won't be upset by mutex destrunction
    for (i = 0; i < sched.nr_tasks; i++) {
    task = sched.tasks[i];
    err = pthread_join(task.thread, core::ptr::null_mut());
    BUG_ON(err);
...
#[no_mangle]
unsafe extern "C" fn wait_for_tasks(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn run_one_test(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn test_calibrations(sched: *mut perf_sched) {
    ...
    let mut pid: u32 = perf_sample__intval(sample, "pid");
    ...
    if (waker == core::ptr::null_mut() || wakee == core::ptr::null_mut())
    return -1;
...
// next_comm  = perf_sample__strval(sample, "next_comm");
    ...
    if (prev == core::ptr::null_mut() || next == core::ptr::null_mut())
    return -1;
    ...
    event.fork.ptid);
    if (child == core::ptr::null_mut() || parent == core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn init_prio(r: *mut thread_runtime) {
...
//
    ...
    let mut key: work_atoms = { .thread = thread };
    while (node) {
...
#[no_mangle]
unsafe extern "C" fn thread_atoms_insert(sched: *mut perf_sched, thread: *mut thread) -> c_int {
    ...
    u64 timestamp)
    {
    struct work_atom *atom = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn free_work_atoms(atoms: *mut work_atoms) {
    ...
    const u32 prev_pid = perf_sample__intval(sample, "prev_pid"),
    next_pid = perf_sample__intval(sample, "next_pid");
    let mut prev_state: c_char = perf_sample__taskstate(sample, "prev_state");
    struct work_atoms *out_events, *in_events;
    struct thread *sched_out, *sched_in;
    u64 timestamp0, timestamp = sample.time;
    let mut cpu: c_int = sample.cpu, err = -1;
    s64 delta;
// perf.data is untrusted input — CPU may be absent or corrupted
    ...
    if (sched_out == core::ptr::null_mut() || sched_in == core::ptr::null_mut())
    goto out_put;
...
//
    ...
    struct work_atoms *atoms;
    let mut timestamp: u64 = sample.time;
    let mut cpu: c_int = sample.cpu, err = -1;
    if (perf_time__skip_sample(&sched.ptime, timestamp))
    return 0;
...
#[no_mangle]
unsafe extern "C" fn output_lat_thread(sched: *mut perf_sched, work_list: *mut work_atoms) {
...
#[no_mangle]
unsafe extern "C" fn pid_cmp(l: *mut work_atoms, r: *mut work_atoms) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn avg_cmp(l: *mut work_atoms, r: *mut work_atoms) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn max_cmp(l: *mut work_atoms, r: *mut work_atoms) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn switch_cmp(l: *mut work_atoms, r: *mut work_atoms) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn runtime_cmp(l: *mut work_atoms, r: *mut work_atoms) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sort_dimension__add(tok: *const c_char, list: *mut list_head) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sched__sort_lat(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn thread__has_color(thread: *mut thread) -> bool {
    static bool thread__has_color(struct thread *thread)
    {
    struct thread_runtime *tr = thread__priv(thread);
    return tr != core::ptr::null_mut() && tr.color;
    }
...
#[no_mangle]
unsafe extern "C" fn sched_match_task(sched: *mut perf_sched, comm_str: *const c_char) -> bool {
    static bool sched_match_task(struct perf_sched *sched, const char *comm_str)
    {
    let mut fuzzy_match: bool = sched.map.fuzzy;
    ...
    struct machine *machine)
    {
    let mut next_pid: u32 = perf_sample__intval(sample, "next_pid");
    let mut prev_pid: u32 = perf_sample__intval(sample, "prev_pid");
...
// perf.data is untrusted input — CPU may be absent or corrupted
    ...
    if (sched_in == core::ptr::null_mut() || sched_out == core::ptr::null_mut())
    goto out;
...
//
    ...
    if (tr == core::ptr::null_mut() || strcmp(tr.shortname, "") == 0)
    goto out;
    if (proceed == 1)
    color_fprintf(stdout, color, "\n");
    printf("  ");
    ...
    {
    struct perf_sched *sched = container_of(tool, struct perf_sched, tool);
    let mut this_cpu: c_int = sample.cpu, err = 0;
...
#[no_mangle]
unsafe extern "C" fn perf_sched__read_events(sched: *mut perf_sched) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn print_sched_time(nsecs: c_ulonglong, width: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn evsel__save_time(evsel: *mut evsel, timestamp: u64, cpu: u32) {
    static void evsel__save_time(struct evsel *evsel, u64 timestamp, u32 cpu)
    {
    struct evsel_runtime *r = evsel__get_runtime(evsel);
    if (r == core::ptr::null_mut())
    return;
    if ((cpu >= r.ncpu) || (r.last_time == core::ptr::null_mut())) {
...
#[no_mangle]
unsafe extern "C" fn evsel__get_time(evsel: *mut evsel, cpu: u32) -> u64 {
    static u64 evsel__get_time(struct evsel *evsel, u32 cpu)
    {
    struct evsel_runtime *r = evsel__get_runtime(evsel);
    if ((r == core::ptr::null_mut()) || (r.last_time == core::ptr::null_mut()) || (cpu >= r.ncpu))
    return 0;
    return r.last_time[cpu];
    }
#[no_mangle]
unsafe extern "C" fn timehist__evsel_priv_destructor(priv: *mut c_void) {
    ...
    let mut comm_width: static int = 30;
    static char *timehist_get_commstr(struct thread *thread)
    {
    static char str[32];
    const char *comm = thread__comm_str(thread);
    let mut tid: pid_t = thread__tid(thread);
    let mut pid: pid_t = thread__pid(thread);
    int n;
    if (pid == 0)
    n = scnprintf(str, sizeof(str), "%s", comm);
#[no_mangle]
pub unsafe extern "C" fn if(pid: tid !=) -> else {
    else if (tid != pid)
...
pub const MAX_PRIO_STR_LEN: c_int = 8;
    static char *timehist_get_priostr(struct thread *thread,
    struct perf_sample *sample)
    {
    static char prio_str[16];
    let mut prev_prio: c_int = (int)perf_sample__intval(sample, "prev_prio");
    struct thread_runtime *tr = thread__priv(thread);
...
#[no_mangle]
unsafe extern "C" fn timehist_header(sched: *mut perf_sched) {
    static void timehist_header(struct perf_sched *sched)
    {
    let mut ncpus: u32 = sched.max_cpu.cpu + 1;
    u32 i, j;
...
//
    ...
    let mut next_pid: u32 = perf_sample__intval(sample, "next_pid");
    let mut max_cpus: u32 = sched.max_cpu.cpu + 1;
    char tstr[64];
    char nstr[30];
    u64 wait_time;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: r->last_time) -> else {
    let mut dt_wait: u64 = tprev - r.last_time;
    if (r.last_state == 'R')
    r.dt_preempt = dt_wait;
#[no_mangle]
pub unsafe extern "C" fn if('D': r->last_state ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn is_idle_sample(sample: *mut perf_sample) -> bool {
...
// want main thread for process - has maps
    ...
    if (!sched.show_callchain || sample.callchain == core::ptr::null_mut()) {
    thread__put(thread);
    return;
    }
    cursor = get_tls_callchain_cursor();
    if (thread__resolve_callchain(thread, cursor, sample,
    core::ptr::null_mut(), core::ptr::null_mut(), sched.max_stack + 2) != 0) {
    if (verbose > 0)
...
//
#[no_mangle]
unsafe extern "C" fn init_idle_threads(ncpu: c_int) -> c_int {
...
// allocate the actual thread struct if needed
    ...
    if (idle_threads[i] == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn free_idle_threads() {
    static void free_idle_threads(void)
    {
    int i;
    if (idle_threads == core::ptr::null_mut())
    return;
...
// structs if needed
//
    if ((cpu >= idle_max_cpu) || (idle_threads == core::ptr::null_mut())) {
    ...
    idle_threads[i] = core::ptr::null_mut();
    idle_max_cpu = j;
    }
// allocate a new thread struct if needed
    if (idle_threads[cpu] == core::ptr::null_mut()) {
...
// copy task callchain when entering to idle
    ...
    struct thread_runtime *tr = core::ptr::null_mut();
...
//
    ...
    prio = tr.prio;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: evsel__name_is(sample->evsel, _arg: "sched:sched_switch")) -> else {
...
// show migrations if requested
    ...
    struct thread_runtime *tr = core::ptr::null_mut();
    let mut next_pid: u32 = perf_sample__intval(sample, "next_pid");
    let mut next_prio: u32 = perf_sample__intval(sample, "next_prio");
    if (next_pid == 0)
    thread = get_idle_thread(sample.cpu);
    else
    ...
    struct addr_location al;
    struct thread *thread = core::ptr::null_mut();
    struct thread_runtime *tr = core::ptr::null_mut();
    u64 tprev, t = sample.time;
    let mut rc: c_int = 0;
    let mut state: c_char = perf_sample__taskstate(sample, "prev_state");
// perf.data is untrusted input — CPU may be absent or corrupted
    if (sample.cpu >= MAX_CPUS) {
    pr_warning("WARNING: at offset %#" PRIx64 ": out-of-bound sample CPU %d, skipping sample\n",
    sample.file_offset, sample.cpu);
...
// sched out event for task so reset ready to run time and migrated time
    ...
    struct thread_runtime *r)
    {
    let mut mean: double = avg_stats(&r.run_stats);
    float stddev;
...
#[no_mangle]
unsafe extern "C" fn show_thread_runtime(t: *mut thread, priv: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn callchain__fprintf_folded(fp: *mut FILE, node: *mut callchain_node) -> usize {
...
#[no_mangle]
unsafe extern "C" fn timehist_print_idlehist_callchain(root: *mut rb_root_cached) -> usize {
...
// sort() uses rb_insert_color() on rb_root, not rb_root_cached
    ...
    struct thread_runtime *r;
    int i;
    let mut hist_time: u64 = sched.hist_time.end - sched.hist_time.start;
...
// max_cpu indexes arrays allocated with MAX_CPUS entries
    ...
    sched.max_cpu = this_cpu;
    if (evsel.handler != core::ptr::null_mut()) {
    let mut f: sched_handler = evsel.handler;
...
#[no_mangle]
unsafe extern "C" fn timehist_parse_prio_str(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sched__timehist(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn print_bad_events(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn __merge_work_atoms(root: *mut rb_root_cached, data: *mut work_atoms) {
...
#[no_mangle]
unsafe extern "C" fn perf_sched__merge_lat(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn setup_cpus_switch_event(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_cpus_switch_event(sched: *mut perf_sched) {
...
#[no_mangle]
unsafe extern "C" fn perf_sched__lat(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(sched->hist_mode_str, _arg: "log")) -> else {
...
#[no_mangle]
unsafe extern "C" fn setup_map_cpus(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setup_color_pids(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setup_color_cpus(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sched__map(sched: *mut perf_sched) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn perf_sched__replay(sched: *mut perf_sched) -> c_int {
    ...
    tok; tok = strtok_r(core::ptr::null_mut(), ", ", &tmp)) {
...
#[no_mangle]
unsafe extern "C" fn enable_sched_schedstats(reset: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn disable_sched_schedstat() -> c_int {
...
//
    ...
    while (!done) {
    if (argc && waitpid(evlist__workload_pid(evlist), core::ptr::null_mut(), WNOHANG) > 0)
    break;
    sleep(1);
    }
    if (reset) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct schedstat_domain {
    pub domain_list: list_head,
    pub domain_data: *mut perf_record_schedstat_domain,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct schedstat_cpu {
    pub cpu_list: list_head,
    pub domain_head: list_head,
    pub cpu_data: *mut perf_record_schedstat_cpu,
}

    let mut cpu_head: static struct list_head = LIST_HEAD_INIT(cpu_head);
...
#[no_mangle]
unsafe extern "C" fn store_schedstat_cpu_diff(after_workload: *mut schedstat_cpu) {
    ...
    let mut version: __u16 = after_workload.cpu_data.version;
...
#[no_mangle]
unsafe extern "C" fn store_schedstat_domain_diff(after_workload: *mut schedstat_domain) {
    ...
    let mut version: __u16 = after_workload.domain_data.version;
...
//
#[no_mangle]
unsafe extern "C" fn get_all_cpu_stats(head: *mut list_head) -> c_int {
    static int get_all_cpu_stats(struct list_head *head)
    {
    struct schedstat_cpu *cptr, *summary_head = core::ptr::null_mut();
    ...
    struct perf_record_schedstat_domain *ds1 = core::ptr::null_mut(), *ds2 = core::ptr::null_mut();
    struct schedstat_domain *dptr1 = core::ptr::null_mut(), *dptr2 = core::ptr::null_mut();
    struct schedstat_cpu *cptr2 = core::ptr::null_mut();
    let mut jiffies1: __u64 = 0, jiffies2 = 0;
    let mut is_summary: bool = true;
    let mut ret: c_int = 0;
    if (!cd_map1) {
    ...
    struct cpu_domain_map *cd_info1 = core::ptr::null_mut(), *cd_info2 = core::ptr::null_mut();
    struct perf_record_schedstat_cpu *cs1 = cptr1.cpu_data;
    struct perf_record_schedstat_cpu *cs2 = core::ptr::null_mut();
    dptr2 = core::ptr::null_mut();
    if (cs1.cpu >= (u32)nr1) {
    ...
    struct domain_info *dinfo1 = core::ptr::null_mut(), *dinfo2 = core::ptr::null_mut();
    ds1 = dptr1.domain_data;
    ds2 = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn free_schedstat(head: *mut list_head) {
    static void free_schedstat(struct list_head *head)
    {
...
#[no_mangle]
unsafe extern "C" fn perf_sched__schedstat_report(sched: *mut perf_sched) -> c_int {
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), 0, false);
    }
    out:
    ...
    int argc, const char **argv)
    {
    struct cpu_domain_map **cd_map0 = core::ptr::null_mut(), **cd_map1 = core::ptr::null_mut();
...
pub unsafe extern "C" fn if(2: argc ==) -> else {
    ...
    struct machine *machine __maybe_unused)
    {
    return perf_sched__process_schedstat(tool, core::ptr::null_mut(), event);
    }
    static int perf_sched__schedstat_live(struct perf_sched *sched,
    int argc, const char **argv)
    {
    struct cpu_domain_map **cd_map = core::ptr::null_mut();
    let mut target: target = {};
...
//
    ...
    err = show_schedstat_data(&cpu_head, cd_map, nr, core::ptr::null_mut(), core::ptr::null_mut(), 0, false);
...
#[no_mangle]
unsafe extern "C" fn schedstat_events_exposed() -> bool {
...
//
    ...
    if (rec_argv_copy == core::ptr::null_mut()) {
    free(rec_argv);
    return -ENOMEM;
    }
...
#[no_mangle]
pub unsafe extern "C" fn cmd_sched(argc: c_int, argv: *const c_char) -> c_int {
    ...
    "replay", "script",
    "timehist", "stats", core::ptr::null_mut() };
    ...
    let mut stats_ops: trace_sched_handler = {};
    int ret;
...
//
    ...
    if (sched.map.task_name) {
    sched.map.task_names = strlist__new(sched.map.task_name, core::ptr::null_mut());
    if (sched.map.task_names == core::ptr::null_mut()) {
    ...
    if (sched.show_wakeups)
    parse_options_usage(core::ptr::null_mut(), timehist_options, "w", true);
    if (sched.show_next)
    parse_options_usage(core::ptr::null_mut(), timehist_options, "n", true);
    ret = -EINVAL;
    goto out;
    }
    ...
    const char *const stats_subcommands[] = {"record", "report", core::ptr::null_mut()};
