...
//! Source: tools/perf/builtin-report.c
#![no_std]
...
unsafe extern "C" fn report__config(var: *const c_char, value: *const c_char, cb: *mut c_void) -> c_int {
    ...
    let mut pcnt: double = strtof(value, core::ptr::null_mut());
    ...
    let mut ret: c_int = perf_event__process_feature(tool, session, event);
...
//
    ...
    if (symbol_conf.hide_unresolved && al.sym == core::ptr::null_mut())
    goto out_put;
...
#[no_mangle]
unsafe extern "C" fn report__setup_sample_type(rep: *mut report) -> c_int {
    static int report__setup_sample_type(struct report *rep)
    {
    struct perf_session *session = rep.session;
    let mut sample_type: u64 = evlist__combined_sample_type(session.evlist);
    let mut is_pipe: bool = perf_data__is_pipe(session.data);
    struct evsel *evsel;
...
unsafe extern "C" fn sig_handler(__maybe_unused: int sig) {
    ...
    size_t ret;
    char unit;
    let mut nr_samples: c_ulong = hists.stats.nr_samples;
    let mut nr_events: u64 = hists.stats.total_period;
    struct evsel *evsel = hists_to_evsel(hists);
    char buf[512];
    let mut size: usize = sizeof(buf);
    let mut socked_id: c_int = hists.socket_filter;
    if (quiet)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn evlist__tui_block_hists_browse(evlist: *mut evlist, rep: *mut report) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn evlist__tty_browse_hists(evlist: *mut evlist, rep: *mut report, help: *const c_char) -> c_int {
    ...
    rep.min_percent, pos, core::ptr::null_mut());
    continue;
    }
    ...
    fprintf(stdout, "#\n# (%s)\n#\n", help);
    if (rep.show_threads) {
    let mut style: bool = !strcmp(rep.pretty_printing_style, "raw");
...
#[no_mangle]
unsafe extern "C" fn report__warn_kptr_restrict(rep: *const report) {
    static void report__warn_kptr_restrict(const struct report *rep)
    {
    struct map *kernel_map = machine__kernel_map(&rep.session.machines.host);
    struct kmap *kernel_kmap = kernel_map ? map__kmap(kernel_map) : core::ptr::null_mut();
    if (evlist__exclude_kernel(rep.session.evlist))
    return;
    if (kernel_map == core::ptr::null_mut() ||
    (dso__hit(map__dso(kernel_map)) &&
    (kernel_kmap.ref_reloc_sym == core::ptr::null_mut() ||
...
#[no_mangle]
unsafe extern "C" fn report__gtk_browse_hists(rep: *mut report, help: *const c_char) -> c_int {
    ...
    if (hist_browser == core::ptr::null_mut()) {
    ui__error("GTK browser not found!\n");
    return -1;
    }
    return hist_browser(rep.session.evlist, help, core::ptr::null_mut(), rep.min_percent);
    }
#[no_mangle]
unsafe extern "C" fn report__browse_hists(rep: *mut report) -> c_int {
    static int report__browse_hists(struct report *rep)
    {
    int ret;
    struct perf_session *session = rep.session;
    struct evlist *evlist = session.evlist;
    char *help = core::ptr::null_mut(), *path = core::ptr::null_mut();
    path = system_path(TIPDIR);
    if (perf_tip(&help, path) || help == core::ptr::null_mut()) {
// fallback for people who don't install perf ;-)
    ...
    ret = evlist__tui_browse_hists(evlist, help, core::ptr::null_mut(), rep.min_percent,
...
#[no_mangle]
unsafe extern "C" fn report__collapse_hists(rep: *mut report) -> c_int {
...
//
    ...
    strdup(rep.symbol_filter_str) : core::ptr::null_mut();
    }
...
unsafe extern "C" fn hists__resort_cb(he: *mut hist_entry, arg: *mut c_void) -> c_int {
    ...
    symbol__annotate2(&he.ms, evsel, core::ptr::null_mut());
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn report__output_resort(rep: *mut report) {
    ...
    let mut count: u32 = event.lost_samples.lost;
...
#[no_mangle]
unsafe extern "C" fn stats_setup(rep: *mut report) {
...
#[no_mangle]
unsafe extern "C" fn stats_print(rep: *mut report) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tasks_setup(rep: *mut report) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct maps__fprintf_task_args {
    pub indent: c_int,
    pub fp: *mut FILE,
    pub printed: usize,
}
...
unsafe extern "C" fn maps__fprintf_task_cb(map: *mut map, data: *mut c_void) -> c_int {
    ...
    let mut prot: u32 = map__prot(map);
...
#[no_mangle]
unsafe extern "C" fn maps__fprintf_task(maps: *mut maps, indent: c_int, fp: *mut FILE) -> usize {
...
#[no_mangle]
unsafe extern "C" fn thread_level(machine: *mut machine, thread: *const thread) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn task__print_level(machine: *mut machine, thread: *mut thread, fp: *mut FILE) {
    static void task__print_level(struct machine *machine, struct thread *thread, FILE *fp)
    {
    let mut level: c_int = thread_level(machine, thread);
...
//
#[no_mangle]
unsafe extern "C" fn task_list_cmp(priv: *mut c_void, la: *const list_head, lb: *const list_head) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tasks_print(rep: *mut report, fp: *mut FILE) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __cmd_report(rep: *mut report) -> c_int {
...
// time_q *= NSEC_PER_USEC;
    ...
    {
    if (arg) {
    let mut err: c_int = regcomp(&ignore_callees_regex, arg, REG_EXTENDED);
    if (err) {
    char buf[BUFSIZ];
...
// branch_mode = !unset;
    ...
    const char *arg, int unset __maybe_unused)
    {
    return addr2line_configure("addr2line.style", arg, core::ptr::null_mut());
    }
    static int process_attr(const struct perf_tool *tool __maybe_unused,
    union perf_event *event,
    struct evlist **pevlist)
...
#[no_mangle]
pub unsafe extern "C" fn cmd_report(argc: c_int, argv: *const c_char) -> c_int {
    int cmd_report(int argc, const char **argv)
    {
    struct perf_session *session;
    let mut itrace_synth_opts: itrace_synth_opts = { .set = 0, };
    struct stat st;
    let mut has_br_stack: bool = false;
    let mut branch_mode: c_int = -1;
    let mut last_key: c_int = 0;
    let mut branch_call_mode: bool = false;
    ...
    OPT_CALLBACK(0, "ignore-callees", core::ptr::null_mut(), "regex",
    ...
    OPT_CALLBACK(0, "addr2line-style", core::ptr::null_mut(), "addr2line style",
    ...
    char sort_tmp[128];
    let mut ordered_events: bool = true;
    if (ret < 0)
    goto exit;
//
...
//
    ...
    parse_options_usage(core::ptr::null_mut(), options, "group-sort-idx", 0);
    ret = -EINVAL;
    goto error;
    }
...
//
    ...
    if (sort_order == core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: report.use_tui) -> else {
    else if (report.use_tui)
    use_browser = 1;

#[no_mangle]
pub unsafe extern "C" fn if(_arg: report.use_gtk) -> else {
...
//
    ...
    parse_options_usage(report_usage, options, "s", 1);
    if (field_order)
    parse_options_usage(sort_order ? core::ptr::null_mut() : report_usage,
    options, "F", 1);
    goto error;
    }
...
//
    ...
    report.block_reports = core::ptr::null_mut();
    }
