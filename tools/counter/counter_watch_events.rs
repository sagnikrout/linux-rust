...
//! Source: tools/counter/counter_watch_events.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn print_watch(watch: *mut counter_watch, nwatch: c_int) {
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    char *device_name = core::ptr::null_mut(), *subopts, *value;
...
//
    while ((c = getopt_long(argc, argv, "dhn:l:w:", longopts, core::ptr::null_mut())) != -1) {
    switch (c) {
    case 'd':
    debug = 1;
    break;
    ...
    return EXIT_SUCCESS;
    case 'n':
    dev_num = strtoul(optarg, core::ptr::null_mut(), 10);
    if (errno) {
    ...
    break;
    case 'l':
    loop = strtol(optarg, core::ptr::null_mut(), 10);
    if (errno) {
...
// match counter_event_type: subtract enum value
    ...
    watches[i].channel = strtoul(value, core::ptr::null_mut(), 10);
    if (errno) {
    ...
    watches[i].component.id = strtoul(value, core::ptr::null_mut(), 10);
    if (errno) {
    ...
    watches[i].component.parent = strtoul(value, core::ptr::null_mut(), 10);
    if (errno) {
