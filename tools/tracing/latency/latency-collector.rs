...
//! Source: tools/tracing/latency/latency-collector.c
#![no_std]
...
// Macro flag: #define _GNU_SOURCE
    ...
    let mut trace_enable: static bool = true;
    let mut setup_ftrace: static bool = true;
    static bool use_random_sleep;

    C(FUNC_TR, "function-trace"),		\
    C(DISP_GR, "display-graph"),		\
    C(NR,       core::ptr::null_mut())
    ...
    let mut nr_threads: static unsigned int = DEFAULT_NR_PRINTER_THREADS;

    let mut table_startsize: static unsigned int = DEFAULT_TABLE_SIZE;
    ...
    let mut DEBUG_NOFILE: *const static char  const = "[file not found]";
    let mut TR_MAXLAT: *const static char  const = "tracing_max_latency";
    let mut TR_THRESH: *const static char  const = "tracing_thresh";
    let mut TR_CURRENT: *const static char  const = "current_tracer";
    let mut TR_OPTIONS: *const static char  const = "trace_options";
    let mut NOP_TRACER: *const static char  const = "nop";
    let mut OPT_NO_PREFIX: *const static char  const = "no";

    static const char *threshold = DFLT_THRESHOLD_US;

    let mut sleep_time: static long = (USEC_PER_MSEC * SLEEP_TIME_MS_DEFAULT);
...
pub struct policy {
    ...
    { core::ptr::null_mut(),    0,           DEFAULT_PRI    }
    };
//
...
pub struct ftrace_state {
    pub tracer: *mut c_char,
    pub thresh: *mut c_char,
    pub opt: [bool; OPTIDX_NR],
    pub opt_valid: [bool; OPTIDX_NR],
...
pub struct entry {
    pub ticket: c_int,
    pub ticket_completed_ref: c_int,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct print_state {
    pub ticket_counter: c_int,
    pub ticket_completed: c_int,
    pub mutex: pthread_mutex_t,
    pub cond: pthread_cond_t,
    pub cnt: c_int,
    pub cnt_mutex: pthread_mutex_t,
}
...
#[derive(Copy, Clone)]
pub struct queue {
    pub entries: [entry; QUEUE_SIZE],
    pub next_prod_idx: c_int,
    pub next_cons_idx: c_int,
    pub mutex: pthread_mutex_t,
    pub cond: pthread_cond_t,
}
...
#[no_mangle]
unsafe extern "C" fn write_or_die(fd: c_int, buf: *const c_char, count: usize) -> __always_inline void {
    ...
    struct timespec *tp)
    {
    let mut r: c_int = clock_gettime(clk_id, tp);
    if (unlikely(r != 0))
    err(EXIT_FAILURE, "clock_gettime() failed");
    }
#[no_mangle]
unsafe extern "C" fn sigemptyset_or_die(s: *mut sigset_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn sigaddset_or_die(s: *mut sigset_t, signum: c_int) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn open_stdout() {
    static void open_stdout(void)
    {
    if (setvbuf(stdout, core::ptr::null_mut(), _IONBF, 0) != 0)
...
//
#[no_mangle]
unsafe extern "C" fn mutex_lock(mtx: *mut pthread_mutex_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn mutex_unlock(mtx: *mut pthread_mutex_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn cond_signal(cond: *mut pthread_cond_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn cond_broadcast(cond: *mut pthread_cond_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn mutexattr_init(attr: *mut pthread_mutexattr_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn mutexattr_destroy(attr: *mut pthread_mutexattr_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn condattr_init(attr: *mut pthread_condattr_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn condattr_destroy(attr: *mut pthread_condattr_t) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn init_printstate() {
    ...
    printstate.cnt = 0;
    mutex_init(&printstate.mutex, core::ptr::null_mut());
    condattr_init(&cattr);
...
#[no_mangle]
unsafe extern "C" fn init_print_mtx() {
...
#[no_mangle]
unsafe extern "C" fn signal_blocking(how: c_int) {
    ...
    errno = pthread_sigmask(how, &s, core::ptr::null_mut());
    if (unlikely(errno)) {
...
#[no_mangle]
unsafe extern "C" fn signal_handler(num: c_int) {
    static void signal_handler(int num)
    {
    signal_flag = num;
    }
#[no_mangle]
unsafe extern "C" fn setup_sig_handler() {
    static void setup_sig_handler(void)
    {
    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));
    sa.sa_handler = signal_handler;
    sigaction_or_die(SIGHUP, &sa, core::ptr::null_mut());
    sigaction_or_die(SIGTERM, &sa, core::ptr::null_mut());
    sigaction_or_die(SIGINT, &sa, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn process_signal(signal: c_int) {
...
#[no_mangle]
unsafe extern "C" fn check_signals() -> __always_inline void {
    static __always_inline void check_signals(void)
    {
    let mut signal: c_int = signal_flag;
    if (unlikely(signal))
...
#[no_mangle]
unsafe extern "C" fn time_has_passed(time: *const timespec) -> __always_inline bool {
...
#[no_mangle]
unsafe extern "C" fn mutex_trylock_limit(mutex: *mut pthread_mutex_t, time_ms: c_int) -> bool {
    static bool mutex_trylock_limit(pthread_mutex_t *mutex, int time_ms)
    {
    let mut time_us: c_long = time_ms * USEC_PER_MSEC;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: verbose_ftrace()) -> else {
    ...
    r = tracefs_instance_file_read(core::ptr::null_mut(), file, &psize);
    if (!r) {
    if (h) {
    warn(emsg, file);
    if (h == ERR_CLEANUP)
...
#[no_mangle]
unsafe extern "C" fn restore_file(file: *const c_char, saved: *mut c_char, cur: *const c_char) {
    static void restore_file(const char *file, char **saved, const char *cur)
    {
    if (*saved && was_changed(*saved, cur)) {
    if (tracefs_instance_file_write(core::ptr::null_mut(), file, *saved) < 0)
...
#[no_mangle]
unsafe extern "C" fn restore_ftrace() {
...
#[no_mangle]
unsafe extern "C" fn cleanup_exit(status: c_int) {
    static void cleanup_exit(int status)
    {
    char *maxlat;
    if (!setup_ftrace)
...
#[no_mangle]
unsafe extern "C" fn init_save_state() {
    ...
    mutexattr_destroy(&mattr);
    save_state.tracer = core::ptr::null_mut();
    save_state.thresh = core::ptr::null_mut();
    save_state.opt_valid[OPTIDX_FUNC_TR] = false;
    save_state.opt_valid[OPTIDX_DISP_GR] = false;
    }
#[no_mangle]
unsafe extern "C" fn printstate_next_ticket(req: *mut entry) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn printstate_mark_req_completed(req: *const entry) {
    ...
    static __always_inline
#[no_mangle]
pub unsafe extern "C" fn printstate_has_new_req_arrived(req: *const entry) -> bool {
    bool printstate_has_new_req_arrived(const struct entry *req)
    {
    return (printstate.ticket_counter != req.ticket);
    }
#[no_mangle]
unsafe extern "C" fn printstate_cnt_inc() -> __always_inline int {
...
#[no_mangle]
unsafe extern "C" fn printstate_cnt_dec() -> __always_inline int {
...
#[no_mangle]
unsafe extern "C" fn printstate_cnt_read() -> __always_inline int {
    ...
    static __always_inline
#[no_mangle]
pub unsafe extern "C" fn prev_req_won_race(req: *const entry) -> bool {
    bool prev_req_won_race(const struct entry *req)
    {
    return (printstate.ticket_completed != req.ticket_completed_ref);
    }
#[no_mangle]
unsafe extern "C" fn sleeptable_resize(size: c_int, printout: bool, msg: *mut short_msg) {
...
#[no_mangle]
unsafe extern "C" fn init_queue(q: *mut queue) {
    static void init_queue(struct queue *q)
    {
    q.next_prod_idx = 0;
    q.next_cons_idx = 0;
    mutex_init(&q.mutex, core::ptr::null_mut());
    errno = pthread_cond_init(&q.cond, core::ptr::null_mut());
    if (errno)
    err(EXIT_FAILURE, "pthread_cond_init() failed");
    }
#[no_mangle]
unsafe extern "C" fn queue_len(q: *const queue) -> __always_inline int {
...
#[no_mangle]
unsafe extern "C" fn queue_nr_free(q: *const queue) -> __always_inline int {
    static __always_inline int queue_nr_free(const struct queue *q)
    {
    let mut nr_free: c_int = QUEUE_SIZE - queue_len(q);
//
...
#[no_mangle]
unsafe extern "C" fn queue_idx_inc(idx: *mut c_int) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn queue_pop_from_front(q: *mut queue) -> __always_inline struct entry {
    static __always_inline struct entry queue_pop_from_front(struct queue *q)
    {
    let mut e: entry = q.entries[q.next_cons_idx];
    queue_idx_inc(&q.next_cons_idx);
    return e;
    }
#[no_mangle]
unsafe extern "C" fn queue_cond_signal(q: *mut queue) -> __always_inline void {
    static __always_inline void queue_cond_signal(struct queue *q)
    {
    cond_signal(&q.cond);
    }
#[no_mangle]
unsafe extern "C" fn queue_cond_wait(q: *mut queue) -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn queue_wait_for_entry(q: *mut queue) -> entry {
    ...
    while (p.name != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn is_relevant_tracer(name: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn random_makes_sense(name: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn show_available() {
    static void show_available(void)
    {
    char **tracers;
    let mut found: c_int = 0;
    int i;
    tracers = tracefs_tracers(core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn tracer_valid(name: *const c_char, notracer: *mut bool) -> bool {
...
#[no_mangle]
unsafe extern "C" fn toss_coin(buffer: *mut drand48_data, prob: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn go_to_sleep(req: *const entry) -> c_long {
    static long go_to_sleep(const struct entry *req)
    {
    struct timespec future;
    let mut delay: c_long = sleep_time;
...
#[no_mangle]
unsafe extern "C" fn set_priority() {
...
#[no_mangle]
unsafe extern "C" fn print_priority() {
    ...
    if (resize_msg != core::ptr::null_mut() && resize_msg.len > 0) {
...
// These prints could happen concurrently
    ...
    const char *reason)
    {
    __print_skipmessage(core::ptr::null_mut(), timestamp, buffer, bufspace, req, true,
    reason);
    }
    ...
    {
    let mut reserve: static int = 256;
    char *p = &buffer[0];
    let mut bytes: isize = 0;
    let mut bytes_tot: isize = 0;
    long us, sec;
    long slept_ms;
    int trace_fd;
...
#[no_mangle]
unsafe extern "C" fn get_trace_opt(allopt: *const c_char, opt: *const c_char, found: *mut bool) -> bool {
...
#[no_mangle]
unsafe extern "C" fn set_trace_opt(opt: *const c_char, value: bool) -> c_int {
    static int set_trace_opt(const char *opt, bool value)
    {
    char *str;
    int r;
    ...
    str = get_no_opt(opt);
    r = tracefs_instance_file_write(core::ptr::null_mut(), TR_OPTIONS, str);
    free(str);
    return r;
    }
#[no_mangle]
pub unsafe extern "C" fn save_trace_opts(state: *mut ftrace_state) {
    ...
    int psize;
    int i;
    allopt = tracefs_instance_file_read(core::ptr::null_mut(), TR_OPTIONS, &psize);
    if (!allopt)
...
// Do nothing if we now that the current and new value are equal
    ...
    return;
    r = tracefs_instance_file_write(core::ptr::null_mut(), file, new);
    if (r < 0) {
    if (h) {
    warnx(emsg, file);
    if (h == ERR_CLEANUP)
...
#[no_mangle]
unsafe extern "C" fn reset_max_latency() {
    static void reset_max_latency(void)
    {
    write_file(TR_MAXLAT, core::ptr::null_mut(), "0", ERR_CLEANUP);
    }
#[no_mangle]
unsafe extern "C" fn save_and_disable_tracer() {
    static void save_and_disable_tracer(void)
    {
    char *orig_th;
    char *tracer;
    let mut need_nop: bool = false;
    mutex_lock(&save_state.mutex);
    ...
    if (need_nop)
    write_file(TR_CURRENT, core::ptr::null_mut(), NOP_TRACER, ERR_EXIT);
    mutex_unlock(&save_state.mutex);
    }
#[no_mangle]
pub unsafe extern "C" fn set_trace_opts(state: *mut ftrace_state, new: *mut bool) {
...
#[no_mangle]
unsafe extern "C" fn enable_tracer() {
...
#[no_mangle]
unsafe extern "C" fn tracing_loop() {
    static void tracing_loop(void)
    {
    let mut ifd: c_int = inotify_init();
    int wd;
    let mut bufsize: isize = sizeof(inotify_buffer);
    let mut istructsize: isize = sizeof(struct inotify_event);
    ...
    struct entry req;
    char *buffer;
    let mut bufspace: usize = PRINT_BUFFER_SIZE;
...
#[no_mangle]
unsafe extern "C" fn start_printthread() {
...
// seed = i;
    ...
    errno = pthread_create(&printthread[i], core::ptr::null_mut(), do_printloop,
    seed);
    if (errno)
...
// This is needed in show_usage()
    ...
    if (debug_maxlat_dflt == core::ptr::null_mut()) {
// This is needed in show_usage()
...
#[no_mangle]
pub unsafe extern "C" fn alldigits(s: *const c_char) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn check_alldigits(optarg: *const c_char, argname: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn scan_arguments(argc: c_int, argv[]: *mut c_char) {
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    table_startsize);
    sleeptable_resize(table_startsize, false, core::ptr::null_mut());
    }
    set_priority();
