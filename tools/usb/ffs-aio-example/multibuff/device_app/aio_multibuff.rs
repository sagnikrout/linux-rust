...
//! Source: tools/usb/ffs-aio-example/multibuff/device_app/aio_multibuff.c
#![no_std]
...
pub struct io_buffer {
    ...
    pub cnt: unsigned,
    pub len: unsigned,
    pub requested: unsigned,
}

// Endpoints handling
#[no_mangle]
unsafe extern "C" fn display_event(event: *mut usb_functionfs_event) {
...
#[no_mangle]
unsafe extern "C" fn handle_ep0(ep0: c_int, ready: *mut bool) {
    ...
    write(ep0, core::ptr::null_mut(), 0);
    else
    read(ep0, core::ptr::null_mut(), 0);
    break;
    case FUNCTIONFS_ENABLE:
// ready = true;
    break;
...
#[no_mangle]
pub unsafe extern "C" fn init_bufs(iobuf: *mut io_buffer, n: unsigned, len: unsigned) {
...
// setup aio context to handle up to AIO_MAX requests
    ...
    &rfds, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    if (ret < 0) {
    if (errno == EINTR)
    continue;
    perror("select");
...
// we read aio events
    ret = io_getevents(ctx, 1, BUFS_MAX, e, core::ptr::null_mut());
