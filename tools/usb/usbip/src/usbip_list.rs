...
//! Source: tools/usb/usbip/src/usbip_list.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn get_exported_devices(host: *mut c_char, sockfd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn list_exported_devices(host: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn print_product_name(product_name: *mut c_char, parsable: bool) {
...
#[no_mangle]
unsafe extern "C" fn list_devices(parsable: bool) -> c_int {
...
//
    ...
    udev_enumerate_add_nomatch_sysattr(enumerate, "bInterfaceNumber", core::ptr::null_mut());
...
// Get product name.
    usbip_names_get_product(product_name, sizeof(product_name),
    strtol(idVendor, core::ptr::null_mut(), 16),
    strtol(idProduct, core::ptr::null_mut(), 16));
// Print information.
...
#[no_mangle]
unsafe extern "C" fn list_gadget_devices(parsable: bool) -> c_int {
...
// We only have mechanism to enumerate gadgets bound to vudc
    if (driver == core::ptr::null_mut() || strcmp(driver, USBIP_DEVICE_DRV_NAME))
    continue;
// Get device information.
...
#[no_mangle]
pub unsafe extern "C" fn usbip_list(argc: c_int, argv[]: *mut c_char) -> c_int {
    int usbip_list(int argc, char *argv[])
    {
    static const struct option opts[] = {
    { "parsable", no_argument,       core::ptr::null_mut(), 'p' },
    { "remote",   required_argument, core::ptr::null_mut(), 'r' },
    { "local",    no_argument,       core::ptr::null_mut(), 'l' },
    { "device",    no_argument,       core::ptr::null_mut(), 'd' },
    { core::ptr::null_mut(),       0,                 core::ptr::null_mut(),  0  }
    };
    let mut parsable: bool = false;
    int opt;
    let mut ret: c_int = -1;
    if (usbip_names_init(USBIDS_FILE))
    err("failed to open %s", USBIDS_FILE);
    for (;;) {
    opt = getopt_long(argc, argv, "pr:ld", opts, core::ptr::null_mut());
    if (opt == -1)
    break;
    switch (opt) {
    case 'p':
