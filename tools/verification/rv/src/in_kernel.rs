...
//! Source: tools/verification/rv/src/in_kernel.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn __ikm_read_enable(monitor_name: *mut c_char) -> c_int {
    ...
    retval = tracefs_instance_file_read_number(core::ptr::null_mut(), path, &enabled);
    if (retval < 0)
    return -1;
    return enabled;
    }
...
//
#[no_mangle]
unsafe extern "C" fn __ikm_find_monitor_name(monitor_name: *mut c_char, out_name: *mut c_char) -> c_int {
    static int __ikm_find_monitor_name(char *monitor_name, char *out_name)
    {
    char *available_monitors, *cursor, *line;
    let mut len: c_int = strlen(monitor_name);
    let mut found: c_int = 0;
    available_monitors = tracefs_instance_file_read(core::ptr::null_mut(), "rv/available_monitors", core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn ikm_read_enable(monitor_name: *mut c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ikm_write_enable(monitor_name: *mut c_char, enable_disable: *mut c_char) -> c_int {
    ...
    snprintf(path, MAX_PATH, "rv/monitors/%s/enable", monitor_name);
    retval = tracefs_instance_file_write(core::ptr::null_mut(), path, enable_disable);
...
//
#[no_mangle]
unsafe extern "C" fn ikm_enable(monitor_name: *mut c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ikm_disable(monitor_name: *mut c_char) -> c_int {
...
//
    ...
    desc = tracefs_instance_file_read(core::ptr::null_mut(), path, core::ptr::null_mut());
    if (!desc) {
...
//
#[no_mangle]
unsafe extern "C" fn ikm_fill_monitor_definition(name: *mut c_char, ikm: *mut monitor, container: *mut c_char) -> c_int {
...
// it belongs in container if it starts with "container:"
    if (container) {
    let mut len: c_int = strlen(container);
...
//
#[no_mangle]
unsafe extern "C" fn ikm_write_reactor(monitor_name: *mut c_char, reactor: *mut c_char) -> c_int {
    static int ikm_write_reactor(char *monitor_name, char *reactor)
    {
    char path[MAX_PATH];
    int retval;
    snprintf(path, MAX_PATH, "rv/monitors/%s/reactors", monitor_name);
    retval = tracefs_instance_file_write(core::ptr::null_mut(), path, reactor);
...
//
    ...
    char *reactors;
    snprintf(path, MAX_PATH, "rv/monitors/%s/reactors", monitor_name);
    reactors = tracefs_instance_file_read(core::ptr::null_mut(), path, core::ptr::null_mut());
    if (!reactors) {
...
//
    ...
    char *curr_reactor = core::ptr::null_mut();
    char *start;
    char *end;
    if (!reactors)
    return core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ikm_has_id(monitor_name: *mut c_char) -> c_int {
    ...
    format = tracefs_instance_file_read(core::ptr::null_mut(), path, core::ptr::null_mut());
    if (!format) {
...
// next = '\0';
    ...
    if (!retval) {
    let mut indent: c_int = ikm.nested && !container;
    list_monitor = 1;
    printf("%s%-*s %s %s\n", indent ? " - " : "",
...
#[no_mangle]
unsafe extern "C" fn ikm_print_header(s: *mut trace_seq) {
...
// if needed: struct trace_instance *inst = context;
    ...
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(pid: config_my_pid ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn ikm_enable_trace_events(monitor_name: *mut c_char, inst: *mut trace_instance) -> c_int {
    ...
    ikm_event_handler, core::ptr::null_mut());
    snprintf(event, sizeof(event), "error_%s", monitor_name);
    retval = tracefs_event_enable(inst.inst, "rv", event);
    if (retval)
    return -1;
    tep_register_event_handler(inst.tep, -1, "rv", event,
    ikm_error_handler, core::ptr::null_mut());
// set if at least 1 monitor has id in case of a container
    ...
    abs_path = tracefs_instance_get_file(core::ptr::null_mut(), rv_path);
    if (!abs_path)
    return -1;
    dp = opendir(abs_path);
    if (!dp)
...
//
#[no_mangle]
unsafe extern "C" fn ikm_destroy_trace_instance(inst: *mut trace_instance) {
...
//
#[no_mangle]
unsafe extern "C" fn ikm_usage_print_reactors() {
    static void ikm_usage_print_reactors(void)
    {
    char *reactors = tracefs_instance_file_read(core::ptr::null_mut(), "rv/available_reactors", core::ptr::null_mut());
    char *start, *end;
    if (!reactors)
    return;
...
//
#[no_mangle]
unsafe extern "C" fn ikm_usage(exit_val: c_int, monitor_name: *mut c_char, fmt: *const c_char, ...) {
...
//
#[no_mangle]
unsafe extern "C" fn parse_arguments(monitor_name: *mut c_char, argc: c_int, argv: *mut c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ikm_run_monitor(monitor_name: *mut c_char, argc: c_int, argv: *mut c_char) -> c_int {
    int ikm_run_monitor(char *monitor_name, int argc, char **argv)
    {
    struct trace_instance *inst = core::ptr::null_mut();
