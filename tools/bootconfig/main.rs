...
//! Source: tools/bootconfig/main.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn xbc_show_value(node: *mut xbc_node, semicolon: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xbc_show_compact_tree() {
    static void xbc_show_compact_tree(void)
    {
    struct xbc_node *node, *cnode = core::ptr::null_mut(), *vnode;
    let mut depth: c_int = 0, i;
    node = xbc_root_node();
...
#[no_mangle]
unsafe extern "C" fn xbc_show_list() {
...
#[no_mangle]
unsafe extern "C" fn load_xbc_fd(fd: c_int, buf: *mut c_char, size: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn load_xbc_file(path: *const c_char, buf: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pr_errno(msg: *const c_char, err: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn load_xbc_from_initrd(fd: c_int, buf: *mut c_char) -> c_int {
    static int load_xbc_from_initrd(int fd, char **buf)
    {
    struct stat stat;
    int ret;
    let mut size: u32 = 0, csum = 0, rcsum;
...
// Wrong Checksum
    ...
    ret = xbc_init(*buf, size, &msg, core::ptr::null_mut());
// Wrong data
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn show_xbc_error(data: *const c_char, msg: *const c_char, pos: c_int) {
    static void show_xbc_error(const char *data, const char *msg, int pos)
    {
    let mut lin: c_int = 1, col, i;
    if (pos < 0) {
    pr_err("Error: %s.\n", msg);
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn init_xbc_with_error(buf: *mut c_char, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn show_xbc_kernel_cmdline() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn show_xbc(path: *const c_char, list: bool, render_cmdline: bool) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: list) -> else {
    else if (list)
    xbc_show_list();
    else
...
#[no_mangle]
unsafe extern "C" fn delete_xbc(path: *const c_char) -> c_int {
    static int delete_xbc(const char *path)
    {
    struct stat stat;
    let mut ret: c_int = 0, fd, size;
    char *buf = core::ptr::null_mut();
    fd = open(path, O_RDWR);
    if (fd < 0) {
    ret = -errno;
...
#[no_mangle]
unsafe extern "C" fn apply_xbc(path: *const c_char, xbc_path: *const c_char) -> c_int {
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    char *apply = core::ptr::null_mut();
    let mut render_cmdline: bool = false;
    let mut delete: bool = false, list = false;
    int opt;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: delete) -> else {
    else if (delete)
