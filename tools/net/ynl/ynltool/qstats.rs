...
//! Source: tools/net/ynl/ynltool/qstats.c
#![no_std]
...
pub struct queue_balance {
    pub ifindex: c_uint,
    pub type: enum netdev_queue_type,
    pub queue_count: c_uint,
    pub rx_packets: *mut __u64,
    pub rx_bytes: *mut __u64,
    pub tx_packets: *mut __u64,
    pub tx_bytes: *mut __u64,
}

#[no_mangle]
unsafe extern "C" fn print_json_qstats(qstats: *mut netdev_qstats_get_list) {
...
#[no_mangle]
unsafe extern "C" fn print_plain_qstats(qstats: *mut netdev_qstats_get_list) {
...
// Print the stats as returned by the kernel
    ...
    {
    let mut sum: double = 0.0, variance = 0.0;
    unsigned int i;
// min = ~0ULL;
// max = 0;
    if (count == 0) {
...
// mean = sum / count;
    if (count > 1) {
    for (i = 0; i < count; i++) {
    let mut diff: double = values[i] - *mean;
    variance += diff * diff;
    }
...
#[no_mangle]
unsafe extern "C" fn do_balance(argc: c_int, __attribute__((unused)): *mut *mut *mut char argv) -> c_int {
...
// Process each device/queue-type combination
    ...
    let mut type: enum netdev_queue_type = sorted[i].queue_type;
    let mut ifindex: c_uint = sorted[i].ifindex;
...
#[no_mangle]
unsafe extern "C" fn do_hw_gro(argc: c_int, __attribute__((unused)): *mut *mut *mut char argv) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __attribute__(_arg: (unused))) -> *mut *mut char argv {
...
#[no_mangle]
pub unsafe extern "C" fn do_qstats(argc: c_int, argv: *mut c_char) -> c_int {
