...
//! Source: tools/bpf/bpftool/btf.c
#![no_std]
...
pub const MAX_ROOT_IDS: c_int = 16;
pub const MAX_BTF_FILES: c_int = 64;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sort_datum {
    pub index: c_int,
    pub type_rank: c_int,
    pub sort_name: *const c_char,
    pub own_name: *const c_char,
    pub disambig_hash: __u64,
}
...
#[no_mangle]
unsafe extern "C" fn btf_kind_safe(kind: c_int) -> c_int {
    ...
    for (i = 0; i < vlen; i++, v++) {
    const char *name = btf_str(btf, v.name_off);
    let mut val: __u64 = ((__u64)v.val_hi32 << 32) | v.val_lo32;
    if (json_output) {
    jsonw_start_object(w);
    jsonw_string_field(w, "name", name);
    if (btf_kflag(t))
    ...
    let mut cnt: c_int = btf__type_cnt(btf);
    let mut start_id: c_int = 1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ptr_array {
    pub cnt: __u32,
    pub cap: __u32,
    pub elems: *const c_void,
}

#[no_mangle]
unsafe extern "C" fn ptr_array_push(ptr: *const c_void, arr: *mut ptr_array) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ptr_array_free(arr: *mut ptr_array) {
...
#[no_mangle]
unsafe extern "C" fn dump_btf_kfuncs(d: *mut btf_dump, btf: *const btf) -> c_int {
    static int dump_btf_kfuncs(struct btf_dump *d, const struct btf *btf)
    {
    LIBBPF_OPTS(btf_dump_emit_type_decl_opts, opts);
    let mut cnt: __u32 = btf__type_cnt(btf), i, j;
    let mut fastcalls: ptr_array = {};
    let mut kfuncs: ptr_array = {};
    let mut err: c_int = 0;
    printf("\n/* BPF kfuncs */\n");
...
#[no_mangle]
unsafe extern "C" fn btf_type_rank(btf: *const btf, index: __u32, has_name: bool) -> c_int {
    static int btf_type_rank(const struct btf *btf, __u32 index, bool has_name)
    {
    const struct btf_type *t = btf__type_by_id(btf, index);
    let mut kind: c_int = btf_kind(t);
    let mut max_rank: c_int = 10;
    if (t.name_off)
    has_name = true;
    switch (kind) {
    case BTF_KIND_ENUM:
    ...
    case BTF_KIND_ENUM:
    case BTF_KIND_ENUM64: {
    let mut name_off: c_int = t.name_off;
...
#[no_mangle]
unsafe extern "C" fn hasher(hash: __u64, val: __u64) -> __u64 {
    static __u64 hasher(__u64 hash, __u64 val)
    {
    return hash * 31 + val;
    }
#[no_mangle]
unsafe extern "C" fn btf_name_hasher(hash: __u64, btf: *const btf, name_off: __u32) -> __u64 {
...
#[no_mangle]
unsafe extern "C" fn btf_type_disambig_hash(btf: *const btf, id: __u32, include_members: bool) -> __u64 {
...
unsafe extern "C" fn btf_type_compare(left: *const c_void, right: *const c_void) -> c_int {
    ...
    {
    struct sort_datum *datums = core::ptr::null_mut();
    struct btf_dump *d;
    let mut err: c_int = 0, i;
    d = btf_dump__new(btf, btf_dump_printf, core::ptr::null_mut(), core::ptr::null_mut());
    if (!d)
    return -errno;
    ...
    for (i = 1; i < cnt; i++) {
    let mut idx: c_int = datums ? datums[i].index : i;
    ...
    {
    struct btf *base;
    base = btf__parse(sysfs_vmlinux, core::ptr::null_mut());
    if (!base)
    p_err("failed to parse vmlinux BTF at '%s': %d\n",
    sysfs_vmlinux, -errno);
    return base;
    }
pub const BTF_NAME_BUFF_LEN: c_int = 64;
#[no_mangle]
unsafe extern "C" fn btf_is_kernel_module(btf_id: __u32) -> bool {
    static bool btf_is_kernel_module(__u32 btf_id)
    {
    let mut btf_info: bpf_btf_info = {};
    ...
    ret = btf__dedup(combined, core::ptr::null_mut());
    if (ret) {
...
unsafe extern "C" fn do_dump(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    {
    let mut dump_c: bool = false, sort_dump_c = true;
    struct btf *btf = core::ptr::null_mut(), *base = core::ptr::null_mut();
    __u32 root_type_ids[MAX_ROOT_IDS];
    bool have_id_filtering;
    let mut root_type_cnt: c_int = 0;
    let mut btf_id: __u32 = -1;
    const char *src;
    let mut fd: c_int = -1;
    let mut err: c_int = 0;
    int i;
...
#[no_mangle]
unsafe extern "C" fn btf_parse_fd(argc: *mut c_int, argv: *mut c_char) -> c_int {
    ...
    let mut prog_len: __u32 = sizeof(prog_info);
    struct bpf_map_info map_info;
    let mut map_len: __u32 = sizeof(map_info);
    let mut err: c_int = 0;
    ...
    printf("name [%s]  ", name);
#[no_mangle]
pub unsafe extern "C" fn if(name[0]: name &&) -> else {
    else if (name && name[0])
    printf("name %s  ", name);
    else
    printf("name <anon>  ");
...
#[no_mangle]
pub unsafe extern "C" fn do_btf(argc: c_int, argv: *mut c_char) -> c_int {
