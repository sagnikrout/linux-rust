...
//! Source: tools/bpf/bpftool/prog.c
#![no_std]
...
//
    ...
    [__MAX_BPF_ATTACH_TYPE] = core::ptr::null_mut(),
    };
    static struct hashmap *prog_table;
#[no_mangle]
unsafe extern "C" fn parse_attach_type(str: *const c_char) -> enum bpf_attach_type {
...
#[no_mangle]
unsafe extern "C" fn print_boot_time(nsecs: __u64, buf: *mut c_char, size: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn do_show_subset(argc: c_int, argv: *mut c_char) -> c_int {
...
unsafe extern "C" fn do_show(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    equal_fn_for_key_as_id, core::ptr::null_mut());
    ...
    {
    struct bpf_prog_linfo *prog_linfo = core::ptr::null_mut();
    const char *disasm_opt = core::ptr::null_mut();
    let mut dd: dump_data = {};
    ...
    struct kernel_sym *sym = core::ptr::null_mut();
    struct bpf_func_info *record;
    char sym_name[SYM_MAX_NAME];
    unsigned char *img = buf;
    __u64 *ksyms = core::ptr::null_mut();
    __u32 *lens;
    __u32 i;
    ...
    disasm_opt, btf, core::ptr::null_mut(), 0, 0,
    false))
    goto exit_free;
    }
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: visual) -> else {
    else if (visual)
...
#[no_mangle]
unsafe extern "C" fn do_dump(argc: c_int, argv: *mut c_char) -> c_int {
    static int do_dump(int argc, char **argv)
    {
    struct bpf_prog_info info;
    let mut info_len: __u32 = sizeof(info);
    let mut info_data_sz: usize = 0;
    void *info_data = core::ptr::null_mut();
    char *filepath = core::ptr::null_mut();
    let mut opcodes: bool = false;
    let mut visual: bool = false;
    enum dump_mode mode;
    let mut linum: bool = false;
    int nb_fds, i = 0;
    int *fds = core::ptr::null_mut();
    let mut err: c_int = -1;
...
#[no_mangle]
pub unsafe extern "C" fn if(1: i != nb_fds - 1 && nb_fds >) -> else {
...
unsafe extern "C" fn do_detach(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    let mut stream_id: c_int = mode == TRACE_STDOUT ? 1 : 2;
    char buf[512];
    int ret;
    ret = 0;
    do {
    ret = bpf_prog_stream_read(prog_fd, stream_id, buf, sizeof(buf), core::ptr::null_mut());
    if (ret > 0)
...
#[no_mangle]
unsafe extern "C" fn do_tracelog_any(argc: c_int, argv: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_run_data(fname: *const c_char, data_ptr: *mut c_void, size: *mut c_uint) -> c_int {
    static int get_run_data(const char *fname, void **data_ptr, unsigned int *size)
    {
    let mut block_size: usize = 256;
    let mut buf_size: usize = block_size;
...
#[no_mangle]
unsafe extern "C" fn alloc_run_data(data_ptr: *mut c_void, size_out: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_run(argc: c_int, argv: *mut c_char) -> c_int {
    static int do_run(int argc, char **argv)
    {
    char *data_fname_in = core::ptr::null_mut(), *data_fname_out = core::ptr::null_mut();
    char *ctx_fname_in = core::ptr::null_mut(), *ctx_fname_out = core::ptr::null_mut();
    let mut default_size: c_uint = SZ_32K;
    void *data_in = core::ptr::null_mut(), *data_out = core::ptr::null_mut();
    void *ctx_in = core::ptr::null_mut(), *ctx_out = core::ptr::null_mut();
    ...
    "data_size_out", core::ptr::null_mut()))
    return -1;
    if (detect_common_prefix(*argv, "ctx_in", "ctx_out",
    "ctx_size_out", core::ptr::null_mut()))
    return -1;
...
#[no_mangle]
unsafe extern "C" fn load_with_options(argc: c_int, argv: *mut c_char, first_prog_only: bool) -> c_int {
    static int load_with_options(int argc, char **argv, bool first_prog_only)
    {
    let mut common_prog_type: enum bpf_prog_type = BPF_PROG_TYPE_UNSPEC;
    DECLARE_LIBBPF_OPTS(bpf_object_open_opts, open_opts,
    .relaxed_maps = relaxed_maps,
    );
    enum bpf_attach_type expected_attach_type;
    struct map_replace *map_replace = core::ptr::null_mut();
    struct bpf_program *prog = core::ptr::null_mut(), *pos;
    let mut old_map_fds: c_uint = 0;
    const char *pinmaps = core::ptr::null_mut();
    let mut xdpmeta_ifindex: __u32 = 0;
    let mut offload_ifindex: __u32 = 0;
    let mut auto_attach: bool = false;
...
// log_level1 + log_level2 + stats, but not stable UAPI
    ...
    let mut prog_type: enum bpf_prog_type = common_prog_type;
...
unsafe extern "C" fn try_loader(gen: *mut gen_loader_opts) -> c_int {
    ...
    {
    let mut opts: bpf_load_and_run_opts = {};
    struct bpf_loader_ctx *ctx;
    char sig_buf[MAX_SIG_SIZE];
    __u8 prog_sha[SHA256_DIGEST_LENGTH];
    int ctx_sz = sizeof(*ctx) + 64 * max(sizeof(struct bpf_map_desc),
    sizeof(struct bpf_prog_desc));
    let mut log_buf_sz: c_int = (1u << 24) - 1;
    int err, fds_before, fd_delta;
    char *log_buf = core::ptr::null_mut();
    ctx = alloca(ctx_sz);
    memset(ctx, 0, ctx_sz);
    ctx.sz = ctx_sz;
    if (verifier_logs) {
...
#[no_mangle]
unsafe extern "C" fn do_loader(argc: c_int, argv: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_load(argc: c_int, argv: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_loadall(argc: c_int, argv: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_profile(argc: c_int, argv: *mut c_char) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct profile_metric {
    pub name: *const c_char,
    pub val: bpf_perf_event_value,
    pub attr: perf_event_attr,
    pub selected: bool,
// calculate ratios like instructions per cycle
    pub /: *const *const int ratio_metric; / 0 for N/A, 1 for index 0 (cycles),
    pub ratio_desc: *const c_char,
    pub ratio_mul: float,
    } metrics[] = {
    {
    .name = "cycles",
...
pub const MAX_NUM_PROFILE_METRICS: c_int = 4;
#[no_mangle]
unsafe extern "C" fn profile_parse_metrics(argc: c_int, argv: *mut c_char) -> c_int {
    static int profile_parse_metrics(int argc, char **argv)
    {
    unsigned int metric_cnt;
    let mut selected_cnt: c_int = 0;
    unsigned int i;
...
unsafe extern "C" fn profile_print_readings() {
    ...
    {
    let mut func_info: bpf_func_info = {};
    let mut info: bpf_prog_info = {};
    let mut info_len: __u32 = sizeof(info);
    ...
    let mut profile_tgt_fd: static int = -1;
    static char *profile_tgt_name;
    static int *profile_perf_events;
    static int profile_perf_event_cnt;
#[no_mangle]
unsafe extern "C" fn profile_close_perf_events(obj: *mut profiler_bpf) {
...
#[no_mangle]
pub unsafe extern "C" fn do_prog(argc: c_int, argv: *mut c_char) -> c_int {
