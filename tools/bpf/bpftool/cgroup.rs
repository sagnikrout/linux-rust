...
//! Source: tools/bpf/bpftool/cgroup.c
#![no_std]
...
pub const _XOPEN_SOURCE: c_int = 500;
...
#[no_mangle]
unsafe extern "C" fn guess_vmlinux_btf_id(attach_btf_obj_id: __u32) {
    static void guess_vmlinux_btf_id(__u32 attach_btf_obj_id)
    {
    let mut btf_info: bpf_btf_info = {};
    let mut btf_len: __u32 = sizeof(btf_info);
    char name[16] = {};
    int err;
    int fd;
    ...
    const char *attach_btf_name = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: info.attach_btf_id) -> else {
...
#[no_mangle]
unsafe extern "C" fn count_attached_bpf_progs(cgroup_fd: c_int, type: enum bpf_attach_type) -> c_int {
    static int count_attached_bpf_progs(int cgroup_fd, enum bpf_attach_type type)
    {
    let mut prog_cnt: __u32 = 0;
    int ret;
    ret = bpf_prog_query(cgroup_fd, type, query_flags, core::ptr::null_mut(),
    core::ptr::null_mut(), &prog_cnt);
    if (ret)
    return -1;
    return prog_cnt;
    }
#[no_mangle]
unsafe extern "C" fn cgroup_has_attached_progs(cgroup_fd: c_int) -> c_int {
    static int cgroup_has_attached_progs(int cgroup_fd)
    {
    let mut i: c_uint = 0;
    let mut no_prog: bool = true;
    for (i = 0; i < ARRAY_SIZE(cgroup_attach_types); i++) {
    let mut count: c_int = count_attached_bpf_progs(cgroup_fd, cgroup_attach_types[i]);
    ...
    show_bpf_prog(prog_ids[iter], type, core::ptr::null_mut(), level);
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(BPF_F_QUERY_EFFECTIVE: query_flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn do_show_tree(argc: c_int, argv: *mut c_char) -> c_int {
    static int do_show_tree(int argc, char **argv)
    {
    char *cgroup_root, *cgroup_alloced = core::ptr::null_mut();
    int ret;
    query_flags = 0;
    if (!argc) {
...
#[no_mangle]
pub unsafe extern "C" fn do_cgroup(argc: c_int, argv: *mut c_char) -> c_int {
