...
//! Source: tools/bpf/bpftool/btf_dumper.c
#![no_std]
...
// Copyright (c) 2018 Facebook
    ...
    let mut prog_fd: c_int = -1, func_sig_len;
    let mut info: bpf_prog_info = {};
    let mut info_len: __u32 = sizeof(info);
    const char *prog_name = core::ptr::null_mut();
...
// Get the ptr's func_proto
    func_sig_len = btf_dump_func(d.btf, prog_str, func_proto, core::ptr::null_mut(), 0,
    sizeof(prog_str));
    if (func_sig_len == -1)
    return -1;
    if (!prog_id)
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: prog_name) -> else {
    else if (prog_name)
    snprintf(&prog_str[func_sig_len],
    ...
    const void *data)
    {
    let mut value: c_ulong = *(unsigned long *)data;
...
// shake out un-needed bits by shift/or operations
    ...
    bool is_plain_text)
    {
    let mut nr_bits: c_int = BTF_INT_BITS(int_type);
...
#[no_mangle]
pub unsafe extern "C" fn if(32: *mut *mut BTF_INT_BITS(int_type) ==) -> else {
    else if (BTF_INT_BITS(*int_type) == 32)
    jsonw_printf(jw, "%u", *(__u32 *)data);
#[no_mangle]
pub unsafe extern "C" fn if(16: *mut *mut BTF_INT_BITS(int_type) ==) -> else {
    else if (BTF_INT_BITS(*int_type) == 16)
    jsonw_printf(jw, "%hu", *(__u16 *)data);
#[no_mangle]
pub unsafe extern "C" fn if(8: *mut *mut BTF_INT_BITS(int_type) ==) -> else {
...
// shouldn't happen
    ...
    for (i = 0; i < vlen; i++) {
    let mut bit_offset: __u32 = m[i].offset;
...
// map key or value can't be forward
    ...
    pos = btf_dump_func(btf, func_sig, t, core::ptr::null_mut(), pos, size);
    if (pos == -1)
    return -1;
    break;
    case BTF_KIND_FUNC:
...
#[no_mangle]
pub unsafe extern "C" fn if(': pos && func_sig[pos - 1] == ') -> else {
...
#[no_mangle]
unsafe extern "C" fn dotlabel_puts(s: *const c_char) {
    ...
    {
    let mut MAX_PATH_LEN: c_uint = 32;
    let mut len: usize = strlen(path);
