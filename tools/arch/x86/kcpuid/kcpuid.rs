...
//! Source: tools/arch/x86/kcpuid/kcpuid.c
#![no_std]
...
pub const CPUID_INDEX_MASK: c_uint = 0xffff0000;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct cpuid_range {
// array of main leafs
    pub funcs: *mut cpuid_func,
// number of valid leafs
    pub nr: c_int,
    pub index: enum range_index,
}
    ...
    {
    let mut func_idx: u32 = index & CPUID_FUNCTION_MASK;
    let mut range_idx: u32 = index & CPUID_INDEX_MASK;
    ...
    let mut show_flags_only: static bool = true;
    let mut user_index: static u32 = 0xFFFFFFFF;
    let mut user_sub: static u32 = 0xFFFFFFFF;
    static int flines;
//
...
#[no_mangle]
pub unsafe extern "C" fn has_subleafs(f: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn leaf_print_raw(leaf: *mut subleaf) {
...
//
    ...
    if (!func.leafs)
    err(EXIT_FAILURE, core::ptr::null_mut());
    func.nr = 1;
    } else {
    s = func.nr;
...
#[no_mangle]
unsafe extern "C" fn raw_dump_range(range: *mut cpuid_range) {
...
pub const MAX_SUBLEAF_NUM: c_int = 64;
pub const MAX_RANGE_INDEX_OFFSET: c_uint = 0xff;
#[no_mangle]
pub unsafe extern "C" fn setup_cpuid_range(range: *mut cpuid_range) {
...
//
#[no_mangle]
unsafe extern "C" fn parse_line(line: *mut c_char) {
...
// Skip comments and NULL line
    ...
    goto err_exit;
// index/main-leaf
    index = strtoull(tokens[0], core::ptr::null_mut(), 0);
//
...
// subleaf
    ...
    start = strtok(core::ptr::null_mut(), ":");
    subleaf_end = strtoul(end, core::ptr::null_mut(), 0);
// A subleaf range is given?
    if (start) {
    subleaf_start = strtoul(start, core::ptr::null_mut(), 0);
...
// register
    ...
    reg_index = R_EAX;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcasestr(buf, _arg: "EBX")) -> else {
    else if (strcasestr(buf, "EBX"))
    reg_index = R_EBX;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcasestr(buf, _arg: "ECX")) -> else {
    else if (strcasestr(buf, "ECX"))
    reg_index = R_ECX;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcasestr(buf, _arg: "EDX")) -> else {
...
// bit flag or bits field
    ...
    start = strtok(core::ptr::null_mut(), ":");
    bit_end = strtoul(end, core::ptr::null_mut(), 0);
    bit_start = (start) ? strtoul(start, core::ptr::null_mut(), 0) : bit_end;
...
#[no_mangle]
unsafe extern "C" fn parse_text() {
    static void parse_text(void)
    {
    FILE *file;
    char *filename, *line = core::ptr::null_mut();
    let mut len: usize = 0;
    int ret;
    if (show_raw)
    return;
...
#[no_mangle]
unsafe extern "C" fn show_reg(rdesc: *const reg_desc, value: u32) {
...
#[no_mangle]
unsafe extern "C" fn show_reg_header(has_entries: bool, leaf: u32, subleaf: u32, reg_name: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn show_leaf(leaf: *mut subleaf) {
...
#[no_mangle]
unsafe extern "C" fn show_func(func: *mut cpuid_func) {
...
#[no_mangle]
unsafe extern "C" fn show_range(range: *mut cpuid_range) {
...
#[no_mangle]
unsafe extern "C" fn usage(exit_code: c_int) -> void __noreturn {
    ...
    { "all", no_argument, core::ptr::null_mut(), 'a' },		/* show both bit flags and fields */
    { "bitflags", no_argument, core::ptr::null_mut(), 'b' },		/* only show bit flags, default on */
    { "detail", no_argument, core::ptr::null_mut(), 'd' },		/* show detail descriptions */
    { "file", required_argument, core::ptr::null_mut(), 'f' },	/* use user's cpuid file */
    { "help", no_argument, core::ptr::null_mut(), 'h'},		/* show usage */
    { "leaf", required_argument, core::ptr::null_mut(), 'l'},	/* only check a specific leaf */
    { "raw", no_argument, core::ptr::null_mut(), 'r'},		/* show raw CPUID leaf data */
    { "subleaf", required_argument, core::ptr::null_mut(), 's'},	/* check a specific subleaf */
    { core::ptr::null_mut(), 0, core::ptr::null_mut(), 0 }
    };
#[no_mangle]
unsafe extern "C" fn parse_options(argc: c_int, argv[]: *mut c_char) {
    ...
    case 'l':
// main leaf
    user_index = strtoul(optarg, core::ptr::null_mut(), 0);
    break;
    case 'r':
    show_raw = true;
    break;
    case 's':
// subleaf
    user_sub = strtoul(optarg, core::ptr::null_mut(), 0);
    break;
    default:
    usage(EXIT_FAILURE);
    }
