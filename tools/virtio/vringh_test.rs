...
//! Source: tools/virtio/vringh_test.c
#![no_std]
...
pub const RINGSIZE: c_int = 256;
pub const ALIGN: c_int = 4096;
#[no_mangle]
unsafe extern "C" fn never_notify_host(vq: *mut virtqueue) -> bool {
    static bool never_notify_host(struct virtqueue *vq)
    {
    abort();
    }
#[no_mangle]
unsafe extern "C" fn never_callback_guest(vq: *mut virtqueue) {
    static void never_callback_guest(struct virtqueue *vq)
    {
    abort();
    }
#[no_mangle]
unsafe extern "C" fn getrange_iov(vrh: *mut vringh, addr: u64, r: *mut vringh_range) -> bool {
...
#[no_mangle]
unsafe extern "C" fn getrange_slow(vrh: *mut vringh, addr: u64, r: *mut vringh_range) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct guest_virtio_device {
    pub vdev: virtio_device,
    pub to_host_fd: c_int,
    pub notifies: c_ulong,
}

#[no_mangle]
unsafe extern "C" fn parallel_notify_host(vq: *mut virtqueue) -> bool {
...
#[no_mangle]
unsafe extern "C" fn no_notify_host(vq: *mut virtqueue) -> bool {
...
#[no_mangle]
unsafe extern "C" fn find_cpus(first: *mut c_uint, last: *mut c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn vringh_get_head(vrh: *mut vringh, head: *mut u16) -> c_int {
...
// Only get avail ring entries after they have been exposed by guest.
    ...
    let mut xfers: c_ulong = 0, notifies = 0, receives = 0;
...
// Parent and child use separate addresses, to check our mapping logic!
    host_map = mmap(core::ptr::null_mut(), mapsize, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
    if (host_map == MAP_FAILED)
    err(1, "mmap host_map");
    guest_map = mmap(core::ptr::null_mut(), mapsize, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
...
// Don't kfree indirects.
    ...
    int *dbuf, err;
    let mut output: bool = !(xfers % 2);
// Consume bufs.
    while ((dbuf = virtqueue_get_buf(vq, &len)) != core::ptr::null_mut()) {
    if (len == 4)
    assert(*dbuf == finished - 1);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !fast_vringh) -> else {
    else if (!fast_vringh)
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    let mut fast_vringh: bool = false, parallel = false;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[1], 0: "--eventidx") ==) -> else {
    else if (strcmp(argv[1], "--eventidx") == 0)
    __virtio_set_bit(&vdev, VIRTIO_RING_F_EVENT_IDX);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[1], 0: "--virtio-1") ==) -> else {
    else if (strcmp(argv[1], "--virtio-1") == 0)
    __virtio_set_bit(&vdev, VIRTIO_F_VERSION_1);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[1], 0: "--slow-range") ==) -> else {
    else if (strcmp(argv[1], "--slow-range") == 0)
    getrange = getrange_slow;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[1], 0: "--fast-vringh") ==) -> else {
    else if (strcmp(argv[1], "--fast-vringh") == 0)
    fast_vringh = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[1], 0: "--parallel") ==) -> else {
...
// May allocate an indirect, so force it to allocate user addr
    ...
    __kmalloc_fake = core::ptr::null_mut();
// Host retrieves it.
...
// Free those buffers.
    for (i = 0; i < RINGSIZE; i++) {
    unsigned len;
    assert(virtqueue_get_buf(vq, &len) != core::ptr::null_mut());
    }
