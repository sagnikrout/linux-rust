...
//! Source: tools/iio/iio_event_monitor.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn event_is_known(event: *mut iio_event_data) -> bool {
    static bool event_is_known(struct iio_event_data *event)
    {
    let mut type: enum iio_chan_type = IIO_EVENT_CODE_EXTRACT_CHAN_TYPE(event.id);
    let mut mod: enum iio_modifier = IIO_EVENT_CODE_EXTRACT_MODIFIER(event.id);
    let mut ev_type: enum iio_event_type = IIO_EVENT_CODE_EXTRACT_TYPE(event.id);
    let mut dir: enum iio_event_direction = IIO_EVENT_CODE_EXTRACT_DIR(event.id);
    switch (type) {
    case IIO_VOLTAGE:
    case IIO_CURRENT:
    case IIO_POWER:
...
#[no_mangle]
unsafe extern "C" fn print_event(event: *mut iio_event_data) {
    ...
    let mut ev_type: enum iio_event_type = IIO_EVENT_CODE_EXTRACT_TYPE(event.id);
    let mut dir: enum iio_event_direction = IIO_EVENT_CODE_EXTRACT_DIR(event.id);
    let mut chan: c_int = IIO_EVENT_CODE_EXTRACT_CHAN(event.id);
    let mut chan2: c_int = IIO_EVENT_CODE_EXTRACT_CHAN2(event.id);
    let mut diff: bool = IIO_EVENT_CODE_EXTRACT_DIFF(event.id);
...
#[no_mangle]
unsafe extern "C" fn enable_events(dev_dir: *mut c_char, enable: c_int) {
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    char *dev_dir_name = core::ptr::null_mut();
    char *chrdev_name;
    int ret;
    int dev_num;
    int fd, event_fd;
