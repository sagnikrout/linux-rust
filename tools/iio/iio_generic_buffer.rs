...
//! Source: tools/iio/iio_generic_buffer.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn size_from_channelarray(channels: *mut iio_channel_info, num_channels: c_int) -> c_uint {
    static unsigned int size_from_channelarray(struct iio_channel_info *channels, int num_channels)
    {
    let mut bytes: c_uint = 0;
    let mut i: c_int = 0, max = 0;
...
#[no_mangle]
unsafe extern "C" fn print1byte(input: u8, info: *mut iio_channel_info) {
...
#[no_mangle]
unsafe extern "C" fn print2byte(input: u16, info: *mut iio_channel_info) {
...
#[no_mangle]
unsafe extern "C" fn print4byte(input: u32, info: *mut iio_channel_info) {
...
#[no_mangle]
unsafe extern "C" fn print8byte(input: u64, info: *mut iio_channel_info) {
...
#[no_mangle]
unsafe extern "C" fn enable_disable_all_channels(dev_dir_name: *mut c_char, buffer_idx: c_int, enable: c_int) -> c_int {
...
unsafe extern "C" fn print_usage() {
    ...
    let mut autochannels: static enum autochan = AUTOCHANNELS_DISABLED;
    static char *dev_dir_name = core::ptr::null_mut();
    static char *buf_dir_name = core::ptr::null_mut();
    let mut buffer_idx: static int = 0;
    let mut current_trigger_set: static bool = false;
#[no_mangle]
...
// Disconnect the trigger - just write a dummy name.
    ...
    dev_dir_name, "core::ptr::null_mut()");
    if (ret < 0)
...
#[no_mangle]
unsafe extern "C" fn register_cleanup() {
    static void register_cleanup(void)
    {
    let mut sa: sigaction = { .sa_handler = sig_handler };
    const int signums[] = { SIGINT, SIGTERM, SIGABRT };
    int ret, i;
    for (i = 0; i < ARRAY_SIZE(signums); ++i) {
    ret = sigaction(signums[i], &sa, core::ptr::null_mut());
    if (ret) {
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    {
    let mut num_loops: c_longlong = 2;
    ...
    char *trigger_name = core::ptr::null_mut(), *device_name = core::ptr::null_mut();
    char *data = core::ptr::null_mut();
    ssize_t read_size;
    let mut dev_num: c_int = -1, trig_num = -1;
    char *buffer_access = core::ptr::null_mut();
    unsigned int scan_size;
    let mut noevents: c_int = 0;
    let mut notrigger: c_int = 0;
    char *dummy;
    let mut force_autochannels: bool = false;
    struct iio_channel_info *channels = core::ptr::null_mut();
    register_cleanup();
...
// Enable the buffer
    ...
    let mut total_buf_len: usize = scan_size * buf_len;
