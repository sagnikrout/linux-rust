...
//! Source: tools/lib/bpf/btf_dump.c
#![no_std]
...
//
    ...
    let mut PREFIX_CNT: static size_t = sizeof(PREFIXES) - 1;
...
// topological sorting state
    pub 2: enum btf_dump_type_order_state order_state:,
// emitting state used to determine the need for forward declaration
    pub 2: enum btf_dump_type_emit_state emit_state:,
// whether forward declaration was already emitted
    pub 1: __u8 fwd_emitted:,
// whether unique non-duplicate name was already assigned
    pub 1: __u8 name_resolved:,
// whether type is referenced from any other type
    pub 1: __u8 referenced:,
}

// indent string length; one indent string is added for each indent level
pub const BTF_DATA_INDENT_STR_LEN: c_int = 32;
//
...
pub struct btf_dump_data {
    pub /: *const *const *const void data_end; / end of valid data to show,
    pub compact: bool,
    pub skip_names: bool,
    pub emit_zeroes: bool,
    pub emit_strings: bool,
    pub /: *mut *mut __u8 indent_lvl; / base indent level,
    pub indent_str: [c_char; BTF_DATA_INDENT_STR_LEN],
// below are used during iteration
    pub depth: c_int,
    pub is_array_member: bool,
    pub is_array_terminated: bool,
    pub is_array_char: bool,
}
...
pub struct btf_dump {
    ...
    pub ptr_sz: c_int,
    pub strip_mods: bool,
    pub skip_anon_defs: bool,
    pub last_id: c_int,
// per-type auxiliary state
    pub type_states: *mut btf_dump_type_aux_state,
    pub type_states_cap: usize,
// per-type optional cached unique name, must be freed, if present
    pub cached_names: *const c_char,
    pub cached_names_cap: usize,
...
#[no_mangle]
unsafe extern "C" fn str_hash_fn(key: c_long, ctx: *mut c_void) -> usize {
    static size_t str_hash_fn(long key, void *ctx)
    {
    return str_hash((void *)key);
    }
#[no_mangle]
unsafe extern "C" fn str_equal_fn(a: c_long, b: c_long, ctx: *mut c_void) -> bool {
...
#[no_mangle]
unsafe extern "C" fn btf_dump_printf(d: *const btf_dump, fmt: *const c_char, ...) {
    ...
    d.type_names = hashmap__new(str_hash_fn, str_equal_fn, core::ptr::null_mut());
    if (IS_ERR(d.type_names)) {
    err = PTR_ERR(d.type_names);
    d.type_names = core::ptr::null_mut();
    goto err;
    }
    d.ident_names = hashmap__new(str_hash_fn, str_equal_fn, core::ptr::null_mut());
    if (IS_ERR(d.ident_names)) {
    err = PTR_ERR(d.ident_names);
    d.ident_names = core::ptr::null_mut();
    goto err;
    }
...
#[no_mangle]
unsafe extern "C" fn btf_dump_resize(d: *mut btf_dump) -> c_int {
    static int btf_dump_resize(struct btf_dump *d)
    {
...
#[no_mangle]
unsafe extern "C" fn btf_dump_free_names(map: *mut hashmap) {
...
//
#[no_mangle]
unsafe extern "C" fn btf_dump_mark_referenced(d: *mut btf_dump) -> c_int {
    static int btf_dump_mark_referenced(struct btf_dump *d)
    {
...
#[no_mangle]
unsafe extern "C" fn btf_dump_add_emit_queue_id(d: *mut btf_dump, id: __u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn btf_dump_order_type(d: *mut btf_dump, id: __u32, through_ptr: bool) -> c_int {
    static int btf_dump_order_type(struct btf_dump *d, __u32 id, bool through_ptr)
    {
//
...
#[no_mangle]
unsafe extern "C" fn btf_dump_is_blacklisted(d: *mut btf_dump, id: __u32) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn btf_dump_emit_type(d: *mut btf_dump, id: __u32, cont_id: __u32) {
    static void btf_dump_emit_type(struct btf_dump *d, __u32 id, __u32 cont_id)
    {
    struct btf_dump_type_aux_state *tstate = &d.type_states[id];
    let mut top_level_def: bool = cont_id == 0;
    const struct btf_type *t;
    __u16 kind;
...
//
    ...
    let mut max_align: c_int = 1, align, i, bit_sz;
    __u32 vlen;
    m = btf_members(t);
    vlen = btf_vlen(t);
...
//
    ...
    let mut new_off: c_int = 0, pad_bits = 0, bits, i;
    const char *pad_type = core::ptr::null_mut();
    if (cur_off >= next_off)
    return; /* no gap */
...
//
    ...
    {
    const struct btf_member *m = btf_members(t);
    let mut is_struct: bool = btf_is_struct(t);
...
//
    ...
    let mut is_signed: bool = btf_kflag(t);
    const char *fmt_str;
    const char *name;
    size_t dup_cnt;
    int i;
...
#[no_mangle]
unsafe extern "C" fn btf_dump_push_decl_stack_id(d: *mut btf_dump, id: __u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btf_dump_emit_mods(d: *mut btf_dump, decl_stack: *mut id_stack) {
...
#[no_mangle]
unsafe extern "C" fn btf_dump_drop_mods(d: *mut btf_dump, decl_stack: *mut id_stack) {
    ...
    {
    let mut separate: bool = name[0] && !last_was_ptr;
...
//
    let mut last_was_ptr: bool = true;
    const struct btf_type *t;
    const char *name;
    __u16 kind;
    __u32 id;
...
// return number of duplicates (occurrences) of a given name
    ...
    dup_cnt++;
    err = hashmap__set(name_map, new_name, dup_cnt, &old_name, core::ptr::null_mut());
    if (err)
    free(new_name);
    free(old_name);
    return dup_cnt;
...
#[no_mangle]
unsafe extern "C" fn btf_dump_data_pfx(d: *mut btf_dump) {
...
//
    ...
    const void *data)
    {
    let mut alignment: c_int = btf__align_of(btf, type_id);
    if (alignment == 0)
    return false;
    ...
    __u8 bits_offset)
    {
    let mut encoding: __u8 = btf_int_encoding(t);
    let mut sign: bool = encoding & BTF_INT_SIGNED;
    char buf[16] __attribute__((aligned(16)));
    let mut sz: c_int = t.size;
...
// handle unaligned data; copy to local union
    ...
    const void *data)
    {
    let mut linkage: enum btf_func_linkage = btf_var(v).linkage;
    const struct btf_type *t;
    const char *l;
    __u32 type_id;
    switch (linkage) {
...
//
    ...
    return btf_dump_dump_type_data(d, core::ptr::null_mut(), t, type_id, data, 0, 0);
    }
...
//
    ...
    elem_type = skip_mods_and_typedefs(d.btf, elem_type_id, core::ptr::null_mut());
...
//
    ...
    if (d.typed_dump.is_array_terminated)
    break;
    btf_dump_dump_type_data(d, core::ptr::null_mut(), elem_type, elem_type_id, data, 0, 0);
    }
...
// value = is_signed ? *(__s8 *)data : *(__u8 *)data;
    ...
    err = btf_dump_dump_type_data(d, core::ptr::null_mut(), var, vsi.type, data + vsi.offset, 0, 0);
    if (err < 0)
    return err;
    btf_dump_printf(d, ";");
    }
...
// bits_offset is at most 7. bit_sz is at most 128.
    let mut nr_bytes: __u8 = (bits_offset + bit_sz + 7) / 8;
...
//
    t = skip_mods_and_typedefs(d.btf, id, core::ptr::null_mut());
    if (!t) {
...
// handle bitfield and int enum values
    ...
    {
    let mut typed_dump: btf_dump_data = {};
    const struct btf_type *t;
    int ret;
...
// default indent string is a tab
    if (!OPTS_GET(opts, indent_str, core::ptr::null_mut()))
    ...
    ret = btf_dump_dump_type_data(d, core::ptr::null_mut(), t, id, data, 0, 0);
    d.typed_dump = core::ptr::null_mut();
