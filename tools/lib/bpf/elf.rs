...
//
pub const VERSYM_HIDDEN: c_uint = 0x8000;
// This is the mask for the rest of the data in a word read from a
// SHT_GNU_versym section.
//
pub const VERSYM_VERSION: c_uint = 0x7fff;
#[no_mangle]
...
pub struct elf_sym_iter {
    ...
    scn = elf_find_next_scn_by_type(elf, sh_type, core::ptr::null_mut());
    if (!scn) {
...
// Version symbol table is meaningful to dynsym only
    ...
    return 0;
    scn = elf_find_next_scn_by_type(elf, SHT_GNU_versym, core::ptr::null_mut());
    if (!scn)
    return 0;
    iter.versyms = elf_getdata(scn, 0);
    scn = elf_find_next_scn_by_type(elf, SHT_GNU_verdef, core::ptr::null_mut());
    if (!scn)
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn elf_sym_offset(sym: *mut elf_sym) -> c_ulong {
...
//
#[no_mangle]
pub unsafe extern "C" fn elf_find_func_offset(elf: *mut Elf, binary_path: *const c_char, name: *const c_char) -> c_long {
    long elf_find_func_offset(Elf *elf, const char *binary_path, const char *name)
    {
    int i, sh_types[2] = { SHT_DYNSYM, SHT_SYMTAB };
    const char *at_symbol, *lib_ver;
    bool is_shared_lib;
    let mut ret: c_long = -ENOENT;
    size_t name_len;
    GElf_Ehdr ehdr;
    if (!gelf_getehdr(elf, &ehdr)) {
    pr_warn("elf: failed to get ehdr from %s: %s\n", binary_path, elf_errmsg(-1));
...
//
#[no_mangle]
pub unsafe extern "C" fn elf_find_func_offset_from_file(binary_path: *const c_char, name: *const c_char) -> c_long {
...
//
    ...
    let mut err: c_int = 0, i, cnt_done = 0;
    ...
    goto out;
    while ((sym = elf_sym_iter_next(&iter))) {
    let mut sym_offset: c_ulong = elf_sym_offset(sym);
    let mut bind: c_int = GELF_ST_BIND(sym.sym.st_info);
