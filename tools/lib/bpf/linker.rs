...
// corresponding STT_SECTION symbol index in SYMTAB
    pub sec_sym_idx: c_int,
// section's DATASEC variable info, emitted on BTF finalization
    pub has_btf: bool,
    pub sec_var_cnt: c_int,
    pub sec_vars: *mut btf_var_secinfo,
// section's .BTF.ext data
    pub func_info: btf_ext_sec_data,
    pub line_info: btf_ext_sec_data,
    pub core_relo_info: btf_ext_sec_data,
}
...
pub struct bpf_linker {
    ...
    pub fd: c_int,
    pub elf: *mut Elf,
    pub elf_hdr: *mut Elf64_Ehdr,
    pub swapped_endian: bool,
// Output sections metadata
    pub secs: *mut dst_sec,
    pub sec_cnt: c_int,
    pub /: *mut *mut *mut strset strtab_strs; / STRTAB unique strings,
    pub /: *mut *mut size_t strtab_sec_idx; / STRTAB section index,
    pub /: *mut *mut size_t symtab_sec_idx; / SYMTAB section index,
    pub btf: *mut btf,
    pub btf_ext: *mut btf_ext,
// global (including extern) ELF symbols
    pub glob_sym_cnt: c_int,
    pub glob_syms: *mut glob_sym,
    pub fd_is_owned: bool,
}
...
pub unsafe extern "C" fn bpf_linker__free(linker: *mut bpf_linker) {
    ...
    let mut new_cnt: usize = linker.sec_cnt ? linker.sec_cnt + 1 : 2;
...
// zero out newly allocated memory
    ...
    Elf64_Sym *syms, *sym;
    let mut sym_cnt: usize = symtab.sec_sz / sizeof(*sym);
...
#[no_mangle]
unsafe extern "C" fn init_output_elf(linker: *mut bpf_linker) -> c_int {
    static int init_output_elf(struct bpf_linker *linker)
    {
    int err, str_off;
    Elf64_Sym *init_sym;
    struct dst_sec *sec;
    linker.elf = elf_begin(linker.fd, ELF_C_WRITE, core::ptr::null_mut());
    if (!linker.elf) {
...
// add the special all-zero symbol
    init_sym = add_new_sym(linker, core::ptr::null_mut());
    if (!init_sym)
    return -EINVAL;
    init_sym.st_name = 0;
    init_sym.st_info = 0;
...
#[no_mangle]
unsafe extern "C" fn is_dwarf_sec_name(name: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_ignored_sec(sec: *mut src_sec) -> bool {
...
// .BTF and .BTF.ext don't need relocations
    ...
    let mut new_cnt: usize = obj.sec_cnt ? obj.sec_cnt + 1 : 2;
    secs = libbpf_reallocarray(secs, new_cnt, sizeof(*secs));
    if (!secs)
    return core::ptr::null_mut();
// zero out newly allocated memory
    ...
    let mut link_byteorder: c_uchar = linker.elf_hdr.e_ident[EI_DATA];

    let mut host_byteorder: c_uchar = ELFDATA2LSB;

    let mut host_byteorder: c_uchar = ELFDATA2MSB;

    pr_debug("linker: adding object file '%s'...\n", obj.filename);
    obj.elf = elf_begin(obj.fd, ELF_C_READ_MMAP, core::ptr::null_mut());
    if (!obj.elf) {
...
// Linker output endianness set by first input object
    ...
    scn = core::ptr::null_mut();
    while ((scn = elf_nextscn(obj.elf, scn)) != core::ptr::null_mut()) {
    let mut sec_idx: usize = elf_ndxscn(scn);
    const char *sec_name;
...
#[no_mangle]
unsafe extern "C" fn linker_sanity_check_elf(obj: *mut src_obj) -> c_int {
    static int linker_sanity_check_elf(struct src_obj *obj)
    {
    struct src_sec *sec;
    int i, err;
...
#[no_mangle]
unsafe extern "C" fn linker_sanity_check_elf_symtab(obj: *mut src_obj, sec: *mut src_sec) -> c_int {
    static int linker_sanity_check_elf_symtab(struct src_obj *obj, struct src_sec *sec)
    {
    ...
    sym = sec.data.d_buf;
    for (i = 0; i < n; i++, sym++) {
    let mut sym_type: c_int = ELF64_ST_TYPE(sym.st_info);
    let mut sym_bind: c_int = ELF64_ST_BIND(sym.st_info);
    let mut sym_vis: c_int = ELF64_ST_VISIBILITY(sym.st_other);
    if (i == 0) {
...
#[no_mangle]
unsafe extern "C" fn linker_sanity_check_elf_relos(obj: *mut src_obj, sec: *mut src_sec) -> c_int {
    static int linker_sanity_check_elf_relos(struct src_obj *obj, struct src_sec *sec)
    {
...
// check sanity of each relocation
    ...
    for (i = 0; i < n; i++, relo++) {
    let mut sym_idx: usize = ELF64_R_SYM(relo.r_info);
    let mut sym_type: usize = ELF64_R_TYPE(relo.r_info);
...
#[no_mangle]
unsafe extern "C" fn linker_sanity_check_btf(obj: *mut src_obj) -> c_int {
    static int linker_sanity_check_btf(struct src_obj *obj)
    {
    struct btf_type *t;
    int i, n, err;
...
#[no_mangle]
unsafe extern "C" fn linker_sanity_check_btf_ext(obj: *mut src_obj) -> c_int {
    static int linker_sanity_check_btf_ext(struct src_obj *obj)
    {
    let mut err: c_int = 0;
    if (!obj.btf_ext)
...
#[no_mangle]
unsafe extern "C" fn init_sec(linker: *mut bpf_linker, dst_sec: *mut dst_sec, src_sec: *mut src_sec) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn secs_match(dst: *mut dst_sec, src: *mut src_sec) -> bool {
...
#[no_mangle]
unsafe extern "C" fn sec_content_is_same(dst_sec: *mut dst_sec, src_sec: *mut src_sec) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_exec_sec(sec: *mut dst_sec) -> bool {
...
#[no_mangle]
unsafe extern "C" fn exec_sec_bswap(raw_data: *mut c_void, size: c_int) {
    static void exec_sec_bswap(void *raw_data, int size)
    {
    let mut insn_cnt: c_int = size / sizeof(struct bpf_insn);
...
#[no_mangle]
unsafe extern "C" fn extend_sec(linker: *mut bpf_linker, dst: *mut dst_sec, src: *mut src_sec) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_data_sec(sec: *mut src_sec) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_relo_sec(sec: *mut src_sec) -> bool {
...
#[no_mangle]
unsafe extern "C" fn linker_append_sec_data(linker: *mut bpf_linker, obj: *mut src_obj) -> c_int {
    static int linker_append_sec_data(struct bpf_linker *linker, struct src_obj *obj)
    {
    int i, err;
    for (i = 1; i < obj.sec_cnt; i++) {
...
#[no_mangle]
unsafe extern "C" fn linker_append_elf_syms(linker: *mut bpf_linker, obj: *mut src_obj) -> c_int {
    static int linker_append_elf_syms(struct bpf_linker *linker, struct src_obj *obj)
    {
    struct src_sec *symtab = &obj.secs[obj.symtab_sec_idx];
    Elf64_Sym *sym = symtab.data.d_buf;
    int i, n = symtab.shdr.sh_size / symtab.shdr.sh_entsize, err;
    let mut str_sec_idx: c_int = symtab.shdr.sh_link;
    const char *sym_name;
...
// validate value type/size match
    ...
    main_btf, main_inner_def, core::ptr::null_mut(),
    extra_btf, extra_inner_def, core::ptr::null_mut());
    }
    return true;
    mismatch:
    ...
    {
    let mut dst_def: btf_map_def = {}, dst_inner_def = {};
    let mut src_def: btf_map_def = {}, src_inner_def = {};
    ...
    t = skip_mods_and_typedefs(obj.btf, t.type, core::ptr::null_mut());
...
// re-parse existing map definition
    ...
    t = skip_mods_and_typedefs(linker.btf, t.type, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn btf_is_non_static(t: *const btf_type) -> bool {
...
//
    ...
    Elf64_Sym *sym, const char *sym_name, int src_sym_idx)
    {
    struct src_sec *src_sec = core::ptr::null_mut();
    struct dst_sec *dst_sec = core::ptr::null_mut();
    struct glob_sym *glob_sym = core::ptr::null_mut();
    int name_off, sym_type, sym_bind, sym_vis, err;
    let mut btf_sec_id: c_int = 0, btf_id = 0;
    size_t dst_sym_idx;
    Elf64_Sym *dst_sym;
    bool sym_is_extern;
...
// find matching BTF info
    ...
    const char *sec_name = core::ptr::null_mut();
    const struct btf_type *t;
...
#[no_mangle]
unsafe extern "C" fn linker_append_elf_relos(linker: *mut bpf_linker, obj: *mut src_obj) -> c_int {
    static int linker_append_elf_relos(struct bpf_linker *linker, struct src_obj *obj)
    {
...
#[no_mangle]
unsafe extern "C" fn linker_fixup_btf(obj: *mut src_obj) -> c_int {
    static int linker_fixup_btf(struct src_obj *obj)
    {
    const char *sec_name;
    struct src_sec *sec;
...
#[no_mangle]
unsafe extern "C" fn linker_append_btf(linker: *mut bpf_linker, obj: *mut src_obj) -> c_int {
    static int linker_append_btf(struct bpf_linker *linker, struct src_obj *obj)
    {
    const struct btf_type *t;
...
// remap all the types except DATASECs
    ...
    return err;
    while ((type_id = btf_field_iter_next(&it))) {
    let mut new_id: c_int = obj.btf_type_map[*type_id];
...
//
    ...
    let mut new_id: c_int = obj.btf_type_map[src_var.type];
    struct glob_sym *glob_sym = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn linker_append_btf_ext(linker: *mut bpf_linker, obj: *mut src_obj) -> c_int {
    static int linker_append_btf_ext(struct bpf_linker *linker, struct src_obj *obj)
    {
...
#[no_mangle]
unsafe extern "C" fn finalize_btf(linker: *mut bpf_linker) -> c_int {
    static int finalize_btf(struct bpf_linker *linker)
    {
...
#[no_mangle]
unsafe extern "C" fn finalize_btf_ext(linker: *mut bpf_linker) -> c_int {
    static int finalize_btf_ext(struct bpf_linker *linker)
    {
    let mut funcs_sz: usize = 0, lines_sz = 0, core_relos_sz = 0, total_sz = 0;
    let mut func_rec_sz: usize = 0, line_rec_sz = 0, core_relo_rec_sz = 0;
...
// (__u32 *)cur = core_relo_rec_sz;
    ...
    if (err) {
    linker.btf_ext = core::ptr::null_mut();
