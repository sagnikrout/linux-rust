...
//! Source: tools/lib/bpf/libbpf_probes.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn get_ubuntu_kernel_version() -> __u32 {
...
//
#[no_mangle]
unsafe extern "C" fn get_debian_kernel_version(info: *mut utsname) -> __u32 {
...
// Check if this is a Debian kernel.
    ...
    const char *exp_msg = core::ptr::null_mut();
    char buf[4096];
    switch (prog_type) {
...
#[no_mangle]
pub unsafe extern "C" fn libbpf_probe_bpf_prog_type(prog_type: enum bpf_prog_type, opts: *const c_void) -> c_int {
    ...
    ret = probe_prog_load(prog_type, insns, insn_cnt, core::ptr::null_mut(), 0);
    ...
    return libbpf__load_raw_btf_hdr(&hdr, raw_types, str_sec, core::ptr::null_mut(), token_fd);
    }
#[no_mangle]
unsafe extern "C" fn load_local_storage_btf() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn probe_map_create(map_type: enum bpf_map_type) -> c_int {
    static int probe_map_create(enum bpf_map_type map_type)
    {
    LIBBPF_OPTS(bpf_map_create_opts, opts);
    int key_size, value_size, max_entries;
    let mut btf_key_type_id: __u32 = 0, btf_value_type_id = 0;
    let mut fd: c_int = -1, btf_fd = -1, fd_inner = -1, exp_err = 0, err = 0;
...
// we'll get -ENOTSUPP for invalid BTF type ID for struct_ops
    ...
    fd_inner = bpf_map_create(BPF_MAP_TYPE_HASH, core::ptr::null_mut(),
    sizeof(__u32), sizeof(__u32), 1, core::ptr::null_mut());
    if (fd_inner < 0)
    goto cleanup;
    ...
    fd = bpf_map_create(map_type, core::ptr::null_mut(), key_size, value_size, max_entries, &opts);
    err = -errno;
    cleanup:
    if (fd >= 0)
    close(fd);
...
#[no_mangle]
pub unsafe extern "C" fn libbpf_probe_bpf_map_type(map_type: enum bpf_map_type, opts: *const c_void) -> c_int {
