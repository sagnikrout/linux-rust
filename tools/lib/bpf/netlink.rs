...
//! Source: tools/lib/bpf/netlink.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xdp_id_md {
    pub ifindex: c_int,
    pub flags: __u32,
    pub info: xdp_link_info,
    pub feature_flags: __u64,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct xdp_features_md {
    pub ifindex: c_int,
...
unsafe extern "C" fn alloc_iov(iov: *mut iovec, len: c_int) -> c_int {
    ...
    return sock;
    req.nh.nlmsg_pid = 0;
    req.nh.nlmsg_seq = time(core::ptr::null_mut());
    ...
    NLMSG_PAYLOAD(nh, sizeof(*gnl)), core::ptr::null_mut());
...
// id = libbpf_nla_getattr_u16(tb[CTRL_ATTR_FAMILY_ID]);
    ...
    parse_genl_family_id, core::ptr::null_mut(), id);
    }
    ...
    return libbpf_netlink_send_recv(&req, NETLINK_ROUTE, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn bpf_xdp_attach(ifindex: c_int, prog_fd: c_int, flags: __u32, opts: *const bpf_xdp_attach_opts) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_xdp_detach(ifindex: c_int, flags: __u32, opts: *const bpf_xdp_attach_opts) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_xdp_info(cookie: *mut c_void, msg: *mut c_void, tb: *mut nlattr) -> c_int {
    ...
    if (!tb[IFLA_XDP])
    return 0;
    ret = libbpf_nla_parse_nested(xdp_tb, IFLA_XDP_MAX, tb[IFLA_XDP], core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn bpf_xdp_query(ifindex: c_int, xdp_flags: c_int, opts: *mut bpf_xdp_query_opts) -> c_int {
...
// Check whether the single {HW,DRV,SKB} mode is set
    ...
    parse_xdp_features, core::ptr::null_mut(), &md);
    if (err)
    return libbpf_err(err);
...
#[no_mangle]
pub unsafe extern "C" fn bpf_xdp_query_id(ifindex: c_int, flags: c_int, prog_id: *mut __u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(XDP_FLAGS_DRV_MODE: flags &) -> else {
    else if (flags & XDP_FLAGS_DRV_MODE)
// prog_id = opts.drv_prog_id;
#[no_mangle]
pub unsafe extern "C" fn if(XDP_FLAGS_HW_MODE: flags &) -> else {
    else if (flags & XDP_FLAGS_HW_MODE)
// prog_id = opts.hw_prog_id;
#[no_mangle]
pub unsafe extern "C" fn if(XDP_FLAGS_SKB_MODE: flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn clsact_config(req: *mut libbpf_nla_req, hook: *const bpf_tc_hook) -> c_int {
...
unsafe extern "C" fn qdisc_config(req: *mut libbpf_nla_req, hook: *const bpf_tc_hook) -> c_int {
    ...
    {
    const char *qdisc = OPTS_GET(hook, qdisc, core::ptr::null_mut());
...
pub unsafe extern "C" fn bpf_tc_hook_destroy(hook: *mut bpf_tc_hook) -> c_int {
    ...
    case BPF_TC_INGRESS:
    case BPF_TC_EGRESS:
    return libbpf_err(__bpf_tc_detach(hook, core::ptr::null_mut(), true));
    case BPF_TC_QDISC:
    case BPF_TC_INGRESS | BPF_TC_EGRESS:
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bpf_cb_ctx {
    pub opts: *mut bpf_tc_opts,
    pub processed: bool,
}
    ...
    if (!tb[TCA_OPTIONS])
    return NL_CONT;
    libbpf_nla_parse_nested(tbb, TCA_BPF_MAX, tb[TCA_OPTIONS], core::ptr::null_mut());
    if (!tbb[TCA_BPF_ID])
    return -EINVAL;
    ...
    NLMSG_PAYLOAD(nh, sizeof(*tc)), core::ptr::null_mut());
    if (!tb[TCA_KIND])
    return NL_CONT;
...
pub unsafe extern "C" fn bpf_tc_attach(hook: *const bpf_tc_hook, opts: *mut bpf_tc_opts) -> c_int {
    ...
    nlattr_end_nested(&req, nla);
    info.opts = opts;
    ret = libbpf_netlink_send_recv(&req, NETLINK_ROUTE, get_tc_info, core::ptr::null_mut(),
    &info);
    if (ret < 0)
    return libbpf_err(ret);
    if (!info.processed)
    ...
    const bool flush)
    {
    let mut protocol: __u32 = 0, handle, priority, parent, prog_id, flags;
    int ret, ifindex, attach_point, prog_fd;
    struct libbpf_nla_req req;
    if (!hook ||
    !OPTS_VALID(hook, bpf_tc_hook) ||
