...
pub unsafe extern "C" fn perf_evsel__exit(evsel: *mut perf_evsel) {
    ...
    {
    assert(evsel.fd == core::ptr::null_mut());  /* If not fds were not closed. */
    assert(evsel.mmap == core::ptr::null_mut()); /* If not munmap wasn't called. */
    assert(evsel.sample_id == core::ptr::null_mut()); /* If not free_id wasn't called. */
...
pub unsafe extern "C" fn perf_evsel__delete(evsel: *mut perf_evsel) {
    ...
    : core::ptr::null_mut())
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__alloc_fd(evsel: *mut perf_evsel, ncpus: c_int, nthreads: c_int) -> c_int {
...
// fd = -1;
    ...
    return evsel.fd != core::ptr::null_mut() ? 0 : -ENOMEM;
    }
#[no_mangle]
unsafe extern "C" fn perf_evsel__alloc_mmap(evsel: *mut perf_evsel, ncpus: c_int, nthreads: c_int) -> c_int {
    static int perf_evsel__alloc_mmap(struct perf_evsel *evsel, int ncpus, int nthreads)
    {
    evsel.mmap = xyarray__new(ncpus, nthreads, sizeof(struct perf_mmap));
    return evsel.mmap != core::ptr::null_mut() ? 0 : -ENOMEM;
    }
    static int
...
#[no_mangle]
unsafe extern "C" fn get_group_fd(evsel: *mut perf_evsel, cpu_map_idx: c_int, thread: c_int, group_fd: *mut c_int) -> c_int {
...
// group_fd = *fd;
    ...
    if (empty_cpu_map == core::ptr::null_mut()) {
    ...
    if (empty_thread_map == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn perf_evsel__close_fd_cpu(evsel: *mut perf_evsel, cpu_map_idx: c_int) {
...
pub unsafe extern "C" fn perf_evsel__munmap(evsel: *mut perf_evsel) {
    ...
    {
    int idx, thread;
    if (evsel.fd == core::ptr::null_mut() || evsel.mmap == core::ptr::null_mut())
    return;
    for (idx = 0; idx < xyarray__max_x(evsel.fd); idx++) {
    for (thread = 0; thread < xyarray__max_y(evsel.fd); thread++) {
    int *fd = FD(evsel, idx, thread);
    if (fd == core::ptr::null_mut() || *fd < 0)
    continue;
    perf_mmap__munmap(MMAP(evsel, idx, thread));
    }
    }
    xyarray__delete(evsel.mmap);
    evsel.mmap = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__mmap(evsel: *mut perf_evsel, pages: c_int) -> c_int {
    ...
    if (evsel.fd == core::ptr::null_mut() || evsel.mmap)
    return -EINVAL;
    ...
    let mut cpu: perf_cpu = perf_cpu_map__cpu(evsel.cpus, idx);
    if (fd == core::ptr::null_mut() || *fd < 0)
    continue;
    map = MMAP(evsel, idx, thread);
    perf_mmap__init(map, core::ptr::null_mut(), false, core::ptr::null_mut());
    ...
    {
    int *fd = FD(evsel, cpu_map_idx, thread);
    if (fd == core::ptr::null_mut() || *fd < 0 || MMAP(evsel, cpu_map_idx, thread) == core::ptr::null_mut())
...
pub unsafe extern "C" fn perf_evsel__read_size(evsel: *mut perf_evsel) -> c_int {
    ...
    {
    let mut read_format: u64 = evsel.attr.read_format;
...
// This only reads values for the leader
    ...
    {
    let mut size: usize = perf_evsel__read_size(evsel);
    int *fd = FD(evsel, cpu_map_idx, thread);
    let mut read_format: u64 = evsel.attr.read_format;
    u64 *data;
    let mut idx: c_int = 1;
...
//
    ...
    int thread;
    for (thread = 0; thread < xyarray__max_y(evsel.fd); thread++) {
    let mut err: c_int = perf_evsel__ioctl(evsel, ioc, arg, cpu_map_idx, thread);
    if (err)
    return err;
    }
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__enable_cpu(evsel: *mut perf_evsel, cpu_map_idx: c_int) -> c_int {
    int perf_evsel__enable_cpu(struct perf_evsel *evsel, int cpu_map_idx)
    {
    return perf_evsel__run_ioctl(evsel, PERF_EVENT_IOC_ENABLE, core::ptr::null_mut(), cpu_map_idx);
    }
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__enable_thread(evsel: *mut perf_evsel, thread: c_int) -> c_int {
    ...
    err = perf_evsel__ioctl(evsel, PERF_EVENT_IOC_ENABLE, core::ptr::null_mut(), idx, thread);
    if (err)
    return err;
    }
    return 0;
...
pub unsafe extern "C" fn perf_evsel__enable(evsel: *mut perf_evsel) -> c_int {
    ...
    err = perf_evsel__run_ioctl(evsel, PERF_EVENT_IOC_ENABLE, core::ptr::null_mut(), i);
    return err;
    }
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__disable_cpu(evsel: *mut perf_evsel, cpu_map_idx: c_int) -> c_int {
    int perf_evsel__disable_cpu(struct perf_evsel *evsel, int cpu_map_idx)
    {
    return perf_evsel__run_ioctl(evsel, PERF_EVENT_IOC_DISABLE, core::ptr::null_mut(), cpu_map_idx);
    }
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__disable(evsel: *mut perf_evsel) -> c_int {
    ...
    let mut err: c_int = 0;
    for (i = 0; i < xyarray__max_x(evsel.fd) && !err; i++)
    err = perf_evsel__run_ioctl(evsel, PERF_EVENT_IOC_DISABLE, core::ptr::null_mut(), i);
    return err;
    }
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__apply_filter(evsel: *mut perf_evsel, filter: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__alloc_id(evsel: *mut perf_evsel, ncpus: c_int, nthreads: c_int) -> c_int {
    int perf_evsel__alloc_id(struct perf_evsel *evsel, int ncpus, int nthreads)
    {
    if (ncpus == 0 || nthreads == 0)
    return 0;
    evsel.sample_id = xyarray__new(ncpus, nthreads, sizeof(struct perf_sample_id));
    if (evsel.sample_id == core::ptr::null_mut())
    return -ENOMEM;
    evsel.id = zalloc(ncpus * nthreads * sizeof(u64));
    if (evsel.id == core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__free_id(evsel: *mut perf_evsel) {
...
#[no_mangle]
pub unsafe extern "C" fn perf_evsel__attr_has_per_thread_sample_period(evsel: *mut perf_evsel) -> bool {
    ...
    if (res.tid == tid)
    return &res.period;
    if (sid.evsel == core::ptr::null_mut())
    return core::ptr::null_mut();
