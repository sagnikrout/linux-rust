...
//! Source: tools/hv/hv_vss_daemon.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn vss_do_freeze(dir: *mut c_char, cmd: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_dev_loop(blkname: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn vss_operate(operation: c_int) -> c_int {
    ...
    unsigned int cmd;
    let mut error: c_int = 0, root_seen = 0, save_errno = 0;
    switch (operation) {
    case VSS_OP_FREEZE:
    cmd = FIFREEZE;
    break;
    ...
    if (hasmntopt(ent, MNTOPT_RO) != core::ptr::null_mut())
    continue;
