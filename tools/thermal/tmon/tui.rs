...
//! Source: tools/thermal/tmon/tui.c
#![no_std]
...
//
    ...
    let mut maxwidth: static int = 200;
pub const TITLE_BAR_HIGHT: c_int = 1;

// daemon mode flag (set by startup parameter -d)
    static int  tui_disabled;
#[no_mangle]
unsafe extern "C" fn close_panel(p: *mut PANEL) {
...
#[no_mangle]
unsafe extern "C" fn close_window(win: *mut WINDOW) {
...
// wrap at 5
pub const DIAG_DEV_ROWS: c_int = 5;
//
// list cooling devices + "set temp" entry; wraps after 5 rows, if they fit
//
#[no_mangle]
unsafe extern "C" fn diag_dev_rows() -> c_int {
    static int diag_dev_rows(void)
    {
    let mut entries: c_int = ptdata.nr_cooling_dev + 1;
    let mut rows: c_int = max(DIAG_DEV_ROWS, (entries + 1) / 2);
...
// show cdev binding, y is the global cooling device instance
    ...
    let mut tz_inst: c_int = ptdata.tzi[i].instance;
    for (j = 0; j < ptdata.nr_cooling_dev; j++) {
    int cdev_inst;
    y = j;
...
#[no_mangle]
unsafe extern "C" fn handle_input_val(ch: c_int) {
...
#[no_mangle]
unsafe extern "C" fn handle_input_choice(ch: c_int) {
...
#[no_mangle]
unsafe extern "C" fn trip_type_to_char(type: c_int) -> c_char {
...
//
#[no_mangle]
unsafe extern "C" fn draw_tp_line(tz: c_int, y: c_int) {
...
// draw a line as ruler
    ...
    for (i = 0; i < ptdata.nr_tz_sensor; i++) {
    let mut temp: c_int = trec[cur_thermal_record].temp[i] / 1000;
    let mut y: c_int = 0;
...
// draw trip point from low to high for each tz
    for (i = 0; i < ptdata.nr_tz_sensor; i++) {
    let mut inst: c_int = ptdata.tzi[i].instance;
