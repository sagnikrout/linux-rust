...
//! Source: tools/thermal/tmon/sysfs.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn sysfs_set_ulong(path: *mut c_char, filename: *mut c_char, val: c_ulong) -> c_int {
...
pub const NR_THERMAL_RECORDS: c_int = 3;
    struct thermal_data_record trec[NR_THERMAL_RECORDS];
    int cur_thermal_record; /* index to the trec array */
#[no_mangle]
unsafe extern "C" fn sysfs_get_ulong(path: *mut c_char, filename: *mut c_char, p_ulong: *mut c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sysfs_get_string(path: *mut c_char, filename: *mut c_char, str: *mut c_char) -> c_int {
...
unsafe extern "C" fn get_instance_id(name: *mut c_char, pos: c_int, skip: c_int) -> c_int {
    ...
    let mut i: c_int = 0;
    ch = strtok(name, "_");
    while (ch != core::ptr::null_mut()) {
    ++i;
    syslog(LOG_INFO, "%s:%s:%s:%d", __func__, name, ch, i);
    ch = strtok(core::ptr::null_mut(), "_");
    if (pos == i)
    return atol(ch + skip);
    }
    return -1;
...
// circular buffer for keeping historic data
    ...
    gettimeofday(&trec[next_thermal_record].tv, core::ptr::null_mut());
    if (tmon_log) {
