...
//! Source: arch/powerpc/lib/sstep.c
#![no_std]
...
pub const MSR_MASK: c_uint = 0xffffffff87c0ffffUL;

pub const MSR_MASK: c_uint = 0x87c0ffff;

// Bits in XER
pub const XER_SO: c_uint = 0x80000000U;
pub const XER_OV: c_uint = 0x40000000U;
pub const XER_CA: c_uint = 0x20000000U;
pub const XER_OV32: c_uint = 0x00080000U;
pub const XER_CA32: c_uint = 0x00040000U;
...
pub const IS_LE: c_int = 1;
pub const IS_BE: c_int = 0;

pub const IS_LE: c_int = 0;
pub const IS_BE: c_int = 1;
...
//
    ...
    {
    let mut bo: c_uint = (instr >> 21) & 0x1f;
    unsigned int bi;
    if ((bo & 4) == 0) {
// decrement counter
    op.type |= DECCTR;
...
//
    ...
    ea += regs.gpr[ra];
#[no_mangle]
pub unsafe extern "C" fn if(!ra: !prefix_r &&) -> else {
    else if (!prefix_r && !ra)
    ; /* Leave ea as is */
#[no_mangle]
pub unsafe extern "C" fn if(_arg: prefix_r) -> else {
    else if (prefix_r)
    ea += regs.nip;
//
...
//
#[no_mangle]
unsafe extern "C" fn max_align(x: c_ulong) -> nokprobe_inline unsigned long {
...
#[no_mangle]
unsafe extern "C" fn byterev_2(x: c_ulong) -> nokprobe_inline unsigned long {
    static nokprobe_inline unsigned long byterev_2(unsigned long x)
    {
    return ((x >> 8) & 0xff) | ((x & 0xff) << 8);
    }
#[no_mangle]
unsafe extern "C" fn byterev_4(x: c_ulong) -> nokprobe_inline unsigned long {
...
#[no_mangle]
unsafe extern "C" fn byterev_8(x: c_ulong) -> nokprobe_inline unsigned long {
...
#[no_mangle]
unsafe extern "C" fn do_byte_reverse(ptr: *mut c_void, nb: c_int) -> nokprobe_inline void {
...
//
#[no_mangle]
unsafe extern "C" fn __copy_mem_in(dest: *mut u8, ea: c_ulong, nb: c_int, regs: *mut pt_regs) -> __always_inline int {
...
#[no_mangle]
unsafe extern "C" fn copy_mem_in(dest: *mut u8, ea: c_ulong, nb: c_int, regs: *mut pt_regs) -> nokprobe_inline int {
...
//
#[no_mangle]
unsafe extern "C" fn __copy_mem_out(dest: *mut u8, ea: c_ulong, nb: c_int, regs: *mut pt_regs) -> __always_inline int {
...
#[no_mangle]
unsafe extern "C" fn copy_mem_out(dest: *mut u8, ea: c_ulong, nb: c_int, regs: *mut pt_regs) -> nokprobe_inline int {
...
#[no_mangle]
pub unsafe extern "C" fn if(SIGNEXT: op->type &) -> else {
...
// size == 4 is the only case here
    ...
    if (size == 16) {
    let mut v: c_ulong = *(unsigned long *)(mem + 8);
...
// lxvw4x, lxvwsx
    ...
    let mut val: u32 = reg.w[IS_LE ? 3 : 0];
    for (; j < 4; ++j) {
    i = IS_LE ? 3 - j : j;
    reg.w[i] = val;
    }
...
#[no_mangle]
unsafe extern "C" fn __emulate_dcbz(ea: c_ulong) -> __always_inline int {
    static __always_inline int __emulate_dcbz(unsigned long ea)
    {
    unsigned long i;
    let mut size: c_ulong = l1_dcache_bytes();
...
#[no_mangle]
pub unsafe extern "C" fn emulate_dcbz(ea: c_ulong, regs: *mut pt_regs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_ca32(op: *mut instruction_op, val: bool) -> nokprobe_inline void {
...
#[no_mangle]
unsafe extern "C" fn trap_compare(v1: c_long, v2: c_long) -> nokprobe_inline int {
...
pub unsafe extern "C" fn if(v2: v1 >) -> else {
    ...
    ret |= 0x02;
#[no_mangle]
pub unsafe extern "C" fn if(long)v2: (unsigned long)v1 > (unsigned) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn handle_stack_update(ea: c_ulong, regs: *mut pt_regs) -> nokprobe_inline int {
...
#[no_mangle]
unsafe extern "C" fn do_signext(valp: *mut c_ulong, size: c_int) -> nokprobe_inline void {
...
#[no_mangle]
unsafe extern "C" fn do_byterev(valp: *mut c_ulong, size: c_int) -> nokprobe_inline void {
...
//
#[no_mangle]
pub unsafe extern "C" fn emulate_update_regs(regs: *mut pt_regs, op: *mut instruction_op) {
...
//
#[no_mangle]
pub unsafe extern "C" fn emulate_loadstore(regs: *mut pt_regs, op: *mut instruction_op) -> c_int {
...
//
    ...
    case LOAD_VSX: {
    let mut msrbit: c_ulong = MSR_VSX;
//
...
//
    ...
    rd = op.reg;
    for (i = 0; i < size; i += 4) {
    let mut v32: c_uint = regs.gpr[rd];
    nb = size - i;
    if (nb > 4)
    nb = 4;
    if (unlikely(cross_endian))
...
//
#[no_mangle]
pub unsafe extern "C" fn emulate_step(regs: *mut pt_regs, instr: ppc_inst_t) -> c_int {
