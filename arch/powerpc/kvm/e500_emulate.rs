...
//! Source: arch/powerpc/kvm/e500_emulate.c
#![no_std]
...
pub const XOP_DCBTLS: c_int = 166;
pub const XOP_MSGSND: c_int = 206;
pub const XOP_MSGCLR: c_int = 238;
pub const XOP_MFTMR: c_int = 366;
pub const XOP_TLBIVAX: c_int = 786;
pub const XOP_TLBSX: c_int = 914;
pub const XOP_TLBRE: c_int = 946;
pub const XOP_TLBWE: c_int = 978;
pub const XOP_TLBILX: c_int = 18;
pub const XOP_EHPRIV: c_int = 270;

#[no_mangle]
unsafe extern "C" fn dbell2prio(param: c_ulong) -> c_int {
    static int dbell2prio(ulong param)
    {
    let mut msg: c_int = param & PPC_DBELL_TYPE_MASK;
...
#[no_mangle]
unsafe extern "C" fn kvmppc_e500_emul_msgclr(vcpu: *mut kvm_vcpu, rb: c_int) -> c_int {
    static int kvmppc_e500_emul_msgclr(struct kvm_vcpu *vcpu, int rb)
    {
    let mut param: c_ulong = vcpu.arch.regs.gpr[rb];
    let mut prio: c_int = dbell2prio(param);
    if (prio < 0)
    return EMULATE_FAIL;
...
#[no_mangle]
unsafe extern "C" fn kvmppc_e500_emul_msgsnd(vcpu: *mut kvm_vcpu, rb: c_int) -> c_int {
    static int kvmppc_e500_emul_msgsnd(struct kvm_vcpu *vcpu, int rb)
    {
    let mut param: c_ulong = vcpu.arch.regs.gpr[rb];
    let mut prio: c_int = dbell2prio(rb);
    let mut pir: c_int = param & PPC_DBELL_PIR_MASK;
    unsigned long i;
    struct kvm_vcpu *cvcpu;
    if (prio < 0)
    return EMULATE_FAIL;
    kvm_for_each_vcpu(i, cvcpu, vcpu.kvm) {
    let mut cpir: c_int = cvcpu.arch.shared.pir;
...
#[no_mangle]
unsafe extern "C" fn kvmppc_e500_emul_dcbtls(vcpu: *mut kvm_vcpu) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_core_emulate_mtspr_e500(vcpu: *mut kvm_vcpu, sprn: c_int, spr_val: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_core_emulate_mfspr_e500(vcpu: *mut kvm_vcpu, sprn: c_int, spr_val: *mut c_ulong) -> c_int {
