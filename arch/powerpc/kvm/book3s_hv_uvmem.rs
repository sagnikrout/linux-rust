...
//! Source: arch/powerpc/kvm/book3s_hv_uvmem.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct kvmppc_uvmem_page_pvt {
    pub kvm: *mut kvm,
    pub gpa: c_ulong,
    pub skip_page_out: bool,
    pub remove_gfn: bool,
}

#[no_mangle]
pub unsafe extern "C" fn kvmppc_uvmem_available() -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_uvmem_slot_init(kvm: *mut kvm, slot: *const kvm_memory_slot) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn kvmppc_uvmem_slot_free(kvm: *mut kvm, slot: *const kvm_memory_slot) {
    ...
    let mut index: c_ulong = gfn - p.base_pfn;
...
#[no_mangle]
unsafe extern "C" fn kvmppc_gfn_secure_mem_pfn(gfn: c_ulong, kvm: *mut kvm) {
...
#[no_mangle]
unsafe extern "C" fn kvmppc_gfn_shared(gfn: c_ulong, kvm: *mut kvm) {
...
#[no_mangle]
unsafe extern "C" fn kvmppc_gfn_remove(gfn: c_ulong, kvm: *mut kvm) {
...
//
    ...
    {
    struct kvmppc_uvmem_slot *p = core::ptr::null_mut(), *iter;
...
//
    ...
    let mut index: c_ulong = i - p.base_pfn;
...
// gfn = i;
    ...
    {
    let mut gfn: c_ulong = memslot.base_gfn;
    unsigned long end, start = gfn_to_hva(kvm, gfn);
    vm_flags_t vm_flags;
    let mut ret: c_int = 0;
    struct vm_area_struct *vma;
    let mut merge_flag: c_int = (merge) ? MADV_MERGEABLE : MADV_UNMERGEABLE;
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_h_svm_init_start(kvm: *mut kvm) -> c_ulong {
...
//
    ...
    let mut mig: migrate_vma = { 0 };
    struct page *dpage, *spage;
    struct kvmppc_uvmem_page_pvt *pvt;
    unsigned long pfn;
    let mut ret: c_int = U_SUCCESS;
...
// The requested page is already paged-out, nothing to do
    if (!kvmppc_gfn_is_uvmem_pfn(gpa >> page_shift, kvm, core::ptr::null_mut()))
    return ret;
...
// Fetch the VMA if addr is not in the latest fetched one
    ...
    PAGE_SHIFT, kvm, pvt.gpa, core::ptr::null_mut()))
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_h_svm_init_abort(kvm: *mut kvm) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_h_svm_init_done(kvm: *mut kvm) -> c_ulong {
...
// Fail the page-in request of an already paged-in page
    if (kvmppc_gfn_is_uvmem_pfn(gfn, kvm, core::ptr::null_mut()))
    goto out_unlock;
    end = start + (1UL << page_shift);
    vma = find_vma_intersection(kvm.mm, start, end);
    if (!vma || vma.vm_start > start || vma.vm_end < end)
...
//
#[no_mangle]
unsafe extern "C" fn kvmppc_uvmem_migrate_to_ram(vmf: *mut vm_fault) -> vm_fault_t {
...
//
#[no_mangle]
unsafe extern "C" fn kvmppc_uvmem_folio_free(folio: *mut folio) {
    ...
    page.zone_device_data = core::ptr::null_mut();
    if (pvt.remove_gfn)
...
//
    ...
    if (!vma || vma.vm_start > start || vma.vm_end < end)
    goto out;
    if (!kvmppc_svm_page_out(vma, start, end, page_shift, kvm, gpa, core::ptr::null_mut()))
    ret = H_SUCCESS;
    out:
    mmap_read_unlock(kvm.mm);
    srcu_read_unlock(&kvm.srcu, srcu_idx);
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_send_page_to_uv(kvm: *mut kvm, gfn: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_uvmem_memslot_create(kvm: *mut kvm, new: *const kvm_memory_slot) -> c_int {
    int kvmppc_uvmem_memslot_create(struct kvm *kvm, const struct kvm_memory_slot *new)
    {
    let mut ret: c_int = __kvmppc_uvmem_memslot_create(kvm, new);
    if (!ret)
    ret = kvmppc_uv_migrate_mem_slot(kvm, new);
    return ret;
    }
#[no_mangle]
pub unsafe extern "C" fn kvmppc_uvmem_memslot_delete(kvm: *mut kvm, old: *const kvm_memory_slot) {
    void kvmppc_uvmem_memslot_delete(struct kvm *kvm, const struct kvm_memory_slot *old)
    {
    __kvmppc_uvmem_memslot_delete(kvm, old);
    }
#[no_mangle]
unsafe extern "C" fn kvmppc_get_secmem_size() -> u64 {
...
//
    for_each_compatible_node(np, core::ptr::null_mut(), "ibm,secure-memory") {
    ...
    if (size)
    return size;
    np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "ibm,uv-firmware");
    if (!np)
    goto out;
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_uvmem_init() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_uvmem_free() {
