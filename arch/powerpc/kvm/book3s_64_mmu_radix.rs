...
//! Source: arch/powerpc/kvm/book3s_64_mmu_radix.c
#![no_std]
...
//
    ...
    let mut is_load: bool = !!to;
    if (kvmhv_is_nestedv2())
    return H_UNSUPPORTED;
// Can't access quadrants 1 or 2 in non-HV mode, call the HV to do it
    if (kvmhv_on_pseries())
    return plpar_hcall_norets(H_COPY_TOFROM_GUEST, lpid, pid, eaddr,
    (to != core::ptr::null_mut()) ? __pa(to): 0,
    (from != core::ptr::null_mut()) ? __pa(from): 0, n);
    if (eaddr & (0xFFFUL << 52))
    return ret;
    quadrant = 1;
    if (!pid)
...
// switch the pid first to avoid running host with unallocated pid
    ...
    {
    let mut lpid: c_int = vcpu.kvm.arch.lpid;
    int pid;
...
// If accessing quadrant 3 then pid is expected to be 0
    ...
    {
    long ret;
    ret = kvmhv_copy_tofrom_guest_radix(vcpu, eaddr, to, core::ptr::null_mut(), n);
    if (ret > 0)
    ...
    unsigned long n)
    {
    return kvmhv_copy_tofrom_guest_radix(vcpu, eaddr, core::ptr::null_mut(), from, n);
    }
...
// Check AMR/IAMR to see if strict mode is in force
    ...
    {
    let mut psize: c_ulong = PAGE_SIZE;
    int psi;
    long rc;
    unsigned long rb;
    if (pshift)
...
#[no_mangle]
unsafe extern "C" fn kvmppc_radix_flush_pwc(kvm: *mut kvm, lpid: u64) {
...
#[no_mangle]
unsafe extern "C" fn kvmppc_pte_free(ptep: *mut pte_t) {
...
#[no_mangle]
unsafe extern "C" fn kvmppc_pmd_free(pmdp: *mut pmd_t) {
...
// Called with kvm->mmu_lock held
    ...
    let mut page_size: c_ulong = PAGE_SIZE;
    unsigned long hpa;
...
#[no_mangle]
pub unsafe extern "C" fn if(PUD_SHIFT: shift ==) -> else {
...
//
    ...
    PAGE_SHIFT, core::ptr::null_mut(), lpid);
    }
    }
    kvmppc_pte_free(pte);
    }
    ...
    PMD_SHIFT, core::ptr::null_mut(), lpid);
    }
    } else {
    pte_t *pte;
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_free_pgtable_radix(kvm: *mut kvm, pgd: *mut pgd_t, lpid: u64) {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_free_radix(kvm: *mut kvm) {
    ...
    kvm.arch.pgtable = core::ptr::null_mut();
    }
    }
...
//
    ...
    pgd_t *pgd;
    p4d_t *p4d;
    pud_t *pud, *new_pud = core::ptr::null_mut();
    pmd_t *pmd, *new_pmd = core::ptr::null_mut();
    pte_t *ptep, *new_ptep = core::ptr::null_mut();
    int ret;
// Traverse the guest's 2nd-level tree, allocate new levels needed
    pgd = pgtable + pgd_index(gpa);
    p4d = p4d_offset(pgd, gpa);
    pud = core::ptr::null_mut();
    if (p4d_present(*p4d))
    pud = pud_offset(p4d, gpa);
    else
    new_pud = pud_alloc_one(kvm.mm, gpa);
    pmd = core::ptr::null_mut();
    if (pud && pud_present(*pud) && !pud_leaf(*pud))
    pmd = pmd_offset(pud, gpa);
#[no_mangle]
pub unsafe extern "C" fn if(1: level <=) -> else {
    else if (level <= 1)
...
// Now traverse again under the lock and change the tree
    ...
    new_pud = core::ptr::null_mut();
    }
    pud = pud_offset(p4d, gpa);
    if (pud_leaf(*pud)) {
    let mut hgpa: c_ulong = gpa & PUD_MASK;
...
// Valid 1GB page here already, remove it
    kvmppc_unmap_pte(kvm, (pte_t *)pud, hgpa, PUD_SHIFT, core::ptr::null_mut(),
    lpid);
    }
    if (level == 2) {
    if (!pud_none(*pud)) {
...
//
    ...
    new_pmd = core::ptr::null_mut();
    }
    pmd = pmd_offset(pud, gpa);
    if (pmd_leaf(*pmd)) {
    let mut lgpa: c_ulong = gpa & PMD_MASK;
// Check if we raced and someone else has set the same thing
    if (level == 1) {
...
// Valid 2MB page here already, remove it
    kvmppc_unmap_pte(kvm, pmdp_ptep(pmd), lgpa, PMD_SHIFT, core::ptr::null_mut(),
    lpid);
    }
    if (level == 1) {
    if (!pmd_none(*pmd)) {
...
//
    ...
    let mut upgrade_write: bool = false;
    pte_t pte, *ptep;
...
// Allocate space in the tree and write the PTE
    ...
    mmu_seq, kvm.arch.lpid, core::ptr::null_mut(), core::ptr::null_mut());
    if (inserted_pte)
// inserted_pte = pte;
    if (levelp)
// levelp = level;
...
// Increment number of large pages if we (successfully) inserted one
    ...
    kvm.stat.num_2M_pages++;
#[no_mangle]
pub unsafe extern "C" fn if(2: level ==) -> else {
    else if (level == 2)
    kvm.stat.num_1G_pages++;
    }
    return ret;
...
// Called with kvm->mmu_lock held
    ...
    {
    pte_t *ptep;
    let mut gpa: c_ulong = gfn << PAGE_SHIFT;
    unsigned int shift;
    if (kvm.arch.secure_guest & KVMPPC_SECURE_INIT_DONE) {
...
// Returns the number of PAGE_SIZE pages that are dirty
    ...
    {
    let mut gfn: c_ulong = memslot.base_gfn + pagenum;
    let mut gpa: c_ulong = gfn << PAGE_SHIFT;
    pte_t *ptep, pte;
    unsigned int shift;
    let mut ret: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn kvmhv_get_rmmu_info(kvm: *mut kvm, info: *mut kvm_ppc_rmmu_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_init_vm_radix(kvm: *mut kvm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pte_ctor(addr: *mut c_void) {
    static void pte_ctor(void *addr)
    {
    memset(addr, 0, RADIX_PTE_TABLE_SIZE);
    }
#[no_mangle]
unsafe extern "C" fn pmd_ctor(addr: *mut c_void) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct debugfs_radix_state {
    pub kvm: *mut kvm,
    pub mutex: mutex,
    pub gpa: c_ulong,
    pub lpid: c_int,
...
#[no_mangle]
unsafe extern "C" fn debugfs_radix_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn debugfs_radix_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvmhv_radix_debugfs_init(kvm: *mut kvm) {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_radix_init() -> c_int {
    int kvmppc_radix_init(void)
    {
    let mut size: c_ulong = sizeof(void *) << RADIX_PTE_INDEX_SIZE;
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_radix_exit() {
