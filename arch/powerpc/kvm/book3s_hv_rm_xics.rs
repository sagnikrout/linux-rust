...
//! Source: arch/powerpc/kvm/book3s_hv_rm_xics.c
#![no_std]
...
// Macro flag: #define DEBUG_PASSUP
    let mut h_ipi_redirect: c_int = 1;
    EXPORT_SYMBOL(h_ipi_redirect);
    let mut kvm_irq_bypass: c_int = 1;
...
#[no_mangle]
pub unsafe extern "C" fn icp_send_hcore_msg(hcore: c_int, vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
pub unsafe extern "C" fn find_available_hostcore(action: c_int) -> c_int {
    static inline int find_available_hostcore(int action)
    {
    int core;
    let mut my_core: c_int = smp_processor_id() >> threads_shift;
...
#[no_mangle]
unsafe extern "C" fn icp_rm_clr_vcpu_irq(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
pub unsafe extern "C" fn xics_rm_h_xirr_x(vcpu: *mut kvm_vcpu) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn xics_rm_h_xirr(vcpu: *mut kvm_vcpu) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn xics_rm_h_cppr(vcpu: *mut kvm_vcpu, cppr: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ics_rm_eoi(vcpu: *mut kvm_vcpu, irq: u32) -> c_int {
...
//
    ...
    icp_rm_deliver_irq(xics, core::ptr::null_mut(), irq, false);
...
// Handle passthrough interrupts
    ...
    ++vcpu.stat.pthru_all;
    if (state.intr_cpu != -1) {
    let mut pcpu: c_int = raw_smp_processor_id();
...
#[no_mangle]
pub unsafe extern "C" fn xics_rm_h_eoi(vcpu: *mut kvm_vcpu, xirr: c_ulong) -> c_int {
    int xics_rm_h_eoi(struct kvm_vcpu *vcpu, unsigned long xirr)
    {
    struct kvmppc_xics *xics = vcpu.kvm.arch.xics;
    struct kvmppc_icp *icp = vcpu.arch.icp;
    let mut irq: u32 = xirr & 0x00ffffff;
    if (!xics || !xics.real_mode)
    return H_TOO_HARD;
//
// ICP State: EOI
...
#[no_mangle]
unsafe extern "C" fn icp_eoi(d: *mut irq_data, hwirq: u32, xirr: __be32, again: *mut bool) {
...
#[no_mangle]
unsafe extern "C" fn xics_opal_set_server(hw_irq: c_uint, server_cpu: c_int) -> c_int {
    static int xics_opal_set_server(unsigned int hw_irq, int server_cpu)
    {
    let mut mangle_cpu: c_uint = get_hard_smp_processor_id(server_cpu) << 2;
...
//
#[no_mangle]
pub unsafe extern "C" fn this_cpu_inc_rm(addr: *mut unsigned int __percpu) {
...
//
#[no_mangle]
unsafe extern "C" fn kvmppc_rm_handle_irq_desc(desc: *mut irq_desc) {
...
//
#[no_mangle]
unsafe extern "C" fn rm_host_ipi_action(action: c_int, data: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_xics_ipi_action() {
...
// Order these stores against the real mode KVM
    rm_corep.rm_data = core::ptr::null_mut();
    smp_wmb();
