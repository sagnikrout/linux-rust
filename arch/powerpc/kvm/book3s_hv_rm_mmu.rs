...
//! Source: arch/powerpc/kvm/book3s_hv_rm_mmu.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn global_invalidates(kvm: *mut kvm) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn is_mmio_hpte(v: c_ulong, r: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn fixup_tlbie_lpid(rb_value: c_ulong, lpid: c_ulong) {
...
#[no_mangle]
pub unsafe extern "C" fn kvmppc_h_bulk_remove(vcpu: *mut kvm_vcpu) -> c_long {
    ...
    int global;
    let mut ret: long int = H_SUCCESS;
...
// Don't lose R/C bit updates done by hardware
    ...
    unsigned long v, r;
    int i, n = 1;
    struct revmap_entry *rev = core::ptr::null_mut();
    if (kvm_is_radix(kvm))
    return H_FUNCTION;
    if (pte_index >= kvmppc_hpt_npte(&kvm.arch.hpt))
    return H_PARAMETER;
    ...
    struct revmap_entry *rev;
    unsigned long *rmap;
    let mut ret: c_long = H_NOT_FOUND;
    if (kvm_is_radix(kvm))
    return H_FUNCTION;
    if (pte_index >= kvmppc_hpt_npte(&kvm.arch.hpt))
    return H_PARAMETER;
    ...
    rmap = revmap_for_hpte(kvm, v, gr, core::ptr::null_mut(), core::ptr::null_mut());
    if (rmap) {
    lock_rmap(rmap);
...
// Used later to detect if we might have been invalidated
    ...
    if (ret != H_SUCCESS)
    goto out_unlock;
    ret = kvmppc_get_hpa(vcpu, mmu_seq, src, 0, &src_pa, core::ptr::null_mut());
    if (ret != H_SUCCESS)
    goto out_unlock;
// Copy the page
...
#[no_mangle]
pub unsafe extern "C" fn if(H_ZERO_PAGE: flags &) -> else {
...
// ((char *)hptep + 14) = rbyte;
    ...
    {
    struct mmio_hpte_cache_entry *entry = core::ptr::null_mut();
    unsigned int pshift;
    unsigned int i;
    ...
    {
    let mut index: c_uint = vcpu.arch.mmio_cache.index;
...
//
    ...
    struct mmio_hpte_cache_entry *cache_entry = core::ptr::null_mut();
    let mut mmio_update: c_long = 0;
...
// Check storage key, if applicable
    ...
    let mut perm: c_uint = hpte_get_skey_perm(gr, vcpu.arch.amr);
