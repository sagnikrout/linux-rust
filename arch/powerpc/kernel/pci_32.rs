...
//! Source: arch/powerpc/kernel/pci_32.c
#![no_std]
...
//
    ...
    let mut isa_io_base: c_ulong = 0;
    let mut pci_dram_offset: c_ulong = 0;
    let mut pcibios_assign_bus_offset: c_int = 1;
...
// For each hose, we begin searching bridges
    ...
    let mut node: *mut device_node = hose.dn;
    if (!node)
    continue;
    make_one_node_map(node, hose.first_busno);
    }
    dn = of_find_node_by_path("/");
    map_prop = of_find_property(dn, "pci-OF-bus-map", core::ptr::null_mut());
    if (map_prop) {
...
//
#[no_mangle]
pub unsafe extern "C" fn pci_device_from_OF_node(node: *mut device_node, bus: *mut u8, devfn: *mut u8) -> c_int {
...
//
    ...
    let mut result: c_long = -EOPNOTSUPP;
