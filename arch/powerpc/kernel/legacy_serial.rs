...
//! Source: arch/powerpc/kernel/legacy_serial.c
#![no_std]
...
pub const MAX_LEGACY_SERIAL_PORTS: c_int = 8;
    ...
    let mut legacy_serial_console: static int = -1;
    static const upf_t legacy_port_flags = UPF_BOOT_AUTOCONF | UPF_SKIP_TEST |
    UPF_SHARE_IRQ | UPF_FIXED_PORT;
#[no_mangle]
unsafe extern "C" fn tsi_serial_in(p: *mut uart_port, offset: c_uint) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn tsi_serial_out(p: *mut uart_port, offset: c_uint, value: u32) {
...
// get clock freq. if present
    clk = of_get_property(np, "clock-frequency", core::ptr::null_mut());
    if (clk && *clk)
    clock = be32_to_cpup(clk);
// get default speed if present
    spd = of_get_property(np, "current-speed", core::ptr::null_mut());
// get register shift if present
    rs = of_get_property(np, "reg-shift", core::ptr::null_mut());
    if (rs && *rs)
...
// Check if there is a port who already claimed our slot
    if (legacy_info.np != core::ptr::null_mut()) {
...
// if rtas uses this device, don't try to use it as well
    ...
    return -1;
// Get the address
    addrp = of_get_address(soc_dev, 0, core::ptr::null_mut(), core::ptr::null_mut());
...
// if any...
//
    typep = of_get_property(np, "ibm,aix-loc", core::ptr::null_mut());
// If we have a location index, then use it
    if (typep && *typep == 'S')
    index = simple_strtol(typep+1, core::ptr::null_mut(), 0) - 1;
...
// Get the PCI address. Assume BAR 0
    addrp = of_get_pci_address(pci_dev, 0, core::ptr::null_mut(), &flags);
    if (addrp == core::ptr::null_mut())
    return -1;
...
//
    if (np != pci_dev) {
    const __be32 *reg = of_get_property(np, "reg", core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn setup_legacy_serial_console(console: c_int) -> void __init {
...
// Check if a translated MMIO address has been found
    ...
    if (info.early_addr == core::ptr::null_mut())
    return;
...
#[no_mangle]
unsafe extern "C" fn ioremap_legacy_serial_console() -> int __init {
    ...
    info.early_addr = core::ptr::null_mut();
    return 0;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn find_legacy_serial_ports() -> void __init {
    void __init find_legacy_serial_ports(void)
    {
    struct device_node *np, *stdout = core::ptr::null_mut();
    const char *path;
    int index;
...
// Now find out if one of these is out firmware console
    ...
    path = of_get_property(of_chosen, "stdout-path", core::ptr::null_mut());
    if (path != core::ptr::null_mut()) {
...
// Iterate over all the 16550 ports, looking for known parents
    ...
    if (!parent)
    continue;
    if (of_match_node(legacy_serial_parents, parent) != core::ptr::null_mut()) {
...
// Next, try to locate PCI ports
    for (np = core::ptr::null_mut(); (np = of_find_all_nodes(np));) {
...
//
#[no_mangle]
unsafe extern "C" fn serial_dev_init() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn check_legacy_serial_console() -> int __init {
...
// The user has requested a console so this is already set up.
    ...
    if (!of_chosen) {
    DBG(" of_chosen is core::ptr::null_mut() !\n");
    return -ENODEV;
    }
    if (legacy_serial_console < 0) {
...
// ... So use the full path instead
    ...
    name = of_get_property(of_chosen, "stdout-path", core::ptr::null_mut());
    if (name == core::ptr::null_mut()) {
    DBG(" no stdout-path !\n");
    return -ENODEV;
    }
    ...
    DBG("stdout is %pOF\n", prom_stdout);
    name = of_get_property(prom_stdout, "name", core::ptr::null_mut());
    if (!name) {
    DBG(" stdout package has no name !\n");
    goto not_found;
    }
    spd = of_get_property(prom_stdout, "current-speed", core::ptr::null_mut());
    if (spd)
...
// Look for it in probed array
    ...
    return add_preferred_console("ttyS", offset, core::ptr::null_mut());
    not_found:
