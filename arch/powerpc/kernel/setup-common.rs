...
//
    ...
    let mut boot_cpuid: c_int = -1;
    EXPORT_SYMBOL_GPL(boot_cpuid);
    let mut boot_core_hwid: int __initdata = -1;

    let mut boot_cpu_hwid: c_int = -1;
...
#[no_mangle]
unsafe extern "C" fn machine_hang() {
...
#[no_mangle]
pub unsafe extern "C" fn arch_get_random_seed_longs(v: *mut c_ulong, max_longs: usize) -> size_t __must_check {
...
#[no_mangle]
unsafe extern "C" fn show_cpuinfo_summary(m: *mut seq_file) {
    ...
    of_node_put(root);
    if (ppc_md.show_cpuinfo != core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn check_for_initrd() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn cpu_init_thread_core_maps(tpc: c_int) -> void __init {
...
//
    ...
    u32 *cpu_to_phys_id = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn smp_setup_cpu_maps() -> void __init {
    void __init smp_setup_cpu_maps(void)
    {
    struct device_node *dn;
    let mut cpu: c_int = 0;
    let mut nthreads: c_int = 1;
...
// XXX: what is this? uninitialized??
    ...
    let mut avail: bool = of_device_is_available(dn);
    if (!avail)
...
//
    ...
    ireg = of_get_property(dn, "ibm,lrdr-capacity", core::ptr::null_mut());
    if (!ireg)
    goto out;
...
#[no_mangle]
unsafe extern "C" fn add_pcspkr() -> __init int {
    static __init int add_pcspkr(void)
    {
    struct device_node *np;
    struct platform_device *pd;
    int ret;
    np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "pnpPNP,100");
    of_node_put(np);
    if (!np)
    return -ENODEV;
...
#[no_mangle]
unsafe extern "C" fn probe_machine() -> __init void {
...
#[no_mangle]
pub unsafe extern "C" fn check_legacy_ioport(base_port: c_ulong) -> c_int {
    int check_legacy_ioport(unsigned long base_port)
    {
    struct device_node *parent, *np = core::ptr::null_mut();
    let mut ret: c_int = -ENODEV;
    switch(base_port) {
    case I8042_DATA_REG:
    if (!(np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "pnpPNP,303")))
    np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "pnpPNP,f03");
    if (np) {
    ...
    np = of_find_node_by_type(core::ptr::null_mut(), "8042");
// Pegasos has no device_type on its 8042 node, look for the
// name instead
    if (!np)
    np = of_find_node_by_name(core::ptr::null_mut(), "8042");
    if (np) {
    of_i8042_kbd_irq = 1;
    of_i8042_aux_irq = 12;
    }
    break;
    case FDC_BASE: /* FDC1 */
    np = of_find_node_by_type(core::ptr::null_mut(), "fdc");
    break;
    default:
...
//
    crash_fadump(core::ptr::null_mut(), ptr);
    return NOTIFY_DONE;
    }
...
#[no_mangle]
pub unsafe extern "C" fn setup_panic() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn check_cache_coherency() -> int __init {
    ...
    prop = of_get_property(np, "coherency-off", core::ptr::null_mut());
    of_node_put(np);
...
#[no_mangle]
pub unsafe extern "C" fn ppc_printk_progress(s: *mut c_char, hex: c_ushort) {
    void ppc_printk_progress(char *s, unsigned short hex)
    {
    pr_info("%s\n", s);
    }
#[no_mangle]
unsafe extern "C" fn print_system_info() -> __init void {
...
#[no_mangle]
unsafe extern "C" fn smp_setup_pacas() -> void __init {
    ...
    cpu_to_phys_id = core::ptr::null_mut();
    }
