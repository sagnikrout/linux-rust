...
//! Source: arch/powerpc/kernel/prom.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn early_parse_mem(p: *mut c_char) -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn overlaps_initrd(start: c_ulong, size: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn move_device_tree() -> void __init {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ibm_feature {
    pub /: *mut *mut unsigned long cpu_features; / CPU_FTR_xxx bit,
    pub /: *mut *mut unsigned long mmu_features; / MMU_FTR_xxx bit,
    pub /: *mut *mut unsigned int cpu_user_ftrs; / PPC_FEATURE_xxx bit,
    pub /: *mut *mut unsigned int cpu_user_ftrs2; / PPC_FEATURE2_xxx bit,
    pub /: *mut *mut unsigned char pabyte; / byte number in ibm,pa/pi-features,
    pub /: *mut *mut unsigned char pabit; / bit number (big-endian),
    pub /: *mut *mut unsigned char clear; / if 1, pa bit set => clear feature,
}
...
// loop over bits we know about
    ...
    if (pa_ftrs == core::ptr::null_mut())
    return;
    scan_features(node, pa_ftrs, tablelen, fp, size);
    }

#[no_mangle]
unsafe extern "C" fn init_mmu_slb_size(node: c_ulong) -> void __init {
    static void __init init_mmu_slb_size(unsigned long node)
    {
    const __be32 *slb_size_ptr;
    slb_size_ptr = of_get_flat_dt_prop(node, "slb-size", core::ptr::null_mut()) ? :
    of_get_flat_dt_prop(node, "ibm,slb-size", core::ptr::null_mut());
    if (slb_size_ptr)
...
#[no_mangle]
unsafe extern "C" fn identical_pvr_fixup(node: c_ulong) -> __init void {
    static __init void identical_pvr_fixup(unsigned long node)
    {
    unsigned int pvr;
    const char *model = of_get_flat_dt_prop(node, "model", core::ptr::null_mut());
//
...
#[no_mangle]
unsafe extern "C" fn check_cpu_feature_properties(node: c_ulong) -> void __init {
    ...
    prop = of_get_flat_dt_prop(node, fp.name, core::ptr::null_mut());
    ...
    const __be32 *cpu_version = core::ptr::null_mut();
    const __be32 *prop;
    const __be32 *intserv;
    int i, nthreads;
    int len;
    let mut found: c_int = -1;
    let mut found_thread: c_int = 0;
...
//
    ...
    prop = of_get_flat_dt_prop(node, "cpu-version", core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !dt_cpu_ftrs_in_use()) -> else {
...
// check if iommu is forced on or off
    if (of_get_flat_dt_prop(node, "linux,iommu-off", core::ptr::null_mut()) != core::ptr::null_mut())
    iommu_is_off = 1;
    if (of_get_flat_dt_prop(node, "linux,iommu-force-on", core::ptr::null_mut()) != core::ptr::null_mut())
    iommu_force_on = 1;

// mem=x on the command line is the preferred mechanism
    lprop = of_get_flat_dt_prop(node, "linux,memory-limit", core::ptr::null_mut());
    if (lprop)
    memory_limit = *lprop;

    lprop = of_get_flat_dt_prop(node, "linux,tce-alloc-start", core::ptr::null_mut());
    if (lprop)
    tce_alloc_start = *lprop;
    lprop = of_get_flat_dt_prop(node, "linux,tce-alloc-end", core::ptr::null_mut());
    if (lprop)
    tce_alloc_end = *lprop;

    lprop = of_get_flat_dt_prop(node, "linux,crashkernel-base", core::ptr::null_mut());
    if (lprop)
    crashk_res.start = *lprop;
    lprop = of_get_flat_dt_prop(node, "linux,crashkernel-size", core::ptr::null_mut());
    if (lprop)
...
#[no_mangle]
unsafe extern "C" fn validate_mem_limit(base: u64, size: *mut u64) -> bool __init {
    static bool __init validate_mem_limit(u64 base, u64 *size)
    {
    let mut max_mem: u64 = 1UL << (MAX_PHYSMEM_BITS);
    if (base >= max_mem)
    return false;
...
//
    ...
    {
    u64 base, size;
    let mut is_kexec_kdump: c_int = 0, rngs;
    base = lmb.base_addr;
    size = drmem_lmb_size();
    rngs = 1;
//
...
#[no_mangle]
unsafe extern "C" fn early_init_dt_scan_memory_ppc() -> int __init {
    static int __init early_init_dt_scan_memory_ppc(void)
    {

    const void *fdt = initial_boot_params;
    let mut node: c_int = fdt_path_offset(fdt, "/ibm,dynamic-reconfiguration-memory");
    if (node > 0)
    walk_drmem_lmbs_early(node, core::ptr::null_mut(), early_init_drmem_lmb);
...
//
    ...
    let mut add_mem_to_memblock: static int = 1;

pub const add_mem_to_memblock: c_int = 1;

#[no_mangle]
pub unsafe extern "C" fn early_init_dt_add_memory_arch(base: u64, size: u64) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn early_reserve_mem_dt() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn early_reserve_mem() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn parse_ppc_tm(str: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tm_init() -> void __init {
    ...
    if (depth != 0)
    return 0;
    prop = of_get_flat_dt_prop(node, "model", core::ptr::null_mut());
    if (prop)
...
#[no_mangle]
unsafe extern "C" fn save_fscr_to_task() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn early_init_devtree(params: *mut c_void) -> void __init {
...
// Too early to BUG_ON(), do it by hand
    ...
    of_scan_flat_dt(early_init_dt_scan_model, core::ptr::null_mut());

// Some machines might need RTAS info for debugging, grab it now.
    of_scan_flat_dt(early_init_dt_scan_rtas, core::ptr::null_mut());

// Some machines might need OPAL info for debugging, grab it now.
    of_scan_flat_dt(early_init_dt_scan_opal, core::ptr::null_mut());
// Scan tree for ultravisor feature
    of_scan_flat_dt(early_init_dt_scan_ultravisor, core::ptr::null_mut());

// scan tree to see if dump is active during last boot
    of_scan_flat_dt(early_init_dt_scan_fw_dump, core::ptr::null_mut());
...
//
    of_scan_flat_dt(early_init_dt_scan_cpus, core::ptr::null_mut());
    if (boot_cpuid < 0) {
...
// Scan and build the list of machine check recoverable ranges
    of_scan_flat_dt(early_init_dt_scan_recoverable_ranges, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn early_get_first_memblock_info(params: *mut c_void, size: *mut phys_addr_t) -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_get_ibm_chip_id(np: *mut device_node) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn cpu_to_chip_id(cpu: c_int) -> c_int {
    int cpu_to_chip_id(int cpu)
    {
    struct device_node *np;
    let mut ret: c_int = -1, idx;
...
//
    if (cpu_to_phys_id != core::ptr::null_mut())
