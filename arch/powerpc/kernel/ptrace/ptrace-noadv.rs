...
//! Source: arch/powerpc/kernel/ptrace/ptrace-noadv.c
#![no_std]
...
// Create a new breakpoint request if one doesn't exist already
    ...
    ptrace_triggered, core::ptr::null_mut(), task);
    if (IS_ERR(bp)) {
    thread.ptrace_bps[0] = core::ptr::null_mut();
    return PTR_ERR(bp);
    }
...
#[no_mangle]
unsafe extern "C" fn find_empty_ptrace_bp(thread: *mut thread_struct) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn find_empty_hw_brk(thread: *mut thread_struct) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(PPC_BREAKPOINT_MODE_EXACT: bp_info->addr_mode ==) -> else {
...
// Create a new breakpoint request if one doesn't exist already
    ...
    bp = register_user_hw_breakpoint(&attr, ptrace_triggered, core::ptr::null_mut(), child);
    thread.ptrace_bps[i] = bp;
    if (IS_ERR(bp)) {
    thread.ptrace_bps[i] = core::ptr::null_mut();
    return PTR_ERR(bp);
    }
    return i + 1;
...
pub unsafe extern "C" fn ppc_del_hwdebug(child: *mut task_struct, data: c_long) -> c_long {
    ...
    if (bp) {
    unregister_hw_breakpoint(bp);
    thread.ptrace_bps[data - 1] = core::ptr::null_mut();
    } else {
    ret = -ENOENT;
    }
    return ret;
