...
//! Source: arch/powerpc/kernel/nvram_64.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nvram_partition {
    pub partition: list_head,
    pub header: nvram_header,
    pub index: c_uint,
}
...
//
#[no_mangle]
pub unsafe extern "C" fn nvram_init_os_partition(part: *mut nvram_os_partition) -> int __init {
...
// Found one but too small, remove it
    ...
    " removing it...\n", part.name);
    nvram_remove_partition(part.name, NVRAM_SIG_OS, core::ptr::null_mut());
    p = 0;
    }
// Create one if we didn't find
    ...
    part.name);
    nvram_remove_partition(core::ptr::null_mut(), NVRAM_SIG_OS,
    nvram_os_partitions);
    p = nvram_create_partition(part.name, NVRAM_SIG_OS,
    part.req_size, part.min_size);
    }
...
#[no_mangle]
unsafe extern "C" fn zip_oops(text_len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvram_pstore_open(psi: *mut pstore_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn nvram_pstore_write(record: *mut pstore_record) -> c_int {
    static int nvram_pstore_write(struct pstore_record *record)
    {
    int rc;
    let mut err_type: c_uint = ERR_TYPE_KERNEL_PANIC;
    struct oops_log_info *oops_hdr = (struct oops_log_info *) oops_buf;
...
//
#[no_mangle]
unsafe extern "C" fn nvram_pstore_read(record: *mut pstore_record) -> isize {
    static ssize_t nvram_pstore_read(struct pstore_record *record)
    {
    struct oops_log_info *oops_hdr;
    unsigned int err_type, id_no, size = 0;
    struct nvram_os_partition *part = core::ptr::null_mut();
    char *buff = core::ptr::null_mut();
    let mut sig: c_int = 0;
    loff_t p;
    read_type++;
...
#[no_mangle]
unsafe extern "C" fn nvram_pstore_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn nvram_init_oops_partition(rtas_partition_exists: c_int) -> void __init {
...
//
    ...
    oops_log_partition.name);
    kfree(big_oops_buf);
    big_oops_buf = core::ptr::null_mut();
    }
    } else {
...
//
    ...
    {
    struct oops_log_info *oops_hdr = (struct oops_log_info *)oops_buf;
    let mut oops_count: static unsigned int = 0;
    static struct kmsg_dump_iter iter;
    let mut panicking: static bool = false;
    static DEFINE_SPINLOCK(lock);
    unsigned long flags;
    size_t text_len;
    let mut err_type: c_uint = ERR_TYPE_KERNEL_PANIC_GZ;
    let mut rc: c_int = -1;
...
#[no_mangle]
unsafe extern "C" fn nvram_print_partitions(label: *mut *mut c_char) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn nvram_write_header(part: *mut *mut nvram_partition) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nvram_checksum(p: *mut nvram_header) -> unsigned char __init {
...
//
    ...
    struct nvram_partition *free_part = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn nvram_get_partition_size(data_index: loff_t) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn nvram_find_partition(name: *const c_char, sig: c_int, out_size: *mut c_int) -> loff_t {
...
#[no_mangle]
pub unsafe extern "C" fn nvram_scan_partitions() -> int __init {
    int __init nvram_scan_partitions(void)
    {
    let mut cur_index: loff_t = 0;
    struct nvram_header phead;
    struct nvram_partition * tmp_part;
    unsigned char c_sum;
    char * header;
    int total_size;
    int err;
    if (ppc_md.nvram_size == core::ptr::null_mut() || ppc_md.nvram_size() <= 0)
    return -ENODEV;
