...
//! Source: arch/powerpc/platforms/powernv/idle.c
#![no_std]
...
pub const MAX_STOP_STATE: c_uint = 0xF;
pub const P9_STOP_SPR_MSR: c_int = 2000;
pub const P9_STOP_SPR_PSSCR: c_int = 855;
...
//
    let mut pnv_first_tb_loss_level: static u64 = MAX_STOP_STATE + 1;
    let mut deep_spr_loss_state: static u64 = MAX_STOP_STATE + 1;
//
...
#[no_mangle]
unsafe extern "C" fn pnv_save_sprs_for_deep_states() -> int __init {
...
//
    let mut lpcr_val: u64 = mfspr(SPRN_LPCR);
    let mut hid0_val: u64 = mfspr(SPRN_HID0);
    let mut hmeer_val: u64 = mfspr(SPRN_HMEER);
    let mut msr_val: u64 = MSR_IDLE;
    let mut psscr_val: u64 = pnv_deepest_stop_psscr_val;
    for_each_present_cpu(cpu) {
    let mut pir: u64 = get_hard_smp_processor_id(cpu);
    let mut hsprg0_val: u64 = (uint64_t)paca_ptrs[cpu];
...
// Only p8 needs to set extra HID registers
    if (!cpu_has_feature(CPU_FTR_ARCH_300)) {
    let mut hid1_val: u64 = mfspr(SPRN_HID1);
    let mut hid4_val: u64 = mfspr(SPRN_HID4);
    let mut hid5_val: u64 = mfspr(SPRN_HID5);
...
#[no_mangle]
pub unsafe extern "C" fn pnv_get_supported_cpuidle_states() -> u32 {
...
#[no_mangle]
unsafe extern "C" fn pnv_fastsleep_workaround_apply(info: *mut c_void) {
...
// err = 1;
    ...
    let mut power7_fastsleep_workaround_entry: static bool = true;
    let mut power7_fastsleep_workaround_exit: static bool = true;
//
...
#[no_mangle]
pub unsafe extern "C" fn atomic_start_thread_idle() {
    ...
    let mut thread_nr: c_int = cpu_thread_in_core(cpu);
    unsigned long *state = &paca_ptrs[first].idle_state;
    clear_bit(thread_nr, state);
    }
#[no_mangle]
pub unsafe extern "C" fn atomic_stop_thread_idle() {
...
#[no_mangle]
pub unsafe extern "C" fn atomic_lock_thread_idle() {
...
#[no_mangle]
pub unsafe extern "C" fn atomic_unlock_and_stop_thread_idle() {
    static inline void atomic_unlock_and_stop_thread_idle(void)
    {
    let mut cpu: c_int = raw_smp_processor_id();
    let mut first: c_int = cpu_first_thread_sibling(cpu);
    let mut thread: c_ulong = 1UL << cpu_thread_in_core(cpu);
    unsigned long *state = &paca_ptrs[first].idle_state;
    unsigned long *lock = &paca_ptrs[first].idle_lock;
    let mut s: u64 = READ_ONCE(*state);
    u64 new, tmp;
...
#[no_mangle]
pub unsafe extern "C" fn atomic_unlock_thread_idle() {
...
#[no_mangle]
unsafe extern "C" fn power7_idle_insn(type: c_ulong) -> c_ulong {
    ...
    unsigned long *state = &paca_ptrs[first].idle_state;
    let mut thread: c_ulong = 1UL << cpu_thread_in_core(cpu);
    let mut core_thread_mask: c_ulong = (1UL << threads_per_core) - 1;
    unsigned long srr1;
    bool full_winkle;
    struct p7_sprs sprs = {}; /* avoid false use-uninitialised */
    let mut sprs_saved: bool = false;
    int rc;
...
//
    ...
    hmi_exception_realmode(core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn power7_offline() -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn power7_idle_type(type: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn power7_idle() {
...
#[no_mangle]
unsafe extern "C" fn power9_idle_stop(psscr: c_ulong) -> c_ulong {
    ...
    unsigned long srr1;
    unsigned long pls;
    let mut mmcr0: c_ulong = 0;
    let mut mmcra: c_ulong = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn pnv_power9_force_smt4_catch() {
    void pnv_power9_force_smt4_catch(void)
    {
    int cpu, cpu0, thr;
    int awake_threads = 1;		/* this thread is awake */
    let mut poke_threads: c_int = 0;
    let mut need_awake: c_int = threads_per_core;
...
#[no_mangle]
pub unsafe extern "C" fn pnv_power9_force_smt4_release() {
...
#[no_mangle]
unsafe extern "C" fn power10_idle_stop(psscr: c_ulong) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn arch300_offline_stop(psscr: c_ulong) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn arch300_idle() {
...
#[no_mangle]
pub unsafe extern "C" fn pnv_program_cpu_hotplug_lpcr(cpu: c_uint, lpcr_val: u64) {
...
//
#[no_mangle]
pub unsafe extern "C" fn pnv_cpu_offline(cpu: c_uint) -> c_ulong {
...
//
#[no_mangle]
pub unsafe extern "C" fn validate_psscr_val_mask(psscr_val: *mut u64, psscr_mask: *mut u64, flags: u32) -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn pnv_arch300_idle_init() -> void __init {
    static void __init pnv_arch300_idle_init(void)
    {
    let mut max_residency_ns: u64 = 0;
    int i;
...
//
    ...
    let mut psscr_rl: u64 = state.psscr_val & PSSCR_RL_MASK;
...
#[no_mangle]
unsafe extern "C" fn pnv_disable_deep_states() -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn pnv_probe_idle_states() -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn pnv_parse_cpuidle_dt() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn pnv_init_idle_states() -> int __init {
