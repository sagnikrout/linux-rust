...
//! Source: arch/powerpc/platforms/powernv/pci-ioda-tce.c
#![no_std]
...
// Add 16M for POWER8 by default
    ...
    {
    struct page *tce_mem = core::ptr::null_mut();
    __be64 *addr;
    ...
    let mut level: c_int = tbl.it_indirect_levels;
    let mut shift: c_long = ilog2(tbl.it_level_size);
    let mut mask: c_ulong = (tbl.it_level_size - 1) << (level * shift);
    while (level) {
    let mut n: c_int = (idx & mask) >> (level * shift);
    ...
    let mut rpn: u64 = __pa(uaddr) >> tbl.it_page_shift;
    long i;
    ...
    let mut idx: c_ulong = index - tbl.it_offset + i;
// (pnv_tce(tbl, false, idx, true)) = cpu_to_be64(newtce);
    ...
    {
    let mut proto_tce: u64 = iommu_direction_to_tce_perm(*direction);
    let mut newtce: c_ulong = *hpa | proto_tce, oldtce;
    let mut idx: c_ulong = index - tbl.it_offset;
    __be64 *ptce = core::ptr::null_mut();
...
pub unsafe extern "C" fn pnv_tce_get(tbl: *mut iommu_table, index: c_long) -> c_ulong {
    ...
    u64 *tmp = (u64 *) addr_ul;
    for (i = 0; i < size; ++i) {
    let mut hpa: c_ulong = be64_to_cpu(tmp[i]);
...
pub unsafe extern "C" fn pnv_pci_ioda2_table_free_pages(tbl: *mut iommu_table) {
    ...
    {
    __be64 *addr, *tmp;
    let mut allocated: c_ulong = 1UL << shift;
    let mut entries: c_uint = 1UL << (shift - 3);
    long i;
...
// current_offset += allocated;
    ...
    {
    void *addr, *uas = core::ptr::null_mut();
    let mut offset: c_ulong = 0, level_shift, total_allocated = 0;
    let mut total_allocated_uas: c_ulong = 0;
    let mut window_shift: c_uint = ilog2(window_size);
    let mut entries_shift: c_uint = window_shift - page_shift;
    unsigned int table_shift = max_t(unsigned int, entries_shift + 3,
    PAGE_SHIFT);
    let mut tce_table_size: c_ulong = 1UL << table_shift;
...
// Clean a pointer to iommu_table in iommu_table_group::tables[]
    ...
    table_group.tables[i] = core::ptr::null_mut();
    found = true;
    break;
    }
    }
    ...
    struct iommu_table_group *table_group)
    {
    struct iommu_table_group_link *tgl = core::ptr::null_mut();
