...
//! Source: arch/powerpc/platforms/powernv/opal-core.c
#![no_std]
...
pub const MAX_PT_LOAD_CNT: c_int = 8;
// NT_AUXV note related info
pub const AUXV_CNT: c_int = 1;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct opalcore_config {
    pub num_cpus: u32,
// PIR value of crashing CPU
    pub crashing_cpu: u32,
// CPU state data info from F/W
    pub cpu_state_destination_vaddr: u64,
    pub cpu_state_data_size: u64,
    pub cpu_state_entry_size: u64,
// OPAL memory to be exported as PT_LOAD segments
    pub ptload_addr: [u64; MAX_PT_LOAD_CNT],
    pub ptload_size: [u64; MAX_PT_LOAD_CNT],
    pub ptload_cnt: u64,
// Pointer to the first PT_LOAD in the ELF core file
    pub ptload_phdr: *mut Elf64_Phdr,
// Total size of opalcore file.
    pub opalcore_size: usize,
// Buffer for all the ELF core headers and the PT_NOTE
    pub opalcorebuf_sz: usize,
    pub opalcorebuf: *mut c_char,
// NT_AUXV buffer
    pub auxv_buf: [c_char; AUXV_DESC_SZ],
}
...
#[no_mangle]
unsafe extern "C" fn get_new_element() -> *mut opalcore  __init {
    static struct opalcore * __init get_new_element(void)
    {
    return kzalloc_obj(struct opalcore);
    }
#[no_mangle]
pub unsafe extern "C" fn is_opalcore_usable() -> c_int {
    static inline int is_opalcore_usable(void)
    {
    return (oc_conf && oc_conf.opalcorebuf != core::ptr::null_mut()) ? 1 : 0;
    }
    ...
    let mut namesz: Elf64_Word = strlen(name) + 1;
...
//
#[no_mangle]
unsafe extern "C" fn opalcore_append_cpu_notes(buf: *mut Elf64_Word) -> *mut Elf64_Word  __init {
...
#[no_mangle]
unsafe extern "C" fn create_opalcore() -> int __init {
...
// Read OPAL related device-tree entries
    dn = of_find_node_by_name(core::ptr::null_mut(), "ibm,opal");
    if (dn) {
...
// free the buffer used for setting up OPAL core
    ...
    oc_conf.opalcorebuf = core::ptr::null_mut();
    oc_conf.opalcorebuf_sz = 0;
    }
    kfree(oc_conf);
    oc_conf = core::ptr::null_mut();
    }
    __exitcall(opalcore_cleanup);
#[no_mangle]
unsafe extern "C" fn opalcore_config_init() -> void __init {
...
// Check if dump has been initiated on last reboot
    prop = of_get_property(np, "mpipl-boot", core::ptr::null_mut());
    if (!prop) {
    of_node_put(np);
    return;
    }
...
//
    ...
    let mut opalcore_rel_attr: static struct kobj_attribute = __ATTR_WO(release_core);
...
#[no_mangle]
unsafe extern "C" fn opalcore_init() -> int __init {
