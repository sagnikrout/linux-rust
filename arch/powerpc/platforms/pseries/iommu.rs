...
//! Source: arch/powerpc/platforms/pseries/iommu.c
#![no_std]
...
//
    ...
    __be64 *tcep;
    u64 rpn;
    let mut tceshift: c_ulong = tbl.it_page_shift;
    let mut pagesize: c_ulong = IOMMU_PAGE_SIZE(tbl);
...
#[no_mangle]
unsafe extern "C" fn tce_clear_pSeries(tbl: *mut iommu_table, index: c_long, npages: c_long) {
...
#[no_mangle]
unsafe extern "C" fn tce_get_pseries(tbl: *mut iommu_table, index: c_long) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn pseries_tce_iommu_userspace_view_alloc(tbl: *mut iommu_table) -> c_long {
    static long pseries_tce_iommu_userspace_view_alloc(struct iommu_table *tbl)
    {
    let mut cb: c_ulong = ALIGN(sizeof(tbl.it_userspace[0]) * tbl.it_size, PAGE_SIZE);
    unsigned long *uas;
...
#[no_mangle]
unsafe extern "C" fn tce_iommu_userspace_view_free(tbl: *mut iommu_table) {
    static void tce_iommu_userspace_view_free(struct iommu_table *tbl)
    {
    vfree(tbl.it_userspace);
    tbl.it_userspace = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn tce_free_pSeries(tbl: *mut iommu_table) {
    ...
    u64 rpn;
    let mut ret: c_int = 0;
    let mut tcenum_start: c_long = tcenum, npages_start = npages;
    rpn = __pa(uaddr) >> tceshift;
...
#[no_mangle]
unsafe extern "C" fn tce_freemulti_pSeriesLP(tbl: *mut iommu_table, tcenum: c_long, npages: c_long) {
    static void tce_freemulti_pSeriesLP(struct iommu_table *tbl, long tcenum, long npages)
    {
    u64 rc;
    let mut rpages: c_long = npages;
    unsigned long limit;
...
#[no_mangle]
unsafe extern "C" fn tce_get_pSeriesLP(tbl: *mut iommu_table, tcenum: c_long) -> c_ulong {
...
pub struct dynamic_dma_window_prop {
    pub /: *mut *mut __be32 liobn; / tce table number,
    pub /: *mut *mut __be64 dma_base; / address hi,lo,
    pub /: *mut *mut __be32 tce_shift; / ilog2(tce_page_size),
    pub /: *mut *mut __be32 window_shift; / ilog2(tce_window_size),
}
...
//
    ...
    let mut tceshift: c_ulong = be32_to_cpu(maprange.tce_shift);
    unsigned long dmastart = (start_pfn << PAGE_SHIFT) +
    be64_to_cpu(maprange.dma_base);
    let mut tcenum: c_ulong = dmastart >> tceshift;
    let mut npages: c_ulong = num_pfn << PAGE_SHIFT >> tceshift;
...
#[no_mangle]
unsafe extern "C" fn pci_dma_bus_setup_pSeries(bus: *mut pci_bus) {
...
// this PHB.
//
    isa_dn = isa_dn_orig = of_find_node_by_type(core::ptr::null_mut(), "isa");
...
// Divide the rest (1.75GB) among the children
    ...
    {
    long rc;
    let mut ioba: c_ulong = (unsigned long) index << tbl.it_page_shift;
    unsigned long flags, oldtce = 0;
    let mut proto_tce: u64 = iommu_direction_to_tce_perm(*direction);
    let mut newtce: c_ulong = *tce | proto_tce;
...
// tce = oldtce & ~(TCE_PCI_READ | TCE_PCI_WRITE);
    ...
    {
    return tbl.it_userspace ? &tbl.it_userspace[index - tbl.it_offset] : core::ptr::null_mut();
    }
...
//
    ...
    {
    const __be32 *default_prop = core::ptr::null_mut();
    const __be32 *ddw_prop = core::ptr::null_mut();
    struct device_node *rdn = core::ptr::null_mut();
    let mut default_win: bool = false, ddw_win = false;
    for ( ; dn && PCI_DN(dn); dn = dn.parent) {
    default_prop = of_get_property(dn, "ibm,dma-window", core::ptr::null_mut());
    if (default_prop) {
    rdn = dn;
    default_win = true;
    }
    ddw_prop = of_get_property(dn, DIRECT64_PROPNAME, core::ptr::null_mut());
    if (ddw_prop) {
    rdn = dn;
    ddw_win = true;
    break;
    }
    ddw_prop = of_get_property(dn, DMA64_PROPNAME, core::ptr::null_mut());
    if (ddw_prop) {
    rdn = dn;
    ddw_win = true;
    break;
...
#[no_mangle]
unsafe extern "C" fn pci_dma_bus_setup_pSeriesLP(bus: *mut pci_bus) {
...
//
    ...
    be32_to_cpu(prop.tce_shift), core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn pci_dma_dev_setup_pSeries(dev: *mut pci_dev) {
...
//
    while (dn && PCI_DN(dn) && PCI_DN(dn).table_group == core::ptr::null_mut())
    dn = dn.parent;
    if (dn && PCI_DN(dn))
...
#[no_mangle]
unsafe extern "C" fn disable_ddw_setup(str: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn clean_dma_window(np: *mut device_node, dwp: *mut dynamic_dma_window_prop) {
...
//
#[no_mangle]
unsafe extern "C" fn __remove_dma_window(np: *mut device_node, ddw_avail: *mut u32, liobn: u64) {
    static void __remove_dma_window(struct device_node *np, u32 *ddw_avail, u64 liobn)
    {
    int ret;
    ret = rtas_call(ddw_avail[DDW_REMOVE_PE_DMA_WIN], 1, 1, core::ptr::null_mut(), liobn);
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn copy_property(pdn: *mut device_node, from: *const c_char, to: *const c_char) {
    static void copy_property(struct device_node *pdn, const char *from, const char *to)
    {
    struct property *src, *dst;
    src = of_find_property(pdn, from, core::ptr::null_mut());
    if (!src)
    return;
    dst = kzalloc_obj(*dst);
    if (!dst)
    ...
    let mut ret: c_int = 0;
    win = of_find_property(np, win_name, core::ptr::null_mut());
    if (!win)
    return -EINVAL;
...
//
    ...
    !of_find_property(np, "ibm,dma-window-saved", core::ptr::null_mut()))
...
#[no_mangle]
unsafe extern "C" fn find_existing_ddw_windows_named(name: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn find_existing_ddw_windows() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ddw_memory_hotplug_max() -> phys_addr_t {
...
//
#[no_mangle]
unsafe extern "C" fn reset_dma_window(dev: *mut pci_dev, par_dn: *mut device_node) {
    ...
    buid = pdn.phb.buid;
    cfg_addr = (pdn.busno << 16) | (pdn.devfn << 8);
    ret = rtas_call(reset_dma_win, 3, 1, core::ptr::null_mut(), cfg_addr, BUID_HI(buid),
    BUID_LO(buid));
    if (ret)
    dev_info(&dev.dev,
...
//
#[no_mangle]
unsafe extern "C" fn limited_dma_window(dev: *mut pci_dev, par_dn: *mut device_node) -> c_int {
...
//
    ...
    buid = pdn.phb.buid;
    cfg_addr = (pdn.busno << 16) | (pdn.devfn << 8);
    ret = rtas_call(reset_dma_win, 4, 1, core::ptr::null_mut(), cfg_addr, BUID_HI(buid),
    BUID_LO(buid), 1);
    if (ret)
    dev_info(&dev.dev,
...
#[no_mangle]
unsafe extern "C" fn iommu_get_page_shift(query_page_size: u32) -> c_int {
...
// Supported IO page-sizes according to LoPAR, note that 2M is out of order
    ...
    let mut i: c_int = ARRAY_SIZE(shift) - 1;
    let mut ret: c_int = 0;
//
...
//
#[no_mangle]
unsafe extern "C" fn enable_ddw(dev: *mut pci_dev, pdn: *mut device_node, dma_mask: u64) -> bool {
    static bool enable_ddw(struct pci_dev *dev, struct device_node *pdn, u64 dma_mask)
    {
    let mut len: c_int = 0, ret;
    let mut max_ram_len: c_int = order_base_2(ddw_memory_hotplug_max());
    struct ddw_query_response query;
    ...
    let mut default_win_removed: bool = false, direct_mapping = false;
    let mut dynamic_mapping: bool = false;
    bool pmem_present;
    struct pci_dn *pci = PCI_DN(pdn);
    struct property *default_win = core::ptr::null_mut();
    let mut limited_addr_req: bool = false, limited_addr_enabled = false;
    int dev_max_ddw;
    int ddw_sz;
    dn = of_find_node_by_type(core::ptr::null_mut(), "ibm,pmemory");
    pmem_present = dn != core::ptr::null_mut();
    of_node_put(dn);
...
// DDW + IOMMU on single window may fail if there is any allocation
    ...
    default_win = of_find_property(pdn, "ibm,dma-window", core::ptr::null_mut());
    if (!default_win)
    goto out_failed;
    reset_win_ext = ddw_read_ext(pdn, DDW_EXT_RESET_DMA_WIN, core::ptr::null_mut());
    if (reset_win_ext)
    goto out_failed;
...
// Make sure to clean DDW if any TCE was set
    ...
    pci.table_group.tables[0] = core::ptr::null_mut();
    set_iommu_table_base(&dev.dev, core::ptr::null_mut());
    }
    }
    if (dynamic_mapping) {
    ...
    let mut mask: c_ulong = IORESOURCE_MEM_64 | IORESOURCE_MEM;
// Look for MMIO32
...
//
    ...
    dynamic_addr, dynamic_len, page_shift, core::ptr::null_mut(),
    &iommu_table_lpar_multi_ops);
...
// default_win is valid here because default_win_removed == true
    if (!of_find_property(pdn, "ibm,dma-window-saved", core::ptr::null_mut()))
...
#[no_mangle]
unsafe extern "C" fn query_page_size_to_mask(query_page_size: u32) -> __u64 {
...
//
    ...
    if (!of_find_property(pdn, "ibm,dma-window", core::ptr::null_mut()))
...
#[no_mangle]
unsafe extern "C" fn pci_dma_dev_setup_pSeriesLP(dev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn iommu_bypass_supported_pSeriesLP(pdev: *mut pci_dev, dma_mask: u64) -> bool {
...
//
    pdn = pci_dma_find(dn, core::ptr::null_mut());
    if (pdn && PCI_DN(pdn))
...
#[no_mangle]
unsafe extern "C" fn restore_default_dma_window(pdev: *mut pci_dev, pdn: *mut device_node) {
...
#[no_mangle]
unsafe extern "C" fn remove_dynamic_dma_windows(pdev: *mut pci_dev, pdn: *mut device_node) -> c_long {
    ...
    pci.table_group.tables[1] = core::ptr::null_mut();
...
//
    ...
    default_prop = of_get_property(pdn, "ibm,dma-window", core::ptr::null_mut());
    ...
    size, IOMMU_PAGE_SHIFT_4K, core::ptr::null_mut(),
    &iommu_table_lpar_multi_ops);
    iommu_init_table(tbl, pci.phb.node, 0, 0);
...
// Limit the default window size to window_size
    ...
    IOMMU_PAGE_SHIFT_4K, core::ptr::null_mut(),
    &iommu_table_lpar_multi_ops);
...
// New table for using DDW instead of the default DMA window
    ...
    1UL << len, page_shift, core::ptr::null_mut(), &iommu_table_lpar_multi_ops);
...
#[no_mangle]
unsafe extern "C" fn is_default_window_table(table_group: *mut iommu_table_group, tbl: *mut iommu_table) -> bool {
    ...
    {
    let mut tbl: return = = table_group.tables[num] ? 0 : -EPERM;
    }
#[no_mangle]
unsafe extern "C" fn spapr_tce_unset_window(table_group: *mut iommu_table_group, num: c_int) -> c_long {
...
// Dont clear the TCEs, User should have done it
    ...
    table_group.tables[num] = core::ptr::null_mut();
    ret = 0;
    goto out_unlock;
    out_failed:
    fpdn = kzalloc_obj(*fpdn);
...
#[no_mangle]
unsafe extern "C" fn spapr_tce_take_ownership(table_group: *mut iommu_table_group, dev: *mut device) -> c_long {
...
//
    ...
    table_group.tables[0] = core::ptr::null_mut();
    }
    set_iommu_table_base(dev, core::ptr::null_mut());
    mutex_unlock(&dma_win_init_mutex);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn spapr_tce_release_ownership(table_group: *mut iommu_table_group, dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn iommu_reconfig_notifier(nb: *mut notifier_block, action: c_ulong, data: *mut c_void) -> c_int {
    static int iommu_reconfig_notifier(struct notifier_block *nb, unsigned long action, void *data)
    {
    let mut err: c_int = NOTIFY_OK;
...
#[no_mangle]
pub unsafe extern "C" fn iommu_init_early_pSeries() -> void __init {
    void __init iommu_init_early_pSeries(void)
    {
    if (of_chosen && of_get_property(of_chosen, "linux,iommu-off", core::ptr::null_mut()))
    return;
    if (firmware_has_feature(FW_FEATURE_LPAR)) {
...
#[no_mangle]
unsafe extern "C" fn disable_multitce(str: *mut c_char) -> int __init {
