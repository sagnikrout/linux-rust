...
//! Source: arch/powerpc/platforms/pseries/lparcfg.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn cpu_get_purr(arg: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn get_purr() -> c_ulong {
    static unsigned long get_purr(void)
    {
    let mut purr: core::sync::atomic::AtomicI64 = ATOMIC64_INIT(0);
...
//
#[no_mangle]
unsafe extern "C" fn h_get_ppp(ppp_data: *mut hvcall_ppp_data) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn show_gpci_data(m: *mut seq_file) {
...
//
#[no_mangle]
unsafe extern "C" fn parse_ppp_data(m: *mut seq_file) {
...
//
#[no_mangle]
unsafe extern "C" fn parse_mpp_data(m: *mut seq_file) {
...
//
#[no_mangle]
unsafe extern "C" fn parse_mpp_x_data(m: *mut seq_file) {
...
//
#[no_mangle]
unsafe extern "C" fn read_rtas_lpar_name(m: *mut seq_file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn read_dt_lpar_name(m: *mut seq_file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn read_lpar_name(m: *mut seq_file) {
...
//
#[no_mangle]
unsafe extern "C" fn parse_system_parameter_string(m: *mut seq_file) {
...
//
#[no_mangle]
unsafe extern "C" fn lparcfg_count_active_processors() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pseries_cmo_data(m: *mut seq_file) {
    static void pseries_cmo_data(struct seq_file *m)
    {
    int cpu;
    let mut cmo_faults: c_ulong = 0;
    let mut cmo_fault_time: c_ulong = 0;
...
#[no_mangle]
unsafe extern "C" fn splpar_dispatch_data(m: *mut seq_file) {
    static void splpar_dispatch_data(struct seq_file *m)
    {
    int cpu;
    let mut dispatches: c_ulong = 0;
    let mut dispatch_dispersions: c_ulong = 0;
    for_each_possible_cpu(cpu) {
...
#[no_mangle]
unsafe extern "C" fn parse_em_data(m: *mut seq_file) {
...
#[no_mangle]
unsafe extern "C" fn maxmem_data(m: *mut seq_file) {
...
#[no_mangle]
unsafe extern "C" fn pseries_lparcfg_data(m: *mut seq_file, v: *mut c_void) -> c_int {
    static int pseries_lparcfg_data(struct seq_file *m, void *v)
    {
    int partition_potential_processors;
    int partition_active_processors;
    struct device_node *rtas_node;
    const __be32 *lrdrp = core::ptr::null_mut();
    rtas_node = of_find_node_by_path("/rtas");
    if (rtas_node)
    lrdrp = of_get_property(rtas_node, "ibm,lrdr-capacity", core::ptr::null_mut());
    if (lrdrp == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn update_ppp(entitlement: *mut u64, weight: *mut u8) -> isize {
...
//
#[no_mangle]
unsafe extern "C" fn update_mpp(entitlement: *mut u64, weight: *mut u8) -> isize {
...
// new_entitled_ptr = (u64) simple_strtoul(tmp, &endp, 10);
    if (endp == tmp)
    return -EINVAL;
    retval = update_ppp(new_entitled_ptr, core::ptr::null_mut());
...
// new_weight_ptr = (u8) simple_strtoul(tmp, &endp, 10);
    if (endp == tmp)
    return -EINVAL;
    retval = update_ppp(core::ptr::null_mut(), new_weight_ptr);
    } else if (!strcmp(kbuf, "entitled_memory")) {
    char *endp;
// new_entitled_ptr = (u64) simple_strtoul(tmp, &endp, 10);
    if (endp == tmp)
    return -EINVAL;
    retval = update_mpp(new_entitled_ptr, core::ptr::null_mut());
    } else if (!strcmp(kbuf, "entitled_memory_weight")) {
    char *endp;
// new_weight_ptr = (u8) simple_strtoul(tmp, &endp, 10);
    if (endp == tmp)
    return -EINVAL;
    retval = update_mpp(core::ptr::null_mut(), new_weight_ptr);
    } else
    return -EINVAL;
    if (retval == H_SUCCESS || retval == H_CONSTRAINED) {
    retval = count;
...
#[no_mangle]
unsafe extern "C" fn lparcfg_data(m: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    let mut lp_index: c_uint = 0;
    seq_printf(m, "%s %s\n", MODULE_NAME, MODULE_VERS);
    rootdn = of_find_node_by_path("/");
    if (rootdn) {
    tmp = of_get_property(rootdn, "model", core::ptr::null_mut());
    if (tmp)
    model = tmp;
    tmp = of_get_property(rootdn, "system-id", core::ptr::null_mut());
    if (tmp)
    system_id = tmp;
...
#[no_mangle]
unsafe extern "C" fn lparcfg_open(inode: *mut inode, file: *mut file) -> c_int {
    static int lparcfg_open(struct inode *inode, struct file *file)
    {
    return single_open(file, lparcfg_data, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn lparcfg_init() -> int __init {
...
// Allow writing if we have FW_FEATURE_SPLPAR
    if (firmware_has_feature(FW_FEATURE_SPLPAR))
    mode |= 0200;
    if (!proc_create("powerpc/lparcfg", mode, core::ptr::null_mut(), &lparcfg_proc_ops)) {
...
//
    retval = h_pic(&boot_pool_idle_time, core::ptr::null_mut());
    if (retval != H_SUCCESS)
