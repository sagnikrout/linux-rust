...
//! Source: arch/powerpc/platforms/pseries/vio.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vio_cmo_pool {
    pub size: usize,
    pub free: usize,
}

// How many ms to delay queued balance work
pub const VIO_CMO_BALANCE_DELAY: c_int = 100;
// Portion out IO memory to CMO devices by this chunk size
pub const VIO_CMO_BALANCE_CHUNK: c_int = 131072;
//
...
//
#[no_mangle]
unsafe extern "C" fn vio_cmo_num_OF_devs() -> c_int {
...
//
    node_vroot = of_find_node_by_name(core::ptr::null_mut(), "vdevice");
    if (node_vroot) {
...
//
#[no_mangle]
pub unsafe extern "C" fn vio_cmo_alloc(viodev: *mut vio_dev, size: usize) -> c_int {
    static inline int vio_cmo_alloc(struct vio_dev *viodev, size_t size)
    {
    unsigned long flags;
    let mut reserve_free: usize = 0;
    let mut excess_free: usize = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn vio_cmo_dealloc(viodev: *mut vio_dev, size: usize) {
    static inline void vio_cmo_dealloc(struct vio_dev *viodev, size_t size)
    {
    unsigned long flags;
    let mut spare_needed: usize = 0;
    let mut excess_freed: usize = 0;
    let mut reserve_freed: usize = size;
    size_t tmp;
    let mut balance: c_int = 0;
    spin_lock_irqsave(&vio_cmo.lock, flags);
    vio_cmo.curr -= size;
...
//
#[no_mangle]
pub unsafe extern "C" fn vio_cmo_entitlement_update(new_entitlement: usize) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vio_cmo_balance(work: *mut work_struct) {
    ...
    struct vio_cmo_dev_entry *dev_ent;
    unsigned long flags;
    let mut avail: usize = 0, level, chunk, need;
    let mut devcount: c_int = 0, fulfilled;
...
// Calculated used excess entitlement
    ...
    dev_to_node(dev));
    if (unlikely(ret == core::ptr::null_mut())) {
    ...
    let mut ret: dma_addr_t = DMA_MAPPING_ERROR;
...
//
#[no_mangle]
pub unsafe extern "C" fn vio_cmo_set_dev_desired(viodev: *mut vio_dev, desired: usize) {
...
//
#[no_mangle]
unsafe extern "C" fn vio_cmo_bus_probe(viodev: *mut vio_dev) -> c_int {
    ...
    unsigned long flags;
    size_t size;
    let mut dma_capable: bool = false;
    tbl = get_iommu_table_base(dev);
// A device requires entitlement if it has a DMA window property
    switch (viodev.family) {
    case VDEVICE:
    if (of_get_property(viodev.dev.of_node,
    "ibm,my-dma-window", core::ptr::null_mut()))
    dma_capable = true;
    break;
    case PFO:
    dma_capable = false;
...
//
#[no_mangle]
unsafe extern "C" fn vio_cmo_bus_remove(viodev: *mut vio_dev) {
...
#[no_mangle]
unsafe extern "C" fn vio_cmo_set_dma_ops(viodev: *mut vio_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn vio_cmo_bus_init() {
...
// sysfs bus functions and data structures for CMO
    ...
    __ATTR(cmo_##name, S_IRUGO, cmo_bus_##name##_show, core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn BUS_ATTR_RO(_arg: cmo_##name##_##var) -> static {
...
#[no_mangle]
unsafe extern "C" fn cmo_high_show(bt: *const bus_type, buf: *mut c_char) -> isize {
...
//
#[no_mangle]
pub unsafe extern "C" fn vio_h_cop_sync(vdev: *mut vio_dev, op: *mut vio_pfo_op) -> c_int {
    int vio_h_cop_sync(struct vio_dev *vdev, struct vio_pfo_op *op)
    {
    struct device *dev = &vdev.dev;
    let mut deadline: c_ulong = 0;
    let mut hret: c_long = 0;
    let mut ret: c_int = 0;
    if (op.timeout)
...
//
#[no_mangle]
unsafe extern "C" fn vio_bus_probe(dev: *mut device) -> c_int {
...
// convert from struct device to struct vio_dev and pass to driver.
#[no_mangle]
unsafe extern "C" fn vio_bus_remove(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn vio_bus_shutdown(dev: *mut device) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: kexec_in_progress) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn vio_unregister_driver(viodrv: *mut vio_driver) {
...
#[no_mangle]
unsafe extern "C" fn vio_dev_release(dev: *mut device) {
...
//
    ...
    family = PFO;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: of_node_is_type(parent_node, _arg: "vdevice")) -> else {
...
// we need the 'device_type' property, in order to match with drivers
    ...
    prop = of_get_property(of_node, "reg", core::ptr::null_mut());
...
// performing synchronous ops
    prop = of_get_property(of_node, "ibm,resource-id", core::ptr::null_mut());
    if (prop != core::ptr::null_mut())
...
//
#[no_mangle]
unsafe extern "C" fn vio_bus_scan_register_devices(root_name: *mut c_char) -> void __init {
    static void __init vio_bus_scan_register_devices(char *root_name)
    {
    struct device_node *node_root, *node_child;
    if (!root_name)
    return;
    node_root = of_find_node_by_name(core::ptr::null_mut(), root_name);
    if (node_root) {
//
// Create struct vio_devices for each virtual device in
// the device tree. Drivers will associate with them later.
//
    node_child = of_get_next_child(node_root, core::ptr::null_mut());
    while (node_child) {
...
//
#[no_mangle]
unsafe extern "C" fn vio_bus_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn vio_device_init() -> int __init {
    ...
    cp = of_get_property(dn, "compatible", core::ptr::null_mut());
    if (!cp)
    return sysfs_emit(buf, "\n");
...
#[no_mangle]
pub unsafe extern "C" fn vio_unregister_device(viodev: *mut vio_dev) {
...
#[no_mangle]
unsafe extern "C" fn vio_bus_match(dev: *mut device, drv: *const device_driver) -> c_int {
    ...
    return (ids != core::ptr::null_mut()) && (vio_match_device(ids, vio_dev) != core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn vio_hotplug(dev: *const device, env: *mut kobj_uevent_env) -> c_int {
    ...
    const char *cp;
    dn = dev.of_node;
    if (dn && (cp = of_get_property(dn, "compatible", core::ptr::null_mut())))
...
//
    ...
    {
    struct device *found;
    found = bus_find_device_by_name(&vio_bus_type, core::ptr::null_mut(), name);
    if (!found)
    return core::ptr::null_mut();
...
// construct the kobject name from the device node
    ...
    const __be32 *prop;
    prop = of_get_property(vnode, "reg", core::ptr::null_mut());
    if (!prop)
    goto out;
...
#[no_mangle]
pub unsafe extern "C" fn vio_enable_interrupts(dev: *mut vio_dev) -> c_int {
    int vio_enable_interrupts(struct vio_dev *dev)
    {
    let mut rc: c_int = h_vio_signal(dev.unit_address, VIO_IRQ_ENABLE);
    if (rc != H_SUCCESS)
...
#[no_mangle]
pub unsafe extern "C" fn vio_disable_interrupts(dev: *mut vio_dev) -> c_int {
    int vio_disable_interrupts(struct vio_dev *dev)
    {
    let mut rc: c_int = h_vio_signal(dev.unit_address, VIO_IRQ_DISABLE);
    if (rc != H_SUCCESS)
...
#[no_mangle]
unsafe extern "C" fn vio_init() -> int __init {
