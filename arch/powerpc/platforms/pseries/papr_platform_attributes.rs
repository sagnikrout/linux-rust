...
//! Source: arch/powerpc/platforms/pseries/papr_platform_attributes.c
#![no_std]
...
//
pub const ESI_FLAGS_ALL: c_int = 0;

pub const KOBJ_MAX_ATTRS: c_int = 3;

pub const CURR_MAX_ESI_ATTRS: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
...
pub struct h_energy_scale_info_hdr {
    pub num_attrs: __be64,
    pub array_offset: __be64,
...
//
#[no_mangle]
unsafe extern "C" fn papr_get_attr(id: u64, esi: *mut energy_scale_attribute) -> c_int {
    static int papr_get_attr(u64 id, struct energy_scale_attribute *esi)
    {
    let mut esi_buf_size: c_int = ESI_HDR_SIZE + (CURR_MAX_ESI_ATTRS * ESI_ATTR_SIZE);
...
#[no_mangle]
unsafe extern "C" fn add_attr(id: u64, index: c_int, attr: *mut papr_attr) {
...
#[no_mangle]
unsafe extern "C" fn add_attr_group(id: u64, pg: *mut papr_group, show_val_desc: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn papr_init() -> int __init {
...
// Allocate the groups before registering
    ...
    if (papr_groups[idx].pg.name == core::ptr::null_mut())
    goto out_pgattrs;
    }
    for (idx = 0; idx < num_attrs; idx++) {
// Do not add the value desc attr if it does not exist
    let mut show_val_desc: bool = *esi_attrs[idx].value_desc != '\0';
