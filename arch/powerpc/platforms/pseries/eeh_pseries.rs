...
//! Source: arch/powerpc/platforms/pseries/eeh_pseries.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn pseries_pcibios_bus_add_device(pdev: *mut pci_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_get_pe_config_addr(pdn: *mut pci_dn) -> c_int {
    static int pseries_eeh_get_pe_config_addr(struct pci_dn *pdn)
    {
    let mut config_addr: c_int = rtas_config_addr(pdn.busno, pdn.devfn, 0);
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_phb_reset(phb: *mut pci_controller, config_addr: c_int, option: c_int) -> c_int {
    static int pseries_eeh_phb_reset(struct pci_controller *phb, int config_addr, int option)
    {
    int ret;
// Reset PE through RTAS call
    ret = rtas_call(ibm_set_slot_reset, 4, 1, core::ptr::null_mut(),
    config_addr, BUID_HI(phb.buid),
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_phb_configure_bridge(phb: *mut pci_controller, config_addr: c_int) -> c_int {
    static int pseries_eeh_phb_configure_bridge(struct pci_controller *phb, int config_addr)
    {
    int ret;
// Waiting 0.2s maximum before skipping configuration
    let mut max_wait: c_int = 200;
    while (max_wait > 0) {
    ret = rtas_call(ibm_configure_pe, 3, 1, core::ptr::null_mut(),
    config_addr, BUID_HI(phb.buid),
    BUID_LO(phb.buid));
    if (!ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn pseries_eeh_cap_start(pdn: *mut pci_dn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pseries_eeh_find_cap(pdn: *mut pci_dn, cap: c_int) -> c_int {
    static int pseries_eeh_find_cap(struct pci_dn *pdn, int cap)
    {
    let mut pos: c_int = pseries_eeh_cap_start(pdn);
...
#[no_mangle]
unsafe extern "C" fn pseries_eeh_find_ecap(pdn: *mut pci_dn, cap: c_int) -> c_int {
    static int pseries_eeh_find_ecap(struct pci_dn *pdn, int cap)
    {
    struct eeh_dev *edev = pdn_to_eeh_dev(pdn);
    u32 header;
    let mut pos: c_int = 256;
    let mut ttl: c_int = (4096 - 256) / 8;
...
//
    ...
    pdn = pdn ? pdn.parent : core::ptr::null_mut();
    while (pdn) {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_init_edev(pdn: *mut pci_dn) {
    static void pseries_eeh_init_edev(struct pci_dn *pdn)
    {
...
//
#[no_mangle]
pub unsafe extern "C" fn pseries_eeh_init_edev_recursive(pdn: *mut pci_dn) {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_set_option(pe: *mut eeh_pe, option: c_int) -> c_int {
...
// Not support
    ...
    ret = rtas_call(ibm_set_eeh_option, 4, 1, core::ptr::null_mut(),
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_get_state(pe: *mut eeh_pe, delay: *mut c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_reset(pe: *mut eeh_pe, option: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_get_log(pe: *mut eeh_pe, severity: c_int, drv_log: *mut c_char, len: c_ulong) -> c_int {
    ...
    ret = rtas_call(ibm_slot_error_detail, 8, 1, core::ptr::null_mut(), pe.addr,
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_configure_bridge(pe: *mut eeh_pe) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_read_config(edev: *mut eeh_dev, where: c_int, size: c_int, val: *mut u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_eeh_write_config(edev: *mut eeh_dev, where: c_int, size: c_int, val: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pseries_send_allow_unfreeze(pdn: *mut pci_dn, vf_pe_array: *mut u16, cur_vfs: c_int) -> c_int {
    static int pseries_send_allow_unfreeze(struct pci_dn *pdn, u16 *vf_pe_array, int cur_vfs)
    {
    int rc;
    let mut ibm_allow_unfreeze: c_int = rtas_function_token(RTAS_FN_IBM_OPEN_SRIOV_ALLOW_UNFREEZE);
    unsigned long buid, addr;
    addr = rtas_config_addr(pdn.busno, pdn.devfn, 0);
    buid = pdn.phb.buid;
    spin_lock(&rtas_data_buf_lock);
    memcpy(rtas_data_buf, vf_pe_array, RTAS_DATA_BUF_SIZE);
    rc = rtas_call(ibm_allow_unfreeze, 5, 1, core::ptr::null_mut(),
    addr,
    BUID_HI(buid),
    BUID_LO(buid),
...
#[no_mangle]
unsafe extern "C" fn pseries_call_allow_unfreeze(edev: *mut eeh_dev) -> c_int {
    static int pseries_call_allow_unfreeze(struct eeh_dev *edev)
    {
    let mut cur_vfs: c_int = 0, rc = 0, vf_index, bus, devfn, vf_pe_num;
...
#[no_mangle]
unsafe extern "C" fn pseries_notify_resume(edev: *mut eeh_dev) -> c_int {
...
// injects a MMIO error for all pdev's belonging to PE
    ...
    .next_error		= core::ptr::null_mut(),
    .restore_config		= core::ptr::null_mut(), /* NB: configure_bridge() does this */
...
//
#[no_mangle]
unsafe extern "C" fn eeh_pseries_init() -> int __init {
