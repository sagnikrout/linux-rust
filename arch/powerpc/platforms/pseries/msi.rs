...
//! Source: arch/powerpc/platforms/pseries/msi.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pseries_msi_device {
    pub msi_quota: c_uint,
    pub msi_used: c_uint,
}

    static int query_token, change_token;
pub const RTAS_QUERY_FN: c_int = 0;
pub const RTAS_CHANGE_FN: c_int = 1;
pub const RTAS_RESET_FN: c_int = 2;
pub const RTAS_CHANGE_MSI_FN: c_int = 3;
pub const RTAS_CHANGE_MSIX_FN: c_int = 4;
pub const RTAS_CHANGE_32MSI_FN: c_int = 5;
pub const RTAS_CHANGE_32MSIX_FN: c_int = 6;
// RTAS Helpers
#[no_mangle]
unsafe extern "C" fn rtas_change_msi(pdn: *mut pci_dn, func: u32, num_irqs: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rtas_disable_msi(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn rtas_query_irq_number(pdn: *mut pci_dn, offset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn check_req(pdev: *mut pci_dev, nvec: c_int, prop_name: *mut c_char) -> c_int {
    ...
    p = of_get_property(dn, prop_name, core::ptr::null_mut());
    if (!p) {
...
#[no_mangle]
unsafe extern "C" fn check_req_msi(pdev: *mut pci_dev, nvec: c_int) -> c_int {
    static int check_req_msi(struct pci_dev *pdev, int nvec)
    {
    return check_req(pdev, nvec, "ibm,req#msi");
    }
#[no_mangle]
unsafe extern "C" fn check_req_msix(pdev: *mut pci_dev, nvec: c_int) -> c_int {
...
// Quota calculation
    ...
    dn = of_node_get(node);
    while (dn) {
    p = of_get_property(dn, "ibm,pe-total-#msi", core::ptr::null_mut());
    if (p) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct msi_counts {
    pub requestor: *mut device_node,
    pub num_devices: c_int,
    pub request: c_int,
    pub quota: c_int,
    pub spare: c_int,
    pub over_quota: c_int,
}
    ...
    p = of_get_property(dn, "class-code", core::ptr::null_mut());
...
// so we just have to punt and use the larger of the two.
    req = 0;
    p = of_get_property(dn, "ibm,req#msi", core::ptr::null_mut());
    if (p)
    req = be32_to_cpup(p);
    p = of_get_property(dn, "ibm,req#msi-x", core::ptr::null_mut());
    if (p)
...
#[no_mangle]
pub unsafe extern "C" fn if(counts->quota: req >) -> else {
    else if (req > counts.quota)
    counts.over_quota++;
    return core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn msi_quota_for_device(dev: *mut pci_dev, request: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rtas_hack_32bit_msi_gen2(pdev: *mut pci_dev) {
...
//
    ...
    struct pci_dn *pdn;
    int quota, rc;
    let mut nvec: c_int = nvec_in;
    let mut use_32bit_msi_hack: c_int = 0;
...
//
    if (type == PCI_CAP_ID_MSIX) {
    let mut m: c_int = roundup_pow_of_two(nvec);
...
//
    ...
    let mut type: c_int = (info.flags & MSI_FLAG_PCI_MSIX) ? PCI_CAP_ID_MSIX : PCI_CAP_ID_MSI;
    int ret;
#[no_mangle]
pub unsafe extern "C" fn __free(_arg: kfree) -> *mut pseries_msi_device pseries_dev {
...
//
#[no_mangle]
unsafe extern "C" fn pseries_msi_ops_teardown(domain: *mut irq_domain, arg: *mut msi_alloc_info_t) {
...
#[no_mangle]
unsafe extern "C" fn pseries_msi_compose_msg(data: *mut irq_data, msg: *mut msi_msg) {
...
#[no_mangle]
pub unsafe extern "C" fn pseries_msi_allocate_domains(phb: *mut pci_controller) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pseries_msi_free_domains(phb: *mut pci_controller) {
...
#[no_mangle]
unsafe extern "C" fn rtas_msi_init() -> c_int {
