...
//! Source: arch/powerpc/platforms/powermac/bootx_init.c
#![no_std]
...
// Macro flag: #define SET_BOOT_BAT
...
#[no_mangle]
unsafe extern "C" fn bootx_printf(format: *const c_char, ...) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn bootx_dt_find_string(str: *mut c_char) -> unsigned long __init {
    ...
    {
    let mut soff: c_ulong = bootx_dt_find_string(name);
...
// mem_end = ALIGN(*mem_end + size, 4);
    ...
    {
    u32 val;
    bootx_dt_add_prop("linux,bootx", core::ptr::null_mut(), 0, mem_end);
    ...
    u32 tmp;
    if (has_real_node) {
    bootx_dt_add_prop("linux,boot-display", core::ptr::null_mut(), 0, mem_end);
    bootx_dt_add_prop("linux,opened", core::ptr::null_mut(), 0, mem_end);
    } else
    bootx_dt_add_prop("linux,bootx-noscreen", core::ptr::null_mut(), 0, mem_end);
...
#[no_mangle]
unsafe extern "C" fn bootx_dt_add_string(s: *mut c_char, mem_end: *mut c_ulong) -> void __init {
    static void __init bootx_dt_add_string(char *s, unsigned long *mem_end)
    {
    let mut l: c_uint = strlen(s) + 1;
...
// Keep refs to known nodes
    namep = np.full_name ? (char *)(base + np.full_name) : core::ptr::null_mut();
    if (namep == core::ptr::null_mut()) {
...
// get and store all property names
    ...
    struct bootx_dt_prop *pp =
    (struct bootx_dt_prop *)(base + *ppp);
    namep = pp.name ? (char *)(base + pp.name) : core::ptr::null_mut();
    if (namep == core::ptr::null_mut() || strcmp(namep, "name") == 0)
    goto next;
...
// get and store all properties
    ...
    namep = pp.name ? (char *)(base + pp.name) : core::ptr::null_mut();
// Skip "name"
    if (namep == core::ptr::null_mut() || !strcmp(namep, "name"))
    goto next;
...
// push property head
    ...
    pp.value ? (void *)(base + pp.value): core::ptr::null_mut(),
    pp.length, mem_end);
    next:
    ppp = &pp.next;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(bootx_info->dispDeviceRegEntryOffset: node ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn bootx_flatten_dt(start: c_ulong) -> unsigned long __init {
...
#[no_mangle]
unsafe extern "C" fn btext_welcome(bi: *mut boot_infos_t) -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn bootx_init(r3: c_ulong, r4: c_ulong) -> void __init {
