...
//! Source: arch/powerpc/platforms/44x/soc.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn l2c_diag(addr: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn l2c_error_handler(irq: c_int, dev: *mut c_void) -> irqreturn_t {
    static irqreturn_t l2c_error_handler(int irq, void *dev)
    {
    let mut sr: u32 = mfdcr(dcrbase_l2c + DCRN_L2C0_SR);
    if (sr & L2C_SR_CPE) {
// Read cache trapped address
    let mut addr: u32 = l2c_diag(0x42000000);
    printk(KERN_EMERG "L2C: Cache Parity Error, addr[16:26] = 0x%08x\n",
    addr);
    }
    if (sr & L2C_SR_TPE) {
// Read tag trapped address
    let mut addr: u32 = l2c_diag(0x82000000) >> 16;
...
#[no_mangle]
unsafe extern "C" fn ppc4xx_l2c_probe() -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn ppc4xx_reset_system(cmd: *mut c_char) {
    void ppc4xx_reset_system(char *cmd)
    {
    struct device_node *np;
    let mut reset_type: u32 = DBCR0_RST_SYSTEM;
    const u32 *prop;
    np = of_get_cpu_node(0, core::ptr::null_mut());
    if (np) {
    prop = of_get_property(np, "reset-type", core::ptr::null_mut());
//
