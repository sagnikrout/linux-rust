...
//! Source: arch/powerpc/platforms/chrp/pci.c
#![no_std]
...
// val = ret;
    ...
    volatile struct Hydra __iomem *Hydra = core::ptr::null_mut();
#[no_mangle]
unsafe extern "C" fn hydra_init() -> int __init {
    static int __init hydra_init(void)
    {
    struct device_node *np;
    struct resource r;
    np = of_find_node_by_name(core::ptr::null_mut(), "mac-io");
    if (np == core::ptr::null_mut() || of_address_to_resource(np, 0, &r)) {
    of_node_put(np);
    return 0;
    }
    of_node_put(np);
...
#[no_mangle]
unsafe extern "C" fn setup_peg2(hose: *mut pci_controller, dev: *mut device_node) -> void __init {
...
// keep the reference to the root node
    ...
    let mut is_longtrail: c_int = 0, is_mot = 0, is_pegasos = 0;
    struct device_node *root = of_find_node_by_path("/");
    struct resource r;
//
...
//
    ...
    is_pegasos = 2;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(machine, _arg: "Pegasos", 0: 7) ==) -> else {
...
// The GG2 bridge on the LongTrail doesn't have an address
    ...
    model = of_get_property(dev, "model", core::ptr::null_mut());
...
// check the first bridge for a property that we can
    ...
    if (index == 0 && dma != core::ptr::null_mut() && len >= 6 * sizeof(*dma)) {
...
pub const SL82C105_IDECSR: c_uint = 0x40;
...
//
#[no_mangle]
unsafe extern "C" fn chrp_pci_fixup_winbond_ata(sl82c105: *mut pci_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn chrp_pci_fixup_vt8231_ata(viaide: *mut pci_dev) {
    ...
    if (viaide.irq != 14)
    return;
    viaisa = pci_get_device(PCI_VENDOR_ID_VIA, PCI_DEVICE_ID_VIA_8231, core::ptr::null_mut());
    if (!viaisa)
    return;
