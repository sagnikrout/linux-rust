...
//! Source: arch/powerpc/platforms/512x/mpc512x_lpbfifo.c
#![no_std]
...
pub struct cs_range {
    pub csnum: u32,
    pub /: *mut *mut u32 base; / must be zero,
    pub addr: u32,
    pub size: u32,
}
...
//
#[no_mangle]
unsafe extern "C" fn mpc512x_lpbfifo_irq(irq: c_int, param: *mut c_void) -> irqreturn_t {
    static irqreturn_t mpc512x_lpbfifo_irq(int irq, void *param)
    {
    struct device *dev = (struct device *)param;
    struct mpc512x_lpbfifo_request *req = core::ptr::null_mut();
    unsigned long flags;
    u32 status;
...
//
#[no_mangle]
unsafe extern "C" fn mpc512x_lpbfifo_callback(param: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn mpc512x_lpbfifo_kick() -> c_int {
    static int mpc512x_lpbfifo_kick(void)
    {
    u32 bits;
    let mut no_incr: bool = false;
    u32 bpt = 32; /* max bytes per LPBFIFO transaction involving DMA */
    let mut cs: u32 = 0;
    size_t i;
    struct dma_device *dma_dev = core::ptr::null_mut();
    struct scatterlist sg;
    enum dma_data_direction dir;
    let mut dma_conf: dma_slave_config = {};
    struct dma_async_tx_descriptor *dma_tx = core::ptr::null_mut();
    dma_cookie_t cookie;
    int ret;
//
...
//
    ...
    let mut cs_start: phys_addr_t = lpbfifo.cs_ranges[i].addr;
    let mut cs_end: phys_addr_t = cs_start + lpbfifo.cs_ranges[i].size;
    let mut access_start: phys_addr_t = lpbfifo.req.dev_phys_addr;
    let mut access_end: phys_addr_t = access_start + lpbfifo.req.size;
...
// Make DMA channel work with LPB FIFO data register
    ...
    dma_tx.callback_param = core::ptr::null_mut();
// 3. Prepare FIFO
    out_be32(&lpbfifo.regs.enable,
    MPC512X_SCLPC_RESET | MPC512X_SCLPC_FIFO_RESET);
...
#[no_mangle]
unsafe extern "C" fn mpc512x_lpbfifo_submit_locked(req: *mut mpc512x_lpbfifo_request) -> c_int {
...
// Check whether a transfer is in progress
    ...
    if (ret != 0)
    lpbfifo.req = core::ptr::null_mut(); /* Set the FIFO as idle */
    return ret;
    }
#[no_mangle]
pub unsafe extern "C" fn mpc512x_lpbfifo_submit(req: *mut mpc512x_lpbfifo_request) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn get_cs_ranges(dev: *mut device) -> c_int {
    ...
    lb_node = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "fsl,mpc5121-localbus");
    if (!lb_node)
    return ret;
    ...
    let mut base: u32 = lower_32_bits(range.bus_addr);
    if (base)
    goto end;
...
#[no_mangle]
unsafe extern "C" fn mpc512x_lpbfifo_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mpc512x_lpbfifo_remove(pdev: *mut platform_device) {
    static void mpc512x_lpbfifo_remove(struct platform_device *pdev)
    {
    unsigned long flags;
    struct dma_device *dma_dev = lpbfifo.chan.device;
    struct mpc512x_lpbfifo __iomem *regs = core::ptr::null_mut();
    spin_lock_irqsave(&lpbfifo.lock, flags);
    regs = lpbfifo.regs;
    lpbfifo.regs = core::ptr::null_mut();
    spin_unlock_irqrestore(&lpbfifo.lock, flags);
