...
//! Source: arch/powerpc/net/bpf_jit_comp32.c
#![no_std]
...
pub const SEEN_VREG_MASK: c_uint = 0x1ff80000 /* Volatile registers r3-r12 */;
pub const SEEN_NVREG_FULL_MASK: c_uint = 0x0003ffff /* Non volatile registers r14-r31 */;
pub const SEEN_NVREG_TEMP_MASK: c_uint = 0x00001e01 /* BPF_REG_5, BPF_REG_AX, TMP_REG */;
#[no_mangle]
...
pub unsafe extern "C" fn bpf_jit_realloc_regs(ctx: *mut codegen_context) {
    ...
    let mut old: c_int = 32 - fls(ctx.seen & (nvreg_mask & 0xaaaaaaab));
    let mut new: c_int = 32 - fls(~ctx.seen & (SEEN_VREG_MASK & 0xaaaaaaaa));
    int i;
...
// Initialize tail_call_cnt, to be skipped if we do tail calls.
    ...
    EMIT(PPC_RAW_NOP());
pub const BPF_TAILCALL_PROLOGUE_SIZE: c_int = 8;
...
pub unsafe extern "C" fn bpf_jit_emit_func_call_rel(image: *mut u32, fimage: *mut u32, ctx: *mut codegen_context, func: u64) -> c_int {
    ...
    {
    let mut rel: i32 = (s32)func - (s32)(fimage + ctx.idx);
...
// Start of epilogue code - will only be valid 2nd pass onwards
    ...
    let mut prevcode: u32 = i ? insn[i - 1].code : 0;
    let mut dst_reg: u32 = bpf_to_ppc(insn[i].dst_reg);
    let mut dst_reg_h: u32 = dst_reg - 1;
    let mut src_reg: u32 = bpf_to_ppc(insn[i].src_reg);
    let mut src_reg_h: u32 = src_reg - 1;
    let mut src2_reg: u32 = dst_reg;
    let mut src2_reg_h: u32 = dst_reg_h;
    let mut ax_reg: u32 = bpf_to_ppc(BPF_REG_AX);
    let mut tmp_reg: u32 = bpf_to_ppc(TMP_REG);
...
// special mov32 for zext
    ...
    EMIT(PPC_RAW_LI(dst_reg_h, 0));
#[no_mangle]
pub unsafe extern "C" fn if(8: off ==) -> else {
    else if (off == 8)
    EMIT(PPC_RAW_EXTSB(dst_reg, src_reg));
#[no_mangle]
pub unsafe extern "C" fn if(16: off ==) -> else {
    else if (off == 16)
    EMIT(PPC_RAW_EXTSH(dst_reg, src_reg));
#[no_mangle]
pub unsafe extern "C" fn if(src_reg: dst_reg !=) -> else {
    else if (dst_reg != src_reg)
    EMIT(PPC_RAW_MR(dst_reg, src_reg));
    break;
...
//
    ...
    let mut insn_idx: c_int = ctx.idx - 1;
    let mut jmp_off: c_int = 4;
//
