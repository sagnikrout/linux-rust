...
//! Source: arch/powerpc/net/bpf_jit_comp.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn bpf_jit_fill_ill_insns(area: *mut c_void, size: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_build_fentry_stubs(image: *mut u32, ctx: *mut codegen_context) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_emit_exit_insn(image: *mut u32, ctx: *mut codegen_context, tmp_reg: c_int, exit_addr: c_long) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct powerpc_jit_data {
// address of rw header
    pub hdr: *mut bpf_binary_header,
// address of ro final header
    pub fhdr: *mut bpf_binary_header,
    pub addrs: *mut u32,
    pub fimage: *mut u8,
    pub proglen: u32,
    pub ctx: codegen_context,
}
...
unsafe extern "C" fn priv_stack_init_guard(priv_stack_ptr: *mut void __percpu, alloc_size: c_int) {
    ...
    u32 *code_base = core::ptr::null_mut();
    u32 *addrs = core::ptr::null_mut();
    struct powerpc_jit_data *jit_data = core::ptr::null_mut();
    struct codegen_context cgctx;
    int pass;
    int flen;
    int priv_stack_alloc_size;
    void __percpu *priv_stack_ptr = core::ptr::null_mut();
    struct bpf_binary_header *fhdr = core::ptr::null_mut();
    struct bpf_binary_header *hdr = core::ptr::null_mut();
    let mut extra_pass: bool = false;
    u8 *fimage = core::ptr::null_mut();
    u32 *fcode_base = core::ptr::null_mut();
    u32 extable_len;
    u32 fixup_len;
    if (!fp.jit_requested)
    return fp;
...
// Scouting faux-generate pass 0
    if (bpf_jit_build_body(fp, core::ptr::null_mut(), core::ptr::null_mut(), &cgctx, addrs, 0, false))
...
//
    bpf_jit_build_prologue(core::ptr::null_mut(), &cgctx);
    addrs[fp.len] = cgctx.idx * 4;
    bpf_jit_build_epilogue(core::ptr::null_mut(), &cgctx);
...
//
    ...
    fp.aux.priv_stack_ptr = core::ptr::null_mut();
...
pub unsafe extern "C" fn bpf_arch_text_invalidate(dst: *mut c_void, len: usize) -> c_int {
    ...
    {
    let mut insn: u32 = BREAKPOINT_INSTRUCTION;
    int ret;
    if (WARN_ON_ONCE(core_kernel_text((unsigned long)dst)))
    return -EINVAL;
    mutex_lock(&text_mutex);
...
// Used when we call into the traced function. Replicate parameter save area
    ...
    ppc_inst_t branch_insn;
    u32 *branches = core::ptr::null_mut();
    bool save_ret;
    if (IS_ENABLED(CONFIG_PPC32))
    return -EOPNOTSUPP;
    nr_regs = m.nr_args;
...
//
    ...
    let mut meta: u64 = func_meta | (cookie_ctx_off << BPF_TRAMP_COOKIE_INDEX_SHIFT);
...
// Reserve space to patch branch instruction to skip fexit progs
    if (ro_image) /* image is core::ptr::null_mut() for dummy pass */
...
// Make sure the trampoline generation logic doesn't overflow
    ...
    int ret;
    ret = __arch_prepare_bpf_trampoline(&im, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), m, flags, tnodes, func_addr);
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn bpf_modify_inst(ip: *mut c_void, old_inst: ppc_inst_t, new_inst: ppc_inst_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_isync(__maybe_unused: *mut *mut void info) {
...
//
    ...
    let mut ret: c_int = 0, branch_flags;
...
// Make sure all cpus see the new instruction
    smp_call_function(do_isync, core::ptr::null_mut(), 1);
    return ret;
    }
