...
//! Source: arch/powerpc/net/bpf_jit_comp64.c
#![no_std]
...
pub const BPF_PPC_STACK_LOCALS: c_int = 24;
//
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_init_reg_mapping(ctx: *mut codegen_context) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_has_stack_frame(ctx: *mut codegen_context) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn bpf_jit_stack_local(ctx: *mut codegen_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_jit_stack_tailcallinfo_offset(ctx: *mut codegen_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_jit_stack_offsetof(ctx: *mut codegen_context, reg: c_int) -> c_int {
    static int bpf_jit_stack_offsetof(struct codegen_context *ctx, int reg)
    {
    let mut min_valid_nvreg: c_int = BPF_PPC_NVR_MIN;
// Default frame size for all cases except exception boundary
    let mut frame_nvr_size: c_int = BPF_PPC_STACKFRAME;
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_realloc_regs(ctx: *mut codegen_context) {
    void bpf_jit_realloc_regs(struct codegen_context *ctx)
    {
    }
#[no_mangle]
unsafe extern "C" fn emit_fp_priv_stack(image: *mut u32, ctx: *mut codegen_context) {
    static void emit_fp_priv_stack(u32 *image, struct codegen_context *ctx)
    {
    PPC_LI64(bpf_to_ppc(BPF_REG_FP), ( long)ctx.priv_sp);
//
...
//
#[no_mangle]
unsafe extern "C" fn bpf_jit_stack_size(ctx: *mut codegen_context) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_build_prologue(image: *mut u32, ctx: *mut codegen_context) {
...
#[no_mangle]
unsafe extern "C" fn bpf_jit_emit_common_epilogue(image: *mut u32, ctx: *mut codegen_context) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_build_epilogue(image: *mut u32, ctx: *mut codegen_context) {
...
//
#[no_mangle]
pub unsafe extern "C" fn arch_bpf_stack_walk(: *mut *mut bool (consume_fn)(void, _arg: u64, _arg: u64, _arg: u64), cookie: *mut c_void) {
    void arch_bpf_stack_walk(bool (*consume_fn)(void *, u64, u64, u64), void *cookie)
    {
// callback processing always in current context
    let mut sp: c_ulong = current_stack_frame();
    for (;;) {
...
#[no_mangle]
unsafe extern "C" fn bpf_jit_emit_func_call(image: *mut u32, ctx: *mut codegen_context, func_addr: u64, reg: c_int) -> c_int {
    static int bpf_jit_emit_func_call(u32 *image, struct codegen_context *ctx, u64 func_addr, int reg)
    {
    let mut reladdr: c_long = func_addr - kernel_toc_addr();
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_emit_func_call_rel(image: *mut u32, fimage: *mut u32, ctx: *mut codegen_context, func: u64) -> c_int {
    int bpf_jit_emit_func_call_rel(u32 *image, u32 *fimage, struct codegen_context *ctx, u64 func)
    {
    let mut func_addr: c_ulong = func ? ppc_function_entry((void *)func) : 0;
...
// paddi r12,r12,addr
    ...
    let mut pc: c_ulong = (unsigned long)fimage + CTX_NIA(ctx);
    let mut alignment_needed: bool = !IS_ALIGNED(pc, 8);
...
// We can clobber r12
    ...
    let mut ret: c_int = bpf_jit_emit_func_call(image, ctx, func_addr, _R12);
    if (ret)
    return ret;
    } else {
    if (IS_ENABLED(CONFIG_PPC64_ELF_ABI_V1)) {
...
#[no_mangle]
unsafe extern "C" fn zero_extend(image: *mut u32, ctx: *mut codegen_context, src_reg: u32, dst_reg: u32, size: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sign_extend(image: *mut u32, ctx: *mut codegen_context, src_reg: u32, dst_reg: u32, size: u32) -> c_int {
...
// Note that BPF ABI only allows up to 5 args for kfuncs
    let mut reg: u32 = bpf_to_ppc(BPF_REG_1 + i), size = m.arg_size[i];
...
#[no_mangle]
unsafe extern "C" fn bpf_jit_emit_tail_call(image: *mut u32, ctx: *mut codegen_context, out: u32) -> c_int {
...
//
    let mut b2p_bpf_array: c_int = bpf_to_ppc(BPF_REG_2);
    let mut b2p_index: c_int = bpf_to_ppc(BPF_REG_3);
    let mut bpf_tailcall_prologue_size: c_int = 12;
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_bypass_spec_v1() -> bool {
...
//
    ...
    {
    let mut tmp1_reg: u32 = bpf_to_ppc(TMP_REG_1);
    let mut tmp2_reg: u32 = bpf_to_ppc(TMP_REG_2);
    let mut size: u32 = BPF_SIZE(insn.code);
    let mut src_reg: u32 = bpf_to_ppc(insn.src_reg);
    let mut dst_reg: u32 = bpf_to_ppc(insn.dst_reg);
    let mut imm: i32 = insn.imm;
    let mut save_reg: u32 = tmp2_reg;
    let mut ret_reg: u32 = src_reg;
    u32 fixup_idx;
...
#[no_mangle]
unsafe extern "C" fn emit_atomic_ld_st(insn: bpf_insn, ctx: *mut codegen_context, image: *mut u32) -> c_int {
    static int emit_atomic_ld_st(const struct bpf_insn insn, struct codegen_context *ctx, u32 *image)
    {
    let mut code: u32 = insn.code;
    let mut dst_reg: u32 = bpf_to_ppc(insn.dst_reg);
    let mut src_reg: u32 = bpf_to_ppc(insn.src_reg);
    let mut size: u32 = BPF_SIZE(code);
    let mut tmp1_reg: u32 = bpf_to_ppc(TMP_REG_1);
    let mut tmp2_reg: u32 = bpf_to_ppc(TMP_REG_2);
...
// Assemble the body code between the prologue & epilogue
    ...
    {
    let mut stf_barrier: enum stf_barrier_type = stf_barrier_type_get();
    bool sync_emitted, ori31_emitted;
    const struct bpf_insn *insn = fp.insnsi;
    let mut flen: c_int = fp.len;
    int i, ret;
// Start of epilogue code - will only be valid 2nd pass onwards
    let mut exit_addr: u32 = addrs[flen];
    for (i = 0; i < flen; i++) {
    let mut code: u32 = insn[i].code;
    let mut dst_reg: u32 = bpf_to_ppc(insn[i].dst_reg);
    let mut src_reg: u32 = bpf_to_ppc(insn[i].src_reg);
    let mut size: u32 = BPF_SIZE(code);
    let mut tmp1_reg: u32 = bpf_to_ppc(TMP_REG_1);
    let mut tmp2_reg: u32 = bpf_to_ppc(TMP_REG_2);
    let mut off: i16 = insn[i].off;
    let mut imm: i32 = insn[i].imm;
...
// MOVSX: dst = (s8,s16,s32)src (off = 8,16,32)
    ...
    goto bpf_alu32_trunc;
#[no_mangle]
pub unsafe extern "C" fn if(1]): insn_is_zext(&insn[i +) -> else {
...
// signed comparison
    ...
    case BPF_JMP32 | BPF_JLE | BPF_K:
    {
    let mut is_jmp32: bool = BPF_CLASS(code) == BPF_JMP32;
//
