...
//! Source: arch/powerpc/sysdev/ehv_pic.c
#![no_std]
...
pub const IRQ_TYPE_MPIC_DIRECT: c_int = 4;
pub const MPIC_EOI: c_uint = 0x00B0;
//
// Linux descriptor level callbacks
//
#[no_mangle]
unsafe extern "C" fn ehv_pic_unmask_irq(d: *mut irq_data) {
    static void ehv_pic_unmask_irq(struct irq_data *d)
    {
    let mut src: c_uint = virq_to_hw(d.irq);
    ev_int_set_mask(src, 0);
    }
#[no_mangle]
unsafe extern "C" fn ehv_pic_mask_irq(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn ehv_pic_end_irq(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn ehv_pic_direct_end_irq(d: *mut irq_data) {
    ...
    let mut cpuid: c_int = irq_choose_cpu(dest);
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn ehv_pic_type_to_vecpri(type: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn ehv_pic_set_irq_type(d: *mut irq_data, flow_type: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ehv_pic_get_irq() -> c_uint {
    unsigned int ehv_pic_get_irq(void)
    {
    int irq;
    BUG_ON(global_ehv_pic == core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn ehv_pic_init() -> void __init {
    void __init ehv_pic_init(void)
    {
    struct device_node *np, *np2;
    struct ehv_pic *ehv_pic;
    np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "epapr,hv-pic");
    if (!np) {
    ...
    np2 = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "fsl,hv-mpic-per-cpu");
    if (np2) {
