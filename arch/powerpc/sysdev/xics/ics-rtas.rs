...
//! Source: arch/powerpc/sysdev/xics/ics-rtas.c
#![no_std]
...
unsafe extern "C" fn ics_rtas_unmask_irq(d: *mut irq_data) {
    ...
    call_status = rtas_call(ibm_set_xive, 3, 1, core::ptr::null_mut(), hw_irq, server,
    DEFAULT_PRIORITY);
    if (call_status != 0) {
    printk(KERN_ERR
...
// Now unmask the interrupt (often a no-op)
    call_status = rtas_call(ibm_int_on, 1, 1, core::ptr::null_mut(), hw_irq);
    if (call_status != 0) {
...
#[no_mangle]
unsafe extern "C" fn ics_rtas_startup(d: *mut irq_data) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn ics_rtas_mask_real_irq(hw_irq: c_uint) {
    static void ics_rtas_mask_real_irq(unsigned int hw_irq)
    {
    int call_status;
    if (hw_irq == XICS_IPI)
    return;
    call_status = rtas_call(ibm_int_off, 1, 1, core::ptr::null_mut(), hw_irq);
    if (call_status != 0) {
...
// Have to set XIVE to 0xff to be able to remove a slot
    call_status = rtas_call(ibm_set_xive, 3, 1, core::ptr::null_mut(), hw_irq,
...
unsafe extern "C" fn ics_rtas_mask_irq(d: *mut irq_data) {
    ...
    hw_irq, irq_server);
    status = rtas_call(ibm_set_xive, 3, 1, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn ics_rtas_check(ics: *mut ics, hw_irq: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ics_rtas_mask_unknown(ics: *mut ics, vec: c_ulong) {
    static void ics_rtas_mask_unknown(struct ics *ics, unsigned long vec)
    {
    ics_rtas_mask_real_irq(vec);
    }
#[no_mangle]
unsafe extern "C" fn ics_rtas_get_server(ics: *mut ics, vec: c_ulong) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn ics_rtas_host_match(ics: *mut ics, node: *mut device_node) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ics_rtas_init() -> __init int {
