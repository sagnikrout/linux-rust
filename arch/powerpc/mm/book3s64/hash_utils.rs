...
//! Source: arch/powerpc/mm/book3s64/hash_utils.c
#![no_std]
...
//
    ...
    let mut mmu_linear_psize: c_int = MMU_PAGE_4K;
    EXPORT_SYMBOL_GPL(mmu_linear_psize);
    let mut mmu_virtual_psize: c_int = MMU_PAGE_4K;
    let mut mmu_vmalloc_psize: c_int = MMU_PAGE_4K;
    EXPORT_SYMBOL_GPL(mmu_vmalloc_psize);
    let mut mmu_io_psize: c_int = MMU_PAGE_4K;
    let mut mmu_kernel_ssize: c_int = MMU_SEGSIZE_256M;
    EXPORT_SYMBOL_GPL(mmu_kernel_ssize);
    let mut mmu_highuser_ssize: c_int = MMU_SEGSIZE_256M;
    let mut mmu_slb_size: u16 = 64;
...
#[no_mangle]
pub unsafe extern "C" fn tlbiel_hash_set_isa206(set: c_uint, is: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn volatile(_arg: PPC_TLBIEL(%0, _arg: %1, _arg: %2, _arg: %3, _arg: %4) -> asm {
    asm volatile(PPC_TLBIEL(%0, %1, %2, %3, %4)
    : : "r"(rb), "r"(rs), "i"(ric), "i"(prs), "i"(r)
    : "memory");
    }
#[no_mangle]
unsafe extern "C" fn tlbiel_all_isa206(num_sets: c_uint, is: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn hash__tlbiel_all(action: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: early_cpu_has_feature(CPU_FTR_ARCH_206)) -> else {
    ...
    {
    unsigned long hash;
    let mut vsid: c_ulong = get_kernel_vsid(vaddr, mmu_kernel_ssize);
    let mut vpn: c_ulong = hpt_vpn(vaddr, vsid, mmu_kernel_ssize);
    let mut mode: c_ulong = htab_convert_pte_flags(pgprot_val(PAGE_KERNEL), HPTE_USE_KERNEL_KEY);
    long ret;
...
#[no_mangle]
pub unsafe extern "C" fn hash_supports_debug_pagealloc() -> bool {
    static inline bool hash_supports_debug_pagealloc(void)
    {
    let mut max_hash_count: c_ulong = ppc64_rma_size / 4;
    let mut linear_map_count: c_ulong = memblock_end_of_DRAM() >> PAGE_SHIFT;
...
#[no_mangle]
unsafe extern "C" fn hash_debug_pagealloc_alloc_slots() -> __init void {
...
#[no_mangle]
unsafe extern "C" fn hash_kfence_alloc_pool() -> __init void {
...
#[no_mangle]
unsafe extern "C" fn hash_kfence_map_pool() -> __init void {
    static __init void hash_kfence_map_pool(void)
    {
    unsigned long kfence_pool_start, kfence_pool_end;
    let mut prot: c_ulong = pgprot_val(PAGE_KERNEL);
    let mut pshift: c_uint = mmu_psize_defs[mmu_linear_psize].shift;
    if (!kfence_pool)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn hash_kfence_add_slot(paddr: phys_addr_t, slot: c_int) {
    static inline void hash_kfence_add_slot(phys_addr_t paddr, int slot)
    {
    let mut vaddr: c_ulong = (unsigned long) __va(paddr);
...
#[no_mangle]
unsafe extern "C" fn hash_kfence_map_pages(page: *mut page, numpages: c_int, enable: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hash__kernel_map_pages(page: *mut page, numpages: c_int, enable: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hash_linear_map_add_slot(paddr: phys_addr_t, slot: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn htab_convert_pte_flags(pteflags: c_ulong, flags: c_ulong) -> c_ulong {
    unsigned long htab_convert_pte_flags(unsigned long pteflags, unsigned long flags)
    {
    let mut rflags: c_ulong = 0;
// _PAGE_EXEC -> NOEXEC
...
//
    ...
    rflags |= HPTE_R_I;
#[no_mangle]
pub unsafe extern "C" fn if(_PAGE_NON_IDEMPOTENT: (pteflags & _PAGE_CACHE_CTL) ==) -> else {
    else if ((pteflags & _PAGE_CACHE_CTL) == _PAGE_NON_IDEMPOTENT)
    rflags |= (HPTE_R_I | HPTE_R_G);
#[no_mangle]
pub unsafe extern "C" fn if(_PAGE_SAO: (pteflags & _PAGE_CACHE_CTL) ==) -> else {
...
// Carefully map only the possible range
    ...
    let mut vsid: c_ulong = get_kernel_vsid(vaddr, ssize);
    let mut vpn: c_ulong = hpt_vpn(vaddr, vsid, ssize);
    let mut tprot: c_ulong = prot;
    let mut secondary_hash: bool = false;
//
...
#[no_mangle]
unsafe extern "C" fn parse_disable_1tb_segments(p: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn parse_stress_hpt(p: *mut c_char) -> int __init {
...
//
pub const STRESS_MAX_GROUPS: c_int = 16;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stress_hpt_struct {
    pub last_group: [c_ulong; STRESS_MAX_GROUPS],
}

#[no_mangle]
pub unsafe extern "C" fn stress_nr_groups() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_idx_from_shift(shift: c_uint) -> int __init {
...
// We are scanning "cpu" nodes only
    ...
    while(size > 0) {
    let mut base_shift: c_uint = be32_to_cpu(prop[0]);
    let mut slbenc: c_uint = be32_to_cpu(prop[1]);
    let mut lpnum: c_uint = be32_to_cpu(prop[2]);
...
//
    ...
    def.tlbiel = 0;
    while (size > 0 && lpnum) {
    let mut shift: c_uint = be32_to_cpu(prop[0]);
    let mut penc: c_int = be32_to_cpu(prop[1]);
    prop += 2; size -= 2;
    lpnum--;
...
//
    page_count_prop = of_get_flat_dt_prop(node, "ibm,expected#pages", core::ptr::null_mut());
    if (page_count_prop == core::ptr::null_mut())
    return 0;
    expected_pages = (1 << be32_to_cpu(page_count_prop[0]));
    addr_prop = of_get_flat_dt_prop(node, "reg", core::ptr::null_mut());
    if (addr_prop == core::ptr::null_mut())
    return 0;
...
#[no_mangle]
unsafe extern "C" fn mmu_psize_set_default_penc() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn might_have_hea() -> bool __init {
...
#[no_mangle]
unsafe extern "C" fn htab_scan_page_sizes() -> void __init {
...
//
    rc = of_scan_flat_dt(htab_dt_scan_page_sizes, core::ptr::null_mut());
...
// Reserve 16G huge page memory sections for huge pages
    of_scan_flat_dt(htab_dt_scan_hugepage_blocks, core::ptr::null_mut());
    }
...
//
#[no_mangle]
unsafe extern "C" fn init_hpte_page_sizes() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn htab_init_page_sizes() -> void __init {
...
// We are scanning "cpu" nodes only
    if (type == core::ptr::null_mut() || strcmp(type, "cpu") != 0)
    return 0;
    prop = of_get_flat_dt_prop(node, "ibm,pft-size", core::ptr::null_mut());
    if (prop != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn htab_shift_for_mem_size(mem_size: c_ulong) -> unsigned {
    unsigned htab_shift_for_mem_size(unsigned long mem_size)
    {
    let mut memshift: unsigned = __ilog2(mem_size);
    let mut pshift: unsigned = mmu_psize_defs[mmu_virtual_psize].shift;
    unsigned pteg_shift;
...
#[no_mangle]
unsafe extern "C" fn htab_get_table_size() -> unsigned long __init {
...
//
    if (ppc64_pft_size == 0)
    of_scan_flat_dt(htab_dt_scan_pftsize, core::ptr::null_mut());
    if (ppc64_pft_size)
...
#[no_mangle]
unsafe extern "C" fn resize_hpt_for_hotplug(new_mem_size: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hash__remove_section_mapping(start: c_ulong, end: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stress_hpt_timer_fn(timer: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn htab_initialize() -> void __init {
    static void __init htab_initialize(void)
    {
    unsigned long table;
    unsigned long pteg_count;
    unsigned long prot;
    let mut base: phys_addr_t = 0, size = 0, end, limit = MEMBLOCK_ALLOC_ANYWHERE;
    u64 i;
    let mut pshift: c_uint = mmu_psize_defs[mmu_linear_psize].shift;
    DBG(" . htab_initialize()\n");
    if (firmware_has_feature(FW_FEATURE_LPAR))
...
// Using a hypervisor which owns the htab
    htab_address = core::ptr::null_mut();
    _SDR1 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn hash__early_init_devtree() -> void __init {
    void __init hash__early_init_devtree(void)
    {
// Initialize segment sizes
    of_scan_flat_dt(htab_dt_scan_seg_sizes, core::ptr::null_mut());
// Initialize page sizes
    htab_scan_page_sizes();
    }
    static struct hash_mm_context init_hash_mm_context;
#[no_mangle]
pub unsafe extern "C" fn hash__early_init_mmu() -> void __init {
...
// Select appropriate backend
    ...
    ps3_early_mm_init();
#[no_mangle]
pub unsafe extern "C" fn if(_arg: firmware_has_feature(FW_FEATURE_LPAR)) -> else {
    else if (firmware_has_feature(FW_FEATURE_LPAR))
    hpte_init_pseries();
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ENABLED(CONFIG_PPC_HASH_MMU_NATIVE)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn hash__early_init_mmu_secondary() {
...
//
#[no_mangle]
pub unsafe extern "C" fn hash_page_do_lazy_icache(pp: c_uint, pte: pte_t, trap: c_int) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn get_paca_psize(addr: c_ulong) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn demote_segment_4k(mm: *mut mm_struct, addr: c_ulong) {
...
//
#[no_mangle]
unsafe extern "C" fn subpage_protection(mm: *mut mm_struct, ea: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn subpage_protection(mm: *mut mm_struct, ea: c_ulong) -> c_int {
...
// Get PTE and page size from page tables
    ...
    if (ptep == core::ptr::null_mut() || !pte_present(*ptep)) {
    DBG_LOW(" no PTE !\n");
    rc = 1;
    goto bail;
    }
...
//
    ...
    {
    let mut spp: c_int = subpage_protection(mm, ea);
    if (access & spp)
    rc = -2;
    else
...
//
    ...
    {
    let mut ea: c_ulong = regs.dar;
    let mut dsisr: c_ulong = regs.dsisr;
    let mut access: c_ulong = _PAGE_PRESENT | _PAGE_READ;
    let mut flags: c_ulong = 0;
    struct mm_struct *mm;
...
#[no_mangle]
unsafe extern "C" fn should_hash_preload(mm: *mut mm_struct, ea: c_ulong) -> bool {
    static bool should_hash_preload(struct mm_struct *mm, unsigned long ea)
    {
    let mut psize: c_int = get_slice_psize(mm, ea);
...
//
    ...
    let mut access: c_ulong = _PAGE_PRESENT | _PAGE_READ | (is_exec ? _PAGE_EXEC : 0);
    unsigned long flags;
...
#[no_mangle]
pub unsafe extern "C" fn tm_flush_hash_page(local: c_int) {
...
//
    ...
    let mut local: c_int = flags & HPTE_LOCAL_UPDATE;
...
#[no_mangle]
pub unsafe extern "C" fn flush_hash_range(number: c_ulong, local: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn hpt_clear_stress() {
    void hpt_clear_stress(void)
    {
...
#[no_mangle]
pub unsafe extern "C" fn hpt_do_stress(ea: c_ulong, hpte_group: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn hpt_order_get(data: *mut c_void, val: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hpt_order_set(data: *mut c_void, val: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hash64_debugfs() -> int __init {
    static int __init hash64_debugfs(void)
    {
    if (radix_enabled())
    return 0;
    debugfs_create_file("hpt_order", 0600, arch_debugfs_dir, core::ptr::null_mut(),
    &fops_hpt_order);
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(MMU_SEGSIZE_1T: !radix_enabled() && mmu_highuser_ssize ==) -> else {
