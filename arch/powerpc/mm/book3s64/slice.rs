...
//! Source: arch/powerpc/mm/book3s64/slice.c
#![no_std]
...
//
    ...
    let mut _slice_debug: c_int = 1;
#[no_mangle]
unsafe extern "C" fn slice_print_mask(label: *const c_char, mask: *const slice_mask) {
    ...
    static void slice_print_mask(const char *label, const struct slice_mask *mask) {}
// Macro flag: #define slice_dbg(fmt...)

#[no_mangle]
pub unsafe extern "C" fn slice_addr_is_low(addr: c_ulong) -> notrace bool {
    static inline notrace bool slice_addr_is_low(unsigned long addr)
    {
    let mut tmp: u64 = (u64)addr;
    return tmp < SLICE_LOW_TOP;
    }
    static void slice_range_to_mask(unsigned long start, unsigned long len,
    struct slice_mask *ret)
    {
    let mut end: c_ulong = start + len - 1;
    ret.low_slices = 0;
    if (SLICE_NUM_HIGH)
    ...
    let mut start_index: c_ulong = GET_HIGH_SLICE_INDEX(start);
    let mut align_end: c_ulong = ALIGN(end, (1UL << SLICE_HIGH_SHIFT));
    let mut count: c_ulong = GET_HIGH_SLICE_INDEX(align_end) - start_index;
...
#[no_mangle]
unsafe extern "C" fn slice_low_has_vma(mm: *mut mm_struct, slice: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn slice_high_has_vma(mm: *mut mm_struct, slice: c_ulong) -> c_int {
    static int slice_high_has_vma(struct mm_struct *mm, unsigned long slice)
    {
    let mut start: c_ulong = slice << SLICE_HIGH_SHIFT;
    let mut end: c_ulong = start + (1ul << SLICE_HIGH_SHIFT);
...
#[no_mangle]
unsafe extern "C" fn slice_flush_segments(parm: *mut c_void) {
...
// boundary_addr = (slice + end) ?
    ...
    {
    let mut pshift: c_int = max_t(int, mmu_psize_defs[psize].shift, PAGE_SHIFT);
...
//
    ...
    addr = high_limit;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: slice_scan_available(addr, _arg: available, _arg: 1, _arg: &next_end)) -> else {
    addr = next_end;
    goto next_slice;
    }
    ...
    let mut min_addr: c_ulong = max(PAGE_SIZE, mmap_min_addr);
//
...
//
    ...
    addr = min_addr;
#[no_mangle]
pub unsafe extern "C" fn if(1: slice_scan_available(addr -, _arg: available, _arg: 0, _arg: &prev)) -> else {
    addr = prev;
    goto prev_slice;
    }
    info.low_limit = addr;
...
//
    ...
    const struct slice_mask *compat_maskp = core::ptr::null_mut();
    let mut fixed: c_int = (flags & MAP_FIXED);
    let mut pshift: c_int = max_t(int, mmu_psize_defs[psize].shift, PAGE_SHIFT);
    let mut page_size: c_ulong = 1UL << pshift;
...
#[no_mangle]
unsafe extern "C" fn file_to_psize(file: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn get_slice_psize(mm: *mut mm_struct, addr: c_ulong) -> unsigned int notrace {
...
#[no_mangle]
pub unsafe extern "C" fn slice_init_new_context_exec(mm: *mut mm_struct) {
    void slice_init_new_context_exec(struct mm_struct *mm)
    {
    unsigned char *hpsizes, *lpsizes;
    struct slice_mask *mask;
    let mut psize: c_uint = mmu_virtual_psize;
...
#[no_mangle]
pub unsafe extern "C" fn slice_setup_new_exec() {
...
//
    ...
    {
    const struct slice_mask *maskp;
    let mut psize: c_uint = mm_ctx_user_psize(&mm.context);
    VM_BUG_ON(radix_enabled());
    maskp = slice_mask_for_size(&mm.context, psize);
...
#[no_mangle]
pub unsafe extern "C" fn vma_mmu_pagesize(vma: *mut vm_area_struct) -> c_ulong {
