...
//! Source: arch/powerpc/mm/book3s64/radix_pgtable.c
#![no_std]
...
//
    ...
    {
    let mut min_addr: phys_addr_t = MEMBLOCK_LOW_LIMIT;
    let mut max_addr: phys_addr_t = MEMBLOCK_ALLOC_ANYWHERE;
    void *ptr;
    if (region_start)
...
//
    ...
    unsigned long region_start, unsigned long region_end)
    {
    let mut pfn: c_ulong = pa >> PAGE_SHIFT;
    pgd_t *pgdp;
    p4d_t *p4dp;
    pud_t *pudp;
    pmd_t *pmdp;
...
#[no_mangle]
pub unsafe extern "C" fn radix__mark_rodata_ro() {
...
#[no_mangle]
pub unsafe extern "C" fn radix__mark_initmem_nx() {
    void radix__mark_initmem_nx(void)
    {
    let mut start: c_ulong = (unsigned long)__init_begin;
    let mut end: c_ulong = (unsigned long)__init_end;
...
#[no_mangle]
unsafe extern "C" fn next_boundary(addr: c_ulong, end: c_ulong) -> c_ulong {
...
// The end of interrupts code at zero is a rodata boundary
    let mut end_intr: c_ulong = __pa_symbol(__end_interrupts) - stext_phys;
    if (addr < end_intr)
    return end_intr;
// Start of relocated kernel text is a rodata boundary
    ...
    pgprot_t prot;
    int psize;
    let mut max_mapping_size: c_ulong = memory_block_size;
...
#[no_mangle]
unsafe extern "C" fn alloc_kfence_pool() -> __init phys_addr_t {
...
#[no_mangle]
unsafe extern "C" fn map_kfence_pool(kfence_pool: phys_addr_t) -> __init void {
...
#[no_mangle]
unsafe extern "C" fn radix_init_pgtable() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn radix_init_partition_table() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn radix__early_init_devtree() -> void __init {
...
//
    rc = of_scan_flat_dt(radix_dt_scan_page_sizes, core::ptr::null_mut());
    if (!rc) {
//
...
#[no_mangle]
pub unsafe extern "C" fn radix__early_init_mmu() -> void __init {
...
// Switch to the guard PID before turning on MMU
    radix__switch_mmu_context(core::ptr::null_mut(), &init_mm);
    tlbiel_all();
    }
#[no_mangle]
pub unsafe extern "C" fn radix__early_init_mmu_secondary() {
...
#[no_mangle]
pub unsafe extern "C" fn radix__mmu_cleanup_all() -> notrace void {
...
#[no_mangle]
unsafe extern "C" fn free_pte_table(pte_start: *mut pte_t, pmd: *mut pmd_t) {
...
#[no_mangle]
unsafe extern "C" fn free_pmd_table(pmd_start: *mut pmd_t, pud: *mut pud_t) {
...
#[no_mangle]
unsafe extern "C" fn free_pud_table(pud_start: *mut pud_t, p4d: *mut p4d_t) {
...
#[no_mangle]
unsafe extern "C" fn vmemmap_pmd_is_unused(addr: c_ulong, end: c_ulong) -> bool __meminit {
    static bool __meminit vmemmap_pmd_is_unused(unsigned long addr, unsigned long end)
    {
    let mut start: c_ulong = ALIGN_DOWN(addr, PMD_SIZE);
    return !vmemmap_populated(start, PMD_SIZE);
    }
#[no_mangle]
unsafe extern "C" fn vmemmap_page_is_unused(addr: c_ulong, end: c_ulong) -> bool __meminit {
    ...
    let mut base_pfn: c_ulong = page_to_pfn(page);
//
...
#[no_mangle]
pub unsafe extern "C" fn if(vmemmap_page_is_unused(addr: !direct &&, _arg: next)) -> else {
    free_vmemmap_pages(pte_page(*pte), altmap, 0);
    pte_clear(&init_mm, addr, pte);
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(vmemmap_pmd_is_unused(addr: !direct &&, _arg: next)) -> else {
    free_vmemmap_pages(pmd_page(*pmd), altmap, get_order(PMD_SIZE));
    pte_clear(&init_mm, addr, (pte_t *)pmd);
    }
...
#[no_mangle]
pub unsafe extern "C" fn radix__remove_section_mapping(start: c_ulong, end: c_ulong) -> int __meminit {
    int __meminit radix__remove_section_mapping(unsigned long start, unsigned long end)
    {
    remove_pagetable(start, end, true, core::ptr::null_mut());
    return 0;
    }
...
// Create a PTE encoding
    let mut nid: c_int = early_pfn_to_nid(phys >> PAGE_SHIFT);
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn vmemmap_can_optimize(altmap: *mut vmem_altmap, pgmap: *mut dev_pagemap) -> bool {
    ...
    {
    let mut large: c_int = pmd_leaf(*pmdp);
    if (large)
...
//
    ...
    if (!pte)
    return -ENOMEM;
    pte = radix__vmemmap_pte_populate(pmd, addr, node, altmap, core::ptr::null_mut());
    if (!pte)
    return -ENOMEM;
...
//
    ...
    if (!pte)
    return core::ptr::null_mut();
    radix__vmemmap_pte_populate(pmd, addr, node, core::ptr::null_mut(), core::ptr::null_mut());
    vmemmap_verify(pte, node, addr, addr + PAGE_SIZE);
    return pte;
    }
...
//
    pte = radix__vmemmap_populate_address(map_addr - PAGE_SIZE, node, core::ptr::null_mut(), core::ptr::null_mut());
    if (!pte)
    return core::ptr::null_mut();
//
// Populate the tail pages vmemmap page
//
    pte = radix__vmemmap_pte_populate(pmd, map_addr, node, core::ptr::null_mut(), core::ptr::null_mut());
    if (!pte)
    return core::ptr::null_mut();
...
//
    ...
    let mut addr_pfn: c_ulong = page_to_pfn((struct page *)addr);
    let mut pfn_offset: c_ulong = addr_pfn - ALIGN_DOWN(addr_pfn, nr_pages);
    pte_t *tail_page_pte;
//
...
//
    if (pfn_offset == 0) {
// Populate the head page vmemmap page
    pte = radix__vmemmap_pte_populate(pmd, addr, node, core::ptr::null_mut(), core::ptr::null_mut());
    if (!pte)
    return -ENOMEM;
    vmemmap_verify(pte, node, addr, addr + PAGE_SIZE);
//
...
// vmemmap_populate_address()
//
    pte = radix__vmemmap_populate_address(addr + PAGE_SIZE, node, core::ptr::null_mut(), core::ptr::null_mut());
    if (!pte)
    return -ENOMEM;
...
//
    ...
    pte = radix__vmemmap_pte_populate(pmd, addr, node, core::ptr::null_mut(), pte_page(*tail_page_pte));
    if (!pte)
    return -ENOMEM;
    vmemmap_verify(pte, node, addr, addr + PAGE_SIZE);
    next = addr + PAGE_SIZE;
...
#[no_mangle]
pub unsafe extern "C" fn radix__vmemmap_remove_mapping(start: c_ulong, page_size: c_ulong) -> void __meminit {
    void __meminit radix__vmemmap_remove_mapping(unsigned long start, unsigned long page_size)
    {
    remove_pagetable(start, start + page_size, true, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn radix__pgtable_trans_huge_withdraw(mm: *mut mm_struct, pmdp: *mut pmd_t) -> pgtable_t {
...
// ptep = __pte(0);
    ...
    let mut change: c_ulong = pte_val(entry) ^ pte_val(*ptep);
//
...
pub unsafe extern "C" fn pud_set_huge(pud: *mut pud_t, addr: phys_addr_t, prot: pgprot_t) -> c_int {
    ...
    let mut new_pud: pte_t = pfn_pte(__phys_to_pfn(addr), prot);
    if (!radix_enabled())
    return 0;
...
pub unsafe extern "C" fn pmd_set_huge(pmd: *mut pmd_t, addr: phys_addr_t, prot: pgprot_t) -> c_int {
    ...
    let mut new_pmd: pte_t = pfn_pte(__phys_to_pfn(addr), prot);
    if (!radix_enabled())
    return 0;
