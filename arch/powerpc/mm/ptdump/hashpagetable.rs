...
//! Source: arch/powerpc/mm/ptdump/hashpagetable.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn calculate_pagesize(st: *mut pg_state, ps: c_int, s[]: c_char) -> c_int {
...
// flag, u64 pte, int num)
    ...
    const char *s = core::ptr::null_mut();
    u64 val;
...
// r)
    ...
    let mut shift: c_ulong = mmu_psize_defs[psize].shift;
// calculate hash
...
#[no_mangle]
unsafe extern "C" fn pseries_find(ea: c_ulong, psize: c_int, primary: bool, v: *mut u64, r: *mut u64) -> c_int {
...
// r = ptes[j].r;
    ...
    let mut penc: c_int = -2, idx = 0, shift;
// .
...
#[no_mangle]
unsafe extern "C" fn hpte_find(st: *mut pg_state, ea: c_ulong, psize: c_int) -> c_ulong {
    static unsigned long hpte_find(struct pg_state *st, unsigned long ea, int psize)
    {
    unsigned long slot;
    let mut v: u64 = 0, r = 0;
    unsigned long rpn, lp_bits;
    let mut base_psize: c_int = 0, actual_psize = 0;
    if (ea < PAGE_OFFSET)
    return -1;
...
#[no_mangle]
unsafe extern "C" fn walk_pte(st: *mut pg_state, pmd: *mut pmd_t, start: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn walk_pmd(st: *mut pg_state, pud: *mut pud_t, start: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn walk_pud(st: *mut pg_state, p4d: *mut p4d_t, start: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn walk_p4d(st: *mut pg_state, pgd: *mut pgd_t, start: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn walk_pagetables(st: *mut pg_state) {
...
#[no_mangle]
unsafe extern "C" fn walk_linearmapping(st: *mut pg_state) {
...
//
    let mut psize: c_ulong = 1 << mmu_psize_defs[mmu_linear_psize].shift;
    for (addr = PAGE_OFFSET; addr < PAGE_OFFSET +
    memblock_end_of_DRAM(); addr += psize)
    hpte_find(st, addr, mmu_linear_psize);
    }
#[no_mangle]
unsafe extern "C" fn walk_vmemmap(st: *mut pg_state) {
...
#[no_mangle]
unsafe extern "C" fn populate_markers() {
...
#[no_mangle]
unsafe extern "C" fn ptdump_init() -> c_int {
    static int ptdump_init(void)
    {
    if (!radix_enabled()) {
    populate_markers();
    debugfs_create_file("kernel_hash_pagetable", 0400, core::ptr::null_mut(), core::ptr::null_mut(),
    &ptdump_fops);
    }
    return 0;
    }
