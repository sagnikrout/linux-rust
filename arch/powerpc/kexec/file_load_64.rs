...
//! Source: arch/powerpc/kexec/file_load_64.c
#![no_std]
...
pub struct umem_info {
    pub /: *mut *mut *mut __be64 buf; / data buffer for usable-memory property,
    pub /: *mut *mut u32 size; / size allocated for the data buffer,
    pub /: *mut *mut u32 max_entries; / maximum no. of entries,
    pub /: *mut *mut u32 idx; / index of current entry,
...
//
#[no_mangle]
unsafe extern "C" fn add_usable_mem(um_info: *mut umem_info, base: u64, end: u64) -> c_int {
    ...
    add = true;
#[no_mangle]
pub unsafe extern "C" fn if(loc_base: base < loc_end && end >) -> else {
    if (loc_base < base)
    loc_base = base;
    if (loc_end > end)
    loc_end = end;
...
pub const NODE_PATH_LEN: c_int = 256;
//
...
//
#[no_mangle]
unsafe extern "C" fn update_usable_mem_fdt(fdt: *mut c_void, usable_mem: *mut crash_mem) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: node <) -> else {
    pr_err("Malformed device tree: error reading /ibm,dynamic-reconfiguration-memory.\n");
    return -EINVAL;
    }
    um_info.buf  = core::ptr::null_mut();
    um_info.size = 0;
...
//
#[no_mangle]
unsafe extern "C" fn load_backup_segment(image: *mut kimage, kbuf: *mut kexec_buf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn kdump_extra_elfcorehdr_size(cmem: *mut crash_mem) -> c_uint {
    static unsigned int kdump_extra_elfcorehdr_size(struct crash_mem *cmem)
    {

    let mut extra_sz: c_uint = 0;
    if (CONFIG_CRASH_MAX_MEMORY_RANGES > (unsigned int)PN_XNUM)
    pr_warn("Number of Phdrs %u exceeds max\n", CONFIG_CRASH_MAX_MEMORY_RANGES);
#[no_mangle]
pub unsafe extern "C" fn if(CONFIG_CRASH_MAX_MEMORY_RANGES: cmem->nr_ranges >=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn load_elfcorehdr_segment(image: *mut kimage, kbuf: *mut kexec_buf) -> c_int {
    static int load_elfcorehdr_segment(struct kimage *image, struct kexec_buf *kbuf)
    {
    struct crash_mem *cmem = core::ptr::null_mut();
    unsigned long headers_sz;
    void *headers = core::ptr::null_mut();
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn cpu_node_size() -> c_uint {
...
//
    if (size)
    return size;
    dn = of_find_node_by_type(core::ptr::null_mut(), "cpu");
    if (WARN_ON_ONCE(!dn)) {
// Unlikely to happen
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn kdump_extra_fdt_size_ppc64(image: *mut kimage, cpu_nodes: c_uint) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn kexec_extra_fdt_size_ppc64(image: *mut kimage, rmem: *mut crash_mem) -> c_uint {
    unsigned int kexec_extra_fdt_size_ppc64(struct kimage *image, struct crash_mem *rmem)
    {
    struct device_node *dn;
    let mut cpu_nodes: c_uint = 0, extra_size = 0;
...
// Consider extra space for reserved memory ranges if any
    ...
    let mut len: c_int = 0, fdtlen = 0;
    prop = of_get_property(dn, propname, &len);
    fdtprop = fdt_getprop(fdt, node_offset, propname, &fdtlen);
    if (fdtprop && !prop)
    return fdt_delprop(fdt, node_offset, propname);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: prop) -> else {
    else if (prop)
...
#[no_mangle]
unsafe extern "C" fn update_pci_dma_nodes(fdt: *mut c_void, dmapropname: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn setup_new_fdt_ppc64(image: *const kimage, fdt: *mut c_void, rmem: *mut crash_mem) -> c_int {
    int setup_new_fdt_ppc64(const struct kimage *image, void *fdt, struct crash_mem *rmem)
    {
    struct crash_mem *umem = core::ptr::null_mut();
    int i, nr_ranges, ret;
...
pub unsafe extern "C" fn arch_kimage_file_post_load_cleanup(image: *mut kimage) -> c_int {
    ...
    image.arch.exclude_ranges = core::ptr::null_mut();
    vfree(image.arch.backup_buf);
    image.arch.backup_buf = core::ptr::null_mut();
