...
//! Source: arch/powerpc/perf/8xx-pmu.c
#![no_std]
...
pub const PERF_8xx_ID_CPU_CYCLES: c_int = 1;
pub const PERF_8xx_ID_HW_INSTRUCTIONS: c_int = 2;
pub const PERF_8xx_ID_ITLB_LOAD_MISS: c_int = 3;
pub const PERF_8xx_ID_DTLB_LOAD_MISS: c_int = 4;
...
#[no_mangle]
unsafe extern "C" fn get_insn_ctr() -> i64 {
...
#[no_mangle]
unsafe extern "C" fn event_type(event: *mut perf_event) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mpc8xx_pmu_event_init(event: *mut perf_event) -> c_int {
    static int mpc8xx_pmu_event_init(struct perf_event *event)
    {
    let mut type: c_int = event_type(event);
    if (type < 0)
    return type;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn mpc8xx_pmu_add(event: *mut perf_event, flags: c_int) -> c_int {
    ...
    let mut target: c_ulong = patch_site_addr(&patch__itlbmiss_perf);
    ...
    let mut target: c_ulong = patch_site_addr(&patch__dtlbmiss_perf);
...
#[no_mangle]
unsafe extern "C" fn mpc8xx_pmu_read(event: *mut perf_event) {
...
#[no_mangle]
unsafe extern "C" fn mpc8xx_pmu_del(event: *mut perf_event, flags: c_int) {
    static void mpc8xx_pmu_del(struct perf_event *event, int flags)
    {
    let mut insn: ppc_inst_t = ppc_inst(PPC_RAW_MFSPR(10, SPRN_SPRG_SCRATCH2));
...
#[no_mangle]
unsafe extern "C" fn init_mpc8xx_pmu() -> c_int {
