...
//! Source: arch/powerpc/perf/power6-pmu.c
#![no_std]
...
pub const PM_PMC_MSK: c_uint = 0x7;

pub const PM_UNIT_MSK: c_uint = 0xf;

pub const PM_LLAV: c_uint = 0x8000	/* Load lookahead match value */;
pub const PM_LLA: c_uint = 0x4000	/* Load lookahead match enable */;

pub const PM_BYTE_MSK: c_int = 3;

pub const PM_SUBUNIT_MSK: c_int = 7;

pub const PM_PMCSEL_MSK: c_uint = 0xff	/* PMCxSEL value */;
pub const PM_BUSEVENT_MSK: c_uint = 0xf3700;
//
...
pub const MMCR1_NESTSEL_SH: c_int = 45;
pub const MMCR1_NESTSEL_MSK: c_uint = 0x7;
...
//
#[no_mangle]
unsafe extern "C" fn power6_marked_instr_event(event: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn p6_limited_pmc_event(event: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn find_alternatives_list(event: u64) -> c_int {
    ...
    let mut hi: c_int = ARRAY_SIZE(presorted_event_table) - 1;
    let mut lo: c_int = 0;
    while (lo <= hi) {
    let mut mid: c_int = lo + (hi - lo) / 2;
    let mut alt: c_uint = presorted_event_table[mid];
    if (alt < event)
    lo = mid + 1;
#[no_mangle]
pub unsafe extern "C" fn if(event: alt >) -> else {
    else if (alt > event)
    hi = mid - 1;
    else
...
#[no_mangle]
unsafe extern "C" fn p6_get_alternatives(event: u64, flags: c_uint, alt[]: u64) -> c_int {
    static int p6_get_alternatives(u64 event, unsigned int flags, u64 alt[])
    {
    int i, j, nlim;
    unsigned int psel, pmc;
    let mut nalt: c_uint = 1;
    u64 aevent;
    alt[0] = event;
...
#[no_mangle]
unsafe extern "C" fn p6_disable_pmc(pmc: c_uint, mmcr: *mut mmcr_regs) {
...
#[no_mangle]
pub unsafe extern "C" fn init_power6_pmu() -> int __init {
