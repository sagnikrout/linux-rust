...
//! Source: arch/powerpc/perf/vpa-dtl.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct vpa_dtl {
    pub buf: *mut dtl_entry,
    pub last_idx: u64,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct vpa_pmu_ctx {
    pub handle: perf_output_handle,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct vpa_pmu_buf {
    pub nr_pages: c_int,
    pub snapshot: bool,
    pub base: *mut u64,
    pub size: u64,
...
// variable to capture reference count for the active dtl threads
    ...
    let mut dtl_global_lock: static spinlock_t = __SPIN_LOCK_UNLOCKED(dtl_global_lock);
//
...
//
#[no_mangle]
unsafe extern "C" fn vpa_dtl_dump_sample_data(event: *mut perf_event) {
...
// actual number of entries read
    let mut n_read: c_long = 0, read_size = 0;
...
//
#[no_mangle]
unsafe extern "C" fn vpa_dtl_hrtimer_handle(hrtimer: *mut hrtimer) -> enum hrtimer_restart {
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_start_hrtimer(event: *mut perf_event) {
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_stop_hrtimer(event: *mut perf_event) {
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_reset_global_refc(event: *mut perf_event) {
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_mem_alloc(cpu: c_int) -> c_int {
    static int vpa_dtl_mem_alloc(int cpu)
    {
    struct vpa_dtl *dtl = &per_cpu(vpa_dtl_cpu, cpu);
    struct dtl_entry *buf = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_event_init(event: *mut perf_event) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_event_add(event: *mut perf_event, flags: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_event_del(event: *mut perf_event, flags: c_int) {
    static void vpa_dtl_event_del(struct perf_event *event, int flags)
    {
    let mut hwcpu: c_int = get_hard_smp_processor_id(event.cpu);
    struct vpa_dtl *dtl = &per_cpu(vpa_dtl_cpu, event.cpu);
...
//
#[no_mangle]
unsafe extern "C" fn vpa_dtl_event_read(event: *mut perf_event) {
...
//
    ...
    {
    int i, cpu = event.cpu;
    struct vpa_pmu_buf *buf __free(kfree) = core::ptr::null_mut();
    struct page **pglist __free(kfree) = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn vpa_dtl_free_aux(aux: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn vpa_dtl_init() -> c_int {
