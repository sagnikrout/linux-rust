...
//! Source: arch/powerpc/boot/main.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn prep_kernel() -> addr_range {
    static struct addr_range prep_kernel(void)
    {
    char elfheader[256];
    unsigned char *vmlinuz_addr = (unsigned char *)_vmlinux_start;
    let mut vmlinuz_size: c_ulong = _vmlinux_end - _vmlinux_start;
    void *addr = 0;
    struct elf_info ei;
    long len;
    let mut uncompressed_image: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn prep_esm_blob(vmlinux: addr_range, chosen: *mut c_void) {
...
unsafe extern "C" fn prep_cmdline(chosen: *mut c_void) {
    ...
    {
    let mut getline_timeout: c_uint = 5000;
    int v;
    int n;
// Wait-for-input time
...
// Ensure that the device tree has a /chosen node
    ...
    if (!chosen)
    chosen = create_node(core::ptr::null_mut(), "chosen");
    vmlinux = prep_kernel();
    ...
    kentry(ft_addr, 0, core::ptr::null_mut());
    }
    else
