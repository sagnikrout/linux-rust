...
//! Source: arch/powerpc/boot/cuboot-pq2.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn update_cs_ranges() {
    ...
    u32 base, option;
    let mut cs: c_int = cs_ranges_buf[i].csnum;
    if (cs >= ctrl_size / 8)
    goto err;
...
//
#[no_mangle]
unsafe extern "C" fn fixup_pci() {
    static void fixup_pci(void)
    {
    struct pci_range *mem = core::ptr::null_mut(), *mmio = core::ptr::null_mut(),
// io = NULL, *mem_base = NULL;
    ...
    (unsigned long *)&pci_regs[i], core::ptr::null_mut()))
    goto err;
    ...
    let mut flags: u32 = pci_ranges_buf[i].flags & 0x43000000;
    if (flags == 0x42000000)
    mem = &pci_ranges_buf[i];
#[no_mangle]
pub unsafe extern "C" fn if(0x02000000: flags ==) -> else {
    else if (flags == 0x02000000)
    mmio = &pci_ranges_buf[i];
#[no_mangle]
pub unsafe extern "C" fn if(0x01000000: flags ==) -> else {
    ...
    mem_base = mem;
#[no_mangle]
pub unsafe extern "C" fn if(mem->phys_addr: mmio->phys_addr + mmio->size[1] ==) -> else {
