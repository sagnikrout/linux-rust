...
//! Source: arch/loongarch/kernel/module.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn rela_stack_push(stack_value: c_long, rela_stack: *mut c_long, rela_stack_top: *mut usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rela_stack_pop(stack_value: *mut c_long, rela_stack: *mut c_long, rela_stack_top: *mut usize) -> c_int {
...
// (Elf_Addr *)location = v;
    ...
    long *rela_stack, size_t *rela_stack_top, unsigned int type)
    {
    let mut offset: ptrdiff_t = (void *)v - (void *)location;
    if (offset >= SZ_128M)
...
// Use s32 for a sign-extension deliberately.
    let mut offset_hi20: i32 = (void *)((v + 0x800)) - (void *)((Elf_Addr)location);
    switch (type) {
...
// Use s32 for a sign-extension deliberately.
    ...
    let mut anchor: Elf_Addr = (((Elf_Addr)location) & ~0xfff) + offset_hi20;
    let mut offset_rem: ptrdiff_t = (void *)v - (void *)anchor;
...
// The handlers for known reloc types
    ...
    let mut rela_stack_top: usize = 0;
...
// Ignore unresolved weak symbol
    ...
    let mut j: c_uint = idx;
    do {
    let mut hi20_type: u32 = ELF_R_TYPE(rel[j].r_info);
...
// Find the corresponding HI20 relocation entry
    ...
    let mut hi20_sym_val: c_ulong = hi20_sym.st_value + rel[j].r_addend;
// Calculate LO12 offset
    let mut offset: usize = hi20_sym_val - hi20_location;
    ...
    const Elf_Shdr *s, *alt = core::ptr::null_mut(), *orc = core::ptr::null_mut(), *orc_ip = core::ptr::null_mut(), *ftrace = core::ptr::null_mut();
