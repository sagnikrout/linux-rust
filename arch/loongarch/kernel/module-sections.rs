...
//! Source: arch/loongarch/kernel/module-sections.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn module_emit_got_entry(mod: *mut module, sechdrs: *mut Elf_Shdr, val: Elf_Addr) -> Elf_Addr {
...
#[no_mangle]
pub unsafe extern "C" fn module_emit_plt_entry(mod: *mut module, sechdrs: *mut Elf_Shdr, val: Elf_Addr) -> Elf_Addr {
...
unsafe extern "C" fn compare_rela(x: *const c_void, y: *const c_void) -> c_int {
    ...
    {
    unsigned int i;
    sort(relas, num, sizeof(Elf_Rela), compare_rela, core::ptr::null_mut());
    for (i = 0; i < num; i++) {
    ...
    Elf_Shdr *got_sec, *plt_sec, *plt_idx_sec, *tramp = core::ptr::null_mut();
//
...
#[no_mangle]
pub unsafe extern "C" fn if(sechdrs[i].sh_name: !strcmp(secstrings +, _arg: ".plt")) -> else {
    else if (!strcmp(secstrings + sechdrs[i].sh_name, ".plt"))
    mod.arch.plt.shndx = i;
#[no_mangle]
pub unsafe extern "C" fn if(sechdrs[i].sh_name: !strcmp(secstrings +, _arg: ".plt.idx")) -> else {
    else if (!strcmp(secstrings + sechdrs[i].sh_name, ".plt.idx"))
    mod.arch.plt_idx.shndx = i;
#[no_mangle]
pub unsafe extern "C" fn if(sechdrs[i].sh_name: !strcmp(secstrings +, _arg: ".ftrace_trampoline")) -> else {
...
// Calculate the maxinum number of entries
    for (i = 0; i < ehdr.e_shnum; i++) {
    let mut num_rela: c_int = sechdrs[i].sh_size / sizeof(Elf_Rela);
