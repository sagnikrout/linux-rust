...
//! Source: arch/loongarch/kernel/setup.c
#![no_std]
...
pub const SMBIOS_BIOSSIZE_OFFSET: c_uint = 0x09;
pub const SMBIOS_BIOSEXTERN_OFFSET: c_uint = 0x13;
pub const SMBIOS_FREQLOW_OFFSET: c_uint = 0x16;
pub const SMBIOS_FREQHIGH_OFFSET: c_uint = 0x17;
pub const SMBIOS_FREQLOW_MASK: c_uint = 0xFF;
pub const SMBIOS_CORE_PACKAGE_OFFSET: c_uint = 0x23;
pub const SMBIOS_THREAD_PACKAGE_OFFSET: c_uint = 0x25;
pub const SMBIOS_THREAD_PACKAGE_2_OFFSET: c_uint = 0x2E;
...
//
    ...
    let mut code_resource: static struct resource = { .name = "Kernel code", };
    let mut data_resource: static struct resource = { .name = "Kernel data", };
    let mut bss_resource: static struct resource = { .name = "Kernel bss", };
...
pub unsafe extern "C" fn arch_cpu_finalize_init() -> void __init {
    ...
    if (*bp != 0) {
    let mut len: usize = strlen(bp)+1;
    let mut cmp_len: usize = len > 8 ? 8 : len;
...
#[no_mangle]
unsafe extern "C" fn parse_cpu_table(dm: *const dmi_header) -> void __init {
    static void __init parse_cpu_table(const struct dmi_header *dm)
    {
    let mut freq_temp: c_long = 0;
...
#[no_mangle]
unsafe extern "C" fn parse_bios_table(dm: *const dmi_header) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn find_tokens(dm: *const dmi_header, dummy: *mut c_void) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn smbios_parse() -> void __init {
    ...
    dmi_walk(find_tokens, core::ptr::null_mut());
    }

    let mut wc_enabled: bool = true;

    let mut wc_enabled: bool = false;

    EXPORT_SYMBOL(wc_enabled);
#[no_mangle]
unsafe extern "C" fn setup_writecombine(p: *mut c_char) -> int __init {
    static int __init setup_writecombine(char *p)
    {
    if (!strcmp(p, "on"))
    wc_enabled = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(p, _arg: "off")) -> else {
...
#[no_mangle]
unsafe extern "C" fn arch_reserve_vmcore() -> void __init {
...
unsafe extern "C" fn arch_reserve_crashkernel() -> void __init {
    ...
    {
    int ret;
    let mut low_size: c_ulonglong = 0;
    unsigned long long crash_base, crash_size;
    let mut high: bool = false;
    if (!IS_ENABLED(CONFIG_CRASH_RESERVE))
    return;
    ret = parse_crashkernel(boot_command_line, memblock_phys_mem_size(),
    &crash_size, &crash_base, &low_size, core::ptr::null_mut(), &high);
    if (ret)
    return;
    reserve_crashkernel_generic(crash_size, crash_base, low_size, high);
    }
#[no_mangle]
unsafe extern "C" fn fdt_setup() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn bootcmdline_init(cmdline_p: *mut c_char) -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn platform_init() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn check_kernel_sections_mem() -> void __init {
    static void __init check_kernel_sections_mem(void)
    {
    let mut start: phys_addr_t = __pa_symbol(&_text);
    let mut size: phys_addr_t = __pa_symbol(&_end) - start;
...
//
#[no_mangle]
unsafe extern "C" fn arch_mem_init(cmdline_p: *mut c_char) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn resource_init() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn arch_reserve_pio_range() -> __init int {
...
#[no_mangle]
unsafe extern "C" fn prefill_possible_map() -> void __init {
