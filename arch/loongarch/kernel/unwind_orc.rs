...
//! Source: arch/loongarch/kernel/unwind_orc.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn orc_sort_swap(_a: *mut c_void, _b: *mut c_void, size: c_int) {
    static void orc_sort_swap(void *_a, void *_b, int size)
    {
    let mut delta: c_int = _b - _a;
    int *a = _a, *b = _b, tmp;
...
#[no_mangle]
pub unsafe extern "C" fn unwind_init() -> void __init {
    void __init unwind_init(void)
    {
    int i;
    let mut orc_size: usize = (void *)__stop_orc_unwind - (void *)__start_orc_unwind;
    let mut orc_ip_size: usize = (void *)__stop_orc_unwind_ip - (void *)__start_orc_unwind_ip;
    let mut num_entries: usize = orc_ip_size / sizeof(int);
...
#[no_mangle]
pub unsafe extern "C" fn on_stack(info: *mut stack_info, addr: c_ulong, len: usize) -> bool {
    static inline bool on_stack(struct stack_info *info, unsigned long addr, size_t len)
    {
    let mut begin: c_ulong = info.begin;
    let mut end: c_ulong = info.end;
    return (info.type != STACK_TYPE_UNKNOWN &&
    addr >= begin && addr < end && addr + len > begin && addr + len <= end);
    }
#[no_mangle]
unsafe extern "C" fn stack_access_ok(state: *mut unwind_state, addr: c_ulong, len: usize) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn bt_address(ra: c_ulong) -> c_ulong {
    static inline unsigned long bt_address(unsigned long ra)
    {

    int cpu;
    let mut vec_sz: c_int = sizeof(exception_handlers);
    ...
    unsigned long func;
    let mut type: c_ulong = (ra - eentry) / VECSIZE;
    let mut offset: c_ulong = (ra - eentry) % VECSIZE;
    switch (type) {
