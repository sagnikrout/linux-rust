...
//! Source: arch/loongarch/pci/acpi.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn acpi_release_root_info(ci: *mut acpi_pci_root_info) {
...
#[no_mangle]
unsafe extern "C" fn acpi_prepare_root_resources(ci: *mut acpi_pci_root_info) -> c_int {
    static int acpi_prepare_root_resources(struct acpi_pci_root_info *ci)
    {
    int status;
    let mut pci_h: c_ulonglong = 0;
    struct resource_entry *entry, *tmp;
    struct acpi_device *device = ci.bridge;
    acpi_remove_early_pio();
    status = acpi_pci_probe_root_resources(ci);
    if (status > 0) {
    acpi_evaluate_integer(device.handle, "PCIH", core::ptr::null_mut(), &pci_h);
    if (pci_h)
    return status;
