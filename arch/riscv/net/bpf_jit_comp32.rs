...
//! Source: arch/riscv/net/bpf_jit_comp32.c
#![no_std]
...
pub const NR_SAVED_REGISTERS: c_int = 9;
...
#[no_mangle]
unsafe extern "C" fn hi(r: *const i8) -> i8 {
    static s8 hi(const s8 *r)
    {
    return r[0];
    }
#[no_mangle]
unsafe extern "C" fn lo(r: *const i8) -> i8 {
    static s8 lo(const s8 *r)
    {
    return r[1];
    }
#[no_mangle]
unsafe extern "C" fn emit_imm(rd: i8, imm: i32, ctx: *mut rv_jit_context) {
    static void emit_imm(const s8 rd, s32 imm, struct rv_jit_context *ctx)
    {
    let mut upper: u32 = (imm + (1 << 11)) >> 12;
    let mut lower: u32 = imm & 0xfff;
    if (upper) {
...
#[no_mangle]
unsafe extern "C" fn emit_imm32(rd: *const i8, imm: i32, ctx: *mut rv_jit_context) {
...
unsafe extern "C" fn __build_epilogue(is_tail_call: bool, ctx: *mut rv_jit_context) {
    ...
    {
    let mut stack_adjust: c_int = ctx.stack_size;
...
#[no_mangle]
unsafe extern "C" fn is_stacked(reg: i8) -> bool {
...
// Do nothing.
    ...
    let mut is_signed: bool = insn.off == 1;
    switch (op) {
    case BPF_MOV:
    emit(rv_addi(lo(rd), lo(rs), 0), ctx);
    break;
...
#[no_mangle]
unsafe extern "C" fn emit_bcc(op: u8, rd: u8, rs: u8, rvoff: c_int, ctx: *mut rv_jit_context) -> c_int {
    static int emit_bcc(u8 op, u8 rd, u8 rs, int rvoff, struct rv_jit_context *ctx)
    {
    int e, s = ctx.ninsns;
    let mut far: bool = false;
    int off;
    if (op == BPF_JSET) {
//
...
#[no_mangle]
unsafe extern "C" fn emit_call(fixed: bool, addr: u64, ctx: *mut rv_jit_context) {
    static void emit_call(bool fixed, u64 addr, struct rv_jit_context *ctx)
    {
    const s8 *r0 = bpf2rv32[BPF_REG_0];
    const s8 *r5 = bpf2rv32[BPF_REG_5];
    let mut upper: u32 = ((u32)addr + (1 << 11)) >> 12;
    let mut lower: u32 = addr & 0xfff;
...
// Epilogue jumps to *(t0 + 4).
    ...
    let mut is_fetch: bool = (imm & BPF_FETCH) || (imm == BPF_XCHG);
    let mut fetch_reg: i8 = is_fetch ? lo(rs) : RV_REG_ZERO;
    let mut aq: c_int = is_fetch ? 1 : 0;
    let mut rl: c_int = is_fetch ? 1 : 0;
    switch (imm) {
    case BPF_ADD:
...
#[no_mangle]
unsafe extern "C" fn emit_rev16(rd: i8, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn emit_rev32(rd: i8, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn emit_zext64(dst: *const i8, ctx: *mut rv_jit_context) {
...
// Swap each half.
    ...
    let mut imm_lo: i32 = imm;
    let mut imm_hi: i32 = insn1.imm;
    const s8 *rd = bpf_get_reg64(dst, tmp1, ctx);
