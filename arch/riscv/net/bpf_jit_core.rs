...
//! Source: arch/riscv/net/bpf_jit_core.c
#![no_std]
...
pub const NR_JIT_ITERATIONS: c_int = 32;
#[no_mangle]
unsafe extern "C" fn build_body(ctx: *mut rv_jit_context, extra_pass: bool, offset: *mut c_int) -> c_int {
...
pub unsafe extern "C" fn bpf_jit_needs_zext() -> bool {
    ...
    {
    let mut prog_size: c_uint = 0, extable_size = 0;
    let mut extra_pass: bool = false;
    let mut pass: c_int = 0, prev_ninsns = 0, i;
    ...
    if (!ctx.offset)
    goto out_offset;
    if (build_body(ctx, extra_pass, core::ptr::null_mut()))
    goto out_offset;
    for (i = 0; i < prog.len; i++) {
    prev_ninsns += 32;
