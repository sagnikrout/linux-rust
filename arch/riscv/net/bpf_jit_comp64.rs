...
//! Source: arch/riscv/net/bpf_jit_comp64.c
#![no_std]
...
pub const RV_MAX_REG_ARGS: c_int = 8;
pub const RV_FENTRY_NINSNS: c_int = 2;

// imm that allows emit_imm to emit max count insns
pub const RV_MAX_COUNT_IMM: c_uint = 0x7FFF7FF7FF7FF7FF;
...
#[no_mangle]
unsafe extern "C" fn bpf_to_rv_reg(bpf_reg: c_int, ctx: *mut rv_jit_context) -> u8 {
    static u8 bpf_to_rv_reg(int bpf_reg, struct rv_jit_context *ctx)
    {
    let mut reg: u8 = regmap[bpf_reg];
    switch (reg) {
...
#[no_mangle]
unsafe extern "C" fn seen_reg(reg: c_int, ctx: *mut rv_jit_context) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mark_fp(ctx: *mut rv_jit_context) {
    static void mark_fp(struct rv_jit_context *ctx)
    {
    __set_bit(RV_CTX_F_SEEN_S5, &ctx.flags);
    }
#[no_mangle]
unsafe extern "C" fn mark_call(ctx: *mut rv_jit_context) {
    static void mark_call(struct rv_jit_context *ctx)
    {
    __set_bit(RV_CTX_F_SEEN_CALL, &ctx.flags);
    }
#[no_mangle]
unsafe extern "C" fn is_32b_int(val: i64) -> bool {
    static bool is_32b_int(s64 val)
    {
    return -(1L << 31) <= val && val < (1L << 31);
    }
#[no_mangle]
unsafe extern "C" fn in_auipc_jalr_range(val: i64) -> bool {
...
#[no_mangle]
unsafe extern "C" fn emit_sextw_alt(rd: *mut u8, ra: u8, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn emit_zextw_alt(rd: *mut u8, ra: u8, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn emit_addr(rd: u8, addr: u64, extra_pass: bool, ctx: *mut rv_jit_context) -> c_int {
...
//
    let mut ip: u64 = (u64)(ctx.ro_insns + ctx.ninsns);
    let mut off: i64 = addr - ip;
    let mut upper: i64 = (off + (1 << 11)) >> 12;
    let mut lower: i64 = off & 0xfff;
...
#[no_mangle]
unsafe extern "C" fn emit_imm(rd: u8, val: i64, ctx: *mut rv_jit_context) {
...
//
    let mut lower: i64 = ((val & 0xfff) << 52) >> 52;
    int shift;
    if (is_32b_int(val)) {
    if (upper)
...
#[no_mangle]
unsafe extern "C" fn __build_epilogue(is_tail_call: bool, ctx: *mut rv_jit_context) {
    static void __build_epilogue(bool is_tail_call, struct rv_jit_context *ctx)
    {
    let mut stack_adjust: c_int = ctx.stack_size, store_offset = stack_adjust - 8;
...
#[no_mangle]
unsafe extern "C" fn emit_bpf_tail_call(insn: c_int, ctx: *mut rv_jit_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_signed_bpf_cond(cond: u8) -> bool {
...
#[no_mangle]
unsafe extern "C" fn emit_call(addr: u64, fixed_addr: bool, ctx: *mut rv_jit_context) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn emit_kcfi(hash: u32, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn emit_stx_insn(rd: u8, off: i16, rs: u8, size: u8, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn emit_st(rd: u8, off: i16, imm: i32, size: u8, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn emit_stx(rd: u8, off: i16, rs: u8, size: u8, ctx: *mut rv_jit_context) {
...
// store_release(dst_reg + off16, src_reg)
    ...
    let mut off: i16 = insn.off;
    let mut imm: i32 = insn.imm;
    let mut is64: bool = BPF_SIZE(code) == BPF_DW;
...
//
#[no_mangle]
unsafe extern "C" fn sign_extend(rd: u8, rs: u8, sz: u8, sign: bool, ctx: *mut rv_jit_context) -> c_int {
    ...
    struct pt_regs *regs)
    {
    let mut offset: off_t = FIELD_GET(BPF_FIXUP_OFFSET_MASK, ex.fixup);
    let mut regs_offset: c_int = FIELD_GET(BPF_FIXUP_REG_MASK, ex.fixup);
...
#[no_mangle]
unsafe extern "C" fn gen_jump_or_nops(target: *mut c_void, ip: *mut c_void, insns: *mut u32, is_call: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn store_args(nr_arg_slots: c_int, args_off: c_int, ctx: *mut rv_jit_context) {
...
#[no_mangle]
unsafe extern "C" fn restore_args(nr_reg_args: c_int, args_off: c_int, ctx: *mut rv_jit_context) {
...
// update branch with beqz
    if (ctx.insns) {
    let mut offset: c_int = ninsns_rvoff(ctx.ninsns - branch_off);
    let mut insn: u32 = rv_beq(RV_REG_A0, RV_REG_ZERO, offset >> 1);
...
// arg3: &run_ctx
    ...
    {
    int i, ret, offset;
    int *branches_off = core::ptr::null_mut();
    let mut stack_size: c_int = 0, nr_arg_slots = 0;
...
// trampoline called directly
    ...
    int ret;
    ctx.ninsns = 0;
    ctx.insns = core::ptr::null_mut();
    ctx.ro_insns = core::ptr::null_mut();
...
pub unsafe extern "C" fn arch_free_bpf_trampoline(image: *mut c_void, size: c_uint) {
    ...
    let mut rd: u8 = -1, rs = -1, code = insn.code;
    let mut off: i16 = insn.off;
    let mut imm: i32 = insn.imm;
...
// Implement helper call to bpf_get_current_task/_btf() inline
    ...
    let mut reg: u8 = bpf_to_rv_reg(BPF_REG_1 + idx, ctx);
    let mut sign: bool = fm.arg_flags[idx] & BTF_FMODEL_SIGNED_ARG;
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_build_prologue(ctx: *mut rv_jit_context, is_subprog: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_build_epilogue(ctx: *mut rv_jit_context) {
