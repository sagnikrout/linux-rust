...
//! Source: arch/riscv/kernel/sys_hwprobe.c
#![no_std]
...
//
    ...
    if (riscv_isa_extension_available(core::ptr::null_mut(), C))
    pair.value |= RISCV_HWPROBE_IMA_C;
    if (has_vector() && riscv_isa_extension_available(core::ptr::null_mut(), V))
...
#[no_mangle]
unsafe extern "C" fn hwprobe_ext0_has(cpus: *const cpumask, ext: u64) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hwprobe_misaligned(cpus: *const cpumask) -> u64 {
    static u64 hwprobe_misaligned(const struct cpumask *cpus)
    {
    int cpu;
    let mut perf: u64 = -1ULL;
    for_each_cpu(cpu, cpus) {
    let mut this_perf: c_int = per_cpu(misaligned_access_speed, cpu);
    if (perf == -1ULL)
    perf = this_perf;
    if (perf != this_perf) {
...
#[no_mangle]
unsafe extern "C" fn hwprobe_vec_misaligned(cpus: *const cpumask) -> u64 {
...
// Return if supported or not even if speed wasn't probed
    for_each_cpu(cpu, cpus) {
    let mut this_perf: c_int = per_cpu(vector_misaligned_access, cpu);
    if (perf == -1ULL)
    perf = this_perf;
    if (perf != this_perf) {
...
#[no_mangle]
unsafe extern "C" fn init_hwprobe_vdso_data() -> int __init {
    static int __init init_hwprobe_vdso_data(void)
    {
    struct vdso_arch_data *avd = vdso_k_arch_data;
    let mut id_bitsmash: u64 = 0;
    struct riscv_hwprobe pair;
    int key;
//
