...
//! Source: arch/riscv/kernel/kgdb.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn decode_register_index(opcode: c_ulong, offset: c_int) -> c_int {
    static int decode_register_index(unsigned long opcode, int offset)
    {
    return (opcode >> offset) & 0x1F;
    }
#[no_mangle]
unsafe extern "C" fn decode_register_index_short(opcode: c_ulong, offset: c_int) -> c_int {
...
// next_addr = pc + 2;
    ...
    let mut imm: c_long = RV_EXTRACT_BTYPE_IMM(op_code);
    let mut rs1_val: c_ulong = 0, rs2_val = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: riscv_insn_is_bne(op_code)) -> else {
    else if (riscv_insn_is_bne(op_code))
    result = (rs1_val != rs2_val) ? true : false;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: riscv_insn_is_blt(op_code)) -> else {
    else if (riscv_insn_is_blt(op_code))
    result =
    ((long)rs1_val <
    (long)rs2_val) ? true : false;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: riscv_insn_is_bge(op_code)) -> else {
    else if (riscv_insn_is_bge(op_code))
    result =
    ((long)rs1_val >=
    (long)rs2_val) ? true : false;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: riscv_insn_is_bltu(op_code)) -> else {
    else if (riscv_insn_is_bltu(op_code))
    result = (rs1_val < rs2_val) ? true : false;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: riscv_insn_is_bgeu(op_code)) -> else {
...
// Determine where the target instruction will send us to
    ...
    let mut error: c_int = get_step_address(regs, &addr);
    if (error)
    return error;
...
#[no_mangle]
unsafe extern "C" fn kgdb_riscv_kgdbbreak(addr: c_ulong) -> c_int {
