...
//! Source: arch/riscv/kvm/vcpu_insn.c
#![no_std]
...
//
    pub insn): *mut *mut *mut *mut int (func)(struct kvm_vcpu vcpu, struct kvm_run run, ulong,
}
...
#[no_mangle]
unsafe extern "C" fn wfi_insn(vcpu: *mut kvm_vcpu, run: *mut kvm_run, insn: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wrs_insn(vcpu: *mut kvm_vcpu, run: *mut kvm_run, insn: c_ulong) -> c_int {
...
unsafe extern "C" fn csr_insn(vcpu: *mut kvm_vcpu, run: *mut kvm_run, insn: c_ulong) -> c_int {
    ...
    let mut csr_num: c_uint = insn >> SH_RS2;
    let mut rs1_num: c_uint = (insn >> SH_RS1) & MASK_RX;
    let mut rs1_val: c_ulong = GET_RS1(insn, &vcpu.arch.guest_context);
    const struct csr_func *tcfn, *cfn = core::ptr::null_mut();
    let mut val: c_ulong = 0, wr_mask = 0, new_val = 0;
...
#[no_mangle]
unsafe extern "C" fn is_load_guest_page_fault(scause: c_ulong) -> bool {
...
//
    ...
    {
    let mut insn: c_ulong = trap.stval;
    let mut utrap: kvm_cpu_trap = { 0 };
...
//
    ...
    let mut shift: c_int = 0, len = 0, insn_len = 0;
    let mut utrap: kvm_cpu_trap = { 0 };
...
//
    ...
    ulong data;
    unsigned long insn, raw_insn;
    let mut len: c_int = 0, insn_len = 0;
    let mut utrap: kvm_cpu_trap = { 0 };
    struct kvm_cpu_context *ct = &vcpu.arch.guest_context;
// Determine trapped instruction
    if (htinst & 0x1) {
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_vcpu_mmio_return(vcpu: *mut kvm_vcpu, run: *mut kvm_run) -> c_int {
