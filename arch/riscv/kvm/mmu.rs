...
//
    ...
    let mut eager_page_split: static bool __read_mostly = true;
    module_param(eager_page_split, bool, 0644);
#[no_mangle]
unsafe extern "C" fn mmu_wp_memory_region(kvm: *mut kvm, slot: c_int) {
    static void mmu_wp_memory_region(struct kvm *kvm, int slot)
    {
    struct kvm_memslots *slots = kvm_memslots(kvm);
    struct kvm_memory_slot *memslot = id_to_memslot(slots, slot);
    let mut start: phys_addr_t = memslot.base_gfn << PAGE_SHIFT;
    let mut end: phys_addr_t = (memslot.base_gfn + memslot.npages) << PAGE_SHIFT;
    ...
    let mut pgd_levels: c_ulong = kvm.arch.pgd_levels;
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_mmu_iounmap(kvm: *mut kvm, gpa: gpa_t, size: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn need_topup_split_caches_or_resched(kvm: *mut kvm, count: c_int) -> bool {
    ...
    let mut addr: phys_addr_t = ALIGN_DOWN(start, PMD_SIZE);
    let mut last_flush_gfn: phys_addr_t = addr >> PAGE_SHIFT;
    let mut count: c_int = gstage.pgd_levels;
...
#[no_mangle]
pub unsafe extern "C" fn kvm_arch_free_memslot(kvm: *mut kvm, free: *mut kvm_memory_slot) {
...
#[no_mangle]
unsafe extern "C" fn mmu_split_memory_region(kvm: *mut kvm, slot: c_int) {
...
pub unsafe extern "C" fn kvm_age_gfn(kvm: *mut kvm, range: *mut kvm_gfn_range) -> bool {
    ...
    &ptep, &ptep_level))
    return false;
    return ptep_test_and_clear_young(core::ptr::null_mut(), 0, ptep);
    }
#[no_mangle]
...
// hfnp = hfn;
    ...
    u32 ptep_level;
    let mut dirty_marked: bool = false;
    bool ret;
    kvm_riscv_gstage_init(&gstage, kvm);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_hugetlb) -> else {
    else if (is_hugetlb)
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_mmu_alloc_pgd(kvm: *mut kvm) -> c_int {
    int kvm_riscv_mmu_alloc_pgd(struct kvm *kvm)
    {
    struct page *pgd_page;
    if (kvm.arch.pgd != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_mmu_free_pgd(kvm: *mut kvm) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_mmu_update_hgatp(vcpu: *mut kvm_vcpu) {
