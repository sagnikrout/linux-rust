...
//! Source: arch/riscv/kvm/vcpu_onereg.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_vcpu_setup_isa(vcpu: *mut kvm_vcpu) {
...
// Do not modify anything beyond single letter extensions
    ...
    KVM_REG_SIZE_MASK |
    KVM_REG_RISCV_CORE);
    let mut regs_max: c_ulong = sizeof(struct kvm_riscv_core) / sizeof(unsigned long);
    unsigned long reg_val;
    if (KVM_REG_SIZE(reg.id) != sizeof(unsigned long))
    return -EINVAL;
    if (reg_num >= regs_max)
...
#[no_mangle]
pub unsafe extern "C" fn if(KVM_REG_RISCV_CORE_REG(mode): reg_num ==) -> else {
...
pub unsafe extern "C" fn if(KVM_REG_RISCV_CORE_REG(mode): reg_num ==) -> else {
    ...
    let mut regs_max: c_ulong = sizeof(struct kvm_riscv_csr) / sizeof(unsigned long);
    if (reg_num >= regs_max)
    return -ENOENT;
    reg_num = array_index_nospec(reg_num, regs_max);
...
#[no_mangle]
unsafe extern "C" fn num_config_regs(vcpu: *const kvm_vcpu) -> c_ulong {
    static unsigned long num_config_regs(const struct kvm_vcpu *vcpu)
    {
    return copy_config_reg_indices(vcpu, core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn num_core_regs() -> c_ulong {
    static inline unsigned long num_core_regs(void)
    {
    return sizeof(struct kvm_riscv_core) / sizeof(unsigned long);
    }
#[no_mangle]
unsafe extern "C" fn copy_core_reg_indices(uindices: *mut u64 __user) -> c_int {
    static int copy_core_reg_indices(u64 __user *uindices)
    {
    let mut n: c_int = num_core_regs();
    for (int i = 0; i < n; i++) {
    u64 size = IS_ENABLED(CONFIG_32BIT) ?
    KVM_REG_SIZE_U32 : KVM_REG_SIZE_U64;
    let mut reg: u64 = KVM_REG_RISCV | size | KVM_REG_RISCV_CORE | i;
    if (uindices) {
    if (put_user(reg, uindices))
    return -EFAULT;
    uindices++;
...
#[no_mangle]
pub unsafe extern "C" fn num_csr_regs(vcpu: *const kvm_vcpu) -> c_ulong {
    static inline unsigned long num_csr_regs(const struct kvm_vcpu *vcpu)
    {
    let mut n: c_ulong = sizeof(struct kvm_riscv_csr) / sizeof(unsigned long);
    ...
    u64 __user *uindices)
    {
    let mut n1: c_int = sizeof(struct kvm_riscv_csr) / sizeof(unsigned long);
    let mut n2: c_int = 0, n3 = 0, n4 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn num_timer_regs() -> c_ulong {
    static inline unsigned long num_timer_regs(void)
    {
    return sizeof(struct kvm_riscv_timer) / sizeof(u64);
    }
#[no_mangle]
unsafe extern "C" fn copy_timer_reg_indices(uindices: *mut u64 __user) -> c_int {
    static int copy_timer_reg_indices(u64 __user *uindices)
    {
    let mut n: c_int = num_timer_regs();
    for (int i = 0; i < n; i++) {
...
#[no_mangle]
pub unsafe extern "C" fn num_fp_f_regs(vcpu: *const kvm_vcpu) -> c_ulong {
    ...
    u64 __user *uindices)
    {
    let mut n: c_int = num_fp_f_regs(vcpu);
    for (int i = 0; i < n; i++) {
...
#[no_mangle]
pub unsafe extern "C" fn num_fp_d_regs(vcpu: *const kvm_vcpu) -> c_ulong {
    ...
    {
    int i;
    let mut n: c_int = num_fp_d_regs(vcpu);
    u64 reg;
// copy fp.d.f indices
    for (i = 0; i < n-1; i++) {
...
// copy fp.d.fcsr indices
    ...
    u64 size = IS_ENABLED(CONFIG_32BIT) ?
    KVM_REG_SIZE_U32 : KVM_REG_SIZE_U64;
    let mut reg: u64 = KVM_REG_RISCV | size | KVM_REG_RISCV_ISA_EXT | i;
    if (__kvm_riscv_isa_check_host(i, &guest_ext))
    continue;
    if (uindices) {
    if (put_user(reg, uindices))
...
#[no_mangle]
pub unsafe extern "C" fn num_isa_ext_regs(vcpu: *const kvm_vcpu) -> c_ulong {
    static inline unsigned long num_isa_ext_regs(const struct kvm_vcpu *vcpu)
    {
    return copy_isa_ext_reg_indices(vcpu, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn num_sbi_ext_regs(vcpu: *mut kvm_vcpu) -> c_ulong {
    static unsigned long num_sbi_ext_regs(struct kvm_vcpu *vcpu)
    {
    return kvm_riscv_vcpu_reg_indices_sbi_ext(vcpu, core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn num_sbi_regs(vcpu: *mut kvm_vcpu) -> c_ulong {
    static inline unsigned long num_sbi_regs(struct kvm_vcpu *vcpu)
    {
    return kvm_riscv_vcpu_reg_indices_sbi(vcpu, core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn num_vector_regs(vcpu: *const kvm_vcpu) -> c_ulong {
...
// vstart, vl, vtype, vcsr, vlenb and 32 vector regs
    ...
    {
    const struct kvm_cpu_context *cntx = &vcpu.arch.guest_context;
    let mut n: c_int = num_vector_regs(vcpu);
    u64 reg, size;
    int i;
    if (n == 0)
    return 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_vcpu_num_regs(vcpu: *mut kvm_vcpu) -> c_ulong {
