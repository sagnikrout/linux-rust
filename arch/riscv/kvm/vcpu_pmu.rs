...
//! Source: arch/riscv/kvm/vcpu_pmu.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_get_sample_period(pmc: *mut kvm_pmc) -> u64 {
    static u64 kvm_pmu_get_sample_period(struct kvm_pmc *pmc)
    {
    let mut counter_val_mask: u64 = GENMASK(pmc.cinfo.width, 0);
    u64 sample_period;
    if (!pmc.counter_val)
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_get_perf_event_type(eidx: c_ulong) -> u32 {
    static u32 kvm_pmu_get_perf_event_type(unsigned long eidx)
    {
    let mut etype: enum sbi_pmu_event_type = get_event_type(eidx);
    let mut type: u32 = PERF_TYPE_MAX;
    switch (etype) {
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_is_fw_event(eidx: c_ulong) -> bool {
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_get_perf_event_hw_config(sbi_event_code: u32) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_get_perf_event_cache_config(sbi_event_code: u32) -> u64 {
    static u64 kvm_pmu_get_perf_event_cache_config(u32 sbi_event_code)
    {
    let mut config: u64 = U64_MAX;
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_get_perf_event_config(eidx: c_ulong, evt_data: u64) -> u64 {
    static u64 kvm_pmu_get_perf_event_config(unsigned long eidx, uint64_t evt_data)
    {
    let mut etype: enum sbi_pmu_event_type = get_event_type(eidx);
    let mut ecode: u32 = get_event_code(eidx);
    let mut config: u64 = U64_MAX;
    switch (etype) {
    case SBI_PMU_EVENT_TYPE_HW:
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_get_fixed_pmc_index(eidx: c_ulong) -> c_int {
    static int kvm_pmu_get_fixed_pmc_index(unsigned long eidx)
    {
    let mut etype: u32 = kvm_pmu_get_perf_event_type(eidx);
    let mut ecode: u32 = get_event_code(eidx);
    ...
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(SBI_PMU_HW_INSTRUCTIONS: ecode ==) -> else {
...
// out_val = pmc->counter_val;
    ...
    {
    let mut num_ctrs: c_ulong = kvm_pmu_num_counters(kvpmu);
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_vcpu_pmu_incr_fw(vcpu: *mut kvm_vcpu, fid: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn kvm_pmu_clear_snapshot_area(vcpu: *mut kvm_vcpu) {
    static void kvm_pmu_clear_snapshot_area(struct kvm_vcpu *vcpu)
    {
    struct kvm_pmu *kvpmu = vcpu_to_pmu(vcpu);
    kfree(kvpmu.sdata);
    kvpmu.sdata = core::ptr::null_mut();
    ...
    {
    struct kvm_pmu *kvpmu = vcpu_to_pmu(vcpu);
    let mut snapshot_area_size: c_int = sizeof(struct riscv_pmu_snapshot_data);
    let mut sbiret: c_int = 0;
    gpa_t saddr;
    if (!kvpmu || flags) {
...
// No need to check writable slot explicitly as kvm_vcpu_write_guest does it internally
    ...
    {
    struct riscv_pmu_event_info *einfo = core::ptr::null_mut();
    size_t shmem_size;
    gpa_t shmem;
    u32 eidx, etype;
    u64 econfig;
    ...
    ret = riscv_pmu_get_event_info(etype, econfig, core::ptr::null_mut());
    ...
    struct kvm_pmc *pmc;
    int fevent_code;
    let mut snap_flag_set: bool = flags & SBI_PMU_START_FLAG_INIT_SNAPSHOT;
...
// Check if the counter was already started for some reason
    ...
    struct kvm_pmc *pmc;
    int fevent_code;
    let mut snap_flag_set: bool = flags & SBI_PMU_STOP_FLAG_TAKE_SNAPSHOT;
    let mut shmem_needs_update: bool = false;
    if (kvm_pmu_validate_counter_mask(kvpmu, ctr_base, ctr_mask) < 0) {
    sbiret = SBI_ERR_INVALID_PARAM;
    goto out;
    }
...
// Release the counter if this is a reset request
    ...
    pmc.counter_val = kvpmu.fw_event[fevent_code].value;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: pmc->perf_event) -> else {
    else if (pmc.perf_event)
...
//
    ...
    let mut etype: u32 = kvm_pmu_get_perf_event_type(eidx);
    struct kvm_pmu *kvpmu = vcpu_to_pmu(vcpu);
    struct kvm_pmc *pmc = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_vcpu_pmu_init(vcpu: *mut kvm_vcpu) {
    void kvm_riscv_vcpu_pmu_init(struct kvm_vcpu *vcpu)
    {
    let mut i: c_int = 0, ret, num_hw_ctrs = 0, hpm_width = 0;
    struct kvm_pmu *kvpmu = vcpu_to_pmu(vcpu);
    struct kvm_pmc *pmc;
//
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_vcpu_pmu_deinit(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_vcpu_pmu_reset(vcpu: *mut kvm_vcpu) {
