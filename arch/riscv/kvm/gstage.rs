...
//! Source: arch/riscv/kvm/gstage.c
#![no_std]
...
//
    ...
    let mut __ro_after_init: unsigned long kvm_riscv_gstage_max_pgd_levels = 3;

    let mut __ro_after_init: unsigned long kvm_riscv_gstage_max_pgd_levels = 2;
    ...
    {
    unsigned long mask;
    let mut shift: c_ulong = HGATP_PAGE_SHIFT + (kvm_riscv_gstage_index_bits * level);
...
#[no_mangle]
pub unsafe extern "C" fn gstage_pte_page_vaddr(pte: pte_t) -> c_ulong {
...
// out_pgorder = 12 + (level * kvm_riscv_gstage_index_bits);
    ...
    {
    int rc;
    let mut page_order: c_ulong = PAGE_SHIFT;
...
// out_pgsize = BIT(page_order);
    ...
    {
    pte_t *ptep;
    let mut current_level: u32 = gstage.pgd_levels - 1;
...
#[no_mangle]
unsafe extern "C" fn gstage_tlb_flush(gstage: *mut kvm_gstage, level: u32, addr: gpa_t) {
    static void gstage_tlb_flush(struct kvm_gstage *gstage, u32 level, gpa_t addr)
    {
    let mut order: c_ulong = PAGE_SHIFT;
    ...
    let mut was_invalid: bool = !pte_val(*ptep);
...
//
    ...
    {
    let mut child_pte: c_ulong = huge_pte;
...
//
    ...
    set_pte(ptep, __pte(0));
#[no_mangle]
pub unsafe extern "C" fn if(GSTAGE_OP_WP: op ==) -> else {
    ...
    let mut addr: gpa_t = start, end = start + size;
    let mut flush: bool = false;
    while (addr < end) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_gstage_wp_range(gstage: *mut kvm_gstage, start: gpa_t, end: gpa_t) -> bool {
    ...
    {
    let mut start_index: c_ulong = 0;
    let mut end_index: c_ulong = BITS_PER_LONG - 1;
    let mut end_gfn: c_ulong = base_gfn + end_index;
    let mut aligned_start_gfn: c_ulong = addr >> PAGE_SHIFT;
    let mut aligned_end_gfn: c_ulong = aligned_start_gfn + (page_size >> PAGE_SHIFT) - 1;
    let mut nbits: c_uint = 0;
...
#[no_mangle]
pub unsafe extern "C" fn kvm_riscv_gstage_mode_detect() -> void __init {
