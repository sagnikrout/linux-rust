...
//! Source: arch/riscv/mm/pageattr.c
#![no_std]
...
// Nothing to do here
    ...
    let mut pfn: c_ulong = _pmd_pfn(pmdp_get(pmdp));
    let mut prot: pgprot_t = __pgprot(pmd_val(pmdp_get(pmdp)) & ~_PAGE_PFN_MASK);
    pte_t *ptep_new;
    int i;
    ...
    let mut pfn: c_ulong = _pud_pfn(pudp_get(pudp));
    let mut prot: pgprot_t = __pgprot(pud_val(pudp_get(pudp)) & ~_PAGE_PFN_MASK);
    pmd_t *pmdp_new;
    int i;
...
//
    ...
    let mut pfn: c_ulong = _p4d_pfn(p4dp_get(p4dp));
    let mut prot: pgprot_t = __pgprot(p4d_val(p4dp_get(p4dp)) & ~_PAGE_PFN_MASK);
    pud_t *pudp_new;
    int i;
...
#[no_mangle]
unsafe extern "C" fn split_linear_mapping(start: c_ulong, end: c_ulong) -> c_int {
...
//
    ...
    struct vm_struct *area = core::ptr::null_mut();
    int i, page_start;
    ...
    &pageattr_ops, core::ptr::null_mut(), &masks);
    if (ret)
    goto unlock;
    }
    ...
    ret =  walk_kernel_page_table_range(start, end, &pageattr_ops, core::ptr::null_mut(),
    &masks);
    unlock:
...
#[no_mangle]
pub unsafe extern "C" fn set_memory_rw_nx(addr: c_ulong, numpages: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn debug_pagealloc_set_page(pte: *mut pte_t, addr: c_ulong, data: *mut c_void) -> c_int {
    static int debug_pagealloc_set_page(pte_t *pte, unsigned long addr, void *data)
    {
    let mut enable: c_int = *(int *)data;
    let mut val: c_ulong = pte_val(ptep_get(pte));
    if (enable)
    val |= _PAGE_PRESENT;
    else
    val &= ~_PAGE_PRESENT;
