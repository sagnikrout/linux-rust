...
//! Source: arch/riscv/mm/context.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn __flush_context() {
...
#[no_mangle]
unsafe extern "C" fn __new_context(mm: *mut mm_struct) -> c_ulong {
    static unsigned long __new_context(struct mm_struct *mm)
    {
    let mut cur_idx: static u32 = 1;
    let mut cntx: c_ulong = atomic_long_read(&mm.context.id);
    unsigned long asid, ver = atomic_long_read(&current_version);
// Must be called with context_lock held
    lockdep_assert_held(&context_lock);
    if (cntx != 0) {
    let mut newcntx: c_ulong = ver | cntx2asid(cntx);
//
...
#[no_mangle]
unsafe extern "C" fn set_mm_asid(mm: *mut mm_struct, cpu: c_uint) {
    static void set_mm_asid(struct mm_struct *mm, unsigned int cpu)
    {
    unsigned long flags;
    let mut need_flush_tlb: bool = false;
...
#[no_mangle]
unsafe extern "C" fn set_mm_noasid(mm: *mut mm_struct) {
...
#[no_mangle]
unsafe extern "C" fn asids_init() -> int __init {
