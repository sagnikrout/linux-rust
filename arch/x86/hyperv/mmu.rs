...
//
    ...
    {
    let mut gva_n: c_int = offset;
    let mut cur: c_ulong = start, diff;
    do {
...
#[no_mangle]
unsafe extern "C" fn cpu_is_lazy(cpu: c_int) -> bool {
    ...
    u64 status;
    unsigned long flags;
    let mut do_lazy: bool = !info.freed_tables;
...
//
    ...
    gva_n, 0, flush, core::ptr::null_mut());
    }
    goto check_status;
    do_ex_hypercall:
    ...
    const struct flush_tlb_info *info)
    {
    let mut nr_bank: c_int = 0, max_gvas, gva_n;
...
//
    ...
    info.freed_tables ? core::ptr::null_mut() : cpu_is_lazy);
    if (nr_bank < 0)
    return HV_STATUS_INVALID_PARAMETER;
//
// We can flush not more than max_gvas with one hypercall. Flush the
...
//
    ...
    0, nr_bank, flush, core::ptr::null_mut());
    } else if (info.end &&
    ((info.end - info.start)/HV_TLB_FLUSH_UNIT) > max_gvas) {
    status = hv_do_rep_hypercall(
    HVCALL_FLUSH_VIRTUAL_ADDRESS_SPACE_EX,
    ...
    gva_n, nr_bank, flush, core::ptr::null_mut());
    }
    return status;
    }
#[no_mangle]
pub unsafe extern "C" fn hyperv_setup_mmu_ops() {
