...
//! Source: arch/x86/platform/uv/uv_irq.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct uv_irq_2_mmr_pnode {
    pub offset: c_ulong,
    pub pnode: c_int,
}

#[no_mangle]
unsafe extern "C" fn uv_program_mmr(cfg: *mut irq_cfg, info: *mut uv_irq_2_mmr_pnode) {
    ...
    handle_percpu_irq, core::ptr::null_mut(), info.uv.name);
    } else {
    kfree(chip_data);
    }
    return ret;
...
//
    ...
    &uv_domain_ops, core::ptr::null_mut());
    if (!uv_domain)
