...
//! Source: arch/x86/kernel/amd_gart_64.c
#![no_std]
...
//
    let mut iommu_fullflush: static int = 1;
...
#[no_mangle]
unsafe extern "C" fn free_iommu(offset: c_ulong, size: c_int) {
...
unsafe extern "C" fn dump_leak() {
    ...
    return;
    dump = 1;
    show_stack(core::ptr::null_mut(), core::ptr::null_mut(), KERN_ERR);
    debug_dma_dump_mappings(core::ptr::null_mut());
    }

#[no_mangle]
unsafe extern "C" fn iommu_full(dev: *mut device, size: usize, dir: c_int) {
...
//
    ...
    {
    let mut npages: c_ulong = iommu_num_pages(phys_mem, size, PAGE_SIZE);
...
// Fallback for dma_map_sg in case of overflow
    ...
    for_each_sg(sg, s, nents, i) {
    let mut addr: c_ulong = sg_phys(s);
...
// Map multiple scatterlist entries continuous into the first.
    ...
    unsigned long pages)
    {
    let mut iommu_start: c_ulong = alloc_iommu(dev, pages, 0);
    let mut iommu_page: c_ulong = iommu_start;
    struct scatterlist *s;
    int i;
    if (iommu_start == -1)
    return -ENOMEM;
    ...
    let mut phys_addr: c_ulong = s.dma_address;
...
//
    ...
    let mut need: c_int = 0, nextneed, i, out, start, ret;
    ...
    ps		= core::ptr::null_mut(); /* shut up gcc */
    for_each_sg(sg, s, nents, i) {
    let mut addr: dma_addr_t = sg_phys(s);
    s.dma_address = addr;
    BUG_ON(s.length == 0);
...
// free a coherent mapping
    ...
    static int no_agp;
#[no_mangle]
unsafe extern "C" fn check_iommu_size(aper: c_ulong, aper_size: u64) -> __init unsigned long {
...
#[no_mangle]
unsafe extern "C" fn read_aperture(dev: *mut pci_dev, size: *mut u32) -> __init unsigned {
    static __init unsigned read_aperture(struct pci_dev *dev, u32 *size)
    {
    let mut aper_size: unsigned = 0, aper_base_32, aper_order;
    u64 aper_base;
...
#[no_mangle]
unsafe extern "C" fn enable_gart_translations() {
...
#[no_mangle]
pub unsafe extern "C" fn set_up_gart_resume(aper_order: u32, aper_alloc: u32) {
...
#[no_mangle]
unsafe extern "C" fn gart_fixup_northbridges() {
...
#[no_mangle]
unsafe extern "C" fn gart_resume(data: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn init_amd_gatt(info: *mut agp_kern_info) -> __init int {
...
#[no_mangle]
unsafe extern "C" fn gart_iommu_shutdown() {
...
#[no_mangle]
pub unsafe extern "C" fn gart_iommu_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn gart_parse_options(p: *mut c_char) -> void __init {
