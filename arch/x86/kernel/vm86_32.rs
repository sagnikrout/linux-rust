...
//! Source: arch/x86/kernel/vm86_32.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn save_v86_state(regs: *mut kernel_vm86_regs, retval: c_int) {
...
#[no_mangle]
unsafe extern "C" fn do_sys_vm86(user_vm86: *mut vm86plus___user, plus: bool) -> c_long {
    static long do_sys_vm86(struct vm86plus_struct __user *user_vm86, bool plus)
    {
    struct task_struct *tsk = current;
...
#[no_mangle]
pub unsafe extern "C" fn set_IF(regs: *mut kernel_vm86_regs) {
    static inline void set_IF(struct kernel_vm86_regs *regs)
    {
    VEFLAGS |= X86_EFLAGS_VIF;
    }
#[no_mangle]
pub unsafe extern "C" fn clear_IF(regs: *mut kernel_vm86_regs) {
    static inline void clear_IF(struct kernel_vm86_regs *regs)
    {
    VEFLAGS &= ~X86_EFLAGS_VIF;
    }
#[no_mangle]
pub unsafe extern "C" fn clear_TF(regs: *mut kernel_vm86_regs) {
    static inline void clear_TF(struct kernel_vm86_regs *regs)
    {
    regs.pt.flags &= ~X86_EFLAGS_TF;
    }
#[no_mangle]
pub unsafe extern "C" fn clear_AC(regs: *mut kernel_vm86_regs) {
...
//
#[no_mangle]
pub unsafe extern "C" fn set_vflags_long(flags: c_ulong, regs: *mut kernel_vm86_regs) {
...
#[no_mangle]
pub unsafe extern "C" fn set_vflags_short(flags: c_ushort, regs: *mut kernel_vm86_regs) {
...
#[no_mangle]
pub unsafe extern "C" fn get_vflags(regs: *mut kernel_vm86_regs) -> c_ulong {
    static inline unsigned long get_vflags(struct kernel_vm86_regs *regs)
    {
    let mut flags: c_ulong = regs.pt.flags & RETURN_MASK;
...
#[no_mangle]
pub unsafe extern "C" fn is_revectored(nr: c_int, bitmap: *mut revectored_struct) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn handle_vm86_trap(regs: *mut kernel_vm86_regs, error_code: c_long, trapno: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn handle_vm86_fault(regs: *mut kernel_vm86_regs, error_code: c_long) {
...
// int xx
    case 0xcd: {
    let mut intno: c_int = popb(csp, ip, simulate_sigsegv);
    IP(regs) = ip;
...
// ---------------- vm86 special IRQ passing stuff -----------------
    ...
    | (1 << SIGUNUSED))
#[no_mangle]
unsafe extern "C" fn irq_handler(intno: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
pub unsafe extern "C" fn free_vm86_irq(irqnumber: c_int) {
    static inline void free_vm86_irq(int irqnumber)
    {
    unsigned long flags;
    free_irq(irqnumber, core::ptr::null_mut());
    vm86_irqs[irqnumber].tsk = core::ptr::null_mut();
    spin_lock_irqsave(&irqbits_lock, flags);
    irqbits &= ~(1 << irqnumber);
    spin_unlock_irqrestore(&irqbits_lock, flags);
    }
#[no_mangle]
pub unsafe extern "C" fn release_vm86_irqs(task: *mut task_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn get_and_reset_irq(irqnumber: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_vm86_irq_handling(subfunction: c_int, irqnumber: c_int) -> c_int {
    static int do_vm86_irq_handling(int subfunction, int irqnumber)
    {
    int ret;
    switch (subfunction) {
    ...
    case VM86_REQUEST_IRQ: {
    let mut sig: c_int = irqnumber >> 8;
    let mut irq: c_int = irqnumber & 255;
    if (!capable(CAP_SYS_ADMIN)) return -EPERM;
    if (!((1 << sig) & ALLOWED_SIGS)) return -EPERM;
    if (invalid_vm86_irq(irq)) return -EPERM;
    if (vm86_irqs[irq].tsk) return -EPERM;
    ret = request_irq(irq, &irq_handler, 0, VM86_IRQNAME, core::ptr::null_mut());
    if (ret) return ret;
