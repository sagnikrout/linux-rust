...
//! Source: arch/x86/kernel/sys_ia32.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn cp_stat64(ubuf: *mut stat64 __user, stat: *mut kstat) -> c_int {
    ...
    {
    struct kstat stat;
    let mut ret: c_int = vfs_stat(filename, &stat);
    if (!ret)
    ...
    {
    struct kstat stat;
    let mut ret: c_int = vfs_lstat(filename, &stat);
    if (!ret)
    ret = cp_stat64(statbuf, &stat);
    return ret;
    }
    ...
    {
    struct kstat stat;
    let mut ret: c_int = vfs_fstat(fd, &stat);
    if (!ret)
    ret = cp_stat64(statbuf, &stat);
    return ret;
    }
