...
//! Source: arch/x86/kernel/fpu/regset.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn regset_fpregs_active(target: *mut task_struct, regset: *const user_regset) -> c_int {
    int regset_fpregs_active(struct task_struct *target, const struct user_regset *regset)
    {
    return regset.n;
    }
#[no_mangle]
pub unsafe extern "C" fn regset_xregset_fpregs_active(target: *mut task_struct, regset: *const user_regset) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sync_fpstate(fpu: *mut fpu) {
...
//
#[no_mangle]
unsafe extern "C" fn fpu_force_restore(fpu: *mut fpu) {
...
// Mark FP and SSE as in use when XSAVE is enabled
    ...
    {
    struct fpu *fpu = x86_task_fpu(target);
    struct xregs_state *tmpbuf = core::ptr::null_mut();
    int ret;
    if (!cpu_feature_enabled(X86_FEATURE_XSAVE))
    return -ENODEV;
//
...
#[no_mangle]
pub unsafe extern "C" fn ssp_active(target: *mut task_struct, regset: *const user_regset) -> c_int {
...
pub const FP_EXP_TAG_VALID: c_int = 0;
pub const FP_EXP_TAG_ZERO: c_int = 1;
pub const FP_EXP_TAG_SPECIAL: c_int = 2;
pub const FP_EXP_TAG_EMPTY: c_int = 3;
#[no_mangle]
pub unsafe extern "C" fn twd_fxsr_to_i387(fxsave: *mut fxregs_state) -> u32 {
    static inline u32 twd_fxsr_to_i387(struct fxregs_state *fxsave)
    {
    struct _fpxreg *st;
    let mut tos: u32 = (fxsave.swd >> 11) & 7;
    let mut twd: u32 = (unsigned long) fxsave.twd;
    u32 tag;
    let mut ret: u32 = 0xffff0000u;
    int i;
    for (i = 0; i < 8; i++, twd >>= 1) {
    if (twd & 0x1) {
...
// cs and ds ignored
    ...
    if (use_xsave()) {
    let mut mb: membuf = { .p = &fxsave, .left = sizeof(fxsave) };
