...
//! Source: arch/x86/kernel/fpu/signal.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn save_fsave_header(tsk: *mut task_struct, buf: *mut void __user) -> bool {
...
//
    ...
    let mut sw_bytes: _fpx_sw_bytes = {};
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn copy_fpregs_to_sigframe(buf: *mut xregs_state __user, pkru: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn copy_fpstate_to_sigframe(buf: *mut void __user, buf_fx: *mut void __user, size: c_int, pkru: u32) -> bool {
    bool copy_fpstate_to_sigframe(void __user *buf, void __user *buf_fx, int size, u32 pkru)
    {
    struct task_struct *tsk = current;
    struct fpstate *fpstate = x86_task_fpu(tsk).fpstate;
    let mut ia32_fxstate: bool = (buf != buf_fx);
    int ret;
    ia32_fxstate &= (IS_ENABLED(CONFIG_X86_32) ||
    IS_ENABLED(CONFIG_IA32_EMULATION));
    if (!cpu_feature_enabled(X86_FEATURE_FPU)) {
    struct user_i387_ia32_struct fp;
    fpregs_soft_get(current, core::ptr::null_mut(), (struct membuf){.p = &fp,
    .left = sizeof(fp)});
...
// Save the fsave header for the 32-bit frames.
    ...
    {
    if (use_xsave()) {
    let mut init_bv: u64 = ufeatures & ~xrestore;
    int ret;
    if (likely(!fx_only))
...
//
#[no_mangle]
unsafe extern "C" fn restore_fpregs_from_user(buf: *mut void __user, xrestore: u64, fx_only: bool) -> bool {
...
//
    let mut mask: u64 = user_xfeatures | xfeatures_mask_supervisor();
...
#[no_mangle]
pub unsafe extern "C" fn xstate_sigframe_size(fpstate: *mut fpstate) -> c_uint {
    static inline unsigned int xstate_sigframe_size(struct fpstate *fpstate)
    {
    let mut size: c_uint = fpstate.user_size;
...
//
#[no_mangle]
pub unsafe extern "C" fn fpu__restore_sig(buf: *mut void __user, ia32_frame: c_int) -> bool {
    bool fpu__restore_sig(void __user *buf, int ia32_frame)
    {
    struct fpu *fpu = x86_task_fpu(current);
    void __user *buf_fx = buf;
    let mut ia32_fxstate: bool = false;
...
//
    ...
    success = !fpregs_soft_set(current, core::ptr::null_mut(), 0,
    ...
    {
    let mut frame_size: c_ulong = xstate_sigframe_size(x86_task_fpu(current).fpstate);
...
#[no_mangle]
pub unsafe extern "C" fn fpu__get_fpstate_size() -> unsigned long __init {
    unsigned long __init fpu__get_fpstate_size(void)
    {
    let mut ret: c_ulong = fpu_user_cfg.max_size;
    if (use_xsave())
