...
//
#[no_mangle]
pub unsafe extern "C" fn invalid_selector(value: u16) -> bool {
...
#[no_mangle]
unsafe extern "C" fn get_segment_reg(task: *mut task_struct, offset: c_ulong) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn get_flags(task: *mut task_struct) -> c_ulong {
    static unsigned long get_flags(struct task_struct *task)
    {
    let mut retval: c_ulong = task_pt_regs(task).flags;
//
...
#[no_mangle]
unsafe extern "C" fn set_flags(task: *mut task_struct, value: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_tsk_thread_flag(task, _arg: TIF_FORCED_TF)) -> else {
...
#[no_mangle]
unsafe extern "C" fn getreg(task: *mut task_struct, offset: c_ulong) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn ptrace_get_dr7(bp[]: *mut perf_event) -> c_ulong {
    ...
    int disabled)
    {
    let mut attr: perf_event_attr = bp.attr;
    int err;
    err = ptrace_fill_bp_fields(&attr, len, type, disabled);
    if (err)
    return err;
...
//
#[no_mangle]
unsafe extern "C" fn ptrace_write_dr7(tsk: *mut task_struct, data: c_ulong) -> c_int {
    ...
    for (i = 0; i < HBP_NUM; i++) {
    unsigned len, type;
    let mut disabled: bool = !decode_dr7(data, i, &len, &type);
...
//
#[no_mangle]
unsafe extern "C" fn ptrace_get_debugreg(tsk: *mut task_struct, n: c_int) -> c_ulong {
    static unsigned long ptrace_get_debugreg(struct task_struct *tsk, int n)
    {
    struct thread_struct *thread = &tsk.thread;
    let mut val: c_ulong = 0;
    if (n < HBP_NUM) {
    let mut index: c_int = array_index_nospec(n, HBP_NUM);
...
// normal 64bit interface to access TLS data.
    ...
    break
#[no_mangle]
unsafe extern "C" fn putreg32(child: *mut task_struct, regno: unsigned, value: u32) -> c_int {
...
// val = get_segment_reg(child,				\
    ...
    break
#[no_mangle]
unsafe extern "C" fn getreg32(child: *mut task_struct, regno: unsigned, val: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn update_regset_xstate_info(size: c_uint, xstate_mask: u64) -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn send_sigtrap(regs: *mut pt_regs, error_code: c_int, si_code: c_int) {
...
// Send us the fake SIGTRAP
    ...
    user_mode(regs) ? (void __user *)regs.ip : core::ptr::null_mut());
    }
#[no_mangle]
