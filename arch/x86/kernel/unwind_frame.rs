...
//! Source: arch/x86/kernel/unwind_frame.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn unwind_dump(state: *mut unwind_state) {
    static void unwind_dump(struct unwind_state *state)
    {
    let mut dumped_before: static bool = false;
...
#[no_mangle]
unsafe extern "C" fn in_entry_code(ip: c_ulong) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_last_frame(state: *mut unwind_state) -> bool {
    static bool is_last_frame(struct unwind_state *state)
    {
    return state.bp == last_frame(state);
    }

pub const GCC_REALIGN_WORDS: c_int = 3;

pub const GCC_REALIGN_WORDS: c_int = 1;
...
#[no_mangle]
unsafe extern "C" fn is_last_aligned_frame(state: *mut unwind_state) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_last_ftrace_frame(state: *mut unwind_state) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_last_task_frame(state: *mut unwind_state) -> bool {
...
//
    ...
    {
    let mut regs: c_ulong = (unsigned long)bp;
    if (!(regs & 0x1))
    return core::ptr::null_mut();
...
//
    ...
    let mut prev_type: enum stack_type = info.type;
    struct pt_regs *regs;
...
// Get the next frame pointer:
    ...
    state.next_bp = core::ptr::null_mut();
    } else if (state.regs) {
...
//
    ...
    (state.next_bp == core::ptr::null_mut() && state.bp < first_frame)))
