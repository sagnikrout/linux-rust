...
//! Source: arch/x86/kernel/machine_kexec_64.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct init_pgtable_data {
    pub info: *mut x86_mapping_info,
    pub level4p: *mut pgd_t,
}

#[no_mangle]
unsafe extern "C" fn mem_region_callback(res: *mut resource, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn map_mmio_serial(info: *mut x86_mapping_info, level4p: *mut pgd_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_transition_pgtable(image: *mut kimage) {
    static void free_transition_pgtable(struct kimage *image)
    {
    free_page((unsigned long)image.arch.p4d);
    image.arch.p4d = core::ptr::null_mut();
    free_page((unsigned long)image.arch.pud);
    image.arch.pud = core::ptr::null_mut();
    free_page((unsigned long)image.arch.pmd);
    image.arch.pmd = core::ptr::null_mut();
    free_page((unsigned long)image.arch.pte);
    image.arch.pte = core::ptr::null_mut();
    }
    static int init_transition_pgtable(struct kimage *image, pgd_t *pgd,
    unsigned long control_page)
    {
    let mut prot: pgprot_t = PAGE_KERNEL_EXEC_NOENC;
...
#[no_mangle]
unsafe extern "C" fn init_pgtable(image: *mut kimage, control_page: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn prepare_debug_idt(control_page: c_ulong, vec_ofs: c_ulong) {
    static void prepare_debug_idt(unsigned long control_page, unsigned long vec_ofs)
    {
    let mut idtentry: gate_desc = { 0 };
    int i;
    idtentry.bits.p		= 1;
...
pub unsafe extern "C" fn machine_kexec_prepare(image: *mut kimage) -> c_int {
    ...
    let mut reloc_start: c_ulong = (unsigned long)__relocate_kernel_start;
    let mut reloc_end: c_ulong = (unsigned long)__relocate_kernel_end;
    int result;
...
//
#[no_mangle]
pub unsafe extern "C" fn machine_kexec(image: *mut kimage) -> void __nocfi {
...
#[no_mangle]
unsafe extern "C" fn kexec_mark_crashkres(protect: bool) {
...
#[no_mangle]
unsafe extern "C" fn kexec_mark_dm_crypt_keys(protect: bool) {
...
//
#[no_mangle]
pub unsafe extern "C" fn arch_kexec_post_alloc_pages(vaddr: *mut c_void, pages: c_uint, gfp: gfp_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn arch_kexec_pre_free_pages(vaddr: *mut c_void, pages: c_uint) {
