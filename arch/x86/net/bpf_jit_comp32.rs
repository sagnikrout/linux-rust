...
//! Source: arch/x86/net/bpf_jit_comp32.c
#![no_std]
...
//
pub const IA32_JB: c_uint = 0x72;
pub const IA32_JAE: c_uint = 0x73;
pub const IA32_JE: c_uint = 0x74;
pub const IA32_JNE: c_uint = 0x75;
pub const IA32_JBE: c_uint = 0x76;
pub const IA32_JA: c_uint = 0x77;
pub const IA32_JL: c_uint = 0x7C;
pub const IA32_JGE: c_uint = 0x7D;
pub const IA32_JLE: c_uint = 0x7E;
pub const IA32_JG: c_uint = 0x7F;
...
// dst = imm (4 bytes)
    ...
    u8 *prog = *pprog;
    let mut cnt: c_int = 0;
    let mut sreg: u8 = sstk ? IA32_EAX : src;
    if (sstk)
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !aux->verifier_zext) -> else {
...
//
    ...
    u8 *prog = *pprog;
    let mut cnt: c_int = 0;
    let mut sreg: u8 = sstk ? IA32_ECX : src;
    if (sstk)
...
// pprog = prog;
    ...
    u8 *prog = *pprog;
    let mut cnt: c_int = 0;
    let mut dreg_lo: u8 = dstk ? IA32_EAX : dst_lo;
    let mut dreg_hi: u8 = dstk ? IA32_EDX : dst_hi;
    if (dstk && val != 64) {
...
// mov ecx,dword ptr [ebp+off]
    ...
    STACK_VAR(src));
#[no_mangle]
pub unsafe extern "C" fn if(IA32_ECX: src !=) -> else {
...
//
    ...
    u8 *prog = *pprog;
    let mut cnt: c_int = 0;
    let mut dreg: u8 = dstk ? IA32_EAX : dst;
    u8 b2;
    if (dstk)
// mov eax,dword ptr [ebp+off]
    EMIT3(0x8B, add_2reg(0x40, IA32_EBP, IA32_EAX), STACK_VAR(dst));
...
//
    ...
    let mut cnt: c_int = 0;
    let mut sreg: u8 = sstk ? IA32_EAX : src;
    let mut dreg: u8 = dstk ? IA32_EDX : dst;
    if (sstk)
// mov eax,dword ptr [ebp+off]
    EMIT3(0x8B, add_2reg(0x40, IA32_EBP, IA32_EAX), STACK_VAR(src));
    if (dstk)
...
// dst = ~dst (64 bit)
#[no_mangle]
pub unsafe extern "C" fn emit_ia32_neg64(dst[]: u8, dstk: bool, pprog: *mut u8) {
...
pub const PROLOGUE_SIZE: c_int = 35;
//
// Emit prologue code for BPF program and check its size.
// bpf_tail_call helper will skip it while jumping into another program.
//
#[no_mangle]
unsafe extern "C" fn emit_prologue(pprog: *mut u8, stack_depth: u32) {
    static void emit_prologue(u8 **pprog, u32 stack_depth)
    {
    u8 *prog = *pprog;
    let mut cnt: c_int = 0;
    const u8 *r1 = bpf2ia32[BPF_REG_1];
    let mut fplo: u8 = bpf2ia32[BPF_REG_FP][0];
    let mut fphi: u8 = bpf2ia32[BPF_REG_FP][1];
...
#[no_mangle]
unsafe extern "C" fn emit_epilogue(pprog: *mut u8, stack_depth: u32) {
...
#[no_mangle]
unsafe extern "C" fn emit_jmp_edx(pprog: *mut u8, ip: *mut u8) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn emit_bpf_tail_call(pprog: *mut u8, ip: *mut u8) {
    ...
    const u8 *tcc = bpf2ia32[TCALL_CNT];
    u32 lo, hi;
    let mut jmp_label1: static int = -1;
//
// if (index >= array->map.max_entries)
// goto out;
//
...
#[no_mangle]
pub unsafe extern "C" fn emit_push_r64(src[]: u8, pprog: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn emit_push_r32(src[]: u8, pprog: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn get_cond_jmp_opcode(op: u8, is_cmp_lo: bool) -> u8 {
...
//
    ...
    let mut free_arg_regs: c_int = ARRAY_SIZE(arg_regs);
    const struct btf_func_model *fm;
    let mut bytes_in_stack: c_int = 0;
    ...
    let mut regs_needed: c_int = fm.arg_size[i] > sizeof(u32) ? 2 : 1;
...
// pprog = prog;
    ...
    int i, cnt = 0;
    let mut proglen: c_int = 0;
    u8 *prog = temp;
    emit_prologue(&prog, bpf_prog.aux.stack_depth);
    for (i = 0; i < insn_cnt; i++, insn++) {
    let mut imm32: i32 = insn.imm;
    let mut is64: bool = BPF_CLASS(insn.code) == BPF_ALU64;
    let mut dstk: bool = insn.dst_reg != BPF_REG_AX;
    let mut sstk: bool = insn.src_reg != BPF_REG_AX;
...
// cond jump
    ...
    let mut is_jmp64: bool = BPF_CLASS(insn.code) == BPF_JMP;
    let mut dreg_lo: u8 = dstk ? IA32_EAX : dst_lo;
    let mut dreg_hi: u8 = dstk ? IA32_EDX : dst_hi;
    let mut sreg_lo: u8 = sstk ? IA32_ECX : src_lo;
    let mut sreg_hi: u8 = sstk ? IA32_EBX : src_hi;
    if (dstk) {
    EMIT3(0x8B, add_2reg(0x40, IA32_EBP, IA32_EAX),
    STACK_VAR(dst_lo));
    if (is_jmp64)
...
// cmp dreg_lo,sreg_lo
    ...
    let mut is_jmp64: bool = BPF_CLASS(insn.code) == BPF_JMP;
    let mut dreg_lo: u8 = IA32_EAX;
    let mut dreg_hi: u8 = IA32_EDX;
    let mut sreg_lo: u8 = sstk ? IA32_ECX : src_lo;
    let mut sreg_hi: u8 = sstk ? IA32_EBX : src_hi;
    if (dstk) {
    EMIT3(0x8B, add_2reg(0x40, IA32_EBP, IA32_EAX),
...
// or dreg_lo,dreg_hi
    ...
    let mut dreg_lo: u8 = IA32_EAX;
    let mut dreg_hi: u8 = IA32_EDX;
    let mut sreg_lo: u8 = IA32_ECX;
    let mut sreg_hi: u8 = IA32_EBX;
    u32 hi;
    if (dstk) {
    EMIT3(0x8B, add_2reg(0x40, IA32_EBP, IA32_EAX),
    STACK_VAR(dst_lo));
