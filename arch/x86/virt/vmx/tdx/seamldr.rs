...
//! Source: arch/x86/virt/vmx/tdx/seamldr.c
#![no_std]
...
pub const P_SEAMLDR_INFO: c_uint = 0x8000000000000000;
pub const P_SEAMLDR_INSTALL: c_uint = 0x8000000000000001;
pub const SEAMLDR_MAX_NR_MODULE_PAGES: c_int = 496;
pub const SEAMLDR_MAX_NR_SIG_PAGES: c_int = 1;
//
...
//
pub const SEAMLDR_SCENARIO_UPDATE: c_int = 1;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct seamldr_params {
    pub version: u32,
    pub scenario: u32,
    pub sigstruct_pages_pa_list: [u64; SEAMLDR_MAX_NR_SIG_PAGES],
    pub reserved: [u8; 104],
    pub module_nr_pages: u64,
    pub module_pages_pa_list: [u64; SEAMLDR_MAX_NR_MODULE_PAGES],
    pub __packed: },
    pub 4096): static_assert(sizeof(struct seamldr_params) ==,
//
...
//
    pub DEFINE_RAW_SPINLOCK(seamldr_lock): static,
#[no_mangle]
unsafe extern "C" fn seamldr_call(fn: u64, args: *mut tdx_module_args) -> c_int {
...
//
    ...
    pub args): return seamcall_prerr(fn,,
    }
#[no_mangle]
pub unsafe extern "C" fn seamldr_get_info(seamldr_info: *mut seamldr_info) -> c_int {
    int seamldr_get_info(struct seamldr_info *seamldr_info)
    {
    pub {}: tdx_module_args args =,
//
// Use slow_virt_to_phys() since @seamldr_info may be allocated on
// the stack.
//
    pub slow_virt_to_phys(seamldr_info): args.rcx =,
    pub &args): return seamldr_call(P_SEAMLDR_INFO,,
    }
    pub "tdx-host"): EXPORT_SYMBOL_FOR_MODULES(seamldr_get_info,,
// Call into P-SEAMLDR to install a TDX module update
#[no_mangle]
unsafe extern "C" fn seamldr_install(params: *const seamldr_params) -> c_int {
    static int seamldr_install(const struct seamldr_params *params)
    {
    pub {}: tdx_module_args args =,
    pub __pa(params): args.rcx =,
    pub &args): return seamldr_call(P_SEAMLDR_INSTALL,,
    }
pub const TDX_IMAGE_VERSION_2: c_uint = 0x200;
// First page of the on-disk module update image:
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tdx_image_header {
    pub version: u16,
    pub checksum: u16,
    pub signature: [u8; 8],
    pub sigstruct_nr_pages: u32,
    pub module_nr_pages: u32,
    pub reserved: [u8; 4076],
    pub __packed: },

    pub 4096): static_assert(TDX_IMAGE_HEADER_SIZE ==,
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tdx_image {
    pub header: tdx_image_header,
    pub payload: [u8; ],
}
...
//
#[no_mangle]
unsafe extern "C" fn populate_pa_list(pa_list: *mut u64, vmalloc_addr: *const u8, vmalloc_len_pages: u32) {
    static void populate_pa_list(u64 *pa_list, const u8 *vmalloc_addr, u32 vmalloc_len_pages)
    {
    int i;
    for (i = 0; i < vmalloc_len_pages; i++) {
    let mut offset: c_ulong = i * PAGE_SIZE;
    let mut pfn: c_ulong = vmalloc_to_pfn(&vmalloc_addr[offset]);
...
//
    ...
    let mut sigstruct_len: u32 = header.sigstruct_nr_pages * PAGE_SIZE;
    let mut module_len: u32 = header.module_nr_pages    * PAGE_SIZE;
...
#[no_mangle]
unsafe extern "C" fn ack_state(ctrl: *mut update_ctrl, result: c_int) {
...
#[no_mangle]
unsafe extern "C" fn init_state(ctrl: *mut update_ctrl) {
...
//
#[no_mangle]
unsafe extern "C" fn do_seamldr_install_module(seamldr_params: *mut c_void) -> c_int {
    static int do_seamldr_install_module(void *seamldr_params)
    {
    let mut curstate: enum module_update_state = MODULE_UPDATE_START;
    enum module_update_state newstate;
    let mut is_lead_cpu: bool = false;
    let mut ret: c_int = 0;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn seamldr_install_module(data: *const u8, data_len: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn seamldr_lock_module_update() {
...
#[no_mangle]
pub unsafe extern "C" fn seamldr_unlock_module_update() {
