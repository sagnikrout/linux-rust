...
//
    ...
    let mut nx_huge_pages: int __read_mostly = -1;
    static uint __read_mostly nx_huge_pages_recovery_period_ms;

// Recovery can cause latency spikes, disable it for PREEMPT_RT.
    let mut nx_huge_pages_recovery_ratio: static uint __read_mostly = 0;

    let mut nx_huge_pages_recovery_ratio: static uint __read_mostly = 60;
...
//
    let mut tdp_enabled: bool __read_mostly = false;
    static bool __ro_after_init tdp_mmu_allowed;

    let mut tdp_mmu_enabled: bool __read_mostly = true;
    module_param_named(tdp_mmu, tdp_mmu_enabled, bool, 0444);
    EXPORT_SYMBOL_FOR_KVM_INTERNAL(tdp_mmu_enabled);

    let mut eager_page_split: bool __read_mostly = true;
    module_param(eager_page_split, bool, 0644);
    static int max_huge_page_level __read_mostly;
    static int tdp_root_level __read_mostly;
    static int max_tdp_level __read_mostly;
pub const PTE_PREFETCH_NUM: c_int = 8;

// make pte_list_desc fit well in cache lines
pub const PTE_LIST_EXT: c_int = 14;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pte_list_desc {
    pub more: *mut pte_list_desc,
// The number of PTEs stored in _this_ descriptor.
    pub spte_count: u32,
// The number of PTEs stored in all tails of this descriptor.
    pub tail_count: u32,
    pub sptes: [*mut u64; PTE_LIST_EXT],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct kvm_shadow_walk_iterator {
    pub addr: u64,
    pub shadow_addr: hpa_t,
    pub sptep: *mut u64,
    pub level: c_int,
    pub index: unsigned,
}
...
#[no_mangle]
unsafe extern "C" fn get_guest_cr3(vcpu: *mut kvm_vcpu) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_available_flush_remote_tlbs_range() -> bool {
...
#[no_mangle]
unsafe extern "C" fn kvm_flush_remote_tlbs_sptep(kvm: *mut kvm, sptep: *mut u64) {
    static void kvm_flush_remote_tlbs_sptep(struct kvm *kvm, u64 *sptep)
    {
    struct kvm_mmu_page *sp = sptep_to_sp(sptep);
    let mut gfn: gfn_t = kvm_mmu_page_get_gfn(sp, spte_index(sptep));
    kvm_flush_remote_tlbs_gfn(kvm, gfn, sp.role.level);
    }
    static void mark_mmio_spte(struct kvm_vcpu *vcpu, u64 *sptep, u64 gfn,
    unsigned int access)
    {
    let mut spte: u64 = make_mmio_spte(vcpu, gfn, access);
    trace_mark_mmio_spte(sptep, gfn, spte);
    mmu_spte_set(sptep, spte);
    }
#[no_mangle]
unsafe extern "C" fn get_mmio_spte_gfn(spte: u64) -> gfn_t {
    static gfn_t get_mmio_spte_gfn(u64 spte)
    {
    let mut gpa: u64 = spte & shadow_nonpresent_or_rsvd_lower_gfn_mask;
    gpa |= (spte >> SHADOW_NONPRESENT_OR_RSVD_MASK_LEN)
    & shadow_nonpresent_or_rsvd_mask;
    return gpa >> PAGE_SHIFT;
    }
#[no_mangle]
unsafe extern "C" fn get_mmio_spte_access(spte: u64) -> unsigned {
    static unsigned get_mmio_spte_access(u64 spte)
    {
    return spte & shadow_mmio_access_mask;
    }
#[no_mangle]
unsafe extern "C" fn check_mmio_spte(vcpu: *mut kvm_vcpu, spte: u64) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_cpuid_PSE36() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __set_spte(sptep: *mut u64, spte: u64) {
...
#[no_mangle]
unsafe extern "C" fn __update_clear_spte_fast(sptep: *mut u64, spte: u64) {
...
#[no_mangle]
unsafe extern "C" fn __update_clear_spte_slow(sptep: *mut u64, spte: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn __get_spte_lockless(sptep: *mut u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn count_spte_clear(sptep: *mut u64, spte: u64) {
...
//
#[no_mangle]
unsafe extern "C" fn mmu_spte_set(sptep: *mut u64, new_spte: u64) {
...
//
#[no_mangle]
unsafe extern "C" fn mmu_spte_update(sptep: *mut u64, new_spte: u64) -> bool {
    static bool mmu_spte_update(u64 *sptep, u64 new_spte)
    {
    let mut old_spte: u64 = *sptep;
...
//
#[no_mangle]
unsafe extern "C" fn mmu_spte_clear_track_bits(kvm: *mut kvm, sptep: *mut u64) -> u64 {
    static u64 mmu_spte_clear_track_bits(struct kvm *kvm, u64 *sptep)
    {
    let mut old_spte: u64 = *sptep;
    let mut level: c_int = sptep_to_sp(sptep).role.level;
    if (!is_shadow_present_pte(old_spte) ||
...
//
#[no_mangle]
unsafe extern "C" fn mmu_spte_clear_no_track(sptep: *mut u64) {
    static void mmu_spte_clear_no_track(u64 *sptep)
    {
    __update_clear_spte_fast(sptep, SHADOW_NONPRESENT_VALUE);
    }
#[no_mangle]
unsafe extern "C" fn mmu_spte_get_lockless(sptep: *mut u64) -> u64 {
    static u64 mmu_spte_get_lockless(u64 *sptep)
    {
    return __get_spte_lockless(sptep);
    }
#[no_mangle]
pub unsafe extern "C" fn is_tdp_mmu_active(vcpu: *mut kvm_vcpu) -> bool {
    static inline bool is_tdp_mmu_active(struct kvm_vcpu *vcpu)
    {
    return tdp_mmu_enabled && vcpu.arch.mmu.root_role.direct;
    }
#[no_mangle]
unsafe extern "C" fn walk_shadow_page_lockless_begin(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
unsafe extern "C" fn walk_shadow_page_lockless_end(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
unsafe extern "C" fn mmu_topup_memory_caches(vcpu: *mut kvm_vcpu, maybe_indirect: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mmu_free_memory_caches(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
unsafe extern "C" fn mmu_free_pte_list_desc(pte_list_desc: *mut pte_list_desc) {
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_page_get_gfn(sp: *mut kvm_mmu_page, index: c_int) -> gfn_t {
    static gfn_t kvm_mmu_page_get_gfn(struct kvm_mmu_page *sp, int index)
    {
...
//
#[no_mangle]
unsafe extern "C" fn kvm_mmu_page_get_access(sp: *mut kvm_mmu_page, index: c_int) -> u32 {
...
//
    ...
    unsigned int access)
    {
    let mut gfn: gfn_t = kvm_mmu_page_get_gfn(sp, index);
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_gfn_disallow_lpage(slot: *const kvm_memory_slot, gfn: gfn_t) {
    void kvm_mmu_gfn_disallow_lpage(const struct kvm_memory_slot *slot, gfn_t gfn)
    {
    update_gfn_disallow_lpage_count(slot, gfn, 1);
    }
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_gfn_allow_lpage(slot: *const kvm_memory_slot, gfn: gfn_t) {
    void kvm_mmu_gfn_allow_lpage(const struct kvm_memory_slot *slot, gfn_t gfn)
    {
    update_gfn_disallow_lpage_count(slot, gfn, -1);
    }
#[no_mangle]
unsafe extern "C" fn account_shadowed(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn unaccount_shadowed(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn unaccount_nx_huge_page(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn __kvm_rmap_lock(rmap_head: *mut kvm_rmap_head) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn kvm_rmap_get(rmap_head: *mut kvm_rmap_head) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn kvm_rmap_lock_readonly(rmap_head: *mut kvm_rmap_head) -> c_ulong {
...
//
    ...
    let mut j: c_int = head_desc.spte_count - 1;
//
...
#[no_mangle]
pub unsafe extern "C" fn pte_list_count(rmap_head: *mut kvm_rmap_head) -> c_uint {
    unsigned int pte_list_count(struct kvm_rmap_head *rmap_head)
    {
    let mut rmap_val: c_ulong = kvm_rmap_get(rmap_head);
    struct pte_list_desc *desc;
    if (!rmap_val)
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(KVM_RMAP_MANY): !(rmap_val &) -> else {
...
#[no_mangle]
unsafe extern "C" fn rmap_remove(kvm: *mut kvm, spte: *mut u64) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rmap_iterator {
// private fields
    pub head: *mut rmap_head,
    pub /: *mut *mut *mut pte_list_desc desc; / holds the sptep if not NULL,
    pub /: *mut *mut int pos; / index of the sptep,
}
...
#[no_mangle]
unsafe extern "C" fn drop_spte(kvm: *mut kvm, sptep: *mut u64) {
    static void drop_spte(struct kvm *kvm, u64 *sptep)
    {
    let mut old_spte: u64 = mmu_spte_clear_track_bits(kvm, sptep);
    if (is_shadow_present_pte(old_spte))
...
//
#[no_mangle]
unsafe extern "C" fn spte_write_protect(sptep: *mut u64, pt_protect: bool) -> bool {
    static bool spte_write_protect(u64 *sptep, bool pt_protect)
    {
    let mut spte: u64 = *sptep;
...
#[no_mangle]
unsafe extern "C" fn spte_clear_dirty(sptep: *mut u64) -> bool {
...
//
    ...
    let mut start: gfn_t = slot.base_gfn + gfn_offset + __ffs(mask);
    let mut end: gfn_t = slot.base_gfn + gfn_offset + __fls(mask);
...
#[no_mangle]
pub unsafe extern "C" fn kvm_cpu_dirty_log_size(kvm: *mut kvm) -> c_int {
    ...
    struct kvm_rmap_head *rmap_head;
    int i;
    let mut write_protected: bool = false;
    if (kvm_memslots_have_rmaps(kvm)) {
...
#[no_mangle]
unsafe extern "C" fn kvm_vcpu_write_protect_gfn(vcpu: *mut kvm_vcpu, gfn: u64) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct slot_rmap_walk_iterator {
// input fields.
    pub slot: *const kvm_memory_slot,
    pub start_gfn: gfn_t,
    pub end_gfn: gfn_t,
    pub start_level: c_int,
    pub end_level: c_int,
// output fields.
    pub gfn: gfn_t,
    pub rmap: *mut kvm_rmap_head,
    pub level: c_int,
// private field.
    pub end_rmap: *mut kvm_rmap_head,
}
...
#[no_mangle]
unsafe extern "C" fn slot_rmap_walk_okay(iterator: *mut slot_rmap_walk_iterator) -> bool {
    static bool slot_rmap_walk_okay(struct slot_rmap_walk_iterator *iterator)
    {
    return !!iterator.rmap;
    }
#[no_mangle]
unsafe extern "C" fn slot_rmap_walk_next(iterator: *mut slot_rmap_walk_iterator) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_unmap_gfn_range(kvm: *mut kvm, range: *mut kvm_gfn_range) -> bool {
...
pub const RMAP_RECYCLE_THRESHOLD: c_int = 1000;
...
#[no_mangle]
unsafe extern "C" fn kvm_may_have_shadow_mmu_sptes(kvm: *mut kvm) -> bool {
    static bool kvm_may_have_shadow_mmu_sptes(struct kvm *kvm)
    {
    return !tdp_mmu_enabled || READ_ONCE(kvm.arch.indirect_shadow_pages);
    }
#[no_mangle]
pub unsafe extern "C" fn kvm_age_gfn(kvm: *mut kvm, range: *mut kvm_gfn_range) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_test_age_gfn(kvm: *mut kvm, range: *mut kvm_gfn_range) -> bool {
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_check_sptes_at_free(sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn kvm_account_mmu_page(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn kvm_unaccount_mmu_page(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_free_shadow_page(sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn kvm_page_table_hashfn(gfn: gfn_t) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_mark_parents_unsync(sp: *mut kvm_mmu_page) {
...
#[no_mangle]
unsafe extern "C" fn mark_unsync(spte: *mut u64) {
    static void mark_unsync(u64 *spte)
    {
    struct kvm_mmu_page *sp;
    sp = sptep_to_sp(spte);
...
pub const KVM_PAGE_ARRAY_NR: c_int = 16;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct kvm_mmu_pages {
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mmu_page_and_offset {
    pub sp: *mut kvm_mmu_page,
    pub idx: c_uint,
    pub page: [}; KVM_PAGE_ARRAY_NR],
    pub nr: c_uint,
}
...
#[no_mangle]
pub unsafe extern "C" fn clear_unsync_child_bit(sp: *mut kvm_mmu_page, idx: c_int) {
...
#[no_mangle]
unsafe extern "C" fn kvm_unlink_unsync_page(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
    ...
    struct list_head *invalid_list);
#[no_mangle]
unsafe extern "C" fn sp_has_gptes(sp: *mut kvm_mmu_page) -> bool {
    static bool sp_has_gptes(struct kvm_mmu_page *sp)
    {
    if (sp.role.direct)
    return false;
...
#[no_mangle]
unsafe extern "C" fn kvm_sync_page_check(vcpu: *mut kvm_vcpu, sp: *mut kvm_mmu_page) -> bool {
    static bool kvm_sync_page_check(struct kvm_vcpu *vcpu, struct kvm_mmu_page *sp)
    {
    let mut root_role: union kvm_mmu_page_role = vcpu.arch.mmu.root_role;
//
...
#[no_mangle]
unsafe extern "C" fn kvm_sync_spte(vcpu: *mut kvm_vcpu, sp: *mut kvm_mmu_page, i: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __kvm_sync_page(vcpu: *mut kvm_vcpu, sp: *mut kvm_mmu_page) -> c_int {
    static int __kvm_sync_page(struct kvm_vcpu *vcpu, struct kvm_mmu_page *sp)
    {
    let mut flush: c_int = 0;
    int i;
    if (!kvm_sync_page_check(vcpu, sp))
    return -1;
    for (i = 0; i < SPTE_ENT_PER_PAGE; i++) {
    let mut ret: c_int = kvm_sync_spte(vcpu, sp, i);
    if (ret < -1)
    return -1;
    flush |= ret;
    }
...
//
    ...
    struct list_head *invalid_list)
    {
    let mut ret: c_int = __kvm_sync_page(vcpu, sp);
    if (ret < 0)
...
#[no_mangle]
unsafe extern "C" fn is_obsolete_sp(kvm: *mut kvm, sp: *mut kvm_mmu_page) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mmu_page_path {
    pub parent: [*mut kvm_mmu_page; PT64_ROOT_MAX_LEVEL],
    pub idx: [c_uint; PT64_ROOT_MAX_LEVEL],
}
    ...
    let mut idx: unsigned = pvec.page[n].idx;
    let mut level: c_int = sp.role.level;
...
#[no_mangle]
unsafe extern "C" fn mmu_pages_clear_parents(parents: *mut mmu_page_path) {
    static void mmu_pages_clear_parents(struct mmu_page_path *parents)
    {
    struct kvm_mmu_page *sp;
    let mut level: c_uint = 0;
    do {
    let mut idx: c_uint = parents.idx[level];
...
#[no_mangle]
unsafe extern "C" fn __clear_sp_write_flooding_count(sp: *mut kvm_mmu_page) {
    static void __clear_sp_write_flooding_count(struct kvm_mmu_page *sp)
    {
    atomic_set(&sp.write_flooding_count,  0);
    }
#[no_mangle]
unsafe extern "C" fn clear_sp_write_flooding_count(spte: *mut u64) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct shadow_page_caches {
    pub page_header_cache: *mut kvm_mmu_memory_cache,
    pub shadow_page_cache: *mut kvm_mmu_memory_cache,
    pub shadowed_info_cache: *mut kvm_mmu_memory_cache,
}
...
//
    ...
    {
    let mut role: union kvm_mmu_page_role = kvm_mmu_child_role(sptep, direct, access);
...
#[no_mangle]
unsafe extern "C" fn shadow_walk_okay(iterator: *mut kvm_shadow_walk_iterator) -> bool {
...
#[no_mangle]
unsafe extern "C" fn shadow_walk_next(iterator: *mut kvm_shadow_walk_iterator) {
...
//
    ...
    kvm_mmu_commit_zap_page(kvm, &invalid_list);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: flush) -> else {
    else if (flush)
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_unlink_parents(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
...
//
    ...
    {
    let mut total_zapped: c_ulong = 0;
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_available_pages(kvm: *mut kvm) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn make_mmu_pages_available(vcpu: *mut kvm_vcpu) -> c_int {
    static int make_mmu_pages_available(struct kvm_vcpu *vcpu)
    {
    let mut avail: c_ulong = kvm_mmu_available_pages(vcpu.kvm);
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_change_mmu_pages(kvm: *mut kvm, goal_nr_mmu_pages: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn kvm_unsync_page(kvm: *mut kvm, sp: *mut kvm_mmu_page) {
...
//
    ...
    struct kvm_mmu_page *sp = sptep_to_sp(sptep);
    let mut level: c_int = sp.role.level;
    let mut was_rmapped: c_int = 0;
    let mut ret: c_int = RET_PF_FIXED;
    let mut flush: bool = false;
    bool wrprot;
    u64 spte;
// Prefetching always gets a writable pfn.
    let mut host_writable: bool = !fault || fault.map_writable;
    let mut prefetch: bool = !fault || fault.prefetch;
    let mut write_fault: bool = fault && fault.write;
    if (is_shadow_present_pte(*sptep)) {
...
//
    ...
    u64 *start, u64 *end)
    {
    let mut gfn: gfn_t = kvm_mmu_page_get_gfn(sp, spte_index(start));
    let mut access: c_uint = sp.role.access;
...
#[no_mangle]
unsafe extern "C" fn direct_pte_prefetch(vcpu: *mut kvm_vcpu, sptep: *mut u64) {
...
//
    ...
    const struct kvm_memory_slot *slot)
    {
    let mut level: c_int = PG_LEVEL_4K;
    unsigned long hva;
    unsigned long flags;
    pgd_t pgd;
    p4d_t p4d;
...
#[no_mangle]
unsafe extern "C" fn kvm_max_level_for_order(order: c_int) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_hugepage_adjust(vcpu: *mut kvm_vcpu, fault: *mut kvm_page_fault) {
...
#[no_mangle]
pub unsafe extern "C" fn disallowed_hugepage_adjust(fault: *mut kvm_page_fault, spte: u64, cur_level: c_int) {
...
#[no_mangle]
unsafe extern "C" fn direct_map(vcpu: *mut kvm_vcpu, fault: *mut kvm_page_fault) -> c_int {
    static int direct_map(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
    {
    struct kvm_shadow_walk_iterator it;
    struct kvm_mmu_page *sp;
    int ret, access;
    let mut base_gfn: gfn_t = fault.gfn;
...
#[no_mangle]
unsafe extern "C" fn kvm_send_hwpoison_signal(slot: *mut kvm_memory_slot, gfn: gfn_t) {
    static void kvm_send_hwpoison_signal(struct kvm_memory_slot *slot, gfn_t gfn)
    {
    let mut hva: c_ulong = gfn_to_hva_memslot(slot, gfn);
    send_sig_mceerr(BUS_MCEERR_AR, (void __user *)hva, PAGE_SHIFT, current);
    }
#[no_mangle]
unsafe extern "C" fn kvm_handle_error_pfn(vcpu: *mut kvm_vcpu, fault: *mut kvm_page_fault) -> c_int {
...
//
    ...
    unsigned int access)
    {
    let mut gva: gva_t = fault.is_tdp ? 0 : fault.addr;
    if (fault.is_private) {
...
#[no_mangle]
unsafe extern "C" fn page_fault_can_be_fast(kvm: *mut kvm, fault: *mut kvm_page_fault) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn fast_page_fault(vcpu: *mut kvm_vcpu, fault: *mut kvm_page_fault) -> c_int {
    static int fast_page_fault(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
    {
    struct kvm_mmu *mmu;
    struct kvm_mmu_page *sp;
    let mut ret: c_int = RET_PF_INVALID;
    u64 spte;
    u64 *sptep;
...
// roots_to_free must be some combination of the KVM_MMU_ROOT_* flags
    ...
    ulong roots_to_free)
    {
    let mut is_tdp_mmu: bool = tdp_mmu_enabled && mmu.root_role.direct;
    int i;
    LIST_HEAD(invalid_list);
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_free_guest_mode_roots(kvm: *mut kvm, mmu: *mut kvm_mmu) {
    void kvm_mmu_free_guest_mode_roots(struct kvm *kvm, struct kvm_mmu *mmu)
    {
    let mut roots_to_free: c_ulong = 0;
    struct kvm_mmu_page *sp;
    hpa_t root_hpa;
    int i;
//
...
//
    ...
    u8 level)
    {
    let mut role: union kvm_mmu_page_role = vcpu.arch.mmu.root_role;
    struct kvm_mmu_page *sp;
    role.level = level;
...
#[no_mangle]
unsafe extern "C" fn mmu_alloc_direct_roots(vcpu: *mut kvm_vcpu) -> c_int {
    static int mmu_alloc_direct_roots(struct kvm_vcpu *vcpu)
    {
    struct kvm_mmu *mmu = vcpu.arch.mmu;
    let mut shadow_root_level: u8 = mmu.root_role.level;
    hpa_t root;
    unsigned i;
    int r;
    if (tdp_mmu_enabled) {
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_alloc_page_hash(kvm: *mut kvm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mmu_first_shadow_root_alloc(kvm: *mut kvm) -> c_int {
    static int mmu_first_shadow_root_alloc(struct kvm *kvm)
    {
    struct kvm_memslots *slots;
    struct kvm_memory_slot *slot;
    let mut r: c_int = 0, i, bkt;
//
...
#[no_mangle]
unsafe extern "C" fn mmu_alloc_shadow_roots(vcpu: *mut kvm_vcpu) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(PT64_ROOT_4LEVEL: mmu->root_role.level ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn mmu_alloc_special_roots(vcpu: *mut kvm_vcpu) -> c_int {
    static int mmu_alloc_special_roots(struct kvm_vcpu *vcpu)
    {
    struct kvm_mmu *mmu = vcpu.arch.mmu;
    struct kvm_pagewalk *w = mmu.w;
    let mut need_pml5: bool = mmu.root_role.level > PT64_ROOT_4LEVEL;
...
#[no_mangle]
unsafe extern "C" fn is_unsync_root(root: hpa_t) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_sync_roots(vcpu: *mut kvm_vcpu) {
    ...
    let mut root: hpa_t = vcpu.arch.mmu.root.hpa;
    ...
    write_lock(&vcpu.kvm.mmu_lock);
    for (i = 0; i < 4; ++i) {
    let mut root: hpa_t = vcpu.arch.mmu.pae_root[i];
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_sync_prev_roots(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
unsafe extern "C" fn mmio_info_in_cache(vcpu: *mut kvm_vcpu, addr: u64, direct: bool) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn get_walk(vcpu: *mut kvm_vcpu, addr: u64, sptes: *mut u64, root_level: *mut c_int) -> c_int {
    static int get_walk(struct kvm_vcpu *vcpu, u64 addr, u64 *sptes, int *root_level)
    {
    struct kvm_shadow_walk_iterator iterator;
    let mut leaf: c_int = -1;
    u64 spte;
...
#[no_mangle]
unsafe extern "C" fn get_mmio_spte(vcpu: *mut kvm_vcpu, addr: u64, sptep: *mut u64) -> bool {
...
#[no_mangle]
unsafe extern "C" fn handle_mmio_page_fault(vcpu: *mut kvm_vcpu, addr: u64, direct: bool) -> c_int {
    ...
    let mut gfn: gfn_t = get_mmio_spte_gfn(spte);
    let mut access: c_uint = get_mmio_spte_access(spte);
...
#[no_mangle]
unsafe extern "C" fn shadow_page_table_clear_flood(vcpu: *mut kvm_vcpu, addr: gva_t) {
...
#[no_mangle]
unsafe extern "C" fn alloc_apf_token(vcpu: *mut kvm_vcpu) -> u32 {
    static u32 alloc_apf_token(struct kvm_vcpu *vcpu)
    {
// make sure the token value is not 0
    let mut id: u32 = vcpu.arch.apf.id;
    if (id << 12 == 0)
    vcpu.arch.apf.id = 1;
    ...
    struct kvm_page_fault *fault)
    {
    let mut foll: c_uint = fault.write ? FOLL_WRITE : 0;
...
#[no_mangle]
unsafe extern "C" fn direct_page_fault(vcpu: *mut kvm_vcpu, fault: *mut kvm_page_fault) -> c_int {
...
// This path builds a PAE pagetable, we can map 2mb pages at maximum.
    ...
    {
    let mut r: c_int = 1;
    let mut flags: u32 = vcpu.arch.apf.host_apf_flags;
...
#[no_mangle]
unsafe extern "C" fn kvm_tdp_page_fault(vcpu: *mut kvm_vcpu, fault: *mut kvm_page_fault) -> c_int {
...
//
    ...
    {
    let mut error_code: u64 = PFERR_GUEST_FINAL_MASK;
    let mut level: u8 = PG_LEVEL_4K;
    u64 direct_bits;
    u64 end;
    int r;
...
#[no_mangle]
pub unsafe extern "C" fn kvm_arch_async_page_ready(vcpu: *mut kvm_vcpu, work: *mut kvm_async_pf) {
...
#[no_mangle]
unsafe extern "C" fn kvm_assert_gmem_invalidate_lock_held(slot: *mut kvm_memory_slot) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_tdp_mmu_map_private_pfn(vcpu: *mut kvm_vcpu, gfn: gfn_t, pfn: kvm_pfn_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nonpaging_init_context(context: *mut kvm_mmu) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_new_pgd(vcpu: *mut kvm_vcpu, new_pgd: gpa_t) {
    void kvm_mmu_new_pgd(struct kvm_vcpu *vcpu, gpa_t new_pgd)
    {
    struct kvm_mmu *mmu = vcpu.arch.mmu;
    let mut new_role: union kvm_mmu_page_role = mmu.root_role;
//
...
pub const PTTYPE: c_int = 64;

pub const PTTYPE: c_int = 32;

    static void __reset_rsvds_bits_mask(struct kvm_page_format *fmt,
    u64 pa_bits_rsvd, int level, bool nx,
    bool gbpages, bool pse, bool amd)
    {
    let mut gbpages_bit_rsvd: u64 = 0;
    let mut nonleaf_bit8_rsvd: u64 = 0;
    u64 high_bits_rsvd;
    fmt.bad_mt_xwr = 0;
    if (!gbpages)
...
// 32 bits PSE 4MB page
    ...
    int huge_page_level)
    {
    let mut high_bits_rsvd: u64 = pa_bits_rsvd & rsvd_bits(0, 51);
    let mut large_1g_rsvd: u64 = 0, large_2m_rsvd = 0;
    u64 bad_mt_xwr;
...
#[no_mangle]
pub unsafe extern "C" fn reserved_hpa_bits() -> u64 {
...
// @amd adds a check on bit of SPTEs, which KVM shouldn't use anyways.
    let mut is_amd: bool = true;
// KVM doesn't use 2-level page tables for the shadow MMU.
    let mut is_pse: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn boot_cpu_is_amd() -> bool {
    static inline bool boot_cpu_is_amd(void)
    {
    WARN_ON_ONCE(!tdp_enabled);
    let mut shadow_xs_mask: return = = 0;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn reset_tdp_shadow_zero_bits_mask(context: *mut kvm_mmu) {
...
//
    ...
    {
    unsigned index;
    let mut w: u16 = ACC_BITS_MASK(ACC_WRITE_MASK);
    let mut r: u16 = ACC_BITS_MASK(ACC_READ_MASK);
//
...
//
    ...
    let mut pfec: unsigned = index << 1;
//
// Each "*f" variable has a 1 bit for each ACC_* combo
// that causes a fault with the given PFEC.
//
// Faults from reads to non-readable pages
    let mut rf: u16 = (pfec & (PFERR_WRITE_MASK|PFERR_FETCH_MASK)) ? 0 : (u16)~r;
// Faults from writes to non-writable pages
    let mut wf: u16 = (pfec & PFERR_WRITE_MASK) ? (u16)~w : 0;
...
// Faults from kernel mode accesses of user pages
    let mut smapf: u16 = 0;
    if (ept) {
    let mut xs: u16 = ACC_BITS_MASK(ACC_EXEC_MASK);
    let mut xu: u16 = ACC_BITS_MASK(ACC_USER_EXEC_MASK);
...
// Ignore XU unless MBEC is enabled.
    ...
    let mut x: u16 = ACC_BITS_MASK(ACC_EXEC_MASK);
    let mut u: u16 = ACC_BITS_MASK(ACC_USER_MASK);
// Faults from kernel mode accesses to user pages
    let mut kf: u16 = (pfec & PFERR_USER_MASK) ? 0 : u;
//
...
#[no_mangle]
unsafe extern "C" fn update_permission_bitmask(w: *mut kvm_pagewalk, tdp: bool, ept: bool) {
...
#[no_mangle]
unsafe extern "C" fn update_spte_permission_bitmask(mmu: *mut kvm_mmu, tdp: bool, ept: bool) {
...
//
#[no_mangle]
unsafe extern "C" fn update_pkru_bitmask(w: *mut kvm_pagewalk) {
...
#[no_mangle]
unsafe extern "C" fn paging64_init_context(context: *mut kvm_mmu) {
...
#[no_mangle]
unsafe extern "C" fn paging32_init_context(context: *mut kvm_mmu) {
    ...
    {
    let mut role: union kvm_cpu_role = {0};
    ...
    : PT64_ROOT_4LEVEL;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ____is_cr4_pae(regs)) -> else {
...
// PKEY and LA57 are active iff long mode is active.
    ...
    struct kvm_pagewalk *w)
    {
    let mut cr0_wp: bool = kvm_is_cr0_bit_set(vcpu, X86_CR0_WP);
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_get_tdp_level(vcpu: *mut kvm_vcpu) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_get_max_tdp_level() -> u8 {
    ...
    {
    let mut role: union kvm_mmu_page_role = {0};
    role.cr0_wp = true;
...
// All TDP pages are supervisor-executable
    ...
    let mut root_role: union kvm_mmu_page_role = kvm_calc_tdp_mmu_root_page_role(vcpu, cpu_role);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_cr4_pae(context->w)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_cr4_pae(w)) -> else {
    ...
    let mut cpu_role: union kvm_cpu_role = kvm_calc_cpu_role(vcpu, &regs);
    union kvm_mmu_page_role root_role;
...
//
    ...
    {
    struct kvm_mmu *context = &vcpu.arch.guest_mmu;
    let mut level: u8 = vmx_eptp_page_walk_level(new_eptp);
...
#[no_mangle]
pub unsafe extern "C" fn kvm_init_mmu(vcpu: *mut kvm_vcpu) {
    void kvm_init_mmu(struct kvm_vcpu *vcpu)
    {
    let mut regs: kvm_mmu_role_regs = vcpu_to_role_regs(vcpu);
    let mut cpu_role: union kvm_cpu_role = kvm_calc_cpu_role(vcpu, &regs);
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_after_set_cpuid(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_reset_context(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_load(vcpu: *mut kvm_vcpu) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_unload(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
unsafe extern "C" fn is_obsolete_root(kvm: *mut kvm, root_hpa: hpa_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn __kvm_mmu_free_obsolete_roots(kvm: *mut kvm, mmu: *mut kvm_mmu) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_free_obsolete_roots(vcpu: *mut kvm_vcpu) {
...
//
#[no_mangle]
unsafe extern "C" fn detect_write_flooding(sp: *mut kvm_mmu_page) -> bool {
...
// nspte = 2;
    ...
    int bytes)
    {
    let mut gfn: gfn_t = gpa >> PAGE_SHIFT;
    struct kvm_mmu_page *sp;
    LIST_HEAD(invalid_list);
...
#[no_mangle]
unsafe extern "C" fn is_write_to_guest_page_table(error_code: u64) -> bool {
    static bool is_write_to_guest_page_table(u64 error_code)
    {
    let mut mask: u64 = PFERR_GUEST_PAGE_MASK | PFERR_WRITE_MASK | PFERR_PRESENT_MASK;
    ...
    {
    let mut direct: bool = vcpu.arch.mmu.root_role.direct;
//
...
//
    ...
    vcpu.stat.pf_fixed++;
#[no_mangle]
pub unsafe extern "C" fn if(RET_PF_EMULATE: r ==) -> else {
    else if (r == RET_PF_EMULATE)
    vcpu.stat.pf_emulate++;
#[no_mangle]
pub unsafe extern "C" fn if(RET_PF_SPURIOUS: r ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_print_sptes(vcpu: *mut kvm_vcpu, gpa: gpa_t, msg: *const c_char) {
...
//
    ...
    if (sp.unsync) {
    let mut ret: c_int = kvm_sync_spte(vcpu, sp, iterator.index);
    if (ret < 0)
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_invlpg(vcpu: *mut kvm_vcpu, gva: gva_t) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_invpcid_gva(vcpu: *mut kvm_vcpu, gva: gva_t, pcid: c_ulong) {
    void kvm_mmu_invpcid_gva(struct kvm_vcpu *vcpu, gva_t gva, unsigned long pcid)
    {
    struct kvm_mmu *mmu = vcpu.arch.mmu;
    let mut roots: c_ulong = 0;
    uint i;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: boot_cpu_has(X86_FEATURE_GBPAGES)) -> else {
...
#[no_mangle]
unsafe extern "C" fn free_mmu_pages(mmu: *mut kvm_mmu) {
...
#[no_mangle]
unsafe extern "C" fn __kvm_mmu_create(vcpu: *mut kvm_vcpu, mmu: *mut kvm_mmu, w: *mut kvm_pagewalk) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_create(vcpu: *mut kvm_vcpu) -> c_int {
...
pub const BATCH_ZAP_PAGES: c_int = 10;
#[no_mangle]
unsafe extern "C" fn kvm_zap_obsolete_pages(kvm: *mut kvm) {
...
#[no_mangle]
unsafe extern "C" fn __kvm_mmu_zap_all_fast_front_half(kvm: *mut kvm) {
...
#[no_mangle]
unsafe extern "C" fn __kvm_mmu_zap_all_fast_back_half(kvm: *mut kvm) {
...
//
#[no_mangle]
unsafe extern "C" fn kvm_mmu_zap_all_fast(kvm: *mut kvm) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_init_vm(kvm: *mut kvm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mmu_free_vm_memory_caches(kvm: *mut kvm) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_uninit_vm(kvm: *mut kvm) {
...
#[no_mangle]
unsafe extern "C" fn kvm_rmap_zap_gfn_range(kvm: *mut kvm, gfn_start: gfn_t, gfn_end: gfn_t) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_zap_gfn_range(kvm: *mut kvm, gfn_start: gfn_t, gfn_end: gfn_t) {
...
#[no_mangle]
pub unsafe extern "C" fn need_topup(cache: *mut kvm_mmu_memory_cache, min: c_int) -> bool {
    static inline bool need_topup(struct kvm_mmu_memory_cache *cache, int min)
    {
    return kvm_mmu_memory_cache_nr_free_objects(cache) < min;
    }
#[no_mangle]
unsafe extern "C" fn need_topup_split_caches_or_resched(kvm: *mut kvm) -> bool {
...
#[no_mangle]
unsafe extern "C" fn topup_split_caches(kvm: *mut kvm) -> c_int {
...
//
    ...
    let mut caches: shadow_page_caches = {};
    union kvm_mmu_page_role role;
    unsigned int access;
    gfn_t gfn;
...
// Safe to pass NULL for vCPU since requesting a direct SP.
    ...
    let mut huge_spte: u64 = READ_ONCE(*huge_sptep);
    struct kvm_mmu_page *sp;
    let mut flush: bool = false;
    u64 *sptep, spte;
    gfn_t gfn;
...
//
    ...
    int target_level)
    {
    let mut start: u64 = memslot.base_gfn;
    let mut end: u64 = start + memslot.npages;
    if (!tdp_mmu_enabled)
    return;
    if (kvm_memslots_have_rmaps(kvm)) {
    write_lock(&kvm.mmu_lock);
...
//
    ...
    u64 *sptep;
    struct rmap_iterator iter;
    let mut need_tlb_flush: c_int = 0;
    struct kvm_mmu_page *sp;
    restart:
    for_each_rmap_spte(rmap_head, &iter, sptep) {
    sp = sptep_to_sp(sptep);
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_zap_all(kvm: *mut kvm) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_arch_flush_shadow_all(kvm: *mut kvm) {
...
//
    ...
    struct kvm_mmu_page *sp;
    let mut gfn: gfn_t = slot.base_gfn + i;
    for_each_gfn_valid_sp_with_gptes(kvm, sp, gfn)
    kvm_mmu_prepare_zap_page(kvm, sp, &invalid_list);
    if (need_resched() || rwlock_needbreak(&kvm.mmu_lock)) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_invalidate_mmio_sptes(kvm: *mut kvm, gen: u64) {
...
#[no_mangle]
unsafe extern "C" fn mmu_destroy_caches() {
...
#[no_mangle]
unsafe extern "C" fn kvm_wake_nx_recovery_thread(kvm: *mut kvm) {
...
#[no_mangle]
unsafe extern "C" fn get_nx_huge_pages(buffer: *mut c_char, kp: *const kernel_param) -> c_int {
    static int get_nx_huge_pages(char *buffer, const struct kernel_param *kp)
    {
    let mut val: c_int = *(int *)kp.arg;
...
#[no_mangle]
unsafe extern "C" fn get_nx_auto_mode() -> bool {
...
#[no_mangle]
unsafe extern "C" fn __set_nx_huge_pages(val: bool) {
    static void __set_nx_huge_pages(bool val)
    {
    nx_huge_pages = itlb_multihit_kvm_mitigation = val;
    }
#[no_mangle]
unsafe extern "C" fn set_nx_huge_pages(val: *const c_char, kp: *const kernel_param) -> c_int {
    static int set_nx_huge_pages(const char *val, const struct kernel_param *kp)
    {
    let mut old_val: bool = nx_huge_pages;
    bool new_val;
    if (nx_hugepage_mitigation_hard_disabled)
    return -EPERM;
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_x86_module_init() -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_vendor_module_init() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_destroy(vcpu: *mut kvm_vcpu) {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_vendor_module_exit() {
...
//
#[no_mangle]
unsafe extern "C" fn calc_nx_huge_pages_recovery_period(period: *mut c_uint) -> bool {
...
//
    let mut enabled: bool = READ_ONCE(nx_huge_pages);
    let mut ratio: c_uint = READ_ONCE(nx_huge_pages_recovery_ratio);
    if (!enabled || !ratio)
    return false;
...
#[no_mangle]
unsafe extern "C" fn set_nx_huge_pages_recovery_param(val: *const c_char, kp: *const kernel_param) -> c_int {
    static int set_nx_huge_pages_recovery_param(const char *val, const struct kernel_param *kp)
    {
    ...
    enum kvm_mmu_type mmu_type)
    {
    let mut pages: c_ulong = READ_ONCE(kvm.arch.possible_nx_huge_pages[mmu_type].nr_pages);
    let mut ratio: c_uint = READ_ONCE(nx_huge_pages_recovery_ratio);
...
//
    ...
    let mut is_tdp_mmu: bool = mmu_type == KVM_TDP_MMU;
    spinlock_t *tdp_mmu_pages_lock = &kvm.arch.tdp_mmu_pages_lock;

    let mut is_tdp_mmu: bool = false;
    spinlock_t *tdp_mmu_pages_lock = core::ptr::null_mut();

    let mut to_zap: c_ulong = nx_huge_pages_to_zap(kvm, mmu_type);
...
#[no_mangle]
unsafe extern "C" fn kvm_nx_huge_page_recovery_worker_kill(data: *mut c_void) {
    static void kvm_nx_huge_page_recovery_worker_kill(void *data)
    {
    }
#[no_mangle]
unsafe extern "C" fn kvm_nx_huge_page_recovery_worker(data: *mut c_void) -> bool {
...
#[no_mangle]
unsafe extern "C" fn kvm_mmu_start_lpage_recovery(once: *mut once) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_post_init_vm(kvm: *mut kvm) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_mmu_pre_destroy_vm(kvm: *mut kvm) {
...
//
    ...
    let mut start: gfn_t = gfn_round_for_level(range.start, level);
    let mut end: gfn_t = gfn_round_for_level(range.end - 1, level);
    let mut nr_pages: gfn_t = KVM_PAGES_PER_HPAGE(level);
...
// Unmap the old attribute page.
    ...
    {
    let mut start: c_ulong = gfn;
    let mut end: c_ulong = start + KVM_PAGES_PER_HPAGE(level);
    ...
    struct kvm_gfn_range *range)
    {
    let mut attrs: c_ulong = range.arg.attributes;
    struct kvm_memory_slot *slot = range.slot;
    int level;
    lockdep_assert_held_write(&kvm.mmu_lock);
    lockdep_assert_held(&kvm.slots_lock);
...
//
    for (level = PG_LEVEL_2M; level <= KVM_MAX_HUGEPAGE_LEVEL; level++) {
    let mut nr_pages: gfn_t = KVM_PAGES_PER_HPAGE(level);
    let mut gfn: gfn_t = gfn_round_for_level(range.start, level);
...
//
    let mut end: gfn_t = gfn_round_for_level(slot.base_gfn + slot.npages, level);
    let mut start: gfn_t = gfn_round_for_level(slot.base_gfn, level);
    let mut nr_pages: gfn_t = KVM_PAGES_PER_HPAGE(level);
    gfn_t gfn;
...
//
    ...
    let mut attrs: c_ulong = kvm_get_memory_attributes(kvm, gfn);
    if (hugepage_has_attrs(kvm, slot, gfn, level, attrs))
    hugepage_clear_mixed(slot, gfn, level);
    else
    hugepage_set_mixed(slot, gfn, level);
