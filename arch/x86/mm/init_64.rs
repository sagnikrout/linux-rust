...
#[no_mangle]
pub unsafe extern "C" fn prot_sethuge(prot: pgprot_t) -> pgprot_t {
...
// Bits supported by the hardware:
    let mut __read_mostly: pteval_t __supported_pte_mask = ~0;
...
//
#[no_mangle]
unsafe extern "C" fn nonx32_setup(str: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn sync_global_pgds_l5(start: c_ulong, end: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn sync_global_pgds_l4(start: c_ulong, end: c_ulong) {
...
//
#[no_mangle]
unsafe extern "C" fn sync_global_pgds(start: c_ulong, end: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn __set_pte_vaddr(pud: *mut pud_t, vaddr: c_ulong, new_pte: pte_t) {
...
#[no_mangle]
pub unsafe extern "C" fn init_extra_mapping_wb(phys: c_ulong, size: c_ulong) -> void __init {
    void __init init_extra_mapping_wb(unsigned long phys, unsigned long size)
    {
    __init_extra_mapping(phys, size, _PAGE_CACHE_MODE_WB);
    }
#[no_mangle]
pub unsafe extern "C" fn init_extra_mapping_uc(phys: c_ulong, size: c_ulong) -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn cleanup_highmap() -> void __init {
    void __init cleanup_highmap(void)
    {
    let mut vaddr: c_ulong = __START_KERNEL_map;
    let mut vaddr_end: c_ulong = __START_KERNEL_map + KERNEL_IMAGE_SIZE;
    let mut end: c_ulong = roundup((unsigned long)_brk_end, PMD_SIZE) - 1;
...
//
    ...
    {
    let mut pages: c_ulong = 0, paddr_next;
    let mut paddr_last: c_ulong = paddr_end;
    pte_t *pte;
    int i;
...
//
    ...
    let mut pages: c_ulong = 0, paddr_next;
    let mut paddr_last: c_ulong = paddr_end;
    let mut i: c_int = pmd_index(paddr);
    for (; i < PTRS_PER_PMD; i++, paddr = paddr_next) {
    pmd_t *pmd = pmd_page + pmd_index(paddr);
    pte_t *pte;
    let mut new_prot: pgprot_t = prot;
...
//
    ...
    let mut i: c_int = pud_index(vaddr);
    for (; i < PTRS_PER_PUD; i++, paddr = paddr_next) {
    pud_t *pud;
    pmd_t *pmd;
    let mut prot: pgprot_t = _prot;
...
//
    ...
    pgprot_t prot, bool init)
    {
    let mut pgd_changed: bool = false;
...
#[no_mangle]
unsafe extern "C" fn x86_numa_init() -> __always_inline void {
...
#[no_mangle]
unsafe extern "C" fn vmemmap_flush_unused_pmd() -> void __meminit {
...
#[no_mangle]
unsafe extern "C" fn __vmemmap_use_sub_pmd(start: c_ulong) -> void __meminit {
...
#[no_mangle]
unsafe extern "C" fn vmemmap_use_sub_pmd(start: c_ulong, end: c_ulong) -> void __meminit {
...
#[no_mangle]
unsafe extern "C" fn vmemmap_use_new_sub_pmd(start: c_ulong, end: c_ulong) -> void __meminit {
...
unsafe extern "C" fn update_end_of_memory_vars(start: u64, size: u64) {
    ...
    {
    let mut end: c_ulong = ((start_pfn + nr_pages) << PAGE_SHIFT) - 1;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn free_pagetable(page: *mut page) -> void __meminit {
    ...
    {
    let mut nr_pages: c_ulong = 1u << order;
    if (altmap)
    vmem_altmap_free(altmap, nr_pages);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: PageReserved(page)) -> else {
...
#[no_mangle]
unsafe extern "C" fn free_pmd_table(pmd_start: *mut pmd_t, pud: *mut pud_t) -> void __meminit {
...
// We never populate base pages from the altmap.
    free_vmemmap_pages(pte_page(*pte), 0, core::ptr::null_mut());
    spin_lock(&init_mm.page_table_lock);
...
//
#[no_mangle]
unsafe extern "C" fn preallocate_vmalloc_pages() -> void __init {
...
pub unsafe extern "C" fn mark_rodata_ro() {
    ...
    let mut end: c_ulong = (unsigned long)__end_rodata_hpage_align;
    let mut text_end: c_ulong = PFN_ALIGN(_etext);
...
#[no_mangle]
pub unsafe extern "C" fn set_memory_block_size_order(order: c_uint) -> int __init {
    int __init set_memory_block_size_order(unsigned int order)
    {
    let mut size: c_ulong = 1UL << order;
...
#[no_mangle]
unsafe extern "C" fn probe_memory_block_size() -> c_ulong {
    static unsigned long probe_memory_block_size(void)
    {
    let mut boot_mem_end: c_ulong = max_pfn << PAGE_SHIFT;
    unsigned long bz;
...
// check to see if we have contiguous blocks
    ...
    unsigned long addr, unsigned long next)
    {
    let mut large: c_int = pmd_leaf(*pmd);
    if (pmd_leaf(*pmd)) {
    ...
    err = vmemmap_populate_basepages(start, end, node, core::ptr::null_mut());
#[no_mangle]
pub unsafe extern "C" fn if(_arg: boot_cpu_has(X86_FEATURE_PSE)) -> else {
    else if (boot_cpu_has(X86_FEATURE_PSE))
    err = vmemmap_populate_hugepages(start, end, node, altmap);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: altmap) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn vmemmap_populate_print_last() -> void __meminit {
