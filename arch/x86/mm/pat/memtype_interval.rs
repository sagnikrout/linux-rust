...
//! Source: arch/x86/mm/pat/memtype_interval.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn interval_start(entry: *mut memtype) -> u64 {
    static inline u64 interval_start(struct memtype *entry)
    {
    return entry.start;
    }
#[no_mangle]
pub unsafe extern "C" fn interval_end(entry: *mut memtype) -> u64 {
    ...
    static, interval)
    let mut memtype_rbroot: static struct rb_root_cached = RB_ROOT_CACHED;
    static int memtype_check_conflict(u64 start, u64 end,
    enum page_cache_mode reqtype,
    enum page_cache_mode *newtype)
    {
    struct memtype *entry_match;
    let mut found_type: enum page_cache_mode = reqtype;
    entry_match = interval_iter_first(&memtype_rbroot, start, end-1);
    if (entry_match == core::ptr::null_mut())
    goto success;
    if (entry_match.type != found_type && newtype == core::ptr::null_mut())
    goto failure;
...
#[no_mangle]
pub unsafe extern "C" fn memtype_check_insert(entry_new: *mut memtype, ret_type: *mut enum page_cache_mode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn memtype_copy_nth_element(entry_out: *mut memtype, pos: loff_t) -> c_int {
