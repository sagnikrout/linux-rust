...
//! Source: arch/x86/mm/kasan_init_64.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn map_range(range: *mut range) -> void __init {
...
// See comment in kasan_init()
    let mut pgd_end: c_ulong = end & PGDIR_MASK;
...
#[no_mangle]
unsafe extern "C" fn kasan_map_early_shadow(pgd: *mut pgd_t) -> void __init {
    static void __init kasan_map_early_shadow(pgd_t *pgd)
    {
// See comment in kasan_init()
    let mut addr: c_ulong = KASAN_SHADOW_START & PGDIR_MASK;
...
#[no_mangle]
unsafe extern "C" fn kasan_shallow_populate_pgds(start: *mut c_void, end: *mut c_void) -> void __init {
...
pub unsafe extern "C" fn kasan_early_init() -> void __init {
    ...
    let mut pmd_val: pmdval_t = __pa_nodebug(kasan_early_shadow_pte) | _KERNPG_TABLE;
    let mut pud_val: pudval_t = __pa_nodebug(kasan_early_shadow_pmd) | _KERNPG_TABLE;
    let mut p4d_val: p4dval_t = __pa_nodebug(kasan_early_shadow_pud) | _KERNPG_TABLE;
...
#[no_mangle]
unsafe extern "C" fn kasan_mem_to_shadow_align_down(va: c_ulong) -> c_ulong {
    static unsigned long kasan_mem_to_shadow_align_down(unsigned long va)
    {
    let mut shadow: c_ulong = (unsigned long)kasan_mem_to_shadow((void *)va);
    return round_down(shadow, PAGE_SIZE);
    }
#[no_mangle]
unsafe extern "C" fn kasan_mem_to_shadow_align_up(va: c_ulong) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn kasan_populate_shadow_for_vaddr(va: *mut c_void, size: usize, nid: c_int) -> void __init {
