...
//! Source: arch/um/drivers/virtio_uml.c
#![no_std]
...
pub const MAX_SUPPORTED_QUEUE_SIZE: c_int = 256;

    container_of(_vdev, struct virtio_uml_device, vdev)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct virtio_uml_platform_data {
    pub virtio_device_id: u32,
    pub socket_path: *const c_char,
    pub conn_broken_wk: work_struct,
    pub pdev: *mut platform_device,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct virtio_uml_device {
    pub vdev: virtio_device,
    pub pdev: *mut platform_device,
    pub pdata: *mut virtio_uml_platform_data,
    pub sock_lock: raw_spinlock_t,
    pub irq: int sock, req_fd,,
    pub features: u64,
    ...
    pub registered:1: u8,
    pub suspended:1: u8,
    pub no_vq_suspend:1: u8,
    pub config_changed_irq:1: u8,
    pub vq_irq_vq_map: u64,
    pub recv_rc: c_int,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct virtio_uml_vq_info {
    pub call_fd: int kick_fd,,
    pub name: [c_char; 32],
    pub suspended: bool,
}
...
#[no_mangle]
unsafe extern "C" fn full_read(fd: c_int, buf: *mut c_void, len: c_int, abortable: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhost_user_recv_header(fd: c_int, msg: *mut vhost_user_msg) -> c_int {
...
// value = msg.payload.integer;
    ...
    {
    let mut size: usize = sizeof(msg.header) + msg.header.size;
    unsigned long flags;
    bool request_ack;
    int rc;
...
//
    ...
    return vhost_user_send(vu_dev, need_response, &msg, core::ptr::null_mut(), 0);
    }
    ...
    return vhost_user_send(vu_dev, false, &msg, core::ptr::null_mut(), 0);
    }
#[no_mangle]
unsafe extern "C" fn vhost_user_set_owner(vu_dev: *mut virtio_uml_device) -> c_int {
    ...
    let mut size: usize = sizeof(reply.header) + sizeof(reply.payload.integer);
    int rc;
    ...
    rc = full_sendmsg_fds(vu_dev.req_fd, &reply, size, core::ptr::null_mut(), 0);
    if (rc)
    vu_err(vu_dev,
...
#[no_mangle]
unsafe extern "C" fn vu_req_interrupt(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t vu_req_interrupt(int irq, void *data)
    {
    struct virtio_uml_device *vu_dev = data;
    let mut ret: irqreturn_t = IRQ_HANDLED;
    if (!um_irq_timetravel_handler_used())
    ret = vu_req_read_message(vu_dev, core::ptr::null_mut());
    if (vu_dev.recv_rc) {
...
#[no_mangle]
unsafe extern "C" fn vhost_user_init_slave_req(vu_dev: *mut virtio_uml_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhost_user_init(vu_dev: *mut virtio_uml_device) -> c_int {
    static int vhost_user_init(struct virtio_uml_device *vu_dev)
    {
    let mut rc: c_int = vhost_user_set_owner(vu_dev);
    if (rc)
    return rc;
    ...
    {
    let mut cfg_size: u32 = offset + len;
    struct vhost_user_msg *msg;
    let mut payload_size: usize = sizeof(msg.payload.config) + cfg_size;
    let mut msg_size: usize = sizeof(msg.header) + payload_size;
    int rc;
    if (!(vu_dev.protocol_features &
    ...
    rc = vhost_user_send(vu_dev, true, msg, core::ptr::null_mut(), 0);
    if (rc) {
    ...
    {
    struct vhost_user_msg *msg;
    let mut payload_size: usize = sizeof(msg.payload.config) + len;
    let mut msg_size: usize = sizeof(msg.header) + payload_size;
    int rc;
    if (!(vu_dev.protocol_features &
    BIT_ULL(VHOST_USER_PROTOCOL_F_CONFIG)))
    ...
    rc = vhost_user_send(vu_dev, false, msg, core::ptr::null_mut(), 0);
    if (rc)
    ...
    let mut rc: c_int = phys_mapping(addr, &mem_offset);
...
#[no_mangle]
unsafe extern "C" fn vhost_user_set_mem_table(vu_dev: *mut virtio_uml_device) -> c_int {
    ...
    let mut reserved: c_ulong = uml_reserved - uml_physmem;
    int fds[2];
    int rc;
//
...
// Virtio interface
#[no_mangle]
unsafe extern "C" fn vu_notify(vq: *mut virtqueue) -> bool {
...
#[no_mangle]
unsafe extern "C" fn vu_interrupt(irq: c_int, opaque: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn vu_get_status(vdev: *mut virtio_device) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn vu_set_status(vdev: *mut virtio_device, status: u8) {
...
#[no_mangle]
unsafe extern "C" fn vu_reset(vdev: *mut virtio_device) {
...
#[no_mangle]
unsafe extern "C" fn vu_del_vq(vq: *mut virtqueue) {
...
#[no_mangle]
unsafe extern "C" fn vu_del_vqs(vdev: *mut virtio_device) {
...
// Close (unused) write end of call fds
    ...
    struct virtqueue *vq;
    let mut num: c_int = MAX_SUPPORTED_QUEUE_SIZE;
    int rc;
...
#[no_mangle]
unsafe extern "C" fn vu_get_features(vdev: *mut virtio_device) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn vu_finalize_features(vdev: *mut virtio_device) -> c_int {
    static int vu_finalize_features(struct virtio_device *vdev)
    {
    struct virtio_uml_device *vu_dev = to_virtio_uml_device(vdev);
    let mut supported: u64 = vdev.features & VHOST_USER_SUPPORTED_F;
...
#[no_mangle]
unsafe extern "C" fn virtio_uml_release_dev(d: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn vu_of_conn_broken(wk: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn virtio_uml_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_uml_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn vu_cmdline_release_dev(d: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn vu_unregister_cmdline_device(dev: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vu_conn_broken(wk: *mut work_struct) {
    ...
    vu_dev = platform_get_drvdata(pdata.pdev);
    virtio_break_device(&vu_dev.vdev);
    vu_unregister_cmdline_device(&pdata.pdev.dev, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn vu_cmdline_set_device(device: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vu_cmdline_set(device: *const c_char, kp: *const kernel_param) -> c_int {
    static int vu_cmdline_set(const char *device, const struct kernel_param *kp)
    {
    return vu_cmdline_set_device(device);
    }
#[no_mangle]
unsafe extern "C" fn vu_cmdline_get_device(dev: *mut device, data: *mut c_void) -> c_int {
    static int vu_cmdline_get_device(struct device *dev, void *data)
    {
    struct platform_device *pdev = to_platform_device(dev);
    struct virtio_uml_platform_data *pdata = pdev.dev.platform_data;
    char *buffer = data;
    let mut len: c_uint = strlen(buffer);
    snprintf(buffer + len, PAGE_SIZE - len, "%s:%d:%d\n",
    pdata.socket_path, pdata.virtio_device_id, pdev.id);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn vu_cmdline_get(buffer: *mut c_char, kp: *const kernel_param) -> c_int {
    ...
    device_param_cb(device, &vu_cmdline_param_ops, core::ptr::null_mut(), S_IRUSR);
...
#[no_mangle]
unsafe extern "C" fn vu_unregister_cmdline_devices() {
    static void vu_unregister_cmdline_devices(void)
    {
    guard(mutex)(&vu_cmdline_lock);
    if (vu_cmdline_parent_registered) {
    device_for_each_child(&vu_cmdline_parent, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn vu_mc_config(str: *mut c_char, error_out: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vu_mc_id(str: *mut c_char, start_out: *mut c_int, end_out: *mut c_int) -> c_int {
    static int vu_mc_id(char **str, int *start_out, int *end_out)
    {
    return -EOPNOTSUPP;
    }
#[no_mangle]
unsafe extern "C" fn vu_mc_remove(n: c_int, error_out: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_uml_mc_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn virtio_uml_suspend(pdev: *mut platform_device, state: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_uml_resume(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_uml_init() -> int __init {
    static int __init virtio_uml_init(void)
    {
    return platform_driver_register(&virtio_uml_driver);
    }
#[no_mangle]
unsafe extern "C" fn virtio_uml_exit() -> void __exit {
