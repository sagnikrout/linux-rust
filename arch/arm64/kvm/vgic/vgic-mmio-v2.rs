...
//! Source: arch/arm64/kvm/vgic/vgic-mmio-v2.c
#![no_std]
...
//
    ...
    let mut was_enabled: bool = dist.enabled;
    switch (addr & 0x0c) {
    case GIC_DIST_CTRL:
...
//
    ...
    unsigned long val)
    {
    let mut nr_vcpus: c_int = atomic_read(&source_vcpu.kvm.online_vcpus);
    let mut intid: c_int = val & 0xf;
    let mut targets: c_int = (val >> 16) & 0xff;
    let mut mode: c_int = (val >> 24) & 0x03;
    ...
    gpa_t addr, unsigned int len)
    {
    let mut intid: u32 = VGIC_ADDR_TO_INTID(addr, 8);
    int i;
    let mut val: u64 = 0;
    for (i = 0; i < len; i++) {
    ...
    {
    let mut intid: u32 = VGIC_ADDR_TO_INTID(addr, 8);
    let mut cpu_mask: u8 = GENMASK(atomic_read(&vcpu.kvm.online_vcpus) - 1, 0);
    int i;
    unsigned long flags;
...
pub const GICC_ARCH_VERSION_V2: c_uint = 0x2;
...
// GICv3 only uses ICH_AP1Rn for memory mapped (GICv2) guests
    ...
    core::ptr::null_mut(), vgic_mmio_uaccess_write_v2_misc,
    12, VGIC_ACCESS_32bit),
    REGISTER_DESC_WITH_BITS_PER_IRQ(GIC_DIST_IGROUP,
    vgic_mmio_read_group, vgic_mmio_write_group,
    core::ptr::null_mut(), vgic_mmio_uaccess_write_v2_group, 1,
    VGIC_ACCESS_32bit),
    ...
    vgic_mmio_read_target, vgic_mmio_write_target, core::ptr::null_mut(), core::ptr::null_mut(), 8,
...
#[no_mangle]
pub unsafe extern "C" fn vgic_v2_init_dist_iodev(dev: *mut vgic_io_device) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn vgic_v2_init_cpuif_iodev(dev: *mut vgic_io_device) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn vgic_v2_has_attr_regs(dev: *mut kvm_device, attr: *mut kvm_device_attr) -> c_int {
