...
//! Source: arch/arm64/kvm/vgic/vgic-kvm-device.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn vgic_check_type(kvm: *mut kvm, type_needed: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn kvm_set_legacy_vgic_v2_addr(kvm: *mut kvm, dev_addr: *mut kvm_arm_device_addr) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn kvm_vgic_addr(kvm: *mut kvm, attr: *mut kvm_device_attr, write: bool) -> c_int {
    ...
    let mut undef_value: u64 = VGIC_ADDR_UNDEF;
    u64 addr;
    int r;
...
//
    ...
    if (write) {
    let mut base: gpa_t = addr & KVM_VGIC_V3_RDIST_BASE_MASK;
    let mut count: u32 = FIELD_GET(KVM_VGIC_V3_RDIST_COUNT_MASK, addr);
    let mut flags: u8 = FIELD_GET(KVM_VGIC_V3_RDIST_FLAGS_MASK, addr);
    if (!count || flags)
    r = -EINVAL;
    else
...
#[no_mangle]
unsafe extern "C" fn vgic_create(dev: *mut kvm_device, type: u32) -> c_int {
    static int vgic_create(struct kvm_device *dev, u32 type)
    {
    return kvm_vgic_create(dev.kvm, type);
    }
#[no_mangle]
unsafe extern "C" fn vgic_destroy(dev: *mut kvm_device) {
    static void vgic_destroy(struct kvm_device *dev)
    {
    kfree(dev);
    }
#[no_mangle]
pub unsafe extern "C" fn kvm_register_vgic_device(type: c_ulong) -> c_int {
    ...
    {
    let mut cpuid: c_int = FIELD_GET(KVM_DEV_ARM_VGIC_CPUID_MASK, attr.attr);
...
//
#[no_mangle]
unsafe extern "C" fn reg_allowed_pre_init(attr: *mut kvm_device_attr) -> bool {
