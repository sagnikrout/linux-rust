...
#[no_mangle]
unsafe extern "C" fn get_ia_size(wi: *mut s1_walk_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn check_output_size(ipa: u64, wi: *mut s1_walk_info) -> bool {
...
#[no_mangle]
unsafe extern "C" fn has_52bit_pa(vcpu: *mut kvm_vcpu, wi: *mut s1_walk_info, tcr: u64) -> bool {
...
#[no_mangle]
unsafe extern "C" fn desc_to_oa(wi: *mut s1_walk_info, desc: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn compute_translation_regime(vcpu: *mut kvm_vcpu, op: u32) -> enum trans_regime {
...
#[no_mangle]
unsafe extern "C" fn effective_tcr2(vcpu: *mut kvm_vcpu, regime: enum trans_regime) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn s1pie_enabled(vcpu: *mut kvm_vcpu, regime: enum trans_regime) -> bool {
...
#[no_mangle]
unsafe extern "C" fn compute_s1poe(vcpu: *mut kvm_vcpu, wi: *mut s1_walk_info) {
...
#[no_mangle]
unsafe extern "C" fn has_tgran(mmfr0: u64, shift: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcr_to_tg0_pgshift(tcr: u64) -> c_uint {
    static unsigned int tcr_to_tg0_pgshift(u64 tcr)
    {
    let mut tg0: u64 = tcr & TCR_TG0_MASK;
    switch (tg0) {
    case TCR_TG0_4K:
    return 12;
    case TCR_TG0_16K:
...
#[no_mangle]
unsafe extern "C" fn tcr_to_tg1_pgshift(tcr: u64) -> c_uint {
    static unsigned int tcr_to_tg1_pgshift(u64 tcr)
    {
    let mut tg1: u64 = tcr & TCR_TG1_MASK;
    switch (tg1) {
    case TCR_TG1_4K:
    return 12;
    case TCR_TG1_16K:
...
#[no_mangle]
unsafe extern "C" fn fallback_tgran_shift(mmfr0: u64) -> c_uint {
    static unsigned int fallback_tgran_shift(u64 mmfr0)
    {
    if (has_tgran(mmfr0, PAGE_SHIFT))
    return PAGE_SHIFT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_tgran(mmfr0, _arg: 12)) -> else {
    else if (has_tgran(mmfr0, 12))
    return 12;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_tgran(mmfr0, _arg: 14)) -> else {
    else if (has_tgran(mmfr0, 14))
    return 14;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_tgran(mmfr0, _arg: 16)) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcr_tg_pgshift(kvm: *mut kvm, tcr: u64, upper_range: bool) -> c_uint {
...
// Block mapping, check the validity of the level
    if (!(desc & BIT(1))) {
    let mut valid_block: bool = false;
    let mut lpa: bool = kvm_has_feat_enum(vcpu.kvm, ID_AA64MMFR0_EL1, PARANGE, 52);
    switch (BIT(wi.pgshift)) {
    case SZ_4K:
...
#[no_mangle]
unsafe extern "C" fn __mmu_config_save(config: *mut mmu_config) {
...
#[no_mangle]
unsafe extern "C" fn __mmu_config_restore(config: *mut mmu_config) {
...
#[no_mangle]
unsafe extern "C" fn at_s1e1p_fast(vcpu: *mut kvm_vcpu, op: u32, vaddr: u64) -> bool {
...
#[no_mangle]
unsafe extern "C" fn s2_memattr_to_attr(memattr: u8) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn combine_s1_s2_attr(s1: u8, s2: u8) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn compute_final_sh(attr: u8, sh: u8) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn combine_sh(s1_sh: u8, s2_sh: u8) -> u8 {
...
// Combination of R_HMNDG, R_TNHFM and R_GQFSF
    let mut s2_parattr: u8 = s2_memattr_to_attr(s2_memattr);
    if (MEMATTR_IS_DEVICE(s1_parattr) ||
...
#[no_mangle]
unsafe extern "C" fn pan3_enabled(vcpu: *mut kvm_vcpu, regime: enum trans_regime) -> bool {
...
#[no_mangle]
unsafe extern "C" fn handle_at_slow(vcpu: *mut kvm_vcpu, op: u32, vaddr: u64, par: *mut u64) -> c_int {
    static int handle_at_slow(struct kvm_vcpu *vcpu, u32 op, u64 vaddr, u64 *par)
    {
    let mut wr: s1_walk_result = {};
    let mut wi: s1_walk_info = {};
    let mut perm_fail: bool = false;
    int ret, idx;
...
//
#[no_mangle]
unsafe extern "C" fn __kvm_at_s1e01_fast(vcpu: *mut kvm_vcpu, op: u32, vaddr: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn par_check_s1_perm_fault(par: u64) -> bool {
    static bool par_check_s1_perm_fault(u64 par)
    {
    let mut fst: u8 = FIELD_GET(SYS_PAR_EL1_FST, par);
    return  ((fst & ESR_ELx_FSC_TYPE) == ESR_ELx_FSC_PERM &&
    !(par & SYS_PAR_EL1_S));
    }
#[no_mangle]
unsafe extern "C" fn par_check_s1_access_fault(par: u64) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn __kvm_at_s1e01(vcpu: *mut kvm_vcpu, op: u32, vaddr: u64) -> c_int {
    int __kvm_at_s1e01(struct kvm_vcpu *vcpu, u32 op, u64 vaddr)
    {
    let mut par: u64 = __kvm_at_s1e01_fast(vcpu, op, vaddr);
    int ret;
//
...
#[no_mangle]
pub unsafe extern "C" fn __kvm_at_s1e2(vcpu: *mut kvm_vcpu, op: u32, vaddr: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __kvm_at_s12(vcpu: *mut kvm_vcpu, op: u32, vaddr: u64) -> c_int {
    int __kvm_at_s12(struct kvm_vcpu *vcpu, u32 op, u64 vaddr)
    {
    let mut out: kvm_s2_trans = {};
    u64 ipa, par;
    bool write;
    int ret;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct desc_match {
    pub ipa: u64,
    pub level: c_int,
}

#[no_mangle]
unsafe extern "C" fn match_s1_desc(ctxt: *mut s1_walk_context, priv: *mut c_void) -> c_int {
    static int match_s1_desc(struct s1_walk_context *ctxt, void *priv)
    {
    struct desc_match *dm = priv;
    let mut ipa: u64 = dm.ipa;
...
#[no_mangle]
pub unsafe extern "C" fn __kvm_find_s1_desc_level(vcpu: *mut kvm_vcpu, va: u64, ipa: u64, level: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __lsui_swap_desc(ptep: *mut u64 __user, old: u64, new: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __lse_swap_desc(ptep: *mut u64 __user, old: u64, new: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __llsc_swap_desc(ptep: *mut u64 __user, old: u64, new: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __kvm_at_swap_desc(kvm: *mut kvm, ipa: gpa_t, old: u64, new: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cpus_have_final_cap(ARM64_HAS_LSE_ATOMICS)) -> else {
