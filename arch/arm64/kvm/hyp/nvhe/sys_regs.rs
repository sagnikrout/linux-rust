...
//! Source: arch/arm64/kvm/hyp/nvhe/sys_regs.c
#![no_std]
...
pub struct pvm_ftr_bits {
    ...
    pub width: u8,
    pub max_val: u8,
    pub kvm): *const *const bool (vm_supported)(struct kvm,
}
    ...
    MAX_FEAT_FUNC(id, fld, max, core::ptr::null_mut())

    __MAX_FEAT_FUNC(id, fld, max, core::ptr::null_mut(), false)

#[no_mangle]
unsafe extern "C" fn vm_has_ptrauth(kvm: *const kvm) -> bool {
...
#[no_mangle]
unsafe extern "C" fn vm_has_sve(kvm: *const kvm) -> bool {
...
//
    ...
    let mut sign: bool = restrictions[i].sign;
    let mut shift: c_int = restrictions[i].shift;
    let mut width: c_int = restrictions[i].width;
    let mut min_signed: u64 = (1UL << width) - 1UL;
    let mut sign_bit: u64 = 1UL << (width - 1);
    let mut mask: u64 = GENMASK_ULL(width + shift - 1, shift);
    let mut sys_val: u64 = (sys_reg_val & mask) >> shift;
    let mut pvm_max: u64 = restrictions[i].max_val;
    if (vm_supported && !vm_supported(vcpu.kvm))
    val |= (sign ? min_signed : 0) << shift;
#[no_mangle]
pub unsafe extern "C" fn if(sign_bit): sign && (sys_val >= sign_bit || pvm_max >=) -> else {
...
#[no_mangle]
unsafe extern "C" fn pvm_calc_id_reg(vcpu: *const kvm_vcpu, id: u32) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn inject_sync64(vcpu: *mut kvm_vcpu, esr: u64) {
...
unsafe extern "C" fn inject_undef64(vcpu: *mut kvm_vcpu) {
    ...
    let mut reg: u32 = reg_to_encoding(r);
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_init_pvm_id_regs(vcpu: *mut kvm_vcpu) {
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_check_pvm_sysreg_table() -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_handle_pvm_sysreg(vcpu: *mut kvm_vcpu, exit_code: *mut u64) -> bool {
    bool kvm_handle_pvm_sysreg(struct kvm_vcpu *vcpu, u64 *exit_code)
    {
    const struct sys_reg_desc *r;
    struct sys_reg_params params;
    let mut esr: c_ulong = kvm_vcpu_get_esr(vcpu);
    let mut Rt: c_int = kvm_vcpu_sys_get_rt(vcpu);
...
//
#[no_mangle]
pub unsafe extern "C" fn kvm_handle_pvm_restricted(vcpu: *mut kvm_vcpu, exit_code: *mut u64) -> bool {
