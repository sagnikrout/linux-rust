...
//! Source: arch/arm64/net/bpf_jit_comp.c
#![no_std]
...
pub struct jit_ctx {
    ...
    pub exentry_idx: c_int,
    pub nr_used_callee_reg: c_int,
    pub /: *mut *mut u8 used_callee_reg[8]; / r6~r9, fp, arena_vm_start,
    pub image: *mut __le32,
    pub ro_image: *mut __le32,
    pub stack_size: u32,
...
#[derive(Copy, Clone)]
pub struct bpf_plt {
    pub /: *mut *mut u32 insn_ldr; / load target,
    pub /: *mut *mut u32 insn_br; / branch to target,
    pub /: *mut *mut u64 target; / target value,
}
...
#[no_mangle]
pub unsafe extern "C" fn emit(insn: u32, ctx: *mut jit_ctx) {
    static inline void emit(const u32 insn, struct jit_ctx *ctx)
    {
    if (ctx.image != core::ptr::null_mut() && ctx.write)
    ctx.image[ctx.idx] = cpu_to_le32(insn);
    ctx.idx++;
    }
#[no_mangle]
pub unsafe extern "C" fn emit_u32_data(data: u32, ctx: *mut jit_ctx) {
    static inline void emit_u32_data(const u32 data, struct jit_ctx *ctx)
    {
    if (ctx.image != core::ptr::null_mut() && ctx.write)
    ctx.image[ctx.idx] = ( __le32)data;
    ctx.idx++;
    }
    static inline void emit_a64_mov_i(const int is64, const int reg,
    const s32 val, struct jit_ctx *ctx)
    {
    let mut hi: u16 = val >> 16;
    let mut lo: u16 = val & 0xffff;
    if (hi & 0x8000) {
    if (hi == 0xffff) {
...
#[no_mangle]
unsafe extern "C" fn i64_i16_blocks(val: u64, inverse: bool) -> c_int {
    ...
    struct jit_ctx *ctx)
    {
    let mut nrm_tmp: u64 = val, rev_tmp = ~val;
    bool inverse;
    int shift;
    if (!(nrm_tmp >> 32))
...
#[no_mangle]
pub unsafe extern "C" fn emit_bti(insn: u32, ctx: *mut jit_ctx) {
...
#[no_mangle]
pub unsafe extern "C" fn emit_kcfi(hash: u32, ctx: *mut jit_ctx) {
...
#[no_mangle]
unsafe extern "C" fn should_emit_indirect_call(target: c_long, ctx: *const jit_ctx) -> bool {
...
#[no_mangle]
unsafe extern "C" fn emit_direct_call(target: u64, ctx: *mut jit_ctx) {
...
#[no_mangle]
unsafe extern "C" fn emit_indirect_call(target: u64, ctx: *mut jit_ctx) {
...
#[no_mangle]
unsafe extern "C" fn emit_call(target: u64, ctx: *mut jit_ctx) {
...
#[no_mangle]
pub unsafe extern "C" fn epilogue_offset(ctx: *const jit_ctx) -> c_int {
    static inline int epilogue_offset(const struct jit_ctx *ctx)
    {
    let mut to: c_int = ctx.epilogue_offset;
    let mut from: c_int = ctx.idx;
    return to - from;
    }
#[no_mangle]
unsafe extern "C" fn is_addsub_imm(imm: u32) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_lsi_offset(offset: c_int, scale: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn prepare_bpf_tail_call_cnt(ctx: *mut jit_ctx) {
    static void prepare_bpf_tail_call_cnt(struct jit_ctx *ctx)
    {
    let mut is_main_prog: bool = !bpf_is_subprog(ctx.prog);
    let mut ptr: u8 = bpf2a64[TCCNT_PTR];
    if (is_main_prog) {
...
// Save callee-saved registers
#[no_mangle]
unsafe extern "C" fn push_callee_regs(ctx: *mut jit_ctx) {
...
#[no_mangle]
unsafe extern "C" fn pop_callee_regs(ctx: *mut jit_ctx) {
...
//
    ...
    struct jit_ctx *ctx)
    {
    let mut tmp: u8 = bpf2a64[TMP_REG_1];
    emit_a64_mov_i64(dst_reg, ( const u64)ptr, ctx);
...
#[no_mangle]
unsafe extern "C" fn build_prologue(ctx: *mut jit_ctx, ebpf_from_cbpf: bool) -> c_int {
    static int build_prologue(struct jit_ctx *ctx, bool ebpf_from_cbpf)
    {
    const struct bpf_prog *prog = ctx.prog;
    let mut is_main_prog: bool = !bpf_is_subprog(prog);
    let mut fp: u8 = bpf2a64[BPF_REG_FP];
    let mut arena_vm_base: u8 = bpf2a64[ARENA_VM_START];
    let mut priv_sp: u8 = bpf2a64[PRIVATE_SP];
...
#[no_mangle]
unsafe extern "C" fn emit_bpf_tail_call(ctx: *mut jit_ctx) -> c_int {
    static int emit_bpf_tail_call(struct jit_ctx *ctx)
    {
// bpf_tail_call(void *prog_ctx, struct bpf_array *array, u64 index)
    let mut r2: u8 = bpf2a64[BPF_REG_2];
    let mut r3: u8 = bpf2a64[BPF_REG_3];
    let mut tmp: u8 = bpf2a64[TMP_REG_1];
    let mut prg: u8 = bpf2a64[TMP_REG_2];
    let mut tcc: u8 = bpf2a64[TMP_REG_3];
    let mut ptr: u8 = bpf2a64[TCCNT_PTR];
    size_t off;
    __le32 *branch1 = core::ptr::null_mut();
    __le32 *branch2 = core::ptr::null_mut();
    __le32 *branch3 = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn emit_atomic_ld_st(insn: *const bpf_insn, ctx: *mut jit_ctx) -> c_int {
    static int emit_atomic_ld_st(const struct bpf_insn *insn, struct jit_ctx *ctx)
    {
    let mut imm: i32 = insn.imm;
    let mut off: i16 = insn.off;
    let mut code: u8 = insn.code;
    let mut arena: bool = BPF_MODE(code) == BPF_PROBE_ATOMIC;
    let mut arena_vm_base: u8 = bpf2a64[ARENA_VM_START];
    let mut dst: u8 = bpf2a64[insn.dst_reg];
    let mut src: u8 = bpf2a64[insn.src_reg];
    let mut tmp: u8 = bpf2a64[TMP_REG_1];
    u8 reg;
    switch (imm) {
    case BPF_LOAD_ACQ:
...
#[no_mangle]
unsafe extern "C" fn emit_lse_atomic(insn: *const bpf_insn, ctx: *mut jit_ctx) -> c_int {
    ...
    let mut src: u8 = bpf2a64[insn.src_reg];
    let mut tmp: u8 = bpf2a64[TMP_REG_1];
    let mut tmp2: u8 = bpf2a64[TMP_REG_2];
    let mut isdw: bool = BPF_SIZE(code) == BPF_DW;
    let mut arena: bool = BPF_MODE(code) == BPF_PROBE_ATOMIC;
    let mut off: i16 = insn.off;
    let mut reg: u8 = dst;
    if (off) {
...
#[no_mangle]
unsafe extern "C" fn emit_ll_sc_atomic(insn: *const bpf_insn, ctx: *mut jit_ctx) -> c_int {
    ...
    let mut tmp: u8 = bpf2a64[TMP_REG_1];
    let mut tmp2: u8 = bpf2a64[TMP_REG_2];
    let mut tmp3: u8 = bpf2a64[TMP_REG_3];
    let mut i: c_int = insn - ctx.prog.insnsi;
    let mut imm: i32 = insn.imm;
    let mut off: i16 = insn.off;
    let mut isdw: bool = BPF_SIZE(code) == BPF_DW;
    let mut reg: u8 = dst;
...
#[no_mangle]
pub unsafe extern "C" fn if(BPF_AND: imm ==) -> else {
    else if (imm == BPF_AND)
    emit(A64_AND(isdw, tmp2, tmp2, src), ctx);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_OR: imm ==) -> else {
    else if (imm == BPF_OR)
...
// r0 = atomic_cmpxchg(dst_reg + off, r0, src_reg);
    let mut r0: u8 = bpf2a64[BPF_REG_0];
...
//
#[no_mangle]
unsafe extern "C" fn build_plt(ctx: *mut jit_ctx) {
    static void build_plt(struct jit_ctx *ctx)
    {
    let mut tmp: u8 = bpf2a64[TMP_REG_1];
    struct bpf_plt *plt = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn build_bhb_mitigation(ctx: *mut jit_ctx) -> void __maybe_unused {
    static void __maybe_unused build_bhb_mitigation(struct jit_ctx *ctx)
    {
    const u8 r1 = bpf2a64[BPF_REG_1]; /* aka x0 */
    let mut k: u8 = get_spectre_bhb_loop_value();
...
#[no_mangle]
unsafe extern "C" fn build_epilogue(ctx: *mut jit_ctx, was_classic: bool) {
...
//
    ...
    struct pt_regs *regs)
    {
    let mut dst_reg: c_int = FIELD_GET(BPF_FIXUP_REG_MASK, ex.fixup);
    let mut off: i16 = FIELD_GET(BPF_FIXUP_OFFSET_MASK, ex.fixup);
    let mut arena_reg: c_int = FIELD_GET(BPF_FIXUP_ARENA_REG_MASK, ex.fixup);
    let mut is_arena: bool = !!(ex.fixup & BPF_ARENA_ACCESS);
    let mut is_write: bool = !!(ex.fixup & BPF_ARENA_WRITE);
    unsigned long addr;
    if (is_arena) {
...
#[no_mangle]
unsafe extern "C" fn emit_stack_arg_load(dst: u8, bpf_off: i16, ctx: *mut jit_ctx) {
    static void emit_stack_arg_load(u8 dst, s16 bpf_off, struct jit_ctx *ctx)
    {
    let mut idx: c_int = bpf_off / sizeof(u64) - 1;
...
#[no_mangle]
unsafe extern "C" fn emit_stack_arg_store(src_a64: u8, bpf_off: i16, ctx: *mut jit_ctx) {
...
#[no_mangle]
unsafe extern "C" fn emit_stack_arg_store_imm(imm: i32, bpf_off: i16, tmp: u8, ctx: *mut jit_ctx) {
...
//
#[no_mangle]
unsafe extern "C" fn emit_kfunc_arena_args(ctx: *mut jit_ctx, insn: *const bpf_insn) -> c_int {
    ...
    let mut reg: u8 = bpf2a64[BPF_REG_1 + i];
...
// dst = src
    ...
    let mut upper: u32 = ctx.user_vm_start >> 32;
    let mut upper_low: u16 = upper & 0xffff;
    let mut upper_high: u16 = upper >> 16;
    let mut nr_movk: c_int = !!upper_low + !!upper_high;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cpus_have_cap(ARM64_HAS_LSE_ATOMICS)) -> else {
...
#[no_mangle]
unsafe extern "C" fn build_body(env: *mut bpf_verifier_env, ctx: *mut jit_ctx, extra_pass: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn validate_code(ctx: *mut jit_ctx) -> c_int {
    static int validate_code(struct jit_ctx *ctx)
    {
    int i;
    for (i = 0; i < ctx.idx; i++) {
    let mut a64_insn: u32 = le32_to_cpu(ctx.image[i]);
    if (a64_insn == AARCH64_BREAK_FAULT)
    return -1;
    }
    return 0;
...
#[no_mangle]
unsafe extern "C" fn validate_ctx(ctx: *mut jit_ctx) -> c_int {
...
pub struct arm64_jit_data {
    ...
    struct bpf_binary_header *ro_header = core::ptr::null_mut();
    struct arm64_jit_data *jit_data;
    void __percpu *priv_stack_ptr = core::ptr::null_mut();
    let mut was_classic: bool = bpf_prog_was_classic(prog);
...
//
    ...
    if (ctx.offset == core::ptr::null_mut())
    goto out_off;
    ctx.user_vm_start = bpf_arena_get_user_vm_start(prog.aux.arena);
    ctx.arena_vm_start = bpf_arena_get_kern_vm_start(prog.aux.arena);
    out_cnt = bpf_out_stack_arg_cnt(env, prog);
    if (out_cnt) {
    let mut nr_on_stack: c_int = out_cnt - NR_STACK_ARG_REGS;
    if (nr_on_stack > 0)
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_alloc_exec_limit() -> u64 {
...
pub unsafe extern "C" fn bpf_jit_supports_subprog_tailcalls() -> bool {
    ...
    let mut cookie_off: c_int = offsetof(struct bpf_tramp_run_ctx, bpf_cookie);
...
//
    ...
    if (ctx.image) {
    let mut offset: c_int = &ctx.image[ctx.idx] - branch;
...
#[no_mangle]
unsafe extern "C" fn clear_garbage(ctx: *mut jit_ctx, reg: c_int, effective_bytes: c_int) {
    static void clear_garbage(struct jit_ctx *ctx, int reg, int effective_bytes)
    {
    if (effective_bytes) {
    let mut garbage_bits: c_int = 64 - 8 * effective_bytes;
...
//
#[no_mangle]
unsafe extern "C" fn emit_arena_arg_conv(ctx: *mut jit_ctx, dst: u8, src: u8, nullable: bool, base_lo: u8) {
...
// skip the subtraction so that NULL stays NULL
    ...
    {
    let mut tmp: u8 = bpf2a64[TMP_REG_1];
    let mut base_lo: u8 = bpf2a64[TMP_REG_2];
...
#[no_mangle]
unsafe extern "C" fn restore_args(ctx: *mut jit_ctx, bargs_off: c_int, nregs: c_int) {
...
#[no_mangle]
unsafe extern "C" fn store_func_meta(ctx: *mut jit_ctx, func_meta: u64, func_meta_off: c_int) {
...
//
    ...
    bool save_ret;
    __le32 **branches = core::ptr::null_mut();
    let mut is_struct_ops: bool = is_struct_ops_tramp(fentry);
    int cookie_off, cookie_cnt, cookie_bargs_off;
    let mut fsession_cnt: c_int = bpf_fsession_cnt(tnodes);
    u64 arena_base;
    u64 func_meta;
//
...
// clear the return value to make sure fentry always gets 0
    ...
    let mut meta: u64 = func_meta | (cookie_bargs_off << BPF_TRAMP_COOKIE_INDEX_SHIFT);
...
// update the branches saved in invoke_bpf_mod_ret with cbnz
    for (i = 0; i < fmod_ret.nr_nodes && ctx.image != core::ptr::null_mut(); i++) {
    let mut offset: c_int = &ctx.image[ctx.idx] - branches[i];
...
pub unsafe extern "C" fn arch_protect_bpf_trampoline(image: *mut c_void, size: c_uint) -> c_int {
    ...
    void *func_addr)
    {
    let mut size: u32 = ro_image_end - ro_image;
    struct arg_aux aaux;
    void *image, *tmp;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn is_long_jump(ip: *mut c_void, target: *mut c_void) -> bool {
...
//
    ...
    u32 replaced;
    struct bpf_plt *plt = core::ptr::null_mut();
    let mut size: c_ulong = 0UL;
    let mut offset: c_ulong = ~0UL;
    enum aarch64_insn_branch_type branch_type;
    char namebuf[KSYM_NAME_LEN];
    void *image = core::ptr::null_mut();
    let mut plt_target: u64 = 0ULL;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_long_jump(ip, _arg: old_addr)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_bypass_spec_v4() -> bool {
