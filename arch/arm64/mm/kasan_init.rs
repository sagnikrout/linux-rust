...
//! Source: arch/arm64/mm/kasan_init.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn kasan_alloc_zeroed_page(node: c_int) -> phys_addr_t __init {
...
#[no_mangle]
unsafe extern "C" fn kasan_alloc_raw_page(node: c_int) -> phys_addr_t __init {
...
//
#[no_mangle]
unsafe extern "C" fn root_level_aligned(addr: u64) -> bool __init {
    static bool __init root_level_aligned(u64 addr)
    {
    let mut shift: c_int = (ARM64_HW_PGTABLE_LEVELS(vabits_actual) - 1) * PTDESC_TABLE_SHIFT;
    return (addr % (PAGE_SIZE << shift)) == 0;
    }
// The early shadow maps everything to a single page of zeroes
#[no_mangle]
pub unsafe extern "C" fn kasan_early_init() -> asmlinkage void __init {
...
//
#[no_mangle]
unsafe extern "C" fn root_level_idx(addr: u64) -> int __init {
...
//
    ...
    : vabits_actual;
    let mut shift: c_int = (ARM64_HW_PGTABLE_LEVELS(vabits) - 1) * PTDESC_TABLE_SHIFT;
...
//
#[no_mangle]
unsafe extern "C" fn clone_next_level(addr: u64, tmp_pg_dir: *mut pgd_t, pud: *mut pud_t) -> void __init {
    static void __init clone_next_level(u64 addr, pgd_t *tmp_pg_dir, pud_t *pud)
    {
    let mut idx: c_int = root_level_idx(addr);
    let mut pgd: pgd_t = READ_ONCE(swapper_pg_dir[idx]);
...
//
#[no_mangle]
unsafe extern "C" fn next_level_idx(addr: u64) -> int __init {
    static int __init next_level_idx(u64 addr)
    {
    let mut shift: c_int = (ARM64_HW_PGTABLE_LEVELS(vabits_actual) - 2) * PTDESC_TABLE_SHIFT;
...
//
#[no_mangle]
unsafe extern "C" fn clear_next_level(pgd_idx: c_int, start: c_int, end: c_int) -> void __init {
    static void __init clear_next_level(int pgd_idx, int start, int end)
    {
    let mut pgd: pgd_t = READ_ONCE(swapper_pg_dir[pgd_idx]);
    pud_t *pudp = (pud_t *)__phys_to_kimg(__pgd_to_phys(pgd));
    memset(&pudp[start], 0, (end - start) * sizeof(pud_t));
    }
#[no_mangle]
unsafe extern "C" fn clear_shadow(start: u64, end: u64) -> void __init {
    static void __init clear_shadow(u64 start, u64 end)
    {
    let mut l: c_int = root_level_idx(start), m = root_level_idx(end);
...
#[no_mangle]
unsafe extern "C" fn kasan_init_shadow() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn kasan_init_depth() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn kasan_populate_early_vm_area_shadow(start: *mut c_void, size: c_ulong) -> void __init {
