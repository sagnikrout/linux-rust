...
//! Source: arch/arm64/mm/contpte.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn mm_is_user(mm: *mut mm_struct) -> bool {
...
//
    ...
    let mut last_addr: c_ulong = addr + PAGE_SIZE * (nr - 1);
    ...
    let mut ptent: pte_t = __ptep_get_and_clear(mm, addr, ptep);
    if (pte_dirty(ptent))
    pte = pte_mkdirty(pte);
    if (pte_young(ptent))
    pte = pte_mkyoung(pte);
...
#[no_mangle]
pub unsafe extern "C" fn contpte_ptep_get(ptep: *mut pte_t, orig_pte: pte_t) -> pte_t {
...
//
    ...
    pgprot_t orig_prot)
    {
    let mut prot: pgprot_t = pte_pgprot(pte_mkold(pte_mkclean(pte)));
    return pte_valid_cont(pte) && pte_pfn(pte) == pfn &&
    pgprot_val(prot) == pgprot_val(orig_prot);
    }
#[no_mangle]
pub unsafe extern "C" fn contpte_ptep_get_lockless(orig_ptep: *mut pte_t) -> pte_t {
...
//
    let mut end: c_ulong = addr + nr * PAGE_SIZE;
...
//
    ...
    let mut end: c_ulong = start + nr * PAGE_SIZE;
    ptep = contpte_align_addr_ptep(&start, &end, ptep, nr);
    __clear_young_dirty_ptes(vma, start, ptep, (end - start) / PAGE_SIZE, flags);
    }
    EXPORT_SYMBOL_GPL(contpte_clear_young_dirty_ptes);
#[no_mangle]
unsafe extern "C" fn contpte_all_subptes_match_access_flags(ptep: *mut pte_t, entry: pte_t) -> bool {
...
//
    let mut cmp_mask: pteval_t = PTE_RDONLY | PTE_AF | PTE_WRITE | PTE_DIRTY;
    let mut entry_cmp: pteval_t = pte_val(entry) & cmp_mask;
    int i;
    for (i = 0; i < CONT_PTES; i++) {
    let mut pte_cmp: pteval_t = pte_val(__ptep_get(cont_ptep + i)) & cmp_mask;
