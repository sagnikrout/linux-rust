...
#[no_mangle]
unsafe extern "C" fn note_prot_uxn(st: *mut ptdump_pg_state, addr: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn note_prot_wx(st: *mut ptdump_pg_state, addr: c_ulong) {
...
#[no_mangle]
pub unsafe extern "C" fn note_page_pte(pt_st: *mut ptdump_state, addr: c_ulong, pte: pte_t) {
    void note_page_pte(struct ptdump_state *pt_st, unsigned long addr, pte_t pte)
    {
    note_page(pt_st, addr, 4, pte_val(pte));
    }
#[no_mangle]
pub unsafe extern "C" fn note_page_pmd(pt_st: *mut ptdump_state, addr: c_ulong, pmd: pmd_t) {
    void note_page_pmd(struct ptdump_state *pt_st, unsigned long addr, pmd_t pmd)
    {
    note_page(pt_st, addr, 3, pmd_val(pmd));
    }
#[no_mangle]
pub unsafe extern "C" fn note_page_pud(pt_st: *mut ptdump_state, addr: c_ulong, pud: pud_t) {
    void note_page_pud(struct ptdump_state *pt_st, unsigned long addr, pud_t pud)
    {
    note_page(pt_st, addr, 2, pud_val(pud));
    }
#[no_mangle]
pub unsafe extern "C" fn note_page_p4d(pt_st: *mut ptdump_state, addr: c_ulong, p4d: p4d_t) {
    void note_page_p4d(struct ptdump_state *pt_st, unsigned long addr, p4d_t p4d)
    {
    note_page(pt_st, addr, 1, p4d_val(p4d));
    }
#[no_mangle]
pub unsafe extern "C" fn note_page_pgd(pt_st: *mut ptdump_state, addr: c_ulong, pgd: pgd_t) {
    void note_page_pgd(struct ptdump_state *pt_st, unsigned long addr, pgd_t pgd)
    {
    note_page(pt_st, addr, 0, pgd_val(pgd));
    }
#[no_mangle]
pub unsafe extern "C" fn note_page_flush(pt_st: *mut ptdump_state) {
    void note_page_flush(struct ptdump_state *pt_st)
    {
    struct ptdump_pg_state *st = container_of(pt_st, struct ptdump_pg_state, ptdump);
    let mut end: c_ulong = st.end_address;
...
pub unsafe extern "C" fn ptdump_walk(s: *mut seq_file, info: *mut ptdump_info) {
    ...
    ptdump_walk_pgd(&st.ptdump, info.mm, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn ptdump_initialize() -> void __init {
...
unsafe extern "C" fn ptdump_init() -> int __init {
    ...
    {
    let mut page_offset: u64 = _PAGE_OFFSET(vabits_actual);
    let mut vmemmap_start: u64 = (u64)virt_to_page((void *)page_offset);
