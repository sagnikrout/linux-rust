...
//! Source: arch/arm64/mm/trans_pgd.c
#![no_std]
...
//
    ...
    do {
    let mut pte: pte_t = __ptep_get(src_ptep);
    if (pte_none(pte))
    continue;
    ...
    do {
    let mut pmd: pmd_t = READ_ONCE(*src_pmdp);
    ...
    do {
    let mut pud: pud_t = READ_ONCE(*src_pudp);
...
//
    ...
    {
    let mut dst_addr: phys_addr_t = virt_to_phys(page);
    let mut pfn: c_ulong = __phys_to_pfn(dst_addr);
    let mut max_msb: c_int = (dst_addr & GENMASK(52, 48)) ? 51 : 47;
    let mut bits_mapped: c_int = PAGE_SHIFT - 4;
