...
//! Source: arch/arm64/kernel/probes/kprobes.c
#![no_std]
...
pub unsafe extern "C" fn arch_prepare_kprobe(p: *mut kprobe) -> int __kprobes {
    ...
    {
    let mut probe_addr: c_ulong = (unsigned long)p.addr;
    if (probe_addr & 0x3)
    return -EINVAL;
// copy instruction
    p.opcode = *p.addr;
...
pub unsafe extern "C" fn arch_arm_kprobe(p: *mut kprobe) -> void __kprobes {
    ...
    {
    void *addr = p.addr;
    let mut insn: u32 = BRK64_OPCODE_KPROBES;
...
pub unsafe extern "C" fn arch_disarm_kprobe(p: *mut kprobe) -> void __kprobes {
    ...
    {
    void *addr = p.addr;
    let mut insn: u32 = le32_to_cpu(p.opcode);
    aarch64_insn_patch_text(&addr, &insn, 1);
    }
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn kprobe_fault_handler(regs: *mut pt_regs, fsr: c_uint) -> int __kprobes {
