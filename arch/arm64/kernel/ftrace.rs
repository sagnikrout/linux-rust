...
//! Source: arch/arm64/kernel/ftrace.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn ftrace_regs_query_register_offset(name: *const c_char) -> c_int {
...
// Skip any BTI
    ...
    let mut insn: u32 = le32_to_cpu(*(__le32 *)addr);
...
//
    ...
    struct plt_entry *plt = core::ptr::null_mut();
    if (within_module_mem_type(addr, mod, MOD_INIT_TEXT))
    plt = mod.arch.init_ftrace_trampolines;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: within_module_mem_type(addr, _arg: mod, _arg: MOD_TEXT)) -> else {
...
pub unsafe extern "C" fn ftrace_init_nop(mod: *mut module, rec: *mut dyn_ftrace) -> c_int {
    ...
    {
    let mut pc: c_ulong = rec.ip - AARCH64_INSN_SIZE;
    u32 old, new;
    int ret;
...
// parent = return_hooker;
    ...
    {
    let mut return_hooker: c_ulong = (unsigned long)&return_to_handler;
    let mut frame_pointer: c_ulong = arch_ftrace_regs(fregs).fp;
...
unsafe extern "C" fn ftrace_modify_graph_caller(enable: bool) -> c_int {
    ...
    {
    let mut pc: c_ulong = (unsigned long)&ftrace_graph_call;
    u32 branch, nop;
