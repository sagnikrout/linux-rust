...
//! Source: arch/arm64/kernel/ptrace.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn regs_within_kernel_stack(regs: *mut pt_regs, addr: c_ulong) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn flush_ptrace_hw_breakpoint(tsk: *mut task_struct) {
    ...
    t.debug.hbp_break[i] = core::ptr::null_mut();
    }
    }
    for (i = 0; i < ARM_MAX_WRP; i++) {
    if (t.debug.hbp_watch[i]) {
    unregister_hw_breakpoint(t.debug.hbp_watch[i]);
    t.debug.hbp_watch[i] = core::ptr::null_mut();
    }
    }
    }
#[no_mangle]
pub unsafe extern "C" fn ptrace_hw_copy_thread(tsk: *mut task_struct) {
...
#[no_mangle]
unsafe extern "C" fn ptrace_hbp_get_resource_info(note_type: c_uint, info: *mut u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn update_syscall_orig_x0_after_ptrace(target: *mut task_struct) {
...
//
    ...
    {
    int ret;
    let mut newregs: user_pt_regs = task_pt_regs(target).user_regs;
...
#[no_mangle]
unsafe extern "C" fn fpr_active(target: *mut task_struct, regset: *const user_regset) -> c_int {
...
//
    ...
    const void *kbuf, const void __user *ubuf)
    {
    let mut syscallno: c_int = task_pt_regs(target).syscallno;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn sve_size_from_header(header: *const user_sve_header) -> c_uint {
...
//
    let mut mask: c_ulong = ptrauth_user_pac_mask();
    ...
    struct membuf to)
    {
    let mut enabled_keys: c_long = ptrauth_get_enabled_keys(target);
...
#[no_mangle]
unsafe extern "C" fn pac_key_to_user(key: *const ptrauth_key) -> __uint128_t {
    static __uint128_t pac_key_to_user(const struct ptrauth_key *key)
    {
    return (__uint128_t)key.hi << 64 | key.lo;
    }
#[no_mangle]
unsafe extern "C" fn pac_key_from_user(ukey: __uint128_t) -> ptrauth_key {
    ...
    struct membuf to)
    {
    let mut ctrl: c_long = get_tagged_addr_ctrl(target);
...
#[no_mangle]
pub unsafe extern "C" fn compat_get_user_reg(task: *mut task_struct, idx: c_int) -> compat_ulong_t {
...
//
    ...
    {
    int ret;
    let mut tls: compat_ulong_t = target.thread.uw.tp_value;
...
#[no_mangle]
pub unsafe extern "C" fn if(COMPAT_PT_DATA_ADDR: off ==) -> else {
    else if (off == COMPAT_PT_DATA_ADDR)
    tmp = tsk.mm.start_data;
#[no_mangle]
pub unsafe extern "C" fn if(COMPAT_PT_TEXT_END_ADDR: off ==) -> else {
    else if (off == COMPAT_PT_TEXT_END_ADDR)
    tmp = tsk.mm.end_code;
#[no_mangle]
pub unsafe extern "C" fn if(sizeof(compat_elf_gregset_t): off <) -> else {
    else if (off < sizeof(compat_elf_gregset_t))
    tmp = compat_get_user_reg(tsk, off >> 2);
#[no_mangle]
pub unsafe extern "C" fn if(COMPAT_USER_SZ: off >=) -> else {
    ...
    compat_ulong_t val)
    {
    let mut newregs: pt_regs = *task_pt_regs(tsk);
    let mut idx: c_uint = off / 4;
...
//
#[no_mangle]
unsafe extern "C" fn compat_ptrace_hbp_num_to_idx(num: compat_long_t) -> c_int {
    static int compat_ptrace_hbp_num_to_idx(compat_long_t num)
    {
    return (abs(num) - 1) >> 1;
    }
#[no_mangle]
unsafe extern "C" fn compat_ptrace_hbp_get_resource_info(kdata: *mut u32) -> c_int {
...
// Breakpoint
    ...
    {
    let mut addr: c_ulong = caddr;
    let mut data: c_ulong = cdata;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_compat_thread(task_thread_info(task))) -> else {
...
#[no_mangle]
unsafe extern "C" fn report_syscall_entry(regs: *mut pt_regs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn report_syscall_exit(regs: *mut pt_regs) {
...
#[no_mangle]
pub unsafe extern "C" fn syscall_trace_exit(regs: *mut pt_regs) {
...
#[no_mangle]
unsafe extern "C" fn valid_compat_regs(regs: *mut user_pt_regs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn valid_native_regs(regs: *mut user_pt_regs) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn valid_user_regs(regs: *mut user_pt_regs, task: *mut task_struct) -> c_int {
