...
//! Source: arch/arm64/kernel/module-plts.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn get_plt_entry(dst: u64, pc: *mut c_void) -> plt_entry {
...
//
    ...
    let mut i: c_int = pltsec.plt_num_entries;
    let mut j: c_int = i - 1;
    let mut val: u64 = sym.st_value + rela.r_addend;
...
#[no_mangle]
unsafe extern "C" fn duplicate_rel(rela: *const Elf64_Rela, num: c_int) -> bool {
...
// Group branch PLT relas at the front end of the array.
    ...
    {
    let mut i: c_int = 0, j = numrels - 1;
    while (i < j) {
    if (branch_rela_needs_plt(syms, &rela[i], dstidx))
    i++;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: branch_rela_needs_plt(syms, _arg: &rela[j], _arg: dstidx)) -> else {
    ...
    {
    let mut core_plts: c_ulong = 0;
    let mut init_plts: c_ulong = 0;
    Elf64_Sym *syms = core::ptr::null_mut();
    Elf_Shdr *pltsec, *tramp = core::ptr::null_mut(), *init_tramp = core::ptr::null_mut();
    int i;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(sechdrs[i].sh_name: !strcmp(secstrings +, _arg: ".init.plt")) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(SHT_SYMTAB: sechdrs[i].sh_type ==) -> else {
...
//
    ...
    sechdrs[i].sh_info);
    if (nents)
    sort(rels, nents, sizeof(Elf64_Rela), cmp_rela, core::ptr::null_mut());
