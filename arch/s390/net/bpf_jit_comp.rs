...
//! Source: arch/s390/net/bpf_jit_comp.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct bpf_jit {
    pub /: *mut *mut u32 seen; / Flags to remember seen eBPF instructions,
    pub /: *mut *mut u16 seen_regs; / Mask to remember which registers are used,
    pub /: *mut *mut *mut u32 addrs; / Array with relative instruction addresses,
    pub /: *mut *mut *mut u8 prg_buf; / Start of program,
    pub /: *mut *mut int size; / Size of program and literal pool,
    pub /: *mut *mut int size_prg; / Size of program,
    pub /: *mut *mut int prg; / Current position in program,
    pub /: *mut *mut int lit32_start; / Start of 32-bit literal pool,
    pub /: *mut *mut int lit32; / Current position in 32-bit literal pool,
    pub /: *mut *mut int lit64_start; / Start of 64-bit literal pool,
    pub /: *mut *mut int lit64; / Current position in 64-bit literal pool,
    pub /: *mut *mut int base_ip; / Base address for literal pool,
    pub /: *mut *mut int exit_ip; / Address of exit,
    pub /: *mut *mut int tail_call_start; / Tail call start offset,
    pub /: *mut *mut int excnt; / Number of exception table entries,
    pub /: *mut *mut int prologue_plt_ret; / Return address for prologue hotpatch PLT,
    pub /: *mut *mut int prologue_plt; / Start of prologue hotpatch PLT,
    pub /: *mut *mut int kern_arena; / Pool offset of kernel arena address,
    pub /: *mut *mut u64 user_arena; / User arena address,
    pub /: *mut *mut u32 frame_off; / Offset of struct bpf_prog from %r15,
}

pub const NVREGS: c_uint = 0xffc0		/* %r6-%r15 */;
//
// s390 registers
//
...
#[no_mangle]
pub unsafe extern "C" fn reg(dst_reg: u32, src_reg: u32) -> u32 {
    static inline u32 reg(u32 dst_reg, u32 src_reg)
    {
    return reg2hex[dst_reg] << 4 | reg2hex[src_reg];
    }
#[no_mangle]
pub unsafe extern "C" fn reg_high(reg: u32) -> u32 {
    static inline u32 reg_high(u32 reg)
    {
    return reg2hex[reg] << 4;
    }
#[no_mangle]
pub unsafe extern "C" fn reg_set_seen(jit: *mut bpf_jit, b1: u32) {
    static inline void reg_set_seen(struct bpf_jit *jit, u32 b1)
    {
    let mut r1: u32 = reg2hex[b1];
    if (r1 >= 6 && r1 <= 15)
    jit.seen_regs |= (1 << r1);
    }
#[no_mangle]
unsafe extern "C" fn off_to_pcrel(jit: *mut bpf_jit, off: u32) -> i32 {
    static s32 off_to_pcrel(struct bpf_jit *jit, u32 off)
    {
    return off - jit.prg;
    }
#[no_mangle]
unsafe extern "C" fn ptr_to_pcrel(jit: *mut bpf_jit, ptr: *const c_void) -> i64 {
...
#[no_mangle]
unsafe extern "C" fn emit6_pcrel_ril(jit: *mut bpf_jit, op: u32, pcrel: i64) {
    static void emit6_pcrel_ril(struct bpf_jit *jit, u32 op, s64 pcrel)
    {
    let mut pc32dbl: u32 = (s32)(pcrel / 2);
    _EMIT6(op | pc32dbl >> 16, pc32dbl & 0xffff);
    }
#[no_mangle]
unsafe extern "C" fn emit6_pcrel_rilb(jit: *mut bpf_jit, op: u32, b: u8, pcrel: i64) {
...
#[no_mangle]
unsafe extern "C" fn emit6_pcrel_rilc(jit: *mut bpf_jit, op: u32, mask: u8, pcrel: i64) {
...
#[no_mangle]
pub unsafe extern "C" fn emit_u32_data(data: u32, jit: *mut bpf_jit) {
...
#[no_mangle]
pub unsafe extern "C" fn emit_kcfi(hash: u32, jit: *mut bpf_jit) {
...
//
#[no_mangle]
unsafe extern "C" fn is_first_pass(jit: *mut bpf_jit) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_codegen_pass(jit: *mut bpf_jit) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_valid_rel(rel: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn can_use_rel(jit: *mut bpf_jit, off: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_valid_ldisp(disp: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn can_use_ldisp_for_lit32(jit: *mut bpf_jit) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn can_use_ldisp_for_lit64(jit: *mut bpf_jit) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn save_regs(jit: *mut bpf_jit, rs: u32, re: u32) {
    static void save_regs(struct bpf_jit *jit, u32 rs, u32 re)
    {
    pub 8: *mut *mut u32 off = offsetof(struct prog_frame, r6) + (rs - 6),
    if (rs == re)
// stg %rs,off(%r15)
    pub 0x0024): _EMIT6(0xe300f000 | rs << 20 | off,,
    else
// stmg %rs,%re,off(%r15)
    pub off): _EMIT6_DISP(0xeb00f000 | rs << 20 | re << 16, 0x0024,,
    }
//
// Restore registers from "rs" (register start) to "re" (register end) on stack
//
#[no_mangle]
unsafe extern "C" fn restore_regs(jit: *mut bpf_jit, rs: u32, re: u32) {
    static void restore_regs(struct bpf_jit *jit, u32 rs, u32 re)
    {
    pub 8: *mut *mut u32 off = jit->frame_off + offsetof(struct prog_frame, r6) + (rs - 6),
    if (rs == re)
// lg %rs,off(%r15)
    pub 0x0004): _EMIT6(0xe300f000 | rs << 20 | off,,
    else
// lmg %rs,%re,off(%r15)
    pub off): _EMIT6_DISP(0xeb00f000 | rs << 20 | re << 16, 0x0004,,
    }
//
// Return first seen register (from start)
//
#[no_mangle]
unsafe extern "C" fn get_start(seen_regs: u16, start: c_int) -> c_int {
    static int get_start(u16 seen_regs, int start)
    {
    pub i: c_int,
    pub {: for (i = start; i <= 15; i++),
    if (seen_regs & (1 << i))
    pub i: return,
    }
    pub 0: return,
...
//
#[no_mangle]
unsafe extern "C" fn get_end(seen_regs: u16, start: c_int) -> c_int {
    ...
    pub 14: return (seen_regs & (1 << 15)) ? 15 :,
    }
pub const REGS_SAVE: c_int = 1;
pub const REGS_RESTORE: c_int = 0;
//
// Save and restore clobbered registers (6-15) on stack.
// We save/restore registers in chunks with gap >= 2 registers.
//
#[no_mangle]
unsafe extern "C" fn save_restore_regs(jit: *mut bpf_jit, op: c_int, extra_regs: u16) {
    static void save_restore_regs(struct bpf_jit *jit, int op, u16 extra_regs)
    {
    pub extra_regs: u16 seen_regs = jit->seen_regs |,
    pub 6: int last = 15, save_restore_size =,
    pub rs: int re = 6,,
...
// conservatively.
//
    pub save_restore_size: *mut *mut jit->prg += (last - re + 1),
    }
    do {
    pub re): rs = get_start(seen_regs,,
    if (!rs)
    pub 1): re = get_end(seen_regs, rs +,
    if (op == REGS_SAVE)
    pub re): save_regs(jit, rs,,
    else
    pub re): restore_regs(jit, rs,,
    pub last): } while (re <=,
    }
#[no_mangle]
unsafe extern "C" fn bpf_skip(jit: *mut bpf_jit, size: c_int) {
    static void bpf_skip(struct bpf_jit *jit, int size)
    {
    if (size >= 6 && !is_valid_rel(size)) {
// brcl 0xf,size
    pub size): EMIT6_PCREL_RILC(0xc0040000, 0xf,,
    pub 6: size -=,
    } else if (size >= 4 && is_valid_rel(size)) {
// brc 0xf,size
    pub size): EMIT4_PCREL(0xa7f40000,,
    pub 4: size -=,
    }
    while (size >= 2) {
// bcr 0,%0
...
pub struct bpf_plt {
    ...
    pub __packed: },
    pub bpf_plt: extern struct bpf_plt,
    asm(
    ".pushsection .rodata\n"
    "	.balign 8\n"
    "bpf_plt:\n"
    ...
    "	.popsection\n"
#[no_mangle]
unsafe extern "C" fn bpf_jit_plt(plt: *mut bpf_plt, ret: *mut c_void, target: *mut c_void) {
    static void bpf_jit_plt(struct bpf_plt *plt, void *ret, void *target)
    {
    pub sizeof(*plt)): *mut memcpy(plt, &bpf_plt,,
    pub ret: plt->ret =,
//
...
//
#[no_mangle]
unsafe extern "C" fn bpf_jit_prologue(jit: *mut bpf_jit, fp: *mut bpf_prog) {
    static void bpf_jit_prologue(struct bpf_jit *jit, struct bpf_prog *fp)
    {
    pub STACK_FRAME_OVERHEAD): BUILD_BUG_ON(sizeof(struct prog_frame) !=,
    pub jit): emit_kcfi(bpf_is_subprog(fp) ? cfi_bpf_subprog_hash : cfi_bpf_hash,,
// No-op for hotpatching
// brcl 0,prologue_plt
    pub jit->prologue_plt): EMIT6_PCREL_RILC(0xc0040000, 0,,
    pub jit->prg: jit->prologue_plt_ret =,
    if (!bpf_is_subprog(fp)) {
// Initialize the tail call counter in the main program.
// xc tail_call_cnt(4,%r15),tail_call_cnt(%r15)
    _EMIT6(0xd703f000 | offsetof(struct prog_frame, tail_call_cnt),
    pub tail_call_cnt)): 0xf000 | offsetof(struct prog_frame,,
    } else {
//
...
// Tail calls have to skip above initialization
    pub cfi_get_offset(): jit->tail_call_start = jit->prg -,
...
//
// lgr %r15,%r3
    pub REG_3): EMIT4(0xb9040000, REG_15,,
    pub NVREGS: jit->seen_regs |=,
    } else {
// Save registers
    save_restore_regs(jit, REGS_SAVE,
    pub 0): fp->aux->exception_boundary ? NVREGS :,
    }
// Setup literal pool
...
// basr %l,0
    pub REG_0): EMIT2(0x0d00, REG_L,,
    pub jit->prg: jit->base_ip =,
    } else {
// larl %l,lit32_start
    pub jit->lit32_start): EMIT6_PCREL_RILB(0xc0000000, REG_L,,
    pub jit->lit32_start: jit->base_ip =,
    }
    }
// Setup stack and backchain
    if (is_first_pass(jit) || (jit.seen & SEEN_STACK)) {
// lgr %w1,%r15 (backchain)
    pub REG_15): EMIT4(0xb9040000, REG_W1,,
// la %bfp,unused_end(%r15) (BPF frame pointer)
    EMIT4_DISP(0x41000000, BPF_REG_FP, REG_15,
    pub unused)): offsetofend(struct prog_frame,,
// aghi %r15,-frame_off
    pub -jit->frame_off): EMIT4_IMM(0xa70b0000, REG_15,,
// stg %w1,backchain(%r15)
    EMIT6_DISP_LH(0xe3000000, 0x0024, REG_W1, REG_0,
    REG_15,
    pub backchain)): offsetof(struct prog_frame,,
    }
    }
//
...
// brcl 0xf,__s390_indirect_jump_rN */			\
    ...
    pub \: __s390_indirect_jump_r ## reg);,
    else								\
// br %rN */						\
    pub \: _EMIT2(0x07f0 | reg);,
    } while (0)
//
// Function epilogue
//
#[no_mangle]
unsafe extern "C" fn bpf_jit_epilogue(jit: *mut bpf_jit) {
    static void bpf_jit_epilogue(struct bpf_jit *jit)
    {
    pub jit->prg: jit->exit_ip =,
// Load exit code: lgr %r2,%b0
    pub BPF_REG_0): EMIT4(0xb9040000, REG_2,,
// Restore registers
    pub 0): save_restore_regs(jit, REGS_RESTORE,,
    pub 8): jit->prg = ALIGN(jit->prg,,
    pub jit->prg: jit->prologue_plt =,
    if (jit.prg_buf)
    bpf_jit_plt((struct bpf_plt *)(jit.prg_buf + jit.prg),
    pub NULL): jit->prg_buf + jit->prologue_plt_ret,,
    pub bpf_plt): jit->prg += sizeof(struct,
    }
#[no_mangle]
pub unsafe extern "C" fn ex_handler_bpf(x: *const exception_table_entry, regs: *mut pt_regs) -> bool {
    bool ex_handler_bpf(const struct exception_table_entry *x, struct pt_regs *regs)
    {
    pub extable_fixup(x): regs->psw.addr =,
    if (x.data != -1)
    pub 0: regs->gprs[x->data] =,
    pub true: return,
    }
//
...
pub struct bpf_jit_probe {
    pub /: *mut *mut int prg; / JITed instruction offset,
    pub /: *mut *mut int nop_prg; / JITed nop offset,
    pub /: *mut *mut int reg; / Register to clear on exception,
    pub /: *mut *mut int arena_reg; / Register to use for arena addressing,
}

#[no_mangle]
unsafe extern "C" fn bpf_jit_probe_init(probe: *mut bpf_jit_probe) {
...
#[no_mangle]
unsafe extern "C" fn emit_ldx(jit: *mut bpf_jit, fp: *mut bpf_prog, insn: *mut bpf_insn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn emit_stx(jit: *mut bpf_jit, fp: *mut bpf_prog, insn: *mut bpf_insn) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sign_zero_extend(jit: *mut bpf_jit, r: c_int, size: u8, flags: u8) -> c_int {
...
//
    ...
    {
    let mut rc_reg: c_int = BPF_OP(insn.code) == BPF_DIV ? REG_W1 : REG_W0;
    switch (off) {
...
//
    ...
    {
    let mut is32: bool = BPF_SIZE(insn.code) == BPF_W;
//
...
// {lax|laxg}
    ...
    case BPF_XCHG: {
    let mut load_probe: bpf_jit_probe = probe;
    int loop_start;
...
// ngr %w1,%dst
    ...
    {
    let mut is_jmp32: bool = BPF_CLASS(insn.code) == BPF_JMP32;
    mask = 0x7000; /* jnz */
...
//
#[no_mangle]
unsafe extern "C" fn bpf_is_new_addr_sane(jit: *mut bpf_jit, i: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn bpf_set_addr(jit: *mut bpf_jit, i: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_needs_zext() -> bool {
...
//
    ...
    if (jit.addrs == core::ptr::null_mut())
    goto out_err;
//
...
//
    ...
    kvfree(jit.addrs);
    kfree(jit_data);
    fp.aux.jit_data = core::ptr::null_mut();
    }
    return fp;
    out_err:
    if (extra_pass) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_supports_far_kfunc_call() -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bpf_tramp_jit {
    pub common: bpf_jit,
    pub the: *mut *mut int orig_stack_args_off;/ Offset of arguments placed on stack by,
// func_addr's original caller
//
    pub /: *mut *mut int stack_size; / Trampoline stack size,
    pub /: *mut *mut int backchain_off; / Offset of backchain,
    pub calling: *mut *mut int stack_args_off; / Offset of stack arguments for,
// func_addr, has to be at the top
//
    pub calling: *mut *mut int reg_args_off; / Offset of register arguments for,
// func_addr
//
    pub at: *mut *mut int ip_off; / For bpf_get_func_ip(), has to be,
// (ctx - 16)
//
    pub has: *mut *mut int func_meta_off; / For bpf_get_func_arg_cnt()/fsession,,
// to be at (ctx - 8)
//
    pub consists: *mut *mut int bpf_args_off; / Offset of BPF_PROG context, which,
// of BPF arguments followed by return value
//
    pub /: *mut *mut int retval_off; / Offset of return value (see above),
    pub used: *mut *mut int r7_r8_off; / Offset of saved %r7 and %r8, which are,
// for __bpf_prog_enter() return value and
// func_addr respectively
//
    pub /: *mut *mut int run_ctx_off; / Offset of struct bpf_tramp_run_ctx,
    pub /: *mut *mut int tccnt_off; / Offset of saved tailcall counter,
    pub the: *mut *mut int r14_off; / Offset of saved %r14, has to be at,
// bottom
    pub /: *mut *mut int do_fexit; / do_fexit: label,
}

#[no_mangle]
unsafe extern "C" fn load_imm64(jit: *mut bpf_jit, dst_reg: c_int, val: u64) {
...
#[no_mangle]
unsafe extern "C" fn emit_store_stack_imm64(jit: *mut bpf_jit, tmp_reg: c_int, stack_off: c_int, imm: u64) {
...
// brasl %r14,__bpf_prog_exit
    ...
    let mut meta: u64 = func_meta | ((u64)cur_cookie << BPF_TRAMP_COOKIE_INDEX_SHIFT);
...
#[no_mangle]
unsafe extern "C" fn alloc_stack(tjit: *mut bpf_tramp_jit, size: usize) -> c_int {
    static int alloc_stack(struct bpf_tramp_jit *tjit, size_t size)
    {
    let mut stack_offset: c_int = tjit.stack_size;
    tjit.stack_size += size;
    return stack_offset;
    }
// ABI uses %r2 - %r6 for parameter passing.
pub const MAX_NR_REG_ARGS: c_int = 5;
// The "L" field of the "mvc" instruction is 8 bits.
pub const MAX_MVC_SIZE: c_int = 256;

// -mfentry generates a 6-byte nop on s390x.
pub const S390X_PATCH_SIZE: c_int = 6;
...
//
    ...
    nr_bpf_args += 1;
#[no_mangle]
pub unsafe extern "C" fn if(16: m->arg_size[i] <=) -> else {
...
// aghi %r15,stack_size
    ...
    EMIT_JUMP_REG(14);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_TRAMP_F_ORIG_STACK: flags &) -> else {
...
//
    ...
    unwind_for_each_frame(&state, core::ptr::null_mut(), core::ptr::null_mut(), 0) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_jit_inlines_helper_call(imm: i32) -> bool {
