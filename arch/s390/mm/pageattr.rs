...
//! Source: arch/s390/mm/pageattr.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn __storage_key_init_range(start: c_ulong, end: c_ulong) {
...
#[no_mangle]
pub unsafe extern "C" fn if(SET_MEMORY_RW: flags &) -> else {
    else if (flags & SET_MEMORY_RW)
    new = pte_mkwrite_novma(pte_mkdirty(new));
    if (flags & SET_MEMORY_NX)
    new = set_pte_bit(new, __pgprot(_PAGE_NOEXEC));
#[no_mangle]
pub unsafe extern "C" fn if(SET_MEMORY_X: flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn split_pmd_page(pmdp: *mut pmd_t, addr: c_ulong) -> c_int {
    ...
    unsigned long flags)
    {
    let mut new: pmd_t = pmdp_get(pmdp);
    if (flags & SET_MEMORY_RO)
...
#[no_mangle]
pub unsafe extern "C" fn split_pud_page(pudp: *mut pud_t, addr: c_ulong) -> c_int {
    ...
    unsigned long flags)
    {
    let mut new: pud_t = pudp_get(pudp);
    if (flags & SET_MEMORY_RO)
...
#[no_mangle]
pub unsafe extern "C" fn __set_memory(addr: c_ulong, numpages: c_ulong, flags: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ipte_range(pte: *mut pte_t, address: c_ulong, nr: c_int) {
