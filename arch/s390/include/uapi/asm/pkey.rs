...
//! Source: arch/s390/include/uapi/asm/pkey.h
#![no_std]
...
pub const PKEY_KEYTYPE_AES_128: c_int = 1;
pub const PKEY_KEYTYPE_AES_192: c_int = 2;
pub const PKEY_KEYTYPE_AES_256: c_int = 3;
pub const PKEY_KEYTYPE_ECC: c_int = 4;
pub const PKEY_KEYTYPE_ECC_P256: c_int = 5;
pub const PKEY_KEYTYPE_ECC_P384: c_int = 6;
pub const PKEY_KEYTYPE_ECC_P521: c_int = 7;
pub const PKEY_KEYTYPE_ECC_ED25519: c_int = 8;
pub const PKEY_KEYTYPE_ECC_ED448: c_int = 9;
pub const PKEY_KEYTYPE_AES_XTS_128: c_int = 10;
pub const PKEY_KEYTYPE_AES_XTS_256: c_int = 11;
pub const PKEY_KEYTYPE_HMAC_512: c_int = 12;
pub const PKEY_KEYTYPE_HMAC_1024: c_int = 13;
...
pub const PKEY_FLAGS_MATCH_CUR_MKVP: c_uint = 0x00000002;
pub const PKEY_FLAGS_MATCH_ALT_MKVP: c_uint = 0x00000004;
// keygenflags defines for CCA AES cipher keys
pub const PKEY_KEYGEN_XPRT_SYM: c_uint = 0x00008000;
pub const PKEY_KEYGEN_XPRT_UASY: c_uint = 0x00004000;
pub const PKEY_KEYGEN_XPRT_AASY: c_uint = 0x00002000;
pub const PKEY_KEYGEN_XPRT_RAW: c_uint = 0x00001000;
pub const PKEY_KEYGEN_XPRT_CPAC: c_uint = 0x00000800;
pub const PKEY_KEYGEN_XPRT_DES: c_uint = 0x00000080;
pub const PKEY_KEYGEN_XPRT_AES: c_uint = 0x00000040;
pub const PKEY_KEYGEN_XPRT_RSA: c_uint = 0x00000008;
// Struct to hold apqn target info (card/domain pair)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_apqn {
    pub card: __u16,
    pub domain: __u16,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_seckey {
    pub /: *mut *mut __u8 seckey[SECKEYBLOBSIZE]; / the secure key blob,
}

// Struct to hold protected key and length info
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_protkey {
    pub /: *mut *mut __u32 type; / key type, one of the PKEY_KEYTYPE_AES values,
    pub /: *mut *mut __u32 len; / bytes actually stored in protkey[],
    pub /: *mut *mut __u8 protkey[MAXPROTKEYSIZE]; / the protected key blob,
}

// Struct to hold an AES clear key value
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_clrkey {
    pub /: *mut *mut __u8 clrkey[MAXCLRKEYSIZE]; / 16, 24, or 32 byte clear key value,
}
...
pub struct ep11kblob_header {
    pub /: *mut *mut __u8 type; / always 0x00,
    pub /: *mut *mut __u8 hver; / header version, currently needs to be 0x00,
    pub /: *mut *mut __u16 len; / total length in bytes (including this header),
    pub /: *mut *mut __u8 version; / PKEY_TYPE_EP11_AES or PKEY_TYPE_EP11_ECC,
    pub /: *mut *mut __u8 res0; / unused,
    pub /: *mut *mut __u16 bitlen; / clear key bit len, 0 for unknown,
    pub /: *mut *mut __u8 res1[8]; / unused,
    pub __packed: },
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_genseck {
    pub /: *mut *mut __u16 cardnr; / in: card to use or FFFF for any,
    pub /: *mut *mut __u16 domain; / in: domain or FFFF for any,
    pub /: *mut *mut __u32 keytype; / in: key type to generate,
    pub /: *mut *mut pkey_seckey seckey; / out: the secure key blob,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_clr2seck {
    pub /: *mut *mut __u16 cardnr; / in: card to use or FFFF for any,
    pub /: *mut *mut __u16 domain; / in: domain or FFFF for any,
    pub /: *mut *mut __u32 keytype; / in: key type to generate,
    pub /: *mut *mut pkey_clrkey clrkey; / in: the clear key value,
    pub /: *mut *mut pkey_seckey seckey; / out: the secure key blob,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_sec2protk {
    pub /: *mut *mut __u16 cardnr; / in: card to use or FFFF for any,
    pub /: *mut *mut __u16 domain; / in: domain or FFFF for any,
    pub /: *mut *mut pkey_seckey seckey; / in: the secure key blob,
    pub /: *mut *mut pkey_protkey protkey; / out: the protected key,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_findcard {
    pub /: *mut *mut pkey_seckey seckey; / in: the secure key blob,
    pub /: *mut *mut __u16 cardnr; / out: card number,
    pub /: *mut *mut __u16 domain; / out: domain number,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_verifykey {
    pub /: *mut *mut pkey_seckey seckey; / in: the secure key blob,
    pub /: *mut *mut __u16 cardnr; / out: card number,
    pub /: *mut *mut __u16 domain; / out: domain number,
    pub /: *mut *mut __u16 keysize; / out: key size in bits,
    pub /: *mut *mut __u32 attributes; / out: attribute bits,
}

pub const PKEY_VERIFY_ATTR_AES: c_uint = 0x00000001  /* key is an AES key */;
pub const PKEY_VERIFY_ATTR_OLD_MKVP: c_uint = 0x00000100  /* key has old MKVP value */;
//
// Generate AES random protected key.
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_genprotk {
    pub /: *mut *mut __u32 keytype; / in: key type to generate,
    pub /: *mut *mut pkey_protkey protkey; / out: the protected key,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_verifyprotk {
    pub /: *mut *mut pkey_protkey protkey; / in: the protected key to verify,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_kblob2pkey {
    pub /: *mut *mut *mut __u8 __user key; / in: the key blob,
    pub /: *mut *mut __u32 keylen; / in: the key blob length,
    pub /: *mut *mut pkey_protkey protkey; / out: the protected key,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_genseck2 {
    pub targets*/: *mut *mut *mut pkey_apqn __user apqns; / in: ptr to list of apqn,
    pub /: *mut *mut __u32 apqn_entries; / in: # of apqn target list entries,
    pub /: *mut *mut pkey_key_type type; / in: key type to generate,
    pub /: *mut *mut pkey_key_size size; / in: key size to generate,
    pub /: *mut *mut __u32 keygenflags; / in: key generation flags,
    pub /: *mut *mut *mut __u8 __user key; / in: pointer to key blob buffer,
    pub /: *mut *mut __u32 keylen; / in: available key blob buffer size,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_clr2seck2 {
    pub /: *mut *mut *mut pkey_apqn __user apqns; / in: ptr to list of apqn targets,
    pub /: *mut *mut __u32 apqn_entries; / in: # of apqn target list entries,
    pub /: *mut *mut pkey_key_type type; / in: key type to generate,
    pub /: *mut *mut pkey_key_size size; / in: key size to generate,
    pub /: *mut *mut __u32 keygenflags; / in: key generation flags,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_verifykey2 {
    pub /: *mut *mut *mut __u8 __user key; / in: pointer to key blob,
    pub /: *mut *mut __u32 keylen; / in: key blob size,
    pub /: *mut *mut __u16 cardnr; / in/out: card number,
    pub /: *mut *mut __u16 domain; / in/out: domain number,
    pub /: *mut *mut pkey_key_type type; / out: the key type,
    pub /: *mut *mut pkey_key_size size; / out: the key size,
    pub /: *mut *mut __u32 flags; / out: additional key info flags,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_apqns4key {
    pub /: *mut *mut *mut __u8 __user key; / in: pointer to key blob,
    pub /: *mut *mut __u32 keylen; / in: key blob size,
    pub /: *mut *mut __u32 flags; / in: match controlling flags,
    pub targets*/: *mut *mut *mut pkey_apqn __user apqns; / in/out: ptr to list of apqn,
    pub /: *mut *mut __u32 apqn_entries; / in: max # of apqn entries in the list,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_apqns4keytype {
    pub /: *mut *mut pkey_key_type type; / in: key type,
    pub /: *mut *mut __u8 cur_mkvp[32]; / in: current mkvp,
    pub /: *mut *mut __u8 alt_mkvp[32]; / in: alternate mkvp,
    pub /: *mut *mut __u32 flags; / in: match controlling flags,
    pub targets*/: *mut *mut *mut pkey_apqn __user apqns; / in/out: ptr to list of apqn,
    pub /: *mut *mut __u32 apqn_entries; / in: max # of apqn entries in the list,
// out: # apqns stored into the list
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pkey_kblob2pkey3 {
    pub /: *mut *mut *mut __u8 __user key; / in: pointer to key blob,
    pub /: *mut *mut __u32 keylen; / in: key blob size,
    pub /: *mut *mut *mut pkey_apqn __user apqns; / in: ptr to list of apqn targets,
    pub /: *mut *mut __u32 apqn_entries; / in: # of apqn target list entries,
    pub /: *mut *mut __u32 pkeytype; / out: prot key type (enum pkey_key_type),
    pub /: *mut *mut __u32 pkeylen; / in/out: size of pkey buffer/actual len of pkey,
    pub /: *mut *mut *mut __u8 __user pkey; / in: pkey blob buffer space ptr,
}
