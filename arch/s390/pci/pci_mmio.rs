...
//! Source: arch/s390/pci/pci_mmio.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn zpci_err_mmio(cc: u8, status: u8, offset: u64) {
...
// status = len >> 24 & 0xff;
    ...
    u64 ulen, u8 *status)
    {
    let mut ioaddr_len: union register_pair = {.even = (u64 )ioaddr, .odd = ulen};
    int cc, exception;
    bool sacf_flag;
    let mut val: u64 = 0;
    let mut cnt: u64 = ulen;
    u8 tmp;
//
...
// did we read everything from user memory?
    ...
    return -EINVAL;
    while (n > 0) {
    size = zpci_get_max_io_size((u64 ) dst,
    (u64 ) src, n,
    ZPCI_MAX_WRITE_SIZE);
    if (size > 8) /* main path */
    ...
    if (rc)
    zpci_err_mmio(rc, status, ( u64) dst);
    return rc;
    }
    SYSCALL_DEFINE3(s390_pci_mmio_write, unsigned long, mmio_addr,
    const void __user *, user_buffer, size_t, length)
    {
    let mut args: follow_pfnmap_args = { };
    u8 local_buf[64];
...
//
    ...
    if (ret) {
    fixup_user_fault(current.mm, mmio_addr, FAULT_FLAG_WRITE, core::ptr::null_mut());
    ret = follow_pfnmap_start(&args);
    if (ret)
    goto out_unlock_mmap;
    }
...
// status = ioaddr_len.odd >> 24 & 0xff;
    ...
    u8 status;
    while (n > 0) {
    size = zpci_get_max_io_size((u64 ) src,
    (u64 ) dst, n,
    ZPCI_MAX_READ_SIZE);
...
//
    ...
    ret = follow_pfnmap_start(&args);
    if (ret) {
    fixup_user_fault(current.mm, mmio_addr, 0, core::ptr::null_mut());
    ret = follow_pfnmap_start(&args);
    if (ret)
    goto out_unlock_mmap;
    }
