...
//! Source: arch/s390/kernel/ptrace.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn update_cr_regs(task: *mut task_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn user_enable_block_step(task: *mut task_struct) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ptrace_disable(task: *mut task_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: cr10)) -> else {
    else if (addr == offsetof(struct per_struct_kernel, cr10))
// Start address of the active per set.
    return test_thread_flag(TIF_SINGLE_STEP) ?
    0 : child.thread.per_user.start;
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: cr11)) -> else {
    else if (addr == offsetof(struct per_struct_kernel, cr11))
// End address of the active per set.
    return test_thread_flag(TIF_SINGLE_STEP) ?
    -1UL : child.thread.per_user.end;
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: bits)) -> else {
    else if (addr == offsetof(struct per_struct_kernel, bits))
// Single-step bit.
    return test_thread_flag(TIF_SINGLE_STEP) ?
    (1UL << (BITS_PER_LONG - 1)) : 0;
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: starting_addr)) -> else {
    else if (addr == offsetof(struct per_struct_kernel, starting_addr))
// Start address of the user specified per set.
    return child.thread.per_user.start;
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: ending_addr)) -> else {
    else if (addr == offsetof(struct per_struct_kernel, ending_addr))
// End address of the user specified per set.
    return child.thread.per_user.end;
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: perc_atmid)) -> else {
    else if (addr == offsetof(struct per_struct_kernel, perc_atmid))
// PER code, ATMID and AI of the last PER trap
    return (unsigned long)
    child.thread.per_event.cause << (BITS_PER_LONG - 16);
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: address)) -> else {
    else if (addr == offsetof(struct per_struct_kernel, address))
// Address of the last PER trap
    return child.thread.per_event.address;
#[no_mangle]
pub unsafe extern "C" fn if(per_struct_kernel: addr == offsetof(struct, _arg: access_id)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn __peek_user(child: *mut task_struct, addr: addr_t) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn __poke_user(child: *mut task_struct, addr: addr_t, data: addr_t) -> c_int {
...
//
    if (addr == offsetof(struct user, regs.psw.mask)) {
    let mut mask: c_ulong = PSW_MASK_USER;
...
#[no_mangle]
unsafe extern "C" fn poke_user(child: *mut task_struct, addr: addr_t, data: addr_t) -> c_int {
...
//
    ...
    const void *kbuf, const void __user *ubuf)
    {
    let mut gs_cb: gs_cb = { }, *data = core::ptr::null_mut();
    int rc;
    if (!cpu_has_gs())
    return -ENODEV;
    ...
    gs_cb.gsd = 25;
#[no_mangle]
pub unsafe extern "C" fn if(current: target ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn is_ri_cb_valid(cb: *mut runtime_instr_cb) -> bool {
    ...
    const void *kbuf, const void __user *ubuf)
    {
    let mut ri_cb: runtime_instr_cb = { }, *data = core::ptr::null_mut();
    int rc;
    if (!test_facility(64))
    return -ENODEV;
