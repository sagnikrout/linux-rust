...
//! Source: arch/s390/kernel/dumpstack.c
#![no_std]
...
//
    ...
    {
    let mut stack: c_ulong = (unsigned long)task_stack_page(task);
    return in_stack(sp, info, STACK_TYPE_TASK, stack);
    }
#[no_mangle]
unsafe extern "C" fn in_irq_stack(sp: c_ulong, info: *mut stack_info) -> bool {
    static bool in_irq_stack(unsigned long sp, struct stack_info *info)
    {
    let mut stack: c_ulong = get_lowcore().async_stack - STACK_INIT_OFFSET;
    return in_stack(sp, info, STACK_TYPE_IRQ, stack);
    }
#[no_mangle]
unsafe extern "C" fn in_nodat_stack(sp: c_ulong, info: *mut stack_info) -> bool {
    static bool in_nodat_stack(unsigned long sp, struct stack_info *info)
    {
    let mut stack: c_ulong = get_lowcore().nodat_stack - STACK_INIT_OFFSET;
    return in_stack(sp, info, STACK_TYPE_NODAT, stack);
    }
#[no_mangle]
unsafe extern "C" fn in_mcck_stack(sp: c_ulong, info: *mut stack_info) -> bool {
    static bool in_mcck_stack(unsigned long sp, struct stack_info *info)
    {
    let mut stack: c_ulong = get_lowcore().mcck_stack - STACK_INIT_OFFSET;
    return in_stack(sp, info, STACK_TYPE_MCCK, stack);
    }
#[no_mangle]
unsafe extern "C" fn in_restart_stack(sp: c_ulong, info: *mut stack_info) -> bool {
    static bool in_restart_stack(unsigned long sp, struct stack_info *info)
    {
    let mut stack: c_ulong = get_lowcore().restart_stack - STACK_INIT_OFFSET;
...
// visit_mask |= 1UL << info->type;
    ...
    printk("%sCall Trace:\n", loglvl);
    unwind_for_each_frame(&state, task, core::ptr::null_mut(), (unsigned long) stack)
...
#[no_mangle]
unsafe extern "C" fn show_last_breaking_event(regs: *mut pt_regs) {
...
// Show stack backtrace if pt_regs is from kernel mode
    if (!user_mode(regs))
    show_stack(core::ptr::null_mut(), (unsigned long *) regs.gprs[15], KERN_DEFAULT);
    show_last_breaking_event(regs);
    }
    static DEFINE_SPINLOCK(die_lock);
#[no_mangle]
pub unsafe extern "C" fn die(regs: *mut pt_regs, str: *const c_char) -> void __noreturn {
