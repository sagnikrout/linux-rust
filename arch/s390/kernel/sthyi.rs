...
//! Source: arch/s390/kernel/sthyi.c
#![no_std]
...
pub const DED_WEIGHT: c_uint = 0xffff;
//
// CP and IFL as EBCDIC strings, SP/0x40 determines the end of string
// as they are justified with spaces.
//
pub const CP: c_uint = 0xc3d7404040404040UL;
pub const IFL: c_uint = 0xc9c6d34040404040UL;
    enum hdr_flags {
    HDR_NOT_LPAR   = 0x10,
    HDR_STACK_INCM = 0x20,
    HDR_STSI_UNAV  = 0x40,
...
pub struct hdr_sctn {
    pub infhflg1: u8,
    pub /: *mut *mut u8 infhflg2; / reserved,
    pub /: *mut *mut u8 infhval1; / reserved,
    pub /: *mut *mut u8 infhval2; / reserved,
    pub reserved: [u8; 3],
    pub infhygct: u8,
    pub infhtotl: u16,
    pub infhdln: u16,
...
#[derive(Copy, Clone)]
pub struct mac_sctn {
    pub /: *mut *mut u8 infmflg1; / reserved,
    pub /: *mut *mut u8 infmflg2; / reserved,
    pub infmval1: u8,
    pub /: *mut *mut u8 infmval2; / reserved,
    pub infmscps: u16,
    pub infmdcps: u16,
    pub infmsifl: u16,
    pub infmdifl: u16,
    pub infmname: [c_char; 8],
    pub infmtype: [c_char; 4],
    pub infmmanu: [c_char; 16],
    pub infmseq: [c_char; 16],
    pub infmpman: [c_char; 4],
    pub reserved: [u8; 4],
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct par_sctn {
    pub infpflg1: u8,
    pub /: *mut *mut u8 infpflg2; / reserved,
    pub infpval1: u8,
    pub /: *mut *mut u8 infpval2; / reserved,
    pub infppnum: u16,
    pub infpscps: u16,
    pub infpdcps: u16,
    pub infpsifl: u16,
    pub infpdifl: u16,
    pub reserved: u16,
    pub infppnam: [c_char; 8],
    pub infpwbcp: u32,
    pub infpabcp: u32,
    pub infpwbif: u32,
    pub infpabif: u32,
    pub infplgnm: [c_char; 8],
    pub infplgcp: u32,
    pub infplgif: u32,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sthyi_sctns {
    pub hdr: hdr_sctn,
    pub mac: mac_sctn,
    pub par: par_sctn,
    pub __packed: },
...
pub struct cpu_inf {
    pub lpar_cap: u64,
    pub lpar_grp_cap: u64,
    pub lpar_weight: u64,
    pub all_weight: u64,
    pub cpu_num_ded: c_int,
    pub cpu_num_shd: c_int,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct lpar_cpu_inf {
    pub cp: cpu_inf,
    pub ifl: cpu_inf,
}
...
#[no_mangle]
pub unsafe extern "C" fn cpu_id(ctidx: u8, diag224_buf: *mut c_void) -> u64 {
...
//
#[no_mangle]
unsafe extern "C" fn scale_cap(in: u32) -> u32 {
    static u32 scale_cap(u32 in)
    {
    return (0x10000 * in) / 100;
    }
#[no_mangle]
unsafe extern "C" fn fill_hdr(sctns: *mut sthyi_sctns) {
...
#[no_mangle]
unsafe extern "C" fn fill_stsi(sctns: *mut sthyi_sctns) {
...
// Returns a pointer to the next partition block.
    ...
    pages = diag204(subcode, 0, core::ptr::null_mut());
    if (pages < 0)
...
#[no_mangle]
unsafe extern "C" fn is_diag204_cached(sctns: *mut sthyi_sctns) -> bool {
...
#[no_mangle]
unsafe extern "C" fn fill_diag(sctns: *mut sthyi_sctns, diag204_buf: *mut c_void) {
    static void fill_diag(struct sthyi_sctns *sctns, void *diag204_buf)
    {
    int i;
    bool this_lpar;
    void *diag224_buf = core::ptr::null_mut();
    struct diag204_x_info_blk_hdr *ti_hdr;
    struct diag204_x_part_block *part_block;
    struct diag204_x_phys_block *phys_block;
    let mut lpar_inf: lpar_cpu_inf = {};
...
#[no_mangle]
unsafe extern "C" fn sthyi(vaddr: u64, rc: *mut u64) -> c_int {
    static int sthyi(u64 vaddr, u64 *rc)
    {
    union register_pair r1 = { .even = 0, }; /* subcode */
    let mut r2: union register_pair = { .even = vaddr, };
    int cc;
    asm volatile(
...
#[no_mangle]
unsafe extern "C" fn fill_dst(dst: *mut c_void, rc: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sthyi_init_cache() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sthyi_update_cache(rc: *mut u64) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn sthyi_fill(dst: *mut c_void, rc: *mut u64) -> c_int {
