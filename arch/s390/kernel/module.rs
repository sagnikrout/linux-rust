...
//! Source: arch/s390/kernel/module.c
#![no_std]
...
pub const PLT_ENTRY_SIZE: c_int = 22;

#[no_mangle]
pub unsafe extern "C" fn module_arch_cleanup(mod: *mut module) {
...
#[no_mangle]
pub unsafe extern "C" fn module_arch_freeing_init(mod: *mut module) {
    ...
    mod.arch.syminfo = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn check_rela(rela: *mut Elf_Rela, me: *mut module) {
...
// Increase core size by size of got & plt and set start
    ...
    let mut tmp: c_uint = val;
    write(dest, &tmp, 4);
    } else if (bits == 64) {
    let mut tmp: c_ulong = val;
    write(dest, &tmp, 8);
    }
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(R_390_12: r_type ==) -> else {
    else if (r_type == R_390_12)
    rc = apply_rela_bits(loc, val, 0, 12, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_16: r_type ==) -> else {
    else if (r_type == R_390_16)
    rc = apply_rela_bits(loc, val, 0, 16, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_20: r_type ==) -> else {
    else if (r_type == R_390_20)
    rc = apply_rela_bits(loc, val, 1, 20, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_32: r_type ==) -> else {
    else if (r_type == R_390_32)
    rc = apply_rela_bits(loc, val, 0, 32, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_64: r_type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(R_390_PC16DBL: r_type ==) -> else {
    else if (r_type == R_390_PC16DBL)
    rc = apply_rela_bits(loc, val, 1, 16, 1, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_PC32DBL: r_type ==) -> else {
    else if (r_type == R_390_PC32DBL)
    rc = apply_rela_bits(loc, val, 1, 32, 1, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_PC32: r_type ==) -> else {
    else if (r_type == R_390_PC32)
    rc = apply_rela_bits(loc, val, 1, 32, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_PC64: r_type ==) -> else {
...
// (long *)&insn[14] = val;
    ...
    rc = apply_rela_bits(loc, val, 1, 16, 1, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_PLTOFF16: r_type ==) -> else {
    else if (r_type == R_390_PLTOFF16)
    rc = apply_rela_bits(loc, val, 0, 16, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_PLT32DBL: r_type ==) -> else {
    ...
    rc = apply_rela_bits(loc, val, 0, 16, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_GOTOFF32: r_type ==) -> else {
    else if (r_type == R_390_GOTOFF32)
    rc = apply_rela_bits(loc, val, 0, 32, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_GOTOFF64: r_type ==) -> else {
    ...
    rc = apply_rela_bits(loc, val, 1, 32, 0, write);
#[no_mangle]
pub unsafe extern "C" fn if(R_390_GOTPCDBL: r_type ==) -> else {
...
// Only needed if we want to support loading of
    ...
    struct module *me)
    {
    let mut early: bool = me.state == MODULE_STATE_UNFORMED;
