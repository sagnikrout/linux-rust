...
//! Source: arch/s390/kernel/kprobes.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn copy_instruction(p: *mut kprobe) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct swap_insn_args {
    pub p: *mut kprobe,
    pub 1: unsigned int arm_kprobe :,
}

#[no_mangle]
unsafe extern "C" fn swap_instruction(data: *mut c_void) -> c_int {
...
pub unsafe extern "C" fn arch_arm_kprobe(p: *mut kprobe) {
    ...
    {
    let mut args: swap_insn_args = {.p = p, .arm_kprobe = 1};
    if (cpu_has_seq_insn()) {
    swap_instruction(&args);
    text_poke_sync();
    } else {
    stop_machine_cpuslocked(swap_instruction, &args, core::ptr::null_mut());
    }
    }
...
pub unsafe extern "C" fn arch_disarm_kprobe(p: *mut kprobe) {
    ...
    {
    let mut args: swap_insn_args = {.p = p, .arm_kprobe = 0};
    if (cpu_has_seq_insn()) {
    swap_instruction(&args);
    text_poke_sync();
    } else {
...
//
#[no_mangle]
unsafe extern "C" fn push_kprobe(kcb: *mut kprobe_ctlblk, p: *mut kprobe) {
...
//
#[no_mangle]
unsafe extern "C" fn pop_kprobe(kcb: *mut kprobe_ctlblk) {
    static void pop_kprobe(struct kprobe_ctlblk *kcb)
    {
    __this_cpu_write(current_kprobe, kcb.prev_kprobe.kp);
    kcb.kprobe_status = kcb.prev_kprobe.status;
    kcb.prev_kprobe.kp = core::ptr::null_mut();
    }
    NOKPROBE_SYMBOL(pop_kprobe);
#[no_mangle]
unsafe extern "C" fn kprobe_reenter_check(kcb: *mut kprobe_ctlblk, p: *mut kprobe) {
...
#[no_mangle]
unsafe extern "C" fn kprobe_handler(regs: *mut pt_regs) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn resume_execution(p: *mut kprobe, regs: *mut pt_regs) {
    static void resume_execution(struct kprobe *p, struct pt_regs *regs)
    {
    struct kprobe_ctlblk *kcb = get_kprobe_ctlblk();
    let mut ip: c_ulong = regs.psw.addr;
    let mut fixup: c_int = probe_get_fixup_type(p.ainsn.insn);
    if (fixup & FIXUP_PSW_NORMAL)
    ip += (unsigned long) p.addr - (unsigned long) p.ainsn.insn;
    if (fixup & FIXUP_BRANCH_NOT_TAKEN) {
    let mut ilen: c_int = insn_length(p.ainsn.insn[0] >> 8);
    if (ip - (unsigned long) p.ainsn.insn == ilen)
    ip = (unsigned long) p.addr + ilen;
    }
    if (fixup & FIXUP_RETURN_REGISTER) {
    let mut reg: c_int = (p.ainsn.insn[0] & 0xf0) >> 4;
...
#[no_mangle]
unsafe extern "C" fn post_kprobe_handler(regs: *mut pt_regs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn kprobe_trap_handler(regs: *mut pt_regs, trapnr: c_int) -> c_int {
