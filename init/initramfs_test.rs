...
#[no_mangle]
unsafe extern "C" fn initramfs_test_extract(test: *mut kunit) -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn initramfs_test_fname_overrun(test: *mut kunit) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn initramfs_test_data(test: *mut kunit) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn initramfs_test_csum(test: *mut kunit) -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn initramfs_test_hardlink(test: *mut kunit) -> void __init {
    static void __init initramfs_test_hardlink(struct kunit *test)
    {
    char *err, *cpio_srcbuf;
    size_t len;
    let mut st0: kstat = {}, st1 = {};
    struct initramfs_test_cpio c[] = { {
    .magic = "070701",
    .ino = 1,
    .mode = S_IFREG | 0777,
...
#[no_mangle]
unsafe extern "C" fn initramfs_test_many(test: *mut kunit) -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn initramfs_test_fname_pad(test: *mut kunit) -> void __init {
...
// align file data at 4K archive offset via padded fname
    ...
    .data = fdata,
    pub }: },
    pub sizeof("padded_fname")): memcpy(tbufs->padded_fname, "padded_fname",,
    pub tbufs->cpio_srcbuf): len = fill_cpio(c, ARRAY_SIZE(c), false,,
    pub len): err = unpack_to_rootfs(tbufs->cpio_srcbuf,,
    pub err): KUNIT_EXPECT_NULL(test,,
    pub 0): file = filp_open(c[0].fname, O_RDONLY,,
    if (IS_ERR(file)) {
    pub failed"): KUNIT_FAIL(test, "open,
    pub out: goto,
    }
// read back file contents into @cpio_srcbuf and confirm match
    pub NULL): len = kernel_read(file, tbufs->cpio_srcbuf, c[0].filesize,,
    pub c[0].filesize): KUNIT_EXPECT_EQ(test, len,,
    pub len): KUNIT_EXPECT_MEMEQ(test, tbufs->cpio_srcbuf, c[0].data,,
    pub 0): KUNIT_EXPECT_EQ(test, init_unlink(c[0].fname),,
    out:
    }
#[no_mangle]
unsafe extern "C" fn initramfs_test_fname_path_max(test: *mut kunit) -> void __init {
    static void __init initramfs_test_fname_path_max(struct kunit *test)
    {
    pub err: *mut c_char,
    pub len: usize,
    pub {}: kstat st0 = {}, st1 =,
    pub unpacked": char fdata[] = "this file data will not be,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_fname_path_max {
    pub 1]: char fname_oversize[PATH_MAX +,
    pub fname_ok: [c_char; PATH_MAX],
    pub 2]: *mut *mut char cpio_src[(CPIO_HDRLEN + PATH_MAX + 3 + sizeof(fdata)),
    pub test_fname_path_max): *mut *mut } tbufs = kzalloc_obj(struct,
    struct initramfs_test_cpio c[] = { {
    .magic = "070701",
    .ino = 1,
    .mode = S_IFDIR | 0777,
    ...
    pub }: },
    pub 1): memset(tbufs->fname_oversize, '/', sizeof(tbufs->fname_oversize) -,
    pub 1): memset(tbufs->fname_ok, '/', sizeof(tbufs->fname_ok) -,
    memcpy(tbufs.fname_oversize, "fname_oversize",
    pub 1): sizeof("fname_oversize") -,
    pub 1): memcpy(tbufs->fname_ok, "fname_ok", sizeof("fname_ok") -,
    pub tbufs->cpio_src): len = fill_cpio(c, ARRAY_SIZE(c), false,,
// unpack skips over fname_oversize instead of returning an error
    pub len): err = unpack_to_rootfs(tbufs->cpio_src,,
    pub err): KUNIT_EXPECT_NULL(test,,
    pub -ENOENT): KUNIT_EXPECT_EQ(test, init_stat("fname_oversize", &st0, 0),,
    pub 0): KUNIT_EXPECT_EQ(test, init_stat("fname_ok", &st1, 0),,
    pub 0): KUNIT_EXPECT_EQ(test, init_rmdir("fname_ok"),,
    }
#[no_mangle]
unsafe extern "C" fn initramfs_test_hdr_hex(test: *mut kunit) -> void __init {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct initramfs_test_bufs {
    pub 2]: *mut *mut char cpio_src[(CPIO_HDRLEN + PATH_MAX + 3 + sizeof(fdata)),
    pub initramfs_test_bufs): *mut *mut } tbufs = kzalloc_obj(struct,
    struct initramfs_test_cpio c[] = { {
    .magic = "070701",
    .ino = 1,
    .mode = S_IFREG | 0777,
...
// inject_ox=true to add "0x" cpio field prefixes
    pub tbufs->cpio_src): len = fill_cpio(c, ARRAY_SIZE(c), true,,
    pub len): err = unpack_to_rootfs(tbufs->cpio_src,,
    pub err): KUNIT_EXPECT_NOT_NULL(test,,
    }
//
...
#[no_mangle]
unsafe extern "C" fn initramfs_suite_init(suite: *mut kunit_suite) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn initramfs_test_init(test: *mut kunit) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn initramfs_test_exit(test: *mut kunit) -> void __init {
