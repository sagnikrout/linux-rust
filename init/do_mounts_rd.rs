...
#[no_mangle]
unsafe extern "C" fn ramdisk_start_setup(str: *mut c_char) -> int __init {
...
//
    ...
    unsigned long n;
    let mut start_block: c_int = rd_image_start;
...
#[no_mangle]
pub unsafe extern "C" fn rd_load_image() -> int __init {
    ...
    let mut rotate: c_ushort = 0;
    let mut decompressor: decompress_fn = core::ptr::null_mut();
    char rotator[4] = { '|' , '/' , '-' , '\\' };
...
unsafe extern "C" fn compr_fill(buf: *mut c_void, len: c_ulong) -> long __init {
    ...
    {
    let mut r: c_long = kernel_read(in_file, buf, len, &in_pos);
    if (r < 0)
...
unsafe extern "C" fn compr_flush(window: *mut c_void, outcnt: c_ulong) -> long __init {
    ...
    {
    let mut written: c_long = kernel_write(out_file, window, outcnt, &out_pos);
    if (written != outcnt) {
...
#[no_mangle]
unsafe extern "C" fn crd_load(deco: decompress_fn) -> int __init {
    static int __init crd_load(decompress_fn deco)
    {
    int result;
    if (!deco) {
