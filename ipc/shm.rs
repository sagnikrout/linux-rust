...
#[no_mangle]
unsafe extern "C" fn ipc_ns_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn shm_init() -> void __init {
...
unsafe extern "C" fn newseg(ns: *mut ipc_namespace, params: *mut ipc_params) -> c_int {
    ...
    {
    let mut key: key_t = params.key;
    let mut shmflg: c_int = params.flg;
    let mut size: usize = params.u.size;
    int error;
    struct shmid_kernel *shp;
    let mut numpages: usize = (size + PAGE_SIZE - 1) >> PAGE_SHIFT;
    let mut has_no_reserve: bool = shmflg & SHM_NORESERVE;
    let mut acctflag: vma_flags_t = EMPTY_VMA_FLAGS;
    struct file *file;
    char name[13];
...
#[no_mangle]
pub unsafe extern "C" fn copy_shmid_to_user(buf: *mut void __user, in: *mut shmid64_ds, version: c_int) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn copy_shminfo_to_user(buf: *mut void __user, in: *mut shminfo64, version: c_int) -> c_ulong {
...
// do_shm_rmid unlocks the ipc object and rcu
    ...
    {
    let mut err: c_int = security_shm_shmctl(core::ptr::null_mut(), IPC_INFO);
    if (!err) {
    ...
    {
    let mut err: c_int = security_shm_shmctl(core::ptr::null_mut(), SHM_INFO);
    if (!err) {
...
// check if shm_destroy() is tearing down shp
    ...
    let mut euid: kuid_t = current_euid();
...
#[no_mangle]
unsafe extern "C" fn ksys_shmctl(shmid: c_int, cmd: c_int, buf: *mut shmid_ds __user, version: c_int) -> c_long {
...
#[no_mangle]
pub unsafe extern "C" fn ksys_old_shmctl(shmid: c_int, cmd: c_int, buf: *mut shmid_ds __user) -> c_long {
    long ksys_old_shmctl(int shmid, int cmd, struct shmid_ds __user *buf)
    {
    let mut version: c_int = ipc_parse_version(&cmd);
...
pub struct compat_shm_info {
    ...
    pub shm_swp: compat_ulong_t shm_tot, shm_rss,,
    pub swap_successes: compat_ulong_t swap_attempts,,
}
...
#[no_mangle]
unsafe extern "C" fn compat_ksys_shmctl(shmid: c_int, cmd: c_int, uptr: *mut void __user, version: c_int) -> c_long {
...
#[no_mangle]
pub unsafe extern "C" fn compat_ksys_old_shmctl(shmid: c_int, cmd: c_int, uptr: *mut void __user) -> c_long {
    long compat_ksys_old_shmctl(int shmid, int cmd, void __user *uptr)
    {
    let mut version: c_int = compat_ipc_parse_version(&cmd);
...
//
    ...
    {
    struct shmid_kernel *shp;
    let mut addr: c_ulong = (unsigned long)shmaddr;
    unsigned long size;
    struct file *file, *base;
    int    err;
    let mut flags: c_ulong = MAP_SHARED;
    unsigned long prot;
    int acc_mode;
    struct ipc_namespace *ns;
    struct shm_file_data *sfd;
    int f_flags;
    let mut populate: c_ulong = 0;
    err = -EINVAL;
    if (shmid < 0)
    goto out;
    if (addr) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ksys_shmdt(shmaddr: *mut char __user) -> c_long {
