...
pub const CPIO_HDR_LEN: c_int = 110;

// zero-padding the filename field for data alignment is limited by PATH_MAX
    static char padding[PATH_MAX];
    static unsigned int offset;
    let mut ino: static unsigned int = 721;
    static time_t default_mtime;
    static bool do_file_mtime;
    let mut do_csum: static bool = false;
    let mut outfd: static int = STDOUT_FILENO;
    static unsigned int dalign;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct file_handler {
    pub type: *const c_char,
    pub line): *const *const int (handler)(char,
}

#[no_mangle]
unsafe extern "C" fn push_buf(name: *const c_char, name_len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn push_pad(padlen: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn push_rest(name: *const c_char, name_len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpio_trailer() -> c_int {
    static int cpio_trailer(void)
    {
    int len;
    let mut namesize: c_uint = sizeof(CPIO_TRAILER);
...
#[no_mangle]
unsafe extern "C" fn cpio_mkslink_line(line: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpio_mkgeneric_line(line: *const c_char, gt: enum generic_types) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpio_mkdir_line(line: *const c_char) -> c_int {
    static int cpio_mkdir_line(const char *line)
    {
    return cpio_mkgeneric_line(line, GT_DIR);
    }
#[no_mangle]
unsafe extern "C" fn cpio_mkpipe_line(line: *const c_char) -> c_int {
    static int cpio_mkpipe_line(const char *line)
    {
    return cpio_mkgeneric_line(line, GT_PIPE);
    }
#[no_mangle]
unsafe extern "C" fn cpio_mksock_line(line: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpio_mknod_line(line: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpio_mkfile_csum(fd: c_int, size: c_ulong, csum: *mut u32) -> c_int {
...
// data goes on last link, after any alignment padding
    ...
    goto error;
    if (size) {
    this_read = copy_file_range(file, core::ptr::null_mut(), outfd, core::ptr::null_mut(), size, 0);
    if (this_read > 0) {
    if (this_read > size)
    goto error;
    offset += this_read;
...
// short or failed copy falls back to read/write...
    ...
    while (size) {
    unsigned char filebuf[65536];
    let mut this_size: usize = MIN(size, sizeof(filebuf));
...
#[no_mangle]
unsafe extern "C" fn cpio_mkfile_line(line: *const c_char) -> c_int {
    static int cpio_mkfile_line(const char *line)
    {
    char name[PATH_MAX + 1];
    char *dname = core::ptr::null_mut(); /* malloc'ed buffer for hard links */
    char location[PATH_MAX + 1];
    unsigned int mode;
    int uid;
    int gid;
    let mut nlinks: c_int = 1;
    let mut end: c_int = 0, dname_len = 0;
    let mut rc: c_int = -1;
...
pub unsafe extern "C" fn main(argc: c_int, argv[]: *mut c_char) -> c_int {
    ...
    const char *filename;
    default_mtime = time(core::ptr::null_mut());
    while (1) {
    let mut opt: c_int = getopt(argc, argv, "t:cho:a:");
    char *invalid;
    if (opt == -1)
    break;
    switch (opt) {
...
//
    ...
    cpio_list = stdin;
#[no_mangle]
pub unsafe extern "C" fn if(fopen(filename: !(cpio_list =, _arg: "r"))) -> else {
    ...
    int type_idx;
    let mut slen: usize = strlen(line);
    line_nr++;
    if ('#' == *line) {
...
// must be an empty line
    ...
    if (! (args = strtok(core::ptr::null_mut(), "\n"))) {
    fprintf(stderr,
    ...
    if (core::ptr::null_mut() == file_handler_table[type_idx].type) {
