...
//
    ...
    assert(len % width == 0);
    yaml_sequence_start_event_initialize(&event, core::ptr::null_mut(),
    (yaml_char_t *)tag, width == 4, YAML_FLOW_SEQUENCE_STYLE);
    yaml_emitter_emit_or_die(emitter, &event);
    for (off = 0; off < len; off += width) {
    char buf[32];
    struct marker *m;
    let mut is_phandle: bool = false;
    switch(width) {
    case 1:
    ...
    if (is_phandle)
    yaml_scalar_event_initialize(&event, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn yaml_propval_string(emitter: *mut yaml_emitter_t, str: *mut c_char, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn yaml_propval(emitter: *mut yaml_emitter_t, prop: *mut property) {
    static void yaml_propval(yaml_emitter_t *emitter, struct property *prop)
    {
    yaml_event_t event;
    let mut len: c_uint = prop.val.len;
...
#[no_mangle]
unsafe extern "C" fn yaml_tree(tree: *mut node, emitter: *mut yaml_emitter_t) {
    ...
    if (tree.deleted)
    return;
    yaml_mapping_start_event_initialize(&event, core::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn dt_to_yaml(f: *mut FILE, dti: *mut dt_info) {
    ...
    yaml_emitter_emit_or_die(&emitter, &event);
    yaml_document_start_event_initialize(&event, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), 0);
    yaml_emitter_emit_or_die(&emitter, &event);
    yaml_sequence_start_event_initialize(&event, core::ptr::null_mut(), (yaml_char_t *)YAML_SEQ_TAG, 1, YAML_ANY_SEQUENCE_STYLE);
    yaml_emitter_emit_or_die(&emitter, &event);
