...
//! Source: scripts/asn1_compiler.c
#![no_std]
...
// 15
    ...
    [DIRECTIVE_NULL] = "core::ptr::null_mut()",
    _(NumericString),
    _(OBJECT),
    _(OCTET),
    _(OF),
...
pub struct token {
    pub line: c_ushort,
    pub 8: enum token_type token_type :,
    pub size: c_uchar,
...
#[no_mangle]
unsafe extern "C" fn directive_compare(_key: *const c_void, _pdir: *const c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tokenise(buffer: *mut c_char, end: *mut c_char) {
...
pub unsafe extern "C" fn main(argc: c_int, argv: *mut c_char) -> c_int {
    ...
    verbose_opt = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[1], 0: "-d") ==) -> else {
...
#[derive(Copy, Clone)]
pub struct element {
    pub type_def: *mut type,
    pub name: *mut token,
    pub type: *mut token,
    pub action: *mut action,
    pub children: *mut element,
    pub next: *mut element,
    pub render_next: *mut element,
    pub list_next: *mut element,
    pub n_elements: u8,
    pub 8: enum compound compound :,
    pub 8: enum asn1_class class :,
    pub 8: enum asn1_method method :,
    pub tag: u8,
    pub entry_index: unsigned,
    pub flags: unsigned,
pub const ELEMENT_IMPLICIT: c_uint = 0x0001;
pub const ELEMENT_EXPLICIT: c_uint = 0x0002;
pub const ELEMENT_TAG_SPECIFIED: c_uint = 0x0004;
pub const ELEMENT_RENDERED: c_uint = 0x0008;
pub const ELEMENT_SKIPPABLE: c_uint = 0x0010;
pub const ELEMENT_CONDITIONAL: c_uint = 0x0020;
}
...
pub struct type {
    pub name: *mut token,
    pub def: *mut token,
    pub element: *mut element,
    pub ref_count: unsigned,
    pub flags: unsigned,
pub const TYPE_STOP_MARKER: c_uint = 0x0001;
pub const TYPE_BEGIN: c_uint = 0x0002;
}
...
#[no_mangle]
unsafe extern "C" fn type_index_compare(_a: *const c_void, _b: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn type_finder(_key: *const c_void, _ti: *const c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn build_type_list() {
    static void build_type_list(void)
    {
    struct type *types;
    unsigned nr, t, n;
...
// Parse one type definition statement at a time
    ...
    abort();
    cursor += 2;
    type.element = parse_type(&cursor, type[1].name, core::ptr::null_mut());
...
//
    ...
    struct type **ref;
    char *p;
    let mut labelled: c_int = 0, implicit = 0;
...
// Extract the type we're expecting here
    ...
    if (cursor >= end)
    goto overrun_error;
    element.children = parse_type(&cursor, end, core::ptr::null_mut());
    } else {
...
// ppaction;
    ...
    let mut cmp: c_int = strcmp(action.name, (*ppaction).name);
    if (cmp == 0) {
    free(action);
    action = *ppaction;
    goto found;
...
#[no_mangle]
unsafe extern "C" fn dump_element(e: *const element, level: c_int) {
    ...
    strcpy(tag, "<...>");
#[no_mangle]
pub unsafe extern "C" fn if(ASN1_UNIV: e->class ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn dump_elements() {
    static void dump_elements(void)
    {
    if (debug_opt)
    ...
    let mut render_depth: static int = 1;
    static struct element *render_list, **render_list_p = &render_list;
    __attribute__((format(printf, 2, 3)))
#[no_mangle]
unsafe extern "C" fn render_opcode(out: *mut FILE, fmt: *const c_char, ...) {
    ...
    __attribute__((format(printf, 2, 3)))
#[no_mangle]
unsafe extern "C" fn render_more(out: *mut FILE, fmt: *const c_char, ...) {
...
//
#[no_mangle]
unsafe extern "C" fn render(out: *mut FILE, hdr: *mut FILE) {
    static void render(FILE *out, FILE *hdr)
    {
    struct element *e;
...
// We do two passes - the first one calculates all the offsets
    ...
    nr_entries = 0;
    root = &type_list[0];
    render_element(core::ptr::null_mut(), root.element, core::ptr::null_mut());
    render_opcode(core::ptr::null_mut(), "ASN1_OP_COMPLETE,\n");
    render_out_of_line_list(core::ptr::null_mut());
...
// And then we actually render
    ...
    nr_entries = 0;
    root = &type_list[0];
    render_element(out, root.element, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn render_out_of_line_list(out: *mut FILE) {
    static void render_out_of_line_list(FILE *out)
    {
    struct element *e, *ce;
    const char *act;
    ...
    render_element(out, ce, core::ptr::null_mut());
    render_depth--;
    act = e.action ? "_ACT" : "";
    switch (e.compound) {
    case SEQUENCE:
...
//
#[no_mangle]
unsafe extern "C" fn render_element(out: *mut FILE, e: *mut element, tag: *mut element) {
    static void render_element(FILE *out, struct element *e, struct element *tag)
    {
    struct element *ec, *x;
    const char *cond, *act;
...
// Render inline for single use
    ...
    render_element(out, ec, core::ptr::null_mut());
    render_depth--;
    render_opcode(out, "ASN1_OP_END_SEQ%s,\n", act);
    }
    break;
...
// Render inline for single use
    entry = nr_entries;
    render_depth++;
    render_element(out, e.children, core::ptr::null_mut());
    render_depth--;
