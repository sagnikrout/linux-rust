...
pub const HASHSZ: c_int = 256;
    static struct item *config_hashtab[HASHSZ], *file_hashtab[HASHSZ];
#[no_mangle]
unsafe extern "C" fn strhash(str: *const c_char, sz: c_uint) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn in_hashtable(name: *const c_char, len: c_int, hashtab[]: *mut item) -> bool {
    static bool in_hashtable(const char *name, int len, struct item *hashtab[])
    {
    struct item *aux;
    let mut hash: c_uint = strhash(name, len);
...
//
#[no_mangle]
unsafe extern "C" fn use_config(m: *const c_char, slen: c_int) {
...
#[no_mangle]
unsafe extern "C" fn str_ends_with(s: *const c_char, slen: c_int, sub: *const c_char) -> c_int {
    static int str_ends_with(const char *s, int slen, const char *sub)
    {
    let mut sublen: c_int = strlen(sub);
    if (sublen > slen)
    return 0;
    return !memcmp(s + slen - sublen, sub, sublen);
    }
#[no_mangle]
unsafe extern "C" fn parse_config_file(p: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn is_ignored_file(s: *const c_char, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_no_parse_file(s: *const c_char, len: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn parse_dep_file(p: *mut c_char, target: *const c_char) {
    static void parse_dep_file(char *p, const char *target)
    {
    let mut saw_any_target: bool = false;
    let mut is_target: bool = true;
