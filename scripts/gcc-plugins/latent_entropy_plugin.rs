...
//! Source: scripts/gcc-plugins/latent_entropy_plugin.c
#![no_std]
...
//
    ...
    let mut rnd_idx: static size_t = ARRAY_SIZE(rnd_buf);
    let mut urandom_fd: static int = -1;
#[no_mangle]
unsafe extern "C" fn get_random_const() -> unsigned HOST_WIDE_INT {
    static unsigned HOST_WIDE_INT get_random_const(void)
    {
    if (deterministic_seed) {
    let mut w: unsigned HOST_WIDE_INT = deterministic_seed;
    w ^= w << 13;
    w ^= w >> 7;
    w ^= w << 17;
...
#[no_mangle]
unsafe extern "C" fn tree_get_random_const(type: tree) -> tree {
...
// node, name);
    ...
    vec_alloc(vals, nelt);
    for (i = 0; i < nelt; i++) {
    let mut cst: tree = size_int(i);
    let mut rand_cst: tree = tree_get_random_const(elt_type);
...
//
    ...
    let mut latent_entropy_attr: static struct attribute_spec = { };
#[no_mangle]
unsafe extern "C" fn register_attributes(__unused: *mut *mut void event_data, __unused: *mut *mut void data) {
...
#[no_mangle]
unsafe extern "C" fn latent_entropy_gate() -> bool {
...
#[no_mangle]
unsafe extern "C" fn create_var(type: tree, name: *const c_char) -> tree {
...
//
#[no_mangle]
unsafe extern "C" fn get_op(rhs: *mut tree) -> enum tree_code {
...
#[no_mangle]
unsafe extern "C" fn perturb_local_entropy(bb: basic_block, local_entropy: tree) {
...
#[no_mangle]
unsafe extern "C" fn handle_tail_calls(bb: basic_block, local_entropy: tree) -> bool {
    static bool handle_tail_calls(basic_block bb, tree local_entropy)
    {
    gimple_stmt_iterator gsi;
    for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
    gcall *call;
    let mut stmt: gimple = gsi_stmt(gsi);
...
#[no_mangle]
unsafe extern "C" fn perturb_latent_entropy(local_entropy: tree) {
    ...
    let mut gsi: gimple_stmt_iterator = gsi_last_bb(last_bb);
    __perturb_latent_entropy(&gsi, local_entropy);
    }
    }
#[no_mangle]
unsafe extern "C" fn init_local_entropy(bb: basic_block, local_entropy: tree) {
    ...
    let mut gsi: gimple_stmt_iterator = gsi_after_labels(bb);
...
#[no_mangle]
unsafe extern "C" fn create_latent_entropy_decl() -> bool {
...
#[no_mangle]
unsafe extern "C" fn latent_entropy_execute() -> c_uint {
...
//
    ...
    &latent_entropy_start_unit, core::ptr::null_mut());
    register_callback(plugin_name, PLUGIN_REGISTER_GGC_ROOTS,
    core::ptr::null_mut(), (void *)&gt_ggc_r_gt_latent_entropy);
