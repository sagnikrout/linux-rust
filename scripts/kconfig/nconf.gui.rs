...
//! Source: scripts/kconfig/nconf.gui.c
#![no_std]
...
pub unsafe extern "C" fn set_colors() {
    ...
    for (; p.attr; p++) {
    let mut attr: c_int = p.highlight;
    if (p.has_color) {
    pair++;
...
#[no_mangle]
pub unsafe extern "C" fn print_in_middle(win: *mut WINDOW, y: c_int, width: c_int, str: *const c_char, attrs: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn get_line_no(text: *const c_char) -> c_int {
    int get_line_no(const char *text)
    {
    int i;
    let mut total: c_int = 1;
    if (!text)
    return 0;
    for (i = 0; text[i] != '\0'; i++)
    if (text[i] == '\n')
...
#[no_mangle]
pub unsafe extern "C" fn get_line_length(line: *const c_char) -> c_int {
...
pub unsafe extern "C" fn fill_window(win: *mut WINDOW, text: *const c_char) {
    ...
    {
    int x, y;
    let mut total_lines: c_int = get_line_no(text);
    int i;
    getmaxyx(win, y, x);
// do not go over end of line
    ...
    let mut len: c_int = min(get_line_length(line), x);
...
pub unsafe extern "C" fn btn_dialog(main_window: *mut WINDOW, msg: *const c_char, btn_num: c_int, ...) -> c_int {
    ...
    va_list ap;
    char *btn;
    let mut btns_width: c_int = 0;
    let mut msg_lines: c_int = 0;
    let mut msg_width: c_int = 0;
    int total_width;
    let mut win_rows: c_int = 0;
    WINDOW *win;
    WINDOW *msg_win;
    WINDOW *menu_win;
    MENU *menu;
    ...
    va_end(ap);
    btns[btn_num] = core::ptr::null_mut();
// find the widest line of msg:
    msg_lines = get_line_no(msg);
    for (i = 0; i < msg_lines; i++) {
    const char *line = get_line(msg, i);
    let mut len: c_int = get_line_length(line);
    if (msg_width < len)
    msg_width = len;
    }
...
// cycle through buttons
    ...
    {
    let mut prompt_lines: c_int = 0;
    let mut prompt_width: c_int = 0;
    WINDOW *win;
    WINDOW *prompt_win;
    WINDOW *form_win;
    PANEL *panel;
    ...
    let mut res: c_int = -1;
    let mut cursor_position: c_int = strlen(init);
    int cursor_form_win;
    char *result = *resultp;
...
// show the cursor
    ...
    let mut len: c_int = strlen(result);
    switch (res) {
    case 10: /* ENTER */
    case 27: /* ESCAPE */
    case KEY_F(F_HELP):
...
// insert the char at the proper position
    ...
    cursor_form_win = 0;
#[no_mangle]
pub unsafe extern "C" fn if(prompt_width-1: cursor_form_win >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn refresh_all_windows(main_window: *mut WINDOW) {
    ...
    const char *text)
    {
    (void)show_scroll_win_ext(main_window, title, (char *)text, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    }
// layman's scrollable window...
    ...
    let mut start_x: c_int = 0, start_y = 0;
    let mut text_lines: c_int = 0, text_cols = 0;
    let mut total_cols: c_int = 0;
    let mut win_cols: c_int = 0;
    let mut win_lines: c_int = 0;
    let mut i: c_int = 0;
    WINDOW *win;
    WINDOW *pad;
    PANEL *panel;
...
// handle scrolling
    ...
    default:
    if (extra_key_cb) {
    let mut start: usize = (get_line(text, start_y) - text);
    let mut end: usize = (get_line(text, start_y + text_lines) - text);
