...
//! Source: sound/virtio/virtio_kctl.c
#![no_std]
...
// Map for converting VirtIO event masks to ALSA event masks.
    ...
    let mut type: c_uint = le32_to_cpu(kinfo.type);
    let mut count: c_uint = le32_to_cpu(kinfo.count);
...
//
    ...
    let mut request_size: usize = sizeof(*hdr);
    let mut response_size: usize = sizeof(struct virtio_snd_hdr) + sizeof(*kvalue);
    unsigned int i;
    int rc;
...
//
    ...
    struct virtio_snd_ctl_hdr *hdr;
    struct virtio_snd_ctl_value *kvalue;
    let mut request_size: usize = sizeof(*hdr) + sizeof(*kvalue);
    let mut response_size: usize = sizeof(struct virtio_snd_hdr);
    unsigned int i;
    msg = virtsnd_ctl_msg_alloc(request_size, response_size, GFP_KERNEL);
    if (!msg)
    return -ENOMEM;
...
//
    ...
    rc = virtsnd_ctl_msg_send(snd, msg, core::ptr::null_mut(), &sg, false);
    if (!rc) {
    ...
    rc = virtsnd_ctl_msg_send(snd, msg, &sg, core::ptr::null_mut(), false);
    }
    break;
    default:
    rc = -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn virtsnd_kctl_get_enum_items(snd: *mut virtio_snd, cid: c_uint) -> c_int {
    ...
    struct virtio_snd_msg *msg;
    struct virtio_snd_ctl_hdr *hdr;
    let mut n: c_uint = le32_to_cpu(kinfo.value.enumerated.items);
    struct scatterlist sg;
    ...
    return virtsnd_ctl_msg_send(snd, msg, core::ptr::null_mut(), &sg, false);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn virtsnd_kctl_parse_cfg(snd: *mut virtio_snd) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn virtsnd_kctl_build_devs(snd: *mut virtio_snd) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn virtsnd_kctl_event(snd: *mut virtio_snd, event: *mut virtio_snd_event) {
    ...
    let mut cid: c_uint = le16_to_cpu(kevent.control_id);
