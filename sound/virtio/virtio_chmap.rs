...
//! Source: sound/virtio/virtio_chmap.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn virtsnd_chmap_parse_cfg(snd: *mut virtio_snd) -> c_int {
...
// Count the number of channel maps per each PCM device/stream.
    ...
    let mut nid: u32 = le32_to_cpu(info.hdr.hda_fn_nid);
...
//
#[no_mangle]
pub unsafe extern "C" fn virtsnd_chmap_build_devs(snd: *mut virtio_snd) -> c_int {
...
// Initialize channel maps per each PCM device/stream.
    for (i = 0; i < snd.nchmaps; ++i) {
    struct virtio_snd_chmap_info *info = &snd.chmaps[i];
    let mut channels: c_uint = info.channels;
    unsigned int ch;
    ...
    let mut position: u8 = info.positions[ch];
