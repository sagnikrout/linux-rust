...
//! Source: sound/drivers/mpu401/mpu401_uart.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn snd_mpu401_uart_clear_rx(mpu: *mut snd_mpu401) {
...
#[no_mangle]
unsafe extern "C" fn uart_interrupt_tx(mpu: *mut snd_mpu401) {
...
#[no_mangle]
unsafe extern "C" fn _snd_mpu401_uart_interrupt(mpu: *mut snd_mpu401) {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_mpu401_uart_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_mpu401_uart_interrupt_tx(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn snd_mpu401_uart_add_timer(mpu: *mut snd_mpu401, input: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn snd_mpu401_uart_remove_timer(mpu: *mut snd_mpu401, input: c_int) {
...
#[no_mangle]
unsafe extern "C" fn snd_mpu401_do_reset(mpu: *mut snd_mpu401) -> c_int {
...
unsafe extern "C" fn snd_mpu401_uart_input_close(substream: *mut snd_rawmidi_substream) -> c_int {
    ...
    mpu.substream_input = core::ptr::null_mut();
    if (! test_bit(MPU401_MODE_BIT_OUTPUT, &mpu.mode))
...
unsafe extern "C" fn snd_mpu401_uart_output_close(substream: *mut snd_rawmidi_substream) -> c_int {
    ...
    mpu.substream_output = core::ptr::null_mut();
    if (! test_bit(MPU401_MODE_BIT_INPUT, &mpu.mode))
    err = snd_mpu401_uart_cmd(mpu, MPU401_RESET, 0);
    if (mpu.close_output)
    mpu.close_output(mpu);
...
//
#[no_mangle]
unsafe extern "C" fn snd_mpu401_uart_input_read(mpu: *mut *mut snd_mpu401) {
    static void snd_mpu401_uart_input_read(struct snd_mpu401 * mpu)
    {
...
//
#[no_mangle]
unsafe extern "C" fn snd_mpu401_uart_output_write(mpu: *mut *mut snd_mpu401) {
    static void snd_mpu401_uart_output_write(struct snd_mpu401 * mpu)
    {
    unsigned char byte;
...
// rrawmidi = NULL;
    ...
    let mut res_size: c_int = hardware == MPU401_HW_PC98II ? 4 : 2;
