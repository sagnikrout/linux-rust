...
//! Source: sound/drivers/opl3/opl3_oss.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn snd_opl3_oss_create_port(opl3: *mut *mut snd_opl3) -> c_int {
    ...
    if (opl3.oss_chset == core::ptr::null_mut())
    return -ENOMEM;
...
// The instance should have been released in prior
    opl3.oss_seq_dev = core::ptr::null_mut();
    }
    }
// ------------------------------
// open OSS sequencer
#[no_mangle]
unsafe extern "C" fn snd_opl3_open_seq_oss(arg: *mut snd_seq_oss_arg, closure: *mut c_void) -> c_int {
    static int snd_opl3_open_seq_oss(struct snd_seq_oss_arg *arg, void *closure)
    {
...
// load patch
// from sound_config.h
pub const SBFM_MAXINSTR: c_int = 256;
    ...
    type = FM_PATCH_OPL2;
#[no_mangle]
pub unsafe extern "C" fn if(OPL3_PATCH: format ==) -> else {
    ...
    err = snd_opl3_load_patch(opl3, sbi.channel, 127, type, name, core::ptr::null_mut(),
    sbi.operators);
    if (err < 0)
    return err;
    return sizeof(sbi);
