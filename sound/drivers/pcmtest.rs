...
//! Source: sound/drivers/pcmtest.c
#![no_std]
...
pub const TIMER_PER_SEC: c_int = 5;

pub const PLAYBACK_SUBSTREAM_CNT: c_int = 8;
pub const CAPTURE_SUBSTREAM_CNT: c_int = 8;
pub const MAX_CHANNELS_NUM: c_int = 4;

pub const DEFAULT_PATTERN_LEN: c_int = 7;
pub const FILL_MODE_RAND: c_int = 0;
pub const FILL_MODE_PAT: c_int = 1;
pub const MAX_PATTERN_LEN: c_int = 4096;
    let mut index: static int = -1;
    ...
    let mut fill_mode: static short = FILL_MODE_PAT;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pcmtst_buf_iter {
    pub buffer: size_t buf_pos; // position in the DMA,
    pub position: size_t period_pos; // period-relative,
    pub tick: size_t b_rw; // Bytes to write on every timer,
    pub tick: size_t s_rw_ch; // Samples to write to one channel on every,
    pub 8: unsigned int sample_bytes; // sample_bits /,
    pub indicator: bool is_buf_corrupted; // playback test result,
    pub period: size_t period_bytes; // bytes in a one,
    pub mode: bool interleaved; // Interleaved/Non-interleaved,
    pub read/written: size_t total_bytes; // Total bytes,
    pub non-interleaved: size_t chan_block; // Bytes in one channel buffer when,
    pub substream: *mut snd_pcm_substream,
    pub down: bool suspend; // We need to pause timer without shutting it,
    pub timer_instance: timer_list,
}
...
#[no_mangle]
pub unsafe extern "C" fn inc_buf_pos(v_iter: *mut pcmtst_buf_iter, by: usize, bytes: usize) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ch_pos_i(b_total: usize, channels: c_uint, b_sample: c_uint) -> usize {
...
unsafe extern "C" fn check_buf_block_ni(v_iter: *mut pcmtst_buf_iter, runtime: *mut snd_pcm_runtime) {
    ...
    {
    let mut channels: c_uint = runtime.channels;
    size_t i;
    short ch_num;
    u8 current_byte;
    for (i = 0; i < v_iter.b_rw; i++) {
...
// Remaining space in all channel buffers
    let mut bytes_remain: usize = runtime.dma_bytes - v_iter.buf_pos;
    unsigned int i;
...
unsafe extern "C" fn fill_block_rand_i(v_iter: *mut pcmtst_buf_iter, runtime: *mut snd_pcm_runtime) {
    ...
    {
    let mut in_cur_block: usize = runtime.dma_bytes - v_iter.buf_pos;
...
#[no_mangle]
pub unsafe extern "C" fn reset_buf_iterator(v_iter: *mut pcmtst_buf_iter) {
...
#[no_mangle]
pub unsafe extern "C" fn start_pcmtest_timer(v_iter: *mut pcmtst_buf_iter) {
...
#[no_mangle]
unsafe extern "C" fn snd_pcmtst_dev_free(device: *mut snd_device) -> c_int {
    static int snd_pcmtst_dev_free(struct snd_device *device)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn pcmtst_pdev_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn snd_pcmtst_pcm_hw_free(substream: *mut snd_pcm_substream) -> c_int {
    static int snd_pcmtst_pcm_hw_free(struct snd_pcm_substream *substream)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn snd_pcmtst_ioctl(substream: *mut snd_pcm_substream, cmd: c_uint, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pattern_write(file: *mut file, u_buff: *const char __user, len: usize, off: *mut loff_t) -> isize {
...
#[no_mangle]
unsafe extern "C" fn pattern_read(file: *mut file, u_buff: *mut char __user, len: usize, off: *mut loff_t) -> isize {
    static ssize_t pattern_read(struct file *file, char __user *u_buff, size_t len, loff_t *off)
    {
    struct pattern_buf *patt_buf = file.f_inode.i_private;
    let mut to_read: isize = len;
...
unsafe extern "C" fn init_debug_files(buf_count: c_int) -> c_int {
    ...
    driver_debug_dir = debugfs_create_dir("pcmtest", core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn mod_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn mod_exit() -> void __exit {
