...
// Chip specific
//
pub const MTS64_NUM_INPUT_PORTS: c_int = 5;
pub const MTS64_NUM_OUTPUT_PORTS: c_int = 4;
pub const MTS64_SMPTE_SUBSTREAM: c_int = 4;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mts64 {
    pub lock: spinlock_t,
    ...
    pub current_midi_output_port: c_int,
    pub current_midi_input_port: c_int,
    pub mode: [u8; MTS64_NUM_INPUT_PORTS],
    pub midi_input_substream: [*mut snd_rawmidi_substream; MTS64_NUM_INPUT_PORTS],
    pub smpte_switch: c_int,
    pub /: *mut *mut u8 time[4]; / [0]=hh, [1]=mm, [2]=ss, [3]=ff,
    pub fps: u8,
}
...
//
// Status Bits
pub const MTS64_STAT_BSY: c_uint = 0x80;
pub const MTS64_STAT_BIT_SET: c_uint = 0x20  /* readout process, bit is set */;
pub const MTS64_STAT_PORT: c_uint = 0x10  /* read byte is a port number */;
// Control Bits
pub const MTS64_CTL_READOUT: c_uint = 0x08  /* enable readout */;
pub const MTS64_CTL_WRITE_CMD: c_uint = 0x06;
pub const MTS64_CTL_WRITE_DATA: c_uint = 0x02;
pub const MTS64_CTL_STROBE: c_uint = 0x01;
// Command
pub const MTS64_CMD_RESET: c_uint = 0xfe;
pub const MTS64_CMD_PROBE: c_uint = 0x8f  /* Used in probing procedure */;
pub const MTS64_CMD_SMPTE_SET_TIME: c_uint = 0xe8;
pub const MTS64_CMD_SMPTE_SET_FPS: c_uint = 0xee;
pub const MTS64_CMD_SMPTE_STOP: c_uint = 0xef;
pub const MTS64_CMD_SMPTE_FPS_24: c_uint = 0xe3;
pub const MTS64_CMD_SMPTE_FPS_25: c_uint = 0xe2;
pub const MTS64_CMD_SMPTE_FPS_2997: c_uint = 0xe4;
pub const MTS64_CMD_SMPTE_FPS_30D: c_uint = 0xe1;
pub const MTS64_CMD_SMPTE_FPS_30: c_uint = 0xe0;
pub const MTS64_CMD_COM_OPEN: c_uint = 0xf8  /* setting the communication mode */;
pub const MTS64_CMD_COM_CLOSE1: c_uint = 0xff  /* clearing communication mode */;
pub const MTS64_CMD_COM_CLOSE2: c_uint = 0xf5;
//
...
//
#[no_mangle]
unsafe extern "C" fn mts64_enable_readout(p: *mut parport) {
    static void mts64_enable_readout(struct parport *p)
    {
    u8 c;
...
//
#[no_mangle]
unsafe extern "C" fn mts64_disable_readout(p: *mut parport) {
    static void mts64_disable_readout(struct parport *p)
    {
    u8 c;
    c = parport_read_control(p);
...
//
#[no_mangle]
unsafe extern "C" fn mts64_device_ready(p: *mut parport) -> c_int {
    static int mts64_device_ready(struct parport *p)
    {
    int i;
    u8 c;
...
//
#[no_mangle]
unsafe extern "C" fn mts64_device_init(p: *mut parport) -> c_int {
    static int mts64_device_init(struct parport *p)
    {
    int i;
...
//
#[no_mangle]
unsafe extern "C" fn mts64_device_open(mts: *mut mts64) -> c_int {
    static int mts64_device_open(struct mts64 *mts)
    {
    int i;
...
//
#[no_mangle]
unsafe extern "C" fn mts64_device_close(mts: *mut mts64) -> c_int {
    static int mts64_device_close(struct mts64 *mts)
    {
    int i;
    struct parport *p = mts.pardev.port;
...
//
#[no_mangle]
unsafe extern "C" fn mts64_map_midi_input(c: u8) -> u8 {
    static u8 mts64_map_midi_input(u8 c)
    {
...
//
#[no_mangle]
unsafe extern "C" fn mts64_probe(p: *mut parport) -> c_int {
    static int mts64_probe(struct parport *p)
    {
    u8 c;
    mts64_smpte_stop(p);
...
//
#[no_mangle]
unsafe extern "C" fn mts64_read(p: *mut parport) -> u16 {
    static u16 mts64_read(struct parport *p)
    {
    u8 data, status;
...
//
#[no_mangle]
unsafe extern "C" fn mts64_read_char(p: *mut parport) -> u8 {
    static u8 mts64_read_char(struct parport *p)
    {
    let mut c: u8 = 0;
    u8 status;
...
//
#[no_mangle]
unsafe extern "C" fn mts64_smpte_stop(p: *mut parport) {
    static void mts64_smpte_stop(struct parport *p)
    {
...
//
#[no_mangle]
unsafe extern "C" fn mts64_write_command(p: *mut parport, c: u8) {
    static void mts64_write_command(struct parport *p, u8 c)
    {
    mts64_device_ready(p);
...
//
#[no_mangle]
unsafe extern "C" fn mts64_write_data(p: *mut parport, c: u8) {
    static void mts64_write_data(struct parport *p, u8 c)
    {
    mts64_device_ready(p);
    parport_write_data(p, c);
...
// smpte_switch is not accessed from IRQ handler, so we just need
    ...
    {
    struct mts64 *mts = snd_kcontrol_chip(kctl);
    let mut val: c_int = !!uctl.value.integer.value[0];
    guard(spinlock_irq)(&mts.lock);
...
// Time
    ...
    {
    struct mts64 *mts = snd_kcontrol_chip(kctl);
    let mut idx: c_int = kctl.private_value;
    guard(spinlock_irq)(&mts.lock);
    ...
    struct mts64 *mts = snd_kcontrol_chip(kctl);
    let mut idx: c_int = kctl.private_value;
    let mut time: c_uint = uctl.value.integer.value[0] % 60;
    guard(spinlock_irq)(&mts.lock);
...
// Rawmidi
//
pub const MTS64_MODE_INPUT_TRIGGERED: c_uint = 0x01;
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn snd_mts64_rawmidi_create(card: *mut snd_card) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_mts64_detach(p: *mut parport) {
...
#[no_mangle]
unsafe extern "C" fn snd_mts64_probe(pdev: *mut platform_device) -> c_int {
    ...
    struct mts64 *mts = core::ptr::null_mut();
    int err;
...
//
#[no_mangle]
unsafe extern "C" fn snd_mts64_unregister_all() {
...
#[no_mangle]
unsafe extern "C" fn snd_mts64_module_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn snd_mts64_module_exit() -> void __exit {
