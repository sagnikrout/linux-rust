...
//! Source: sound/soc/sdca/sdca_fdl.c
#![no_std]
...
//
    ...
    {
    let mut fdl_retries: static int = 6;
    let mut begin_timeout: c_ulong = msecs_to_jiffies(100);
    let mut done_timeout: c_ulong = msecs_to_jiffies(4000);
    int nfdl;
    int i, j;
...
//
    ...
    struct acpi_sw_file *swf = core::ptr::null_mut(), *tmp;
    struct sdca_fdl_file *fdl_file;
    char *disk_filename;
    int ret;
    int i;
    ...
    const struct firmware *firmware __free(firmware) = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn fdl_status_process(interrupt: *mut sdca_interrupt, status: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(SDCA_CTL_XU_FDLD_REQ_ABORT: status &) -> else {
