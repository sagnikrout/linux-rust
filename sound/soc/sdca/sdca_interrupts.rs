...
//! Source: sound/soc/sdca/sdca_interrupts.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn base_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn function_status_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn detected_mode_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn hid_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn no_pm_in_progress(dev: *mut device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn fdl_owner_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
//
    ...
    if (irq < 0)
    return irq;
    ret = request_threaded_irq(irq, core::ptr::null_mut(), handler, IRQF_ONESHOT, name, data);
    if (ret)
    return ret;
...
//
    ...
    let mut irq: c_int = control.interrupt_position;
    struct sdca_interrupt *interrupt;
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !interrupt) -> else {
    else if (!interrupt)
    continue;
    ret = sdca_irq_data_populate(dev, regmap, core::ptr::null_mut(), function,
