...
//! Source: sound/soc/sdca/sdca_asoc.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn exported_control(entity: *mut sdca_entity, control: *mut sdca_control) -> bool {
...
#[no_mangle]
unsafe extern "C" fn readonly_control(control: *mut sdca_control) -> bool {
    static bool readonly_control(struct sdca_control *control)
    {
    return control.has_fixed || control.mode == SDCA_ACCESS_MODE_RO;
    }
#[no_mangle]
unsafe extern "C" fn ge_count_routes(entity: *mut sdca_entity) -> c_int {
...
// num_dais = 0;
    ...
    let mut skip_primary_routes: bool = false;
...
// num_routes += entity->num_sources;
    ...
    return ret;
#[no_mangle]
pub unsafe extern "C" fn if(SDCA_DETECTED_MODE_DETECTION_IN_PROGRESS: ret <=) -> else {
    ...
    add_route(route, entity.label, core::ptr::null_mut(), entity.sources[i].label);
    return 0;
    }
    ...
    add_route(route, entity.label, core::ptr::null_mut(), aif_name);
    } else {
    (*widget).id = snd_soc_dapm_mic;
    }
    if (entity.iot.clock)
    add_route(route, entity.label, core::ptr::null_mut(), entity.iot.clock.label);
    for (i = 0; i < entity.num_sources; i++)
    add_route(route, entity.label, core::ptr::null_mut(), entity.sources[i].label);
    (*widget)++;
    return 0;
    ...
    add_route(route, aif_name, core::ptr::null_mut(), entity.label);
    } else {
...
//
    ...
    int num_delays)
    {
    let mut polls: static int = 100;
    let mut default_poll_us: static int = 1000;
    ...
    return ret;
#[no_mangle]
pub unsafe extern "C" fn if(to_ps: val ==) -> else {
    else if (val == to_ps)
    return 0;
    }
    return -ETIMEDOUT;
    ...
    struct snd_soc_dapm_route **route)
    {
    let mut target: c_uint = (1 << SDCA_PDE_PS0) | (1 << SDCA_PDE_PS3);
    struct sdca_control_range *range;
    struct sdca_control *control;
...
// Power should only be controlled by the driver
    ...
    add_route(route, entity.pde.managed[i].label, core::ptr::null_mut(), entity.label);
    for (i = 0; i < entity.num_sources; i++)
    add_route(route, entity.label, core::ptr::null_mut(), entity.sources[i].label);
    return 0;
    }
...
//
    ...
    return ret;
    if (entity.group)
    add_route(&route, entity.label, core::ptr::null_mut(), entity.group.label);
    }
    return 0;
    }
    ...
    {
    let mut mask: c_uint = GENMASK(mc.sign_bit, 0);
...
#[no_mangle]
unsafe extern "C" fn rate_find_mask(rate: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn width_find_mask(bits: c_uint) -> u64 {
...
//
    ...
    return -EINVAL;
    for (i = 0; i < range.rows; i++) {
    let mut clusterid: c_int = sdca_range(range, SDCA_CLUSTER_CLUSTERID, i);
...
//
    ...
    for (i = 0; i < range.rows; i++) {
    let mut port_mask: static u8 = 0xF;
...
//
    ...
    {
    let mut sel: c_int = SDCA_CTL_IT_CLUSTERINDEX;
    struct sdca_control_range *range;
    int i, ret;
    ...
    return -EINVAL;
    for (i = 0; i < range.rows; i++) {
    let mut cluster_id: c_int = sdca_range(range, SDCA_CLUSTER_CLUSTERID, i);
    struct sdca_cluster *cluster;
    cluster = sdca_id_find_cluster(dev, function, cluster_id);
    if (!cluster)
    return -ENODEV;
    if (cluster.num_channels == channels) {
    let mut index: c_int = sdca_range(range, SDCA_CLUSTER_BYTEINDEX, i);
    ...
    {
    let mut sel: c_int = SDCA_CTL_CS_SAMPLERATEINDEX;
    struct sdca_control_range *range;
    int i, ret;
    ...
    return -EINVAL;
    for (i = 0; i < range.rows; i++) {
    let mut rate: c_uint = sdca_range(range, SDCA_SAMPLERATEINDEX_RATE, i);
    if (rate == target_rate) {
    ...
    return -EINVAL;
    for (i = 0; i < range.rows; i++) {
    let mut rate: c_uint = sdca_range(range, SDCA_USAGE_SAMPLE_RATE, i);
    let mut width: c_uint = sdca_range(range, SDCA_USAGE_SAMPLE_WIDTH, i);
    if ((!rate || rate == target_rate) && (!width || width == target_width)) {
    let mut usage: c_uint = sdca_range(range, SDCA_USAGE_NUMBER, i);
    unsigned int reg = SDW_SDCA_CTL(function.desc.adr,
    entity.id, sel, 0);
...
//
    ...
    let mut width: c_int = params_width(params);
