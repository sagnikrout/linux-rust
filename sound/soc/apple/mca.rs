...
// Macro flag: #define USE_RXB_FOR_CAPTURE
// Relative to cluster base
pub const REG_STATUS: c_uint = 0x0;

pub const REG_MCLK_CONF: c_uint = 0x4;

pub const REG_SYNCGEN_STATUS: c_uint = 0x100;

pub const REG_SYNCGEN_MCLK_SEL: c_uint = 0x104;

pub const REG_SYNCGEN_HI_PERIOD: c_uint = 0x108;
pub const REG_SYNCGEN_LO_PERIOD: c_uint = 0x10c;
pub const REG_PORT_ENABLES: c_uint = 0x600;

pub const REG_PORT_CLOCK_SEL: c_uint = 0x604;

pub const REG_PORT_DATA_SEL: c_uint = 0x608;

pub const REG_INTSTATE: c_uint = 0x700;
pub const REG_INTMASK: c_uint = 0x704;
// Bases of serdes units (relative to cluster)
pub const CLUSTER_RXA_OFF: c_uint = 0x200;
pub const CLUSTER_TXA_OFF: c_uint = 0x300;
pub const CLUSTER_RXB_OFF: c_uint = 0x400;
pub const CLUSTER_TXB_OFF: c_uint = 0x500;

// Relative to serdes unit base
pub const REG_SERDES_STATUS: c_uint = 0x00;

pub const REG_TX_SERDES_CONF: c_uint = 0x04;
pub const REG_RX_SERDES_CONF: c_uint = 0x08;

pub const SERDES_CONF_WIDTH_16BIT: c_uint = 0x40;
pub const SERDES_CONF_WIDTH_20BIT: c_uint = 0x80;
pub const SERDES_CONF_WIDTH_24BIT: c_uint = 0xc0;
pub const SERDES_CONF_WIDTH_32BIT: c_uint = 0x100;
pub const SERDES_CONF_BCLK_POL: c_uint = 0x400;
pub const SERDES_CONF_LSB_FIRST: c_uint = 0x800;

pub const REG_TX_SERDES_BITSTART: c_uint = 0x08;
pub const REG_RX_SERDES_BITSTART: c_uint = 0x0c;
pub const REG_TX_SERDES_SLOTMASK: c_uint = 0x0c;
pub const REG_RX_SERDES_SLOTMASK: c_uint = 0x10;
pub const REG_RX_SERDES_PORT: c_uint = 0x04;
// Relative to switch base

pub const SWITCH_STRIDE: c_uint = 0x8000;
pub const CLUSTER_STRIDE: c_uint = 0x4000;
pub const MAX_NCLUSTERS: c_int = 6;

    SNDRV_PCM_FMTBIT_S24_LE | \
    SNDRV_PCM_FMTBIT_S32_LE)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mca_cluster {
    pub no: c_int,
    pub base: *mut __iomem void,
    pub host: *mut mca_data,
    pub pd_dev: *mut device,
    pub clk_parent: *mut clk,
    pub 1]: *mut *mut dma_chan dma_chans[SNDRV_PCM_STREAM_LAST +,
    pub clk_provider: bool,
    pub 1]: bool port_clk_started[SNDRV_PCM_STREAM_LAST +,
    pub /: *mut *mut int port_clk_driver; / The cluster driving this cluster's port,
    pub 1]: bool clocks_in_use[SNDRV_PCM_STREAM_LAST +,
    pub pd_link: *mut device_link,
// In case of clock consumer FE
    pub syncgen_in_use: c_int,
    pub bclk_ratio: c_uint,
// Masks etc. picked up via the set_tdm_slot method
    pub tdm_slots: c_int,
    pub tdm_slot_width: c_int,
    pub tdm_tx_mask: c_uint,
    pub tdm_rx_mask: c_uint,
}
...
pub struct mca_data {
    pub dev: *mut device,
    pub switch_base: *mut __iomem void,
    pub pd_dev: *mut device,
...
// Mutex for accessing port_clk_driver of foreign clusters
    ...
    pub nclusters: c_int,
    pub __counted_by(nclusters): mca_cluster clusters[],
}

#[no_mangle]
unsafe extern "C" fn mca_modify(cl: *mut mca_cluster, regoffset: c_int, mask: u32, val: u32) {
...
//
    let mut cluster_no: c_int = dai.id % mca.nclusters;
...
// called before PCM trigger
    ...
    let mut is_tx: bool = substream.stream == SNDRV_PCM_STREAM_PLAYBACK;
    let mut serdes_unit: c_int = is_tx ? CLUSTER_TX_OFF : CLUSTER_RX_OFF;
    int serdes_conf =
...
#[no_mangle]
unsafe extern "C" fn mca_fe_get_portmask(substream: *mut snd_pcm_substream) -> c_int {
    ...
    let mut no: c_int = mca_dai_to_cluster(snd_soc_rtd_to_cpu(dpcm.be, 0)).no;
    mask |= 1 << no;
    }
    return mask;
    }
#[no_mangle]
unsafe extern "C" fn mca_fe_enable_clocks(cl: *mut mca_cluster) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mca_fe_disable_clocks(cl: *mut mca_cluster) {
...
#[no_mangle]
unsafe extern "C" fn mca_fe_clocks_in_use(cl: *mut mca_cluster) -> bool {
...
// Turn on the cluster power domain if not already in use
    ...
    let mut port: c_int = ffs(mca_fe_get_portmask(substream));
    cl.pd_link = device_link_add(mca.dev, cl.pd_dev,
    DL_FLAG_STATELESS | DL_FLAG_PM_RUNTIME |
    DL_FLAG_RPM_ACTIVE);
    if (!cl.pd_link) {
...
#[no_mangle]
unsafe extern "C" fn mca_crop_mask(mask: c_uint, nchans: c_int) -> c_uint {
...
unsafe extern "C" fn mca_set_bclk_ratio(dai: *mut snd_soc_dai, ratio: c_uint) -> c_int {
    ...
    let mut samp_rate: c_uint = params_rate(params);
    let mut is_tx: bool = substream.stream == SNDRV_PCM_STREAM_PLAYBACK;
    let mut refine_tdm: bool = false;
...
//
    ...
    if (refine_tdm) {
    let mut nchannels: c_int = params_channels(params);
    if (nchannels > 2) {
...
//
#[no_mangle]
unsafe extern "C" fn mca_be_clk_started(cl: *mut mca_cluster) -> bool {
    ...
    int stream)
    {
    struct snd_soc_pcm_runtime *fe = core::ptr::null_mut();
    struct snd_soc_dpcm *dpcm;
...
//
    ...
    let mut dma_data: snd_dmaengine_dai_dma_data = {};
    int ret;
...
//
    ...
    {
    let mut is_tx: bool = (stream == SNDRV_PCM_STREAM_PLAYBACK);
    ...
    cl.dma_chans[i] = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn apple_mca_release(mca: *mut mca_data) {
...
unsafe extern "C" fn apple_mca_probe(pdev: *mut platform_device) -> c_int {
    ...
    mca.rstc = devm_reset_control_get_optional_shared(&pdev.dev, core::ptr::null_mut());
    if (IS_ERR(mca.rstc))
