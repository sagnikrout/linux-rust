...
//! Source: sound/soc/generic/audio-graph-card2.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn __graph_get_type(lnk: *mut device_node) -> enum graph_type {
...
//
    ...
    {
    let mut type: enum graph_type = __graph_get_type(lnk);
...
#[no_mangle]
unsafe extern "C" fn graph_lnk_is_multi(lnk: *mut device_node) -> c_int {
    ...
    struct device_node *rep = core::ptr::null_mut();
//
// multi {
// ports {
// =>	lnk:	port@0 { ...		   }; // to pair
...
// port = of_graph_get_port_by_id(ports, idx);
    if (*port) {
    struct device_node *ep __free(device_node) = of_graph_get_next_port_endpoint(*port, core::ptr::null_mut());
    ...
    simple_util_parse_convert(ports, core::ptr::null_mut(), adata);
    simple_util_parse_convert(port, core::ptr::null_mut(), adata);
    simple_util_parse_convert(ep,   core::ptr::null_mut(), adata);
    }
...
// };
//
    struct device_node *mcpu_ep		__free(device_node) = of_graph_get_next_port_endpoint(mcpu_port, core::ptr::null_mut());
    struct device_node *mcpu_ports		__free(device_node) = port_to_ports(mcpu_port);
    struct device_node *mcpu_port_top	__free(device_node) = of_graph_get_next_port(mcpu_ports, core::ptr::null_mut());
    struct device_node *mcpu_ep_top		__free(device_node) = of_graph_get_next_port_endpoint(mcpu_port_top, core::ptr::null_mut());
    ...
    let mut nm_max: c_int = max(dai_link.num_cpus, dai_link.num_codecs);
    let mut ret: c_int = -EINVAL;
    if (cpu_idx > dai_link.num_cpus)
    goto end;
    for_each_of_graph_port_endpoint(mcpu_port, mcpu_ep_n) {
    let mut codec_idx: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn graph_parse_daifmt(node: *mut device_node, daifmt: *mut c_uint) {
...
//
    fmt = snd_soc_daifmt_parse_format(node, core::ptr::null_mut());
    update_daifmt(FORMAT);
    update_daifmt(CLOCK);
    update_daifmt(INV);
    }
#[no_mangle]
unsafe extern "C" fn graph_parse_bitframe(ep: *mut device_node) -> c_uint {
    static unsigned int graph_parse_bitframe(struct device_node *ep)
    {
    struct device_node *port  __free(device_node) = ep_to_port(ep);
    struct device_node *ports __free(device_node) = port_to_ports(port);
    return	snd_soc_daifmt_clock_provider_from_bitmap(
    snd_soc_daifmt_parse_clock_provider_as_bitmap(ep,    core::ptr::null_mut()) |
    snd_soc_daifmt_parse_clock_provider_as_bitmap(port,  core::ptr::null_mut()) |
    snd_soc_daifmt_parse_clock_provider_as_bitmap(ports, core::ptr::null_mut()));
    }
    ...
    struct device_node *multi_cpu_port = core::ptr::null_mut(), *multi_codec_port = core::ptr::null_mut();
    struct snd_soc_dai_link_component *dlc;
    let mut daifmt: c_uint = 0;
    let mut playback_only: bool = 0, capture_only = 0;
    let mut trigger_start: enum snd_soc_trigger_order = SND_SOC_TRIGGER_ORDER_DEFAULT;
    let mut trigger_stop: enum snd_soc_trigger_order = SND_SOC_TRIGGER_ORDER_DEFAULT;
    let mut multi_cpu_port_idx: c_int = 1, multi_codec_port_idx = 1;
    int i;
...
// Don't use port_cpu / port_codec after here
    ...
    struct device_node *cpu_ep	__free(device_node) = of_graph_get_next_port_endpoint(cpu_port, core::ptr::null_mut());
...
//
    ...
    struct link_info *li)
    {
    struct device_node *ep	__free(device_node) = of_graph_get_next_port_endpoint(lnk, core::ptr::null_mut());
    struct device_node *rep	__free(device_node) = of_graph_get_remote_endpoint(ep);
    struct device_node *cpu_ep = core::ptr::null_mut();
    struct device_node *codec_ep = core::ptr::null_mut();
    struct snd_soc_dai_link *dai_link = simple_priv_to_link(priv, li.link);
    struct simple_dai_props *dai_props = simple_priv_to_props(priv, li.link);
    let mut is_cpu: c_int = graph_util_is_ports0(lnk);
    int ret;
    if (is_cpu) {
    cpu_ep = rep;
//
...
//
    ...
    struct device_node *ep0 __free(device_node) = of_graph_get_next_port_endpoint(port0, core::ptr::null_mut());
    struct device_node *ep1 __free(device_node) = of_graph_get_next_port_endpoint(port1, core::ptr::null_mut());
...
//
    ...
    {
    struct device *dev = simple_priv_to_dev(priv);
    let mut func: GRAPH2_CUSTOM = core::ptr::null_mut();
    let mut ret: c_int = -EINVAL;
    switch (gtype) {
    case GRAPH_NORMAL:
...
#[no_mangle]
unsafe extern "C" fn graph_counter(lnk: *mut device_node) -> c_int {
...
// loop for all listed CPU port
    of_for_each_phandle(&it, rc, node, "links", core::ptr::null_mut(), 0) {
    lnk = it.node;
    ...
    ret = simple_util_parse_widgets(priv, core::ptr::null_mut());
    if (ret < 0)
    goto end;
    ret = simple_util_parse_routing(priv, core::ptr::null_mut());
    if (ret < 0)
    goto end;
    ret = simple_util_parse_aux_devs(priv, core::ptr::null_mut());
    if (ret < 0)
    goto err;
...
// Card name should be set after graph_for_each_link()
    ret = simple_util_parse_card_name(priv, core::ptr::null_mut());
    if (ret < 0)
    goto err;
...
// Allocate the private data and the DAI link array
    ...
    if (!priv)
    return -ENOMEM;
    return audio_graph2_parse_of(priv, dev, core::ptr::null_mut());
    }
