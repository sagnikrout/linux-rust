...
//! Source: sound/soc/generic/audio-graph-card.c
#![no_std]
...
pub const DPCM_SELECTABLE: c_int = 1;
    ...
    0, 0, core::ptr::null_mut(), 0, graph_outdrv_event,
...
unsafe extern "C" fn soc_component_is_pcm(dlc: *mut snd_soc_dai_link_component) -> bool {
    ...
    simple_util_parse_convert(top,   core::ptr::null_mut(),   adata);
    ...
    core::ptr::null_mut(), &dai_link.dai_fmt);
    if (ret < 0)
    goto end;
...
// BE settings
    dai_link.no_pcm		= 1;
    dai_link.be_hw_params_fixup	= simple_util_be_hw_params_fixup;
    ret = graph_parse_node(priv, codec_ep, li, core::ptr::null_mut());
    if (ret < 0)
    goto end;
    snprintf(dai_name, sizeof(dai_name),
...
//
    ...
    struct device_node *codec_port_old = core::ptr::null_mut();
    struct simple_util_data adata;
    int rc, ret = 0;
// loop for all listed CPU port
    of_for_each_phandle(&it, rc, node, "dais", core::ptr::null_mut(), 0) {
    cpu_port = it.node;
...
#[no_mangle]
pub unsafe extern "C" fn audio_graph_parse_of(priv: *mut simple_util_priv, dev: *mut device) -> c_int {
