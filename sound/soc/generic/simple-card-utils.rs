...
//! Source: sound/soc/generic/simple-card-utils.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn simple_util_is_convert_required(data: *const simple_util_data) -> bool {
...
//
    ...
    daifmt |= snd_soc_daifmt_parse_clock_provider_as_flag(codec, core::ptr::null_mut());
    } else {
...
#[no_mangle]
unsafe extern "C" fn simple_clk_enable(dai: *mut simple_util_dai) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn simple_clk_disable(dai: *mut simple_util_dai) {
...
//
    clk = devm_get_clk_from_child(dev, node, core::ptr::null_mut());
    ...
    clk = devm_get_clk_from_child(dev, dlc.of_node, core::ptr::null_mut());
    if (!IS_ERR(clk))
    simple_dai.sysclk = clk_get_rate(clk);
    }
...
pub unsafe extern "C" fn simple_util_startup(substream: *mut snd_pcm_substream) -> c_int {
    ...
    let mut fixed_rate: c_uint = fixed_sysclk / props.mclk_fs;
    if (fixed_sysclk % props.mclk_fs) {
...
pub unsafe extern "C" fn simple_util_shutdown(substream: *mut snd_pcm_substream) {
    ...
    struct snd_pcm_hw_params *params)
    {
    let mut sample_bits: c_int = params_width(params);
    ...
    struct simple_util_priv *priv = snd_soc_card_get_drvdata(rtd.card);
    struct simple_dai_props *props = runtime_simple_priv_to_props(priv, rtd);
    let mut order: enum simple_util_sysclk_order = props.sysclk_order;
...
#[no_mangle]
pub unsafe extern "C" fn simple_component_is_codec(component: *mut snd_soc_component) -> c_int {
...
//
    if (is_single_links)
    cpus.dai_name = core::ptr::null_mut();
    }
    EXPORT_SYMBOL_GPL(simple_util_canonicalize_cpu);
#[no_mangle]
pub unsafe extern "C" fn simple_util_clean_reference(priv: *mut simple_util_priv) {
...
#[no_mangle]
pub unsafe extern "C" fn simple_util_init_aux_jacks(card: *mut snd_soc_card, prefix: *mut c_char) -> c_int {
    ...
    struct snd_soc_component *component;
    let mut found_jack_index: c_int = 0;
...
// create jack
    ...
    if (ret)
    continue;
    (void)snd_soc_component_set_jack(component, jack, core::ptr::null_mut());
    }
    return 0;
    }
    ...
    struct snd_soc_codec_conf *cconf = core::ptr::null_mut();
...
//
    for (i = 0; i < li.link; i++) {
    let mut cc: c_int = li.num[i].cpus + li.num[i].codecs;
    dai_num += cc;
...
// Doesn't have Platform
    dai_link[i].platforms		= core::ptr::null_mut();
    dai_props[i].num.platforms	=
...
pub unsafe extern "C" fn graph_util_card_probe(card: *mut snd_soc_card) -> c_int {
    ...
    struct simple_util_priv *priv = snd_soc_card_get_drvdata(card);
    int ret;
    ret = simple_util_init_hp(card, &priv.hp_jack, core::ptr::null_mut());
    if (ret < 0)
    goto end;
    ret = simple_util_init_mic(card, &priv.mic_jack, core::ptr::null_mut());
    end:
    return simple_ret(priv, ret);
    }
...
#[no_mangle]
unsafe extern "C" fn graph_get_dai_id(ep: *mut device_node) -> c_int {
...
//
    ...
    let mut resolved_dlc: snd_soc_dai_link_component = {};
    struct snd_soc_dai *dai;
...
// is_single_link = of_graph_get_endpoint_count(node) == 1;
    ...
    {
    let mut is_playback_only: bool = of_property_read_bool(np, "playback-only");
    let mut is_capture_only: bool = of_property_read_bool(np, "capture-only");
