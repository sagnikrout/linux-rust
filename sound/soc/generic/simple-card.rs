...
//! Source: sound/soc/generic/simple-card.c
#![no_std]
...
// is_single_link = !args.args_count;
    ...
    simple_util_parse_convert(node, core::ptr::null_mut(),   adata);
    simple_util_parse_convert(np,   core::ptr::null_mut(),   adata);
    }
...
// BE settings
    ...
    ret = simple_parse_node(priv, np, li, prefix, core::ptr::null_mut());
    if (ret < 0)
    goto out_put_node;
...
// For single DAI link & old style of DT node
    ...
    ret = simple_parse_node(priv, codec, li, prefix, core::ptr::null_mut());
    if (ret < 0)
    goto dai_link_of_err;
    ...
    struct device_node *top = dev.of_node;
    struct device_node *node;
    let mut dpcm_selectable: uintptr_t = (uintptr_t)of_device_get_match_data(dev);
    let mut is_top: bool = 0;
    let mut ret: c_int = 0;
...
// loop for all dai-link
    do {
    struct simple_util_data adata;
    let mut num: c_int = of_get_child_count(node);
...
#[no_mangle]
unsafe extern "C" fn simple_populate_aux(priv: *mut simple_util_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn simple_parse_of(priv: *mut simple_util_priv) -> c_int {
