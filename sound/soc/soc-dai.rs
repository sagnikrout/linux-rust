...
//
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dai_set_bclk_ratio(dai: *mut snd_soc_dai, ratio: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_dai_fmt_match_cnt(fmt: u64) -> c_int {
    ...
    let mut max_idx: c_int = rtd.dai_link.num_cpus + rtd.dai_link.num_codecs;
    u64 available_fmt;
//
// NOTE
...
// In case of last DAI
    if (idx + 1 >= max_idx) {
    let mut cnt1: c_int = soc_dai_fmt_match_cnt(*best_fmt);
    let mut cnt2: c_int = soc_dai_fmt_match_cnt(available_fmt);
    if (cnt1 < cnt2)
...
#[no_mangle]
unsafe extern "C" fn soc_dai_convert_possiblefmt_to_daifmt(possible_fmt: u64, configured_fmt: c_uint) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dai_set_tristate(dai: *mut snd_soc_dai, tristate: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_pcm_dai_probe(rtd: *mut snd_soc_pcm_runtime, order: c_int) -> c_int {
    ...
    let mut ret: c_int = dai.driver.ops.probe(dai);
    if (ret < 0)
    return soc_dai_ret(dai, ret);
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_pcm_dai_remove(rtd: *mut snd_soc_pcm_runtime, order: c_int) -> c_int {
...
pub unsafe extern "C" fn snd_soc_pcm_dai_new(rtd: *mut snd_soc_pcm_runtime) -> c_int {
    ...
    let mut ret: c_int = dai.driver.ops.pcm_new(rtd, dai);
    if (ret < 0)
    return soc_dai_ret(dai, ret);
    }
    }
