...
//! Source: sound/soc/intel/avs/board_selection.c
#![no_std]
...
//
    ...
    let mut obsolete_card_names: bool = IS_ENABLED(CONFIG_SND_SOC_INTEL_AVS_CARDNAME_OBSOLETE);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct avs_acpi_boards {
    pub id: c_int,
    pub machs: *mut snd_soc_acpi_mach,
}
...
unsafe extern "C" fn avs_unregister_board(pdev: *mut c_void) {
    ...
    int ret;
    pdev = platform_device_register_data(core::ptr::null_mut(), name, PLATFORM_DEVID_AUTO, data, size);
    if (IS_ERR(pdev))
    return pdev;
...
#[no_mangle]
unsafe extern "C" fn avs_register_probe_board(adev: *mut avs_dev) -> int __maybe_unused {
    static int __maybe_unused avs_register_probe_board(struct avs_dev *adev)
    {
    struct platform_device *pdev;
    pdev = avs_register_board(adev, "avs_probe_mb", core::ptr::null_mut(), 0);
    if (IS_ERR(pdev))
    return PTR_ERR(pdev);
    return avs_register_probe_component(adev, dev_name(&pdev.dev));
    }
#[no_mangle]
unsafe extern "C" fn avs_register_dmic_board(adev: *mut avs_dev) -> c_int {
...
// DMIC present in Intel PCH is enumerated statically.
    pdev = avs_register_board(adev, "dmic-codec", core::ptr::null_mut(), 0);
    if (IS_ERR(pdev))
    return PTR_ERR(pdev);
    codec_name = devm_kstrdup(adev.dev, dev_name(&pdev.dev), GFP_KERNEL);
    if (!codec_name)
    return -ENOMEM;
    pdev = avs_register_board_pdata(adev, "avs_dmic", &mach, core::ptr::null_mut(), core::ptr::null_mut(), codec_name);
    if (IS_ERR(pdev))
    return PTR_ERR(pdev);
    return avs_register_dmic_component(adev, dev_name(&pdev.dev));
    }
#[no_mangle]
unsafe extern "C" fn avs_register_i2s_test_board(adev: *mut avs_dev, ssp_port: c_int, tdm_slot: c_int) -> c_int {
    static int avs_register_i2s_test_board(struct avs_dev *adev, int ssp_port, int tdm_slot)
    {
    let mut mach: snd_soc_acpi_mach = {{0}};
    struct platform_device *pdev;
    unsigned long *tdms;
    ...
    pdev = avs_register_board_pdata(adev, mach.drv_name, &mach, core::ptr::null_mut(), tdms, core::ptr::null_mut());
    if (IS_ERR(pdev))
    return PTR_ERR(pdev);
    return avs_register_i2s_component(adev, dev_name(&pdev.dev), AVS_SSP(ssp_port), tdms);
    }
#[no_mangle]
unsafe extern "C" fn avs_register_i2s_test_boards(adev: *mut avs_dev) -> c_int {
    static int avs_register_i2s_test_boards(struct avs_dev *adev)
    {
    let mut max_ssps: c_int = adev.hw_cfg.i2s_caps.ctrl_count;
...
#[no_mangle]
unsafe extern "C" fn avs_register_i2s_board(adev: *mut avs_dev, mach: *mut snd_soc_acpi_mach) -> c_int {
    static int avs_register_i2s_board(struct avs_dev *adev, struct snd_soc_acpi_mach *mach)
    {
    let mut i2s_mask: u32 = mach.mach_params.i2s_link_mask;
    struct platform_device *pdev;
    unsigned long *tdms = core::ptr::null_mut();
    if (mach.pdata)
...
#[no_mangle]
unsafe extern "C" fn avs_register_i2s_boards(adev: *mut avs_dev) -> c_int {
    static int avs_register_i2s_boards(struct avs_dev *adev)
    {
    let mut num_ssps: c_int = adev.hw_cfg.i2s_caps.ctrl_count;
...
#[no_mangle]
unsafe extern "C" fn avs_register_hda_board(adev: *mut avs_dev, codec: *mut hda_codec) -> c_int {
    ...
    return -ENOMEM;
    pdev = avs_register_board_pdata(adev, "avs_hdaudio", &mach, codec, core::ptr::null_mut(), core::ptr::null_mut());
    if (IS_ERR(pdev))
    return PTR_ERR(pdev);
    return avs_register_hda_component(adev, dev_name(&pdev.dev));
    }
#[no_mangle]
unsafe extern "C" fn avs_register_hda_boards(adev: *mut avs_dev) -> c_int {
