...
//! Source: sound/soc/intel/catpt/ipc.c
#![no_std]
...
//
    ...
    {
    let mut header: u32 = tx.header | CATPT_IPCC_BUSY;
...
unsafe extern "C" fn catpt_dsp_process_response(cdev: *mut catpt_dev, header: u32) {
    ...
    {
    let mut msg: union catpt_notify_msg = CATPT_MSG(header);
    struct catpt_ipc *ipc = &cdev.ipc;
    if (msg.fw_ready) {
    struct catpt_fw_ready config;
// to fit 32b header original address is shifted right by 3
    let mut off: u32 = msg.mailbox_address << 3;
