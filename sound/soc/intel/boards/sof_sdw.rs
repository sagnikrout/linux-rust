...
//! Source: sound/soc/intel/boards/sof_sdw.c
#![no_std]
...
pub const DMIC_DEFAULT_CHANNELS: c_int = 2;
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn sof_sdw_check_ssid_quirk(mach: *const snd_soc_acpi_mach) {
...
// create stream name according to first link id
    ...
    let mut pin_num: c_int = intel_ctx.sdw_pin_index[link_num]++;
...
// be_id = current_be_id;
    ...
    return -ENOMEM;
    let mut playback: c_int = ssp_info.dais[0].direction[SNDRV_PCM_STREAM_PLAYBACK];
    let mut capture: c_int = ssp_info.dais[0].direction[SNDRV_PCM_STREAM_CAPTURE];
    ret = asoc_sdw_init_simple_dai_link(dev, *dai_links, be_id, name,
    playback, capture, cpu_dai_name,
    "dummy", codec_name,
    ssp_info.dais[0].dai_name, 1, core::ptr::null_mut(),
    ssp_info.ops);
    if (ret)
    return ret;
...
// don't call asoc_sdw_dmic_init() twice
    ...
    i == 0 ? sof_sdw_hdmi_init : core::ptr::null_mut(), core::ptr::null_mut());
    if (ret)
    return ret;
    (*dai_links)++;
    }
    ...
    1, core::ptr::null_mut(), core::ptr::null_mut());
    if (ret)
    return ret;
    (*dai_links)++;
    return 0;
...
unsafe extern "C" fn sof_card_dai_links_create(card: *mut snd_soc_card) -> c_int {
    ...
    struct device *dev = card.dev;
    struct snd_soc_acpi_mach *mach = dev_get_platdata(card.dev);
    let mut sdw_be_num: c_int = 0, ssp_num = 0, dmic_num = 0, bt_num = 0;
    struct asoc_sdw_mc_private *ctx = snd_soc_card_get_drvdata(card);
    struct intel_mc_ctx *intel_ctx = (struct intel_mc_ctx *)ctx.private;
    struct snd_soc_acpi_mach_params *mach_params = &mach.mach_params;
    struct snd_soc_codec_conf *codec_conf;
