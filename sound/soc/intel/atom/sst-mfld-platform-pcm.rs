...
//! Source: sound/soc/intel/atom/sst-mfld-platform-pcm.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn sst_get_stream_status(stream: *mut sst_runtime_stream) -> c_int {
    ...
    ssize_t periodbytes;
    let mut buffer_bytes: isize = snd_pcm_lib_buffer_bytes(substream);
    let mut buffer_addr: u32 = substream.runtime.dma_addr;
...
// index 0 is not used in stream map
    ...
    struct snd_pcm_substream *pstream = core::ptr::null_mut();
    struct snd_compr_stream *cstream = core::ptr::null_mut();
...
// For pcm streams
    ...
    let mut param: snd_sst_stream_params = {{{0,},},};
    let mut str_params: snd_sst_params = {0};
    let mut alloc_params: snd_sst_alloc_params_ext = {0};
...
#[no_mangle]
unsafe extern "C" fn sst_platform_init_stream(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn power_up_sst(stream: *mut sst_runtime_stream) -> c_int {
    static int power_up_sst(struct sst_runtime_stream *stream)
    {
    return stream.ops.power(sst.dev, true);
    }
#[no_mangle]
unsafe extern "C" fn power_down_sst(stream: *mut sst_runtime_stream) {
    ...
    struct sst_runtime_stream *stream __free(kfree) = core::ptr::null_mut();
...
// BE CPU  Dais
    ...
    {
    let mut ret_val: c_int = 0, str_id;
    struct sst_runtime_stream *stream;
    int status;
    struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
...
unsafe extern "C" fn sst_soc_remove(component: *mut snd_soc_component) {
    ...
    {
    struct sst_data *drv = dev_get_drvdata(component.dev);
    drv.soc_card = core::ptr::null_mut();
    }
