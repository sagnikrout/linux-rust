...
//! Source: sound/soc/au1x/dbdma2.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct au1xpsc_audio_dmadata {
// DDMA control data
    pub /: *mut *mut unsigned int ddma_id; / DDMA direction ID for this PSC,
    pub /: *mut *mut u32 ddma_chan; / DDMA context,
// PCM context (for irq handlers)
    pub substream: *mut snd_pcm_substream,
    pub /: *mut *mut unsigned long curr_period; / current segment DDMA is working on,
    pub /: *mut *mut unsigned long q_period; / queue period(s),
    pub /: *mut *mut dma_addr_t dma_area; / address of queued DMA area,
    pub /: *mut *mut dma_addr_t dma_area_s; / start address of DMA area,
    pub /: *mut *mut unsigned long pos; / current byte position being played,
    pub /: *mut *mut unsigned long periods; / number of SG segments in total,
    pub /: *mut *mut unsigned long period_bytes; / size in bytes of one SG segment,
// runtime data
    pub msbits: c_int,
}
...
//
pub const AU1XPSC_PERIOD_MIN_BYTES: c_int = 1024;
pub const AU1XPSC_BUFFER_MIN_BYTES: c_int = 65536;
...
#[no_mangle]
unsafe extern "C" fn au1x_pcm_queue_tx(cd: *mut au1xpsc_audio_dmadata) {
...
#[no_mangle]
unsafe extern "C" fn au1x_pcm_queue_rx(cd: *mut au1xpsc_audio_dmadata) {
...
#[no_mangle]
unsafe extern "C" fn au1x_pcm_dmatx_cb(irq: c_int, dev_id: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn au1x_pcm_dmarx_cb(irq: c_int, dev_id: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn au1x_pcm_dbdma_free(pcd: *mut au1xpsc_audio_dmadata) {
...
// check current config: correct bits and descriptors allocated?
    ...
    {
    let mut c: u32 = to_dmadata(substream, component).ddma_chan;
    switch (cmd) {
    ...
    let mut stype: c_int = substream.stream, *dmaids;
...
#[no_mangle]
unsafe extern "C" fn au1xpsc_pcm_drvprobe(pdev: *mut platform_device) -> c_int {
    ...
    &au1xpsc_soc_component, core::ptr::null_mut(), 0);
    }
