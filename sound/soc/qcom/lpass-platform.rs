...
//! Source: sound/soc/qcom/lpass-platform.c
#![no_std]
...
pub const LPASS_PLATFORM_PERIODS: c_int = 2;

pub const LPASS_CDC_DMA_REGISTER_FIELDS_MAX: c_int = 15;
    ...
    GFP_KERNEL);
    if (drvdata.rd_dmactl == core::ptr::null_mut())
    return -ENOMEM;
    drvdata.wr_dmactl = devm_kzalloc(dev, sizeof(struct lpaif_dmactl),
    GFP_KERNEL);
    if (drvdata.wr_dmactl == core::ptr::null_mut())
    return -ENOMEM;
    ...
    if (rd_dmactl == core::ptr::null_mut())
    return -ENOMEM;
    ...
    struct regmap *map;
    let mut dai_id: c_uint = cpu_dai.driver.id;
    ...
    switch (dai_id) {
    case MI2S_PRIMARY ... MI2S_QUINARY:
    drvdata.substream[data.dma_ch] = core::ptr::null_mut();
    break;
    case LPASS_DP_RX:
    drvdata.hdmi_substream[data.dma_ch] = core::ptr::null_mut();
    break;
    case LPASS_CDC_DMA_RX0 ... LPASS_CDC_DMA_RX9:
    case LPASS_CDC_DMA_TX0 ... LPASS_CDC_DMA_TX8:
    drvdata.rxtx_substream[data.dma_ch] = core::ptr::null_mut();
    break;
    case LPASS_CDC_DMA_VA_TX0 ... LPASS_CDC_DMA_VA_TX8:
    drvdata.va_substream[data.dma_ch] = core::ptr::null_mut();
    break;
    default:
    break;
    }
    ...
    struct snd_soc_dai *cpu_dai = snd_soc_rtd_to_cpu(soc_runtime, 0);
    struct lpass_data *drvdata = snd_soc_component_get_drvdata(component);
    struct lpaif_dmactl *dmactl = core::ptr::null_mut();
    ...
    struct snd_soc_dai *cpu_dai = snd_soc_rtd_to_cpu(soc_runtime, 0);
    struct lpass_data *drvdata = snd_soc_component_get_drvdata(component);
    struct regmap *map = core::ptr::null_mut();
    switch (cpu_dai.driver.id) {
    case MI2S_PRIMARY ... MI2S_QUINARY:
    map = drvdata.lpaif_map;
    break;
    ...
    struct regmap *map;
    int ret, ch, id;
    let mut reg_irqclr: c_uint = 0, val_irqclr = 0;
    let mut reg_irqen: c_uint = 0, val_irqen = 0, val_mask = 0;
    let mut dai_id: c_uint = cpu_dai.driver.id;
    ch = pcm_data.dma_ch;
    dmactl = __lpass_get_dmactl_handle(substream, component);
    id = __lpass_get_id(substream, component);
...
#[no_mangle]
unsafe extern "C" fn lpass_platform_lpaif_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn lpass_platform_hdmiif_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn lpass_platform_rxtxif_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn lpass_platform_vaif_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
// Assign Codec DMA buffer pointers
    ...
    buf.area = (unsigned char * )memremap(buf.addr, buf.bytes, MEMREMAP_WC);
    return 0;
    }
    ...
    struct snd_soc_dai *cpu_dai = snd_soc_rtd_to_cpu(soc_runtime, 0);
    let mut dai_id: c_uint = cpu_dai.driver.id;
    let mut size: usize = lpass_platform_pcm_hardware.buffer_bytes_max;
//
...
unsafe extern "C" fn lpass_platform_pcmops_resume(component: *mut snd_soc_component) -> c_int {
    ...
    if (copy_from_iter((void  *)dma_buf, bytes, buf) != bytes)
    ret = -EFAULT;
    }
    ...
    if (copy_to_iter((void  *)dma_buf, bytes, buf) != bytes)
    ret = -EFAULT;
    }
    }
    return ret;
...
// ensure audio hardware is disabled
    ...
    &lpass_component_driver, core::ptr::null_mut(), 0);
    }
