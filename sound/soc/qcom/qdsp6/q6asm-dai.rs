...
//! Source: sound/soc/qcom/qdsp6/q6asm-dai.c
#![no_std]
...
pub const CAPTURE_MAX_PERIOD_SIZE: c_int = 4096;
pub const CAPTURE_MIN_PERIOD_SIZE: c_int = 320;
...
pub struct q6asm_dai_rtd {
    ...
    pub /: *mut *mut uint16_t source; / Encoding source bit mask,
    pub audio_client: *mut audio_client,
    pub next_track_stream_id: u32,
    pub next_track: bool,
    pub stream_id: u32,
    pub session_id: u16,
    pub state: enum stream_state,
    pub initial_samples_drop: u32,
    pub trailing_samples_drop: u32,
    pub notify_on_drain: bool,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct q6asm_dai_data {
    pub dais: *mut snd_soc_dai_driver,
    pub num_dais: c_int,
    pub sid: long long int,
}
...
// clear the previous setup if any
    ...
    runtime.rate, runtime.channels, core::ptr::null_mut(),
    prtd.bits_per_sample);
    } else if (substream.stream == SNDRV_PCM_STREAM_CAPTURE) {
...
#[no_mangle]
unsafe extern "C" fn q6asm_dai_ack(component: *mut snd_soc_component, substream: *mut snd_pcm_substream) -> c_int {
...
// Ensure that buffer size is a multiple of period size
    ...
    prtd.audio_client = core::ptr::null_mut();
    }
    q6routing_stream_close(soc_prtd.dai_link.id,
    substream.stream);
    kfree(prtd);
...
#[no_mangle]
pub unsafe extern "C" fn if(2: wma_cfg.num_channels ==) -> else {
...
// Kick off the data to dsp if its starving!!
    ...
    let mut bytes_to_write: u32 = prtd.pcm_count;
    avail = prtd.bytes_received - prtd.bytes_sent;
    if (avail < prtd.pcm_count)
    bytes_to_write = avail;
    q6asm_write_async(prtd.audio_client, prtd.stream_id,
...
// codec = q6asm_compr_caps;
    ...
    let mut size: usize = q6asm_dai_hardware_playback.buffer_bytes_max;
...
#[no_mangle]
pub unsafe extern "C" fn if(Q6ASM_DAI_TX: dir ==) -> else {
