...
//! Source: sound/soc/qcom/qdsp6/topology.c
#![no_std]
...
pub struct snd_ar_control {
    pub /: *mut *mut u32 graph_id; / Graph ID,
    pub /: *mut *mut u32 sgid; / Sub Graph ID,
    pub /: *mut *mut u32 module_instance_id; / Connected Module Instance ID,
...
// add to module list
    ...
    {
    const struct snd_soc_tplg_vendor_array *sg_array = core::ptr::null_mut();
    ...
    {
    const struct snd_soc_tplg_vendor_array *cont_array = core::ptr::null_mut();
    let mut found: bool = false;
    int sz;
    for (sz = 0; !found && (sz < le32_to_cpu(private.size)); ) {
    ...
    {
    const struct snd_soc_tplg_vendor_array *mod_array = core::ptr::null_mut();
    let mut found: bool = false;
    let mut sz: c_int = 0;
    for (sz = 0; !found && (sz < le32_to_cpu(private.size)); ) {
    ...
    struct audioreach_graph_info *info = core::ptr::null_mut();
...
// Sub graph is associated with predefined graph
    ...
    struct snd_soc_dapm_widget *w)
    {
    let mut max_ip_port: u32 = 0, max_op_port = 0;
    ...
    let mut src_mod_inst_id: u32 = 0;
    let mut module_id: c_int = 0, instance_id = 0, tkn_count = 0;
    const struct snd_soc_tplg_vendor_value_elem *mod_elem;
    const struct snd_soc_tplg_vendor_array *mod_array;
    struct audioreach_module *mod = core::ptr::null_mut();
    uint32_t token;
    bool found;
    int max_tokens;
...
#[no_mangle]
unsafe extern "C" fn audioreach_tplg_complete(component: *mut snd_soc_component) -> c_int {
...
// DAI link - used for any driver specific init
    ...
    link.nonatomic = true;
    link.dynamic = true;
    link.platforms.name = core::ptr::null_mut();
    ...
    snd_soc_dapm_mixer_update_power(dapm, kcontrol, 1, core::ptr::null_mut());
    } else {
    audioreach_connect_sub_graphs(data, scontrol, dapm_scontrol, false);
    snd_soc_dapm_mixer_update_power(dapm, kcontrol, 0, core::ptr::null_mut());
    }
    return 0;
    }
