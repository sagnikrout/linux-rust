...
//
    ...
    bool sx)
    {
    let mut val: c_int = (reg_val >> shift) & mask;
    if (mc.sign_bit)
...
#[no_mangle]
unsafe extern "C" fn soc_mixer_valid_ctl(mc: *mut soc_mixer_control, val: c_long, max: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_mixer_mask(mc: *mut soc_mixer_control) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_mixer_sx_mask(mc: *mut soc_mixer_control) -> c_int {
...
//
    ...
    struct soc_mixer_control *mc =
    (struct soc_mixer_control *)kcontrol.private_value;
    let mut mask: c_uint = soc_mixer_mask(mc);
...
//
    ...
    struct soc_mixer_control *mc =
    (struct soc_mixer_control *)kcontrol.private_value;
    let mut mask: c_uint = soc_mixer_sx_mask(mc);
...
#[no_mangle]
unsafe extern "C" fn snd_soc_clip_to_platform_max(kctl: *mut snd_kcontrol) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_soc_limit_volume(card: *mut snd_soc_card, name: *const c_char, max: c_int) -> c_int {
...
// Some cards blindly add limits for multiple variants.
    ...
    let mut val_bytes: c_int = snd_soc_component_regmap_val_bytes(component);
...
//
    ...
    {
    struct soc_bytes_ext *params = (void *)kcontrol.private_value;
    let mut count: c_uint = size < params.max ? size : params.max;
...
//
    ...
    (struct soc_mreg_control *)kcontrol.private_value;
    let mut val_bytes: c_int = snd_soc_component_regmap_val_bytes(component);
    let mut regbase: c_uint = mc.regbase;
    let mut regcount: c_uint = mc.regcount;
    let mut regwshift: c_uint = val_bytes * BITS_PER_BYTE;
    let mut regwmask: c_uint = GENMASK(regwshift - 1, 0);
    let mut mask: c_ulong = GENMASK(mc.nbits - 1, 0);
    let mut val: c_long = 0;
    unsigned int i;
    for (i = 0; i < regcount; i++) {
    let mut regval: c_uint = snd_soc_component_read(component, regbase + i);
...
//
    ...
    struct soc_mixer_control *mc =
    (struct soc_mixer_control *)kcontrol.private_value;
    let mut invert: c_uint = mc.invert != 0;
    let mut mask: c_uint = BIT(mc.shift);
    unsigned int val;
...
//
    ...
    struct soc_mixer_control *mc =
    (struct soc_mixer_control *)kcontrol.private_value;
    let mut strobe: c_uint = ucontrol.value.enumerated.item[0] != 0;
    let mut invert: c_uint = mc.invert != 0;
    let mut mask: c_uint = BIT(mc.shift);
    let mut val1: c_uint = (strobe ^ invert) ? mask : 0;
    let mut val2: c_uint = (strobe ^ invert) ? 0 : mask;
    int ret;
