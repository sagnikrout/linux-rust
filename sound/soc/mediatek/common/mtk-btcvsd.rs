...
//! Source: sound/soc/mediatek/common/mtk-btcvsd.c
#![no_std]
...
pub const SCO_CVSD_PACKET_VALID_SIZE: c_int = 2;
pub const SCO_PACKET_120: c_int = 120;
pub const SCO_PACKET_180: c_int = 180;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_btcvsd_snd_hw_info {
    pub num_valid_addr: c_uint,
    pub bt_sram_addr: [c_ulong; 20],
    pub packet_length: c_uint,
    pub packet_num: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_btcvsd_snd_stream {
    pub substream: *mut snd_pcm_substream,
    pub stream: c_int,
    pub state: enum bt_sco_state,
    pub packet_size: c_uint,
    pub buf_size: c_uint,
    pub temp_packet_buf: [u8; SCO_PACKET_180],
    pub packet_w: c_int,
    pub packet_r: c_int,
    pub prev_frame: snd_pcm_uframes_t,
    pub prev_packet_idx: c_int,
    pub xrun:1: c_uint,
    pub timeout:1: c_uint,
    pub mute:1: c_uint,
    pub trigger_start:1: c_uint,
    pub wait_flag:1: c_uint,
    pub rw_cnt: c_uint,
    pub time_stamp: c_ulonglong,
    pub buf_data_equivalent_time: c_ulonglong,
    pub buffer_info: mtk_btcvsd_snd_hw_info,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_btcvsd_snd {
    pub dev: *mut device,
    pub irq_id: c_int,
    pub infra: *mut regmap,
    pub bt_pkv_base: *mut void __iomem,
    pub bt_sram_bank2_base: *mut void __iomem,
    pub infra_misc_offset: c_uint,
    pub conn_bt_cvsd_mask: c_uint,
    pub cvsd_mcu_read_offset: c_uint,
    pub cvsd_mcu_write_offset: c_uint,
    pub cvsd_packet_indicator: c_uint,
    pub bt_reg_pkt_r: *mut u32,
    pub bt_reg_pkt_w: *mut u32,
    pub bt_reg_ctl: *mut u32,
    pub irq_disabled:1: c_uint,
    pub /: *mut *mut spinlock_t tx_lock; / spinlock for bt tx stream control,
    pub /: *mut *mut spinlock_t rx_lock; / spinlock for bt rx stream control,
    pub tx_wait: wait_queue_head_t,
    pub rx_wait: wait_queue_head_t,
    pub tx: *mut mtk_btcvsd_snd_stream,
    pub rx: *mut mtk_btcvsd_snd_stream,
    pub tx_packet_buf: [u8; BTCVSD_TX_BUF_SIZE],
    pub rx_packet_buf: [u8; BTCVSD_RX_BUF_SIZE],
    pub band: enum BT_SCO_BAND,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_btcvsd_snd_time_buffer_info {
    pub data_count_equi_time: c_ulonglong,
    pub time_stamp_us: c_ulonglong,
}
...
#[no_mangle]
unsafe extern "C" fn mtk_btcvsd_snd_irq_enable(bt: *mut mtk_btcvsd_snd) {
...
#[no_mangle]
unsafe extern "C" fn mtk_btcvsd_snd_irq_disable(bt: *mut mtk_btcvsd_snd) {
...
#[no_mangle]
unsafe extern "C" fn mtk_btcvsd_snd_tx_init(bt: *mut mtk_btcvsd_snd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mtk_btcvsd_snd_rx_init(bt: *mut mtk_btcvsd_snd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btcvsd_tx_clean_buffer(bt: *mut mtk_btcvsd_snd) -> c_int {
    static int btcvsd_tx_clean_buffer(struct mtk_btcvsd_snd *bt)
    {
    unsigned int i;
    unsigned int num_valid_addr;
    let mut band: enum BT_SCO_BAND = bt.band;
...
// bt return 0xdeadfeed if read register during bt sleep
    ...
    let mut new_ap_addr_tx: bool = true;
...
#[no_mangle]
unsafe extern "C" fn mtk_btcvsd_snd_irq_handler(irq_id: c_int, dev: *mut c_void) -> irqreturn_t {
...
// one interrupt period = 22.5ms
    let mut timeout_limit: c_ulonglong = 22500000;
...
// conidtion is false after timeout
    ...
    size_t count)
    {
    let mut read_size: isize = 0, read_count = 0, cur_read_idx, cont;
    unsigned long avail;
    let mut packet_size: c_uint = bt.rx.packet_size;
    while (count) {
...
// available data in RX packet buffer
    ...
    if (!avail) {
    let mut ret: c_int = wait_for_bt_irq(bt, bt.rx);
    if (ret)
    return read_count;
    continue;
    }
...
// return equivalent time(us) to data count
    ...
    size_t count)
    {
    let mut written_size: c_int = count, avail, cur_write_idx, write_size, cont;
    let mut packet_size: c_uint = bt.tx.packet_size;
//
...
// free space of TX packet buffer
    ...
    if (!avail) {
    let mut ret: c_int = wait_for_bt_irq(bt, bt.tx);
    if (ret)
    return written_size;
    continue;
    }
...
// pcm ops
    ...
    bt_stream.substream = core::ptr::null_mut();
    return 0;
    }
    ...
    struct mtk_btcvsd_snd *bt = snd_soc_component_get_drvdata(component);
    struct mtk_btcvsd_snd_stream *bt_stream;
    let mut frame: snd_pcm_uframes_t = 0;
...
// kcontrol
    ...
    struct snd_soc_component *cmpnt = snd_kcontrol_chip(kcontrol);
    struct mtk_btcvsd_snd *bt = snd_soc_component_get_drvdata(cmpnt);
    let mut lpbk_en: bool = bt.tx.state == BT_SCO_STATE_LOOPBACK;
    ...
    btcvsd_tx_irq_received_get, core::ptr::null_mut()),
    SOC_SINGLE_BOOL_EXT("BTCVSD Tx Timeout Switch", 0,
    btcvsd_tx_timeout_get, core::ptr::null_mut()),
    SOC_SINGLE_BOOL_EXT("BTCVSD Rx Irq Received Switch", 0,
    btcvsd_rx_irq_received_get, core::ptr::null_mut()),
    SOC_SINGLE_BOOL_EXT("BTCVSD Rx Timeout Switch", 0,
    btcvsd_rx_timeout_get, core::ptr::null_mut()),
    SND_SOC_BYTES_TLV("BTCVSD Rx Timestamp",
    sizeof(struct mtk_btcvsd_snd_time_buffer_info),
    btcvsd_rx_timestamp_get, core::ptr::null_mut()),
    SND_SOC_BYTES_TLV("BTCVSD Tx Timestamp",
    sizeof(struct mtk_btcvsd_snd_time_buffer_info),
    btcvsd_tx_timestamp_get, core::ptr::null_mut()),
    };
#[no_mangle]
