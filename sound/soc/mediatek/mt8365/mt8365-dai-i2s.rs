...
//! Source: sound/soc/mediatek/mt8365/mt8365-dai-i2s.c
#![no_std]
...
pub const IIR_RATIOVER: c_int = 9;
pub const IIR_INV_COEF: c_int = 10;
pub const IIR_NO_NEED: c_int = 11;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_afe_i2s_priv {
    pub adda_link: bool,
    pub i2s_out_on_ref_cnt: c_int,
    pub id: c_int,
    pub low_jitter_en: c_int,
    pub mclk_id: c_int,
    pub share_i2s_id: c_int,
    pub clk_id_in: c_uint,
    pub clk_id_in_m_sel: c_uint,
    pub clk_id_out: c_uint,
    pub clk_id_out_m_sel: c_uint,
    pub clk_in_mult: c_uint,
    pub clk_out_mult: c_uint,
    pub config_val_in: c_uint,
    pub config_val_out: c_uint,
    pub dynamic_bck: c_uint,
    pub reg_off_in: c_uint,
    pub reg_off_out: c_uint,
}
...
// 7: 0.1667
    ...
    const u32 *coef = core::ptr::null_mut();
    let mut cnt: c_uint = 0;
    let mut i: u32 = freq_new_index[input_fs];
    let mut j: u32 = freq_new_index[output_fs];
    if (i < 13 && j < 13) {
    let mut k: u32 = iir_coef_tbl_matrix[i][j];
    if (k >= IIR_NO_NEED) {
...
// count = cnt;
    ...
    let mut fs: c_int = mt8365_afe_fs_timing(rate);
    if (fs < 0)
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn mt8365_afe_set_i2s_out_enable(afe: *mut mtk_base_afe, enable: bool) {
    void mt8365_afe_set_i2s_out_enable(struct mtk_base_afe *afe, bool enable)
    {
    int i;
    struct mt8365_afe_private *afe_priv = afe.platform_priv;
    struct mtk_afe_i2s_priv *i2s_data = core::ptr::null_mut();
    ...
    0x1, enable);
#[no_mangle]
pub unsafe extern "C" fn if(0: i2s_data->i2s_out_on_ref_cnt <) -> else {
    ...
    struct mtk_afe_i2s_priv *i2s_data = afe_priv.dai_priv[dai.id];
    struct mt8365_be_dai_data *be = &afe_priv.be_data[dai.id - MT8365_AFE_BACKEND_BASE];
    let mut reset_i2s_out_change: bool = (substream.stream == SNDRV_PCM_STREAM_PLAYBACK);
    let mut reset_i2s_in_change: bool = (substream.stream == SNDRV_PCM_STREAM_CAPTURE);
    bool i2s_in_slave =
    (substream.stream == SNDRV_PCM_STREAM_CAPTURE) &&
    ((be.fmt_mode & SND_SOC_DAIFMT_MASTER_MASK) ==
    SND_SOC_DAIFMT_CBP_CFP);
    ...
    struct mtk_afe_i2s_priv *i2s_data = afe_priv.dai_priv[dai.id];
    struct mt8365_be_dai_data *be = &afe_priv.be_data[dai.id - MT8365_AFE_BACKEND_BASE];
    let mut apply_i2s_out_change: bool = (substream.stream == SNDRV_PCM_STREAM_PLAYBACK);
    let mut apply_i2s_in_change: bool = (substream.stream == SNDRV_PCM_STREAM_CAPTURE);
    let mut rate: c_uint = substream.runtime.rate;
    let mut bit_width: c_int = snd_pcm_format_width(substream.runtime.format);
    int ret;
    if (be.prepared[substream.stream]) {
...
#[no_mangle]
unsafe extern "C" fn mt8365_afe_2nd_i2s_set_fmt(dai: *mut snd_soc_dai, fmt: c_uint) -> c_int {
    ...
    {"I2S Playback", core::ptr::null_mut(), "I2S O03_O04"},
    {"2ND I2S Playback", core::ptr::null_mut(), "O00"},
    {"2ND I2S Playback", core::ptr::null_mut(), "O01"},
    {"2ND I2S Capture", core::ptr::null_mut(), "2ND I2S In"},
    {"FM 2ND I2S Mux", "FM_2ND_I2S_IN", "2ND I2S Capture"},
    };
#[no_mangle]
unsafe extern "C" fn mt8365_dai_i2s_set_priv(afe: *mut mtk_base_afe) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mt8365_dai_i2s_register(afe: *mut mtk_base_afe) -> c_int {
