...
//! Source: sound/soc/mediatek/mt8189/mt8189-nau8825.c
#![no_std]
...
// Configure pll for codec
    ...
    struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
    let mut rate: c_uint = params_rate(params);
    let mut mclk_fs: c_uint = rate * 128;
    struct snd_soc_dai *cpu_dai = snd_soc_rtd_to_cpu(rtd, 0);
    struct snd_soc_dai *codec_dai;
    let mut clk_freq: c_int = rate * 32;
    int rx_slot[] = {0, 1};
    int i, ret;
    ...
    ret = snd_soc_dai_set_channel_map(codec_dai, 0, core::ptr::null_mut(),
    1, &rx_slot[i]);
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn mt8189_nau8825_soc_card_probe(soc_card_data: *mut mtk_soc_card_data, legacy: bool) -> c_int {
    ...
    let mut init_rt5682i: bool = false;
