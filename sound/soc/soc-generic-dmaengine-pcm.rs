...
//! Source: sound/soc/soc-generic-dmaengine-pcm.c
#![no_std]
...
// Author: Lars-Peter Clausen <lars@metafoo.de>
    ...
    let mut prealloc_buffer_size_kbytes: static unsigned int = 512;
...
//
    ...
    return -EFAULT;
    if (process) {
    let mut ret: c_int = process(substream, channel, hwoff, bytes);
    if (ret < 0)
    return ret;
    }
    if (!is_playback)
...
//
    ...
    ret = snd_soc_register_component(component, driver, core::ptr::null_mut(), 0);
    if (ret)
    goto err_free_dma;
    return 0;
    err_free_dma:
