...
//! Source: sound/soc/sti/sti_uniperif.c
#![no_std]
...
//
pub const UNIPERIF_MAX_FRAME_SZ: c_uint = 0x20;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sti_uniperiph_dev_data {
    pub /: *mut *mut unsigned int id; / Nb available player instances,
    pub /: *mut *mut unsigned int version; / player IP version,
    pub stream: c_uint,
    pub dai_names: *const c_char,
    pub type: enum uniperif_type,
}
...
// clear remaining indexes
    ...
    {
    let mut slot_width: c_int = uni.tdm_slot.slot_width / 8;
    let mut slots_num: c_int = uni.tdm_slot.slots;
    let mut slots_mask: c_uint = uni.tdm_slot.mask;
    int i, j, k;
...
#[no_mangle]
pub unsafe extern "C" fn sti_uniperiph_dai_set_fmt(dai: *mut snd_soc_dai, fmt: c_uint) -> c_int {
...
// DMA settings
    if (priv.dai_data.stream == SNDRV_PCM_STREAM_PLAYBACK)
    snd_soc_dai_init_dma_data(dai, &dai_data.dma_data, core::ptr::null_mut());
    else
    snd_soc_dai_init_dma_data(dai, core::ptr::null_mut(), &dai_data.dma_data);
...
// Allocate the private data and the CPU_DAI array
    ...
    if (ret < 0)
    return ret;
    return devm_snd_dmaengine_pcm_register(&pdev.dev, core::ptr::null_mut(), 0);
    }
