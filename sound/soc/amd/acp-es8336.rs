...
//! Source: sound/soc/amd/acp-es8336.c
#![no_std]
...
unsafe extern "C" fn st_es8336_init(rtd: *mut snd_soc_pcm_runtime) -> c_int {
    ...
    ret = snd_soc_component_set_jack(codec, &st_jack, core::ptr::null_mut());
    if (ret) {
...
unsafe extern "C" fn st_es8336_codec_startup(substream: *mut snd_pcm_substream) -> c_int {
    ...
    let mut pa_enable_gpio: static struct acpi_gpio_params = { 0, 0, false };
...
unsafe extern "C" fn st_es8336_late_probe(card: *mut snd_soc_card) -> c_int {
    ...
    int ret;
    adev = acpi_dev_get_first_match_dev("ESSX8336", core::ptr::null_mut(), -1);
    if (!adev)
    return -ENODEV;
