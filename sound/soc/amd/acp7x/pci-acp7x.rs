...
//! Source: sound/soc/amd/acp7x/pci-acp7x.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn snd_acp_suspend(dev: *mut device) -> int __maybe_unused {
    static int __maybe_unused snd_acp_suspend(struct device *dev)
    {
    return acp_hw_suspend(dev);
    }
#[no_mangle]
unsafe extern "C" fn snd_acp_runtime_resume(dev: *mut device) -> int __maybe_unused {
    static int __maybe_unused snd_acp_runtime_resume(struct device *dev)
    {
    return acp_hw_runtime_resume(dev);
    }
#[no_mangle]
unsafe extern "C" fn snd_acp_resume(dev: *mut device) -> int __maybe_unused {
    ...
    SET_RUNTIME_PM_OPS(snd_acp_suspend, snd_acp_runtime_resume, core::ptr::null_mut())
