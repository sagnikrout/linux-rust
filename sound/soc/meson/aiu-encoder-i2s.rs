...
//! Source: sound/soc/meson/aiu-encoder-i2s.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn aiu_encoder_is_bs_quirk(channels: c_uint, width: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(16: snd_interval_single(phys_width) && phys_width->min ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(8: snd_interval_single(ch) && ch->min ==) -> else {
...
unsafe extern "C" fn aiu_encoder_i2s_remove_dai(dai: *mut snd_soc_dai) -> c_int {
    ...
    if (ts)
    gx_stream_free(ts);
    snd_soc_dai_dma_data_set(dai, stream, core::ptr::null_mut());
    }
    return 0;
    }
#[no_mangle]
