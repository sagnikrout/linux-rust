...
//! Source: sound/soc/meson/meson-card-utils.c
#![no_std]
...
// meson_card_clean_references() will free the links on this error path
    ...
    if (!dlc || !node)
    return -EINVAL;
    ret = snd_soc_of_get_dlc(node, core::ptr::null_mut(), dlc, 0);
    if (ret)
    ...
    unsigned int daifmt;
    daifmt = snd_soc_daifmt_parse_format(node, core::ptr::null_mut());
    snd_soc_daifmt_parse_clock_provider_as_phandle(node, core::ptr::null_mut(), &bitclkmaster, &framemaster);
...
#[no_mangle]
unsafe extern "C" fn meson_card_clean_references(priv: *mut meson_card) {
