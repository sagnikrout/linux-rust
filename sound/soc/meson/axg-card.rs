...
//! Source: sound/soc/meson/axg-card.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct axg_dai_link_tdm_mask {
    pub tx: u32,
    pub rx: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct axg_dai_link_tdm_data {
    pub mclk_fs: c_uint,
    pub slots: c_uint,
    pub slot_width: c_uint,
    pub tx_mask: *mut u32,
    pub rx_mask: *mut u32,
    pub codec_masks: *mut axg_dai_link_tdm_mask,
}
...
// The loopback rx_mask is the pad tx_mask
    ret = axg_tdm_set_tdm_slots(snd_soc_rtd_to_cpu(rtd, 0), core::ptr::null_mut(), be.tx_mask,
    be.slots, be.slot_width);
    if (ret) {
    dev_err(snd_soc_rtd_to_cpu(rtd, 0).dev, "setting tdm link slots failed\n");
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: axg_card_cpu_is_capture_fe(dai_link->cpus->of_node)) -> else {
