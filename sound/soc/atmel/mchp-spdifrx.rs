...
//! Source: sound/soc/atmel/mchp-spdifrx.c
#![no_std]
...
//
pub const SPDIFRX_CR: c_uint = 0x00	/* Control Register */;
pub const SPDIFRX_MR: c_uint = 0x04	/* Mode Register */;
pub const SPDIFRX_IER: c_uint = 0x10	/* Interrupt Enable Register */;
pub const SPDIFRX_IDR: c_uint = 0x14	/* Interrupt Disable Register */;
pub const SPDIFRX_IMR: c_uint = 0x18	/* Interrupt Mask Register */;
pub const SPDIFRX_ISR: c_uint = 0x1c	/* Interrupt Status Register */;
pub const SPDIFRX_RSR: c_uint = 0x20	/* Status Register */;
pub const SPDIFRX_RHR: c_uint = 0x24	/* Holding Register */;

    (0x30 + (channel) * 0x30 + (reg) * 4)	/* Channel x Status Registers */

    (0x48 + (channel) * 0x30 + (reg) * 4)	/* Channel x User Data Registers */
pub const SPDIFRX_WPMR: c_uint = 0xE4	/* Write Protection Mode Register */;
pub const SPDIFRX_WPSR: c_uint = 0xE8	/* Write Protection Status Register */;
pub const SPDIFRX_VERSION: c_uint = 0xFC	/* Version Register */;
...
#[no_mangle]
unsafe extern "C" fn mchp_spdifrx_readable_reg(dev: *mut device, reg: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mchp_spdifrx_writeable_reg(dev: *mut device, reg: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mchp_spdifrx_precious_reg(dev: *mut device, reg: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mchp_spdifrx_volatile_reg(dev: *mut device, reg: c_uint) -> bool {
...
pub const SPDIFRX_CS_BITS: c_int = 192;
pub const SPDIFRX_UD_BITS: c_int = 192;
pub const SPDIFRX_CHANNELS: c_int = 2;
//
...
pub struct mchp_spdifrx_ch_stat {
    pub 8]: unsigned char data[SPDIFRX_CS_BITS /,
    pub done: completion,
}
...
pub struct mchp_spdifrx_user_data {
    pub 8]: unsigned char data[SPDIFRX_UD_BITS /,
    pub done: completion,
}
...
#[no_mangle]
unsafe extern "C" fn mchp_spdif_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
// Update software cache with last available data.
    ...
    u32 val;
    int ret;
    let mut ulock_old: bool = ctrl.ulock;
    mutex_lock(&dev.mlock);
    ret = pm_runtime_resume_and_get(dev.dev);
    if (ret < 0)
    goto unlock;
...
//
    ...
    u32 val;
    int ret;
    let mut badf_old: bool = ctrl.badf;
    mutex_lock(&dev.mlock);
    ret = pm_runtime_resume_and_get(dev.dev);
    if (ret < 0)
    goto unlock;
...
//
    ...
    struct mchp_spdifrx_dev *dev = snd_soc_dai_get_drvdata(dai);
    struct mchp_spdifrx_mixer_control *ctrl = &dev.control;
    let mut val: u32 = ~0U, loops = 10;
    int ret;
    let mut signal_old: bool = ctrl.signal;
    mutex_lock(&dev.mlock);
    ret = pm_runtime_resume_and_get(dev.dev);
    if (ret < 0)
    goto unlock;
...
unsafe extern "C" fn mchp_spdifrx_dai_probe(dai: *mut snd_soc_dai) -> c_int {
    ...
    struct mchp_spdifrx_mixer_control *ctrl = &dev.control;
    int ch;
    snd_soc_dai_init_dma_data(dai, core::ptr::null_mut(), &dev.capture);
