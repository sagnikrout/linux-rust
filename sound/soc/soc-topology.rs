...
//! Source: sound/soc/soc-topology.c
#![no_std]
...
pub const SOC_TPLG_MAGIC_BIG_ENDIAN: c_uint = 0x436F5341 /* ASoC in reverse */;
//
...
//
pub const SOC_TPLG_PASS_MANIFEST: c_int = 0;
pub const SOC_TPLG_PASS_VENDOR: c_int = 1;
pub const SOC_TPLG_PASS_CONTROL: c_int = 2;
pub const SOC_TPLG_PASS_WIDGET: c_int = 3;
pub const SOC_TPLG_PASS_PCM_DAI: c_int = 4;
pub const SOC_TPLG_PASS_GRAPH: c_int = 5;
pub const SOC_TPLG_PASS_BE_DAI: c_int = 6;
pub const SOC_TPLG_PASS_LINK: c_int = 7;
...
// runtime FW parsing
    pub /: *const *const *const u8 pos; / read position,
    pub /: *const *const *const u8 hdr_pos; / header position,
    pub /: *mut *mut unsigned int pass; / pass number,
// component caller
    pub dev: *mut device,
    pub comp: *mut snd_soc_component,
    pub /: *mut *mut u32 index; / current block index,
...
#[no_mangle]
pub unsafe extern "C" fn soc_tplg_is_eof(tplg: *mut soc_tplg) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn soc_tplg_get_hdr_offset(tplg: *mut soc_tplg) -> c_ulong {
    static inline unsigned long soc_tplg_get_hdr_offset(struct soc_tplg *tplg)
    {
    return (unsigned long)(tplg.hdr_pos - tplg.fw.data);
    }
#[no_mangle]
pub unsafe extern "C" fn soc_tplg_get_offset(tplg: *mut soc_tplg) -> c_ulong {
...
// mapping of Kcontrol types and associated operations.
    ...
    snd_soc_put_volsw_sx, core::ptr::null_mut()},
    ...
    snd_soc_put_enum_double, core::ptr::null_mut()},
...
#[no_mangle]
unsafe extern "C" fn get_widget_id(tplg_type: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_complete(tplg: *mut soc_tplg) -> c_int {
...
// try and map vendor specific kcontrol handlers first
    ...
    for (i = 0; i < num_ops; i++) {
    if (k.put == core::ptr::null_mut() && ops[i].id == le32_to_cpu(hdr.ops.put))
    k.put = ops[i].put;
    if (k.get == core::ptr::null_mut() && ops[i].id == le32_to_cpu(hdr.ops.get))
    k.get = ops[i].get;
    if (k.info == core::ptr::null_mut() && ops[i].id == le32_to_cpu(hdr.ops.info))
    k.info = ops[i].info;
    }
...
// optionally pass new dynamic kcontrol to component driver.
    ...
    {
    let mut item_len: c_uint = 2 * sizeof(unsigned int);
    unsigned int *p;
    ...
    let mut access: u32 = le32_to_cpu(tc.access);
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_control_dmixer_create(tplg: *mut soc_tplg, kc: *mut snd_kcontrol_new) -> c_int {
...
// pass control to driver for optional further init
    ...
    if (se.dobj.control.dtexts == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_control_denum_create(tplg: *mut soc_tplg, kc: *mut snd_kcontrol_new) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_control_dbytes_create(tplg: *mut soc_tplg, kc: *mut snd_kcontrol_new) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_dbytes_create(tplg: *mut soc_tplg, size: usize) -> c_int {
    static int soc_tplg_dbytes_create(struct soc_tplg *tplg, size_t size)
    {
    let mut kc: snd_kcontrol_new = {0};
    struct soc_bytes_ext *sbe;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_dmixer_create(tplg: *mut soc_tplg, size: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_denum_create(tplg: *mut soc_tplg, size: usize) -> c_int {
...
// optionally pass new dynamic kcontrol to component driver.
    ...
    let mut maxlen: usize = SNDRV_CTL_ELEM_ID_NAME_MAXLEN;
...
// add route dobj to dobj_list
    ...
    unsigned int *kcontrol_type = core::ptr::null_mut();
    struct snd_kcontrol_new *kc;
    let mut mixer_count: c_int = 0;
    let mut bytes_count: c_int = 0;
    let mut enum_count: c_int = 0;
    let mut ret: c_int = 0;
    int i;
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_dapm_complete(tplg: *mut soc_tplg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_check_name(name: *const c_char) -> c_int {
...
// pass control to component driver for optional further init
    ret = soc_tplg_dai_load(tplg, dai_drv, pcm, core::ptr::null_mut());
    if (ret < 0) {
...
// pass control to component driver for optional further init
    ret = soc_tplg_dai_link_load(tplg, link, core::ptr::null_mut());
    if (ret < 0) {
...
#[no_mangle]
pub unsafe extern "C" fn if(invert_fsync: !invert_bclk &&) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(fsync_provider: !bclk_provider &&) -> else {
...
// Find and configure an existing physical DAI link
    ...
    return -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: len) -> else {
    else if (len)
    name = cfg.name;
    else
...
//
    ...
    le32_to_cpu(d.flags));
// pass control to component driver for optional further init
    ret = soc_tplg_dai_load(tplg, dai_drv, core::ptr::null_mut(), dai);
    if (ret < 0) {
    dev_err(tplg.dev, "ASoC: DAI loading failed\n");
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_process_headers(tplg: *mut soc_tplg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn soc_tplg_load(tplg: *mut soc_tplg) -> c_int {
