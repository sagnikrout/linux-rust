...
//! Source: sound/soc/sof/intel/hda-stream.c
#![no_std]
...
//
    ...
    let mut sof_hda_position_quirk: c_int = SOF_HDA_POSITION_QUIRK_USE_DPIB_REGISTERS;
    module_param_named(position_quirk, sof_hda_position_quirk, int, 0444);
    MODULE_PARM_DESC(position_quirk, "SOF HDaudio position quirk");
    EXPORT_SYMBOL_NS(sof_hda_position_quirk, "SND_SOC_SOF_INTEL_HDA_COMMON");
pub const HDA_LTRP_GB_VALUE_US: c_int = 95;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: hstream->cstream) -> else {
...
// one BDLE should not cross 4K boundary
    ...
    let mut remain: u32 = 0x1000 - (offset & 0xfff);
    if (chunk > remain)
    chunk = remain;
    }
...
// set the last small entry
    ...
    dev_err(sdev.dev, "error: address of spib capability is core::ptr::null_mut()\n");
    return -EINVAL;
    }
...
// get next unused stream
    ...
    struct hdac_ext_stream *hext_stream = core::ptr::null_mut();
...
// free a stream
#[no_mangle]
unsafe extern "C" fn _hda_dsp_stream_put(sdev: *mut snd_sof_dev, direction: c_int, stream_tag: c_int, pair: bool) -> c_int {
    ...
    struct hdac_stream *s;
    let mut dmi_l1_enable: bool = true;
...
#[no_mangle]
unsafe extern "C" fn hda_dsp_stream_reset(sdev: *mut snd_sof_dev, hstream: *mut hdac_stream) -> c_int {
    static int hda_dsp_stream_reset(struct snd_sof_dev *sdev, struct hdac_stream *hstream)
    {
    let mut sd_offset: c_int = SOF_STREAM_SD_OFFSET(hstream);
    let mut timeout: c_int = HDA_DSP_STREAM_RESET_TIMEOUT;
    u32 val;
// enter stream reset
...
// wait for hardware to report that stream is out of reset
    ...
    struct hdac_stream *hstream = &hext_stream.hstream;
    let mut sd_offset: c_int = SOF_STREAM_SD_OFFSET(hstream);
    let mut dma_start: u32 = SOF_HDA_SD_CTL_DMA_START;
    let mut ret: c_int = 0;
    u32 run;
...
// read FIFO size
    ...
    if (!sdev.dspless_mode_selected) {
    struct hdac_bus *bus = sof_to_bus(sdev);
    let mut mask: u32 = BIT(hstream.index);
...
// if Register inaccessible, ignore it.
    ...
    {
    let mut buffer_size: u64 = hstream.bufsize;
...
#[no_mangle]
unsafe extern "C" fn hda_dsp_stream_check(bus: *mut hdac_bus, status: u32) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn hda_dsp_stream_threaded_handler(irq: c_int, context: *mut c_void) -> irqreturn_t {
...
//
    ...
    struct snd_soc_pcm_runtime *be_rtd = core::ptr::null_mut();
    struct hdac_ext_stream *hext_stream;
