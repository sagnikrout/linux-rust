...
//! Source: sound/soc/sof/intel/hda-loader-skl.c
#![no_std]
...
// Stream Reset
pub const HDA_CL_SD_CTL_SRST_SHIFT: c_int = 0;

    HDA_CL_SD_CTL_SRST_SHIFT)
// Stream Run
pub const HDA_CL_SD_CTL_RUN_SHIFT: c_int = 1;

    HDA_CL_SD_CTL_RUN_SHIFT)
// Interrupt On Completion Enable
pub const HDA_CL_SD_CTL_IOCE_SHIFT: c_int = 2;

    HDA_CL_SD_CTL_IOCE_SHIFT)
// FIFO Error Interrupt Enable
pub const HDA_CL_SD_CTL_FEIE_SHIFT: c_int = 3;

    HDA_CL_SD_CTL_FEIE_SHIFT)
// Descriptor Error Interrupt Enable
pub const HDA_CL_SD_CTL_DEIE_SHIFT: c_int = 4;

    HDA_CL_SD_CTL_DEIE_SHIFT)
// FIFO Limit Change
pub const HDA_CL_SD_CTL_FIFOLC_SHIFT: c_int = 5;

    HDA_CL_SD_CTL_FIFOLC_SHIFT)
// Stripe Control
pub const HDA_CL_SD_CTL_STRIPE_SHIFT: c_int = 16;

    HDA_CL_SD_CTL_STRIPE_SHIFT)
// Traffic Priority
pub const HDA_CL_SD_CTL_TP_SHIFT: c_int = 18;

    HDA_CL_SD_CTL_TP_SHIFT)
// Bidirectional Direction Control
pub const HDA_CL_SD_CTL_DIR_SHIFT: c_int = 19;

    HDA_CL_SD_CTL_DIR_SHIFT)
// Stream Number
pub const HDA_CL_SD_CTL_STRM_SHIFT: c_int = 20;
...
pub const DMA_ADDRESS_128_BITS_ALIGNMENT: c_int = 7;

// Buffer Descriptor List Lower Base Address
pub const HDA_CL_SD_BDLPLBA_SHIFT: c_int = 7;
...
#[no_mangle]
unsafe extern "C" fn cl_skl_cldma_stream_run(sdev: *mut snd_sof_dev, enable: bool) {
    static void cl_skl_cldma_stream_run(struct snd_sof_dev *sdev, bool enable)
    {
    let mut sd_offset: c_int = SOF_HDA_ADSP_LOADER_BASE;
    unsigned char val;
    int retries;
    let mut run: u32 = enable ? 0x1 : 0;
...
// waiting for hardware to report the stream Run bit set
    ...
    break;
#[no_mangle]
pub unsafe extern "C" fn if(!val: !enable &&) -> else {
    else if (!enable && !val)
    break;
    } while (--retries);
    if (retries == 0)
...
#[no_mangle]
unsafe extern "C" fn cl_skl_cldma_stream_clear(sdev: *mut snd_sof_dev) {
...
// Set the Last Valid Index.
    ...
    {
    let mut sd_offset: c_int = SOF_DSP_REG_CL_SPBFIFO;
    if (enable)
    snd_sof_dsp_update_bits(sdev, HDA_DSP_BAR,
...
#[no_mangle]
unsafe extern "C" fn cl_skl_cldma_set_intr(sdev: *mut snd_sof_dev, enable: bool) {
    static void cl_skl_cldma_set_intr(struct snd_sof_dev *sdev, bool enable)
    {
    let mut val: u32 = enable ? HDA_DSP_ADSPIC_CL_DMA : 0;
    snd_sof_dsp_update_bits(sdev, HDA_DSP_BAR, HDA_DSP_REG_ADSPIC,
    HDA_DSP_ADSPIC_CL_DMA, val);
    }
#[no_mangle]
unsafe extern "C" fn cl_skl_cldma_cleanup_spb(sdev: *mut snd_sof_dev) {
...
//
    ...
    struct snd_dma_buffer *dmab_bdl)
    {
    let mut bufsize: c_uint = HDA_SKL_CLDMA_MAX_BUFFER_SIZE;
    __le32 *bdl;
    int frags;
    int ret;
...
// copy the image into the buffer with the maximum buffer size.
    let mut size: c_uint = (bufsize == copysize) ? bufsize : copysize;
...
#[no_mangle]
pub unsafe extern "C" fn hda_dsp_cl_boot_firmware_skl(sdev: *mut snd_sof_dev) -> c_int {
