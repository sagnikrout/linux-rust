...
//! Source: sound/soc/sof/intel/hda-dai-ops.c
#![no_std]
...
//
    ...
    struct hdac_ext_stream *res = core::ptr::null_mut();
    struct hdac_stream *hstream = core::ptr::null_mut();
    let mut stream_dir: c_int = substream.stream;
    let mut is_multi: bool = link_type == HDA_BUS_ML_LINK_HDA || link_type == HDA_BUS_ML_LINK_UAOL;
    let mut is_play: bool = stream_dir == SNDRV_PCM_STREAM_PLAYBACK;
    let mut is_sdw: bool = link_type == HDA_BUS_ML_LINK_SDW;
    let mut is_hda: bool = link_type == HDA_BUS_ML_LINK_HDA;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_multi) -> else {
    else if (is_multi)
...
// mark pipeline so that it can be skipped during FE trigger
    ...
    {
    struct hdac_ext_stream *hext_stream;
    let mut link_type: enum hda_bus_ml_link_type = hda_bus_ml_link_get_type(hlink);
    ...
    let mut stream_idx: c_uint = hext_stream.hstream.stream_tag - 1;
//
...
#[no_mangle]
unsafe extern "C" fn hda_reset_hext_stream(sdev: *mut snd_sof_dev, hext_stream: *mut hdac_ext_stream) {
...
//
    ...
    case SNDRV_PCM_TRIGGER_STOP:
    {
    let mut data: snd_sof_dai_config_data = { 0 };
    int ret;
