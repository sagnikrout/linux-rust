...
//! Source: sound/soc/sof/ipc4-loader.c
#![no_std]
...
pub const SOF_IPC4_MOD_LIB_ID_SHIFT: c_int = 12;
...
// copy the fw_version of basefw into debugfs at first boot
    ...
    fw_module.private = core::ptr::null_mut();
    fw_module++;
    fm_entry++;
    }
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_fw_parse_basefw_ext_man(sdev: *mut snd_sof_dev) -> usize {
...
unsafe extern "C" fn sof_ipc4_validate_firmware(sdev: *mut snd_sof_dev) -> c_int {
    ...
    {
    struct sof_ipc4_fw_data *ipc4_data = sdev.private;
    let mut fw_hdr_offset: u32 = ipc4_data.manifest_fw_hdr_offset;
    struct sof_man4_fw_binary_header *fw_header;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !cpc_pick) -> else {
    else if (!cpc_pick)
