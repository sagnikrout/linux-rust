...
//! Source: sound/soc/sof/ipc4-pcm.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn sof_ipc4_set_pipeline_state(sdev: *mut snd_sof_dev, instance_id: u32, state: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(pipe_priority[i]: !ascend && pipeline->priority >) -> else {
...
// command is not relevant with RESET, so just pass 0
    ...
    let mut dai_link_found: bool = false;
...
//
    ...
    let mut num_input_formats: c_int = ipc4_copier.available_fmt.num_input_formats;
    let mut fe_rate: c_uint = params_rate(params);
    let mut fe_be_rate_match: bool = false;
    let mut single_be_rate: bool = true;
...
//
    ...
    let mut val: c_uint = pin_fmts[i].audio_fmt.sampling_frequency;
    if (val != be_rate)
...
//
    ...
    let mut num_input_formats: c_int = ipc4_copier.available_fmt.num_input_formats;
    let mut fe_channels: c_uint = params_channels(params);
    let mut fe_be_match: bool = false;
    let mut single_be_channels: bool = true;
...
//
    ...
    struct sof_ipc4_copier *ipc4_copier;
    let mut single_bitdepth: bool = false;
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_pcm_free(sdev: *mut snd_sof_dev, spcm: *mut snd_sof_pcm) {
    ...
    pipeline_list.pipelines = core::ptr::null_mut();
    stream_priv = spcm.stream[stream].private;
    kfree(stream_priv.time_info);
    kfree(spcm.stream[stream].private);
    spcm.stream[stream].private = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn sof_ipc4_pcm_setup(sdev: *mut snd_sof_dev, spcm: *mut snd_sof_pcm) -> c_int {
    ...
    struct sof_ipc4_pcm_stream_priv *stream_priv;
    struct sof_ipc4_timestamp_info *time_info;
    let mut support_info: bool = true;
    u32 abi_version;
    u32 abi_offset;
    int stream;
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_build_time_info(sdev: *mut snd_sof_dev, sps: *mut snd_sof_pcm_stream) {
    static void sof_ipc4_build_time_info(struct snd_sof_dev *sdev, struct snd_sof_pcm_stream *sps)
    {
    struct sof_ipc4_copier *host_copier = core::ptr::null_mut();
    struct sof_ipc4_copier *dai_copier = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_frames_dai_to_host(time_info: *mut sof_ipc4_timestamp_info, value: u64) -> u64 {
...
//
    let mut pre_ms: c_int = SOF_IPC4_CHAIN_DMA_BUF_SIZE_MS * 5 / 2 + 1;
