...
// query DAPM for list of connected widgets and set them up
    ...
    spcm.stream[dir].list = core::ptr::null_mut();
...
// if this is a repeated hw_params without hw_free, skip setting up widgets
    ...
    if (!sdev.dspless_mode_selected) {
    let mut host_comp_id: c_int = spcm.stream[substream.stream].comp_id;
...
//
    ...
    const struct sof_ipc_pcm_ops *pcm_ops = sof_ipc_get_ops(sdev, pcm);
    struct snd_sof_pcm *spcm;
    let mut reset_hw_params: bool = false;
    let mut ipc_first: bool = false;
    let mut ret: c_int = 0;
// nothing to do for BE
    if (rtd.dai_link.no_pcm)
    return 0;
...
//
    ...
    spcm.stream[substream.stream].substream = core::ptr::null_mut();
    return 0;
    }
//
...
//
    ...
    struct snd_soc_tplg_stream_caps *caps;
    let mut stream: c_int = SNDRV_PCM_STREAM_PLAYBACK;
...
// do we need to pre-allocate playback audio buffer pages
    ...
    spcm_err(spcm, stream, "core::ptr::null_mut() playback substream!\n");
    return -EINVAL;
    }
...
// do we need to pre-allocate capture audio buffer pages
    ...
    caps = &spcm.pcm.caps[stream];
    if (!pcm.streams[stream].substream) {
    spcm_err(spcm, stream, "core::ptr::null_mut() capture substream!\n");
    return -EINVAL;
    }
...
#[no_mangle]
pub unsafe extern "C" fn sof_pcm_dai_link_fixup(rtd: *mut snd_soc_pcm_runtime, params: *mut snd_pcm_hw_params) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: plat_data->of_machine) -> else {
    else if (plat_data.of_machine)
    drv_name = plat_data.of_machine.drv_name;
    else
    drv_name = core::ptr::null_mut();
