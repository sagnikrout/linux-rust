...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_check_reply_status(sdev: *mut snd_sof_dev, status: u32) -> c_int {
    ...
    {
    u32 val, type;
    const u8 *str2 = core::ptr::null_mut();
    const u8 *str = core::ptr::null_mut();
...
// Notification message
    let mut notif: u32 = SOF_IPC4_NOTIFICATION_TYPE_GET(msg.primary);
...
// Print resource details
    ...
    let mut module_id: u32 = SOF_IPC4_MOD_ID_GET(data.resource_id);
    let mut instance_id: u32 = SOF_IPC4_MOD_INSTANCE_GET(data.resource_id);
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_get_reply(sdev: *mut snd_sof_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ipc4_wait_tx_done(ipc: *mut snd_sof_ipc, reply_data: *mut c_void) -> c_int {
...
// Serialise IPC TX
    ...
    struct sof_ipc4_msg *msg = core::ptr::null_mut();
// payload is indicated by non zero msg/reply_bytes
    if (msg_bytes)
    msg = msg_data;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: reply_bytes) -> else {
    else if (reply_bytes)
    msg = reply_data;
    if (msg)
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_tx_payload_for_get_data(tx: *mut sof_ipc4_msg) -> bool {
...
//
    ...
    let mut payload_limit: usize = sdev.ipc.max_payload_size;
    struct sof_ipc4_msg *ipc4_msg = data;
    let mut tx: sof_ipc4_msg = {{ 0 }};
    let mut rx: sof_ipc4_msg = {{ 0 }};
    let mut remaining: usize = payload_bytes;
    void *tx_payload_for_get = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_init_msg_memory(sdev: *mut snd_sof_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ipc4_fw_ready(sdev: *mut snd_sof_dev, ipc4_msg: *mut sof_ipc4_msg) -> c_int {
...
// re-read the whole payload
    ...
    ret = snd_sof_ipc_msg_data(sdev, core::ptr::null_mut(), ipc4_msg.data_ptr,
    ipc4_msg.data_size);
    if (ret < 0) {
    dev_err(sdev.dev,
...
unsafe extern "C" fn sof_ipc4_rx_msg(sdev: *mut snd_sof_dev) {
    ...
    let mut handler_func: ipc4_notification_handler = core::ptr::null_mut();
    let mut data_size: usize = 0;
    int err;
    if (!ipc4_msg || !SOF_IPC4_MSG_IS_NOTIFICATION(ipc4_msg.primary))
    return;
    ipc4_msg.data_ptr = core::ptr::null_mut();
...
// wake up firmware loader
    ...
    err = snd_sof_ipc_msg_data(sdev, core::ptr::null_mut(), ipc4_msg.data_ptr, ipc4_msg.data_size);
    if (err < 0) {
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_set_core_state(sdev: *mut snd_sof_dev, core_idx: c_int, on: bool) -> c_int {
    ...
    return sof_ipc4_tx_msg(sdev, &msg, msg.data_size, core::ptr::null_mut(), 0, false);
    }
//
...
unsafe extern "C" fn sof_ipc4_set_pm_gate(sdev: *mut snd_sof_dev, flags: u32) -> c_int {
    ...
    msg.primary |= SOF_IPC4_MSG_TARGET(SOF_IPC4_MODULE_MSG);
    msg.extension = flags;
    return sof_ipc4_tx_msg(sdev, &msg, 0, core::ptr::null_mut(), 0, true);
    }
...
//
    ...
    fw_lib.sof_fw.fw = core::ptr::null_mut();
    }
...
unsafe extern "C" fn sof_ipc4_post_boot(sdev: *mut snd_sof_dev) -> c_int {
    ...
    {
    if (sdev.first_boot) {
    let mut ret: c_int = sof_ipc4_complete_split_release(sdev);
    if (ret)
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn sof_ipc4_mic_privacy_state_change(sdev: *mut snd_sof_dev, state: bool) {
    void sof_ipc4_mic_privacy_state_change(struct snd_sof_dev *sdev, bool state)
    {
    struct sof_ipc4_msg msg;
    let mut data: u32 = state;
//
