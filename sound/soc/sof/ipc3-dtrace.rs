...
//! Source: sound/soc/sof/ipc3-dtrace.c
#![no_std]
...
pub const TRACE_FILTER_ELEMENTS_PER_ENTRY: c_int = 4;
pub const TRACE_FILTER_MAX_CONFIG_STRING_LENGTH: c_int = 1024;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sof_dtrace_priv {
    pub dmatb: snd_dma_buffer,
    pub dmatp: snd_dma_buffer,
    pub dma_trace_pages: c_int,
    pub trace_sleep: wait_queue_head_t,
    pub host_offset: u32,
    pub dtrace_error: bool,
    pub dtrace_draining: bool,
    pub dtrace_state: enum sof_dtrace_state,
}

#[no_mangle]
unsafe extern "C" fn trace_pos_update_expected(priv: *mut sof_dtrace_priv) -> bool {
    ...
    let mut len: c_int = strlen(line);
    let mut cnt: c_int = *counter;
    u32 uuid_id;
...
// Make sure the DSP/firmware is booted up
    ...
    struct sof_ipc_trace_filter_elem *elems = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn debugfs_create_trace_filter(sdev: *mut snd_sof_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sof_dtrace_set_host_offset(priv: *mut sof_dtrace_priv, new_offset: u32) -> bool {
    static bool sof_dtrace_set_host_offset(struct sof_dtrace_priv *priv, u32 new_offset)
    {
    let mut host_offset: u32 = READ_ONCE(priv.host_offset);
    if (host_offset != new_offset) {
// This is a bit paranoid and unlikely that it is needed
    let mut ret: u32 = cmpxchg(&priv.host_offset, host_offset, new_offset);
    if (ret == host_offset)
    return true;
    }
    return false;
    ...
    let mut host_offset: loff_t = READ_ONCE(priv.host_offset);
//
...
// If there is available trace data now, it is unnecessary to wait.
    ...
    size_t buffer_size)
    {
    let mut ret: usize = sof_dtrace_avail(sdev, pos, buffer_size);
    struct sof_dtrace_priv *priv = sdev.fw_trace_data;
...
#[no_mangle]
unsafe extern "C" fn dfsentry_dtrace_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn debugfs_create_dtrace(sdev: *mut snd_sof_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ipc3_dtrace_enable(sdev: *mut snd_sof_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ipc3_dtrace_init(sdev: *mut snd_sof_dev) -> c_int {
