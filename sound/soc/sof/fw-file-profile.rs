...
//! Source: sound/soc/sof/fw-file-profile.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn sof_platform_uses_generic_loader(sdev: *mut snd_sof_dev) -> bool {
    ...
    let mut fw_lib_path_allocated: bool = false;
    struct device *dev = sdev.dev;
    let mut fw_path_allocated: bool = false;
    let mut ret: c_int = 0;
// firmware path
...
// Test only default firmware file
    if ((!base_profile.fw_path && !base_profile.fw_name) &&
    sof_platform_uses_generic_loader(sdev))
    ret = sof_test_firmware_file(dev, out_profile, core::ptr::null_mut());
    if (!ret)
