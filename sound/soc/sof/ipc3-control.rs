...
//! Source: sound/soc/sof/ipc3-control.c
#![no_std]
...
// notify DSP of mixer updates
    if (pm_runtime_active(scomp.dev)) {
    let mut ret: c_int = sof_ipc3_set_get_kcontrol_data(scontrol, true, true);
    if (ret < 0) {
...
// get all the component data from DSP
    if (from_dsp) {
    let mut ret: c_int = sof_ipc3_set_get_kcontrol_data(scontrol, false, true);
    if (ret < 0)
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn sof_ipc3_control_update(sdev: *mut snd_sof_dev, ipc_control_message: *mut c_void) {
