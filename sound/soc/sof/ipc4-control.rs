...
//! Source: sound/soc/sof/ipc4-control.c
#![no_std]
...
//
    ...
    let mut widget_found: bool = false;
    let mut ret: c_int = 0;
...
// Send the last known good configuration to firmware
    ...
    let mut all_channels_equal: bool = true;
    struct sof_ipc4_msg msg;
    u32 value;
    int ret, i;
...
// get all the component data from DSP
    if (from_dsp) {
    struct snd_sof_dev *sdev = snd_soc_component_get_drvdata(scomp);
    let mut ret: c_int = sof_ipc4_set_get_bytes_data(sdev, scontrol, false, true);
    if (ret < 0)
    return ret;
...
unsafe extern "C" fn sof_ipc4_control_update(sdev: *mut snd_sof_dev, ipc_message: *mut c_void) {
    ...
    struct snd_sof_control *scontrol;
    struct snd_sof_widget *swidget;
    struct snd_kcontrol *kc = core::ptr::null_mut();
    let mut scontrol_found: bool = false;
    u32 event_param_id;
    int i, type;
...
//
    ...
    struct sof_abi_hdr *data = cdata.data;
    let mut source_size: usize = struct_size(msg_data, data, msg_data.num_elems);
    ...
    let mut source_size: usize = struct_size(msg_data, chanv, msg_data.num_elems);
    if (source_size > ndata.event_data_size) {
    dev_warn(sdev.dev,
    ...
    let mut channel: u32 = msg_data.chanv[i].channel;
...
#[no_mangle]
unsafe extern "C" fn sof_ipc4_widget_kcontrol_setup(sdev: *mut snd_sof_dev, swidget: *mut snd_sof_widget) -> c_int {
...
// populate the volume table
    for (i = 0; i < size ; i++) {
    let mut val: u32 = vol_compute_gain(i, tlv);
