...
//! Source: sound/soc/stm/stm32_adfsdm.c
#![no_std]
...
pub const DFSDM_MAX_PERIODS: c_int = 6;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_adfsdm_priv {
    pub dai_drv: snd_soc_dai_driver,
    pub substream: *mut snd_pcm_substream,
    pub dev: *mut device,
// IIO
    pub iio_ch: *mut iio_channel,
    pub iio_cb: *mut iio_cb_buffer,
    pub iio_active: bool,
// PCM buffer
    pub pcm_buff: *mut c_uchar,
    pub pos: c_uint,
    pub /: *mut *mut mutex lock; / protect against race condition on iio state,
}
...
#[no_mangle]
unsafe extern "C" fn stm32_memcpy_32to16(dest: *mut c_void, src: *const c_void, n: usize) {
...
#[no_mangle]
unsafe extern "C" fn stm32_afsdm_pcm_cb(data: *const c_void, size: usize, private: *mut c_void) -> c_int {
    ...
    u8 *src_buff = (u8 *)data;
    let mut old_pos: c_uint = priv.pos;
    let mut buff_size: usize = snd_pcm_lib_buffer_bytes(priv.substream);
    let mut period_size: usize = snd_pcm_lib_period_bytes(priv.substream);
    size_t cur_size, src_size = size;
    let mut format: snd_pcm_format_t = priv.substream.runtime.format;
    ...
    struct stm32_adfsdm_priv *priv =
    snd_soc_dai_get_drvdata(snd_soc_rtd_to_cpu(rtd, 0));
    let mut size: c_uint = DFSDM_MAX_PERIODS * DFSDM_MAX_PERIOD_SIZE;
    snd_pcm_set_managed_buffer_all(pcm, SNDRV_DMA_TYPE_DEV,
    priv.dev, size, size);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn stm32_adfsdm_dummy_cb(data: *const c_void, private: *mut c_void) -> c_int {
...
// Associate iio channel
    ...
    priv.iio_cb = iio_channel_get_all_cb(&pdev.dev, &stm32_adfsdm_dummy_cb, core::ptr::null_mut());
