...
pub struct snd_soc_dapm_context {
    pub bias_level: enum snd_soc_bias_level,
    pub /: *mut *mut bool idle_bias; / Use BIAS_OFF instead of STANDBY when false,
    pub /: *mut *mut *mut snd_soc_component component; / parent component,
    pub /: *mut *mut *mut snd_soc_card card; / parent card,
// used during DAPM updates
    pub target_bias_level: enum snd_soc_bias_level,
    pub list: list_head,
...
#[no_mangle]
unsafe extern "C" fn dapm_assert_locked(dapm: *mut snd_soc_dapm_context) {
...
#[no_mangle]
unsafe extern "C" fn dapm_pop_wait() {
    ...
    __printf(2, 3)
#[no_mangle]
unsafe extern "C" fn dapm_pop_dbg(dev: *mut device, fmt: *const c_char, ...) {
...
#[no_mangle]
unsafe extern "C" fn dapm_dirty_widget(w: *mut snd_soc_dapm_widget) -> bool {
    static bool dapm_dirty_widget(struct snd_soc_dapm_widget *w)
    {
    return !list_empty(&w.dirty);
    }
#[no_mangle]
unsafe extern "C" fn dapm_mark_dirty(w: *mut snd_soc_dapm_widget, reason: *const c_char) {
...
//
    ...
    {
    let mut rdir: enum snd_soc_dapm_direction = DAPM_DIR_REVERSE(dir);
...
//
#[no_mangle]
unsafe extern "C" fn dapm_widget_invalidate_input_paths(w: *mut snd_soc_dapm_widget) {
...
//
#[no_mangle]
unsafe extern "C" fn dapm_widget_invalidate_output_paths(w: *mut snd_soc_dapm_widget) {
...
//
#[no_mangle]
unsafe extern "C" fn dapm_path_invalidate(p: *mut snd_soc_dapm_path) {
...
#[no_mangle]
unsafe extern "C" fn dapm_read(dapm: *mut snd_soc_dapm_context, reg: c_int) -> c_uint {
...
// set up initial codec paths
    ...
    let mut max: c_uint = mc.max;
    let mut mask: c_uint = (1 << fls(max)) - 1;
    let mut val: c_uint = dapm_read(p.sink.dapm, reg);
//
...
//
#[no_mangle]
unsafe extern "C" fn dapm_update_widget_flags(w: *mut snd_soc_dapm_widget) {
...
// On a fully routed card a output is never a sink
    ...
    let mut dynamic_source: bool = false;
    let mut dynamic_sink: bool = false;
    if (!control)
    return 0;
    switch (source.id) {
...
// connect static paths
    ...
    widget, core::ptr::null_mut(), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn dapm_kcontrol_is_powered(kcontrol: *const snd_kcontrol) -> bool {
...
#[no_mangle]
unsafe extern "C" fn dapm_reset(card: *mut snd_soc_card) {
...
#[no_mangle]
unsafe extern "C" fn dapm_async_complete(dapm: *mut snd_soc_dapm_context) {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_init_bias_level(dapm: *mut snd_soc_dapm_context, level: enum snd_soc_bias_level) {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_get_bias_level(dapm: *mut snd_soc_dapm_context) -> enum snd_soc_bias_level {
...
//
    ...
    char *long_name = core::ptr::null_mut();
    const char *name;
    let mut ret: c_int = 0;
...
//
    ...
    if (long_name == core::ptr::null_mut())
    return -ENOMEM;
    name = long_name;
    } else if (wname_in_long_name) {
    long_name = core::ptr::null_mut();
    name = w.name + prefix_len;
    } else {
    long_name = core::ptr::null_mut();
    name = w.kcontrol_news[kci].name;
    }
    kcontrol = snd_soc_cnew(&w.kcontrol_news[kci], core::ptr::null_mut(), name,
    prefix);
    if (!kcontrol) {
    ret = -ENOMEM;
    goto exit_free;
...
#[no_mangle]
unsafe extern "C" fn dapm_new_mixer(w: *mut snd_soc_dapm_widget) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dapm_new_mux(w: *mut snd_soc_dapm_widget) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dapm_new_pga(w: *mut snd_soc_dapm_widget) -> c_int {
    static int dapm_new_pga(struct snd_soc_dapm_widget *w)
    {
    int i;
    for (i = 0; i < w.num_kcontrols; i++) {
    let mut ret: c_int = dapm_create_or_share_kcontrol(w, i);
    if (ret < 0)
    return ret;
    }
    return 0;
...
#[no_mangle]
unsafe extern "C" fn dapm_new_dai_link(w: *mut snd_soc_dapm_widget) -> c_int {
...
// add kcontrol
    ...
    w, w.name, core::ptr::null_mut());
    let mut ret: c_int = snd_ctl_add(card, kcontrol);
    if (ret < 0) {
    dev_err(dev,
    "ASoC: failed to add widget %s dapm kcontrol %s: %d\n",
...
//
#[no_mangle]
unsafe extern "C" fn dapm_suspend_check(widget: *mut snd_soc_dapm_widget) -> c_int {
    static int dapm_suspend_check(struct snd_soc_dapm_widget *widget)
    {
    struct device *dev = snd_soc_dapm_to_dev(widget.dapm);
    let mut level: c_int = snd_power_get_state(widget.dapm.card.snd_card);
    switch (level) {
...
#[no_mangle]
unsafe extern "C" fn dapm_widget_list_free(list: *mut snd_soc_dapm_widget_list) {
...
// do we need to add this widget to the list ?
    ...
    custom_stop_condition = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_dai_free_widgets(list: *mut snd_soc_dapm_widget_list) {
...
#[no_mangle]
unsafe extern "C" fn dapm_widget_power_check(w: *mut snd_soc_dapm_widget) -> c_int {
...
unsafe extern "C" fn dapm_generic_check_power(w: *mut snd_soc_dapm_widget) -> c_int {
    ...
    in  = dapm_is_connected_input_ep(w, core::ptr::null_mut(), core::ptr::null_mut());
    out = dapm_is_connected_output_ep(w, core::ptr::null_mut(), core::ptr::null_mut());
    return out != 0 && in != 0;
    }
// Check to see if a power supply is needed
#[no_mangle]
unsafe extern "C" fn dapm_supply_check_power(w: *mut snd_soc_dapm_widget) -> c_int {
...
// Insert a widget in order into a DAPM power sequence.
    ...
    ret = w.event(w, core::ptr::null_mut(), event);
...
//
    ...
    LIST_HEAD(pending);
    let mut cur_sort: c_int = -1;
    let mut cur_subseq: c_int = -1;
    let mut cur_reg: c_int = SND_SOC_NOPM;
    struct snd_soc_dapm_context *cur_dapm = core::ptr::null_mut();
    int i;
    int *sort;
    if (power_up)
    sort = dapm_up_seq;
...
// Do we need to apply any queued changes?
    ...
    ret = w.event(w,
    core::ptr::null_mut(), SND_SOC_DAPM_PRE_PMU);
#[no_mangle]
pub unsafe extern "C" fn if(SND_SOC_DAPM_STREAM_STOP: event ==) -> else {
    else if (event == SND_SOC_DAPM_STREAM_STOP)
    ret = w.event(w,
    core::ptr::null_mut(), SND_SOC_DAPM_PRE_PMD);
    break;
    ...
    if (event == SND_SOC_DAPM_STREAM_START)
    ret = w.event(w,
    core::ptr::null_mut(), SND_SOC_DAPM_POST_PMU);
#[no_mangle]
pub unsafe extern "C" fn if(SND_SOC_DAPM_STREAM_STOP: event ==) -> else {
    else if (event == SND_SOC_DAPM_STREAM_STOP)
    ret = w.event(w,
    core::ptr::null_mut(), SND_SOC_DAPM_POST_PMD);
    break;
    default:
...
#[no_mangle]
unsafe extern "C" fn dapm_widget_update(card: *mut snd_soc_card, update: *mut snd_soc_dapm_update) {
    static void dapm_widget_update(struct snd_soc_card *card, struct snd_soc_dapm_update *update)
    {
    struct device *dev = card.dev;
    struct snd_soc_dapm_widget_list *wlist;
    struct snd_soc_dapm_widget *w = core::ptr::null_mut();
    unsigned int wi;
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_get_idle_bias(dapm: *mut snd_soc_dapm_context) -> bool {
    bool snd_soc_dapm_get_idle_bias(struct snd_soc_dapm_context *dapm)
    {
    if (dapm.idle_bias) {
    struct snd_soc_component *component = snd_soc_dapm_to_component(dapm);
    let mut state: c_uint = snd_power_get_state(dapm.card.snd_card);
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_set_idle_bias(dapm: *mut snd_soc_dapm_context, on: bool) {
...
// do we need to notify any clients that DAPM event is complete
    ...
    int in, out;
    ssize_t ret;
    struct snd_soc_dapm_path *p = core::ptr::null_mut();
    const char *c_name;
...
// Supply widgets are not handled by dapm_is_connected_{input,output}_ep()
    ...
    p.node[rdir].dapm.component.name : core::ptr::null_mut();
    ret += scnprintf(buf + ret, PAGE_SIZE - ret,
    " %s  \"%s\" \"%s\" \"%s\"\n",
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_debugfs_pop_time(parent: *mut dentry) {
...
#[no_mangle]
unsafe extern "C" fn dapm_debugfs_add_widget(w: *mut snd_soc_dapm_widget) {
...
#[no_mangle]
unsafe extern "C" fn dapm_debugfs_free_widget(w: *mut snd_soc_dapm_widget) {
...
#[no_mangle]
unsafe extern "C" fn dapm_debugfs_cleanup(dapm: *mut snd_soc_dapm_context) {
    static void dapm_debugfs_cleanup(struct snd_soc_dapm_context *dapm)
    {
    debugfs_remove_recursive(dapm.debugfs_dapm);
    dapm.debugfs_dapm = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn dapm_debugfs_add_widget(w: *mut snd_soc_dapm_widget) {
    static inline void dapm_debugfs_add_widget(struct snd_soc_dapm_widget *w)
    {
    }
#[no_mangle]
pub unsafe extern "C" fn dapm_debugfs_free_widget(w: *mut snd_soc_dapm_widget) {
    static inline void dapm_debugfs_free_widget(struct snd_soc_dapm_widget *w)
    {
    }
#[no_mangle]
pub unsafe extern "C" fn dapm_debugfs_cleanup(dapm: *mut snd_soc_dapm_context) {
...
#[no_mangle]
unsafe extern "C" fn dapm_free_path(path: *mut snd_soc_dapm_path) {
...
#[no_mangle]
unsafe extern "C" fn dapm_free_widgets(dapm: *mut snd_soc_dapm_context) {
    ...
    dapm.wcache_sink	= core::ptr::null_mut();
    dapm.wcache_source	= core::ptr::null_mut();
    }
    ...
    {
    struct snd_soc_dapm_widget *w;
    struct snd_soc_dapm_widget *fallback = core::ptr::null_mut();
    let mut pin_has_prefix: bool = snd_soc_dapm_pin_has_prefix(dapm.card, pin);
    bool match;
    for_each_card_widgets(dapm.card, w) {
    match = false;
    ...
    match = true;
#[no_mangle]
pub unsafe extern "C" fn if(!snd_soc_dapm_widget_name_cmp(w: !pin_has_prefix &&, _arg: pin)) -> else {
...
//
    ...
    const char *pin, int status)
    {
    let mut ret: c_int = __dapm_set_pin(dapm, pin, status);
    return ret < 0 ? ret : 0;
    }
//
...
//
    if (!snd_soc_card_is_instantiated(dapm.card))
    return 0;
    return dapm_power_widgets(dapm.card, SND_SOC_DAPM_STREAM_NOP, core::ptr::null_mut());
    }
...
pub unsafe extern "C" fn snd_soc_dapm_sync(dapm: *mut snd_soc_dapm_context) -> c_int {
    ...
    {
    let mut dir: c_int = substream.stream;
...
pub unsafe extern "C" fn snd_soc_dapm_widget_name_cmp(widget: *mut snd_soc_dapm_widget, s: *const c_char) -> c_int {
    ...
    {
    struct snd_soc_dapm_widget *wsource = core::ptr::null_mut(), *wsink = core::ptr::null_mut(), *w;
    struct snd_soc_dapm_widget *wtsource = core::ptr::null_mut(), *wtsink = core::ptr::null_mut();
    struct device *dev = snd_soc_dapm_to_dev(dapm);
    const char *sink;
    const char *source;
    ...
    const char *prefix;
    let mut sink_ref: c_uint = 0;
    let mut source_ref: c_uint = 0;
    int ret;
    prefix = dapm_prefix(dapm);
    if (prefix) {
...
//
    ...
    snd_soc_dapm_mutex_lock(dapm);
    for (i = 0; i < num; i++) {
    let mut r: c_int = snd_soc_dapm_add_route(dapm, route);
    if (r < 0)
    ret = r;
    route++;
    }
...
// Read the initial power state from the device
    ...
    dapm_power_widgets(card, SND_SOC_DAPM_STREAM_NOP, core::ptr::null_mut());
    snd_soc_dapm_mutex_unlock(card);
    return 0;
    }
...
//
    ...
    let mut reg: c_int = mc.reg;
    let mut shift: c_uint = mc.shift;
    let mut max: c_int = mc.max;
    let mut width: c_uint = fls(max);
    let mut mask: c_uint = (1 << fls(max)) - 1;
    let mut invert: c_uint = mc.invert;
...
//
    ...
    let mut max: c_int = mc.max;
    let mut width: c_uint = fls(max);
    let mut mask: c_uint = (1 << width) - 1;
    let mut invert: c_uint = mc.invert;
    unsigned int val, rval = 0;
    int connect, rconnect = -1, change, reg_change = 0;
    let mut update: snd_soc_dapm_update = {};
    struct snd_soc_dapm_update *pupdate = core::ptr::null_mut();
    let mut ret: c_int = 0;
...
// set to sleep_state when initializing
    snd_soc_dapm_pinctrl_event(w, core::ptr::null_mut(), SND_SOC_DAPM_POST_PMD);
    break;
    case snd_soc_dapm_clock_supply:
...
//
    ...
    struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
    const struct snd_soc_pcm_stream *config = core::ptr::null_mut();
    struct snd_pcm_runtime *runtime = core::ptr::null_mut();
    unsigned int fmt;
    int ret;
//
// NOTE
...
// Be a little careful as we don't want to overflow the mask array
    ...
    let mut ret: c_int = 0, saved_stream = substream.stream;
    ...
    substream.runtime = core::ptr::null_mut();
    break;
    default:
    WARN(1, "Unknown event %d\n", event);
    ret = -EINVAL;
...
// Can't change the config when widget is already powered
    ...
    SOC_ENUM_SINGLE(0, 0, 0, core::ptr::null_mut()),
    };
    struct snd_kcontrol_new kcontrol_dai_link[] = {
    SOC_ENUM_EXT(core::ptr::null_mut(), w_param_enum[0],
    dapm_dai_link_get,
    dapm_dai_link_put),
    };
...
// duplicate kcontrol_dai_link on heap so that memory persists
    ...
    int num_kcontrols;
    const char **w_param_text;
    let mut private_value: c_ulong = 0;
    char *link_name;
    let mut ret: c_int = -ENOMEM;
...
// allocate memory for control, only in case of multiple configs
    w_param_text	= core::ptr::null_mut();
    kcontrol_news	= core::ptr::null_mut();
    num_kcontrols	= 0;
    if (rtd.dai_link.num_c2c_params > 1) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_link_dai_widgets(card: *mut snd_soc_card) -> c_int {
...
// ...find all widgets with the same stream and link them
    ...
    dev_dbg(dai.dev, "%s . %s\n", src.name, sink.name);
    dapm_add_path(w.dapm, src, sink, core::ptr::null_mut(), core::ptr::null_mut());
    }
    }
    return 0;
    }
    ...
    if (dai) {
    dapm_add_path(dapm, src, dai, core::ptr::null_mut(), core::ptr::null_mut());
    src = dai;
    }
    dapm_add_path(dapm, src, sink, core::ptr::null_mut(), core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn dapm_dai_stream_event(dai: *mut snd_soc_dai, stream: c_int, event: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_connect_dai_link_widgets(card: *mut snd_soc_card) {
...
#[no_mangle]
unsafe extern "C" fn dapm_stream_event(rtd: *mut snd_soc_pcm_runtime, stream: c_int, event: c_int) {
    ...
    dapm_power_widgets(rtd.card, event, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_enable_pin(dapm: *mut snd_soc_dapm_context, pin: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_soc_dapm_pin_has_prefix(card: *mut snd_soc_card, pin: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn dapm_shutdown(dapm: *mut snd_soc_dapm_context) {
    static void dapm_shutdown(struct snd_soc_dapm_context *dapm)
    {
    struct snd_soc_card *card = dapm.card;
    struct snd_soc_dapm_widget *w;
    LIST_HEAD(down_list);
    let mut powerdown: c_int = 0;
    snd_soc_dapm_mutex_lock_root(card);
    for_each_card_widgets(dapm.card, w) {
    if (w.dapm != dapm)
    continue;
