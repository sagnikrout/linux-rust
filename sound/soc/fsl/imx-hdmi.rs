...
//! Source: sound/soc/fsl/imx-hdmi.c
#![no_std]
...
pub struct imx_hdmi_data {
    ...
    let mut slot_width: u32 = data.cpu_priv.slot_width;
    int ret;
// MCLK always is (256 or 192) * rate.
    ...
    SND_SOC_DAPM_LINE("HDMI Jack", core::ptr::null_mut()),
    };
#[no_mangle]
...
// enable jack detection
    ...
    ret = snd_soc_component_set_jack(component, &data.hdmi_jack, core::ptr::null_mut());
    if (ret && ret != -ENOTSUPP) {
...
unsafe extern "C" fn imx_hdmi_probe(pdev: *mut platform_device) -> c_int {
    ...
    let mut hdmi_out: bool = of_property_read_bool(np, "hdmi-out");
    let mut hdmi_in: bool = of_property_read_bool(np, "hdmi-in");
