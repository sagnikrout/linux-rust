...
//! Source: sound/soc/fsl/fsl-asoc-card.c
#![no_std]
...
pub const CS427x_SYSCLK_MCLK: c_int = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct codec_priv {
    pub mclk_freq: c_ulong,
    pub free_freq: c_ulong,
    pub mclk_id: u32,
    pub fll_id: c_int,
    pub pll_id: c_int,
    pub pll_ratio_s24: c_int,
}
...
#[derive(Copy, Clone)]
pub struct cpu_priv {
    pub sysclk_freq: [c_ulong; 2],
    pub sysclk_dir: [u32; 2],
    pub sysclk_id: [u32; 2],
    pub sysclk_ratio: [u32; 2],
    pub slot_width: u32,
    pub slot_num: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fsl_asoc_card_pdata {
    pub sysclk_dir: [u32; 2],
    pub sysclk_ratio: [u32; 2],
    pub slot_width: u32,
    pub codec_dai_name: *const c_char,
    pub codec_mclk_id: u32,
    pub codec_fll_id: c_int,
    pub codec_pll_id: c_int,
    pub codec_pll_ratio_s24: c_int,
    pub has_pll: bool,
    ...
    pub exclude_format: u64,
    pub priv): *mut *mut int (codec_init)(struct fsl_asoc_card_priv,
    int (*probe_init)(struct device_node *codec_np[],
    struct device_node *cpu_np,
    const char *codec_dai_name[],
    pub priv): *mut fsl_asoc_card_priv,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fsl_asoc_card_priv {
    pub dai_link: [snd_soc_dai_link; 3],
    pub hp_jack: simple_util_jack,
    pub mic_jack: simple_util_jack,
    pub pdev: *mut platform_device,
    pub pdata: *const fsl_asoc_card_pdata,
    pub codec_priv: [codec_priv; 2],
    pub cpu_priv: cpu_priv,
    pub card: snd_soc_card,
    pub constraint_rates: *const snd_pcm_hw_constraint_list,
    pub constraint_channels: *const snd_pcm_hw_constraint_list,
    pub streams: u8,
    pub sample_rate: u32,
...
// 1st half -- Normal DAPM routes
    {"Playback",  core::ptr::null_mut(), "CPU-Playback"},
    {"CPU-Capture",  core::ptr::null_mut(), "Capture"},
// 2nd half -- ASRC DAPM routes
    {"CPU-Playback",  core::ptr::null_mut(), "ASRC-Playback"},
    {"ASRC-Capture",  core::ptr::null_mut(), "CPU-Capture"},
    };
    static const struct snd_soc_dapm_route audio_map_ac97[] = {
// 1st half -- Normal DAPM routes
    {"AC97 Playback",  core::ptr::null_mut(), "CPU AC97 Playback"},
    {"CPU AC97 Capture",  core::ptr::null_mut(), "AC97 Capture"},
// 2nd half -- ASRC DAPM routes
    {"CPU AC97 Playback",  core::ptr::null_mut(), "ASRC-Playback"},
    {"ASRC-Capture",  core::ptr::null_mut(), "CPU AC97 Capture"},
    };
...
// Add all possible widgets into here without being redundant
    ...
    SND_SOC_DAPM_LINE("Line Out Jack", core::ptr::null_mut()),
...
unsafe extern "C" fn fsl_asoc_card_is_ac97(priv: *mut fsl_asoc_card_priv) -> bool {
    ...
    let mut tx: bool = substream.stream == SNDRV_PCM_STREAM_PLAYBACK;
...
// DPCM Link between Front-End and Back-End (Optional)
    ...
    let mut int_ptcr: u32 = 0, ext_ptcr = 0;
...
// No DAPM routes with old bindings and dummy codec
    if (!codec_np[0]) {
    priv.card.dapm_routes = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn fsl_asoc_card_cs42888_codec_init(priv: *mut fsl_asoc_card_priv) -> c_int {
    static int fsl_asoc_card_cs42888_codec_init(struct fsl_asoc_card_priv *priv)
    {
    let mut mclk_freq: c_ulong = priv.codec_priv[0].mclk_freq;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(0: mclk_freq % 11289600 ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn fsl_asoc_card_wm8958_codec_init(priv: *mut fsl_asoc_card_priv) -> c_int {
...
// Read MCLK rate from each bound codec component
    for_each_rtd_codec_dais(rtd, codec_idx, codec_dai) {
    struct clk *codec_clk = clk_get(codec_dai.component.dev, core::ptr::null_mut());
    codec_priv = &priv.codec_priv[codec_idx];
    if (!IS_ERR(codec_clk)) {
...
unsafe extern "C" fn fsl_asoc_card_free_jack(card: *mut snd_soc_card) {
    ...
    priv.hp_jack.gpio.desc = core::ptr::null_mut();
    }
    ...
    priv.mic_jack.gpio.desc = core::ptr::null_mut();
    }
    }
//
...
unsafe extern "C" fn fsl_asoc_card_probe(pdev: *mut platform_device) -> c_int {
    ...
    struct platform_device *asrc_pdev = core::ptr::null_mut();
    struct device_node *bitclkprovider = core::ptr::null_mut();
    struct device_node *frameprovider = core::ptr::null_mut();
    struct fsl_asoc_card_priv *priv;
    const struct fsl_asoc_card_pdata *pdata;
    struct snd_soc_dai_link_component *dlc;
    const char *codec_dai_name[2] = { core::ptr::null_mut(), core::ptr::null_mut() };
...
// Format info from DT is optional.
    snd_soc_daifmt_parse_clock_provider_as_phandle(np, core::ptr::null_mut(), &bitclkprovider, &frameprovider);
    if (bitclkprovider || frameprovider) {
    let mut daifmt: c_uint = snd_soc_daifmt_parse_format(np, core::ptr::null_mut());
    let mut codec_bitclkprovider: bool = false;
    let mut codec_frameprovider: bool = false;
    for_each_link_codecs((&(priv.dai_link[0])), codec_idx, codec_comp) {
