...
//! Source: sound/soc/fsl/fsl_asrc_dma.c
#![no_std]
...
unsafe extern "C" fn fsl_asrc_dma_complete(arg: *mut c_void) {
    ...
    {
    let mut dir: u8 = substream.stream == SNDRV_PCM_STREAM_PLAYBACK ? OUT : IN;
    struct snd_pcm_runtime *runtime = substream.runtime;
    struct fsl_asrc_pair *pair = runtime.private_data;
...
// Prepare and submit Back-End DMA channel
    ...
    {
    let mut buswidth: enum dma_slave_buswidth = DMA_SLAVE_BUSWIDTH_2_BYTES;
    let mut be_peripheral_type: enum sdma_peripheral_type = IMX_DMATYPE_SSI;
    struct snd_soc_pcm_runtime *rtd = snd_soc_substream_to_rtd(substream);
    let mut tx: bool = substream.stream == SNDRV_PCM_STREAM_PLAYBACK;
    struct snd_dmaengine_dai_dma_data *dma_params_fe = core::ptr::null_mut();
    struct snd_dmaengine_dai_dma_data *dma_params_be = core::ptr::null_mut();
    struct snd_pcm_runtime *runtime = substream.runtime;
    struct fsl_asrc_pair *pair = runtime.private_data;
    struct dma_chan *tmp_chan = core::ptr::null_mut(), *be_chan = core::ptr::null_mut();
    struct snd_soc_component *component_be = core::ptr::null_mut();
    struct fsl_asrc *asrc = pair.asrc;
    let mut config_fe: dma_slave_config = {}, config_be = {};
    struct sdma_peripheral_config audio_config;
    let mut index: enum asrc_pair_index = pair.index;
    struct device *dev = component.dev;
...
// Do not flag to release if we are reusing the Back-End one
    ...
    return -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(8: width ==) -> else {
    else if (width == 8)
...
// release dev_to_dev chan if we aren't reusing the Back-End one
    ...
    dma_release_channel(pair.dma_chan[dir]);
    pair.dma_chan[!dir] = core::ptr::null_mut();
    pair.dma_chan[dir] = core::ptr::null_mut();
    return 0;
    }
    ...
    struct dma_chan *tmp_chan = core::ptr::null_mut();
    let mut dir: u8 = tx ? OUT : IN;
    let mut release_pair: bool = true;
    let mut ret: c_int = 0;
...
// Refine the snd_imx_hardware according to caps of DMA.
    ...
    asrc.pair[pair.index] = core::ptr::null_mut();
    kfree(pair);
    return 0;
    }
