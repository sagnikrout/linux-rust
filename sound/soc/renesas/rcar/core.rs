...
//! Source: sound/soc/renesas/rcar/core.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn rsnd_mod_make_sure(mod: *mut rsnd_mod, type: enum rsnd_mod_type) {
...
pub const MOD_NAME_NUM: c_int = 5;
pub const MOD_NAME_SIZE: c_int = 16;
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_mod_id_raw(mod: *mut rsnd_mod) -> c_int {
    int rsnd_mod_id_raw(struct rsnd_mod *mod)
    {
    return mod.id;
    }
#[no_mangle]
pub unsafe extern "C" fn rsnd_mod_id(mod: *mut rsnd_mod) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_mod_id_sub(mod: *mut rsnd_mod) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_mod_quit(mod: *mut rsnd_mod) {
    void rsnd_mod_quit(struct rsnd_mod *mod)
    {
    reset_control_assert(mod.rstc);
    mod.rstc = core::ptr::null_mut();
    clk_unprepare(mod.clk);
    mod.clk = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: runtime) -> else {
    else if (runtime)
    ...
    struct snd_pcm_hw_params *params)
    {
    let mut chan: c_int = rsnd_runtime_channel_original_with_params(io, params);
    struct rsnd_mod *ctu_mod = rsnd_io_to_mod_ctu(io);
    if (ctu_mod) {
    let mut converted_chan: u32 = rsnd_io_converted_chan(io);
//
// !! Note !!
//
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_channel_normalization(chan: c_int) -> c_int {
...
pub unsafe extern "C" fn rsnd_runtime_is_multi_ssi(io: *mut rsnd_dai_stream) -> c_int {
    ...
    {
    struct rsnd_dai *rdai = rsnd_io_to_rdai(io);
    let mut lane: c_int = rsnd_rdai_ssi_lane_get(rdai);
    int chan = rsnd_io_is_play(io) ?
...
//
#[no_mangle]
pub unsafe extern "C" fn rsnd_get_adinr_bit(mod: *mut rsnd_mod, io: *mut rsnd_dai_stream) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn rsnd_get_dalign(mod: *mut rsnd_mod, io: *mut rsnd_dai_stream) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_get_busif_shift(io: *mut rsnd_dai_stream, mod: *mut rsnd_mod) -> u32 {
    ...
    struct snd_pcm_runtime *runtime = rsnd_io_to_runtime(io);
    struct rsnd_mod *tmod = core::ptr::null_mut();
...
//
    ...
    int array_size)
    {
    let mut max: c_int = array ? array_size : RSND_MOD_MAX;
    for (; *iterator < max; (*iterator)++) {
    let mut type: enum rsnd_mod_type = (array) ? array[*iterator] : *iterator;
...
// PLAYBACK
    ...
    let mut mask: u32 = 0xF << shift;
    let mut val: u8 = (*status >> shift) & 0xF;
    let mut next_val: u8 = (val + add) & 0xF;
    let mut func_call: c_int = (val == timing);
// no status update
    if (add == 0 || shift == 28)
    return 1;
...
#[no_mangle]
unsafe extern "C" fn rsnd_dai_stream_quit(io: *mut rsnd_dai_stream) {
...
// TDM Extend Mode
    ...
    baseline.min, list[i], core::ptr::null_mut());
    if (rate > 0) {
    p.min = min(p.min, list[i]);
    p.max = max(p.max, list[i]);
    }
    rate = rsnd_ssi_clk_query(rdai,
    baseline.max, list[i], core::ptr::null_mut());
    if (rate > 0) {
    p.min = min(p.min, list[i]);
    p.max = max(p.max, list[i]);
    }
...
//
    ...
    let mut max_channels: c_uint = rsnd_rdai_channels_get(rdai);
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_node_fixed_index(dev: *mut device, node: *mut device_node, name: *mut c_char, idx: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_node_count(priv: *mut rsnd_priv, node: *mut device_node, name: *mut c_char) -> c_int {
...
//
pub const RSND_INDEXED_NAME_MAX: c_int = 32;
...
//
    ...
    return comma ? comma + 1 : core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn rsnd_dai_of_node(priv: *mut rsnd_priv, is_graph: *mut c_int) -> c_int {
...
unsafe extern "C" fn rsnd_soc_dai_pcm_new(rtd: *mut snd_soc_pcm_runtime, dai: *mut snd_soc_dai) -> c_int {
    ...
    struct rsnd_dai *rdai;
    struct device *dev = rsnd_priv_to_dev(priv);
    let mut playback_exist: c_int = 0, capture_exist = 0;
    int io_i;
...
#[no_mangle]
unsafe extern "C" fn rsnd_dai_probe(priv: *mut rsnd_priv) -> c_int {
...
//
    let mut k_up: c_int = 6;
    let mut k_down: c_int = 6;
    int channel;
...
//
    ...
    struct snd_pcm_substream *substream)
    {
    return rsnd_hw_update(substream, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn rsnd_kctrl_accept_anytime(io: *mut rsnd_dai_stream) -> c_int {
...
//
    ...
    let mut nr: c_int = priv.component_dais[i];
