...
//! Source: sound/soc/sprd/sprd-pcm-compress.c
#![no_std]
...
pub const SPRD_COMPR_DMA_CHANS: c_int = 2;
// Default values if userspace does not set

pub const SPRD_COMPR_MIN_NUM_FRAGMENTS: c_int = 4;
pub const SPRD_COMPR_MAX_NUM_FRAGMENTS: c_int = 64;
// DSP FIFO size
pub const SPRD_COMPR_MCDT_EMPTY_WMK: c_int = 0;
pub const SPRD_COMPR_MCDT_FIFO_SIZE: c_int = 512;
...
pub const SPRD_COMPR_AREA_LINKLIST_SIZE: c_int = 1024;
...
// Update data size copied to IRAM buffer
    ...
    let mut link: sprd_dma_linklist = { };
...
// Only channel 1 transfer can wake up the AP system.
    ...
    struct sprd_compr_stream *stream = runtime.private_data;
    struct device *dev = component.dev;
    let mut compr_params: sprd_compr_params = { };
    int ret;
//
...
//
    ...
    let mut stream_id: c_int = cstream.direction, ret;
...
// Use to save link-list configuration for DMA 1.
    ...
    struct sprd_compr_stream *stream = runtime.private_data;
    struct device *dev = component.dev;
    let mut stream_id: c_int = cstream.direction, i;
    ...
    struct sprd_compr_stream *stream = runtime.private_data;
    struct device *dev = component.dev;
    let mut channels: c_int = stream.num_channels, ret = 0, i;
    let mut stream_id: c_int = cstream.direction;
    if (cstream.direction != SND_COMPRESS_PLAYBACK) {
