...
//! Source: sound/soc/codecs/tas2781-i2c.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn partial_cali_data_update(reg: *mut c_int, j: c_int) -> c_int {
    ...
    let mut sum: c_int = ARRAY_SIZE(tas2781_cali_start_reg);
    unsigned char val[4];
    int j, r;
...
#[no_mangle]
unsafe extern "C" fn tas2781_calib_stop_put(priv: *mut tasdevice_priv) {
    ...
    let mut sum: c_int = ARRAY_SIZE(tas2563_cali_start_reg);
    int i, j;
...
#[no_mangle]
unsafe extern "C" fn tas2563_calib_stop_put(tas_priv: *mut tasdevice_priv) {
...
// First byte is the device index.
    ...
    (struct soc_bytes_ext *) kcontrol.private_value;
    unsigned char *dst = ucontrol.value.bytes.data;
    let mut reg: c_uint = TAS2781_RUNTIME_RE_REG_TF;
    ...
    (struct soc_bytes_ext *) kcontrol.private_value;
    unsigned char *dst = ucontrol.value.bytes.data;
    let mut reg: c_uint = TAS2781_RUNTIME_RE_REG;
    if (tas_priv.chip_id == TAS2781) {
    struct tasdevice_fw *tas_fmw = tas_priv.fmw;
    struct fct_param_address *p = &(tas_fmw.fct_par_addr);
    if (tas_priv.dspbin_typ)
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cali_data->cali_reg_array.r0_reg) -> else {
    ...
    (struct soc_bytes_ext *) kcontrol.private_value;
    unsigned char *dst = ucontrol.value.bytes.data;
    let mut reg: c_uint = TASDEVICE_XM_A1_REG;
    if (tas_priv.dspbin_typ)
    ...
    (struct soc_bytes_ext *) kcontrol.private_value;
    unsigned char *dst = ucontrol.value.bytes.data;
    let mut reg: c_uint = TASDEVICE_XM_A2_REG;
    if (tas_priv.dspbin_typ)
...
// find out the member same as or closer to the current volume
    ...
    struct snd_soc_component *codec = snd_kcontrol_chip(kcontrol);
    struct tasdevice_priv *tas_dev = snd_soc_component_get_drvdata(codec);
    let mut vol: c_int = ucontrol.value.integer.value[0];
    let mut status: c_int = 0, max = mc.max;
    int i, ret;
    let mut reg: c_uint = mc.reg;
...
// Read the primary device
    ...
    SND_SOC_BYTES_EXT("Amp TF Data", 6, tasdev_tf_data_get, core::ptr::null_mut()),
    SND_SOC_BYTES_EXT("Amp RE Data", 6, tasdev_re_data_get, core::ptr::null_mut()),
    SND_SOC_BYTES_EXT("Amp R0 Data", 6, tasdev_r0_data_get, core::ptr::null_mut()),
    SND_SOC_BYTES_EXT("Amp XMA1 Data", 6, tasdev_XMA1_data_get, core::ptr::null_mut()),
    SND_SOC_BYTES_EXT("Amp XMA2 Data", 6, tasdev_XMA2_data_get, core::ptr::null_mut()),
    };
    ...
    SND_SOC_BYTES_EXT("Amp Latch Data", 3, tas2781_latch_reg_get, core::ptr::null_mut()),
    };
...
//
    ...
    struct snd_soc_component *codec = snd_kcontrol_chip(kcontrol);
    struct tasdevice_priv *tas_priv = snd_soc_component_get_drvdata(codec);
    let mut user_prof_id: c_uint = ucontrol.value.integer.value[0];
...
#[no_mangle]
unsafe extern "C" fn tasdevice_create_control(tas_priv: *mut tasdevice_priv) -> c_int {
    static int tasdevice_create_control(struct tasdevice_priv *tas_priv)
    {
    struct snd_kcontrol_new *prof_ctrls;
    let mut nr_controls: c_int = 1;
    let mut mix_index: c_int = 0;
    int ret;
    char *name;
...
// Create a mixer item for selecting the capture profile
    ...
    struct snd_soc_component *codec = snd_kcontrol_chip(kcontrol);
    struct tasdevice_priv *tas_priv = snd_soc_component_get_drvdata(codec);
    let mut nr_program: c_uint = ucontrol.value.integer.value[0];
    let mut ret: c_int = 0;
    ...
    struct snd_soc_component *codec = snd_kcontrol_chip(kcontrol);
    struct tasdevice_priv *tas_priv = snd_soc_component_get_drvdata(codec);
    let mut nr_configuration: c_uint = ucontrol.value.integer.value[0];
    let mut ret: c_int = 0;
    ...
    struct snd_soc_component *codec = snd_kcontrol_chip(kcontrol);
    struct tasdevice_priv *tas_priv = snd_soc_component_get_drvdata(codec);
    let mut dev_id: c_int = ucontrol.value.integer.value[0];
    let mut max: c_int = tas_priv.ndev - 1;
    dev_id = clamp(dev_id, 0, max);
    guard(mutex)(&tas_priv.codec_lock);
    return tasdev_chn_switch(tas_priv, dev_id);
    }
#[no_mangle]
unsafe extern "C" fn tasdevice_dsp_create_ctrls(tas_priv: *mut tasdevice_priv) -> c_int {
    static int tasdevice_dsp_create_ctrls(struct tasdevice_priv *tas_priv)
    {
    struct snd_kcontrol_new *dsp_ctrls;
    char *active_dev_num, *chip_id, *fw_load;
    char *conf_name, *prog_name;
    let mut nr_controls: c_int = 5;
    let mut mix_index: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn tasdevice_create_cali_ctrls(priv: *mut tasdevice_priv) -> c_int {
...
//
    ...
    let mut max_pkg_len: c_uint = sizeof(*p);
    unsigned char *src;
    int j, len, reg, val;
    unsigned short chn;
    let mut ret: c_int = -1;
...
// If DSP FW fail, DSP kcontrol won't be created.
    ...
    {"SPK", core::ptr::null_mut(), "ASI"},
    {"OUT", core::ptr::null_mut(), "SPK"},
...
#[no_mangle]
unsafe extern "C" fn tasdevice_parse_dt(tas_priv: *mut tasdevice_priv) {
    ...
    "ti,audio-slots", core::ptr::null_mut(), 0);
    if (ndev <= 0) {
    ndev = 1;
    ...
    if (of_property_read_reg(np, i, &addr, core::ptr::null_mut()))
    break;
    dev_addrs[ndev++] = addr;
    }
