...
//! Source: sound/soc/codecs/tlv320aic32x4-clk.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct clk_aic32x4_pll_muldiv {
    pub p: u8,
    pub r: u16,
    pub j: u8,
    pub d: u16,
...
unsafe extern "C" fn clk_aic32x4_bdiv_get_parent(hw: *mut clk_hw) -> u8 {
    ...
    priv.dev = dev;
    priv.hw.init = &init;
    priv.regmap = dev_get_regmap(dev, core::ptr::null_mut());
    priv.reg = desc.reg;
    clk_hw_register_clkdev(&priv.hw, desc.name, devname);
    return devm_clk_register(dev, &priv.hw);
    }
#[no_mangle]
pub unsafe extern "C" fn aic32x4_register_clocks(dev: *mut device, mclk_name: *const c_char) -> c_int {
