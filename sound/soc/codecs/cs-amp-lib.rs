...
//! Source: sound/soc/codecs/cs-amp-lib.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cs_amp_time_now_in_windows_time() -> u64 {
    static u64 cs_amp_time_now_in_windows_time(void)
    {
    let mut time_in_100ns: u64 = div_u64(ktime_get_real_ns(), 100);
    ...
    let mut beval: __be32 = cpu_to_be32(val);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn cs_amp_convert_efi_status(status: efi_status_t) -> c_int {
    ...
    status = cs_amp_get_efi_variable(name, guid, core::ptr::null_mut(), &size, core::ptr::null_mut());
...
// Find EFI variable and get size
    ...
    attr, &data_size, core::ptr::null_mut());
    if (status == EFI_BUFFER_TOO_SMALL)
    break;
    }
    if (status != EFI_BUFFER_TOO_SMALL)
...
// This could be zero-filled space pre-allocated by the BIOS
    ...
    {
    struct cirrus_amp_efi_data *efi_data;
    struct cirrus_amp_cal_data *cal = core::ptr::null_mut();
    int i, ret;
    efi_data = cs_amp_get_cal_efi_buffer(dev, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    if (IS_ERR(efi_data))
    return PTR_ERR(efi_data);
    if (target_uid) {
    for (i = 0; i < efi_data.count; ++i) {
    let mut cal_target: u64 = cs_amp_cal_target_u64(&efi_data.data[i]);
// Skip empty entries
...
// Skip entries with unpopulated silicon ID
    ...
    let mut cal_target: u64 = cs_amp_cal_target_u64(&efi_data.data[amp_index]);
//
...
//
    ...
    {
    let mut cal_target: u64 = cs_amp_cal_target_u64(in_data);
...
//
    ...
    let mut new_data_size: c_uint = struct_size(data, data, num_entries);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cs_amp_spkid_efi {
    pub name: *mut efi_char16_t,
    pub guid: *mut efi_guid_t,
    pub values: [u8; 2],
}

#[no_mangle]
unsafe extern "C" fn cs_amp_get_efi_byte_spkid(dev: *mut device, info: *const cs_amp_spkid_efi) -> c_int {
    ...
    int i, ret;
    size = sizeof(spkid);
    status = cs_amp_get_efi_variable(info.name, info.guid, core::ptr::null_mut(), &size, &spkid);
...
// debugfs_lookup() can return NULL or ERR_PTR on error
    dir = debugfs_lookup("cirrus_logic", core::ptr::null_mut());
    if (!IS_ERR_OR_NULL(dir)) {
    created = debugfs_create_dir(dev_name(dev), dir);
    dput(dir);
    return created;
    }
    dir = debugfs_create_dir("cirrus_logic", core::ptr::null_mut());
