...
//! Source: sound/soc/sdw_utils/soc_sdw_utils.c
#![no_std]
...
//
    ...
    SND_SOC_DAPM_SPK("Left Spk", core::ptr::null_mut()),
    SND_SOC_DAPM_SPK("Right Spk", core::ptr::null_mut()),
    SND_SOC_DAPM_SPK("Left Spk2", core::ptr::null_mut()),
    SND_SOC_DAPM_SPK("Right Spk2", core::ptr::null_mut()),
    };
...
pub unsafe extern "C" fn asoc_sdw_rtd_init(rtd: *mut snd_soc_pcm_runtime) -> c_int {
    ...
    const char *spk_components = core::ptr::null_mut();
    int dai_index;
    int ret;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(is_sdw_slave(dai->component->dev->parent): dai->component->dev->parent &&) -> else {
...
//
    ...
    let mut ch: c_int = params_channels(params);
...
// skip itself
    ...
    int adr_index)
    {
    let mut adr: u64 = adr_link.adr_d[adr_index].adr;
    let mut sdw_version: c_uint = SDW_VERSION(adr);
    let mut link_id: c_uint = SDW_DISCO_LINK_ID(adr);
    let mut unique_id: c_uint = SDW_UNIQUE_ID(adr);
    let mut mfg_id: c_uint = SDW_MFG_ID(adr);
    let mut part_id: c_uint = SDW_PART_ID(adr);
    let mut class_id: c_uint = SDW_CLASS_ID(adr);
...
//
    ...
    bus_find_device_by_name(&sdw_bus_type, core::ptr::null_mut(), sdw_codec_name);
    if (!sdw_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn asoc_sdw_get_dai_type(type: u32) -> c_int {
...
// Make sure BIOS provides SDCA properties
    ...
    for (i = 0; i < slave.sdca_data.num_functions; i++) {
    let mut dai_type: c_int = asoc_sdw_get_dai_type(slave.sdca_data.function[i].type);
    ...
    int ret;
    for (adr_link = mach_params.links; adr_link.num_adr; adr_link++) {
    let mut num_link_dailinks: c_int = 0;
