...
//! Source: sound/soc/samsung/smdk_spdif.c
#![no_std]
...
unsafe extern "C" fn set_audio_clock_heirachy(pdev: *mut platform_device) -> c_int {
    ...
    let mut ret: c_int = 0;
    fout_epll = clk_get(core::ptr::null_mut(), "fout_epll");
    if (IS_ERR(fout_epll)) {
    printk(KERN_WARNING "%s: Cannot find fout_epll.\n",
    __func__);
    return -EINVAL;
    }
    mout_epll = clk_get(core::ptr::null_mut(), "mout_epll");
    if (IS_ERR(mout_epll)) {
    ...
    sclk_spdif = clk_get(core::ptr::null_mut(), "sclk_spdif");
...
#[no_mangle]
unsafe extern "C" fn smdk_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn smdk_exit() -> void __exit {
