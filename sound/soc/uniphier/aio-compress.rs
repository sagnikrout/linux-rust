...
//! Source: sound/soc/uniphier/aio-compress.c
#![no_std]
...
// Copyright (c) 2017-2018 Socionext Inc.
    ...
    struct snd_compr_stream *cstream);
#[no_mangle]
unsafe extern "C" fn uniphier_aio_comprdma_new(rtd: *mut snd_soc_pcm_runtime) -> c_int {
    ...
    let mut size: usize = AUD_RING_SIZE;
    let mut dma_dir: c_int = DMA_FROM_DEVICE, ret;
...
#[no_mangle]
unsafe extern "C" fn uniphier_aio_comprdma_free(rtd: *mut snd_soc_pcm_runtime) -> c_int {
    ...
    struct uniphier_aio *aio = uniphier_priv(snd_soc_rtd_to_cpu(rtd, 0));
    struct uniphier_aio_sub *sub = &aio.sub[compr.direction];
    let mut dma_dir: c_int = DMA_FROM_DEVICE;
    if (sub.swm.dir == PORT_DIR_OUTPUT)
    dma_dir = DMA_TO_DEVICE;
...
// IEC frame type will be changed after received valid data
    ...
    struct uniphier_aio *aio = uniphier_priv(snd_soc_rtd_to_cpu(rtd, 0));
    struct uniphier_aio_sub *sub = &aio.sub[cstream.direction];
    let mut bytes: c_int = runtime.fragment_size;
    int ret;
    ...
    struct uniphier_aio_sub *sub = &aio.sub[cstream.direction];
    struct device *dev = &aio.chip.pdev.dev;
    let mut bytes: c_int = runtime.fragment_size, ret = 0;
...
// Next data is Pc and Pd
    ...
    let mut pc: u16 = be16_to_cpu((__be16)frm_a);
...
// Force overwrite IEC frame type
    ...
    struct uniphier_aio *aio = uniphier_priv(snd_soc_rtd_to_cpu(rtd, 0));
    struct uniphier_aio_sub *sub = &aio.sub[cstream.direction];
    let mut cnt: usize = min_t(size_t, count, aio_rb_space_to_end(sub) / 2);
    let mut bytes: c_int = runtime.fragment_size;
    size_t s;
    int ret;
    if (cnt < sizeof(u32))
    return 0;
    if (sub.swm.dir == PORT_DIR_OUTPUT) {
    let mut dmapos: dma_addr_t = sub.compr_addr + sub.wr_offs;
// Size of AIO output format is double of IEC61937
    s = cnt * 2;
    ...
    let mut dmapos: dma_addr_t = sub.compr_addr + sub.rd_offs;
    s = cnt;
