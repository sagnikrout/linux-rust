...
pub unsafe extern "C" fn snd_soc_card_probe(card: *mut snd_soc_card) -> c_int {
    ...
    {
    if (card.probe) {
    let mut ret: c_int = card.probe(card);
    if (ret < 0)
    return soc_card_ret(card, ret);
//
...
pub unsafe extern "C" fn snd_soc_card_late_probe(card: *mut snd_soc_card) -> c_int {
    ...
    {
    if (card.late_probe) {
    let mut ret: c_int = card.late_probe(card);
    if (ret < 0)
    return soc_card_ret(card, ret);
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn snd_soc_card_set_topology_name(card: *mut snd_soc_card, prefix: *const c_char) {
