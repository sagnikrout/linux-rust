...
//! Source: sound/soc/loongson/loongson_card.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct loongson_card_data {
    pub snd_card: snd_soc_card,
    pub mclk_fs: c_uint,
    pub gpiod_hp_det: *mut gpio_desc,
    pub gpiod_hp_ctl: *mut gpio_desc,
    pub gpiod_spkr_en: *mut gpio_desc,
    pub cfg: *const loongson_card_config,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct loongson_card_config {
    pub fmt: c_uint,
    pub add_hp_jack: bool,
    pub add_dapm_widgets: bool,
    pub add_dapm_routes: bool,
}
...
#[no_mangle]
unsafe extern "C" fn loongson_asoc_machine_init(rtd: *mut snd_soc_pcm_runtime) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loongson_card_parse_acpi(data: *mut loongson_card_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loongson_card_parse_of(data: *mut loongson_card_data) -> c_int {
    ...
    for (i = 0; i < card.num_links; i++) {
    ret = snd_soc_of_get_dlc(cpu, core::ptr::null_mut(), loongson_dai_links[i].cpus, 0);
    if (ret < 0) {
    dev_err(dev, "getting cpu dlc error (%d)\n", ret);
    goto err;
    }
    loongson_dai_links[i].platforms.of_node = loongson_dai_links[i].cpus.of_node;
    ret = snd_soc_of_get_dlc(codec, core::ptr::null_mut(), loongson_dai_links[i].codecs, 0);
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn loongson_asoc_card_probe(pdev: *mut platform_device) -> c_int {
