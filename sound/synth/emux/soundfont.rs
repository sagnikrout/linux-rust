...
//! Source: sound/synth/emux/soundfont.c
#![no_std]
...
// open patch; create sf list
    ...
    sf = newsf(sflist, parm.type, core::ptr::null_mut());
    } else
    sf = newsf(sflist, parm.type, parm.name);
    if (sf == core::ptr::null_mut()) {
    return -ENOMEM;
    }
    scoped_guard(spinlock_irqsave, &sflist.lock) {
...
// prepend this record
    ...
    sflist.fonts = sf;
    sf.type = type;
    sf.zones = core::ptr::null_mut();
    sf.samples = core::ptr::null_mut();
    if (name)
...
//
    ...
    {
    scoped_guard(spinlock_irqsave, &sflist.lock) {
    sflist.currsf = core::ptr::null_mut();
...
// get the link info
    ...
    return -EINVAL;
    sf = newsf(sflist, SNDRV_SFNT_PAT_TYPE_MAP|SNDRV_SFNT_PAT_SHARED, core::ptr::null_mut());
    if (sf == core::ptr::null_mut())
    return -ENOMEM;
    prevp = core::ptr::null_mut();
...
pub const ABSCENT_RATIO: c_int = 1200;
pub const TIMECENT_RATIO: c_int = 1200;
pub const SAMPLERATE_RATIO: c_int = 4096;
//
// mHz to abscent
...
// delay time = 0x8000 - msec/92
    ...
    {
    let mut val: c_int = (0x7f * 92 - msec) / 92;
    if (val < 1) val = 1;
    if (val >= 126) val = 126;
    return val;
    }
// search an index for specified time from given time table
    ...
    {
    let mut left: c_int = 1, right = 127, mid;
    while (left < right) {
    mid = (left + right) / 2;
...
// load GUS patch
    ...
    return -EINVAL;
    sf = newsf(sflist, SNDRV_SFNT_PAT_TYPE_GUS|SNDRV_SFNT_PAT_SHARED, core::ptr::null_mut());
    if (sf == core::ptr::null_mut())
    return -ENOMEM;
...
// tremolo effect
    ...
    let mut rate: c_int = (patch.tremolo_rate * 1000 / 38) / 42;
    zone.v.parm.tremfrq = ((patch.tremolo_depth / 2) << 8) | rate;
    }
// vibrato effect
    if (patch.mode & WAVE_VIBRATO) {
    let mut rate: c_int = (patch.vibrato_rate * 1000 / 38) / 42;
...
// remove old zones
    ...
    zone = core::ptr::null_mut(); /* do not forget to clear this! */
    }
// prepend this zone
...
//
    ...
    sflist.currsf = core::ptr::null_mut();
    sflist.open_client = -1;
    sflist.fonts = core::ptr::null_mut();
    sflist.fonts_size = 0;
