...
//! Source: sound/synth/emux/emux_effect.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn if(parm_defs[type].high: effect >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(0x80: type &) -> else {
    else if (type & 0x80)
...
//
    ...
    emu = port.emu;
    fx = chan.private;
    if (emu == core::ptr::null_mut() || fx == core::ptr::null_mut())
    return;
...
//
    ...
    for (i = 0; i < p.chset.max_channels; i++)
    p.chset.channels[i].private = core::ptr::null_mut();
    }
    }
    void
