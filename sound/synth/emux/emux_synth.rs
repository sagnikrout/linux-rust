...
//! Source: sound/synth/emux/emux_synth.c
#![no_std]
...
// at this stage, we don't trigger the voice yet.
    if (table[i] == core::ptr::null_mut())
    continue;
    vp = emu.ops.get_voice(emu, port);
    if (vp == core::ptr::null_mut() || vp.ch < 0)
    continue;
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_emux_timer_callback(t: *mut timer_list) {
...
//
    ...
    emu.ops.terminate(vp);
    vp.time = emu.use_time++;
    vp.chan = core::ptr::null_mut();
    vp.port = core::ptr::null_mut();
    vp.zone = core::ptr::null_mut();
    vp.block = core::ptr::null_mut();
    vp.state = SNDRV_EMUX_ST_OFF;
...
//
    ...
    if (!STATE_IS_PLAYING(vp.state))
    return;
    if (vp.chan == core::ptr::null_mut() || vp.port == core::ptr::null_mut())
    return;
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_emux_lock_voice(emu: *mut snd_emux, voice: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_emux_unlock_voice(emu: *mut snd_emux, voice: c_int) {
