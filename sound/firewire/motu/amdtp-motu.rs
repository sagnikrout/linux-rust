...
//! Source: sound/firewire/motu/amdtp-motu.c
#![no_std]
...
pub const CIP_FMT_MOTU: c_uint = 0x02;
pub const CIP_FMT_MOTU_TX_V3: c_uint = 0x22;
pub const MOTU_FDF_AM824: c_uint = 0x22;
pub const TICKS_PER_CYCLE: c_int = 3072;
pub const CYCLES_PER_SECOND: c_int = 8000;

pub const CIP_SPH_CYCLE_SHIFT: c_int = 12;
pub const CIP_SPH_CYCLE_MASK: c_uint = 0x01fff000;
pub const CIP_SPH_OFFSET_MASK: c_uint = 0x00000fff;
//
// Nominally 3125 bytes/second, but the MIDI port's clock might be
// 1% too slow, and the bus clock 100 ppm too fast.
//
pub const MIDI_BYTES_PER_SECOND: c_int = 3093;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct amdtp_motu {
    pub pcm_chunks: c_uint,
    pub pcm_byte_offset: c_uint,
    pub midi: *mut snd_rawmidi_substream,
    pub midi_ports: c_uint,
    pub midi_flag_offset: c_uint,
    pub midi_byte_offset: c_uint,
    pub midi_db_count: c_int,
    pub midi_db_interval: c_uint,
    pub cache: *mut amdtp_motu_cache,
}
...
// alignment.
    ...
    {
    struct amdtp_motu *p = s.protocol;
    let mut channels: c_uint = p.pcm_chunks;
...
// frames |= be32_to_cpu(buffer[2]) >> 16;
    ...
    let mut cache_size: c_uint = cache.size;
    let mut cache_tail: c_uint = cache.tail;
    let mut base_tick: c_uint = cache.tx_cycle_count * TICKS_PER_CYCLE;
    int i;
    for (i = 0; i < data_blocks; ++i) {
    let mut sph: u32 = be32_to_cpu(*buf);
    unsigned int tick;
...
#[no_mangle]
pub unsafe extern "C" fn if(SND_MOTU_SPEC_COMMAND_DSP: motu->spec->flags &) -> else {
...
// For tracepoints.
    ...
    unsigned int *event_offsets = cache.event_offsets;
    let mut cache_size: c_uint = cache.size;
    let mut cache_head: c_uint = cache.head;
    let mut base_tick: c_uint = cache.rx_cycle_count * TICKS_PER_CYCLE;
    int i;
    for (i = 0; i < data_blocks; i++) {
    let mut tick: c_uint = (base_tick + event_offsets[cache_head]) % TICKS_PER_SECOND;
    let mut sph: u32 = ((tick / TICKS_PER_CYCLE) << CIP_SPH_CYCLE_SHIFT) | (tick % TICKS_PER_CYCLE);
...
// For tracepoints.
    ...
    let mut fmt: c_int = CIP_FMT_MOTU;
    let mut flags: c_uint = CIP_BLOCKING | CIP_UNAWARE_SYT;
    struct amdtp_motu *p;
    int err;
