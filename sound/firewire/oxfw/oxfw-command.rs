...
//! Source: sound/firewire/oxfw/oxfw-command.c
#![no_std]
...
// do transaction and check buf[1-8] are the same against command
    ...
    ;
#[no_mangle]
pub unsafe extern "C" fn if(10: err < len +) -> else {
    else if (err < len + 10)
    err = -EIO;
...
#[no_mangle]
pub unsafe extern "C" fn if(eid): (subfunc == 0xc1) && (buf[10] !=) -> else {
...
// do transaction and check buf[1-5] are the same against command
    ...
    ;
#[no_mangle]
pub unsafe extern "C" fn if(8: err <) -> else {
    else if (err < 8)
    err = -EIO;
    else if (buf[0] == 0x08)	/* NOT IMPLEMENTED */
    err = -ENXIO;
