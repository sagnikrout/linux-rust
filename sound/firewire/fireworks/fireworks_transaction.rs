...
//! Source: sound/firewire/fireworks/fireworks_transaction.c
#![no_std]
...
pub const MEMORY_SPACE_EFW_COMMAND: c_uint = 0xecc000000000ULL;
pub const MEMORY_SPACE_EFW_RESPONSE: c_uint = 0xecc080000000ULL;
pub const ERROR_RETRIES: c_int = 3;
pub const ERROR_DELAY_MS: c_int = 5;
pub const EFC_TIMEOUT_MS: c_int = 125;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct transaction_queue {
    pub list: list_head,
    pub unit: *mut fw_unit,
    pub buf: *mut c_void,
    pub size: c_uint,
    pub seqnum: u32,
    pub state: enum transaction_queue_state,
...
pub unsafe extern "C" fn snd_efw_transaction_add_instance(efw: *mut snd_efw) {
    ...
    guard(spinlock_irq)(&instances_lock);
    for (i = 0; i < SNDRV_CARDS; i++) {
    if (instances[i] != core::ptr::null_mut())
    continue;
    instances[i] = efw;
    break;
    }
