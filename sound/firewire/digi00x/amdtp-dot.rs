...
//! Source: sound/firewire/digi00x/amdtp-dot.c
#![no_std]
...
pub const CIP_FMT_AM: c_uint = 0x10;
// 'Clock-based rate control mode' is just supported.
pub const AMDTP_FDF_AM824: c_uint = 0x00;
//
...
//
pub const MAX_MIDI_RX_BLOCKS: c_int = 8;
// 3 = MAX(DOT_MIDI_IN_PORTS, DOT_MIDI_OUT_PORTS) + 1.
pub const MAX_MIDI_PORTS: c_int = 3;
//
...
pub struct amdtp_dot {
    ...
    pub midi_fifo_used: [c_int; MAX_MIDI_PORTS],
    pub midi_fifo_limit: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn dot_scrt(idx: u8, off: c_uint) -> u8 {
...
//
    ...
    let mut ln: u8 = idx & 0xf;
    let mut hn: u8 = (idx >> 4) & 0xf;
    let mut hr: u8 = (hn == 0x9) ? 0x9 : hir[(hio[hn] + off) % 15];
    if (len[ln] < off)
    return 0x00;
    return ((nib[14 + off - len[ln]]) | (hr << 4));
    }
#[no_mangle]
unsafe extern "C" fn dot_encode_step(state: *mut dot_state, buffer: *const *const __be32) {
    static void dot_encode_step(struct dot_state *state, __be32 *const buffer)
    {
    let mut data: *mut u8  const = (u8 *) buffer;
...
#[no_mangle]
unsafe extern "C" fn midi_ratelimit_per_packet(s: *mut amdtp_stream, port: c_uint) -> bool {
...
// This protocol delivers 24 bit data in 32bit data channel.
    ...
    let mut flags: c_uint = CIP_NONBLOCKING | CIP_UNAWARE_SYT;
