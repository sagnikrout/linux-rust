...
//! Source: sound/firewire/dice/dice-stream.c
#![no_std]
...
pub const NOTIFICATION_TIMEOUT_MS: c_int = 100;
#[repr(C)]
#[derive(Copy, Clone)]
...
#[no_mangle]
unsafe extern "C" fn select_clock(dice: *mut snd_dice, rate: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn release_resources(dice: *mut snd_dice) {
...
// register can be changed.
    ...
    {
    let mut max_speed: c_uint = fw_parent_device(dice.unit).max_speed;
    int i;
    int err;
    for (i = 0; i < params.count; i++) {
    struct amdtp_stream *stream;
...
pub unsafe extern "C" fn snd_dice_stream_start_duplex(dice: *mut snd_dice) -> c_int {
    ...
    {
    let mut generation: c_uint = dice.rx_resources[0].generation;
    struct reg_params tx_params, rx_params;
    unsigned int i;
    unsigned int rate;
    enum snd_dice_rate_mode mode;
...
#[no_mangle]
pub unsafe extern "C" fn snd_dice_stream_detect_current_formats(dice: *mut snd_dice) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dice_lock_changed(dice: *mut snd_dice) {
