...
//! Source: sound/hda/codecs/side-codecs/tas2781_hda_i2c.c
#![no_std]
...
pub const TAS2563_CAL_VAR_NAME_MAX: c_int = 16;
pub const TAS2563_CAL_ARRAY_SIZE: c_int = 80;
pub const TAS2563_CAL_DATA_SIZE: c_int = 4;
pub const TAS2563_MAX_CHANNELS: c_int = 4;
pub const TAS2563_CAL_CH_SIZE: c_int = 20;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tas2781_hda_i2c_priv {
    pub snd_ctls: [*mut snd_kcontrol; 2],
    pub h): *mut *mut int (save_calibration)(struct tas2781_hda,
    pub hda_chip_id: c_int,
}
...
unsafe extern "C" fn tas2781_get_i2c_res(ares: *mut acpi_resource, data: *mut c_void) -> c_int {
    ...
    let mut speakerid_gpios: static struct acpi_gpio_params = { 0, 0, false };
...
unsafe extern "C" fn tas2781_read_acpi(p: *mut tasdevice_priv, hid: *const c_char) -> c_int {
    ...
    uint32_t subid;
    int ret;
    adev = acpi_dev_get_first_match_dev(hid, core::ptr::null_mut(), -1);
    if (!adev) {
    dev_err(p.dev,
...
// Speaker id was needed for ASUS projects.
    ...
    "speakerid", 0, GPIOD_IN, core::ptr::null_mut());
...
unsafe extern "C" fn tas2563_save_calibration(h: *mut tas2781_hda) -> c_int {
    ...
    {
    let mut efi_guid: efi_guid_t = tasdev_fct_efi_guid[LENOVO];
    char *vars[TASDEV_CALIB_N] = {
    "R0_%d", "R0_Low_%d", "InvR0_%d", "Power_%d", "TLim_%d"
    };
    efi_char16_t efi_name[TAS2563_CAL_VAR_NAME_MAX];
    let mut max_size: c_ulong = TAS2563_CAL_DATA_SIZE;
...
unsafe extern "C" fn tasdev_fw_ready(fmw: *const firmware, context: *mut c_void) {
    ...
    comp.dev = core::ptr::null_mut();
    memset(comp.name, 0, sizeof(comp.name));
    comp.playback_hook = core::ptr::null_mut();
    }
...
// Init common setting for different audio profiles
    ...
    RUNTIME_PM_OPS(tas2781_runtime_suspend, tas2781_runtime_resume, core::ptr::null_mut())
