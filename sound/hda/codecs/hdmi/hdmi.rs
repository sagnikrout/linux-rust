...
//! Source: sound/hda/codecs/hdmi/hdmi.c
#![no_std]
...
//
    ...
    let mut enable_acomp: static bool = true;
...
#[no_mangle]
unsafe extern "C" fn hdmi_init_pin(codec: *mut hda_codec, pin_nid: hda_nid_t) {
...
#[no_mangle]
unsafe extern "C" fn eld_proc_new(per_pin: *mut hdmi_spec_per_pin, index: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn eld_proc_free(per_pin: *mut hdmi_spec_per_pin) {
    static void eld_proc_free(struct hdmi_spec_per_pin *per_pin)
    {
    if (!per_pin.codec.bus.shutdown) {
    snd_info_free_entry(per_pin.proc_entry);
    per_pin.proc_entry = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn eld_proc_free(per_pin: *mut hdmi_spec_per_pin) {
...
#[no_mangle]
unsafe extern "C" fn hdmi_debug_dip_size(codec: *mut hda_codec, pin_nid: hda_nid_t) {
...
#[no_mangle]
unsafe extern "C" fn hdmi_clear_dip_buffers(codec: *mut hda_codec, pin_nid: hda_nid_t) {
...
#[no_mangle]
unsafe extern "C" fn hdmi_checksum_audio_infoframe(hdmi_ai: *mut hdmi_audio_infoframe) {
...
//
    ...
    let mut pin_nid: hda_nid_t = per_pin.pin_nid;
    let mut dev_id: c_int = per_pin.dev_id;
    let mut channels: c_int = per_pin.channels;
    int active_channels;
    struct hdmi_eld *eld;
    int ca;
    if (!channels)
...
//
    ...
    {
    struct hdmi_spec *spec = codec.spec;
    let mut pin_idx: c_int = pin_id_to_pin_index(codec, nid, dev_id);
    if (pin_idx < 0)
    return;
    guard(mutex)(&spec.pcm_lock);
...
#[no_mangle]
unsafe extern "C" fn hdmi_non_intrinsic_event(codec: *mut hda_codec, res: c_uint) {
    static void hdmi_non_intrinsic_event(struct hda_codec *codec, unsigned int res)
    {
    let mut tag: c_int = res >> AC_UNSOL_RES_TAG_SHIFT;
    let mut subtag: c_int = (res & AC_UNSOL_RES_SUBTAG) >> AC_UNSOL_RES_SUBTAG_SHIFT;
    let mut cp_state: c_int = !!(res & AC_UNSOL_RES_CP_STATE);
    let mut cp_ready: c_int = !!(res & AC_UNSOL_RES_CP_READY);
    codec_info(codec,
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_unsol_event(codec: *mut hda_codec, res: c_uint) {
...
//
    ...
    struct hdmi_spec *spec = codec.spec;
    struct hdmi_spec_per_pin *per_pin;
    struct hdmi_spec_per_cvt *per_cvt = core::ptr::null_mut();
...
// called in hdmi_pcm_open when no pin is assigned to the PCM
    ...
    pin_cvt_fixup(codec, core::ptr::null_mut(), per_cvt.cvt_nid);
...
//
#[no_mangle]
unsafe extern "C" fn hdmi_read_pin_conn(codec: *mut hda_codec, pin_idx: c_int) -> c_int {
...
//
    ...
    let mut old_eld_valid: bool = pin_eld.eld_valid;
    bool eld_changed;
    int pcm_idx;
    if (eld.eld_valid) {
    if (eld.eld_size <= 0 ||
...
#[no_mangle]
pub unsafe extern "C" fn if(!monitor_next: monitor_prev &&) -> else {
    else if (monitor_prev && !monitor_next)
    silent_stream_disable(codec, per_pin);
    }
    }
#[no_mangle]
unsafe extern "C" fn hdmi_present_sense(per_pin: *mut hdmi_spec_per_pin, repoll: c_int) {
    static void hdmi_present_sense(struct hdmi_spec_per_pin *per_pin, int repoll)
    {
    struct hda_codec *codec = per_pin.codec;
    if (!codec_has_acomp(codec))
...
#[no_mangle]
unsafe extern "C" fn hdmi_add_pin(codec: *mut hda_codec, pin_nid: hda_nid_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hdmi_add_cvt(codec: *mut hda_codec, cvt_nid: hda_nid_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_parse_codec(codec: *mut hda_codec) -> c_int {
...
//
    for (i = 0; i < nodes; i++) {
    let mut nid: hda_nid_t = start_nid + i;
...
//
#[no_mangle]
unsafe extern "C" fn check_non_pcm_per_cvt(codec: *mut hda_codec, cvt_nid: hda_nid_t) -> bool {
    static bool check_non_pcm_per_cvt(struct hda_codec *codec, hda_nid_t cvt_nid)
    {
...
//
    ...
    struct snd_pcm_substream *substream)
    {
    let mut cvt_nid: hda_nid_t = hinfo.nid;
    struct hdmi_spec *spec = codec.spec;
    int pin_idx;
    struct hdmi_spec_per_pin *per_pin;
    struct snd_pcm_runtime *runtime = substream.runtime;
...
//
    pin_cvt_fixup(codec, core::ptr::null_mut(), cvt_nid);
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_build_pcms(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn generic_hdmi_build_jack(codec: *mut hda_codec, pcm_idx: c_int) -> c_int {
    static int generic_hdmi_build_jack(struct hda_codec *codec, int pcm_idx)
    {
    char hdmi_str[32] = "HDMI/DP";
    struct hdmi_spec *spec = codec.spec;
    struct snd_jack *jack;
    let mut pcmdev: c_int = get_pcm_rec(spec, pcm_idx).device;
    int err;
    if (pcmdev > 0)
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_build_controls(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_init_per_pins(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_init(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hdmi_array_init(spec: *mut hdmi_spec, nums: c_int) {
...
#[no_mangle]
unsafe extern "C" fn hdmi_array_free(spec: *mut hdmi_spec) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_spec_free(codec: *mut hda_codec) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_remove(codec: *mut hda_codec) {
    ...
    snd_hdac_acomp_register_notifier(&codec.bus.core, core::ptr::null_mut());
    }
    ...
    for (pcm_idx = 0; pcm_idx < spec.pcm_used; pcm_idx++) {
    if (spec.pcm_rec[pcm_idx].jack == core::ptr::null_mut())
    continue;
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_suspend(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_resume(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_alloc(codec: *mut hda_codec) -> c_int {
...
// generic HDMI parser
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_generic_probe(codec: *mut hda_codec) -> c_int {
...
//
    let mut val: c_uint = use_acomp ? 0 : (AC_USRSP_EN | tbl.tag);
...
#[no_mangle]
unsafe extern "C" fn match_bound_vga(dev: *mut device, subtype: c_int, data: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_hda_hdmi_acomp_pin_eld_notify(audio_ptr: *mut c_void, port: c_int, dev_id: c_int) {
    void snd_hda_hdmi_acomp_pin_eld_notify(void *audio_ptr, int port, int dev_id)
    {
    struct hda_codec *codec = audio_ptr;
    struct hdmi_spec *spec = codec.spec;
    let mut pin_nid: hda_nid_t = spec.port2pin(codec, port);
    if (!pin_nid)
    return;
