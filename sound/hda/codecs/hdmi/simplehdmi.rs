...
//! Source: sound/hda/codecs/hdmi/simplehdmi.c
#![no_std]
...
unsafe extern "C" fn simple_hdmi_build_jack(codec: *mut hda_codec) -> c_int {
    ...
    let mut pcmdev: c_int = pcmp.pcm.device;
    int err;
    if (pcmdev > 0)
...
pub unsafe extern "C" fn snd_hda_hdmi_simple_init(codec: *mut hda_codec) -> c_int {
    ...
    let mut pin: hda_nid_t = per_pin.pin_nid;
...
pub const VIAHDMI_CVT_NID: c_uint = 0x02	/* audio converter1 */;
pub const VIAHDMI_PIN_NID: c_uint = 0x03	/* HDMI output pin1 */;
