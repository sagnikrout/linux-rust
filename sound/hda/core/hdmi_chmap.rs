...
//! Source: sound/hda/core/hdmi_chmap.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn init_channel_allocations() {
...
#[no_mangle]
unsafe extern "C" fn get_channel_allocation_order(ca: c_int) -> c_int {
...
// fill the rest of the slots with ALSA channel 0xf
    ...
    for (i = 0; i < 8; i++) {
    let mut slotsetup: c_int = non_pcm ? non_pcm_mapping[i] : hdmi_channel_mapping[ca][i];
    let mut hdmi_slot: c_int = slotsetup & 0x0f;
    let mut channel: c_int = (slotsetup & 0xf0) >> 4;
...
#[no_mangle]
pub unsafe extern "C" fn snd_hdac_chmap_to_spk_mask(c: c_uchar) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn to_cea_slot(ordered_ca: c_int, pos: c_uchar) -> c_int {
    static int to_cea_slot(int ordered_ca, unsigned char pos)
    {
    let mut mask: c_int = snd_hdac_chmap_to_spk_mask(pos);
    int i;
...
#[no_mangle]
unsafe extern "C" fn from_cea_slot(ordered_ca: c_int, slot: c_uchar) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hdmi_manual_channel_allocation(chs: c_int, map: *mut c_uchar) -> c_int {
...
// set up the channel slots for the given ALSA API channel map
    ...
    int ca)
    {
    let mut ordered_ca: c_int = get_channel_allocation_order(ca);
...
#[no_mangle]
unsafe extern "C" fn hdmi_setup_fake_chmap(map: *mut c_uchar, ca: c_int) {
...
// all channels are remappable freely
    ...
    int c;
    for (c = 7; c >= 0; c--) {
    let mut spk: c_int = cap.speakers[c];
    if (!spk)
    continue;
...
#[no_mangle]
unsafe extern "C" fn spk_mask_from_spk_alloc(spk_alloc: c_int) -> c_int {
    static int spk_mask_from_spk_alloc(int spk_alloc)
    {
    int i;
    let mut spk_mask: c_int = eld_speaker_allocation_bits[0];
    for (i = 0; i < ARRAY_SIZE(eld_speaker_allocation_bits); i++) {
    if (spk_alloc & (1 << i))
    spk_mask |= eld_speaker_allocation_bits[i];
    }
    ...
    struct snd_pcm_chmap *info = snd_kcontrol_chip(kcontrol);
    struct hdac_chmap *chmap = info.private_data;
    let mut pcm_idx: c_int = kcontrol.private_value;
...
//
    ...
    core::ptr::null_mut(), 0, pcm_idx, &chmap);
    if (err < 0)
    return err;
// override handlers
