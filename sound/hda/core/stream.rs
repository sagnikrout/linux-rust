...
//! Source: sound/hda/core/stream.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn snd_hdac_stream_clear(azx_dev: *mut hdac_stream) {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hdac_stream_setup(azx_dev: *mut hdac_stream, code_loading: bool) -> c_int {
...
//
    ...
    struct hdac_stream *res = core::ptr::null_mut();
...
pub unsafe extern "C" fn snd_hdac_stream_release_locked(azx_dev: *mut hdac_stream) {
    ...
    azx_dev.opened = 0;
    azx_dev.running = 0;
    azx_dev.substream = core::ptr::null_mut();
    }
...
// one BDLE cannot cross 4K boundary on CTHDA chips
    ...
    let mut remain: u32 = 0x1000 - (ofs & 0xfff);
    if (chunk > remain)
    chunk = remain;
    }
...
//
    ...
    let mut mask: u32 = 0;
    if (!bus.spbcap) {
    dev_err(bus.dev, "Address of SPB capability is core::ptr::null_mut()\n");
    return;
    }
    mask |= (1 << index);
    if (enable)
...
//
    ...
    let mut mask: u32 = 0;
    if (!bus.drsmcap) {
    dev_err(bus.dev, "Address of DRSM capability is core::ptr::null_mut()\n");
    return;
    }
    mask |= (1 << index);
    if (enable)
...
// It is recommended to transfer the firmware in two or more chunks.
    ...
    err = snd_hdac_stream_setup_bdle(azx_dev, bufp, core::ptr::null_mut());
    if (err < 0)
    goto error;
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hdac_dsp_trigger(azx_dev: *mut hdac_stream, start: bool) {
