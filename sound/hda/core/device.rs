...
//! Source: sound/hda/core/device.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hdac_device_set_chip_name(codec: *mut hdac_device, name: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hdac_codec_modalias(codec: *const hdac_device, buf: *mut c_char, size: usize) -> c_int {
...
//
    ...
    {
    let mut cmd: c_uint = snd_hdac_make_cmd(codec, nid, verb, parm);
...
//
    ...
    {
    let mut verb: c_uint = (AC_VERB_PARAMETERS << 8) | (nid << 20) | parm;
    int err;
    if (!codec.regmap)
    return -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn setup_fg_nodes(codec: *mut hdac_device) {
    static void setup_fg_nodes(struct hdac_device *codec)
    {
...
pub unsafe extern "C" fn snd_hdac_refresh_widgets(codec: *mut hdac_device) -> c_int {
    ...
    {
    let mut fg: hda_nid_t = codec.afg ? codec.afg : codec.mfg;
    hda_nid_t start_nid;
    int nums, err = 0;
//
...
#[no_mangle]
unsafe extern "C" fn get_num_conns(codec: *mut hdac_device, nid: hda_nid_t) -> c_uint {
    static unsigned int get_num_conns(struct hdac_device *codec, hda_nid_t nid)
    {
    let mut wcaps: c_uint = snd_hdac_get_wcaps(codec, nid);
    unsigned int parm;
...
pub unsafe extern "C" fn snd_hdac_keep_power_up(codec: *mut hdac_device) -> c_int {
    ...
    {
    if (!atomic_inc_not_zero(&codec.in_pm)) {
    let mut ret: c_int = pm_runtime_get_if_active(&codec.dev);
    if (!ret)
    return -1;
    if (ret < 0)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn get_codec_vendor_name(codec: *mut hdac_device) -> c_int {
    static int get_codec_vendor_name(struct hdac_device *codec)
    {
    const struct hda_vendor_id *c;
    let mut vendor_id: u16 = codec.vendor_id >> 16;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hda_rate_tbl {
    pub hz: c_uint,
    pub alsa_bits: c_uint,
    pub hda_fmt: c_uint,
}
...
pub const AC_PAR_PCM_RATE_BITS: c_int = 11;
...
#[no_mangle]
unsafe extern "C" fn snd_hdac_format_normalize(format: snd_pcm_format_t) -> snd_pcm_format_t {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hdac_stream_format(channels: c_uint, bits: c_uint, rate: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn query_pcm_param(codec: *mut hdac_device, nid: hda_nid_t) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn query_stream_param(codec: *mut hdac_device, nid: hda_nid_t) -> c_uint {
    static unsigned int query_stream_param(struct hdac_device *codec, hda_nid_t nid)
    {
    let mut streams: c_uint = snd_hdac_read_parm(codec, nid, AC_PAR_STREAM);
...
//
    ...
    {
    int i;
    let mut val: c_uint = 0, rate, stream;
    val = query_pcm_param(codec, nid);
    if (!val)
    return false;
    rate = format & 0xff00;
...
// FIXME: check for float32 and AC3?
    ...
    {
    let mut cmd: c_uint = snd_hdac_make_cmd(hdac, nid, verb, parm);
    unsigned int res;
    ...
    {
    let mut cmd: c_uint = snd_hdac_make_cmd(hdac, nid, verb, parm);
    return snd_hdac_exec_verb(hdac, cmd, flags, core::ptr::null_mut());
    }
//
...
//
    ...
    {
    let mut end_time: c_ulong = jiffies + msecs_to_jiffies(500);
