...
//! Source: sound/hda/core/regmap.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn codec_pm_lock(codec: *mut hdac_device) -> c_int {
    static int codec_pm_lock(struct hdac_device *codec)
    {
    return snd_hdac_keep_power_up(codec);
    }
#[no_mangle]
unsafe extern "C" fn codec_pm_unlock(codec: *mut hdac_device, lock: c_int) {
...
#[no_mangle]
unsafe extern "C" fn hda_volatile_reg(dev: *mut device, reg: c_uint) -> bool {
    static bool hda_volatile_reg(struct device *dev, unsigned int reg)
    {
    struct hdac_device *codec = dev_to_hdac_dev(dev);
    let mut verb: c_uint = get_verb(reg);
    switch (verb) {
...
#[no_mangle]
unsafe extern "C" fn hda_writeable_reg(dev: *mut device, reg: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hda_readable_reg(dev: *mut device, reg: c_uint) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn is_stereo_amp_verb(reg: c_uint) -> bool {
...
// write a pseudo stereo amp register (16bit left+right)
    ...
    if (left == right) {
    reg |= AC_AMP_SET_LEFT | AC_AMP_SET_RIGHT;
    return snd_hdac_exec_verb(codec, reg | left, 0, core::ptr::null_mut());
    }
    err = snd_hdac_exec_verb(codec, reg | AC_AMP_SET_LEFT | left, 0, core::ptr::null_mut());
    if (err < 0)
    return err;
    err = snd_hdac_exec_verb(codec, reg | AC_AMP_SET_RIGHT | right, 0, core::ptr::null_mut());
    if (err < 0)
    return err;
    return 0;
    }
...
// LSB 8bit = coef index
    ...
    err = snd_hdac_exec_verb(codec, verb, 0, core::ptr::null_mut());
    if (err < 0)
    return err;
...
// LSB 8bit = coef index
    ...
    (val & 0xffff);
    return snd_hdac_exec_verb(codec, verb, 0, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn hda_reg_read(context: *mut c_void, reg: c_uint, val: *mut c_uint) -> c_int {
    static int hda_reg_read(void *context, unsigned int reg, unsigned int *val)
    {
    struct hdac_device *codec = context;
    let mut verb: c_int = get_verb(reg);
    int err;
...
#[no_mangle]
unsafe extern "C" fn hda_reg_write(context: *mut c_void, reg: c_uint, val: c_uint) -> c_int {
    ...
    err = snd_hdac_exec_verb(codec, reg, 0, core::ptr::null_mut());
    if (err < 0)
    goto out;
    }
    out:
...
pub unsafe extern "C" fn snd_hdac_regmap_init(codec: *mut hdac_device) -> c_int {
    ...
    regmap = regmap_init(&codec.dev, core::ptr::null_mut(), codec, &hda_regmap_cfg);
    if (IS_ERR(regmap))
...
pub unsafe extern "C" fn snd_hdac_regmap_exit(codec: *mut hdac_device) {
    ...
    codec.regmap = core::ptr::null_mut();
