...
//! Source: sound/hda/controllers/cix-ipbloq.c
#![no_std]
...
pub const CIX_IPBLOQ_JACKPOLL_DEFAULT_TIME_MS: c_int = 1000;
pub const CIX_IPBLOQ_POWER_SAVE_DEFAULT_TIME_MS: c_int = 100;
...
#[no_mangle]
unsafe extern "C" fn cix_ipbloq_hda_probe_codec(hda: *mut cix_ipbloq_hda) -> c_int {
...
// create codec instances
    ...
    {
    const char *sname = core::ptr::null_mut(), *drv_name = "cix-ipbloq-hda";
    struct hdac_bus *bus = azx_bus(chip);
...
// shortname for card
    sname = of_get_property(pdev.dev.of_node, "model", core::ptr::null_mut());
    if (!sname)
    sname = drv_name;
...
unsafe extern "C" fn cix_ipbloq_hda_probe(pdev: *mut platform_device) -> c_int {
    ...
    {
    let mut driver_flags: c_uint = AZX_DCAPS_PM_RUNTIME;
    ...
    return -ENOMEM;
    hda.dev = &pdev.dev;
    hda.reset = devm_reset_control_get(hda.dev, core::ptr::null_mut());
    if (IS_ERR(hda.reset))
...
unsafe extern "C" fn cix_ipbloq_hda_runtime_resume(dev: *mut device) -> c_int {
    ...
    cix_ipbloq_hda_runtime_resume, core::ptr::null_mut())
    };
