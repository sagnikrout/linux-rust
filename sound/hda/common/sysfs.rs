...
//! Source: sound/hda/common/sysfs.c
#![no_std]
...
pub struct hda_hint {
    ...
    pub /: *const *const *const char val; / contained in the same alloc as key,
}
...
//
#[no_mangle]
unsafe extern "C" fn clear_codec(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn reconfig_codec(codec: *mut hda_codec) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parse_init_verbs(codec: *mut hda_codec, buf: *const c_char) -> c_int {
    ...
    {
    struct hda_codec *codec = dev_get_drvdata(dev);
    let mut err: c_int = parse_init_verbs(codec, buf);
    if (err < 0)
    return err;
    return count;
    }
...
#[no_mangle]
unsafe extern "C" fn remove_trail_spaces(str: *mut c_char) {
...
pub const MAX_HINTS: c_int = 1024;
#[no_mangle]
unsafe extern "C" fn parse_hints(codec: *mut hda_codec, buf: *const c_char) -> c_int {
...
// allocate a new hint entry
    ...
    {
    struct hda_codec *codec = dev_get_drvdata(dev);
    let mut err: c_int = parse_hints(codec, buf);
    if (err < 0)
    return err;
    return count;
    }
...
#[no_mangle]
unsafe extern "C" fn parse_user_pin_configs(codec: *mut hda_codec, buf: *const c_char) -> c_int {
    ...
    {
    struct hda_codec *codec = dev_get_drvdata(dev);
    let mut err: c_int = parse_user_pin_configs(codec, buf);
    if (err < 0)
    return err;
    return count;
    }
...
//
    ...
    return hint ? hint.val : core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hda_get_bool_hint(codec: *mut hda_codec, key: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hda_get_int_hint(codec: *mut hda_codec, key: *const c_char, valp: *mut c_int) -> c_int {
    ...
    return -ENOENT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: kstrtoul(p, _arg: 0, _arg: &val)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn strmatch(a: *const c_char, b: *const c_char) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hda_patch_item {
    pub tag: *const c_char,
    pub alias: *const c_char,
    pub retc): *mut *mut *mut *mut void (parser)(char buf, struct hda_bus bus, struct hda_codec,
}
...
#[no_mangle]
unsafe extern "C" fn parse_line_mode(buf: *mut c_char, bus: *mut hda_bus) -> c_int {
...
//
    ...
    {
    int len;
    let mut fw_size: usize = *fw_size_p;
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hda_load_patch(bus: *mut hda_bus, fw_size: usize, fw_buf: *const c_void) -> c_int {
