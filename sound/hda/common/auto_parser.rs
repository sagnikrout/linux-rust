...
//! Source: sound/hda/common/auto_parser.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn is_in_nid_list(nid: hda_nid_t, list: *const hda_nid_t) -> c_int {
...
//
    ...
    {
    int i;
    sort(list, num_pins, sizeof(list[0]), compare_seq, core::ptr::null_mut());
...
// If has both hs_mic and hp_mic, pick the hs_mic ahead of hp_mic.
    ...
    return -1; /* don't swap */
#[no_mangle]
pub unsafe extern "C" fn if(b->is_headset_mic: a->is_headphone_mic &&) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn reorder_outputs(nums: c_uint, pins: *mut hda_nid_t) {
...
// check whether the given pin has a proper pin I/O capability bit
    ...
    unsigned int dev)
    {
    let mut pincap: c_uint = snd_hda_query_pin_caps(codec, pin);
// some old hardware don't return the proper pincaps
    ...
    return false;
    if (seq_number >= 0) {
    let mut seq: c_int = get_defcfg_sequence(def_conf);
    if (seq != seq_number)
    return false;
    }
    return true;
...
//
    ...
    let mut wid_caps: c_uint = get_wcaps(codec, nid);
    let mut wid_type: c_uint = get_wcaps_type(wid_caps);
    unsigned int def_conf;
...
#[no_mangle]
pub unsafe extern "C" fn if(assoc: assoc_line_out !=) -> else {
    codec_info(codec,
...
// Find a pin that could be a headset or headphone mic
    ...
    let mut hsmic: bool = !!(cond_flags & HDA_PINCFG_HEADSET_MIC);
    let mut hpmic: bool = !!(cond_flags & HDA_PINCFG_HEADPHONE_MIC);
...
// sort inputs in the order of AUTO_PIN_* type
    ...
    compare_input_type, core::ptr::null_mut());
//
...
pub unsafe extern "C" fn snd_hda_get_input_pin_attr(def_conf: c_uint) -> c_int {
    ...
    {
    let mut loc: c_uint = get_defcfg_location(def_conf);
    let mut conn: c_uint = get_defcfg_connect(def_conf);
    if (conn == AC_JACK_PORT_NONE)
...
//
    ...
    int input)
    {
    let mut type: c_int = cfg.inputs[input].type;
    let mut has_multiple_pins: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn find_idx_in_nid_list(nid: hda_nid_t, list: *const hda_nid_t, nums: c_int) -> c_int {
...
// get a unique suffix
    ...
    let mut attr: c_int = snd_hda_get_input_pin_attr(def_conf);
// check the location
    switch (attr) {
...
//
    ...
    if (!name)
    name = hda_get_input_pin_label(codec, core::ptr::null_mut(), nid, true);
    break;
    }
    if (!name)
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn __snd_hda_apply_fixup(codec: *mut hda_codec, id: c_int, action: c_int, depth: c_int) {
...
pub unsafe extern "C" fn snd_hda_apply_fixup(codec: *mut hda_codec, action: c_int) {
    ...
    let mut nid: hda_nid_t = pin.nid;
    let mut cfg: u32 = pin.cfg;
    ...
    break;
#[no_mangle]
pub unsafe extern "C" fn if(0x40000000: (cfg & 0xf0000000) == 0x40000000 && (t_pins->val & 0xf0000000) ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn hda_quirk_match(vendor: u16, device: u16, q: *const hda_quirk) -> bool {
...
//
    ...
    {
    const struct hda_quirk *q;
    let mut id: c_int = HDA_FIXUP_ID_NOT_SET;
    const char *name = core::ptr::null_mut();
    const char *type = core::ptr::null_mut();
    unsigned int vendor, device;
    let mut pci_vendor: u16 = 0, pci_device = 0;
