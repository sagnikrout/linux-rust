...
//! Source: sound/hda/common/proc.c
#![no_std]
...
//
    ...
    let mut dump_coef: static int = -1;
...
// always use noncached version
    ...
    if (cpcm.stream[type].nid != nid || cpcm.pcm == core::ptr::null_mut())
    continue;
...
#[no_mangle]
unsafe extern "C" fn print_pcm_rates(buffer: *mut snd_info_buffer, pcm: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn print_pcm_bits(buffer: *mut snd_info_buffer, pcm: c_uint) {
    ...
    struct hda_codec *codec, hda_nid_t nid)
    {
    let mut pcm: c_uint = param_read(codec, nid, AC_PAR_PCM);
    let mut stream: c_uint = param_read(codec, nid, AC_PAR_STREAM);
    if (pcm == -1 || stream == -1) {
    snd_iprintf(buffer, "N/A\n");
    return;
    }
    ...
    "DIN", "XLR", "RJ11", "Comb",
    core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), "Other"
    };
    ...
    "Purple", "Pink", core::ptr::null_mut(), core::ptr::null_mut(),
    core::ptr::null_mut(), core::ptr::null_mut(), "White", "Other",
    };
...
//
    ...
    snd_iprintf(buffer, " HP");
    if (supports_vref) {
    let mut vref: c_int = pinctls & AC_PINCTL_VREFEN;
    switch (vref) {
    ...
    struct hda_codec *codec, hda_nid_t nid)
    {
    let mut cap: c_uint = param_read(codec, nid, AC_PAR_VOL_KNB_CAP);
    ...
    {
    let mut conv: c_int = snd_hda_codec_read(codec, nid, 0, AC_VERB_GET_CONV, 0);
    snd_iprintf(buffer,
    ...
    let mut digi2: c_uchar = digi1 >> 8;
    let mut digi3: c_uchar = digi1 >> 16;
    ...
    let mut sup: c_int = param_read(codec, nid, AC_PAR_POWER_STATE);
...
#[no_mangle]
pub unsafe extern "C" fn can_dump_coef(codec: *mut hda_codec) -> bool {
    ...
    let mut proc_caps: c_uint = param_read(codec, nid, AC_PAR_PROC_CAP);
...
// Get Cache connections info
    ...
    hda_nid_t *conn = core::ptr::null_mut();
    let mut conn_len: c_int = 0;
    snd_iprintf(buffer, "Node 0x%02x [%s] wcaps 0x%x:", nid,
    get_wid_type_name(wid_type), wid_caps);
    if (wid_caps & AC_WCAP_STEREO) {
    let mut chans: c_uint = get_wcaps_channels(wid_caps);
    if (chans == 2)
