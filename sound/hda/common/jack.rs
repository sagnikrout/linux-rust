...
//! Source: sound/hda/common/jack.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn read_pin_sense(codec: *mut hda_codec, nid: hda_nid_t, dev_id: c_int) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_hda_jack_pin_sense(codec: *mut hda_codec, nid: hda_nid_t, dev_id: c_int) -> u32 {
...
//
    ...
    {
    struct hda_jack_tbl *jack;
    struct hda_jack_callback *callback = core::ptr::null_mut();
    int err;
...
#[no_mangle]
unsafe extern "C" fn get_input_jack_type(codec: *mut hda_codec, nid: hda_nid_t) -> c_int {
...
// Example final name: "Internal Mic Phantom Jack"
    ...
    err = snd_hda_jack_add_kctl(codec, nid, name, phantom_jack, 0, core::ptr::null_mut());
    if (err < 0)
    return err;
    if (!phantom_jack)
...
// If we have headphone mics; make sure they get the right name
    ...
    err = add_jack_kctl(codec, *p, cfg, core::ptr::null_mut());
    if (err < 0)
    return err;
    }
    ...
    err = add_jack_kctl(codec, cfg.dig_in_pin, cfg, core::ptr::null_mut());
    if (err < 0)
    return err;
    err = add_jack_kctl(codec, cfg.mono_out_pin, cfg, core::ptr::null_mut());
    if (err < 0)
    return err;
    return 0;
    }
...
pub unsafe extern "C" fn snd_hda_jack_unsol_event(codec: *mut hda_codec, res: c_uint) {
    ...
    let mut tag: c_int = (res & AC_UNSOL_RES_TAG) >> AC_UNSOL_RES_TAG_SHIFT;
    if (codec.dp_mst) {
    int dev_entry =
