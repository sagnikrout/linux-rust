...
//! Source: sound/aoa/core/gpio-feature.c
#![no_std]
...
// check if we can get it the easy way ...
    np = of_find_node_by_name(core::ptr::null_mut(), name);
    if (!np) {
...
// of the gpio node and check them all.
    gpio = of_find_node_by_name(core::ptr::null_mut(), "gpio");
    if (!gpio)
    return core::ptr::null_mut();
    while ((np = of_get_next_child(gpio, np))) {
    audio_gpio = of_get_property(np, "audio-gpio", core::ptr::null_mut());
    if (!audio_gpio)
    continue;
...
// which is at 0x50, but apparently not always...
    if (*gpioptr < 0x50)
// gpioptr += 0x50;
    reg = of_get_property(np, "audio-gpio-active-state", core::ptr::null_mut());
    if (!reg)
...
#[no_mangle]
unsafe extern "C" fn get_irq(np: *mut *mut device_node, irqptr: *mut c_int) {
...
// 0x4 is outenable, 0x1 is out, thus 4 or 5
    ...
    return;						\
    \
    v = pmac_call_feature(PMAC_FTR_READ_GPIO, core::ptr::null_mut(),		\
    name##_mute_gpio,			\
    0);				\
    \
// muted = !on... */					\
    ...
    \
    pmac_call_feature(PMAC_FTR_WRITE_GPIO, core::ptr::null_mut(),		\
    name##_mute_gpio, v);			\
    \
...
unsafe extern "C" fn gpio_enable_dual_edge(gpio: c_int) {
    ...
    if (gpio == -1)
    return;
    v = pmac_call_feature(PMAC_FTR_READ_GPIO, core::ptr::null_mut(), gpio, 0);
    v |= 0x80; /* enable dual edge */
    pmac_call_feature(PMAC_FTR_WRITE_GPIO, core::ptr::null_mut(), gpio, v);
    }
#[no_mangle]
unsafe extern "C" fn ftr_gpio_init(rt: *mut gpio_runtime) {
    static void ftr_gpio_init(struct gpio_runtime *rt)
    {
    get_gpio("headphone-mute", core::ptr::null_mut(),
    &headphone_mute_gpio,
    &headphone_mute_gpio_activestate);
    get_gpio("amp-mute", core::ptr::null_mut(),
    &amp_mute_gpio,
    &amp_mute_gpio_activestate);
    get_gpio("lineout-mute", core::ptr::null_mut(),
    &lineout_mute_gpio,
    &lineout_mute_gpio_activestate);
    get_gpio("hw-reset", "audio-hw-reset",
    &hw_reset_gpio,
    &hw_reset_gpio_activestate);
    if (get_gpio("master-mute", core::ptr::null_mut(),
    &master_mute_gpio,
    &master_mute_gpio_activestate)) {
    methods.set_master = ftr_gpio_set_master;
    methods.get_master = ftr_gpio_get_master;
    }
    headphone_detect_node = get_gpio("headphone-detect", core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn ftr_handle_notify_irq(xx: c_int, data: *mut c_void) -> irqreturn_t {
    ...
    if (gpio == -1)
    return -ENODEV;
    ret = pmac_call_feature(PMAC_FTR_READ_GPIO, core::ptr::null_mut(), gpio, 0);
    if (ret < 0)
    return ret;
