...
//! Source: sound/aoa/core/gpio-pmf.c
#![no_std]
...
unsafe extern "C" fn pmf_gpio_set_hw_reset(rt: *mut gpio_runtime, on: c_int) {
    ...
    {
    let mut args: pmf_args = { .count = 1, .u[0].v = !!on };
    int rc;
    if (unlikely(!rt)) return;
...
unsafe extern "C" fn pmf_handle_notify_irq(data: *mut c_void) {
    ...
    kfree(irq_client);
    notif.gpio_private = core::ptr::null_mut();
    }
    if (!old && notify) {
    ...
    {
    char *name;
    let mut err: c_int = -EBUSY, ret;
    let mut args: pmf_args = { .count = 1, .u[0].p = &ret };
    switch (type) {
    case AOA_NOTIFY_HEADPHONE:
    name = "headphone-detect";
    break;
