...
//! Source: sound/aoa/fabrics/layout.c
#![no_std]
...
pub const MAX_CODECS_PER_BUS: c_int = 2;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct codec_connect_info {
    pub name: *mut c_char,
    pub connections: *const codec_connection,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct layout {
    pub device_id: unsigned int layout_id,,
    pub codecs: [codec_connect_info; MAX_CODECS_PER_BUS],
    pub flags: c_int,
...
#[no_mangle]
unsafe extern "C" fn use_layout(l: *mut layout) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct layout_dev_ptr {
    pub ptr: *mut layout_dev,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct layout_dev {
    pub list: list_head,
    pub sdev: *mut soundbus_dev,
    pub sound: *mut device_node,
    pub codecs: [*mut aoa_codec; MAX_CODECS_PER_BUS],
    pub layout: *mut layout,
    pub gpio: gpio_runtime,
// we need these for headphone/lineout detection
    pub headphone_ctrl: *mut snd_kcontrol,
    pub lineout_ctrl: *mut snd_kcontrol,
    pub speaker_ctrl: *mut snd_kcontrol,
    pub master_ctrl: *mut snd_kcontrol,
    pub headphone_detected_ctrl: *mut snd_kcontrol,
    pub lineout_detected_ctrl: *mut snd_kcontrol,
    pub selfptr_headphone: layout_dev_ptr,
    pub selfptr_lineout: layout_dev_ptr,
...
// if the codec has a 'codec' node, we require a reference
    ...
    ref = of_get_property(ldev.sound, propname, core::ptr::null_mut());
    if (!ref) {
...
#[no_mangle]
unsafe extern "C" fn layout_found_codec(codec: *mut aoa_codec) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn layout_remove_codec(codec: *mut aoa_codec) {
    static void layout_remove_codec(struct aoa_codec *codec)
    {
    int i;
// here remove the codec from the layout dev's
// codec reference
    codec.soundbus_dev = core::ptr::null_mut();
    codec.gpio = core::ptr::null_mut();
    for (i=0; i<MAX_CODECS_PER_BUS; i++) {
    }
    }
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn layout_attached_codec(codec: *mut aoa_codec) {
...
#[no_mangle]
unsafe extern "C" fn aoa_fabric_layout_probe(sdev: *mut soundbus_dev) -> c_int {
    static int aoa_fabric_layout_probe(struct soundbus_dev *sdev)
    {
    struct device_node *sound = core::ptr::null_mut();
    const unsigned int *id;
    struct layout *layout = core::ptr::null_mut();
    struct layout_dev *ldev = core::ptr::null_mut();
    int err;
...
// by breaking out we keep a reference
    ...
    if (!sound)
    return -ENODEV;
    id = of_get_property(sound, "layout-id", core::ptr::null_mut());
    if (id) {
    layout = find_layout_by_id(*id);
    } else {
    id = of_get_property(sound, "device-id", core::ptr::null_mut());
    if (id)
...
// reset if we didn't use it
    sdev.pcmname = core::ptr::null_mut();
    sdev.pcmid = -1;
    list_del(&ldev.list);
    layouts_list_items--;
    kfree(ldev);
    outnodev:
    of_node_put(sound);
    layout_device = core::ptr::null_mut();
    return -ENODEV;
    }
#[no_mangle]
unsafe extern "C" fn aoa_fabric_layout_remove(sdev: *mut soundbus_dev) {
    ...
    ldev.codecs[i] = core::ptr::null_mut();
    }
    list_del(&ldev.list);
    layouts_list_items--;
...
#[no_mangle]
unsafe extern "C" fn aoa_fabric_layout_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn aoa_fabric_layout_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn aoa_fabric_layout_init() -> int __init {
    static int __init aoa_fabric_layout_init(void)
    {
    return soundbus_register_driver(&aoa_soundbus_driver);
    }
#[no_mangle]
unsafe extern "C" fn aoa_fabric_layout_exit() -> void __exit {
