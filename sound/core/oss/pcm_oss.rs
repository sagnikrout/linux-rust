...
//! Source: sound/core/oss/pcm_oss.c
#![no_std]
...
// Macro flag: #define PLUGIN_DEBUG

// Macro flag: #define OSS_DEBUG

    static int dsp_map[SNDRV_CARDS] __ro_after_init;
    static int adsp_map[SNDRV_CARDS] __ro_after_init = {[0 ... (SNDRV_CARDS-1)] = 1};
    let mut nonblock_open: static bool = 1;
    MODULE_AUTHOR("Jaroslav Kysela <perex@perex.cz>, Abramo Bagnara <abramo@alsa-project.org>");
    MODULE_DESCRIPTION("PCM OSS emulation for ALSA.");
    MODULE_LICENSE("GPL");
    module_param_array(dsp_map, int, core::ptr::null_mut(), 0444);
    MODULE_PARM_DESC(dsp_map, "PCM device number assigned to 1st OSS device.");
    module_param_array(adsp_map, int, core::ptr::null_mut(), 0444);
...
//
#[no_mangle]
unsafe extern "C" fn snd_interval_refine_min(i: *mut snd_interval, min: c_uint, openmin: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_interval_refine_max(i: *mut snd_interval, max: c_uint, openmax: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_interval_refine_set(i: *mut snd_interval, val: c_uint) -> c_int {
...
// dir = - (int) i->openmax;
    ...
    const struct snd_mask *val)
    {
    let mut changed: c_int = _snd_pcm_hw_param_mask(params, var, val);
    if (changed < 0)
    return changed;
    if (params.rmask) {
    let mut err: c_int = snd_pcm_hw_refine(pcm, params);
    if (err < 0)
    return err;
    }
    return 0;
    ...
    val + !!open);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: hw_is_interval(var)) -> else {
...
//
    ...
    int *dir)
    {
    let mut changed: c_int = _snd_pcm_hw_param_min(params, var, val, dir ? *dir : 0);
    if (changed < 0)
    return changed;
    if (params.rmask) {
    let mut err: c_int = snd_pcm_hw_refine(pcm, params);
...
//
    ...
    int *dir)
    {
    let mut changed: c_int = _snd_pcm_hw_param_max(params, var, val, dir ? *dir : 0);
    if (changed < 0)
    return changed;
    if (params.rmask) {
    let mut err: c_int = snd_pcm_hw_refine(pcm, params);
...
//
    ...
    int min, max;
    int mindir, maxdir;
    let mut valdir: c_int = dir ? *dir : 0;
// FIXME
    if (best > INT_MAX)
    best = INT_MAX;
    min = max = best;
    ...
    maxdir = 0;
#[no_mangle]
pub unsafe extern "C" fn if(0: maxdir ==) -> else {
    else if (maxdir == 0)
    maxdir = -1;
    else {
    maxdir = 1;
...
// params = *save;
    ...
    val++;
#[no_mangle]
pub unsafe extern "C" fn if(0: dir <) -> else {
    else if (dir < 0)
    val--;
...
//
    ...
    int dir)
    {
    let mut changed: c_int = _snd_pcm_hw_param_set(params, var, val, dir);
    if (changed < 0)
    return changed;
    if (params.rmask) {
    let mut err: c_int = snd_pcm_hw_refine(pcm, params);
    ...
    return snd_pcm_hw_param_value(params, var, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_plugin_clear(substream: *mut snd_pcm_substream) -> c_int {
    ...
    runtime.oss.plugin_first = runtime.oss.plugin_last = core::ptr::null_mut();
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn snd_pcm_plugin_insert(plugin: *mut snd_pcm_plugin) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_bytes(substream: *mut snd_pcm_substream, frames: c_long) -> c_long {
    static long snd_pcm_oss_bytes(struct snd_pcm_substream *substream, long frames)
    {
    struct snd_pcm_runtime *runtime = substream.runtime;
    let mut buffer_size: c_long = snd_pcm_lib_buffer_bytes(substream);
    let mut bytes: c_long = frames_to_bytes(runtime, frames);
    ...
    {
    let mut bsize: u64 = (u64)runtime.oss.buffer_bytes * (u64)bytes;
    return div_u64(bsize, buffer_size);
    }

    }
#[no_mangle]
unsafe extern "C" fn snd_pcm_alsa_frames(substream: *mut snd_pcm_substream, bytes: c_long) -> c_long {
    ...
    static inline
#[no_mangle]
pub unsafe extern "C" fn get_hw_ptr_period(runtime: *mut snd_pcm_runtime) -> snd_pcm_uframes_t {
...
// linear formats
pub const AFMT_S32_LE: c_uint = 0x00001000;
pub const AFMT_S32_BE: c_uint = 0x00002000;
pub const AFMT_S24_LE: c_uint = 0x00008000;
pub const AFMT_S24_BE: c_uint = 0x00010000;
pub const AFMT_S24_PACKED: c_uint = 0x00040000;
// other supported formats
pub const AFMT_FLOAT: c_uint = 0x00004000;
pub const AFMT_SPDIF_RAW: c_uint = 0x00020000;
// unsupported formats
pub const AFMT_AC3: c_uint = 0x00000400;
pub const AFMT_VORBIS: c_uint = 0x00000800;
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_format_from(format: c_int) -> snd_pcm_format_t {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_format_to(format: snd_pcm_format_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: runtime->oss.fragshift) -> else {
    oss_period_size = 1 << runtime.oss.fragshift;
    if (oss_period_size > oss_buffer_size / 2)
    oss_period_size = oss_buffer_size / 2;
    } else {
    int sd;
    let mut bytes_per_sec: usize = params_rate(oss_params) * snd_pcm_format_physical_width(params_format(oss_params)) * params_channels(oss_params) / 8;
    ...
    snd_pcm_hw_param_value_min(slave_params, SNDRV_PCM_HW_PARAM_PERIOD_SIZE, core::ptr::null_mut()));
    ...
    snd_pcm_hw_param_value_max(slave_params, SNDRV_PCM_HW_PARAM_PERIOD_SIZE, core::ptr::null_mut()));
    ...
    s = snd_pcm_hw_param_value_max(slave_params, SNDRV_PCM_HW_PARAM_PERIODS, core::ptr::null_mut());
    if (s > 0 && runtime.oss.maxfrags && s > runtime.oss.maxfrags)
    s = runtime.oss.maxfrags;
    if (oss_periods > s)
    oss_periods = s;
    s = snd_pcm_hw_param_value_min(slave_params, SNDRV_PCM_HW_PARAM_PERIODS, core::ptr::null_mut());
    if (s < 2)
    s = 2;
    if (oss_periods < s)
    oss_periods = s;
...
// not found, use the nearest rate
    return snd_pcm_hw_param_near(substream, params, SNDRV_PCM_HW_PARAM_RATE, best_rate, core::ptr::null_mut());
    }
// parameter locking: returns immediately if tried during streaming
#[no_mangle]
unsafe extern "C" fn lock_params(runtime: *mut snd_pcm_runtime) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn unlock_params(runtime: *mut snd_pcm_runtime) {
    static void unlock_params(struct snd_pcm_runtime *runtime)
    {
    mutex_unlock(&runtime.oss.params_lock);
    }
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_release_buffers(substream: *mut snd_pcm_substream) {
    static void snd_pcm_oss_release_buffers(struct snd_pcm_substream *substream)
    {
    struct snd_pcm_runtime *runtime = substream.runtime;
    kvfree(runtime.oss.buffer);
    runtime.oss.buffer = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_change_params_locked(substream: *mut snd_pcm_substream) -> c_int {
    ...
    runtime.oss.channels, core::ptr::null_mut());
    if (err < 0)
    goto failure;
    ...
    err = snd_pcm_hw_param_near(substream, sparams, SNDRV_PCM_HW_PARAM_PERIOD_SIZE, n, core::ptr::null_mut());
    if (err < 0)
    goto failure;
    err = snd_pcm_hw_param_near(substream, sparams, SNDRV_PCM_HW_PARAM_PERIODS,
    runtime.oss.periods, core::ptr::null_mut());
    if (err < 0)
    goto failure;
    snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_DROP, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_active_substream(pcm_oss_file: *mut snd_pcm_oss_file, r_substream: *mut snd_pcm_substream) -> c_int {
    static int snd_pcm_oss_get_active_substream(struct snd_pcm_oss_file *pcm_oss_file, struct snd_pcm_substream **r_substream)
    {
    int idx, err;
    struct snd_pcm_substream *asubstream = core::ptr::null_mut(), *substream;
    for (idx = 0; idx < 2; idx++) {
    substream = pcm_oss_file.streams[idx];
    if (substream == core::ptr::null_mut())
    continue;
    if (asubstream == core::ptr::null_mut())
...
//
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_prepare(substream: *mut snd_pcm_substream) -> c_int {
    static int snd_pcm_oss_prepare(struct snd_pcm_substream *substream)
    {
    int err;
    struct snd_pcm_runtime *runtime = substream.runtime;
    err = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_PREPARE, core::ptr::null_mut());
    if (err < 0) {
    pcm_dbg(substream.pcm,
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_make_ready(substream: *mut snd_pcm_substream) -> c_int {
...
// call with params_lock held
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_make_ready_locked(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_capture_position_fixup(substream: *mut snd_pcm_substream, delay: *mut snd_pcm_sframes_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_oss_write3(substream: *mut snd_pcm_substream, ptr: *const c_char, frames: snd_pcm_uframes_t, in_kernel: c_int) -> snd_pcm_sframes_t {
...
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_oss_read3(substream: *mut snd_pcm_substream, ptr: *mut c_char, frames: snd_pcm_uframes_t, in_kernel: c_int) -> snd_pcm_sframes_t {
    ...
    ret = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_DRAIN, core::ptr::null_mut());
    if (ret < 0)
    break;
    ...
    ret = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_DROP, core::ptr::null_mut());
    if (ret < 0)
    break;
    }
    continue;
...
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_oss_writev3(substream: *mut snd_pcm_substream, bufs: *mut c_void, frames: snd_pcm_uframes_t) -> snd_pcm_sframes_t {
...
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_oss_readv3(substream: *mut snd_pcm_substream, bufs: *mut c_void, frames: snd_pcm_uframes_t) -> snd_pcm_sframes_t {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_write2(substream: *mut snd_pcm_substream, buf: *const c_char, bytes: usize, in_kernel: c_int) -> isize {
    ...
    let mut oss_frame_bytes: usize = (runtime.oss.plugin_first.src_width * runtime.oss.plugin_first.src_format.channels) / 8;
    if (!in_kernel) {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_write1(substream: *mut snd_pcm_substream, buf: *const char __user, bytes: usize) -> isize {
    static ssize_t snd_pcm_oss_write1(struct snd_pcm_substream *substream, const char __user *buf, size_t bytes)
    {
    let mut xfer: usize = 0;
    let mut tmp: isize = 0;
    struct snd_pcm_runtime *runtime = substream.runtime;
    if (atomic_read(&substream.mmap_count))
    return -ENXIO;
    ...
    runtime.oss.buffer_used = 0;
#[no_mangle]
pub unsafe extern "C" fn if(0: (substream->f_flags & O_NONBLOCK) !=) -> else {
    tmp = -EAGAIN;
    goto err;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_read2(substream: *mut snd_pcm_substream, buf: *mut c_char, bytes: usize, in_kernel: c_int) -> isize {
    ...
    if (runtime.oss.plugin_first) {
    struct snd_pcm_plugin_channel *channels;
    let mut oss_frame_bytes: usize = (runtime.oss.plugin_last.dst_width * runtime.oss.plugin_last.dst_format.channels) / 8;
    if (!in_kernel)
    buf = runtime.oss.buffer;
    frames = bytes / oss_frame_bytes;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_read1(substream: *mut snd_pcm_substream, buf: *mut char __user, bytes: usize) -> isize {
    ...
    tmp = snd_pcm_oss_read2(substream, (char  *)buf,
    runtime.oss.period_bytes, 0);
    if (tmp <= 0)
    goto err;
    runtime.oss.bytes += tmp;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_reset(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_post(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
    ...
    if (err < 0)
    return err;
    snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_START, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_sync1(substream: *mut snd_pcm_substream, size: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_sync(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
...
//
    ...
    snd_pcm_lib_write(substream, core::ptr::null_mut(), size);
#[no_mangle]
pub unsafe extern "C" fn if(SNDRV_PCM_ACCESS_RW_NONINTERLEAVED: runtime->access ==) -> else {
    else if (runtime.access == SNDRV_PCM_ACCESS_RW_NONINTERLEAVED)
    snd_pcm_lib_writev(substream, core::ptr::null_mut(), size);
    }
    unlock:
    mutex_unlock(&runtime.oss.params_lock);
    atomic_dec(&runtime.oss.rw_ref);
...
//
    ...
    err = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_DRAIN, core::ptr::null_mut());
    ...
    return err;
    runtime = substream.runtime;
    err = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_DROP, core::ptr::null_mut());
    if (err < 0)
    return err;
    mutex_lock(&runtime.oss.params_lock);
    runtime.oss.buffer_used = 0;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_rate(pcm_oss_file: *mut snd_pcm_oss_file, rate: c_int) -> c_int {
    ...
    rate = 1000;
#[no_mangle]
pub unsafe extern "C" fn if(192000: rate >) -> else {
    else if (rate > 192000)
    rate = 192000;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_rate(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
    static int snd_pcm_oss_get_rate(struct snd_pcm_oss_file *pcm_oss_file)
    {
    struct snd_pcm_substream *substream;
    int err;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_channels(pcm_oss_file: *mut snd_pcm_oss_file, channels: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_channels(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
    static int snd_pcm_oss_get_channels(struct snd_pcm_oss_file *pcm_oss_file)
    {
    struct snd_pcm_substream *substream;
    int err;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_block_size(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_formats(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
    static int snd_pcm_oss_get_formats(struct snd_pcm_oss_file *pcm_oss_file)
    {
    struct snd_pcm_substream *substream;
    int err;
    int direct;
    let mut formats: c_uint = 0;
    ...
    let mut f: c_int = snd_pcm_oss_format_to(fmt);
    if (f >= 0)
    formats |= f;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_format(pcm_oss_file: *mut snd_pcm_oss_file, format: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_format(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
    static int snd_pcm_oss_get_format(struct snd_pcm_oss_file *pcm_oss_file)
    {
    struct snd_pcm_substream *substream;
    int err;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_subdivide1(substream: *mut snd_pcm_substream, subdivide: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_subdivide(pcm_oss_file: *mut snd_pcm_oss_file, subdivide: c_int) -> c_int {
    static int snd_pcm_oss_set_subdivide(struct snd_pcm_oss_file *pcm_oss_file, int subdivide)
    {
    let mut err: c_int = -EINVAL, idx;
    for (idx = 1; idx >= 0; --idx) {
    struct snd_pcm_substream *substream = pcm_oss_file.streams[idx];
    struct snd_pcm_runtime *runtime;
    if (substream == core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_fragment1(substream: *mut snd_pcm_substream, val: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_fragment(pcm_oss_file: *mut snd_pcm_oss_file, val: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_nonblock(file: *mut *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_caps1(substream: *mut snd_pcm_substream, res: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_caps(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_set_trigger(pcm_oss_file: *mut snd_pcm_oss_file, trigger: c_int) -> c_int {
    static int snd_pcm_oss_set_trigger(struct snd_pcm_oss_file *pcm_oss_file, int trigger)
    {
    struct snd_pcm_runtime *runtime;
    struct snd_pcm_substream *psubstream = core::ptr::null_mut(), *csubstream = core::ptr::null_mut();
    int err, cmd;
    ...
    mutex_unlock(&runtime.oss.params_lock);
    if (cmd) {
    err = snd_pcm_kernel_ioctl(psubstream, cmd, core::ptr::null_mut());
    if (err < 0)
    return err;
    }
    }
    ...
    mutex_unlock(&runtime.oss.params_lock);
    if (cmd) {
    err = snd_pcm_kernel_ioctl(csubstream, cmd, core::ptr::null_mut());
    if (err < 0)
    return err;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_trigger(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_odelay(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_ptr(pcm_oss_file: *mut snd_pcm_oss_file, stream: c_int, _info: *mut *mut count_info __user) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_space(pcm_oss_file: *mut snd_pcm_oss_file, stream: c_int, _info: *mut audio_buf_info __user) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_get_mapbuf(pcm_oss_file: *mut snd_pcm_oss_file, stream: c_int, _info: *mut *mut buffmem_desc __user) -> c_int {
...
// pr_debug("TODO: snd_pcm_oss_get_mapbuf\n");
    ...
    {
    const char *ptr, *ptrl = core::ptr::null_mut();
    ...
    } while ((task_name = strip_task_path(task_name)) != core::ptr::null_mut());
    out:
    if (setup)
// rsetup = *setup;
    }
...
unsafe extern "C" fn snd_pcm_oss_release_substream(substream: *mut snd_pcm_substream) {
    ...
    substream.f_flags |= O_NONBLOCK;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: setup->block) -> else {
    else if (setup.block)
    substream.f_flags &= ~O_NONBLOCK;
    runtime = substream.runtime;
    runtime.oss.params = 1;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_release_file(pcm_oss_file: *mut snd_pcm_oss_file) -> c_int {
    ...
    struct snd_pcm_substream *substream;
    let mut f_mode: fmode_t = file.f_mode;
    if (rpcm_oss_file)
// rpcm_oss_file = NULL;
    pcm_oss_file = kzalloc_obj(*pcm_oss_file);
    if (pcm_oss_file == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn snd_task_name(task: *mut task_struct, name: *mut c_char, size: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_read(file: *mut file, buf: *mut char __user, count: usize, offset: *mut loff_t) -> isize {
    ...
    {
    let mut res: isize = snd_pcm_oss_read1(substream, buf, count);
    pcm_dbg(substream.pcm,
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_write(file: *mut file, buf: *const char __user, count: usize, offset: *mut loff_t) -> isize {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_playback_ready(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_capture_ready(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn need_input_retrigger(runtime: *mut snd_pcm_runtime) -> bool {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_poll(file: *mut file, wait: *mut *mut poll_table) -> __poll_t {
    ...
    csubstream = pcm_oss_file.streams[SNDRV_PCM_STREAM_CAPTURE];
    mask = 0;
    if (psubstream != core::ptr::null_mut()) {
    ...
    if (csubstream != core::ptr::null_mut()) {
...
//
    ...
    if (runtime.oss.plugin_first != core::ptr::null_mut())
    return -EIO;
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_proc_free_setup_list(pstr: *mut *mut snd_pcm_str) {
    static void snd_pcm_oss_proc_free_setup_list(struct snd_pcm_str * pstr)
    {
    struct snd_pcm_oss_setup *setup, *setupn;
    for (setup = pstr.oss.setup_list, pstr.oss.setup_list = core::ptr::null_mut();
    setup; setup = setupn) {
    setupn = setup.next;
    kfree(setup.task_name);
    kfree(setup);
    }
    pstr.oss.setup_list = core::ptr::null_mut();
    }
    ...
    template.periods = simple_strtoul(str, core::ptr::null_mut(), 10);
    ptr = snd_info_get_str(str, ptr, sizeof(str));
    template.period_size = simple_strtoul(str, core::ptr::null_mut(), 10);
...
// setup = template;
    if (pstr.oss.setup_list == core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_proc_init(pcm: *mut snd_pcm) {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_oss_proc_done(pcm: *mut snd_pcm) {
    ...
    pstr.oss.proc_entry = core::ptr::null_mut();
    snd_pcm_oss_proc_free_setup_list(pstr);
    }
    }

#[no_mangle]
pub unsafe extern "C" fn snd_pcm_oss_proc_init(pcm: *mut snd_pcm) {
    static inline void snd_pcm_oss_proc_init(struct snd_pcm *pcm)
    {
    }
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_oss_proc_done(pcm: *mut snd_pcm) {
...
#[no_mangle]
unsafe extern "C" fn register_oss_dsp(pcm: *mut snd_pcm, index: c_int) {
...
#[no_mangle]
unsafe extern "C" fn alsa_pcm_oss_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn alsa_pcm_oss_exit() -> void __exit {
