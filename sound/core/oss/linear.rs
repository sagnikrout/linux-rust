...
//! Source: sound/core/oss/linear.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct linear_priv {
    pub /: *mut *mut int cvt_endian; / need endian conversion?,
    pub /: *mut *mut unsigned int src_ofs; / byte offset in source format,
    pub /: *mut *mut unsigned int dst_ofs; / byte soffset in destination format,
    pub /: *mut *mut unsigned int copy_ofs; / byte offset in temporary u32 data,
    pub /: *mut *mut unsigned int dst_bytes; / byte size of destination format,
    pub /: *mut *mut unsigned int copy_bytes; / bytes to copy per conversion,
    pub /: *mut *mut unsigned int flip; / MSB flip for signeness, done after endian conv,
}
    ...
    if (dst_le)
    data.flip = ( u32)cpu_to_le32(0x80000000);
    else
    data.flip = ( u32)cpu_to_be32(0x80000000);
    }
    }
