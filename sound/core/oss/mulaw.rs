...
#[no_mangle]
pub unsafe extern "C" fn val_seg(val: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ulaw2linear(u_val: c_uchar) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mulaw_priv {
    pub func: mulaw_f,
    pub /: *mut *mut int cvt_endian; / need endian conversion?,
    pub /: *mut *mut unsigned int native_ofs; / byte offset in native format,
    pub /: *mut *mut unsigned int copy_ofs; / byte offset in s16 format,
    pub /: *mut *mut unsigned int native_bytes; / byte size of the native format,
    pub /: *mut *mut unsigned int copy_bytes; / bytes to copy per conversion,
    pub /: *mut *mut u16 flip; / MSB flip for signedness, done after endian conversion,
}
    ...
    int channel;
    let mut nchannels: c_int = plugin.src_format.channels;
    ...
    frames1 = frames;
    while (frames1-- > 0) {
    let mut sample: signed short = ulaw2linear(*src);
    ...
    frames1 = frames;
    while (frames1-- > 0) {
    let mut sample: signed short = cvt_native_to_s16(data, src);
...
#[no_mangle]
unsafe extern "C" fn init_data(data: *mut mulaw_priv, format: snd_pcm_format_t) {
...
#[no_mangle]
pub unsafe extern "C" fn if(SNDRV_PCM_FORMAT_MU_LAW: src_format->format ==) -> else {
    format = dst_format;
    func = mulaw_decode;
    }
    else {
