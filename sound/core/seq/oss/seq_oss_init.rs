...
//! Source: sound/core/seq/oss/seq_oss_init.c
#![no_std]
...
// common variables
//
    let mut maxqlen: static int = SNDRV_SEQ_OSS_MAX_QLEN;
    ...
    let mut __ro_after_init: static int system_port = -1;
...
#[no_mangle]
unsafe extern "C" fn async_call_lookup_ports(work: *mut work_struct) {
...
//
    ...
    return -ENOMEM;
// create ALSA client
    rc = snd_seq_create_kernel_client(core::ptr::null_mut(), SNDRV_SEQ_CLIENT_OSS,
    "OSS sequencer");
    if (rc < 0)
    return rc;
    system_client = rc;
...
//
    ...
    if (client_table[i] == core::ptr::null_mut())
    break;
    }
    dp.index = i;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_read_mode(dp->file_mode)) -> else {
...
//
    ...
    {
    int queue;
    client_table[dp.index] = core::ptr::null_mut();
    num_clients--;
