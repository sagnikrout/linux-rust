...
//! Source: sound/core/seq/seq_compat.c
#![no_std]
...
pub struct snd_seq_port_info32 {
    ...
    pub /: *mut *mut u32 capability; / port capability bits,
    pub /: *mut *mut u32 type; / port type bits,
    pub /: *mut *mut s32 midi_channels; / channels per MIDI port,
    pub /: *mut *mut s32 midi_voices; / voices per MIDI port,
    pub /: *mut *mut s32 synth_voices; / voices per SYNTH port,
    pub /: *mut *mut s32 read_use; / R/O: subscribers for output (from this port),
    pub /: *mut *mut s32 write_use; / R/O: subscribers for input (to this port),
    pub /: *mut *mut u32 kernel; / reserved for kernel use (must be NULL),
    pub /: *mut *mut u32 flags; / misc. conditioning,
    pub /: *mut *mut unsigned char time_queue; / queue # for timestamping,
    pub /: *mut *mut char reserved[59]; / for future use,
}
...
#[no_mangle]
unsafe extern "C" fn snd_seq_ioctl_compat(file: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
