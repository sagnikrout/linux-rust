...
//! Source: sound/core/seq/seq_midi_emul.c
#![no_std]
...
//
    ...
    if (ev == core::ptr::null_mut() || chanset == core::ptr::null_mut()) {
    pr_debug("ALSA: seq_midi_emul: ev or chanbase core::ptr::null_mut() (snd_midi_process_event)\n");
    return;
    }
...
//
    ...
    {
    let mut p: c_int = cmd & 0x0f;
    if (p == 0)
    p = 9;
#[no_mangle]
pub unsafe extern "C" fn if(10: p <) -> else {
    else if (p < 10)
    p--;
    return p;
    }
...
//
    ...
    let mut parsed: c_int = SNDRV_MIDI_SYSEX_NOT_PARSED;
    if (len <= 0 || buf[0] != 0xf0)
    return;
// skip first byte
    buf++;
...
#[no_mangle]
pub unsafe extern "C" fn if(0x15: (buf[5] & 0xf0) == 0x10 && buf[6] ==) -> else {
// drum pattern
    let mut p: c_int = get_channel(buf[5]);
...
//
#[no_mangle]
unsafe extern "C" fn snd_midi_channel_init(p: *mut snd_midi_channel, n: c_int) {
...
//
    ...
    chset.private_data = core::ptr::null_mut();
    for (i = 0; i < n; i++)
...
//
#[no_mangle]
unsafe extern "C" fn snd_midi_reset_controllers(chan: *mut snd_midi_channel) {
    static void snd_midi_reset_controllers(struct snd_midi_channel *chan)
    {
