...
//! Source: sound/core/control_led.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct snd_ctl_led_card {
    pub dev: device,
    pub number: c_int,
    pub led: *mut snd_ctl_led,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct snd_ctl_led {
    pub dev: device,
    pub controls: list_head,
    pub name: *const c_char,
    pub group: c_uint,
    pub trigger_type: enum led_audio,
    pub mode: enum snd_ctl_led_mode,
    pub cards: [*mut snd_ctl_led_card; SNDRV_CARDS],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct snd_ctl_led_ctl {
    pub list: list_head,
    pub card: *mut snd_card,
    pub access: c_uint,
    pub kctl: *mut snd_kcontrol,
    pub index_offset: c_uint,
}
...
#[no_mangle]
pub unsafe extern "C" fn access_to_group(access: c_uint) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn group_to_access(group: c_uint) -> c_uint {
    ...
    {
    let mut group: c_uint = access_to_group(access);
    if (group >= MAX_LED)
...
//
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_get(lctl: *mut snd_ctl_led_ctl) -> c_int {
...
// the card may not be registered (active) at this point
    ...
    if (access)
    snd_ctl_led_set_state(card, access, core::ptr::null_mut(), 0);
    } else if (mask & SNDRV_CTL_EVENT_MASK_INFO) {
    vd = &kctl.vd[ioff];
    access = vd.access & SNDRV_CTL_ELEM_ACCESS_LED_MASK;
    access2 = snd_ctl_led_remove(kctl, ioff, access);
    if (access2)
    snd_ctl_led_set_state(card, access2, core::ptr::null_mut(), 0);
    if (access)
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_refresh() {
    static void snd_ctl_led_refresh(void)
    {
    unsigned int group;
    for (group = 0; group < MAX_LED; group++)
    snd_ctl_led_set_state(core::ptr::null_mut(), group_to_access(group), core::ptr::null_mut(), 0);
    }
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_ctl_destroy(lctl: *mut snd_ctl_led_ctl) {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_clean(card: *mut snd_card) {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_reset(card_number: c_int, group: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_register(card: *mut snd_card) {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_disconnect(card: *mut snd_card) {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_release(dev: *mut device) {
    static void snd_ctl_led_release(struct device *dev)
    {
    }
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_dev_release(dev: *mut device) {
...
//
    ...
    struct snd_ctl_led *led = container_of(dev, struct snd_ctl_led, dev);
    char _buf[16];
    let mut l: usize = min(count, sizeof(_buf) - 1);
    ...
    mode = MODE_FOLLOW_MUTE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strstr(_buf, _arg: "route")) -> else {
    else if (strstr(_buf, "route"))
    mode = MODE_FOLLOW_ROUTE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(_buf, _arg: "off", strncmp(_buf: 3) == 0 ||, _arg: "0", 0: 1) ==) -> else {
    else if (strncmp(_buf, "off", 3) == 0 || strncmp(_buf, "0", 1) == 0)
    mode = MODE_OFF;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(_buf, _arg: "on", strncmp(_buf: 2) == 0 ||, _arg: "1", 0: 1) ==) -> else {
    ...
    scoped_guard(mutex, &snd_ctl_led_mutex)
    led.mode = mode;
    snd_ctl_led_set_state(core::ptr::null_mut(), group_to_access(led.group), core::ptr::null_mut(), 0);
    return count;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncasecmp(s, _arg: "mixer", _arg: 5)) -> else {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_sysfs_add(card: *mut snd_card) {
    static void snd_ctl_led_sysfs_add(struct snd_card *card)
    {
    unsigned int group;
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_sysfs_remove(card: *mut snd_card) {
    static void snd_ctl_led_sysfs_remove(struct snd_card *card)
    {
    unsigned int group;
    struct snd_ctl_led_card *led_card;
    ...
    led.cards[card.number] = core::ptr::null_mut();
    }
    }
//
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_led_exit() -> void __exit {
    ...
    device_unregister(&snd_ctl_led_dev);
    snd_ctl_led_clean(core::ptr::null_mut());
