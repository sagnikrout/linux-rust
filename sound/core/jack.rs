...
pub struct snd_jack_kctl {
    ...
    pub /: *mut *mut list_head list; / list of controls belong to the same jack,
    pub /: *mut *mut unsigned int mask_bits; / only masked status bits are reported via kctl,
    pub /: *mut *mut *mut snd_jack jack; / pointer to snd_jack,
    pub /: *mut *mut bool sw_inject_enable; / allow to inject plug event via debugfs,

    pub /: *mut *mut *mut dentry jack_debugfs_root; / jack_kctl debugfs root,
...
// then we need to use a different deallocator.
    ...
    jack.input_dev = core::ptr::null_mut();
...
// Add capabilities for any keys that are enabled
    ...
    let mut testbit: c_int = SND_JACK_BTN_0 >> i;
...
#[no_mangle]
unsafe extern "C" fn snd_jack_inject_report(jack_kctl: *mut snd_jack_kctl, status: c_int) {
    ...
    return;
    for (i = 0; i < ARRAY_SIZE(jack.key); i++) {
    let mut testbit: c_int = ((SND_JACK_BTN_0 >> i) & jack_kctl.mask_bits);
    if (jack.type & testbit)
    input_report_key(jack.input_dev, jack.key[i],
    status & testbit);
    }
    for (i = 0; i < ARRAY_SIZE(jack_switch_types); i++) {
    let mut testbit: c_int = ((1 << i) & jack_kctl.mask_bits);
    if (jack.type & testbit)
...
#[no_mangle]
unsafe extern "C" fn parse_mask_bits(mask_bits: c_uint, buf: *mut c_char, buf_size: usize) -> c_int {
    static int parse_mask_bits(unsigned int mask_bits, char *buf, size_t buf_size)
    {
    let mut len: c_int = scnprintf(buf, buf_size, "0x%04x", mask_bits);
    int i;
...
#[no_mangle]
unsafe extern "C" fn snd_jack_remove_debugfs(jack: *mut snd_jack) {
    static void snd_jack_remove_debugfs(struct snd_jack *jack)
    {
    struct snd_jack_kctl *jack_kctl;
    list_for_each_entry(jack_kctl, &jack.kctl_list, list) {
    debugfs_remove(jack_kctl.jack_debugfs_root);
    jack_kctl.jack_debugfs_root = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn snd_jack_kctl_add(jack: *mut snd_jack, jack_kctl: *mut snd_jack_kctl) {
...
#[no_mangle]
unsafe extern "C" fn snd_jack_kctl_new(card: *mut snd_card, name: *const c_char, mask: c_uint) -> *mut snd_jack_kctl {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_jack_add_new_kctl(jack: *mut snd_jack, name: *const *const c_char, mask: c_int) -> c_int {
...
//
    ...
    {
    struct snd_jack *jack;
    struct snd_jack_kctl *jack_kctl = core::ptr::null_mut();
    int err;
    ...
    if (jack == core::ptr::null_mut())
    return -ENOMEM;
    jack.id = kstrdup(id, GFP_KERNEL);
    if (jack.id == core::ptr::null_mut()) {
    kfree(jack);
    return -ENOMEM;
    }
...
// don't create input device for phantom jack
    ...
    if (jack.input_dev == core::ptr::null_mut()) {
    err = -ENOMEM;
    goto fail_input;
    }
...
//
    ...
    int keytype)
    {
    let mut key: c_int = fls(SND_JACK_BTN_0) - fls(type);
...
pub unsafe extern "C" fn snd_jack_report(jack: *mut snd_jack, status: c_int) {
    ...
    return;
    for (i = 0; i < ARRAY_SIZE(jack.key); i++) {
    let mut testbit: c_int = ((SND_JACK_BTN_0 >> i) & ~mask_bits);
    if (jack.type & testbit)
    input_report_key(idev, jack.key[i],
    status & testbit);
    }
    for (i = 0; i < ARRAY_SIZE(jack_switch_types); i++) {
    let mut testbit: c_int = ((1 << i) & ~mask_bits);
    if (jack.type & testbit)
