...
//! Source: sound/core/pcm_memory.c
#![no_std]
...
//
    ...
    let mut preallocate_dma: static int = 1;
    module_param(preallocate_dma, int, 0444);
    MODULE_PARM_DESC(preallocate_dma, "Preallocate DMA memory when the PCM devices are initialized.");
    let mut maximum_substreams: static int = 4;
    module_param(maximum_substreams, int, 0444);
    MODULE_PARM_DESC(maximum_substreams, "Maximum substreams with preallocated DMA memory.");
    let mut snd_minimum_buffer: static size_t = 16384;
    let mut max_alloc_per_card: static unsigned long = 32UL * 1024UL * 1024UL;
    module_param(max_alloc_per_card, ulong, 0644);
    MODULE_PARM_DESC(max_alloc_per_card, "Max total allocation bytes per card.");
#[no_mangle]
unsafe extern "C" fn __update_allocated_size(card: *mut snd_card, bytes: isize) {
    static void __update_allocated_size(struct snd_card *card, ssize_t bytes)
    {
    card.total_pcm_alloc_bytes += bytes;
    }
#[no_mangle]
unsafe extern "C" fn update_allocated_size(card: *mut snd_card, bytes: isize) {
...
#[no_mangle]
unsafe extern "C" fn decrease_allocated_size(card: *mut snd_card, bytes: usize) {
...
#[no_mangle]
unsafe extern "C" fn do_free_pages(card: *mut snd_card, dmab: *mut snd_dma_buffer) {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_lib_preallocate_free_for_all(pcm: *mut snd_pcm) {
...
#[no_mangle]
pub unsafe extern "C" fn preallocate_info_init(substream: *mut snd_pcm_substream) {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_lib_malloc_pages(substream: *mut snd_pcm_substream, size: usize) -> c_int {
    int snd_pcm_lib_malloc_pages(struct snd_pcm_substream *substream, size_t size)
    {
    struct snd_card *card;
    struct snd_pcm_runtime *runtime;
    struct snd_dma_buffer *dmab = core::ptr::null_mut();
...
// perphaps, we might free the large DMA memory region
    ...
    if (substream.dma_buffer.area != core::ptr::null_mut() &&
