...
//! Source: sound/core/ump_convert.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn downscale_32_to_7bit(src: u32) -> u8 {
    static u8 downscale_32_to_7bit(u32 src)
    {
    return src >> 25;
    }
#[no_mangle]
unsafe extern "C" fn downscale_32_to_14bit(src: u32) -> u16 {
    static u16 downscale_32_to_14bit(u32 src)
    {
    return src >> 18;
    }
#[no_mangle]
unsafe extern "C" fn downscale_16_to_7bit(src: u16) -> u8 {
    static u8 downscale_16_to_7bit(u16 src)
    {
    return src >> 9;
    }
#[no_mangle]
unsafe extern "C" fn upscale_7_to_16bit(src: u8) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn upscale_7_to_32bit(src: u8) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn upscale_14_to_32bit(src: u16) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn cvt_ump_system_to_legacy(data: u32, buf: *mut c_uchar) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cvt_ump_midi1_to_legacy(data: u32, buf: *mut c_uchar) -> c_int {
...
// convert a UMP MIDI 2.0 Channel Voice message to MIDI 1.0 byte stream
    ...
    unsigned char *buf)
    {
    let mut status: c_uchar = midi2.note.status;
    let mut channel: c_uchar = midi2.note.channel;
    u16 v;
...
#[no_mangle]
unsafe extern "C" fn cvt_ump_sysex7_to_legacy(data: *const u32, buf: *mut c_uchar) -> c_int {
...
// convert MIDI 1.0 SysEx to a UMP packet
    ...
    {
    unsigned char status;
    let mut start: bool = cvt.in_sysex == 1;
    int i, offset;
    if (start && finish)
    status = UMP_SYSEX_STATUS_SINGLE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: start) -> else {
    else if (start)
    status = UMP_SYSEX_STATUS_START;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: finish) -> else {
    else if (finish)
    status = UMP_SYSEX_STATUS_END;
    else
...
#[no_mangle]
unsafe extern "C" fn reset_rpn(cc: *mut ump_cvt_to_ump_bank) {
