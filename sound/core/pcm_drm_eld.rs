...
//! Source: sound/core/pcm_drm_eld.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn sad_rate_mask(sad: *const u8) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn sad_max_channels(sad: *const u8) -> c_uint {
...
// TODO adjust for other compressed formats as well
    ...
    let mut rate_mask: c_uint = 7, i;
    const u8 *sad, *eld = rule.private;
    sad = drm_eld_sad(eld);
    if (sad) {
    c = hw_param_interval_c(params, SNDRV_PCM_HW_PARAM_CHANNELS);
    for (i = drm_eld_sad_count(eld); i > 0; i--, sad += 3) {
    let mut max_channels: unsigned = sad_max_channels(sad);
//
...
//
    ...
    let mut t: snd_interval = { .min = 1, .max = 2, .integer = 1, };
    unsigned int i;
    const u8 *sad, *eld = rule.private;
    sad = drm_eld_sad(eld);
    if (sad) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_pcm_hw_constraint_eld(runtime: *mut snd_pcm_runtime, eld: *mut c_void) -> c_int {
...
pub const SND_PRINT_RATES_ADVISED_BUFSIZE: c_int = 80;
...
//
#[no_mangle]
unsafe extern "C" fn hdmi_print_pcm_rates(pcm: c_int, buf: *mut c_char, buflen: c_int) {
...
#[no_mangle]
unsafe extern "C" fn eld_print_pcm_bits(pcm: c_int, buf: *mut c_char, buflen: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: a->max_bitrate) -> else {
...
#[no_mangle]
unsafe extern "C" fn snd_eld_print_channel_allocation(spk_alloc: c_int, buf: *mut c_char, buflen: c_int) {
