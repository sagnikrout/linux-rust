...
// Max allocation size for user controls.
    let mut max_user_ctl_alloc_size: static int = 8 * 1024 * 1024;
...
pub const MAX_CONTROL_COUNT: c_int = 1028;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct snd_kctl_ioctl {
    pub /: *mut *mut list_head list; / list of all ioctls,
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_empty_read_queue(ctl: *mut *mut snd_ctl_file) {
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_release(inode: *mut inode, file: *mut file) -> c_int {
    ...
    control.vd[idx].owner = core::ptr::null_mut();
    }
...
//
    ...
    {
    let mut id: snd_ctl_elem_id = kctl.id;
...
//
    ...
    err = snd_ctl_new(&kctl, count, access, core::ptr::null_mut());
    if (err < 0)
    return core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_find_hole(card: *mut snd_card, count: c_uint) -> c_int {
...
//
pub const MULTIPLIER: c_int = 37;
#[no_mangle]
unsafe extern "C" fn get_ctl_id_hash(id: *const snd_ctl_elem_id) -> c_ulong {
...
// add hash entries to numid and ctl xarray tables
    ...
    struct snd_kcontrol *kcontrol)
    {
    let mut id: snd_ctl_elem_id = kcontrol.id;
    int i;
...
//
    ...
    if (kctl.vd[idx].owner != core::ptr::null_mut() && kctl.vd[idx].owner != file)
    return -EBUSY;
    return snd_ctl_remove_locked(card, kctl);
    }
//
...
//
    ...
    return core::ptr::null_mut(); /* we can rely on only hash table */
...
// no matching in hash table - try all as the last resort
    ...
    if (n < 0) // mark the truncation with '>' before core::ptr::null_mut() terminator
...
// fill the remaining snd_ctl_elem_value data with the given pattern
    ...
    u32 pattern)
    {
    let mut offset: usize = value_sizes[info.type] * info.count;
...
// drop internal access flags
    ...
    unsigned int index_offset;
    struct snd_ctl_elem_info info;
    let mut pattern: u32 = 0xdeadbeef;
    int ret;
    guard(rwsem_read)(&card.controls_rwsem);
...
// info is needed only for validation
    ...
    info.id = control.id;
    ret = __snd_ctl_elem_info(card, kctl, &info, core::ptr::null_mut());
    if (ret < 0)
    return ret;
...
// With known state for both new and original, do the comparison.
    ...
    index_offset = snd_ctl_get_ioff(kctl, &control.id);
    vd = &kctl.vd[index_offset];
    if (!(vd.access & SNDRV_CTL_ELEM_ACCESS_WRITE) || kctl.put == core::ptr::null_mut() ||
...
// validate input values
    ...
    memset(&info, 0, sizeof(info));
    info.id = control.id;
    result = __snd_ctl_elem_info(card, kctl, &info, core::ptr::null_mut());
    if (!result)
...
pub struct user_element {
    ...
    pub /: *mut *mut *mut char elem_data; / element data,
    pub /: *mut *mut unsigned long elem_data_size; / size of element data in bytes,
    pub /: *mut *mut *mut void tlv_data; / TLV data,
    pub /: *mut *mut unsigned long tlv_data_size; / TLV data size,
    pub /: *mut *mut *mut void priv_data; / private data (like strings for enumerated type),
}

// check whether the addition (in bytes) of user ctl element may overflow the limit.
#[no_mangle]
unsafe extern "C" fn check_user_elem_overflow(card: *mut snd_card, add: isize) -> bool {
...
// uinfo = ue->info;
    ...
    {
    struct user_element *ue = snd_kcontrol_chip(kcontrol);
    let mut size: c_uint = ue.elem_data_size;
...
// does the TLV size change cause overflow?
    ...
    if (!change)
    return 0;
    if (ue.tlv_data == core::ptr::null_mut()) {
...
// decremented at private_free.
    ...
    {
    struct user_element *ue = snd_kcontrol_chip(kctl);
    if (ue.tlv_data_size == 0 || ue.tlv_data == core::ptr::null_mut())
    return -ENXIO;
...
// called in controls_rwsem write lock
#[no_mangle]
unsafe extern "C" fn snd_ctl_elem_init_enum_names(ue: *mut user_element) -> c_int {
    static int snd_ctl_elem_init_enum_names(struct user_element *ue)
    {
    size_t buf_len, name_len;
    unsigned int i;
    let mut user_ptrval: uintptr_t = ue.info.value.enumerated.names_ptr;
    lockdep_assert_held_write(&ue.card.controls_rwsem);
...
#[no_mangle]
unsafe extern "C" fn compute_user_elem_size(size: usize, count: c_uint) -> usize {
...
// pass NULL to card for suppressing error messages
    err = snd_ctl_check_elem_info(core::ptr::null_mut(), info);
    if (err < 0)
    return err;
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_subscribe_events(file: *mut snd_ctl_file, ptr: *mut int __user) -> c_int {
...
// Check support of the request for this element.
    ...
    return -ENXIO;
    if (kctl.tlv.c == core::ptr::null_mut())
    return -ENXIO;
// Write and command operations are not allowed for locked element.
    if (op_flag != SNDRV_CTL_TLV_OP_READ &&
    vd.owner != core::ptr::null_mut() && vd.owner != file)
    return -EPERM;
    ...
    return -ENXIO;
    if (kctl.tlv.p == core::ptr::null_mut())
    return -ENXIO;
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_poll(file: *mut file, wait: *mut *mut poll_table) -> __poll_t {
...
//
#[no_mangle]
unsafe extern "C" fn _snd_ctl_register_ioctl(fcn: snd_kctl_ioctl_func_t, lists: *mut list_head) -> c_int {
    static int _snd_ctl_register_ioctl(snd_kctl_ioctl_func_t fcn, struct list_head *lists)
    {
    struct snd_kctl_ioctl *pn;
    pn = kzalloc_obj(struct snd_kctl_ioctl);
    if (pn == core::ptr::null_mut())
    return -ENOMEM;
    pn.fioctl = fcn;
...
#[no_mangle]
unsafe extern "C" fn snd_ctl_fasync(fd: c_int, file: *mut *mut file, on: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_ctl_get_preferred_subdevice(card: *mut snd_card, type: c_int) -> c_int {
    int snd_ctl_get_preferred_subdevice(struct snd_card *card, int type)
    {
    struct snd_ctl_file *kctl;
    let mut subdevice: c_int = -1;
    guard(read_lock_irqsave)(&card.controls_rwlock);
...
pub unsafe extern "C" fn snd_ctl_request_layer(module_name: *const c_char) -> c_int {
    ...
    {
    struct snd_ctl_layer_ops *lops;
    if (module_name == core::ptr::null_mut())
    return 0;
...
pub unsafe extern "C" fn snd_ctl_disconnect_layer(lops: *mut snd_ctl_layer_ops) {
    ...
    for (lops2 = snd_ctl_layer, prev_lops2 = core::ptr::null_mut(); lops2; lops2 = lops2.next) {
    if (lops2 == lops) {
    if (!prev_lops2)
