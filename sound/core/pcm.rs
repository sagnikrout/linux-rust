...
#[no_mangle]
unsafe extern "C" fn snd_pcm_next(card: *mut snd_card, device: c_int) -> c_int {
    ...
    return pcm.device;
#[no_mangle]
pub unsafe extern "C" fn if(card->number: pcm->card->number >) -> else {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_add(newpcm: *mut snd_pcm) -> c_int {
...
//
    ...
    pstr.xrun_debug = simple_strtoul(line, core::ptr::null_mut(), 10);
    }

#[no_mangle]
unsafe extern "C" fn snd_pcm_stream_proc_init(pstr: *mut snd_pcm_str) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_stream_proc_done(pstr: *mut snd_pcm_str) -> c_int {
    static int snd_pcm_stream_proc_done(struct snd_pcm_str *pstr)
    {
    snd_info_free_entry(pstr.proc_root);
    pstr.proc_root = core::ptr::null_mut();
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_substream_proc_init(substream: *mut snd_pcm_substream) -> c_int {
    ...
    entry = create_substream_info_entry(substream, "xrun_injection", core::ptr::null_mut());
    if (entry) {
...
unsafe extern "C" fn do_pcm_suspend(dev: *mut device) -> c_int {
    ...
    SYSTEM_SLEEP_PM_OPS(do_pcm_suspend, core::ptr::null_mut())
    };
...
pub unsafe extern "C" fn snd_pcm_new_stream(pcm: *mut snd_pcm, stream: c_int, substream_count: c_int) -> c_int {
    ...
    for (idx = 0, prev = core::ptr::null_mut(); idx < substream_count; idx++) {
    ...
    if (prev == core::ptr::null_mut())
    pstr.substream = core::ptr::null_mut();
    else
...
#[no_mangle]
unsafe extern "C" fn free_chmap(pstr: *mut snd_pcm_str) {
    static void free_chmap(struct snd_pcm_str *pstr)
    {
    if (pstr.chmap_kctl) {
    struct snd_card *card = pstr.pcm.card;
    snd_ctl_remove(card, pstr.chmap_kctl);
    pstr.chmap_kctl = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn snd_pcm_free_stream(pstr: *mut *mut snd_pcm_str) {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_free(pcm: *mut snd_pcm) -> c_int {
    static int snd_pcm_free(struct snd_pcm *pcm)
    {
    if (!pcm)
    return 0;
...
// rsubstream = NULL;
    pstr = &pcm.streams[stream];
    if (pstr.substream == core::ptr::null_mut() || pstr.substream_count == 0)
    return -ENODEV;
    card = pcm.card;
    prefer_subdevice = snd_ctl_get_preferred_subdevice(card, SND_CTL_SUBDEV_PCM);
    if (pcm.info_flags & SNDRV_PCM_INFO_HALF_DUPLEX) {
    let mut opposite: c_int = !stream;
...
// rsubstream = substream;
    ...
    if (runtime == core::ptr::null_mut())
    return -ENOMEM;
    size = PAGE_ALIGN(sizeof(struct snd_pcm_mmap_status));
    runtime.status = alloc_pages_exact(size, GFP_KERNEL);
    if (runtime.status == core::ptr::null_mut()) {
    kfree(runtime);
    return -ENOMEM;
    }
    ...
    if (runtime.control == core::ptr::null_mut()) {
...
pub unsafe extern "C" fn snd_pcm_detach_substream(substream: *mut snd_pcm_substream) {
    ...
    return;
    runtime = substream.runtime;
    if (runtime.private_free != core::ptr::null_mut())
...
unsafe extern "C" fn snd_pcm_dev_register(device: *mut snd_device) -> c_int {
    ...
    let mut devtype: c_int = -1;
    if (pcm.streams[cidx].substream == core::ptr::null_mut())
    continue;
    switch (cidx) {
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_proc_init() {
    static void snd_pcm_proc_init(void)
    {
    struct snd_info_entry *entry;
    entry = snd_info_create_module_entry(THIS_MODULE, "pcm", core::ptr::null_mut());
    if (entry) {
    snd_info_set_text_ops(entry, core::ptr::null_mut(), snd_pcm_proc_read);
...
#[no_mangle]
unsafe extern "C" fn snd_pcm_proc_done() {
    static void snd_pcm_proc_done(void)
    {
    snd_info_free_entry(snd_pcm_proc_entry);
    }

// Macro flag: #define snd_pcm_proc_init()
// Macro flag: #define snd_pcm_proc_done()

//
// ENTRY functions
//
#[no_mangle]
unsafe extern "C" fn alsa_pcm_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn alsa_pcm_exit() -> void __exit {
