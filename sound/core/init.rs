...
pub struct snd_monitor_file {
    ...
    pub /: *mut *mut list_head shutdown_list; / still need to shutdown,
    pub /: *mut *mut list_head list; / link of monitor files,
}
...
// locked for registering/using
    ...
    module_param_array(slots, charp, core::ptr::null_mut(), 0444);
    MODULE_PARM_DESC(slots, "Module names assigned to the slots.");
// return non-zero if the given index is reserved for the given
// module via slots option
//
#[no_mangle]
unsafe extern "C" fn module_slot_match(module: *mut module, idx: c_int) -> c_int {
...
//
    for (;;) {
    let mut c1: c_char = *s1++;
    let mut c2: c_char = *s2++;
    if (c1 == '-')
    c1 = '_';
    if (c2 == '-')
    c2 = '_';
...
#[no_mangle]
unsafe extern "C" fn check_empty_slot(module: *mut module, slot: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_device_alloc(dev_p: *mut device, card: *mut snd_card) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __snd_card_release(dev: *mut device, data: *mut c_void) {
...
pub unsafe extern "C" fn snd_card_free_on_error(dev: *mut device, ret: c_int) -> c_int {
    ...
    if (!ret)
    return 0;
    card = devres_find(dev, __snd_card_release, core::ptr::null_mut(), core::ptr::null_mut());
    if (card)
    snd_card_free(card);
    return ret;
    }
    ...
    err = -ENODEV;
#[no_mangle]
pub unsafe extern "C" fn if(snd_ecards_limit: idx <) -> else {
...
#[no_mangle]
unsafe extern "C" fn snd_disconnect_llseek(file: *mut file, offset: loff_t, orig: c_int) -> loff_t {
...
unsafe extern "C" fn snd_disconnect_release(inode: *mut inode, file: *mut file) -> c_int {
    ...
    {
    struct snd_monitor_file *df = core::ptr::null_mut(), *_df;
...
#[no_mangle]
unsafe extern "C" fn snd_disconnect_poll(file: *mut *mut file, wait: *mut *mut poll_table) -> __poll_t {
...
#[no_mangle]
unsafe extern "C" fn snd_disconnect_mmap(file: *mut file, vma: *mut vm_area_struct) -> c_int {
    static int snd_disconnect_mmap(struct file *file, struct vm_area_struct *vma)
    {
    return -ENODEV;
    }
#[no_mangle]
unsafe extern "C" fn snd_disconnect_fasync(fd: c_int, file: *mut file, on: c_int) -> c_int {
...
// notify all devices that we are disconnected
    ...
    card.debugfs_root = core::ptr::null_mut();
...
// disable fops (user space) operations for ALSA API
    scoped_guard(mutex, &snd_card_mutex) {
    snd_cards[card.number] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn snd_card_do_free(card: *mut snd_card) -> c_int {
    static int snd_card_do_free(struct snd_card *card)
    {
    let mut managed: bool = card.managed;
    card.releasing = true;
    ...
    card.components = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn safe_ascii_char(c: c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn card_id_ok(card: *mut snd_card, id: *const c_char) -> bool {
...
// check if user specified own card->id
    ...
    buf1[copy] = '\0';
    guard(mutex)(&snd_card_mutex);
    if (!card_id_ok(core::ptr::null_mut(), buf1))
    return -EEXIST;
...
#[no_mangle]
pub unsafe extern "C" fn snd_card_info_init() -> int __init {
    int __init snd_card_info_init(void)
    {
    struct snd_info_entry *entry;
    entry = snd_info_create_module_entry(THIS_MODULE, "cards", core::ptr::null_mut());
    if (! entry)
    return -ENOMEM;
    ...
    entry = snd_info_create_module_entry(THIS_MODULE, "modules", core::ptr::null_mut());
    if (!entry)
    return -ENOMEM;
...
pub unsafe extern "C" fn snd_component_add(card: *mut snd_card, component: *const c_char) -> c_int {
    ...
    {
    char *ptr;
    let mut len: c_int = strlen(component);
    ...
    let mut new_alloc: c_uint = roundup(need_len, 32);
...
pub unsafe extern "C" fn snd_card_file_add(card: *mut snd_card, file: *mut file) -> c_int {
    ...
    if (mfile == core::ptr::null_mut())
    return -ENOMEM;
    mfile.file = file;
    mfile.disconnected_f_op = core::ptr::null_mut();
...
pub unsafe extern "C" fn snd_card_file_remove(card: *mut snd_card, file: *mut file) -> c_int {
    ...
    {
    struct snd_monitor_file *mfile, *found = core::ptr::null_mut();
    scoped_guard(spinlock, &card.files_lock) {
    list_for_each_entry(mfile, &card.files_list, list) {
    if (mfile.file == file) {
    list_del(&mfile.list);
