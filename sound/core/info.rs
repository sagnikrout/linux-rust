...
#[no_mangle]
pub unsafe extern "C" fn snd_info_check_reserved_words(str: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn valid_pos(pos: loff_t, count: usize) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn snd_info_entry_llseek(file: *mut file, offset: loff_t, orig: c_int) -> loff_t {
...
// offset = pos + size;
    ...
    return -EIO;
    if (count > 0) {
    let mut maxsize: usize = entry.size - pos;
...
#[no_mangle]
unsafe extern "C" fn snd_info_entry_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_info_seq_show(seq: *mut seq_file, p: *mut c_void) -> c_int {
...
unsafe extern "C" fn snd_info_text_entry_release(inode: *mut inode, file: *mut file) -> c_int {
    ...
    {
    struct snd_info_entry *entry;
    entry = snd_info_create_module_entry(mod, name, core::ptr::null_mut());
    if (!entry)
...
#[no_mangle]
pub unsafe extern "C" fn snd_info_init() -> int __init {
    int __init snd_info_init(void)
    {
    snd_proc_root = snd_info_create_entry("asound", core::ptr::null_mut(), THIS_MODULE);
    if (!snd_proc_root)
    return -ENOMEM;
    snd_proc_root.mode = S_IFDIR | 0555;
    snd_proc_root.p = proc_mkdir("asound", core::ptr::null_mut());
    if (!snd_proc_root.p)
    goto error;
...
#[no_mangle]
pub unsafe extern "C" fn snd_info_done() -> int __exit {
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_info_get_line(buffer: *mut snd_info_buffer, line: *mut c_char, len: c_int) -> c_int {
...
//
    ...
    if (entry.name == core::ptr::null_mut()) {
    kfree(entry);
    return core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_info_free_entry(entry: *mut *mut snd_info_entry) {
...
unsafe extern "C" fn __snd_info_register(entry: *mut snd_info_entry) -> c_int {
    ...
    {
    struct proc_dir_entry *root, *p = core::ptr::null_mut();
    if (snd_BUG_ON(!entry))
    return -ENXIO;
    root = entry.parent == core::ptr::null_mut() ? snd_proc_root.p : entry.parent.p;
    guard(mutex)(&info_mutex);
    if (entry.p || !root)
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn snd_info_version_read(entry: *mut snd_info_entry, buffer: *mut snd_info_buffer) {
...
#[no_mangle]
unsafe extern "C" fn snd_info_version_init() -> int __init {
    static int __init snd_info_version_init(void)
    {
    struct snd_info_entry *entry;
    entry = snd_info_create_module_entry(THIS_MODULE, "version", core::ptr::null_mut());
    if (entry == core::ptr::null_mut())
    return -ENOMEM;
