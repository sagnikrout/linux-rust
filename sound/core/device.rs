...
#[no_mangle]
unsafe extern "C" fn __snd_device_disconnect(dev: *mut snd_device) {
...
#[no_mangle]
unsafe extern "C" fn __snd_device_free(dev: *mut snd_device) {
...
#[no_mangle]
unsafe extern "C" fn __snd_device_register(dev: *mut snd_device) -> c_int {
    static int __snd_device_register(struct snd_device *dev)
    {
    if (dev.state == SNDRV_DEV_BUILD) {
    if (dev.ops.dev_register) {
    let mut err: c_int = dev.ops.dev_register(dev);
    if (err < 0)
    return err;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn snd_device_register(card: *mut snd_card, device_data: *mut c_void) -> c_int {
