...
//! Source: sound/pci/emu10k1/emufx.c
#![no_std]
...
// Macro flag: #define EMU10K1_CAPTURE_DIGITAL_OUT

// Macro flag: #define EMU10K1_SET_AC3_IEC958

// Macro flag: #define EMU10K1_CENTER_LFE_FROM_FRONT
...
//
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_gpr_ctl_info(kcontrol: *mut snd_kcontrol, uinfo: *mut snd_ctl_elem_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_gpr_ctl_get(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_gpr_ctl_put(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_fx8010_interrupt(emu: *mut snd_emu10k1) {
    ...
    irq.next = core::ptr::null_mut();
    guard(spinlock_irqsave)(&emu.fx8010.irq_lock);
    if (emu.fx8010.irq_handlers == core::ptr::null_mut()) {
    ...
    emu.dsp_interrupt = core::ptr::null_mut();
    }
    } else {
...
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_efx_write(emu: *mut snd_emu10k1, pc: c_uint, data: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_emu10k1_efx_read(emu: *mut snd_emu10k1, pc: c_uint) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: get_user(val, )&icode->gpr_map[gpr]): *mut (__user u32) -> else {
...
pub const MAX_TLV_SIZE: c_int = 256;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: copy_from_user(data, _arg: _tlv, _arg: sizeof(data))) -> else {
...
// dst = src[idx];
#[no_mangle]
pub unsafe extern "C" fn if(_arg: copy_from_user(dst, _arg: &_src[idx], _arg: *mut sizeof(src))) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: copy_from_user(dst, _arg: &_octl[idx], _arg: *mut sizeof(octl))) -> else {
    else if (copy_from_user(dst, &_octl[idx], sizeof(*octl)))
    return -EFAULT;
    dst.tlv = core::ptr::null_mut();
    return 0;
    }
...
// ret = list[i];
#[no_mangle]
pub unsafe extern "C" fn if(_arg: copy_from_user(ret, _arg: _id, _arg: *mut sizeof(ret))) -> else {
    ...
    if (err < 0)
    return err;
    if (snd_emu10k1_look_for_ctl(emu, &id) == core::ptr::null_mut())
    return -ENOENT;
    }
...
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_ctl_private_free(kctl: *mut snd_kcontrol) {
...
pub const SND_EMU10K1_GPR_CONTROLS: c_int = 44;
pub const SND_EMU10K1_INPUTS: c_int = 12;
pub const SND_EMU10K1_PLAYBACK_CHANNELS: c_int = 8;
pub const SND_EMU10K1_CAPTURE_CHANNELS: c_int = 4;
...
//
#[no_mangle]
unsafe extern "C" fn _snd_emu10k1_audigy_init_efx(emu: *mut snd_emu10k1) -> c_int {
...
// down for center/lfe. But it works for stereo/quadro, so whatever.
    ...
    let mut gpr: c_int = num_static_gprs;
    u32 ptr, ptr_skip;
    struct snd_emu10k1_fx8010_code *icode = core::ptr::null_mut();
    struct snd_emu10k1_fx8010_control_gpr *controls = core::ptr::null_mut(), *ctl;
    u32 *gpr_map;
    err = -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn _volume(icode: *mut snd_emu10k1_fx8010_code, ptr: *mut u32, dst: u32, src: u32, vol: u32) {
...
#[no_mangle]
unsafe extern "C" fn _volume_add(icode: *mut snd_emu10k1_fx8010_code, ptr: *mut u32, dst: u32, src: u32, vol: u32) {
...
#[no_mangle]
unsafe extern "C" fn _snd_emu10k1_init_efx(emu: *mut snd_emu10k1) -> c_int {
    static int _snd_emu10k1_init_efx(struct snd_emu10k1 *emu)
    {
    int err, i, z, gpr, tmp, playback, capture;
    u32 ptr, ptr_skip;
    struct snd_emu10k1_fx8010_code *icode;
    struct snd_emu10k1_fx8010_pcm_rec *ipcm = core::ptr::null_mut();
    struct snd_emu10k1_fx8010_control_gpr *controls = core::ptr::null_mut(), *ctl;
    u32 *gpr_map;
    err = -ENOMEM;
    icode = kzalloc_obj(*icode);
...
//
    ...
    ctl.tlv = snd_emu10k1_bass_treble_db_scale;
    ctl.translation = EMU10K1_GPR_TRANSLATION_TREBLE;
pub const BASS_GPR: c_uint = 0x8c;
pub const TREBLE_GPR: c_uint = 0x96;
    for (z = 0; z < 5; z++) {
    int j;
    for (j = 0; j < 2; j++) {
...
// EFX capture - capture the 16 EXTINS
    ...
    for (z = 0; z < 16; z++) {
    let mut c: i8 = snd_emu10k1_sblive51_fxbus2_map[z];
    if (c != -1)
...
#[no_mangle]
pub unsafe extern "C" fn snd_emu10k1_fx8010_tone_control_activate(emu: *mut snd_emu10k1, output: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_emu10k1_fx8010_tone_control_deactivate(emu: *mut snd_emu10k1, output: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_emu10k1_fx8010_tram_setup(emu: *mut snd_emu10k1, size: u32) -> c_int {
...
// size is in samples
    ...
    if (emu.fx8010.etram_pages.area != core::ptr::null_mut()) {
    snd_dma_free_pages(&emu.fx8010.etram_pages);
    emu.fx8010.etram_pages.area = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_fx8010_open(hw: *mut *mut snd_hwdep, file: *mut file) -> c_int {
    static int snd_emu10k1_fx8010_open(struct snd_hwdep * hw, struct file *file)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn copy_string(dst: *mut c_char, src: *const c_char, null: *const c_char, idx: c_int) {
    ...
    copy_string(info.extin_names[res], extin_mask & (1 << res) ? *extin : core::ptr::null_mut(), "Unused", res);
    copy_string(info.extout_names[res], extout_mask & (1 << res) ? *extout : core::ptr::null_mut(), "Unused", res);
    }
...
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_fx8010_ioctl(hw: *mut *mut snd_hwdep, file: *mut file, cmd: c_uint, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_emu10k1_fx8010_release(hw: *mut *mut snd_hwdep, file: *mut file) -> c_int {
