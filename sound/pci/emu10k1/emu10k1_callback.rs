...
//! Source: sound/pci/emu10k1/emu10k1_callback.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn if(SNDRV_EMUX_ST_STANDBY: state ==) -> else {
    else if (state == SNDRV_EMUX_ST_STANDBY)
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(SNDRV_EMUX_ST_ON: state &) -> else {
...
// allocate a voice
    ...
    if (snd_emu10k1_voice_alloc(hw, EMU10K1_SYNTH, 1, 1, core::ptr::null_mut(), &hwvoice) < 0)
    continue;
...
// A = left(0), B = right(1), C = reverb(c), D = chorus(d)
    ...
    let mut shift: c_uint = (vp.apitch - 0xe000) >> 10;
    ccca |= shift << 25;
    }
    if (!w_16)
...
#[no_mangle]
unsafe extern "C" fn get_pitch_shift(emu: *mut snd_emux) -> c_int {
    static int get_pitch_shift(struct snd_emux *emu)
    {
