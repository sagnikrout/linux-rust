...
//! Source: sound/pci/emu10k1/emuproc.c
#![no_std]
...
// From ((Rate * 48000 ) / 262144);
    ...
    let mut extin_mask: c_ushort = emu.audigy ? ~0 : emu.fx8010.extin_mask;
    let mut extout_mask: c_ushort = emu.audigy ? ~0 : emu.fx8010.extout_mask;
    unsigned int val, val1, ptrx, psst, dsl, snda;
    let mut nefx: c_int = emu.audigy ? 32 : 16;
    int idx;
...
// Audigy actually has 64, but we don't use them all.
    ...
    let mut c: i8 = snd_emu10k1_sblive51_fxbus2_map[idx];
    if (c == -1)
    snd_iprintf(buffer, "  Channel %02i [Output: %s]\n",
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct emu10k1_reg_entry {
    pub size: unsigned short base,,
    ...
    { 0xa0, 0x10, "EMU32OUTH" },
    { 0xb0, 0x10, "EMU32OUTL" },
    { 0xc0, 0x20, core::ptr::null_mut() },  // Constants
...
// { 0x1c0, 0x40, "ETRAM_CTL" },
    ...
    {
    for (int i = 0; ; i++) {
    let mut base: unsigned = entries[i].base;
    let mut size: unsigned = entries[i].size;
    if (!size)
...
#[no_mangle]
unsafe extern "C" fn disasm_sblive_reg(buffer: *mut c_char, reg: unsigned, pfx: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn disasm_audigy_reg(buffer: *mut c_char, reg: unsigned, pfx: *const c_char) -> c_int {
    ...
    let mut nspaces: c_int = sizeof(spaces) - 1;
    ...
    u32 dst)
    {
    let mut src: u32 = snd_emu1010_fpga_link_dst_src_read(emu, dst);
