...
//! Source: sound/pci/asihpi/asihpi.c
#![no_std]
...
//
    ...
    let mut enable_hpi_hwdep: static bool = 1;
...
pub const DEFAULT_SAMPLERATE: c_int = 44100;
    let mut adapter_fs: static int = DEFAULT_SAMPLERATE;
// defaults
pub const PERIODS_MIN: c_int = 2;
pub const PERIOD_BYTES_MIN: c_int = 2048;
...
pub struct clk_cache {
    pub count: c_int,
    pub has_local: c_int,
    pub s: [clk_source; MAX_CLOCKSOURCES],
}
...
//
    pub llmode_streampriv: *mut snd_card_asihpi_pcm,
    pub substream): *mut *mut void (pcm_start)(struct snd_pcm_substream,
    pub substream): *mut *mut void (pcm_stop)(struct snd_pcm_substream,
    pub h_mixer: u32,
    pub cc: clk_cache,
    pub can_dma: u16,
    pub support_grouping: u16,
    pub support_mrx: u16,
    pub update_interval_frames: u16,
    pub in_max_chans: u16,
    pub out_max_chans: u16,
    pub in_min_chans: u16,
    pub out_min_chans: u16,
}
...
pub struct snd_card_asihpi_pcm {
    ...
    pub respawn_timer: c_uint,
    pub hpi_buffer_attached: c_uint,
    pub buffer_bytes: c_uint,
    pub period_bytes: c_uint,
    pub bytes_per_sec: c_uint,
    pub /: *mut *mut unsigned int pcm_buf_host_rw_ofs; / Host R/W pos,
    pub /: *mut *mut unsigned int pcm_buf_dma_ofs; / DMA R/W offset in buffer,
    pub /: *mut *mut unsigned int pcm_buf_elapsed_dma_ofs; / DMA R/W offset in buffer,
    pub drained_count: c_uint,
...
// Functions to allow driver to give a buffer to HPI for busmastering
    ...
    let mut obj: c_uint = hpi_handle_object(h_stream);
    if (!h_stream)
...
#[no_mangle]
pub unsafe extern "C" fn hpi_stream_start(h_stream: u32) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn hpi_stream_stop(h_stream: u32) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn hpi_stream_group_reset(h_stream: u32) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn handle_error(err: u16, line: c_int, filename: *mut c_char) -> u16 {
...
// hpi_format = 0;
    ...
    u32 sample_rate;
    int idx;
    let mut rate_min: c_uint = 200000;
...
// on cards without SRC,
    ...
    hpi_stream_get_info_ex(dpcm.h_stream, core::ptr::null_mut(),
    &dpcm.hpi_buffer_attached, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_pcm_timer_stop(substream: *mut snd_pcm_substream) {
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_pcm_int_start(substream: *mut snd_pcm_substream) {
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_pcm_int_stop(substream: *mut snd_pcm_substream) {
    ...
    card.llmode_streampriv = core::ptr::null_mut();
    }
...
// data??
//
    let mut preload: c_uint = ds.period_bytes * 1;
...
//
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_timer_function(t: *mut timer_list) {
...
// Limit use of on card fifo for playback
    ...
    {
    let mut buf_ofs: c_uint = ds.pcm_buf_host_rw_ofs % ds.buffer_bytes;
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_playback_open(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_playback_close(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_capture_prepare(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_capture_open(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_capture_close(substream: *mut snd_pcm_substream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_pcm_new(asihpi: *mut snd_card_asihpi, device: c_int) -> c_int {
...
pub struct hpi_control {
    ...
    pub band: u16,
    pub /: *mut *mut char name[SNDRV_CTL_ELEM_ID_NAME_MAXLEN]; / copied to snd_ctl_elem_id.name[44];,
}
...
// Number of strings must match the enumerations for HPI_DESTNODES in hpi.h
    ...
    return err;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mixer_dump) -> else {
    else if (mixer_dump)
...
#[no_mangle]
pub unsafe extern "C" fn if(HPI_DESTNODE_ISTREAM: hpi_ctl->dst_node_type + HPI_DESTNODE_NONE ==) -> else {
    ...
    dir, name);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: hpi_ctl->dst_node_type) -> else {
    len = snprintf(hpi_ctl.name, sizeof(hpi_ctl.name),
    "%s %d %s%s",
    asihpi_dst_names[hpi_ctl.dst_node_type],
    hpi_ctl.dst_node_index,
...
// ------------------------------------------------------------
    Volume controls
    ------------------------------------------------------------*/
pub const VOL_STEP_mB: c_int = 1;
    static int snd_asihpi_volume_info(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_info *uinfo)
    {
    let mut h_control: u32 = kcontrol.private_value;
    u32 count;
    u16 err;
...
//
    let mut mute: c_int = ucontrol.value.integer.value[0] ? 0 : HPI_BITMASK_ALL_CHANNELS;
...
// return success but set the control to N/A
    ...
    let mut h_control: u32 = kcontrol.private_value;
// default to S/PDIF
    let mut source: u16 = HPI_AESEBU_FORMAT_SPDIF;
...
// Freq
    ...
    u16 err;
    u16 tuner_bands[HPI_TUNER_BAND_LAST];
    let mut num_bands: u16 = 0, band_iter, idx;
...
// Tuner control group initializer
    ...
    snd_control.private_value = hpi_ctl.h_control;
    snd_control.access = SNDRV_CTL_ELEM_ACCESS_READWRITE;
    if (!hpi_tuner_get_gain(hpi_ctl.h_control, core::ptr::null_mut())) {
...
// ------------------------------------------------------------
    ...
    ------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_mux_count_sources(snd_control: *mut snd_kcontrol) -> c_int {
    static int snd_card_asihpi_mux_count_sources(struct snd_kcontrol *snd_control)
    {
    let mut h_control: u32 = snd_control.private_value;
    struct hpi_control hpi_ctl;
    int s, err;
    for (s = 0; s < 32; s++) {
...
//
    ...
    let mut hSC: u32 = hpi_ctl.h_control;
    let mut has_aes_in: c_int = 0;
    int i, j;
    u16 source;
...
// ------------------------------------------------------------
    ...
    ------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_card_asihpi_mixer_new(asihpi: *mut snd_card_asihpi) -> c_int {
    static int snd_card_asihpi_mixer_new(struct snd_card_asihpi *asihpi)
    {
    struct snd_card *card;
    let mut idx: c_uint = 0;
    let mut subindex: c_uint = 0;
    int err;
...
#[no_mangle]
unsafe extern "C" fn snd_asihpi_proc_init(asihpi: *mut snd_card_asihpi) {
...
// ------------------------------------------------------------
    ...
    ------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_asihpi_hpi_open(hw: *mut snd_hwdep, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_asihpi_hpi_release(hw: *mut snd_hwdep, file: *mut file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn snd_asihpi_hpi_new(asihpi: *mut snd_card_asihpi, device: c_int) -> c_int {
...
// if that fails, try the default index==next available
    ...
    core::ptr::null_mut(), &asihpi.support_grouping);
    if (err)
    asihpi.support_grouping = 0;
    err = hpi_adapter_get_property(adapter_index,
    HPI_ADAPTER_PROPERTY_CAPS2,
    &asihpi.support_mrx, core::ptr::null_mut());
    if (err)
    asihpi.support_mrx = 0;
    err = hpi_adapter_get_property(adapter_index,
    HPI_ADAPTER_PROPERTY_INTERVAL,
    core::ptr::null_mut(), &asihpi.update_interval_frames);
    if (err)
...
#[no_mangle]
unsafe extern "C" fn snd_asihpi_remove(pci_dev: *mut pci_dev) {
    static void snd_asihpi_remove(struct pci_dev *pci_dev)
    {
    struct hpi_adapter *hpi = pci_get_drvdata(pci_dev);
// Stop interrupts
    if (hpi.interrupt_mode) {
    hpi.interrupt_callback = core::ptr::null_mut();
    hpi_handle_error(hpi_adapter_set_property(hpi.adapter.index,
    HPI_ADAPTER_PROPERTY_IRQ_RATE, 0, 0));
    }
    snd_card_free(hpi.snd_card);
    hpi.snd_card = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn snd_asihpi_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn snd_asihpi_exit() -> void __exit {
