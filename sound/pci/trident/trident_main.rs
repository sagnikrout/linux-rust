...
//! Source: sound/pci/trident/trident_main.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn snd_trident_print_voice_regs(trident: *mut snd_trident, voice: c_int) {
...
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_codec_read(ac97: *mut snd_ac97, reg: c_ushort) -> c_ushort {
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_codec_read(ac97: *mut snd_ac97, reg: c_ushort) -> c_ushort {
    static unsigned short snd_trident_codec_read(struct snd_ac97 *ac97, unsigned short reg)
    {
    let mut data: c_uint = 0, treg;
...
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_enable_eso(trident: *mut snd_trident) {
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_enable_eso(trident: *mut *mut snd_trident) {
...
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_disable_eso(trident: *mut snd_trident) {
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_disable_eso(trident: *mut *mut snd_trident) {
...
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_start_voice(trident: *mut *mut snd_trident, voice: c_uint) {
...
pub unsafe extern "C" fn snd_trident_start_voice(trident: *mut *mut snd_trident, voice: c_uint) {
    void snd_trident_start_voice(struct snd_trident * trident, unsigned int voice)
    {
    let mut mask: c_uint = 1 << (voice & 0x1f);
    let mut reg: c_uint = (voice & 0x20) ? T4D_START_B : T4D_START_A;
    outl(mask, TRID_REG(trident, reg));
    }
    EXPORT_SYMBOL(snd_trident_start_voice);
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_stop_voice(trident: *mut *mut snd_trident, voice: c_uint) {
...
pub unsafe extern "C" fn snd_trident_stop_voice(trident: *mut *mut snd_trident, voice: c_uint) {
    ...
    {
    let mut mask: c_uint = 1 << (voice & 0x1f);
    let mut reg: c_uint = (voice & 0x20) ? T4D_STOP_B : T4D_STOP_A;
    outl(mask, TRID_REG(trident, reg));
    }
    EXPORT_SYMBOL(snd_trident_stop_voice);
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_allocate_pcm_channel(trident: *mut snd_trident) -> c_int {
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_allocate_pcm_channel(trident: *mut *mut snd_trident) -> c_int {
...
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_free_pcm_channel(channel: c_int) {
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_free_pcm_channel(trident: *mut snd_trident, channel: c_int) {
...
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_allocate_synth_channel() -> c_uint {
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_allocate_synth_channel(trident: *mut *mut snd_trident) -> c_int {
...
// ---------------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn snd_trident_free_synth_channel(channel: c_int) {
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_free_synth_channel(trident: *mut snd_trident, channel: c_int) {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_convert_rate(rate: c_uint) -> c_uint {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_convert_adc_rate(rate: c_uint) -> c_uint {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_control_mode(substream: *mut snd_pcm_substream) -> c_uint {
...
// ---------------------------------------------------------------------------
    ...
    if (voice.memblk == core::ptr::null_mut())
    return -ENOMEM;
    }
    }
    return 0;
...
// voice management
    ...
    voice.extra = evoice = core::ptr::null_mut();
    }
    }
    return 0;
    }
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_hw_free(substream: *mut snd_pcm_substream) -> c_int {
    static int snd_trident_hw_free(struct snd_pcm_substream *substream)
    {
    struct snd_trident *trident = snd_pcm_substream_chip(substream);
    struct snd_pcm_runtime *runtime = substream.runtime;
    struct snd_trident_voice *voice = runtime.private_data;
    struct snd_trident_voice *evoice = voice ? voice.extra : core::ptr::null_mut();
    if (trident.tlb.entries) {
    if (voice && voice.memblk) {
    snd_trident_free_pages(trident, voice.memblk);
    voice.memblk = core::ptr::null_mut();
    }
    }
    if (evoice != core::ptr::null_mut()) {
    snd_trident_free_voice(trident, evoice);
    voice.extra = core::ptr::null_mut();
    }
    return 0;
    }
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_playback_prepare(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_capture_prepare(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_si7018_capture_hw_free(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_si7018_capture_prepare(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_foldback_prepare(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    {
    struct snd_trident *trident = snd_pcm_substream_chip(substream);
    let mut old_bits: c_uint = 0, change = 0;
    int err;
    err = snd_trident_allocate_pcm_mem(substream, hw_params);
    if (err < 0)
    return err;
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_spdif_prepare(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_playback_pointer(substream: *mut snd_pcm_substream) -> snd_pcm_uframes_t {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_capture_pointer(substream: *mut snd_pcm_substream) -> snd_pcm_uframes_t {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_spdif_pointer(substream: *mut snd_pcm_substream) -> snd_pcm_uframes_t {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_pcm_free_substream(runtime: *mut snd_pcm_runtime) {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_playback_open(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_playback_close(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_spdif_open(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_spdif_close(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_capture_open(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_capture_close(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_foldback_open(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_foldback_close(substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
pub unsafe extern "C" fn snd_trident_pcm(trident: *mut snd_trident, device: c_int) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
pub unsafe extern "C" fn snd_trident_foldback_pcm(trident: *mut snd_trident, device: c_int) -> c_int {
    int snd_trident_foldback_pcm(struct snd_trident *trident, int device)
    {
    struct snd_pcm *foldback;
    int err;
    let mut num_chan: c_int = 3;
    struct snd_pcm_substream *substream;
    if (trident.device == TRIDENT_DEVICE_ID_NX)
    num_chan = 4;
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
pub unsafe extern "C" fn snd_trident_spdif_pcm(trident: *mut snd_trident, device: c_int) -> c_int {
...
// S/PDIF C Channel bits 0-31 : 48khz, SCMS disabled
    ...
    if (trident.spdif == core::ptr::null_mut()) {
    unsigned int temp;
    outl(trident.spdif_bits, TRID_REG(trident, SI_SPDIF_CS));
...
// ---------------------------------------------------------------------------
    ...
    change = val != mix.vol;
    mix.vol = val;
    if (mix.voice != core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn snd_trident_pcm_mixer_free(trident: *mut snd_trident, voice: *mut snd_trident_voice, substream: *mut snd_pcm_substream) -> c_int {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_mixer(trident: *mut snd_trident, pcm_spdif_device: c_int) -> c_int {
    ...
    if (!uctl)
    return -ENOMEM;
    err = snd_ac97_bus(trident.card, 0, &ops, core::ptr::null_mut(), &trident.ac97_bus);
    if (err < 0)
    goto __out;
...
#[no_mangle]
unsafe extern "C" fn snd_trident_gameport_read(gameport: *mut gameport) -> c_uchar {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_gameport_trigger(gameport: *mut gameport) {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_gameport_cooked_read(gameport: *mut gameport, axes: *mut c_int, buttons: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_gameport_open(gameport: *mut gameport, mode: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_trident_create_gameport(chip: *mut snd_trident) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_trident_free_gameport(chip: *mut snd_trident) {
...
//
#[no_mangle]
pub unsafe extern "C" fn do_delay(chip: *mut snd_trident) {
...
//
#[no_mangle]
unsafe extern "C" fn snd_trident_sis_reset(trident: *mut snd_trident) -> c_int {
    static int snd_trident_sis_reset(struct snd_trident *trident)
    {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_proc_init(trident: *mut snd_trident) {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_tlb_alloc(trident: *mut snd_trident) -> c_int {
...
// use emu memory block manager code to manage tlb page allocation
    ...
    if (trident.tlb.memhdr == core::ptr::null_mut())
    return -ENOMEM;
...
//
#[no_mangle]
unsafe extern "C" fn snd_trident_stop_all_voices(trident: *mut snd_trident) {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_4d_dx_init(trident: *mut snd_trident) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn snd_trident_4d_nx_init(trident: *mut snd_trident) -> c_int {
...
// disable SB IRQ
    ...
    snd_trident_stop_all_voices(trident);
    if (trident.tlb.entries != core::ptr::null_mut()) {
    unsigned int i;
...
//
#[no_mangle]
unsafe extern "C" fn snd_trident_sis_init(trident: *mut snd_trident) -> c_int {
...
// allocate 16k-aligned TLB for NX cards
    trident.tlb.entries = core::ptr::null_mut();
    if (trident.device == TRIDENT_DEVICE_ID_NX) {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_free(card: *mut snd_card) {
    static void snd_trident_free(struct snd_card *card)
    {
    struct snd_trident *trident = card.private_data;
...
#[no_mangle]
pub unsafe extern "C" fn if(TRIDENT_DEVICE_ID_SI7018: trident->device ==) -> else {
...
// ---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------*/
#[no_mangle]
unsafe extern "C" fn snd_trident_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t snd_trident_interrupt(int irq, void *dev_id)
    {
...
// get interrupt status for all channels
    ...
    if (!voice.pcm || voice.substream == core::ptr::null_mut()) {
...
// outl((ST_TARGET_REACHED | MIXER_OVERFLOW | MIXER_UNDERFLOW), TRID_REG(trident, T4D_MISCINT));
    ...
    pvoice.capture = 0;
    pvoice.spdif = 0;
    pvoice.memblk = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn snd_trident_free_voice(trident: *mut *mut snd_trident, voice: *mut snd_trident_voice) {
    void snd_trident_free_voice(struct snd_trident * trident, struct snd_trident_voice *voice)
    {
    void (*private_free)(struct snd_trident_voice *);
    if (voice == core::ptr::null_mut() || !voice.use)
    return;
    snd_trident_clear_voices(trident, voice.number, voice.number);
    scoped_guard(spinlock_irqsave, &trident.voice_alloc) {
    private_free = voice.private_free;
    voice.private_free = core::ptr::null_mut();
    voice.private_data = core::ptr::null_mut();
    if (voice.pcm)
    ...
    voice.sample_ops = core::ptr::null_mut();
    voice.substream = core::ptr::null_mut();
    voice.extra = core::ptr::null_mut();
    }
    if (private_free)
    private_free(voice);
...
#[no_mangle]
unsafe extern "C" fn snd_trident_clear_voices(trident: *mut *mut snd_trident, v_min: c_ushort, v_max: c_ushort) {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_trident_resume(dev: *mut device) -> c_int {
