...
//! Source: sound/pci/au88x0/au88x0_pcm.c
#![no_std]
...
// open callback
#[no_mangle]
unsafe extern "C" fn snd_vortex_pcm_open(substream: *mut snd_pcm_substream) -> c_int {
...
// close callback
#[no_mangle]
unsafe extern "C" fn snd_vortex_pcm_close(substream: *mut snd_pcm_substream) -> c_int {
...
// hw_free callback
#[no_mangle]
unsafe extern "C" fn snd_vortex_pcm_hw_free(substream: *mut snd_pcm_substream) -> c_int {
...
// prepare callback
#[no_mangle]
unsafe extern "C" fn snd_vortex_pcm_prepare(substream: *mut snd_pcm_substream) -> c_int {
    static int snd_vortex_pcm_prepare(struct snd_pcm_substream *substream)
    {
    vortex_t *chip = snd_pcm_substream_chip(substream);
    struct snd_pcm_runtime *runtime = substream.runtime;
    stream_t *stream = (stream_t *) substream.runtime.private_data;
    let mut dma: c_int = stream.dma, fmt, dir;
...
// trigger callback
#[no_mangle]
unsafe extern "C" fn snd_vortex_pcm_trigger(substream: *mut snd_pcm_substream, cmd: c_int) -> c_int {
    static int snd_vortex_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
    {
    vortex_t *chip = snd_pcm_substream_chip(substream);
    stream_t *stream = (stream_t *) substream.runtime.private_data;
    let mut dma: c_int = stream.dma;
    spin_lock(&chip.lock);
    switch (cmd) {
...
// pointer callback
#[no_mangle]
unsafe extern "C" fn snd_vortex_pcm_pointer(substream: *mut snd_pcm_substream) -> snd_pcm_uframes_t {
...
// SPDIF kcontrol
#[no_mangle]
unsafe extern "C" fn snd_vortex_spdif_info(kcontrol: *mut snd_kcontrol, uinfo: *mut snd_ctl_elem_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_vortex_spdif_mask_get(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_vortex_spdif_get(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_vortex_spdif_put(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
    static int snd_vortex_spdif_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
    {
    vortex_t *vortex = snd_kcontrol_chip(kcontrol);
    let mut spdif_sr: c_int = 48000;
...
// subdevice PCM Volume control
    ...
    int i;
    vortex_t *vortex = snd_kcontrol_chip(kcontrol);
    let mut subdev: c_int = kcontrol.id.subdevice;
    struct pcm_vol *p = &vortex.pcm_vol[subdev];
    let mut max_chn: c_int = (VORTEX_IS_QUAD(vortex) ? 4 : 2);
    for (i = 0; i < max_chn; i++)
...
// create a pcm device
#[no_mangle]
unsafe extern "C" fn snd_vortex_new_pcm(chip: *mut vortex_t, idx: c_int, nr: c_int) -> c_int {
...
// pre-allocation of Scatter-Gather buffers
    ...
    case VORTEX_PCM_A3D:
    err = snd_pcm_add_chmap_ctls(pcm, SNDRV_PCM_STREAM_PLAYBACK,
    snd_pcm_std_chmaps, 1, 0, core::ptr::null_mut());
    if (err < 0)
    return err;
    break;
