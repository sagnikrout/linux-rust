...
//! Source: sound/pci/nm256/nm256.c
#![no_std]
...
//
    ...
    let mut playback_bufsize: static int = 16;
    let mut capture_bufsize: static int = 16;
...
//
// The BIOS signature.
pub const NM_SIGNATURE: c_uint = 0x4e4d0000;
// Signature mask.
pub const NM_SIG_MASK: c_uint = 0xffff0000;
// Size of the second memory area.
pub const NM_PORT2_SIZE: c_int = 4096;
// The base offset of the mixer in the second memory area.
pub const NM_MIXER_OFFSET: c_uint = 0x600;
// The maximum size of a coefficient entry.
pub const NM_MAX_PLAYBACK_COEF_SIZE: c_uint = 0x5000;
pub const NM_MAX_RECORD_COEF_SIZE: c_uint = 0x1260;
// The interrupt register.
pub const NM_INT_REG: c_uint = 0xa04;
// And its bits.
pub const NM_PLAYBACK_INT: c_uint = 0x40;
pub const NM_RECORD_INT: c_uint = 0x100;
pub const NM_MISC_INT_1: c_uint = 0x4000;
pub const NM_MISC_INT_2: c_uint = 0x1;

// The AV's "mixer ready" status bit and location.
pub const NM_MIXER_STATUS_OFFSET: c_uint = 0xa04;
pub const NM_MIXER_READY_MASK: c_uint = 0x0800;
pub const NM_MIXER_PRESENCE: c_uint = 0xa06;
pub const NM_PRESENCE_MASK: c_uint = 0x0050;
pub const NM_PRESENCE_VALUE: c_uint = 0x0040;
//
// For the ZX.  It uses the same interrupt register, but it holds 32
// bits instead of 16.
//
pub const NM2_PLAYBACK_INT: c_uint = 0x10000;
pub const NM2_RECORD_INT: c_uint = 0x80000;
pub const NM2_MISC_INT_1: c_uint = 0x8;
pub const NM2_MISC_INT_2: c_uint = 0x2;

// The ZX's "mixer ready" status bit and location.
pub const NM2_MIXER_STATUS_OFFSET: c_uint = 0xa06;
pub const NM2_MIXER_READY_MASK: c_uint = 0x0800;
// The playback registers start from here.
pub const NM_PLAYBACK_REG_OFFSET: c_uint = 0x0;
// The record registers start from here.
pub const NM_RECORD_REG_OFFSET: c_uint = 0x200;
// The rate register is located 2 bytes from the start of the register area.
pub const NM_RATE_REG_OFFSET: c_int = 2;
// Mono/stereo flag, number of bits on playback, and rate mask.
pub const NM_RATE_STEREO: c_int = 1;
pub const NM_RATE_BITS_16: c_int = 2;
pub const NM_RATE_MASK: c_uint = 0xf0;
// Playback enable register.

pub const NM_PLAYBACK_ENABLE_FLAG: c_int = 1;
pub const NM_PLAYBACK_ONESHOT: c_int = 2;
pub const NM_PLAYBACK_FREERUN: c_int = 4;
// Mutes the audio output.

pub const NM_AUDIO_MUTE_LEFT: c_uint = 0x8000;
pub const NM_AUDIO_MUTE_RIGHT: c_uint = 0x0080;
// Recording enable register.

pub const NM_RECORD_ENABLE_FLAG: c_int = 1;
pub const NM_RECORD_FREERUN: c_int = 2;
// coefficient buffer pointer
pub const NM_COEFF_START_OFFSET: c_uint = 0x1c;
pub const NM_COEFF_END_OFFSET: c_uint = 0x20;
// DMA buffer offsets
...
pub struct nm256_stream {
    ...
    pub running: c_int,
    pub suspended: c_int,
    pub /: *mut *mut u32 buf; / offset from chip->buffer,
    pub /: *mut *mut int bufsize; / buffer size in bytes,
    pub /: *mut *mut *mut void __iomem bufptr; / mapped pointer,
    pub /: *mut *mut unsigned long bufptr_addr; / physical address of the mapped pointer,
    pub /: *mut *mut int dma_size; / buffer size of the substream in bytes,
    pub /: *mut *mut int period_size; / period size in bytes,
    pub /: *mut *mut int periods; / # of periods,
    pub /: *mut *mut int shift; / bit shifts,
    pub /: *mut *mut int cur_period; / current period #,
}
...
pub struct nm256 {
    ...
    pub /: *mut *mut *mut void __iomem cport; / control port,
    pub /: *mut *mut unsigned long cport_addr; / physical address,
    pub /: *mut *mut *mut void __iomem buffer; / buffer,
    pub /: *mut *mut unsigned long buffer_addr; / buffer phyiscal address,
    pub /: *mut *mut u32 buffer_start; / start offset from pci resource 0,
    pub /: *mut *mut u32 buffer_end; / end offset,
    pub /: *mut *mut u32 buffer_size; / total buffer size,
    pub /: *mut *mut u32 all_coeff_buf; / coefficient buffer,
    pub /: *mut *mut u32 coeff_buf[2]; / coefficient buffer for each stream,
    pub /: *mut *mut unsigned int coeffs_current: 1; / coeff. table is loaded?,
    pub /: *mut *mut unsigned int use_cache: 1; / use one big coef. table,
    pub /: *mut *mut unsigned int reset_workaround: 1; / Workaround for some laptops to avoid freeze,
    pub /: *mut *mut unsigned int reset_workaround_2: 1; / Extended workaround for some other laptops to avoid freeze,
    pub 1: unsigned int in_resume:,
    pub /: *mut *mut int mixer_base; / register offset of ac97 mixer,
    pub /: *mut *mut int mixer_status_offset; / offset of mixer status reg.,
    pub /: *mut *mut int mixer_status_mask; / bit mask to test the mixer status,
    pub irq: c_int,
    pub irq_acks: c_int,
    pub interrupt: irq_handler_t,
    pub /: *mut *mut int badintrcount; / counter to check bogus interrupts,
    pub irq_mutex: mutex,
    pub streams: [nm256_stream; 2],
    pub ac97: *mut snd_ac97,
    pub /: *mut *mut *mut unsigned short ac97_regs; / register caches, only for valid regs,
    pub pcm: *mut snd_pcm,
    pub pci: *mut pci_dev,
...
//
    ...
    {
    let mut coeff_buf: u32 = chip.coeff_buf[stream];
    let mut offset: u16 = snd_nm256_get_start_offset(which);
    let mut size: u16 = coefficient_sizes[which];
...
// The enable register for the specified engine.
    ...
    let mut addr: u32 = NM_COEFF_START_OFFSET;
...
// The recording engine uses coefficient values 8-15.
    ...
    let mut base: u32 = chip.all_coeff_buf;
    let mut offset: u32 = snd_nm256_get_start_offset(number);
    let mut end_offset: u32 = offset + coefficient_sizes[number];
...
//
    ...
    let mut rate_index: c_int = snd_nm256_fixed_rate(runtime.rate);
    let mut ratebits: c_uchar = (rate_index << 4) & NM_RATE_MASK;
    s.shift = 0;
...
// acquire interrupt
#[no_mangle]
unsafe extern "C" fn snd_nm256_acquire_irq(chip: *mut nm256) -> c_int {
...
// release interrupt
#[no_mangle]
unsafe extern "C" fn snd_nm256_release_irq(chip: *mut nm256) {
...
#[no_mangle]
unsafe extern "C" fn snd_nm256_pcm_mark(chip: *mut nm256, s: *mut nm256_stream, reg: c_int) {
...
#[no_mangle]
unsafe extern "C" fn nm256_ac97_idx(reg: c_ushort) -> c_int {
...
//
    ...
    let mut idx: c_int = nm256_ac97_idx(reg);
    if (idx < 0)
    return 0;
...
// Default buffer end is 5120 bytes below the top of RAM.
    let mut pointer_found: c_ulong = chip.buffer_end - 0x1400;
    u32 sig;
    temp = ioremap(chip.buffer_addr + chip.buffer_end - 0x400, 16);
    if (temp == core::ptr::null_mut()) {
    dev_err(chip.card.dev,
    ...
    let mut pointer: u32 = readl(temp + 4);
//
