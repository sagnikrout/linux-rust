...
//! Source: sound/pci/lola/lola_mixer.c
#![no_std]
...
//
    ...
    bool external_call);
#[no_mangle]
pub unsafe extern "C" fn lola_setup_all_analog_gains(chip: *mut lola, dir: c_int, mute: bool) -> c_int {
    ...
    let mut val: c_uint = mute ? 0 : pin[idx].cur_gain_step;
...
#[no_mangle]
pub unsafe extern "C" fn lola_set_src_config(chip: *mut lola, src_mask: c_uint, update: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn init_mixer_values(chip: *mut lola) -> c_int {
...
// set gain value 0dB diagonally in matrix - part INPUT -> CAPTURE
    for (i = 0; i < chip.mixer.dest_stream_ins; i++) {
    let mut src: c_int = i % chip.mixer.src_phys_ins;
...
//
    for (i = 0; i < chip.mixer.src_stream_outs; i++) {
    let mut src: c_int = chip.mixer.src_stream_out_ofs + i;
...
//
    ...
    let mut dir: c_int = kcontrol.private_value;
...
#[no_mangle]
unsafe extern "C" fn create_analog_mixer(chip: *mut lola, dir: c_int, name: *mut c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn create_input_src_mixer(chip: *mut lola) -> c_int {
...
//
    ...
    struct snd_ctl_elem_info *uinfo)
    {
    let mut count: c_uint = (kcontrol.private_value >> 8) & 0xff;
    uinfo.type = SNDRV_CTL_ELEM_TYPE_INTEGER;
    uinfo.count = count;
    uinfo.value.integer.min = 0;
    ...
    {
    struct lola *chip = snd_kcontrol_chip(kcontrol);
    let mut ofs: c_uint = kcontrol.private_value & 0xff;
    let mut count: c_uint = (kcontrol.private_value >> 8) & 0xff;
    unsigned int mask, i;
    mask = readl(&chip.mixer.array.src_gain_enable);
    for (i = 0; i < count; i++) {
    let mut idx: c_uint = ofs + i;
    unsigned short val;
    ...
    for (i = 0; i < count; i++) {
    let mut idx: c_uint = ofs + i;
    let mut val: c_ushort = ucontrol.value.integer.value[i];
    if (val)
    val--;
...
//
    ...
    struct snd_ctl_elem_info *uinfo)
    {
    let mut src_num: c_uint = (kcontrol.private_value >> 8) & 0xff;
    uinfo.type = SNDRV_CTL_ELEM_TYPE_INTEGER;
    uinfo.count = src_num;
    uinfo.value.integer.min = 0;
    ...
    {
    struct lola *chip = snd_kcontrol_chip(kcontrol);
    let mut src_ofs: c_uint = kcontrol.private_value & 0xff;
    let mut src_num: c_uint = (kcontrol.private_value >> 8) & 0xff;
    let mut dst_ofs: c_uint = (kcontrol.private_value >> 16) & 0xff;
