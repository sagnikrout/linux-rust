...
//! Source: sound/pci/korg1212/korg1212.c
#![no_std]
...
// Debug Stuff
// ----------------------------------------------------------------------------
pub const K1212_DEBUG_LEVEL: c_int = 0;
...
pub const MAILBOX0_OFFSET: c_uint = 0x40	// location of mailbox 0 relative to base address;
pub const MAILBOX1_OFFSET: c_uint = 0x44	// location of mailbox 1 relative to base address;
pub const MAILBOX2_OFFSET: c_uint = 0x48	// location of mailbox 2 relative to base address;
pub const MAILBOX3_OFFSET: c_uint = 0x4c	// location of mailbox 3 relative to base address;
pub const OUT_DOORBELL_OFFSET: c_uint = 0x60	// location of PCI to local doorbell;
pub const IN_DOORBELL_OFFSET: c_uint = 0x64	// location of local to PCI doorbell;
pub const STATUS_REG_OFFSET: c_uint = 0x68	// location of interrupt control/status register;
pub const PCI_CONTROL_OFFSET: c_uint = 0x6c	// location of the EEPROM, PCI, User I/O, init control;
// register
pub const SENS_CONTROL_OFFSET: c_uint = 0x6e	// location of the input sensitivity setting register.;
// this is the upper word of the PCI control reg.
pub const DEV_VEND_ID_OFFSET: c_uint = 0x70	// location of the device and vendor ID register;

// to send a command before giving up.
pub const COMMAND_ACK_MASK: c_uint = 0x8000    // the MSB is set in the command acknowledgment from;
// the card.
pub const DOORBELL_VAL_MASK: c_uint = 0x00FF    // the doorbell value is one byte;
pub const CARD_BOOT_DELAY_IN_MS: c_int = 10;
pub const CARD_BOOT_TIMEOUT: c_int = 10;
pub const DSP_BOOT_DELAY_IN_MS: c_int = 200;
pub const kNumBuffers: c_int = 8;
pub const k1212MaxCards: c_int = 4;
pub const k1212NumWaveDevices: c_int = 6;
pub const k16BitChannels: c_int = 10;
pub const k32BitChannels: c_int = 2;

pub const kPlayBufferFrames: c_int = 1024;
pub const K1212_ANALOG_CHANNELS: c_int = 2;
pub const K1212_SPDIF_CHANNELS: c_int = 2;
pub const K1212_ADAT_CHANNELS: c_int = 8;

pub const K1212_MIN_CHANNELS: c_int = 1;

pub const k1212MinADCSens: c_uint = 0x00;
pub const k1212MaxADCSens: c_uint = 0x7f;
pub const k1212MaxVolume: c_uint = 0x7fff;
pub const k1212MaxWaveVolume: c_uint = 0xffff;
pub const k1212MinVolume: c_uint = 0x0000;
pub const k1212MaxVolInverted: c_uint = 0x8000;
// -----------------------------------------------------------------
// the following bits are used for controlling interrupts in the
// interrupt control/status reg
// -----------------------------------------------------------------
pub const PCI_INT_ENABLE_BIT: c_uint = 0x00000100;
pub const PCI_DOORBELL_INT_ENABLE_BIT: c_uint = 0x00000200;
pub const LOCAL_INT_ENABLE_BIT: c_uint = 0x00010000;
pub const LOCAL_DOORBELL_INT_ENABLE_BIT: c_uint = 0x00020000;
pub const LOCAL_DMA1_INT_ENABLE_BIT: c_uint = 0x00080000;
// -----------------------------------------------------------------
// the following bits are defined for the PCI command register
// -----------------------------------------------------------------
pub const PCI_CMD_MEM_SPACE_ENABLE_BIT: c_uint = 0x0002;
pub const PCI_CMD_IO_SPACE_ENABLE_BIT: c_uint = 0x0001;
pub const PCI_CMD_BUS_MASTER_ENABLE_BIT: c_uint = 0x0004;
// -----------------------------------------------------------------
// the following bits are defined for the PCI status register
// -----------------------------------------------------------------
pub const PCI_STAT_PARITY_ERROR_BIT: c_uint = 0x8000;
pub const PCI_STAT_SYSTEM_ERROR_BIT: c_uint = 0x4000;
pub const PCI_STAT_MASTER_ABORT_RCVD_BIT: c_uint = 0x2000;
pub const PCI_STAT_TARGET_ABORT_RCVD_BIT: c_uint = 0x1000;
pub const PCI_STAT_TARGET_ABORT_SENT_BIT: c_uint = 0x0800;
// ------------------------------------------------------------------------
// the following constants are used in setting the 1212 I/O card's input
// sensitivity.
// ------------------------------------------------------------------------
pub const SET_SENS_LOCALINIT_BITPOS: c_int = 15;
pub const SET_SENS_DATA_BITPOS: c_int = 10;
pub const SET_SENS_CLOCK_BITPOS: c_int = 8;
pub const SET_SENS_LOADSHIFT_BITPOS: c_int = 0;
pub const SET_SENS_LEFTCHANID: c_uint = 0x00;
pub const SET_SENS_RIGHTCHANID: c_uint = 0x01;
pub const K1212SENSUPDATE_DELAY_IN_MS: c_int = 50;
// --------------------------------------------------------------------------
// WaitRTCTicks
//
...
// --------------------------------------------------------------------------
pub const ONE_RTC_TICK: c_int = 1;
pub const SENSCLKPULSE_WIDTH: c_int = 4;
pub const LOADSHIFT_DELAY: c_int = 4;
pub const INTERCOMMAND_DELAY: c_int = 40;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct KorgAudioFrame {
    pub /: *mut *mut u16 frameData16[k16BitChannels]; / channels 0-9 use 16 bit samples,
    pub /: *mut *mut u32 frameData32[k32BitChannels]; / channels 10-11 use 32 bits - only 20 are sent across S/PDIF,
    pub /: *mut *mut u32 timeCodeVal; / holds the ADAT timecode value,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct KorgAudioBuffer {
    pub /: *mut *mut KorgAudioFrame bufferData[kPlayBufferFrames]; / buffer definition,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct KorgSharedBuffer {
    pub playDataBufs: [KorgAudioBuffer; kNumBuffers],
    pub recordDataBufs: [KorgAudioBuffer; kNumBuffers],    pub volumeData: [c_short; kAudioChannels],
    pub cardCommand: u32,
    pub [kAudioChannels]: u16 routeData,
    pub value: u32 AdatTimeCode; // ADAT timecode,
}
...
pub struct SensBits {
    union {
    struct {
    pub leftChanVal:8: c_uint,
    pub leftChanId:8: c_uint,
    pub v: },
    pub leftSensBits: u16,
    pub l: },
    union {
    struct {
    pub rightChanVal:8: c_uint,
    pub rightChanId:8: c_uint,
    pub v: },
    pub rightSensBits: u16,
    pub r: },
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct snd_korg1212 {
    ...
    pub ioport: c_ulong,
    pub iomem2: c_ulong,
    pub irqcount: c_ulong,
    pub inIRQ: c_ulong,
    pub iobase: *mut void __iomem,
    pub dma_dsp: *mut snd_dma_buffer,
    pub dma_play: *mut snd_dma_buffer,
    pub dma_rec: *mut snd_dma_buffer,
    pub dma_shared: *mut snd_dma_buffer,
    pub DataBufsSize: u32,
    pub playDataBufsPtr: *mut *mut KorgAudioBuffer,
    pub recordDataBufsPtr: *mut *mut KorgAudioBuffer,
    pub sharedBufferPtr: *mut *mut KorgSharedBuffer,
    pub RecDataPhy: u32,
    pub PlayDataPhy: u32,
    pub sharedBufferPhy: c_ulong,
    pub VolumeTablePhy: u32,
    pub RoutingTablePhy: u32,
    pub AdatTimeCodePhy: u32,
    pub register: *mut *mut u32 __iomem  statusRegPtr; // address of the interrupt status/control,
    pub register: *mut *mut u32 __iomem  outDoorbellPtr; // address of the host->card doorbell,
    pub register: *mut *mut u32 __iomem  inDoorbellPtr; // address of the card->host doorbell,
    pub card: *mut *mut u32 __iomem  mailbox0Ptr; // address of mailbox 0 on the,
    pub card: *mut *mut u32 __iomem  mailbox1Ptr; // address of mailbox 1 on the,
    pub card: *mut *mut u32 __iomem  mailbox2Ptr; // address of mailbox 2 on the,
    pub card: *mut *mut u32 __iomem  mailbox3Ptr; // address of mailbox 3 on the,
    pub reg: *mut *mut u32 __iomem  controlRegPtr; // address of the EEPROM, PCI, I/O, Init ctrl,
    pub register: *mut *mut u16 __iomem  sensRegPtr; // address of the sensitivity setting,
    pub registers: *mut *mut u32 __iomem  idRegPtr; // address of the device and vendor ID,
    pub periodsize: usize,
    pub channels: c_int,
    ...
    pub cardState: enum CardState,
    pub running: c_int,
    pub mode.: int idleMonitorOn; // indicates whether the card is in idle monitor,
    pub card.: u32 cmdRetryCount; // tracks how many times we have retried sending to the,
    pub source: enum ClockSourceIndex clkSrcRate; // sample rate and clock,
    pub source: enum ClockSourceType clkSource; // clock,
    pub rate: int clkRate; // clock,
    pub volumePhase: [c_int; kAudioChannels],
    pub sensitivity: u16 leftADCInSens; // ADC left channel input,
    pub sensitivity: u16 rightADCInSens; // ADC right channel input,
    pub count: int opencnt; // Open/Close,
    pub count: int setcnt; // SetupForPlay,
    pub count: int playcnt; // TriggerPlay,
    pub Count: int errorcnt; // Error,
    pub Count: unsigned long totalerrorcnt; // Total Error,
    pub dsp_is_loaded: c_int,
    pub dsp_stop_processing: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn LowerWordSwap(swappee: u32) -> u32 {
    static u32 LowerWordSwap(u32 swappee)

#[no_mangle]
unsafe extern "C" fn UpperWordSwap(swappee: u32) -> u32 {
...
unsafe extern "C" fn LowerWordSwap(swappee: u32) -> u32 {
    ...
    u32 retryCount;
    u16 mailBox3Lo;
    let mut rc: c_int = K1212_CMDRET_Success;
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_SendStop(korg1212: *mut snd_korg1212) {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_SendStopAndWait(korg1212: *mut snd_korg1212) {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_TurnOnIdleMonitor(korg1212: *mut snd_korg1212) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_TurnOffIdleMonitor(korg1212: *mut snd_korg1212) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_korg1212_setCardState(korg1212: *mut *mut snd_korg1212, csState: enum CardState) {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_OpenCard(korg1212: *mut *mut snd_korg1212) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_CloseCard(korg1212: *mut *mut snd_korg1212) -> c_int {
...
// spinlock already held
#[no_mangle]
unsafe extern "C" fn snd_korg1212_SetupForPlay(korg1212: *mut *mut snd_korg1212) -> c_int {
...
// spinlock already held
#[no_mangle]
unsafe extern "C" fn snd_korg1212_TriggerPlay(korg1212: *mut *mut snd_korg1212) -> c_int {
...
// spinlock already held
#[no_mangle]
unsafe extern "C" fn snd_korg1212_StopPlay(korg1212: *mut *mut snd_korg1212) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_EnableCardInterrupts(korg1212: *mut *mut snd_korg1212) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_korg1212_use_is_exclusive(korg1212: *mut snd_korg1212) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_SetRate(korg1212: *mut snd_korg1212, rate: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_SetClockSource(korg1212: *mut snd_korg1212, source: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_DisableCardInterrupts(korg1212: *mut snd_korg1212) {
    static void snd_korg1212_DisableCardInterrupts(struct snd_korg1212 *korg1212)
    {
    writel(0, korg1212.statusRegPtr);
    }
#[no_mangle]
unsafe extern "C" fn snd_korg1212_WriteADCSensitivity(korg1212: *mut snd_korg1212) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_OnDSPDownloadComplete(korg1212: *mut snd_korg1212) {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_downloadDSPCode(korg1212: *mut snd_korg1212) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_silence(korg1212: *mut snd_korg1212, pos: c_int, count: c_int, offset: c_int, size: c_int) -> c_int {
    static int snd_korg1212_silence(struct snd_korg1212 *korg1212, int pos, int count, int offset, int size)
    {
    let mut dst: *mut KorgAudioFrame = korg1212.playDataBufsPtr[0].bufferData + pos;
    int i;
...
unsafe extern "C" fn snd_korg1212_free_pcm(pcm: *mut snd_pcm) {
    ...
    stateName[korg1212.cardState]);
    korg1212.pcm = core::ptr::null_mut();
    }
#[no_mangle]
...
unsafe extern "C" fn snd_korg1212_playback_close(substream: *mut snd_pcm_substream) -> c_int {
    ...
    korg1212.playback_substream = core::ptr::null_mut();
...
unsafe extern "C" fn snd_korg1212_capture_close(substream: *mut snd_pcm_substream) -> c_int {
    ...
    korg1212.capture_substream = core::ptr::null_mut();
    korg1212.periodsize = 0;
    }
    snd_korg1212_CloseCard(korg1212);
    return 0;
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_capture_pointer(substream: *mut snd_pcm_substream) -> snd_pcm_uframes_t {
...
//
    ...
    let mut i: c_int = kcontrol.private_value;
    guard(spinlock_irq)(&korg1212.lock);
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_proc_init(korg1212: *mut snd_korg1212) {
...
#[no_mangle]
unsafe extern "C" fn snd_korg1212_create(card: *mut snd_card, pci: *mut pci_dev) -> c_int {
    ...
    struct snd_korg1212 *korg1212 = card.private_data;
    const struct firmware *dsp_code __free(firmware) = core::ptr::null_mut();
