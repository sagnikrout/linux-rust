...
//! Source: sound/pci/oxygen/oxygen_pcm.c
#![no_std]
...
pub const FIFO_BYTES: c_int = 256;
pub const FIFO_BYTES_MULTICH: c_int = 1024;
pub const PERIOD_BYTES_MIN: c_int = 64;
...
unsafe extern "C" fn oxygen_close(substream: *mut snd_pcm_substream) -> c_int {
    ...
    {
    struct oxygen *chip = snd_pcm_substream_chip(substream);
    let mut channel: c_uint = oxygen_substream_channel(substream);
    guard(mutex)(&chip.mutex);
    chip.pcm_active &= ~(1 << channel);
    if (channel == PCM_SPDIF) {
    chip.controls[CONTROL_SPDIF_PCM].vd[0].access |=
    ...
    chip.streams[channel] = core::ptr::null_mut();
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn oxygen_format(hw_params: *mut snd_pcm_hw_params) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn oxygen_rate(hw_params: *mut snd_pcm_hw_params) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn oxygen_i2s_bits(hw_params: *mut snd_pcm_hw_params) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn oxygen_play_channels(hw_params: *mut snd_pcm_hw_params) -> c_uint {
    ...
    shift = 0;
#[no_mangle]
pub unsafe extern "C" fn if(96000: params_rate(params) <=) -> else {
...
unsafe extern "C" fn oxygen_hw_free(substream: *mut snd_pcm_substream) -> c_int {
    ...
    struct oxygen *chip = snd_pcm_substream_chip(substream);
    let mut channel: c_uint = oxygen_substream_channel(substream);
    let mut channel_mask: c_uint = 1 << channel;
    guard(spinlock_irq)(&chip.reg_lock);
...
pub unsafe extern "C" fn oxygen_pcm_init(chip: *mut oxygen) -> c_int {
    ...
    &oxygen_rec_a_ops);
#[no_mangle]
pub unsafe extern "C" fn if(CAPTURE_0_FROM_I2S_2: chip->model.device_config &) -> else {
