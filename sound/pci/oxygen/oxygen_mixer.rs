...
//! Source: sound/pci/oxygen/oxygen_mixer.c
#![no_std]
...
unsafe extern "C" fn upmix_item_count(chip: *mut oxygen) -> c_uint {
    ...
    return 2;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: chip->model.update_center_lfe_mix) -> else {
...
#[no_mangle]
unsafe extern "C" fn upmix_info(ctl: *mut snd_kcontrol, info: *mut snd_ctl_elem_info) -> c_int {
    ...
    struct oxygen *chip = ctl.private_data;
    let mut count: c_uint = upmix_item_count(chip);
    return snd_ctl_enum_info(info, 1, count, names);
    }
#[no_mangle]
unsafe extern "C" fn upmix_get(ctl: *mut snd_kcontrol, value: *mut snd_ctl_elem_value) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(OXYGEN_PLAY_CHANNELS_8: channels ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn upmix_put(ctl: *mut snd_kcontrol, value: *mut snd_ctl_elem_value) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spdif_info(ctl: *mut snd_kcontrol, info: *mut snd_ctl_elem_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn write_spdif_bits(chip: *mut oxygen, bits: u32) {
    ...
    {
    struct oxygen *chip = ctl.private_data;
    let mut bit: u32 = ctl.private_value;
    ...
    {
    struct oxygen *chip = ctl.private_data;
    let mut bit: u8 = ctl.private_value;
    let mut invert: c_int = ctl.private_value & (1 << 8);
    value.value.integer.value[0] =
    ...
    {
    struct oxygen *chip = ctl.private_data;
    let mut codec: c_uint = (ctl.private_value >> 24) & 1;
    let mut index: c_uint = ctl.private_value & 0xff;
    let mut bitnr: c_uint = (ctl.private_value >> 8) & 0xff;
    let mut invert: c_int = ctl.private_value & (1 << 16);
    u16 reg;
    guard(mutex)(&chip.mutex);
...
unsafe extern "C" fn mute_ac97_ctl(chip: *mut oxygen, control: c_uint) {
    ...
    struct snd_ctl_elem_info *info)
    {
    let mut stereo: c_int = (ctl.private_value >> 16) & 1;
    info.type = SNDRV_CTL_ELEM_TYPE_INTEGER;
...
// I'm too lazy to write a function for each control :-)
    ...
    chip.controls[i] = core::ptr::null_mut();
    }
