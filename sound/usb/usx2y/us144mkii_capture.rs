...
//! Source: sound/usb/usx2y/us144mkii_capture.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn tascam_capture_open(substream: *mut snd_pcm_substream) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tascam_capture_close(substream: *mut snd_pcm_substream) -> c_int {
    static int tascam_capture_close(struct snd_pcm_substream *substream)
    {
    struct tascam_card *tascam = snd_pcm_substream_chip(substream);
    tascam.capture_substream = core::ptr::null_mut();
    return 0;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn tascam_capture_prepare(substream: *mut snd_pcm_substream) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn decode_tascam_capture_block(src_block: *const u8, dst_block: *mut i32) {
    ...
    let mut byte1: u8 = p_src_frame_base[bit];
    let mut byte2: u8 = p_src_frame_base[bit + 32];
    ch[0] = (ch[0] << 1) | (byte1 & 1);
...
pub unsafe extern "C" fn capture_urb_complete(urb: *mut urb) {
    ...
    scoped_guard(spinlock_irqsave, &tascam.lock) {
    let mut write_ptr: usize = tascam.capture_ring_buffer_write_ptr;
    let mut bytes_to_end: usize = CAPTURE_RING_BUFFER_SIZE - write_ptr;
