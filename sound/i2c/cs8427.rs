...
pub struct cs8427_stream {
    ...
    pub /: *mut *mut char hw_status[24]; / hardware status,
    pub /: *mut *mut char def_status[24]; / default status,
    pub /: *mut *mut char pcm_status[24]; / PCM private status,
...
#[derive(Copy, Clone)]
pub struct cs8427 {
    pub /: *mut *mut unsigned char regmap[0x14]; / map of first 1 + 13 registers,
    pub rate: c_uint,
...
#[no_mangle]
unsafe extern "C" fn snd_cs8427_reg_read(device: *mut snd_i2c_device, reg: c_uchar) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn snd_cs8427_select_corudata(device: *mut snd_i2c_device, udata: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn snd_cs8427_reset(cs8427: *mut snd_i2c_device) {
    static void snd_cs8427_reset(struct snd_i2c_device *cs8427)
    {
    struct cs8427 *chip;
    ...
    {
    struct snd_i2c_device *device = snd_kcontrol_chip(kcontrol);
    let mut reg: c_uchar = CS8427_REG_QSUBCODE;
    int err;
    snd_i2c_lock(device.bus);
    err = snd_i2c_sendbytes(device, &reg, 1);
    if (err != 1) {
    ...
    chip.playback.substream.runtime : core::ptr::null_mut();
    int err, change;
    snd_i2c_lock(device.bus);
    ...
    runtime != core::ptr::null_mut() : runtime == core::ptr::null_mut())) {
...
#[no_mangle]
pub unsafe extern "C" fn snd_cs8427_iec958_active(cs8427: *mut snd_i2c_device, active: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn snd_cs8427_iec958_pcm(cs8427: *mut snd_i2c_device, rate: c_uint) -> c_int {
