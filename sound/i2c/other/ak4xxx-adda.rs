...
//! Source: sound/i2c/other/ak4xxx-adda.c
#![no_std]
...
//
    ...
    {
    let mut mask: c_uint = AK_GET_MASK(kcontrol.private_value);
    ...
    let mut chip: c_int = AK_GET_CHIP(kcontrol.private_value);
    let mut addr: c_int = AK_GET_ADDR(kcontrol.private_value);
    ...
    let mut chip: c_int = AK_GET_CHIP(kcontrol.private_value);
    let mut addr: c_int = AK_GET_ADDR(kcontrol.private_value);
    let mut shift: c_int = AK_GET_SHIFT(kcontrol.private_value);
    ...
    let mut addr: c_int = AK_GET_ADDR(kcontrol.private_value);
    let mut shift: c_int = AK_GET_SHIFT(kcontrol.private_value);
    let mut nval: c_uchar = ucontrol.value.enumerated.item[0] & 3;
    int change;
    nval = (nval << shift) |
    ...
    let mut addr: c_int = AK_GET_ADDR(kcontrol.private_value);
    let mut shift: c_int = AK_GET_SHIFT(kcontrol.private_value);
    let mut invert: c_int = AK_GET_INVERT(kcontrol.private_value);
// we observe the (1<<shift) bit only
    let mut val: c_uchar = snd_akm4xxx_get(ak, chip, addr) & (1<<shift);
    if (invert)
    val = ! val;
    ...
    let mut shift: c_int = AK_GET_SHIFT(kcontrol.private_value);
    let mut invert: c_int = AK_GET_INVERT(kcontrol.private_value);
    let mut flag: c_long = ucontrol.value.integer.value[0];
...
pub const AK5365_NUM_INPUTS: c_int = 5;
#[no_mangle]
unsafe extern "C" fn ak4xxx_capture_num_inputs(ak: *mut snd_akm4xxx, mixer_ch: c_int) -> c_int {
    ...
    {
    struct snd_akm4xxx *ak = snd_kcontrol_chip(kcontrol);
    let mut mixer_ch: c_int = AK_GET_SHIFT(kcontrol.private_value);
    ...
    let mut chip: c_int = AK_GET_CHIP(kcontrol.private_value);
    let mut addr: c_int = AK_GET_ADDR(kcontrol.private_value);
    let mut mask: c_int = AK_GET_MASK(kcontrol.private_value);
    unsigned char val;
    ...
    let mut num_names: c_int = ak4xxx_capture_num_inputs(ak, mixer_ch);
...
// registers 2-7 and b,c
    let mut val: c_int = idx < 6 ? idx + 2 : (idx - 6) + 0xb;
    knew.private_value =
...
// register 4-9 and 11-12, chip #0 only
    let mut addr: c_int = idx < 6 ? idx + 4 : idx + 5;
    knew.private_value =
...
// register 3
    ...
    break;
    case SND_AK4529: {
    let mut shift: c_int = idx == 3 ? 6 : (2 - idx) * 2;
...
pub unsafe extern "C" fn snd_akm4xxx_build_controls(ak: *mut snd_akm4xxx) -> c_int {
    ...
    num_emphs = 1;
#[no_mangle]
pub unsafe extern "C" fn if(SND_AK4620: ak->type ==) -> else {
