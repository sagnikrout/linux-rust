...
pub const IEEE802154_BEACON_MHR_SZ: c_int = 13;
pub const IEEE802154_BEACON_PL_SZ: c_int = 4;
pub const IEEE802154_MAC_CMD_MHR_SZ: c_int = 23;
pub const IEEE802154_MAC_CMD_PL_SZ: c_int = 1;
...
// Prevent any further use of the scan request
    ...
    request = rcu_replace_pointer(local.scan_req, core::ptr::null_mut(), 1);
    if (!request)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn mac802154_flush_queued_beacons(local: *mut ieee802154_local) {
...
#[no_mangle]
unsafe extern "C" fn mac802154_scan_prepare_beacon_req(local: *mut ieee802154_local) -> c_int {
    ...
    return -ENOBUFS;
    skb.dev = sdata.dev;
    ret = ieee802154_mac_cmd_push(skb, &local.scan_beacon_req, core::ptr::null_mut(), 0);
    if (ret) {
    kfree_skb(skb);
    return ret;
    }
...
#[no_mangle]
pub unsafe extern "C" fn mac802154_scan_worker(work: *mut work_struct) {
    ...
    struct ieee802154_sub_if_data *sdata;
    let mut scan_duration: c_uint = 0;
...
#[no_mangle]
pub unsafe extern "C" fn mac802154_beacon_worker(work: *mut work_struct) {
...
// Wait an arbitrary amount of time in case we cannot use the device
    ...
    request = rcu_replace_pointer(local.beacon_req, core::ptr::null_mut(), 1);
    if (!request)
    return 0;
    kvfree_rcu_mightsleep(request);
...
// Start the beacon work
    ...
    __le16 *short_addr)
    {
    let mut ceaddr: u64 = swab64(( u64)coord.extended_addr);
    let mut frame: ieee802154_association_req_frame = {};
    struct ieee802154_local *local = sdata.local;
    struct wpan_dev *wpan_dev = &sdata.wpan_dev;
...
// short_addr = resp_short_addr;
    ...
    let mut deaddr: u64 = swab64(( u64)dest.extended_addr);
    struct ieee802154_local *local = sdata.local;
    struct wpan_dev *wpan_dev = &sdata.wpan_dev;
    let mut resp_pl: ieee802154_assoc_resp_pl = {};
...
//
    ...
    u8 reason)
    {
    let mut frame: ieee802154_disassociation_notif_frame = {};
    let mut teaddr: u64 = swab64(( u64)target.extended_addr);
    struct ieee802154_local *local = sdata.local;
    struct wpan_dev *wpan_dev = &sdata.wpan_dev;
    struct sk_buff *skb;
    int ret;
    ...
    {
    let mut teaddr: u64 = swab64(( u64)target.extended_addr);
    let mut frame: ieee802154_association_resp_frame = {};
    struct ieee802154_local *local = sdata.local;
    struct wpan_dev *wpan_dev = &sdata.wpan_dev;
    struct sk_buff *skb;
    int ret;
    ...
    struct ieee802154_addr *src = &mac_cb(skb).source;
    struct ieee802154_addr *dest = &mac_cb(skb).dest;
    let mut assoc_resp_pl: ieee802154_assoc_resp_pl = {};
    struct ieee802154_assoc_req_pl assoc_req_pl;
    struct ieee802154_pan_device *child, *exchild;
    let mut tmp: ieee802154_addr = {};
    u64 ceaddr;
    int ret;
    ...
    ceaddr = swab64(( u64)child.extended_addr);
...
//
    ...
    teaddr = swab64(( u64)target.extended_addr);
    ...
    wpan_dev.parent = core::ptr::null_mut();
    } else {
    list_del(&child.node);
    kfree(child);
    wpan_dev.nchildren--;
