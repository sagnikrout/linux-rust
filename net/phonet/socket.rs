...
#[no_mangle]
unsafe extern "C" fn pn_socket_release(sock: *mut socket) -> c_int {
...
pub const PN_HASHSIZE: c_int = 16;
...
#[no_mangle]
pub unsafe extern "C" fn pn_sock_init() -> void __init {
...
//
    ...
    {
    struct sock *sknode;
    struct sock *rval = core::ptr::null_mut();
    let mut obj: u16 = pn_sockaddr_get_object(spn);
    let mut res: u8 = spn.spn_resource;
...
#[no_mangle]
pub unsafe extern "C" fn pn_deliver_sock_broadcast(net: *mut net, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn pn_sock_hash(sk: *mut sock) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pn_sock_unhash(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn pn_socket_bind(sock: *mut socket, addr: *mut sockaddr_unsized, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pn_socket_autobind(sock: *mut socket) -> c_int {
...
//
    ...
    let mut timeo: c_long = sock_rcvtimeo(sk, flags & O_NONBLOCK);
    int err;
    ...
    err = 0;
#[no_mangle]
pub unsafe extern "C" fn if(TCP_CLOSE_WAIT: sk->sk_state ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn pn_socket_listen(sock: *mut socket, backlog: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pn_sock_get_port(sk: *mut sock, sport: c_ushort) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pn_sock_seq_stop(seq: *mut seq_file, v: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn pn_sock_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pn_sock_bind_res(sk: *mut sock, res: u8) -> c_int {
    int pn_sock_bind_res(struct sock *sk, u8 res)
    {
    let mut ret: c_int = -EADDRINUSE;
    ...
    if (pnres.sk[res] == core::ptr::null_mut()) {
    sock_hold(sk);
...
#[no_mangle]
pub unsafe extern "C" fn pn_sock_unbind_res(sk: *mut sock, res: u8) -> c_int {
    ...
    RCU_INIT_POINTER(pnres.sk[res], core::ptr::null_mut());
    ret = 0;
    }
    mutex_unlock(&resource_mutex);
    if (ret == 0) {
...
#[no_mangle]
pub unsafe extern "C" fn pn_sock_unbind_all_res(sk: *mut sock) {
...
// Caller is responsible for RCU sync before final sock_put()
    ...
    return core::ptr::null_mut();
    for (i = 0; i < 256; i++) {
    if (rcu_access_pointer(pnres.sk[i]) == core::ptr::null_mut())
    continue;
    if (!pos)
    return pnres.sk + i;
    pos--;
...
#[no_mangle]
unsafe extern "C" fn pn_res_seq_stop(seq: *mut seq_file, v: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn pn_res_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
