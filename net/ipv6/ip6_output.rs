...
#[no_mangle]
unsafe extern "C" fn ip6_finish_output2(net: *mut net, sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
//
    ...
    net, sk, newskb, core::ptr::null_mut(), newskb.dev,
    dev_loopback_xmit);
    if (hdr.hop_limit == 0) {
    ...
    let mut res: c_int = lwtunnel_xmit(skb);
...
#[no_mangle]
unsafe extern "C" fn __ip6_finish_output(net: *mut net, sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip6_finish_output(net: *mut net, sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_output(net: *mut net, sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_autoflowlabel(net: *mut net, sk: *const sock) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_dst_hoplimit(dst: *mut dst_entry) -> c_int {
    int ip6_dst_hoplimit(struct dst_entry *dst)
    {
    let mut hoplimit: c_int = dst_metric_raw(dst, RTAX_HOPLIMIT);
    rcu_read_lock();
    if (hoplimit == 0) {
    struct net_device *dev = dst_dev_rcu(dst);
    struct inet6_dev *idev;
...
//
    ...
    struct ipv6hdr *hdr;
    let mut proto: u8 = fl6.flowi6_proto;
    let mut seg_len: c_int = skb.len;
    int ret, hlimit = -1;
    u32 mtu;
    rcu_read_lock();
    dev = dst_dev_rcu(dst);
...
//
    ...
    net, (struct sock *)sk, skb, core::ptr::null_mut(), dev,
    dst_output);
    goto unlock;
    }
    ret = -EMSGSIZE;
...
#[no_mangle]
unsafe extern "C" fn ip6_call_ra_chain(skb: *mut sk_buff, sel: c_int) -> c_int {
    static int ip6_call_ra_chain(struct sk_buff *skb, int sel)
    {
    struct ip6_ra_chain *ra;
    struct sock *last = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ip6_forward_proxy_check(skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip6_pkt_too_big(skb: *const sk_buff, mtu: c_uint) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_forward(skb: *mut sk_buff) -> c_int {
    ...
    !xfrm6_policy_check(core::ptr::null_mut(), XFRM_POLICY_FWD, skb)) {
...
// XXX: idev->cnf.proxy_ndp?
    ...
    let mut proxied: c_int = ip6_forward_proxy_check(skb);
    hdr = ipv6_hdr(skb);
    if (proxied > 0) {
...
//
    ...
    struct in6_addr *target = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn source(ndisc_send_redirect: inside) -> and by {
...
//
    ...
    let mut addrtype: c_int = ipv6_addr_type(&hdr.saddr);
...
// Mangling hops number delayed to point after skb COW
    ...
    net, core::ptr::null_mut(), skb, skb.dev, dev,
    ip6_forward_finish);
    error:
...
#[no_mangle]
unsafe extern "C" fn ip6_copy_metadata(to: *mut sk_buff, from: *mut sk_buff) {
...
//
    ...
    inet6_sk(skb.sk) : core::ptr::null_mut();
...
// don't send fragments larger than what we received
    ...
    if (np) {
    let mut frag_size: u32 = READ_ONCE(np.frag_size);
...
// before previous one went down.
    ...
    if (frag2 == frag)
    break;
    frag2.sk = core::ptr::null_mut();
    frag2.destructor = core::ptr::null_mut();
...
//
    ...
    np.daddr_cache ? &sk.sk_v6_daddr : core::ptr::null_mut()) ||

    ip6_rt_check(&rt.rt6i_src, &fl6.saddr,
    np.saddr_cache ? &np.saddr : core::ptr::null_mut()) ||
...
// dst = ip6_route_output(net, sk, fl6);
    rt = (*dst).error ? core::ptr::null_mut() : dst_rt6_info(*dst);
    rcu_read_lock();
    from = rt ? rcu_dereference(rt.from) : core::ptr::null_mut();
...
// dst = NULL;
    if (err == -ENETUNREACH)
    IP6_INC_STATS(net, core::ptr::null_mut(), IPSTATS_MIB_OUTNOROUTES);
    return err;
    }
//
...
//
    ...
    gfp_t gfp)
    {
    return src ? kmemdup(src, (src.hdrlen + 1) * 8, gfp) : core::ptr::null_mut();
    }
...
// need source address above miyazawa
    ...
    struct sk_buff *skb, *skb_prev = core::ptr::null_mut();
    struct ubuf_info *uarg = core::ptr::null_mut();
    let mut exthdrlen: c_int = 0;
    let mut dst_exthdrlen: c_int = 0;
    int hh_len;
    int copy;
    int err;
    let mut offset: c_int = 0;
    let mut zc: bool = false;
    let mut tskey: u32 = 0;
    struct rt6_info *rt = dst_rt6_info(cork.dst);
    bool paged, hold_tskey = false, extra_uref = false;
    struct ipv6_txoptions *opt = v6_cork.opt;
    let mut csummode: c_int = CHECKSUM_NONE;
    unsigned int maxnonfragsize, headersize;
    let mut wmem_alloc_delta: c_uint = 0;
...
#[no_mangle]
unsafe extern "C" fn ip6_cork_steal_dst(skb: *mut sk_buff, cork: *mut inet_cork_full) {
    static void ip6_cork_steal_dst(struct sk_buff *skb, struct inet_cork_full *cork)
    {
    struct dst_entry *dst = cork.base.dst;
    cork.base.dst = core::ptr::null_mut();
    skb_dst_set(skb, dst);
    }
#[no_mangle]
unsafe extern "C" fn ip6_cork_release(cork: *mut inet_cork_full) {
    ...
    let mut proto: c_uchar = fl6.flowi6_proto;
...
// move skb->data to ip header from ext header
    ...
    while ((tmp_skb = __skb_dequeue(queue)) != core::ptr::null_mut()) {
...
// tail_skb = tmp_skb;
    ...
    tmp_skb.destructor = core::ptr::null_mut();
    tmp_skb.sk = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn ip6_send_skb(skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_push_pending_frames(sk: *mut sock) -> c_int {
    ...
    {
    struct sk_buff *skb;
    while ((skb = __skb_dequeue_tail(queue)) != core::ptr::null_mut()) {
    if (skb_dst(skb))
...
pub unsafe extern "C" fn ip6_flush_pending_frames(sk: *mut sock) {
    ...
    {
    let mut exthdrlen: c_int = (ipc6.opt ? ipc6.opt.opt_flen : 0);
    struct sk_buff_head queue;
    int err;
    if (flags & MSG_PROBE) {
    dst_release(&rt.dst);
    ...
    cork.base.flags = 0;
    cork.base.addr = 0;
    cork.base.opt = core::ptr::null_mut();
    cork.base6.opt = core::ptr::null_mut();
