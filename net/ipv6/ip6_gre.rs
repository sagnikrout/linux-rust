...
pub const IP6_GRE_HASH_SIZE_SHIFT: c_int = 5;

    static unsigned int ip6gre_net_id __read_mostly;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ip6gre_net {
    pub tunnels: [*mut ip6_tnl __rcu; 4][IP6_GRE_HASH_SIZE],
    pub collect_md_tun: *mut ip6_tnl __rcu,
    pub collect_md_tun_erspan: *mut ip6_tnl __rcu,
...
#[no_mangle]
unsafe extern "C" fn HASH_ADDR(addr: *const in6_addr) -> u32 {
    static u32 HASH_ADDR(const struct in6_addr *addr)
    {
    let mut hash: u32 = ipv6_addr_hash(addr);
...
// Given src, dst and key, find appropriate for input tunnel.
    ...
    let mut link: c_int = dev.ifindex;
    let mut h0: c_uint = HASH_ADDR(remote);
    let mut h1: c_uint = HASH_KEY(key);
    ...
    let mut h: c_uint = HASH_KEY(p.i_key);
...
#[no_mangle]
unsafe extern "C" fn ip6gre_tunnel_link_md(ign: *mut ip6gre_net, t: *mut ip6_tnl) {
...
#[no_mangle]
unsafe extern "C" fn ip6erspan_tunnel_link_md(ign: *mut ip6gre_net, t: *mut ip6_tnl) {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_tunnel_unlink_md(ign: *mut ip6gre_net, t: *mut ip6_tnl) {
    static void ip6gre_tunnel_unlink_md(struct ip6gre_net *ign, struct ip6_tnl *t)
    {
    if (t.parms.collect_md)
    rcu_assign_pointer(ign.collect_md_tun, core::ptr::null_mut());
    }
    static void ip6erspan_tunnel_unlink_md(struct ip6gre_net *ign,
    struct ip6_tnl *t)
    {
    if (t.parms.collect_md)
    rcu_assign_pointer(ign.collect_md_tun_erspan, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn ip6gre_tunnel_link(ign: *mut ip6gre_net, t: *mut ip6_tnl) {
    static void ip6gre_tunnel_link(struct ip6gre_net *ign, struct ip6_tnl *t)
    {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_tunnel_unlink(ign: *mut ip6gre_net, t: *mut ip6_tnl) {
...
unsafe extern "C" fn ip6gre_tunnel_uninit(dev: *mut net_device) {
    ...
    WRITE_ONCE(ign.fb_tunnel_dev, core::ptr::null_mut());
    dst_cache_reset(&t.dst_cache);
    netdev_put(dev, &t.dev_tracker);
    }
    ...
    struct ip6_tnl *t;
    if (gre_parse_header(skb, &tpi, core::ptr::null_mut(), htons(ETH_P_IPV6),
    offset) < 0)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn ip6gre_rcv(skb: *mut sk_buff, tpi: *const tnl_ptk_info) -> c_int {
    ...
    ip6_tnl_rcv(tunnel, skb, tpi, core::ptr::null_mut(), log_ecn_error);
    }
    return PACKET_RCVD;
    }
    return PACKET_REJECT;
    ...
    tpi.proto);
    if (tunnel) {
    let mut len: c_int = erspan_hdr_len(ver);
...
#[no_mangle]
unsafe extern "C" fn gre_handle_offloads(skb: *mut sk_buff, csum: bool) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip6gre_xmit_ipv4(skb: *mut sk_buff, dev: *mut net_device) -> c_int {
    static inline int ip6gre_xmit_ipv4(struct sk_buff *skb, struct net_device *dev)
    {
    struct ip6_tnl *t = netdev_priv(dev);
    let mut encap_limit: c_int = -1;
    struct flowi6 fl6;
    let mut dsfield: __u8 = 0;
    __u32 mtu;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn ip6gre_xmit_ipv6(skb: *mut sk_buff, dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_xmit_other(skb: *mut sk_buff, dev: *mut net_device) -> c_int {
    ...
    IP_TUNNEL_DECLARE_FLAGS(flags) = { };
    let mut truncate: bool = false;
    let mut encap_limit: c_int = -1;
    let mut dsfield: __u8 = false;
    struct flowi6 fl6;
...
// TooBig packet may have updated dst->dev's mtu
    ...
    if (dst_mtu(dst) > mtu)
    dst.ops.update_pmtu(dst, core::ptr::null_mut(), skb, mtu, false);
    }
...
#[no_mangle]
unsafe extern "C" fn ip6gre_tnl_link_config_common(t: *mut ip6_tnl) {
...
// Set up flowi template
    ...
    if (set_mtu) {
    let mut mtu: c_int = rt.dst.dev.mtu - t_hlen;
    if (!(t.parms.flags & IP6_TNL_F_IGN_ENCAP_LIMIT))
    mtu -= 8;
    if (dev.type == ARPHRD_ETHER)
    mtu -= ETH_HLEN;
...
#[no_mangle]
unsafe extern "C" fn ip6gre_calc_hlen(tunnel: *mut ip6_tnl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_tnl_link_config(t: *mut ip6_tnl, set_mtu: c_int) {
    static void ip6gre_tnl_link_config(struct ip6_tnl *t, int set_mtu)
    {
...
// This perm addr will be used as interface identifier by IPv6
    ...
    NETIF_F_HW_CSUM)
#[no_mangle]
unsafe extern "C" fn ip6gre_tnl_init_features(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_tunnel_init_common(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_fb_tunnel_init(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_exit_rtnl_net(net: *mut net, head: *mut list_head) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_init_net(net: *mut net) -> int __net_init {
...
//
    ...
    let mut index: u32 = nla_get_u32(data[IFLA_GRE_ERSPAN_INDEX]);
    if (index & ~INDEX_MASK)
    return -EINVAL;
    }
    } else if (ver == 2) {
    if (data[IFLA_GRE_ERSPAN_DIR]) {
    let mut dir: u16 = nla_get_u8(data[IFLA_GRE_ERSPAN_DIR]);
    if (dir & ~(DIR_MASK >> DIR_OFFSET))
    return -EINVAL;
    }
    if (data[IFLA_GRE_ERSPAN_HWID]) {
    let mut hwid: u16 = nla_get_u16(data[IFLA_GRE_ERSPAN_HWID]);
...
#[no_mangle]
unsafe extern "C" fn ip6erspan_calc_hlen(tunnel: *mut ip6_tnl) -> c_int {
...
unsafe extern "C" fn ip6gre_tap_setup(dev: *mut net_device) {
    ...
    let mut err: c_int = ip6_tnl_encap_setup(nt, &ipencap);
    if (err < 0)
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn ip6gre_get_size(dev: *const net_device) -> usize {
...
#[no_mangle]
unsafe extern "C" fn ip6erspan_tnl_link_config(t: *mut ip6_tnl, set_mtu: c_int) {
    static void ip6erspan_tnl_link_config(struct ip6_tnl *t, int set_mtu)
    {
    ip6gre_tnl_link_config_common(t);
...
//
#[no_mangle]
unsafe extern "C" fn ip6gre_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ip6gre_fini() -> void __exit {
