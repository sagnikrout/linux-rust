...
//! Source: net/ipv6/ipv6_sockglue.c
#![no_std]
...
// RA packet may be delivered ONLY to IPPROTO_RAW socket
    ...
    return -ENOPROTOOPT;
    new_ra = (sel >= 0) ? kmalloc_obj(*new_ra) : core::ptr::null_mut();
    if (sel >= 0 && !new_ra)
    return -ENOMEM;
    write_lock_bh(&ip6_ra_lock);
    for (rap = &ip6_ra_chain; (ra = *rap) != core::ptr::null_mut(); rap = &ra.next) {
    if (ra.sk == sk) {
    if (sel >= 0) {
...
// numsrc >= (4G-140)/128 overflow in 32 bits
    ...
    struct ipv6_opt_hdr *new = core::ptr::null_mut();
...
// routing header option needs extra check
    ...
    struct ipv6_pinfo *np = inet6_sk(sk);
    struct net *net = sock_net(sk);
    let mut retv: c_int = -ENOPROTOOPT;
    int val, valbool;
    if (sockptr_is_null(optval))
    val = 0;
    else {
...
// we need to exclude all possible ENOPROTOOPTs except default case
    ...
    let mut mcast_oif: c_int = READ_ONCE(np.mcast_oif);
    ...
    let mut hlim: c_int = READ_ONCE(np.mcast_hops);
    put_cmsg(&msg, SOL_IPV6, IPV6_HOPLIMIT, sizeof(hlim), &hlim);
    }
    if (np.rxopt.bits.rxtclass) {
    let mut tclass: c_int = (int)ip6_tclass(np.rcv_flowinfo);
    ...
    let mut flowinfo: __be32 = np.rcv_flowinfo;
...
// check if ipv6_getsockopt_sticky() returns err code
    ...
    break;
    case IPV6_UNICAST_IF:
    val = ( int)htonl((__u32) READ_ONCE(np.ucast_oif));
    break;
    case IPV6_MTU_DISCOVER:
    ...
    case IPV6_ADDR_PREFERENCES:
    {
    let mut srcprefs: u8 = READ_ONCE(np.srcprefs);
    val = 0;
    if (srcprefs & IPV6_PREFER_SRC_TMP)
    val |= IPV6_PREFER_SRC_TMP;
#[no_mangle]
pub unsafe extern "C" fn if(IPV6_PREFER_SRC_PUBLIC: srcprefs &) -> else {
