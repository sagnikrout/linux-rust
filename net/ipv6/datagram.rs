...
#[no_mangle]
unsafe extern "C" fn ipv6_mapped_addr_any(a: *const in6_addr) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_datagram_dst_update(sk: *mut sock, fix_sk_saddr: bool) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_datagram_release_cb(sk: *mut sock) {
    ...
    let mut fl6_flowlabel: __be32 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn ip6_datagram_connect(sk: *mut sock, uaddr: *mut sockaddr_unsized, addr_len: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ipv6_local_error(sk: *mut sock, err: c_int, fl6: *mut flowi6, info: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn ipv6_local_rxpmtu(sk: *mut sock, fl6: *mut flowi6, mtu: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ipv6_datagram_support_addr(serr: *mut sock_exterr_skb) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ipv6_recv_error(sk: *mut sock, msg: *mut msghdr, len: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ipv6_recv_rxpmtu(sk: *mut sock, msg: *mut msghdr, len: c_int) -> c_int {
    ...
    {
    struct ipv6_pinfo *np = inet6_sk(sk);
    let mut is_ipv6: bool = skb.protocol == htons(ETH_P_IPV6);
...
#[no_mangle]
unsafe extern "C" fn ipv6_get_exthdr_len(skb: *const sk_buff, ptr: *const u8) -> u16 {
    ...
    let mut hlim: c_int = ipv6_hdr(skb).hop_limit;
    put_cmsg(msg, SOL_IPV6, IPV6_HOPLIMIT, sizeof(hlim), &hlim);
    }
    if (np.rxopt.bits.rxtclass) {
    let mut tclass: c_int = ipv6_get_dsfield(ipv6_hdr(skb));
    put_cmsg(msg, SOL_IPV6, IPV6_TCLASS, sizeof(tclass), &tclass);
    }
    if (np.rxopt.bits.rxflow) {
    let mut flowinfo: __be32 = ip6_flowinfo((struct ipv6hdr *)nh);
    if (flowinfo)
...
// HbH is allowed only once
    ...
    let mut len: u16 = ipv6_get_exthdr_len(skb, ptr);
    if (len)
...
//
    let mut off: c_uint = sizeof(struct ipv6hdr);
...
// socket options in old style
    ...
    let mut len: u16 = ipv6_get_exthdr_len(skb, (u8 *)rthdr);
    if (len)
...
//
    ...
    let mut val: c_int = opt.frag_max_size;
    ...
    let mut strict: c_int = __ipv6_addr_src_scope(addr_type) <= IPV6_ADDR_SCOPE_LINKLOCAL;
...
// segments left must also match
    ...
    let mut dsthdrlen: c_int = ((opt.dst1opt.hdrlen+1)<<3);
    if (opt.dst0opt)
    opt.opt_nflen -= ipv6_optlen(opt.dst0opt);
    opt.opt_nflen += dsthdrlen;
    opt.dst0opt = opt.dst1opt;
    opt.dst1opt = core::ptr::null_mut();
