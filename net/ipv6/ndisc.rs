...
//
    ...
    {
    let mut space: c_int = __ndisc_opt_addr_space(data_len, pad);
    ...
    return cur <= end && cur.nd_opt_type == type ? cur : core::ptr::null_mut();
    }
    ...
    return cur <= end && ndisc_is_useropt(dev, cur) ? cur : core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn ndisc_mc_map(addr: *const in6_addr, buf: *mut c_char, dev: *mut net_device, dir: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ndisc_key_eq(n: *const neighbour, pkey: *const c_void) -> bool {
    static bool ndisc_key_eq(const struct neighbour *n, const void *pkey)
    {
    return neigh_key_eq128(n, pkey);
    }
#[no_mangle]
unsafe extern "C" fn ndisc_constructor(neigh: *mut neighbour) -> c_int {
    static int ndisc_constructor(struct neighbour *neigh)
    {
    ...
    let mut is_multicast: bool = ipv6_addr_is_multicast(addr);
...
#[no_mangle]
unsafe extern "C" fn pndisc_constructor(n: *mut pneigh_entry) -> c_int {
    static int pndisc_constructor(struct pneigh_entry *n)
    {
...
#[no_mangle]
unsafe extern "C" fn pndisc_destructor(n: *mut pneigh_entry) {
    static void pndisc_destructor(struct pneigh_entry *n)
    {
    struct in6_addr *addr = (struct in6_addr *)&n.key;
    struct net_device *dev = n.dev;
...
#[no_mangle]
unsafe extern "C" fn ndisc_send_unsol_na(dev: *mut net_device) {
...
// inc_opt=*/ true);
    ...
    {
    let mut inc_opt: c_int = dev.addr_len;
    struct sk_buff *skb;
    struct nd_msg *msg;
    let mut optlen: c_int = 0;
    if (!saddr)
...
// msg = (struct nd_msg) {
    ...
    struct sk_buff *skb;
    struct rs_msg *msg;
    let mut send_sllao: c_int = dev.addr_len;
    let mut optlen: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn ndisc_error_report(neigh: *mut neighbour, skb: *mut sk_buff) {
    static void ndisc_error_report(struct neighbour *neigh, struct sk_buff *skb)
    {
//
...
#[no_mangle]
unsafe extern "C" fn ndisc_solicit(neigh: *mut neighbour, skb: *mut sk_buff) {
    static void ndisc_solicit(struct neighbour *neigh, struct sk_buff *skb)
    {
    struct in6_addr *saddr = core::ptr::null_mut();
    struct in6_addr mcaddr;
    struct net_device *dev = neigh.dev;
    struct in6_addr *target = (struct in6_addr *)&neigh.primary_key;
    let mut probes: c_int = atomic_read(&neigh.probes);
...
#[no_mangle]
unsafe extern "C" fn ndisc_recv_ns(skb: *mut sk_buff) -> enum skb_drop_reason {
    static enum skb_drop_reason ndisc_recv_ns(struct sk_buff *skb)
    {
    struct nd_msg *msg = (struct nd_msg *)skb_transport_header(skb);
    const struct in6_addr *saddr = &ipv6_hdr(skb).saddr;
    const struct in6_addr *daddr = &ipv6_hdr(skb).daddr;
    u8 *lladdr = core::ptr::null_mut();
    ...
    let mut dad: c_int = ipv6_addr_any(saddr);
    let mut is_router: c_int = -1;
    SKB_DR(reason);
    let mut nonce: u64 = 0;
    bool inc;
...
//
    ...
    !!is_router, false, (ifp != core::ptr::null_mut()), true);
    goto out;
    }
    if (inc)
...
//
    ...
    true, (ifp != core::ptr::null_mut() && inc), inc);
    if (neigh)
    neigh_release(neigh);
    reason = SKB_CONSUMED;
    }
...
#[no_mangle]
unsafe extern "C" fn accept_untracked_na(idev: *mut inet6_dev, saddr: *mut in6_addr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ndisc_recv_na(skb: *mut sk_buff) -> enum skb_drop_reason {
...
//
    ...
    let mut old_flags: u8 = neigh.flags;
    struct net *net = dev_net(dev);
...
#[no_mangle]
unsafe extern "C" fn ndisc_recv_rs(skb: *mut sk_buff) -> enum skb_drop_reason {
    static enum skb_drop_reason ndisc_recv_rs(struct sk_buff *skb)
    {
    struct rs_msg *rs_msg = (struct rs_msg *)skb_transport_header(skb);
    let mut ndoptlen: c_ulong = skb.len - sizeof(*rs_msg);
    struct neighbour *neigh;
    struct inet6_dev *idev;
    const struct in6_addr *saddr = &ipv6_hdr(skb).saddr;
    struct ndisc_options ndopts;
...
#[no_mangle]
unsafe extern "C" fn ndisc_ra_useropt(ra: *mut sk_buff, opt: *mut nd_opt_hdr) {
    ...
    + (opt.nd_opt_len << 3));
    let mut msg_size: usize = base_size + nla_total_size(sizeof(struct in6_addr));
    ...
    goto nla_put_failure;
    nlmsg_end(skb, nlh);
    rtnl_notify(skb, net, 0, RTNLGRP_ND_USEROPT, core::ptr::null_mut(), GFP_ATOMIC);
    return;
    nla_put_failure:
    nlmsg_free(skb);
    err = -EMSGSIZE;
...
#[no_mangle]
unsafe extern "C" fn ndisc_router_discovery(skb: *mut sk_buff) -> enum skb_drop_reason {
    static enum skb_drop_reason ndisc_router_discovery(struct sk_buff *skb)
    {
    struct ra_msg *ra_msg = (struct ra_msg *)skb_transport_header(skb);
    let mut send_ifinfo_notify: bool = false;
...
// routes added from RAs do not use nexthop objects
    ...
    rt.fib6_nh.fib_nh_dev, core::ptr::null_mut(),
    &ipv6_hdr(skb).saddr);
    if (!neigh) {
...
//
    if (in6_dev.nd_parms) {
    let mut rtime: c_ulong = ntohl(ra_msg.retrans_timer);
...
// skip link-specific ndopts from interior routers
    ...
    (p.nd_opt_len) << 3,
    ndopts.nd_opts_src_lladdr != core::ptr::null_mut());
    }
    }
...
#[no_mangle]
unsafe extern "C" fn ndisc_redirect_rcv(skb: *mut sk_buff) -> enum skb_drop_reason {
...
pub unsafe extern "C" fn ndisc_send_redirect(skb: *mut sk_buff, target: *const in6_addr) {
    ...
    struct flowi6 fl6;
    int rd_len;
    u8 ha_buf[MAX_ADDR_LEN], *ha = core::ptr::null_mut(),
    ops_data_buf[NDISC_OPS_REDIRECT_DATA_SPACE], *ops_data = core::ptr::null_mut();
    bool ret;
...
#[no_mangle]
unsafe extern "C" fn pndisc_redo(skb: *mut sk_buff) {
    static void pndisc_redo(struct sk_buff *skb)
    {
    let mut reason: enum skb_drop_reason = ndisc_recv_ns(skb);
    kfree_skb_reason(skb, reason);
    }
#[no_mangle]
unsafe extern "C" fn ndisc_is_multicast(pkey: *const c_void) -> c_int {
    static int ndisc_is_multicast(const void *pkey)
    {
    return ipv6_addr_is_multicast((struct in6_addr *)pkey);
    }
#[no_mangle]
unsafe extern "C" fn ndisc_suppress_frag_ndisc(skb: *mut sk_buff) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ndisc_rcv(skb: *mut sk_buff) -> enum skb_drop_reason {
...
#[no_mangle]
unsafe extern "C" fn ndisc_netdev_event(this: *mut notifier_block, event: c_ulong, ptr: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(ctl->procname, 0: "base_reachable_time") ==) -> else {
    ...
    ret = -1;
    if (write && ret == 0 && dev && (idev = in6_dev_get(dev)) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn ndisc_net_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn ndisc_net_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
pub unsafe extern "C" fn ndisc_init() -> int __init {
...
//
    ...
    err = neigh_sysctl_register(core::ptr::null_mut(), &nd_tbl.parms,
...
#[no_mangle]
pub unsafe extern "C" fn ndisc_late_init() -> int __init {
    int __init ndisc_late_init(void)
    {
    return register_netdevice_notifier(&ndisc_netdev_notifier);
    }
#[no_mangle]
pub unsafe extern "C" fn ndisc_late_cleanup() {
    void ndisc_late_cleanup(void)
    {
    unregister_netdevice_notifier(&ndisc_netdev_notifier);
    }
#[no_mangle]
pub unsafe extern "C" fn ndisc_cleanup() {
