...
pub const IP6_TUNNEL_MAX_DEST_TLVS: c_int = 8;
pub const IP6_TUNNEL_HASH_SIZE_SHIFT: c_int = 5;

    let mut log_ecn_error: static bool = true;
    module_param(log_ecn_error, bool, 0644);
    MODULE_PARM_DESC(log_ecn_error, "Log packets received with corrupted ECN");
#[no_mangle]
unsafe extern "C" fn HASH(addr1: *const in6_addr, addr2: *const in6_addr) -> u32 {
    static u32 HASH(const struct in6_addr *addr1, const struct in6_addr *addr2)
    {
    let mut hash: u32 = ipv6_addr_hash(addr1) ^ ipv6_addr_hash(addr2);
...
// lists for storing tunnels in use
    pub tnls_r_l: [*mut ip6_tnl __rcu; IP6_TUNNEL_HASH_SIZE],
    pub tnls_wc: [*mut ip6_tnl __rcu; 1],
    pub tnls: [*mut ip6_tnl __rcu; 2],
    pub collect_md_tun: *mut ip6_tnl __rcu,
}

#[no_mangle]
pub unsafe extern "C" fn ip6_tnl_mpls_supported() -> c_int {
...
//
    ...
    {
    let mut hash: c_uint = HASH(remote, local);
    struct ip6_tnl *t, *cand = core::ptr::null_mut();
    ...
    return t;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !cand) -> else {
    else if (!cand)
    cand = t;
    }
...
//
    ...
    struct ip6_tnl *iter;
    if (t.parms.collect_md)
    rcu_assign_pointer(ip6n.collect_md_tun, core::ptr::null_mut());
    for (tp = ip6_tnl_bucket(ip6n, &t.parms);
    (iter = rtnl_dereference(*tp)) != core::ptr::null_mut();
    tp = &iter.next) {
    if (t == iter) {
...
#[no_mangle]
unsafe extern "C" fn ip6_dev_free(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_create2(dev: *mut net_device) -> c_int {
...
//
    ...
    (t = rtnl_dereference(*tp)) != core::ptr::null_mut();
    tp = &t.next) {
    if (ipv6_addr_equal(local, &t.parms.laddr) &&
    ipv6_addr_equal(remote, &t.parms.raddr) &&
    p.link == t.parms.link) {
...
//
    ...
    RCU_INIT_POINTER(ip6n.tnls_wc[0], core::ptr::null_mut());
    else
...
//
#[no_mangle]
pub unsafe extern "C" fn ip6_tnl_parse_tlv_enc_lim(skb: *mut sk_buff, raw: *mut __u8) -> __u16 {
    __u16 ip6_tnl_parse_tlv_enc_lim(struct sk_buff *skb, __u8 *raw)
    {
    const struct ipv6hdr *ipv6h = (const struct ipv6hdr *)raw;
    let mut nhoff: c_uint = raw - skb.data;
    let mut off: c_uint = nhoff + sizeof(*ipv6h);
    let mut nexthdr: u8 = ipv6h.nexthdr;
    let mut exthdr_cnt: c_int = 0;
...
//
    ...
    let mut rel_type: u8 = ICMPV6_DEST_UNREACH;
    let mut rel_code: u8 = ICMPV6_ADDR_UNREACH;
    let mut rel_info: __u32 = 0;
    struct ip6_tnl *t;
    let mut err: c_int = -ENOENT;
    let mut rel_msg: c_int = 0;
    u8 tproto;
    __u16 len;
...
// msg = rel_msg;
    ...
    {
    let mut rel_info: __u32 = ntohl(info);
    const struct iphdr *eiph;
    struct sk_buff *skb2;
    int err, rel_msg = 0;
    let mut rel_type: u8 = type;
    let mut rel_code: u8 = code;
    struct rtable *rt;
    struct flowi4 fl4;
...
// Try to guess incoming interface
    rt = ip_route_output_ports(dev_net(skb.dev), &fl4, core::ptr::null_mut(), eiph.saddr,
    0, 0, 0, IPPROTO_IPIP,
...
// route "incoming" packet
    ...
    rt = ip_route_output_ports(dev_net(skb.dev), &fl4, core::ptr::null_mut(),
...
// Try to guess incoming interface
    ...
    struct sk_buff *skb)
    {
    let mut dsfield: __u8 = ipv6_get_dsfield(ipv6h) & ~INET_ECN_MASK;
...
// ECN is not supported in AF_MPLS
    ...
    let mut ltype: c_int = ipv6_addr_type(laddr);
    let mut rtype: c_int = ipv6_addr_type(raddr);
...
// called with rcu_read_lock()
    ...
    struct net_device *ldev = core::ptr::null_mut();
    if (p.link)
...
// no tunnel info required for mplsip6.
    ...
    if (t) {
    let mut tproto: u8 = READ_ONCE(t.parms.proto);
    if (tproto != ipproto && tproto != 0)
    goto drop;
    if (!xfrm6_policy_check(core::ptr::null_mut(), XFRM_POLICY_IN, skb))
    goto drop;
    ipv6h = ipv6_hdr(skb);
...
#[no_mangle]
unsafe extern "C" fn ip4ip6_rcv(skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip6ip6_rcv(skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mplsip6_rcv(skb: *mut sk_buff) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ipv6_tel_txoption {
    pub ops: ipv6_txoptions,
    pub dst_opt: [__u8; 8],
}

#[no_mangle]
unsafe extern "C" fn init_tel_txopt(opt: *mut ipv6_tel_txoption, encap_limit: __u8) {
...
//
    ...
    struct dst_entry *dst = core::ptr::null_mut(), *ndst = core::ptr::null_mut();
    struct net_device *tdev;
    int err_count, mtu;
    let mut eth_hlen: c_uint = t.dev.type == ARPHRD_ETHER ? ETH_HLEN : 0;
    let mut psh_hlen: c_uint = sizeof(struct ipv6hdr) + t.encap_hlen;
    let mut max_headroom: c_uint = psh_hlen;
...
//
    ...
    ip6tunnel_xmit(core::ptr::null_mut(), skb, dev, 0);
    return 0;
    tx_err_link_failure:
...
//
    ...
    fl6.flowi6_uid = sock_net_uid(dev_net(dev), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_link_config(t: *mut ip6_tnl) {
...
// Set up flowi template
    ...
    &p.raddr, &p.laddr,
    p.link, core::ptr::null_mut(), strict);
    if (rt) {
    tdev = rt.dst.dev;
    ip6_rt_put(rt);
    }
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_update(t: *mut ip6_tnl, p: *mut __ip6_tnl_parm) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ip6_tnl_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip6_tnl_get_iflink(dev: *const net_device) -> c_int {
    ...
    fl6.flowi6_mark = t.parms.fwmark;
    fl6.flowi6_proto = 0;
    dst = ip6_route_output(dev_net(ctx.dev), core::ptr::null_mut(), &fl6);
    if (!dst.error) {
...
//
#[no_mangle]
unsafe extern "C" fn ip6_tnl_dev_setup(dev: *mut net_device) {
    static void ip6_tnl_dev_setup(struct net_device *dev)
    {
...
//
#[no_mangle]
unsafe extern "C" fn ip6_tnl_dev_init(dev: *mut net_device) -> c_int {
    static int ip6_tnl_dev_init(struct net_device *dev)
    {
    struct ip6_tnl *t = netdev_priv(dev);
    let mut err: c_int = ip6_tnl_dev_init_gen(dev);
    if (err)
    return err;
    ip6_tnl_link_config(t);
    if (t.parms.collect_md)
...
//
#[no_mangle]
unsafe extern "C" fn ip6_fb_tnl_dev_init(dev: *mut net_device) -> int __net_init {
...
//
    ...
    if (ip_tunnel_netlink_encap_parms(data, &ipencap)) {
    let mut err: c_int = ip6_tnl_encap_setup(t, &ipencap);
    if (err < 0)
    return err;
    }
    ip6_tnl_netlink_parms(data, &p);
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_dellink(dev: *mut net_device, head: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_get_size(dev: *const net_device) -> usize {
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_fill_info(skb: *mut sk_buff, dev: *const net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_exit_rtnl_net(net: *mut net, list: *mut list_head) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn ip6_tnl_init_net(net: *mut net) -> int __net_init {
    static int __net_init ip6_tnl_init_net(struct net *net)
    {
    struct ip6_tnl_net *ip6n = net_generic(net, ip6_tnl_net_id);
    struct ip6_tnl *t = core::ptr::null_mut();
    int err;
...
//
#[no_mangle]
unsafe extern "C" fn ip6_tunnel_init() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn ip6_tunnel_cleanup() -> void __exit {
