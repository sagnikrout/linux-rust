...
pub const SJA1110_META_TSTAMP_SIZE: c_int = 10;
pub const SJA1110_HEADER_LEN: c_int = 4;
pub const SJA1110_RX_TRAILER_LEN: c_int = 13;
pub const SJA1110_TX_TRAILER_LEN: c_int = 4;
pub const SJA1110_MAX_PADDING_LEN: c_int = 15;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sja1105_tagger_private {
    pub /: *mut *mut sja1105_tagger_data data; / Must be first,
// Protects concurrent access to the meta state machine
// from taggers running on multiple ports on SMP systems
//
    pub meta_lock: spinlock_t,
    pub stampable_skb: *mut sk_buff,
...
#[no_mangle]
unsafe extern "C" fn sja1105_is_link_local(skb: *const sk_buff) -> bool {
    static bool sja1105_is_link_local(const struct sk_buff *skb)
    {
    const struct ethhdr *hdr = eth_hdr(skb);
    let mut dmac: u64 = ether_addr_to_u64(hdr.h_dest);
...
#[no_mangle]
unsafe extern "C" fn sja1105_is_meta_frame(skb: *const sk_buff) -> bool {
    static bool sja1105_is_meta_frame(const struct sk_buff *skb)
    {
    const struct ethhdr *hdr = eth_hdr(skb);
    let mut smac: u64 = ether_addr_to_u64(hdr.h_source);
    let mut dmac: u64 = ether_addr_to_u64(hdr.h_dest);
...
//
#[no_mangle]
unsafe extern "C" fn sja1105_xmit_tpid(dp: *mut dsa_port) -> u16 {
...
//
    ...
    {
    let mut xmit_tpid: __be16 = htons(sja1105_xmit_tpid(dp));
...
// tx_trailer = cpu_to_be32(SJA1110_TX_TRAILER_PRIO(pcp) |
    ...
    if (clone) {
    let mut ts_id: u8 = SJA1105_SKB_CB(clone).ts_id;
...
//
    ...
    priv.stampable_skb = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sja1105_skb_has_tag_8021q(skb: *const sk_buff) -> bool {
    static bool sja1105_skb_has_tag_8021q(const struct sk_buff *skb)
    {
    let mut tpid: u16 = ntohs(eth_hdr(skb).h_proto);
    return tpid == ETH_P_SJA1105 || tpid == ETH_P_8021Q ||
    skb_vlan_tag_present(skb);
    }
#[no_mangle]
unsafe extern "C" fn sja1110_skb_has_inband_control_extension(skb: *const sk_buff) -> bool {
    ...
    struct net_device *netdev)
    {
    let mut source_port: c_int = -1, switch_id = -1, vbid = -1, vid = -1;
    let mut meta: sja1105_meta = {0};
    struct ethhdr *hdr;
    bool is_link_local;
    bool is_meta;
    hdr = eth_hdr(skb);
...
//
    ...
    let mut switch_id: c_int = SJA1110_RX_HEADER_SWITCH_ID(rx_header);
    let mut n_ts: c_int = SJA1110_RX_HEADER_N_TS(rx_header);
...
// Timestamp frame, we have a trailer
    ...
    let mut start_of_padding: c_int = SJA1110_RX_HEADER_TRAILER_POS(rx_header);
    u8 *rx_trailer = skb_tail_pointer(skb) - SJA1110_RX_TRAILER_LEN;
    u64 *tstamp = &SJA1105_SKB_CB(skb).tstamp;
    let mut last_byte: u8 = rx_trailer[12];
...
#[no_mangle]
unsafe extern "C" fn sja1105_disconnect(ds: *mut dsa_switch) {
...
#[no_mangle]
unsafe extern "C" fn sja1105_connect(ds: *mut dsa_switch) -> c_int {
