...
// Construct DSA header from untagged frame.
    ...
    u8 extra)
    {
    let mut trap: bool = false, trunk = false;
...
//
    ...
    skb.dev = lag ? lag.dev : core::ptr::null_mut();
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !trap) -> else {
    else if (!trap)
