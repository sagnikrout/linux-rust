...
//
    ...
    {
    let mut quadlen: c_uint = XDR_QUADLEN(obj.len);
...
//
    ...
    {
    if (likely(nbytes != 0)) {
    let mut quadlen: c_uint = XDR_QUADLEN(nbytes);
    let mut padding: c_uint = (quadlen << 2) - nbytes;
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_terminate_string(buf: *const xdr_buf, len: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn xdr_buf_pagecount(buf: *const xdr_buf) -> usize {
    ...
    if (n != 0 && buf.bvec == core::ptr::null_mut()) {
...
//
    ...
    struct scatterlist *cur = sg, *prev = core::ptr::null_mut();
    let mut nents: c_int = 0;
    int i;
    if (len == 0)
    return 0;
...
//
    ...
    {
    let mut nsg: c_uint = 0, thislen, page_offset;
    if (len == 0)
    return 0;
    if (offset < buf.head[0].iov_len) {
    thislen = min_t(unsigned int,
...
// +1 replaces the slot sg_chain() consumes as the link.
    let mut overflow_nents: c_uint = nsg - sg_head_nents + 1;
...
//
    ...
    let mut buflen: c_uint = head.iov_len;
    head.iov_len  = offset;
    xdr.pages = pages;
    xdr.page_base = base;
    xdr.page_len = len;
...
#[no_mangle]
unsafe extern "C" fn xdr_buf_try_expand(buf: *mut xdr_buf, len: c_uint) {
    static void xdr_buf_try_expand(struct xdr_buf *buf, unsigned int len)
    {
    struct kvec *head = buf.head;
    struct kvec *tail = buf.tail;
    let mut sum: c_uint = head.iov_len + buf.page_len + tail.iov_len;
...
// Expand the tail buffer
    ...
    const struct kvec *tail = buf.tail;
    let mut to: c_uint = base + shift;
    let mut pglen: c_uint = 0;
    let mut talen: c_uint = 0, tato = 0;
    ...
    talen = len;
#[no_mangle]
pub unsafe extern "C" fn if(tato: tail->iov_len >) -> else {
...
// Shift data into pages
    if (shift > base) {
    let mut pgto: c_uint = buf.page_len + base - shift;
    let mut pglen: c_uint = len;
    if (pgto + pglen > buf.page_len)
...
// Shift data into head
    if (shift > base) {
    const struct kvec *head = buf.head;
    let mut hdto: c_uint = head.iov_len + base - shift;
    let mut hdlen: c_uint = len;
...
//
#[no_mangle]
unsafe extern "C" fn xdr_shrink_bufhead(buf: *mut xdr_buf, len: c_uint) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn xdr_shrink_pagelen(buf: *mut xdr_buf, len: c_uint) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_stream_pos(xdr: *const xdr_stream) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn xdr_stream_set_pos(xdr: *mut xdr_stream, pos: c_uint) {
    static void xdr_stream_set_pos(struct xdr_stream *xdr, unsigned int pos)
    {
    let mut blen: c_uint = xdr.buf.len;
    xdr.nwords = blen > pos ? XDR_QUADLEN(blen) - XDR_QUADLEN(pos) : 0;
    }
#[no_mangle]
unsafe extern "C" fn xdr_stream_page_set_pos(xdr: *mut xdr_stream, pos: c_uint) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_page_pos(xdr: *const xdr_stream) -> c_uint {
    unsigned int xdr_page_pos(const struct xdr_stream *xdr)
    {
    let mut pos: c_uint = xdr_stream_pos(xdr);
...
//
    ...
    let mut scratch_len: c_int = buf.buflen - buf.page_len - buf.tail[0].iov_len;
    ...
    if (p != xdr.p && p != core::ptr::null_mut()) {
    size_t len;
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_init_encode_pages(xdr: *mut xdr_stream, buf: *mut xdr_buf) {
    void xdr_init_encode_pages(struct xdr_stream *xdr, struct xdr_buf *buf)
    {
    xdr_reset_scratch_buffer(xdr);
    xdr.buf = buf;
    xdr.page_ptr = buf.pages;
    xdr.iov = core::ptr::null_mut();
    xdr.p = page_address(*xdr.page_ptr);
    xdr.end = (void *)xdr.p + min_t(u32, buf.buflen, PAGE_SIZE);
    xdr.rqst = core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn __xdr_commit_encode(xdr: *mut xdr_stream) {
    void __xdr_commit_encode(struct xdr_stream *xdr)
    {
    let mut shift: usize = xdr.scratch.iov_len;
    void *page;
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_reserve_space(xdr: *mut xdr_stream, nbytes: usize) -> *mut __be32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_reserve_space_vec(xdr: *mut xdr_stream, nbytes: usize) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_truncate_encode(xdr: *mut xdr_stream, len: usize) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_truncate_decode(xdr: *mut xdr_stream, len: usize) {
    void xdr_truncate_decode(struct xdr_stream *xdr, size_t len)
    {
    let mut nbytes: c_uint = xdr_align_size(len);
    xdr.buf.len -= nbytes;
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_restrict_buflen(xdr: *mut xdr_stream, newbuflen: c_int) -> c_int {
    int xdr_restrict_buflen(struct xdr_stream *xdr, int newbuflen)
    {
    struct xdr_buf *buf = xdr.buf;
    let mut left_in_this_buf: c_int = (void *)xdr.end - (void *)xdr.p;
    let mut end_offset: c_int = buf.len + left_in_this_buf;
...
// xdr->p++ = 0;
    ...
    xdr.iov = iov;
    xdr.page_ptr = core::ptr::null_mut();
    return len - base;
    }
...
#[no_mangle]
unsafe extern "C" fn xdr_stream_unmap_current_page(xdr: *mut xdr_stream) {
    static void xdr_stream_unmap_current_page(struct xdr_stream *xdr)
    {
    if (xdr.page_kaddr) {
    kunmap_local(xdr.page_kaddr);
    xdr.page_kaddr = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn xdr_set_next_page(xdr: *mut xdr_stream) {
...
#[no_mangle]
unsafe extern "C" fn xdr_set_next_buffer(xdr: *mut xdr_stream) -> bool {
    static bool xdr_set_next_buffer(struct xdr_stream *xdr)
    {
    if (xdr.page_ptr != core::ptr::null_mut())
    xdr_set_next_page(xdr);
#[no_mangle]
pub unsafe extern "C" fn if(xdr->buf->head: xdr->iov ==) -> else {
...
//
    ...
    if (p != core::ptr::null_mut() && p > xdr.p && xdr.end >= p) {
    xdr.nwords -= p - xdr.p;
    xdr.p = p;
    }
    xdr.rqst = rqst;
...
//
    ...
    buf.buflen =  len;
    buf.len = len;
    xdr_init_decode(xdr, buf, core::ptr::null_mut(), core::ptr::null_mut());
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_finish_decode(xdr: *mut xdr_stream) {
...
#[no_mangle]
unsafe extern "C" fn __xdr_inline_decode(xdr: *mut xdr_stream, nbytes: usize) -> *mut __be32 {
    static __be32 * __xdr_inline_decode(struct xdr_stream *xdr, size_t nbytes)
    {
    let mut nwords: c_uint = XDR_QUADLEN(nbytes);
    __be32 *p = xdr.p;
    __be32 *q = p + nwords;
    ...
    let mut cplen: usize = (char *)xdr.end - (char *)xdr.p;
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_inline_decode(xdr: *mut xdr_stream, nbytes: usize) -> *mut __be32 {
...
#[no_mangle]
unsafe extern "C" fn xdr_realign_pages(xdr: *mut xdr_stream) {
    static void xdr_realign_pages(struct xdr_stream *xdr)
    {
    struct xdr_buf *buf = xdr.buf;
    struct kvec *iov = buf.head;
    let mut cur: c_uint = xdr_stream_pos(xdr);
    unsigned int copied;
...
#[no_mangle]
unsafe extern "C" fn xdr_align_pages(xdr: *mut xdr_stream, len: c_uint) -> c_uint {
    static unsigned int xdr_align_pages(struct xdr_stream *xdr, unsigned int len)
    {
    struct xdr_buf *buf = xdr.buf;
    let mut nwords: c_uint = XDR_QUADLEN(len);
    unsigned int copied;
    if (xdr.nwords == 0)
    return 0;
    ...
    len = buf.page_len;
#[no_mangle]
pub unsafe extern "C" fn if(xdr->nwords: nwords <) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_read_pages(xdr: *mut xdr_stream, len: c_uint) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_set_pagelen(xdr: *mut xdr_stream, len: c_uint) {
    void xdr_set_pagelen(struct xdr_stream *xdr, unsigned int len)
    {
    struct xdr_buf *buf = xdr.buf;
    let mut remaining: usize = xdr_stream_remaining(xdr);
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_enter_page(xdr: *mut xdr_stream, len: c_uint) {
...
//
    ...
    let mut empty_iov: static struct kvec = {.iov_base = core::ptr::null_mut(), .iov_len = 0};
#[no_mangle]
pub unsafe extern "C" fn xdr_buf_from_iov(iov: *const kvec, buf: *mut xdr_buf) {
...
//
    ...
    unsigned int nbytes)
    {
    let mut start: c_uint = xdr_stream_pos(xdr);
...
//
#[no_mangle]
pub unsafe extern "C" fn xdr_buf_trim(buf: *mut xdr_buf, len: c_uint) {
    void xdr_buf_trim(struct xdr_buf *buf, unsigned int len)
    {
    size_t cur;
    let mut trim: c_uint = len;
    if (buf.tail[0].iov_len) {
...
#[no_mangle]
pub unsafe extern "C" fn xdr_decode_word(buf: *const xdr_buf, base: c_uint, obj: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn xdr_encode_word(buf: *const xdr_buf, base: c_uint, obj: u32) -> c_int {
    int xdr_encode_word(const struct xdr_buf *buf, unsigned int base, u32 obj)
    {
    let mut raw: __be32 = cpu_to_be32(obj);
...
// Returns 0 on success, or else a negative error code.
    ...
    {
    char *elem = core::ptr::null_mut(), *c;
    let mut copied: c_uint = 0, todo, avail_here;
    struct page **ppages = core::ptr::null_mut();
    int err;
    if (encode) {
...
// process tail
    ...
    if (copied) {
    let mut l: c_uint = desc.elem_size - copied;
    if (encode)
    memcpy(c, elem + copied, l);
    else {
    memcpy(elem + copied, c, l);
