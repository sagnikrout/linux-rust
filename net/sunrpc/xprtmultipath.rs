...
//! Source: net/sunrpc/xprtmultipath.c
#![no_std]
...
//
    ...
    {
    if (unlikely(xprt_get(xprt) == core::ptr::null_mut()))
    return;
...
//
    ...
    if (xps.xps_net == xprt.xprt_net || xps.xps_net == core::ptr::null_mut())
    ...
    {
    if (unlikely(xprt == core::ptr::null_mut()))
    return;
    if (!test_bit(XPRT_OFFLINE, &xprt.state) && offline)
    xps.xps_nactive--;
    xps.xps_nxprts--;
    if (xps.xps_nxprts == 0)
    xps.xps_net = core::ptr::null_mut();
    smp_wmb();
...
#[no_mangle]
pub unsafe extern "C" fn xprt_multipath_cleanup_ids() {
    void xprt_multipath_cleanup_ids(void)
    {
    ida_destroy(&rpc_xprtswitch_ids);
    }
#[no_mangle]
unsafe extern "C" fn xprt_switch_alloc_id(xps: *mut rpc_xprt_switch, gfp_flags: gfp_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xprt_switch_free_id(xps: *mut rpc_xprt_switch) {
...
#[no_mangle]
unsafe extern "C" fn xprt_switch_free_entries(xps: *mut rpc_xprt_switch) {
...
#[no_mangle]
unsafe extern "C" fn xprt_switch_free(kref: *mut kref) {
...
//
    ...
    {
    if (xps != core::ptr::null_mut() && kref_get_unless_zero(&xps.xps_kref))
    return xps;
...
//
#[no_mangle]
pub unsafe extern "C" fn xprt_switch_put(xps: *mut rpc_xprt_switch) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rpc_xprt_switch_set_roundrobin(xps: *mut rpc_xprt_switch) {
    ...
    {
    if (xpi.xpi_ops != core::ptr::null_mut())
    return xpi.xpi_ops;
    return rcu_dereference(xpi.xpi_xpswitch).xps_iter_ops;
    }
    static
#[no_mangle]
pub unsafe extern "C" fn xprt_iter_no_rewind(xpi: *mut rpc_xprt_iter) {
    void xprt_iter_no_rewind(struct rpc_xprt_iter *xpi)
    {
    }
    static
#[no_mangle]
pub unsafe extern "C" fn xprt_iter_default_rewind(xpi: *mut rpc_xprt_iter) {
    void xprt_iter_default_rewind(struct rpc_xprt_iter *xpi)
    {
    WRITE_ONCE(xpi.xpi_cursor, core::ptr::null_mut());
    }
    static
#[no_mangle]
pub unsafe extern "C" fn xprt_is_active(xprt: *const rpc_xprt) -> bool {
    ...
    if (xpi.xpi_cursor == core::ptr::null_mut() || xps.xps_nxprts < 2)
    ...
    struct list_head *head;
    struct rpc_xprt *pos;
    if (xps == core::ptr::null_mut() || sap == core::ptr::null_mut())
    return false;
    head = &xps.xps_xprt_list;
    list_for_each_entry_rcu(pos, head, xprt_switch) {
    ...
    {
    struct rpc_xprt *pos, *prev = core::ptr::null_mut();
    let mut found: bool = false;
    list_for_each_entry_rcu(pos, head, xprt_switch) {
    if (cur == prev)
    found = true;
...
//
#[no_mangle]
pub unsafe extern "C" fn xprt_iter_rewind(xpi: *mut rpc_xprt_iter) {
    ...
    xpi.xpi_cursor = core::ptr::null_mut();
    xpi.xpi_ops = ops;
    }
//
...
//
    ...
    struct rpc_xprt_switch *xps)
    {
    __xprt_iter_init(xpi, xps, core::ptr::null_mut());
    }
//
...
// Atomically swap out the old xpswitch
    ...
    if (newswitch != core::ptr::null_mut())
...
//
#[no_mangle]
pub unsafe extern "C" fn xprt_iter_destroy(xpi: *mut rpc_xprt_iter) {
    void xprt_iter_destroy(struct rpc_xprt_iter *xpi)
    {
    xprt_switch_put(xprt_iter_xchg_switch(xpi, core::ptr::null_mut()));
    }
//
