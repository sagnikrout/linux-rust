...
//! Source: net/sunrpc/auth_gss/gss_krb5_crypto.c
#![no_std]
...
//
    ...
    let mut conflen: c_uint = krb5.conf_len;
    let mut cksum_len: c_uint = krb5.cksum_len;
    unsigned int sec_offset, sec_len, data_len;
    struct scatterlist sg[XDR_BUF_TO_SG_NENTS];
    struct scatterlist *sg_overflow = core::ptr::null_mut();
    ssize_t ret;
    int nsg;
...
// Ensure a tail segment exists
    if (buf.tail[0].iov_base == core::ptr::null_mut()) {
...
//
    if (pages != buf.pages) {
    let mut poff: c_uint = buf.page_base;
    let mut plen: c_uint = buf.page_len;
    let mut i: c_uint = poff >> PAGE_SHIFT;
    let mut off: c_uint = offset_in_page(poff);
    while (plen) {
