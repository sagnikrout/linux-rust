...
//! Source: net/sunrpc/auth_gss/auth_gss.c
#![no_std]
...
pub const GSS_RETRY_EXPIRED: c_int = 5;
    let mut gss_expired_cred_retry_delay: static unsigned int = GSS_RETRY_EXPIRED;
pub const GSS_KEY_EXPIRE_TIMEO: c_int = 240;
    let mut gss_key_expire_timeo: static unsigned int = GSS_KEY_EXPIRE_TIMEO;
...
pub const GSS_VERF_SLACK: c_int = 100;
...
//
    ...
    {
    struct gss_cred *gss_cred = container_of(cred, struct gss_cred, gc_base);
    struct gss_cl_ctx *ctx = core::ptr::null_mut();
    rcu_read_lock();
...
// import the opaque security context
    ...
    ret = gss_import_sec_context(p, seclen, gm, &ctx.gc_gss_ctx, core::ptr::null_mut(), GFP_KERNEL);
    if (ret < 0) {
...
//
pub const UPCALL_BUF_LEN: c_int = 256;
#[repr(C)]
#[derive(Copy, Clone)]
...
#[no_mangle]
unsafe extern "C" fn get_pipe_version(net: *mut net) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn put_pipe_version(net: *mut net) {
    ...
    if (gss_msg.ctx != core::ptr::null_mut())
...
//
    ...
    gss_cred.gc_upcall = core::ptr::null_mut();
    rpc_wake_up_status(&gss_msg.rpc_waitqueue, gss_msg.msg.errno);
    }
    static void
    ...
    let mut uid: uid_t = from_kuid_munged(userns, gss_msg.uid);
    ...
    let mut buflen: usize = sizeof(gss_msg.databuf);
    int len;
...
//
    ...
    let mut uid: kuid_t = cred.cr_cred.fsuid;
...
// shouldn't normally hit this case on a refresh.
    ...
    task, core::ptr::null_mut(), jiffies + (15 * HZ));
    err = -EAGAIN;
    goto out;
    }
    if (IS_ERR(gss_msg)) {
    ...
    pipe = gss_msg.pipe;
    spin_lock(&pipe.lock);
    if (gss_cred.gc_upcall != core::ptr::null_mut())
    rpc_sleep_on(&gss_cred.gc_upcall.rpc_waitqueue, task, core::ptr::null_mut());
#[no_mangle]
pub unsafe extern "C" fn if(0: gss_msg->ctx == NULL && gss_msg->msg.errno >=) -> else {
...
// if gssd is down, just skip upcalling altogether
    ...
    spin_lock(&pipe.lock);
    if (gss_msg.ctx != core::ptr::null_mut() || gss_msg.msg.errno < 0) {
    break;
    }
    spin_unlock(&pipe.lock);
...
pub const MSG_BUF_MAXSIZE: c_int = 1024;
    static ssize_t
...
#[no_mangle]
unsafe extern "C" fn gss_pipe_open(inode: *mut inode, new_version: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gss_pipe_open_v0(inode: *mut inode) -> c_int {
    static int gss_pipe_open_v0(struct inode *inode)
    {
    return gss_pipe_open(inode, 0);
    }
#[no_mangle]
unsafe extern "C" fn gss_pipe_open_v1(inode: *mut inode) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gss_alloc_pdo {
    pub clnt: *mut rpc_clnt,
    pub name: *const c_char,
    pub upcall_ops: *const rpc_pipe_ops,
}

#[no_mangle]
unsafe extern "C" fn gss_pipe_match_pdo(pdo: *mut rpc_pipe_dir_object, data: *mut c_void) -> c_int {
    ...
    gss_pipe_alloc_pdo,
    &args);
    if (pdo != core::ptr::null_mut())
    return container_of(pdo, struct gss_pipe, pdo);
    return ERR_PTR(-ENOMEM);
    }
#[no_mangle]
unsafe extern "C" fn __gss_pipe_free(p: *mut gss_pipe) {
...
#[no_mangle]
unsafe extern "C" fn __gss_pipe_release(kref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn gss_pipe_free(p: *mut gss_pipe) {
...
//
    ...
    {
    let mut flavor: rpc_authflavor_t = args.pseudoflavor;
    ...
    gss_auth.target_name = core::ptr::null_mut();
    if (args.target_name) {
    gss_auth.target_name = kstrdup(args.target_name, GFP_KERNEL);
    if (gss_auth.target_name == core::ptr::null_mut())
    goto err_free;
    }
...
//
    ...
    gss_pipe_free(gss_auth.gss_pipe[0]);
    gss_auth.gss_pipe[0] = core::ptr::null_mut();
    gss_pipe_free(gss_auth.gss_pipe[1]);
    gss_auth.gss_pipe[1] = core::ptr::null_mut();
    rpcauth_destroy_credcache(auth);
...
//
    ...
    {
    struct gss_auth *gss_auth;
    let mut hashval: c_ulong = (unsigned long)clnt;
    spin_lock(&gss_auth_hash_lock);
    ...
    if (gss_auth.target_name == core::ptr::null_mut())
    continue;
    if (args.target_name == core::ptr::null_mut())
    continue;
    ...
    struct gss_auth *gss_auth;
    struct gss_auth *new;
    gss_auth = gss_auth_find_or_add_hashed(args, clnt, core::ptr::null_mut());
    if (gss_auth != core::ptr::null_mut())
    goto out;
...
// allocated before freeing them.
    ...
    struct gss_auth *gss_auth = container_of(cred.cr_auth, struct gss_auth, rpc_auth);
    struct gss_cl_ctx *ctx = rcu_dereference_protected(gss_cred.gc_ctx, 1);
    RCU_INIT_POINTER(gss_cred.gc_ctx, core::ptr::null_mut());
...
//
    ...
    struct gss_cred	*cred = core::ptr::null_mut();
    let mut err: c_int = -ENOMEM;
...
//
    ...
    struct gss_cl_ctx *ctx;
    let mut timeout: c_ulong = jiffies + (gss_key_expire_timeo * HZ);
    let mut ret: c_int = 0;
    rcu_read_lock();
    ctx = rcu_dereference(gss_cred.gc_ctx);
...
// Don't match with creds that have expired.
    ...
    return 0;
    out:
    if (acred.principal != core::ptr::null_mut()) {
    if (gss_cred.gc_principal == core::ptr::null_mut())
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn gss_marshal(task: *mut rpc_task, xdr: *mut xdr_stream) -> c_int {
...
// p++ = rpc_auth_gss;
    ...
    goto expired;
#[no_mangle]
pub unsafe extern "C" fn if(0: maj_stat !=) -> else {
    else if (maj_stat != 0)
    goto bad_mic;
...
#[no_mangle]
unsafe extern "C" fn gss_renew_cred(task: *mut rpc_task) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gss_cred_is_negative_entry(cred: *mut rpc_cred) -> c_int {
...
// seq = cpu_to_be32(seqno);
    ...
    struct rpc_cred *cred = task.tk_rqstp.rq_cred;
    struct gss_cl_ctx *ctx = gss_cred_get_ctx(cred);
    __be32		*p, *seq = core::ptr::null_mut();
    u32		len, maj_stat;
    int		status;
...
// integ_len = cpu_to_be32(integ_buf.len);
    ...
    clear_bit(RPCAUTH_CRED_UPTODATE, &cred.cr_flags);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: maj_stat) -> else {
    else if (maj_stat)
    goto bad_mic;
// Check that the trailing MIC fit in the buffer, after the fact
    ...
    rqstp.rq_release_snd_buf = core::ptr::null_mut();
    }
    static int
    ...
    if (rqstp.rq_enc_pages[i] == core::ptr::null_mut())
    goto out_free;
    }
...
#[no_mangle]
unsafe extern "C" fn gss_wrap_req(task: *mut rpc_task, xdr: *mut xdr_stream) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rpcsec_gss_init_net(net: *mut net) -> __net_init int {
    static __net_init int rpcsec_gss_init_net(struct net *net)
    {
    return gss_svc_init_net(net);
    }
#[no_mangle]
unsafe extern "C" fn rpcsec_gss_exit_net(net: *mut net) -> __net_exit void {
...
//
#[no_mangle]
unsafe extern "C" fn init_rpcsec_gss() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_rpcsec_gss() -> void __exit {
