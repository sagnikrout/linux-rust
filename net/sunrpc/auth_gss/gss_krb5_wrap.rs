...
//! Source: net/sunrpc/auth_gss/gss_krb5_wrap.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn rotate_buf_a_little(buf: *mut xdr_buf, shift: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn _rotate_left(buf: *mut xdr_buf, shift: c_uint) {
    static void _rotate_left(struct xdr_buf *buf, unsigned int shift)
    {
    let mut shifted: c_int = 0;
    int this_shift;
    if (!buf.len)
    return;
    shift %= buf.len;
...
#[no_mangle]
unsafe extern "C" fn rotate_left(base: u32, buf: *mut xdr_buf, shift: c_uint) {
