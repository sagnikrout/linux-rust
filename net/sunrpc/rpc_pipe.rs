...
pub unsafe extern "C" fn rpc_pipefs_notifier_unregister(nb: *mut notifier_block) {
    ...
    rpc_purge_list(dentry ? &RPC_I(d_inode(dentry)).waitq : core::ptr::null_mut(),
    ...
    let mut mlen: usize = min(msg.len - msg.copied, buflen);
    unsigned long left;
...
//
    ...
    rpc_inode_setowner(inode, core::ptr::null_mut());
    RPC_I(inode).pipe = core::ptr::null_mut();
    inode_unlock(inode);
    }
    static struct inode *
...
// NOTE: it is up to the callback to update msg->copied
    ...
    inode_lock(inode);
    res = -EPIPE;
    if (RPC_I(inode).pipe != core::ptr::null_mut())
    ...
    inode_lock(inode);
    if (rpci.pipe == core::ptr::null_mut())
    mask |= EPOLLERR | EPOLLHUP;
#[no_mangle]
pub unsafe extern "C" fn if(!list_empty(&rpci->pipe->pipe): filp->private_data ||) -> else {
    ...
    let mut ret: c_int = single_open(file, rpc_show_info, core::ptr::null_mut());
    if (!ret) {
    struct seq_file *m = file.private_data;
    spin_lock(&file.f_path.dentry.d_lock);
    if (!d_unhashed(file.f_path.dentry))
    clnt = RPC_I(inode).private;
    if (clnt != core::ptr::null_mut() && refcount_inc_not_zero(&clnt.cl_count)) {
...
pub struct rpc_filelist {
    ...
    pipe.ops = core::ptr::null_mut();
    spin_lock_init(&pipe.lock);
    pipe.dentry = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn rpc_destroy_pipe_data(pipe: *mut rpc_pipe) {
...
//
    ...
    struct inode *inode;
    struct rpc_inode *rpci;
    let mut umode: umode_t = S_IFIFO | 0600;
    int err;
    if (pipe.ops.upcall == core::ptr::null_mut())
    umode &= ~0444;
    if (pipe.ops.downcall == core::ptr::null_mut())
    umode &= ~0222;
...
//
#[no_mangle]
pub unsafe extern "C" fn rpc_init_pipe_dir_head(pdh: *mut rpc_pipe_dir_head) {
    void rpc_init_pipe_dir_head(struct rpc_pipe_dir_head *pdh)
    {
    INIT_LIST_HEAD(&pdh.pdh_entries);
    pdh.pdh_dentry = core::ptr::null_mut();
    }
...
//
    ...
    __func__, ret);
    simple_recursive_removal(ret, core::ptr::null_mut());
    return err;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn rpc_remove_client_dir(rpc_client: *mut rpc_clnt) -> c_int {
    int rpc_remove_client_dir(struct rpc_clnt *rpc_client)
    {
    struct dentry *dentry = rpc_client.cl_pipedir_objects.pdh_dentry;
    if (dentry == core::ptr::null_mut())
    return 0;
    rpc_destroy_pipe_dir_objects(&rpc_client.cl_pipedir_objects);
    rpc_client.cl_pipedir_objects.pdh_dentry = core::ptr::null_mut();
    simple_recursive_removal(dentry, core::ptr::null_mut());
    return 0;
    }
    ...
    if (!IS_ERR(dentry)) {
    let mut error: c_int = rpc_populate(dentry, cache_pipefs_files, 0, 3, cd);
    if (error) {
    simple_recursive_removal(dentry, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn rpc_remove_cache_dir(dentry: *mut dentry) {
...
pub const RPCAUTH_GSSMAGIC: c_uint = 0x67596969;
//
...
#[no_mangle]
pub unsafe extern "C" fn rpc_pipefs_init_net(net: *mut net) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rpc_pipefs_exit_net(net: *mut net) {
...
#[no_mangle]
pub unsafe extern "C" fn rpc_put_sb_net(net: *const net) {
    void rpc_put_sb_net(const struct net *net)
    {
    struct sunrpc_net *sn = net_generic(net, sunrpc_net_id);
    WARN_ON(sn.pipefs_sb == core::ptr::null_mut());
    mutex_unlock(&sn.pipefs_sb_lock);
    }
...
//
    ...
    &rpc_dummy_info_fops, core::ptr::null_mut());
    if (!err)
    err = rpc_mkpipe_dentry(clnt_dentry, "gssd", core::ptr::null_mut(), pipe_data);
    return err;
    }
    static int
    ...
    if (!root)
    return -ENOMEM;
    if (rpc_populate(root, files, RPCAUTH_lockd, RPCAUTH_RootEOF, core::ptr::null_mut()))
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn rpc_fs_get_tree(fc: *mut fs_context) -> c_int {
    static int rpc_fs_get_tree(struct fs_context *fc)
    {
    return get_tree_keyed(fc, rpc_fill_super, get_net(fc.net_ns));
    }
#[no_mangle]
unsafe extern "C" fn rpc_fs_free_fc(fc: *mut fs_context) {
...
#[no_mangle]
unsafe extern "C" fn rpc_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rpc_kill_sb(sb: *mut super_block) {
    ...
    sn.pipefs_sb = core::ptr::null_mut();
    ...
    rpci.private = core::ptr::null_mut();
    rpci.pipe = core::ptr::null_mut();
    init_waitqueue_head(&rpci.waitq);
    }
#[no_mangle]
pub unsafe extern "C" fn register_rpc_pipefs() -> c_int {
