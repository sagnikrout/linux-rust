...
//! Source: net/sunrpc/xprtrdma/svc_rdma_rw.c
#![no_std]
...
pub struct svc_rdma_rw_ctxt {
    ...
    let mut first_bvec_nents: c_uint = dev.attrs.max_send_sge;
    struct svc_rdma_rw_ctxt *ctxt;
...
//
#[no_mangle]
pub unsafe extern "C" fn svc_rdma_destroy_rw_ctxts(rdma: *mut svcxprt_rdma) {
    void svc_rdma_destroy_rw_ctxts(struct svcxprt_rdma *rdma)
    {
    struct svc_rdma_rw_ctxt *ctxt;
    struct llist_node *node;
    while ((node = llist_del_first(&rdma.sc_rw_ctxts)) != core::ptr::null_mut()) {
    ctxt = llist_entry(node, struct svc_rdma_rw_ctxt, rw_node);
    kfree(ctxt);
    }
    }
...
//
    ...
    first = last = core::ptr::null_mut();
    while ((ctxt = svc_rdma_next_ctxt(&cc.cc_rwctxts)) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn svc_rdma_write_info_free(info: *mut svc_rdma_write_info) {
...
//
#[no_mangle]
unsafe extern "C" fn svc_rdma_reply_done(cq: *mut ib_cq, wc: *mut ib_wc) {
...
//
#[no_mangle]
unsafe extern "C" fn svc_rdma_write_done(cq: *mut ib_cq, wc: *mut ib_wc) {
    static void svc_rdma_write_done(struct ib_cq *cq, struct ib_wc *wc)
    {
    struct svcxprt_rdma *rdma = cq.cq_context;
    struct ib_cqe *cqe = wc.wr_cqe;
...
//
#[no_mangle]
unsafe extern "C" fn svc_rdma_wc_read_done(cq: *mut ib_cq, wc: *mut ib_wc) {
    static void svc_rdma_wc_read_done(struct ib_cq *cq, struct ib_wc *wc)
    {
    struct svcxprt_rdma *rdma = cq.cq_context;
    struct ib_cqe *cqe = wc.wr_cqe;
...
//
#[no_mangle]
unsafe extern "C" fn svc_rdma_xb_write(xdr: *const xdr_buf, data: *mut c_void) -> c_int {
...
//
    ...
    let mut inline_len: c_uint = head.rc_saved_arg.head[0].iov_len;
