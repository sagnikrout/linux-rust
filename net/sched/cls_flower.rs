...
pub struct fl_flow_key {
    ...
    pub /: *mut *mut } __aligned(BITS_PER_LONG / 8); / Ensure that we can do comparisons as longs.,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fl_flow_mask_range {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fl_flow_mask {
    pub key: fl_flow_key,
    pub range: fl_flow_mask_range,
    pub flags: u32,
    pub ht_node: rhash_head,
    pub ht: rhashtable,
    pub filter_ht_params: rhashtable_params,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fl_flow_tmplt {
    pub dummy_key: fl_flow_key,
    pub mask: fl_flow_key,
    pub dissector: flow_dissector,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cls_fl_head {
    pub ht: rhashtable,
    pub /: *mut *mut spinlock_t masks_lock; / Protect masks list,
    pub masks: list_head,
    pub hw_filters: list_head,
    pub rwork: rcu_work,
    pub handle_idr: idr,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cls_fl_filter {
    pub mask: *mut fl_flow_mask,
    pub ht_node: rhash_head,
    pub mkey: fl_flow_key,
    pub exts: tcf_exts,
    pub res: tcf_result,
    pub key: fl_flow_key,
    pub list: list_head,
    pub hw_list: list_head,
    pub handle: u32,
    pub flags: u32,
    pub in_hw_count: u32,
    pub needs_tc_skb_ext:1: u8,
    pub rwork: rcu_work,
    pub hw_dev: *mut net_device,
...
#[no_mangle]
unsafe extern "C" fn fl_mask_range(mask: *const fl_flow_mask) -> unsigned short int {
    static unsigned short int fl_mask_range(const struct fl_flow_mask *mask)
    {
    return mask.range.end - mask.range.start;
    }
#[no_mangle]
unsafe extern "C" fn fl_mask_update_range(mask: *mut fl_flow_mask) {
    static void fl_mask_update_range(struct fl_flow_mask *mask)
    {
    const u8 *bytes = (const u8 *) &mask.key;
    let mut size: usize = sizeof(mask.key);
...
// skb does not have min and max values
    ...
    let mut post_ct: bool = qdisc_skb_cb(skb).post_ct;
    let mut zone: u16 = tc_skb_cb(skb).zone;
...
#[no_mangle]
unsafe extern "C" fn fl_init(tp: *mut tcf_proto) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fl_mask_free(mask: *mut fl_flow_mask, mask_init_done: bool) {
...
#[no_mangle]
unsafe extern "C" fn fl_mask_free_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn fl_uninit_mask_free_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn fl_mask_put(head: *mut cls_fl_head, mask: *mut fl_flow_mask) -> bool {
...
#[no_mangle]
unsafe extern "C" fn __fl_destroy_filter(f: *mut cls_fl_filter) {
...
#[no_mangle]
unsafe extern "C" fn fl_destroy_filter_work(work: *mut work_struct) {
    ...
    let mut cls_flower: flow_cls_offload = {};
    ...
    struct tcf_block *block = tp.chain.block;
    let mut cls_flower: flow_cls_offload = {};
    let mut skip_sw: bool = tc_skip_sw(f.flags);
    let mut err: c_int = 0;
    ...
    kfree(cls_flower.rule);
    if (err) {
    fl_hw_destroy_filter(tp, f, rtnl_held, core::ptr::null_mut());
    return err;
    }
    ...
    struct tcf_block *block = tp.chain.block;
    let mut cls_flower: flow_cls_offload = {};
    tc_cls_common_offload_init(&cls_flower.common, tp, f.flags, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn __fl_put(f: *mut cls_fl_filter) {
    ...
    struct cls_fl_filter *f;
    f = idr_find(&head.handle_idr, handle);
    return f ? &f.exts : core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn fl_destroy_sleepable(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn fl_put(tp: *mut tcf_proto, arg: *mut c_void) {
...
//
    ...
    let mut bos: u8 = nla_get_u8(tb[TCA_FLOWER_KEY_MPLS_OPT_LSE_BOS]);
    ...
    let mut tc: u8 = nla_get_u8(tb[TCA_FLOWER_KEY_MPLS_OPT_LSE_TC]);
    if (tc & ~MPLS_TC_MASK) {
    NL_SET_ERR_MSG_ATTR(extack,
    ...
    let mut label: u32 = nla_get_u32(tb[TCA_FLOWER_KEY_MPLS_OPT_LSE_LABEL]);
    ...
    let mut bos: u8 = nla_get_u8(tb[TCA_FLOWER_KEY_MPLS_BOS]);
    if (bos & ~MPLS_BOS_MASK) {
    NL_SET_ERR_MSG_ATTR(extack,
    tb[TCA_FLOWER_KEY_MPLS_BOS],
    "Bottom Of Stack (BOS) must be 0 or 1");
    ...
    let mut tc: u8 = nla_get_u8(tb[TCA_FLOWER_KEY_MPLS_TC]);
    if (tc & ~MPLS_TC_MASK) {
    NL_SET_ERR_MSG_ATTR(extack,
    tb[TCA_FLOWER_KEY_MPLS_TC],
    "Traffic Class (TC) must be between 0 and 7");
    ...
    let mut label: u32 = nla_get_u32(tb[TCA_FLOWER_KEY_MPLS_LABEL]);
    if (label & ~MPLS_LABEL_MASK) {
    NL_SET_ERR_MSG_ATTR(extack,
    tb[TCA_FLOWER_KEY_MPLS_LABEL],
    "Label must be between 0 and 1048575");
    ...
    {
pub const VLAN_PRIORITY_MASK: c_uint = 0x7;
    if (tb[vlan_id_key]) {
    key_val.vlan_id =
...
// flags_mask = 0;
    ...
    {
    let mut tos_key: c_int = encap ? TCA_FLOWER_KEY_ENC_IP_TOS : TCA_FLOWER_KEY_IP_TOS;
    let mut ttl_key: c_int = encap ? TCA_FLOWER_KEY_ENC_IP_TTL : TCA_FLOWER_KEY_IP_TTL;
    let mut tos_mask: c_int = encap ? TCA_FLOWER_KEY_ENC_IP_TOS_MASK : TCA_FLOWER_KEY_IP_TOS_MASK;
    let mut ttl_mask: c_int = encap ? TCA_FLOWER_KEY_ENC_IP_TTL_MASK : TCA_FLOWER_KEY_IP_TTL_MASK;
    ...
    struct nlattr *class = core::ptr::null_mut(), *type = core::ptr::null_mut(), *data = core::ptr::null_mut();
...
//
    ...
    let mut new_len: c_int = key.enc_opts.len;
    ...
    let mut len: u8 = key.enc_opts.len;
    int err;
    ...
    struct netlink_ext_ack *extack)
    {
    const struct nlattr *nla_enc_key, *nla_opt_key, *nla_opt_msk = core::ptr::null_mut();
...
//
    ...
    int vthresh)
    {
    let mut good_num_of_vlans: bool = key.num_of_vlans.num_of_vlans > vthresh;
    if (!tb) {
// ethertype = 0;
...
// ethertype = nla_get_be16(tb);
    ...
    let mut err: c_int = tcf_change_indev(net, tb[TCA_FLOWER_INDEV], extack);
    if (err < 0)
    return err;
...
#[no_mangle]
unsafe extern "C" fn fl_init_mask_hashtable(mask: *mut fl_flow_mask) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fl_needs_tc_skb_ext(mask: *const fl_flow_key) -> bool {
...
// in_ht = true;
    ...
    let mut bound_to_filter: bool = false;
    struct cls_fl_filter *fnew;
    struct fl_flow_mask *mask;
    struct nlattr **tb;
    bool in_ht;
    ...
    tca_opts, fl_policy, core::ptr::null_mut());
    if (err < 0)
    goto errout_tb;
    ...
    if (!handle) {
    handle = 1;
    err = idr_alloc_u32(&head.handle_idr, core::ptr::null_mut(), &handle,
    INT_MAX, GFP_ATOMIC);
    } else {
    err = idr_alloc_u32(&head.handle_idr, core::ptr::null_mut(), &handle,
    handle, GFP_ATOMIC);
...
// Fold filter was deleted concurrently. Retry lookup.
    ...
    fl_hw_destroy_filter(tp, fold, rtnl_held, core::ptr::null_mut());
...
// arg = fnew;
    ...
    spin_unlock(&tp.lock);
    if (!tc_skip_hw(fnew.flags))
    fl_hw_destroy_filter(tp, fnew, rtnl_held, core::ptr::null_mut());
    if (in_ht)
...
// last = list_empty(&head->masks);
    ...
    {
    struct cls_fl_head *head = fl_head_dereference(tp);
    let mut id: c_ulong = arg.cookie, tmp;
    struct cls_fl_filter *f;
    arg.count = arg.skip;
    rcu_read_lock();
...
// don't return filters that are being deleted
    ...
    struct tcf_block *block = tp.chain.block;
    let mut cls_flower: flow_cls_offload = {};
    struct cls_fl_filter *f = core::ptr::null_mut();
    int err;
...
#[no_mangle]
unsafe extern "C" fn fl_hw_add(tp: *mut tcf_proto, type_data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn fl_hw_del(tp: *mut tcf_proto, type_data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn fl_tmplt_destroy(tmplt_priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn fl_tmplt_dump(skb: *mut sk_buff, net: *mut net, tmplt_priv: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fl_delete_empty(tp: *mut tcf_proto) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cls_fl_init() -> int __init {
    static int __init cls_fl_init(void)
    {
    return register_tcf_proto_ops(&cls_fl_ops);
    }
#[no_mangle]
unsafe extern "C" fn cls_fl_exit() -> void __exit {
