...
pub const CLS_BPF_NAME_LEN: c_int = 256;

    (TCA_CLS_FLAGS_SKIP_HW | TCA_CLS_FLAGS_SKIP_SW)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cls_bpf_head {
    pub plist: list_head,
    pub handle_idr: idr,
    pub rcu: rcu_head,
}
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_exec_opcode(code: c_int) -> c_int {
    ...
    let mut at_ingress: bool = skb_at_tc_ingress(skb);
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_is_ebpf(prog: *const cls_bpf_prog) -> bool {
    ...
    let mut cls_bpf: tc_cls_bpf_offload = {};
    ...
    cls_bpf.prog = prog ? prog.filter : core::ptr::null_mut();
    cls_bpf.oldprog = oldprog ? oldprog.filter : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_flags(flags: u32) -> u32 {
    ...
    {
    int err;
    err = cls_bpf_offload_cmd(tp, core::ptr::null_mut(), prog, extack, false);
    if (err)
    ...
    struct tcf_block *block = tp.chain.block;
    let mut cls_bpf: tc_cls_bpf_offload = {};
    tc_cls_common_offload_init(&cls_bpf.common, tp, prog.gen_flags, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_init(tp: *mut tcf_proto) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_free_parms(prog: *mut cls_bpf_prog) {
...
#[no_mangle]
unsafe extern "C" fn __cls_bpf_delete_prog(prog: *mut cls_bpf_prog) {
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_delete_prog_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_prog_from_ops(tb: *mut nlattr, prog: *mut cls_bpf_prog) -> c_int {
    ...
    prog.bpf_name = core::ptr::null_mut();
    prog.filter = fp;
    return 0;
    }
    ...
    dev = block.q ? qdisc_dev(block.q) : core::ptr::null_mut();
    ...
    prog.bpf_ops = core::ptr::null_mut();
    prog.bpf_name = name;
    prog.filter = fp;
    if (fp.dst_needed)
    ...
    goto errout_idr;
    if (tb[TCA_BPF_FLAGS]) {
    let mut bpf_flags: u32 = nla_get_u32(tb[TCA_BPF_FLAGS]);
...
// arg = prog;
    ...
    cls_bpf.command = TC_CLSBPF_OFFLOAD;
    cls_bpf.exts = &prog.exts;
    cls_bpf.prog = add ? prog.filter : core::ptr::null_mut();
    cls_bpf.oldprog = add ? core::ptr::null_mut() : prog.filter;
    cls_bpf.name = prog.bpf_name;
    cls_bpf.exts_integrated = prog.exts_integrated;
...
#[no_mangle]
unsafe extern "C" fn cls_bpf_init_mod() -> int __init {
    static int __init cls_bpf_init_mod(void)
    {
    return register_tcf_proto_ops(&cls_bpf_ops);
    }
#[no_mangle]
unsafe extern "C" fn cls_bpf_exit_mod() -> void __exit {
