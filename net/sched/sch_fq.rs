...
// First cache line : used in fq_gc(), fq_enqueue(), fq_dequeue()
    pub t_root: rb_root,
    pub /: *mut *mut *mut sk_buff head; / list of skbs for this flow : first skb,
    union {
    pub /: *mut *mut *mut sk_buff tail; / last skb in the list,
    pub /: *mut *mut unsigned long age; / (jiffies | 1UL) when flow was emptied, for gc,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fq_flow_head {
    pub first: *mut fq_flow,
    pub last: *mut fq_flow,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct fq_perband_flows {
    pub new_flows: fq_flow_head,
    pub old_flows: fq_flow_head,
    pub credit: c_int,
    pub /: *mut *mut int quantum; / based on band nr : 576KB, 192KB, 64KB,
}
...
// Read mostly cache line
    pub offload_horizon: u64,
    pub quantum: u32,
    pub initial_quantum: u32,
    pub flow_refill_delay: u32,
    pub /: *mut *mut u32 flow_plimit; / max packets per flow,
    pub /: *mut *mut unsigned long flow_max_rate; / optional max rate per flow,
    pub ce_threshold: u64,
    pub /: *mut *mut u64 horizon; / horizon in ns,
    pub /: *mut *mut u32 orphan_mask; / mask for orphaned skb,
    pub low_rate_threshold: u32,
    pub fq_root: *mut rb_root,
    pub rate_enable: u8,
    pub fq_trees_log: u8,
    pub horizon_drop: u8,
    pub prio2band: [u8; FQ_PRIO2BAND_CRUMB_SIZE],
    pub /: *mut *mut u32 timer_slack; / hrtimer slack in ns,
// Read/Write fields.
    pub /: *mut *mut unsigned int band_nr; / band being serviced in fq_dequeue(),
    pub band_flows: [fq_perband_flows; FQ_BANDS],
    pub /: *mut *mut fq_flow internal; / fastpath queue.,
    pub /: *mut *mut rb_root delayed; / for rate limited flows,
    pub time_next_delayed_flow: u64,
    pub unthrottle_latency_ns: c_ulong,
    pub band_pkt_count: [u32; FQ_BANDS],
    pub flows: u32,
    pub /: *mut *mut u32 inactive_flows; / Flows with no packet to send.,
    pub throttled_flows: u32,
    pub stat_throttled: u64,
    pub watchdog: qdisc_watchdog,
    pub stat_gc_flows: u64,
// Seldom used fields.
    pub stat_band_drops: [u64; FQ_BANDS],
    pub stat_ce_mark: u64,
    pub stat_horizon_drops: u64,
    pub stat_horizon_caps: u64,
    pub stat_flows_plimit: u64,
    pub stat_pkts_too_long: u64,
    pub stat_allocation_errors: u64,
}

// return the i-th 2-bit value ("crumb")
#[no_mangle]
unsafe extern "C" fn fq_prio2band(prio2band: *const u8, prio: c_uint) -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn fq_flow_set_detached(f: *mut fq_flow) {
    static void fq_flow_set_detached(struct fq_flow *f)
    {
    f.age = jiffies | 1UL;
    }
#[no_mangle]
unsafe extern "C" fn fq_flow_is_detached(f: *const fq_flow) -> bool {
...
#[no_mangle]
unsafe extern "C" fn fq_flow_is_throttled(f: *const fq_flow) -> bool {
    ...
    head.first = flow;
    head.last = flow;
    flow.next = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn fq_flow_unset_throttled(q: *mut fq_sched_data, f: *mut fq_flow) {
...
#[no_mangle]
unsafe extern "C" fn fq_flow_rb_insert(q: *mut fq_sched_data, f: *mut fq_flow) {
    static void fq_flow_rb_insert(struct fq_sched_data *q, struct fq_flow *f)
    {
    struct rb_node **p = &q.delayed.rb_node, *parent = core::ptr::null_mut();
    while (*p) {
    struct fq_flow *aux;
    parent = *p;
...
#[no_mangle]
unsafe extern "C" fn fq_flow_set_throttled(q: *mut fq_sched_data, f: *mut fq_flow) {
...
#[no_mangle]
unsafe extern "C" fn fq_gc_candidate(f: *const fq_flow) -> bool {
    ...
    struct sock *sk)
    {
    struct fq_flow *f, *tofree = core::ptr::null_mut();
...
//
    ...
    let mut hash: c_ulong = skb_get_hash(skb) & q.orphan_mask;
...
#[no_mangle]
unsafe extern "C" fn flow_queue_add(flow: *mut fq_flow, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn fq_skb_tstamp_to_mono(skb: *mut sk_buff) -> ktime_t {
    static ktime_t fq_skb_tstamp_to_mono(struct sk_buff *skb)
    {
    let mut mono_max: ktime_t = NSEC_PER_SEC * TIME_UPTIME_SEC_MAX;
...
#[no_mangle]
unsafe extern "C" fn fq_mono_to_skb_tstamp(skb: *mut sk_buff, time_to_send: ktime_t) {
...
#[no_mangle]
pub unsafe extern "C" fn if(SKB_CLOCK_REALTIME: skb->tstamp_type ==) -> else {
    ...
    let mut time_to_send: ktime_t = fq_skb_tstamp_to_mono(skb);
...
#[no_mangle]
unsafe extern "C" fn fq_check_throttled(q: *mut fq_sched_data, now: u64) {
...
//
    ...
    while ((p = rb_first(&q.delayed)) != core::ptr::null_mut()) {
    ...
    return pband.old_flows.first ? &pband.old_flows : core::ptr::null_mut();
    }
...
//
    ...
    if (rate != ~0UL) {
    let mut len: u64 = (u64)plen * NSEC_PER_SEC;
    if (likely(rate))
...
//
    ...
    let mut drift: i64 = now - f.time_next_packet;
    if (drift > 0)
...
#[no_mangle]
unsafe extern "C" fn fq_flow_purge(flow: *mut fq_flow) {
    ...
    flow.head = core::ptr::null_mut();
    flow.qlen = 0;
    }
#[no_mangle]
unsafe extern "C" fn fq_reset(sch: *mut Qdisc) {
    ...
    root = &q.fq_root[idx];
    while ((p = rb_first(root)) != core::ptr::null_mut()) {
    ...
    q.band_flows[idx].new_flows.first = core::ptr::null_mut();
    q.band_flows[idx].old_flows.first = core::ptr::null_mut();
    ...
    let mut fcnt: c_int = 0;
    u32 idx;
    for (idx = 0; idx < (1U << old_log); idx++) {
    oroot = &old_array[idx];
    while ((op = rb_first(oroot)) != core::ptr::null_mut()) {
    rb_erase(op, oroot);
...
#[no_mangle]
unsafe extern "C" fn fq_free(addr: *mut c_void) {
    static void fq_free(void *addr)
    {
    kvfree(addr);
    }
#[no_mangle]
unsafe extern "C" fn fq_resize(sch: *mut Qdisc, log: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fq_prio2band_compress_crumb(in: *const u8, out: *mut u8) {
    static void fq_prio2band_compress_crumb(const u8 *in, u8 *out)
    {
    let mut num_elems: c_int = TC_PRIO_MAX + 1;
...
#[no_mangle]
unsafe extern "C" fn fq_prio2band_decompress_crumb(in: *const u8, out: *mut u8) {
    ...
    let mut nval: u32 = nla_get_u32(tb[TCA_FQ_BUCKETS_LOG]);
    ...
    let mut quantum: u32 = nla_get_u32(tb[TCA_FQ_QUANTUM]);
    ...
    let mut rate: u32 = nla_get_u32(tb[TCA_FQ_FLOW_MAX_RATE]);
    ...
    let mut enable: u32 = nla_get_u32(tb[TCA_FQ_RATE_ENABLE]);
    if (enable <= 1)
    ...
    let mut usecs_delay: u32 = nla_get_u32(tb[TCA_FQ_FLOW_REFILL_DELAY]) ;
...
#[no_mangle]
unsafe extern "C" fn fq_destroy(sch: *mut Qdisc) {
    ...
    q.rate_enable		= 1;
    for (i = 0; i < FQ_BANDS; i++) {
    q.band_flows[i].new_flows.first = core::ptr::null_mut();
    q.band_flows[i].old_flows.first = core::ptr::null_mut();
    }
    q.band_flows[0].quantum = 9 << 16;
    q.band_flows[1].quantum = 3 << 16;
    q.band_flows[2].quantum = 1 << 16;
    q.delayed		= RB_ROOT;
    q.fq_root		= core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn fq_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fq_dump_stats(sch: *mut Qdisc, d: *mut gnet_dump) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fq_module_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn fq_module_exit() -> void __exit {
