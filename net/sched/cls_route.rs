...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct route4_fastmap {
    pub filter: *mut route4_filter,
    pub id: u32,
    pub iif: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct route4_head {
    pub fastmap: [route4_fastmap; 16],
    pub 1]: *mut *mut route4_bucket __rcu table[256 +,
    pub rcu: rcu_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct route4_bucket {
// 16 FROM buckets + 16 IIF buckets + 1 wildcard bucket
    pub 1]: *mut *mut route4_filter __rcu ht[16 + 16 +,
    pub rcu: rcu_head,
}
...
pub struct route4_filter {
    pub next: *mut route4_filter __rcu,
    pub id: u32,
    pub iif: c_int,
    pub res: tcf_result,
    pub exts: tcf_exts,
    pub handle: u32,
    pub dying: bool,
    pub bkt: *mut route4_bucket,
    pub tp: *mut tcf_proto,
    pub rwork: rcu_work,
}

#[no_mangle]
pub unsafe extern "C" fn route4_fastmap_hash(id: u32, iif: c_int) -> c_int {
    ...
    {
    let mut h: c_int = route4_fastmap_hash(id, iif);
...
#[no_mangle]
pub unsafe extern "C" fn route4_hash_to(id: u32) -> c_int {
    static inline int route4_hash_to(u32 id)
    {
    return id & 0xFF;
    }
#[no_mangle]
pub unsafe extern "C" fn route4_hash_from(id: u32) -> c_int {
    static inline int route4_hash_from(u32 id)
    {
    return (id >> 16) & 0xF;
    }
#[no_mangle]
pub unsafe extern "C" fn route4_hash_iif(iif: c_int) -> c_int {
    static inline int route4_hash_iif(int iif)
    {
    return 16 + ((iif >> 16) & 0xF);
    }
#[no_mangle]
pub unsafe extern "C" fn route4_hash_wild() -> c_int {
...
// res = f->res;						\
    ...
    (f = head.fastmap[h].filter) != core::ptr::null_mut()) {
    if (f == ROUTE4_FAILURE) {
...
#[no_mangle]
pub unsafe extern "C" fn to_hash(id: u32) -> u32 {
    static inline u32 to_hash(u32 id)
    {
    let mut h: u32 = id & 0xFF;
    if (id & 0x8000)
    h += 256;
    return h;
    }
#[no_mangle]
pub unsafe extern "C" fn from_hash(id: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn route4_init(tp: *mut tcf_proto) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __route4_delete_filter(f: *mut route4_filter) {
...
#[no_mangle]
unsafe extern "C" fn route4_delete_filter_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn route4_queue_work(f: *mut route4_filter) {
    ...
    for (h2 = 0; h2 <= 32; h2++) {
    struct route4_filter *f;
    while ((f = rtnl_dereference(b.ht[h2])) != core::ptr::null_mut()) {
...
//
    ...
    RCU_INIT_POINTER(head.table[h1], core::ptr::null_mut());
    kfree_rcu(b, rcu);
    }
    }
...
//
    route4_reset_fastmap(head, core::ptr::null_mut());
    kfree_rcu(head, rcu);
    }
...
// OK, session has no flows
    RCU_INIT_POINTER(head.table[to_hash(h)], core::ptr::null_mut());
    kfree_rcu(b, rcu);
    break;
    }
    }
...
// last = false;
    ...
    {
    let mut id: u32 = 0, to = 0, nhandle = 0x8000;
    ...
    let mut h2: c_uint = from_hash(nhandle >> 16);
    ...
    f.id = to | id<<16;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tb[TCA_ROUTE4_IIF]) -> else {
    else if (tb[TCA_ROUTE4_IIF])
    f.iif = id;
    f.handle = nhandle;
    f.bkt = b;
    ...
    struct route4_head *head = rtnl_dereference(tp.root);
    struct route4_filter __rcu **fp;
    struct route4_filter *fold, *f1, *pfp, *f = core::ptr::null_mut();
    struct route4_bucket *b;
    ...
    route4_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    fold = *arg;
    ...
    (f1 = rtnl_dereference(*fp)) != core::ptr::null_mut();
    fp = &f1.next)
...
// arg = f;
    ...
    struct route4_head *head = rtnl_dereference(tp.root);
    unsigned int h, h1;
    if (head == core::ptr::null_mut() || arg.stop)
    return;
    for (h = 0; h <= 256; h++) {
...
#[no_mangle]
unsafe extern "C" fn init_route4() -> int __init {
    static int __init init_route4(void)
    {
    return register_tcf_proto_ops(&cls_route4_ops);
    }
#[no_mangle]
unsafe extern "C" fn exit_route4() -> void __exit {
