...
// Qdisc to use by default
    ...
    {
    while (skb) {
    let mut reason: u32 = tc_skb_cb(skb).drop_reason;
...
//
    ...
    spinlock_t *lock = core::ptr::null_mut();
    struct sk_buff *skb;
...
#[no_mangle]
pub unsafe extern "C" fn dev_requeue_skb(skb: *mut sk_buff, q: *mut Qdisc) {
...
// it's still part of the queue
    ...
    {
    let mut bytelimit: c_int = qdisc_avail_bulklimit(txq) - skb.len;
    let mut cnt: c_int = 0;
    while (bytelimit > 0) {
...
//
    ...
    int *packets)
    {
    let mut mapping: c_int = skb_get_queue_mapping(skb);
    struct sk_buff *nskb;
    let mut cnt: c_int = 0;
    do {
    nskb = q.dequeue(q);
...
//
#[no_mangle]
pub unsafe extern "C" fn qdisc_restart(q: *mut Qdisc, packets: *mut c_int, budget: c_int) -> bool {
    static inline bool qdisc_restart(struct Qdisc *q, int *packets, int budget)
    {
    spinlock_t *root_lock = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn __qdisc_run(q: *mut Qdisc) {
    void __qdisc_run(struct Qdisc *q)
    {
    let mut quota: c_int = READ_ONCE(net_hotdata.dev_tx_weight);
    int packets;
...
#[no_mangle]
pub unsafe extern "C" fn dev_trans_start(dev: *mut net_device) -> c_ulong {
    unsigned long dev_trans_start(struct net_device *dev)
    {
    let mut res: c_ulong = READ_ONCE(netdev_get_tx_queue(dev, 0).trans_start);
    unsigned long val;
    unsigned int i;
...
#[no_mangle]
unsafe extern "C" fn netif_freeze_queues(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn netif_tx_lock(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn netif_unfreeze_queues(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn netif_tx_unlock(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn dev_watchdog(t: *mut timer_list) {
    ...
    let mut timedout_ms: c_uint = 0;
    unsigned int i;
    unsigned long trans_start;
    let mut oldest_start: c_ulong = jiffies;
    for (i = 0; i < dev.num_tx_queues; i++) {
    struct netdev_queue *txq;
...
#[no_mangle]
pub unsafe extern "C" fn netdev_watchdog_up(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn netdev_watchdog_down(dev: *mut net_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netif_carrier_on(dev: *mut net_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netif_carrier_off(dev: *mut net_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netif_carrier_event(dev: *mut net_device) {
...
// if this is NULL - so clear it here.
    qdisc.enqueue = core::ptr::null_mut();
    return 0;
    }
...
//
pub const PFIFO_FAST_BANDS: c_int = 3;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pfifo_fast_priv {
    pub q: [skb_array; PFIFO_FAST_BANDS],
}
    ...
    struct sk_buff **to_free)
    {
    let mut band: c_int = sch_default_prio2band[skb.priority & TC_PRIO_MAX];
...
#[no_mangle]
unsafe extern "C" fn pfifo_fast_reset(qdisc: *mut Qdisc) {
...
//
    if (!q.ring.queue)
    continue;
    while ((skb = __skb_array_consume(q)) != core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn pfifo_fast_dump(qdisc: *mut Qdisc, skb: *mut sk_buff) -> c_int {
    static int pfifo_fast_dump(struct Qdisc *qdisc, struct sk_buff *skb)
    {
    let mut opt: tc_prio_qopt = { .bands = PFIFO_FAST_BANDS };
    ...
    struct netlink_ext_ack *extack)
    {
    let mut qlen: c_uint = qdisc_dev(qdisc).tx_queue_len;
    struct pfifo_fast_priv *priv = qdisc_priv(qdisc);
    int prio;
...
#[no_mangle]
unsafe extern "C" fn pfifo_fast_destroy(sch: *mut Qdisc) {
...
//
    ptr_ring_cleanup(&q.ring, core::ptr::null_mut());
    }
    }
    ...
    {
    struct Qdisc *sch;
    let mut size: c_uint = sizeof(*sch) + ops.priv_size;
...
// seqlock has the same scope of busylock, for NOLOCK qdisc
    ...
    sch.parent = parentid;
    if (!ops.init || ops.init(sch, core::ptr::null_mut(), extack) == 0) {
...
#[no_mangle]
pub unsafe extern "C" fn qdisc_reset(qdisc: *mut Qdisc) {
...
#[no_mangle]
pub unsafe extern "C" fn qdisc_free(qdisc: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn qdisc_free_cb(head: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn __qdisc_destroy(qdisc: *mut Qdisc) {
...
#[no_mangle]
pub unsafe extern "C" fn qdisc_destroy(qdisc: *mut Qdisc) {
...
#[no_mangle]
pub unsafe extern "C" fn qdisc_put(qdisc: *mut Qdisc) {
...
//
#[no_mangle]
pub unsafe extern "C" fn qdisc_put_unlocked(qdisc: *mut Qdisc) {
...
#[no_mangle]
pub unsafe extern "C" fn if(ARPHRD_CAN: dev->type ==) -> else {
    else if(dev.type == ARPHRD_CAN)
    ops = &pfifo_fast_ops;
    qdisc = qdisc_create_dflt(dev_queue, ops, TC_H_ROOT, core::ptr::null_mut());
    if (!qdisc)
    return;
...
#[no_mangle]
unsafe extern "C" fn attach_default_qdiscs(dev: *mut net_device) {
    ...
    netdev_for_each_tx_queue(dev, attach_one_default_qdisc, core::ptr::null_mut());
    qdisc = rtnl_dereference(txq.qdisc_sleeping);
    rcu_assign_pointer(dev.qdisc, qdisc);
    qdisc_refcount_inc(qdisc);
    } else {
    qdisc = qdisc_create_dflt(txq, &mq_qdisc_ops, TC_H_ROOT, core::ptr::null_mut());
    if (qdisc) {
    rcu_assign_pointer(dev.qdisc, qdisc);
...
#[no_mangle]
pub unsafe extern "C" fn dev_activate(dev: *mut net_device) {
...
// Delay activation until next carrier-on event
    ...
    transition_one_qdisc(dev, dev_ingress_queue(dev), core::ptr::null_mut());
    if (need_watchdog) {
...
#[no_mangle]
unsafe extern "C" fn qdisc_deactivate(qdisc: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn some_qdisc_is_busy(dev: *mut net_device) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_deactivate_many(head: *mut list_head, reset_needed: bool) {
...
// Wait for outstanding qdisc enqueuing calls.
    ...
    if (reset_needed) {
    list_for_each_entry(dev, head, close_list) {
    netdev_for_each_tx_queue(dev, dev_reset_queue, core::ptr::null_mut());
    if (dev_ingress_queue(dev))
...
#[no_mangle]
pub unsafe extern "C" fn dev_deactivate(dev: *mut net_device, reset_needed: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn mq_change_real_num_tx(sch: *mut Qdisc, new_real_tx: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn dev_qdisc_change_tx_queue_len(dev: *mut net_device) -> c_int {
    int dev_qdisc_change_tx_queue_len(struct net_device *dev)
    {
    let mut up: bool = dev.flags & IFF_UP;
    unsigned int i;
    let mut ret: c_int = 0;
    if (up)
...
#[no_mangle]
pub unsafe extern "C" fn dev_init_scheduler(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn dev_shutdown(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn psched_ratecfg_precompute__(rate: u64, mult: *mut u32, shift: *mut u8) {
    static void psched_ratecfg_precompute__(u64 rate, u32 *mult, u8 *shift)
    {
    let mut factor: u64 = NSEC_PER_SEC;
// mult = 1;
// shift = 0;
    if (rate <= 0)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn psched_ppscfg_precompute(r: *mut psched_pktrate, pktrate64: u64) {
...
//
    ...
    if (!tp_head) {
    RCU_INIT_POINTER(*miniqp.p_miniq, core::ptr::null_mut());
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !poll_state_synchronize_rcu(miniq->rcu_state)) -> else {
