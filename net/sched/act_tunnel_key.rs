...
//! Source: net/sched/act_tunnel_key.c
#![no_std]
...
//
    ...
    struct netlink_ext_ack *extack)
    {
    return tunnel_key_copy_opts(nla, core::ptr::null_mut(), 0, extack);
    }
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_release_params_rcu(head: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_release_params(p: *mut tcf_tunnel_key_params) {
    ...
    struct metadata_dst *metadata = core::ptr::null_mut();
    struct tcf_chain *goto_ch = core::ptr::null_mut();
    struct tc_tunnel_key *parm;
    struct tcf_tunnel_key *t;
    let mut exists: bool = false;
    let mut dst_port: __be16 = 0;
    let mut key_id: __be64 = 0;
    let mut opts_len: c_int = 0;
    u8 tos, ttl;
    let mut ret: c_int = 0;
    u32 index;
    int err;
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_release(a: *mut tc_action) {
    ...
    let mut len: c_int = info.options_len;
    struct nlattr *start;
    ...
    const struct ip_tunnel_info *info)
    {
    let mut family: c_ushort = ip_tunnel_info_af(info);
    if (family == AF_INET) {
    let mut saddr: __be32 = info.key.u.ipv4.src;
    let mut daddr: __be32 = info.key.u.ipv4.dst;
    ...
    let mut key_id: __be32 = tunnel_id_to_key32(key.tun_id);
...
#[no_mangle]
unsafe extern "C" fn tcf_tunnel_encap_put_tunnel(priv: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_tcf_tunnel_release(act)) -> else {
    else if (is_tcf_tunnel_release(act))
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_opts_fill_size(info: *const ip_tunnel_info) -> usize {
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_get_fill_size(act: *const tc_action) -> usize {
...
// TCA_TUNNEL_KEY_PARMS
    let mut size: usize = nla_total_size(sizeof(struct tc_tunnel_key));
    rcu_read_lock();
    params = rcu_dereference(t.params);
    if (params.tcft_action == TCA_TUNNEL_KEY_ACT_SET) {
    const struct ip_tunnel_info *info =
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_exit_net(net_list: *mut list_head) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn tunnel_key_init_module() -> int __init {
    static int __init tunnel_key_init_module(void)
    {
    return tcf_register_action(&act_tunnel_key_ops, &tunnel_key_net_ops);
    }
#[no_mangle]
unsafe extern "C" fn tunnel_key_cleanup_module() -> void __exit {
