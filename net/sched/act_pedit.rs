...
#[no_mangle]
unsafe extern "C" fn tcf_pedit_cleanup_rcu(head: *mut rcu_head) {
    ...
    pedit_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    ...
    let mut offmask: u32 = nparms.tcfp_keys[i].offmask;
    let mut cur: u32 = nparms.tcfp_keys[i].off;
...
#[no_mangle]
unsafe extern "C" fn tcf_pedit_cleanup(a: *mut tc_action) {
...
#[no_mangle]
unsafe extern "C" fn offset_valid(skb: *mut sk_buff, offset: c_int, len: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn pedit_l4_skb_offset(skb: *mut sk_buff, hoffset: *mut c_int, header_type: c_int) -> c_int {
...
// hoffset = noff + iph->ihl * 4;
    ...
    ret = ipv6_find_hdr(skb, hoffset, header_type, core::ptr::null_mut(), core::ptr::null_mut()) == header_type ? 0 : -EINVAL;
    break;
    }
    out:
    return ret;
...
// hoffset = skb_network_offset(skb);
    ...
    {
    let mut htype: enum pedit_header_type = TCA_PEDIT_KEY_EX_HDR_TYPE_NETWORK;
    let mut cmd: enum pedit_cmd = TCA_PEDIT_KEY_EX_CMD_SET;
    struct tcf_pedit *p = to_pedit(a);
    ...
    let mut offset: c_int = tkey.off;
    let mut hoffset: c_int = 0;
    u32 cur_val, val;
    u32 *ptr;
    int rc;
    if (tkey_ex) {
...
// just do it, baby
    ...
    let mut nkeys: c_int = tcf_pedit_nkeys_locked(act);
    int k;
...
// index_inc = k;
    ...
    let mut cmd: u32 = tcf_pedit_cmd(act, 0);
    int k;
    switch (cmd) {
    case TCA_PEDIT_KEY_EX_CMD_SET:
...
#[no_mangle]
unsafe extern "C" fn tcf_pedit_get_fill_size(act: *const tc_action) -> usize {
...
#[no_mangle]
unsafe extern "C" fn pedit_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn pedit_exit_net(net_list: *mut list_head) -> void __net_exit {
    ...
    MODULE_LICENSE("GPL");
#[no_mangle]
unsafe extern "C" fn pedit_init_module() -> int __init {
    static int __init pedit_init_module(void)
    {
    return tcf_register_action(&act_pedit_ops, &pedit_net_ops);
    }
#[no_mangle]
unsafe extern "C" fn pedit_cleanup_module() -> void __exit {
