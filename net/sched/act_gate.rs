...
#[no_mangle]
unsafe extern "C" fn gate_get_time(gact: *mut tcf_gate) -> ktime_t {
    static ktime_t gate_get_time(struct tcf_gate *gact)
    {
    let mut mono: ktime_t = ktime_get();
...
#[no_mangle]
unsafe extern "C" fn gate_start_timer(gact: *mut tcf_gate, start: ktime_t) {
...
#[no_mangle]
unsafe extern "C" fn gate_timer_func(timer: *mut hrtimer) -> enum hrtimer_restart {
...
#[no_mangle]
unsafe extern "C" fn release_entry_list(entries: *mut list_head) {
...
// tko = TK_OFFS_TAI;
    ...
    let mut cycletime: u64 = 0, basetime = 0, cycletime_ext = 0;
    struct tcf_gate_params *p = core::ptr::null_mut(), *old_p = core::ptr::null_mut();
    let mut old_tk_offset: enum tk_offsets = TK_OFFS_TAI;
    const struct tcf_gate_params *cur_p = core::ptr::null_mut();
    let mut bind: bool = flags & TCA_ACT_FLAGS_BIND;
    struct nlattr *tb[TCA_GATE_MAX + 1];
    let mut tko: enum tk_offsets = TK_OFFS_TAI;
    struct tcf_chain *goto_ch = core::ptr::null_mut();
    let mut timer_clockid: i32 = CLOCK_TAI;
    let mut use_old_entries: bool = false;
    let mut old_clockid: i32 = CLOCK_TAI;
    let mut need_cancel: bool = false;
    let mut clockid: i32 = CLOCK_TAI;
    struct tcf_gate *gact;
    struct tc_gate *parm;
    let mut old_basetime: u64 = 0;
...
#[no_mangle]
unsafe extern "C" fn tcf_gate_cleanup(a: *mut tc_action) {
...
#[no_mangle]
unsafe extern "C" fn tcf_gate_get_fill_size(act: *const tc_action) -> usize {
    static size_t tcf_gate_get_fill_size(const struct tc_action *act)
    {
    return nla_total_size(sizeof(struct tc_gate));
    }
#[no_mangle]
unsafe extern "C" fn tcf_gate_entry_destructor(priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn gate_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn gate_exit_net(net_list: *mut list_head) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn gate_init_module() -> int __init {
    static int __init gate_init_module(void)
    {
    return tcf_register_action(&act_gate_ops, &gate_net_ops);
    }
#[no_mangle]
unsafe extern "C" fn gate_cleanup_module() -> void __exit {
