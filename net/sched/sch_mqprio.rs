...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mqprio_sched {
    pub qdiscs: *mut Qdisc,
    pub mode: u16,
    pub shaper: u16,
    pub hw_offload: c_int,
    pub flags: u32,
...
#[no_mangle]
unsafe extern "C" fn mqprio_disable_offload(sch: *mut Qdisc) {
    static void mqprio_disable_offload(struct Qdisc *sch)
    {
    let mut mqprio: tc_mqprio_qopt_offload = { { 0 } };
    struct mqprio_sched *priv = qdisc_priv(sch);
    struct net_device *dev = qdisc_dev(sch);
    switch (priv.mode) {
    case TC_MQPRIO_MODE_DCB:
...
#[no_mangle]
unsafe extern "C" fn mqprio_destroy(sch: *mut Qdisc) {
...
//
    ...
    let mut nlattr_opt_len: c_int = nla_len(opt) - NLA_ALIGN(sizeof(*qopt));
    struct mqprio_sched *priv = qdisc_priv(sch);
    ...
    struct tc_mqprio_qopt *qopt = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn mqprio_attach(sch: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn mqprio_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mqprio_find(sch: *mut Qdisc, classid: u32) -> c_ulong {
...
//
    ...
    struct netdev_queue *dev_queue = mqprio_queue_get(sch, cl);
    struct net_device *dev = qdisc_dev(sch);
    let mut tc: c_int = netdev_txq_to_tc(dev, cl - 1);
    ...
    if (gnet_stats_copy_basic(d, core::ptr::null_mut(), &bstats, false) < 0 ||
    gnet_stats_copy_queue(d, core::ptr::null_mut(), &qstats, qlen) < 0)
    res = -1;
    } else {
    struct netdev_queue *dev_queue = mqprio_queue_get(sch, cl);
...
#[no_mangle]
unsafe extern "C" fn mqprio_walk(sch: *mut Qdisc, arg: *mut qdisc_walker) {
...
#[no_mangle]
unsafe extern "C" fn mqprio_module_init() -> int __init {
    static int __init mqprio_module_init(void)
    {
    return register_qdisc(&mqprio_qdisc_ops);
    }
#[no_mangle]
unsafe extern "C" fn mqprio_module_exit() -> void __exit {
