...
pub const MIRRED_DEFER_LIMIT: c_int = 3;
    _Static_assert(MIRRED_DEFER_LIMIT <= 3,
    "MIRRED_DEFER_LIMIT exceeds tc_depth bitfield width");
    static LIST_HEAD(mirred_list);
    static DEFINE_SPINLOCK(mirred_list_lock);
#[no_mangle]
unsafe extern "C" fn tcf_mirred_is_act_redirect(action: c_int) -> bool {
    static bool tcf_mirred_is_act_redirect(int action)
    {
    let mut action: return = = TCA_EGRESS_REDIR || action == TCA_INGRESS_REDIR;
    }
#[no_mangle]
unsafe extern "C" fn tcf_mirred_act_wants_ingress(action: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcf_mirred_can_reinsert(action: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcf_mirred_release(a: *mut tc_action) {
...
// last reference to action, no need to lock
    ...
    let mut mac_header_xmit: bool = false;
    ...
    tcf_mirred_replace_dev(m, core::ptr::null_mut());
...
// target device/action expect data at mac
    ...
    {
    struct net_device *dev_prev = core::ptr::null_mut();
...
// If the packet wasn't redirected, we have to register as a drop
    ...
    {
    let mut exception_ifindex: u32 = skb.dev.ifindex;
...
// If it's not redirect, it is mirror
    ...
    {
    struct tcf_mirred *m = to_mirred(a);
    let mut retval: c_int = READ_ONCE(m.tcf_action);
    ...
    if (blockid) {
    if (!want_ingress)
    xmit.sched_mirred_dev[xmit.sched_mirred_nest++] = core::ptr::null_mut();
...
//
    RCU_INIT_POINTER(m.tcfm_dev, core::ptr::null_mut());
    }
    spin_unlock_bh(&m.tcf_lock);
    }
    spin_unlock(&mirred_list_lock);
...
#[no_mangle]
unsafe extern "C" fn tcf_mirred_dev_put(priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn tcf_mirred_get_fill_size(act: *const tc_action) -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_tcf_mirred_egress_mirror(act)) -> else {
    else if (is_tcf_mirred_egress_mirror(act))
    fl_action.id = FLOW_ACTION_MIRRED;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_tcf_mirred_ingress_redirect(act)) -> else {
    else if (is_tcf_mirred_ingress_redirect(act))
    fl_action.id = FLOW_ACTION_REDIRECT_INGRESS;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_tcf_mirred_ingress_mirror(act)) -> else {
    else if (is_tcf_mirred_ingress_mirror(act))
...
#[no_mangle]
unsafe extern "C" fn mirred_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn mirred_exit_net(net_list: *mut list_head) -> void __net_exit {
    ...
    MODULE_LICENSE("GPL");
#[no_mangle]
unsafe extern "C" fn mirred_init_module() -> int __init {
    static int __init mirred_init_module(void)
    {
    let mut err: c_int = register_netdevice_notifier(&mirred_device_notifier);
    if (err)
    return err;
...
#[no_mangle]
unsafe extern "C" fn mirred_cleanup_module() -> void __exit {
