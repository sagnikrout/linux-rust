...
pub struct pie_sched_data {
    ...
    let mut mtu: u32 = psched_mtu(qdisc_dev(sch));
...
// target is in us
    let mut target: u32 = nla_get_u32(tb[TCA_PIE_TARGET]);
...
// tupdate is in jiffies
    ...
    if (tb[TCA_PIE_LIMIT]) {
    let mut limit: u32 = nla_get_u32(tb[TCA_PIE_LIMIT]);
...
// Drop excess packets if new limit is lower
    ...
    {
    let mut now: psched_time_t = psched_get_time();
    let mut dtime: u32 = 0;
...
//
    ...
    let mut count: u32 = vars.dq_count << PIE_SCALE;
    dtime = now - vars.dq_tstamp;
    if (dtime == 0)
    return;
    count = count / dtime;
...
#[no_mangle]
unsafe extern "C" fn pie_timer(t: *mut timer_list) {
...
// reset the timer to fire after 'tupdate'. tupdate is in jiffies.
    ...
    if (opt) {
    let mut err: c_int = pie_change(sch, opt, extack);
    if (err)
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn pie_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pie_dump_stats(sch: *mut Qdisc, d: *mut gnet_dump) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pie_reset(sch: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn pie_destroy(sch: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn pie_module_init() -> int __init {
    static int __init pie_module_init(void)
    {
    return register_qdisc(&pie_qdisc_ops);
    }
#[no_mangle]
unsafe extern "C" fn pie_module_exit() -> void __exit {
