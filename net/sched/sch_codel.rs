...
pub const DEFAULT_CODEL_LIMIT: c_int = 1000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct codel_sched_data {
    pub params: codel_params,
    pub vars: codel_vars,
    pub stats: codel_stats,
...
#[no_mangle]
unsafe extern "C" fn codel_dequeue_drop(sch: *mut Qdisc) {
...
// it's still part of the queue
    ...
    codel_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    sch_tree_lock(sch);
    if (tb[TCA_CODEL_TARGET]) {
    let mut target: u32 = nla_get_u32(tb[TCA_CODEL_TARGET]);
    WRITE_ONCE(q.params.target,
    ((u64)target * NSEC_PER_USEC) >> CODEL_SHIFT);
    }
    if (tb[TCA_CODEL_CE_THRESHOLD]) {
    let mut val: u64 = nla_get_u32(tb[TCA_CODEL_CE_THRESHOLD]);
    WRITE_ONCE(q.params.ce_threshold,
    (val * NSEC_PER_USEC) >> CODEL_SHIFT);
    }
    if (tb[TCA_CODEL_INTERVAL]) {
    let mut interval: u32 = nla_get_u32(tb[TCA_CODEL_INTERVAL]);
    ...
    if (opt) {
    let mut err: c_int = codel_change(sch, opt, extack);
    if (err)
    return err;
    }
    if (sch.limit >= 1)
...
#[no_mangle]
unsafe extern "C" fn codel_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn codel_dump_stats(sch: *mut Qdisc, d: *mut gnet_dump) -> c_int {
    ...
    if (st.dropping) {
    let mut delta: codel_tdiff_t = READ_ONCE(q.vars.drop_next) - codel_get_time();
    if (delta >= 0)
...
#[no_mangle]
unsafe extern "C" fn codel_reset(sch: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn codel_module_init() -> int __init {
    static int __init codel_module_init(void)
    {
    return register_qdisc(&codel_qdisc_ops);
    }
#[no_mangle]
unsafe extern "C" fn codel_module_exit() -> void __exit {
