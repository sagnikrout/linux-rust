...
pub const HHF_BIT_MASK: c_uint = 0x3FF /* bitmask of 10 bits */;
...
pub struct hh_flow_state {
    pub /: *mut *mut u32 hash_id; / hash of flow-id (e.g. TCP 5-tuple),
    pub /: *mut *mut u32 hit_timestamp; / last time heavy-hitter was seen,
    pub /: *mut *mut list_head flowchain; / chaining under hash collision,
}
...
pub struct hhf_sched_data {
    ...
    pub /: *mut *mut siphash_key_t perturbation; / hash perturbation,
    pub /: *mut *mut u32 quantum; / psched_mtu(qdisc_dev(sch));,
    pub packet: *mut *mut u32 drop_overlimit; / number of times max qdisc,
// limit was hit
//
    pub /: *mut *mut *mut list_head hh_flows; / table T (currently active HHs),
    pub /: *mut *mut u32 hh_flows_limit; / max active HH allocs,
    pub /: *mut *mut u32 hh_flows_overlimit; / num of disallowed HH allocs,
    pub /: *mut *mut u32 hh_flows_total_cnt; / total admitted HHs,
    pub /: *mut *mut u32 hh_flows_current_cnt; / total current HHs,
    pub /: *mut *mut *mut u32 hhf_arrays[HHF_ARRAYS_CNT]; / HH filter F,
    pub hhf_arrays: *mut *mut u32 hhf_arrays_reset_timestamp; / last time,
// was reset
//
    pub bits: *mut *mut *mut unsigned long hhf_valid_bits[HHF_ARRAYS_CNT]; / shadow valid,
// of hhf_arrays
//
// Similar to the "new_flows" vs. "old_flows" concept in fq_codel DRR
    pub /: *mut *mut list_head new_buckets; / list of new buckets,
    pub /: *mut *mut list_head old_buckets; / list of old buckets,
// Configurable HHF parameters
    pub counter: *mut *mut u32 hhf_reset_timeout; / interval to reset,
// arrays in filter F
// (default 40ms)
//
    pub as: *mut *mut u32 hhf_admit_bytes; / counter thresh to classify,
// HH (default 128KB).
...
// sending > 25 Mbps.
//
    pub idle: *mut *mut u32 hhf_evict_timeout; / aging threshold to evict,
// HHs out of table T. This should
// be large enough to avoid
// reordering during HH eviction.
// (default 1s)
//
    pub non-HHs: *mut *mut u32 hhf_non_hh_weight; / WDRR weight for,
// (default 2,
// i.e., non-HH : HH = 2 : 1)
//
}
...
#[no_mangle]
unsafe extern "C" fn hhf_time_stamp() -> u32 {
...
// Looks up a heavy-hitter flow in a chaining list of table T.
    ...
    let mut now: u32 = hhf_time_stamp();
    if (list_empty(head))
    return core::ptr::null_mut();
    list_for_each_entry_safe(flow, next, head, flowchain) {
    let mut prev: u32 = flow.hit_timestamp + q.hhf_evict_timeout;
...
//
#[no_mangle]
unsafe extern "C" fn hhf_classify(skb: *mut sk_buff, sch: *mut Qdisc) -> enum wdrr_bucket_idx {
...
#[no_mangle]
unsafe extern "C" fn bucket_add(bucket: *mut wdrr_bucket, skb: *mut sk_buff) {
    static void bucket_add(struct wdrr_bucket *bucket, struct sk_buff *skb)
    {
    if (bucket.head == core::ptr::null_mut())
    bucket.head = skb;
    else
...
#[no_mangle]
unsafe extern "C" fn hhf_drop(sch: *mut Qdisc, to_free: *mut sk_buff) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn hhf_reset_classifier(q: *mut hhf_sched_data) {
...
#[no_mangle]
unsafe extern "C" fn hhf_reset(sch: *mut Qdisc) {
    static void hhf_reset(struct Qdisc *sch)
    {
    struct hhf_sched_data *q = qdisc_priv(sch);
    struct sk_buff *skb;
    while ((skb = hhf_dequeue(sch)) != core::ptr::null_mut())
    rtnl_kfree_skbs(skb, skb);
    hhf_reset_classifier(q);
    }
#[no_mangle]
unsafe extern "C" fn hhf_destroy(sch: *mut Qdisc) {
    ...
    int err;
    u64 non_hh_quantum;
    let mut new_quantum: u32 = q.quantum;
    let mut new_hhf_non_hh_weight: u32 = q.hhf_non_hh_weight;
    ...
    let mut us: u32 = nla_get_u32(tb[TCA_HHF_RESET_TIMEOUT]);
    ...
    let mut us: u32 = nla_get_u32(tb[TCA_HHF_EVICT_TIMEOUT]);
...
// Configurable HHF parameters
    ...
    q.quantum = 256;
    if (opt) {
    let mut err: c_int = hhf_change(sch, opt, extack);
    if (err)
    return err;
    }
    if (!q.hh_flows) {
...
#[no_mangle]
unsafe extern "C" fn hhf_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hhf_dump_stats(sch: *mut Qdisc, d: *mut gnet_dump) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hhf_module_init() -> int __init {
    static int __init hhf_module_init(void)
    {
    return register_qdisc(&hhf_qdisc_ops);
    }
#[no_mangle]
unsafe extern "C" fn hhf_module_exit() -> void __exit {
