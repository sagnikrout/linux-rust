...
unsafe extern "C" fn fifo_offload_dump(sch: *mut Qdisc) -> c_int {
    ...
    {
    bool bypass;
    let mut is_bfifo: bool = sch.ops == &bfifo_qdisc_ops;
    if (opt == core::ptr::null_mut()) {
    let mut limit: u32 = qdisc_dev(sch).tx_queue_len;
    if (is_bfifo)
...
#[no_mangle]
unsafe extern "C" fn fifo_destroy(sch: *mut Qdisc) {
...
unsafe extern "C" fn __fifo_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
    ...
    {
    let mut opt: tc_fifo_qopt = { .limit = READ_ONCE(sch.limit) };
...
#[no_mangle]
unsafe extern "C" fn fifo_hd_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
...
// Hack to avoid sending change message to non-FIFO
    ...
    ret = q.ops.change(q, nla, core::ptr::null_mut());
    kfree(nla);
    }
    return ret;
    }
