...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcf_exts_miss_cookie_node {
    pub chain: *const tcf_chain,
    pub tp: *const tcf_proto,
    pub exts: *const tcf_exts,
    pub chain_index: u32,
    pub tp_prio: u32,
    pub handle: u32,
    pub miss_cookie_base: u32,
    pub rcu: rcu_head,
}
...
#[no_mangle]
unsafe extern "C" fn tcf_exts_miss_cookie_base_destroy(exts: *mut tcf_exts) {
    ...
    {
    let mut mc: union tcf_exts_miss_cookie = { .miss_cookie = miss_cookie, };
...
#[no_mangle]
unsafe extern "C" fn tcf_exts_miss_cookie_get(miss_cookie_base: u32, act_index: c_int) -> u64 {
    static u64 tcf_exts_miss_cookie_get(u32 miss_cookie_base, int act_index)
    {
    let mut mc: union tcf_exts_miss_cookie = { .act_index = act_index, };
    if (!miss_cookie_base)
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn tc_skb_ext_tc_enable() {
...
#[no_mangle]
pub unsafe extern "C" fn tc_skb_ext_tc_disable() {
...
#[no_mangle]
unsafe extern "C" fn destroy_obj_hashfn(tp: *const tcf_proto) -> u32 {
    ...
    struct tcf_proto *tp)
    {
    let mut hash: u32 = destroy_obj_hashfn(tp);
...
// Find classifier type by string name
    ...
    {
    const struct tcf_proto_ops *t, *res = core::ptr::null_mut();
    if (kind) {
...
#[no_mangle]
pub unsafe extern "C" fn register_tcf_proto_ops(ops: *mut tcf_proto_ops) -> c_int {
    int register_tcf_proto_ops(struct tcf_proto_ops *ops)
    {
    struct tcf_proto_ops *t;
    let mut rc: c_int = -EEXIST;
...
#[no_mangle]
pub unsafe extern "C" fn unregister_tcf_proto_ops(ops: *mut tcf_proto_ops) {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_queue_work(rwork: *mut rcu_work, func: work_func_t) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_auto_prio(tp: *mut tcf_proto) -> u32 {
    static inline u32 tcf_auto_prio(struct tcf_proto *tp)
    {
    let mut first: u32 = TC_H_MAKE(0xC0000000U, 0U);
    if (tp)
    first = tp.prio - 1;
    return TC_H_MAJ(first);
    }
#[no_mangle]
unsafe extern "C" fn tcf_proto_check_kind(kind: *mut nlattr, name: *mut c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcf_proto_is_unlocked(kind: *const c_char) -> bool {
    ...
    if (strlen(kind) == 0)
    return false;
    ops = tcf_proto_lookup_ops(kind, false, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn tcf_proto_get(tp: *mut tcf_proto) {
    static void tcf_proto_get(struct tcf_proto *tp)
    {
    refcount_inc(&tp.refcnt);
    }
#[no_mangle]
unsafe extern "C" fn tcf_proto_count_usesw(tp: *mut tcf_proto, add: bool) {
...
#[no_mangle]
unsafe extern "C" fn tcf_proto_check_delete(tp: *mut tcf_proto) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcf_proto_mark_delete(tp: *mut tcf_proto) {
...
#[no_mangle]
unsafe extern "C" fn tcf_proto_is_deleting(tp: *mut tcf_proto) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcf_filter_chain_list_item {
    pub list: list_head,
...
#[no_mangle]
unsafe extern "C" fn tcf_chain_detach(chain: *mut tcf_chain) -> bool {
    ...
    if (!chain.index)
    block.chain0.chain = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn tcf_block_destroy(block: *mut tcf_block) {
...
#[no_mangle]
unsafe extern "C" fn tcf_chain_destroy(chain: *mut tcf_chain, free_block: bool) {
...
#[no_mangle]
unsafe extern "C" fn tcf_chain_hold(chain: *mut tcf_chain) {
...
#[no_mangle]
unsafe extern "C" fn tcf_chain_held_by_acts_only(chain: *mut tcf_chain) -> bool {
...
//
    ...
    bool by_act)
    {
    struct tcf_chain *chain = core::ptr::null_mut();
...
//
    ...
    tc_chain_notify(chain, core::ptr::null_mut(), 0, NLM_F_CREATE | NLM_F_EXCL,
    RTM_NEWCHAIN, false, core::ptr::null_mut());
    return chain;
    errout:
    mutex_unlock(&block.lock);
    return chain;
...
//
    ...
    chain.index, block, core::ptr::null_mut(), 0, 0);
...
#[no_mangle]
unsafe extern "C" fn tcf_chain_put(chain: *mut tcf_chain) {
    static void tcf_chain_put(struct tcf_chain *chain)
    {
    __tcf_chain_put(chain, false, false);
    }
#[no_mangle]
pub unsafe extern "C" fn tcf_chain_put_by_act(chain: *mut tcf_chain) {
...
#[no_mangle]
unsafe extern "C" fn tcf_chain_put_explicitly_created(chain: *mut tcf_chain) {
    static void tcf_chain_put_explicitly_created(struct tcf_chain *chain)
    {
    __tcf_chain_put(chain, false, true);
    }
#[no_mangle]
unsafe extern "C" fn tcf_chain_flush(chain: *mut tcf_chain, rtnl_held: bool) {
    ...
    tp = tcf_chain_dereference(chain.filter_chain, chain);
    RCU_INIT_POINTER(chain.filter_chain, core::ptr::null_mut());
    tcf_chain0_head_change(chain, core::ptr::null_mut());
    chain.flushing = true;
    mutex_unlock(&chain.filter_chain_lock);
    while (tp) {
    tp_next = rcu_dereference_protected(tp.next, 1);
    tcf_proto_put(tp, rtnl_held, core::ptr::null_mut());
    tp = tp_next;
    }
    }
    ...
    struct flow_block_offload *bo);
#[no_mangle]
unsafe extern "C" fn tc_block_indr_cleanup(block_cb: *mut flow_block_cb) {
    static void tc_block_indr_cleanup(struct flow_block_cb *block_cb)
    {
    struct tcf_block *block = block_cb.indr.data;
    struct net_device *dev = block_cb.indr.dev;
    struct Qdisc *sch = block_cb.indr.sch;
    let mut extack: netlink_ext_ack = {};
    let mut bo: flow_block_offload = {};
...
#[no_mangle]
unsafe extern "C" fn tcf_block_offload_in_use(block: *mut tcf_block) -> bool {
...
//
    ...
    int err;
    down_write(&block.cb_lock);
    err = tcf_block_offload_cmd(block, dev, q, ei, FLOW_BLOCK_UNBIND, core::ptr::null_mut());
    if (err == -EOPNOTSUPP)
    ...
    tcf_chain_head_change_item(item, core::ptr::null_mut());
    list_del(&item.list);
    mutex_unlock(&block.lock);
    kfree(item);
    return;
...
#[derive(Copy, Clone)]
pub struct tcf_net {
    pub /: *mut *mut spinlock_t idr_lock; / Protects idr,
    pub idr: idr,
}
...
#[no_mangle]
unsafe extern "C" fn tcf_block_remove(block: *mut tcf_block, net: *mut net) {
...
// Don't store q pointer for blocks which are shared
    ...
    core::ptr::null_mut() : list_next_entry(chain, list);
    else
...
//
    ...
    if (tp)
    tcf_proto_put(tp, true, core::ptr::null_mut());
    return tp_next;
    }
    EXPORT_SYMBOL(tcf_get_next_proto);
#[no_mangle]
unsafe extern "C" fn tcf_block_flush_all_chains(block: *mut tcf_block, rtnl_held: bool) {
...
//
    for (chain = tcf_get_next_chain(block, core::ptr::null_mut());
    chain;
...
//
    let mut free_block: bool = list_empty(&block.chain_list);
    mutex_unlock(&block.lock);
    if (tcf_block_shared(block))
...
#[no_mangle]
unsafe extern "C" fn tcf_block_refcnt_put(block: *mut tcf_block, rtnl_held: bool) {
    static void tcf_block_refcnt_put(struct tcf_block *block, bool rtnl_held)
    {
    __tcf_block_put(block, core::ptr::null_mut(), core::ptr::null_mut(), rtnl_held);
    }
// Find tcf block.
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcf_block_owner_item {
    pub list: list_head,
    pub q: *mut Qdisc,
...
#[no_mangle]
pub unsafe extern "C" fn tcf_block_netif_keep_dst(block: *mut tcf_block) {
    ...
    struct tcf_block *block = core::ptr::null_mut();
    int err;
    if (ei.block_index)
...
#[no_mangle]
unsafe extern "C" fn tcf_chain_head_change_dflt(tp_head: *mut tcf_proto, priv: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_block_put(block: *mut tcf_block) {
    void tcf_block_put(struct tcf_block *block)
    {
    let mut ei: tcf_block_ext_info = {0, };
    if (!block)
    return;
    ...
    for (chain = __tcf_get_next_chain(block, core::ptr::null_mut());
    chain;
    chain_prev = chain,
    ...
    cb_priv);
    for (tp = __tcf_get_next_proto(chain, core::ptr::null_mut()); tp;
    tp_prev = tp,
    tp = __tcf_get_next_proto(chain, tp),
    tcf_proto_put(tp_prev, true, core::ptr::null_mut())) {
    if (tp.ops.reoffload) {
...
//
    ...
    let mut max_reclassify_loop: c_int = 16;
    const struct tcf_proto *first_tp;
    let mut limit: c_int = 0;
    reclassify:

    for (; tp; tp = rcu_dereference_bh(tp.next)) {
    let mut protocol: __be16 = skb_protocol(skb, false);
    let mut err: c_int = 0;
    if (n) {
...
// last_executed_chain = err & TC_ACT_EXT_VAL_MASK;
    ...
    let mut last_executed_chain: u32 = 0;
    return __tcf_classify(skb, tp, tp, res, compat_mode, core::ptr::null_mut(), 0,
    &last_executed_chain);

    let mut last_executed_chain: u32 = tp ? tp.chain.index : 0;
    struct tcf_exts_miss_cookie_node *n = core::ptr::null_mut();
    const struct tcf_proto *orig_tp = tp;
    struct tc_skb_ext *ext;
    let mut act_index: c_int = 0;
    int ret;
    if (block) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcf_chain_info {
    pub pprev: *mut tcf_proto __rcu,
...
//
    ...
    mutex_unlock(&chain.filter_chain_lock);
    tcf_proto_destroy(tp_new, rtnl_held, false, core::ptr::null_mut());
    return ERR_PTR(-EAGAIN);
    }
    tp = tcf_chain_tp_find(chain, &chain_info, protocol, prio, false, core::ptr::null_mut());
    if (!tp)
...
// Check the chain for existence of proto-tcf with this priority
    ...
    chain_info.next = core::ptr::null_mut();
    }
    return tp;
    }
    ...
    struct netlink_ext_ack *extack)
    {
    let mut size: c_uint = oskb ? max(NLMSG_GOODSIZE, oskb.len) : NLMSG_GOODSIZE;
    struct sk_buff *skb;
    int ret;
    retry:
    ...
    {
    struct sk_buff *skb;
    let mut portid: u32 = oskb ? NETLINK_CB(oskb).portid : 0;
    let mut err: c_int = 0;
    ...
    {
    struct tcf_proto *tp;
    for (tp = tcf_get_next_proto(chain, core::ptr::null_mut());
    tp; tp = tcf_get_next_proto(chain, tp))
    tfilter_notify(net, oskb, n, tp, block, q, parent, core::ptr::null_mut(),
    event, false, true, extack);
    }
#[no_mangle]
unsafe extern "C" fn tfilter_put(tp: *mut tcf_proto, fh: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn is_qdisc_ingress(classid: __u32) -> bool {
    static bool is_qdisc_ingress(__u32 classid)
    {
    return (TC_H_MIN(classid) == TC_H_MIN(TC_H_MIN_INGRESS));
    }
#[no_mangle]
unsafe extern "C" fn is_ingress_or_clsact(block: *mut tcf_block, q: *mut Qdisc) -> bool {
...
//
    ...
    struct tcf_proto *tp_new = core::ptr::null_mut();
    if (chain.flushing) {
    err = -EAGAIN;
    goto errout_locked;
    }
// Proto-tcf does not exist, create new one
    if (tca[TCA_KIND] == core::ptr::null_mut() || !protocol) {
...
// q pointer is NULL for shared blocks
    ...
    tcf_chain_tp_delete_empty(chain, tp, rtnl_held, core::ptr::null_mut());
    errout_tp:
    if (chain) {
    if (tp && !IS_ERR(tp))
    tcf_proto_put(tp, rtnl_held, core::ptr::null_mut());
...
// Replay the request.
    ...
    u32 parent;
    u32 chain_index;
    struct Qdisc *q = core::ptr::null_mut();
    struct tcf_chain_info chain_info;
    struct tcf_chain *chain = core::ptr::null_mut();
    struct tcf_block *block = core::ptr::null_mut();
    struct tcf_proto *tp = core::ptr::null_mut();
    let mut cl: c_ulong = 0;
    void *fh = core::ptr::null_mut();
    int err;
    let mut rtnl_held: bool = false;
    err = nlmsg_parse_deprecated(n, sizeof(*t), tca, TCA_MAX,
    rtm_tca_policy, extack);
...
//
    ...
    fh, RTM_NEWTFILTER, true, rtnl_held, core::ptr::null_mut());
    if (err < 0)
...
#[no_mangle]
unsafe extern "C" fn tcf_node_dump(tp: *mut tcf_proto, n: *mut c_void, arg: *mut tcf_walker) -> c_int {
    ...
    RTM_NEWTFILTER, a.terse_dump, true, core::ptr::null_mut());
    }
    ...
    for (tp = __tcf_get_next_proto(chain, core::ptr::null_mut());
    tp;
    tp_prev = tp,
    tp = __tcf_get_next_proto(chain, tp),
    tcf_proto_put(tp_prev, true, core::ptr::null_mut()),
    ...
    if (cb.args[1] == 0) {
    if (tcf_fill_node(net, skb, tp, block, q, parent, core::ptr::null_mut(),
    NETLINK_CB(cb.skb).portid,
    cb.nlh.nlmsg_seq, NLM_F_MULTI,
    RTM_NEWTFILTER, false, true, core::ptr::null_mut()) <= 0)
    goto errout;
    cb.args[1] = 1;
    }
    if (!tp.ops.walk)
...
// called with RTNL
#[no_mangle]
unsafe extern "C" fn tc_dump_tfilter(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
...
//
    ...
    block = cops.tcf_block(q, cl, core::ptr::null_mut());
    if (!block)
    goto out;
...
// If we did no progress, the error (EMSGSIZE) is real
    ...
    block, portid, seq, flags, RTM_DELCHAIN, core::ptr::null_mut()) <= 0) {
    kfree_skb(skb);
    return -EINVAL;
    }
...
// called with RTNL
#[no_mangle]
unsafe extern "C" fn tc_dump_chain(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
    ...
    NETLINK_CB(cb.skb).portid,
    cb.nlh.nlmsg_seq, NLM_F_MULTI,
    RTM_NEWCHAIN, core::ptr::null_mut());
    if (err <= 0)
    break;
    index++;
    }
...
// If we did no progress, the error (EMSGSIZE) is real
    ...
    exts.type = 0;
    exts.nr_actions = 0;
    exts.miss_cookie_node = core::ptr::null_mut();
...
//
    ...
    exts.actions = core::ptr::null_mut();

    return err;
    }
    EXPORT_SYMBOL(tcf_exts_init_ex);
#[no_mangle]
pub unsafe extern "C" fn tcf_exts_destroy(exts: *mut tcf_exts) {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_exts_change(dst: *mut tcf_exts, src: *mut tcf_exts) {
    void tcf_exts_change(struct tcf_exts *dst, struct tcf_exts *src)
    {

    let mut old: tcf_exts = *dst;
// dst = *src;
...
#[no_mangle]
pub unsafe extern "C" fn tcf_exts_dump(skb: *mut sk_buff, exts: *mut tcf_exts) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_exts_terse_dump(skb: *mut sk_buff, exts: *mut tcf_exts) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_exts_dump_stats(skb: *mut sk_buff, exts: *mut tcf_exts) -> c_int {
    int tcf_exts_dump_stats(struct sk_buff *skb, struct tcf_exts *exts)
    {

    struct tc_action *a = tcf_exts_first_act(exts);
    if (a != core::ptr::null_mut() && tcf_action_copy_stats(skb, a, 1) < 0)
    return -1;
...
#[no_mangle]
unsafe extern "C" fn tcf_block_offload_inc(block: *mut tcf_block, flags: *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn tcf_block_offload_dec(block: *mut tcf_block, flags: *mut u32) {
...
// cnt = 0;
    ...
    {
    let mut take_rtnl: bool = READ_ONCE(block.lockeddevcnt) && !rtnl_held;
    int ok_count;
    retry:
    if (take_rtnl)
    rtnl_lock();
...
//
    ...
    {
    let mut err: c_int = cb(type, type_data, cb_priv);
    if (err) {
...
#[no_mangle]
unsafe extern "C" fn tcf_act_put_user_cookie(entry: *mut flow_action_entry) {
    static void tcf_act_put_user_cookie(struct flow_action_entry *entry)
    {
    flow_action_cookie_destroy(entry.user_cookie);
    }
#[no_mangle]
pub unsafe extern "C" fn tc_cleanup_offload_action(flow_action: *mut flow_action) {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_exts_num_actions(exts: *mut tcf_exts) -> c_uint {
    unsigned int tcf_exts_num_actions(struct tcf_exts *exts)
    {
    let mut num_acts: c_uint = 0;
    struct tc_action *act;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn tcf_qevent_destroy(qe: *mut tcf_qevent, sch: *mut Qdisc) {
...
#[no_mangle]
pub unsafe extern "C" fn tcf_qevent_dump(skb: *mut sk_buff, attr_name: c_int, qe: *mut tcf_qevent) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tcf_net_init(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn tcf_net_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn tc_filter_init() -> int __init {
