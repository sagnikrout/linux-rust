...
// Each policer is serialized by its individual spinlock
    ...
    {
    let mut ret: c_int = 0, tcfp_result = TC_ACT_OK, err, size;
    let mut bind: bool = flags & TCA_ACT_FLAGS_BIND;
    struct nlattr *tb[TCA_POLICE_MAX + 1];
    struct tcf_chain *goto_ch = core::ptr::null_mut();
    struct tc_police *parm;
    struct tcf_police *police;
    struct qdisc_rate_table *R_tab = core::ptr::null_mut(), *P_tab = core::ptr::null_mut();
    ...
    police_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    if (tb[TCA_POLICE_TBF] == core::ptr::null_mut())
    return -EINVAL;
    ...
    return ACT_P_BOUND;
    if (!exists) {
    ret = tcf_idr_create(tn, index, core::ptr::null_mut(), a,
    ...
    if (parm.rate.rate) {
    err = -ENOMEM;
    R_tab = qdisc_get_rtab(&parm.rate, tb[TCA_POLICE_RATE], core::ptr::null_mut());
    if (R_tab == core::ptr::null_mut())
    goto failure;
    if (parm.peakrate.rate) {
    P_tab = qdisc_get_rtab(&parm.peakrate,
    tb[TCA_POLICE_PEAKRATE], core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn tcf_police_mtu_check(skb: *mut sk_buff, limit: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcf_police_cleanup(a: *mut tc_action) {
    ...
    struct netlink_ext_ack *extack)
    {
    let mut act_id: c_int = -EOPNOTSUPP;
    if (!TC_ACT_EXT_OPCODE(tc_act)) {
    if (tc_act == TC_ACT_OK)
    act_id = FLOW_ACTION_ACCEPT;
#[no_mangle]
pub unsafe extern "C" fn if(TC_ACT_SHOT: tc_act ==) -> else {
    else if (tc_act ==  TC_ACT_SHOT)
    act_id = FLOW_ACTION_DROP;
#[no_mangle]
pub unsafe extern "C" fn if(TC_ACT_PIPE: tc_act ==) -> else {
    else if (tc_act == TC_ACT_PIPE)
    act_id = FLOW_ACTION_PIPE;
#[no_mangle]
pub unsafe extern "C" fn if(TC_ACT_RECLASSIFY: tc_act ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcf_police_get_fill_size(act: *const tc_action) -> usize {
...
#[no_mangle]
unsafe extern "C" fn police_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn police_exit_net(net_list: *mut list_head) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn police_init_module() -> int __init {
    static int __init police_init_module(void)
    {
    return tcf_register_action(&act_police_ops, &police_net_ops);
    }
#[no_mangle]
unsafe extern "C" fn police_cleanup_module() -> void __exit {
