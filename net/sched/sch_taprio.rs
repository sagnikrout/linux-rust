...
//
pub const TAPRIO_PICOS_PER_BYTE_MIN: c_int = 17;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sched_entry {
// Durations between this GCL entry and the GCL entry where the
// respective traffic class gate closes
//
    pub gate_duration: [u64; TC_MAX_QUEUE],
    pub budget: [core::sync::atomic::AtomicI32; TC_MAX_QUEUE],
// The qdisc makes some effort so that no packet leaves
// after this time
//
    pub gate_close_time: [ktime_t; TC_MAX_QUEUE],
    pub list: list_head,
// Used to calculate when to advance the schedule
    pub end_time: ktime_t,
    pub next_txtime: ktime_t,
    pub index: c_int,
    pub gate_mask: u32,
    pub interval: u32,
    pub command: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sched_gate_list {
// Longest non-zero contiguous gate durations per traffic class,
// or 0 if a traffic class gate never opens during the schedule.
//
    pub max_open_gate_duration: [u64; TC_MAX_QUEUE],
    pub /: *mut *mut u32 max_frm_len[TC_MAX_QUEUE]; / for the fast path,
    pub /: *mut *mut u32 max_sdu[TC_MAX_QUEUE]; / for dump,
    pub rcu: rcu_head,
    pub entries: list_head,
    pub num_entries: usize,
    pub cycle_end_time: ktime_t,
    pub cycle_time: i64,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct taprio_sched {
    pub qdiscs: *mut Qdisc,
    pub root: *mut Qdisc,
    pub flags: u32,
    pub tk_offset: enum tk_offsets,
    pub clockid: c_int,
    pub offloaded: bool,
    pub detected_mqprio: bool,
    pub broken_mqprio: bool,
    pub 10Gbps+: *mut *mut atomic64_t picos_per_byte; / Using picoseconds because for,
// speeds it's sub-nanoseconds per byte
//
// Protects the update side of the RCU protected current_entry
    pub current_entry_lock: spinlock_t,
    pub current_entry: *mut sched_entry __rcu,
    pub oper_sched: *mut sched_gate_list __rcu,
    pub admin_sched: *mut sched_gate_list __rcu,
    pub advance_timer: hrtimer,
    pub taprio_list: list_head,
    pub cur_txq: [c_int; TC_MAX_QUEUE],
    pub /: *mut *mut u32 max_sdu[TC_MAX_QUEUE]; / save info from the user,
    pub /: *mut *mut u32 fp[TC_QOPT_MAX_QUEUE]; / only for dump and offloading,
    pub txtime_delay: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct __tc_taprio_qopt_offload {
    pub users: refcount_t,
    pub offload: tc_taprio_qopt_offload,
}
    ...
    let mut num_tc: c_int = netdev_get_num_tc(dev);
    struct sched_entry *entry, *cur;
    int tc;
    list_for_each_entry(entry, &sched.entries, list) {
    let mut gates_still_open: u32 = entry.gate_mask;
...
#[no_mangle]
unsafe extern "C" fn sched_base_time(sched: *const sched_gate_list) -> ktime_t {
...
#[no_mangle]
unsafe extern "C" fn taprio_mono_to_any(q: *const taprio_sched, mono: ktime_t) -> ktime_t {
    static ktime_t taprio_mono_to_any(const struct taprio_sched *q, ktime_t mono)
    {
// This pairs with WRITE_ONCE() in taprio_parse_clockid()
    let mut tk_offset: enum tk_offsets = READ_ONCE(q.tk_offset);
    switch (tk_offset) {
    case TK_OFFS_MAX:
    return mono;
    default:
...
#[no_mangle]
unsafe extern "C" fn taprio_get_time(q: *const taprio_sched) -> ktime_t {
    static ktime_t taprio_get_time(const struct taprio_sched *q)
    {
    return taprio_mono_to_any(q, ktime_get());
    }
#[no_mangle]
unsafe extern "C" fn taprio_free_sched_cb(head: *mut rcu_head) {
    ...
    rcu_assign_pointer(q.admin_sched, core::ptr::null_mut());
    if (*oper)
...
#[no_mangle]
unsafe extern "C" fn get_cycle_time_elapsed(sched: *mut sched_gate_list, time: ktime_t) -> i32 {
    ...
    ktime_t intv_start)
    {
    let mut cycle_elapsed: i32 = get_cycle_time_elapsed(sched, intv_start);
...
#[no_mangle]
unsafe extern "C" fn length_to_duration(q: *mut taprio_sched, len: c_int) -> c_int {
    static int length_to_duration(struct taprio_sched *q, int len)
    {
    return div_u64(len * atomic64_read(&q.picos_per_byte), PSEC_PER_NSEC);
    }
#[no_mangle]
unsafe extern "C" fn duration_to_length(q: *mut taprio_sched, duration: u64) -> c_int {
...
//
    ...
    let mut earliest_txtime: ktime_t = KTIME_MAX, txtime, cycle, transmit_end_time;
    struct sched_entry *entry = core::ptr::null_mut(), *entry_found = core::ptr::null_mut();
    struct taprio_sched *q = qdisc_priv(sch);
    struct net_device *dev = qdisc_dev(sch);
    let mut entry_available: bool = false;
    s32 cycle_elapsed;
    int tc, n;
...
#[no_mangle]
unsafe extern "C" fn is_valid_interval(skb: *mut sk_buff, sch: *mut Qdisc) -> bool {
...
#[no_mangle]
unsafe extern "C" fn get_tcp_tstamp(q: *mut taprio_sched, skb: *mut sk_buff) -> ktime_t {
...
//
#[no_mangle]
unsafe extern "C" fn get_packet_txtime(skb: *mut sk_buff, sch: *mut Qdisc) -> c_long {
...
// Devices with full offload are expected to honor this in hardware
    ...
    struct net_device *dev = qdisc_dev(sch);
    struct sched_gate_list *sched;
    let mut prio: c_int = skb.priority;
    let mut exceeds: bool = false;
    u8 tc;
...
// sk_flags are only safe to use on full sockets.
    ...
    {
    let mut slen: c_uint = 0, numsegs = 0, len = qdisc_pkt_len(skb);
...
#[no_mangle]
unsafe extern "C" fn taprio_next_tc_txq(dev: *mut net_device, tc: c_int, txq: *mut c_int) {
...
//
    ...
    let mut first_txq: c_int = q.cur_txq[tc];
    if (!(gate_mask & BIT(tc)))
    continue;
    do {
...
#[no_mangle]
unsafe extern "C" fn advance_sched(timer: *mut hrtimer) -> enum hrtimer_restart {
...
//
    ...
    {
    let mut min_duration: c_int = length_to_duration(q, ETH_ZLEN);
...
//
    ...
    entry_policy, core::ptr::null_mut());
    if (err < 0) {
    ...
    u32 taprio_flags)
    {
    let mut allow_overlapping_txqs: bool = TXTIME_ASSIST_IS_ENABLED(taprio_flags);
    if (!qopt) {
...
// FIXME: find a better place to do this
    ...
    rcu_assign_pointer(q.current_entry, core::ptr::null_mut());
    }
...
//
    ...
    continue;
    taprio_set_picos_per_byte(dev, q, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn taprio_offload_free(offload: *mut tc_taprio_qopt_offload) {
...
//
#[no_mangle]
unsafe extern "C" fn taprio_offload_config_changed(q: *mut taprio_sched) {
...
#[no_mangle]
unsafe extern "C" fn tc_map_to_queue_mask(dev: *mut net_device, tc_mask: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn taprio_detect_broken_mqprio(q: *mut taprio_sched) {
...
#[no_mangle]
unsafe extern "C" fn taprio_cleanup_broken_mqprio(q: *mut taprio_sched) {
...
//
    offload.extack = core::ptr::null_mut();
    offload.mqprio.extack = core::ptr::null_mut();
...
//
    ...
    let mut clockid: c_int = nla_get_s32(tb[TCA_TAPRIO_ATTR_SCHED_CLOCKID]);
...
// seen_tcs |= BIT(tc);
    ...
    struct net_device *dev = qdisc_dev(sch);
    u32 max_sdu[TC_QOPT_MAX_QUEUE];
    let mut have_preemption: bool = false;
    let mut seen_tcs: c_ulong = 0;
    u32 fp[TC_QOPT_MAX_QUEUE];
    struct nlattr *n;
    int tc, rem;
    let mut err: c_int = 0;
    ...
    struct taprio_sched *q = qdisc_priv(sch);
    struct net_device *dev = qdisc_dev(sch);
    struct tc_mqprio_qopt *mqprio = core::ptr::null_mut();
    unsigned long flags;
    u32 taprio_flags;
    ktime_t start;
    int i, err;
...
#[no_mangle]
unsafe extern "C" fn taprio_reset(sch: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn taprio_destroy(sch: *mut Qdisc) {
...
//
    ...
    taprio_disable_offload(dev, q, core::ptr::null_mut());
    if (q.qdiscs) {
    for (i = 0; i < dev.num_tx_queues; i++)
    qdisc_put(q.qdiscs[i]);
    kfree(q.qdiscs);
    }
    q.qdiscs = core::ptr::null_mut();
    netdev_reset_tc(dev);
    oper = rtnl_dereference(q.oper_sched);
    admin = rtnl_dereference(q.admin_sched);
    if (oper)
...
#[no_mangle]
unsafe extern "C" fn taprio_attach(sch: *mut Qdisc) {
...
#[no_mangle]
unsafe extern "C" fn taprio_put_stat(skb: *mut sk_buff, val: u64, attrtype: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn taprio_dump_stats(sch: *mut Qdisc, d: *mut gnet_dump) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn taprio_dump(sch: *mut Qdisc, skb: *mut sk_buff) -> c_int {
    static int taprio_dump(struct Qdisc *sch, struct sk_buff *skb)
    {
    struct taprio_sched *q = qdisc_priv(sch);
    struct net_device *dev = qdisc_dev(sch);
    struct sched_gate_list *oper, *admin;
    let mut opt: tc_mqprio_qopt = { 0 };
    ...
    struct taprio_sched *q = qdisc_priv(sch);
    struct net_device *dev = qdisc_dev(sch);
    let mut ntx: c_uint = cl - 1;
    if (ntx >= dev.num_tx_queues)
    return core::ptr::null_mut();
    return q.qdiscs[ntx];
    }
#[no_mangle]
unsafe extern "C" fn taprio_find(sch: *mut Qdisc, classid: u32) -> c_ulong {
    static unsigned long taprio_find(struct Qdisc *sch, u32 classid)
    {
    let mut ntx: c_uint = TC_H_MIN(classid);
    ...
    if (gnet_stats_copy_basic(d, core::ptr::null_mut(), &child.bstats, true) < 0 ||
    qdisc_qstats_copy(d, child) < 0)
    return -1;
    return taprio_dump_xstats(sch, d, &offload, &offload.queue_stats.stats);
    }
#[no_mangle]
unsafe extern "C" fn taprio_walk(sch: *mut Qdisc, arg: *mut qdisc_walker) {
...
#[no_mangle]
unsafe extern "C" fn taprio_module_init() -> int __init {
    static int __init taprio_module_init(void)
    {
    let mut err: c_int = register_netdevice_notifier(&taprio_device_notifier);
    if (err)
    return err;
    return register_qdisc(&taprio_qdisc_ops);
    }
#[no_mangle]
unsafe extern "C" fn taprio_module_exit() -> void __exit {
