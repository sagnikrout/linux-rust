...
pub struct tc_u_knode {
    pub next: *mut tc_u_knode __rcu,
    pub handle: u32,
    pub ht_up: *mut tc_u_hnode __rcu,
    pub exts: tcf_exts,
    pub ifindex: c_int,
    pub fshift: u8,
    pub res: tcf_result,
    pub ht_down: *mut tc_u_hnode __rcu,

    pub pf: *mut tc_u32_pcnt __percpu,
    ...
    pub val: u32,
    pub mask: u32,
    pub pcpu_success: *mut u32 __percpu,
...
//
    pub ht: [*mut tc_u_knode __rcu; ],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct tc_u_common {
    pub hlist: *mut tc_u_hnode __rcu,
    pub ptr: *mut c_void,
    pub refcnt: refcount_t,
    pub handle_idr: idr,
...
#[no_mangle]
unsafe extern "C" fn handle2id(h: u32) -> u32 {
    static u32 handle2id(u32 h)
    {
    return ((h & 0x80000000) ? ((h >> 20) & 0x7FF) : h);
    }
#[no_mangle]
unsafe extern "C" fn id2handle(id: u32) -> u32 {
    ...
    u8 fshift)
    {
    let mut h: c_uint = ntohl(key & sel.hmask) >> fshift;
    return h;
    }
    ...
    let mut off: c_uint = skb_network_offset(skb);
    struct tc_u_knode *n;
    let mut sdepth: c_int = 0;
    ...
    let mut toff: c_int = off + key.off + (off2 & key.offmask);
    __be32 *data, hdata;
...
// POP
    ...
    {
    unsigned int sel;
    struct tc_u_knode *n = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn gen_new_htid(tp_c: *mut tc_u_common, ptr: *mut tc_u_hnode) -> u32 {
    static u32 gen_new_htid(struct tc_u_common *tp_c, struct tc_u_hnode *ptr)
    {
    let mut id: c_int = idr_alloc_cyclic(&tp_c.handle_idr, ptr, 1, 0x7FF, GFP_KERNEL);
    if (id < 0)
    return 0;
    return id2handle(id);
    }
    static struct hlist_head *tc_u_common_hash;
pub const U32_HASH_SHIFT: c_int = 10;
...
#[no_mangle]
unsafe extern "C" fn u32_init(tp: *mut tcf_proto) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __u32_destroy_key(n: *mut tc_u_knode) {
...
#[no_mangle]
unsafe extern "C" fn u32_destroy_key(n: *mut tc_u_knode, free_pf: bool) {
...
//
#[no_mangle]
unsafe extern "C" fn u32_delete_key_work(work: *mut work_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn u32_delete_key_freepf_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn u32_delete_key(tp: *mut tcf_proto, key: *mut tc_u_knode) -> c_int {
    ...
    struct tcf_block *block = tp.chain.block;
    let mut cls_u32: tc_cls_u32_offload = {};
    let mut skip_sw: bool = tc_skip_sw(flags);
    let mut offloaded: bool = false;
    int err;
    ...
    if (err < 0) {
    u32_clear_hw_hnode(tp, h, core::ptr::null_mut());
    return err;
    } else if (err > 0) {
    offloaded = true;
    }
    ...
    &n.flags, &n.in_hw_count, true);
    if (err) {
    u32_remove_hw_knode(tp, n, core::ptr::null_mut());
    return err;
    }
    ...
    while ((n = rtnl_dereference(ht.ht[h])) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn gen_new_kid(ht: *mut tc_u_hnode, htid: u32, err: *mut c_int) -> u32 {
    static u32 gen_new_kid(struct tc_u_hnode *ht, u32 htid, int *err)
    {
    let mut index: u32 = htid | 0x800;
    let mut max: u32 = htid | 0xFFF;
// err = 0;
    if (idr_alloc_u32(&ht.handle_idr, core::ptr::null_mut(), &index, max, GFP_KERNEL)) {
    index = htid + 1;
...
#[no_mangle]
unsafe extern "C" fn u32_kid_extack(err: c_int, extack: *mut netlink_ext_ack) -> c_int {
    ...
    if (tb[TCA_U32_LINK]) {
    let mut handle: u32 = nla_get_u32(tb[TCA_U32_LINK]);
    struct tc_u_hnode *ht_down = core::ptr::null_mut(), *ht_old;
...
// bump reference count as long as we hold pointer to structure
    ...
    let mut divisor: c_uint = nla_get_u32(tb[TCA_U32_DIVISOR]);
...
//
    ...
    err = idr_alloc_u32(&ht.handle_idr, core::ptr::null_mut(), &handle,
    handle, GFP_KERNEL);
    if (err)
    return err;
    }
...
//
    ...
    if (tb[TCA_U32_SEL] == core::ptr::null_mut()) {
...
//
    ...
    let mut divisor: u32 = ht.divisor + 1;
    ...
    if (ht_up) {
    let mut htid: u32 = n.handle & 0xFFFFF000;
    ...
    let mut cnt: __u32 = *per_cpu_ptr(n.pcpu_success, cpum);
    mark.success += cnt;
    }
...
#[no_mangle]
unsafe extern "C" fn init_u32() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_u32() -> void __exit {
