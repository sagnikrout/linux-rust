...
//
    ...
    {
    struct tcf_ematch_ops *e = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn tcf_em_register(ops: *mut tcf_ematch_ops) -> c_int {
    int tcf_em_register(struct tcf_ematch_ops *ops)
    {
    let mut err: c_int = -EEXIST;
    struct tcf_ematch_ops *e;
    if (ops.match == core::ptr::null_mut())
    return -EINVAL;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcf_em_unregister(ops: *mut tcf_ematch_ops) {
    ...
    let mut data_len: c_int = nla_len(nla) - sizeof(*em_hdr);
...
//
    ...
    em_policy, core::ptr::null_mut());
    if (err < 0)
    goto errout;
    err = -EINVAL;
    rt_hdr = tb[TCA_EMATCH_TREE_HDR];
    rt_list = tb[TCA_EMATCH_TREE_LIST];
    if (rt_hdr == core::ptr::null_mut() || rt_list == core::ptr::null_mut())
    goto errout;
    ...
    if (tree.matches == core::ptr::null_mut())
    goto errout;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcf_em_tree_destroy(tree: *mut tcf_ematch_tree) {
    ...
    em.ops.destroy(em);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !tcf_em_is_simple(em)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcf_em_tree_dump(skb: *mut sk_buff, tree: *mut tcf_ematch_tree, tlv: c_int) -> c_int {
    ...
    struct tcf_pkt_info *info)
    {
    let mut r: c_int = em.ops.match(skb, em, info);
...
// Do not use this function directly, use tcf_em_tree_match instead
    ...
    struct tcf_pkt_info *info)
    {
    let mut stackp: c_int = 0, match_idx = 0, res = 0;
