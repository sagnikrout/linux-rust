...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct netns_pfkey {
// List of all pfkey sockets.
    pub table: hlist_head,
    pub socks_nr: core::sync::atomic::AtomicI32,
}

    static DEFINE_MUTEX(pfkey_mutex);
pub const DUMMY_MARK: c_int = 0;
    let mut dummy_mark: static struct xfrm_mark = {0, 0};
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pfkey_sock {
// struct sock must be the first member of struct pfkey_sock
    ...
    pub msg_version: u8,
    pub msg_portid: u32,
    pub sk): *mut *mut int (dump)(struct pfkey_sock,
    pub sk): *mut *mut void (done)(struct pfkey_sock,
    union {
    pub policy: xfrm_policy_walk,
    pub state: xfrm_state_walk,
    pub u: },
    pub skb: *mut sk_buff,
    pub dump: },
    pub dump_lock: mutex,
...
#[no_mangle]
unsafe extern "C" fn pfkey_can_dump(sk: *const sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_terminate_dump(pfk: *mut pfkey_sock) {
    static void pfkey_terminate_dump(struct pfkey_sock *pfk)
    {
    if (pfk.dump.dump) {
    if (pfk.dump.skb) {
    kfree_skb(pfk.dump.skb);
    pfk.dump.skb = core::ptr::null_mut();
    }
    pfk.dump.done(pfk);
    pfk.dump.dump = core::ptr::null_mut();
    pfk.dump.done = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn pfkey_sock_destruct(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_insert(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_remove(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_release(sock: *mut socket) -> c_int {
...
pub const BROADCAST_ALL: c_int = 0;
pub const BROADCAST_ONE: c_int = 1;
pub const BROADCAST_REGISTERED: c_int = 2;
pub const BROADCAST_PROMISC_ONLY: c_int = 4;
...
// registered KM
    ...
    rcu_read_unlock();
    if (one_sk != core::ptr::null_mut())
    err = pfkey_broadcast_one(skb, allocation, one_sk);
    kfree_skb(skb);
    return err;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_do_dump(pfk: *mut pfkey_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_error(orig: *const sadb_msg, err: c_int, sk: *mut sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn verify_address_len(p: *const c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sadb_key_len(key: *const sadb_key) -> c_int {
    static inline int sadb_key_len(const struct sadb_key *key)
    {
    let mut key_bytes: c_int = DIV_ROUND_UP(key.sadb_key_bits, 8);
    return DIV_ROUND_UP(sizeof(struct sadb_key) + key_bytes,
    sizeof(uint64_t));
    }
#[no_mangle]
unsafe extern "C" fn verify_key_len(p: *const c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pfkey_sec_ctx_len(sec_ctx: *const sadb_x_sec_ctx) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn verify_sec_ctx_len(p: *const c_void) -> c_int {
    static inline int verify_sec_ctx_len(const void *p)
    {
    const struct sadb_x_sec_ctx *sec_ctx = p;
    let mut len: c_int = sec_ctx.sadb_x_ctx_len;
    if (len > PAGE_SIZE)
    return -EINVAL;
    ...
    gfp_t gfp)
    {
    struct xfrm_user_sec_ctx *uctx = core::ptr::null_mut();
    let mut ctx_size: c_int = sec_ctx.sadb_x_ctx_len;
...
#[no_mangle]
unsafe extern "C" fn parse_exthdrs(skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *mut c_void) -> c_int {
    ...
    let mut min: c_int = (int) sadb_ext_min_len[ext_type];
    if (ext_len < min)
    return -EINVAL;
    if (ext_hdrs[ext_type-1] != core::ptr::null_mut())
    return -EINVAL;
    switch (ext_type) {
...
//
#[no_mangle]
unsafe extern "C" fn pfkey_proto_to_xfrm(proto: u8) -> u8 {
    static uint8_t pfkey_proto_to_xfrm(uint8_t proto)
    {
    let mut proto: return = = IPSEC_PROTO_ANY ? 0 : proto;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_proto_from_xfrm(proto: u8) -> u8 {
    static uint8_t pfkey_proto_from_xfrm(uint8_t proto)
    {
    return proto ? proto : IPSEC_PROTO_ANY;
    }
#[no_mangle]
pub unsafe extern "C" fn pfkey_sockaddr_len(family: sa_family_t) -> c_int {
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn pfkey_sockaddr_extract(sa: *const sockaddr, xaddr: *mut xfrm_address_t) -> c_int {
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn pfkey_sadb_addr2xfrm_addr(addr: *const sadb_address, xaddr: *mut xfrm_address_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pfkey_mode_from_xfrm(mode: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pfkey_mode_to_xfrm(mode: c_int) -> c_int {
    ...
    let mut sockaddr_len: c_int = pfkey_sockaddr_len(family);
    let mut sockaddr_size: c_int = pfkey_sockaddr_size(family);
    ...
    let mut ctx_size: c_int = 0;
    int size;
    let mut auth_key_size: c_int = 0;
    let mut encrypt_key_size: c_int = 0;
    int sockaddr_size;
    struct xfrm_encap_tmpl *natt = core::ptr::null_mut();
    int mode;
...
// x->algo.flags = sa->sadb_sa_flags;
    ...
    let mut mode: c_int = pfkey_mode_to_xfrm(sa2.sadb_x_sa2_mode);
    if (mode < 0) {
    err = -EINVAL;
    goto out;
    }
...
// Nobody uses this, but we try.
    ...
    err = xfrm_init_state(x, core::ptr::null_mut());
    if (err)
    goto out;
...
#[no_mangle]
unsafe extern "C" fn pfkey_reserved(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    static int pfkey_reserved(struct sock *sk, struct sk_buff *skb, const struct sadb_msg *hdr, void * const *ext_hdrs)
    {
    return -EOPNOTSUPP;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_getspi(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    ...
    u8 proto;
    unsigned short family;
    xfrm_address_t *xsaddr = core::ptr::null_mut(), *xdaddr = core::ptr::null_mut();
    ...
    if (proto == 0)
    return -EINVAL;
    if ((sa2 = ext_hdrs[SADB_X_EXT_SA2-1]) != core::ptr::null_mut()) {
    ...
    err = verify_spi_info(x.id.proto, min_spi, max_spi, core::ptr::null_mut());
    if (err) {
    xfrm_state_put(x);
    return err;
    }
    err = xfrm_alloc_spi(x, min_spi, max_spi, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn pfkey_acquire(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn event2poltype(event: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn event2keytype(event: c_int) -> c_int {
...
// ADD/UPD/DEL
#[no_mangle]
unsafe extern "C" fn key_notify_sa(x: *mut xfrm_state, c: *const km_event) -> c_int {
    ...
    pfkey_broadcast(skb, GFP_ATOMIC, BROADCAST_ALL, core::ptr::null_mut(), xs_net(x));
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_add(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_delete(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_get(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_register(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn unicast_flush_resp(sk: *mut sock, ihdr: *const sadb_msg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn key_notify_sa_flush(c: *const km_event) -> c_int {
    ...
    hdr.sadb_msg_len = (sizeof(struct sadb_msg) / sizeof(uint64_t));
    hdr.sadb_msg_reserved = 0;
    pfkey_broadcast(skb, GFP_ATOMIC, BROADCAST_ALL, core::ptr::null_mut(), c.net);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_flush(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    ...
    c.net = net;
    km_state_notify(core::ptr::null_mut(), &c);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn dump_sa(x: *mut xfrm_state, count: c_int, ptr: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_dump_sa(pfk: *mut pfkey_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_dump_sa_done(pfk: *mut pfkey_sock) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_dump(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    static int pfkey_dump(struct sock *sk, struct sk_buff *skb, const struct sadb_msg *hdr, void * const *ext_hdrs)
    {
    u8 proto;
    struct xfrm_address_filter *filter = core::ptr::null_mut();
    struct pfkey_sock *pfk = pfkey_sk(sk);
    mutex_lock(&pfk.dump_lock);
    if (pfk.dump.dump != core::ptr::null_mut()) {
    mutex_unlock(&pfk.dump_lock);
    return -EBUSY;
    }
    proto = pfkey_satype2proto(hdr.sadb_msg_satype);
...
#[no_mangle]
unsafe extern "C" fn pfkey_promisc(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    static int pfkey_promisc(struct sock *sk, struct sk_buff *skb, const struct sadb_msg *hdr, void * const *ext_hdrs)
    {
    struct pfkey_sock *pfk = pfkey_sk(sk);
    let mut satype: c_int = hdr.sadb_msg_satype;
    let mut reset_errno: bool = false;
    ...
    pfkey_broadcast(skb, GFP_KERNEL, BROADCAST_ALL, core::ptr::null_mut(), sock_net(sk));
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn check_reqid(xp: *mut xfrm_policy, dir: c_int, count: c_int, ptr: *mut c_void) -> c_int {
    static int check_reqid(struct xfrm_policy *xp, int dir, int count, void *ptr)
    {
    int i;
    let mut reqid: u32 = *(u32*)ptr;
...
#[no_mangle]
unsafe extern "C" fn gen_reqid(net: *mut net) -> u32 {
    static u32 gen_reqid(struct net *net)
    {
    struct xfrm_policy_walk walk;
    u32 start;
    int rc;
    let mut reqid: static u32 = IPSEC_MANUAL_REQID_MAX;
    start = reqid;
    do {
    ++reqid;
    if (reqid == 0)
...
// No way to set this via kame pfkey
    ...
    {
    int err;
    let mut len: c_int = pol.sadb_x_policy_len*8 - sizeof(struct sadb_x_policy);
...
#[no_mangle]
pub unsafe extern "C" fn pfkey_xfrm_policy2sec_ctx_size(xp: *const xfrm_policy) -> c_int {
    static inline int pfkey_xfrm_policy2sec_ctx_size(const struct xfrm_policy *xp)
    {
    struct xfrm_sec_ctx *xfrm_ctx = xp.security;
    if (xfrm_ctx) {
    let mut len: c_int = sizeof(struct sadb_x_sec_ctx);
...
#[no_mangle]
unsafe extern "C" fn pfkey_xfrm_policy2msg_size(xp: *const xfrm_policy) -> c_int {
    static int pfkey_xfrm_policy2msg_size(const struct xfrm_policy *xp)
    {
    const struct xfrm_tmpl *t;
    let mut sockaddr_size: c_int = pfkey_sockaddr_size(xp.family);
    let mut socklen: c_int = 0;
    int i;
    for (i=0; i<xp.xfrm_nr; i++) {
    t = xp.xfrm_vec + i;
...
#[no_mangle]
unsafe extern "C" fn pfkey_xfrm_policy2msg_prep(xp: *const xfrm_policy) -> *mut sk_buff {
...
#[no_mangle]
unsafe extern "C" fn pfkey_xfrm_policy2msg(skb: *mut sk_buff, xp: *const xfrm_policy, dir: c_int) -> c_int {
    ...
    int size;
    let mut sockaddr_size: c_int = pfkey_sockaddr_size(xp.family);
    let mut socklen: c_int = pfkey_sockaddr_len(xp.family);
    size = pfkey_xfrm_policy2msg_size(xp);
// call should fill header later
    hdr = skb_put(skb, sizeof(struct sadb_msg));
    memset(hdr, 0, size);	/* XXX do we need this ? */
...
// security context
    ...
    let mut ctx_size: c_int = pfkey_xfrm_policy2sec_ctx_size(xp);
...
#[no_mangle]
unsafe extern "C" fn key_notify_policy(xp: *mut xfrm_policy, dir: c_int, c: *const km_event) -> c_int {
    ...
    pfkey_broadcast(out_skb, GFP_ATOMIC, BROADCAST_ALL, core::ptr::null_mut(), xp_net(xp));
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_spdadd(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
...
//
    ...
    if ((lifetime = ext_hdrs[SADB_EXT_LIFETIME_HARD-1]) != core::ptr::null_mut()) {
    xp.lft.hard_packet_limit = _KEY2X(lifetime.sadb_lifetime_allocations);
    xp.lft.hard_byte_limit = _KEY2X(lifetime.sadb_lifetime_bytes);
    xp.lft.hard_add_expires_seconds = lifetime.sadb_lifetime_addtime;
    xp.lft.hard_use_expires_seconds = lifetime.sadb_lifetime_usetime;
    }
    if ((lifetime = ext_hdrs[SADB_EXT_LIFETIME_SOFT-1]) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_spddelete(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    ...
    struct km_event c;
    struct sadb_x_sec_ctx *sec_ctx;
    struct xfrm_sec_ctx *pol_ctx = core::ptr::null_mut();
    if (!present_and_same_family(ext_hdrs[SADB_EXT_ADDRESS_SRC-1],
    ext_hdrs[SADB_EXT_ADDRESS_DST-1]) ||
    !ext_hdrs[SADB_X_EXT_POLICY-1])
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn key_pol_get_resp(sk: *mut sock, xp: *mut xfrm_policy, hdr: *const sadb_msg, dir: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_sockaddr_pair_size(family: sa_family_t) -> c_int {
...
// extract ipsecrequests
    ...
    kma ? &k : core::ptr::null_mut(), net, core::ptr::null_mut(), 0, core::ptr::null_mut(), core::ptr::null_mut());
    out:
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn pfkey_spdget(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    static int pfkey_spdget(struct sock *sk, struct sk_buff *skb, const struct sadb_msg *hdr, void * const *ext_hdrs)
    {
    struct net *net = sock_net(sk);
    unsigned int dir;
    let mut err: c_int = 0, delete;
    struct sadb_x_policy *pol;
    struct xfrm_policy *xp;
    struct km_event c;
    if ((pol = ext_hdrs[SADB_X_EXT_POLICY-1]) == core::ptr::null_mut())
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn dump_sp(xp: *mut xfrm_policy, dir: c_int, count: c_int, ptr: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_dump_sp(pfk: *mut pfkey_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_dump_sp_done(pfk: *mut pfkey_sock) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_spddump(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn key_notify_policy_flush(c: *const km_event) -> c_int {
    ...
    hdr.sadb_msg_len = (sizeof(struct sadb_msg) / sizeof(uint64_t));
    hdr.sadb_msg_reserved = 0;
    pfkey_broadcast(skb_out, GFP_ATOMIC, BROADCAST_ALL, core::ptr::null_mut(), c.net);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_spdflush(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg, ext_hdrs: *const *const c_void) -> c_int {
    ...
    c.seq = hdr.sadb_msg_seq;
    c.net = net;
    km_policy_notify(core::ptr::null_mut(), 0, &c);
    return 0;
    }
    ...
    [SADB_EXPIRE]		= core::ptr::null_mut(),
    [SADB_FLUSH]		= pfkey_flush,
    [SADB_DUMP]		= pfkey_dump,
    [SADB_X_PROMISC]	= pfkey_promisc,
    [SADB_X_PCHANGE]	= core::ptr::null_mut(),
    [SADB_X_SPDUPDATE]	= pfkey_spdadd,
    [SADB_X_SPDADD]		= pfkey_spdadd,
    [SADB_X_SPDDELETE]	= pfkey_spddelete,
    [SADB_X_SPDGET]		= pfkey_spdget,
    [SADB_X_SPDACQUIRE]	= core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn pfkey_process(sk: *mut sock, skb: *mut sk_buff, hdr: *const sadb_msg) -> c_int {
...
//
    ...
    BROADCAST_PROMISC_ONLY, core::ptr::null_mut(), sock_net(sk));
    ...
    {
    struct sadb_msg *hdr = core::ptr::null_mut();
...
// errp = 0;
    ...
    {
    let mut id: c_uint = d.desc.sadb_alg_id;
...
#[no_mangle]
unsafe extern "C" fn count_ah_combs(t: *const xfrm_tmpl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn count_esp_combs(t: *const xfrm_tmpl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dump_ah_combs(skb: *mut sk_buff, t: *const xfrm_tmpl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dump_esp_combs(skb: *mut sk_buff, t: *const xfrm_tmpl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn key_notify_policy_expire(xp: *mut xfrm_policy, c: *const km_event) -> c_int {
    static int key_notify_policy_expire(struct xfrm_policy *xp, const struct km_event *c)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn key_notify_sa_expire(x: *mut xfrm_state, c: *const km_event) -> c_int {
    ...
    pfkey_broadcast(out_skb, GFP_ATOMIC, BROADCAST_REGISTERED, core::ptr::null_mut(),
    xs_net(x));
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn pfkey_send_notify(x: *mut xfrm_state, c: *const km_event) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_send_policy_notify(xp: *mut xfrm_policy, dir: c_int, c: *const km_event) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_acqseq() -> u32 {
...
#[no_mangle]
unsafe extern "C" fn pfkey_send_acquire(x: *mut xfrm_state, t: *mut xfrm_tmpl, xp: *mut xfrm_policy) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_ESP: x->id.proto ==) -> else {
...
// security context
    ...
    return pfkey_broadcast(skb, GFP_ATOMIC, BROADCAST_REGISTERED, core::ptr::null_mut(),
    xs_net(x));
    }
...
#[no_mangle]
unsafe extern "C" fn pfkey_send_new_mapping(x: *mut xfrm_state, ipaddr: *mut xfrm_address_t, sport: __be16) -> c_int {
    ...
    int sockaddr_size;
    int size;
    let mut satype: __u8 = (x.id.proto == IPPROTO_ESP ? SADB_SATYPE_ESP : 0);
    struct xfrm_encap_tmpl *natt = core::ptr::null_mut();
    sockaddr_size = pfkey_sockaddr_size(x.props.family);
    if (!sockaddr_size)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn set_sadb_kmaddress(skb: *mut sk_buff, k: *const xfrm_kmaddress) -> c_int {
    static int set_sadb_kmaddress(struct sk_buff *skb, const struct xfrm_kmaddress *k)
    {
    struct sadb_x_kmaddress *kma;
    u8 *sa;
    let mut family: c_int = k.family;
    let mut socklen: c_int = pfkey_sockaddr_len(family);
    int size_req;
...
// Addresses to be used by KM for negotiation, if ext is available
    if (k != core::ptr::null_mut() && (set_sadb_kmaddress(skb, k) < 0))
    goto err;
// selector src
...
// policy information
    ...
    for (i = 0, mp = m; i < num_bundles; i++, mp++) {
// old ipsecrequest
    let mut mode: c_int = pfkey_mode_from_xfrm(mp.mode);
    if (mode < 0)
    goto err;
...
// broadcast migrate message to sockets
    pfkey_broadcast(skb, GFP_ATOMIC, BROADCAST_ALL, core::ptr::null_mut(), net);
    return 0;
    err:
    kfree_skb(skb);
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn pfkey_sendmsg(sock: *mut socket, msg: *mut msghdr, len: usize) -> c_int {
    ...
    if (pfk.dump.dump != core::ptr::null_mut() &&
#[no_mangle]
pub unsafe extern "C" fn atomic_read(sk->sk_rcvbuf: &sk->sk_rmem_alloc) <=) -> *mut 3 {
...
#[no_mangle]
unsafe extern "C" fn pfkey_seq_show(f: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pfkey_seq_stop(f: *mut seq_file, v: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_init_proc(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn pfkey_exit_proc(net: *mut net) -> void __net_exit {
...
#[no_mangle]
pub unsafe extern "C" fn pfkey_init_proc(net: *mut net) -> c_int {
    static inline int pfkey_init_proc(struct net *net)
    {
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn pfkey_exit_proc(net: *mut net) {
...
#[no_mangle]
unsafe extern "C" fn pfkey_net_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn pfkey_net_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn ipsec_pfkey_exit() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn ipsec_pfkey_init() -> int __init {
    static int __init ipsec_pfkey_init(void)
    {
    let mut err: c_int = proto_register(&key_proto, 0);
