...
pub const IPTFS_SUBTYPE_BASIC: c_int = 0;
pub const IPTFS_SUBTYPE_CC: c_int = 1;
// -----------------------------------------------
...
// Default 1s.
//
pub const IPTFS_DEFAULT_DROP_TIME_USECS: c_int = 1000000;
//
...
//
pub const IPTFS_DEFAULT_REORDER_WINDOW: c_int = 3;
// ------------------------------------------------
...
// Default 0.
//
pub const IPTFS_DEFAULT_INIT_DELAY_USECS: c_int = 0;
//
...
//
pub const XFRM_IPTFS_MIN_L3HEADROOM: c_int = 128;

// Min to try to share outer iptfs skb data vs copying into new skb
pub const IPTFS_PKT_SHARE_MIN: c_int = 129;
pub const NSECS_IN_USEC: c_int = 1000;
...
pub struct xfrm_iptfs_config {
    pub /: *mut *mut u32 pkt_size; / outer_packet_size or 0,
    pub /: *mut *mut u32 max_queue_size; / octets,
...
pub struct xfrm_iptfs_data {
    pub cfg: xfrm_iptfs_config,
// Ingress User Input
    pub /: *mut *mut *mut xfrm_state x; / owning state,
    pub /: *mut *mut sk_buff_head queue; / output queue,
    pub /: *mut *mut u32 queue_size; / octets,
    pub /: *mut *mut u32 ecn_queue_size; / octets above which ECN mark,
    pub /: *mut *mut u64 init_delay_ns; / nanoseconds,
    pub /: *mut *mut hrtimer iptfs_timer; / output timer,
    pub /: *mut *mut time64_t iptfs_settime; / time timer was set,
    pub /: *mut *mut u32 payload_mtu; / max payload size,
// Tunnel input reordering
    pub /: *mut *mut bool w_seq_set; / true after first seq received,
    pub /: *mut *mut u64 w_wantseq; / expected next sequence,
    pub /: *mut *mut *mut skb_wseq w_saved; / the saved buf array,
    pub /: *mut *mut u32 w_savedlen; / the saved len (not size),
    pub drop_lock: spinlock_t,
    pub drop_timer: hrtimer,
    pub drop_time_ns: u64,
// Tunnel input reassembly
    pub /: *mut *mut *mut sk_buff ra_newskb; / new pkt being reassembled,
    pub /: *mut *mut u64 ra_wantseq; / expected next sequence,
    pub /: *mut *mut u8 ra_runt[6]; / last pkt bytes from last skb,
    pub /: *mut *mut u8 ra_runtlen; / count of ra_runt,
}
...
#[no_mangle]
unsafe extern "C" fn __trace_ip_proto(iph: *mut iphdr) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn __trace_ip_proto_seq(iph: *mut iphdr) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn __esp_seq(skb: *mut sk_buff) -> u64 {
    static u64 __esp_seq(struct sk_buff *skb)
    {
    let mut seq: u64 = ntohl(XFRM_SKB_CB(skb).seq.input.low);
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_skb_head_to_frag(skb: *const sk_buff, frag: *mut skb_frag_t) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iptfs_skb_frag_walk {
    pub fragi: u32,
    pub past: u32,
    pub total: u32,
    pub nr_frags: u32,
    pub initial_offset: u32,
    pub 1]: skb_frag_t frags[MAX_SKB_FRAGS +,
    pub pp_recycle: bool,
}
...
//
#[no_mangle]
unsafe extern "C" fn __iptfs_iphlen(data: *mut u8) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn __iptfs_iplen(data: *mut u8) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_complete_inner_skb(x: *mut xfrm_state, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn __iptfs_reassem_done(xtfs: *mut xfrm_iptfs_data, free: bool) {
...
// We don't care if it works locking takes care of things
    ...
    xtfs.ra_newskb = core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_reassem_abort(xtfs: *mut xfrm_iptfs_data) {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_reassem_done(xtfs: *mut xfrm_iptfs_data) {
...
//
    ...
    struct iptfs_skb_frag_walk *fragwalk = core::ptr::null_mut();
    struct sk_buff *newskb = xtfs.ra_newskb;
    let mut remaining: u32 = skb.len - data;
    let mut runtlen: u32 = xtfs.ra_runtlen;
...
// skip past fragment, maybe to end
    ...
    u32 capturelen;
    u64 seq;
    let mut first_skb_partial: bool = false;
    xtfs = x.mode_data;
    net = xs_net(x);
    skb = skbseq.root_skb;
    first_skb = core::ptr::null_mut();
    defer = core::ptr::null_mut();
    seq = __esp_seq(skb);
// Save the old mac header if set
    old_mac = skb_mac_header_was_set(skb) ? skb_mac_header(skb) : core::ptr::null_mut();
// New packets
    tail = skb.len;
    while (data < tail) {
    let mut protocol: __be16 = 0;
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_input_ordered(x: *mut xfrm_state, skb: *mut sk_buff) {
...
// -------------------------------
#[no_mangle]
unsafe extern "C" fn __vec_shift(xtfs: *mut xfrm_iptfs_data, shift: u32) {
    static void __vec_shift(struct xfrm_iptfs_data *xtfs, u32 shift)
    {
    let mut savedlen: u32 = xtfs.w_savedlen;
    if (shift > savedlen)
    shift = savedlen;
    if (shift != savedlen)
...
#[no_mangle]
unsafe extern "C" fn __reorder_drop(xtfs: *mut xfrm_iptfs_data, list: *mut list_head) -> u32 {
    static u32 __reorder_drop(struct xfrm_iptfs_data *xtfs, struct list_head *list)
    {
    struct skb_wseq *s, *se;
    let mut savedlen: u32 = xtfs.w_savedlen;
    let mut now: time64_t = ktime_get_raw_fast_ns();
    let mut count: u32 = 0;
    let mut scount: u32 = 0;
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_set_window_drop_times(xtfs: *mut xfrm_iptfs_data, index: c_int) {
...
// If we walked all the way back, schedule the drop timer if needed
    ...
    struct list_head *freelist)
    {
    let mut inseq: u64 = __esp_seq(inskb);
    let mut wantseq: u64 = xtfs.w_wantseq;
    let mut distance: u64 = inseq - wantseq;
    let mut savedlen: u32 = xtfs.w_savedlen;
    let mut index: u32 = distance - 1;
...
// a dup of a future
    ...
    struct list_head *list)
    {
    let mut nslots: u32 = xtfs.cfg.reorder_win_size + 1;
    let mut inseq: u64 = __esp_seq(inskb);
    let mut savedlen: u32 = xtfs.w_savedlen;
    let mut wantseq: u64 = xtfs.w_wantseq;
...
#[no_mangle]
pub unsafe extern "C" fn if(wantseq: inseq <) -> else {
    else if (inseq < wantseq)
    __reorder_past(xtfs, inskb, freelist);
#[no_mangle]
pub unsafe extern "C" fn if(nslots: (inseq - wantseq) <) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_drop_timer(me: *mut hrtimer) -> enum hrtimer_restart {
    static enum hrtimer_restart iptfs_drop_timer(struct hrtimer *me)
    {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_input(x: *mut xfrm_state, skb: *mut sk_buff) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_enqueue(xtfs: *mut xfrm_iptfs_data, skb: *mut sk_buff) -> bool {
    static bool iptfs_enqueue(struct xfrm_iptfs_data *xtfs, struct sk_buff *skb)
    {
    let mut newsz: u64 = xtfs.queue_size + skb.len;
    struct iphdr *iph;
...
pub unsafe extern "C" fn if(6: iph->version ==) -> else {
    ...
    let mut payload_mtu: u32 = xtfs.payload_mtu;
    let mut pmtu: u32 = __iptfs_get_inner_mtu(x, xdst.child_mtu_cached);
    if (payload_mtu && payload_mtu < pmtu)
    pmtu = payload_mtu;
    return pmtu;
    }
#[no_mangle]
unsafe extern "C" fn iptfs_is_too_big(sk: *mut sock, skb: *mut sk_buff, pmtu: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(4: ip_hdr(skb)->version ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_output_collect(net: *mut net, sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
// --------------------------
#[no_mangle]
unsafe extern "C" fn iptfs_output_prepare_skb(skb: *mut sk_buff, blkoff: u32) {
    static void iptfs_output_prepare_skb(struct sk_buff *skb, u32 blkoff)
    {
    struct ip_iptfs_hdr *h;
    let mut hsz: usize = sizeof(*h);
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_copy_create_frags(skbp: *mut sk_buff, xtfs: *mut xfrm_iptfs_data, mtu: u32) -> c_int {
    ...
    u32 copy_len, offset;
    let mut to_copy: u32 = skb.len - mtu;
...
// Send all but last fragment to allow agg. append
    trace_iptfs_first_fragmenting(nskb, mtu, to_copy, core::ptr::null_mut());
...
// skbp = nskb;
    if (nskb)
    trace_iptfs_first_final_fragment(nskb, mtu, blkoff, core::ptr::null_mut());
...
//
    ...
    skb_list_del_init(skb);
    if (!err)
    err = xfrm_output(core::ptr::null_mut(), skb);
    else
    kfree_skb(skb);
    }
    if (err)
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_first_skb(skbp: *mut sk_buff, xtfs: *mut xfrm_iptfs_data, mtu: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iptfs_consume_frags(to: *mut sk_buff, from: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn iptfs_output_queued(x: *mut xfrm_state, list: *mut sk_buff_head) {
...
//
    ...
    let mut mtu: u32 = iptfs_get_cur_pmtu(x, xtfs, skb);
    let mut share_ok: bool = true;
    int remaining;
...
// nextp = skb2;
    ...
    xfrm_output(core::ptr::null_mut(), skb);
    }
    }
#[no_mangle]
unsafe extern "C" fn iptfs_delay_timer(me: *mut hrtimer) -> enum hrtimer_restart {
    static enum hrtimer_restart iptfs_delay_timer(struct hrtimer *me)
    {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_encap_add_ipv4(x: *mut xfrm_state, skb: *mut sk_buff) -> c_int {
...
//
    ...
    ip_select_ident(dev_net(dst.dev), skb, core::ptr::null_mut());
    return 0;
    }
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_encap_add_ipv6(x: *mut xfrm_state, skb: *mut sk_buff) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_prepare_output(x: *mut xfrm_state, skb: *mut sk_buff) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __iptfs_get_inner_mtu(x: *mut xfrm_state, outer_mtu: c_int) -> u32 {
    static u32 __iptfs_get_inner_mtu(struct xfrm_state *x, int outer_mtu)
    {
...
//
#[no_mangle]
unsafe extern "C" fn iptfs_get_inner_mtu(x: *mut xfrm_state, outer_mtu: c_int) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn iptfs_sa_len(x: *const xfrm_state) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn iptfs_copy_to_user(x: *mut xfrm_state, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iptfs_clone_state(x: *mut xfrm_state, orig: *mut xfrm_state) -> c_int {
    static int iptfs_clone_state(struct xfrm_state *x, struct xfrm_state *orig)
    {
    struct skb_wseq *w_saved = core::ptr::null_mut();
    struct xfrm_iptfs_data *xtfs;
...
#[no_mangle]
unsafe extern "C" fn iptfs_init_state(x: *mut xfrm_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iptfs_destroy_state(x: *mut xfrm_state) {
...
#[no_mangle]
unsafe extern "C" fn xfrm_iptfs_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn xfrm_iptfs_fini() -> void __exit {
