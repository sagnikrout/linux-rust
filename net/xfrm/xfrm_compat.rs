...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_lifetime_cfg {
    pub hard_byte_limit: compat_u64 soft_byte_limit,,
    pub hard_packet_limit: compat_u64 soft_packet_limit,,
    pub hard_add_expires_seconds: compat_u64 soft_add_expires_seconds,,
    pub hard_use_expires_seconds: compat_u64 soft_use_expires_seconds,,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_lifetime_cur {
    pub use_time: compat_u64 bytes, packets, add_time,,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_userpolicy_info {
    pub sel: xfrm_selector,
    pub lft: compat_xfrm_lifetime_cfg,
    pub curlft: compat_xfrm_lifetime_cur,
    pub index: __u32 priority,,
    pub share: u8 dir, action, flags,,
// 4 bytes additional padding on 64bit
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_usersa_info {
    pub sel: xfrm_selector,
    pub id: xfrm_id,
    ...
    pub reqid: __u32 seq,,
    pub family: u16,
    pub flags: u8 mode, replay_window,,
// 4 bytes additional padding on 64bit
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_user_acquire {
    pub id: xfrm_id,
    pub saddr: xfrm_address_t,
    pub sel: xfrm_selector,
    pub policy: compat_xfrm_userpolicy_info,
// 4 bytes additional padding on 64bit
    pub seq: __u32 aalgos, ealgos, calgos,,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_userspi_info {
    pub info: compat_xfrm_usersa_info,
// 4 bytes additional padding on 64bit
    pub max: __u32 min,,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_user_expire {
    pub state: compat_xfrm_usersa_info,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xfrm_user_polexpire {
    pub pol: compat_xfrm_userpolicy_info,
// 8 bytes additional padding on 64bit
    pub hard: u8,
}
    ...
    {
    let mut payload: c_int = compat_msg_min[type];
    let mut src_len: c_int = xfrm_msg_min[type];
...
#[no_mangle]
unsafe extern "C" fn xfrm_nla_cpy(dst: *mut sk_buff, src: *const nlattr, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xfrm_xlate64_attr(dst: *mut sk_buff, src: *const nlattr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xfrm_xlate64(dst: *mut sk_buff, nlh_src: *const nlmsghdr) -> c_int {
    static int xfrm_xlate64(struct sk_buff *dst, const struct nlmsghdr *nlh_src)
    {
    let mut type: u16 = nlh_src.nlmsg_type - XFRM_MSG_BASE;
...
#[no_mangle]
unsafe extern "C" fn xfrm_alloc_compat(skb: *mut sk_buff, nlh_src: *const nlmsghdr) -> c_int {
    ...
    if (skb_shinfo(skb).frag_list == core::ptr::null_mut()) {
...
// Calculates len of translated 64-bit message.
    ...
    int maxtype)
    {
    let mut len: usize = nlmsg_len(src);
...
// netlink_rcv_skb() checks if a message has full (struct nlmsghdr)
    let mut type: u16 = h32.nlmsg_type - XFRM_MSG_BASE;
    struct nlattr *attrs[XFRMA_MAX+1];
    struct nlmsghdr *h64;
    size_t len;
    int err;
...
#[no_mangle]
unsafe extern "C" fn xfrm_user_policy_compat(pdata32: *mut u8, optlen: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xfrm_compat_init() -> int __init {
    static int __init xfrm_compat_init(void)
    {
    return xfrm_register_translator(&xfrm_translator);
    }
#[no_mangle]
unsafe extern "C" fn xfrm_compat_exit() -> void __exit {
