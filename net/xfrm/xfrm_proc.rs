...
#[no_mangle]
unsafe extern "C" fn xfrm_statistics_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
    static int xfrm_statistics_seq_show(struct seq_file *seq, void *v)
    {
    unsigned long buff[ARRAY_SIZE(xfrm_mib_list)];
    let mut cnt: c_int = ARRAY_SIZE(xfrm_mib_list);
...
#[no_mangle]
pub unsafe extern "C" fn xfrm_proc_init(net: *mut net) -> int __net_init {
    int __net_init xfrm_proc_init(struct net *net)
    {
    if (!proc_create_net_single("xfrm_stat", 0444, net.proc_net,
    xfrm_statistics_seq_show, core::ptr::null_mut()))
    return -ENOMEM;
    return 0;
    }
#[no_mangle]
