...
//! Source: net/wireless/wext-core.c
#![no_std]
...
//
    ...
    let mut standard_ioctl_num: static unsigned int = ARRAY_SIZE(standard_ioctl);
//
...
//
    ...
    let mut standard_event_num: static unsigned int = ARRAY_SIZE(standard_event);
...
// IW event code
#[no_mangle]
pub unsafe extern "C" fn wireless_nlevent_flush() {
    ...
    rtnl_notify(skb, net, 0, RTNLGRP_LINK, core::ptr::null_mut(),
    GFP_KERNEL);
    }
    up_read(&net_rwsem);
    }
...
#[no_mangle]
unsafe extern "C" fn wext_pernet_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn wext_pernet_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn wireless_nlevent_init() -> int __init {
    static int __init wireless_nlevent_init(void)
    {
    let mut err: c_int = register_pernet_subsys(&wext_pernet_ops);
    if (err)
    return err;
...
#[no_mangle]
unsafe extern "C" fn wireless_nlevent_process(work: *mut work_struct) {
...
//
    ...
    const char *		extra)
    {
    let mut descr: *const iw_ioctl_description = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn wireless_warn_cfg80211_wext() {
...
// IW handlers
    ...
    (dev.wireless_handlers.get_wireless_stats != core::ptr::null_mut()))
...
#[no_mangle]
unsafe extern "C" fn get_handler(dev: *mut net_device, cmd: c_uint) -> iw_handler {
    static iw_handler get_handler(struct net_device *dev, unsigned int cmd)
    {
// Don't "optimise" the following variable, it will crash
    unsigned int	index;		/* *MUST* be unsigned */
    const struct iw_handler_def *handlers = core::ptr::null_mut();
...
// Check need for ESSID compatibility for WE < 21
    ...
    essid_compat = 1;
#[no_mangle]
pub unsafe extern "C" fn if(0): IW_IS_SET(cmd) && (iwp->length !=) -> else {
...
// export the payload.
//
    wireless_send_event(dev, cmd, data, core::ptr::null_mut());
    else
...
//
#[no_mangle]
pub unsafe extern "C" fn call_commit_handler(dev: *mut net_device) -> c_int {
...
// Call the commit handler on the driver
    return dev.wireless_handlers.standard[0](dev, core::ptr::null_mut(),
...
// Make sure the device exist
    if ((dev = __dev_get_by_name(net, iwr.ifr_name)) == core::ptr::null_mut())
    return -ENODEV;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: private) -> else {
    else if (private)
...
//
#[no_mangle]
unsafe extern "C" fn wext_permission_check(cmd: c_uint) -> c_int {
...
// entry point from dev ioctl
    ...
    wext_ioctl_func private)
    {
    let mut ret: c_int = wext_permission_check(cmd);
    if (ret)
    return ret;
...
// No extra arguments. Trivial to handle
    ret = handler(dev, info, &(iwr.u), core::ptr::null_mut());
// Generate an event to notify listeners of the change
    if ((descr.flags & IW_DESCR_FLAG_EVENT) &&
    ((ret == 0) || (ret == -EIWCOMMIT)))
    wireless_send_event(dev, cmd, &(iwr.u), core::ptr::null_mut());
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn wext_handle_ioctl(net: *mut net, cmd: c_uint, arg: *mut void __user) -> c_int {
    int wext_handle_ioctl(struct net *net, unsigned int cmd, void __user *arg)
    {
    let mut info: iw_request_info = { .cmd = cmd, .flags = 0 };
    struct iwreq iwr;
    int ret;
...
// colon = 0;
    ...
    {
    let mut lcp_len: c_int = iwe_stream_lcp_len(info);
...
// Beware of alignment issues on 64 bits
    ...
    {
    let mut event_len: c_int = iwe_stream_point_len(info) + iwe.u.data.length;
    let mut point_len: c_int = iwe_stream_point_len(info);
    let mut lcp_len: c_int = iwe_stream_lcp_len(info);
// Check if it's possible
    if (likely((stream + event_len) < ends)) {
    iwe.len = event_len;
