...
#[no_mangle]
unsafe extern "C" fn cfg80211_valid_60g_freq(freq: u32) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cfg80211_per_bw_puncturing_values {
    pub len: u8,
    pub valid_values: *const u16,
}
...
#[no_mangle]
unsafe extern "C" fn cfg80211_edmg_chandef_valid(chandef: *const cfg80211_chan_def) -> bool {
    static bool cfg80211_edmg_chandef_valid(const struct cfg80211_chan_def *chandef)
    {
    let mut max_contiguous: c_int = 0;
    let mut num_of_enabled: c_int = 0;
    let mut contiguous: c_int = 0;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn nl80211_chan_width_to_mhz(chan_width: enum nl80211_chan_width) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cfg80211_chandef_valid(chandef: *const cfg80211_chan_def) -> bool {
...
// channel 14 is only for IEEE 802.11b
    ...
    u16 *punctured)
    {
    let mut pri_width: c_int = nl80211_chan_width_to_mhz(primary_chan_width);
    let mut width: c_int = cfg80211_chandef_get_width(c);
    let mut control: u32 = c.chan.center_freq;
    let mut center: u32 = c.center_freq1;
    let mut _punct: u16 = 0;
...
// punctured = c->punctured;
    ...
    let mut bits_to_drop: c_uint = width / 20 / 2;
    if (control > center) {
    center += width / 4;
...
// punctured &= (1 << bits_to_drop) - 1;
    ...
    {
    let mut tmp: cfg80211_chan_def = *chandef;
    ...
    const struct ieee80211_uhr_npca_info *npca)
    {
    let mut new_chandef: cfg80211_chan_def = *chandef;
    u32 width, npca_freq;
    u8 offs;
    if (chandef.npca_chan || chandef.npca_punctured)
    return -EINVAL;
...
//
    ...
    let mut punct_mask: u16 = ((1 << (bw_mhz / 40)) - 1) << punct_shift;
    let mut punctured: u16 = le16_to_cpu(dbe.dis_subch_bmap[0]);
...
// chandef = new_chandef;
    ...
    enum nl80211_chan_width primary_chan_width)
    {
    let mut punct_c1: u16 = 0, punct_c2 = 0;
...
// Get the same timestamp for all subchannels
    ...
    struct ieee80211_channel *other_chan = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn cfg80211_beaconing_iface_active(wdev: *mut wireless_dev) -> bool {
...
// check if the operating channels are valid and supported
    ...
    struct ieee80211_channel *chan;
    int i, freq;
    let mut channels_counter: c_int = 0;
...
//
    ...
    let mut pri_khz: u32 = ieee80211_channel_to_khz(chandef.chan);
    let mut end_khz: u32 = cfg80211_s1g_get_end_freq_khz(chandef);
    let mut start_khz: u32 = cfg80211_s1g_get_start_freq_khz(chandef);
    let mut width_mhz: c_int = cfg80211_chandef_get_width(chandef);
    let mut prohibited_flags: u32 = IEEE80211_CHAN_DISABLED;
    if (width_mhz >= 16)
...
// We can skip IEEE80211_CHAN_NO_IR if chandef dfs available
    ...
    {
    struct cfg80211_registered_device *rdev = wiphy_to_rdev(wiphy);
    let mut permitting_flags: u32 = 0;
    let mut check_no_ir: bool = true;
//
