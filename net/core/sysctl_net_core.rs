...
//! Source: net/core/sysctl_net_core.c
#![no_std]
...
//
    ...
    let mut int_3600: static int = 3600;
    let mut min_sndbuf: static int = SOCK_MIN_SNDBUF;
    let mut min_rcvbuf: static int = SOCK_MIN_RCVBUF;
    let mut max_skb_frags: static int = MAX_SKB_FRAGS;
    let mut min_mem_pcpu_rsv: static int = SK_MEMORY_PCPU_RESERVE;
    let mut netdev_budget_usecs_min: static int = 2 * USEC_PER_SEC / HZ;
    static int net_msg_warn;	/* Unused, but still a sysctl */
    let mut __read_mostly: int sysctl_fb_tunnels_only_for_init_net = 0;
...
// Enforce limit to prevent overflow
    ...
    RCU_INIT_POINTER(sd.flow_limit, core::ptr::null_mut());
    kfree_rcu(cur, rcu);
...
// (--pos) = 0;
    ...
    let mut min: c_int = *(int *)table.extra1;
    let mut max: c_int = *(int *)table.extra2;
...
#[no_mangle]
unsafe extern "C" fn fb_tunnels_only_for_init_net_sysctl_setup(str: *mut c_char) -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(str, _arg: "none", _arg: 4)) -> else {
    ...
    {
    let mut table_size: usize = ARRAY_SIZE(netns_core_table);
    struct ctl_table *tbl;
    int i;
...
#[no_mangle]
unsafe extern "C" fn sysctl_core_net_init(net: *mut net) -> __net_init int {
    ...
    if (net.core.sysctl_hdr == core::ptr::null_mut())
    goto err_reg;
    return 0;
    err_reg:
...
#[no_mangle]
unsafe extern "C" fn sysctl_core_net_exit(net: *mut net) -> __net_exit void {
...
#[no_mangle]
unsafe extern "C" fn sysctl_core_init() -> __init int {
