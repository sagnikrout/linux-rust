...
pub const PKTGEN_MAGIC: c_uint = 0xbe9be955;

pub const MAX_CFLOWS: c_int = 65536;
...
//
    pub /: *mut *mut *mut proc_dir_entry entry; / proc file,
    pub /: *mut *mut *mut pktgen_thread pg_thread;/ the owner,
    pub /: *mut *mut list_head list; / chaining in the thread's run-queue,
    pub /: *mut *mut rcu_head rcu; / freed by RCU,
    pub /: *mut *mut int running; / if false, the test will stop,
// If min != max, then we will either do a linear iteration, or
// we will do a random selection from within the range.
//
    pub flags: __u32,
    pub xmit_mode: c_int,
    pub min_pkt_size: c_int,
    pub max_pkt_size: c_int,
    pub /: *mut *mut int pkt_overhead; / overhead for MPLS, VLANs, IPSEC etc,
    pub nfrags: c_int,
    pub for: *mut *mut int removal_mark; / non-zero => the device is marked,
// removal by worker thread
//
    pub page: *mut page,
    pub /: *mut *mut u64 delay; / nano-seconds,
    pub /: *mut *mut __u64 count; / Default No packets to send,
    pub /: *mut *mut __u64 sofar; / How many pkts we've sent so far,
    pub /: *mut *mut __u64 tx_bytes; / How many bytes we've transmitted,
    pub /: *mut *mut __u64 errors; / Errors when trying to transmit,,
// runtime counters relating to clone_skb
    pub clone_count: __u32,
    pub sent?: *mut *mut int last_ok; / Was last skb,
// Or a failed transmit of some sort?
// This will keep sequence numbers in order
//
    pub next_tx: ktime_t,
    pub started_at: ktime_t,
    pub stopped_at: ktime_t,
    pub /: *mut *mut u64 idle_acc; / nano-seconds,
    pub seq_num: __u32,
    pub /*: *mut int clone_skb;,
...
//
    pub /: *mut *mut char dst_min[IP_NAME_SZ]; / IP, ie 1.2.3.4,
    pub /: *mut *mut char dst_max[IP_NAME_SZ]; / IP, ie 1.2.3.4,
    pub /: *mut *mut char src_min[IP_NAME_SZ]; / IP, ie 1.2.3.4,
    pub /: *mut *mut char src_max[IP_NAME_SZ]; / IP, ie 1.2.3.4,
    pub in6_saddr: in6_addr,
    pub in6_daddr: in6_addr,
    pub cur_in6_daddr: in6_addr,
    pub cur_in6_saddr: in6_addr,
// For ranges
    pub min_in6_daddr: in6_addr,
    pub max_in6_daddr: in6_addr,
    pub min_in6_saddr: in6_addr,
    pub max_in6_saddr: in6_addr,
// If we're doing ranges, random or incremental, then this
// defines the min/max for those ranges.
//
    pub /: *mut *mut __be32 saddr_min; / inclusive, source IP address,
    pub /: *mut *mut __be32 saddr_max; / exclusive, source IP address,
    pub /: *mut *mut __be32 daddr_min; / inclusive, dest IP address,
    pub /: *mut *mut __be32 daddr_max; / exclusive, dest IP address,
    pub /: *mut *mut __u16 udp_src_min; / inclusive, source UDP port,
    pub /: *mut *mut __u16 udp_src_max; / exclusive, source UDP port,
    pub /: *mut *mut __u16 udp_dst_min; / inclusive, dest UDP port,
    pub /: *mut *mut __u16 udp_dst_max; / exclusive, dest UDP port,
// DSCP + ECN
    pub TOS: *mut *mut __u8 tos; / six MSB of (former) IPv4,
// are for dscp codepoint
//
    pub IPv6: *mut *mut __u8 traffic_class; / ditto for the (former) Traffic Class in,
// (see RFC 3260, sec. 4)
//
// IMIX
    pub n_imix_entries: c_uint,
    pub imix_entries: [imix_pkt; MAX_IMIX_ENTRIES],
// Maps 0-IMIX_PRECISION range to imix_entry based on probability
    pub imix_distribution: [__u8; IMIX_PRECISION],
// MPLS
    pub /: *mut *mut unsigned int nr_labels; / Depth of stack, 0 = no MPLS,
    pub labels: [__be32; MAX_MPLS_LABELS],
// VLAN/SVLAN (802.1Q/Q-in-Q)
    pub vlan_p: __u8,
    pub vlan_cfi: __u8,
    pub /: *mut *mut __u16 vlan_id; / 0xffff means no vlan tag,
    pub svlan_p: __u8,
    pub svlan_cfi: __u8,
    pub /: *mut *mut __u16 svlan_id; / 0xffff means no svlan tag,
    pub /: *mut *mut __u32 src_mac_count; / How many MACs to iterate through,
    pub /: *mut *mut __u32 dst_mac_count; / How many MACs to iterate through,
    pub dst_mac: [c_uchar; ETH_ALEN],
    pub src_mac: [c_uchar; ETH_ALEN],
    pub cur_dst_mac_offset: __u32,
    pub cur_src_mac_offset: __u32,
    pub cur_saddr: __be32,
    pub cur_daddr: __be32,
    pub ip_id: __u16,
    pub cur_udp_dst: __u16,
    pub cur_udp_src: __u16,
    pub cur_queue_map: __u16,
    pub cur_pkt_size: __u32,
    pub last_pkt_size: __u32,
    pub hh: [__u8; 14],
// = {
...
// };
//
    pub /: *mut *mut __u16 pad; / pad out the hh struct to an even 16 bytes,
    pub we: *mut *mut *mut sk_buff skb; / skb we are to transmit next, used for when,
// are transmitting the same one multiple times
//
    pub device.: *mut *mut *mut net_device odev; / The out-going,
...
//
    ...
    pub odevname: [c_char; 32],
    pub flows: *mut flow_state,
    pub /: *mut *mut unsigned int cflows; / Concurrent flows (config),
    pub /: *mut *mut unsigned int lflow; / Flow length (config),
    pub /: *mut *mut unsigned int nflows; / accumulated flows (stats),
    pub (state)*/: *mut *mut unsigned int curfl; / current sequenced flow,
    pub queue_map_min: u16,
    pub queue_map_max: u16,
    pub /: *mut *mut __u32 skb_priority; / skb priority field,
    pub /: *mut *mut unsigned int burst; / number of duplicated packets to burst,
    pub /: *mut *mut int node; / Memory node,

    pub /: *mut *mut __u8 ipsmode; / IPSEC mode (config),
    pub /: *mut *mut __u8 ipsproto; / IPSEC type (config),
    pub spi: __u32,
    pub xdst: xfrm_dst,
    pub dstops: dst_ops,
    pub result: [c_char; 512],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct pktgen_hdr {
    pub pgh_magic: __be32,
    pub seq_num: __be32,
    pub tv_sec: __be32,
    pub tv_usec: __be32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pktgen_net {
    pub net: *mut net,
    pub proc_dir: *mut proc_dir_entry,
    pub pktgen_threads: list_head,
    pub pktgen_exiting: bool,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct pktgen_thread {
    pub /: *mut *mut mutex if_lock; / for list of devices,
    pub /: *mut *mut list_head if_list; / All device here,
    pub th_list: list_head,
...
//
    ...
    pub net: *mut pktgen_net,
}

pub const REMOVE: c_int = 1;
...
// Module parameters, defaults.
    let mut __read_mostly: static int pg_count_d = 1000;
...
//
#[no_mangle]
unsafe extern "C" fn pgctrl_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(data, _arg: "start")) -> else {
    else if (!strcmp(data, "start"))
    pktgen_run_all_threads(pn);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(data, _arg: "reset")) -> else {
...
#[no_mangle]
unsafe extern "C" fn pgctrl_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pktgen_if_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
    static int pktgen_if_show(struct seq_file *seq, void *v)
    {
    const struct pktgen_dev *pkt_dev = seq.private;
    let mut cflows: c_uint = READ_ONCE(pkt_dev.cflows);
    ktime_t stopped;
    unsigned int i;
    u64 idle;
    seq_printf(seq,
...
#[no_mangle]
pub unsafe extern "C" fn if(M_QUEUE_XMIT: pkt_dev->xmit_mode ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn count_trail_chars(user_buffer: *const char __user, maxlen: usize) -> isize {
...
#[no_mangle]
unsafe extern "C" fn strn_len(user_buffer: *const char __user, maxlen: usize) -> isize {
...
#[no_mangle]
unsafe extern "C" fn pktgen_read_flag(f: *const c_char, disable: *mut bool) -> __u32 {
...
// disable = !*disable;
    ...
    char *pg_result = core::ptr::null_mut();
    char buf[128];
...
// Read variable name
    ...
    i += len;
    if (debug) {
    let mut copy: usize = min_t(size_t, count + 1, 1024);
...
//
    ...
    pkt_dev.page = core::ptr::null_mut();
    }
    } else {
...
// (pg_result - 2) = '\0';
    ...
    return -EFAULT;
    buf[len] = 0;
    in6_pton(buf, -1, pkt_dev.in6_daddr.s6_addr, -1, core::ptr::null_mut());
    ...
    return -EFAULT;
    buf[len] = 0;
    in6_pton(buf, -1, pkt_dev.min_in6_daddr.s6_addr, -1, core::ptr::null_mut());
    ...
    return -EFAULT;
    buf[len] = 0;
    in6_pton(buf, -1, pkt_dev.max_in6_daddr.s6_addr, -1, core::ptr::null_mut());
    ...
    return -EFAULT;
    buf[len] = 0;
    in6_pton(buf, -1, pkt_dev.in6_saddr.s6_addr, -1, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn pktgen_if_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pktgen_thread_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pktgen_thread_open(inode: *mut inode, file: *mut file) -> c_int {
...
// Think find or remove for NN
    ...
    struct pktgen_dev *pkt_dev = core::ptr::null_mut();
    let mut exact: bool = (remove == FIND);
...
//
#[no_mangle]
unsafe extern "C" fn pktgen_mark_device(pn: *const pktgen_net, ifname: *const c_char) {
    static void pktgen_mark_device(const struct pktgen_net *pn, const char *ifname)
    {
    struct pktgen_dev *pkt_dev = core::ptr::null_mut();
    let mut max_tries: c_int = 10, msec_per_try = 125;
    let mut i: c_int = 0;
    mutex_lock(&pktgen_thread_lock);
    pr_debug("%s: marking %s for removal\n", __func__, ifname);
    while (1) {
    pkt_dev = __pktgen_NN_threads(pn, ifname, REMOVE);
    if (pkt_dev == core::ptr::null_mut())
    break;	/* success */
    mutex_unlock(&pktgen_thread_lock);
...
#[no_mangle]
unsafe extern "C" fn pktgen_change_name(pn: *const pktgen_net, dev: *mut net_device) {
...
// Clean old setups
    ...
    pkt_dev.odev = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn pktgen_setup_inject(pkt_dev: *mut pktgen_dev) {
    static void pktgen_setup_inject(struct pktgen_dev *pkt_dev)
    {
    int ntxq;
    if (!pkt_dev.odev) {
    pr_err("ERROR: pkt_dev.odev == core::ptr::null_mut() in setup_inject\n");
    sprintf(pkt_dev.result,
    "ERROR: pkt_dev.odev == core::ptr::null_mut() in setup_inject.\n");
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn spin(pkt_dev: *mut pktgen_dev, spin_until: ktime_t) {
...
#[no_mangle]
pub unsafe extern "C" fn set_pkt_overhead(pkt_dev: *mut pktgen_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn f_seen(pkt_dev: *const pktgen_dev, flow: c_int) -> c_int {
    static inline int f_seen(const struct pktgen_dev *pkt_dev, int flow)
    {
    return !!(pkt_dev.flows[flow].flags & F_INIT);
    }
#[no_mangle]
pub unsafe extern "C" fn f_pick(pkt_dev: *mut pktgen_dev, cflows: c_uint) -> c_int {
    static inline int f_pick(struct pktgen_dev *pkt_dev, unsigned int cflows)
    {
    let mut flow: c_int = pkt_dev.curfl;
...
#[no_mangle]
unsafe extern "C" fn get_ipsec_sa(pkt_dev: *mut pktgen_dev, flow: c_int) {
...
#[no_mangle]
unsafe extern "C" fn set_cur_queue_map(pkt_dev: *mut pktgen_dev) {
    static void set_cur_queue_map(struct pktgen_dev *pkt_dev)
    {
    if (pkt_dev.flags & F_QUEUE_MAP_CPU)
    pkt_dev.cur_queue_map = smp_processor_id();
#[no_mangle]
pub unsafe extern "C" fn if(pkt_dev->queue_map_max: pkt_dev->queue_map_min <=) -> else {
    __u16 t;
...
//
#[no_mangle]
unsafe extern "C" fn mod_cur_headers(pkt_dev: *mut pktgen_dev) {
    static void mod_cur_headers(struct pktgen_dev *pkt_dev)
    {
    unsigned int cflows;
    __u32 imn;
    __u32 imx;
    let mut flow: c_int = 0;
...
// Only random destinations yet
    ...
    let mut t: __u32 = get_random_u32_below(IMIX_PRECISION);
    let mut entry_index: __u8 = pkt_dev.imix_distribution[t];
...
#[no_mangle]
unsafe extern "C" fn fill_imix_distribution(pkt_dev: *mut pktgen_dev) {
    static void fill_imix_distribution(struct pktgen_dev *pkt_dev)
    {
    int cumulative_probabilites[MAX_IMIX_ENTRIES];
    let mut j: c_int = 0;
    let mut cumulative_prob: __u64 = 0;
    let mut total_weight: __u64 = 0;
    let mut i: c_int = 0;
    for (i = 0; i < pkt_dev.n_imix_entries; i++)
...
#[no_mangle]
unsafe extern "C" fn pktgen_output_ipsec(skb: *mut sk_buff, pkt_dev: *mut pktgen_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_SAs(pkt_dev: *mut pktgen_dev) {
...
// let go of the SAs if we have them
    ...
    if (x) {
    xfrm_state_put(x);
    pkt_dev.flows[i].x = core::ptr::null_mut();
    }
    }
    }
    }
...
#[no_mangle]
unsafe extern "C" fn mpls_push(mpls: *mut __be32, pkt_dev: *mut pktgen_dev) {
...
// mpls |= MPLS_STACK_BOTTOM;
    ...
    let mut frags: c_int = pkt_dev.nfrags;
    int i, len;
    int frag_len;
    ...
    let mut node: c_int = numa_node_id();
...
//
    ...
    struct pktgen_dev *pkt_dev)
    {
    let mut extralen: c_uint = LL_RESERVED_SPACE(dev);
    struct sk_buff *skb = core::ptr::null_mut();
    unsigned int size;
    size = pkt_dev.cur_pkt_size + 64 + extralen + pkt_dev.pkt_overhead;
    if (pkt_dev.flags & F_NODE) {
    let mut node: c_int = pkt_dev.node >= 0 ? pkt_dev.node : numa_node_id();
...
// the caller pre-fetches from skb->data and reserves for the mac hdr
    ...
    int datalen, iplen;
    struct iphdr *iph;
    let mut protocol: __be16 = htons(ETH_P_IP);
    __be32 *mpls;
    __be16 *vlan_tci = core::ptr::null_mut();                 /* Encapsulates priority and VLAN ID */
    __be16 *vlan_encapsulated_proto = core::ptr::null_mut();  /* packet type ID field (or len) for VLAN tag */
    __be16 *svlan_tci = core::ptr::null_mut();                /* Encapsulates priority and SVLAN ID */
    __be16 *svlan_encapsulated_proto = core::ptr::null_mut(); /* packet type ID field (or len) for SVLAN tag */
    u16 queue_map;
    if (pkt_dev.nr_labels)
...
// Eth + IPh + UDPh + mpls
    ...
    let mut csum: __wsum = skb_checksum(skb, skb_transport_offset(skb), datalen + 8, 0);
// add protocol-dependent pseudo-header
    ...
    int datalen, udplen;
    struct ipv6hdr *iph;
    let mut protocol: __be16 = htons(ETH_P_IPV6);
    __be32 *mpls;
    __be16 *vlan_tci = core::ptr::null_mut();                 /* Encapsulates priority and VLAN ID */
    __be16 *vlan_encapsulated_proto = core::ptr::null_mut();  /* packet type ID field (or len) for VLAN tag */
    __be16 *svlan_tci = core::ptr::null_mut();                /* Encapsulates priority and SVLAN ID */
...
// (__be32 *)iph |= htonl(0x60000000 | (pkt_dev->traffic_class << 20));
    ...
    let mut csum: __wsum = skb_checksum(skb, skb_transport_offset(skb), udplen, 0);
// add protocol-dependent pseudo-header
...
#[no_mangle]
unsafe extern "C" fn pktgen_clear_counters(pkt_dev: *mut pktgen_dev) {
    static void pktgen_clear_counters(struct pktgen_dev *pkt_dev)
    {
    pkt_dev.seq_num = 1;
    pkt_dev.idle_acc = 0;
...
#[no_mangle]
unsafe extern "C" fn pktgen_run(t: *mut pktgen_thread) {
...
#[no_mangle]
unsafe extern "C" fn pktgen_handle_all_threads(pn: *mut pktgen_net, flags: u32) {
...
#[no_mangle]
unsafe extern "C" fn pktgen_stop_all_threads(pn: *mut pktgen_net) {
    static void pktgen_stop_all_threads(struct pktgen_net *pn)
    {
    func_enter();
...
#[no_mangle]
unsafe extern "C" fn thread_is_running(t: *const pktgen_thread) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pktgen_wait_thread_run(t: *mut pktgen_thread) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pktgen_wait_all_threads_run(pn: *mut pktgen_net) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pktgen_run_all_threads(pn: *mut pktgen_net) {
    static void pktgen_run_all_threads(struct pktgen_net *pn)
    {
    func_enter();
...
#[no_mangle]
unsafe extern "C" fn pktgen_reset_all_threads(pn: *mut pktgen_net) {
    static void pktgen_reset_all_threads(struct pktgen_net *pn)
    {
    func_enter();
...
#[no_mangle]
unsafe extern "C" fn show_results(pkt_dev: *mut pktgen_dev, nr_frags: c_int) {
    ...
    pkt_dev.started_at);
    let mut idle: ktime_t = ns_to_ktime(pkt_dev.idle_acc);
...
#[no_mangle]
unsafe extern "C" fn pktgen_stop_device(pkt_dev: *mut pktgen_dev) -> c_int {
    static int pktgen_stop_device(struct pktgen_dev *pkt_dev)
    {
    let mut nr_frags: c_int = pkt_dev.skb ? skb_shinfo(pkt_dev.skb).nr_frags : -1;
    if (!pkt_dev.running) {
    ...
    {
    struct pktgen_dev *pkt_dev, *best = core::ptr::null_mut();
    rcu_read_lock();
    list_for_each_entry_rcu(pkt_dev, &t.if_list, list) {
    if (!pkt_dev.running)
    continue;
    if (best == core::ptr::null_mut())
    best = pkt_dev;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ktime_compare(pkt_dev->next_tx, 0: best->next_tx) <) -> else {
...
#[no_mangle]
unsafe extern "C" fn pktgen_stop(t: *mut pktgen_thread) {
    static void pktgen_stop(struct pktgen_thread *t)
    {
    struct pktgen_dev *pkt_dev;
    func_enter();
...
//
#[no_mangle]
unsafe extern "C" fn pktgen_rem_one_if(t: *mut pktgen_thread) {
    ...
    continue;
    kfree_skb(cur.skb);
    cur.skb = core::ptr::null_mut();
    pktgen_remove_device(t, cur);
    break;
    }
    }
#[no_mangle]
unsafe extern "C" fn pktgen_rem_all_ifs(t: *mut pktgen_thread) {
...
#[no_mangle]
unsafe extern "C" fn pktgen_rem_thread(t: *mut pktgen_thread) {
...
#[no_mangle]
unsafe extern "C" fn pktgen_resched(pkt_dev: *mut pktgen_dev) {
    static void pktgen_resched(struct pktgen_dev *pkt_dev)
    {
    let mut idle_start: ktime_t = ktime_get();
    schedule();
    pkt_dev.idle_acc += ktime_to_ns(ktime_sub(ktime_get(), idle_start));
    }
#[no_mangle]
unsafe extern "C" fn pktgen_wait_for_skb(pkt_dev: *mut pktgen_dev) {
...
#[no_mangle]
unsafe extern "C" fn pktgen_xmit(pkt_dev: *mut pktgen_dev) {
    static void pktgen_xmit(struct pktgen_dev *pkt_dev)
    {
    let mut skb_shared: bool = !!(READ_ONCE(pkt_dev.flags) & F_SHARED);
    struct net_device *odev = pkt_dev.odev;
    struct netdev_queue *txq;
    let mut burst: c_uint = 1;
    struct sk_buff *skb;
    let mut clone_skb: c_int = 0;
    int ret;
...
// build a new pkt
    ...
    if (pkt_dev.skb == core::ptr::null_mut()) {
...
//
#[no_mangle]
unsafe extern "C" fn pktgen_thread_worker(arg: *mut c_void) -> c_int {
    static int pktgen_thread_worker(void *arg)
    {
    struct pktgen_thread *t = arg;
    struct pktgen_dev *pkt_dev = core::ptr::null_mut();
    let mut cpu: c_int = t.cpu;
    ...
    const char *ifname, bool exact)
    {
    struct pktgen_dev *p, *pkt_dev = core::ptr::null_mut();
    let mut len: usize = strlen(ifname);
    rcu_read_lock();
...
#[no_mangle]
unsafe extern "C" fn pktgen_add_device(t: *mut pktgen_thread, ifname: *const c_char) -> c_int {
    static int pktgen_add_device(struct pktgen_thread *t, const char *ifname)
    {
    struct pktgen_dev *pkt_dev;
    int err;
    let mut node: c_int = cpu_to_node(t.cpu);
// We don't allow a device to be on several threads
    ...
    node);
    if (pkt_dev.flows == core::ptr::null_mut()) {
    kfree(pkt_dev);
    return -ENOMEM;
    }
...
#[no_mangle]
unsafe extern "C" fn pktgen_create_thread(cpu: c_int, pn: *mut pktgen_net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn pg_net_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn pg_net_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn pg_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn pg_cleanup() -> void __exit {
