...
#[no_mangle]
pub unsafe extern "C" fn page_pool_ethtool_stats_get_count() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn page_pool_producer_lock(pool: *mut page_pool) -> bool {
    static bool page_pool_producer_lock(struct page_pool *pool)
    __acquires(&pool.ring.producer_lock)
    {
    let mut in_softirq: bool = in_softirq();
    if (in_softirq)
...
#[no_mangle]
unsafe extern "C" fn page_pool_struct_check() {
...
// Driver calling page_pool_create() also call page_pool_destroy()
    ...
    return 0;
    free_ptr_ring:
    ptr_ring_cleanup(&pool.ring, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn page_pool_uninit(pool: *mut page_pool) {
...
#[no_mangle]
unsafe extern "C" fn page_pool_refill_alloc_cache(pool: *mut page_pool) -> noinline netmem_ref {
...
// Refill alloc array, but only if NUMA match
    do {
    netmem = ( netmem_ref)__ptr_ring_consume(r);
    if (unlikely(!netmem))
    break;
...
// fast path
#[no_mangle]
unsafe extern "C" fn __page_pool_get_cached(pool: *mut page_pool) -> netmem_ref {
...
// Fast-path
    ...
    let mut dma_addr: dma_addr_t = page_pool_get_dma_addr_netmem(netmem);
...
#[no_mangle]
unsafe extern "C" fn page_pool_dma_map(pool: *mut page_pool, netmem: netmem_ref, gfp: gfp_t) -> bool {
...
// slow path
    ...
    gfp_t gfp)
    {
    let mut bulk: c_int = PP_ALLOC_CACHE_REFILL;
    let mut pp_order: c_uint = pool.p.order;
    let mut dma_map: bool = pool.dma_map;
    netmem_ref netmem;
    int i, nr_pages;
...
//
#[no_mangle]
pub unsafe extern "C" fn page_pool_alloc_netmems(pool: *mut page_pool, gfp: gfp_t) -> netmem_ref {
...
// Slow-path: cache empty, do real allocation
    ...
    ALLOW_ERROR_INJECTION(page_pool_alloc_netmems, core::ptr::null_mut());
...
pub unsafe extern "C" fn page_pool_inflight(pool: *const page_pool, strict: bool) -> i32 {
    ...
    {
    let mut release_cnt: u32 = atomic_read(&pool.pages_state_release_cnt);
    let mut hold_cnt: u32 = READ_ONCE(pool.pages_state_hold_cnt);
    s32 inflight;
...
#[no_mangle]
pub unsafe extern "C" fn page_pool_set_pp_info(pool: *mut page_pool, netmem: netmem_ref) {
...
#[no_mangle]
pub unsafe extern "C" fn page_pool_clear_pp_info(netmem: netmem_ref) {
    void page_pool_clear_pp_info(netmem_ref netmem)
    {
    netmem_clear_pp_magic(netmem);
    netmem_set_pp(netmem, core::ptr::null_mut());
    }
...
//
#[no_mangle]
unsafe extern "C" fn page_pool_return_netmem(pool: *mut page_pool, netmem: netmem_ref) {
    static void page_pool_return_netmem(struct page_pool *pool, netmem_ref netmem)
    {
    int count;
    bool put;
...
#[no_mangle]
unsafe extern "C" fn page_pool_recycle_in_ring(pool: *mut page_pool, netmem: netmem_ref) -> bool {
    static bool page_pool_recycle_in_ring(struct page_pool *pool, netmem_ref netmem)
    {
    bool in_softirq, ret;
// BH protection not needed if current is softirq
    in_softirq = page_pool_producer_lock(pool);
    ret = !__ptr_ring_produce(&pool.ring, ( void *)netmem);
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn __page_pool_page_can_be_recycled(netmem: netmem_ref) -> bool {
...
#[no_mangle]
unsafe extern "C" fn page_pool_napi_local(pool: *const page_pool) -> bool {
...
// Bulk produce into ptr_ring page_pool cache
    ...
    if (__ptr_ring_produce(&pool.ring, ( void *)bulk[i])) {
// ring full
    recycle_stat_inc(pool, ring_full);
    break;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn page_pool_put_netmem_bulk(data: *mut netmem_ref, count: u32) {
    void page_pool_put_netmem_bulk(netmem_ref *data, u32 count)
    {
    let mut bulk_len: u32 = 0;
    for (u32 i = 0; i < count; i++) {
    let mut netmem: netmem_ref = netmem_compound_head(data[i]);
    ...
    struct page_pool *pool = core::ptr::null_mut();
    bool allow_direct;
    let mut foreign: u32 = 0;
    bulk_len = 0;
    for (u32 i = 0; i < count; i++) {
    struct page_pool *netmem_pp;
    let mut netmem: netmem_ref = data[i];
...
// Approved for bulk recycling in ptr_ring cache
    ...
    netmem_ref netmem)
    {
    let mut drain_count: c_long = BIAS_MAX - pool.frag_users;
...
#[no_mangle]
unsafe extern "C" fn page_pool_free_frag(pool: *mut page_pool) {
    static void page_pool_free_frag(struct page_pool *pool)
    {
    let mut drain_count: c_long = BIAS_MAX - pool.frag_users;
    let mut netmem: netmem_ref = pool.frag_page;
    pool.frag_page = 0;
    ...
    gfp_t gfp)
    {
    let mut max_size: c_uint = PAGE_SIZE << pool.p.order;
    let mut netmem: netmem_ref = pool.frag_page;
...
#[no_mangle]
unsafe extern "C" fn page_pool_empty_ring(pool: *mut page_pool) {
    static void page_pool_empty_ring(struct page_pool *pool)
    {
    netmem_ref netmem;
// Empty recycle ring
    while ((netmem = ( netmem_ref)ptr_ring_consume_bh(&pool.ring))) {
...
#[no_mangle]
unsafe extern "C" fn __page_pool_destroy(pool: *mut page_pool) {
...
#[no_mangle]
unsafe extern "C" fn page_pool_empty_alloc_cache_once(pool: *mut page_pool) {
...
#[no_mangle]
unsafe extern "C" fn page_pool_scrub(pool: *mut page_pool) {
...
#[no_mangle]
unsafe extern "C" fn page_pool_release(pool: *mut page_pool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn page_pool_release_retry(wq: *mut work_struct) {
...
// Periodic warning for page pools the user can't see
    ...
    let mut sec: c_int = (s32)((u32)jiffies - (u32)pool.defer_start) / HZ;
...
//
    ...
    mutex_lock(&page_pools_lock);
    WRITE_ONCE(pool.p.napi, core::ptr::null_mut());
    mutex_unlock(&page_pools_lock);
    }
...
#[no_mangle]
pub unsafe extern "C" fn page_pool_update_nid(pool: *mut page_pool, new_nid: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn net_mp_niov_set_dma_addr(niov: *mut net_iov, addr: dma_addr_t) -> bool {
...
pub unsafe extern "C" fn net_mp_niov_set_page_pool(pool: *mut page_pool, niov: *mut net_iov) {
    ...
    {
    let mut netmem: netmem_ref = net_iov_to_netmem(niov);
    page_pool_set_pp_info(pool, netmem);
    pool.pages_state_hold_cnt++;
