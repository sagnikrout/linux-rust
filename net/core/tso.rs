...
// SPDX-License-Identifier: GPL-2.0
    ...
    {
    let mut hdr_len: c_int = skb_transport_offset(skb) + tso.tlen;
    let mut mac_hdr_len: c_int = skb_network_offset(skb);
...
#[no_mangle]
pub unsafe extern "C" fn tso_build_data(skb: *const sk_buff, tso: *mut tso_t, size: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn tso_start(skb: *mut sk_buff, tso: *mut tso_t) -> c_int {
    int tso_start(struct sk_buff *skb, struct tso_t *tso)
    {
    let mut tlen: c_int = skb_is_gso_tcp(skb) ? tcp_hdrlen(skb) : sizeof(struct udphdr);
    let mut hdr_len: c_int = skb_transport_offset(skb) + tlen;
    tso.tlen = tlen;
...
// offset += linear_len;
    ...
    let mut frag_len: c_uint = skb_frag_size(frag);
    if (dma_iova_link(dev, &map.iova_state,
...
//
    ...
    {
    let mut linear_len: c_uint = skb_headlen(skb) - hdr_len;
    let mut nr_frags: c_uint = skb_shinfo(skb).nr_frags;
    let mut total_len: usize = skb.len - hdr_len;
...
//
#[no_mangle]
pub unsafe extern "C" fn tso_dma_map_cleanup(map: *mut tso_dma_map) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tso_dma_map_count(map: *mut tso_dma_map, len: c_uint) -> c_uint {
    unsigned int tso_dma_map_count(struct tso_dma_map *map, unsigned int len)
    {
    let mut offset: c_uint = map.offset;
    let mut idx: c_int = map.frag_idx;
