...
//
    ...
    let mut pad_len: usize = attr.max_size - skb.len;
    ...
    if (attr.tcp) {
    let mut l4len: c_int = skb.len - skb_transport_offset(skb);
...
// Force mangled checksum
    ...
    ret = tpriv.ok;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !tpriv->ok) -> else {
    else if (!tpriv.ok)
    ret = -ETIMEDOUT;
    else
    ret = 0;
...
#[no_mangle]
unsafe extern "C" fn net_test_netif_carrier(ndev: *mut net_device) -> c_int {
    static int net_test_netif_carrier(struct net_device *ndev)
    {
    return netif_carrier_ok(ndev) ? 0 : -ENOLINK;
    }
#[no_mangle]
unsafe extern "C" fn net_test_phy_phydev(ndev: *mut net_device) -> c_int {
    static int net_test_phy_phydev(struct net_device *ndev)
    {
    return ndev.phydev ? 0 : -EOPNOTSUPP;
    }
#[no_mangle]
unsafe extern "C" fn net_test_phy_loopback_enable(ndev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn net_test_phy_loopback_disable(ndev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn net_test_phy_loopback_udp(ndev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn net_test_phy_loopback_udp_mtu(ndev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn net_test_phy_loopback_tcp(ndev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn net_test_phy_loopback_tcp_bad_csum(ndev: *mut net_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn net_selftest(ndev: *mut net_device, etest: *mut ethtool_test, buf: *mut u64) {
    void net_selftest(struct net_device *ndev, struct ethtool_test *etest, u64 *buf)
    {
    let mut count: c_int = net_selftest_get_count();
    int i;
