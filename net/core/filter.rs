...
#[no_mangle]
pub unsafe extern "C" fn copy_bpf_fprog_from_user(dst: *mut sock_fprog, src: sockptr_t, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_load_helper_convert_offset(skb: *const sk_buff, offset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn convert_bpf_ld_abs(fp: *mut sock_filter, insnp: *mut bpf_insn) -> bool {
    static bool convert_bpf_ld_abs(struct sock_filter *fp, struct bpf_insn **insnp)
    {
    let mut unaligned_ok: bool = IS_BUILTIN(CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS);
    let mut size: c_int = bpf_size_to_bytes(BPF_SIZE(fp.code));
    bool endian = BPF_SIZE(fp.code) == BPF_H ||
    BPF_SIZE(fp.code) == BPF_W;
    let mut indirect: bool = BPF_MODE(fp.code) == BPF_IND;
    let mut ip_align: c_int = NET_IP_ALIGN;
    struct bpf_insn *insn = *insnp;
    let mut offset: c_int = fp.k;
    if (!indirect &&
    ((unaligned_ok && offset >= 0) ||
    (!unaligned_ok && offset >= 0 &&
    offset + ip_align >= 0 &&
    (offset + ip_align) % size == 0))) {
    let mut ldx_off_ok: bool = offset <= S16_MAX;
...
//
    ...
    bool *seen_ld_abs)
    {
    let mut new_flen: c_int = 0, pass = 0, target, i, stack_off;
    struct bpf_insn *new_insn, *first_insn = core::ptr::null_mut();
    struct sock_filter *fp;
    int *addrs = core::ptr::null_mut();
    u8 bpf_src;
...
//
#[no_mangle]
unsafe extern "C" fn check_load_and_stores(filter: *const sock_filter, flen: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn chk_code_allowed(code_to_probe: u16) -> bool {
...
// Last instruction must be a RET code
    ...
    {
    let mut fsize: c_uint = bpf_classic_proglen(fprog);
...
#[no_mangle]
unsafe extern "C" fn bpf_release_orig_filter(fp: *mut bpf_prog) {
...
#[no_mangle]
unsafe extern "C" fn __bpf_prog_release(prog: *mut bpf_prog) {
...
#[no_mangle]
unsafe extern "C" fn __sk_filter_release(fp: *mut sk_filter) {
...
//
#[no_mangle]
unsafe extern "C" fn sk_filter_release_rcu(rcu: *mut rcu_head) {
...
//
#[no_mangle]
unsafe extern "C" fn sk_filter_release(fp: *mut sk_filter) {
...
#[no_mangle]
pub unsafe extern "C" fn sk_filter_uncharge(sk: *mut sock, fp: *mut sk_filter) {
    void sk_filter_uncharge(struct sock *sk, struct sk_filter *fp)
    {
    let mut filter_size: u32 = bpf_prog_size(fp.prog.len);
...
//
#[no_mangle]
unsafe extern "C" fn __sk_filter_charge(sk: *mut sock, fp: *mut sk_filter) -> bool {
    static bool __sk_filter_charge(struct sock *sk, struct sk_filter *fp)
    {
    let mut optmem_max: c_int = READ_ONCE(sock_net(sk).core.sysctl_optmem_max);
    let mut filter_size: u32 = bpf_prog_size(fp.prog.len);
...
#[no_mangle]
pub unsafe extern "C" fn sk_filter_charge(sk: *mut sock, fp: *mut sk_filter) -> bool {
    ...
    let mut seen_ld_abs: bool = false;
...
// 1st pass: calculate the new program length.
    err = bpf_convert_filter(old_prog, old_len, core::ptr::null_mut(), &new_len,
    &seen_ld_abs);
    if (err)
    goto out_err_free;
...
//
#[no_mangle]
pub unsafe extern "C" fn bpf_prog_create(pfp: *mut bpf_prog, fprog: *mut sock_fprog_kern) -> c_int {
...
//
    fp.orig_prog = core::ptr::null_mut();
// bpf_prepare_filter() already takes care of freeing
// memory in case something goes wrong.
//
    fp = bpf_prepare_filter(fp, core::ptr::null_mut());
    if (IS_ERR(fp))
    return PTR_ERR(fp);
// pfp = fp;
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn bpf_prog_destroy(fp: *mut bpf_prog) {
...
#[no_mangle]
unsafe extern "C" fn __sk_attach_prog(prog: *mut bpf_prog, sk: *mut sock) -> c_int {
...
// memory in case something goes wrong.
//
    return bpf_prepare_filter(prog, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn sk_attach_filter(fprog: *mut sock_fprog, sk: *mut sock) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sk_reuseport_attach_filter(fprog: *mut sock_fprog, sk: *mut sock) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sk_attach_bpf(ufd: u32, sk: *mut sock) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sk_reuseport_attach_bpf(ufd: u32, sk: *mut sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sk_reuseport_prog_free_rcu(rcu: *mut rcu_head) {
...
#[no_mangle]
pub unsafe extern "C" fn sk_reuseport_prog_free(prog: *mut bpf_prog) {
...
// Avoid a splat in pskb_may_pull_reason()
    ...
    unsigned int write_len)
    {
    let mut err: c_int = __bpf_try_make_writable(skb, write_len);
    bpf_compute_data_pointers(skb);
    return err;
    }
#[no_mangle]
unsafe extern "C" fn bpf_try_make_head_writable(skb: *mut sk_buff) -> c_int {
    static int bpf_try_make_head_writable(struct sk_buff *skb)
    {
    return bpf_try_make_writable(skb, skb_headlen(skb));
    }
#[no_mangle]
pub unsafe extern "C" fn bpf_push_mac_rcsum(skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_pull_mac_rcsum(skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn __bpf_skb_load_bytes(skb: *const sk_buff, offset: u32, to: *mut c_void, len: u32) -> c_int {
...
//
    ...
    {
    return sk_fullsock(sk) ? (unsigned long)sk : (unsigned long)core::ptr::null_mut();
    }
...
//
    ...
    u64, from, u64, to, u64, flags)
    {
    let mut is_pseudo: bool = flags & BPF_F_PSEUDO_HDR;
    let mut is_mmzero: bool = flags & BPF_F_MARK_MANGLED_0;
    let mut do_mforce: bool = flags & BPF_F_MARK_ENFORCE;
    let mut is_ipv6: bool = flags & BPF_F_IPV6;
    __sum16 *ptr;
...
//
    let mut ret: __wsum = seed;
    if (from_size && to_size)
    ret = csum_sub(csum_partial(to, to_size, ret),
    csum_partial(from, from_size, 0));
#[no_mangle]
pub unsafe extern "C" fn if(_arg: to_size) -> else {
    else if (to_size)
    ret = csum_partial(to, to_size, ret);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: from_size) -> else {
    else if (from_size)
    ret = ~csum_partial(from, from_size, ~ret);
    return csum_from32to16(( unsigned int)ret);
    }
...
#[no_mangle]
pub unsafe extern "C" fn __bpf_rx_skb(dev: *mut net_device, skb: *mut sk_buff) -> c_int {
    ...
    struct sk_buff *skb)
    {
    let mut ret: c_int = ____dev_forward_skb(dev, skb, false);
    if (likely(!ret)) {
    skb.dev = dev;
    ret = netif_rx(skb);
    }
...
#[no_mangle]
pub unsafe extern "C" fn __bpf_tx_skb(dev: *mut net_device, skb: *mut sk_buff) -> c_int {
    ...
    u32 flags)
    {
    let mut mlen: c_uint = skb_network_offset(skb);
...
// Verify that a link layer header is carried
    ...
    {
    let mut hh_len: u32 = LL_RESERVED_SPACE(dev);
    ...
    dst = ip6_dst_lookup_flow(net, core::ptr::null_mut(), &fl6, core::ptr::null_mut());
    if (IS_ERR(dst))
    goto out_drop;
    skb_dst_drop(skb);
    skb_dst_set(skb, dst);
    ...
    struct rtable *rt;
    rt = ip_route_output_flow(net, &fl4, core::ptr::null_mut());
    if (IS_ERR(rt))
    goto out_drop;
...
#[no_mangle]
pub unsafe extern "C" fn skb_do_redirect(skb: *mut sk_buff) -> c_int {
    int skb_do_redirect(struct sk_buff *skb)
    {
    struct bpf_redirect_info *ri = bpf_net_ctx_get_ri();
    struct net *net = dev_net(skb.dev);
    struct net_device *dev;
    let mut flags: u32 = ri.flags;
...
#[no_mangle]
unsafe extern "C" fn sk_msg_reset_curr(msg: *mut sk_msg) {
...
#[no_mangle]
unsafe extern "C" fn sk_msg_elem_is_copy(msg: *const sk_msg, i: u32) -> bool {
    static bool sk_msg_elem_is_copy(const struct sk_msg *msg, u32 i)
    {
    return test_bit(i, msg.sg.copy);
    }
#[no_mangle]
unsafe extern "C" fn sk_msg_clear_elem_copy(msg: *mut sk_msg, i: u32) {
    static void sk_msg_clear_elem_copy(struct sk_msg *msg, u32 i)
    {
    __clear_bit(i, msg.sg.copy);
    }
#[no_mangle]
unsafe extern "C" fn sk_msg_set_elem_copy(msg: *mut sk_msg, i: u32, sg_copy: bool) {
    static void sk_msg_set_elem_copy(struct sk_msg *msg, u32 i, bool sg_copy)
    {
    __assign_bit(i, msg.sg.copy, sg_copy);
    }
#[no_mangle]
unsafe extern "C" fn sk_msg_clear_copy_range(msg: *mut sk_msg, start: u32, end: u32) {
...
#[no_mangle]
unsafe extern "C" fn sk_msg_sg_move(msg: *mut sk_msg, dst: u32, src: u32) {
    ...
    u32, end, u64, flags)
    {
    let mut len: u32 = 0, offset = 0, copy = 0, poffset = 0, bytes = end - start;
...
//
    ...
    u32, len, u64, flags)
    {
    let mut sge_copy: bool = false, nsge_copy = false, nnsge_copy = false;
    struct scatterlist sge, nsge, nnsge, rsge = {0}, *psge;
    u32 new, i = 0, l = 0, space, copy = 0, offset = 0;
    let mut rsge_copy: bool = false;
    u8 *raw, *to, *from;
    struct page *page;
    if (unlikely(flags))
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn sk_msg_shift_left(msg: *mut sk_msg, i: c_int) {
...
#[no_mangle]
unsafe extern "C" fn sk_msg_shift_right(msg: *mut sk_msg, i: c_int) {
    ...
    u32, len, u64, flags)
    {
    let mut i: u32 = 0, l = 0, space, offset = 0;
    let mut last: u64 = (u64)start + len;
    u32 pop;
    if (unlikely(flags))
    return -EINVAL;
    if (unlikely(len == 0))
...
//
    ...
    let mut sge_copy: bool = sk_msg_elem_is_copy(msg, i);
    let mut a: c_int = start - offset;
    let mut b: c_int = sge.length - pop - a;
    let mut sge_idx: u32 = i;
    sk_msg_iter_var_next(i);
    if (b > 0) {
    if (space) {
    sge.length = a;
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_generic_push(skb: *mut sk_buff, off: u32, len: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_generic_pop(skb: *mut sk_buff, off: u32, len: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_net_hdr_push(skb: *mut sk_buff, off: u32, len: u32) -> c_int {
    static int bpf_skb_net_hdr_push(struct sk_buff *skb, u32 off, u32 len)
    {
    let mut trans_same: bool = skb.transport_header == skb.network_header;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_net_hdr_pop(skb: *mut sk_buff, off: u32, len: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_proto_4_to_6(skb: *mut sk_buff) -> c_int {
    static int bpf_skb_proto_4_to_6(struct sk_buff *skb)
    {
    let mut len_diff: u32 = sizeof(struct ipv6hdr) - sizeof(struct iphdr);
    let mut meta_len: u8 = skb_metadata_len(skb);
    let mut off: u32 = skb_mac_header_len(skb);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_proto_6_to_4(skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_proto_xlat(skb: *mut sk_buff, to_proto: __be16) -> c_int {
    static int bpf_skb_proto_xlat(struct sk_buff *skb, __be16 to_proto)
    {
    let mut from_proto: __be16 = skb.protocol;
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_net_base_len(skb: *const sk_buff) -> u32 {
    ...
    u64 flags)
    {
    let mut inner_mac_len: u8 = flags >> BPF_ADJ_ROOM_ENCAP_L2_SHIFT;
    let mut encap: bool = flags & BPF_F_ADJ_ROOM_ENCAP_L3_MASK;
    let mut mac_len: u16 = 0, inner_net = 0, inner_trans = 0;
    let mut meta_len: u8 = skb_metadata_len(skb);
    let mut gso_type: c_uint = SKB_GSO_DODGY;
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(BPF_F_ADJ_ROOM_ENCAP_L4_GRE: flags &) -> else {
    else if (flags & BPF_F_ADJ_ROOM_ENCAP_L4_GRE)
    gso_type |= SKB_GSO_GRE;
#[no_mangle]
pub unsafe extern "C" fn if(BPF_F_ADJ_ROOM_ENCAP_L3_IPV6: flags &) -> else {
    else if (flags & BPF_F_ADJ_ROOM_ENCAP_L3_IPV6)
    gso_type |= SKB_GSO_IPXIP6;
#[no_mangle]
pub unsafe extern "C" fn if(BPF_F_ADJ_ROOM_ENCAP_L3_IPV4: flags &) -> else {
...
//
    ...
    u64 flags)
    {
    let mut decap: bool = flags & BPF_F_ADJ_ROOM_DECAP_L3_MASK;
    int ret;
...
// Match skb->protocol to new outer l3 protocol
    ...
    skb.protocol = htons(ETH_P_IPV6);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_F_ADJ_ROOM_DECAP_L3_IPV4: flags &) -> else {
...
// For non-GSO packets, clear encapsulation if decap flags are set
    ...
    u32, mode, u64, flags)
    {
    let mut len_diff_abs: u32 = abs(len_diff);
    let mut shrink: bool = len_diff < 0;
    let mut ret: c_int = 0;
    ...
    let mut len_min: u32 = bpf_skb_net_base_len(skb);
    let mut len_max: u32 = BPF_SKB_MAX_LEN;
    let mut proto: __be16 = skb.protocol;
    let mut shrink: bool = len_diff < 0;
    u32 off;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn __bpf_skb_min_len(skb: *const sk_buff) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_grow_rcsum(skb: *mut sk_buff, new_len: c_uint) -> c_int {
    static int bpf_skb_grow_rcsum(struct sk_buff *skb, unsigned int new_len)
    {
    let mut old_len: c_uint = skb.len;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn bpf_skb_trim_rcsum(skb: *mut sk_buff, new_len: c_uint) -> c_int {
    ...
    u64 flags)
    {
    let mut max_len: u32 = BPF_SKB_MAX_LEN;
    let mut min_len: u32 = __bpf_skb_min_len(skb);
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(skb->len: new_len <) -> else {
    ...
    u64, flags)
    {
    let mut ret: c_int = __bpf_skb_change_tail(skb, new_len, flags);
    bpf_compute_data_pointers(skb);
    return ret;
    }
    ...
    let mut meta_len: u8 = skb_metadata_len(skb);
    let mut max_len: u32 = BPF_SKB_MAX_LEN;
    let mut new_len: u32 = skb.len + head_room;
    int ret;
...
//
    ...
    u64, flags)
    {
    let mut ret: c_int = __bpf_skb_change_head(skb, head_room, flags);
    bpf_compute_data_pointers(skb);
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn xdp_get_metalen(xdp: *const xdp_buff) -> c_ulong {
    ...
    let mut metalen: c_ulong = xdp_get_metalen(xdp);
    ...
    let mut copy_off: c_ulong = off - ptr_off;
    let mut copy_len: c_ulong = min(len, ptr_len - copy_off);
    ...
    {
    let mut size: u32 = xdp.data_end - xdp.data;
    struct skb_shared_info *sinfo;
    void *addr = xdp.data;
    int i;
    ...
    let mut frag_size: u32 = skb_frag_size(&sinfo.frags[i]);
    if  (offset < frag_size) {
    ...
    out:
    return offset + len <= size ? addr + offset : core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn __bpf_xdp_load_bytes(xdp: *mut xdp_buff, offset: u32, buf: *mut c_void, len: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __bpf_xdp_store_bytes(xdp: *mut xdp_buff, offset: u32, buf: *mut c_void, len: u32) -> c_int {
    int __bpf_xdp_store_bytes(struct xdp_buff *xdp, u32 offset, void *buf, u32 len)
    {
    return ____bpf_xdp_store_bytes(xdp, offset, buf, len);
    }
#[no_mangle]
unsafe extern "C" fn bpf_xdp_frags_increase_tail(xdp: *mut xdp_buff, offset: c_int) -> c_int {
    ...
    {
    let mut mem_type: enum xdp_mem_type = xdp.rxq.mem.type;
    let mut release: bool = skb_frag_size(frag) == shrink;
    let mut netmem: netmem_ref = skb_frag_netmem(frag);
    struct xdp_buff *zc_frag = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn bpf_xdp_frags_shrink_tail(xdp: *mut xdp_buff, offset: c_int) -> c_int {
    ...
    let mut shrink: c_int = min_t(int, offset, skb_frag_size(frag));
    len_free += shrink;
    offset -= shrink;
...
// Clear memory area on grow, can contain uninit kernel memory
    ...
    let mut metalen: c_ulong = xdp.data - meta;
...
//
#[no_mangle]
pub unsafe extern "C" fn xdp_do_flush() {
...
#[no_mangle]
pub unsafe extern "C" fn xdp_do_check_flushed(napi: *mut napi_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn xdp_master_redirect(xdp: *mut xdp_buff) -> u32 {
...
//
    ...
    {
    let mut map_type: enum bpf_map_type = ri.map_type;
    void *fwd = ri.tgt_value;
    let mut map_id: u32 = ri.map_id;
    int err;
...
//
    ...
    WRITE_ONCE(ri.map, core::ptr::null_mut());
...
//
    ...
    {
    let mut skb_size: u64 = (flags & BPF_F_CTXLEN_MASK) >> 32;
...
#[no_mangle]
unsafe extern "C" fn bpf_tunnel_key_af(flags: u64) -> c_ushort {
...
//
    ...
    if (!tmp)
    return core::ptr::null_mut();
    if (cmpxchg(&md_dst, core::ptr::null_mut(), tmp))
...
#[no_mangle]
pub unsafe extern "C" fn __bpf_sk_cgroup_id(sk: *mut sock) -> u64 {
    ...
    u64, flags, void *, meta, u64, meta_size)
    {
    let mut xdp_size: u64 = (flags & BPF_F_CTXLEN_MASK) >> 32;
    if (unlikely(flags & ~(BPF_F_CTXLEN_MASK | BPF_F_INDEX_MASK)))
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn __bpf_get_netns_cookie(sk: *mut sock) -> u64 {
    ...
    {
    return __bpf_get_netns_cookie(skb && skb.sk ? skb.sk : core::ptr::null_mut());
    }
    ...
    {
    return __bpf_get_netns_cookie(ctx ? ctx.sk : core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn sk_bpf_set_get_cb_flags(sk: *mut sock, optval: *mut c_char, getopt: bool) -> c_int {
...
// (u32 *)optval = sk->sk_bpf_cb_flags;
    ...
    let mut cb_flags: c_int = tcp_sk(sk).bpf_sock_ops_cb_flags;
    memcpy(optval, &cb_flags, optlen);
    break;
    }
    case TCP_BPF_RTO_MIN: {
    let mut rto_min_us: c_int = jiffies_to_usecs(inet_csk(sk).icsk_rto_min);
    memcpy(optval, &rto_min_us, optlen);
    break;
    }
    case TCP_BPF_DELACK_MAX: {
    let mut delack_max_us: c_int = jiffies_to_usecs(inet_csk(sk).icsk_delack_max);
...
#[no_mangle]
unsafe extern "C" fn sk_allows_sol_ip_sockopt(sk: *mut sock) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(SOL_IP: IS_ENABLED(CONFIG_INET) && level ==) -> else {
    else if (IS_ENABLED(CONFIG_INET) && level == SOL_IP)
    return sol_ip_sockopt(sk, optname, optval, &optlen, false);
#[no_mangle]
pub unsafe extern "C" fn if(SOL_IPV6: IS_ENABLED(CONFIG_IPV6) && level ==) -> else {
    else if (IS_ENABLED(CONFIG_IPV6) && level == SOL_IPV6)
    return sol_ipv6_sockopt(sk, optname, optval, &optlen, false);
#[no_mangle]
pub unsafe extern "C" fn if(SOL_TCP: IS_ENABLED(CONFIG_INET) && level ==) -> else {
    else if (IS_ENABLED(CONFIG_INET) && level == SOL_TCP)
    return sol_tcp_sockopt(sk, optname, optval, &optlen, false);
    return -EINVAL;
    }
#[no_mangle]
unsafe extern "C" fn is_locked_tcp_sock_ops(bpf_sock: *mut bpf_sock_ops_kern) -> bool {
...
// (int *)optval = sk->sk_bypass_prot_mem;
    ...
    {
    if (level == SOL_SOCKET && optname == SK_BPF_BYPASS_PROT_MEM) {
    let mut err: c_int = sk_bpf_set_get_bypass_prot_mem(sk, optval, optlen, true);
    if (err)
...
// Zero out unused buffer at the end
    ...
    {
    struct sock *sk = bpf_sock.sk;
    let mut val: c_int = argval & BPF_SOCK_OPS_ALL_CB_FLAGS;
    if (!is_locked_tcp_sock_ops(bpf_sock))
    return -EOPNOTSUPP;
    ...
    let mut flags: u32 = BIND_FROM_BPF;
    int err;
    err = -EINVAL;
    ...
    {
    let mut proto: __be16 = params.h_vlan_proto;
    ...
    {
    let mut in_ifindex: u32 = params.ifindex;
    struct neighbour *neigh = core::ptr::null_mut();
...
// zero out for vlan output
    ...
    fl4.flowi4_uid = sock_net_uid(net, core::ptr::null_mut());
...
// map fib lookup errors to RTN_ type
    ...
    fib_select_path(net, &res, &fl4, core::ptr::null_mut());
    if (check_mtu) {
...
// dst = nhc->nhc_gw.ipv6;
    ...
    ( u32)params.ipv4_dst);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ENABLED(CONFIG_IPV6)) -> else {
...
// zero out for vlan output
    ...
    fl6.flowi6_uid = sock_net_uid(net, core::ptr::null_mut());
...
// only unicast is forwarded
    ...
    fl6.flowi6_oif != 0, core::ptr::null_mut(), strict);
    if (check_mtu) {
...
#[no_mangle]
unsafe extern "C" fn bpf_fib_lookup_flags_ok(flags: u32) -> bool {
    ...
    {
    struct net *net = dev_net(skb.dev);
    let mut rc: c_int = -EAFNOSUPPORT;
    let mut check_mtu: bool = false;
    if (plen < sizeof(*params))
    return -EINVAL;
    if (!bpf_fib_lookup_flags_ok(flags))
    return -EINVAL;
...
// Non-redirect use-cases can use ifindex=0 and save ifindex lookup
    ...
    {
    let mut ret: c_int = BPF_MTU_CHK_RET_FRAG_NEEDED;
...
// mtu_len = mtu;
    ...
    let mut xdp_len: c_int = xdp.data_end - xdp.data;
    let mut ret: c_int = BPF_MTU_CHK_RET_SUCCESS;
    int mtu, dev_len;
...
#[no_mangle]
unsafe extern "C" fn bpf_push_seg6_encap(skb: *mut sk_buff, type: u32, hdr: *mut c_void, len: u32) -> c_int {
    ...
    return seg6_lookup_nexthop(skb, core::ptr::null_mut(), 0);
    }
...
#[no_mangle]
unsafe extern "C" fn bpf_update_srh_state(skb: *mut sk_buff) {
    ...
    let mut srhoff: c_int = 0;
    if (ipv6_find_hdr(skb, &srhoff, IPPROTO_ROUTING, core::ptr::null_mut(), core::ptr::null_mut()) < 0) {
    srh_state.srh = core::ptr::null_mut();
    } else {
    srh_state.srh = (struct ipv6_sr_hdr *)(skb.data + srhoff);
    ...
    return -EINVAL;
    return seg6_lookup_nexthop(skb, core::ptr::null_mut(), *(int *)param);
    case SEG6_LOCAL_ACTION_END_DT6:
    if (!seg6_bpf_has_valid_srh(skb))
    return -EBADMSG;
    if (param_len != sizeof(int))
    return -EINVAL;
    if (ipv6_find_hdr(skb, &hdroff, IPPROTO_IPV6, core::ptr::null_mut(), core::ptr::null_mut()) < 0)
    return -EBADMSG;
    ...
    int ret;
    lockdep_assert_held(&srh_state.bh_lock);
    if (unlikely(srh == core::ptr::null_mut()))
    return -EINVAL;
    ...
    if (family == AF_INET) {
    let mut src4: __be32 = tuple.ipv4.saddr;
    let mut dst4: __be32 = tuple.ipv4.daddr;
    if (proto == IPPROTO_TCP)
    sk = __inet_lookup(net, core::ptr::null_mut(), 0,
    ...
    dst4, tuple.ipv4.dport,
    dif, sdif, core::ptr::null_mut());

    } else {
    struct in6_addr *src6 = (struct in6_addr *)&tuple.ipv6.saddr;
    struct in6_addr *dst6 = (struct in6_addr *)&tuple.ipv6.daddr;
    if (proto == IPPROTO_TCP)
    sk = __inet6_lookup(net, core::ptr::null_mut(), 0,
    src6, tuple.ipv6.sport,
    dst6, ntohs(tuple.ipv6.dport),
    dif, sdif, &refcounted);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: likely(ipv6_mod_enabled())) -> else {
...
//
    ...
    family = AF_INET;
#[no_mangle]
pub unsafe extern "C" fn if(sizeof(tuple->ipv6): len ==) -> else {
...
// Ensure there is no need to bump sk2 refcnt.
    ...
    {
    struct net_device *dev = skb.dev;
    let mut ifindex: c_int = dev.ifindex, sdif = dev_sdif(dev);
    ...
    let mut ifindex: c_int = dev.ifindex, sdif = dev_sdif(dev);
    struct net *caller_net = dev_net(dev);
    return (unsigned long)__bpf_sk_lookup(core::ptr::null_mut(), tuple, len, caller_net,
    ifindex, IPPROTO_UDP, netns_id,
    flags, sdif);
    }
    ...
    let mut ifindex: c_int = dev.ifindex, sdif = dev_sdif(dev);
    struct net *caller_net = dev_net(dev);
    return (unsigned long)__bpf_skc_lookup(core::ptr::null_mut(), tuple, len, caller_net,
    ifindex, IPPROTO_TCP, netns_id,
    flags, sdif);
    }
    ...
    struct bpf_sock_tuple *, tuple, u32, len, u64, netns_id, u64, flags)
    {
    return (unsigned long)__bpf_skc_lookup(core::ptr::null_mut(), tuple, len,
    sock_net(ctx.sk), 0,
    ...
    struct bpf_sock_tuple *, tuple, u32, len, u64, netns_id, u64, flags)
    {
    return (unsigned long)__bpf_sk_lookup(core::ptr::null_mut(), tuple, len,
    ...
    let mut size: return = = sizeof(__u64);
    default:
    let mut size: return = = sizeof(__u32);
    }
    }
...
// This bpf_sock->op cannot call this helper
    ...
    const u8 *op, *new_op, *magic = core::ptr::null_mut();
    struct sk_buff *skb;
    bool eol;
...
#[no_mangle]
pub unsafe extern "C" fn bpf_helper_changes_pkt_data(func_id: enum bpf_func_id) -> bool {
...
//
    ...
    struct bpf_insn_access_aux *info)
    {
    let mut size_default: c_int = sizeof(__u32);
...
// Attach type specific accesses
    ...
    read_only:
    let mut access_type: return = = BPF_READ;
    full_access:
    return true;
    }
    ...
    let mut size: return = = size_default;
    }
...
// insn++ = prog->insnsi[0];
    ...
    struct bpf_insn *insn_buf)
    {
    let mut indirect: bool = BPF_MODE(orig.code) == BPF_IND;
    struct bpf_insn *insn = insn_buf;
    if (!indirect) {
...
//
    ...
    let mut size: return = = sizeof(__u8);
    }
    return bpf_skb_is_valid_access(off, size, type, prog, info);
    }
...
#[no_mangle]
unsafe extern "C" fn __is_valid_xdp_access(off: c_int, size: c_int) -> bool {
    ...
    {
    let mut act_max: u32 = XDP_REDIRECT;
...
// insn++ = BPF_LDX_MEM(BPF_FIELD_SIZEOF(struct bpf_flow_dissector, flow_keys),
    ...
    {
    let mut value_reg: __u8 = si.dst_reg;
    let mut skb_reg: __u8 = si.src_reg;
...
// AX is needed because src_reg and dst_reg could be the same
    let mut tmp_reg: __u8 = BPF_REG_AX;
...
// insn++ = BPF_LDX_MEM(BPF_DW, value_reg, skb_reg,
    ...
    struct bpf_insn *insn)
    {
    let mut value_reg: __u8 = si.src_reg;
    let mut skb_reg: __u8 = si.dst_reg;
...
// insn++ = BPF_ALU32_IMM(BPF_RSH, si->dst_reg, 5);
    ...
    if (type == BPF_WRITE) {
    let mut offset: u32 = bpf_target_off(struct sk_buff, queue_mapping, 2, target_size);
...
#[no_mangle]
pub unsafe extern "C" fn sk_detach_filter(sk: *mut sock) -> c_int {
    ...
    lockdep_sock_is_held(sk));
    if (filter) {
    RCU_INIT_POINTER(sk.sk_filter, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn sk_get_filter(sk: *mut sock, optval: sockptr_t, len: c_uint) -> c_int {
...
//
    ...
    reuse_kern.skb = skb;
    reuse_kern.sk = sk;
    reuse_kern.selected_sk = core::ptr::null_mut();
    ...
    {
    let mut is_sockarray: bool = map.map_type == BPF_MAP_TYPE_REUSEPORT_SOCKARRAY;
    struct sock_reuseport *reuse;
...
// Lookup in sock_map can return TCP ESTABLISHED sockets.
    ...
    struct bpf_insn_access_aux *info)
    {
    let mut size_default: u32 = sizeof(__u32);
...
#[no_mangle]
pub unsafe extern "C" fn bpf_prog_change_xdp(prev_prog: *mut bpf_prog, prog: *mut bpf_prog) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bpf_xdp_pull_data(x: *mut xdp_md, len: u32) -> __bpf_kfunc int {
    ...
    let mut data_len: c_int = xdp.data_end - xdp.data;
    void *start;
    if (len <= data_len)
    return 0;
    ...
    skb_frag_t *frag = &sinfo.frags[i];
    let mut shrink: u32 = min_t(u32, delta, skb_frag_size(frag));
...
//
#[no_mangle]
pub unsafe extern "C" fn bpf_icmp_send(skb_ctx: *mut __sk_buff, type: c_int, code: c_int) -> __bpf_kfunc int {
...
#[no_mangle]
unsafe extern "C" fn bpf_kfunc_init() -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn bpf_sock_destroy(sock: *mut sock_common) -> __bpf_kfunc int {
...
#[no_mangle]
unsafe extern "C" fn tracing_iter_filter(prog: *const bpf_prog, kfunc_id: u32) -> c_int {
