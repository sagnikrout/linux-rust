...
//
#[no_mangle]
pub unsafe extern "C" fn connection_based(sk: *mut sock) -> c_int {
...
// We have to unshare an skb before modifying it.
    ...
    struct sk_buff **last)
    {
    let mut peek_at_off: bool = false;
    struct sk_buff *skb;
    let mut _off: c_int = 0;
...
//
    let mut error: c_int = sock_error(sk);
    if (error)
    goto no_packet;
    do {
...
//
#[no_mangle]
pub unsafe extern "C" fn skb_kill_datagram(sk: *mut sock, skb: *mut sk_buff, flags: c_uint) -> c_int {
...
//
    ...
    simple_copy_to_iter, core::ptr::null_mut());
    }
...
// Copy paged appendix. Hmm... why does this look so complicated?
    ...
    {
    let mut frag: c_int = skb_shinfo(skb).nr_frags;
    if (!skb_frags_readable(skb))
    return -EFAULT;
    while (length && iov_iter_count(from)) {
    struct page *head, *last_head = core::ptr::null_mut();
    ...
    let mut size: c_int = min_t(int, copied, PAGE_SIZE - start);
...
//
    ...
    struct net_devmem_dmabuf_binding *binding)
    {
    let mut orig_size: c_ulong = skb.truesize;
    unsigned long truesize;
    int ret;
    if (msg && msg.msg_ubuf && msg.sg_from_iter)
    ret = msg.sg_from_iter(skb, from, length);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: binding) -> else {
    else if (binding)
...
//
#[no_mangle]
pub unsafe extern "C" fn zerocopy_sg_from_iter(skb: *mut sk_buff, from: *mut iov_iter) -> c_int {
    int zerocopy_sg_from_iter(struct sk_buff *skb, struct iov_iter *from)
    {
    let mut copy: c_int = min_t(int, skb_headlen(skb), iov_iter_count(from));
// copy up to skb headlen
    if (skb_copy_datagram_from_iter(skb, 0, from, copy))
    return -EFAULT;
    return __zerocopy_sg_from_iter(core::ptr::null_mut(), core::ptr::null_mut(), skb, from, ~0U, core::ptr::null_mut());
    }
...
// csum = csum_block_add(*csum, next, progress);
    ...
    {
    __wsum *csum = priv2;
    let mut next: __wsum = csum_partial_copy_nocheck(from + progress, iter_to, len);
// csum = csum_block_add(*csum, next, progress);
    return 0;
    }
#[repr(C)]
...
//
    ...
    __wsum *csump)
    {
    let mut csdata: csum_state = { .csum = *csump };
    int ret;
...
//
    ...
    {
    __wsum csum;
    let mut chunk: c_int = skb.len - hlen;
    if (!chunk)
    return 0;
    ...
    netdev_rx_csum_fault(core::ptr::null_mut(), skb);
    }
    return 0;
    fault:
    return -EFAULT;
...
// Connection-based need to check for termination and startup
    if (connection_based(sk)) {
    let mut state: c_int = READ_ONCE(sk.sk_state);
    if (state == TCP_CLOSE)
    mask |= EPOLLHUP;
...
//
#[no_mangle]
pub unsafe extern "C" fn datagram_poll(file: *mut file, sock: *mut socket, wait: *mut poll_table) -> __poll_t {
