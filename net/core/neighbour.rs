...
pub const NEIGH_DEBUG: c_int = 1;

    do {						\
    if (level <= NEIGH_DEBUG)		\
    pr_debug(fmt, ##__VA_ARGS__);	\
    } while (0)
pub const PNEIGH_HASHMASK: c_uint = 0xF;
...
//
#[no_mangle]
unsafe extern "C" fn neigh_blackhole(neigh: *mut neighbour, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn neigh_cleanup_and_release(neigh: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_mark_dead(n: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_update_gc_list(n: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_update_managed_list(n: *mut neighbour) {
    ...
    list_del_init(&n.managed_list);
#[no_mangle]
pub unsafe extern "C" fn if(!on_managed_list: add_to_managed &&) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_remove_one(n: *mut neighbour) -> bool {
...
#[no_mangle]
unsafe extern "C" fn neigh_forced_gc(tbl: *mut neigh_table) -> c_int {
    static int neigh_forced_gc(struct neigh_table *tbl)
    {
    int max_clean = atomic_read(&tbl.gc_entries) -
    READ_ONCE(tbl.gc_thresh2);
    let mut tmax: u64 = ktime_get_ns() + NSEC_PER_MSEC;
    let mut tref: c_ulong = jiffies - 5 * HZ;
    struct neighbour *n, *tmp;
    let mut shrunk: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn neigh_add_timer(n: *mut neighbour, when: c_ulong) {
...
//
    let mut mint: c_ulong = jiffies - (LONG_MAX - 86400 * HZ);
    neigh_hold(n);
...
#[no_mangle]
unsafe extern "C" fn neigh_del_timer(n: *mut neighbour) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn neigh_parms_qlen_dec(dev: *mut net_device, family: c_int) {
    ...
    skb = skb_peek(list);
    while (skb != core::ptr::null_mut()) {
    struct sk_buff *skb_next = skb_peek_next(skb, list);
    struct net_device *dev = skb.dev;
    if (net == core::ptr::null_mut() || net_eq(dev_net(dev), net)) {
...
#[no_mangle]
unsafe extern "C" fn neigh_flush_one(n: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_flush_table(tbl: *mut neigh_table) {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_changeaddr(tbl: *mut neigh_table, dev: *mut net_device) {
    ...
    pneigh_queue_purge(&tbl.proxy_queue, dev ? dev_net(dev) : core::ptr::null_mut(),
    tbl.family);
...
#[no_mangle]
pub unsafe extern "C" fn neigh_carrier_down(tbl: *mut neigh_table, dev: *mut net_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_ifdown(tbl: *mut neigh_table, dev: *mut net_device) -> c_int {
    ...
    {
    struct neighbour *n = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn neigh_get_hash_rnd(x: *mut u32) {
...
// x = get_random_u32() | 1;
    ...
    {
    let mut size: usize = (1 << shift) * sizeof(struct hlist_head);
...
#[no_mangle]
unsafe extern "C" fn neigh_hash_free_rcu(head: *mut rcu_head) {
...
// Device specific setup.
    ...
    {
    let mut exempt_from_gc: bool = !!(dev.flags & IFF_LOOPBACK);
    return ___neigh_create(tbl, pkey, dev, 0, exempt_from_gc, want_ref);
    }
    EXPORT_SYMBOL(__neigh_create);
#[no_mangle]
unsafe extern "C" fn pneigh_hash(pkey: *const c_void, key_len: c_uint) -> u32 {
    static u32 pneigh_hash(const void *pkey, unsigned int key_len)
    {
    let mut hash_val: u32 = *(u32 *)(pkey + key_len - 4);
...
#[no_mangle]
unsafe extern "C" fn pneigh_destroy(rcu: *mut rcu_head) {
    ...
    (n = rcu_dereference_protected(*np, 1)) != core::ptr::null_mut();
    np = &n.next) {
    ...
    while ((n = rcu_dereference_protected(*np, 1)) != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_parms_put(parms: *mut neigh_parms) {
...
//
#[no_mangle]
pub unsafe extern "C" fn neigh_destroy(neigh: *mut neighbour) {
...
//
#[no_mangle]
unsafe extern "C" fn neigh_suspect(neigh: *mut neighbour) {
...
//
#[no_mangle]
unsafe extern "C" fn neigh_connect(neigh: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_periodic_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn neigh_max_probes(n: *mut neighbour) -> __inline__ int {
...
#[no_mangle]
unsafe extern "C" fn neigh_invalidate(neigh: *mut neighbour) {
...
//
    ...
    (skb = __skb_dequeue(&neigh.arp_queue)) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn neigh_probe(neigh: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_timer_handler(t: *mut timer_list) {
    static void neigh_timer_handler(struct timer_list *t)
    {
    unsigned long now, next;
    struct neighbour *neigh = timer_container_of(neigh, t, timer);
    let mut skip_probe: bool = false;
    unsigned int state;
...
// NUD_PROBE|NUD_INCOMPLETE
    ...
    {
    int rc;
    let mut immediate_probe: bool = false;
    write_lock_bh(&neigh.lock);
    rc = 0;
...
#[no_mangle]
unsafe extern "C" fn neigh_update_hhs(neigh: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_update_process_arp_queue(neigh: *mut neighbour) {
...
// Generic update routine.
    -- lladdr is new lladdr or core::ptr::null_mut(), if it is not supplied.
    -- new    is new state.
    -- flags
...
//
    ...
    {
    let mut gc_update: bool = false, managed_update = false;
    let mut process_arp_queue: bool = false;
    let mut update_isrouter: c_int = 0;
    struct net_device *dev;
    int err, notify = 0;
    u8 old;
...
//
    ...
    {
    return __neigh_update(neigh, lladdr, new, flags, nlmsg_pid, core::ptr::null_mut());
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn __neigh_set_probe_once(neigh: *mut neighbour) {
...
#[no_mangle]
unsafe extern "C" fn neigh_hh_init(n: *mut neighbour) {
    static void neigh_hh_init(struct neighbour *n)
    {
    struct net_device *dev = n.dev;
    let mut prot: __be16 = n.tbl.protocol;
...
// Slow and careful.
#[no_mangle]
pub unsafe extern "C" fn neigh_resolve_output(neigh: *mut neighbour, skb: *mut sk_buff) -> c_int {
    ...
    neigh.ha, core::ptr::null_mut(), skb.len);
...
#[no_mangle]
pub unsafe extern "C" fn neigh_connected_output(neigh: *mut neighbour, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_direct_output(neigh: *mut neighbour, skb: *mut sk_buff) -> c_int {
    int neigh_direct_output(struct neighbour *neigh, struct sk_buff *skb)
    {
    return dev_queue_xmit(skb);
    }
#[no_mangle]
unsafe extern "C" fn neigh_managed_work(work: *mut work_struct) {
    ...
    neigh_event_send_probe(neigh, core::ptr::null_mut(), false);
    queue_delayed_work(system_power_efficient_wq, &tbl.managed_work,
    NEIGH_VAR(&tbl.parms, INTERVAL_PROBE_TIME_MS));
    spin_unlock_bh(&tbl.lock);
    }
#[no_mangle]
unsafe extern "C" fn neigh_proxy_process(t: *mut timer_list) {
    static void neigh_proxy_process(struct timer_list *t)
    {
    struct neigh_table *tbl = timer_container_of(tbl, t, proxy_timer);
    let mut sched_next: c_long = 0;
    let mut now: c_ulong = jiffies;
    struct sk_buff *skb, *n;
    spin_lock(&tbl.proxy_queue.lock);
    skb_queue_walk_safe(&tbl.proxy_queue, skb, n) {
    let mut tdif: c_long = NEIGH_CB(skb).sched_next - now;
    if (tdif <= 0) {
    struct net_device *dev = skb.dev;
...
#[no_mangle]
unsafe extern "C" fn neigh_proxy_delay(p: *mut neigh_parms) -> c_ulong {
...
//
    let mut proxy_delay: c_ulong = NEIGH_VAR(p, PROXY_DELAY);
    return proxy_delay ?
    ...
    struct sk_buff *skb)
    {
    let mut sched_next: c_ulong = neigh_proxy_delay(p);
    ...
    p.sysctl_table = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn neigh_rcu_free_parms(head: *mut rcu_head) {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_parms_release(tbl: *mut neigh_table, parms: *mut neigh_parms) {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_table_init(index: c_int, tbl: *mut neigh_table) {
...
//
#[no_mangle]
pub unsafe extern "C" fn neigh_table_clear(index: c_int, tbl: *mut neigh_table) -> c_int {
    int neigh_table_clear(int index, struct neigh_table *tbl)
    {
    RCU_INIT_POINTER(neigh_tables[index], core::ptr::null_mut());
    synchronize_rcu();
// It is not clean... Fix it to unload IPv6 module safely
    cancel_delayed_work_sync(&tbl.managed_work);
    cancel_delayed_work_sync(&tbl.gc_work);
    timer_delete_sync(&tbl.proxy_timer);
    pneigh_queue_purge(&tbl.proxy_queue, core::ptr::null_mut(), tbl.family);
    neigh_ifdown(tbl, core::ptr::null_mut());
    if (atomic_read(&tbl.entries))
    pr_crit("neighbour leakage\n");
    call_rcu(&rcu_dereference_protected(tbl.nht, 1).rcu,
    neigh_hash_free_rcu);
    tbl.nht = core::ptr::null_mut();
    kfree(tbl.phash_buckets);
    tbl.phash_buckets = core::ptr::null_mut();
    remove_proc_entry(tbl.id, init_net.proc_net_stat);
    free_percpu(tbl.stats);
    tbl.stats = core::ptr::null_mut();
    return 0;
    }
    static struct neigh_table *neigh_find_table(int family)
    {
    struct neigh_table *tbl = core::ptr::null_mut();
    switch (family) {
    case AF_INET:
    ...
    if (neigh == core::ptr::null_mut()) {
    err = -ENOENT;
    goto out;
    }
    err = __neigh_update(neigh, core::ptr::null_mut(), NUD_FAILED,
    ...
    if (tb[NDA_FLAGS_EXT]) {
    let mut ext: u32 = nla_get_u32(tb[NDA_FLAGS_EXT]);
    ...
    lladdr = tb[NDA_LLADDR] ? nla_data(tb[NDA_LLADDR]) : core::ptr::null_mut();
    if (tb[NDA_PROTOCOL])
    ...
    if (neigh == core::ptr::null_mut()) {
    let mut ndm_permanent: bool = ndm.ndm_state & NUD_PERMANENT;
    ...
    if (ndm_flags & NTF_EXT_VALIDATED) {
    let mut state: u8 = ndm.ndm_state;
...
//
    ...
    neigh_event_send(neigh, core::ptr::null_mut());
    neigh_release(neigh);
    out:
    return err;
    }
#[no_mangle]
unsafe extern "C" fn neightbl_fill_parms(skb: *mut sk_buff, parms: *mut neigh_parms) -> c_int {
...
// approximative value for deprecated QUEUE_LEN (in packets)
    ...
    {
    let mut now: c_ulong = jiffies;
    let mut flush_delta: c_long = now - READ_ONCE(tbl.last_flush);
    let mut rand_delta: c_long = now - READ_ONCE(tbl.last_rand);
    struct neigh_hash_table *nht;
    ...
    if (err < 0)
    goto errout;
    if (tb[NDTA_NAME] == core::ptr::null_mut()) {
    err = -EINVAL;
    goto errout;
    }
    ndtmsg = nlmsg_data(nlh);
...
#[no_mangle]
unsafe extern "C" fn neightbl_dump_info(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
    static int neightbl_dump_info(struct sk_buff *skb, struct netlink_callback *cb)
    {
    const struct nlmsghdr *nlh = cb.nlh;
    struct net *net = sock_net(skb.sk);
    int family, tidx, nidx = 0;
    let mut tbl_skip: c_int = cb.args[0];
    let mut neigh_skip: c_int = cb.args[1];
    struct neigh_table *tbl;
    if (cb.strict_check) {
    let mut err: c_int = neightbl_valid_dump_info(nlh, cb.extack);
    if (err < 0)
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn neigh_master_filtered(dev: *mut net_device, master_idx: c_int) -> bool {
    static bool neigh_master_filtered(struct net_device *dev, int master_idx)
    {
    struct net_device *master;
    if (!master_idx)
    return false;
    master = dev ? netdev_master_upper_dev_get_rcu(dev) : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn neigh_ifindex_filtered(dev: *mut net_device, filter_idx: c_int) -> bool {
...
pub struct neigh_dump_filter {
    ...
    struct net *net = sock_net(skb.sk);
    struct neighbour *n;
    let mut err: c_int = 0, h, s_h = cb.args[1];
    ...
    struct pneigh_entry *n;
    struct net *net = sock_net(skb.sk);
    let mut err: c_int = 0, h, s_h = cb.args[3];
...
#[no_mangle]
unsafe extern "C" fn neigh_dump_info(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
    static int neigh_dump_info(struct sk_buff *skb, struct netlink_callback *cb)
    {
    const struct nlmsghdr *nlh = cb.nlh;
    let mut filter: neigh_dump_filter = {};
    struct neigh_table *tbl;
    int t, family, s_t;
    let mut proxy: c_int = 0;
    int err;
    family = ((struct rtgenmsg *)nlmsg_data(nlh)).rtgen_family;
...
//
    ...
    case NDA_DST:
    if (!tb[i]) {
    NL_SET_ERR_ATTR_MISS(extack, core::ptr::null_mut(), NDA_DST);
    return ERR_PTR(-EINVAL);
    }
    break;
    default:
...
#[no_mangle]
pub unsafe extern "C" fn neigh_nlmsg_size() -> usize {
    static inline size_t neigh_nlmsg_size(void)
    {
#[no_mangle]
pub unsafe extern "C" fn NLMSG_ALIGN(ndmsg): sizeof(struct) -> return {
...
#[no_mangle]
pub unsafe extern "C" fn pneigh_nlmsg_size() -> usize {
...
pub unsafe extern "C" fn NLMSG_ALIGN(ndmsg): sizeof(struct) -> return {
    ...
    let mut pid: u32 = NETLINK_CB(in_skb).portid;
    struct nlattr *tb[NDA_MAX + 1];
    struct net_device *dev = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn neigh_for_each(tbl: *mut neigh_table, : *mut *mut void (cb)(struct neighbour, ): *mut c_void, cookie: *mut c_void) {
...
// The tbl->lock must be held as a writer and BH disabled.
    ...
    {
    let mut err: c_int = -EAFNOSUPPORT;
...
#[no_mangle]
pub unsafe extern "C" fn if(NEIGH_LINK_TABLE: index ==) -> else {
    err = dev_hard_header(skb, dev, ntohs(skb.protocol),
    addr, core::ptr::null_mut(), skb.len);
    if (err < 0)
    goto out_kfree_skb;
    err = dev_queue_xmit(skb);
    }
    ...
    tmp = neigh_get_valid(seq, n, core::ptr::null_mut());
    if (tmp)
    return tmp;
    }
    }
    ...
    return *pos ? core::ptr::null_mut() : n;
    }
    ...
    struct pneigh_entry *pn = core::ptr::null_mut();
    int bucket;
    state.flags |= NEIGH_SEQ_IS_PNEIGH;
    ...
    return *pos ? core::ptr::null_mut() : pn;
    }
    static void *neigh_get_idx_any(struct seq_file *seq, loff_t *pos)
    {
    struct neigh_seq_state *state = seq.private;
    void *rc;
    let mut idxpos: loff_t = *pos;
    ...
    rc = neigh_get_next(seq, v, core::ptr::null_mut());
    if (rc)
    goto out;
    ...
    rc = pneigh_get_next(seq, v, core::ptr::null_mut());
    }
    out:
    ++(*pos);
    return rc;
...
#[no_mangle]
pub unsafe extern "C" fn neigh_seq_stop(seq: *mut seq_file, v: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn neigh_stat_seq_stop(seq: *mut seq_file, v: *mut c_void) {
    static void neigh_stat_seq_stop(struct seq_file *seq, void *v)
    {
    }
#[no_mangle]
unsafe extern "C" fn neigh_stat_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn neigh_notify(neigh: *mut neighbour, type: c_int, flags: c_int, pid: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_app_ns(n: *mut neighbour) {
    ...
    let mut unres_qlen_max: static int = INT_MAX / SKB_TRUESIZE(ETH_FRAME_LEN);
    static int proc_unres_qlen(const struct ctl_table *ctl, int write,
    void *buffer, size_t *lenp, loff_t *ppos)
    {
    int size, ret;
    let mut tmp: ctl_table = *ctl;
...
// (int *)ctl->data = size * SKB_TRUESIZE(ETH_FRAME_LEN);
    ...
    {
    struct net_device *dev;
    let mut family: c_int = neigh_parms_family(p);
    rcu_read_lock();
...
#[no_mangle]
unsafe extern "C" fn neigh_proc_update(ctl: *const ctl_table, write: c_int) {
    static void neigh_proc_update(const struct ctl_table *ctl, int write)
    {
    struct net_device *dev = ctl.extra1;
    struct neigh_parms *p = ctl.extra2;
    struct net *net = neigh_parms_net(p);
    let mut index: c_int = (int *) ctl.data - p.data;
    if (!write)
    return;
    set_bit(index, p.data_state);
    if (index == NEIGH_VAR_DELAY_PROBE_TIME)
    call_netevent_notifiers(NETEVENT_DELAY_PROBE_TIME_UPDATE, p);
    if (!dev) /* core::ptr::null_mut() dev means this is default value */
    ...
    let mut tmp: ctl_table = *ctl;
    int ret;
    let mut min: c_int = msecs_to_jiffies(1);
    tmp.extra1 = &min;
    tmp.extra2 = core::ptr::null_mut();
    ...
    {
    let mut ret: c_int = proc_dointvec(ctl, write, buffer, lenp, ppos);
    neigh_proc_update(ctl, write);
    return ret;
    }
    ...
    size_t *lenp, loff_t *ppos)
    {
    let mut ret: c_int = proc_dointvec_jiffies(ctl, write, buffer, lenp, ppos);
    neigh_proc_update(ctl, write);
    return ret;
    }
    ...
    loff_t *ppos)
    {
    let mut ret: c_int = proc_dointvec_userhz_jiffies(ctl, write, buffer, lenp, ppos);
    neigh_proc_update(ctl, write);
    return ret;
    }
    ...
    void *buffer, size_t *lenp, loff_t *ppos)
    {
    let mut ret: c_int = proc_dointvec_ms_jiffies(ctl, write, buffer, lenp, ppos);
    neigh_proc_update(ctl, write);
    return ret;
    }
    ...
    loff_t *ppos)
    {
    let mut ret: c_int = proc_unres_qlen(ctl, write, buffer, lenp, ppos);
    neigh_proc_update(ctl, write);
    return ret;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(ctl->procname, 0: "base_reachable_time_ms") ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn neigh_sysctl_unregister(p: *mut neigh_parms) {
...
#[no_mangle]
unsafe extern "C" fn neigh_init() -> int __init {
