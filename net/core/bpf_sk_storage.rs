...
//! Source: net/core/bpf_sk_storage.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn bpf_sk_storage_del(sk: *mut sock, map: *mut bpf_map) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bpf_sk_storage_map_free(map: *mut bpf_map) {
...
#[no_mangle]
unsafe extern "C" fn bpf_fd_sk_storage_delete_elem(map: *mut bpf_map, key: *mut c_void) -> c_long {
    ...
    copy_selem = bpf_selem_alloc(smap, newsk, core::ptr::null_mut(), false);
    if (!copy_selem)
    return core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn bpf_sk_storage_clone(sk: *const sock, newsk: *mut sock) -> c_int {
    int bpf_sk_storage_clone(const struct sock *sk, struct sock *newsk)
    {
    struct bpf_local_storage *new_sk_storage = core::ptr::null_mut();
    struct bpf_local_storage *sk_storage;
    struct bpf_local_storage_elem *selem;
    let mut ret: c_int = 0;
    rcu_read_lock_dont_migrate();
...
//
    sock_put(sk);
    return IS_ERR(sdata) ?
    (unsigned long)core::ptr::null_mut() : (unsigned long)sdata.data;
    }
    return (unsigned long)core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn bpf_sk_storage_tracing_allowed(prog: *const bpf_prog) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn nla_value_size(value_size: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_sk_storage_diag_free(diag: *mut bpf_sk_storage_diag) {
...
//
    ...
    nla_for_each_nested_type(nla, SK_DIAG_BPF_STORAGE_REQ_MAP_FD,
    nla_stgs, rem) {
    let mut map_fd: c_int = nla_get_u32(nla);
...
// stg_array_type (e.g. INET_DIAG_BPF_SK_STORAGES)
    let mut diag_size: c_uint = nla_total_size(0);
    struct bpf_local_storage *sk_storage;
    struct bpf_local_storage_elem *selem;
    struct bpf_local_storage_map *smap;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bpf_iter_seq_sk_storage_map_info {
    pub map: *mut bpf_map,
    pub bucket_id: c_uint,
    pub skip_elems: unsigned,
}
...
// not found, unlock and go to the next bucket
    ...
    {
    struct bpf_local_storage_elem *selem;
    selem = bpf_sk_storage_map_seq_find_next(seq.private, core::ptr::null_mut());
    if (!selem)
    return core::ptr::null_mut();
    if (*pos == 0)
    ++*pos;
...
pub struct bpf_iter__bpf_sk_storage_map {
    ...
    {
    struct bpf_iter_seq_sk_storage_map_info *info = seq.private;
    let mut ctx: bpf_iter__bpf_sk_storage_map = {};
    struct bpf_local_storage *sk_storage;
    struct bpf_iter_meta meta;
    struct bpf_prog *prog;
    let mut ret: c_int = 0;
    meta.seq = seq;
    prog = bpf_iter_get_info(&meta, selem == core::ptr::null_mut());
    if (prog) {
    ctx.meta = &meta;
    ctx.map = info.map;
    if (selem) {
...
#[no_mangle]
unsafe extern "C" fn bpf_sk_storage_map_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
    static int bpf_sk_storage_map_seq_show(struct seq_file *seq, void *v)
    {
    return __bpf_sk_storage_map_seq_show(seq, v);
    }
#[no_mangle]
unsafe extern "C" fn bpf_sk_storage_map_seq_stop(seq: *mut seq_file, v: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn bpf_iter_fini_sk_storage_map(priv_data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn bpf_sk_storage_map_iter_init() -> int __init {
