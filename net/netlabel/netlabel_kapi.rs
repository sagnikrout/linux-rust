...
//! Source: net/netlabel/netlabel_kapi.c
#![no_std]
...
//
    ...
    {
    if (addr == core::ptr::null_mut() && mask == core::ptr::null_mut()) {
    return netlbl_domhsh_remove(domain, family, audit_info);
    } else if (addr != core::ptr::null_mut() && mask != core::ptr::null_mut()) {
    switch (family) {
    case AF_INET:
...
//
    ...
    struct netlbl_audit *audit_info)
    {
    let mut ret_val: c_int = -ENOMEM;
    struct netlbl_dom_map *entry;
    struct netlbl_domaddr_map *addrmap = core::ptr::null_mut();
    struct netlbl_domaddr4_map *map4 = core::ptr::null_mut();
    struct netlbl_domaddr6_map *map6 = core::ptr::null_mut();
    entry = kzalloc_obj(*entry, GFP_ATOMIC);
    if (entry == core::ptr::null_mut())
    return -ENOMEM;
    if (domain != core::ptr::null_mut()) {
    entry.domain = kstrdup(domain, GFP_ATOMIC);
    if (entry.domain == core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn if(NULL: addr != NULL && mask !=) -> else {
    addrmap = kzalloc_obj(*addrmap, GFP_ATOMIC);
    if (addrmap == core::ptr::null_mut())
    goto cfg_unlbl_map_add_failure;
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_cfg_cipsov4_del(doi: u32, audit_info: *mut netlbl_audit) {
...
//
    ...
    struct netlbl_dom_map *entry;
    struct netlbl_domaddr_map *addrmap = core::ptr::null_mut();
    struct netlbl_domaddr4_map *addrinfo = core::ptr::null_mut();
    doi_def = cipso_v4_doi_getdef(doi);
    if (doi_def == core::ptr::null_mut())
    return -ENOENT;
    entry = kzalloc_obj(*entry, GFP_ATOMIC);
    if (entry == core::ptr::null_mut())
    goto out_entry;
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_cfg_calipso_del(doi: u32, audit_info: *mut netlbl_audit) {
...
//
    ...
    struct netlbl_dom_map *entry;
    struct netlbl_domaddr_map *addrmap = core::ptr::null_mut();
    struct netlbl_domaddr6_map *addrinfo = core::ptr::null_mut();
...
//
pub const _CM_F_NONE: c_uint = 0x00000000;
pub const _CM_F_ALLOC: c_uint = 0x00000001;
pub const _CM_F_WALK: c_uint = 0x00000002;
//
...
//
    ...
    struct netlbl_lsm_catmap *prev = core::ptr::null_mut();
    if (iter == core::ptr::null_mut())
    ...
    if (iter == core::ptr::null_mut() || offset < iter.startbit)
    goto catmap_getnode_walk;
    return iter;
    catmap_getnode_walk:
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_catmap_walk(catmap: *mut netlbl_lsm_catmap, offset: u32) -> c_int {
    ...
    if (iter.next != core::ptr::null_mut()) {
    iter = iter.next;
    idx = 0;
    } else
    return -ENOENT;
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_catmap_walkrng(catmap: *mut netlbl_lsm_catmap, offset: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: bitmask !=) -> else {
    else if (bitmask != 0)
    return iter.startbit +
    (NETLBL_CATMAP_MAPSIZE * idx) + bit - 1;
#[no_mangle]
pub unsafe extern "C" fn if(NETLBL_CATMAP_MAPCNT: ++idx >=) -> else {
    if (iter.next == core::ptr::null_mut())
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_bitmap_setbit(bitmap: *mut c_uchar, bit: u32, state: u8) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_enabled() -> c_int {
...
//
    ...
    if (dom_entry == core::ptr::null_mut()) {
    ret_val = -ENOENT;
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_sock_delattr(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn netlbl_sk_lock_check(sk: *mut sock) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_req_delattr(req: *mut request_sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_skbuff_err(skb: *mut sk_buff, family: u16, error: c_int, gateway: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netlbl_cache_invalidate() {
...
//
#[no_mangle]
unsafe extern "C" fn netlbl_init() -> int __init {
