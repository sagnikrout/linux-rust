...
pub const DIGITAL_NFC_DEP_N_RETRY_NACK: c_int = 2;
pub const DIGITAL_NFC_DEP_N_RETRY_ATN: c_int = 2;
pub const DIGITAL_NFC_DEP_FRAME_DIR_OUT: c_uint = 0xD4;
pub const DIGITAL_NFC_DEP_FRAME_DIR_IN: c_uint = 0xD5;
pub const DIGITAL_NFC_DEP_NFCA_SOD_SB: c_uint = 0xF0;
pub const DIGITAL_CMD_ATR_REQ: c_uint = 0x00;
pub const DIGITAL_CMD_ATR_RES: c_uint = 0x01;
pub const DIGITAL_CMD_PSL_REQ: c_uint = 0x04;
pub const DIGITAL_CMD_PSL_RES: c_uint = 0x05;
pub const DIGITAL_CMD_DEP_REQ: c_uint = 0x06;
pub const DIGITAL_CMD_DEP_RES: c_uint = 0x07;
pub const DIGITAL_ATR_REQ_MIN_SIZE: c_int = 16;
pub const DIGITAL_ATR_REQ_MAX_SIZE: c_int = 64;

pub const DIGITAL_DID_MAX: c_int = 14;
pub const DIGITAL_PAYLOAD_SIZE_MAX: c_int = 254;

pub const DIGITAL_GB_BIT: c_uint = 0x02;

pub const DIGITAL_NFC_DEP_PFB_TIMEOUT_BIT: c_uint = 0x10;
pub const DIGITAL_NFC_DEP_PFB_MI_BIT: c_uint = 0x10;
pub const DIGITAL_NFC_DEP_PFB_NACK_BIT: c_uint = 0x10;
pub const DIGITAL_NFC_DEP_PFB_DID_BIT: c_uint = 0x04;

    ((pfb) & DIGITAL_NFC_DEP_PFB_TIMEOUT_BIT)

pub const DIGITAL_NFC_DEP_RTOX_MAX: c_int = 59;
pub const DIGITAL_NFC_DEP_PFB_I_PDU: c_uint = 0x00;
pub const DIGITAL_NFC_DEP_PFB_ACK_NACK_PDU: c_uint = 0x40;
pub const DIGITAL_NFC_DEP_PFB_SUPERVISOR_PDU: c_uint = 0x80;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct digital_atr_req {
    pub dir: u8,
    pub cmd: u8,
    pub nfcid3: [u8; 10],
    pub did: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct digital_dep_req_res {
    pub dir: u8,
    pub cmd: u8,
    pub pfb: u8,
    pub __packed: },
...
// f(c) = 13560000 Hz
//
pub const DIGITAL_ATR_RES_RWT: c_int = 1337;
...
//
pub const DIGITAL_NFC_DEP_IN_MAX_WT: c_int = 14;
pub const DIGITAL_NFC_DEP_TG_MAX_WT: c_int = 14;
...
#[no_mangle]
unsafe extern "C" fn digital_payload_bits_to_size(payload_bits: u8) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn digital_payload_size_to_bits(payload_size: u8) -> u8 {
...
// (u8 *)skb_push(skb, sizeof(u8)) = DIGITAL_NFC_DEP_NFCA_SOD_SB;
    ...
    ddev.chaining_skb = core::ptr::null_mut();
...
// data_exch),
    ...
    ddev.saved_skb = core::ptr::null_mut();
    }
    return rc;
    }
...
//
    kfree_skb(ddev.saved_skb);
    ddev.saved_skb = core::ptr::null_mut();
    rc = digital_in_send_dep_req(ddev, core::ptr::null_mut(),
    ddev.chaining_skb,
    ddev.data_exch);
    if (rc)
    goto error;
...
#[no_mangle]
unsafe extern "C" fn digital_tg_set_rf_tech(ddev: *mut nfc_digital_dev, rf_tech: u8) {
...
#[no_mangle]
unsafe extern "C" fn digital_tg_send_atn(ddev: *mut nfc_digital_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn digital_tg_send_saved_skb(ddev: *mut nfc_digital_dev) -> c_int {
    ...
    digital_tg_recv_dep_req, core::ptr::null_mut());
    if (rc)
    kfree_skb(ddev.saved_skb);
    return rc;
    }
...
//
    ...
    ddev.saved_skb = core::ptr::null_mut();
    resp = digital_recv_dep_data_gather(ddev, pfb, resp,
    digital_tg_send_ack, core::ptr::null_mut());
    if (IS_ERR(resp)) {
    rc = PTR_ERR(resp);
    resp = core::ptr::null_mut();
    goto exit;
...
#[no_mangle]
pub unsafe extern "C" fn digital_tg_send_dep_res(ddev: *mut nfc_digital_dev, skb: *mut sk_buff) -> c_int {
    ...
    DIGITAL_NFC_DEP_PFB_PNI(ddev.curr_nfc_dep_pni + 1);
    chaining_skb = ddev.chaining_skb;
    tmp_skb = digital_send_dep_data_prep(ddev, skb, dep_res, core::ptr::null_mut());
    if (IS_ERR(tmp_skb))
    return PTR_ERR(tmp_skb);
    digital_skb_push_dep_sod(ddev, tmp_skb);
    ddev.skb_add_crc(tmp_skb);
    ...
    {
    let mut rf_tech: u8 = (unsigned long)arg;
    if (IS_ERR(resp))
    return;
    digital_tg_set_rf_tech(ddev, rf_tech);
    digital_tg_configure_hw(ddev, NFC_DIGITAL_CONFIG_RF_TECH, rf_tech);
    digital_tg_listen(ddev, 1500, digital_tg_recv_dep_req, core::ptr::null_mut());
    dev_kfree_skb(resp);
    }
    ...
    digital_tg_send_atr_res_complete, core::ptr::null_mut());
    if (rc)
    kfree_skb(skb);
    return rc;
    }
