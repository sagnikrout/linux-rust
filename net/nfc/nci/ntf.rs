...
// complete the data exchange transaction, if exists
    ...
    nci_data_exchange_complete(ndev, core::ptr::null_mut(), ntf.conn_id, -EIO);
    return 0;
    }
    static const __u8 *
...
#[no_mangle]
unsafe extern "C" fn nci_get_prop_rf_protocol(ndev: *mut nci_dev, rf_protocol: __u8) -> __u32 {
...
#[no_mangle]
pub unsafe extern "C" fn if(NCI_RF_PROTOCOL_T2T: rf_protocol ==) -> else {
    else if (rf_protocol == NCI_RF_PROTOCOL_T2T)
    protocol = NFC_PROTO_MIFARE_MASK;
#[no_mangle]
pub unsafe extern "C" fn if(NCI_RF_PROTOCOL_ISO_DEP: rf_protocol ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(NCI_RF_PROTOCOL_T3T: rf_protocol ==) -> else {
    else if (rf_protocol == NCI_RF_PROTOCOL_T3T)
    protocol = NFC_PROTO_FELICA_MASK;
#[no_mangle]
pub unsafe extern "C" fn if(NCI_RF_PROTOCOL_NFC_DEP: rf_protocol ==) -> else {
    else if (rf_protocol == NCI_RF_PROTOCOL_NFC_DEP)
    protocol = NFC_PROTO_NFC_DEP_MASK;
#[no_mangle]
pub unsafe extern "C" fn if(NCI_RF_PROTOCOL_T5T: rf_protocol ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn nci_clear_target_list(ndev: *mut nci_dev) {
    ...
    const struct sk_buff *skb)
    {
    let mut ntf: nci_rf_discover_ntf = {};
    const __u8 *data;
    let mut add_target: bool = true;
...
// This is a new target, check if we've enough room
    ...
    {
    struct nci_conn_info *conn_info;
    let mut ntf: nci_rf_intf_activated_ntf = {};
    const __u8 *data;
    let mut err: c_int = NCI_STATUS_OK;
...
// complete the data exchange transaction, if exists
    if (test_bit(NCI_DATA_EXCHANGE, &ndev.flags))
    nci_data_exchange_complete(ndev, core::ptr::null_mut(), NCI_STATIC_RF_CONN_ID,
    -EIO);
    switch (ntf.type) {
    ...
    const struct sk_buff *skb)
    {
    let mut status: u8 = NCI_STATUS_OK;
...
#[no_mangle]
pub unsafe extern "C" fn nci_ntf_packet(ndev: *mut nci_dev, skb: *mut sk_buff) {
    void nci_ntf_packet(struct nci_dev *ndev, struct sk_buff *skb)
    {
    let mut ntf_opcode: __u16 = nci_opcode(skb.data);
