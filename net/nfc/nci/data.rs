...
#[no_mangle]
pub unsafe extern "C" fn nci_conn_max_data_pkt_payload_size(ndev: *mut nci_dev, conn_id: __u8) -> c_int {
    ...
    struct sk_buff *skb) {
    const struct nci_conn_info *conn_info;
    let mut total_len: c_int = skb.len;
...
// queue all fragments atomically
    ...
    while ((skb_frag = __skb_dequeue(&frags_q)) != core::ptr::null_mut())
...
// Send NCI data
#[no_mangle]
pub unsafe extern "C" fn nci_send_data(ndev: *mut nci_dev, conn_id: __u8, skb: *mut sk_buff) -> c_int {
...
// Rx Data packet
#[no_mangle]
pub unsafe extern "C" fn nci_rx_data_packet(ndev: *mut nci_dev, skb: *mut sk_buff) {
    void nci_rx_data_packet(struct nci_dev *ndev, struct sk_buff *skb)
    {
    let mut pbf: __u8 = nci_pbf(skb.data);
    let mut status: __u8 = 0;
    let mut conn_id: __u8 = nci_conn_id(skb.data);
    const struct nci_conn_info *conn_info;
    pr_debug("len %d\n", skb.len);
