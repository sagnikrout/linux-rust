...
//! Source: net/rxrpc/tests/rxrpc_kunit.c
#![no_std]
...
pub struct fcrypt_pcbc_testvec {
    ...
    pub /: *const *const *const u8 ptext; / plaintext,
    pub /: *const *const *const u8 ctext; / ciphertext,
    pub /: *mut *mut size_t nblocks; / length of ptext and ctext in blocks,
}
...
#[no_mangle]
unsafe extern "C" fn test_fcrypt_pcbc(test: *mut kunit) {
    ...
    let mut nblocks: usize = tv.nblocks;
    let mut len: usize = nblocks * FCRYPT_BSIZE;
...
#[no_mangle]
unsafe extern "C" fn test_des_pcbc(test: *mut kunit) {
