...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_congestion_degrade(call: *mut rxrpc_call) {
...
//
    ...
    int ix)
    {
    let mut xmit_ts: ktime_t = ktime_add_us(tq.xmit_ts_base, tq.segment_xmit_ts[ix]);
...
//
    ...
    let mut seq: rxrpc_seq_t = call.tx_bottom + 1;
    let mut rot_last: bool = false, trace = false;
...
//
    ...
    let mut ix: c_uint = seq - call.tx_qbase;
...
// Soft NAK -> hard ACK.
    ...
    trace_rxrpc_tq(call, tq, seq, rxrpc_tq_rotate_and_free);
    kfree(tq);
    call.tx_queue = core::ptr::null_mut();
    }
    }
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_receiving_reply(call: *mut rxrpc_call) -> bool {
    static bool rxrpc_receiving_reply(struct rxrpc_call *call)
    {
    let mut summary: rxrpc_ack_summary = { 0 };
    let mut top: rxrpc_seq_t = READ_ONCE(call.tx_top);
    if (call.ackr_reason) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_end_rx_phase(call: *mut rxrpc_call, serial: rxrpc_serial_t) {
    static void rxrpc_end_rx_phase(struct rxrpc_call *call, rxrpc_serial_t serial)
    {
    let mut whigh: rxrpc_seq_t = READ_ONCE(call.rx_highest_seq);
...
//
    ...
    let mut last: bool = sp.hdr.flags & RXRPC_LAST_PACKET;
...
//
    ...
    struct rxrpc_skb_priv *sp = rxrpc_skb(skb);
    struct sk_buff *oos;
    let mut serial: rxrpc_serial_t = sp.hdr.serial;
    let mut sack: c_uint = call.ackr_sack_base;
    let mut window: rxrpc_seq_t = call.ackr_window;
    let mut wtop: rxrpc_seq_t = call.ackr_wtop;
    let mut wlimit: rxrpc_seq_t = window + call.rx_winsize - 1;
    let mut seq: rxrpc_seq_t = sp.hdr.seq;
    let mut last: bool = sp.hdr.flags & RXRPC_LAST_PACKET;
    let mut ack_reason: c_int = -1;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !skb_queue_empty(&call->rx_oos_queue)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_input_split_jumbo(call: *mut rxrpc_call, skb: *mut sk_buff) -> bool {
    static bool rxrpc_input_split_jumbo(struct rxrpc_call *call, struct sk_buff *skb)
    {
    struct rxrpc_jumbo_header jhdr;
    struct rxrpc_skb_priv *sp = rxrpc_skb(skb), *jsp;
    struct sk_buff *jskb;
    let mut ack_serial: rxrpc_serial_t = 0;
    let mut offset: c_uint = sizeof(struct rxrpc_wire_header);
    let mut len: c_uint = skb.len - offset;
    let mut notify: bool = false;
    let mut ack_reason: c_int = 0, count = 1, stat_ix;
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_input_data(call: *mut rxrpc_call, skb: *mut sk_buff) {
    static void rxrpc_input_data(struct rxrpc_call *call, struct sk_buff *skb)
    {
    struct rxrpc_skb_priv *sp = rxrpc_skb(skb);
    let mut serial: rxrpc_serial_t = sp.hdr.serial;
    let mut seq0: rxrpc_seq_t = sp.hdr.seq;
    _enter("{%x,%x,%x},{%u,%x}",
...
//
    ...
    break;
    case RXRPC_CALL_SERVER_RECV_REQUEST: {
    let mut timo: c_ulong = READ_ONCE(call.next_req_timo);
    if (timo) {
    let mut delay: ktime_t = ms_to_ktime(timo);
...
//
    ...
    let mut max_mtu: u32 = ntohl(trailer.maxMTU);
// u32 if_mtu	= ntohl(trailer->ifMTU);
    let mut rwind: u32 = ntohl(trailer.rwind);
    let mut jumbo_max: u32 = ntohl(trailer.jumbo_max);
...
//
    ...
    {
    let mut old_reported: c_ulong = 0, flipped, new_acks = 0;
...
// See how many new ACKs/NAKs have been acquired.
    ...
    let mut lowest: rxrpc_seq_t = seq + ffz(extracted_acks);
...
//
    ...
    struct rxrpc_skb_priv *sp = rxrpc_skb(skb);
    struct rxrpc_txqueue *tq = call.tx_queue;
    let mut extracted: c_ulong = ~0UL;
    let mut nr: c_uint = 0, nsack;
    let mut seq: rxrpc_seq_t = call.acks_hard_ack + 1;
    let mut lowest_nak: rxrpc_seq_t = seq + sp.ack.nr_acks;
...
// Decant ACKs until we hit a txqueue boundary.
    ...
    if (nr) {
    let mut nr_reported: c_uint = seq & RXRPC_TXQ_MASK;
...
//
    ...
    {
    let mut base: rxrpc_seq_t = READ_ONCE(call.acks_hard_ack);
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_input_ack(call: *mut rxrpc_call, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_REQUEST_ACK: sp->hdr.flags &) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_input_ackall(call: *mut rxrpc_call, skb: *mut sk_buff) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_input_abort(call: *mut rxrpc_call, skb: *mut sk_buff) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_input_call_packet(call: *mut rxrpc_call, skb: *mut sk_buff) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_implicit_end_call(call: *mut rxrpc_call, skb: *mut sk_buff) {
