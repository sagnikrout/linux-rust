...
#[no_mangle]
pub unsafe extern "C" fn do_udp_sendmsg(socket: *mut socket, msg: *mut msghdr, len: usize) -> isize {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_tx_backoff(call: *mut rxrpc_call, ret: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_set_keepalive(call: *mut rxrpc_call, now: ktime_t) {
    static void rxrpc_set_keepalive(struct rxrpc_call *call, ktime_t now)
    {
    let mut delay: ktime_t = ms_to_ktime(READ_ONCE(call.next_rx_timo) / 6);
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_alloc_ack(call: *mut rxrpc_call, sack_size: usize) -> c_int {
    ...
    let mut gfp: gfp_t = rcu_read_lock_held() ? GFP_ATOMIC | __GFP_NOWARN : GFP_NOFS;
    void *buf, *buf2 = core::ptr::null_mut();
    u8 *filler;
...
#[no_mangle]
unsafe extern "C" fn rxrpc_free_ack(call: *mut rxrpc_call) {
...
//
    ...
    {
    let mut avail: c_ulong = call.rtt_avail;
    let mut rtt_slot: c_int = 9;
...
// Extend a path MTU probe ACK.
    if (why == rxrpc_propose_ack_ping_for_mtu_probe) {
    let mut probe_mtu: usize = call.peer.pmtud_trial + sizeof(struct rxrpc_wire_header);
    if (len > probe_mtu)
    goto skip;
    while (len < probe_mtu) {
    let mut part: usize = umin(probe_mtu - len, PAGE_SIZE);
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_send_probe_for_pmtud(call: *mut rxrpc_call) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_send_abort_packet(call: *mut rxrpc_call) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_and_clear_bit(RXRPC_CALL_EV_ACK_LOST, _arg: &call->events)) -> else {
    else if (test_and_clear_bit(RXRPC_CALL_EV_ACK_LOST, &call.events))
    why = rxrpc_reqack_ack_lost;
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_TXBUF_RESENT: txb->flags &) -> else {
    else if (txb.flags & RXRPC_TXBUF_RESENT)
    why = rxrpc_reqack_retrans;
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_MIN_CWND: call->cong_ca_state == RXRPC_CA_SLOW_START && call->cong_cwnd <=) -> else {
    else if (call.cong_ca_state == RXRPC_CA_SLOW_START && call.cong_cwnd <= RXRPC_MIN_CWND)
    why = rxrpc_reqack_slow_start;
#[no_mangle]
pub unsafe extern "C" fn if(2: call->tx_winsize <=) -> else {
    else if (call.tx_winsize <= 2)
    why = rxrpc_reqack_small_txwin;
#[no_mangle]
pub unsafe extern "C" fn if(3: call->rtt_count <) -> else {
    else if (call.rtt_count < 3)
    why = rxrpc_reqack_more_rtt;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ktime_before(ktime_add_ms(call->rtt_last_req, _arg: 1000), _arg: ktime_get_real())) -> else {
    else if (ktime_before(ktime_add_ms(call.rtt_last_req, 1000), ktime_get_real()))
    why = rxrpc_reqack_old_rtt;
#[no_mangle]
pub unsafe extern "C" fn if(!after(READ_ONCE(call->send_top): !last &&, _arg: txb->seq)) -> else {
    else if (!last && !after(READ_ONCE(call.send_top), txb.seq))
    why = rxrpc_reqack_app_stall;
#[no_mangle]
pub unsafe extern "C" fn if(req->n): *mut *mut *mut call->tx_winsize <= (2  req->n) || call->cong_cwnd <= (2) -> else {
...
//
    ...
    let mut seq: rxrpc_seq_t = req.seq;
    let mut len: usize = 0;
    let mut start_tlp: bool = false;
...
// Each transmission of a Tx packet needs a new serial number
    ...
    prefetch(tq.next);
    for (int i = 0;;) {
    let mut ix: c_int = seq & RXRPC_TXQ_MASK;
...
// The first packet may take longer to elicit a response.
    ...
    let mut delay: ktime_t = ms_to_ktime(READ_ONCE(call.next_rx_timo));
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_send_data_packet(call: *mut rxrpc_call, req: *mut rxrpc_send_data_req) {
    ...
    struct msghdr msg;
    let mut seq: rxrpc_seq_t = req.seq;
    let mut len: usize = sizeof(*whdr);
    let mut new_call: bool = test_bit(RXRPC_CALL_BEGAN_RX_TIMER, &call.flags);
    int ret, stat_ix;
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_send_conn_abort(conn: *mut rxrpc_connection) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_reject_packet(local: *mut rxrpc_local, skb: *mut sk_buff) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_send_keepalive(peer: *mut rxrpc_peer) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_send_response(conn: *mut rxrpc_connection, response: *mut sk_buff) {
