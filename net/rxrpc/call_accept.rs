...
//! Source: net/rxrpc/call_accept.c
#![no_std]
...
//
    ...
    int max, tmp;
    let mut size: c_uint = RXRPC_BACKLOG_MAX;
...
// Check the user ID isn't already in use
    ...
    pp = &(*pp).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(xcall->user_call_ID: user_call_ID >) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_service_prealloc(rx: *mut rxrpc_sock, gfp: gfp_t) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_discard_prealloc(rx: *mut rxrpc_sock) {
    void rxrpc_discard_prealloc(struct rxrpc_sock *rx)
    {
    struct rxrpc_backlog *b = rx.backlog;
    struct rxrpc_net *rxnet = rxrpc_net(sock_net(&rx.sk));
    let mut size: c_uint = RXRPC_BACKLOG_MAX, head, tail;
    if (!b)
    return;
...
// #calls >= #conns >= #peers must hold true.
    ...
    peer.srx = *peer_srx;
    b.peer_backlog[peer_tail] = core::ptr::null_mut();
...
// Now allocate and set up the connection
    ...
    b.conn_backlog[conn_tail] = core::ptr::null_mut();
...
// And now we can allocate and set up a new call
    ...
    b.call_backlog[call_tail] = core::ptr::null_mut();
...
//
    ...
    struct sk_buff *skb)
    {
    const struct rxrpc_security *sec = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_user_charge_accept(rx: *mut rxrpc_sock, user_call_ID: c_ulong) -> c_int {
    int rxrpc_user_charge_accept(struct rxrpc_sock *rx, unsigned long user_call_ID)
    {
    struct rxrpc_backlog *b = rx.backlog;
    if (rx.sk.sk_state == RXRPC_CLOSE)
    return -ESHUTDOWN;
    return rxrpc_service_prealloc_one(rx, b, core::ptr::null_mut(), user_call_ID, GFP_KERNEL,
