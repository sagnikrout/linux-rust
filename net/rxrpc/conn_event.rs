...
//
    ...
    {
    let mut cid: u32 = conn.proto.cid, call = 0, seq = 0;
    if (skb) {
...
//
    ...
    {
    struct rxrpc_skb_priv *sp = skb ? rxrpc_skb(skb) : core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_abort_calls(conn: *mut rxrpc_connection) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_call_is_secure(call: *mut rxrpc_call) {
    ...
    struct sk_buff *skb)
    {
    let mut len: c_uint = skb.len - sizeof(struct rxrpc_wire_header);
    void *buffer;
    int ret;
...
//
    ...
    sp.chall.conn = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_secure_connection(conn: *mut rxrpc_connection) {
    static void rxrpc_secure_connection(struct rxrpc_connection *conn)
    {
    if (conn.security.issue_challenge(conn) < 0)
    rxrpc_abort_conn(conn, core::ptr::null_mut(), RX_CALL_DEAD, -ENOMEM,
    rxrpc_abort_nomem);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_process_delayed_final_acks(conn: *mut rxrpc_connection, force: bool) {
    void rxrpc_process_delayed_final_acks(struct rxrpc_connection *conn, bool force)
    {
    let mut j: c_ulong = jiffies, next_j;
    unsigned int channel;
    bool set;
    again:
    next_j = j + LONG_MAX;
    ...
    &conn.flags))
    rxrpc_conn_retransmit_call(conn, core::ptr::null_mut(), channel);
    }
    j = jiffies;
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_do_process_connection(conn: *mut rxrpc_connection) {
...
#[no_mangle]
pub unsafe extern "C" fn rxrpc_process_connection(work: *mut work_struct) {
...
//
    ...
    {
    struct rxrpc_skb_priv *sp = rxrpc_skb(skb);
    struct rxrpc_call *call = core::ptr::null_mut();
    struct rxrpc_sock *rx;
    let mut respond: bool = false, queued = false;
    sp.chall.conn =
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_input_conn_packet(conn: *mut rxrpc_connection, skb: *mut sk_buff) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_input_conn_event(conn: *mut rxrpc_connection, skb: *mut sk_buff) {
    ...
    conn.tx_response = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_post_response(conn: *mut rxrpc_connection, skb: *mut sk_buff) {
