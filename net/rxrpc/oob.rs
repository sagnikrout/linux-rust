...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rxrpc_oob_params {
    pub /: *mut *mut u64 oob_id; / ID number of message if reply,
    pub abort_code: i32,
    pub command: enum rxrpc_oob_command,
    pub have_oob_id:1: bool,
}
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_notify_socket_oob(call: *mut rxrpc_call, skb: *mut sk_buff) -> bool {
...
//
    ...
    p = p.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(skb->skb_mstamp_ns: oob_id >) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_add_pending_oob(rx: *mut rxrpc_sock, skb: *mut sk_buff) {
    void rxrpc_add_pending_oob(struct rxrpc_sock *rx, struct sk_buff *skb)
    {
    struct rb_node **pp = &rx.pending_oobq.rb_node, *p = core::ptr::null_mut();
    while (*pp) {
    p = *pp;
    pp = &(*pp).rb_right;
    }
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_sendmsg_oob_cmsg(msg: *mut msghdr, p: *mut rxrpc_oob_params) -> c_int {
...
//
    ...
    break;
    if (p.abort_code) {
    rxrpc_abort_conn(conn, core::ptr::null_mut(), p.abort_code, -ECONNABORTED,
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_sendmsg_oob(rx: *mut rxrpc_sock, msg: *mut msghdr, len: usize) -> c_int {
    int rxrpc_sendmsg_oob(struct rxrpc_sock *rx, struct msghdr *msg, size_t len)
    {
    let mut p: rxrpc_oob_params = {};
    int ret;
    _enter("");
...
//
    ...
    let mut type: enum rxrpc_oob_type = oob.mark;
    switch (type) {
    case RXRPC_OOB_CHALLENGE:
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_kernel_free_oob(oob: *mut sk_buff) {
...
//
    ...
    rxrpc_abort_conn(sp.chall.conn, core::ptr::null_mut(), abort_code, error, why);
    return error;
    }
