...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_notify_socket(call: *mut rxrpc_call) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_recvmsg_term(call: *mut rxrpc_call, msg: *mut msghdr) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_rotate_rx_window(call: *mut rxrpc_call) {
    static void rxrpc_rotate_rx_window(struct rxrpc_call *call)
    {
    struct rxrpc_skb_priv *sp;
    struct sk_buff *skb;
    rxrpc_serial_t serial;
    let mut old_consumed: rxrpc_seq_t = call.rx_consumed, tseq;
    bool last;
    int acked;
    _enter("%d", call.debug_id);
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_verify_data(call: *mut rxrpc_call, skb: *mut sk_buff) -> c_int {
...
//
    let mut size: usize = clamp(round_up(sp.len, 32), 2048, 65535);
    ...
    call.rx_dec_buffer = core::ptr::null_mut();
    call.rx_dec_bsize = 0;
    call.rx_dec_offset = 0;
    call.rx_dec_len = 0;
    return ret;
...
//
    ...
    let mut id32: c_uint = call.user_call_ID;
    return put_cmsg(msg, SOL_RXRPC, RXRPC_USER_CALL_ID,
    sizeof(unsigned int), &id32);
    } else {
    let mut idl: c_ulong = call.user_call_ID;
    return put_cmsg(msg, SOL_RXRPC, RXRPC_USER_CALL_ID,
...
//
    ...
    struct sk_buff *skb;
    let mut need_response: bool = false;
    int ret;
...
//
    ...
    struct rxrpc_sock *rx = rxrpc_sk(sock.sk);
    struct list_head *l;
    let mut call_debug_id: c_uint = 0;
...
//
    ...
    let mut len: usize = sizeof(call.dest_srx);
...
//
    ...
    ret = rxrpc_recvmsg_data(sock, call, core::ptr::null_mut(), iter, *_len, 0, &offset);
// _len -= offset;
    if (ret == -EIO)
    goto call_failed;
    if (ret < 0)
