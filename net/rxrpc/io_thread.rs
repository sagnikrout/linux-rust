...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_encap_rcv(udp_sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rxrpc_bad_message(skb: *mut sk_buff, why: enum rxrpc_abort_reason) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_input_version(local: *mut rxrpc_local, skb: *mut sk_buff) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_extract_abort(skb: *mut sk_buff) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_input_packet(local: *mut rxrpc_local, skb: *mut sk_buff) -> bool {
...
//
    ...
    if (!peer) {
    rcu_read_unlock();
    return rxrpc_new_incoming_call(local, core::ptr::null_mut(), core::ptr::null_mut(), &peer_srx, skb);
    }
    ...
    rcu_read_unlock();
    ret = rxrpc_new_incoming_call(local, peer, core::ptr::null_mut(), &peer_srx, skb);
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_io_thread(data: *mut c_void) -> c_int {
...
// Deal with connections that want immediate attention.
    ...
    spin_unlock_irq(&local.lock);
    rxrpc_input_conn_event(conn, core::ptr::null_mut());
...
// Deal with calls that want immediate attention.
    ...
    let mut delay_ns: i64 = ktime_to_ns(ktime_sub(tstamp, now));
    if (delay_ns <= 0) {
    __set_current_state(TASK_RUNNING);
    continue;
    }
    ...
    WRITE_ONCE(local.io_thread, core::ptr::null_mut());
