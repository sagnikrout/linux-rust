...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_adjust_mtu(peer: *mut rxrpc_peer, mtu: c_uint) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_input_error(local: *mut rxrpc_local, skb: *mut sk_buff) {
    void rxrpc_input_error(struct rxrpc_local *local, struct sk_buff *skb)
    {
    struct sock_exterr_skb *serr = SKB_EXT_ERR(skb);
    struct sockaddr_rxrpc srx;
    struct rxrpc_peer *peer = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_store_error(peer: *mut rxrpc_peer, skb: *mut sk_buff) {
    static void rxrpc_store_error(struct rxrpc_peer *peer, struct sk_buff *skb)
    {
    let mut compl: enum rxrpc_call_completion = RXRPC_CALL_NETWORK_ERROR;
    struct sock_exterr_skb *serr = SKB_EXT_ERR(skb);
    struct sock_extended_err *ee = &serr.ee;
    let mut err: c_int = ee.ee_errno;
    _enter("");
    switch (ee.ee_origin) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_peer_get_tx_mark(peer: *const rxrpc_peer, base: time64_t) -> time64_t {
    static time64_t rxrpc_peer_get_tx_mark(const struct rxrpc_peer *peer, time64_t base)
    {
    let mut last_tx_at: i32 = READ_ONCE(peer.last_tx_at);
    let mut base_lsw: i32 = base;
    let mut diff: i32 = last_tx_at - base_lsw;
...
//
    ...
    let mut mask: u8 = ARRAY_SIZE(rxnet.peer_keepalive) - 1;
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_peer_keepalive_worker(work: *mut work_struct) {
...
//
    ...
    let mut max_data: c_uint = peer.max_data;
...
// It did get through.
    ...
    trial = RXRPC_JUMBO(2);
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_JUMBO(4): good < RXRPC_JUMBO(4) && bad >) -> else {
    else if (good < RXRPC_JUMBO(4) && bad > RXRPC_JUMBO(4))
    trial = RXRPC_JUMBO(4);
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_JUMBO(3): good < RXRPC_JUMBO(3) && bad >) -> else {
    else if (good < RXRPC_JUMBO(3) && bad > RXRPC_JUMBO(3))
    trial = RXRPC_JUMBO(3);
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_JUMBO(6): good < RXRPC_JUMBO(6) && bad >) -> else {
    else if (good < RXRPC_JUMBO(6) && bad > RXRPC_JUMBO(6))
    trial = RXRPC_JUMBO(6);
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_JUMBO(5): good < RXRPC_JUMBO(5) && bad >) -> else {
    else if (good < RXRPC_JUMBO(5) && bad > RXRPC_JUMBO(5))
    trial = RXRPC_JUMBO(5);
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_JUMBO(8): good < RXRPC_JUMBO(8) && bad >) -> else {
    else if (good < RXRPC_JUMBO(8) && bad > RXRPC_JUMBO(8))
    trial = RXRPC_JUMBO(8);
#[no_mangle]
pub unsafe extern "C" fn if(RXRPC_JUMBO(7): good < RXRPC_JUMBO(7) && bad >) -> else {
