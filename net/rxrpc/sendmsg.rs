...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_wait_to_be_connected(call: *mut rxrpc_call, timeo: *mut c_long) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_check_tx_space(call: *mut rxrpc_call, _tx_win: *mut rxrpc_seq_t) -> bool {
    static bool rxrpc_check_tx_space(struct rxrpc_call *call, rxrpc_seq_t *_tx_win)
    {
    let mut tx_bottom: rxrpc_seq_t = READ_ONCE(call.tx_bottom);
    if (_tx_win)
// _tx_win = tx_bottom;
...
//
    ...
    for (;;) {
    set_current_state(TASK_INTERRUPTIBLE);
    if (rxrpc_check_tx_space(call, core::ptr::null_mut()))
    return 0;
...
//
    ...
    let mut seq: rxrpc_seq_t = txb.seq;
...
// Order send_top after the queue->next pointer and txb content.
    ...
    call.send_queue = core::ptr::null_mut();
    }
    if (poke)
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_alloc_txqueue(sk: *mut sock, call: *mut rxrpc_call) -> c_int {
...
//
    ...
    long timeo;
    let mut more: bool = msg.msg_flags & MSG_MORE;
    int ret, copied = 0;
...
// this should be in poll
    ...
    reload:
    txb = call.tx_pending;
    call.tx_pending = core::ptr::null_mut();
    if (txb)
...
// append next segment of data to the current buffer
    ...
    let mut copy: usize = umin(txb.space, msg_data_left(msg));
    _debug("add %zu", copy);
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_sendmsg_cmsg(msg: *mut msghdr, p: *mut rxrpc_send_params) -> c_int {
    static int rxrpc_sendmsg_cmsg(struct msghdr *msg, struct rxrpc_send_params *p)
    {
    struct cmsghdr *cmsg;
    let mut got_user_ID: bool = false;
    int len;
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_do_sendmsg(rx: *mut rxrpc_sock, msg: *mut msghdr, len: usize) -> c_int {
    int rxrpc_do_sendmsg(struct rxrpc_sock *rx, struct msghdr *msg, size_t len)
    {
    struct rxrpc_call *call;
    let mut dropped_lock: bool = false;
    int ret;
    struct rxrpc_send_params p = {
    .call.tx_total_len	= -1,
    .call.user_call_ID	= 0,
...
// ... and we have the call lock.
    ...
    let mut delay: ktime_t = ms_to_ktime(p.call.timeouts.hard * MSEC_PER_SEC);
...
// it's too late for this call
    ...
    ret = rxrpc_send_data(rx, call, msg, len, core::ptr::null_mut(), &dropped_lock);
    break;
    default:
    ret = -EINVAL;
    break;
...
//
    ...
    int ret;
    _enter("{%d},", call.debug_id);
    ASSERTCMP(msg.msg_name, ==, core::ptr::null_mut());
    ASSERTCMP(msg.msg_control, ==, core::ptr::null_mut());
    mutex_lock(&call.user_mutex);
