...
//
    ...
    let mut rxrpc_reap_client_connections: __read_mostly unsigned int = 900;
    let mut rxrpc_conn_idle_client_expiry: __read_mostly unsigned long = 2 * 60 * HZ;
    let mut rxrpc_conn_idle_client_fast_expiry: __read_mostly unsigned long = 2 * HZ;
#[no_mangle]
unsafe extern "C" fn rxrpc_activate_bundle(bundle: *mut rxrpc_bundle) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_destroy_client_conn_ids(local: *mut rxrpc_local) {
...
#[no_mangle]
unsafe extern "C" fn rxrpc_free_bundle(bundle: *mut rxrpc_bundle) {
...
#[no_mangle]
pub unsafe extern "C" fn rxrpc_put_bundle(bundle: *mut rxrpc_bundle, why: enum rxrpc_bundle_trace) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_purge_client_connections(local: *mut rxrpc_local) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_may_reuse_conn(conn: *mut rxrpc_connection) -> bool {
...
pub unsafe extern "C" fn rxrpc_look_up_bundle(call: *mut rxrpc_call, gfp: gfp_t) -> c_int {
    ...
    long diff;
    let mut upgrade: bool = test_bit(RXRPC_CALL_UPGRADE, &call.flags);
    _enter("{%px,%x,%u,%u}",
...
//
    ...
    let mut shift: c_uint = slot * RXRPC_MAXCALLS;
    unsigned int i;
    old = bundle.conns[slot];
    if (old) {
    bundle.conns[slot] = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_bundle_has_space(bundle: *mut rxrpc_bundle) -> bool {
    static bool rxrpc_bundle_has_space(struct rxrpc_bundle *bundle)
    {
    let mut slot: c_int = -1, i, usable;
    _enter("");
...
//
    ...
    let mut call_id: u32 = chan.call_counter + 1;
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_unidle_conn(conn: *mut rxrpc_connection) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_activate_channels(bundle: *mut rxrpc_bundle) {
    static void rxrpc_activate_channels(struct rxrpc_bundle *bundle)
    {
    struct rxrpc_connection *conn;
    unsigned long avail, mask;
    unsigned int channel, slot;
    trace_rxrpc_client(core::ptr::null_mut(), -1, rxrpc_client_activate_chans);
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_connect_client_calls(local: *mut rxrpc_local) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_expose_client_call(call: *mut rxrpc_call) {
    void rxrpc_expose_client_call(struct rxrpc_call *call)
    {
    let mut channel: c_uint = call.cid & RXRPC_CHANNELMASK;
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_set_client_reap_timer(local: *mut rxrpc_local) {
    static void rxrpc_set_client_reap_timer(struct rxrpc_local *local)
    {
    if (!local.kill_all_client_conns) {
    let mut now: c_ulong = jiffies;
    let mut reap_at: c_ulong = now + rxrpc_conn_idle_client_expiry;
    if (local.rxnet.live)
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_disconnect_client_call(bundle: *mut rxrpc_bundle, call: *mut rxrpc_call) {
    void rxrpc_disconnect_client_call(struct rxrpc_bundle *bundle, struct rxrpc_call *call)
    {
    struct rxrpc_connection *conn;
    struct rxrpc_channel *chan = core::ptr::null_mut();
    struct rxrpc_local *local = bundle.local;
    unsigned int channel;
    bool may_reuse;
    u32 cid;
...
//
    ...
    let mut final_ack_at: c_ulong = jiffies + 2;
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_unbundle_conn(conn: *mut rxrpc_connection) {
    ...
    bundle.conns[bindex] = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_deactivate_bundle(bundle: *mut rxrpc_bundle) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_kill_client_conn(conn: *mut rxrpc_connection) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_discard_expired_client_conns(local: *mut rxrpc_local) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_clean_up_local_conns(local: *mut rxrpc_local) {
