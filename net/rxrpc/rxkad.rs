...
pub const RXKAD_VERSION: c_int = 2;
pub const MAXKRB5TICKETLEN: c_int = 1024;
pub const RXKAD_TKT_TYPE_KERBEROS_V5: c_int = 256;

pub const RXKAD_ALIGN: c_int = 8;
    static const u8 zero_iv[FCRYPT_BSIZE];
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rxkad_level1_hdr {
    pub /: *mut *mut __be32 data_size; / true data size (excluding padding),
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct rxkad_level2_hdr {
    pub /: *mut *mut __be32 data_size; / true data size (excluding padding),
    pub /: *mut *mut __be32 checksum; / decrypted data checksum,
}
...
//
#[no_mangle]
unsafe extern "C" fn rxkad_preparse_server_key(prep: *mut key_preparsed_payload) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rxkad_free_preparse_server_key(prep: *mut key_preparsed_payload) {
    static void rxkad_free_preparse_server_key(struct key_preparsed_payload *prep)
    {
    kfree_sensitive(prep.payload.data[0]);
    }
#[no_mangle]
unsafe extern "C" fn rxkad_destroy_server_key(key: *mut key) {
    static void rxkad_destroy_server_key(struct key *key)
    {
    kfree_sensitive(key.payload.data[0]);
    key.payload.data[0] = core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn rxkad_free_call_crypto(call: *mut rxrpc_call) {
...
//
#[no_mangle]
unsafe extern "C" fn rxkad_secure_packet(call: *mut rxrpc_call, txb: *mut rxrpc_txbuf) -> c_int {
...
// Clear excess space in the packet
    ...
    let mut gap: usize = txb.alloc_size - txb.pkt_len;
    void *p = txb.data;
...
//
    ...
    let mut len: u32 = sp.len, data_size, buf;
    u16 check;
    _enter("");
    if (len < 8)
...
//
#[no_mangle]
unsafe extern "C" fn rxkad_verify_packet(call: *mut rxrpc_call, skb: *mut sk_buff) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rxkad_issue_challenge(conn: *mut rxrpc_connection) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rxkad_calc_response_checksum(response: *mut rxkad_response) {
    static void rxkad_calc_response_checksum(struct rxkad_response *response)
    {
    let mut csum: u32 = 1000003;
    int loop;
    u8 *p = (u8 *) response;
...
//
#[no_mangle]
pub unsafe extern "C" fn rxkad_kernel_respond_to_challenge(challenge: *mut sk_buff) -> c_int {
...
// _expiry = 0;
    ASSERT(server_key.payload.data[0] != core::ptr::null_mut());
...
// response
    ...
    let mut call_id: u32 = ntohl(response.encrypted.call_id[i]);
    let mut counter: u32 = READ_ONCE(conn.channels[i].call_counter);
    if (call_id > INT_MAX) {
    ret = rxrpc_abort_conn(conn, skb, RXKADSEALEDINCON, -EPROTO,
...
//
#[no_mangle]
unsafe extern "C" fn rxkad_clear(conn: *mut rxrpc_connection) {
    static void rxkad_clear(struct rxrpc_connection *conn)
    {
    _enter("");
    kfree_sensitive(conn.rxkad.cipher);
    conn.rxkad.cipher = core::ptr::null_mut();
    }
//
// Initialise the rxkad security service.
//
#[no_mangle]
unsafe extern "C" fn rxkad_init() -> c_int {
