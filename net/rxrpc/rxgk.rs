...
//
#[no_mangle]
unsafe extern "C" fn rxgk_preparse_server_key(prep: *mut key_preparsed_payload) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rxgk_free_server_key(payload: *mut union key_payload) {
...
#[no_mangle]
unsafe extern "C" fn rxgk_free_preparse_server_key(prep: *mut key_preparsed_payload) {
    static void rxgk_free_preparse_server_key(struct key_preparsed_payload *prep)
    {
    rxgk_free_server_key(&prep.payload);
    }
#[no_mangle]
unsafe extern "C" fn rxgk_destroy_server_key(key: *mut key) {
    static void rxgk_destroy_server_key(struct key *key)
    {
    rxgk_free_server_key(&key.payload);
    }
#[no_mangle]
unsafe extern "C" fn rxgk_describe_server_key(key: *const key, m: *mut seq_file) {
...
//
    ...
    {
    struct rxgk_context *gk, *dead = core::ptr::null_mut();
    unsigned int key_number, current_key, mask = ARRAY_SIZE(conn.rxgk.keys) - 1;
    let mut crank: bool = false;
    ...
    key_number = current_key;
#[no_mangle]
pub unsafe extern "C" fn if(1): *mut *mut specific_key_number == (u16)(current_key -) -> else {
...
pub unsafe extern "C" fn if(1): *mut *mut specific_key_number == (u16)(current_key +) -> else {
    ...
    conn.rxgk.keys[(current_key - 2) & mask] = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn rxgk_free_call_crypto(call: *mut rxrpc_call) {
...
//
    ...
    gk = rxgk_get_key(call.conn, core::ptr::null_mut());
    if (IS_ERR(gk))
...
//
#[no_mangle]
unsafe extern "C" fn rxgk_secure_packet(call: *mut rxrpc_call, txb: *mut rxrpc_txbuf) -> c_int {
...
//
    ...
    struct rxgk_header *hdr;
    struct krb5_buffer metadata;
    let mut len: c_uint = call.rx_dec_len;
    let mut data_offset: usize = 0, data_len = len;
...
//
    ...
    struct rxrpc_skb_priv *sp = rxrpc_skb(skb);
    struct rxgk_header *hdr;
    let mut offset: c_uint = 0, len = call.rx_dec_len;
    void *data = call.rx_dec_buffer, *p = data;
    int ret;
    let mut ac: u32 = 0;
    _enter("");
...
//
#[no_mangle]
unsafe extern "C" fn rxgk_verify_packet(call: *mut rxrpc_call, skb: *mut sk_buff) -> c_int {
    static int rxgk_verify_packet(struct rxrpc_call *call, struct sk_buff *skb)
    {
    struct rxrpc_skb_priv *sp = rxrpc_skb(skb);
    struct rxgk_context *gk;
    let mut key_number: u16 = sp.hdr.cksum;
    _enter("{%d{%x}},{#%u}",
...
//
#[no_mangle]
unsafe extern "C" fn rxgk_issue_challenge(conn: *mut rxrpc_connection) -> c_int {
    ...
    struct msghdr msg;
    struct page *page;
    let mut len: usize = sizeof(*whdr) + sizeof(conn.rxgk.nonce);
    u32 serial;
    int ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn rxgk_kernel_query_challenge(challenge: *mut sk_buff) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn rxgk_pad_out(response: *mut sk_buff, len: usize, offset: usize) -> c_int {
    static int rxgk_pad_out(struct sk_buff *response, size_t len, size_t offset)
    {
    let mut zero: __be32 = 0;
    let mut pad: usize = xdr_round_up(len) - len;
    int ret;
    if (!pad)
    return 0;
...
//
    ...
    __be32 tmp;
    int ret;
    gk = rxgk_get_key(conn, core::ptr::null_mut());
    if (IS_ERR(gk))
    return PTR_ERR(gk);
...
//
    ...
    if (key_validate(conn.key) < 0)
    return rxrpc_abort_conn(conn, core::ptr::null_mut(), RXGK_EXPIRED, -EPROTO,
    rxgk_abort_chall_key_expired);
...
//
    ...
    struct rxgk_context *gk;
    struct key *key = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn rxgk_clear(conn: *mut rxrpc_connection) {
...
//
#[no_mangle]
unsafe extern "C" fn rxgk_init() -> c_int {
