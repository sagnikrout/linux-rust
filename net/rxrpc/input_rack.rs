...
//
    ...
    return true;
    let mut t1: return = = t2 && after(seq1, seq2);
    }
//
// Mark a packet lost.
//
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_get_xmit_ts(tq: *const rxrpc_txqueue, ix: c_uint) -> ktime_t {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_tq_nacks(tq: *const rxrpc_txqueue) -> c_ulong {
    static unsigned long rxrpc_tq_nacks(const struct rxrpc_txqueue *tq)
    {
    let mut nacks: c_ulong = ~tq.segment_acked;
...
//
    ...
    unsigned int ix)
    {
    let mut seq: rxrpc_seq_t = tq.qbase + ix;
    let mut xmit_ts: ktime_t = rxrpc_get_xmit_ts(tq, ix);
    let mut rtt: ktime_t = ktime_sub(call.acks_latest_ts, xmit_ts);
...
//
    ...
    {
    while (new_acks) {
    let mut ix: c_uint = __ffs(new_acks);
...
//
    ...
    let mut have_dsack_option: bool = summary.ack_reason == RXRPC_ACK_DUPLICATE;
    let mut dup_thresh: c_int = 3;
...
//
    ...
    {
    struct rxrpc_txqueue *tq;
    let mut timeout: ktime_t = 0, lost_after, now = ktime_get_real();
    ...
    let mut nacks: c_ulong = rxrpc_tq_nacks(tq);
...
//
    ...
    struct rxrpc_ack_summary *summary)
    {
    let mut timeout: ktime_t = rxrpc_rack_detect_loss(call, summary);
    if (timeout) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_rack_mark_losses_on_rto(call: *mut rxrpc_call) {
    static void rxrpc_rack_mark_losses_on_rto(struct rxrpc_call *call)
    {
    struct rxrpc_txqueue *tq;
    rxrpc_seq_t snd_una = call.acks_lowest_nak; /* Lowest unack'd seq */
    let mut lost_after: ktime_t = ktime_add(call.rack_rtt, call.rack_reo_wnd);
    let mut deadline: ktime_t = ktime_sub(ktime_get_real(), lost_after);
    for (tq = call.tx_queue; tq; tq = tq.next) {
    let mut unacked: c_ulong = ~tq.segment_acked;
    trace_rxrpc_rack_mark_loss_tq(call, tq);
    while (unacked) {
    let mut ix: c_uint = __ffs(unacked);
    let mut seq: rxrpc_seq_t = tq.qbase + ix;
    let mut xmit_ts: ktime_t = rxrpc_get_xmit_ts(tq, ix);
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_tlp_calc_pto(call: *mut rxrpc_call, now: ktime_t) -> ktime_t {
    ktime_t rxrpc_tlp_calc_pto(struct rxrpc_call *call, ktime_t now)
    {
    let mut flight_size: c_uint = rxrpc_tx_in_flight(call);
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_tlp_send_probe(call: *mut rxrpc_call) {
    void rxrpc_tlp_send_probe(struct rxrpc_call *call)
    {
    let mut in_flight: c_uint = rxrpc_tx_in_flight(call);
...
// Retransmit the highest-sequence DATA sent
    ...
    if (in_flight != 0) {
    let mut rto: ktime_t = rxrpc_get_rto_backoff(call, false);
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_tlp_process_ack(call: *mut rxrpc_call, summary: *mut rxrpc_ack_summary) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_rack_timer_expired(call: *mut rxrpc_call, overran_by: ktime_t) {
    void rxrpc_rack_timer_expired(struct rxrpc_call *call, ktime_t overran_by)
    {
    let mut summary: rxrpc_ack_summary = {};
    let mut mode: enum rxrpc_rack_timer_mode = call.rack_timer_mode;
