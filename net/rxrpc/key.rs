...
#[no_mangle]
unsafe extern "C" fn xdr_dec64(xdr: *const __be32) -> u64 {
    static u64 xdr_dec64(const __be32 *xdr)
    {
    return (u64)ntohl(xdr[0]) << 32 | (u64)ntohl(xdr[1]);
    }
#[no_mangle]
unsafe extern "C" fn rxrpc_s64_to_time64(time_in_100ns: i64) -> time64_t {
    static time64_t rxrpc_s64_to_time64(s64 time_in_100ns)
    {
    let mut neg: bool = false;
    let mut tmp: u64 = time_in_100ns;
    if (time_in_100ns < 0) {
    tmp = -time_in_100ns;
    neg = true;
    }
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_preparse_xdr(prep: *mut key_preparsed_payload) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_preparse(prep: *mut key_preparsed_payload) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_free_token_list(token: *mut rxrpc_key_token) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_free_preparse(prep: *mut key_preparsed_payload) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_destroy(key: *mut key) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_describe(key: *const key, m: *mut seq_file) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_request_key(rx: *mut rxrpc_sock, optval: sockptr_t, optlen: c_int) -> c_int {
    ...
    key = request_key_net(&key_type_rxrpc, description, sock_net(&rx.sk), core::ptr::null_mut());
    if (IS_ERR(key)) {
    kfree(description);
...
//
    ...
    KEY_ALLOC_NOT_IN_QUOTA, core::ptr::null_mut());
    if (IS_ERR(key)) {
    ...
    ret = key_instantiate_and_link(key, &data, sizeof(data), core::ptr::null_mut(), core::ptr::null_mut());
    if (ret < 0)
    goto error;
    conn.key = key;
...
//
    ...
    KEY_POS_SEARCH, KEY_ALLOC_NOT_IN_QUOTA, core::ptr::null_mut());
    if (IS_ERR(key))
    return key;
    ret = key_instantiate_and_link(key, core::ptr::null_mut(), 0, core::ptr::null_mut(), core::ptr::null_mut());
    if (ret < 0) {
    key_revoke(key);
    key_put(key);
    return ERR_PTR(ret);
