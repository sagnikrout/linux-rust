...
//
    ...
    {
    let mut delay: ktime_t = ms_to_ktime(READ_ONCE(rxrpc_idle_ack_delay));
    let mut now: ktime_t = ktime_get_real();
    let mut ping_at: ktime_t = ktime_add(now, delay);
...
//
    ...
    enum rxrpc_propose_ack_trace why)
    {
    let mut now: ktime_t = ktime_get_real(), delay;
...
//
    ...
    let mut ix: c_uint = req.seq & RXRPC_TXQ_MASK;
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_resend(call: *mut rxrpc_call) {
...
// Scan the transmission queue, looking for lost packets.
    ...
    let mut lost: c_ulong = tq.segment_lost;
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_resend_tlp(call: *mut rxrpc_call) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_begin_service_reply(call: *mut rxrpc_call) {
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_close_tx_phase(call: *mut rxrpc_call) {
...
//
    ...
    {
    let mut space: c_int = rxrpc_tx_window_space(call);
    ...
    let mut limit: c_int = min(space, max(call.peer.pmtud_jumbo, 1));
...
//
#[no_mangle]
unsafe extern "C" fn rxrpc_send_initial_ping(call: *mut rxrpc_call) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rxrpc_input_call_event(call: *mut rxrpc_call) -> bool {
    bool rxrpc_input_call_event(struct rxrpc_call *call)
    {
    struct sk_buff *skb;
    ktime_t now, t;
    let mut did_receive: bool = false, saw_ack = false;
    s32 abort_code;
...
// Make sure the timer is restarted
    if (!__rxrpc_call_is_complete(call)) {
    let mut next: ktime_t = READ_ONCE(call.expect_term_by), delay;
    ...
    let mut nowj: c_ulong = jiffies, delayj, nextj;
