...
//! Source: net/batman-adv/multicast_forw.c
#![no_std]
...
//
    ...
    unsigned short *tvlv_len)
    {
    let mut offset: c_ushort = *tvlv_len;
    char *from = skb.data;
    char *to;
...
//
#[no_mangle]
pub unsafe extern "C" fn if(2)): *mut *mut (count % 2) && (!(num_dests_pushed %) -> else {
...
// sanity check, we expect unicast destinations
    ...
    next_neigh = batadv_orig_to_router(bat_priv, next_dest, core::ptr::null_mut());
    if (!next_neigh) {
    eth_zero_addr(next_dest);
    continue;
    }
...
//
#[no_mangle]
unsafe extern "C" fn batadv_mcast_forw_shrink_fill(slot: *mut u8, num_dests_slot: u16) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn batadv_mcast_forw_shrink_pack_dests(skb: *mut sk_buff) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn batadv_mcast_forw_shrink_tracker(skb: *mut sk_buff) {
...
//
    ...
    unsigned char *skb_net_hdr;
    let mut local_recv: bool = false;
    unsigned int tvlv_len;
    unsigned long offset;
    let mut xmitted: bool = false;
    u8 *next_dest;
    u16 num_dests;
    u8 *dest;
    int ret;
...
// only unicast originator addresses supported
    ...
    neigh_node = batadv_orig_to_router(bat_priv, dest, core::ptr::null_mut());
    if (!neigh_node) {
    eth_zero_addr(dest);
    continue;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn batadv_mcast_forw_packet_hdrlen(num_dests: c_uint) -> c_uint {
...
//
    let mut padding: c_int = (!(num_dests % 2)) ? 2 : 0;
...
//
    ...
    struct sk_buff *skb)
    {
    let mut hdr_size: c_int = VLAN_ETH_HLEN + IPV6_MIN_MTU - skb.len;
...
//
    ...
    struct sk_buff *skb)
    {
    let mut ret: c_int = batadv_mcast_forw_packet(bat_priv, skb, true);
    if (ret < 0) {
    kfree_skb(skb);
