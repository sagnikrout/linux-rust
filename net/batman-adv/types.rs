//! Automatically rewritten from C Header to Rust Module
//! Source: net/batman-adv/types.h
#![no_std]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(dead_code)]
#![allow(unused_variables)]
#![allow(unused_mut)]

use core::ffi::*;

// --- Linux Kernel Primitives Prelude ---
pub type uid_t = u32;
pub type gid_t = u32;
pub type uid16_t = u16;
pub type gid16_t = u16;
pub type pid_t = i32;
pub type mode_t = u32;
pub type umode_t = u16;
pub type nlink_t = u32;
pub type off_t = i64;
pub type loff_t = i64;
pub type dev_t = u32;
pub type ino_t = u64;
pub type size_t = usize;
pub type ssize_t = isize;
pub type uintptr_t = usize;
pub type intptr_t = isize;
pub type ptrdiff_t = isize;
pub type clockid_t = i32;
pub type timer_t = i32;
pub type time64_t = i64;
pub type atomic_t = core::sync::atomic::AtomicI32;
pub type atomic64_t = core::sync::atomic::AtomicI64;
// ---------------------------------------


// SPDX-License-Identifier: GPL-2.0
// Copyright (C) B.A.T.M.A.N. contributors:
//
// Marek Lindner, Simon Wunderlich
//

//
// typedef batadv_dat_addr_t - type used for all DHT addresses
//
// If it is changed, BATADV_DAT_ADDR_MAX is changed as well.
//
// *Please be careful: batadv_dat_addr_t must be UNSIGNED
//
pub type batadv_dat_addr_t = u16;

//
// enum batadv_dhcp_recipient - dhcp destination
//
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum batadv_dhcp_recipient {
// @BATADV_DHCP_NO: packet is not a dhcp message
    BATADV_DHCP_NO = 0,

// @BATADV_DHCP_TO_SERVER: dhcp message is directed to a server
    BATADV_DHCP_TO_SERVER,

// @BATADV_DHCP_TO_CLIENT: dhcp message is directed to a client
    BATADV_DHCP_TO_CLIENT,
}

//
// BATADV_TT_REMOTE_MASK - bitmask selecting the flags that are sent over the
// wire only
//
pub const BATADV_TT_REMOTE_MASK: c_uint = 0x00FF;
//
// BATADV_TT_SYNC_MASK - bitmask of the flags that need to be kept in sync
// among the nodes. These flags are used to compute the global/local CRC
//
pub const BATADV_TT_SYNC_MASK: c_uint = 0x00F0;
//
// struct batadv_ogm_buf - Buffer to construct an OGM with TVLV
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_ogm_buf {
// @buf: buffer holding the OGM packet
    pub buf: *mut c_void,
// @len: length of the OGM packet buffer data
    pub len: usize,
// @capacity: size of allocated buf
    pub capacity: usize,
// @header_length: fixed size header length (must be <= len)
    pub header_length: usize,
}

//
// struct batadv_hard_iface_bat_iv - per hard-interface B.A.T.M.A.N. IV data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_hard_iface_bat_iv {
// @ogm_buff: buffer holding the OGM packet
    pub ogm_buff: batadv_ogm_buf,
// @ogm_seqno: OGM sequence number - used to identify each OGM
    pub ogm_seqno: core::sync::atomic::AtomicI32,
// @reschedule_work: recover OGM schedule after schedule error
    pub reschedule_work: delayed_work,
// @ogm_buff_mutex: lock protecting ogm_buff
    pub ogm_buff_mutex: mutex,
}

//
// enum batadv_v_hard_iface_flags - interface flags useful to B.A.T.M.A.N. V
//
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum batadv_v_hard_iface_flags {
//
// @BATADV_FULL_DUPLEX: tells if the connection over this link is
// full-duplex
//
    BATADV_FULL_DUPLEX	= BIT(0),

//
// @BATADV_WARNING_DEFAULT: tells whether we have warned the user that
// no throughput data is available for this interface and that default
// values are assumed.
//
    BATADV_WARNING_DEFAULT	= BIT(1),
}

//
// struct batadv_hard_iface_bat_v - per hard-interface B.A.T.M.A.N. V data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_hard_iface_bat_v {
// @elp_interval: time interval between two ELP transmissions
    pub elp_interval: u32,
// @elp_seqno: current ELP sequence number
    pub elp_seqno: core::sync::atomic::AtomicI32,
// @elp_skb: base skb containing the ELP message to send
    pub elp_skb: *mut sk_buff,
// @elp_wq: workqueue used to schedule ELP transmissions
    pub elp_wq: delayed_work,
// @aggr_wq: workqueue used to transmit queued OGM packets
    pub aggr_wq: delayed_work,
// @aggr_list: queue for to be aggregated OGM packets
    pub aggr_list: sk_buff_head,
//
// @aggr_list_enabled: aggr_list is active and new skbs can be
// enqueued. Protected by aggr_list.lock after initialization
//
    pub aggr_list_enabled:1: bool,
// @aggr_len: size of the OGM aggregate (excluding ethernet header)
    pub aggr_len: c_uint,
//
// @throughput_override: throughput override to disable link
// auto-detection
//
    pub throughput_override: u32,
// @flags: interface specific flags
    pub flags: u8,
}

//
// enum batadv_hard_iface_wifi_flags - Flags describing the wifi configuration
// of a batadv_hard_iface
//
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum batadv_hard_iface_wifi_flags {
// @BATADV_HARDIF_WIFI_WEXT_DIRECT: it is a wext wifi device
    BATADV_HARDIF_WIFI_WEXT_DIRECT = BIT(0),

// @BATADV_HARDIF_WIFI_CFG80211_DIRECT: it is a cfg80211 wifi device
    BATADV_HARDIF_WIFI_CFG80211_DIRECT = BIT(1),

//
// @BATADV_HARDIF_WIFI_WEXT_INDIRECT: link device is a wext wifi device
//
    BATADV_HARDIF_WIFI_WEXT_INDIRECT = BIT(2),

//
// @BATADV_HARDIF_WIFI_CFG80211_INDIRECT: link device is a cfg80211 wifi
// device
//
    BATADV_HARDIF_WIFI_CFG80211_INDIRECT = BIT(3),
}

//
// struct batadv_wifi_net_device_state - cache of wifi information of net_devices
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_wifi_net_device_state {
// @l: anchor in rhashtable
    pub l: rhash_head,
// @netdev: pointer to the net_device
    pub netdev: *mut net_device,
// @dev_tracker: device tracker for @netdev
    pub dev_tracker: netdevice_tracker,
//
// @wifi_flags: flags whether this is (directly or indirectly) a wifi
// interface
//
    pub wifi_flags: u32,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_hard_iface - network device known to batman-adv
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_hard_iface {
// @if_status: status of the interface for batman-adv
    pub if_status: c_char,
//
// @num_bcasts: number of payload re-broadcasts on this interface (ARQ)
//
    pub num_bcasts: u8,
// @net_dev: pointer to the net_device
    pub net_dev: *mut net_device,
// @dev_tracker: device tracker for @net_dev
    pub dev_tracker: netdevice_tracker,
// @refcount: number of contexts the object is used
    pub refcount: kref,
//
// @batman_adv_ptype: packet type describing packets that should be
// processed by batman-adv for this interface
//
    pub batman_adv_ptype: packet_type,
//
// @mesh_iface: the batman-adv interface which uses this network
// interface
//
    pub mesh_iface: *mut net_device,
// @meshif_dev_tracker: device tracker for @mesh_iface
    pub meshif_dev_tracker: netdevice_tracker,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
//
// @hop_penalty: penalty which will be applied to the tq-field
// of an OGM received via this interface
//
    pub hop_penalty: u8,
// @bat_iv: per hard-interface B.A.T.M.A.N. IV data
    pub bat_iv: batadv_hard_iface_bat_iv,

// @bat_v: per hard-interface B.A.T.M.A.N. V data
    pub bat_v: batadv_hard_iface_bat_v,

//
// @neigh_list: list of unique single hop neighbors via this interface
//
    pub neigh_list: hlist_head,
// @neigh_list_lock: lock protecting neigh_list
    pub neigh_list_lock: spinlock_t,
}

//
// struct batadv_orig_ifinfo_bat_iv - B.A.T.M.A.N. IV private orig_ifinfo
// members
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_orig_ifinfo_bat_iv {
//
// @bcast_own: bitfield which counts the number of our OGMs this
// orig_node rebroadcasted "back" to us  (relative to last_real_seqno)
//
    pub BATADV_TQ_LOCAL_WINDOW_SIZE): DECLARE_BITMAP(bcast_own,,
// @bcast_own_sum: sum of bcast_own
    pub bcast_own_sum: u8,
}

//
// struct batadv_orig_ifinfo - originator info per outgoing interface
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_orig_ifinfo {
// @list: list node for &batadv_orig_node.ifinfo_list
    pub list: hlist_node,
// @if_outgoing: pointer to outgoing hard-interface
    pub if_outgoing: *mut batadv_hard_iface,
// @router: router that should be used to reach this originator
    pub router: *mut batadv_neigh_node __rcu,
// @last_real_seqno: last and best known sequence number
    pub last_real_seqno: u32,
// @last_ttl: ttl of last received packet
    pub last_ttl: u8,
// @last_seqno_forwarded: seqno of the OGM which was forwarded last
    pub last_seqno_forwarded: u32,
// @batman_seqno_reset: time when the batman seqno window was reset
    pub batman_seqno_reset: c_ulong,
// @bat_iv: B.A.T.M.A.N. IV private structure
    pub bat_iv: batadv_orig_ifinfo_bat_iv,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_frag_table_entry - head in the fragment buffer table
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_frag_table_entry {
// @fragment_list: head of list with fragments
    pub fragment_list: hlist_head,
// @lock: lock to protect the list of fragments
    pub lock: spinlock_t,
// @timestamp: time (jiffy) of last received fragment
    pub timestamp: c_ulong,
// @seqno: sequence number of the fragments in the list
    pub seqno: u16,
// @size: accumulated size of packets in list
    pub size: usize,
// @total_size: expected size of the assembled packet
    pub total_size: u16,
}

//
// struct batadv_frag_list_entry - entry in a list of fragments
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_frag_list_entry {
// @list: list node information
    pub list: hlist_node,
// @skb: fragment
    pub skb: *mut sk_buff,
// @no: fragment number in the set
    pub no: u8,
}

//
// struct batadv_vlan_tt - VLAN specific TT attributes
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_vlan_tt {
// @crc: CRC32 checksum of the entries belonging to this vlan
    pub crc: u32,
// @num_entries: number of TT entries for this VLAN
    pub num_entries: core::sync::atomic::AtomicI32,
}

//
// struct batadv_orig_node_vlan - VLAN specific data per orig_node
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_orig_node_vlan {
// @vid: the VLAN identifier
    pub vid: c_ushort,
// @tt: VLAN specific TT attributes
    pub tt: batadv_vlan_tt,
// @list: list node for &batadv_orig_node.vlan_list
    pub list: hlist_node,
//
// @refcount: number of context where this object is currently in use
//
    pub refcount: kref,
// @rcu: struct used for freeing in a RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_orig_bat_iv - B.A.T.M.A.N. IV private orig_node members
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_orig_bat_iv {
//
// @ogm_cnt_lock: lock protecting &batadv_orig_ifinfo_bat_iv.bcast_own,
// &batadv_orig_ifinfo_bat_iv.bcast_own_sum,
// &batadv_neigh_ifinfo_bat_iv.bat_iv.real_bits and
// &batadv_neigh_ifinfo_bat_iv.real_packet_count
//
    pub ogm_cnt_lock: spinlock_t,
}

//
// struct batadv_orig_node - structure for orig_list maintaining nodes of mesh
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_orig_node {
// @orig: originator ethernet address
    pub orig: [u8; ETH_ALEN],
// @ifinfo_list: list for routers per outgoing interface
    pub ifinfo_list: hlist_head,
//
// @last_bonding_candidate: pointer to last ifinfo of last used router
//
    pub last_bonding_candidate: *mut batadv_orig_ifinfo,

// @dat_addr: address of the orig node in the distributed hash
    pub dat_addr: batadv_dat_addr_t,

// @last_seen: time when last packet from this node was received
    pub last_seen: c_ulong,
//
// @bcast_seqno_reset: time when the broadcast seqno window was reset
//
    pub bcast_seqno_reset: c_ulong,

//
// @mcast_handler_lock: synchronizes mcast-capability and -flag changes
//
    pub mcast_handler_lock: spinlock_t,
// @mcast_flags: multicast flags announced by the orig node
    pub mcast_flags: u8,
//
// @mcast_want_all_unsnoopables_node: a list node for the
// mcast.want_all_unsnoopables list
//
    pub mcast_want_all_unsnoopables_node: hlist_node,
//
// @mcast_want_all_ipv4_node: a list node for the mcast.want_all_ipv4
// list
//
    pub mcast_want_all_ipv4_node: hlist_node,
//
// @mcast_want_all_ipv6_node: a list node for the mcast.want_all_ipv6
// list
//
    pub mcast_want_all_ipv6_node: hlist_node,
//
// @mcast_want_all_rtr4_node: a list node for the mcast.want_all_rtr4
// list
//
    pub mcast_want_all_rtr4_node: hlist_node,
//
// @mcast_want_all_rtr6_node: a list node for the mcast.want_all_rtr6
// list
//
    pub mcast_want_all_rtr6_node: hlist_node,

// @capabilities: announced capabilities of this originator
    pub capabilities: c_ulong,
//
// @capa_initialized: bitfield to remember whether a capability was
// initialized
//
    pub capa_initialized: c_ulong,
// @last_ttvn: last seen translation table version number
    pub last_ttvn: u8,
// @tt_buff: last tt changeset this node received from the orig node
    pub tt_buff: *mut c_uchar,
//
// @tt_buff_len: length of the last tt changeset this node received
// from the orig node
//
    pub tt_buff_len: u16,
// @tt_buff_lock: lock that protects tt_buff and tt_buff_len
    pub tt_buff_lock: spinlock_t,
//
// @tt_lock: avoids concurrent read from and write to the table. Table
// update is made up of two operations (data structure update and
// metadata -CRC/TTVN-recalculation) and they have to be executed
// atomically in order to avoid another thread to read the
// table/metadata between those.
//
    pub tt_lock: spinlock_t,
//
// @bcast_bits: bitfield containing the info which payload broadcast
// originated from this orig node this host already has seen (relative
// to last_bcast_seqno)
//
    pub BATADV_TQ_LOCAL_WINDOW_SIZE): DECLARE_BITMAP(bcast_bits,,
//
// @last_bcast_seqno: last broadcast sequence number received by this
// host
//
    pub last_bcast_seqno: u32,
//
// @neigh_list: list of potential next hop neighbor towards this orig
// node
//
    pub neigh_list: hlist_head,
//
// @neigh_list_lock: lock protecting neigh_list, ifinfo_list,
// last_bonding_candidate and router
//
    pub neigh_list_lock: spinlock_t,
// @hash_entry: hlist node for &batadv_priv.orig_hash
    pub hash_entry: hlist_node,
// @bat_priv: pointer to mesh_iface this orig node belongs to
    pub bat_priv: *mut batadv_priv,
// @bcast_seqno_lock: lock protecting bcast_bits & last_bcast_seqno
    pub bcast_seqno_lock: spinlock_t,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
// @fragments: array with heads for fragment chains
    pub fragments: [batadv_frag_table_entry; BATADV_FRAG_BUFFER_COUNT],
//
// @vlan_list: a list of orig_node_vlan structs, one per VLAN served by
// the originator represented by this object
//
    pub vlan_list: hlist_head,
// @vlan_list_lock: lock protecting vlan_list
    pub vlan_list_lock: spinlock_t,
// @bat_iv: B.A.T.M.A.N. IV private structure
    pub bat_iv: batadv_orig_bat_iv,
}

//
// enum batadv_orig_capabilities - orig node capabilities
//
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum batadv_orig_capabilities {
//
// @BATADV_ORIG_CAPA_HAS_DAT: orig node has distributed arp table
// enabled
//
    BATADV_ORIG_CAPA_HAS_DAT,

// @BATADV_ORIG_CAPA_HAS_TT: orig node has tt capability
    BATADV_ORIG_CAPA_HAS_TT,

//
// @BATADV_ORIG_CAPA_HAS_MCAST: orig node has some multicast capability
// (= orig node announces a tvlv of type BATADV_TVLV_MCAST)
//
    BATADV_ORIG_CAPA_HAS_MCAST,
}

//
// struct batadv_gw_node - structure for orig nodes announcing gw capabilities
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_gw_node {
// @list: list node for &batadv_priv_gw.list
    pub list: hlist_node,
// @orig_node: pointer to corresponding orig node
    pub orig_node: *mut batadv_orig_node,
// @bandwidth_down: advertised uplink download bandwidth
    pub bandwidth_down: u32,
// @bandwidth_up: advertised uplink upload bandwidth
    pub bandwidth_up: u32,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_hardif_neigh_node_bat_v - B.A.T.M.A.N. V private neighbor
// information
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_hardif_neigh_node_bat_v {
// @throughput: ewma link throughput towards this neighbor
    pub throughput: ewma_throughput,
// @elp_interval: time interval between two ELP transmissions
    pub elp_interval: u32,
// @elp_latest_seqno: latest and best known ELP sequence number
    pub elp_latest_seqno: u32,
//
// @last_unicast_tx: when the last unicast packet has been sent to this
// neighbor
//
    pub last_unicast_tx: c_ulong,
}

//
// struct batadv_hardif_neigh_node - unique neighbor per hard-interface
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_hardif_neigh_node {
// @list: list node for &batadv_hard_iface.neigh_list
    pub list: hlist_node,
// @addr: the MAC address of the neighboring interface
    pub addr: [u8; ETH_ALEN],
//
// @orig: the address of the originator this neighbor node belongs to
//
    pub orig: [u8; ETH_ALEN],
// @if_incoming: pointer to incoming hard-interface
    pub if_incoming: *mut batadv_hard_iface,
// @last_seen: when last packet via this neighbor was received
    pub last_seen: c_ulong,

// @bat_v: B.A.T.M.A.N. V private data
    pub bat_v: batadv_hardif_neigh_node_bat_v,

// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in a RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_neigh_node - structure for single hops neighbors
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_neigh_node {
// @list: list node for &batadv_orig_node.neigh_list
    pub list: hlist_node,

//
// @orig_node_id: pointer to corresponding orig_node. It must only be used
// to identify the node but must NEVER be dereferenced. The reference counter
// was not increased when this was assigned because it would otherwise create
// a reference cycle.
//
    pub orig_node_id: *mut *mut batadv_orig_node __private,

// @addr: the MAC address of the neighboring interface
    pub addr: [u8; ETH_ALEN],
// @ifinfo_list: list for routing metrics per outgoing interface
    pub ifinfo_list: hlist_head,
// @ifinfo_lock: lock protecting ifinfo_list and its members
    pub ifinfo_lock: spinlock_t,
// @if_incoming: pointer to incoming hard-interface
    pub if_incoming: *mut batadv_hard_iface,
// @last_seen: when last packet via this neighbor was received
    pub last_seen: c_ulong,
// @hardif_neigh: hardif_neigh of this neighbor
    pub hardif_neigh: *mut batadv_hardif_neigh_node,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_neigh_ifinfo_bat_iv - neighbor information per outgoing
// interface for B.A.T.M.A.N. IV
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_neigh_ifinfo_bat_iv {
// @tq_recv: ring buffer of received TQ values from this neigh node
    pub tq_recv: [u8; BATADV_TQ_GLOBAL_WINDOW_SIZE],
// @tq_index: ring buffer index
    pub tq_index: u8,
//
// @tq_avg: averaged tq of all tq values in the ring buffer (tq_recv)
//
    pub tq_avg: u8,
//
// @real_bits: bitfield containing the number of OGMs received from this
// neigh node (relative to orig_node->last_real_seqno)
//
    pub BATADV_TQ_LOCAL_WINDOW_SIZE): DECLARE_BITMAP(real_bits,,
// @real_packet_count: counted result of real_bits
    pub real_packet_count: u8,
}

//
// struct batadv_neigh_ifinfo_bat_v - neighbor information per outgoing
// interface for B.A.T.M.A.N. V
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_neigh_ifinfo_bat_v {
//
// @throughput: last throughput metric received from originator via this
// neigh
//
    pub throughput: u32,
// @last_seqno: last sequence number known for this neighbor
    pub last_seqno: u32,
}

//
// struct batadv_neigh_ifinfo - neighbor information per outgoing interface
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_neigh_ifinfo {
// @list: list node for &batadv_neigh_node.ifinfo_list
    pub list: hlist_node,
// @if_outgoing: pointer to outgoing hard-interface
    pub if_outgoing: *mut batadv_hard_iface,
// @bat_iv: B.A.T.M.A.N. IV private structure
    pub bat_iv: batadv_neigh_ifinfo_bat_iv,

// @bat_v: B.A.T.M.A.N. V private data
    pub bat_v: batadv_neigh_ifinfo_bat_v,

// @last_ttl: last received ttl from this neigh node
    pub last_ttl: u8,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in a RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_bcast_duplist_entry - structure for LAN broadcast suppression
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_bcast_duplist_entry {
// @orig: mac address of orig node originating the broadcast
    pub orig: [u8; ETH_ALEN],
// @crc: crc32 checksum of broadcast payload
    pub crc: u32,
// @entrytime: time when the broadcast packet was received
    pub entrytime: c_ulong,
}

//
// enum batadv_counters - indices for traffic counters
//
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum batadv_counters {
// @BATADV_CNT_TX: transmitted payload traffic packet counter
    BATADV_CNT_TX,

// @BATADV_CNT_TX_BYTES: transmitted payload traffic bytes counter
    BATADV_CNT_TX_BYTES,

//
// @BATADV_CNT_TX_DROPPED: dropped transmission payload traffic packet
// counter
//
    BATADV_CNT_TX_DROPPED,

// @BATADV_CNT_RX: received payload traffic packet counter
    BATADV_CNT_RX,

// @BATADV_CNT_RX_BYTES: received payload traffic bytes counter
    BATADV_CNT_RX_BYTES,

// @BATADV_CNT_FORWARD: forwarded payload traffic packet counter
    BATADV_CNT_FORWARD,

//
// @BATADV_CNT_FORWARD_BYTES: forwarded payload traffic bytes counter
//
    BATADV_CNT_FORWARD_BYTES,

//
// @BATADV_CNT_MGMT_TX: transmitted routing protocol traffic packet
// counter
//
    BATADV_CNT_MGMT_TX,

//
// @BATADV_CNT_MGMT_TX_BYTES: transmitted routing protocol traffic bytes
// counter
//
    BATADV_CNT_MGMT_TX_BYTES,

//
// @BATADV_CNT_MGMT_RX: received routing protocol traffic packet counter
//
    BATADV_CNT_MGMT_RX,

//
// @BATADV_CNT_MGMT_RX_BYTES: received routing protocol traffic bytes
// counter
//
    BATADV_CNT_MGMT_RX_BYTES,

// @BATADV_CNT_FRAG_TX: transmitted fragment traffic packet counter
    BATADV_CNT_FRAG_TX,

//
// @BATADV_CNT_FRAG_TX_BYTES: transmitted fragment traffic bytes counter
//
    BATADV_CNT_FRAG_TX_BYTES,

// @BATADV_CNT_FRAG_RX: received fragment traffic packet counter
    BATADV_CNT_FRAG_RX,

//
// @BATADV_CNT_FRAG_RX_BYTES: received fragment traffic bytes counter
//
    BATADV_CNT_FRAG_RX_BYTES,

// @BATADV_CNT_FRAG_FWD: forwarded fragment traffic packet counter
    BATADV_CNT_FRAG_FWD,

//
// @BATADV_CNT_FRAG_FWD_BYTES: forwarded fragment traffic bytes counter
//
    BATADV_CNT_FRAG_FWD_BYTES,

//
// @BATADV_CNT_TT_REQUEST_TX: transmitted tt req traffic packet counter
//
    BATADV_CNT_TT_REQUEST_TX,

// @BATADV_CNT_TT_REQUEST_RX: received tt req traffic packet counter
    BATADV_CNT_TT_REQUEST_RX,

//
// @BATADV_CNT_TT_RESPONSE_TX: transmitted tt resp traffic packet
// counter
//
    BATADV_CNT_TT_RESPONSE_TX,

//
// @BATADV_CNT_TT_RESPONSE_RX: received tt resp traffic packet counter
//
    BATADV_CNT_TT_RESPONSE_RX,

//
// @BATADV_CNT_TT_ROAM_ADV_TX: transmitted tt roam traffic packet
// counter
//
    BATADV_CNT_TT_ROAM_ADV_TX,

//
// @BATADV_CNT_TT_ROAM_ADV_RX: received tt roam traffic packet counter
//
    BATADV_CNT_TT_ROAM_ADV_RX,

//
// @BATADV_CNT_MCAST_TX: transmitted batman-adv multicast packets
// counter
//
    BATADV_CNT_MCAST_TX,

//
// @BATADV_CNT_MCAST_TX_BYTES: transmitted batman-adv multicast packets
// bytes counter
//
    BATADV_CNT_MCAST_TX_BYTES,

//
// @BATADV_CNT_MCAST_TX_LOCAL: counter for multicast packets which
// were locally encapsulated and transmitted as batman-adv multicast
// packets
//
    BATADV_CNT_MCAST_TX_LOCAL,

//
// @BATADV_CNT_MCAST_TX_LOCAL_BYTES: bytes counter for multicast packets
// which were locally encapsulated and transmitted as batman-adv
// multicast packets
//
    BATADV_CNT_MCAST_TX_LOCAL_BYTES,

//
// @BATADV_CNT_MCAST_RX: received batman-adv multicast packet counter
//
    BATADV_CNT_MCAST_RX,

//
// @BATADV_CNT_MCAST_RX_BYTES: received batman-adv multicast packet
// bytes counter
//
    BATADV_CNT_MCAST_RX_BYTES,

//
// @BATADV_CNT_MCAST_RX_LOCAL: counter for received batman-adv multicast
// packets which were forwarded to the local mesh interface
//
    BATADV_CNT_MCAST_RX_LOCAL,

//
// @BATADV_CNT_MCAST_RX_LOCAL_BYTES: bytes counter for received
// batman-adv multicast packets which were forwarded to the local mesh
// interface
//
    BATADV_CNT_MCAST_RX_LOCAL_BYTES,

//
// @BATADV_CNT_MCAST_FWD: counter for received batman-adv multicast
// packets which were forwarded to other, neighboring nodes
//
    BATADV_CNT_MCAST_FWD,

//
// @BATADV_CNT_MCAST_FWD_BYTES: bytes counter for received batman-adv
// multicast packets which were forwarded to other, neighboring nodes
//
    BATADV_CNT_MCAST_FWD_BYTES,

//
// @BATADV_CNT_DAT_GET_TX: transmitted dht GET traffic packet counter
//
    BATADV_CNT_DAT_GET_TX,

// @BATADV_CNT_DAT_GET_RX: received dht GET traffic packet counter
    BATADV_CNT_DAT_GET_RX,

//
// @BATADV_CNT_DAT_PUT_TX: transmitted dht PUT traffic packet counter
//
    BATADV_CNT_DAT_PUT_TX,

// @BATADV_CNT_DAT_PUT_RX: received dht PUT traffic packet counter
    BATADV_CNT_DAT_PUT_RX,

//
// @BATADV_CNT_DAT_CACHED_REPLY_TX: transmitted dat cache reply traffic
// packet counter
//
    BATADV_CNT_DAT_CACHED_REPLY_TX,

// @BATADV_CNT_NUM: number of traffic counters
    BATADV_CNT_NUM,
}

//
// struct batadv_priv_tt - per mesh interface translation table data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv_tt {
// @vn: translation table version number
    pub vn: core::sync::atomic::AtomicI32,
//
// @ogm_append_cnt: counter of number of OGMs containing the local tt
// diff
//
    pub ogm_append_cnt: core::sync::atomic::AtomicI32,
// @local_changes: changes registered in an originator interval
    pub local_changes: usize,
//
// @changes_list: tracks tt local changes within an originator interval
//
    pub changes_list: list_head,
// @local_hash: local translation table hash table
    pub local_hash: *mut batadv_hashtable,
// @global_hash: global translation table hash table
    pub global_hash: *mut batadv_hashtable,
// @req_list: list of pending & unanswered tt_requests
    pub req_list: hlist_head,
//
// @roam_list: list of the last roaming events of each client limiting
// the number of roaming events to avoid route flapping
//
    pub roam_list: list_head,
// @changes_list_lock: lock protecting changes_list & local_changes
    pub changes_list_lock: spinlock_t,
// @req_list_lock: lock protecting req_list
    pub req_list_lock: spinlock_t,
// @roam_list_lock: lock protecting roam_list
    pub roam_list_lock: spinlock_t,
// @last_changeset: last tt changeset this host has generated
    pub last_changeset: *mut c_uchar,
//
// @last_changeset_len: length of last tt changeset this host has
// generated
//
    pub last_changeset_len: u16,
//
// @last_changeset_lock: lock protecting last_changeset &
// last_changeset_len
//
    pub last_changeset_lock: spinlock_t,
//
// @commit_lock: prevents from executing a local TT commit while reading
// the local table. The local TT commit is made up of two operations
// (data structure update and metadata -CRC/TTVN- recalculation) and
// they have to be executed atomically in order to avoid another thread
// to read the table/metadata between those.
//
    pub commit_lock: spinlock_t,
// @work: work queue callback item for translation table purging
    pub work: delayed_work,
}

//
// struct batadv_priv_bla - per mesh interface bridge loop avoidance data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv_bla {
// @num_requests: number of bla requests in flight
    pub num_requests: core::sync::atomic::AtomicI32,
//
// @num_requests_lock: locks update num_requests +
// batadv_backbone_gw::state + batadv_backbone_gw::wait_periods update
//
    pub num_requests_lock: spinlock_t,
//
// @claim_hash: hash table containing mesh nodes this host has claimed
//
    pub claim_hash: *mut batadv_hashtable,
//
// @backbone_hash: hash table containing all detected backbone gateways
//
    pub backbone_hash: *mut batadv_hashtable,
// @loopdetect_addr: MAC address used for own loopdetection frames
    pub loopdetect_addr: [u8; ETH_ALEN],
//
// @loopdetect_lasttime: time when the loopdetection frames were sent
//
    pub loopdetect_lasttime: c_ulong,
//
// @loopdetect_next: how many periods to wait for the next loopdetect
// process
//
    pub loopdetect_next: core::sync::atomic::AtomicI32,
//
// @bcast_duplist: recently received broadcast packets array (for
// broadcast duplicate suppression)
//
    pub bcast_duplist: [batadv_bcast_duplist_entry; BATADV_DUPLIST_SIZE],
//
// @bcast_duplist_curr: index of last broadcast packet added to
// bcast_duplist
//
    pub bcast_duplist_curr: c_int,
//
// @bcast_duplist_lock: lock protecting bcast_duplist &
// bcast_duplist_curr
//
    pub bcast_duplist_lock: spinlock_t,
// @claim_dest: local claim data (e.g. claim group)
    pub claim_dest: batadv_bla_claim_dst,
// @work: work queue callback item for cleanups & bla announcements
    pub work: delayed_work,
}

//
// struct batadv_priv_gw - per mesh interface gateway data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv_gw {
// @gateway_list: list of available gateway nodes
    pub gateway_list: hlist_head,
// @list_lock: lock protecting gateway_list, curr_gw, generation
    pub list_lock: spinlock_t,
// @curr_gw: pointer to currently selected gateway node
    pub curr_gw: *mut batadv_gw_node __rcu,
// @generation: current (generation) sequence number
    pub generation: c_uint,
//
// @mode: gateway operation: off, client or server (see batadv_gw_modes)
//
    pub mode: batadv_gw_modes,
// @sel_class: gateway selection class (applies if gw_mode client)
    pub sel_class: u32,
//
// @bandwidth_down: advertised uplink download bandwidth (if gw_mode
// server)
//
    pub bandwidth_down: u32,
//
// @bandwidth_up: advertised uplink upload bandwidth (if gw_mode server)
//
    pub bandwidth_up: u32,
// @reselect: bool indicating a gateway re-selection is in progress
    pub reselect: core::sync::atomic::AtomicI32,
}

//
// struct batadv_priv_tvlv - per mesh interface tvlv data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv_tvlv {
//
// @container_list: list of registered tvlv containers to be sent with
// each OGM
//
    pub container_list: hlist_head,
// @handler_list: list of the various tvlv content handlers
    pub handler_list: hlist_head,
// @container_list_lock: protects tvlv container list access
    pub container_list_lock: spinlock_t,
// @handler_list_lock: protects handler list access
    pub handler_list_lock: spinlock_t,
}

//
// struct batadv_priv_dat - per mesh interface DAT private data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv_dat {
// @addr: node DAT address
    pub addr: batadv_dat_addr_t,
// @hash: hashtable representing the local ARP cache
    pub hash: *mut batadv_hashtable,
// @work: work queue callback item for cache purging
    pub work: delayed_work,
}

//
// struct batadv_mcast_querier_state - IGMP/MLD querier state when bridged
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_mcast_querier_state {
// @exists: whether a querier exists in the mesh
    pub exists:1: c_uchar,
//
// @shadowing: if a querier exists, whether it is potentially shadowing
// multicast listeners (i.e. querier is behind our own bridge segment)
//
    pub shadowing:1: c_uchar,
}

//
// struct batadv_mcast_mla_flags - flags for the querier, bridge and tvlv state
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_mcast_mla_flags {
// @querier_ipv4: the current state of an IGMP querier in the mesh
    pub querier_ipv4: batadv_mcast_querier_state,
// @querier_ipv6: the current state of an MLD querier in the mesh
    pub querier_ipv6: batadv_mcast_querier_state,
// @enabled: whether the multicast tvlv is currently enabled
    pub enabled:1: c_uchar,
// @bridged: whether the mesh interface has a bridge on top
    pub bridged:1: c_uchar,
// @tvlv_flags: the flags we have last sent in our mcast tvlv
    pub tvlv_flags: u8,
}

//
// struct batadv_priv_mcast - per mesh interface mcast data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv_mcast {
//
// @mla_list: list of multicast addresses we are currently announcing
// via TT
//
    pub /: *mut *mut hlist_head mla_list; / see __batadv_mcast_mla_update(),
//
// @want_all_unsnoopables_list: a list of orig_nodes wanting all
// unsnoopable multicast traffic
//
    pub want_all_unsnoopables_list: hlist_head,
//
// @want_all_ipv4_list: a list of orig_nodes wanting all IPv4 multicast
// traffic
//
    pub want_all_ipv4_list: hlist_head,
//
// @want_all_ipv6_list: a list of orig_nodes wanting all IPv6 multicast
// traffic
//
    pub want_all_ipv6_list: hlist_head,
//
// @want_all_rtr4_list: a list of orig_nodes wanting all routable IPv4
// multicast traffic
//
    pub want_all_rtr4_list: hlist_head,
//
// @want_all_rtr6_list: a list of orig_nodes wanting all routable IPv6
// multicast traffic
//
    pub want_all_rtr6_list: hlist_head,
//
// @mla_flags: flags for the querier, bridge and tvlv state
//
    pub mla_flags: batadv_mcast_mla_flags,
//
// @mla_lock: a lock protecting mla_list and mla_flags
//
    pub mla_lock: spinlock_t,
//
// @num_want_all_unsnoopables: number of nodes wanting unsnoopable IP
// traffic
//
    pub num_want_all_unsnoopables: core::sync::atomic::AtomicI32,
// @num_want_all_ipv4: counter for items in want_all_ipv4_list
    pub num_want_all_ipv4: core::sync::atomic::AtomicI32,
// @num_want_all_ipv6: counter for items in want_all_ipv6_list
    pub num_want_all_ipv6: core::sync::atomic::AtomicI32,
// @num_want_all_rtr4: counter for items in want_all_rtr4_list
    pub num_want_all_rtr4: core::sync::atomic::AtomicI32,
// @num_want_all_rtr6: counter for items in want_all_rtr6_list
    pub num_want_all_rtr6: core::sync::atomic::AtomicI32,
//
// @num_no_mc_ptype_capa: counter for number of nodes without the
// BATADV_MCAST_HAVE_MC_PTYPE_CAPA flag
//
    pub num_no_mc_ptype_capa: core::sync::atomic::AtomicI32,
//
// @want_lists_lock: lock for protecting modifications to mcasts
// want_all_{unsnoopables,ipv4,ipv6}_list (traversals are rcu-locked)
//
    pub want_lists_lock: spinlock_t,
// @work: work queue callback item for multicast TT and TVLV updates
    pub work: delayed_work,
}

//
// struct batadv_tp_unacked - unacked packet meta-information
//
// This struct is supposed to represent a buffer unacked packet. However, since
// the purpose of the TP meter is to count the traffic only, there is no need to
// store the entire sk_buff, the starting offset and the length are enough
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tp_unacked {
// @seqno: seqno of the unacked packet
    pub seqno: u32,
// @len: length of the packet
    pub len: u32,
// @list: list node for &batadv_tp_receiver.unacked_list
    pub list: list_head,
}

//
// struct batadv_tp_vars_common - common tp meter private variables per session
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tp_vars_common {
// @list: list node for &bat_priv.tp_sender_list/&bat_priv.tp_receiver_list
    pub list: hlist_node,
// @timer: timer for ack (receiver) and retry (sender)
    pub timer: timer_list,
// @bat_priv: pointer to the mesh object
    pub bat_priv: *mut batadv_priv,
// @other_end: mac address of remote
    pub other_end: [u8; ETH_ALEN],
// @session: TP session identifier
    pub session: [u8; 2],
// @refcount: number of context where the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_tp_sender_cc - congestion control variables
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tp_sender_cc {
// @fast_recovery: true if in Fast Recovery mode
    pub fast_recovery:1: bool,
// @dup_acks: duplicate ACKs counter
    pub dup_acks: u8,
// @dec_cwnd: decimal part of the cwnd used during linear growth
    pub dec_cwnd: u16,
// @cwnd: current size of the congestion window
    pub cwnd: u32,
//
// @ss_threshold: Slow Start threshold. Once cwnd exceeds this value the
// connection switches to the Congestion Avoidance state
//
    pub ss_threshold: u32,
// @last_acked: last acked byte
    pub last_acked: u32,
// @last_sent: last sent byte, not yet acked
    pub last_sent: u32,
// @recover: last sent seqno when entering Fast Recovery
    pub recover: u32,
// @rto: sender timeout
    pub rto: u32,
// @srtt: smoothed RTT scaled by 2^3
    pub srtt: u32,
// @rttvar: RTT variation scaled by 2^2
    pub rttvar: u32,
}

//
// struct batadv_tp_sender - sender tp meter private variables per session
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tp_sender {
// @common: common batadv_tp_vars_common (must be first member)
    pub common: batadv_tp_vars_common,
// @start_time: start time in jiffies
    pub start_time: c_ulong,
//
// @send_result: 0 when sending is ongoing and otherwise
// enum batadv_tp_meter_reason
//
    pub send_result: core::sync::atomic::AtomicI32,
// @finish_work: work item for the finishing procedure
    pub finish_work: delayed_work,
// @finished: completion signaled when a sender thread exits
    pub finished: completion,
// @test_length: test length in milliseconds
    pub test_length: u32,
// @icmp_uid: local ICMP "socket" index
    pub icmp_uid: u8,
// @cc: congestion control variables
    pub cc: batadv_tp_sender_cc,
// @cc_lock: lock to protect @cc
    pub cc_lock: spinlock_t,
// @tot_sent: amount of data sent/ACKed so far
    pub tot_sent: core::sync::atomic::AtomicI64,
//
// @more_bytes: waiting queue anchor when waiting for more ack/retry
// timeout
//
    pub more_bytes: wait_queue_head_t,
// @prerandom_offset: offset inside the prerandom buffer
    pub prerandom_offset: u32,
// @prerandom_lock: spinlock protecting access to prerandom_offset
    pub prerandom_lock: spinlock_t,
}

//
// struct batadv_tp_receiver - receiver tp meter private variables per session
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tp_receiver {
// @common: common batadv_tp_vars_common (must be first member)
    pub common: batadv_tp_vars_common,
// @receiving: receiving binary semaphore: 1 if receiving, 0 is not
    pub receiving: core::sync::atomic::AtomicI32,
// @last_recv: last in-order received packet
    pub last_recv: u32,
// @last_recv_time: time (jiffies) a msg was received
    pub last_recv_time: c_ulong,
// @unacked_list: list of unacked packets (meta-info only)
    pub unacked_list: list_head,
// @ack_seqno_lock: protect unacked_list + &batadv_tp_receiver.last_recv
    pub ack_seqno_lock: spinlock_t,
// @unacked_count: number of unacked entries
    pub unacked_count: usize,
}

//
// struct batadv_meshif_vlan - per VLAN attributes set
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_meshif_vlan {
// @bat_priv: pointer to the mesh object
    pub bat_priv: *mut batadv_priv,
// @vid: VLAN identifier
    pub vid: c_ushort,
// @ap_isolation: AP isolation state
    pub /: *mut *mut u8 ap_isolation; / boolean,
// @tt: TT private attributes (VLAN specific)
    pub tt: batadv_vlan_tt,
// @list: list node for &bat_priv.meshif_vlan_list
    pub list: hlist_node,
//
// @refcount: number of context where this object is currently in use
//
    pub refcount: kref,
// @rcu: struct used for freeing in a RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_priv_bat_v - B.A.T.M.A.N. V per mesh-interface private data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv_bat_v {
// @ogm_buff: buffer holding the OGM packet
    pub ogm_buff: batadv_ogm_buf,
// @ogm_seqno: OGM sequence number - used to identify each OGM
    pub ogm_seqno: core::sync::atomic::AtomicI32,
// @ogm_buff_mutex: lock protecting ogm_buff
    pub ogm_buff_mutex: mutex,
// @ogm_wq: workqueue used to schedule OGM transmissions
    pub ogm_wq: delayed_work,
}

//
// struct batadv_priv - per mesh interface data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_priv {
//
// @mesh_state: current status of the mesh
// (inactive/active/deactivating)
//
    pub mesh_state: batadv_mesh_state,
// @mesh_iface: net device which holds this struct as private data
    pub mesh_iface: *mut net_device,
//
// @mtu_set_by_user: MTU was set once by user
// protected by rtnl_lock
//
    pub mtu_set_by_user: c_int,
//
// @bat_counters: mesh internal traffic statistic counters (see
// batadv_counters)
//
    pub /: *mut *mut *mut u64 __percpu bat_counters; / Per cpu counters,
//
// @aggregated_ogms: bool indicating whether OGM aggregation is enabled
//
    pub aggregated_ogms: u8,
// @bonding: bool indicating whether traffic bonding is enabled
    pub bonding: u8,
//
// @fragmentation: bool indicating whether traffic fragmentation is
// enabled
//
    pub fragmentation: u8,

//
// @bridge_loop_avoidance: bool indicating whether bridge loop
// avoidance is enabled
//
    pub bridge_loop_avoidance: u8,

//
// @distributed_arp_table: bool indicating whether distributed ARP table
// is enabled
//
    pub distributed_arp_table: u8,

//
// @multicast_mode: Enable or disable multicast optimizations on this
// node's sender/originating side
//
    pub multicast_mode: u8,
//
// @multicast_fanout: Maximum number of packet copies to generate for a
// multicast-to-unicast conversion
//
    pub multicast_fanout: u32,

//
// @packet_size_max: max packet size that can be transmitted via
// multiple fragmented skbs or a single frame if fragmentation is
// disabled
//
    pub packet_size_max: c_int,
//
// @frag_seqno: incremental counter to identify chains of egress
// fragments
//
    pub frag_seqno: core::sync::atomic::AtomicI32,
// @orig_interval: OGM broadcast interval in milliseconds
    pub orig_interval: u32,
//
// @hop_penalty: penalty which will be applied to an OGM's tq-field on
// every hop
//
    pub hop_penalty: u8,

// @log_level: configured log level (see batadv_dbg_level)
    pub log_level: u32,

//
// @isolation_mark: the skb->mark value used to match packets for AP
// isolation
//
    pub isolation_mark: u32,
//
// @isolation_mark_mask: bitmask identifying the bits in skb->mark to be
// used for the isolation mark
//
    pub isolation_mark_mask: u32,
// @bcast_seqno: last sent broadcast packet sequence number
    pub bcast_seqno: core::sync::atomic::AtomicI32,
//
// @bcast_queue_left: number of remaining buffered broadcast packet
// slots
//
    pub bcast_queue_left: core::sync::atomic::AtomicI32,
// @batman_queue_left: number of remaining OGM packet slots
    pub batman_queue_left: core::sync::atomic::AtomicI32,
// @forw_bat_list: list of aggregated OGMs that will be forwarded
    pub forw_bat_list: hlist_head,
//
// @forw_bcast_list: list of broadcast packets that will be
// rebroadcasted
//
    pub forw_bcast_list: hlist_head,
// @tp_sender_list: list of tp sender sessions
    pub tp_sender_list: hlist_head,
// @tp_receiver_list: list of tp receiver sessions
    pub tp_receiver_list: hlist_head,
// @orig_hash: hash table containing mesh participants (orig nodes)
    pub orig_hash: *mut batadv_hashtable,
// @forw_bat_list_lock: lock protecting forw_bat_list
    pub forw_bat_list_lock: spinlock_t,
// @forw_bcast_list_lock: lock protecting forw_bcast_list
    pub forw_bcast_list_lock: spinlock_t,
// @tp_list_lock: spinlock protecting @tp_sender_list + @tp_receiver_list
    pub tp_list_lock: spinlock_t,
// @tp_num: number of currently active tp sessions
    pub tp_num: core::sync::atomic::AtomicI32,
// @hardif_generation: generation counter added to netlink hardif dumps
    pub hardif_generation: c_uint,
// @orig_work: work queue callback item for orig node purging
    pub orig_work: delayed_work,
//
// @primary_if: one of the hard-interfaces assigned to this mesh
// interface becomes the primary interface
//
    pub /: *mut *mut *mut batadv_hard_iface __rcu primary_if; / rcu protected pointer,
// @algo_ops: routing algorithm used by this mesh interface
    pub algo_ops: *mut batadv_algo_ops,
//
// @meshif_vlan_list: a list of meshif_vlan structs, one per VLAN
// created on top of the mesh interface represented by this object
//
    pub meshif_vlan_list: hlist_head,
// @meshif_vlan_list_lock: lock protecting meshif_vlan_list
    pub meshif_vlan_list_lock: spinlock_t,

// @bla: bridge loop avoidance data
    pub bla: batadv_priv_bla,

// @gw: gateway data
    pub gw: batadv_priv_gw,
// @tt: translation table data
    pub tt: batadv_priv_tt,
// @tvlv: type-version-length-value data
    pub tvlv: batadv_priv_tvlv,

// @dat: distributed arp table data
    pub dat: batadv_priv_dat,

// @mcast: multicast data
    pub mcast: batadv_priv_mcast,

// @bat_v: B.A.T.M.A.N. V per mesh-interface private data
    pub bat_v: batadv_priv_bat_v,

}

//
// enum batadv_bla_backbone_gw_state - state of a bridge loop avoidance
// backbone gateway
//
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum batadv_bla_backbone_gw_state {
//
// @BATADV_BLA_BACKBONE_GW_STOPPED: backbone gw is being removed
// and it must no longer work on requests
//
    BATADV_BLA_BACKBONE_GW_STOPPED,

//
// @BATADV_BLA_BACKBONE_GW_UNSYNCED: backbone was detected out
// of sync and a request was sent. No traffic is forwarded until the
// situation is resolved
//
    BATADV_BLA_BACKBONE_GW_UNSYNCED,

//
// @BATADV_BLA_BACKBONE_GW_SYNCED: backbone is considered to be in
// sync. traffic can be forwarded
//
    BATADV_BLA_BACKBONE_GW_SYNCED,
}

//
// struct batadv_bla_backbone_gw - batman-adv gateway bridged into the LAN
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_bla_backbone_gw {
//
// @orig: originator address of backbone node (mac address of primary
// iface)
//
    pub orig: [u8; ETH_ALEN],
// @vid: vlan id this gateway was detected on
    pub vid: c_ushort,
// @hash_entry: hlist node for &batadv_priv_bla.backbone_hash
    pub hash_entry: hlist_node,
// @bat_priv: pointer to mesh_iface this backbone gateway belongs to
    pub bat_priv: *mut batadv_priv,
// @lasttime: last time we heard of this backbone gw
    pub lasttime: c_ulong,
//
// @wait_periods: grace time for bridge forward delays and bla group
// forming at bootup phase - no bcast traffic is formwared until it has
// elapsed. Must only be access with num_requests_lock.
//
    pub wait_periods: u8,
// @state: sync state. Must only be access with num_requests_lock.
    pub state: batadv_bla_backbone_gw_state,
// @crc: crc16 checksum over all claims
    pub crc: u16,
// @crc_lock: lock protecting crc
    pub crc_lock: spinlock_t,
// @report_work: work struct for reporting detected loops
    pub report_work: work_struct,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_bla_claim - claimed non-mesh client structure
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_bla_claim {
// @addr: mac address of claimed non-mesh client
    pub addr: [u8; ETH_ALEN],
// @vid: vlan id this client was detected on
    pub vid: c_ushort,
// @backbone_gw: pointer to backbone gw claiming this client
    pub backbone_gw: *mut batadv_bla_backbone_gw,
// @backbone_lock: lock protecting backbone_gw pointer
    pub backbone_lock: spinlock_t,
// @lasttime: last time we heard of claim (locals only)
    pub lasttime: c_ulong,
// @hash_entry: hlist node for &batadv_priv_bla.claim_hash
    pub hash_entry: hlist_node,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
// @refcount: number of contexts the object is used
    pub refcount: kref,
}

//
// struct batadv_tt_common_entry - tt local & tt global common data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tt_common_entry {
// @addr: mac address of non-mesh client
    pub addr: [u8; ETH_ALEN],
// @vid: VLAN identifier
    pub vid: c_ushort,
//
// @hash_entry: hlist node for &batadv_priv_tt.local_hash or for
// &batadv_priv_tt.global_hash
//
    pub hash_entry: hlist_node,
// @flags: various state handling flags (see batadv_tt_client_flags)
    pub flags: u16,
// @added_at: timestamp used for purging stale tt common entries
    pub added_at: c_ulong,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_tt_local_entry - translation table local entry data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tt_local_entry {
// @common: general translation table data
    pub common: batadv_tt_common_entry,
// @last_seen: timestamp used for purging stale tt local entries
    pub last_seen: c_ulong,
// @vlan: mesh-interface vlan of the entry
    pub vlan: *mut batadv_meshif_vlan,
}

//
// struct batadv_tt_global_entry - translation table global entry data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tt_global_entry {
// @common: general translation table data
    pub common: batadv_tt_common_entry,
// @orig_list: list of orig nodes announcing this non-mesh client
    pub orig_list: hlist_head,
// @orig_list_count: number of items in the orig_list
    pub orig_list_count: core::sync::atomic::AtomicI32,
// @list_lock: lock protecting orig_list
    pub list_lock: spinlock_t,
// @roam_at: time at which TT_GLOBAL_ROAM was set
    pub roam_at: c_ulong,
}

//
// struct batadv_tt_orig_list_entry - orig node announcing a non-mesh client
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tt_orig_list_entry {
// @orig_node: pointer to orig node announcing this non-mesh client
    pub orig_node: *mut batadv_orig_node,
//
// @ttvn: translation table version number which added the non-mesh
// client
//
    pub ttvn: u8,
// @flags: per orig entry TT sync flags
    pub flags: u8,
// @list: list node for &batadv_tt_global_entry.orig_list
    pub list: hlist_node,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_tt_change_node - structure for tt changes occurred
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tt_change_node {
// @list: list node for &batadv_priv_tt.changes_list
    pub list: list_head,
// @change: holds the actual translation table diff data
    pub change: batadv_tvlv_tt_change,
}

//
// struct batadv_tt_req_node - data to keep track of the tt requests in flight
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tt_req_node {
//
// @addr: mac address of the originator this request was sent to
//
    pub addr: [u8; ETH_ALEN],
// @issued_at: timestamp used for purging stale tt requests
    pub issued_at: c_ulong,
// @refcount: number of contexts the object is used by
    pub refcount: kref,
// @list: list node for &batadv_priv_tt.req_list
    pub list: hlist_node,
}

//
// struct batadv_tt_roam_node - roaming client data
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tt_roam_node {
// @addr: mac address of the client in the roaming phase
    pub addr: [u8; ETH_ALEN],
// @vid: VLAN identifier
    pub vid: u16,
//
// @counter: number of allowed roaming events per client within a single
// OGM interval (changes are committed with each OGM)
//
    pub counter: core::sync::atomic::AtomicI32,
//
// @first_time: timestamp used for purging stale roaming node entries
//
    pub first_time: c_ulong,
// @list: list node for &batadv_priv_tt.roam_list
    pub list: list_head,
}

//
// struct batadv_skb_cb - control buffer structure used to store private data
// relevant to batman-adv in the skb->cb buffer in skbs.
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_skb_cb {
// @num_bcasts: Counter for broadcast packet retransmissions
    pub num_bcasts: c_uchar,
}

//
// struct batadv_forw_packet - structure for bcast packets to be sent/forwarded
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_forw_packet {
//
// @list: list node for &batadv_priv.forw.bcast_list and
// &batadv_priv.forw.bat_list
//
    pub list: hlist_node,
// @cleanup_list: list node for purging functions
    pub cleanup_list: hlist_node,
// @send_time: execution time for delayed_work (packet sending)
    pub send_time: c_ulong,
//
// @own: bool for locally generated packets (local OGMs are re-scheduled
// after sending)
//
    pub own: u8,
// @skb: bcast packet's skb buffer
    pub skb: *mut sk_buff,
// @packet_len: size of aggregated OGM packet inside the skb buffer
    pub packet_len: u16,
// @direct_link_flags: direct link flags for aggregated OGM packets
    pub BATADV_MAX_AGGREGATION_PACKETS): DECLARE_BITMAP(direct_link_flags,,
// @num_packets: counter for aggregated OGMv1 packets
    pub num_packets: u8,
// @delayed_work: work queue callback item for packet sending
    pub delayed_work: delayed_work,
//
// @if_incoming: pointer to incoming hard-iface or primary iface if
// locally generated packet
//
    pub if_incoming: *mut batadv_hard_iface,
//
// @if_outgoing: packet where the packet should be sent to, or NULL if
// unspecified
//
    pub if_outgoing: *mut batadv_hard_iface,
// @queue_left: The queue (counter) this packet was applied to
    pub queue_left: *mut core::sync::atomic::AtomicI32,
}

//
// struct batadv_algo_iface_ops - mesh algorithm callbacks (interface specific)
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_algo_iface_ops {
//
// @activate: start routing mechanisms when hard-interface is brought up
// (optional)
//
    pub hard_iface): *mut *mut void (activate)(struct batadv_hard_iface,
// @enable: init routing info when hard-interface is enabled
    pub hard_iface): *mut *mut int (enable)(struct batadv_hard_iface,
// @enabled: notification when hard-interface was enabled (optional)
    pub hard_iface): *mut *mut void (enabled)(struct batadv_hard_iface,
// @disable: de-init routing info when hard-interface is disabled
    pub hard_iface): *mut *mut void (disable)(struct batadv_hard_iface,
//
// @update_mac: (re-)init mac addresses of the protocol information
// belonging to this hard-interface
//
    pub hard_iface): *mut *mut void (update_mac)(struct batadv_hard_iface,
// @primary_set: called when primary interface is selected / changed
    pub hard_iface): *mut *mut void (primary_set)(struct batadv_hard_iface,
}

//
// struct batadv_algo_neigh_ops - mesh algorithm callbacks (neighbour specific)
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_algo_neigh_ops {
// @hardif_init: called on creation of single hop entry (optional)
    pub neigh): *mut *mut void (hardif_init)(struct batadv_hardif_neigh_node,
//
// @cmp: compare the metrics of two neighbors for their respective
// outgoing interfaces
//
    pub if_outgoing2): *mut batadv_hard_iface,
//
// @is_similar_or_better: check if neigh1 is equally similar or better
// than neigh2 for their respective outgoing interface from the metric
// prospective
//
    pub if_outgoing2): *mut batadv_hard_iface,
// @dump: dump neighbors to a netlink socket (optional)
    pub hard_iface): *mut batadv_hard_iface,
}

//
// struct batadv_algo_orig_ops - mesh algorithm callbacks (originator specific)
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_algo_orig_ops {
// @dump: dump originators to a netlink socket (optional)
    pub hard_iface): *mut batadv_hard_iface,
}

//
// struct batadv_algo_gw_ops - mesh algorithm callbacks (GW specific)
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_algo_gw_ops {
// @init_sel_class: initialize GW selection class (optional)
    pub bat_priv): *mut *mut void (init_sel_class)(struct batadv_priv,
//
// @sel_class_max: maximum allowed GW selection class
//
    pub sel_class_max: u32,
//
// @get_best_gw_node: select the best GW from the list of available
// nodes (optional)
//
    pub bat_priv): *mut (struct batadv_priv,
//
// @is_eligible: check if a newly discovered GW is a potential candidate
// for the election as best GW (optional)
//
    pub orig_node): *mut batadv_orig_node,
// @dump: dump gateways to a netlink socket (optional)
    pub priv): *mut batadv_priv,
}

//
// struct batadv_algo_ops - mesh algorithm callbacks
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_algo_ops {
// @list: list node for the batadv_algo_list
    pub list: hlist_node,
// @name: name of the algorithm
    pub name: *mut c_char,
// @iface: callbacks related to interface handling
    pub iface: batadv_algo_iface_ops,
// @neigh: callbacks related to neighbors handling
    pub neigh: batadv_algo_neigh_ops,
// @orig: callbacks related to originators handling
    pub orig: batadv_algo_orig_ops,
// @gw: callbacks related to GW mode
    pub gw: batadv_algo_gw_ops,
}

//
// struct batadv_dat_entry - it is a single entry of batman-adv ARP backend. It
// is used to stored ARP entries needed for the global DAT cache
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_dat_entry {
// @ip: the IPv4 corresponding to this DAT/ARP entry
    pub ip: __be32,
// @mac_addr: the MAC address associated to the stored IPv4
    pub mac_addr: core::sync::atomic::AtomicI64,
// @vid: the vlan ID associated to this entry
    pub vid: c_ushort,
//
// @last_update: time in jiffies when this entry was refreshed last time
//
    pub last_update: c_ulong,
// @hash_entry: hlist node for &batadv_priv_dat.hash
    pub hash_entry: hlist_node,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// struct batadv_hw_addr - a list entry for a MAC address
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_hw_addr {
// @list: list node for the linking of entries
    pub list: hlist_node,
// @addr: the MAC address of this list entry
    pub addr: [c_uchar; ETH_ALEN],
}

//
// struct batadv_dat_candidate - candidate destination for DAT operations
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_dat_candidate {
//
// @type: the type of the selected candidate. It can one of the
// following:
// - BATADV_DAT_CANDIDATE_NOT_FOUND
// - BATADV_DAT_CANDIDATE_ORIG
//
    pub type: c_int,
//
// @orig_node: if type is BATADV_DAT_CANDIDATE_ORIG this field points to
// the corresponding originator node structure
//
    pub orig_node: *mut batadv_orig_node,
}

//
// struct batadv_tvlv_container - container for tvlv appended to OGMs
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tvlv_container {
// @list: hlist node for &batadv_priv_tvlv.container_list
    pub list: hlist_node,
// @tvlv_hdr: tvlv header information needed to construct the tvlv
    pub tvlv_hdr: batadv_tvlv_hdr,
// @refcount: number of contexts the object is used
    pub refcount: kref,
}

//
// struct batadv_tvlv_handler - handler for specific tvlv type and version
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct batadv_tvlv_handler {
// @list: hlist node for &batadv_priv_tvlv.handler_list
    pub list: hlist_node,
//
// @ogm_handler: handler callback which is given the tvlv payload to
// process on incoming OGM packets
//
    pub tvlv_value_len): *mut *mut u8 flags, void tvlv_value, u16,
//
// @unicast_handler: handler callback which is given the tvlv payload to
// process on incoming unicast tvlv packets
//
    pub tvlv_value_len): *mut *mut void tvlv_value, u16,
//
// @mcast_handler: handler callback which is given the tvlv payload to
// process on incoming mcast packet
//
    pub skb): *mut *mut *mut int (mcast_handler)(struct batadv_priv bat_priv, struct sk_buff,
// @type: tvlv type this handler feels responsible for
    pub type: u8,
// @version: tvlv version this handler feels responsible for
    pub version: u8,
// @flags: tvlv handler flags
    pub flags: u8,
// @refcount: number of contexts the object is used
    pub refcount: kref,
// @rcu: struct used for freeing in an RCU-safe manner
    pub rcu: rcu_head,
}

//
// enum batadv_tvlv_handler_flags - tvlv handler flags definitions
//
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum batadv_tvlv_handler_flags {
//
// @BATADV_TVLV_HANDLER_OGM_CIFNOTFND: tvlv ogm processing function
// will call this handler even if its type was not found (with no data)
//
    BATADV_TVLV_HANDLER_OGM_CIFNOTFND = BIT(1),
}
