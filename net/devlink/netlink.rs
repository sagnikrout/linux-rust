...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct devlink_nl_sock_priv {
    pub flt: *mut devlink_obj_desc __rcu,
    pub /: *mut *mut spinlock_t flt_lock; / Protects flt.,
}

#[no_mangle]
unsafe extern "C" fn devlink_nl_sock_priv_init(priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn devlink_nl_sock_priv_destroy(priv: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn devlink_nl_notify_filter(dsk: *mut sock, skb: *mut sk_buff, data: *mut c_void) -> c_int {
    ...
    let mut id: c_int = peernet2id_alloc(net, devl_net, GFP_ATOMIC);
    rcu_read_unlock();
...
#[no_mangle]
pub unsafe extern "C" fn devlink_nl_msg_reply_and_new(msg: *mut sk_buff, info: *mut genl_info) -> c_int {
...
// msg = genlmsg_new(GENLMSG_DEFAULT_SIZE, GFP_KERNEL);
    ...
    {
    let mut maxtype: c_uint = ARRAY_SIZE(devlink_dl_parent_dev_nl_policy) - 1;
    struct devlink *devlink;
    struct nlattr **tb;
    int err;
    ...
    devlink_dl_parent_dev_nl_policy, core::ptr::null_mut());
    if (err)
    goto out;
    ...
    u8 flags)
    {
    let mut parent_dev: bool = flags & DEVLINK_NL_FLAG_OPTIONAL_PARENT_DEV;
    let mut dev_lock: bool = flags & DEVLINK_NL_FLAG_NEED_DEV_LOCK;
    struct devlink *devlink, *parent_devlink = core::ptr::null_mut();
...
// restart sub-object walk for the next instance
    ...
    struct nlattr **attrs = info.attrs;
    let mut flags: c_int = NLM_F_MULTI;
    if (attrs &&
