...
#[no_mangle]
pub unsafe extern "C" fn devlink_rate_is_node(devlink_rate: *const devlink_rate) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: attrs[DEVLINK_ATTR_RATE_NODE_NAME]) -> else {
...
#[no_mangle]
unsafe extern "C" fn devlink_rate_put_tc_bws(msg: *mut sk_buff, tc_bw: *mut u32) -> c_int {
    ...
    if (!msg)
    return;
    err = devlink_nl_rate_fill(msg, devlink_rate, cmd, 0, 0, 0, core::ptr::null_mut());
    if (err) {
    nlmsg_free(msg);
    return;
    }
...
#[no_mangle]
pub unsafe extern "C" fn devlink_rates_notify_register(devlink: *mut devlink) {
...
#[no_mangle]
pub unsafe extern "C" fn devlink_rates_notify_unregister(devlink: *mut devlink) {
    ...
    rate_devlink = devl_rate_lock(devlink);
    list_for_each_entry(devlink_rate, &rate_devlink.rate_list, list) {
    let mut cmd: enum devlink_command = DEVLINK_CMD_RATE_NEW;
    let mut id: u32 = NETLINK_CB(cb.skb).portid;
    if (idx < state.idx || devlink_rate.devlink != devlink) {
    idx++;
    continue;
    }
    err = devlink_nl_rate_fill(msg, devlink_rate, cmd, id,
    cb.nlh.nlmsg_seq, flags, core::ptr::null_mut());
    if (err) {
    state.idx = idx;
    break;
    }
...
#[no_mangle]
pub unsafe extern "C" fn devlink_nl_rate_get_dumpit(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn devlink_nl_rate_get_doit(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
    ...
    let mut len: usize = strlen(parent_name);
    struct devlink_rate *parent;
    ...
    if (parent && !len) {
    if (devlink_rate_is_leaf(devlink_rate))
    err = ops.rate_leaf_parent_set(devlink_rate, core::ptr::null_mut(),
    devlink_rate.priv, core::ptr::null_mut(),
    info.extack);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: devlink_rate_is_node(devlink_rate)) -> else {
    else if (devlink_rate_is_node(devlink_rate))
    err = ops.rate_node_parent_set(devlink_rate, core::ptr::null_mut(),
    devlink_rate.priv, core::ptr::null_mut(),
    info.extack);
    if (err)
    return err;
    refcount_dec(&parent.refcnt);
    devlink_rate.parent = core::ptr::null_mut();
    } else if (len) {
...
#[no_mangle]
pub unsafe extern "C" fn devlink_nl_rate_set_doit(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn devlink_nl_rate_new_doit(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn devlink_nl_rate_del_doit(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn devl_rate_leaf_destroy(devlink_port: *mut devlink_port) {
    ...
    devlink_port.devlink_rate = core::ptr::null_mut();
    devl_rate_unlock(devlink, rate_devlink);
    kfree(devlink_rate);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn devl_rate_nodes_destroy(devlink: *mut devlink) {
...
//
    ...
    if (devlink_rate_is_leaf(devlink_rate))
    ops.rate_leaf_parent_set(devlink_rate, core::ptr::null_mut(), devlink_rate.priv,
    core::ptr::null_mut(), core::ptr::null_mut());
#[no_mangle]
pub unsafe extern "C" fn if(_arg: devlink_rate_is_node(devlink_rate)) -> else {
    else if (devlink_rate_is_node(devlink_rate))
    ops.rate_node_parent_set(devlink_rate, core::ptr::null_mut(), devlink_rate.priv,
    core::ptr::null_mut(), core::ptr::null_mut());
    refcount_dec(&devlink_rate.parent.refcnt);
    devlink_rate.parent = core::ptr::null_mut();
    }
    ...
    if (devlink_rate.devlink == devlink &&
    devlink_rate_is_node(devlink_rate)) {
    ops.rate_node_del(devlink_rate, devlink_rate.priv, core::ptr::null_mut());
    list_del(&devlink_rate.list);
