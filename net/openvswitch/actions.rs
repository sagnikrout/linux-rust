...
//! Source: net/openvswitch/actions.c
#![no_std]
...
//
    ...
    let mut level: c_int = ovs_pcpu.exec_level;
    struct sw_flow_key *key = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn action_fifo_init(fifo: *mut action_fifo) {
...
#[no_mangle]
unsafe extern "C" fn action_fifo_is_empty(fifo: *const action_fifo) -> bool {
...
#[no_mangle]
unsafe extern "C" fn invalidate_flow_key(key: *mut sw_flow_key) {
    static void invalidate_flow_key(struct sw_flow_key *key)
    {
    key.mac_proto |= SW_FLOW_KEY_INVALID;
    }
#[no_mangle]
unsafe extern "C" fn is_flow_key_valid(key: *const sw_flow_key) -> bool {
...
#[no_mangle]
unsafe extern "C" fn pop_vlan(skb: *mut sk_buff, key: *mut sw_flow_key) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ether_addr_copy_masked(dst_: *mut u8, src_: *const u8, mask_: *const u8) {
...
//
#[no_mangle]
unsafe extern "C" fn pop_eth(skb: *mut sk_buff, key: *mut sw_flow_key) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pop_nsh(skb: *mut sk_buff, key: *mut sw_flow_key) -> c_int {
...
// safe right before invalidate_flow_key
    ...
    {
    let mut transport_len: c_int = skb.len - skb_transport_offset(skb);
    ...
    skb_clear_hash(skb);
    ovs_ct_clear(skb, core::ptr::null_mut());
// addr = new_addr;
    }
...
#[no_mangle]
unsafe extern "C" fn set_ipv6_dsfield(skb: *mut sk_buff, nh: *mut ipv6hdr, ipv6_tclass: u8, mask: u8) {
    static void set_ipv6_dsfield(struct sk_buff *skb, struct ipv6hdr *nh, u8 ipv6_tclass, u8 mask)
    {
    let mut old_ipv6_tclass: u8 = ipv6_get_dsfield(nh);
    ipv6_tclass = OVS_MASKED(old_ipv6_tclass, ipv6_tclass, mask);
    if (skb.ip_summed == CHECKSUM_COMPLETE)
    csum_replace(&skb.csum, ( __wsum)(old_ipv6_tclass << 12),
    ( __wsum)(ipv6_tclass << 12));
    ipv6_change_dsfield(nh, ~mask, ipv6_tclass);
    }
#[no_mangle]
unsafe extern "C" fn set_ipv6_fl(skb: *mut sk_buff, nh: *mut ipv6hdr, fl: u32, mask: u32) {
...
// Bits 21-24 are always unmasked, so this retains their values.
    ...
    nh.flow_lbl[2] = (u8)fl;
    if (skb.ip_summed == CHECKSUM_COMPLETE)
    csum_replace(&skb.csum, ( __wsum)htonl(ofl), ( __wsum)htonl(fl));
    }
#[no_mangle]
unsafe extern "C" fn set_ipv6_ttl(skb: *mut sk_buff, nh: *mut ipv6hdr, new_ttl: u8, mask: u8) {
    static void set_ipv6_ttl(struct sk_buff *skb, struct ipv6hdr *nh, u8 new_ttl, u8 mask)
    {
    new_ttl = OVS_MASKED(nh.hop_limit, new_ttl, mask);
    if (skb.ip_summed == CHECKSUM_COMPLETE)
    csum_replace(&skb.csum, ( __wsum)(nh.hop_limit << 8),
    ( __wsum)(new_ttl << 8));
...
#[no_mangle]
unsafe extern "C" fn is_ipv6_mask_nonzero(addr[4]: __be32) -> bool {
...
//
    ...
    let mut flags: c_int = IP6_FH_F_SKIP_RH;
    let mut recalc_csum: bool = true;
...
// Either of the masks is non-zero, so do not bother checking them.
    ...
    {
    let mut sctphoff: c_uint = skb_transport_offset(skb);
    struct sctphdr *sh;
...
//
    ...
    let mut orig_network_offset: u16 = 0;
    if (eth_p_mpls(skb.protocol)) {
    ...
    let mut ovs_rt: rtable = { 0 };
    unsigned long orig_dst;
    prepare_frag(vport, skb, orig_network_offset,
    ovs_key_mac_proto(key));
    dst_init(&ovs_rt.dst, &ovs_dst_ops, core::ptr::null_mut(),
    ...
    let mut mru: u16 = OVS_CB(skb).mru;
    let mut cutlen: u32 = OVS_CB(skb).cutlen;
...
//
    ...
    let mut rem: c_int = nla_len(attr);
...
//
    ...
    return clone_execute(dp, skb, key, recirc_id, core::ptr::null_mut(), 0, last, true);
    }
...
#[no_mangle]
unsafe extern "C" fn execute_dec_ttl(skb: *mut sk_buff, key: *mut sw_flow_key) -> c_int {
    ...
    const struct nlattr *attr)
    {
    let mut psample_group: psample_group = {};
...
//
    ...
    let mut port: c_int = nla_get_u32(a);
...
//
    ...
    let mut last: bool = nla_is_last(a, rem);
...
// Hide stolen IP fragments from user space.
    if (err)
    let mut err: return = = -EINPROGRESS ? 0 : err;
    break;
...
#[no_mangle]
unsafe extern "C" fn process_deferred_actions(dp: *mut datapath) {
...
// Finishing executing all deferred actions.
    ...
    let mut actions_len: c_int = da.actions_len;
    if (actions)
