...
//! Source: net/netfilter/nft_limit.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn nft_limit_eval(priv: *mut nft_limit_priv, cost: u64) -> bool {
...
pub const NFT_LIMIT_PKT_BURST_DEFAULT: c_int = 5;
    static int nft_limit_init(struct nft_limit_priv *priv,
    const struct nlattr * const tb[], bool pkts)
    {
    u64 unit, tokens, rate_with_burst;
    let mut invert: bool = false;
    if (tb[NFTA_LIMIT_RATE] == core::ptr::null_mut() ||
    tb[NFTA_LIMIT_UNIT] == core::ptr::null_mut())
    return -EINVAL;
    ...
    return -EOVERFLOW;
    if (pkts) {
    let mut tmp: u64 = div64_u64(priv.nsecs, priv.rate);
...
//
    ...
    let mut flags: u32 = ntohl(nla_get_be32(tb[NFTA_LIMIT_FLAGS]));
    ...
    {
    let mut flags: u32 = priv.invert ? NFT_LIMIT_F_INV : 0;
    let mut secs: u64 = div_u64(priv.nsecs, NSEC_PER_SEC);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_limit_priv_pkts {
    pub limit: nft_limit_priv,
    pub cost: u64,
}
    ...
    let mut cost: u64 = div64_u64(priv.nsecs * pkt.skb.len, priv.rate);
    ...
    const struct nlattr * const tb[])
    {
    if (tb[NFTA_LIMIT_TYPE] == core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn nft_limit_module_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nft_limit_module_exit() -> void __exit {
