...
pub const NF_NAT_MAX_ATTEMPTS: c_int = 128;
...
#[no_mangle]
unsafe extern "C" fn __nf_nat_decode_session(skb: *mut sk_buff, fl: *mut flowi) {
...
#[no_mangle]
unsafe extern "C" fn nf_nat_allow_clash(ct: *const nf_conn) -> bool {
...
//
    ...
    {
    let mut uses_nat: static unsigned long = IPS_NAT_MASK | IPS_SEQ_ADJUST;
...
#[no_mangle]
unsafe extern "C" fn nf_nat_may_kill(ct: *mut nf_conn, flags: c_ulong) -> bool {
    ...
    IPS_DYING;
    let mut flags_needed: static unsigned long = IPS_SRC_NAT;
...
//
#[no_mangle]
unsafe extern "C" fn nf_seq_has_advanced(old: *const nf_conn, new: *const nf_conn) -> bool {
    ...
    {
    let mut flags_offload: static unsigned long = IPS_OFFLOAD | IPS_HW_OFFLOAD;
...
// Only called for SRC manip
    ...
    const struct nf_nat_range2 *range)
    {
    let mut h: c_uint = hash_by_src(net, zone, tuple);
    const struct nf_conn *ct;
...
//
    ...
    0 : ( u32)tuple.dst.u3.all[max] ^ zone.id);
    full_range = false;
    for (i = 0; i <= max; i++) {
...
//
    if (!full_range) {
    minip = ntohl(( __be32)range.min_addr.all[i]);
    maxip = ntohl(( __be32)range.max_addr.all[i]);
    dist  = maxip - minip + 1;
    } else {
    minip = 0;
    dist  = ~0;
...
// Do packet manipulations according to nf_nat_setup_info.
    ...
    struct sk_buff *skb)
    {
    let mut mtype: enum nf_nat_manip_type = HOOK2MANIP(hooknum);
    let mut dir: enum ip_conntrack_dir = CTINFO2DIR(ctinfo);
    let mut verdict: c_uint = NF_ACCEPT;
    unsigned long statusbit;
...
#[no_mangle]
unsafe extern "C" fn in_vrf_postrouting(state: *const nf_hook_state) -> bool {
...
// maniptype == SRC for postrouting.
    let mut maniptype: enum nf_nat_manip_type = HOOK2MANIP(state.hook);
    ct = nf_ct_get(skb, &ctinfo);
...
#[no_mangle]
unsafe extern "C" fn nf_nat_proto_remove(i: *mut nf_conn, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nf_nat_cleanup_conntrack(ct: *mut nf_conn) {
...
#[no_mangle]
unsafe extern "C" fn nf_nat_proto_clean(ct: *mut nf_conn, data: *mut c_void) -> c_int {
...
//
    ...
    protonat_nla_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    ...
    nat_nla_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
...
// No NAT information has been passed, allocate the null-binding
    ...
    let mut hooknum: c_uint = ops.hooknum;
...
//
    ...
    struct nf_nat_lookup_hook_priv *priv;
    struct nf_hook_ops *nat_ops;
    let mut hooknum: c_int = ops.hooknum;
    int i;
    ...
    nat_proto_net.nat_hook_ops = core::ptr::null_mut();
    kfree_rcu(nat_ops, rcu);
    }
    unlock:
    mutex_unlock(&nf_nat_proto_mutex);
...
#[no_mangle]
unsafe extern "C" fn nf_nat_init() -> int __init {
...
// Leave them the same for the moment.
    ...
    WARN_ON(nf_nat_hook != core::ptr::null_mut());
    RCU_INIT_POINTER(nf_nat_hook, &nat_hook);
    ret = register_nf_nat_bpf();
    if (ret < 0) {
    RCU_INIT_POINTER(nf_nat_hook, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn nf_nat_cleanup() -> void __exit {
    static void __exit nf_nat_cleanup(void)
    {
    let mut clean: nf_nat_proto_clean = {};
