...
//! Source: net/netfilter/nft_payload.c
#![no_std]
...
// add vlan header into the user buffer for if tag was removed by offloads
    ...
    {
    let mut mac_off: c_int = skb_mac_header(skb) - skb.data;
    u8 *vlanh, *dst_u8 = (u8 *) d;
    struct vlan_ethhdr veth;
    vlanh = (u8 *) &veth;
    if (offset < VLAN_ETH_HLEN) {
    let mut ethlen: u8 = len;
...
#[no_mangle]
unsafe extern "C" fn __nft_payload_inner_offset(pkt: *mut nft_pktinfo) -> c_int {
    static int __nft_payload_inner_offset(struct nft_pktinfo *pkt)
    {
    let mut thoff: c_uint = nft_thoff(pkt);
    ...
    break;
    case IPPROTO_GRE: {
    let mut offset: u32 = sizeof(struct gre_base_hdr);
...
#[no_mangle]
pub unsafe extern "C" fn nft_payload_inner_offset(pkt: *const nft_pktinfo) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nft_payload_need_vlan_adjust(offset: u32, len: u32) -> bool {
    static bool nft_payload_need_vlan_adjust(u32 offset, u32 len)
    {
    let mut boundary: c_uint = offset + len;
...
#[no_mangle]
pub unsafe extern "C" fn nft_csum_replace(sum: *mut __sum16, fsum: __wsum, tsum: __wsum) {
    static inline void nft_csum_replace(__sum16 *sum, __wsum fsum, __wsum tsum)
    {
    csum_replace4(sum, ( __be32)fsum, ( __be32)tsum);
    if (*sum == 0)
// sum = CSUM_MANGLED_0;
    }
#[no_mangle]
unsafe extern "C" fn nft_payload_udp_checksum(skb: *mut sk_buff, thoff: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nft_payload_csum_sctp(skb: *mut sk_buff, offset: c_int) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_payload_set {
    pub base:8: enum nft_payload_bases,
    pub offset: u16,
    pub len: u8,
    pub sreg: u8,
    pub csum_type: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_payload_vlan_hdr {
...
//
#[no_mangle]
unsafe extern "C" fn nft_in_ingress(s: *const nf_hook_state) -> bool {
    ...
    const u32 *src)
    {
    let mut offset: c_uint = priv.offset + skb_network_offset(pkt.skb);
...
#[no_mangle]
unsafe extern "C" fn nft_nh_write_ok_arp(priv: *const nft_payload_set) -> bool {
    static bool nft_nh_write_ok_arp(const struct nft_payload_set *priv)
    {
// Variable size for standard ethernet arp
    let mut eth_ip: c_uint = 2 * (ETH_ALEN + 4);
    let mut offset: c_uint = priv.offset;
    switch (offset) {
...
#[no_mangle]
unsafe extern "C" fn nft_ll_write_ok(pkt: *const nft_pktinfo, offset: c_int) -> bool {
...
//
    ...
    const struct nft_payload_set *priv)
    {
    let mut doff: c_uint = offsetof(struct tcphdr, ack_seq) + sizeof(__be32);
...
//
    ...
    int err;
    if (tb[NFTA_PAYLOAD_BASE] == core::ptr::null_mut() ||
    tb[NFTA_PAYLOAD_OFFSET] == core::ptr::null_mut() ||
    tb[NFTA_PAYLOAD_LEN] == core::ptr::null_mut())
    ...
    if (tb[NFTA_PAYLOAD_SREG] != core::ptr::null_mut()) {
    if (tb[NFTA_PAYLOAD_DREG] != core::ptr::null_mut())
    return ERR_PTR(-EINVAL);
