...
//! Source: net/netfilter/nft_set_rbtree.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_array_interval {
    pub from: *mut nft_set_ext,
    pub to: *mut nft_set_ext,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_array {
    pub max_intervals: u32,
    pub num_intervals: u32,
    pub intervals: *mut nft_array_interval,
    pub rcu_head: rcu_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_rbtree {
    pub root: rb_root,
    pub lock: rwlock_t,
    pub array: *mut nft_array __rcu,
    pub array_next: *mut nft_array,
    pub start_rbe_cookie: c_ulong,
    pub last_gc: c_ulong,
    pub expired: list_head,
    pub last_tstamp: u64,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_rbtree_elem {
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_interval_end(rbe: *const nft_rbtree_elem) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_interval_start(rbe: *const nft_rbtree_elem) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_array_lookup_ctx {
    pub key: *const u32,
    pub klen: u32,
}

#[no_mangle]
unsafe extern "C" fn nft_array_lookup_cmp(pkey: *const c_void, entry: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nft_array_get_cmp(pkey: *const c_void, entry: *const c_void) -> c_int {
    ...
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(0: b <) -> else {
    else if (b < 0)
    return 0;
    }
    if (a < 0)
...
// Closest start element differs from last element added.
    ...
    {
    struct nft_rbtree_elem *rbe, *rbe_le = core::ptr::null_mut(), *rbe_ge = core::ptr::null_mut(), *rbe_prev;
    struct rb_node *node, *next, *parent, **p, *first = core::ptr::null_mut();
    struct nft_rbtree *priv = nft_set_priv(set);
    let mut cur_genmask: u8 = nft_genmask_cur(net);
    let mut genmask: u8 = nft_genmask_next(net);
    int d;
...
//
    for (node = first; node != core::ptr::null_mut(); node = next) {
    next = rb_next(node);
...
// annotate element less than the new element.
    ...
    priv.start_rbe_cookie = 0;
#[no_mangle]
pub unsafe extern "C" fn if(priv->start_rbe_cookie: nft_rbtree_interval_start(new) &&) -> else {
...
// Accepted element: pick insertion point depending on key value
    ...
    p = &parent.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(0: d >) -> else {
    else if (d > 0)
    p = &parent.rb_right;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: nft_rbtree_interval_end(rbe)) -> else {
...
#[no_mangle]
unsafe extern "C" fn nft_array_intervals_alloc(array: *mut nft_array, max_intervals: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn nft_array_elems(set: *const nft_set) -> u32 {
    static u32 nft_array_elems(const struct nft_set *set)
    {
    let mut nelems: u32 = atomic_read(&set.nelems) - set.ndeact;
...
pub const NFT_ARRAY_INITIAL_SIZE: c_int = 1024;
pub const NFT_ARRAY_INITIAL_ANON_SIZE: c_int = 16;

#[no_mangle]
unsafe extern "C" fn nft_array_may_resize(set: *const nft_set, flush: bool) -> c_int {
...
// Set flush just started, nelems still report elements.
    ...
    struct nft_rbtree *priv = nft_set_priv(set);
    let mut tstamp: u64 = nft_net_tstamp(net);
    int err;
...
// Closest start element differs from last element added.
    ...
    if (nft_array_may_resize(set, false) < 0)
    return core::ptr::null_mut();
    while (parent != core::ptr::null_mut()) {
    rbe = rb_entry(parent, struct nft_rbtree_elem, node);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !nft_rbtree_deactivate_same_interval(net, _arg: priv, _arg: rbe)) -> else {
    ...
    struct nft_rbtree_elem *rbe;
    struct rb_node *node;
    for (node = rb_first(&priv.root); node != core::ptr::null_mut(); node = rb_next(node)) {
    rbe = rb_entry(node, struct nft_rbtree_elem, node);
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_gc_scan(set: *mut nft_set) {
    static void nft_rbtree_gc_scan(struct nft_set *set)
    {
    struct nft_rbtree *priv = nft_set_priv(set);
    struct nft_rbtree_elem *rbe, *rbe_end = core::ptr::null_mut();
    struct net *net = read_pnet(&set.net);
    let mut tstamp: u64 = nft_net_tstamp(net);
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_gc_queue(set: *mut nft_set) {
    ...
    priv.array = core::ptr::null_mut();
    priv.array_next = core::ptr::null_mut();
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn __nft_array_free(array: *mut nft_array) {
    ...
    while ((node = priv.root.rb_node) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn nft_array_free_rcu(rcu_head: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_abort(set: *const nft_set) {
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_gc_init(set: *const nft_set) {
...
//
#[no_mangle]
unsafe extern "C" fn nft_rbtree_ksize(size: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_usize(size: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn nft_rbtree_adjust_maxsize(set: *const nft_set) -> u32 {
