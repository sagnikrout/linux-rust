...
//! Source: net/netfilter/ipset/ip_set_hash_netportnet.c
#![no_std]
...
pub const IPSET_NET_COUNT: c_int = 2;
// IPv4 variant
// Member elements
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hash_netportnet4_elem {
    union {
    pub ip: [__be32; 2],
    pub ipcmp: __be64,
}
...
pub const HOST_MASK: c_int = 32;
    ...
    let mut e: hash_netportnet4_elem = { };
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hash_netportnet6_elem {
    pub ip: [union nf_inet_addr; 2],
    pub port: __be16,
    union {
    pub cidr: [u8; 2],
    pub ccmp: u16,
...
// Macro flag: #define IP_SET_EMIT_CREATE
    ...
    let mut adtfn: ipset_adtfn = set.variant.adt[adt];
    let mut e: hash_netportnet6_elem = { };
    let mut ext: ip_set_ext = IP_SET_INIT_KEXT(skb, opt, set);
    rcu_read_lock_bh();
    e.cidr[0] = INIT_CIDR(h.rnets[0], HOST_MASK);
    e.cidr[1] = INIT_CIDR(h.rnets[1], HOST_MASK);
