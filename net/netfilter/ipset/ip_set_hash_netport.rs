...
//! Source: net/netfilter/ipset/ip_set_hash_netport.c
#![no_std]
...
pub const HOST_MASK: c_int = 32;
    ...
    let mut adtfn: ipset_adtfn = set.variant.adt[adt];
    let mut e: hash_netport4_elem = { .cidr = HOST_MASK - 1 };
...
// Macro flag: #define IP_SET_EMIT_CREATE
    ...
    const struct hash_netport6 *h = set.data;
    let mut adtfn: ipset_adtfn = set.variant.adt[adt];
    let mut e: hash_netport6_elem = { .cidr = HOST_MASK  - 1 };
    let mut ext: ip_set_ext = IP_SET_INIT_UEXT(set);
    u32 port, port_to;
    let mut with_ports: bool = false;
    u8 cidr;
