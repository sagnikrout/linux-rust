...
//! Source: net/netfilter/ipset/ip_set_core.c
#![no_std]
...
pub struct ip_set_net {
    pub /: *mut *mut *mut *mut ip_set  __rcu ip_set_list; / all individual sets,
    pub /: *mut *mut ip_set_id_t ip_set_max; / max number of sets,
    pub /: *mut *mut bool is_deleted; / deleted by ip_set_net_exit,
    pub /: *mut *mut bool is_destroyed; / all sets are destroyed,
}
...
// Utility functions
    ...
    ipaddr_policy, core::ptr::null_mut()))
    return -IPSET_ERR_PROTOCOL;
...
//
    ...
    let mut len: usize = ext.comment ? strlen(ext.comment) : 0;
    if (unlikely(c)) {
    atomic64_sub(sizeof(*c) + strlen(c.str) + 1, &set.ext_size);
    rcu_assign_pointer(comment.c, core::ptr::null_mut());
    kfree_rcu(c, rcu);
    }
    if (!len)
    return;
...
//
#[no_mangle]
pub unsafe extern "C" fn protocol(tb[]: *const *const nlattr) -> u8 {
...
// Create a set
    ...
    {
    struct ip_set *set = core::ptr::null_mut();
    ip_set_id_t i;
// id = IPSET_INVALID_ID;
    for (i = 0; i < inst.ip_set_max; i++) {
    set = ip_set(inst, i);
...
// id = i;
    ...
    return (*id == IPSET_INVALID_ID ? core::ptr::null_mut() : set);
    }
...
// No free slot remained
    ...
    struct ip_set *set, *clash = core::ptr::null_mut();
    let mut index: ip_set_id_t = IPSET_INVALID_ID;
    struct nlattr *tb[IPSET_ATTR_CREATE_MAX + 1] = {};
    const char *name, *typename;
    u8 family, revision;
    let mut flags: u32 = flag_exist(info.nlh);
    let mut ret: c_int = 0;
...
// Without holding any locks, create private part.
    ...
    set.type.create_policy, core::ptr::null_mut())) {
...
// If this is the same set and requested, ignore error
    ...
    let mut i: ip_set_id_t = inst.ip_set_max + IP_SET_INC;
...
// Must wait for flush to be really finished
    ...
    set = ip_set(inst, i);
    if (set) {
    ip_set(inst, i) = core::ptr::null_mut();
    set.variant.destroy(set);
    module_put(set.type.me);
    kfree(set);
    }
...
// List/save set data
pub const DUMP_INIT: c_int = 0;
pub const DUMP_ALL: c_int = 1;
pub const DUMP_ONE: c_int = 2;
pub const DUMP_LAST: c_int = 3;
    ...
    let mut index: ip_set_id_t = (ip_set_id_t)cb.args[IPSET_CB_INDEX];
    struct ip_set *set;
    rcu_read_lock();
    ...
    ip_set_dump_policy, core::ptr::null_mut());
    if (ret)
    goto error;
    ...
    let mut f: u32 = ip_set_get_h32(cda[IPSET_ATTR_FLAGS]);
    dump_type |= (f << 16);
    }
...
// We have to create and send the error message manually :-(
    ...
    netlink_ack(cb.skb, nlh, ret, core::ptr::null_mut());
    }
    return ret;
    }
    static int
    ...
    {
    let mut index: ip_set_id_t = IPSET_INVALID_ID, max;
    struct ip_set *set = core::ptr::null_mut();
    struct nlmsghdr *nlh = core::ptr::null_mut();
    let mut flags: c_uint = NETLINK_CB(cb.skb).portid ? NLM_F_MULTI : 0;
    struct ip_set_net *inst = ip_set_pernet(sock_net(skb.sk));
...
// Add, del and test
    ...
    let mut eexist: bool = flags & IPSET_FLAG_EXIST, retried = false;
    do {
    if (retried) {
...
// Signal netlink not to send its ACK/errmsg.
    ...
    let mut flags: u32 = flag_exist(nlh);
    bool use_lineno;
    let mut ret: c_int = 0;
    if (unlikely(protocol_min_failed(attr) ||
    !attr[IPSET_ATTR_SETNAME] ||
    !((attr[IPSET_ATTR_DATA] != core::ptr::null_mut()) ^
    (attr[IPSET_ATTR_ADT] != core::ptr::null_mut())) ||
    (attr[IPSET_ATTR_DATA] &&
    !flag_nested(attr[IPSET_ATTR_DATA])) ||
    (attr[IPSET_ATTR_ADT] &&
    ...
    attr[IPSET_ATTR_DATA],
    set.type.adt_policy, core::ptr::null_mut()))
    return -IPSET_ERR_PROTOCOL;
...
// Get set by name or index, from userspace
    ...
    struct sk_buff *skb2;
    struct nlmsghdr *nlh2;
    let mut id: ip_set_id_t = IPSET_INVALID_ID;
    const struct ip_set *set;
...
// Interface to iptables/ip6tables
    ...
    unsigned int *op;
    void *data;
    let mut copylen: c_int = *len, ret = 0;
