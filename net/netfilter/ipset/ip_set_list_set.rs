...
//! Source: net/netfilter/ipset/ip_set_list_set.c
#![no_std]
...
pub struct set_elem {
    pub rcu: rcu_head,
    pub list: list_head,
    pub /: *mut *mut *mut ip_set set; / Sigh, in order to cleanup reference,
    pub id: ip_set_id_t,
    pub __aligned(__alignof__(u64)): },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct set_adt_elem {
    pub id: ip_set_id_t,
    pub refid: ip_set_id_t,
    pub before: c_int,
}
...
#[derive(Copy, Clone)]
pub struct list_set {
    pub /: *mut *mut u32 size; / size of set list array,
    pub /: *mut *mut timer_list gc; / garbage collection,
    pub /: *mut *mut *mut ip_set set; / attached to this ip_set,
    pub /: *mut *mut *mut net net; / namespace,
    pub /: *mut *mut list_head members; / the set members,
}
    ...
    struct set_elem *e;
    let mut flags: u32 = opt.cmdflags;
    int ret;
...
// Userspace interfaces: we are protected by the nfnl mutex
    ...
    struct set_elem *e, *next, *prev = core::ptr::null_mut();
    let mut ret: c_int = 0;
    rcu_read_lock();
    list_for_each_entry_rcu(e, &map.members, list) {
    if (SET_WITH_TIMEOUT(set) &&
    ...
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(d->id: e->id !=) -> else {
    prev = e;
    continue;
    }
    if (d.before == 0) {
...
// Update timeout last
    ...
    let mut flag_exist: bool = flags & IPSET_FLAG_EXIST;
// Find where to add the new entry
    n = prev = next = core::ptr::null_mut();
    list_for_each_entry_rcu(e, &map.members, list) {
    if (SET_WITH_TIMEOUT(set) &&
    ip_set_timeout_expired(ext_timeout(e, set)))
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(e->id: d->id ==) -> else {
    else if (d.id == e.id)
    n = e;
#[no_mangle]
pub unsafe extern "C" fn if(d->refid: d->before == 0 || e->id !=) -> else {
    else if (d.before == 0 || e.id != d.refid)
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(0: d->before >) -> else {
    else if (d.before > 0)
    next = e;
    else
    prev = e;
...
// Add new entry
    if (d.before == 0) {
// Append
    n = list_empty(&map.members) ? core::ptr::null_mut() :
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: prev) -> else {
    else if (prev)
    ...
    struct list_set *map = set.data;
    struct set_adt_elem *d = value;
    struct set_elem *e, *n, *next, *prev = core::ptr::null_mut();
    list_for_each_entry_safe(e, n, &map.members, list) {
    if (SET_WITH_TIMEOUT(set) &&
    ip_set_timeout_expired(ext_timeout(e, set)))
    continue;
...
pub unsafe extern "C" fn if(d->id: e->id !=) -> else {
    ...
    let mut e: set_adt_elem = { .refid = IPSET_INVALID_ID };
...
// "Loop detection"
    ...
    let mut f: u32 = ip_set_get_h32(tb[IPSET_ATTR_CADT_FLAGS]);
...
// Calculate the actual memory size of the set data
    ...
    let mut memsize: usize = list_set_memsize(map, set.dsize) + atomic64_read(&set.ext_size);
    ...
    let mut i: u32 = 0, first = cb.args[IPSET_CB_ARG0];
...
// Create list:set type of sets
    ...
    u32 flags)
    {
    let mut size: u32 = IP_SET_LIST_DEFAULT_SIZE;
