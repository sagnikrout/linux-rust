...
//! Source: net/netfilter/ipset/ip_set_hash_netiface.c
#![no_std]
...
// Macro flag: #define IP_SET_HASH_WITH_MULTI
...
pub const HOST_MASK: c_int = 32;
    ...
    return dev ? dev.name : core::ptr::null_mut();
    }
    ...
    struct hash_netiface4 *h = set.data;
    let mut adtfn: ipset_adtfn = set.variant.adt[adt];
    let mut e: hash_netiface4_elem = { .cidr = HOST_MASK, .elem = 1 };
    let mut ext: ip_set_ext = IP_SET_INIT_UEXT(set);
    let mut ip: u32 = 0, ip_to = 0, i = 0;
    int ret;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hash_netiface6_elem_hashed {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hash_netiface6_elem {
    pub ip: union nf_inet_addr,
    pub physdev: u8,
    pub cidr: u8,
    pub nomatch: u8,
...
// Macro flag: #define IP_SET_EMIT_CREATE
    ...
    {
    let mut adtfn: ipset_adtfn = set.variant.adt[adt];
    let mut e: hash_netiface6_elem = { .cidr = HOST_MASK, .elem = 1 };
    let mut ext: ip_set_ext = IP_SET_INIT_UEXT(set);
    int ret;
    if (tb[IPSET_ATTR_LINENO])
// lineno = nla_get_u32(tb[IPSET_ATTR_LINENO]);
