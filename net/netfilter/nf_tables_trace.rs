...
//! Source: net/netfilter/nf_tables_trace.c
#![no_std]
...
pub const NFT_TRACETYPE_LL_HSIZE: c_int = 20;
pub const NFT_TRACETYPE_NETWORK_HSIZE: c_int = 40;
pub const NFT_TRACETYPE_TRANSPORT_HSIZE: c_int = 20;
    ...
    return -1;
    if (ct) {
    let mut id: u32 = ct_hook.get_id(&ct.ct_general);
    let mut status: u32 = READ_ONCE(ct.status);
    let mut dir: u8 = CTINFO2DIR(ctinfo);
    if (nla_put_u8(nlskb, NFTA_TRACE_CT_DIRECTION, dir))
    return -1;
    if (nla_put_be32(nlskb, NFTA_TRACE_CT_ID, ( __be32)id))
    return -1;
