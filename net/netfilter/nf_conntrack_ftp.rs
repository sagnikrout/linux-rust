...
//! Source: net/netfilter/nf_conntrack_ftp.c
#![no_std]
...
//
    ...
    {
    const char *end;
    let mut ret: c_int = in6_pton(src, min_t(size_t, dlen, 0xffff), (u8 *)dst, term, &end);
    if (ret > 0)
    return (int)(end - src);
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(sep: *mut *mut data ==) -> else {
    else if (*data == sep)
    i++;
    else {
...
#[no_mangle]
unsafe extern "C" fn find_nl_seq(seq: u32, info: *const nf_ct_ftp_master, dir: c_int) -> c_int {
...
// Look for oldest: if we find exact match, we're done.
    ...
    unsigned int i;
    let mut found: c_int = 0, ends_in_nl;
...
// Thanks to Cristiano Lincoln Mattos
    ...
    IPPROTO_TCP, core::ptr::null_mut(), &cmd.u.tcp.port);
...
#[no_mangle]
unsafe extern "C" fn nf_ct_ftp_from_nlattr(attr: *mut nlattr, ct: *mut nf_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nf_conntrack_ftp_fini() -> void __exit {
    static void __exit nf_conntrack_ftp_fini(void)
    {
    nf_conntrack_helper_unregister(ftp_ptr);
    }
#[no_mangle]
unsafe extern "C" fn nf_conntrack_ftp_init() -> int __init {
