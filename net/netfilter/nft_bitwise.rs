...
//! Source: net/netfilter/nft_bitwise.c
#![no_std]
...
pub struct nft_bitwise {
    ...
    const struct nft_bitwise *priv)
    {
    let mut shift: u32 = priv.data.data[0];
    unsigned int i;
    let mut carry: u32 = 0;
    for (i = DIV_ROUND_UP(priv.len, sizeof(u32)); i > 0; i--) {
    let mut tmp_src: u32 = src[i - 1];
    ...
    !tb[NFTA_BITWISE_XOR])
    return -EINVAL;
    err = nft_data_init(core::ptr::null_mut(), &priv.mask, &mask, tb[NFTA_BITWISE_MASK]);
    if (err < 0)
    return err;
    err = nft_data_init(core::ptr::null_mut(), &priv.xor, &xor, tb[NFTA_BITWISE_XOR]);
    if (err < 0)
    goto err_xor_err;
    return 0;
    err_xor_err:
    ...
    if (!tb[NFTA_BITWISE_DATA])
    return -EINVAL;
    err = nft_data_init(core::ptr::null_mut(), &priv.data, &desc, tb[NFTA_BITWISE_DATA]);
    if (err < 0)
    return err;
    if (!priv.data.data[0] ||
    ...
    err = nft_data_init(core::ptr::null_mut(), &priv.data, &desc,
    ...
    int err;
    err = nft_data_init(core::ptr::null_mut(), &data, &desc, tb);
    if (err < 0)
    return err;
// out = data.data[0];
    return 0;
