...
//! Source: net/netfilter/nft_compat.c
#![no_std]
...
pub const NFT_MATCH_LARGE_THRESH: c_int = 192;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_xt_match_priv {
    pub info: *mut c_void,
}
    ...
    {
    let mut type: enum nft_chain_types = NFT_CHAIN_T_DEFAULT;
    ...
    break;
    case NFPROTO_BRIDGE:
    entry.ebt.ethproto = ( __be16)proto;
...
#[no_mangle]
unsafe extern "C" fn target_compat_from_user(t: *mut xt_target, in: *mut c_void, out: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn nft_parse_compat(attr: *const nlattr, proto: *mut u16, inv: *mut bool) -> c_int {
    ...
    nft_rule_compat_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
...
#[no_mangle]
unsafe extern "C" fn nft_compat_wait_for_destructors(net: *mut net) {
...
//
    ...
    let mut size: usize = XT_ALIGN(nla_len(tb[NFTA_TARGET_INFO]));
    let mut proto: u16 = 0;
    let mut inv: bool = false;
    let mut e: union nft_entry = {};
    int ret;
    ...
    if (ret < 0) {
    if (ret == -ENOENT) {
    const char *modname = core::ptr::null_mut();
    if (strcmp(target.name, "LOG") == 0)
    modname = "nf_log_syslog";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(target->name, 0: "NFLOG") ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn __nft_mt_tg_destroy(me: *mut module, expr: *const nft_expr) {
    ...
    nft_base_chain(ctx.chain);
    const struct nf_hook_ops *ops = &basechain.ops;
    let mut hook_mask: c_uint = 1 << ops.hooknum;
    struct xt_target *target = expr.ops.data;
    void *info = nft_expr_priv(expr);
    struct xt_tgchk_param par;
    let mut e: union nft_entry = {};
...
#[no_mangle]
unsafe extern "C" fn match_compat_from_user(m: *mut xt_match, in: *mut c_void, out: *mut c_void) {
    ...
    let mut size: usize = XT_ALIGN(nla_len(tb[NFTA_MATCH_INFO]));
    let mut proto: u16 = 0;
    let mut inv: bool = false;
    let mut e: union nft_entry = {};
    int ret;
    ...
    let mut hook_mask: c_uint = 1 << ops.hooknum;
    struct xt_match *match = expr.ops.data;
    let mut size: usize = XT_ALIGN(match.matchsize);
    struct xt_mtchk_param par;
    let mut e: union nft_entry = {};
    void *info;
    ...
    const char *name, *fmt;
    struct sk_buff *skb2;
    let mut ret: c_int = 0, target;
    u32 rev;
    if (tb[NFTA_COMPAT_NAME] == core::ptr::null_mut() ||
    tb[NFTA_COMPAT_REV] == core::ptr::null_mut() ||
    tb[NFTA_COMPAT_TYPE] == core::ptr::null_mut())
    return -EINVAL;
...
// include the best revision for this extension in the message
    ...
    u32 rev, family;
    int err;
    if (tb[NFTA_MATCH_NAME] == core::ptr::null_mut() ||
    tb[NFTA_MATCH_REV] == core::ptr::null_mut() ||
    tb[NFTA_MATCH_INFO] == core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn nft_match_release_ops(ops: *const nft_expr_ops) {
    ...
    u32 rev, family;
    int err;
    if (tb[NFTA_TARGET_NAME] == core::ptr::null_mut() ||
    tb[NFTA_TARGET_REV] == core::ptr::null_mut() ||
    tb[NFTA_TARGET_INFO] == core::ptr::null_mut())
    return ERR_PTR(-EINVAL);
...
#[no_mangle]
unsafe extern "C" fn nft_target_release_ops(ops: *const nft_expr_ops) {
...
#[no_mangle]
unsafe extern "C" fn nft_compat_module_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nft_compat_module_exit() -> void __exit {
