...
//! Source: net/netfilter/xt_RATEEST.c
#![no_std]
...
pub const RATEEST_HSIZE: c_int = 16;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xt_rateest_net {
    pub hash_lock: mutex,
    pub hash: [hlist_head; RATEEST_HSIZE],
}

    static unsigned int xt_rateest_id;
    static unsigned int jhash_rnd __read_mostly;
#[no_mangle]
unsafe extern "C" fn xt_rateest_hash(name: *const c_char) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn xt_rateest_tg_checkentry(par: *const xt_tgchk_param) -> c_int {
...
//
    ...
    ret = gen_new_estimator(&est.bstats, core::ptr::null_mut(), &est.rate_est,
    &est.lock, core::ptr::null_mut(), &cfg.opt);
    if (ret < 0)
    goto err2;
    info.est = est;
...
#[no_mangle]
unsafe extern "C" fn xt_rateest_tg_destroy(par: *const xt_tgdtor_param) {
...
#[no_mangle]
unsafe extern "C" fn xt_rateest_net_init(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn xt_rateest_tg_init() -> int __init {
    static int __init xt_rateest_tg_init(void)
    {
    let mut err: c_int = register_pernet_subsys(&xt_rateest_net_ops);
    if (err)
    return err;
    return xt_register_targets(xt_rateest_tg_reg, ARRAY_SIZE(xt_rateest_tg_reg));
    }
#[no_mangle]
unsafe extern "C" fn xt_rateest_tg_fini() -> void __exit {
