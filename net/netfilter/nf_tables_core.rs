...
#[no_mangle]
pub unsafe extern "C" fn nf_skip_indirect_calls() -> bool {
    static inline bool nf_skip_indirect_calls(void)
    {
    return static_branch_likely(&nf_tables_skip_direct_calls);
    }
#[no_mangle]
pub unsafe extern "C" fn nf_skip_indirect_calls_enable() -> void __init {
...
// (u16 *)dest = *(u16 *)ptr;
#[no_mangle]
pub unsafe extern "C" fn if(4: priv->len ==) -> else {
    else if (priv.len == 4)
// (u32 *)dest = *(u32 *)ptr;
    else
// (u8 *)dest = *(u8 *)ptr;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_jumpstack {
    pub rule: *const nft_rule_dp,
}
...
#[no_mangle]
pub unsafe extern "C" fn fun(_arg: expr, _arg: regs, (0: pkt); } while) -> return {
    ...
    const struct nft_rule_dp *rule;
    struct nft_regs regs;
    let mut stackptr: c_uint = 0;
    struct nft_jumpstack jumpstack[NFT_JUMP_STACK_SIZE];
    let mut genbit: bool = READ_ONCE(net.nft.gencursor);
...
#[no_mangle]
pub unsafe extern "C" fn if(&nft_cmp16_fast_ops: expr->ops ==) -> else {
    else if (expr.ops == &nft_cmp16_fast_ops)
    nft_cmp16_fast_eval(expr, &regs);
#[no_mangle]
pub unsafe extern "C" fn if(&nft_bitwise_fast_ops: expr->ops ==) -> else {
    ...
    nft_trace_packet(pkt, &regs.verdict, &info, core::ptr::null_mut(), NFT_TRACETYPE_POLICY);
...
#[no_mangle]
pub unsafe extern "C" fn nf_tables_core_module_init() -> int __init {
