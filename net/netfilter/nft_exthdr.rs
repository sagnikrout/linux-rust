...
//! Source: net/netfilter/nft_exthdr.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn optlen(opt: *const u8, offset: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn nft_skb_copy_to_reg(skb: *const sk_buff, offset: c_int, dest: *mut u32, len: c_uint) -> c_int {
    ...
    goto err;
    err = ipv6_find_hdr(pkt.skb, &offset, priv.type, core::ptr::null_mut(), core::ptr::null_mut());
...
//
    ...
    return -EBADMSG;
    opt.optlen = optlen;
    if (__ip_options_compile(net, opt, core::ptr::null_mut(), &info))
    return -EBADMSG;
    switch (target) {
    case IPOPT_SSRR:
    case IPOPT_LSRR:
...
// dest = 0;
    ...
    switch (priv.len) {
    case 2:
    old.v16 = ( __be16)get_unaligned((u16 *)(opt + offset));
    new.v16 = ( __be16)nft_reg_load16(
    &regs.data[priv.sreg]);
    switch (priv.type) {
    case TCPOPT_MSS:
// increase can cause connection to stall
    ...
    old.v32 = ( __be32)get_unaligned((u32 *)(opt + offset));
    if (old.v32 == new.v32)
    return;
    ...
    goto drop;
    for (j = 0; j < optl; ++j) {
    let mut n: u16 = TCPOPT_NOP;
    let mut o: u16 = opt[i+j];
    if ((i + j) % 2 == 0) {
    o <<= 8;
    n <<= 8;
    }
...
// can't remove, no choice but to drop
    ...
    const struct nft_pktinfo *pkt)
    {
    let mut offset: c_uint = nft_thoff(pkt) + sizeof(struct sctphdr);
    struct nft_exthdr *priv = nft_expr_priv(expr);
    u32 *dest = &regs.data[priv.dreg];
...
// time.
//
    let mut buflen: c_uint = optlen - i;
    u8 buf[2], *bufp;
    u8 type, len;
...
// dest = 0;
    ...
    {
    struct nft_exthdr *priv = nft_expr_priv(expr);
    let mut err: c_int = nft_exthdr_init(ctx, expr, tb);
    if (err < 0)
    return err;
    switch (priv.type) {
    case IPOPT_SSRR:
...
#[no_mangle]
unsafe extern "C" fn nft_exthdr_dump_common(skb: *mut sk_buff, priv: *const nft_exthdr) -> c_int {
