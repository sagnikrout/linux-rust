...
//! Source: net/netfilter/nft_dynset.c
#![no_std]
...
pub struct nft_dynset {
    ...
    &regs.data[priv.sreg_key], core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn nft_dynset_ext_add_expr(priv: *mut nft_dynset) {
    ...
    if (tb[NFTA_DYNSET_SET_NAME] == core::ptr::null_mut() ||
    tb[NFTA_DYNSET_OP] == core::ptr::null_mut() ||
    tb[NFTA_DYNSET_SREG_KEY] == core::ptr::null_mut())
    return -EINVAL;
    if (tb[NFTA_DYNSET_FLAGS]) {
    let mut flags: u32 = ntohl(nla_get_be32(tb[NFTA_DYNSET_FLAGS]));
    ...
    return -EOPNOTSUPP;
    if (set.ops.update == core::ptr::null_mut())
    return -EOPNOTSUPP;
    ...
    return -EOPNOTSUPP;
    timeout = 0;
    if (tb[NFTA_DYNSET_TIMEOUT] != core::ptr::null_mut()) {
    ...
    if (err < 0)
    return err;
    if (tb[NFTA_DYNSET_SREG_DATA] != core::ptr::null_mut()) {
    ...
    {
    const struct nft_dynset *priv = nft_expr_priv(expr);
    let mut flags: u32 = priv.invert ? NFT_DYNSET_F_INV : 0;
    int i;
