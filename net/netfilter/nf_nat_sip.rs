...
//! Source: net/netfilter/nf_nat_sip.c
#![no_std]
...
// datalen += buflen - matchlen;
    ...
    __be16 port;
    if (ct_sip_parse_header_uri(ct, *dptr, core::ptr::null_mut(), *datalen, type, core::ptr::null_mut(),
...
// Translate topmost Via header and parameters
    ...
    hdr, core::ptr::null_mut(), &matchoff, &matchlen,
    &addr, &port) > 0) {
...
// from which the server received the request.
    ...
    let mut p: __be16 = ct.tuplehash[!dir].tuple.src.u.udp.port;
...
// Translate Contact headers
    ...
    &matchoff, &matchlen,
    &addr, &port) > 0) {
    let mut old_len: c_int = skb.len, delta;
    if (!map_addr(skb, protoff, dataoff, dptr, datalen,
    matchoff, matchlen,
    &addr, port)) {
...
// Mangle destination port for Cisco phones, then fix up checksums
    ...
    let mut doff: c_int = *dptr - (const char *)skb.data;
    struct udphdr *uh;
    if (doff <= 0) {
...
// Handles expected signalling connections and media streams
    ...
    let mut range_set_for_snat: c_int = 0;
...
// Try to get same pair of ports: if not, try to change them.
    ...
    break;
#[no_mangle]
pub unsafe extern "C" fn if(-EBUSY: ret ==) -> else {
    else if (ret == -EBUSY)
    continue;
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn nf_nat_sip_fini() -> void __exit {
    static void __exit nf_nat_sip_fini(void)
    {
    nf_nat_helper_unregister(&nat_helper_sip);
    RCU_INIT_POINTER(nf_nat_sip_hooks, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn nf_nat_sip_init() -> int __init {
    static int __init nf_nat_sip_init(void)
    {
    BUG_ON(nf_nat_sip_hooks != core::ptr::null_mut());
