...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_ct_helper_obj {
    pub helper4: *mut nf_conntrack_helper,
    pub helper6: *mut nf_conntrack_helper,
    pub l4proto: u8,
}
...
// dest = jiffies_to_msecs(nf_ct_expires(ct));
    return;
    case NFT_CT_HELPER:
    if (ct.master == core::ptr::null_mut())
    goto err;
    ...
    case NFT_CT_AVGPKT: {
    const struct nf_conn_acct *acct = nf_conn_acct_find(ct);
    let mut avgcnt: u64 = 0, bcnt = 0, pcnt = 0;
    if (acct) {
...
// dest = nf_ct_get_id(ct);
    ...
    nft_reg_store16(dest, ( u16)tuple.src.u.all);
    return;
    case NFT_CT_PROTO_DST:
    nft_reg_store16(dest, ( u16)tuple.dst.u.all);
    return;
    case NFT_CT_SRC_IP:
    if (nf_ct_l3num(ct) != NFPROTO_IPV4)
    goto err;
// dest = ( __u32)tuple->src.u3.ip;
    return;
    case NFT_CT_DST_IP:
    if (nf_ct_l3num(ct) != NFPROTO_IPV4)
    goto err;
// dest = ( __u32)tuple->dst.u3.ip;
    return;
    case NFT_CT_SRC_IP6:
    ...
    const struct nft_pktinfo *pkt)
    {
    let mut zone: nf_conntrack_zone = { .dir = NF_CT_DEFAULT_ZONE_DIR };
    const struct nft_ct *priv = nft_expr_priv(expr);
    struct sk_buff *skb = pkt.skb;
    enum ip_conntrack_info ctinfo;
    let mut value: u16 = nft_reg_load16(&regs.data[priv.sreg]);
    struct nf_conn *ct;
    int oldcnt;
...
//
    ...
    let mut value: u32 = regs.data[priv.sreg];

    enum ip_conntrack_info ctinfo;
    struct nf_conn *ct;
    ct = nf_ct_get(skb, &ctinfo);
    if (ct == core::ptr::null_mut() || nf_ct_is_template(ct))
    return;
    switch (priv.key) {
    ...
    let mut ctmask: u32 = regs.data[priv.sreg];
    if (e) {
    if (e.ctmask != ctmask)
    e.ctmask = ctmask;
    break;
...
#[no_mangle]
unsafe extern "C" fn nft_ct_tmpl_put_pcpu() {
    ...
    break;
    nf_ct_put(ct);
    per_cpu(nft_ct_pcpu_template, cpu) = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn nft_ct_tmpl_alloc_pcpu() -> bool {
    static bool nft_ct_tmpl_alloc_pcpu(void)
    {
    let mut zone: nf_conntrack_zone = { .id = 0 };
    struct nf_conn *tmp;
    int cpu;
...
#[no_mangle]
unsafe extern "C" fn __nft_ct_get_destroy(ctx: *const nft_ctx, priv: *mut nft_ct) {
    ...
    switch (priv.key) {
    case NFT_CT_DIRECTION:
    if (tb[NFTA_CT_DIRECTION] != core::ptr::null_mut())
    return -EINVAL;
    len = sizeof(u8);
    break;
    case NFT_CT_STATE:
...
//
    ...
    priv.len = len;
    err = nft_parse_register_store(ctx, tb[NFTA_CT_DREG], &priv.dreg, core::ptr::null_mut(),
    NFT_DATA_VALUE, len);
    if (err < 0)
    goto err;
...
#[no_mangle]
unsafe extern "C" fn __nft_ct_set_destroy(ctx: *const nft_ctx, priv: *mut nft_ct) {
    ...
    const struct nlattr * const tb[])
    {
    if (tb[NFTA_CT_KEY] == core::ptr::null_mut())
    return ERR_PTR(-EINVAL);
    if (tb[NFTA_CT_DREG] && tb[NFTA_CT_SREG])
    return ERR_PTR(-EINVAL);
    if (tb[NFTA_CT_DREG]) {

    let mut k: u32 = ntohl(nla_get_be32(tb[NFTA_CT_KEY]));
    switch (k) {
    case NFT_CT_STATE:
    case NFT_CT_DIRECTION:
    case NFT_CT_STATUS:
...
//
    ...
    let mut l3num: c_int = ctx.family;
    __u8 l4num;
    int ret;
    ...
    let mut family: c_int = ctx.family;
    int err;
    ...
    help4 = core::ptr::null_mut();
    help6 = core::ptr::null_mut();
    switch (family) {
    case NFPROTO_IPV4:
...
// && is intentional; only error if INET found neither ipv4 or ipv6
    ...
    struct nf_conn *ct = (struct nf_conn *)skb_nfct(pkt.skb);
    struct nf_conntrack_helper *to_assign = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn nft_ct_expect_timeout_get(attr: *const nlattr, val: *mut u32) -> c_int {
    static int nft_ct_expect_timeout_get(const struct nlattr *attr, u32 *val)
    {
    let mut jiffies_val: c_ulong = msecs_to_jiffies(nla_get_u32(attr));
...
#[no_mangle]
unsafe extern "C" fn nft_ct_nat_follow_master(ct: *mut nf_conn, this: *mut nf_conntrack_expect) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nft_ct_expect_data {
    pub obj: nft_ct_expect_obj,
    pub dir: enum ip_conntrack_dir,
}
    ...
    expect_data.obj.l4proto, core::ptr::null_mut(), &expect_data.obj.dport);
...
#[no_mangle]
unsafe extern "C" fn nft_ct_expect_helper_alloc(priv: *mut nft_ct_expect_obj) -> c_int {
...
// This helper is going away, disable it.
    ...
    expect_data.obj.helper = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn nft_ct_module_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nft_ct_module_exit() -> void __exit {
