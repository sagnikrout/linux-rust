...
//! Source: net/netfilter/nf_conntrack_proto_tcp.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn tcp_print_conntrack(s: *mut seq_file, ct: *mut nf_conn) {
...
#[no_mangle]
unsafe extern "C" fn get_conntrack_index(tcph: *const tcphdr) -> c_uint {
...
pub const MAXACKWINCONST: c_int = 66000;
...
//
    ...
    let mut length: c_int = (tcph.doff*4) - sizeof(struct tcphdr);
    if (!length)
    return;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nf_tcp_invalid_log {
    pub type: enum nf_tcp_invalid_log_type,
    pub value: u32,
}
...
//
    ...
    if (!seq_ok) {
    let mut overshot: u32 = end - sender.td_maxend + 1;
    bool ack_ok;
...
// Protect conntrack against broken packets. Code taken from ipt_unclean.c.
    ...
    const struct nf_hook_state *state)
    {
    let mut tcplen: c_uint = skb.len - dataoff;
    u8 tcpflags;
...
#[no_mangle]
unsafe extern "C" fn tcp_can_early_drop(ct: *const nf_conn) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn nf_conntrack_tcp_set_closing(ct: *mut nf_conn) {
...
#[no_mangle]
unsafe extern "C" fn nf_ct_tcp_state_reset(state: *mut ip_ct_tcp_state) {
...
// Returns verdict for packet, or -1 for invalid.
    ...
    let mut log: nf_tcp_invalid_log = {};
    unsigned int index, *timeouts;
    let mut lowered_timeout: bool = false;
...
// Set ASSURED if we see valid ack in ESTABLISHED
    ...
    NLA_ALIGN(NLA_HDRLEN + sizeof(struct nf_ct_tcp_flags)))
#[no_mangle]
unsafe extern "C" fn nlattr_to_tcp(cda[]: *mut nlattr, ct: *mut nf_conn) -> c_int {
...
// protocol info, in that case skip the parsing
    ...
    tcp_nla_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    spin_lock_bh(&ct.lock);
...
#[no_mangle]
unsafe extern "C" fn tcp_nlattr_tuple_size() -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn nf_conntrack_tcp_init_net(net: *mut net) {
