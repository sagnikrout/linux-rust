...
//! Source: net/netfilter/nf_conntrack_sip.c
#![no_std]
...
//
    ...
    let mut __read_mostly: static unsigned int sip_timeout = SIP_TIMEOUT;
    module_param(sip_timeout, uint, 0600);
    MODULE_PARM_DESC(sip_timeout, "timeout for the master SIP session");
    let mut __read_mostly: static int sip_direct_signalling = 1;
    module_param(sip_direct_signalling, int, 0600);
    MODULE_PARM_DESC(sip_direct_signalling, "expect incoming calls from registrar "
    "only (default 1)");
    let mut __read_mostly: static int sip_direct_media = 1;
    module_param(sip_direct_media, int, 0600);
    MODULE_PARM_DESC(sip_direct_media, "Expect Media streams between signalling "
    "endpoints only (default 1)");
    let mut __read_mostly: static int sip_external_media = 0;
...
#[no_mangle]
unsafe extern "C" fn iswordc(c: c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn word_len(dptr: *const c_char, limit: *const c_char) -> c_int {
...
// get media type + port length
    ...
    cp++;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: delim) -> else {
    else if (delim)
    return 0;
...
// skip ip address. returns its length.
    ...
    if (!sip_parse_port(dptr, &dptr, limit, core::ptr::null_mut()))
    return 0;
    return dptr - aux;
    }
...
//
#[no_mangle]
unsafe extern "C" fn sip_strtouint(cp: *const c_char, len: c_uint, endp: *mut c_char) -> c_uint {
    static unsigned int sip_strtouint(const char *cp, unsigned int len, char **endp)
    {
    let mut max: c_uint = sizeof("4294967295");
    let mut olen: c_uint = len;
    const char *s = cp;
...
// Skip method and following whitespace
    mlen = string_len(ct, dptr, limit, core::ptr::null_mut());
    if (!mlen)
    return 0;
    dptr += mlen;
    if (++dptr >= limit)
...
//
    ...
    [SIP_HDR_CSEQ]			= SIP_HDR("CSeq", core::ptr::null_mut(), core::ptr::null_mut(), digits_len),
    [SIP_HDR_FROM]			= SIP_HDR("From", "f", "sip:", skp_epaddr_len),
    [SIP_HDR_TO]			= SIP_HDR("To", "t", "sip:", skp_epaddr_len),
    [SIP_HDR_CONTACT]		= SIP_HDR("Contact", "m", "sip:", skp_epaddr_len),
    [SIP_HDR_VIA_UDP]		= SIP_HDR("Via", "v", "UDP ", epaddr_len),
    [SIP_HDR_VIA_TCP]		= SIP_HDR("Via", "v", "TCP ", epaddr_len),
    [SIP_HDR_EXPIRES]		= SIP_HDR("Expires", core::ptr::null_mut(), core::ptr::null_mut(), digits_len),
    [SIP_HDR_CONTENT_LENGTH]	= SIP_HDR("Content-Length", "l", core::ptr::null_mut(), digits_len),
    [SIP_HDR_CALL_ID]		= SIP_HDR("Call-Id", "i", core::ptr::null_mut(), callid_len),
    };
...
// proto = IPPROTO_TCP;
#[no_mangle]
pub unsafe extern "C" fn if(matchoff: !strncasecmp(dptr +, _arg: "UDP", _arg: strlen("UDP"))) -> else {
...
//
    ...
    [SDP_HDR_VERSION]	= SDP_HDR("v=", core::ptr::null_mut(), digits_len),
    [SDP_HDR_OWNER]		= SDP_HDR("o=", "IN IP4 ", sdp_addr_len),
    [SDP_HDR_CONNECTION]	= SDP_HDR("c=", "IN IP4 ", sdp_addr_len),
    [SDP_HDR_MEDIA]		= SDP_HDR("m=", core::ptr::null_mut(), media_len),
    };
...
// matchoff = dptr - start + shift;
    ...
    if (ret <= 0)
    return ret;
    if (!sdp_parse_addr(ct, dptr + *matchoff, core::ptr::null_mut(), addr,
...
#[no_mangle]
unsafe extern "C" fn flush_expectations(ct: *mut nf_conn, media: bool) {
    ...
    let mut dir: enum ip_conntrack_dir = CTINFO2DIR(ctinfo);
    union nf_inet_addr *saddr;
    struct nf_conntrack_tuple tuple;
    let mut direct_rtp: c_int = 0, skip_expect = 0, ret = NF_DROP;
    u_int16_t base_port;
...
//
    ...
    let mut external_media: bool = false;
...
// daddr			= exp->saved_addr;
    ...
    if (rtp_exp == core::ptr::null_mut())
    goto err1;
    nf_ct_expect_init(rtp_exp, class, nf_ct_l3num(ct), saddr, daddr,
    IPPROTO_UDP, core::ptr::null_mut(), &rtp_port);
    rtcp_exp = nf_ct_expect_alloc(ct);
    if (rtcp_exp == core::ptr::null_mut())
    goto err2;
    nf_ct_expect_init(rtcp_exp, class, nf_ct_l3num(ct), saddr, daddr,
    IPPROTO_UDP, core::ptr::null_mut(), &rtcp_port);
    hooks = rcu_dereference(nf_nat_sip_hooks);
...
//
    ...
    ret = NF_ACCEPT;
#[no_mangle]
pub unsafe extern "C" fn if(0: errp ==) -> else {
    else if (errp == 0)
    ...
    let mut have_rtp_addr: bool = false;
    hooks = rcu_dereference(nf_nat_sip_hooks);
...
#[no_mangle]
pub unsafe extern "C" fn if(cseq: ct_sip_info->invite_cseq ==) -> else {
...
//
    ...
    &matchoff, &matchlen) > 0)
    expires = sip_strtouint(*dptr + matchoff, *datalen - matchoff, core::ptr::null_mut());
    ret = ct_sip_parse_header_uri(ct, *dptr, core::ptr::null_mut(), *datalen,
    SIP_HDR_CONTACT, core::ptr::null_mut(),
...
// We don't support third-party registrations
    ...
    "expires=", core::ptr::null_mut(), core::ptr::null_mut(), &expires) < 0) {
    ...
    saddr, &daddr, proto, core::ptr::null_mut(), &port);
    ...
    let mut expires: c_uint = 0;
    let mut in_contact: c_int = 0, ret;
    if (!ct_sip_info)
    return NF_DROP;
...
//
    ...
    expires = sip_strtouint(*dptr + matchoff, *datalen - matchoff, core::ptr::null_mut());
    while (1) {
    let mut c_expires: c_uint = expires;
...
// datalen, &proto) == 0)
    ...
    matchoff + matchlen,
// datalen, "expires=",
    core::ptr::null_mut(), core::ptr::null_mut(), &c_expires);
    if (ret < 0) {
    nf_ct_helper_log(skb, ct, "cannot parse expires");
    return NF_DROP;
    }
    ...
    SIP_HANDLER("ACK", process_sdp, core::ptr::null_mut()),
    SIP_HANDLER("PRACK", process_sdp, process_prack_response),
    SIP_HANDLER("BYE", process_bye_request, core::ptr::null_mut()),
...
// datalen - strlen("SIP/2.0 "), NULL);
    ...
    if (handler.response == core::ptr::null_mut())
    continue;
...
// the correct port.
//
    if (ct_sip_parse_header_uri(ct, *dptr, core::ptr::null_mut(), *datalen,
    SIP_HDR_VIA_UDP, core::ptr::null_mut(), &matchoff,
    ...
    char *end;
    handler = &sip_handlers[i];
    if (handler.request == core::ptr::null_mut())
    continue;
...
#[no_mangle]
unsafe extern "C" fn nf_conntrack_sip_fini() -> void __exit {
    static void __exit nf_conntrack_sip_fini(void)
    {
    nf_conntrack_helpers_unregister(sip_ptr, 2);
    }
#[no_mangle]
unsafe extern "C" fn nf_conntrack_sip_init() -> int __init {
