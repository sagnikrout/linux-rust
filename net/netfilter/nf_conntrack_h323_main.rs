...
//! Source: net/netfilter/nf_conntrack_h323_main.c
#![no_std]
...
pub const H323_MAX_SIZE: c_int = 65535;
// Parameters
    let mut __read_mostly: static unsigned int default_rrq_ttl = 300;
    module_param(default_rrq_ttl, uint, 0600);
    MODULE_PARM_DESC(default_rrq_ttl, "use this TTL if it's missing in RRQ");
    let mut __read_mostly: static int gkrouted_only = 1;
    module_param(gkrouted_only, int, 0600);
    MODULE_PARM_DESC(gkrouted_only, "only accept calls from gatekeeper");
    let mut __read_mostly: static bool callforward_filter = true;
...
// Get TCP data length
    ...
    if (*data == core::ptr::null_mut()) {	/* first TPKT */
...
// Create expect for RTP
    if ((rtp_exp = nf_ct_expect_alloc(ct)) == core::ptr::null_mut())
    return -1;
...
// Create expect for RTCP
    if ((rtcp_exp = nf_ct_expect_alloc(ct)) == core::ptr::null_mut()) {
...
// Create expect for T.120 connections
    if ((exp = nf_ct_expect_alloc(ct)) == core::ptr::null_mut())
    return -1;
    nf_ct_expect_init(exp, NF_CT_EXPECT_CLASS_DEFAULT, nf_ct_l3num(ct),
    &ct.tuplehash[!dir].tuple.src.u3,
    &ct.tuplehash[!dir].tuple.dst.u3,
    IPPROTO_TCP, core::ptr::null_mut(), &port);
...
// RTCP
    ...
    unsigned char *data = core::ptr::null_mut();
    int datalen;
    int dataoff;
    int ret;
...
// Create expect for Q.931
    ...
    &ct.tuplehash[!dir].tuple.src.u3 : core::ptr::null_mut(),
    &ct.tuplehash[!dir].tuple.dst.u3,
    IPPROTO_TCP, core::ptr::null_mut(), &port);
    rcu_assign_pointer(exp.assign_helper, nf_conntrack_helper_q931_ptr[0]);
...
// Need new expect
    ...
    nf_ct_expect_init(exp, NF_CT_EXPECT_CLASS_DEFAULT, nf_ct_l3num(ct),
    &ct.tuplehash[!dir].tuple.src.u3, &addr,
    IPPROTO_UDP, core::ptr::null_mut(), &port);
...
unsafe extern "C" fn h323_helper_init() -> int __init {
    ...
    "H.245",
    &h245_exp_policy, 0, h245_help, core::ptr::null_mut(), THIS_MODULE);
    nf_ct_helper_init(&nf_conntrack_helper_q931[0], AF_INET, IPPROTO_TCP,
    "Q.931",
    &q931_exp_policy, 0, q931_help, core::ptr::null_mut(), THIS_MODULE);
...
#[no_mangle]
unsafe extern "C" fn h323_helper_exit() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn nf_conntrack_h323_fini() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn nf_conntrack_h323_init() -> int __init {
