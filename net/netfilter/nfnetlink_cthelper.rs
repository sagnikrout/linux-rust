...
//! Source: net/netfilter/nfnetlink_cthelper.c
#![no_std]
...
// If the user-space helper is not available, don't block traffic.
    ...
    nfnl_cthelper_tuple_pol, core::ptr::null_mut());
    if (err < 0)
    return err;
...
// Not all fields are initialized so first zero the tuple
    ...
    nfnl_cthelper_expect_pol, core::ptr::null_mut());
    if (err < 0)
    return err;
...
// Default to queue number zero, this can be updated at any time.
    ...
    if (tb[NFCTH_STATUS]) {
    let mut status: c_int = ntohl(nla_get_be32(tb[NFCTH_STATUS]));
    switch(status) {
...
// Now we can safely update them.
    ...
    struct nf_conntrack_helper *cur, *helper = core::ptr::null_mut();
    struct nf_conntrack_tuple tuple;
    ...
    if (nest_parms == core::ptr::null_mut())
    goto nla_put_failure;
    ...
    if (nest_parms1 == core::ptr::null_mut())
    goto nla_put_failure;
    ...
    if (nest_parms2 == core::ptr::null_mut())
    goto nla_put_failure;
...
// skip non-userspace conntrack helpers.
    ...
    struct sk_buff *skb2;
    char *helper_name = core::ptr::null_mut();
    struct nf_conntrack_tuple tuple;
    struct nfnl_cthelper *nlcth;
    let mut tuple_set: bool = false;
    if (!capable(CAP_NET_ADMIN))
    return -EPERM;
    ...
    struct nf_conntrack_helper *cur;
    struct nf_conntrack_tuple tuple;
    let mut tuple_set: bool = false, found = false;
...
#[no_mangle]
unsafe extern "C" fn nfnl_cthelper_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nfnl_cthelper_exit() -> void __exit {
