...
// Internal logging interface, which relies on the real
    LOG target modules */
pub const NFLOGGER_NAME_LEN: c_int = 64;
...
#[no_mangle]
pub unsafe extern "C" fn nf_log_set(net: *mut net, pf: u_int8_t, logger: *const nf_logger) -> c_int {
...
pub unsafe extern "C" fn nf_log_unset(net: *mut net, logger: *const nf_logger) {
    ...
    if (log == logger)
    RCU_INIT_POINTER(net.nf.nf_loggers[i], core::ptr::null_mut());
    }
    mutex_unlock(&nf_log_mutex);
    }
...
#[no_mangle]
pub unsafe extern "C" fn nf_log_register(pf: u_int8_t, logger: *mut nf_logger) -> c_int {
...
pub unsafe extern "C" fn nf_log_unregister(logger: *mut nf_logger) {
    ...
    if (log == logger)
    RCU_INIT_POINTER(loggers[i][logger.type], core::ptr::null_mut());
    }
    mutex_unlock(&nf_log_mutex);
    synchronize_rcu();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn nf_log_is_registered(pf: u_int8_t) -> bool {
    ...
    return -EINVAL;
    mutex_lock(&nf_log_mutex);
    if (__find_logger(pf, logger.name) == core::ptr::null_mut()) {
    mutex_unlock(&nf_log_mutex);
    return -ENOENT;
    }
    rcu_assign_pointer(net.nf.nf_loggers[pf], logger);
...
#[no_mangle]
pub unsafe extern "C" fn nf_log_unbind_pf(net: *mut net, pf: u_int8_t) {
    void nf_log_unbind_pf(struct net *net, u_int8_t pf)
    {
    if (pf >= ARRAY_SIZE(net.nf.nf_loggers))
    return;
    mutex_lock(&nf_log_mutex);
    RCU_INIT_POINTER(net.nf.nf_loggers[pf], core::ptr::null_mut());
    mutex_unlock(&nf_log_mutex);
    }
    EXPORT_SYMBOL(nf_log_unbind_pf);
#[no_mangle]
pub unsafe extern "C" fn nf_logger_find_get(pf: c_int, type: enum nf_log_type) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nf_logger_put(pf: c_int, type: enum nf_log_type) {
...
pub struct nf_log_buf {
    ...
    local_bh_disable();
    do {
    m = xchg(&emergency_ptr, core::ptr::null_mut());
    } while (!m);
    }
    m.count = 0;
    return m;
...
#[no_mangle]
pub unsafe extern "C" fn nf_log_buf_close(m: *mut nf_log_buf) {
...
unsafe extern "C" fn seq_show(s: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    return -ENOSPC;
    for (i = 0; i < NF_LOG_TYPE_MAX; i++) {
    if (loggers[*pos][i] == core::ptr::null_mut())
    continue;
    logger = nft_log_dereference(loggers[*pos][i]);
    seq_puts(s, logger.name);
    if (i == 0 && loggers[*pos][i + 1] != core::ptr::null_mut())
    seq_puts(s, ",");
    if (seq_has_overflowed(s))
    return -ENOSPC;
    }
    ...
    let mut r: c_int = 0;
    let mut tindex: c_int = (unsigned long)table.extra1;
...
#[no_mangle]
unsafe extern "C" fn netfilter_log_sysctl_init(net: *mut net) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn netfilter_log_sysctl_exit(net: *mut net) {
...
#[no_mangle]
unsafe extern "C" fn nf_log_net_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn nf_log_net_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
pub unsafe extern "C" fn netfilter_log_init() -> int __init {
