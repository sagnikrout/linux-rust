...
//! Source: net/netfilter/nfnetlink_queue.c
#![no_std]
...
pub const NFQNL_QMAX_DEFAULT: c_int = 1024;
pub const NFQNL_HASH_MIN: c_int = 8;
pub const NFQNL_HASH_MAX: c_int = 32768;
...
pub struct nfqnl_instance {
    pub /: *mut *mut hlist_node hlist; / global list of queues,
    ...
    pub /: *mut *mut u_int16_t queue_num; / number of this queue,
    pub copy_mode: u_int8_t,
    pub /: *mut *mut u_int32_t flags; / Set using NFQA_CFG_FLAGS,
//
...
//
    ...
    pub /: *mut *mut unsigned int id_sequence; / 'sequence' of pkt ids,
    pub /: *mut *mut list_head queue_list; / packets in queue,
}

    typedef int (*nfqnl_cmpfn)(struct nf_queue_entry *, unsigned long);
    static struct workqueue_struct *nfq_cleanup_wq __read_mostly;
    static unsigned int nfnl_queue_net_id __read_mostly;
pub const INSTANCE_BUCKETS: c_int = 16;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfnl_queue_net {
...
#[no_mangle]
pub unsafe extern "C" fn instance_hashfn(queue_num: u_int16_t) -> u_int8_t {
    ...
    unsigned long data);
#[no_mangle]
unsafe extern "C" fn instance_destroy_work(work: *mut work_struct) {
    ...
    rwork);
    rcu_read_lock();
    nfqnl_flush(inst, core::ptr::null_mut(), 0);
    rcu_read_unlock();
    rhashtable_destroy(&inst.nfqnl_packet_map);
    kfree(inst);
...
#[no_mangle]
unsafe extern "C" fn nf_ip_reroute(skb: *mut sk_buff, entry: *const nf_queue_entry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nf_reroute(skb: *mut sk_buff, entry: *mut nf_queue_entry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nf_reinject(entry: *mut nf_queue_entry, verdict: c_uint) {
...
//
#[no_mangle]
unsafe extern "C" fn nf_ct_drop_unconfirmed(entry: *const nf_queue_entry, is_unconfirmed: *mut bool) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nf_bridge_port_valid(dev: *const net_device) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn nf_bridge_ports_valid(entry: *const nf_queue_entry) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nfqnl_reinject(entry: *mut nf_queue_entry, verdict: c_uint) {
    ...
    verdict == NF_REPEAT ||
    verdict == NF_STOP) {
    let mut ct_verdict: c_uint = verdict;
    rcu_read_lock();
...
//
    if (nf_ct_drop_unconfirmed(entry, core::ptr::null_mut()))
    verdict = NF_DROP;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: csum_verify) -> else {
    else if (csum_verify)
...
#[no_mangle]
unsafe extern "C" fn nfqnl_put_sk_uidgid(skb: *mut sk_buff, sk: *mut sock) -> c_int {
...
//
    ...
    file = sock ? READ_ONCE(sock.file) : core::ptr::null_mut();
    if (file) {
    cred = file.f_cred;
...
#[no_mangle]
unsafe extern "C" fn nfqnl_put_sk_classid(skb: *mut sk_buff, sk: *mut sock) -> c_int {
    static int nfqnl_put_sk_classid(struct sk_buff *skb, struct sock *sk)
    {

    if (sk && sk_fullsock(sk)) {
    let mut classid: u32 = sock_cgroup_classid(&sk.sk_cgrp_data);
...
#[no_mangle]
unsafe extern "C" fn nfqnl_get_sk_secctx(skb: *mut sk_buff, ctx: *mut lsm_context) -> c_int {
    static int nfqnl_get_sk_secctx(struct sk_buff *skb, struct lsm_context *ctx)
    {
    let mut seclen: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn nfqnl_get_bridge_size(entry: *mut nf_queue_entry) -> u32 {
    static u32 nfqnl_get_bridge_size(struct nf_queue_entry *entry)
    {
    struct sk_buff *entskb = entry.skb;
    let mut nlalen: u32 = 0;
    u32 mac_len;
...
#[no_mangle]
unsafe extern "C" fn nfqnl_put_bridge(entry: *mut nf_queue_entry, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nf_queue_checksum_help(entskb: *mut sk_buff) -> c_int {
    ...
    {
    let mut data_len: c_uint = entskb.len;
    ...
    {
    size_t size;
    let mut data_len: usize = 0, cap_len = 0;
    ...
    let mut ctinfo: enum ip_conntrack_info = 0;
    const struct nfnl_ct_hook *nfnl_ct;
    bool csum_verify;
    let mut ctx: lsm_context = { core::ptr::null_mut(), 0, 0 };
    let mut seclen: c_int = 0;
    ktime_t tstamp;
    ...
    (entry.state.hook == NF_INET_POST_ROUTING && entskb.sk == core::ptr::null_mut()))
...
// physical output device (when called from ipv4)
    ...
    let mut kts: timespec64 = ktime_to_timespec64(tstamp);
...
//
#[no_mangle]
unsafe extern "C" fn nf_bridge_adjust_skb_data(skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn nf_bridge_adjust_segmented_data(skb: *mut sk_buff) {
    ...
    if (skb.next == core::ptr::null_mut()) { /* last packet, no need to copy entry */
    ...
    let mut ct_is_unconfirmed: bool = false;
...
//
    ...
    let mut ihl: c_uint = iph_new.ihl * 4;
...
//
    ...
    let mut exthdr_cnt: c_int = 0, start = sizeof(*ip6_orig);
    const u8 *data = (const u8 *)ip6_new;
    let mut orig_nexthdr: u8 = ip6_orig.nexthdr;
    let mut new_nexthdr: u8 = ip6_new.nexthdr;
    let mut fragment: bool = false;
...
// No write support. Bridge is dubious: userspace doesn't even see L2 header
    ...
    return -EINVAL;
    if (diff < 0) {
    let mut min_len: c_uint = skb_transport_offset(e.skb);
    if (data_len < min_len)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn nfqnl_nf_hook_drop(net: *mut net) {
...
#[no_mangle]
unsafe extern "C" fn nfq_id_after(id: c_uint, max: c_uint) -> c_int {
    ...
    let mut queue_num: u16 = ntohs(info.nfmsg.res_id);
    ...
    nfqa[NFQA_VLAN],
    nfqa_vlan_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    ...
    if (nfqa[NFQA_L2HDR]) {
    let mut mac_header_len: u32 = skb_mac_header_len(entry.skb);
    if (mac_header_len != nla_len(nfqa[NFQA_L2HDR]))
    return -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(0: mac_header_len >) -> else {
    ...
    {
    struct nfnl_queue_net *q = nfnl_queue_pernet(info.net);
    let mut queue_num: u_int16_t = ntohs(info.nfmsg.res_id);
    const struct nfnl_ct_hook *nfnl_ct;
    struct nfqnl_msg_verdict_hdr *vhdr;
    enum ip_conntrack_info ctinfo;
    struct nfqnl_instance *queue;
...
// rcu lock already held from nfnl->call_rcu.
    ...
    let mut payload_len: u16 = nla_len(nfqa[NFQA_PAYLOAD]);
    let mut diff: c_int = payload_len - entry.skb.len;
    if (nfqnl_mangle(nla_data(nfqa[NFQA_PAYLOAD]),
    payload_len, entry, diff) < 0)
    verdict = NF_DROP;
#[no_mangle]
pub unsafe extern "C" fn if(diff: ct &&) -> else {
    else if (ct && diff)
    ...
    struct nfnl_queue_net *q = nfnl_queue_pernet(info.net);
    let mut queue_num: u_int16_t = ntohs(info.nfmsg.res_id);
    struct nfqnl_msg_config_cmd *cmd = core::ptr::null_mut();
    struct nfqnl_instance *queue;
    let mut flags: __u32 = 0, mask = 0;
...
pub struct iter_state {
    ...
    pos--;
    return pos ? core::ptr::null_mut() : head;
    }
...
#[no_mangle]
unsafe extern "C" fn seq_stop(s: *mut seq_file, v: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn seq_show(s: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfnl_queue_net_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn nfnl_queue_net_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn nfnetlink_queue_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nfnetlink_queue_fini() -> void __exit {
