...
//! Source: net/netfilter/ipvs/ip_vs_core.c
#![no_std]
...
// netns cnt used for uniqueness
    let mut ipvs_netns_cnt: static atomic_t = ATOMIC_INIT(0);
...
#[no_mangle]
pub unsafe extern "C" fn ip_vs_init_hash_table(table: *mut list_head, rows: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ip_vs_rht_free(t: *mut ip_vs_rht) {
...
#[no_mangle]
pub unsafe extern "C" fn ip_vs_rht_rcu_free(head: *mut rcu_head) {
    ...
    if (scounts) {
    let mut ml: c_int = roundup_pow_of_two(nr_cpu_ids);
...
//
    ...
    if (lfactor < 0) {
    let mut factor: c_int = min(-lfactor, max_bits);
...
#[no_mangle]
pub unsafe extern "C" fn if(0: lfactor <=) -> else {
    else if (lfactor <= 0)
...
#[no_mangle]
pub unsafe extern "C" fn if(min_bits: size > 1 <<) -> else {
...
//
    ...
    {
    struct ip_vs_conn *cp = core::ptr::null_mut();
    struct ip_vs_dest *dest;
    struct ip_vs_conn *ct;
    __be16 dport = 0;		/* destination port to forward */
    unsigned int flags;
    struct ip_vs_conn_param param;
    let mut fwmark: union nf_inet_addr = { .ip = htonl(svc.fwmark) };
...
// Check if a template already exists
    ...
    if (!ct || !ip_vs_check_template(ct, core::ptr::null_mut())) {
...
// if it is fwmark-based service, the cache_bypass sysctl is up
    ...
    iph.protocol == IPPROTO_UDP) ?
    IP_VS_CONN_F_ONE_PACKET : 0;
    let mut daddr: union nf_inet_addr = { .all = { 0, 0, 0, 0 } };
...
#[no_mangle]
unsafe extern "C" fn sysctl_snat_reroute(ipvs: *mut netns_ipvs) -> c_int {
    static int sysctl_snat_reroute(struct netns_ipvs *ipvs)
    {
    return ipvs.sysctl_snat_reroute;
    }
#[no_mangle]
unsafe extern "C" fn sysctl_nat_icmp_send(ipvs: *mut netns_ipvs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ip_vs_checksum_complete(skb: *mut sk_buff, offset: c_int) -> __sum16 {
    static __sum16 ip_vs_checksum_complete(struct sk_buff *skb, int offset)
    {
    return csum_fold(skb_checksum(skb, offset, skb.len - offset, 0));
    }
#[no_mangle]
pub unsafe extern "C" fn ip_vs_defrag_user(hooknum: c_uint) -> enum ip_defrag_users {
...
//
    ...
    {
    let mut iproto: c_int = af == AF_INET6 ? IPPROTO_ICMPV6 : IPPROTO_ICMP;
    let mut verdict: c_uint = NF_DROP;
    let mut ctoff: c_uint = ciph.len;
    let mut has_ports: bool = false;
...
//
#[no_mangle]
pub unsafe extern "C" fn is_sctp_abort(skb: *const sk_buff, nh_len: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn is_tcp_reset(skb: *const sk_buff, nh_len: c_int) -> c_int {
...
//
    ...
    {
    struct ip_vs_conn_param param;
    struct ip_vs_conn *ct = core::ptr::null_mut(), *cp = core::ptr::null_mut();
...
//
    ...
    let mut hooknum: c_uint = state.hook;
    struct ip_vs_iphdr iph;
    struct ip_vs_protocol *pp;
    struct ip_vs_proto_data *pd;
    struct ip_vs_conn *cp;
    let mut af: c_int = state.pf;
    struct sock *sk;
...
//
    ...
    let mut __maybe_unused: *mut char outer_proto = "IPIP";
...
// Failed checksum!
    ...
    if (tunnel) {
    let mut hlen_orig: c_uint = ciph.len - ciph.off;
    let mut info: __be32 = ic.un.gateway;
    let mut type: __u8 = ic.type;
    let mut code: __u8 = ic.code;
    offset2 = offset;
// Update the MTU
    if (ic.type == ICMP_DEST_UNREACH &&
    ic.code == ICMP_FRAG_NEEDED) {
    struct ip_vs_dest *dest = cp.dest;
    let mut mtu: u32 = ntohs(ic.un.frag.mtu);
    let mut frag_off: __be16 = cih.frag_off;
// Strip outer IP and ICMP, go to IPIP/UDP/GRE header
    if (pskb_pull(skb, ihl + sizeof(_icmph)) == core::ptr::null_mut())
    goto ignore_tunnel;
...
// original request.
//
    if (pskb_pull(skb, offset2) == core::ptr::null_mut())
    goto ignore_tunnel;
    skb_reset_network_header(skb);
...
//
    ...
    let mut conn_reuse_mode: c_int = sysctl_conn_reuse_mode(ipvs);
    let mut old_ct: bool = false, resched = false;
...
// the destination server is not available
    ...
    let mut old_ct: bool = ip_vs_conn_uses_old_conntrack(cp, skb);
    if (!old_ct) {
    spin_lock_bh(&cp.lock);
    cp.flags &= ~IP_VS_CONN_F_NFCT;
    spin_unlock_bh(&cp.lock);
...
#[no_mangle]
pub unsafe extern "C" fn if(cp->control: (cp->flags & IP_VS_CONN_F_ONE_PACKET) &&) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn ip_vs_register_hooks(ipvs: *mut netns_ipvs, af: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip_vs_unregister_hooks(ipvs: *mut netns_ipvs, af: c_uint) {
...
//
#[no_mangle]
unsafe extern "C" fn __ip_vs_init(net: *mut net) -> int __net_init {
    static int __net_init __ip_vs_init(struct net *net)
    {
    struct netns_ipvs *ipvs;
    ipvs = net_generic(net, ip_vs_net_id);
    if (ipvs == core::ptr::null_mut())
    return -ENOMEM;
...
//
    ...
    estimator_fail:
    net.ipvs = core::ptr::null_mut();
    return -ENOMEM;
    }
#[no_mangle]
unsafe extern "C" fn __ip_vs_cleanup_batch(net_list: *mut list_head) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn __ip_vs_dev_cleanup_batch(net_list: *mut list_head) -> void __net_exit {
...
//
#[no_mangle]
unsafe extern "C" fn ip_vs_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ip_vs_cleanup() -> void __exit {
