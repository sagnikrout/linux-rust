...
//! Source: net/netfilter/ipvs/ip_vs_xmit.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn ip_vs_dest_dst_free(dest_dst: *mut ip_vs_dest_dst) {
...
//
    ...
    dst.ops.check(dst, dest_dst.dst_cookie) == core::ptr::null_mut())
    return core::ptr::null_mut();
    return dest_dst;
    }
// Based on ip_exceeds_mtu().
#[no_mangle]
unsafe extern "C" fn ip_vs_exceeds_mtu(skb: *const sk_buff, mtu: c_uint) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn __ip_vs_is_local_route6(rt: *mut rt6_info) -> c_int {
    ...
    {
    let mut rt_mode_allow_local: bool = !!(rt_mode & IP_VS_RT_MODE_LOCAL);
    let mut rt_mode_allow_non_local: bool = !!(rt_mode & IP_VS_RT_MODE_NON_LOCAL);
    let mut rt_mode_allow_redirect: bool = !!(rt_mode & IP_VS_RT_MODE_RDR);
    bool source_is_loopback;
    bool old_rt_is_local;

    if (skb_af == AF_INET6) {
    let mut addr_type: c_int = ipv6_addr_type(&ipv6_hdr(skb).saddr);
    source_is_loopback =
...
#[no_mangle]
pub unsafe extern "C" fn maybe_update_pmtu(skb_af: c_int, skb: *mut sk_buff, mtu: c_int) {
    static inline void maybe_update_pmtu(int skb_af, struct sk_buff *skb, int mtu)
    {
    struct sock *sk = skb.sk;
    struct rtable *ort = skb_rtable(skb);
    if (!skb.dev && sk && sk_fullsock(sk))
    ort.dst.ops.update_pmtu(&ort.dst, sk, core::ptr::null_mut(), mtu, true);
    }
...
#[no_mangle]
unsafe extern "C" fn rt_dev_is_down(dev: *const net_device) -> bool {
...
// ret_saddr = dest_dst->dst_saddr.ip;
    ...
    if (!dest_dst) {
    __ip_vs_dst_set(dest, core::ptr::null_mut(), core::ptr::null_mut(), 0);
...
//
#[no_mangle]
pub unsafe extern "C" fn ip_vs_drop_early_demux_sk(skb: *mut sk_buff) {
...
//
    ...
    if (skb.dev)
    skb_clear_tstamp(skb);
    NF_HOOK(pf, NF_INET_LOCAL_OUT, cp.ipvs.net, core::ptr::null_mut(), skb,
    core::ptr::null_mut(), skb_dst(skb).dev, dst_output);
    } else
    ret = NF_ACCEPT;
    return ret;
    }
...
//
    ...
    struct ip_vs_protocol *pp, struct ip_vs_iphdr *ipvsh)
    {
    if (__ip_vs_get_out_rt(cp.ipvs, cp.af, skb, core::ptr::null_mut(), ip_hdr(skb).daddr,
    IP_VS_RT_MODE_NON_LOCAL, core::ptr::null_mut(), ipvsh) < 0)
    goto tx_error;
...
// Another hack: avoid icmp_send in ip_fragment
    ...
    if (__ip_vs_get_out_rt_v6(cp.ipvs, cp.af, skb, core::ptr::null_mut(),
    &iph.daddr, core::ptr::null_mut(),
...
// check if it is a connection of no-client-port
    ...
    IP_VS_RT_MODE_RDR, core::ptr::null_mut(), ipvsh);
    if (local < 0)
    goto tx_error;
    rt = skb_rtable(skb);
//
...
//
    ...
    struct iphdr *old_iph = core::ptr::null_mut();
    __u8 old_dsfield;

    struct ipv6hdr *old_ipv6h = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn __tun_gso_type_mask(encaps_af: c_int, orig_af: c_int) -> c_int {
    ...
    {
    __be16 dport;
    let mut sport: __be16 = udp_flow_src_port(net, skb, 0, 0, false);
    struct udphdr  *udph;	/* Our new UDP header */
    struct guehdr  *gueh;	/* Our new GUE header */
    size_t hdrlen, optlen = 0;
    void *data;
    let mut need_priv: bool = false;
    ...
    if (need_priv) {
    __be32 *flags = data;
    let mut csum_start: u16 = skb_checksum_start_offset(skb);
    __be16 *pd;
...
//
    ...
    let mut next_protocol: __u8 = 0;
    let mut dsfield: __u8 = 0;
    let mut ttl: __u8 = 0;
    let mut df: __be16 = 0;
    __be16 *dfp = core::ptr::null_mut();
...
// We only care about the df field if sysctl_pmtu_disc(ipvs) is set
    dfp = sysctl_pmtu_disc(ipvs) ? &df : core::ptr::null_mut();
    skb = ip_vs_prepare_tunneled_skb(skb, cp.af, max_headroom,
    &next_protocol, core::ptr::null_mut(), &dsfield,
    &ttl, dfp);
    if (IS_ERR(skb))
    return NF_STOLEN;
...
#[no_mangle]
pub unsafe extern "C" fn if(NF_DROP: ret ==) -> else {
    else if (ret == NF_DROP)
    kfree_skb(skb);
    return NF_STOLEN;
    tx_error:
...
//
    ...
    IP_VS_RT_MODE_LOCAL |
    IP_VS_RT_MODE_NON_LOCAL |
    IP_VS_RT_MODE_KNOWN_NH, core::ptr::null_mut(), ipvsh);
    if (local < 0)
    goto tx_error;
    if (local)
    return ip_vs_send_or_cont(NFPROTO_IPV4, skb, cp, 1);
...
// LOCALNODE from FORWARD hook is not supported
    ...
    IP_VS_RT_MODE_RDR : IP_VS_RT_MODE_NON_LOCAL;
    local = __ip_vs_get_out_rt_v6(cp.ipvs, cp.af, skb, cp.dest,
    &cp.daddr.in6, core::ptr::null_mut(), ciph, 0, rt_mode);
    if (local < 0)
    goto tx_error;
    rt = dst_rt6_info(skb_dst(skb));
//
