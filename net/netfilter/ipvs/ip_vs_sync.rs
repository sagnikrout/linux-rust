...
//! Source: net/netfilter/ipvs/ip_vs_sync.c
#![no_std]
...
pub const IP_VS_SYNC_GROUP: c_uint = 0xe0000051    /* multicast addr - 224.0.0.81 */;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ip_vs_sync_conn_v0 {
    pub reserved: __u8,
// Protocol, addresses and port numbers
    pub /: *mut *mut __u8 protocol; / Which protocol (TCP/UDP),
    pub cport: __be16,
    pub vport: __be16,
    pub dport: __be16,
    pub /: *mut *mut __be32 caddr; / client address,
    pub /: *mut *mut __be32 vaddr; / virtual address,
    pub /: *mut *mut __be32 daddr; / destination address,
// Flags and state transition
    pub /: *mut *mut __be16 flags; / status flags,
    pub /: *mut *mut __be16 state; / state info,
// The sequence options start here
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ip_vs_sync_conn_options {
...
//
#[no_mangle]
pub unsafe extern "C" fn format(_arg: sync_conn) -> Sync Connection {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ip_vs_sync_v4 {
    pub type: __u8,
    pub /: *mut *mut __u8 protocol; / Which protocol (TCP/UDP),
    pub /: *mut *mut __be16 ver_size; / Version msb 4 bits,
// Flags and state transition
    pub /: *mut *mut __be32 flags; / status flags,
    pub /: *mut *mut __be16 state; / state info,
// Protocol, addresses and port numbers
    pub cport: __be16,
    pub vport: __be16,
    pub dport: __be16,
    pub /: *mut *mut __be32 fwmark; / Firewall mark from skb,
    pub /: *mut *mut __be32 timeout; / cp timeout,
    pub /: *mut *mut __be32 caddr; / client address,
    pub /: *mut *mut __be32 vaddr; / virtual address,
    pub /: *mut *mut __be32 daddr; / destination address,
// The sequence options start here
...
// Protocol, addresses and port numbers
    ...
    pub /: *mut *mut __be32 fwmark; / Firewall mark from skb,
    pub /: *mut *mut __be32 timeout; / cp timeout,
    pub /: *mut *mut in6_addr caddr; / client address,
    pub /: *mut *mut in6_addr vaddr; / virtual address,
    pub /: *mut *mut in6_addr daddr; / destination address,
// The sequence options start here
// PE data padded to 32bit alignment after seq. options
}
...
pub const STYPE_INET6: c_int = 0;

pub const SVER_MASK: c_uint = 0x0fff		/* Mask to strip version */;
pub const IPVS_OPT_SEQ_DATA: c_int = 1;
pub const IPVS_OPT_PE_DATA: c_int = 2;
pub const IPVS_OPT_PE_NAME: c_int = 3;
pub const IPVS_OPT_PARAM: c_int = 7;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ip_vs_sync_thread_data {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ip_vs_sync_mesg {
    pub /: *mut *mut __u8 reserved; / must be zero,
    pub syncid: __u8,
    pub size: __be16,
    pub nr_conns: __u8,
    pub /: *mut *mut __s8 version; / SYNC_PROTO_VER,
    pub spare: __u16,
// ip_vs_sync_conn entries start here
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ip_vs_sync_buff {
    pub list: list_head,
    pub firstuse: c_ulong,
// pointers for the message data
    pub mesg: *mut ip_vs_sync_mesg,
...
//
#[no_mangle]
unsafe extern "C" fn ntoh_seq(no: *mut ip_vs_seq, ho: *mut ip_vs_seq) {
...
//
#[no_mangle]
unsafe extern "C" fn hton_seq(ho: *mut ip_vs_seq, no: *mut ip_vs_seq) {
...
#[no_mangle]
pub unsafe extern "C" fn ip_vs_sync_buff_release(sb: *mut ip_vs_sync_buff) {
    ...
    let mut id: c_int = (int)(ms - ipvs.ms);
...
//
    ...
    ms.sync_buff = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn in_persistence(cp: *mut ip_vs_conn) -> bool {
...
//
    ...
    {
    let mut orig: c_ulong = READ_ONCE(cp.sync_endtime);
    let mut now: c_ulong = jiffies;
    let mut n: c_ulong = (now + cp.timeout) & ~3UL;
...
// Check if we sync in current state
    ...
    force = 0;
#[no_mangle]
pub unsafe extern "C" fn if(in_persistence(cp)): unlikely(sysctl_sync_persist_mode(ipvs) &&) -> else {
    else if (unlikely(sysctl_sync_persist_mode(ipvs) && in_persistence(cp)))
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_TCP): likely(cp->protocol ==) -> else {
    if (!((1 << cp.state) &
...
// UDP or another protocol with single state
    ...
    let mut diff: c_long = n - orig;
    let mut min_diff: c_long = max(cp.timeout >> 1, 10UL * HZ);
...
//
#[no_mangle]
pub unsafe extern "C" fn ip_vs_sync_conn(ipvs: *mut netns_ipvs, cp: *mut ip_vs_conn, pkts: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ip_vs_proc_sync_conn(ipvs: *mut netns_ipvs, p: *mut __u8, msg_end: *mut __u8) -> c_int {
    ...
    __u8 *pe_data=core::ptr::null_mut(), *pe_name=core::ptr::null_mut();
    let mut opt_flags: __u32 = 0;
    let mut retc: c_int = 0;
    s = (union ip_vs_sync_conn *) p;
...
// If only IPv4, just silent skip IPv6
    ...
    (opt_flags & IPVS_OPT_F_SEQ_DATA ? &opt : core::ptr::null_mut())
    );
...
//
#[no_mangle]
unsafe extern "C" fn set_sock_size(sk: *mut sock, mode: c_int, val: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn set_mcast_loop(sk: *mut sock, loop: u_char) {
...
//
#[no_mangle]
unsafe extern "C" fn set_mcast_ttl(sk: *mut sock, ttl: u_char) {
...
#[no_mangle]
unsafe extern "C" fn set_mcast_pmtudisc(sk: *mut sock, val: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn set_mcast_if(sk: *mut sock, dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bind_mcastif_addr(sock: *mut socket, dev: *mut net_device) -> c_int {
...
// join the multicast group
    ...
    {
    let mut msg: msghdr = {core::ptr::null_mut(),};
    let mut iov: kvec = {buffer, buflen};
    int			len;
// Receive a packet
...
#[no_mangle]
unsafe extern "C" fn master_wakeup_work_handler(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn sync_thread_master(data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sync_thread_backup(data: *mut c_void) -> c_int {
...
// do we have data now?
    ...
    int state)
    {
    struct ip_vs_sync_thread_data *ti = core::ptr::null_mut(), *tinfo;
    struct task_struct *task;
    struct net_device *dev;
    char *name;
    int (*threadfn)(void *data);
    let mut id: c_int = 0, count, hlen;
...
//
    ...
    int new_size = ip_vs_conn_desired_size(ipvs, core::ptr::null_mut(),
    lfactor);
...
#[no_mangle]
pub unsafe extern "C" fn stop_sync_thread(ipvs: *mut netns_ipvs, state: c_int) -> c_int {
    int stop_sync_thread(struct netns_ipvs *ipvs, int state)
    {
    struct ip_vs_sync_thread_data *ti, *tinfo;
    int id;
    let mut retc: c_int = -EINVAL;
    IP_VS_DBG(7, "%s(): pid %d\n", __func__, task_pid_nr(current));
    mutex_lock(&ipvs.sync_mutex);
    if (state == IP_VS_STATE_MASTER) {
    retc = -ESRCH;
...
//
    ...
    kfree(ipvs.ms);
    ipvs.ms = core::ptr::null_mut();
    ipvs.master_tinfo = core::ptr::null_mut();
    } else if (state == IP_VS_STATE_BACKUP) {
    retc = -ESRCH;
    if (!ipvs.backup_tinfo)
    goto err;
    ...
    ipvs.backup_tinfo = core::ptr::null_mut();
    } else {
    goto err;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ip_vs_sync_net_init(ipvs: *mut netns_ipvs) -> int __net_init {
...
#[no_mangle]
pub unsafe extern "C" fn ip_vs_sync_net_cleanup(ipvs: *mut netns_ipvs) {
