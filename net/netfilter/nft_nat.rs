...
//! Source: net/netfilter/nft_nat.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct nft_nat {
    pub sreg_addr_min: u8,
    pub sreg_addr_max: u8,
    pub sreg_proto_min: u8,
    pub sreg_proto_max: u8,
    pub type:8: enum nf_nat_manip_type,
    pub family: u8,
    pub flags: u16,
}
    ...
    switch (priv.family) {
    case AF_INET:
    range.min_addr.ip = ( __be32)
    regs.data[priv.sreg_addr_min];
    range.max_addr.ip = ( __be32)
    ...
    u32 family;
    int err;
    if (tb[NFTA_NAT_TYPE] == core::ptr::null_mut() ||
    (tb[NFTA_NAT_REG_ADDR_MIN] == core::ptr::null_mut() &&
    tb[NFTA_NAT_REG_PROTO_MIN] == core::ptr::null_mut()))
    return -EINVAL;
    ...
    if (tb[NFTA_NAT_FAMILY] == core::ptr::null_mut())
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn nft_nat_inet_module_init() -> c_int {
    static int nft_nat_inet_module_init(void)
    {
    return nft_register_expr(&nft_inet_nat_type);
    }
#[no_mangle]
unsafe extern "C" fn nft_nat_inet_module_exit() {
...
#[no_mangle]
unsafe extern "C" fn nft_nat_module_init() -> int __init {
    static int __init nft_nat_module_init(void)
    {
    let mut ret: c_int = nft_nat_inet_module_init();
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn nft_nat_module_exit() -> void __exit {
