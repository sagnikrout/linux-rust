...
//! Source: net/netfilter/nf_conntrack_netlink.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_status(skb: *mut sk_buff, ct: *const nf_conn) -> c_int {
    ...
    {
    let mut attr: enum ctattr_type = dir ? CTA_COUNTERS_REPLY: CTA_COUNTERS_ORIG;
    ...
    bool dump)
    {
    let mut mark: u32 = READ_ONCE(ct.mark);
    if (!mark && !dump)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_secctx(skb: *mut sk_buff, ct: *const nf_conn) -> c_int {
    ...
    if (e) {
    let mut ts: u64 = local64_read(&e.timestamp);
    if (ts)
...
#[no_mangle]
pub unsafe extern "C" fn ctnetlink_label_size(ct: *const nf_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_master(skb: *mut sk_buff, ct: *const nf_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_ct_seq_adj(skb: *mut sk_buff, ct: *mut nf_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_ct_synproxy(skb: *mut sk_buff, ct: *mut nf_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_id(skb: *mut sk_buff, ct: *const nf_conn) -> c_int {
    static int ctnetlink_dump_id(struct sk_buff *skb, const struct nf_conn *ct)
    {
    let mut id: __be32 = ( __be32)nf_ct_get_id(ct);
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_use(skb: *mut sk_buff, ct: *const nf_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_info(skb: *mut sk_buff, ct: *mut nf_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_proto_size(ct: *const nf_conn) -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn ctnetlink_acct_size(ct: *const nf_conn) -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn ctnetlink_secctx_size(ct: *const nf_conn) -> c_int {
    static inline int ctnetlink_secctx_size(const struct nf_conn *ct)
    {

    int ret;
    ret = security_secid_to_secctx(ct.secmark, core::ptr::null_mut());
    if (ret < 0)
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn ctnetlink_timestamp_size(ct: *const nf_conn) -> usize {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_nlmsg_size(ct: *const nf_conn) -> usize {
    static size_t ctnetlink_nlmsg_size(const struct nf_conn *ct)
    {
#[no_mangle]
pub unsafe extern "C" fn NLMSG_ALIGN(nfgenmsg): sizeof(struct) -> return {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_done(cb: *mut netlink_callback) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ctnetlink_filter_u32 {
    pub val: u32,
    pub mask: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ctnetlink_filter {
    pub family: u8,
    pub zone_filter: bool,
    pub orig_flags: u_int32_t,
    pub reply_flags: u_int32_t,
    pub orig: nf_conntrack_tuple,
    pub reply: nf_conntrack_tuple,
    pub zone: nf_conntrack_zone,
    pub mark: ctnetlink_filter_u32,
    pub status: ctnetlink_filter_u32,
}
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_needs_filter(family: u8, cda: *const *const nlattr) -> bool {
    static bool ctnetlink_needs_filter(u8 family, const struct nlattr * const *cda)
    {
    return family || cda[CTA_MARK] || cda[CTA_FILTER] || cda[CTA_STATUS] || cda[CTA_ZONE];
    }
#[no_mangle]
unsafe extern "C" fn ctnetlink_start(cb: *mut netlink_callback) -> c_int {
    static int ctnetlink_start(struct netlink_callback *cb)
    {
    const struct nlattr * const *cda = cb.data;
    struct ctnetlink_filter *filter = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_filter_match(ct: *mut nf_conn, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_get_id(ct: *const nf_conn) -> c_ulong {
    static unsigned long ctnetlink_get_id(const struct nf_conn *ct)
    {
    let mut id: c_ulong = nf_ct_get_id(ct);
    return id ? id : 1;
    }
    static int
    ctnetlink_dump_table(struct sk_buff *skb, struct netlink_callback *cb)
    {
    let mut flags: c_uint = cb.data ? NLM_F_DUMP_FILTERED : 0;
    struct net *net = sock_net(skb.sk);
    let mut last_id: c_ulong = cb.args[1];
...
// nf ct hash resize happened, now clear the leftover.
    ...
    cta_ip_nla_policy, core::ptr::null_mut());
    if (ret < 0)
    return ret;
    switch (tuple.src.l3num) {
    case NFPROTO_IPV4:
    ...
    proto_nla_policy, core::ptr::null_mut());
    if (ret < 0)
    return ret;
    if (!(flags & CTA_FILTER_FLAG(CTA_PROTO_NUM)))
    return 0;
    ...
    tuple_nla_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
...
// orig and expect tuples get DIR_ORIGINAL
    ...
    help_nla_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    if (!tb[CTA_HELP_NAME])
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_flush_iterate(ct: *mut nf_conn, data: *mut c_void) -> c_int {
    ...
    family, &zone);
#[no_mangle]
pub unsafe extern "C" fn if(!cda[CTA_FILTER]: cda[CTA_TUPLE_REPLY] &&) -> else {
    else if (cda[CTA_TUPLE_REPLY] && !cda[CTA_FILTER])
    err = ctnetlink_parse_tuple(cda, &tuple, CTA_TUPLE_REPLY,
    family, &zone);
    else {
    let mut u3: u8 = info.nfmsg.version || cda[CTA_FILTER] ? family : AF_UNSPEC;
    ...
    ct = nf_ct_tuplehash_to_ctrack(h);
    if (cda[CTA_ID]) {
    let mut id: __be32 = nla_get_be32(cda[CTA_ID]);
    if (id != ( __be32)nf_ct_get_id(ct)) {
    nf_ct_put(ct);
    return -ENOENT;
    }
    }
    ...
    const struct nlattr * const cda[])
    {
    let mut u3: u_int8_t = info.nfmsg.nfgen_family;
    struct nf_conntrack_tuple_hash *h;
    struct nf_conntrack_tuple tuple;
    struct nf_conntrack_zone zone;
    struct sk_buff *skb2;
    ...
    u3, &zone);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cda[CTA_TUPLE_REPLY]) -> else {
    ...
    struct nfgenmsg *nfmsg = nlmsg_data(cb.nlh);
    let mut l3proto: u8 = nfmsg.nfgen_family;
    int res;
...
//
    ...
    let mut last_id: c_ulong = ctx.last_id;
    struct nf_conntrack_tuple_hash *h;
    struct hlist_nulls_node *n;
    ...
    char *helpname = core::ptr::null_mut();
    struct nlattr *helpinfo = core::ptr::null_mut();
    int err;
...
// we cannot set a helper for an existing conntrack
    ...
    protoinfo_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    l4proto = nf_ct_l4proto_find(nf_ct_protonum(ct));
    ...
    seqadj_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
    ...
    let mut len: usize = nla_len(cda[CTA_LABELS]);
...
// setup master conntrack: this is a confirmed expectation
    ...
    u3, core::ptr::null_mut());
    if (err < 0)
    goto err2;
    master_h = nf_conntrack_find_get(net, zone, &master);
    if (master_h == core::ptr::null_mut()) {
    err = -ENOENT;
    goto err2;
    }
    ...
    struct nf_conntrack_tuple_hash *h = core::ptr::null_mut();
    let mut u3: u_int8_t = info.nfmsg.nfgen_family;
    struct nf_conntrack_zone zone;
    struct nf_conn *ct;
    int err;
...
// implicit 'else'
    ...
    {
    struct nlmsghdr *nlh;
    let mut flags: c_uint = portid ? NLM_F_MULTI : 0, event;
...
#[no_mangle]
unsafe extern "C" fn __ctnetlink_glue_build(skb: *mut sk_buff, ct: *mut nf_conn) -> c_int {
    ...
    {
    let mut status: c_uint = ntohl(nla_get_be32(cda[CTA_STATUS]));
    let mut d: c_ulong = ct.status ^ status;
...
//
    ...
    nf_ct_l3num(ct), core::ptr::null_mut());
    if (err < 0)
    return err;
    ...
    {
    struct nf_conntrack_helper *assign_helper = core::ptr::null_mut();
    struct nlattr *cda[CTA_EXPECT_MAX+1];
    struct nf_conntrack_tuple tuple, mask;
    struct nf_conntrack_expect *exp;
    int err;
    err = nla_parse_nested_deprecated(cda, CTA_EXPECT_MAX, attr,
    exp_nla_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
...
#[no_mangle]
unsafe extern "C" fn nf_expect_get_id(exp: *const nf_conntrack_expect) -> __be32 {
    ...
    return ( __be32)siphash_4u64((u64)a, (u64)b, (u64)c, (u64)d, &exp_id_seed);

    return ( __be32)siphash_4u32((u32)a, (u32)b, (u32)c, (u32)d, &exp_id_seed);
    ...
    {
    let mut timeout: __s32 = (__s32)(READ_ONCE(exp.timeout) - nfct_time_stamp) / HZ;
    struct nf_conntrack_helper *helper;

    struct nlattr *nest_parms;
    let mut nat_tuple: nf_conntrack_tuple = {};
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_exp_id(exp: *const nf_conntrack_expect) -> c_ulong {
    static unsigned long ctnetlink_exp_id(const struct nf_conntrack_expect *exp)
    {
    let mut id: c_ulong = (unsigned long)exp;
    ...
    struct net *net = sock_net(skb.sk);
    struct nfgenmsg *nfmsg = nlmsg_data(cb.nlh);
    let mut l3proto: u_int8_t = nfmsg.nfgen_family;
    let mut last_id: c_ulong = cb.args[1];
    struct nf_conntrack_expect *exp;
    rcu_read_lock();
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_exp_ct_start(cb: *mut netlink_callback) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_dump_exp_ct_done(cb: *mut netlink_callback) -> c_int {
    ...
    let mut u3: u_int8_t = nfmsg.nfgen_family;
    struct nf_conntrack_tuple tuple;
    struct nf_conntrack_tuple_hash *h;
    struct nf_conn *ct;
    struct nf_conntrack_zone zone;
...
// No expectation linked to this connection tracking.
    ...
    u3, core::ptr::null_mut());
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cda[CTA_EXPECT_MASTER]) -> else {
    ...
    let mut id: __be32 = nla_get_be32(cda[CTA_EXPECT_ID]);
...
#[no_mangle]
unsafe extern "C" fn expect_iter_name(exp: *mut nf_conntrack_expect, data: *mut c_void) -> bool {
...
#[no_mangle]
unsafe extern "C" fn expect_iter_all(exp: *mut nf_conntrack_expect, data: *mut c_void) -> bool {
...
// This basically means we have to flush everything
    nf_ct_expect_iterate_net(info.net, expect_iter_all, core::ptr::null_mut(),
    NETLINK_CB(skb).portid,
    nlmsg_report(info.nlh));
    }
    return 0;
    ...
    exp_nat_nla_policy, core::ptr::null_mut());
    if (err < 0)
    return err;
...
// Look for master conntrack of this expectation
    ...
    ct = nf_ct_tuplehash_to_ctrack(h);
    rcu_read_lock();
    exp = ctnetlink_alloc_expect(cda, ct, core::ptr::null_mut(), &tuple, &mask);
    if (IS_ERR(exp)) {
    err = PTR_ERR(exp);
    goto err_rcu;
    }
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_net_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_net_pre_exit(net: *mut net) {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ctnetlink_exit() -> void __exit {
    ...
    RCU_INIT_POINTER(nfnl_ct_hook, core::ptr::null_mut());
