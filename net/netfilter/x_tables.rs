...
pub const XT_PCPU_BLOCK_SIZE: c_int = 4096;
...
// called when table is needed in the given netns
    pub net): *mut *mut int (table_init)(struct net,
    pub me: *mut module,
// A unique name...
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xt_pernet {
    pub tables: [list_head; NFPROTO_NUMPROTO],
// stash area used during netns exit
    pub dead_tables: [list_head; NFPROTO_NUMPROTO],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_delta {
    pub /: *mut *mut unsigned int offset; / offset in kernel,
    pub /: *mut *mut int delta; / delta in 32bit user land,
}
...
pub struct xt_af {
    pub mutex: mutex,
    pub match: list_head,
    pub target: list_head,

    pub compat_mutex: mutex,
    pub compat_tab: *mut compat_delta,
    pub /: *mut *mut unsigned int number; / number of slots in compat_tab[],
    pub /: *mut *mut unsigned int cur; / number of used slots in compat_tab[],
...
#[no_mangle]
pub unsafe extern "C" fn xt_register_target(target: *mut xt_target) -> c_int {
    int xt_register_target(struct xt_target *target)
    {
    let mut af: u_int8_t = target.family;
...
#[no_mangle]
pub unsafe extern "C" fn xt_register_match(match: *mut xt_match) -> c_int {
    int xt_register_match(struct xt_match *match)
    {
    let mut af: u_int8_t = match.family;
    mutex_lock(&xt[af].mutex);
...
#[no_mangle]
unsafe extern "C" fn match_revfn(af: u8, name: *const c_char, revision: u8, bestp: *mut c_int) -> c_int {
    static int match_revfn(u8 af, const char *name, u8 revision, int *bestp)
    {
    const struct xt_match *m;
    let mut have_rev: c_int = 0;
    mutex_lock(&xt[af].mutex);
    list_for_each_entry(m, &xt[af].match, list) {
    if (strcmp(m.name, name) == 0) {
    if (m.revision > *bestp)
...
#[no_mangle]
unsafe extern "C" fn target_revfn(af: u8, name: *const c_char, revision: u8, bestp: *mut c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn xt_check_proc_name(name: *const c_char, size: c_uint) -> c_int {
...
//
    ...
    if (par.match.table != core::ptr::null_mut() &&
...
#[no_mangle]
unsafe extern "C" fn xt_checkentry_match(par: *mut xt_mtchk_param) -> c_int {
    static int xt_checkentry_match(struct xt_mtchk_param *par)
    {
    int ret;
    if (par.match.checkentry != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn xt_check_hooks_match(par: *mut xt_mtchk_param) -> c_int {
    int xt_check_hooks_match(struct xt_mtchk_param *par)
    {
    if (par.match.check_hooks != core::ptr::null_mut())
...
//
    ...
    let mut length: c_int = target - match;
...
//
#[no_mangle]
pub unsafe extern "C" fn xt_check_table_hooks(info: *const xt_table_info, valid_hooks: c_uint) -> c_int {
    int xt_check_table_hooks(const struct xt_table_info *info, unsigned int valid_hooks)
    {
    const char *err = "unsorted underflow";
    unsigned int i, max_uflow, max_entry;
    let mut check_hooks: bool = false;
...
#[no_mangle]
unsafe extern "C" fn verdict_ok(verdict: c_int) -> bool {
    static bool verdict_ok(int verdict)
    {
    if (verdict > 0)
    return true;
    if (verdict < 0) {
    let mut v: c_int = -verdict - 1;
    ...
    {
    let mut usersize: return = = kernsize && strnlen(msg, msglen) < msglen;
    }

#[no_mangle]
pub unsafe extern "C" fn xt_compat_add_offset(af: u_int8_t, offset: c_uint, delta: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn xt_compat_flush_offsets(af: u_int8_t) {
    void xt_compat_flush_offsets(u_int8_t af)
    {
    WARN_ON(!mutex_is_locked(&xt[af].compat_mutex));
    if (xt[af].compat_tab) {
    vfree(xt[af].compat_tab);
    xt[af].compat_tab = core::ptr::null_mut();
    xt[af].number = 0;
    xt[af].cur = 0;
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn xt_compat_calc_jump(af: u_int8_t, offset: c_uint) -> c_int {
    ...
    left = mid + 1;
#[no_mangle]
pub unsafe extern "C" fn if(tmp[mid].offset: offset <) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn xt_compat_init_offsets(af: u8, number: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn xt_compat_match_offset(match: *const xt_match) -> c_int {
    int xt_compat_match_offset(const struct xt_match *match)
    {
    let mut csize: u_int16_t = match.compatsize ? : match.matchsize;
    ...
    let mut off: c_int = xt_compat_match_offset(match);
    let mut msize: u_int16_t = cm.u.user.match_size;
...
// dstptr += msize;
    ...
    let mut off: c_int = xt_compat_match_offset(match);
    let mut msize: u_int16_t = m.u.user.match_size - off;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xt_standard_target {
// Must be last as it ends in a flexible-array member.
    TRAILING_OVERLAP(struct compat_xt_entry_target, t, data,
    pub verdict: compat_uint_t,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_xt_error_target {
// Must be last as it ends in a flexible-array member.
    TRAILING_OVERLAP(struct compat_xt_entry_target, t, data,
    ...
    {
    let mut size_of_base_struct: c_long = elems - (const char *)base;
...
//
    ...
    hi = m;
#[no_mangle]
pub unsafe extern "C" fn if(target: offsets[m] <) -> else {
    ...
    if (par.target.table != core::ptr::null_mut() &&
...
#[no_mangle]
pub unsafe extern "C" fn xt_check_hooks_target(par: *mut xt_tgchk_param) -> c_int {
    int xt_check_hooks_target(struct xt_tgchk_param *par)
    {
    if (par.target.check_hooks != core::ptr::null_mut())
    return par.target.check_hooks(par);
    return 0;
    }
    EXPORT_SYMBOL_GPL(xt_check_hooks_target);
#[no_mangle]
unsafe extern "C" fn xt_checkentry_target(par: *mut xt_tgchk_param) -> c_int {
    static int xt_checkentry_target(struct xt_tgchk_param *par)
    {
    int ret;
    if (par.target.checkentry != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn xt_compat_target_offset(target: *const xt_target) -> c_int {
    int xt_compat_target_offset(const struct xt_target *target)
    {
    let mut csize: u_int16_t = target.compatsize ? : target.targetsize;
    ...
    let mut off: c_int = xt_compat_target_offset(target);
    let mut tsize: u_int16_t = ct.u.user.target_size;
...
// dstptr += tsize;
    ...
    let mut off: c_int = xt_compat_target_offset(target);
    let mut tsize: u_int16_t = t.u.user.target_size - off;
...
// dstptr += tsize;
    ...
    {
    struct xt_table_info *info = core::ptr::null_mut();
    let mut sz: usize = sizeof(*info) + size;
...
#[no_mangle]
pub unsafe extern "C" fn xt_free_table_info(info: *mut xt_table_info) {
    void xt_free_table_info(struct xt_table_info *info)
    {
    int cpu;
    if (info.jumpstack != core::ptr::null_mut()) {
...
// Find table by name, grabs mutex & ref.  Returns ERR_PTR on error.
    ...
    {
    struct xt_pernet *xt_net = net_generic(net, xt_pernet_id);
    struct module *owner = core::ptr::null_mut();
...
// and once again:
    ...
    if (IS_ERR(t)) {
    let mut err: c_int = request_module("%stable_%s", xt_prefix[af], name);
    if (err < 0)
    return ERR_PTR(err);
...
#[no_mangle]
pub unsafe extern "C" fn xt_table_unlock(table: *mut xt_table) {
...
#[no_mangle]
pub unsafe extern "C" fn xt_compat_lock(af: u_int8_t) {
...
#[no_mangle]
pub unsafe extern "C" fn xt_compat_unlock(af: u_int8_t) {
...
#[no_mangle]
unsafe extern "C" fn xt_jumpstack_alloc(i: *mut xt_table_info) -> c_int {
    ...
    if (i.jumpstack == core::ptr::null_mut())
    return -ENOMEM;
...
//
    ...
    cpu_to_node(cpu));
    if (i.jumpstack[cpu] == core::ptr::null_mut())
//
...
// ... so wait for even xt_recseq on all cpus
    ...
    let mut seq: u32 = raw_read_seqcount(s);
    if (seq & 1) {
    do {
    cond_resched();
    cpu_relax();
    ...
    {
    struct xt_pernet *xt_net = net_generic(net, xt_pernet_id);
    struct xt_table *t, *table = core::ptr::null_mut();
    struct nf_hook_ops *ops = core::ptr::null_mut();
    struct xt_table_info *private;
...
//
#[no_mangle]
pub unsafe extern "C" fn xt_unregister_table_pre_exit(net: *mut net, af: u8, name: *const c_char) {
    ...
    mutex_unlock(&xt[af].mutex);
    if (t.ops) /* nat table registers with nat core, t.ops is core::ptr::null_mut(). */
...
//
    ...
    {
    let mut af: u8 = (unsigned long)pde_data(file_inode(seq.file));
...
#[no_mangle]
unsafe extern "C" fn xt_table_seq_stop(seq: *mut seq_file, v: *mut c_void) {
    static void xt_table_seq_stop(struct seq_file *seq, void *v)
    {
    let mut af: u_int8_t = (unsigned long)pde_data(file_inode(seq.file));
    mutex_unlock(&xt[af].mutex);
    }
#[no_mangle]
unsafe extern "C" fn xt_table_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nf_mttg_trav {
    pub curr: *mut *mut list_head head,,
    pub class: u8,
}
    ...
    let mut nfproto: u8 = (unsigned long)pde_data(file_inode(seq.file));
    struct nf_mttg_trav *trav = seq.private;
    if (ppos != core::ptr::null_mut())
    ++(*ppos);
    switch (trav.class) {
    case MTTG_TRAV_INIT:
    ...
    for (j = 0; j < *pos; ++j)
    if (xt_mttg_seq_next(seq, core::ptr::null_mut(), core::ptr::null_mut(), is_target) == core::ptr::null_mut())
    return core::ptr::null_mut();
    return trav;
    }
#[no_mangle]
unsafe extern "C" fn xt_mttg_seq_stop(seq: *mut seq_file, v: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn xt_match_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xt_target_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
//
    ...
    {
    let mut hook_mask: c_uint = table.valid_hooks;
    ...
    {
    let mut ret: c_int = -EBUSY, af = table.af;
...
#[no_mangle]
pub unsafe extern "C" fn xt_unregister_template(table: *const xt_table) {
    void xt_unregister_template(const struct xt_table *table)
    {
    struct xt_template *t;
    let mut af: c_int = table.af;
    mutex_lock(&xt[af].mutex);
    list_for_each_entry(t, &xt_templates[af], list) {
...
#[no_mangle]
pub unsafe extern "C" fn xt_proto_init(net: *mut net, af: u_int8_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn xt_proto_fini(net: *mut net, af: u_int8_t) {
...
//
    ...
    counter.pcnt = ( unsigned long)(state.mem + state.off);
    state.off += sizeof(*counter);
    if (state.off > (XT_PCPU_BLOCK_SIZE - sizeof(*counter))) {
    state.mem = core::ptr::null_mut();
    state.off = 0;
    }
    return true;
    }
...
#[no_mangle]
pub unsafe extern "C" fn xt_percpu_counter_free(counters: *mut xt_counters) {
    void xt_percpu_counter_free(struct xt_counters *counters)
    {
    let mut pcnt: c_ulong = counters.pcnt;
...
#[no_mangle]
unsafe extern "C" fn xt_net_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn xt_net_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn xt_init() -> int __init {
    ...
    xt[i].compat_tab = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn xt_fini() -> void __exit {
