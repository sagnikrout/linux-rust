...
pub const INITIAL_POLICIES_ALLOC: c_int = 10;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct netlink_policy_dump_state {
    pub policy_idx: c_uint,
    pub attr_idx: c_uint,
    pub n_alloc: c_uint,
    struct {
    pub policy: *const nla_policy,
    pub maxtype: c_uint,
    pub __counted_by(n_alloc): } policies[],
}
...
//
#[no_mangle]
pub unsafe extern "C" fn netlink_policy_dump_loop(state: *mut netlink_policy_dump_state) -> bool {
    bool netlink_policy_dump_loop(struct netlink_policy_dump_state *state)
    {
    return !netlink_policy_dump_finished(state);
    }
#[no_mangle]
pub unsafe extern "C" fn netlink_policy_dump_attr_size_estimate(pt: *const nla_policy) -> c_int {
    int netlink_policy_dump_attr_size_estimate(const struct nla_policy *pt)
    {
// nested + type
    let mut common: c_int = 2 * nla_attr_size(sizeof(u32));
    switch (pt.type) {
    case NLA_UNSPEC:
    case NLA_REJECT:
...
// this should then cause a warning later
    ...
    int nestattr)
    {
    let mut estimate: c_int = netlink_policy_dump_attr_size_estimate(pt);
...
#[no_mangle]
pub unsafe extern "C" fn if(NLA_U16: pt->type ==) -> else {
    else if (pt.type == NLA_U16)
    type = NL_ATTR_TYPE_U16;
#[no_mangle]
pub unsafe extern "C" fn if(NLA_U32: pt->type ==) -> else {
    else if (pt.type == NLA_U32)
    type = NL_ATTR_TYPE_U32;
#[no_mangle]
pub unsafe extern "C" fn if(NLA_U64: pt->type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(NLA_S16: pt->type ==) -> else {
    else if (pt.type == NLA_S16)
    type = NL_ATTR_TYPE_S16;
#[no_mangle]
pub unsafe extern "C" fn if(NLA_S32: pt->type ==) -> else {
    else if (pt.type == NLA_S32)
    type = NL_ATTR_TYPE_S32;
#[no_mangle]
pub unsafe extern "C" fn if(NLA_S64: pt->type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(NLA_NUL_STRING: pt->type ==) -> else {
...
//
    ...
    int nestattr)
    {
    return __netlink_policy_dump_write_attr(core::ptr::null_mut(), skb, pt, nestattr);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn netlink_policy_dump_free(state: *mut netlink_policy_dump_state) {
