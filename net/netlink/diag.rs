...
#[no_mangle]
unsafe extern "C" fn sk_diag_dump_groups(sk: *mut sock, nlskb: *mut sk_buff) -> c_int {
    static int sk_diag_dump_groups(struct sock *sk, struct sk_buff *nlskb)
    {
    struct netlink_sock *nlk = nlk_sk(sk);
    if (nlk.groups == core::ptr::null_mut())
    return 0;
    return nla_put(nlskb, NETLINK_DIAG_GROUPS, NLGRPSZ(nlk.ngroups),
    nlk.groups);
    }
#[no_mangle]
unsafe extern "C" fn sk_diag_put_flags(sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn netlink_diag_dump(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
    static int netlink_diag_dump(struct sk_buff *skb, struct netlink_callback *cb)
    {
    struct netlink_diag_req *req;
    let mut s_num: c_int = cb.args[0];
    let mut err: c_int = 0;
    req = nlmsg_data(cb.nlh);
...
#[no_mangle]
unsafe extern "C" fn netlink_diag_dump_done(cb: *mut netlink_callback) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn netlink_diag_handler_dump(skb: *mut sk_buff, h: *mut nlmsghdr) -> c_int {
    static int netlink_diag_handler_dump(struct sk_buff *skb, struct nlmsghdr *h)
    {
    let mut hdrlen: c_int = sizeof(struct netlink_diag_req);
    struct net *net = sock_net(skb.sk);
...
#[no_mangle]
unsafe extern "C" fn netlink_diag_init() -> int __init {
    static int __init netlink_diag_init(void)
    {
    return sock_diag_register(&netlink_diag_handler);
    }
#[no_mangle]
unsafe extern "C" fn netlink_diag_exit() -> void __exit {
