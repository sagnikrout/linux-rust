...
// remember: uninitialized global data is zeroed because its in .bss
    let mut llc_ui_sap_last_autoport: static u16 = LLC_SAP_DYN_START;
...
pub const LLC_CMSG_PKTINFO: c_int = 1;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn llc_ui_next_link_no(sap: c_int) -> u16 {
...
//
#[no_mangle]
pub unsafe extern "C" fn llc_proto_type(arphrd: u16) -> __be16 {
...
//
#[no_mangle]
pub unsafe extern "C" fn llc_ui_addr_null(addr: *mut sockaddr_llc) -> u8 {
...
//
#[no_mangle]
pub unsafe extern "C" fn llc_ui_header_len(sk: *mut sock, addr: *mut sockaddr_llc) -> u8 {
    static inline u8 llc_ui_header_len(struct sock *sk, struct sockaddr_llc *addr)
    {
    let mut rc: u8 = LLC_PDU_LEN_U;
    if (addr.sllc_test)
    rc = LLC_PDU_LEN_U;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: addr->sllc_xid) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(SOCK_STREAM: sk->sk_type ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_send_data(sk: *mut *mut sock, skb: *mut sk_buff, noblock: c_int) -> c_int {
    static int llc_ui_send_data(struct sock* sk, struct sk_buff *skb, int noblock)
    {
    let mut llc: *mut llc_sock = llc_sk(sk);
    if (unlikely(llc_data_accept_state(llc.state) ||
    llc.remote_busy_flag ||
    llc.p_flag)) {
    let mut timeout: c_long = sock_sndtimeo(sk, noblock);
    int rc;
...
#[no_mangle]
unsafe extern "C" fn llc_ui_sk_init(sock: *mut socket, sk: *mut sock) {
...
//
    ...
    {
    struct sock *sk;
    let mut rc: c_int = -ESOCKTNOSUPPORT;
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_release(sock: *mut socket) -> c_int {
    static int llc_ui_release(struct socket *sock)
    {
    struct sock *sk = sock.sk;
    struct llc_sock *llc;
    if (unlikely(sk == core::ptr::null_mut()))
    goto out;
    sock_hold(sk);
    lock_sock(sk);
    llc = llc_sk(sk);
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_autoport() -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_autobind(sock: *mut socket, addr: *mut sockaddr_llc) -> c_int {
    ...
    sap = llc_sap_open(llc.laddr.lsap, core::ptr::null_mut());
    if (!sap)
    goto out;
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_bind(sock: *mut socket, uaddr: *mut sockaddr_unsized, addrlen: c_int) -> c_int {
    ...
    if (!sap) {
    sap = llc_sap_open(addr.sllc_sap, core::ptr::null_mut());
    rc = -EBUSY; /* some other network layer is using the sap */
    if (!sap)
    goto out;
    } else {
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_shutdown(sock: *mut socket, how: c_int) -> c_int {
...
// bind to sap with null dev, exclusive
    ...
    let mut timeo: c_long = sock_sndtimeo(sk, flags & O_NONBLOCK);
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_listen(sock: *mut socket, backlog: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn llc_ui_wait_for_disc(sk: *mut sock, timeout: c_long) -> c_int {
    static int llc_ui_wait_for_disc(struct sock *sk, long timeout)
    {
...
#[no_mangle]
unsafe extern "C" fn llc_ui_wait_for_conn(sk: *mut sock, timeout: c_long) -> bool {
    static bool llc_ui_wait_for_conn(struct sock *sk, long timeout)
    {
    DEFINE_WAIT_FUNC(wait, woken_wake_function);
    add_wait_queue(sk_sleep(sk), &wait);
...
#[no_mangle]
unsafe extern "C" fn llc_ui_wait_for_busy_core(sk: *mut sock, timeout: c_long) -> c_int {
    static int llc_ui_wait_for_busy_core(struct sock *sk, long timeout)
    {
    DEFINE_WAIT_FUNC(wait, woken_wake_function);
    struct llc_sock *llc = llc_sk(sk);
...
#[no_mangle]
unsafe extern "C" fn llc_wait_data(sk: *mut sock, timeo: c_long) -> c_int {
...
//
    ...
    break;
    rc = 0;
    if (sk_wait_data(sk, &timeo, core::ptr::null_mut()))
    break;
    }
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn llc_cmsg_rcv(msg: *mut msghdr, skb: *mut sk_buff) {
...
//
    ...
    let mut nonblock: c_int = flags & MSG_DONTWAIT;
    struct sk_buff *skb = core::ptr::null_mut();
    struct sock *sk = sock.sk;
    struct llc_sock *llc = llc_sk(sk);
    let mut copied: usize = 0;
    let mut peek_seq: u32 = 0;
    u32 *seq, skb_len;
    unsigned long used;
...
//
    ...
    sk_wait_data(sk, &timeo, core::ptr::null_mut());
...
// Ok so how much can we use?
    ...
    used = len;
    if (!(flags & MSG_TRUNC)) {
    let mut rc: c_int = skb_copy_datagram_msg(skb, offset, msg, used);
    if (rc) {
// Exception. Bailout!
    if (!copied)
    copied = -EFAULT;
...
// seq = 0;
    ...
    return copied;
    copy_uaddr:
    if (uaddr != core::ptr::null_mut() && skb != core::ptr::null_mut()) {
...
//
#[no_mangle]
unsafe extern "C" fn llc_ui_sendmsg(sock: *mut socket, msg: *mut msghdr, len: usize) -> c_int {
    ...
    let mut noblock: c_int = flags & MSG_DONTWAIT;
    let mut rc: c_int = -EINVAL, copied = 0, hdrlen, hh_len;
    struct sk_buff *skb = core::ptr::null_mut();
...
//
    ...
    struct sock *sk = sock.sk;
    struct llc_sock *llc = llc_sk(sk);
    let mut val: c_int = 0, len, rc = -EINVAL;
    lock_sock(sk);
...
#[no_mangle]
unsafe extern "C" fn llc2_init() -> int __init {
    static int __init llc2_init(void)
    {
    let mut rc: c_int = proto_register(&llc_proto, 0);
    if (rc != 0)
    goto out;
...
#[no_mangle]
unsafe extern "C" fn llc2_exit() -> void __exit {
