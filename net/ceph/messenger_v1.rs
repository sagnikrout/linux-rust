...
//! Source: net/ceph/messenger_v1.c
#![no_std]
...
// static tag bytes (protocol control messages)
    let mut tag_msg: static char = CEPH_MSGR_TAG_MSG;
    let mut tag_ack: static char = CEPH_MSGR_TAG_ACK;
    let mut tag_keepalive: static char = CEPH_MSGR_TAG_KEEPALIVE;
    let mut tag_keepalive2: static char = CEPH_MSGR_TAG_KEEPALIVE2;
//
// If @buf is NULL, discard up to @len bytes.
//
#[no_mangle]
unsafe extern "C" fn ceph_tcp_recvmsg(sock: *mut socket, buf: *mut c_void, len: usize) -> c_int {
    static int ceph_tcp_recvmsg(struct socket *sock, void *buf, size_t len)
    {
    let mut iov: kvec = {buf, len};
    let mut msg: msghdr = { .msg_flags = MSG_DONTWAIT | MSG_NOSIGNAL };
    int r;
    if (!buf)
...
#[no_mangle]
unsafe extern "C" fn con_out_kvec_reset(con: *mut ceph_connection) {
    ...
    {
    let mut index: c_int = con.v1.out_kvec_left;
    BUG_ON(con.v1.out_skip);
...
//
#[no_mangle]
unsafe extern "C" fn con_out_kvec_skip(con: *mut ceph_connection) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sizeof_footer(con: *mut ceph_connection) -> usize {
...
#[no_mangle]
unsafe extern "C" fn prepare_message_data(msg: *mut ceph_msg, data_len: u32) {
    static void prepare_message_data(struct ceph_msg *msg, u32 data_len)
    {
// Initialize data cursor if it's not a sparse read
    let mut len: u64 = msg.sparse_read_total ? : data_len;
...
//
#[no_mangle]
unsafe extern "C" fn prepare_write_ack(con: *mut ceph_connection) {
...
//
#[no_mangle]
unsafe extern "C" fn prepare_write_seq(con: *mut ceph_connection) {
...
//
#[no_mangle]
unsafe extern "C" fn prepare_write_keepalive(con: *mut ceph_connection) {
...
//
#[no_mangle]
unsafe extern "C" fn get_connect_authorizer(con: *mut ceph_connection) -> c_int {
    static int get_connect_authorizer(struct ceph_connection *con)
    {
    struct ceph_auth_handshake *auth;
    int auth_proto;
    if (!con.ops.get_authorizer) {
    con.v1.auth = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn prepare_write_banner(con: *mut ceph_connection) {
...
#[no_mangle]
unsafe extern "C" fn __prepare_write_connect(con: *mut ceph_connection) {
...
#[no_mangle]
unsafe extern "C" fn prepare_write_connect(con: *mut ceph_connection) -> c_int {
    static int prepare_write_connect(struct ceph_connection *con)
    {
    let mut global_seq: c_uint = ceph_get_global_seq(con.msgr, 0);
    int proto;
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn write_partial_kvec(con: *mut ceph_connection) -> c_int {
...
//
    ...
    let mut do_datacrc: bool = !ceph_test_opt(from_msgr(con.msgr), NOCRC);
    u32 crc;
    dout("%s %p msg %p\n", __func__, con, msg);
...
//
#[no_mangle]
unsafe extern "C" fn write_partial_skip(con: *mut ceph_connection) -> c_int {
    static int write_partial_skip(struct ceph_connection *con)
    {
    int ret;
    dout("%s %p %d left\n", __func__, con, con.v1.out_skip);
    while (con.v1.out_skip > 0) {
    let mut size: usize = min(con.v1.out_skip, (int)PAGE_SIZE);
...
//
#[no_mangle]
unsafe extern "C" fn prepare_read_banner(con: *mut ceph_connection) {
...
#[no_mangle]
unsafe extern "C" fn prepare_read_connect(con: *mut ceph_connection) {
...
#[no_mangle]
unsafe extern "C" fn prepare_read_ack(con: *mut ceph_connection) {
...
#[no_mangle]
unsafe extern "C" fn prepare_read_seq(con: *mut ceph_connection) {
...
#[no_mangle]
unsafe extern "C" fn prepare_read_tag(con: *mut ceph_connection) {
...
#[no_mangle]
unsafe extern "C" fn prepare_read_keepalive_ack(con: *mut ceph_connection) {
...
//
#[no_mangle]
unsafe extern "C" fn prepare_read_message(con: *mut ceph_connection) -> c_int {
    ...
    let mut left: c_int = end - con.v1.in_base_pos;
    let mut have: c_int = size - left;
    let mut ret: c_int = ceph_tcp_recvmsg(con.sock, object + have, left);
    if (ret <= 0)
    return ret;
...
//
#[no_mangle]
unsafe extern "C" fn read_partial_banner(con: *mut ceph_connection) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn read_partial_connect(con: *mut ceph_connection) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn verify_hello(con: *mut ceph_connection) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn process_banner(con: *mut ceph_connection) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn process_connect(con: *mut ceph_connection) -> c_int {
    static int process_connect(struct ceph_connection *con)
    {
    let mut sup_feat: u64 = from_msgr(con.msgr).supported_features;
    let mut req_feat: u64 = from_msgr(con.msgr).required_features;
    let mut server_feat: u64 = le64_to_cpu(con.v1.in_reply.features);
    int ret;
    dout("process_connect on %p tag %d\n", con, con.v1.in_tag);
    if (con.v1.auth) {
    let mut len: c_int = le32_to_cpu(con.v1.in_reply.authorizer_len);
//
...
//
#[no_mangle]
unsafe extern "C" fn read_partial_ack(con: *mut ceph_connection) -> c_int {
    static int read_partial_ack(struct ceph_connection *con)
    {
    let mut size: c_int = sizeof(con.v1.in_temp_ack);
    let mut end: c_int = size;
...
//
#[no_mangle]
unsafe extern "C" fn process_ack(con: *mut ceph_connection) {
    static void process_ack(struct ceph_connection *con)
    {
    let mut ack: u64 = le64_to_cpu(con.v1.in_temp_ack);
    ...
    BUG_ON(section.iov_base == core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn read_partial_sparse_msg_extent(con: *mut ceph_connection, crc: *mut u32) -> c_int {
    static int read_partial_sparse_msg_extent(struct ceph_connection *con, u32 *crc)
    {
    struct ceph_msg_data_cursor *cursor = &con.in_msg.cursor;
    let mut do_bounce: bool = ceph_test_opt(from_msgr(con.msgr), RXBOUNCE);
...
#[no_mangle]
unsafe extern "C" fn read_partial_sparse_msg_data(con: *mut ceph_connection) -> c_int {
    ...
    &crc);
#[no_mangle]
pub unsafe extern "C" fn if(0: cursor->sr_resid >) -> else {
...
#[no_mangle]
unsafe extern "C" fn read_partial_msg_data(con: *mut ceph_connection) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn read_partial_msg_data_bounce(con: *mut ceph_connection) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn read_partial_message(con: *mut ceph_connection) -> c_int {
    ...
    let mut do_datacrc: bool = !ceph_test_opt(from_msgr(con.msgr), NOCRC);
    let mut need_sign: bool = (con.peer_features & CEPH_FEATURE_MSG_AUTH);
    u64 seq;
    u32 crc;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ceph_test_opt(from_msgr(con->msgr), _arg: RXBOUNCE)) -> else {
...
#[no_mangle]
unsafe extern "C" fn read_keepalive_ack(con: *mut ceph_connection) -> c_int {
    static int read_keepalive_ack(struct ceph_connection *con)
    {
    struct ceph_timespec ceph_ts;
    let mut size: usize = sizeof(ceph_ts);
    let mut ret: c_int = read_partial(con, size, size, &ceph_ts);
    if (ret <= 0)
    return ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_con_v1_try_read(con: *mut ceph_connection) -> c_int {
...
//
    ret = ceph_tcp_recvmsg(con.sock, core::ptr::null_mut(), -con.v1.in_base_pos);
    if (ret <= 0)
    goto out;
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_con_v1_try_write(con: *mut ceph_connection) -> c_int {
...
// msg pages?
    ...
    ceph_msg_put(msg);
    con.out_msg = core::ptr::null_mut();   /* we're done with this one */
    goto do_next;
    }
...
// is anything else pending?
    if ((msg = ceph_con_get_out_msg(con)) != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_con_v1_revoke(con: *mut ceph_connection, msg: *mut ceph_msg) {
...
pub unsafe extern "C" fn ceph_con_v1_revoke_incoming(con: *mut ceph_connection) {
    ...
    {
    let mut front_len: c_uint = le32_to_cpu(con.v1.in_hdr.front_len);
    let mut middle_len: c_uint = le32_to_cpu(con.v1.in_hdr.middle_len);
    let mut data_len: c_uint = le32_to_cpu(con.v1.in_hdr.data_len);
