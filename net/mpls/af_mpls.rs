...
pub const MAX_MPLS_ROUTE_MEM: c_int = 4096;
// Maximum number of labels to look ahead at when selecting a path of
// a multipath route
//
pub const MAX_MP_SELECT_LABELS: c_int = 4;

    let mut label_limit: static int = (1 << 20) - 1;
    let mut ttl_max: static int = 255;

#[no_mangle]
unsafe extern "C" fn ipgre_mpls_encap_hlen(e: *mut ip_tunnel_encap) -> usize {
...
#[no_mangle]
unsafe extern "C" fn ipgre_tunnel_encap_add_mpls_ops() -> c_int {
    static int ipgre_tunnel_encap_add_mpls_ops(void)
    {
    return ip_tunnel_encap_add_ops(&mpls_iptun_ops, TUNNEL_ENCAP_MPLS);
    }
#[no_mangle]
unsafe extern "C" fn ipgre_tunnel_encap_del_mpls_ops() {
...
#[no_mangle]
pub unsafe extern "C" fn mpls_output_possible(dev: *const net_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mpls_nh_header_size(nh: *const mpls_nh) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn mpls_dev_mtu(dev: *const net_device) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn mpls_pkt_too_big(skb: *const sk_buff, mtu: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mpls_multipath_hash(rt: *mut mpls_route, skb: *mut sk_buff) -> u32 {
    static u32 mpls_multipath_hash(struct mpls_route *rt, struct sk_buff *skb)
    {
    struct mpls_entry_decoded dec;
    let mut mpls_hdr_len: c_uint = 0;
    struct mpls_shim_hdr *hdr;
    let mut eli_seen: bool = false;
    int label_index;
    let mut hash: u32 = 0;
...
//
    ...
    goto out;
    for_nexthops(rt) {
    let mut nh_flags: c_uint = READ_ONCE(nh.nh_flags);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: hdr6->hop_limit) -> else {
...
// Careful this entire function runs inside of an rcu critical section
    ...
    goto err;
    if ((skb = skb_share_check(skb, GFP_ATOMIC)) == core::ptr::null_mut())
    goto err;
...
// If via wasn't specified then send out using device address
    ...
    return 0;
    tx_err:
    out_mdev = out_dev ? mpls_dev_rcu(out_dev) : core::ptr::null_mut();
    if (out_mdev)
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mpls_route_config {
    pub rc_protocol: u32,
    pub rc_ifindex: u32,
    pub rc_via_table: u8,
    pub rc_via_alen: u8,
    pub rc_via: [u8; MAX_VIA_ALEN],
    pub rc_label: u32,
    pub rc_ttl_propagate: u8,
    pub rc_output_labels: u8,
    pub rc_output_label: [u32; MAX_NEW_LABELS],
    pub rc_nlflags: u32,
    pub rc_payload_type: enum mpls_payload_type,
    pub rc_nlinfo: nl_info,
    pub rc_mp: *mut rtnexthop,
    pub rc_mp_len: c_int,
}
...
//
    ...
    {
    let mut nh_size: u8 = MPLS_NH_SIZE(max_labels, max_alen);
    struct mpls_route *rt;
    size_t size;
...
#[no_mangle]
unsafe extern "C" fn mpls_rt_free_rcu(head: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn mpls_rt_free(rt: *mut mpls_route) {
    ...
    {
    struct nlmsghdr *nlh = info ? info.nlh : core::ptr::null_mut();
    let mut portid: unsigned = info ? info.portid : 0;
    let mut event: c_int = new ? RTM_NEWROUTE : RTM_DELROUTE;
    struct mpls_route *rt = new ? new : old;
    let mut nlm_flags: unsigned = (old && new) ? NLM_F_REPLACE : 0;
...
#[no_mangle]
unsafe extern "C" fn find_free_label(net: *mut net) -> c_uint {
...
// leftover implies invalid nexthop configuration, discard it
    ...
    let mut remaining: c_int = cfg.rc_mp_len;
    let mut err: c_int = 0;
    let mut nhs: u8 = 0;
    change_nexthops(rt) {
    int attrlen;
    nla_via = core::ptr::null_mut();
    nla_newdst = core::ptr::null_mut();
    err = -EINVAL;
...
// Append makes no sense with mpls
    ...
    if (!mpls_label_ok(net, &index, extack))
    goto errout;
    mpls_route_update(net, index, core::ptr::null_mut(), &cfg.rc_nlinfo);
    err = 0;
    errout:
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn mpls_get_stats_af_size(dev: *const net_device) -> usize {
...
#[no_mangle]
unsafe extern "C" fn mpls_netconf_msgsize_devconf(type: c_int) -> c_int {
...
// -EMSGSIZE implies BUG in mpls_netconf_msgsize_devconf()
    ...
    rtnl_notify(skb, net, 0, RTNLGRP_MPLS_NETCONF, core::ptr::null_mut(), GFP_KERNEL);
    return;
    errout:
...
// -EMSGSIZE implies BUG in mpls_netconf_msgsize_devconf()
    ...
    {
    let mut oval: c_int = *(int *)ctl.data;
    let mut ret: c_int = proc_dointvec(ctl, write, buffer, lenp, ppos);
    if (write) {
    struct mpls_dev *mdev = ctl.extra1;
    let mut i: c_int = (int *)ctl.data - (int *)mdev;
    struct net *net = ctl.extra2;
    let mut val: c_int = *(int *)ctl.data;
    ...
    let mut table_size: usize = ARRAY_SIZE(mpls_dev_table);
...
#[no_mangle]
unsafe extern "C" fn mpls_dev_destroy_rcu(head: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn mpls_ifdown(dev: *mut net_device, event: c_int) -> c_int {
...
// if there are no more nexthops, delete the route
    ...
    mpls_route_update(net, index, core::ptr::null_mut(), core::ptr::null_mut());
    continue;
    }
    if (nh_del) {
    ...
    change_nexthops(rt) {
    let mut nh_flags: c_uint = nh.nh_flags;
    if (nh.nh_dev != dev) {
    if (nh_del)
    ...
    if (nh_del)
    mpls_route_update(net, index, rt, core::ptr::null_mut());
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn mpls_ifup(dev: *mut net_device, flags: c_uint) {
    ...
    RCU_INIT_POINTER(dev.mpls_ptr, core::ptr::null_mut());
    ...
    struct nlattr *nla;
    struct rtvia *via;
    let mut family: c_int = AF_UNSPEC;
    ...
    {
    let mut len: unsigned = nla_len(nla);
    struct mpls_shim_hdr *nla_label;
    u8 nla_labels;
    bool bos;
    int i;
...
//
    NL_SET_ERR_MSG_ATTR(extack, nla,
    "Implicit core::ptr::null_mut() Label (3) can not be used in encapsulation");
    return -EINVAL;
    }
    label[i] = dec.label;
    }
...
//
    ...
    {
    let mut ttl_propagate: u8 = nla_get_u8(nla);
    if (ttl_propagate > 1) {
    NL_SET_ERR_MSG_ATTR(extack, nla,
...
#[no_mangle]
unsafe extern "C" fn mpls_dump_routes(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn lfib_nlmsg_size(rt: *mut mpls_route) -> usize {
...
// nested attribute
    ...
    {
    struct sk_buff *skb;
    let mut seq: u32 = nlh ? nlh.nlmsg_seq : 0;
    let mut err: c_int = -ENOBUFS;
...
// -EMSGSIZE implies BUG in lfib_nlmsg_size
    ...
    {
    struct net *net = sock_net(in_skb.sk);
    let mut portid: u32 = NETLINK_CB(in_skb).portid;
    let mut in_label: u32 = LABEL_NOT_SPECIFIED;
    struct nlattr *tb[RTA_MAX + 1];
    struct mpls_route *rt = core::ptr::null_mut();
    u32 labels[MAX_NEW_LABELS];
    struct mpls_shim_hdr *hdr;
    let mut hdr_size: c_uint = 0;
    const struct mpls_nh *nh;
    struct net_device *dev;
    struct rtmsg *rtm, *r;
    struct nlmsghdr *nlh;
...
#[no_mangle]
unsafe extern "C" fn resize_platform_label_table(net: *mut net, limit: usize) -> c_int {
    static int resize_platform_label_table(struct net *net, size_t limit)
    {
    let mut size: usize = sizeof(struct mpls_route *) * limit;
    size_t old_limit;
    size_t cp_size;
    struct mpls_route __rcu **labels = core::ptr::null_mut(), **old;
    struct mpls_route *rt0 = core::ptr::null_mut(), *rt2 = core::ptr::null_mut();
    unsigned index;
    if (size) {
...
// If needed set the predefined labels
    ...
    rt2 = core::ptr::null_mut();
    }
    if ((old_limit <= MPLS_LABEL_IPV4NULL) &&
    (limit > MPLS_LABEL_IPV4NULL)) {
    RCU_INIT_POINTER(labels[MPLS_LABEL_IPV4NULL], rt0);
    rt0 = core::ptr::null_mut();
    }
// Update the global pointers
    ...
    let mut platform_labels: c_int = net.mpls.platform_labels;
    int ret;
...
//
    ...
    table_size);
    if (net.mpls.ctl == core::ptr::null_mut()) {
    kfree(table);
    return -ENOMEM;
    }
    return 0;
...
#[no_mangle]
unsafe extern "C" fn mpls_net_exit(net: *mut net) -> __net_exit void {
...
//
    ...
    struct mpls_route *rt;
    rt = mpls_dereference(net, platform_label[index]);
    mpls_notify_route(net, index, rt, core::ptr::null_mut(), core::ptr::null_mut());
    mpls_rt_free(rt);
    }
    mutex_unlock(&net.mpls.platform_mutex);
    ...
    {THIS_MODULE, PF_MPLS, RTM_NEWROUTE, mpls_rtm_newroute, core::ptr::null_mut(),
    RTNL_FLAG_DOIT_UNLOCKED},
    {THIS_MODULE, PF_MPLS, RTM_DELROUTE, mpls_rtm_delroute, core::ptr::null_mut(),
    RTNL_FLAG_DOIT_UNLOCKED},
...
#[no_mangle]
unsafe extern "C" fn mpls_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn mpls_exit() -> void __exit {
