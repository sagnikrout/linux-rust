...
#[no_mangle]
unsafe extern "C" fn mptcp_cap_flag_sha256(flags: u8) -> bool {
...
//
    ...
    mp_opt.csum = get_unaligned(( __sum16 *)ptr);
    mp_opt.suboptions |= OPTION_MPTCP_CSUMREQD;
    ptr += 2;
    }
...
#[no_mangle]
unsafe extern "C" fn clear_3rdack_retransmission(sk: *mut sock) {
...
//
    let mut data_fin_tx_seq: u64 = READ_ONCE(mptcp_sk(subflow.conn).write_seq) - 1;
...
// Zero `use_ack` and `use_map` flags with one shot.
    ...
    opts.csum_reqd = READ_ONCE(msk.csum_enabled);
    mpext = skb ? mptcp_get_ext(skb) : core::ptr::null_mut();
    if (!skb || (mpext && mpext.use_map) || snd_data_fin_enable) {
    let mut map_size: c_uint = TCPOLEN_MPTCP_DSS_BASE + TCPOLEN_MPTCP_DSS_MAP64;
    if (mpext) {
    if (opts.csum_reqd)
...
// size = ALIGN(dss_size, 4);
    ...
    {
    let mut port: u16 = ntohs(addr.port);
...
// MP_RST can be used with MP_FASTCLOSE and MP_FAIL if there is room
    ...
    ret = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mptcp_established_options_dss(sk, _arg: skb, _arg: snd_data_fin, _arg: &opt_size, _arg: opts)) -> else {
    int mp_fail_size;
    ret = true;
...
#[no_mangle]
pub unsafe extern "C" fn __mptcp_expand_seq(old_seq: u64, cur_seq: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn __mptcp_snd_una_update(msk: *mut mptcp_sock, new_snd_una: u64) {
...
#[no_mangle]
pub unsafe extern "C" fn mptcp_update_rcv_data_fin(msk: *mut mptcp_sock, data_fin_seq: u64, use_64bit: bool) -> bool {
...
//
    ...
    let mut hmac: return = = mp_opt.ahmac;
    }
    static bool mptcp_over_limit(struct sock *sk, struct sock *ssk,
    const struct sk_buff *skb)
    {
    struct mptcp_sock *msk = mptcp_sk(sk);
    let mut rcvbuf: u32 = READ_ONCE(sk.sk_rcvbuf);
...
//
#[no_mangle]
pub unsafe extern "C" fn mptcp_incoming_options(sk: *mut sock, skb: *mut sk_buff) -> bool {
...
//
    mptcp_crypto_key_sha(subflow.remote_key, core::ptr::null_mut(),
    &mpext.data_seq);
    mpext.data_seq++;
    mpext.subflow_seq = 1;
    mpext.dsn64 = 1;
...
#[no_mangle]
unsafe extern "C" fn mptcp_track_rwin(tp: *mut tcp_sock) {
...
#[no_mangle]
pub unsafe extern "C" fn __mptcp_make_csum(data_seq: u64, subflow_seq: u32, data_len: u16, sum: __wsum) -> __sum16 {
...
#[no_mangle]
unsafe extern "C" fn mptcp_make_csum(mpext: *const mptcp_ext) -> __sum16 {
...
#[no_mangle]
unsafe extern "C" fn put_len_csum(len: u16, csum: __sum16, data: *mut c_void) {
...
//
    ...
    let mut len: u8 = TCPOLEN_MPTCP_DSS_BASE;
    let mut flags: u8 = 0;
    if (mpext.use_ack) {
...
// ptr++ = mptcp_option(MPTCPOPT_MP_JOIN,
    ...
    let mut len: u8 = TCPOLEN_MPTCP_ADD_ADDR_BASE;
    let mut echo: u8 = MPTCP_ADDR_ECHO;
...
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: opts->addr.family ==) -> else {
    ...
    let mut port: u16 = ntohs(opts.addr.port);
    if (opts.ahmac) {
    u8 *bptr = (u8 *)ptr;
...
#[no_mangle]
pub unsafe extern "C" fn mptcp_get_reset_option(skb: *const sk_buff) -> __be32 {
