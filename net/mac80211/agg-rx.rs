...
#[no_mangle]
unsafe extern "C" fn ieee80211_free_tid_rx(h: *mut rcu_head) {
    ...
    return;
    RCU_INIT_POINTER(sta.ampdu_mlme.tid_rx[tid], core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn sta_rx_agg_reorder_timer_expired(t: *mut timer_list) {
...
// buf_size |= (u16)buf_size_1k <<
    ...
    let mut amsdu: bool = ieee80211_hw_check(&local.hw, SUPPORTS_AMSDU_IN_AMPDU);
    let mut use_ndp: bool = ieee80211_s1g_use_ndp_ba(sdata, sta);
    u16 capab;
    ...
    let mut status: u16 = WLAN_STATUS_REQUEST_DECLINED;
    u16 max_buf_size;
    lockdep_assert_wiphy(sta.local.hw.wiphy);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sta->sta.deflink.he_cap.has_he) -> else {
...
// activate it for RX
    ...
    size_t len)
    {
    let mut req_ndp: bool = mgmt.u.action.action_code == WLAN_ACTION_NDP_ADDBA_REQ;
