...
#[no_mangle]
unsafe extern "C" fn ieee80211_update_apvlan_links(sdata: *mut ieee80211_sub_if_data) {
    ...
    let mut add: c_ulong = sdata.vif.valid_links;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_apvlan_link_setup(sdata: *mut ieee80211_sub_if_data) {
    ...
    let mut new_links: u16 = ap_bss.vif.valid_links;
    unsigned long add;
    int link_id;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_apvlan_link_clear(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_link_setup(link: *mut ieee80211_link_data) {
    ...
    {
    let mut deflink: bool = link_id < 0;
    if (link_id < 0)
    link_id = 0;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_link_stop(link: *mut ieee80211_link_data) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct link_container {
    pub data: ieee80211_link_data,
    pub conf: ieee80211_bss_conf,
}
...
#[no_mangle]
unsafe extern "C" fn ieee80211_check_dup_link_addrs(sdata: *mut ieee80211_sub_if_data) -> c_int {
...
// AP links are not expected to be disabled
    ...
    {
    let mut old_links: u16 = sdata.vif.valid_links;
    let mut old_active: u16 = sdata.vif.active_links;
    let mut add: c_ulong = new_links & ~old_links;
    let mut rem: c_ulong = old_links & ~new_links;
    let mut sta_rem: c_ulong = rem;
    unsigned int link_id;
    int ret;
    ...
    let mut non_sta: bool = sdata.vif.type != NL80211_IFTYPE_STATION;
    struct sta_info *sta;
...
//
    ...
    RCU_INIT_POINTER(sdata.link[link_id], core::ptr::null_mut());
    RCU_INIT_POINTER(sdata.vif.link_conf[link_id], core::ptr::null_mut());
    }
    if (!old_links)
...
// try to remove links that are now invalid from (MLO) stations
    ...
    let mut rem_links: c_ulong = sta.sta.valid_links & sta_rem;
    if (sta.sdata != sdata)
    continue;
//
...
// if we failed during allocation, only free all
    for (link_id = 0; link_id < IEEE80211_MLD_MAX_NUM_LINKS; link_id++) {
    kfree(links[link_id]);
    links[link_id] = core::ptr::null_mut();
    }
    deinit:
    if (use_deflink)
    ieee80211_link_init(sdata, -1, &sdata.deflink,
    ...
    struct ieee80211_local *local = sdata.local;
    let mut old_active: u16 = sdata.vif.active_links;
    let mut rem: c_ulong = old_active & ~active_links;
    let mut add: c_ulong = active_links & ~old_active;
    struct sta_info *sta;
    unsigned int link_id;
    int ret, i;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_set_active_links(vif: *mut ieee80211_vif, active_links: u16) -> c_int {
