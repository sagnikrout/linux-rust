...
//
    ...
    {
    let mut mu_mimo_groups: bool = false;
    let mut mu_mimo_follow: bool = false;
...
// Notify only after setting mu_mimo_owner
    ...
    struct ieee80211_sub_if_data *monitor_sdata = core::ptr::null_mut();
// check flags first
    if (params.flags && ieee80211_sdata_running(sdata)) {
    let mut mask: u32 = MONITOR_FLAG_ACTIVE;
//
...
#[no_mangle]
unsafe extern "C" fn ieee80211_del_iface(wiphy: *mut wiphy, wdev: *mut wireless_dev) -> c_int {
    ...
    RCU_INIT_POINTER(sdata.u.vlan.sta, core::ptr::null_mut());
    ...
    ret = ieee80211_check_combinations(sdata, core::ptr::null_mut(), 0, 0, -1);
    if (ret < 0)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn ieee80211_nan_conf_free(conf: *mut cfg80211_nan_conf) {
    ...
    dst.extra_nan_attrs = core::ptr::null_mut();
    dst.extra_nan_attrs_len = 0;
    kfree(dst.vendor_elems);
    dst.vendor_elems = core::ptr::null_mut();
    ...
    {
    struct ieee80211_sub_if_data *sdata = IEEE80211_WDEV_TO_SUB_IF(wdev);
    let mut new_conf: cfg80211_nan_conf = {};
    let mut ret: c_int = 0;
...
// KRACK protection, shouldn't happen but just silently accept key
    ...
    {
    let mut __maybe_unused: *mut ieee80211_local local = sdata.local;
...
// or maybe it was a WEP key
    ...
    let mut kseq: ieee80211_key_seq = {};
    sdata = IEEE80211_WDEV_TO_SUB_IF(wdev);
    rcu_read_lock();
    key = ieee80211_lookup_key(sdata, link_id, key_idx, pairwise, mac_addr);
    if (!key)
...
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_TX_RC_80_MHZ_WIDTH: rate->flags &) -> else {
    else if (rate.flags & IEEE80211_TX_RC_80_MHZ_WIDTH)
    rinfo.bw = RATE_INFO_BW_80;
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_TX_RC_160_MHZ_WIDTH: rate->flags &) -> else {
...
//
    ...
    struct ieee80211_local *local = wiphy_priv(wiphy);
    struct ieee80211_sub_if_data *sdata;
    let mut chanreq: ieee80211_chan_req = { .oper = *chandef };
    int ret;
    lockdep_assert_wiphy(local.hw.wiphy);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cca) -> else {
    else if (cca)
    ...
    u64 *changed)
    {
    struct fils_discovery_data *new, *old = core::ptr::null_mut();
    ...
    RCU_INIT_POINTER(link.u.ap.fils_discovery, core::ptr::null_mut());
    }
    if (old)
    kfree_rcu(old, rcu_head);
// changed |= BSS_CHANGED_FILS_DISCOVERY;
    ...
    u64 *changed)
    {
    struct unsol_bcast_probe_resp_data *new, *old = core::ptr::null_mut();
    if (!params.update)
    return 0;
    ...
    RCU_INIT_POINTER(link.u.ap.unsol_bcast_probe_resp, core::ptr::null_mut());
    }
    if (old)
    kfree_rcu(old, rcu_head);
...
//
    ...
    u64 *changed)
    {
    struct cfg80211_mbssid_elems *mbssid = core::ptr::null_mut();
    struct cfg80211_rnr_elems *rnr = core::ptr::null_mut();
    struct beacon_data *new, *old;
    int new_head_len, new_tail_len;
    int size, err;
    let mut _changed: u64 = BSS_CHANGED_BEACON;
...
#[no_mangle]
unsafe extern "C" fn ieee80211_num_beaconing_links(sdata: *mut ieee80211_sub_if_data) -> u8 {
...
//
    ...
    int i, err;
    int prev_beacon_int;
    let mut link_id: c_uint = params.beacon.link_id;
    struct ieee80211_link_data *link;
    struct ieee80211_bss_conf *link_conf;
    ...
    let mut npca_params: ieee80211_bss_npca_params = {};
...
//
    ...
    err = ieee80211_assign_beacon(sdata, link, &params.beacon, core::ptr::null_mut(), core::ptr::null_mut(),
    &changed);
    if (err < 0)
    goto error;
    ...
    if (old)
    kfree_rcu(old, rcu_head);
    RCU_INIT_POINTER(link.u.ap.beacon, core::ptr::null_mut());
...
//
    ...
    if (!old)
    return -ENOENT;
    err = ieee80211_assign_beacon(sdata, link, beacon, core::ptr::null_mut(), core::ptr::null_mut(),
    &changed);
    if (err < 0)
    return err;
    err = ieee80211_set_fils_discovery(sdata, &params.fils_discovery,
...
#[no_mangle]
unsafe extern "C" fn ieee80211_free_next_beacon(link: *mut ieee80211_link_data) {
    ...
    link.u.ap.next_beacon = core::ptr::null_mut();
    }
    ...
    struct ieee80211_bss_conf *link_conf = link.conf;
    let mut changes: u64 = BSS_CHANGED_BEACON_ENABLED;
    LIST_HEAD(keys);
    lockdep_assert_wiphy(local.hw.wiphy);
...
// remove beacon and probe response
    RCU_INIT_POINTER(link.u.ap.beacon, core::ptr::null_mut());
    RCU_INIT_POINTER(link.u.ap.probe_resp, core::ptr::null_mut());
    RCU_INIT_POINTER(link.u.ap.fils_discovery, core::ptr::null_mut());
    RCU_INIT_POINTER(link.u.ap.unsol_bcast_probe_resp, core::ptr::null_mut());
    RCU_INIT_POINTER(link.u.ap.s1g_short_beacon, core::ptr::null_mut());
    ...
    link_conf.ftmr_params = core::ptr::null_mut();
    link_conf.bssid_index = 0;
    link_conf.nontransmitted = false;
    link_conf.ema_ap = false;
    link_conf.bssid_indicator = 0;
    ...
    __sta_info_flush(sdata, true, link_id, core::ptr::null_mut());
    ieee80211_stop_mbssid(sdata);
    RCU_INIT_POINTER(link_conf.tx_bss_conf, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_sta_flag(sta, _arg: WLAN_STA_AUTHORIZED)) -> else {
...
// nothing
    ...
    {
    struct ieee80211_supported_band *sband = core::ptr::null_mut();
    struct ieee80211_sub_if_data *sdata = sta.sdata;
    let mut link_id: u32 = params.link_id < 0 ? 0 : params.link_id;
    struct ieee80211_link_data *link =
    sdata_dereference(sdata.link[link_id], sdata);
    struct link_sta_info *link_sta =
...
// VHT can override some HT caps such as the A-MSDU max length
    ...
    own_vht_cap,
    params.vht_capa, core::ptr::null_mut(),
    link_sta);
    if (params.he_capa)
    ...
    (void *)params.he_6ghz_capa : core::ptr::null_mut(),
    link_sta);
...
// not applicable - they don't use NSS/BW as capability
    ...
    let mut width: enum nl80211_chan_width = link.conf.chanreq.oper.width;
    switch (width) {
...
//
    ...
    let mut sta_links: u16 = sta.sta.valid_links;
    let mut new_links: u16 = master.vif.valid_links & sta_links;
    let mut orig_links: u16 = wdev.valid_links;
    ...
    RCU_INIT_POINTER(sta.sdata.u.vlan.sta, core::ptr::null_mut());
    if (test_sta_flag(sta, WLAN_STA_AUTHORIZED))
...
#[no_mangle]
pub unsafe extern "C" fn _chg_mesh_attr(parm: enum nl80211_meshconf_params, mask: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_leave_mesh(wiphy: *mut wiphy, dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_resume(wiphy: *mut wiphy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_abort_scan(wiphy: *mut wiphy, wdev: *mut wireless_dev) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_leave_ibss(wiphy: *mut wiphy, dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_leave_ocb(wiphy: *mut wiphy, dev: *mut net_device) -> c_int {
    ...
    struct ieee80211_local *local = wiphy_priv(wiphy);
    struct ieee80211_sub_if_data *sdata;
    let mut txp_type: enum nl80211_tx_power_setting = type;
    let mut update_txp_type: bool = false;
    let mut has_monitor: bool = false;
    int user_power_level;
    let mut old_power: c_int = local.user_power_level;
    lockdep_assert_wiphy(local.hw.wiphy);
    switch (type) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_rfkill_poll(wiphy: *mut wiphy) {
    ...
    {
    struct ieee80211_local *local = wiphy_priv(wiphy);
    struct ieee80211_vif *vif = core::ptr::null_mut();
    ...
    int err;
    struct sta_info *sta;
    let mut tdls_peer_found: bool = false;
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
...
#[no_mangle]
pub unsafe extern "C" fn if(tdls_peer_found: smps_mode != IEEE80211_SMPS_OFF &&) -> else {
...
//
    ...
    let mut basic_rates: u32 = sdata.vif.bss_conf.basic_rates;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_csa_finish(vif: *mut ieee80211_vif, link_id: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_channel_switch_disconnect(vif: *mut ieee80211_vif) {
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), changed);
...
#[no_mangle]
unsafe extern "C" fn __ieee80211_csa_finalize(link_data: *mut ieee80211_link_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_csa_finalize(link_data: *mut ieee80211_link_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_csa_finalize_work(wiphy: *mut wiphy, work: *mut wiphy_work) {
...
// AP might have been stopped while waiting for the lock.
    ...
    {
    struct ieee80211_sub_if_data *sdata = link_data.sdata;
    let mut csa: ieee80211_csa_settings = {};
    int err;
    switch (sdata.vif.type) {
    case NL80211_IFTYPE_AP:
    link_data.u.ap.next_beacon =
...
#[no_mangle]
unsafe extern "C" fn ieee80211_color_change_abort(link: *mut ieee80211_link_data) {
    ...
    struct ieee80211_link_data *link_data;
    let mut changed: u64 = 0;
    let mut link_id: u8 = params.link_id;
    int err;
    lockdep_assert_wiphy(local.hw.wiphy);
...
// if reservation is invalid then this will fail
    err = ieee80211_check_combinations(sdata, core::ptr::null_mut(), 0, 0, -1);
    if (err) {
...
// if the beacon didn't change, we can finalize immediately
    ...
    struct ieee80211_local *local = wiphy_priv(wiphy);
    struct ieee80211_sub_if_data *sdata = IEEE80211_WDEV_TO_SUB_IF(wdev);
    let mut preq_mask: u32 = BIT(IEEE80211_STYPE_PROBE_REQ >> 4);
    let mut action_mask: u32 = BIT(IEEE80211_STYPE_ACTION >> 4);
...
#[no_mangle]
unsafe extern "C" fn ieee80211_set_wakeup(wiphy: *mut wiphy, enabled: bool) {
...
// A NULL qos_map was passed to disable QoS mapping
    ...
    struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
    struct ieee80211_if_managed *ifmgd = &sdata.u.mgd;
    let mut ac: c_int = ieee802_1d_to_ac[up];
    if (sdata.vif.type != NL80211_IFTYPE_STATION)
    return -EOPNOTSUPP;
...
// phy stats
    ...
    return -EOPNOTSUPP;
    if (!tid_conf.peer)
    return drv_set_tid_config(sdata.local, sdata, core::ptr::null_mut(), tid_conf);
    ...
    return -EOPNOTSUPP;
    if (!peer)
    return drv_reset_tid_config(sdata.local, sdata, core::ptr::null_mut(), tids);
    sta = sta_info_get_bss(sdata, peer);
    if (!sta)
    return -ENOENT;
    ...
    {
    struct ieee80211_sub_if_data *sdata = link.sdata;
    let mut color_change: ieee80211_color_change_settings = {};
    int err;
    switch (sdata.vif.type) {
    case NL80211_IFTYPE_AP:
    link.u.ap.next_beacon =
    ...
    core::ptr::null_mut(), &color_change, changed);
    if (err < 0) {
    ieee80211_free_next_beacon(link);
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn ieee80211_color_change_finalize(link: *mut ieee80211_link_data) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_color_change_finish(vif: *mut ieee80211_vif, link_id: u8) {
...
// if the beacon didn't change, we can finalize immediately
    ...
    {
    struct ieee80211_sub_if_data *sdata = IEEE80211_WDEV_TO_SUB_IF(wdev);
    let mut new_links: u16 = wdev.valid_links & ~BIT(link_id);
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
