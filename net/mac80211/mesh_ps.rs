...
//
    ...
    struct sk_buff *skb;
    let mut size: c_int = sizeof(*nullfunc);
    __le16 fc;
...
//
#[no_mangle]
unsafe extern "C" fn mps_qos_null_tx(sta: *mut sta_info) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mps_local_status_update(sdata: *mut ieee80211_sub_if_data) -> u64 {
    u64 ieee80211_mps_local_status_update(struct ieee80211_sub_if_data *sdata)
    {
    struct ieee80211_if_mesh *ifmsh = &sdata.u.mesh;
    struct sta_info *sta;
    let mut peering: bool = false;
    let mut light_sleep_cnt: c_int = 0;
    let mut deep_sleep_cnt: c_int = 0;
    ...
    light_sleep_cnt++;
#[no_mangle]
pub unsafe extern "C" fn if(NL80211_MESH_POWER_DEEP_SLEEP: sta->mesh->local_pm ==) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mps_sta_status_update(sta: *mut sta_info) {
...
#[no_mangle]
unsafe extern "C" fn mpsp_trigger_send(sta: *mut sta_info, rspi: bool, eosp: bool) {
...
//
#[no_mangle]
unsafe extern "C" fn mps_frame_deliver(sta: *mut sta_info, n_frames: c_int) {
...
//
    ...
    bool tx, bool acked)
    {
    let mut rspi: u8 = qc[1] & (IEEE80211_QOS_CTL_RSPI >> 8);
    let mut eosp: u8 = qc[0] & IEEE80211_QOS_CTL_EOSP;
    if (tx) {
    if (rspi && acked)
    ...
    clear_sta_flag(sta, WLAN_STA_MPSP_RECIPIENT);
#[no_mangle]
pub unsafe extern "C" fn if(NL80211_MESH_POWER_ACTIVE: sta->mesh->local_pm !=) -> else {
...
//
    ...
    let mut has_buffered: bool = false;
    if (sta.mesh.plink_state == NL80211_PLINK_ESTAB)
