...
//
    ...
    let mut tid: c_int = *p & IEEE80211_QOS_CTL_TID_MASK;
//
...
#[no_mangle]
unsafe extern "C" fn ieee80211_check_pending_bar(sta: *mut sta_info, addr: *mut u8, tid: u8) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_frame_acked(sta: *mut sta_info, skb: *mut sk_buff) {
    ...
    let mut tid: u16 = qc[0] & 0xf;
    ieee80211_check_pending_bar(sta, hdr.addr1, tid);
    }
    }
#[no_mangle]
unsafe extern "C" fn ieee80211_set_bar_pending(sta: *mut sta_info, tid: u8, ssn: u16) {
    ...
    {
    struct ieee80211_rate_status *status_rate = core::ptr::null_mut();
    let mut len: c_int = sizeof(struct ieee80211_radiotap_header);
...
// IEEE80211_RADIOTAP_VHT
    ...
    len += 3;
#[no_mangle]
pub unsafe extern "C" fn if(RATE_INFO_FLAGS_VHT_MCS: status_rate->rate_idx.flags &) -> else {
    else if (status_rate.rate_idx.flags & RATE_INFO_FLAGS_VHT_MCS)
    len = ALIGN(len, 2) + 12;
#[no_mangle]
pub unsafe extern "C" fn if(RATE_INFO_FLAGS_HE_MCS: status_rate->rate_idx.flags &) -> else {
    ...
    len += 3;
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_TX_RC_VHT_MCS: info->status.rates[0].flags &) -> else {
...
// pos = 1;
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_TX_RC_80_MHZ_WIDTH: info->status.rates[0].flags &) -> else {
    else if (info.status.rates[0].flags & IEEE80211_TX_RC_80_MHZ_WIDTH)
// pos = 4;
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_TX_RC_160_MHZ_WIDTH: info->status.rates[0].flags &) -> else {
...
//
    ...
    let mut is_teardown: bool = false;
// Get the teardown data we need and free the lock
    ...
    sdata.u.mgd.teardown_skb = core::ptr::null_mut();
    sdata.u.mgd.orig_teardown_skb = core::ptr::null_mut();
    is_teardown = true;
    }
...
// Check if peer has ACKed
    ...
    let mut cookie: u64 = IEEE80211_SKB_CB(skb).ack.cookie;
    struct ieee80211_sub_if_data *sdata;
    struct ieee80211_hdr *hdr = (void *)skb.data;
    ...
    GFP_ATOMIC);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ieee80211_is_any_nullfunc(hdr->frame_control)) -> else {
...
// consumes skb
    ...
    {
    let mut sub_data: u16 = u16_get_bits(status_data, IEEE80211_STATUS_SUBDATA_MASK);
    let mut smps_mode: enum ieee80211_smps_mode = sub_data & 3;
    let mut link_id: c_int = (sub_data >> 2);
...
//
    ...
    {
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    let mut tx_time_est: u16 = ieee80211_info_get_tx_time_est(info);
    struct ieee80211_hdr *hdr = (void *)skb.data;
    let mut acked: bool = info.flags & IEEE80211_TX_STAT_ACK;
    if (dropped)
    acked = false;
    if (tx_time_est) {
    struct sta_info *sta;
...
//
pub const STA_LOST_PKT_THRESHOLD: c_int = 50;

    static void ieee80211_lost_packet(struct sta_info *sta,
    struct ieee80211_tx_info *info)
    {
    let mut pkt_time: c_ulong = STA_LOST_PKT_TIME;
    let mut pkt_thr: c_uint = STA_LOST_PKT_THRESHOLD;
...
// retry_count = count;
    ...
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct ieee80211_sub_if_data *sdata;
    struct net_device *prev_dev = core::ptr::null_mut();
    int rtap_len;
...
// XXX: is this sufficient for BPF?
    ...
    let mut tid: c_int = IEEE80211_NUM_TIDS;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: status->free_list) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_tx_status_skb(hw: *mut ieee80211_hw, skb: *mut sk_buff) {
...
//
    let mut mcast: bool = IEEE80211_SKB_CB(skb).tx_time_mc;
    ieee80211_sta_update_pending_airtime(local, sta,
    skb_get_queue_mapping(skb),
    tx_time_est,
    true, mcast);
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_report_low_ack(pubsta: *mut ieee80211_sta, num_packets: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_free_txskb(hw: *mut ieee80211_hw, skb: *mut sk_buff) {
    void ieee80211_free_txskb(struct ieee80211_hw *hw, struct sk_buff *skb)
    {
    struct ieee80211_local *local = hw_to_local(hw);
    let mut kt: ktime_t = ktime_set(0, 0);
