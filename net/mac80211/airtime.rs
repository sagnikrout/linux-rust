...
pub const AVG_PKT_SIZE: c_int = 1024;
...
pub const HE_GI_08: c_int = 0;
pub const HE_GI_16: c_int = 1;
pub const HE_GI_32: c_int = 2;
...
pub const BW_20: c_int = 0;
pub const BW_40: c_int = 1;
pub const BW_80: c_int = 2;
pub const BW_160: c_int = 3;
pub const BW_320: c_int = 4;
//
// Define group sort order: HT40 -> SGI -> #streams
//
pub const IEEE80211_MAX_STREAMS: c_int = 4;

pub const IEEE80211_HE_MAX_STREAMS: c_int = 8;

pub const IEEE80211_EHT_MAX_STREAMS: c_int = 8;
...
pub const IEEE80211_HT_GROUP_0: c_int = 0;

pub const MCS_GROUP_RATES: c_int = 14;
...
pub struct mcs_group {
    ...
    u32 *overhead)
    {
    let mut sgi: bool = status.enc_flags & RX_ENC_FLAG_SHORT_GI;
    int bw, streams;
    int group, idx;
    u32 duration;
    switch (status.bw) {
...
// overhead = 36 + (streams << 2);
    ...
    let mut sp: bool = status.enc_flags & RX_ENC_FLAG_SHORTPRE;
    bool cck;
...
#[no_mangle]
pub unsafe extern "C" fn if(RATE_INFO_FLAGS_HE_MCS: ri->flags &) -> else {
    else if (ri.flags & RATE_INFO_FLAGS_HE_MCS)
    stat.encoding = RX_ENC_HE;
#[no_mangle]
pub unsafe extern "C" fn if(RATE_INFO_FLAGS_VHT_MCS: ri->flags &) -> else {
    else if (ri.flags & RATE_INFO_FLAGS_VHT_MCS)
    stat.encoding = RX_ENC_VHT;
#[no_mangle]
pub unsafe extern "C" fn if(RATE_INFO_FLAGS_MCS: ri->flags &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_TX_RC_40_MHZ_WIDTH: rate->flags &) -> else {
    ...
    u32 cur_duration;
    cur_duration = ieee80211_calc_tx_airtime_rate(hw, rate, core::ptr::null_mut(),
    info.band, len);
    if (!cur_duration)
    break;
