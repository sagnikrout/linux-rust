...
//! Source: net/mac80211/spectmgmt.c
#![no_std]
...
//
    ...
    {
    let mut ccfs0: u8 = wbcs_elem.new_center_freq_seg0;
    let mut ccfs1: u8 = wbcs_elem.new_center_freq_seg1;
    let mut cf0: u32 = ieee80211_channel_to_frequency(ccfs0, chandef.chan.band);
    let mut cf1: u32 = ieee80211_channel_to_frequency(ccfs1, chandef.chan.band);
...
// deprecated encoding
    ...
    chandef.center_freq1 = cf0;
    if (ccfs1) {
    let mut diff: u8 = abs(ccfs0 - ccfs1);
    if (diff == 8) {
    chandef.width = NL80211_CHAN_WIDTH_160;
...
//
    ...
    {
    let mut new_band: enum nl80211_band = current_band;
    int new_freq;
    let mut new_chan_no: c_int = -1, new_op_class = -1;
    struct ieee80211_channel *new_chan;
    let mut new_chandef: cfg80211_chan_def = {};
    const struct ieee80211_sec_chan_offs_ie *sec_chan_offs;
    const struct ieee80211_wide_bw_chansw_ie *wide_bw_chansw_ie;
    const struct ieee80211_bandwidth_indication *bwi;
    const struct ieee80211_ext_chansw_ie *ext_chansw_elem;
    let mut secondary_channel_offset: c_int = -1;
    ...
    if (conn.mode < IEEE80211_CONN_MODE_HT ||
    conn.bw_limit < IEEE80211_CONN_BW_LIMIT_40) {
    sec_chan_offs = core::ptr::null_mut();
    wide_bw_chansw_ie = core::ptr::null_mut();
    }
    if (conn.mode < IEEE80211_CONN_MODE_VHT)
    wide_bw_chansw_ie = core::ptr::null_mut();
    if (ext_chansw_elem) {
...
// capture the AP chandef before (potential) downgrading
    ...
    ieee80211_chandef_downgrade(&new_chandef, core::ptr::null_mut());
