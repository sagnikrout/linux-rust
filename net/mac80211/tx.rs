...
#[no_mangle]
unsafe extern "C" fn purge_old_ps_buffers(local: *mut ieee80211_local) {
    static void purge_old_ps_buffers(struct ieee80211_local *local)
    {
    let mut total: c_int = 0, purged = 0;
    struct sk_buff *skb;
    ...
    ps = &sdata.u.ap.ps;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ieee80211_vif_is_mesh(&sdata->vif)) -> else {
...
// buffered in mac80211
    ...
    return 0;
    if (sta == core::ptr::null_mut() || !test_sta_flag(sta, WLAN_STA_MFP))
    return 0;
    ...
    let mut ac: c_int = skb_get_queue_mapping(tx.skb);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_multicast_ether_addr(hdr->addr1)) -> else {
    ...
    tx.key = key;
#[no_mangle]
pub unsafe extern "C" fn if(ieee80211_select_link_key(tx)): (key =) -> else {
...
// TODO: add threshold stuff again
    ...
    struct ieee80211_sta_rates *ratetbl = core::ptr::null_mut();
    let mut encap: bool = info.flags & IEEE80211_TX_CTL_HW_80211_ENCAP;
    let mut assoc: bool = false;
...
#[no_mangle]
unsafe extern "C" fn ieee80211_tx_next_seq(sta: *mut sta_info, tid: c_int) -> __le16 {
    static __le16 ieee80211_tx_next_seq(struct sta_info *sta, int tid)
    {
    u16 *seq = &sta.tid_seq[tid];
    let mut ret: __le16 = cpu_to_le16(*seq);
...
// include per-STA, per-TID sequence counter
    ...
    struct sk_buff *tmp;
    let mut per_fragm: c_int = frag_threshold - hdrlen - FCS_LEN;
    let mut pos: c_int = hdrlen + per_fragm;
    let mut rem: c_int = skb.len - hdrlen - per_fragm;
    if (WARN_ON(rem < 0))
    return -EINVAL;
// first fragment was already added to queue by caller
    while (rem) {
    let mut fraglen: c_int = per_fragm;
    if (fraglen > rem)
    fraglen = rem;
    rem -= fraglen;
...
// adjust first fragment's length
    ...
    let mut frag_threshold: c_int = tx.local.hw.wiphy.frag_threshold;
    int hdrlen;
    int fragnum;
// no matter what happens, tx->skb moves to tx->skbs
    __skb_queue_tail(&tx.skbs, skb);
    tx.skb = core::ptr::null_mut();
...
// update duration/seq/flags of fragments
    ...
    let mut morefrags: __le16 = cpu_to_le16(IEEE80211_FCTL_MOREFRAGS);
    hdr = (void *)skb.data;
...
// actual transmit path
    ...
    let mut reset_agg_timer: bool = false;
    struct sk_buff *purge_skb = core::ptr::null_mut();
...
//
    ...
    info.flags |= IEEE80211_TX_CTL_CLEAR_PS_FILT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_and_clear_sta_flag(tx->sta, _arg: WLAN_STA_CLEAR_PS_FILT)) -> else {
    info.flags |= IEEE80211_TX_CTL_CLEAR_PS_FILT;
    ...
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct ieee80211_txq *txq = core::ptr::null_mut();
...
//
    ...
    let mut tid: u8 = skb.priority & IEEE80211_QOS_CTL_TID_MASK;
    if (!sta.uploaded)
    return core::ptr::null_mut();
    txq = sta.sta.txq[tid];
    } else {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_set_skb_enqueue_time(skb: *mut sk_buff) {
    static void ieee80211_set_skb_enqueue_time(struct sk_buff *skb)
    {
    struct sk_buff *next;
    let mut now: codel_time_t = codel_get_time();
    skb_list_walk_safe(skb, skb, next)
    IEEE80211_SKB_CB(skb).control.enqueue_time = now;
    }
#[no_mangle]
unsafe extern "C" fn codel_skb_len_func(skb: *const sk_buff) -> u32 {
    static u32 codel_skb_len_func(const struct sk_buff *skb)
    {
    return skb.len;
    }
#[no_mangle]
unsafe extern "C" fn codel_skb_time_func(skb: *const sk_buff) -> codel_time_t {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_txq_set_params(local: *mut ieee80211_local, radio_idx: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_txq_setup_flows(local: *mut ieee80211_local) -> c_int {
    int ieee80211_txq_setup_flows(struct ieee80211_local *local)
    {
    struct fq *fq = &local.fq;
    int ret;
    int i;
    let mut supp_vht: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_txq_teardown_flows(local: *mut ieee80211_local) {
    void ieee80211_txq_teardown_flows(struct ieee80211_local *local)
    {
    struct fq *fq = &local.fq;
    kvfree(local.cvars);
    local.cvars = core::ptr::null_mut();
    spin_lock_bh(&fq.lock);
    fq_reset(fq, fq_skb_free_func);
    spin_unlock_bh(&fq.lock);
    }
    ...
    bool txpending)
    {
    let mut control: ieee80211_tx_control = {};
    struct sk_buff *skb, *tmp;
    unsigned long flags;
    skb_queue_walk_safe(skbs, skb, tmp) {
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    let mut q: c_int = info.hw_queue;
...
//
    ...
    control.sta = sta ? &sta.sta : core::ptr::null_mut();
    __skb_unlink(skb, skbs);
...
//
#[no_mangle]
unsafe extern "C" fn invoke_tx_handlers_early(tx: *mut ieee80211_tx_data) -> c_int {
    static int invoke_tx_handlers_early(struct ieee80211_tx_data *tx)
    {
    let mut res: ieee80211_tx_result = TX_DROP;
...
//
#[no_mangle]
unsafe extern "C" fn invoke_tx_handlers_late(tx: *mut ieee80211_tx_data) -> c_int {
    static int invoke_tx_handlers_late(struct ieee80211_tx_data *tx)
    {
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(tx.skb);
    let mut res: ieee80211_tx_result = TX_CONTINUE;
...
#[no_mangle]
unsafe extern "C" fn invoke_tx_handlers(tx: *mut ieee80211_tx_data) -> c_int {
    static int invoke_tx_handlers(struct ieee80211_tx_data *tx)
    {
    let mut r: c_int = invoke_tx_handlers_early(tx);
    if (r)
    return r;
    ...
    struct ieee80211_tx_data tx;
    struct sk_buff *skb2;
    if (ieee80211_tx_prepare(sdata, &tx, core::ptr::null_mut(), skb) == TX_DROP) {
    kfree_skb(skb);
    return false;
    }
    info.band = band;
...
#[no_mangle]
pub unsafe extern "C" fn if(tail_need: head_need ||) -> else {
...
// reload after potential resize
    ...
    ieee80211_mps_set_frame_flags(sdata, core::ptr::null_mut(), hdr);
    }
    }
...
#[no_mangle]
unsafe extern "C" fn ieee80211_validate_radiotap_len(skb: *mut sk_buff) -> bool {
...
// does the skb contain enough to deliver on the alleged length?
    ...
    u16 txflags;
    let mut rate: u16 = 0;
    let mut rate_found: bool = false;
    let mut rate_retries: u8 = 0;
    let mut rate_flags: u16 = 0;
    u8 mcs_known, mcs_flags, mcs_bw;
    u16 vht_known;
    let mut vht_mcs: u8 = 0, vht_nss = 0;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(4: iterator.this_arg[3] ==) -> else {
    else if (iterator.this_arg[3] == 4)
    rate_flags |=
    IEEE80211_TX_RC_80_MHZ_WIDTH;
#[no_mangle]
pub unsafe extern "C" fn if(11: iterator.this_arg[3] ==) -> else {
...
// remove the injection radiotap header
    ...
    ieee80211_xmit(sdata, core::ptr::null_mut(), skb);
    rcu_read_unlock();
    return NETDEV_TX_OK;
    fail_rcu:
    rcu_read_unlock();
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_is_tdls_setup(skb: *mut sk_buff) -> bool {
    static inline bool ieee80211_is_tdls_setup(struct sk_buff *skb)
    {
    let mut ethertype: u16 = (skb.data[12] << 8) | skb.data[13];
...
#[no_mangle]
unsafe extern "C" fn ieee80211_remove_ack_skb(local: *mut ieee80211_local, info_id: u16) {
...
//
    ...
    struct mesh_path __maybe_unused *mppath = core::ptr::null_mut(), *mpath = core::ptr::null_mut();
    const u8 *encaps_data;
    int encaps_len, skip_header_bytes;
    let mut wme_sta: bool = false, authorized = false;
    bool tdls_peer;
    bool multicast;
    let mut info_id: u16 = 0;
    struct ieee80211_chanctx_conf *chanctx_conf = core::ptr::null_mut();
    enum nl80211_band band;
    int ret;
    let mut link_id: u8 = u32_get_bits(ctrl_flags, IEEE80211_TX_CTRL_MLO_LINK);
    if (IS_ERR(sta))
    sta = core::ptr::null_mut();
...
// leave the original DA. This also works for mcast
    ...
    mesh_da = mppath.mpp;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mpath) -> else {
    else if (mpath)
    mesh_da = mpath.dst;
    hdrlen = ieee80211_fill_mesh_addresses(&hdr, &fc,
...
// DA TA mSA AE:SA
    ...
    sdata, &mesh_hdr,
    skb.data + ETH_ALEN, core::ptr::null_mut());
    else
...
// For TDLS only one link can be valid with peer STA
    let mut tdls_link_id: c_int = ieee80211_tdls_sta_link_id(sta);
    struct ieee80211_link_data *link;
// DA SA BSSID
    memcpy(hdr.addr1, skb.data, ETH_ALEN);
...
//
    ...
    (cpu_to_be16(ethertype) != sdata.control_port_protocol ||
    !ieee80211_is_our_addr(sdata, skb.data + ETH_ALEN, core::ptr::null_mut())))) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ieee80211_check_fast_xmit(sta: *mut sta_info) {
    void ieee80211_check_fast_xmit(struct sta_info *sta)
    {
    let mut build: ieee80211_fast_tx = {}, *fast_tx = core::ptr::null_mut(), *old;
...
// DA BSSID SA
    ...
    let mut link_id: c_uint = sta.deflink.link_id;
    struct ieee80211_link_data *link;
    rcu_read_lock();
    link = rcu_dereference(sdata.link[link_id]);
    if (WARN_ON(!link)) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_check_fast_xmit_all(local: *mut ieee80211_local) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_check_fast_xmit_iface(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_clear_fast_xmit(sta: *mut sta_info) {
    ...
    lockdep_is_held(&sta.lock));
    RCU_INIT_POINTER(sta.fast_tx, core::ptr::null_mut());
    spin_unlock_bh(&sta.lock);
    if (fast_tx)
    ...
    let mut hdr_len: c_int = fast_tx.hdr_len - sizeof(rfc1042_header);
    let mut subframe_len: c_int = skb.len - hdr_len;
    void *data;
...
// qc |= IEEE80211_QOS_CTL_A_MSDU_PRESENT;
    ...
    struct fq_tin *tin;
    struct fq_flow *flow;
    let mut tid: u8 = skb.priority & IEEE80211_QOS_CTL_TAG1D_MASK;
    struct ieee80211_txq *txq = sta.sta.txq[tid];
    struct txq_info *txqi;
    struct sk_buff **frag_tail, *head;
    let mut subframe_len: c_int = skb.len - ETH_ALEN;
    let mut max_subframes: u8 = sta.sta.max_amsdu_subframes;
    let mut max_frags: c_int = local.hw.max_tx_fragments;
    let mut max_amsdu_len: c_int = sta.sta.cur.max_amsdu_len;
    int orig_truesize;
    u32 flow_idx;
    __be16 len;
    void *data;
    ...
    let mut n: c_int = 2, nfrags, pad = 0;
    u16 hdrlen;
...
//
    ...
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct ieee80211_hdr *hdr = (void *)skb.data;
    let mut tid: u8 = IEEE80211_NUM_TIDS;
    if (!ieee80211_hw_check(&tx.local.hw, HAS_RATE_CONTROL) &&
...
//
    ...
    let mut ofs: c_int = skb_checksum_start_offset(skb);
    ...
    let mut hw_headroom: c_int = sdata.local.hw.extra_tx_headroom;
    let mut extra_head: c_int = fast_tx.hdr_len - (ETH_HLEN - 2);
    skb = skb_share_check(skb, GFP_ATOMIC);
    if (unlikely(!skb))
    return;
    if ((hdr.frame_control & cpu_to_le16(IEEE80211_STYPE_QOS_DATA)) &&
...
// ieee80211_get_qos_ctl(hdr) = tid;
    ...
    let mut ethertype: u16 = (skb.data[12] << 8) | skb.data[13];
    struct ieee80211_hdr *hdr = (void *)fast_tx.hdr;
    struct tid_ampdu_tx *tid_tx = core::ptr::null_mut();
    struct sk_buff *next;
    struct ethhdr eth;
    let mut tid: u8 = IEEE80211_NUM_TIDS;
...
// after this point (skb is modified) we cannot return false
    ...
    let mut q: c_int = vif.hw_queue[txq.ac];
    unsigned long flags;
    bool q_stopped;
...
//
    info.control.hw_key = core::ptr::null_mut();
    ...
    info.control.vif = core::ptr::null_mut();
    return skb;
    }
    break;
    case NL80211_IFTYPE_AP_VLAN:
    ...
    let mut ampdu: bool = txq.sta && txq.ac != IEEE80211_AC_VO;
    u32 airtime;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_sta_deficit(sta: *mut sta_info, ac: u8) -> i32 {
    ...
    {
    struct ieee80211_local *local = hw_to_local(hw);
    struct ieee80211_txq *ret = core::ptr::null_mut();
    struct txq_info *txqi = core::ptr::null_mut(), *head = core::ptr::null_mut();
    let mut found_eligible_txq: bool = false;
    bool aql_check;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_txq_schedule_start(hw: *mut ieee80211_hw, ac: u8) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_change_da(skb: *mut sk_buff, sta: *mut sta_info) -> c_int {
...
// multicast to unicast conversion only for some payload
    ...
    struct sta_info *sta, *first = core::ptr::null_mut();
...
// no STA connected, drop
    ...
    {
    struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
    let mut links: c_ulong = sdata.vif.active_links;
    unsigned int link;
    let mut ctrl_flags: u32 = IEEE80211_TX_CTRL_MCAST_MLO_FIRST_TX;
...
//
    ...
    let mut control: ieee80211_tx_control = {};
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct ieee80211_sta *pubsta = core::ptr::null_mut();
    unsigned long flags;
    let mut q: c_int = info.hw_queue;
    spin_lock_irqsave(&local.queue_stop_reason_lock, flags);
    if (local.queue_stop_reasons[q] ||
...
//
#[no_mangle]
pub unsafe extern "C" fn ieee80211_clear_tx_pending(local: *mut ieee80211_local) {
    void ieee80211_clear_tx_pending(struct ieee80211_local *local)
    {
    struct sk_buff *skb;
    int i;
    for (i = 0; i < local.hw.queues; i++) {
    while ((skb = skb_dequeue(&local.pending[i])) != core::ptr::null_mut())
    ieee80211_free_txskb(&local.hw, skb);
    }
    }
//
...
// update band only for non-MLD
    ...
    result = ieee80211_tx(sdata, core::ptr::null_mut(), skb, true);
...
//
#[no_mangle]
pub unsafe extern "C" fn ieee80211_tx_pending(t: *mut tasklet_struct) {
...
// Emit an encoded block for each non-zero sub-block
    ...
    let mut blk_bmap: u8 = 0;
    int sblk;
    for (sblk = 0; sblk < 8; sblk++) {
    let mut sblk_idx: c_int = blk * 8 + sblk;
//
...
// Emit the present subblocks
    ...
    {
    struct element *tim;
    let mut mcast_traffic: bool = false, have_bits = false;
    struct ieee80211_bss_conf *link_conf = link.conf;
    let mut s1g: bool = ieee80211_get_link_sband(link).band == NL80211_BAND_S1GHZ;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_beacon_update_cntdwn(vif: *mut ieee80211_vif, link_id: c_uint) -> u8 {
    u8 ieee80211_beacon_update_cntdwn(struct ieee80211_vif *vif, unsigned int link_id)
    {
    struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);
    struct ieee80211_link_data *link;
    struct beacon_data *beacon = core::ptr::null_mut();
    let mut count: u8 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(NL80211_IFTYPE_ADHOC: sdata->vif.type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_beacon_set_cntdwn(vif: *mut ieee80211_vif, counter: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_encrypt_tx_skb(skb: *mut sk_buff) -> c_int {
    int ieee80211_encrypt_tx_skb(struct sk_buff *skb)
    {
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct ieee80211_sub_if_data *sdata = core::ptr::null_mut();
    struct sk_buff *check_skb;
    struct ieee80211_tx_data tx;
    ieee80211_tx_result res;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: info->control.vif) -> else {
...
// CSA offsets
    ...
    let mut csa_off: u16 = beacon.cntdwn_counter_offsets[i];
    if (!csa_off)
    continue;
    ...
    txrc.bss = true;
    rate_control_get_rate(sdata, core::ptr::null_mut(), &txrc);
    info.control.vif = vif;
...
// for AP the csa offsets are from tail
    ...
    ieee80211_beacon_get_finish(hw, vif, link, core::ptr::null_mut(), core::ptr::null_mut(), skb,
    chanctx_conf, 0);
    return skb;
    }
    static struct sk_buff *
    ...
    struct ieee80211_chanctx_conf *chanctx_conf)
    {
    struct ieee80211_ema_beacons *ema = core::ptr::null_mut();
    if (!beacon.mbssid_ies || !beacon.mbssid_ies.cnt)
    return core::ptr::null_mut();
    ...
    is_template, beacon,
    chanctx_conf, ema.cnt, core::ptr::null_mut());
    ...
    struct ieee80211_chanctx_conf *chanctx_conf;
    struct ieee80211_link_data *link;
    struct s1g_short_beacon_data *s1g_short_bcn = core::ptr::null_mut();
    rcu_read_lock();
...
//
    ...
    IEEE80211_INCLUDE_ALL_MBSSID_ELEMS, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_beacon_free_ema_list(ema_beacons: *mut ieee80211_ema_beacons) {
    ...
    unsigned int link_id)
    {
    struct ieee80211_ema_beacons *ema_beacons = core::ptr::null_mut();
    WARN_ON(__ieee80211_beacon_get(hw, vif, core::ptr::null_mut(), true, link_id, 0,
    &ema_beacons));
    return ema_beacons;
    }
    ...
    unsigned int link_id)
    {
    let mut offs: ieee80211_mutable_offsets = {};
...
// send a copy to monitor interfaces
    ...
    if (!copy)
    return bcn;
    ieee80211_tx_monitor(hw_to_local(hw), copy, 1, core::ptr::null_mut());
    return bcn;
    }
    ...
    {
    struct sk_buff *skb = core::ptr::null_mut();
    struct probe_resp *presp = core::ptr::null_mut();
    struct ieee80211_hdr *hdr;
    struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);
    if (sdata.vif.type != NL80211_IFTYPE_AP)
    return core::ptr::null_mut();
    ...
    {
    struct sk_buff *skb = core::ptr::null_mut();
    struct fils_discovery_data *tmpl = core::ptr::null_mut();
    struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);
    struct ieee80211_link_data *link;
    if (sdata.vif.type != NL80211_IFTYPE_AP)
    return core::ptr::null_mut();
    ...
    {
    struct sk_buff *skb = core::ptr::null_mut();
    struct unsol_bcast_probe_resp_data *tmpl = core::ptr::null_mut();
    struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);
    struct ieee80211_link_data *link;
    if (sdata.vif.type != NL80211_IFTYPE_AP)
    return core::ptr::null_mut();
...
// aid in PS-Poll has its two MSBs each set to 1
    ...
    struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_link_data *link = core::ptr::null_mut();
    ...
    IEEE80211_FCTL_TODS);
    if (qos) {
    let mut qoshdr: __le16 = cpu_to_le16(7);
...
// STAs
    ...
    if (sdata.vif.type == NL80211_IFTYPE_AP)
    sdata = IEEE80211_DEV_TO_SUB_IF(skb.dev);
    if (!ieee80211_tx_prepare(sdata, &tx, core::ptr::null_mut(), skb))
    break;
    ieee80211_free_txskb(hw, skb);
    }
    info = IEEE80211_SKB_CB(skb);
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_reserve_tid(pubsta: *mut ieee80211_sta, tid: u8) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_unreserve_tid(pubsta: *mut ieee80211_sta, tid: u8) {
...
// only some cases are supported right now
    ...
    let mut ac: c_int = ieee80211_ac_from_tid(tid);
    unsigned int link;
    skb_reset_mac_header(skb);
...
//
    ...
    if (!IS_ERR(sta)) {
    let mut queue: u16 = ieee80211_select_queue(sdata, sta, skb);
    skb_set_queue_mapping(skb, queue);
//
