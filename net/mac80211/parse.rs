...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ieee80211_elem_defrag {
    pub elem: *const element,
// container start/len
    pub start: *const u8,
    pub len: usize,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ieee80211_elems_parse {
// must be first for kfree to work
    pub elems: ieee802_11_elems,
    pub ml_basic: ieee80211_elem_defrag ml_reconf, ml_epcs,,
    pub inside_multilink: bool,
...
//
    ...
    pub scratch_pos: *mut u8,
    pub __counted_by(scratch_len): u8 scratch[],
}
...
// crc = crc32_be(*crc, (void *)elem, elem->datalen + 2);
    ...
    u8 *out, N, *cnt_out = core::ptr::null_mut(), *N_out = core::ptr::null_mut();
...
// N_out = N;
    ...
    struct ieee802_11_elems *elems = &elems_parse.elems;
    const struct element *elem;
    let mut calc_crc: bool = params.filter != 0;
    DECLARE_BITMAP(seen_elems, 256);
    let mut crc: u32 = params.crc;
...
// we don't need to parse assoc request, luckily (it's value 0)
    ...
    let mut id: u8 = elem.id;
    let mut elen: u8 = elem.datalen;
...
//
    ...
    let mut edatalen: u8 = subelem.datalen - 1;
...
// Invalid MBSSID Index element
    ...
    let mut ml_len: isize = elems.ml_basic_len;
...
// the sub element can be fragmented
    ...
    const struct element *ml_basic_elem = core::ptr::null_mut();
...
// out_len = ml_len;
    ...
    {
    let mut sub: ieee80211_elems_parse_params = {};
    struct ieee80211_elems_parse *elems_parse;
    const struct element *non_inherit = core::ptr::null_mut();
    struct ieee802_11_elems *elems;
    let mut scratch_len: usize = 3 * params.len;
    let mut inside_multilink: bool = false;
...
// Override with nontransmitted/per-STA profile if found
    ...
    _ieee802_11_parse_elems_full(&sub, elems_parse, core::ptr::null_mut());
    }
