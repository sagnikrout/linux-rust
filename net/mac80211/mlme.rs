...
pub const IEEE80211_AUTH_MAX_TRIES: c_int = 3;

pub const IEEE80211_ASSOC_MAX_TRIES: c_int = 3;

pub const IEEE80211_ADV_TTLM_ST_UNDERFLOW: c_uint = 0xff00;

    let mut max_nullfunc_tries: static int = 2;
    module_param(max_nullfunc_tries, int, 0644);
    MODULE_PARM_DESC(max_nullfunc_tries,
    "Maximum nullfunc tx tries before disconnecting (reason 4).");
    let mut max_probe_tries: static int = 5;
...
//
    let mut beacon_loss_count: static int = 7;
...
//
    let mut probe_wait_ms: static int = 500;
...
//
pub const IEEE80211_SIGNAL_AVE_MIN_COUNT: c_int = 4;
//
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_sta_reset_beacon_monitor(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_sta_reset_conn_monitor(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
unsafe extern "C" fn ecw2cw(ecw: c_int) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ieee80211_determine_ap_chan_data {
// input data
    pub channel: *mut ieee80211_channel,
    pub elems: *const ieee802_11_elems,
    pub conn: *const ieee80211_conn_settings,
    pub vht_cap_info: u32,
    pub ignore_ht_channel_mismatch: bool,
    pub cur_chandef: *const cfg80211_chan_def,
    pub cur_dbe_used: bool,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ieee80211_determine_ap_chan_output {
// filled to indicate UHR DBE was used
    pub dbe_used: bool,
// and need to know non-DBE width
    pub non_dbe_width: enum nl80211_chan_width,
}
    ...
    {
    let mut ignore_ht_channel_mismatch: bool = data.ignore_ht_channel_mismatch;
...
// get special 6 GHz case out of the way
    ...
    let mut mode: enum ieee80211_conn_mode = IEEE80211_CONN_MODE_HIGHEST;
// this is an error
    ...
    let mut he_chandef: cfg80211_chan_def = *chandef;
    if (!ieee80211_chandef_he_6ghz_oper(sdata.local,
    he_oper, core::ptr::null_mut(),
    &he_chandef)) {
    sdata_info(sdata,
...
//
    ...
    let mut eht_chandef: cfg80211_chan_def = *chandef;
...
// chandef = eht_chandef;
    ...
    let mut npca_chandef: cfg80211_chan_def = *chandef;
...
//
    for (nss = 8; nss > 0; nss--) {
    let mut ap_op_val: u8 = (ap_min_req_set >> (2 * (nss - 1))) & 3;
    u8 sta_rx_val;
    u8 sta_tx_val;
...
//
    for (nss = 8; nss > 0; nss--) {
    let mut sta_rx_val: u8 = (sta_mcs_map_rx >> (2 * (nss - 1))) & 3;
    let mut sta_tx_val: u8 = (sta_mcs_map_tx >> (2 * (nss - 1))) & 3;
    let mut ap_val: u8 = (ap_min_req_set >> (2 * (nss - 1))) & 3;
...
// If here, STA doesn't meet AP's HE min requirements
    ...
    {
    let mut he_phy_cap0: u8 = sta_he_cap.he_cap_elem.phy_cap_info[0];
    let mut eht_phy_cap0: u8 = sta_eht_cap.eht_cap_elem.phy_cap_info[0];
...
// the others have MCS 0-9 together, rather than separately from 0-7
    ...
    let mut rate: c_int = supp_rate & 0x7f;
    let mut is_basic: bool = !!(supp_rate & 0x80);
...
#[no_mangle]
unsafe extern "C" fn ieee80211_chandef_num_subchans(c: *const cfg80211_chan_def) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_chandef_num_widths(c: *const cfg80211_chan_def) -> c_int {
    ...
    {
    let mut n: c_int = ieee80211_chandef_num_subchans(ap);
    let mut tmp: cfg80211_chan_def = *ap;
...
// don't WARN - misconfigured APs could cause this if their N > width
    ...
    let mut prev: u32 = tmp.center_freq1;
    ieee80211_chandef_downgrade(&tmp, core::ptr::null_mut());
//
...
//
    ...
    {
    let mut needed: u8 = ieee80211_chandef_num_subchans(used);
    let mut have: u8 = ieee80211_chandef_num_subchans(ap);
...
#[no_mangle]
pub unsafe extern "C" fn if(psd->n: i < offset +) -> else {
...
//
    ...
    {
    chanreq.ap.chan = core::ptr::null_mut();
    if (conn.mode < IEEE80211_CONN_MODE_EHT)
    return;
...
// this should be impossible since parsing depends on our mode
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), unknown_rates_selectors, core::ptr::null_mut(), core::ptr::null_mut(),
    core::ptr::null_mut());
    switch (channel.band) {
...
//
    ...
    let mut chanreq: ieee80211_chan_req = {};
    enum ieee80211_conn_mode ap_mode;
    const char *frame;
    u16 ht_opmode;
    int ret;
...
//
    ...
    ieee80211_chandef_downgrade(&chanreq.oper, core::ptr::null_mut());
...
//
    ...
    const struct ieee80211_uhr_npca_info *npca;
    let mut params: ieee80211_bss_npca_params = {};
    npca = ieee80211_uhr_npca_info(elems.uhr_operation);
    if (!npca) {
    chanreq.oper.npca_chan = core::ptr::null_mut();
...
// frame sending functions
    ...
    {
    u8 *pos;
    let mut flags: u32 = channel.flags;
    u16 cap;
...
//
    ...
    let mut mu_mimo_owner: bool = false;
...
//
    ...
    let mut disable_mu_mimo: bool = false;
...
pub const PRESENT_ELEMS_MAX: c_int = 8;
pub const PRESENT_ELEM_EXT_OFFS: c_uint = 0x100;
    static void
    ...
    struct ieee80211_mgd_assoc_data *assoc_data)
    {
    let mut iftype: enum nl80211_iftype = ieee80211_vif_type_p2p(&sdata.vif);
    struct cfg80211_bss *cbss = assoc_data.link[link_id].bss;
    struct ieee80211_channel *chan = cbss.channel;
    const struct ieee80211_sband_iftype_data *iftd;
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_supported_band *sband;
    let mut width: enum nl80211_chan_width = NL80211_CHAN_WIDTH_20;
    struct ieee80211_chanctx_conf *chanctx_conf;
    enum ieee80211_smps_mode smps_mode;
    let mut orig_capab: u16 = *capab;
    let mut offset: usize = 0;
    let mut present_elems_len: c_int = 0;
    u8 *pos;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sdata->u.mgd.powersave) -> else {
...
// pos++ = 2 * sband->n_channels;
    ...
    let mut cf: c_int = sband.channels[i].center_freq;
...
// crash if somebody gets it wrong
    present_elems = core::ptr::null_mut();
    if (assoc_data.link[link_id].conn.mode >= IEEE80211_CONN_MODE_EHT)
    ...
    const u16 *inner)
    {
    let mut skb_len: c_uint = skb.len;
    let mut at_extension: bool = false;
    let mut added: bool = false;
    int i, j;
    u8 *len, *list_len = core::ptr::null_mut();
    skb_put_u8(skb, WLAN_EID_EXTENSION);
    len = skb_put(skb, 1);
    skb_put_u8(skb, WLAN_EID_EXT_NON_INHERITANCE);
    for (i = 0; i < PRESENT_ELEMS_MAX && outer[i]; i++) {
    let mut elem: u16 = outer[i];
    let mut have_inner: bool = false;
...
// len = skb->len - skb_len - 2;
    ...
    let mut eml_capa: __le16 = 0, mld_capa_ops = 0;
    unsigned int link_id;
    u8 *ml_elem_len;
    void *capab_pos;
...
// add EML_CAPA only if needed, see Draft P802.11be_D2.1, 35.3.17
    ...
    size_t extra_elems_len;
    size_t extra_used;
    u8 *subelem_len = core::ptr::null_mut();
    __le16 ctrl;
...
#[no_mangle]
unsafe extern "C" fn ieee80211_send_assoc(sdata: *mut ieee80211_sub_if_data) -> c_int {
    ...
    let mut capab: u16 = 0, link_capab;
    __le16 listen_int;
    struct element *ext_capa = core::ptr::null_mut();
    let mut iftype: enum nl80211_iftype = ieee80211_vif_type_p2p(&sdata.vif);
    let mut info: ieee80211_prep_tx_info = {};
...
// we know it's writable, cast away the const
    ...
    for (link_id = 0; link_id < IEEE80211_MLD_MAX_NUM_LINKS; link_id++) {
    struct cfg80211_bss *cbss = assoc_data.link[link_id].bss;
    let mut elems_len: usize = assoc_data.link[link_id].elems_len;
    if (!cbss)
    continue;
    n_links++;
...
#[no_mangle]
unsafe extern "C" fn ieee80211_chswitch_post_beacon(link: *mut ieee80211_link_data) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sta_csa_rnr_iter_data {
...
//
    ...
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_if_managed *ifmgd = &sdata.u.mgd;
    struct ieee80211_chanctx *chanctx = core::ptr::null_mut();
    struct ieee80211_chanctx_conf *conf;
    let mut csa_ie: ieee80211_csa_ie = {};
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sta_bss_param_ch_cnt_data {
    pub sdata: *mut ieee80211_sub_if_data,
    pub reporting_link_id: u8,
    pub mld_id: u8,
}
...
//
    ...
    let mut chan: c_int = ieee80211_frequency_to_channel(channel.center_freq);
    int i, chan_increment;
    let mut have_chan_pwr: bool = false;
// Invalid IE
...
// find channel
    ...
    let mut first_channel: u8 = triplet.chans.first_channel;
...
// pwr_level = (__s8)cisco_dtpc_ie[4];
    ...
    {
    struct ieee80211_sub_if_data *sdata = link.sdata;
    let mut has_80211h_pwr: bool = false, has_cisco_pwr = false;
    let mut chan_pwr: c_int = 0, pwr_reduction_80211h = 0;
    int pwr_level_cisco, pwr_level_80211h;
    int new_ap_level;
    let mut capab: __le16 = mgmt.u.probe_resp.capab_info;
...
#[no_mangle]
unsafe extern "C" fn ieee80211_change_ps(local: *mut ieee80211_local) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_powersave_allowed(sdata: *mut ieee80211_sub_if_data) -> bool {
    static bool ieee80211_powersave_allowed(struct ieee80211_sub_if_data *sdata)
    {
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_if_managed *mgd = &sdata.u.mgd;
    struct sta_info *sta = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_recalc_ps(local: *mut ieee80211_local) {
    void ieee80211_recalc_ps(struct ieee80211_local *local)
    {
    struct ieee80211_sub_if_data *sdata, *found = core::ptr::null_mut();
    let mut count: c_int = 0;
    int timeout;
    if (!ieee80211_hw_check(&local.hw, SUPPORTS_PS) ||
    ieee80211_hw_check(&local.hw, SUPPORTS_DYNAMIC_PS)) {
    local.ps_sdata = core::ptr::null_mut();
    return;
    }
...
//
    ...
    let mut dtimper: u8 = found.deflink.u.mgd.dtim_period;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_recalc_ps_vif(sdata: *mut ieee80211_sub_if_data) {
    void ieee80211_recalc_ps_vif(struct ieee80211_sub_if_data *sdata)
    {
    let mut ps_allowed: bool = ieee80211_powersave_allowed(sdata);
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_dynamic_ps_timer(t: *mut timer_list) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_dfs_cac_timer_work(wiphy: *mut wiphy, work: *mut wiphy_work) {
    void ieee80211_dfs_cac_timer_work(struct wiphy *wiphy, struct wiphy_work *work)
    {
    struct ieee80211_link_data *link =
    container_of(work, struct ieee80211_link_data,
    dfs_cac_timer_work.work);
    let mut chandef: cfg80211_chan_def = link.conf.chanreq.oper;
    struct ieee80211_sub_if_data *sdata = link.sdata;
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_sta_handle_tspec_ac_params(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mgd_set_link_qos_params(link: *mut ieee80211_link_data) {
...
//
    ...
    let mut aci: c_int = (pos[0] >> 5) & 0x03;
    let mut acm: c_int = (pos[0] >> 4) & 0x01;
    let mut uapsd: bool = false;
    switch (aci) {
    case 1: /* AC_BK */
    ac = IEEE80211_AC_BK;
    if (acm)
...
#[no_mangle]
unsafe extern "C" fn __ieee80211_stop_poll(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_stop_poll(sdata: *mut ieee80211_sub_if_data) {
    ...
    struct ieee80211_bss_conf *bss_conf = link.conf;
    struct ieee80211_bss *bss = (void *)cbss.priv;
    let mut changed: u64 = BSS_CHANGED_QOS;
// not really used in MLO
    ...
    bss_conf.beacon_rate = core::ptr::null_mut();
    }
// Tell the driver to monitor connection quality (if supported)
    ...
    let mut vif_changed: u64 = BSS_CHANGED_ASSOC;
    unsigned int link_id;
    lockdep_assert_wiphy(local.hw.wiphy);
...
#[no_mangle]
unsafe extern "C" fn ieee80211_ml_reconf_reset(sdata: *mut ieee80211_sub_if_data) {
    ...
    if (add_links_data) {
    let mut done_data: cfg80211_mlo_reconf_done_data = {};
    u8 link_id;
    ...
    sdata.u.mgd.reconf.add_links_data = core::ptr::null_mut();
    }
    }
    ...
    if (tx) {
    let mut tx_link_found: bool = false;
    for (link_id = 0;
    link_id < ARRAY_SIZE(sdata.link);
    link_id++) {
    struct ieee80211_link_data *link;
...
// other links will be destroyed
    sdata.deflink.conf.bss = core::ptr::null_mut();
...
// disassociated - set to defaults now
    ...
    sdata.vif.bss_conf.beacon_rate = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ieee80211_reset_ap_probe(sdata: *mut ieee80211_sub_if_data) {
...
//
    ...
    ssid, ssid_len, core::ptr::null_mut(), 0,
    IEEE80211_PROBE_FLAG_DIRECTED);
    if (skb)
    ieee80211_tx_skb(sdata, skb);
    }
#[no_mangle]
unsafe extern "C" fn ieee80211_mgd_probe_ap_send(sdata: *mut ieee80211_sub_if_data) {
    static void ieee80211_mgd_probe_ap_send(struct ieee80211_sub_if_data *sdata)
    {
    struct ieee80211_if_managed *ifmgd = &sdata.u.mgd;
    u8 *dst = sdata.vif.cfg.ap_addr;
    let mut unicast_limit: u8 = max(1, max_probe_tries - 3);
    struct sta_info *sta;
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ifmgd->auth_data) -> else {
    else if (ifmgd.auth_data)
    cbss = ifmgd.auth_data.bss;
#[no_mangle]
pub unsafe extern "C" fn if(ifmgd->assoc_data->link[0].bss: ifmgd->assoc_data &&) -> else {
    ...
    (u32) -1, cbss.channel,
    ssid.data, ssid_len,
    core::ptr::null_mut(), 0, IEEE80211_PROBE_FLAG_DIRECTED);
    rcu_read_unlock();
    return skb;
    }
...
#[no_mangle]
unsafe extern "C" fn __ieee80211_disconnect(sdata: *mut ieee80211_sub_if_data) {
...
//
    ...
    link.conf.bss = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn ieee80211_uhr_omp_req_status(sdata: *mut ieee80211_sub_if_data) {
    static void ieee80211_uhr_omp_req_status(struct ieee80211_sub_if_data *sdata)
    {
    let mut acked: bool = sdata.u.mgd.uhr_omp.acked;
    let mut links: u16 = sdata.u.mgd.uhr_omp.links;
    struct ieee80211_link_data *link;
    struct sta_info *ap;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_beacon_loss(vif: *mut ieee80211_vif) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_connection_loss(vif: *mut ieee80211_vif) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_disconnect(vif: *mut ieee80211_vif, reconnect: bool) {
    ...
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
    sdata.u.mgd.auth_data = core::ptr::null_mut();
    if (info)
    drv_mgd_complete_tx(sdata.local, sdata, info);
    if (!assoc) {
//
...
// other links are destroyed
    ...
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
    sdata.u.mgd.assoc_data = core::ptr::null_mut();
    if (info)
    drv_mgd_complete_tx(sdata.local, sdata, info);
...
#[no_mangle]
unsafe extern "C" fn ieee80211_mark_sta_auth(sdata: *mut ieee80211_sub_if_data) -> bool {
...
// move station state to auth
    ...
    let mut sae_need_confirm: bool = false;
    let mut auth_fail: bool = false;
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
    if (len < 24 + 6)
    return;
...
//
    ...
    ieee80211_set_disassoc(sdata, 0, 0, false, core::ptr::null_mut());
    ...
    ieee80211_get_reason_code_string(reason_code));
    ieee80211_destroy_assoc_data(sdata, ASSOC_ABANDON, core::ptr::null_mut());
    cfg80211_rx_mlme_mgmt(sdata.dev, (u8 *)mgmt, len);
    return;
    }
    }
    ...
    struct ieee802_11_elems *elems)
    {
    let mut twt: bool = ieee80211_twt_req_supported(sdata, sband, link_sta, elems);
...
#[no_mangle]
unsafe extern "C" fn ieee80211_epcs_teardown(sdata: *mut ieee80211_sub_if_data) {
    ...
    struct ieee80211_bss_conf *bss_conf = link.conf;
    struct ieee80211_local *local = sdata.local;
    let mut link_id: c_uint = link.link_id;
    struct ieee80211_elems_parse_params parse_params = {
    ...
    let mut is_5ghz: bool = cbss.channel.band == NL80211_BAND_5GHZ;
    let mut is_6ghz: bool = cbss.channel.band == NL80211_BAND_6GHZ;
    let mut is_s1g: bool = cbss.channel.band == NL80211_BAND_S1GHZ;
    const struct cfg80211_bss_ies *bss_ies = core::ptr::null_mut();
    struct ieee80211_supported_band *sband;
    struct ieee802_11_elems *elems;
    u16 capab_info;
    bool ret;
...
//
    ...
    let mut bss_param_ch_cnt: c_int = ieee80211_mle_get_bss_param_ch_cnt(mle);
...
// Set up internal HT/VHT capabilities
    ...
    if (elems.vht_cap_elem &&
    link.u.mgd.conn.mode >= IEEE80211_CONN_MODE_VHT) {
    const struct ieee80211_vht_cap *bss_vht_cap = core::ptr::null_mut();
    const struct cfg80211_bss_ies *ies;
//
...
// ieee80211_set_associated() will tell the driver
    ...
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_bss *bss = (void *)cbss.priv;
    let mut rates: u32 = 0, basic_rates = 0;
    let mut have_higher_than_11mbit: bool = false;
    let mut min_rate: c_int = INT_MAX, min_rate_index = -1;
    struct ieee80211_supported_band *sband;
...
// TODO: S1G Basic Rate Set is expressed elsewhere
    ...
    core::ptr::null_mut(), 0,
    &rates, &basic_rates, core::ptr::null_mut(),
...
// mcs_nss is right after he_cap info
    ...
    for (i = 7; i >= 0; i--) {
    let mut mcs_80: u8 = mcs_80_map >> (2 * i) & 3;
    ...
    for (i = 7; i >= 0; i--) {
    let mut mcs_160: u8 = mcs_160_map >> (2 * i) & 3;
    ...
    struct ieee80211_conn_settings *conn)
    {
    let mut sta_ht_cap: ieee80211_sta_ht_cap = sband.ht_cap;
    let mut is_5ghz: bool = sband.band == NL80211_BAND_5GHZ;
    let mut is_6ghz: bool = sband.band == NL80211_BAND_6GHZ;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_6ghz) -> else {
    else if (is_6ghz)
    mlme_link_id_dbg(sdata, link_id,
    ...
    struct ieee80211_conn_settings *conn)
    {
    ieee80211_determine_our_sta_mode(sdata, sband, core::ptr::null_mut(), wmm_used,
...
// don't downgrade for S1G channels, though.
    ...
    ies.len);
    const struct ieee80211_tim_ie *tim = core::ptr::null_mut();
    const struct ieee80211_bssid_index *idx;
    let mut valid: bool = tim_ie && tim_ie[1] >= 2;
    if (valid)
    tim = (void *)(tim_ie + 2);
    if (dtim_count)
...
#[no_mangle]
unsafe extern "C" fn ieee80211_get_ttlm(bm_size: u8, data: *mut u8) -> u16 {
...
//
    ...
    pos += map_size;
    for (tid = 1; tid < 8; tid++) {
    let mut map: u16 = ieee80211_get_ttlm(map_size, pos);
    ...
    struct sta_info *sta;
    u64 changed[IEEE80211_MLD_MAX_NUM_LINKS] = {};
    let mut valid_links: u16 = 0, dormant_links = 0;
    int err;
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
//
...
//
    ...
    struct ieee80211_if_managed *ifmgd = &sdata.u.mgd;
    struct ieee80211_mgd_assoc_data *assoc_data = ifmgd.assoc_data;
    let mut assoc_status: enum assoc_status = ASSOC_ABANDON;
    ...
    unsigned int link_id;
    let mut max_aid: u16 = IEEE80211_MAX_AID;
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
    if (!assoc_data)
    return;
    info.link_id = assoc_data.assoc_link_id;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !assoc_data->s1g) -> else {
    else if (!assoc_data.s1g)
    aid = le16_to_cpu(mgmt.u.assoc_resp.aid);
#[no_mangle]
pub unsafe extern "C" fn if(WLAN_STATUS_SUCCESS: status_code ==) -> else {
...
// Track average RSSI from the Beacon frames of the current AP
    ...
    let mut sig: c_int = -ewma_beacon_signal_read(&link.u.mgd.ave_beacon_signal);
    let mut last_sig: c_int = link.u.mgd.last_ave_beacon_signal;
    struct ieee80211_event event = {
    .type = RSSI_EVENT,
    };
//
...
//
    ...
    let mut sig: c_int = -ewma_beacon_signal_read(&link.u.mgd.ave_beacon_signal);
    let mut last_event: c_int = link.u.mgd.last_cqm_event_signal;
    let mut thold: c_int = bss_conf.cqm_rssi_thold;
    let mut hyst: c_int = bss_conf.cqm_rssi_hyst;
    if (sig < thold &&
    ...
    let mut sig: c_int = -ewma_beacon_signal_read(&link.u.mgd.ave_beacon_signal);
    let mut last_event: c_int = link.u.mgd.last_cqm_event_signal;
    let mut low: c_int = bss_conf.cqm_rssi_low;
    let mut high: c_int = bss_conf.cqm_rssi_high;
    if (sig < low &&
    ...
    let mut removed_links: c_ulong = 0;
...
// hidden SSID: zeroed out
    ...
    let mut pri_khz: u32 = ieee80211_channel_to_khz(chanctx.def.chan);
    let mut rx_khz: u32 = ieee80211_rx_status_to_khz(rx_status);
    if (rx_khz == pri_khz)
    return true;
...
//
    ...
    let mut rx_khz: return = = pri_2mhz_khz;
    }
    ...
    struct ieee80211_ext *ext = core::ptr::null_mut();
    size_t baselen;
    struct ieee802_11_elems *elems;
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_chanctx_conf *chanctx_conf;
...
//
    if (sdata.vif.p2p ||
    sdata.vif.driver_flags & IEEE80211_VIF_GET_NOA_UPDATE) {
    let mut noa: ieee80211_p2p_noa_attr = {};
    int ret;
...
// len_pos = len;
    ...
    struct ieee80211_mgmt *mgmt;
    struct sk_buff *skb;
    let mut hdr_len: c_int = IEEE80211_MIN_ACTION_SIZE(ttlm_req);
    ...
    struct ieee80211_mgmt *mgmt;
    struct sk_buff *skb;
    let mut hdr_len: c_int = IEEE80211_MIN_ACTION_SIZE(ttlm_res);
    int ttlm_max_len = 2 + 1 + sizeof(struct ieee80211_ttlm_elem) + 1 +
    2 * 2 * IEEE80211_TTLM_NUM_TIDS;
    u16 status_code;
    skb = dev_alloc_skb(local.tx_headroom + hdr_len + ttlm_max_len);
...
// direction != IEEE80211_TTLM_DIRECTION_BOTH)
    ...
    size_t ies_len;
    let mut ttlm_res: enum ieee80211_neg_ttlm_res = NEG_TTLM_RES_ACCEPT;
    struct ieee802_11_elems *elems = core::ptr::null_mut();
    let mut neg_ttlm: ieee80211_neg_ttlm = {};
...
#[no_mangle]
unsafe extern "C" fn ieee80211_process_ttlm_teardown(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_send_teardown_neg_ttlm(vif: *mut ieee80211_vif) {
    void ieee80211_send_teardown_neg_ttlm(struct ieee80211_vif *vif)
    {
    let mut frame_len: c_int = IEEE80211_MIN_ACTION_SIZE(ttlm_tear_down);
    struct ieee80211_sub_if_data *sdata = vif_to_sdata(vif);
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_mgmt *mgmt;
    struct sk_buff *skb;
...
#[no_mangle]
unsafe extern "C" fn ieee80211_sta_timer(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_auth(sdata: *mut ieee80211_sub_if_data) -> c_int {
...
//
    ...
    auth_data.data, auth_data.data_len,
    auth_data.ap_addr, auth_data.ap_addr,
    core::ptr::null_mut(), 0, 0, tx_flags);
    if (tx_flags == 0) {
    if (auth_data.algorithm == WLAN_AUTH_SAE)
...
#[no_mangle]
unsafe extern "C" fn ieee80211_do_assoc(sdata: *mut ieee80211_sub_if_data) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_sta_work(sdata: *mut ieee80211_sub_if_data) {
    ...
    let mut fc: __le16 = ifmgd.status_fc;
    let mut status_acked: bool = ifmgd.status_acked;
...
//
    ieee80211_destroy_auth_data(sdata, false, core::ptr::null_mut());
...
// ACK received for nullfunc probing frame
    ...
    ieee80211_reset_ap_probe(sdata);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ifmgd->nullfunc_failed) -> else {
    ...
    run_again(sdata, ifmgd.probe_timeout);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ieee80211_hw_check(&local->hw, _arg: REPORTS_TX_ACK_STATUS)) -> else {
    mlme_dbg(sdata,
...
#[no_mangle]
unsafe extern "C" fn ieee80211_sta_bcn_mon_timer(t: *mut timer_list) {
    ...
    {
    let mut latest_timeout: c_ulong = jiffies;
    unsigned int link_id;
    struct sta_info *sta;
    guard(rcu)();
    sta = sta_info_get(sdata, sdata.vif.cfg.ap_addr);
...
#[no_mangle]
unsafe extern "C" fn ieee80211_sta_conn_mon_timer(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn ieee80211_restart_sta_timer(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mgd_quiesce(sdata: *mut ieee80211_sub_if_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_sta_restart(sdata: *mut ieee80211_sub_if_data) {
...
// interface setup
#[no_mangle]
pub unsafe extern "C" fn ieee80211_sta_setup_sdata(sdata: *mut ieee80211_sub_if_data) {
...
// Setup TDLS data
    ...
    ifmgd.teardown_skb = core::ptr::null_mut();
    ifmgd.orig_teardown_skb = core::ptr::null_mut();
    ifmgd.mcast_seq_last = IEEE80211_SN_MODULO;
    }
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mgd_setup_link(link: *mut ieee80211_link_data) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sdata->u.mgd.reconf.add_links_data) -> else {
    else if (sdata.u.mgd.reconf.add_links_data)
    ether_addr_copy(link.conf.addr,
    sdata.u.mgd.reconf.add_links_data.link[link_id].addr);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !is_valid_ether_addr(link->conf->addr)) -> else {
    else if (!is_valid_ether_addr(link.conf.addr))
    eth_random_addr(link.conf.addr);
    }
// scan finished notification
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mlme_notify_scan_completed(local: *mut ieee80211_local) {
...
// Restart STA timers
    ...
    struct ieee80211_if_managed *ifmgd = &sdata.u.mgd;
    struct ieee80211_bss *bss = (void *)cbss.priv;
    struct sta_info *new_sta = core::ptr::null_mut();
    struct ieee80211_link_data *link;
    struct sta_info *have_sta = core::ptr::null_mut();
    bool mlo;
    int err;
    u16 new_links;
    if (link_id >= 0) {
...
// Cancel scan to ensure that nothing interferes with connection
    ...
    struct ieee80211_channel_sw_ie *csa = core::ptr::null_mut();
    struct ieee80211_ext_chansw_ie *ecsa = core::ptr::null_mut();
    if (!ies)
    return false;
...
// try to authenticate/probe
    ...
    ieee80211_destroy_auth_data(sdata, cont_auth, core::ptr::null_mut());
    }
...
// hold our own reference
    ...
    ifmgd.auth_data = core::ptr::null_mut();
    kfree(auth_data);
    return err;
    }
    static void
...
// override HT configuration only if the AP and we support it
    ...
    {
    struct cfg80211_bss *cbss = assoc_data.link[link_id].bss;
    let mut band: enum nl80211_band = cbss.channel.band;
    struct ieee80211_supported_band *sband;
    const struct element *elem;
    int err;
...
// not an MLO connection if link_id < 0, so irrelevant
    ...
    struct cfg80211_assoc_request *req)
    {
    let mut assoc_link_id: c_uint = req.link_id < 0 ? 0 : req.link_id;
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_if_managed *ifmgd = &sdata.u.mgd;
    struct ieee80211_mgd_assoc_data *assoc_data;
    ...
    const struct wiphy_iftype_ext_capab *ift_ext_capa;
    struct ieee80211_link_data *link;
    let mut driver_ext_mld_capa_ops: u16 = 0;
    struct cfg80211_bss *cbss;
    bool override, uapsd_supported;
    bool match_auth;
    int i, err;
    let mut size: usize = sizeof(*assoc_data) + req.ie_len;
...
// only calculate the mode, hence link/link_sta == NULL
    err = ieee80211_prep_channel(sdata, core::ptr::null_mut(), core::ptr::null_mut(), i,
...
// We are associating, clean up auth_data
    if (ifmgd.auth_data)
    ieee80211_destroy_auth_data(sdata, true, core::ptr::null_mut());
    return 0;
    err_clear:
    if (!ifmgd.auth_data) {
    eth_zero_addr(sdata.deflink.u.mgd.bssid);
    ...
    ifmgd.assoc_data = core::ptr::null_mut();
    err_free:
    kfree(assoc_data);
    return err;
    }
    ...
    struct ieee80211_if_managed *ifmgd = &sdata.u.mgd;
    u8 frame_buf[IEEE80211_DEAUTH_FRAME_LEN];
    let mut tx: bool = !req.local_state_change;
    struct ieee80211_prep_tx_info info = {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mgd_stop_link(link: *mut ieee80211_link_data) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mgd_stop(sdata: *mut ieee80211_sub_if_data) {
...
//
    ...
    if (ifmgd.assoc_data)
    ieee80211_destroy_assoc_data(sdata, ASSOC_TIMEOUT, core::ptr::null_mut());
    if (ifmgd.auth_data)
    ieee80211_destroy_auth_data(sdata, false, core::ptr::null_mut());
    ...
    kfree(ifmgd.assoc_req_ies);
    ifmgd.assoc_req_ies = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_cqm_beacon_loss_notify(vif: *mut ieee80211_vif, gfp: gfp_t) {
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_disable_rssi_reports(vif: *mut ieee80211_vif) {
    ...
    let mut status: u16 = le16_to_cpu(reconf_status.status);
...
// Skip links that are not changing
    ...
    capab_pos = skb_put(skb, 2);
    extra_used =
    ieee80211_add_link_elems(sdata, skb, &capab, core::ptr::null_mut(),
    ...
    const struct wiphy_iftype_ext_capab *ift_ext_capa;
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_mgd_assoc_data *data = core::ptr::null_mut();
    struct sta_info *sta;
    struct sk_buff *skb;
...
//
    ...
    core::ptr::null_mut(), true, link_id,
    &data.link[link_id].conn);
...
// only used to verify the mode, nothing is allocated
    err = ieee80211_prep_channel(sdata, core::ptr::null_mut(), core::ptr::null_mut(), link_id,
...
#[no_mangle]
unsafe extern "C" fn ieee80211_mgd_epcs_supp(sdata: *mut ieee80211_sub_if_data) -> bool {
    static bool ieee80211_mgd_epcs_supp(struct ieee80211_sub_if_data *sdata)
    {
    let mut valid_links: c_ulong = sdata.vif.valid_links;
    u8 link_id;
    lockdep_assert_wiphy(sdata.local.hw.wiphy);
    if (!ieee80211_vif_is_mld(&sdata.vif))
    return false;
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_mgd_set_epcs(sdata: *mut ieee80211_sub_if_data, enable: bool) -> c_int {
    int ieee80211_mgd_set_epcs(struct ieee80211_sub_if_data *sdata, bool enable)
    {
    let mut frame_len: c_int = IEEE80211_MIN_ACTION_SIZE(epcs) + (enable ? 1 : 0);
    struct ieee80211_local *local = sdata.local;
    struct ieee80211_mgmt *mgmt;
    struct sk_buff *skb;
...
// pos = ++sdata->u.mgd.dialog_token_alloc;
    ...
    {
    const struct element *sub;
    let mut scratch_len: usize = elems.ml_epcs_len;
...
//
    ...
    elems.ml_epcs_len) {
    struct ieee802_11_elems *link_elems __free(kfree) = core::ptr::null_mut();
    struct ieee80211_link_data *link;
    ...
    size_t len)
    {
    struct ieee802_11_elems *elems __free(kfree) = core::ptr::null_mut();
    size_t ies_len;
    u16 status_code;
    u8 *pos, dialog_token;
    if (!ieee80211_mgd_epcs_supp(sdata))
