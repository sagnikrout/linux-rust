...
// Clear any remaining entries at the end
    ...
    sched_cfg.schedule[slot] = core::ptr::null_mut();
    conf = nan_channel.chanctx_conf;
...
//
    for (int i = 0; i < ARRAY_SIZE(sched_cfg.channels); i++) {
    let mut still_needed: bool = false;
    if (!sched_cfg.channels[i].chanreq.oper.chan)
    continue;
...
// Also a pre-existing channel might have been ULWed, so no chanctx
    ...
    sched_cfg.schedule[s] = core::ptr::null_mut();
    }
...
// Clear the stale chanctx pointer
    chan.chanctx_conf = core::ptr::null_mut();
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: conf) -> else {
    else if (conf)
//
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_nan_free_peer_sched(sched: *mut ieee80211_nan_peer_sched) {
...
// Set up the slots array
    ...
    let mut chan_idx: u8 = cfg_map.schedule[slot];
...
#[no_mangle]
pub unsafe extern "C" fn ieee80211_nan_update_ndi_carrier(ndi_sdata: *mut ieee80211_sub_if_data) {
...
// Success - free old schedule
    ...
    if (WARN_ON(!conf))
    return;
    nan_channel.chanctx_conf = core::ptr::null_mut();
...
// Clean up the channel context if no longer used
    ...
    struct ieee80211_nan_channel *evac_chan = core::ptr::null_mut();
    let mut min_slot_count: c_int = INT_MAX;
    let mut usable_channels: c_int = 0;
    lockdep_assert_wiphy(local.hw.wiphy);
...
// NAN needs at least one remaining usable channel after evacuation
    ...
    struct ieee80211_chanctx *ctx = core::ptr::null_mut();
    lockdep_assert_wiphy(local.hw.wiphy);
    if (!sdata)
    return false;
    if (conf)
