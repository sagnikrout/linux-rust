...
pub const PSAMPLE_MAX_PACKET_SIZE: c_uint = 0xffff;
...
#[no_mangle]
unsafe extern "C" fn psample_group_destroy(group: *mut psample_group) {
...
pub unsafe extern "C" fn psample_group_put(group: *mut psample_group) {
    ...
    {
    let mut tun_proto: c_ushort = ip_tunnel_info_af(tun_info);
    const void *tun_opts = ip_tunnel_info_opts(tun_info);
    const struct ip_tunnel_key *tun_key = &tun_info.key;
    let mut tun_opts_len: c_int = tun_info.options_len;
...
#[no_mangle]
unsafe extern "C" fn psample_tunnel_meta_len(tun_info: *mut ip_tunnel_info) -> c_int {
    ...
    let mut out_ifindex: c_int = md.out_ifindex;
    let mut in_ifindex: c_int = md.in_ifindex;
    let mut trunc_size: u32 = md.trunc_size;
    ...
    goto error;
    if (data_len) {
    let mut nla_len: c_int = nla_attr_size(data_len);
    struct nlattr *nla;
...
#[no_mangle]
unsafe extern "C" fn psample_module_init() -> int __init {
    static int __init psample_module_init(void)
    {
    return genl_register_family(&psample_nl_family);
    }
#[no_mangle]
unsafe extern "C" fn psample_module_exit() -> void __exit {
