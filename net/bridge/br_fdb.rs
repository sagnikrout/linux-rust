...
#[no_mangle]
pub unsafe extern "C" fn br_fdb_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn br_fdb_fini() {
    void br_fdb_fini(void)
    {
    kmem_cache_destroy(br_fdb_cache);
    }
#[no_mangle]
pub unsafe extern "C" fn br_fdb_hash_init(br: *mut net_bridge) -> c_int {
    int br_fdb_hash_init(struct net_bridge *br)
    {
    return rhashtable_init(&br.fdb_hash_tbl, &br_fdb_rht_params);
    }
#[no_mangle]
pub unsafe extern "C" fn br_fdb_hash_fini(br: *mut net_bridge) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hold_time(br: *const net_bridge) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(BR_FDB_STATIC, _arg: &fdb->flags)) -> else {
    else if (test_bit(BR_FDB_STATIC, &fdb.flags))
    return NUD_NOARP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_expired(br, _arg: fdb)) -> else {
    ...
    let mut notify_bits: u8 = FDB_NOTIFY_BIT;
    if (!nest)
    goto nla_put_failure;
...
#[no_mangle]
pub unsafe extern "C" fn fdb_nlmsg_size() -> usize {
...
//
#[no_mangle]
unsafe extern "C" fn fdb_add_hw_addr(br: *mut net_bridge, addr: *const c_uchar) {
...
//
#[no_mangle]
unsafe extern "C" fn fdb_del_hw_addr(br: *mut net_bridge, addr: *const c_uchar) {
...
// Maybe bridge device has same hw addr?
    ...
    WRITE_ONCE(f.dst, core::ptr::null_mut());
    clear_bit(BR_FDB_ADDED_BY_USER, &f.flags);
    return;
    }
    ...
    let mut max_learned: u32 = READ_ONCE(br.fdb_max_learned);
    struct net_bridge_fdb_entry *fdb;
    int err;
    if (likely(learned)) {
    let mut n_learned: c_int = atomic_read(&br.fdb_n_learned);
...
#[no_mangle]
pub unsafe extern "C" fn br_fdb_changeaddr(p: *mut net_bridge_port, newaddr: *const c_uchar) {
...
#[no_mangle]
pub unsafe extern "C" fn br_fdb_change_mac_address(br: *mut net_bridge, newaddr: *const u8) {
...
// If old entry was unassociated with any port, then delete it.
    ...
    fdb_delete_local(br, core::ptr::null_mut(), f);
    fdb_add_local(br, core::ptr::null_mut(), newaddr, 0);
    vg = br_vlan_group(br);
    if (!vg || !vg.num_vlans || local_vlan_0)
    goto out;
...
//
    ...
    !f.dst && !test_bit(BR_FDB_ADDED_BY_USER, &f.flags))
    fdb_delete_local(br, core::ptr::null_mut(), f);
    fdb_add_local(br, core::ptr::null_mut(), newaddr, v.vid);
    }
    out:
    spin_unlock_bh(&br.hash_lock);
    }
#[no_mangle]
pub unsafe extern "C" fn br_fdb_cleanup(work: *mut work_struct) {
    void br_fdb_cleanup(struct work_struct *work)
    {
    struct net_bridge *br = container_of(work, struct net_bridge,
    gc_work.work);
    struct net_bridge_fdb_entry *f = core::ptr::null_mut();
    let mut delay: c_ulong = hold_time(br);
    let mut work_delay: c_ulong = delay;
    let mut now: c_ulong = jiffies;
...
//
    ...
    let mut this_timer: c_ulong = READ_ONCE(f.updated) + delay;
    if (test_bit(BR_FDB_STATIC, &f.flags) ||
...
#[no_mangle]
unsafe extern "C" fn br_fdb_delete_locals_per_vlan(br: *mut net_bridge) {
    ...
    br_fdb_delete_locals_per_vlan_port(br, core::ptr::null_mut());
    }
    ...
    err = br_fdb_insert_locals_per_vlan_port(br, core::ptr::null_mut(), extack);
    if (err)
    goto rollback;
    return 0;
    rollback:
    ...
    let mut port_ifidx: c_int = dst ? dst.dev.ifindex : br.dev.ifindex;
...
#[no_mangle]
unsafe extern "C" fn __ndm_state_to_fdb_flags(ndm_state: u16) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn __ndm_flags_to_fdb_flags(ndm_flags: u8) -> c_ulong {
    ...
    struct netlink_ext_ack *extack)
    {
    let mut desc: net_bridge_fdb_flush_desc = {};
    ...
    let mut ndm_state_mask: u16 = nla_get_u16(tb[NDA_NDM_STATE_MASK]);
    desc.flags_mask |= __ndm_state_to_fdb_flags(ndm_state_mask);
    }
    if (tb[NDA_NDM_FLAGS_MASK]) {
    let mut ndm_flags_mask: u8 = nla_get_u8(tb[NDA_NDM_FLAGS_MASK]);
    desc.flags_mask |= __ndm_flags_to_fdb_flags(ndm_flags_mask);
    }
    if (tb[NDA_IFINDEX]) {
    let mut ifidx: c_int = nla_get_s32(tb[NDA_IFINDEX]);
...
#[no_mangle]
unsafe extern "C" fn __fdb_mark_active(fdb: *mut net_bridge_fdb_entry) -> bool {
...
// attempt to update an entry for a local interface
    ...
    let mut now: c_ulong = jiffies;
    let mut fdb_modified: bool = false;
...
// Dump information about entries, in response to GETNEIGH
    ...
    return err;
    if (!filter_dev) {
    err = ndo_dflt_fdb_dump(skb, cb, dev, core::ptr::null_mut(), idx);
    if (err < 0)
    return err;
    }
    rcu_read_lock();
...
#[no_mangle]
unsafe extern "C" fn fdb_handle_notify(fdb: *mut net_bridge_fdb_entry, notify: u8) -> bool {
...
// Update (create or replace) forwarding database entry
    ...
    {
    let mut is_sticky: bool = !!(ndm.ndm_flags & NTF_STICKY);
    let mut refresh: bool = !nfea_tb[NFEA_DONT_REFRESH];
    struct net_bridge_fdb_entry *fdb;
    let mut state: u16 = ndm.ndm_state;
    let mut modified: bool = false;
    let mut notify: u8 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn br_fdb_sync_static(br: *mut net_bridge, p: *mut net_bridge_port) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn br_fdb_unsync_static(br: *mut net_bridge, p: *mut net_bridge_port) {
...
// We only care for static entries
    ...
    fdb = br_fdb_find(br, addr, vid);
    if (!fdb) {
    let mut flags: c_ulong = BIT(BR_FDB_ADDED_BY_EXT_LEARN);
    if (swdev_notify)
...
#[no_mangle]
pub unsafe extern "C" fn br_fdb_clear_offload(dev: *const net_device, vid: u16) {
