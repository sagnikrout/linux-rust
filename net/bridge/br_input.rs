...
#[no_mangle]
unsafe extern "C" fn br_pass_frame_up(skb: *mut sk_buff, promisc: bool) -> c_int {
...
//
    ...
    indev = skb.dev;
    skb.dev = brdev;
    skb = br_handle_vlan(br, core::ptr::null_mut(), vg, skb);
    if (!skb)
    return NET_RX_DROP;
// update the multicast stats if the packet is IGMP/MLD
    br_multicast_count(br, core::ptr::null_mut(), skb, br_multicast_igmp_type(skb),
    BR_MCAST_DIR_TX);
    BR_INPUT_SKB_CB(skb).promisc = promisc;
    return NF_HOOK(NFPROTO_BRIDGE, NF_BR_LOCAL_IN,
    dev_net(indev), core::ptr::null_mut(), skb, indev, core::ptr::null_mut(),
    br_netif_receive_skb);
    }
// note: already called with rcu_read_lock
#[no_mangle]
pub unsafe extern "C" fn br_handle_frame_finish(net: *mut net, sk: *mut sock, skb: *mut sk_buff) -> c_int {
    int br_handle_frame_finish(struct net *net, struct sock *sk, struct sk_buff *skb)
    {
    let mut reason: enum skb_drop_reason = SKB_DROP_REASON_NOT_SPECIFIED;
    struct net_bridge_port *p = br_port_get_rcu(skb.dev);
    let mut pkt_type: enum br_pkt_type = BR_PKT_UNICAST;
    struct net_bridge_fdb_entry *dst = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn __br_handle_local_finish(skb: *mut sk_buff) {
...
// note: already called with rcu_read_lock
#[no_mangle]
unsafe extern "C" fn br_handle_local_finish(net: *mut net, sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nf_hook_bridge_pre(skb: *mut sk_buff, pskb: *mut sk_buff) -> c_int {
    static int nf_hook_bridge_pre(struct sk_buff *skb, struct sk_buff **pskb)
    {

    struct nf_hook_entries *e = core::ptr::null_mut();
    ...
    NFPROTO_BRIDGE, skb.dev, core::ptr::null_mut(), core::ptr::null_mut(),
...
// pskb = skb;
    ...
    frame_finish:
    net = dev_net(skb.dev);
    br_handle_frame_finish(net, core::ptr::null_mut(), skb);
...
//
#[no_mangle]
unsafe extern "C" fn br_handle_frame(pskb: *mut sk_buff) -> rx_handler_result_t {
    ...
    let mut fwd_mask: u16 = p.br.group_fwd_mask_required;
//
...
//
#[no_mangle]
unsafe extern "C" fn br_handle_frame_dummy(pskb: *mut sk_buff) -> rx_handler_result_t {
...
#[no_mangle]
pub unsafe extern "C" fn br_add_frame(br: *mut net_bridge, ft: *mut br_frame_type) {
    void br_add_frame(struct net_bridge *br, struct br_frame_type *ft)
    {
    hlist_add_head_rcu(&ft.list, &br.frame_type_list);
    }
#[no_mangle]
pub unsafe extern "C" fn br_del_frame(br: *mut net_bridge, ft: *mut br_frame_type) {
