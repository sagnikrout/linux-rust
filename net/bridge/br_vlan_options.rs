...
//! Source: net/bridge/br_vlan_options.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn __vlan_tun_put(skb: *mut sk_buff, v: *const net_bridge_vlan) -> bool {
    static bool __vlan_tun_put(struct sk_buff *skb, const struct net_bridge_vlan *v)
    {
    let mut tid: __be32 = tunnel_id_to_key32(v.tinfo.tunnel_id);
    struct nlattr *nest;
...
// check if the options' state of v_curr allow it to enter the range
    ...
    const struct net_bridge_vlan *range_end)
    {
    let mut range_mc_rtr: u8 = br_vlan_multicast_router(range_end);
    let mut curr_mc_rtr: u8 = br_vlan_multicast_router(v_curr);
...
#[no_mangle]
pub unsafe extern "C" fn br_vlan_opts_nl_size() -> usize {
...
// changed = false;
    ...
    let mut state: u8 = nla_get_u8(tb[BRIDGE_VLANDB_ENTRY_STATE]);
...
// changed = true;
    ...
    let mut enabled: bool = v.priv_flags & BR_VLFLAG_NEIGH_SUPPRESS_ENABLED;
    let mut val: bool = nla_get_u8(tb[BRIDGE_VLANDB_ENTRY_NEIGH_SUPPRESS]);
    if (!p) {
...
// changed = true;
    ...
    let mut enabled: bool = v.priv_flags & BR_VLFLAG_NEIGH_FORWARD_GRAT_ENABLED;
    let mut val: bool = nla_get_u8(tb[BRIDGE_VLANDB_ENTRY_NEIGH_FORWARD_GRAT]);
    if (!p) {
...
// changed = true;
    ...
    struct netlink_ext_ack *extack)
    {
    struct net_bridge_vlan *v, *curr_start = core::ptr::null_mut(), *curr_end = core::ptr::null_mut();
    struct net_bridge_vlan_group *vg;
    int vid, err = 0;
    u16 pvid;
    if (p)
...
#[no_mangle]
unsafe extern "C" fn rtnl_vlan_global_opts_nlmsg_size(v: *const net_bridge_vlan) -> usize {
...
// need to find the vlan due to flags/options
    ...
    goto out_err;
    nlmsg_end(skb, nlh);
    rtnl_notify(skb, dev_net(br.dev), 0, RTNLGRP_BRVLAN, core::ptr::null_mut(), GFP_KERNEL);
    return;
    out_err:
