...
//! Source: net/bridge/netfilter/ebtables.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ebt_pernet {
    pub tables: list_head,
    pub dead_tables: list_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ebt_template {
    pub list: list_head,
...
#[no_mangle]
unsafe extern "C" fn ebt_standard_compat_from_user(dst: *mut c_void, src: *const c_void) {
    static void ebt_standard_compat_from_user(void *dst, const void *src)
    {
    let mut v: c_int = *(compat_int_t *)src;
    if (v >= 0)
    v += xt_compat_calc_jump(NFPROTO_BRIDGE, v);
    memcpy(dst, &v, sizeof(v));
    }
#[no_mangle]
unsafe extern "C" fn ebt_standard_compat_to_user(dst: *mut void __user, src: *const c_void) -> c_int {
    static int ebt_standard_compat_to_user(void __user *dst, const void *src)
    {
    let mut cv: compat_int_t = *(int *)src;
    if (cv >= 0)
...
// rcu_read_lock()ed by nf_hook_thresh
    if (in && (p = br_port_get_rcu(in)) != core::ptr::null_mut() &&
    NF_INVF(e, EBT_ILOGICALIN,
    ebt_dev_check(e.logical_in, p.br.dev)))
    return 1;
    if (out && (p = br_port_get_rcu(out)) != core::ptr::null_mut() &&
...
// Do some firewalling
    ...
    let mut hook: c_uint = state.hook;
    int i, nentries;
...
#[no_mangle]
pub unsafe extern "C" fn ebt_free_table_info(info: *mut ebt_table_info) {
    ...
    let mut left: usize = ((char *)e + e.watchers_offset) - (char *)m;
    int ret;
    ...
    let mut left: usize = ((char *)e + e.target_offset) - (char *)w;
    int ret;
    ...
    {
    let mut limit: c_uint = repl.entries_size;
    let mut valid_hooks: c_uint = repl.valid_hooks;
    let mut offset: c_uint = 0;
    int i;
    for (i = 0; i < NF_BR_NUMHOOKS; i++)
    newinfo.hook_entry[i] = core::ptr::null_mut();
    newinfo.entries_size = repl.entries_size;
    newinfo.nentries = repl.nentries;
    while (offset < limit) {
    let mut left: usize = limit - offset;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ebt_cl_stack {
    pub cs: ebt_chainstack,
    pub from: c_int,
    pub hookmask: c_uint,
}
...
// these initialisations are depended on later in check_chainloops()
    ...
    if (par.match.destroy != core::ptr::null_mut())
    ...
    par.targinfo = w.data;
    par.family   = NFPROTO_BRIDGE;
    if (par.target.destroy != core::ptr::null_mut())
...
// we're done
    if (cnt && (*cnt)-- == 0)
    return 1;
    EBT_WATCHER_ITERATE(e, ebt_cleanup_watcher, net, core::ptr::null_mut());
    EBT_MATCH_ITERATE(e, ebt_cleanup_match, net, core::ptr::null_mut());
    t = ebt_get_target(e);
    par.net      = net;
...
// do the parsing of the table/chains/entries/matches/watchers/targets, heh
    ...
    int ret;
    struct ebt_cl_stack *cl_s = core::ptr::null_mut(); /* used in the checking for chain loops */
    i = 0;
...
//
    ...
    newinfo.chainstack = core::ptr::null_mut();
    return -ENOMEM;
    }
    }
...
// add other counters to those of cpu 0
    ...
    {
    int ret;
    struct ebt_counter *counterstmp = core::ptr::null_mut();
...
// decrease module count and free resources
    ...
    ebt_cleanup_entry, net, core::ptr::null_mut());
    vfree(table.entries);
...
// replace the table
#[no_mangle]
unsafe extern "C" fn do_replace(net: *mut net, arg: sockptr_t, len: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __ebt_unregister_table(net: *mut net, table: *mut ebt_table) {
    ...
    void *p;
    if (input_table == core::ptr::null_mut() || (repl = input_table.table) == core::ptr::null_mut() ||
    repl.entries == core::ptr::null_mut() || repl.entries_size == 0 ||
    repl.counters != core::ptr::null_mut() || input_table.private != core::ptr::null_mut())
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn ebt_register_template(t: *const ebt_table, net): *mut *mut int (table_init)(struct net) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ebt_unregister_template(t: *const ebt_table) {
...
#[no_mangle]
pub unsafe extern "C" fn ebt_unregister_table_pre_exit(net: *mut net, name: *const c_char) {
...
#[no_mangle]
pub unsafe extern "C" fn ebt_unregister_table(net: *mut net, name: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn update_counters(net: *mut net, arg: sockptr_t, len: c_uint) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_ebt_replace {
    pub name: [c_char; EBT_TABLE_MAXNAMELEN],
    pub valid_hooks: compat_uint_t,
    pub nentries: compat_uint_t,
    pub entries_size: compat_uint_t,
// start of the chains
    pub hook_entry: [compat_uptr_t; NF_BR_NUMHOOKS],
// nr of counters userspace expects back
    pub num_counters: compat_uint_t,
// where the kernel will put the old counters.
    pub counters: compat_uptr_t,
    pub entries: compat_uptr_t,
}

// struct ebt_entry_match, _target and _watcher have same layout
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_ebt_entry_mwt {
    union {
    struct {
...
#[no_mangle]
unsafe extern "C" fn ebt_compat_entry_padsize() -> c_int {
...
//
    ...
    let mut off: c_int = ebt_compat_match_offset(match, m.match_size);
    let mut msize: compat_uint_t = m.match_size - off;
...
// dstptr += msize;
    ...
    let mut tsize: compat_uint_t = t.target_size - off;
...
#[no_mangle]
unsafe extern "C" fn compat_calc_match(m: *mut ebt_entry_match, off: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compat_calc_watcher(w: *mut ebt_entry_watcher, off: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ebt_compat_init_offsets(number: c_uint) -> c_int {
...
// also count the base chain policies
    ...
    struct compat_ebt_replace *newinfo)
    {
    let mut size: c_uint = info.entries_size;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ebt_entries_buf_state {
    pub /: *mut *mut *mut char buf_kern_start; / kernel buffer to copy (translated) data to,
    pub /: *mut *mut u32 buf_kern_len; / total size of kernel buffer,
    pub /: *mut *mut u32 buf_kern_offset; / amount of data copied so far,
    pub /: *mut *mut u32 buf_user_offset; / read position in userspace buffer,
}

#[no_mangle]
unsafe extern "C" fn ebt_buf_count(state: *mut ebt_entries_buf_state, sz: c_uint) -> c_int {
    ...
    {
    if (state.buf_kern_start == core::ptr::null_mut())
    goto count_only;
...
#[no_mangle]
unsafe extern "C" fn ebt_buf_add_pad(state: *mut ebt_entries_buf_state, sz: c_uint) -> c_int {
    static int ebt_buf_add_pad(struct ebt_entries_buf_state *state, unsigned int sz)
    {
    char *b = state.buf_kern_start;
    if (WARN_ON(b && state.buf_kern_offset > state.buf_kern_len))
    return -EINVAL;
    if (b != core::ptr::null_mut() && sz > 0)
...
#[no_mangle]
unsafe extern "C" fn match_size_ok(match: *const xt_match, match_size: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tgt_size_ok(tgt: *const xt_target, tgt_size: c_uint) -> bool {
    static bool tgt_size_ok(const struct xt_target *tgt, unsigned int tgt_size)
    {
    let mut csize: u16 = tgt.compatsize ? : tgt.targetsize;
...
// stores match/watchers/targets & offset of next struct ebt_entry:
    ...
    unsigned int *offsets_update = core::ptr::null_mut();
    int ret;
    char *buf_start;
...
//
    ...
    if (state.buf_kern_start == core::ptr::null_mut()) {
    let mut offset: c_uint = buf_start - (char *) base;
...
//
    ...
    struct ebt_entries_buf_state *state)
    {
    let mut size_remaining: c_uint = size_user;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn compat_do_replace(net: *mut net, arg: sockptr_t, len: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_ebt_get_ctl(sk: *mut sock, cmd: c_int, user: *mut void __user, len: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ebt_pernet_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
unsafe extern "C" fn ebt_pernet_exit(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn ebtables_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ebtables_fini() {
