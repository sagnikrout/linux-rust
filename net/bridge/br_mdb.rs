...
#[no_mangle]
unsafe extern "C" fn __br_rports_one_size() -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn br_rports_size(brmctx: *const net_bridge_mcast) -> usize {
    ...
    {
    let mut vid: u16 = brmctx.vlan ? brmctx.vlan.vid : 0;
...
#[no_mangle]
unsafe extern "C" fn __mdb_entry_fill_flags(e: *mut br_mdb_entry, flags: c_uchar) {
    ...
    struct net_bridge_port_group *p)
    {
    let mut dump_srcs_mode: bool = false;
    ...
    {
    let mut idx: c_int = 0, s_idx = cb.args[1], err = 0, pidx = 0, s_pidx = cb.args[2];
    ...
    err = __mdb_fill_info(skb, mp, core::ptr::null_mut());
    if (err) {
    nla_nest_cancel(skb, nest2);
    break;
    }
    }
    for (pp = &mp.ports; (p = rcu_dereference(*pp)) != core::ptr::null_mut();
    pp = &p.next) {
    if (!p.key.port)
    continue;
    if (pidx < s_pidx)
...
#[no_mangle]
unsafe extern "C" fn rtnl_mdb_nlmsg_pg_size(pg: *const net_bridge_port_group) -> usize {
...
#[no_mangle]
unsafe extern "C" fn rtnl_mdb_nlmsg_size(pg: *const net_bridge_port_group) -> usize {
...
// Port group entry
    ...
    rtnl_notify(skb, net, 0, RTNLGRP_MDB, core::ptr::null_mut(), GFP_ATOMIC);
    return;
    errout:
...
#[no_mangle]
pub unsafe extern "C" fn rtnl_rtr_nlmsg_size() -> usize {
    static inline size_t rtnl_rtr_nlmsg_size(void)
    {
#[no_mangle]
pub unsafe extern "C" fn NLMSG_ALIGN(br_port_msg): sizeof(struct) -> return {
    return NLMSG_ALIGN(sizeof(struct br_port_msg))
    ...
    struct netlink_ext_ack *extack)
    {
    struct net_bridge_mcast *brmctx = core::ptr::null_mut();
    struct net_bridge_vlan *v;
    ...
    let mut now: c_ulong = jiffies;
    for (pp = &mp.ports;
    (p = mlock_dereference(*pp, cfg.br)) != core::ptr::null_mut();
    pp = &p.next) {
    if (p.key.port == cfg.p) {
    ...
    p = br_multicast_new_port_group(cfg.p, &cfg.group, *pp, flags, core::ptr::null_mut(),
...
// host join
    ...
    br_mdb_notify(br.dev, mp, core::ptr::null_mut(), RTM_NEWMDB);
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn br_mdb_config_src_list_fini(cfg: *mut br_mdb_config) {
    ...
    __mdb_entry_to_br_ip(cfg.entry, &cfg.group, core::ptr::null_mut());
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn br_mdb_config_fini(cfg: *mut br_mdb_config) {
...
#[no_mangle]
unsafe extern "C" fn __br_mdb_del(cfg: *const br_mdb_config) -> c_int {
...
// host leave
    ...
    err = 0;
    br_mdb_notify(br.dev, mp, core::ptr::null_mut(), RTM_DELMDB);
...
pub struct br_mdb_flush_desc {
    ...
    struct net_bridge_port_group __rcu **pp;
    struct net_bridge_port_group *p;
    for (pp = &mp.ports; (p = mlock_dereference(*pp, br)) != core::ptr::null_mut();) {
    u8 state;
...
//
    ...
    {
    struct net_bridge *br = netdev_priv(dev);
    let mut desc: br_mdb_flush_desc = {};
    int err;
    ...
    __mdb_entry_to_br_ip(entry, group, core::ptr::null_mut());
    return 0;
    }
    err = nla_parse_nested(mdbe_attrs, MDBE_ATTR_MAX,
...
// MDBA_MDB_ENTRY
    nla_total_size(0);
    if (mp.host_joined)
    nlmsg_size += rtnl_mdb_nlmsg_pg_size(core::ptr::null_mut());
