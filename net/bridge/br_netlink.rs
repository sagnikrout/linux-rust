...
//! Source: net/bridge/br_netlink.c
#![no_std]
...
//
    ...
    let mut vid_range_start: u16 = 0, vid_range_end = 0, vid_range_flags = 0;
    u16 flags, pvid;
    let mut num_vlans: c_int = 0;
...
// only a context, bridge vlan not activated
    ...
    u32 filter_mask)
    {
    struct net_bridge_vlan_group *vg = core::ptr::null_mut();
    struct net_bridge_port *p = core::ptr::null_mut();
    struct net_bridge *br = core::ptr::null_mut();
    u32 num_cfm_peer_mep_infos;
    u32 num_cfm_mep_infos;
    let mut vinfo_sz: usize = 0;
    int num_vlan_infos;
    rcu_read_lock();
...
#[no_mangle]
pub unsafe extern "C" fn br_port_info_size() -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn br_nlmsg_size(dev: *mut net_device, filter_mask: u32) -> usize {
...
pub unsafe extern "C" fn NLMSG_ALIGN(ifinfomsg): sizeof(struct) -> return {
    ...
    {
    let mut flags: c_ulong = READ_ONCE(p.flags);
    let mut mode: u8 = !!(flags & BR_HAIRPIN_MODE);
...
//
    ...
    IF_OPER_DOWN;
    struct nlattr *af = core::ptr::null_mut();
    ...
    if (nest == core::ptr::null_mut() || br_port_fill_attrs(skb, port) < 0)
    goto nla_put_failure;
...
// RCU needed because of the VLAN locking rules (rcu || rtnl)
    ...
    struct nlattr *cfm_nest = core::ptr::null_mut();
    int err;
...
// Notify listeners of a change in bridge or port information
    ...
    const struct net_bridge_port *port)
    {
    let mut filter: u32 = RTEXT_FILTER_BRVLAN_COMPRESSED;
...
// vinfo_last = NULL;
    ...
    struct netlink_ext_ack *extack)
    {
    struct bridge_vlan_info *vinfo_curr = core::ptr::null_mut();
    struct bridge_vlan_info *vinfo_last = core::ptr::null_mut();
    struct nlattr *attr;
    let mut tinfo_last: vtunnel_info = {};
    let mut tinfo_curr: vtunnel_info = {};
    let mut err: c_int = 0, rem;
...
#[no_mangle]
unsafe extern "C" fn br_set_port_state(p: *mut net_bridge_port, state: u8) -> c_int {
...
// Process bridge protocol info on port
    ...
    let mut mcast_router: u8 = nla_get_u8(tb[IFLA_BRPORT_MULTICAST_ROUTER]);
    ...
    let mut fwd_mask: u16 = nla_get_u16(tb[IFLA_BRPORT_GROUP_FWD_MASK]);
    ...
    let mut backup_nhid: u32 = nla_get_u32(tb[IFLA_BRPORT_BACKUP_NHID]);
...
#[no_mangle]
pub unsafe extern "C" fn br_dellink(dev: *mut net_device, nlh: *mut nlmsghdr, flags: u16) -> c_int {
...
// We want to accept dev as bridge itself as well
    ...
    return -EINVAL;
    err = br_afspec(br, p, afspec, RTM_DELLINK, &changed, core::ptr::null_mut());
    if (changed)
...
//
    ...
    let mut defpvid: __u16 = nla_get_u16(data[IFLA_BR_VLAN_DEFAULT_PVID]);
    ...
    let mut mode: u32 = nla_get_u32(data[IFLA_BR_STP_MODE]);
    if (mode != br.stp_mode) {
    ...
    let mut stp_enabled: u32 = nla_get_u32(data[IFLA_BR_STP_STATE]);
    err = br_stp_set_enabled(br, stp_enabled, extack);
    if (err)
    return err;
    }
    if (data[IFLA_BR_PRIORITY]) {
    let mut priority: u32 = nla_get_u16(data[IFLA_BR_PRIORITY]);
    br_stp_set_bridge_priority(br, priority);
    }
    if (data[IFLA_BR_VLAN_FILTERING]) {
    let mut vlan_filter: u8 = nla_get_u8(data[IFLA_BR_VLAN_FILTERING]);
    ...
    if (data[IFLA_BR_VLAN_PROTOCOL]) {
    let mut vlan_proto: __be16 = nla_get_be16(data[IFLA_BR_VLAN_PROTOCOL]);
    ...
    let mut vlan_stats: __u8 = nla_get_u8(data[IFLA_BR_VLAN_STATS_ENABLED]);
    err = br_vlan_set_stats(br, vlan_stats);
    if (err)
    return err;
    }
    if (data[IFLA_BR_VLAN_STATS_PER_PORT]) {
    let mut per_port: __u8 = nla_get_u8(data[IFLA_BR_VLAN_STATS_PER_PORT]);
    ...
    let mut fwd_mask: u16 = nla_get_u16(data[IFLA_BR_GROUP_FWD_MASK]);
    ...
    let mut multicast_router: u8 = nla_get_u8(data[IFLA_BR_MCAST_ROUTER]);
    ...
    let mut mcast_snooping: u8 = nla_get_u8(data[IFLA_BR_MCAST_SNOOPING]);
    ...
    let mut mcast_querier: u8 = nla_get_u8(data[IFLA_BR_MCAST_QUERIER]);
    ...
    let mut val: u32 = nla_get_u32(data[IFLA_BR_MCAST_LAST_MEMBER_CNT]);
    br.multicast_ctx.multicast_last_member_count = val;
    }
    if (data[IFLA_BR_MCAST_STARTUP_QUERY_CNT]) {
    let mut val: u32 = nla_get_u32(data[IFLA_BR_MCAST_STARTUP_QUERY_CNT]);
    br.multicast_ctx.multicast_startup_query_count = val;
    }
    if (data[IFLA_BR_MCAST_LAST_MEMBER_INTVL]) {
    let mut val: u64 = nla_get_u64(data[IFLA_BR_MCAST_LAST_MEMBER_INTVL]);
    br.multicast_ctx.multicast_last_member_interval = clock_t_to_jiffies(val);
    }
    if (data[IFLA_BR_MCAST_MEMBERSHIP_INTVL]) {
    let mut val: u64 = nla_get_u64(data[IFLA_BR_MCAST_MEMBERSHIP_INTVL]);
    br.multicast_ctx.multicast_membership_interval = clock_t_to_jiffies(val);
    }
    if (data[IFLA_BR_MCAST_QUERIER_INTVL]) {
    let mut val: u64 = nla_get_u64(data[IFLA_BR_MCAST_QUERIER_INTVL]);
    br.multicast_ctx.multicast_querier_interval = clock_t_to_jiffies(val);
    }
    if (data[IFLA_BR_MCAST_QUERY_INTVL]) {
    let mut val: u64 = nla_get_u64(data[IFLA_BR_MCAST_QUERY_INTVL]);
    br_multicast_set_query_intvl(&br.multicast_ctx, val);
    }
    if (data[IFLA_BR_MCAST_QUERY_RESPONSE_INTVL]) {
    let mut val: u64 = nla_get_u64(data[IFLA_BR_MCAST_QUERY_RESPONSE_INTVL]);
    br.multicast_ctx.multicast_query_response_interval = clock_t_to_jiffies(val);
    }
    if (data[IFLA_BR_MCAST_STARTUP_QUERY_INTVL]) {
    let mut val: u64 = nla_get_u64(data[IFLA_BR_MCAST_STARTUP_QUERY_INTVL]);
    ...
    let mut val: u8 = nla_get_u8(data[IFLA_BR_NF_CALL_IPTABLES]);
    br_opt_toggle(br, BROPT_NF_CALL_IPTABLES, !!val);
    }
    if (data[IFLA_BR_NF_CALL_IP6TABLES]) {
    let mut val: u8 = nla_get_u8(data[IFLA_BR_NF_CALL_IP6TABLES]);
    br_opt_toggle(br, BROPT_NF_CALL_IP6TABLES, !!val);
    }
    if (data[IFLA_BR_NF_CALL_ARPTABLES]) {
    let mut val: u8 = nla_get_u8(data[IFLA_BR_NF_CALL_ARPTABLES]);
    ...
    let mut val: u32 = nla_get_u32(data[IFLA_BR_FDB_MAX_LEARNED]);
    ...
    if (err)
    br_dev_delete(dev, core::ptr::null_mut());
    return err;
    }
#[no_mangle]
unsafe extern "C" fn br_get_size(brdev: *const net_device) -> usize {
...
#[no_mangle]
unsafe extern "C" fn br_fill_info(skb: *mut sk_buff, brdev: *const net_device) -> c_int {
    static int br_fill_info(struct sk_buff *skb, const struct net_device *brdev)
    {
    struct net_bridge *br = netdev_priv(brdev);
    let mut forward_delay: u32 = jiffies_to_clock_t(br.forward_delay);
    let mut hello_time: u32 = jiffies_to_clock_t(br.hello_time);
    let mut age_time: u32 = jiffies_to_clock_t(br.max_age);
    let mut ageing_time: u32 = jiffies_to_clock_t(br.ageing_time);
    let mut stp_enabled: u32 = br.stp_enabled;
    let mut priority: u16 = (br.bridge_id.prio[0] << 8) | br.bridge_id.prio[1];
    let mut vlan_enabled: u8 = br_vlan_enabled(br.dev);
    struct br_boolopt_multi bm;
    u64 clockval;
...
#[no_mangle]
unsafe extern "C" fn br_get_linkxstats_size(dev: *const net_device, attr: c_int) -> usize {
...
// we need to count all, even placeholder entries
    ...
    {
    let mut limit: c_uint = U16_MAX - nla_total_size(0);
...
#[no_mangle]
pub unsafe extern "C" fn br_netlink_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn br_netlink_fini() {
