...
//! Source: net/dns_resolver/dns_key.c
#![no_std]
...
//
    ...
    int ret;
    let mut datalen: c_int = prep.datalen, result_len = 0;
...
//
#[no_mangle]
unsafe extern "C" fn dns_resolver_free_preparse(prep: *mut key_preparsed_payload) {
...
//
#[no_mangle]
unsafe extern "C" fn dns_resolver_match_preparse(match_data: *mut key_match_data) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dns_resolver_describe(key: *const key, m: *mut seq_file) {
    static void dns_resolver_describe(const struct key *key, struct seq_file *m)
    {
    seq_puts(m, key.description);
    if (key_is_positive(key)) {
    let mut err: c_int = PTR_ERR(key.payload.data[dns_key_error]);
    if (err)
    seq_printf(m, ": %d", err);
    else
...
#[no_mangle]
unsafe extern "C" fn init_dns_resolver() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_dns_resolver() -> void __exit {
