...
//! Source: net/ieee802154/header_ops.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ieee802154_hdr_addr_len(mode: c_int, omit_pan: bool) -> c_int {
    static int ieee802154_hdr_addr_len(int mode, bool omit_pan)
    {
    let mut pan_len: c_int = omit_pan ? 0 : 2;
    switch (mode) {
...
#[no_mangle]
unsafe extern "C" fn ieee802154_hdr_sechdr_len(sc: u8) -> c_int {
    static int ieee802154_hdr_sechdr_len(u8 sc)
    {
    return ieee802154_sechdr_lengths[IEEE802154_SCF_KEY_ID_MODE(sc)];
    }
#[no_mangle]
unsafe extern "C" fn ieee802154_hdr_minlen(hdr: *const ieee802154_hdr) -> c_int {
    ...
    let mut want: c_int = pos + ieee802154_hdr_sechdr_len(skb.data[pos]);
    ...
    return -EINVAL;
    if (hdr.fc.security_enabled) {
    let mut key_id_mode: u8 = IEEE802154_SCF_KEY_ID_MODE(*(buf + pos));
    let mut want: c_int = pos + ieee802154_sechdr_lengths[key_id_mode];
...
#[no_mangle]
pub unsafe extern "C" fn ieee802154_max_payload(hdr: *const ieee802154_hdr) -> c_int {
    int ieee802154_max_payload(const struct ieee802154_hdr *hdr)
    {
    let mut hlen: c_int = ieee802154_hdr_minlen(hdr);
    if (hdr.fc.security_enabled) {
