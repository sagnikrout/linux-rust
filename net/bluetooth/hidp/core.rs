...
//! Source: net/bluetooth/hidp/core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn hidp_copy_session(session: *mut hidp_session, ci: *mut hidp_conninfo) {
    static void hidp_copy_session(struct hidp_session *session, struct hidp_conninfo *ci)
    {
    let mut valid_flags: u32 = 0;
...
#[no_mangle]
unsafe extern "C" fn hidp_input_report(session: *mut hidp_session, skb: *mut sk_buff) {
...
// too many keys were pressed at the same time.
    ...
    struct sk_buff *skb;
    size_t len;
    let mut numbered_reports: c_int = hid.report_enum[report_type].numbered;
    int ret;
    if (atomic_read(&session.terminate))
    return -EIO;
    switch (report_type) {
...
// signal
    ...
    kfree_skb(skb);
    session.report_return = core::ptr::null_mut();
    } else {
...
#[no_mangle]
unsafe extern "C" fn hidp_output_report(hid: *mut hid_device, data: *mut __u8, count: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hidp_idle_timeout(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn hidp_set_timer(session: *mut hidp_session) {
...
#[no_mangle]
unsafe extern "C" fn hidp_del_timer(session: *mut hidp_session) {
...
// can be recovered.
    ...
    HIDP_TRANS_HID_CONTROL | HIDP_CTRL_SOFT_RESET, core::ptr::null_mut(), 0);
    break;
    default:
    hidp_send_ctrl_message(session,
    HIDP_TRANS_HANDSHAKE | HIDP_HSHK_ERR_INVALID_PARAMETER, core::ptr::null_mut(), 0);
    break;
    }
...
// hidp_get_raw_report() is waiting on this report.
    ...
    default:
    hidp_send_ctrl_message(session,
    HIDP_TRANS_HANDSHAKE | HIDP_HSHK_ERR_UNSUPPORTED_REQUEST, core::ptr::null_mut(), 0);
    break;
    }
    free:
    if (free_skb)
...
#[no_mangle]
unsafe extern "C" fn hidp_send_frame(sock: *mut socket, data: *mut c_uchar, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hidp_open(hid: *mut hid_device) -> c_int {
    static int hidp_open(struct hid_device *hid)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn hidp_close(hid: *mut hid_device) {
    static void hidp_close(struct hid_device *hid)
    {
    }
#[no_mangle]
unsafe extern "C" fn hidp_parse(hid: *mut hid_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hidp_start(hid: *mut hid_device) -> c_int {
    static int hidp_start(struct hid_device *hid)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn hidp_stop(hid: *mut hid_device) {
...
// True if device is blocked in drivers/hid/hid-quirks.c
    ...
    session.hid = core::ptr::null_mut();
    return -ENODEV;
    }
    return 0;
    fault:
    kfree(session.rd_data);
    session.rd_data = core::ptr::null_mut();
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn hidp_session_dev_destroy(session: *mut hidp_session) {
    static void hidp_session_dev_destroy(struct hidp_session *session)
    {
    if (session.hid)
    put_device(&session.hid.dev);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: session->input) -> else {
...
#[no_mangle]
unsafe extern "C" fn hidp_session_dev_add(session: *mut hidp_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hidp_session_dev_del(session: *mut hidp_session) {
...
//
#[no_mangle]
unsafe extern "C" fn hidp_session_dev_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn hidp_session_get(session: *mut hidp_session) {
...
// release callback
#[no_mangle]
unsafe extern "C" fn session_free(ref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn hidp_session_put(session: *mut hidp_session) {
...
//
#[no_mangle]
unsafe extern "C" fn hidp_session_unregister_conn(session: *mut hidp_session) {
    ...
    conn = session.conn;
    if (conn)
    session.conn = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn hidp_session_start_sync(session: *mut hidp_session) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hidp_session_terminate(session: *mut hidp_session) {
...
//
#[no_mangle]
unsafe extern "C" fn hidp_session_run(session: *mut hidp_session) {
...
//
#[no_mangle]
unsafe extern "C" fn hidp_session_thread(arg: *mut c_void) -> c_int {
    static int hidp_session_thread(void *arg)
    {
...
#[no_mangle]
pub unsafe extern "C" fn hidp_connection_del(req: *mut hidp_conndel_req) -> c_int {
    int hidp_connection_del(struct hidp_conndel_req *req)
    {
    let mut valid_flags: u32 = BIT(HIDP_VIRTUAL_CABLE_UNPLUG);
    struct hidp_session *session;
    if (req.flags & ~valid_flags)
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn hidp_get_connlist(req: *mut hidp_connlist_req) -> c_int {
    int hidp_get_connlist(struct hidp_connlist_req *req)
    {
    struct hidp_session *session;
    let mut err: c_int = 0, n = 0;
    BT_DBG("");
    down_read(&hidp_session_sem);
    list_for_each_entry(session, &hidp_session_list, list) {
...
#[no_mangle]
pub unsafe extern "C" fn hidp_get_conninfo(ci: *mut hidp_conninfo) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hidp_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn hidp_exit() -> void __exit {
