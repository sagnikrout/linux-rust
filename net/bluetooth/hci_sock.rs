...
// Bluetooth HCI sockets.
    ...
    let mut monitor_promisc: static atomic_t = ATOMIC_INIT(0);
...
#[no_mangle]
pub unsafe extern "C" fn hci_sock_set_flag(sk: *mut sock, nr: c_int) {
    void hci_sock_set_flag(struct sock *sk, int nr)
    {
    set_bit(nr, &hci_pi(sk).flags);
    }
#[no_mangle]
pub unsafe extern "C" fn hci_sock_clear_flag(sk: *mut sock, nr: c_int) {
    void hci_sock_clear_flag(struct sock *sk, int nr)
    {
    clear_bit(nr, &hci_pi(sk).flags);
    }
#[no_mangle]
pub unsafe extern "C" fn hci_sock_test_flag(sk: *mut sock, nr: c_int) -> c_int {
    int hci_sock_test_flag(struct sock *sk, int nr)
    {
    return test_bit(nr, &hci_pi(sk).flags);
    }
#[no_mangle]
pub unsafe extern "C" fn hci_sock_get_channel(sk: *mut sock) -> c_ushort {
    unsigned short hci_sock_get_channel(struct sock *sk)
    {
    return hci_pi(sk).channel;
    }
#[no_mangle]
pub unsafe extern "C" fn hci_sock_get_cookie(sk: *mut sock) -> u32 {
    u32 hci_sock_get_cookie(struct sock *sk)
    {
    return hci_pi(sk).cookie;
    }
#[no_mangle]
unsafe extern "C" fn hci_sock_gen_cookie(sk: *mut sock) -> bool {
    static bool hci_sock_gen_cookie(struct sock *sk)
    {
    let mut id: c_int = hci_pi(sk).cookie;
    if (!id) {
...
#[no_mangle]
unsafe extern "C" fn hci_sock_free_cookie(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn hci_test_bit(nr: c_int, addr: *const c_void) -> c_int {
    static inline int hci_test_bit(int nr, const void *addr)
    {
    return *((const __u32 *) addr + (nr >> 5)) & ((__u32) 1 << (nr & 31));
    }
// Security filter
pub const HCI_SFLT_MAX_OGF: c_int = 5;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hci_sec_filter {
    pub type_mask: __u32,
    pub event_mask: [__u32; 2],
    pub 1][4]: __u32 ocf_mask[HCI_SFLT_MAX_OGF +,
}
...
#[no_mangle]
unsafe extern "C" fn is_filtered_packet(sk: *mut sock, skb: *mut sk_buff) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn hci_send_to_sock(hdev: *mut hci_dev, skb: *mut sk_buff) {
    void hci_send_to_sock(struct hci_dev *hdev, struct sk_buff *skb)
    {
    struct sock *sk;
    struct sk_buff *skb_copy = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn hci_sock_copy_creds(sk: *mut sock, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn hci_send_to_monitor(hdev: *mut hci_dev, skb: *mut sk_buff) {
...
// Put header before the data
    ...
    HCI_SOCK_TRUSTED, core::ptr::null_mut());
    kfree_skb(skb_copy);
    }
...
#[no_mangle]
unsafe extern "C" fn __printf(_arg: 2, _arg: 3) {
    ...
    va_list args;
    va_start(args, fmt);
    len = vsnprintf(core::ptr::null_mut(), 0, fmt, args);
    va_end(args);
...
#[no_mangle]
unsafe extern "C" fn send_monitor_replay(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: hci_dev_test_flag(hdev, _arg: HCI_SETUP)) -> else {
...
#[no_mangle]
unsafe extern "C" fn send_monitor_control_replay(mon_sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn hci_si_event(hdev: *mut hci_dev, type: c_int, dlen: c_int, data: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn hci_sock_dev_event(hdev: *mut hci_dev, event: c_int) {
...
// Send event to sockets
    ev.event  = event;
    ev.dev_id = hdev.id;
    hci_si_event(core::ptr::null_mut(), HCI_EV_SI_DEVICE, sizeof(ev), &ev);
    }
...
#[no_mangle]
pub unsafe extern "C" fn hci_mgmt_chan_register(c: *mut hci_mgmt_chan) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hci_mgmt_chan_unregister(c: *mut hci_mgmt_chan) {
...
#[no_mangle]
unsafe extern "C" fn hci_sock_release(sock: *mut socket) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hci_sock_reject_list_add(hdev: *mut hci_dev, arg: *mut void __user) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hci_sock_reject_list_del(hdev: *mut hci_dev, arg: *mut void __user) -> c_int {
...
//
    ...
    hci_pi(sk).hdev = core::ptr::null_mut();
    sk.sk_state = BT_OPEN;
    hci_dev_put(hdev);
    }
...
// Default MTU to HCI_MAX_FRAME_SIZE if not set
    ...
    struct sk_buff *skb)
    {
    let mut mask: __u8 = hci_pi(sk).cmsg_mask;
    if (mask & HCI_CMSG_DIR) {
    let mut incoming: c_int = bt_cb(skb).incoming;
...
// Send event to monitor
    ...
    chan.handlers[opcode].func == core::ptr::null_mut()) {
...
//
    ...
    let mut priority: __u8 = skb.data[sizeof(*hdr)];
    let mut ident_len: __u8 = skb.data[sizeof(*hdr) + 1];
...
//
    ...
    hci_send_to_channel(HCI_CHANNEL_MONITOR, skb, HCI_SOCK_TRUSTED, core::ptr::null_mut());
    err = skb.len;
    if (hdev)
    hci_dev_put(hdev);
    return err;
    ...
    struct sk_buff *skb;
    int err;
    let mut flags: c_uint = msg.msg_flags;
    BT_DBG("sock %p sk %p", sock, sk);
    if (flags & MSG_OOB)
    return -EOPNOTSUPP;
...
//
    ...
    let mut opcode: u16 = get_unaligned_le16(skb.data);
    let mut ogf: u16 = hci_opcode_ogf(opcode);
    let mut ocf: u16 = hci_opcode_ocf(opcode);
...
//
    ...
    {
    let mut uf: hci_ufilter = { .opcode = 0 };
    struct sock *sk = sock.sk;
    let mut err: c_int = 0, opt = 0;
...
#[no_mangle]
unsafe extern "C" fn hci_sock_destruct(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn hci_sock_init() -> int __init {
    ...
    err = bt_procfs_init(&init_net, "hci", &hci_sk_list, core::ptr::null_mut());
    if (err < 0) {
...
#[no_mangle]
pub unsafe extern "C" fn hci_sock_cleanup() {
