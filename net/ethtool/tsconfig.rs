...
pub struct tsconfig_reply_data {
    ...
    struct hwtstamp_provider *hwprov = core::ptr::null_mut();
    struct net_device *dev = reply_base.dev;
    let mut cfg: kernel_hwtstamp_config = {};
    int ret;
    ...
    core::ptr::null_mut(), __HWTSTAMP_FLAG_CNT,
    ...
    core::ptr::null_mut(), __HWTSTAMP_TX_CNT,
    ...
    core::ptr::null_mut(), __HWTSTAMP_FILTER_CNT,
...
// _TSCONFIG_HWTSTAMP_PROVIDER
    ...
    &data.hwtst_config.flags, core::ptr::null_mut(),
    __HWTSTAMP_FLAG_CNT,
    ts_flags_names, compact);
    if (ret < 0)
    return ret;
    ...
    &data.hwtst_config.tx_type, core::ptr::null_mut(),
    __HWTSTAMP_TX_CNT,
    ts_tx_type_names, compact);
    if (ret < 0)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn tsconfig_send_reply(dev: *mut net_device, info: *mut genl_info) -> c_int {
...
// Found
    ...
    struct genl_info *info)
    {
    let mut hwtst_config: kernel_hwtstamp_config = {0};
    let mut hwprov_mod: bool = false, config_mod = false;
    struct hwtstamp_provider *hwprov = core::ptr::null_mut();
    ...
    let mut __hwprov_desc: hwtstamp_provider_desc = {.index = -1};
...
// Select exactly one rx filter at a time
    ...
    goto err_free_hwprov;
    if (hwprov_mod) {
    let mut zero_config: kernel_hwtstamp_config = {0};
    struct hwtstamp_provider *__hwprov;
