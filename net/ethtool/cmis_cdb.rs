...
//
#[no_mangle]
pub unsafe extern "C" fn ethtool_cmis_get_max_lpl_size(num_of_byte_octs: u8) -> u32 {
...
pub const CMIS_REVISION_PAGE: c_uint = 0x00;
pub const CMIS_REVISION_OFFSET: c_uint = 0x01;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmis_rev_rpl {
    pub rev: u8,
}

#[no_mangle]
unsafe extern "C" fn cmis_rev_rpl_major(rpl: *mut cmis_rev_rpl) -> u8 {
    static u8 cmis_rev_rpl_major(struct cmis_rev_rpl *rpl)
    {
    return rpl.rev >> 4;
    }
#[no_mangle]
unsafe extern "C" fn cmis_rev_major_get(dev: *mut net_device, rev_major: *mut u8) -> c_int {
    static int cmis_rev_major_get(struct net_device *dev, u8 *rev_major)
    {
    const struct ethtool_ops *ops = dev.ethtool_ops;
    let mut page_data: ethtool_module_eeprom = {0};
    let mut extack: netlink_ext_ack = {};
    let mut rpl: cmis_rev_rpl = {};
    int err;
...
pub const CMIS_CDB_ADVERTISEMENT_PAGE: c_uint = 0x01;
pub const CMIS_CDB_ADVERTISEMENT_OFFSET: c_uint = 0xA3;
// Based on section 8.4.11 "CDB Messaging Support Advertisement" in CMIS
// standard revision 5.2.
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmis_cdb_advert_rpl {
    pub inst_supported: u8,
...
#[no_mangle]
unsafe extern "C" fn cmis_cdb_advert_rpl_inst_supported(rpl: *mut cmis_cdb_advert_rpl) -> u8 {
    ...
    {
    const struct ethtool_ops *ops = dev.ethtool_ops;
    let mut page_data: ethtool_module_eeprom = {};
    let mut rpl: cmis_cdb_advert_rpl = {};
    let mut extack: netlink_ext_ack = {};
    int err;
...
pub const CMIS_PASSWORD_ENTRY_PAGE: c_uint = 0x00;
pub const CMIS_PASSWORD_ENTRY_OFFSET: c_uint = 0x7A;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmis_password_entry_pl {
    pub password: __be32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmis_cdb_query_status_pl {
    pub response_delay: u16,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmis_cdb_query_status_rpl {
    pub length: u8,
    pub status: u8,
}
    ...
    {
    const struct ethtool_ops *ops = dev.ethtool_ops;
    let mut qs_pl: cmis_cdb_query_status_pl = {0};
    let mut page_data: ethtool_module_eeprom = {};
    let mut args: ethtool_cmis_cdb_cmd_args = {};
    let mut pe_pl: cmis_password_entry_pl = {};
    ...
    (u8 *)&qs_pl, sizeof(qs_pl), core::ptr::null_mut(), 0, 0,
...
//
#[no_mangle]
pub unsafe extern "C" fn ethtool_cmis_cdb_check_completion_flag(cmis_rev: u8, flags: *mut u8) {
    void ethtool_cmis_cdb_check_completion_flag(u8 cmis_rev, u8 *flags)
    {
// flags |= cmis_rev >= 5 ? CDB_F_COMPLETION_VALID : 0;
    }
pub const CMIS_CDB_MODULE_FEATURES_RESV_DATA: c_int = 34;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmis_cdb_module_features_rpl {
    pub resv1: [u8; CMIS_CDB_MODULE_FEATURES_RESV_DATA],
    pub max_completion_time: __be16,
}
...
#[no_mangle]
pub unsafe extern "C" fn ethtool_cmis_cdb_fini(cdb: *mut ethtool_cmis_cdb) {
    void ethtool_cmis_cdb_fini(struct ethtool_cmis_cdb *cdb)
    {
    kfree(cdb);
    }
#[no_mangle]
unsafe extern "C" fn is_completed(data: u8) -> bool {
    static bool is_completed(u8 data)
    {
    return !!(data & 0x40);
    }
pub const CMIS_CDB_STATUS_SUCCESS: c_uint = 0x01;
#[no_mangle]
unsafe extern "C" fn status_success(data: u8) -> bool {
    static bool status_success(u8 data)
    {
    let mut data: return = = CMIS_CDB_STATUS_SUCCESS;
    }
pub const CMIS_CDB_STATUS_FAIL: c_uint = 0x40;
#[no_mangle]
unsafe extern "C" fn status_fail(data: u8) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmis_wait_for_cond_rpl {
    pub state: u8,
}
    ...
    u8 *state)
    {
    let mut rpl: cmis_wait_for_cond_rpl = {};
    unsigned long end;
    int err;
    if (!(flags & flag))
    return 0;
...
pub const CMIS_CDB_COMPLETION_FLAG_OFFSET: c_uint = 0x08;
...
//
    ...
    is_completed, core::ptr::null_mut(), &flag);
    if (err < 0)
    args.err_msg = "Completion Flag did not set on time";
    return err;
    }
pub const CMIS_CDB_STATUS_OFFSET: c_uint = 0x25;
#[no_mangle]
unsafe extern "C" fn cmis_cdb_status_fail_msg_get(status: u8, err_msg: *mut c_char) {
...
pub const CMIS_CDB_REPLY_OFFSET: c_uint = 0x86;
    static int cmis_cdb_process_reply(struct net_device *dev,
    struct ethtool_module_eeprom *page_data,
    struct ethtool_cmis_cdb_cmd_args *args)
    {
    let mut rpl_hdr_len: u8 = sizeof(struct ethtool_cmis_cdb_rpl_hdr);
    let mut rpl_exp_len: u8 = args.rpl_exp_len + rpl_hdr_len;
    const struct ethtool_ops *ops = dev.ethtool_ops;
    let mut extack: netlink_ext_ack = {};
...
pub const CMIS_CDB_EPL_PAGE_START: c_uint = 0xA0;
pub const CMIS_CDB_EPL_PAGE_END: c_uint = 0xAF;
pub const CMIS_CDB_EPL_FW_BLOCK_OFFSET_START: c_int = 128;
pub const CMIS_CDB_EPL_FW_BLOCK_OFFSET_END: c_int = 255;
    static int
    ethtool_cmis_cdb_execute_epl_cmd(struct net_device *dev,
    struct ethtool_cmis_cdb_cmd_args *args,
    struct ethtool_module_eeprom *page_data)
    {
    let mut epl_len: u16 = be16_to_cpu(args.req.epl_len);
    let mut bytes_written: u32 = 0;
    u8 page;
    int err;
    for (page = CMIS_CDB_EPL_PAGE_START;
    page <= CMIS_CDB_EPL_PAGE_END && bytes_written < epl_len; page++) {
    let mut offset: u16 = CMIS_CDB_EPL_FW_BLOCK_OFFSET_START;
    while (offset <= CMIS_CDB_EPL_FW_BLOCK_OFFSET_END &&
    bytes_written < epl_len) {
    let mut bytes_left: u32 = epl_len - bytes_written;
...
#[no_mangle]
unsafe extern "C" fn cmis_cdb_calc_checksum(data: *const c_void, size: usize) -> u8 {
...
pub const CMIS_CDB_CMD_ID_OFFSET: c_uint = 0x80;
