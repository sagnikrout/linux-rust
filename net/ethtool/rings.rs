...
pub struct rings_reply_data {
    ...
    let mut supported_ring_params: u32 = data.supported_ring_params;
...
#[no_mangle]
pub unsafe extern "C" fn if(ringparam.rx_mini_max_pending: ringparam.rx_mini_pending >) -> else {
    else if (ringparam.rx_mini_pending > ringparam.rx_mini_max_pending)
    err_attr = tb[ETHTOOL_A_RINGS_RX_MINI];
#[no_mangle]
pub unsafe extern "C" fn if(ringparam.rx_jumbo_max_pending: ringparam.rx_jumbo_pending >) -> else {
    else if (ringparam.rx_jumbo_pending > ringparam.rx_jumbo_max_pending)
    err_attr = tb[ETHTOOL_A_RINGS_RX_JUMBO];
#[no_mangle]
pub unsafe extern "C" fn if(ringparam.tx_max_pending: ringparam.tx_pending >) -> else {
    else if (ringparam.tx_pending > ringparam.tx_max_pending)
    err_attr = tb[ETHTOOL_A_RINGS_TX];
#[no_mangle]
pub unsafe extern "C" fn if(kernel_ringparam.hds_thresh_max: kernel_ringparam.hds_thresh >) -> else {
    else if (kernel_ringparam.hds_thresh > kernel_ringparam.hds_thresh_max)
    err_attr = tb[ETHTOOL_A_RINGS_HDS_THRESH];
    else
    err_attr = core::ptr::null_mut();
    if (err_attr) {
