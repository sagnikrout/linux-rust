...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rss_reply_data {
    pub base: ethnl_reply_data,
    pub has_flow_hash: bool,
    pub no_key_fields: bool,
    pub indir_size: u32,
    pub hkey_size: u32,
    pub hfunc: u32,
    pub input_xfrm: u32,
    ...
    pub hkey: *mut u8,
    pub flow_hash: [c_int; __ETHTOOL_A_FLOW_CNT],
}
...
#[no_mangle]
unsafe extern "C" fn rss_get_data_free(data: *const rss_reply_data) {
...
#[no_mangle]
unsafe extern "C" fn rss_cleanup_data(reply_base: *mut ethnl_reply_data) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rss_nl_dump_ctx {
    pub ifindex: c_ulong,
    pub ctx_idx: c_ulong,
// User wants to only dump contexts from given ifindex
    pub match_ifindex: c_uint,
    pub start_ctx: c_uint,
}
...
#[no_mangle]
pub unsafe extern "C" fn ethnl_rss_dump_start(cb: *mut netlink_callback) -> c_int {
...
// Filtering by context not supported
    ...
    req_info.dev = core::ptr::null_mut();
    }
    return ret;
    }
    static int
    ...
    {
    const struct genl_info *info = genl_info_dump(cb);
    let mut data: rss_reply_data = {};
    let mut req: rss_req_info = {};
    void *ehdr;
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn ethnl_rss_dumpit(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
...
// RSS_NTF
#[no_mangle]
unsafe extern "C" fn ethnl_rss_delete_notify(dev: *mut net_device, rss_context: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn ethtool_rss_notify(dev: *mut net_device, type: u32, rss_context: u32) {
...
// RSS_SET
    ...
    struct rss_req_info *request = RSS_REQINFO(req_info);
    struct nlattr **tb = info.attrs;
    struct nlattr *bad_attr = core::ptr::null_mut();
    u32 input_xfrm;
    if (request.rss_context && !ops.create_rxfh_context)
...
// mod = true;
    ...
    {
    struct rss_req_info *request = RSS_REQINFO(req_info);
    let mut indir_reset: bool = false, indir_mod, xfrm_sym;
    struct ethtool_rxfh_context *ctx = core::ptr::null_mut();
    struct net_device *dev = req_info.dev;
    let mut mod: bool = false, fields_mod = false;
    let mut rxfh: ethtool_rxfh_param = {};
    struct nlattr **tb = info.attrs;
    let mut data: rss_reply_data = {};
    const struct ethtool_ops *ops;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !ops->set_rxfh) -> else {
    else if (!ops.set_rxfh)
    ret = -EOPNOTSUPP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !rxfh.rss_context) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn ethnl_rss_create_doit(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
    int ethnl_rss_create_doit(struct sk_buff *skb, struct genl_info *info)
    {
    let mut indir_dflt: bool = false, mod = false, ntf_fail = false;
    let mut rxfh: ethtool_rxfh_param = {};
    struct ethtool_rxfh_context *ctx;
    struct nlattr **tb = info.attrs;
    let mut data: rss_reply_data = {};
...
// .. copy from Xarray to data.
    ...
    return ret;
    err_remove_ctx:
    if (ops.remove_rxfh_context(dev, ctx, req.rss_context, core::ptr::null_mut()))
...
#[no_mangle]
pub unsafe extern "C" fn ethnl_rss_delete_doit(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
    int ethnl_rss_delete_doit(struct sk_buff *skb, struct genl_info *info)
    {
    struct ethtool_rxfh_context *ctx;
    struct nlattr **tb = info.attrs;
    let mut req: ethnl_req_info = {};
    const struct ethtool_ops *ops;
    struct net_device *dev;
    u32 rss_context;
    int ret;
