...
//! Source: net/ethtool/privflags.c
#![no_std]
...
pub struct privflags_reply_data {
    ...
    pub (*priv_flag_names)[ETH_GSTRING_LEN]: *const c_char,
    pub n_priv_flags: c_uint,
    pub priv_flags: u32,
}
    ...
    unsigned int *count,
#[no_mangle]
pub unsafe extern "C" fn char(_arg: *mut *mut names)[ETH_GSTRING_LEN]) -> const {
...
// count = nflags;
    ...
    let mut all_flags: u32 = ~(u32)0 >> (32 - data.n_priv_flags);
...
#[no_mangle]
unsafe extern "C" fn privflags_cleanup_data(reply_data: *mut ethnl_reply_data) {
...
// PRIVFLAGS_SET
    ...
    {
    const char (*names)[ETH_GSTRING_LEN] = core::ptr::null_mut();
    ...
    if (ret < 0)
    return ret;
    ret = ethnl_get_priv_flags_info(dev, &nflags, compact ? core::ptr::null_mut() : &names);
    if (ret < 0)
    return ret;
