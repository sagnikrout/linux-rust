...
pub struct fec_stat_grp {
    pub ETHTOOL_MAX_LANES]: u64 stats[1 +,
    pub cnt: u8,
    pub corr_bits: } corr, uncorr,,
...
// fec_auto = !!(fec & ETHTOOL_FEC_AUTO);
    ...
    &data.fec_auto);
    ethtool_fec_to_link_modes(fec.active_fec, active_fec_modes, core::ptr::null_mut());
...
//
    ...
    let mut len: c_int = 0;
    int ret;
    ret = ethnl_bitset_size(data.fec_link_modes, core::ptr::null_mut(),
    __ETHTOOL_LINK_MODE_MASK_NBITS,
...
#[no_mangle]
unsafe extern "C" fn fec_put_stats(skb: *mut sk_buff, data: *const fec_reply_data) -> c_int {
    ...
    data.fec_link_modes, core::ptr::null_mut(),
    __ETHTOOL_LINK_MODE_MASK_NBITS,
    link_mode_names, compact);
    if (ret < 0)
    return ret;
