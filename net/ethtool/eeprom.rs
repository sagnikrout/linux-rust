...
pub struct eeprom_reply_data {
    ...
    {
    let mut offset: u32 = request.offset;
    let mut length: u32 = request.length;
    if (request.page)
    ...
    let mut modinfo: ethtool_modinfo = {0};
    let mut eeprom: ethtool_eeprom = {0};
    u8 *data;
    int err;
...
#[no_mangle]
unsafe extern "C" fn eeprom_cleanup_data(reply_base: *mut ethnl_reply_data) {
