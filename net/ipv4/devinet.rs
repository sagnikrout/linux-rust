...
pub const IN4_ADDR_HSIZE_SHIFT: c_int = 8;

#[no_mangle]
unsafe extern "C" fn inet_addr_hash(net: *const net, addr: __be32) -> u32 {
    static u32 inet_addr_hash(const struct net *net, __be32 addr)
    {
    let mut val: u32 = __ipv4_addr_hash(addr, net_hash_mix(net));
    return hash_32(val, IN4_ADDR_HSIZE_SHIFT);
    }
#[no_mangle]
unsafe extern "C" fn inet_hash_insert(net: *mut net, ifa: *mut in_ifaddr) {
    static void inet_hash_insert(struct net *net, struct in_ifaddr *ifa)
    {
    let mut hash: u32 = inet_addr_hash(net, ifa.ifa_local);
    ASSERT_RTNL();
    hlist_add_head_rcu(&ifa.addr_lst, &net.ipv4.inet_addr_lst[hash]);
    }
#[no_mangle]
unsafe extern "C" fn inet_hash_remove(ifa: *mut in_ifaddr) {
...
//
    ...
    {
    struct net_device *result = core::ptr::null_mut();
...
// called under RCU lock
    ...
    {
    let mut hash: u32 = inet_addr_hash(net, addr);
    struct in_ifaddr *ifa;
...
#[no_mangle]
unsafe extern "C" fn devinet_sysctl_register(idev: *mut in_device) -> c_int {
    static int devinet_sysctl_register(struct in_device *idev)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn devinet_sysctl_unregister(idev: *mut in_device) {
    static void devinet_sysctl_unregister(struct in_device *idev)
    {
    }
...
#[no_mangle]
unsafe extern "C" fn inet_rcu_free_ifa(head: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn inet_free_ifa(ifa: *mut in_ifaddr) {
...
#[no_mangle]
unsafe extern "C" fn in_dev_free_rcu(head: *mut rcu_head) {
...
pub unsafe extern "C" fn in_dev_finish_destroy(idev: *mut in_device) {
    ...
    sizeof(in_dev.cnf));
    in_dev.cnf.sysctl = core::ptr::null_mut();
    in_dev.dev = dev;
...
#[no_mangle]
unsafe extern "C" fn inetdev_destroy(in_dev: *mut in_device) {
    ...
    dev = in_dev.dev;
    in_dev.dead = 1;
    RCU_INIT_POINTER(dev.ip_ptr, core::ptr::null_mut());
    ip_mc_destroy_dev(in_dev);
    while ((ifa = rtnl_dereference(in_dev.ifa_list)) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn inet_blackhole_dev_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn inet_addr_onlink(in_dev: *mut in_device, a: __be32, b: __be32) -> c_int {
    ...
    {
    struct in_ifaddr *promote = core::ptr::null_mut();
    struct in_ifaddr *ifa, *ifa1;
    struct in_ifaddr __rcu **last_prim;
    struct in_ifaddr *prev_prom = core::ptr::null_mut();
    let mut do_promote: c_int = IN_DEV_PROMOTE_SECONDARIES(in_dev);
    ASSERT_RTNL();
...
//
    ...
    while ((ifa = rtnl_dereference(*ifap1)) != core::ptr::null_mut()) {
...
//
    ...
    int destroy)
    {
    __inet_del_ifa(in_dev, ifap, destroy, core::ptr::null_mut(), 0);
    }
...
#[no_mangle]
unsafe extern "C" fn inet_insert_ifa(ifa: *mut in_ifaddr) -> c_int {
    ...
    return __inet_insert_ifa(ifa, core::ptr::null_mut(), 0, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn inet_set_ifa(dev: *mut net_device, ifa: *mut in_ifaddr) -> c_int {
...
// Called only from RTNL semaphored context. No locks.
    ...
    (ifa = rtnl_net_dereference(net, *ifap)) != core::ptr::null_mut();
    ifap = &ifa.ifa_next) {
    if (tb[IFA_LOCAL] &&
...
#[no_mangle]
unsafe extern "C" fn check_lifetime(work: *mut work_struct) {
    ...
    let mut change_needed: bool = false;
    rcu_read_lock();
...
// We try to batch several events at once.
    ...
    ifa.ifa_flags |= IFA_F_DEPRECATED;
    rtmsg_ifa(RTM_NEWADDR, ifa, core::ptr::null_mut(), 0);
    }
    }
    rtnl_net_unlock(net);
    }
...
//
    ...
    struct netlink_ext_ack *extack)
    {
    let mut prefered_lft: __u32 = INFINITY_LIFE_TIME;
    let mut valid_lft: __u32 = INFINITY_LIFE_TIME;
    struct net *net = sock_net(skb.sk);
...
//
    ...
    let mut new_metric: u32 = ifa.ifa_rt_priority;
    let mut new_proto: u8 = ifa.ifa_proto;
    inet_free_ifa(ifa);
...
//
#[no_mangle]
unsafe extern "C" fn inet_abc_len(addr: __be32) -> c_int {
    ...
    let mut haddr: __u32 = ntohl(addr);
    if (IN_CLASSA(haddr))
    rc = 8;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IN_CLASSB(haddr)) -> else {
    else if (IN_CLASSB(haddr))
    rc = 16;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IN_CLASSC(haddr)) -> else {
    else if (IN_CLASSC(haddr))
    rc = 24;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IN_CLASSE(haddr)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn devinet_ioctl(net: *mut net, cmd: c_uint, ifr: *mut ifreq) -> c_int {
    int devinet_ioctl(struct net *net, unsigned int cmd, struct ifreq *ifr)
    {
    struct sockaddr_in sin_orig;
    struct sockaddr_in *sin = (struct sockaddr_in *)&ifr.ifr_addr;
    struct in_ifaddr __rcu **ifap = core::ptr::null_mut();
    struct in_device *in_dev;
    struct in_ifaddr *ifa = core::ptr::null_mut();
    struct net_device *dev;
    char *colon;
    let mut ret: c_int = -EFAULT;
    let mut tryaddrmatch: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn here(SMP: I mean) -> One day we will be forced to put shlock {
...
// NETDEV_UP/DOWN/CHANGE could touch a peer dev
    ASSERT_RTNL();
    ret = dev_change_flags(dev, ifr.ifr_flags, core::ptr::null_mut());
    break;
    case SIOCSIFADDR:	/* Set interface address (and family) */
    ret = -EINVAL;
...
//
    ...
    let mut old_mask: __be32 = ifa.ifa_mask;
    inet_del_ifa(in_dev, ifap, 0);
...
#[no_mangle]
pub unsafe extern "C" fn inet_gifconf(dev: *mut net_device, buf: *mut char __user, len: c_int, size: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn inet_select_addr(dev: *const net_device, dst: __be32, scope: c_int) -> __be32 {
    __be32 inet_select_addr(const struct net_device *dev, __be32 dst, int scope)
    {
    const struct in_ifaddr *ifa;
    let mut addr: __be32 = 0;
    let mut localnet_scope: c_uchar = RT_SCOPE_HOST;
    struct in_device *in_dev;
    struct net *net;
    int master_idx;
    rcu_read_lock();
...
//
    ...
    localnet_scope = RT_SCOPE_LINK;
    in_dev_for_each_ifa_rcu(ifa, in_dev) {
    let mut min_scope: c_uchar = min(ifa.ifa_scope, localnet_scope);
    if (!addr &&
...
//
#[no_mangle]
pub unsafe extern "C" fn register_inetaddr_notifier(nb: *mut notifier_block) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn unregister_inetaddr_notifier(nb: *mut notifier_block) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn register_inetaddr_validator_notifier(nb: *mut notifier_block) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn unregister_inetaddr_validator_notifier(nb: *mut notifier_block) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn inetdev_changename(dev: *mut net_device, in_dev: *mut in_device) {
    ...
    ifa.ifa_local, dev,
    ifa.ifa_local, core::ptr::null_mut(),
    dev.dev_addr, core::ptr::null_mut());
    }
    }
...
#[no_mangle]
unsafe extern "C" fn inet_nlmsg_size() -> usize {
...
//
    ...
    let mut tval: c_long = (jiffies - tstamp) / HZ;
    if (preferred > tval)
    preferred -= tval;
    else
    preferred = 0;
...
//
#[no_mangle]
unsafe extern "C" fn inet_base_seq(net: *const net) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn inet_dump_ifaddr(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
    static int inet_dump_ifaddr(struct sk_buff *skb, struct netlink_callback *cb)
    {
    return inet_dump_addr(skb, cb, RTM_NEWADDR);
    }
#[no_mangle]
unsafe extern "C" fn inet_dump_ifmcaddr(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
...
//
    ...
    let mut flush_cache: bool = false;
    int rem;
    if (!in_dev)
    return -EAFNOSUPPORT;
    if (nla_parse_nested_deprecated(tb, IFLA_INET_MAX, nla, core::ptr::null_mut(), core::ptr::null_mut()) < 0)
    return -EINVAL;
    if (tb[IFLA_INET_CONF]) {
    nla_for_each_nested(a, tb[IFLA_INET_CONF], rem) {
    let mut old_value: c_int = ipv4_devconf_get(in_dev, nla_type(a));
    let mut new_value: c_int = nla_get_u32(a);
...
#[no_mangle]
unsafe extern "C" fn inet_netconf_msgsize_devconf(type: c_int) -> c_int {
...
// -EMSGSIZE implies BUG in inet_netconf_msgsize_devconf()
    ...
    rtnl_notify(skb, net, 0, RTNLGRP_IPV4_NETCONF, core::ptr::null_mut(), GFP_KERNEL);
    return;
    errout:
...
#[no_mangle]
unsafe extern "C" fn devinet_copy_dflt_conf(net: *mut net, i: c_int) {
...
#[no_mangle]
unsafe extern "C" fn inet_forward_change(net: *mut net) {
    static void inet_forward_change(struct net *net)
    {
    struct net_device *dev;
    let mut on: c_int = IPV4_DEVCONF_ALL(net, FORWARDING);
...
#[no_mangle]
unsafe extern "C" fn devinet_conf_ifindex(net: *mut net, cnf: *mut ipv4_devconf) -> c_int {
    static int devinet_conf_ifindex(struct net *net, struct ipv4_devconf *cnf)
    {
    if (cnf == net.ipv4.devconf_dflt)
    return NETCONFA_IFINDEX_DEFAULT;
#[no_mangle]
pub unsafe extern "C" fn if(net->ipv4.devconf_all: cnf ==) -> else {
    ...
    {
    let mut old_value: c_int = *(int *)ctl.data;
    let mut ret: c_int = proc_dointvec(ctl, write, buffer, lenp, ppos);
    let mut new_value: c_int = *(int *)ctl.data;
    if (write) {
    struct ipv4_devconf *cnf = ctl.extra1;
    let mut i: c_int = (int *)ctl.data - cnf.data;
...
// valp = val;
    ...
    if (t) {
    cnf.sysctl = core::ptr::null_mut();
    unregister_net_sysctl_table(t.sysctl_header);
    kfree(t);
    }
    inet_netconf_notify_devconf(net, RTM_DELNETCONF, 0, ifindex, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn devinet_sysctl_register(idev: *mut in_device) -> c_int {
    static int devinet_sysctl_register(struct in_device *idev)
    ...
    return -EINVAL;
    err = neigh_sysctl_register(idev.dev, idev.arp_parms, core::ptr::null_mut());
    if (err)
    return err;
...
#[no_mangle]
unsafe extern "C" fn devinet_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn devinet_exit_net(net: *mut net) -> __net_exit void {
...
#[no_mangle]
pub unsafe extern "C" fn devinet_init() -> void __init {
