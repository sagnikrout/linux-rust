...
#[no_mangle]
pub unsafe extern "C" fn tcp_mstamp_refresh(tp: *mut tcp_sock) -> void noinline {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_rbtree_insert(root: *mut rb_root, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn tcp_event_new_data_sent(sk: *mut sock, skb: *mut sk_buff) {
    static void tcp_event_new_data_sent(struct sock *sk, struct sk_buff *skb)
    {
    struct inet_connection_sock *icsk = inet_csk(sk);
    struct tcp_sock *tp = tcp_sk(sk);
    let mut prior_packets: c_uint = tp.packets_out;
    WRITE_ONCE(tp.snd_nxt, TCP_SKB_CB(skb).end_seq);
    __skb_unlink(skb, &sk.sk_write_queue);
    tcp_rbtree_insert(&sk.tcp_rtx_queue, skb);
    if (tp.highest_sack == core::ptr::null_mut())
    tp.highest_sack = skb;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_acceptable_seq(sk: *const sock) -> __u32 {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_advertise_mss(sk: *mut sock) -> __u16 {
    static __u16 tcp_advertise_mss(struct sock *sk)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    const struct dst_entry *dst = __sk_dst_get(sk);
    let mut mss: c_int = tp.advmss;
    if (dst) {
    let mut metric: c_uint = tcp_dst_advmss(dst);
    if (metric < mss) {
    mss = metric;
    tp.advmss = mss;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_cwnd_restart(sk: *mut sock, delta: i32) {
    void tcp_cwnd_restart(struct sock *sk, s32 delta)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut restart_cwnd: u32 = tcp_init_cwnd(tp, __sk_dst_get(sk));
    let mut cwnd: u32 = tcp_snd_cwnd(tp);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_event_ack_sent(sk: *mut sock, rcv_nxt: u32) {
...
//
    ...
    __u32 init_rcv_wnd)
    {
    let mut space: c_uint = (__space < 0 ? 0 : __space);
    let mut window_clamp: u32 = READ_ONCE(*__window_clamp);
...
//
#[no_mangle]
unsafe extern "C" fn tcp_select_window(sk: *mut sock) -> u16 {
    static u16 tcp_select_window(struct sock *sk)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    struct net *net = sock_net(sk);
    let mut old_win: u32 = tp.rcv_wnd;
    u32 cur_win, new_win;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_urg_mode(tp: *const tcp_sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn smc_options_write(ptr: *mut __be32, options: *mut u16) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcp_out_options {
// Following group is cleared in __tcp_transmit_skb()
    struct_group(cleared,
    pub /: *mut *mut u16 mss; / 0 to disable,
    pub /: *mut *mut u8 bpf_opt_len; / length of BPF hdr option,
    pub /: *mut *mut u8 num_sack_blocks; / number of SACK blocks to include,
// Caution: following fields are not cleared in __tcp_transmit_skb()
    pub /: *mut *mut *mut u16 options; / bit field of OPTION_,
    pub /: *mut *mut u8 ws; / window scale, 0 to disable,
    u8 num_accecn_fields:7,	/* number of AccECN fields needed */
    pub /: *mut *mut use_synack_ecn_bytes:1; / Use synack_ecn_bytes or not,
    pub /: *mut *mut *mut __u8 hash_location; / temporary pointer, overloaded,
    pub /: *mut *mut __u32 tsval, tsecr; / need to include OPTION_TS,
    pub /: *mut *mut *mut tcp_fastopen_cookie fastopen_cookie; / Fast open cookie,
    pub mptcp: mptcp_out_options,
}
...
// round up to 4 bytes
    ...
    let mut maclen: u8 = tcp_ao_maclen(key.ao_key);
    if (tcprsk) {
    let mut aolen: u8 = maclen + sizeof(struct tcp_ao_hdr);
...
// ptr++ = htonl((leftover_highbyte << 24) |
    ...
    let mut highbyte: u8 = TCPOPT_NOP;
...
#[no_mangle]
unsafe extern "C" fn tcp_synack_options_combine_saving(opts: *mut tcp_out_options) -> u32 {
...
// How much there's room for combining with the alignment padding?
    ...
    return 2;
#[no_mangle]
pub unsafe extern "C" fn if(OPTION_WSCALE: opts->options &) -> else {
...
//
    ...
    int remaining)
    {
    let mut size: c_int = TCP_ACCECN_MAXSIZE;
    let mut sack_blocks_reduce: c_int = 0;
    int max_combine_saving;
    let mut rem: c_int = remaining;
    int align_size;
...
//
    ...
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut remaining: c_uint = MAX_TCP_OPTION_SPACE;
...
// going out.
    ...
    let mut need: u32 = fastopen.cookie.len;
...
//
    ...
    remaining = bpf_skops_hdr_opt_len(sk, skb, core::ptr::null_mut(), core::ptr::null_mut(), 0, opts,
    remaining);
...
// We always send an MSS option.
    ...
    if (foc != core::ptr::null_mut() && foc.len >= 0) {
    let mut need: u32 = foc.len;
...
//
    if (sk_is_mptcp(sk)) {
    let mut remaining: c_uint = MAX_TCP_OPTION_SPACE - size;
    let mut has_ts: bool = opts.options & OPTION_TS;
    int opt_size;
    ...
    if (tcp_ecn_mode_accecn(tp)) {
    let mut ecn_opt: c_int = READ_ONCE(sock_net(sk).ipv4.sysctl_tcp_ecn_option);
...
pub struct tsq_work {
    ...
    pub /: *mut *mut list_head head; / queue of tcp sockets,
}

    static DEFINE_PER_CPU(struct tsq_work, tsq_work);
#[no_mangle]
unsafe extern "C" fn tcp_tsq_write(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_tsq_handler(sk: *mut sock) {
    ...
    tcp_tsq_write(sk);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !test_and_set_bit(TCP_TSQ_DEFERRED, _arg: &sk->sk_tsq_flags)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_tsq_workfn(work: *mut work_struct) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_release_cb(sk: *mut sock) {
    void tcp_release_cb(struct sock *sk)
    {
    let mut flags: c_ulong = smp_load_acquire(&sk.sk_tsq_flags);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_tsq_work_init() -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_wfree(skb: *mut sk_buff) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_pace_kick(timer: *mut hrtimer) -> enum hrtimer_restart {
    ...
    let mut rate: c_ulong = READ_ONCE(sk.sk_pacing_rate);
// Original sch_fq does not pace first 10 MSS
// Note that tp->data_segs_out overflows after 2^32 packets,
// this is a minor annoyance.
//
    if (rate != ~0UL && rate && tp.data_segs_out >= 10) {
    let mut len_ns: u64 = div64_ul((u64)skb.len * NSEC_PER_SEC, rate);
    let mut credit: u64 = tp.tcp_wstamp_ns - prior_wstamp;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_rate_skb_sent(sk: *mut sock, skb: *mut sk_buff) {
...
//
    if (!tp.packets_out) {
    let mut tstamp_us: u64 = tcp_skb_timestamp_us(skb);
...
#[no_mangle]
unsafe extern "C" fn tcp_v4_send_check(sk: *mut sock, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn tcp_v6_send_check(sk: *mut sock, skb: *mut sk_buff) {
...
//
    ...
    struct sk_buff *oskb = core::ptr::null_mut();
    struct tcp_key key;
    struct tcphdr *th;
    u64 prior_wstamp;
    int err;
...
//
    ...
    tcp_options_write(th, tp, core::ptr::null_mut(), &opts, &key);
    if (tcp_key_is_md5(&key)) {
...
// BPF prog is the last one writing header option
    bpf_skops_write_hdr_opt(sk, skb, core::ptr::null_mut(), core::ptr::null_mut(), 0, &opts);
...
//
#[no_mangle]
unsafe extern "C" fn tcp_queue_skb(sk: *mut sock, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn tcp_set_skb_tso_segs(skb: *mut sk_buff, mss_now: c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_adjust_pcount(sk: *mut sock, skb: *const sk_buff, decr: c_int) {
...
#[no_mangle]
unsafe extern "C" fn tcp_has_tx_tstamp(skb: *const sk_buff) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_fragment_tstamp(skb: *mut sk_buff, skb2: *mut sk_buff) {
    ...
    let mut tsflags: u8 = shinfo.tx_flags & SKBTX_ANY_TSTAMP;
...
#[no_mangle]
unsafe extern "C" fn tcp_skb_fragment_eor(skb: *mut sk_buff, skb2: *mut sk_buff) {
...
//
#[no_mangle]
unsafe extern "C" fn __pskb_trim_head(skb: *mut sk_buff, len: c_int) -> c_int {
    ...
    let mut size: c_int = skb_frag_size(&shinfo.frags[i]);
    if (size <= eat) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_trim_head(sk: *mut sock, skb: *mut sk_buff, len: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __tcp_mtu_to_mss(sk: *mut sock, pmtu: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_mtu_to_mss(sk: *mut sock, pmtu: c_int) -> c_int {
...
// Inverse of above
#[no_mangle]
pub unsafe extern "C" fn tcp_mss_to_mtu(sk: *mut sock, mss: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_mtup_init(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_sync_mss(sk: *mut sock, pmtu: u32) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_current_mss(sk: *mut sock) -> c_uint {
    ...
    if (dst) {
    let mut mtu: u32 = dst_mtu(dst);
    if (mtu != inet_csk(sk).icsk_pmtu_cookie)
    mss_now = tcp_sync_mss(sk, mtu);
    }
    tcp_get_current_key(sk, &key);
    header_len = tcp_established_options(sk, core::ptr::null_mut(), &opts, &key) +
...
// we have to adjust mss_now correspondingly
    ...
    let mut delta: c_int = (int) header_len - tp.tcp_header_len;
    mss_now -= delta;
    }
    return mss_now;
    }
...
//
#[no_mangle]
unsafe extern "C" fn tcp_cwnd_application_limited(sk: *mut sock) {
...
// Limited by application or receiver window.
    let mut init_win: u32 = tcp_init_cwnd(tp, __sk_dst_get(sk));
    let mut win_used: u32 = max(tp.snd_cwnd_used, init_win);
...
#[no_mangle]
unsafe extern "C" fn tcp_cwnd_validate(sk: *mut sock, is_cwnd_limited: bool) {
...
#[no_mangle]
unsafe extern "C" fn tcp_minshall_check(tp: *const tcp_sock) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_tso_segs(sk: *mut sock, mss_now: c_uint) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_cwnd_test(tp: *const tcp_sock) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_init_tso_segs(skb: *mut sk_buff, mss_now: c_uint) -> c_int {
    static int tcp_init_tso_segs(struct sk_buff *skb, unsigned int mss_now)
    {
    let mut tso_segs: c_int = tcp_skb_pcount(skb);
...
//
    ...
    unsigned int mss_now, gfp_t gfp)
    {
    let mut nlen: c_int = skb.len - len;
    struct sk_buff *buff;
    u16 flags;
...
// Middle in queue won't get any more data, full sendable already?
    ...
    if (win_divisor) {
    let mut chunk: u32 = min(tp.snd_wnd, tcp_snd_cwnd(tp) * tp.mss_cache);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_mtu_check_reprobe(sk: *mut sock) {
    ...
    let mut mss: c_int = tcp_current_mss(sk);
...
#[no_mangle]
unsafe extern "C" fn tcp_can_coalesce_send_queue_head(sk: *mut sock, len: c_int) -> bool {
    ...
    int probe_size)
    {
    skb_frag_t *lastfrag = core::ptr::null_mut(), *fragto = skb_shinfo(to).frags;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_mtu_probe(sk: *mut sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tcp_pacing_check(sk: *mut sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_rtx_queue_empty_or_single_skb(sk: *const sock) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_chrono_stop(sk: *mut sock, type: enum tcp_chrono) {
...
#[no_mangle]
pub unsafe extern "C" fn if(tp->chrono_type: type ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_grow_skb(sk: *mut sock, skb: *mut sk_buff, amount: c_int) {
...
//
    ...
    int result;
    let mut is_cwnd_limited: bool = false, is_rwnd_limited = false;
    sent_pkts = 0;
    tcp_mstamp_refresh_inline(tp);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_schedule_loss_probe(sk: *mut sock, advancing_rto: bool) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_send_loss_probe(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_push_one(sk: *mut sock, mss_now: c_uint) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __tcp_select_window(sk: *mut sock) -> u32 {
...
//
    let mut mss: c_int = icsk.icsk_ack.rcv_mss;
    let mut free_space: c_int = tcp_space(sk);
    let mut allowed_space: c_int = tcp_full_space(sk);
...
#[no_mangle]
unsafe extern "C" fn tcp_collapse_retrans(sk: *mut sock, skb: *mut sk_buff) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_can_collapse(sk: *const sock, skb: *const sk_buff) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn __tcp_retransmit_skb(sk: *mut sock, skb: *mut sk_buff, segs: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_retransmit_skb(sk: *mut sock, skb: *mut sk_buff, segs: c_int) -> c_int {
    int tcp_retransmit_skb(struct sock *sk, struct sk_buff *skb, int segs)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut err: c_int = __tcp_retransmit_skb(sk, skb, segs);
    if (err == 0) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_xmit_retransmit_queue(sk: *mut sock) {
    void tcp_xmit_retransmit_queue(struct sock *sk)
    {
    const struct inet_connection_sock *icsk = inet_csk(sk);
    struct sk_buff *skb, *rtx_head, *hole = core::ptr::null_mut();
    struct tcp_sock *tp = tcp_sk(sk);
    let mut rearm_timer: bool = false;
    u32 max_segs;
    int mib_idx;
    if (!tp.packets_out)
    return;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_send_fin(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_send_active_reset(sk: *mut sock, reason: enum sk_rst_reason) {
    void tcp_send_active_reset(struct sock *sk, enum sk_rst_reason reason)
    {
    let mut priority: gfp_t = sk_gfp_mask(sk, GFP_ATOMIC | __GFP_NOWARN);
    struct sk_buff *skb;
...
//
    trace_tcp_send_reset(sk, core::ptr::null_mut(), reason);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_send_synack(sk: *mut sock) -> c_int {
...
//
    ...
    struct tcp_ao_key *ao_key = core::ptr::null_mut();
    let mut keyid: u8 = tcp_rsk(req).ao_keyid;
    let mut rnext: u8 = tcp_rsk(req).ao_rcv_next;
...
// RFC1323: The window in SYN & SYN/ACK segments is never scaled.
    ...
    tcp_options_write(th, core::ptr::null_mut(), tcp_rsk(req), &opts, &key);
...
#[no_mangle]
unsafe extern "C" fn tcp_ca_dst_init(sk: *mut sock, dst: *const dst_entry) {
    static void tcp_ca_dst_init(struct sock *sk, const struct dst_entry *dst)
    {
    struct inet_connection_sock *icsk = inet_csk(sk);
    const struct tcp_congestion_ops *ca;
    let mut ca_key: u32 = dst_metric(dst, RTAX_CC_ALGO);
...
#[no_mangle]
unsafe extern "C" fn tcp_connect_init(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn if(tp->advmss: *mut *mut full_space < (u64)rcv_wnd) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcp_connect_queue_skb(sk: *mut sock, skb: *mut sk_buff) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_send_syn_data(sk: *mut sock, syn: *mut sk_buff) -> c_int {
...
// Sync mss_cache after updating the mss_clamp
    ...
    skb_zcopy_set(syn_data, fo.uarg, core::ptr::null_mut());
    }
// No more data pending in inet_wait_for_connect()
    if (space == fo.size)
    fo.data = core::ptr::null_mut();
    fo.copied = space;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_connect(sk: *mut sock) -> c_int {
    int tcp_connect(struct sock *sk)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    struct sk_buff *buff;
    int err;
    tcp_call_bpf(sk, BPF_SOCK_OPS_TCP_CONNECT_CB, 0, core::ptr::null_mut());
...
//
    ...
    let mut needs_ao: bool = !!tp.af_specific.ao_lookup(sk, sk, -1, -1);
    let mut needs_md5: bool = !!tp.af_specific.md5_lookup(sk, sk);
    struct tcp_ao_info *ao_info;
    ao_info = rcu_dereference_check(tp.ao_info,
    lockdep_sock_is_held(sk));
    if (ao_info) {
...
//
    ...
    md5sig = rcu_replace_pointer(tp.md5sig_info, core::ptr::null_mut(),
    lockdep_sock_is_held(sk));
...
//
    ...
    tcp_finish_connect(sk, core::ptr::null_mut());
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn tcp_delack_max(sk: *const sock) -> u32 {
    u32 tcp_delack_max(const struct sock *sk)
    {
    let mut delack_from_rto_min: u32 = max(tcp_rto_min(sk), 2) - 1;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_send_delayed_ack(sk: *mut sock) {
    void tcp_send_delayed_ack(struct sock *sk)
    {
    struct inet_connection_sock *icsk = inet_csk(sk);
    let mut ato: c_int = icsk.icsk_ack.ato;
    unsigned long timeout;
    if (ato > TCP_DELACK_MIN) {
    const struct tcp_sock *tp = tcp_sk(sk);
    let mut max_ato: c_int = HZ / 2;
...
#[no_mangle]
pub unsafe extern "C" fn __tcp_send_ack(sk: *mut sock, rcv_nxt: u32, flags: u16) {
...
// Send it off, this clears delayed acks for us.
    __tcp_transmit_skb(sk, buff, 0, ( gfp_t)0, rcv_nxt);
    }
    EXPORT_SYMBOL_GPL(__tcp_send_ack);
#[no_mangle]
pub unsafe extern "C" fn tcp_send_ack(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_xmit_probe_skb(sk: *mut sock, urgent: c_int, mib: c_int) -> c_int {
...
//
    ...
    return tcp_transmit_skb(sk, skb, 0, ( gfp_t)0);
    }
// Called from setsockopt( ... TCP_REPAIR )
#[no_mangle]
pub unsafe extern "C" fn tcp_send_window_probe(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_write_wakeup(sk: *mut sock, mib: c_int) -> c_int {
    ...
    int err;
    let mut mss: c_uint = tcp_current_mss(sk);
    let mut seg_size: c_uint = tcp_wnd_end(tp) - TCP_SKB_CB(skb).seq;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_send_probe0(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_rtx_synack(sk: *const sock, req: *mut request_sock) -> c_int {
...
// Paired with WRITE_ONCE() in sock_setsockopt()
    ...
    res = af_ops.send_synack(sk, core::ptr::null_mut(), &fl, req, core::ptr::null_mut(), TCP_SYNACK_RETRANS,
    core::ptr::null_mut());
    if (!res) {
