...
#[no_mangle]
unsafe extern "C" fn __mr_free_table(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn mr_table_free(mrt: *mut mr_table) {
...
// If cache is unresolved, don't try to parse IIF and OIF
    ...
    {
    let mut e: c_uint = 0, s_e = cb.args[1];
    ...
    spinlock_t *lock, struct fib_dump_filter *filter)
    {
    let mut t: c_uint = 0, s_t = cb.args[0];
...
//
    ...
    rcu_read_lock();
    for (mrt = iter(net, core::ptr::null_mut()); mrt; mrt = iter(net, mrt)) {
    if (t < s_t)
    goto next_table;
    ...
    if (err)
    return err;
    for (mrt = mr_iter(net, core::ptr::null_mut()); mrt; mrt = mr_iter(net, mrt)) {
