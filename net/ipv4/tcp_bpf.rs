...
#[no_mangle]
pub unsafe extern "C" fn tcp_eat_skb(sk: *mut sock, skb: *mut sk_buff) {
    ...
    {
    let mut apply: bool = apply_bytes;
...
#[no_mangle]
unsafe extern "C" fn is_next_msg_fin(psock: *mut sk_psock) -> bool {
    ...
    struct tcp_sock *tcp;
    let mut copied_from_self: c_int = 0;
...
//
    if (copied == -EFAULT) {
    let mut is_fin: bool = is_next_msg_fin(psock);
    if (is_fin) {
    copied = 0;
    seq++;
    goto out;
...
#[no_mangle]
unsafe extern "C" fn tcp_bpf_ioctl(sk: *mut sock, cmd: c_int, karg: *mut c_int) -> c_int {
...
// karg += tcp_inq(sk);
    ...
    {
    let mut cork: bool = false, enospc = sk_msg_full(msg), redir_ingress;
...
// Clean up before releasing the sock lock.
    ...
    if (unlikely(ret < 0)) {
    let mut free: c_int = sk_msg_free(sk, msg);
    if (!cork)
// copied -= free;
    }
    if (cork) {
...
#[no_mangle]
unsafe extern "C" fn tcp_bpf_sendmsg(sk: *mut sock, msg: *mut msghdr, size: usize) -> c_int {
    static int tcp_bpf_sendmsg(struct sock *sk, struct msghdr *msg, size_t size)
    {
    struct sk_msg tmp, *msg_tx = core::ptr::null_mut();
    let mut copied: c_int = 0, err = 0, ret = 0;
    struct sk_psock *psock;
    long timeo;
    int flags;
...
#[no_mangle]
unsafe extern "C" fn tcp_bpf_check_v6_needs_rebuild(ops: *mut proto) {
...
#[no_mangle]
unsafe extern "C" fn tcp_bpf_v4_build_proto() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tcp_bpf_assert_proto_ops(ops: *mut proto) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_bpf_update_proto(sk: *mut sock, psock: *mut sk_psock, restore: bool) -> c_int {
    int tcp_bpf_update_proto(struct sock *sk, struct sk_psock *psock, bool restore)
    {
    let mut family: c_int = sk.sk_family == AF_INET6 ? TCP_BPF_IPV6 : TCP_BPF_IPV4;
    let mut config: c_int = psock.progs.msg_parser   ? TCP_BPF_TX   : TCP_BPF_BASE;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_bpf_clone(sk: *const sock, newsk: *mut sock) {
