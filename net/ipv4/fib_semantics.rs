...
//! Source: net/ipv4/fib_semantics.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn rt_fibinfo_free(rtp: *mut rtable __rcu) {
...
#[no_mangle]
unsafe extern "C" fn free_nh_exceptions(nhc: *mut fib_nh_common) {
...
#[no_mangle]
unsafe extern "C" fn rt_fibinfo_free_cpus(rtp: *mut *mut rtable __rcu  __percpu) {
...
#[no_mangle]
pub unsafe extern "C" fn fib_nh_release(net: *mut net, fib_nh: *mut fib_nh) {
...
#[no_mangle]
unsafe extern "C" fn free_fib_info_rcu(head: *mut rcu_head) {
...
#[no_mangle]
pub unsafe extern "C" fn nh_comp(fi: *mut fib_info, ofi: *mut fib_info) -> c_int {
    ...
    fi.fib_scope, ( u32)fi.fib_prefsrc,
    fi.fib_priority);
    if (fi.nh) {
    val ^= fi.nh.id;
    } else {
    ...
    __be32 val)
    {
    let mut hash_bits: c_uint = net.ipv4.fib_info_hash_bits;
    u32 slot;
    slot = hash_32(net_hash_mix(net) ^ ( u32)val, hash_bits);
...
#[no_mangle]
unsafe extern "C" fn fib_info_hash_free(head: *mut hlist_head) {
    static void fib_info_hash_free(struct hlist_head *head)
    {
    kvfree(head);
    }
#[no_mangle]
unsafe extern "C" fn fib_info_hash_grow(net: *mut net) {
    static void fib_info_hash_grow(struct net *net)
    {
    let mut old_size: c_uint = 1 << net.ipv4.fib_info_hash_bits;
...
//
#[no_mangle]
pub unsafe extern "C" fn ip_fib_check_default(gw: __be32, dev: *mut net_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn fib_nlmsg_size(fi: *mut fib_info) -> usize {
    ...
    let mut nhs: c_uint = fib_info_num_path(fi);
// space for nested metrics
    payload += nla_total_size((RTAX_MAX * nla_total_size(4)));
    if (fi.nh)
    payload += nla_total_size(4); /* RTA_NH_ID */
    if (nhs) {
    let mut mpsize: usize = 0;
    unsigned int i;
...
// -EMSGSIZE implies BUG in fib_nlmsg_size()
    ...
    struct neighbour *n;
    let mut state: c_int = NUD_NONE;
    if (likely(nhc.nhc_gw_family == AF_INET))
    n = neigh_lookup(&arp_tbl, &nhc.nhc_gw.ipv4, nhc.nhc_dev);
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: IS_ENABLED(CONFIG_IPV6) && nhc->nhc_gw_family ==) -> else {
...
// last_idx = order;
    ...
    lwt_failure:
    rt_fibinfo_free_cpus(nhc.nhc_pcpu_rth_output);
    nhc.nhc_pcpu_rth_output = core::ptr::null_mut();
    return err;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: cfg->fc_gw_family ==) -> else {
...
// only called when fib_nh is integrated into fib_info
#[no_mangle]
unsafe extern "C" fn fib_rebalance(fi: *mut fib_info) {
...
#[no_mangle]
pub unsafe extern "C" fn fib_metrics_match(cfg: *mut fib_config, fi: *mut fib_info) -> bool {
...
//
    ...
    rcu_read_lock();
    {
    struct fib_table *tbl = core::ptr::null_mut();
    struct flowi4 fl4 = {
    .daddr = nh.fib_nh_gw4,
...
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: nh->fib_nh_gw_family ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn fib_result_prefsrc(net: *mut net, res: *mut fib_result) -> __be32 {
...
#[no_mangle]
unsafe extern "C" fn fib_valid_prefsrc(cfg: *mut fib_config, fib_prefsrc: __be32) -> bool {
    static bool fib_valid_prefsrc(struct fib_config *cfg, __be32 fib_prefsrc)
    {
    if (cfg.fc_type != RTN_LOCAL || !cfg.fc_dst ||
    fib_prefsrc != cfg.fc_dst) {
    let mut tb_id: u32 = cfg.fc_table;
    int rc;
    ...
    {
    int err;
    struct fib_info *fi = core::ptr::null_mut();
    struct nexthop *nh = core::ptr::null_mut();
    struct fib_info *ofi;
    let mut nhs: c_int = 1;
...
#[no_mangle]
unsafe extern "C" fn fib_add_multipath(skb: *mut sk_buff, fi: *mut fib_info) -> c_int {
...
unsafe extern "C" fn fib_add_multipath(skb: *mut sk_buff, fi: *mut fib_info) -> c_int {
    ...
    {
    let mut nhs: c_uint = fib_info_num_path(fri.fi);
    struct fib_info *fi = fri.fi;
    let mut tb_id: u32 = fri.tb_id;
    struct nlmsghdr *nlh;
    struct rtmsg *rtm;
...
//
#[no_mangle]
pub unsafe extern "C" fn fib_sync_down_addr(dev: *mut net_device, local: __be32) -> c_int {
    int fib_sync_down_addr(struct net_device *dev, __be32 local)
    {
    let mut tb_id: c_int = l3mdev_fib_table(dev) ? : RT_TABLE_MAIN;
    ...
    {
    let mut ignore_link_down: bool = ip_ignore_linkdown(nh.fib_nh_dev);
...
//
#[no_mangle]
pub unsafe extern "C" fn fib_nhc_update_mtu(nhc: *mut fib_nh_common, new: u32, orig: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn fib_sync_mtu(dev: *mut net_device, orig_mtu: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn fib_sync_down_dev(dev: *mut net_device, event: c_ulong, force: bool) -> c_int {
    int fib_sync_down_dev(struct net_device *dev, unsigned long event, bool force)
    {
    struct hlist_head *head = fib_nh_head(dev);
    struct fib_info *prev_fi = core::ptr::null_mut();
    let mut scope: c_int = RT_SCOPE_NOWHERE;
    struct fib_nh *nh;
    let mut ret: c_int = 0;
    if (force)
    scope = -1;
...
#[no_mangle]
unsafe extern "C" fn fib_select_default(flp: *const flowi4, res: *mut fib_result) {
    static void fib_select_default(const struct flowi4 *flp, struct fib_result *res)
    {
    struct fib_info *fi = core::ptr::null_mut(), *last_resort = core::ptr::null_mut();
    struct hlist_head *fa_head = res.fa_head;
    struct fib_table *tb = res.table;
    let mut slen: u8 = 32 - res.prefixlen;
    let mut order: c_int = -1, last_idx = -1;
    struct fib_alias *fa, *fa1 = core::ptr::null_mut();
    let mut last_prio: u32 = res.fi.fib_priority;
    let mut last_dscp: dscp_t = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn fib_sync_up(dev: *mut net_device, nh_flags: c_uchar) -> c_int {
    ...
    let mut flags: c_uint = netif_get_flags(dev);
    if (flags & (IFF_RUNNING | IFF_LOWER_UP))
    nh_flags |= RTNH_F_LINKDOWN;
    }
    prev_fi = core::ptr::null_mut();
    head = fib_nh_head(dev);
    ret = 0;
    hlist_for_each_entry(nh, head, nh_hash) {
    struct fib_info *fi = nh.nh_parent;
...
#[no_mangle]
unsafe extern "C" fn fib_good_nh(nh: *const fib_nh) -> bool {
...
//
    ...
    let mut h: c_int = fib_multipath_hash(net, fl4, skb, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn fib4_semantics_init(net: *mut net) -> int __net_init {
    int __net_init fib4_semantics_init(struct net *net)
    {
    let mut hash_bits: c_uint = 4;
...
#[no_mangle]
pub unsafe extern "C" fn fib4_semantics_exit(net: *mut net) -> void __net_exit {
