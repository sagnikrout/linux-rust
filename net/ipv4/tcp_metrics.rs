...
pub struct tcp_fastopen_metrics {
    ...
    pub /: *mut *mut try_exp:2; / Request w/ exp. option (once),
    pub /: *mut *mut unsigned long last_syn_loss; / Last Fast Open SYN loss,
...
pub struct tcp_metrics_block {
    pub tcpm_next: *mut tcp_metrics_block __rcu,
    pub tcpm_net: *mut net,
    pub tcpm_saddr: inetpeer_addr,
    pub tcpm_daddr: inetpeer_addr,
    pub tcpm_stamp: c_ulong,
    pub tcpm_lock: u32,
    pub 1]: u32 tcpm_vals[TCP_METRIC_MAX_KERNEL +,
...
pub struct tcpm_hash_bucket {
    pub chain: *mut tcp_metrics_block __rcu,
}
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_update_metrics(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_init_metrics(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_peer_is_proven(req: *mut request_sock, dst: *mut dst_entry) -> bool {
...
// Add attributes, caller cancels its header on failure
    ...
    let mut val: u32 = tcp_metric_get(tm, i);
    if (!val)
    continue;
    if (i == TCP_METRIC_RTT) {
    ...
    let mut max_rows: c_uint = 1U << tcp_metrics_hash_log;
    unsigned int row, s_row = cb.args[0];
    let mut s_col: c_int = cb.args[1], col = s_col;
    let mut res: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn parse_nl_saddr(info: *mut genl_info, addr: *mut inetpeer_addr) -> c_int {
    static int parse_nl_saddr(struct genl_info *info, struct inetpeer_addr *addr)
    {
    return __parse_nl_addr(info, addr, core::ptr::null_mut(), 0,
    TCP_METRICS_ATTR_SADDR_IPV4,
    TCP_METRICS_ATTR_SADDR_IPV6);
    }
#[no_mangle]
unsafe extern "C" fn tcp_metrics_nl_cmd_get(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tcp_metrics_flush_all(net: *mut net) {
...
#[no_mangle]
unsafe extern "C" fn tcp_metrics_nl_cmd_del(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
    ...
    struct net *net = genl_info_net(info);
    int ret;
    let mut src: bool = true, found = false;
...
#[no_mangle]
unsafe extern "C" fn set_tcpmhash_entries(str: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tcp_metrics_hash_alloc() -> void __init {
    static void __init tcp_metrics_hash_alloc(void)
    {
    let mut slots: c_uint = tcpmhash_entries;
    size_t size;
    if (!slots) {
...
#[no_mangle]
unsafe extern "C" fn tcp_net_metrics_exit_batch(net_exit_list: *mut list_head) -> void __net_exit {
    static void __net_exit tcp_net_metrics_exit_batch(struct list_head *net_exit_list)
    {
    tcp_metrics_flush_all(core::ptr::null_mut());
    }
    static __net_initdata struct pernet_operations tcp_net_metrics_ops = {
    .exit_batch	=	tcp_net_metrics_exit_batch,
    };
#[no_mangle]
pub unsafe extern "C" fn tcp_metrics_init() -> void __init {
