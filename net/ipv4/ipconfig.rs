...
// Macro flag: #define IPCONFIG_DHCP

// Macro flag: #define IPCONFIG_BOOTP

// Macro flag: #define IPCONFIG_RARP

// Macro flag: #define IPCONFIG_DYNAMIC
...
// Wait for carrier timeout default in seconds
    let mut carrier_timeout: static unsigned int = 120;
//
...
pub const ic_proto_used: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn ic_is_init_dev(dev: *mut net_device) -> bool __init {
...
#[no_mangle]
unsafe extern "C" fn ic_open_devs() -> int __init {
...
// bring loopback device up first
    ...
    if (!(dev.flags & IFF_LOOPBACK))
    continue;
    if (dev_change_flags(dev, dev.flags | IFF_UP, core::ptr::null_mut()) < 0)
    pr_err("IP-Config: Failed to open %s\n", dev.name);
    }
    for_each_netdev(&init_net, dev) {
    if (ic_is_init_dev(dev)) {
    let mut able: c_int = 0;
    if (dev.mtu >= 364)
    able |= IC_BOOTP;
    else
    ...
    continue;
    oflags = dev.flags;
    if (dev_change_flags(dev, oflags | IFF_UP, core::ptr::null_mut()) < 0) {
    pr_err("IP-Config: Failed to open %s\n",
    dev.name);
    continue;
    }
...
//
#[no_mangle]
unsafe extern "C" fn ic_close_devs() -> void __init {
    static void __init ic_close_devs(void)
    {
    struct net_device *selected_dev = ic_dev ? ic_dev.dev : core::ptr::null_mut();
    struct ic_device *d, *next;
    struct net_device *dev;
    rtnl_lock();
    next = ic_first_dev;
    while ((d = next)) {
    let mut bring_down: bool = (d != ic_dev);
    ...
    dev_change_flags(dev, d.flags, core::ptr::null_mut());
    }
    kfree(d);
    }
    rtnl_unlock();
...
//
#[no_mangle]
unsafe extern "C" fn ic_setup_if() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ic_setup_routes() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn ic_defaults() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IN_CLASSB(ntohl(ic_myaddr))) -> else {
    else if (IN_CLASSB(ntohl(ic_myaddr)))
    ic_netmask = htonl(IN_CLASSB_NET);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IN_CLASSC(ntohl(ic_myaddr))) -> else {
    else if (IN_CLASSC(ntohl(ic_myaddr)))
    ic_netmask = htonl(IN_CLASSC_NET);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IN_CLASSE(ntohl(ic_myaddr))) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn ic_rarp_init() -> void __init {
    static inline void __init ic_rarp_init(void)
    {
    dev_add_pack(&rarp_packet_type);
    }
#[no_mangle]
pub unsafe extern "C" fn ic_rarp_cleanup() -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn ic_rarp_send_if(d: *mut ic_device) -> void __init {
    static void __init ic_rarp_send_if(struct ic_device *d)
    {
    struct net_device *dev = d.dev;
    arp_send(ARPOP_RREQUEST, ETH_P_RARP, 0, dev, 0, core::ptr::null_mut(),
...
//
#[no_mangle]
pub unsafe extern "C" fn ic_nameservers_predef() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn ic_ntp_servers_predef() -> void __init {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bootp_pkt {
    pub /: *mut *mut iphdr iph; / IP header,
    pub /: *mut *mut udphdr udph; / UDP header,
    pub /: *mut *mut u8 op; / 1=request, 2=reply,
    pub /: *mut *mut u8 htype; / HW address type,
    pub /: *mut *mut u8 hlen; / HW address length,
    pub /: *mut *mut u8 hops; / Used only by gateways,
    pub /: *mut *mut __be32 xid; / Transaction ID,
    pub /: *mut *mut __be16 secs; / Seconds since we started,
    pub /: *mut *mut __be16 flags; / Just what it says,
    pub /: *mut *mut __be32 client_ip; / Client's IP address if known,
    pub /: *mut *mut __be32 your_ip; / Assigned IP address,
    pub /: *mut *mut __be32 server_ip; / (Next, e.g. NFS) Server's IP address,
    pub /: *mut *mut __be32 relay_ip; / IP address of BOOTP relay,
    pub /: *mut *mut u8 hw_addr[16]; / Client's HW address,
    pub /: *mut *mut u8 serv_name[64]; / Server host name,
    pub /: *mut *mut u8 boot_file[128]; / Name of boot file,
    pub /: *mut *mut u8 exten[312]; / DHCP options / BOOTP vendor extensions,
}

// packet ops
pub const BOOTP_REQUEST: c_int = 1;
pub const BOOTP_REPLY: c_int = 2;
// DHCP message types
pub const DHCPDISCOVER: c_int = 1;
pub const DHCPOFFER: c_int = 2;
pub const DHCPREQUEST: c_int = 3;
pub const DHCPDECLINE: c_int = 4;
pub const DHCPACK: c_int = 5;
pub const DHCPNAK: c_int = 6;
pub const DHCPRELEASE: c_int = 7;
pub const DHCPINFORM: c_int = 8;
...
//
    ...
    let mut mt: u8 = (ic_servaddr == NONE) ? DHCPDISCOVER : DHCPREQUEST;
    u8 *e = options;
    int len;
...
#[no_mangle]
unsafe extern "C" fn ic_bootp_init_ext(e: *mut u8) -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn ic_bootp_init() -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn ic_bootp_cleanup() -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn ic_bootp_send_if(d: *mut ic_device, jiffies_diff: c_ulong) -> void __init {
...
// Construct DHCP/BOOTP header
    ...
    b.htype = dev.type;
#[no_mangle]
pub unsafe extern "C" fn if(ARPHRD_FDDI: dev->type ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn ic_bootp_string(dest: *mut c_char, src: *mut c_char, len: c_int, max: c_int) -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn ic_do_bootp_ext(ext: *mut u8) -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn ic_bootp_recv(skb: *mut sk_buff, dev: *mut net_device, pt: *mut packet_type, orig_dev: *mut net_device) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ic_dynamic() -> int __init {
    static int __init ic_dynamic(void)
    {
    int retries;
    struct ic_device *d;
    unsigned long start_jiffies, timeout, jiff;
    let mut do_bootp: c_int = ic_proto_have_if & IC_BOOTP;
    let mut do_rarp: c_int = ic_proto_have_if & IC_RARP;
//
...
// Name servers:
#[no_mangle]
unsafe extern "C" fn pnp_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ipconfig_proc_net_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ntp_servers_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn root_nfs_parse_addr(name: *mut c_char) -> __be32 __init {
    __be32 __init root_nfs_parse_addr(char *name)
    {
    __be32 addr;
    let mut octets: c_int = 0;
    char *cp, *cq;
    cp = cq = name;
    while (octets < 4) {
    while (*cp >= '0' && *cp <= '9')
...
#[no_mangle]
unsafe extern "C" fn wait_for_devices() -> int __init {
    static int __init wait_for_devices(void)
    {
    int i;
    let mut try_init_devs: bool = true;
...
//
#[no_mangle]
unsafe extern "C" fn ip_auto_config() -> int __init {
    static int __init ip_auto_config(void)
    {
    __be32 addr;

    let mut retries: c_int = CONF_OPEN_RETRIES;
...
//
#[no_mangle]
unsafe extern "C" fn ic_proto_name(name: *mut c_char) -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(name, _arg: "dhcp", _arg: 4)) -> else {
    char *client_id;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(name, _arg: "bootp")) -> else {
    ic_proto_enabled &= ~(IC_RARP | IC_USE_DHCP);
    return 1;
    }

#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(name, _arg: "rarp")) -> else {
    ic_proto_enabled &= ~(IC_BOOTP | IC_USE_DHCP);
    return 1;
    }

#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(name, _arg: "both")) -> else {
...
#[no_mangle]
unsafe extern "C" fn ip_auto_config_setup(addrs: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nfsaddrs_config_setup(addrs: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn vendor_class_identifier_setup(addrs: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn set_carrier_timeout(str: *mut c_char) -> int __init {
