...
// otherwise.
//
    let mut __read_mostly: static int nv_pad = 10;
    let mut __read_mostly: static int nv_pad_buffer = 2;
    static int nv_reset_period __read_mostly = 5; /* in seconds */
    let mut __read_mostly: static int nv_min_cwnd = 2;
    ...
    let mut __read_mostly: static int nv_cwnd_growth_rate_neg = 8;
    static int nv_cwnd_growth_rate_pos __read_mostly; /* 0 => fixed like Reno */
    let mut __read_mostly: static int nv_dec_eval_min_calls = 60;
    let mut __read_mostly: static int nv_inc_eval_min_calls = 20;
    let mut __read_mostly: static int nv_ssthresh_eval_min_calls = 30;
    let mut __read_mostly: static int nv_stop_rtt_cnt = 10;
    let mut __read_mostly: static int nv_rtt_min_cnt = 2;
...
#[derive(Copy, Clone)]
pub struct tcpnv {
    pub to: *mut *mut unsigned long nv_min_rtt_reset_jiffies; / when to switch,
// nv_min_rtt_new
    pub factor,: *mut *mut s8 cwnd_growth_factor; / Current cwnd growth,
// < 0 => less than 1 packet/RTT
    pub available8: u8,
    pub available16: u16,
    u8  nv_allow_cwnd_growth:1, /* whether cwnd can grow */
    nv_reset:1,	    /* whether to reset values */
    pub because: *mut *mut nv_catchup:1; / whether we are growing,
// of temporary cwnd decrease
    pub /: *mut *mut u8 nv_eval_call_cnt; / call count since last eval,
    pub is: *mut *mut u8 nv_min_cwnd; / nv won't make a ca decision if cwnd,
...
// the file local nv_min_cwnd
    pub /: *mut *mut u8 nv_rtt_cnt; / RTTs without making ca decision,
    pub /: *mut *mut u32 nv_last_rtt; / last rtt,
    pub /: *mut *mut u32 nv_min_rtt; / active min rtt. Used to determine slope,
    pub /: *mut *mut u32 nv_min_rtt_new; / min rtt for future use,
    pub for: *mut *mut u32 nv_base_rtt; / If non-zero it represents the threshold,
// congestion
    pub is: *mut *mut u32 nv_lower_bound_rtt; / Used in conjunction with nv_base_rtt. It,
// set to 80% of nv_base_rtt. It helps reduce
// unfairness between flows
    pub /: *mut *mut u32 nv_rtt_max_rate; / max rate seen during current RTT,
    pub arrives: *mut *mut u32 nv_rtt_start_seq; / current RTT ends when packet,
// acking beyond nv_rtt_start_seq
    pub is: *mut *mut u32 nv_last_snd_una; / Previous value of tp->snd_una. It,
// used to determine bytes acked since last
// call to bictcp_acked
    pub /: *mut *mut u32 nv_no_cong_cnt; / Consecutive no congestion decisions,
}

pub const NV_MIN_CWND: c_int = 4;
pub const NV_MIN_CWND_GROW: c_int = 2;
pub const NV_TSO_CWND_BOUND: c_int = 80;
#[no_mangle]
pub unsafe extern "C" fn tcpnv_reset(ca: *mut tcpnv, sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcpnv_init(sk: *mut sock) {
...
// RTTs
//
    base_rtt = tcp_call_bpf(sk, BPF_SOCK_OPS_BASE_RTT, 0, core::ptr::null_mut());
    if (base_rtt > 0) {
...
//
#[no_mangle]
pub unsafe extern "C" fn nv_get_bounded_rtt(ca: *mut tcpnv, val: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn if(ca->nv_base_rtt: ca->nv_base_rtt > 0 && val >) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcpnv_cong_avoid(sk: *mut sock, ack: u32, acked: u32) {
...
#[no_mangle]
unsafe extern "C" fn tcpnv_recalc_ssthresh(sk: *mut sock) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn tcpnv_state(sk: *mut sock, new_state: u8) {
...
//
#[no_mangle]
unsafe extern "C" fn tcpnv_acked(sk: *mut sock, sample: *const ack_sample) {
...
#[no_mangle]
unsafe extern "C" fn tcpnv_register() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tcpnv_unregister() -> void __exit {
