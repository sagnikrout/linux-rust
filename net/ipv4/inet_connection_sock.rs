...
//
    ...
    {
    let mut addr_type: c_int = ipv6_addr_type(sk1_rcv_saddr6);
    let mut addr_type2: c_int = sk2_rcv_saddr6 ? ipv6_addr_type(sk2_rcv_saddr6) : IPV6_ADDR_MAPPED;
...
//
#[no_mangle]
pub unsafe extern "C" fn inet_sk_get_local_port_range(sk: *const sock, low: *mut c_int, high: *mut c_int) -> bool {
    bool inet_sk_get_local_port_range(const struct sock *sk, int *low, int *high)
    {
    int lo, hi, sk_lo, sk_hi;
    let mut local_range: bool = false;
    u32 sk_range;
...
//
    ...
    attempt_half = 0;
    if (attempt_half) {
    let mut half: c_int = low + (((high - low) >> 2) << 1);
    if (attempt_half == 1)
    high = half;
    else
    low = half;
...
//
    ...
    struct inet_bind2_bucket *tb2)
    {
    let mut reuse: bool = sk.sk_reuse && sk.sk_state != TCP_LISTEN;
...
//
#[no_mangle]
pub unsafe extern "C" fn inet_csk_get_port(sk: *mut sock, snum: c_ushort) -> c_int {
    int inet_csk_get_port(struct sock *sk, unsigned short snum)
    {
    let mut reuse: bool = sk.sk_reuse && sk.sk_state != TCP_LISTEN;
    let mut found_port: bool = false, check_bind_conflict = true;
    let mut bhash_created: bool = false, bhash2_created = false;
    struct inet_hashinfo *hinfo = tcp_get_hashinfo(sk);
    let mut ret: c_int = -EADDRINUSE, port = snum, l3mdev;
    struct inet_bind_hashbucket *head, *head2;
    struct inet_bind2_bucket *tb2 = core::ptr::null_mut();
    struct inet_bind_bucket *tb = core::ptr::null_mut();
    let mut head2_lock_acquired: bool = false;
    struct net *net = sock_net(sk);
    l3mdev = inet_sk_bound_l3mdev(sk);
    if (!port) {
...
//
#[no_mangle]
unsafe extern "C" fn inet_csk_wait_for_connect(sk: *mut sock, timeo: c_long) -> c_int {
...
// Find already established connection
    ...
    let mut timeo: c_long = sock_rcvtimeo(sk, arg.flags & O_NONBLOCK);
...
#[no_mangle]
pub unsafe extern "C" fn inet_csk_clear_xmit_timers(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn inet_csk_clear_xmit_timers_sync(sk: *mut sock) {
...
// resend = !inet_rsk(req)->acked ||
    ...
    if (!req)
    return core::ptr::null_mut();
    req.rsk_listener = core::ptr::null_mut();
    if (attach_listener) {
    ...
    req.saved_syn = core::ptr::null_mut();
    req.syncookie = 0;
    req.num_timeout = 0;
    req.num_retrans = 0;
    req.sk = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn __reqsk_free(req: *mut request_sock) {
...
#[no_mangle]
unsafe extern "C" fn reqsk_migrate_reset(req: *mut request_sock) {
    static void reqsk_migrate_reset(struct request_sock *req)
    {
    req.saved_syn = core::ptr::null_mut();

    inet_rsk(req).ipv6_opt = core::ptr::null_mut();
    inet_rsk(req).pktopts = core::ptr::null_mut();

    inet_rsk(req).ireq_opt = core::ptr::null_mut();

    }
// return true if req was found in the ehash table
#[no_mangle]
unsafe extern "C" fn reqsk_queue_unlink(req: *mut request_sock) -> bool {
    ...
    bool from_timer)
    {
    let mut unlinked: bool = reqsk_queue_unlink(req);
...
#[no_mangle]
pub unsafe extern "C" fn inet_csk_reqsk_queue_drop(sk: *mut sock, req: *mut request_sock) -> bool {
    bool inet_csk_reqsk_queue_drop(struct sock *sk, struct request_sock *req)
    {
    return __inet_csk_reqsk_queue_drop(sk, req, false);
    }
#[no_mangle]
pub unsafe extern "C" fn inet_csk_reqsk_queue_drop_and_put(sk: *mut sock, req: *mut request_sock) {
...
#[no_mangle]
unsafe extern "C" fn reqsk_timer_handler(t: *mut timer_list) {
    static void reqsk_timer_handler(struct timer_list *t)
    {
    struct request_sock *req = timer_container_of(req, t, rsk_timer);
    struct request_sock *nreq = core::ptr::null_mut(), *oreq = req;
    ...
    struct sock *nsk;
    nsk = reuseport_migrate_sock(sk_listener, req_to_sk(req), core::ptr::null_mut());
    if (!nsk)
    goto drop;
...
//
    ...
    let mut young: c_int = reqsk_queue_len_young(queue) << 1;
    ...
    if (!nreq)
    return;
    if (!inet_ehash_insert(req_to_sk(nreq), req_to_sk(oreq), core::ptr::null_mut())) {
// delete timer
...
#[no_mangle]
unsafe extern "C" fn reqsk_queue_hash_req(req: *mut request_sock) -> bool {
    static bool reqsk_queue_hash_req(struct request_sock *req)
    {
    let mut found_dup_sk: bool = false;
    if (!inet_ehash_insert(req_to_sk(req), core::ptr::null_mut(), &found_dup_sk))
    return false;
...
#[no_mangle]
pub unsafe extern "C" fn inet_csk_reqsk_queue_hash_add(sk: *mut sock, req: *mut request_sock) -> bool {
...
//
    ...
    ireq = inet_rsk(req);
    newicsk.icsk_bind_hash = core::ptr::null_mut();
    newicsk.icsk_bind2_hash = core::ptr::null_mut();
...
// listeners have SOCK_RCU_FREE, not the children
    ...
    inet_sk(newsk).mc_list = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn inet_csk_destroy_sock(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn inet_csk_prepare_for_destroy_sock(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn inet_csk_prepare_forced_close(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn inet_ulp_can_listen(sk: *const sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn reqsk_queue_alloc(queue: *mut request_sock_queue) {
    static void reqsk_queue_alloc(struct request_sock_queue *queue)
    {
    queue.fastopenq.rskq_rst_head = core::ptr::null_mut();
    queue.fastopenq.rskq_rst_tail = core::ptr::null_mut();
    queue.fastopenq.qlen = 0;
    queue.rskq_accept_head = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn inet_csk_listen_start(sk: *mut sock) -> c_int {
...
// tcp_v4_destroy_sock().
//
    RCU_INIT_POINTER(tcp_sk(child).fastopen_rsk, core::ptr::null_mut());
    }
    ...
    if (queue.rskq_accept_head == core::ptr::null_mut())
...
//
#[no_mangle]
pub unsafe extern "C" fn inet_csk_listen_stop(sk: *mut sock) {
...
//
    while ((req = reqsk_queue_remove(queue, sk)) != core::ptr::null_mut()) {
    ...
    sock_hold(child);
    nsk = reuseport_migrate_sock(sk, child, core::ptr::null_mut());
    if (nsk) {
    nreq = inet_reqsk_clone(req, nsk);
    if (nreq) {
    refcount_set(&nreq.rsk_refcnt, 1);
