...
//! Source: net/ipv4/tcp_fastopen.c
#![no_std]
...
//
    ...
    RCU_INIT_POINTER(tcp_sk(sk).fastopen_rsk, core::ptr::null_mut());
...
//
    ...
    if (fastopenq.rskq_rst_head == core::ptr::null_mut())
...
//
    ...
    tcp_fastopen_reset_cipher(net, core::ptr::null_mut(), key, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn tcp_fastopen_ctx_free(head: *mut rcu_head) {
...
pub unsafe extern "C" fn tcp_fastopen_ctx_destroy(net: *mut net) {
    ...
    {
    struct tcp_fastopen_context *ctxt;
    ctxt = unrcu_pointer(xchg(&net.ipv4.tcp_fastopen_ctx, core::ptr::null_mut()));
    if (ctxt)
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_fastopen_add_skb(sk: *mut sock, skb: *mut sk_buff) {
...
// returns 0 - no key match, 1 for primary, 2 for backup
    ...
    {
    let mut search_foc: tcp_fastopen_cookie = { .len = -1 };
    ...
    core::ptr::null_mut(), &own_req, core::ptr::null_mut());
    if (!child)
...
#[no_mangle]
unsafe extern "C" fn tcp_fastopen_queue_check(sk: *mut sock) -> bool {
...
//
    ...
    const struct dst_entry *dst)
    {
    let mut syn_data: bool = TCP_SKB_CB(skb).end_seq != TCP_SKB_CB(skb).seq + 1;
    let mut tcp_fastopen: c_int = READ_ONCE(sock_net(sk).ipv4.sysctl_tcp_fastopen);
    let mut valid_foc: tcp_fastopen_cookie = { .len = -1 };
    struct sock *child;
    let mut ret: c_int = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_fastopen_defer_connect(sk: *mut sock, err: *mut c_int) -> bool {
    bool tcp_fastopen_defer_connect(struct sock *sk, int *err)
    {
    let mut cookie: tcp_fastopen_cookie = { .len = 0 };
    struct tcp_sock *tp = tcp_sk(sk);
    u16 mss;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_fastopen_active_disable(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_fastopen_active_should_disable(sk: *mut sock) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_fastopen_active_disable_ofo_check(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_fastopen_active_detect_blackhole(sk: *mut sock, expired: bool) {
    void tcp_fastopen_active_detect_blackhole(struct sock *sk, bool expired)
    {
    let mut timeouts: u32 = inet_csk(sk).icsk_retransmits;
    struct tcp_sock *tp = tcp_sk(sk);
