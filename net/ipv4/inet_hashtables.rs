...
//! Source: net/ipv4/inet_hashtables.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn inet_init_ehash_secret() {
...
//
#[no_mangle]
unsafe extern "C" fn sk_ehashfn(sk: *const sock) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn sk_is_connect_bind(sk: *const sock) -> bool {
...
// Caller must hold hashbucket lock for this tb with local BH disabled
#[no_mangle]
pub unsafe extern "C" fn inet_bind2_bucket_destroy(cachep: *mut kmem_cache, tb: *mut inet_bind2_bucket) {
...
//
#[no_mangle]
unsafe extern "C" fn __inet_put_port(sk: *mut sock) {
    ...
    inet_csk(sk).icsk_bind_hash = core::ptr::null_mut();
    ...
    inet_csk(sk).icsk_bind2_hash = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn inet_put_port(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn __inet_inherit_port(sk: *const sock, child: *mut sock) -> c_int {
    int __inet_inherit_port(const struct sock *sk, struct sock *child)
    {
    struct inet_hashinfo *table = tcp_get_hashinfo(sk);
    let mut port: c_ushort = inet_sk(child).inet_num;
    struct inet_bind_hashbucket *head, *head2;
    let mut created_inet_bind_bucket: bool = false;
    struct net *net = sock_net(sk);
    let mut update_fastreuse: bool = false;
...
//
    ...
    {
    struct sock *reuse_sk = core::ptr::null_mut();
    u32 phash;
    if (sk.sk_reuseport) {
...
// called with rcu_read_lock() : No refcount taken on the socket
    ...
    struct sock *result = core::ptr::null_mut();
    unsigned int hash2;
...
#[no_mangle]
pub unsafe extern "C" fn sock_gen_put(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn sock_edemux(skb: *mut sk_buff) {
...
// called with local bh disabled
    ...
    let mut daddr: __be32 = inet.inet_rcv_saddr;
    let mut saddr: __be32 = inet.inet_daddr;
    let mut dif: c_int = sk.sk_bound_dev_if;
    struct net *net = sock_net(sk);
    let mut sdif: c_int = l3mdev_master_ifindex_by_index(net, dif);
    INET_ADDR_COOKIE(acookie, saddr, daddr);
    let mut ports: __portpair = INET_COMBINED_PORTS(inet.inet_dport, lport);
    struct inet_ehash_bucket *head = inet_ehash_bucket(hinfo, hash);
    struct inet_timewait_sock *tw = core::ptr::null_mut();
    const struct hlist_nulls_node *node;
    struct sock *sk2;
    spinlock_t *lock;
    if (rcu_lookup) {
...
#[no_mangle]
unsafe extern "C" fn inet_sk_port_offset(sk: *const sock) -> u64 {
...
//
    ...
    {
    let mut ports: __portpair = INET_COMBINED_PORTS(sk.sk_dport, sk.sk_num);
    let mut sdif: c_int = sk.sk_bound_dev_if;
    let mut dif: c_int = sk.sk_bound_dev_if;
    const struct hlist_nulls_node *node;
    struct net *net = sock_net(sk);
    struct sock *esk;
...
//
#[no_mangle]
pub unsafe extern "C" fn inet_ehash_insert(sk: *mut sock, osk: *mut sock, found_dup_sk: *mut bool) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn inet_ehash_nolisten(sk: *mut sock, osk: *mut sock, found_dup_sk: *mut bool) -> bool {
    bool inet_ehash_nolisten(struct sock *sk, struct sock *osk, bool *found_dup_sk)
    {
    let mut ok: bool = inet_ehash_insert(sk, osk, found_dup_sk);
    if (ok) {
    sock_prot_inuse_add(sock_net(sk), sk.sk_prot, 1);
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn inet_hash(sk: *mut sock) -> c_int {
    ...
    local_bh_disable();
    inet_ehash_nolisten(sk, core::ptr::null_mut(), core::ptr::null_mut());
    local_bh_enable();
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn inet_unhash(sk: *mut sock) {
...
// The socket's bhash2 hashbucket spinlock must be held when this is called
    ...
    unsigned short port, int l3mdev, const struct sock *sk)
    {
    struct inet_bind2_bucket *bhash2 = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn inet_update_saddr(sk: *mut sock, saddr: *mut c_void, family: c_int) {
...
#[no_mangle]
unsafe extern "C" fn __inet_bhash2_update_saddr(sk: *mut sock, saddr: *mut c_void, family: c_int, reset: bool) -> c_int {
    static int __inet_bhash2_update_saddr(struct sock *sk, void *saddr, int family, bool reset)
    {
    struct inet_hashinfo *hinfo = tcp_get_hashinfo(sk);
    struct inet_bind_hashbucket *head, *head2;
    struct inet_bind2_bucket *tb2, *new_tb2;
    let mut l3mdev: c_int = inet_sk_bound_l3mdev(sk);
    let mut port: c_int = inet_sk(sk).inet_num;
    struct net *net = sock_net(sk);
    int bhash;
    if (!inet_csk(sk).icsk_bind2_hash) {
// Not bind()ed before.
...
#[no_mangle]
pub unsafe extern "C" fn inet_bhash2_update_saddr(sk: *mut sock, saddr: *mut c_void, family: c_int) -> c_int {
    int inet_bhash2_update_saddr(struct sock *sk, void *saddr, int family)
    {
    return __inet_bhash2_update_saddr(sk, saddr, family, false);
    }
#[no_mangle]
pub unsafe extern "C" fn inet_bhash2_reset_saddr(sk: *mut sock) {
    void inet_bhash2_reset_saddr(struct sock *sk)
    {
    if (!(sk.sk_userlocks & SOCK_BINDADDR_LOCK))
    __inet_bhash2_update_saddr(sk, core::ptr::null_mut(), 0, true);
    }
...
//
    ...
    let mut tb_created: bool = false;
    u32 remaining, offset;
    int ret, i, low, high;
    bool local_ports;
    if (port) {
    local_bh_disable();
    ret = check_established(death_row, sk, port, core::ptr::null_mut(), false,
    hash_port0 + port);
    local_bh_enable();
    return ret;
    }
...
//
    ...
    let mut range: u32 = remaining / step;
    u32 upper_bound;
...
// Head lock still held and bh's disabled
    ...
    inet_ehash_nolisten(sk, (struct sock *)tw, core::ptr::null_mut());
    }
    if (tw)
...
#[no_mangle]
pub unsafe extern "C" fn inet_ehash_locks_alloc(hashinfo: *mut inet_hashinfo) -> c_int {
    int inet_ehash_locks_alloc(struct inet_hashinfo *hashinfo)
    {
    let mut locksz: c_uint = sizeof(spinlock_t);
    unsigned int i, nblocks = 1;
    spinlock_t *ptr = core::ptr::null_mut();
    if (locksz == 0)
    goto set_mask;
...
#[no_mangle]
pub unsafe extern "C" fn inet_pernet_hashinfo_free(hashinfo: *mut inet_hashinfo) {
