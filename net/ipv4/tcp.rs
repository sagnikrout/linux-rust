...
#[no_mangle]
pub unsafe extern "C" fn tcp_enter_memory_pressure(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_leave_memory_pressure(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn secs_to_retrans(seconds: c_int, timeout: c_int, rto_max: c_int) -> u8 {
    static u8 secs_to_retrans(int seconds, int timeout, int rto_max)
    {
    let mut res: u8 = 0;
    if (seconds > 0) {
    let mut period: c_int = timeout;
    res = 1;
...
#[no_mangle]
unsafe extern "C" fn retrans_to_secs(retrans: u8, timeout: c_int, rto_max: c_int) -> c_int {
    static int retrans_to_secs(u8 retrans, int timeout, int rto_max)
    {
    let mut period: c_int = 0;
    if (retrans > 0) {
    period = timeout;
    while (--retrans) {
    timeout <<= 1;
...
#[no_mangle]
unsafe extern "C" fn tcp_compute_delivery_rate(tp: *const tcp_sock) -> u64 {
    static u64 tcp_compute_delivery_rate(const struct tcp_sock *tp)
    {
    let mut rate: u32 = READ_ONCE(tp.rate_delivered);
    let mut intv: u32 = READ_ONCE(tp.rate_interval_us);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_md5_destruct_sock(sk: *mut sock) {
    void tcp_md5_destruct_sock(struct sock *sk)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    if (tp.md5sig_info) {
    tcp_clear_md5_list(sk);
    kfree(rcu_replace_pointer(tp.md5sig_info, core::ptr::null_mut(), 1));
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_init_sock(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_tx_timestamp(sk: *mut sock, sockc: *mut sockcm_cookie) {
    static void tcp_tx_timestamp(struct sock *sk, struct sockcm_cookie *sockc)
    {
    struct sk_buff *skb = tcp_write_queue_tail(sk);
    let mut tsflags: u32 = sockc.tsflags;
    if (unlikely(!skb))
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_stream_memory_free(sk: *const sock, wake: c_int) -> bool {
    bool tcp_stream_memory_free(const struct sock *sk, int wake)
    {
    const struct tcp_sock *tp = tcp_sk(sk);
    let mut notsent_bytes: u32 = READ_ONCE(tp.write_seq) - READ_ONCE(tp.snd_nxt);
    return (notsent_bytes << wake) < tcp_notsent_lowat(tp);
    }
    EXPORT_SYMBOL(tcp_stream_memory_free);
#[no_mangle]
unsafe extern "C" fn tcp_stream_is_readable(sk: *mut sock, target: c_int) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_poll(file: *mut file, sock: *mut socket, wait: *mut poll_table) -> __poll_t {
...
// Connected or passive Fast Open socket?
    ...
    let mut target: c_int = sock_rcvlowat(sk, 0, INT_MAX);
    let mut urg_data: u16 = READ_ONCE(tp.urg_data);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_ioctl(sk: *mut sock, cmd: c_int, karg: *mut c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_mark_push(tp: *mut tcp_sock, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn forced_push(tp: *const tcp_sock) -> bool {
    static inline bool forced_push(const struct tcp_sock *tp)
    {
    return after(tp.write_seq, tp.pushed_seq + (tp.max_window >> 1));
    }
#[no_mangle]
pub unsafe extern "C" fn tcp_skb_entail(sk: *mut sock, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_mark_urg(tp: *mut tcp_sock, flags: c_int) {
...
#[no_mangle]
unsafe extern "C" fn __tcp_splice_read(sk: *mut sock, tss: *mut tcp_splice_state) -> c_int {
...
//
    ...
    break;
    ret = sk_wait_data(sk, &timeo, core::ptr::null_mut());
    if (ret < 0)
    break;
...
//
#[no_mangle]
pub unsafe extern "C" fn sk_forced_mem_schedule(sk: *mut sock, size: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_send_mss(sk: *mut sock, size_goal: *mut c_int, flags: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_remove_empty_skb(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_downgrade_zcopy_pure(sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_wmem_schedule(sk: *mut sock, copy: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_free_fastopen_req(tp: *mut tcp_sock) {
    void tcp_free_fastopen_req(struct tcp_sock *tp)
    {
    if (tp.fastopen_req) {
    kfree(tp.fastopen_req);
    tp.fastopen_req = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn tcp_rate_check_app_limited(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sendmsg_locked(sk: *mut sock, msg: *mut msghdr, size: usize) -> c_int {
    int tcp_sendmsg_locked(struct sock *sk, struct msghdr *msg, size_t size)
    {
    struct net_devmem_dmabuf_binding *binding = core::ptr::null_mut();
    struct tcp_sock *tp = tcp_sk(sk);
    struct ubuf_info *uarg = core::ptr::null_mut();
    struct sk_buff *skb;
    struct sockcm_cookie sockc;
    int flags, err, copied = 0;
    let mut mss_now: c_int = 0, size_goal, copied_syn = 0;
    let mut process_backlog: c_int = 0;
    let mut sockc_err: c_int = 0;
    let mut zc: c_int = 0;
    long timeo;
    flags = msg.msg_flags;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sendmsg(sk: *mut sock, msg: *mut msghdr, size: usize) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_splice_eof(sock: *mut socket) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_recv_urg(sk: *mut sock, msg: *mut msghdr, len: c_int, flags: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tcp_peek_sndq(sk: *mut sock, msg: *mut msghdr, len: c_int) -> c_int {
    static int tcp_peek_sndq(struct sock *sk, struct msghdr *msg, int len)
    {
    struct sk_buff *skb;
    let mut copied: c_int = 0, err = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn __tcp_cleanup_rbuf(sk: *mut sock, copied: c_int) {
    void __tcp_cleanup_rbuf(struct sock *sk, int copied)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut time_to_ack: bool = false;
...
//
    ...
    let mut rcv_window_now: __u32 = tcp_receive_window(tp);
// Optimize, __tcp_select_window() is not cheap.
    if (2*rcv_window_now <= tp.window_clamp) {
    let mut new_window: __u32 = __tcp_select_window(sk);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_cleanup_rbuf(sk: *mut sock, copied: c_int) {
...
#[no_mangle]
unsafe extern "C" fn tcp_eat_recv_skb(sk: *mut sock, skb: *mut sk_buff) {
    ...
    struct sk_buff *skb;
    u32 offset;
    while ((skb = skb_peek(&sk.sk_receive_queue)) != core::ptr::null_mut()) {
...
//
    ...
    struct sk_buff *skb;
    struct tcp_sock *tp = tcp_sk(sk);
    let mut seq: u32 = *copied_seq;
    u32 offset;
    let mut copied: c_int = 0;
    if (sk.sk_state == TCP_LISTEN)
    return -ENOTCONN;
    while ((skb = tcp_recv_skb(sk, seq, &offset)) != core::ptr::null_mut()) {
    if (offset < skb.len) {
    int used;
    size_t len;
    len = skb.len - offset;
// Stop reading if we hit a patch of urgent data
    ...
    let mut urg_offset: u32 = tp.urg_seq - seq;
    if (urg_offset < len)
    len = urg_offset;
    if (!len)
    break;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_read_skb(sk: *mut sock, recv_actor: skb_read_actor_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_read_done(sk: *mut sock, len: usize) {
    void tcp_read_done(struct sock *sk, size_t len)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut seq: u32 = tp.copied_seq;
    struct sk_buff *skb;
    size_t left;
    u32 offset;
    if (sk.sk_state == TCP_LISTEN)
    return;
    left = len;
    while (left && (skb = tcp_recv_skb(sk, seq, &offset)) != core::ptr::null_mut()) {
    int used;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_peek_len(sock: *mut socket) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_set_rcvlowat(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_set_rcvbuf(sk: *mut sock, val: c_int) {
...
#[no_mangle]
unsafe extern "C" fn can_map_frag(frag: *const skb_frag_t) -> bool {
...
//
    ...
    {
    let mut copy_address: c_ulong = (unsigned long)zc.copybuf_address;
    let mut msg: msghdr = {};
    int err;
    zc.length = 0;
    zc.recv_skip_hint = 0;
...
// length + /* Mapped or pending
    ...
    if (!err) {
    let mut leftover_pages: c_ulong = pages_remaining;
    int bytes_mapped;
...
//
    let mut bytes_not_mapped: c_int = PAGE_SIZE * pages_remaining;
// length -= bytes_not_mapped;
    ...
    u32 total_bytes_to_map)
    {
    let mut pages_remaining: c_ulong = pages_to_map;
...
pub const TCP_ZEROCOPY_PAGE_BATCH_SIZE: c_int = 32;
    static int tcp_zerocopy_receive(struct sock *sk,
    struct tcp_zerocopy_receive *zc,
    struct scm_timestamping_internal *tss)
    {
    let mut length: u32 = 0, offset, vma_len, avail_len, copylen = 0;
    let mut address: c_ulong = (unsigned long)zc.address;
    struct page *pages[TCP_ZEROCOPY_PAGE_BATCH_SIZE];
    let mut copybuf_len: i32 = zc.copybuf_len;
    struct tcp_sock *tp = tcp_sk(sk);
    const skb_frag_t *frags = core::ptr::null_mut();
    let mut pages_to_map: c_uint = 0;
    struct vm_area_struct *vma;
    struct sk_buff *skb = core::ptr::null_mut();
    let mut seq: u32 = tp.copied_seq;
    u32 total_bytes_to_map;
    let mut inq: c_int = tcp_inq(sk);
    bool mmap_locked;
    int ret;
    zc.copybuf_len = 0;
    zc.msg_flags = 0;
...
// Similar to __sock_recv_timestamp, but does not require an skb
    ...
    struct scm_timestamping_internal *tss)
    {
    let mut new_tstamp: c_int = sock_flag(sk, SOCK_TSTAMP_NEW);
    let mut tsflags: u32 = READ_ONCE(sk.sk_tsflags);
    if (tss.ts[0]) {
    if (sock_flag(sk, SOCK_RCVTSTAMP)) {
    let mut tv: timespec64 = ktime_to_timespec64(tss.ts[0]);
...
#[no_mangle]
unsafe extern "C" fn tcp_inq_hint(sk: *mut sock) -> c_int {
    static int tcp_inq_hint(struct sock *sk)
    {
    const struct tcp_sock *tp = tcp_sk(sk);
    let mut copied_seq: u32 = READ_ONCE(tp.copied_seq);
    let mut rcv_nxt: u32 = READ_ONCE(tp.rcv_nxt);
    int inq;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tcp_xa_pool {
    pub /: *mut *mut u8 max; / max <= MAX_SKB_FRAGS,
    pub /: *mut *mut u8 idx; / idx <= max,
    pub tokens: [__u32; MAX_SKB_FRAGS],
    pub netmems: [netmem_ref; MAX_SKB_FRAGS],
}

#[no_mangle]
unsafe extern "C" fn tcp_xa_pool_commit_locked(sk: *mut sock, p: *mut tcp_xa_pool) {
...
#[no_mangle]
unsafe extern "C" fn tcp_xa_pool_commit(sk: *mut sock, p: *mut tcp_xa_pool) {
...
//
    ...
    int remaining_len)
    {
    let mut dmabuf_cmsg: dmabuf_cmsg = { 0 };
...
//
    ...
    let mut peek_offset: u32 = 0;
    let mut urg_hole: u32 = 0;
    err = -ENOTCONN;
    if (sk.sk_state == TCP_LISTEN)
    goto out;
    if (tp.recvmsg_inq)
...
#[no_mangle]
pub unsafe extern "C" fn tcp_recvmsg(sk: *mut sock, msg: *mut msghdr, len: usize, flags: c_int) -> c_int {
    int tcp_recvmsg(struct sock *sk, struct msghdr *msg, size_t len, int flags)
    {
    let mut cmsg_flags: c_int = 0, ret;
    struct scm_timestamping_internal tss;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_set_state(sk: *mut sock, state: c_int) {
    void tcp_set_state(struct sock *sk, int state)
    {
    let mut oldstate: c_int = sk.sk_state;
...
#[no_mangle]
unsafe extern "C" fn tcp_close_state(sk: *mut sock) -> c_int {
    static int tcp_close_state(struct sock *sk)
    {
    let mut next: c_int = (int)new_state[sk.sk_state];
    let mut ns: c_int = next & TCP_STATE_MASK;
    tcp_set_state(sk, ns);
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_shutdown(sk: *mut sock, how: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_orphan_count_sum() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tcp_orphan_update(unused: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn tcp_too_many_orphans(shift: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_out_of_memory(sk: *const sock) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_check_oom(sk: *const sock, shift: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn __tcp_close(sk: *mut sock, timeout: c_long) {
    void __tcp_close(struct sock *sk, long timeout)
    {
    let mut data_was_unread: bool = false;
    struct sk_buff *skb;
    int state;
...
//
    ...
    let mut tmo: c_int = tcp_fin_time(sk);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_close(sk: *mut sock, timeout: c_long) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_need_reset(state: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_rtx_queue_purge(sk: *mut sock) {
    static void tcp_rtx_queue_purge(struct sock *sk)
    {
    struct rb_node *p = rb_first(&sk.tcp_rtx_queue);
    tcp_sk(sk).highest_sack = core::ptr::null_mut();
    while (p) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_write_queue_purge(sk: *mut sock) {
    void tcp_write_queue_purge(struct sock *sk)
    {
    struct sk_buff *skb;
    tcp_chrono_stop(sk, TCP_CHRONO_BUSY);
    while ((skb = __skb_dequeue(&sk.sk_write_queue)) != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_disconnect(sk: *mut sock, flags: c_int) -> c_int {
    int tcp_disconnect(struct sock *sk, int flags)
    {
    struct inet_sock *inet = inet_sk(sk);
    struct inet_connection_sock *icsk = inet_csk(sk);
    struct tcp_sock *tp = tcp_sk(sk);
    let mut old_state: c_int = sk.sk_state;
    struct request_sock *req;
    u32 seq;
...
//
    ...
    __sk_dst_reset(sk);
    dst_release(unrcu_pointer(xchg(&sk.sk_rx_dst, core::ptr::null_mut())));
...
#[no_mangle]
pub unsafe extern "C" fn tcp_can_repair_sock(sk: *const sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_repair_set_window(tp: *mut tcp_sock, optbuf: sockptr_t, len: c_int) -> c_int {
    ...
    case TCPOPT_WINDOW:
    {
    let mut snd_wscale: u16 = opt.opt_val & 0xFFFF;
    let mut rcv_wscale: u16 = opt.opt_val >> 16;
...
#[no_mangle]
unsafe extern "C" fn tcp_enable_tx_delay(sk: *mut sock, val: c_int) {
    static void tcp_enable_tx_delay(struct sock *sk, int val)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut delta: i32 = (val - tp.tcp_tx_delay) << 3;
    if (val && !static_branch_unlikely(&tcp_tx_delay_enabled)) {
    let mut __tcp_tx_delay_enabled: static int = 0;
...
//
    ...
    let mut srtt: i64 = (s64)tp.srtt_us + delta;
...
//
#[no_mangle]
pub unsafe extern "C" fn __tcp_sock_set_cork(sk: *mut sock, on: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_cork(sk: *mut sock, on: bool) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __tcp_sock_set_nodelay(sk: *mut sock, on: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_nodelay(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn __tcp_sock_set_quickack(sk: *mut sock, val: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_quickack(sk: *mut sock, val: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_syncnt(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_user_timeout(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_keepidle_locked(sk: *mut sock, val: c_int) -> c_int {
...
// Paired with WRITE_ONCE() in keepalive_time_when()
    ...
    let mut elapsed: u32 = keepalive_time_elapsed(tp);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_keepidle(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_keepintvl(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_keepcnt(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_set_window_clamp(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sock_set_maxseg(sk: *mut sock, val: c_int) -> c_int {
...
// These are data/string values, all the others are ints
    ...
    __u8 *backup_key = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(HZ: val > TCP_FIN_TIMEOUT_MAX /) -> else {
...
// Translate value in seconds to number of retransmits
    ...
    return 0;
    case TCP_RTO_MIN_US: {
    let mut rto_min: c_int = usecs_to_jiffies(val);
    ...
    let mut delack_max: c_int = usecs_to_jiffies(val);
    ...
    err = -EPERM;
#[no_mangle]
pub unsafe extern "C" fn if(TCP_REPAIR_ON: val ==) -> else {
    tp.repair = 1;
    ...
    err = -EPERM;
#[no_mangle]
pub unsafe extern "C" fn if(TCP_QUEUES_NR: (unsigned int)val <) -> else {
    ...
    err = -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(!tp->bytes_sent: sk->sk_state == TCP_ESTABLISHED &&) -> else {
...
//
    ...
    err = -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(TCPF_LISTEN)): !((1 << sk->sk_state) & (TCPF_CLOSE |) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_get_info(sk: *mut sock, info: *mut tcp_info) {
    void tcp_get_info(struct sock *sk, struct tcp_info *info)
    {
    const struct tcp_sock *tp = tcp_sk(sk); /* iff sk_type == SOCK_STREAM */
    const struct inet_connection_sock *icsk = inet_csk(sk);
    let mut ect1_idx: u8 = INET_ECN_ECT_1 - 1;
    let mut ect0_idx: u8 = INET_ECN_ECT_0 - 1;
    let mut ce_idx: u8 = INET_ECN_CE - 1;
    unsigned long rate;
    u32 now;
    u64 rate64;
    bool slow;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tcp_ecn_mode_rfc3168(tp)) -> else {
    else if (tcp_ecn_mode_rfc3168(tp))
    info.tcpi_ecn_mode = TCPI_ECN_MODE_RFC3168;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tcp_ecn_mode_accecn(tp)) -> else {
    else if (tcp_ecn_mode_accecn(tp))
    info.tcpi_ecn_mode = TCPI_ECN_MODE_ACCECN;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tcp_ecn_mode_pending(tp)) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcp_opt_stats_get_size() -> usize {
...
#[no_mangle]
unsafe extern "C" fn tcp_skb_ttl_or_hop_limit(skb: *const sk_buff) -> u8 {
...
pub unsafe extern "C" fn if(htons(ETH_P_IPV6): skb->protocol ==) -> else {
    ...
    val = tp.write_seq;
#[no_mangle]
pub unsafe extern "C" fn if(TCP_RECV_QUEUE: tp->repair_queue ==) -> else {
    else if (tp.repair_queue == TCP_RECV_QUEUE)
    val = tp.rcv_nxt;
    else
    return -EINVAL;
    ...
    struct scm_timestamping_internal tss;
    let mut zc: tcp_zerocopy_receive = {};
    int err;
    if (copy_from_sockptr(&len, optlen, sizeof(int)))
    return -EFAULT;
    if (len < 0 ||
    ...
    len - sizeof(zc));
    if (err < 1)
    let mut err: return = = 0 ? -EINVAL : err;
    len = sizeof(zc);
    if (copy_to_sockptr(optlen, &len, sizeof(int)))
    return -EFAULT;
    }
...
#[no_mangle]
pub unsafe extern "C" fn tcp_bpf_bypass_getsockopt(level: c_int, optname: c_int) -> bool {
...
//
    if (family == AF_INET)
    tcp_v4_md5_hash_skb(newhash, key, core::ptr::null_mut(), skb);
    else
    tp.af_specific.calc_md5_hash(newhash, key, core::ptr::null_mut(), skb);
...
//
    ...
    {
    let mut length: c_int = (th.doff << 2) - sizeof(*th);
    const u8 *ptr = (const u8 *)(th + 1);
    let mut minlen: c_uint = TCPOLEN_MD5SIG;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_done(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_abort(sk: *mut sock, err: c_int) -> c_int {
    int tcp_abort(struct sock *sk, int err)
    {
    let mut state: c_int = inet_sk_state_load(sk);
...
#[no_mangle]
unsafe extern "C" fn set_thash_entries(str: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tcp_init_mem() -> void __init {
    static void __init tcp_init_mem(void)
    {
    let mut limit: c_ulong = nr_free_buffer_pages() / 16;
...
#[no_mangle]
unsafe extern "C" fn tcp_struct_check() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_init() -> void __init {
