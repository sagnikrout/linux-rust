...
pub const HYSTART_ACK_TRAIN: c_int = 1;
pub const HYSTART_DELAY: c_int = 2;
    let mut __read_mostly: static int window = 8;
    static unsigned int backoff_beta __read_mostly = 0.7071 * 1024; /* sqrt 0.5 */
    let mut __read_mostly: static unsigned int backoff_factor = 42;
    let mut __read_mostly: static unsigned int hystart_detect = 3;
    let mut __read_mostly: static unsigned int use_ineff = 5;
    let mut __read_mostly: static bool use_shadow = true;
...
//
#[no_mangle]
unsafe extern "C" fn nexp_u32(ux: u32) -> u32 __pure {
...
// Obtain e^(x + y + ...) by computing e^x * e^y * ...:
    for (i = 1; msb; i++, msb >>= 1) {
    let mut y: u32 = v[i & -(msb & 1)] + U32_C(1);
    res = ((u64)res * y) >> 16;
    }
    return res;
    }
...
//
#[no_mangle]
unsafe extern "C" fn tcp_cdg_hystart_update(sk: *mut sock) {
    ...
    let mut now_us: u32 = tp.tcp_mstamp;
    if (ca.last_ack == 0 || !tcp_is_cwnd_limited(sk)) {
    ca.last_ack = now_us;
    ca.round_start = now_us;
    } else if (before(now_us, ca.last_ack + 3000)) {
    let mut base_owd: u32 = max(ca.delay_min / 2U, 125U);
    ca.last_ack = now_us;
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_grad(ca: *mut cdg) -> i32 {
    static s32 tcp_cdg_grad(struct cdg *ca)
    {
    let mut gmin: i32 = ca.rtt.min - ca.rtt_prev.min;
    let mut gmax: i32 = ca.rtt.max - ca.rtt_prev.max;
    s32 grad;
    if (ca.gradients) {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: ca->tail ==) -> else {
    else if (ca.tail == 0)
    ca.gfilled = true;
    else
...
// Reduce small variations to zero:
    ...
    ca.state = CDG_FULL;
#[no_mangle]
pub unsafe extern "C" fn if(0: (gmin > 0 && gmax > 0) || gmax <) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_backoff(sk: *mut sock, grad: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_cong_avoid(sk: *mut sock, ack: u32, acked: u32) {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_acked(sk: *mut sock, sample: *const ack_sample) {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_ssthresh(sk: *mut sock) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_cwnd_event(sk: *mut sock, ev: enum tcp_ca_event) {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_init(sk: *mut sock) {
    static void tcp_cdg_init(struct sock *sk)
    {
    struct cdg *ca = inet_csk_ca(sk);
    struct tcp_sock *tp = tcp_sk(sk);
    ca.gradients = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_release(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_register() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tcp_cdg_unregister() -> void __exit {
