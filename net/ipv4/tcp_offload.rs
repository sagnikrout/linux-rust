...
//
    ...
    {
    let mut flags: u32 = skb_shinfo(gso_skb).tx_flags & SKBTX_ANY_TSTAMP;
    let mut ts_seq: u32 = skb_shinfo(gso_skb).tskey;
    while (skb) {
...
//
    ...
    delta = ( __wsum)htonl(oldlen + thlen + mss);
    skb = segs;
    th = tcp_hdr(skb);
    seq = ntohl(th.seq);
...
//
    ...
    struct tcphdr *th)
    {
    let mut thlen: c_uint = th.doff * 4;
    struct sk_buff *pp = core::ptr::null_mut();
    struct sk_buff *p;
    struct tcphdr *th2;
    unsigned int len;
    __be32 flags;
    let mut mss: c_uint = 1;
    ...
    th2 = tcp_hdr(p);
    flush = ( int)((flags ^ tcp_flag_word(th2)) &
    ~(TCP_FLAG_FIN | TCP_FLAG_PSH));
    flush |= ( int)(th.ack_seq ^ th2.ack_seq);
...
// Force a flush if last segment is smaller than mss.
    ...
    flush = len < mss;
    flush |= ( int)(flags & (TCP_FLAG_URG | TCP_FLAG_PSH |
...
#[no_mangle]
pub unsafe extern "C" fn tcp_gro_complete(skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp4_gro_complete(skb: *mut sk_buff, thoff: c_int) -> INDIRECT_CALLABLE_SCOPE int {
...
#[no_mangle]
pub unsafe extern "C" fn tcpv4_offload_init() -> int __init {
