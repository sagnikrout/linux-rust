...
//
#[no_mangle]
unsafe extern "C" fn ip_cmsg_recv_pktinfo(msg: *mut msghdr, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn ip_cmsg_recv_ttl(msg: *mut msghdr, skb: *mut sk_buff) {
    static void ip_cmsg_recv_ttl(struct msghdr *msg, struct sk_buff *skb)
    {
    let mut ttl: c_int = ip_hdr(skb).ttl;
    put_cmsg(msg, SOL_IP, IP_TTL, sizeof(int), &ttl);
    }
#[no_mangle]
unsafe extern "C" fn ip_cmsg_recv_tos(msg: *mut msghdr, skb: *mut sk_buff) {
    static void ip_cmsg_recv_tos(struct msghdr *msg, struct sk_buff *skb)
    {
    put_cmsg(msg, SOL_IP, IP_TOS, 1, &ip_hdr(skb).tos);
    }
#[no_mangle]
unsafe extern "C" fn ip_cmsg_recv_opts(msg: *mut msghdr, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn ip_cmsg_recv_fragsize(msg: *mut msghdr, skb: *mut sk_buff) {
    ...
    return;
    if (offset != 0) {
    let mut tend_off: c_int = skb_transport_offset(skb) + tlen;
    csum = csum_sub(csum, skb_checksum(skb, tend_off, offset, 0));
    }
    put_cmsg(msg, SOL_IP, IP_CHECKSUM, sizeof(__wsum), &csum);
    }
#[no_mangle]
unsafe extern "C" fn ip_cmsg_recv_security(msg: *mut msghdr, skb: *mut sk_buff) {
    static void ip_cmsg_recv_security(struct msghdr *msg, struct sk_buff *skb)
    {
    struct lsm_context ctx;
    u32 secid;
    int err;
    err = security_socket_getpeersec_dgram(core::ptr::null_mut(), skb, &secid);
    if (err)
    return;
...
#[no_mangle]
unsafe extern "C" fn ip_cmsg_recv_dstaddr(msg: *mut msghdr, skb: *mut sk_buff) {
...
// Our caller is responsible for freeing ipc->opt
    ...
    val = *(int *)CMSG_DATA(cmsg);
#[no_mangle]
pub unsafe extern "C" fn if(CMSG_LEN(sizeof(u8)): cmsg->cmsg_len ==) -> else {
...
unsafe extern "C" fn ip_ra_destroy_rcu(head: *mut rcu_head) {
    ...
    return -EINVAL;
    new_ra = on ? kmalloc_obj(*new_ra) : core::ptr::null_mut();
    if (on && !new_ra)
    return -ENOMEM;
    mutex_lock(&net.ipv4.ra_mutex);
    for (rap = &net.ipv4.ra_chain;
    (ra = rcu_dereference_protected(*rap,
    lockdep_is_held(&net.ipv4.ra_mutex))) != core::ptr::null_mut();
    rap = &ra.next) {
    if (ra.sk == sk) {
    if (on) {
...
#[no_mangle]
pub unsafe extern "C" fn ip_local_error(sk: *mut sock, err: c_int, daddr: __be32, port: __be16, info: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ipv4_datagram_support_addr(serr: *mut sock_exterr_skb) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ip_recv_error(sk: *mut sock, msg: *mut msghdr, len: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __ip_sock_set_tos(sk: *mut sock, val: c_int) {
    void __ip_sock_set_tos(struct sock *sk, int val)
    {
    let mut old_tos: u8 = inet_sk(sk).tos;
...
#[no_mangle]
pub unsafe extern "C" fn ip_sock_set_tos(sk: *mut sock, val: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn ip_sock_set_freebind(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn ip_sock_set_recverr(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn ip_sock_set_mtu_discover(sk: *mut sock, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ip_sock_set_pktinfo(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn setsockopt_needs_rtnl(optname: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ip_set_mcast_msfilter(sk: *mut sock, optval: sockptr_t, optlen: c_int) -> c_int {
    static int ip_set_mcast_msfilter(struct sock *sk, sockptr_t optval, int optlen)
    {
    struct group_filter *gsf = core::ptr::null_mut();
    int err;
...
// numsrc >= (4G-140)/128 overflow in 32 bits
    ...
    int optlen)
    {
    let mut size0: c_int = offsetof(struct compat_group_filter, gf_slist_flex);
...
// numsrc >= (4G-140)/128 overflow in 32 bits
    ...
    struct net *net = sock_net(sk);
    let mut val: c_int = 0, err, retv;
    let mut needs_rtnl: bool = setsockopt_needs_rtnl(optname);
    switch (optname) {
    case IP_PKTINFO:
    case IP_RECVTTL:
    case IP_RECVOPTS:
...
// If optlen==0, it is equivalent to val == 0
    ...
    retv = ip_ra_control(sk, val ? 1 : 0, core::ptr::null_mut());
    if (retv == 0)
...
//
#[no_mangle]
pub unsafe extern "C" fn ipv4_pktinfo_prepare(sk: *const sock, skb: *mut sk_buff, drop_dst: bool) {
...
//
    ...
    let mut l3slave: bool = ipv4_l3mdev_skb(IPCB(skb).flags);
    if (pktinfo.ipi_ifindex == LOOPBACK_IFINDEX)
    pktinfo.ipi_ifindex = inet_iif(skb);
#[no_mangle]
pub unsafe extern "C" fn if(rt->rt_iif: l3slave && rt &&) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn getsockopt_needs_rtnl(optname: c_int) -> bool {
    ...
    {
    let mut size0: c_int = offsetof(struct group_filter, gf_slist_flex);
    ...
    {
    struct inet_sock *inet = inet_sk(sk);
    let mut needs_rtnl: bool = getsockopt_needs_rtnl(optname);
    int val, err = 0;
    int len;
    if (level != SOL_IP)
    return -EOPNOTSUPP;
...
// Handle options that can be read without locking the socket.
    ...
    let mut hlim: c_int = READ_ONCE(inet.mc_ttl);
    put_cmsg(&msg, SOL_IP, IP_TTL, sizeof(hlim), &hlim);
    }
    if (inet_test_bit(TOS, sk)) {
    let mut tos: c_int = READ_ONCE(inet.rcv_tos);
    ...
    case IP_UNICAST_IF:
    val = ( int)htonl((__u32) READ_ONCE(inet.uc_index));
    goto copyval;
    case IP_MULTICAST_IF:
    {
    struct in_addr addr;
    ...
    let mut ucval: c_uchar = (unsigned char)val;
    len = 1;
    if (copy_to_sockptr(optlen, &len, sizeof(int)))
    return -EFAULT;
