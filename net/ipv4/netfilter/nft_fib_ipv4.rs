...
//! Source: net/ipv4/netfilter/nft_fib_ipv4.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn get_saddr(addr: __be32) -> __be32 {
...
// dst = inet_addr_type_dev_table(nft_net(pkt), pkt->skb->dev, addr);
    ...
    .flowi4_uid = sock_net_uid(nft_net(pkt), core::ptr::null_mut()),
    };
...
//
    ...
    oif = nft_out(pkt);
#[no_mangle]
pub unsafe extern "C" fn if(NFTA_FIB_F_IIF: priv->flags &) -> else {
    else if (priv.flags & NFTA_FIB_F_IIF)
    oif = nft_in(pkt);
    else
    oif = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn nft_fib4_module_init() -> int __init {
    static int __init nft_fib4_module_init(void)
    {
    return nft_register_expr(&nft_fib4_type);
    }
#[no_mangle]
unsafe extern "C" fn nft_fib4_module_exit() -> void __exit {
