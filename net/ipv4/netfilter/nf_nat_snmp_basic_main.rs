...
//! Source: net/ipv4/netfilter/nf_nat_snmp_basic_main.c
#![no_std]
...
pub const SNMP_PORT: c_int = 161;
pub const SNMP_TRAP_PORT: c_int = 162;
    static DEFINE_SPINLOCK(snmp_lock);
#[repr(C)]
#[derive(Copy, Clone)]
pub struct snmp_ctx {
    pub begin: *mut c_uchar,
    pub check: *mut __sum16,
    pub from: __be32,
    pub to: __be32,
}

#[no_mangle]
unsafe extern "C" fn fast_csum(ctx: *mut snmp_ctx, offset: c_uchar) {
...
#[no_mangle]
unsafe extern "C" fn snmp_translate(ct: *mut nf_conn, dir: c_int, skb: *mut sk_buff) -> c_int {
    static int snmp_translate(struct nf_conn *ct, int dir, struct sk_buff *skb)
    {
    struct iphdr *iph = ip_hdr(skb);
    struct udphdr *udph = (struct udphdr *)((__be32 *)iph + iph.ihl);
    let mut datalen: u16 = udp_get_len_short(udph) - sizeof(struct udphdr);
...
#[no_mangle]
unsafe extern "C" fn nf_nat_snmp_basic_init() -> int __init {
    static int __init nf_nat_snmp_basic_init(void)
    {
    int err;
    BUG_ON(nf_nat_snmp_hook != core::ptr::null_mut());
    RCU_INIT_POINTER(nf_nat_snmp_hook, help);
    nf_ct_helper_init(&snmp_trap_helper, AF_INET, IPPROTO_UDP,
    "snmp_trap",
    &snmp_exp_policy, 0, help, core::ptr::null_mut(), THIS_MODULE);
    err = nf_conntrack_helper_register(&snmp_trap_helper, &snmp_trap_helper_ptr);
    if (err < 0) {
    RCU_INIT_POINTER(nf_nat_snmp_hook, core::ptr::null_mut());
    return err;
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn nf_nat_snmp_basic_fini() -> void __exit {
