...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct udp_tunnel_type_entry {
    pub gro_receive: udp_tunnel_gro_rcv_t,
...
pub unsafe extern "C" fn udp_tunnel_update_gro_lookup(net: *mut net, sk: *mut sock, add: bool) {
    ...
    {
    let mut is_ipv6: bool = sk.sk_family == AF_INET6;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: up->tunnel_list.pprev) -> else {
    ...
    RCU_INIT_POINTER(udp_tunnel_gro.sk, core::ptr::null_mut());
    }
...
pub unsafe extern "C" fn udp_tunnel_update_gro_rcv(sk: *mut sock, add: bool) {
    ...
    {
    struct udp_tunnel_type_entry *cur = core::ptr::null_mut();
...
// cur = udp_tunnel_gro_types[--udp_tunnel_gro_type_nr];
    ...
    {
    let mut tnl_hlen: c_int = skb_inner_mac_header(skb) - skb_transport_header(skb);
...
//
    ...
    partial = ( __wsum)uh.len;
    else
    partial = ( __wsum)htonl(skb.len);
...
// clear destructor to avoid skb_segment assigning it to tail
    ...
    if (copy_dtor) {
    gso_skb.destructor = core::ptr::null_mut();
    gso_skb.sk = core::ptr::null_mut();
    }
...
// update refcount for the packet
    if (copy_dtor) {
    let mut delta: c_int = sum_truesize - gso_skb.truesize;
...
pub const UDP_GRO_CNT_MAX: c_int = 64;
...
// mismatch, but we never need to flush
    ...
    struct sk_buff *p;
    struct udphdr *uh2;
    let mut off: c_uint = skb_gro_offset(skb);
    unsigned int ulen;
...
#[no_mangle]
unsafe extern "C" fn udp_gro_complete_segment(skb: *mut sk_buff) -> c_int {
    ...
    let mut newlen: c_uint = skb.len - nhoff;
    struct sock *sk;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn udp4_gro_complete(skb: *mut sk_buff, nhoff: c_int) -> INDIRECT_CALLABLE_SCOPE int {
...
#[no_mangle]
pub unsafe extern "C" fn udpv4_offload_init() -> int __init {
