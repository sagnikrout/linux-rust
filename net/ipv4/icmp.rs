...
pub struct icmp_control {
    pub skb): *mut *mut enum skb_drop_reason (handler)(struct sk_buff,
    pub /: *mut *mut short error; / This ICMP is classed as an error message,
}
...
#[no_mangle]
pub unsafe extern "C" fn icmp_xmit_unlock(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn icmp_global_allow(net: *mut net) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn icmp_global_consume(net: *mut net) {
    void icmp_global_consume(struct net *net)
    {
    let mut credits: c_int = get_random_u32_below(3);
// Note: this might make icmp_global.credit negative.
    if (credits)
    atomic_sub(credits, &net.ipv4.icmp_global_credit);
    }
#[no_mangle]
unsafe extern "C" fn icmpv4_mask_allow(net: *mut net, type: c_int, code: c_int) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn icmp_out_count(net: *mut net, type: c_uchar) {
...
//
    ...
    } else if ((skb = skb_peek(&sk.sk_write_queue)) != core::ptr::null_mut()) {
...
//
#[no_mangle]
unsafe extern "C" fn icmp_reply(icmp_param: *mut icmp_bxm, skb: *mut sk_buff) {
    static void icmp_reply(struct icmp_bxm *icmp_param, struct sk_buff *skb)
    {
    struct rtable *rt = skb_rtable(skb);
    struct net *net = dev_net_rcu(rt.dst.dev);
    let mut apply_ratelimit: bool = false;
    struct ipcm_cookie ipc;
    struct flowi4 fl4;
    struct sock *sk;
    __be32 daddr, saddr;
    let mut mark: u32 = IP4_REPLY_MARK(net, skb.mark);
    let mut type: c_int = icmp_param.data.icmph.type;
    let mut code: c_int = icmp_param.data.icmph.code;
...
//
    ...
    return dev;
    dst = skb_dst(skb);
    return dst ? dst_dev(dst) : core::ptr::null_mut();
    }
...
// No need to clone since we're just using its address.
    ...
    flowi4_to_flowi(fl4), core::ptr::null_mut(), 0);
    rt = dst_rtable(dst);
    if (!IS_ERR(dst)) {
    if (rt != rt2)
    return rt;
...
//
    ...
    if (err)
    goto relookup_failed;
    dst2 = xfrm_lookup(net, &rt2.dst, flowi4_to_flowi(&fl4_dec), core::ptr::null_mut(),
    XFRM_LOOKUP_ICMP);
    rt2 = dst_rtable(dst2);
    if (!IS_ERR(dst2)) {
    dst_release(&rt.dst);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct icmp_ext_iio_addr4_subobj {
    pub afi: __be16,
    pub reserved: __be16,
    pub addr4: __be32,
}
...
#[no_mangle]
unsafe extern "C" fn icmp_ext_iio_len() -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn icmp_ext_max_len(ext_objs: u8) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn icmp_ext_iio_addr4_find(dev: *const net_device) -> __be32 {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: skb_in->dev) -> else {
    else if (skb_in.dev)
...
#[no_mangle]
pub unsafe extern "C" fn icmp_ndo_send(skb_in: *mut sk_buff, type: c_int, code: c_int, info: __be32) {
...
#[no_mangle]
unsafe extern "C" fn icmp_socket_deliver(skb: *mut sk_buff, info: u32) {
...
#[no_mangle]
unsafe extern "C" fn icmp_tag_validation(proto: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn icmp_unreach(skb: *mut sk_buff) -> enum skb_drop_reason {
    static enum skb_drop_reason icmp_unreach(struct sk_buff *skb)
    {
    let mut reason: enum skb_drop_reason = SKB_NOT_DROPPED_YET;
...
//
#[no_mangle]
unsafe extern "C" fn icmp_redirect(skb: *mut sk_buff) -> enum skb_drop_reason {
...
//
#[no_mangle]
unsafe extern "C" fn icmp_echo(skb: *mut sk_buff) -> enum skb_drop_reason {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !icmp_build_probe(skb, _arg: &icmp_param->data.icmph)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn icmp_build_probe(skb: *mut sk_buff, icmphdr: *mut icmphdr) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn icmp_timestamp(skb: *mut sk_buff) -> enum skb_drop_reason {
...
#[no_mangle]
unsafe extern "C" fn icmp_discard(skb: *mut sk_buff) -> enum skb_drop_reason {
...
//
#[no_mangle]
pub unsafe extern "C" fn icmp_rcv(skb: *mut sk_buff) -> c_int {
    ...
    if (!xfrm4_policy_check_reverse(core::ptr::null_mut(), XFRM_POLICY_IN,
    skb)) {
    reason = SKB_DROP_REASON_XFRM_POLICY;
    goto drop;
    }
...
#[no_mangle]
unsafe extern "C" fn ip_icmp_error_rfc4884_validate(skb: *const sk_buff, off: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn icmp_err(skb: *mut sk_buff, info: u32) -> c_int {
    int icmp_err(struct sk_buff *skb, u32 info)
    {
    struct iphdr *iph = (struct iphdr *)skb.data;
    let mut offset: c_int = iph.ihl<<2;
...
#[no_mangle]
unsafe extern "C" fn icmp_sk_init(net: *mut net) -> int __net_init {
...
#[no_mangle]
pub unsafe extern "C" fn icmp_init() -> int __init {
