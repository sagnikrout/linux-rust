...
pub struct lp {
    ...
    pub owd_max_rsv: u32,
    pub remote_hz: u32,
    pub remote_ref_time: u32,
    pub local_ref_time: u32,
    pub last_drop: u32,
    pub inference: u32,
}
...
//
#[no_mangle]
unsafe extern "C" fn tcp_lp_init(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_lp_cong_avoid(sk: *mut sock, ack: u32, acked: u32) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_lp_remote_hz_estimator(sk: *mut sock) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_lp_owd_calculator(sk: *mut sock) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_lp_rtt_sample(sk: *mut sock, rtt: u32) {
    static void tcp_lp_rtt_sample(struct sock *sk, u32 rtt)
    {
    struct lp *lp = inet_csk_ca(sk);
    let mut mowd: i64 = tcp_lp_owd_calculator(sk);
...
//
#[no_mangle]
unsafe extern "C" fn tcp_lp_pkts_acked(sk: *mut sock, sample: *const ack_sample) {
    static void tcp_lp_pkts_acked(struct sock *sk, const struct ack_sample *sample)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    struct lp *lp = inet_csk_ca(sk);
    let mut now: u32 = tcp_time_stamp_ts(tp);
    u32 delta;
    if (sample.rtt_us > 0)
...
#[no_mangle]
unsafe extern "C" fn tcp_lp_register() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tcp_lp_unregister() -> void __exit {
