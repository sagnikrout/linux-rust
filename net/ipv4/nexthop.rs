...
pub const NH_DEV_HASHBITS: c_int = 8;
...
#[no_mangle]
unsafe extern "C" fn nexthop_notifiers_is_empty(net: *mut net) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: nh_info->gw_family ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn nh_notifier_single_info_fini(info: *mut nh_notifier_info) {
    ...
    let mut num_nh_buckets: u16 = res_table.num_nh_buckets;
    unsigned long size;
    u16 i;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: nhg->resilient) -> else {
...
#[no_mangle]
unsafe extern "C" fn nh_notifier_res_bucket_info_fini(info: *mut nh_notifier_info) {
...
#[no_mangle]
unsafe extern "C" fn nh_dev_hashfn(val: c_uint) -> c_uint {
    static unsigned int nh_dev_hashfn(unsigned int val)
    {
    let mut mask: c_uint = NH_DEV_HASHSIZE - 1;
    return (val ^
    (val >> NH_DEV_HASHBITS) ^
    (val >> (NH_DEV_HASHBITS * 2))) & mask;
    }
#[no_mangle]
unsafe extern "C" fn nexthop_devhash_add(net: *mut net, nhi: *mut nh_info) {
...
#[no_mangle]
unsafe extern "C" fn nexthop_free_group(nh: *mut nexthop) {
...
#[no_mangle]
unsafe extern "C" fn nexthop_free_single(nh: *mut nexthop) {
...
#[no_mangle]
pub unsafe extern "C" fn nexthop_free_rcu(head: *mut rcu_head) {
    ...
    {
    let mut num_nh_buckets: u16 = cfg.nh_grp_res_num_buckets;
    struct nh_res_table *res_table;
    unsigned long size;
...
#[no_mangle]
unsafe extern "C" fn nh_base_seq_inc(net: *mut net) {
...
// no reference taken; rcu lock or rtnl must be held
    ...
    {
    struct rb_node **pp, *parent = core::ptr::null_mut(), *next;
    ...
    pp = &next.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(nh->id: id >) -> else {
    else if (id > nh.id)
    pp = &next.rb_right;
    else
    return nh;
...
#[no_mangle]
unsafe extern "C" fn nh_find_unused_id(net: *mut net) -> u32 {
    static u32 nh_find_unused_id(struct net *net)
    {
    let mut id_start: u32 = net.nexthop.last_id_allocated;
    while (1) {
...
#[no_mangle]
unsafe extern "C" fn nh_res_table_unbalanced_time(res_table: *mut nh_res_table) -> clock_t {
...
#[no_mangle]
unsafe extern "C" fn nla_put_nh_group_res(skb: *mut sk_buff, nhg: *mut nh_group) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nh_grp_entry_stats_inc(nhge: *mut nh_grp_entry) {
...
#[no_mangle]
unsafe extern "C" fn nh_notifier_grp_hw_stats_fini(info: *mut nh_notifier_info) {
...
#[no_mangle]
unsafe extern "C" fn nh_grp_hw_stats_update(nh: *mut nexthop, hw_stats_used: *mut bool) -> c_int {
...
// hw_stats_used = info.nh_grp_hw_stats->hw_stats_used;
    ...
    let mut len: usize = nhg.num_nh * sizeof(*p);
    struct nlattr *nla;
    let mut group_type: u16 = 0;
    u16 weight;
    int i;
...
#[no_mangle]
unsafe extern "C" fn nh_nlmsg_size_grp_res(nhg: *mut nh_group) -> usize {
...
#[no_mangle]
unsafe extern "C" fn nh_nlmsg_size_grp(nh: *mut nexthop, op_flags: u32) -> usize {
    static size_t nh_nlmsg_size_grp(struct nexthop *nh, u32 op_flags)
    {
    struct nh_group *nhg = rtnl_dereference(nh.nh_grp);
    let mut sz: usize = sizeof(struct nexthop_grp) * nhg.num_nh;
...
#[no_mangle]
unsafe extern "C" fn nh_nlmsg_size_single(nh: *mut nexthop) -> usize {
...
#[no_mangle]
unsafe extern "C" fn nh_nlmsg_size(nh: *mut nexthop, op_flags: u32) -> usize {
    static size_t nh_nlmsg_size(struct nexthop *nh, u32 op_flags)
    {
    let mut sz: usize = NLMSG_ALIGN(sizeof(struct nhmsg));
...
#[no_mangle]
unsafe extern "C" fn nexthop_notify(event: c_int, nh: *mut nexthop, info: *mut nl_info) {
    static void nexthop_notify(int event, struct nexthop *nh, struct nl_info *info)
    {
    let mut nlflags: c_uint = info.nlh ? info.nlh.nlmsg_flags : 0;
...
#[no_mangle]
unsafe extern "C" fn nh_res_bucket_used_time(bucket: *const nh_res_bucket) -> c_ulong {
    ...
    unsigned long now)
    {
    let mut time: c_ulong = nh_res_bucket_used_time(bucket);
...
#[no_mangle]
unsafe extern "C" fn nh_res_bucket_set_busy(bucket: *mut nh_res_bucket) {
    static void nh_res_bucket_set_busy(struct nh_res_bucket *bucket)
    {
    atomic_long_set(&bucket.used_time, (long)jiffies);
    }
#[no_mangle]
unsafe extern "C" fn nh_res_bucket_idle_time(bucket: *const nh_res_bucket) -> clock_t {
    static clock_t nh_res_bucket_idle_time(const struct nh_res_bucket *bucket)
    {
    let mut used_time: c_ulong = nh_res_bucket_used_time(bucket);
    ...
    rtnl_notify(skb, nh.net, 0, RTNLGRP_NEXTHOP, core::ptr::null_mut(), GFP_KERNEL);
    return;
    errout:
...
// nh_family = nhi->family;
    ...
    {
    let mut len: c_uint = nla_len(tb[NHA_GROUP]);
...
//
    ...
    struct netlink_ext_ack *extack)
    {
    let mut nh_family: u8 = AF_UNSPEC;
    struct nexthop_grp *nhg;
    unsigned int len;
    unsigned int i;
    u8 nhg_fdb;
...
#[no_mangle]
unsafe extern "C" fn ipv6_good_nh(nh: *const fib6_nh) -> bool {
    static bool ipv6_good_nh(const struct fib6_nh *nh)
    {
    let mut state: c_int = NUD_REACHABLE;
    struct neighbour *n;
    rcu_read_lock();
...
#[no_mangle]
unsafe extern "C" fn ipv4_good_nh(nh: *const fib_nh) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nexthop_is_good_nh(nh: *const nexthop) -> bool {
    ...
    {
    struct nh_grp_entry *nhge0 = core::ptr::null_mut();
    int i;
    if (nhg.fdb_nh)
...
//
    ...
    let mut bucket_index: u16 = hash % res_table.num_nh_buckets;
    struct nh_res_bucket *bucket;
    struct nh_grp_entry *nhge;
...
//
    ...
    if (err)
    return err;
    return fib6_check_nexthop(new, core::ptr::null_mut(), extack);
    }
...
#[no_mangle]
unsafe extern "C" fn nh_res_table_upkeep_dw(work: *mut work_struct) {
    static void nh_res_table_upkeep_dw(struct work_struct *work)
    {
...
#[no_mangle]
unsafe extern "C" fn nh_res_table_cancel_upkeep(res_table: *mut nh_res_table) {
    ...
    struct nh_res_table *res_table)
    {
    let mut prev_upper_bound: u16 = 0;
    let mut total: u32 = 0;
    let mut w: u32 = 0;
    int i;
    INIT_LIST_HEAD(&res_table.uw_nh_entries);
...
//
    ...
    for (i = 0; i < res_table.num_nh_buckets; i++) {
    struct nh_res_bucket *bucket = &res_table.nh_buckets[i];
    let mut id: u32 = rtnl_dereference(bucket.nh_entry).nh.id;
...
//
    ...
    let mut prev_unbalanced_since: c_ulong = old_res_table.unbalanced_since;
    let mut prev_has_uw: bool = !list_empty(&old_res_table.uw_nh_entries);
...
#[no_mangle]
unsafe extern "C" fn nh_hthr_group_rebalance(nhg: *mut nh_group) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: newg->resilient) -> else {
...
#[no_mangle]
unsafe extern "C" fn remove_nexthop_group(nh: *mut nexthop, nlinfo: *mut nl_info) {
...
// not called for nexthop replace
    ...
    struct nexthop *nh)
    {
    let mut need_flush: bool = !list_empty(&nh.fi_list);
    struct fib6_info *f6i;
    struct fib_info *fi;
...
// __ip6_del_rt does a release, so do a hold here
    ...
    struct nl_info *nlinfo)
    {
    let mut need_flush: bool = __remove_nexthop_fib(net, nh);
    if (nh.is_group) {
    ...
    {
    bool need_flush;
    call_nexthop_notifiers(net, NEXTHOP_EVENT_DEL, nh, core::ptr::null_mut());
...
//
    ...
    struct netlink_ext_ack *extack)
    {
    struct nh_res_table *tmp_table = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn nh_group_v4_update(nhg: *mut nh_group) {
    ...
    struct netlink_ext_ack *extack)
    {
    let mut nhg_id: u32 = res_table.nhg_id;
    int err;
    u16 i;
    for (i = 0; i < res_table.num_nh_buckets; i++) {
    struct nh_res_bucket *bucket = &res_table.nh_buckets[i];
...
//
    ...
    struct nexthop *nhp = nhge.nh_parent;
    replace_nexthop_single_notify(net, nhp, old, newi, oldi, core::ptr::null_mut());
    }
...
//
    ...
    struct netlink_ext_ack *extack)
    {
    let mut new_is_reject: bool = false;
    struct nh_grp_entry *nhge;
    int err;
...
//
    ...
    WARN_ON_ONCE(__remove_nexthop(net, new, core::ptr::null_mut()));
    nexthop_put(new);
    }
    return err;
    }
// called with rtnl_lock held
    ...
    let mut replace: bool = !!(cfg.nlflags & NLM_F_REPLACE);
    let mut create: bool = !!(cfg.nlflags & NLM_F_CREATE);
    let mut new_id: u32 = new_nh.id;
    let mut replace_notify: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn nexthop_flush_dev(dev: *mut net_device, event: c_ulong) {
    static void nexthop_flush_dev(struct net_device *dev, unsigned long event)
    {
    let mut hash: c_uint = nh_dev_hashfn(dev.ifindex);
    ...
    continue;
    need_flush |= remove_nexthop(net, nhi.nh_parent, core::ptr::null_mut());
    }
    if (need_flush)
    fib_flush(net);
    }
...
#[no_mangle]
unsafe extern "C" fn flush_all_nexthops(net: *mut net) {
    ...
    need_flush |= remove_nexthop(net, nh, core::ptr::null_mut());
    cond_resched();
    }
    if (need_flush)
    fib_flush(net);
    ...
    let mut num_nh: u16 = nla_len(grps_attr) / sizeof(*entry);
    struct nh_group *nhg;
    struct nexthop *nh;
    int err;
    int i;
...
// spare group used for removals
    ...
    let mut tb_id: u32 = (cfg.dev ? l3mdev_fib_table(cfg.dev) : RT_TABLE_MAIN);
    int err;
...
// called with rtnl lock held
    ...
    if (err) {
    WARN_ON_ONCE(__remove_nexthop(net, nh, core::ptr::null_mut()));
    nexthop_put(nh);
    nh = ERR_PTR(err);
    }
    return nh;
...
// rtnl
    ...
    if (err < 0)
    return err;
    err = nh_valid_get_del_req(nlh, tb, &id, core::ptr::null_mut(), extack);
    if (err)
    return err;
    rtnl_net_lock(net);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nh_dump_filter {
    pub nh_id: u32,
    pub dev_idx: c_int,
    pub master_idx: c_int,
    pub group_filter: bool,
    pub fdb_filter: bool,
    pub res_bucket_nh_id: u32,
    pub op_flags: u32,
}
...
// rtnl
#[no_mangle]
unsafe extern "C" fn rtm_dump_nexthop(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
    static int rtm_dump_nexthop(struct sk_buff *skb, struct netlink_callback *cb)
    {
    struct rtm_dump_nh_ctx *ctx = rtm_dump_nh_ctx(cb);
    struct net *net = sock_net(skb.sk);
    struct rb_root *root = &net.nexthop.rb_root;
    let mut filter: nh_dump_filter = {};
    int err;
...
// nh_id_p = 0;
    ...
    rtm_nh_policy_dump_bucket, core::ptr::null_mut());
    if (err < 0)
    return err;
    ...
    let mut max: usize = ARRAY_SIZE(rtm_nh_res_bucket_policy_dump) - 1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rtm_dump_res_bucket_ctx {
    pub nh: rtm_dump_nh_ctx,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rtm_dump_nexthop_bucket_data {
    pub ctx: *mut rtm_dump_res_bucket_ctx,
    pub filter: nh_dump_filter,
}
...
// rtnl
    ...
    let mut dd: rtm_dump_nexthop_bucket_data = { .ctx = ctx };
    struct net *net = sock_net(skb.sk);
    struct nexthop *nh;
    int err;
...
#[no_mangle]
unsafe extern "C" fn nexthop_sync_mtu(dev: *mut net_device, orig_mtu: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn __unregister_nexthop_notifier(net: *mut net, nb: *mut notifier_block) -> c_int {
    ...
    nb);
    if (!err)
    nexthops_dump(net, nb, NEXTHOP_EVENT_DEL, core::ptr::null_mut());
    return err;
    }
    EXPORT_SYMBOL(__unregister_nexthop_notifier);
#[no_mangle]
pub unsafe extern "C" fn unregister_nexthop_notifier(net: *mut net, nb: *mut notifier_block) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nexthop_set_hw_flags(net: *mut net, id: u32, offload: bool, trap: bool) {
...
#[no_mangle]
unsafe extern "C" fn nexthop_net_exit(net: *mut net) -> void __net_exit {
    static void __net_exit nexthop_net_exit(struct net *net)
    {
    kfree(net.nexthop.devhash);
    net.nexthop.devhash = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn nexthop_net_init(net: *mut net) -> int __net_init {
    static int __net_init nexthop_net_init(struct net *net)
    {
    let mut sz: usize = sizeof(struct hlist_head) * NH_DEV_HASHSIZE;
...
#[no_mangle]
unsafe extern "C" fn nexthop_init() -> int __init {
