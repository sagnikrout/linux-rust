...
//! Source: net/ipv4/udp_tunnel_nic.c
#![no_std]
...
pub struct udp_tunnel_nic {
    ...
    pub lock: mutex,
    pub need_sync:1: u8,
    pub need_replay:1: u8,
    pub n_tables: c_uint,
    pub missed: c_ulong,
    pub __counted_by(n_tables): *mut *mut udp_tunnel_nic_table_entry entries[],
}
...
//
    ...
    int err)
    {
    let mut dodgy: bool = entry.flags & UDP_TUNNEL_NIC_ENTRY_OP_FAIL;
...
#[no_mangle]
unsafe extern "C" fn __udp_tunnel_nic_reset_ntf(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn __udp_tunnel_nic_assert_locked(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn __udp_tunnel_nic_lock(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn __udp_tunnel_nic_unlock(dev: *mut net_device) {
    ...
    for (i = 0; i < utn.n_tables; i++)
    for (j = 0; j < info.tables[i].n_entries; j++) {
    let mut adj_cnt: c_int = -utn.entries[i][j].use_cnt;
    if (adj_cnt)
...
#[no_mangle]
unsafe extern "C" fn udp_tunnel_nic_device_sync_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn udp_tunnel_nic_free(utn: *mut udp_tunnel_nic) {
...
#[no_mangle]
unsafe extern "C" fn udp_tunnel_nic_register(dev: *mut net_device) -> c_int {
    static int udp_tunnel_nic_register(struct net_device *dev)
    {
    const struct udp_tunnel_nic_info *info = dev.udp_tunnel_nic_info;
    struct udp_tunnel_nic_shared_node *node = core::ptr::null_mut();
    struct udp_tunnel_nic *utn;
...
//
    ...
    info.shared.udp_tunnel_nic_info = core::ptr::null_mut();
    }
...
//
    ...
    release_dev:
    dev.udp_tunnel_nic = core::ptr::null_mut();
    dev_put(dev);
    }
    static int
...
#[no_mangle]
unsafe extern "C" fn udp_tunnel_nic_init_module() -> int __init {
    ...
    err_unset_ops:
    rtnl_lock();
    udp_tunnel_nic_ops = core::ptr::null_mut();
    rtnl_unlock();
...
#[no_mangle]
unsafe extern "C" fn udp_tunnel_nic_cleanup_module() -> void __exit {
