...
//
    ...
    let mut __read_mostly: int sysctl_tcp_max_orphans = NR_FILE;
pub const FLAG_DATA: c_uint = 0x01 /* Incoming frame contained data.		*/;
pub const FLAG_WIN_UPDATE: c_uint = 0x02 /* Incoming ACK was a window update.	*/;
pub const FLAG_DATA_ACKED: c_uint = 0x04 /* This ACK acknowledged new data.		*/;
pub const FLAG_RETRANS_DATA_ACKED: c_uint = 0x08 /* "" "" some of which was retransmitted.	*/;
pub const FLAG_SYN_ACKED: c_uint = 0x10 /* This ACK acknowledged SYN.		*/;
pub const FLAG_DATA_SACKED: c_uint = 0x20 /* New SACK.				*/;
pub const FLAG_ECE: c_uint = 0x40 /* ECE in this ACK				*/;
pub const FLAG_LOST_RETRANS: c_uint = 0x80 /* This ACK marks some retransmission lost */;
pub const FLAG_SLOWPATH: c_uint = 0x100 /* Do not skip RFC checks for window update.*/;
pub const FLAG_ORIG_SACK_ACKED: c_uint = 0x200 /* Never retransmitted data are (s)acked	*/;
pub const FLAG_SND_UNA_ADVANCED: c_uint = 0x400 /* Snd_una was changed (!= FLAG_DATA_ACKED) */;
pub const FLAG_DSACKING_ACK: c_uint = 0x800 /* SACK blocks contained D-SACK info */;
pub const FLAG_SET_XMIT_TIMER: c_uint = 0x1000 /* Set TLP or RTO timer */;
pub const FLAG_SACK_RENEGING: c_uint = 0x2000 /* snd_una advanced to a sacked seq */;
pub const FLAG_UPDATE_TS_RECENT: c_uint = 0x4000 /* tcp_replace_ts_recent() */;
pub const FLAG_NO_CHALLENGE_ACK: c_uint = 0x8000 /* do not call tcp_send_challenge_ack()	*/;
pub const FLAG_ACK_MAYBE_DELAYED: c_uint = 0x10000 /* Likely a delayed ACK */;
pub const FLAG_DSACK_TLP: c_uint = 0x20000 /* DSACK for tail loss probe */;
pub const FLAG_TS_PROGRESS: c_uint = 0x40000 /* Positive timestamp delta */;
...
#[no_mangle]
pub unsafe extern "C" fn clean_acked_data_disable(tp: *mut tcp_sock) {
    void clean_acked_data_disable(struct tcp_sock *tp)
    {
    static_branch_slow_dec_deferred(&clean_acked_data_enabled);
    tp.tcp_clean_acked = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn bpf_skops_parse_hdr(sk: *mut sock, skb: *mut sk_buff) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_measure_rcv_mss(sk: *mut sock, skb: *const sk_buff) {
    static void tcp_measure_rcv_mss(struct sock *sk, const struct sk_buff *skb)
    {
    struct inet_connection_sock *icsk = inet_csk(sk);
    let mut lss: c_uint = icsk.icsk_ack.last_seg_size;
    unsigned int len;
...
//
    ...
    let mut val: u64 = (u64)skb.len << TCP_RMEM_TO_WIN_SCALE;
    let mut old_ratio: u8 = tcp_sk(sk).scaling_ratio;
...
#[no_mangle]
unsafe extern "C" fn tcp_incr_quickack(sk: *mut sock, max_quickacks: c_uint) {
    static void tcp_incr_quickack(struct sock *sk, unsigned int max_quickacks)
    {
    struct inet_connection_sock *icsk = inet_csk(sk);
    let mut quickacks: c_uint = tcp_sk(sk).rcv_wnd / (2 * icsk.icsk_ack.rcv_mss);
    if (quickacks == 0)
    quickacks = 2;
...
#[no_mangle]
unsafe extern "C" fn tcp_enter_quickack_mode(sk: *mut sock, max_quickacks: c_uint) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_in_quickack_mode(sk: *mut sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_data_ecn_check(sk: *mut sock, skb: *const sk_buff) {
...
// Returns true if the byte counters can be used
    ...
    {
    let mut estimate_ecnfield: u8 = tp.est_ecnfield;
    let mut ambiguous_ecn_bytes_incr: bool = false;
    let mut first_changed: bool = false;
    unsigned int optlen;
    bool order1, res;
    unsigned int i;
    u8 *ptr;
...
//
    ...
    let mut saw_opt: u8 = TCP_ACCECN_OPT_FAIL_SEEN;
    tcp_accecn_saw_opt_fail_recv(tp, saw_opt);
    }
    return false;
    }
    if (estimate_ecnfield) {
    let mut ecnfield: u8 = estimate_ecnfield - 1;
...
#[no_mangle]
unsafe extern "C" fn tcp_count_delivered_ce(tp: *mut tcp_sock, ecn_count: u32) {
...
pub const PKTS_ACKED_WEIGHT: c_int = 6;
pub const PKTS_ACKED_PREC: c_int = 6;
pub const ACK_COMP_THRESH: c_int = 4;
// Returns the ECN CE delta
    static u32 __tcp_accecn_process(struct sock *sk, const struct sk_buff *skb,
    u32 delivered_pkts, u32 delivered_bytes,
    int flag)
    {
    let mut old_ceb: u32 = tcp_sk(sk).delivered_ecn_bytes[INET_ECN_CE - 1];
...
//
#[no_mangle]
unsafe extern "C" fn tcp_sndbuf_expand(sk: *mut sock) {
...
// Slow part of check#2.
    ...
    const struct tcp_sock *tp = tcp_sk(sk);
// Optimize this!
    let mut truesize: c_int = tcp_win_from_space(sk, skbtruesize) >> 1;
    let mut window: c_int = tcp_win_from_space(sk, READ_ONCE(sock_net(sk).ipv4.sysctl_tcp_rmem[2])) >> 1;
...
//
#[no_mangle]
unsafe extern "C" fn truesize_adjust(adjust: bool, skb: *const sk_buff) -> u32 {
    static u32 truesize_adjust(bool adjust, const struct sk_buff *skb)
    {
    let mut truesize: u32 = skb.truesize;
...
// Check #1
    ...
    let mut truesize: c_uint = truesize_adjust(adjust, skb);
    int incr;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_init_buffer_space(sk: *mut sock) {
    static void tcp_init_buffer_space(struct sock *sk)
    {
    let mut tcp_app_win: c_int = READ_ONCE(sock_net(sk).ipv4.sysctl_tcp_app_win);
    struct tcp_sock *tp = tcp_sk(sk);
    int maxwin;
...
#[no_mangle]
unsafe extern "C" fn tcp_clamp_window(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_initialize_rcv_mss(sk: *mut sock) {
    void tcp_initialize_rcv_mss(struct sock *sk)
    {
    const struct tcp_sock *tp = tcp_sk(sk);
    let mut hint: c_uint = min_t(unsigned int, tp.advmss, tp.mss_cache);
...
//
#[no_mangle]
unsafe extern "C" fn tcp_rcv_rtt_update(tp: *mut tcp_sock, sample: u32, win_dep: c_int) {
    static void tcp_rcv_rtt_update(struct tcp_sock *tp, u32 sample, int win_dep)
    {
    u32 new_sample, old_sample = tp.rcv_rtt_est.rtt_us;
    let mut m: c_long = sample << 3;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_rcv_rtt_measure(tp: *mut tcp_sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_rtt_tsopt_us(tp: *const tcp_sock, min_delta: u32) -> i32 {
    ...
    let mut delta: i32 = tcp_rtt_tsopt_us(tp, 0);
    if (delta > 0)
    tcp_rcv_rtt_update(tp, delta, 0);
    }
    }
#[no_mangle]
pub unsafe extern "C" fn tcp_rcvbuf_grow(sk: *mut sock, newval: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_rcv_space_adjust(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_save_lrcv_flowlabel(sk: *mut sock, skb: *const sk_buff) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_event_data_recv(sk: *mut sock, skb: *mut sk_buff) {
...
//
    ...
    let mut m: c_int = now - icsk.icsk_ack.lrcvtime;
    if (m <= TCP_ATO_MIN / 2) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_rtt_estimator(sk: *mut sock, mrtt_us: c_long) {
    static void tcp_rtt_estimator(struct sock *sk, long mrtt_us)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    long m = mrtt_us; /* RTT */
    let mut srtt: u32 = tp.srtt_us;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_update_pacing_rate(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_set_rto(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_init_cwnd(tp: *const tcp_sock, dst: *const dst_entry) -> __u32 {
    __u32 tcp_init_cwnd(const struct tcp_sock *tp, const struct dst_entry *dst)
    {
    let mut cwnd: __u32 = (dst ? dst_metric(dst, RTAX_INITCWND) : 0);
    if (!cwnd)
    cwnd = TCP_INIT_CWND;
...
//
    ...
    pub last_sackt: u64,
    pub reord: u32,
    pub sack_delivered: u32,
    pub delivered_bytes: u32,
    pub flag: c_int,
    pub mss_now: c_uint,
    pub rate: *mut rate_sample,
}
...
#[no_mangle]
pub unsafe extern "C" fn if(end_seq: tp->tlp_high_seq && tp->tlp_high_seq ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_verify_retransmit_hint(tp: *mut tcp_sock, skb: *mut sk_buff) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_notify_skb_loss_event(tp: *mut tcp_sock, skb: *const sk_buff) {
    static void tcp_notify_skb_loss_event(struct tcp_sock *tp, const struct sk_buff *skb)
    {
    tp.lost += tcp_skb_pcount(skb);
    }
#[no_mangle]
pub unsafe extern "C" fn tcp_mark_skb_lost(sk: *mut sock, skb: *mut sk_buff) {
    void tcp_mark_skb_lost(struct sock *sk, struct sk_buff *skb)
    {
    let mut sacked: __u8 = TCP_SKB_CB(skb).sacked;
    struct tcp_sock *tp = tcp_sk(sk);
...
//
    ...
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut start_seq_0: u32 = get_unaligned_be32(&sp[0].start_seq);
    let mut end_seq_0: u32 = get_unaligned_be32(&sp[0].end_seq);
    u32 dup_segs;
    if (before(start_seq_0, TCP_SKB_CB(ack_skb).ack_seq)) {
    NET_INC_STATS(sock_net(sk), LINUX_MIB_TCPDSACKRECV);
    } else if (num_sacks > 1) {
    let mut end_seq_1: u32 = get_unaligned_be32(&sp[1].end_seq);
    let mut start_seq_1: u32 = get_unaligned_be32(&sp[1].start_seq);
...
//
    if (pkt_len > mss) {
    let mut new_len: c_uint = (pkt_len / mss) * mss;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_skb_seglen(skb: *const sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn skb_can_shift(skb: *const sk_buff) -> c_int {
...
//
    ...
    let mut in_sack: c_int = 0;
    let mut dup_sack: bool = dup_sack_in;
...
//
    ...
    skb = tcp_sacktag_walk(skb, sk, core::ptr::null_mut(), state,
...
#[no_mangle]
unsafe extern "C" fn tcp_sack_cache_ok(tp: *const tcp_sock, cache: *const tcp_sack_block) -> c_int {
    ...
    struct sk_buff *skb;
    let mut num_sacks: c_int = min(TCP_NUM_SACKS, (ptr[1] - TCPOLEN_SACK_BASE) >> 3);
    int used_sacks;
    let mut found_dup_sack: bool = false;
    int i, j;
...
//
    ...
    let mut dup_sack: bool = !i && found_dup_sack;
...
// Skip empty blocks in at head of the cache
    ...
    while (i < used_sacks) {
    let mut start_seq: u32 = sp[i].start_seq;
    let mut end_seq: u32 = sp[i].end_seq;
    let mut dup_sack: bool = (found_dup_sack && (i == first_sack_index));
    struct tcp_sack_block *next_dup = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn tcp_limit_reno_sacked(tp: *mut tcp_sock) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_check_reno_reordering(sk: *mut sock, addend: c_int) {
...
#[no_mangle]
unsafe extern "C" fn tcp_add_reno_sack(sk: *mut sock, num_dupack: c_int, ece_ack: bool) {
    static void tcp_add_reno_sack(struct sock *sk, int num_dupack, bool ece_ack)
    {
    if (num_dupack) {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut prior_sacked: u32 = tp.sacked_out;
    s32 delivered;
...
#[no_mangle]
unsafe extern "C" fn tcp_remove_reno_sacks(sk: *mut sock, acked: c_int, ece_ack: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_reset_reno_sack(tp: *mut tcp_sock) {
    static inline void tcp_reset_reno_sack(struct tcp_sock *tp)
    {
    tp.sacked_out = 0;
    }
#[no_mangle]
pub unsafe extern "C" fn tcp_clear_retrans(tp: *mut tcp_sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_init_undo(tp: *mut tcp_sock) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_timeout_mark_lost(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn if(tcp_rack_skb_timeout(tp: skb != head &&, _arg: skb, 0: 0) >) -> else {
...
// Enter Loss state.
#[no_mangle]
pub unsafe extern "C" fn tcp_enter_loss(sk: *mut sock) {
    void tcp_enter_loss(struct sock *sk)
    {
    const struct inet_connection_sock *icsk = inet_csk(sk);
    struct tcp_sock *tp = tcp_sk(sk);
    struct net *net = sock_net(sk);
    let mut new_recovery: bool = icsk.icsk_ca_state < TCP_CA_Recovery;
    u8 reordering;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_check_sack_reneging(sk: *mut sock, ack_flag: *mut c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_time_to_recover(tp: *const tcp_sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_tsopt_ecr_before(tp: *const tcp_sock, when: u32) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_packet_delayed(tp: *const tcp_sock) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_any_retrans_done(sk: *const sock) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_retrans_stamp_cleanup(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn DBGUNDO(sk: *mut sock, msg: *const c_char) {
...
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: sk->sk_family ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcp_undo_cwnd_reduction(sk: *mut sock, unmark_loss: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_may_undo(tp: *const tcp_sock) -> bool {
    static inline bool tcp_may_undo(const struct tcp_sock *tp)
    {
    return tp.undo_marker && (!tp.undo_retrans || tcp_packet_delayed(tp));
    }
#[no_mangle]
unsafe extern "C" fn tcp_is_non_sack_preventing_reopen(sk: *mut sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_try_undo_recovery(sk: *mut sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_try_undo_dsack(sk: *mut sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_try_undo_loss(sk: *mut sock, frto_undo: bool) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_init_cwnd_reduction(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_cwnd_reduction(sk: *mut sock, newly_acked_sacked: c_int, newly_lost: c_int, flag: c_int) {
    void tcp_cwnd_reduction(struct sock *sk, int newly_acked_sacked, int newly_lost, int flag)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut sndcnt: c_int = 0;
    let mut delta: c_int = tp.snd_ssthresh - tcp_packets_in_flight(tp);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_end_cwnd_reduction(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_enter_cwr(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_try_keep_open(sk: *mut sock) {
    static void tcp_try_keep_open(struct sock *sk)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut state: c_int = TCP_CA_Open;
...
#[no_mangle]
unsafe extern "C" fn tcp_try_to_open(sk: *mut sock, flag: c_int) {
...
#[no_mangle]
unsafe extern "C" fn tcp_mtup_probe_failed(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_mtup_probe_success(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_non_congestion_loss_retransmit(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_simple_retransmit(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_enter_recovery(sk: *mut sock, ece_ack: bool) {
...
#[no_mangle]
unsafe extern "C" fn tcp_update_rto_time(tp: *mut tcp_sock) {
...
//
    ...
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut recovered: bool = !before(tp.snd_una, tp.high_seq);
...
#[no_mangle]
pub unsafe extern "C" fn if(FLAG_SND_UNA_ADVANCED: flag &) -> else {
...
#[no_mangle]
unsafe extern "C" fn tcp_try_undo_partial(sk: *mut sock, prior_snd_una: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_identify_packet_loss(sk: *mut sock, ack_flag: *mut c_int) {
    ...
    let mut prior_retrans: u32 = tp.retrans_out;
...
//
    ...
    struct inet_connection_sock *icsk = inet_csk(sk);
    struct tcp_sock *tp = tcp_sk(sk);
    let mut flag: c_int = *ack_flag;
    let mut ece_ack: bool = flag & FLAG_ECE;
...
#[no_mangle]
unsafe extern "C" fn tcp_update_rtt_min(sk: *mut sock, rtt_us: u32, flag: c_int) {
    static void tcp_update_rtt_min(struct sock *sk, u32 rtt_us, const int flag)
    {
    let mut wlen: u32 = READ_ONCE(sock_net(sk).ipv4.sysctl_tcp_min_rtt_wlen) * HZ;
    struct tcp_sock *tp = tcp_sk(sk);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_synack_rtt_meas(sk: *mut sock, req: *mut request_sock) {
    void tcp_synack_rtt_meas(struct sock *sk, struct request_sock *req)
    {
    struct rate_sample rs;
    let mut rtt_us: c_long = -1L;
    if (req && !req.num_retrans && tcp_rsk(req).snt_synack)
    rtt_us = tcp_stamp_us_delta(tcp_clock_us(), tcp_rsk(req).snt_synack);
    tcp_ack_update_rtt(sk, FLAG_SYN_ACKED, rtt_us, -1L, rtt_us, &rs);
    }
#[no_mangle]
unsafe extern "C" fn tcp_cong_avoid(sk: *mut sock, ack: u32, acked: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_rearm_rto(sk: *mut sock) {
...
//
    ...
    let mut rto: u32 = inet_csk(sk).icsk_rto;
// Offset the time elapsed after installing regular RTO
    if (icsk.icsk_pending == ICSK_TIME_REO_TIMEOUT ||
    icsk.icsk_pending == ICSK_TIME_LOSS_PROBE) {
    let mut delta_us: i64 = tcp_rto_delta_us(sk);
...
#[no_mangle]
unsafe extern "C" fn tcp_set_xmit_timer(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_tso_acked(sk: *mut sock, skb: *mut sk_buff) -> u32 {
...
// Avoid cache line misses to get skb_shinfo() and shinfo->tx_flags
    ...
    __skb_tstamp_tx(skb, ack_skb, core::ptr::null_mut(), sk, SCM_TSTAMP_ACK);
...
//
    ...
    let mut fully_acked: bool = true;
    let mut sack_rtt_us: c_long = -1L;
    let mut seq_rtt_us: c_long = -1L;
    let mut ca_rtt_us: c_long = -1L;
    let mut pkts_acked: u32 = 0;
    bool rtt_update;
    let mut flag: c_int = 0;
    first_ackt = 0;
    for (skb = skb_rb_first(&sk.tcp_rtx_queue); skb; skb = next) {
    struct tcp_skb_cb *scb = TCP_SKB_CB(skb);
    let mut start_seq: u32 = scb.seq;
    let mut sacked: u8 = scb.sacked;
    u32 acked_pcount;
...
//
    ...
    tp.retransmit_skb_hint = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn tcp_ack_probe(sk: *mut sock) {
...
//
    ...
    let mut when: c_ulong = tcp_probe0_when(sk, tcp_rto_max(sk));
    when = tcp_clamp_probe0_to_user_timeout(sk, when);
    tcp_reset_xmit_timer(sk, ICSK_TIME_PROBE0, when, true);
    }
    }
#[no_mangle]
pub unsafe extern "C" fn tcp_ack_is_dubious(sk: *const sock, flag: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_may_raise_cwnd(sk: *const sock, flag: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tcp_snd_sne_update(tp: *mut tcp_sock, ack: u32) {
...
#[no_mangle]
unsafe extern "C" fn tcp_snd_una_update(tp: *mut tcp_sock, ack: u32) {
    static void tcp_snd_una_update(struct tcp_sock *tp, u32 ack)
    {
    let mut delta: u32 = ack - tp.snd_una;
...
#[no_mangle]
unsafe extern "C" fn tcp_rcv_sne_update(tp: *mut tcp_sock, seq: u32) {
...
#[no_mangle]
unsafe extern "C" fn tcp_rcv_nxt_update(tp: *mut tcp_sock, seq: u32) {
    static void tcp_rcv_nxt_update(struct tcp_sock *tp, u32 seq)
    {
    let mut delta: u32 = seq - tp.rcv_nxt;
    sock_owned_by_me((struct sock *)tp);
...
//
    ...
    struct tcp_sock *tp = tcp_sk(sk);
    let mut flag: c_int = 0;
    let mut nwin: u32 = ntohs(tcp_hdr(skb).window);
...
// Paired with the WRITE_ONCE() in this function.
    let mut val: u32 = READ_ONCE(*last_oow_ack_time);
    if (val) {
    let mut elapsed: i32 = (s32)(tcp_jiffies32 - val);
    if (0 <= elapsed &&
...
#[no_mangle]
unsafe extern "C" fn tcp_send_ack_reflect_ect(sk: *mut sock, accecn_reflector: bool) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_challenge_ack_allowed(net: *mut net) -> bool {
    ...
    let mut half: u32 = (ack_limit + 1) >> 1;
...
#[no_mangle]
unsafe extern "C" fn tcp_send_challenge_ack(sk: *mut sock, accecn_reflector: bool) {
...
#[no_mangle]
unsafe extern "C" fn tcp_store_ts_recent(tp: *mut tcp_sock) {
...
#[no_mangle]
unsafe extern "C" fn __tcp_replace_ts_recent(tp: *mut tcp_sock, tstamp_delta: i32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tcp_replace_ts_recent(tp: *mut tcp_sock, seq: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_process_tlp_ack(sk: *mut sock, ack: u32, flag: c_int) {
...
#[no_mangle]
unsafe extern "C" fn tcp_in_ack_event(sk: *mut sock, flag: c_int) {
    static void tcp_in_ack_event(struct sock *sk, int flag)
    {
    const struct inet_connection_sock *icsk = inet_csk(sk);
    if (icsk.icsk_ca_ops.in_ack_event) {
    let mut ack_ev_flags: u32 = 0;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_xmit_recovery(sk: *mut sock, rexmit: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_rack_update_reo_wnd(sk: *mut sock, rs: *mut rate_sample) {
...
#[no_mangle]
unsafe extern "C" fn tcp_ack(sk: *mut sock, skb: *const sk_buff, flag: c_int) -> c_int {
    static int tcp_ack(struct sock *sk, const struct sk_buff *skb, int flag)
    {
    struct inet_connection_sock *icsk = inet_csk(sk);
    struct tcp_sock *tp = tcp_sk(sk);
    struct tcp_sacktag_state sack_state;
    let mut rs: rate_sample = { .prior_delivered = 0 };
    let mut prior_snd_una: u32 = tp.snd_una;
    let mut is_sack_reneg: bool = tp.is_sack_reneg;
    let mut ack_seq: u32 = TCP_SKB_CB(skb).seq;
    let mut ack: u32 = TCP_SKB_CB(skb).ack_seq;
    let mut num_dupack: c_int = 0;
    let mut prior_packets: c_int = tp.packets_out;
    let mut delivered: u32 = tp.delivered;
    let mut lost: u32 = tp.lost;
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_parse_mss_option(th: *const tcphdr, user_mss: u16) -> u16 {
    u16 tcp_parse_mss_option(const struct tcphdr *th, u16 user_mss)
    {
    const unsigned char *ptr = (const unsigned char *)(th + 1);
    let mut length: c_int = (th.doff * 4) - sizeof(struct tcphdr);
    let mut mss: u16 = 0;
    while (length > 0) {
    let mut opcode: c_int = *ptr++;
    int opsize;
    switch (opcode) {
    case TCPOPT_EOL:
    return mss;
    ...
    let mut in_mss: u16 = get_unaligned_be16(ptr);
    if (in_mss) {
...
//
    ...
    let mut snd_wscale: __u8 = *(__u8 *)ptr;
    opt_rx.wscale_ok = 1;
...
#[no_mangle]
unsafe extern "C" fn tcp_parse_aligned_timestamp(tp: *mut tcp_sock, th: *const tcphdr) -> bool {
...
//
    ...
    tcp_parse_options(net, skb, &tp.rx_opt, 1, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn tcp_tsval_replay(sk: *const sock) -> u32 {
...
//
    ...
    let mut ack: u32 = TCP_SKB_CB(skb).ack_seq;
    let mut seq: u32 = TCP_SKB_CB(skb).seq;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_done_with_error(sk: *mut sock, err: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_reset(sk: *mut sock, skb: *mut sk_buff) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_fin(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_dsack_set(sk: *mut sock, seq: u32, end_seq: u32) {
...
#[no_mangle]
unsafe extern "C" fn tcp_dsack_extend(sk: *mut sock, seq: u32, end_seq: u32) {
...
#[no_mangle]
unsafe extern "C" fn tcp_send_dupack(sk: *mut sock, skb: *const sk_buff) {
    ...
    if (tcp_is_sack(tp) && READ_ONCE(sock_net(sk).ipv4.sysctl_tcp_dsack)) {
    let mut end_seq: u32 = TCP_SKB_CB(skb).end_seq;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_sack_maybe_coalesce(tp: *mut tcp_sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_sack_compress_send_ack(sk: *mut sock) {
...
//
pub const TCP_SACK_BLOCKS_EXPECTED: c_int = 2;
#[no_mangle]
unsafe extern "C" fn tcp_sack_new_ofo_skb(sk: *mut sock, seq: u32, end_seq: u32) {
    static void tcp_sack_new_ofo_skb(struct sock *sk, u32 seq, u32 end_seq)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    struct tcp_sack_block *sp = &tp.selective_acks[0];
    let mut cur_sacks: c_int = tp.rx_opt.num_sacks;
    int this_sack;
    if (!cur_sacks)
    goto new_sack;
...
#[no_mangle]
unsafe extern "C" fn tcp_sack_remove(tp: *mut tcp_sock) {
    static void tcp_sack_remove(struct tcp_sock *tp)
    {
    struct tcp_sack_block *sp = &tp.selective_acks[0];
    let mut num_sacks: c_int = tp.rx_opt.num_sacks;
    int this_sack;
...
// Its possible this segment overlaps with prior segment in queue
    ...
    bool *fragstolen)
    {
    let mut res: bool = tcp_try_coalesce(sk, to, from, fragstolen);
...
//
#[no_mangle]
unsafe extern "C" fn tcp_ofo_queue(sk: *mut sock) {
    static void tcp_ofo_queue(struct sock *sk)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut dsack_high: __u32 = tp.rcv_nxt;
    ...
    let mut dsack: __u32 = dsack_high;
...
#[no_mangle]
unsafe extern "C" fn tcp_can_ingest(sk: *const sock, skb: *const sk_buff) -> bool {
    static bool tcp_can_ingest(const struct sock *sk, const struct sk_buff *skb)
    {
    let mut rmem: c_uint = atomic_read(&sk.sk_rmem_alloc);
...
#[no_mangle]
unsafe extern "C" fn tcp_data_queue_ofo(sk: *mut sock, skb: *mut sk_buff) {
...
// Initial out of order segment, build 1 SACK.
    ...
    rb_link_node(&skb.rbnode, core::ptr::null_mut(), p);
...
// Remove other segments covered by skb.
    while ((skb1 = skb_rb_next(skb)) != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_send_rcvq(sk: *mut sock, msg: *mut msghdr, size: usize) -> c_int {
    ...
    return 0;
    if (size > PAGE_SIZE) {
    let mut npages: c_int = min_t(size_t, size >> PAGE_SHIFT, MAX_SKB_FRAGS);
...
#[no_mangle]
pub unsafe extern "C" fn tcp_data_ready(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_data_queue(sk: *mut sock, skb: *mut sk_buff) {
...
//
    ...
    {
    if (list)
    return !skb_queue_is_last(list, skb) ? skb.next : core::ptr::null_mut();
...
//
    restart:
    for (end_of_skbs = true; skb != core::ptr::null_mut() && skb != tail; skb = n) {
...
// Decided to skip this, advance start seq.
    ...
    let mut copy: c_int = min_t(int, SKB_MAX_ORDER(0, 0), end - start);
    struct sk_buff *nskb;
...
// Copy data, releasing collapsed skbs.
    while (copy > 0) {
    let mut offset: c_int = start - TCP_SKB_CB(skb).seq;
    let mut size: c_int = TCP_SKB_CB(skb).end_seq - start;
    BUG_ON(offset < 0);
    if (size > 0) {
    size = min(copy, size);
...
//
#[no_mangle]
unsafe extern "C" fn tcp_collapse_ofo_queue(sk: *mut sock) {
...
// Do not attempt collapsing tiny skbs
    ...
    tcp_collapse(sk, core::ptr::null_mut(), &tp.out_of_order_queue,
    head, skb, start, end);
    } else {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_prune_ofo_queue(sk: *mut sock, in_skb: *const sk_buff) -> bool {
    static bool tcp_prune_ofo_queue(struct sock *sk, const struct sk_buff *in_skb)
    {
    struct tcp_sock *tp = tcp_sk(sk);
...
//
#[no_mangle]
unsafe extern "C" fn tcp_prune_queue(sk: *mut sock, in_skb: *const sk_buff) -> c_int {
    static int tcp_prune_queue(struct sock *sk, const struct sk_buff *in_skb)
    {
    struct tcp_sock *tp = tcp_sk(sk);
// Do nothing if our queues are empty.
    ...
    tcp_clamp_window(sk);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tcp_under_memory_pressure(sk)) -> else {
    ...
    tcp_collapse(sk, &sk.sk_receive_queue, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn tcp_should_expand_sndbuf(sk: *mut sock) -> bool {
...
// If we are under global TCP memory pressure, do not expand.
    if (tcp_under_memory_pressure(sk)) {
    let mut unused_mem: c_int = sk_unused_reserved_mem(sk);
...
#[no_mangle]
unsafe extern "C" fn tcp_new_space(sk: *mut sock) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __tcp_check_space(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_data_snd_check(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn __tcp_ack_snd_check(sk: *mut sock, ofo_possible: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_ack_snd_check(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_check_urg(sk: *mut sock, th: *const tcphdr) {
    static void tcp_check_urg(struct sock *sk, const struct tcphdr *th)
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut ptr: u32 = ntohs(th.urg_ptr);
...
#[no_mangle]
unsafe extern "C" fn tcp_urg(sk: *mut sock, skb: *mut sk_buff, th: *const tcphdr) {
...
//
#[no_mangle]
unsafe extern "C" fn tcp_reset_check(sk: *const sock, skb: *const sk_buff) -> bool {
...
//
    ...
    {
    struct tcp_sock *tp = tcp_sk(sk);
    let mut accecn_reflector: bool = false;
    SKB_DR(reason);
...
//
    ...
    struct tcp_sack_block *sp = &tp.selective_acks[0];
    let mut max_sack: c_int = sp[0].end_seq;
    int this_sack;
    for (this_sack = 1; this_sack < tp.rx_opt.num_sacks;
    ++this_sack) {
...
//
    ...
    let mut saw_opt: u8 = tcp_accecn_option_init(skb, tp.rx_opt.accecn);
    tcp_accecn_saw_opt_fail_recv(tp, saw_opt);
...
//
#[no_mangle]
pub unsafe extern "C" fn tcp_rcv_established(sk: *mut sock, skb: *mut sk_buff) {
    void tcp_rcv_established(struct sock *sk, struct sk_buff *skb)
    {
    let mut reason: enum skb_drop_reason = SKB_DROP_REASON_NOT_SPECIFIED;
...
//
    ...
    let mut tcp_header_len: c_int = tp.tcp_header_len;
    let mut delta: i32 = 0;
    let mut flag: c_int = 0;
...
//
    ...
    let mut eaten: c_int = 0;
    let mut fragstolen: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn tcp_init_transfer(sk: *mut sock, bpf_op: c_int, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn tcp_finish_connect(sk: *mut sock, skb: *mut sk_buff) {
...
//
    ...
    {
    struct tcp_sock *tp = tcp_sk(sk);
    struct sk_buff *data = tp.syn_data ? tcp_rtx_queue_head(sk) : core::ptr::null_mut();
    let mut mss: u16 = tp.rx_opt.mss_clamp, try_exp = 0;
    let mut syn_drop: bool = false;
    if (mss == READ_ONCE(tp.rx_opt.user_mss)) {
    struct tcp_options_received opt;
// Get original SYNACK MSS value if user MSS sets mss_clamp
    tcp_clear_options(&opt);
    opt.user_mss = opt.mss_clamp = 0;
    tcp_parse_options(sock_net(sk), synack, &opt, 0, core::ptr::null_mut());
    mss = opt.mss_clamp;
    }
    if (!tp.syn_fastopen) {
...
#[no_mangle]
unsafe extern "C" fn smc_check_reset_syn(tp: *mut tcp_sock) {
...
#[no_mangle]
unsafe extern "C" fn tcp_try_undo_spurious_syn(sk: *mut sock) {
...
//
    ...
    struct inet_connection_sock *icsk = inet_csk(sk);
    struct tcp_sock *tp = tcp_sk(sk);
    let mut foc: tcp_fastopen_cookie = { .len = -1 };
    let mut saved_clamp: c_int = tp.rx_opt.mss_clamp;
    bool fastopen_fail;
    SKB_DR(reason);
...
#[no_mangle]
unsafe extern "C" fn tcp_rcv_synrecv_state_fastopen(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn pr_drop_req(req: *mut request_sock, port: __u16, family: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: family ==) -> else {
...
//
    ...
    let mut th_ecn: bool = th.ece && th.cwr;
    bool ect, ecn_ok;
    u32 ecn_ok_dst;
...
//
#[no_mangle]
unsafe extern "C" fn tcp_syn_flood_action(sk: *mut sock, proto: *const c_char) -> bool {
    static bool tcp_syn_flood_action(struct sock *sk, const char *proto)
    {
    struct request_sock_queue *queue = &inet_csk(sk).icsk_accept_queue;
    const char *msg = "Dropping request";
    struct net *net = sock_net(sk);
    let mut want_cookie: bool = false;
    u8 syncookies;
    ...
    let mut len: u32 = skb_network_header_len(skb) + tcp_hdrlen(skb);
...
//
    ...
    {
    let mut foc: tcp_fastopen_cookie = { .len = -1 };
    let mut isn: u32 = TCP_SKB_CB(skb).tcp_tw_isn;
    struct tcp_options_received tmp_opt;
    const struct tcp_sock *tp = tcp_sk(sk);
    struct net *net = sock_net(sk);
    struct sock *fastopen_sk = core::ptr::null_mut();
...
//
    ...
    want_cookie ? core::ptr::null_mut() : &foc);
...
//
    ...
    if (!want_cookie && !isn) {
    let mut max_syn_backlog: c_int = READ_ONCE(net.ipv4.sysctl_max_syn_backlog);
...
//
    ...
    if (tcp_parse_auth_options(tcp_hdr(skb), core::ptr::null_mut(), &aoh))
