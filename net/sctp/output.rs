...
// Forward declarations for private helpers.
    ...
    u16 chunk_len);
#[no_mangle]
unsafe extern "C" fn sctp_packet_reset(packet: *mut sctp_packet) {
...
// Free a packet.
#[no_mangle]
pub unsafe extern "C" fn sctp_packet_free(packet: *mut sctp_packet) {
...
#[no_mangle]
unsafe extern "C" fn sctp_packet_bundle_pad(pkt: *mut sctp_packet, chunk: *mut sctp_chunk) -> enum sctp_xmit {
...
// Try to bundle an auth chunk into the packet.
    ...
    let mut retval: enum sctp_xmit = SCTP_XMIT_OK;
...
//
    ...
    struct sctp_chunk *chunk)
    {
    let mut chunk_len: __u16 = SCTP_PAD4(ntohs(chunk.chunk_hdr.length));
    let mut retval: enum sctp_xmit = SCTP_XMIT_OK;
...
#[no_mangle]
unsafe extern "C" fn sctp_packet_gso_append(head: *mut sk_buff, skb: *mut sk_buff) {
    ...
    {
    struct sctp_transport *tp = packet.transport;
    struct sctp_auth_chunk *auth = core::ptr::null_mut();
    struct sctp_chunk *chunk, *tmp;
    let mut pkt_count: c_int = 0, pkt_size;
    struct sock *sk = head.sk;
    struct sk_buff *nskb;
    let mut auth_len: c_int = 0;
    if (gso) {
...
// calculate the pkt_size and alloc nskb
    ...
    list) {
    let mut padded: c_int = SCTP_PAD4(chunk.skb.len);
    ...
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(tp->pathmtu: pkt_size + padded >) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn sctp_packet_transmit(packet: *mut sctp_packet, gfp: gfp_t) -> c_int {
...
// This private function does management things when adding DATA chunk
    ...
    {
    struct sctp_transport *transport = packet.transport;
    let mut datasize: usize = sctp_data_size(chunk);
    struct sctp_association *asoc = transport.asoc;
    let mut rwnd: u32 = asoc.peer.rwnd;
