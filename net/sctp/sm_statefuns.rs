...
//! Source: net/sctp/sm_statefuns.c
#![no_std]
...
//
    ...
    {
    let mut chunk_length: __u16 = ntohs(chunk.chunk_hdr.length);
...
#[no_mangle]
pub unsafe extern "C" fn sctp_err_chunk_valid(chunk: *mut sctp_chunk) -> bool {
...
//
    ...
    0, 0, 0, core::ptr::null_mut(), GFP_ATOMIC);
    if (ev)
...
// Verify the INIT chunk before processing it.
    ...
    (struct sctp_init_chunk *)chunk.chunk_hdr, chunk,
    &err_chunk)) {
    let mut error: enum sctp_error = SCTP_ERROR_NO_RESOURCE;
// This chunk contains fatal error. It is to be discarded.
...
//
    ...
    struct sctp_cmd_seq *commands)
    {
    struct sctp_ulpevent *ev, *ai_ev = core::ptr::null_mut(), *auth_ev = core::ptr::null_mut();
    struct sctp_chunk *err_chk_p = core::ptr::null_mut();
    struct sctp_association *new_asoc;
...
//
    pkt = sctp_abort_pkt_new(net, ep, core::ptr::null_mut(), init, errhdr, len);
    if (!pkt)
    goto out;
...
//
    ...
    return 0;
    abort = sctp_make_abort(asoc, core::ptr::null_mut(), sizeof(struct sctp_errhdr));
    if (abort) {
...
// This case represents an initialization collision.
    ...
    struct sctp_association *new_asoc)
    {
    struct sctp_ulpevent *ev = core::ptr::null_mut(), *ai_ev = core::ptr::null_mut(), *auth_ev = core::ptr::null_mut();
    struct sctp_chunk *repl;
...
//
    ...
    struct sctp_errhdr *err)
    {
    let mut attempts: c_int = asoc.init_err_counter + 1;
...
//
    ...
    struct sctp_cmd_seq *commands)
    {
    let mut error: __be16 = SCTP_ERROR_NO_ERROR;
    struct sctp_chunk *chunk = arg;
    unsigned int len;
...
//
    ...
    struct sctp_cmd_seq *commands)
    {
    let mut force: union sctp_arg = SCTP_NOFORCE();
    struct sctp_chunk *chunk = arg;
    int error;
    if (!sctp_vtag_verify(chunk, asoc)) {
...
//
    ...
    struct sctp_cmd_seq *commands)
    {
    struct sctp_packet *packet = core::ptr::null_mut();
    struct sctp_chunk *chunk = arg;
    struct sctp_chunk *abort;
...
//
    ...
    struct sctp_errhdr *err;
    let mut ootb_cookie_ack: c_int = 0;
    let mut ootb_shut_ack: c_int = 0;
    __u8 *ch_end;
    SCTP_INC_STATS(net, SCTP_MIB_OUTOFBLUES);
    if (asoc && !sctp_vtag_verify(chunk, asoc))
    asoc = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ootb_cookie_ack) -> else {
...
//
    SCTP_INC_STATS(net, SCTP_MIB_OUTOFBLUES);
    return sctp_sf_shut_8_4_5(net, ep, core::ptr::null_mut(), type, arg, commands);
    }
// ADDIP Section 4.2 Upon reception of an ASCONF Chunk.
    ...
    struct sctp_cmd_seq *commands)
    {
    struct sctp_paramhdr *err_param = core::ptr::null_mut();
    struct sctp_chunk *asconf_ack = core::ptr::null_mut();
    struct sctp_chunk *chunk = arg;
...
//
    asconf_ack.transport = core::ptr::null_mut();
    } else {
...
//
    ...
    ((struct sctp_association *)asoc).new_transport = core::ptr::null_mut();
    }
    return SCTP_DISPOSITION_CONSUME;
    }
...
// Make sure that the RECONF chunk has a valid length.
    ...
    struct sctp_chunk *reply = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(SCTP_PARAM_RESET_IN_REQUEST: param.p->type ==) -> else {
    else if (param.p.type == SCTP_PARAM_RESET_IN_REQUEST)
    reply = sctp_process_strreset_inreq(
    (struct sctp_association *)asoc, param, &ev);
#[no_mangle]
pub unsafe extern "C" fn if(SCTP_PARAM_RESET_TSN_REQUEST: param.p->type ==) -> else {
    else if (param.p.type == SCTP_PARAM_RESET_TSN_REQUEST)
    reply = sctp_process_strreset_tsnreq(
    (struct sctp_association *)asoc, param, &ev);
#[no_mangle]
pub unsafe extern "C" fn if(SCTP_PARAM_RESET_ADD_OUT_STREAMS: param.p->type ==) -> else {
    else if (param.p.type == SCTP_PARAM_RESET_ADD_OUT_STREAMS)
    reply = sctp_process_strreset_addstrm_out(
    (struct sctp_association *)asoc, param, &ev);
#[no_mangle]
pub unsafe extern "C" fn if(SCTP_PARAM_RESET_ADD_IN_STREAMS: param.p->type ==) -> else {
    else if (param.p.type == SCTP_PARAM_RESET_ADD_IN_STREAMS)
    reply = sctp_process_strreset_addstrm_in(
    (struct sctp_association *)asoc, param, &ev);
#[no_mangle]
pub unsafe extern "C" fn if(SCTP_PARAM_RESET_RESPONSE: param.p->type ==) -> else {
...
//
    ...
    struct sctp_chunk *chunk)
    {
    struct sctp_shared_key *sh_key = core::ptr::null_mut();
...
//
    ...
    struct sctp_packet *packet = core::ptr::null_mut();
    struct sctp_chunk *chunk =  arg;
    struct sctp_chunk *abort = core::ptr::null_mut();
// SCTP-AUTH, Section 6.3:
// It should be noted that if the receiver wants to tear
// down an association in an authenticated way only, the
// handling of malformed packets should not result in
...
//
    ...
    {
    let mut attempts: c_int = asoc.init_err_counter + 1;
    struct sctp_chunk *repl = core::ptr::null_mut();
    struct sctp_bind_addr *bp;
...
//
    ...
    if (attempts <= asoc.max_init_attempts) {
    repl = sctp_make_cookie_echo(asoc, core::ptr::null_mut());
    if (!repl)
    return SCTP_DISPOSITION_NOMEM;
    sctp_add_cmd_sf(commands, SCTP_CMD_INIT_CHOOSE_TRANSPORT,
    SCTP_CHUNK(repl));
...
// Note:  CMD_ASSOC_FAILED calls CMD_DELETE_TCB.
    ...
    reply = sctp_make_shutdown(asoc, core::ptr::null_mut());
    break;
    case SCTP_STATE_SHUTDOWN_ACK_SENT:
    reply = sctp_make_shutdown_ack(asoc, core::ptr::null_mut());
    break;
    default:
    BUG();
    break;
...
//
    ...
    reply = sctp_make_abort(asoc, core::ptr::null_mut(), 0);
    if (!reply)
    goto nomem;
    sctp_add_cmd_sf(commands, SCTP_CMD_REPLY, SCTP_CHUNK(reply));
    sctp_add_cmd_sf(commands, SCTP_CMD_SET_SK_ERR,
...
#[no_mangle]
pub unsafe extern "C" fn sctp_ootb_pkt_free(packet: *mut sctp_packet) {
