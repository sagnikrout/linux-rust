...
#[no_mangle]
unsafe extern "C" fn sctp_enter_memory_pressure(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn sctp_wspace(asoc: *mut sctp_association) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn sctp_set_owner_w(chunk: *mut sctp_chunk) {
...
#[no_mangle]
unsafe extern "C" fn sctp_clear_owner_w(chunk: *mut sctp_chunk) {
    ...
    {
    struct sctp_datamsg *msg, *prev_msg = core::ptr::null_mut();
...
// Is this a valid SCTP address?
    if (!af.addr_valid(addr, sctp_sk(sk), core::ptr::null_mut()))
    return -EINVAL;
...
//
    ...
    {
    struct sctp_association *asoc = core::ptr::null_mut();
...
//
    ...
    sctp_assoc_t id)
    {
    struct sctp_association *addr_asoc = core::ptr::null_mut(), *id_asoc = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sctp_auto_asconf_init(sp: *mut sctp_sock) {
...
#[no_mangle]
unsafe extern "C" fn sctp_do_bind(sk: *mut sock, addr: *mut union sctp_addr, len: c_int) -> c_int {
...
//
    ...
    snum = bp.port;
#[no_mangle]
pub unsafe extern "C" fn if(bp->port: snum !=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_bindx_add(sk: *mut sock, addrs: *mut sockaddr, addrcnt: c_int) -> c_int {
...
// Clear the source and route cache
    sctp_transport_route(trans, core::ptr::null_mut(),
...
//
#[no_mangle]
unsafe extern "C" fn sctp_bindx_rem(sk: *mut sock, addrs: *mut sockaddr, addrcnt: c_int) -> c_int {
...
//
    ...
    if (!af.addr_valid(sa_addr, sp, core::ptr::null_mut())) {
...
//
    ...
    addrcnt, sp);
    if ((laddr == core::ptr::null_mut()) && (addrcnt == 1)) {
...
//
    ...
    transports) {
    sctp_transport_route(transport, core::ptr::null_mut(),
    sctp_sk(asoc.base.sk));
    }
    if (stored)
...
#[no_mangle]
pub unsafe extern "C" fn sctp_asconf_mgmt(sp: *mut sctp_sock, addrw: *mut sctp_sockaddr_entry) -> c_int {
...
// tp = sctp_assoc_add_peer(asoc, daddr, GFP_KERNEL, SCTP_UNKNOWN);
    ...
    let mut outcnt: __u16 = init.sinit_num_ostreams;
...
//
    ...
    : -EALREADY;
    err = sctp_connect_new_asoc(ep, daddr, core::ptr::null_mut(), &transport);
    if (err)
    return err;
    asoc = transport.asoc;
    addr_buf += af.sockaddr_len;
...
//
    ...
    err = sctp_primitive_ASSOCIATE(sock_net(sk), asoc, core::ptr::null_mut());
    if (err < 0)
    goto out_free;
...
//
    ...
    int addrs_size)
    {
    return __sctp_setsockopt_connectx(sk, kaddrs, addrs_size, core::ptr::null_mut());
    }
//
...
//
    ...
    int addrs_size)
    {
    let mut assoc_id: sctp_assoc_t = 0;
    let mut err: c_int = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_sctp_getaddrs_old {
    pub assoc_id: sctp_assoc_t,
    pub addr_num: i32,
    pub /: *mut *mut *mut compat_uptr_t addrs; / struct sockaddr,
}
...
//
#[no_mangle]
unsafe extern "C" fn sctp_close(sk: *mut sock, timeout: c_long) {
...
//
    ...
    struct sctp_chunk *chunk;
    chunk = sctp_make_abort_user(asoc, core::ptr::null_mut(), 0);
    sctp_primitive_ABORT(net, asoc, chunk);
    } else
    sctp_primitive_SHUTDOWN(net, asoc, core::ptr::null_mut());
    }
...
// Handle EPIPE error.
#[no_mangle]
unsafe extern "C" fn sctp_error(sk: *mut sock, flags: c_int, err: c_int) -> c_int {
...
// sendv addr list parse
    ...
    let mut wait_connect: bool = false;
    struct sctp_chunk *chunk;
    long timeo;
    int err;
    ...
    if (sctp_state(asoc, CLOSED)) {
    err = sctp_primitive_ASSOCIATE(net, asoc, core::ptr::null_mut());
    if (err)
    goto err;
    ...
    struct sctp_cmsgs *cmsgs)
    {
    union sctp_addr *daddr = core::ptr::null_mut();
    int err;
    if (!sctp_style(sk, UDP_HIGH_BANDWIDTH) && msg.msg_name) {
    let mut len: c_int = msg.msg_namelen;
    if (len > sizeof(*daddr))
    len = sizeof(*daddr);
...
#[no_mangle]
unsafe extern "C" fn sctp_sendmsg(sk: *mut sock, msg: *mut msghdr, msg_len: usize) -> c_int {
    static int sctp_sendmsg(struct sock *sk, struct msghdr *msg, size_t msg_len)
    {
    struct sctp_endpoint *ep = sctp_sk(sk).ep;
    struct sctp_transport *transport = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn sctp_skb_pull(skb: *mut sk_buff, len: c_int) -> c_int {
    static int sctp_skb_pull(struct sk_buff *skb, int len)
    {
    struct sk_buff *list;
    let mut skb_len: c_int = skb_headlen(skb);
    int rlen;
    if (len <= skb_len) {
    __skb_pull(skb, len);
    return 0;
...
//
    ...
    int flags)
    {
    struct sctp_ulpevent *event = core::ptr::null_mut();
    struct sctp_sock *sp = sctp_sk(sk);
...
//
    ...
    unsigned int optlen)
    {
    struct sctp_transport   *trans = core::ptr::null_mut();
    struct sctp_association *asoc = core::ptr::null_mut();
    struct sctp_sock        *sp = sctp_sk(sk);
    int error;
...
#[no_mangle]
pub unsafe extern "C" fn sctp_spp_sackdelay_enable(param_flags: __u32) -> __u32 {
    static inline __u32 sctp_spp_sackdelay_enable(__u32 param_flags)
    {
    return (param_flags & ~SPP_SACKDELAY) | SPP_SACKDELAY_ENABLE;
    }
#[no_mangle]
pub unsafe extern "C" fn sctp_spp_sackdelay_disable(param_flags: __u32) -> __u32 {
...
//
    ...
    if (!af)
    return -EINVAL;
    if (!af.addr_valid((union sctp_addr *)&prim.sspp_addr, sp, core::ptr::null_mut()))
    return -EADDRNOTAVAIL;
...
//
    ...
    authkey.sca_assoc_id == SCTP_ALL_ASSOC) {
    list_for_each_entry(asoc, &ep.asocs, asocs) {
    let mut res: c_int = sctp_auth_set_key(ep, asoc, authkey);
    if (res && !ret)
    ret = res;
    }
    }
...
// srs_number_streams is u16, so optlen can't be bigger than this.
    ...
    param.se_assoc_id == SCTP_ALL_ASSOC) {
    list_for_each_entry(asoc, &sp.ep.asocs, asocs) {
    let mut ret: c_int = sctp_assoc_ulpevent_type_set(param, asoc);
    if (ret && !retval)
    retval = ret;
    }
    }
...
// no transport is found, then the request is invalid.
//
    encap_port = ( __be16)encap.sue_port;
...
// Validate addr_len before calling common connect/connectx routine.
    ...
    err = __sctp_connect(sk, addr, af.sockaddr_len, flags, core::ptr::null_mut());
    release_sock(sk);
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn sctp_disconnect(sk: *mut sock, flags: c_int) -> c_int {
...
// sk_clone() sets refcnt to 2 and increments sockets_allocated
    ...
    newinet.mc_ttl = 1;
    newinet.mc_index = 0;
    newinet.mc_list = core::ptr::null_mut();

    if (sk.sk_family == AF_INET6) {
    struct ipv6_pinfo *newnp;
    newinet.pinet6 = &((struct sctp6_sock *)newsk).inet6;
    newinet.ipv6_fl_list = core::ptr::null_mut();
    newnp = inet6_sk(newsk);
    memcpy(newnp, inet6_sk(sk), sizeof(struct ipv6_pinfo));
    newnp.ipv6_mc_list = core::ptr::null_mut();
    newnp.ipv6_ac_list = core::ptr::null_mut();
    }
...
//
    ...
    {
    struct sctp_association *asoc;
    struct sock *newsk = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sctp_ioctl(sk: *mut sock, cmd: c_int, karg: *mut c_int) -> c_int {
    static int sctp_ioctl(struct sock *sk, int cmd, int *karg)
    {
    let mut rc: c_int = -ENOTCONN;
    lock_sock(sk);
//
...
//
#[no_mangle]
unsafe extern "C" fn sctp_init_sock(sk: *mut sock) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_destroy_sock(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn sctp_destruct_sock(sk: *mut sock) {
    static void sctp_destruct_sock(struct sock *sk)
    {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_shutdown(sk: *mut sock, how: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn sctp_transport_walk_start(__acquires(RCU: *mut *mut rhashtable_iter iter)) {
...
#[no_mangle]
pub unsafe extern "C" fn sctp_transport_walk_stop(__releases(RCU: *mut *mut rhashtable_iter iter)) {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_getsockopt_autoclose(sk: *mut sock, len: c_int, optval: *mut char __user, optlen: *mut int __user) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sctp_getsockopt_peeloff(sk: *mut sock, len: c_int, optval: *mut char __user, optlen: *mut int __user) -> c_int {
    static int sctp_getsockopt_peeloff(struct sock *sk, int len, char __user *optval, int __user *optlen)
    {
    sctp_peeloff_arg_t peeloff;
    struct file *newfile = core::ptr::null_mut();
    let mut retval: c_int = 0;
...
//
#[no_mangle]
unsafe extern "C" fn sctp_getsockopt_initmsg(sk: *mut sock, len: c_int, optval: *mut char __user, optlen: *mut int __user) -> c_int {
...
//
    ...
    let mut hmacid: __u16 = ntohs(hmacs.hmac_ids[i]);
...
//
    ...
    out:
    encap.sue_port = ( uint16_t)encap_port;
...
#[no_mangle]
unsafe extern "C" fn sctp_bpf_bypass_getsockopt(level: c_int, optname: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn sctp_hash(sk: *mut sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sctp_unhash(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn sctp_get_port_local(sk: *mut sock, addr: *mut union sctp_addr) -> c_int {
    static int sctp_get_port_local(struct sock *sk, union sctp_addr *addr)
    {
    struct sctp_sock *sp = sctp_sk(sk);
    let mut reuse: bool = (sk.sk_reuse || sp.reuse);
...
//
    ...
    let mut bound_dev_if2: c_int = READ_ONCE(sk2.sk_bound_dev_if);
...
//
#[no_mangle]
unsafe extern "C" fn sctp_get_port(sk: *mut sock, snum: c_ushort) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_listen_start(sk: *mut sock, backlog: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn sctp_inet_listen(sock: *mut socket, backlog: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn sctp_poll(file: *mut file, sock: *mut socket, wait: *mut poll_table) -> __poll_t {
...
#[no_mangle]
unsafe extern "C" fn sctp_bucket_destroy(pp: *mut sctp_bind_bucket) {
...
#[no_mangle]
pub unsafe extern "C" fn __sctp_put_port(sk: *mut sock) {
    ...
    sctp_sk(sk).bind_hash = core::ptr::null_mut();
    inet_sk(sk).inet_num = 0;
    sctp_bucket_destroy(pp);
    spin_unlock(&head.lock);
    }
#[no_mangle]
pub unsafe extern "C" fn sctp_put_port(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_autobind(sk: *mut sock) -> c_int {
    static int sctp_autobind(struct sock *sk)
    {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_msghdr_parse(msg: *const msghdr, cmsgs: *mut sctp_cmsgs) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_wait_for_packet(sk: *mut sock, err: *mut c_int, timeo_p: *mut c_long) -> c_int {
    static int sctp_wait_for_packet(struct sock *sk, int *err, long *timeo_p)
    {
    int error;
    DEFINE_WAIT(wait);
...
#[no_mangle]
unsafe extern "C" fn __sctp_write_space(asoc: *mut sctp_association) {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_wfree(skb: *mut sk_buff) {
    static void sctp_wfree(struct sk_buff *skb)
    {
...
//
#[no_mangle]
pub unsafe extern "C" fn sctp_sock_rfree(skb: *mut sk_buff) {
...
// Helper function to wait for space in the sndbuf.
    ...
    {
    struct sock *sk = asoc.base.sk;
    let mut current_timeo: c_long = *timeo_p;
    DEFINE_WAIT(wait);
    let mut err: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn sctp_data_ready(sk: *mut sock) {
...
#[no_mangle]
pub unsafe extern "C" fn sctp_write_space(sk: *mut sock) {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_writeable(sk: *const sock) -> bool {
    static bool sctp_writeable(const struct sock *sk)
    {
...
//
#[no_mangle]
unsafe extern "C" fn sctp_wait_for_connect(asoc: *mut sctp_association, timeo_p: *mut c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sctp_wait_for_accept(sk: *mut sock, timeo: c_long) -> c_int {
    static int sctp_wait_for_accept(struct sock *sk, long timeo)
    {
    struct sctp_endpoint *ep;
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn sctp_wait_for_close(sk: *mut sock, timeout: c_long) {
...
#[no_mangle]
unsafe extern "C" fn sctp_skb_set_owner_r_frag(skb: *mut sk_buff, sk: *mut sock) {
...
//
    if (assoc.ulpq.pd_mode)
    sctp_clear_pd(oldsk, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn sctp_v6_destruct_sock(sk: *mut sock) {
    static void sctp_v6_destruct_sock(struct sock *sk)
    {
    inet6_sock_destruct(sk);
    }
#[no_mangle]
unsafe extern "C" fn sctp_v6_init_sock(sk: *mut sock) -> c_int {
    static int sctp_v6_init_sock(struct sock *sk)
    {
    let mut ret: c_int = sctp_init_sock(sk);
    if (!ret)
