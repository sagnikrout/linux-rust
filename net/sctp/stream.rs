...
//
    ...
    ((asoc).strreset_outstanding &= ~SCTP_STRRESET_MASK(type))
#[no_mangle]
unsafe extern "C" fn sctp_stream_shrink_out(stream: *mut sctp_stream, outcnt: __u16) {
...
//
// Mark as failed send.
    sctp_chunk_fail(ch, ( __u32)SCTP_ERROR_INV_STRM);
...
#[no_mangle]
unsafe extern "C" fn sctp_stream_free_ext(stream: *mut sctp_stream, sid: __u16) {
    ...
    SCTP_SO(stream, sid).ext = core::ptr::null_mut();
    }
...
//
    ...
    SCTP_SO(stream, i).ext = core::ptr::null_mut();
    }
    }
...
// Filter out chunks queued on streams that won't exist anymore
    ...
    sctp_stream_outq_migrate(stream, core::ptr::null_mut(), outcnt);
...
#[no_mangle]
pub unsafe extern "C" fn sctp_stream_init_ext(stream: *mut sctp_stream, sid: __u16) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sctp_stream_free(stream: *mut sctp_stream) {
...
#[no_mangle]
pub unsafe extern "C" fn sctp_stream_clear(stream: *mut sctp_stream) {
...
#[no_mangle]
pub unsafe extern "C" fn sctp_stream_update(stream: *mut sctp_stream, new: *mut sctp_stream) {
    ...
    sched.sched_all(stream);
    new.out.tree.root = core::ptr::null_mut();
    new.in.tree.root  = core::ptr::null_mut();
    new.outcnt = 0;
    new.incnt  = 0;
    }
    ...
    let mut sid: __u16 = ntohs(str_list[i]);
    if (SCTP_SO(stream, sid).ext &&
    ...
    asoc.strreset_chunk = core::ptr::null_mut();
    if (!out)
    goto out;
    if (str_nums)
    for (i = 0; i < str_nums; i++)
...
#[no_mangle]
pub unsafe extern "C" fn sctp_send_reset_assoc(asoc: *mut sctp_association) -> c_int {
    int sctp_send_reset_assoc(struct sctp_association *asoc)
    {
    struct sctp_stream *stream = &asoc.stream;
    struct sctp_chunk *chunk = core::ptr::null_mut();
    int retval;
    __u16 i;
    if (!asoc.peer.reconf_capable ||
...
//
    ...
    struct sctp_stream *stream = &asoc.stream;
    let mut result: __u32 = SCTP_STRRESET_DENIED;
    __be16 *str_p = core::ptr::null_mut();
    __u32 request_seq;
    __u16 i, nums;
...
// evp = sctp_ulpevent_make_stream_reset_event(asoc,
    ...
    struct sctp_ulpevent **evp)
    {
    let mut init_tsn: __u32 = 0, next_tsn = 0, max_tsn_seen;
...
// if in progress, do nothing but retransmit
    ...
    return core::ptr::null_mut();
#[no_mangle]
pub unsafe extern "C" fn if(SCTP_STRRESET_DENIED: result ==) -> else {
...
// evp = sctp_ulpevent_make_assoc_reset_event(asoc, flags,
    ...
    sched = sctp_sched_ops_from_stream(stream);
    sched.unsched_all(stream);
    sctp_stream_outq_migrate(stream, core::ptr::null_mut(), number);
    sched.sched_all(stream);
