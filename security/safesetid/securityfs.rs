...
//! Source: security/safesetid/securityfs.c
#![no_std]
...
// Format of |buf| string should be <UID>:<UID> or <GID>:<GID>
    ...
    if (child_str == core::ptr::null_mut())
    return -EINVAL;
// child_str = '\0';
    child_str++;
...
#[no_mangle]
unsafe extern "C" fn release_ruleset(pol: *mut setid_ruleset) {
    call_rcu(&pol.rcu, __release_ruleset);
    }
#[no_mangle]
unsafe extern "C" fn insert_rule(pol: *mut setid_ruleset, rule: *mut setid_rule) {
...
#[no_mangle]
pub unsafe extern "C" fn if(GID: pol->type ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn verify_ruleset(pol: *mut setid_ruleset) -> c_int {
...
// fix it up
    ...
    if (!pol)
    return -ENOMEM;
    pol.policy_str = core::ptr::null_mut();
    ...
    if (pol.policy_str == core::ptr::null_mut()) {
    err = -ENOMEM;
    goto out_free_buf;
    }
...
#[no_mangle]
pub unsafe extern "C" fn safesetid_init_securityfs() -> int __init {
