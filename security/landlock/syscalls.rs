...
//! Source: security/landlock/syscalls.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn build_check_abi() {
...
//
    let mut landlock_abi_version: c_int = 11;
//
...
//
#[no_mangle]
unsafe extern "C" fn get_path_from_fd(fd: i32, path: *const *const path) -> c_int {
    static int get_path_from_fd(const s32 fd, struct path *const path)
    {
    CLASS(fd_raw, f)(fd);
    BUILD_BUG_ON(!__same_type(
    fd, ((struct landlock_path_beneath_attr *)core::ptr::null_mut()).parent_fd));
    if (fd_empty(f))
    return -EBADF;
//
...
//
    ...
    {
    struct landlock_ruleset *ruleset __free(landlock_put_ruleset) = core::ptr::null_mut();
    if (!is_initialized())
    return -EOPNOTSUPP;
...
//
    ...
    {
    struct landlock_ruleset *ruleset __free(landlock_put_ruleset) = core::ptr::null_mut();
    struct landlock_domain *new_dom = core::ptr::null_mut();
