...
//! Source: security/integrity/ima/ima_appraise.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn ima_appraise_parse_cmdline() -> void __init {
    void __init ima_appraise_parse_cmdline(void)
    {
    const char *str = ima_appraise_cmdline_default;
    let mut sb_state: bool = arch_get_secureboot();
    let mut appraisal_state: c_int = ima_appraise;
    if (!str)
    return;
    if (strncmp(str, "off", 3) == 0)
    appraisal_state = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(str, _arg: "log", 0: 3) ==) -> else {
    else if (strncmp(str, "log", 3) == 0)
    appraisal_state = IMA_APPRAISE_LOG;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(str, _arg: "fix", 0: 3) ==) -> else {
    else if (strncmp(str, "fix", 3) == 0)
    appraisal_state = IMA_APPRAISE_FIX;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(str, _arg: "enforce", 0: 7) ==) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn is_ima_appraise_enabled() -> bool {
...
//
    ...
    func, mask, IMA_APPRAISE | IMA_HASH, core::ptr::null_mut(),
    core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn ima_fix_xattr(dentry: *mut dentry, iint: *mut ima_iint_cache) -> c_int {
    static int ima_fix_xattr(struct dentry *dentry, struct ima_iint_cache *iint)
    {
    int rc, offset;
    let mut algo: u8 = iint.ima_hash.algo;
...
#[no_mangle]
unsafe extern "C" fn ima_cache_flags(iint: *mut ima_iint_cache, func: enum ima_hooks) {
...
//
    ...
    {
    struct signature_v2_hdr *sig;
    let mut rc: c_int = -EINVAL, hash_start = 0;
    int mask;
    switch (xattr_value.type) {
    case IMA_XATTR_DIGEST_NG:
// first byte contains algorithm id
...
//
    ...
    const u8 *digest = core::ptr::null_mut();
    ...
    process_buffer_measurement(&nop_mnt_idmap, core::ptr::null_mut(), digest, digestsize,
    "blacklisted-hash", NONE,
    pcr, core::ptr::null_mut(), false, core::ptr::null_mut(), 0);
    return rc;
    }
//
...
//
    ...
    let mut audit_msgno: c_int = AUDIT_INTEGRITY_DATA;
    const char *cause = "unknown";
    struct dentry *dentry = file_dentry(file);
    struct inode *inode = d_backing_inode(dentry);
    let mut status: enum integrity_status = INTEGRITY_UNKNOWN;
    let mut rc: c_int = xattr_len;
    let mut try_modsig: bool = iint.flags & IMA_MODSIG_ALLOWED && modsig;
...
//
#[no_mangle]
pub unsafe extern "C" fn ima_update_xattr(iint: *mut ima_iint_cache, file: *mut file) {
...
// do not collect and update hash for digital signatures
    ...
    !(iint.flags & IMA_HASH))
    return;
    rc = ima_collect_measurement(iint, file, core::ptr::null_mut(), 0, ima_hash_algo, core::ptr::null_mut());
    if (rc < 0)
    return;
...
//
#[no_mangle]
unsafe extern "C" fn ima_reset_appraise_flags(inode: *mut inode, digsig: c_int) {
    ...
    set_bit(IMA_DIGSIG, &iint.atomic_flags);
#[no_mangle]
pub unsafe extern "C" fn if(0: digsig ==) -> else {
    else if (digsig == 0)
    clear_bit(IMA_DIGSIG, &iint.atomic_flags);
    }
//
...
//
    ...
    {
    char *path = core::ptr::null_mut(), *pathbuf = core::ptr::null_mut();
...
// allow any xattr using an algorithm built in the kernel
    ...
    {
    int result, digsig = -1;
    result = ima_protect_xattr(dentry, xattr_name, core::ptr::null_mut(), 0);
    if (result == 1 || evm_revalidate_status(xattr_name)) {
    ...
    const char *acl_name)
    {
    return ima_inode_set_acl(idmap, dentry, acl_name, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn init_ima_appraise_lsm(lsmid: *const lsm_id) -> void __init {
