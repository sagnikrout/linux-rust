...
//
pub const STAGED_REQ_LENGTH: c_int = 21;
...
#[no_mangle]
unsafe extern "C" fn default_canonical_fmt_setup(str: *mut c_char) -> int __init {
    ...
    let mut valid_policy: static int = 1;
...
//
    ...
    rcu_read_unlock();
    (*pos)++;
    return (&qe.later == head) ? core::ptr::null_mut() : qe;
    }
...
#[no_mangle]
unsafe extern "C" fn ima_measurements_stop(m: *mut seq_file, v: *mut c_void) {
    static void ima_measurements_stop(struct seq_file *m, void *v)
    {
    }
#[no_mangle]
pub unsafe extern "C" fn ima_putc(m: *mut seq_file, data: *mut c_void, datalen: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ima_measurements_show(m: *mut seq_file, v: *mut c_void) -> c_int {
...
// the list never shrinks, so we don't need a lock here
    ...
    let mut is_ima_template: bool = false;
    int i, algo_idx;
    algo_idx = ima_sha1_idx;
    if (m.file != core::ptr::null_mut())
...
//
    pcr = !ima_canonical_fmt ? e.pcr : ( u32)cpu_to_le32(e.pcr);
...
// 3rd: template name size
    ...
    ( u32)cpu_to_le32(strlen(template_name));
...
// 5th:  template length (except for 'ima' template)
    ...
    ( u32)cpu_to_le32(e.template_data_len);
    ima_putc(m, &template_data_len, sizeof(e.template_data_len));
    }
// 6th:  template specific data
    for (i = 0; i < e.template_desc.num_fields; i++) {
    let mut show: enum ima_show_type = IMA_SHOW_BINARY;
...
#[no_mangle]
unsafe extern "C" fn ima_measure_lock(write: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ima_measure_unlock(write: bool) {
...
// Decrement additional writes or read/writes.
    ...
    if (write) {
    ima_measure_users++;
    measure_writer = core::ptr::null_mut();
    } else {
    ima_measure_users--;
    }
    mutex_unlock(&ima_measure_mutex);
    ...
    {
    let mut write: bool = (file.f_mode & FMODE_WRITE);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn ima_measurements_open(inode: *mut inode, file: *mut file) -> c_int {
    static int ima_measurements_open(struct inode *inode, struct file *file)
    {
    return _ima_measurements_open(inode, file, &ima_measurments_seqops);
    }
#[no_mangle]
unsafe extern "C" fn ima_measurements_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ima_measurements_staged_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ima_print_digest(m: *mut seq_file, digest: *mut u8, size: u32) {
...
// print in ascii
#[no_mangle]
unsafe extern "C" fn ima_ascii_measurements_show(m: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ima_ascii_measurements_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ima_read_policy(path: *mut c_char) -> isize {
...
// remove \n
    ...
    return rc;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: size) -> else {
    else if (size)
    return -EINVAL;
    else
    return pathlen;
...
// No partial writes.
    ...
    integrity_audit_msg(AUDIT_INTEGRITY_STATUS, core::ptr::null_mut(), core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn create_securityfs_measurement_lists(staging: bool) -> int __init {
    ...
    let mut permissions: umode_t = (S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP);
    const char *file_suffix = "";
    let mut count: c_int = NR_BANKS(ima_tpm_chip);
    if (staging) {
    ...
    let mut algo: u16 = ima_algo_array[i].algo;
...
#[no_mangle]
unsafe extern "C" fn create_securityfs_staging_links() -> int __init {
    ...
    ima_dir, "binary_runtime_measurements_sha1_staged", core::ptr::null_mut());
    if (IS_ERR(dentry))
    return PTR_ERR(dentry);
    dentry = securityfs_create_symlink("ascii_runtime_measurements_staged",
    ima_dir, "ascii_runtime_measurements_sha1_staged", core::ptr::null_mut());
    if (IS_ERR(dentry))
    return PTR_ERR(dentry);
    return 0;
    }
...
//
#[no_mangle]
unsafe extern "C" fn ima_open_policy(inode: *mut inode, filp: *mut file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ima_release_policy(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ima_fs_init() -> int __init {
    ...
    ima_symlink = securityfs_create_symlink("ima", core::ptr::null_mut(), "integrity/ima",
    core::ptr::null_mut());
    ...
    "binary_runtime_measurements_sha1", core::ptr::null_mut());
    if (IS_ERR(dentry)) {
    ret = PTR_ERR(dentry);
    goto out;
    }
    dentry = securityfs_create_symlink("ascii_runtime_measurements", ima_dir,
    "ascii_runtime_measurements_sha1", core::ptr::null_mut());
    if (IS_ERR(dentry)) {
    ret = PTR_ERR(dentry);
    goto out;
    }
    dentry = securityfs_create_file("runtime_measurements_count",
    S_IRUSR | S_IRGRP, ima_dir, core::ptr::null_mut(),
    ...
    ima_dir, core::ptr::null_mut(), &ima_num_violations_ops);
    if (IS_ERR(dentry)) {
    ret = PTR_ERR(dentry);
    goto out;
    }
    dentry = securityfs_create_file("policy", POLICY_FILE_FLAGS,
    ima_dir, core::ptr::null_mut(),
