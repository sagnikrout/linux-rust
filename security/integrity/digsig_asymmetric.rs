...
//! Source: security/integrity/digsig_asymmetric.c
#![no_std]
...
// search in specific keyring
    ...
    key = request_key(&key_type_asymmetric, name, core::ptr::null_mut());
    }
    if (IS_ERR(key)) {
    if (keyring)
...
//
    ...
    {
    let mut file_id: ima_file_id = {.hash_type = type, .hash_algorithm = algo};
    let mut digest_size: usize = hash_digest_size[algo];
