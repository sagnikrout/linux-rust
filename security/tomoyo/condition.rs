...
//! Source: security/tomoyo/condition.c
#![no_std]
...
//
    ...
    char *arg_ptr = ee.tmp;
    let mut arg_len: c_int = 0;
    let mut pos: c_ulong = bprm.p;
    let mut offset: c_int = pos % PAGE_SIZE;
    let mut argv_count: c_int = bprm.argc;
    let mut envp_count: c_int = bprm.envc;
...
// Read.
    ...
    let mut c: c_uchar = kaddr[offset++];
...
//
    ...
    return argv.value != core::ptr::null_mut();
    }
//
...
// cp = '\0';
    ...
    if (!name)
    goto out;
    if (!strcmp(right, "core::ptr::null_mut()")) {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_condition_type(word: *const c_char) -> u8 {
...
//
    ...
    {
    let mut pos: *mut char  const = param.data;
    bool flag;
    if (*pos == '<') {
...
//
    ...
    {
    struct tomoyo_condition *entry = core::ptr::null_mut();
    struct tomoyo_condition_element *condp = core::ptr::null_mut();
    struct tomoyo_number_union *numbers_p = core::ptr::null_mut();
    struct tomoyo_name_union *names_p = core::ptr::null_mut();
    struct tomoyo_argv *argv = core::ptr::null_mut();
    struct tomoyo_envp *envp = core::ptr::null_mut();
    let mut e: tomoyo_condition = { };
    char * const start_of_string =
    tomoyo_get_transit_preference(param, &e);
    let mut end_of_string: *mut char  const = start_of_string + strlen(start_of_string);
    char *pos;
    rerun:
    pos = start_of_string;
    while (1) {
...
#[no_mangle]
pub unsafe extern "C" fn if('=': *mut *mut (right_word + 1) !=) -> else {
...
// right_word++ = '\0'; /* Will restore later.
    ...
    goto out;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(right_word, _arg: "yes")) -> else {
    else if (!strcmp(right_word, "yes"))
    entry.grant_log = TOMOYO_GRANTLOG_YES;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(right_word, _arg: "no")) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_get_attributes(obj: *mut tomoyo_obj_info) {
...
//
    ...
    u16 argc;
    u16 envc;
    struct linux_binprm *bprm = core::ptr::null_mut();
    if (!cond)
    return true;
    condc = cond.condc;
    argc = cond.argc;
    ...
    for (i = 0; i < condc; i++) {
    let mut match: bool = condp.equals;
    let mut left: u8 = condp.left;
    let mut right: u8 = condp.right;
...
// Check string expressions.
    ...
    symlink = obj ? obj.symlink_target : core::ptr::null_mut();
    if (!symlink ||
    ...
    ee = r.ee;
    file = ee ? ee.bprm.file : core::ptr::null_mut();
