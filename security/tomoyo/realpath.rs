...
//! Source: security/tomoyo/realpath.c
#![no_std]
...
//
    ...
    for (i = 0; i < str_len; i++) {
    let mut c: c_uchar = p[i];
    if (c == '\\')
    len += 2;
#[no_mangle]
pub unsafe extern "C" fn if(127: c > ' ' && c <) -> else {
    else if (c > ' ' && c < 127)
    len++;
    else
    len += 4;
...
//
    ...
    {
    return str ? tomoyo_encode2(str, strlen(str)) : core::ptr::null_mut();
    }
//
...
// Convert from $PID to self if $PID is current thread.
    ...
    char *ep;
    let mut pid: pid_t = (pid_t) simple_strtoul(pos + 1, &ep, 10);
...
//
    ...
    let mut buf_len: c_uint = PAGE_SIZE / 2;
