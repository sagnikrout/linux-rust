...
pub struct tomoyo_inet_addr_info {
    pub /: *mut *mut __be16 port; / In network byte order.,
    pub /: *const *const *const __be32 address; / In network byte order.,
    pub is_ipv6: bool,
}
...
pub struct tomoyo_unix_addr_info {
    pub /: *mut *mut *mut u8 addr; / This may not be '\0' terminated string.,
    pub addr_len: c_uint,
}
...
// String table for socket's protocols.
    ...
    [0] = " ", /* Dummy for avoiding core::ptr::null_mut() pointer dereference. */
    [4] = " ", /* Dummy for avoiding core::ptr::null_mut() pointer dereference. */
    };
//
...
//
    ...
    {
    let mut min: *mut u8  const = ptr.ip[0].in6_u.u6_addr8;
    let mut max: *mut u8  const = ptr.ip[1].in6_u.u6_addr8;
...
//
    ...
    let mut perm: u8 = READ_ONCE(*a_perm);
    let mut b_perm: u8 = container_of(b, struct tomoyo_inet_acl, head).perm;
    if (is_delete)
    perm &= ~b_perm;
    else
    perm |= b_perm;
...
//
    ...
    let mut perm: u8 = READ_ONCE(*a_perm);
    let mut b_perm: u8 = container_of(b, struct tomoyo_unix_acl, head).perm;
    if (is_delete)
    perm &= ~b_perm;
    else
    perm |= b_perm;
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_write_inet_network(param: *mut tomoyo_acl_param) -> c_int {
    int tomoyo_write_inet_network(struct tomoyo_acl_param *param)
    {
    let mut e: tomoyo_inet_acl = { .head.type = TOMOYO_TYPE_INET_ACL };
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_write_unix_network(param: *mut tomoyo_acl_param) -> c_int {
    int tomoyo_write_unix_network(struct tomoyo_acl_param *param)
    {
    let mut e: tomoyo_unix_acl = { .head.type = TOMOYO_TYPE_UNIX_ACL };
    int error;
    u8 type;
    const char *protocol = tomoyo_read_token(param);
    const char *operation = tomoyo_read_token(param);
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_audit_inet_log(r: *mut tomoyo_request_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_audit_unix_log(r: *mut tomoyo_request_info) -> c_int {
...
//
    ...
    let mut size: u8 = r.param.inet_network.is_ipv6 ? 16 : 4;
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_inet_entry(address: *const tomoyo_addr_info) -> c_int {
    static int tomoyo_inet_entry(const struct tomoyo_addr_info *address)
    {
    let mut idx: c_int = tomoyo_read_lock();
    struct tomoyo_request_info r;
    let mut error: c_int = 0;
    let mut type: u8 = tomoyo_inet2mac[address.protocol][address.operation];
    if (type && tomoyo_init_request_info(&r, core::ptr::null_mut(), type)
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_unix_entry(address: *const tomoyo_addr_info) -> c_int {
    static int tomoyo_unix_entry(const struct tomoyo_addr_info *address)
    {
    let mut idx: c_int = tomoyo_read_lock();
    struct tomoyo_request_info r;
    let mut error: c_int = 0;
    let mut type: u8 = tomoyo_unix2mac[address.protocol][address.operation];
    if (type && tomoyo_init_request_info(&r, core::ptr::null_mut(), type)
    != TOMOYO_CONFIG_DISABLED) {
    char *buf = address.unix0.addr;
    let mut len: c_int = address.unix0.addr_len - sizeof(sa_family_t);
    if (len <= 0) {
    buf = "anonymous";
    len = 9;
    } else if (buf[0]) {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_kernel_service() -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_sock_family(sk: *mut sock) -> u8 {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_socket_listen_permission(sock: *mut socket) -> c_int {
    int tomoyo_socket_listen_permission(struct socket *sock)
    {
    struct tomoyo_addr_info address;
    let mut family: u8 = tomoyo_sock_family(sock.sk);
    let mut type: c_uint = sock.type;
...
//
    ...
    if (family == PF_UNIX)
#[no_mangle]
pub unsafe extern "C" fn tomoyo_check_unix_address(: *mut (struct sockaddr) -> return {
