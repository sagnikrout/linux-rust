...
//! Source: security/tomoyo/mount.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_audit_mount_log(r: *mut tomoyo_request_info) -> c_int {
...
//
    ...
    __must_hold_shared(&tomoyo_ss)
    {
    let mut obj: tomoyo_obj_info = { };
    struct path path;
    struct file_system_type *fstype = core::ptr::null_mut();
    const char *requested_type = core::ptr::null_mut();
    const char *requested_dir_name = core::ptr::null_mut();
    const char *requested_dev_name = core::ptr::null_mut();
...
//
    ...
    int error;
    int idx;
    if (tomoyo_init_request_info(&r, core::ptr::null_mut(), TOMOYO_MAC_FILE_MOUNT)
