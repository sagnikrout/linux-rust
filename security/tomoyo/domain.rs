...
//! Source: security/tomoyo/domain.c
#![no_std]
...
// ))
    {
    let mut error: c_int = param.is_delete ? -ENOENT : -ENOMEM;
...
// ),
    ...
    const bool))
    {
    let mut is_delete: bool = param.is_delete;
    let mut error: c_int = is_delete ? -ENOENT : -ENOMEM;
    struct tomoyo_acl_info *entry;
    let mut list: *mut list_head  const = param.list;
    if (param.data[0]) {
...
//
    ...
    const u8 type)
    {
    let mut e: tomoyo_transition_control = { .type = type };
    let mut error: c_int = param.is_delete ? -ENOENT : -ENOMEM;
...
//
    ...
    let mut type: enum tomoyo_transition_type = TOMOYO_TRANSITION_CONTROL_NO_RESET;
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_write_aggregator(param: *mut tomoyo_acl_param) -> c_int {
    int tomoyo_write_aggregator(struct tomoyo_acl_param *param)
    {
    let mut e: tomoyo_aggregator = { };
    let mut error: c_int = param.is_delete ? -ENOENT : -ENOMEM;
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_namespace_jump(domainname: *const c_char) -> bool {
    static bool tomoyo_namespace_jump(const char *domainname)
    {
    const char *namespace = tomoyo_current_namespace().name;
    let mut len: c_int = strlen(namespace);
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_environ(ee: *mut tomoyo_execve) -> c_int {
...
// env_page.data is allocated by tomoyo_dump_page().
    let mut env_page: tomoyo_page_dump = { };
...
// Read.
    ...
    let mut c: c_uchar = env_page.data[offset++];
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_find_next_domain(bprm: *mut linux_binprm) -> c_int {
    int tomoyo_find_next_domain(struct linux_binprm *bprm)
    {
    struct tomoyo_domain_info *old_domain = tomoyo_domain();
    struct tomoyo_domain_info *domain = core::ptr::null_mut();
    const char *original_name = bprm.filename;
    let mut retval: c_int = -ENOMEM;
    let mut reject_on_transition_failure: bool = false;
...
// ee->dump->data is allocated by tomoyo_dump_page().
    tomoyo_init_request_info(&ee.r, core::ptr::null_mut(), TOMOYO_MAC_FILE_EXECUTE);
    ee.r.ee = ee;
    ee.bprm = bprm;
    ee.r.obj = &ee.obj;
...
//
    ...
    kfree(exename.name);
    exename.name = core::ptr::null_mut();
    }
    if (!exename.name) {
...
// Normal domain transition.
    ...
    retval = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: reject_on_transition_failure) -> else {
...
//
    ...
    FOLL_FORCE, &page, core::ptr::null_mut());
    ...
    if (page != dump.page) {
    let mut offset: c_uint = pos % PAGE_SIZE;
