...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_bprm_committed_creds(bprm: *const linux_binprm) {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_bprm_creds_for_exec(bprm: *mut linux_binprm) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_bprm_check_security(bprm: *mut linux_binprm) -> c_int {
...
//
    ...
    let mut err: c_int = tomoyo_find_next_domain(bprm);
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_inode_getattr(path: *const path) -> c_int {
    static int tomoyo_inode_getattr(const struct path *path)
    {
    return tomoyo_path_perm(TOMOYO_TYPE_GETATTR, path, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_path_truncate(path: *const path) -> c_int {
    static int tomoyo_path_truncate(const struct path *path)
    {
    return tomoyo_path_perm(TOMOYO_TYPE_TRUNCATE, path, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_file_truncate(file: *mut file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_path_unlink(parent: *const path, dentry: *mut dentry) -> c_int {
    static int tomoyo_path_unlink(const struct path *parent, struct dentry *dentry)
    {
    let mut path: path = { .mnt = parent.mnt, .dentry = dentry };
    return tomoyo_path_perm(TOMOYO_TYPE_UNLINK, &path, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_path_rmdir(parent: *const path, dentry: *mut dentry) -> c_int {
    static int tomoyo_path_rmdir(const struct path *parent, struct dentry *dentry)
    {
    let mut path: path = { .mnt = parent.mnt, .dentry = dentry };
    return tomoyo_path_perm(TOMOYO_TYPE_RMDIR, &path, core::ptr::null_mut());
    }
//
...
//
    ...
    {
    let mut path: path = { .mnt = parent.mnt, .dentry = dentry };
    let mut type: c_int = TOMOYO_TYPE_CREATE;
    let mut perm: c_uint = mode & S_IALLUGO;
    switch (mode & S_IFMT) {
    case S_IFCHR:
...
//
    ...
    {
    let mut path1: path = { .mnt = new_dir.mnt, .dentry = old_dentry };
    let mut path2: path = { .mnt = new_dir.mnt, .dentry = new_dentry };
...
//
    ...
    {
    let mut path1: path = { .mnt = old_parent.mnt, .dentry = old_dentry };
    let mut path2: path = { .mnt = new_parent.mnt, .dentry = new_dentry };
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_file_open(f: *mut file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_path_chmod(path: *const path, mode: umode_t) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_path_chown(path: *const path, uid: kuid_t, gid: kgid_t) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_path_chroot(path: *const path) -> c_int {
    static int tomoyo_path_chroot(const struct path *path)
    {
    return tomoyo_path_perm(TOMOYO_TYPE_CHROOT, path, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_sb_umount(mnt: *mut vfsmount, flags: c_int) -> c_int {
    static int tomoyo_sb_umount(struct vfsmount *mnt, int flags)
    {
    let mut path: path = { .mnt = mnt, .dentry = mnt.mnt_root };
    return tomoyo_path_perm(TOMOYO_TYPE_UMOUNT, &path, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_sb_pivotroot(old_path: *const path, new_path: *const path) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_socket_listen(sock: *mut socket, backlog: c_int) -> c_int {
...
// Lock for GC.
    ...
    let mut __ro_after_init: int tomoyo_enabled = 1;
//
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_init() -> int __init {
