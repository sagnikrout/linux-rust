...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_convert_time(time64: time64_t, stamp: *mut tomoyo_time) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_permstr(string: *const c_char, keyword: *const c_char) -> bool {
    bool tomoyo_permstr(const char *string, const char *keyword)
    {
    const char *cp = strstr(string, keyword);
    if (cp)
    let mut cp: return = = string || *(cp - 1) == '/';
    return false;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_parse_ulong(result: *mut c_ulong, str: *mut c_char) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn if(TOMOYO_VALUE_TYPE_OCTAL: type ==) -> else {
    else if (type == TOMOYO_VALUE_TYPE_OCTAL)
    snprintf(buffer, buffer_len, "0%lo", value);
#[no_mangle]
pub unsafe extern "C" fn if(TOMOYO_VALUE_TYPE_HEXADECIMAL: type ==) -> else {
...
//
    ...
    return ptr.group != core::ptr::null_mut();
    }
    filename = tomoyo_read_token(param);
    if (!tomoyo_correct_word(filename))
    return false;
    ptr.filename = tomoyo_get_name(filename);
    return ptr.filename != core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_byte_range(str: *const c_char) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_alphabet_char(c: c_char) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_make_byte(c1: u8, c2: u8, c3: u8) -> u8 {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_valid(c: c_uchar) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_invalid(c: c_uchar) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_str_starts(src: *mut c_char, find: *const c_char) -> bool {
    bool tomoyo_str_starts(char **src, const char *find)
    {
    let mut len: c_int = strlen(find);
    char *tmp = *src;
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_normalize_line(buffer: *mut c_uchar) {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_correct_word2(string: *const c_char, len: usize) -> bool {
    static bool tomoyo_correct_word2(const char *string, size_t len)
    {
    let mut recursion: u8 = 20;
    let mut start: *const char const = string;
    let mut in_repetition: bool = false;
    if (!len)
    goto out;
    while (len--) {
    let mut c: c_uchar = *string++;
    if (c == '\\') {
    if (!len--)
    goto out;
    c = *string++;
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_correct_word(string: *const c_char) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_correct_path2(filename: *const c_char, len: usize) -> bool {
    static bool tomoyo_correct_path2(const char *filename, const size_t len)
    {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_correct_path(filename: *const c_char) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_correct_domain(domainname: *const c_uchar) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_domain_def(buffer: *const c_uchar) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_const_part_length(filename: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_fill_path_info(ptr: *mut tomoyo_path_info) {
    ...
    ptr.hash = full_name_hash(core::ptr::null_mut(), name, len);
    }
//
...
//
    ...
    filename++;
#[no_mangle]
pub unsafe extern "C" fn if(1): tomoyo_byte_range(filename +) -> else {
    ...
    i++;
#[no_mangle]
pub unsafe extern "C" fn if(1): tomoyo_byte_range(filename + i +) -> else {
    ...
    (*(pattern + 1) == '*' || *(pattern + 1) == '@'))
    pattern += 2;
    let mut filename: return = = filename_end && pattern == pattern_end;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_path_matches_pattern2(f: *const c_char, p: *const c_char) -> bool {
...
//
    ...
    let mut len: c_int = pattern.const_len;
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_domain_quota_is_ok(r: *mut tomoyo_request_info) -> bool {
