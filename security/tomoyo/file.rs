...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_put_name_union(ptr: *mut tomoyo_name_union) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_put_number_union(ptr: *mut tomoyo_number_union) {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_add_slash(buf: *mut tomoyo_path_info) {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_get_realpath(buf: *mut tomoyo_path_info, path: *const path) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_audit_path_log(r: *mut tomoyo_request_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_audit_path2_log(r: *mut tomoyo_request_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_audit_mkdev_log(r: *mut tomoyo_request_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_audit_path_number_log(r: *mut tomoyo_request_info) -> c_int {
    static int tomoyo_audit_path_number_log(struct tomoyo_request_info *r)
    __must_hold_shared(&tomoyo_ss)
    {
    let mut type: u8 = r.param.path_number.operation;
    u8 radix;
    char buffer[64];
    switch (type) {
...
//
    ...
    &acl.name);
    return r.param.path.matched_path != core::ptr::null_mut();
    }
    return false;
    }
//
...
//
    ...
    .perm;
    let mut perm: u16 = READ_ONCE(*a_perm);
    let mut b_perm: u16 = container_of(b, struct tomoyo_path_acl, head).perm;
    if (is_delete)
    perm &= ~b_perm;
    else
    perm |= b_perm;
...
//
    ...
    .perm;
    let mut perm: u8 = READ_ONCE(*a_perm);
    let mut b_perm: u8 = container_of(b, struct tomoyo_path2_acl, head).perm;
    if (is_delete)
    perm &= ~b_perm;
    else
    perm |= b_perm;
...
//
    ...
    r.matched_acl.cond.transit : core::ptr::null_mut();
    if (r.mode != TOMOYO_CONFIG_DISABLED)
...
//
    ...
    int idx;
    if (tomoyo_init_request_info(&r, core::ptr::null_mut(), tomoyo_pn2mac[type])
...
//
    ...
    {
    let mut acc_mode: u8 = ACC_MODE(flag);
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_path_perm(operation: u8, path: *const path, target: *const c_char) -> c_int {
    ...
    int idx;
    if (tomoyo_init_request_info(&r, core::ptr::null_mut(), tomoyo_p2mac[operation])
    == TOMOYO_CONFIG_DISABLED)
    return 0;
...
//
    ...
    struct tomoyo_path_info buf;
    int idx;
    if (tomoyo_init_request_info(&r, core::ptr::null_mut(), tomoyo_pnnn2mac[operation])
    == TOMOYO_CONFIG_DISABLED)
    return 0;
    idx = tomoyo_read_lock();
    error = -ENOMEM;
...
//
    ...
    int idx;
    if (tomoyo_init_request_info(&r, core::ptr::null_mut(), tomoyo_pp2mac[operation])
    == TOMOYO_CONFIG_DISABLED)
    return 0;
    buf1.name = core::ptr::null_mut();
    buf2.name = core::ptr::null_mut();
    idx = tomoyo_read_lock();
...
//
#[no_mangle]
unsafe extern "C" fn tomoyo_update_mount_acl(param: *mut tomoyo_acl_param) -> c_int {
    static int tomoyo_update_mount_acl(struct tomoyo_acl_param *param)
    {
    let mut e: tomoyo_mount_acl = { .head.type = TOMOYO_TYPE_MOUNT_ACL };
    int error;
    ...
    error = tomoyo_update_domain(&e.head, sizeof(e), param,
    tomoyo_same_mount_acl, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_write_file(param: *mut tomoyo_acl_param) -> c_int {
