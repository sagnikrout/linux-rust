...
//! Source: security/tomoyo/audit.c
#![no_std]
...
//
    ...
    {
    let mut tomoyo_buffer_len: static int = 4096 * 2;
...
//
    ...
    let mut gpid: pid_t = task_pid_nr(current);
    struct tomoyo_obj_info *obj = r.obj;
    let mut tomoyo_buffer_len: static int = 4096;
...
//
    ...
    char *bprm_info = core::ptr::null_mut();
    const char *header = core::ptr::null_mut();
    char *realpath = core::ptr::null_mut();
    const char *symlink = core::ptr::null_mut();
    int pos;
...
//
    ...
    let mut quota_exceeded: bool = false;
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_write_log(r: *mut tomoyo_request_info, fmt: *const c_char, ...) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_read_log(head: *mut tomoyo_io_buffer) {
    void tomoyo_read_log(struct tomoyo_io_buffer *head)
    {
    struct tomoyo_log *ptr = core::ptr::null_mut();
    if (head.r.w_pos)
    return;
    kfree(head.read_buf);
    head.read_buf = core::ptr::null_mut();
    spin_lock(&tomoyo_log_lock);
...
//
#[no_mangle]
pub unsafe extern "C" fn tomoyo_poll_log(file: *mut file, wait: *mut poll_table) -> __poll_t {
