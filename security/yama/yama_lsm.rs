...
//! Source: security/yama/yama_lsm.c
#![no_std]
...
pub const YAMA_SCOPE_DISABLED: c_int = 0;
pub const YAMA_SCOPE_RELATIONAL: c_int = 1;
pub const YAMA_SCOPE_CAPABILITY: c_int = 2;
pub const YAMA_SCOPE_NO_ATTACH: c_int = 3;
    let mut ptrace_scope: static int = YAMA_SCOPE_RELATIONAL;
// describe a ptrace relationship for potential exception
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ptrace_relation {
    pub tracer: *mut task_struct,
    pub tracee: *mut task_struct,
    pub invalid: bool,
    pub node: list_head,
    pub rcu: rcu_head,
}
...
//
#[no_mangle]
unsafe extern "C" fn yama_task_free(task: *mut task_struct) {
...
//
    ...
    if (arg2 == 0) {
    yama_ptracer_del(core::ptr::null_mut(), myself);
    rc = 0;
    } else if (arg2 == PR_SET_PTRACER_ANY || (int)arg2 == -1) {
    rc = yama_ptracer_add(core::ptr::null_mut(), myself);
    } else {
...
//
    ...
    let mut rc: c_int = 0;
    struct ptrace_relation *relation;
    struct task_struct *parent = core::ptr::null_mut();
...
//
    ...
    if (parent != core::ptr::null_mut() && same_thread_group(parent, tracer)) {
    rc = 1;
    goto unlock;
    }
// Look for a PR_SET_PTRACER relationship.
    ...
    if (found && (parent == core::ptr::null_mut() || task_is_descendant(parent, tracer)))
    rc = 1;
    unlock:
    rcu_read_unlock();
    return rc;
...
//
#[no_mangle]
unsafe extern "C" fn yama_ptrace_traceme(parent: *mut task_struct) -> c_int {
...
// Lock the max value if it ever gets set.
    ...
    let mut max_scope: static int = YAMA_SCOPE_NO_ATTACH;
...
#[no_mangle]
unsafe extern "C" fn yama_init_sysctl() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn yama_init() -> int __init {
