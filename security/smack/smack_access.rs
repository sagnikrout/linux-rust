...
//! Source: security/smack/smack_access.c
#![no_std]
...
// known values above.
//
    let mut smack_next_secid: static u32 = 10;

//
// what events do we log
// can be overwritten at run-time by /smack/logging
//
    let mut log_policy: c_int = SMACK_AUDIT_DENIED;
...
//
    ...
    let mut may: c_int = srp.smk_access;
//
...
//
    rc = smk_access(sbj_known, obj_known, mode, core::ptr::null_mut());
    if (rc >= 0) {
//
...
// check if we have to log the current event
    ...
    if (sad.function == core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn if(SMACK_UNCONFINED_OBJECT: result ==) -> else {
    ...
    sad.result  = result;
    common_lsm_audit(a, smack_log_callback, core::ptr::null_mut());
    }
...
pub unsafe extern "C" fn smk_insert_entry(skp: *mut smack_known) {
    ...
    hash = full_name_hash(core::ptr::null_mut(), skp.smk_known, strlen(skp.smk_known));
...
//
    ...
    hash = full_name_hash(core::ptr::null_mut(), string, strlen(string));
    head = &smack_known_hash[hash & (SMACK_HASH_SLOTS - 1)];
...
//
#[no_mangle]
pub unsafe extern "C" fn smk_parse_label_len(string: *const c_char, len: c_int) -> c_int {
...
//
    ...
    {
    char *smack;
    let mut i: c_int = smk_parse_label_len(string, len);
    if (i < 0)
...
//
    ...
    sap.attr.mls.cat = core::ptr::null_mut();
...
pub unsafe extern "C" fn smack_populate_secattr(skp: *mut smack_known) -> c_int {
    ...
    if (skp.smk_netlabel.cache != core::ptr::null_mut()) {
    skp.smk_netlabel.flags |= NETLBL_SECATTR_CACHE;
    skp.smk_netlabel.cache.free = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn smack_privileged_cred(cap: c_int, cred: *const cred) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn smack_privileged(cap: c_int) -> bool {
