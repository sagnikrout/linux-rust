...
//
    ...
    {
    struct key_user *candidate = core::ptr::null_mut(), *user;
...
// search the tree for a user record with a matching UID
    ...
    p = &(*p).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: uid_gt(uid, _arg: user->uid)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn key_alloc_serial(key: *mut key) {
...
// serial number tree
    ...
    p = &(*p).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(xkey->serial: key->serial >) -> else {
...
//
    ...
    {
    struct key_user *user = core::ptr::null_mut();
    struct key *key;
...
//
#[no_mangle]
pub unsafe extern "C" fn key_payload_reserve(key: *mut key, datalen: usize) -> c_int {
    int key_payload_reserve(struct key *key, size_t datalen)
    {
    let mut delta: c_int = (int)datalen - key.datalen;
    let mut ret: c_int = 0;
    key_check(key);
...
//
#[no_mangle]
unsafe extern "C" fn mark_key_instantiated(key: *mut key, reject_error: c_int) {
...
// wake up anyone waiting for a key to be constructed
    if (awaken)
    wake_up_bit(&key.flags, KEY_FLAG_USER_CONSTRUCT);
    let mut ret: return = = 0 ? link_ret : ret;
    }
...
// search the tree for the specified key
    ...
    n = n.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(key->serial: id >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn key_set_timeout(key: *mut key, timeout: unsigned) {
...
//
#[no_mangle]
pub unsafe extern "C" fn key_type_put(ktype: *mut key_type) {
...
//
    ...
    struct key *keyring, *key = core::ptr::null_mut();
    key_ref_t key_ref;
    int ret;
    struct key_restriction *restrict_link = core::ptr::null_mut();
// look up the key type to see if it's one of the registered kernel
// types
...
// allocate a new key
    ...
    cred.fsuid, cred.fsgid, cred, perm, flags, core::ptr::null_mut());
    if (IS_ERR(key)) {
    key_ref = ERR_CAST(key);
    goto error_link_end;
    }
// instantiate it and link it into the target keyring
    ret = __key_instantiate_and_link(key, &prep, keyring, core::ptr::null_mut(), &edit);
    if (ret < 0) {
    key_put(key);
    key_ref = ERR_PTR(ret);
    goto error_link_end;
...
//
#[no_mangle]
pub unsafe extern "C" fn key_update(key_ref: key_ref_t, payload: *const c_void, plen: usize) -> c_int {
...
pub unsafe extern "C" fn generic_key_instantiate(key: *mut key, prep: *mut key_preparsed_payload) -> c_int {
    ...
    prep.payload.data[1] = core::ptr::null_mut();
    prep.payload.data[2] = core::ptr::null_mut();
    prep.payload.data[3] = core::ptr::null_mut();
    }
    pr_devel("<==%s() = %d\n", __func__, ret);
    return ret;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn key_init() -> void __init {
    void __init key_init(void)
    {
// allocate a slab in which we can store keys
    key_jar = kmem_cache_create("key_jar", sizeof(struct key),
    0, SLAB_HWCACHE_ALIGN | SLAB_PANIC | SLAB_NO_MERGE, core::ptr::null_mut());
