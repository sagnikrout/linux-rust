...
//! Source: security/keys/process_keys.c
#![no_std]
...
//
    ...
    let mut uid: uid_t = from_kuid(user_ns, cred.user.uid);
    char buf[20];
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn install_thread_keyring() -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn install_process_keyring() -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn install_session_keyring_to_cred(cred: *mut cred, keyring: *mut key) -> c_int {
...
// create an empty session keyring
    ...
    flags, core::ptr::null_mut(), core::ptr::null_mut());
    if (IS_ERR(keyring))
    return PTR_ERR(keyring);
    } else {
    __key_get(keyring);
...
//
#[no_mangle]
unsafe extern "C" fn install_session_keyring(keyring: *mut key) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(get_user_session_keyring_rcu(cred)): (user_session =) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn search_process_keyrings_rcu(ctx: *mut keyring_search_context) -> key_ref_t {
...
#[no_mangle]
pub unsafe extern "C" fn if(ERR_PTR(-EACCES): err ==) -> else {
...
//
    ...
    {
    let mut key: return = = match_data.raw_data;
    }
//
...
// doesn't exist yet
    ret = look_up_user_keyrings(core::ptr::null_mut(), &user_session);
    if (ret < 0)
    goto error;
    if (lflags & KEY_LOOKUP_CREATE)
    ret = join_session_keyring(core::ptr::null_mut());
    else
    ...
    ret = look_up_user_keyrings(&key, core::ptr::null_mut());
    if (ret < 0)
    goto error;
    key_ref = make_key_ref(key, 1);
    break;
    case KEY_SPEC_USER_SESSION_KEYRING:
    ret = look_up_user_keyrings(core::ptr::null_mut(), &key);
    if (ret < 0)
    goto error;
    key_ref = make_key_ref(key, 1);
    break;
...
// if no name is provided, install an anonymous keyring
    if (!name) {
    ret = install_session_keyring_to_cred(new, core::ptr::null_mut());
    if (ret < 0)
    goto error;
...
// not found - try and create a new one
    ...
    KEY_ALLOC_IN_QUOTA, core::ptr::null_mut(), core::ptr::null_mut());
    if (IS_ERR(keyring)) {
    ret = PTR_ERR(keyring);
    goto error2;
    }
...
//
#[no_mangle]
unsafe extern "C" fn init_root_keyring() -> int __init {
    static int __init init_root_keyring(void)
    {
    return look_up_user_keyrings(core::ptr::null_mut(), core::ptr::null_mut());
    }
