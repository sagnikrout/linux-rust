...
//! Source: security/keys/trusted-keys/trusted_tpm2.c
#![no_std]
...
//
    ...
    u8 *src, u32 len)
    {
    let mut SCRATCH_SIZE: c_int = PAGE_SIZE;
...
// buf = blob;
    ...
    const void *value, size_t vlen)
    {
    let mut oid: enum OID = look_up_OID(value, vlen);
...
//
    ...
    struct tpm_buf *sized __free(kfree) = core::ptr::null_mut();
    let mut blob_len: c_int = 0;
    int hash;
    u32 flags;
    int rc;
...
//
    ...
    u32 *blob_handle)
    {
    u8 *blob_ref __free(kfree) = core::ptr::null_mut();
    struct tpm_buf *buf __free(kfree) = core::ptr::null_mut();
