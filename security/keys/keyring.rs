...
//! Source: security/keys/keyring.c
#![no_std]
...
//
pub const KEYRING_SEARCH_MAX_DEPTH: c_int = 6;
//
// We mark pointers we pass to the associative array with bit 1 set if
// they're keyrings and clear otherwise.
//
pub const KEYRING_PTR_SUBTYPE: c_uint = 0x2UL;
#[no_mangle]
pub unsafe extern "C" fn keyring_ptr_is_keyring(x: *const assoc_array_ptr) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn keyring_free_preparse(prep: *mut key_preparsed_payload) {
    static void keyring_free_preparse(struct key_preparsed_payload *prep)
    {
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn mult_64x32_and_fold(x: u64, y: u32) -> u64 {
    static u64 mult_64x32_and_fold(u64 x, u32 y)
    {
    let mut hi: u64 = (u64)(u32)(x >> 32) * y;
    let mut lo: u64 = (u64)(u32)(x) * y;
...
//
#[no_mangle]
unsafe extern "C" fn hash_key_type_and_desc(index_key: *mut keyring_index_key) {
    static void hash_key_type_and_desc(struct keyring_index_key *index_key)
    {
    let mut level_shift: unsigned = ASSOC_ARRAY_LEVEL_STEP;
    let mut fan_mask: c_ulong = ASSOC_ARRAY_FAN_MASK;
...
#[no_mangle]
pub unsafe extern "C" fn if(0: index_key->type == &key_type_keyring && (hash & fan_mask) !=) -> else {
...
pub unsafe extern "C" fn key_set_index_key(index_key: *mut keyring_index_key) {
    ...
    {
    let mut default_domain_tag: static struct key_tag = { .usage = REFCOUNT_INIT(1), };
    let mut n: usize = min_t(size_t, index_key.desc_len, sizeof(index_key.desc));
...
//
#[no_mangle]
pub unsafe extern "C" fn key_put_tag(tag: *mut key_tag) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn keyring_get_key_chunk(data: *const c_void, level: c_int) -> c_ulong {
    static unsigned long keyring_get_key_chunk(const void *data, int level)
    {
    const struct keyring_index_key *index_key = data;
    let mut chunk: c_ulong = 0;
    const u8 *d;
    let mut desc_len: c_int = index_key.desc_len, n = sizeof(chunk);
    unsigned int offset;
...
#[no_mangle]
unsafe extern "C" fn keyring_compare_object(object: *const c_void, data: *const c_void) -> bool {
...
unsafe extern "C" fn keyring_destroy(keyring: *mut key) {
    ...
    if (keyring.description) {
    write_lock(&keyring_name_lock);
    if (keyring.name_link.next != core::ptr::null_mut() &&
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct keyring_read_iterator_context {
    pub buflen: usize,
    pub count: usize,
    pub buffer: *mut key_serial_t,
}
...
//
    ...
    if (!IS_ERR(keyring)) {
    ret = key_instantiate_and_link(keyring, core::ptr::null_mut(), 0, dest, core::ptr::null_mut());
    if (ret < 0) {
    key_put(keyring);
    keyring = ERR_PTR(ret);
    }
...
unsafe extern "C" fn keyring_search_iterator(object: *const c_void, iterator_data: *mut c_void) -> c_int {
    ...
    const struct key *key = keyring_ptr_to_key(object);
    let mut kflags: c_ulong = READ_ONCE(key.flags);
    let mut state: c_short = key_read_state(key);
    kenter("{%d}", key.serial);
...
//
#[no_mangle]
unsafe extern "C" fn search_keyring(keyring: *mut key, ctx: *mut keyring_search_context) -> c_int {
...
//
    ...
    {
    struct key *keyring;
    struct key_type *restrict_type = core::ptr::null_mut();
    struct key_restriction *restrict_link;
    let mut ret: c_int = 0;
    keyring = key_ref_to_ptr(keyring_ref);
    key_check(keyring);
...
//
    ...
    if (object)
    goto found;
    kleave(" = core::ptr::null_mut()");
    return core::ptr::null_mut();
    found:
    key = keyring_ptr_to_key(object);
    if (key.flags & ((1 << KEY_FLAG_INVALIDATED) |
    (1 << KEY_FLAG_REVOKED))) {
    kleave(" = core::ptr::null_mut() [x]");
    return core::ptr::null_mut();
    }
    __key_get(key);
    kleave(" = {%d}", key.serial);
...
//
#[no_mangle]
unsafe extern "C" fn keyring_detect_cycle(A: *mut key, B: *mut key) -> c_int {
...
//
    ...
    BUG_ON(*_edit != core::ptr::null_mut());
// _edit = NULL;
    ret = -EKEYREVOKED;
    if (test_bit(KEY_FLAG_REVOKED, &keyring.flags))
    goto error;
...
//
    ...
    {
    BUG_ON(index_key.type == core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn __key_link_check_restriction(keyring: *mut key, key: *mut key) -> c_int {
...
pub unsafe extern "C" fn key_link(keyring: *mut key, key: *mut key) -> c_int {
    ...
    {
    struct assoc_array_edit *edit = core::ptr::null_mut();
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn __key_unlink_lock(keyring: *mut key) -> c_int {
...
//
    ...
    unsigned int flags)
    {
    struct assoc_array_edit *from_edit = core::ptr::null_mut(), *to_edit = core::ptr::null_mut();
    int ret;
...
#[no_mangle]
unsafe extern "C" fn keyring_gc_select_iterator(object: *mut c_void, iterator_data: *mut c_void) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn keyring_gc(keyring: *mut key, limit: time64_t) {
...
// Lock the keyring to ensure that a link is not in progress
    ...
    key_put(keyres.key);
    keyres.key = core::ptr::null_mut();
    keyres.keytype = core::ptr::null_mut();
    up_write(&keyring.sem);
