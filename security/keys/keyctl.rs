...
pub const KEY_MAX_DESC_SIZE: c_int = 4096;
...
// pull the callout info into kernel space
    callout_info = core::ptr::null_mut();
    callout_len = 0;
    if (_callout_info) {
...
// do the search
    key = request_key_and_link(ktype, description, core::ptr::null_mut(), callout_info,
    callout_len, core::ptr::null_mut(), key_ref_to_ptr(dest_ref),
    KEY_ALLOC_IN_QUOTA);
    if (IS_ERR(key)) {
    ret = PTR_ERR(key);
    goto error5;
...
//
#[no_mangle]
pub unsafe extern "C" fn keyctl_join_session_keyring(_name: *const char __user) -> c_long {
...
//
#[no_mangle]
unsafe extern "C" fn __keyctl_read_key(key: *mut key, buffer: *mut c_char, buflen: usize) -> c_long {
...
//
#[no_mangle]
pub unsafe extern "C" fn keyctl_read_key(keyid: key_serial_t, buffer: *mut char __user, buflen: usize) -> c_long {
    long keyctl_read_key(key_serial_t keyid, char __user *buffer, size_t buflen)
    {
    struct key *key;
    key_ref_t key_ref;
    long ret;
    char *key_data = core::ptr::null_mut();
    size_t key_data_len;
// find the key first
...
// Get the key length from the read method
    ret = __keyctl_read_key(key, core::ptr::null_mut(), 0);
    goto key_put_out;
    }
//
...
pub unsafe extern "C" fn keyctl_chown_key(id: key_serial_t, user: uid_t, group: gid_t) -> c_long {
    ...
    {
    struct key_user *newowner, *zapowner = core::ptr::null_mut();
    struct key *key;
    key_ref_t key_ref;
    long ret;
    kuid_t uid;
...
// make the changes with the locks held to prevent chown/chown races
    ...
    down_write(&key.sem);
    {
    let mut is_privileged_op: bool = false;
...
//
#[no_mangle]
unsafe extern "C" fn keyctl_change_reqkey_auth(key: *mut key) -> c_int {
...
//
    ...
    let mut plen: usize = from ? iov_iter_count(from) : 0;
    void *payload;
    long ret;
...
// instantiation of the key
    if (ret == 0)
    keyctl_change_reqkey_auth(core::ptr::null_mut());
    error2:
    kvfree_sensitive(payload, plen);
    error_put_rka:
...
//
    ...
    return keyctl_instantiate_key_common(id, core::ptr::null_mut(), ringid);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn keyctl_negate_key(id: key_serial_t, timeout: unsigned, ringid: key_serial_t) -> c_long {
...
//
#[no_mangle]
pub unsafe extern "C" fn keyctl_set_timeout(id: key_serial_t, timeout: unsigned) -> c_long {
...
// we divest ourselves of authority if given an ID of 0
    if (id == 0) {
    ret = keyctl_change_reqkey_auth(core::ptr::null_mut());
    goto error;
    }
...
// our parent
    ...
    keyring_r = core::ptr::null_mut();
    init_task_work(newwork, key_change_session_keyring);
    me = current;
    rcu_read_lock();
    write_lock_irq(&tasklist_lock);
    ret = -EPERM;
    oldwork = core::ptr::null_mut();
...
//
    ...
    key_ref_t key_ref;
    char type[32];
    char *restriction = core::ptr::null_mut();
    long ret;
    key_ref = lookup_user_key(id, 0, KEY_NEED_SETATTR);
    if (IS_ERR(key_ref))
    return PTR_ERR(key_ref);
    ...
    ret = keyring_restrict(key_ref, _type ? type : core::ptr::null_mut(), restriction);
    kfree(restriction);
    error:
    key_ref_put(key_ref);
    return ret;
...
pub unsafe extern "C" fn keyctl_watch_key(id: key_serial_t, watch_queue_fd: c_int, watch_id: c_int) -> c_long {
    ...
    struct watch_list *wlist = core::ptr::null_mut();
    struct watch *watch = core::ptr::null_mut();
    struct key *key;
    key_ref_t key_ref;
    long ret;
    ...
    if (!wlist)
    goto err_wqueue;
    init_watch_list(wlist, core::ptr::null_mut());
    }
    ...
    if (!key.watchers) {
    key.watchers = wlist;
    wlist = core::ptr::null_mut();
    }
    ret = add_watch_to_object(watch, key.watchers);
    up_write(&key.sem);
    if (ret == 0)
    watch = core::ptr::null_mut();
    } else {
    ret = -EBADSLT;
    if (key.watchers) {
    down_write(&key.sem);
...
//
#[no_mangle]
pub unsafe extern "C" fn keyctl_capabilities(_buffer: *mut unsigned char __user, buflen: usize) -> c_long {
    long keyctl_capabilities(unsigned char __user *_buffer, size_t buflen)
    {
    let mut size: usize = buflen;
    if (size > 0) {
