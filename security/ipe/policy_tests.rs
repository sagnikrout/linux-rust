...
//! Source: security/ipe/policy_tests.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct policy_case {
    pub policy: *const *const c_char,
    pub errno: c_int,
    pub desc: *const *const c_char,
}
    ...
    -EBADMSG,
    "incomplete policy from core::ptr::null_mut()",
    },
    {
    "policy_name=test policy_version=0.0.0\n"
    "DEFAULT action=DENY\n\0"
    ...
    0,
    "core::ptr::null_mut() truncates policy",
    },
    {
    "policy_name=test policy_version=0.0.0\n"
    "DEFAULT action=ALLOW\n"
...
#[no_mangle]
unsafe extern "C" fn pol_to_desc(c: *const policy_case, desc: *mut c_char) {
...
//
#[no_mangle]
unsafe extern "C" fn ipe_parser_unsigned_test(test: *mut kunit) {
    static void ipe_parser_unsigned_test(struct kunit *test)
    {
    const struct policy_case *p = test.param_value;
    struct ipe_policy *pol;
    pol = ipe_new_policy(p.policy, strlen(p.policy), core::ptr::null_mut(), 0);
    if (p.errno) {
    ...
    KUNIT_EXPECT_PTR_EQ(test, core::ptr::null_mut(), pol.pkcs7);
...
//
#[no_mangle]
unsafe extern "C" fn ipe_parser_widestring_test(test: *mut kunit) {
    ...
    struct ipe_policy *pol = core::ptr::null_mut();
    pol = ipe_new_policy((const char *)policy, (ARRAY_SIZE(policy) - 1) * 2, core::ptr::null_mut(), 0);
