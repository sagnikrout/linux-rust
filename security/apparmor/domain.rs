...
//! Source: security/apparmor/domain.c
#![no_std]
...
//
    ...
    let mut CONFLICTING_ATTACH_STR: *const static char  const = "conflicting profile attachments";
...
//
    ...
    struct aa_label *tracerl = core::ptr::null_mut();
    const struct cred *tracer_cred = core::ptr::null_mut();
...
//
    ...
    {
    let mut candidate_len: c_int = 0, candidate_xattrs = 0;
    let mut conflict: bool = false;
    struct aa_profile *profile, *candidate = core::ptr::null_mut();
    AA_BUG(!path);
    AA_BUG(!name);
    AA_BUG(!head);
    rcu_read_lock();
...
// any accepting state means a valid match.
    ...
    continue;
    if (attach.xattr_count) {
    let mut rev: c_long = READ_ONCE(ns.revision);
...
//
    ...
    let mut xtype: u32 = xindex & AA_X_TYPE_MASK;
    let mut index: c_int = xindex & AA_X_INDEX_MASK;
    const char *next;
    AA_BUG(!name);
...
//
    ...
    const char **info)
    {
    struct aa_label *new = core::ptr::null_mut();
    struct aa_label *stack = core::ptr::null_mut();
    struct aa_ns *ns = profile.ns;
    let mut xtype: u32 = xindex & AA_X_TYPE_MASK;
// Used for info checks during fallback handling
    const char *old_info = core::ptr::null_mut();
    switch (xtype) {
    case AA_X_NONE:
...
// released by caller
    ...
    struct aa_ruleset *rules = profile.label.rules[0];
    struct aa_label *new = core::ptr::null_mut();
    struct aa_profile *new_profile = core::ptr::null_mut();
    const char *info = core::ptr::null_mut(), *name = core::ptr::null_mut(), *target = core::ptr::null_mut();
    let mut state: aa_state_t = rules.file.start[AA_CLASS_FILE];
    let mut perms: aa_perms = {};
    let mut nonewprivs: bool = false;
    let mut error: c_int = 0;
    AA_BUG(!profile);
    AA_BUG(!bprm);
    AA_BUG(!buffer);
...
// secure_exec = true;
    ...
    let mut state: aa_state_t = rules.file.start[AA_CLASS_FILE];
    let mut perms: aa_perms = {};
    const char *xname = core::ptr::null_mut(), *info = "change_profile onexec";
...
// secure_exec = true;
    ...
    AA_MAY_ONEXEC, xname,
    core::ptr::null_mut(), onexec, cond.uid, info, error);
    }
...
//
    ...
    request, name, core::ptr::null_mut(), target,
...
// TODO: get rid of GLOBAL_ROOT_UID
    ...
    OP_CHANGE_ONEXEC,
    AA_MAY_ONEXEC, bprm.filename, core::ptr::null_mut(),
...
pub unsafe extern "C" fn apparmor_bprm_creds_for_exec(bprm: *mut linux_binprm) -> c_int {
    ...
    struct aa_label *label, *new = core::ptr::null_mut();
...
// transfer reference, released when cred is freed
    ...
    OP_EXEC, MAY_EXEC,
    bprm.filename, core::ptr::null_mut(), new,
...
//
    ...
    {
    struct aa_profile *root, *hat = core::ptr::null_mut();
    const char *info = core::ptr::null_mut();
    let mut error: c_int = 0;
    ...
    AA_MAY_CHANGEHAT,
    name, hat ? hat.base.hname : core::ptr::null_mut(),
    hat ? &hat.label : core::ptr::null_mut(), GLOBAL_ROOT_UID, info,
    error);
...
//
    ...
    int count, int flags)
    {
    struct aa_profile *profile, *root, *hat = core::ptr::null_mut();
    struct aa_ns *ns, *new_ns;
    struct aa_label *new;
    struct label_it it;
    let mut sibling: bool = false;
    const char *name, *info = core::ptr::null_mut();
    int i, error;
...
// TODO: get rid of GLOBAL_ROOT_UID
    ...
    aa_audit_file(subj_cred, profile, &nullperms,
    OP_CHANGE_HAT,
    AA_MAY_CHANGEHAT, name, core::ptr::null_mut(), core::ptr::null_mut(),
...
//
#[no_mangle]
pub unsafe extern "C" fn aa_change_hat(hats[]: *const c_char, count: c_int, token: u64, flags: c_int) -> c_int {
    int aa_change_hat(const char *hats[], int count, u64 token, int flags)
    {
    const struct cred *subj_cred;
    struct aa_task_ctx *ctx = task_ctx(current);
    struct aa_label *label, *previous, *new = core::ptr::null_mut(), *target = core::ptr::null_mut();
    struct aa_profile *profile;
    let mut perms: aa_perms = {};
    const char *info = core::ptr::null_mut();
    let mut error: c_int = 0;
...
//
    ...
    AA_MAY_CHANGEHAT, core::ptr::null_mut(), core::ptr::null_mut(), target,
    GLOBAL_ROOT_UID, info, error));
    goto out;
    }
    ...
    name,
    core::ptr::null_mut(), target, GLOBAL_ROOT_UID, info,
    error);
    return error;
    }
//
...
pub unsafe extern "C" fn aa_change_profile(fqname: *const c_char, flags: c_int) -> c_int {
    ...
    {
    struct aa_label *label, *new = core::ptr::null_mut(), *target = core::ptr::null_mut();
    struct aa_profile *profile;
    let mut perms: aa_perms = {};
    const char *info = core::ptr::null_mut();
    const char *auditname = fqname;		/* retain leading & if stack */
    let mut stack: bool = flags & AA_CHANGE_STACK;
    struct aa_task_ctx *ctx = task_ctx(current);
...
// full transition will be built in exec path
    ...
    core::ptr::null_mut(), new ? new : target,
    GLOBAL_ROOT_UID, info, error));
    out:
    aa_put_label(new);
    aa_put_label(target);
