...
//! Source: security/apparmor/policy.c
#![no_std]
...
//
    ...
    let mut unprivileged_userns_apparmor_policy: c_int = 1;
...
#[no_mangle]
unsafe extern "C" fn aa_free_pdb(pdb: *mut aa_policydb) {
...
//
#[no_mangle]
unsafe extern "C" fn __add_profile(list: *mut list_head, profile: *mut aa_profile) {
...
//
#[no_mangle]
unsafe extern "C" fn __list_remove_profile(profile: *mut aa_profile) {
...
//
#[no_mangle]
unsafe extern "C" fn __remove_profile(profile: *mut aa_profile) {
...
//
    ...
    profile.rawdata = core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn aa_free_data(ptr: *mut c_void, arg: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn free_attachment(attach: *mut aa_attachment) {
...
#[no_mangle]
unsafe extern "C" fn free_ruleset(rules: *mut aa_ruleset) {
...
//
    ...
    rhashtable_free_and_destroy(rht, aa_free_data, core::ptr::null_mut());
    kfree_sensitive(rht);
    }
...
//
    ...
    if (!profile)
    return core::ptr::null_mut();
    if (!aa_policy_init(&profile.base, core::ptr::null_mut(), hname, gfp))
    goto fail;
...
//
    ...
    {
    struct aa_policy *policy;
    struct aa_profile *parent, *profile = core::ptr::null_mut();
    char *split;
    AA_BUG(!ns);
    AA_BUG(!hname);
    policy = &ns.base;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ns) -> else {
    else if (ns)
// default profile for ns, currently unconfined
    ...
    struct aa_profile *profile;
    struct aa_ruleset *rules;
    profile = aa_alloc_profile(name, core::ptr::null_mut(), gfp);
    if (!profile)
    return core::ptr::null_mut();
...
//
    ...
    op = OP_PROF_RM;
#[no_mangle]
pub unsafe extern "C" fn if(AA_MAY_REPLACE_POLICY: mask &) -> else {
...
// check if loading policy is locked out
    if (aa_g_lock_policy)
    return audit_policy(label, op, core::ptr::null_mut(), core::ptr::null_mut(), "policy_locked",
    -EACCES);
    if (ocred && !is_subset_of_obj_privilege(subj_cred, label, ocred))
    return audit_policy(label, op, core::ptr::null_mut(), core::ptr::null_mut(),
    "not privileged for target profile",
    -EACCES);
    if (!aa_policy_admin_capable(subj_cred, label, ns))
    return audit_policy(label, op, core::ptr::null_mut(), core::ptr::null_mut(), "not policy admin",
    -EACCES);
// TODO: add fine grained mediation of policy loads
    ...
    let mut len: c_long = base - profile.base.hname;
...
//
#[no_mangle]
unsafe extern "C" fn __replace_profile(old: *mut aa_profile, new: *mut aa_profile) {
...
// p = aa_get_profile(__lookup_profile(&ns->base, hname));
    if (*p) {
    let mut error: c_int = replacement_allowed(*p, noreplace, info);
    if (error) {
...
#[no_mangle]
unsafe extern "C" fn share_name(old: *mut aa_profile, new: *mut aa_profile) {
...
//
    ...
    {
    const char *ns_name = core::ptr::null_mut(), *info = core::ptr::null_mut();
    struct aa_ns *ns = core::ptr::null_mut();
...
// break refcount cycle with proxy.
    ...
    ent.new.label.proxy = core::ptr::null_mut();
    goto skip;
    }
    if (!aa_g_export_binary) {
...
//
    audit_policy(label, op, ns_name, ent.new.base.hname, core::ptr::null_mut(),
    error);
    if (ent.old) {
...
// recreate rawdata symlinks
    ...
    out:
    aa_put_ns(ns);
    let mut udata_sz: isize = udata.size;
    aa_put_profile_loaddata(udata);
    kfree(ns_name);
    if (error)
    return error;
...
// audit cause of failure
    ...
    fail:
    audit_policy(label, op, ns_name, ent ? ent.new.base.hname : core::ptr::null_mut(),
    info, error);
...
//
    ...
    struct aa_ns *ns = core::ptr::null_mut();
    struct aa_profile *profile = core::ptr::null_mut();
    const char *name = fqname, *info = core::ptr::null_mut();
