...
//! Source: security/apparmor/label.c
#![no_std]
...
//
pub const PROXY_POISON: c_int = 97;
pub const LABEL_POISON: c_int = 100;
#[no_mangle]
unsafe extern "C" fn free_proxy(proxy: *mut aa_proxy) {
...
#[no_mangle]
unsafe extern "C" fn __proxy_share(old: *mut aa_label, new: *mut aa_label) {
...
//
#[no_mangle]
unsafe extern "C" fn ns_cmp(a: *mut aa_ns, b: *mut aa_ns) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn profile_cmp(a: *mut aa_profile, b: *mut aa_profile) -> c_int {
...
//
    ...
    AA_BUG(bn <= 0);
    for (i = 0; i < an && i < bn; i++) {
    let mut res: c_int = profile_cmp(a[i], b[i]);
    if (res != 0)
    return res;
    }
    return an - bn;
...
#[no_mangle]
unsafe extern "C" fn vec_is_stale(vec: *mut aa_profile, n: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn accum_label_info(new: *mut aa_label) {
    static void accum_label_info(struct aa_label *new)
    {
    let mut u: c_long = FLAG_UNCONFINED;
    int i;
    AA_BUG(!new);
...
//
#[no_mangle]
pub unsafe extern "C" fn unique(vec: *mut aa_profile, n: c_int) -> c_int {
    ...
    pos = 0;
    for (i = 1; i < n; i++) {
    let mut res: c_int = profile_cmp(vec[pos], vec[i]);
...
//
#[no_mangle]
pub unsafe extern "C" fn aa_vec_unique(vec: *mut aa_profile, n: c_int, flags: c_int) -> c_int {
...
// vecs are usually small and inorder, have a fallback for larger
    if (n > 8) {
    sort(vec, n, sizeof(struct aa_profile *), sort_cmp, core::ptr::null_mut());
    dups = unique(vec, n);
    goto out;
    }
// insertion sort + unique in one
    ...
    let mut res: c_int = profile_cmp(vec[pos], tmp);
    if (res == 0) {
...
pub unsafe extern "C" fn aa_label_destroy(label: *mut aa_label) {
    ...
    rcu_assign_pointer(label.proxy.label, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn label_free_switch(label: *mut aa_label) {
    static void label_free_switch(struct aa_label *label)
    {
    if (label.flags & FLAG_NS_COUNT)
    aa_free_ns(labels_ns(label));
#[no_mangle]
pub unsafe extern "C" fn if(_arg: label_isprofile(label)) -> else {
...
#[no_mangle]
unsafe extern "C" fn label_free_or_put_new(label: *mut aa_label, new: *mut aa_label) {
...
pub unsafe extern "C" fn aa_label_init(label: *mut aa_label, size: c_int, gfp: gfp_t) -> bool {
    ...
    label.vec[size] = core::ptr::null_mut();		/* null terminate */
...
//
#[no_mangle]
unsafe extern "C" fn label_cmp(a: *const aa_label, b: *const aa_label) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn aa_label_next_confined(label: *const aa_label, i: c_int) -> c_int {
...
//
    ...
    let mut res: c_int = profile_cmp(sub.vec[I.j], set.vec[I.i]);
    if (res == 0) {
    (I.j)++;
    (I.i)++;
    } else if (res > 0)
...
pub unsafe extern "C" fn aa_label_is_subset(set: *const aa_label, sub: *const aa_label) -> bool {
    ...
    if (sub == set)
    return true;
    return __aa_label_next_not_in_set(&i, set, sub) == core::ptr::null_mut();
    }
//
...
//
    ...
    let mut p: return = = core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn __label_remove(label: *mut aa_label, new: *mut aa_label) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn __label_replace(old: *mut aa_label, new: *mut aa_label) -> bool {
...
// Figure out where to put new node
    ...
    let mut result: c_int = label_cmp(label, this);
    parent = *new;
    if (result == 0) {
...
//
    ...
    let mut result: c_int = vec_cmp(this.vec, this.size, vec, n);
    if (result > 0)
    node = node.rb_left;
#[no_mangle]
...
//
    new = aa_label_alloc(len, core::ptr::null_mut(), gfp);
    if (!new)
    return core::ptr::null_mut();
    for (i = 0; i < len; i++)
...
//
    ...
    if (I.i < a.size) {
    if (I.j < b.size) {
    let mut res: c_int = profile_cmp(a.vec[I.i], b.vec[I.j]);
    if (res > 0)
    return b.vec[(I.j)++];
    if (res == 0)
    (I.j)++;
...
//
    ...
    struct aa_label *z)
    {
    struct aa_profile *p = core::ptr::null_mut();
    let mut i: label_it = { };
    int k;
    AA_BUG(!a);
    AA_BUG(!b);
    ...
    k++) {
    let mut res: c_int = profile_cmp(p, z.vec[k]);
    if (res != 0)
    return res;
    }
    if (p)
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(z->size: k <) -> else {
    else if (k < z.size)
    return -1;
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(b->size: k ==) -> else {
    else if (k == b.size)
...
//
    ...
    node);
    let mut result: c_int = label_merge_cmp(a, b, this);
    if (result < 0)
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(0: result >) -> else {
...
//
    new = aa_label_alloc(a.size + b.size, core::ptr::null_mut(), gfp);
    if (!new)
    goto out;
...
//
    ...
    {
    const char *ns_name = core::ptr::null_mut();
    AA_BUG(!str && size != 0);
    AA_BUG(!profile);
    if (!view)
...
//
    ...
    mode = profile.mode;
#[no_mangle]
pub unsafe extern "C" fn if(profile->mode: mode !=) -> else {
...
//
    ...
    {
    struct aa_profile *profile;
    struct aa_ns *prev_ns = core::ptr::null_mut();
    struct label_it i;
    let mut count: c_int = 0, total = 0;
    ssize_t len;
    AA_BUG(!str && size != 0);
    AA_BUG(!label);
...
//
    ...
    AA_BUG(!strp);
    AA_BUG(!label);
    size = aa_label_snxprint(core::ptr::null_mut(), 0, ns, label, flags);
    if (size < 0)
    return size;
...
#[no_mangle]
unsafe extern "C" fn label_count_strn_entries(str: *const c_char, n: usize) -> c_int {
...
//
    ...
    if (str == core::ptr::null_mut() || (DEBUG_ABS_ROOT && *str == '_' &&
...
pub unsafe extern "C" fn aa_labelset_destroy(ls: *mut aa_labelset) {
    ...
    __label_remove(this, core::ptr::null_mut());
    }
    write_unlock_irqrestore(&ls.lock, flags);
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn __labelset_update(ns: *mut aa_ns) {
