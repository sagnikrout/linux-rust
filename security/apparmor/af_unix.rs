...
//! Source: security/apparmor/af_unix.c
#![no_std]
...
// the sunpath may not be valid for this ns so use the path
    ...
    let mut vfsuid: vfsuid_t = i_uid_into_vfsuid(mnt_idmap(path.mnt), inode);
...
// todo: could change to out of band for cleaner separation
    ...
    protocol, core::ptr::null_mut(), info);
    if (state) {
...
pub const CMD_ADDR: c_int = 1;
pub const CMD_LISTEN: c_int = 2;
pub const CMD_OPT: c_int = 4;
...
//
    ...
    PF_UNIX, type, protocol, core::ptr::null_mut(),
    &ad.info);
    return aa_do_perms(profile, rules.policy, state, AA_MAY_CREATE,
    core::ptr::null_mut(), ad);
    }
    ...
    {
    struct aa_ruleset *rules = profile.label.rules[0];
    struct aa_perms *p = core::ptr::null_mut();
    aa_state_t state;
    AA_BUG(!profile);
    AA_BUG(!sk);
    AA_BUG(profile_unconfined(profile));
...
// bind for abstract socket
    ...
    state = RULE_MEDIATES_UNIX(rules);
    if (state) {
    let mut b: __be16 = cpu_to_be16(backlog);
    if (is_unix_fs(sk))
    ...
    state = RULE_MEDIATES_UNIX(rules);
    if (state) {
    let mut b: __be16 = cpu_to_be16(optname);
    if (is_unix_fs(sk))
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: path) -> else {
    else if (path)
    return unix_fs_perm(ad.op, request, ad.subj_cred,
    &profile.label, path);
...
// --------------------------------
    ...
    DEFINE_AUDIT_NET(ad, OP_CREATE, current_cred(), core::ptr::null_mut(), family,
    type, protocol);
...
#[no_mangle]
pub unsafe extern "C" fn aa_unix_sock_perm(op: *const c_char, request: u32, sock: *mut socket) -> c_int {
    ...
    request, sock.sk,
    is_unix_fs(sock.sk) ? &unix_sk(sock.sk).path : core::ptr::null_mut());
    end_current_label_crit_section(label, needput);
    return error;
    }
#[no_mangle]
unsafe extern "C" fn valid_addr(addr: *mut sockaddr, addr_len: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn aa_unix_listen_perm(sock: *mut socket, backlog: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn aa_unix_accept_perm(sock: *mut socket, newsock: *mut socket) -> c_int {
...
//
    ...
    is_unix_fs(sk) ? &u.path : core::ptr::null_mut(),
    paddr, plen,
    is_unix_fs(peer_sk) ? &peeru.path : core::ptr::null_mut(),
    peer_label);
    }
...
//
    ...
    struct aa_label *plabel = core::ptr::null_mut();
    struct sock *peer_sk = core::ptr::null_mut();
    let mut sk_req: u32 = request & ~NET_PEER_MASK;
    struct path path;
    bool is_sk_fs;
    let mut error: c_int = 0;
    AA_BUG(!label);
...
//
    ...
    sk_req, sock.sk,
    is_sk_fs ? &path : core::ptr::null_mut());
    }
    if (!peer_sk)
    goto out;
    if (!is_sk_fs) {
    let mut is_peer_fs: bool = is_unix_fs(peer_sk);
