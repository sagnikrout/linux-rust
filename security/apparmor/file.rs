...
//! Source: security/apparmor/file.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn map_mask_to_chr_mask(mask: u32) -> u32 {
    static u32 map_mask_to_chr_mask(u32 mask)
    {
    let mut m: u32 = mask & PERMS_CHRS_MASK;
...
//
#[no_mangle]
unsafe extern "C" fn file_audit_cb(ab: *mut audit_buffer, va: *mut c_void) {
    static void file_audit_cb(struct audit_buffer *ab, void *va)
    {
    struct common_audit_data *sa = va;
    struct apparmor_audit_data *ad = aad(sa);
    let mut fsuid: kuid_t = ad.subj_cred ? ad.subj_cred.fsuid : current_fsuid();
    char str[10];
...
//
    ...
    {
    let mut quiet: u32 = perms.quiet;
    let mut complain: u32 = perms.complain;
    let mut type: c_int = AUDIT_APPARMOR_AUTO;
    ...
    ad.info = info;
    ad.error = error;
    ad.common.u.tsk = core::ptr::null_mut();
    if (COMPLAIN_MODE(profile))
    complain |= ~(perms.allow | perms.deny);
    if (likely(!ad.error)) {
    let mut mask: u32 = perms.audit;
...
// don't reaudit files closed during inheritance
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), cond.uid, info, error));
    return error;
    }
    return 0;
    }
    let mut default_perms: aa_perms = {};
//
...
//
    ...
    {
    let mut index: c_uint = ACCEPT_TABLE(rules.dfa)[state];
    if (!(rules.perms))
...
// perms = *(aa_lookup_condperms(current_fsuid(), file_rules, state,
    ...
    profile, perms, op, request, name, core::ptr::null_mut(), core::ptr::null_mut(),
    cond.uid, core::ptr::null_mut(), e);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn xindex_is_subset(link: u32, target: u32) -> bool {
    ...
    {
    struct aa_ruleset *rules = profile.label.rules[0];
    const char *lname = core::ptr::null_mut(), *tname = core::ptr::null_mut();
    let mut lperms: aa_perms = {}, perms;
    const char *info = core::ptr::null_mut();
    let mut request: u32 = AA_MAY_LINK;
    aa_state_t state;
    int error;
...
// AA_MAY_LINK is not considered in the subset test
    ...
    core::ptr::null_mut(), cond.uid, info, error);
    }
//
...
//
    ...
    {
    let mut link: path = { .mnt = new_dir.mnt, .dentry = new_dentry };
    let mut target: path = { .mnt = new_dir.mnt, .dentry = old_dentry };
    struct inode *inode = d_backing_inode(old_dentry);
    let mut vfsuid: vfsuid_t = i_uid_into_vfsuid(mnt_idmap(target.mnt), inode);
    struct path_cond cond = {
    .uid = vfsuid_into_kuid(vfsuid),
    .mode = inode.i_mode,
    };
    char *buffer = core::ptr::null_mut(), *buffer2 = core::ptr::null_mut();
    struct aa_profile *profile;
    int error;
...
#[no_mangle]
unsafe extern "C" fn __file_is_delegated(obj_label: *mut aa_label) -> bool {
    static bool __file_is_delegated(struct aa_label *obj_label)
    {
    return unconfined(obj_label);
    }
#[no_mangle]
unsafe extern "C" fn __is_unix_file(file: *mut file) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISSOCK(file_inode(file)->i_mode)) -> else {
...
#[no_mangle]
unsafe extern "C" fn revalidate_tty(subj_cred: *const cred, label: *mut aa_label) {
...
#[no_mangle]
unsafe extern "C" fn match_file(p: *const c_void, file: *mut file, fd: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn aa_inherit_files(cred: *const cred, files: *mut files_struct) {
    ...
    struct file *devnull = core::ptr::null_mut();
    unsigned int n;
