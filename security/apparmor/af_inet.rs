...
//! Source: security/apparmor/af_inet.c
#![no_std]
...
// do we need early bailout for !family ...
    ...
    maddr.addrlen = 0;
    return map_addr(core::ptr::null_mut(), 0, 0, addrtype, &maddr.maddr, ad);
...
// NOTE: already lifted label_mediates into lsm.c
    ...
    DEFINE_AUDIT_NET(ad, OP_CREATE, current_cred(), core::ptr::null_mut(), family, type,
    protocol);
...
#[no_mangle]
pub unsafe extern "C" fn aa_inet_listen_perm(sock: *mut socket, backlog: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn aa_inet_accept_perm(sock: *mut socket, newsock: *mut socket) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn aa_inet_sock_perm(op: *const c_char, request: u32, sock: *mut socket) -> c_int {
...
// TODO: have ad here: instead of in CB so we do have to redo
    error = map_sock_addr(sock, ADDR_REMOTE, &raddr, core::ptr::null_mut());
