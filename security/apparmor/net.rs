...
//! Source: security/apparmor/net.c
#![no_std]
...
//
    ...
    {
    let mut len: c_int = unix_addr_len(addrlen);
    if (!addr || len <= 0) {
    ...
    audit_unix_addr(ab, str, core::ptr::null_mut(), 0);
    }
    }
// audit callback for net specific fields
#[no_mangle]
pub unsafe extern "C" fn audit_net_cb(ab: *mut audit_buffer, va: *mut c_void) {
    ...
    aa_audit_perm_mask(ab, ad.request, core::ptr::null_mut(), 0,
    net_mask_names, NET_PERMS_MASK);
    if (ad.denied & NET_PERMS_MASK) {
    audit_log_format(ab, " denied=");
    aa_audit_perm_mask(ab, ad.denied, core::ptr::null_mut(), 0,
    net_mask_names, NET_PERMS_MASK);
    }
    }
...
//
    ...
    u16 data)
    {
    let mut buffer: __be16 = cpu_to_be16(data);
...
// Generic af perm
    ...
    DEFINE_AUDIT_NET(ad, op, subj_cred, core::ptr::null_mut(), family, type, protocol);
...
#[no_mangle]
pub unsafe extern "C" fn aa_sk_perm(op: *const c_char, request: u32, sk: *const sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn apparmor_secmark_init(secmark: *mut aa_secmark) -> c_int {
    ...
    {
    struct aa_profile *profile;
    DEFINE_AUDIT_SK(ad, op, core::ptr::null_mut(), sk);
    return fn_for_each_confined(label, profile,
