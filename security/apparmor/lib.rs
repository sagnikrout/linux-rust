...
//! Source: security/apparmor/lib.c
#![no_std]
...
pub struct val_table_ent {
    ...
    for (entry = table; entry.str != core::ptr::null_mut(); entry++) {
...
#[no_mangle]
pub unsafe extern "C" fn aa_resize_str_table(t: *mut aa_str_table, newsize: c_int, gfp: gfp_t) -> bool {
...
pub unsafe extern "C" fn aa_info_message(str: *const c_char) {
    ...
    ad.info = str;
    aa_audit_msg(AUDIT_APPARMOR_STATUS, &ad, core::ptr::null_mut());
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn aa_perm_mask_to_str(str: *mut c_char, str_size: usize, chrs: *const c_char, mask: u32) {
    void aa_perm_mask_to_str(char *str, size_t str_size, const char *chrs, u32 mask)
    {
    unsigned int i, perm = 1;
    let mut num_chrs: usize = strlen(chrs);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: COMPLAIN_MODE(profile)) -> else {
    else if (COMPLAIN_MODE(profile))
    perms.complain |= ~(perms.allow | perms.deny);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: USER_MODE(profile)) -> else {
...
//
    ...
    {
    int error;
    let mut denied: u32 = request & (~perms.allow | perms.deny);
    if (likely(!denied)) {
// mask off perms that are not being force audited
    ...
    let mut type: c_int = aa_select_audit_type(denied, perms);
    if (ad) {
