...
//! Source: security/apparmor/match.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn if(YYTD_DATA16: th.td_flags ==) -> else {
    else if (th.td_flags == YYTD_DATA16)
    UNPACK_ARRAY(table.td_data, blob, th.td_lolen,
    u16, __be16, get_unaligned_be16);
#[no_mangle]
pub unsafe extern "C" fn if(YYTD_DATA32: th.td_flags ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn verify_table_headers(tables: *mut table_header, flags: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn verify_dfa(dfa: *const aa_dfa) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dfa_free(dfa: *mut aa_dfa) {
    ...
    kvfree(dfa.tables[i]);
    dfa.tables[i] = core::ptr::null_mut();
    }
    kfree(dfa);
    }
    }
...
//
    ...
    struct table_header *table = core::ptr::null_mut();
...
//
    ...
    let mut state: aa_state_t = start;
...
//
    ...
    let mut state: aa_state_t = start, pos;
    if (state == DFA_NOMATCH)
    return DFA_NOMATCH;
// current state is <state>, matching character *str
    if (dfa.tables[YYTD_ID_EC]) {
