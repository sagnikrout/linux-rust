...
//! Source: include/crypto/aes-ccm.h
#![no_std]
...
// private:
    pub aes: aes_enckey,
    pub /: *mut *mut size_t authtag_len; / Length of authentication tags in bytes,
}
...
//
    pub __aligned(__alignof__(__be64)): u8 mac[AES_BLOCK_SIZE],
// The current counter, a 128-bit big endian value
    pub __aligned(__alignof__(__be64)): u8 ctr[AES_BLOCK_SIZE],
// Buffered keystream for partial block updates
    pub __aligned(__alignof__(__be64)): u8 keystream[AES_BLOCK_SIZE],
// Encrypted counter of 0.  This gets XOR'ed with the tag at the end.
    pub __aligned(__alignof__(__be64)): u8 s0[AES_BLOCK_SIZE],
