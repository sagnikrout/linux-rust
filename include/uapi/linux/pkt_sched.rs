...
//! Source: include/uapi/linux/pkt_sched.h
#![no_std]
...
pub const TC_RED_NODROP: c_int = 8;
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tc_netem_xstats {
    pub /: *mut *mut __u64 delayed; / packets delayed,
    pub /: *mut *mut __u64 dropped; / packets dropped by loss model,
    pub /: *mut *mut __u64 corrupted; / packets with bit errors injected,
    pub /: *mut *mut __u64 duplicated; / duplicate packets generated,
    pub /: *mut *mut __u64 reordered; / packets sent out of order,
    pub dropped)*/: *mut *mut __u64 ecn_marked; / packets ECN CE-marked (not,
...
pub const FQ_BANDS: c_int = 3;
pub const FQ_MIN_WEIGHT: c_int = 16384;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tc_fq_qd_stats {
    pub gc_flows: __u64,
    pub /: *mut *mut __u64 highprio_packets; / obsolete,
    pub /: *mut *mut __u64 tcp_retrans; / obsolete,
    pub throttled: __u64,
    ...
    pub band_drops: [__u64; FQ_BANDS],
    pub band_pkt_count: [__u32; FQ_BANDS],
    pub pad: __u32,
}
...
pub struct tc_pie_xstats {
    pub /: *mut *mut __u64 prob; / current probability,
    pub /: *mut *mut __u32 delay; / current delay in ms,
    pub in: *mut *mut __u32 avg_dq_rate; / current average dq_rate,
// bits/pie_time
//
    pub /: *mut *mut __u32 dq_rate_estimating; / is avg_dq_rate being calculated?,
    pub /: *mut *mut __u32 packets_in; / total number of packets enqueued,
    pub /: *mut *mut __u32 dropped; / packets dropped due to pie_action,
    pub space: *mut *mut __u32 overlimit; / dropped due to lack of,
// in queue
//
...
pub struct tc_fq_pie_xstats {
    pub /: *mut *mut __u32 packets_in; / total number of packets enqueued,
    pub /: *mut *mut __u32 dropped; / packets dropped due to fq_pie_action,
    pub /: *mut *mut __u32 overlimit; / dropped due to lack of space in queue,
    pub /: *mut *mut __u32 overmemory; / dropped due to lack of memory in queue,
    pub /: *mut *mut __u32 ecn_mark; / packets marked with ecn,
    pub /: *mut *mut __u32 new_flow_count; / count of new flows created by packets,
    pub /: *mut *mut __u32 new_flows_len; / count of flows in new list,
    pub /: *mut *mut __u32 old_flows_len; / count of flows in old list,
    pub /: *mut *mut __u32 memory_usage; / total memory across all queues,
}
...
// ETS
pub const TCQ_ETS_MAX_BANDS: c_int = 16;

// DUALPI2
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum tc_dualpi2_drop_overload {
...
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum tc_dualpi2_drop_early {
...
pub struct tc_dualpi2_xstats {
    ...
    pub /: *mut *mut __u32 delay_c; / current delay in C queue,
    pub /: *mut *mut __u32 delay_l; / current delay in L queue,
    pub /: *mut *mut __u32 packets_in_c; / number of packets enqueued in C queue,
    pub /: *mut *mut __u32 packets_in_l; / number of packets enqueued in L queue,
    pub /: *mut *mut __u32 maxq; / maximum queue size,
    pub ecn*/: *mut *mut __u32 ecn_mark; / packets marked with,
    pub /: *mut *mut __u32 step_marks; / ECN marks due to the step AQM,
    pub /: *mut *mut __s32 credit; / current c_protection credit,
    pub /: *mut *mut __u32 memory_used; / Memory used by both queues,
    pub /: *mut *mut __u32 max_memory_used; / Maximum used memory,
    pub /: *mut *mut __u32 memory_limit; / Memory limit of both queues,
}
