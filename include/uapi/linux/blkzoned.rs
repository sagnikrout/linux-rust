...
//! Source: include/uapi/linux/blkzoned.h
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum blk_zone_report_flags {
// Output flags
...
#[derive(Copy, Clone)]
pub struct blk_zone {
    pub /: *mut *mut __u64 start; / Zone start sector,
    pub /: *mut *mut __u64 len; / Zone length in number of sectors,
    pub /: *mut *mut __u64 wp; / Zone write pointer position,
    pub /: *mut *mut __u8 type; / Zone type,
    pub /: *mut *mut __u8 cond; / Zone condition,
    pub /: *mut *mut __u8 non_seq; / Non-sequential write resources active,
    pub /: *mut *mut __u8 reset; / Reset write pointer recommended,
    pub resv: [__u8; 4],
    pub /: *mut *mut __u64 capacity; / Zone capacity in number of sectors,
