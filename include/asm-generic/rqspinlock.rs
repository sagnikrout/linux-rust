...
//! Source: include/asm-generic/rqspinlock.h
#![no_std]
...
pub struct bpf_res_spin_lock {
    pub val: u32,
    pub rqspinlock)): } __aligned(__alignof__(struct,
    pub qspinlock: struct,

pub type rqspinlock_t = qspinlock;

pub type rqspinlock_t = rqspinlock;

    pub lock): *mut extern int resilient_tas_spin_lock(rqspinlock_t,

    pub val): *mut *mut extern int resilient_queued_spin_lock_slowpath(rqspinlock_t lock, u32,
...
//
pub const RES_NR_HELD: c_int = 31;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rqspinlock_held {
    pub cnt: c_int,
    pub locks: [*mut c_void; RES_NR_HELD],
}
...
//
extern "C" {
    pub fn resilient_queued_spin_lock_slowpath(_arg: lock, _arg: val) -> return;
}
