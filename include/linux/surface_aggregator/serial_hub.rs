...
//! Source: include/linux/surface_aggregator/serial_hub.h
#![no_std]
...
pub struct ssh_frame {
    ...
    pub seq: u8,
    pub __packed: },
    pub 4): static_assert(sizeof(struct ssh_frame) ==,
//
...
pub struct ssh_command {
    ...
    pub cid: u8,
    pub __packed: },
    pub 8): static_assert(sizeof(struct ssh_command) ==,
//
...
// messages.
//
    pub len): return crc_itu_t(0xffff, buf,,
//
...
//
pub const SSH_NUM_EVENTS: c_int = 38;
//
// SSH_NUM_TARGETS - The number of communication targets used in the protocol.
//
pub const SSH_NUM_TARGETS: c_int = 2;
//
...
// events.
//
    pub 1u: return rqid > 0 ? rqid + 1u : rqid + SSH_NUM_EVENTS +,
//
...
//
    pub SSH_NUM_EVENTS: return ssh_rqid_to_event(rqid) <,
//
...
//
    pub SSH_NUM_TARGETS: return ssh_tid_to_index(tid) <,
//
...
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum ssh_packet_base_priority {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ssh_packet_ops {
    pub p): *mut *mut void (release)(struct ssh_packet,
    pub status): *mut *mut *mut void (complete)(struct ssh_packet p, int,
}
...
pub struct ssh_packet {
    ...
    pub ops: *const ssh_packet_ops,
}

extern "C" {
    pub fn ssh_packet_put(p: *mut ssh_packet);
}
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ssh_request_ops {
    pub rqst): *mut *mut void (release)(struct ssh_request,
    pub status): *const *const ssam_span data, int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ssh_request {
    pub packet: ssh_packet,
    pub node: list_head,
    pub state: c_ulong,
    pub timestamp: ktime_t,
    pub ops: *const ssh_request_ops,
}
...
//
extern "C" {
    pub fn container_of(_arg: p, ssh_request: struct, _arg: packet) -> return;
}
//
