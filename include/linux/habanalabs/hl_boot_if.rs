...
//! Source: include/linux/habanalabs/hl_boot_if.h
#![no_std]
...
// All Rights Reserved.
//
pub const LKD_HARD_RESET_MAGIC: c_uint = 0xED7BD694 /* deprecated - do not use */;
pub const HL_POWER9_HOST_MAGIC: c_uint = 0x1DA30009;
pub const BOOT_FIT_SRAM_OFFSET: c_uint = 0x200000;
pub const VERSION_MAX_LEN: c_int = 128;
#[repr(C)]
...
pub struct cpu_dyn_regs {
    pub cpu_pq_base_addr_low: __le32,
    pub cpu_pq_base_addr_high: __le32,
    pub cpu_pq_length: __le32,
    pub cpu_pq_init_status: __le32,
    pub cpu_eq_base_addr_low: __le32,
    pub cpu_eq_base_addr_high: __le32,
    pub cpu_eq_length: __le32,
    pub cpu_eq_ci: __le32,
    pub cpu_cq_base_addr_low: __le32,
    pub cpu_cq_base_addr_high: __le32,
    pub cpu_cq_length: __le32,
    pub cpu_pf_pq_pi: __le32,
    pub cpu_boot_dev_sts0: __le32,
    pub cpu_boot_dev_sts1: __le32,
    pub cpu_boot_err0: __le32,
    pub cpu_boot_err1: __le32,
    pub cpu_boot_status: __le32,
    pub fw_upd_sts: __le32,
    pub fw_upd_cmd: __le32,
    pub fw_upd_pending_sts: __le32,
    pub fuse_ver_offset: __le32,
    pub preboot_ver_offset: __le32,
    pub uboot_ver_offset: __le32,
    pub hw_state: __le32,
    pub kmd_msg_to_cpu: __le32,
    pub cpu_cmd_status_to_host: __le32,
    pub gic_host_pi_upd_irq: __le32,
    pub gic_tpc_qm_irq_ctrl: __le32,
    pub gic_mme_qm_irq_ctrl: __le32,
    pub gic_dma_qm_irq_ctrl: __le32,
    pub gic_nic_qm_irq_ctrl: __le32,
    pub gic_dma_core_irq_ctrl: __le32,
    pub gic_host_halt_irq: __le32,
    pub gic_host_ints_irq: __le32,
    pub reserved0: __le32,
    pub gic_rot_qm_irq_ctrl: __le32,
    pub reserved1: __le32,
    pub eng_arc_irq_ctrl: __le32,
    pub /: *mut *mut __le32 reserved2[20]; / reserve for future use,
}

// TODO: remove the desc magic after the code is updated to use message
// HCDM - Habana Communications Descriptor Magic
pub const HL_COMMS_DESC_MAGIC: c_uint = 0x4843444D;
pub const HL_COMMS_DESC_VER: c_int = 3;
// HCMv - Habana Communications Message + header version
pub const HL_COMMS_MSG_MAGIC_VALUE: c_uint = 0x48434D00;
pub const HL_COMMS_MSG_MAGIC_MASK: c_uint = 0xFFFFFF00;
pub const HL_COMMS_MSG_MAGIC_VER_MASK: c_uint = 0xFF;

#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum comms_msg_type {
...
pub struct lkd_fw_binning_info {
    pub tpc_mask_l: __le64,
    pub dec_mask: __le32,
    pub dram_mask: __le32,
    pub edma_mask: __le32,
    pub mme_mask_l: __le32,
    pub mme_mask_h: __le32,
    pub rot_mask: __le32,
    pub xbar_mask: __le32,
    pub reserved0: __le32,
    pub tpc_mask_h: __le64,
    pub nic_mask: __le64,
    pub reserved1: [__le32; 8],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct comms_desc_header {
    pub /: *mut *mut __le32 magic; / magic for validation,
    pub /: *mut *mut __le32 crc32; / CRC32 of the descriptor w/o header,
    pub /: *mut *mut __le16 size; / size of the descriptor w/o header,
    pub /: *mut *mut __u8 version; / descriptor version,
    pub /: *mut *mut __u8 reserved[5]; / pad to 64 bit,
}

// this is the comms message header - meta data
#[repr(C)]
#[derive(Copy, Clone)]
pub struct comms_msg_header {
    pub /: *mut *mut __le32 magic; / magic for validation,
    pub /: *mut *mut __le32 crc32; / CRC32 of the message w/o header,
    pub /: *mut *mut __le16 size; / size of the message w/o header,
    pub /: *mut *mut __u8 version; / message payload version,
    pub /: *mut *mut __u8 type; / message type,
    pub /: *mut *mut __u8 reserved[4]; / pad to 64 bit,
}

#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum lkd_fw_ascii_msg_lvls {
...
pub const LKD_FW_ASCII_MSG_MAX_LEN: c_int = 128;

pub const LKD_FW_ASCII_MSG_MIN_DESC_VERSION: c_int = 3;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lkd_fw_ascii_msg {
    pub valid: __u8,
    pub msg_lvl: __u8,
    pub reserved: [__u8; 6],
    pub msg: [c_char; LKD_FW_ASCII_MSG_MAX_LEN],
}

// this is the main FW descriptor - consider ABI when changing
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lkd_fw_comms_desc {
    pub header: comms_desc_header,
    pub cpu_dyn_regs: cpu_dyn_regs,
    pub fuse_ver: [c_char; VERSION_MAX_LEN],
    pub cur_fw_ver: [c_char; VERSION_MAX_LEN],
// can be used for 1 more version w/o ABI change
    pub reserved0: [c_char; VERSION_MAX_LEN],
    pub /: *mut *mut __le64 img_addr; / address for next FW component load,
    pub binning_info: lkd_fw_binning_info,
    pub ascii_msg: [lkd_fw_ascii_msg; LKD_FW_ASCII_MSG_MAX],
    pub /: *mut *mut __le32 rsvd_mem_size_mb; / reserved memory size [MB] for FW/SVE,
    pub reserved1: [c_char; 4],
}

#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum comms_reset_cause {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lkd_fw_comms_msg {
    pub header: comms_msg_header,
...
pub const COMMS_COMMAND_SIZE_SHIFT: c_int = 0;
pub const COMMS_COMMAND_SIZE_MASK: c_uint = 0x1FFFFFF;
pub const COMMS_COMMAND_CMD_SHIFT: c_int = 27;
pub const COMMS_COMMAND_CMD_MASK: c_uint = 0xF8000000;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct comms_command {
    pub /: *mut *mut u32 size :25; / 32MB max.,
    pub :2: u32 reserved,
    pub /: *mut *mut comms_cmd cmd :5; / 32 commands,
}
...
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum comms_ram_types {
    COMMS_SRAM = 0,
    COMMS_DRAM = 1,
}

pub const COMMS_STATUS_OFFSET_SHIFT: c_int = 0;
pub const COMMS_STATUS_OFFSET_MASK: c_uint = 0x03FFFFFF;
pub const COMMS_STATUS_OFFSET_ALIGN_SHIFT: c_int = 2;
pub const COMMS_STATUS_RAM_TYPE_SHIFT: c_int = 26;
pub const COMMS_STATUS_RAM_TYPE_MASK: c_uint = 0x0C000000;
pub const COMMS_STATUS_STATUS_SHIFT: c_int = 28;
pub const COMMS_STATUS_STATUS_MASK: c_uint = 0xF0000000;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct comms_status {
    pub :26: u32 offset,
    pub :2: comms_ram_types ram_type,
    pub /: *mut *mut comms_sts status :4; / 16 statuses,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct hl_module_data {
    pub name: [__u8; NAME_MAX_LEN],
    pub version: [__u8; VERSION_MAX_LEN],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hl_component_versions {
    pub struct_size: __le16,
    pub modules_offset: __le16,
    pub component: [__u8; VERSION_MAX_LEN],
    pub fw_os: [__u8; VERSION_MAX_LEN],
    pub comp_name: [__u8; NAME_MAX_LEN],
    pub modules_counter: __u8,
    pub reserved: [__u8; 3],
    pub modules: [hl_module_data; ],
}

// Max size of fit size
pub const HL_FW_VERSIONS_FIT_SIZE: c_int = 4096;
