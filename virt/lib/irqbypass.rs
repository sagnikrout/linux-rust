...
//
    ...
    {
    let mut index: c_ulong = (unsigned long)eventfd;
...
//
#[no_mangle]
pub unsafe extern "C" fn irq_bypass_unregister_producer(producer: *mut irq_bypass_producer) {
    void irq_bypass_unregister_producer(struct irq_bypass_producer *producer)
    {
    let mut index: c_ulong = (unsigned long)producer.eventfd;
    if (!producer.eventfd)
    return;
    guard(mutex)(&lock);
...
//
#[no_mangle]
pub unsafe extern "C" fn irq_bypass_unregister_consumer(consumer: *mut irq_bypass_consumer) {
    void irq_bypass_unregister_consumer(struct irq_bypass_consumer *consumer)
    {
    let mut index: c_ulong = (unsigned long)consumer.eventfd;
    if (!consumer.eventfd)
    return;
    guard(mutex)(&lock);
