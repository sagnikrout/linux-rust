...
//
    ...
    {
    int ret;
    struct folio *fault_folio = fault_page ? page_folio(fault_page) : core::ptr::null_mut();
    struct folio *new_fault_folio = core::ptr::null_mut();
...
//
    ...
    let mut entry: softleaf_t = softleaf_from_pmd(*pmdp);
    ...
    let mut pfn: c_ulong = page_to_pfn(folio_page(folio, 0));
    ...
    let mut addr: c_ulong = start, unmapped = 0;
    spinlock_t *ptl;
    ...
    let mut ret: c_int = migrate_vma_collect_huge_pmd(pmdp, start, end, walk, fault_folio);
    if (ret == -EAGAIN)
    goto again;
    if (ret == 0)
    return 0;
...
// a device folio.
//
    let mut extra: c_int = 1 + (page == fault_page);
...
pub unsafe extern "C" fn migrate_vma_setup(args: *mut migrate_vma) -> c_int {
    ...
    {
    let mut nr_pages: c_long = (args.end - args.start) >> PAGE_SHIFT;
...
//
    ...
    flush = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: *mut !pmd_none(pmdp)) -> else {
...
//
    ...
    let mut pfn: c_ulong = pte_pfn(orig_pte);
    if (!is_zero_pfn(pfn))
    goto unlock_abort;
    flush = true;
...
pub unsafe extern "C" fn migrate_vma_pages(migrate: *mut migrate_vma) {
    ...
    unsigned long i;
    for (i = 0; i < npages; i++) {
    struct folio *dst = core::ptr::null_mut(), *src = core::ptr::null_mut();
    struct page *newpage = migrate_pfn_to_page(dst_pfns[i]);
    struct page *page = migrate_pfn_to_page(src_pfns[i]);
    if (newpage)
