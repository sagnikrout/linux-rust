...
pub const SEQNR_MASK: c_uint = 0x0ff	/* low bits of unstable tree seqnr */;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct advisor_ctx {
    pub start_scan: ktime_t,
    pub scan_time: c_ulong,
    pub change: c_ulong,
...
pub const KSM_RUN_STOP: c_int = 0;
...
unsafe extern "C" fn ksm_slab_free() -> void __init {
    ...
    kmem_cache_destroy(stable_node_cache);
    kmem_cache_destroy(rmap_item_cache);
    mm_slot_cache = core::ptr::null_mut();
    }
#[no_mangle]
...
//
    folio_set_stable_node(folio, core::ptr::null_mut());
    remove_node_from_stable_tree(stable_node);
    err = 0;
    }
    folio_unlock(folio);
