...
//
    ...
    let mut bdi_tree: static struct rb_root = RB_ROOT;
    LIST_HEAD(bdi_list);
...
unsafe extern "C" fn cgwb_debug_stats_show(m: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    rcu_read_lock();
    list_for_each_entry_rcu(wb, &bdi.wb_list, bdi_node) {
    let mut stats: wb_stats = { .dirty_thresh = dirty_thresh };
    if (!wb_tryget(wb))
    continue;
    collect_wb_stats(&stats, wb);
//
...
#[no_mangle]
unsafe extern "C" fn default_bdi_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn cgwb_init() -> int __init {
...
pub unsafe extern "C" fn bdi_init(bdi: *mut backing_dev_info) -> c_int {
    ...
    p = &(*p).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(id: bdi->id <) -> else {
    else if (bdi.id < id)
    p = &(*p).rb_right;
    else
    break;
...
//
    ...
    {
    struct backing_dev_info *bdi = core::ptr::null_mut();
    struct rb_node **p;
