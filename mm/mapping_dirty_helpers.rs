...
//
    ...
    if (pte_write(ptent)) {
    let mut old_pte: pte_t = ptep_modify_prot_start(walk.vma, addr, pte);
...
//
    ...
    struct mm_walk *walk)
    {
    let mut pmdval: pmd_t = pmdp_get_lockless(pmd);
...
unsafe extern "C" fn wp_clean_post_vma(walk: *mut mm_walk) {
    ...
    wpwalk.range.end);
#[no_mangle]
pub unsafe extern "C" fn if(wpwalk->tlbflush_start: wpwalk->tlbflush_end >) -> else {
...
//
    ...
    struct mm_walk *walk)
    {
    let mut vm_flags: vm_flags_t = READ_ONCE(walk.vma.vm_flags);
...
//
    ...
    {
    let mut wpwalk: wp_walk = { .total = 0 };
...
//
    ...
    pgoff_t *end)
    {
    let mut none_set: bool = (*start >= *end);
