...
#[no_mangle]
pub unsafe extern "C" fn anon_vma_init() -> void __init {
...
pub const TLB_FLUSH_BATCH_FLUSHED_SHIFT: c_int = 16;
    ...
    struct tlbflush_unmap_batch *tlb_ubc = &current.tlb_ubc;
    int batch;
    let mut writable: bool = pte_dirty(pteval);
...
//
#[no_mangle]
unsafe extern "C" fn should_defer_flush(mm: *mut mm_struct, flags: enum ttu_flags) -> bool {
...
pub unsafe extern "C" fn flush_tlb_batched_pending(mm: *mut mm_struct) {
    ...
    {
    let mut batch: c_int = atomic_read(&mm.tlb_flush_batched);
    let mut pending: c_int = batch & TLB_FLUSH_BATCH_PENDING_MASK;
    let mut flushed: c_int = batch >> TLB_FLUSH_BATCH_FLUSHED_SHIFT;
    if (pending != flushed) {
    flush_tlb_mm(mm);
//
...
//
    ...
    let mut ptes: c_int = 0, referenced = 0;
    unsigned int nr;
...
//
    ...
    let mut end_addr: c_ulong = pmd_addr_end(address, vma.vm_end);
    let mut max_nr: c_uint = (end_addr - address) >> PAGE_SHIFT;
    let mut pteval: pte_t = ptep_get(pvmw.pte);
...
//
    ...
    if (pvmw.pte) {
    pte_t *pte = pvmw.pte;
    let mut entry: pte_t = ptep_get(pte);
//
...
//
    ...
    pmd_t *pmd = pvmw.pmd;
    let mut entry: pmd_t = pmdp_get(pmd);
//
...
// NR_*_PMDMAPPED are not maintained per-memcg
    ...
    let mut orig_nr_pages: c_int = nr_pages;
    let mut first: c_int = 0, nr = 0, nr_pmdmapped = 0;
...
//
    ...
    {
    let mut exclusive: bool = flags & RMAP_EXCLUSIVE;
    let mut nr: c_int = 1, nr_pmdmapped = 0;
...
//
    ...
    {
    atomic_t *mapped = &folio._nr_pages_mapped;
    let mut last: c_int = 0, nr = 0, nr_pmdmapped = 0;
    let mut partially_mapped: bool = false;
    __folio_rmap_sanity_checks(folio, page, nr_pages, level);
    switch (level) {
    case PGTABLE_LEVEL_PTE:
    if (!folio_test_large(folio)) {
...
//
    ...
    {
    DEFINE_FOLIO_VMA_WALK(pvmw, folio, vma, address, 0);
    let mut hsz: c_ulong = huge_page_size(hstate_vma(vma));
    let mut flags: enum ttu_flags = (enum ttu_flags)(long)arg;
    struct mm_struct *mm = vma.vm_mm;
    struct mmu_notifier_range range;
...
//
    ...
    bool anon_exclusive)
    {
    let mut swp_pte: pte_t = swp_entry_to_pte(entry);
    if (anon_exclusive)
...
//
    ...
    struct mmu_notifier_range range;
    let mut flags: enum ttu_flags = (enum ttu_flags)(long)arg;
    let mut nr_pages: c_ulong = 1, end_addr;
    unsigned long pfn;
...
//
    let mut entry: softleaf_t = softleaf_from_pte(pteval);
...
//
#[no_mangle]
pub unsafe extern "C" fn try_to_unmap(folio: *mut folio, flags: enum ttu_flags) {
...
//
    ...
    if (folio_test_hugetlb(folio)) {
    let mut anon: bool = folio_test_anon(folio);
//
...
//
#[no_mangle]
pub unsafe extern "C" fn try_to_migrate(folio: *mut folio, flags: enum ttu_flags) {
...
//
    ...
    struct rmap_walk_control *rwc, bool locked)
    {
    let mut pgoff_end: pgoff_t = pgoff_start + nr_pages - 1;
    struct vm_area_struct *vma;
