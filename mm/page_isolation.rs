...
unsafe extern "C" fn unset_migratetype_isolate(page: *mut page) {
    ...
    {
    struct zone *zone;
    let mut isolated_page: bool = false;
    unsigned int order;
    struct page *buddy;
    zone = page_zone(page);
...
//
    ...
    if (PageBuddy(page)) {
    let mut order: c_int = buddy_order(page);
...
//
    ...
    let mut head_pfn: c_ulong = page_to_pfn(head);
    let mut nr_pages: c_ulong = compound_nr(head);
...
// isolation is done at page block granularity
    let mut isolate_start: c_ulong = pageblock_start_pfn(start_pfn);
    let mut isolate_end: c_ulong = pageblock_align(end_pfn);
    int ret;
    let mut skip_isolation: bool = false;
