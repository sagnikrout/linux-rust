...
#[no_mangle]
pub unsafe extern "C" fn page_is_unmovable(zone: *mut zone, page: *mut page, mode: pb_isolate_mode, step: *mut c_ulong) -> bool {
//
...
//
#[no_mangle]
pub unsafe extern "C" fn has_unmovable_pages(start_pfn: c_ulong, end_pfn: c_ulong, mode: pb_isolate_mode) -> *mut c_void {
...
//
#[no_mangle]
pub unsafe extern "C" fn set_migratetype_isolate(page: *mut page, mode: pb_isolate_mode, start_pfn: c_ulong, end_pfn: c_ulong) -> c_int {
    let mut zone = page_zone(page);
pub static mut unmovable: *mut c_void = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn unset_migratetype_isolate(page: *mut page) {
pub static mut zone: *mut c_void = core::ptr::null_mut();
pub static mut isolated_page: bool = false;
    let mut order = 0;
pub static mut buddy: *mut c_void = core::ptr::null_mut();
    zone = page_zone(page);
...
#[no_mangle]
pub unsafe extern "C" fn __first_valid_page(pfn: c_ulong, nr_pages: c_ulong) -> *mut c_void {
    let mut i = 0;
    while (i < nr_pages) {
...
//
#[no_mangle]
pub unsafe extern "C" fn isolate_single_pageblock(boundary_pfn: c_ulong, mode: pb_isolate_mode, isolate_before: bool, skip_isolation: bool) -> c_int {
    let mut start_pfn = 0;
    let mut isolate_pageblock = 0;
    let mut pfn = 0;
pub static mut zone: *mut c_void = core::ptr::null_mut();
    let mut ret = 0;
...
//
    ...
    let mut page = __first_valid_page(pfn, boundary_pfn - pfn);
    VM_BUG_ON(!page);
    pfn = page_to_pfn(page);
    if (PageUnaccepted(page)) {
...
//
    if (PageCompound(page)) {
    let mut head = compound_head(page);
...
//
#[no_mangle]
pub unsafe extern "C" fn start_isolate_page_range(start_pfn: c_ulong, end_pfn: c_ulong, mode: pb_isolate_mode) -> c_int {
    let mut pfn = 0;
pub static mut page: *mut c_void = core::ptr::null_mut();
// isolation is done at page block granularity
pub static mut isolate_start: c_ulong = 0;
pub static mut isolate_end: c_ulong = 0;
    let mut ret = 0;
pub static mut skip_isolation: bool = false;
...
// skip isolated pageblocks at the beginning and end
    while (pfn < isolate_end - pageblock_nr_pages) {
...
//
#[no_mangle]
pub unsafe extern "C" fn undo_isolate_page_range(start_pfn: c_ulong, end_pfn: c_ulong) {
    let mut pfn = 0;
pub static mut page: *mut c_void = core::ptr::null_mut();
pub static mut isolate_start: c_ulong = 0;
pub static mut isolate_end: c_ulong = 0;
    while (pfn < isolate_end) {
    page = __first_valid_page(pfn, pageblock_nr_pages);
...
//
#[no_mangle]
pub unsafe extern "C" fn __test_page_isolated_in_pageblock(pfn: c_ulong, end_pfn: c_ulong, mode: pb_isolate_mode) -> c_ulong {
pub static mut page: *mut c_void = core::ptr::null_mut();
    while (pfn < end_pfn) {
...
//
#[no_mangle]
pub unsafe extern "C" fn test_pages_isolated(start_pfn: c_ulong, end_pfn: c_ulong, mode: pb_isolate_mode) -> c_int {
