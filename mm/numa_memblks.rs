...
//
#[no_mangle]
pub unsafe extern "C" fn numa_reset_distance() -> void __init {
    void __init numa_reset_distance(void)
    {
    let mut size: usize = numa_distance_cnt * numa_distance_cnt * sizeof(numa_distance[0]);
...
#[no_mangle]
unsafe extern "C" fn numa_alloc_distance() -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn numa_set_distance(from: c_int, to: c_int, distance: c_int) -> void __init {
...
pub unsafe extern "C" fn __node_distance(from: c_int, to: c_int) -> c_int {
    ...
    {
    if (from >= numa_distance_cnt || to >= numa_distance_cnt)
    let mut from: return = = to ? LOCAL_DISTANCE : REMOTE_DISTANCE;
...
//
#[no_mangle]
pub unsafe extern "C" fn numa_remove_memblk_from(idx: c_int, mi: *mut numa_meminfo) -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn numa_add_memblk(nid: c_int, start: u64, end: u64) -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn numa_add_reserved_memblk(nid: c_int, start: u64, end: u64) -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn numa_cleanup_meminfo(mi: *mut numa_meminfo) -> int __init {
    int __init numa_cleanup_meminfo(struct numa_meminfo *mi)
    {
    let mut low: u64 = memblock_start_of_DRAM();
    let mut high: u64 = memblock_end_of_DRAM();
    int i, j, k;
...
//
#[no_mangle]
unsafe extern "C" fn numa_clear_kernel_node_hotplug() -> void __init {
    static void __init numa_clear_kernel_node_hotplug(void)
    {
    let mut reserved_nodemask: nodemask_t = NODE_MASK_NONE;
...
//
    ...
    let mut nid: c_int = memblock_get_region_node(mb_region);
...
#[no_mangle]
unsafe extern "C" fn numa_register_meminfo(mi: *mut numa_meminfo) -> int __init {
...
//
    ...
    let mut pfn_align: c_ulong = node_map_pfn_alignment();
    if (pfn_align && pfn_align < PAGES_PER_SECTION) {
    let mut node_align_mb: c_ulong = PFN_PHYS(pfn_align) / SZ_1M;
    let mut sect_align_mb: c_ulong = PFN_PHYS(PAGES_PER_SECTION) / SZ_1M;
    ...
    {
    let mut max_addr: phys_addr_t = (phys_addr_t)ULLONG_MAX;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn cmp_memblk(a: *const c_void, b: *const c_void) -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn numa_fill_memblks(start: u64, end: u64) -> int __init {
...
pub unsafe extern "C" fn phys_to_target_node(start: u64) -> c_int {
    ...
    {
    let mut nid: c_int = meminfo_to_nid(&numa_meminfo, start);
    let mut reserved_nid: c_int = meminfo_to_nid(&numa_reserved_meminfo, start);
//
