...
unsafe extern "C" fn shmem_get_next_id(sb: *mut super_block, qid: *mut kqid) -> c_int {
    ...
    struct rb_node *node;
    let mut id: qid_t = from_kqid(&init_user_ns, *qid);
    struct quota_info *dqopt = sb_dqopt(sb);
    struct quota_id *entry = core::ptr::null_mut();
    let mut ret: c_int = 0;
    ...
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(entry->id: id >) -> else {
    else if (id > entry.id)
    node = node.rb_right;
    else
    goto got_next_id;
...
unsafe extern "C" fn shmem_acquire_dquot(dquot: *mut dquot) -> c_int {
    ...
    struct rb_node *parent = core::ptr::null_mut(), *new_node = core::ptr::null_mut();
    struct quota_id *new_entry, *entry;
    let mut id: qid_t = from_kqid(&init_user_ns, dquot.dq_id);
