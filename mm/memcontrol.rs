...
// Local (CPU and cgroup) state
    pub state: [c_long; NR_MEMCG_NODE_STAT_ITEMS],
// Delta calculation for lockless upward propagation
    pub state_prev: [c_long; NR_MEMCG_NODE_STAT_ITEMS],
}
...
// Non-hierarchical (CPU aggregated) state
    pub state_local: [c_long; NR_MEMCG_NODE_STAT_ITEMS],
// Pending child counts during tree propagation
    pub state_pending: [c_long; NR_MEMCG_NODE_STAT_ITEMS],
}
...
// Non-hierarchical (CPU aggregated) page state & events
    pub state_local: [c_long; MEMCG_VMSTAT_SIZE],
    pub events_local: [c_ulong; NR_MEMCG_EVENTS],
// Pending child counts during tree propagation
    pub state_pending: [c_long; MEMCG_VMSTAT_SIZE],
    pub events_pending: [c_ulong; NR_MEMCG_EVENTS],
// Stats updates since the last flush
...
//
    ...
    iter = &mz.iter;
    cmpxchg(&iter.position, dead_memcg, core::ptr::null_mut());
    }
    }
#[no_mangle]
...
//
pub const NR_MEMCG_STOCK: c_int = 7;
pub const FLUSHING_CACHED_CHARGE: c_int = 0;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct memcg_stock_pcp {
    pub lock: local_trylock_t,
    pub nr_pages: [u8; NR_MEMCG_STOCK],
    pub cached: [*mut mem_cgroup; NR_MEMCG_STOCK],
...
//
pub const NR_OBJ_STOCK: c_int = 5;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct obj_stock_pcp {
    pub lock: local_trylock_t,
    pub index: i8,
    pub node_id: i16,
    pub nr_slab_reclaimable_b: i16,
    pub nr_slab_unreclaimable_b: i16,
...
//
    pub nr_bytes: [c_uint; NR_OBJ_STOCK],    pub nr_bytes: [u16; NR_OBJ_STOCK],    pub cached: [*mut obj_cgroup; NR_OBJ_STOCK],
    pub work: work_struct,
    pub flags: c_ulong,
    pub drain_idx: u8,
}
...
// +-------+------------------------+
//
pub const MEMCG_DELAY_PRECISION_SHIFT: c_int = 20;
...
unsafe extern "C" fn mem_cgroup_free(memcg: *mut mem_cgroup) {
    ...
    init_memcg_stats();
    init_memcg_events();
    page_counter_init(&memcg.memory, core::ptr::null_mut(), true);
    page_counter_init(&memcg.swap, core::ptr::null_mut(), false);

    page_counter_init(&memcg.kmem, core::ptr::null_mut(), false);
    page_counter_init(&memcg.tcpmem, core::ptr::null_mut(), false);
...
// __mem_cgroup_free().
//
    pn.orig_objcg = core::ptr::null_mut();
    }
    }
...
unsafe extern "C" fn mem_cgroup_css_rstat_flush(css: *mut cgroup_subsys_state, cpu: c_int) {
    ...
    .ppending = parent ? parent.vmstats.state_pending : core::ptr::null_mut(),
    .cstat = statc.state,
    ...
    .ppending = parent ? parent.vmstats.events_pending : core::ptr::null_mut(),
    .cstat = statc.events,
    ...
    .ppending = plstats ? plstats.state_pending : core::ptr::null_mut(),
    .cstat = lstatc.state,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct uncharge_gather {
    pub objcg: *mut obj_cgroup,
    pub nr_memory: c_ulong,
    pub pgpgout: c_ulong,
    pub nr_kmem: c_ulong,
    pub nid: c_int,
}
