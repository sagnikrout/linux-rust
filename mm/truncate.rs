...
//
    ...
    let mut nr: c_int = folio_batch_count(fbatch);
    struct folio *folio;
    int i, j;
...
pub unsafe extern "C" fn truncate_inode_partial_folio(folio: *mut folio, start: loff_t, end: loff_t) -> bool {
    ...
    let mut size: usize = folio_size(folio);
...
//
    ...
    let mut xa_has_values: bool = false;
    let mut nr: c_int = folio_batch_count(&fbatch);
    for (i = 0; i < nr; i++) {
    struct folio *folio = fbatch.folios[i];
// We rely upon deletion not changing folio->index
...
//
    ...
    let mut did_range_unmap: c_int = 0;
    if (mapping_empty(mapping))
    return 0;
    folio_batch_init(&fbatch);
    index = start;
...
pub unsafe extern "C" fn truncate_pagecache_range(inode: *mut inode, lstart: loff_t, lend: loff_t) {
    ...
    {
    struct address_space *mapping = inode.i_mapping;
    let mut unmap_start: loff_t = round_up(lstart, PAGE_SIZE);
    let mut unmap_end: loff_t = round_down(1 + lend, PAGE_SIZE) - 1;
//
