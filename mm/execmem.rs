...
//
    ...
    {
    let mut kasan: bool = range.flags & EXECMEM_KASAN_SHADOW;
    let mut gfp_flags: gfp_t = GFP_KERNEL | __GFP_NOWARN;
    let mut align: c_uint = range.alignment;
    let mut start: c_ulong = range.start;
    let mut end: c_ulong = range.end;
    void *p;
    if (kasan)
...
unsafe extern "C" fn execmem_set_direct_map_valid(vm: *mut vm_struct, valid: bool) -> c_int {
    ...
    {
    let mut nr: c_uint = (1 << get_vm_area_page_order(vm));
    let mut updated: c_uint = 0;
    let mut err: c_int = 0;
...
unsafe extern "C" fn execmem_force_rw(ptr: *mut c_void, size: usize) -> c_int {
    ...
    {
    let mut nr: c_uint = PAGE_ALIGN(size) >> PAGE_SHIFT;
...
unsafe extern "C" fn execmem_cache_clean(work: *mut work_struct) {
    ...
    let mut size: usize = mas_range_len(&mas);
...
unsafe extern "C" fn execmem_cache_add_locked(ptr: *mut c_void, size: usize, gfp_mask: gfp_t) -> c_int {
    ...
    size_t size)
    {
    let mut addr: c_ulong = mas.index;
...
//
    ...
    {
    let mut vm_flags: c_ulong = VM_ALLOW_HUGE_VMAP;
    struct mutex *mutex = &execmem_cache.mutex;
    struct vm_struct *vm;
    size_t alloc_size;
...
unsafe extern "C" fn execmem_cache_free(ptr: *mut c_void) -> bool {
    ...
    let mut use_cache: bool = range.flags & EXECMEM_ROX_CACHE;
    let mut vm_flags: c_ulong = VM_FLUSH_RESET_PERMS;
    let mut pgprot: pgprot_t = range.pgprot;
...
#[no_mangle]
pub unsafe extern "C" fn execmem_is_rox(type: enum execmem_type) -> bool {
...
#[no_mangle]
unsafe extern "C" fn __execmem_init() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn execmem_late_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn execmem_init() -> void __init {
