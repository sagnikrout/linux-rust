...
#[no_mangle]
pub unsafe extern "C" fn stat(s: *const kmem_cache, si: enum stat_item) {
...
//
    ...
    static inline
#[no_mangle]
pub unsafe extern "C" fn stat_add(s: *const kmem_cache, si: enum stat_item, v: c_int) {
...
pub unsafe extern "C" fn set_freepointer(s: *mut kmem_cache, object: *mut c_void, fp: *mut c_void) {
    ...
    {
    let mut freeptr_addr: c_ulong = (unsigned long)object + s.offset;
...
pub unsafe extern "C" fn obj_exts_size_in_slab(slab: *mut slab) -> c_uint {
    ...
    struct slab *slab)
    {
    let mut objext_offset: c_ulong = obj_exts_offset_in_slab(s, slab);
    let mut objext_size: c_ulong = obj_exts_size_in_slab(slab);
...
unsafe extern "C" fn obj_exts_offset_in_object(s: *mut kmem_cache) -> c_uint {
    ...
    {
    let mut offset: c_uint = get_info_end(s);
...
//
    ...
    let mut slub_debug: static slab_flags_t = DEBUG_DEFAULT_FLAGS;
...
pub unsafe extern "C" fn set_track_prepare(gfp_flags: gfp_t) -> depot_stack_handle_t {
    ...
    {
    let mut handle: depot_stack_handle_t = set_track_prepare(gfp_flags);
...
pub unsafe extern "C" fn print_tracking(s: *mut kmem_cache, object: *mut c_void) {
    ...
    {
    let mut pr_time: c_ulong = jiffies;
    if (!(s.flags & SLAB_STORE_USER))
    return;
...
unsafe extern "C" fn print_trailer(s: *mut kmem_cache, slab: *mut slab, p: *mut u8) {
    ...
    s.red_left_pad);
#[no_mangle]
pub unsafe extern "C" fn if(16: p > addr +) -> else {
    else if (p > addr + 16)
...
unsafe extern "C" fn init_object(s: *mut kmem_cache, object: *mut c_void, val: u8) {
    ...
    let mut poison_size: c_uint = s.object_size;
    if (s.flags & SLAB_RED_ZONE) {
//
...
unsafe extern "C" fn on_freelist(s: *mut kmem_cache, slab: *mut slab, search: *mut c_void) -> bool {
    ...
    let mut search: return = = core::ptr::null_mut();
    }
...
//
    ...
    {
    let mut higher_order_disable: bool = false;
...
#[no_mangle]
unsafe extern "C" fn setup_slub_debug(str: *const c_char, kp: *const kernel_param) -> int __init {
    ...
    let mut global_slub_debug_changed: bool = false;
    let mut slab_list_specified: bool = false;
...
pub unsafe extern "C" fn kmem_cache_flags(flags: slab_flags_t, name: *const c_char) -> slab_flags_t {
    ...
    let mut slub_debug_local: slab_flags_t = slub_debug;
...
pub unsafe extern "C" fn init_slab_obj_exts(slab: *mut slab) {
    ...
    {
    let mut allow_spin: bool = alloc_flags_allow_spinning(alloc_flags);
    let mut new_slab: bool = alloc_flags & SLAB_ALLOC_NEW_SLAB;
    unsigned long new_exts;
    unsigned long old_exts;
    struct slabobj_ext *vec;
    let mut sz: usize = slab_obj_ext_size(slab) * slab.objects;
...
unsafe extern "C" fn alloc_slab_obj_exts_early(s: *mut kmem_cache, slab: *mut slab) {
    ...
    let mut offset: c_uint = obj_exts_offset_in_object(s);
...
//
#[no_mangle]
unsafe extern "C" fn slab_obj_ext_has_codetag_init() -> void __init {
    static void __init slab_obj_ext_has_codetag_init(void)
    {
    let mut need_codetag: bool = !mem_alloc_profiling_permanently_disabled();
...
// Are the object contents still accessible?
    let mut still_accessible: bool = (s.flags & SLAB_TYPESAFE_BY_RCU) && !after_rcu_delay;
...
unsafe extern "C" fn free_empty_sheaf(s: *mut kmem_cache, sheaf: *mut slab_sheaf) {
    ...
    gfp_t gfp)
    {
    let mut to_fill: c_int = s.sheaf_capacity - sheaf.size;
    int filled;
    if (!to_fill)
    return 0;
...
unsafe extern "C" fn sheaf_flush_unused(s: *mut kmem_cache, sheaf: *mut slab_sheaf) {
    ...
    struct slab_sheaf *sheaf)
    {
    let mut init: bool = slab_want_init_on_free(s);
    void **p = &sheaf.objects[0];
    let mut i: c_uint = 0;
    let mut pfmemalloc: bool = false;
    while (i < sheaf.size) {
...
//
    ...
    bool allow_spin)
    {
    struct slab_sheaf *empty = core::ptr::null_mut();
    unsigned long flags;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !spin_trylock_irqsave(&barn->lock, _arg: flags)) -> else {
...
unsafe extern "C" fn barn_put_full_sheaf(barn: *mut node_barn, sheaf: *mut slab_sheaf) {
    ...
    {
    struct slab_sheaf *sheaf = core::ptr::null_mut();
    unsigned long flags;
...
//
    ...
    bool allow_spin)
    {
    struct slab_sheaf *full = core::ptr::null_mut();
    unsigned long flags;
...
//
    ...
    struct page *page;
    struct slab *slab;
    let mut order: c_uint = oo_order(oo);
    if (unlikely(!allow_spin))
    page = alloc_frozen_pages_nolock(0/* __GFP_COMP is implied */,
    node, order);
#[no_mangle]
pub unsafe extern "C" fn if(NUMA_NO_NODE: node ==) -> else {
...
unsafe extern "C" fn init_cache_random_seq(s: *mut kmem_cache) -> c_int {
    ...
    {
    let mut count: c_uint = oo_objects(s.oo);
    int err;
...
#[no_mangle]
unsafe extern "C" fn init_freelist_randomization() -> void __init {
...
// Allocate and initialize a slab without building its freelist.
    ...
    let mut allow_spin: bool = alloc_flags_allow_spinning(alloc_flags);
    struct slab *slab;
    let mut oo: kmem_cache_order_objects = s.oo;
    gfp_t alloc_gfp;
    void *start;
...
unsafe extern "C" fn __free_slab(s: *mut kmem_cache, slab: *mut slab, allow_spin: bool) {
    ...
    let mut order: c_int = compound_order(page);
...
// Build a freelist from the objects not yet allocated from a fresh slab.
    ...
    struct slab_obj_iter *iter)
    {
    let mut nr: c_uint = slab.objects - slab.inuse;
    unsigned int i;
    void *cur, *next;
    if (!nr) {
    slab.freelist = core::ptr::null_mut();
...
//
    ...
    const struct slab_alloc_context *ac)
    {
    let mut allow_spin: bool = alloc_flags_allow_spinning(ac.alloc_flags);
...
//
    ...
    struct slab *first = core::ptr::null_mut(), *last = core::ptr::null_mut();
    let mut total_free: c_uint = 0;
    unsigned long flags;
// Racy check to avoid taking the lock unnecessarily.
    ...
    spin_lock_irqsave(&n.list_lock, flags);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !spin_trylock_irqsave(&n->list_lock, _arg: flags)) -> else {
    else if (!spin_trylock_irqsave(&n.list_lock, flags))
    return false;
...
//
    ...
    struct zoneref *z;
    struct zone *zone;
    let mut highest_zoneidx: enum zone_type = gfp_zone(gfp_flags);
...
//
    ...
    struct slab_obj_iter iter;
    let mut needs_add_partial: bool = true;
    unsigned long flags;
//
...
//
    ...
    void *object;
    struct slab *slab;
    let mut try_thisnode: bool = true;
...
//
    ...
    {
    let mut init: bool = slab_want_init_on_alloc(flags, s);
    let mut zero_size: c_uint = s.object_size;
    let mut init_flags: gfp_t = flags & gfp_allowed_mask;
    let mut kasan_init: bool = false;
//
...
// As ptr might get tagged, call kmemleak hook after KASAN.
    ...
    {
    let mut size: usize = ac.orig_size;
    struct kmem_cache *s;
    void *ret;
...
//
    ...
    {
    struct kmem_cache_node *n = get_node(s, slab_nid(slab));
    struct slab *slab_free = core::ptr::null_mut();
    let mut cnt: c_int = bulk_cnt;
    unsigned long flags;
    let mut handle: depot_stack_handle_t = 0;
//
...
//
    ...
    bool was_full;
    struct freelist_counters old, new;
    struct kmem_cache_node *n = core::ptr::null_mut();
    unsigned long flags;
...
unsafe extern "C" fn rcu_free_sheaf(head: *mut rcu_head) {
    ...
    {
    struct slab_sheaf *sheaf;
    struct node_barn *barn = core::ptr::null_mut();
    struct kmem_cache *s;
    sheaf = container_of(head, struct slab_sheaf, rcu_head);
    s = sheaf.cache;
//
...
pub unsafe extern "C" fn __kfree_rcu_sheaf(s: *mut kmem_cache, obj: *mut c_void, free_flags: c_uint) -> bool {
    ...
    let mut allow_spin: bool = free_flags_allow_spinning(free_flags);
    ...
    struct slab_sheaf *empty;
    struct node_barn *barn;
    let mut alloc_flags: c_uint = to_alloc_flags(free_flags);
    let mut gfp: gfp_t = allow_spin ? GFP_NOWAIT : __GFP_NOWARN;
...
//
    ...
    size -= remote_nr;
    while (size) {
    let mut batch_freed: c_uint = __free_to_pcs_batch(s, size, p);
    if (!batch_freed) {
...
//
    ...
    return s.inuse;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: obj_exts_in_object(slab)) -> else {
...
//
    ...
    struct zoneref *z;
    struct zone *zone;
    let mut highest_zoneidx: enum zone_type = gfp_zone(gfp);
    unsigned int cpuset_mems_cookie;
    let mut refilled: c_uint = 0;
...
//
    ...
    unsigned int max)
    {
    let mut local_node: c_int = numa_mem_id();
...
//
    ...
    if (unlikely(kfence_obj)) {
    let mut idx: c_int = get_random_u32_below(size + 1);
    if (idx != size)
    p[size] = p[idx];
    p[idx] = kfence_obj;
    size++;
...
//
    ...
    let mut slab_size: c_uint = (unsigned int)PAGE_SIZE << order;
    unsigned int rem;
    rem = slab_size % size;
...
//
    let mut nr_cpus: c_uint = num_present_cpus();
    if (nr_cpus <= 1)
    nr_cpus = nr_cpu_ids;
...
unsafe extern "C" fn init_percpu_sheaves(s: *mut kmem_cache) -> c_int {
    ...
    {
    let mut bootstrap_sheaf: static struct slab_sheaf = {};
    int cpu;
    for_each_possible_cpu(cpu) {
    struct slub_percpu_sheaves *pcs;
    pcs = per_cpu_ptr(s.cpu_sheaves, cpu);
...
//
    ...
    capacity = 4;
#[no_mangle]
pub unsafe extern "C" fn if(1024: s->size >=) -> else {
    else if (s.size >= 1024)
    capacity = 12;
#[no_mangle]
pub unsafe extern "C" fn if(256: s->size >=) -> else {
    else if (s.size >= 256)
    capacity = 26;
    else
    capacity = 60;
...
unsafe extern "C" fn calculate_sizes(args: *mut kmem_cache_args, s: *mut kmem_cache) -> c_int {
    ...
    {
    let mut flags: slab_flags_t = s.flags;
    let mut size: c_uint = s.object_size;
...
//
#[no_mangle]
unsafe extern "C" fn setup_slub_min_order(str: *const c_char, kp: *const kernel_param) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn setup_slub_max_order(str: *const c_char, kp: *const kernel_param) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn setup_slab_strict_numa(str: *const c_char, kp: *const kernel_param) -> int __init {
...
//
    ...
    struct kmem_cache *s;
    unsigned int offset;
    let mut is_kfence: bool = is_kfence_address(ptr);
...
//
    ...
    let mut free: c_int = slab.objects - slab.inuse;
...
//
#[no_mangle]
unsafe extern "C" fn bootstrap_cache_sheaves(s: *mut kmem_cache) -> void __init {
    static void __init bootstrap_cache_sheaves(struct kmem_cache *s)
    {
    let mut empty_args: kmem_cache_args = {};
    unsigned int capacity;
...
#[no_mangle]
unsafe extern "C" fn bootstrap_kmalloc_sheaves() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn kmem_cache_init() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn kmem_cache_init_late() -> void __init {
...
unsafe extern "C" fn alloc_loc_track(t: *mut loc_track, max: c_ulong, flags: gfp_t) -> c_int {
    ...
    let mut age: c_ulong = jiffies - track.when;
    let mut handle: depot_stack_handle_t = 0;
    let mut waste: c_uint = s.object_size - orig_size;
...
#[no_mangle]
pub unsafe extern "C" fn if(chandle: track->addr == caddr && handle <) -> else {
...
//
    ...
    {
    void *addr = slab_address(slab);
    let mut is_alloc: bool = (alloc == TRACK_ALLOC);
    void *p;
    __fill_map(obj_map, s, slab);
    for_each_object(p, s, addr, slab.objects)
...
//
    ...
    x = node_nr_objs(n);
#[no_mangle]
pub unsafe extern "C" fn if(SO_OBJECTS: flags &) -> else {
...
pub struct slab_attribute {
    pub attr: attribute,
    pub buf): *mut *mut *mut ssize_t (show)(struct kmem_cache s, char,
    pub count): *const *const *const *const ssize_t (store)(struct kmem_cache s, char x, size_t,
}
...
#[no_mangle]
unsafe extern "C" fn show_stat(s: *mut kmem_cache, buf: *mut c_char, si: enum stat_item) -> c_int {
    ...
    return -ENOMEM;
    for_each_online_cpu(cpu) {
    let mut x: c_uint = per_cpu_ptr(s.cpu_stats, cpu).stat[si];
    data[cpu] = x;
    sum += x;
    }
...
#[no_mangle]
unsafe extern "C" fn clear_stat(s: *mut kmem_cache, si: enum stat_item) {
...
unsafe extern "C" fn skip_kfence_show(s: *mut kmem_cache, buf: *mut c_char) -> isize {
    ...
    s.flags &= ~SLAB_SKIP_KFENCE;
#[no_mangle]
pub unsafe extern "C" fn if('1': buf[0] ==) -> else {
    else if (buf[0] == '1')
    s.flags |= SLAB_SKIP_KFENCE;
    else
    ret = -EINVAL;
...
unsafe extern "C" fn sysfs_slab_add(s: *mut kmem_cache) -> c_int {
    ...
    let mut unmergeable: c_int = slab_unmergeable(s);
...
#[no_mangle]
unsafe extern "C" fn slab_kset_init() -> int __init {
    static int __init slab_kset_init(void)
    {
...
#[no_mangle]
unsafe extern "C" fn slab_sysfs_process_aliases() -> void __init {
    static void __init slab_sysfs_process_aliases(void)
    {
    int err;
    while (alias_list) {
...
#[no_mangle]
unsafe extern "C" fn slab_late_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn slab_debugfs_root_init() -> void __init {
    static void __init slab_debugfs_root_init(void)
    {
...
pub unsafe extern "C" fn get_slabinfo(s: *mut kmem_cache, sinfo: *mut slabinfo) {
    ...
    {
    let mut nr_slabs: c_ulong = 0;
    let mut nr_objs: c_ulong = 0;
    let mut nr_free: c_ulong = 0;
    int node;
    struct kmem_cache_node *n;
    for_each_kmem_cache_node(s, node, n) {
    nr_slabs += node_nr_slabs(n);
