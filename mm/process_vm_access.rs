...
// Do the copy for each page
    ...
    let mut copy: usize = PAGE_SIZE - offset;
    size_t copied;
    if (copy > len)
    copy = len;
    if (vm_write)
...
//
    ...
    int vm_write)
    {
    let mut pa: c_ulong = addr & PAGE_MASK;
    let mut start_offset: c_ulong = addr - pa;
...
// Work out address and page range required
    ...
    let mut pinned_pages: c_int = min_t(unsigned long, nr_pages, PVM_MAX_USER_PAGES);
...
//
    ...
    ssize_t iov_len;
    let mut total_len: usize = iov_iter_count(iter);
//
...
//
    ...
    struct iov_iter iter;
    ssize_t rc;
    let mut dir: c_int = vm_write ? ITER_SOURCE : ITER_DEST;
    if (flags != 0)
    return -EINVAL;
// Check iovecs
