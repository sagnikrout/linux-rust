...
pub unsafe extern "C" fn kmsan_internal_task_create(task: *mut task_struct) {
    ...
    let mut checked: bool = poison_flags & KMSAN_POISON_CHECK;
...
pub unsafe extern "C" fn kmsan_internal_memmove_metadata(dst: *mut c_void, src: *mut c_void, n: usize) {
    ...
    {
    let mut prev_old_origin: depot_stack_handle_t = 0, prev_new_origin = 0;
    int i, iter, step, src_off, dst_off, oiter_src, oiter_dst;
    let mut old_origin: depot_stack_handle_t = 0, new_origin = 0;
...
//
    ...
    {
    let mut cur_origin: depot_stack_handle_t = 0, new_origin = 0;
    let mut addr64: c_ulong = (unsigned long)addr;
    depot_stack_handle_t *origin = core::ptr::null_mut();
    unsigned char *shadow = core::ptr::null_mut();
    let mut cur_off_start: c_int = -1;
    int chunk_size;
    let mut pos: usize = 0;
    if (!size)
    return;
...
pub unsafe extern "C" fn kmsan_metadata_is_contiguous(addr: *mut c_void, size: usize) -> bool {
    ...
    {
    char *cur_shadow = core::ptr::null_mut(), *next_shadow = core::ptr::null_mut(), *cur_origin = core::ptr::null_mut(),
// next_origin = NULL;
    let mut cur_addr: u64 = (u64)addr, next_addr = cur_addr + PAGE_SIZE;
    depot_stack_handle_t *origin_p;
    let mut all_untracked: bool = false;
    if (!size)
    return true;
