...
pub struct madvise_behavior {
    ...
    pub behavior: c_int,
    pub tlb: *mut mmu_gather,
    pub lock_mode: enum madvise_lock_mode,
...
//
    ...
    let mut new_vma_flags: vma_flags_t = legacy_to_vma_flags(new_flags);
    struct madvise_behavior_range *range = &madv_behavior.range;
    struct anon_vma_name *anon_name = madv_behavior.anon_name;
    let mut set_new_anon_name: bool = madv_behavior.behavior == __MADV_SET_ANON_VMA_NAME;
...
//
    ...
    let mut end_index: pgoff_t = linear_page_index(vma, end) - 1;
    struct folio *folio;
    let mut ctx: swap_io_ctx = {};
    rcu_read_lock();
...
unsafe extern "C" fn madvise_willneed(madv_behavior: *mut madvise_behavior) -> c_long {
    ...
    let mut start: c_ulong = madv_behavior.range.start;
    let mut end: c_ulong = madv_behavior.range.end;
    loff_t offset;
...
//
    ...
    let mut pageout: bool = private.pageout;
    ...
    pmd_t orig_pmd;
    let mut next: c_ulong = pmd_addr_end(addr, end);
...
//
    ...
    unsigned long end, struct mm_walk *walk)
    {
    let mut cydp_flags: cydp_t = CYDP_CLEAR_YOUNG | CYDP_CLEAR_DIRTY;
...
#[no_mangle]
pub unsafe extern "C" fn get_walk_lock(mode: enum madvise_lock_mode) -> enum page_walk_lock {
...
pub unsafe extern "C" fn madvise_dontneed_free_valid_vma(madv_behavior: *mut madvise_behavior) -> bool {
    ...
    {
    struct vm_area_struct *vma = madv_behavior.vma;
    let mut behavior: c_int = madv_behavior.behavior;
    struct madvise_behavior_range *range = &madv_behavior.range;
    if (!is_vm_hugetlb_page(vma)) {
    let mut forbidden: c_uint = VM_PFNMAP;
...
#[no_mangle]
pub unsafe extern "C" fn if(MADV_FREE: behavior ==) -> else {
...
unsafe extern "C" fn madvise_populate(madv_behavior: *mut madvise_behavior) -> c_long {
    ...
    {
    struct mm_struct *mm = madv_behavior.mm;
    let mut write: bool = madv_behavior.behavior == MADV_POPULATE_WRITE;
...
unsafe extern "C" fn is_valid_guard_vma(vma: *mut vm_area_struct, allow_locked: bool) -> bool {
    ...
    {
    let mut disallowed: vm_flags_t = VM_SPECIAL | VM_HUGETLB;
//
...
unsafe extern "C" fn madvise_vma_behavior(madv_behavior: *mut madvise_behavior) -> c_int {
    ...
    let mut behavior: c_int = madv_behavior.behavior;
    struct vm_area_struct *vma = madv_behavior.vma;
    let mut new_flags: vm_flags_t = vma.vm_flags;
    struct madvise_behavior_range *range = &madv_behavior.range;
    int error;
...
// range is updated to span each VMA, so store end of entire range.
    let mut last_end: c_ulong = range.end;
    let mut unmapped_error: c_int = 0;
    int error;
...
unsafe extern "C" fn madvise_lock(madv_behavior: *mut madvise_behavior) -> c_int {
    ...
    {
    struct mm_struct *mm = madv_behavior.mm;
    let mut lock_mode: enum madvise_lock_mode = get_lock_mode(madv_behavior);
    switch (lock_mode) {
...
// Perform an madvise operation over a vector of addresses and lengths.
    ...
    let mut len_in: usize = iter_iov_len(iter);
    int error;
    error = check_input_range(start, len_in);
    if (error || !len_in)
    ret = error;
...
// Check to see whether len was rounded up from small -ve to zero
    ...
    {
    struct anon_vma_name *anon_name = core::ptr::null_mut();
