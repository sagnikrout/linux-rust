...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: irq) -> else {
    else if (irq)
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: irq_off) -> else {
    else if (irq_off)
...
pub unsafe extern "C" fn list_lru_add_obj(lru: *mut list_lru, item: *mut list_head) -> bool {
    ...
    {
    bool ret;
    let mut nid: c_int = page_to_nid(virt_to_page(item));
    if (list_lru_memcg_aware(lru)) {
    rcu_read_lock();
...
pub unsafe extern "C" fn list_lru_count_node(lru: *mut list_lru, nid: c_int) -> c_ulong {
    ...
    struct list_lru_one *l = core::ptr::null_mut();
    struct list_head *item, *n;
    let mut isolated: c_ulong = 0;
    restart:
...
unsafe extern "C" fn memcg_destroy_list_lru(lru: *mut list_lru) {
    ...
    {
    let mut dst_idx: c_int = dst_memcg.kmemcg_id;
...
//
    ...
    struct list_lru *lru)
    {
    let mut idx: c_int = memcg.kmemcg_id;
    ...
    {
    unsigned long flags;
    struct list_lru_memcg *mlru = core::ptr::null_mut();
