...
//
    ...
    if (!IS_ERR(vma)) {
    let mut locked: bool = vma_start_read_locked(vma);
    if (!locked)
    vma = ERR_PTR(-EAGAIN);
    }
    mmap_read_unlock(mm);
...
unsafe extern "C" fn mfill_get_vma(state: *mut mfill_state) -> c_int {
    ...
    let mut flags: uffd_flags_t = state.flags;
    struct vm_area_struct *dst_vma;
...
//
    ...
    let mut writable: bool = dst_vma.vm_flags & VM_WRITE;
    let mut vm_shared: bool = dst_vma.vm_flags & VM_SHARED;
    spinlock_t *ptl;
    struct folio *folio = page_folio(page);
    let mut page_in_cache: bool = folio_mapping(folio);
    pte_t dst_ptep;
...
unsafe extern "C" fn mfill_retry_state_put(s: *mut mfill_retry_state) {
    ...
    struct folio *folio)
    {
    let mut retry_state: mfill_retry_state = { 0 };
    struct mfill_retry_state *for_free __free(retry_put) = &retry_state;
    let mut src_addr: c_ulong = mfill_state.src_addr;
    void *kaddr;
    int err;
...
// reget VMA and PMD, they could change underneath us
    ...
    const struct vm_uffd_ops *ops)
    {
    let mut dst_addr: c_ulong = state.dst_addr;
    let mut src_addr: c_ulong = state.src_addr;
    let mut flags: uffd_flags_t = state.flags;
    struct folio *folio;
    int ret;
    if (!ops) {
...
unsafe extern "C" fn mfill_atomic_pte_continue(state: *mut mfill_state) -> c_int {
    ...
    let mut dst_addr: c_ulong = state.dst_addr;
    let mut pgoff: pgoff_t = linear_page_index(dst_vma, dst_addr);
...
//
    ...
    if (!uffd_flags_mode_is(flags, MFILL_ATOMIC_CONTINUE)) {
    let mut ptep: pte_t = huge_ptep_get(dst_mm, dst_addr, dst_pte);
...
//
    ...
    for_each_vma_range(vmi, dst_vma, end) {
    let mut vma_start: c_ulong = max(dst_vma.vm_start, start);
    let mut vma_end: c_ulong = min(dst_vma.vm_end, end);
    let mut flags: c_uint = mm_cp_flags;
//
...
//
    ...
    let mut src_start: c_ulong = src_addr;
...
//
    ...
    pte_t *src_pte = core::ptr::null_mut();
    pte_t *dst_pte = core::ptr::null_mut();
    pmd_t dummy_pmdval;
    pmd_t dst_pmdval;
    struct folio *src_folio = core::ptr::null_mut();
...
// have to reacquire the folio after it got split
    ...
    let mut entry: softleaf_t = softleaf_from_pte(orig_src_pte);
...
//
    ...
    vm_flags_t vm_flags)
    {
    let mut uffd_wp_changed: bool = (vma.vm_flags ^ vm_flags) & VM_UFFD_WP;
...
unsafe extern "C" fn userfaultfd_reset_ctx(vma: *mut vm_area_struct) {
    ...
    let mut give_up_on_oom: bool = false;
    let mut new_vma_flags: vma_flags_t = vma.flags;
...
// Assumes mmap write lock taken, and mm_struct pinned.
    ...
    bool wp_async)
    {
    let mut vma_flags: vma_flags_t = legacy_to_vma_flags(vm_flags);
...
//
    ...
    {
    struct mm_struct *mm = ctx.mm;
    let mut address: c_ulong = vmf.address;
    pgd_t *pgd;
    p4d_t *p4d;
    pud_t *pud;
    pmd_t *pmd, _pmd;
...
pub unsafe extern "C" fn dup_userfaultfd(vma: *mut vm_area_struct, fcs: *mut list_head) -> c_int {
    ...
    {
    struct userfaultfd_ctx *ctx = core::ptr::null_mut(), *octx;
...
unsafe extern "C" fn userfaultfd_release(inode: *mut inode, file: *mut file) -> c_int {
    ...
    struct mm_struct *mm = ctx.mm;
// len == 0 means wake all
    let mut range: userfaultfd_wake_range = { .len = 0, };
...
//
    ...
    ret = EPOLLIN;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: waitqueue_active(&ctx->event_wqh)) -> else {
...
//
    ...
    struct userfaultfd_ctx *fork_nctx = core::ptr::null_mut();
...
//
    ...
    {
    let mut task_size: __u64 = mm.task_size;
    if (len & ~PAGE_MASK)
    return -EINVAL;
    if (!len)
    return -EINVAL;
    ...
    let mut wp_async: bool = userfaultfd_wp_async_ctx(ctx);
...
//
    if (is_vm_hugetlb_page(vma)) {
    let mut vma_hpagesize: c_ulong = vma_kernel_pagesize(vma);
...
//
    ...
    let mut vma_hpagesize: c_ulong = vma_kernel_pagesize(cur);
    ret = -EINVAL;
...
unsafe extern "C" fn uffd_api_available_features() -> __u64 {
    ...
    {
    let mut f: __u64 = UFFD_API_FEATURES;
...
unsafe extern "C" fn userfaultfd_show_fdinfo(m: *mut seq_file, f: *mut file) {
    ...
    wait_queue_entry_t *wq;
    let mut pending: c_ulong = 0, total = 0;
    spin_lock_irq(&ctx.fault_pending_wqh.lock);
...
#[no_mangle]
unsafe extern "C" fn userfaultfd_init() -> int __init {
