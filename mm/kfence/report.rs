...
// May be overridden by <asm/kfence.h>.
    ...
    let mut __ro_after_init: static enum kfence_fault kfence_fault = KFENCE_FAULT_REPORT;
#[no_mangle]
unsafe extern "C" fn early_kfence_fault(arg: *mut c_char) -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(arg, _arg: "oops")) -> else {
...
//
    ...
    let mut len: c_int = scnprintf(buf, sizeof(buf), "%ps", (void *)stack_entries[skipnr]);
...
//
    ...
    let mut ts_sec: u64 = track.ts_nsec;
    let mut rem_nsec: c_ulong = do_div(ts_sec, NSEC_PER_SEC);
    let mut interval_nsec: u64 = local_clock() - track.ts_nsec;
    let mut rem_interval_nsec: c_ulong = do_div(interval_nsec, NSEC_PER_SEC);
...
// Skip allocation/free internals stack.
    let mut i: c_int = get_stack_skipnr(track.stack_entries, track.num_stack_entries, core::ptr::null_mut());
...
pub unsafe extern "C" fn kfence_print_object(seq: *mut seq_file, meta: *const kfence_metadata) {
    ...
    {
    let mut size: c_int = abs(meta.size);
    let mut start: c_ulong = meta.addr;
    let mut cache: *const kmem_cache const = meta.cache;
...
//
    ...
    {
    let mut show_until_addr: c_ulong = address + bytes_to_show;
    const u8 *cur, *end;
// Do not show contents of object nor read into following guard page.
    ...
    pr_cont(" .");
#[no_mangle]
pub unsafe extern "C" fn if(_arg: no_hash_pointers) -> else {
    ...
    let mut object_index: ptrdiff_t = meta ? meta - kfence_metadata : -1;
    int num_stack_entries;
    let mut skipnr: c_int = 0;
    if (regs) {
...
// Print report header.
    switch (type) {
    case KFENCE_ERROR_OOB: {
    let mut left_of_object: bool = address < meta.addr;
...
#[no_mangle]
pub unsafe extern "C" fn kfence_handle_fault(fault: enum kfence_fault) {
