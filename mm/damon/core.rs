...
#[no_mangle]
unsafe extern "C" fn __damon_is_registered_ops(id: enum damon_ops_id) -> bool {
    static bool __damon_is_registered_ops(enum damon_ops_id id)
    {
    let mut empty_ops: damon_operations = {};
...
//
#[no_mangle]
pub unsafe extern "C" fn damon_is_registered_ops(id: enum damon_ops_id) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn damon_select_ops(ctx: *mut damon_ctx, id: enum damon_ops_id) -> c_int {
...
// Add new regions or resize existing regions to fit in the ranges
    for (i = 0; i < nr_ranges; i++) {
    struct damon_region *first = core::ptr::null_mut(), *last, *newr;
    struct damon_addr_range *range;
    let mut insert_before_r: bool = false;
    range = &ranges[i];
...
//
#[no_mangle]
pub unsafe extern "C" fn damos_filter_for_ops(type: enum damos_filter_type) -> bool {
...
unsafe extern "C" fn damon_apply_min_nr_regions(ctx: *mut damon_ctx) -> c_ulong {
    ...
    {
    let mut max_region_sz: c_ulong = damon_region_sz_limit(ctx);
    struct damon_target *t;
    struct damon_region *r, *next;
...
unsafe extern "C" fn damon_get_intervals_score(c: *mut damon_ctx) -> c_ulong {
    ...
    let mut goal_bp: c_ulong = c.attrs.intervals_goal.access_bp;
    damon_for_each_target(t, c) {
    damon_for_each_region(r, t) {
...
unsafe extern "C" fn kdamond_tune_intervals(c: *mut damon_ctx) -> noinline_for_stack void {
    ...
    {
    unsigned long sz;
    let mut nr_accesses: c_uint = damon_nr_accesses_mvsum(r, c);
    sz = damon_sz_region(r);
...
unsafe extern "C" fn damos_quota_is_set(quota: *mut damos_quota) -> bool {
    ...
    struct damos *s)
    {
    let mut ret: bool = __damos_valid_target(r, s, c);
...
// start inside the range
    ...
    let mut matched: return = = filter.matching;
    }
...
//
    ...
    {
    struct damos_quota *quota = &s.quota;
    let mut sz: c_ulong = damon_sz_region(r);
    struct timespec64 begin, end;
    let mut sz_applied: c_ulong = 0;
    let mut sz_ops_filter_passed: c_ulong = 0;
//
...
// index here.
//
    let mut cidx: c_uint = 0;
    struct damos *siter;		/* schemes iterator */
    let mut sidx: c_uint = 0;
    struct damon_target *titer;	/* targets iterator */
    let mut tidx: c_uint = 0, nr_accesses = 0;
    let mut do_trace: bool = false;
...
//
    ...
    unsigned long score)
    {
    let mut goal: c_ulong = 10000;
// Set minimum input as 10000 to avoid compensation be zero
    let mut min_input: c_ulong = 10000;
    unsigned long score_goal_diff, compensation;
    let mut over_achieving: bool = score > goal;
    if (score == goal)
    return last_input;
    if (score >= goal * 2)
    return min_input;
...
//
    ...
    struct damon_target *t;
    struct damon_region *r;
    let mut total_eligible: phys_addr_t = 0;
    let mut node_eligible: phys_addr_t = 0;
    damon_for_each_target(t, c) {
    damon_for_each_region(r, t) {
...
unsafe extern "C" fn damos_quota_score(c: *mut damon_ctx, s: *mut damos) -> c_ulong {
    ...
    struct damos_quota_goal *goal;
    struct damos_quota *quota = &s.quota;
    let mut highest_score: c_ulong = 0;
    damos_for_each_quota_goal(goal, quota) {
...
unsafe extern "C" fn damos_goal_tune_esz_bp_consist(c: *mut damon_ctx, s: *mut damos) {
    ...
    {
    struct damos_quota *quota = &s.quota;
    let mut score: c_ulong = damos_quota_score(c, s);
    ...
    quota.esz_bp = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: quota->sz) -> else {
    else if (quota.sz)
...
#[no_mangle]
pub unsafe extern "C" fn if(DAMOS_QUOTA_GOAL_TUNER_TEMPORAL: quota->goal_tuner ==) -> else {
...
unsafe extern "C" fn kdamond_apply_schemes(c: *mut damon_ctx) {
    ...
    struct damon_target *t;
    struct damos *s;
    let mut has_schemes_to_apply: bool = false;
    unsigned long max_region_sz;
    damon_for_each_scheme(s, c) {
    if (time_before(c.passed_sample_intervals, s.next_apply_sis))
    continue;
...
//
    ...
    struct damon_region *l, struct damon_region *r)
    {
    let mut sz_l: c_ulong = damon_sz_region(l), sz_r = damon_sz_region(r);
    int i;
...
//
    ...
    let mut use_probe_hits: bool = damon_has_probe_weights(ctx);
    damon_for_each_region_safe(r, next, t) {
    ...
    r.age = 0;
#[no_mangle]
pub unsafe extern "C" fn if(0): (score == 0) != (last_score ==) -> else {
...
// Split one in every @split_step regions into two, from a rotating offset
    ...
    struct damon_target *t;
    struct damon_region *r, *next;
    let mut offset: c_ulong = rotation++ % split_step;
...
unsafe extern "C" fn kdamond_split_regions(ctx: *mut damon_ctx) {
    ...
    {
    struct damon_target *t;
    let mut nr_regions: c_ulong = 0;
    let mut max_nr_regions: c_ulong = ctx.attrs.max_nr_regions;
    static unsigned long last_nr_regions;
    let mut nr_subregions: c_int = 2;
    damon_for_each_target(t, ctx)
    nr_regions += damon_nr_regions(t);
...
unsafe extern "C" fn damon_verify_ctx(c: *mut damon_ctx) {
    ...
    struct damon_region *r;
    damon_for_each_target(t, c) {
    struct damon_region *prev_r = core::ptr::null_mut();
...
unsafe extern "C" fn kdamond_call(ctx: *mut damon_ctx, cancel: bool) {
    ...
    complete(&control.completion);
#[no_mangle]
pub unsafe extern "C" fn if(control->dealloc_on_cancel: control->canceled &&) -> else {
...
unsafe extern "C" fn kdamond_wait_activation(ctx: *mut damon_ctx) -> c_int {
    ...
    let mut min_wait_time: c_ulong = 0;
    let mut init_wait_time: bool = false;
...
//
    let mut next_aggregation_sis: c_ulong = ctx.next_aggregation_sis;
    let mut next_ops_update_sis: c_ulong = ctx.next_ops_update_sis;
    let mut sample_interval: c_ulong = ctx.attrs.sample_interval;
    let mut access_check_disabled: bool = damon_has_probe_weights(ctx);
    let mut max_merge_score: c_uint = 0, max_wsum;
    bool get_max_wsum;
...
//
    ...
    {
    let mut arg: damon_system_ram_range_walk_arg = {};
...
#[no_mangle]
unsafe extern "C" fn damon_init() -> int __init {
