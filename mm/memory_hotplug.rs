...
//
    ...
    let mut __read_mostly: static int memmap_mode = MEMMAP_ON_MEMORY_DISABLE;
#[no_mangle]
...
pub unsafe extern "C" fn memory_block_memmap_on_memory_pages() -> c_ulong {
    ...
    {
    let mut nr_pages: c_ulong = PFN_UP(memory_block_memmap_size());
//
...
// ((int *)kp->arg) = mode;
    ...
    let mut memmap_pages: c_ulong = memory_block_memmap_on_memory_pages();
...
unsafe extern "C" fn set_online_policy(val: *const c_char, kp: *const kernel_param) -> c_int {
    ...
    {
    let mut ret: c_int = sysfs_match_string(online_policy_to_str, val);
    if (ret < 0)
    return ret;
// ((int *)kp->arg) = ret;
    return 0;
...
//
    let mut __read_mostly: static int online_policy = ONLINE_POLICY_CONTIG_ZONES;
...
//
    let mut __read_mostly: static unsigned int auto_movable_ratio = 301;
...
//
    ...
    let mut __read_mostly: static bool auto_movable_numa_aware = true;
...
//
    let mut online_page_callback: static online_page_callback_t = generic_online_page;
...
pub unsafe extern "C" fn put_online_mems() {
    ...
    let mut movable_node_enabled: bool = false;
    let mut mhp_default_online_type: static int = -1;
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ENABLED(CONFIG_MHP_DEFAULT_ONLINE_TYPE_ONLINE_AUTO)) -> else {
    else if (IS_ENABLED(CONFIG_MHP_DEFAULT_ONLINE_TYPE_ONLINE_AUTO))
    mhp_default_online_type = MMOP_ONLINE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ENABLED(CONFIG_MHP_DEFAULT_ONLINE_TYPE_ONLINE_KERNEL)) -> else {
    else if (IS_ENABLED(CONFIG_MHP_DEFAULT_ONLINE_TYPE_ONLINE_KERNEL))
    mhp_default_online_type = MMOP_ONLINE_KERNEL;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ENABLED(CONFIG_MHP_DEFAULT_ONLINE_TYPE_ONLINE_MOVABLE)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn mhp_set_default_online_type(online_type: enum mmop) {
    void mhp_set_default_online_type(enum mmop online_type)
    {
    mhp_default_online_type = online_type;
    }
#[no_mangle]
unsafe extern "C" fn setup_memhp_default_state(str: *mut c_char) -> int __init {
    static int __init setup_memhp_default_state(char *str)
    {
    let mut online_type: c_int = mhp_online_type_from_str(str);
    if (online_type >= 0)
    mhp_default_online_type = online_type;
    return 1;
    }
...
pub unsafe extern "C" fn mem_hotplug_done() {
    ...
    let mut max_mem_size: u64 = U64_MAX;
...
//
    ...
    {
    let mut nr: c_ulong = pfn_to_section_nr(pfn);
...
// The presence of a pgmap indicates ZONE_DEVICE offline pfn
    ...
    {
    let mut end_pfn: c_ulong = pfn + nr_pages;
...
// pfn is the end pfn of a memory section.
    ...
    {
    unsigned long pfn;
    let mut nid: c_int = zone_to_nid(zone);
...
unsafe extern "C" fn update_pgdat_span(pgdat: *mut pglist_data) {
    ...
    {
    let mut node_start_pfn: c_ulong = 0, node_end_pfn = 0;
    struct zone *zone;
...
// mark all involved sections as online
    ...
    unsigned long nr_pages)
    {
    let mut old_end_pfn: c_ulong = zone_end_pfn(zone);
    ...
    unsigned long nr_pages)
    {
    let mut old_end_pfn: c_ulong = pgdat_end_pfn(pgdat);
...
//
    ...
    {
    struct pglist_data *pgdat = zone.zone_pgdat;
    let mut nid: c_int = pgdat.node_id;
    clear_zone_contiguous(zone);
...
pub struct auto_movable_group_stats {
    ...
    void *arg)
    {
    let mut ratio: c_int = READ_ONCE(auto_movable_ratio);
...
//
    ...
    let mut group_stats: auto_movable_group_stats = {};
    let mut stats: auto_movable_stats = {};
    struct zone *zone;
    int i;
...
//
    ...
    unsigned long nr_pages)
    {
    let mut online_pages: c_ulong = 0, max_pages, end_pfn;
    struct page *page;
...
//
    ...
    let mut in_kernel: bool = zone_intersects(kernel_zone, start_pfn, nr_pages);
    let mut in_movable: bool = zone_intersects(movable_zone, start_pfn, nr_pages);
//
...
//
    ...
    let mut movable: bool = zone_idx(zone) == ZONE_MOVABLE;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(!movable: group &&) -> else {
...
//
    ...
    let mut nid: c_int = zone_to_nid(zone);
    let mut need_zonelists_rebuild: c_int = 0;
    unsigned long flags;
    int ret;
//
...
pub unsafe extern "C" fn mhp_supports_memmap_on_memory() -> bool {
    ...
    {
    let mut vmemmap_size: c_ulong = memory_block_memmap_size();
    let mut memmap_pages: c_ulong = memory_block_memmap_on_memory_pages();
//
...
unsafe extern "C" fn remove_memory_blocks_and_altmaps(start: u64, size: u64) {
    ...
    {
    let mut memblock_size: c_ulong = memory_block_size_bytes();
    u64 cur_start;
//
...
//
    ...
    struct vmem_altmap *altmap = core::ptr::null_mut();
...
//
    ...
    {
    let mut params: mhp_params = { .pgprot = pgprot_mhp(PAGE_KERNEL) };
    let mut memblock_flags: enum memblock_flags = MEMBLOCK_NONE;
    struct memory_group *group = core::ptr::null_mut();
    u64 start, size;
    let mut new_node: bool = false;
    int ret;
    start = res.start;
...
pub unsafe extern "C" fn mhp_get_pluggable_range(need_mapping: bool) -> range {
    ...
    {
    let mut max_phys: u64 = DIRECT_MAP_PHYSMEM_END;
    struct range mhp_range;
    if (need_mapping) {
...
pub unsafe extern "C" fn mhp_range_allowed(start: u64, size: u64, need_mapping: bool) -> bool {
    ...
    {
    let mut mhp_range: range = mhp_get_pluggable_range(need_mapping);
...
//
    ...
    let mut nmask: nodemask_t = node_states[N_MEMORY];
...
#[no_mangle]
unsafe extern "C" fn cmdline_parse_movable_node(p: *mut c_char) -> int __init {
...
//
    ...
    char *reason;
    int ret;
    let mut normal_pages: c_ulong = 0;
    enum zone_type zt;
//
// {on,off}lining is constrained to full memory sections (or more
// precisely to memory blocks from the user space POV).
...
unsafe extern "C" fn memory_blocks_have_altmaps(start: u64, size: u64) -> c_int {
    ...
    {
    let mut num_memblocks: u64 = size / memory_block_size_bytes();
    let mut num_altmaps: u64 = 0;
    if (!mhp_memmap_on_memory())
    return 0;
...
unsafe extern "C" fn try_offline_memory_block(mem: *mut memory_block, arg: *mut c_void) -> c_int {
    ...
    {
    let mut online_type: enum mmop = MMOP_ONLINE_KERNEL;
...
//
    ...
    let mut start: u64 = ranges[i].start;
    let mut size: u64 = range_len(&ranges[i]);
