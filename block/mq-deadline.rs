...
//
    let mut prio_aging_expire: static int = 10 * HZ;
...
//
    ...
    let mut ioprio_class: u8 = dd_rq_ioclass(req);
    let mut prio: enum dd_prio = ioprio_class_to_prio[ioprio_class];
...
//
    ...
    {
    struct deadline_data *dd = q.elevator.elevator_data;
    let mut ioprio_class: u8 = dd_rq_ioclass(next);
    let mut prio: enum dd_prio = ioprio_class_to_prio[ioprio_class];
...
#[no_mangle]
unsafe extern "C" fn dd_queued(dd: *mut deadline_data, prio: enum dd_prio) -> u32 {
...
//
    ...
    {
    let mut start_time: c_ulong = (unsigned long)rq.fifo_time;
    ...
    struct request *rq)
    {
    let mut ioprio_class: u8 = dd_rq_ioclass(rq);
    let mut prio: enum dd_prio = ioprio_class_to_prio[ioprio_class];
...
//
    ...
    {
    struct deadline_data *dd = q.elevator.elevator_data;
    let mut ioprio_class: u8 = IOPRIO_PRIO_CLASS(bio.bi_ioprio);
    let mut prio: enum dd_prio = ioprio_class_to_prio[ioprio_class];
    struct dd_per_prio *per_prio = &dd.per_prio[prio];
    let mut sector: sector_t = bio_end_sector(bio);
    struct request *__rq;
    if (!dd.front_merges)
...
//
    ...
    {
    struct deadline_data *dd = q.elevator.elevator_data;
    struct request *free = core::ptr::null_mut();
    bool ret;
    spin_lock(&dd.lock);
...
//
    ...
    struct deadline_data *dd = q.elevator.elevator_data;
    let mut data_dir: enum dd_data_dir = rq_data_dir(rq);
    let mut ioprio: u16 = req_get_ioprio(rq);
    let mut ioprio_class: u8 = IOPRIO_PRIO_CLASS(ioprio);
    struct dd_per_prio *per_prio;
    enum dd_prio prio;
    lockdep_assert_held(&dd.lock);
...
#[no_mangle]
unsafe extern "C" fn dd_owned_by_driver(dd: *mut deadline_data, prio: enum dd_prio) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn deadline_init() -> int __init {
    static int __init deadline_init(void)
    {
    return elv_register(&mq_deadline);
    }
#[no_mangle]
unsafe extern "C" fn deadline_exit() -> void __exit {
