...
// Should we allow writing to mounted block devices?
    let mut bdev_allow_write_mounted: static bool = IS_ENABLED(CONFIG_BLK_DEV_WRITE_MOUNTED);
#[repr(C)]
#[derive(Copy, Clone)]
...
//
    ...
    let mut err: c_int = bd_prepare_to_claim(bdev, truncate_bdev_range, core::ptr::null_mut());
    if (err)
    goto invalidate;
    }
...
unsafe extern "C" fn set_init_blocksize(bdev: *mut block_device) {
    ...
    {
    let mut bsize: c_uint = bdev_logical_block_size(bdev);
    let mut size: loff_t = i_size_read(BD_INODE(bdev));
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE)) -> else {
...
pub unsafe extern "C" fn sb_min_blocksize(sb: *mut super_block, size: c_int) -> int __must_check {
    ...
    {
    let mut minsize: c_int = bdev_logical_block_size(sb.s_bdev);
    if (size < minsize)
    size = minsize;
...
pub unsafe extern "C" fn bdev_thaw(bdev: *mut block_device) -> c_int {
    ...
    {
    let mut error: c_int = -EINVAL, nr_freeze;
    mutex_lock(&bdev.bd_fsfreeze_mutex);
...
#[no_mangle]
pub unsafe extern "C" fn bdev_cache_init() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(BLK_OPEN_WRITE: mode &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(FMODE_WRITE: bdev_file->f_mode &) -> else {
...
//
    ...
    {
    let mut unblock_events: bool = true;
    struct gendisk *disk = bdev.bd_disk;
    int ret;
    if (holder) {
    mode |= BLK_OPEN_EXCL;
...
unsafe extern "C" fn blk_to_file_flags(mode: blk_mode_t) -> unsigned {
    ...
    flags |= O_RDWR;
#[no_mangle]
pub unsafe extern "C" fn if(BLK_OPEN_WRITE_IOCTL: mode &) -> else {
...
pub unsafe extern "C" fn if(BLK_OPEN_WRITE: mode &) -> else {
    ...
    flags |= O_WRONLY;
#[no_mangle]
pub unsafe extern "C" fn if(BLK_OPEN_READ: mode &) -> else {
...
#[no_mangle]
unsafe extern "C" fn setup_bdev_allow_write_mounted(str: *mut c_char) -> int __init {
