...
unsafe extern "C" fn blk_queue_max_guaranteed_bio(lim: *mut queue_limits) -> c_uint {
    ...
    {
    let mut max_segments: c_uint = min(BIO_MAX_VECS, lim.max_segments);
    unsigned int length;
...
//
    ...
    lim.logical_block_size = SECTOR_SIZE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: blk_validate_block_size(lim->logical_block_size)) -> else {
...
pub unsafe extern "C" fn queue_limits_set(q: *mut request_queue, lim: *mut queue_limits) -> c_int {
    ...
    sector_t sector)
    {
    let mut granularity: c_uint = max(lim.physical_block_size, lim.io_min);
