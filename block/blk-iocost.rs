...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ioc_params {
    pub qos: [u32; NR_QOS_PARAMS],
    pub i_lcoefs: [u64; NR_I_LCOEFS],
    pub lcoefs: [u64; NR_LCOEFS],
    pub too_fast_vrate_pct: u32,
    pub too_slow_vrate_pct: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ioc_margins {
    pub min: i64,
    pub low: i64,
    pub target: i64,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ioc_missed {
    pub nr_met: local_t,
    pub nr_missed: local_t,
    pub last_met: u32,
    pub last_missed: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ioc_pcpu_stat {
    pub missed: [ioc_missed; 2],
    pub rq_wait_ns: local64_t,
    pub last_rq_wait_ns: u64,
}
...
pub struct ioc {
    pub rqos: rq_qos,
    pub enabled: bool,
    pub params: ioc_params,
    pub margins: ioc_margins,
    pub period_us: u32,
    pub timer_slack_ns: u32,
    pub vrate_min: u64,
    pub vrate_max: u64,
    pub lock: spinlock_t,
...
// debt forgivness
    ...
    pub dfgv_usage_us_sum: u64,
    pub autop_too_fast_at: u64,
    pub autop_too_slow_at: u64,
    pub autop_idx: c_int,
    pub user_qos_params:1: bool,
    pub user_cost_model:1: bool,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct iocg_pcpu_stat {
    pub abs_vusage: local64_t,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct iocg_stat {
    pub usage_us: u64,
    pub wait_us: u64,
    pub indebt_us: u64,
    pub indelay_us: u64,
...
//
    pub vtime: core::sync::atomic::AtomicI64,
    pub done_vtime: core::sync::atomic::AtomicI64,
    pub abs_vdebt: u64,
...
//
    pub active_period: core::sync::atomic::AtomicI64,
    pub active_list: list_head,
// see __propagate_weights() and current_hweight() for details
    pub child_active_sum: u64,
    pub child_inuse_sum: u64,
    pub child_adjusted_sum: u64,
    pub hweight_gen: c_int,
    pub hweight_active: u32,
    pub hweight_inuse: u32,
    pub hweight_donating: u32,
    pub hweight_after_donation: u32,
    pub walk_list: list_head,
    pub surplus_list: list_head,
    pub waitq: wait_queue_head,
    pub waitq_timer: hrtimer,
...
// statistics
    ...
    pub wait_since: u64,
    pub indebt_since: u64,
    pub indelay_since: u64,
// this iocg's depth in the hierarchy and ancestors including self
    pub level: c_int,
    pub ancestors: [*mut ioc_gq; ],
}
...
#[derive(Copy, Clone)]
pub struct ioc_cgrp {
    pub cpd: blkcg_policy_data,
    pub dfl_weight: c_uint,
}
...
//
    ctx.vbudget = vbudget;
    current_hweight(iocg, core::ptr::null_mut(), &ctx.hw_inuse);
...
unsafe extern "C" fn ioc_qos_show(sf: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    { QOS_ENABLE,		"enable=%u"	},
    { QOS_CTRL,		"ctrl=%s"	},
    { NR_QOS_CTRL_PARAMS,	core::ptr::null_mut()		},
    };
...
unsafe extern "C" fn ioc_cost_model_show(sf: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    { COST_CTRL,		"ctrl=%s"	},
    { COST_MODEL,		"model=%s"	},
    { NR_COST_CTRL_PARAMS,	core::ptr::null_mut()		},
    };
