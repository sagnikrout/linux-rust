...
pub unsafe extern "C" fn unregister_blkdev(major: c_uint, name: *const c_char) {
    ...
    struct blk_major_name *p = core::ptr::null_mut();
    let mut index: c_int = major_to_index(major);
    mutex_lock(&major_names_lock);
    spin_lock(&major_names_spinlock);
    for (n = &major_names[index]; *n; n = &(*n).next)
    if ((*n).major == major)
...
#[no_mangle]
pub unsafe extern "C" fn disk_uevent(disk: *mut gendisk, action: enum kobject_action) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: queue_is_mq(q)) -> else {
...
unsafe extern "C" fn blk_probe_dev(devt: dev_t) -> bool {
    ...
    {
    let mut major: c_uint = MAJOR(devt);
    struct blk_major_name **n;
    mutex_lock(&major_names_lock);
...
#[no_mangle]
unsafe extern "C" fn genhd_device_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn proc_genhd_init() -> int __init {
...
// bdev_alloc() might need the queue, set before the first call
    ...
    struct lock_class_key *lkclass)
    {
    let mut default_lim: queue_limits = { };
