...
//
    ...
    bio_io_error(bio);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !blk_throtl_bio(bio)) -> else {
...
//
    ...
    {
    let mut pbs: unsigned = lim.physical_block_size >> SECTOR_SHIFT;
    let mut lbs: unsigned = lim.logical_block_size >> SECTOR_SHIFT;
    let mut is_atomic: bool = bio.bi_opf & REQ_ATOMIC;
    let mut boundary_sectors: unsigned = blk_boundary_sectors(lim, is_atomic);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_atomic) -> else {
    else if (is_atomic)
...
//
    ...
    {
    let mut max_len: unsigned = max_bytes - *bytes;
    let mut len: unsigned = min(bv.bv_len, max_len);
    let mut total_len: unsigned = 0;
    let mut seg_size: unsigned = 0;
...
//
    ...
    let mut nsegs: unsigned = 0, bytes = 0, gaps = 0;
    struct bvec_iter iter;
    let mut start_align_mask: unsigned = lim.dma_alignment;
    if (bc) {
...
//
    ...
    const struct queue_limits *lim, unsigned *nsegs)
    {
    let mut max_sectors: c_uint = get_max_io_size(bio, lim);
// nsegs = 0;
//
...
pub unsafe extern "C" fn blk_recalc_rq_segments(rq: *mut request) -> c_uint {
    ...
    let mut is_atomic: bool = rq.cmd_flags & REQ_ATOMIC;
...
#[no_mangle]
pub unsafe extern "C" fn if(blk_rq_pos(next): blk_rq_pos(req) + blk_rq_sectors(req) ==) -> else {
...
pub unsafe extern "C" fn blk_try_merge(rq: *mut request, bio: *mut bio) -> enum elv_merge {
    ...
    return ELEVATOR_DISCARD_MERGE;
#[no_mangle]
pub unsafe extern "C" fn if(bio->bi_iter.bi_sector: blk_rq_pos(rq) + blk_rq_sectors(rq) ==) -> else {
    else if (blk_rq_pos(rq) + blk_rq_sectors(rq) == bio.bi_iter.bi_sector)
    return ELEVATOR_BACK_MERGE;
#[no_mangle]
pub unsafe extern "C" fn if(bio->bi_iter.bi_sector: blk_rq_pos(rq) - bio_sectors(bio) ==) -> else {
...
unsafe extern "C" fn blk_account_io_merge_bio(req: *mut request) {
    ...
    {
    let mut ff: blk_opf_t = bio_failfast(bio);
...
//
    ...
    BIO_MERGE_OK;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !plug->multiple_queues) -> else {
