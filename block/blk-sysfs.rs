...
pub struct queue_sysfs_entry {
    pub attr: attribute,
    pub page): *mut *mut *mut ssize_t (show)(struct gendisk disk, char,
    pub page): *mut *mut *mut ssize_t (show_limit)(struct gendisk disk, char,
    pub count): *const *const *const *const ssize_t (store)(struct gendisk disk, char page, size_t,
    int (*store_limit)(struct gendisk *disk, const char *page,
    pub lim): *mut size_t count, struct queue_limits,
}
...
unsafe extern "C" fn queue_requests_show(disk: *mut gendisk, page: *mut c_char) -> isize {
    ...
    struct elevator_tags *et = core::ptr::null_mut();
...
unsafe extern "C" fn queue_nomerges_show(disk: *mut gendisk, page: *mut c_char) -> isize {
    ...
    unsigned long nm;
    struct request_queue *q = disk.queue;
    let mut ret: isize = queue_var_store(&nm, page, count);
    if (ret < 0)
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: nm) -> else {
    else if (nm)
...
unsafe extern "C" fn queue_rq_affinity_show(disk: *mut gendisk, page: *mut c_char) -> isize {
    ...
    {
    let mut set: bool = test_bit(QUEUE_FLAG_SAME_COMP, &disk.queue.queue_flags);
    let mut force: bool = test_bit(QUEUE_FLAG_SAME_FORCE, &disk.queue.queue_flags);
...
//
    ...
    ssize_t res;
    let mut lim: queue_limits = queue_limits_start_update(q);
