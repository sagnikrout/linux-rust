...
unsafe extern "C" fn dio_bio_write_op(iocb: *mut kiocb) -> blk_opf_t {
    ...
    {
    let mut opf: blk_opf_t = REQ_OP_WRITE | REQ_SYNC | REQ_IDLE;
...
pub const DIO_INLINE_BIO_VECS: c_int = 4;
    ...
    let mut should_dirty: bool = false;
    struct bio bio;
    ssize_t ret;
...
unsafe extern "C" fn blkdev_bio_end_io(bio: *mut bio) {
    ...
    let mut should_dirty: bool = dio.flags & DIO_SHOULD_DIRTY;
    let mut is_sync: bool = dio.flags & DIO_IS_SYNC;
    ...
    struct bio *bio;
    let mut is_read: bool = (iov_iter_rw(iter) == READ), is_sync;
    let mut opf: blk_opf_t = is_read ? REQ_OP_READ : dio_bio_write_op(iocb);
    let mut pos: loff_t = iocb.ki_pos;
    let mut ret: c_int = 0;
...
unsafe extern "C" fn blkdev_bio_end_io_async(bio: *mut bio) {
    ...
    unsigned int nr_pages)
    {
    let mut is_read: bool = iov_iter_rw(iter) == READ;
    let mut opf: blk_opf_t = is_read ? REQ_OP_READ : dio_bio_write_op(iocb);
    struct blkdev_dio *dio;
    struct bio *bio;
    let mut pos: loff_t = iocb.ki_pos;
...
unsafe extern "C" fn blkdev_direct_IO(iocb: *mut kiocb, iter: *mut iov_iter) -> isize {
    ...
    return -EINVAL;
    if (iov_iter_rw(iter) == WRITE) {
    let mut max_write_streams: u16 = bdev_max_write_streams(bdev);
...
unsafe extern "C" fn blkdev_readahead(rac: *mut readahead_control) {
    ...
    {
    let mut isize: loff_t = i_size_read(wpc.inode);
...
//
    ...
    mode |= BLK_OPEN_EXCL;
#[no_mangle]
pub unsafe extern "C" fn if(O_EXCL: file->f_flags &) -> else {
...
unsafe extern "C" fn blkdev_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    ...
    let mut atomic: bool = iocb.ki_flags & IOCB_ATOMIC;
    let mut size: loff_t = bdev_nr_bytes(bdev);
    let mut shorted: usize = 0;
    ssize_t ret;
...
//
    ...
    struct block_device *bdev = I_BDEV(inode);
    let mut end: loff_t = start + len - 1;
    loff_t isize;
    unsigned int flags;
    int error;
