...
//
    ...
    let mut real_tag: c_int = tag - tags.nr_reserved_tags;
...
unsafe extern "C" fn bt_tags_iter(bitmap: *mut sbitmap, bitnr: c_uint, data: *mut c_void) -> bool {
    ...
    struct request *rq;
    let mut ret: bool = true;
    let mut iter_static_rqs: bool = !!(iter_data.flags & BT_TAG_ITER_STATIC_RQS);
...
//
    ...
    {
    let mut flags: c_uint = tagset.flags;
...
//
    ...
    {
    let mut depth: c_uint = total_tags - reserved_tags;
    let mut round_robin: bool = flags & BLK_MQ_F_TAG_RR;
    struct blk_mq_tags *tags;
