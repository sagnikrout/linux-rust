...
pub struct solaris_x86_vtoc {
    ...
    pub /: *mut *mut unsigned int v_reserved[10]; / free space,
...
pub struct unixware_vtoc {
    ...
    pub /: *mut *mut __le32 v_reserved[10]; / reserved,
...
//
    state.next = 5;
    for (slot = 1 ; slot <= 4 ; slot++, p++) {
    let mut start: sector_t = start_sect(p)*sector_size;
    let mut size: sector_t = nr_sects(p)*sector_size;
    if (!size)
    continue;
