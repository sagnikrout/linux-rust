...
// we're only returning the result field in the reply
    ...
    let mut len: c_int = min(hdr.max_response_len, job.reply_len);
...
// we assume all request payload was transferred, residual == 0
    hdr.dout_resid = 0;
    if (job.bidi_rq) {
    let mut rsp_len: c_uint = job.reply_payload.payload_len;
...
unsafe extern "C" fn bsg_map_buffer(buf: *mut bsg_buffer, req: *mut request) -> c_int {
    ...
    {
    let mut sz: usize = (sizeof(struct scatterlist) * req.nr_phys_segments);
...
//
    ...
    let mut sts: blk_status_t = BLK_STS_IOERR;
    int ret;
