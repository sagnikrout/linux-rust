...
//
    ...
    {
    const char *expect_or_assert = core::ptr::null_mut();
    switch (type) {
...
#[no_mangle]
pub unsafe extern "C" fn is_literal(text: *const c_char, value: c_longlong) -> VISIBLE_IF_KUNIT bool {
    VISIBLE_IF_KUNIT bool is_literal(const char *text, long long value)
    {
    char *buffer;
    int len;
    bool ret;
    len = snprintf(core::ptr::null_mut(), 0, "%lld", value);
    if (strlen(text) != len)
    return false;
...
//
#[no_mangle]
pub unsafe extern "C" fn is_str_literal(text: *const c_char, value: *const c_char) -> VISIBLE_IF_KUNIT bool {
