...
#[derive(Copy, Clone)]
pub struct test_obj {
    pub value: test_obj_val,
    pub node: rhash_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_obj_rhl {
    pub value: test_obj_val,
    pub list_node: rhlist_head,
}
...
pub struct thread_data {
    ...
    pub objs: *mut test_obj,
}

#[no_mangle]
unsafe extern "C" fn my_hashfn(data: *const c_void, len: u32, seed: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn my_cmpfn(arg: *mut rhashtable_compare_arg, obj: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_bucket_stats(ht: *mut rhashtable, entries: c_uint) {
    static void test_bucket_stats(struct rhashtable *ht, unsigned int entries)
    {
    let mut total: c_uint = 0, chain_len = 0;
...
#[no_mangle]
unsafe extern "C" fn test_rhltable(entries: c_uint) -> int __init {
    ...
    for (j = 0; j < entries; j++) {
    let mut i: u32 = get_random_u32_below(entries);
    let mut prand: u32 = get_random_u32_below(4);
    cond_resched();
    err = rhltable_remove(&rhlt, &rhl_test_objects[i].list_node, test_rht_params);
...
#[no_mangle]
unsafe extern "C" fn print_ht(rhlt: *mut rhltable) -> unsigned int __init {
...
// Take the mutex to avoid RCU warning
    ...
    next = !rht_is_a_nulls(pos) ? rht_dereference(pos.next, ht) : core::ptr::null_mut();
    ...
    rht_dereference(pos.next, ht) : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn test_insert_duplicates_run() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn thread_lookup_test(tdata: *mut thread_data) -> c_int {
    static int thread_lookup_test(struct thread_data *tdata)
    {
    let mut entries: c_uint = tdata.entries;
    int i, err = 0;
    for (i = 0; i < entries; i++) {
    struct test_obj *obj;
    struct test_obj_val key = {
...
#[no_mangle]
unsafe extern "C" fn threadfunc(data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_rhashtable_next_key() -> int __init {
    static int __init test_rhashtable_next_key(void)
    {
    let mut params: rhashtable_params = test_rht_params;
    let mut key_missing: test_obj_val = { .id = 99999, .tid = 0 };
    struct test_obj_val *prev_key = core::ptr::null_mut();
    struct rhashtable ht;
    struct test_obj *objs, *cur;
    int i, count = 0, err;
    int visited_keys[8] = { 0 };
    let mut n: c_int = ARRAY_SIZE(visited_keys);
    params.nelem_hint = n;
...
#[no_mangle]
unsafe extern "C" fn test_rht_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn test_rht_exit() -> void __exit {
