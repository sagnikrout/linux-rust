...
//
    ...
    s64 integer)
    {
    let mut data_len: c_int = end_data - data;
...
// d++ = 0;
    ...
    let mut byte: c_int = integer >> (8 * (i - 1));
    if (!found && byte == 0)
    continue;
//
...
#[no_mangle]
unsafe extern "C" fn asn1_encode_oid_digit(_data: *mut c_uchar, data_len: *mut c_int, oid: u32) -> c_int {
    static int asn1_encode_oid_digit(unsigned char **_data, int *data_len, u32 oid)
    {
    unsigned char *data = *_data;
    let mut start: c_int = 7 + 7 + 7 + 7;
    let mut ret: c_int = 0;
    if (*data_len < 1)
    return -EINVAL;
// quick case
...
//
#[no_mangle]
unsafe extern "C" fn asn1_encode_length(data: *mut c_uchar, data_len: *mut c_int, len: c_int) -> c_int {
