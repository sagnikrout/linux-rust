...
//
    ...
    let mut off: unsigned = shift/BITS_PER_LONG, rem = shift % BITS_PER_LONG;
    let mut mask: c_ulong = BITMAP_LAST_WORD_MASK(nbits);
    for (k = 0; off + k < lim; ++k) {
...
//
    ...
    {
    int k;
    let mut lim: c_uint = BITS_TO_LONGS(nbits);
    let mut off: c_uint = shift/BITS_PER_LONG, rem = shift % BITS_PER_LONG;
...
//
    ...
    {
    let mut len: c_uint = BITS_TO_LONGS(nbits);
    let mut keep: c_ulong = 0, carry;
    int i;
    ...
    {
    unsigned int k;
    let mut nr: c_uint = BITS_TO_LONGS(nbits);
    for (k = 0; k < nr; k++)
...
//
#[no_mangle]
unsafe extern "C" fn bitmap_pos_to_ord(buf: *const c_ulong, pos: c_uint, nbits: c_uint) -> c_int {
...
//
    ...
    let mut n: c_int = bitmap_pos_to_ord(old, oldbit, nbits);
    if (n < 0 || w == 0)
...
//
    ...
    {
    let mut w: c_int = bitmap_weight(new, bits);
    let mut n: c_int = bitmap_pos_to_ord(old, oldbit, bits);
    if (n < 0 || w == 0)
    return oldbit;
    else
...
#[no_mangle]
pub unsafe extern "C" fn bitmap_free(bitmap: *const c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn devm_bitmap_free(data: *mut c_void) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bitmap_from_arr64(bitmap: *mut c_ulong, buf: *const u64, nbits: c_uint) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bitmap_to_arr64(buf: *mut u64, bitmap: *const c_ulong, nbits: c_uint) {
