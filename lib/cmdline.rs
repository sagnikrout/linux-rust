...
//
#[no_mangle]
unsafe extern "C" fn get_range(str: *mut c_char, pint: *mut c_int, n: c_int) -> c_int {
    static int get_range(char **str, int *pint, int n)
    {
    int x, inc_counter, upper_range;
    (*str)++;
    upper_range = simple_strtol((*str), core::ptr::null_mut(), 0);
...
//
#[no_mangle]
pub unsafe extern "C" fn get_option(str: *mut c_char, pint: *mut c_int) -> c_int {
...
//
    ...
    break;
    if (res == 3) {
    let mut n: c_int = validate ? 0 : nints - i;
    int range_nums;
...
pub unsafe extern "C" fn memparse(ptr: *const c_char, retptr: *mut c_char) -> c_ulonglong {
    ...
    let mut ret: c_ulonglong = simple_strtoull(ptr, &endptr, 0);
...
//
    ...
    let mut in_quote: c_int = 0, quoted = 0;
    if (*args == '"') {
    args++;
    in_quote = 1;
    quoted = 1;
