...
//
#[no_mangle]
pub unsafe extern "C" fn xa_lock_type(xa: *const xarray) -> c_uint {
    static inline unsigned int xa_lock_type(const struct xarray *xa)
    {
    return ( unsigned int)xa.xa_flags & 3;
    }
#[no_mangle]
pub unsafe extern "C" fn xas_lock_type(xas: *mut xa_state, lock_type: c_uint) {
    static inline void xas_lock_type(struct xa_state *xas, unsigned int lock_type)
    {
    if (lock_type == XA_LOCK_IRQ)
    xas_lock_irq(xas);
#[no_mangle]
pub unsafe extern "C" fn if(XA_LOCK_BH: lock_type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn xas_unlock_type(xas: *mut xa_state, lock_type: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn xa_track_free(xa: *const xarray) -> bool {
    static inline bool xa_track_free(const struct xarray *xa)
    {
    return xa.xa_flags & XA_FLAGS_TRACK_FREE;
    }
#[no_mangle]
pub unsafe extern "C" fn xa_zero_busy(xa: *const xarray) -> bool {
    static inline bool xa_zero_busy(const struct xarray *xa)
    {
    return xa.xa_flags & XA_FLAGS_ZERO_BUSY;
    }
#[no_mangle]
pub unsafe extern "C" fn xa_mark_set(xa: *mut xarray, mark: xa_mark_t) {
...
#[no_mangle]
pub unsafe extern "C" fn xa_mark_clear(xa: *mut xarray, mark: xa_mark_t) {
...
#[no_mangle]
pub unsafe extern "C" fn node_any_mark(node: *mut xa_node, mark: xa_mark_t) -> bool {
    static inline bool node_any_mark(struct xa_node *node, xa_mark_t mark)
    {
    return !bitmap_empty(node_marks(node, mark), XA_CHUNK_SIZE);
    }
#[no_mangle]
pub unsafe extern "C" fn node_mark_all(node: *mut xa_node, mark: xa_mark_t) {
...
//
#[no_mangle]
unsafe extern "C" fn xas_squash_marks(xas: *const xa_state) {
    static void xas_squash_marks(const struct xa_state *xas)
    {
    let mut mark: xa_mark_t = 0;
    let mut limit: c_uint = xas.xa_offset + xas.xa_sibs + 1;
    for (;;) {
...
#[no_mangle]
unsafe extern "C" fn get_offset(index: c_ulong, node: *mut xa_node) -> c_uint {
    static unsigned int get_offset(unsigned long index, struct xa_node *node)
    {
    return (index >> node.shift) & XA_CHUNK_MASK;
    }
#[no_mangle]
unsafe extern "C" fn xas_set_offset(xas: *mut xa_state) {
...
#[no_mangle]
unsafe extern "C" fn xas_move_index(xas: *mut xa_state, offset: c_ulong) {
    static void xas_move_index(struct xa_state *xas, unsigned long offset)
    {
    let mut shift: c_uint = xas.xa_node.shift;
    xas.xa_index &= ~XA_CHUNK_MASK << shift;
    xas.xa_index += offset << shift;
    }
#[no_mangle]
unsafe extern "C" fn xas_next_offset(xas: *mut xa_state) {
...
//
    ...
    struct xa_node *node)
    {
    let mut offset: c_uint = get_offset(xas.xa_index, node);
...
#[no_mangle]
unsafe extern "C" fn xa_node_free(node: *mut xa_node) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_destroy(xas: *mut xa_state) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_nomem(xas: *mut xa_state, gfp: gfp_t) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn __xas_nomem(xas: *mut xa_state, gfp: gfp_t) -> bool {
    static bool __xas_nomem(struct xa_state *xas, gfp_t gfp)
    __must_hold(xas.xa.xa_lock)
    {
    let mut lock_type: c_uint = xa_lock_type(xas.xa);
    if (xas.xa_node != XA_ERROR(-ENOMEM)) {
    xas_destroy(xas);
    return false;
    }
...
#[no_mangle]
unsafe extern "C" fn xas_update(xas: *mut xa_state, node: *mut xa_node) {
...
#[no_mangle]
unsafe extern "C" fn xas_size(xas: *const xa_state) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn xas_max(xas: *mut xa_state) -> c_ulong {
    static unsigned long xas_max(struct xa_state *xas)
    {
    let mut max: c_ulong = xas.xa_index;

    if (xas.xa_shift || xas.xa_sibs) {
    let mut mask: c_ulong = xas_size(xas) - 1;
    max |= mask;
    if (mask == max)
    max++;
    }
...
#[no_mangle]
unsafe extern "C" fn max_index(entry: *mut c_void) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn xas_shrink(xas: *mut xa_state) {
...
//
#[no_mangle]
unsafe extern "C" fn xas_delete_node(xas: *mut xa_state) {
...
//
#[no_mangle]
unsafe extern "C" fn xas_free_nodes(xas: *mut xa_state, top: *mut xa_node) {
...
//
#[no_mangle]
unsafe extern "C" fn xas_expand(xas: *mut xa_state, head: *mut c_void) -> c_int {
    static int xas_expand(struct xa_state *xas, void *head)
    {
    struct xarray *xa = xas.xa;
    struct xa_node *node = core::ptr::null_mut();
    let mut shift: c_uint = 0;
    let mut max: c_ulong = xas_max(xas);
    if (!head) {
    if (max == 0)
    return 0;
...
//
    ...
    struct xa_node *node = xas.xa_node;
    int shift;
    let mut order: c_uint = xas.xa_shift;
    if (xas_top(node)) {
    ...
    let mut offset: c_uint = xas.xa_offset;
    shift = node.shift;
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_create_range(xas: *mut xa_state) {
    void xas_create_range(struct xa_state *xas)
    {
    let mut index: c_ulong = xas.xa_index;
    let mut shift: c_uchar = xas.xa_shift;
    let mut sibs: c_uchar = xas.xa_sibs;
...
//
    ...
    let mut values: c_int = 0;
    void *first, *next;
    let mut value: bool = xa_is_value(entry);
    if (entry) {
    let mut allow_root: bool = !xa_is_node(entry) && !xa_is_zero(entry);
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_get_mark(xas: *const xa_state, mark: xa_mark_t) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_set_mark(xas: *const xa_state, mark: xa_mark_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_clear_mark(xas: *const xa_state, mark: xa_mark_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_init_marks(xas: *const xa_state) {
...
#[no_mangle]
unsafe extern "C" fn node_get_marks(node: *mut xa_node, offset: c_uint) -> c_uint {
    static unsigned int node_get_marks(struct xa_node *node, unsigned int offset)
    {
    let mut marks: c_uint = 0;
    let mut mark: xa_mark_t = XA_MARK_0;
    for (;;) {
    if (node_get_mark(node, offset, mark))
    marks |= 1 << ( unsigned int)mark;
    if (mark == XA_MARK_MAX)
    break;
    mark_inc(mark);
    }
    ...
    {
    unsigned int i;
    void *sibling = core::ptr::null_mut();
    let mut mask: c_uint = xas.xa_sibs;
    if (!node)
    return;
    node.array = xas.xa;
...
//
    ...
    gfp_t gfp)
    {
    let mut sibs: c_uint = (1 << (order % XA_CHUNK_SHIFT)) - 1;
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_split(xas: *mut xa_state, entry: *mut c_void, order: c_uint) {
    ...
    let mut canon: c_uint = offset - xas.xa_sibs;
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_try_split_min_order(order: c_uint) -> c_uint {
    unsigned int xas_try_split_min_order(unsigned int order)
    {
    if (order % XA_CHUNK_SHIFT == 0)
    let mut order: return = = 0 ? 0 : order - 1;
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_try_split(xas: *mut xa_state, entry: *mut c_void, order: c_uint) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_pause(xas: *mut xa_state) {
    ...
    xas.xa_node = XAS_RESTART;
    if (node) {
    let mut offset: c_ulong = xas.xa_offset;
    while (++offset < XA_CHUNK_SIZE) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __xa_insert(xa: *mut xarray, index: c_ulong, entry: *mut c_void, gfp: gfp_t) -> c_int {
    ...
    {
    let mut shift: c_uint = 0;
    let mut sibs: c_ulong = last - first;
    let mut offset: c_uint = XA_CHUNK_MASK;
    xas_set(xas, first);
...
//
    ...
    xas_lock(&xas);
    if (entry) {
    let mut order: c_uint = BITS_PER_LONG;
    if (last + 1)
    order = __ffs(last + 1);
...
//
#[no_mangle]
pub unsafe extern "C" fn xas_get_order(xas: *mut xa_state) -> c_int {
    ...
    for (;;) {
    let mut slot: c_uint = xas.xa_offset + (1 << order);
...
//
#[no_mangle]
pub unsafe extern "C" fn xa_get_order(xa: *mut xarray, index: c_ulong) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn __xa_set_mark(xa: *mut xarray, index: c_ulong, mark: xa_mark_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __xa_clear_mark(xa: *mut xarray, index: c_ulong, mark: xa_mark_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xa_get_mark(xa: *mut xarray, index: c_ulong, mark: xa_mark_t) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn xa_set_mark(xa: *mut xarray, index: c_ulong, mark: xa_mark_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xa_clear_mark(xa: *mut xarray, index: c_ulong, mark: xa_mark_t) {
...
#[no_mangle]
unsafe extern "C" fn xas_sibling(xas: *mut xa_state) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn xa_delete_node(node: *mut xa_node, update: xa_update_node_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn xa_destroy(xa: *mut xarray) {
...
#[no_mangle]
pub unsafe extern "C" fn xa_dump_node(node: *const xa_node) {
...
#[no_mangle]
pub unsafe extern "C" fn xa_dump_index(index: c_ulong, shift: c_uint) {
    void xa_dump_index(unsigned long index, unsigned int shift)
    {
    if (!shift)
    pr_info("%lu: ", index);
#[no_mangle]
pub unsafe extern "C" fn if(BITS_PER_LONG: shift >=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn xa_dump_entry(entry: *const c_void, index: c_ulong, shift: c_ulong) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !xa_is_internal(entry)) -> else {
    else if (!xa_is_internal(entry))
    pr_cont("%px\n", entry);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: xa_is_retry(entry)) -> else {
    else if (xa_is_retry(entry))
    pr_cont("retry (%ld)\n", xa_to_internal(entry));
#[no_mangle]
pub unsafe extern "C" fn if(_arg: xa_is_sibling(entry)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn xa_dump(xa: *const xarray) {
