...
//! Source: lib/raid/raid6/tests/raid6_kunit.c
#![no_std]
...
pub const RAID6_KUNIT_SEED: c_int = 42;
pub const RAID6_KUNIT_NUM_TEST_ITERS: c_int = 10;

pub const RAID6_KUNIT_MAX_FAILURES: c_int = 2;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_args {
    pub recov_idx: c_uint,
    pub recov: *const raid6_recov_calls,
    pub gen_idx: c_uint,
    pub gen: *const raid6_calls,
}
...
#[no_mangle]
unsafe extern "C" fn random_nr_buffers() -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn makedata(start: c_int, stop: c_int) {
...
#[no_mangle]
unsafe extern "C" fn member_type(nr_buffers: c_uint, d: c_int) -> c_char {
...
// data+data failure.
    ...
    unsigned int len)
    {
    let mut nr_data: c_uint = nr_buffers - 2;
    int iterations, i;
// Test P+Q recovery
...
// Test data+data recovery using random sampling
    ...
    let mut faila: c_int = rand32() % nr_data, failb;
    do {
...
// Simulate rmw run
    ...
    unsigned int len)
    {
    let mut iterations: c_int = nr_buffers / 2, i;
    for (i = 0; i < iterations; i++) {
    let mut p1: c_int = rand32() % (nr_buffers - 2);
    let mut p2: c_int = rand32() % (nr_buffers - 2);
    if (p2 < p1)
    swap(p1, p2);
...
#[no_mangle]
unsafe extern "C" fn raid6_test_one(test: *mut kunit) {
    static void raid6_test_one(struct kunit *test)
    {
    const struct test_args *ta = test.param_value;
    let mut nr_buffers: c_uint = random_nr_buffers();
    let mut len: c_uint = random_length(RAID6_KUNIT_MAX_BYTES);
...
// Go up to the guard page, to catch buffer overreads
    let mut align: c_uint = test_buflen - len;
    for (i = 0; i < nr_buffers; i++)
...
#[no_mangle]
unsafe extern "C" fn raid6_test(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn raid6_benchmark(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn raid6_suite_init(suite: *mut kunit_suite) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn raid6_suite_exit(suite: *mut kunit_suite) {
