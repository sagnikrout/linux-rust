...
//
#[no_mangle]
pub unsafe extern "C" fn polynomial_calc(poly: *const polynomial, data: c_long) -> c_long {
    long polynomial_calc(const struct polynomial *poly, long data)
    {
    const struct polynomial_term *term = poly.terms;
    let mut total_divider: c_long = poly.total_divider ?: 1;
    long tmp, ret = 0;
    int deg;
//
