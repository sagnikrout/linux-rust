...
#[no_mangle]
unsafe extern "C" fn is_aligned(base: *const c_void, size: usize, align: c_uchar) -> bool {
    static bool is_aligned(const void *base, size_t size, unsigned char align)
    {
    let mut lsbits: c_uchar = (unsigned char)size;
    (void)base;
...
//
#[no_mangle]
unsafe extern "C" fn swap_words_32(a: *mut c_void, b: *mut c_void, n: usize) {
    static void swap_words_32(void *a, void *b, size_t n)
    {
    do {
    let mut t: u32 = *(u32 *)(a + (n -= 4));
// (u32 *)(a + n) = *(u32 *)(b + n);
// (u32 *)(b + n) = t;
    } while (n);
    }
...
//
#[no_mangle]
unsafe extern "C" fn swap_words_64(a: *mut c_void, b: *mut c_void, n: usize) {
    static void swap_words_64(void *a, void *b, size_t n)
    {
    do {

    let mut t: u64 = *(u64 *)(a + (n -= 8));
// (u64 *)(a + n) = *(u64 *)(b + n);
// (u64 *)(b + n) = t;
...
//
#[no_mangle]
unsafe extern "C" fn swap_bytes(a: *mut c_void, b: *mut c_void, n: usize) {
    static void swap_bytes(void *a, void *b, size_t n)
    {
    do {
    let mut t: c_char = ((char *)a)[--n];
    ((char *)a)[n] = ((char *)b)[n];
    ((char *)b)[n] = t;
    } while (n);
    }
...
//
#[no_mangle]
unsafe extern "C" fn do_swap(a: *mut c_void, b: *mut c_void, size: usize, swap_func: swap_r_func_t, priv: *const c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn if(SWAP_WORDS_32: swap_func ==) -> else {
    else if (swap_func == SWAP_WORDS_32)
    swap_words_32(a, b, size);
#[no_mangle]
pub unsafe extern "C" fn if(SWAP_BYTES: swap_func ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn do_cmp(a: *const c_void, b: *const c_void, cmp: cmp_r_func_t, priv: *const c_void) -> c_int {
...
//
    __attribute_const__ __always_inline
#[no_mangle]
unsafe extern "C" fn parent(i: usize, lsbit: c_uint, size: usize) -> usize {
...
// pre-scale counters for performance
    let mut n: usize = num * size, a = (num/2) * size;
...
// called from 'sort' without swap function, let's pick the default
    ...
    swap_func = core::ptr::null_mut();
    if (!swap_func) {
    if (is_aligned(base, size, 8))
    swap_func = SWAP_WORDS_64;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_aligned(base, _arg: size, _arg: 4)) -> else {
