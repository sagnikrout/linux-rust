...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sg_splitter {
    pub in_sg0: *mut scatterlist,
    pub nents: c_int,
    pub skip_sg0: off_t,
    pub length_last_sg: c_uint,
    ...
    int i;
    unsigned int sglen;
    let mut size: usize = sizes[0], len;
    struct sg_splitter *curr = splitters;
    struct scatterlist *sg;
    for (i = 0; i < nb_splits; i++) {
    splitters[i].in_sg0 = core::ptr::null_mut();
    splitters[i].nents = 0;
    }
...
#[no_mangle]
unsafe extern "C" fn sg_split_phys(splitters: *mut sg_splitter, nb_splits: c_int) {
...
#[no_mangle]
unsafe extern "C" fn sg_split_mapped(splitters: *mut sg_splitter, nb_splits: c_int) {
