...
//! Source: lib/tests/checksum_kunit.c
#![no_std]
...
pub const MAX_LEN: c_int = 512;
pub const MAX_ALIGN: c_int = 64;

pub const IPv4_MIN_WORDS: c_int = 5;
pub const IPv4_MAX_WORDS: c_int = 15;
pub const NUM_IPv6_TESTS: c_int = 200;
pub const NUM_IP_FAST_CSUM_TESTS: c_int = 181;
// Values for a little endian CPU. Byte swap each half on big endian CPU.
    let mut random_init_sum: static u32 = 0x2847aab;
...
#[no_mangle]
unsafe extern "C" fn to_sum16(x: u16) -> __sum16 {
    static __sum16 to_sum16(u16 x)
    {
    return ( __sum16)le16_to_cpu(( __le16)x);
    }
// This function swaps the bytes inside each half of a __wsum
#[no_mangle]
unsafe extern "C" fn to_wsum(x: u32) -> __wsum {
    static __wsum to_wsum(u32 x)
    {
    let mut hi: u16 = le16_to_cpu(( __le16)(x >> 16));
    let mut lo: u16 = le16_to_cpu(( __le16)x);
    return ( __wsum)((hi << 16) | lo);
    }
#[no_mangle]
unsafe extern "C" fn assert_setup_correct(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn test_csum_fixed_random_inputs(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn test_csum_all_carry_inputs(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn test_csum_no_carry_inputs(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_ip_fast_csum(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_csum_ipv6_magic(test: *mut kunit) {
    ...
    return;
    let mut daddr_offset: c_int = sizeof(struct in6_addr);
    let mut len_offset: c_int = sizeof(struct in6_addr) + sizeof(struct in6_addr);
