...
//! Source: lib/tests/min_heap_kunit.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn less_than(lhs: *const c_void, rhs: *const c_void, args: *mut void __always_unused) -> bool {
    static bool less_than(const void *lhs, const void *rhs, void __always_unused *args)
    {
    return *(int *)lhs < *(int *)rhs;
    }
#[no_mangle]
unsafe extern "C" fn greater_than(lhs: *const c_void, rhs: *const c_void, args: *mut void __always_unused) -> bool {
...
#[no_mangle]
unsafe extern "C" fn test_heapify_all(test: *mut kunit) {
...
// Test with known set of values.
    min_heapify_all_inline(&heap, &funcs, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn test_heap_push(test: *mut kunit) {
...
// Test with known set of values copied from data.
    ...
    min_heap_push_inline(&heap, &data[i], &funcs, core::ptr::null_mut());
    pop_verify_heap(test, params.min_heap, &heap, &funcs);
// Test with randomly generated values.
    while (heap.nr < heap.size) {
    temp = get_random_u32();
    min_heap_push_inline(&heap, &temp, &funcs, core::ptr::null_mut());
    }
    pop_verify_heap(test, params.min_heap, &heap, &funcs);
    }
#[no_mangle]
unsafe extern "C" fn test_heap_pop_push(test: *mut kunit) {
...
// Test with known set of values copied from data.
    for (i = 0; i < ARRAY_SIZE(data); i++)
    min_heap_pop_push_inline(&heap, &data[i], &funcs, core::ptr::null_mut());
    pop_verify_heap(test, params.min_heap, &heap, &funcs);
    heap.nr = 0;
    for (i = 0; i < ARRAY_SIZE(data); i++)
    min_heap_push_inline(&heap, &temp, &funcs, core::ptr::null_mut());
// Test with randomly generated values.
    for (i = 0; i < ARRAY_SIZE(data); i++) {
    temp = get_random_u32();
    min_heap_pop_push_inline(&heap, &temp, &funcs, core::ptr::null_mut());
    }
    pop_verify_heap(test, params.min_heap, &heap, &funcs);
    }
#[no_mangle]
unsafe extern "C" fn test_heap_del(test: *mut kunit) {
...
// Test with known set of values.
    ...
    min_heap_del_inline(&heap, get_random_u32() % heap.nr, &funcs, core::ptr::null_mut());
    pop_verify_heap(test, params.min_heap, &heap, &funcs);
// Test with randomly generated values.
    heap.nr = ARRAY_SIZE(values);
    for (i = 0; i < heap.nr; i++)
