...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ffs_test_case {
    pub input: c_ulong,
    pub /: *mut *mut int expected_ffs; / ffs() result (1-based),
    pub /: *mut *mut int expected_fls; / fls() result (1-based),
    pub description: *const c_char,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ffs64_test_case {
    pub input: u64,
    pub /: *mut *mut int expected_fls64; / fls64() result (1-based),
    pub /: *mut *mut unsigned int expected_ffs64_0based; / __ffs64() result (0-based),
    pub description: *const c_char,
}
...
//
#[no_mangle]
unsafe extern "C" fn validate_ffs_relationships(test: *mut kunit, input: c_ulong) {
...
//
#[no_mangle]
unsafe extern "C" fn validate_ffs64_relationships(test: *mut kunit, input: u64) {
...
//
    ...
    let mut input_32: c_ulong = (unsigned long)input;
...
//
#[no_mangle]
unsafe extern "C" fn ffs_basic_correctness_test(test: *mut kunit) {
...
// Calculate expected __ffs() result: __ffs(x) == ffs(x) - 1
    let mut expected_ffs_0based: c_uint = tc.expected_ffs - 1;
...
// Calculate expected __fls() result: __fls(x) == fls(x) - 1
    let mut expected_fls_0based: c_uint = tc.expected_fls - 1;
...
//
#[no_mangle]
unsafe extern "C" fn ffs64_correctness_test(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn ffs_mathematical_relationships_test(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn ffs_edge_cases_test(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn ffs64_edge_cases_test(test: *mut kunit) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ffz_test_case {
    pub input: c_ulong,
    pub expected_ffz: c_ulong,
    pub description: *const c_char,
}
...
//
#[no_mangle]
unsafe extern "C" fn ffz_basic_correctness_test(test: *mut kunit) {
    ...
    let mut result: c_ulong = ffz(tc.input);
...
//
#[no_mangle]
unsafe extern "C" fn validate_ffz_relationships(test: *mut kunit, input: c_ulong) {
...
// Core relationship: if we set the ffz bit, ffz should find a different bit
    ...
    let mut modified: c_ulong = input | (1UL << ffz_result);
    if (modified != ~0UL) { /* Skip if all bits would be set */
    let mut new_ffz: c_ulong = ffz(modified);
...
#[no_mangle]
unsafe extern "C" fn ffz_mathematical_relationships_test(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn ffz_edge_cases_test(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn ffs_attribute_const_test(test: *mut kunit) {
