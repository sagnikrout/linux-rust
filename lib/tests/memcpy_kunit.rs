...
//! Source: lib/tests/memcpy_kunit.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn memcpy_test(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn memmove_test(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn memset_test(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn set_random_nonzero(test: *mut kunit, byte: *mut u8) {
    static void set_random_nonzero(struct kunit *test, u8 *byte)
    {
    let mut failed_rng: c_int = 0;
    while (*byte == 0) {
...
#[no_mangle]
unsafe extern "C" fn init_large(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn copy_large_test(test: *mut kunit, use_memmove: bool) {
...
// Over a shifting destination window ...
    ...
    let mut right_zero_pos: c_int = offset + bytes;
    let mut right_zero_size: c_int = ARRAY_SIZE(large_dst) - right_zero_pos;
// Copy!
    if (use_memmove)
...
#[no_mangle]
unsafe extern "C" fn memcpy_large_test(test: *mut kunit) {
    static void memcpy_large_test(struct kunit *test)
    {
    copy_large_test(test, false);
    }
#[no_mangle]
unsafe extern "C" fn memmove_large_test(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn next_step(idx: c_int, start: c_int, end: c_int, inc: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn inner_loop(test: *mut kunit, bytes: c_int, d_off: c_int, s_off: c_int) {
...
#[no_mangle]
unsafe extern "C" fn memmove_overlap_test(test: *mut kunit) {
...
//
    ...
    let mut bytes_step: static int = 7;
    let mut window_step: static int = 7;
    let mut bytes_start: static int = 1;
    let mut bytes_end: static int = ARRAY_SIZE(large_src) + 1;
    init_large(test);
...
// Over a shifting destination window ...
    ...
    let mut s_start: c_int = max(d_off - bytes, 0);
    let mut s_end: c_int = min_t(int, d_off + bytes, ARRAY_SIZE(large_src));
