...
//! Source: lib/tests/base64_kunit.c
#![no_std]
...
// ---------- Benchmark helpers ----------
    ...
    enum base64_variant variant)
    {
    let mut reps: c_int = 1000;
    let mut outlen: usize = DIV_ROUND_UP(size, 3) * 4;
    ...
    let mut enc_len: c_int = base64_encode(in, size, enc, true, variant);
    let mut dec_len: c_int = base64_decode(enc, enc_len, decoded, true, variant);
// correctness sanity check
    KUNIT_EXPECT_EQ(test, dec_len, size);
    KUNIT_EXPECT_MEMEQ(test, decoded, in, size);
// benchmark encode
    let mut t1: u64 = bench_encode_ns(in, size, enc, reps, variant);
    kunit_info(test, "[%s] encode run : %lluns", label, t1);
    let mut t2: u64 = bench_decode_ns(enc, enc_len, decoded, reps, variant);
...
#[no_mangle]
unsafe extern "C" fn base64_performance_tests(test: *mut kunit) {
...
// ---------- Helpers for encode ----------
    ...
    {
    char buf[128];
    let mut encoded_len: c_int = base64_encode(src, srclen, buf, padding, variant);
    buf[encoded_len] = '\0';
...
// ---------- Helpers for decode ----------
    ...
    {
    u8 buf[128];
    let mut decoded_len: c_int = base64_decode(src, strlen(src), buf, padding, variant);
    ...
    {
    u8 buf[64];
    let mut decoded_len: c_int = base64_decode(src, srclen, buf, padding, variant);
    KUNIT_EXPECT_EQ(test, decoded_len, -1);
    }
// ---------- Encode Tests ----------
#[no_mangle]
unsafe extern "C" fn base64_std_encode_tests(test: *mut kunit) {
...
// ---------- Decode Tests ----------
#[no_mangle]
unsafe extern "C" fn base64_std_decode_tests(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn base64_variant_tests(test: *mut kunit) {
    ...
    std_buf[i] = '-';
#[no_mangle]
pub unsafe extern "C" fn if('/': std_buf[i] ==) -> else {
    else if (std_buf[i] == '/')
    std_buf[i] = '_';
    }
