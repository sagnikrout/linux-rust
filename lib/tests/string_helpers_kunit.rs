...
//! Source: lib/tests/string_helpers_kunit.c
#![no_std]
...
pub struct test_string {
    ...
    let mut len: c_int = strlen(strings[i].in);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_string_1 {
    pub out: *const c_char,
    pub flags: c_uint,
}

pub const TEST_STRING_2_MAX_S1: c_int = 32;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_string_2 {
    pub in: *const c_char,
    pub s1: [test_string_1; TEST_STRING_2_MAX_S1],
}
...
// ESCAPE_OCTAL has a higher priority
    ...
    {
    int q_real;
    q_real = string_escape_mem(in, p, core::ptr::null_mut(), 0, flags, esc);
...
pub const string_get_size_maxbuf: c_int = 16;
...
#[no_mangle]
unsafe extern "C" fn __strchrcut(dst: *mut c_char, src: *const c_char, cut: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn test_get_size(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_upper_lower(test: *mut kunit) {
    ...
    let mut len: c_int = strlen(strings_upper[i].in) + 1;
    ...
    let mut len: c_int = strlen(strings_lower[i].in) + 1;
    dst = kmalloc(len, GFP_KERNEL);
    KUNIT_ASSERT_NOT_NULL(test, dst);
    string_lower(dst, s);
...
#[no_mangle]
unsafe extern "C" fn test_unescape(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_escape(test: *mut kunit) {
