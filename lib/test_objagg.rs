...
pub const NUM_KEYS: c_int = 32;
#[no_mangle]
unsafe extern "C" fn key_id_index(key_id: c_uint) -> c_int {
...
pub const BUF_LEN: c_int = 128;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct world {
    pub root_count: c_uint,
    pub delta_count: c_uint,
    pub next_root_buf: [c_char; BUF_LEN],
    pub objagg_objs: [*mut objagg_obj; NUM_KEYS],
    pub key_refs: [c_uint; NUM_KEYS],
}
...
pub struct root {
    pub key: tokey,
    pub buf: [c_char; BUF_LEN],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct delta {
    pub key_id_diff: c_uint,
}
...
pub const MAX_KEY_ID_DIFF: c_int = 5;
#[no_mangle]
unsafe extern "C" fn delta_check(priv: *mut c_void, parent_obj: *const c_void, obj: *const c_void) -> bool {
    ...
    let mut diff: c_int = key.id - parent_key.id;
...
#[no_mangle]
unsafe extern "C" fn delta_destroy(priv: *mut c_void, delta_priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn root_destroy(priv: *mut c_void, root_priv: *mut c_void) {
    ...
    {
    let mut orig_root_count: c_uint = world.root_count;
    struct objagg_obj *objagg_obj;
...
#[no_mangle]
unsafe extern "C" fn check_stats_zero(objagg: *mut objagg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn check_stats_nodelta(objagg: *mut objagg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn delta_destroy_dummy(priv: *mut c_void, delta_priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn test_nodelta() -> c_int {
    ...
    int i;
    int err;
    objagg = objagg_create(&nodelta_ops, core::ptr::null_mut(), &world);
    if (IS_ERR(objagg))
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct expect_stats {
    pub info_count: c_uint,
    pub info: [expect_stats_info; NUM_KEYS],
}
...
pub struct action_item {
    pub key_id: c_uint,
    pub action: enum action,
    pub expect_delta: enum expect_delta,
    pub expect_root: enum expect_root,
    pub expect_stats: expect_stats,
}
    ...
    {
    let mut key_id: c_uint = action_item.key_id;
...
#[no_mangle]
unsafe extern "C" fn obj_to_key_id(objagg_obj: *mut objagg_obj) -> c_uint {
...
// errmsg = "incorrect key id";
    ...
    &expect_stats.info[pos], core::ptr::null_mut());
    if (err)
    break;
...
// errmsg = "objagg_stats_get() failed.";
    ...
    bool inverse)
    {
    let mut orig_delta_count: c_uint = world.delta_count;
    let mut orig_root_count: c_uint = world.root_count;
    let mut key_id: c_uint = action_item.key_id;
    let mut action: enum action = action_item.action;
    struct objagg_obj *objagg_obj;
    const char *errmsg;
    int err;
    if (inverse)
...
#[no_mangle]
unsafe extern "C" fn test_delta() -> c_int {
    ...
    int i;
    int err;
    objagg = objagg_create(&delta_ops, core::ptr::null_mut(), &world);
    if (IS_ERR(objagg))
    return PTR_ERR(objagg);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hints_case {
    pub key_ids: *const c_uint,
    pub key_ids_count: usize,
    pub expect_stats: expect_stats,
    pub expect_stats_hints: expect_stats,
}
...
#[no_mangle]
unsafe extern "C" fn __pr_debug_stats(stats: *const objagg_stats) {
...
#[no_mangle]
unsafe extern "C" fn pr_debug_stats(objagg: *mut objagg) {
...
#[no_mangle]
unsafe extern "C" fn pr_debug_hints_stats(objagg_hints: *mut objagg_hints) {
...
#[no_mangle]
unsafe extern "C" fn test_hints_case(hints_case: *const hints_case) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_hints() -> c_int {
    static int test_hints(void)
    {
    return test_hints_case(&hints_case);
    }
#[no_mangle]
unsafe extern "C" fn test_objagg_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn test_objagg_exit() -> void __exit {
