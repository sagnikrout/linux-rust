...
//! Source: lib/lzo/lzo1x_compress.c
#![no_std]
...
pub const LZO_UNSAFE: c_int = 1;

pub const HAVE_OP(x): c_int = 1;
    ...
    unsigned char *op;
    let mut in_end: *const unsigned char  const = in + in_len;
    let mut ip_end: *const unsigned char  const = in + in_len - 20;
    const unsigned char *ii;
    let mut dict: *mut lzo_dict_t  const = (lzo_dict_t *) wrkmem;
    let mut ti: usize = *tp;
    op = *out;
    ip = in;
    ii = ip;
    ip += ti < 4 ? 4 - ti : 0;
    for (;;) {
    const unsigned char *m_pos = core::ptr::null_mut();
...
// tp = in_end - (ii - ti);
    ...
    {
    let mut op_end: *mut unsigned char  const = out + *out_len;
    const unsigned char *ip = in;
    unsigned char *op = out;
    unsigned char *data_start;
    let mut l: usize = in_len;
    let mut t: usize = 0;
    let mut state_offset: signed char = -2;
...
// op++ = bitstream_version;
    ...
    data_start = op;
    while (l > 20) {
    let mut ll: usize = min_t(size_t, l, m4_max_offset + 1);
    let mut ll_end: uintptr_t = (uintptr_t) ip + ll;
    int err;
