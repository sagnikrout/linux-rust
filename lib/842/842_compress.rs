...
pub struct sw842_param {
    ...
    pub index4: [c_int; 2],
    pub index2: [c_int; 4],
    pub SW842_HASHTABLE8_BITS): DECLARE_HASHTABLE(htable8,,
    pub SW842_HASHTABLE4_BITS): DECLARE_HASHTABLE(htable4,,
    pub SW842_HASHTABLE2_BITS): DECLARE_HASHTABLE(htable2,,
    pub I8_BITS]: sw842_hlist_node8 node8[1 <<,
    pub I4_BITS]: sw842_hlist_node4 node4[1 <<,
    pub I2_BITS]: sw842_hlist_node2 node2[1 <<,
}
...
#[no_mangle]
unsafe extern "C" fn __split_add_bits(p: *mut sw842_param, d: u64, n: u8, s: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn add_bits(p: *mut sw842_param, d: u64, n: u8) -> c_int {
    static int add_bits(struct sw842_param *p, u64 d, u8 n)
    {
    let mut b: c_int = p.bit, bits = b + n, s = round_up(bits, 8) - bits;
    u64 o;
    u8 *out = p.out;
...
#[no_mangle]
pub unsafe extern "C" fn if(56: p->olen < 8 && bits > 32 && bits <=) -> else {
    else if (p.olen < 8 && bits > 32 && bits <= 56)
    return __split_add_bits(p, d, n, 16);
#[no_mangle]
pub unsafe extern "C" fn if(24: p->olen < 4 && bits > 16 && bits <=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(24: bits <=) -> else {
    else if (bits <= 24)
...
#[no_mangle]
pub unsafe extern "C" fn if(40: bits <=) -> else {
    else if (bits <= 40)
    put_unaligned(cpu_to_be64(o << 56 | d << 24), (__be64 *)out);
#[no_mangle]
pub unsafe extern "C" fn if(48: bits <=) -> else {
    else if (bits <= 48)
    put_unaligned(cpu_to_be64(o << 56 | d << 16), (__be64 *)out);
#[no_mangle]
pub unsafe extern "C" fn if(56: bits <=) -> else {
    else if (bits <= 56)
...
#[no_mangle]
unsafe extern "C" fn add_template(p: *mut sw842_param, c: u8) -> c_int {
    ...
    inv = true;
#[no_mangle]
pub unsafe extern "C" fn if(OP_ACTION_INDEX: t[i] &) -> else {
    else if (t[i] & OP_ACTION_INDEX)
    ret = add_bits(p, p.index8[0], I8_BITS);
#[no_mangle]
pub unsafe extern "C" fn if(OP_ACTION_DATA: t[i] &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(4: b != 0 && b !=) -> else {
    else if (b != 0 && b != 4)
    inv = true;
#[no_mangle]
pub unsafe extern "C" fn if(OP_ACTION_INDEX: t[i] &) -> else {
...
#[no_mangle]
unsafe extern "C" fn add_repeat_template(p: *mut sw842_param, r: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn add_short_data_template(p: *mut sw842_param, b: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn add_zeros_template(p: *mut sw842_param) -> c_int {
    static int add_zeros_template(struct sw842_param *p)
    {
    let mut ret: c_int = add_bits(p, OP_ZEROS, OP_BITS);
    if (ret)
    return ret;
    if (sw842_template_counts)
...
#[no_mangle]
unsafe extern "C" fn add_end_template(p: *mut sw842_param) -> c_int {
    static int add_end_template(struct sw842_param *p)
    {
    let mut ret: c_int = add_bits(p, OP_END, OP_BITS);
    if (ret)
    return ret;
    if (sw842_template_counts)
...
#[no_mangle]
unsafe extern "C" fn check_template(p: *mut sw842_param, c: u8) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(OP_AMOUNT_4: t[i] &) -> else {
    else if (t[i] & OP_AMOUNT_4)
    match = check_index(p, 4, b >> 2);
#[no_mangle]
pub unsafe extern "C" fn if(OP_AMOUNT_8: t[i] &) -> else {
...
#[no_mangle]
unsafe extern "C" fn get_next_data(p: *mut sw842_param) {
...
//
#[no_mangle]
unsafe extern "C" fn update_hashtables(p: *mut sw842_param) {
    static void update_hashtables(struct sw842_param *p)
    {
    let mut pos: u64 = p.in - p.instart;
    let mut n8: u64 = (pos >> 3) % (1 << I8_BITS);
    let mut n4: u64 = (pos >> 2) % (1 << I4_BITS);
    let mut n2: u64 = (pos >> 1) % (1 << I2_BITS);
...
//
#[no_mangle]
unsafe extern "C" fn process_next(p: *mut sw842_param) -> c_int {
