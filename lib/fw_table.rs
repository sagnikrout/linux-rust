...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct acpi_subtable_entry {
    pub hdr: *mut union acpi_subtable_headers,
    pub type: enum acpi_subtable_type,
}
    ...
    case CDAT_SUBTABLE: {
    let mut length: __le16 = ( __le16)entry.hdr.cdat.length;
    ...
    let mut length: __le32 = ( __le32)header.cdat.length;
