...
#[derive(Copy, Clone)]
pub struct gf_poly {
    pub /: *mut *mut unsigned int deg; / polynomial degree,
    pub /: *mut *mut unsigned int c[]; / polynomial terms,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gf_poly_deg1 {
    pub poly: gf_poly,
    pub c: [c_uint; 2],
}

#[no_mangle]
unsafe extern "C" fn swap_bits(bch: *mut bch_control, in: u8) -> u8 {
...
//
    ...
    int i;
    const uint32_t *p;
    let mut l: c_int = BCH_ECC_WORDS(bch)-1;
    while (len--) {
    let mut tmp: u8 = swap_bits(bch, *data++);
...
//
    ...
    {
    let mut l: c_uint = BCH_ECC_WORDS(bch)-1;
    unsigned int i, mlen;
    unsigned long m;
    uint32_t w, r[BCH_ECC_MAX_WORDS];
    let mut r_bytes: usize = BCH_ECC_WORDS(bch) * sizeof(*r);
    let mut tab0: *const uint32_t  const = bch.mod8_tab;
    let mut tab1: *const uint32_t  const = tab0 + 256*(l+1);
    let mut tab2: *const uint32_t  const = tab1 + 256*(l+1);
    let mut tab3: *const uint32_t  const = tab2 + 256*(l+1);
...
#[no_mangle]
pub unsafe extern "C" fn modulo(bch: *mut bch_control, v: c_uint) -> c_int {
    static inline int modulo(struct bch_control *bch, unsigned int v)
    {
    let mut n: c_uint = GF_N(bch);
    while (v >= n) {
    v -= n;
    v = (v & n) + (v >> GF_M(bch));
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn mod_s(bch: *mut bch_control, v: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn deg(poly: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn parity(x: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn gf_sqr(bch: *mut bch_control, a: c_uint) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn gf_inv(bch: *mut bch_control, a: c_uint) -> c_uint {
    static inline unsigned int gf_inv(struct bch_control *bch, unsigned int a)
    {
    return bch.a_pow_tab[GF_N(bch)-bch.a_log_tab[a]];
    }
#[no_mangle]
pub unsafe extern "C" fn a_pow(bch: *mut bch_control, i: c_int) -> c_uint {
    static inline unsigned int a_pow(struct bch_control *bch, int i)
    {
    return bch.a_pow_tab[modulo(bch, i)];
    }
#[no_mangle]
pub unsafe extern "C" fn a_log(bch: *mut bch_control, x: c_uint) -> c_int {
    static inline int a_log(struct bch_control *bch, unsigned int x)
    {
    return bch.a_log_tab[x];
    }
#[no_mangle]
pub unsafe extern "C" fn a_ilog(bch: *mut bch_control, x: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gf_poly_copy(dst: *mut gf_poly, src: *mut gf_poly) {
...
//
    ...
    int i, j, k;
    let mut m: c_int = GF_M(bch);
    let mut mask: c_uint = 0xff, t, rows[16] = {0,};
    j = a_log(bch, b);
    k = a_log(bch, a);
    rows[0] = c;
...
// compute a mod b (modifies a)
    gf_poly_mod(bch, a, b, core::ptr::null_mut());
...
//
    ...
    let mut k: c_uint = 8*len+bch.ecc_bits;
...
//
    ...
    {
    let mut ecc_words: c_uint = BCH_ECC_WORDS(bch);
    unsigned int nbits;
    int i, err, nroots;
    uint32_t sum;
...
// compute received data ecc into an internal buffer
    if (!data || !recv_ecc)
    return -EINVAL;
    bch_encode(bch, data, len, core::ptr::null_mut());
    } else {
...
//
#[no_mangle]
unsafe extern "C" fn build_gf_tables(bch: *mut bch_control, poly: c_uint) -> c_int {
    static int build_gf_tables(struct bch_control *bch, unsigned int poly)
    {
    unsigned int i, x = 1;
    let mut k: c_uint = 1 << deg(poly);
...
//
#[no_mangle]
unsafe extern "C" fn build_mod8_tables(bch: *mut bch_control, g: *const u32) {
    static void build_mod8_tables(struct bch_control *bch, const uint32_t *g)
    {
    int i, j, b, d;
    uint32_t data, hi, lo, *tab;
    let mut l: c_int = BCH_ECC_WORDS(bch);
    let mut plen: c_int = DIV_ROUND_UP(bch.ecc_bits+1, 32);
    let mut ecclen: c_int = DIV_ROUND_UP(bch.ecc_bits, 32);
...
//
#[no_mangle]
unsafe extern "C" fn build_deg2_base(bch: *mut bch_control) -> c_int {
...
//
    ...
    uint32_t *genpoly;
    struct bch_control *bch = core::ptr::null_mut();
    let mut min_m: c_int = 5;
...
//
#[no_mangle]
pub unsafe extern "C" fn bch_free(bch: *mut bch_control) {
