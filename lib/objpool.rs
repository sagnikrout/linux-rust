...
// initialize elements of percpu objpool_slot
    ...
    for (i = 0; i < nodes; i++) {
    if (objinit) {
    let mut rc: c_int = objinit(obj, context);
    if (rc)
    return rc;
    }
...
#[no_mangle]
unsafe extern "C" fn objpool_fini_percpu_slots(pool: *mut objpool_head) {
...
#[no_mangle]
pub unsafe extern "C" fn objpool_drop(obj: *mut c_void, pool: *mut objpool_head) -> c_int {
