...
pub const DMIRROR_NDEVICES: c_int = 4;
pub const DMIRROR_RANGE_FAULT_TIMEOUT: c_int = 1000;

pub const DEVMEM_CHUNKS_RESERVE: c_int = 16;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dmirror_interval {
    pub notifier: mmu_interval_notifier,
    pub dmirror: *mut dmirror,
}
...
#[derive(Copy, Clone)]
pub struct dmirror {
    pub mdevice: *mut dmirror_device,
    pub pt: xarray,
    pub notifier: mmu_interval_notifier,
    pub mutex: mutex,
    pub flags: __u64,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dmirror_device {
    pub cdevice: cdev,
    pub zone_device_type: c_uint,
    pub device: device,
    pub devmem_capacity: c_uint,
    pub devmem_count: c_uint,
    pub devmem_chunks: *mut dmirror_chunk,
    pub /: *mut *mut mutex devmem_lock; / protects the above,
    pub calloc: c_ulong,
    pub cfree: c_ulong,
    pub free_pages: *mut page,
    pub free_folios: *mut folio,
    pub /: *mut *mut spinlock_t lock; / protects the above,
}
...
#[no_mangle]
unsafe extern "C" fn dmirror_is_private_zone(mdevice: *mut dmirror_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn dmirror_bounce_fini(bounce: *mut dmirror_bounce) {
    static void dmirror_bounce_fini(struct dmirror_bounce *bounce)
    {
    vfree(bounce.ptr);
    }
#[no_mangle]
unsafe extern "C" fn dmirror_fops_open(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmirror_device_evict_chunk(chunk: *mut dmirror_chunk) {
    static void dmirror_device_evict_chunk(struct dmirror_chunk *chunk)
    {
    let mut start_pfn: c_ulong = chunk.pagemap.range.start >> PAGE_SHIFT;
    let mut end_pfn: c_ulong = chunk.pagemap.range.end >> PAGE_SHIFT;
    let mut npages: c_ulong = end_pfn - start_pfn + 1;
    unsigned long i;
...
#[no_mangle]
unsafe extern "C" fn dmirror_fops_release(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmirror_do_fault(dmirror: *mut dmirror, range: *mut hmm_range) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(HMM_PFN_WRITE): WARN_ON(range->default_flags &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !mutex_trylock(&dmirror->mutex)) -> else {
...
#[no_mangle]
unsafe extern "C" fn dmirror_read(dmirror: *mut dmirror, cmd: *mut hmm_dmirror_cmd) -> c_int {
    static int dmirror_read(struct dmirror *dmirror, struct hmm_dmirror_cmd *cmd)
    {
    struct dmirror_bounce bounce;
    unsigned long start, end;
    let mut size: c_ulong = cmd.npages << PAGE_SHIFT;
    int ret;
    start = cmd.addr;
    end = start + size;
    if (end < start)
...
#[no_mangle]
unsafe extern "C" fn dmirror_write(dmirror: *mut dmirror, cmd: *mut hmm_dmirror_cmd) -> c_int {
...
// ppage = mdevice->free_pages;
    ...
    bool is_large)
    {
    struct page *dpage = core::ptr::null_mut();
    struct page *rpage = core::ptr::null_mut();
    let mut order: c_uint = is_large ? HPAGE_PMD_ORDER : 0;
    struct dmirror_device *mdevice = dmirror.mdevice;
//
...
//
    ...
    struct page *dpage;
    struct page *rpage;
    let mut is_large: bool = *src & MIGRATE_PFN_COMPOUND;
    let mut write: c_int = (*src & MIGRATE_PFN_WRITE) ? MIGRATE_PFN_WRITE : 0;
...
//
    ...
    struct folio *folio;
    unsigned long i;
    let mut spfn: c_ulong = *src >> MIGRATE_PFN_SHIFT;
...
// Map the migrated pages into the device's page tables.
    ...
    struct dmirror *dmirror)
    {
    let mut start: c_ulong = args.start;
    let mut end: c_ulong = args.end;
    const unsigned long *src = args.src;
    const unsigned long *dst = args.dst;
    unsigned long pfn;
    let mut start_pfn: c_ulong = start >> PAGE_SHIFT;
    let mut end_pfn: c_ulong = end >> PAGE_SHIFT;
// Map the migrated pages into the device's page tables.
    mutex_lock(&dmirror.mutex);
...
// Since the mm is for the mirrored process, get a reference first.
    ...
    struct folio *folio;
    struct page *page;
    page = make_device_exclusive(mm, addr, core::ptr::null_mut(), &folio);
    if (IS_ERR(page)) {
    ret = PTR_ERR(page);
    break;
    }
...
//
    ...
    dpage = core::ptr::null_mut(); /* For the next iteration */
    } else {
...
// Since the mm is for the mirrored process, get a reference first.
    ...
    unsigned long next;
    int ret;
    unsigned long *src_pfns = core::ptr::null_mut();
    unsigned long *dst_pfns = core::ptr::null_mut();
    start = cmd.addr;
    end = start + size;
    if (end < start)
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn if(PUD_SHIFT: hmm_pfn_to_map_order(entry) + PAGE_SHIFT ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn dmirror_remove_free_pages(devmem: *mut dmirror_chunk) {
...
#[no_mangle]
unsafe extern "C" fn dmirror_device_remove_chunks(mdevice: *mut dmirror_device) {
    ...
    mdevice.free_pages = core::ptr::null_mut();
    mdevice.free_folios = core::ptr::null_mut();
    kfree(mdevice.devmem_chunks);
    mdevice.devmem_chunks = core::ptr::null_mut();
    }
    mutex_unlock(&mdevice.devmem_lock);
    }
...
#[no_mangle]
unsafe extern "C" fn dmirror_fops_mmap(file: *mut file, vma: *mut vm_area_struct) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmirror_devmem_free(folio: *mut folio) {
    ...
    let mut order: c_uint = folio_order(rfolio);
    if (rpage != page) {
    if (order)
    __free_pages(rpage, order);
    else
    __free_page(rpage);
    rpage = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn dmirror_devmem_fault(vmf: *mut vm_fault) -> vm_fault_t {
...
#[no_mangle]
unsafe extern "C" fn dmirror_devmem_folio_split(head: *mut folio, tail: *mut folio) {
    ...
    if (!rpage) {
    tail.page.zone_device_data = core::ptr::null_mut();
    return;
    }
    rfolio = page_folio(rpage);
    if (tail == core::ptr::null_mut()) {
    folio_reset_order(rfolio);
    rfolio.mapping = core::ptr::null_mut();
    ...
    rpage_tail.mapping = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn dmirror_device_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn dmirror_device_init(mdevice: *mut dmirror_device, id: c_int) -> c_int {
...
// Build a list of free ZONE_DEVICE struct pages
    ret = dmirror_allocate_chunk(mdevice, core::ptr::null_mut(), false);
    if (ret)
    goto put_device;
...
#[no_mangle]
unsafe extern "C" fn dmirror_device_remove(mdevice: *mut dmirror_device) {
...
#[no_mangle]
unsafe extern "C" fn hmm_dmirror_init() -> int __init {
    static int __init hmm_dmirror_init(void)
    {
    int ret;
    let mut id: c_int = 0;
    let mut ndevices: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn hmm_dmirror_exit() -> void __exit {
