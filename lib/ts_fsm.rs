...
pub const _A: c_uint = 0x100 /* ascii */;
pub const _W: c_uint = 0x200 /* wildcard */;
...
#[no_mangle]
pub unsafe extern "C" fn match_token(t: *mut ts_fsm_token, d: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fsm_find(conf: *mut ts_config, state: *mut ts_state) -> c_uint {
    static unsigned int fsm_find(struct ts_config *conf, struct ts_state *state)
    {
    struct ts_fsm *fsm = ts_config_priv(conf);
    struct ts_fsm_token *cur = core::ptr::null_mut(), *next;
    unsigned int match_start, block_idx = 0, tok_idx;
    let mut block_len: unsigned = 0, strict, consumed = state.offset;
    const u8 *data;
...
//
    ...
    let mut ntokens: c_uint = len / sizeof(*tokens);
    let mut priv_size: usize = sizeof(*fsm) + len;
...
#[no_mangle]
unsafe extern "C" fn fsm_get_pattern_len(conf: *mut ts_config) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn init_fsm() -> int __init {
    static int __init init_fsm(void)
    {
    return textsearch_register(&fsm_ops);
    }
#[no_mangle]
unsafe extern "C" fn exit_fsm() -> void __exit {
