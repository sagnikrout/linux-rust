...
//
#[no_mangle]
pub unsafe extern "C" fn kfifo_unused(fifo: *mut __kfifo) -> c_uint {
...
pub unsafe extern "C" fn __kfifo_free(fifo: *mut __kfifo) {
    ...
    {
    let mut size: c_uint = fifo.mask + 1;
    let mut esize: c_uint = fifo.esize;
    unsigned int l;
    off &= fifo.mask;
    if (esize != 1) {
    off *= esize;
...
//
    ...
    {
    let mut size: c_uint = fifo.mask + 1;
    let mut off: c_uint = fifo.out & fifo.mask;
    if (tail)
// tail = off;
...
#[no_mangle]
pub unsafe extern "C" fn __kfifo_max_r(len: c_uint, recsize: usize) -> c_uint {
    unsigned int __kfifo_max_r(unsigned int len, size_t recsize)
    {
    let mut max: c_uint = (1 << (recsize << 3)) - 1;
    if (len > max)
    return max;
    return len;
    }
...
//
#[no_mangle]
unsafe extern "C" fn __kfifo_peek_n(fifo: *mut __kfifo, recsize: usize) -> c_uint {
    static unsigned int __kfifo_peek_n(struct __kfifo *fifo, size_t recsize)
    {
    unsigned int l;
    let mut mask: c_uint = fifo.mask;
...
//
#[no_mangle]
unsafe extern "C" fn __kfifo_poke_n(fifo: *mut __kfifo, n: c_uint, recsize: usize) {
