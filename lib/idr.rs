...
//
    ...
    void __rcu **slot;
    let mut base: c_uint = idr.idr_base;
    let mut id: c_uint = *nextid;
...
//
#[no_mangle]
pub unsafe extern "C" fn idr_alloc(idr: *mut idr, ptr: *mut c_void, start: c_int, end: c_int, gfp: gfp_t) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn idr_alloc_cyclic(idr: *mut idr, ptr: *mut c_void, start: c_int, end: c_int, gfp: gfp_t) -> c_int {
    int idr_alloc_cyclic(struct idr *idr, void *ptr, int start, int end, gfp_t gfp)
    {
    let mut id: u32 = idr.idr_next;
...
//
    ...
    {
    return radix_tree_delete_item(&idr.idr_rt, id - idr.idr_base, core::ptr::null_mut());
    }
...
//
    ...
    struct radix_tree_iter iter;
    void __rcu **slot;
    let mut base: c_int = idr.idr_base;
    radix_tree_for_each_slot(slot, &idr.idr_rt, &iter, 0) {
    int ret;
    let mut id: c_ulong = iter.index + base;
...
//
    ...
    let mut base: c_ulong = idr.idr_base;
    let mut id: c_ulong = *nextid;
    id = (id < base) ? 0 : id - base;
...
//
    ...
    void __rcu **slot = core::ptr::null_mut();
    void *entry;
    id -= idr.idr_base;
...
//
    ...
    let mut bit: unsigned = min % IDA_BITMAP_BITS;
    unsigned long flags;
    struct ida_bitmap *bitmap, *alloc = core::ptr::null_mut();
    if ((int)min < 0)
    return -ENOSPC;
    if ((int)max < 0)
    max = INT_MAX;
    ...
    let mut tmp: c_ulong = xa_to_value(bitmap);
...
//
#[no_mangle]
pub unsafe extern "C" fn ida_find_first_range(ida: *mut ida, min: c_uint, max: c_uint) -> c_int {
    int ida_find_first_range(struct ida *ida, unsigned int min, unsigned int max)
    {
    let mut index: c_ulong = min / IDA_BITMAP_BITS;
    let mut offset: c_uint = min % IDA_BITMAP_BITS;
...
//
#[no_mangle]
pub unsafe extern "C" fn ida_free(ida: *mut ida, id: c_uint) {
    void ida_free(struct ida *ida, unsigned int id)
    {
    XA_STATE(xas, &ida.xa, id / IDA_BITMAP_BITS);
    let mut bit: unsigned = id % IDA_BITMAP_BITS;
    ...
    bitmap = xas_load(&xas);
    if (xa_is_value(bitmap)) {
    let mut v: c_ulong = xa_to_value(bitmap);
    if (bit >= BITS_PER_XA_VALUE)
    goto err;
    if (!(v & (1UL << bit)))
    goto err;
...
#[no_mangle]
unsafe extern "C" fn ida_dump_entry(entry: *mut c_void, index: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn ida_dump(ida: *mut ida) {
