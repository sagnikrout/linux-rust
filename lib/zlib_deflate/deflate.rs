...
//! Source: lib/zlib_deflate/deflate.c
#![no_std]
...
pub const DEFLATE_NEED_CHECKSUM(strm): c_int = 1;
pub const DEFLATE_DFLTCC_ENABLED(): c_int = 0;
...
//
    if (strm == core::ptr::null_mut()) return Z_STREAM_ERROR;
...
// =========================================================================
    ...
    {
    deflate_state *s;
    if (strm == core::ptr::null_mut() || strm.state == core::ptr::null_mut())
    return Z_STREAM_ERROR;
...
// =========================================================================
    ...
    s = (deflate_state *) strm.state;
    if ((strm.next_in == core::ptr::null_mut() && strm.avail_in != 0) ||
...
// Write the zlib header
    ...
    let mut header: uInt = (Z_DEFLATED + ((s.w_bits-8)<<4)) << 8;
    let mut level_flags: uInt = (s.level-1) >> 1;
...
// =========================================================================
    ...
    int status;
    deflate_state *s;
    if (strm == core::ptr::null_mut() || strm.state == core::ptr::null_mut()) return Z_STREAM_ERROR;
    s = (deflate_state *) strm.state;
    status = s.status;
    ...
    let mut status: return = = BUSY_STATE ? Z_DATA_ERROR : Z_OK;
    }
// ===========================================================================
...
//
    ...
    {
    let mut len: unsigned = strm.avail_in;
    if (len > size) len = size;
    if (len == 0) return 0;
    strm.avail_in  -= len;
    if (!DEFLATE_NEED_CHECKSUM(strm)) {}
#[no_mangle]
pub unsafe extern "C" fn if()(strm->state))->noheader: *mut !((deflate_state) -> else {
...
//
    ...
    let mut scan_start: register ush = *(ush*)scan;
    let mut scan_end: register ush = *(ush*)(scan+best_len-1);

    register Byte *strend = s.window + s.strstart + MAX_MATCH;
    let mut scan_end1: register Byte = scan[best_len-1];
    let mut scan_end: register Byte = scan[best_len];
...
//
    let mut max_block_size: ulg = 0xffff;
    ulg max_start;
...
//
    ...
    let mut flush: return = = Z_FINISH ? finish_done : block_done;
    }
// ===========================================================================
...
//
    ...
    let mut max_insert: uInt = s.strstart + s.lookahead - MIN_MATCH;
...
#[no_mangle]
pub unsafe extern "C" fn zlib_deflate_workspacesize(windowBits: c_int, memLevel: c_int) -> c_int {
...
// Since the return value is typically passed to vmalloc() unchecked...
    ...
    windowBits > 15);
#[no_mangle]
pub unsafe extern "C" fn sizeof(_arg: deflate_workspace) -> return {
...
#[no_mangle]
pub unsafe extern "C" fn zlib_deflate_dfltcc_enabled() -> c_int {
