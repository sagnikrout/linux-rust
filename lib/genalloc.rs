...
#[no_mangle]
pub unsafe extern "C" fn chunk_size(chunk: *const gen_pool_chunk) -> usize {
    ...
    {
    let mut val: c_ulong = READ_ONCE(*addr);
    do {
    if (val & mask_to_set)
    return -EBUSY;
    cpu_relax();
...
//
    ...
    if (pool != core::ptr::null_mut()) {
    spin_lock_init(&pool.lock);
    INIT_LIST_HEAD(&pool.chunks);
    pool.min_alloc_order = min_alloc_order;
    pool.algo = gen_pool_first_fit;
    pool.data = core::ptr::null_mut();
    pool.name = core::ptr::null_mut();
    }
    return pool;
    }
...
//
    ...
    let mut nbits: c_ulong = size >> pool.min_alloc_order;
    unsigned long nbytes = sizeof(struct gen_pool_chunk) +
    BITS_TO_LONGS(nbits) * sizeof(long);
    chunk = vzalloc_node(nbytes, nid);
    if (unlikely(chunk == core::ptr::null_mut()))
    return -ENOMEM;
...
//
#[no_mangle]
pub unsafe extern "C" fn gen_pool_virt_to_phys(pool: *mut gen_pool, addr: c_ulong) -> phys_addr_t {
    phys_addr_t gen_pool_virt_to_phys(struct gen_pool *pool, unsigned long addr)
    {
    struct gen_pool_chunk *chunk;
    let mut paddr: phys_addr_t = -1;
    rcu_read_lock();
...
//
#[no_mangle]
pub unsafe extern "C" fn gen_pool_destroy(pool: *mut gen_pool) {
    void gen_pool_destroy(struct gen_pool *pool)
    {
    struct list_head *_chunk, *_next_chunk;
    struct gen_pool_chunk *chunk;
    let mut order: c_int = pool.min_alloc_order;
...
//
    ...
    {
    let mut data: genpool_data_align = { .align = align };
...
//
    ...
    let mut end: c_ulong = start + size - 1;
    struct gen_pool_chunk *chunk;
    rcu_read_lock();
    list_for_each_entry_rcu(chunk, &(pool).chunks, next_chunk) {
...
//
#[no_mangle]
pub unsafe extern "C" fn gen_pool_avail(pool: *mut gen_pool) -> usize {
...
//
#[no_mangle]
pub unsafe extern "C" fn gen_pool_size(pool: *mut gen_pool) -> usize {
...
//
#[no_mangle]
pub unsafe extern "C" fn gen_pool_set_algo(pool: *mut gen_pool, algo: genpool_algo_t, data: *mut c_void) {
...
//
    ...
    {
    let mut align_mask: c_ulong = roundup_pow_of_two(nr) - 1;
...
//
    ...
    struct gen_pool *pool, unsigned long start_addr)
    {
    let mut start_bit: c_ulong = size;
    let mut len: c_ulong = size + 1;
    unsigned long index;
    index = bitmap_find_next_zero_area(map, size, start, nr, 0);
    while (index < size) {
    let mut next_bit: c_ulong = find_next_bit(map, size, index + nr);
...
#[no_mangle]
unsafe extern "C" fn devm_gen_pool_release(dev: *mut device, res: *mut c_void) {
    static void devm_gen_pool_release(struct device *dev, void *res)
    {
    gen_pool_destroy(*(struct gen_pool **)res);
    }
#[no_mangle]
unsafe extern "C" fn devm_gen_pool_match(dev: *mut device, res: *mut c_void, data: *mut c_void) -> c_int {
...
//
    ...
    const char *pool_name = core::ptr::null_mut();
