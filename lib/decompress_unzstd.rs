...
//! Source: lib/decompress_unzstd.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn handle_zstd_error(ret: usize, x): *mut *mut void (error)(char) -> int INIT {
    static int INIT handle_zstd_error(size_t ret, void (*error)(char *x))
    {
    let mut err: zstd_error_code = zstd_get_error_code(ret);
...
//
    ...
    void (*error)(char *x))
    {
    let mut wksp_size: usize = zstd_dctx_workspace_bound();
    void *wksp = large_malloc(wksp_size);
    zstd_dctx *dctx = zstd_init_dctx(wksp, wksp_size);
    int err;
    size_t ret;
    if (dctx == core::ptr::null_mut()) {
...
// in_pos = in_len;
    ...
    void *in_allocated = core::ptr::null_mut();
    void *out_allocated = core::ptr::null_mut();
...
//
    ...
    if (fill == core::ptr::null_mut() && flush == core::ptr::null_mut())
//
...
//
    if (in_buf == core::ptr::null_mut()) {
    in_allocated = large_malloc(ZSTD_IOBUF_SIZE);
    if (in_allocated == core::ptr::null_mut()) {
...
// Allocate the output buffer if we are using flush().
    if (flush != core::ptr::null_mut()) {
    out_allocated = large_malloc(ZSTD_IOBUF_SIZE);
    if (out_allocated == core::ptr::null_mut()) {
...
// Flush all of the data produced if using flush().
    if (flush != core::ptr::null_mut() && out.pos > 0) {
