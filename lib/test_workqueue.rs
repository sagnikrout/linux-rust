...
//
    ...
    let mut wq_items: static int = 50000;
...
pub struct thread_ctx {
    pub work_done: completion,
    pub work: work_struct,
    pub latencies: *mut u64,
    pub cpu: c_int,
    pub items: c_int,
}

#[no_mangle]
unsafe extern "C" fn bench_work_fn(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn bench_kthread_fn(data: *mut c_void) -> c_int {
...
unsafe extern "C" fn cmp_u64(a: *const c_void, b: *const c_void) -> c_int {
    ...
    {
    let mut va: u64 = *(const u64 *)a;
    let mut vb: u64 = *(const u64 *)b;
    if (va < vb)
    return -1;
    if (va > vb)
    return 1;
...
#[no_mangle]
unsafe extern "C" fn set_affn_scope(scope: *const c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn run_bench(n_threads: c_int, scope: *const c_char, label: *const c_char) -> int __init {
...
// Merge all per-thread latencies and sort for percentile calculation
    ...
    sort(all_latencies, total_items, sizeof(u64), cmp_u64, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn test_workqueue_init() -> int __init {
    static int __init test_workqueue_init(void)
    {
    let mut n_threads: c_int = min(nr_threads ?: num_online_cpus(), num_online_cpus());
    int i;
    if (wq_items <= 0) {
