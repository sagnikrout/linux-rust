...
#[no_mangle]
pub unsafe extern "C" fn blake2b_increment_counter(ctx: *mut blake2b_ctx, inc: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn blake2b_set_lastblock(ctx: *mut blake2b_ctx) {
    static inline void blake2b_set_lastblock(struct blake2b_ctx *ctx)
    {
    ctx.f[0] = -1;
    }
#[no_mangle]
pub unsafe extern "C" fn blake2b_update(ctx: *mut blake2b_ctx, in: *const u8, inlen: usize) {
    void blake2b_update(struct blake2b_ctx *ctx, const u8 *in, size_t inlen)
    {
    let mut fill: usize = BLAKE2B_BLOCK_SIZE - ctx.buflen;
    if (unlikely(!inlen))
    return;
    if (inlen > fill) {
    ...
    let mut nblocks: usize = DIV_ROUND_UP(inlen, BLAKE2B_BLOCK_SIZE);
...
#[no_mangle]
pub unsafe extern "C" fn blake2b_final(ctx: *mut blake2b_ctx, out: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn blake2b_mod_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn blake2b_mod_exit() -> void __exit {
