...
#[no_mangle]
pub unsafe extern "C" fn blake2s_increment_counter(ctx: *mut blake2s_ctx, inc: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn blake2s_set_lastblock(ctx: *mut blake2s_ctx) {
    static inline void blake2s_set_lastblock(struct blake2s_ctx *ctx)
    {
    ctx.f[0] = -1;
    }
#[no_mangle]
pub unsafe extern "C" fn blake2s_update(ctx: *mut blake2s_ctx, in: *const u8, inlen: usize) {
    void blake2s_update(struct blake2s_ctx *ctx, const u8 *in, size_t inlen)
    {
    let mut fill: usize = BLAKE2S_BLOCK_SIZE - ctx.buflen;
    if (unlikely(!inlen))
    return;
    if (inlen > fill) {
    ...
    let mut nblocks: usize = DIV_ROUND_UP(inlen, BLAKE2S_BLOCK_SIZE);
...
#[no_mangle]
unsafe extern "C" fn blake2s_mod_init() -> int __init {
