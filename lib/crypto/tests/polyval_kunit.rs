...
//! Source: lib/crypto/tests/polyval_kunit.c
#![no_std]
...
unsafe extern "C" fn test_polyval_with_minimally_aligned_key(test: *mut kunit) {
    ...
    u8 raw_key[POLYVAL_BLOCK_SIZE];
    struct polyval_key key;
    let mut align: usize = __alignof__(struct polyval_key);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct polyval_irq_test_state {
    pub expected_key: polyval_key,
    pub raw_key: [u8; POLYVAL_BLOCK_SIZE],
}
...
#[no_mangle]
unsafe extern "C" fn polyval_suite_init(suite: *mut kunit_suite) -> c_int {
