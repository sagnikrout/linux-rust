...
//! Source: lib/crypto/tests/sha256_kunit.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn test_sha256_finup_2x(test: *mut kunit) {
    static void test_sha256_finup_2x(struct kunit *test)
    {
    let mut max_data_len: usize = 16384;
    ...
    let mut salt_len: usize = rand_length(sizeof(salt));
    let mut data_len: usize = rand_length(max_data_len);
...
#[no_mangle]
unsafe extern "C" fn test_sha256_finup_2x_defaultctx(test: *mut kunit) {
    static void test_sha256_finup_2x_defaultctx(struct kunit *test)
    {
    let mut data_len: usize = 128;
    ...
    hash2_a);
    sha256_finup_2x(core::ptr::null_mut(), data, &data[data_len], data_len, hash1_b,
    hash2_b);
...
//
#[no_mangle]
unsafe extern "C" fn test_sha256_finup_2x_hugelen(test: *mut kunit) {
    static void test_sha256_finup_2x_hugelen(struct kunit *test)
    {
    let mut data_len: usize = 4 * SHA256_BLOCK_SIZE;
...
#[no_mangle]
unsafe extern "C" fn benchmark_sha256_finup_2x(test: *mut kunit) {
...
//
    ...
    let mut data_len: usize = 4096;
    let mut num_iters: usize = 4096;
...
// Warm-up
    ...
    sha256_finup_2x(core::ptr::null_mut(), &data[0], &data[data_len], data_len,
    hash1, hash2);
    for (size_t i = 0; i < ARRAY_SIZE(salt_lens_to_test); i++) {
    let mut salt_len: usize = salt_lens_to_test[i];
    u64 t0, t1;
//
