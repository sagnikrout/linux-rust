...
//! Source: lib/crypto/tests/sha3_kunit.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn test_sha3_224_basic(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_sha3_256_basic(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_sha3_384_basic(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_sha3_512_basic(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_shake128_basic(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_shake256_basic(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_shake128_nist(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_shake256_nist(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn shake(alg: c_int, in: *const u8, in_len: usize, out: *mut u8, out_len: usize) {
...
#[no_mangle]
unsafe extern "C" fn shake_init(ctx: *mut shake_ctx, alg: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn test_shake_all_lens_up_to_4096(test: *mut kunit) {
    static void test_shake_all_lens_up_to_4096(struct kunit *test)
    {
    struct sha3_ctx main_ctx;
    let mut max_len: usize = 4096;
    let mut in: *mut u8 const = alloc_buf(test, max_len);
    let mut out: *mut u8 const = alloc_buf(test, max_len);
    ...
    let mut out_len: usize = (in_len * 293) % (max_len + 1);
...
//
#[no_mangle]
unsafe extern "C" fn test_shake_multiple_squeezes(test: *mut kunit) {
    static void test_shake_multiple_squeezes(struct kunit *test)
    {
    let mut max_len: usize = 512;
    u8 *buf = alloc_buf(test, max_len);
    u8 *ref_out = alloc_buf(test, max_len);
    for (int i = 0; i < 2000; i++) {
    let mut alg: c_int = rand32() % 2;
    let mut in_len: usize = rand_length(max_len);
    let mut out_len: usize = rand_length(max_len);
    let mut in_offs: usize = rand_offset(max_len - in_len);
    let mut out_offs: usize = rand_offset(max_len - out_len);
    let mut in: *mut u8 const = &buf[in_offs];
    let mut out: *mut u8 const = &buf[out_offs];
...
// Compute the output using a random sequence of squeezes.
    ...
    num_parts = 0;
    while (rand_bool()) {
    let mut part_len: usize = rand_length(remaining_len);
...
//
#[no_mangle]
unsafe extern "C" fn test_shake_with_guarded_bufs(test: *mut kunit) {
