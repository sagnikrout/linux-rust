...
#[no_mangle]
pub unsafe extern "C" fn __sha512_update(ctx: *mut __sha512_ctx, data: *const u8, len: usize) {
    void __sha512_update(struct __sha512_ctx *ctx, const u8 *data, size_t len)
    {
    let mut partial: usize = ctx.bytecount_lo % SHA512_BLOCK_SIZE;
    if (check_add_overflow(ctx.bytecount_lo, len, &ctx.bytecount_lo))
    ctx.bytecount_hi++;
    if (partial + len >= SHA512_BLOCK_SIZE) {
    size_t nblocks;
    if (partial) {
    let mut l: usize = SHA512_BLOCK_SIZE - partial;
    ...
    {
    let mut bitcount_hi: u64 = (ctx.bytecount_hi << 3) | (ctx.bytecount_lo >> 61);
    let mut bitcount_lo: u64 = ctx.bytecount_lo << 3;
    let mut partial: usize = ctx.bytecount_lo % SHA512_BLOCK_SIZE;
...
#[no_mangle]
pub unsafe extern "C" fn sha384_final(ctx: *mut sha384_ctx, out[SHA384_DIGEST_SIZE]: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn sha512_final(ctx: *mut sha512_ctx, out[SHA512_DIGEST_SIZE]: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn sha384(data: *const u8, len: usize, out[SHA384_DIGEST_SIZE]: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn sha512(data: *const u8, len: usize, out[SHA512_DIGEST_SIZE]: u8) {
...
#[no_mangle]
unsafe extern "C" fn sha512_mod_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn sha512_mod_exit() -> void __exit {
