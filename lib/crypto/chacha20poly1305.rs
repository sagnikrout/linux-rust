...
//! Source: lib/crypto/chacha20poly1305.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn chacha_load_key(k: *mut u32, in: *const u8) {
...
// Compute the subkey given the original key and first 128 nonce bits
    ...
    u8 *addr = miter.addr;
    let mut length: usize = min_t(size_t, sl, miter.length);
    if (!encrypt)
    poly1305_update(&poly1305_state, addr, length);
    if (unlikely(partial)) {
    let mut l: usize = min(length, CHACHA_BLOCK_SIZE - partial);
