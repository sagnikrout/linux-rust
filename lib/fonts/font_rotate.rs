...
//! Source: lib/fonts/font_rotate.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn font_glyph_bit_pitch(width: c_uint) -> c_uint {
    ...
    unsigned int *bit)
    {
    let mut off: c_uint = y * bit_pitch + x;
    let mut bit_shift: c_uint = off % 8;
// bit = 0x80 >> bit_shift; /* MSB has position 0, LSB has position 7
    ...
    {
    unsigned int bit;
    let mut i: c_uint = __font_glyph_pos(x, y, bit_pitch, &bit);
    return glyph[i] & bit;
    }
    ...
    {
    unsigned int x, y;
    let mut shift: c_uint = (8 - (height % 8)) & 7;
    let mut bit_pitch: c_uint = font_glyph_bit_pitch(width);
    let mut out_bit_pitch: c_uint = font_glyph_bit_pitch(height);
    for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
...
//
    ...
    {
    unsigned int x, y;
    let mut shift: c_uint = (8 - (width % 8)) & 7;
    let mut bit_pitch: c_uint = font_glyph_bit_pitch(width);
    for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
    if (font_glyph_test_bit(glyph, x, y, bit_pitch)) {
...
//
    ...
    let mut s_cellsize: c_uint = font_glyph_size(width, height);
