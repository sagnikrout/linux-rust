...
pub unsafe extern "C" fn strlcat(dest: *mut c_char, src: *const c_char, count: usize) -> usize {
    ...
    {
    let mut dsize: usize = strlen(dest);
    let mut len: usize = strlen(src);
...
//
    ...
    {
    const char *last = core::ptr::null_mut();
    do {
    if (*s == (char)c)
    last = s;
    } while (*s++);
...
#[no_mangle]
pub unsafe extern "C" fn memcmp(cs: *const c_void, ct: *const c_void, count: usize) -> __visible int {
