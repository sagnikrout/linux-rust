...
#[no_mangle]
unsafe extern "C" fn bm_find(conf: *mut ts_config, state: *mut ts_state) -> c_uint {
    ...
    const u8 *text;
    int bs;
    let mut icase: u8 = conf.flags & TS_IGNORECASE;
    for (;;) {
    let mut shift: c_int = bm.patlen - 1;
...
#[no_mangle]
unsafe extern "C" fn subpattern(pattern: *mut u8, i: c_int, j: c_int, g: c_int) -> c_int {
    static int subpattern(u8 *pattern, int i, int j, int g)
    {
    let mut x: c_int = i+g-1, y = j+g-1, ret = 0;
...
#[no_mangle]
unsafe extern "C" fn compute_prefix_tbl(bm: *mut ts_bm, flags: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bm_get_pattern_len(conf: *mut ts_config) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn init_bm() -> int __init {
    static int __init init_bm(void)
    {
    return textsearch_register(&bm_ops);
    }
#[no_mangle]
unsafe extern "C" fn exit_bm() -> void __exit {
