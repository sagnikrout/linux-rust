...
pub static mut dynamic_scs_enabled: usize = 0;

#[no_mangle]
unsafe extern "C" fn __scs_account(s: *mut c_void, account: c_int) {
    let mut scs_page = vmalloc_to_page(s);
...
#[no_mangle]
pub unsafe extern "C" fn __scs_alloc(node: c_int) -> *mut c_void {
    let mut i = 0;
...
#[no_mangle]
pub unsafe extern "C" fn scs_alloc(node: c_int) -> *mut c_void {
pub static mut s: *mut c_void = core::ptr::null_mut();
    s = __scs_alloc(node);
    if (!s) {
...
unsafe extern "C" fn scs_cleanup(cpu: c_uint) -> c_int {
    let mut i = 0;
    let mut cache = per_cpu_ptr(scs_cache, cpu);
    while (i < NR_CACHED_SCS) {
    vfree(cache[i]);
