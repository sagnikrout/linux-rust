...
pub struct trace_uprobe {
    ...
    pub nhits: *mut unsigned long __percpu,
    pub tp: trace_probe,
}
...
unsafe extern "C" fn get_user_stack_nth(regs: *mut pt_regs, n: c_uint) -> c_ulong {
    ...
    {
    unsigned long ret;
    let mut addr: c_ulong = user_stack_pointer(regs);
    addr = adjust_stack_addr(addr, n);
    if (copy_from_user(&ret, (void  __user *) addr, sizeof(ret)))
    return 0;
    return ret;
    }
//
...
//
    ...
    {
    void __user *vaddr = (void  __user *)src;
...
//
    ...
    {
    long ret;
    let mut loc: u32 = *(u32 *)dest;
    let mut maxlen: c_int = get_loc_len(loc);
    u8 *dst = get_loc_data(dest, base);
    void __user *src = (void  __user *) addr;
    if (unlikely(!maxlen))
    return -ENOMEM;
...
// Return the length of string -- including null terminal byte
    ...
    {
    int len;
    void __user *vaddr = (void  __user *) addr;
    if (addr == FETCH_TOKEN_COMM)
...
unsafe extern "C" fn __trace_uprobe_create(argc: c_int, argv: *const c_char) -> c_int {
    ...
    const char *event = core::ptr::null_mut(), *group = UPROBE_EVENT_SYSTEM;
...
unsafe extern "C" fn trace_uprobe_show(m: *mut seq_file, ev: *mut dyn_event) -> c_int {
    ...
    {
    struct trace_uprobe *tu = to_trace_uprobe(ev);
    let mut c: c_char = is_ret_probe(tu) ? 'r' : 'p';
    int i;
...
// probe_addr = tu->ref_ctr_offset;
    ...
    struct trace_uprobe *tu;
    struct uprobe_dispatch_data udd;
    struct uprobe_cpu_buffer *ucb = core::ptr::null_mut();
    unsigned int flags;
    let mut ret: c_int = 0;
