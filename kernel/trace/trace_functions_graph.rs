...
pub struct fgraph_data {
    pub cpu_data: *mut fgraph_cpu_data __percpu,
...
//
    ...
    let mut retaddr: c_ulong = ftrace_graph_top_ret_addr(current);
    ...
    {
    let mut time: u64 = trace_clock_local();
...
//
    ...
    struct ring_buffer_iter *ring_iter = core::ptr::null_mut();
    struct ring_buffer_event *event;
...
// if the output fails.
//
    let mut size: c_int = min_t(int, sizeof(data.rent), iter.ent_size);
...
// No overhead
    ...
    {
    let mut nsecs_rem: c_ulong = do_div(duration, 1000);
// log10(ULONG_MAX) + '\0'
    char usecs_str[21];
    char nsecs_str[5];
    int len;
...
// Print nsecs (we don't want to exceed 7 numbers)
    if (len < 7) {
    let mut slen: usize = min_t(size_t, sizeof(nsecs_str), 8UL - len);
...
pub const __TRACE_GRAPH_PRINT_RETADDR: c_int = 0;
    ...
    {
    let mut err_code: c_ulong = 0;
    let mut retval: c_ulong = 0;
    let mut print_retaddr: bool = false;
    let mut print_retval: bool = false;
    let mut hex_format: bool = !!(opt_flags & TRACE_GRAPH_PRINT_RETVAL_HEX);
...
//
    ...
    let mut calltime: u64 = retentry.calltime;
    let mut rettime: u64 = retentry.rettime;
    let mut duration: c_ulonglong = rettime - calltime;
    struct fgraph_data *data = iter.private;
    struct trace_array *tr = iter.tr;
    unsigned long func;
    let mut pid: pid_t = ent.pid;
    let mut cpu: c_int = iter.cpu;
    let mut func_match: c_int = 1;
    int i;
...
//
    ...
    print_graph_retval(s, core::ptr::null_mut(), trace, (void *)func, flags,
    tr.trace_flags, 0);
    } else {
//
...
// Overrun
    ...
    {
    struct trace_array *tr = iter.tr;
    let mut sym_flags: c_ulong = (tr.trace_flags & TRACE_ITER_SYM_MASK);
    struct fgraph_data *data = iter.private;
...
unsafe extern "C" fn print_lat_header(s: *mut seq_file, flags: u32) {
    ...
    {
    let mut lat: c_int = tr.trace_flags & TRACE_ITER(LATENCY_FMT);
    if (lat)
