...
unsafe extern "C" fn trace_get_entry_size(call: *mut trace_event_call) -> c_int {
    ...
    {
    let mut entry_size: c_int = trace_get_entry_size(call);
    struct ftrace_event_field *field;
    struct list_head *head;
...
// We need an extra '\0' at the end.
    ...
    u32 *str_item;
    let mut str_loc: c_int = entry_size & 0xffff;
...
// pentry = entry;
    ...
    field.filter_type == FILTER_RDYN_STRING) {
    let mut str_len: c_int = strlen(addr) + 1;
    let mut str_loc: c_int = entry_size & 0xffff;
    u32 *str_item;
    entry_size += str_len;
...
// paddr = INJECT_STRING;
    ...
    switch (field.size) {
    case 1: {
    let mut tmp: u8 = (u8) val;
    memcpy(entry + field.offset, &tmp, 1);
    break;
    }
    case 2: {
    let mut tmp: u16 = (u16) val;
    memcpy(entry + field.offset, &tmp, 2);
    break;
    }
    case 4: {
    let mut tmp: u32 = (u32) val;
