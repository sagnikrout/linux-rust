...
#[no_mangle]
unsafe extern "C" fn set_kprobe_boot_events(str: *mut c_char) -> int __init {
...
pub struct trace_kprobe {
    ...
    pub nhit: *mut unsigned long __percpu,
...
unsafe extern "C" fn trace_kprobe_has_gone(tk: *mut trace_kprobe) -> nokprobe_inline bool {
    ...
    struct module *mod)
    {
    let mut len: c_int = strlen(module_name(mod));
...
unsafe extern "C" fn within_notrace_func(tk: *mut trace_kprobe) -> bool {
    ...
    {
    let mut addr: c_ulong = trace_kprobe_address(tk);
...
unsafe extern "C" fn number_of_same_symbols(mod: *const c_char, func_name: *const c_char) -> c_uint {
    ...
    {
    let mut ctx: sym_count_ctx = { .count = 0, .name = func_name };
    if (!mod)
...
unsafe extern "C" fn validate_module_probe_symbol(modname: *const c_char, symbol: *const c_char) -> c_int {
    static int validate_module_probe_symbol(const char *modname, const char *symbol)
    {
    let mut count: c_uint = number_of_same_symbols(modname, symbol);
    if (count > 1) {
//
...
unsafe extern "C" fn validate_probe_symbol(symbol: *mut c_char) -> c_int {
    ...
    char *modname = core::ptr::null_mut(), *p;
    let mut ret: c_int = 0;
    p = strchr(symbol, ':');
    if (p) {
    modname = symbol;
...
//
    struct trace_kprobe *tk __free(free_trace_kprobe) = core::ptr::null_mut();
    const char *event = core::ptr::null_mut(), *group = KPROBE_EVENT_SYSTEM;
...
pub unsafe extern "C" fn if(-ENOENT: ret ==) -> else {
    ...
    trace_probe_log_err(0, BAD_PROBE_ADDR);
#[no_mangle]
pub unsafe extern "C" fn if(-EEXIST: ret != -ENOMEM && ret !=) -> else {
...
unsafe extern "C" fn create_or_delete_trace_kprobe(raw_command: *const c_char) -> c_int {
    ...
    let mut ret: return = = -ECANCELED ? -EINVAL : ret;
    }
#[no_mangle]
...
unsafe extern "C" fn trace_kprobe_release(ev: *mut dyn_event) -> c_int {
    ...
    {
    struct trace_kprobe *tk = to_trace_kprobe(ev);
    let mut ret: c_int = unregister_trace_kprobe(tk);
    if (!ret)
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tk->rp.kp.offset) -> else {
    else if (tk.rp.kp.offset)
...
unsafe extern "C" fn kprobe_dispatcher(kp: *mut kprobe, regs: *mut pt_regs) -> c_int {
    ...
    let mut flags: c_uint = trace_probe_load_flag(&tk.tp);
    let mut ret: c_int = 0;
    raw_cpu_inc(*tk.nhit);
