...
// Used to optimize division by constants
    ...
    void *event)
    {
    let mut str_item: u32 = *(u32 *)(event + hist_field.field.offset);
    let mut str_loc: c_int = str_item & 0xffff;
    ...
    let mut val: u64 = hist_fn_call(operand, elt, buffer, rbe, event);
    ...
    {
    struct hist_field *operand = hist_field.operands[0];
    let mut buckets: c_ulong = hist_field.buckets;
    let mut val: u64 = hist_fn_call(operand, elt, buffer, rbe, event);
    ...
    let mut val1: u64 = hist_fn_call(operand1, elt, buffer, rbe, event);
    let mut val2: u64 = hist_fn_call(operand2, elt, buffer, rbe, event);
    return val1 + val2;
    }
...
//
    ...
    let mut mult: u64 = operand2.div_multiplier;
    ...
    {
    struct hist_field *operand = hist_field.operands[0];
    let mut sval: i64 = (s64)hist_fn_call(operand, elt, buffer, rbe, event);
    let mut val: u64 = (u64)-sval;
    return val;
    }
...
pub struct action_data {
    pub handler: enum handler_id,
    pub action: enum action_id,
...
unsafe extern "C" fn hist_field_get_div_fn(divisor: *mut hist_field) -> enum hist_field_fn {
    ...
    {
    let mut div: u64 = divisor.constant;
    if (!(div & (div - 1)))
...
unsafe extern "C" fn last_cmd_set(file: *mut trace_event_file, str: *mut c_char) {
    ...
    {
    const char *system = core::ptr::null_mut(), *name = core::ptr::null_mut();
...
pub struct hist_var_data {
    ...
    let mut ts: u64 = ring_buffer_event_time_stamp(buffer, rbe);
...
//
    ...
    {
    struct trace_array *tr = hist_data.event_file.tr;
    struct hist_field *found = core::ptr::null_mut();
...
unsafe extern "C" fn remove_hist_vars(hist_data: *mut hist_trigger_data) {
    ...
    {
    struct trace_array *tr = hist_data.event_file.tr;
    struct hist_field *hist_field = core::ptr::null_mut();
    struct trace_event_file *file;
...
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_CPU: field->flags &) -> else {
    else if (field.flags & HIST_FIELD_FL_CPU)
    field_name = "common_cpu";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_COMM: field->flags &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_STACKTRACE: field->flags &) -> else {
...
unsafe extern "C" fn parse_action(str: *mut c_char, attrs: *mut hist_trigger_attrs) -> c_int {
    ...
    let mut map_bits: c_int = parse_map_size(str + len);
    if (map_bits < 0) {
    ret = map_bits;
    goto out;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str, 0: "pause") ==) -> else {
    ...
    attrs.cont = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str, 0: "clear") ==) -> else {
...
unsafe extern "C" fn hist_trigger_elt_data_alloc(elt: *mut tracing_map_elt) -> c_int {
    ...
    let mut size: c_uint = TASK_COMM_LEN;
    struct hist_elt_data *elt_data;
    struct hist_field *hist_field;
...
unsafe extern "C" fn hist_trigger_elt_data_init(elt: *mut tracing_map_elt) {
    ...
    {
    const char *flags_str = core::ptr::null_mut();
    if (hist_field.flags & HIST_FIELD_FL_HEX)
    flags_str = "hex";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_SYM: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_SYM)
    flags_str = "sym";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_SYM_OFFSET: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_SYM_OFFSET)
    flags_str = "sym-offset";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_EXECNAME: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_EXECNAME)
    flags_str = "execname";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_SYSCALL: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_SYSCALL)
    flags_str = "syscall";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_LOG2: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_LOG2)
    flags_str = "log2";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_BUCKET: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_BUCKET)
    flags_str = "buckets";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_TIMESTAMP_USECS: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_TIMESTAMP_USECS)
    flags_str = "usecs";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_PERCENT: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_PERCENT)
    flags_str = "percent";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_GRAPH: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_GRAPH)
    flags_str = "graph";
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_STACKTRACE: hist_field->flags &) -> else {
...
unsafe extern "C" fn contains_operator(str: *mut c_char, sep: *mut c_char) -> c_int {
    ...
    {
    let mut field_op: enum field_op_id = FIELD_OP_NONE;
...
// Can likely be a const
    ...
    let mut fl: c_ulong = flags & ~(HIST_FIELD_FL_LOG2 | HIST_FIELD_FL_BUCKET);
...
//
    ...
    char *system, char *event_name)
    {
    let mut flags: c_ulong = HIST_FIELD_FL_VAR_REF;
    struct hist_field *ref_field;
    int i;
...
unsafe extern "C" fn is_var_ref(var_name: *mut c_char) -> bool {
    ...
    char *var_name)
    {
    struct hist_field *var_field = core::ptr::null_mut(), *ref_field = core::ptr::null_mut();
    struct trace_array *tr = hist_data.event_file.tr;
    if (!is_var_ref(var_name))
    return core::ptr::null_mut();
    var_name++;
    ...
    {
    struct ftrace_event_field *field = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(modifier, 0: "sym") ==) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(modifier, 0: "symXoffset") ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(modifier, 0: "syscall") ==) -> else {
    else if (strcmp(modifier, "syscall") == 0)
// flags |= HIST_FIELD_FL_SYSCALL;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(modifier, 0: "stacktrace") ==) -> else {
    else if (strcmp(modifier, "stacktrace") == 0)
// flags |= HIST_FIELD_FL_STACKTRACE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(modifier, 0: "log2") ==) -> else {
    else if (strcmp(modifier, "log2") == 0)
// flags |= HIST_FIELD_FL_LOG2;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(modifier, 0: "usecs") ==) -> else {
    else if (strcmp(modifier, "usecs") == 0)
// flags |= HIST_FIELD_FL_TIMESTAMP_USECS;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(modifier, _arg: "bucket", 0: 6) ==) -> else {
    int ret;
    modifier += 6;
    if (*modifier == 's')
    modifier++;
...
// flags |= HIST_FIELD_FL_COMM | HIST_FIELD_FL_STRING;
    ...
    char *var_name)
    {
    struct hist_field *alias = core::ptr::null_mut();
    let mut flags: c_ulong = HIST_FIELD_FL_ALIAS | HIST_FIELD_FL_VAR;
    ...
    {
    struct trace_array *tr = hist_data.event_file.tr;
    struct hist_field *field = core::ptr::null_mut();
    u64 constant;
...
//
    ...
    {
    let mut operand1_flags: c_ulong = operand1.flags;
    let mut operand2_flags: c_ulong = operand2.flags;
...
// var2 = var;
    ...
    char *var_name, unsigned int *n_subexprs)
    {
    struct hist_field *operand1 = core::ptr::null_mut(), *operand2 = core::ptr::null_mut(), *expr = core::ptr::null_mut();
    struct hist_field *var1 = core::ptr::null_mut(), *var2 = core::ptr::null_mut();
...
// The operands are now owned and free'd by 'expr'
    ...
    let mut divisor: u64 = var2 ? var2.constant : operand2.constant;
    if (!divisor) {
...
// ((unsigned long *)str) = e;
    ...
    char *field_name)
    {
    struct hist_field *val = core::ptr::null_mut(), *var = core::ptr::null_mut();
    let mut flags: c_ulong = HIST_FIELD_FL_VAR;
    struct trace_array *tr = file.tr;
...
unsafe extern "C" fn check_track_val_changed(track_val: u64, var_val: u64) -> bool {
    ...
    {
    let mut track_var_idx: c_uint = data.track_data.track_var.var.idx;
    u64 track_val;
...
unsafe extern "C" fn cond_snapshot_update(tr: *mut trace_array, cond_data: *mut c_void) -> bool {
    ...
    struct action_data *data)
    {
    let mut track_val: u64 = get_track_val(hist_data, elt, data);
    unsigned int i, save_var_idx;
    if (data.handler == HANDLER_ONMAX)
    seq_printf(m, "\n\tmax: %10llu", track_val);
#[no_mangle]
pub unsafe extern "C" fn if(HANDLER_ONCHANGE: data->handler ==) -> else {
    ...
    struct action_data *data, u64 *var_ref_vals)
    {
    let mut var_val: u64 = var_ref_vals[data.track_data.var_ref.var_ref_idx];
...
#[no_mangle]
pub unsafe extern "C" fn if(HANDLER_ONCHANGE: handler ==) -> else {
...
unsafe extern "C" fn create_hitcount_val(hist_data: *mut hist_trigger_data) -> c_int {
    ...
    {
    struct hist_field *hist_field;
    let mut ret: c_int = 0, n_subexprs = 0;
...
// There is only raw hitcount but nohitcount suppresses it.
    ...
    let mut ret: c_int = 0;
    let mut n_vars: c_uint = hist_data.attrs.var_defs.n_vars;
    for (i = 0; i < n_vars; i++) {
...
unsafe extern "C" fn parse_actions(hist_data: *mut hist_trigger_data) -> c_int {
    ...
    let mut hid: enum handler_id = 0;
    char *action_str;
    str = hist_data.attrs.action_str[i];
    if ((len = str_has_prefix(str, "onmatch(")))
    hid = HANDLER_ONMATCH;
#[no_mangle]
pub unsafe extern "C" fn if(str_has_prefix(str: (len =, _arg: "onmax("))) -> else {
    else if ((len = str_has_prefix(str, "onmax(")))
    hid = HANDLER_ONMAX;
#[no_mangle]
pub unsafe extern "C" fn if(str_has_prefix(str: (len =, _arg: "onchange("))) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_CPU: !field || hist_field->flags &) -> else {
    else if (!field || hist_field.flags & HIST_FIELD_FL_CPU)
    cmp_fn = tracing_map_cmp_num(hist_field.size,
    hist_field.is_signed);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_string_field(field)) -> else {
    ...
    bool remove)
    {
    const struct tracing_map_ops *map_ops = core::ptr::null_mut();
    struct hist_trigger_data *hist_data;
    let mut ret: c_int = 0;
...
// ((unsigned long *)str) = e;
    ...
    {
    let mut size: usize = key_field.size;
...
#[no_mangle]
pub unsafe extern "C" fn if(FILTER_STATIC_STRING: field->filter_type ==) -> else {
...
unsafe extern "C" fn put_hist_pad() {
    ...
    let mut use_compound_key: bool = (hist_data.n_keys > 1);
    struct tracing_map_elt *elt = core::ptr::null_mut();
    struct hist_field *key_field;
    struct hist_pad *hist_pad;
    u64 field_contents;
    void *key = core::ptr::null_mut();
    unsigned int i;
    if (unlikely(!rbe))
    return;
...
// (u64 *)(key + key_field->offset));
    ...
    let mut buckets: c_ulong = key_field.buckets;
    uval = *(u64 *)(key + key_field.offset);
...
pub unsafe extern "C" fn __get_percentage(val: u64, total: u64) -> c_uint {
    ...
    {
    let mut len: c_uint = __get_percentage(val, max);
    int i;
    if (len == UINT_MAX) {
...
pub struct hist_val_stat {
    ...
    struct tracing_map_elt *elt)
    {
    let mut val: u64 = tracing_map_read_sum(elt, idx);
    unsigned int pc;
    char bar[21];
...
// At first, show the raw hitcount if !nohitcount
    ...
    struct hist_trigger_data *hist_data)
    {
    struct tracing_map_sort_entry **sort_entries = core::ptr::null_mut();
    struct tracing_map *map = hist_data.map;
    int i, j, n_entries;
    struct hist_val_stat *stats = core::ptr::null_mut();
    u64 val;
...
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_HITCOUNT: flags &) -> else {
    else if (flags & HIST_FIELD_FL_HITCOUNT)
    seq_puts(m, "        VAL: HIST_FIELD_FL_HITCOUNT\n");
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_VAR: flags &) -> else {
    else if (flags & HIST_FIELD_FL_VAR)
    seq_puts(m, "        HIST_FIELD_FL_VAR\n");
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_VAR_REF: flags &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_CONST: flags &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(HIST_FIELD_FL_CONST: hist_field->flags &) -> else {
    else if (hist_field.flags & HIST_FIELD_FL_CONST)
    seq_printf(m, "%llu", hist_field.constant);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: field_name) -> else {
    if (hist_field.flags & HIST_FIELD_FL_VAR_REF ||
    ...
    struct hist_trigger_data *hist_data = data.private_data;
    struct hist_field *field;
    let mut have_var: bool = false;
    let mut show_val: bool = false;
    unsigned int i;
...
unsafe extern "C" fn hist_clear(data: *mut event_trigger_data) {
    ...
    test.paused = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: hist_data->attrs->cont) -> else {
    else if (hist_data.attrs.cont)
    test.paused = false;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: hist_data->attrs->clear) -> else {
...
// cmd_ops = *data->cmd_ops;
    ...
    struct trace_event_file *file)
    {
    struct event_trigger_data *test = core::ptr::null_mut(), *iter, *named_data = core::ptr::null_mut();
    struct hist_trigger_data *hist_data = data.private_data;
    lockdep_assert_held(&event_mutex);
    if (hist_data.attrs.name)
    named_data = find_named_trigger(hist_data.attrs.name);
...
unsafe extern "C" fn hist_unreg_all(file: *mut trace_event_file) {
    ...
    char *param_and_filter)
    {
    let mut hist_trigger_bits: c_uint = TRACING_MAP_BITS_DEFAULT;
