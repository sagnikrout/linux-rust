...
pub struct filter_pred {
    ...
    pub val: u64,
    pub val2: u64,
    pub fn_num: enum filter_pred_fn,
    pub offset: c_int,
    pub not: c_int,
    pub op: c_int,
}
...
//
    ...
    const char *ptr = str;
    char *inverts = core::ptr::null_mut();
    int *op_stack;
    int *top;
...
// Second Pass
    for (i = N-1 ; i--; ) {
    let mut target: c_int = prog[i].target;
...
unsafe extern "C" fn filter_pred_strloc(pred: *mut filter_pred, event: *mut c_void) -> c_int {
    ...
    {
    let mut str_item: u32 = *(u32 *)(event + pred.offset);
    let mut str_loc: c_int = str_item & 0xffff;
    let mut str_len: c_int = str_item >> 16;
...
unsafe extern "C" fn filter_pred_cpumask(pred: *mut filter_pred, event: *mut c_void) -> c_int {
    ...
    {
    let mut item: u32 = *(u32 *)(event + pred.offset);
    let mut loc: c_int = item & 0xffff;
...
unsafe extern "C" fn filter_pred_function(pred: *mut filter_pred, event: *mut c_void) -> c_int {
    ...
    let mut start: c_ulong = (unsigned long)pred.val;
    let mut end: c_ulong = (unsigned long)pred.val2;
    let mut ret: c_int = *addr >= start && *addr < end;
...
unsafe extern "C" fn filter_build_regex(pred: *mut filter_pred) {
    ...
    char *search;
    let mut type: enum regex_type = MATCH_FULL;
    if (pred.op == OP_GLOB) {
...
// Protected by either SRCU(tracepoint_srcu) or preempt_disable
    ...
    let mut match: c_int = filter_pred_fn_call(pred, rec);
...
unsafe extern "C" fn remove_filter_string(filter: *mut event_filter) {
    ...
    {
    struct trace_seq *s;
    let mut pos: c_int = pe.lasterr_pos;
    char *buf;
    int len;
...
unsafe extern "C" fn filter_disable(file: *mut trace_event_file) {
    ...
    {
    let mut old_flags: c_ulong = file.flags;
...
pub unsafe extern "C" fn filter_assign_type(type: *const c_char) -> c_int {
    ...
    {
    let mut fn: enum filter_pred_fn = FILTER_PRED_FN_NOP;
    let mut pred_func_index: c_int = -1;
    switch (op) {
    case OP_EQ:
    case OP_NE:
    break;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: field_is_signed) -> else {
...
// Called when a predicate is encountered by predicate_parse()
    ...
    struct filter_pred *pred = core::ptr::null_mut();
    ...
    char *field_name;
    char *name;
    let mut function: bool = false;
    let mut ustring: bool = false;
    char q;
    u64 val;
    int len;
    int ret;
...
unsafe extern "C" fn calc_stack(str: *const c_char, parens: *mut c_int, preds: *mut c_int, err: *mut c_int) -> c_int {
    ...
    {
    let mut is_pred: bool = false;
    let mut nr_preds: c_int = 0;
    int open = 1; /* Count the expression as "(E)" */
    let mut last_quote: c_int = 0;
    let mut max_open: c_int = 1;
...
// If any call succeeded, we still need to sync
    ...
    {
    struct event_filter *filter;
    struct filter_parse_error *pe = core::ptr::null_mut();
    let mut err: c_int = 0;
