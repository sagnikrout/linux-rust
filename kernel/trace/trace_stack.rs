...
unsafe extern "C" fn check_stack(ip: c_ulong, stack: *mut c_ulong) {
    ...
    let mut frame_size: c_int = READ_ONCE(tracer_frame);
    int i, x;
...
unsafe extern "C" fn trace_lookup_stack(m: *mut seq_file, i: c_long) {
    ...
    {
    let mut addr: c_ulong = stack_dump_trace[i];
    seq_printf(m, "%pS\n", (void *)addr);
    }
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn init_trace_stack_sysctls() -> int __init {
