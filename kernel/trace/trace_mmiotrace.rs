...
unsafe extern "C" fn count_overruns(iter: *mut trace_iterator) -> c_ulong {
    ...
    {
    let mut cnt: c_ulong = atomic_xchg(&dropped_count, 0);
    let mut over: c_ulong = ring_buffer_overruns(iter.array_buffer.buffer);
...
unsafe extern "C" fn mmio_print_rw(iter: *mut trace_iterator) -> enum print_line_t {
    ...
    struct trace_seq *s	= &iter.seq;
    let mut t: c_ulonglong = ns2usecs(iter.ts);
    let mut usec_rem: c_ulong = do_div(t, USEC_PER_SEC);
    let mut secs: unsigned = (unsigned long)t;
