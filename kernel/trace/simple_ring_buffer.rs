...
unsafe extern "C" fn simple_bpage_set_head_link(bpage: *mut simple_buffer_page) {
    ...
    {
    let mut link: c_ulong = (unsigned long)bpage.link.next;
...
//
    ...
    let mut old: c_ulong = (*link & SIMPLE_RB_LINK_MASK) | SIMPLE_RB_LINK_HEAD;
    let mut new: c_ulong = (unsigned long)(&dst.link) | new_type;
...
unsafe extern "C" fn simple_bpage_set_normal_link(bpage: *mut simple_buffer_page) {
    ...
    {
    let mut ptr: c_ulong = (unsigned long)link & SIMPLE_RB_LINK_MASK;
...
unsafe extern "C" fn simple_rb_find_head(cpu_buffer: *mut simple_rb_per_cpu) -> c_int {
    ...
    {
    let mut retry: c_int = cpu_buffer.nr_pages * 2;
...
unsafe extern "C" fn rb_event_size(length: c_ulong) -> c_ulong {
    ...
    {
    let mut ts_ext_size: c_ulong = 0, event_size = rb_event_size(length);
