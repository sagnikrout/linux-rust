...
// function exit handler
    ...
    {
    struct trace_fprobe *tf = container_of(fp, struct trace_fprobe, fp);
    let mut flags: c_uint = trace_probe_load_flag(&tf.tp);
    let mut ret: c_int = 0;
...
// Before this notification, tracepoint notifier has already done.
    ...
    let mut ip: c_ulong = tracepoint_user_ip(tf.tuser);
...
//
    struct trace_fprobe *tf __free(free_trace_fprobe) = core::ptr::null_mut();
    const char *event = core::ptr::null_mut(), *group = FPROBE_EVENT_SYSTEM;
    struct module *mod __free(module_put) = core::ptr::null_mut();
    ...
    let mut is_tracepoint: bool = false;
...
unsafe extern "C" fn trace_fprobe_release(ev: *mut dyn_event) -> c_int {
    ...
    {
    struct trace_fprobe *tf = to_trace_fprobe(ev);
    let mut ret: c_int = unregister_trace_fprobe(tf);
    if (!ret)
