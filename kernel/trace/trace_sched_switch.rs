...
//
    let mut trace_cmdline_lock: static arch_spinlock_t = __ARCH_SPIN_LOCK_UNLOCKED;
#[repr(C)]
#[derive(Copy, Clone)]
...
unsafe extern "C" fn free_saved_cmdlines_buffer(s: *mut saved_cmdlines_buffer) {
    ...
    {
    let mut order: c_int = get_order(sizeof(*s) + s.cmdline_num * TASK_COMM_LEN);
    kmemleak_free(s);
...
unsafe extern "C" fn saved_tgids_show(m: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    {
    int *entry = (int *)v;
    let mut pid: c_int = entry - tgid_map;
    let mut tgid: c_int = *entry;
    if (tgid == 0)
    return SEQ_SKIP;
    seq_printf(m, "%d %d\n", pid, tgid);
    return 0;
