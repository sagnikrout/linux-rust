...
unsafe extern "C" fn fprobe_node_hashfn(data: *const c_void, len: u32, seed: u32) -> u32 {
    ...
    const void *ptr)
    {
    let mut key: c_ulong = *(unsigned long *)arg.key;
...
// fgraph_ops callback, this processes fprobes which have exit_handler.
    ...
    struct ftrace_regs *fregs)
    {
    unsigned long *fgraph_data = core::ptr::null_mut();
    let mut func: c_ulong = trace.func;
    struct fprobe_hlist_node *node;
    struct rhlist_head *head, *pos;
...
// If entry_handler returns !0, nmissed is not counted but skips exit_handler.
    ...
    let mut size_words: c_int = SIZE_IN_LONG(data_size);
...
// Handle module unloading to manage fprobe_ip_table.
    ...
    {
    let mut alist: fprobe_addr_list = {.size = FPROBE_IPS_BATCH_INIT};
    struct fprobe_hlist_node *node;
...
#[no_mangle]
unsafe extern "C" fn init_fprobe_module() -> int __init {
...
pub unsafe extern "C" fn unregister_fprobe(fp: *mut fprobe) -> c_int {
    ...
    {
    let mut ret: c_int = unregister_fprobe_async(fp);
    if (!ret)
    synchronize_rcu();
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn fprobe_initcall() -> int __init {
