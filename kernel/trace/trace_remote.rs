...
// trace_remote_iter(struct trace_remote *remote, int cpu, enum tri_type type)
    {
    struct trace_remote_iterator *iter = core::ptr::null_mut();
    int ret;
    lockdep_assert_held(&remote.lock);
...
unsafe extern "C" fn trace_pipe_open(inode: *mut inode, filp: *mut file) -> c_int {
    ...
    struct trace_remote_iterator *iter;
    let mut cpu: c_int = tracing_get_cpu(inode);
    guard(mutex)(&remote.lock);
...
#[no_mangle]
unsafe extern "C" fn trace_pipe_read(filp: *mut file, ubuf: *mut char __user, cnt: usize, ppos: *mut loff_t) -> isize {
    ...
    let mut prev_len: c_int = iter.seq.seq.len;
...
#[no_mangle]
unsafe extern "C" fn trace_write(filp: *mut file, ubuf: *const char __user, cnt: usize, ppos: *mut loff_t) -> isize {
...
unsafe extern "C" fn trace_remote_init_tracefs(name: *const c_char, remote: *mut trace_remote) -> c_int {
    ...
    let mut root_inited: bool = false;
    int cpu;
    guard(mutex)(&lock);
    if (!root) {
...
//
    ...
    {
    let mut min_desc_size: usize = trace_buffer_desc_size(buffer_size, cpumask_weight(cpumask));
    let mut nr_pages: c_uint = max(DIV_ROUND_UP(buffer_size, PAGE_SIZE), 2UL) + 1;
    struct ring_buffer_desc *rb_desc;
    int cpu, ret = -ENOMEM;
...
unsafe extern "C" fn remote_event_format_show(s: *mut seq_file, unused: *mut c_void) -> c_int {
    ...
    {
    let mut offset: usize = sizeof(struct remote_event_hdr);
    struct remote_event *evt = s.private;
...
// fops = &remote_events_dir_header_event_fops;
    ...
    let mut eventfs_create: bool = false;
    if (!eventfs) {
