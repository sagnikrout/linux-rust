...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !iter->second_pass) -> else {
    else if (!iter.second_pass)
    print_cpu(m, iter.cpu, iter.now);

#[no_mangle]
pub unsafe extern "C" fn if(iter->second_pass: iter->cpu == -1 &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn init_timer_list_procfs() -> int __init {
    static int __init init_timer_list_procfs(void)
    {
    struct proc_dir_entry *pe;
    pe = proc_create_seq_private("timer_list", 0400, core::ptr::null_mut(), &timer_list_sops,
    sizeof(struct timer_list_iter), core::ptr::null_mut());
    if (!pe)
    return -ENOMEM;
    return 0;
    }
