...
pub struct swsusp_extent {
    ...
    let mut swsusp_extents: static struct rb_root = RB_ROOT;
#[no_mangle]
...
// General things
//
    let mut root_swap: static unsigned short = 0xffff;
...
unsafe extern "C" fn hib_end_io(bio: *mut bio) {
    ...
    put_page(page);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: clean_pages_on_read) -> else {
...
unsafe extern "C" fn write_page(buf: *mut c_void, offset: sector_t, hb: *mut hib_bio_batch) -> c_int {
    ...
    {
    let mut gfp: gfp_t = GFP_NOIO | __GFP_NOWARN | __GFP_NORETRY;
    void *src;
    int ret;
    if (!offset)
    return -ENOSPC;
...
#[no_mangle]
pub unsafe extern "C" fn if(handle->cur_swap: handle->cur &&) -> else {
...
pub const CMP_THREADS: c_int = 3;
    let mut hibernate_compression_threads: static unsigned int = CMP_THREADS;
...
// Indicates the image size after compression
    let mut compressed_size: static atomic64_t = ATOMIC_INIT(0);
#[no_mangle]
unsafe extern "C" fn compress_threadfn(data: *mut c_void) -> c_int {
    ...
    unsigned char *page = core::ptr::null_mut();
    struct cmp_data *data = core::ptr::null_mut();
    struct crc_data *crc = core::ptr::null_mut();
    hib_init_batch(&hb);
...
unsafe extern "C" fn enough_swap(nr_pages: c_uint) -> c_int {
    ...
    {
    let mut free_swap: c_uint = count_swap_pages(root_swap, 1);
...
unsafe extern "C" fn decompress_threadfn(data: *mut c_void) -> c_int {
    ...
    let mut read_pages: c_ulong = 0;
    unsigned char **page = core::ptr::null_mut();
    struct dec_data *data = core::ptr::null_mut();
    struct crc_data *crc = core::ptr::null_mut();
    hib_init_batch(&hb);
//
...
pub unsafe extern "C" fn swsusp_check(exclusive: bool) -> c_int {
    ...
    {
    void *holder = exclusive ? &swsusp_holder : core::ptr::null_mut();
    int error;
    hib_resume_bdev_file = bdev_file_open_by_dev(swsusp_resume_device,
...
#[no_mangle]
unsafe extern "C" fn swsusp_header_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn hibernate_compression_threads_setup(str: *mut c_char) -> int __init {
    static int __init hibernate_compression_threads_setup(char *str)
    {
    let mut rc: c_int = kstrtouint(str, 0, &hibernate_compression_threads);
    if (rc)
    return rc;
