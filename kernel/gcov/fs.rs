...
// If non-zero, keep copies of profiling data for unloaded modules.
    let mut gcov_persist: static int = 1;
#[no_mangle]
unsafe extern "C" fn gcov_persist_setup(str: *mut c_char) -> int __init {
...
//
    ...
    reset_node(node);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: list_empty(&node->children)) -> else {
    remove_node(node);
...
unsafe extern "C" fn add_info(node: *mut gcov_node, info: *mut gcov_info) {
    ...
    let mut num: c_int = node.num_loaded;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn gcov_event(action: enum gcov_action, info: *mut gcov_info) {
