...
// If retrieved is set, it means this session is from incoming list
    ...
    let mut err: c_int = luo_session_finish_one(session);
    if (err) {
...
pub struct luo_ioctl_op {
    ...
    pub type: enum luo_ioctl_type,
    pub ucmd): *mut *mut *mut int (execute)(struct luo_session session, struct luo_ucmd,
}
...
pub unsafe extern "C" fn luo_session_create(name: *const c_char, filep: *mut file) -> c_int {
    ...
    {
    let mut len: usize = strnlen(name, LIVEUPDATE_SESSION_NAME_LENGTH);
    struct luo_session *session;
    int err;
...
pub unsafe extern "C" fn luo_session_retrieve(name: *const c_char, filep: *mut file) -> c_int {
    ...
    struct luo_session *session = core::ptr::null_mut();
    struct luo_session *it;
    int err;
    guard(rwsem_read)(&luo_session_serialize_rwsem);
...
#[no_mangle]
pub unsafe extern "C" fn luo_session_setup_outgoing(sessions_pa: *mut u64) -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn luo_session_setup_incoming(sessions_pa: u64) -> int __init {
