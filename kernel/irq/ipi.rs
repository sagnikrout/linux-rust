...
//
    ...
    (void *) dest, true, core::ptr::null_mut());
    if (virq <= 0) {
...
// use the correct data for that cpu
    let mut irq: unsigned = data.irq + cpu - data.common.ipi_offset;
...
//
    ...
    if (irq_domain_is_ipi_per_cpu(data.domain)) {
    let mut base: c_uint = data.irq;
    for_each_cpu(cpu, dest) {
    let mut irq: unsigned = base + cpu - data.common.ipi_offset;
    data = irq_get_irq_data(irq);
    chip.ipi_send_single(data, cpu);
    }
    } else {
...
pub unsafe extern "C" fn ipi_send_single(virq: c_uint, cpu: c_uint) -> c_int {
    ...
    struct irq_data *data = desc ? irq_desc_get_irq_data(desc) : core::ptr::null_mut();
    struct irq_chip *chip = data ? irq_data_get_irq_chip(data) : core::ptr::null_mut();
    if (WARN_ON_ONCE(ipi_send_verify(chip, data, core::ptr::null_mut(), cpu)))
    return -EINVAL;
