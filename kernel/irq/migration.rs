...
pub unsafe extern "C" fn irq_force_complete_move(desc: *mut irq_desc) {
    for (irq_data *d = irq_desc_get_irq_data(desc); d; d = irqd_get_parent_data(d)) {
...
pub unsafe extern "C" fn irq_move_masked_irq(idata: *mut irq_data) {
    let mut desc = irq_data_to_desc(idata);
    let mut data = &desc.irq_data;
    let mut chip = data.chip;
