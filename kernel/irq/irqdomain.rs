...
#[no_mangle]
unsafe extern "C" fn alloc_name(domain: *mut irq_domain, base: *mut c_char, bus_token: enum irq_domain_bus_token) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn alloc_unknown_name(domain: *mut irq_domain, bus_token: enum irq_domain_bus_token) -> c_int {
    static int alloc_unknown_name(struct irq_domain *domain, enum irq_domain_bus_token bus_token)
    {
    static atomic_t unknown_domains;
    let mut id: c_int = atomic_inc_return(&unknown_domains);
    if (bus_token == DOMAIN_BUS_ANY)
...
unsafe extern "C" fn irq_domain_set_name(domain: *mut irq_domain, info: *const irq_domain_info) -> c_int {
    ...
    {
    let mut bus_token: enum irq_domain_bus_token = info.bus_token;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: h->ops->match) -> else {
    else if (h.ops.match)
...
//
    ...
    unsigned int *irq)
    {
    struct irq_desc *desc = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn irq_domain_debugfs_init(root: *mut dentry) -> void __init {
