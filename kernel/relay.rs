...
//
    ...
    {
    let mut length: c_ulong = vma_desc_size(desc);
    if (!buf)
    return -EBADF;
...
//
    ...
    void *prev_subbuf)
    {
    let mut full: c_int = relay_buf_full(buf);
    if (full)
...
#[no_mangle]
pub unsafe extern "C" fn if(RELAY_STATS_WRT_BIG: flags &) -> else {
...
unsafe extern "C" fn relay_file_read_start_pos(buf: *mut rchan_buf) -> usize {
    ...
    let mut subbuf_size: usize = buf.chan.subbuf_size;
    let mut n_subbufs: usize = buf.chan.n_subbufs;
    let mut consumed: usize = buf.subbufs_consumed % n_subbufs;
