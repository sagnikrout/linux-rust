...
unsafe extern "C" fn kdb_parsebp(argc: c_int, argv: *const c_char, nextargp: *mut c_int, bp: *mut kdb_bp_t) -> c_int {
    ...
    {
    let mut nextarg: c_int = *nextargp;
    int diag;
    bp.bph_length = 1;
    if ((argc + 1) != nextarg) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncasecmp(argv[nextarg], _arg: "dataw", 0: sizeof("dataw")) ==) -> else {
    else if (strncasecmp(argv[nextarg], "dataw", sizeof("dataw")) == 0)
    bp.bp_type = BP_WRITE_WATCHPOINT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncasecmp(argv[nextarg], _arg: "inst", 0: sizeof("inst")) ==) -> else {
...
unsafe extern "C" fn kdb_bp(argc: c_int, argv: *const c_char) -> c_int {
    ...
    int diag;
    char *symname = core::ptr::null_mut();
    let mut offset: c_long = 0ul;
    int nextarg;
    let mut template: kdb_bp_t = {0};
    if (argc == 0) {
//
...
unsafe extern "C" fn kdb_bc(argc: c_int, argv: *const c_char) -> c_int {
    ...
    {
    unsigned long addr;
    kdb_bp_t *bp = core::ptr::null_mut();
    let mut lowbp: c_int = KDB_MAXBPT;
    let mut highbp: c_int = 0;
    let mut done: c_int = 0;
    int i;
    let mut diag: c_int = 0;
    int cmd;			/* KDBCMD_B? */
...
pub const KDBCMD_BD: c_int = 2;
    ...
    cmd = KDBCMD_BE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[0], 0: "bd") ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn kdb_initbptab() -> void __init {
