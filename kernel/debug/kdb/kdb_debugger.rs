...
pub unsafe extern "C" fn kdb_stub(ks: *mut kgdb_state) -> c_int {
    ...
    kdb_bp_t *bp;
    let mut addr: c_ulong = kgdb_arch_pc(ks.ex_vector, ks.linux_regs);
    let mut reason: kdb_reason_t = KDB_REASON_OOPS;
    let mut db_result: kdb_dbtrap_t = KDB_DB_NOBPT;
    int i;
    kdb_ks = ks;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: in_nmi()) -> else {
    else if (in_nmi())
