...
#[no_mangle]
pub unsafe extern "C" fn if(this_cpu_ksoftirqd(): (pc & SOFTIRQ_OFFSET) && curr !=) -> else {
...
pub unsafe extern "C" fn account_system_time(p: *mut task_struct, hardirq_offset: c_int, cputime: u64) {
    ...
    index = CPUTIME_IRQ;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: in_serving_softirq()) -> else {
...
//
    ...
    let mut iowait: c_int = idx == CPUTIME_IOWAIT;
    u64 *cpustat = kc.cpustat;
    unsigned int seq;
    u64 idle;
    do {
    ...
    let mut delta: u64 = now - kc.idle_entrytime;
...
pub unsafe extern "C" fn vtime_account_irq(tsk: *mut task_struct, offset: c_uint) {
    ...
    {
    let mut pc: c_uint = irq_count() - offset;
...
pub unsafe extern "C" fn account_process_tick(p: *mut task_struct, user_tick: c_int) {
    ...
    account_user_time(p, cputime);
#[no_mangle]
pub unsafe extern "C" fn if(HARDIRQ_OFFSET): (p != this_rq()->idle) || (irq_count() !=) -> else {
...
unsafe extern "C" fn get_vtime_delta(vtime: *mut vtime) -> u64 {
    ...
    {
    let mut delta: u64 = vtime_delta(vtime);
    u64 other;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(PF_VCPU: current->flags &) -> else {
...
unsafe extern "C" fn vtime_state_fetch(vtime: *mut vtime, cpu: c_int) -> c_int {
    ...
    {
    let mut state: c_int = READ_ONCE(vtime.state);
//
...
#[no_mangle]
pub unsafe extern "C" fn if(VTIME_GUEST: vtime->state ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn kcpustat_field(usage: enum cpu_usage_stat, cpu: c_int) -> u64 {
    u64 kcpustat_field(enum cpu_usage_stat usage, int cpu)
    {
    u64 *cpustat = kcpustat_cpu(cpu).cpustat;
    let mut val: u64 = cpustat[usage];
    struct rq *rq;
    int err;
    if (!vtime_generic_enabled_cpu(cpu))
