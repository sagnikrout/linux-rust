...
pub unsafe extern "C" fn wait_for_completion_interruptible(x: *mut completion) -> int __sched {
    ...
    {
    let mut t: c_long = wait_for_common(x, MAX_SCHEDULE_TIMEOUT, TASK_INTERRUPTIBLE);
    if (t == -ERESTARTSYS)
    return t;
    return 0;
    }
...
pub unsafe extern "C" fn wait_for_completion_killable(x: *mut completion) -> int __sched {
    ...
    {
    let mut t: c_long = wait_for_common(x, MAX_SCHEDULE_TIMEOUT, TASK_KILLABLE);
    if (t == -ERESTARTSYS)
    return t;
    return 0;
    }
...
pub unsafe extern "C" fn wait_for_completion_state(x: *mut completion, state: c_uint) -> int __sched {
    ...
    {
    let mut t: c_long = wait_for_common(x, MAX_SCHEDULE_TIMEOUT, state);
    if (t == -ERESTARTSYS)
    return t;
    return 0;
    }
...
//
    ...
    ret = false;
#[no_mangle]
pub unsafe extern "C" fn if(UINT_MAX: x->done !=) -> else {
