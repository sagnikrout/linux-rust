...
pub struct numa_group {
    ...
    pub rcu: rcu_head,
    pub total_faults: c_ulong,
    pub max_faults_cpu: c_ulong,
//
...
pub const NR_NUMA_HINT_FAULT_TYPES: c_int = 2;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct numa_stats {
    pub load: c_ulong,
    pub runnable: c_ulong,
    pub util: c_ulong,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct task_numa_env {
    pub p: *mut task_struct,
    pub src_nid: int src_cpu,,
    pub dst_nid: int dst_cpu,,
    pub imb_numa_nr: c_int,
    pub dst_stats: numa_stats src_stats,,
    pub imbalance_pct: c_int,
    pub dist: c_int,
    pub best_task: *mut task_struct,
    pub best_imp: c_long,
    pub best_cpu: c_int,
}
...
// Use idle CPU if there is no imbalance
    ...
    task_numa_assign(env, core::ptr::null_mut(), 0);
    return;
    }
    }
    } else {
...
// No better CPU than the current one was found.
    if (env.best_cpu == -1) {
    trace_sched_stick_numa(p, env.src_cpu, core::ptr::null_mut(), -1);
    return -EAGAIN;
    }
...
//
pub const NUMA_PERIOD_SLOTS: c_int = 10;
pub const NUMA_PERIOD_THRESHOLD: c_int = 7;
//
...
// safe: p either is current or is being freed by current
    ...
    if (final) {
    p.numa_faults = core::ptr::null_mut();
    kfree(numa_faults);
    } else {
...
//
pub const UPDATE_TG: c_uint = 0x01;
pub const SKIP_AGE_LOAD: c_uint = 0x02;
pub const DO_ATTACH: c_uint = 0x04;
pub const DO_DETACH: c_uint = 0x08;
pub const UPDATE_UTIL_EST: c_uint = 0x10;
// Update task and its cfs_rq load average
#[no_mangle]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct energy_env {
    pub task_busy_time: c_ulong,
    pub pd_busy_time: c_ulong,
    pub cpu_cap: c_ulong,
    pub pd_cap: c_ulong,
}
...
pub const LBF_ALL_PINNED: c_uint = 0x01;
pub const LBF_NEED_BREAK: c_uint = 0x02;
pub const LBF_DST_PINNED: c_uint = 0x04;
pub const LBF_SOME_PINNED: c_uint = 0x08;
pub const LBF_ACTIVE_LB: c_uint = 0x10;
pub const LBF_LLC_PINNED: c_uint = 0x20;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lb_env {
...
unsafe extern "C" fn update_cfs_rq_h_load(cfs_rq: *mut cfs_rq) {
    ...
    while ((se = READ_ONCE(cfs_rq.h_load_next)) != core::ptr::null_mut()) {
    load = cfs_rq.h_load;
