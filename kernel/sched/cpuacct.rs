...
// cpuusage holds pointer to a u64-type object on every CPU
    pub cpuusage: *mut u64 __percpu,
    pub cpustat: *mut kernel_cpustat __percpu,
}

    static inline struct cpuacct *css_ca(struct cgroup_subsys_state *css)
    {
    return css ? container_of(css, struct cpuacct, css) : core::ptr::null_mut();
    }
...
pub unsafe extern "C" fn cpuacct_charge(tsk: *mut task_struct, cputime: u64) {
    ...
    {
    let mut cpu: c_uint = task_cpu(tsk);
    struct cpuacct *ca;
