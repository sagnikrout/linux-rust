...
//
    ...
    {
    let mut min: c_ulong = (unsigned long)base;
    let mut max: c_ulong = min + size;
...
#[no_mangle]
unsafe extern "C" fn init_module_sysctl() -> int __init {
...
unsafe extern "C" fn mod_strncmp(str_a: *const c_char, str_b: *const c_char, n: usize) -> c_int {
    ...
    {
    for (int i = 0; i < n; i++) {
    let mut a: c_char = str_a[i];
    let mut b: c_char = str_b[i];
    int d;
    if (a == '-') a = '_';
    if (b == '-') b = '_';
    d = a - b;
...
// Find a module section, or NULL.  Fill in number of "objects" in section.
    ...
    unsigned int *num)
    {
    let mut sec: c_uint = find_sec(info, name);
...
//
    ...
    unsigned int *num)
    {
    let mut sec: c_uint = find_any_sec(info, name);
// Section 0 has sh_addr 0 and sh_size 0.
// num = info->sechdrs[sec].sh_size / object_size;
    return (void *)info.sechdrs[sec].sh_addr;
    }
...
unsafe extern "C" fn percpu_modalloc(mod: *mut module, info: *mut load_info) -> c_int {
    ...
    let mut align: c_ulong = pcpusec.sh_addralign;
    if (!pcpusec.sh_size)
    return 0;
    if (align > PAGE_SIZE) {
...
pub unsafe extern "C" fn try_force_unload(flags: c_uint) -> c_int {
    ...
    {
    let mut ret: c_int = (flags & O_TRUNC);
    if (ret)
...
pub unsafe extern "C" fn symbol_put_addr(addr: *mut c_void) {
    ...
    let mut a: c_ulong = (unsigned long)dereference_function_descriptor(addr);
...
pub unsafe extern "C" fn module_flags_taint(taints: c_ulong, buf: *mut c_char) -> usize {
    ...
    struct module_kobject *mk, char *buffer)
    {
    let mut size: c_uint = mk.mod.mem[MOD_TEXT].size;
    ...
    let mut modinfo_attrs_count: usize = ARRAY_SIZE(modinfo_attrs);
...
// Skip any zero padding.
    ...
    {
    char *p;
    let mut taglen: c_uint = strlen(tag);
    Elf_Shdr *infosec = &info.sechdrs[info.index.info];
    let mut size: c_ulong = infosec.sh_size;
//
...
#[no_mangle]
unsafe extern "C" fn module_memory_alloc(mod: *mut module, type: enum mod_mem_type) -> c_int {
    static int module_memory_alloc(struct module *mod, enum mod_mem_type type)
    {
    let mut size: c_uint = PAGE_ALIGN(mod.mem[type].size);
...
#[no_mangle]
unsafe extern "C" fn module_memory_free(mod: *mut module, type: enum mod_mem_type) {
...
// Now do relocations.
    for (i = 1; i < info.hdr.e_shnum; i++) {
    let mut infosec: c_uint = info.sechdrs[i].sh_info;
...
//
    ...
    core::ptr::null_mut());
#[no_mangle]
pub unsafe extern "C" fn if(SHT_REL: info->sechdrs[i].sh_type ==) -> else {
    else if (info.sechdrs[i].sh_type == SHT_REL)
    err = apply_relocate(info.sechdrs, info.strtab,
    info.index.sym, i, mod);
#[no_mangle]
pub unsafe extern "C" fn if(SHT_RELA: info->sechdrs[i].sh_type ==) -> else {
...
// default implementation just returns zero
    ...
    {
    long offset;
    let mut mask: c_long = ((unsigned long)(type) & SH_ENTSIZE_TYPE_MASK) << SH_ENTSIZE_TYPE_SHIFT;
...
//
    ...
    let mut type: enum mod_mem_type = is_init ? init_m_to_mem_type[m] : core_m_to_mem_type[m];
...
unsafe extern "C" fn elf_validity_cache_index_str(info: *mut load_info) -> c_int {
    ...
    {
    let mut str_idx: c_uint = info.sechdrs[info.index.sym].sh_link;
...
#[no_mangle]
unsafe extern "C" fn copy_chunked_from_user(dst: *mut c_void, usrc: *const void __user, len: c_ulong) -> c_int {
    static int copy_chunked_from_user(void *dst, const void __user *usrc, unsigned long len)
    {
    do {
    let mut n: c_ulong = min(len, COPY_CHUNK_SIZE);
...
unsafe extern "C" fn module_augment_kernel_taints(mod: *mut module, info: *mut load_info) {
    ...
    {
    let mut prev_taint: c_int = test_taint(TAINT_PROPRIETARY_MODULE);
    size_t i;
...
unsafe extern "C" fn find_module_sections(mod: *mut module, info: *mut load_info) -> c_int {
    ...
    sizeof(*mod.ctors), &mod.num_ctors);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: find_sec(info, _arg: ".init_array")) -> else {
//
...
unsafe extern "C" fn move_module(mod: *mut module, info: *mut load_info) -> c_int {
    ...
    {
    int i, ret;
    let mut t: enum mod_mem_type = MOD_MEM_NUM_TYPES;
    let mut codetag_section_found: bool = false;
    for_each_mod_mem_type(type) {
...
//
    ...
    let mut type: enum mod_mem_type = shdr.sh_entsize >> SH_ENTSIZE_TYPE_SHIFT;
    let mut offset: c_ulong = shdr.sh_entsize & SH_ENTSIZE_OFFSET_MASK;
...
unsafe extern "C" fn do_init_module(mod: *mut module) -> noinline int {
    ...
    let mut text_size: c_uint = 0, total_size = 0;
    for_each_mod_mem_type(type) {
    const struct module_memory *mod_mem = &mod.mem[type];
    if (mod_mem.size) {
...
//
    ...
    {
    struct module *mod;
    let mut module_allocated: bool = false;
    let mut err: c_long = 0;
    char *args = core::ptr::null_mut(), *after_dashes;
//
...
unsafe extern "C" fn idempotent(u: *mut idempotent, cookie: *const c_void) -> bool {
    ...
    {
    let mut hash: c_int = hash_ptr(cookie, IDEM_HASH_BITS);
...
#[no_mangle]
unsafe extern "C" fn init_module_from_file(f: *mut file, uargs: *const *const char __user, flags: c_int) -> c_int {
    static int init_module_from_file(struct file *f, const char __user * uargs, int flags)
    {
    let mut compressed: bool = !!(flags & MODULE_INIT_COMPRESSED_FILE);
    let mut info: load_info = { };
...
#[no_mangle]
unsafe extern "C" fn idempotent_init_module(f: *mut file, uargs: *const *const char __user, flags: c_int) -> c_int {
...
// Are we the winners of the race and get to do this?
    ...
    let mut ret: c_int = init_module_from_file(f, uargs, flags);
...
//
    ...
    {
    let mut err: c_int = may_init_module();
    if (err)
    return err;
...
#[no_mangle]
pub unsafe extern "C" fn module_for_each_mod(mod: *mut *mut int(func)(struct module, data): *mut c_void, data: *mut c_void) {
