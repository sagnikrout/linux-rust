...
unsafe extern "C" fn __mod_tree_val(n: *mut latch_tree_node) -> __always_inline unsigned long {
    let mut mod_mem = container_of!(n, module_memory, mtn.node);
...
unsafe extern "C" fn __mod_tree_size(n: *mut latch_tree_node) -> __always_inline unsigned long {
    ...
    mod_tree_less(latch_tree_node *a, latch_tree_node *b)
    {
    return __mod_tree_val(a) < __mod_tree_val(b);
    }
    static __always_inline int
    mod_tree_comp(void *key, latch_tree_node *n)
    {
...
#[no_mangle]
pub unsafe extern "C" fn mod_find(addr: c_ulong, tree: *mut mod_tree_root) -> *mut c_void {
pub static mut ltn: *mut c_void = core::ptr::null_mut();
    ltn = latch_tree_find(addr, &tree.root, &mod_tree_ops);
    if (!ltn) {
    return core::ptr::null_mut();
    }
    return container_of!(ltn, mod_tree_node, node).mod;
    }