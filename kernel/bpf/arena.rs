...
//
    ...
    let mut old: pte_t = ptep_get(pte);
    if (pte_none(old))
    continue;
...
// User space requested to segfault when page is not allocated by bpf prog
    ...
    if (ret)
    goto out_sigsegv_memcg;
    let mut data: apply_range_data = { .arena = arena, .pages = &page, .i = 0 };
...
// user_vm_end/start are fixed before bpf prog runs
    let mut page_cnt_max: c_long = (arena.user_vm_end - arena.user_vm_start) >> PAGE_SHIFT;
    let mut kern_vm_start: u64 = bpf_arena_get_kern_vm_start(arena);
    struct mem_cgroup *new_memcg, *old_memcg;
...
// Cap allocation size to KMALLOC_MAX_CACHE_SIZE so kmalloc_nolock() can succeed.
    ...
    while (remaining) {
    let mut this_batch: c_long = min(remaining, alloc_pages);
...
unsafe extern "C" fn zap_pages(arena: *mut bpf_arena, uaddr: c_long, page_cnt: c_long) {
    ...
    {
    let mut size: c_ulong = (unsigned long)page_cnt << PAGE_SHIFT;
...
pub unsafe extern "C" fn bpf_arena_handle_page_fault(addr: c_ulong, is_write: bool, fault_ip: c_ulong) -> bool {
    ...
    unsigned long kbase;
    let mut page_addr: c_ulong = addr & PAGE_MASK;
