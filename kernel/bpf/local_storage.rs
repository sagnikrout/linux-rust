...
unsafe extern "C" fn attach_type_isolated(map: *const bpf_map) -> bool {
    ...
    return -1;
#[no_mangle]
pub unsafe extern "C" fn if(key2->cgroup_inode_id: key1->cgroup_inode_id >) -> else {
    else if (key1.cgroup_inode_id > key2.cgroup_inode_id)
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(key2->attach_type: key1->attach_type <) -> else {
...
pub unsafe extern "C" fn if(key2->attach_type: key1->attach_type >) -> else {
    ...
    return -1;
#[no_mangle]
pub unsafe extern "C" fn if(cgroup_inode_id2: *mut *mut cgroup_inode_id1 >) -> else {
...
// next = storage->key.cgroup_inode_id;
    ...
    {
    let mut max_value_size: __u32 = BPF_LOCAL_STORAGE_MAX_VALUE_SIZE;
...
pub unsafe extern "C" fn bpf_cgroup_storage_assign(aux: *mut bpf_prog_aux, _map: *mut bpf_map) -> c_int {
    ...
    {
    let mut stype: enum bpf_cgroup_storage_type = cgroup_storage_type(_map);
...
// pages = round_up(round_up(size, 8) * num_possible_cpus(),
    ...
    enum bpf_cgroup_storage_type stype)
    {
    let mut gfp: gfp_t = __GFP_ZERO | GFP_USER;
    struct bpf_cgroup_storage *storage;
    struct bpf_map *map;
    size_t size;
    u32 pages;
