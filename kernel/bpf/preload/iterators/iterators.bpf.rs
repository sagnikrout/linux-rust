...
pub struct bpf_iter__bpf_prog {
    ...
    static const char *get_name(btf *btf, long btf_id, const char *fallback)
    {
    let mut types = core::ptr::null_mut();
...
pub static mut str: *mut c_void = core::ptr::null_mut();
    ...
    __s64 bpf_map_sum_elem_count(bpf_map *map) __ksym;
    SEC("iter/bpf_map")
#[no_mangle]
pub unsafe extern "C" fn dump_bpf_map(ctx: *mut bpf_iter__bpf_map) -> c_int {
    let mut seq = ctx.meta.seq;
pub static mut seq_num: __u64 = 0;
    let mut map = ctx.map;
    if (!map) {
    return 0;
    }
