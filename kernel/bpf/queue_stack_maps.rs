...
#[no_mangle]
pub unsafe extern "C" fn bpf_queue_stack(map: *mut bpf_map) -> *mut c_void {
    return container_of!(map, bpf_queue_stack, map);
    }
#[no_mangle]
...
pub static mut head: u32 = 0;
    if (unlikely(head >= qs.size)) {
    head = 0;
    }
pub static mut head: return = 0;
    }
// Called from syscall
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn queue_stack_map_alloc(attr: *mut union bpf_attr) -> *mut c_void {
pub static mut numa_node: c_int = 0;
pub static mut qs: *mut c_void = core::ptr::null_mut();
    u64 size, queue_size;
...
unsafe extern "C" fn queue_stack_map_free(map: *mut bpf_map) {
    let mut qs = bpf_queue_stack(map);
...
// Called from syscall or from eBPF program
#[no_mangle]
pub unsafe extern "C" fn queue_stack_map_push_elem(map: *mut bpf_map, value: *mut c_void, flags: u64) -> c_long {
    let mut qs = bpf_queue_stack(map);
    let mut irq_flags = 0;
pub static mut err: c_int = 0;
...
// Called from syscall or from eBPF program
#[no_mangle]
pub unsafe extern "C" fn queue_stack_map_lookup_elem(map: *mut bpf_map, key: *mut c_void) -> *mut c_void {
    return core::ptr::null_mut();
    }
// Called from syscall or from eBPF program
#[no_mangle]
pub unsafe extern "C" fn queue_stack_map_update_elem(map: *mut bpf_map, key: *mut c_void, value: *mut c_void, flags: u64) -> c_long {
    return -EINVAL;
    }
// Called from syscall or from eBPF program
#[no_mangle]
...
// Called from syscall
#[no_mangle]
pub unsafe extern "C" fn queue_stack_map_get_next_key(map: *mut bpf_map, key: *mut c_void, next_key: *mut c_void) -> c_int {
    return -EINVAL;
    }
#[no_mangle]
