...
#[no_mangle]
unsafe extern "C" fn cgroup_bpf_wq_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn cgroup_bpf_lifetime_notifier_init() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn cgroup_bpf_hook_returns_errno(atype: enum cgroup_bpf_attach_type) -> bool {
...
pub unsafe extern "C" fn bpf_cgroup_atype_get(attach_btf_id: u32, cgroup_atype: c_int) {
    ...
    {
    let mut i: c_int = cgroup_atype - CGROUP_LSM_START;
    lockdep_assert_held(&cgroup_mutex);
...
//
    ...
    return true;
    do {
    let mut flags: u32 = p.bpf.flags[atype];
    u32 cnt;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: id_or_fd) -> else {
    else if (id_or_fd)
...
pub unsafe extern "C" fn if(_arg: id_or_fd) -> else {
    ...
    {
    let mut is_link: bool = flags & BPF_F_LINK, is_id = flags & BPF_F_ID;
    struct bpf_prog_list *pltmp, *pl = ERR_PTR(-EINVAL);
    let mut preorder: bool = flags & BPF_F_PREORDER;
    struct bpf_link *anchor_link = core::ptr::null_mut();
    struct bpf_prog *anchor_prog = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(BPF_F_BEFORE: flags &) -> else {
...
//
    ...
    u64 revision)
    {
    let mut saved_flags: u32 = (flags & (BPF_F_ALLOW_OVERRIDE | BPF_F_ALLOW_MULTI));
    struct bpf_prog *old_prog = core::ptr::null_mut();
...
//
    ...
    {
    let mut cnt: c_int = 0, preorder_cnt = 0, fstart, bstart, init_bstart, pos = -1;
    struct bpf_prog_list *pl;
    struct cgroup *p = cgrp;
...
// Must be called with cgroup_mutex held to avoid races.
    ...
    let mut effective_query: bool = attr.query.query_flags & BPF_F_QUERY_EFFECTIVE;
    __u32 __user *prog_ids = u64_to_user_ptr(attr.query.prog_ids);
    let mut type: enum bpf_attach_type = attr.query.attach_type;
    enum cgroup_bpf_attach_type from_atype, to_atype;
    enum cgroup_bpf_attach_type atype;
    struct bpf_prog_array *effective;
    int cnt, ret = 0, i;
    let mut total_cnt: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn cgroup_bpf_prog_detach(attr: *const union bpf_attr, ptype: enum bpf_prog_type) -> c_int {
...
//
    let mut stype: enum bpf_cgroup_storage_type = cgroup_storage_type(map);
