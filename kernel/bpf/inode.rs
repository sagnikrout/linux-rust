...
#[no_mangle]
unsafe extern "C" fn bpf_any_put(raw: *mut c_void, type: enum bpf_type) {
...
// type = BPF_TYPE_PROG;
#[no_mangle]
pub unsafe extern "C" fn if(&bpf_map_iops: inode->i_op ==) -> else {
    else if (inode.i_op == &bpf_map_iops)
// type = BPF_TYPE_MAP;
#[no_mangle]
pub unsafe extern "C" fn if(&bpf_link_iops: inode->i_op ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_obj_pin_user(ufd: u32, path_fd: c_int, pathname: *const char __user) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_obj_get_user(path_fd: c_int, pathname: *const char __user, flags: c_int) -> c_int {
    int bpf_obj_get_user(int path_fd, const char __user *pathname, int flags)
    {
    let mut type: enum bpf_type = BPF_TYPE_UNSPEC;
    int f_flags;
    void *raw;
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(BPF_TYPE_MAP: type ==) -> else {
    else if (type == BPF_TYPE_MAP)
    ret = bpf_map_new_fd(raw, f_flags);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_TYPE_LINK: type ==) -> else {
    ...
    let mut ret: c_int = inode_permission(&nop_mnt_idmap, inode, MAY_READ);
    if (ret)
    return ERR_PTR(ret);
    ...
    struct bpf_prog *prog;
    struct path path;
    let mut ret: c_int = kern_path(name, LOOKUP_FOLLOW, &path);
    if (ret)
    return ERR_PTR(ret);
...
unsafe extern "C" fn bpf_show_options(m: *mut seq_file, root: *mut dentry) -> c_int {
    ...
    let mut mode: umode_t = inode.i_mode & S_IALLUGO & ~S_ISVTX;
...
#[no_mangle]
unsafe extern "C" fn bpf_init() -> int __init {
