...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_FD_MAP(map)) -> else {
...
//
    let mut gfp: gfp_t = bpf_memcg_flags(__GFP_NOWARN | __GFP_ZERO);
    let mut flags: c_uint = 0;
    let mut align: c_ulong = 1;
    void *area;
    if (size >= SIZE_MAX)
...
unsafe extern "C" fn btf_field_cmp(a: *const c_void, b: *const c_void) -> c_int {
    ...
    return -1;
#[no_mangle]
pub unsafe extern "C" fn if(f2->offset: f1->offset >) -> else {
...
pub unsafe extern "C" fn btf_record_equal(rec_a: *const btf_record, rec_b: *const btf_record) -> bool {
    ...
    {
    let mut a_has_fields: bool = !IS_ERR_OR_NULL(rec_a), b_has_fields = !IS_ERR_OR_NULL(rec_b);
    int size;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: READ_ONCE(map->free_after_rcu_gp)) -> else {
...
unsafe extern "C" fn map_get_sys_perms(map: *mut bpf_map, f: fd) -> fmode_t {
    ...
    {
    let mut mode: fmode_t = fd_file(f).f_mode;
...
// called via syscall
    ...
    struct bpf_token *token = core::ptr::null_mut();
    let mut numa_node: c_int = bpf_map_attr_numa_node(attr);
    let mut map_type: u32 = attr.map_type;
    struct bpf_map *map;
    bool token_flag;
    int err;
...
//
    ...
    let mut uprog_hash: bpfptr_t = make_bpfptr(attr.excl_prog_hash, uattr.is_kernel);
...
unsafe extern "C" fn map_update_elem(attr: *mut union bpf_attr, uattr: bpfptr_t) -> c_int {
    ...
    {
    let mut ukey: bpfptr_t = make_bpfptr(attr.key, uattr.is_kernel);
    let mut uvalue: bpfptr_t = make_bpfptr(attr.value, uattr.is_kernel);
    struct bpf_map *map;
    void *key, *value;
    u32 value_size;
    int err;
...
#[no_mangle]
unsafe extern "C" fn find_prog_type(type: enum bpf_prog_type, prog: *mut bpf_prog) -> c_int {
...
unsafe extern "C" fn bpf_audit_prog(prog: *const bpf_prog, op: c_uint) {
    ...
    {
    struct audit_context *ctx = core::ptr::null_mut();
...
pub unsafe extern "C" fn bpf_prog_inc_misses_counter(prog: *mut bpf_prog) -> void notrace {
    ...
    {
    let mut nsecs: u64 = 0, cnt = 0, misses = 0;
    int cpu;
...
#[no_mangle]
unsafe extern "C" fn is_net_admin_prog_type(prog_type: enum bpf_prog_type) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_perfmon_prog_type(prog_type: enum bpf_prog_type) -> bool {
...
unsafe extern "C" fn bpf_prog_load(attr: *mut union bpf_attr, uattr: bpfptr_t, attr_log: *mut bpf_log_attr) -> c_int {
    ...
    {
    let mut type: enum bpf_prog_type = attr.prog_type;
    struct bpf_prog *prog, *dst_prog = core::ptr::null_mut();
    struct btf *attach_btf = core::ptr::null_mut();
    struct bpf_token *token = core::ptr::null_mut();
    bool bpf_cap;
    int err;
    char license[128];
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: bpf_link_is_tracepoint(link)) -> else {
...
unsafe extern "C" fn bpf_link_show_fdinfo(m: *mut seq_file, filp: *mut file) {
    ...
    let mut type: enum bpf_link_type = link.type;
    char prog_tag[sizeof(prog.tag) * 2 + 1] = { };
...
#[no_mangle]
pub unsafe extern "C" fn if(BPF_LINK_TYPE_UPROBE_MULTI: link->type ==) -> else {
...
unsafe extern "C" fn bpf_tracing_link_dealloc(link: *mut bpf_link) {
    ...
    struct bpf_prog *tgt_prog = core::ptr::null_mut();
    struct bpf_trampoline *tr = core::ptr::null_mut();
...
unsafe extern "C" fn bpf_raw_tp_link_dealloc(link: *mut bpf_link) {
    ...
    let mut ulen: u32 = info.raw_tracepoint.tp_name_len;
    let mut tp_len: usize = strlen(tp_name);
    if (!ulen ^ !ubuf)
    return -EINVAL;
...
unsafe extern "C" fn bpf_raw_tracepoint_open(attr: *const union bpf_attr) -> c_int {
    ...
    case BPF_PROG_TYPE_CGROUP_SOCKOPT:
    case BPF_PROG_TYPE_SK_LOOKUP:
    let mut attach_type: return = = prog.expected_attach_type ? 0 : -EINVAL;
    case BPF_PROG_TYPE_CGROUP_SKB:
...
unsafe extern "C" fn bpf_prog_detach(attr: *const union bpf_attr) -> c_int {
    ...
    spinlock_t *lock)
    {
    let mut next_id: u32 = attr.start_id;
    let mut err: c_int = 0;
...
//
    ...
    let mut info_len: u32 = attr.info.info_len;
    struct bpf_prog_kstats stats;
    char __user *uinsns;
    u32 ulen, len;
    int err;
...
// copy the JITed image lengths for each function
    ...
    let mut info_len: u32 = attr.info.info_len, len;
    int err;
...
pub unsafe extern "C" fn if(&bpf_link_fops_poll: fd_file(f)->f_op == &bpf_link_fops || fd_file(f)->f_op ==) -> else {
    ...
    attr, uattr);
#[no_mangle]
pub unsafe extern "C" fn if(&bpf_token_fops: fd_file(f)->f_op ==) -> else {
...
unsafe extern "C" fn bpf_btf_get_fd_by_id(attr: *const union bpf_attr) -> c_int {
    ...
    let mut len: u32 = buf ? strlen(buf) : 0, input_len;
    let mut err: c_int = 0;
...
// nothing to copy, just make ubuf NULL terminated
    ...
    union bpf_attr __user *uattr)
    {
    let mut pid: pid_t = attr.task_fd_query.pid;
    let mut fd: u32 = attr.task_fd_query.fd;
    const struct perf_event *event;
    ...
    let mut has_write: bool = cmd != BPF_MAP_LOOKUP_BATCH;
    struct bpf_map *map;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn if(BPF_MAP_LOOKUP_AND_DELETE_BATCH: cmd ==) -> else {
    else if (cmd == BPF_MAP_LOOKUP_AND_DELETE_BATCH)
    BPF_DO_BATCH(map.ops.map_lookup_and_delete_batch, map, attr, uattr);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_MAP_UPDATE_BATCH: cmd ==) -> else {
...
unsafe extern "C" fn link_create(attr: *mut union bpf_attr, uattr: bpfptr_t) -> c_int {
    ...
    attr.link_create.attach_type);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_TRACE_ITER: prog->expected_attach_type ==) -> else {
    else if (prog.expected_attach_type == BPF_TRACE_ITER)
    ret = bpf_iter_link_attach(attr, uattr, prog);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_LSM_CGROUP: prog->expected_attach_type ==) -> else {
    else if (prog.expected_attach_type == BPF_LSM_CGROUP)
    ret = cgroup_bpf_link_attach(attr, prog);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_tracing_multi(prog->expected_attach_type)) -> else {
...
unsafe extern "C" fn link_update(attr: *mut union bpf_attr) -> c_int {
    ...
    {
    struct bpf_prog *old_prog = core::ptr::null_mut(), *new_prog;
    struct bpf_link *link;
    u32 flags;
    int ret;
...
unsafe extern "C" fn prog_stream_read(attr: *mut union bpf_attr) -> c_int {
    ...
    let mut len: u32 = attr.prog_stream_read.stream_buf_len;
    struct bpf_prog *prog;
    int ret;
...
unsafe extern "C" fn prog_assoc_struct_ops(attr: *mut union bpf_attr) -> c_int {
    ...
    let mut offsetof_log_true_size: u32 = 0;
    struct bpf_log_attr attr_log;
    union bpf_attr attr;
    int err;
...
pub unsafe extern "C" fn unpriv_ebpf_notify(new_state: c_int) -> void __weak {
    ...
    let mut locked_state: bool = unpriv_enable == 1;
...
#[no_mangle]
unsafe extern "C" fn bpf_syscall_sysctl_init() -> int __init {
