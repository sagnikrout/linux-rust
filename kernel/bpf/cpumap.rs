...
// XDP can run multiple RX-ring queues, need __percpu enqueue store
    pub bulkq: *mut xdp_bulk_queue __percpu,
...
// Below members specific for map type
    pub cpu_map: *mut bpf_cpu_map_entry __rcu,
}
...
unsafe extern "C" fn cpu_map_kthread_run(data: *mut c_void) -> c_int {
    ...
    let mut last_qs: c_ulong = jiffies;
...
//
    ...
    let mut kmem_alloc_drops: c_uint = 0, sched = 0;
    let mut ret: cpu_map_ret = { };
...
// notice caller map_delete_elem() uses rcu_read_lock()
    ...
    {
    struct bpf_cpu_map *cmap = container_of(map, struct bpf_cpu_map, map);
    let mut cpumap_value: bpf_cpumap_val = {};
    struct bpf_cpu_map_entry *rcpu;
...
unsafe extern "C" fn cpu_map_mem_usage(map: *const bpf_map) -> u64 {
    ...
    {
    let mut usage: u64 = sizeof(struct bpf_cpu_map);
...
unsafe extern "C" fn bq_flush_to_queue(bq: *mut xdp_bulk_queue) {
    ...
    let mut processed: c_uint = 0, drops = 0;
    let mut to_cpu: c_int = rcpu.cpu;
    struct ptr_ring *q;
    int i;
