...
pub struct const_arg_info {
    pub state: enum const_arg_state,
    pub map_index: u32,
    pub val: u64,
}
...
// Transfer function: compute output register state from instruction.
    ...
    {
    let mut unknown: const_arg_info = { .state = CONST_ARG_UNKNOWN, .val = 0 };
    ...
    let mut opcode: u8 = BPF_OP(insn.code) | BPF_SRC(insn.code);
    int r;
...
// dst = unknown;
    ...
    dst.val = (u32)dst.val;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !ci_is_unknown(dst)) -> else {
...
// dst = unknown;
    ...
    let mut subprog: c_int = bpf_find_subprog(env, idx + insn.imm + 1);
    if (subprog >= 0) {
...
// dst = unknown;
    ...
    let mut size: c_int = bpf_size_to_bytes(BPF_SIZE(insn.code));
    let mut is_ldsx: bool = mode == BPF_MEMSX;
    let mut off: c_int = src.val + insn.off;
    let mut val: u64 = 0;
...
// Subprogram entries (including main at subprog 0): all registers unknown
    ...
    let mut start: c_int = env.subprog_info[i].start;
    for (r = 0; r < MAX_BPF_REG; r++)
    ...
    let mut idx: c_int = env.cfg.insn_postorder[i];
...
// Save computed constants into insn_aux[] if they fit into 32-bit
    ...
    let mut mask: u16 = 0, map_mask = 0, subprog_mask = 0;
