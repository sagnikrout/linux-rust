...
pub const BPF_STRTOX_BASE_MASK: c_uint = 0x1F;
    ...
    {
    let mut base: c_uint = flags & BPF_STRTOX_BASE_MASK;
    const char *cur_buf = buf;
    let mut cur_len: usize = buf_len;
...
// res = _res;
    ...
    {
    let mut ret: c_int = copy_from_user(dst, user_ptr, size);
    if (unlikely(ret)) {
    memset(dst, 0, size);
    ret = -EFAULT;
    }
...
// Return -EFAULT for partial read
    ...
    size_t bufsz)
    {
    void __user *user_ptr = ( void __user *)unsafe_ptr;
    buf[0] = 0;
    switch (fmt_ptype) {
    case 's':
...
//
    ...
    {
    let mut get_buffers: bool = (data.get_bin_args && num_args) || data.get_buf;
    char *unsafe_ptr = core::ptr::null_mut(), *tmp_buf = core::ptr::null_mut(), *tmp_buf_end, *fmt_end;
    struct bpf_bprintf_buffers *buffers = core::ptr::null_mut();
...
pub struct bpf_async_cb {
    ...
    pub callback_fn: *mut void __rcu,
    pub value: *mut c_void,
    pub rcu: rcu_head,
    pub flags: u64,
    pub worker: irq_work,
    pub refcnt: refcount_t,
    pub type: enum bpf_async_type,
...
//
    ...
    u64, flags)
    {
    let mut clockid: clock_t = flags & (MAX_CLOCKS - 1);
...
//
    ...
    (void  *)READ_ONCE(cb.callback_fn) != callback_fn);
    if (prog)
    bpf_prog_put(prog);
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn bpf_dynptr_set_type(ptr: *mut bpf_dynptr_kern, type: enum bpf_dynptr_type) {
...
unsafe extern "C" fn bpf_dynptr_set_size(ptr: *mut bpf_dynptr_kern, new_size: u64) {
    ...
    {
    let mut metadata: u32 = ptr.size & ~DYNPTR_SIZE_MASK;
    if (bpf_dynptr_get_type(ptr) == BPF_DYNPTR_TYPE_FILE) {
    struct bpf_dynptr_file_impl *df = ptr.data;
    df.size = new_size;
    return;
...
//
    ...
    {
    let mut size: u64 = local_type_id__k;
    void *p;
...
//
    ...
    struct rb_node *parent = core::ptr::null_mut(), *n = &node.rb_node;
    let mut cb: bpf_callback_t = (bpf_callback_t)less;
...
//
    ...
    return __bpf_rbtree_add(root, n, (void *)less, meta ? meta.record : core::ptr::null_mut(), off);
    }
...
//
    ...
    off = 0;
    while (off < size) {
    let mut chunk_sz: u64 = min_t(u64, sizeof(buf), size - off);
    int err;
...
//
    ...
    let mut nr_bytes: u32 = nr_words * sizeof(u64);
    let mut nr_bits: u32 = BYTES_TO_BITS(nr_bytes);
    int err;
...
//
    ...
    {
    struct bpf_iter_bits_kern *kit = (void *)it;
    let mut bit: c_int = kit.bit, nr_bits = kit.nr_bits;
    const void *bits;
...
//
#[no_mangle]
pub unsafe extern "C" fn bpf_copy_from_user_str(dst: *mut c_void, dst__sz: u32, unsafe_ptr__ign: *const void __user, flags: u64) -> __bpf_kfunc int {
...
unsafe extern "C" fn __bpf_strncasecmp(s1: *const c_char, s2: *const c_char, ignore_case: bool, len: usize) -> c_int {
    ...
    let mut i: return = = XATTR_SIZE_MAX ? -E2BIG : 0;
    err_out:
    return -EFAULT;
    }
//
...
pub unsafe extern "C" fn bpf_strnchr(s__ign: *const c_char, count: usize, c: c_char) -> __bpf_kfunc int {
    ...
    let mut i: return = = XATTR_SIZE_MAX ? -E2BIG : -ENOENT;
    err_out:
    return -EFAULT;
    }
//
...
pub unsafe extern "C" fn bpf_strnlen(s__ign: *const c_char, count: usize) -> __bpf_kfunc int {
    ...
    let mut i: return = = XATTR_SIZE_MAX ? -E2BIG : i;
    err_out:
    return -EFAULT;
    }
//
...
pub struct bpf_task_work_ctx {
    pub state: enum bpf_task_work_state,
    pub refcnt: refcount_t,
...
// the map and map value associated with this context
    ...
    pub mode: enum task_work_notify_mode,
...
#[no_mangle]
unsafe extern "C" fn kfunc_init() -> int __init {
