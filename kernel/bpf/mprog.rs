...
// Copyright (c) 2023 Isovalent
    ...
    let mut id: bool = flags & BPF_F_ID;
    if (id)
...
pub unsafe extern "C" fn if(_arg: id_or_fd) -> else {
    ...
    enum bpf_prog_type type)
    {
    let mut link: bool = flags & BPF_F_LINK;
    let mut id: bool = flags & BPF_F_ID;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tuple->prog) -> else {
    else if (tuple.prog)
...
// entry_new = entry;
    ...
    {
    let mut total: c_int = bpf_mprog_total(entry);
...
// entry_new = peer;
    ...
    idx = 0;
#[no_mangle]
pub unsafe extern "C" fn if(total: idx ==) -> else {
    else if (idx == total)
    idx = total - 1;
