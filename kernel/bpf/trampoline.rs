...
pub struct bpf_trampoline_ops {
    pub data): *mut *mut *mut *mut int (register_fentry)(struct bpf_trampoline tr, struct bpf_tramp_image im, void,
    pub data): *mut *mut *mut int (unregister_fentry)(struct bpf_trampoline tr, u32 orig_flags, void,
...
pub unsafe extern "C" fn bpf_prog_has_trampoline(prog: *const bpf_prog) -> bool {
    ...
    let mut ptype: enum bpf_prog_type = prog.type;
    switch (ptype) {
    ...
    let mut eatype: return = = BPF_LSM_MAC;
    default:
    return false;
    }
    }
...
unsafe extern "C" fn direct_ops_mod(tr: *mut bpf_trampoline, ptr: *mut c_void, lock_direct_mutex: bool) -> c_int {
    ...
    {
    let mut new_t: enum bpf_text_poke_type = BPF_MOD_CALL, old_t = BPF_MOD_CALL;
    void *ip = tr.func.addr;
    if (!new_addr)
    new_t = BPF_MOD_NOP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: bpf_trampoline_use_jmp(tr->flags)) -> else {
    else if (bpf_trampoline_use_jmp(tr.flags))
    new_t = BPF_MOD_JUMP;
    if (!old_addr)
    old_t = BPF_MOD_NOP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: bpf_trampoline_use_jmp(orig_flags)) -> else {
...
#[no_mangle]
unsafe extern "C" fn unregister_fentry(tr: *mut bpf_trampoline, orig_flags: u32, __maybe_unused: *mut *mut void data) -> c_int {
...
pub unsafe extern "C" fn bpf_trampoline_set_flags(tr: *mut bpf_trampoline, flags: u32) {
    ...
    struct bpf_tramp_image *im;
    struct bpf_tramp_nodes *tnodes;
    let mut orig_flags: u32 = tr.flags;
    let mut ip_arg: bool = false;
    int err, total, size;
...
unsafe extern "C" fn bpf_shim_tramp_link_dealloc(link: *mut bpf_link) {
    ...
    {
    struct bpf_shim_tramp_link *shim_link = core::ptr::null_mut();
    struct bpf_prog *p;
...
unsafe extern "C" fn bpf_prog_start_time() -> __always_inline u64 notrace {
    ...
    {
    let mut start: u64 = NO_START_TIME;
...
pub unsafe extern "C" fn bpf_trampoline_enter(prog: *const bpf_prog) -> bpf_trampoline_enter_t {
    ...
    {
    let mut sleepable: bool = prog.sleepable;
...
#[no_mangle]
pub unsafe extern "C" fn arch_alloc_bpf_trampoline(size: c_uint) -> *mut void  __weak {
...
unsafe extern "C" fn register_fentry_multi(tr: *mut bpf_trampoline, im: *mut bpf_tramp_image, ptr: *mut c_void) -> c_int {
    ...
    {
    let mut addr: c_ulong = (unsigned long) im.image;
    let mut ip: c_ulong = ftrace_location(tr.ip);
...
unsafe extern "C" fn unregister_fentry_multi(tr: *mut bpf_trampoline, orig_flags: u32, ptr: *mut c_void) -> c_int {
    ...
    {
    let mut addr: c_ulong = (unsigned long) tr.cur_image.image;
    let mut ip: c_ulong = ftrace_location(tr.ip);
    struct bpf_tracing_multi_data *data = ptr;
    if (bpf_trampoline_use_jmp(tr.flags))
    addr = ftrace_jmp_set(addr);
...
unsafe extern "C" fn bpf_trampoline_multi_attach_rollback(tr: *mut bpf_trampoline) {
    ...
    CONFIG_BPF_SYSCALL */
#[no_mangle]
unsafe extern "C" fn init_trampolines() -> int __init {
