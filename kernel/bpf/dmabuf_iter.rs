...
#[no_mangle]
pub unsafe extern "C" fn dmabuf_iter_seq_start(seq: *mut seq_file, pos: *mut loff_t) -> *mut c_void {
    let mut p = seq.private;
    if (*pos) {
    let mut dmabuf = p.dmabuf;
    if (!dmabuf) {
...
#[no_mangle]
pub unsafe extern "C" fn dmabuf_iter_seq_next(seq: *mut seq_file, v: *mut c_void, pos: *mut loff_t) -> *mut c_void {
    let mut dmabuf = v;
    ++*pos;
...
pub struct bpf_iter__dmabuf {
    ...
    pub dmabuf): *mut *mut __bpf_md_ptr(dma_buf ,,
}

#[no_mangle]
unsafe extern "C" fn __dmabuf_seq_show(seq: *mut seq_file, v: *mut c_void, in_stop: bool) -> c_int {
pub static mut bpf_iter_meta: usize = 0;
pub static mut bpf_iter__dmabuf: usize = 0;
    let mut prog = bpf_iter_get_info(&meta, in_stop);
    if (prog) {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_iter_dmabuf_show_fdinfo(aux: *mut bpf_iter_aux_info, seq: *mut seq_file) {
...
pub static mut bpf_iter_reg: usize = 0;
    DEFINE_BPF_ITER_FUNC(dmabuf, bpf_iter_meta *meta, dma_buf *dmabuf)
    BTF_ID_LIST_SINGLE(bpf_dmabuf_btf_id, struct, dma_buf)
#[no_mangle]
unsafe extern "C" fn dmabuf_iter_init() -> c_int {
...
pub unsafe extern "C" fn bpf_iter_dmabuf_new(it: *mut bpf_iter_dmabuf) -> __bpf_kfunc int {
    ...
    __bpf_kfunc struct dma_buf *bpf_iter_dmabuf_next(bpf_iter_dmabuf *it)
    {
    pub )it: *mut *mut bpf_iter_dmabuf_kern kit = (void,
    if (kit.dmabuf) {
