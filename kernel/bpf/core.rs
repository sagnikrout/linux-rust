...
// tell bpf programs that include vmlinux.h kernel's PAGE_SIZE
    ...
    {
    let mut gfp_flags: gfp_t = bpf_memcg_flags(GFP_KERNEL | __GFP_ZERO | gfp_extra_flags);
...
pub unsafe extern "C" fn bpf_prog_calc_tag(fp: *mut bpf_prog) -> c_int {
    ...
    {
    let mut size: usize = bpf_prog_insn_size(fp);
    struct bpf_insn *dst;
    bool was_ld_map;
    u32 i;
...
//
    ...
    {
    let mut imm_min: i64 = S32_MIN, imm_max = S32_MAX;
    let mut delta: i32 = end_new - end_old;
    let mut imm: i64 = insn.imm;
    if (curr < pos && curr + imm + 1 >= end_old)
    imm += delta;
#[no_mangle]
pub unsafe extern "C" fn if(end_new: curr >= end_new && curr + imm + 1 <) -> else {
    ...
    off += delta;
#[no_mangle]
pub unsafe extern "C" fn if(end_new: curr >= end_new && curr + off + 1 <) -> else {
...
// All BPF JIT sysctl knobs here.
    let mut __read_mostly: int bpf_jit_enable = IS_BUILTIN(CONFIG_BPF_JIT_DEFAULT_ON);
    let mut __read_mostly: int bpf_jit_kallsyms = IS_BUILTIN(CONFIG_BPF_JIT_DEFAULT_ON);
...
pub unsafe extern "C" fn bpf_prog_kallsyms_del(fp: *mut bpf_prog) {
    ...
    return n ? container_of(n, struct bpf_ksym, tnode) : core::ptr::null_mut();
    }
    ...
    if (ksym) {
    let mut symbol_start: c_ulong = ksym.start;
    let mut symbol_end: c_ulong = ksym.end;
...
pub unsafe extern "C" fn bpf_has_frame_pointer(ip: c_ulong) -> bool {
    ...
    {
    const struct exception_table_entry *e = core::ptr::null_mut();
    struct bpf_prog *prog;
    rcu_read_lock();
...
// type  = BPF_SYM_ELF_TYPE;
    ...
    let mut poke_tab_max: static u32 = 1024;
    let mut slot: u32 = prog.aux.size_poke_tab;
    let mut size: u32 = slot + 1;
...
unsafe extern "C" fn bpf_jit_mem_is_rox() -> bool {
    ...
    {
    let mut nbits: c_uint = BPF_PROG_SIZE_TO_NBITS(size);
...
pub unsafe extern "C" fn bpf_prog_pack_free(ptr: *mut c_void, size: u32) {
    ...
    {
    struct bpf_prog_pack *pack = core::ptr::null_mut(), *tmp;
    unsigned int nbits;
    unsigned long pos;
    mutex_lock(&pack_mutex);
    if (size > BPF_PROG_PACK_SIZE) {
...
#[no_mangle]
unsafe extern "C" fn bpf_jit_charge_init() -> int __init {
...
//
    ...
    struct bpf_binary_header *rw_header)
    {
    let mut size: u32 = ro_header.size;
    bpf_prog_pack_free(ro_header, size);
    kvfree(rw_header);
    bpf_jit_uncharge_modmem(size);
    }
    ...
    {
    let mut real_start: c_ulong = (unsigned long)fp.bpf_func;
    unsigned long addr;
...
// func_addr = (unsigned long)addr;
    ...
    let mut imm_rnd: u32 = get_random_u32();
    s16 off;
...
// to++ = (struct bpf_insn) {
    ...
    gfp_t gfp_extra_flags)
    {
    let mut gfp_flags: gfp_t = GFP_KERNEL | __GFP_ZERO | gfp_extra_flags;
    struct bpf_prog *fp;
...
//
    ...
    (u32) SRC);
#[no_mangle]
pub unsafe extern "C" fn if(BPF_DW: BPF_SIZE(insn->code) ==) -> else {
    else if (BPF_SIZE(insn.code) == BPF_DW)
...
//
    ...
    {
    let mut prog_type: enum bpf_prog_type = resolve_prog_type(fp);
...
unsafe extern "C" fn bpf_prog_select_interpreter(fp: *mut bpf_prog) -> bool {
    ...
    {
    let mut select_interpreter: bool = false;

    let mut stack_depth: u32 = max_t(u32, fp.aux.stack_depth, 1);
    let mut idx: u32 = (round_up(stack_depth, 32) / 32) - 1;
...
//
    let mut jit_needed: bool = fp.jit_required;
    if (fp.bpf_func)
    goto finalize;
...
//
    ...
    let mut found_exclude: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn bpf_helper_changes_pkt_data(func_id: enum bpf_func_id) -> bool __weak {
...
#[no_mangle]
pub unsafe extern "C" fn bpf_arch_text_copy(dst: *mut c_void, src: *mut c_void, len: usize) -> *mut void  __weak {
...
pub unsafe extern "C" fn bpf_check_timed_may_goto(p: *mut bpf_timed_may_goto) -> u64 {
    ...
    {
    let mut time: u64 = ktime_get_mono_fast_ns();
...
#[no_mangle]
unsafe extern "C" fn bpf_global_ma_init() -> int __init {
...
//
    ...
    {
    struct bpf_linfo_source src;
    let mut idx: c_int = -1, insn_start, insn_end, len;
    struct bpf_line_info *linfo;
    void **jited_linfo;
    struct btf *btf;
    int nr_linfo;
