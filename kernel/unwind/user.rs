...
#[no_mangle]
pub unsafe extern "C" fn get_user_word(word: *mut c_ulong, base: c_ulong, off: c_int, ws: c_uint) -> c_int {
    }
...
#[no_mangle]
pub unsafe extern "C" fn unwind_user_next_common(state: *mut unwind_user_state, frame: *mut unwind_user_frame) -> c_int {
...
pub static mut unwind_user_frame: usize = 0;
...
pub static mut iter_mask: c_ulong = 0;
    let mut bit = 0;
    if (state.done) {
    return -EINVAL;
    }
    for_each_set_bit(bit, &iter_mask, NR_UNWIND_USER_TYPE_BITS) {
pub static mut type: unwind_user_type = 0;
