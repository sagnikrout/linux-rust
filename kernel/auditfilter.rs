...
#[no_mangle]
pub unsafe extern "C" fn audit_register_class(class: c_int, list: *mut c_uint) -> int __init {
...
// Common user-space to kernel rule translation.
    ...
    let mut bit: c_int = AUDIT_BITMASK_SIZE * 32 - i - 1;
...
// Translate struct audit_rule_data to kernel's rule representation.
    ...
    struct audit_entry *entry;
    void *bufp;
    let mut remain: usize = datasz - sizeof(struct audit_rule_data);
    int i;
    char *str;
...
// the initial copy.
    ...
    {
    let mut fcount: u32 = old.field_count;
    struct audit_entry *entry;
...
// p = list = &audit_filter_list[entry->rule.listnr];
    ...
    let mut prio_low: static u64 = ~0ULL/2;
    let mut prio_high: static u64 = ~0ULL/2 - 1;
...
// audit_filter_mutex synchronizes the writers
    ...
    let mut res: c_int = update_lsm_rule(r);
    if (!err)
    err = res;
    }
    }
