...
pub struct freezer {
    ...
    {
    return css ? container_of(css, struct freezer, css) : core::ptr::null_mut();
    }
...
// also synchronizes against task migration, see freezer_attach()
    ...
    let mut was_freezing: bool = freezer.state & CGROUP_FREEZING;
    freezer.state &= ~state;
    if (!(freezer.state & CGROUP_FREEZING)) {
    freezer.state &= ~CGROUP_FROZEN;
    if (was_freezing)
...
//
    ...
    freeze = false;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(buf, 0: freezer_state_strs(CGROUP_FROZEN)) ==) -> else {
