...
// Check if max_ranges can even hold initial_ranges
    ...
    alloc_size = sizeof!(crash_mem) + (size_t)max_ranges * sizeof!(range);
    ...
    nr_initial_ranges * sizeof!(range));
    }
    return mem;
    }
...
unsafe extern "C" fn exclude_range_regression_test(test: *mut kunit) {
    let mut i = 0;
    while (i < ARRAY_SIZE!(exclude_range_regression_test_data)) {
