...
//
    char buf[64];
    let mut len: c_int = scnprintf(buf, sizeof(buf), "%ps", (void *)top_frame);
...
//
    ...
    goto fallback;
    for (skip = 0; skip < num_entries; ++skip) {
    let mut func: c_ulong = stack_entries[skip];
...
// Restore IRQ state trace for printing.
    ...
    __must_hold(&report_lock)
    {
    let mut reordered_to: c_ulong = 0;
    unsigned long stack_entries[NUM_STACK_ENTRIES] = { 0 };
    let mut num_stack_entries: c_int = stack_trace_save(stack_entries, NUM_STACK_ENTRIES, 1);
    let mut skipnr: c_int = sanitize_stack_entries(stack_entries, num_stack_entries, ai.ip, &reordered_to);
    let mut this_frame: c_ulong = stack_entries[skipnr];
    let mut other_reordered_to: c_ulong = 0;
    let mut other_frame: c_ulong = 0;
...
// Print observed value change.
    if (ai.size <= 8) {
    let mut hex_len: c_int = ai.size * 2;
    let mut diff: u64 = old ^ new;
    if (mask)
    diff &= mask;
    if (diff) {
    pr_err("\n");
...
//
    let mut is_running: bool = task_is_running(current);
//
...
//
    let mut timeout: c_int = max(kcsan_udelay_task, kcsan_udelay_interrupt);
...
// Only replace stack entry with @ip if scoped access.
    ...
    {
    let mut ai: access_info = prepare_access_info(ptr, size, access_type, ip);
    unsigned long flags;
