...
// Macro flag: #define CREATE_TRACE_POINTS
    ...
    let mut dma_default_coherent: bool = IS_ENABLED(CONFIG_ARCH_DMA_DEFAULT_COHERENT);
...
//
    ...
    {
    let mut match_data: dma_devres = { size, vaddr, dma_handle };
...
//
    ...
    let mut is_mmio: bool = attrs & DMA_ATTR_MMIO;
    let mut is_cc_shared: bool = attrs & DMA_ATTR_CC_SHARED;
    let mut addr: dma_addr_t = DMA_MAPPING_ERROR;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_cc_shared) -> else {
    else if (is_cc_shared)
    return DMA_MAPPING_ERROR;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: use_dma_iommu(dev)) -> else {
    else if (use_dma_iommu(dev))
    addr = iommu_dma_map_phys(dev, phys, size, dir, attrs);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->map_phys) -> else {
    ...
    unsigned long attrs)
    {
    let mut phys: phys_addr_t = page_to_phys(page) + offset;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->unmap_sg) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->sync_single_for_cpu) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->sync_single_for_device) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->sync_sg_for_cpu) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->sync_sg_for_device) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(DMA_ATTR_WRITE_COMBINE: attrs &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->free) -> else {
    else if (ops.free)
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ops->free_pages) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(ops->max_mapping_size: ops &&) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(ops->opt_mapping_size: ops &&) -> else {
