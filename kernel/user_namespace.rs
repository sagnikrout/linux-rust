...
unsafe extern "C" fn enforced_nproc_rlimit() -> c_ulong {
    ...
    {
    let mut limit: c_ulong = RLIM_INFINITY;
...
pub unsafe extern "C" fn create_user_ns(new: *mut cred) -> c_int {
    ...
    let mut owner: kuid_t = new.euid;
    let mut group: kgid_t = new.egid;
...
unsafe extern "C" fn map_id_range_down(map: *mut uid_gid_map, id: u32, count: u32) -> u32 {
    ...
    let mut extents: unsigned = map.nr_extents;
    smp_rmb();
...
//
    ...
    struct uid_gid_extent *extent0 = core::ptr::null_mut();
    for (idx = 0; idx < new_map.nr_extents; idx++) {
    if (new_map.nr_extents <= UID_GID_MAP_MAX_BASE_EXTENTS)
...
//
    ...
    let mut id: u32 = new_map.extent[0].lower_first;
    if (cap_setid == CAP_SETUID) {
    let mut uid: kuid_t = make_kuid(ns.parent, id);
    if (uid_eq(uid, cred.euid))
    return true;
    } else if (cap_setid == CAP_SETGID) {
    let mut gid: kgid_t = make_kgid(ns.parent, id);
...
pub unsafe extern "C" fn proc_setgroups_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    {
    struct user_namespace *ns = seq.private;
    let mut userns_flags: c_ulong = READ_ONCE(ns.flags);
    seq_printf(seq, "%s\n",
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(pos, _arg: "deny", 0: 4) ==) -> else {
    pos += 4;
