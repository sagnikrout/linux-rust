...
//! Source: drivers/tty/serial/suncore.c
#![no_std]
...
//
    ...
    let mut sunserial_current_minor: static int = 64;
#[no_mangle]
pub unsafe extern "C" fn sunserial_register_minors(drv: *mut uart_driver, count: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sunserial_unregister_minors(drv: *mut uart_driver, count: c_int) {
...
// of_console_options == 'b')
    ...
    con.index = line;
    add_preferred_console(con.name, line, core::ptr::null_mut());
    }
    return 1;
    }
    EXPORT_SYMBOL(sunserial_console_match);
#[no_mangle]
pub unsafe extern "C" fn sunserial_console_termios(con: *mut console, uart_dp: *mut device_node) {
    ...
    "ssp-console-modes", core::ptr::null_mut());
    if (!mode)
    mode = "115200,8,n,1,-";
    ...
    mode = of_get_property(dp, mode_prop, core::ptr::null_mut());
    if (!mode)
    mode = "9600,8,n,1,-";
    of_node_put(dp);
    }
    ...
    s = mode;
    baud = simple_strtoul(s, core::ptr::null_mut(), 0);
    s = strchr(s, ',');
    bits = simple_strtoul(++s, core::ptr::null_mut(), 0);
    s = strchr(s, ',');
    parity = *(++s);
    s = strchr(s, ',');
    stop = simple_strtoul(++s, core::ptr::null_mut(), 0);
    s = strchr(s, ',');
...
#[no_mangle]
pub unsafe extern "C" fn suncore_mouse_baud_cflag_next(cflag: c_uint, new_baud: *mut c_int) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn suncore_mouse_baud_detection(ch: c_uchar, is_break: c_int) -> c_int {
    int suncore_mouse_baud_detection(unsigned char ch, int is_break)
    {
    let mut mouse_got_break: static int = 0;
    let mut ctr: static int = 0;
    if (is_break) {
...
#[no_mangle]
unsafe extern "C" fn suncore_init() -> int __init {
