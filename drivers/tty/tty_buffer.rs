...
//! Source: drivers/tty/tty_buffer.c
#![no_std]
...
pub const MIN_TTYB_SIZE: c_int = 256;
pub const TTYB_ALIGN_MASK: c_uint = 0xff;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_lock_exclusive(port: *mut tty_port) {
...
#[no_mangle]
unsafe extern "C" fn tty_buffer_queue_work(buf: *mut tty_bufhead) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_unlock_exclusive(port: *mut tty_port) {
    void tty_buffer_unlock_exclusive(struct tty_port *port)
    {
    struct tty_bufhead *buf = &port.buf;
    let mut restart: bool = buf.head.commit != buf.head.read;
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_space_avail(port: *mut tty_port) -> c_uint {
    unsigned int tty_buffer_space_avail(struct tty_port *port)
    {
    let mut space: c_int = port.buf.mem_limit - atomic_read(&port.buf.mem_used);
    return max(space, 0);
    }
    EXPORT_SYMBOL_GPL(tty_buffer_space_avail);
#[no_mangle]
unsafe extern "C" fn tty_buffer_reset(p: *mut tty_buffer, size: usize) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_free_all(port: *mut tty_port) {
    ...
    int still_used;
    while ((p = buf.head) != core::ptr::null_mut()) {
    buf.head = p.next;
    freed += p.size;
    if (p.size > 0)
    kfree(p);
...
//
#[no_mangle]
unsafe extern "C" fn tty_buffer_free(port: *mut tty_port, b: *mut tty_buffer) {
...
// Dumb strategy for now - should keep some stats
    ...
    kfree(b);
#[no_mangle]
pub unsafe extern "C" fn if(0: b->size >) -> else {
    else if (b.size > 0)
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_flush(tty: *mut tty_struct, ld: *mut tty_ldisc) {
...
//
    while ((next = smp_load_acquire(&buf.head.next)) != core::ptr::null_mut()) {
...
//
    ...
    let mut left: usize = (b.flags ? 1 : 2) * b.size - b.used;
    let mut change: bool = !b.flags && flags;
    if (!change && left >= size)
    return size;
...
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_request_room(port: *mut tty_port, size: usize) -> c_int {
    ...
    size_t size)
    {
    let mut need_flags: bool = mutable_flags || flags[0] != TTY_NORMAL;
    let mut copied: usize = 0;
    do {
    let mut goal: usize = min_t(size_t, size - copied, TTY_BUFFER_PAGE);
    let mut space: usize = __tty_buffer_request_room(port, goal, need_flags);
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_prepare_flip_string(port: *mut tty_port, chars: *mut u8, size: usize) -> usize {
    size_t tty_prepare_flip_string(struct tty_port *port, u8 **chars, size_t size)
    {
    let mut space: usize = __tty_buffer_request_room(port, size, false);
    if (likely(space)) {
    struct tty_buffer *tb = port.buf.tail;
...
#[no_mangle]
unsafe extern "C" fn lookahead_bufs(port: *mut tty_port, head: *mut tty_buffer) {
...
//
#[no_mangle]
unsafe extern "C" fn flush_to_ldisc(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn tty_flip_buffer_commit(tail: *mut tty_buffer) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_flip_buffer_push(port: *mut tty_port) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_init(port: *mut tty_port) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_set_limit(port: *mut tty_port, limit: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_set_lock_subclass(port: *mut tty_port) {
    void tty_buffer_set_lock_subclass(struct tty_port *port)
    {
    lockdep_set_subclass(&port.buf.lock, TTY_LOCK_SLAVE);
    }
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_restart_work(port: *mut tty_port) -> bool {
    bool tty_buffer_restart_work(struct tty_port *port)
    {
    return tty_buffer_queue_work(&port.buf);
    }
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_cancel_work(port: *mut tty_port) -> bool {
    bool tty_buffer_cancel_work(struct tty_port *port)
    {
    return cancel_work_sync(&port.buf.work);
    }
#[no_mangle]
pub unsafe extern "C" fn tty_buffer_flush_work(port: *mut tty_port) {
