...
pub const MAX_NR_CON_DRIVER: c_int = 16;
pub const CON_DRIVER_FLAG_MODULE: c_int = 1;
pub const CON_DRIVER_FLAG_INIT: c_int = 2;
pub const CON_DRIVER_FLAG_ATTR: c_int = 4;
pub const CON_DRIVER_FLAG_ZOMBIE: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct con_driver {
    pub con: *const consw,
...
//
pub const DEFAULT_BELL_PITCH: c_int = 750;

pub const DEFAULT_CURSOR_BLINK_MS: c_int = 200;
    ...
    let mut default_utf8: c_int = true;
    module_param(default_utf8, int, S_IRUGO | S_IWUSR);
    let mut global_cursor_default: c_int = -1;
    module_param(global_cursor_default, int, S_IRUGO | S_IWUSR);
    EXPORT_SYMBOL(global_cursor_default);
    let mut cur_default: static int = CUR_UNDERLINE;
...
//
#[no_mangle]
pub unsafe extern "C" fn vc_font_pitch(font: *const vc_font) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn vc_font_size(font: *const vc_font) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn register_vt_notifier(nb: *mut notifier_block) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn unregister_vt_notifier(nb: *mut notifier_block) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn notify_write(vc: *mut vc_data, unicode: c_uint) {
    static void notify_write(struct vc_data *vc, unsigned int unicode)
    {
    let mut param: vt_notifier_param = { .vc = vc, .c = unicode };
    atomic_notifier_call_chain(&vt_notifier_list, VT_WRITE, &param);
    }
#[no_mangle]
unsafe extern "C" fn notify_update(vc: *mut vc_data) {
    static void notify_update(struct vc_data *vc)
    {
    let mut param: vt_notifier_param = { .vc = vc };
...
//
#[no_mangle]
pub unsafe extern "C" fn con_is_fg(vc: *const vc_data) -> bool {
    static inline bool con_is_fg(const struct vc_data *vc)
    {
    return vc.vc_num == fg_console;
    }
#[no_mangle]
pub unsafe extern "C" fn con_should_update(vc: *const vc_data) -> bool {
    ...
    bool viewed)
    {
    let mut origin: c_ulong = viewed ? vc.vc_visible_origin : vc.vc_origin;
    return (u16 *)(origin + offset);
    }
#[no_mangle]
unsafe extern "C" fn con_putc(vc: *mut vc_data, ca: u16, y: c_uint, x: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn scrolldelta(lines: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn schedule_console_callback() {
...
#[no_mangle]
unsafe extern "C" fn vc_uniscr_free(uni_lines: *mut u32) {
    static void vc_uniscr_free(u32 **uni_lines)
    {
    vfree(uni_lines);
    }
#[no_mangle]
unsafe extern "C" fn vc_uniscr_set(vc: *mut vc_data, new_uni_lines: *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn vc_uniscr_putc(vc: *mut vc_data, uc: u32) {
...
#[no_mangle]
unsafe extern "C" fn vc_uniscr_insert(vc: *mut vc_data, nr: c_uint) {
    static void vc_uniscr_insert(struct vc_data *vc, unsigned int nr)
    {
    if (vc.vc_uni_lines) {
    u32 *ln = vc.vc_uni_lines[vc.state.y];
    let mut x: c_uint = vc.state.x, cols = vc.vc_cols;
    memmove(&ln[x + nr], &ln[x], (cols - x - nr) * sizeof(*ln));
    memset32(&ln[x], ' ', nr);
    }
    }
#[no_mangle]
unsafe extern "C" fn vc_uniscr_delete(vc: *mut vc_data, nr: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn juggle_array(array: *mut u32, size: c_uint, nr: c_uint) {
    ...
    let mut dst_idx: c_uint = gcd_idx;
    while (1) {
    let mut src_idx: c_uint = (dst_idx + nr) % size;
    if (src_idx == gcd_idx)
    break;
    ...
    let mut size: c_uint = bottom - top;
    if (!uni_lines)
    return;
    if (dir == SM_DOWN) {
...
#[no_mangle]
unsafe extern "C" fn vc_uniscr_getc(vc: *mut vc_data, relative_pos: c_int) -> u32 {
    static u32 vc_uniscr_getc(struct vc_data *vc, int relative_pos)
    {
    let mut pos: c_int = vc.state.x + vc.vc_need_wrap + relative_pos;
...
//
#[no_mangle]
pub unsafe extern "C" fn vc_uniscr_check(vc: *mut vc_data) -> c_int {
...
//
    ...
    let mut glyph: u16 = scr_readw(p++) & mask;
...
//
    ...
    {
    u32 **uni_lines = vc.vc_uni_lines;
    let mut offset: c_int = row * vc.vc_size_row + col * 2;
    unsigned long pos;
...
//
    u16 *p = (u16 *)pos;
    let mut mask: c_int = vc.vc_hi_font_mask | 0xff;
    u32 *uni_buf = dest;
    while (nr--) {
    let mut glyph: u16 = scr_readw(p++) & mask;
// uni_buf++ = inverse_translate(vc, glyph, true);
    ...
    unsigned int nr)
    {
    let mut rows: c_uint = bottom - top;
    u16 *clear, *dst, *src;
    if (top + nr >= bottom)
    nr = rows - 1;
...
#[no_mangle]
unsafe extern "C" fn do_update_region(vc: *mut vc_data, start: c_ulong, count: c_int) {
    ...
    for(;;) {
    let mut attrib: u16 = scr_readw(p) & 0xff00;
    let mut startx: c_int = xx;
    u16 *q = p;
...
#[no_mangle]
pub unsafe extern "C" fn update_region(vc: *mut vc_data, start: c_ulong, count: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: _underline) -> else {
    else if (_underline)
    a = (a & 0xf0) | vc.vc_ulcolor;
#[no_mangle]
pub unsafe extern "C" fn if(VCI_HALF_BRIGHT: _intensity ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn update_attr(vc: *mut vc_data) {
...
#[no_mangle]
pub unsafe extern "C" fn invert_screen(vc: *mut vc_data, offset: c_int, count: c_int, viewed: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn complement_pos(vc: *mut vc_data, offset: c_int) {
    void complement_pos(struct vc_data *vc, int offset)
    {
    let mut old_offset: static int = -1;
...
#[no_mangle]
unsafe extern "C" fn insert_char(vc: *mut vc_data, nr: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn delete_char(vc: *mut vc_data, nr: c_uint) {
    ...
    let mut softcursor_original: static int = -1;
#[no_mangle]
unsafe extern "C" fn add_softcursor(vc: *mut vc_data) {
    static void add_softcursor(struct vc_data *vc)
    {
    let mut i: c_int = scr_readw((u16 *) vc.vc_pos);
    let mut type: u32 = vc.vc_cursor_type;
    if (!(type & CUR_SW))
    return;
...
#[no_mangle]
unsafe extern "C" fn hide_softcursor(vc: *mut vc_data) {
...
#[no_mangle]
unsafe extern "C" fn hide_cursor(vc: *mut vc_data) {
    static void hide_cursor(struct vc_data *vc)
    {
    if (vc_is_sel(vc))
    clear_selection();
...
#[no_mangle]
unsafe extern "C" fn set_cursor(vc: *mut vc_data) {
    static void set_cursor(struct vc_data *vc)
    {
...
#[no_mangle]
unsafe extern "C" fn set_origin(vc: *mut vc_data) {
...
#[no_mangle]
unsafe extern "C" fn save_screen(vc: *mut vc_data) {
...
#[no_mangle]
unsafe extern "C" fn flush_scrollback(vc: *mut vc_data) {
...
//
#[no_mangle]
pub unsafe extern "C" fn clear_buffer_attributes(vc: *mut vc_data) {
    void clear_buffer_attributes(struct vc_data *vc)
    {
    unsigned short *p = (unsigned short *)vc.vc_origin;
    let mut count: c_int = vc.vc_screenbuf_size / 2;
    let mut mask: c_int = vc.vc_hi_font_mask | 0xff;
    for (; count > 0; count--, p++) {
...
#[no_mangle]
pub unsafe extern "C" fn redraw_screen(vc: *mut vc_data, is_switch: c_int) {
    void redraw_screen(struct vc_data *vc, int is_switch)
    {
    let mut redraw: c_int = 0;
    WARN_CONSOLE_UNLOCKED();
    if (!vc) {
// strange ...
...
// vc->vc_display_fg = vc;
    ...
    if (tty0dev)
    sysfs_notify(&tty0dev.kobj, core::ptr::null_mut(), "active");
    } else {
    hide_cursor(vc);
    redraw = 1;
    }
    if (redraw) {
    bool update;
    let mut old_was_color: c_int = vc.vc_can_do_color;
    set_origin(vc);
...
//
#[no_mangle]
pub unsafe extern "C" fn vc_cons_allocated(i: c_uint) -> c_int {
    int vc_cons_allocated(unsigned int i)
    {
    return (i < MAX_NR_CONSOLES && vc_cons[i].d);
    }
#[no_mangle]
unsafe extern "C" fn visual_init(vc: *mut vc_data, num: c_int, init: bool) {
...
// ++Geert: vc->vc_sw->con_init determines console size
    ...
    vc.uni_pagedict = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn visual_deinit(vc: *mut vc_data) {
...
#[no_mangle]
unsafe extern "C" fn vc_port_destruct(port: *mut tty_port) {
...
// If no drivers have overridden us and the user didn't pass a
    ...
    visual_deinit(vc);
    kfree(vc);
    vc_cons[currcons].d = core::ptr::null_mut();
    return err;
    }
...
//
    ...
    u32 **new_uniscr = core::ptr::null_mut();
    WARN_CONSOLE_UNLOCKED();
...
//
#[no_mangle]
unsafe extern "C" fn vt_resize(tty: *mut tty_struct, ws: *mut winsize) -> c_int {
    ...
    {
    struct vc_data *vc = core::ptr::null_mut();
    WARN_CONSOLE_UNLOCKED();
    ...
    con_free_unimap(vc);
    put_pid(vc.vt_pid);
    vc_uniscr_set(vc, core::ptr::null_mut());
    kfree(vc.vc_screenbuf);
    vc_cons[currcons].d = core::ptr::null_mut();
    if (vc.vc_saved_screen != core::ptr::null_mut()) {
    kfree(vc.vc_saved_screen);
    vc.vc_saved_screen = core::ptr::null_mut();
    }
    vc_uniscr_free(vc.vc_saved_uni_lines);
    vc.vc_saved_uni_lines = core::ptr::null_mut();
    }
    return vc;
    }
//
...
// the default colour table, for VGA+ colour systems
    ...
    module_param_array(default_red, byte, core::ptr::null_mut(), S_IRUGO | S_IWUSR);
    EXPORT_SYMBOL(default_red);
    unsigned char default_grn[] = {
    0x00, 0x00, 0xaa, 0x55, 0x00, 0x00, 0xaa, 0xaa,
    0x55, 0x55, 0xff, 0xff, 0x55, 0x55, 0xff, 0xff
    };
    module_param_array(default_grn, byte, core::ptr::null_mut(), S_IRUGO | S_IWUSR);
    EXPORT_SYMBOL(default_grn);
    unsigned char default_blu[] = {
    0x00, 0x00, 0x00, 0x00, 0xaa, 0xaa, 0xaa, 0xaa,
    0x55, 0x55, 0x55, 0x55, 0xff, 0xff, 0xff, 0xff
    };
    module_param_array(default_blu, byte, core::ptr::null_mut(), S_IRUGO | S_IWUSR);
...
//
#[no_mangle]
unsafe extern "C" fn gotoxy(vc: *mut vc_data, new_x: c_int, new_y: c_int) {
    static void gotoxy(struct vc_data *vc, int new_x, int new_y)
    {
    int min_y, max_y;
    if (new_x < 0)
    ...
    vc.state.y = min_y;
#[no_mangle]
pub unsafe extern "C" fn if(max_y: new_y >=) -> else {
    else if (new_y >= max_y)
    vc.state.y = max_y - 1;
    else
    vc.state.y = new_y;
...
#[no_mangle]
unsafe extern "C" fn gotoxay(vc: *mut vc_data, new_x: c_int, new_y: c_int) {
    static void gotoxay(struct vc_data *vc, int new_x, int new_y)
    {
    gotoxy(vc, new_x, vc.vc_decom ? (vc.vc_top + new_y) : new_y);
    }
#[no_mangle]
pub unsafe extern "C" fn scrollback(vc: *mut vc_data) {
    void scrollback(struct vc_data *vc)
    {
    scrolldelta(-(vc.vc_rows / 2));
    }
#[no_mangle]
pub unsafe extern "C" fn scrollfront(vc: *mut vc_data, lines: c_int) {
...
#[no_mangle]
unsafe extern "C" fn lf(vc: *mut vc_data) {
...
#[no_mangle]
pub unsafe extern "C" fn if(1: vc->state.y < vc->vc_rows -) -> else {
    vc.state.y++;
...
#[no_mangle]
unsafe extern "C" fn ri(vc: *mut vc_data) {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: vc->state.y >) -> else {
    vc.state.y--;
    vc.vc_pos -= vc.vc_size_row;
    }
    vc.vc_need_wrap = 0;
...
#[no_mangle]
pub unsafe extern "C" fn cr(vc: *mut vc_data) {
...
#[no_mangle]
pub unsafe extern "C" fn bs(vc: *mut vc_data) {
...
#[no_mangle]
pub unsafe extern "C" fn del(vc: *mut vc_data) {
...
#[no_mangle]
unsafe extern "C" fn csi_J(vc: *mut vc_data, vpar: enum CSI_J) {
...
#[no_mangle]
unsafe extern "C" fn csi_K(vc: *mut vc_data) {
...
#[no_mangle]
unsafe extern "C" fn csi_X(vc: *mut vc_data) {
    static void csi_X(struct vc_data *vc)
    {					  /* not vt100? */
    let mut count: c_uint = clamp(vc.vc_par[0], 1, vc.vc_cols - vc.state.x);
...
#[no_mangle]
unsafe extern "C" fn default_attr(vc: *mut vc_data) {
...
pub struct rgb {
#[no_mangle]
unsafe extern "C" fn rgb_from_256(i: c_uint, c: *mut rgb) {
    static void rgb_from_256(unsigned int i, struct rgb *c)
    {
    if (i < 8) {            /* Standard colours. */
    pub 0x00: c->r = i&1 ? 0xaa :,
    pub 0x00: c->g = i&2 ? 0xaa :,
    pub 0x00: c->b = i&4 ? 0xaa :,
    } else if (i < 16) {
    pub 0x55: c->r = i&1 ? 0xff :,
    pub 0x55: c->g = i&2 ? 0xff :,
    pub 0x55: c->b = i&4 ? 0xff :,
    } else if (i < 232) {   /* 6x6x6 colour cube. */
    pub 16: i -=,
    pub 6: *mut *mut c->b = i % 6  255 /,
    pub 6: i /=,
    pub 6: *mut *mut c->g = i % 6  255 /,
    pub 6: i /=,
    pub 6: *mut *mut c->r = i  255 /,
    } else                  /* Grayscale ramp. */
    pub 2312: *mut *mut c->r = c->g = c->b = i  10 -,
    }
#[no_mangle]
unsafe extern "C" fn rgb_foreground(vc: *mut vc_data, c: *const rgb) {
    static void rgb_foreground(struct vc_data *vc, const struct rgb *c)
    {
    pub c->b): u8 hue = 0, max = max3(c->r, c->g,,
    if (c.r > max / 2)
    pub 4: hue |=,
    if (c.g > max / 2)
    pub 2: hue |=,
    ...
    if (hue == 7 && max <= 0x55) {
    pub 0: hue =,
    pub VCI_BOLD: vc->state.intensity =,
    } else if (max > 0xaa)
    pub VCI_BOLD: vc->state.intensity =,
    else
    pub VCI_NORMAL: vc->state.intensity =,
    pub hue: vc->state.color = (vc->state.color & 0xf0) |,
    }
#[no_mangle]
unsafe extern "C" fn rgb_background(vc: *mut vc_data, c: *const rgb) {
    static void rgb_background(struct vc_data *vc, const struct rgb *c)
    {
// For backgrounds, err on the dark side.
    vc.state.color = (vc.state.color & 0x0f)
    pub 3: | (c->r&0x80) >> 1 | (c->g&0x80) >> 2 | (c->b&0x80) >>,
    }
//
...
// 256 colours
    pub &c): rgb_from_256(vc->vc_par[i],,
    } else if (vc.vc_par[i] == 2 && i + 3 <= vc.vc_npar) {
// 24 bit
    pub 1]: c.r = vc->vc_par[i +,
    pub 2]: c.g = vc->vc_par[i +,
    pub 3]: c.b = vc->vc_par[i +,
    pub 3: i +=,
    } else
    pub i: return,
    pub &c): set_color(vc,,
...
#[no_mangle]
unsafe extern "C" fn csi_m(vc: *mut vc_data) {
...
#[no_mangle]
unsafe extern "C" fn respond_string(p: *const c_char, len: usize, port: *mut tty_port) {
...
#[no_mangle]
unsafe extern "C" fn cursor_report(vc: *mut vc_data, tty: *mut tty_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn status_report(tty: *mut tty_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn respond_ID(tty: *mut tty_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn mouse_report(tty: *mut tty_struct, butt: c_int, mrx: c_int, mry: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn mouse_reporting() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_bracketed_paste(tty: *mut tty_struct) -> c_int {
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn enter_alt_screen(vc: *mut vc_data) {
    static void enter_alt_screen(struct vc_data *vc)
    {
    let mut size: c_uint = vc.vc_rows * vc.vc_cols * 2;
    if (vc.vc_saved_screen != core::ptr::null_mut())
    ...
    vc.vc_uni_lines = core::ptr::null_mut();
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn leave_alt_screen(vc: *mut vc_data) {
    static void leave_alt_screen(struct vc_data *vc)
    {
    let mut rows: c_uint = min(vc.vc_saved_rows, vc.vc_rows);
    let mut cols: c_uint = min(vc.vc_saved_cols, vc.vc_cols);
    u16 *src, *dest;
    if (vc.vc_saved_screen == core::ptr::null_mut())
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn csi_DEC_hl(vc: *mut vc_data, on_off: bool) {
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn csi_hl(vc: *mut vc_data, on_off: bool) {
...
//
#[no_mangle]
unsafe extern "C" fn csi_RSB(vc: *mut vc_data) {
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn csi_at(vc: *mut vc_data, nr: c_uint) {
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn csi_L(vc: *mut vc_data) {
    static void csi_L(struct vc_data *vc)
    {
    let mut nr: c_uint = clamp(vc.vc_par[0], 1, vc.vc_rows - vc.state.y);
    con_scroll(vc, vc.state.y, vc.vc_bottom, SM_DOWN, nr);
    vc.vc_need_wrap = 0;
    }
// console_lock is held
#[no_mangle]
unsafe extern "C" fn csi_P(vc: *mut vc_data) {
    static void csi_P(struct vc_data *vc)
    {
    let mut nr: c_uint = clamp(vc.vc_par[0], 1, vc.vc_cols - vc.state.x);
    delete_char(vc, nr);
    }
// console_lock is held
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn save_cur(vc: *mut vc_data) {
    static void save_cur(struct vc_data *vc)
    {
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn restore_cur(vc: *mut vc_data) {
    static void restore_cur(struct vc_data *vc)
    {
...
#[no_mangle]
unsafe extern "C" fn reset_terminal(vc: *mut vc_data, do_clear: c_int) {
    static void reset_terminal(struct vc_data *vc, int do_clear)
    {
    unsigned int i;
    vc.vc_top		= 0;
...
#[no_mangle]
unsafe extern "C" fn vc_setGx(vc: *mut vc_data, which: c_uint, c: u8) {
...
#[no_mangle]
unsafe extern "C" fn ansi_control_string(state: enum vc_ctl_state) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn handle_ascii(tty: *mut tty_struct, vc: *mut vc_data, c: u8) -> bool {
    ...
    vc.vc_state = ESnormal;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: vc->vc_bell_duration) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn handle_esc(tty: *mut tty_struct, vc: *mut vc_data, c: u8) {
...
//
#[no_mangle]
unsafe extern "C" fn csi_DEC(tty: *mut tty_struct, vc: *mut vc_data, c: u8) {
    ...
    status_report(tty);
#[no_mangle]
pub unsafe extern "C" fn if(6: vc->vc_par[0] ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn csi_ECMA(tty: *mut tty_struct, vc: *mut vc_data, c: u8) {
    ...
    set_bit(vc.state.x, vc.vc_tab_stop);
#[no_mangle]
pub unsafe extern "C" fn if(3: vc->vc_par[0] ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn vc_reset_params(vc: *mut vc_data) {
...
// console_lock is held
#[no_mangle]
unsafe extern "C" fn do_con_trol(tty: *mut tty_struct, vc: *mut vc_data, c: u8) {
...
//
    ...
    if (vc.vc_npar == 7) {
    let mut i: c_int = vc.vc_par[0] * 3, j = 1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vc_draw_region {
    pub to: unsigned long from,,
    pub x: c_int,
}

#[no_mangle]
unsafe extern "C" fn con_flush(vc: *mut vc_data, draw: *mut vc_draw_region) {
...
#[no_mangle]
pub unsafe extern "C" fn vc_translate_ascii(vc: *const vc_data, c: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn vc_sanitize_unicode(c: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vc_translate_unicode(vc: *mut vc_data, c: c_int, rescan: *mut bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vc_translate(vc: *mut vc_data, c: *mut c_int, rescan: *mut bool) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn vc_invert_attr(vc: *const vc_data) -> c_uchar {
...
#[no_mangle]
unsafe extern "C" fn vc_is_control(vc: *mut vc_data, tc: c_int, c: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn vc_con_rewind(vc: *mut vc_data) {
...
pub const UCS_ZWS: c_uint = 0x200b	/* Zero Width Space */;
pub const UCS_VS16: c_uint = 0xfe0f	/* Variation Selector 16 */;
pub const UCS_REPLACEMENT: c_uint = 0xfffd	/* Replacement Character */;
#[no_mangle]
unsafe extern "C" fn vc_process_ucs(vc: *mut vc_data, c: *mut c_int, tc: *mut c_int) -> c_int {
    static int vc_process_ucs(struct vc_data *vc, int *c, int *tc)
    {
    u32 prev_c, curr_c = *c;
    let mut w: c_uint = ucs_get_width(curr_c);
    if (likely(w == 1))
    return 1;
    if (w == 2) {
//
...
#[no_mangle]
unsafe extern "C" fn vc_get_glyph(vc: *mut vc_data, tc: c_int) -> c_int {
    static int vc_get_glyph(struct vc_data *vc, int tc)
    {
    let mut glyph: c_int = conv_uni_to_pc(vc, tc);
...
// Display U+FFFD (Unicode Replacement Character).
    ...
    {
    int next_c;
    let mut vc_attr: c_uchar = vc.vc_attr;
    let mut himask: u16 = vc.vc_hi_font_mask;
    let mut width: u8 = 1;
...
#[no_mangle]
unsafe extern "C" fn do_con_write(tty: *mut tty_struct, buf: *const u8, count: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn console_callback(ignored: *mut work_struct) {
    static void console_callback(struct work_struct *ignored)
    {
    guard(console_lock)();
    if (want_console >= 0) {
...
#[no_mangle]
pub unsafe extern "C" fn set_console(nr: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn vt_kmsg_redirect(new: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vt_console_print(co: *mut console, b: *const c_char, count: unsigned) {
...
#[no_mangle]
unsafe extern "C" fn vt_console_setup(co: *mut console, options: *mut c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn tioclinux(tty: *mut tty_struct, arg: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn con_write(tty: *mut tty_struct, buf: *const u8, count: usize) -> isize {
...
#[no_mangle]
unsafe extern "C" fn con_put_char(tty: *mut tty_struct, ch: u8) -> c_int {
    static int con_put_char(struct tty_struct *tty, u8 ch)
    {
    return do_con_write(tty, &ch, 1);
    }
#[no_mangle]
unsafe extern "C" fn con_write_room(tty: *mut tty_struct) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn con_throttle(tty: *mut tty_struct) {
    static void con_throttle(struct tty_struct *tty)
    {
    }
#[no_mangle]
unsafe extern "C" fn con_unthrottle(tty: *mut tty_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn con_stop(tty: *mut tty_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn con_start(tty: *mut tty_struct) {
...
#[no_mangle]
unsafe extern "C" fn con_flush_chars(tty: *mut tty_struct) {
    static void con_flush_chars(struct tty_struct *tty)
    {
    struct vc_data *vc = tty.driver_data;
...
//
#[no_mangle]
unsafe extern "C" fn con_install(driver: *mut tty_driver, tty: *mut tty_struct) -> c_int {
    static int con_install(struct tty_driver *driver, struct tty_struct *tty)
    {
    let mut currcons: c_uint = tty.index;
    struct vc_data *vc;
    int ret;
    guard(console_lock)();
...
#[no_mangle]
unsafe extern "C" fn con_open(tty: *mut tty_struct, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn con_close(tty: *mut tty_struct, filp: *mut file) {
    static void con_close(struct tty_struct *tty, struct file *filp)
    {
// Nothing to do - we defer to shutdown
    }
#[no_mangle]
unsafe extern "C" fn con_shutdown(tty: *mut tty_struct) {
    static void con_shutdown(struct tty_struct *tty)
    {
    struct vc_data *vc = tty.driver_data;
    BUG_ON(vc == core::ptr::null_mut());
    guard(console_lock)();
    vc.port.tty = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn con_cleanup(tty: *mut tty_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn con_ldisc_ok(tty: *mut tty_struct, ldisc: c_int) -> c_int {
    static int con_ldisc_ok(struct tty_struct *tty, int ldisc)
    {
    let mut ldisc: return = = N_TTY ? 0 : -EINVAL;
    }
...
#[no_mangle]
unsafe extern "C" fn vc_init(vc: *mut vc_data, do_clear: c_int) {
    static void vc_init(struct vc_data *vc, int do_clear)
    {
    int j, k ;
    set_origin(vc);
...
//
#[no_mangle]
unsafe extern "C" fn con_init() -> int __init {
    static int __init con_init(void)
    {
    const char *display_desc = core::ptr::null_mut();
    struct vc_data *vc;
    let mut currcons: c_uint = 0, i;
    console_lock();
    if (!conswitchp)
    ...
    if (con_driver.con == core::ptr::null_mut()) {
...
// Assuming vc->vc_{cols,rows,screenbuf_size} are sane here.
    ...
    static DEVICE_ATTR(active, S_IRUGO, show_tty_active, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn vty_init(console_fops: *const file_operations) -> int __init {
    ...
    tty0dev = device_create_with_groups(&tty_class, core::ptr::null_mut(),
    MKDEV(TTY_MAJOR, 0), core::ptr::null_mut(),
    vt_dev_groups, "tty0");
    if (IS_ERR(tty0dev))
    tty0dev = core::ptr::null_mut();
    vcs_init();
    ...
    const char *desc = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn do_unbind_con_driver(csw: *const consw, first: c_int, last: c_int, deflt: c_int) -> c_int {
    int do_unbind_con_driver(const struct consw *csw, int first, int last, int deflt)
    {
    struct module *owner = csw.owner;
    const struct consw *defcsw = core::ptr::null_mut();
    struct con_driver *con_driver = core::ptr::null_mut(), *con_back = core::ptr::null_mut();
    int i, retval = -ENODEV;
    if (!try_module_get(owner))
    return -ENODEV;
    WARN_CONSOLE_UNLOCKED();
...
// check if backup driver exists
    ...
    con_driver_map[i] = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn vt_bind(con: *mut con_driver) -> c_int {
    static int vt_bind(struct con_driver *con)
    {
    const struct consw *defcsw = core::ptr::null_mut(), *csw = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn vt_unbind(con: *mut con_driver) -> c_int {
    static int vt_unbind(struct con_driver *con)
    {
    const struct consw *csw = core::ptr::null_mut();
    int i, more = 1, first = -1, last = -1, deflt = 0;
    int ret;
    if (!con.con || !(con.flag & CON_DRIVER_FLAG_MODULE))
    goto err;
...
#[no_mangle]
pub unsafe extern "C" fn vt_bind(con: *mut con_driver) -> c_int {
    static inline int vt_bind(struct con_driver *con)
    {
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn vt_unbind(con: *mut con_driver) -> c_int {
    ...
    let mut bind: c_int = simple_strtoul(buf, core::ptr::null_mut(), 0);
    guard(console_lock)();
    if (bind)
    vt_bind(con);
    else
    ...
    static DEVICE_ATTR(name, S_IRUGO, show_name, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn vtconsole_init_device(con: *mut con_driver) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vtconsole_deinit_device(con: *mut con_driver) {
...
//
#[no_mangle]
pub unsafe extern "C" fn con_is_bound(csw: *const consw) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn con_is_visible(vc: *const vc_data) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn con_debug_enter(vc: *mut vc_data) {
...
//
#[no_mangle]
pub unsafe extern "C" fn con_debug_leave() {
    void con_debug_leave(void)
    { }
    EXPORT_SYMBOL_GPL(con_debug_leave);
#[no_mangle]
unsafe extern "C" fn do_register_con_driver(csw: *const consw, first: c_int, last: c_int) -> c_int {
...
// already registered
    ...
    goto err;
    con_driver.dev =
    device_create_with_groups(&vtconsole_class, core::ptr::null_mut(),
    ...
    con_driver.dev = core::ptr::null_mut();
    } else {
...
//
#[no_mangle]
pub unsafe extern "C" fn do_unregister_con_driver(csw: *const consw) -> c_int {
...
//
    con_driver.con = core::ptr::null_mut();
    con_driver.flag = CON_DRIVER_FLAG_ZOMBIE;
...
#[no_mangle]
unsafe extern "C" fn con_driver_unregister_callback(ignored: *mut work_struct) {
    static void con_driver_unregister_callback(struct work_struct *ignored)
    {
    int i;
    guard(console_lock)();
    ...
    con_driver.con = core::ptr::null_mut();
    con_driver.desc = core::ptr::null_mut();
    con_driver.dev = core::ptr::null_mut();
    con_driver.node = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn do_take_over_console(csw: *const consw, first: c_int, last: c_int, deflt: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn give_up_console(csw: *const consw) {
...
#[no_mangle]
unsafe extern "C" fn vtconsole_class_init() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn set_vesa_blanking(mode_user: *mut u8 __user) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn do_blank_screen(entering_gfx: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn do_unblank_screen(leaving_gfx: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn unblank_screen() {
    static void unblank_screen(void)
    {
    do_unblank_screen(0);
    }
...
//
#[no_mangle]
unsafe extern "C" fn blank_screen_t(unused: *mut timer_list) {
    static void blank_screen_t(struct timer_list *unused)
    {
...
#[no_mangle]
pub unsafe extern "C" fn poke_blanked_console() {
...
//
    ...
    unblank_screen();
#[no_mangle]
pub unsafe extern "C" fn if(_arg: blankinterval) -> else {
    mod_timer(&console_timer, jiffies + (blankinterval * HZ));
    blank_state = blank_normal_wait;
    }
    }
...
//
#[no_mangle]
unsafe extern "C" fn set_palette(vc: *mut vc_data) {
    static void set_palette(struct vc_data *vc)
    {
    WARN_CONSOLE_UNLOCKED();
...
//
#[no_mangle]
pub unsafe extern "C" fn con_set_cmap(arg: *mut unsigned char __user) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn con_get_cmap(arg: *mut unsigned char __user) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn reset_palette(vc: *mut vc_data) {
...
// /Jes
//
pub const max_font_width: c_int = 64;
pub const max_font_height: c_int = 128;
pub const max_font_glyphs: c_int = 512;

#[no_mangle]
unsafe extern "C" fn con_font_get(vc: *mut vc_data, op: *mut console_font_op) -> c_int {
    static int con_font_get(struct vc_data *vc, struct console_font_op *op)
    {
    struct console_font font;
    int c;
    let mut vpitch: c_uint = op.op == KD_FONT_OP_GET_TALL ? op.height : 32;
    if (vpitch > max_font_height)
    return -EINVAL;
    void *font_data __free(kvfree) = core::ptr::null_mut();
    if (op.data) {
    font.data = font_data = kvzalloc(max_font_size, GFP_KERNEL);
    if (!font.data)
    return -ENOMEM;
    } else
    font.data = core::ptr::null_mut();
    scoped_guard(console_lock) {
    if (vc.vc_mode != KD_TEXT)
    return -EINVAL;
    if (!vc.vc_sw.con_font_get)
    return -ENOSYS;
    let mut ret: c_int = vc.vc_sw.con_font_get(vc, &font, vpitch);
    if (ret)
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn con_font_set(vc: *mut vc_data, op: *const console_font_op) -> c_int {
    static int con_font_set(struct vc_data *vc, const struct console_font_op *op)
    {
    struct console_font font;
    int size;
    let mut vpitch: c_uint = op.op == KD_FONT_OP_SET_TALL ? op.height : 32;
    if (!op.data)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn con_font_default(vc: *mut vc_data, op: *mut console_font_op) -> c_int {
    static int con_font_default(struct vc_data *vc, struct console_font_op *op)
    {
    let mut font: console_font = {.width = op.width, .height = op.height};
    char name[MAX_FONT_NAME];
    char *s = name;
    if (!op.data)
    s = core::ptr::null_mut();
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncpy_from_user(name, _arg: op->data, 0: MAX_FONT_NAME - 1) <) -> else {
    ...
    if (vc_is_sel(vc))
    clear_selection();
    let mut ret: c_int = vc.vc_sw.con_font_default(vc, &font, s);
    if (ret)
    return ret;
    }
    op.width = font.width;
...
#[no_mangle]
pub unsafe extern "C" fn con_font_op(vc: *mut vc_data, op: *mut console_font_op) -> c_int {
...
// used by selection
#[no_mangle]
pub unsafe extern "C" fn screen_glyph(vc: *const vc_data, offset: c_int) -> u16 {
    u16 screen_glyph(const struct vc_data *vc, int offset)
    {
    let mut w: u16 = scr_readw(screenpos(vc, offset, true));
    let mut c: u16 = w & 0xff;
...
#[no_mangle]
pub unsafe extern "C" fn screen_glyph_unicode(vc: *const vc_data, n: c_int) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn getconsxy(vc: *const vc_data, 2]: unsigned char xy[static) {
...
#[no_mangle]
pub unsafe extern "C" fn putconsxy(vc: *mut vc_data, 2]: unsigned char xy[static const) {
...
#[no_mangle]
pub unsafe extern "C" fn vcs_scr_readw(vc: *const vc_data, org: *const u16) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn vcs_scr_writew(vc: *mut vc_data, val: u16, org: *mut u16) {
...
#[no_mangle]
pub unsafe extern "C" fn vcs_scr_updated(vc: *mut vc_data) {
