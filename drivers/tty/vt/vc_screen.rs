...
//! Source: drivers/tty/vt/vc_screen.c
#![no_std]
...
pub struct vcs_poll_data {
    ...
    let mut currcons: c_int = poll.cons_num;
    int fa_band;
    switch (code) {
    case VT_UPDATE:
    fa_band = POLL_PRI;
    ...
    {
    struct vcs_poll_data *poll = file.private_data, *kill = core::ptr::null_mut();
    if (poll)
    return poll;
...
//
    ...
    {
    let mut currcons: c_uint = console(inode);
...
//
#[no_mangle]
unsafe extern "C" fn vcs_size(vc: *const vc_data, attr: bool, unicode: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vcs_lseek(file: *mut file, offset: loff_t, orig: c_int) -> loff_t {
    ...
    vc = vcs_vc(inode, core::ptr::null_mut());
    if (!vc)
    return -ENXIO;
...
//
#[no_mangle]
pub unsafe extern "C" fn vc_compile_le16(hi: u8, lo: u8) -> u16 {
...
//
    ...
    written = 0;
    while (count) {
    let mut this_round: c_uint = count;
    if (this_round > CON_BUF_SIZE)
    this_round = CON_BUF_SIZE;
...
// ppos += written;
    ...
    let mut ret: __poll_t = DEFAULT_POLLMASK|EPOLLERR;
    if (poll) {
...
// don't allocate anything if all we want is disable fasync
    ...
    {
    let mut currcons: c_uint = console(inode);
    let mut attr: bool = use_attributes(inode);
    let mut uni_mode: bool = use_unicode(inode);
...
#[no_mangle]
unsafe extern "C" fn vcs_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn vcs_make_sysfs(index: c_int) {
    void vcs_make_sysfs(int index)
    {
    device_create(&vc_class, core::ptr::null_mut(), MKDEV(VCS_MAJOR, index + 1), core::ptr::null_mut(), "vcs%u", index + 1);
    device_create(&vc_class, core::ptr::null_mut(), MKDEV(VCS_MAJOR, index + 65), core::ptr::null_mut(), "vcsu%u", index + 1);
    device_create(&vc_class, core::ptr::null_mut(), MKDEV(VCS_MAJOR, index + 129), core::ptr::null_mut(), "vcsa%u", index + 1);
    }
#[no_mangle]
pub unsafe extern "C" fn vcs_remove_sysfs(index: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn vcs_init() -> int __init {
    ...
    device_create(&vc_class, core::ptr::null_mut(), MKDEV(VCS_MAJOR, 0), core::ptr::null_mut(), "vcs");
    device_create(&vc_class, core::ptr::null_mut(), MKDEV(VCS_MAJOR, 64), core::ptr::null_mut(), "vcsu");
    device_create(&vc_class, core::ptr::null_mut(), MKDEV(VCS_MAJOR, 128), core::ptr::null_mut(), "vcsa");
