...
//! Source: drivers/mfd/tps6594-i2c.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn tps6594_i2c_transfer(adap: *mut i2c_adapter, msgs: *mut i2c_msg, num: c_int) -> c_int {
    static int tps6594_i2c_transfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
    {
    let mut ret: c_int = i2c_transfer(adap, msgs, num);
    if (ret == num)
    return 0;
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn tps6594_i2c_reg_read_with_crc(client: *mut i2c_client, page: u8, reg: u8, val: *mut u8) -> c_int {
...
// I2C address = I2C base address + Page index
    let mut addr: u8 = client.addr + page;
//
...
#[no_mangle]
unsafe extern "C" fn tps6594_i2c_reg_write_with_crc(client: *mut i2c_client, page: u8, reg: u8, val: u8) -> c_int {
...
// Write register, data and CRC
    ...
    let mut page: u8 = reg_bytes[1];
    let mut reg: u8 = reg_bytes[0];
    let mut ret: c_int = 0;
    int i;
    if (tps.use_crc) {
//
...
#[no_mangle]
unsafe extern "C" fn tps6594_i2c_write(context: *mut c_void, data: *const c_void, count: usize) -> c_int {
    ...
    const u8 *bytes = data;
    u8 *buf;
    let mut page: u8 = bytes[1];
    let mut reg: u8 = bytes[0];
    let mut ret: c_int = 0;
    int i;
    if (tps.use_crc) {
//
...
#[no_mangle]
unsafe extern "C" fn tps6594_i2c_probe(client: *mut i2c_client) -> c_int {
    ...
    tps.regmap = devm_regmap_init(dev, core::ptr::null_mut(), client, &tps6594_i2c_regmap_config);
