...
//! Source: drivers/mfd/mxs-lradc.c
#![no_std]
...
pub const ADC_CELL: c_int = 0;
pub const TSC_CELL: c_int = 1;
pub const RES_MEM: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn mxs_lradc_probe(pdev: *mut platform_device) -> c_int {
    static int mxs_lradc_probe(struct platform_device *pdev)
    {
    struct device *dev = &pdev.dev;
    struct device_node *node = dev.of_node;
    struct mxs_lradc *lradc;
    struct mfd_cell *cells = core::ptr::null_mut();
    struct resource *res;
    let mut ret: c_int = 0;
    let mut ts_wires: u32 = 0;
    lradc = devm_kzalloc(&pdev.dev, sizeof(*lradc), GFP_KERNEL);
    if (!lradc)
    return -ENOMEM;
    lradc.soc = (kernel_ulong_t)device_get_match_data(&pdev.dev);
    lradc.clk = devm_clk_get(&pdev.dev, core::ptr::null_mut());
    if (IS_ERR(lradc.clk)) {
    ...
    &cells[ADC_CELL], 1, core::ptr::null_mut(), 0, core::ptr::null_mut());
    if (ret) {
    ...
    return 0;
    ret = devm_mfd_add_devices(&pdev.dev, PLATFORM_DEVID_NONE,
    &cells[TSC_CELL], 1, core::ptr::null_mut(), 0, core::ptr::null_mut());
    if (ret) {
    dev_err(&pdev.dev,
...
#[no_mangle]
unsafe extern "C" fn mxs_lradc_remove(pdev: *mut platform_device) {
