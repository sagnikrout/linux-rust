...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct match_ids_walk_data {
    pub ids: *mut acpi_device_id,
    pub adev: *mut acpi_device,
}

#[no_mangle]
unsafe extern "C" fn match_device_ids(adev: *mut acpi_device, data: *mut c_void) -> c_int {
...
// We only care about each node's first defined address
    if (of_property_read_reg(np, 0, &of_node_addr, core::ptr::null_mut()))
...
#[no_mangle]
unsafe extern "C" fn mfd_remove_devices_fn(dev: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mfd_remove_devices_late(parent: *mut device) {
    void mfd_remove_devices_late(struct device *parent)
    {
    let mut level: c_int = MFD_DEP_LEVEL_HIGH;
    device_for_each_child_reverse(parent, &level, mfd_remove_devices_fn);
    }
    EXPORT_SYMBOL(mfd_remove_devices_late);
#[no_mangle]
pub unsafe extern "C" fn mfd_remove_devices(parent: *mut device) {
    void mfd_remove_devices(struct device *parent)
    {
    let mut level: c_int = MFD_DEP_LEVEL_NORMAL;
    device_for_each_child_reverse(parent, &level, mfd_remove_devices_fn);
    }
    EXPORT_SYMBOL(mfd_remove_devices);
#[no_mangle]
unsafe extern "C" fn devm_mfd_dev_release(dev: *mut device, res: *mut c_void) {
