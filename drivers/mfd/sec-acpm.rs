...
pub const ACPM_ADDR_BITS: c_int = 8;
pub const ACPM_MAX_BULK_DATA: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sec_pmic_acpm_platform_data {
    pub device_type: c_int,
    pub acpm_chan_id: c_uint,
    pub speedy_channel: u8,
    pub regmap_cfg_common: *const regmap_config,
    pub regmap_cfg_pmic: *const regmap_config,
    pub regmap_cfg_rtc: *const regmap_config,
    pub regmap_cfg_meter: *const regmap_config,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sec_pmic_acpm_shared_bus_context {
    pub acpm: *mut acpm_handle,
    pub acpm_chan_id: c_uint,
    pub speedy_channel: u8,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sec_pmic_acpm_bus_context {
    pub shared: *mut sec_pmic_acpm_shared_bus_context,
    pub type: enum sec_pmic_acpm_accesstype,
}
    ...
    let mut val_count: usize = count - BITS_TO_BYTES(ACPM_ADDR_BITS);
    const u8 *d = data;
...
#[no_mangle]
unsafe extern "C" fn sec_pmic_acpm_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (IS_ERR(regmap))
    return PTR_ERR(regmap);
    ret = sec_pmic_probe(dev, pdata.device_type, irq, regmap_pmic, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn sec_pmic_acpm_shutdown(pdev: *mut platform_device) {
