...
//! Source: drivers/mfd/tps6594-core.c
#![no_std]
...
pub const TPS6594_CRC_SYNC_TIMEOUT_MS: c_int = 150;
pub const TPS65224_EN_SEL_PB: c_int = 1;
pub const TPS65224_GPIO3_SEL_PB: c_int = 3;
...
#[no_mangle]
unsafe extern "C" fn tps6594_handle_post_irq(irq_drv_data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tps6594_check_crc_mode(tps: *mut tps6594, primary_pmic: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tps6594_set_crc_feature(tps: *mut tps6594) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tps6594_enable_crc(tps: *mut tps6594) -> c_int {
    static int tps6594_enable_crc(struct tps6594 *tps)
    {
    struct device *dev = tps.dev;
    unsigned int is_primary;
    let mut timeout: c_ulong = msecs_to_jiffies(TPS6594_CRC_SYNC_TIMEOUT_MS);
    int ret;
//
...
#[no_mangle]
unsafe extern "C" fn tps6594_power_off_handler(data: *mut sys_off_data) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tps6594_device_init(tps: *mut tps6594, enable_crc: bool) -> c_int {
...
// Keep PMIC in ACTIVE state
    ...
    ret = devm_mfd_add_devices(dev, PLATFORM_DEVID_AUTO, cells, n_cells, core::ptr::null_mut(), 0,
...
// If either the PB/EN/VSENSE or GPIO3 is configured as PB, register a driver for it
    ...
    &tps6594_pwrbutton_cell, 1, core::ptr::null_mut(), 0,
    regmap_irq_get_domain(tps.irq_data));
    if (ret)
...
// No RTC for LP8764, TPS65224 and TPS652G1
    ...
    ARRAY_SIZE(tps6594_rtc_cells), core::ptr::null_mut(), 0,
    regmap_irq_get_domain(tps.irq_data));
    if (ret)
