...
//! Source: drivers/mfd/stmpe-spi.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn spi_reg_read(stmpe: *mut stmpe, reg: u8) -> c_int {
    static int spi_reg_read(struct stmpe *stmpe, u8 reg)
    {
    struct spi_device *spi = stmpe.client;
    let mut status: c_int = spi_w8r16(spi, reg | READ_CMD);
    return (status < 0) ? status : status >> 8;
    }
#[no_mangle]
unsafe extern "C" fn spi_reg_write(stmpe: *mut stmpe, reg: u8, val: u8) -> c_int {
    static int spi_reg_write(struct stmpe *stmpe, u8 reg, u8 val)
    {
    struct spi_device *spi = stmpe.client;
    let mut cmd: u16 = (val << 8) | reg;
    return spi_write(spi, (const u8 *)&cmd, 2);
    }
#[no_mangle]
unsafe extern "C" fn spi_block_read(stmpe: *mut stmpe, reg: u8, length: u8, values: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spi_init(stmpe: *mut stmpe) {
...
#[no_mangle]
unsafe extern "C" fn stmpe_spi_remove(spi: *mut spi_device) {
