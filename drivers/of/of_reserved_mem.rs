...
//! Source: drivers/of/of_reserved_mem.c
#![no_std]
...
//
    ...
    let mut __refdata: *mut static struct reserved_mem reserved_mem = reserved_mem_array;
    let mut total_reserved_mem_cnt: static int = MAX_RESERVED_REGIONS;
...
//
#[no_mangle]
unsafe extern "C" fn alloc_reserved_mem_array() -> int __init {
...
//
    ...
    uname, len);
    nomap = of_get_flat_dt_prop(node, "no-map", core::ptr::null_mut()) != core::ptr::null_mut();
    err = fdt_validate_reserved_mem_node(node, core::ptr::null_mut());
    if (err && err != -ENODEV)
    return err;
...
//
#[no_mangle]
unsafe extern "C" fn __reserved_mem_check_root(node: c_ulong) -> int __init {
    ...
    return -EINVAL;
    prop = of_get_flat_dt_prop(node, "ranges", core::ptr::null_mut());
    if (!prop)
    return -EINVAL;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn __rmem_cmp(a: *const c_void, b: *const c_void) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn __rmem_check_for_overlap() -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn fdt_scan_reserved_mem_late() -> void __init {
...
// Attempt dynamic allocation of a new reserved_mem array
    ...
    if (!prop || !len)
    continue;
    ret = fdt_validate_reserved_mem_node(child, core::ptr::null_mut());
    if (ret && ret != -ENODEV)
    continue;
    of_flat_dt_read_addr_size(prop, 0, &b, &s);
    base = b;
    size = s;
    if (size) {
    uname = fdt_get_name(fdt, child, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn fdt_scan_reserved_mem() -> int __init {
    int __init fdt_scan_reserved_mem(void)
    {
    int node, child;
    let mut dynamic_nodes_cnt: c_int = 0, count = 0;
...
//
    if (err != -ENOENT || !of_get_flat_dt_prop(child, "size", core::ptr::null_mut()))
    continue;
...
//
    ...
    phys_addr_t *res_base)
    {
    let mut prev_bottom_up: bool = memblock_bottom_up();
    let mut bottom_up: bool = false, top_down = false;
    int ret, i;
...
//
#[no_mangle]
unsafe extern "C" fn __reserved_mem_alloc_size(node: c_ulong, uname: *const c_char) -> int __init {
    static int __init __reserved_mem_alloc_size(unsigned long node, const char *uname)
    {
    let mut start: phys_addr_t = 0, end = 0;
    let mut base: phys_addr_t = 0, align = 0, size;
    int i, len;
    const __be32 *prop;
    bool nomap;
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn fdt_validate_reserved_mem_node(node: c_ulong, align: *mut phys_addr_t) -> int __init {
...
//
    ...
    let mut end: phys_addr_t = rmem.base + rmem.size - 1;
    bool reusable =
    (of_get_flat_dt_prop(node, "reusable", core::ptr::null_mut())) != core::ptr::null_mut();
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rmem_assigned_device {
    pub dev: *mut device,
    pub rmem: *mut reserved_mem,
    pub list: list_head,
}
...
//
    ...
    const char *name)
    {
    let mut idx: c_int = of_property_match_string(np, "memory-region-names", name);
...
//
#[no_mangle]
pub unsafe extern "C" fn of_reserved_mem_device_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn devm_of_reserved_mem_device_release(dev: *mut device, res: *mut c_void) {
...
//
#[no_mangle]
pub unsafe extern "C" fn devm_of_reserved_mem_device_init(dev: *mut device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_reserved_mem_region_count(np: *const device_node) -> c_int {
    int of_reserved_mem_region_count(const struct device_node *np)
    {
    return of_count_phandle_with_args(np, "memory-region", core::ptr::null_mut());
    }
