...
pub struct of_bus {
    ...
    pub parent): *mut *mut int (match)(struct device_node,
    void		(*count_cells)(struct device_node *child,
    pub sizec): *mut *mut int addrc, int,
    u64		(*map)(__be32 *addr, const __be32 *range,
    pub fna): int na, int ns, int pna, int,
    pub na): *mut *mut *mut int (translate)(__be32 addr, u64 offset, int,
    pub flag_cells: c_int,
    pub addr): *const *const unsigned int (get_flags)(__be32,
}
...
#[no_mangle]
unsafe extern "C" fn of_bus_default_translate(addr: *mut __be32, offset: u64, na: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn of_bus_default_flags_get_flags(addr: *const __be32) -> c_uint {
    static unsigned int of_bus_default_flags_get_flags(const __be32 *addr)
    {
    return of_read_number(addr, 1);
    }
#[no_mangle]
unsafe extern "C" fn of_bus_default_get_flags(addr: *const __be32) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn of_bus_default_flags_translate(addr: *mut __be32, offset: u64, na: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn of_bus_pci_get_flags(addr: *const __be32) -> c_uint {
    static unsigned int of_bus_pci_get_flags(const __be32 *addr)
    {
    let mut flags: c_uint = 0;
    let mut w: u32 = be32_to_cpup(addr);
...
//
#[no_mangle]
unsafe extern "C" fn of_node_is_pcie(np: *const device_node) -> bool {
    static bool of_node_is_pcie(const struct device_node *np)
    {
    let mut is_pcie: bool = of_node_name_eq(np, "pcie");
    if (is_pcie)
    pr_warn_once("%pOF: Missing device_type\n", np);
    return is_pcie;
    }
#[no_mangle]
unsafe extern "C" fn of_bus_pci_match(np: *mut device_node) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __of_address_resource_bounds(r: *mut resource, start: u64, size: u64) -> VISIBLE_IF_KUNIT int {
...
#[no_mangle]
pub unsafe extern "C" fn if(check_add_overflow(r->start: size &&, 1: size -, _arg: &r->end)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_range_to_resource(np: *mut device_node, index: c_int, res: *mut resource) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn of_bus_isa_match(np: *mut device_node) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn of_bus_isa_get_flags(addr: *const __be32) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn of_bus_default_flags_match(np: *mut device_node) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn of_bus_default_match(np: *mut device_node) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn of_empty_ranges_quirk(np: *const device_node) -> c_int {
    static int of_empty_ranges_quirk(const struct device_node *np)
    {
    if (IS_ENABLED(CONFIG_PPC)) {
// To save cycles, we cache the result for global "Mac" setting
    let mut quirk_state: static int = -1;
// PA-SEMI sdc DT bug
...
//
    ...
    if (ranges == core::ptr::null_mut() && !of_empty_ranges_quirk(parent) &&
    strcmp(rprop, "dma-ranges")) {
    pr_debug("no ranges; cannot translate\n");
    return 1;
    }
    if (ranges == core::ptr::null_mut() || rlen == 0) {
...
//
    ...
    let mut result: u64 = of_read_number(addr + 1, na - 1);
...
#[no_mangle]
pub unsafe extern "C" fn of_translate_address(dev: *mut device_node, in_addr: *const __be32) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn of_translate_dma_address(dev: *mut device_node, in_addr: *const __be32) -> u64 {
...
// Get "reg" or "assigned-addresses" property
    ...
    let mut val: u32 = be32_to_cpu(prop[0]);
...
//
#[no_mangle]
pub unsafe extern "C" fn of_property_read_reg(np: *mut device_node, idx: c_int, addr: *mut u64, size: *mut u64) -> c_int {
    int of_property_read_reg(struct device_node *np, int idx, u64 *addr, u64 *size)
    {
    const __be32 *prop = of_get_address(np, idx, size, core::ptr::null_mut());
    if (!prop)
    return -EINVAL;
// addr = of_read_number(prop, of_n_addr_cells(np));
    ...
    parser.range = core::ptr::null_mut();
    parser.end = core::ptr::null_mut();
    ...
    {
    let mut na: c_int = parser.na;
    let mut ns: c_int = parser.ns;
    let mut np: c_int = parser.pna + na + ns;
    int busflag_na;
    if (!range)
    return core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn of_dma_get_range(np: *mut device_node, map: *const bus_dma_region) -> c_int {
    int of_dma_get_range(struct device_node *np, const struct bus_dma_region **map)
    {
    struct device_node *node __free(device_node) = of_node_get(np);
    const __be32 *ranges = core::ptr::null_mut();
    let mut found_dma_ranges: bool = false;
    struct of_range_parser parser;
    struct of_range range;
...
//
#[no_mangle]
pub unsafe extern "C" fn of_dma_get_max_cpu_address(np: *mut device_node) -> phys_addr_t __init {
    phys_addr_t __init of_dma_get_max_cpu_address(struct device_node *np)
    {
    let mut max_cpu_addr: phys_addr_t = PHYS_ADDR_MAX;
    struct of_range_parser parser;
...
//
#[no_mangle]
pub unsafe extern "C" fn of_dma_is_coherent(np: *mut device_node) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn of_mmio_is_nonposted(np: *const device_node) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(IORESOURCE_IO: flags &) -> else {
