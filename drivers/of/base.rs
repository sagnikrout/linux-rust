...
#[no_mangle]
pub unsafe extern "C" fn of_node_name_eq(np: *const device_node, name: *const c_char) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn of_node_name_prefix(np: *const device_node, prefix: *const c_char) -> bool {
...
#[no_mangle]
unsafe extern "C" fn __of_node_is_type(np: *const device_node, type: *const c_char) -> bool {
    ...
    of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "coreboot") \
    )
#[no_mangle]
pub unsafe extern "C" fn of_bus_n_addr_cells(np: *mut device_node) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn of_n_addr_cells(np: *mut device_node) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn of_bus_n_size_cells(np: *mut device_node) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn of_n_size_cells(np: *mut device_node) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn of_node_to_nid(np: *mut device_node) -> int __weak {
    int __weak of_node_to_nid(struct device_node *np)
    {
    return NUMA_NO_NODE;
    }

pub const OF_PHANDLE_CACHE_BITS: c_int = 7;

    static struct device_node *phandle_cache[OF_PHANDLE_CACHE_SZ];
#[no_mangle]
unsafe extern "C" fn of_phandle_cache_hash(handle: phandle) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn __of_phandle_cache_inv_entry(handle: phandle) {
    ...
    phandle_cache[handle_hash] = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn of_core_init() -> void __init {
...
// Create the kset, and register existing nodes
    mutex_lock(&of_mutex);
    of_kset = kset_create_and_add("devicetree", core::ptr::null_mut(), firmware_kobj);
    if (!of_kset) {
...
// Symlink in /proc as required by userspace ABI
    if (of_root)
    proc_symlink("device-tree", core::ptr::null_mut(), "/sys/firmware/devicetree/base");
    }
...
//
    ...
    return pp ? pp.value : core::ptr::null_mut();
    }
//
// Find a property with a given name for a given node
// and return the value.
...
//
    ...
    const char *cp;
    let mut index: c_int = 0, score = 0;
// Compatible match has highest priority
    if (compat && compat[0]) {
    prop = __of_find_property(device, "compatible", core::ptr::null_mut());
    for (cp = of_prop_next_string(prop, core::ptr::null_mut()); cp;
...
//
    ...
    int res;
    raw_spin_lock_irqsave(&devtree_lock, flags);
    res = __of_device_is_compatible(device, compat, core::ptr::null_mut(), core::ptr::null_mut());
    raw_spin_unlock_irqrestore(&devtree_lock, flags);
    return res;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn of_machine_compatible_match(compats: *const *const c_char) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_machine_read_compatible(compatible: *const c_char, index: c_uint) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_machine_read_model(model: *const c_char) -> c_int {
...
//
    ...
    return false;
    while (*strings) {
    let mut len: c_uint = strlen(*strings);
    if ((*strings)[len - 1] == '-') {
...
//
#[no_mangle]
unsafe extern "C" fn __of_device_is_available(device: *const device_node) -> bool {
    static bool __of_device_is_available(const struct device_node *device)
    {
    static const char * const ok[] = {"okay", "ok", core::ptr::null_mut()};
    if (!device)
    return false;
    return !__of_get_property(device, "status", core::ptr::null_mut()) ||
...
//
#[no_mangle]
unsafe extern "C" fn __of_device_is_reserved(device: *const device_node) -> bool {
    static bool __of_device_is_reserved(const struct device_node *device)
    {
    static const char * const reserved[] = {"reserved", core::ptr::null_mut()};
...
//
#[no_mangle]
pub unsafe extern "C" fn of_device_is_available(device: *const device_node) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn __of_device_is_fail(device: *const device_node) -> bool {
    static bool __of_device_is_fail(const struct device_node *device)
    {
    static const char * const fail[] = {"fail", "fail-", core::ptr::null_mut()};
...
//
#[no_mangle]
pub unsafe extern "C" fn of_device_is_big_endian(device: *const device_node) -> bool {
...
//
    ...
    for (child = __of_get_next_child(parent, core::ptr::null_mut()); child != core::ptr::null_mut(); \
...
//
    ...
    {
    struct device_node *next = core::ptr::null_mut();
    unsigned long flags;
    ...
    next = prev.sibling;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: node) -> else {
    next = node.child;
    of_node_put(node);
    }
    for (; next; next = next.sibling) {
...
//
    ...
    raw_spin_lock_irqsave(&devtree_lock, flags);
    for_each_of_allnodes_from(from, np)
    if (__of_device_is_compatible(np, compatible, type, core::ptr::null_mut()) &&
    of_node_get(np))
    break;
    of_node_put(from);
    raw_spin_unlock_irqrestore(&devtree_lock, flags);
...
//
    ...
    raw_spin_lock_irqsave(&devtree_lock, flags);
    for_each_of_allnodes_from(from, np) {
    if (__of_find_property(np, prop_name, core::ptr::null_mut())) {
    of_node_get(np);
    break;
    }
    }
    ...
    {
    const struct of_device_id *best_match = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn of_alias_from_compatible(node: *const device_node, alias: *mut c_char, len: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn of_print_phandle_args(msg: *const c_char, args: *const of_phandle_args) {
    void of_print_phandle_args(const char *msg, const struct of_phandle_args *args)
    {
    int i;
    printk("%s %pOF", msg, args.np);
    for (i = 0; i < args.args_count; i++) {
    let mut delim: c_char = i ? ',' : ':';
...
#[no_mangle]
pub unsafe extern "C" fn of_phandle_iterator_next(it: *mut of_phandle_iterator) -> c_int {
    int of_phandle_iterator_next(struct of_phandle_iterator *it)
    {
    let mut count: u32 = 0;
    if (it.node) {
    of_node_put(it.node);
    it.node = core::ptr::null_mut();
    }
...
//
    ...
    struct device_node *cur, *new = core::ptr::null_mut();
...
// Get the <list>-map-mask property (optional)
    mask = of_get_property(cur, mask_name, core::ptr::null_mut());
    if (!mask)
    mask = dummy_mask;
...
// Get the <list>-map-pass-thru property (optional)
    pass = of_get_property(cur, pass_name, core::ptr::null_mut());
    if (!pass)
    pass = dummy_pass;
//
...
//
    ...
    let mut val: __be32 = *(map - new_size + i);
    if (i < list_size) {
    val &= ~pass[i];
...
//
#[no_mangle]
pub unsafe extern "C" fn __of_add_property(np: *mut device_node, prop: *mut property) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_add_property(np: *mut device_node, prop: *mut property) -> c_int {
    ...
    mutex_unlock(&of_mutex);
    if (!rc)
    of_property_notify(OF_RECONFIG_ADD_PROPERTY, np, prop, core::ptr::null_mut());
    return rc;
    }
    EXPORT_SYMBOL_GPL(of_add_property);
#[no_mangle]
pub unsafe extern "C" fn __of_remove_property(np: *mut device_node, prop: *mut property) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_remove_property(np: *mut device_node, prop: *mut property) -> c_int {
    ...
    mutex_unlock(&of_mutex);
    if (!rc)
    of_property_notify(OF_RECONFIG_REMOVE_PROPERTY, np, prop, core::ptr::null_mut());
    return rc;
    }
...
// new node
    newprop.next = core::ptr::null_mut();
// next = newprop;
    }
    raw_spin_unlock_irqrestore(&devtree_lock, flags);
...
//
#[no_mangle]
pub unsafe extern "C" fn of_update_property(np: *mut device_node, newprop: *mut property) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_alias_scan(size: *mut *mut *mut void  (dt_alloc)(u64, align): u64) {
    void of_alias_scan(void * (*dt_alloc)(u64 size, u64 align))
    {
    const struct property *pp;
    of_aliases = of_find_node_by_path("/aliases");
    of_chosen = of_find_node_by_path("/chosen");
    if (of_chosen == core::ptr::null_mut())
...
//
#[no_mangle]
pub unsafe extern "C" fn of_alias_get_id(np: *const device_node, stem: *const c_char) -> c_int {
    int of_alias_get_id(const struct device_node *np, const char *stem)
    {
    struct alias_prop *app;
    let mut id: c_int = -ENODEV;
    mutex_lock(&of_mutex);
...
//
#[no_mangle]
pub unsafe extern "C" fn of_alias_get_highest_id(stem: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_console_check(dn: *const device_node, name: *mut c_char, index: c_int) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_find_last_cache_level(cpu: c_uint) -> c_int {
    int of_find_last_cache_level(unsigned int cpu)
    {
    let mut cache_level: u32 = 0;
    struct device_node *prev = core::ptr::null_mut(), *np = of_cpu_device_node_get(cpu);
    while (np) {
    of_node_put(prev);
    prev = np;
...
//
#[no_mangle]
unsafe extern "C" fn of_check_bad_map(map: *const __be32, len: c_int) -> bool {
    static bool of_check_bad_map(const __be32 *map, int len)
    {
    let mut phandle: __be32 = map[1];
    if (len % 4)
    return false;
...
//
    ...
    id, filter_np && *filter_np ? *filter_np : core::ptr::null_mut());
...
//
    ...
    {
    return of_map_id(np, id, "iommu-map", "#iommu-cells", "iommu-map-mask", core::ptr::null_mut(), arg);
    }
