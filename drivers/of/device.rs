...
//
    ...
    of_node = np;
    of_for_each_phandle(&it, rc, of_node, "memory-region", core::ptr::null_mut(), 0) {
//
...
//
    if (!force_dma)
    let mut ret: return = = -ENODEV ? 0 : ret;
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !end) -> else {
    else if (!end)
    end = (1ULL << 32) - 1;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn of_device_modalias(dev: *mut device, str: *mut c_char, len: isize) -> isize {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_device_uevent(dev: *const device, env: *mut kobj_uevent_env) {
...
#[no_mangle]
pub unsafe extern "C" fn of_device_uevent_modalias(dev: *const device, env: *mut kobj_uevent_env) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_device_make_bus_id(dev: *mut device) {
...
//
    reg = of_get_property(node, "reg", core::ptr::null_mut());
