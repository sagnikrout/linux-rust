...
pub const RNG_SEED_SIZE: c_int = 128;
//
...
//
pub const FDT_EXTRA_SPACE: c_uint = 0x1000;
//
...
//
#[no_mangle]
unsafe extern "C" fn fdt_find_and_del_mem_rsv(fdt: *mut c_void, start: c_ulong, size: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn get_addr_size_cells(addr_cells: *mut c_int, size_cells: *mut c_int) -> c_int {
...
pub unsafe extern "C" fn ima_free_kexec_buffer() -> int __init {
    ...
    prop = of_find_property(of_chosen, "linux,ima-kexec-buffer", core::ptr::null_mut());
    if (!prop)
    return -ENOENT;
...
//
#[no_mangle]
unsafe extern "C" fn remove_ima_buffer(fdt: *mut c_void, chosen_node: c_int) {
...
#[no_mangle]
unsafe extern "C" fn kho_add_chosen(image: *const kimage, fdt: *mut c_void, chosen_node: c_int) -> c_int {
    static int kho_add_chosen(const struct kimage *image, void *fdt, int chosen_node)
    {
    let mut ret: c_int = 0;

    let mut fdt_mem: phys_addr_t = 0;
    let mut fdt_len: phys_addr_t = 0;
    let mut scratch_mem: phys_addr_t = 0;
    let mut scratch_len: phys_addr_t = 0;
...
pub unsafe extern "C" fn if(_arg: ret) -> else {
    ...
    let mut seed: u64 = get_random_u64();
...
// add rng-seed
    ...
    ret = fdt_setprop(fdt, chosen_node, "linux,booted-from-kexec", core::ptr::null_mut(), 0);
    if (ret)
    goto out;
