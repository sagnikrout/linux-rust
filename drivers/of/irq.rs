...
//
#[no_mangle]
pub unsafe extern "C" fn irq_of_parse_and_map(dev: *mut device_node, index: c_int) -> c_uint {
...
//
    ...
    of_node_put(child);
    child = p;
    } while (p && of_get_property(p, "#interrupt-cells", core::ptr::null_mut()) == core::ptr::null_mut());
    return p;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn of_irq_parse_raw(addr: *const __be32, out_irq: *mut of_phandle_args) -> c_int {
    int of_irq_parse_raw(const __be32 *addr, struct of_phandle_args *out_irq)
    {
    struct device_node *ipar, *tnode, *old = core::ptr::null_mut();
    __be32 initial_match_array[MAX_PHANDLE_ARGS];
    const __be32 *match_array = initial_match_array;
    const __be32 *tmp, dummy_imask[] = { [0 ... (MAX_PHANDLE_ARGS - 1)] = cpu_to_be32(~0) };
    let mut intsize: u32 = 1, addrsize;
    int i, rc = -EINVAL;
...
//
    ...
    do {
    tmp = of_get_property(old, "#address-cells", core::ptr::null_mut());
    tnode = of_get_parent(old);
    of_node_put(old);
    old = tnode;
    } while (old && tmp == core::ptr::null_mut());
    of_node_put(old);
    old = core::ptr::null_mut();
    addrsize = (tmp == core::ptr::null_mut()) ? 2 : be32_to_cpu(*tmp);
    pr_debug(" . addrsize=%d\n", addrsize);
...
//
    let mut intc: bool = of_property_read_bool(ipar, "interrupt-controller");
...
// No interrupt map, check for an interrupt parent
    ...
    imaplen /= sizeof(u32);
// Look for a mask
    imask = of_get_property(ipar, "interrupt-map-mask", core::ptr::null_mut());
    if (!imask)
    imask = dummy_imask;
// Parse interrupt-map
    match = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn of_irq_parse_one(device: *mut device_node, index: c_int, out_irq: *mut of_phandle_args) -> c_int {
    int of_irq_parse_one(struct device_node *device, int index, struct of_phandle_args *out_irq)
    {
    struct device_node __free(device_node) *p = core::ptr::null_mut();
    const __be32 *addr;
    u32 intsize;
    int i, res, addr_len;
    __be32 addr_buf[3] = { 0 };
...
//
#[no_mangle]
pub unsafe extern "C" fn of_irq_to_resource(dev: *mut device_node, index: c_int, r: *mut resource) -> c_int {
    int of_irq_to_resource(struct device_node *dev, int index, struct resource *r)
    {
    let mut irq: c_int = of_irq_get(dev, index);
    if (irq < 0)
    return irq;
...
//
#[no_mangle]
pub unsafe extern "C" fn of_irq_get(dev: *mut device_node, index: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_irq_get_byname(dev: *mut device_node, name: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_irq_count(dev: *mut device_node) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct of_intc_desc {
    pub list: list_head,
    pub irq_init_cb: of_irq_init_cb_t,
    pub dev: *mut device_node,
    pub interrupt_parent: *mut device_node,
}
...
//
#[no_mangle]
pub unsafe extern "C" fn of_irq_init(matches: *const of_device_id) -> void __init {
    void __init of_irq_init(const struct of_device_id *matches)
    {
    const struct of_device_id *match;
    struct device_node *np, *parent = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn of_check_msi_parent(dev_node: *mut device_node, msi_node: *mut device_node) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_msi_xlate(dev: *mut device, msi_np: *mut device_node, id_in: u32) -> u32 {
    u32 of_msi_xlate(struct device *dev, struct device_node **msi_np, u32 id_in)
    {
    struct device *parent_dev;
    let mut id_out: u32 = id_in;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn of_msi_configure(dev: *mut device, np: *const device_node) {
