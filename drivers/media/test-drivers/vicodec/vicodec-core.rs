...
//! Source: drivers/media/test-drivers/vicodec/vicodec-core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn vicodec_dev_release(dev: *mut device) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vicodec_dev_instance {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vicodec_dev {
    pub v4l2_dev: v4l2_device,
    pub stateful_enc: vicodec_dev_instance,
    pub stateful_dec: vicodec_dev_instance,
    pub stateless_dec: vicodec_dev_instance,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vicodec_ctx {
    pub fh: v4l2_fh,
    pub dev: *mut vicodec_dev,
    pub is_enc: bool,
    pub is_stateless: bool,
    pub lock: *mut spinlock_t,
    pub hdl: v4l2_ctrl_handler,
// Source and destination queue data
    pub q_data: [vicodec_q_data; 2],
    pub state: v4l2_fwht_state,
    pub cur_buf_offset: u32,
    pub comp_max_size: u32,
    pub comp_size: u32,
    pub header_size: u32,
    pub comp_magic_cnt: u32,
    pub comp_has_frame: bool,
    pub comp_has_next_frame: bool,
    pub first_source_change_sent: bool,
    ...
    let mut cap_stride: c_uint = state.stride;
    let mut ref_stride: c_uint = state.ref_stride;
...
#[no_mangle]
unsafe extern "C" fn validate_by_version(flags: c_uint, version: c_uint) -> bool {
    ...
    let mut pixenc: c_uint = flags & V4L2_FWHT_FL_PIXENC_MSK;
    ...
    let mut components_num: c_uint = 3;
    let mut pixenc: c_uint = 0;
    if (params.version < 3)
    return false;
...
#[no_mangle]
unsafe extern "C" fn update_state_from_header(ctx: *mut vicodec_ctx) {
...
//
    ...
    ctx.state.ref_frame.buf = core::ptr::null_mut();
    }
    }
    ...
    let mut comp_frame_size: c_uint = ntohl(ctx.state.header.size);
...
// pp = p;
    ...
    let mut copy: u32 = sizeof(struct fwht_cframe_hdr) - ctx.header_size;
    if (*pp + sz - p < copy)
    copy = *pp + sz - p;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ctx->is_stateless) -> else {
...
#[no_mangle]
unsafe extern "C" fn job_remove_src_buf(ctx: *mut vicodec_ctx, state: u32) {
    ...
    {
    let mut flags: c_uint = ntohl(p_hdr.flags);
    let mut width_div: c_uint = (flags & V4L2_FWHT_FL_CHROMA_FULL_WIDTH) ? 1 : 2;
    let mut height_div: c_uint = (flags & V4L2_FWHT_FL_CHROMA_FULL_HEIGHT) ? 1 : 2;
    let mut components_num: c_uint = 3;
    let mut pixenc: c_uint = 0;
    let mut version: c_uint = ntohl(p_hdr.version);
    if (version >= 2) {
...
#[no_mangle]
unsafe extern "C" fn is_header_valid(p_hdr: *const fwht_cframe_hdr) -> bool {
    static bool is_header_valid(const struct fwht_cframe_hdr *p_hdr)
    {
    const struct v4l2_fwht_pixfmt_info *info;
    let mut w: c_uint = ntohl(p_hdr.width);
    let mut h: c_uint = ntohl(p_hdr.height);
    let mut version: c_uint = ntohl(p_hdr.version);
    let mut flags: c_uint = ntohl(p_hdr.flags);
...
#[no_mangle]
unsafe extern "C" fn update_capture_data_from_header(ctx: *mut vicodec_ctx) {
    ...
    let mut flags: c_uint = ntohl(p_hdr.flags);
    let mut hdr_width_div: c_uint = (flags & V4L2_FWHT_FL_CHROMA_FULL_WIDTH) ? 1 : 2;
    let mut hdr_height_div: c_uint = (flags & V4L2_FWHT_FL_CHROMA_FULL_HEIGHT) ? 1 : 2;
//
...
//
    ...
    let mut copy: u32 = max_to_copy - ctx.comp_size;
    if (copy > p_src + sz - p)
    copy = p_src + sz - p;
    ...
    let mut frame_size: u32 = ntohl(p_hdr.size);
    let mut remaining: u32 = sz - ctx.cur_buf_offset - sizeof(*p_hdr);
...
//
    ...
    bool is_out)
    {
    let mut is_uncomp: bool = (ctx.is_enc && is_out) || (!ctx.is_enc && !is_out);
...
#[no_mangle]
unsafe extern "C" fn vidioc_g_fmt(ctx: *mut vicodec_ctx, f: *mut v4l2_format) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vidioc_try_fmt(ctx: *mut vicodec_ctx, f: *mut v4l2_format) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vidioc_s_fmt(ctx: *mut vicodec_ctx, f: *mut v4l2_format) -> c_int {
    static int vidioc_s_fmt(struct vicodec_ctx *ctx, struct v4l2_format *f)
    {
    struct vicodec_q_data *q_data;
    struct vb2_queue *vq;
    let mut fmt_changed: bool = true;
    struct v4l2_pix_format_mplane *pix_mp;
    struct v4l2_pix_format *pix;
...
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_PIX_FMT_FWHT_STATELESS: pix->pixelformat ==) -> else {
    ...
    q_data.info = &pixfmt_fwht;
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_PIX_FMT_FWHT_STATELESS: pix_mp->pixelformat ==) -> else {
    ...
    struct v4l2_pix_format *pix;
    struct v4l2_pix_format_mplane *pix_mp;
    let mut coded_w: u32 = 0, coded_h = 0;
...
//
    ...
    let mut size: c_uint = q_data.sizeimage;
    if (*nplanes)
...
#[no_mangle]
unsafe extern "C" fn vicodec_buf_out_validate(vb: *mut vb2_buffer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vicodec_buf_prepare(vb: *mut vb2_buffer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vicodec_buf_queue(vb: *mut vb2_buffer) {
    static void vicodec_buf_queue(struct vb2_buffer *vb)
    {
    struct vb2_v4l2_buffer *vbuf = to_vb2_v4l2_buffer(vb);
    struct vicodec_ctx *ctx = vb2_get_drv_priv(vb.vb2_queue);
    let mut sz: c_uint = vb2_get_plane_payload(&vbuf.vb2_buf, 0);
    ...
    struct vb2_queue *vq_cap = v4l2_m2m_get_vq(ctx.fh.m2m_ctx,
    V4L2_BUF_TYPE_VIDEO_CAPTURE);
    let mut header_valid: bool = false;
    static const struct v4l2_event rs_event = {
    .type = V4L2_EVENT_SOURCE_CHANGE,
    .u.src_change.changes = V4L2_EVENT_SRC_CH_RESOLUTION,
    };
...
#[no_mangle]
unsafe extern "C" fn vicodec_return_bufs(q: *mut vb2_queue, state: u32) {
...
#[no_mangle]
unsafe extern "C" fn total_frame_size(q_data: *mut vicodec_q_data) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn if(3: q_data->info->components_num ==) -> else {
    ...
    let mut size: c_uint = q_data.coded_width * q_data.coded_height;
    unsigned int chroma_div;
    unsigned int total_planes_size;
    u8 *new_comp_frame = core::ptr::null_mut();
...
//
    ...
    state.ref_frame.cb = core::ptr::null_mut();
    state.ref_frame.cr = core::ptr::null_mut();
    state.ref_frame.alpha = core::ptr::null_mut();
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn vicodec_stop_streaming(q: *mut vb2_queue) {
    ...
    ctx.state.ref_frame.buf = core::ptr::null_mut();
    ctx.state.ref_frame.luma = core::ptr::null_mut();
    ctx.comp_max_size = 0;
...
#[no_mangle]
unsafe extern "C" fn vicodec_buf_request_complete(vb: *mut vb2_buffer) {
...
#[no_mangle]
unsafe extern "C" fn vicodec_try_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vicodec_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
unsafe extern "C" fn vicodec_open(file: *mut file) -> c_int {
    ...
    ctx.is_enc = true;
#[no_mangle]
pub unsafe extern "C" fn if(&dev->stateless_dec.vfd: vfd ==) -> else {
    ...
    1, 31, 1, 20);
    if (ctx.is_stateless)
    v4l2_ctrl_new_custom(hdl, &vicodec_ctrl_stateless_state, core::ptr::null_mut());
    else
...
#[no_mangle]
unsafe extern "C" fn vicodec_release(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vicodec_request_validate(req: *mut media_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vicodec_request_queue(req: *mut media_request) {
...
#[no_mangle]
unsafe extern "C" fn vicodec_v4l2_dev_release(v4l2_dev: *mut v4l2_device) {
...
#[no_mangle]
unsafe extern "C" fn vicodec_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vicodec_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn vicodec_exit() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn vicodec_init() -> int __init {
