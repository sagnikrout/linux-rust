...
//! Source: drivers/media/test-drivers/vimc/vimc-sensor.c
#![no_std]
...
// mf = fmt_default;
    ...
    {
    let mut mbus_code: u32 = vimc_mbus_code_by_index(code.index);
    if (!mbus_code)
    return -EINVAL;
    code.code = mbus_code;
    return 0;
...
// TODO: add support for V4L2_FIELD_ALTERNATE
    ...
    struct vimc_sensor_device *vsensor =
    container_of(sd, struct vimc_sensor_device, sd);
    let mut pixel_rate: u64 = vsensor.pixel_rate.val;
    let mut hts: u32 = width + vsensor.hblank.val;
    let mut vts: u32 = height + vsensor.vblank.val;
    let mut total_pixels: u64 = (u64)hts * vts;
...
#[no_mangle]
unsafe extern "C" fn vimc_sensor_adjust_fmt(fmt: *mut v4l2_mbus_framefmt) {
...
// mf = fmt->format;
    ...
    let mut line_height: c_uint = 16;
    u8 *basep[TPG_MAX_PLANES][2];
    let mut line: c_uint = 1;
    char str[100];
...
#[no_mangle]
unsafe extern "C" fn vimc_sensor_s_stream(sd: *mut v4l2_subdev, enable: c_int) -> c_int {
...
//
    ...
    vfree(vsensor.frame);
    vsensor.frame = core::ptr::null_mut();
    }
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn vimc_sensor_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vimc_sensor_release(ved: *mut vimc_ent_device) {
...
// Allocate the vsensor struct
    ...
    v4l2_ctrl_new_custom(&vsensor.hdl, &vimc_sensor_ctrl_class, core::ptr::null_mut());
    v4l2_ctrl_new_custom(&vsensor.hdl, &vimc_sensor_ctrl_test_pattern, core::ptr::null_mut());
    v4l2_ctrl_new_custom(&vsensor.hdl, &vimc_sensor_ctrl_osd_mode, core::ptr::null_mut());
