...
//! Source: drivers/media/cec/usb/pulse8/pulse8-cec.c
#![no_std]
...
pub const MSGSTART: c_uint = 0xff;
pub const MSGEND: c_uint = 0xfe;
pub const MSGESC: c_uint = 0xfd;
pub const MSGOFFSET: c_int = 3;
pub const DATA_SIZE: c_int = 256;

pub const NUM_MSGS: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pulse8 {
    pub dev: *mut device,
    ...
    pub vers: c_uint,
    pub ping_eeprom_work: delayed_work,
...
// protect rx_msg_cur_idx and rx_msg_num
    ...
    pub new_rx_msg: [u8; CEC_MAX_MSG_SIZE],
    pub new_rx_msg_len: u8,
    pub tx_work: work_struct,
    pub tx_done_status: u32,
    pub tx_signal_free_time: u32,
    pub tx_msg: cec_msg,
    pub tx_msg_is_bcast: bool,
    pub cmd_done: completion,
    pub data: [u8; DATA_SIZE],
    pub len: c_uint,
    pub buf: [u8; DATA_SIZE],
    pub idx: c_uint,
    pub escape: bool,
    pub started: bool,
// locks access to the adapter
    ...
    pub restoring_config: bool,
    pub autonomous: bool,
}

#[no_mangle]
unsafe extern "C" fn pulse8_send(serio: *mut serio, command: *const u8, cmd_len: u8) -> c_int {
    ...
    response, size);
    let mut err: return = = -ENOTTY ? -EIO : err;
    }
#[no_mangle]
unsafe extern "C" fn pulse8_tx_work_handler(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn pulse8_irq_work_handler(work: *mut work_struct) {
    ...
    let mut msgcode: u8 = pulse8.buf[0];
    if (debug > 1)
...
#[no_mangle]
unsafe extern "C" fn pulse8_cec_adap_enable(adap: *mut cec_adapter, enable: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pulse8_cec_adap_log_addr(adap: *mut cec_adapter, log_addr: u8) -> c_int {
    static int pulse8_cec_adap_log_addr(struct cec_adapter *adap, u8 log_addr)
    {
    struct pulse8 *pulse8 = cec_get_drvdata(adap);
    let mut mask: u16 = 0;
    let mut pa: u16 = adap.phys_addr;
    u8 cmd[16];
    let mut err: c_int = 0;
    mutex_lock(&pulse8.lock);
    ...
    let mut osd_len: usize = strlen(adap.log_addrs.osd_name);
    char *osd_str = cmd + 1;
    ...
    mutex_unlock(&pulse8.lock);
    let mut log_addr: return = = CEC_LOG_ADDR_INVALID ? 0 : err;
    }
...
#[no_mangle]
unsafe extern "C" fn pulse8_cec_adap_free(adap: *mut cec_adapter) {
...
#[no_mangle]
unsafe extern "C" fn pulse8_disconnect(serio: *mut serio) {
    static void pulse8_disconnect(struct serio *serio)
    {
    struct pulse8 *pulse8 = serio_get_drvdata(serio);
    cancel_delayed_work_sync(&pulse8.ping_eeprom_work);
    mutex_lock(&pulse8.lock);
    pulse8.serio = core::ptr::null_mut();
    mutex_unlock(&pulse8.lock);
...
#[no_mangle]
unsafe extern "C" fn pulse8_ping_eeprom_work_handler(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn pulse8_connect(serio: *mut serio, drv: *mut serio_driver) -> c_int {
