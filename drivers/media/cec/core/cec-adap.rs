...
//! Source: drivers/media/cec/core/cec-adap.c
#![no_std]
...
//
    ...
    unsigned int la_idx);
#[no_mangle]
unsafe extern "C" fn cec_log_addr2idx(adap: *const cec_adapter, log_addr: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cec_log_addr2dev(adap: *const cec_adapter, log_addr: u8) -> c_uint {
    static unsigned int cec_log_addr2dev(const struct cec_adapter *adap, u8 log_addr)
    {
    let mut i: c_int = cec_log_addr2idx(adap, log_addr);
    return adap.log_addrs.primary_device_type[i < 0 ? 0 : i];
    }
    u16 cec_get_edid_phys_addr(const u8 *edid, unsigned int size,
    unsigned int *offset)
    {
    let mut loc: c_uint = cec_get_edid_spa_location(edid, size);
    if (offset)
// offset = loc;
    if (loc == 0)
...
//
    ...
    let mut ev_idx: c_uint = new_ev.event - 1;
...
#[no_mangle]
pub unsafe extern "C" fn cec_queue_pin_hpd_event(adap: *mut cec_adapter, is_high: bool, ts: ktime_t) {
...
#[no_mangle]
pub unsafe extern "C" fn cec_queue_pin_5v_event(adap: *mut cec_adapter, is_high: bool, ts: ktime_t) {
...
//
#[no_mangle]
unsafe extern "C" fn cec_queue_msg_fh(fh: *mut cec_fh, msg: *const cec_msg) {
...
#[no_mangle]
unsafe extern "C" fn cec_post_state_event(adap: *mut cec_adapter) {
...
//
#[no_mangle]
unsafe extern "C" fn cec_data_completed(data: *mut cec_data) {
...
//
#[no_mangle]
unsafe extern "C" fn cec_data_cancel(data: *mut cec_data, tx_status: u8, rx_status: u8) {
...
//
    ...
    adap.transmitting = core::ptr::null_mut();
    } else {
...
//
#[no_mangle]
unsafe extern "C" fn cec_flush(adap: *mut cec_adapter) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cec_thread_func(_adap: *mut c_void) -> c_int {
...
//
    ...
    let mut done: bool = status & (CEC_TX_STATUS_MAX_RETRIES | CEC_TX_STATUS_OK);
    let mut aborted: bool = adap.transmit_in_progress_aborted;
...
//
#[no_mangle]
unsafe extern "C" fn cec_wait_timeout(work: *mut work_struct) {
...
//
    ...
    {
    struct cec_data *data;
    let mut is_raw: bool = msg_is_raw(msg);
...
// Helper function to be used by drivers and this framework.
    ...
    int ret;
    mutex_lock(&adap.lock);
    ret = cec_transmit_msg_fh(adap, msg, core::ptr::null_mut(), block);
    mutex_unlock(&adap.lock);
    return ret;
    }
...
//
    ...
    bool is_reply);
pub const DIRECTED: c_uint = 0x80;
pub const BCAST1_4: c_uint = 0x40;
pub const BCAST2_0: c_uint = 0x20	/* broadcast only allowed for >= 2.0 */;
...
// Called by the CEC adapter if a message is received
    ...
    {
    struct cec_data *data;
    let mut msg_init: u8 = cec_msg_initiator(msg);
    let mut msg_dest: u8 = cec_msg_destination(msg);
    let mut cmd: u8 = msg.msg[1];
    let mut is_reply: bool = false;
    let mut valid_la: bool = true;
    let mut monitor_valid_la: bool = true;
    let mut min_len: u8 = 0;
...
//
    ...
    let mut dir_fl: u8 = cec_msg_size[cmd] & BOTH;
    min_len = cec_msg_size[cmd] & 0x1f;
    if (msg.len < min_len)
    valid_la = false;
#[no_mangle]
pub unsafe extern "C" fn if(DIRECTED): !cec_msg_is_broadcast(msg) && !(dir_fl &) -> else {
    else if (!cec_msg_is_broadcast(msg) && !(dir_fl & DIRECTED))
    valid_la = false;
#[no_mangle]
pub unsafe extern "C" fn if(BCAST): cec_msg_is_broadcast(msg) && !(dir_fl &) -> else {
...
// It's a valid message and not a poll or CDC message
    ...
    let mut abort: bool = cmd == CEC_MSG_FEATURE_ABORT;
...
// Send poll message
    ...
    err = cec_transmit_msg_fh(adap, &msg, core::ptr::null_mut(), true);
//
...
//
#[no_mangle]
unsafe extern "C" fn cec_adap_unconfigure(adap: *mut cec_adapter) {
...
//
#[no_mangle]
unsafe extern "C" fn cec_config_thread_func(arg: *mut c_void) -> c_int {
...
// The various LAs for each type of device
    ...
    let mut type: c_uint = las.log_addr_type[i];
    const u8 *la_list;
    u8 last_la;
//
...
// Report Features must come first according to CEC 2.0
    ...
    cec_transmit_msg_fh(adap, &msg, core::ptr::null_mut(), false);
    }
...
// Report Vendor ID
    ...
    adap.kthread_config = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn cec_claim_log_addrs(adap: *mut cec_adapter, block: bool) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cec_adap_enable(adap: *mut cec_adapter) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn __cec_s_phys_addr(adap: *mut cec_adapter, phys_addr: u16, block: bool) {
    void __cec_s_phys_addr(struct cec_adapter *adap, u16 phys_addr, bool block)
    {
    let mut becomes_invalid: bool = phys_addr == CEC_PHYS_ADDR_INVALID;
    let mut is_invalid: bool = adap.phys_addr == CEC_PHYS_ADDR_INVALID;
...
#[no_mangle]
pub unsafe extern "C" fn cec_s_phys_addr(adap: *mut cec_adapter, phys_addr: u16, block: bool) {
...
//
    ...
    for (i = 0; i < log_addrs.num_log_addrs; i++) {
    let mut feature_sz: u8 = ARRAY_SIZE(log_addrs.features[0]);
    u8 *features = log_addrs.features[i];
    let mut op_is_dev_features: bool = false;
    unsigned int j;
...
#[no_mangle]
unsafe extern "C" fn cec_feature_abort(adap: *mut cec_adapter, msg: *mut cec_msg) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cec_feature_refused(adap: *mut cec_adapter, msg: *mut cec_msg) -> c_int {
...
//
    ...
    bool is_reply)
    {
    let mut is_broadcast: bool = cec_msg_is_broadcast(msg);
    let mut dest_laddr: u8 = cec_msg_destination(msg);
    let mut init_laddr: u8 = cec_msg_initiator(msg);
    let mut devtype: u8 = cec_log_addr2dev(adap, dest_laddr);
    let mut la_idx: c_int = cec_log_addr2idx(adap, dest_laddr);
    let mut from_unregistered: bool = init_laddr == 0xf;
    let mut tx_cec_msg: cec_msg = { };
    dprintk(2, "%s: %*ph\n", __func__, msg.len, msg.msg);
...
// The following messages are processed but still passed through
    case CEC_MSG_REPORT_PHYSICAL_ADDR: {
    let mut pa: u16 = (msg.msg[2] << 8) | msg.msg[3];
...
//
#[no_mangle]
pub unsafe extern "C" fn cec_monitor_all_cnt_inc(adap: *mut cec_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cec_monitor_all_cnt_dec(adap: *mut cec_adapter) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cec_monitor_pin_cnt_inc(adap: *mut cec_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cec_monitor_pin_cnt_dec(adap: *mut cec_adapter) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cec_adap_status(file: *mut seq_file, priv: *mut c_void) -> c_int {
