...
//! Source: drivers/media/spi/cxd2880-spi.c
#![no_std]
...
pub const CXD2880_MAX_FILTER_SIZE: c_int = 32;
pub const BURST_WRITE_MAX: c_int = 128;
pub const MAX_TRANS_PKT: c_int = 300;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxd2880_ts_buf_info {
    pub read_ready:1: u8,
    pub almost_full:1: u8,
    pub almost_empty:1: u8,
    pub overflow:1: u8,
    pub underflow:1: u8,
    pub pkt_num: u16,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxd2880_pid_filter_config {
    pub is_negative: u8,
    pub pid_config: [cxd2880_pid_config; CXD2880_MAX_FILTER_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxd2880_dvb_spi {
    pub dvb_fe: dvb_frontend,
    pub adapter: dvb_adapter,
    pub demux: dvb_demux,
    pub dmxdev: dmxdev,
    pub dmx_fe: dmx_frontend,
    pub cxd2880_ts_read_thread: *mut task_struct,
    pub spi: *mut spi_device,
    pub /: *mut *mut mutex spi_mutex; / For SPI access exclusive control,
    pub feed_count: c_int,
    pub all_pid_feed_count: c_int,
    pub vcc_supply: *mut regulator,
    pub ts_buf: *mut u8,
    pub filter_config: cxd2880_pid_filter_config,
}

    DVB_DEFINE_MOD_OPT_ADAPTER_NR(adapter_nr);
#[no_mangle]
unsafe extern "C" fn cxd2880_write_spi(spi: *mut spi_device, data: *mut u8, size: u32) -> c_int {
    static int cxd2880_write_spi(struct spi_device *spi, u8 *data, u32 size)
    {
    struct spi_message msg;
    let mut tx: spi_transfer = {};
    if (!spi || !data) {
    pr_err("invalid arg\n");
    return -EINVAL;
    }
    ...
    const u8 *write_data_top = core::ptr::null_mut();
    let mut ret: c_int = 0;
    if (!spi || !data) {
    pr_err("invalid arg\n");
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn cxd2880_spi_clear_ts_buffer(spi: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxd2880_ts_read(arg: *mut c_void) -> c_int {
    static int cxd2880_ts_read(void *arg)
    {
    struct cxd2880_dvb_spi *dvb_spi = core::ptr::null_mut();
    struct cxd2880_ts_buf_info info;
    ktime_t start;
    u32 i;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn cxd2880_start_feed(feed: *mut dvb_demux_feed) -> c_int {
    static int cxd2880_start_feed(struct dvb_demux_feed *feed)
    {
    let mut ret: c_int = 0;
    let mut i: c_int = 0;
    struct dvb_demux *demux = core::ptr::null_mut();
    struct cxd2880_dvb_spi *dvb_spi = core::ptr::null_mut();
    if (!feed) {
    pr_err("invalid arg\n");
    return -EINVAL;
    ...
    demux = feed.demux;
    if (!demux) {
    pr_err("feed.demux is core::ptr::null_mut()\n");
    return -EINVAL;
    }
    dvb_spi = demux.priv;
    ...
    dvb_spi.ts_buf = core::ptr::null_mut();
    memset(&dvb_spi.filter_config, 0,
    sizeof(dvb_spi.filter_config));
    dvb_spi.all_pid_feed_count = 0;
...
#[no_mangle]
unsafe extern "C" fn cxd2880_stop_feed(feed: *mut dvb_demux_feed) -> c_int {
...
//
    ...
    dvb_spi.vcc_supply = core::ptr::null_mut();
    } else {
