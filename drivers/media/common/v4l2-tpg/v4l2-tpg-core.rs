...
//! Source: drivers/media/common/v4l2-tpg/v4l2-tpg-core.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn tpg_set_font(f: *const u8) {
...
#[no_mangle]
pub unsafe extern "C" fn tpg_init(tpg: *mut tpg_data, w: unsigned, h: unsigned) {
...
#[no_mangle]
pub unsafe extern "C" fn tpg_alloc(tpg: *mut tpg_data, max_w: unsigned) -> c_int {
    ...
    let mut pixelsz: unsigned = plane ? 2 : 4;
    tpg.lines[pat][plane] =
    ...
    tpg.contrast_line[plane] = core::ptr::null_mut();
    tpg.black_line[plane] = core::ptr::null_mut();
    tpg.random_line[plane] = core::ptr::null_mut();
    }
    free_lines:
    for (pat = 0; pat < TPG_MAX_PAT_LINES; pat++)
    for (plane = 0; plane < TPG_MAX_PLANES; plane++) {
    vfree(tpg.lines[pat][plane]);
    tpg.lines[pat][plane] = core::ptr::null_mut();
    if (plane == 0)
    continue;
    vfree(tpg.downsampled_lines[pat][plane]);
    tpg.downsampled_lines[pat][plane] = core::ptr::null_mut();
    }
    return ret;
    }
    EXPORT_SYMBOL_GPL(tpg_alloc);
#[no_mangle]
pub unsafe extern "C" fn tpg_free(tpg: *mut tpg_data) {
...
#[no_mangle]
pub unsafe extern "C" fn tpg_s_fourcc(tpg: *mut tpg_data, fourcc: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn tpg_get_textbg_color(tpg: *mut tpg_data) -> enum tpg_color {
...
#[no_mangle]
unsafe extern "C" fn tpg_get_textfg_color(tpg: *mut tpg_data) -> enum tpg_color {
...
#[no_mangle]
pub unsafe extern "C" fn rec709_to_linear(v: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn linear_to_rec709(v: c_int) -> c_int {
...
// Clamp Hue
    ...
    aux += 180;
#[no_mangle]
pub unsafe extern "C" fn if(180: aux >) -> else {
    else if (aux > 180)
    aux -= 180;
    } else {
    aux = aux & 0xff;
...
// cr = ((m[2][0] * r + m[2][1] * g + m[2][2] * b) >> 16) + (128 << 4);
    ...
    let mut full: bool = tpg.real_quantization == V4L2_QUANTIZATION_FULL_RANGE;
    let mut y_offset: unsigned = full ? 0 : 16;
    int lin_y, yc;
...
// b = clamp(*b >> 12, 0, 0xff0);
    ...
    let mut full: bool = tpg.real_quantization == V4L2_QUANTIZATION_FULL_RANGE;
    let mut y_offset: unsigned = full ? 0 : 16;
    let mut y_fac: c_int = full ? COEFF(1.0, 255) : COEFF(1.0, 219);
...
#[no_mangle]
unsafe extern "C" fn precalculate_color(tpg: *mut tpg_data, k: c_int) {
    static void precalculate_color(struct tpg_data *tpg, int k)
    {
    let mut col: c_int = k;
    let mut r: c_int = tpg_colors[col].r;
    let mut g: c_int = tpg_colors[col].g;
    let mut b: c_int = tpg_colors[col].b;
    int y, cb, cr;
    let mut ycbcr_valid: bool = false;
...
#[no_mangle]
unsafe extern "C" fn tpg_precalculate_colors(tpg: *mut tpg_data) {
...
// 'odd' is true for pixels 1, 3, 5, etc. and false for pixels 0, 2, 4, etc.
    ...
    {
    let mut offset: unsigned = odd * tpg.twopixelsize[0] / 2;
    let mut alpha: u8 = tpg.alpha_component;
    u8 r_y_h, g_u_s, b_v;
...
#[no_mangle]
pub unsafe extern "C" fn tpg_g_interleaved_plane(tpg: *const tpg_data, buf_line: unsigned) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn tpg_get_pat_lines(tpg: *const tpg_data) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn tpg_get_pat_line(tpg: *const tpg_data, line: unsigned) -> unsigned {
    ...
    case TPG_PAT_CROSS_1_PIXEL:
    let mut line: return = = tpg.src_height / 2;
    case TPG_PAT_CROSS_2_PIXELS:
...
//
#[no_mangle]
unsafe extern "C" fn tpg_calculate_square_border(tpg: *mut tpg_data) {
    static void tpg_calculate_square_border(struct tpg_data *tpg)
    {
    let mut w: unsigned = tpg.src_width;
    let mut h: unsigned = tpg.src_height;
    unsigned sq_w, sq_h;
    sq_w = (w * 2 / 5) & ~1;
    if (((w - sq_w) / 2) & 1)
    sq_w += 2;
    ...
    let mut ana_sq_w: unsigned = (sq_w / 4) * 3;
    if (((w - ana_sq_w) / 2) & 1)
    ana_sq_w += 2;
    ...
    sq_h = sq_w * 10 / 11;
#[no_mangle]
pub unsafe extern "C" fn if(TPG_PIXEL_ASPECT_PAL: tpg->pix_aspect ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn tpg_precalculate_line(tpg: *mut tpg_data) {
...
// Coarse scaling with Bresenham
    let mut int_part: unsigned = tpg.src_width / tpg.scaled_width;
    let mut fract_part: unsigned = tpg.src_width % tpg.scaled_width;
    let mut src_x: unsigned = 0;
    let mut error: unsigned = 0;
    for (x = 0; x < tpg.scaled_width * 2; x += 2) {
    let mut real_x: unsigned = src_x;
    ...
    for (p = 0; p < tpg.planes; p++) {
    let mut twopixsize: unsigned = tpg.twopixelsize[p];
    let mut hdiv: unsigned = tpg.hdownsampling[p];
    ...
    let mut pat_lines: unsigned = tpg_get_pat_lines(tpg);
    for (pat = 0; pat < pat_lines; pat++) {
    let mut next_pat: unsigned = (pat + 1) % pat_lines;
    for (p = 1; p < tpg.planes; p++) {
    let mut w: unsigned = tpg_hdiv(tpg, p, tpg.scaled_width * 2);
...
// need this to do rgb24 rendering
    ...
    {
    let mut step: unsigned = V4L2_FIELD_HAS_T_OR_B(tpg.field) ? 2 : 1;
    let mut div: unsigned = step;
    let mut first: unsigned = 0;
    unsigned len;
    unsigned p;
    if (font8x16 == core::ptr::null_mut() || basep == core::ptr::null_mut() || text == core::ptr::null_mut())
    return;
    len = strlen(text);
// Checks if it is possible to show string
    ...
    first = 1;
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_FIELD_SEQ_BT: tpg->field == V4L2_FIELD_SEQ_TB || tpg->field ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn tpg_update_mv_step(tpg: *mut tpg_data) {
    void tpg_update_mv_step(struct tpg_data *tpg)
    {
    let mut factor: c_int = tpg.mv_hor_mode > TPG_MOVE_NONE ? -1 : 1;
    if (tpg.hflip)
    factor = -factor;
...
#[no_mangle]
unsafe extern "C" fn tpg_recalc(tpg: *mut tpg_data) {
    ...
    {
    let mut stride: unsigned = tpg.bytesperline[p];
    let mut h: unsigned = tpg.buf_height;
    tpg_recalc(tpg);
    basep[p][0] = vbuf;
    basep[p][1] = vbuf;
...
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_FIELD_SEQ_BT: tpg->field ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn tpg_log_status(tpg: *mut tpg_data) {
...
#[no_mangle]
pub unsafe extern "C" fn if(TGP_COLOR_ENC_YCBCR: tpg->color_enc ==) -> else {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tpg_draw_params {
// common data
    pub is_tv: bool,
    pub is_60hz: bool,
    pub twopixsize: unsigned,
    pub img_width: unsigned,
    pub stride: unsigned,
    pub hmax: unsigned,
    pub frame_line: unsigned,
    pub frame_line_next: unsigned,
// test pattern
    pub mv_hor_old: unsigned,
    pub mv_hor_new: unsigned,
    pub mv_vert_old: unsigned,
    pub mv_vert_new: unsigned,
// extras
    pub wss_width: unsigned,
    pub wss_random_offset: unsigned,
    pub sav_eav_f: unsigned,
    pub left_pillar_width: unsigned,
    pub right_pillar_start: unsigned,
}
    ...
    struct tpg_draw_params *params)
    {
    let mut left_pillar_width: unsigned = 0;
    let mut right_pillar_start: unsigned = params.img_width;
...
// noinline to work around clang KASAN issues
    ...
    {
    let mut twopixsize: unsigned = params.twopixsize;
    let mut img_width: unsigned = params.img_width;
    let mut frame_line: unsigned = params.frame_line;
...
//
    ...
    let mut bottom: unsigned = b.top + b.height - 1;
    let mut left: unsigned = params.left_pillar_width;
    let mut right: unsigned = params.right_pillar_start;
    ...
    let mut left: unsigned = sq.left;
    let mut width: unsigned = sq.width;
    if (c.left > left) {
    width -= c.left - left;
    left = c.left;
    }
    ...
    if (tpg.insert_sav) {
    let mut offset: unsigned = tpg_hdiv(tpg, p, tpg.compose.width / 3);
    u8 *p = vbuf + offset;
    let mut vact: unsigned = 0, hact = 0;
    p[0] = 0xff;
    p[1] = 0;
    p[2] = 0;
    ...
    if (tpg.insert_eav) {
    let mut offset: unsigned = tpg_hdiv(tpg, p, tpg.compose.width * 2 / 3);
    u8 *p = vbuf + offset;
    let mut vact: unsigned = 0, hact = 1;
    p[0] = 0xff;
    p[1] = 0;
    p[2] = 0;
    p[3] = 0x80 | (params.sav_eav_f << 6) |
    ...
    let mut twopixsize: unsigned = params.twopixsize;
    let mut img_width: unsigned = params.img_width;
    let mut mv_hor_old: unsigned = params.mv_hor_old;
    let mut mv_hor_new: unsigned = params.mv_hor_new;
    let mut mv_vert_old: unsigned = params.mv_vert_old;
    let mut mv_vert_new: unsigned = params.mv_vert_new;
    let mut frame_line: unsigned = params.frame_line;
    let mut frame_line_next: unsigned = params.frame_line_next;
    let mut line_offset: unsigned = tpg_hscale_div(tpg, p, tpg.crop.left);
    bool even;
    let mut fill_blank: bool = false;
...
//
    ...
    {
    struct tpg_draw_params params;
    let mut factor: unsigned = V4L2_FIELD_HAS_T_OR_B(tpg.field) ? 2 : 1;
// Coarse scaling with Bresenham
    let mut int_part: unsigned = (tpg.crop.height / factor) / tpg.compose.height;
    let mut fract_part: unsigned = (tpg.crop.height / factor) % tpg.compose.height;
    let mut src_y: unsigned = 0;
    let mut error: unsigned = 0;
    unsigned h;
    tpg_recalc(tpg);
    params.is_tv = std;
...
//
    ...
    let mut next_src_y: unsigned = src_y;
    if ((h & 3) >= 2)
    continue;
    next_src_y += int_part;
...
#[no_mangle]
pub unsafe extern "C" fn tpg_fillbuffer(tpg: *mut tpg_data, std: v4l2_std_id, p: unsigned, vbuf: *mut u8) {
