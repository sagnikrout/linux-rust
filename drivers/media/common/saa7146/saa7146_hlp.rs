...
//! Source: drivers/media/common/saa7146/saa7146_hlp.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn calculate_output_format_register(saa: *mut *mut saa7146_dev, palette: u32, clip_format: *mut *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn calculate_hps_source_and_sync(dev: *mut saa7146_dev, source: c_int, sync: c_int, hps_ctrl: *mut *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn calculate_hxo_and_hyo(vv: *mut saa7146_vv, hps_h_scale: *mut *mut u32, hps_ctrl: *mut *mut u32) {
    static void calculate_hxo_and_hyo(struct saa7146_vv *vv, u32* hps_h_scale, u32* hps_ctrl)
    {
    let mut hyo: c_int = 0, hxo = 0;
...
// horizontal prescaler
    let mut dcgx: u32 = 0, xpsc = 0, xacm = 0, cxy = 0, cxuv = 0;
// horizontal scaler
    let mut xim: u32 = 0, xp = 0, xsci =0;
// vertical scale & gain
    let mut pfuv: u32 = 0;
// helper variables
    let mut h_atten: u32 = 0, i = 0;
    if ( 0 == out_x ) {
    return -EINVAL;
    }
// mask out vanity-bit
...
// the horizontal scaling increment controls the UV filter
    ...
    pfuv = 0x00;
#[no_mangle]
pub unsafe extern "C" fn if(0x600: xsci <) -> else {
    else if ( xsci < 0x600)
    pfuv = 0x01;
#[no_mangle]
pub unsafe extern "C" fn if(0x680: xsci <) -> else {
    else if ( xsci < 0x680)
    pfuv = 0x11;
#[no_mangle]
pub unsafe extern "C" fn if(0x700: xsci <) -> else {
    else if ( xsci < 0x700)
    pfuv = 0x22;
    else
    pfuv = 0x33;
...
// calculate vertical scale registers
    ...
    let mut lpi: c_int = 0;
// vertical scaling
    let mut yacm: u32 = 0, ysci = 0, yacl = 0, ypo = 0, ype = 0;
// vertical scale & gain
    let mut dcgy: u32 = 0, cya_cyb = 0;
// helper variables
    let mut v_atten: u32 = 0, i = 0;
...
#[no_mangle]
unsafe extern "C" fn saa7146_set_window(dev: *mut saa7146_dev, width: c_int, height: c_int, field: enum v4l2_field) {
    static void saa7146_set_window(struct saa7146_dev *dev, int width, int height, enum v4l2_field field)
    {
    struct saa7146_vv *vv = dev.vv_data;
    let mut source: c_int = vv.current_hps_source;
    let mut sync: c_int = vv.current_hps_sync;
    let mut hps_v_scale: u32 = 0, hps_v_gain  = 0, hps_ctrl = 0, hps_h_prescale = 0, hps_h_scale = 0;
// set vertical scale
...
#[no_mangle]
unsafe extern "C" fn saa7146_set_output_format(dev: *mut saa7146_dev, palette: c_ulong) {
    static void saa7146_set_output_format(struct saa7146_dev *dev, unsigned long palette)
    {
    let mut clip_format: u32 = saa7146_read(dev, CLIP_FORMAT_CTRL);
...
// select input-source
#[no_mangle]
pub unsafe extern "C" fn saa7146_set_hps_source_and_sync(dev: *mut saa7146_dev, source: c_int, sync: c_int) {
    void saa7146_set_hps_source_and_sync(struct saa7146_dev *dev, int source, int sync)
    {
    struct saa7146_vv *vv = dev.vv_data;
    let mut hps_ctrl: u32 = 0;
// read old state
...
#[no_mangle]
pub unsafe extern "C" fn saa7146_write_out_dma(dev: *mut *mut saa7146_dev, which: c_int, vdma: *mut *mut saa7146_video_dma) {
    void saa7146_write_out_dma(struct saa7146_dev* dev, int which, struct saa7146_video_dma* vdma)
    {
    let mut where: c_int = 0;
    if( which < 1 || which > 3) {
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn calculate_video_dma_grab_packed(dev: *mut *mut saa7146_dev, buf: *mut saa7146_buf) -> c_int {
    ...
    let mut width: c_int = pix.width;
    let mut height: c_int = pix.height;
    let mut bytesperline: c_int = pix.bytesperline;
    let mut field: enum v4l2_field = pix.field;
    let mut depth: c_int = sfmt.depth;
...
#[no_mangle]
unsafe extern "C" fn calc_planar_422(vv: *mut saa7146_vv, buf: *mut saa7146_buf, vdma2: *mut saa7146_video_dma, vdma3: *mut saa7146_video_dma) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn calc_planar_420(vv: *mut saa7146_vv, buf: *mut saa7146_buf, vdma2: *mut saa7146_video_dma, vdma3: *mut saa7146_video_dma) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn calculate_video_dma_grab_planar(dev: *mut *mut saa7146_dev, buf: *mut saa7146_buf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn program_capture_engine(dev: *mut saa7146_dev, planar: c_int) {
    static void program_capture_engine(struct saa7146_dev *dev, int planar)
    {
    struct saa7146_vv *vv = dev.vv_data;
    let mut count: c_int = 0;
    let mut e_wait: c_ulong = vv.current_hps_sync == SAA7146_HPS_SYNC_PORT_A ? CMD_E_FID_A : CMD_E_FID_B;
    let mut o_wait: c_ulong = vv.current_hps_sync == SAA7146_HPS_SYNC_PORT_A ? CMD_O_FID_A : CMD_O_FID_B;
...
// disable clipping
#[no_mangle]
unsafe extern "C" fn saa7146_disable_clipping(dev: *mut saa7146_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn saa7146_set_capture(dev: *mut saa7146_dev, buf: *mut saa7146_buf, next: *mut saa7146_buf) {
