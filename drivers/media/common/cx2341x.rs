...
//! Source: drivers/media/common/cx2341x.c
#![no_std]
...
pub const CX2341X_AUDIO_ENCODING_METHOD_MPEG: c_int = 0;
pub const CX2341X_AUDIO_ENCODING_METHOD_AC3: c_int = 1;
pub const CX2341X_AUDIO_ENCODING_METHOD_LPCM: c_int = 2;
...
// Map the control ID to the correct field in the cx2341x_mpeg_params
    ...
    break;
    case V4L2_CID_MPEG_VIDEO_B_FRAMES: {
    let mut b: c_int = ctrl.value + 1;
    let mut gop: c_int = params.video_gop_size;
...
// Max GOP size = 34
    ...
    case V4L2_CID_MPEG_VIDEO_GOP_SIZE: {
    let mut b: c_int = params.video_b_frames + 1;
    let mut gop: c_int = ctrl.value;
    params.video_gop_size = b * ((gop + b - 1) / b);
// Max GOP size = 34
    while (params.video_gop_size > 34)
    params.video_gop_size -= b;
...
#[no_mangle]
unsafe extern "C" fn cx2341x_calc_audio_properties(params: *mut cx2341x_mpeg_params) {
...
// Check for correctness of the ctrl's value based on the data from
    ...
    menu_items may be core::ptr::null_mut(), in that case it is ignored. */
    ...
    return -ERANGE;
    if (qctrl.type == V4L2_CTRL_TYPE_MENU && menu_items != core::ptr::null_mut()) {
    if (menu_items[ctrl.value] == core::ptr::null_mut() ||
    ...
    struct v4l2_ext_control *ctrl = ctrls.controls + i;
    struct v4l2_queryctrl qctrl;
    const char * const *menu_items = core::ptr::null_mut();
    qctrl.id = ctrl.id;
...
#[no_mangle]
pub unsafe extern "C" fn cx2341x_fill_defaults(p: *mut cx2341x_mpeg_params) {
...
// p = default_params;
    ...
    let mut w: u16 = new.width;
    let mut h: u16 = new.height;
...
#[no_mangle]
pub unsafe extern "C" fn cx2341x_log_status(p: *const cx2341x_mpeg_params, prefix: *const c_char) {
    void cx2341x_log_status(const struct cx2341x_mpeg_params *p, const char *prefix)
    {
    let mut is_mpeg1: c_int = p.video_encoding == V4L2_MPEG_VIDEO_ENCODING_MPEG_1;
// Stream
...
#[no_mangle]
pub unsafe extern "C" fn cx2341x_neq(ctrl: *mut v4l2_ctrl) -> c_int {
    static inline int cx2341x_neq(struct v4l2_ctrl *ctrl)
    {
    return ctrl && ctrl.val != ctrl.cur.val;
    }
#[no_mangle]
unsafe extern "C" fn cx2341x_try_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
    static int cx2341x_try_ctrl(struct v4l2_ctrl *ctrl)
    {
    struct cx2341x_handler *hdl = to_cxhdl(ctrl);
    let mut val: i32 = ctrl.val;
    switch (ctrl.id) {
    case V4L2_CID_MPEG_VIDEO_B_FRAMES: {
// video gop cluster
    let mut b: c_int = val + 1;
    let mut gop: c_int = hdl.video_gop_size.val;
    gop = b * ((gop + b - 1) / b);
// Max GOP size = 34
    while (gop > 34)
    gop -= b;
...
#[no_mangle]
unsafe extern "C" fn cx2341x_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
// video median cluster
    ...
    return v4l2_ctrl_new_custom(hdl, &cfg, core::ptr::null_mut());
    }
    ...
    let mut caps: u32 = cxhdl.capabilities;
    let mut has_sliced_vbi: c_int = caps & CX2341X_CAP_HAS_SLICED_VBI;
    let mut has_ac3: c_int = caps & CX2341X_CAP_HAS_AC3;
    let mut has_ts: c_int = caps & CX2341X_CAP_HAS_TS;
    cxhdl.width = 720;
    cxhdl.height = 480;
...
#[no_mangle]
pub unsafe extern "C" fn cx2341x_handler_set_50hz(cxhdl: *mut cx2341x_handler, is_50hz: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn cx2341x_handler_setup(cxhdl: *mut cx2341x_handler) -> c_int {
    int cx2341x_handler_setup(struct cx2341x_handler *cxhdl)
    {
    let mut h: c_int = cxhdl.height;
    let mut w: c_int = cxhdl.width;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn cx2341x_handler_set_busy(cxhdl: *mut cx2341x_handler, busy: c_int) {
