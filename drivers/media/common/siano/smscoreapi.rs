//! Automatically rewritten from C Header to Rust Module
//! Source: drivers/media/common/siano/smscoreapi.h
#![no_std]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(dead_code)]
#![allow(unused_variables)]
#![allow(unused_mut)]

use core::ffi::*;

// --- Linux Kernel Primitives Prelude ---
pub type uid_t = u32;
pub type gid_t = u32;
pub type uid16_t = u16;
pub type gid16_t = u16;
pub type pid_t = i32;
pub type mode_t = u32;
pub type umode_t = u16;
pub type nlink_t = u32;
pub type off_t = i64;
pub type loff_t = i64;
pub type dev_t = u32;
pub type ino_t = u64;
pub type size_t = usize;
pub type ssize_t = isize;
pub type uintptr_t = usize;
pub type intptr_t = isize;
pub type ptrdiff_t = isize;
pub type clockid_t = i32;
pub type timer_t = i32;
pub type time64_t = i64;
pub type atomic_t = core::sync::atomic::AtomicI32;
pub type atomic64_t = core::sync::atomic::AtomicI64;
// ---------------------------------------


// SPDX-License-Identifier: GPL-2.0-or-later
//

//
// Define the firmware names used by the driver.
// Those should match what's used at smscoreapi.c and sms-cards.c
// including the MODULE_FIRMWARE() macros at the end of smscoreapi.c
//

pub const SMS_ALLOC_ALIGNMENT: c_int = 128;
pub const SMS_DMA_ALIGNMENT: c_int = 16;

pub const SMS_DEVICE_FAMILY1: c_int = 0;
pub const SMS_DEVICE_FAMILY2: c_int = 1;
pub const SMS_ROM_NO_RESPONSE: c_int = 2;
pub const SMS_DEVICE_NOT_READY: c_uint = 0x8000000;
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum sms_device_type_st {
    SMS_UNKNOWN_TYPE = -1,
    SMS_STELLAR = 0,
    SMS_NOVA_A0,
    SMS_NOVA_B0,
    SMS_VEGA,
    SMS_VENICE,
    SMS_MING,
    SMS_PELE,
    SMS_RIO,
    SMS_DENVER_1530,
    SMS_DENVER_2160,
    SMS_NUM_OF_DEVICE_TYPES
}

#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum sms_power_mode_st {
    SMS_POWER_MODE_ACTIVE,
    SMS_POWER_MODE_SUSPENDED
}

extern "C" {
    pub fn int(context: *mut *mut setmode_t)(void, mode: c_int) -> typedef;
}
extern "C" {
    pub fn void(context: *mut *mut detectmode_t)(void, mode: *mut c_int) -> typedef;
}
extern "C" {
    pub fn int(context: *mut *mut sendrequest_t)(void, buffer: *mut c_void, size: usize) -> typedef;
}
extern "C" {
    pub fn int(context: *mut *mut preload_t)(void) -> typedef;
}
extern "C" {
    pub fn int(context: *mut *mut postload_t)(void) -> typedef;
}
extern "C" {
    pub fn int(context: *mut *mut onresponse_t)(void, cb: *mut smscore_buffer_t) -> typedef;
}
extern "C" {
    pub fn void(context: *mut *mut onremove_t)(void) -> typedef;
}
#[repr(C)]
#[derive(Copy, Clone)]
pub struct smscore_buffer_t {
// public members, once passed to clients can be changed freely
    pub entry: list_head,
    pub size: c_int,
    pub offset: c_int,
// private members, read-only for clients
    pub p: *mut c_void,
    pub phys: dma_addr_t,
    pub offset_in_common: c_ulong,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct smsdevice_params_t {
    pub device: *mut device,
    pub usb_device: *mut usb_device,
    pub buffer_size: c_int,
    pub num_buffers: c_int,
    pub devpath: [c_char; 32],
    pub flags: c_ulong,
    pub setmode_handler: setmode_t,
    pub detectmode_handler: detectmode_t,
    pub sendrequest_handler: sendrequest_t,
    pub preload_handler: preload_t,
    pub postload_handler: postload_t,
    pub context: *mut c_void,
    pub device_type: sms_device_type_st,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct smsclient_params_t {
    pub initial_id: c_int,
    pub data_type: c_int,
    pub onresponse_handler: onresponse_t,
    pub onremove_handler: onremove_t,
    pub context: *mut c_void,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct smscore_device_t {
    pub entry: list_head,
    pub clients: list_head,
    pub subclients: list_head,
    pub clientslock: spinlock_t,
    pub buffers: list_head,
    pub bufferslock: spinlock_t,
    pub num_buffers: c_int,
    pub common_buffer: *mut c_void,
    pub common_buffer_size: c_int,
    pub common_buffer_phys: dma_addr_t,
    pub context: *mut c_void,
    pub device: *mut device,
    pub usb_device: *mut usb_device,
    pub devpath: [c_char; 32],
    pub device_flags: c_ulong,
    pub setmode_handler: setmode_t,
    pub detectmode_handler: detectmode_t,
    pub sendrequest_handler: sendrequest_t,
    pub preload_handler: preload_t,
    pub postload_handler: postload_t,
    pub modes_supported: int mode,,
    pub gfp_buf_flags: gfp_t,
// host <--> device messages
    pub trigger_done: completion version_ex_done, data_download_done,,
    pub device_ready_done: completion data_validity_done,,
    pub resume_done: completion init_device_done, reload_start_done,,
    pub gpio_set_level_done: completion gpio_configuration_done,,
    pub ir_init_done: completion gpio_get_level_done,,
// Buffer management
    pub buffer_mng_waitq: wait_queue_head_t,
// GPIO
    pub gpio_get_res: c_int,
// Target hardware board
    pub board_id: c_int,
// Firmware
    pub fw_buf: *mut u8,
    pub fw_buf_size: u32,
    pub fw_version: u16,
// Infrared (IR)
    pub ir: ir_t,
//
// Identify if device is USB or not.
// Used by smsdvb-sysfs to know the root node for debugfs
//
    pub is_usb_device: bool,
    pub led_state: c_int,

    pub media_dev: *mut media_device,

}

// GPIO definitions for antenna frequency domain control (SMS8021)
pub const SMS_ANTENNA_GPIO_0: c_int = 1;
pub const SMS_ANTENNA_GPIO_1: c_int = 0;
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum sms_bandwidth_mode {
    BW_8_MHZ = 0,
    BW_7_MHZ = 1,
    BW_6_MHZ = 2,
    BW_5_MHZ = 3,
    BW_ISDBT_1SEG = 4,
    BW_ISDBT_3SEG = 5,
    BW_2_MHZ = 6,
    BW_FM_RADIO = 7,
    BW_ISDBT_13SEG = 8,
    BW_1_5_MHZ = 15,
    BW_UNKNOWN = 0xffff
}

pub const MSG_HDR_FLAG_SPLIT_MSG: c_int = 4;
pub const MAX_GPIO_PIN_NUMBER: c_int = 31;
pub const HIF_TASK: c_int = 11;
pub const HIF_TASK_SLAVE: c_int = 22;
pub const HIF_TASK_SLAVE2: c_int = 33;
pub const HIF_TASK_SLAVE3: c_int = 44;
pub const SMS_HOST_LIB: c_int = 150;
pub const DVBT_BDA_CONTROL_MSG_ID: c_int = 201;
pub const SMS_MAX_PAYLOAD_SIZE: c_int = 240;
pub const SMS_TUNE_TIMEOUT: c_int = 500;
#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum msg_types {
    MSG_TYPE_BASE_VAL = 500,
    MSG_SMS_GET_VERSION_REQ = 503,
    MSG_SMS_GET_VERSION_RES = 504,
    MSG_SMS_MULTI_BRIDGE_CFG = 505,
    MSG_SMS_GPIO_CONFIG_REQ = 507,
    MSG_SMS_GPIO_CONFIG_RES = 508,
    MSG_SMS_GPIO_SET_LEVEL_REQ = 509,
    MSG_SMS_GPIO_SET_LEVEL_RES = 510,
    MSG_SMS_GPIO_GET_LEVEL_REQ = 511,
    MSG_SMS_GPIO_GET_LEVEL_RES = 512,
    MSG_SMS_EEPROM_BURN_IND = 513,
    MSG_SMS_LOG_ENABLE_CHANGE_REQ = 514,
    MSG_SMS_LOG_ENABLE_CHANGE_RES = 515,
    MSG_SMS_SET_MAX_TX_MSG_LEN_REQ = 516,
    MSG_SMS_SET_MAX_TX_MSG_LEN_RES = 517,
    MSG_SMS_SPI_HALFDUPLEX_TOKEN_HOST_TO_DEVICE = 518,
    MSG_SMS_SPI_HALFDUPLEX_TOKEN_DEVICE_TO_HOST = 519,
    MSG_SMS_BACKGROUND_SCAN_FLAG_CHANGE_REQ = 520,
    MSG_SMS_BACKGROUND_SCAN_FLAG_CHANGE_RES = 521,
    MSG_SMS_BACKGROUND_SCAN_SIGNAL_DETECTED_IND = 522,
    MSG_SMS_BACKGROUND_SCAN_NO_SIGNAL_IND = 523,
    MSG_SMS_CONFIGURE_RF_SWITCH_REQ = 524,
    MSG_SMS_CONFIGURE_RF_SWITCH_RES = 525,
    MSG_SMS_MRC_PATH_DISCONNECT_REQ = 526,
    MSG_SMS_MRC_PATH_DISCONNECT_RES = 527,
    MSG_SMS_RECEIVE_1SEG_THROUGH_FULLSEG_REQ = 528,
    MSG_SMS_RECEIVE_1SEG_THROUGH_FULLSEG_RES = 529,
    MSG_SMS_RECEIVE_VHF_VIA_VHF_INPUT_REQ = 530,
    MSG_SMS_RECEIVE_VHF_VIA_VHF_INPUT_RES = 531,
    MSG_WR_REG_RFT_REQ = 533,
    MSG_WR_REG_RFT_RES = 534,
    MSG_RD_REG_RFT_REQ = 535,
    MSG_RD_REG_RFT_RES = 536,
    MSG_RD_REG_ALL_RFT_REQ = 537,
    MSG_RD_REG_ALL_RFT_RES = 538,
    MSG_HELP_INT = 539,
    MSG_RUN_SCRIPT_INT = 540,
    MSG_SMS_EWS_INBAND_REQ = 541,
    MSG_SMS_EWS_INBAND_RES = 542,
    MSG_SMS_RFS_SELECT_REQ = 543,
    MSG_SMS_RFS_SELECT_RES = 544,
    MSG_SMS_MB_GET_VER_REQ = 545,
    MSG_SMS_MB_GET_VER_RES = 546,
    MSG_SMS_MB_WRITE_CFGFILE_REQ = 547,
    MSG_SMS_MB_WRITE_CFGFILE_RES = 548,
    MSG_SMS_MB_READ_CFGFILE_REQ = 549,
    MSG_SMS_MB_READ_CFGFILE_RES = 550,
    MSG_SMS_RD_MEM_REQ = 552,
    MSG_SMS_RD_MEM_RES = 553,
    MSG_SMS_WR_MEM_REQ = 554,
    MSG_SMS_WR_MEM_RES = 555,
    MSG_SMS_UPDATE_MEM_REQ = 556,
    MSG_SMS_UPDATE_MEM_RES = 557,
    MSG_SMS_ISDBT_ENABLE_FULL_PARAMS_SET_REQ = 558,
    MSG_SMS_ISDBT_ENABLE_FULL_PARAMS_SET_RES = 559,
    MSG_SMS_RF_TUNE_REQ = 561,
    MSG_SMS_RF_TUNE_RES = 562,
    MSG_SMS_ISDBT_ENABLE_HIGH_MOBILITY_REQ = 563,
    MSG_SMS_ISDBT_ENABLE_HIGH_MOBILITY_RES = 564,
    MSG_SMS_ISDBT_SB_RECEPTION_REQ = 565,
    MSG_SMS_ISDBT_SB_RECEPTION_RES = 566,
    MSG_SMS_GENERIC_EPROM_WRITE_REQ = 567,
    MSG_SMS_GENERIC_EPROM_WRITE_RES = 568,
    MSG_SMS_GENERIC_EPROM_READ_REQ = 569,
    MSG_SMS_GENERIC_EPROM_READ_RES = 570,
    MSG_SMS_EEPROM_WRITE_REQ = 571,
    MSG_SMS_EEPROM_WRITE_RES = 572,
    MSG_SMS_CUSTOM_READ_REQ = 574,
    MSG_SMS_CUSTOM_READ_RES = 575,
    MSG_SMS_CUSTOM_WRITE_REQ = 576,
    MSG_SMS_CUSTOM_WRITE_RES = 577,
    MSG_SMS_INIT_DEVICE_REQ = 578,
    MSG_SMS_INIT_DEVICE_RES = 579,
    MSG_SMS_ATSC_SET_ALL_IP_REQ = 580,
    MSG_SMS_ATSC_SET_ALL_IP_RES = 581,
    MSG_SMS_ATSC_START_ENSEMBLE_REQ = 582,
    MSG_SMS_ATSC_START_ENSEMBLE_RES = 583,
    MSG_SMS_SET_OUTPUT_MODE_REQ = 584,
    MSG_SMS_SET_OUTPUT_MODE_RES = 585,
    MSG_SMS_ATSC_IP_FILTER_GET_LIST_REQ = 586,
    MSG_SMS_ATSC_IP_FILTER_GET_LIST_RES = 587,
    MSG_SMS_SUB_CHANNEL_START_REQ = 589,
    MSG_SMS_SUB_CHANNEL_START_RES = 590,
    MSG_SMS_SUB_CHANNEL_STOP_REQ = 591,
    MSG_SMS_SUB_CHANNEL_STOP_RES = 592,
    MSG_SMS_ATSC_IP_FILTER_ADD_REQ = 593,
    MSG_SMS_ATSC_IP_FILTER_ADD_RES = 594,
    MSG_SMS_ATSC_IP_FILTER_REMOVE_REQ = 595,
    MSG_SMS_ATSC_IP_FILTER_REMOVE_RES = 596,
    MSG_SMS_ATSC_IP_FILTER_REMOVE_ALL_REQ = 597,
    MSG_SMS_ATSC_IP_FILTER_REMOVE_ALL_RES = 598,
    MSG_SMS_WAIT_CMD = 599,
    MSG_SMS_ADD_PID_FILTER_REQ = 601,
    MSG_SMS_ADD_PID_FILTER_RES = 602,
    MSG_SMS_REMOVE_PID_FILTER_REQ = 603,
    MSG_SMS_REMOVE_PID_FILTER_RES = 604,
    MSG_SMS_FAST_INFORMATION_CHANNEL_REQ = 605,
    MSG_SMS_FAST_INFORMATION_CHANNEL_RES = 606,
    MSG_SMS_DAB_CHANNEL = 607,
    MSG_SMS_GET_PID_FILTER_LIST_REQ = 608,
    MSG_SMS_GET_PID_FILTER_LIST_RES = 609,
    MSG_SMS_POWER_DOWN_REQ = 610,
    MSG_SMS_POWER_DOWN_RES = 611,
    MSG_SMS_ATSC_SLT_EXIST_IND = 612,
    MSG_SMS_ATSC_NO_SLT_IND = 613,
    MSG_SMS_GET_STATISTICS_REQ = 615,
    MSG_SMS_GET_STATISTICS_RES = 616,
    MSG_SMS_SEND_DUMP = 617,
    MSG_SMS_SCAN_START_REQ = 618,
    MSG_SMS_SCAN_START_RES = 619,
    MSG_SMS_SCAN_STOP_REQ = 620,
    MSG_SMS_SCAN_STOP_RES = 621,
    MSG_SMS_SCAN_PROGRESS_IND = 622,
    MSG_SMS_SCAN_COMPLETE_IND = 623,
    MSG_SMS_LOG_ITEM = 624,
    MSG_SMS_DAB_SUBCHANNEL_RECONFIG_REQ = 628,
    MSG_SMS_DAB_SUBCHANNEL_RECONFIG_RES = 629,
    MSG_SMS_HO_PER_SLICES_IND = 630,
    MSG_SMS_HO_INBAND_POWER_IND = 631,
    MSG_SMS_MANUAL_DEMOD_REQ = 632,
    MSG_SMS_HO_TUNE_ON_REQ = 636,
    MSG_SMS_HO_TUNE_ON_RES = 637,
    MSG_SMS_HO_TUNE_OFF_REQ = 638,
    MSG_SMS_HO_TUNE_OFF_RES = 639,
    MSG_SMS_HO_PEEK_FREQ_REQ = 640,
    MSG_SMS_HO_PEEK_FREQ_RES = 641,
    MSG_SMS_HO_PEEK_FREQ_IND = 642,
    MSG_SMS_MB_ATTEN_SET_REQ = 643,
    MSG_SMS_MB_ATTEN_SET_RES = 644,
    MSG_SMS_ENABLE_STAT_IN_I2C_REQ = 649,
    MSG_SMS_ENABLE_STAT_IN_I2C_RES = 650,
    MSG_SMS_SET_ANTENNA_CONFIG_REQ = 651,
    MSG_SMS_SET_ANTENNA_CONFIG_RES = 652,
    MSG_SMS_GET_STATISTICS_EX_REQ = 653,
    MSG_SMS_GET_STATISTICS_EX_RES = 654,
    MSG_SMS_SLEEP_RESUME_COMP_IND = 655,
    MSG_SMS_SWITCH_HOST_INTERFACE_REQ = 656,
    MSG_SMS_SWITCH_HOST_INTERFACE_RES = 657,
    MSG_SMS_DATA_DOWNLOAD_REQ = 660,
    MSG_SMS_DATA_DOWNLOAD_RES = 661,
    MSG_SMS_DATA_VALIDITY_REQ = 662,
    MSG_SMS_DATA_VALIDITY_RES = 663,
    MSG_SMS_SWDOWNLOAD_TRIGGER_REQ = 664,
    MSG_SMS_SWDOWNLOAD_TRIGGER_RES = 665,
    MSG_SMS_SWDOWNLOAD_BACKDOOR_REQ = 666,
    MSG_SMS_SWDOWNLOAD_BACKDOOR_RES = 667,
    MSG_SMS_GET_VERSION_EX_REQ = 668,
    MSG_SMS_GET_VERSION_EX_RES = 669,
    MSG_SMS_CLOCK_OUTPUT_CONFIG_REQ = 670,
    MSG_SMS_CLOCK_OUTPUT_CONFIG_RES = 671,
    MSG_SMS_I2C_SET_FREQ_REQ = 685,
    MSG_SMS_I2C_SET_FREQ_RES = 686,
    MSG_SMS_GENERIC_I2C_REQ = 687,
    MSG_SMS_GENERIC_I2C_RES = 688,
    MSG_SMS_DVBT_BDA_DATA = 693,
    MSG_SW_RELOAD_REQ = 697,
    MSG_SMS_DATA_MSG = 699,
    MSG_TABLE_UPLOAD_REQ = 700,
    MSG_TABLE_UPLOAD_RES = 701,
    MSG_SW_RELOAD_START_REQ = 702,
    MSG_SW_RELOAD_START_RES = 703,
    MSG_SW_RELOAD_EXEC_REQ = 704,
    MSG_SW_RELOAD_EXEC_RES = 705,
    MSG_SMS_SPI_INT_LINE_SET_REQ = 710,
    MSG_SMS_SPI_INT_LINE_SET_RES = 711,
    MSG_SMS_GPIO_CONFIG_EX_REQ = 712,
    MSG_SMS_GPIO_CONFIG_EX_RES = 713,
    MSG_SMS_WATCHDOG_ACT_REQ = 716,
    MSG_SMS_WATCHDOG_ACT_RES = 717,
    MSG_SMS_LOOPBACK_REQ = 718,
    MSG_SMS_LOOPBACK_RES = 719,
    MSG_SMS_RAW_CAPTURE_START_REQ = 720,
    MSG_SMS_RAW_CAPTURE_START_RES = 721,
    MSG_SMS_RAW_CAPTURE_ABORT_REQ = 722,
    MSG_SMS_RAW_CAPTURE_ABORT_RES = 723,
    MSG_SMS_RAW_CAPTURE_COMPLETE_IND = 728,
    MSG_SMS_DATA_PUMP_IND = 729,
    MSG_SMS_DATA_PUMP_REQ = 730,
    MSG_SMS_DATA_PUMP_RES = 731,
    MSG_SMS_FLASH_DL_REQ = 732,
    MSG_SMS_EXEC_TEST_1_REQ = 734,
    MSG_SMS_EXEC_TEST_1_RES = 735,
    MSG_SMS_ENABLE_TS_INTERFACE_REQ = 736,
    MSG_SMS_ENABLE_TS_INTERFACE_RES = 737,
    MSG_SMS_SPI_SET_BUS_WIDTH_REQ = 738,
    MSG_SMS_SPI_SET_BUS_WIDTH_RES = 739,
    MSG_SMS_SEND_EMM_REQ = 740,
    MSG_SMS_SEND_EMM_RES = 741,
    MSG_SMS_DISABLE_TS_INTERFACE_REQ = 742,
    MSG_SMS_DISABLE_TS_INTERFACE_RES = 743,
    MSG_SMS_IS_BUF_FREE_REQ = 744,
    MSG_SMS_IS_BUF_FREE_RES = 745,
    MSG_SMS_EXT_ANTENNA_REQ = 746,
    MSG_SMS_EXT_ANTENNA_RES = 747,
    MSG_SMS_CMMB_GET_NET_OF_FREQ_REQ_OBSOLETE = 748,
    MSG_SMS_CMMB_GET_NET_OF_FREQ_RES_OBSOLETE = 749,
    MSG_SMS_BATTERY_LEVEL_REQ = 750,
    MSG_SMS_BATTERY_LEVEL_RES = 751,
    MSG_SMS_CMMB_INJECT_TABLE_REQ_OBSOLETE = 752,
    MSG_SMS_CMMB_INJECT_TABLE_RES_OBSOLETE = 753,
    MSG_SMS_FM_RADIO_BLOCK_IND = 754,
    MSG_SMS_HOST_NOTIFICATION_IND = 755,
    MSG_SMS_CMMB_GET_CONTROL_TABLE_REQ_OBSOLETE = 756,
    MSG_SMS_CMMB_GET_CONTROL_TABLE_RES_OBSOLETE = 757,
    MSG_SMS_CMMB_GET_NETWORKS_REQ = 760,
    MSG_SMS_CMMB_GET_NETWORKS_RES = 761,
    MSG_SMS_CMMB_START_SERVICE_REQ = 762,
    MSG_SMS_CMMB_START_SERVICE_RES = 763,
    MSG_SMS_CMMB_STOP_SERVICE_REQ = 764,
    MSG_SMS_CMMB_STOP_SERVICE_RES = 765,
    MSG_SMS_CMMB_ADD_CHANNEL_FILTER_REQ = 768,
    MSG_SMS_CMMB_ADD_CHANNEL_FILTER_RES = 769,
    MSG_SMS_CMMB_REMOVE_CHANNEL_FILTER_REQ = 770,
    MSG_SMS_CMMB_REMOVE_CHANNEL_FILTER_RES = 771,
    MSG_SMS_CMMB_START_CONTROL_INFO_REQ = 772,
    MSG_SMS_CMMB_START_CONTROL_INFO_RES = 773,
    MSG_SMS_CMMB_STOP_CONTROL_INFO_REQ = 774,
    MSG_SMS_CMMB_STOP_CONTROL_INFO_RES = 775,
    MSG_SMS_ISDBT_TUNE_REQ = 776,
    MSG_SMS_ISDBT_TUNE_RES = 777,
    MSG_SMS_TRANSMISSION_IND = 782,
    MSG_SMS_PID_STATISTICS_IND = 783,
    MSG_SMS_POWER_DOWN_IND = 784,
    MSG_SMS_POWER_DOWN_CONF = 785,
    MSG_SMS_POWER_UP_IND = 786,
    MSG_SMS_POWER_UP_CONF = 787,
    MSG_SMS_POWER_MODE_SET_REQ = 790,
    MSG_SMS_POWER_MODE_SET_RES = 791,
    MSG_SMS_DEBUG_HOST_EVENT_REQ = 792,
    MSG_SMS_DEBUG_HOST_EVENT_RES = 793,
    MSG_SMS_NEW_CRYSTAL_REQ = 794,
    MSG_SMS_NEW_CRYSTAL_RES = 795,
    MSG_SMS_CONFIG_SPI_REQ = 796,
    MSG_SMS_CONFIG_SPI_RES = 797,
    MSG_SMS_I2C_SHORT_STAT_IND = 798,
    MSG_SMS_START_IR_REQ = 800,
    MSG_SMS_START_IR_RES = 801,
    MSG_SMS_IR_SAMPLES_IND = 802,
    MSG_SMS_CMMB_CA_SERVICE_IND = 803,
    MSG_SMS_SLAVE_DEVICE_DETECTED = 804,
    MSG_SMS_INTERFACE_LOCK_IND = 805,
    MSG_SMS_INTERFACE_UNLOCK_IND = 806,
    MSG_SMS_SEND_ROSUM_BUFF_REQ = 810,
    MSG_SMS_SEND_ROSUM_BUFF_RES = 811,
    MSG_SMS_ROSUM_BUFF = 812,
    MSG_SMS_SET_AES128_KEY_REQ = 815,
    MSG_SMS_SET_AES128_KEY_RES = 816,
    MSG_SMS_MBBMS_WRITE_REQ = 817,
    MSG_SMS_MBBMS_WRITE_RES = 818,
    MSG_SMS_MBBMS_READ_IND = 819,
    MSG_SMS_IQ_STREAM_START_REQ = 820,
    MSG_SMS_IQ_STREAM_START_RES = 821,
    MSG_SMS_IQ_STREAM_STOP_REQ = 822,
    MSG_SMS_IQ_STREAM_STOP_RES = 823,
    MSG_SMS_IQ_STREAM_DATA_BLOCK = 824,
    MSG_SMS_GET_EEPROM_VERSION_REQ = 825,
    MSG_SMS_GET_EEPROM_VERSION_RES = 826,
    MSG_SMS_SIGNAL_DETECTED_IND = 827,
    MSG_SMS_NO_SIGNAL_IND = 828,
    MSG_SMS_MRC_SHUTDOWN_SLAVE_REQ = 830,
    MSG_SMS_MRC_SHUTDOWN_SLAVE_RES = 831,
    MSG_SMS_MRC_BRINGUP_SLAVE_REQ = 832,
    MSG_SMS_MRC_BRINGUP_SLAVE_RES = 833,
    MSG_SMS_EXTERNAL_LNA_CTRL_REQ = 834,
    MSG_SMS_EXTERNAL_LNA_CTRL_RES = 835,
    MSG_SMS_SET_PERIODIC_STATISTICS_REQ = 836,
    MSG_SMS_SET_PERIODIC_STATISTICS_RES = 837,
    MSG_SMS_CMMB_SET_AUTO_OUTPUT_TS0_REQ = 838,
    MSG_SMS_CMMB_SET_AUTO_OUTPUT_TS0_RES = 839,
    LOCAL_TUNE = 850,
    LOCAL_IFFT_H_ICI = 851,
    MSG_RESYNC_REQ = 852,
    MSG_SMS_CMMB_GET_MRC_STATISTICS_REQ = 853,
    MSG_SMS_CMMB_GET_MRC_STATISTICS_RES = 854,
    MSG_SMS_LOG_EX_ITEM = 855,
    MSG_SMS_DEVICE_DATA_LOSS_IND = 856,
    MSG_SMS_MRC_WATCHDOG_TRIGGERED_IND = 857,
    MSG_SMS_USER_MSG_REQ = 858,
    MSG_SMS_USER_MSG_RES = 859,
    MSG_SMS_SMART_CARD_INIT_REQ = 860,
    MSG_SMS_SMART_CARD_INIT_RES = 861,
    MSG_SMS_SMART_CARD_WRITE_REQ = 862,
    MSG_SMS_SMART_CARD_WRITE_RES = 863,
    MSG_SMS_SMART_CARD_READ_IND = 864,
    MSG_SMS_TSE_ENABLE_REQ = 866,
    MSG_SMS_TSE_ENABLE_RES = 867,
    MSG_SMS_CMMB_GET_SHORT_STATISTICS_REQ = 868,
    MSG_SMS_CMMB_GET_SHORT_STATISTICS_RES = 869,
    MSG_SMS_LED_CONFIG_REQ = 870,
    MSG_SMS_LED_CONFIG_RES = 871,
    MSG_PWM_ANTENNA_REQ = 872,
    MSG_PWM_ANTENNA_RES = 873,
    MSG_SMS_CMMB_SMD_SN_REQ = 874,
    MSG_SMS_CMMB_SMD_SN_RES = 875,
    MSG_SMS_CMMB_SET_CA_CW_REQ = 876,
    MSG_SMS_CMMB_SET_CA_CW_RES = 877,
    MSG_SMS_CMMB_SET_CA_SALT_REQ = 878,
    MSG_SMS_CMMB_SET_CA_SALT_RES = 879,
    MSG_SMS_NSCD_INIT_REQ = 880,
    MSG_SMS_NSCD_INIT_RES = 881,
    MSG_SMS_NSCD_PROCESS_SECTION_REQ = 882,
    MSG_SMS_NSCD_PROCESS_SECTION_RES = 883,
    MSG_SMS_DBD_CREATE_OBJECT_REQ = 884,
    MSG_SMS_DBD_CREATE_OBJECT_RES = 885,
    MSG_SMS_DBD_CONFIGURE_REQ = 886,
    MSG_SMS_DBD_CONFIGURE_RES = 887,
    MSG_SMS_DBD_SET_KEYS_REQ = 888,
    MSG_SMS_DBD_SET_KEYS_RES = 889,
    MSG_SMS_DBD_PROCESS_HEADER_REQ = 890,
    MSG_SMS_DBD_PROCESS_HEADER_RES = 891,
    MSG_SMS_DBD_PROCESS_DATA_REQ = 892,
    MSG_SMS_DBD_PROCESS_DATA_RES = 893,
    MSG_SMS_DBD_PROCESS_GET_DATA_REQ = 894,
    MSG_SMS_DBD_PROCESS_GET_DATA_RES = 895,
    MSG_SMS_NSCD_OPEN_SESSION_REQ = 896,
    MSG_SMS_NSCD_OPEN_SESSION_RES = 897,
    MSG_SMS_SEND_HOST_DATA_TO_DEMUX_REQ = 898,
    MSG_SMS_SEND_HOST_DATA_TO_DEMUX_RES = 899,
    MSG_LAST_MSG_TYPE = 900,
}

#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum SMS_DVB3_EVENTS {
    DVB3_EVENT_INIT = 0,
    DVB3_EVENT_SLEEP,
    DVB3_EVENT_HOTPLUG,
    DVB3_EVENT_FE_LOCK,
    DVB3_EVENT_FE_UNLOCK,
    DVB3_EVENT_UNC_OK,
    DVB3_EVENT_UNC_ERR
}

#[repr(C)]
#[derive(Copy, Clone, PartialEq, Eq)]
pub enum SMS_DEVICE_MODE {
    DEVICE_MODE_NONE = -1,
    DEVICE_MODE_DVBT = 0,
    DEVICE_MODE_DVBH,
    DEVICE_MODE_DAB_TDMB,
    DEVICE_MODE_DAB_TDMB_DABIP,
    DEVICE_MODE_DVBT_BDA,
    DEVICE_MODE_ISDBT,
    DEVICE_MODE_ISDBT_BDA,
    DEVICE_MODE_CMMB,
    DEVICE_MODE_RAW_TUNER,
    DEVICE_MODE_FM_RADIO,
    DEVICE_MODE_FM_RADIO_BDA,
    DEVICE_MODE_ATSC,
    DEVICE_MODE_MAX,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_msg_hdr {
    pub msg_type: u16,
    pub msg_src_id: u8,
    pub msg_dst_id: u8,
    pub /: *mut *mut u16 msg_length; / length of entire message, including header,
    pub msg_flags: u16,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_msg_data {
    pub x_msg_header: sms_msg_hdr,
    pub msg_data: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_msg_data2 {
    pub x_msg_header: sms_msg_hdr,
    pub msg_data: [u32; 2],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_msg_data5 {
    pub x_msg_header: sms_msg_hdr,
    pub msg_data: [u32; 5],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_data_download {
    pub x_msg_header: sms_msg_hdr,
    pub mem_addr: u32,
    pub payload: [u8; SMS_MAX_PAYLOAD_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_version_res {
    pub x_msg_header: sms_msg_hdr,
    pub /: *mut *mut u16 chip_model; / e.g. 0x1102 for SMS-1102 "Nova",
    pub /: *mut *mut u8 step; / 0 - step A,
    pub /: *mut *mut u8 metal_fix; / 0 - Metal 0,
// firmware_id 0xFF if ROM, otherwise the
// value indicated by SMSHOSTLIB_DEVICE_MODES_E
    pub firmware_id: u8,
// supported_protocols Bitwise OR combination of
// supported protocols
    pub supported_protocols: u8,
    pub version_major: u8,
    pub version_minor: u8,
    pub version_patch: u8,
    pub version_field_patch: u8,
    pub rom_ver_major: u8,
    pub rom_ver_minor: u8,
    pub rom_ver_patch: u8,
    pub rom_ver_field_patch: u8,
    pub TextLabel: [u8; 34],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_firmware {
    pub check_sum: u32,
    pub length: u32,
    pub start_address: u32,
    pub payload: [u8; ],
}

// statistics information returned as response for
// SmsHostApiGetstatistics_Req
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_stats {
    pub /: *mut *mut u32 reserved; / reserved,
// Common parameters
    pub /: *mut *mut u32 is_rf_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_demod_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_external_lna_on; / 0 - external LNA off, 1 - external LNA on,
// Reception quality
    pub /: *mut *mut s32 SNR; / dB,
    pub /: *mut *mut u32 ber; / Post Viterbi ber [1E-5],
    pub /: *mut *mut u32 FIB_CRC; / CRC errors percentage, valid only for DAB,
    pub PER,: *mut *mut u32 ts_per; / Transport stream,
    pub percentage,: *mut *mut u32 MFER; / DVB-H frame error rate in,
    pub /: *mut *mut s32 RSSI; / dBm,
    pub /: *mut *mut s32 in_band_pwr; / In band power in dBM,
    pub /: *mut *mut s32 carrier_offset; / Carrier Offset in bin/1024,
// Transmission parameters
    pub /: *mut *mut u32 frequency; / frequency in Hz,
    pub /: *mut *mut u32 bandwidth; / bandwidth in MHz, valid only for DVB-T/H,
    pub 1-4,: *mut *mut u32 transmission_mode; / Transmission Mode, for DAB modes,
    pub SMSHOSTLIB_DVB_MODEM_STATE_ET,: *mut *mut u32 modem_state; / from,
    pub from: *mut *mut u32 guard_interval; / Guard Interval,
    pub SMSHOSTLIB_CODE_RATE_ET,: *mut *mut u32 code_rate; / Code Rate from,
    pub from: *mut *mut u32 lp_code_rate; / Low Priority Code Rate,
    pub SMSHOSTLIB_HIERARCHY_ET,: *mut *mut u32 hierarchy; / hierarchy from,
    pub from: *mut *mut u32 constellation; / constellation,
// Burst parameters, valid only for DVB-H
    pub bytes,: *mut *mut u32 burst_size; / Current burst size in,
    pub mSec,: *mut *mut u32 burst_duration; / Current burst duration in,
    pub mSec,: *mut *mut u32 burst_cycle_time; / Current burst cycle time in,
    pub mSec,: *mut *mut u32 calc_burst_cycle_time;/ Current burst cycle time in,
    pub table,: *mut *mut u32 num_of_rows; / Number of rows in MPE,
    pub table,: *mut *mut u32 num_of_padd_cols; / Number of padding columns in MPE,
    pub table,: *mut *mut u32 num_of_punct_cols; / Number of puncturing columns in MPE,
    pub erroneous: *mut *mut u32 error_ts_packets; / Number of,
    pub /: *mut *mut u32 total_ts_packets; / Total number of transport-stream packets,
    pub include: *mut *mut u32 num_of_valid_mpe_tlbs; / Number of MPE tables which do not,
    pub errors: *mut *mut u32 num_of_invalid_mpe_tlbs;/ Number of MPE tables which include,
    pub were: *mut *mut u32 num_of_corrected_mpe_tlbs;/ Number of MPE tables which,
// Common params
    pub /: *mut *mut u32 ber_error_count; / Number of erroneous SYNC bits.,
    pub /: *mut *mut u32 ber_bit_count; / Total number of SYNC bits.,
// Interface information
    pub /: *mut *mut u32 sms_to_host_tx_errors; / Total number of transmission errors.,
// DAB/T-DMB
    pub /: *mut *mut u32 pre_ber; / DAB/T-DMB only: Pre Viterbi ber [1E-5],
// DVB-H TPS parameters
    pub zero: *mut *mut u32 cell_id; / TPS Cell ID in bits 15..0, bits 31..16,
    pub -: *mut *mut u32 dvbh_srv_ind_hp; / DVB-H service indication info, bit 1,
    pub -: *mut *mut u32 dvbh_srv_ind_lp; / DVB-H service indication info, bit 1,
    pub /: *mut *mut u32 num_mpe_received; / DVB-H, Num MPE section received,
    pub /: *mut *mut u32 reservedFields[10]; / reserved,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_msg_statistics_info {
    pub request_result: u32,
    pub stat: sms_stats,
// Split the calc of the SNR in DAB
    pub /: *mut *mut u32 signal; / dB,
    pub /: *mut *mut u32 noise; / dB,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_isdbt_layer_stats {
// Per-layer information
    pub SMSHOSTLIB_CODE_RATE_ET,: *mut *mut u32 code_rate; / Code Rate from,
// 255 means layer does not exist
    pub SMSHOSTLIB_CONSTELLATION_ET,: *mut *mut u32 constellation; / constellation from,
// 255 means layer does not exist
    pub /: *mut *mut u32 ber; / Post Viterbi ber [1E-5], 0xFFFFFFFF indicate N/A,
    pub /: *mut *mut u32 ber_error_count; / Post Viterbi Error Bits Count,
    pub /: *mut *mut u32 ber_bit_count; / Post Viterbi Total Bits Count,
    pub /: *mut *mut u32 pre_ber; / Pre Viterbi ber [1E-5], 0xFFFFFFFF indicate N/A,
    pub /: *mut *mut u32 ts_per; / Transport stream PER [%], 0xFFFFFFFF indicate N/A,
    pub /: *mut *mut u32 error_ts_packets; / Number of erroneous transport-stream packets,
    pub /: *mut *mut u32 total_ts_packets; / Total number of transport-stream packets,
    pub parameter,: *mut *mut u32 ti_ldepth_i; / Time interleaver depth I,
// 255 means layer does not exist
    pub A,: *mut *mut u32 number_of_segments; / Number of segments in layer,
// 255 means layer does not exist
    pub /: *mut *mut u32 tmcc_errors; / TMCC errors,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_isdbt_stats {
    pub the: *mut *mut u32 statistics_type; / Enumerator identifying the type of,
// structure.  Values are the same as
// SMSHOSTLIB_DEVICE_MODES_E
//
// This field MUST always be first in any
// statistics structure
    pub modem.: *mut *mut u32 full_size; / Total size of the structure returned by the,
// If the size requested by the host is smaller than
// full_size, the struct will be truncated
// Common parameters
    pub /: *mut *mut u32 is_rf_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_demod_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_external_lna_on; / 0 - external LNA off, 1 - external LNA on,
// Reception quality
    pub /: *mut *mut s32 SNR; / dB,
    pub /: *mut *mut s32 RSSI; / dBm,
    pub /: *mut *mut s32 in_band_pwr; / In band power in dBM,
    pub /: *mut *mut s32 carrier_offset; / Carrier Offset in Hz,
// Transmission parameters
    pub /: *mut *mut u32 frequency; / frequency in Hz,
    pub /: *mut *mut u32 bandwidth; / bandwidth in MHz,
    pub /: *mut *mut u32 transmission_mode; / ISDB-T transmission mode,
    pub /: *mut *mut u32 modem_state; / 0 - Acquisition, 1 - Locked,
    pub /: *mut *mut u32 guard_interval; / Guard Interval, 1 divided by value,
    pub /: *mut *mut u32 system_type; / ISDB-T system type (ISDB-T / ISDB-Tsb),
    pub /: *mut *mut u32 partial_reception; / TRUE - partial reception, FALSE otherwise,
    pub /: *mut *mut u32 num_of_layers; / Number of ISDB-T layers in the network,
// Per-layer information
// Layers A, B and C
    pub layer_info: [sms_isdbt_layer_stats; 3],
// Per-layer statistics, see sms_isdbt_layer_stats
// Interface information
    pub /: *mut *mut u32 sms_to_host_tx_errors; / Total number of transmission errors.,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_isdbt_stats_ex {
    pub the: *mut *mut u32 statistics_type; / Enumerator identifying the type of,
// structure.  Values are the same as
// SMSHOSTLIB_DEVICE_MODES_E
//
// This field MUST always be first in any
// statistics structure
    pub modem.: *mut *mut u32 full_size; / Total size of the structure returned by the,
// If the size requested by the host is smaller than
// full_size, the struct will be truncated
// Common parameters
    pub /: *mut *mut u32 is_rf_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_demod_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_external_lna_on; / 0 - external LNA off, 1 - external LNA on,
// Reception quality
    pub /: *mut *mut s32 SNR; / dB,
    pub /: *mut *mut s32 RSSI; / dBm,
    pub /: *mut *mut s32 in_band_pwr; / In band power in dBM,
    pub /: *mut *mut s32 carrier_offset; / Carrier Offset in Hz,
// Transmission parameters
    pub /: *mut *mut u32 frequency; / frequency in Hz,
    pub /: *mut *mut u32 bandwidth; / bandwidth in MHz,
    pub /: *mut *mut u32 transmission_mode; / ISDB-T transmission mode,
    pub /: *mut *mut u32 modem_state; / 0 - Acquisition, 1 - Locked,
    pub /: *mut *mut u32 guard_interval; / Guard Interval, 1 divided by value,
    pub /: *mut *mut u32 system_type; / ISDB-T system type (ISDB-T / ISDB-Tsb),
    pub /: *mut *mut u32 partial_reception; / TRUE - partial reception, FALSE otherwise,
    pub /: *mut *mut u32 num_of_layers; / Number of ISDB-T layers in the network,
    pub /: *mut *mut u32 segment_number; / Segment number for ISDB-Tsb,
    pub /: *mut *mut u32 tune_bw; / Tuned bandwidth - BW_ISDBT_1SEG / BW_ISDBT_3SEG,
// Per-layer information
// Layers A, B and C
    pub layer_info: [sms_isdbt_layer_stats; 3],
// Per-layer statistics, see sms_isdbt_layer_stats
// Interface information
    pub /: *mut *mut u32 reserved1; / Was sms_to_host_tx_errors - obsolete .,
// Proprietary information
    pub /: *mut *mut u32 ext_antenna; / Obsolete field.,
    pub reception_quality: u32,
    pub /: *mut *mut u32 ews_alert_active; / signals if EWS alert is currently on,
    pub /: *mut *mut u32 lna_on_off; / Internal LNA state: 0: OFF, 1: ON,
    pub /: *mut *mut u32 rf_agc_level; / RF AGC Level [linear units], full gain = 65535 (20dB),
    pub /: *mut *mut u32 bb_agc_level; / Baseband AGC level [linear units], full gain = 65535 (71.5dB),
    pub /: *mut *mut u32 fw_errors_counter; / Application errors - should be always zero,
    pub /: *mut *mut u8 FwErrorsHistoryArr[8]; / Last FW errors IDs - first is most recent, last is oldest,
    pub /: *mut *mut s32 MRC_SNR; / dB,
    pub /: *mut *mut u32 snr_full_res; / dB x 65536,
    pub reserved4: [u32; 4],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_pid_stats_data {
#[repr(C)]
#[derive(Copy, Clone)]
pub struct PID_BURST_S {
    pub size: u32,
    pub padding_cols: u32,
    pub punct_cols: u32,
    pub duration: u32,
    pub cycle: u32,
    pub calc_cycle: u32,
    pub burst: },
    pub tot_tbl_cnt: u32,
    pub invalid_tbl_cnt: u32,
    pub tot_cor_tbl: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_pid_data {
    pub pid: u32,
    pub num_rows: u32,
    pub pid_statistics: sms_pid_stats_data,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_tx_stats {
    pub /: *mut *mut u32 frequency; / frequency in Hz,
    pub /: *mut *mut u32 bandwidth; / bandwidth in MHz,
    pub /: *mut *mut u32 transmission_mode; / FFT mode carriers in Kilos,
    pub from: *mut *mut u32 guard_interval; / Guard Interval,
    pub /: *mut *mut u32 code_rate; / Code Rate from SMSHOSTLIB_CODE_RATE_ET,
    pub from: *mut *mut u32 lp_code_rate; / Low Priority Code Rate,
    pub /: *mut *mut u32 hierarchy; / hierarchy from SMSHOSTLIB_HIERARCHY_ET,
    pub from: *mut *mut u32 constellation; / constellation,
// DVB-H TPS parameters
    pub zero: *mut *mut u32 cell_id; / TPS Cell ID in bits 15..0, bits 31..16,
    pub -: *mut *mut u32 dvbh_srv_ind_hp; / DVB-H service indication info, bit 1,
    pub -: *mut *mut u32 dvbh_srv_ind_lp; / DVB-H service indication info, bit 1,
    pub /: *mut *mut u32 is_demod_locked; / 0 - not locked, 1 - locked,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_rx_stats {
    pub /: *mut *mut u32 is_rf_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_demod_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_external_lna_on; / 0 - external LNA off, 1 - external LNA on,
    pub /: *mut *mut u32 modem_state; / from SMSHOSTLIB_DVB_MODEM_STATE_ET,
    pub /: *mut *mut s32 SNR; / dB,
    pub /: *mut *mut u32 ber; / Post Viterbi ber [1E-5],
    pub /: *mut *mut u32 ber_error_count; / Number of erroneous SYNC bits.,
    pub /: *mut *mut u32 ber_bit_count; / Total number of SYNC bits.,
    pub PER,: *mut *mut u32 ts_per; / Transport stream,
    pub percentage,: *mut *mut u32 MFER; / DVB-H frame error rate in,
    pub /: *mut *mut s32 RSSI; / dBm,
    pub /: *mut *mut s32 in_band_pwr; / In band power in dBM,
    pub /: *mut *mut s32 carrier_offset; / Carrier Offset in bin/1024,
    pub erroneous: *mut *mut u32 error_ts_packets; / Number of,
    pub /: *mut *mut u32 total_ts_packets; / Total number of transport-stream packets,
    pub /: *mut *mut s32 MRC_SNR; / dB,
    pub /: *mut *mut s32 MRC_RSSI; / dBm,
    pub /: *mut *mut s32 mrc_in_band_pwr; / In band power in dBM,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_rx_stats_ex {
    pub /: *mut *mut u32 is_rf_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_demod_locked; / 0 - not locked, 1 - locked,
    pub /: *mut *mut u32 is_external_lna_on; / 0 - external LNA off, 1 - external LNA on,
    pub /: *mut *mut u32 modem_state; / from SMSHOSTLIB_DVB_MODEM_STATE_ET,
    pub /: *mut *mut s32 SNR; / dB,
    pub /: *mut *mut u32 ber; / Post Viterbi ber [1E-5],
    pub /: *mut *mut u32 ber_error_count; / Number of erroneous SYNC bits.,
    pub /: *mut *mut u32 ber_bit_count; / Total number of SYNC bits.,
    pub PER,: *mut *mut u32 ts_per; / Transport stream,
    pub percentage,: *mut *mut u32 MFER; / DVB-H frame error rate in,
    pub /: *mut *mut s32 RSSI; / dBm,
    pub /: *mut *mut s32 in_band_pwr; / In band power in dBM,
    pub /: *mut *mut s32 carrier_offset; / Carrier Offset in bin/1024,
    pub erroneous: *mut *mut u32 error_ts_packets; / Number of,
    pub /: *mut *mut u32 total_ts_packets; / Total number of transport-stream packets,
    pub ref_dev_ppm: i32,
    pub freq_dev_hz: i32,
    pub /: *mut *mut s32 MRC_SNR; / dB,
    pub /: *mut *mut s32 MRC_RSSI; / dBm,
    pub /: *mut *mut s32 mrc_in_band_pwr; / In band power in dBM,
}

pub const SRVM_MAX_PID_FILTERS: c_int = 8;
// statistics information returned as response for
// SmsHostApiGetstatisticsEx_Req for DVB applications, SMS1100 and up
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_stats_dvb {
// Reception
    pub reception_data: sms_rx_stats,
// Transmission parameters
    pub transmission_data: sms_tx_stats,
// Burst parameters, valid only for DVB-H
    pub pid_data: [sms_pid_data; SRVM_MAX_PID_FILTERS],
}

// statistics information returned as response for
// SmsHostApiGetstatisticsEx_Req for DVB applications, SMS1100 and up
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_stats_dvb_ex {
// Reception
    pub reception_data: sms_rx_stats_ex,
// Transmission parameters
    pub transmission_data: sms_tx_stats,
// Burst parameters, valid only for DVB-H
    pub pid_data: [sms_pid_data; SRVM_MAX_PID_FILTERS],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sms_srvm_signal_status {
    pub result: u32,
    pub snr: u32,
    pub ts_packets: u32,
    pub ets_packets: u32,
    pub constellation: u32,
    pub hp_code: u32,
    pub tps_srv_ind_lp: u32,
    pub tps_srv_ind_hp: u32,
    pub cell_id: u32,
    pub reason: u32,
    pub in_band_power: i32,
    pub request_id: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct smscore_config_gpio {
pub const SMS_GPIO_DIRECTION_INPUT: c_int = 0;
pub const SMS_GPIO_DIRECTION_OUTPUT: c_int = 1;
    pub direction: u8,
pub const SMS_GPIO_PULLUPDOWN_NONE: c_int = 0;
pub const SMS_GPIO_PULLUPDOWN_PULLDOWN: c_int = 1;
pub const SMS_GPIO_PULLUPDOWN_PULLUP: c_int = 2;
pub const SMS_GPIO_PULLUPDOWN_KEEPER: c_int = 3;
    pub pullupdown: u8,
pub const SMS_GPIO_INPUTCHARACTERISTICS_NORMAL: c_int = 0;
pub const SMS_GPIO_INPUTCHARACTERISTICS_SCHMITT: c_int = 1;
    pub inputcharacteristics: u8,
// 10xx
pub const SMS_GPIO_OUTPUT_SLEW_RATE_FAST: c_int = 0;
pub const SMS_GPIO_OUTPUT_SLEW_WRATE_SLOW: c_int = 1;
// 11xx
pub const SMS_GPIO_OUTPUT_SLEW_RATE_0_45_V_NS: c_int = 0;
pub const SMS_GPIO_OUTPUT_SLEW_RATE_0_9_V_NS: c_int = 1;
pub const SMS_GPIO_OUTPUT_SLEW_RATE_1_7_V_NS: c_int = 2;
pub const SMS_GPIO_OUTPUT_SLEW_RATE_3_3_V_NS: c_int = 3;
    pub outputslewrate: u8,
// 10xx
pub const SMS_GPIO_OUTPUTDRIVING_S_4mA: c_int = 0;
pub const SMS_GPIO_OUTPUTDRIVING_S_8mA: c_int = 1;
pub const SMS_GPIO_OUTPUTDRIVING_S_12mA: c_int = 2;
pub const SMS_GPIO_OUTPUTDRIVING_S_16mA: c_int = 3;
// 11xx
pub const SMS_GPIO_OUTPUTDRIVING_1_5mA: c_int = 0;
pub const SMS_GPIO_OUTPUTDRIVING_2_8mA: c_int = 1;
pub const SMS_GPIO_OUTPUTDRIVING_4mA: c_int = 2;
pub const SMS_GPIO_OUTPUTDRIVING_7mA: c_int = 3;
pub const SMS_GPIO_OUTPUTDRIVING_10mA: c_int = 4;
pub const SMS_GPIO_OUTPUTDRIVING_11mA: c_int = 5;
pub const SMS_GPIO_OUTPUTDRIVING_14mA: c_int = 6;
pub const SMS_GPIO_OUTPUTDRIVING_16mA: c_int = 7;
    pub outputdriving: u8,
}

extern "C" {
    pub fn smscore_registry_getmode(devpath: *mut c_char) -> c_int;
}
extern "C" {
    pub fn smscore_register_hotplug(hotplug: hotplug_t) -> c_int;
}
extern "C" {
    pub fn smscore_unregister_hotplug(hotplug: hotplug_t);
}
extern "C" {
    pub fn smscore_unregister_device(coredev: *mut smscore_device_t);
}
extern "C" {
    pub fn smscore_start_device(coredev: *mut smscore_device_t) -> c_int;
}
extern "C" {
    pub fn smscore_set_device_mode(coredev: *mut smscore_device_t, mode: c_int) -> c_int;
}
extern "C" {
    pub fn smscore_get_device_mode(coredev: *mut smscore_device_t) -> c_int;
}
extern "C" {
    pub fn smscore_unregister_client(client: *mut smscore_client_t);
}
// old GPIO management
extern "C" {
    pub fn smscore_set_gpio(coredev: *mut smscore_device_t, pin: u32, level: c_int) -> c_int;
}
// new GPIO management
extern "C" {
    pub fn smscore_set_board_id(core: *mut smscore_device_t, id: c_int);
}
extern "C" {
    pub fn smscore_get_board_id(core: *mut smscore_device_t) -> c_int;
}
extern "C" {
    pub fn smscore_led_state(core: *mut smscore_device_t, led: c_int) -> c_int;
}
// ------------------------------------------------------------------------
