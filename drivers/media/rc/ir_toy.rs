...
//! Source: drivers/media/rc/ir_toy.c
#![no_std]
...
pub const TIMEOUT: c_int = 500;
pub const LEN_XMITRES: c_int = 3;
pub const LEN_VERSION: c_int = 4;
pub const LEN_SAMPLEMODEPROTO: c_int = 3;
pub const MIN_FW_VERSION: c_int = 20;
pub const UNIT_US: c_int = 21;

pub const MAX_PACKET: c_int = 64;
    enum state {
    STATE_IRDATA,
...
pub struct irtoy {
    ...
    pub tx_len: c_uint,
    pub emitted: c_uint,
    pub hw_version: c_uint,
    pub sw_version: c_uint,
    pub proto_version: c_uint,
    pub phys: [c_char; 64],
}

#[no_mangle]
unsafe extern "C" fn irtoy_response(irtoy: *mut irtoy, len: u32) {
    ...
    case STATE_IRDATA: {
    let mut rawir: ir_raw_event = { .pulse = irtoy.pulse };
    __be16 *in = (__be16 *)irtoy.in;
    int i;
    for (i = 0; i < len / sizeof(__be16); i++) {
    let mut v: u16 = be16_to_cpu(in[i]);
    if (v == 0xffff) {
    rawir.pulse = false;
    } else {
    ...
    let mut emitted: u16 = get_unaligned_be16(irtoy.in + 1);
...
// send next part of tx buffer
    let mut space: c_uint = irtoy.in[0];
    uint buf_len;
    int err;
...
#[no_mangle]
unsafe extern "C" fn irtoy_out_callback(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn irtoy_in_callback(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn irtoy_setup(irtoy: *mut irtoy) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn irtoy_tx(rc: *mut rc_dev, txbuf: *mut c_uint, count: c_uint) -> c_int {
    ...
    return -ENOMEM;
    for (i = 0; i < count; i++) {
    let mut v: u16 = DIV_ROUND_CLOSEST(txbuf[i], UNIT_US);
    if (!v)
    v = 1;
    buf[i] = cpu_to_be16(v);
    }
...
#[no_mangle]
unsafe extern "C" fn irtoy_tx_carrier(rc: *mut rc_dev, carrier: u32) -> c_int {
    ...
    struct urb *urb;
    int pipe, err;
    err = usb_find_common_endpoints(idesc, &ep_in, &ep_out, core::ptr::null_mut(), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn irtoy_disconnect(intf: *mut usb_interface) {
