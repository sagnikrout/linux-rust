...
//! Source: drivers/media/rc/imon.c
#![no_std]
...
pub const DISPLAY_MINOR_BASE: c_int = 144;

pub const IMON_CLOCK_ENABLE_PACKETS: c_int = 2;
// P R O T O T Y P E S
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct imon_panel_key_table {
    pub hw_code: u64,
    pub keycode: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct imon_usb_dev_descr {
    pub flags: __u16,
pub const IMON_NO_FLAGS: c_int = 0;
pub const IMON_NEED_20MS_PKT_DELAY: c_int = 1;
pub const IMON_SUPPRESS_REPEATED_KEYS: c_int = 2;
    pub key_table: [imon_panel_key_table; ],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct imon_context {
    pub dev: *mut device,
// Newer devices have two interfaces
    pub usbdev_intf0: *mut usb_device,
    pub usbdev_intf1: *mut usb_device,
    pub /: *mut *mut bool display_supported; / not all controllers do,
    pub /: *mut *mut bool display_isopen; / display port has been opened,
    pub /: *mut *mut bool rf_device; / true if iMON 2.4G LT/DT RF device,
    pub /: *mut *mut bool rf_isassociating; / RF remote associating,
    pub /: *mut *mut bool dev_present_intf0; / USB device presence, interface 0,
    pub /: *mut *mut bool dev_present_intf1; / USB device presence, interface 1,
    pub /: *mut *mut mutex lock; / to lock this object,
    pub /: *mut *mut wait_queue_head_t remove_ok; / For unexpected USB disconnects,
    pub rx_endpoint_intf0: *mut usb_endpoint_descriptor,
    pub rx_endpoint_intf1: *mut usb_endpoint_descriptor,
    pub tx_endpoint: *mut usb_endpoint_descriptor,
    pub rx_urb_intf0: *mut urb,
    pub rx_urb_intf1: *mut urb,
    pub tx_urb: *mut urb,
    pub tx_control: bool,
    pub usb_rx_buf: [c_uchar; 8],
    pub usb_tx_buf: [c_uchar; 8],
    pub send_packet_delay: c_uint,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tx_t {
    pub /: *mut *mut unsigned char data_buf[35]; / user data buffer,
    pub /: *mut *mut completion finished; / wait for write to finish,
    pub /: *mut *mut bool busy; / write in progress,
    pub /: *mut *mut int status; / status of tx completion,
    pub tx: },
    pub /: *mut *mut u16 vendor; / usb vendor ID,
    pub /: *mut *mut u16 product; / usb product ID,
    pub /: *mut *mut *mut rc_dev rdev; / rc-core device for remote,
    pub /: *mut *mut *mut input_dev idev; / input device for panel & IR mouse,
    pub /: *mut *mut *mut input_dev touch; / input device for touchscreen,
    pub /: *mut *mut spinlock_t kc_lock; / make sure we get keycodes right,
    pub /: *mut *mut u32 kc; / current input keycode,
    pub /: *mut *mut u32 last_keycode; / last reported input keycode,
    pub /: *mut *mut u32 rc_scancode; / the computed remote scancode,
    pub /: *mut *mut u8 rc_toggle; / the computed remote toggle bit,
    pub /: *mut *mut u64 rc_proto; / iMON or MCE (RC6) IR protocol?,
    pub /: *mut *mut bool release_code; / some keys send a release code,
    pub /: *mut *mut u8 display_type; / store the display type,
    pub /: *mut *mut bool pad_mouse; / toggle kbd(0)/mouse(1) mode,
    pub /: *mut *mut char name_rdev[128]; / rc input device name,
    pub /: *mut *mut char phys_rdev[64]; / rc input device phys path,
    pub /: *mut *mut char name_idev[128]; / input device name,
    pub /: *mut *mut char phys_idev[64]; / input device phys path,
    pub /: *mut *mut char name_touch[128]; / touch screen name,
    pub /: *mut *mut char phys_touch[64]; / touch screen phys path,
    pub /: *mut *mut timer_list ttimer; / touch screen timer,
    pub /: *mut *mut int touch_x; / x coordinate on touchscreen,
    pub /: *mut *mut int touch_y; / y coordinate on touchscreen,
    pub dev_descr: *const imon_usb_dev_descr,
...
// lcd, vfd, vga or none? should be auto-detected, but can be overridden...
    ...
    let mut pad_stabilize: static int = 1;
...
#[no_mangle]
unsafe extern "C" fn free_imon_context(ictx: *mut imon_context) {
...
//
#[no_mangle]
unsafe extern "C" fn display_open(inode: *mut inode, file: *mut file) -> c_int {
    static int display_open(struct inode *inode, struct file *file)
    {
    struct usb_interface *interface;
    struct imon_context *ictx = core::ptr::null_mut();
    int subminor;
...
//
#[no_mangle]
unsafe extern "C" fn display_close(inode: *mut inode, file: *mut file) -> c_int {
    static int display_close(struct inode *inode, struct file *file)
    {
...
//
#[no_mangle]
unsafe extern "C" fn send_packet(ictx: *mut imon_context) -> c_int {
    ...
    let mut retval: c_int = 0;
    struct usb_ctrlrequest *control_req = core::ptr::null_mut();
...
// fill request into kmalloc'ed space:
    ...
    if (control_req == core::ptr::null_mut())
    return -ENOMEM;
...
//
#[no_mangle]
unsafe extern "C" fn send_associate_24g(ictx: *mut imon_context) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn usb_tx_callback(urb: *mut urb) {
    static void usb_tx_callback(struct urb *urb)
    {
    struct imon_context *ictx;
    if (!urb)
...
//
#[no_mangle]
unsafe extern "C" fn imon_touch_display_timeout(t: *mut timer_list) {
...
//
#[no_mangle]
unsafe extern "C" fn imon_ir_change_protocol(rc: *mut rc_dev, rc_proto: *mut u64) -> c_int {
    static int imon_ir_change_protocol(struct rc_dev *rc, u64 *rc_proto)
    {
    int retval;
    struct imon_context *ictx = rc.priv;
    struct device *dev = ictx.dev;
    let mut unlock: bool = mutex_trylock(&ictx.lock);
...
//
#[no_mangle]
unsafe extern "C" fn stabilize(a: c_int, b: c_int, timeout: u16, threshold: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn imon_remote_key_lookup(ictx: *mut imon_context, scancode: u32) -> u32 {
    static u32 imon_remote_key_lookup(struct imon_context *ictx, u32 scancode)
    {
    u32 keycode;
    u32 release;
    let mut is_release_code: bool = false;
...
#[no_mangle]
unsafe extern "C" fn imon_mce_key_lookup(ictx: *mut imon_context, scancode: u32) -> u32 {
    static u32 imon_mce_key_lookup(struct imon_context *ictx, u32 scancode)
    {
    u32 keycode;
pub const MCE_KEY_MASK: c_uint = 0x7000;
pub const MCE_TOGGLE_BIT: c_uint = 0x8000;
//
...
#[no_mangle]
unsafe extern "C" fn imon_panel_key_lookup(ictx: *mut imon_context, code: u64) -> u32 {
    static u32 imon_panel_key_lookup(struct imon_context *ictx, u64 code)
    {
    const struct imon_panel_key_table *key_table;
    let mut keycode: u32 = KEY_RESERVED;
    int i;
    ...
    {
    let mut rel_x: signed char = 0x00, rel_y = 0x00;
    let mut right_shift: u8 = 1;
    let mut mouse_input: bool = true;
    let mut dir: c_int = 0;
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn imon_touch_event(ictx: *mut imon_context, buf: *mut c_uchar) {
...
#[no_mangle]
unsafe extern "C" fn imon_pad_to_keys(ictx: *mut imon_context, buf: *mut c_uchar) {
    static void imon_pad_to_keys(struct imon_context *ictx, unsigned char *buf)
    {
    let mut dir: c_int = 0;
    let mut rel_x: signed char = 0x00, rel_y = 0x00;
    u16 timeout, threshold;
    let mut scancode: u32 = KEY_RESERVED;
    unsigned long flags;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(IMON_KEY_MCE: ktype ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ictx->release_code) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(RC_PROTO_BIT_IMON: ictx->rc_proto ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn usb_rx_callback_intf0(urb: *mut urb) {
    static void usb_rx_callback_intf0(struct urb *urb)
    {
    struct imon_context *ictx;
    let mut intfnum: c_int = 0;
    if (!urb)
    return;
    ictx = (struct imon_context *)urb.context;
    if (!ictx)
...
#[no_mangle]
unsafe extern "C" fn usb_rx_callback_intf1(urb: *mut urb) {
    static void usb_rx_callback_intf1(struct urb *urb)
    {
    struct imon_context *ictx;
    let mut intfnum: c_int = 1;
    if (!urb)
    return;
    ictx = (struct imon_context *)urb.context;
    if (!ictx)
...
//
#[no_mangle]
unsafe extern "C" fn imon_get_ffdc_type(ictx: *mut imon_context) {
    static void imon_get_ffdc_type(struct imon_context *ictx)
    {
    let mut ffdc_cfg_byte: u8 = ictx.usb_rx_buf[6];
    let mut detected_display_type: u8 = IMON_DISPLAY_TYPE_NONE;
    let mut allowed_protos: u64 = RC_PROTO_BIT_IMON;
...
#[no_mangle]
unsafe extern "C" fn imon_set_display_type(ictx: *mut imon_context) {
    static void imon_set_display_type(struct imon_context *ictx)
    {
    let mut configured_display_type: u8 = IMON_DISPLAY_TYPE_VFD;
//
...
// panel and/or knob code support
    for (i = 0; key_table[i].hw_code != 0; i++) {
    let mut kc: u32 = key_table[i].keycode;
    ...
    struct usb_endpoint_descriptor *rx_endpoint = core::ptr::null_mut();
    struct usb_endpoint_descriptor *tx_endpoint = core::ptr::null_mut();
    let mut ifnum: c_int = iface_desc.desc.bInterfaceNumber;
    let mut num_endpts: c_int = iface_desc.desc.bNumEndpoints;
    int i, ep_dir, ep_type;
    let mut ir_ep_found: bool = false;
    let mut display_ep_found: bool = false;
    let mut tx_control: bool = false;
//
...
// default send_packet delay is 5ms but some devices need more
    ...
    touch_setup_failed:
    find_endpoint_failed:
    ictx.usbdev_intf1 = core::ptr::null_mut();
    mutex_unlock(&ictx.lock);
    usb_free_urb(rx_urb);
    ictx.rx_urb_intf1 = core::ptr::null_mut();
    rx_urb_alloc_failed:
...
//
    ...
    const struct usb_device_id *id)
    {
    struct usb_device *usbdev = core::ptr::null_mut();
    struct usb_host_interface *iface_desc = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn imon_disconnect(interface: *mut usb_interface) {
    static void imon_disconnect(struct usb_interface *interface)
    {
    struct imon_context *ictx;
    struct device *dev;
...
#[no_mangle]
pub unsafe extern "C" fn if(IMON_DISPLAY_TYPE_VFD: ictx->display_type ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn imon_suspend(intf: *mut usb_interface, message: pm_message_t) -> c_int {
    static int imon_suspend(struct usb_interface *intf, pm_message_t message)
    {
    struct imon_context *ictx = usb_get_intfdata(intf);
    let mut ifnum: c_int = intf.cur_altsetting.desc.bInterfaceNumber;
    if (ifnum == 0)
    usb_kill_urb(ictx.rx_urb_intf0);
    else
    usb_kill_urb(ictx.rx_urb_intf1);
...
#[no_mangle]
unsafe extern "C" fn imon_resume(intf: *mut usb_interface) -> c_int {
    static int imon_resume(struct usb_interface *intf)
    {
    let mut rc: c_int = 0;
    struct imon_context *ictx = usb_get_intfdata(intf);
