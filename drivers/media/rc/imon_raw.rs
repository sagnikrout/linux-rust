...
//! Source: drivers/media/rc/imon_raw.c
#![no_std]
...
// Each bit is 250us
pub const BIT_DURATION: c_int = 250;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct imon {
    pub dev: *mut device,
...
//
#[no_mangle]
unsafe extern "C" fn imon_ir_data(imon: *mut imon) {
    static void imon_ir_data(struct imon *imon)
    {
    let mut rawir: ir_raw_event = {};
    let mut data: u64 = be64_to_cpup(imon.ir_buf);
    let mut packet_no: u8 = data & 0xff;
    let mut offset: c_int = 40;
    int bit;
    if (packet_no == 0xff)
    return;
...
#[no_mangle]
unsafe extern "C" fn imon_ir_rx(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn imon_disconnect(intf: *mut usb_interface) {
