...
//! Source: drivers/media/v4l2-core/v4l2-h264.c
#![no_std]
...
//
    let mut tmp_str_size: static int = 1024;
//
...
//
    ...
    let mut fields: u8 = V4L2_H264_FRAME_REF;
    ...
    let mut fields: u8 = V4L2_H264_TOP_FIELD_REF;
    b.unordered_reflist[b.num_valid].index = i;
    b.unordered_reflist[b.num_valid].fields = fields;
    b.num_valid++;
    }
    if (dpb[i].fields & V4L2_H264_BOTTOM_FIELD_REF) {
    let mut fields: u8 = V4L2_H264_BOTTOM_FIELD_REF;
    b.unordered_reflist[b.num_valid].index = i;
    b.unordered_reflist[b.num_valid].fields = fields;
    b.num_valid++;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(builder->cur_pic_order_count: poca <) -> else {
...
#[no_mangle]
unsafe extern "C" fn ref_type_to_char(ref_type: u8) -> c_char {
...
//
    let mut frame_num: c_int = builder.refs[reflist[i].index].frame_num;
    let mut longterm: bool = builder.refs[reflist[i].index].longterm;
...
// out_str = kmalloc(tmp_str_size, GFP_KERNEL);
    ...
    for (i = 0; i < builder.num_valid; i++) {
    let mut frame_num: c_int = builder.refs[reflist[i].index].frame_num;
    let mut poc: u32 = v4l2_h264_get_poc(builder, reflist + i);
    let mut longterm: bool = builder.refs[reflist[i].index].longterm;
    n += scnprintf(*out_str + n, tmp_str_size - n, "%i%c%c|",
...
//
    ...
    v4l2_h264_p_ref_list_cmp, core::ptr::null_mut(), builder);
...
//
    ...
    v4l2_h264_b0_ref_list_cmp, core::ptr::null_mut(), builder);
    memcpy(b1_reflist, builder.unordered_reflist,
    sizeof(builder.unordered_reflist[0]) * builder.num_valid);
    sort_r(b1_reflist, builder.num_valid, sizeof(*b1_reflist),
    v4l2_h264_b1_ref_list_cmp, core::ptr::null_mut(), builder);
    if (builder.cur_pic_fields != V4L2_H264_FRAME_REF) {
