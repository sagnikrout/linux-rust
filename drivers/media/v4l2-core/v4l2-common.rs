...
//! Source: drivers/media/v4l2-core/v4l2-common.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn v4l2_ctrl_query_fill(qctrl: *mut v4l2_queryctrl, _min: i32, _max: i32, _step: i32, _def: i32) -> c_int {
    int v4l2_ctrl_query_fill(struct v4l2_queryctrl *qctrl, s32 _min, s32 _max, s32 _step, s32 _def)
    {
    const char *name;
    let mut min: i64 = _min;
    let mut max: i64 = _max;
    let mut step: u64 = _step;
...
// Max walign where there is still a valid width
    let mut wmaxa: c_uint = __fls(wmax ^ (wmin - 1));
// Max halign where there is still a valid height
    let mut hmaxa: c_uint = __fls(hmax ^ (hmin - 1));
...
// h = clamp_align(*h, hmin, hmax, halign + 1);
    ...
    const void *best = core::ptr::null_mut();
    size_t i;
    if (!array)
...
#[no_mangle]
pub unsafe extern "C" fn v4l2_format_block_width(info: *const v4l2_format_info, plane: c_int) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn v4l2_format_block_height(info: *const v4l2_format_info, plane: c_int) -> c_uint {
    ...
    {
    let mut hdiv: c_uint = plane ? info.hdiv : 1;
...
//
    ...
    unsigned int height)
    {
    let mut vdiv: c_uint = plane ? info.vdiv : 1;
...
// Single planar API cannot be used for multi plane formats.
    ...
    if (ctrl) {
    let mut qm: v4l2_querymenu = { .id = V4L2_CID_LINK_FREQ };
    int ret;
    ...
    unsigned int div)
    {
    let mut mbus_config: v4l2_mbus_config = {};
...
//
#[no_mangle]
pub unsafe extern "C" fn v4l2_fraction_to_interval(numerator: u32, denominator: u32) -> u32 {
...
// bitmap |= BIT(j);
    ...
    {
    let mut of_node: bool = is_of_node(dev_fwnode(dev));
    const char *clk_id __free(kfree) = core::ptr::null_mut();
    struct clk_hw *clk_hw;
    struct clk *clk;
    let mut rate: u32 = clk_rate;
    let mut ret: c_int = 0;
...
// We need a rate to create a clock.
    ...
    clk_hw = devm_clk_hw_register_fixed_rate(dev, clk_id, core::ptr::null_mut(), 0, rate);
    if (IS_ERR(clk_hw))
