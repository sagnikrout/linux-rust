...
//! Source: drivers/media/platform/amlogic/meson-ge2d/ge2d.c
#![no_std]
...
pub const DEFAULT_WIDTH: c_int = 128;
pub const DEFAULT_HEIGHT: c_int = 128;
pub const DEFAULT_STRIDE: c_int = 512;
pub const MAX_WIDTH: c_int = 8191;
pub const MAX_HEIGHT: c_int = 8191;
//
// Missing features:
// - Scaling
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ge2d_frame {
    pub buf: *mut vb2_v4l2_buffer,
// Image Format
    pub pix_fmt: v4l2_pix_format,
// Crop
    pub crop: v4l2_rect,
// Image format
    pub fmt: *const ge2d_fmt,
}
...
pub struct ge2d_ctx {
    pub fh: v4l2_fh,
    pub ge2d: *mut meson_ge2d,
    pub in: ge2d_frame,
    pub out: ge2d_frame,
    pub ctrl_handler: v4l2_ctrl_handler,
    pub sequence_cap: unsigned long sequence_out,,
// Control values
    pub hflip: u32,
    pub vflip: u32,
    pub xy_swap: u32,
...
#[no_mangle]
unsafe extern "C" fn ge2d_hw_start(ge2d: *mut meson_ge2d) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !ctx->out.fmt->alpha) -> else {
...
#[no_mangle]
unsafe extern "C" fn ge2d_isr(irq: c_int, priv: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ge2d_buf_prepare(vb: *mut vb2_buffer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ge2d_buf_queue(vb: *mut vb2_buffer) {
...
#[no_mangle]
unsafe extern "C" fn ge2d_start_streaming(vq: *mut vb2_queue, count: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ge2d_stop_streaming(vq: *mut vb2_queue) {
...
#[no_mangle]
unsafe extern "C" fn vidioc_enum_fmt(file: *mut file, priv: *mut c_void, f: *mut v4l2_fmtdesc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vidioc_setup_cap_fmt(ctx: *mut ge2d_ctx, f: *mut v4l2_pix_format) {
...
#[no_mangle]
unsafe extern "C" fn vidioc_try_fmt_cap(file: *mut file, priv: *mut c_void, f: *mut v4l2_format) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vidioc_s_fmt_cap(file: *mut file, priv: *mut c_void, f: *mut v4l2_format) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vidioc_g_fmt(file: *mut file, priv: *mut c_void, f: *mut v4l2_format) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vidioc_try_fmt_out(file: *mut file, priv: *mut c_void, f: *mut v4l2_format) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vidioc_s_fmt_out(file: *mut file, priv: *mut c_void, f: *mut v4l2_format) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ge2d_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ge2d_setup_ctrls(ctx: *mut ge2d_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ge2d_open(file: *mut file) -> c_int {
    static int ge2d_open(struct file *file)
    {
    struct meson_ge2d *ge2d = video_drvdata(file);
    struct ge2d_ctx *ctx = core::ptr::null_mut();
    let mut ret: c_int = 0;
    ctx = kzalloc_obj(*ctx);
    if (!ctx)
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn ge2d_release(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ge2d_probe(pdev: *mut platform_device) -> c_int {
    ...
    rst = devm_reset_control_get(ge2d.dev, core::ptr::null_mut());
    if (IS_ERR(rst)) {
    dev_err(ge2d.dev, "failed to get core reset controller\n");
    return PTR_ERR(rst);
    }
    ge2d.clk = devm_clk_get(ge2d.dev, core::ptr::null_mut());
    if (IS_ERR(ge2d.clk)) {
...
#[no_mangle]
unsafe extern "C" fn ge2d_remove(pdev: *mut platform_device) {
