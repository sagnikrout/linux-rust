...
//! Source: drivers/media/platform/rockchip/rkvdec/rkvdec-vp9.c
#![no_std]
...
pub const RKVDEC_VP9_PROBE_SIZE: c_int = 4864;
pub const RKVDEC_VP9_COUNT_SIZE: c_int = 13232;
pub const RKVDEC_VP9_MAX_SEGMAP_SIZE: c_int = 73728;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_intra_mode_probs {
    pub y_mode: [u8; 105],
    pub uv_mode: [u8; 23],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_intra_only_frame_probs {
    pub coef_intra: [u8; 4][2][128],
    pub intra_mode: [rkvdec_vp9_intra_mode_probs; 10],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_inter_frame_probs {
    pub y_mode: [u8; 4][9],
    pub comp_mode: [u8; 5],
    pub comp_ref: [u8; 5],
    ...
    pub padding0: [u8; 11],
    pub coef: [u8; 2][4][2][128],
    pub uv_mode_0_2: [u8; 3][9],
    pub padding1: [u8; 5],
    pub uv_mode_3_5: [u8; 3][9],
    pub padding2: [u8; 5],
    pub uv_mode_6_8: [u8; 3][9],
    pub padding3: [u8; 5],
    pub uv_mode_9: [u8; 9],
    pub padding4: [u8; 7],
    pub padding5: [u8; 16],
...
// 128 bit alignment
    pub padding0: [u8; 3],
    union {
    pub inter: rkvdec_vp9_inter_frame_probs,
    pub intra_only: rkvdec_vp9_intra_only_frame_probs,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_priv_tbl {
    pub probs: rkvdec_vp9_probs,
    pub segmap: [u8; 2][RKVDEC_VP9_MAX_SEGMAP_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_refs_counts {
    pub eob: [u32; 2],
    pub coeff: [u32; 3],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_inter_frame_symbol_counts {
    pub partition: [u32; 16][4],
    pub skip: [u32; 3][2],
    pub inter: [u32; 4][2],
    pub tx32p: [u32; 2][4],
    pub tx16p: [u32; 2][4],
    ...
    pub comp: [u32; 5][2],
    pub comp_ref: [u32; 5][2],
    pub single_ref: [u32; 5][2][2],
    pub mv_mode: [u32; 7][4],
    pub filter: [u32; 4][3],
    pub mv_joint: [u32; 4],
    pub sign: [u32; 2][2],
// add 1 element for align
    pub 1]: u32 classes[2][11 +,
    pub class0: [u32; 2][2],
    pub bits: [u32; 2][10][2],
    ...
    pub hp: [u32; 2][2],
    pub ref_cnt: [rkvdec_vp9_refs_counts; 2][4][2][6][6],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_intra_frame_symbol_counts {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_run {
    pub base: rkvdec_run,
    pub decode_params: *const v4l2_ctrl_vp9_frame,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_frame_info {
    pub 1: u32 valid :,
    pub 1: u32 segmapid :,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rkvdec_vp9_ctx {
    pub priv_tbl: rkvdec_aux_buf,
    pub count_tbl: rkvdec_aux_buf,
    pub inter_cnts: v4l2_vp9_frame_symbol_counts,
    pub intra_cnts: v4l2_vp9_frame_symbol_counts,
    pub probability_tables: v4l2_vp9_frame_context,
    pub frame_context: [v4l2_vp9_frame_context; 4],
    pub cur: rkvdec_vp9_frame_info,
    pub last: rkvdec_vp9_frame_info,
    pub regs: rkvdec_regs,
}

#[no_mangle]
unsafe extern "C" fn write_coeff_plane(coef[6][6][3]: u8, coeff_plane: *mut u8) {
    static void write_coeff_plane(const u8 coef[6][6][3], u8 *coeff_plane)
    {
    let mut idx: c_uint = 0, byte_count = 0;
    int k, m, n;
    u8 p;
    for (k = 0; k < 6; k++) {
    for (m = 0; m < 6; m++) {
...
// vp9_kf_y_mode_prob
    ...
    k++) {
    let mut val: u8 = v4l2_vp9_kf_y_mode_prob[i][j][k];
...
#[no_mangle]
unsafe extern "C" fn get_mv_base_addr(buf: *mut rkvdec_decoded_buffer) -> dma_addr_t {
...
#[no_mangle]
unsafe extern "C" fn config_seg_registers(ctx: *mut rkvdec_ctx, segid: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn update_ctx_last_info(vp9_ctx: *mut rkvdec_vp9_ctx) {
...
//
    ...
    let mut refw: c_uint = ref_bufs[i].vp9.width;
    let mut refh: c_uint = ref_bufs[i].vp9.height;
    u32 hscale, vscale;
...
#[no_mangle]
unsafe extern "C" fn rkvdec_vp9_run(ctx: *mut rkvdec_ctx) -> c_int {
    static int rkvdec_vp9_run(struct rkvdec_ctx *ctx)
    {
    struct rkvdec_dev *rkvdec = ctx.dev;
    let mut run: rkvdec_vp9_run = { };
    int ret;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tx_and_skip {
    pub tx8: [u8; 2][1],
    pub tx16: [u8; 2][2],
    pub tx32: [u8; 2][3],
    pub skip: [u8; 3],
    pub &_tx_skip: *mut *mut } _tx_skip, tx_skip =,
    pub counts: *mut v4l2_vp9_frame_symbol_counts,
// buffer the forward-updated TX and skip probs
    if (frame_is_intra)
    pub probs): copy_tx_and_skip(tx_skip,,
...
// if FrameIsIntra then undo the effect of load_probs2()
    if (frame_is_intra)
    pub tx_skip): copy_tx_and_skip(probs,,
    pub &vp9_ctx->inter_cnts: counts = frame_is_intra ? &vp9_ctx->intra_cnts :,
    v4l2_vp9_adapt_coef_probs(probs, counts,
    !vp9_ctx.last.valid ||
    vp9_ctx.last.flags & V4L2_VP9_FRAME_FLAG_KEY_FRAME,
    if (!frame_is_intra) {
    pub inter_cnts: *const rkvdec_vp9_inter_frame_symbol_counts,
    pub classes: [u32; 2][11],
    pub i: c_int,
    pub vp9_ctx->count_tbl.cpu: inter_cnts =,
    pub ++i): for (i = 0; i < ARRAY_SIZE(classes);,
    pub sizeof(classes[0])): memcpy(classes[i], inter_cnts->classes[i],,
    pub &classes: counts->classes =,
// load_probs2() already done
    ...
    pub vp9_ctx->cur.flags): vp9_ctx->cur.tx_mode,,
    }
    }
// 6.1.2 refresh_probs(): save_probs(fctx_idx)
    pub vp9_ctx->probability_tables: vp9_ctx->frame_context[fctx_idx] =,
    out_update_last:
    }
    static noinline_for_stack void
    rkvdec_init_v4l2_vp9_count_tbl(struct rkvdec_ctx *ctx)
    {
    pub ctx->priv: *mut *mut rkvdec_vp9_ctx vp9_ctx =,
    pub vp9_ctx->count_tbl.cpu: *mut *mut rkvdec_vp9_intra_frame_symbol_counts intra_cnts =,
    pub vp9_ctx->count_tbl.cpu: *mut *mut rkvdec_vp9_inter_frame_symbol_counts inter_cnts =,
    pub m: int i, j, k, l,,
    pub &inter_cnts->partition: vp9_ctx->inter_cnts.partition =,
    pub &inter_cnts->skip: vp9_ctx->inter_cnts.skip =,
    pub &inter_cnts->inter: vp9_ctx->inter_cnts.intra_inter =,
    pub &inter_cnts->tx32p: vp9_ctx->inter_cnts.tx32p =,
    pub &inter_cnts->tx16p: vp9_ctx->inter_cnts.tx16p =,
    pub &inter_cnts->tx8p: vp9_ctx->inter_cnts.tx8p =,
    pub (*)[16][4])(&intra_cnts->partition): *mut vp9_ctx->intra_cnts.partition = (u32,
    pub &intra_cnts->skip: vp9_ctx->intra_cnts.skip =,
    pub &intra_cnts->intra: vp9_ctx->intra_cnts.intra_inter =,
    pub &intra_cnts->tx32p: vp9_ctx->intra_cnts.tx32p =,
    pub &intra_cnts->tx16p: vp9_ctx->intra_cnts.tx16p =,
    pub &intra_cnts->tx8p: vp9_ctx->intra_cnts.tx8p =,
    pub &inter_cnts->y_mode: vp9_ctx->inter_cnts.y_mode =,
    pub &inter_cnts->uv_mode: vp9_ctx->inter_cnts.uv_mode =,
    pub &inter_cnts->comp: vp9_ctx->inter_cnts.comp =,
    pub &inter_cnts->comp_ref: vp9_ctx->inter_cnts.comp_ref =,
    pub &inter_cnts->single_ref: vp9_ctx->inter_cnts.single_ref =,
    pub &inter_cnts->mv_mode: vp9_ctx->inter_cnts.mv_mode =,
    pub &inter_cnts->filter: vp9_ctx->inter_cnts.filter =,
    pub &inter_cnts->mv_joint: vp9_ctx->inter_cnts.mv_joint =,
    pub &inter_cnts->sign: vp9_ctx->inter_cnts.sign =,
//
...
//
    pub &inter_cnts->class0: vp9_ctx->inter_cnts.class0 =,
    pub &inter_cnts->bits: vp9_ctx->inter_cnts.bits =,
    pub &inter_cnts->class0_fp: vp9_ctx->inter_cnts.class0_fp =,
    pub &inter_cnts->fp: vp9_ctx->inter_cnts.fp =,
    pub &inter_cnts->class0_hp: vp9_ctx->inter_cnts.class0_hp =,
    pub &inter_cnts->hp: vp9_ctx->inter_cnts.hp =,

    do {										\
    pub {\: for (m = 0; m < ARRAY_SIZE(vp9_ctx->inter_cnts.coeff[0][0][0][0]); ++m),
    vp9_ctx.inter_cnts.coeff[i][j][k][l][m] =			\
    pub \: &inter_cnts->ref_cnt[k][i][j][l][m].coeff;,
    vp9_ctx.inter_cnts.eob[i][j][k][l][m][0] =			\
    pub \: &inter_cnts->ref_cnt[k][i][j][l][m].eob[0];,
    vp9_ctx.inter_cnts.eob[i][j][k][l][m][1] =			\
    pub \: &inter_cnts->ref_cnt[k][i][j][l][m].eob[1];,
    \
    vp9_ctx.intra_cnts.coeff[i][j][k][l][m] =			\
    pub \: &intra_cnts->ref_cnt[k][i][j][l][m].coeff;,
    vp9_ctx.intra_cnts.eob[i][j][k][l][m][0] =			\
    pub \: &intra_cnts->ref_cnt[k][i][j][l][m].eob[0];,
    vp9_ctx.intra_cnts.eob[i][j][k][l][m][1] =			\
    pub \: &intra_cnts->ref_cnt[k][i][j][l][m].eob[1];,
    }									\
    } while (0)
    pub ++i): for (i = 0; i < ARRAY_SIZE(vp9_ctx->inter_cnts.coeff);,
    pub ++j): for (j = 0; j < ARRAY_SIZE(vp9_ctx->inter_cnts.coeff[0]);,
    pub ++k): for (k = 0; k < ARRAY_SIZE(vp9_ctx->inter_cnts.coeff[0][0]);,
    pub ++l): for (l = 0; l < ARRAY_SIZE(vp9_ctx->inter_cnts.coeff[0][0][0]);,

    }
#[no_mangle]
unsafe extern "C" fn rkvdec_vp9_start(ctx: *mut rkvdec_ctx) -> c_int {
    static int rkvdec_vp9_start(struct rkvdec_ctx *ctx)
    {
    pub ctx->dev: *mut *mut rkvdec_dev rkvdec =,
    pub priv_tbl: *mut rkvdec_vp9_priv_tbl,
    pub vp9_ctx: *mut rkvdec_vp9_ctx,
    pub count_tbl: *mut c_uchar,
    pub ret: c_int,
    pub kzalloc_obj(*vp9_ctx): *mut vp9_ctx =,
    if (!vp9_ctx)
    pub -ENOMEM: return,
    pub vp9_ctx: ctx->priv =,
    pub /: *mut *mut BUILD_BUG_ON(sizeof(priv_tbl->probs) % 16); / ensure probs size is 128-bit aligned,
    priv_tbl = dma_alloc_coherent(rkvdec.dev, sizeof(*priv_tbl),
    pub GFP_KERNEL): &vp9_ctx->priv_tbl.dma,,
    if (!priv_tbl) {
    pub -ENOMEM: ret =,
    pub err_free_ctx: goto,
    }
    pub sizeof(*priv_tbl): *mut vp9_ctx->priv_tbl.size =,
    pub priv_tbl: vp9_ctx->priv_tbl.cpu =,
    count_tbl = dma_alloc_coherent(rkvdec.dev, RKVDEC_VP9_COUNT_SIZE,
    pub GFP_KERNEL): &vp9_ctx->count_tbl.dma,,
    if (!count_tbl) {
    pub -ENOMEM: ret =,
    pub err_free_priv_tbl: goto,
    }
    pub RKVDEC_VP9_COUNT_SIZE: vp9_ctx->count_tbl.size =,
    pub count_tbl: vp9_ctx->count_tbl.cpu =,
    pub 0: return,
    err_free_priv_tbl:
    dma_free_coherent(rkvdec.dev, vp9_ctx.priv_tbl.size,
    pub vp9_ctx->priv_tbl.dma): vp9_ctx->priv_tbl.cpu,,
    err_free_ctx:
    pub ret: return,
    }
#[no_mangle]
unsafe extern "C" fn rkvdec_vp9_stop(ctx: *mut rkvdec_ctx) {
    static void rkvdec_vp9_stop(struct rkvdec_ctx *ctx)
    {
    pub ctx->priv: *mut *mut rkvdec_vp9_ctx vp9_ctx =,
    pub ctx->dev: *mut *mut rkvdec_dev rkvdec =,
    dma_free_coherent(rkvdec.dev, vp9_ctx.count_tbl.size,
    pub vp9_ctx->count_tbl.dma): vp9_ctx->count_tbl.cpu,,
    dma_free_coherent(rkvdec.dev, vp9_ctx.priv_tbl.size,
    pub vp9_ctx->priv_tbl.dma): vp9_ctx->priv_tbl.cpu,,
    }
    ...
    {
    pub &f->fmt.pix_mp: *mut *mut v4l2_pix_format_mplane fmt =,
    pub 1: fmt->num_planes =,
    if (!fmt.plane_fmt[0].sizeimage)
    pub 2: *mut *mut *mut fmt->plane_fmt[0].sizeimage = fmt->width  fmt->height,
    pub 0: return,
    }
