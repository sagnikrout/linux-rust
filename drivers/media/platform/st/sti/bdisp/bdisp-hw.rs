...
//! Source: drivers/media/platform/st/sti/bdisp/bdisp-hw.c
#![no_std]
...
pub const MAX_SRC_WIDTH: c_int = 2048;
// Reset & boot poll config
pub const POLL_RST_MAX: c_int = 500;
pub const POLL_RST_DELAY_MS: c_int = 2;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bdisp_op_cfg {
    pub /: *mut *mut bool cconv; / RGB - YUV conversion,
    pub /: *mut *mut bool hflip; / Horizontal flip,
    pub /: *mut *mut bool vflip; / Vertical flip,
    pub /: *mut *mut bool wide; / Wide (>MAX_SRC_WIDTH),
    pub /: *mut *mut bool scale; / Scale,
    pub /: *mut *mut u16 h_inc; / Horizontal increment in 6.10 format,
    pub /: *mut *mut u16 v_inc; / Vertical increment in 6.10 format,
    pub /: *mut *mut bool src_interlaced; / is the src an interlaced buffer,
    pub /: *mut *mut u8 src_nbp; / nb of planes of the src,
    pub /: *mut *mut bool src_yuv; / is the src a YUV color format,
    pub /: *mut *mut bool src_420; / is the src 4:2:0 chroma subsampled,
    pub /: *mut *mut u8 dst_nbp; / nb of planes of the dst,
    pub /: *mut *mut bool dst_yuv; / is the dst a YUV color format,
    pub /: *mut *mut bool dst_420; / is the dst 4:2:0 chroma subsampled,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct bdisp_filter_addr {
    pub /: *mut *mut u16 min; / Filter min scale factor (6.10 fixed point),
    pub /: *mut *mut u16 max; / Filter max scale factor (6.10 fixed point),
    pub /: *mut *mut *mut void virt; / Virtual address for filter table,
    pub /: *mut *mut dma_addr_t paddr; / Physical address for filter table,
}
...
//
#[no_mangle]
pub unsafe extern "C" fn bdisp_hw_reset(bdisp: *mut bdisp_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bdisp_hw_get_and_clear_irq(bdisp: *mut bdisp_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bdisp_hw_free_nodes(ctx: *mut bdisp_ctx) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bdisp_hw_alloc_nodes(ctx: *mut bdisp_ctx) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bdisp_hw_free_filters(dev: *mut device) {
    void bdisp_hw_free_filters(struct device *dev)
    {
    let mut size: c_int = (BDISP_HF_NB * NB_H_FILTER) + (BDISP_VF_NB * NB_V_FILTER);
...
//
#[no_mangle]
pub unsafe extern "C" fn bdisp_hw_alloc_filters(dev: *mut device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_get_hf_addr(inc: u16) -> dma_addr_t {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_get_vf_addr(inc: u16) -> dma_addr_t {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_get_inc(from: u32, to: u32, inc: *mut u16) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_get_hv_inc(ctx: *mut bdisp_ctx, h_inc: *mut u16, v_inc: *mut u16) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_get_op_cfg(ctx: *mut bdisp_ctx, c: *mut bdisp_op_cfg) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_color_format(pixelformat: u32) -> u32 {
...
//
    ...
    let mut src_rect: v4l2_rect = src.crop;
    let mut dst_rect: v4l2_rect = dst.crop;
    int dst_x_offset;
    let mut dst_width: i32 = dst.crop.width;
    u32 src_fmt, dst_fmt;
    const u32 *ivmx;
...
// no need to compute Y when writing CbCr from RGB input
    let mut skip_y: bool = (t_plan == BDISP_CBCR) && !cfg.src_yuv;
// FCTL
    if (cfg.scale) {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_build_all_nodes(ctx: *mut bdisp_ctx) -> c_int {
    static int bdisp_hw_build_all_nodes(struct bdisp_ctx *ctx)
    {
    struct bdisp_op_cfg cfg;
    unsigned int i, nid = 0;
    let mut src_x_offset: c_int = 0;
    for (i = 0; i < MAX_NB_NODE; i++)
    if (!ctx.node[i]) {
...
//
#[no_mangle]
unsafe extern "C" fn bdisp_hw_save_request(ctx: *mut bdisp_ctx) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bdisp_hw_update(ctx: *mut bdisp_ctx) -> c_int {
