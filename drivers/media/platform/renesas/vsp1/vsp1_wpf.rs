...
//! Source: drivers/media/platform/renesas/vsp1/vsp1_wpf.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn vsp1_wpf_set_rotation(wpf: *mut vsp1_rwpf, rotation: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vsp1_wpf_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wpf_init_controls(wpf: *mut vsp1_rwpf) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn vsp1_wpf_stop(wpf: *mut vsp1_rwpf) {
...
#[no_mangle]
unsafe extern "C" fn vsp1_wpf_destroy(entity: *mut vsp1_entity) {
    ...
    {
    let mut index: c_uint = wpf.entity.index;
    ...
    let mut index: c_uint = wpf.entity.index;
    let mut outfmt: u32 = 0;
    let mut srcrpf: u32 = 0;
    int ret;
...
//
    ...
    struct vsp1_rwpf *wpf = to_rwpf(&entity.subdev);
    struct vsp1_device *vsp1 = wpf.entity.vsp1;
    let mut mem: vsp1_rwpf_memory = wpf.mem;
    const struct v4l2_pix_format_mplane *format = &wpf.format;
    const struct vsp1_format_info *fmtinfo = wpf.fmtinfo;
    unsigned int width;
    unsigned int height;
...
#[no_mangle]
pub unsafe extern "C" fn if(wpf->flip.rotate: flip & BIT(WPF_CTRL_VFLIP) &&) -> else {
    else if (flip & BIT(WPF_CTRL_VFLIP) && wpf.flip.rotate)
    offset = format.height - left - width;
    else
    offset = left;
    for (unsigned int i = 0; i < format.num_planes; ++i) {
    let mut hsub: c_uint = i > 0 ? fmtinfo.hsub : 1;
    let mut vsub: c_uint = i > 0 ? fmtinfo.vsub : 1;
    if (wpf.flip.rotate)
...
// format->plane_fmt[1].bytesperline;
    ...
    let mut hoffset: c_uint = max(0, (int)format.width - 16);
//
