...
//! Source: drivers/media/platform/renesas/rcar-vin/rcar-core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn rvin_group_cleanup(group: *mut rvin_group) {
...
#[no_mangle]
unsafe extern "C" fn rvin_group_release(kref: *mut kref) {
    static void rvin_group_release(struct kref *kref)
    {
    struct rvin_group *group =
    container_of(kref, struct rvin_group, refcount);
    mutex_lock(&rvin_group_lock);
    rvin_group_data = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn rvin_group_put(vin: *mut rvin_dev) {
    static void rvin_group_put(struct rvin_dev *vin)
    {
    struct rvin_group *group = vin.group;
    mutex_lock(&group.lock);
    vin.group = core::ptr::null_mut();
    vin.v4l2_dev.mdev = core::ptr::null_mut();
    if (WARN_ON(group.vin[vin.id] != vin))
    goto out;
    group.vin[vin.id] = core::ptr::null_mut();
    out:
    mutex_unlock(&group.lock);
    kref_put(&group.refcount, rvin_group_release);
    }
...
#[no_mangle]
unsafe extern "C" fn rvin_group_notify_complete(notifier: *mut v4l2_async_notifier) -> c_int {
...
// Register all video nodes for the group.
    ...
    continue;
    group.vin[i].parallel.subdev = core::ptr::null_mut();
    ...
    continue;
    group.remotes[i].subdev = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn rvin_parallel_parse_of(vin: *mut rvin_dev) -> c_int {
    static int rvin_parallel_parse_of(struct rvin_dev *vin)
    {
    struct fwnode_handle *fwnode __free(fwnode_handle) = core::ptr::null_mut();
    struct fwnode_handle *ep __free(fwnode_handle) = core::ptr::null_mut();
    struct v4l2_fwnode_endpoint vep = {
    ...
    unsigned int max_id)
    {
    let mut count: c_uint = 0, vin_mask = 0;
    unsigned int i, id;
    int ret;
    mutex_lock(&vin.group.lock);
...
//
#[no_mangle]
unsafe extern "C" fn rvin_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rvin_free_controls(vin: *mut rvin_dev) {
    static void rvin_free_controls(struct rvin_dev *vin)
    {
    v4l2_ctrl_handler_free(&vin.ctrl_handler);
    vin.vdev.ctrl_handler = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn rvin_create_controls(vin: *mut rvin_dev) -> c_int {
...
// Get remote CSI-2, if any.
    ...
    let mut source_idx: c_uint = rvin_group_csi_channel_to_pad(channel);
...
#[no_mangle]
unsafe extern "C" fn rvin_parallel_setup_links(group: *mut rvin_group) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rvin_csi2_setup_links(group: *mut rvin_group) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rvin_csi2_init(vin: *mut rvin_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rvin_isp_setup_links(group: *mut rvin_group) -> c_int {
...
// Create all media device links between VINs and ISP's.
    ...
    let mut source_slot: c_uint = i / 8;
    let mut source_idx: c_uint = i % 8 + 1;
    struct rvin_dev *vin = group.vin[i];
    if (!vin)
    continue;
...
#[no_mangle]
unsafe extern "C" fn rvin_isp_init(vin: *mut rvin_dev) -> c_int {
    static int rvin_isp_init(struct rvin_dev *vin)
    {
    int ret;
    ret = rvin_group_get(vin, rvin_isp_setup_links, core::ptr::null_mut());
    if (ret)
    return ret;
...
//
#[no_mangle]
unsafe extern "C" fn rvin_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rvin_resume(dev: *mut device) -> c_int {
...
//
    ...
    let mut master_id: c_uint = rvin_group_id_to_master(vin.id);
...
#[no_mangle]
unsafe extern "C" fn rvin_id_get(vin: *mut rvin_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rvin_id_put(vin: *mut rvin_dev) {
...
#[no_mangle]
unsafe extern "C" fn rcar_vin_probe(pdev: *mut platform_device) -> c_int {
    ...
    break;
    default:
    ret = rvin_group_get(vin, rvin_parallel_setup_links, core::ptr::null_mut());
    if (!ret)
...
#[no_mangle]
unsafe extern "C" fn rcar_vin_remove(pdev: *mut platform_device) {
