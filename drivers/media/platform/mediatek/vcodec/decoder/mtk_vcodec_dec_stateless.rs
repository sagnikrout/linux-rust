...
//! Source: drivers/media/platform/mediatek/vcodec/decoder/mtk_vcodec_dec_stateless.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_stateless_control {
...
#[no_mangle]
unsafe extern "C" fn mtk_vcodec_dec_request_release(ref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn vb2ops_vdec_buf_request_complete(vb: *mut vb2_buffer) {
...
#[no_mangle]
unsafe extern "C" fn mtk_vdec_worker(work: *mut work_struct) {
    ...
    if (!bs_src.va) {
    v4l2_m2m_job_finish(dev.m2m_dev_dec, ctx.m2m_ctx);
    mtk_v4l2_vdec_err(ctx, "[%d] id=%d source buffer is core::ptr::null_mut()", ctx.id,
    vb2_src.index);
    return;
    }
...
// Apply request controls.
    ...
    if (WARN_ON(!mreq)) {
    v4l2_m2m_job_finish(dev.m2m_dev_dec, ctx.m2m_ctx);
    mtk_v4l2_vdec_err(ctx, "vb2 buffer media request is core::ptr::null_mut()");
    return;
    }
...
//
    ...
    ret = vdec_if_decode(ctx, bs_src, core::ptr::null_mut(), &res_chg);
    if (ret == -EAGAIN)
    goto done;
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn vb2ops_vdec_stateless_buf_queue(vb: *mut vb2_buffer) {
...
#[no_mangle]
unsafe extern "C" fn mtk_vdec_flush_decoder(ctx: *mut mtk_vcodec_dec_ctx) -> c_int {
    static int mtk_vdec_flush_decoder(struct mtk_vcodec_dec_ctx *ctx)
    {
    bool res_chg;
    return vdec_if_decode(ctx, core::ptr::null_mut(), core::ptr::null_mut(), &res_chg);
    }
#[no_mangle]
unsafe extern "C" fn mtk_vcodec_get_pic_info(ctx: *mut mtk_vcodec_dec_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mtk_vdec_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mtk_vcodec_dec_ctrls_setup(ctx: *mut mtk_vcodec_dec_ctx) -> c_int {
    ...
    let mut cfg: v4l2_ctrl_config = mtk_stateless_controls[i].cfg;
    cfg.ops = &mtk_vcodec_dec_ctrl_ops;
    mtk_vcodec_dec_reset_controls(&cfg, ctx);
    v4l2_ctrl_new_custom(&ctx.ctrl_hdl, &cfg, core::ptr::null_mut());
    if (ctx.ctrl_hdl.error) {
...
#[no_mangle]
unsafe extern "C" fn fops_media_request_free(mreq: *mut media_request) {
...
#[no_mangle]
unsafe extern "C" fn fops_media_request_validate(mreq: *mut media_request) -> c_int {
    static int fops_media_request_validate(struct media_request *mreq)
    {
    let mut buffer_cnt: c_uint = vb2_request_buffer_cnt(mreq);
    switch (buffer_cnt) {
    case 1:
...
#[no_mangle]
unsafe extern "C" fn fops_media_request_queue(mreq: *mut media_request) {
    ...
    let mut count_formats: c_int = *pdata.num_formats;
    switch (fourcc) {
...
#[no_mangle]
unsafe extern "C" fn mtk_vcodec_get_supported_formats(ctx: *mut mtk_vcodec_dec_ctx) {
    static void mtk_vcodec_get_supported_formats(struct mtk_vcodec_dec_ctx *ctx)
    {
    let mut cap_format_count: c_int = 0, out_format_count = 0;
    if (num_formats)
    return;
...
#[no_mangle]
unsafe extern "C" fn mtk_init_vdec_params(ctx: *mut mtk_vcodec_dec_ctx) {
...
#[no_mangle]
unsafe extern "C" fn vb2ops_vdec_out_buf_validate(vb: *mut vb2_buffer) -> c_int {
