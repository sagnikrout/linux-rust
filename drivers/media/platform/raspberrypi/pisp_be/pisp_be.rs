...
//! Source: drivers/media/platform/raspberrypi/pisp_be/pisp_be.c
#![no_std]
...
pub const PISP_BE_VERSION_REG: c_uint = 0x0;
pub const PISP_BE_CONTROL_REG: c_uint = 0x4;

pub const PISP_BE_TILE_ADDR_LO_REG: c_uint = 0x8;
pub const PISP_BE_TILE_ADDR_HI_REG: c_uint = 0xc;
pub const PISP_BE_STATUS_REG: c_uint = 0x10;

pub const PISP_BE_BATCH_STATUS_REG: c_uint = 0x14;
pub const PISP_BE_INTERRUPT_EN_REG: c_uint = 0x18;
pub const PISP_BE_INTERRUPT_STATUS_REG: c_uint = 0x1c;
pub const PISP_BE_AXI_REG: c_uint = 0x20;
pub const PISP_BE_CONFIG_BASE_REG: c_uint = 0x40;

pub const PISP_BE_GLOBAL_BAYER_ENABLE: c_uint = 0xb0;
pub const PISP_BE_GLOBAL_RGB_ENABLE: c_uint = 0xb4;
pub const N_HW_ADDRESSES: c_int = 13;
pub const N_HW_ENABLES: c_int = 2;
pub const PISP_BE_VERSION_2712: c_uint = 0x02252700;
pub const PISP_BE_VERSION_MINOR_BITS: c_uint = 0xf;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pispbe_node_description {
    pub ent_name: *const c_char,
    pub buf_type: enum v4l2_buf_type,
    pub caps: c_uint,
}
...
// vb2_queue lock
    ...
    pub pisp_format: *const pisp_be_format,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pispbe_job_descriptor {
    pub queue: list_head,
    pub buffers: [*mut pispbe_buffer; PISPBE_NUM_NODES],
    pub hw_dma_addrs: [dma_addr_t; N_HW_ADDRESSES],
    pub config: *mut pisp_be_tiles_config,
    pub hw_enables: pispbe_hw_enables,
    pub tiles: dma_addr_t,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pispbe_dev {
    pub dev: *mut device,
    pub pispbe: *mut pispbe_dev,
    pub config: *mut pisp_be_tiles_config,
    pub be_reg_base: *mut void __iomem,
    pub clk: *mut clk,
    pub v4l2_dev: v4l2_device,
    pub sd: v4l2_subdev,
    pub mdev: media_device,
    pub /: *mut *mut media_pad pad[PISPBE_NUM_NODES]; / output pads first,
    pub node: [pispbe_node; PISPBE_NUM_NODES],
    pub config_dma_addr: dma_addr_t,
    pub sequence: c_uint,
    pub streaming_map: u32,
    pub running_job: pispbe_job queued_job,,
// protects "hw_busy" flag, streaming_map and job_queue
    pub hw_lock: spinlock_t,
    pub /: *mut *mut bool hw_busy; / non-zero if a job is queued or is being started,
...
#[no_mangle]
unsafe extern "C" fn pispbe_rd(pispbe: *mut pispbe_dev, offset: c_uint) -> u32 {
    static u32 pispbe_rd(struct pispbe_dev *pispbe, unsigned int offset)
    {
    return readl(pispbe.be_reg_base + offset);
    }
#[no_mangle]
unsafe extern "C" fn pispbe_wr(pispbe: *mut pispbe_dev, offset: c_uint, val: u32) {
...
// Read back the addresses -- an error here could be fatal
    for (unsigned int u = 0; u < N_HW_ADDRESSES; ++u) {
    let mut offset: c_uint = PISP_BE_IO_ADDR_LOW(u);
    let mut along: u64 = pispbe_rd(pispbe, offset);
...
pub struct pispbe_buffer {
    ...
    {
    let mut num_planes: c_uint = node.format.fmt.pix_mp.num_planes;
    let mut plane_factor: c_uint = 0;
    unsigned int size;
    unsigned int p;
...
#[no_mangle]
unsafe extern "C" fn pispbe_get_addr(buf: *mut pispbe_buffer) -> dma_addr_t {
...
//
#[no_mangle]
unsafe extern "C" fn pispbe_prepare_job(pispbe: *mut pispbe_dev) -> c_int {
    static int pispbe_prepare_job(struct pispbe_dev *pispbe)
    {
    struct pispbe_job_descriptor __free(kfree) *job = core::ptr::null_mut();
    struct pispbe_buffer *buf[PISPBE_NUM_NODES] = {};
    ...
    let mut mask: static u32 = BIT(CONFIG_NODE) | BIT(MAIN_INPUT_NODE);
...
// remember: srcimages, captures then metadata
    ...
    let mut ignore_buffers: bool = false;
...
#[no_mangle]
unsafe extern "C" fn pispbe_schedule(pispbe: *mut pispbe_dev, clear_hw_busy: bool) {
...
#[no_mangle]
unsafe extern "C" fn pispbe_isr(irq: c_int, dev: *mut c_void) -> irqreturn_t {
    static irqreturn_t pispbe_isr(int irq, void *dev)
    {
    struct pispbe_dev *pispbe = (struct pispbe_dev *)dev;
    let mut can_queue_another: bool = false;
    u8 started, done;
    u32 u;
...
// check if there's more to do before going to sleep
    ...
    {
    let mut bayer_enables: u32 = config.config.global.bayer_enables;
    let mut rgb_enables: u32 = config.config.global.rgb_enables;
...
#[no_mangle]
unsafe extern "C" fn pispbe_node_buffer_prepare(vb: *mut vb2_buffer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pispbe_node_buffer_queue(buf: *mut vb2_buffer) {
...
#[no_mangle]
unsafe extern "C" fn pispbe_node_start_streaming(q: *mut vb2_queue, count: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pispbe_node_stop_streaming(q: *mut vb2_queue) {
...
// f = node->format;
    ...
    {
    let mut nplanes: c_uint = f.fmt.pix_mp.num_planes;
    let mut total_plane_factor: c_uint = 0;
...
#[no_mangle]
unsafe extern "C" fn pispbe_try_format(f: *mut v4l2_format, node: *mut pispbe_node) {
    static void pispbe_try_format(struct v4l2_format *f, struct pispbe_node *node)
    {
    struct pispbe_dev *pispbe = node.pispbe;
    let mut pixfmt: u32 = f.fmt.pix_mp.pixelformat;
    const struct pisp_be_format *fmt;
    bool is_rgb;
    dev_dbg(pispbe.dev,
...
#[no_mangle]
unsafe extern "C" fn pispbe_node_def_fmt(node: *mut pispbe_node) {
...
//
#[no_mangle]
unsafe extern "C" fn pispbe_init_node(pispbe: *mut pispbe_dev, id: c_uint) -> c_int {
    static int pispbe_init_node(struct pispbe_dev *pispbe, unsigned int id)
    {
    let mut output: bool = NODE_DESC_IS_OUTPUT(&node_desc[id]);
...
#[no_mangle]
unsafe extern "C" fn pispbe_init_subdev(pispbe: *mut pispbe_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pispbe_init_devices(pispbe: *mut pispbe_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pispbe_destroy_devices(pispbe: *mut pispbe_dev) {
...
#[no_mangle]
unsafe extern "C" fn pispbe_runtime_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pispbe_runtime_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pispbe_hw_init(pispbe: *mut pispbe_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pispbe_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (ret)
    return ret;
    pispbe.clk = devm_clk_get(&pdev.dev, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn pispbe_remove(pdev: *mut platform_device) {
    ...
    SET_RUNTIME_PM_OPS(pispbe_runtime_suspend, pispbe_runtime_resume, core::ptr::null_mut())
    };
