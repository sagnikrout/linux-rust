...
//! Source: drivers/media/platform/cadence/cdns-csi2rx.c
#![no_std]
...
pub const CSI2RX_DEVICE_CFG_REG: c_uint = 0x000;
pub const CSI2RX_SOFT_RESET_REG: c_uint = 0x004;

pub const CSI2RX_STATIC_CFG_REG: c_uint = 0x008;

pub const CSI2RX_DPHY_LANE_CTRL_REG: c_uint = 0x40;

pub const CSI2RX_LANES_MAX: c_int = 4;
pub const CSI2RX_STREAMS_MAX: c_int = 4;
pub const CSI2RX_ERROR_IRQS_REG: c_uint = 0x28;
pub const CSI2RX_ERROR_IRQS_MASK_REG: c_uint = 0x2C;
...
pub struct csi2rx_priv {
    ...
    pub sys_clk: *mut clk,
    pub p_clk: *mut clk,
    pub pixel_clk: [*mut clk; CSI2RX_STREAMS_MAX],
    pub sys_rst: *mut reset_control,
    pub p_rst: *mut reset_control,
    pub pixel_rst: [*mut reset_control; CSI2RX_STREAMS_MAX],
    pub dphy: *mut phy,
    pub num_pixels: [u8; CSI2RX_STREAMS_MAX],
    pub vc_select: [u32; CSI2RX_STREAMS_MAX],
    pub lanes: [u8; CSI2RX_LANES_MAX],
    pub num_lanes: u8,
    pub max_lanes: u8,
    pub max_streams: u8,
    pub has_internal_dphy: bool,
    pub events: [u32; CSI2RX_NUM_EVENTS],
    pub subdev: v4l2_subdev,
    pub notifier: v4l2_async_notifier,
    pub pads: [media_pad; CSI2RX_PAD_MAX],
// Remote source
    pub source_subdev: *mut v4l2_subdev,
    pub source_pad: c_int,
}
...
//
    ...
    if (remote_pad) {
    let mut pad: c_int = i - CSI2RX_PAD_SOURCE_STREAM0;
    let mut bit_mask: u32 = CSI2RX_STREAM0_FIFO_OVERFLOW_IRQ << pad;
...
#[no_mangle]
unsafe extern "C" fn csi2rx_irq_handler(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_reset(csi2rx: *mut csi2rx_priv) {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_configure_ext_dphy(csi2rx: *mut csi2rx_priv) -> c_int {
    ...
    let mut source_pad: c_int = csi2rx.source_pad;
...
#[no_mangle]
unsafe extern "C" fn csi2rx_start(csi2rx: *mut csi2rx_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_stop(csi2rx: *mut csi2rx_priv) {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_log_status(sd: *mut v4l2_subdev) -> c_int {
    ...
    struct v4l2_subdev_state *state)
    {
    let mut fd: v4l2_mbus_frame_desc = {0};
    struct v4l2_subdev_route *route;
    unsigned int i;
    int ret;
...
// If source provides per-stream VC info, use it to filter by VC
    ...
    let mut cdns_stream: u32 = route.source_pad - CSI2RX_PAD_SOURCE_STREAM0;
...
#[no_mangle]
unsafe extern "C" fn csi2rx_parse_dt(csi2rx: *mut csi2rx_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_runtime_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn csi2rx_runtime_resume(dev: *mut device) -> c_int {
    ...
    RUNTIME_PM_OPS(csi2rx_runtime_suspend, csi2rx_runtime_resume, core::ptr::null_mut())
    };
