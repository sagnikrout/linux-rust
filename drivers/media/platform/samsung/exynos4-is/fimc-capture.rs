...
//! Source: drivers/media/platform/samsung/exynos4-is/fimc-capture.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn fimc_capture_hw_init(fimc: *mut fimc_dev) -> c_int {
    ...
    int ret;
    unsigned long flags;
    if (ctx == core::ptr::null_mut() || ctx.s_frame.fmt == core::ptr::null_mut())
    return -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn fimc_capture_state_cleanup(fimc: *mut fimc_dev, suspend: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fimc_stop_capture(fimc: *mut fimc_dev, suspend: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn fimc_capture_config_update(ctx: *mut fimc_ctx) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn fimc_capture_irq_handler(fimc: *mut fimc_dev, deq_buf: c_int) {
...
//
    ...
    let mut plane: c_uint = ffs(f.fmt.mdataplanes) - 1;
    let mut size: c_uint = f.payload[plane];
    let mut index: i32 = fimc_hw_get_frame_index(fimc);
    void *vaddr;
...
#[no_mangle]
pub unsafe extern "C" fn fimc_capture_suspend(fimc: *mut fimc_dev) -> c_int {
    int fimc_capture_suspend(struct fimc_dev *fimc)
    {
    let mut suspend: bool = fimc_capture_busy(fimc);
    let mut ret: c_int = fimc_stop_capture(fimc, suspend);
    if (ret)
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn fimc_capture_resume(fimc: *mut fimc_dev) -> c_int {
    ...
    let mut wh: c_ulong = frame.f_width * frame.f_height;
    int i;
...
// num_planes = fmt->memplanes;
    ...
    let mut size: c_uint = (wh * fmt.depth[i]) / 8;
...
unsafe extern "C" fn buffer_prepare(vb: *mut vb2_buffer) -> c_int {
    ...
    struct fimc_ctx *ctx = vq.drv_priv;
    int i;
    if (ctx.d_frame.fmt == core::ptr::null_mut())
    return -EINVAL;
    for (i = 0; i < ctx.d_frame.fmt.memplanes; i++) {
    let mut size: c_ulong = ctx.d_frame.payload[i];
...
#[no_mangle]
unsafe extern "C" fn fimc_capture_open(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fimc_capture_release(file: *mut file) -> c_int {
    static int fimc_capture_release(struct file *file)
    {
    struct fimc_dev *fimc = video_drvdata(file);
    struct fimc_vid_cap *vc = &fimc.vid_cap;
    let mut close: bool = v4l2_fh_is_singular_file(file);
    int ret;
    dbg("pid: %d, state: 0x%lx", task_pid_nr(current), fimc.state);
    mutex_lock(&fimc.lock);
...
//
    ...
    {
    let mut rotation: bool = ctx.rotation == 90 || ctx.rotation == 270;
    struct fimc_dev *fimc = ctx.fimc_dev;
    ...
    let mut mask: u32 = FMT_FLAGS_CAM;
...
// Apply the scaler and the output DMA constraints
    ...
    int target)
    {
    let mut rotate: bool = ctx.rotation == 90 || ctx.rotation == 270;
    struct fimc_dev *fimc = ctx.fimc_dev;
    const struct fimc_variant *var = fimc.variant;
    const struct fimc_pix_limit *pl = var.pix_limit;
    const struct fimc_frame *sink = &ctx.s_frame;
    u32 max_w, max_h, min_w = 0, min_h = 0, min_sz;
    let mut align_sz: u32 = 0, align_h = 4;
...
// In JPEG transparent transfer mode cropping is not supported
    ...
    let mut tmp_min_h: u32 = ffs(sink.width) - 3;
    let mut tmp_min_v: u32 = ffs(sink.height) - 1;
    ...
    let mut depth: u32 = fimc_get_format_depth(sink.fmt);
...
//
    ...
    fmt = fimc_find_format(core::ptr::null_mut(), core::ptr::null_mut(), FMT_FLAGS_CAM | FMT_FLAGS_M2M,
    f.index);
    if (!fmt)
    return -EINVAL;
...
//
    ...
    while (1) {
    ffmt = fimc_find_format(core::ptr::null_mut(), mf.code != 0 ? &mf.code : core::ptr::null_mut(),
    FMT_FLAGS_CAM, i++);
    if (ffmt == core::ptr::null_mut()) {
//
...
// set format on all pipeline subdevs
    ...
    sfmt.pad = 0;
    ret = v4l2_subdev_call(sd, pad, set_fmt, core::ptr::null_mut(), &sfmt);
    if (ret)
    return ret;
    if (me.pads[0].flags & MEDIA_PAD_FL_SINK) {
    sfmt.pad = me.num_pads - 1;
    mf.code = tfmt.code;
    ret = v4l2_subdev_call(sd, pad, set_fmt, core::ptr::null_mut(),
    &sfmt);
    if (ret)
    return ret;
    }
    ...
    core::ptr::null_mut(), &fcc, FIMC_SD_PAD_SINK_CAM);
    ffmt = fimc_capture_try_format(ctx, &tfmt.width, &tfmt.height,
    core::ptr::null_mut(), &fcc, FIMC_SD_PAD_SOURCE);
...
//
    ...
    {
    let mut fd: v4l2_mbus_frame_desc = { };
    int i, ret;
    int pad;
    for (i = 0; i < num_planes; i++)
...
// Try to match format at the host and the sensor
    ...
    {
    struct fimc_dev *fimc = video_drvdata(file);
    const struct fimc_fmt *out_fmt = core::ptr::null_mut(), *inp_fmt = core::ptr::null_mut();
    return __video_try_or_set_format(fimc, f, true, &inp_fmt, &out_fmt);
    }
    static void fimc_capture_mark_jpeg_xfer(struct fimc_ctx *ctx,
    enum fimc_color_fmt color)
    {
    let mut jpeg: bool = fimc_fmt_is_user_defined(color);
    ...
    const struct fimc_fmt *inp_fmt = core::ptr::null_mut();
    int ret, i;
...
#[no_mangle]
unsafe extern "C" fn fimc_cap_s_input(file: *mut file, priv: *mut c_void, i: c_uint) -> c_int {
    static int fimc_cap_s_input(struct file *file, void *priv, unsigned int i)
    {
    let mut i: return = = 0 ? i : -EINVAL;
    }
#[no_mangle]
unsafe extern "C" fn fimc_cap_g_input(file: *mut file, priv: *mut c_void, i: *mut c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn fimc_pipeline_validate(fimc: *mut fimc_dev) -> c_int {
...
// Don't call FIMC subdev operation to avoid nested locking
    ...
    ret = v4l2_subdev_call(sd, pad, get_fmt, core::ptr::null_mut(), &sink_fmt);
...
// Retrieve format at the source pad
    ...
    ret = v4l2_subdev_call(sd, pad, get_fmt, core::ptr::null_mut(), &src_fmt);
    if (ret < 0 && ret != -ENOIOCTLCMD)
    return -EPIPE;
    ...
    struct fimc_dev *fimc = video_drvdata(file);
    struct fimc_vid_cap *vc = &fimc.vid_cap;
    struct fimc_source_info *si = core::ptr::null_mut();
    struct v4l2_subdev *sd;
    int ret;
    if (fimc_capture_active(fimc))
    return -EBUSY;
...
//
    ...
    f = &ctx.d_frame;
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_SEL_TGT_CROP: s->target ==) -> else {
...
// Capture subdev media entity operations
    ...
    return -EINVAL;
    if (WARN_ON(fimc == core::ptr::null_mut()))
    return 0;
...
// Inherit V4L2 controls from the image sensor subdev.
    ...
    if (sensor == core::ptr::null_mut())
    return 0;
    return v4l2_ctrl_add_handler(&vc.ctx.ctrls.handler,
    sensor.ctrl_handler, core::ptr::null_mut(), true);
    }
...
//
    ...
    fimc = si ? source_to_sensor_info(si).host : core::ptr::null_mut();
...
// ((u32 *)arg));
    ...
    {
    const struct fimc_fmt *fmt;
    fmt = fimc_find_format(core::ptr::null_mut(), core::ptr::null_mut(), FMT_FLAGS_CAM, code.index);
    if (!fmt)
    return -EINVAL;
    ...
    if (!WARN_ON(ff.fmt == core::ptr::null_mut()))
...
// mf = fimc->vid_cap.ci_fmt;
    ...
    &mf.code, core::ptr::null_mut(), fmt.pad);
    mutex_unlock(&fimc.lock);
    mf.colorspace = V4L2_COLORSPACE_JPEG;
    if (fmt.which == V4L2_SUBDEV_FORMAT_TRY) {
    mf = v4l2_subdev_state_get_format(sd_state, fmt.pad);
...
// There must be a bug in the driver if this happens
    if (WARN_ON(ffmt == core::ptr::null_mut()))
    return -EINVAL;
// Update RGB Alpha control state and value range
    fimc_alpha_ctrl_update(ctx);
...
// Sink pads crop rectangle size
    ...
    vc.wb_fmt = *mf;
#[no_mangle]
pub unsafe extern "C" fn if(FIMC_SD_PAD_SINK_CAM: fmt->pad ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn fimc_capture_set_default_format(fimc: *mut fimc_dev) -> c_int {
    static int fimc_capture_set_default_format(struct fimc_dev *fimc)
    {
...
// fimc->lock must be already initialized
    ...
    ctx.s_frame.fmt = fimc_find_format(core::ptr::null_mut(), core::ptr::null_mut(), FMT_FLAGS_CAM, 0);
...
// Default format configuration
    fmt = fimc_find_format(core::ptr::null_mut(), core::ptr::null_mut(), FMT_FLAGS_CAM, 0);
    ...
    ctx.s_frame.fmt = fmt;
    fmt = fimc_find_format(core::ptr::null_mut(), core::ptr::null_mut(), FMT_FLAGS_WRITEBACK, 0);
...
#[no_mangle]
unsafe extern "C" fn fimc_capture_subdev_registered(sd: *mut v4l2_subdev) -> c_int {
    static int fimc_capture_subdev_registered(struct v4l2_subdev *sd)
    {
    struct fimc_dev *fimc = v4l2_get_subdevdata(sd);
    int ret;
    if (fimc == core::ptr::null_mut())
    return -ENXIO;
    ...
    fimc.vid_cap.ve.pipe = core::ptr::null_mut();
    }
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn fimc_capture_subdev_unregistered(sd: *mut v4l2_subdev) {
    ...
    fimc.vid_cap.ctx = core::ptr::null_mut();
    mutex_unlock(&fimc.lock);
    }
...
#[no_mangle]
pub unsafe extern "C" fn fimc_initialize_capture_subdev(fimc: *mut fimc_dev) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn fimc_unregister_capture_subdev(fimc: *mut fimc_dev) {
    void fimc_unregister_capture_subdev(struct fimc_dev *fimc)
    {
    struct v4l2_subdev *sd = &fimc.vid_cap.subdev;
    v4l2_device_unregister_subdev(sd);
    media_entity_cleanup(&sd.entity);
    v4l2_set_subdevdata(sd, core::ptr::null_mut());
    }
