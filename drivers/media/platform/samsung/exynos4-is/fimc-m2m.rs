...
//! Source: drivers/media/platform/samsung/exynos4-is/fimc-m2m.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn get_m2m_fmt_flags(stream_type: c_uint) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn fimc_m2m_job_finish(ctx: *mut fimc_ctx, vb_state: c_int) {
...
#[no_mangle]
unsafe extern "C" fn fimc_m2m_shutdown(ctx: *mut fimc_ctx) {
...
#[no_mangle]
unsafe extern "C" fn fimc_device_run(priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn fimc_job_abort(priv: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn fimc_buf_prepare(vb: *mut vb2_buffer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fimc_buf_queue(vb: *mut vb2_buffer) {
...
//
    ...
    fmt = fimc_find_format(core::ptr::null_mut(), core::ptr::null_mut(), get_m2m_fmt_flags(f.type),
    f.index);
    if (!fmt)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn fimc_try_fmt_mplane(ctx: *mut fimc_ctx, f: *mut v4l2_format) -> c_int {
    ...
    if (!IS_M2M(f.type))
    return -EINVAL;
    fmt = fimc_find_format(&pix.pixelformat, core::ptr::null_mut(),
    get_m2m_fmt_flags(f.type), 0);
    if (WARN(fmt == core::ptr::null_mut(), "Pixel format lookup failed"))
    return -EINVAL;
    if (pix.field == V4L2_FIELD_ANY)
    pix.field = V4L2_FIELD_NONE;
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_FIELD_NONE: pix->field !=) -> else {
    ...
    frame = &ctx.d_frame;
    fmt = fimc_find_format(&f.fmt.pix_mp.pixelformat, core::ptr::null_mut(),
    get_m2m_fmt_flags(f.type), 0);
    if (!fmt)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn fimc_m2m_set_default_format(ctx: *mut fimc_ctx) -> c_int {
    ...
    const struct fimc_fmt *fmt;
    fmt = fimc_find_format(&pixm.pixelformat, core::ptr::null_mut(), FMT_FLAGS_M2M, 0);
    if (!fmt)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn fimc_m2m_open(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fimc_m2m_release(file: *mut file) -> c_int {
    ...
    ret = media_entity_pads_init(&vfd.entity, 0, core::ptr::null_mut());
    if (ret)
    goto err_me;
...
#[no_mangle]
pub unsafe extern "C" fn fimc_unregister_m2m_device(fimc: *mut fimc_dev) {
