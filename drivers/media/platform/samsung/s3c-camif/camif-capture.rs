...
//! Source: drivers/media/platform/samsung/s3c-camif/camif-capture.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn camif_cfg_video_path(vp: *mut camif_vp) {
...
#[no_mangle]
unsafe extern "C" fn camif_prepare_dma_offset(vp: *mut camif_vp) {
...
#[no_mangle]
unsafe extern "C" fn s3c_camif_hw_init(camif: *mut camif_dev, vp: *mut camif_vp) -> c_int {
    static int s3c_camif_hw_init(struct camif_dev *camif, struct camif_vp *vp)
    {
    const struct s3c_camif_variant *variant = camif.variant;
    if (camif.sensor.sd == core::ptr::null_mut() || vp.out_fmt == core::ptr::null_mut())
    return -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn s3c_camif_hw_vp_init(camif: *mut camif_dev, vp: *mut camif_vp) -> c_int {
    static int s3c_camif_hw_vp_init(struct camif_dev *camif, struct camif_vp *vp)
    {
    let mut ip_rev: c_uint = camif.variant.ip_revision;
    if (vp.out_fmt == core::ptr::null_mut())
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn sensor_set_power(camif: *mut camif_dev, on: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sensor_set_streaming(camif: *mut camif_dev, on: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn camif_reinitialize(vp: *mut camif_vp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s3c_vp_active(vp: *mut camif_vp) -> bool {
...
#[no_mangle]
unsafe extern "C" fn camif_is_streaming(camif: *mut camif_dev) -> bool {
...
#[no_mangle]
unsafe extern "C" fn camif_stop_capture(vp: *mut camif_vp) -> c_int {
...
// Timed out, forcibly stop capture
    ...
    u32 pix_size;
    if (vb == core::ptr::null_mut() || frame == core::ptr::null_mut())
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn s3c_camif_irq_handler(irq: c_int, priv: *mut c_void) -> irqreturn_t {
...
//
    ...
    if (!WARN_ON(vbuf == core::ptr::null_mut())) {
...
#[no_mangle]
unsafe extern "C" fn s3c_camif_open(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s3c_camif_close(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s3c_camif_mmap(file: *mut file, vma: *mut vm_area_struct) -> c_int {
...
//
    ...
    if (input.index || sensor == core::ptr::null_mut())
    return -EINVAL;
...
// i = 0;
    ...
    fmt = s3c_camif_find_format(vp, core::ptr::null_mut(), f.index);
    if (!fmt)
    return -EINVAL;
    ...
    if (WARN_ON(fmt == core::ptr::null_mut()))
    return -EINVAL;
    if (ffmt)
// ffmt = fmt;
...
//
    ...
    {
    struct camif_vp *vp = video_drvdata(file);
    return __camif_video_try_format(vp, &f.fmt.pix, core::ptr::null_mut());
    }
    ...
    const struct camif_fmt *fmt = core::ptr::null_mut();
    int ret;
    pr_debug("[vp%d]\n", vp.id);
...
#[no_mangle]
unsafe extern "C" fn camif_pipeline_validate(camif: *mut camif_dev) -> c_int {
...
// Retrieve format at the sensor subdev source pad
    ...
    ret = v4l2_subdev_call(camif.sensor.sd, pad, get_fmt, core::ptr::null_mut(), &src_fmt);
    ...
    if (vp.owner == core::ptr::null_mut() && rb.count > 0)
    vp.owner = file_to_v4l2_fh(file);
    return ret;
    }
    ...
    if (!ret && vp.owner == core::ptr::null_mut())
    vp.owner = file_to_v4l2_fh(file);
    return ret;
    }
...
//
#[no_mangle]
unsafe extern "C" fn s3c_camif_video_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn s3c_camif_register_video_node(camif: *mut camif_dev, idx: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn s3c_camif_unregister_video_node(camif: *mut camif_dev, idx: c_int) {
...
#[no_mangle]
unsafe extern "C" fn __camif_try_crop(camif: *mut camif_dev, r: *mut v4l2_rect) {
    ...
    let mut left: c_uint = 2 * r.left;
    let mut top: c_uint = 2 * r.top;
//
...
#[no_mangle]
unsafe extern "C" fn s3c_camif_subdev_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn s3c_camif_create_subdev(camif: *mut camif_dev) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn s3c_camif_unregister_subdev(camif: *mut camif_dev) {
    void s3c_camif_unregister_subdev(struct camif_dev *camif)
    {
    struct v4l2_subdev *sd = &camif.subdev;
// Return if not registered
    if (v4l2_get_subdevdata(sd) == core::ptr::null_mut())
    return;
...
#[no_mangle]
pub unsafe extern "C" fn s3c_camif_set_defaults(camif: *mut camif_dev) -> c_int {
    ...
    vp.out_fmt = s3c_camif_find_format(vp, core::ptr::null_mut(), 0);
    BUG_ON(vp.out_fmt == core::ptr::null_mut());
    memset(f, 0, sizeof(*f));
