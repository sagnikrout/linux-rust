...
//! Source: drivers/media/platform/verisilicon/hantro_postproc.c
#![no_std]
...
pub const VPU_PP_IN_YUYV: c_uint = 0x0;
pub const VPU_PP_IN_NV12: c_uint = 0x1;
pub const VPU_PP_IN_YUV420: c_uint = 0x2;
pub const VPU_PP_IN_YUV240_TILED: c_uint = 0x5;
pub const VPU_PP_OUT_RGB: c_uint = 0x0;
pub const VPU_PP_OUT_YUYV: c_uint = 0x3;
...
#[no_mangle]
unsafe extern "C" fn hantro_postproc_g1_enable(ctx: *mut hantro_ctx) {
...
#[no_mangle]
unsafe extern "C" fn down_scale_factor(ctx: *mut hantro_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hantro_postproc_g2_enable(ctx: *mut hantro_ctx) {
    static void hantro_postproc_g2_enable(struct hantro_ctx *ctx)
    {
    struct hantro_dev *vpu = ctx.dev;
    struct vb2_v4l2_buffer *dst_buf;
    let mut down_scale: c_int = down_scale_factor(ctx);
    int out_depth;
...
#[no_mangle]
pub unsafe extern "C" fn hantro_postproc_free(ctx: *mut hantro_ctx) {
    ...
    priv.dma, priv.attrs);
    priv.cpu = core::ptr::null_mut();
    }
    }
    }
#[no_mangle]
unsafe extern "C" fn hantro_postproc_buffer_size(ctx: *mut hantro_ctx) -> c_uint {
    ...
    ctx.ref_fmt.height);
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_PIX_FMT_VP9_FRAME: ctx->vpu_src_fmt->fourcc ==) -> else {
    else if (ctx.vpu_src_fmt.fourcc == V4L2_PIX_FMT_VP9_FRAME)
    buf_size += hantro_vp9_mv_size(ctx.ref_fmt.width,
    ctx.ref_fmt.height);
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_PIX_FMT_HEVC_SLICE: ctx->vpu_src_fmt->fourcc ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_PIX_FMT_AV1_FRAME: ctx->vpu_src_fmt->fourcc ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn hantro_postproc_alloc(ctx: *mut hantro_ctx, index: c_int) -> c_int {
    static int hantro_postproc_alloc(struct hantro_ctx *ctx, int index)
    {
    struct hantro_dev *vpu = ctx.dev;
    struct hantro_aux_buf *priv = &ctx.postproc.dec_q[index];
    let mut buf_size: c_uint = hantro_postproc_buffer_size(ctx);
    if (!buf_size)
    return -EINVAL;
//
...
#[no_mangle]
pub unsafe extern "C" fn hantro_postproc_init(ctx: *mut hantro_ctx) -> c_int {
    int hantro_postproc_init(struct hantro_ctx *ctx)
    {
    struct v4l2_m2m_ctx *m2m_ctx = ctx.fh.m2m_ctx;
    struct vb2_queue *cap_queue = &m2m_ctx.cap_q_ctx.q;
    let mut num_buffers: c_uint = vb2_get_num_buffers(cap_queue);
    unsigned int i;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn hantro_postproc_g1_disable(ctx: *mut hantro_ctx) {
...
#[no_mangle]
unsafe extern "C" fn hantro_postproc_g2_disable(ctx: *mut hantro_ctx) {
...
#[no_mangle]
pub unsafe extern "C" fn hantro_postproc_disable(ctx: *mut hantro_ctx) {
...
#[no_mangle]
pub unsafe extern "C" fn hantro_postproc_enable(ctx: *mut hantro_ctx) {
