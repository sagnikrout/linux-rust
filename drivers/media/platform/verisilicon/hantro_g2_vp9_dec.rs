...
//! Source: drivers/media/platform/verisilicon/hantro_g2_vp9_dec.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn start_prepare_run(ctx: *mut hantro_ctx, dec_params: *const v4l2_ctrl_vp9_frame) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hantro_vp9_ref_reg {
    pub width: hantro_reg,
    pub height: hantro_reg,
    pub hor_scale: hantro_reg,
    pub ver_scale: hantro_reg,
    pub y_base: u32,
    pub c_base: u32,
}
...
#[no_mangle]
unsafe extern "C" fn recompute_tile_info(tile_info: *mut c_ushort, tiles: c_uint, sbs: c_uint) {
    static void recompute_tile_info(unsigned short *tile_info, unsigned int tiles, unsigned int sbs)
    {
    int i;
    let mut accumulated: c_uint = 0;
...
#[no_mangle]
pub unsafe extern "C" fn first_tile_row(tile_r: c_uint, sbs_r: c_uint) -> c_uint {
    ...
    let mut r_info: c_ushort = vp9_ctx.tile_r_info[i];
    if (first) {
    if (i > 0)
...
// tile_mem++ = r_info;
    ...
    let mut tile_r: c_uint = (1 << dec_params.tile_rows_log2);
    let mut tile_c: c_uint = (1 << dec_params.tile_cols_log2);
    let mut sbs_r: c_uint = hantro_vp9_num_sbs(dst.vp9.height);
    let mut sbs_c: c_uint = hantro_vp9_num_sbs(dst.vp9.width);
...
// provide aux buffers even if no tiles are used
    ...
    unsigned int segid)
    {
    let mut mask: u8 = V4L2_VP9_SEGMENT_FEATURE_ENABLED(feature);
...
#[no_mangle]
pub unsafe extern "C" fn clip3(x: i16, y: i16, z: i16) -> i16 {
    static inline s16 clip3(s16 x, s16 y, s16 z)
    {
    return (z < x) ? x : (z > y) ? y : z;
    }
#[no_mangle]
unsafe extern "C" fn feat_val_clip3(feat_val: i16, feature_data: i16, absolute: bool, clip: u8) -> i16 {
...
#[no_mangle]
unsafe extern "C" fn config_segment(ctx: *mut hantro_ctx, dec_params: *const v4l2_ctrl_vp9_frame) {
...
#[no_mangle]
unsafe extern "C" fn config_loop_filter(ctx: *mut hantro_ctx, dec_params: *const v4l2_ctrl_vp9_frame) {
    static void config_loop_filter(struct hantro_ctx *ctx, const struct v4l2_ctrl_vp9_frame *dec_params)
    {
    let mut d: bool = dec_params.lf.flags & V4L2_VP9_LOOP_FILTER_FLAG_DELTA_ENABLED;
...
#[no_mangle]
unsafe extern "C" fn config_picture_dimensions(ctx: *mut hantro_ctx, dst: *mut hantro_decoded_buffer) {
...
#[no_mangle]
pub unsafe extern "C" fn is_lossless(quant: *const v4l2_vp9_quantization) -> bool {
    ...
    let mut comp_ref_allowed: bool = 0;
    comp_fixed_ref = 0;
    comp_var_ref[0] = 0;
    comp_var_ref[1] = 0;
...
#[no_mangle]
unsafe extern "C" fn config_probs(ctx: *mut hantro_ctx, dec_params: *const v4l2_ctrl_vp9_frame) {
...
#[no_mangle]
unsafe extern "C" fn config_counts(ctx: *mut hantro_ctx) {
    ...
    let mut addr: dma_addr_t = misc.dma + vp9_dec.ctx_counters_offset;
...
#[no_mangle]
pub unsafe extern "C" fn hantro_g2_vp9_dec_run(ctx: *mut hantro_ctx) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hantro_g2_vp9_dec_done(ctx: *mut hantro_ctx) {
...
pub struct tx_and_skip {
    ...
    pub hantro_cnts: *mut symbol_counts,
    pub tx16p: [u32; 2][4],
    pub i: c_int,
...
// if FrameIsIntra then undo the effect of load_probs2()
    ...
    pub &vp9_ctx->cnts: counts =,
    pub vp9_ctx->ctx_counters_offset: hantro_cnts = vp9_ctx->misc.cpu +,
    pub {: for (i = 0; i < ARRAY_SIZE(tx16p); ++i),
    memcpy(tx16p[i],
    hantro_cnts.tx16x16_count[i],
    pub 0: tx16p[i][3] =,
    }
    pub &tx16p: counts->tx16p =,
...
// load_probs2() already done
    pub mv_mode: [u32; 7][4],
    pub {: for (i = 0; i < ARRAY_SIZE(mv_mode); ++i),
    pub hantro_cnts->inter_mode_counts[i][1][0]: mv_mode[i][0] =,
    pub hantro_cnts->inter_mode_counts[i][2][0]: mv_mode[i][1] =,
    pub hantro_cnts->inter_mode_counts[i][0][0]: mv_mode[i][2] =,
    pub hantro_cnts->inter_mode_counts[i][2][1]: mv_mode[i][3] =,
    }
    pub &mv_mode: counts->mv_mode =,
