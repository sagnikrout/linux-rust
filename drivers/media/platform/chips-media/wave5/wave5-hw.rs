...
//! Source: drivers/media/platform/chips-media/wave5/wave5-hw.c
#![no_std]
...
pub const FIO_TIMEOUT: c_int = 10000000;

pub const VPU_BUSY_CHECK_TIMEOUT: c_int = 10000000;
pub const QUEUE_REPORT_MASK: c_uint = 0xffff;
// Encoder support fields

pub const ENC_AVC_INTRA_IDR_PARAM_MASK: c_uint = 0x7ff;
pub const ENC_AVC_INTRA_PERIOD_SHIFT: c_int = 6;
pub const ENC_AVC_IDR_PERIOD_SHIFT: c_int = 17;
pub const ENC_AVC_FORCED_IDR_HEADER_SHIFT: c_int = 28;
pub const ENC_HEVC_INTRA_QP_SHIFT: c_int = 3;
pub const ENC_HEVC_FORCED_IDR_HEADER_SHIFT: c_int = 9;
pub const ENC_HEVC_INTRA_PERIOD_SHIFT: c_int = 16;
// Decoder support fields

    (BIT(31) | ((index) << 12) | BIT(11) | REMAP_CTRL_MAX_SIZE_BITS)\
    )

pub const SEQ_BG_PARAM_REG_DATA: c_uint = 0x3800410;
...
#[no_mangle]
unsafe extern "C" fn wave5_wait_fio_readl(vpu_dev: *mut vpu_device, addr: u32, val: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wave5_fio_writel(vpu_dev: *mut vpu_device, addr: c_uint, data: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn wave5_wait_bus_busy(vpu_dev: *mut vpu_device, addr: c_uint) -> c_int {
    static int wave5_wait_bus_busy(struct vpu_device *vpu_dev, unsigned int addr)
    {
    let mut gdi_status_check_value: u32 = 0x3f;
...
#[no_mangle]
unsafe extern "C" fn wave5_wait_vpu_busy(vpu_dev: *mut vpu_device, addr: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wave5_wait_vcpu_bus_busy(vpu_dev: *mut vpu_device, addr: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_is_init(vpu_dev: *mut vpu_device) -> bool {
    bool wave5_vpu_is_init(struct vpu_device *vpu_dev)
    {
    return vpu_read_reg(vpu_dev, W5_VCPU_CUR_PC) != 0;
    }
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_get_product_id(vpu_dev: *mut vpu_device) -> c_uint {
    unsigned int wave5_vpu_get_product_id(struct vpu_device *vpu_dev)
    {
    let mut val: u32 = vpu_read_reg(vpu_dev, W5_PRODUCT_NUMBER);
    switch (val) {
    case WAVE515_CODE:
...
#[no_mangle]
unsafe extern "C" fn wave5_bit_issue_command(vpu_dev: *mut vpu_device, inst: *mut vpu_instance, cmd: u32) {
...
#[no_mangle]
unsafe extern "C" fn wave5_vpu_firmware_command_queue_error_check(dev: *mut vpu_device, fail_res: *mut u32) -> c_int {
...
//
    ...
    return wave5_vpu_firmware_command_queue_error_check(vpu_dev, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn setup_wave5_interrupts(vpu_dev: *mut vpu_device) {
...
#[no_mangle]
unsafe extern "C" fn setup_wave5_properties(dev: *mut device) -> c_int {
    ...
    ret = wave5_send_query(vpu_dev, core::ptr::null_mut(), GET_VPU_INFO);
    if (ret)
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_get_version(vpu_dev: *mut vpu_device, revision: *mut u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn remap_page(vpu_dev: *mut vpu_device, code_base: dma_addr_t, index: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_init(dev: *mut device, fw: *mut u8, size: usize) -> c_int {
...
// This register must be reset explicitly
    ...
    ret = send_firmware_command(inst, W5_CREATE_INSTANCE, true, core::ptr::null_mut(), core::ptr::null_mut());
    if (ret) {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_hw_flush_instance(inst: *mut vpu_instance) -> c_int {
    int wave5_vpu_hw_flush_instance(struct vpu_instance *inst)
    {
    struct dec_info *p_dec_info = &inst.codec_info.dec_info;
    u32 instance_queue_count, report_queue_count;
    let mut reg_val: u32 = 0;
    let mut fail_res: u32 = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn get_bitstream_options(info: *mut dec_info) -> u32 {
    static u32 get_bitstream_options(struct dec_info *info)
    {
    let mut bs_option: u32 = BSOPTION_ENABLE_EXPLICIT_END;
    if (info.stream_endflag)
    bs_option |= BSOPTION_HIGHLIGHT_STREAM_END;
    return bs_option;
    }
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_dec_init_seq(inst: *mut vpu_instance) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wave5_get_dec_seq_result(inst: *mut vpu_instance, info: *mut dec_initial_info) {
...
#[no_mangle]
pub unsafe extern "C" fn if(0x04: profile_compatibility_flag &) -> else {
    else if (profile_compatibility_flag & 0x04)
    info.profile = HEVC_PROFILE_MAIN10; /* main10 profile */
#[no_mangle]
pub unsafe extern "C" fn if(0x08: profile_compatibility_flag &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_dec_get_seq_info(inst: *mut vpu_instance, info: *mut dec_initial_info) -> c_int {
...
// this is not a fatal error, set ret to -EIO but don't return immediately
    ...
    let mut justified: bool = WTL_RIGHT_JUSTIFIED;
    let mut format_no: u32 = WTL_PIXEL_8BIT;
    let mut color_format: u32 = 0;
    let mut pixel_order: u32 = 1;
    let mut bwb_flag: u32 = (map_type == LINEAR_FRAME_MAP) ? 1 : 0;
...
// chroma FBC offset table
    ...
    remain -= i;
    ret = send_firmware_command(inst, W5_SET_FB, false, core::ptr::null_mut(), core::ptr::null_mut());
    if (ret)
    goto free_buffers;
    }
...
#[no_mangle]
unsafe extern "C" fn wave5_vpu_dec_validate_sec_axi(inst: *mut vpu_instance) -> u32 {
    static u32 wave5_vpu_dec_validate_sec_axi(struct vpu_instance *inst)
    {
    let mut bitdepth: u32 = inst.codec_info.dec_info.initial_info.luma_bitdepth;
    struct dec_info *p_dec_info = &inst.codec_info.dec_info;
    let mut bit_size: u32 = 0, ip_size = 0, lf_size = 0, ret = 0;
    let mut sram_size: u32 = inst.dev.sram_size;
    let mut width: u32 = inst.src_fmt.width;
    if (!sram_size)
    return 0;
//
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_decode(inst: *mut vpu_instance, fail_res: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_dec_get_result(inst: *mut vpu_instance, result: *mut dec_output_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(0x02: reg_val &) -> else {
    else if (reg_val & 0x02)
    result.pic_type = PIC_TYPE_P;
#[no_mangle]
pub unsafe extern "C" fn if(0x01: reg_val &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_re_init(dev: *mut device, fw: *mut u8, size: usize) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_reset(dev: *mut device, reset_mode: enum sw_reset_mode) -> c_int {
...
// VPU doesn't send response. force to set BUSY flag to 0.
    ...
    ret = wave5_vpu_sleep_wake(dev, true, core::ptr::null_mut(), 0);
    if (ret)
    return ret;
    }
...
// step3 : must clear GDI_BUS_CTRL after done SW_RESET
    ...
    ret = wave5_vpu_sleep_wake(dev, false, core::ptr::null_mut(), 0);
    return ret;
    }
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_dec_finish_seq(inst: *mut vpu_instance, fail_res: *mut u32) -> c_int {
    int wave5_vpu_dec_finish_seq(struct vpu_instance *inst, u32 *fail_res)
    {
    return send_firmware_command(inst, W5_DESTROY_INSTANCE, true, core::ptr::null_mut(), fail_res);
    }
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_dec_set_bitstream_flag(inst: *mut vpu_instance, eos: bool) -> c_int {
    ...
    vpu_write_reg(inst.dev, W5_BS_OPTION, get_bitstream_options(p_dec_info));
    vpu_write_reg(inst.dev, W5_BS_WR_PTR, p_dec_info.stream_wr_ptr);
    return send_firmware_command(inst, W5_UPDATE_BS, true, core::ptr::null_mut(), core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn wave5_dec_clr_disp_flag(inst: *mut vpu_instance, index: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_dec_set_disp_flag(inst: *mut vpu_instance, index: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_clear_interrupt(inst: *mut vpu_instance, flags: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_dec_get_rd_ptr(inst: *mut vpu_instance) -> dma_addr_t {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_dec_set_rd_ptr(inst: *mut vpu_instance, addr: dma_addr_t) -> c_int {
...
// This register must be reset explicitly
    ...
    {
    let mut aligned_width: c_int = (codec == W_HEVC_ENC) ? ALIGN(src_width, 32) : ALIGN(src_width, 16);
    let mut aligned_height: c_int = (codec == W_HEVC_ENC) ? ALIGN(src_height, 32) : ALIGN(src_height, 16);
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_enc_init_seq(inst: *mut vpu_instance) -> c_int {
    int wave5_vpu_enc_init_seq(struct vpu_instance *inst)
    {
    let mut reg_val: u32 = 0, rot_mir_mode, fixed_cu_size_mode = 0x7;
    struct enc_info *p_enc_info = &inst.codec_info.enc_info;
...
#[no_mangle]
pub unsafe extern "C" fn if(W_AVC_ENC: inst->std ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(W_HEVC_ENC: inst->std ==) -> else {
...
pub unsafe extern "C" fn if(W_HEVC_ENC: inst->std ==) -> else {
    ...
    return send_firmware_command(inst, W5_ENC_SET_PARAM, true, core::ptr::null_mut(), core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_enc_get_seq_info(inst: *mut vpu_instance, info: *mut enc_initial_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn calculate_luma_stride(width: u32, bit_depth: u32) -> u32 {
    static u32 calculate_luma_stride(u32 width, u32 bit_depth)
    {
    return ALIGN(ALIGN(width, 16) * ((bit_depth > 8) ? 5 : 4), 32);
    }
#[no_mangle]
unsafe extern "C" fn calculate_chroma_stride(width: u32, bit_depth: u32) -> u32 {
    ...
    let mut reg_val: u32 = 0, pic_size = 0, mv_col_size, fbc_y_tbl_size, fbc_c_tbl_size;
    let mut sub_sampled_size: u32 = 0;
    u32 luma_stride, chroma_stride;
    let mut buf_height: u32 = 0, buf_width = 0;
    u32 bit_depth;
    let mut avc_encoding: bool = (inst.std == W_AVC_ENC);
    let mut vb_mv: vpu_buf = {0};
    let mut vb_fbc_y_tbl: vpu_buf = {0};
    let mut vb_fbc_c_tbl: vpu_buf = {0};
    let mut vb_sub_sam_buf: vpu_buf = {0};
    let mut vb_task: vpu_buf = {0};
...
// chroma FBC offset table
    ...
    ret = wave5_vpu_firmware_command_queue_error_check(vpu_dev, core::ptr::null_mut());
    if (ret)
    goto free_vb_mem;
    return ret;
    free_vb_mem:
...
#[no_mangle]
unsafe extern "C" fn wave5_vpu_enc_validate_sec_axi(inst: *mut vpu_instance) -> u32 {
    static u32 wave5_vpu_enc_validate_sec_axi(struct vpu_instance *inst)
    {
    struct enc_info *p_enc_info = &inst.codec_info.enc_info;
    let mut rdo_size: u32 = 0, lf_size = 0, ret = 0;
    let mut sram_size: u32 = inst.dev.sram_size;
    if (!sram_size)
    return 0;
//
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_encode(inst: *mut vpu_instance, option: *mut enc_param, fail_res: *mut u32) -> c_int {
    int wave5_vpu_encode(struct vpu_instance *inst, struct enc_param *option, u32 *fail_res)
    {
    u32 src_frame_format;
    let mut reg_val: u32 = 0;
    let mut src_stride_c: u32 = 0;
    struct enc_info *p_enc_info = &inst.codec_info.enc_info;
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_enc_get_result(inst: *mut vpu_instance, result: *mut enc_output_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: result->recon_frame_index <) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_enc_finish_seq(inst: *mut vpu_instance, fail_res: *mut u32) -> c_int {
    ...
    struct enc_open_param *open_param)
    {
    let mut low_delay: bool = true;
    struct enc_wave_param *param = &open_param.wave_param;
    struct vpu_device *vpu_dev = inst.dev;
    struct device *dev = vpu_dev.dev;
    let mut num_ctu_row: u32 = (open_param.pic_height + 64 - 1) / 64;
    let mut num_ctu_col: u32 = (open_param.pic_width + 64 - 1) / 64;
    let mut ctu_sz: u32 = num_ctu_col * num_ctu_row;
    ...
    let mut num_ctb_in_width: c_uint = ALIGN(open_param.pic_width, 64) >> 6;
...
#[no_mangle]
pub unsafe extern "C" fn wave5_vpu_enc_check_open_param(inst: *mut vpu_instance, open_param: *mut enc_open_param) -> c_int {
    int wave5_vpu_enc_check_open_param(struct vpu_instance *inst, struct enc_open_param *open_param)
    {
    u32 pic_width;
    u32 pic_height;
    let mut product_id: i32 = inst.dev.product;
