...
//! Source: drivers/media/usb/gspca/vicam.c
#![no_std]
...
pub const HEADER_SIZE: c_int = 64;
...
#[no_mangle]
unsafe extern "C" fn vicam_set_camera_power(gspca_dev: *mut gspca_dev, state: c_int) -> c_int {
    static int vicam_set_camera_power(struct gspca_dev *gspca_dev, int state)
    {
    int ret;
    ret = vicam_control_msg(gspca_dev, 0x50, state, 0, core::ptr::null_mut(), 0);
    if (ret < 0)
    return ret;
    if (state)
    ret = vicam_control_msg(gspca_dev, 0x55, 1, 0, core::ptr::null_mut(), 0);
    return ret;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn vicam_read_frame(gspca_dev: *mut gspca_dev, data: *mut u8, size: c_int) -> c_int {
    static int vicam_read_frame(struct gspca_dev *gspca_dev, u8 *data, int size)
    {
    int ret, unscaled_height, act_len = 0;
    u8 *req_data = gspca_dev.usb_buf;
    let mut expo: i32 = v4l2_ctrl_g_ctrl(gspca_dev.exposure);
    let mut gain: i32 = v4l2_ctrl_g_ctrl(gspca_dev.gain);
...
//
#[no_mangle]
unsafe extern "C" fn vicam_dostream(work: *mut work_struct) {
...
unsafe extern "C" fn sd_init_controls(gspca_dev: *mut gspca_dev) -> c_int {
    ...
    gspca_dev.exposure = v4l2_ctrl_new_std(hdl, core::ptr::null_mut(),
    V4L2_CID_EXPOSURE, 0, 2047, 1, 256);
    gspca_dev.gain = v4l2_ctrl_new_std(hdl, core::ptr::null_mut(),
