...
//! Source: drivers/media/usb/gspca/kinect.c
#![no_std]
...
pub struct sd {
    ...
    pub /: *mut *mut uint16_t cam_tag; / a sequence number for packets,
    pub /: *mut *mut uint8_t stream_flag; / to identify different stream types,
    pub /: *mut *mut uint8_t obuf[0x400]; / output buffer for control commands,
    pub /: *mut *mut uint8_t ibuf[0x200]; / input buffer for control commands,
}

pub const MODE_640x480: c_uint = 0x0001;
pub const MODE_640x488: c_uint = 0x0002;
pub const MODE_1280x1024: c_uint = 0x0004;
pub const FORMAT_BAYER: c_uint = 0x0010;
pub const FORMAT_UYVY: c_uint = 0x0020;
pub const FORMAT_Y10B: c_uint = 0x0040;
pub const FPS_HIGH: c_uint = 0x0100;
...
#[no_mangle]
unsafe extern "C" fn kinect_read(udev: *mut usb_device, data: *mut u8, wLength: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sd_start_video(gspca_dev: *mut gspca_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sd_start_depth(gspca_dev: *mut gspca_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sd_stopN_video(gspca_dev: *mut gspca_dev) {
...
#[no_mangle]
unsafe extern "C" fn sd_stopN_depth(gspca_dev: *mut gspca_dev) {
...
#[no_mangle]
unsafe extern "C" fn sd_pkt_scan(gspca_dev: *mut gspca_dev, __data: *mut u8, len: c_int) {
    static void sd_pkt_scan(struct gspca_dev *gspca_dev, u8 *__data, int len)
    {
    struct sd *sd = (struct sd *) gspca_dev;
    struct pkt_hdr *hdr = (void *)__data;
    uint8_t *data = __data + sizeof(*hdr);
    let mut datalen: c_int = len - sizeof(*hdr);
    let mut sof: u8 = sd.stream_flag | 1;
    let mut mof: u8 = sd.stream_flag | 2;
    let mut eof: u8 = sd.stream_flag | 5;
    if (len < 12)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn if(mof: hdr->flag ==) -> else {
    else if (hdr.flag == mof)
    gspca_frame_add(gspca_dev, INTER_PACKET, data, datalen);
#[no_mangle]
pub unsafe extern "C" fn if(eof: hdr->flag ==) -> else {
