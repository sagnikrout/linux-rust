...
//! Source: drivers/media/usb/gspca/autogain_functions.c
#![no_std]
...
// If we are of a multiple of deadzone, do multiple steps to reach the
    ...
    gain--;
#[no_mangle]
pub unsafe extern "C" fn if(exposure_knee: exposure >) -> else {
    else if (exposure > exposure_knee)
    exposure--;
#[no_mangle]
pub unsafe extern "C" fn if(gspca_dev->gain->default_value: gain >) -> else {
    else if (gain > gspca_dev.gain.default_value)
    gain--;
#[no_mangle]
pub unsafe extern "C" fn if(gspca_dev->exposure->minimum: exposure >) -> else {
    else if (exposure > gspca_dev.exposure.minimum)
    exposure--;
#[no_mangle]
pub unsafe extern "C" fn if(gspca_dev->gain->minimum: gain >) -> else {
...
pub unsafe extern "C" fn if(exposure_knee: exposure <) -> else {
    ...
    exposure++;
#[no_mangle]
pub unsafe extern "C" fn if(gain_knee: gain <) -> else {
    else if (gain < gain_knee)
    gain++;
#[no_mangle]
pub unsafe extern "C" fn if(gspca_dev->exposure->maximum: exposure <) -> else {
    else if (exposure < gspca_dev.exposure.maximum)
    exposure++;
#[no_mangle]
pub unsafe extern "C" fn if(gspca_dev->gain->maximum: gain <) -> else {
