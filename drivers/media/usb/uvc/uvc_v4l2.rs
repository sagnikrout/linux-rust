...
//! Source: drivers/media/usb/uvc/uvc_v4l2.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn uvc_pm_get(dev: *mut uvc_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn uvc_pm_put(dev: *mut uvc_device) {
...
// Non standard control id.
    if (v4l2_ctrl_get_name(map.id) == core::ptr::null_mut()) {
    if (xmap.name[0] == '\0') {
    ret = -EINVAL;
    goto free_map;
    }
...
//
#[no_mangle]
unsafe extern "C" fn uvc_try_frame_interval(frame: *const uvc_frame, interval: u32) -> u32 {
    static u32 uvc_try_frame_interval(const struct uvc_frame *frame, u32 interval)
    {
    unsigned int i;
    if (frame.bFrameIntervalType) {
    let mut best: u32 = -1, dist;
    ...
    let mut min: u32 = frame.dwFrameInterval[0];
    let mut max: u32 = frame.dwFrameInterval[1];
    let mut step: u32 = frame.dwFrameInterval[2];
...
//
    ...
    let mut w: u16 = format.frames[i].wWidth;
    let mut h: u16 = format.frames[i].wHeight;
    d = min(w, rw) * min(h, rh);
    d = w*h + rw*rh - 2*d;
    if (d < maxd) {
    maxd = d;
...
//
    ...
    if (uvc_format != core::ptr::null_mut())
// uvc_format = format;
    if (uvc_frame != core::ptr::null_mut())
// uvc_frame = frame;
    return ret;
    }
...
//
#[no_mangle]
unsafe extern "C" fn uvc_v4l2_open(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uvc_v4l2_release(file: *mut file) -> c_int {
...
// Release the file handle.
    ...
    struct uvc_streaming *stream = handle.stream;
    struct uvc_streaming_control probe;
    return uvc_v4l2_try_format(stream, fmt, &probe, core::ptr::null_mut(), core::ptr::null_mut());
    }
    ...
    let mut index: u32 = input.index;
    if (selector == core::ptr::null_mut() ||
...
#[no_mangle]
unsafe extern "C" fn uvc_ioctl_g_input(file: *mut file, priv: *mut c_void, input: *mut c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uvc_ioctl_s_input(file: *mut file, priv: *mut c_void, input: c_uint) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct uvc_xu_control_mapping32 {
    pub id: u32,
    pub name: [u8; 32],
    pub entity: [u8; 16],
    pub selector: u8,
    ...
    kp.menu_info = count ? compat_ptr(info) : core::ptr::null_mut();
    kp.menu_count = count;
    return 0;
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct uvc_xu_control_query32 {
    pub unit: u8,
    pub selector: u8,
    pub query: u8,
    pub size: u16,
...
// kp = (struct uvc_xu_control_query){
    ...
    .query = v.query,
    .size = v.size,
    .data = v.size ? compat_ptr(v.data) : core::ptr::null_mut()
    };
    return 0;
    }
    ...
    {
    struct uvc_fh *handle = to_uvc_fh(file);
    let mut converted_cmd: c_uint = v4l2_translate_cmd(cmd);
    int ret;
