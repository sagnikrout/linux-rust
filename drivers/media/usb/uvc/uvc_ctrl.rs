...
//! Source: drivers/media/usb/uvc/uvc_ctrl.c
#![no_std]
...
pub const UVC_CTRL_DATA_CURRENT: c_int = 0;
pub const UVC_CTRL_DATA_BACKUP: c_int = 1;
pub const UVC_CTRL_DATA_MIN: c_int = 2;
pub const UVC_CTRL_DATA_MAX: c_int = 3;
pub const UVC_CTRL_DATA_RES: c_int = 4;
pub const UVC_CTRL_DATA_DEF: c_int = 5;
pub const UVC_CTRL_DATA_LAST: c_int = 6;
// ------------------------------------------------------------------------
// Controls
//
...
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_mapping_is_compound(mapping: *mut uvc_control_mapping) -> bool {
...
//
    ...
    void *v4l2_out)
    {
    let mut value: u8 = ((u8 *)uvc_in)[2];
    let mut sign: i8 = ((s8 *)uvc_in)[0];
    s32 *out = v4l2_out;
...
// out = value;
    ...
    {
    let mut first: c_uint = mapping.offset / 8;
    let mut value: u8 = ((u8 *)uvc_in)[first + 1];
    let mut sign: i8 = ((s8 *)uvc_in)[first];
    s32 *out = v4l2_out;
    if (WARN_ON(v4l2_size != sizeof(s32)))
    return -EINVAL;
    switch (query) {
...
#[no_mangle]
pub unsafe extern "C" fn uvc_test_bit(data: *const u8, bit: c_int) -> c_int {
    static inline int uvc_test_bit(const u8 *data, int bit)
    {
    return (data[bit >> 3] >> (bit & 7)) & 1;
    }
#[no_mangle]
pub unsafe extern "C" fn uvc_clear_bit(data: *mut u8, bit: c_int) {
    static inline void uvc_clear_bit(u8 *data, int bit)
    {
    data[bit >> 3] &= ~(1 << (bit & 7));
    }
#[no_mangle]
unsafe extern "C" fn uvc_menu_to_v4l2_menu(mapping: *mut uvc_control_mapping, val: i32) -> i32 {
...
//
    ...
    void *v4l2_out)
    {
    let mut offset: c_int = mapping.offset;
    let mut bits: c_int = mapping.size;
...
// mapping = map;
    ...
    if ((*mapping == core::ptr::null_mut() || (*mapping).id > map.id) &&
    (map.id > v4l2_id) &&
...
// mapping = map;
    ...
    {
    struct uvc_control *ctrl = core::ptr::null_mut();
    struct uvc_entity *entity;
    let mut next: c_int = v4l2_id & V4L2_CTRL_FLAG_NEXT_CTRL;
    let mut next_compound: c_int = v4l2_id & V4L2_CTRL_FLAG_NEXT_COMPOUND;
// mapping = NULL;
...
//
    ...
    unsigned long ioctl)
    {
    struct uvc_control_mapping *master_map = core::ptr::null_mut();
    struct uvc_control *master_ctrl = core::ptr::null_mut();
    struct uvc_control_mapping *mapping;
    struct uvc_control *ctrl;
    s32 val;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_is_relative_ptz(ctrl_id: __u32) -> bool {
...
//
pub const MAX_QUERY_RETRIES: c_int = 2;
...
#[no_mangle]
unsafe extern "C" fn uvc_mapping_v4l2_size(mapping: *mut uvc_control_mapping) -> usize {
...
//
    ...
    struct uvc_control_mapping *mapping;
    struct uvc_control *ctrl;
    let mut index: u32 = query_menu.index;
    let mut id: u32 = query_menu.id;
    const char *name;
    int ret;
    ...
    if (ctrl == core::ptr::null_mut() || mapping.v4l2_type != V4L2_CTRL_TYPE_MENU) {
    ret = -EINVAL;
    goto done;
    }
...
//
    ...
    {
    struct v4l2_fh *originator = handle ? &handle.vfh : core::ptr::null_mut();
...
//
    ...
    {
    struct uvc_control_mapping *mapping = core::ptr::null_mut();
    struct uvc_control *ctrl = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_set_handle(ctrl: *mut uvc_control, handle: *mut uvc_fh) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_clear_handle(ctrl: *mut uvc_control) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_status_event_work(work: *mut work_struct) {
...
// Resubmit the URB.
    ...
    unsigned int j;
    for (i = 0; i < xctrls_count; ++i) {
    let mut changes: u32 = V4L2_EVENT_CTRL_CH_VALUE;
    s32 value;
...
// Notification will be sent from an Interrupt event.
    ...
    let mut slave_id: u32 = mapping.slave_ids[j];
    if (!slave_id)
    break;
//
...
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_add_event(sev: *mut v4l2_subscribed_event, elems: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_del_event(sev: *mut v4l2_subscribed_event) {
...
//
#[no_mangle]
pub unsafe extern "C" fn uvc_ctrl_begin(chain: *mut uvc_video_chain) -> c_int {
...
//
    ...
    {
    let mut processed_ctrls: c_uint = 0;
    struct uvc_control *ctrl;
    unsigned int i;
    let mut ret: c_int = 0;
    if (entity == core::ptr::null_mut())
...
//
    ...
    {
    struct uvc_control_mapping *mapping = core::ptr::null_mut();
    struct uvc_control *ctrl_found = core::ptr::null_mut();
    unsigned int i;
    if (!entity)
    return ctrls.count;
...
//
    ...
    {
    u8 *data __free(kfree) = core::ptr::null_mut();
    size_t size;
    u8 query;
    int ret;
    int id;
...
//
    ...
    s32 *value_in_out)
    {
    let mut value: i32 = *value_in_out;
    u32 step;
    s32 min;
    s32 max;
    int ret;
...
//
    if (mapping.data_type == UVC_CTRL_DATA_TYPE_BITMASK) {
    let mut val: c_int = uvc_mapping_get_menu_value(mapping, value);
    if (!ctrl.cached) {
    ret = uvc_ctrl_populate_cache(chain, ctrl);
    if (ret < 0)
    return ret;
    ...
    {
    u8 *data __free(kfree) = core::ptr::null_mut();
    let mut size: usize = uvc_mapping_v4l2_size(mapping);
    if (xctrl.size != size)
    return -EINVAL;
//
// v4l2_ext_control does not have enough room to fit a compound control.
...
#[no_mangle]
pub unsafe extern "C" fn uvc_ctrl_set(handle: *mut uvc_fh, xctrl: *mut v4l2_ext_control) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn uvc_ctrl_is_privacy_control(entity[16]: u8, selector: u8) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn uvc_ctrl_restore_values(dev: *mut uvc_device) -> c_int {
...
// Walk the entities list and restore controls when possible.
    ...
    ret = uvc_ctrl_commit_entity(dev, core::ptr::null_mut(), entity, 0, core::ptr::null_mut());
    if (ret < 0)
    return ret;
    }
    return 0;
...
//
    ...
    if (!map)
    return -ENOMEM;
    map.name = core::ptr::null_mut();
    map.menu_names = core::ptr::null_mut();
    map.menu_mapping = core::ptr::null_mut();
...
// fls(mapping->menu_mask);
    ...
    if (map.get == core::ptr::null_mut())
    map.get = uvc_get_le_value;
    if (map.set == core::ptr::null_mut())
...
//
#[no_mangle]
unsafe extern "C" fn uvc_ctrl_init_chain(chain: *mut uvc_video_chain) -> c_int {
...
// Walk the entities list and instantiate controls
    list_for_each_entry(entity, &chain.entities, chain) {
    struct uvc_control *ctrl;
    let mut bControlSize: c_uint = 0, ncontrols;
    u8 *bmControls = core::ptr::null_mut();
    if (UVC_ENTITY_TYPE(entity) == UVC_VC_EXTENSION_UNIT) {
...
// Count supported controls and allocate the controls array
    ...
    if (entity.controls == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
pub unsafe extern "C" fn uvc_ctrl_init_device(dev: *mut uvc_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn uvc_ctrl_cleanup_fh(handle: *mut uvc_fh) {
...
#[no_mangle]
pub unsafe extern "C" fn uvc_ctrl_cleanup_device(dev: *mut uvc_device) {
