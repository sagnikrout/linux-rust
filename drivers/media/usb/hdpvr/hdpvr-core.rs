...
//! Source: drivers/media/usb/hdpvr/hdpvr-core.c
#![no_std]
...
//
    ...
    module_param_array(video_nr, int, core::ptr::null_mut(), 0);
    MODULE_PARM_DESC(video_nr, "video device number (-1=Auto)");
// holds the number of currently registered devices
    let mut dev_nr: static atomic_t = ATOMIC_INIT(-1);
    int hdpvr_debug;
    module_param(hdpvr_debug, int, S_IRUGO|S_IWUSR);
    MODULE_PARM_DESC(hdpvr_debug, "enable debugging output");
    let mut default_video_input: static uint = HDPVR_VIDEO_INPUTS;
    module_param(default_video_input, uint, S_IRUGO|S_IWUSR);
    MODULE_PARM_DESC(default_video_input, "default video input: 0=Component / 1=S-Video / 2=Composite");
    let mut default_audio_input: static uint = HDPVR_AUDIO_INPUTS;
...
#[no_mangle]
pub unsafe extern "C" fn hdpvr_delete(dev: *mut hdpvr_device) {
...
#[no_mangle]
unsafe extern "C" fn challenge(bytes: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn device_authorization(dev: *mut hdpvr_device) -> c_int {
    static int device_authorization(struct hdpvr_device *dev)
    {
    int ret, retval = -ENOMEM;
    let mut request_type: c_char = 0x38, rcv_request = 0x81;
    char *response;
...
#[no_mangle]
unsafe extern "C" fn hdpvr_device_init(dev: *mut hdpvr_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hdpvr_disconnect(interface: *mut usb_interface) {
