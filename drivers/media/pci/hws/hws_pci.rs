...
//! Source: drivers/media/pci/hws/hws_pci.c
#![no_std]
...
pub const HWS_BUSY_POLL_DELAY_US: c_int = 10;
pub const HWS_BUSY_POLL_TIMEOUT_US: c_int = 1000000;
#[no_mangle]
unsafe extern "C" fn hws_elapsed_us(start_ns: u64) -> c_ulonglong {
...
// HWS family, SKU unknown.
    MAKE_ENTRY(0x8888, 0x9534, 0x8888, 0x0007, core::ptr::null_mut()),
    MAKE_ENTRY(0x1F33, 0x8534, 0x8888, 0x0007, core::ptr::null_mut()),
    MAKE_ENTRY(0x1F33, 0x8554, 0x8888, 0x0007, core::ptr::null_mut()),
// HWS 2x2 HDMI family.
    MAKE_ENTRY(0x8888, 0x8524, 0x8888, 0x0007, core::ptr::null_mut()),
// HWS 2x2 SDI family.
    MAKE_ENTRY(0x1F33, 0x6524, 0x8888, 0x0007, core::ptr::null_mut()),
// HWS X4 HDMI family.
    MAKE_ENTRY(0x8888, 0x8504, 0x8888, 0x0007, core::ptr::null_mut()),
// HWS X4 SDI family.
    MAKE_ENTRY(0x8888, 0x6504, 0x8888, 0x0007, core::ptr::null_mut()),
// HWS family, SKU unknown.
    MAKE_ENTRY(0x8888, 0x8532, 0x8888, 0x0007, core::ptr::null_mut()),
    MAKE_ENTRY(0x8888, 0x8512, 0x8888, 0x0007, core::ptr::null_mut()),
    MAKE_ENTRY(0x8888, 0x8501, 0x8888, 0x0007, core::ptr::null_mut()),
    MAKE_ENTRY(0x1F33, 0x6502, 0x8888, 0x0007, core::ptr::null_mut()),
// HWS X4 HDMI family (alternate vendor ID).
    MAKE_ENTRY(0x1F33, 0x8504, 0x8888, 0x0007, core::ptr::null_mut()),
// HWS 2x2 HDMI family (alternate vendor ID).
    MAKE_ENTRY(0x1F33, 0x8524, 0x8888, 0x0007, core::ptr::null_mut()),
    {}
    };
#[no_mangle]
unsafe extern "C" fn enable_pcie_relaxed_ordering(dev: *mut pci_dev) {
    static void enable_pcie_relaxed_ordering(struct pci_dev *dev)
    {
    pcie_capability_set_word(dev, PCI_EXP_DEVCTL, PCI_EXP_DEVCTL_RELAX_EN);
    }
#[no_mangle]
unsafe extern "C" fn hws_configure_hardware_capabilities(hdev: *mut hws_pcie_dev) {
    static void hws_configure_hardware_capabilities(struct hws_pcie_dev *hdev)
    {
    let mut id: u16 = hdev.device_id;
...
// decide hardware-version and program DMA max size if needed
    ...
    hdev.hw_ver = 1;
    let mut dma_max: u32 = (u32)(MAX_VIDEO_SCALER_SIZE / 16);
...
#[no_mangle]
unsafe extern "C" fn read_chip_id(hdev: *mut hws_pcie_dev) -> c_int {
...
// mirror PCI IDs for later switches
    ...
    hdev.main_task = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn main_ks_thread_handle(data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hws_stop_kthread_action(data: *mut c_void) {
    ...
    t.comm, t.pid);
    WRITE_ONCE(hws.main_task, core::ptr::null_mut());
    kthread_stop(t);
    dev_dbg(&hws.pdev.dev,
...
#[no_mangle]
unsafe extern "C" fn hws_alloc_seed_buffers(hws: *mut hws_pcie_dev) -> c_int {
...
// 64 KiB is plenty for a safe dummy; hardware needs 64-byte alignment.
    let mut need: usize = ALIGN(64 * 1024, 64);
...
// not fatal: free earlier ones and continue without seeding
    ...
    hws.scratch_vid[ch].dma);
    hws.scratch_vid[ch].cpu = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn hws_free_seed_buffers(hws: *mut hws_pcie_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_seed_channel(hws: *mut hws_pcie_dev, ch: c_int) {
    static void hws_seed_channel(struct hws_pcie_dev *hws, int ch)
    {
    let mut paddr: dma_addr_t = hws.scratch_vid[ch].dma;
    let mut lo: u32 = lower_32_bits(paddr);
    let mut hi: u32 = upper_32_bits(paddr);
    let mut pci_addr: u32 = lo & PCI_E_BAR_ADD_LOWMASK;
...
#[no_mangle]
unsafe extern "C" fn hws_seed_all_channels(hws: *mut hws_pcie_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_irq_mask_gate(hws: *mut hws_pcie_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_irq_unmask_gate(hws: *mut hws_pcie_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_irq_clear_pending(hws: *mut hws_pcie_dev) {
    static void hws_irq_clear_pending(struct hws_pcie_dev *hws)
    {
    let mut st: u32 = readl(hws.bar0_base + HWS_REG_INT_STATUS);
    if (st) {
...
#[no_mangle]
unsafe extern "C" fn hws_block_hotpaths(hws: *mut hws_pcie_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_probe(pdev: *mut pci_dev, pci_id: *const pci_device_id) -> c_int {
    static int hws_probe(struct pci_dev *pdev, const struct pci_device_id *pci_id)
    {
    struct hws_pcie_dev *hws;
    int i, ret, irq;
    let mut irqf: c_ulong = 0;
    let mut v4l2_registered: bool = false;
...
// E) Set the global interrupt enable bit in main control register
    {
    let mut ctl_reg: u32 = readl(hws.bar0_base + HWS_REG_CTL);
...
// 12) Background monitor thread (managed)
    ...
    hws.main_task = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn hws_check_busy(pdx: *mut hws_pcie_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hws_stop_dsp(hws: *mut hws_pcie_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_publish_stop_flags(hws: *mut hws_pcie_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_drain_after_stop(hws: *mut hws_pcie_dev) {
    static void hws_drain_after_stop(struct hws_pcie_dev *hws)
    {
    let mut ackmask: u32 = 0;
    unsigned int i;
    let mut start_ns: u64 = ktime_get_mono_fast_ns();
...
#[no_mangle]
unsafe extern "C" fn hws_stop_device(hws: *mut hws_pcie_dev) {
    static void hws_stop_device(struct hws_pcie_dev *hws)
    {
    let mut status: u32 = readl(hws.bar0_base + HWS_REG_SYS_STATUS);
    let mut start_ns: u64 = ktime_get_mono_fast_ns();
    let mut live: bool = status != 0xFFFFFFFF;
    dev_dbg(&hws.pdev.dev, "%s: status=0x%08x\n", __func__, status);
    if (!live) {
    hws.pci_lost = true;
    goto out;
...
#[no_mangle]
unsafe extern "C" fn hws_remove(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn hws_pm_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hws_pm_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hws_shutdown(pdev: *mut pci_dev) {
    static void hws_shutdown(struct pci_dev *pdev)
    {
    struct hws_pcie_dev *hws = pci_get_drvdata(pdev);
    let mut vret: c_int = 0;
    let mut start_ns: u64 = ktime_get_mono_fast_ns();
    u64 step_ns;
    if (!hws)
    return;
...
#[no_mangle]
unsafe extern "C" fn pcie_hws_init() -> int __init {
    static int __init pcie_hws_init(void)
    {
    return pci_register_driver(&hws_pci_driver);
    }
#[no_mangle]
unsafe extern "C" fn pcie_hws_exit() -> void __exit {
