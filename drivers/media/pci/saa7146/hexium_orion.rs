...
//! Source: drivers/media/pci/saa7146/hexium_orion.c
#![no_std]
...
pub const HEXIUM_HV_PCI6_ORION: c_int = 1;
pub const HEXIUM_ORION_1SVHS_3BNC: c_int = 2;
pub const HEXIUM_ORION_4BNC: c_int = 3;
...
// this is only called for old HV-PCI6/Orion cards
    without eeprom */
#[no_mangle]
unsafe extern "C" fn hexium_probe(dev: *mut saa7146_dev) -> c_int {
    static int hexium_probe(struct saa7146_dev *dev)
    {
    struct hexium *hexium = core::ptr::null_mut();
...
unsafe extern "C" fn hexium_set_input(hexium: *mut hexium, input: c_int) -> c_int {
    ...
    DEB_D("\n");
    for (i = 0; i < 8; i++) {
    let mut adr: c_int = hexium_input_select[input].data[i].adr;
