...
//! Source: drivers/media/pci/solo6x10/solo6x10-core.c
#![no_std]
...
//
    ...
    MODULE_LICENSE("GPL");
    let mut video_nr: static unsigned = -1;
...
#[no_mangle]
unsafe extern "C" fn solo_set_time(solo_dev: *mut solo_dev) {
...
#[no_mangle]
unsafe extern "C" fn solo_timer_sync(solo_dev: *mut solo_dev) {
    ...
    let mut usec_lsb: c_long = solo_dev.usec_lsb;
    usec_lsb -= diff / 4;
    if (usec_lsb < 0)
    usec_lsb = 0;
#[no_mangle]
pub unsafe extern "C" fn if(255: usec_lsb >) -> else {
...
#[no_mangle]
unsafe extern "C" fn solo_isr(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn free_solo_dev(solo_dev: *mut solo_dev) {
...
// Now cleanup the PCI device
    ...
    container_of(dev, struct solo_dev, dev);
    u16 *p = (u16 *)buf;
    let mut count: c_int = (full_eeprom ? 128 : 64);
    int i;
    ...
    container_of(dev, struct solo_dev, dev);
    unsigned long ms;
    let mut ret: c_int = kstrtoul(buf, 10, &ms);
    if (ret < 0 || ms > 200)
    return -EINVAL;
    ...
    container_of(dev, struct solo_dev, dev);
    char *out = buf;
    let mut fps: c_int = solo_dev.fps;
    int i;
    ...
    struct solo_dev *solo_dev =
    container_of(dev, struct solo_dev, dev);
    let mut size: c_int = solo_dev.sdram_size;
    if (off >= size)
    return 0;
    if (off + count > size)
    count = size - off;
...
#[no_mangle]
unsafe extern "C" fn solo_device_release(dev: *mut device) {
    static void solo_device_release(struct device *dev)
    {
// Do nothing
    }
#[no_mangle]
unsafe extern "C" fn solo_sysfs_init(solo_dev: *mut solo_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn solo_pci_probe(pdev: *mut pci_dev, id: *const pci_device_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn solo_pci_remove(pdev: *mut pci_dev) {
