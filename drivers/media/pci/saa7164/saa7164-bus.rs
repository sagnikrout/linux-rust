...
//! Source: drivers/media/pci/saa7164/saa7164-bus.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn saa7164_bus_setup(dev: *mut saa7164_dev) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn saa7164_bus_dump(dev: *mut saa7164_dev) {
...
#[no_mangle]
unsafe extern "C" fn saa7164_bus_verify(dev: *mut saa7164_dev) {
...
//
    ...
    let mut ret: c_int = SAA_ERR_BAD_PARAMETER;
    u16 size;
    if (!msg) {
    ...
    if ((msg.size > 0) && (buf == core::ptr::null_mut())) {
...
// Convert to le16/le32
    msg.size = ( u16)cpu_to_le16(msg.size);
    msg.command = ( u32)cpu_to_le32(msg.command);
    msg.controlselector = ( u16)cpu_to_le16(msg.controlselector);
...
// Convert back to cpu after writing the msg to the ringbuffer.
    msg.size = le16_to_cpu(( __le16)msg.size);
    msg.command = le32_to_cpu(( __le32)msg.command);
    msg.controlselector = le16_to_cpu(( __le16)msg.controlselector);
    ret = SAA_OK;
    out:
    saa7164_bus_dump(dev);
...
//
    ...
    if ((peekonly == 0) && (msg.size > 0) && (buf == core::ptr::null_mut())) {
    printk(KERN_ERR
...
// Convert from little endian to CPU
    msg_tmp.size = le16_to_cpu(( __le16)msg_tmp.size);
    msg_tmp.command = le32_to_cpu(( __le32)msg_tmp.command);
    msg_tmp.controlselector = le16_to_cpu(( __le16)msg_tmp.controlselector);
...
// Check if the command/response matches what is expected
    ...
    saa7164_bus_dumpmsg(dev, &msg_tmp, core::ptr::null_mut());
    ret = SAA_ERR_INVALID_COMMAND;
    goto out;
    }
