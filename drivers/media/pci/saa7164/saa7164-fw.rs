...
//! Source: drivers/media/pci/saa7164/saa7164-fw.c
#![no_std]
...
pub const SAA7164_REV2_FIRMWARE_SIZE: c_int = 4038864;

pub const SAA7164_REV3_FIRMWARE_SIZE: c_int = 4019072;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fw_header {
    pub firmwaresize: u32,
    pub bslsize: u32,
    pub reserved: u32,
    pub version: u32,
}
...
#[no_mangle]
unsafe extern "C" fn saa7164_dl_wait_ack(dev: *mut saa7164_dev, reg: u32) -> c_int {
    static int saa7164_dl_wait_ack(struct saa7164_dev *dev, u32 reg)
    {
    let mut timeout: u32 = SAA_DEVICE_TIMEOUT;
...
#[no_mangle]
unsafe extern "C" fn saa7164_dl_wait_clr(dev: *mut saa7164_dev, reg: u32) -> c_int {
...
// TODO: Excessive levels of debug
    ...
    u8 *srcbuf = core::ptr::null_mut();
    int ret;
    let mut dlflag: u32 = dlflags;
    let mut dlflag_ack: u32 = dlflag + 4;
    let mut drflag: u32 = dlflag_ack + 4;
    let mut drflag_ack: u32 = drflag + 4;
    let mut bleflag: u32 = drflag_ack + 4;
    dprintk(DBGLVL_FW,
    "%s(image=%p, size=%d, flags=0x%x, dst=%p, dstsize=0x%x)\n",
    __func__, src, srcsize, dlflags, dst, dstsize);
    if ((src == core::ptr::null_mut()) || (dst == core::ptr::null_mut())) {
    ret = -EIO;
    goto out;
    }
    srcbuf = kzalloc(4 * 1048576, GFP_KERNEL);
    if (core::ptr::null_mut() == srcbuf) {
    ret = -ENOMEM;
    goto out;
    }
...
#[no_mangle]
pub unsafe extern "C" fn saa7164_downloadfirmware(dev: *mut saa7164_dev) -> c_int {
...
// u32 second_timeout = 60 * SAA_DEVICE_TIMEOUT;
    ...
    struct fw_header *hdr, *boothdr = core::ptr::null_mut(), *fwhdr;
    let mut bootloaderversion: u32 = 0, fwloadersize;
    u8 *bootloaderoffset = core::ptr::null_mut(), *fwloaderoffset;
    char *fwname;
    int ret;
    dprintk(DBGLVL_FW, "%s()\n", __func__);
