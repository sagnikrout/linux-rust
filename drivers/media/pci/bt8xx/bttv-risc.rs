...
//! Source: drivers/media/pci/bt8xx/bttv-risc.c
#![no_std]
...
pub const VCR_HACK_LINES: c_int = 4;
// ----------------------------------------------------------
...
// save pointer to jmp instruction address
    ...
    let mut topfield: c_int = (0 == yoffset);
    int rc;
// estimate risc mem: worst case is one write per page border +
#[no_mangle]
pub unsafe extern "C" fn line(dwords: 5) -> one write per scan {
...
// ----------------------------------------------------------
    ...
    u32 xsf, sr;
    int vdelay;
    let mut swidth: c_int = tvnorm.swidth;
    let mut totalwidth: c_int = tvnorm.totalwidth;
    let mut scaledtwidth: c_int = tvnorm.scaledtwidth;
...
// Even to store Cb first, odd for Cr.
    ...
    {
    let mut off: c_int = odd ? 0x80 : 0x00;
    if (geo.comb)
...
#[no_mangle]
unsafe extern "C" fn bttv_set_risc_status(btv: *mut bttv) {
    static void bttv_set_risc_status(struct bttv *btv)
    {
    let mut cmd: c_ulong = BT848_RISC_JUMP;
    if (btv.loop_irq) {
    cmd |= BT848_RISC_IRQ;
...
#[no_mangle]
unsafe extern "C" fn bttv_set_irq_timer(btv: *mut bttv) {
...
#[no_mangle]
unsafe extern "C" fn bttv_set_capture_control(btv: *mut bttv, start_capture: c_int) -> c_int {
    static int bttv_set_capture_control(struct bttv *btv, int start_capture)
    {
    let mut capctl: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn bttv_start_dma(btv: *mut bttv) {
...
#[no_mangle]
unsafe extern "C" fn bttv_stop_dma(btv: *mut bttv) {
...
#[no_mangle]
pub unsafe extern "C" fn bttv_set_dma(btv: *mut bttv, start_capture: c_int) {
...
// jump back to top field
    ...
    {
    unsigned long cmd;
    let mut next: c_ulong = btv.main.dma + ((slot+2) << 2);
    if (core::ptr::null_mut() == risc) {
    d2printk("%d: risc=%p slot[%d]=core::ptr::null_mut()\n", btv.c.nr, risc, slot);
...
#[no_mangle]
pub unsafe extern "C" fn bttv_buffer_risc_vbi(btv: *mut bttv, buf: *mut bttv_buffer) -> c_int {
    int bttv_buffer_risc_vbi(struct bttv *btv, struct bttv_buffer *buf)
    {
    let mut r: c_int = 0;
    unsigned int offset;
    unsigned int bpl = 2044; /* max. vbipack */
    let mut padding: c_uint = VBI_BPL - bpl;
    let mut skip_lines0: c_uint = 0;
    let mut skip_lines1: c_uint = 0;
    let mut min_vdelay: c_uint = MIN_VDELAY;
...
// VDELAY is start of video, end of VBI capturing.
    ...
    {
// video capture
    if (core::ptr::null_mut() != set.top  &&  core::ptr::null_mut() != set.bottom) {
    ...
    } else if (core::ptr::null_mut() != set.top) {
    if (set.top.list.next)
    list_del(&set.top.list);
    bttv_apply_geo(btv, &set.top.geo,1);
    ...
    bttv_risc_hook(btv, RISC_SLOT_O_FIELD, &set.top.top,
    set.frame_irq);
    bttv_risc_hook(btv, RISC_SLOT_E_FIELD, core::ptr::null_mut(),           0);
    btaor(set.top.btformat & 0xff, ~0xff, BT848_COLOR_FMT);
    btaor(set.top.btswap & 0x0f,   ~0x0f, BT848_COLOR_CTL);
    } else if (core::ptr::null_mut() != set.bottom) {
    if (set.bottom.list.next)
    list_del(&set.bottom.list);
...
// calculate geometry, build risc code
    ...
    struct sg_table *sgt = vb2_dma_sg_plane_desc(&buf.vbuf.vb2_buf, 0);
    struct scatterlist *list = sgt.sgl;
    let mut size: c_ulong = (btv.fmt.depth * btv.width * btv.height) >> 3;
// packed pixel modes
    if (btv.fmt.flags & FORMAT_FLAGS_PACKED) {
    let mut bpl: c_int = (btv.fmt.depth >> 3) * btv.width;
    let mut bpf: c_int = bpl * (btv.height >> 1);
