...
//! Source: drivers/media/pci/b2c2/flexcop-dma.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn flexcop_dma_free(dma: *mut flexcop_dma) {
...
// start the DMA transfers, but not the DMA IRQs
    ...
    int onoff)
    {
    let mut r: flexcop_ibi_register = (dma_idx & FC_DMA_1) ? dma1_00c : dma2_01c;
    let mut v: flexcop_ibi_value = fc.read_ibi_reg(fc, r);
    deb_info("%s\n", __func__);
    ...
    int onoff)
    {
    let mut v: flexcop_ibi_value = fc.read_ibi_reg(fc, ctrl_208);
    if (no & FC_DMA_1)
...
// 1 cycles = 1.97 msec
    ...
    {
    let mut r: flexcop_ibi_register = (dma_idx & FC_DMA_1) ? dma1_004 : dma2_014;
    let mut v: flexcop_ibi_value = fc.read_ibi_reg(fc, r);
