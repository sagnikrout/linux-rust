...
//! Source: drivers/media/pci/tw5864/tw5864-h264.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct bs {
    pub /: *mut *mut *mut u8 buf; / pointer to buffer beginning,
    pub /: *mut *mut *mut u8 buf_end; / pointer to buffer end,
    pub /: *mut *mut *mut u8 ptr; / pointer to current byte in buffer,
    pub /: *mut *mut unsigned int bits_left; / number of available bits in current byte,
}

#[no_mangle]
unsafe extern "C" fn bs_init(s: *mut bs, buf: *mut c_void, size: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bs_len(s: *mut bs) -> c_int {
    static int bs_len(struct bs *s)
    {
    return s.ptr - s.buf;
    }
#[no_mangle]
unsafe extern "C" fn bs_write(s: *mut bs, count: c_int, bits: u32) {
...
#[no_mangle]
unsafe extern "C" fn bs_write1(s: *mut bs, bit: u32) {
...
#[no_mangle]
unsafe extern "C" fn bs_write_ue(s: *mut bs, val: u32) {
...
#[no_mangle]
unsafe extern "C" fn bs_write_se(s: *mut bs, val: c_int) {
    static void bs_write_se(struct bs *s, int val)
    {
    bs_write_ue(s, val <= 0 ? -val * 2 : val * 2 - 1);
    }
#[no_mangle]
unsafe extern "C" fn bs_rbsp_trailing(s: *mut bs) {
...
#[no_mangle]
unsafe extern "C" fn tw5864_h264_gen_sps_rbsp(buf: *mut u8, size: usize, width: c_int, height: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_h264_gen_pps_rbsp(buf: *mut u8, size: usize, qp: c_int) -> c_int {
    ...
    {
    struct bs bs, *s;
    let mut is_i_frame: c_int = frame_gop_seqno == 0;
    s = &bs;
    bs_init(s, buf, size);
