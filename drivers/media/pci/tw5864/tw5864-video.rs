...
//! Source: drivers/media/pci/tw5864/tw5864-video.c
#![no_std]
...
pub const QUANTIZATION_TABLE_LEN: c_int = 96;
pub const VLC_LOOKUP_TABLE_LEN: c_int = 1024;
...
#[no_mangle]
unsafe extern "C" fn tw5864_buf_queue(vb: *mut vb2_buffer) {
    ...
    let mut std_reg: u8 = tw_indir_readb(TW5864_INDIR_VIN_E(input.nr));
// std = (std_reg & 0x70) >> 4;
    if (std_reg & 0x80) {
    dev_dbg(&dev.pci.dev,
...
#[no_mangle]
unsafe extern "C" fn tw5864_enable_input(input: *mut tw5864_input) -> c_int {
    static int tw5864_enable_input(struct tw5864_input *input)
    {
    struct tw5864_dev *dev = input.root;
    let mut nr: c_int = input.nr;
    unsigned long flags;
    let mut d1_width: c_int = 720;
    int d1_height;
    let mut frame_width_bus_value: c_int = 0;
    let mut frame_height_bus_value: c_int = 0;
    let mut reg_frame_bus: c_int = 0x1c;
    let mut fmt_reg_value: c_int = 0;
    let mut downscale_enabled: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn tw5864_request_encoded_frame(input: *mut tw5864_input) {
...
#[no_mangle]
unsafe extern "C" fn tw5864_disable_input(input: *mut tw5864_input) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_start_streaming(q: *mut vb2_queue, count: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_stop_streaming(q: *mut vb2_queue) {
...
#[no_mangle]
unsafe extern "C" fn tw5864_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
// input->md_threshold_grid_ctrl->p_new.p_u16 contains data
    ...
    struct tw5864_input *input = video_drvdata(file);
    struct tw5864_dev *dev = input.root;
    let mut indir_0x000: u8 = tw_indir_readb(TW5864_INDIR_VIN_0(input.nr));
    let mut indir_0x00d: u8 = tw_indir_readb(TW5864_INDIR_VIN_D(input.nr));
    let mut v1: u8 = indir_0x000;
    let mut v2: u8 = indir_0x00d;
    if (i.index)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn tw5864_g_input(file: *mut file, priv: *mut c_void, i: *mut c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_s_input(file: *mut file, priv: *mut c_void, i: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_querystd(file: *mut file, priv: *mut c_void, std: *mut v4l2_std_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_g_std(file: *mut file, priv: *mut c_void, std: *mut v4l2_std_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_s_std(file: *mut file, priv: *mut c_void, std: v4l2_std_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tw5864_frame_interval_set(input: *mut tw5864_input) {
    static void tw5864_frame_interval_set(struct tw5864_input *input)
    {
//
...
//
    struct tw5864_dev *dev = input.root;
    let mut unary_framerate: u32 = 0;
    let mut shift: c_int = 0;
    let mut std_max_fps: c_int = input.std == STD_NTSC ? 30 : 25;
...
pub const INDIR_SPACE_MAP_SHIFT: c_uint = 0x100000;
    ...
    let mut indir_addr: __u64 = reg.reg - INDIR_SPACE_MAP_SHIFT;
    if (indir_addr > 0xefe)
    return -EINVAL;
    reg.size = 1;
...
#[no_mangle]
pub unsafe extern "C" fn tw5864_video_init(dev: *mut tw5864_dev, video_nr: *mut c_int) -> c_int {
    int tw5864_video_init(struct tw5864_dev *dev, int *video_nr)
    {
    int i;
    int ret;
    unsigned long flags;
    let mut last_dma_allocated: c_int = -1;
    let mut last_input_nr_registered: c_int = -1;
...
#[no_mangle]
unsafe extern "C" fn tw5864_video_input_init(input: *mut tw5864_input, video_nr: c_int) -> c_int {
...
// Initialize the device control structures
    ...
    v4l2_ctrl_new_custom(hdl, &tw5864_md_thresholds, core::ptr::null_mut());
    if (hdl.error) {
    ret = hdl.error;
    goto free_v4l2_hdl;
    }
...
#[no_mangle]
unsafe extern "C" fn tw5864_video_input_fini(dev: *mut tw5864_input) {
    static void tw5864_video_input_fini(struct tw5864_input *dev)
    {
...
#[no_mangle]
pub unsafe extern "C" fn tw5864_video_fini(dev: *mut tw5864_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn tw5864_prepare_frame_headers(input: *mut tw5864_input) {
...
//
#[no_mangle]
unsafe extern "C" fn tw5864_md_metric_from_mvd(mvd: u32) -> c_uint {
...
//
    let mut mv_y: c_uint = (mvd >> 10) & 0x3ff;
    let mut mv_x: c_uint = mvd & 0x3ff;
// heuristic:
    mv_x &= 0x0f;
    mv_y &= 0x0f;
    return mv_y + mv_x;
...
#[no_mangle]
unsafe extern "C" fn tw5864_is_motion_triggered(frame: *mut tw5864_h264_frame) -> c_int {
    ...
    let mut thresh: u16 = input.md_threshold_grid_values[i];
    let mut metric: c_uint = tw5864_md_metric_from_mvd(mv[i]);
    if (metric > thresh)
    detected = 1;
    if (detected)
    break;
...
#[no_mangle]
unsafe extern "C" fn tw5864_handle_frame_work(t: *mut work_struct) {
    static void tw5864_handle_frame_work(struct work_struct *t)
    {
    struct tw5864_dev *dev = from_work(dev, t, bh_work);
    unsigned long flags;
    let mut batch_size: c_int = H264_BUF_CNT;
    spin_lock_irqsave(&dev.slock, flags);
...
#[no_mangle]
unsafe extern "C" fn tw5864_vlc_checksum(data: *mut u32, len: c_int) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn tw5864_handle_frame(frame: *mut tw5864_h264_frame) {
    static void tw5864_handle_frame(struct tw5864_h264_frame *frame)
    {
pub const SKIP_VLCBUF_BYTES: c_int = 3;
    struct tw5864_input *input = frame.input;
    struct tw5864_dev *dev = input.root;
    struct tw5864_buf *vb;
    struct vb2_v4l2_buffer *v4l2_buf;
    let mut frame_len: c_int = frame.vlc_len - SKIP_VLCBUF_BYTES;
    u8 *dst = input.buf_cur_ptr;
    u8 tail_mask, vlc_mask = 0;
    int i;
    let mut vlc_first_byte: u8 = ((u8 *)(frame.vlc.addr + SKIP_VLCBUF_BYTES))[0];
    unsigned long flags;
    int zero_run;
    u8 *src;
    u8 *src_end;
...
#[no_mangle]
unsafe extern "C" fn tw5864_get_v4l2_std(std: enum tw5864_vid_std) -> v4l2_std_id {
    static v4l2_std_id tw5864_get_v4l2_std(enum tw5864_vid_std std)
    {
    switch (std) {
...
#[no_mangle]
unsafe extern "C" fn tw5864_from_v4l2_std(v4l2_std: v4l2_std_id) -> enum tw5864_vid_std {
    static enum tw5864_vid_std tw5864_from_v4l2_std(v4l2_std_id v4l2_std)
    {
...
#[no_mangle]
unsafe extern "C" fn tw5864_encoder_tables_upload(dev: *mut tw5864_dev) {
    static void tw5864_encoder_tables_upload(struct tw5864_dev *dev)
    {
    int i;
