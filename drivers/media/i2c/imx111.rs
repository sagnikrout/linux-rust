...
//! Source: drivers/media/i2c/imx111.c
#![no_std]
...
pub const IMX111_CHIP_ID: c_uint = 0x111;

// general configuration registers

pub const IMX111_MODE_STANDBY: c_int = 0;
pub const IMX111_MODE_STREAMING: c_int = 1;

pub const IMX111_RESET_ON: c_int = 1;

pub const IMX111_GROUP_WRITE_ON: c_int = 1;

pub const IMX111_FRAME_DROP_ON: c_int = 1;

pub const IMX111_DATA_DEPTH_RAW8: c_uint = 0x08;
pub const IMX111_DATA_DEPTH_RAW10: c_uint = 0x0a;
// integration time registers

pub const IMX111_INTEGRATION_TIME_MIN: c_uint = 0x1;
pub const IMX111_INTEGRATION_TIME_MAX: c_uint = 0xffff;
pub const IMX111_INTEGRATION_TIME_STEP: c_int = 1;
pub const IMX111_INTEGRATION_TIME_OFFSET: c_int = 5;
// analog gain control

pub const IMX111_ANA_GAIN_MIN: c_int = 0;
pub const IMX111_ANA_GAIN_MAX: c_int = 240;
pub const IMX111_ANA_GAIN_STEP: c_int = 1;
pub const IMX111_ANA_GAIN_DEFAULT: c_int = 0;
// digital gain control

pub const IMX111_DGTL_GAIN_MIN: c_uint = 0x0100;
pub const IMX111_DGTL_GAIN_MAX: c_uint = 0x0fff;
pub const IMX111_DGTL_GAIN_DEFAULT: c_uint = 0x0100;
pub const IMX111_DGTL_GAIN_STEP: c_int = 1;
// clock configuration registers

pub const IMX111_PLL_SETTLING_TIME_DEFAULT: c_int = 200;

pub const IMX111_POST_DIVIDER_DIV1: c_int = 2;
pub const IMX111_POST_DIVIDER_DIV2: c_int = 0;
pub const IMX111_POST_DIVIDER_DIV4: c_int = 1;
// frame timing registers

pub const IMX111_VTL_MAX: c_uint = 0x09d8;
pub const IMX111_VBLANK_MIN: c_int = 16;

pub const IMX111_HTL_MAX: c_uint = 0x0dd0;
pub const IMX111_HBLANK_MIN: c_int = 16;
// image size registers

// test pattern registers

pub const IMX111_TEST_PATTERN_NONE: c_int = 0;
pub const IMX111_TEST_PATTERN_SOLID: c_int = 1;
pub const IMX111_TEST_PATTERN_BARS: c_int = 2;
pub const IMX111_TEST_PATTERN_FADE: c_int = 3;
pub const IMX111_TEST_PATTERN_PN9: c_int = 4;

pub const IMX111_TESTP_COLOUR_MIN: c_int = 0;
pub const IMX111_TESTP_COLOUR_MAX: c_uint = 0x03ff;
pub const IMX111_TESTP_COLOUR_STEP: c_int = 1;
pub const IMX111_FRAME_RATE_STEP: c_int = 5;
...
// Current mode
    pub cur_mode: *const imx111_mode,
    pub pll: *const imx111_pll,
    pub data_depth: u32,
    pub pixel_clk_raw: u64,
    pub default_link_freq: i64,
}
...
#[no_mangle]
unsafe extern "C" fn to_settle_delay(extclk_rate: u64) -> u8 {
    static u8 to_settle_delay(u64 extclk_rate)
    {
    let mut extclk_mhz: u64 = div_u64(extclk_rate, MEGA);
    return DIV_ROUND_UP(IMX111_PLL_SETTLING_TIME_DEFAULT * extclk_mhz - 63,
    64);
    }
#[no_mangle]
unsafe extern "C" fn imx111_get_format_code(sensor: *mut imx111, code: u32, test: bool) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn imx111_get_format_bpp(format: *const v4l2_mbus_framefmt) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn imx111_update_digital_gain(sensor: *mut imx111, val: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn imx111_set_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn imx111_init_controls(sensor: *mut imx111) -> c_int {
    ...
    sensor.pixel_rate = v4l2_ctrl_new_std(hdl, core::ptr::null_mut(), V4L2_CID_PIXEL_RATE,
    pixel_rate_min, pixel_rate_max,
    1,
    div_u64(sensor.pixel_clk_raw,
    2 *
    sensor.data_depth));
    sensor.link_freq = v4l2_ctrl_new_int_menu(hdl, core::ptr::null_mut(),
...
// Apply default values of current mode
    ...
    mode.reg_list.num_of_regs, core::ptr::null_mut());
    if (ret < 0) {
    ...
    IMX111_MODE_STREAMING, core::ptr::null_mut());
    if (ret)
...
// vflip and hflip cannot change during streaming
    ...
    int ret;
    ret = cci_write(sensor.regmap, IMX111_STREAMING_MODE,
    IMX111_MODE_STANDBY, core::ptr::null_mut());
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn imx111_initialize(sensor: *mut imx111) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn imx111_init_subdev(sensor: *mut imx111, client: *mut i2c_client) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn imx111_power_on(sensor: *mut imx111) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn imx111_power_off(sensor: *mut imx111) {
...
#[no_mangle]
unsafe extern "C" fn imx111_pm_runtime_resume(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn imx111_pm_runtime_suspend(dev: *mut device) -> int __maybe_unused {
    ...
    imx111_pm_runtime_resume, core::ptr::null_mut())
    };
// ----------------------------------------------------------------------------
// Probe & Remove
//
#[no_mangle]
unsafe extern "C" fn imx111_identify_module(sensor: *mut imx111) -> c_int {
    static int imx111_identify_module(struct imx111 *sensor)
    {
    struct device *dev = regmap_get_device(sensor.regmap);
    u64 value, revision, manufacturer;
    let mut ret: c_int = 0;
    ret = cci_read(sensor.regmap, IMX111_PRODUCT_ID, &value, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn imx111_clk_init(sensor: *mut imx111) -> c_int {
    static int imx111_clk_init(struct imx111 *sensor)
    {
    struct device *dev = regmap_get_device(sensor.regmap);
    let mut ndata_lanes: u32 = sensor.bus_cfg.bus.mipi_csi2.num_data_lanes;
...
#[no_mangle]
unsafe extern "C" fn imx111_parse_dt(sensor: *mut imx111) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn imx111_probe(client: *mut i2c_client) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn imx111_remove(client: *mut i2c_client) {
