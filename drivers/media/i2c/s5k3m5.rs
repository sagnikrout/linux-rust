...
//! Source: drivers/media/i2c/s5k3m5.c
#![no_std]
...
pub const S5K3M5_DATA_LANES: c_int = 4;
// Register map is similar to MIPI CCS compliant camera sensors

pub const S5K3M5_CHIP_ID: c_uint = 0x30d5;
// Both streaming and flipping settings are controlled at the same time

pub const S5K3M5_EXPOSURE_MIN: c_int = 8;
pub const S5K3M5_EXPOSURE_STEP: c_int = 1;
pub const S5K3M5_EXPOSURE_MARGIN: c_int = 4;

pub const S5K3M5_AGAIN_MIN: c_int = 1;
pub const S5K3M5_AGAIN_MAX: c_int = 16;
pub const S5K3M5_AGAIN_STEP: c_int = 1;
pub const S5K3M5_AGAIN_DEFAULT: c_int = 1;
pub const S5K3M5_AGAIN_SHIFT: c_int = 5;

pub const S5K3M5_VTS_MAX: c_uint = 0xffff;
...
pub struct s5k3m5_mode {
    ...
    pub /: *const *const s5k3m5_reg_list reg_list; / Sensor register setting,
}
...
pub struct s5k3m5 {
    ...
    pub supplies: [regulator_bulk_data; S5K3M5_NUM_SUPPLIES],
    pub sd: v4l2_subdev,
    pub pad: media_pad,
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_set_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
// V4L2 controls are applied, when sensor is powered up for streaming
    ...
    ctrl.val << S5K3M5_AGAIN_SHIFT, core::ptr::null_mut());
    break;
...
#[no_mangle]
pub unsafe extern "C" fn s5k3m5_freq_to_pixel_rate(freq: u64) -> u64 {
    static inline u64 s5k3m5_freq_to_pixel_rate(const u64 freq)
    {
    return div_u64(freq * 2 * S5K3M5_DATA_LANES, 10);
    }
#[no_mangle]
unsafe extern "C" fn s5k3m5_init_controls(s5k3m5: *mut s5k3m5) -> c_int {
...
// Sensor init settings
    ...
    struct s5k3m5 *s5k3m5 = to_s5k3m5(sd);
    int ret;
    ret = cci_write(s5k3m5.regmap, S5K3M5_REG_CTRL_MODE, 0, core::ptr::null_mut());
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_get_format_code(s5k3m5: *mut s5k3m5) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_identify_sensor(s5k3m5: *mut s5k3m5) -> c_int {
    static int s5k3m5_identify_sensor(struct s5k3m5 *s5k3m5)
    {
    u64 val;
    int ret;
    ret = cci_read(s5k3m5.regmap, S5K3M5_REG_CHIP_ID, &val, core::ptr::null_mut());
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_check_hwcfg(s5k3m5: *mut s5k3m5) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_power_on(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_power_off(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_probe(client: *mut i2c_client) -> c_int {
    ...
    "failed to init CCI\n");
    s5k3m5.mclk = devm_v4l2_sensor_clk_get(s5k3m5.dev, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn s5k3m5_remove(client: *mut i2c_client) {
    ...
    SET_RUNTIME_PM_OPS(s5k3m5_power_off, s5k3m5_power_on, core::ptr::null_mut())
    };
