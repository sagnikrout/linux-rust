...
//! Source: drivers/media/i2c/og0ve1b.c
#![no_std]
...
pub const OG0VE1B_CHIP_ID: c_uint = 0xc75645;

pub const OG0VE1B_MODE_STANDBY: c_uint = 0x00;

// Exposure controls from sensor

pub const OG0VE1B_EXPOSURE_MIN: c_int = 1;
pub const OG0VE1B_EXPOSURE_MAX_MARGIN: c_int = 14;
pub const OG0VE1B_EXPOSURE_STEP: c_int = 1;
pub const OG0VE1B_EXPOSURE_DEFAULT: c_int = 554;
// Analogue gain controls from sensor

pub const OG0VE1B_ANALOGUE_GAIN_MIN: c_int = 1;
pub const OG0VE1B_ANALOGUE_GAIN_MAX: c_uint = 0x1ff;
pub const OG0VE1B_ANALOGUE_GAIN_STEP: c_int = 1;
pub const OG0VE1B_ANALOGUE_GAIN_DEFAULT: c_int = 16;
// Vertical timing size

pub const OG0VE1B_VTS_MAX: c_uint = 0xffff;
// Test pattern
...
pub struct og0ve1b_mode {
    ...
    pub /: *const *const og0ve1b_reg_list reg_list; / Sensor register setting,
}
...
pub struct og0ve1b {
    ...
    pub supplies: [regulator_bulk_data; OG0VE1B_NUM_SUPPLIES],
    pub sd: v4l2_subdev,
    pub pad: media_pad,
...
#[no_mangle]
unsafe extern "C" fn og0ve1b_enable_test_pattern(og0ve1b: *mut og0ve1b, pattern: u32) -> c_int {
    static int og0ve1b_enable_test_pattern(struct og0ve1b *og0ve1b, u32 pattern)
    {
    let mut val: u64 = og0ve1b.pre_isp;
    if (pattern)
    val |= OG0VE1B_TEST_PATTERN_ENABLE;
    else
    val &= ~OG0VE1B_TEST_PATTERN_ENABLE;
    return cci_write(og0ve1b.regmap, OG0VE1B_REG_PRE_ISP, val, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn og0ve1b_set_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn og0ve1b_init_controls(og0ve1b: *mut og0ve1b) -> c_int {
...
// Skip a step of explicit entering into the standby mode
    ...
    OG0VE1B_SOFTWARE_RST, core::ptr::null_mut());
    if (ret) {
    ...
    OG0VE1B_MODE_STREAMING, core::ptr::null_mut());
    if (ret) {
    ...
    int ret;
    ret = cci_write(og0ve1b.regmap, OG0VE1B_REG_MODE_SELECT,
    OG0VE1B_MODE_STANDBY, core::ptr::null_mut());
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn og0ve1b_identify_sensor(og0ve1b: *mut og0ve1b) -> c_int {
    static int og0ve1b_identify_sensor(struct og0ve1b *og0ve1b)
    {
    u64 val;
    int ret;
    ret = cci_read(og0ve1b.regmap, OG0VE1B_REG_CHIP_ID, &val, core::ptr::null_mut());
    if (ret) {
    ...
    &og0ve1b.pre_isp, core::ptr::null_mut());
    if (ret)
    dev_err(og0ve1b.dev, "failed to read pre_isp: %d\n", ret);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn og0ve1b_check_hwcfg(og0ve1b: *mut og0ve1b) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn og0ve1b_power_on(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn og0ve1b_power_off(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn og0ve1b_probe(client: *mut i2c_client) -> c_int {
    ...
    "failed to init CCI\n");
    og0ve1b.xvclk = devm_v4l2_sensor_clk_get(og0ve1b.dev, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn og0ve1b_remove(client: *mut i2c_client) {
    ...
    SET_RUNTIME_PM_OPS(og0ve1b_power_off, og0ve1b_power_on, core::ptr::null_mut())
    };
