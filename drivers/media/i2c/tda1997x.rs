...
//! Source: drivers/media/i2c/tda1997x.c
#![no_std]
...
pub const TDA1997X_MBUS_CODES: c_int = 5;
// debug level
    static int debug;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tda1997x_chip_info {
    pub type: enum tda1997x_type,
    pub name: *const c_char,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct tda1997x_state {
    pub info: *const tda1997x_chip_info,
    pub pdata: tda1997x_platform_data,
    pub client: *mut i2c_client,
    pub client_cec: *mut i2c_client,
    pub sd: v4l2_subdev,
    pub supplies: [regulator_bulk_data; TDA1997X_NUM_SUPPLIES],
    pub pads: [media_pad; TDA1997X_NUM_PADS],
    pub lock: mutex,
    pub page_lock: mutex,
    pub page: c_char,
// detected info from chip
    pub chip_revision: c_int,
    pub port_30bit: c_char,
    pub output_2p5: c_char,
    pub tmdsb_clk: c_char,
    pub tmdsb_soc: c_char,
// status info
    pub hdmi_status: c_char,
    pub mptrw_in_progress: c_char,
    pub activity_status: c_char,
    pub input_detect: [c_char; 2],
// video
    pub avi_infoframe: hdmi_avi_infoframe,
    pub colorimetry: v4l2_hdmi_colorimetry,
    pub rgb_quantization_range: u32,
    pub timings: v4l2_dv_timings,
    pub fps: c_int,
    pub conv: *const color_matrix_coefs,
    pub /: *mut *mut u32 mbus_codes[TDA1997X_MBUS_CODES]; / available modes,
    pub /: *mut *mut u32 mbus_code; / current mode,
    pub vid_fmt: u8,
// controls
...
// audio
    pub audio_ch_alloc: u8,
    pub audio_samplerate: c_int,
    pub audio_channels: c_int,
    pub audio_samplesize: c_int,
    pub audio_type: c_int,
    pub audio_lock: mutex,
    pub audio_stream: *mut snd_pcm_substream,
// EDID
    struct {
    pub edid: [u8; 256],
    pub present: u32,
    pub blocks: c_uint,
    pub edid: },
    pub delayed_work_enable_hpd: delayed_work,
}
...
#[no_mangle]
unsafe extern "C" fn tda1997x_cec_read(sd: *mut v4l2_subdev, reg: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_cec_write(sd: *mut v4l2_subdev, reg: u8, val: u8) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tda1997x_setpage(sd: *mut v4l2_subdev, page: u8) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn io_read(sd: *mut v4l2_subdev, reg: u16) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn io_read16(sd: *mut v4l2_subdev, reg: u16) -> c_long {
...
#[no_mangle]
pub unsafe extern "C" fn io_read24(sd: *mut v4l2_subdev, reg: u16) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn io_readn(sd: *mut v4l2_subdev, reg: u16, len: u8, data: *mut u8) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn io_write(sd: *mut v4l2_subdev, reg: u16, val: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn io_write16(sd: *mut v4l2_subdev, reg: u16, val: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn io_write24(sd: *mut v4l2_subdev, reg: u16, val: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_manual_hpd(sd: *mut v4l2_subdev, mode: enum hpd_mode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_delayed_work_enable_hpd(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_disable_edid(sd: *mut v4l2_subdev) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_enable_edid(sd: *mut v4l2_subdev) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct blanking_codes {
    pub code_gy: i16,
    pub code_bu: i16,
    pub code_rv: i16,
}

    let mut rgb_blanking: static struct blanking_codes = { 64, 64, 64 };
    let mut yuv_blanking: static struct blanking_codes = { 64, 512, 512 };
    const struct blanking_codes *blanking_codes = core::ptr::null_mut();
    u8 reg;
...
#[no_mangle]
pub unsafe extern "C" fn if(V4L2_COLORSPACE_SMPTE170M: c->colorspace ==) -> else {
...
// Configure Audio output port signals
    ...
    struct tda1997x_state *state = to_state(sd);
    struct tda1997x_platform_data *pdata = &state.pdata;
    let mut sp_used_by_fifo: bool = true;
    u8 reg;
...
// Manual control of TMDS
    ...
    {
    let mut reg: u8 = io_read(sd, REG_DETECT_5V);
...
//
    ...
    (u32)timings.bt.pixelclock / 500, core::ptr::null_mut(), core::ptr::null_mut());
    v4l2_print_dv_timings(sd.name, "Detected format: ", timings, false);
    return 0;
    }
// some sort of errata workaround for chip revision 0 (N1)
#[no_mangle]
unsafe extern "C" fn tda1997x_reset_n1(state: *mut tda1997x_state) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_irq_sus(state: *mut tda1997x_state, flags: *mut u8) {
...
// filter FMT interrupt based on SUS state
    ...
    if (debug)
    tda1997x_detect_std(state, core::ptr::null_mut());
// notify user of change in resolution
    v4l2_subdev_notify_event(&state.sd, &tda1997x_ev_fmt);
    }
    }
#[no_mangle]
unsafe extern "C" fn tda1997x_irq_ddc(state: *mut tda1997x_state, flags: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_irq_rate(state: *mut tda1997x_state, flags: *mut u8) {
...
// we only pay attention to stability change events
    ...
    let mut input: c_int = (source & MASK_RATE_A_ST)?0:1;
    let mut mask: u8 = 1<<input;
// state change
...
#[no_mangle]
unsafe extern "C" fn tda1997x_irq_info(state: *mut tda1997x_state, flags: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_irq_audio(state: *mut tda1997x_state, flags: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_irq_hdcp(state: *mut tda1997x_state, flags: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_isr_thread(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(INTERRUPT_AUDIO: flags &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(!hsper: !vper || !hper ||) -> else {
...
// fmt = format->format;
    ...
    let mut ret: c_int = tda1997x_setup_format(state, format.format.code);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn tda1997x_get_edid(sd: *mut v4l2_subdev, edid: *mut v4l2_edid) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_set_edid(sd: *mut v4l2_subdev, edid: *mut v4l2_edid) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tda1997x_log_infoframe(sd: *mut v4l2_subdev, addr: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_log_status(sd: *mut v4l2_subdev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tda1997x_s_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_g_volatile_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_core_init(sd: *mut v4l2_subdev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_set_power(state: *mut tda1997x_state, on: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_parse_dt(state: *mut tda1997x_state) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(str, 0: "spdif") ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_get_regulators(state: *mut tda1997x_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_identify_module(state: *mut tda1997x_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_codec_probe(component: *mut snd_soc_component) -> c_int {
    static int tda1997x_codec_probe(struct snd_soc_component *component)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn tda1997x_codec_remove(component: *mut snd_soc_component) {
...
#[no_mangle]
unsafe extern "C" fn tda1997x_probe(client: *mut i2c_client) -> c_int {
...
// request irq
    ...
    core::ptr::null_mut(), tda1997x_isr_thread,
...
#[no_mangle]
unsafe extern "C" fn tda1997x_remove(client: *mut i2c_client) {
