...
//! Source: drivers/media/i2c/gc05a2.c
#![no_std]
...
pub const GC05A2_TEST_PATTERN_EN: c_uint = 0x01;

pub const GC05A2_EXP_MARGIN: c_int = 16;
pub const GC05A2_EXP_MIN: c_int = 4;
pub const GC05A2_EXP_STEP: c_int = 1;

pub const GC05A2_AGAIN_MIN: c_int = 1024;

pub const GC05A2_AGAIN_STEP: c_int = 1;

pub const GC05A2_VTS_MAX: c_uint = 0xffff;

pub const GC05A2_CHIP_ID: c_uint = 0x05a2;
pub const GC05A2_NATIVE_WIDTH: c_int = 2592;
pub const GC05A2_NATIVE_HEIGHT: c_int = 1944;

pub const GC05A2_DATA_LANES: c_int = 2;
pub const GC05A2_RGB_DEPTH: c_int = 10;
...
pub struct gc05a2 {
    ...
    pub pad: media_pad,
    pub xclk: *mut clk,
    pub supplies: [regulator_bulk_data; ARRAY_SIZE(gc05a2_supply_name)],
...
// True if the device has been identified
    pub identified: bool,
    pub cur_mode: *const gc05a2_mode,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct gc05a2_reg_list {
    pub num_of_regs: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gc05a2_mode {
    pub width: u32,
    pub height: u32,
    pub reg_list: gc05a2_reg_list,
    pub /: *mut *mut u32 hts; / Horizontal timining size,
    pub /: *mut *mut u32 vts_def; / Default vertical timining size,
    pub /: *mut *mut u32 vts_min; / Min vertical timining size,
}
...
#[no_mangle]
unsafe extern "C" fn gc05a2_power_on(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_power_off(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_set_ctrl_hflip(gc05a2: *mut gc05a2, ctrl_val: u32) -> c_int {
    static int gc05a2_set_ctrl_hflip(struct gc05a2 *gc05a2, u32 ctrl_val)
    {
    int ret;
    u64 val;
    ret = cci_read(gc05a2.regmap, GC05A2_FLIP_REG, &val, core::ptr::null_mut());
    if (ret) {
    ...
    GC05A2_FLIP_H_MASK,
    ctrl_val ? GC05A2_FLIP_H_MASK : 0, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn gc05a2_set_ctrl_vflip(gc05a2: *mut gc05a2, ctrl_val: u32) -> c_int {
    ...
    return cci_update_bits(gc05a2.regmap, GC05A2_FLIP_REG,
    GC05A2_FLIP_V_MASK,
    ctrl_val ? GC05A2_FLIP_V_MASK : 0, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn gc05a2_test_pattern(gc05a2: *mut gc05a2, pattern_menu: u32) -> c_int {
...
// Set pattern to 0, it's a safe default.
    ...
    pattern, core::ptr::null_mut());
    if (ret)
    return ret;
    return cci_write(gc05a2.regmap, GC05A2_REG_TEST_PATTERN_EN,
    GC05A2_TEST_PATTERN_EN, core::ptr::null_mut());
    } else {
    return cci_write(gc05a2.regmap, GC05A2_REG_TEST_PATTERN_EN,
    0x00, core::ptr::null_mut());
    }
    }
#[no_mangle]
unsafe extern "C" fn gc05a2_set_ctrl(ctrl: *mut v4l2_ctrl) -> c_int {
...
//
    ...
    gc05a2.cur_mode.height + ctrl.val, core::ptr::null_mut());
    break;
    case V4L2_CID_HFLIP:
...
#[no_mangle]
unsafe extern "C" fn gc05a2_identify_module(gc05a2: *mut gc05a2) -> c_int {
    ...
    return 0;
    ret = cci_read(gc05a2.regmap, GC05A2_REG_CHIP_ID, &val, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn gc05a2_start_streaming(gc05a2: *mut gc05a2) -> c_int {
    ...
    ret = cci_multi_reg_write(gc05a2.regmap,
    reg_list.regs, reg_list.num_of_regs, core::ptr::null_mut());
    if (ret < 0)
    goto err_rpm_put;
    ...
    ret = cci_write(gc05a2.regmap, GC05A2_STREAMING_REG, 1, core::ptr::null_mut());
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_stop_streaming(gc05a2: *mut gc05a2) -> c_int {
    static int gc05a2_stop_streaming(struct gc05a2 *gc05a2)
    {
    int ret;
    ret = cci_write(gc05a2.regmap, GC05A2_STREAMING_REG, 0, core::ptr::null_mut());
    if (ret < 0)
...
#[no_mangle]
unsafe extern "C" fn gc05a2_s_stream(subdev: *mut v4l2_subdev, enable: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_get_regulators(dev: *mut device, gc05a2: *mut gc05a2) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_parse_fwnode(gc05a2: *mut gc05a2) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_to_pixel_rate(f_index: u32) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_init_controls(gc05a2: *mut gc05a2) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gc05a2_probe(client: *mut i2c_client) -> c_int {
    ...
    "failed to init CCI\n");
    gc05a2.xclk = devm_v4l2_sensor_clk_get_legacy(dev, core::ptr::null_mut(), true,
...
#[no_mangle]
unsafe extern "C" fn gc05a2_remove(client: *mut i2c_client) {
