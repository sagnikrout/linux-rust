...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct switch_path {
    pub dmdev: *mut dm_dev,
    pub start: sector_t,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct switch_ctx {
    pub ti: *mut dm_target,
    pub /: *mut *mut unsigned int nr_paths; / Number of paths in path_list.,
    pub /: *mut *mut unsigned int region_size; / Region size in 512-byte sectors,
    pub /: *mut *mut unsigned long nr_regions; / Number of regions making up the device,
    pub /: *mut *mut signed char region_size_bits; / log2 of region_size or -1,
    pub /: *mut *mut unsigned char region_table_entry_bits; / Number of bits in one region table entry,
    pub /: *mut *mut unsigned char region_entries_per_slot; / Number of entries in one region table slot,
    pub /: *mut *mut signed char region_entries_per_slot_bits; / log2 of region_entries_per_slot or -1,
    pub /: *mut *mut *mut region_table_slot_t region_table; / Region table,
//
// Array of dm devices to switch between.
//
    pub path_list: [switch_path; ],
}
...
#[no_mangle]
unsafe extern "C" fn alloc_region_table(ti: *mut dm_target, nr_paths: c_uint) -> c_int {
    static int alloc_region_table(struct dm_target *ti, unsigned int nr_paths)
    {
    struct switch_ctx *sctx = ti.private;
    let mut nr_regions: sector_t = ti.len;
    sector_t nr_slots;
...
#[no_mangle]
unsafe extern "C" fn switch_region_table_read(sctx: *mut switch_ctx, region_nr: c_ulong) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn switch_get_path_nr(sctx: *mut switch_ctx, offset: sector_t) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn initialise_region_table(sctx: *mut switch_ctx) {
    static void initialise_region_table(struct switch_ctx *sctx)
    {
    let mut path_nr: c_uint = 0;
...
#[no_mangle]
unsafe extern "C" fn parse_path(as: *mut dm_arg_set, ti: *mut dm_target) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn switch_dtr(ti: *mut dm_target) {
...
//
#[no_mangle]
unsafe extern "C" fn switch_ctr(ti: *mut dm_target, argc: c_uint, argv: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn switch_map(ti: *mut dm_target, bio: *mut bio) -> c_int {
    static int switch_map(struct dm_target *ti, struct bio *bio)
    {
    struct switch_ctx *sctx = ti.private;
    let mut offset: sector_t = dm_target_offset(ti, bio.bi_iter.bi_sector);
    let mut path_nr: c_uint = switch_get_path_nr(sctx, offset);
...
#[no_mangle]
unsafe extern "C" fn parse_hex(string: *const c_char) -> __always_inline unsigned long {
    ...
    {
    unsigned int i;
    let mut region_index: c_ulong = 0;
    for (i = 1; i < argc; i++) {
