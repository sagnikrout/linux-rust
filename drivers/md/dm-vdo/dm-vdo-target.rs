...
//! Source: drivers/md/dm-vdo/dm-vdo-target.c
#![no_std]
...
pub const TABLE_VERSION: c_int = 4;
...
//
pub const BIT_COUNT_MINIMUM: c_int = 1000;
// Grow the bit array by this many bits when needed
pub const BIT_COUNT_INCREMENT: c_int = 100;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct instance_tracker {
    pub bit_count: c_uint,
    pub words: *mut c_ulong,
    pub count: c_uint,
    pub next: c_uint,
}
...
//
#[no_mangle]
unsafe extern "C" fn free_device_config(config: *mut device_config) {
    static void free_device_config(struct device_config *config)
    {
    if (config == core::ptr::null_mut())
    return;
    if (config.owned_device != core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn free_string_array(string_array: *mut c_char) {
    static void free_string_array(char **string_array)
    {
    unsigned int offset;
    for (offset = 0; string_array[offset] != core::ptr::null_mut(); offset++)
...
//
#[no_mangle]
unsafe extern "C" fn split_string(string: *const c_char, separator: c_char, substring_array_ptr: *mut c_char) -> c_int {
    static int split_string(const char *string, char separator, char ***substring_array_ptr)
    {
    let mut current_substring: c_uint = 0, substring_count = 1;
    const char *s;
    char **substrings;
    int result;
    ptrdiff_t length;
    ...
    for (s = string; *s != 0; s++) {
    if (*s == separator) {
    let mut length: ptrdiff_t = s - string;
...
//
    ...
    for (i = 0; (i < array_length) && (substring_array[i] != core::ptr::null_mut()); i++)
...
//
    ...
    value = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(bool_str, 0: false_str) ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn parse_slab_size(slab_str: *const c_char, slab_size_ptr: *mut block_count_t) -> int __must_check {
...
//
    ...
    if (result != VDO_SUCCESS)
    return result;
    if ((fields[0] == core::ptr::null_mut()) || (fields[1] == core::ptr::null_mut()) || (fields[2] != core::ptr::null_mut())) {
...
//
    ...
    struct thread_count_config *config)
    {
    let mut result: c_int = VDO_SUCCESS;
    char **specs;
    if (strcmp(".", string) != 0) {
    unsigned int i;
    ...
    if (result != VDO_SUCCESS)
    return result;
    for (i = 0; specs[i] != core::ptr::null_mut(); i++) {
...
//
#[no_mangle]
unsafe extern "C" fn parse_key_value_pairs(argc: c_int, argv: *mut c_char, config: *mut device_config) -> c_int {
...
//
    ...
    {
    bool enable_512e;
    let mut logical_bytes: usize = to_bytes(ti.len);
    struct dm_arg_set arg_set;
    char **error_ptr = &ti.error;
    struct device_config *config = core::ptr::null_mut();
    int result;
...
//
    ...
    if (config.version == 0) {
    let mut device_size: u64 = bdev_nr_bytes(config.owned_device.bdev);
...
#[no_mangle]
unsafe extern "C" fn vdo_map_bio(ti: *mut dm_target, bio: *mut bio) -> c_int {
...
// This could deadlock,
    ...
    BUG_ON((current_work_queue != core::ptr::null_mut()) &&
    (vdo == vdo_get_work_queue_owner(current_work_queue).vdo));
    vdo_launch_bio(vdo.data_vio_pool, bio);
    return DM_MAPIO_SUBMITTED;
    }
#[no_mangle]
unsafe extern "C" fn vdo_io_hints(ti: *mut dm_target, limits: *mut queue_limits) {
...
#[no_mangle]
unsafe extern "C" fn get_underlying_device_block_count(vdo: *const vdo) -> block_count_t __must_check {
...
// Pairs with the implicit barrier in cmpxchg just above
    ...
    vdo_register_allocating_thread(&allocating_thread, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn configure_target_capabilities(ti: *mut dm_target) {
...
//
#[no_mangle]
unsafe extern "C" fn vdo_uses_device(vdo: *mut vdo, context: *const c_void) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn get_thread_id_for_phase(vdo: *mut vdo) -> thread_id_t __must_check {
...
//
#[no_mangle]
unsafe extern "C" fn advance_phase(vdo: *mut vdo) -> u32 {
    static u32 advance_phase(struct vdo *vdo)
    {
    let mut phase: u32 = vdo.admin.phase++;
...
#[no_mangle]
unsafe extern "C" fn assert_admin_phase_thread(vdo: *mut vdo, what: *const c_char) {
...
//
#[no_mangle]
unsafe extern "C" fn finish_operation_callback(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn decode_from_super_block(vdo: *mut vdo) -> int __must_check {
...
//
#[no_mangle]
unsafe extern "C" fn decode_vdo(vdo: *mut vdo) -> int __must_check {
...
//
#[no_mangle]
unsafe extern "C" fn pre_load_callback(completion: *mut vdo_completion) {
...
#[no_mangle]
unsafe extern "C" fn release_instance(instance: c_uint) {
    ...
    struct vdo *vdo;
    int result;
    let mut block_size: u64 = VDO_BLOCK_SIZE;
    let mut logical_size: u64 = to_bytes(ti.len);
    let mut logical_blocks: block_count_t = logical_size / block_size;
...
// Implements vdo_filter_fn.
#[no_mangle]
unsafe extern "C" fn vdo_is_named(vdo: *mut vdo, context: *const c_void) -> bool __must_check {
...
//
#[no_mangle]
unsafe extern "C" fn get_bit_array_size(bit_count: c_uint) -> usize {
...
//
#[no_mangle]
unsafe extern "C" fn grow_bit_array() -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn allocate_instance(instance_ptr: *mut c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn construct_new_vdo(ti: *mut dm_target, argc: c_uint, argv: *mut c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn check_may_grow_physical(completion: *mut vdo_completion) {
...
#[no_mangle]
unsafe extern "C" fn get_partition_size(layout: *mut layout, id: enum partition_id) -> block_count_t {
...
//
#[no_mangle]
unsafe extern "C" fn grow_layout(vdo: *mut vdo, old_size: block_count_t, new_size: block_count_t) -> c_int {
...
// Make a copy completion if there isn't one
    if (vdo.partition_copier == core::ptr::null_mut()) {
    vdo.partition_copier = dm_kcopyd_client_create(core::ptr::null_mut());
    if (IS_ERR(vdo.partition_copier)) {
    result = PTR_ERR(vdo.partition_copier);
    vdo.partition_copier = core::ptr::null_mut();
    return result;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn prepare_to_grow_physical(vdo: *mut vdo, new_physical_blocks: block_count_t) -> c_int {
    static int prepare_to_grow_physical(struct vdo *vdo, block_count_t new_physical_blocks)
    {
    int result;
    let mut current_physical_blocks: block_count_t = vdo.states.vdo.config.physical_blocks;
...
// error_ptr = "VDO physical size may not grow in current state";
    ...
    {
    int result;
    let mut may_grow: bool = (vdo_get_admin_state(vdo) != VDO_ADMIN_STATE_PRE_LOADED);
    ...
    let mut logical_blocks: block_count_t = vdo.states.vdo.config.logical_blocks;
...
#[no_mangle]
unsafe extern "C" fn vdo_ctr(ti: *mut dm_target, argc: c_uint, argv: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vdo_dtr(ti: *mut dm_target) {
...
// This was the last config referencing the VDO. Free it.
    let mut instance: c_uint = vdo.instance;
    struct registered_thread allocating_thread, instance_thread;
    vdo_register_thread_device_id(&instance_thread, &instance);
    vdo_register_allocating_thread(&allocating_thread, core::ptr::null_mut());
    device_name = vdo_get_device_name(ti);
...
#[no_mangle]
unsafe extern "C" fn vdo_presuspend(ti: *mut dm_target) {
...
//
#[no_mangle]
unsafe extern "C" fn write_super_block_for_suspend(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn suspend_callback(completion: *mut vdo_completion) {
...
#[no_mangle]
unsafe extern "C" fn vdo_postsuspend(ti: *mut dm_target) {
...
//
#[no_mangle]
unsafe extern "C" fn was_new(vdo: *const vdo) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn requires_repair(vdo: *const vdo) -> bool __must_check {
...
//
#[no_mangle]
unsafe extern "C" fn get_load_type(vdo: *mut vdo) -> enum slab_depot_load_type {
...
//
#[no_mangle]
unsafe extern "C" fn load_callback(completion: *mut vdo_completion) {
...
// Avoid an infinite loop
    completion.error_handler = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn handle_load_error(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn write_super_block_for_resume(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn resume_callback(completion: *mut vdo_completion) {
    ...
    case RESUME_PHASE_PACKER:
    {
    let mut was_enabled: bool = vdo_get_compressing(vdo);
    let mut enable: bool = vdo.device_config.compression;
...
//
#[no_mangle]
unsafe extern "C" fn grow_logical_callback(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn handle_logical_growth_error(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn perform_grow_logical(vdo: *mut vdo, new_logical_blocks: block_count_t) -> c_int {
...
unsafe extern "C" fn copy_callback(read_err: c_int, write_err: c_ulong, context: *mut c_void) {
    ...
    let mut result: c_int = (((read_err == 0) && (write_err == 0)) ? VDO_SUCCESS : -EIO);
    vdo_continue_completion(completion, result);
    }
    static void partition_to_region(struct partition *partition, struct vdo *vdo,
    struct dm_io_region *region)
    {
    let mut pbn: physical_block_number_t = partition.offset - vdo.geometry.bio_offset;
...
//
#[no_mangle]
unsafe extern "C" fn grow_physical_callback(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn handle_physical_growth_error(completion: *mut vdo_completion) {
...
//
#[no_mangle]
unsafe extern "C" fn perform_grow_physical(vdo: *mut vdo, new_physical_blocks: block_count_t) -> c_int {
    ...
    let mut old_physical_blocks: block_count_t = vdo.states.vdo.config.physical_blocks;
// Skip any noop grows.
...
// Validate that we are prepared to grow appropriately.
    ...
    prepared_depot_size = (vdo.depot.new_slabs == core::ptr::null_mut()) ? 0 : vdo.depot.new_size;
...
#[no_mangle]
unsafe extern "C" fn vdo_preresume_registered(ti: *mut dm_target, vdo: *mut vdo) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vdo_preresume(ti: *mut dm_target) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vdo_resume(ti: *mut dm_target) {
...
#[no_mangle]
unsafe extern "C" fn vdo_module_destroy() {
...
#[no_mangle]
unsafe extern "C" fn vdo_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn vdo_exit() -> void __exit {
