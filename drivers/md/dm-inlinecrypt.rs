...
//! Source: drivers/md/dm-inlinecrypt.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct inlinecrypt_ctx {
    pub dev: *mut dm_dev,
    pub start: sector_t,
    pub cipher_string: *const c_char,
    pub key_size: c_uint,
    pub iv_offset: u64,
    pub sector_size: c_uint,
    pub sector_bits: c_uint,
    pub key_type: enum blk_crypto_key_type,
    pub key: blk_crypto_key,
    pub max_dun: u64,
}
...
#[no_mangle]
unsafe extern "C" fn inlinecrypt_dtr(ti: *mut dm_target) {
...
unsafe extern "C" fn get_key_size(key_string: *mut c_char) -> c_int {
    static int get_key_size(char **key_string)
    {
    let mut key_hex_size: c_int = strlen(*key_string);
    if (*key_string[0] == ':')
    return -EINVAL;
...
// key string should be :<logon|user>:<key_desc>
    ...
    let mut iv_large_sectors: bool = false;
    char dummy;
    int err;
    as.argc = argc;
    as.argv = argv;
...
//
#[no_mangle]
unsafe extern "C" fn inlinecrypt_ctr(ti: *mut dm_target, argc: c_uint, argv: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn inlinecrypt_map(ti: *mut dm_target, bio: *mut bio) -> c_int {
...
// Only pass ioctls through if the device sizes match exactly.
    ...
    {
    const struct inlinecrypt_ctx *ctx = ti.private;
    let mut sector_size: c_uint = ctx.sector_size;
