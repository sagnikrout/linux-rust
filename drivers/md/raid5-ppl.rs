...
// array of child logs, one for each raid disk
    pub child_logs: *mut ppl_log,
    pub count: c_int,
    pub data_sector: *mut *mut int block_size; / the logical block size used for,
// in ppl_header_entry
    pub /: *mut *mut u32 signature; / raid array identifier,
    pub /: *mut *mut atomic64_t seq; / current log write sequence number,
    pub io_kc: *mut kmem_cache,
    pub io_pool: mempool_t,
    pub bs: bio_set,
    pub flush_bs: bio_set,
// used only for recovery
    pub recovered_entries: c_int,
    pub mismatch_count: c_int,
// stripes to retry if failed to allocate io_unit
    pub no_mem_stripes: list_head,
    pub no_mem_stripes_lock: spinlock_t,
...
#[derive(Copy, Clone)]
pub struct ppl_log {
    pub /: *mut *mut *mut ppl_conf ppl_conf; / shared between all log instances,
    pub with: *mut *mut *mut md_rdev rdev; / array member disk associated,
// this log instance
    pub io_mutex: mutex,
    pub data: *mut *mut *mut ppl_io_unit current_io; / current io_unit accepting new,
// always at the end of io_list
    pub io_list_lock: spinlock_t,
    pub /: *mut *mut list_head io_list; / all io_units of this log,
    pub next_io_sector: sector_t,
    pub entry_space: c_uint,
    pub use_multippl: bool,
    pub wb_cache_on: bool,
    pub disk_flush_bitmap: c_ulong,
}

pub const PPL_IO_INLINE_BVECS: c_int = 32;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ppl_io_unit {
    pub log: *mut ppl_log,
    pub /: *mut *mut *mut page header_page; / for ppl_header,
    pub /: *mut *mut unsigned int entries_count; / number of entries in ppl_header,
    pub /: *mut *mut unsigned int pp_size; / total size current of partial parity,
    pub /: *mut *mut u64 seq; / sequence number of this log write,
    pub /: *mut *mut list_head log_sibling; / log->io_list,
    pub /: *mut *mut list_head stripe_list; / stripes added to the io_unit,
    pub /: *mut *mut atomic_t pending_stripes; / how many stripes not written to raid,
    pub /: *mut *mut atomic_t pending_flushes; / how many disk flushes are in progress,
    pub /: *mut *mut bool submitted; / true if write to log started,
// inline bio and its biovec for submitting the iounit
    pub bio: bio,
    pub biovec: [bio_vec; PPL_IO_INLINE_BVECS],
}
    ...
    {
    let mut disks: c_int = sh.disks;
    struct page **srcs = percpu.scribble;
    let mut count: c_int = 0, pd_idx = sh.pd_idx, i;
...
// rcw: xor data from all not updated disks
    ...
    core::ptr::null_mut(), sh, (void *) (srcs + sh.disks + 2));
    if (count == 1)
...
#[no_mangle]
unsafe extern "C" fn ppl_io_pool_free(element: *mut c_void, pool_data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn ppl_log_stripe(log: *mut ppl_log, sh: *mut stripe_head) -> c_int {
    static int ppl_log_stripe(struct ppl_log *log, struct stripe_head *sh)
    {
    struct ppl_io_unit *io = log.current_io;
    struct ppl_header_entry *e = core::ptr::null_mut();
    struct ppl_header *pplhdr;
    int i;
    let mut data_sector: sector_t = 0;
    let mut data_disks: c_int = 0;
...
// add a new unit if there is none or the current is full
    ...
    let mut data_sector_last: u64 = le64_to_cpu(last.data_sector);
    let mut data_size_last: u32 = le32_to_cpu(last.data_size);
//
...
#[no_mangle]
pub unsafe extern "C" fn ppl_write_stripe(conf: *mut r5conf, sh: *mut stripe_head) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ppl_log_endio(bio: *mut bio) {
...
#[no_mangle]
unsafe extern "C" fn ppl_submit_iounit_bio(io: *mut ppl_io_unit, bio: *mut bio) {
...
#[no_mangle]
unsafe extern "C" fn ppl_submit_iounit(io: *mut ppl_io_unit) {
...
#[no_mangle]
unsafe extern "C" fn ppl_submit_current_io(log: *mut ppl_log) {
...
#[no_mangle]
pub unsafe extern "C" fn ppl_write_stripe_run(conf: *mut r5conf) {
...
#[no_mangle]
unsafe extern "C" fn ppl_io_unit_finished(io: *mut ppl_io_unit) {
...
#[no_mangle]
unsafe extern "C" fn ppl_flush_endio(bio: *mut bio) {
...
#[no_mangle]
unsafe extern "C" fn ppl_do_flush(io: *mut ppl_io_unit) {
    static void ppl_do_flush(struct ppl_io_unit *io)
    {
    struct ppl_log *log = io.log;
    struct ppl_conf *ppl_conf = log.ppl_conf;
    struct r5conf *conf = ppl_conf.mddev.private;
    let mut raid_disks: c_int = conf.raid_disks;
    let mut flushed_disks: c_int = 0;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn ppl_quiesce(conf: *mut r5conf, quiesce: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn ppl_handle_flush_request(bio: *mut bio) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ppl_stripe_write_finished(sh: *mut stripe_head) {
    void ppl_stripe_write_finished(struct stripe_head *sh)
    {
    struct ppl_io_unit *io;
    io = sh.ppl_io;
    sh.ppl_io = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ppl_xor(size: c_int, page1: *mut page, page2: *mut page) {
...
//
    ...
    let mut block_size: c_int = ppl_conf.block_size;
    struct page *page1;
    struct page *page2;
    ...
    int i;
    let mut ret: c_int = 0;
    let mut pp_size: c_uint = le32_to_cpu(e.pp_size);
    let mut data_size: c_uint = le32_to_cpu(e.data_size);
...
// iterate through blocks in strip
    ...
    let mut update_parity: bool = false;
...
// iterate through all PPL entries saved
    ...
    struct ppl_header_entry *e = &pplhdr.entries[i];
    let mut pp_size: u32 = le32_to_cpu(e.pp_size);
    let mut sector: sector_t = ppl_sector;
    let mut ppl_entry_sectors: c_int = pp_size >> 9;
    u32 crc, crc_stored;
...
// read parial parity for this entry and calculate its checksum
    while (pp_size) {
    let mut s: c_int = pp_size > PAGE_SIZE ? PAGE_SIZE : pp_size;
...
#[no_mangle]
unsafe extern "C" fn ppl_write_empty_header(log: *mut ppl_log) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ppl_load_distributed(log: *mut ppl_log) -> c_int {
    ...
    struct ppl_header *pplhdr = core::ptr::null_mut(), *prev_pplhdr = core::ptr::null_mut();
    u32 crc, crc_stored;
    u32 signature;
    let mut ret: c_int = 0, i;
    let mut pplhdr_offset: sector_t = 0, prev_pplhdr_offset = 0;
...
#[no_mangle]
unsafe extern "C" fn ppl_load(ppl_conf: *mut ppl_conf) -> c_int {
    static int ppl_load(struct ppl_conf *ppl_conf)
    {
    let mut ret: c_int = 0;
    let mut signature: u32 = 0;
    let mut signature_set: bool = false;
    int i;
    for (i = 0; i < ppl_conf.count; i++) {
    struct ppl_log *log = &ppl_conf.child_logs[i];
// skip missing drive
...
#[no_mangle]
unsafe extern "C" fn __ppl_exit_log(ppl_conf: *mut ppl_conf) {
...
#[no_mangle]
pub unsafe extern "C" fn ppl_exit_log(conf: *mut r5conf) {
    void ppl_exit_log(struct r5conf *conf)
    {
    struct ppl_conf *ppl_conf = conf.log_private;
    if (ppl_conf) {
    __ppl_exit_log(ppl_conf);
    conf.log_private = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn ppl_validate_rdev(rdev: *mut md_rdev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ppl_init_child_log(log: *mut ppl_log, rdev: *mut md_rdev) {
...
#[no_mangle]
pub unsafe extern "C" fn ppl_init_log(conf: *mut r5conf) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ppl_modify_log(conf: *mut r5conf, rdev: *mut md_rdev, add: bool) -> c_int {
    ...
    err = -ENODEV;
#[no_mangle]
pub unsafe extern "C" fn if(!conf->log_private: !raid5_has_ppl(conf) ||) -> else {
