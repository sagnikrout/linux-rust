...
pub struct badblock {
    pub node: rb_node,
    pub bb: sector_t,
    pub wr_fail_cnt: c_uchar,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct dust_device {
    pub dev: *mut dm_dev,
    pub badblocklist: rb_root,
    pub badblock_count: c_ulonglong,
    pub dust_lock: spinlock_t,
    pub blksz: c_uint,
    pub sect_per_block_shift: c_int,
    pub sect_per_block: c_uint,
    pub start: sector_t,
    pub fail_read_on_bb:1: bool,
    pub quiet_mode:1: bool,
}
    ...
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(blk: bblk->bb <) -> else {
    else if (bblk.bb < blk)
    node = node.rb_right;
    else
    return bblk;
...
#[no_mangle]
unsafe extern "C" fn dust_rb_insert(root: *mut rb_root, new: *mut badblock) -> bool {
    static bool dust_rb_insert(struct rb_root *root, struct badblock *new)
    {
    struct badblock *bblk;
    struct rb_node **link = &root.rb_node, *parent = core::ptr::null_mut();
    let mut value: sector_t = new.bb;
    while (*link) {
    parent = *link;
    ...
    link = &(*link).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(value: bblk->bb <) -> else {
...
#[no_mangle]
unsafe extern "C" fn dust_remove_block(dd: *mut dust_device, block: c_ulonglong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __dust_map_read(dd: *mut dust_device, thisblock: sector_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __dust_map_write(dd: *mut dust_device, thisblock: sector_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dust_map(ti: *mut dm_target, bio: *mut bio) -> c_int {
    static int dust_map(struct dm_target *ti, struct bio *bio)
    {
    struct dust_device *dd = ti.private;
    let mut dust_sector: sector_t = dm_target_offset(ti, bio.bi_iter.bi_sector);
    int r;
    ...
    {
    struct rb_node *node = core::ptr::null_mut(), *nnode = core::ptr::null_mut();
    nnode = rb_first(tree);
    if (nnode == core::ptr::null_mut()) {
    BUG_ON(count != 0);
    return false;
    }
    while (nnode) {
    ...
    BUG_ON(count != 0);
    BUG_ON(tree.rb_node != core::ptr::null_mut());
    return true;
    }
...
//
#[no_mangle]
unsafe extern "C" fn dust_ctr(ti: *mut dm_target, argc: c_uint, argv: *mut c_char) -> c_int {
    ...
    let mut DUST_MAX_BLKSZ_SECTORS: sector_t = 2097152;
    let mut max_block_sectors: sector_t = min(ti.len, DUST_MAX_BLKSZ_SECTORS);
    if (argc != 3) {
...
#[no_mangle]
unsafe extern "C" fn dust_dtr(ti: *mut dm_target) {
    ...
    {
    struct dust_device *dd = ti.private;
    let mut size: sector_t = dm_sector_div_up(ti.len, dd.sect_per_block);
    let mut invalid_msg: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcasecmp(argv[0], _arg: "removebadblock")) -> else {
    else if (!strcasecmp(argv[0], "removebadblock"))
    r = dust_remove_block(dd, block);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcasecmp(argv[0], _arg: "queryblock")) -> else {
