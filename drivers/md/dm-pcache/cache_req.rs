...
//! Source: drivers/md/dm-pcache/cache_req.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cache_data_head_init(cache: *mut pcache_cache) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn cache_data_alloc(cache: *mut pcache_cache, key: *mut pcache_cache_key) -> c_int {
...
// Put for head_pos->cache_seg: release the reference for the current head's segment.
    ...
    head_pos.cache_seg = core::ptr::null_mut();
    } else {
...
//
#[no_mangle]
unsafe extern "C" fn miss_read_end_req(backing_req: *mut pcache_backing_dev_req, read_ret: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn submit_cache_miss_req(cache: *mut pcache_cache, backing_req: *mut pcache_backing_dev_req) {
...
#[no_mangle]
unsafe extern "C" fn cache_miss_req_free(backing_req: *mut pcache_backing_dev_req) {
    static void cache_miss_req_free(struct pcache_backing_dev_req *backing_req)
    {
    struct pcache_cache_key *key;
    if (backing_req.priv_data) {
    key = backing_req.priv_data;
    backing_req.priv_data = core::ptr::null_mut();
    ...
    struct pcache_cache_key *key = core::ptr::null_mut();
    let mut req_opts: pcache_backing_dev_req_opts = { 0 };
    ...
    ctx.pre_alloc_req = core::ptr::null_mut();
    return backing_req;
    }
...
//
#[no_mangle]
unsafe extern "C" fn read_walk_finally(ctx: *mut pcache_cache_subtree_walk_ctx, ret: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn read_walk_done(ctx: *mut pcache_cache_subtree_walk_ctx) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn cache_read(cache: *mut pcache_cache, pcache_req: *mut pcache_request) -> c_int {
    static int cache_read(struct pcache_cache *cache, struct pcache_request *pcache_req)
    {
    let mut key_data: pcache_cache_key = { .off = pcache_req.off, .len = pcache_req.data_len };
    struct pcache_cache_subtree *cache_subtree;
    ...
    spin_lock(&cache_subtree.tree_lock);
    search:
    prev_node = cache_subtree_search(cache_subtree, key, core::ptr::null_mut(), core::ptr::null_mut(), &delete_key_list);
...
#[no_mangle]
unsafe extern "C" fn cache_write(cache: *mut pcache_cache, pcache_req: *mut pcache_request) -> c_int {
    static int cache_write(struct pcache_cache *cache, struct pcache_request *pcache_req)
    {
    struct pcache_cache_subtree *cache_subtree;
    struct pcache_cache_key *key;
    let mut offset: u64 = pcache_req.off;
    let mut length: u32 = pcache_req.data_len;
    let mut io_done: u32 = 0;
    int ret;
    while (true) {
    if (io_done >= length)
    break;
...
//
#[no_mangle]
pub unsafe extern "C" fn pcache_cache_flush(cache: *mut pcache_cache) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn pcache_cache_handle_req(cache: *mut pcache_cache, pcache_req: *mut pcache_request) -> c_int {
