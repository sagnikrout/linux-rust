...
//! Source: drivers/md/dm-snap-persistent.c
#![no_std]
...
pub const DM_PREFETCH_CHUNKS: c_int = 12;
//
// ---------------------------------------------------------------
...
//
pub const SNAP_MAGIC: c_uint = 0x70416e53;
//
// The on-disk version of the metadata.
//
pub const SNAPSHOT_DISK_VERSION: c_int = 1;
pub const NUM_SNAPSHOT_HDR_CHUNKS: c_int = 1;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct disk_header {
    pub magic: __le32,
//
...
//
    pub version: __le32,
// In sectors
    pub chunk_size: __le32,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct disk_exception {
    pub old_chunk: __le64,
    pub new_chunk: __le64,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct core_exception {
    pub old_chunk: u64,
    pub new_chunk: u64,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct commit_callback {
    pub success): *mut *mut *mut void (callback)(void ref, int,
...
//
    pub zero_area: *mut c_void,
//
// An area used for header. The header can be written
// concurrently with metadata (when invalidating the snapshot),
// so it needs a separate buffer.
//
    pub header_area: *mut c_void,
//
// Used to keep track of which metadata area the data in
// 'chunk' refers to.
//
    pub current_area: chunk_t,
//
...
#[no_mangle]
unsafe extern "C" fn free_area(ps: *mut pstore) {
    static void free_area(struct pstore *ps)
    {
    vfree(ps.area);
    ps.area = core::ptr::null_mut();
    vfree(ps.zero_area);
    ps.zero_area = core::ptr::null_mut();
    vfree(ps.header_area);
    ps.header_area = core::ptr::null_mut();
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mdata_req {
    pub where: *mut dm_io_region,
    pub io_req: *mut dm_io_request,
    pub work: work_struct,
    pub result: c_int,
}

#[no_mangle]
unsafe extern "C" fn do_metadata(work: *mut work_struct) {
    static void do_metadata(struct work_struct *work)
    {
    struct mdata_req *req = container_of(work, struct mdata_req, work);
    req.result = dm_io(req.io_req, 1, req.where, core::ptr::null_mut(), core::ptr::null_mut(), IOPRIO_DEFAULT);
    }
//
...
//
    ...
    .mem.ptr.vma = area,
    .client = ps.io_client,
    .notify.fn = core::ptr::null_mut(),
    };
    struct mdata_req req;
    if (!metadata)
    return dm_io(&io_req, 1, &where, core::ptr::null_mut(), core::ptr::null_mut(), IOPRIO_DEFAULT);
    req.where = &where;
    req.io_req = &io_req;
//
...
//
#[no_mangle]
unsafe extern "C" fn area_location(ps: *mut pstore, area: chunk_t) -> chunk_t {
    static chunk_t area_location(struct pstore *ps, chunk_t area)
    {
    return NUM_SNAPSHOT_HDR_CHUNKS + ((ps.exceptions_per_area + 1) * area);
    }
#[no_mangle]
unsafe extern "C" fn skip_metadata(ps: *mut pstore) {
    static void skip_metadata(struct pstore *ps)
    {
    let mut stride: u32 = ps.exceptions_per_area + 1;
    let mut next_free: chunk_t = ps.next_free;
...
//
#[no_mangle]
unsafe extern "C" fn area_io(ps: *mut pstore, opf: blk_opf_t) -> c_int {
    static int area_io(struct pstore *ps, blk_opf_t opf)
    {
    let mut chunk: chunk_t = area_location(ps, ps.current_area);
    return chunk_io(ps, ps.area, chunk, opf, 0);
    }
#[no_mangle]
unsafe extern "C" fn zero_memory_area(ps: *mut pstore) {
    static void zero_memory_area(struct pstore *ps)
    {
    memset(ps.area, 0, ps.store.chunk_size << SECTOR_SHIFT);
    }
#[no_mangle]
unsafe extern "C" fn zero_disk_area(ps: *mut pstore, area: chunk_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn read_header(ps: *mut pstore, new_snapshot: *mut c_int) -> c_int {
    static int read_header(struct pstore *ps, int *new_snapshot)
    {
    int r;
    struct disk_header *dh;
    unsigned int chunk_size;
    let mut chunk_size_supplied: c_int = 1;
    char *chunk_err;
//
...
#[no_mangle]
unsafe extern "C" fn write_header(ps: *mut pstore) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn clear_exception(ps: *mut pstore, index: u32) {
...
//
    ...
    let mut prefetch_area: chunk_t = 0;
    client = dm_bufio_client_create(dm_snap_cow(ps.store.snap).bdev,
    ps.store.chunk_size << SECTOR_SHIFT,
    1, 0, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (IS_ERR(client))
...
//
    ...
    do {
    let mut pf_chunk: chunk_t = area_location(ps, prefetch_area);
...
#[no_mangle]
unsafe extern "C" fn persistent_dtr(store: *mut dm_exception_store) {
...
#[no_mangle]
unsafe extern "C" fn persistent_drop_snapshot(store: *mut dm_exception_store) {
...
#[no_mangle]
unsafe extern "C" fn persistent_ctr(store: *mut dm_exception_store, options: *mut c_char) -> c_int {
...
// allocate the pstore
    ...
    ps.callbacks = core::ptr::null_mut();
    ...
    if (options) {
    let mut overflow: c_char = toupper(options[0]);
    if (overflow == 'O')
...
#[no_mangle]
pub unsafe extern "C" fn dm_persistent_snapshot_init() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dm_persistent_snapshot_exit() {
