...
//! Source: drivers/cpuidle/governors/teo.c
#![no_std]
...
//
pub const PULSE: c_int = 1024;
pub const DECAY_SHIFT: c_int = 3;
//
...
#[derive(Copy, Clone)]
pub struct teo_cpu {
    pub sleep_length_ns: i64,
    pub state_bins: [teo_bin; CPUIDLE_STATE_MAX],
    pub total: c_uint,
    pub total_tick: c_uint,
    pub tick_intercepts: c_uint,
    pub short_idles: c_uint,
    pub tick_wakeup: bool,
}

    static DEFINE_PER_CPU(struct teo_cpu, teo_cpus);
#[no_mangle]
unsafe extern "C" fn teo_decay(metric: *mut c_uint) {
    static void teo_decay(unsigned int *metric)
    {
    let mut delta: c_uint = *metric >> DECAY_SHIFT;
    if (delta)
// metric -= delta;
    else
// metric = 0;
...
//
#[no_mangle]
unsafe extern "C" fn teo_update(drv: *mut cpuidle_driver, dev: *mut cpuidle_device) {
    static void teo_update(struct cpuidle_driver *drv, struct cpuidle_device *dev)
    {
    let mut lat_ns: i64 = drv.states[dev.last_state_idx].exit_latency_ns;
...
//
    ...
    let mut idx_intercept_sum: c_uint = 0;
    let mut intercept_sum: c_uint = 0;
    let mut intercept_max: c_uint = 0;
    let mut idx_hit_sum: c_uint = 0;
    let mut hit_sum: c_uint = 0;
    let mut intercept_max_idx: c_int = -1;
    let mut constraint_idx: c_int = 0;
    let mut idx0: c_int = 0, idx = -1;
    s64 duration_ns;
    int i;
...
//
    ...
    let mut prev_intercepts: c_uint = prev_bin.intercepts;
...
//
#[no_mangle]
unsafe extern "C" fn teo_reflect(dev: *mut cpuidle_device, state: c_int) {
...
#[no_mangle]
unsafe extern "C" fn teo_governor_init() -> int __init {
