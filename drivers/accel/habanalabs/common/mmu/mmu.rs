...
//! Source: drivers/accel/habanalabs/common/mmu/mmu.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn hl_is_dram_va(hdev: *mut hl_device, virt_addr: u64) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_init(hdev: *mut hl_device) -> c_int {
    ...
    if (hdev.mmu_func[MMU_DR_PGT].init != core::ptr::null_mut()) {
    rc = hdev.mmu_func[MMU_DR_PGT].init(hdev);
    if (rc)
    return rc;
    }
    if (hdev.mmu_func[MMU_HR_PGT].init != core::ptr::null_mut()) {
    ...
    return 0;
    fini_dr_mmu:
    if (hdev.mmu_func[MMU_DR_PGT].fini != core::ptr::null_mut())
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_fini(hdev: *mut hl_device) {
    ...
    if (hdev.mmu_func[MMU_DR_PGT].fini != core::ptr::null_mut())
    hdev.mmu_func[MMU_DR_PGT].fini(hdev);
    if (hdev.mmu_func[MMU_HR_PGT].fini != core::ptr::null_mut())
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_ctx_init(ctx: *mut hl_ctx) -> c_int {
    ...
    if (hdev.mmu_disable)
    return 0;
    if (hdev.mmu_func[MMU_DR_PGT].ctx_init != core::ptr::null_mut()) {
    rc = hdev.mmu_func[MMU_DR_PGT].ctx_init(ctx);
    if (rc)
    return rc;
    }
    if (hdev.mmu_func[MMU_HR_PGT].ctx_init != core::ptr::null_mut()) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_ctx_fini(ctx: *mut hl_ctx) {
    void hl_mmu_ctx_fini(struct hl_ctx *ctx)
    {
    struct hl_device *hdev = ctx.hdev;
    if (hdev.mmu_disable)
    return;
    if (hdev.mmu_func[MMU_DR_PGT].ctx_fini != core::ptr::null_mut())
    hdev.mmu_func[MMU_DR_PGT].ctx_fini(ctx);
    if (hdev.mmu_func[MMU_HR_PGT].ctx_fini != core::ptr::null_mut())
...
// real_page_size = mmu_prop->page_size;
    ...
    return &prop.dmmu;
#[no_mangle]
pub unsafe extern "C" fn if(0: (page_size % prop->pmmu_huge.page_size) ==) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_unmap_page(ctx: *mut hl_ctx, virt_addr: u64, page_size: u32, flush_pte: bool) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_unmap_contiguous(ctx: *mut hl_ctx, virt_addr: u64, size: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(HL_VA_RANGE_TYPE_HOST: hops->range_type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_va_to_pa(ctx: *mut hl_ctx, virt_addr: u64, phys_addr: *mut u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_if_set_funcs(hdev: *mut hl_device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_scramble_addr(hdev: *mut hl_device, addr: u64) -> u64 {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_descramble_addr(hdev: *mut hl_device, addr: u64) -> u64 {
    u64 hl_mmu_descramble_addr(struct hl_device *hdev, u64 addr)
    {
    return addr;
    }
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_invalidate_cache(hdev: *mut hl_device, is_hard: bool, flags: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hl_mmu_prefetch_work_function(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_prefetch_cache_range(ctx: *mut hl_ctx, flags: u32, asid: u32, va: u64, size: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_get_next_hop_addr(ctx: *mut hl_ctx, curr_pte: u64) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_hr_flush(ctx: *mut hl_ctx) {
...
//
    ...
    {
    struct asic_fixed_properties *prop = &hdev.asic_prop;
    let mut pool_chunk_size: usize = SZ_4M;
    struct pgt_info *hop0_pgt;
    dma_addr_t dma_addr;
    u64 virt_addr;
    int i, rc;
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_hr_fini(hdev: *mut hl_device, hr_priv: *mut hl_mmu_hr_priv, hop_table_size: u32) {
...
//
    hr_priv.mmu_asid_hop0 = core::ptr::null_mut();
    }
    }
//
...
//
    ...
    {
    let mut page_mask: u64 = (hop_table_size - 1);
    let mut pte_offset: u64 = phys_pte_addr & page_mask;
...
//
    let mut virt_addr: u64 = hl_mmu_hr_pte_phys_to_virt(ctx, pgt_info, phys_pte_addr, hop_table_size);
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_hr_get_pte(ctx: *mut hl_ctx, hr_func: *mut hl_hr_mmu_funcs, phys_hop_addr: u64) {
...
//
    ...
    u64 curr_pte)
    {
    let mut next_hop_phys_addr: u64 = hl_mmu_get_next_hop_addr(ctx, curr_pte);
...
// No memory in pool - get some and try again
    ...
    pgt_info.shadow_addr = (unsigned long) core::ptr::null_mut();
...
//
    ...
    {
    let mut hop_addr: u64 = hl_mmu_get_next_hop_addr(ctx, curr_pte);
...
// using 6 HOPs as this is the maximum number of HOPs
    struct pgt_info *hops_pgt_info[MMU_ARCH_6_HOPS] = { core::ptr::null_mut() };
    struct hl_device *hdev = ctx.hdev;
    struct hl_mmu_properties *mmu_prop;
    int rc, i, used_hops;
    bool is_huge;
...
// if passed over all hops then no last hop was found
    ...
    {
    struct pgt_info *pgt_info = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_free_hop(ctx: *mut hl_ctx, hop_addr: u64) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_free_pgt_node(ctx: *mut hl_ctx, pgt_info: *mut pgt_info) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_get_phys_hop0_addr(ctx: *mut hl_ctx) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_get_hop0_addr(ctx: *mut hl_ctx) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_get_phys_addr(ctx: *mut hl_ctx, shadow_addr: u64) -> u64 {
    u64 hl_mmu_dr_get_phys_addr(struct hl_ctx *ctx, u64 shadow_addr)
    {
    let mut page_mask: u64 = ctx.hdev.asic_prop.dmmu.hop_table_size - 1;
    let mut shadow_hop_addr: u64 = shadow_addr & (~page_mask);
    let mut pte_offset: u64 = shadow_addr & page_mask;
    u64 phys_hop_addr;
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_write_pte(ctx: *mut hl_ctx, shadow_pte_addr: u64, val: u64) {
    void hl_mmu_dr_write_pte(struct hl_ctx *ctx, u64 shadow_pte_addr, u64 val)
    {
    let mut phys_val: u64 = hl_mmu_dr_get_phys_addr(ctx, val);
    ctx.hdev.asic_funcs.write_pte(ctx.hdev, hl_mmu_dr_get_phys_addr(ctx, shadow_pte_addr),
    phys_val);
// (u64 *) (uintptr_t) shadow_pte_addr = val;
    }
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_write_final_pte(ctx: *mut hl_ctx, shadow_pte_addr: u64, val: u64) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_clear_pte(ctx: *mut hl_ctx, pte_addr: u64) {
    void hl_mmu_dr_clear_pte(struct hl_ctx *ctx, u64 pte_addr)
    {
    hl_mmu_dr_write_final_pte(ctx, pte_addr, 0);
    }
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_get_pte(ctx: *mut hl_ctx, hop_addr: u64) {
    void hl_mmu_dr_get_pte(struct hl_ctx *ctx, u64 hop_addr)
    {
    hl_mmu_dr_get_pgt_info(ctx, hop_addr).num_of_ptes++;
    }
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_put_pte(ctx: *mut hl_ctx, hop_addr: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_alloc_hop(ctx: *mut hl_ctx) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_get_alloc_next_hop_addr(ctx: *mut hl_ctx, curr_pte: u64, is_new_hop: *mut bool) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_flush(ctx: *mut hl_ctx) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_init(hdev: *mut hl_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hl_mmu_dr_fini(hdev: *mut hl_device) {
...
// example if we fail during hard reset code at certain points
//
    hdev.mmu_priv.dr.mmu_shadow_hop0 = core::ptr::null_mut();
    }
