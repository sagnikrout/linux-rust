...
//! Source: drivers/accel/habanalabs/common/device.c
#![no_std]
...
pub const HL_DEVICE_RELEASE_WATCHDOG_TIMEOUT_SEC: c_int = 30;
    enum dma_alloc_type {
    DMA_ALLOC_COHERENT,
    DMA_ALLOC_POOL,
    };
pub const MEM_SCRUB_DEFAULT_VAL: c_uint = 0x1122334455667788;
...
// new_bar_region_base = bar_base_addr;
    ...
    let mut old_base: u64 = 0, rc, bar_region_base = region.region_base;
...
// val = readq(acc_addr);
    ...
    if (set_dram_bar) {
    rc = hl_set_dram_bar(hdev, old_base, region, core::ptr::null_mut());
    if (rc == U64_MAX)
    return -EIO;
    }
    return 0;
...
// this is needed to avoid warning on using freed pointer
    let mut store_cpu_addr: u64 = (u64) (uintptr_t) cpu_addr;
    switch (alloc_type) {
    case DMA_ALLOC_COHERENT:
...
#[no_mangle]
pub unsafe extern "C" fn hl_cpu_accessible_dma_pool_free(hdev: *mut hl_device, size: usize, vaddr: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_engine_data_sprintf(e: *mut engines_data, fmt: *const c_char, ...) {
...
//
    str_size = vsnprintf(core::ptr::null_mut(), 0, fmt, args) + 1;
    va_end(args);
...
#[no_mangle]
pub unsafe extern "C" fn hl_device_status(hdev: *mut hl_device) -> enum hl_device_status {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: idle_mask[2]) -> else {
    else if (idle_mask[2])
    dev_err(hdev.dev, "%s %s (mask %#llx_%016llx_%016llx)\n",
    dev_name(&hdev.pdev.dev), message,
    idle_mask[2], idle_mask[1], idle_mask[0]);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: idle_mask[1]) -> else {
    else if (idle_mask[1])
...
#[no_mangle]
unsafe extern "C" fn hpriv_release(ref: *mut kref) {
...
//
    ...
    HL_BUSY_ENGINES_MASK_EXT_SIZE, core::ptr::null_mut());
    if (!device_is_idle) {
...
// Scrubbing is handled within hl_device_reset(), so here need to do it directly
    let mut rc: c_int = hdev.asic_funcs.scrub_device_mem(hdev);
    if (rc) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_hpriv_get(hpriv: *mut hl_fpriv) {
    void hl_hpriv_get(struct hl_fpriv *hpriv)
    {
    kref_get(&hpriv.refcount);
    }
#[no_mangle]
pub unsafe extern "C" fn hl_hpriv_put(hpriv: *mut hl_fpriv) -> c_int {
    ...
    let mut unknown_reason: bool = true;
    char buf[128];
    size_t size;
    int offset;
    size = sizeof(buf);
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_device_release(ddev: *mut drm_device, file_priv: *mut drm_file) {
...
#[no_mangle]
unsafe extern "C" fn hl_device_release_ctrl(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __hl_mmap(hpriv: *mut hl_fpriv, vma: *mut vm_area_struct) -> c_int {
    ...
    return hl_mem_mgr_mmap(&hpriv.mem_mgr, vma, core::ptr::null_mut());
    }
    return -EINVAL;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_mmap(filp: *mut file, vma: *mut vm_area_struct) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn device_release_func(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn cdev_sysfs_debugfs_add(hdev: *mut hl_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cdev_sysfs_debugfs_remove(hdev: *mut hl_device) {
...
#[no_mangle]
unsafe extern "C" fn device_hard_reset_pending(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn device_release_watchdog_func(work: *mut work_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn device_early_init(hdev: *mut hl_device) -> c_int {
    ...
    if (hdev.cq_wq[i] == core::ptr::null_mut()) {
    ...
    if (hdev.eq_wq == core::ptr::null_mut()) {
    ...
    return 0;
    free_cb_mgr:
    hl_mem_mgr_fini(&hdev.kernel_mem_mgr, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn device_early_fini(hdev: *mut hl_device) {
...
#[no_mangle]
unsafe extern "C" fn is_pci_link_healthy(hdev: *mut hl_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hl_device_eq_heartbeat_received(hdev: *mut hl_device) -> bool {
    static bool hl_device_eq_heartbeat_received(struct hl_device *hdev)
    {
    struct eq_heartbeat_debug_info *heartbeat_debug_info = &hdev.heartbeat_debug_info;
    let mut cpu_q_id: u32 = heartbeat_debug_info.cpu_queue_id, pq_pi_mask = (HL_QUEUE_LENGTH << 1) - 1;
    struct asic_fixed_properties *prop = &hdev.asic_prop;
...
#[no_mangle]
unsafe extern "C" fn hl_device_heartbeat(work: *mut work_struct) {
    static void hl_device_heartbeat(struct work_struct *work)
    {
    struct hl_device *hdev = container_of(work, struct hl_device,
    work_heartbeat.work);
    let mut info: hl_info_fw_err_info = {0};
    let mut event_mask: u64 = HL_NOTIFIER_EVENT_DEVICE_RESET | HL_NOTIFIER_EVENT_DEVICE_UNAVAILABLE;
// Start heartbeat checks only after driver has enabled events from FW
    if (!hl_device_operational(hdev, core::ptr::null_mut()) || !hdev.init_done)
    goto reschedule;
//
...
//
    ...
    goto reschedule;
    if (hl_device_operational(hdev, core::ptr::null_mut()))
...
//
#[no_mangle]
unsafe extern "C" fn device_late_init(hdev: *mut hl_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn device_late_fini(hdev: *mut hl_device) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_device_utilization(hdev: *mut hl_device, utilization: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hl_device_set_debug_mode(hdev: *mut hl_device, ctx: *mut hl_ctx, enable: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn take_release_locks(hdev: *mut hl_device) {
...
#[no_mangle]
unsafe extern "C" fn hl_abort_waiting_for_completions(hdev: *mut hl_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_device_suspend(hdev: *mut hl_device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_device_resume(hdev: *mut hl_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn device_kill_open_processes(hdev: *mut hl_device, timeout: u32, control_dev: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn device_disable_open_processes(hdev: *mut hl_device, control_dev: bool) {
    ...
    mutex_lock(hpriv_lock);
    list_for_each_entry(hpriv, hpriv_list, dev_node)
    hpriv.hdev = core::ptr::null_mut();
    mutex_unlock(hpriv_lock);
    }
#[no_mangle]
unsafe extern "C" fn send_disable_pci_access(hdev: *mut hl_device, flags: u32) {
...
#[no_mangle]
unsafe extern "C" fn handle_reset_trigger(hdev: *mut hl_device, flags: u32) {
    static void handle_reset_trigger(struct hl_device *hdev, u32 flags)
    {
    let mut cur_reset_trigger: u32 = HL_RESET_TRIGGER_DEFAULT;
...
#[no_mangle]
unsafe extern "C" fn reset_heartbeat_debug_info(hdev: *mut hl_device) {
...
#[no_mangle]
pub unsafe extern "C" fn device_heartbeat_schedule(hdev: *mut hl_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_device_reset(hdev: *mut hl_device, flags: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: reset_upon_device_release) -> else {
...
// Release kernel context
    ...
    hdev.kernel_ctx = core::ptr::null_mut();
    hl_vm_fini(hdev);
    hl_mmu_fini(hdev);
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_device_cond_reset(hdev: *mut hl_device, flags: u32, event_mask: u64) -> c_int {
    int hl_device_cond_reset(struct hl_device *hdev, u32 flags, u64 event_mask)
    {
    struct hl_ctx *ctx = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn hl_notifier_event_send(notifier_event: *mut hl_notifier_event, event_mask: u64) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_notifier_event_send_all(hdev: *mut hl_device, event_mask: u64) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_device_init(hdev: *mut hl_device) -> c_int {
    int hl_device_init(struct hl_device *hdev)
    {
    int i, rc, cq_cnt, user_interrupt_cnt, cq_ready_cnt;
    struct hl_ts_free_jobs *free_jobs_data;
    let mut expose_interfaces_on_err: bool = false;
    void *p;
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_device_fini(hdev: *mut hl_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_rreg(hdev: *mut hl_device, reg: u32) -> u32 {
    inline u32 hl_rreg(struct hl_device *hdev, u32 reg)
    {
    let mut val: u32 = readl(hdev.rmmio + reg);
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_wreg(hdev: *mut hl_device, reg: u32, val: u32) {
...
#[no_mangle]
unsafe extern "C" fn hl_capture_user_mappings(hdev: *mut hl_device, is_pmmu: bool) {
    ...
    struct hl_vm_phys_pg_pack *phys_pg_pack = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn hl_capture_page_fault(hdev: *mut hl_device, addr: u64, eng_id: u16, is_pmmu: bool) {
...
#[no_mangle]
unsafe extern "C" fn hl_capture_hw_err(hdev: *mut hl_device, event_id: u16) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_handle_critical_hw_err(hdev: *mut hl_device, event_id: u16, event_mask: *mut u64) {
...
#[no_mangle]
unsafe extern "C" fn hl_capture_fw_err(hdev: *mut hl_device, fw_info: *mut hl_info_fw_err_info) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_handle_fw_err(hdev: *mut hl_device, info: *mut hl_info_fw_err_info) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_capture_engine_err(hdev: *mut hl_device, engine_id: u16, error_count: u16) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_enable_err_info_capture(captured_err_info: *mut hl_error_info) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_init_cpu_for_irq(hdev: *mut hl_device) {
    void hl_init_cpu_for_irq(struct hl_device *hdev)
    {

    struct cpumask *available_mask = &hdev.irq_affinity_mask;
    let mut numa_node: c_int = hdev.pdev.dev.numa_node, i;
...
#[no_mangle]
pub unsafe extern "C" fn hl_set_irq_affinity(hdev: *mut hl_device, irq: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_eq_heartbeat_event_handle(hdev: *mut hl_device) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_handle_clk_change_event(hdev: *mut hl_device, event_type: u16, event_mask: *mut u64) {
...
#[no_mangle]
pub unsafe extern "C" fn hl_eq_cpld_shutdown_event_handle(hdev: *mut hl_device, event_id: u16, event_mask: *mut u64) {
