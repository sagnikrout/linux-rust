...
//! Source: drivers/accel/habanalabs/common/state_dump.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn resize_to_fit(buf: *mut c_char, size: *mut usize, desired_size: usize) -> c_int {
...
//
    ...
    size_t length;
    int rc;
    if (*buf == core::ptr::null_mut() && (*size != 0 || *offset != 0))
    return -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn hl_state_dump_free_sync_objects(sync_objects: *mut u32) {
...
//
    ...
    u32 *sync_objects = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn hl_state_dump_free_monitors(monitors: *mut hl_mon_state_dump) {
...
//
    ...
    {
    struct hl_state_dump_specs *sds = &hdev.state_dump_specs;
    struct hl_mon_state_dump *monitors = core::ptr::null_mut();
    let mut rc: c_int = 0, i;
    if (sds.sync_namager_names) {
    rc = hl_snprintf_resize(
    buf, size, offset, "%s\n",
...
//
#[no_mangle]
pub unsafe extern "C" fn hl_state_dump(hdev: *mut hl_device) -> c_int {
    int hl_state_dump(struct hl_device *hdev)
    {
    char *buf = core::ptr::null_mut();
    let mut offset: usize = 0, size = 0;
    int rc;
    rc = hl_snprintf_resize(&buf, &size, &offset,
