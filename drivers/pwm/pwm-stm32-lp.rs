...
//! Source: drivers/pwm/pwm-stm32-lp.c
#![no_std]
...
pub const STM32_LPTIM_MAX_PRESCALER: c_int = 128;
#[no_mangle]
unsafe extern "C" fn stm32_pwm_lp_update_allowed(priv: *mut stm32_pwm_lp, channel: c_int) -> c_int {
...
// Keep PWM counter clock refcount in sync with PWM initial state
    if (state.enabled) {
    let mut ret: c_int = clk_enable(priv.clk);
    if (ret)
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_lp_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_lp_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_lp_resume(dev: *mut device) -> c_int {
