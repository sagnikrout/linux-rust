...
//! Source: drivers/pwm/pwm-airoha.c
#![no_std]
...
pub const AIROHA_PWM_REG_SGPIO_LED_DATA: c_uint = 0x0024;

pub const AIROHA_PWM_REG_SGPIO_CLK_DIVR: c_uint = 0x0028;

pub const AIROHA_PWM_REG_SGPIO_CLK_DLY: c_uint = 0x002c;
pub const AIROHA_PWM_REG_SIPO_FLASH_MODE_CFG: c_uint = 0x0030;

// Register map is equal to GPIO flash map

// GPIO/SIPO flash map handles 8 pins in one register
pub const AIROHA_PWM_PINS_PER_FLASH_MAP: c_int = 8;
// Cycle(Period) registers handles 4 generators in one 32-bit register
pub const AIROHA_PWM_BUCKET_PER_CYCLE_CFG: c_int = 4;
// Flash(Duty) producer handles 2 generators in one 32-bit register
pub const AIROHA_PWM_BUCKET_PER_FLASH_PROD: c_int = 2;
pub const AIROHA_PWM_NUM_BUCKETS: c_int = 8;
//
...
//
pub const AIROHA_PWM_NUM_GPIO: c_int = 16;
pub const AIROHA_PWM_NUM_SIPO: c_int = 17;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct airoha_pwm_bucket {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct airoha_pwm {
    pub regmap: *mut regmap,
    pub AIROHA_PWM_MAX_CHANNELS): DECLARE_BITMAP(initialized,,
    pub buckets: [airoha_pwm_bucket; AIROHA_PWM_NUM_BUCKETS],
// Cache bucket used by each pwm channel
    pub channel_bucket: [u8; AIROHA_PWM_MAX_CHANNELS],
}

// The PWM hardware supports periods between 4 ms and 1 s

// It is represented internally as 1/250 s between 1 and 250. Unit is ticks.
pub const AIROHA_PWM_PERIOD_MIN: c_int = 1;
pub const AIROHA_PWM_PERIOD_MAX: c_int = 250;
// Duty cycle is relative with 255 corresponding to 100%
pub const AIROHA_PWM_DUTY_FULL: c_int = 255;
    static void airoha_pwm_get_flash_map_addr_and_shift(unsigned int hwpwm,
    u32 *addr, u32 *shift)
    {
    unsigned int offset, hwpwm_bit;
    if (hwpwm >= AIROHA_PWM_NUM_GPIO) {
    let mut sipohwpwm: c_uint = hwpwm - AIROHA_PWM_NUM_GPIO;
...
#[no_mangle]
unsafe extern "C" fn airoha_pwm_get_period_ticks_from_ns(period_ns: u32) -> u32 {
    static u32 airoha_pwm_get_period_ticks_from_ns(u32 period_ns)
    {
    return period_ns / AIROHA_PWM_PERIOD_TICK_NS;
    }
#[no_mangle]
unsafe extern "C" fn airoha_pwm_get_duty_ticks_from_ns(period_ns: u32, duty_ns: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn airoha_pwm_get_period_ns_from_ticks(period_tick: u32) -> u32 {
    static u32 airoha_pwm_get_period_ns_from_ticks(u32 period_tick)
    {
    return period_tick * AIROHA_PWM_PERIOD_TICK_NS;
    }
#[no_mangle]
unsafe extern "C" fn airoha_pwm_get_duty_ns_from_ticks(period_tick: u32, duty_tick: u32) -> u32 {
    static u32 airoha_pwm_get_duty_ns_from_ticks(u32 period_tick, u32 duty_tick)
    {
    let mut period_ns: u32 = period_tick * AIROHA_PWM_PERIOD_TICK_NS;
//
...
// duty_ns = airoha_pwm_get_duty_ns_from_ticks(period_tick, duty_tick);
    ...
    u32 period_ticks)
    {
    let mut best: c_int = -ENOENT, unused = -ENOENT;
    u32 duty_ns, best_duty_ns = 0;
    let mut best_period_ticks: u32 = 0;
    unsigned int i;
    duty_ns = airoha_pwm_get_duty_ns_from_ticks(period_ticks, duty_ticks);
    for (i = 0; i < ARRAY_SIZE(pc.buckets); i++) {
    struct airoha_pwm_bucket *bucket = &pc.buckets[i];
    let mut bucket_period_ticks: u32 = bucket.period_ticks;
    let mut bucket_duty_ticks: u32 = bucket.duty_ticks;
...
// Configure duty cycle
    ...
    unsigned int hwpwm)
    {
    let mut config_bucket: bool = false;
    int bucket, ret;
//
...
#[no_mangle]
unsafe extern "C" fn airoha_pwm_sipo_init(pc: *mut airoha_pwm) -> c_int {
...
//
    ...
    {
    let mut hwpwm: c_uint = pwm.hwpwm;
    int bucket, ret;
...
#[no_mangle]
unsafe extern "C" fn airoha_pwm_disable(pc: *mut airoha_pwm, pwm: *mut pwm_device) {
...
#[no_mangle]
unsafe extern "C" fn airoha_pwm_probe(pdev: *mut platform_device) -> c_int {
