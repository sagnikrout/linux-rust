...
//! Source: drivers/pwm/pwm-stm32.c
#![no_std]
...
pub const CCMR_CHANNEL_SHIFT: c_int = 8;
pub const CCMR_CHANNEL_MASK: c_uint = 0xFF;
pub const MAX_BREAKINPUT: c_int = 2;
pub const STM32_MAX_PWM_OUTPUT: c_int = 4;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_breakinput {
    pub index: u32,
    pub level: u32,
    pub filter: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_pwm {
    pub /: *mut *mut mutex lock; / protect pwm config/enable,
    pub clk: *mut clk,
    pub regmap: *mut regmap,
    pub max_arr: u32,
    pub have_complementary_output: bool,
    pub breakinputs: [stm32_breakinput; MAX_BREAKINPUT],
    pub num_breakinputs: c_uint,
    pub /: *mut *mut u32 capture[4] ____cacheline_aligned; / DMA'able buffer,
}
...
#[no_mangle]
unsafe extern "C" fn active_channels(dev: *mut stm32_pwm) -> u32 {
...
// raw_dty = 0;
#[no_mangle]
pub unsafe extern "C" fn if(priv->capture[3]: priv->capture[0] <=) -> else {
...
// raw_dty -= *raw_prd;
    ...
    unsigned long rate;
    let mut psc: c_uint = 0, icpsc, scale;
    let mut raw_prd: u32 = 0, raw_dty = 0;
    let mut ret: c_int = 0;
...
//
    ...
    .capture = IS_ENABLED(CONFIG_DMA_ENGINE) ? stm32_pwm_capture : core::ptr::null_mut(),
    };
    static int stm32_pwm_set_breakinput(struct stm32_pwm *priv,
    const struct stm32_breakinput *bi)
    {
    let mut shift: u32 = TIM_BDTR_BKF_SHIFT(bi.index);
    let mut bke: u32 = TIM_BDTR_BKE(bi.index);
    let mut bkp: u32 = TIM_BDTR_BKP(bi.index);
    let mut bkf: u32 = TIM_BDTR_BKF(bi.index);
    let mut mask: u32 = bkf | bkp | bke;
    u32 bdtr;
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_apply_breakinputs(priv: *mut stm32_pwm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_detect_complementary(priv: *mut stm32_pwm, ddata: *mut stm32_timers) {
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_pwm_resume(dev: *mut device) -> c_int {
