...
//! Source: drivers/soc/mediatek/mtk-regulator-coupler.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mediatek_regulator_coupler {
    pub coupler: regulator_coupler,
    pub vsram_rdev: *mut regulator_dev,
}
...
//
    ...
    let mut max_spread: c_int = rdev.constraints.max_spread[0];
    let mut vsram_min_uV: c_int = mrc.vsram_rdev.constraints.min_uV;
    let mut vsram_max_uV: c_int = mrc.vsram_rdev.constraints.max_uV;
    int vsram_target_min_uV, vsram_target_max_uV;
    let mut min_uV: c_int = 0;
    let mut max_uV: c_int = INT_MAX;
    int ret;
//
...
//
    ...
    struct mediatek_regulator_coupler *mrc = to_mediatek_coupler(coupler);
    if (rdev == mrc.vsram_rdev)
    mrc.vsram_rdev = core::ptr::null_mut();
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn mediatek_regulator_coupler_init() -> c_int {
