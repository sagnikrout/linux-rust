...
//
    ...
    {
    struct ssp_device *ssp = core::ptr::null_mut();
    mutex_lock(&ssp_lock);
...
#[no_mangle]
pub unsafe extern "C" fn pxa_ssp_free(ssp: *mut ssp_device) {
    void pxa_ssp_free(struct ssp_device *ssp)
    {
    mutex_lock(&ssp_lock);
    if (ssp.use_count) {
    ssp.use_count--;
    ssp.label = core::ptr::null_mut();
    } else
...
#[no_mangle]
unsafe extern "C" fn pxa_ssp_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (ssp.mmio_base == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn pxa_ssp_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn pxa_ssp_init() -> int __init {
    static int __init pxa_ssp_init(void)
    {
    return platform_driver_register(&pxa_ssp_driver);
    }
#[no_mangle]
unsafe extern "C" fn pxa_ssp_exit() -> void __exit {
