...
//! Source: drivers/soc/xilinx/xlnx_event_manager.c
#![no_std]
...
//
    ...
    static int virq_sgi;
    let mut event_manager_availability: static int = -EACCES;
...
// Max HashMap Order for PM API feature check (1<<7 = 128)
    ...
    let mut sgi_num: static int = XLNX_EVENT_SGI_NUM;
...
#[derive(Copy, Clone)]
pub struct agent_cb {
    pub agent_data: *mut c_void,
    pub eve_cb: event_cb_func_t,
    pub list: list_head,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct registered_event_data {
    pub key: u64,
    pub cb_type: enum pm_api_cb_id,
    pub wake: bool,
    pub cb_list_head: list_head,
    pub hentry: hlist_node,
}

#[no_mangle]
unsafe extern "C" fn xlnx_is_error_event(node_id: u32) -> bool {
    ...
    {
    let mut key: u64 = 0;
    let mut present_in_hash: bool = false;
...
#[no_mangle]
unsafe extern "C" fn xlnx_add_cb_for_suspend(cb_fun: event_cb_func_t, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xlnx_remove_cb_for_suspend(cb_fun: event_cb_func_t) -> c_int {
    static int xlnx_remove_cb_for_suspend(event_cb_func_t cb_fun)
    {
    let mut is_callback_found: bool = false;
    struct registered_event_data *eve_data;
    struct agent_cb *cb_pos;
    struct agent_cb *cb_next;
...
// remove an object from a hashtable
    ...
    let mut is_callback_found: bool = false;
    struct registered_event_data *eve_data;
    let mut key: u64 = ((u64)node_id << 32U) | (u64)event;
    struct agent_cb *cb_pos;
    struct agent_cb *cb_next;
    struct hlist_node *tmp;
    is_need_to_unregister = false;
...
#[no_mangle]
unsafe extern "C" fn xlnx_call_suspend_cb_handler(payload: *const u32) {
    static void xlnx_call_suspend_cb_handler(const u32 *payload)
    {
    let mut is_callback_found: bool = false;
    struct registered_event_data *eve_data;
    let mut cb_type: u32 = payload[0];
    struct agent_cb *cb_pos;
    struct agent_cb *cb_next;
// Check for existing entry in hash table for given cb_type
...
#[no_mangle]
unsafe extern "C" fn xlnx_call_notify_cb_handler(payload: *const u32) {
    static void xlnx_call_notify_cb_handler(const u32 *payload)
    {
    let mut is_callback_found: bool = false;
    struct registered_event_data *eve_data;
    let mut key: u64 = ((u64)payload[1] << 32U) | (u64)payload[2];
    int ret;
    struct agent_cb *cb_pos;
    struct agent_cb *cb_next;
// Check for existing entry in hash table for given key id
...
#[no_mangle]
unsafe extern "C" fn xlnx_get_event_callback_data(buf: *mut u32) {
    static void xlnx_get_event_callback_data(u32 *buf)
    {
    zynqmp_pm_invoke_fn(GET_CALLBACK_DATA, buf, 0);
    }
#[no_mangle]
unsafe extern "C" fn xlnx_event_handler(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn xlnx_event_cpuhp_start(cpu: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xlnx_event_cpuhp_down(cpu: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xlnx_event_init_sgi(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xlnx_event_cleanup_sgi(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn xlnx_event_manager_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xlnx_event_manager_remove(pdev: *mut platform_device) {
