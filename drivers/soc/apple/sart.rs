...
//! Source: drivers/soc/apple/sart.c
#![no_std]
...
pub const APPLE_SART_MAX_ENTRIES: c_int = 16;
// SARTv0 registers

pub const APPLE_SART0_CONFIG_SIZE_SHIFT: c_int = 12;

pub const APPLE_SART0_PADDR_SHIFT: c_int = 12;
pub const APPLE_SART0_FLAGS_ALLOW: c_uint = 0xf;
// SARTv2 registers

pub const APPLE_SART2_CONFIG_SIZE_SHIFT: c_int = 12;

pub const APPLE_SART2_PADDR_SHIFT: c_int = 12;
pub const APPLE_SART2_FLAGS_ALLOW: c_uint = 0xff;
// SARTv3 registers

pub const APPLE_SART3_PADDR_SHIFT: c_int = 12;

pub const APPLE_SART3_SIZE_SHIFT: c_int = 12;

pub const APPLE_SART3_FLAGS_ALLOW: c_uint = 0xff;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct apple_sart_ops {
    void (*get_entry)(struct apple_sart *sart, int index, u8 *flags,
    pub size): *mut *mut phys_addr_t paddr, size_t,
    void (*set_entry)(struct apple_sart *sart, int index, u8 flags,
    pub size_shifted): phys_addr_t paddr_shifted, size_t,
// This is probably a bitfield but the exact meaning of each bit is unknown.
    pub flags_allow: c_uint,
    pub size_shift: c_uint,
    pub paddr_shift: c_uint,
    pub size_max: usize,
}
...
pub struct apple_sart {
    ...
    pub ops: *const apple_sart_ops,
    pub protected_entries: c_ulong,
    pub used_entries: c_ulong,
}

    static void sart0_get_entry(struct apple_sart *sart, int index, u8 *flags,
    phys_addr_t *paddr, size_t *size)
    {
    let mut cfg: u32 = readl(sart.regs + APPLE_SART0_CONFIG(index));
    let mut paddr_: phys_addr_t = readl(sart.regs + APPLE_SART0_PADDR(index));
    let mut size_: usize = FIELD_GET(APPLE_SART0_CONFIG_SIZE, cfg);
...
// paddr = paddr_ << APPLE_SART0_PADDR_SHIFT;
    ...
    phys_addr_t *paddr, size_t *size)
    {
    let mut cfg: u32 = readl(sart.regs + APPLE_SART2_CONFIG(index));
    let mut paddr_: phys_addr_t = readl(sart.regs + APPLE_SART2_PADDR(index));
    let mut size_: usize = FIELD_GET(APPLE_SART2_CONFIG_SIZE, cfg);
...
// paddr = paddr_ << APPLE_SART2_PADDR_SHIFT;
    ...
    phys_addr_t *paddr, size_t *size)
    {
    let mut paddr_: phys_addr_t = readl(sart.regs + APPLE_SART3_PADDR(index));
    let mut size_: usize = readl(sart.regs + APPLE_SART3_SIZE(index));
...
#[no_mangle]
unsafe extern "C" fn apple_sart_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn apple_sart_shutdown(pdev: *mut platform_device) {
