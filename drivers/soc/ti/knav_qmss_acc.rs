...
//! Source: drivers/soc/ti/knav_qmss_acc.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn knav_acc_int_handler(irq: c_int, _instdata: *mut c_void) -> irqreturn_t {
    static irqreturn_t knav_acc_int_handler(int irq, void *_instdata)
    {
    struct knav_acc_channel *acc;
    struct knav_queue_inst *kq = core::ptr::null_mut();
...
// ack the interrupt
    ...
    acc.name, acc.name);
    ret = irq_set_affinity_hint(irq, core::ptr::null_mut());
    if (ret)
    dev_warn(range.kdev.dev,
    "Failed to set IRQ affinity\n");
    free_irq(irq, range);
...
#[no_mangle]
unsafe extern "C" fn knav_acc_init_range(range: *mut knav_range_info) -> c_int {
    ...
    struct knav_queue_inst *kq)
    {
    let mut id: unsigned = kq.id - range.queue_base;
    ...
    {
    let mut id: unsigned = inst.id - range.queue_base;
...
#[no_mangle]
unsafe extern "C" fn knav_acc_free_range(range: *mut knav_range_info) -> c_int {
