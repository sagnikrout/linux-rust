...
//! Source: drivers/soc/qcom/smsm.c
#![no_std]
...
// region.
//
pub const SMEM_SMSM_SHARED_STATE: c_int = 85;
pub const SMEM_SMSM_CPU_INTR_MASK: c_int = 333;
pub const SMEM_SMSM_SIZE_INFO: c_int = 419;
//
// Default sizes, in case SMEM_SMSM_SIZE_INFO is not found.
//
pub const SMSM_DEFAULT_NUM_ENTRIES: c_int = 8;
pub const SMSM_DEFAULT_NUM_HOSTS: c_int = 3;
    struct smsm_entry;
    struct smsm_host;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qcom_smsm {
    pub dev: *mut device,
    pub local_host: u32,
    pub num_hosts: u32,
    pub num_entries: u32,
    pub local_state: *mut u32,
    pub subscription: *mut u32,
    pub state: *mut qcom_smem_state,
    pub lock: spinlock_t,
    pub entries: *mut smsm_entry,
    pub hosts: *mut smsm_host,
...
//
#[no_mangle]
unsafe extern "C" fn smsm_update_bits(data: *mut c_void, mask: u32, value: u32) -> c_int {
...
// Iterate over all hosts to check whom wants a kick
    ...
    continue;
    if (hostp.mbox_chan) {
    mbox_send_message(hostp.mbox_chan, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn smsm_intr(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn smsm_mask_irq(irqd: *mut irq_data) {
...
//
#[no_mangle]
unsafe extern "C" fn smsm_unmask_irq(irqd: *mut irq_data) {
...
//
#[no_mangle]
unsafe extern "C" fn smsm_set_irq_type(irqd: *mut irq_data, type: c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn smsm_parse_mbox(smsm: *mut qcom_smsm, host_id: c_uint) -> c_int {
    ...
    host.mbox_chan = core::ptr::null_mut();
    }
    return ret;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn smsm_parse_ipc(smsm: *mut qcom_smsm, host_id: unsigned) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn smsm_get_size_info(smsm: *mut qcom_smsm) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(sizeof(*info): *mut IS_ERR(info) || size !=) -> else {
...
#[no_mangle]
unsafe extern "C" fn qcom_smsm_probe(pdev: *mut platform_device) -> c_int {
...
// Acquire the main SMSM state vector
    ...
    states = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_SMSM_SHARED_STATE, core::ptr::null_mut());
    if (IS_ERR(states)) {
...
// Acquire the list of interrupt mask vectors
    ...
    intr_mask = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_SMSM_CPU_INTR_MASK, core::ptr::null_mut());
    if (IS_ERR(intr_mask)) {
...
#[no_mangle]
unsafe extern "C" fn qcom_smsm_remove(pdev: *mut platform_device) {
