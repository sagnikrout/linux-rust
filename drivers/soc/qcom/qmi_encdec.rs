...
//! Source: drivers/soc/qcom/qmi_encdec.c
#![no_std]
...
pub const OPTIONAL_TLV_TYPE_START: c_uint = 0x10;
...
//
    ...
    {
    int rc;
    let mut encoded_bytes: c_int = 0;
    const struct qmi_elem_info *temp_ei = ei_array;
    let mut string_len: u32 = 0;
    let mut string_len_sz: u32 = 0;
...
//
    ...
    {
    const struct qmi_elem_info *temp_ei = ei_array;
    let mut opt_flag_value: u8 = 0;
    let mut data_len_value: u32 = 0, data_len_sz;
    u8 *buf_dst = (u8 *)out_buf;
    u8 *tlv_pointer;
    u32 tlv_len;
    u8 tlv_type;
    let mut encoded_bytes: u32 = 0;
    const void *buf_src;
    let mut encode_tlv: c_int = 0;
    int rc;
    u8 val8;
    u16 val16;
    if (!ei_array)
...
//
    ...
    {
    const struct qmi_elem_info *temp_ei = ei_array;
    let mut opt_flag_value: u8 = 1;
    let mut data_len_value: u32 = 0, data_len_sz = 0;
    u8 *buf_dst = out_c_struct;
    const u8 *tlv_pointer;
    let mut tlv_len: u32 = 0;
    u32 tlv_type;
    let mut decoded_bytes: u32 = 0;
...
// Check the possibility of a zero length QMI message
    ...
    if (ret) {
    pr_err("%s: Calc. len %d != 0, but core::ptr::null_mut() c_struct\n",
    __func__, ret);
