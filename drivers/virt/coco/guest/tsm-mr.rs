...
//! Source: drivers/virt/coco/guest/tsm-mr.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tm_context {
    pub rwsem: rw_semaphore,
    pub agrp: attribute_group,
    pub tm: *const tsm_measurements,
    pub in_sync: bool,
    pub mrs: [bin_attribute; ],
}
...
#[no_mangle]
pub unsafe extern "C" fn if(end: name <) -> else {
    bap.attr.name = name;
...
//
#[no_mangle]
pub unsafe extern "C" fn tsm_mr_free_attribute_group(attr_grp: *const attribute_group) {
