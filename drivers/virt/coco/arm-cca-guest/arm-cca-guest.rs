...
//! Source: drivers/virt/coco/arm-cca-guest/arm-cca-guest.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct arm_cca_token_info {
    pub granule: phys_addr_t,
    pub offset: c_ulong,
}
...
//
#[no_mangle]
unsafe extern "C" fn arm_cca_report_new(report: *mut tsm_report, data: *mut c_void) -> c_int {
    static int arm_cca_report_new(struct tsm_report *report, void *data)
    {
    let mut ret: c_int = 0;
    unsigned long rsi_result;
    long max_size;
    let mut token_size: c_ulong = 0;
    struct arm_cca_token_info info;
    void *buf;
    u8 *token __free(kvfree) = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn arm_cca_guest_init() -> int __init {
    static int __init arm_cca_guest_init(void)
    {
    int ret;
    if (!is_realm_world())
    return -ENODEV;
    ret = tsm_report_register(&arm_cca_tsm_ops, core::ptr::null_mut());
    if (ret < 0)
...
//
#[no_mangle]
unsafe extern "C" fn arm_cca_guest_exit() -> void __exit {
