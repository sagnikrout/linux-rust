...
//! Source: drivers/virt/coco/sev-guest/sev-guest.c
#![no_std]
...
pub const SVSM_MAX_RETRIES: c_int = 3;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct snp_guest_dev {
    pub dev: *mut device,
    pub misc: miscdevice,
    pub msg_desc: *mut snp_msg_desc,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct snp_req_resp {
    pub req_data: sockptr_t,
    pub resp_data: sockptr_t,
}

#[no_mangle]
unsafe extern "C" fn get_report(snp_dev: *mut snp_guest_dev, arg: *mut snp_guest_request_ioctl) -> c_int {
    static int get_report(struct snp_guest_dev *snp_dev, struct snp_guest_request_ioctl *arg)
    {
    struct snp_report_req *report_req __free(kfree) = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn get_derived_key(snp_dev: *mut snp_guest_dev, arg: *mut snp_guest_request_ioctl) -> c_int {
    static int get_derived_key(struct snp_guest_dev *snp_dev, struct snp_guest_request_ioctl *arg)
    {
    struct snp_derived_key_resp *derived_key_resp __free(kfree) = core::ptr::null_mut();
    struct snp_derived_key_req *derived_key_req __free(kfree) = core::ptr::null_mut();
    struct snp_msg_desc *mdesc = snp_dev.msg_desc;
    let mut req: snp_guest_req = {};
    int rc, resp_len;
    if (!arg.req_data || !arg.resp_data)
...
// The response buffer contains the sensitive data, explicitly clear it.
    ...
    {
    struct snp_ext_report_req *report_req __free(kfree) = core::ptr::null_mut();
    struct snp_msg_desc *mdesc = snp_dev.msg_desc;
    struct snp_report_resp *report_resp;
    let mut req: snp_guest_req = {};
...
#[no_mangle]
unsafe extern "C" fn snp_guest_ioctl(file: *mut file, ioctl: c_uint, arg: c_ulong) -> c_long {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct snp_msg_report_resp_hdr {
    pub status: u32,
    pub report_size: u32,
    pub rsvd: [u8; 24],
}
...
#[no_mangle]
unsafe extern "C" fn sev_svsm_report_new(report: *mut tsm_report, data: *mut c_void) -> c_int {
    static int sev_svsm_report_new(struct tsm_report *report, void *data)
    {
    unsigned int rep_len, man_len, certs_len;
    struct tsm_report_desc *desc = &report.desc;
    let mut ac: svsm_attest_call = {};
...
//
    ...
    void *cbuf __free(kvfree) = certs_len ? kvzalloc(certs_len, GFP_KERNEL) : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sev_report_new(report: *mut tsm_report, data: *mut c_void) -> c_int {
    ...
    let mut report_size: u32 = SZ_4K;
    let mut ext_size: u32 = SEV_FW_BLOB_MAX_SIZE;
...
#[no_mangle]
unsafe extern "C" fn sev_report_attr_visible(n: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn sev_report_bin_attr_visible(n: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn unregister_sev_tsm(data: *mut c_void) {
    static void unregister_sev_tsm(void *data)
    {
    tsm_report_unregister(&sev_tsm_report_ops);
    }
#[no_mangle]
unsafe extern "C" fn sev_guest_probe(pdev: *mut platform_device) -> int __init {
...
// Set the privlevel_floor attribute based on the vmpck_id
    ...
    if (ret)
    goto e_msg_init;
    ret = devm_add_action_or_reset(&pdev.dev, unregister_sev_tsm, core::ptr::null_mut());
    if (ret)
    goto e_msg_init;
...
#[no_mangle]
unsafe extern "C" fn sev_guest_remove(pdev: *mut platform_device) -> void __exit {
