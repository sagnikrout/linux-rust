...
//! Source: drivers/rapidio/rio-sysfs.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn routes_show(dev: *mut device, attr: *mut device_attribute, buf: *mut c_char) -> isize {
...
// Several chips lock up trying to read undefined config space
    ...
    if ((off & 3) && (size > 2)) {
    let mut val: u16 = data[off - init_off + 1];
    ...
    while (size > 3) {
    let mut val: u32 = data[off - init_off + 3];
...
#[no_mangle]
unsafe extern "C" fn scan_store(bus: *const bus_type, buf: *const c_char, count: usize) -> isize {
