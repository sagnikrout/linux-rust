...
//! Source: drivers/parport/probe.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn pretty_print(port: *mut parport, device: c_int) {
...
#[no_mangle]
unsafe extern "C" fn parse_data(port: *mut parport, device: c_int, str: *mut c_char) {
    static void parse_data(struct parport *port, int device, char *str)
    {
    char *txt = kmalloc(strlen(str)+1, GFP_KERNEL);
    char *p = txt, *q;
    let mut guessed_class: c_int = PARPORT_CLASS_UNSPEC;
    struct parport_device_info *info = &port.probe_info[device + 1];
    if (!txt) {
...
// first.
    ...
    let mut idlen: usize = idlens[current_idlen];
    if (idlen+1 >= count)
    break;
...
#[no_mangle]
pub unsafe extern "C" fn parport_device_id(devnum: c_int, buffer: *mut c_char, count: usize) -> isize {
    ssize_t parport_device_id (int devnum, char *buffer, size_t count)
    {
    let mut retval: isize = -ENXIO;
