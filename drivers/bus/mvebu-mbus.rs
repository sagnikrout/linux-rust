...
//! Source: drivers/bus/mvebu-mbus.c
#![no_std]
...
//
pub const TARGET_DDR: c_int = 0;
//
// CPU Address Decode Windows registers
//
pub const WIN_CTRL_OFF: c_uint = 0x0000;

// Only on HW I/O coherency capable platforms

pub const WIN_CTRL_TGT_MASK: c_uint = 0xf0;
pub const WIN_CTRL_TGT_SHIFT: c_int = 4;
pub const WIN_CTRL_ATTR_MASK: c_uint = 0xff00;
pub const WIN_CTRL_ATTR_SHIFT: c_int = 8;
pub const WIN_CTRL_SIZE_MASK: c_uint = 0xffff0000;
pub const WIN_CTRL_SIZE_SHIFT: c_int = 16;
pub const WIN_BASE_OFF: c_uint = 0x0004;
pub const WIN_BASE_LOW: c_uint = 0xffff0000;
pub const WIN_BASE_HIGH: c_uint = 0xf;
pub const WIN_REMAP_LO_OFF: c_uint = 0x0008;
pub const WIN_REMAP_LOW: c_uint = 0xffff0000;
pub const WIN_REMAP_HI_OFF: c_uint = 0x000c;
pub const UNIT_SYNC_BARRIER_OFF: c_uint = 0x84;
pub const UNIT_SYNC_BARRIER_ALL: c_uint = 0xFFFF;

pub const DDR_BASE_CS_HIGH_MASK: c_uint = 0xf;
pub const DDR_BASE_CS_LOW_MASK: c_uint = 0xff000000;

pub const DDR_SIZE_CS_MASK: c_uint = 0x1c;
pub const DDR_SIZE_CS_SHIFT: c_int = 2;
pub const DDR_SIZE_MASK: c_uint = 0xff000000;

// Relative to mbusbridge_base
pub const MBUS_BRIDGE_CTRL_OFF: c_uint = 0x0;
pub const MBUS_BRIDGE_BASE_OFF: c_uint = 0x4;
// Maximum number of windows, for all known platforms
pub const MBUS_WINS_MAX: c_int = 20;
    struct mvebu_mbus_state;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_mbus_soc_data {
    pub num_wins: c_uint,
    pub has_mbus_bridge: bool,
    pub win): *const *const unsigned int (win_cfg_offset)(int,
    pub win): *const *const unsigned int (win_remap_offset)(int,
    pub s): *mut *mut void (setup_cpu_target)(struct mvebu_mbus_state,
    int (*save_cpu_target)(struct mvebu_mbus_state *s,
    pub store_addr): *mut u32 __iomem,
    int (*show_cpu_target)(struct mvebu_mbus_state *s,
    pub v): *mut *mut seq_file seq, void,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_mbus_win_data {
    pub ctrl: u32,
    pub base: u32,
    pub remap_lo: u32,
    pub remap_hi: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_mbus_state {
    pub mbuswins_base: *mut void __iomem,
    pub sdramwins_base: *mut void __iomem,
    pub mbusbridge_base: *mut void __iomem,
    pub sdramwins_phys_base: phys_addr_t,
    pub debugfs_root: *mut dentry,
    pub debugfs_sdram: *mut dentry,
    pub debugfs_devs: *mut dentry,
    pub pcie_mem_aperture: resource,
    pub pcie_io_aperture: resource,
    pub soc: *const mvebu_mbus_soc_data,
    pub hw_io_coherency: c_int,
// Used during suspend/resume
    pub mbus_bridge_ctrl: u32,
    pub mbus_bridge_base: u32,
    pub wins: [mvebu_mbus_win_data; MBUS_WINS_MAX],
}
...
//
    ...
    let mut basereg: u32 = readl(addr + WIN_BASE_OFF);
    let mut ctrlreg: u32 = readl(addr + WIN_CTRL_OFF);
...
// Checks whether the given window number is available
    ...
    void __iomem *addr = mbus.mbuswins_base +
    mbus.soc.win_cfg_offset(win);
    let mut ctrl: u32 = readl(addr + WIN_CTRL_OFF);
...
//
    ...
    &enabled, &wbase, &wsize,
    &wtarget, &wattr, core::ptr::null_mut());
    if (!enabled)
    continue;
    wend = wbase + wsize;
//
...
// Common function used for Dove, Kirkwood, Armada 370/XP and Orion 5x
    ...
    int i;
    for (i = 0; i < 4; i++) {
    let mut basereg: u32 = readl(mbus.sdramwins_base + DDR_BASE_CS_OFF(i));
    let mut sizereg: u32 = readl(mbus.sdramwins_base + DDR_SIZE_CS_OFF(i));
    u64 base;
    u32 size;
...
// Special function for Dove
    ...
    int i;
    for (i = 0; i < 2; i++) {
    let mut map: u32 = readl(mbus.sdramwins_base + DOVE_DDR_BASE_CS_OFF(i));
    u64 base;
    u32 size;
    if (!(map & 1)) {
    seq_printf(seq, "[%d] disabled\n", i);
...
#[no_mangle]
unsafe extern "C" fn mvebu_sdram_debug_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mvebu_devs_debug_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn generic_mbus_win_cfg_offset(win: c_int) -> c_uint {
    static unsigned int generic_mbus_win_cfg_offset(int win)
    {
    return win << 4;
    }
#[no_mangle]
unsafe extern "C" fn armada_370_xp_mbus_win_cfg_offset(win: c_int) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn mv78xx0_mbus_win_cfg_offset(win: c_int) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn generic_mbus_win_remap_2_offset(win: c_int) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn generic_mbus_win_remap_4_offset(win: c_int) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn generic_mbus_win_remap_8_offset(win: c_int) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn armada_xp_mbus_win_remap_offset(win: c_int) -> c_uint {
    static unsigned int armada_xp_mbus_win_remap_offset(int win)
    {
    if (win < 8)
    return generic_mbus_win_cfg_offset(win);
#[no_mangle]
pub unsafe extern "C" fn if(13: win ==) -> else {
    else if (win == 13)
...
//
    ...
    for (i = 0, cs = 0; i < 4; i++) {
    let mut base: u32 = readl(mbus.sdramwins_base + DDR_BASE_CS_OFF(i));
    let mut size: u32 = readl(mbus.sdramwins_base + DDR_SIZE_CS_OFF(i));
//
...
#[no_mangle]
pub unsafe extern "C" fn mvebu_mbus_save_cpu_target(store_addr: *mut u32 __iomem) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mvebu_mbus_del_window(base: phys_addr_t, size: usize) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mvebu_mbus_get_pcie_mem_aperture(res: *mut resource) {
...
#[no_mangle]
pub unsafe extern "C" fn mvebu_mbus_get_pcie_io_aperture(res: *mut resource) {
...
#[no_mangle]
pub unsafe extern "C" fn mvebu_mbus_get_dram_win_info(phyaddr: phys_addr_t, target: *mut u8, attr: *mut u8) -> c_int {
...
// attr = cs->mbus_attr;
    ...
    size, target, attr, core::ptr::null_mut());
    if (!enabled)
    continue;
...
#[no_mangle]
unsafe extern "C" fn mvebu_mbus_debugfs_init() -> __init int {
...
//
    if (!s.mbuswins_base)
    return 0;
    s.debugfs_root = debugfs_create_dir("mvebu-mbus", core::ptr::null_mut());
    if (s.debugfs_root) {
    s.debugfs_sdram = debugfs_create_file("sdram", S_IRUGO,
    s.debugfs_root, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn mvebu_mbus_suspend(data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mvebu_mbus_resume(data: *mut c_void) {
    ...
    mbus.mbusbridge_base = core::ptr::null_mut();
    for (win = 0; win < mbus.soc.num_wins; win++)
...
//
    ...
    let mut windowid: u32 = upper_32_bits(range.bus_addr);
    u8 target, attr;
//
...
#[no_mangle]
pub unsafe extern "C" fn mvebu_mbus_dt_init(is_coherent: bool) -> int __init {
    ...
    const __be32 *prop;
    int ret;
    np = of_find_matching_node_and_match(core::ptr::null_mut(), of_mvebu_mbus_ids, &of_id);
    if (!np) {
    pr_err("could not find a matching SoC family\n");
    return -ENODEV;
    }
    mbus_state.soc = of_id.data;
    prop = of_get_property(np, "controller", core::ptr::null_mut());
    if (!prop) {
