...
// such as BT.
//
pub const FG_INCRADDR_LPC: c_uint = 0x02;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lpc_cycle_para {
    pub opflags: c_uint,
    pub /: *mut *mut unsigned int csize; / data length of each operation,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct hisi_lpc_dev {
    pub cycle_lock: spinlock_t,
    pub membase: *mut void __iomem,
    pub io_host: *mut logic_pio_hwaddr,
}

// The max IO cycle counts supported is four per operation at maximum
pub const LPC_MAX_DWIDTH: c_int = 4;
pub const LPC_REG_STARTUP_SIGNAL: c_uint = 0x00;

pub const LPC_REG_OP_STATUS: c_uint = 0x04;

pub const LPC_REG_OP_LEN: c_uint = 0x10 /* LPC cycles count per start */;
pub const LPC_REG_CMD: c_uint = 0x14;

pub const LPC_REG_ADDR: c_uint = 0x20 /* target address */;
pub const LPC_REG_WDATA: c_uint = 0x24 /* write FIFO */;
pub const LPC_REG_RDATA: c_uint = 0x28 /* read FIFO */;
// The minimal nanosecond interval for each query on LPC cycle status
pub const LPC_NSEC_PERWAIT: c_int = 100;
//
...
//
pub const LPC_MAX_WAITCNT: c_int = 1300;
// About 10us. This is specific for single IO operations, such as inb
pub const LPC_PEROP_WAITCNT: c_int = 100;
#[no_mangle]
unsafe extern "C" fn wait_lpc_idle(mbase: *mut void __iomem, waitcnt: c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hisi_lpc_comm_in(hostdata: *mut c_void, pio: c_ulong, dwidth: usize) -> u32 {
    static u32 hisi_lpc_comm_in(void *hostdata, unsigned long pio, size_t dwidth)
    {
    struct hisi_lpc_dev *lpcdev = hostdata;
    struct lpc_cycle_para iopara;
    unsigned long addr;
    let mut rd_data: __le32 = 0;
    int ret;
...
//
    ...
    unsigned long addr;
    let mut _val: __le32 = cpu_to_le32(val);
    if (!lpcdev || !dwidth || dwidth > LPC_MAX_DWIDTH)
    return;
...
//
    ...
    let mut len: resource_size_t = resource_size(res);
...
//
    ...
    r.end = 0xe4 + 0x04 - 1;
#[no_mangle]
pub unsafe extern "C" fn if(0x2f8: r->start ==) -> else {
...
// in future.
//
    count = acpi_dev_get_resources(adev, &resource_list, core::ptr::null_mut(), core::ptr::null_mut());
    if (count <= 0) {
...
#[no_mangle]
unsafe extern "C" fn hisi_lpc_acpi_remove_subdev(dev: *mut device, unused: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_lpc_acpi_clear_enumerated(adev: *mut acpi_device, not_used: *mut c_void) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hisi_lpc_acpi_cell {
    pub hid: *const c_char,
    pub pdevinfo: *const platform_device_info,
}

#[no_mangle]
unsafe extern "C" fn hisi_lpc_acpi_remove(hostdev: *mut device) {
    static void hisi_lpc_acpi_remove(struct device *hostdev)
    {
    device_for_each_child(hostdev, core::ptr::null_mut(), hisi_lpc_acpi_remove_subdev);
    acpi_dev_for_each_child(ACPI_COMPANION(hostdev),
    hisi_lpc_acpi_clear_enumerated, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn hisi_lpc_acpi_add_child(child: *mut acpi_device, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hisi_lpc_acpi_probe(hostdev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_lpc_acpi_probe(dev: *mut device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hisi_lpc_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_lpc_remove(pdev: *mut platform_device) {
