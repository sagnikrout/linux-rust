...
#[no_mangle]
unsafe extern "C" fn sd_zbc_is_gap_zone(buf[64]: u8) -> bool {
...
//
    ...
    {
    struct scsi_device *sdp = sdkp.device;
    let mut timeout: c_int = sdp.request_queue.rq_timeout;
...
//
#[no_mangle]
pub unsafe extern "C" fn sd_zbc_zone_sectors(sdkp: *mut scsi_disk) -> sector_t {
...
//
    ...
    let mut lba: sector_t = sectors_to_logical(sdkp.device, sector);
    unsigned int nr, i;
    unsigned char *buf;
    u64 zone_length, start_lba;
    size_t offset, buflen = 0;
    let mut zone_idx: c_int = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn sd_zbc_cmnd_checks(cmd: *mut scsi_cmnd) -> blk_status_t {
...
//
    ...
    let mut sector: sector_t = blk_rq_pos(rq);
    struct scsi_disk *sdkp = scsi_disk(rq.q.disk);
    let mut block: sector_t = sectors_to_logical(sdkp.device, sector);
    blk_status_t ret;
...
#[no_mangle]
unsafe extern "C" fn sd_zbc_print_zones(sdkp: *mut scsi_disk) {
...
//
#[no_mangle]
pub unsafe extern "C" fn sd_zbc_revalidate_zones(sdkp: *mut scsi_disk) -> c_int {
    int sd_zbc_revalidate_zones(struct scsi_disk *sdkp)
    {
    struct gendisk *disk = sdkp.disk;
    struct request_queue *q = disk.queue;
    let mut zone_blocks: u32 = sdkp.early_zone_info.zone_blocks;
    let mut nr_zones: c_uint = sdkp.early_zone_info.nr_zones;
    unsigned int flags;
    int ret;
//
