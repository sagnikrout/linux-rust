...
//! Source: drivers/scsi/snic/snic_main.c
#![no_std]
...
pub const PCI_DEVICE_ID_CISCO_SNIC: c_uint = 0x0046;
// Supported devices by snic module
    ...
    let mut snic_max_qdepth: c_uint = SNIC_DFLT_QUEUE_DEPTH;
...
//
    ...
    let mut qdepth: u32 = 0, max_ios = 0;
    let mut tmo: c_int = SNIC_DFLT_CMD_TIMEOUT * HZ;
// Set Queue Depth
...
#[no_mangle]
pub unsafe extern "C" fn if(sdev->queue_depth: qsz >) -> else {
...
//
    ...
    SNIC_BUG_ON(shost.work_q != core::ptr::null_mut());
    ...
    shost.work_q = core::ptr::null_mut();
    scsi_remove_host(shost);
    }
    int
...
// Devcmd2 Resource Allocation and Initialization
    snic.vdev = svnic_dev_alloc_discover(core::ptr::null_mut(), snic, pdev, &snic.bar0, 1);
    if (!snic.vdev) {
...
//
    ...
    {
    SNIC_BUG_ON(snic_glob == core::ptr::null_mut());
