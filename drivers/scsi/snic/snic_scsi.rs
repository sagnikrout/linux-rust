...
//! Source: drivers/scsi/snic/snic_scsi.c
#![no_std]
...
// snic cmd status strings
    ...
    {
    let mut hash: u32 = snic_cmd_tag(sc) & (SNIC_IO_LOCKS - 1);
...
//
    ...
    let mut sg_cnt: c_int = 0;
    let mut ret: c_int = 0;
    let mut tag: u32 = snic_cmd_tag(sc);
    let mut cmd_trc: u64 = 0, cmd_st_flags = 0;
    spinlock_t *io_lock = core::ptr::null_mut();
    unsigned long flags;
...
// create wq desc and enqueue it
    ...
    let mut io_sz: u32 = scsi_bufflen(sc) >> 9;
    let mut qtime: u32 = jiffies - rqi.start_time;
...
//
    ...
    struct scsi_cmnd *sc)
    {
    struct snic_tgt *tgt = core::ptr::null_mut();
...
//
    ...
    u8 cmpl_status)
    {
    let mut state: c_int = CMD_STATE(sc);
    if (state == SNIC_IOREQ_ABTS_PENDING)
    CMD_FLAGS(sc) |= SNIC_IO_ABTS_PENDING;
#[no_mangle]
pub unsafe extern "C" fn if(SNIC_IOREQ_LR_PENDING: state ==) -> else {
...
//
    ...
    struct scsi_cmnd *sc)
    {
    let mut scsi_stat: u8 = icmnd_cmpl.scsi_status;
    let mut ret: c_int = 0;
...
//
    ...
    SNIC_HOST_ERR(snic.shost,
    "Icmnd_cmpl: Scsi Cmnd Not found, sc = core::ptr::null_mut() Hdr Status = %s tag = 0x%x fwreq = 0x%p\n",
...
// For now, consider only successful IO.
    ...
    let mut start_time: u32 = rqi.start_time;
...
//
    ...
    struct scsi_cmnd  *sc = core::ptr::null_mut();
    struct snic_req_info *rqi = core::ptr::null_mut();
    struct snic_itmf_cmpl *itmf_cmpl = core::ptr::null_mut();
    ulong ctx;
    u32 cmnd_id;
    u32 hid;
    u8 typ;
...
// spl case, dev reset issued through ioctl
    ...
    atomic64_inc(&snic.s_stats.io.sc_null);
    SNIC_HOST_ERR(snic.shost,
    "Itmf_cmpl: sc is core::ptr::null_mut() - Hdr Stat %s Tag 0x%x\n",
    ...
    let mut act_ios: c_long = 0, act_fwreqs = 0;
...
// spl case, host reset issued through ioctl
    ...
    atomic64_inc(&snic.s_stats.io.sc_null);
    SNIC_HOST_ERR(snic.shost,
    "reset_cmpl: sc is core::ptr::null_mut() - Hdr Stat %s Tag 0x%x\n",
    snic_io_status_to_str(hdr_stat), cmnd_id);
    ret = 1;
    return ret;
    }
...
//
    ...
    let mut start: u64 = jiffies, cmpl_time;
...
//
    ...
    struct scsi_lun lun;
    let mut tm_tag: c_int = snic_cmd_tag(sc) | rqi.tm_tag;
    let mut ret: c_int = 0;
    SNIC_BUG_ON(!rqi);
...
//
    ...
    int tmf)
    {
    struct snic_host_req *tmreq = core::ptr::null_mut();
    let mut req_id: c_int = 0, tag = snic_cmd_tag(sc);
    let mut ret: c_int = 0;
    if (snic_get_state(snic) == SNIC_FWRESET)
    return -EBUSY;
    atomic_inc(&snic.ios_inflight);
...
//
    ...
    spinlock_t *io_lock = core::ptr::null_mut();
    unsigned long flags;
    let mut ret: c_int = 0, tag = snic_cmd_tag(sc);
    io_lock = snic_io_lock_hash(snic, sc);
    spin_lock_irqsave(io_lock, flags);
    rqi = (struct snic_req_info *) CMD_SP(sc);
    if (!rqi) {
    ...
    rqi.abts_done = core::ptr::null_mut();
    ret = FAILED;
...
//
    ...
    unsigned long flags;
    let mut ret: c_int = 0, tmf = 0, tag = snic_cmd_tag(sc);
    tgt = starget_to_tgt(scsi_target(sc.device));
...
//
    ...
    let mut ret: c_int = SUCCESS, tag = snic_cmd_tag(sc);
    let mut start_time: u32 = jiffies;
    ...
    struct snic_req_info *rqi = core::ptr::null_mut();
    struct scsi_cmnd *sc = core::ptr::null_mut();
    struct scsi_device *lr_sdev = core::ptr::null_mut();
    spinlock_t *io_lock = core::ptr::null_mut();
    u32 tag;
    unsigned long flags;
    if (lr_sc)
...
// Add special tag for device reset
    ...
    unsigned long flags;
    DECLARE_COMPLETION_ONSTACK(tm_done);
    let mut ret: c_int = FAILED, tag = snic_cmd_tag(sc);
    io_lock = snic_io_lock_hash(snic, sc);
    spin_lock_irqsave(io_lock, flags);
...
//
    ...
    struct snic *snic = shost_priv(shost);
    struct snic_req_info *rqi = core::ptr::null_mut();
    let mut tag: c_int = snic_cmd_tag(sc);
    let mut start_time: c_int = jiffies;
    let mut ret: c_int = FAILED;
    let mut dr_supp: c_int = 0;
...
//
    ...
    io_lock = snic_io_lock_hash(snic, sc);
    spin_lock_irqsave(io_lock, flags);
    SNIC_BUG_ON(CMD_SP(sc) != core::ptr::null_mut());
    CMD_STATE(sc) = SNIC_IOREQ_PENDING;
    CMD_SP(sc) = (char *) rqi;
...
// Initialize Request
    ...
    spin_lock_irqsave(io_lock, flags);
    snic.remove_wait = core::ptr::null_mut();
    rqi = (struct snic_req_info *) CMD_SP(sc);
    CMD_SP(sc) = core::ptr::null_mut();
    spin_unlock_irqrestore(io_lock, flags);
    if (rqi)
...
//
    ...
    complete(rqi.dr_done);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: rqi->abts_done) -> else {
...
//
    ...
    {
    struct snic *snic = core::ptr::null_mut();
    struct scsi_cmnd *sc = core::ptr::null_mut();
    struct snic_tgt *sc_tgt = core::ptr::null_mut();
    spinlock_t *io_lock = core::ptr::null_mut();
    unsigned long flags;
    let mut ret: c_int = 0, tag, abt_cnt = 0, tmf = 0;
    if (!tgt)
    return -1;
