...
//! Source: drivers/scsi/zorro_esp.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct blz1230_dma_registers {
    pub /: *mut *mut unsigned char dma_addr; / DMA address [0x0000],
    pub dmapad2: [c_uchar; 0x7fff],
    pub /: *mut *mut unsigned char dma_latch; / DMA latch [0x8000],
}

// Blizzard 1230II DMA interface
#[repr(C)]
#[derive(Copy, Clone)]
pub struct blz1230II_dma_registers {
    pub /: *mut *mut unsigned char dma_addr; / DMA address [0x0000],
    pub dmapad2: [c_uchar; 0xf],
    pub /: *mut *mut unsigned char dma_latch; / DMA latch [0x0010],
}

// Blizzard 2060 DMA interface
#[repr(C)]
#[derive(Copy, Clone)]
pub struct blz2060_dma_registers {
    pub /: *mut *mut unsigned char dma_led_ctrl; / DMA led control [0x000],
    pub dmapad1: [c_uchar; 0x0f],
    pub /: *mut *mut unsigned char dma_addr0; / DMA address (MSB) [0x010],
    pub dmapad2: [c_uchar; 0x03],
    pub /: *mut *mut unsigned char dma_addr1; / DMA address [0x014],
    pub dmapad3: [c_uchar; 0x03],
    pub /: *mut *mut unsigned char dma_addr2; / DMA address [0x018],
    pub dmapad4: [c_uchar; 0x03],
    pub /: *mut *mut unsigned char dma_addr3; / DMA address (LSB) [0x01c],
}

// DMA control bits
pub const DMA_WRITE: c_uint = 0x80000000;
// Cyberstorm DMA interface
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cyber_dma_registers {
    pub /: *mut *mut unsigned char dma_addr0; / DMA address (MSB) [0x000],
    pub dmapad1: [c_uchar; 1],
    pub /: *mut *mut unsigned char dma_addr1; / DMA address [0x002],
    pub dmapad2: [c_uchar; 1],
    pub /: *mut *mut unsigned char dma_addr2; / DMA address [0x004],
    pub dmapad3: [c_uchar; 1],
    pub /: *mut *mut unsigned char dma_addr3; / DMA address (LSB) [0x006],
    pub dmapad4: [c_uchar; 0x3fb],
    pub /: *mut *mut unsigned char cond_reg; / DMA cond (ro) [0x402],

}

// DMA control bits
pub const CYBER_DMA_WRITE: c_uint = 0x40	/* DMA direction. 1 = write */;
pub const CYBER_DMA_Z3: c_uint = 0x20	/* 16 (Z2) or 32 (CHIP/Z3) bit DMA transfer */;
// DMA status bits
pub const CYBER_DMA_HNDL_INTR: c_uint = 0x80	/* DMA IRQ pending? */;
// The CyberStorm II DMA interface
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cyberII_dma_registers {
    pub /: *mut *mut unsigned char cond_reg; / DMA cond (ro) [0x000],
    pub dmapad4: [c_uchar; 0x3f],
    pub /: *mut *mut unsigned char dma_addr0; / DMA address (MSB) [0x040],
    pub dmapad1: [c_uchar; 3],
    pub /: *mut *mut unsigned char dma_addr1; / DMA address [0x044],
    pub dmapad2: [c_uchar; 3],
    pub /: *mut *mut unsigned char dma_addr2; / DMA address [0x048],
    pub dmapad3: [c_uchar; 3],
    pub /: *mut *mut unsigned char dma_addr3; / DMA address (LSB) [0x04c],
}

// Fastlane DMA interface
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fastlane_dma_registers {
    pub /: *mut *mut unsigned char cond_reg; / DMA status (ro) [0x0000],
    pub dmapad1: [c_char; 0x3f],
    pub /: *mut *mut unsigned char clear_strobe; / DMA clear (wo) [0x0040],
}
...
// offsets:
//
pub const FASTLANE_ESP_ADDR: c_uint = 0x1000001;
// DMA status bits
pub const FASTLANE_DMA_MINT: c_uint = 0x80;
pub const FASTLANE_DMA_IACT: c_uint = 0x40;
pub const FASTLANE_DMA_CREQ: c_uint = 0x20;
// DMA control bits
pub const FASTLANE_DMA_FCODE: c_uint = 0xa0;
pub const FASTLANE_DMA_MASK: c_uint = 0xf3;
pub const FASTLANE_DMA_WRITE: c_uint = 0x08	/* 1 = write */;
pub const FASTLANE_DMA_ENABLE: c_uint = 0x04	/* Enable DMA */;
pub const FASTLANE_DMA_EDI: c_uint = 0x02	/* Enable DMA IRQ ? */;
pub const FASTLANE_DMA_ESI: c_uint = 0x01	/* Enable SCSI IRQ */;
//
// private data used for driver
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct zorro_esp_priv {
    pub /: *mut *mut *mut *mut esp esp; / our ESP instance - for Scsi_host,
    pub /: *mut *mut *mut void __iomem board_base; / virtual address (Zorro III board),
    pub /: *mut *mut int zorro3; / board is Zorro III,
    pub /: *mut *mut unsigned char ctrl_data; / shadow copy of ctrl_reg,
}
...
//
#[no_mangle]
unsafe extern "C" fn zorro_esp_write8(esp: *mut esp, val: u8, reg: c_ulong) {
    static void zorro_esp_write8(struct esp *esp, u8 val, unsigned long reg)
    {
    writeb(val, esp.regs + (reg * 4UL));
    }
#[no_mangle]
unsafe extern "C" fn zorro_esp_read8(esp: *mut esp, reg: c_ulong) -> u8 {
    static u8 zorro_esp_read8(struct esp *esp, unsigned long reg)
    {
    return readb(esp.regs + (reg * 4UL));
    }
#[no_mangle]
unsafe extern "C" fn zorro_esp_irq_pending(esp: *mut esp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn zorro_esp_reset_dma(esp: *mut esp) {
    static void zorro_esp_reset_dma(struct esp *esp)
    {
// nothing to do here
    }
#[no_mangle]
unsafe extern "C" fn zorro_esp_dma_drain(esp: *mut esp) {
    static void zorro_esp_dma_drain(struct esp *esp)
    {
// nothing to do here
    }
#[no_mangle]
unsafe extern "C" fn zorro_esp_dma_invalidate(esp: *mut esp) {
    static void zorro_esp_dma_invalidate(struct esp *esp)
    {
// nothing to do here
    }
#[no_mangle]
unsafe extern "C" fn fastlane_esp_dma_invalidate(esp: *mut esp) {
...
// Blizzard 1230/60 SCSI-IV DMA
    ...
    let mut phase: u8 = esp.sreg & ESP_STAT_PMASK;
//
...
#[no_mangle]
unsafe extern "C" fn zorro_esp_dma_error(esp: *mut esp) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct zorro_driver_data {
    pub name: *const c_char,
    pub offset: c_ulong,
    pub dma_offset: c_ulong,
    pub /: *mut *mut int absolute; / offset is absolute address,
    pub scsi_option: c_int,
    pub esp_ops: *const esp_driver_ops,
}
...
#[no_mangle]
unsafe extern "C" fn zorro_esp_remove(z: *mut zorro_dev) {
...
#[no_mangle]
unsafe extern "C" fn zorro_esp_scsi_init() -> int __init {
    static int __init zorro_esp_scsi_init(void)
    {
    return zorro_register_driver(&zorro_esp_driver);
    }
#[no_mangle]
unsafe extern "C" fn zorro_esp_scsi_exit() -> void __exit {
