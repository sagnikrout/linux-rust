...
//! Source: drivers/scsi/leapraid/leapraid_transport.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn leapraid_transport_convert_phy_link_rate(link_rate: u8) -> enum sas_linkrate {
...
// mappings[i].target |= mappings[i].protocol;
    ...
    struct sas_identify *identify)
    {
    let mut cfgp1: union cfg_param_1 = {0};
    let mut cfgp2: union cfg_param_2 = {0};
...
// out_topo_node = topo_node;
    ...
    {
    struct leapraid_vphy *vphy = core::ptr::null_mut();
    int i;
    for (i = 0; i < topo_node.phys_num; i++) {
    if (topo_node.card_phy[i].remote_identify.sas_address !=
    sas_port.remote_identify.sas_address ||
...
// out_vphy = vphy;
    ...
    dev_warn(&adapter.pdev.dev,
    "%s: topo_node parent device is core::ptr::null_mut()\n",  __func__);
    ...
    if (sas_dev)
    sas_dev.rphy = core::ptr::null_mut();
    sas_rphy_free(rphy);
    goto cleanup_card_phy;
    }
    sas_port.port = port;
    ...
    list_for_each_entry(card_phy, &sas_port.phy_list, port_siblings) {
    card_phy.phy_is_assigned = 0;
    card_phy.card_port = core::ptr::null_mut();
    }
    out_delete_port:
    ...
    struct leapraid_topo_node *topo_node = core::ptr::null_mut();
    struct leapraid_sas_port *sas_port;
    struct leapraid_vphy *vphy = core::ptr::null_mut();
    struct sas_rphy *rphy;
    unsigned long flags;
...
// found = true;
    ...
    remove_card_port = core::ptr::null_mut();
    }
    }
    ...
    leapraid_init_sas_or_exp_phy(adapter, card_phy, phy, phy_pg0, core::ptr::null_mut());
    if (sas_phy_add(phy)) {
    dev_err(&adapter.pdev.dev,
    ...
    leapraid_init_sas_or_exp_phy(adapter, card_phy, phy, core::ptr::null_mut(), exp_pg1);
    if (sas_phy_add(phy)) {
    dev_err(&adapter.pdev.dev,
    "%s: SAS PHY add failed!\n", __func__);
    sas_phy_free(phy);
...
#[no_mangle]
unsafe extern "C" fn leapraid_send_smp_req(adapter: *mut leapraid_adapter) -> c_int {
