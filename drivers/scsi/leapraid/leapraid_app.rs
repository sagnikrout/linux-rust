...
//! Source: drivers/scsi/leapraid/leapraid_app.c
#![no_std]
...
// IOCTL version.
pub const LEAPRAID_IOCTL_VERSION: c_uint = 0x07;
// IOCTL commands.
pub const LEAPRAID_ADAPTER_INFO: c_int = 17;
pub const LEAPRAID_COMMAND: c_int = 20;
pub const LEAPRAID_EVENTQUERY: c_int = 21;
pub const LEAPRAID_EVENTREPORT: c_int = 23;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct leapraid_ioctl_header {
    pub adapter_id: u32,
    pub port_number: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct leapraid_ioctl_diag_reset {
    pub hdr: leapraid_ioctl_header,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct leapraid_ioctl_pci_info {
    union {
    struct {
    pub dev:5: u32,
    pub func:3: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct leapraid_ioctl_adapter_info {
    pub hdr: leapraid_ioctl_header,
    ...
    pub fw_ver: u32,
    pub bios_ver: u32,
    pub driver_ver: [u8; 32],
    pub r1: u8,
    pub scsi_id: u8,
    pub r2: u16,
    pub pci_info: leapraid_ioctl_pci_info,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct leapraid_ioctl_command {
    pub hdr: leapraid_ioctl_header,
    pub timeout: u32,
    pub rep_msg_buf_ptr: *mut void __user,
    pub c2h_buf_ptr: *mut void __user,
    pub h2c_buf_ptr: *mut void __user,
    pub sense_data_ptr: *mut void __user,
    pub max_rep_bytes: u32,
    pub c2h_size: u32,
    pub h2c_size: u32,
...
#[no_mangle]
unsafe extern "C" fn leapraid_ctl_put_adapter(adapter: *mut leapraid_adapter) {
    ...
    u16 taskid;
    void *h2c = core::ptr::null_mut();
    let mut h2c_size: usize = 0;
    let mut h2c_dma_addr: dma_addr_t = 0;
    void *c2h = core::ptr::null_mut();
    let mut c2h_size: usize = 0;
    let mut c2h_dma_addr: dma_addr_t = 0;
    void *psge;
    unsigned long timeout;
    let mut dev_hdl: u16 = LEAPRAID_INVALID_DEV_HANDLE;
    ...
    struct leapraid_adapter *adapter = core::ptr::null_mut();
    struct leapraid_ioctl_command __user *uarg;
    struct leapraid_ioctl_command karg;
    let mut rc: c_int = -ENOIOCTLCMD;
...
#[no_mangle]
unsafe extern "C" fn leapraid_fw_mmap_open(vma: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn leapraid_fw_mmap_close(vma: *mut vm_area_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn leapraid_ctl_init() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn leapraid_ctl_exit() {
