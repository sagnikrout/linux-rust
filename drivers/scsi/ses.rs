...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ses_device {
    pub page1: *mut c_uchar,
    pub page1_types: *mut c_uchar,
    pub page2: *mut c_uchar,
    pub page10: *mut c_uchar,
    pub page1_len: c_short,
    pub page1_num_types: c_short,
    pub page2_len: c_short,
    pub page10_len: c_short,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ses_component {
    pub addr: u64,
}

#[no_mangle]
unsafe extern "C" fn ses_page2_supported(edev: *mut enclosure_device) -> bool {
    static bool ses_page2_supported(struct enclosure_device *edev)
    {
    struct ses_device *ses_dev = edev.scratch;
    return (ses_dev.page2 != core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn ses_probe(sdev: *mut scsi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ses_show_id(edev: *mut enclosure_device, buf: *mut c_char) -> c_int {
    static int ses_show_id(struct enclosure_device *edev, char *buf)
    {
    struct ses_device *ses_dev = edev.scratch;
    let mut id: c_ulonglong = get_unaligned_be64(ses_dev.page1+8+4);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ses_host_edev {
    pub shost: *mut Scsi_Host,
    pub edev: *mut enclosure_device,
}

#[no_mangle]
pub unsafe extern "C" fn ses_match_host(edev: *mut enclosure_device, data: *mut c_void) -> c_int {
    ...
    {
    let mut eip: c_int = desc[0] & 0x10;
    let mut invalid: c_int = desc[0] & 0x80;
    let mut proto: enum scsi_protocol = desc[0] & 0x0f;
    let mut addr: u64 = 0;
...
pub const INIT_ALLOC_SIZE: c_int = 32;
    static void ses_enclosure_data_process(struct enclosure_device *edev,
    struct scsi_device *sdev,
    int create)
    {
    u32 result;
    unsigned char *buf = core::ptr::null_mut(), *type_ptr, *desc_ptr, *addl_desc_ptr = core::ptr::null_mut();
    int i, j, page7_len, len, components;
    struct ses_device *ses_dev = edev.scratch;
    let mut types: c_int = ses_dev.page1_num_types;
...
// reserved space
    ...
    name);
#[no_mangle]
pub unsafe extern "C" fn if(edev->components: components <) -> else {
    ...
    addl_desc_ptr,
    max_desc_len))
    addl_desc_ptr = core::ptr::null_mut();
    }
    if (create)
...
#[no_mangle]
unsafe extern "C" fn ses_intf_add(cdev: *mut device) -> c_int {
    static int ses_intf_add(struct device *cdev)
    {
    struct scsi_device *sdev = to_scsi_device(cdev.parent);
    struct scsi_device *tmp_sdev;
    unsigned char *buf = core::ptr::null_mut(), *hdr_buf, *type_ptr, page;
    ...
    struct ses_component *scomp = core::ptr::null_mut();
    if (!scsi_device_enclosure(sdev)) {
// not an enclosure, but might be in one
    struct enclosure_device *prev = core::ptr::null_mut();
    while ((edev = enclosure_find(&sdev.host.shost_gendev, prev)) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn ses_intf_remove_component(sdev: *mut scsi_device) {
    static void ses_intf_remove_component(struct scsi_device *sdev)
    {
    struct enclosure_device *edev, *prev = core::ptr::null_mut();
    while ((edev = enclosure_find(&sdev.host.shost_gendev, prev)) != core::ptr::null_mut()) {
    prev = edev;
...
#[no_mangle]
unsafe extern "C" fn ses_intf_remove_enclosure(sdev: *mut scsi_device) {
    static void ses_intf_remove_enclosure(struct scsi_device *sdev)
    {
    struct enclosure_device *edev;
    struct ses_device *ses_dev;
// exact match to this enclosure
    edev = enclosure_find(&sdev.sdev_gendev, core::ptr::null_mut());
    if (!edev)
    return;
    ses_dev = edev.scratch;
    edev.scratch = core::ptr::null_mut();
    kfree(ses_dev.page10);
    kfree(ses_dev.page1);
    kfree(ses_dev.page2);
    kfree(ses_dev);
...
#[no_mangle]
unsafe extern "C" fn ses_intf_remove(cdev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn ses_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ses_exit() -> void __exit {
