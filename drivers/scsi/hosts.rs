...
//
    ...
    let mut shost_eh_deadline: static int = -1;
    module_param_named(eh_deadline, shost_eh_deadline, int, S_IRUGO|S_IWUSR);
    MODULE_PARM_DESC(eh_deadline,
    "SCSI EH timeout in seconds (should be between 0 and 2^31-1)");
    static DEFINE_IDA(host_index_ida);
#[no_mangle]
unsafe extern "C" fn scsi_host_cls_release(dev: *mut device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn scsi_host_set_state(shost: *mut Scsi_Host, state: enum scsi_host_state) -> c_int {
    int scsi_host_set_state(struct Scsi_Host *shost, enum scsi_host_state state)
    __must_hold(shost.host_lock)
    {
    let mut oldstate: enum scsi_host_state = READ_ONCE(shost.shost_state);
    if (state == oldstate)
    return 0;
    switch (state) {
    case SHOST_CREATED:
...
//
#[no_mangle]
pub unsafe extern "C" fn scsi_remove_host(shost: *mut Scsi_Host) {
...
//
    ...
    GFP_KERNEL);
    if (shost.shost_data == core::ptr::null_mut()) {
    error = -ENOMEM;
    goto out_del_dev;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn scsi_host_dev_release(dev: *mut device) {
    static void scsi_host_dev_release(struct device *dev)
    {
    struct Scsi_Host *shost = dev_to_shost(dev);
    let mut state: enum scsi_host_state = scsi_get_host_state(shost);
...
//
    ...
    shost.eh_deadline = -1;
#[no_mangle]
pub unsafe extern "C" fn if(INT_MAX: *mut *mut (ulong) shost_eh_deadline  HZ >) -> else {
...
#[no_mangle]
unsafe extern "C" fn __scsi_host_match(dev: *mut device, data: *const c_void) -> c_int {
...
//
    ...
    cdev = class_find_device(&shost_class, core::ptr::null_mut(), &hostnum,
    __scsi_host_match);
    if (cdev) {
...
#[no_mangle]
unsafe extern "C" fn scsi_host_check_in_flight(rq: *mut request, data: *mut c_void) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn scsi_host_busy(shost: *mut Scsi_Host) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn scsi_host_put(shost: *mut Scsi_Host) {
...
#[no_mangle]
pub unsafe extern "C" fn scsi_init_hosts() -> c_int {
    int scsi_init_hosts(void)
    {
    return class_register(&shost_class);
    }
#[no_mangle]
pub unsafe extern "C" fn scsi_exit_hosts() {
...
#[no_mangle]
pub unsafe extern "C" fn scsi_is_host_device(dev: *const device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn scsi_queue_work(shost: *mut Scsi_Host, work: *mut work_struct) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn scsi_flush_work(shost: *mut Scsi_Host) {
...
#[no_mangle]
unsafe extern "C" fn complete_all_cmds_iter(rq: *mut request, data: *mut c_void) -> bool {
    static bool complete_all_cmds_iter(struct request *rq, void *data)
    {
    struct scsi_cmnd *scmd = blk_mq_rq_to_pdu(rq);
    let mut status: enum scsi_host_status = *(enum scsi_host_status *)data;
    scsi_dma_unmap(scmd);
    scmd.result = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scsi_host_busy_iter_data {
    pub ): *mut *mut *mut bool (fn)(struct scsi_cmnd , void,
    pub priv: *mut c_void,
}

#[no_mangle]
unsafe extern "C" fn __scsi_host_busy_iter_fn(req: *mut request, priv: *mut c_void) -> bool {
