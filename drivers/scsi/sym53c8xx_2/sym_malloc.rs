...
//! Source: drivers/scsi/sym53c8xx_2/sym_malloc.c
#![no_std]
...
//
    ...
    {
    let mut i: c_int = 0;
    let mut s: c_int = (1 << SYM_MEM_SHIFT);
    int j;
    void *a;
    let mut h: m_link_p = mp.h;
    ...
    if (h[j].next)
    h[j].next.next = core::ptr::null_mut();
    break;
    }
    ++j;
    s <<= 1;
...
//
#[no_mangle]
unsafe extern "C" fn ___sym_mfree(mp: m_pool_p, ptr: *mut c_void, size: c_int) {
...
//
    ...
    memset(p, 0, size);
#[no_mangle]
pub unsafe extern "C" fn if(SYM_MEM_WARN: uflags &) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn __sym_mfree(mp: m_pool_p, ptr: *mut c_void, size: c_int, name: *mut c_char) {
...
#[no_mangle]
unsafe extern "C" fn ___mp0_free_mem_cluster(mp: m_pool_p, m: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn ___get_dma_mem_cluster(mp: m_pool_p) -> *mut c_void {
    ...
    if (vaddr) {
    let mut hc: c_int = VTOB_HASH_CODE(vaddr);
    vbp.next = mp.vtob[hc];
    mp.vtob[hc] = vbp;
    ++mp.nump;
    }
...
#[no_mangle]
unsafe extern "C" fn ___free_dma_mem_cluster(mp: m_pool_p, m: *mut c_void) {
    static void ___free_dma_mem_cluster(m_pool_p mp, void *m)
    {
    m_vtob_p *vbpp, vbp;
    let mut hc: c_int = VTOB_HASH_CODE(m);
    vbpp = &mp.vtob[hc];
...
#[no_mangle]
pub unsafe extern "C" fn ___get_dma_pool(dev_dmat: m_pool_ident_t) -> m_pool_p {
...
#[no_mangle]
unsafe extern "C" fn ___cre_dma_pool(dev_dmat: m_pool_ident_t) -> m_pool_p {
    static m_pool_p ___cre_dma_pool(m_pool_ident_t dev_dmat)
    {
    let mut mp: m_pool_p = __sym_calloc(&mp0, sizeof(*mp), "MPOOL");
    if (mp) {
...
#[no_mangle]
unsafe extern "C" fn ___del_dma_pool(p: m_pool_p) {
...
#[no_mangle]
pub unsafe extern "C" fn __sym_mfree_dma(dev_dmat: m_pool_ident_t, m: *mut c_void, size: c_int, name: *mut c_char) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __vtobus(dev_dmat: m_pool_ident_t, m: *mut c_void) -> dma_addr_t {
    dma_addr_t __vtobus(m_pool_ident_t dev_dmat, void *m)
    {
    unsigned long flags;
    m_pool_p mp;
    let mut hc: c_int = VTOB_HASH_CODE(m);
    let mut vp: m_vtob_p = core::ptr::null_mut();
