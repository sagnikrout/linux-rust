...
//! Source: drivers/scsi/scsi_transport_spi.c
#![no_std]
...
// on" attributes
pub const SPI_HOST_ATTRS: c_int = 1;
pub const SPI_MAX_ECHO_BUFFER_SIZE: c_int = 4096;
pub const DV_LOOPS: c_int = 3;

// two cc/ua clears
// Our blacklist flags
    enum {
    SPI_BLIST_NOIUS = ( blist_flags_t)0x1,
    };
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct spi_internal {
    pub t: scsi_transport_template,
    pub f: *mut spi_function_template,
}
...
// by 4
pub const SPI_STATIC_PPR: c_uint = 0x0c;
#[no_mangle]
unsafe extern "C" fn sprint_frac(dest: *mut c_char, value: c_int, denom: c_int) -> c_int {
    static int sprint_frac(char *dest, int value, int denom)
    {
    let mut frac: c_int = value % denom;
    let mut result: c_int = sprintf(dest, "%d", value / denom);
    if (frac == 0)
    return result;
    dest[result++] = '.';
    do {
...
#[no_mangle]
pub unsafe extern "C" fn spi_signal_to_value(name: *const c_char) -> enum spi_signal_type {
...
// gleaned from the device inquiry
    ...
    \
    tp = (struct spi_transport_attrs *)&starget.starget_data;	\
    val = simple_strtoul(buf, core::ptr::null_mut(), 0);				\
    tp.field = val;						\
    return count;							\
    }
...
#[no_mangle]
unsafe extern "C" fn child_iter(dev: *mut device, data: *mut c_void) -> c_int {
    ...
    {
    struct scsi_target *starget = transport_class_to_starget(dev);
    device_for_each_child(&starget.dev, core::ptr::null_mut(), child_iter);
    return count;
    }
    static DEVICE_ATTR(revalidate, S_IWUSR, core::ptr::null_mut(), store_spi_revalidate);
// Translate the period into ns according to the current spec
// for SDTR/PPR messages
#[no_mangle]
unsafe extern "C" fn period_to_str(buf: *mut c_char, period: c_int) -> c_int {
    ...
    {
    let mut len: c_int = period_to_str(buf, period);
    buf[len++] = '\n';
    buf[len] = '\0';
    return len;
    }
...
// periodp = period;
    ...
    struct spi_internal *i = to_spi_internal(shost.transportt);
    let mut type: enum spi_signal_type = spi_signal_to_value(buf);
    ...
    show_spi_host_width, core::ptr::null_mut());
    ...
    show_spi_host_hba_id, core::ptr::null_mut());
...
// an echo buffer, we do read/write tests to it
    ...
    {
    let mut len: c_int = ptr - buffer;
    int j, k, r, result;
    let mut pattern: c_uint = 0x0000ffff;
    struct scsi_sense_hdr sshdr;
...
// fill with shifting bits (test d)
    ...
    let mut roll: u32 = (pattern & 0x80000000) ? 1 : 0;
// word = pattern;
...
// on the inquiry data from the device
    ...
    {
    int r, result;
    let mut len: c_int = sdev.inquiry_len;
...
// failure
    ...
    struct scsi_target *starget = sdev.sdev_target;
    let mut period: c_int = 0, prevperiod = 0;
...
// TUR succeeded
    ...
    REQ_OP_DRV_IN, buffer, 4, core::ptr::null_mut());
    if (result)
// Device has no echo buffer
    ...
    let mut len: c_int = sdev.inquiry_len;
    let mut min_period: c_int = spi_min_period(starget);
    let mut max_width: c_int = spi_max_width(starget);
...
//
    ...
    {
    struct scsi_target *starget = sdev.sdev_target;
    let mut len: c_int = SPI_MAX_ECHO_BUFFER_SIZE*2;
...
//
#[no_mangle]
pub unsafe extern "C" fn spi_display_xfer_agreement(starget: *mut scsi_target) {
    ...
    scsi = "FAST-20";
#[no_mangle]
pub unsafe extern "C" fn if(50: tp->period <) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn spi_populate_width_msg(msg: *mut c_uchar, width: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn spi_populate_sync_msg(msg: *mut c_uchar, period: c_int, offset: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn spi_populate_tag_msg(msg: *mut c_uchar, cmd: *mut scsi_cmnd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn print_nego(msg: *const c_uchar, per: c_int, off: c_int, width: c_int) {
...
#[no_mangle]
unsafe extern "C" fn print_ptr(msg: *const c_uchar, msb: c_int, desc: *const c_char) {
...
#[no_mangle]
pub unsafe extern "C" fn spi_print_msg(msg: *const c_uchar) -> c_int {
    int spi_print_msg(const unsigned char *msg)
    {
    let mut len: c_int = 1, i;
...
#[no_mangle]
pub unsafe extern "C" fn if(&dev_attr_revalidate.attr: attr ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn spi_release_transport(t: *mut scsi_transport_template) {
...
#[no_mangle]
unsafe extern "C" fn spi_transport_init() -> __init int {
...
#[no_mangle]
unsafe extern "C" fn spi_transport_exit() -> void __exit {
