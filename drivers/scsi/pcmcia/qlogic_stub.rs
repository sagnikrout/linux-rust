...
//! Source: drivers/scsi/pcmcia/qlogic_stub.c
#![no_std]
...
// ====================================================================
    ...
    if (scsi_add_host(shost, core::ptr::null_mut()))
    goto free_interrupt;
...
#[no_mangle]
unsafe extern "C" fn qlogic_probe(link: *mut pcmcia_device) -> c_int {
...
// ====================================================================
#[no_mangle]
unsafe extern "C" fn qlogic_detach(link: *mut pcmcia_device) {
...
// ====================================================================
#[no_mangle]
unsafe extern "C" fn qlogic_config_check(p_dev: *mut pcmcia_device, priv_data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qlogic_config(link: *mut *mut pcmcia_device) -> c_int {
    static int qlogic_config(struct pcmcia_device * link)
    {
    ...
    ret = pcmcia_loop_config(link, qlogic_config_check, core::ptr::null_mut());
    if (ret)
    goto failed;
    if (!link.irq)
    goto failed;
...
// ====================================================================
#[no_mangle]
unsafe extern "C" fn qlogic_release(link: *mut pcmcia_device) {
    static void qlogic_release(struct pcmcia_device *link)
    {
    scsi_info_t *info = link.priv;
...
// ====================================================================
#[no_mangle]
unsafe extern "C" fn qlogic_resume(link: *mut pcmcia_device) -> c_int {
...
// Ugggglllyyyy!!!
    qlogicfas408_host_reset(core::ptr::null_mut());
    return 0;
    }
