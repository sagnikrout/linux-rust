...
//! Source: drivers/scsi/qedi/qedi_fw.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn qedi_iscsi_unmap_sg_list(cmd: *mut qedi_cmd) {
...
#[no_mangle]
unsafe extern "C" fn qedi_tmf_resp_work(work: *mut work_struct) {
    ...
    goto exit_tmf_resp;
    spin_lock(&session.back_lock);
    __iscsi_complete_pdu(conn, (struct iscsi_hdr *)resp_hdr_ptr, core::ptr::null_mut(), 0);
    spin_unlock(&session.back_lock);
    exit_tmf_resp:
    kfree(resp_hdr_ptr);
    ...
    struct qedi_cmd *qedi_cmd = core::ptr::null_mut();
...
// get the scsi command
    sc_cmd = cmd.scsi_cmd;
    if (!sc_cmd) {
    QEDI_WARN(&qedi.dbg_ctx, "sc_cmd is core::ptr::null_mut()!\n");
    goto error;
    }
    if (!iscsi_cmd(sc_cmd).task) {
    QEDI_WARN(&qedi.dbg_ctx,
    "core::ptr::null_mut() task pointer, returned in another context.\n");
    goto error;
    }
    if (!scsi_cmd_to_rq(sc_cmd).q) {
    QEDI_WARN(&qedi.dbg_ctx,
    "request.q is core::ptr::null_mut() so request is not valid, sc_cmd=%p.\n",
    sc_cmd);
    goto error;
    }
...
//
    ...
    let mut proto_itt: u32 = cqe.itid;
    ...
    struct iscsi_tm *tmf_hdr = core::ptr::null_mut();
...
// We found the command
    ...
    kfree(work);
    qedi_cmd.list_tmf_work = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn qedi_fp_process_cqes(work: *mut qedi_work) {
    ...
    u32 iscsi_cid;
    u32 hdr_opcode;
    let mut que_idx: u16 = work.que_idx;
    let mut cqe_err_bits: u8 = 0;
    ...
    task = qedi_cmd.task;
    if (!task) {
    QEDI_WARN(&qedi.dbg_ctx, "task is core::ptr::null_mut()\n");
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn qedi_ring_doorbell(qedi_conn: *mut qedi_conn) {
...
#[no_mangle]
unsafe extern "C" fn qedi_get_wqe_idx(qedi_conn: *mut qedi_conn) -> u16 {
...
// Increament SQ index
    ...
    struct scsi_sge *resp_sge = core::ptr::null_mut();
    struct qedi_cmd *qedi_cmd;
    struct qedi_endpoint *ep;
    let mut tid: i16 = 0;
    let mut sq_idx: u16 = 0;
    let mut rval: c_int = 0;
...
// Fill fw input params
    ...
    struct iscsi_task_params task_params;
    struct iscsi_task_context *fw_task_ctx;
    struct iscsi_logout *logout_hdr = core::ptr::null_mut();
    struct qedi_ctx *qedi = qedi_conn.qedi;
    struct qedi_cmd *qedi_cmd;
    struct qedi_endpoint *ep;
    let mut tid: i16 = 0;
...
// Fill fw input params
    ...
    struct iscsi_tm *tmf_hdr;
    let mut lun: c_uint = 0;
    let mut lun_reset: bool = false;
    struct iscsi_conn *conn = qedi_conn.cls_conn.dd_data;
    struct iscsi_session *session = conn.session;
...
#[no_mangle]
unsafe extern "C" fn qedi_abort_work(work: *mut work_struct) {
    ...
    struct qedi_ctx *qedi = qedi_conn.qedi;
    struct iscsi_conn *conn = qedi_conn.cls_conn.dd_data;
    struct qedi_work_map *list_work = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn qedi_send_iscsi_tmf(qedi_conn: *mut qedi_conn, mtask: *mut iscsi_task) -> c_int {
    ...
    case ISCSI_TM_FUNC_TARGET_WARM_RESET:
    case ISCSI_TM_FUNC_TARGET_COLD_RESET:
    rc = send_iscsi_tmf(qedi_conn, mtask, core::ptr::null_mut());
    break;
    default:
    ...
    struct scsi_sge *req_sge = core::ptr::null_mut();
    struct scsi_sge *resp_sge = core::ptr::null_mut();
    struct qedi_cmd *qedi_cmd;
    struct qedi_endpoint *ep;
    let mut tid: i16 = 0;
...
#[no_mangle]
unsafe extern "C" fn qedi_map_scsi_sg(qedi: *mut qedi_ctx, cmd: *mut qedi_cmd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qedi_iscsi_map_sg_list(cmd: *mut qedi_cmd) {
...
#[no_mangle]
unsafe extern "C" fn qedi_cpy_scsi_cdb(sc: *mut scsi_cmnd, dstp: *mut u32) {
...
#[no_mangle]
pub unsafe extern "C" fn qedi_iscsi_send_ioreq(task: *mut iscsi_task) -> c_int {
    ...
    struct scsi_sgl_task_params tx_sgl_task_params;
    struct scsi_sgl_task_params rx_sgl_task_params;
    struct scsi_sgl_task_params *prx_sgl = core::ptr::null_mut();
    struct scsi_sgl_task_params *ptx_sgl = core::ptr::null_mut();
    struct iscsi_task_params task_params;
    ...
    let mut task_type: enum iscsi_task_type = MAX_ISCSI_TASK_TYPE;
    struct qedi_endpoint *ep;
    u32 scsi_lun[2];
    let mut tid: i16 = 0;
    let mut sq_idx: u16 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_TASK_TYPE_INITIATOR_READ: task_type ==) -> else {
    else if (task_type == ISCSI_TASK_TYPE_INITIATOR_READ)
...
#[no_mangle]
pub unsafe extern "C" fn qedi_iscsi_cleanup_task(task: *mut iscsi_task, mark_cmd_node_deleted: bool) -> c_int {
