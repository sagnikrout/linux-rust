...
//! Source: drivers/scsi/iscsi_boot_sysfs.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn iscsi_boot_kobj_release(kobj: *mut kobject) {
...
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_flags.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_flags.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_FLAGS);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_ip.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_ip.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_IP_ADDR);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_port.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_port.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_PORT);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_lun.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_lun.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_LUN);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_chap.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_chap.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_CHAP_TYPE);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_nic.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_nic.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_NIC_ASSOC);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_name.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_name.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_NAME);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_chap_name.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_chap_name.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_CHAP_NAME);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_chap_secret.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_chap_secret.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_CHAP_SECRET);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_chap_rev_name.attr: attr ==) -> else {
    else if (attr == &iscsi_boot_attr_tgt_chap_rev_name.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_TGT_REV_CHAP_NAME);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_tgt_chap_rev_secret.attr: attr ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_flags.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_flags.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_FLAGS);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_ip.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_ip.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_IP_ADDR);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_prefix.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_prefix.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_PREFIX_LEN);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_subnet.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_subnet.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_SUBNET_MASK);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_origin.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_origin.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_ORIGIN);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_gateway.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_gateway.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_GATEWAY);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_primary_dns.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_primary_dns.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_PRIMARY_DNS);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_secondary_dns.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_secondary_dns.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_SECONDARY_DNS);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_dhcp.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_dhcp.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_DHCP);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_vlan.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_vlan.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_VLAN);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_mac.attr: attr ==) -> else {
    else if (attr ==  &iscsi_boot_attr_eth_mac.attr)
    return boot_kobj.is_visible(boot_kobj.data,
    ISCSI_BOOT_ETH_MAC);
#[no_mangle]
pub unsafe extern "C" fn if(&iscsi_boot_attr_eth_hostname.attr: attr ==) -> else {
...
//
    boot_kobj.release = core::ptr::null_mut();
    kobject_put(&boot_kobj.kobj);
    return core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn iscsi_boot_remove_kobj(boot_kobj: *mut iscsi_boot_kobj) {
...
//
    ...
    if (!boot_kset)
    return core::ptr::null_mut();
    boot_kset.kset = kset_create_and_add(set_name, core::ptr::null_mut(), firmware_kobj);
    if (!boot_kset.kset) {
    kfree(boot_kset);
    return core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn iscsi_boot_destroy_kset(boot_kset: *mut iscsi_boot_kset) {
