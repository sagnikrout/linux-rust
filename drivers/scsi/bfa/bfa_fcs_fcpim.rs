...
//! Source: drivers/scsi/bfa/bfa_fcs_fcpim.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bfa_fcs_itnim_sm_table_s {
    pub /: *mut *mut bfa_fcs_itnim_sm_t sm; / state machine function,
    pub /: *mut *mut enum bfa_itnim_state state; / state machine encoding,
...
//
    ...
    bfa_fcs_itnim_send_prli(itnim, core::ptr::null_mut());
    break;
...
//
    ...
    itnim.bfa_itnim = core::ptr::null_mut();
    }
...
//
    ...
    if (!rport)
    return core::ptr::null_mut();
    WARN_ON(rport.itnim == core::ptr::null_mut());
    return rport.itnim;
    }
    bfa_status_t
    ...
    {
    struct bfa_fcs_itnim_s *itnim = core::ptr::null_mut();
    itnim = bfa_fcs_itnim_lookup(port, rpwwn);
    if (itnim == core::ptr::null_mut())
    return BFA_STATUS_NO_FCPIM_NEXUS;
    ...
    {
    struct bfa_fcs_itnim_s *itnim = core::ptr::null_mut();
    WARN_ON(port == core::ptr::null_mut());
    itnim = bfa_fcs_itnim_lookup(port, rpwwn);
    if (itnim == core::ptr::null_mut())
    return BFA_STATUS_NO_FCPIM_NEXUS;
