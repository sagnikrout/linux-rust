...
//! Source: drivers/scsi/bfa/bfad.c
#![no_std]
...
//
    ...
    let mut rport_del_timeout: c_int = BFA_FCS_RPORT_DEF_DEL_TIMEOUT;
    let mut bfa_lun_queue_depth: c_int = BFAD_LUN_QUEUE_DEPTH;
    let mut bfa_io_max_sge: c_int = BFAD_IO_MAX_SGE;
    int		bfa_log_level = 3; /* WARNING log level */
    let mut ioc_auto_recover: c_int = BFA_TRUE;
    let mut bfa_linkup_delay: c_int = -1;
    let mut fdmi_enable: c_int = BFA_TRUE;
    int		pcie_max_read_reqsz;
    let mut bfa_debugfs_enable: c_int = 1;
    let mut msix_disable_cb: c_int = 0, msix_disable_ct = 0;
    let mut max_xfer_size: c_int = BFAD_MAX_SECTORS >> 1;
    let mut max_rport_logins: static int = BFA_FCS_MAX_RPORT_LOGINS;
// Firmware releated
...
// Set up interrupt handler for each vectors
    ...
    spin_lock_irqsave(&bfad.bfad_lock, flags);
    bfad.bfad_tsk = core::ptr::null_mut();
    spin_unlock_irqrestore(&bfad.bfad_lock, flags);
...
//
    ...
    {
    let mut rc: bfa_status_t = BFA_STATUS_OK;
// rport_drv = kzalloc_obj(struct bfad_rport_s, GFP_ATOMIC);
    if (*rport_drv == core::ptr::null_mut()) {
...
//
    ...
    {
    let mut port_cfg: bfa_lport_cfg_s = {0};
...
// Iterate through the KVA meminfo queue
    ...
    if (kva_elem.kva == core::ptr::null_mut()) {
...
// Iterate through the DMA meminfo queue
    ...
    if (dma_elem.kva == core::ptr::null_mut()) {
    bfad_hal_mem_release(bfad);
    rc = BFA_STATUS_ENOMEM;
    goto ext;
    }
...
//
    ...
    {
    struct bfad_vport_s   *vport;
    let mut rc: c_int = BFA_STATUS_OK;
    unsigned long	flags;
    ...
    if (bfad.pci_bar0_kva == core::ptr::null_mut()) {
...
// Adjust PCIe Maximum Read Request Size
    ...
    let mut max_rq: c_int = pcie_get_readrq(pdev);
...
// Allocate scsi_host for the physical port
    ...
    if (bfad.pport.im_port == core::ptr::null_mut()) {
...
//
    ...
    {
    int	i;
    let mut match: c_int = 0x00000001;
...
//
    ...
    {
    int error;
    let mut mask: u32 = 0, i, num_bit = 0, max_bit = 0;
...
//
    ...
    {
    struct bfad_s	*bfad;
    let mut error: c_int = -ENODEV, retval, i;
...
//
    ...
    spin_lock_irqsave(&bfad.bfad_lock, flags);
    if (bfad.bfad_tsk != core::ptr::null_mut()) {
    spin_unlock_irqrestore(&bfad.bfad_lock, flags);
    kthread_stop(bfad.bfad_tsk);
    } else {
    spin_unlock_irqrestore(&bfad.bfad_lock, flags);
...
//
    ...
    struct bfad_s *bfad = pci_get_drvdata(pdev);
    unsigned long	flags;
    let mut ret: pci_ers_result_t = PCI_ERS_RESULT_NONE;
...
#[no_mangle]
unsafe extern "C" fn restart_bfa(bfad: *mut bfad_s) -> c_int {
...
// bfi_image = vmalloc(fw->size);
    if (core::ptr::null_mut() == *bfi_image) {
