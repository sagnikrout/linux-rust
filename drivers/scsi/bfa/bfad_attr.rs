...
//! Source: drivers/scsi/bfa/bfad_attr.c
#![no_std]
...
//
    ...
    struct bfad_s         *bfad;
    struct bfad_itnim_s   *itnim = core::ptr::null_mut();
    let mut fc_id: u32 = -1;
    unsigned long   flags;
...
//
    ...
    let mut path_tov: u16 = bfa_fcpim_path_tov_get(&bfad.bfa);
    ...
    port_cfg.pwwn);
    spin_unlock_irqrestore(&bfad.bfad_lock, flags);
    if (fcs_vport == core::ptr::null_mut())
...
// For fibre channel services type 0x20
    ...
    return VPCERR_BAD_WWN;
#[no_mangle]
pub unsafe extern "C" fn if(BFA_STATUS_VPORT_EXISTS: rc ==) -> else {
    else if (rc == BFA_STATUS_VPORT_EXISTS)
    return VPCERR_BAD_WWN;
#[no_mangle]
pub unsafe extern "C" fn if(BFA_STATUS_VPORT_MAX: rc ==) -> else {
    else if (rc == BFA_STATUS_VPORT_MAX)
    return VPCERR_NO_FABRIC_SUPP;
#[no_mangle]
pub unsafe extern "C" fn if(BFA_STATUS_VPORT_WWN_BP: rc ==) -> else {
    ...
    vport.drv_port.flags &= ~BFAD_PORT_DELETE;
    vport.comp_del = core::ptr::null_mut();
    return -1;
    }
...
pub unsafe extern "C" fn if(!bfa_ioc_is_cna(&bfad->bfa.ioc): nports == 2 &&) -> else {
    ...
    struct bfad_port_s    *port = im_port.port;
    struct bfad_s         *bfad = im_port.bfad;
    let mut nrports: c_int = 2048;
    struct bfa_rport_qualifier_s *rports = core::ptr::null_mut();
    unsigned long   flags;
    rports = kzalloc_objs(struct bfa_rport_qualifier_s, nrports, GFP_ATOMIC);
    if (rports == core::ptr::null_mut())
    ...
    bfad_im_serial_num_show, core::ptr::null_mut());
    static          DEVICE_ATTR(model, S_IRUGO, bfad_im_model_show, core::ptr::null_mut());
    static          DEVICE_ATTR(model_description, S_IRUGO,
    bfad_im_model_desc_show, core::ptr::null_mut());
    static          DEVICE_ATTR(node_name, S_IRUGO, bfad_im_node_name_show, core::ptr::null_mut());
    static          DEVICE_ATTR(symbolic_name, S_IRUGO,
    bfad_im_symbolic_name_show, core::ptr::null_mut());
    static          DEVICE_ATTR(hardware_version, S_IRUGO,
    bfad_im_hw_version_show, core::ptr::null_mut());
    static		DEVICE_STRING_ATTR_RO(driver_version, S_IRUGO,
    BFAD_DRIVER_VERSION);
    static          DEVICE_ATTR(option_rom_version, S_IRUGO,
    bfad_im_optionrom_version_show, core::ptr::null_mut());
    static          DEVICE_ATTR(firmware_version, S_IRUGO,
    bfad_im_fw_version_show, core::ptr::null_mut());
    static          DEVICE_ATTR(number_of_ports, S_IRUGO,
    bfad_im_num_of_ports_show, core::ptr::null_mut());
    static		DEVICE_STRING_ATTR_RO(driver_name, S_IRUGO, BFAD_DRIVER_NAME);
    static          DEVICE_ATTR(number_of_discovered_ports, S_IRUGO,
    bfad_im_num_of_discovered_ports_show, core::ptr::null_mut());
