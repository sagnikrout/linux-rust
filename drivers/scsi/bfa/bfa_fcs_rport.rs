...
//! Source: drivers/scsi/bfa/bfa_fcs_rport.c
#![no_std]
...
//
    let mut bfa_fcs_rport_max_logins: static u32 = BFA_FCS_MAX_RPORT_LOGINS;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bfa_fcs_rport_sm_table_s {
    pub /: *mut *mut bfa_fcs_rport_sm_t sm; / state machine function,
    pub /: *mut *mut enum bfa_rport_state state; / state machine encoding,
...
//
    ...
    bfa_fcs_rport_send_plogi(rport, core::ptr::null_mut());
    break;
    case RPSM_EVENT_PLOGI_RCVD:
    bfa_sm_set_state(rport, bfa_fcs_rport_sm_plogiacc_sending);
    bfa_fcs_rport_send_plogiacc(rport, core::ptr::null_mut());
    break;
    ...
    rport.ns_retries = 0;
    bfa_fcs_rport_send_nsdisc(rport, core::ptr::null_mut());
    break;
    default:
...
//
    ...
    bfa_fcs_rport_send_adisc(rport, core::ptr::null_mut());
    }
    break;
    case RPSM_EVENT_PLOGI_RCVD:
    case RPSM_EVENT_LOGO_IMP:
...
//
    ...
    bfa_fcs_rport_send_logo(rport, core::ptr::null_mut());
    break;
    case RPSM_EVENT_LOGO_RCVD:
    bfa_fcs_rport_send_logo_acc(rport);
    fallthrough;
...
//
    ...
    bfa_fcxp_send(fcxp, core::ptr::null_mut(), port.fabric.vf_id, port.lp_tag, BFA_FALSE,
    FC_CLASS_3, len, &fchs, core::ptr::null_mut(),
    rport, FC_MAX_PDUSZ, FC_ELS_TOV);
    rport.stats.logos++;
    bfa_fcxp_discard(rport.fcxp);
    bfa_sm_send_event(rport, RPSM_EVENT_FCXP_SENT);
...
//
    ...
    rport.pwwn = pwwn;
    rport.old_pid = 0;
    rport.bfa_rport = core::ptr::null_mut();
//
// allocate FC-4s
//
...
#[no_mangle]
pub unsafe extern "C" fn if(BFA_RPORT_AEN_QOS_FLOWID: event ==) -> else {
...
// Send the AEN notification
    ...
    rpwwn_buf, lpwwn_buf);
    bfa_fcs_rport_aen_post(rport, BFA_RPORT_AEN_ONLINE, core::ptr::null_mut());
    }
    }
    static void
    ...
    BFA_RPORT_AEN_DISCONNECT, core::ptr::null_mut());
    } else {
    BFA_LOG(KERN_INFO, bfad, bfa_log_level,
    ...
    rpwwn_buf, lpwwn_buf);
    bfa_fcs_rport_aen_post(rport,
    BFA_RPORT_AEN_OFFLINE, core::ptr::null_mut());
    }
    }
    if (bfa_fcs_lport_is_initiator(port)) {
    bfa_fcs_itnim_rport_offline(rport.itnim);
...
// send best case  acc to prlo
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), FC_MAX_PDUSZ, 0);
    }
//
// Send a LS reject
//
...
//
    ...
    struct bfa_fcs_lport_s *port = rport.port;
    let mut rport_speed: bfa_port_speed_t = rport.rpf.rpsc_speed;
...
// Send RPSC2 to a Brocade fabric only.
    ...
    rpf.rpsc_retries = 0;
    bfa_fcs_rpf_send_rpsc2(rpf, core::ptr::null_mut());
    }
    break;
...
#[no_mangle]
pub unsafe extern "C" fn if(BFA_PORT_SPEED_UNKNOWN: rpf->assigned_speed !=) -> else {
