...
//! Source: drivers/scsi/hisi_sas/hisi_sas_main.c
#![no_std]
...
pub const LINK_RATE_BIT_MASK: c_int = 2;
pub const FIS_BUF_SIZE: c_int = 20;
pub const WAIT_CMD_COMPLETE_DELAY: c_int = 100;
pub const WAIT_CMD_COMPLETE_TMROUT: c_int = 5000;
pub const DELAY_FOR_LINK_READY: c_int = 2000;
pub const BLK_CNT_OPTIMIZE_MARK: c_int = 64;
pub const HZ_TO_MHZ: c_int = 1000000;
pub const DELAY_FOR_SOFTRESET_MAX: c_int = 1000;
pub const DELAY_FOR_SOFTRESET_MIN: c_int = 900;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hisi_sas_internal_abort_data {
    pub /: *mut *mut bool rst_ha_timeout; / reset the HA for timeout,
}

#[no_mangle]
unsafe extern "C" fn hisi_sas_get_ata_protocol_from_tf(qc: *mut ata_queued_cmd) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_get_ata_protocol(task: *mut sas_task) -> u8 {
    u8 hisi_sas_get_ata_protocol(struct sas_task *task)
    {
    struct host_to_dev_fis *fis = &task.ata_task.fis;
    struct ata_queued_cmd *qc = task.uldd_task;
    let mut direction: c_int = task.data_dir;
    switch (fis.command) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_get_prog_phy_linkrate_mask(max: enum sas_linkrate) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_stop_phys(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_slot_index_clear(hisi_hba: *mut hisi_hba, slot_idx: c_int) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_slot_index_free(hisi_hba: *mut hisi_hba, slot_idx: c_int) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_slot_index_set(hisi_hba: *mut hisi_hba, slot_idx: c_int) {
    ...
    {
    let mut device_id: c_int = slot.device_id;
    struct hisi_sas_device *sas_dev = &hisi_hba.devices[device_id];
    if (task) {
    struct device *dev = hisi_hba.dev;
    if (!task.lldd_task)
    return;
    task.lldd_task = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_queue_command(task: *mut sas_task, gfp_flags: gfp_t) -> c_int {
    static int hisi_sas_queue_command(struct sas_task *task, gfp_t gfp_flags)
    {
    let mut n_elem: c_int = 0, n_elem_dif = 0;
    struct domain_device *device = task.dev;
    struct asd_sas_port *sas_port = device.port;
    struct hisi_sas_device *sas_dev = device.lldd_dev;
    let mut internal_abort: bool = sas_is_internal_abort(task);
    struct hisi_sas_dq *dq = core::ptr::null_mut();
    struct hisi_sas_port *port;
    struct hisi_hba *hisi_hba;
    struct hisi_sas_slot *slot;
    struct request *rq = core::ptr::null_mut();
    struct device *dev;
    int rc;
    if (!sas_port) {
    struct task_status_struct *ts = &task.task_status;
...
// Nothing
    ...
    let mut last: c_int = hisi_hba.last_dev_id;
    let mut first: c_int = (hisi_hba.last_dev_id + 1) % HISI_SAS_MAX_DEVICES;
    int i;
    spin_lock(&hisi_hba.lock);
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_sync_poll_cq(cq: *mut hisi_sas_cq) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_queue_is_poll(cq: *mut hisi_sas_cq) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_sync_cq(cq: *mut hisi_sas_cq) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_sync_poll_cqs(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_sync_cqs(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_tmf_aborted(task: *mut sas_task) {
...
//
    hisi_sas_sync_cq(cq);
    slot.task = core::ptr::null_mut();
    }
    }
pub const HISI_SAS_DISK_RECOVER_CNT: c_int = 3;
#[no_mangle]
unsafe extern "C" fn hisi_sas_init_device(device: *mut domain_device) -> c_int {
    static int hisi_sas_init_device(struct domain_device *device)
    {
    let mut rc: c_int = TMF_RESP_FUNC_COMPLETE;
    struct scsi_lun lun;
    let mut retry: c_int = HISI_SAS_DISK_RECOVER_CNT;
    struct hisi_hba *hisi_hba = dev_to_hisi_hba(device);
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_sdev_init(sdev: *mut scsi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_dev_found(device: *mut domain_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_sdev_configure(sdev: *mut scsi_device, lim: *mut queue_limits) -> c_int {
    int hisi_sas_sdev_configure(struct scsi_device *sdev, struct queue_limits *lim)
    {
    struct domain_device *dev = sdev_to_domain_dev(sdev);
    let mut ret: c_int = sas_sdev_configure(sdev, lim);
    if (ret)
    return ret;
    if (!dev_is_sata(dev))
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_scan_start(shost: *mut Scsi_Host) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_scan_finished(shost: *mut Scsi_Host, time: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_phyup_work(work: *mut work_struct) {
    static void hisi_sas_phyup_work(struct work_struct *work)
    {
    hisi_sas_phyup_work_common(work, HISI_PHYE_PHY_UP);
    }
#[no_mangle]
unsafe extern "C" fn hisi_sas_linkreset_work(work: *mut work_struct) {
    static void hisi_sas_linkreset_work(struct work_struct *work)
    {
    struct hisi_sas_phy *phy =
    container_of(work, typeof(*phy), works[HISI_PHYE_LINK_RESET]);
    struct asd_sas_phy *sas_phy = &phy.sas_phy;
    hisi_sas_control_phy(sas_phy, PHY_FUNC_LINK_RESET, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn hisi_sas_phyup_pm_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_wait_phyup_timedout(t: *mut timer_list) {
...
pub const HISI_SAS_WAIT_PHYUP_RETRIES: c_int = 10;
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_phy_oob_ready(hisi_hba: *mut hisi_hba, phy_no: c_int) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_phy_init(hisi_hba: *mut hisi_hba, phy_no: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_phy_enable(hisi_hba: *mut hisi_hba, phy_no: c_int, enable: c_int) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_port_notify_formed(sas_phy: *mut asd_sas_phy) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_release_tasks(hisi_hba: *mut hisi_hba) {
    ...
    bool rst_ha_timeout)
    {
    let mut data: hisi_sas_internal_abort_data = { rst_ha_timeout };
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_dev_gone(device: *mut domain_device) {
    static void hisi_sas_dev_gone(struct domain_device *device)
    {
    struct hisi_sas_device *sas_dev = device.lldd_dev;
    struct hisi_hba *hisi_hba = dev_to_hisi_hba(device);
    ...
    device.lldd_dev = core::ptr::null_mut();
    }
...
// Don't mark it as SAS_PHY_UNUSED if failed to clear ITCT
    ...
    sas_dev.sas_device = core::ptr::null_mut();
    up(&hisi_hba.sem);
    }
    ...
    let mut phy_no: c_int = sas_phy.id;
    let mut sts: u8 = phy.phy_attached;
    let mut ret: c_int = 0;
    down(&hisi_hba.sem);
    ...
    out:
    phy.reset_completion = core::ptr::null_mut();
    up(&hisi_hba.sem);
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_softreset_ata_disk(device: *mut domain_device) -> c_int {
    static int hisi_sas_softreset_ata_disk(struct domain_device *device)
    {
    u8 fis[FIS_BUF_SIZE] = {0};
    struct ata_port *ap = device.sata_dev.ap;
    struct ata_link *link;
    let mut rc: c_int = TMF_RESP_FUNC_FAILED;
    struct hisi_hba *hisi_hba = dev_to_hisi_hba(device);
    struct device *dev = hisi_hba.dev;
    ata_for_each_link(link, ap, EDGE) {
    let mut pmp: c_int = sata_srst_pmp(link);
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_refresh_port_id(hisi_hba: *mut hisi_hba) {
    static void hisi_sas_refresh_port_id(struct hisi_hba *hisi_hba)
    {
    let mut state: u32 = hisi_hba.hw.get_phys_state(hisi_hba);
    int i;
    for (i = 0; i < HISI_SAS_MAX_DEVICES; i++) {
    ...
    struct hisi_sas_port *port;
    struct hisi_sas_phy *phy = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_rescan_topology(hisi_hba: *mut hisi_hba, state: u32) {
    static void hisi_sas_rescan_topology(struct hisi_hba *hisi_hba, u32 state)
    {
    let mut new_state: u32 = hisi_hba.hw.get_phys_state(hisi_hba);
    struct asd_sas_port *_sas_port = core::ptr::null_mut();
    int phy_no;
    for (phy_no = 0; phy_no < hisi_hba.n_phy; phy_no++) {
    struct hisi_sas_phy *phy = &hisi_hba.phy[phy_no];
    struct asd_sas_phy *sas_phy = &phy.sas_phy;
    struct asd_sas_port *sas_port = sas_phy.port;
    let mut do_port_check: bool = _sas_port != sas_port;
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_reset_init_all_devices(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_terminate_stp_reject(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_controller_reset_prepare(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_async_init_wait_phyup(data: *mut c_void, cookie: async_cookie_t) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_controller_reset_done(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_controller_prereset(hisi_hba: *mut hisi_hba) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_controller_reset(hisi_hba: *mut hisi_hba) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_abort_task(task: *mut sas_task) -> c_int {
    static int hisi_sas_abort_task(struct sas_task *task)
    {
    let mut internal_abort_data: hisi_sas_internal_abort_data = { false };
    struct domain_device *device = task.dev;
    struct hisi_sas_device *sas_dev = device.lldd_dev;
    struct hisi_sas_slot *slot = task.lldd_task;
    struct hisi_hba *hisi_hba;
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_abort_task_set(device: *mut domain_device, lun: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_debug_I_T_nexus_reset(device: *mut domain_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_I_T_nexus_reset(device: *mut domain_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_lu_reset(device: *mut domain_device, lun: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_async_I_T_nexus_reset(data: *mut c_void, cookie: async_cookie_t) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_clear_nexus_ha(sas_ha: *mut sas_ha_struct) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_query_task(task: *mut sas_task) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_port_formed(sas_phy: *mut asd_sas_phy) {
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_phy_disconnected(phy: *mut hisi_sas_phy) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_phy_bcast(phy: *mut hisi_sas_phy) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_host_reset(shost: *mut Scsi_Host, reset_type: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_init_mem(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_alloc(hisi_hba: *mut hisi_hba) -> c_int {
...
// roundup to avoid overly large block size
    ...
    for (i = 0; i < blk_cnt; i++) {
    let mut slot_index: c_int = i * slots_per_blk;
    dma_addr_t buf_dma;
    void *buf;
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_free(hisi_hba: *mut hisi_hba) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_rst_work_handler(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_sync_rst_work_handler(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_get_fw_info(hisi_hba: *mut hisi_hba) -> c_int {
...
//
    ...
    refclk = devm_clk_get(dev, core::ptr::null_mut());
    if (IS_ERR(refclk))
    ...
    timer_setup(&hisi_hba.timer, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_interrupt_preinit(hisi_hba: *mut hisi_hba) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hisi_sas_remove(pdev: *mut platform_device) {
    ...
    let mut hisi_sas_debugfs_enable: bool = true;
    let mut hisi_sas_debugfs_dump_count: u32 = 50;

    bool hisi_sas_debugfs_enable;
    let mut hisi_sas_debugfs_dump_count: u32 = 1;
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_init() -> __init int {
    ...
    return -ENOMEM;
    if (hisi_sas_debugfs_enable) {
    hisi_sas_debugfs_dir = debugfs_create_dir("hisi_sas", core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn hisi_sas_exit() -> __exit void {
