...
//! Source: drivers/scsi/qla4xxx/ql4_os.c
#![no_std]
...
//
    let mut ql4xdisablesysfsboot: static int = 1;
    ...
    "\t\t  2 - debug logging");
    let mut ql4xenablemsix: c_int = 1;
...
pub const QL4_DEF_QDEPTH: c_int = 32;
    let mut ql4xmaxqdepth: static int = QL4_DEF_QDEPTH;
    module_param(ql4xmaxqdepth, int, S_IRUGO | S_IWUSR);
    MODULE_PARM_DESC(ql4xmaxqdepth,
    " Maximum queue depth to report for target devices.\n"
    "\t\t  Default: 32.");
    let mut ql4xqfulltracking: static int = 1;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_isp_check_reg(ha: *mut scsi_qla_host) -> c_int {
    static int qla4xxx_isp_check_reg(struct scsi_qla_host *ha)
    {
    let mut reg_val: u32 = 0;
    let mut rval: c_int = QLA_SUCCESS;
    if (is_qla8022(ha))
    reg_val = readl(&ha.qla4_82xx_reg.host_status);
#[no_mangle]
pub unsafe extern "C" fn if(is_qla8042(ha): is_qla8032(ha) ||) -> else {
...
#[no_mangle]
unsafe extern "C" fn qla4_attr_is_visible(param_type: c_int, param: c_int) -> umode_t {
    static umode_t qla4_attr_is_visible(int param_type, int param)
    {
    switch (param_type) {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_create_chap_list(ha: *mut scsi_qla_host) {
    static void qla4xxx_create_chap_list(struct scsi_qla_host *ha)
    {
    let mut rval: c_int = 0;
    uint8_t *chap_flash_data = core::ptr::null_mut();
    uint32_t offset;
    dma_addr_t chap_dma;
    let mut chap_size: u32 = 0;
    if (is_qla40XX(ha))
...
//
    ...
    if (rval != QLA_SUCCESS)
    goto exit_chap_list;
    if (ha.chap_list == core::ptr::null_mut())
...
//
    ...
    {
    int i, rval;
    let mut free_index: c_int = -1;
    let mut max_chap_entries: c_int = 0;
...
// chap_index = free_index;
    ...
    let mut max_chap_entries: c_int = 0;
    let mut valid_chap_entries: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn __qla4xxx_is_chap_active(dev: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_delete_chap(shost: *mut Scsi_Host, chap_tbl_idx: u16) -> c_int {
    static int qla4xxx_delete_chap(struct Scsi_Host *shost, uint16_t chap_tbl_idx)
    {
    struct scsi_qla_host *ha = to_qla_host(shost);
    struct ql4_chap_table *chap_table;
    ...
    if (chap_table == core::ptr::null_mut())
    return -ENOMEM;
    if (is_qla80XX(ha))
    max_chap_entries = (ha.hw.flt_chap_size / 2) /
    sizeof(struct ql4_chap_table);
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_set_chap_entry(shost: *mut Scsi_Host, data: *mut c_void, len: c_int) -> c_int {
    static int qla4xxx_set_chap_entry(struct Scsi_Host *shost, void *data, int len)
    {
    struct scsi_qla_host *ha = to_qla_host(shost);
    struct iscsi_chap_rec chap_rec;
    struct ql4_chap_table *chap_entry = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_get_host_stats(shost: *mut Scsi_Host, buf: *mut c_char, len: c_int) -> c_int {
    static int qla4xxx_get_host_stats(struct Scsi_Host *shost, char *buf, int len)
    {
    struct scsi_qla_host *ha = to_qla_host(shost);
    struct iscsi_offload_host_stats *host_stats = core::ptr::null_mut();
    int host_stats_size;
    let mut ret: c_int = 0;
    let mut ddb_idx: c_int = 0;
    struct ql_iscsi_stats *ql_iscsi_stats = core::ptr::null_mut();
    int stats_size;
    ...
    struct scsi_qla_host *ha = to_qla_host(shost);
    int ival;
    char *pval = core::ptr::null_mut();
    let mut len: c_int = -ENOSYS;
...
#[no_mangle]
pub unsafe extern "C" fn if(1: iface->iface_num ==) -> else {
    ...
    if (!shost) {
    ret = -ENXIO;
    pr_err("%s: shost is core::ptr::null_mut()\n", __func__);
    return ERR_PTR(ret);
    }
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_ep_poll(ep: *mut iscsi_endpoint, timeout_ms: c_int) -> c_int {
    static int qla4xxx_ep_poll(struct iscsi_endpoint *ep, int timeout_ms)
    {
    struct qla_endpoint *qla_ep;
    struct scsi_qla_host *ha;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_ep_disconnect(ep: *mut iscsi_endpoint) {
    static void qla4xxx_ep_disconnect(struct iscsi_endpoint *ep)
    {
    struct qla_endpoint *qla_ep;
    struct scsi_qla_host *ha;
    ...
    return -ENOTCONN;
#[no_mangle]
pub unsafe extern "C" fn iscsi_conn_get_addr_param(: *mut (struct sockaddr_storage) -> return {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_eh_cmd_timed_out(sc: *mut scsi_cmnd) -> enum scsi_timeout_action {
    static enum scsi_timeout_action qla4xxx_eh_cmd_timed_out(struct scsi_cmnd *sc)
    {
    struct iscsi_cls_session *session;
    unsigned long flags;
    let mut ret: enum scsi_timeout_action = SCSI_EH_NOT_HANDLED;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_set_port_speed(shost: *mut Scsi_Host) {
    static void qla4xxx_set_port_speed(struct Scsi_Host *shost)
    {
    struct scsi_qla_host *ha = to_qla_host(shost);
    struct iscsi_cls_host *ihost = shost.shost_data;
    let mut speed: u32 = ISCSI_PORT_SPEED_UNKNOWN;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_set_port_state(shost: *mut Scsi_Host) {
    static void qla4xxx_set_port_state(struct Scsi_Host *shost)
    {
    struct scsi_qla_host *ha = to_qla_host(shost);
    struct iscsi_cls_host *ihost = shost.shost_data;
    let mut state: u32 = ISCSI_PORT_STATE_DOWN;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_create_ipv4_iface(ha: *mut scsi_qla_host) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_create_ipv6_iface(ha: *mut scsi_qla_host) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_create_ifaces(ha: *mut scsi_qla_host) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_destroy_ipv4_iface(ha: *mut scsi_qla_host) {
    static void qla4xxx_destroy_ipv4_iface(struct scsi_qla_host *ha)
    {
    if (ha.iface_ipv4) {
    iscsi_destroy_iface(ha.iface_ipv4);
    ha.iface_ipv4 = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn qla4xxx_destroy_ipv6_iface(ha: *mut scsi_qla_host) {
    static void qla4xxx_destroy_ipv6_iface(struct scsi_qla_host *ha)
    {
    if (ha.iface_ipv6_0) {
    iscsi_destroy_iface(ha.iface_ipv6_0);
    ha.iface_ipv6_0 = core::ptr::null_mut();
    }
    if (ha.iface_ipv6_1) {
    iscsi_destroy_iface(ha.iface_ipv6_1);
    ha.iface_ipv6_1 = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn qla4xxx_destroy_ifaces(ha: *mut scsi_qla_host) {
...
// Autocfg applies to even interface
    ...
    ~IPV6_ADDOPT_NEIGHBOR_DISCOVERY_ADDR_ENABLE);
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_IPV6_AUTOCFG_ND_ENABLE: iface_param->value[0] ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_BOOTPROTO_STATIC: iface_param->value[0] ==) -> else {
    ...
    struct scsi_qla_host *ha = to_qla_host(shost);
    let mut rval: c_int = 0;
    struct iscsi_iface_param_info *iface_param = core::ptr::null_mut();
    struct addr_ctrl_blk *init_fw_cb = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_get_ddb_index(ha: *mut scsi_qla_host, ddb_index: *mut u16) -> c_int {
    int qla4xxx_get_ddb_index(struct scsi_qla_host *ha, uint16_t *ddb_index)
    {
    let mut mbx_sts: u32 = 0;
...
// ddb_index = tmp_ddb_index;
    ...
    let mut status: c_int = QLA_SUCCESS, ret = 0;
    ...
    struct iscsi_cls_conn *cls_conn)
    {
    let mut idx: c_int = 0, max_ddbs, rval;
    ...
    sess = cls_sess.dd_data;
    conn = cls_conn.dd_data;
    if (sess.targetname == core::ptr::null_mut() ||
    conn.persistent_address == core::ptr::null_mut() ||
    ...
    if (ddb_entry == core::ptr::null_mut())
    continue;
    if (ddb_entry.ddb_type != FLASH_DDB)
    continue;
    existing_sess = ddb_entry.sess.dd_data;
    existing_conn = ddb_entry.conn.dd_data;
    if (existing_sess.targetname == core::ptr::null_mut() ||
    existing_conn.persistent_address == core::ptr::null_mut() ||
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_session_destroy(cls_sess: *mut iscsi_cls_session) {
    ...
    struct dev_db_entry *fw_ddb_entry = core::ptr::null_mut();
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), &ddb_state, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_conn_start(cls_conn: *mut iscsi_cls_conn) -> c_int {
    ...
    let mut mbx_sts: u32 = 0;
    let mut ret: c_int = 0;
    let mut status: c_int = QLA_SUCCESS;
    sess = cls_sess.dd_data;
    ddb_entry = sess.dd_data;
    ha = ddb_entry.ha;
    DEBUG2(ql4_printk(KERN_INFO, ha, "%s: sid = %d, cid = %d\n", __func__,
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_conn_destroy(cls_conn: *mut iscsi_cls_conn) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_task_work(wdata: *mut work_struct) {
    static void qla4xxx_task_work(struct work_struct *wdata)
    {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_alloc_pdu(task: *mut iscsi_task, opcode: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_task_cleanup(task: *mut iscsi_task) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_task_xmit(task: *mut iscsi_task) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(DDB_NO_LINK: ddb_link ==) -> else {
    else if (ddb_link == DDB_NO_LINK)
    disc_parent = ISCSI_DISC_PARENT_UNKNOWN;
#[no_mangle]
pub unsafe extern "C" fn if(MAX_DDB_ENTRIES: ddb_link <) -> else {
    ...
    fw_ddb_entry_dma, core::ptr::null_mut(), core::ptr::null_mut(), &ddb_state,
    core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut()) == QLA_ERROR) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_stop_timer(ha: *mut scsi_qla_host) {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_mark_device_missing(cls_session: *mut iscsi_cls_session) {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_mark_all_devices_missing(ha: *mut scsi_qla_host) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_srb_free_dma(ha: *mut scsi_qla_host, srb: *mut srb) {
    ...
    qla4xxx_cmd_priv(cmd).srb = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_srb_compl(ref: *mut kref) {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_mem_free(ha: *mut scsi_qla_host) {
    ...
    vfree(ha.fw_dump);
    ha.queues_len = 0;
    ha.queues = core::ptr::null_mut();
    ha.queues_dma = 0;
    ha.request_ring = core::ptr::null_mut();
    ha.request_dma = 0;
    ha.response_ring = core::ptr::null_mut();
    ha.response_dma = 0;
    ha.shadow_regs = core::ptr::null_mut();
    ha.shadow_regs_dma = 0;
    ha.fw_dump = core::ptr::null_mut();
    ha.fw_dump_size = 0;
// Free srb pool.
    mempool_destroy(ha.srb_mempool);
    ha.srb_mempool = core::ptr::null_mut();
    dma_pool_destroy(ha.chap_dma_pool);
    vfree(ha.chap_list);
    ha.chap_list = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_mem_alloc(ha: *mut scsi_qla_host) -> c_int {
...
// Allocate contiguous block of DMA memory for queues.
    ...
    if (ha.queues == core::ptr::null_mut()) {
...
// Allocate memory for srb pool.
    ...
    if (ha.srb_mempool == core::ptr::null_mut()) {
    ql4_printk(KERN_WARNING, ha,
    ...
    if (ha.chap_dma_pool == core::ptr::null_mut()) {
    ql4_printk(KERN_WARNING, ha,
    ...
    if (ha.fw_ddb_dma_pool == core::ptr::null_mut()) {
    ql4_printk(KERN_WARNING, ha,
...
//
#[no_mangle]
unsafe extern "C" fn qla4_8xxx_check_temp(ha: *mut scsi_qla_host) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn qla4_8xxx_check_fw_alive(ha: *mut scsi_qla_host) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qla4_8xxx_process_fw_error(ha: *mut scsi_qla_host) {
    static void qla4_8xxx_process_fw_error(struct scsi_qla_host *ha)
    {
    uint32_t halt_status;
    let mut halt_status_unrecoverable: c_int = 0;
    ...
    __func__);
#[no_mangle]
pub unsafe extern "C" fn if(QLA83XX_HALT_STATUS_UNRECOVERABLE: halt_status &) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla4_8xxx_watchdog(ha: *mut scsi_qla_host) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_check_relogin_flash_ddb(cls_sess: *mut iscsi_cls_session) {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_timer(t: *mut timer_list) {
    static void qla4xxx_timer(struct timer_list *t)
    {
    struct scsi_qla_host *ha = timer_container_of(ha, t, timer);
    let mut start_dpc: c_int = 0;
    uint16_t w;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_cmd_wait(ha: *mut scsi_qla_host) -> c_int {
...
//
    if (cmd != core::ptr::null_mut() && qla4xxx_cmd_priv(cmd).srb)
    break;
    }
    spin_unlock_irqrestore(&ha.hardware_lock, flags);
...
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_hw_reset(ha: *mut scsi_qla_host) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_soft_reset(ha: *mut scsi_qla_host) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_abort_active_cmds(ha: *mut scsi_qla_host, res: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_dead_adapter_cleanup(ha: *mut scsi_qla_host) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_fail_session(cls_session: *mut iscsi_cls_session) {
    static void qla4xxx_fail_session(struct iscsi_cls_session *cls_session)
    {
    struct iscsi_session *sess;
    struct ddb_entry *ddb_entry;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_recover_adapter(ha: *mut scsi_qla_host) -> c_int {
    static int qla4xxx_recover_adapter(struct scsi_qla_host *ha)
    {
    let mut status: c_int = QLA_ERROR;
    let mut reset_chip: u8 = 0;
    uint32_t dev_state;
    unsigned long wait;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_relogin_devices(cls_session: *mut iscsi_cls_session) {
...
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_unblock_flash_ddb(cls_session: *mut iscsi_cls_session) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_unblock_ddb(cls_session: *mut iscsi_cls_session) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_relogin_all_devices(ha: *mut scsi_qla_host) {
    static void qla4xxx_relogin_all_devices(struct scsi_qla_host *ha)
    {
    iscsi_host_for_each_session(ha.host, qla4xxx_relogin_devices);
    }
#[no_mangle]
unsafe extern "C" fn qla4xxx_relogin_flash_ddb(cls_sess: *mut iscsi_cls_session) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_dpc_relogin(cls_sess: *mut iscsi_cls_session) {
...
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_wake_dpc(ha: *mut scsi_qla_host) {
    ...
    let mut size: u32 = sizeof(struct qla4_work_evt) + data_size;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_do_work(ha: *mut scsi_qla_host) {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_do_dpc(work: *mut work_struct) {
...
// disable pause frame for ISP83xx
    ...
    if (test_bit(DPC_RESET_HA_INTR, &ha.dpc_flags)) {
    let mut wait_time: u8 = RESET_INTR_TOV;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_free_adapter(ha: *mut scsi_qla_host) {
...
#[no_mangle]
pub unsafe extern "C" fn qla4_8xxx_iospace_config(ha: *mut scsi_qla_host) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_iospace_config(ha: *mut scsi_qla_host) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_rd_shdw_req_q_out(ha: *mut scsi_qla_host) -> u16 {
    uint16_t qla4xxx_rd_shdw_req_q_out(struct scsi_qla_host *ha)
    {
    return (uint16_t)le32_to_cpu(ha.shadow_regs.req_q_out);
    }
#[no_mangle]
pub unsafe extern "C" fn qla4_82xx_rd_shdw_req_q_out(ha: *mut scsi_qla_host) -> u16 {
    uint16_t qla4_82xx_rd_shdw_req_q_out(struct scsi_qla_host *ha)
    {
    return (uint16_t)le32_to_cpu(readl(&ha.qla4_82xx_reg.req_q_out));
    }
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_rd_shdw_rsp_q_in(ha: *mut scsi_qla_host) -> u16 {
    uint16_t qla4xxx_rd_shdw_rsp_q_in(struct scsi_qla_host *ha)
    {
    return (uint16_t)le32_to_cpu(ha.shadow_regs.rsp_q_in);
    }
#[no_mangle]
pub unsafe extern "C" fn qla4_82xx_rd_shdw_rsp_q_in(ha: *mut scsi_qla_host) -> u16 {
    uint16_t qla4_82xx_rd_shdw_rsp_q_in(struct scsi_qla_host *ha)
    {
    return (uint16_t)le32_to_cpu(readl(&ha.qla4_82xx_reg.rsp_q_in));
    }
#[no_mangle]
unsafe extern "C" fn qla4xxx_show_boot_eth_info(data: *mut c_void, type: c_int, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_eth_get_attr_visibility(data: *mut c_void, type: c_int) -> umode_t {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_show_boot_ini_info(data: *mut c_void, type: c_int, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_ini_get_attr_visibility(data: *mut c_void, type: c_int) -> umode_t {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_show_boot_tgt_pri_info(data: *mut c_void, type: c_int, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_show_boot_tgt_sec_info(data: *mut c_void, type: c_int, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_tgt_get_attr_visibility(data: *mut c_void, type: c_int) -> umode_t {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_boot_release(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn get_fw_boot_info(ha: *mut scsi_qla_host, ddb_index[]: u16) -> c_int {
    ...
    uint16_t func_num;
    uint8_t val;
    uint8_t *buf = core::ptr::null_mut();
    let mut size: usize = 13 * sizeof(uint8_t);
    let mut ret: c_int = QLA_SUCCESS;
...
#[no_mangle]
pub unsafe extern "C" fn if(1: ha->port_num ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn sizeof(QLA_SUCCESS: uint8_t)) !=) -> *mut 13 {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_get_boot_info(ha: *mut scsi_qla_host) -> c_int {
    static int qla4xxx_get_boot_info(struct scsi_qla_host *ha)
    {
    uint16_t ddb_index[2];
    let mut ret: c_int = QLA_ERROR;
    int rval;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_setup_boot_info(ha: *mut scsi_qla_host) -> c_int {
    ...
    if (flash_isid == core::ptr::null_mut())
...
//
    ...
    {
    struct ddb_entry *ddb_entry;
    struct ql4_tuple_ddb *fw_tddb = core::ptr::null_mut();
    struct ql4_tuple_ddb *tmp_tddb = core::ptr::null_mut();
    int idx;
    let mut ret: c_int = QLA_ERROR;
    ...
    qla4xxx_convert_param_ddb(fw_ddb_entry, fw_tddb, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_check_existing_isid(list_nt: *mut list_head, isid: *mut u8) -> c_int {
...
// found duplicate ddb
    ...
    list_for_each_entry_safe(nt_ddb_idx, nt_ddb_idx_tmp, list_nt, list) {
    qla4xxx_convert_param_ddb(&nt_ddb_idx.fw_ddb, tmp_tddb, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_free_ddb_list(list_ddb: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_verify_boot_idx(ha: *mut scsi_qla_host, idx: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_wait_for_ip_configuration(ha: *mut scsi_qla_host) {
...
// Break if all IP states checked
    ...
    {
    let mut options: u16 = 0;
    let mut ip_len: usize = IP_ADDR_LEN;
    options = le16_to_cpu(fw_ddb_entry.options);
    if (options & DDB_OPT_IPV6_DEVICE)
    ...
    let mut idx: u32 = 0;
    int max_ddbs;
    let mut ret: c_int = QLA_ERROR, status;
    max_ddbs =  is_qla40XX(ha) ? MAX_DEV_DB_ENTRIES_40XX :
    MAX_DEV_DB_ENTRIES;
    flash_ddb_entry = dma_pool_alloc(ha.fw_ddb_dma_pool, GFP_KERNEL,
    &flash_ddb_entry_dma);
    if (flash_ddb_entry == core::ptr::null_mut() || fw_ddb_entry == core::ptr::null_mut()) {
...
// flash_index = idx;
    ...
    int ret;
    let mut idx: u32 = 0, next_idx = 0;
    let mut state: u32 = 0, conn_err = 0;
    let mut flash_index: u32 = -1;
    let mut conn_id: u16 = 0;
    fw_ddb_entry = dma_pool_alloc(ha.fw_ddb_dma_pool, GFP_KERNEL,
    &fw_ddb_dma);
    if (fw_ddb_entry == core::ptr::null_mut()) {
    ...
    core::ptr::null_mut(), &next_idx, &state,
    &conn_err, core::ptr::null_mut(), &conn_id);
    if (ret == QLA_ERROR)
    break;
...
//
    ...
    core::ptr::null_mut(), 0, core::ptr::null_mut(), &next_idx, &state,
    &conn_err, core::ptr::null_mut(), core::ptr::null_mut());
    if (ret == QLA_ERROR)
    continue;
    ...
    struct iscsi_cls_session *cls_sess;
    struct iscsi_session *sess;
    let mut max_ddbs: u32 = 0;
    let mut ddb_link: u16 = -1;
    max_ddbs =  is_qla40XX(ha) ? MAX_DEV_DB_ENTRIES_40XX :
    MAX_DEV_DB_ENTRIES;
    cls_sess = ddb_entry.sess;
    sess = cls_sess.dd_data;
    ...
    struct iscsi_cls_conn *cls_conn;
    struct iscsi_endpoint *ep;
    let mut cmds_max: u16 = 32;
    let mut conn_id: u16 = 0;
    let mut initial_cmdsn: u32 = 0;
    let mut ret: c_int = QLA_SUCCESS;
    struct ddb_entry *ddb_entry = core::ptr::null_mut();
...
// Check if NT, then add it to list
    ...
    break;
    nt_ddb_idx.fw_ddb_idx = idx;
    ret = qla4xxx_is_session_exists(ha, fw_ddb_entry, core::ptr::null_mut());
    if (ret == QLA_SUCCESS) {
// free nt_ddb_idx and do not add to list_nt
    vfree(nt_ddb_idx);
    goto continue_next_new_nt;
...
//
    ...
    struct iscsi_bus_flash_conn *fnode_conn = core::ptr::null_mut();
    let mut rc: c_int = QLA_ERROR;
...
//
    ...
    struct device *dev;
    let mut idx: u16 = 0;
    let mut max_ddbs: u16 = 0;
    let mut options: u32 = 0;
    let mut rval: u32 = QLA_ERROR;
    ...
    dev = iscsi_find_flashnode_sess(ha.host, core::ptr::null_mut(),
...
//
    ...
    struct scsi_qla_host *ha = to_qla_host(shost);
    let mut dev_db_start_offset: u32 = FLASH_OFFSET_DB_INFO;
    struct dev_db_entry *fw_ddb_entry = core::ptr::null_mut();
    dma_addr_t fw_ddb_entry_dma;
    let mut options: u32 = 0;
    let mut rval: c_int = 0;
    ...
    uint16_t idx)
    {
    struct dev_db_entry *ddb_entry = core::ptr::null_mut();
...
// To ensure that sendtargets is done, wait for at least 12 secs
    ...
    do {
    ret = qla4xxx_get_fwddb_entry(ha, idx, core::ptr::null_mut(), 0, core::ptr::null_mut(),
    core::ptr::null_mut(), &state, &conn_err, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (ret == QLA_ERROR)
    continue;
    if (state == DDB_DS_NO_CONNECTION_ACTIVE ||
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_sysfs_ddb_logout_sid(cls_sess: *mut iscsi_cls_session) -> c_int {
    static int qla4xxx_sysfs_ddb_logout_sid(struct iscsi_cls_session *cls_sess)
    {
    struct iscsi_session *sess;
    struct ddb_entry *ddb_entry = core::ptr::null_mut();
...
//
    ...
    struct Scsi_Host *shost = iscsi_flash_session_to_shost(fnode_sess);
    struct scsi_qla_host *ha = to_qla_host(shost);
    struct ql4_tuple_ddb *flash_tddb = core::ptr::null_mut();
    struct ql4_tuple_ddb *tmp_tddb = core::ptr::null_mut();
    struct dev_db_entry *fw_ddb_entry = core::ptr::null_mut();
    struct ddb_entry *ddb_entry = core::ptr::null_mut();
    dma_addr_t fw_ddb_dma;
    ...
    fw_ddb_dma, core::ptr::null_mut(), &next_idx,
    &state, &conn_err, core::ptr::null_mut(),
    &conn_id);
    if (status == QLA_ERROR) {
    ret = -ENOMEM;
    break;
    }
    qla4xxx_convert_param_ddb(fw_ddb_entry, tmp_tddb, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(DDB_NO_LINK: fnode_sess->discovery_parent_type ==) -> else {
    else if (fnode_sess.discovery_parent_type == DDB_NO_LINK)
    parent_type = ISCSI_DISC_PARENT_UNKNOWN;
#[no_mangle]
pub unsafe extern "C" fn if(MAX_DDB_ENTRIES: fnode_sess->discovery_parent_type <) -> else {
...
//
    ...
    struct ql4_chap_table chap_tbl;
    struct nlattr *attr;
    let mut chap_out_idx: u16 = INVALID_ENTRY;
    let mut rc: c_int = QLA_ERROR;
    let mut rem: u32 = len;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_sysfs_ddb_delete(fnode_sess: *mut iscsi_bus_flash_session) -> c_int {
    ...
    struct dev_db_entry *fw_ddb_entry = core::ptr::null_mut();
    dma_addr_t fw_ddb_entry_dma;
    uint16_t *ddb_cookie = core::ptr::null_mut();
    let mut ddb_size: usize = 0;
    void *pddb = core::ptr::null_mut();
    int target_id;
    let mut rc: c_int = 0;
    if (fnode_sess.is_boot_target) {
    rc = -EPERM;
...
//
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_sysfs_ddb_export(ha: *mut scsi_qla_host) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_sysfs_ddb_remove(ha: *mut scsi_qla_host) {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla4xxx_build_ddb_list(ha: *mut scsi_qla_host, is_reset: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_wait_login_resp_boot_tgt(ha: *mut scsi_qla_host) {
...
//
    ...
    {
    let mut ret: c_int = -ENODEV, status;
    struct Scsi_Host *host;
    struct scsi_qla_host *ha;
    let mut init_retry_count: u8 = 0;
    char buf[34];
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_prevent_other_port_reinit(ha: *mut scsi_qla_host) {
    static void qla4xxx_prevent_other_port_reinit(struct scsi_qla_host *ha)
    {
    struct scsi_qla_host *other_ha = core::ptr::null_mut();
    struct pci_dev *other_pdev = core::ptr::null_mut();
    let mut fn: c_int = ISP4XXX_PCI_FN_2;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_destroy_fw_ddb_session(ha: *mut scsi_qla_host) {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_remove_adapter(pdev: *mut pci_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_config_dma_addressing(ha: *mut scsi_qla_host) {
    static void qla4xxx_config_dma_addressing(struct scsi_qla_host *ha)
    {
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_sdev_init(sdev: *mut scsi_device) -> c_int {
    static int qla4xxx_sdev_init(struct scsi_device *sdev)
    {
    struct iscsi_cls_session *cls_sess;
    struct iscsi_session *sess;
    struct ddb_entry *ddb;
    let mut queue_depth: c_int = QL4_DEF_QDEPTH;
...
//
    ...
    uint32_t index)
    {
    struct srb *srb = core::ptr::null_mut();
    struct scsi_cmnd *cmd = core::ptr::null_mut();
    cmd = scsi_host_find_tag(ha.host, index);
    if (!cmd)
    return srb;
...
//
    ...
    struct srb *rp;
    let mut max_wait_time: u32 = EH_WAIT_CMD_TOV;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_wait_for_hba_online(ha: *mut scsi_qla_host) -> c_int {
    static int qla4xxx_wait_for_hba_online(struct scsi_qla_host *ha)
    {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_eh_abort(cmd: *mut scsi_cmnd) -> c_int {
    static int qla4xxx_eh_abort(struct scsi_cmnd *cmd)
    {
    struct scsi_qla_host *ha = to_qla_host(cmd.device.host);
    let mut id: c_uint = cmd.device.id;
    let mut lun: u64 = cmd.device.lun;
    unsigned long flags;
    struct srb *srb = core::ptr::null_mut();
    let mut ret: c_int = SUCCESS;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_eh_device_reset(cmd: *mut scsi_cmnd) -> c_int {
    static int qla4xxx_eh_device_reset(struct scsi_cmnd *cmd)
    {
    struct scsi_qla_host *ha = to_qla_host(cmd.device.host);
    struct ddb_entry *ddb_entry = cmd.device.hostdata;
    let mut ret: c_int = FAILED, stat;
    int rval;
    if (!ddb_entry)
    return ret;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_eh_target_reset(cmd: *mut scsi_cmnd) -> c_int {
    static int qla4xxx_eh_target_reset(struct scsi_cmnd *cmd)
    {
    struct scsi_qla_host *ha = to_qla_host(cmd.device.host);
    struct ddb_entry *ddb_entry = cmd.device.hostdata;
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_is_eh_active(shost: *mut Scsi_Host) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn qla4xxx_eh_host_reset(cmd: *mut scsi_cmnd) -> c_int {
    static int qla4xxx_eh_host_reset(struct scsi_cmnd *cmd)
    {
    let mut return_status: c_int = FAILED;
    struct scsi_qla_host *ha;
    int rval;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_context_reset(ha: *mut scsi_qla_host) -> c_int {
    static int qla4xxx_context_reset(struct scsi_qla_host *ha)
    {
    uint32_t mbox_cmd[MBOX_REG_COUNT];
    uint32_t mbox_sts[MBOX_REG_COUNT];
    struct addr_ctrl_blk_def *acb = core::ptr::null_mut();
    let mut acb_len: u32 = sizeof(struct addr_ctrl_blk_def);
    let mut rval: c_int = QLA_SUCCESS;
    dma_addr_t acb_dma;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_host_reset(shost: *mut Scsi_Host, reset_type: c_int) -> c_int {
    static int qla4xxx_host_reset(struct Scsi_Host *shost, int reset_type)
    {
    struct scsi_qla_host *ha = to_qla_host(shost);
    let mut rval: c_int = QLA_SUCCESS;
...
#[no_mangle]
unsafe extern "C" fn qla4_8xxx_error_recovery(ha: *mut scsi_qla_host) -> u32 {
...
// Clear driver state register
    ...
    {
    let mut ret: pci_ers_result_t = PCI_ERS_RESULT_DISCONNECT;
    struct scsi_qla_host *ha = pci_get_drvdata(pdev);
    int rc;
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_module_init() -> int __init {
...
// Allocate cache for SRBs.
    ...
    if (srb_cachep == core::ptr::null_mut()) {
    printk(KERN_ERR
...
#[no_mangle]
unsafe extern "C" fn qla4xxx_module_exit() -> void __exit {
