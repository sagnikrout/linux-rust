...
//! Source: drivers/scsi/qla2xxx/qla_isr.c
#![no_std]
...
//
    ...
    {
    void *pkt = &item.iocb;
    let mut pkt_size: u16 = item.size;
...
pub const SFP_DISABLE_LASER_INITIATED: c_uint = 0x15  /* Sub code of 8070 AEN */;
pub const SFP_ENABLE_LASER_INITIATED: c_uint = 0x16  /* Sub code of 8070 AEN */;
...
//
    ...
    struct abts_entry_24xx *abts = core::ptr::null_mut();
    struct abts_entry_24xx_ext *abts_ext = core::ptr::null_mut();
...
//
    ...
    u8 *data;
    u32 data_sz;
    let mut no_bytes: u16 = 0, total_bytes = 0, pending_bytes = 0;
    let mut buffer_copy_offset: u16 = 0;
    uint16_t entry_count_remaining;
    u16 tpad;
    entry_count_remaining = purex.entry_count;
...
//
    ...
    if (!rsp) {
    ql_log(ql_log_info, core::ptr::null_mut(), 0x505d,
    "%s: core::ptr::null_mut() response queue pointer.\n", __func__);
    return (IRQ_NONE);
    }
    ha = rsp.hw;
    reg = &ha.iobase.isp;
...
//
    ...
    rsp = (struct rsp_que *) dev_id;
    if (!rsp) {
    ql_log(ql_log_info, core::ptr::null_mut(), 0x5058,
    "%s: core::ptr::null_mut() response queue pointer.\n", __func__);
    return (IRQ_NONE);
    }
    ha = rsp.hw;
...
#[no_mangle]
pub unsafe extern "C" fn if(BIT_0: mboxes &) -> else {
...
// Seed data -- mailbox1 -> mailbox7.
    ...
    wptr = &reg24.mailbox1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_QLA8044(vha->hw)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(0x13: speed ==) -> else {
    else if (speed == 0x13)
    return link_speeds[QLA_LAST_SPEED];
#[no_mangle]
pub unsafe extern "C" fn if(QLA_LAST_SPEED: speed <) -> else {
...
//
    ...
    bool byte_order)
    {
    struct purex_entry_24xx_ext *purex_ext = core::ptr::null_mut();
    struct purex_entry_24xx *purex = core::ptr::null_mut();
    struct pt_ls4_rx_unsol *purls = core::ptr::null_mut();
    struct qla_hw_data *ha = vha.hw;
    struct rsp_que *rsp_q = *rsp;
    let mut no_bytes: u16 = 0, total_bytes = 0, pending_bytes = 0;
    let mut buffer_copy_offset: u16 = 0, payload_size = 0;
    uint16_t entry_count, entry_count_remaining;
    struct purex_item *item;
    void *iocb_pkt = core::ptr::null_mut();
    u8 *data;
    u32 data_sz;
    if (is_purls) {
    purls = *pkt;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_QLA29XX(ha)) -> else {
    else if (IS_QLA29XX(ha))
...
// pkt = rsp_q->ring_ptr;
    ...
    return f;
#[no_mangle]
pub unsafe extern "C" fn if(0: f->deleted ==) -> else {
    else if (f.deleted == 0)
    return f;
    }
    }
...
// Shall be called only on supported adapters.
    ...
    {
    struct qla_hw_data *ha = vha.hw;
    let mut reset_isp_needed: bool = false;
    ...
    {
    struct purex_item *item = core::ptr::null_mut();
    let mut item_hdr_size: u8 = sizeof(*item);
...
//
    ...
    uint16_t entry_count, entry_count_remaining;
    struct purex_item *item;
    void *fpin_pkt = core::ptr::null_mut();
    total_bytes = (le16_to_cpu(purex.frame_size) & 0x0FFF)
    - PURX_ELS_HEADER_SIZE;
    if (total_bytes > QLA_MAX_IOCB_SIZE)
    total_bytes = QLA_MAX_IOCB_SIZE;
...
//
    ...
    let mut result: c_uint = mb[1] >> 0 & 0x3;
    let mut type: c_uint = mb[1] >> 6 & 0x3;
    let mut sw: c_uint = mb[1] >> 15 & 0x1;
    ql_dbg(ql_dbg_async, vha, 0x5052,
    ...
    "user stopped" };
    let mut reason: c_uint = mb[2] >> 0 & 0xf;
    let mut phase: c_uint = mb[2] >> 12 & 0xf;
    ql_dbg(ql_dbg_async, vha, 0x5052,
...
// Free outstanding command slot.
    req.outstanding_cmds[index] = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(BIT_1: le16_to_cpu(mbx->mb1) &) -> else {
...
//
    ...
    let mut ret: u16 = QLA_SUCCESS;
    let mut comp_status: __le16 = sts.comp_status;
    let mut logit: c_int = 0;
    iocb = &sp.u.iocb_cmd;
    fcport = sp.fcport;
...
//
    ...
    let mut lba_s: sector_t = scsi_get_lba(cmd);
...
// check appl tag
    ...
    uint16_t thread_id;
    let mut rval: u32 = EXT_STATUS_OK;
    struct bsg_job *bsg_job = core::ptr::null_mut();
...
// Validate handle.
    ...
    req.outstanding_cmds[handle] = core::ptr::null_mut();
...
//
    ...
    resid_len, fw_resid_len, sp, cp);
    if (rsp.status_srb == core::ptr::null_mut())
    sp.done(sp, res);
...
//
    ...
    "cmd is core::ptr::null_mut(): already returned to OS (sp=%p).\n", sp);
...
//
    ...
    let mut que: u16 = MSW(pkt.handle);
    struct req_que *req = core::ptr::null_mut();
    let mut res: c_int = DID_ERROR << 16;
    u16 index;
...
// Load return mailbox registers.
    ...
    srb_t *sp;
    srb_t *orig_sp = core::ptr::null_mut();
    struct srb_iocb *abt;
    sp = qla2x00_get_sp_from_handle(vha, func, req, pkt);
    if (!sp)
    return;
...
//
    ...
    let mut rsp_in: u16 = 0, cur_ring_index;
    int is_shadow_hba;
...
// ensure that the ATIO queue is empty
    ...
    purex_entry = core::ptr::null_mut();
    } else {
    purex_entry = (void *)pkt;
    purex_entry_ext = core::ptr::null_mut();
    }
...
//
    ...
    struct rsp_que *rsp;
    unsigned long	flags;
    let mut process_atio: bool = false;
    rsp = (struct rsp_que *) dev_id;
    if (!rsp) {
    ql_log(ql_log_info, core::ptr::null_mut(), 0x5059,
    "%s: core::ptr::null_mut() response queue pointer.\n", __func__);
    return IRQ_NONE;
    }
    ha = rsp.hw;
    ...
    rsp = (struct rsp_que *) dev_id;
    if (!rsp) {
    ql_log(ql_log_info, core::ptr::null_mut(), 0x505a,
    "%s: core::ptr::null_mut() response queue pointer.\n", __func__);
    return IRQ_NONE;
    }
    ha = rsp.hw;
    ...
    rsp = (struct rsp_que *) dev_id;
    if (!rsp) {
    ql_log(ql_log_info, core::ptr::null_mut(), 0x505c,
    "%s: core::ptr::null_mut() response queue pointer.\n", __func__);
    return IRQ_NONE;
    }
    ha = rsp.hw;
    ...
    qpair = dev_id;
    if (!qpair) {
    ql_log(ql_log_info, core::ptr::null_mut(), 0x505b,
    "%s: core::ptr::null_mut() response queue pointer.\n", __func__);
    return IRQ_NONE;
    }
    ha = qpair.hw;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qla_init_msix_entry {
    ...
    let mut min_vecs: c_int = QLA_BASE_VECTORS;
...
// ATIOQ needs 1 vector. That's 1 less QPair
    ...
    qentry.have_irq = 0;
    qentry.in_use = 0;
    qentry.handle = core::ptr::null_mut();
    }
...
//
    ...
    qentry = &ha.msix_entries[i];
    if (qentry.have_irq) {
    irq_set_affinity_notifier(qentry.vector, core::ptr::null_mut());
    free_irq(pci_irq_vector(ha.pdev, i), qentry.handle);
    }
    }
    kfree(ha.msix_entries);
    ha.msix_entries = core::ptr::null_mut();
