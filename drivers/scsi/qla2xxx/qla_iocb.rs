...
//! Source: drivers/scsi/qla2xxx/qla_iocb.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn qla2xxx_get_next_handle(req: *mut req_que) -> u32 {
...
//
    mrk24 = __qla2x00_alloc_iocbs(qpair, core::ptr::null_mut());
    if (!mrk24) {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla2x00_issue_marker(vha: *mut scsi_qla_host_t, ha_locked: c_int) -> c_int {
    ...
    uint16_t tot_dsds)
    {
    struct dsd64 *cur_dsd = core::ptr::null_mut(), *next_dsd;
    struct scsi_cmnd *cmd;
    struct	scatterlist *cur_seg;
    uint8_t avail_dsds;
    let mut first_iocb: u8 = 1;
...
// partial = 1;
    ...
    {
    void *next_dsd;
    let mut avail_dsds: u8 = 0;
    uint32_t dsd_list_len;
    struct dsd_dma *dsd_ptr;
    struct scatterlist *sg_prot;
    struct dsd64 *cur_dsd = dsd;
    let mut used_dsds: u16 = tot_dsds;
...
// Null termination
    ...
    {
    struct dsd_dma *dsd_ptr = core::ptr::null_mut(), *dif_dsd, *nxt_dsd;
    struct scatterlist *sg, *sgl;
    struct crc_context *difctx = core::ptr::null_mut();
    struct scsi_qla_host *vha;
    uint dsd_list_len;
    let mut avail_dsds: c_uint = 0;
    let mut used_dsds: c_uint = tot_dsds;
    let mut dif_local_dma_alloc: bool = false;
    let mut direction_to_device: bool = false;
    int i;
    if (sp) {
    struct scsi_cmnd *cmd = GET_CMD_SP(sp);
...
// if initiator doing write or target doing read
    ...
    for_each_sg(sgl, sg, tot_dsds, i) {
    let mut sle_phys: u64 = sg_phys(sg);
// If SGE addr + len flips bits in upper 32-bits
    ...
    vha, 0xe022,
    "%s: difctx pointer is core::ptr::null_mut()\n",
    __func__);
    }
    break;
    }
    ...
    let mut track_difbundl_buf: u32 = 0;
    let mut ldma_sg_len: u32 = 0;
    let mut ldma_needed: u8 = 1;
...
//
    ...
    uint32_t		data_bytes;
    uint32_t		dif_bytes;
    let mut bundling: u8 = 1;
    uint16_t		blk_size;
    struct crc_context	*crc_ctx_pkt = core::ptr::null_mut();
    struct qla_hw_data	*ha;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_PI_UNINIT_CAPABLE(ha)) -> else {
...
// Walks data segments
    ...
    cur_dsd, tot_dsds, core::ptr::null_mut()))
    goto crc_queuing_error;
    } else if (qla24xx_walk_and_build_sglist(ha, sp, cur_dsd,
    (tot_dsds - tot_prot_dsds), core::ptr::null_mut()))
    goto crc_queuing_error;
    if (bundling && tot_prot_dsds) {
// Walks dif segments
    ...
    tot_prot_dsds, core::ptr::null_mut()))
    goto crc_queuing_error;
    }
    return QLA_SUCCESS;
    crc_queuing_error:
...
//
    ...
    uint32_t		handle;
    uint16_t		cnt;
    let mut req_cnt: u16 = 0;
    uint16_t		tot_dsds;
    uint16_t		tot_prot_dsds;
    let mut fw_prot_opts: u16 = 0;
    struct req_que		*req = core::ptr::null_mut();
...
// hardware_lock assumed to be held.
    ...
    request_t *pkt = core::ptr::null_mut();
    req_cnt = 1;
    handle = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_FWI2_CAPABLE(ha)) -> else {
    else if (IS_FWI2_CAPABLE(ha))
    cnt = rd_reg_dword(&reg.isp24.req_q_out);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_QLAFX00(ha)) -> else {
    else if (IS_QLAFX00(ha))
...
//
    ...
    if (IS_QLAFX00(ha)) {
    memset_io((void __iomem  *)pkt, 0,
    qla_req_entry_size(ha));
    wrt_reg_byte((u8  __iomem *)&pkt.entry_count, req_cnt);
    wrt_reg_dword((__le32  __iomem *)&pkt.handle, handle);
    } else {
...
// Set service parameter BIT_9 for PI control support
    ...
    {
    struct logio_entry_24xx *logio = pkt;
    let mut control_flags: u16 = LCF_COMMAND_LOGO;
    logio.entry_type = LOGINOUT_PORT_IOCB_TYPE;
...
// GPSC & GFPNID use fcport->ct_desc.ct_sns for both req & rsp
    ...
    sp.u.iocb_cmd.u.ctarg.req = core::ptr::null_mut();
    }
    ...
    sp.u.iocb_cmd.u.ctarg.rsp = core::ptr::null_mut();
    }
    break;
    default:
    break;
...
#[no_mangle]
unsafe extern "C" fn qla2x00_els_dcmd_sp_free(sp: *mut srb_t) {
...
#[no_mangle]
unsafe extern "C" fn qla2x00_els_dcmd_sp_done(sp: *mut srb_t, res: c_int) {
    ...
    struct srb_iocb *elsio = core::ptr::null_mut();
    struct qla_hw_data *ha = vha.hw;
...
#[no_mangle]
pub unsafe extern "C" fn qla2x00_els_dcmd2_free(vha: *mut scsi_qla_host_t, els_plogi: *mut els_plogi) {
...
#[no_mangle]
unsafe extern "C" fn qla2x00_els_dcmd2_sp_done(sp: *mut srb_t, res: c_int) {
...
// write, read and verify logic
    ...
    if (ql2xdbwr)
    qla82xx_wr_32(ha, (uintptr_t )ha.nxdb_wr_ptr, dbval);
    else {
...
// Manage unprocessed RIO/ZIO commands in response queue.
    ...
    sp.u.scmd.crc_ctx = core::ptr::null_mut();
    }
    spin_unlock_irqrestore(&ha.hardware_lock, flags);
    return QLA_FUNCTION_FAILED;
    }
...
#[no_mangle]
unsafe extern "C" fn qla_get_iocbs_resource(sp: *mut srb) -> c_int {
    static int qla_get_iocbs_resource(struct srb *sp)
    {
    bool get_exch;
    let mut push_it_through: bool = false;
...
//
    ...
    uint16_t avail_dsds;
    struct dsd64 *cur_dsd;
    let mut req_data_len: u32 = 0;
    let mut rsp_data_len: u32 = 0;
    struct scatterlist *sg;
    int index;
    let mut entry_count: c_int = 1;
    struct bsg_job *bsg_job = sp.u.bsg_job;
...
// This value should be same as number of IOCB required for this cmd
    ...
    uint16_t cnt;
    uint32_t *clr_ptr;
    struct cmd_bidir *cmd_pkt = core::ptr::null_mut();
    struct rsp_que *rsp;
    struct req_que *req;
    let mut rval: c_int = EXT_STATUS_OK;
    rval = QLA_SUCCESS;
    rsp = ha.rsp_q_map[0];
    req = vha.req;
//
