...
//! Source: drivers/scsi/qla2xxx/qla_sup.c
#![no_std]
...
//
    ...
    {
pub const FLASH_BLK_SIZE_4K: c_uint = 0x1000;
pub const FLASH_BLK_SIZE_32K: c_uint = 0x8000;
pub const FLASH_BLK_SIZE_64K: c_uint = 0x10000;
    struct qla_hw_data *ha = vha.hw;
    struct req_que *req = ha.req_q_map[0];
    uint16_t cnt, chksum;
    __le16 *wptr = ( __le16 *)req.ring;
    struct qla_fdt_layout *fdt = (struct qla_fdt_layout *)req.ring;
    let mut fdt_options: u32 = 0;
    void *buf = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn set_segment_bits(options: *mut u16, segment_index: c_int, total: c_int) {
...
// options |= (1 << 10) | (1 << 11);
#[no_mangle]
pub unsafe extern "C" fn if(0: segment_index ==) -> else {
    else if (segment_index == 0)
// options |= (1 << 10);
#[no_mangle]
pub unsafe extern "C" fn if(1: segment_index == total -) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn set_chunk_bits(options: *mut u16, count: c_int, total: c_int) {
...
// options |= (1 << 4);
#[no_mangle]
pub unsafe extern "C" fn if(1: count == total -) -> else {
...
#[no_mangle]
unsafe extern "C" fn set_chunk_mpi_bits(options: *mut u16, count: c_int, total: c_int) {
...
// start = (le16_to_cpu(fltl->start_hi) << 16 |
    ...
    0 };
    struct qla_hw_data *ha = vha.hw;
    let mut def: u32 = IS_QLA81XX(ha) ? 2 : IS_QLA25XX(ha) ? 1 : 0;
...
// Default to 64 kb sector size.
    ...
    {
pub const QLA82XX_IDC_PARAM_ADDR: c_uint = 0x003e885c;
    __le32 *wptr;
    struct qla_hw_data *ha = vha.hw;
    struct req_que *req = ha.req_q_map[0];
    if (!(IS_P3P_TYPE(ha)))
    return;
    wptr = ( __le32 *)req.ring;
...
#[no_mangle]
unsafe extern "C" fn qla28xx_validate_mcu_signature(vha: *mut scsi_qla_host_t) -> c_int {
...
// Set the modified gpio_data values.
    ...
    {
    let mut led_select_value: u32 = 0;
...
// Extract SFUB from end of file
    ...
    dma_addr_t sfub_dma;
    let mut offset: u32 = faddr << 2;
    let mut buf_size_without_sfub: u32 = 0;
    struct qla_flt_region region;
    let mut reset_to_rom: bool = false;
    uint32_t risc_size, risc_attr = 0;
    __be32 *fw_array = core::ptr::null_mut();
...
//
pub const QLA29XX_BOOT_PROBE_LEN: c_uint = 0x100;
    static void
...
// revalidate the entire FCP priority config data, including entries
    ...
    ha.fcp_prio_cfg = core::ptr::null_mut();
    return QLA_FUNCTION_FAILED;
    }
