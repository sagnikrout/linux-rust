...
//! Source: drivers/scsi/qla2xxx/qla_mid.c
#![no_std]
...
//
    ...
    ha.req_q_map[que_id] = core::ptr::null_mut();
...
// Flush any queued response work before freeing the queue/qpair.
    ...
    if (que_id) {
    mutex_lock(&ha.mq_lock);
    ha.rsp_q_map[que_id] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn qla_do_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn qla_ctrlvp_sp_done(sp: *mut srb_t, res: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn qla24xx_control_vp(vha: *mut scsi_qla_host_t, cmd: c_int) -> c_int {
    int qla24xx_control_vp(scsi_qla_host_t *vha, int cmd)
    {
    let mut rval: c_int = QLA_MEMORY_ALLOC_FAILED;
    struct qla_hw_data *ha = vha.hw;
    let mut vp_index: c_int = vha.vp_idx;
    struct scsi_qla_host *base_vha = pci_get_drvdata(ha.pdev);
...
//
    ...
    return QLA_PARAMETER_ERROR;
// ref: INIT
    sp = qla2x00_get_sp(base_vha, core::ptr::null_mut(), GFP_KERNEL);
    if (!sp)
    return rval;
    sp.type = SRB_CTRL_VP;
    sp.name = "ctrl_vp";
...
// ref: INIT
    ...
    return vha;
    BUG_ON(ha.vp_map == core::ptr::null_mut());
...
// vport_slock to be held by the caller
    ...
    break;
    case RESET_VP_IDX:
    vha.hw.vp_map[vha.vp_idx].vha = core::ptr::null_mut();
    break;
    case RESET_AL_PA:
...
#[no_mangle]
pub unsafe extern "C" fn qla_update_host_map(vha: *mut scsi_qla_host, id: port_id_t) {
...
#[no_mangle]
pub unsafe extern "C" fn qla_create_buf_pool(vha: *mut scsi_qla_host, qp: *mut qla_qpair) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qla_free_buf_pool(qp: *mut qla_qpair) {
    ...
    qp.buf_pool.buf_array[i] = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn qla_get_buf(vha: *mut scsi_qla_host, qp: *mut qla_qpair, dsc: *mut qla_buf_dsc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qla_trim_buf(qp: *mut qla_qpair, trim: u16) {
    ...
    qp.buf_pool.buf_array[j] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn __qla_adjust_buf(qp: *mut qla_qpair) {
...
// it is assume qp->qp_lock is held at this point
#[no_mangle]
pub unsafe extern "C" fn qla_put_buf(qp: *mut qla_qpair, dsc: *mut qla_buf_dsc) {
...
#[no_mangle]
pub unsafe extern "C" fn qla_adjust_buf(vha: *mut scsi_qla_host) {
