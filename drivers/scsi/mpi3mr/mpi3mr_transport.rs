...
//! Source: drivers/scsi/mpi3mr/mpi3mr_transport.c
#![no_std]
...
//
    ...
    mrioc.transport_cmds.callback = core::ptr::null_mut();
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rep_manu_reply {
    pub /: *mut *mut u8 smp_frame_type; / 0x41,
    pub /: *mut *mut u8 function; / 0x01,
    ...
    pub sas_format: u8,
    pub reserved2: [u8; 3],
    pub __nonstring: u8 vendor_id[SAS_EXPANDER_VENDOR_ID_LEN],
    pub __nonstring: u8 product_id[SAS_EXPANDER_PRODUCT_ID_LEN],
    pub __nonstring: u8 product_rev[SAS_EXPANDER_PRODUCT_REV_LEN],
    pub __nonstring: u8 component_vendor_id[SAS_EXPANDER_COMPONENT_VENDOR_ID_LEN],
...
//
    ...
    let mut rc: c_int = 0;
    void *psge;
    void *data_out = core::ptr::null_mut();
    dma_addr_t data_out_dma;
    dma_addr_t data_in_dma;
    size_t data_in_sz;
    size_t data_out_sz;
    let mut sgl_flags: u8 = MPI3MR_SGEFLAGS_SYSTEM_SIMPLE_END_OF_LIST;
    let mut request_sz: u16 = sizeof(struct mpi3_smp_passthrough_request);
    let mut reply_sz: u16 = sizeof(struct mpi3_smp_passthrough_reply);
    u16 ioc_status;
    u8 *tmp;
...
//
#[no_mangle]
pub unsafe extern "C" fn mpi3mr_is_expander_device(device_info: u16) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn if(MPI3_DEVICE_DEVFORM_SAS_SATA: dev_pg0.device_form ==) -> else {
...
//
    ...
    struct mpi3mr_tgt_dev *tgtdev = core::ptr::null_mut();
    unsigned long flags;
    let mut was_on_tgtdev_list: u8 = 0;
    if (!hba_port)
    return;
    spin_lock_irqsave(&mrioc.tgtdev_lock, flags);
...
//
    ...
    {
    struct mpi3mr_sas_node *sas_expander, *r = core::ptr::null_mut();
    if (!hba_port)
    goto out;
    list_for_each_entry(sas_expander, &mrioc.sas_expander_list, list) {
...
//
#[no_mangle]
unsafe extern "C" fn mpi3mr_parent_present(mrioc: *mut mpi3mr_ioc, phy: *mut sas_phy) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mpi3mr_convert_phy_link_rate(link_rate: u8) -> enum sas_linkrate {
...
//
    ...
    {
    let mut sas_address: u64 = mr_sas_port.remote_identify.sas_address;
...
//
    ...
    {
    struct sas_phy *phy;
    let mut phy_index: c_int = mr_sas_phy.phy_id;
...
//
#[no_mangle]
pub unsafe extern "C" fn mpi3mr_sas_host_refresh(mrioc: *mut mpi3mr_ioc) {
    void mpi3mr_sas_host_refresh(struct mpi3mr_ioc *mrioc)
    {
    int i;
    u8 link_rate;
    u16 sz, port_id, attached_handle;
    struct mpi3_sas_io_unit_page0 *sas_io_unit_pg0 = core::ptr::null_mut();
    dprint_transport_info(mrioc,
...
//
#[no_mangle]
pub unsafe extern "C" fn mpi3mr_sas_host_add(mrioc: *mut mpi3mr_ioc) {
...
//
    ...
    struct mpi3mr_hba_port *srch_port, *hba_port_next = core::ptr::null_mut();
    if (!hba_port)
    return;
    spin_lock_irqsave(&mrioc.sas_node_lock, flags);
    mr_sas_node = __mpi3mr_sas_node_find_by_sas_address(mrioc,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct host_port {
    pub sas_address: u64,
    pub phy_mask: u64,
    pub handle: u16,
    pub iounit_port_id: u8,
    pub used: u8,
    pub lowest_phy: u8,
}
...
//
    ...
    {
    struct host_port *h_port = core::ptr::null_mut();
...
//
    ...
    spin_unlock_irqrestore(&mrioc.sas_node_lock, flags);
    sas_expander = core::ptr::null_mut();
    handle = 0xffff;
...
//
#[no_mangle]
pub unsafe extern "C" fn mpi3mr_expander_add(mrioc: *mut mpi3mr_ioc, handle: u16) -> c_int {
    ...
    struct mpi3mr_sas_port *mr_sas_port = core::ptr::null_mut();
    struct mpi3mr_hba_port *hba_port;
    u32 phynum_handle;
    let mut rc: c_int = 0;
    if (!handle)
...
//
    ...
    struct mpi3mr_tgt_dev *tgtdev)
    {
    let mut link_rate: u8 = MPI3_SAS_NEG_LINK_RATE_1_5, phy_number;
    struct mpi3_sas_expander_page1 expander_pg1;
    struct mpi3_sas_phy_page0 phy_pg0;
    u32 phynum_handle;
    u16 ioc_status;
...
//
    ...
    if (retval) {
    tgtdev.dev_spec.sas_sata_inf.hba_port = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn mpi3mr_get_port_id_by_sas_phy(phy: *mut sas_phy) -> u8 {
    static inline u8 mpi3mr_get_port_id_by_sas_phy(struct sas_phy *phy)
    {
    let mut port_id: u8 = 0xFF;
    struct mpi3mr_hba_port *hba_port = phy.hostdata;
    if (hba_port)
...
//
#[no_mangle]
unsafe extern "C" fn mpi3mr_get_port_id_by_rphy(mrioc: *mut mpi3mr_ioc, rphy: *mut sas_rphy) -> u8 {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct phy_error_log_request {
    pub /: *mut *mut u8 smp_frame_type; / 0x40,
    pub /: *mut *mut u8 function; / 0x11,
    pub allocated_response_length: u8,
    pub /: *mut *mut u8 request_length; / 02,
    pub reserved_1: [u8; 5],
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct phy_error_log_reply {
    pub /: *mut *mut u8 smp_frame_type; / 0x41,
    pub /: *mut *mut u8 function; / 0x11,
    pub function_result: u8,
    pub response_length: u8,
    pub expander_change_count: __be16,
    pub reserved_1: [u8; 3],
    pub phy_identifier: u8,
    pub reserved_2: [u8; 2],
    pub invalid_dword: __be32,
    pub running_disparity_error: __be32,
    pub loss_of_dword_sync: __be32,
    pub phy_reset_problem: __be32,
}
...
//
#[no_mangle]
unsafe extern "C" fn mpi3mr_transport_get_linkerrors(phy: *mut sas_phy) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct phy_control_request {
    pub /: *mut *mut u8 smp_frame_type; / 0x40,
    pub /: *mut *mut u8 function; / 0x91,
    pub allocated_response_length: u8,
    pub /: *mut *mut u8 request_length; / 0x09,
    pub expander_change_count: u16,
    pub reserved_1: [u8; 3],
    pub phy_identifier: u8,
    pub phy_operation: u8,
    pub reserved_2: [u8; 13],
    pub attached_device_name: u64,
    pub programmed_min_physical_link_rate: u8,
    pub programmed_max_physical_link_rate: u8,
    pub reserved_3: [u8; 6],
}
...
//
    ...
    let mut request_sz: u16 = sizeof(struct mpi3_iounit_control_request);
    let mut reply_sz: u16 = sizeof(struct mpi3_iounit_control_reply);
    let mut rc: c_int = 0;
    u16 ioc_status;
    rc = mpi3mr_parent_present(mrioc, phy);
    if (rc)
...
//
    ...
    struct mpi3mr_ioc *mrioc = phy_to_mrioc(phy);
    struct mpi3_sas_io_unit_page0 *sas_io_unit_pg0 = core::ptr::null_mut();
    struct mpi3_sas_io_unit_page1 *sas_io_unit_pg1 = core::ptr::null_mut();
    u16 sz;
    let mut rc: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(phy->minimum_linkrate_hw: rates->minimum_linkrate <) -> else {
    else if (rates.minimum_linkrate < phy.minimum_linkrate_hw)
    rates.minimum_linkrate = phy.minimum_linkrate_hw;
    if (!rates.maximum_linkrate)
    rates.maximum_linkrate = phy.maximum_linkrate;
#[no_mangle]
pub unsafe extern "C" fn if(phy->maximum_linkrate_hw: rates->maximum_linkrate >) -> else {
...
//
    ...
    void *addr_in = core::ptr::null_mut();
    void *addr_out = core::ptr::null_mut();
    size_t dma_len_in;
    size_t dma_len_out;
    let mut reslen: c_uint = 0;
    let mut request_sz: u16 = sizeof(struct mpi3_smp_passthrough_request);
    let mut reply_sz: u16 = sizeof(struct mpi3_smp_passthrough_reply);
    let mut sgl_flags: u8 = MPI3MR_SGEFLAGS_SYSTEM_SIMPLE_END_OF_LIST;
    u16 ioc_status;
