...
//! Source: drivers/scsi/aic94xx/aic94xx_scb.c
#![no_std]
...
// ---------- EMPTY SCB ----------
pub const DL_PHY_MASK: c_int = 7;
pub const BYTES_DMAED: c_int = 0;
pub const PRIMITIVE_RECVD: c_uint = 0x08;
pub const PHY_EVENT: c_uint = 0x10;
pub const LINK_RESET_ERROR: c_uint = 0x18;
pub const TIMER_EVENT: c_uint = 0x20;
pub const REQ_TASK_ABORT: c_uint = 0xF0;
pub const REQ_DEVICE_RESET: c_uint = 0xF1;
pub const SIGNAL_NCQ_ERROR: c_uint = 0xF2;
pub const CLEAR_NCQ_ERROR: c_uint = 0xF3;

    | CURRENT_SPINUP_HOLD | CURRENT_GTO_TIMEOUT \
    | CURRENT_OOB_ERROR)
#[no_mangle]
unsafe extern "C" fn get_lrate_mode(phy: *mut asd_phy, oob_mode: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn if(SATA_MODE: oob_mode &) -> else {
    ...
    let mut phy_id: c_int = dl.status_block[0] & DL_PHY_MASK;
    struct asd_phy *phy = &asd_ha.phys[phy_id];
    let mut oob_status: u8 = dl.status_block[1] & PHY_EVENTS_STATUS;
    let mut oob_mode: u8 = dl.status_block[2];
    switch (oob_status) {
...
#[no_mangle]
unsafe extern "C" fn ord_phy(asd_ha: *mut asd_ha_struct, phy: *mut asd_phy) -> unsigned {
    static unsigned ord_phy(struct asd_ha_struct *asd_ha, struct asd_phy *phy)
    {
    let mut enabled_mask: u8 = asd_ha.hw_prof.enabled_phys;
    int i, k = 0;
...
//
#[no_mangle]
unsafe extern "C" fn asd_get_attached_sas_addr(phy: *mut asd_phy, sas_addr: *mut u8) {
...
// FIS device-to-host
    let mut addr: u64 = be64_to_cpu(*(__be64 *)phy.phy_desc.sas_addr);
...
#[no_mangle]
unsafe extern "C" fn asd_form_port(asd_ha: *mut asd_ha_struct, phy: *mut asd_phy) {
    static void asd_form_port(struct asd_ha_struct *asd_ha, struct asd_phy *phy)
    {
    int i;
    struct asd_port *free_port = core::ptr::null_mut();
...
// Find a free port
    if (port.num_phys == 0 && free_port == core::ptr::null_mut()) {
    free_port = port;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn asd_deform_port(asd_ha: *mut asd_ha_struct, phy: *mut asd_phy) {
    ...
    port.num_phys--;
    port.phy_mask &= ~(1U << sas_phy.id);
    phy.asd_port = core::ptr::null_mut();
    }
    spin_unlock_irqrestore(&asd_ha.asd_ports_lock, flags);
    }
    ...
    {
    unsigned long flags;
    let mut edb_el: c_int = edb_id + ascb.edb_index;
    struct asd_dma_tok *edb = ascb.ha.seq.edb_arr[edb_el];
    struct asd_phy *phy = &ascb.ha.phys[phy_id];
    let mut size: u16 = ((dl.status_block[3] & 7) << 8) | dl.status_block[2];
    ...
    struct asd_phy *phy = &asd_ha.phys[phy_id];
    let mut lr_error: u8 = dl.status_block[1];
    let mut retries_left: u8 = dl.status_block[2];
    switch (lr_error) {
    case 0:
    ...
    struct asd_ha_struct *asd_ha = ascb.ha;
    struct asd_phy *phy = &asd_ha.phys[phy_id];
    let mut reg: u8 = dl.status_block[1];
    let mut cont: u32 = dl.status_block[2] << ((reg & 3)*8);
    reg &= ~3;
    switch (reg) {
...
//
#[no_mangle]
pub unsafe extern "C" fn asd_invalidate_edb(ascb: *mut asd_ascb, edb_id: c_int) {
...
//
    ...
    let mut sb_opcode: u8 = dl.status_block[0];
    let mut phy_id: c_int = sb_opcode & DL_PHY_MASK;
    struct asd_sas_phy *sas_phy = sas_ha.sas_phy[phy_id];
    struct asd_phy *phy = &asd_ha.phys[phy_id];
    if (edb > 6 || edb < 0) {
...
// Catch these before we mask off the sb_opcode bits
    ...
    struct asd_ascb *a, *b;
    u16 tc_abort;
    struct domain_device *failed_dev = core::ptr::null_mut();
...
//
    ...
    u16 conn_handle;
    unsigned long flags;
    struct sas_task *last_dev_task = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn asd_init_post_escbs(asd_ha: *mut asd_ha_struct) -> c_int {
...
//
    ...
    let mut phy_id: u8 = control_phy.phy_id;
    struct asd_phy *phy = &ascb.ha.phys[phy_id];
    let mut status: u8 = dl.status_block[0];
    let mut oob_status: u8 = dl.status_block[1];
    let mut oob_mode: u8 = dl.status_block[2];
...
#[no_mangle]
unsafe extern "C" fn set_speed_mask(speed_mask: *mut u8, pd: *mut asd_phy_desc) {
...
//
#[no_mangle]
pub unsafe extern "C" fn asd_build_control_phy(ascb: *mut asd_ascb, phy_id: c_int, subfunc: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn if(PHY_ROLE_TARGET: phy->sas_phy.role ==) -> else {
...
// ---------- INITIATE LINK ADM TASK ----------
    ...
    struct done_list_struct *dl)
    {
    let mut opcode: u8 = dl.opcode;
    struct initiate_link_adm *link_adm = &ascb.scb.link_adm;
    let mut phy_id: u8 = link_adm.phy_id;
...
//
#[no_mangle]
pub unsafe extern "C" fn asd_ascb_timedout(t: *mut timer_list) {
...
#[no_mangle]
pub unsafe extern "C" fn asd_control_phy(phy: *mut asd_sas_phy, func: enum phy_func, arg: *mut c_void) -> c_int {
