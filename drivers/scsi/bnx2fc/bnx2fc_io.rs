...
//! Source: drivers/scsi/bnx2fc/bnx2fc_io.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_scsi_done(io_req: *mut bnx2fc_cmd, err_code: c_int) {
...
// Sanity checks before returning command to mid-layer
    if (!sc_cmd) {
    printk(KERN_ERR PFX "scsi_done - sc_cmd core::ptr::null_mut(). "
    "IO(0x%x) already cleaned up\n",
    io_req.xid);
    return;
    }
    if (!sc_cmd.device) {
    pr_err(PFX "0x%x: sc_cmd.device is core::ptr::null_mut().\n", io_req.xid);
    return;
    }
    if (!sc_cmd.device.host) {
    pr_err(PFX "0x%x: sc_cmd.device.host is core::ptr::null_mut().\n",
    io_req.xid);
    return;
    }
    ...
    bnx2fc_priv(sc_cmd).io_req = core::ptr::null_mut();
    scsi_done(sc_cmd);
    }
    ...
    size_t bd_tbl_sz;
    let mut arr_sz: c_int = num_possible_cpus() + 1;
    let mut min_xid: u16 = BNX2FC_MIN_XID;
    let mut max_xid: u16 = hba.max_xid;
    ...
    cmgr.free_list = core::ptr::null_mut();
    goto mem_err;
    }
...
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_cmd_mgr_free(cmgr: *mut bnx2fc_cmd_mgr) {
...
// Free command manager itself
    ...
    let mut index: c_int = RESERVE_FREE_LIST_INDEX;
    u32 free_sqes;
    u32 max_sqes;
    u16 xid;
...
// Hold the io_req  against deletion
    ...
    u32 max_sqes;
    u16 xid;
    let mut index: c_int = raw_smp_processor_id();
...
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_cmd_release(ref: *mut kref) {
    ...
    cmd_mgr.cmds[io_req.xid] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_free_mp_resc(io_req: *mut bnx2fc_cmd) {
    static void bnx2fc_free_mp_resc(struct bnx2fc_cmd *io_req)
    {
    struct bnx2fc_mp_req *mp_req = &(io_req.mp_req);
    struct bnx2fc_interface *interface = io_req.port.priv;
    struct bnx2fc_hba *hba = interface.hba;
    let mut sz: usize = sizeof(struct fcoe_bd_ctx);
// clear tm flags
    mp_req.tm_flags = 0;
    if (mp_req.mp_req_bd) {
...
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_init_mp_req(io_req: *mut bnx2fc_cmd) -> c_int {
...
//
    ...
    interface = port.priv;
    if (rport == core::ptr::null_mut()) {
    printk(KERN_ERR PFX "device_reset: rport is core::ptr::null_mut()\n");
    rc = FAILED;
    goto tmf_err;
    }
    rp = rport.dd_data;
...
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_initiate_abts(io_req: *mut bnx2fc_cmd) -> c_int {
    ...
    u16 xid;
    let mut rc: c_int = SUCCESS;
    let mut r_a_tov: u32 = rdata.r_a_tov;
// called with tgt_lock held
    ...
    if (rport == core::ptr::null_mut()) {
    printk(KERN_ERR PFX "initiate_abts: rport is core::ptr::null_mut()\n");
    rc = FAILED;
    goto abts_err;
    }
    if (lport.state != LPORT_ST_READY || !(lport.link_up)) {
...
// Initialize rest of io_req fields
    abts_io_req.sc_cmd = core::ptr::null_mut();
...
// Ring doorbell
    ...
    struct fcoe_task_ctx_entry *task;
    struct fcoe_task_ctx_entry *task_page;
    struct bnx2fc_els_cb_arg *cb_arg = core::ptr::null_mut();
    int task_idx, index;
    u16 xid;
    let mut rc: c_int = 0;
...
// Initialize rest of io_req fields
    seq_clnp_req.sc_cmd = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_initiate_cleanup(io_req: *mut bnx2fc_cmd) -> c_int {
...
// Initialize rest of io_req fields
    cleanup_io_req.sc_cmd = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_eh_target_reset(sc_cmd: *mut scsi_cmnd) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_eh_device_reset(sc_cmd: *mut scsi_cmnd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_abts_cleanup(io_req: *mut bnx2fc_cmd) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2fc_eh_abort(sc_cmd: *mut scsi_cmnd) -> c_int {
...
// Command might have just completed
    printk(KERN_ERR PFX "eh_abort: io_req is core::ptr::null_mut()\n");
    spin_unlock_bh(&tgt.tgt_lock);
    return SUCCESS;
    }
...
// release the reference taken in eh_abort
    ...
    let mut offset: u32 = cb_arg.offset;
    let mut r_ctl: enum fc_rctl = cb_arg.r_ctl;
    let mut rc: c_int = 0;
    struct bnx2fc_rport *tgt = orig_io_req.tgt;
    ...
    " IO will abort\n");
    seq_clnp_req.cb_arg = core::ptr::null_mut();
...
//
    ...
    {
    u32 r_ctl;
    let mut r_a_tov: u32 = FC_DEF_R_A_TOV;
    let mut issue_rrq: u8 = 0;
    struct bnx2fc_rport *tgt = io_req.tgt;
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_lun_reset_cmpl(io_req: *mut bnx2fc_cmd) {
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_tgt_reset_cmpl(io_req: *mut bnx2fc_cmd) {
...
#[no_mangle]
pub unsafe extern "C" fn if(FCP_TMF_TGT_RESET: tm_req->tm_flags &) -> else {
    ...
    printk(KERN_ERR PFX "tm_compl: io_req is core::ptr::null_mut()\n");
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_map_sg(io_req: *mut bnx2fc_cmd) -> c_int {
    static int bnx2fc_map_sg(struct bnx2fc_cmd *io_req)
    {
    struct bnx2fc_interface *interface = io_req.port.priv;
    struct bnx2fc_hba *hba = interface.hba;
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_build_bd_list_from_sg(io_req: *mut bnx2fc_cmd) -> c_int {
    static int bnx2fc_build_bd_list_from_sg(struct bnx2fc_cmd *io_req)
    {
    struct scsi_cmnd *sc = io_req.sc_cmd;
    struct fcoe_bd_ctx *bd = io_req.bd_tbl.bd_tbl;
...
#[no_mangle]
unsafe extern "C" fn bnx2fc_unmap_sg_list(io_req: *mut bnx2fc_cmd) {
    static void bnx2fc_unmap_sg_list(struct bnx2fc_cmd *io_req)
    {
    struct scsi_cmnd *sc = io_req.sc_cmd;
    struct bnx2fc_interface *interface = io_req.port.priv;
...
//
    ...
    {
    struct scsi_cmnd *sc_cmd = io_req.sc_cmd;
    let mut rsp_flags: u8 = fcp_rsp.fcp_flags.flags;
    let mut rq_buff_len: u32 = 0;
...
// If retry_delay timer is active, flow off the ML
    ...
    struct bnx2fc_rport *tgt = io_req.tgt;
    struct scsi_cmnd *sc_cmd;
    let mut scope: u16 = 0, qualifier = 0;
...
// Cancel the timeout_work, as we received IO completion
    ...
    bnx2fc_cmd_release); /* drop timer hold */
    sc_cmd = io_req.sc_cmd;
    if (sc_cmd == core::ptr::null_mut()) {
    printk(KERN_ERR PFX "scsi_cmd_compl - sc_cmd is core::ptr::null_mut()\n");
    return;
    }
...
// parse fcp_rsp and obtain sense data from RQ if available
    ...
    if (!bnx2fc_priv(sc_cmd).io_req) {
    printk(KERN_ERR PFX "io_req is core::ptr::null_mut()\n");
    return;
    }
    if (io_req.on_active_queue) {
    list_del_init(&io_req.link);
