...
pub const SG_ALLOW_DIO_DEF: c_int = 0;

// SG_MAX_CDB_SIZE should be 260 (spc4r37 section 3.1.30) however the type
// of sg_io_hdr::cmd_len can only represent 255. All SCSI commands greater
// than 16 bytes are "variable length" whose length is a multiple of 4
//
pub const SG_MAX_CDB_SIZE: c_int = 252;
...
// picks up init parameter
    let mut def_reserved_size: static int = SG_DEF_RESERVED_SIZE;
    let mut sg_allow_dio: static int = SG_ALLOW_DIO_DEF;
    let mut scatter_elem_sz: static int = SG_SCATTER_SZ;
    let mut scatter_elem_sz_prev: static int = SG_SCATTER_SZ;
pub const SG_SECTOR_SZ: c_int = 512;
    ...
    struct sg_fd *parentfp;	/* core::ptr::null_mut() . not in use */
...
//
#[no_mangle]
unsafe extern "C" fn sg_check_file_access(filp: *mut file, caller: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sg_allow_access(filp: *mut file, cmd: *mut c_uchar) -> c_int {
...
// Returns 0 on success, else a negated errno value
    ...
    {
    let mut dev: c_int = iminor(inode);
    let mut flags: c_int = filp.f_flags;
...
#[no_mangle]
unsafe extern "C" fn get_sg_io_pack_id(pack_id: *mut c_int, buf: *mut void __user, count: usize) -> c_int {
...
// Now copy the result back to the user buffer.
    ...
    if ((CHECK_CONDITION & hp.masked_status) ||
    (srp.sense_b[0] & 0x70) == 0x70) {
    let mut sb_len: c_int = SCSI_SENSE_BUFFERSIZE;
    ...
    blocking, 0, 0, core::ptr::null_mut());
...
// Determine buffer size.
    ...
    hp.dxferp = core::ptr::null_mut();
    hp.sbp = core::ptr::null_mut();
    hp.timeout = old_hdr.reply_len;	/* structure abuse ... */
    hp.flags = input_size;	/* structure abuse ... */
    hp.pack_id = old_hdr.pack_id;
    hp.usr_ptr = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn srp_done(sfp: *mut Sg_fd, srp: *mut Sg_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn max_sectors_bytes(q: *mut request_queue) -> c_int {
    static int max_sectors_bytes(struct request_queue *q)
    {
    let mut max_sectors: c_uint = queue_max_sectors(q);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_sg_req_info {
    pub req_state: c_char,
    pub orphan: c_char,
    pub sg_io_owned: c_char,
    pub problem: c_char,
    pub pack_id: c_int,
    pub usr_ptr: compat_uptr_t,
    pub duration: c_uint,
    pub unused: c_int,
}
...
// if any read waiting, flag it
    ...
    res |= EPOLLHUP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !sfp->cmd_q) -> else {
    if (0 == count)
    ...
    int k, length;
    if ((core::ptr::null_mut() == vma) || (!(sfp = (Sg_fd *) vma.vm_private_data)))
...
//
    ...
    sfp = srp.parentfp;
    if (WARN_ON(sfp == core::ptr::null_mut()))
    return RQ_END_IO_NONE;
    sdp = sfp.parentdp;
...
//
    ...
    let mut sg_sysfs_valid: static int = 0;
    static Sg_device *
    ...
    Sg_device *sdp = core::ptr::null_mut();
    let mut cdev: *mut cdev = core::ptr::null_mut();
    int error;
    unsigned long iflags;
...
// want sdp->detaching non-zero as soon as possible
    ...
    cdev_del(sdp.cdev);
    sdp.cdev = core::ptr::null_mut();
    kref_put(&sdp.d_ref, sg_device_destroy);
    }
    module_param_named(scatter_elem_sz, scatter_elem_sz, int, S_IRUGO | S_IWUSR);
    module_param_named(allow_dio, sg_allow_dio, int, S_IRUGO | S_IWUSR);
#[no_mangle]
unsafe extern "C" fn def_reserved_size_set(val: *const c_char, kp: *const kernel_param) -> c_int {
...
// limit to 1 MB
    ...
    remove_proc_subtree("scsi/sg", core::ptr::null_mut());
    ...
    sg_io_hdr_t *hp = &srp.header;
    let mut dxfer_len: c_int = (int) hp.dxfer_len;
    let mut dxfer_dir: c_int = hp.dxfer_direction;
    let mut iov_count: c_uint = hp.iovec_count;
    Sg_scatter_hold *req_schp = &srp.data;
    Sg_scatter_hold *rsv_schp = &sfp.reserve;
    struct request_queue *q = sfp.parentdp.device.request_queue;
    struct rq_map_data *md, map_data;
    let mut rw: c_int = hp.dxfer_direction == SG_DXFER_TO_DEV ? ITER_SOURCE : ITER_DEST;
...
//
    ...
    {
    let mut sg_bufflen: c_int = tablesize * sizeof(struct page *);
    let mut gfp_flags: gfp_t = GFP_ATOMIC | __GFP_NOWARN;
    ...
    {
    let mut ret_sz: c_int = 0, i, k, rem_sz, num, mx_sc_elems;
    let mut sg_tablesize: c_int = sfp.parentdp.sg_tablesize;
    let mut blk_size: c_int = buff_size, order;
    let mut gfp_mask: gfp_t = GFP_ATOMIC | __GFP_COMP | __GFP_NOWARN | __GFP_ZERO;
    if (blk_size < 0)
    return -EFAULT;
    if (0 == blk_size)
...
// N.B. ret_sz carried into this block ...
    ...
    req_schp.k_use_sg = 0;
    req_schp.bufflen = 0;
    req_schp.pages = core::ptr::null_mut();
...
// Return of 1 for found; 0 for not found
    ...
    list_del(&srp.entry);
    srp.parentfp = core::ptr::null_mut();
    res = 1;
    }
    write_unlock_irqrestore(&sfp.rq_list_lock, iflags);
//
...
// must be called with sg_index_lock held
    ...
    sdp = ERR_PTR(-ENXIO);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: atomic_read(&sdp->detaching)) -> else {
...
//
    ...
    p = proc_mkdir("scsi/sg", core::ptr::null_mut());
    if (!p)
    return 1;
...
#[no_mangle]
unsafe extern "C" fn sg_proc_seq_show_int(s: *mut seq_file, v: *mut c_void) -> c_int {
    static int sg_proc_seq_show_int(struct seq_file *s, void *v)
    {
    seq_printf(s, "%d\n", *((int *)s.private));
    return 0;
...
#[no_mangle]
unsafe extern "C" fn sg_proc_single_open_adio(inode: *mut inode, file: *mut file) -> c_int {
    static int sg_proc_single_open_adio(struct inode *inode, struct file *file)
    {
...
#[no_mangle]
unsafe extern "C" fn sg_proc_single_open_dressz(inode: *mut inode, file: *mut file) -> c_int {
    static int sg_proc_single_open_dressz(struct inode *inode, struct file *file)
    {
...
#[no_mangle]
unsafe extern "C" fn sg_proc_seq_show_version(s: *mut seq_file, v: *mut c_void) -> c_int {
    static int sg_proc_seq_show_version(struct seq_file *s, void *v)
    {
...
#[no_mangle]
unsafe extern "C" fn sg_proc_seq_show_devhdr(s: *mut seq_file, v: *mut c_void) -> c_int {
    static int sg_proc_seq_show_devhdr(struct seq_file *s, void *v)
    {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sg_proc_deviter {
    pub index: loff_t,
    pub max: usize,
}

#[no_mangle]
unsafe extern "C" fn dev_seq_start(s: *mut seq_file, pos: *mut loff_t) -> *mut c_void {
    static void * dev_seq_start(struct seq_file *s, loff_t *pos)
    {
    let mut it: *mut sg_proc_deviter = kmalloc_obj(*it);
    s.private = it;
    if (! it)
    return core::ptr::null_mut();
    it.index = *pos;
...
#[no_mangle]
unsafe extern "C" fn dev_seq_next(s: *mut seq_file, v: *mut c_void, pos: *mut loff_t) -> *mut c_void {
    static void * dev_seq_next(struct seq_file *s, void *v, loff_t *pos)
    {
    let mut it: *mut sg_proc_deviter = s.private;
// pos = ++it->index;
    return (it.index < it.max) ? it : core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn dev_seq_stop(s: *mut seq_file, v: *mut c_void) {
    static void dev_seq_stop(struct seq_file *s, void *v)
    {
    kfree(s.private);
    }
#[no_mangle]
unsafe extern "C" fn sg_proc_seq_show_dev(s: *mut seq_file, v: *mut c_void) -> c_int {
    static int sg_proc_seq_show_dev(struct seq_file *s, void *v)
    {
    let mut it: *mut sg_proc_deviter = (struct sg_proc_deviter *) v;
    Sg_device *sdp;
    struct scsi_device *scsidp;
    unsigned long iflags;
    read_lock_irqsave(&sg_index_lock, iflags);
    sdp = it ? sg_lookup_dev(it.index) : core::ptr::null_mut();
    if ((core::ptr::null_mut() == sdp) || (core::ptr::null_mut() == sdp.device) ||
    (atomic_read(&sdp.detaching)))
...
#[no_mangle]
unsafe extern "C" fn sg_proc_seq_show_devstrs(s: *mut seq_file, v: *mut c_void) -> c_int {
    static int sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
    {
    let mut it: *mut sg_proc_deviter = (struct sg_proc_deviter *) v;
    Sg_device *sdp;
    ...
    read_lock_irqsave(&sg_index_lock, iflags);
    sdp = it ? sg_lookup_dev(it.index) : core::ptr::null_mut();
    scsidp = sdp ? sdp.device : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sg_proc_debug_helper(s: *mut seq_file, sdp: *mut *mut Sg_device) {
...
#[no_mangle]
unsafe extern "C" fn sg_proc_seq_show_debug(s: *mut seq_file, v: *mut c_void) -> c_int {
    static int sg_proc_seq_show_debug(struct seq_file *s, void *v)
    {
    let mut it: *mut sg_proc_deviter = (struct sg_proc_deviter *) v;
    Sg_device *sdp;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sdp->device) -> else {
