...
//! Source: drivers/scsi/qedf/qedf_io.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn qedf_cmd_timeout(work: *mut work_struct) {
    ...
    if (io_req.fcport == core::ptr::null_mut()) {
    QEDF_INFO(core::ptr::null_mut(), QEDF_LOG_IO,  "fcport is core::ptr::null_mut().\n");
    return;
    }
    qedf = fcport.qedf;
    switch (io_req.cmd_type) {
    case QEDF_ABTS:
    if (qedf == core::ptr::null_mut()) {
    QEDF_INFO(core::ptr::null_mut(), QEDF_LOG_IO,
    "qedf is core::ptr::null_mut() for ABTS xid=0x%x.\n",
    io_req.xid);
    return;
    }
...
//
    ...
    if (!qedf) {
    QEDF_INFO(core::ptr::null_mut(), QEDF_LOG_IO,
    "qedf is core::ptr::null_mut() for ELS xid=0x%x.\n",
    io_req.xid);
    return;
    }
...
// Call callback function to complete command
    ...
    io_req.cb_arg = core::ptr::null_mut();
    }
    kref_put(&io_req.refcount, qedf_release_cmd);
    break;
    ...
    qedf_initiate_cleanup(io_req, true);
    io_req.event = QEDF_IOREQ_EV_ELS_TMO;
    qedf_process_seq_cleanup_compl(qedf, core::ptr::null_mut(), io_req);
    break;
    default:
...
#[no_mangle]
pub unsafe extern "C" fn qedf_cmd_mgr_free(cmgr: *mut qedf_cmd_mgr) {
    void qedf_cmd_mgr_free(struct qedf_cmd_mgr *cmgr)
    {
    struct io_bdt *bdt_info;
    struct qedf_ctx *qedf = cmgr.qedf;
    size_t bd_tbl_sz;
    let mut min_xid: u16 = 0;
    let mut max_xid: u16 = (FCOE_PARAMS_NUM_TASKS - 1);
    int num_ios;
    int i;
    struct qedf_ioreq *io_req;
...
// Free fcoe_bdt_ctx structures
    if (!cmgr.io_bdt_pool) {
    QEDF_ERR(&qedf.dbg_ctx, "io_bdt_pool is core::ptr::null_mut().\n");
    goto free_cmd_pool;
    }
    ...
    bdt_info.bd_tbl = core::ptr::null_mut();
    }
    }
// Destroy io_bdt pool
    for (i = 0; i < num_ios; i++) {
    kfree(cmgr.io_bdt_pool[i]);
    cmgr.io_bdt_pool[i] = core::ptr::null_mut();
    }
    kfree(cmgr.io_bdt_pool);
    cmgr.io_bdt_pool = core::ptr::null_mut();
    free_cmd_pool:
    for (i = 0; i < num_ios; i++) {
    io_req = &cmgr.cmds[i];
...
#[no_mangle]
unsafe extern "C" fn qedf_handle_rrq(work: *mut work_struct) {
...
// Allocate pool of io_bdts - one for each qedf_ioreq
    ...
    if (!free_sqes) {
    QEDF_INFO(&(qedf.dbg_ctx), QEDF_LOG_IO,
    "Returning core::ptr::null_mut(), free_sqes=%d.\n",
    free_sqes);
    goto out_failed;
    }
// Limit the number of outstanding R/W tasks
    ...
    QEDF_INFO(&(qedf.dbg_ctx), QEDF_LOG_IO,
    "Returning core::ptr::null_mut(), num_active_ios=%d.\n",
...
// Limit global TIDs certain tasks
    ...
    QEDF_INFO(&(qedf.dbg_ctx), QEDF_LOG_IO,
    "Returning core::ptr::null_mut(), free_list_cnt=%d.\n",
...
// Clear any stale sc_cmd back pointer
    io_req.sc_cmd = core::ptr::null_mut();
    io_req.lun = -1;
...
// Have a static link between io_req and io_bdt_pool
    ...
    if (bd_tbl == core::ptr::null_mut()) {
    QEDF_ERR(&(qedf.dbg_ctx), "bd_tbl is core::ptr::null_mut(), xid=%x.\n", xid);
    kref_put(&io_req.refcount, qedf_release_cmd);
    goto out_failed;
    }
...
// Increment task retry identifier now that the request is released
    ...
    io_req.fcport = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn qedf_map_sg(io_req: *mut qedf_ioreq) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qedf_build_bd_list_from_sg(io_req: *mut qedf_ioreq) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(DMA_FROM_DEVICE: sc_cmd->sc_data_direction ==) -> else {
...
// 4 bytes: FCP data length
    ...
    let mut bd_count: c_int = 0;
    struct qedf_ctx *qedf = fcport.qedf;
    let mut cq_idx: u16 = smp_processor_id() % qedf.num_queues;
    struct regpair sense_data_buffer_phys_addr;
    let mut tx_io_size: u32 = 0;
    let mut rx_io_size: u32 = 0;
    int i, cnt;
...
#[no_mangle]
pub unsafe extern "C" fn qedf_get_sqe_idx(fcport: *mut qedf_rport) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn qedf_ring_doorbell(fcport: *mut qedf_rport) {
    void qedf_ring_doorbell(struct qedf_rport *fcport)
    {
    let mut dbell: fcoe_db_data = { 0 };
    dbell.agg_flags = 0;
...
#[no_mangle]
pub unsafe extern "C" fn qedf_post_io_req(fcport: *mut qedf_rport, io_req: *mut qedf_ioreq) -> c_int {
...
// Get the task context
    ...
    if (!task_ctx) {
    QEDF_WARN(&(qedf.dbg_ctx), "task_ctx is core::ptr::null_mut(), xid=%d.\n",
    xid);
// Release cmd will release io_req, but sc_cmd is assigned
    io_req.sc_cmd = core::ptr::null_mut();
    kref_put(&io_req.refcount, qedf_release_cmd);
...
// Return SQE to pool
    ...
    struct scsi_cmnd *sc_cmd = io_req.sc_cmd;
    struct qedf_ctx *qedf = io_req.fcport.qedf;
    let mut rsp_flags: u8 = fcp_rsp.rsp_flags.flags;
    let mut fcp_sns_len: c_int = 0;
    let mut fcp_rsp_len: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn qedf_unmap_sg_list(qedf: *mut qedf_ctx, io_req: *mut qedf_ioreq) {
    ...
    let mut fw_residual_flag: u8 = 0;
    let mut flags: c_ulong = 0;
    let mut chk_scope: u16 = 0;
    if (!io_req)
    return;
    if (!cqe)
    return;
    ...
    if (!sc_cmd) {
    QEDF_WARN(&(qedf.dbg_ctx), "sc_cmd is core::ptr::null_mut()!\n");
    return;
    }
    if (!qedf_priv(sc_cmd).io_req) {
    QEDF_WARN(&(qedf.dbg_ctx),
    "io_req is core::ptr::null_mut(), returned in another context.\n");
    return;
    }
    if (!sc_cmd.device) {
    QEDF_ERR(&qedf.dbg_ctx,
    "Device for sc_cmd %p is core::ptr::null_mut().\n", sc_cmd);
    return;
    }
    if (!scsi_cmd_to_rq(sc_cmd).q) {
    QEDF_WARN(&(qedf.dbg_ctx), "request.q is core::ptr::null_mut() so request "
...
//
    clear_bit(QEDF_CMD_OUTSTANDING, &io_req.flags);
    io_req.sc_cmd = core::ptr::null_mut();
    qedf_priv(sc_cmd).io_req =  core::ptr::null_mut();
    scsi_done(sc_cmd);
    kref_put(&io_req.refcount, qedf_release_cmd);
    }
// Return a SCSI command in some other context besides a normal completion
    ...
    int refcount;
    if (!io_req) {
    QEDF_INFO(&qedf.dbg_ctx, QEDF_LOG_IO, "io_req is core::ptr::null_mut()\n");
    return;
    }
...
//
    ...
    if (!sc_cmd.device) {
    QEDF_ERR(&qedf.dbg_ctx, "Device for sc_cmd %p is core::ptr::null_mut().\n",
    sc_cmd);
    goto bad_scsi_ptr;
    }
    ...
    QEDF_ERR(&qedf.dbg_ctx,
    "sc_cmd.sense_buffer for sc_cmd %p is core::ptr::null_mut().\n",
    sc_cmd);
    goto bad_scsi_ptr;
    }
...
//
    ...
    let mut err_warn: u8 = 0xff;
    if (!cqe) {
    QEDF_INFO(&qedf.dbg_ctx, QEDF_LOG_IO,
    "cqe is core::ptr::null_mut() for io_req %p xid=0x%x\n",
    io_req, io_req.xid);
    return;
    }
...
// Cleanup a command when we receive an error detection completion
    ...
    QEDF_INFO(core::ptr::null_mut(), QEDF_LOG_IO, "io_req is core::ptr::null_mut().\n");
    return;
    }
    if (io_req.fcport == core::ptr::null_mut()) {
    QEDF_INFO(core::ptr::null_mut(), QEDF_LOG_IO, "fcport is core::ptr::null_mut().\n");
    ...
    if (!cqe) {
    QEDF_INFO(&qedf.dbg_ctx, QEDF_LOG_IO,
    "cqe is core::ptr::null_mut() for io_req %p\n", io_req);
    return;
    }
...
// Call callback function to complete command
    ...
    els_req.cb_arg = core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn qedf_flush_active_ios(fcport: *mut qedf_rport, lun: u64) {
    ...
    int i, rc;
    unsigned long flags;
    let mut flush_cnt: c_int = 0;
    let mut wait_cnt: c_int = 100;
    let mut refcount: c_int = 0;
    if (!fcport) {
    QEDF_ERR(core::ptr::null_mut(), "fcport is core::ptr::null_mut()\n");
    return;
    }
// Check that fcport is still offloaded
    if (!test_bit(QEDF_RPORT_SESSION_READY, &fcport.flags)) {
    QEDF_ERR(core::ptr::null_mut(), "fcport is no longer offloaded.\n");
    return;
    }
    qedf = fcport.qedf;
    if (!qedf) {
...
// ID: 002
    ...
    QEDF_INFO(&qedf.dbg_ctx, QEDF_LOG_IO,
    "Device backpointer core::ptr::null_mut() for sc_cmd=%p.\n",
    io_req.sc_cmd);
...
//
#[no_mangle]
pub unsafe extern "C" fn qedf_initiate_abts(io_req: *mut qedf_ioreq, return_scsi_cmd_on_abts: bool) -> c_int {
...
// Sanity check qedf_rport before dereferencing any pointers
    if (!test_bit(QEDF_RPORT_SESSION_READY, &fcport.flags)) {
    QEDF_ERR(core::ptr::null_mut(), "tgt not offloaded\n");
    rc = 1;
    goto out;
    }
    qedf = fcport.qedf;
...
//
    if (!fcport) {
    QEDF_INFO(&qedf.dbg_ctx, QEDF_LOG_IO,
    "Dropping ABTS completion xid=0x%x as fcport is core::ptr::null_mut()",
    io_req.xid);
    return;
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn qedf_init_mp_req(io_req: *mut qedf_ioreq) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn qedf_drain_request(qedf: *mut qedf_ctx) {
...
// Complete so we can finish cleaning up the I/O
    ...
    qedf.lun_resets++;
#[no_mangle]
pub unsafe extern "C" fn if(FCP_TMF_TGT_RESET: tm_flags ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn qedf_initiate_tmf(rport: *mut fc_rport, lun: u64, tm_flags: u8) -> c_int {
    ...
    if (!rdata || !kref_get_unless_zero(&rdata.kref)) {
    QEDF_ERR(core::ptr::null_mut(), "stale rport\n");
    return FAILED;
    }
    QEDF_ERR(core::ptr::null_mut(), "portid=%06x tm_flags =%s\n", rdata.ids.port_id,
    ...
    {
    unsigned long flags;
    let mut pktlen: u16 = cqe.cqe_info.unsolic_info.pkt_len;
    u32 payload_len, crc;
    ...
    if (!bdq_addr) {
    QEDF_ERR(&(qedf.dbg_ctx), "bdq_addr is core::ptr::null_mut(), dropping "
    "unsolicited packet.\n");
    goto increment_prod;
    }
    if (qedf_dump_frames) {
