...
//! Source: drivers/scsi/qedf/qedf_fip.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn qedf_fcoe_send_vlan_req(qedf: *mut qedf_ctx) {
...
#[no_mangle]
pub unsafe extern "C" fn qedf_fip_send(fip: *mut fcoe_ctlr, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn qedf_fip_recv(qedf: *mut qedf_ctx, skb: *mut sk_buff) {
    ...
    u16 op;
    u8 sub;
    let mut fcf_valid: bool = false;
// Default is to handle CVL regardless of fabric id descriptor
    let mut fabric_id_valid: bool = true;
    let mut fc_wwpn_valid: bool = false;
    u64 switch_name;
    let mut vlan: u16 = 0;
    eth_hdr = (struct ethhdr *)skb_mac_header(skb);
    fiph = (struct fip_header *) ((void *)skb.data + 2 * ETH_ALEN + 2);
...
// Check that an FCF has been selected by fcoe
    if (qedf.ctlr.sel_fcf == core::ptr::null_mut()) {
    QEDF_INFO(&(qedf.dbg_ctx), QEDF_LOG_DISC,
