...
pub const CH_DT_MAX: c_int = 16;
pub const CH_TYPES: c_int = 8;
pub const CH_MAX_DEVS: c_int = 128;
    ...
    let mut timeout_move: static int = 300;
    module_param(timeout_move, int, 0644);
    MODULE_PARM_DESC(timeout_move,"timeout for move commands "
    "(default: 300 seconds)");
    let mut timeout_init: static int = 3600;
    ...
    module_param_array(dt_id,  int, core::ptr::null_mut(), 0444);
    module_param_array(dt_lun, int, core::ptr::null_mut(), 0444);
// tell the driver about vendor-specific slots
    static int vendor_firsts[CH_TYPES-4];
    static int vendor_counts[CH_TYPES-4];
    module_param_array(vendor_firsts, int, core::ptr::null_mut(), 0444);
    module_param_array(vendor_counts, int, core::ptr::null_mut(), 0444);
...
// -------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn ch_find_errno(sshdr: *mut scsi_sense_hdr) -> c_int {
...
// Check to see if additional sense information is available
    ...
    {
    let mut errno: c_int = 0, timeout, result;
...
// ------------------------------------------------------------------------
    ...
    cmd[1] = (ch.device.lun & 0x7) << 5;
    err = ch_do_scsi(ch, cmd, 6, core::ptr::null_mut(), 0, REQ_OP_DRV_IN);
...
// vendor specific element types
    ...
    continue;
    if (core::ptr::null_mut() == vendor_labels[i])
    continue;
...
// ------------------------------------------------------------------------
    ...
    cmd[5]  =  elem        & 0xff;
    cmd[8]  = rotate ? 1 : 0;
    return ch_do_scsi(ch, cmd, 10, core::ptr::null_mut(), 0, REQ_OP_DRV_IN);
    }
    static int
    ...
    cmd[7]  =  dest        & 0xff;
    cmd[10] = rotate ? 1 : 0;
    return ch_do_scsi(ch, cmd, 12, core::ptr::null_mut(), 0, REQ_OP_DRV_IN);
    }
    static int
...
#[no_mangle]
unsafe extern "C" fn ch_gstatus(ch: *mut scsi_changer, type: c_int, dest: *mut unsigned char __user) -> c_int {
...
// ------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn ch_destroy(ref: *mut kref) {
    static void ch_destroy(struct kref *ref)
    {
    scsi_changer *ch = container_of(ref, scsi_changer, ref);
    ch.device = core::ptr::null_mut();
    kfree(ch.dt);
    kfree(ch);
    }
    static int
    ...
    if (ch == core::ptr::null_mut() || !kref_get_unless_zero(&ch.ref)) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct changer_element_status32 {
    pub ces_type: c_int,
    pub ces_data: compat_uptr_t,
}
...
// ------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn ch_probe(sd: *mut scsi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ch_remove(sd: *mut scsi_device) {
...
#[no_mangle]
unsafe extern "C" fn init_ch_module() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_ch_module() -> void __exit {
