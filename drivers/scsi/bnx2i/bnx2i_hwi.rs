...
//! Source: drivers/scsi/bnx2i/bnx2i_hwi.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_get_cid_num(ep: *mut bnx2i_endpoint) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_adjust_qp_size(hba: *mut bnx2i_hba) {
...
#[no_mangle]
pub unsafe extern "C" fn if(num_elements_per_pg: hba->max_sqes %) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(num_elements_per_pg: hba->max_cqes %) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(num_elements_per_pg: hba->max_rqes %) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_get_link_state(hba: *mut bnx2i_hba) {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_iscsi_license_error(hba: *mut bnx2i_hba, error_code: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_arm_cq_event_coalescing(ep: *mut bnx2i_endpoint, action: u8) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_get_rq_buf(bnx2i_conn: *mut bnx2i_conn, ptr: *mut c_char, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bnx2i_ring_577xx_doorbell(conn: *mut bnx2i_conn) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_put_rq_buf(bnx2i_conn: *mut bnx2i_conn, count: c_int) {
    void bnx2i_put_rq_buf(struct bnx2i_conn *bnx2i_conn, int count)
    {
    struct bnx2i_5771x_sq_rq_db *rq_db;
    let mut hi_bit: u16 = (bnx2i_conn.ep.qp.rq_prod_idx & 0x8000);
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_ring_sq_dbell(bnx2i_conn: *mut bnx2i_conn, count: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn if(RESERVED_ITT: nopout_hdr->itt ==) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_update_iscsi_conn(conn: *mut iscsi_conn) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_ep_ofld_timer(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn bnx2i_power_of2(val: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_send_cmd_cleanup_req(hba: *mut bnx2i_hba, cmd: *mut bnx2i_cmd) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_send_conn_destroy(hba: *mut bnx2i_hba, ep: *mut bnx2i_endpoint) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_send_conn_ofld_req(hba: *mut bnx2i_hba, ep: *mut bnx2i_endpoint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn setup_qp_page_tables(ep: *mut bnx2i_endpoint) {
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_alloc_qp_resc(hba: *mut bnx2i_hba, ep: *mut bnx2i_endpoint) -> c_int {
    int bnx2i_alloc_qp_resc(struct bnx2i_hba *hba, struct bnx2i_endpoint *ep)
    {
    struct bnx2i_5771x_cq_db *cq_db;
    ep.hba = hba;
    ep.conn = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_send_fw_iscsi_init_msg(hba: *mut bnx2i_hba) -> c_int {
...
//
    ...
    __iscsi_complete_pdu(conn, (struct iscsi_hdr *)resp_hdr, core::ptr::null_mut(), 0);
    done:
    spin_unlock(&session.back_lock);
    return 0;
    }
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_unsol_pdu_adjust_rq(bnx2i_conn: *mut bnx2i_conn) {
...
// this is a response to one of our nop-outs
    ...
    __iscsi_complete_pdu(conn, (struct iscsi_hdr *)hdr, core::ptr::null_mut(), 0);
    spin_unlock(&session.back_lock);
...
//
    ...
    (struct iscsi_hdr *)resp_hdr, core::ptr::null_mut(), 0);
    spin_unlock(&session.back_lock);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_percpu_io_thread(arg: *mut c_void) -> c_int {
...
//
    ...
    {
    struct bnx2i_work *bnx2i_work = core::ptr::null_mut();
    struct bnx2i_percpu_s *p = core::ptr::null_mut();
    struct iscsi_task *task;
    struct scsi_cmnd *sc;
    let mut rc: c_int = 0;
    spin_lock(&session.back_lock);
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_process_new_cqes(bnx2i_conn: *mut bnx2i_conn) -> c_int {
    ...
    int tgt_async_msg;
    let mut cqe_cnt: c_int = 0;
    if (bnx2i_conn.ep == core::ptr::null_mut())
    return 0;
    qp = &bnx2i_conn.ep.qp;
    if (!qp.cq_virt) {
...
//
    ...
    let mut i: c_int = 0;
    struct iscsi_kcqe *ikcqe = core::ptr::null_mut();
    while (i < num_cqe) {
...
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_KCQE_OPCODE_OFFLOAD_CONN: ikcqe->op_code ==) -> else {
    else if (ikcqe.op_code == ISCSI_KCQE_OPCODE_OFFLOAD_CONN)
    bnx2i_process_ofld_cmpl(hba, ikcqe);
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_KCQE_OPCODE_UPDATE_CONN: ikcqe->op_code ==) -> else {
    else if (ikcqe.op_code == ISCSI_KCQE_OPCODE_UPDATE_CONN)
    bnx2i_process_update_conn_cmpl(hba, ikcqe);
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_KCQE_OPCODE_INIT: ikcqe->op_code ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_KCQE_OPCODE_ISCSI_ERROR: ikcqe->op_code ==) -> else {
    else if (ikcqe.op_code == ISCSI_KCQE_OPCODE_ISCSI_ERROR)
    bnx2i_process_iscsi_error(hba, ikcqe);
#[no_mangle]
pub unsafe extern "C" fn if(ISCSI_KCQE_OPCODE_TCP_ERROR: ikcqe->op_code ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_cm_connect_cmpl(cm_sk: *mut cnic_sock) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(SK_F_OFFLD_COMPLETE, _arg: &cm_sk->flags)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_cm_close_cmpl(cm_sk: *mut cnic_sock) {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_cm_abort_cmpl(cm_sk: *mut cnic_sock) {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_cm_remote_close(cm_sk: *mut cnic_sock) {
...
//
#[no_mangle]
unsafe extern "C" fn bnx2i_cm_remote_abort(cm_sk: *mut cnic_sock) {
    static void bnx2i_cm_remote_abort(struct cnic_sock *cm_sk)
    {
    struct bnx2i_endpoint *ep = (struct bnx2i_endpoint *) cm_sk.context;
    let mut old_state: u32 = ep.state;
...
//
#[no_mangle]
pub unsafe extern "C" fn bnx2i_map_ep_dbell_regs(ep: *mut bnx2i_endpoint) -> c_int {
