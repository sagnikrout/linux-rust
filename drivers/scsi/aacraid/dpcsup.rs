...
//! Source: drivers/scsi/aacraid/dpcsup.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn aac_response_normal(q: *mut *mut aac_queue) -> c_uint {
    unsigned int aac_response_normal(struct aac_queue * q)
    {
    let mut dev: *mut aac_dev = q.dev;
...
//
    ...
    {
    int fast;
    let mut index: u32 = le32_to_cpu(entry.addr);
    fast = index & 0x01;
...
//
#[no_mangle]
pub unsafe extern "C" fn aac_command_normal(q: *mut aac_queue) -> c_uint {
...
//
    if (dev.aif_thread)
    if((fib = kmalloc_obj(struct fib, GFP_ATOMIC)) == core::ptr::null_mut())
    fib = &fibctx;
...
//
#[no_mangle]
unsafe extern "C" fn aac_aif_callback(context: *mut c_void, fibptr: *mut *mut fib) {
...
//
    ...
    } else if (aif_fib != core::ptr::null_mut()) {
    ...
    let mut start_callback: c_int = 0;
//
// Remove this fib from the Outstanding I/O queue.
// But only if it has not already been timed out.
//
