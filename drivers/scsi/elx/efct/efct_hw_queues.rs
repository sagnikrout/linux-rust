...
//! Source: drivers/scsi/elx/efct/efct_hw_queues.c
#![no_std]
...
//
    ...
    {
    struct hw_eq *eq = core::ptr::null_mut();
    struct hw_cq *cq = core::ptr::null_mut();
    struct hw_wq *wq = core::ptr::null_mut();
    struct hw_mq *mq = core::ptr::null_mut();
...
// Initialise CQS pointers to NULL
    ...
    struct hw_rq *rq = core::ptr::null_mut();
    struct sli4_queue *qs[SLI4_MAX_RQ_SET_COUNT * 2] = { core::ptr::null_mut() };
    u32 i, q_count, size;
// Initialise RQS pointers
    for (i = 0; i < num_rq_pairs; i++)
    rqs[i] = core::ptr::null_mut();
//
...
// Data RQ
    ...
    rq.rq_tracker = core::ptr::null_mut();
    }
    ...
    eq.hw.hw_eq[eq.instance] = core::ptr::null_mut();
    kfree(eq);
    }
    void
    ...
    cq.eq.hw.hw_cq[cq.instance] = core::ptr::null_mut();
    kfree(cq);
    }
    void
    ...
    mq.cq.eq.hw.hw_mq[mq.instance] = core::ptr::null_mut();
    kfree(mq);
    }
    void
    ...
    wq.cq.eq.hw.hw_wq[wq.instance] = core::ptr::null_mut();
    kfree(wq);
    }
    void
    efct_hw_del_rq(struct hw_rq *rq)
    {
    struct efct_hw *hw = core::ptr::null_mut();
    if (!rq)
    return;
// Free RQ tracker
    kfree(rq.rq_tracker);
    ...
    hw = rq.cq.eq.hw;
    hw.hw_rq[rq.instance] = core::ptr::null_mut();
    kfree(rq);
    }
    void
    ...
    struct efc_hw_sequence *seq = core::ptr::null_mut();
...
// rq_hdr lock also covers rqindex+1 queue
    ...
    rq.rq_tracker[bufindex] = core::ptr::null_mut();
    if (!seq) {
    efc_log_err(hw.os,
    "RQbuf core::ptr::null_mut(), rqidx %d, bufidx %d, cur q idx = %d\n",
...
//
    ...
    let mut hw_rq_index: u32 = hw.hw_rq_lookup[seq.header.rqindex];
...
// Update the lookup table
    ...
    efc_log_debug(hw.os,
    "expected rq_tracker[%d][%d] buffer to be core::ptr::null_mut()\n",
