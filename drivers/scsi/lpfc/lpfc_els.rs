...
//! Source: drivers/scsi/lpfc/lpfc_els.c
#![no_std]
...
// See Fibre Channel protocol T11 FC-LS for details
    ...
    struct lpfc_iocbq *);
    let mut lpfc_max_els_tries: static int = 3;
...
#[no_mangle]
unsafe extern "C" fn lpfc_is_els_acc_rsp(buf: *mut lpfc_dmabuf) -> bool {
...
//
    ...
    {
    struct lpfc_hba  *phba = vport.phba;
    LPFC_MBOXQ_t *mboxq = core::ptr::null_mut();
    struct lpfc_nodelist *ndlp;
    struct lpfc_dmabuf *dmabuf = core::ptr::null_mut();
    let mut rc: c_int = 0;
...
//
    ...
    {
    struct lpfc_hba *phba = vport.phba;
    let mut fabric_param_changed: u8 = 0;
...
//
    ...
    let mut flogi_in_retry: bool = false;
...
//
    ...
    ulp_word4);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !test_bit(HBA_FCOE_MODE, _arg: &phba->hba_flag)) -> else {
...
//
    ...
    struct lpfc_hba  *phba = vport.phba;
    struct serv_parm *sp;
    union lpfc_wqe128 *wqe = core::ptr::null_mut();
    IOCB_t *icmd = core::ptr::null_mut();
    struct lpfc_iocbq *elsiocb;
...
//
    ...
    phba.defer_flogi_acc.ndlp = core::ptr::null_mut();
    }
    lpfc_printf_vlog(vport, KERN_INFO, LOG_ELS,
...
//
    ...
    let mut keepDID: u32 = 0;
    int rc;
    let mut keep_nlp_flag: c_ulong = 0, keep_new_nlp_flag = 0;
    uint16_t keep_nlp_state;
    let mut keep_nlp_fc4_type: u32 = 0;
    struct lpfc_nvme_rport *keep_nrport = core::ptr::null_mut();
    unsigned long *active_rrqs_xri_bitmap = core::ptr::null_mut();
...
//
    ...
    lpfc_disc_state_machine(vport, ndlp, core::ptr::null_mut(), NLP_EVT_DEVICE_RM);
    if (phba.sli_rev == LPFC_SLI_REV4 &&
    active_rrqs_xri_bitmap)
    mempool_free(active_rrqs_xri_bitmap,
    phba.active_rrq_pool);
...
//
    ...
    let mut ulp_status: u32 = get_job_ulpstatus(phba, rspiocb);
    let mut ulp_word4: u32 = get_job_word4(phba, rspiocb);
...
//
    ...
    bool disc;
    struct serv_parm *sp = core::ptr::null_mut();
    u32 ulp_status, ulp_word4, did, iotag;
    let mut release_node: bool = false;
// we pass cmdiocb to state machine which needs rspiocb as well
    cmdiocb.rsp_iocb = rspiocb;
    ulp_status = get_job_ulpstatus(phba, rspiocb);
    ulp_word4 = get_job_word4(phba, rspiocb);
...
//
    ...
    struct lpfc_vport *vport = ndlp.vport;
    IOCB_t *irsp;
    let mut skip_recovery: u32 = 0;
    let mut wake_up_waiter: c_int = 0;
    u32 ulp_status;
    u32 ulp_word4;
    u32 tmo, iotag;
// we pass cmdiocb to state machine which needs rspiocb as well
...
//
    ...
    uint32_t nportid;
    let mut cmdsize: u16 = sizeof(*event);
...
//
    ...
    {
    int rc;
    rc = lpfc_els_rsp_acc(vport, ELS_CMD_RDF, cmdiocb, ndlp, core::ptr::null_mut());
// Send LS_ACC
    if (rc) {
    lpfc_printf_vlog(vport, KERN_INFO, LOG_ELS | LOG_CGN_MGMT,
...
//
    ...
    {
    let mut rsp_sig_cap: u32 = 0, drv_sig_cap = 0;
    let mut rsp_sig_freq_cyc: u32 = 0, rsp_sig_freq_scale = 0;
...
//
    ...
    const char *dtag_nm;
    u32 *pdata, dtag;
    let mut desc_cnt: c_int = 0, bytes_remain;
    let mut rcv_cap_desc: bool = false;
    struct lpfc_nodelist *ndlp;
    u32 ulp_status, ulp_word4, tmo, did, iotag;
    ndlp = cmdiocb.ndlp;
    ulp_status = get_job_ulpstatus(phba, rspiocb);
...
//
    ...
    uint32_t *elscmd;
    struct ls_rjt stat;
    let mut retry: c_int = 0, maxretry = lpfc_max_els_tries, delay = 0;
    let mut logerr: c_int = 0;
    let mut cmd: u32 = 0;
    uint32_t did;
    let mut link_reset: c_int = 0, rc;
    let mut ulp_status: u32 = get_job_ulpstatus(phba, rspiocb);
    let mut ulp_word4: u32 = get_job_word4(phba, rspiocb);
    let mut rsn_code_exp: u8 = 0;
...
// retry FLOGI forever
    ...
    delay = 5000;
#[no_mangle]
pub unsafe extern "C" fn if(32: cmdiocb->retry >=) -> else {
...
// delay is specified in milliseconds
    ...
    NLP_STE_PRLI_ISSUE);
#[no_mangle]
pub unsafe extern "C" fn if(ELS_CMD_ADISC: cmd !=) -> else {
...
// The I/O iocb is complete.  Clear the node and first dmabuf
    elsiocb.ndlp = core::ptr::null_mut();
...
//
    ...
    elsiocb.cmd_dmabuf = core::ptr::null_mut();
    if (buf_ptr) {
    buf_ptr1 = core::ptr::null_mut();
    spin_lock_irq(&phba.hbalock);
...
//
    ...
    let mut mbx_flag: u32 = pmb.mbox_flag;
    let mut mbx_cmd: u32 = pmb.u.mb.mbxCommand;
    if (ndlp) {
...
//
    ...
    {
    struct lpfc_nodelist *ndlp = cmdiocb.ndlp;
    struct lpfc_vport *vport = ndlp ? ndlp.vport : core::ptr::null_mut();
    struct Scsi_Host  *shost = vport ? lpfc_shost_from_vport(vport) : core::ptr::null_mut();
    IOCB_t  *irsp;
    LPFC_MBOXQ_t *mbox = core::ptr::null_mut();
    u32 ulp_status, ulp_word4, tmo, did, iotag;
    u32 cmd;
    if (!vport) {
...
//
    ...
    struct lpfc_rdp_context *rdp_context;
    union lpfc_wqe128 *cmd = core::ptr::null_mut();
    struct ls_rjt stat;
    if (phba.sli_rev < LPFC_SLI_REV4 ||
    ...
    lpfc_els_rsp_reject(vport, stat.un.lsRjtError, cmdiocb, ndlp, core::ptr::null_mut());
    return 1;
    }
    static void
...
// Indicate we are walking lpfc_els_flush_rscn on this vport
    ...
    vport.fc_rscn_id_list[i] = core::ptr::null_mut();
    }
    clear_bit(FC_RSCN_MODE, &vport.fc_flag);
...
//
    ...
    {
    struct lpfc_nodelist *ndlp = core::ptr::null_mut(), *n;
...
//
    ...
    int rscn_cnt;
    let mut rscn_id: c_int = 0, hba_id = 0;
    int i, tmo;
    pcmd = cmdiocb.cmd_dmabuf;
...
// lp, vport->fc_flag, payload_len);
    lpfc_els_rsp_acc(vport, ELS_CMD_ACC, cmdiocb, ndlp, core::ptr::null_mut());
...
//
    cmdiocb.cmd_dmabuf = core::ptr::null_mut();
    }
// Deferred RSCN
    lpfc_printf_vlog(vport, KERN_INFO, LOG_DISCOVERY,
    "0235 Deferred RSCN "
...
//
    ...
    uint32_t cmd, did;
    int rc;
    let mut fc_flag: c_ulong = 0;
    let mut port_state: u32 = 0;
// Clear external loopback plug detected flag
    phba.link_flag &= ~LS_EXTERNAL_LOOPBACK;
    cmd = *lp++;
...
// Send back ACC
    lpfc_els_rsp_acc(vport, ELS_CMD_FLOGI, cmdiocb, ndlp, core::ptr::null_mut());
...
//
    ...
    memset(&pmb.ctx_u, 0, sizeof(pmb.ctx_u));
    pmb.ctx_ndlp = core::ptr::null_mut();
    if (mb.mbxStatus) {
    mempool_free(pmb, phba.mbox_mem_pool);
    return;
    }
...
//
    ...
    LPFC_MBOXQ_t *mbox;
    struct ls_rjt stat;
    let mut ctx: u32 = get_job_ulpcontext(phba, cmdiocb);
    let mut ox_id: u32 = get_job_rcvoxid(phba, cmdiocb);
    if ((ndlp.nlp_state != NLP_STE_UNMAPPED_NODE) &&
    (ndlp.nlp_state != NLP_STE_MAPPED_NODE))
...
//
    ...
    IOCB_t *cmd = core::ptr::null_mut();
    struct lpfc_dmabuf *pcmd;
    let mut els_command: u32 = 0;
    uint32_t timeout;
    let mut remote_ID: u32 = 0xffffffff;
    LIST_HEAD(abort_list);
    let mut ulp_command: u32 = 0, ulp_context = 0, did = 0, iotag = 0;
    ...
    lpfc_sli_issue_abort_iotag(phba, pring, piocb, core::ptr::null_mut());
    spin_unlock_irq(&phba.hbalock);
    }
// Make sure HBA is alive
    lpfc_issue_hb_tmo(phba);
...
//
    ...
    lsrjt_event.command = (pcmd != core::ptr::null_mut()) ? *pcmd : 0;
    stat.un.ls_rjt_error_be = cpu_to_be32(ulp_word4);
...
//
    ...
    uint32_t *payload)
    {
    struct lpfc_els_event_header *els_data = core::ptr::null_mut();
    struct lpfc_logo_event *logo_data = core::ptr::null_mut();
    struct Scsi_Host *shost = lpfc_shost_from_vport(vport);
...
// Change the log level to unmaskable for the following event types.
    ...
    const char *dtag_nm;
    let mut desc_cnt: c_int = 0, bytes_remain, cnt;
    u32 dtag, deliver = 0;
    int len;
...
//
    ...
    let mut cmd: u32 = 0, did = 0, newnode, status = 0;
    uint8_t rjt_exp, rjt_err = 0, init_link = 0;
    struct lpfc_wcqe_complete *wcqe_cmpl = core::ptr::null_mut();
    LPFC_MBOXQ_t *mbox;
...
//
    ...
    dma_addr_t paddr;
    elsiocb.cmd_dmabuf = core::ptr::null_mut();
    elsiocb.rsp_dmabuf = core::ptr::null_mut();
    elsiocb.bpl_dmabuf = core::ptr::null_mut();
    wcqe_cmpl = &elsiocb.wcqe_cmpl;
...
//
    ...
    lpfc_issue_init_vpi(vport);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(FC_VPORT_NEEDS_REG_VPI, _arg: &vport->fc_flag)) -> else {
...
//
    ...
    uint8_t *pcmd;
    uint16_t cmdsize;
    let mut did: c_int = ndlp.nlp_DID;
    int rc;
...
// Increment fabric iocb count to hold the position
    ...
    iocb.fabric_cmd_cmpl = core::ptr::null_mut();
    iocb.cmd_flag &= ~LPFC_IO_FABRIC;
...
//
    ...
    cmdiocb.fabric_cmd_cmpl = core::ptr::null_mut();
    cmdiocb.cmd_flag &= ~LPFC_IO_FABRIC;
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_fabric_abort_vport(vport: *mut lpfc_vport) {
    static void lpfc_fabric_abort_vport(struct lpfc_vport *vport)
    {
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_fabric_abort_nport(ndlp: *mut lpfc_nodelist) {
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_fabric_abort_hba(phba: *mut lpfc_hba) {
...
//
    ...
    struct lpfc_nodelist *ndlp = core::ptr::null_mut();
    let mut iflag: c_ulong = 0;
    ...
    sglq_entry.ndlp = core::ptr::null_mut();
...
//
    ...
    {
    let mut xri: u16 = bf_get(lpfc_wcqe_xa_xri, axri);
    let mut rxid: u16 = bf_get(lpfc_wcqe_xa_remote_xid, axri);
    let mut lxri: u16 = 0;
    struct lpfc_sglq *sglq_entry = core::ptr::null_mut(), *sglq_next = core::ptr::null_mut();
    let mut iflag: c_ulong = 0;
    struct lpfc_nodelist *ndlp;
...
#[no_mangle]
unsafe extern "C" fn lpfc_init_cs_ctl_bitmap(vport: *mut lpfc_vport) {
    static void lpfc_init_cs_ctl_bitmap(struct lpfc_vport *vport)
    {
...
#[no_mangle]
unsafe extern "C" fn lpfc_vmid_put_cs_ctl(vport: *mut lpfc_vport, ctcl_vmid: u32) {
    static void lpfc_vmid_put_cs_ctl(struct lpfc_vport *vport, u32 ctcl_vmid)
    {
    set_bit(ctcl_vmid, vport.vmid_priority_range);
    }
#[no_mangle]
pub unsafe extern "C" fn lpfc_vmid_get_cs_ctl(vport: *mut lpfc_vport) -> u32 {
...
pub const MAX_PRIORITY_DESC: c_int = 255;
    static void
    ...
    struct lpfc_dmabuf *prsp = core::ptr::null_mut();
    struct lpfc_vmid_priority_range *vmid_range = core::ptr::null_mut();
    u32 *data;
...
#[no_mangle]
pub unsafe extern "C" fn lpfc_issue_els_qfpa(vport: *mut lpfc_vport) -> c_int {
