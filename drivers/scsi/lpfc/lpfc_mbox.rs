...
//
    ...
    struct lpfc_dmabuf *mp;
    mp = mbox.ctx_buf;
    mbox.ctx_buf = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(LPFC_SLI_REV4: phba->sli_rev >=) -> else {
...
// A ring MUST have both cmd and rsp entries defined to be
    ...
    pring.sli.sli3.cmdringaddr = core::ptr::null_mut();
    pring.sli.sli3.rspringaddr = core::ptr::null_mut();
    continue;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(3: hbq_desc->profile ==) -> else {
    else if (hbq_desc.profile == 3)
    lpfc_build_hbq_profile3(hbqmb, hbq_desc);
#[no_mangle]
pub unsafe extern "C" fn if(5: hbq_desc->profile ==) -> else {
...
//
    ...
    phba.hbq_put = core::ptr::null_mut();
...
//
    ...
    {
    LPFC_MBOXQ_t *mbq = core::ptr::null_mut();
    struct lpfc_sli *psli = &phba.sli;
...
//
    ...
    union lpfc_sli4_cfg_shdr *cfg_shdr = core::ptr::null_mut();
    uint32_t alloc_len;
    uint32_t resid_len;
...
//
    ...
    {
    let mut opcode: u8 = 0;
    struct lpfc_mbx_nembed_rsrc_extent *n_rsrc_extnt = core::ptr::null_mut();
    void *virtaddr = core::ptr::null_mut();
// Set up SLI4 ioctl command header fields
    if (emb == LPFC_SLI4_MBX_NEMBED) {
// Get the first SGE entry from the non-embedded DMA memory
    virtaddr = mbox.sge_array.addr[0];
    if (virtaddr == core::ptr::null_mut())
    return 1;
...
//
    ...
    struct lpfc_hba *phba = vport.phba;
    let mut bbscn_fabric: u8 = 0, bbscn_max = 0, bbscn_def = 0;
    memset(mbox, 0, sizeof(*mbox));
