...
//! Source: drivers/scsi/lpfc/lpfc_nvmet.c
#![no_std]
...
//
    ...
    cmdwqe.context_un.axchg = core::ptr::null_mut();
    cmdwqe.bpl_dmabuf = core::ptr::null_mut();
...
// check if freed in another path whilst acquiring lock
    if (nvmebuf) {
    ctxp.rqb_buffer = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg1_max: seg1 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg2_max: seg2 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg3_max: seg3 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg4_max: seg4 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg5_max: seg5 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg6_max: seg6 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg7_max: seg7 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg8_max: seg8 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg9_max: seg9 >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(phba->ktime_seg10_max: seg10 >) -> else {
...
// clear to be sure there's no reference
    nvmewqeq.bpl_dmabuf = core::ptr::null_mut();
    if (rc == WQE_SUCCESS) {
//
...
// Give back resources
    ...
    nvmewqeq.context_un.axchg = core::ptr::null_mut();
    nvmewqeq.bpl_dmabuf = core::ptr::null_mut();
    rc = -EBUSY;
    aerr:
    return rc;
...
// sizes of additional private data for data structures
    ...
    ctx_buf.sglq.ndlp = core::ptr::null_mut();
...
// Cycle the entire CPU context list for every MRQ
    ...
    phba.sli4_hba.nvmet_ctx_info = core::ptr::null_mut();
    }
    static int
...
// Initialize WQE
    ...
    ctx_buf.iocbq.cmd_dmabuf = core::ptr::null_mut();
    spin_lock(&phba.sli4_hba.sgl_list_lock);
...
//
    ...
    struct lpfc_nvmet_tgtport *tgtp;
    struct nvmefc_tgt_fcp_req *req = core::ptr::null_mut();
    struct lpfc_nodelist *ndlp;
    let mut iflag: c_ulong = 0;
    let mut rrq_empty: c_int = 0;
...
// WQ was full again, so put it back on the list
    ...
    lpfc_nvmet_wqfull_flush(phba, wq, core::ptr::null_mut());
    }
...
// (payload+3), *(payload+4), *(payload+5));
    ...
    if (!nvmebuf) {
    lpfc_printf_log(phba, KERN_ERR, LOG_TRACE_EVENT,
    "6159 process_rcv_fcp_req, nvmebuf is core::ptr::null_mut(), "
...
//
    ...
    struct lpfc_nvmet_ctxbuf *ctx_buf = core::ptr::null_mut();
...
// Xmit NVMET response to remote NPORT <did>
    ...
    nvmewqe.context_un.axchg = core::ptr::null_mut();
    nvmewqe.ndlp = core::ptr::null_mut();
    nvmewqe.bpl_dmabuf = core::ptr::null_mut();
...
//
    ...
    result, wcqe.word3);
    cmdwqe.rsp_dmabuf = core::ptr::null_mut();
    cmdwqe.bpl_dmabuf = core::ptr::null_mut();
//
...
//
    ...
    {
    struct lpfc_nvmet_tgtport *tgtp = core::ptr::null_mut();
...
// Word 11
    ...
    abts_wqeq.bpl_dmabuf = core::ptr::null_mut();
...
// No failure to an ABTS request.
    ...
    if (tgtp)
    atomic_inc(&tgtp.xmt_abort_rsp_error);
    abts_wqeq.rsp_dmabuf = core::ptr::null_mut();
    abts_wqeq.bpl_dmabuf = core::ptr::null_mut();
    lpfc_sli_release_iocbq(phba, abts_wqeq);
    lpfc_printf_log(phba, KERN_ERR, LOG_TRACE_EVENT,
