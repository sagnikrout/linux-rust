...
//! Source: drivers/scsi/lpfc/lpfc_mem.c
#![no_std]
...
//
    ...
    phba.lpfc_hbq_pool = core::ptr::null_mut();
    } else {
    phba.lpfc_hbq_pool = dma_pool_create("lpfc_hbq_pool",
    &phba.pcidev.dev, LPFC_BPL_SIZE, align, 0);
    if (!phba.lpfc_hbq_pool)
    goto fail_free_nlp_mem_pool;
    phba.lpfc_hrb_pool = core::ptr::null_mut();
    phba.lpfc_drb_pool = core::ptr::null_mut();
    }
    ...
    phba.rrq_pool = core::ptr::null_mut();
    fail_free_nlp_mem_pool:
    mempool_destroy(phba.nlp_mem_pool);
    phba.nlp_mem_pool = core::ptr::null_mut();
    fail_free_mbox_pool:
    mempool_destroy(phba.mbox_mem_pool);
    phba.mbox_mem_pool = core::ptr::null_mut();
    fail_free_mbuf_pool:
    while (i--)
    ...
    phba.lpfc_mbuf_pool = core::ptr::null_mut();
    fail:
    return -ENOMEM;
    }
    int
...
// Free HBQ pools
    ...
    phba.lpfc_nvmet_drb_pool = core::ptr::null_mut();
    dma_pool_destroy(phba.lpfc_drb_pool);
    phba.lpfc_drb_pool = core::ptr::null_mut();
    dma_pool_destroy(phba.lpfc_hrb_pool);
    phba.lpfc_hrb_pool = core::ptr::null_mut();
...
// Free NLP memory pool
    ...
    phba.active_rrq_pool = core::ptr::null_mut();
    }
...
// Free the active mailbox command back to the mailbox memory pool
    ...
    lpfc_mem_free_sli_mbox(phba, mbox);
    psli.mbox_active = core::ptr::null_mut();
    }
...
// Free Congestion Data buffer
    ...
    kfree(phba.cgn_i);
    phba.cgn_i = core::ptr::null_mut();
    }
// Free RX Monitor
    if (phba.rx_monitor) {
    lpfc_rx_monitor_destroy_ring(phba.rx_monitor);
    kfree(phba.rx_monitor);
    phba.rx_monitor = core::ptr::null_mut();
    }
// Free the iocb lookup array
    kfree(psli.iocbq_lookup);
    psli.iocbq_lookup = core::ptr::null_mut();
    return;
    }
//
