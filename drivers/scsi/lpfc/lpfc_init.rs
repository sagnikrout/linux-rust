...
//! Source: drivers/scsi/lpfc/lpfc_init.c
#![no_std]
...
// Used when mapping IRQ vectors in a driver centric manner
    ...
    static struct scsi_transport_template *lpfc_transport_template = core::ptr::null_mut();
    static struct scsi_transport_template *lpfc_vport_transport_template = core::ptr::null_mut();
    static DEFINE_IDR(lpfc_hba_index);
pub const LPFC_NVMET_BUF_POST: c_int = 254;
...
//
    ...
    let mut i: c_int = 0, rc;
    LPFC_MBOXQ_t *pmb;
    MAILBOX_t *mb;
    char *lpfc_vpd_data = core::ptr::null_mut();
    let mut offset: u16 = 0;
    static char licensed[56] =
    "key unlock for use with gnu public licensed code only\0";
    let mut init_key: static int = 1;
...
// Disable interrupts
    ...
    if (vports != core::ptr::null_mut())
    for (i = 0; i <= phba.max_vports &&
    vports[i] != core::ptr::null_mut(); i++)
...
//
    ...
    {
    struct lpfc_sli *psli = &phba.sli;
    struct lpfc_queue *qp = core::ptr::null_mut();
    struct lpfc_sli_ring *pring;
...
//
    ...
    struct lpfc_sglq *sglq_entry = core::ptr::null_mut();
    int cnt, idx;
    lpfc_sli_hbqbuf_free_all(phba);
    lpfc_hba_clean_txcmplq(phba);
...
//
    ...
    unsigned char *ena_delay = core::ptr::null_mut();
    uint32_t usdelay;
    int i;
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_hb_mxp_handler(phba: *mut lpfc_hba) {
...
// Multi-XRI pools handler
    ...
    vports = lpfc_create_vport_work_array(phba);
    if (vports != core::ptr::null_mut())
    for (i = 0; i <= phba.max_vports && vports[i] != core::ptr::null_mut(); i++) {
...
//
    ...
    {
    let mut old_host_status: u32 = phba.work_hs;
    struct lpfc_sli *psli = &phba.sli;
...
//
    ...
    phba.sli.mbox_active = core::ptr::null_mut();
    }
    spin_unlock_irq(&phba.hbalock);
    }
...
//
    ...
    struct Scsi_Host *shost;
    uint32_t if_type;
    let mut portstat_reg: lpfc_register = {0};
    uint32_t reg_err1, reg_err2;
    uint32_t uerrlo_reg, uemasklo_reg;
    let mut smphr_port_status: u32 = 0, pci_rd_rc1, pci_rd_rc2;
    let mut en_rn_msg: bool = true;
    struct temp_event temp_event_data;
...
//
    ...
    {
    let mut sub_dev_id: u16 = phba.pcidev.subsystem_device;
    char *model = "<Unknown>";
    let mut tbolt: c_int = 0;
    switch (sub_dev_id) {
...
//
    ...
    {
    lpfc_vpd_t *vp;
    let mut dev_id: u16 = phba.pcidev.device;
    int max_speed;
    let mut GE: c_int = 0;
    ...
    max_speed = 128;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_64Gb: phba->lmt &) -> else {
    else if (phba.lmt & LMT_64Gb)
    max_speed = 64;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_32Gb: phba->lmt &) -> else {
    else if (phba.lmt & LMT_32Gb)
    max_speed = 32;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_16Gb: phba->lmt &) -> else {
    else if (phba.lmt & LMT_16Gb)
    max_speed = 16;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_10Gb: phba->lmt &) -> else {
    else if (phba.lmt & LMT_10Gb)
    max_speed = 10;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_8Gb: phba->lmt &) -> else {
    else if (phba.lmt & LMT_8Gb)
    max_speed = 8;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_4Gb: phba->lmt &) -> else {
    else if (phba.lmt & LMT_4Gb)
    max_speed = 4;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_2Gb: phba->lmt &) -> else {
    else if (phba.lmt & LMT_2Gb)
    max_speed = 2;
#[no_mangle]
pub unsafe extern "C" fn if(LMT_1Gb: phba->lmt &) -> else {
...
//
    ...
    phba.Port);
#[no_mangle]
pub unsafe extern "C" fn if(0: max_speed ==) -> else {
    else if (max_speed == 0)
    snprintf(descp, 255,
    "Emulex %s %s %s",
...
//
    ...
    lpfc_disc_state_machine(vport, ndlp, core::ptr::null_mut(),
...
// Wait for any activity on ndlps to settle
    ...
    lpfc_cleanup_vports_rrqs(vport, core::ptr::null_mut());
    }
//
...
// Resume any blocked IO - Queue unblock on workqueue
    ...
    {
    let mut rate: u64 = lpfc_sli_port_speed_get(phba);
...
//
    ...
    {
    unsigned long iflag;
    let mut actcmd: u8 = MBX_HEARTBEAT;
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_create_expedite_pool(phba: *mut lpfc_hba) {
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_destroy_expedite_pool(phba: *mut lpfc_hba) {
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_create_multixri_pools(phba: *mut lpfc_hba) {
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_destroy_multixri_pools(phba: *mut lpfc_hba) {
    ...
    if (!multixri_pool)
    continue;
    qp.p_multixri_pool = core::ptr::null_mut();
    spin_lock_irqsave(&qp.io_buf_list_put_lock, iflag);
// Deal with public free xri pool
    pbl_pool = &multixri_pool.pbl_pool;
    spin_lock(&pbl_pool.lock);
...
//
    ...
    struct lpfc_vport **vports;
    int i, error = 0;
    let mut vpis_cleared: bool = false;
    if (!phba)
    return 0;
    vport = phba.pport;
...
// Issue an unreg_login to all nodes on all vports
    ...
    core::ptr::null_mut(), NLP_EVT_DEVICE_RECOVERY);
...
//
    ...
    {
    struct lpfc_sglq *sglq_entry = core::ptr::null_mut(), *sglq_entry_next = core::ptr::null_mut();
...
// allocate the additional els sgls
    ...
    if (sglq_entry == core::ptr::null_mut()) {
    lpfc_printf_log(phba, KERN_ERR,
    LOG_TRACE_EVENT,
    ...
    &sglq_entry.phys);
    if (sglq_entry.virt == core::ptr::null_mut()) {
    kfree(sglq_entry);
    lpfc_printf_log(phba, KERN_ERR,
    LOG_TRACE_EVENT,
...
// update xris to els sgls on the list
    sglq_entry = core::ptr::null_mut();
    sglq_entry_next = core::ptr::null_mut();
...
//
    ...
    lpfc_cmd.hdwq = qp;
    lpfc_cmd.cur_iocbq.cmd_cmpl = core::ptr::null_mut();
...
//
    ...
    {
    struct lpfc_io_buf *lpfc_ncmd = core::ptr::null_mut(), *lpfc_ncmd_next = core::ptr::null_mut();
...
// update xris associated to remaining allocated nvme buffers
    lpfc_ncmd = core::ptr::null_mut();
    lpfc_ncmd_next = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn lpfc_get_sg_tablesize(phba: *mut lpfc_hba) -> c_ushort {
...
//
    ...
    int i;
    uint64_t wwn;
    let mut use_no_reset_hba: bool = false;
    int rc;
    u8 if_type;
...
// template is for a no reset SCSI Host
    template.eh_host_reset_handler = core::ptr::null_mut();
...
//
    ...
    {
    int ret;
    ret = idr_alloc(&lpfc_hba_index, core::ptr::null_mut(), 0, 0, GFP_KERNEL);
    return ret < 0 ? -1 : ret;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_scan_finished(shost: *mut Scsi_Host, time: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lpfc_host_supported_speeds_set(shost: *mut Scsi_Host) {
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_host_attrib_init(shost: *mut Scsi_Host) {
...
//
    ...
    lpfc_rx_monitor_report(phba, phba.rx_monitor, core::ptr::null_mut(), 0,
...
//
    ...
    let mut timer_interval: c_int = LPFC_CMF_INTERVAL;
    uint32_t ms;
    struct lpfc_cgn_stat *cgs;
    int cpu;
...
// Macro flag: #define trunk_link_status(__idx)\
...
// Macro flag: #define trunk_port_fault(__idx)\
    bf_get(lpfc_acqe_fc_la_trunk_config_port##__idx, acqe_fc) ?\
    ...
    {
    let mut port_fault: u8 = bf_get(lpfc_acqe_fc_la_trunk_linkmask, acqe_fc);
    let mut err: u8 = bf_get(lpfc_acqe_fc_la_trunk_fault, acqe_fc);
    let mut cnt: u8 = 0;
    phba.sli4_hba.link_state.speed =
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !phba->sli4_hba.conf_trunk) -> else {
...
// Issue READ_CONFIG mbox command to refresh supported speeds
    ...
    vports = lpfc_create_vport_work_array(phba);
    if (vports != core::ptr::null_mut()) {
    for (i = 0; i <= phba.max_vports && vports[i] != core::ptr::null_mut();
    i++) {
    shost = lpfc_shost_from_vport(vports[i]);
    lpfc_host_supported_speeds_set(shost);
    }
...
//
    ...
    {
    let mut event_type: u8 = bf_get(lpfc_trailer_type, acqe_fip);
    int rc;
    struct lpfc_vport *vport;
    struct lpfc_nodelist *ndlp;
...
//
    ...
    {
    let mut ret: c_int = 0;
    struct lpfc_cgn_param *p_cgn_param = core::ptr::null_mut();
    u32 *pdata = core::ptr::null_mut();
    let mut len: u32 = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_sli4_async_event_proc(phba: *mut lpfc_hba) {
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_sli4_fcf_redisc_event_proc(phba: *mut lpfc_hba) {
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_log_intr_mode(phba: *mut lpfc_hba, intr_mode: u32) {
...
//
    ...
    phba.lpfc_sg_dma_buf_pool = core::ptr::null_mut();
    fail_free_mem:
    lpfc_mem_free(phba);
    return -ENOMEM;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(LPFC_MIN_SG_SLI4_BUF_SZ: phba->cfg_sg_dma_buf_size <=) -> else {
...
//
    ...
    phba.lpfc_cmd_rsp_buf_pool = core::ptr::null_mut();
    out_free_sg_dma_buf:
    dma_pool_destroy(phba.lpfc_sg_dma_buf_pool);
    phba.lpfc_sg_dma_buf_pool = core::ptr::null_mut();
    out_free_bsmbx:
...
//
    ...
    {
    struct lpfc_iocbq *iocbq_entry = core::ptr::null_mut(), *iocbq_next = core::ptr::null_mut();
    spin_lock_irq(&phba.hbalock);
...
//
    ...
    {
    struct lpfc_iocbq *iocbq_entry = core::ptr::null_mut();
    uint16_t iotag;
    int i;
// Initialize and populate the iocb list per host.
    ...
    if (iocbq_entry == core::ptr::null_mut()) {
...
//
    ...
    {
    struct lpfc_sglq *sglq_entry = core::ptr::null_mut(), *sglq_next = core::ptr::null_mut();
...
// Free memory allocated with sli3 rings
    ...
    phba.sli.sli3_ring = core::ptr::null_mut();
    kfree(phba);
    return;
    }
//
...
// Only 1 vport (pport) will support NVME target
    phba.targetport = core::ptr::null_mut();
    phba.cfg_enable_fc4_type = LPFC_ENABLE_NVME;
...
pub const LINK_FLAGS_DEF: c_uint = 0x0;
pub const LINK_FLAGS_P2P: c_uint = 0x1;
pub const LINK_FLAGS_LOOP: c_uint = 0x2;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(LPFC_RSRC_DESC_TYPE_FCFCOE_V1_LENGTH: length !=) -> else {
...
// Only needed for header of RQ pair
    ...
    GFP_KERNEL,
    cpu_to_node(cpu));
    if (qdesc.rqbp == core::ptr::null_mut()) {
    lpfc_printf_log(phba, KERN_ERR, LOG_TRACE_EVENT,
...
// Free the CQ/WQ corresponding to the Hardware Queue
    ...
    hdwq[idx].hba_eq = core::ptr::null_mut();
    hdwq[idx].io_cq = core::ptr::null_mut();
    hdwq[idx].io_wq = core::ptr::null_mut();
    if (phba.cfg_xpsgl && !phba.nvmet_support)
...
// Free the EQ corresponding to the IRQ vector
    ...
    phba.sli4_hba.hba_eq_hdl[idx].eq = core::ptr::null_mut();
    }
    }
//
...
// Set up NVME LS Complete Queue
    ...
    core::ptr::null_mut(), 0, LPFC_NVME_LS);
    if (rc) {
    lpfc_printf_log(phba, KERN_ERR, LOG_TRACE_EVENT,
...
// Destroy the EQ corresponding to the IRQ vector
    ...
    kfree(phba.sli4_hba.cq_lookup);
    phba.sli4_hba.cq_lookup = core::ptr::null_mut();
...
//
    ...
    {
    struct lpfc_cq_event *cq_event = core::ptr::null_mut();
...
//
    ...
    {
    LPFC_MBOXQ_t *mboxq;
    let mut rc: u32 = 0, if_type;
    uint32_t shdr_status, shdr_add_status;
    uint32_t rdy_chk;
    let mut port_reset: u32 = 0;
    union lpfc_sli4_cfg_shdr *shdr;
    struct lpfc_register reg_data;
    uint16_t devid;
    if_type = bf_get(lpfc_sli_intf_if_type, &phba.sli4_hba.sli_intf);
...
//
    ...
    {
    let mut intr_mode: u32 = LPFC_INTR_ERROR;
    int retval;
...
#[no_mangle]
unsafe extern "C" fn __lpfc_cpuhp_remove(phba: *mut lpfc_hba) {
    static void __lpfc_cpuhp_remove(struct lpfc_hba *phba)
    {
    if (phba.sli_rev != LPFC_SLI_REV4)
    return;
...
#[no_mangle]
unsafe extern "C" fn lpfc_cpuhp_remove(phba: *mut lpfc_hba) {
    static void lpfc_cpuhp_remove(struct lpfc_hba *phba)
    {
    if (phba.pport &&
    test_bit(FC_OFFLINE_MODE, &phba.pport.fc_flag))
...
#[no_mangle]
unsafe extern "C" fn lpfc_cpuhp_add(phba: *mut lpfc_hba) {
    static void lpfc_cpuhp_add(struct lpfc_hba *phba)
    {
    if (phba.sli_rev != LPFC_SLI_REV4)
    return;
...
#[no_mangle]
unsafe extern "C" fn __lpfc_cpuhp_checks(phba: *mut lpfc_hba, retval: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lpfc_cpu_offline(cpu: c_uint, node: *mut hlist_node) -> c_int {
    ...
    int retval;
    if (!phba) {
    WARN_ONCE(!phba, "cpu: %u. phba:core::ptr::null_mut()", raw_smp_processor_id());
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn lpfc_cpu_online(cpu: c_uint, node: *mut hlist_node) -> c_int {
...
//
    ...
    char *name;
    const struct cpumask *aff_mask = core::ptr::null_mut();
    let mut cpu: c_uint = 0, cpu_cnt = 0, cpu_select = nr_cpu_ids;
    struct lpfc_vector_map_info *cpup;
    struct lpfc_hba_eq_hdl *eqhdl;
    const struct cpumask *maskp;
...
//
    ...
    struct lpfc_sli4_hdw_queue *qp;
    int idx, ccnt;
    let mut wait_time: c_int = 0;
    let mut io_xri_cmpl: c_int = 1;
    let mut nvmet_xri_cmpl: c_int = 1;
    let mut els_xri_cmpl: c_int = list_empty(&phba.sli4_hba.lpfc_abts_els_sgl_list);
...
//
    ...
    uint32_t mbox_tmo;
    int length;
    let mut exp_wqcq_pages: bool = true;
    struct lpfc_sli4_parameters *mbx_sli4_parameters;
//
...
//
    ...
    {
    struct lpfc_hba   *phba;
    struct lpfc_vport *vport = core::ptr::null_mut();
    struct Scsi_Host  *shost = core::ptr::null_mut();
    int error;
...
//
    ...
    {
    let mut max_xri: c_int = phba.sli4_hba.max_cfg_param.max_xri;
    if (phba.sli_rev == LPFC_SLI_REV4) {
    if (max_xri <= 100)
    return 10;
#[no_mangle]
pub unsafe extern "C" fn if(256: max_xri <=) -> else {
    else if (max_xri <= 256)
    return 25;
#[no_mangle]
pub unsafe extern "C" fn if(512: max_xri <=) -> else {
    else if (max_xri <= 512)
    return 50;
#[no_mangle]
pub unsafe extern "C" fn if(1024: max_xri <=) -> else {
    else if (max_xri <= 1024)
    return 100;
#[no_mangle]
pub unsafe extern "C" fn if(1536: max_xri <=) -> else {
    else if (max_xri <= 1536)
    return 150;
#[no_mangle]
pub unsafe extern "C" fn if(2048: max_xri <=) -> else {
...
//
    ...
    {
    let mut max_xri: c_int = lpfc_sli4_get_els_iocb_cnt(phba);
    if (phba.nvmet_support)
...
//
    ...
    let mut offset: u32 = 0, temp_offset = 0;
...
//
    ...
    {
    let mut offline: c_int = pci_channel_offline(phba.pcidev);
    lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
...
//
    ...
    struct Scsi_Host *shost = pci_get_drvdata(pdev);
    struct lpfc_hba *phba = ((struct lpfc_vport *)shost.hostdata).phba;
    let mut rc: pci_ers_result_t = PCI_ERS_RESULT_DISCONNECT;
    if (phba.link_state == LPFC_HBA_ERROR &&
...
//
    ...
    phba.device_data_mem_pool = core::ptr::null_mut();
    }
    return;
    }
//
...
//
    ...
    if (lpfc_transport_template == core::ptr::null_mut())
    goto unregister;
    lpfc_vport_transport_template =
    fc_attach_transport(&lpfc_vport_transport_functions);
    if (lpfc_vport_transport_template == core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn lpfc_dmp_dbg(phba: *mut lpfc_hba) {
    ...
    __printf(2, 3)
#[no_mangle]
pub unsafe extern "C" fn lpfc_dbg_print(phba: *mut lpfc_hba, fmt: *const c_char, ...) {
    void lpfc_dbg_print(struct lpfc_hba *phba, const char *fmt, ...)
    {
    unsigned int idx;
    va_list args;
    let mut dbg_dmping: c_int = atomic_read(&phba.dbg_log_dmping);
