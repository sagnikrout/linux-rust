...
//! Source: drivers/scsi/lpfc/lpfc_vmid.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_vmid_hash_fn(vmid: *const c_char, len: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: vport->phba->cfg_vmid_app_header) -> else {
    else if (vport.phba.cfg_vmid_app_header)
    tag.app_id = vmp.un.app_id;
    if (iodir == DMA_TO_DEVICE)
    vmp.io_wr_cnt++;
#[no_mangle]
pub unsafe extern "C" fn if(DMA_FROM_DEVICE: iodir ==) -> else {
...
//
    ...
    union lpfc_vmid_io_tag *tag)
    {
    struct lpfc_vmid *vmp = core::ptr::null_mut();
