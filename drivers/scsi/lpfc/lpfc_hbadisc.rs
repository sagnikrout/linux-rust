...
//! Source: drivers/scsi/lpfc/lpfc_hbadisc.c
#![no_std]
...
//
    ...
    if (!rport) {
    pr_err("**** %s: core::ptr::null_mut() rport, exit.\n", __func__);
    return -EINVAL;
    }
    ...
    rdata = rport.dd_data;
    if (!rdata) {
    pr_err("**** %s: core::ptr::null_mut() dd_data on rport x%px SID x%x\n",
    __func__, rport, rport.scsi_target_id);
    return -EINVAL;
    }
    ndlp = rdata.pnode;
    if (!rdata.pnode) {
    pr_info("**** %s: core::ptr::null_mut() ndlp on rport x%px SID x%x\n",
    __func__, rport, rport.scsi_target_id);
    return -EINVAL;
    }
    if (!ndlp.vport) {
...
//
    ...
    let mut drop_initial_node_ref: bool = false;
...
//
    ndlp.fc4_xpt_flags &= ~SCSI_XPT_REGD;
    ((struct lpfc_rport_data *)rport.dd_data).pnode = core::ptr::null_mut();
    ndlp.rport = core::ptr::null_mut();
    spin_unlock_irqrestore(&ndlp.lock, iflags);
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_check_inactive_vmid_one(vport: *mut lpfc_vport) {
    static void lpfc_check_inactive_vmid_one(struct lpfc_vport *vport)
    {
    u16 keep;
    let mut difftime: u32 = 0, r, bucket;
    u64 *lta;
    int cpu;
...
// entry in slot as free
    ...
    ht.last_io_time = core::ptr::null_mut();
    hash_del(&ht.hnode);
    }
    }
    }
...
//
#[no_mangle]
unsafe extern "C" fn lpfc_check_inactive_vmid(phba: *mut lpfc_hba) {
    static void lpfc_check_inactive_vmid(struct lpfc_hba *phba)
    {
    struct lpfc_vport *vport;
...
//
    ...
    struct lpfc_hba   *phba;
    uint8_t *name;
    let mut warn_on: c_int = 0;
    let mut fcf_inuse: c_int = 0;
    let mut recovering: bool = false;
    struct fc_vport *fc_vport = core::ptr::null_mut();
    unsigned long iflags;
    vport = ndlp.vport;
...
#[no_mangle]
unsafe extern "C" fn lpfc_check_vmid_qfpa_issue(phba: *mut lpfc_hba) {
    static void lpfc_check_vmid_qfpa_issue(struct lpfc_hba *phba)
    {
    struct lpfc_vport *vport;
    struct lpfc_vport **vports;
...
//
    ...
    {
    struct lpfc_work_evt  *evtp = core::ptr::null_mut();
    struct lpfc_nodelist  *ndlp;
    int free_evt;
    int fcf_inuse;
    uint32_t nlp_did;
...
// Handle the error attention event
    ...
    phba.fw_dump_cmpl = core::ptr::null_mut();
    }
    }
    if (ha_copy & HA_MBATT)
...
//
    if (vports[i] == core::ptr::null_mut() && i == 0)
    vport = phba.pport;
    else
    vport = vports[i];
    if (vport == core::ptr::null_mut())
    break;
...
// Attend pending lpfc data processing
    ...
    phba.worker_thread = core::ptr::null_mut();
...
// turn on Link Attention interrupts
    ...
    let mut status: u16 = pmb.u.mb.mbxStatus;
    int rc;
    mempool_free(pmb, phba.mbox_mem_pool);
    if (status)
    goto out;
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: fcf_vlan_id) -> else {
    else if (fcf_vlan_id)
// vlan_id = fcf_vlan_id;
    else
// vlan_id = LPFC_FCOE_NULL_VID;
...
//
#[no_mangle]
pub unsafe extern "C" fn lpfc_sli4_fcf_rr_next_proc(vport: *mut lpfc_vport, fcf_index: u16) -> c_int {
...
//
    ...
    struct lpfc_fcf_rec *fcf_rec = core::ptr::null_mut();
    let mut vlan_id: u16 = LPFC_FCOE_NULL_VID;
    bool select_new_fcf;
    int rc;
...
// Stop FCF redisc wait timer
    ...
    phba);
#[no_mangle]
pub unsafe extern "C" fn if(FCF_REDISC_FOV: phba->fcf.fcf_flag &) -> else {
...
//
    ...
    {
    struct lpfc_vport *vport = phba.pport;
    LPFC_MBOXQ_t *sparam_mbox, *cfglink_mbox = core::ptr::null_mut();
    int i;
    int rc;
    struct fcf_record *fcf_record;
    unsigned long iflags;
...
#[no_mangle]
pub unsafe extern "C" fn if(LPFC_ATT_UNEXP_WWPN: attn_type ==) -> else {
...
// If VPI is busy, reset the HBA
    ...
    if (!test_bit(FC_UNLOADING, &phba.pport.load_flag))
    lpfc_workq_post_event(phba, core::ptr::null_mut(), core::ptr::null_mut(),
    LPFC_EVT_RESET_HBA);
    }
    set_bit(FC_VPORT_NEEDS_REG_VPI, &vport.fc_flag);
    spin_lock_irq(shost.host_lock);
...
//
    ...
    {
    LPFC_MBOXQ_t *pmb = core::ptr::null_mut();
    MAILBOX_t *mb;
    struct static_vport_info *vport_info;
    let mut mbx_wait_rc: c_int = 0, i;
...
//
    ...
    int state)
    {
    let mut old_state: c_int = ndlp.nlp_state;
    let mut node_dropped: bool = test_bit(NLP_DROPPED, &ndlp.nlp_flag);
...
// The mempool_alloc might sleep
    ...
    mbox.vport = vport;
    mbox.mbox_cmpl = lpfc_sli_def_mbox_cmpl;
    mbox.ctx_ndlp = core::ptr::null_mut();
...
// Search for a nodelist entry
    ...
    {
    struct lpfc_nodelist *ndlp;
    struct lpfc_nodelist *np = core::ptr::null_mut();
    uint32_t data1;
    list_for_each_entry(ndlp, &vport.fc_nodes, nlp_listp) {
...
//
    ...
    return;
// Link up discovery
    if ((mbox = mempool_alloc(phba.mbox_mem_pool, GFP_KERNEL)) != core::ptr::null_mut()) {
...
// Next check the txcmplq
    ...
    if (ulp_command == CMD_ELS_REQUEST64_CR ||
    ulp_command == CMD_XMIT_ELS_RSP64_CX) {
    lpfc_sli_issue_abort_iotag(phba, pring, iocb, core::ptr::null_mut());
    }
    }
    spin_unlock_irq(&phba.hbalock);
...
//
    ...
    {
    struct lpfc_nodelist *ndlp;
    let mut rpi: c_int = LPFC_RPI_ALLOC_ERROR;
    if (vport.phba.sli_rev == LPFC_SLI_REV4) {
...
//
    ...
    WARN_ONCE(!ndlp, "**** %s, get ref on core::ptr::null_mut() ndlp!", __func__);
    }
    return ndlp;
    }
...
//
    ...
    WARN_ONCE(!ndlp, "**** %s, put ref on core::ptr::null_mut() ndlp!", __func__);
    }
...
//
    ...
    {
    let mut offset: u32 = 0, rec_length;
...
// Mask off the extra port data to get just the reason code.
    let mut rsn_code: u32 = IOERR_PARAM_MASK & ulp_word4;
