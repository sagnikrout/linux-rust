...
//! Source: drivers/hid/hid-nvidia-shield.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunderstrike_psy_prop_values {
    pub voltage_min: c_int,
    pub voltage_now: c_int,
    pub voltage_avg: c_int,
    pub voltage_boot: c_int,
    pub capacity: c_int,
    pub status: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunderstrike_hostcmd_battery {
    pub voltage_avg: __le16,
    pub reserved_at_10: u8,
    pub thermistor: __le16,
    pub voltage_min: __le16,
    pub voltage_boot: __le16,
    ...
    pub __packed: },
    pub 1): static_assert(sizeof(enum thunderstrike_charger_type) ==,
    ...
    pub __packed: },
    pub 1): static_assert(sizeof(enum thunderstrike_charger_state) ==,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunderstrike_hostcmd_charger {
    pub connected: u8,
    pub type: enum thunderstrike_charger_type,
    pub state: enum thunderstrike_charger_state,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunderstrike_hostcmd_board_info {
    pub revision: __le16,
    pub serial: [__le16; 7],
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunderstrike_hostcmd_haptics {
    pub motor_left: u8,
    pub motor_right: u8,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunderstrike_hostcmd_resp_report {
    pub /: *mut *mut u8 report_id; / THUNDERSTRIKE_HOSTCMD_RESP_REPORT_ID,
    pub cmd_id: u8,
    pub reserved_at_10: u8,
    union {
    pub board_info: thunderstrike_hostcmd_board_info,
    pub motors: thunderstrike_hostcmd_haptics,
    pub fw_version: __le16,
    pub led_state: enum thunderstrike_led_state,
    pub battery: thunderstrike_hostcmd_battery,
    pub charger: thunderstrike_hostcmd_charger,
    pub payload: [u8; 30],
    pub __packed: },
    pub __packed: },
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunderstrike_hostcmd_req_report {
    pub /: *mut *mut u8 report_id; / THUNDERSTRIKE_HOSTCMD_REQ_REPORT_ID,
    pub cmd_id: u8,
    pub reserved_at_10: u8,
    union {
#[repr(C)]
...
pub struct __packed {
    pub update: u8,
    pub state: enum thunderstrike_led_state,
    pub led: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct __packed {
    ...
    pub haptics: },
    pub __packed: },
    pub reserved_at_30: [u8; 27],
    pub __packed: },
    static_assert(sizeof(struct thunderstrike_hostcmd_req_report) ==
// Common struct for shield accessories.
#[repr(C)]
#[derive(Copy, Clone)]
pub struct shield_device {
    pub hdev: *mut hid_device,
    pub battery_dev: power_supply_dev,
    pub initialized_flags: c_ulong,
    pub codename: *const c_char,
    pub fw_version: u16,
    struct {
    pub revision: u16,
...
pub struct thunderstrike {
    pub base: shield_device,
    pub id: c_int,
// Sub-devices
    pub haptics_dev: *mut input_dev,
    pub led_dev: led_classdev,
// Resources
    pub req_report_dmabuf: *mut c_void,
    pub update_flags: c_ulong,
    pub haptics_val: thunderstrike_hostcmd_haptics,
    pub haptics_update_lock: spinlock_t,
    pub 1: u8 led_state :,
    pub led_value: enum thunderstrike_led_state,
    pub psy_stats: thunderstrike_psy_prop_values,
    pub psy_stats_lock: spinlock_t,
    pub psy_stats_timer: timer_list,
    pub hostcmd_req_work: work_struct,
}
...
#[no_mangle]
pub unsafe extern "C" fn shield_strrev(dest: *mut c_char, len: usize, rev: u16) {
    ...
    ret = input_ff_create_memless(haptics, core::ptr::null_mut(), play_effect);
    if (ret)
    goto err;
...
#[no_mangle]
pub unsafe extern "C" fn thunderstrike_send_hostcmd_request(ts: *mut thunderstrike) {
...
#[no_mangle]
unsafe extern "C" fn thunderstrike_hostcmd_req_work_handler(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn thunderstrike_request_firmware_version(ts: *mut thunderstrike) {
...
#[no_mangle]
pub unsafe extern "C" fn thunderstrike_request_board_info(ts: *mut thunderstrike) {
...
#[no_mangle]
pub unsafe extern "C" fn thunderstrike_request_psy_stats(ts: *mut thunderstrike) {
...
#[no_mangle]
unsafe extern "C" fn thunderstrike_psy_stats_timer_handler(timer: *mut timer_list) {
...
// Query battery statistics from device every five minutes
    ...
    for (i = 0; i < 7; ++i) {
    let mut val: u16 = le16_to_cpu(board_info.serial[i]);
    ...
    {
    struct thunderstrike *ts = container_of(shield_dev, struct thunderstrike, base);
    let mut hostcmd_voltage_boot: u16 = le16_to_cpu(battery.voltage_boot);
    let mut hostcmd_voltage_avg: u16 = le16_to_cpu(battery.voltage_avg);
    let mut hostcmd_voltage_min: u16 = le16_to_cpu(battery.voltage_min);
    let mut hostcmd_voltage_now: u16 = le16_to_cpu(battery.voltage_now);
    let mut hostcmd_thermistor: u16 = le16_to_cpu(battery.thermistor);
    int voltage_boot, voltage_avg, voltage_min, voltage_now;
    struct hid_device *hdev = shield_dev.hdev;
    let mut capacity: u8 = battery.capacity;
    int temp;
...
// Copy converted values
    ...
    {
    struct thunderstrike *ts = container_of(shield_dev, struct thunderstrike, base);
    let mut charge_type: c_int = POWER_SUPPLY_CHARGE_TYPE_UNKNOWN;
    struct hid_device *hdev = shield_dev.hdev;
    let mut status: c_int = POWER_SUPPLY_STATUS_UNKNOWN;
    switch (charger.type) {
...
#[no_mangle]
pub unsafe extern "C" fn thunderstrike_device_init_info(shield_dev: *mut shield_device) {
...
#[no_mangle]
pub unsafe extern "C" fn thunderstrike_led_create(ts: *mut thunderstrike) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn thunderstrike_psy_create(shield_dev: *mut shield_device) -> c_int {
    static inline int thunderstrike_psy_create(struct shield_device *shield_dev)
    {
    struct thunderstrike *ts = container_of(shield_dev, struct thunderstrike, base);
    let mut psy_cfg: power_supply_config = { .drv_data = shield_dev, };
    struct hid_device *hdev = shield_dev.hdev;
    int ret;
//
...
#[no_mangle]
unsafe extern "C" fn thunderstrike_destroy(ts: *mut thunderstrike) {
...
#[no_mangle]
unsafe extern "C" fn shield_probe(hdev: *mut hid_device, id: *const hid_device_id) -> c_int {
    static int shield_probe(struct hid_device *hdev, const struct hid_device_id *id)
    {
    struct shield_device *shield_dev = core::ptr::null_mut();
    struct thunderstrike *ts;
    int ret;
    ret = hid_parse(hdev);
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn shield_remove(hdev: *mut hid_device) {
