...
//! Source: drivers/hid/hid-apple.c
#![no_std]
...
pub const HID_COUNTRY_INTERNATIONAL_ISO: c_int = 13;
pub const APPLE_BATTERY_TIMEOUT_SEC: c_int = 60;
pub const HID_USAGE_MAGIC_BL: c_uint = 0xff00000f;
pub const APPLE_MAGIC_REPORT_ID_POWER: c_int = 3;
pub const APPLE_MAGIC_REPORT_ID_BRIGHTNESS: c_int = 1;
    let mut fnmode: static unsigned int = 3;
    module_param(fnmode, uint, 0644);
    MODULE_PARM_DESC(fnmode, "Mode of fn key on Apple keyboards (0 = disabled, "
    "1 = fkeyslast, 2 = fkeysfirst, [3] = auto, 4 = fkeysdisabled)");
    let mut iso_layout: static int = -1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct apple_non_apple_keyboard {
    pub name: *mut c_char,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct apple_sc_backlight {
    pub cdev: led_classdev,
    pub hdev: *mut hid_device,
}

// T2 VHCI re-enumerates the internal keyboard across system resume.
    let mut apple_backlight_resume_brightness: static int = -1;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct apple_backlight_config_report {
    pub report_id: u8,
    pub version: u8,
    pub backlight_on_max: u16 backlight_off, backlight_on_min,,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct apple_backlight_set_report {
    pub report_id: u8,
    pub version: u8,
    pub backlight: u16,
    pub rate: u16,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct apple_magic_backlight {
    pub cdev: led_classdev,
    pub brightness: *mut hid_report,
    pub power: *mut hid_report,
}
...
pub struct apple_sc {
    ...
    pub fn_on: c_uint,
    pub fn_found: c_uint,
    pub KEY_CNT): DECLARE_BITMAP(pressed_numlock,,
    pub battery_timer: timer_list,
    pub backlight: *mut apple_sc_backlight,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct apple_key_translation {
    pub from: u16,
    pub to: u16,
    pub flags: c_ulong,
}
...
#[no_mangle]
unsafe extern "C" fn apple_is_non_apple_keyboard(hdev: *mut hid_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn apple_is_omoton_kb066(hdev: *mut hid_device) -> bool {
...
// Look for the translation
    ...
    bool do_translate;
    let mut code: u16 = usage.code;
    unsigned int real_fnmode;
    if (fnmode == 3) {
    if (asc.quirks & APPLE_DISABLE_FKEYS)
    real_fnmode = 4;
#[no_mangle]
pub unsafe extern "C" fn if(APPLE_IS_NON_APPLE: asc->quirks &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(0x300: hid->product < 0x21d || hid->product >=) -> else {
    ...
    if (trans) {
    let mut from_is_set: bool = test_bit(trans.from, input.key);
    let mut to_is_set: bool = test_bit(trans.to, input.key);
    if (from_is_set)
    code = trans.from;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: to_is_set) -> else {
    else if (to_is_set)
    code = trans.to;
...
#[no_mangle]
unsafe extern "C" fn apple_fetch_battery(hdev: *mut hid_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn apple_battery_timer_tick(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn apple_setup_input(input: *mut input_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn if(BTN_1: usage->code ==) -> else {
    else if (usage.code == BTN_1)
    hid_map_usage(hi, usage, bit, max, EV_KEY, BTN_2);
#[no_mangle]
pub unsafe extern "C" fn if(BTN_2: usage->code ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn apple_backlight_check_support(hdev: *mut hid_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn apple_backlight_set(hdev: *mut hid_device, value: u16, rate: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn apple_backlight_init(hdev: *mut hid_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn apple_magic_backlight_report_set(rep: *mut hid_report, value: i32, rate: u8) {
...
#[no_mangle]
unsafe extern "C" fn apple_magic_backlight_init(hdev: *mut hid_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn apple_remove(hdev: *mut hid_device) {
