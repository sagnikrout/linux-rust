...
//! Source: drivers/hid/hid-thrustmaster.c
#![no_std]
...
// {0x0407, 0x0001, "Thrustmaster TMX"}
    ...
    let mut tm_wheels_infos_length: static uint8_t = 7;
//
...
pub struct tm_wheel {
    ...
    pub urb: *mut urb,
    pub model_request: *mut usb_ctrlrequest,
    pub response: *mut tm_wheel_response,
    pub change_request: *mut usb_ctrlrequest,
}
...
//
#[no_mangle]
unsafe extern "C" fn thrustmaster_interrupts(hdev: *mut hid_device) {
...
#[no_mangle]
unsafe extern "C" fn thrustmaster_change_handler(urb: *mut urb) {
...
//
#[no_mangle]
unsafe extern "C" fn thrustmaster_model_handler(urb: *mut urb) {
    ...
    let mut model: u16 = 0;
    int i, ret;
    const struct tm_wheel_info *twi = core::ptr::null_mut();
    if (urb.status) {
...
#[no_mangle]
pub unsafe extern "C" fn if(cpu_to_le16(0x47): tm_wheel->response->type ==) -> else {
    ...
    core::ptr::null_mut(), 0, // We do not expect any response from the wheel
...
#[no_mangle]
unsafe extern "C" fn thrustmaster_remove(hdev: *mut hid_device) {
...
//
#[no_mangle]
unsafe extern "C" fn thrustmaster_probe(hdev: *mut hid_device, id: *const hid_device_id) -> c_int {
    static int thrustmaster_probe(struct hid_device *hdev, const struct hid_device_id *id)
    {
    let mut ret: c_int = 0;
    struct tm_wheel *tm_wheel = core::ptr::null_mut();
    if (!hid_is_usb(hdev))
    return -EINVAL;
    ret = hid_parse(hdev);
    if (ret) {
