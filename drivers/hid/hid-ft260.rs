...
//! Source: drivers/hid/hid-ft260.c
#![no_std]
...
//
    ...
    let mut ft260_debug: static int = 1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_get_chip_version_report {
    pub /: *mut *mut u8 report; / FT260_CHIP_VERSION,
    pub /: *mut *mut u8 chip_code[4]; / FTDI chip identification code,
    pub reserved: [u8; 8],
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_get_system_status_report {
    pub /: *mut *mut u8 report; / FT260_SYSTEM_SETTINGS,
    pub /: *mut *mut u8 chip_mode; / DCNF0 and DCNF1 status, bits 0-1,
    pub /: *mut *mut u8 clock_ctl; / 0 - 12MHz, 1 - 24MHz, 2 - 48MHz,
    pub /: *mut *mut u8 suspend_status; / 0 - not suspended, 1 - suspended,
    pub /: *mut *mut u8 pwren_status; / 0 - FT260 is not ready, 1 - ready,
    pub /: *mut *mut u8 i2c_enable; / 0 - disabled, 1 - enabled,
    pub /: *mut *mut u8 uart_mode; / 0 - OFF; 1 - RTS_CTS, 2 - DTR_DSR,,
// 3 - XON_XOFF, 4 - No flow control
    pub /: *mut *mut u8 hid_over_i2c_en; / 0 - disabled, 1 - enabled,
    pub /: *mut *mut u8 gpio2_function; / 0 - GPIO, 1 - SUSPOUT,,
// 2 - PWREN, 4 - TX_LED
    pub /: *mut *mut u8 gpioA_function; / 0 - GPIO, 3 - TX_ACTIVE, 4 - TX_LED,
    pub /: *mut *mut u8 gpioG_function; / 0 - GPIO, 2 - PWREN,,
// 5 - RX_LED, 6 - BCD_DET
    pub /: *mut *mut u8 suspend_out_pol; / 0 - active-high, 1 - active-low,
    pub /: *mut *mut u8 enable_wakeup_int; / 0 - disabled, 1 - enabled,
    pub /: *mut *mut u8 intr_cond; / Interrupt trigger conditions,
    pub /: *mut *mut u8 power_saving_en; / 0 - disabled, 1 - enabled,
    pub reserved: [u8; 10],
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_get_i2c_status_report {
    pub /: *mut *mut u8 report; / FT260_I2C_STATUS,
    pub /: *mut *mut u8 bus_status; / I2C bus status,
    pub /: *mut *mut __le16 clock; / I2C bus clock in range 60-3400 KHz,
    pub reserved: u8,
    pub __packed: },
// Feature Out reports
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_set_system_clock_report {
    pub /: *mut *mut u8 report; / FT260_SYSTEM_SETTINGS,
    pub /: *mut *mut u8 request; / FT260_SET_CLOCK,
    pub /: *mut *mut u8 clock_ctl; / 0 - 12MHz, 1 - 24MHz, 2 - 48MHz,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_set_i2c_mode_report {
    pub /: *mut *mut u8 report; / FT260_SYSTEM_SETTINGS,
    pub /: *mut *mut u8 request; / FT260_SET_I2C_MODE,
    pub /: *mut *mut u8 i2c_enable; / 0 - disabled, 1 - enabled,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_set_uart_mode_report {
    pub /: *mut *mut u8 report; / FT260_SYSTEM_SETTINGS,
    pub /: *mut *mut u8 request; / FT260_SET_UART_MODE,
    pub /: *mut *mut u8 uart_mode; / 0 - OFF; 1 - RTS_CTS, 2 - DTR_DSR,,
// 3 - XON_XOFF, 4 - No flow control
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_set_i2c_reset_report {
    pub /: *mut *mut u8 report; / FT260_SYSTEM_SETTINGS,
    pub /: *mut *mut u8 request; / FT260_SET_I2C_RESET,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_set_i2c_speed_report {
    pub /: *mut *mut u8 report; / FT260_SYSTEM_SETTINGS,
    pub /: *mut *mut u8 request; / FT260_SET_I2C_CLOCK_SPEED,
    pub /: *mut *mut __le16 clock; / I2C bus clock in range 60-3400 KHz,
    pub __packed: },
// Data transfer reports
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_i2c_write_request_report {
    pub /: *mut *mut u8 report; / FT260_I2C_REPORT,
    pub /: *mut *mut u8 address; / 7-bit I2C address,
    pub /: *mut *mut u8 flag; / I2C transaction condition,
    pub /: *mut *mut u8 length; / data payload length,
    pub /: *mut *mut u8 data[FT260_WR_DATA_MAX]; / data payload,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_i2c_read_request_report {
    pub /: *mut *mut u8 report; / FT260_I2C_READ_REQ,
    pub /: *mut *mut u8 address; / 7-bit I2C address,
    pub /: *mut *mut u8 flag; / I2C transaction condition,
    pub /: *mut *mut __le16 length; / data payload length,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_i2c_input_report {
    pub /: *mut *mut u8 report; / FT260_I2C_REPORT,
    pub /: *mut *mut u8 length; / data payload length,
    pub /: *mut *mut u8 data[2]; / data payload,
    pub __packed: },
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ft260_device {
    pub adap: i2c_adapter,
    pub hdev: *mut hid_device,
    pub wait: completion,
    pub lock: mutex,
    pub write_buf: [u8; FT260_REPORT_MAX_LENGTH],
    pub need_wakeup_at: c_ulong,
...
#[no_mangle]
unsafe extern "C" fn ft260_i2c_reset(hdev: *mut hid_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ft260_xfer_status(dev: *mut ft260_device, bus_busy: u8) -> c_int {
...
pub unsafe extern "C" fn if(FT260_FLAG_START: flag &) -> else {
    ...
    spin_lock_irqsave(&dev.read_lock, irqflags);
    dev.read_buf = core::ptr::null_mut();
    spin_unlock_irqrestore(&dev.read_lock, irqflags);
...
//
#[no_mangle]
unsafe extern "C" fn ft260_i2c_write_read(dev: *mut ft260_device, msgs: *mut i2c_msg) -> c_int {
    static int ft260_i2c_write_read(struct ft260_device *dev, struct i2c_msg *msgs)
    {
    int ret;
    let mut wr_len: c_int = msgs[0].len;
    let mut rd_len: c_int = msgs[1].len;
    struct hid_device *hdev = dev.hdev;
    let mut addr: u8 = msgs[0].addr;
    let mut read_off: u16 = 0;
    if (wr_len > 2) {
...
// Combined write then read message
    ...
    ret = ft260_smbus_write(dev, addr, cmd, core::ptr::null_mut(), 0,
    FT260_FLAG_START_STOP);
    break;
...
#[no_mangle]
unsafe extern "C" fn ft260_functionality(adap: *mut i2c_adapter) -> u32 {
    ...
    {
    int ret;
    let mut len: c_int = sizeof(struct ft260_get_system_status_report);
...
#[no_mangle]
unsafe extern "C" fn ft260_is_interface_enabled(hdev: *mut hid_device) -> c_int {
    static int ft260_is_interface_enabled(struct hid_device *hdev)
    {
    struct ft260_get_system_status_report cfg;
    struct usb_interface *usbif = to_usb_interface(hdev.dev.parent);
    let mut interface: c_int = usbif.cur_altsetting.desc.bInterfaceNumber;
    int ret;
    ...
    {
    struct hid_device *hdev = to_hid_device(kdev);
    let mut ret: c_int = ft260_i2c_reset(hdev);
    if (ret)
    return ret;
    return count;
    }
...
#[no_mangle]
unsafe extern "C" fn ft260_probe(hdev: *mut hid_device, id: *const hid_device_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ft260_remove(hdev: *mut hid_device) {
...
//
    spin_lock_irqsave(&dev.read_lock, irqflags);
    if ((dev.read_buf == core::ptr::null_mut()) ||
