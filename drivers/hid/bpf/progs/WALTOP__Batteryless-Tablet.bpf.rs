...
//! Source: drivers/hid/bpf/progs/WALTOP__Batteryless-Tablet.bpf.c
#![no_std]
...
pub const VID_WALTOP: c_uint = 0x172F;
pub const PID_BATTERYLESS_TABLET: c_uint = 0x0505;
    HID_BPF_CONFIG(
    HID_DEVICE(BUS_USB, HID_GROUP_ANY, VID_WALTOP, PID_BATTERYLESS_TABLET)
    );
pub const EXPECTED_RDESC_SIZE: c_int = 335;
pub const PEN_REPORT_ID: c_int = 16;
...
#[no_mangle]
pub unsafe extern "C" fn bitwidth32(x: __u32) -> c_uint {
    static inline unsigned int bitwidth32(__u32 x)
    {
    return 32 - __builtin_clzg(x, 32);
    }
#[no_mangle]
pub unsafe extern "C" fn floor_log2_32(x: __u32) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn scaled_log2(v: __u16) -> c_uint {
    static unsigned int scaled_log2(__u16 v)
    {
    let mut XMAX: c_uint = 2047;
    const unsigned int YMAX = 11; /* log2(2048) = 11 */
    let mut x: c_uint = v + 1;
    let mut n: c_uint = floor_log2_32(x);
    let mut b: c_uint = 1 << n;
// Fixed-point fraction in [0, 1), linearly
// interpolated using delta-y = 1 and
// delta-x = (2b - b) = b.
//
    let mut frac: c_uint = (x - b) << YMAX;
    let mut lerp: c_uint = frac / b;
    let mut log2: c_uint = (n << YMAX) + lerp;
...
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: waltop_fix_events, hctx: *mut hid_bpf_ctx) -> c_int {
    int BPF_PROG(waltop_fix_events, struct hid_bpf_ctx *hctx)
    {
    __u8 *data = hid_bpf_get_data(hctx, 0 /* offset */, 10 /* size */);
    if (!data)
    return 0; /* EPERM check */
    let mut report_id: __u8 = data[0];
...
//
    let mut tip_switch: __u8 = data[1] & TIP_SWITCH;
    let mut barrel_switch: __u8 = data[1] & BARREL_SWITCH;
    let mut tip_held: __u8 = last_button_state & TIP_SWITCH;
    let mut barrel_held: __u8 = last_button_state & BARREL_SWITCH;
...
// value 100.
//
    let mut pressure: __u16 = (((__u16)data[6]) << 0) | (((__u16)data[7]) << 8);
    if (pressure <= 102)
    pressure *= 12;
    else
