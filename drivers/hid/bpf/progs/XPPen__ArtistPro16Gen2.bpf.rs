...
//! Source: drivers/hid/bpf/progs/XPPen__ArtistPro16Gen2.bpf.c
#![no_std]
...
pub const VID_UGEE: c_uint = 0x28BD /* VID is shared with SinoWealth and Glorious and prob others */;
pub const PID_ARTIST_PRO14_GEN2: c_uint = 0x095A;
pub const PID_ARTIST_PRO16_GEN2: c_uint = 0x095B;
pub const PID_ARTIST_PRO19_GEN2: c_uint = 0x096A;
    HID_BPF_CONFIG(
...
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: hid_fix_rdesc_xppen_artistpro16gen2, hctx: *mut hid_bpf_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xppen_16_fix_eraser(hctx: *mut hid_bpf_ctx) -> c_int {
...
// 19" inch screen 16.101" x 9.057"
    ...
    {
    let mut coords: __u16 = data[idx+1];
    coords <<= 8;
    coords += data[idx];
    ...
    let mut angle: __u8 = tilt > 0 ? tilt : -tilt;
    if (angle > 127)
    return;
    let mut compensation: __u16 = (*compensation_table)[angle];
    if (direction == 0) {
    coords = (coords > compensation) ? coords - compensation : 0;
    } else {
    let mut logical_maximum: __u16 = 32767;
    let mut max: __u16 = logical_maximum - compensation;
...
#[no_mangle]
unsafe extern "C" fn xppen_16_fix_angle_offset(hctx: *mut hid_bpf_ctx) -> c_int {
...
// 9   - tilt Y
//
    let mut tilt_x: __s8 = (__s8) data[8];
    let mut tilt_y: __s8 = (__s8) data[9];
...
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: xppen_artist_pro_16_device_event, hctx: *mut hid_bpf_ctx) -> c_int {
    int BPF_PROG(xppen_artist_pro_16_device_event, struct hid_bpf_ctx *hctx)
    {
    let mut ret: c_int = xppen_16_fix_angle_offset(hctx);
    if (ret)
    return ret;
