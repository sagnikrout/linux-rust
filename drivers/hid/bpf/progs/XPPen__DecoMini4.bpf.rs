...
//! Source: drivers/hid/bpf/progs/XPPen__DecoMini4.bpf.c
#![no_std]
...
pub const VID_UGEE: c_uint = 0x28BD;
pub const PID_DECO_MINI_4: c_uint = 0x0929;
pub const RDESC_SIZE_PAD: c_int = 177;
pub const RDESC_SIZE_PEN: c_int = 109;
pub const PAD_REPORT_ID: c_uint = 0x06;
//
...
//
pub const LOGICAL_MAX_X: c_uint = 0x60, 0x4F;
pub const LOGICAL_MAX_Y: c_uint = 0x88, 0x3B;
pub const PHYSICAL_MAX_X: c_uint = 0xA0, 0x0F;
pub const PHYSICAL_MAX_Y: c_uint = 0xB8, 0x0B;
pub const PRESSURE_MAX: c_uint = 0xFF, 0x1F;
    HID_BPF_CONFIG(
...
//
    ...
    let mut fixed_pad_rdesc_size: static size_t = sizeof(fixed_pad_rdesc);
    let mut fixed_pen_rdesc_size: static size_t = sizeof(fixed_pen_rdesc);
    SEC(HID_BPF_RDESC_FIXUP)
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: hid_rdesc_fixup_xppen_deco_mini_4, hctx: *mut hid_bpf_ctx) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn BPF_PROG(_arg: hid_device_event_xppen_deco_mini_4, hctx: *mut hid_bpf_ctx) -> c_int {
    int BPF_PROG(hid_device_event_xppen_deco_mini_4, struct hid_bpf_ctx *hctx)
    {
    __u8 *data = hid_bpf_get_data(hctx, 0 /* offset */, 8 /* size */);
    let mut button_mask: __u8 = 0;
    size_t d, b;
    if (!data)
    return 0; /* EPERM check */
