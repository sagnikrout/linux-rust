...
//! Source: drivers/hid/hid-lenovo-go-s.c
#![no_std]
...
pub const GO_S_CFG_INTF_IN: c_uint = 0x84;
pub const GO_S_PACKET_SIZE: c_int = 64;
...
#[no_mangle]
unsafe extern "C" fn hid_gos_version_event(data: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_mcu_id_event(cmd_rep: *mut command_report) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_gamepad_cfg_event(cmd_rep: *mut command_report) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_touchpad_event(cmd_rep: *mut command_report) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_pl_test_event(cmd_rep: *mut command_report) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_light_event(cmd_rep: *mut command_report) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_set_event_return(cmd_rep: *mut command_report) -> c_int {
...
// PL_TEST commands can take longer because they go out to another device
    ...
    u8 i;
    count = mcu_property_out(drvdata.hdev, GET_GAMEPAD_CFG, index, core::ptr::null_mut(), 0);
    if (count < 0)
    return count;
    switch (index) {
    case FEATURE_GAMEPAD_MODE:
    ...
    let mut ret: c_int = 0;
    u8 i;
    ret = mcu_property_out(drvdata.hdev, GET_TP_PARAM, index, core::ptr::null_mut(), 0);
    if (ret < 0)
    return ret;
    switch (index) {
    case CFG_WINDOWS_MODE:
...
// IMU
    let mut imu_bypass_enabled: static struct gos_cfg_attr = { FEATURE_IMU_BYPASS };
    ...
    let mut imu_manufacturer: static struct gos_cfg_attr = { TEST_IMU_MFR };
    LEGOS_DEVICE_ATTR_RO(imu_manufacturer, "manufacturer", test);
    let mut imu_sensor_enabled: static struct gos_cfg_attr = { FEATURE_IMU_ENABLE };
...
// MCU
    ...
    let mut os_mode: static struct gos_cfg_attr = { FEATURE_OS_MODE };
...
// Mouse
    let mut mouse_wheel_step: static struct gos_cfg_attr = { FEATURE_MOUSE_WHEEL_STEP };
...
// Touchpad
    let mut touchpad_enabled: static struct gos_cfg_attr = { FEATURE_TOUCHPAD_ENABLE };
    ...
    let mut touchpad_linux_mode: static struct gos_cfg_attr = { CFG_LINUX_MODE };
    ...
    let mut touchpad_manufacturer: static struct gos_cfg_attr = { TEST_TP_MFR };
    LEGOS_DEVICE_ATTR_RO(touchpad_manufacturer, "manufacturer", test);
    let mut touchpad_version: static struct gos_cfg_attr = { TEST_TP_VER };
    LEGOS_DEVICE_ATTR_RO(touchpad_version, "version", test);
    let mut touchpad_windows_mode: static struct gos_cfg_attr = { CFG_WINDOWS_MODE };
...
// RGB
    let mut rgb_enabled: static struct gos_cfg_attr = { FEATURE_RGB_ENABLE };
...
unsafe extern "C" fn cfg_setup(work: *mut work_struct) {
    ...
    int ret;
// MCU
    ret = mcu_property_out(drvdata.hdev, GET_MCU_ID, FEATURE_NONE, core::ptr::null_mut(), 0);
    if (ret) {
    dev_err(&drvdata.hdev.dev, "Failed to retrieve MCU ID: %i\n",
    ret);
    return;
    }
    ret = mcu_property_out(drvdata.hdev, GET_VERSION, FEATURE_NONE, core::ptr::null_mut(), 0);
    if (ret) {
    dev_err(&drvdata.hdev.dev, "Failed to retrieve MCU Version: %i\n", ret);
    return;
    }
    ret = mcu_property_out(drvdata.hdev, GET_PL_TEST, TEST_TP_MFR, core::ptr::null_mut(), 0);
    if (ret) {
    dev_err(&drvdata.hdev.dev,
    "Failed to retrieve Touchpad Manufacturer: %i\n", ret);
    return;
    }
    ret = mcu_property_out(drvdata.hdev, GET_PL_TEST, TEST_TP_VER, core::ptr::null_mut(), 0);
    if (ret) {
    dev_err(&drvdata.hdev.dev,
    "Failed to retrieve Touchpad Firmware Version: %i\n", ret);
    return;
    }
    ret = mcu_property_out(drvdata.hdev, GET_PL_TEST, TEST_IMU_MFR, core::ptr::null_mut(), 0);
    if (ret) {
    dev_err(&drvdata.hdev.dev,
...
#[no_mangle]
unsafe extern "C" fn hid_gos_cfg_remove(hdev: *mut hid_device) {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_cfg_reset_resume(hdev: *mut hid_device) -> c_int {
    static int hid_gos_cfg_reset_resume(struct hid_device *hdev)
    {
    let mut os_mode: u8 = drvdata.os_mode;
    int ret;
    ...
    FEATURE_OS_MODE, core::ptr::null_mut(), 0);
    if (ret < 0)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn hid_gos_remove(hdev: *mut hid_device) {
...
#[no_mangle]
unsafe extern "C" fn hid_gos_reset_resume(hdev: *mut hid_device) -> c_int {
