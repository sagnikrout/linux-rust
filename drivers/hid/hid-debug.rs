...
//! Source: drivers/hid/hid-debug.c
#![no_std]
...
//
    ...
    char *buf = core::ptr::null_mut();
    let mut len: c_int = 0;
    const char *modifier = core::ptr::null_mut();
    let mut usage_modifier: c_uint = usage & 0xF000;
    let mut usage_actual: c_uint = usage & 0xFFFF;
...
#[no_mangle]
unsafe extern "C" fn tab(n: c_int, f: *mut seq_file) {
    seq_printf(f, "%*s", n, "");
    }
#[no_mangle]
pub unsafe extern "C" fn hid_dump_field(field: *mut hid_field, n: c_int, f: *mut seq_file) {
    int j;
    if (field.physical) {
    tab(n, f);
...
// First nibble tells us which system we're in.
    ...
    let mut earlier_unit: c_int = 0;
    tab(n, f); seq_printf(f, "Unit(%s : ", systems[sys]);
    for (i=1 ; i<sizeof(__u32)*2 ; i++) {
    let mut nibble: c_char = data & 0xf;
    data >>= 4;
    if (nibble != 0) {
    if(earlier_unit++ > 0)
    seq_printf(f, "*");
...
#[no_mangle]
pub unsafe extern "C" fn hid_dump_device(device: *mut hid_device, f: *mut seq_file) {
...
#[no_mangle]
pub unsafe extern "C" fn hid_debug_event(hdev: *mut hid_device, buf: *mut c_char) {
...
#[no_mangle]
pub unsafe extern "C" fn hid_dump_input(hdev: *mut hid_device, usage: *mut hid_usage, value: __s32) {
    void hid_dump_input(struct hid_device *hdev, struct hid_usage *usage, __s32 value)
    {
    char *buf;
    int len;
    buf = hid_resolv_usage(usage.hid, core::ptr::null_mut());
    if (!buf)
    return;
    len = strlen(buf);
...
#[no_mangle]
unsafe extern "C" fn hid_resolv_event(type: __u8, code: __u16, f: *mut seq_file) {
...
#[no_mangle]
unsafe extern "C" fn hid_dump_input_mapping(hid: *mut hid_device, f: *mut seq_file) {
...
#[no_mangle]
unsafe extern "C" fn hid_debug_rdesc_show(f: *mut seq_file, p: *mut c_void) -> c_int {
    static int hid_debug_rdesc_show(struct seq_file *f, void *p)
    {
    struct hid_device *hdev = f.private;
    const __u8 *rdesc = hdev.rdesc;
    let mut rsize: unsigned = hdev.rsize;
    int i;
    if (!rdesc) {
...
#[no_mangle]
unsafe extern "C" fn hid_debug_events_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hid_debug_events_poll(file: *mut file, wait: *mut poll_table) -> __poll_t {
...
#[no_mangle]
unsafe extern "C" fn hid_debug_events_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hid_debug_register(hdev: *mut hid_device, name: *const c_char) {
...
#[no_mangle]
pub unsafe extern "C" fn hid_debug_unregister(hdev: *mut hid_device) {
...
#[no_mangle]
pub unsafe extern "C" fn hid_debug_init() {
    void hid_debug_init(void)
    {
    hid_debug_root = debugfs_create_dir("hid", core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn hid_debug_exit() {
