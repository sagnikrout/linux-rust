...
pub const OXP_PACKET_SIZE: c_int = 64;
pub const GEN1_MESSAGE_ID: c_uint = 0xff;
pub const GEN2_MESSAGE_ID: c_uint = 0x3f;
pub const GEN1_USAGE_PAGE: c_uint = 0xff01;
pub const GEN2_USAGE_PAGE: c_uint = 0xff00;
...
pub const OXP_MAPPING_GAMEPAD: c_uint = 0x01;
pub const OXP_MAPPING_KEYBOARD: c_uint = 0x02;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct oxp_button_data {
    pub mode: u8,
    pub index: u8,
    pub key_id: u8,
    pub padding: [u8; 2],
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct oxp_button_entry {
    pub data: oxp_button_data,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct oxp_button_idx {
    pub button_idx: enum oxp_joybutton_index,
    pub mapping_idx: u8,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct oxp_bmap_page_1 {
    pub btn_a: oxp_button_idx,
    pub btn_b: oxp_button_idx,
    pub btn_x: oxp_button_idx,
    pub btn_y: oxp_button_idx,
    pub btn_lb: oxp_button_idx,
    pub btn_rb: oxp_button_idx,
    pub btn_lt: oxp_button_idx,
    pub btn_rt: oxp_button_idx,
    pub btn_start: oxp_button_idx,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct oxp_bmap_page_2 {
    pub btn_select: oxp_button_idx,
    pub btn_l3: oxp_button_idx,
    pub btn_r3: oxp_button_idx,
    pub btn_dup: oxp_button_idx,
    pub btn_ddown: oxp_button_idx,
    pub btn_dleft: oxp_button_idx,
    pub btn_dright: oxp_button_idx,
    pub btn_m1: oxp_button_idx,
    pub btn_m2: oxp_button_idx,
    pub __packed: },
    static struct oxp_hid_cfg {
    pub oxp_rgb_queue: delayed_work,
    pub oxp_btn_queue: delayed_work,
    pub bmap_1: *mut oxp_bmap_page_1,
    pub bmap_2: *mut oxp_bmap_page_2,
    pub oxp_mcu_init: delayed_work,
    pub led_mc: *mut led_classdev_mc,
    pub hdev: *mut hid_device,
    pub report*/: *mut *mut mutex cfg_mutex; /ensure single synchronous output,
...
// rgb_effect_text
//
pub const OXP_GET_PROPERTY: c_uint = 0xfc;
pub const OXP_SET_PROPERTY: c_uint = 0xfd;
pub const OXP_EFFECT_MONO_TRUE: c_uint = 0xfe /* actual index for monocolor */;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct oxp_gen_1_rgb_report {
    pub report_id: u8,
    pub message_id: u8,
    pub padding_2: [u8; 2],
    pub effect: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct oxp_gen_2_rgb_report {
    pub report_id: u8,
    pub header_id: u8,
    pub padding_2: u8,
    pub message_id: u8,
...
#[no_mangle]
unsafe extern "C" fn get_usage_page(hdev: *mut hid_device) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn oxp_mcu_init_fn(work: *mut work_struct) {
...
//
    ...
    u8 *data, int size)
    {
    let mut up: u16 = get_usage_page(hdev);
...
// MCU takes 200ms to be ready for another command.
    msleep(200);
    let mut ret: return = = OXP_PACKET_SIZE ? 0 : -EIO;
    }
    ...
    return mcu_property_out(header, header_size, data, data_size, core::ptr::null_mut(), 0);
    }
    ...
    let mut header_size: usize = ARRAY_SIZE(header);
    let mut footer_size: usize = ARRAY_SIZE(footer);
    ...
    size_t count)
    {
    let mut up: u16 = get_usage_page(drvdata.hdev);
...
#[no_mangle]
unsafe extern "C" fn oxp_set_defaults_bmap_1(bmap: *mut oxp_bmap_page_1) {
...
#[no_mangle]
unsafe extern "C" fn oxp_set_defaults_bmap_2(bmap: *mut oxp_bmap_page_2) {
    ...
    size_t len)
    {
    let mut offset_increment: usize = sizeof(u8) + sizeof(struct oxp_button_idx);
...
#[no_mangle]
unsafe extern "C" fn oxp_set_buttons() -> c_int {
    static int oxp_set_buttons(void)
    {
...
#[no_mangle]
unsafe extern "C" fn oxp_reset_buttons() {
...
#[no_mangle]
unsafe extern "C" fn oxp_btn_queue_fn(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn oxp_button_idx_from_str(buf: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn oxp_rumble_intensity_set(intensity: u8) -> c_int {
    static int oxp_rumble_intensity_set(u8 intensity)
    {
    ...
    let mut header_size: usize = ARRAY_SIZE(header);
    u8 data[59] = { 0x0 };
    let mut data_size: usize = ARRAY_SIZE(data);
...
pub unsafe extern "C" fn DEVICE_ATTR_RW(_arg: _name) -> static {
    ...
    let mut button_a: static struct oxp_attr = { BUTTON_A };
    OXP_DEVICE_ATTR_RW(button_a, map_button);
    let mut button_b: static struct oxp_attr = { BUTTON_B };
    OXP_DEVICE_ATTR_RW(button_b, map_button);
    let mut button_x: static struct oxp_attr = { BUTTON_X };
    OXP_DEVICE_ATTR_RW(button_x, map_button);
    let mut button_y: static struct oxp_attr = { BUTTON_Y };
    OXP_DEVICE_ATTR_RW(button_y, map_button);
    let mut button_lb: static struct oxp_attr = { BUTTON_LB };
    OXP_DEVICE_ATTR_RW(button_lb, map_button);
    let mut button_rb: static struct oxp_attr = { BUTTON_RB };
    OXP_DEVICE_ATTR_RW(button_rb, map_button);
    let mut button_lt: static struct oxp_attr = { BUTTON_LT };
    OXP_DEVICE_ATTR_RW(button_lt, map_button);
    let mut button_rt: static struct oxp_attr = { BUTTON_RT };
    OXP_DEVICE_ATTR_RW(button_rt, map_button);
    let mut button_start: static struct oxp_attr = { BUTTON_START };
    OXP_DEVICE_ATTR_RW(button_start, map_button);
    let mut button_select: static struct oxp_attr = { BUTTON_SELECT };
    OXP_DEVICE_ATTR_RW(button_select, map_button);
    let mut button_l3: static struct oxp_attr = { BUTTON_L3 };
    OXP_DEVICE_ATTR_RW(button_l3, map_button);
    let mut button_r3: static struct oxp_attr = { BUTTON_R3 };
    OXP_DEVICE_ATTR_RW(button_r3, map_button);
    let mut button_d_up: static struct oxp_attr = { BUTTON_DUP };
    OXP_DEVICE_ATTR_RW(button_d_up, map_button);
    let mut button_d_down: static struct oxp_attr = { BUTTON_DDOWN };
    OXP_DEVICE_ATTR_RW(button_d_down, map_button);
    let mut button_d_left: static struct oxp_attr = { BUTTON_DLEFT };
    OXP_DEVICE_ATTR_RW(button_d_left, map_button);
    let mut button_d_right: static struct oxp_attr = { BUTTON_DRIGHT };
    OXP_DEVICE_ATTR_RW(button_d_right, map_button);
    let mut button_m1: static struct oxp_attr = { BUTTON_M1 };
    OXP_DEVICE_ATTR_RW(button_m1, map_button);
    let mut button_m2: static struct oxp_attr = { BUTTON_M2 };
...
#[no_mangle]
unsafe extern "C" fn oxp_rgb_status_store(enabled: u8, speed: u8, brightness: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn oxp_rgb_status_show() -> isize {
...
#[no_mangle]
unsafe extern "C" fn oxp_rgb_color_set() -> c_int {
    static int oxp_rgb_color_set(void)
    {
    let mut max_br: u8 = drvdata.led_mc.led_cdev.max_brightness;
    let mut br: u8 = drvdata.led_mc.led_cdev.brightness;
    let mut up: u16 = get_usage_page(drvdata.hdev);
    u8 green, red, blue;
    size_t size;
    u8 *data;
...
#[no_mangle]
unsafe extern "C" fn oxp_rgb_effect_set(effect: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn oxp_rgb_queue_fn(work: *mut work_struct) {
    static void oxp_rgb_queue_fn(struct work_struct *work)
    {
    let mut max_brightness: c_uint = drvdata.led_mc.led_cdev.max_brightness;
    let mut brightness: c_uint = drvdata.led_mc.led_cdev.brightness;
    let mut val: u8 = 4 * brightness / max_brightness;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn oxp_hybrid_mcu_device() -> bool {
...
#[no_mangle]
unsafe extern "C" fn oxp_cfg_probe(hdev: *mut hid_device, up: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn oxp_hid_remove(hdev: *mut hid_device) {
