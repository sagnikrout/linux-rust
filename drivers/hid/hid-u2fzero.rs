...
//! Source: drivers/hid/hid-u2fzero.c
#![no_std]
...
pub const HID_REPORT_SIZE: c_int = 64;
    enum hw_revision {
    HW_U2FZERO,
    HW_NITROKEY_U2F,
    };
...
pub const CID_BROADCAST: c_uint = 0xffffffff;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct u2f_hid_msg {
    pub cid: u32,
    union {
    struct {
    pub cmd: u8,
    pub bcnth: u8,
    pub bcntl: u8,
    pub 7]: u8 data[HID_REPORT_SIZE -,
    pub init: },
    struct {
    pub seq: u8,
    pub 5]: u8 data[HID_REPORT_SIZE -,
    pub cont: },
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct u2fzero_device {
    pub hdev: *mut hid_device,
    pub /: *mut *mut *mut urb urb; / URB for the RNG data,
    pub /: *mut *mut led_classdev ldev; / Embedded for led,
    pub /: *mut *mut hwrng hwrng; / Embedded for hwrng,
    pub led_name: *mut c_char,
    pub rng_name: *mut c_char,
    pub buf_out: *mut u8,
    pub buf_in: *mut u8,
    pub lock: mutex,
    pub present: bool,
    pub hw_revision: kernel_ulong_t,
}

#[no_mangle]
unsafe extern "C" fn u2fzero_send(dev: *mut u2fzero_device, req: *mut u2f_hid_report) -> c_int {
    ...
    if (ret < 0)
    return ret;
    let mut ret: return = = sizeof(struct u2f_hid_msg) ? 0 : -EMSGSIZE;
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct u2fzero_transfer_context {
    pub done: completion,
    pub status: c_int,
}

#[no_mangle]
unsafe extern "C" fn u2fzero_read_callback(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn u2fzero_blink(ldev: *mut led_classdev) -> c_int {
...
// valid packets must have a correct header
    let mut min_length: c_int = offsetof(struct u2f_hid_msg, init.data);
    if (!dev.present) {
...
// only take the minimum amount of data it is safe to take
    ...
    if (dev.led_name == core::ptr::null_mut())
    return -ENOMEM;
    ...
    if (dev.rng_name == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn u2fzero_fill_in_urb(dev: *mut u2fzero_device) -> c_int {
    ...
    if (dev.buf_out == core::ptr::null_mut())
    return -ENOMEM;
    dev.buf_in = devm_kmalloc(&hdev.dev,
    sizeof(struct u2f_hid_msg), GFP_KERNEL);
    if (dev.buf_in == core::ptr::null_mut())
    return -ENOMEM;
    ret = hid_parse(hdev);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn u2fzero_remove(hdev: *mut hid_device) {
