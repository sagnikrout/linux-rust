...
//! Source: drivers/hid/hid-uclogic-core.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn uclogic_inrange_timeout(t: *mut timer_list) {
...
// Digitizer Tip Switch usage
    ...
    if (drvdata.desc_ptr != core::ptr::null_mut()) {
...
//
    let mut key: c_int = (usage.hid & HID_USAGE) - 1;
...
//
    ...
    {
    int rc;
    struct uclogic_drvdata *drvdata = core::ptr::null_mut();
    let mut params_initialized: bool = false;
    if (!hid_is_usb(hdev))
    return -EINVAL;
//
...
#[no_mangle]
unsafe extern "C" fn uclogic_resume(hdev: *mut hid_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn uclogic_exec_event_hook(p: *mut uclogic_params, event: *mut u8, size: c_int) -> bool {
...
//
    ...
    WARN_ON(drvdata == core::ptr::null_mut());
    WARN_ON(data == core::ptr::null_mut() && size != 0);
...
// we move these to the end of the x coord (u16) to create a correct x coord (u32)
    let mut lsb_low_byte: u8 = data[10];
    let mut lsb_high_byte: u8 = data[11];
...
// If need to, and can, read rotary encoder state change
    ...
    let mut byte: c_uint = frame.re_lsb / 8;
    let mut bit: c_uint = frame.re_lsb % 8;
    u8 change;
    let mut prev_state: u8 = drvdata.re_state;
// Read Gray-coded state
    let mut state: u8 = (data[byte] >> bit) & 0x3;
...
// If need to, and can, transform the touch ring reports
    if (frame.touch_byte > 0 && frame.touch_byte < size) {
    let mut value: __s8 = data[frame.touch_byte];
    if (value != 0) {
...
// Everything below here is for tablets that shove multiple dials into 1 byte
    ...
    u8 *data, int size)
    {
    let mut report_id: c_uint = report.id;
    struct uclogic_drvdata *drvdata = hid_get_drvdata(hdev);
    struct uclogic_params *params = &drvdata.params;
...
#[no_mangle]
unsafe extern "C" fn uclogic_remove(hdev: *mut hid_device) {
