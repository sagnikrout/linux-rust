...
//! Source: drivers/hid/hid-samsung.c
#![no_std]
...
//
    ...
    let mut ifnum: c_ushort = intf.cur_altsetting.desc.bInterfaceNumber;
...
#[no_mangle]
pub unsafe extern "C" fn if(USB_DEVICE_ID_SAMSUNG_WIRELESS_KBD: hdev->product ==) -> else {
    else if (hdev.product == USB_DEVICE_ID_SAMSUNG_WIRELESS_KBD)
    ret = samsung_kbd_input_mapping(hdev,
    hi, field, usage, bit, max);
#[no_mangle]
pub unsafe extern "C" fn if(USB_DEVICE_ID_SAMSUNG_WIRELESS_GAMEPAD: hdev->product ==) -> else {
    else if (hdev.product == USB_DEVICE_ID_SAMSUNG_WIRELESS_GAMEPAD)
    ret = samsung_gamepad_input_mapping(hdev,
    hi, field, usage, bit, max);
#[no_mangle]
pub unsafe extern "C" fn if(USB_DEVICE_ID_SAMSUNG_WIRELESS_ACTIONMOUSE: hdev->product ==) -> else {
    else if (hdev.product == USB_DEVICE_ID_SAMSUNG_WIRELESS_ACTIONMOUSE)
    ret = samsung_actionmouse_input_mapping(hdev,
    hi, field, usage, bit, max);
#[no_mangle]
pub unsafe extern "C" fn if(USB_DEVICE_ID_SAMSUNG_WIRELESS_UNIVERSAL_KBD: hdev->product ==) -> else {
    else if (hdev.product == USB_DEVICE_ID_SAMSUNG_WIRELESS_UNIVERSAL_KBD)
    ret = samsung_universal_kbd_input_mapping(hdev,
    hi, field, usage, bit, max);
#[no_mangle]
pub unsafe extern "C" fn if(USB_DEVICE_ID_SAMSUNG_WIRELESS_MULTI_HOGP_KBD: hdev->product ==) -> else {
    ...
    {
    int ret;
    let mut cmask: c_uint = HID_CONNECT_DEFAULT;
    ret = hid_parse(hdev);
    if (ret) {
