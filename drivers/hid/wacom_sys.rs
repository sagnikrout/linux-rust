...
//! Source: drivers/hid/wacom_sys.c
#![no_std]
...
pub const WAC_MSG_RETRIES: c_int = 5;
pub const WAC_CMD_RETRIES: c_int = 10;
    ...
    let mut size: c_int = kfifo_peek_len(fifo);
...
// Queue events which have invalid tool type or serial number
    ...
    let mut equivalent_usage: c_uint = wacom_equivalent_usage(usage.hid);
    unsigned int offset;
    unsigned int size;
    unsigned int value;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: insert) -> else {
    else if (insert)
...
#[no_mangle]
unsafe extern "C" fn wacom_open(dev: *mut input_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wacom_close(dev: *mut input_dev) {
...
//
    ...
    let mut i: c_int = usage.usage_index;
...
//
    ...
    struct wacom *wacom = hid_get_drvdata(hdev);
    struct wacom_features *features = &wacom.wacom_wac.features;
    let mut finger: bool = WACOM_FINGER_FIELD(field);
    let mut pen: bool = WACOM_PEN_FIELD(field);
    let mut equivalent_usage: unsigned = wacom_equivalent_usage(usage.hid);
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: finger) -> else {
    else if (finger)
...
#[no_mangle]
unsafe extern "C" fn wacom_hid_set_device_mode(hdev: *mut hid_device) -> c_int {
    ...
    struct hid_report_enum *re;
    u32 length;
    let mut error: c_int = -ENOMEM, limit = 0;
...
//
#[no_mangle]
unsafe extern "C" fn _wacom_query_tablet_data(wacom: *mut wacom) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wacom_hdev_data {
    pub list: list_head,
    pub kref: kref,
    pub dev: *mut hid_device,
    pub shared: wacom_shared,
}
    ...
    let mut oVid: __u32 = features.oVid ? features.oVid : hdev.vendor;
    let mut oPid: __u32 = features.oPid ? features.oPid : hdev.product;
...
#[no_mangle]
unsafe extern "C" fn wacom_release_shared_data(kref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn wacom_remove_shared_data(res: *mut c_void) {
    ...
    wacom_wac.shared.touch = core::ptr::null_mut();
#[no_mangle]
pub unsafe extern "C" fn if(wacom->hdev: wacom_wac->shared->pen ==) -> else {
    else if (wacom_wac.shared.pen == wacom.hdev)
    wacom_wac.shared.pen = core::ptr::null_mut();
    kref_put(&data.kref, wacom_release_shared_data);
    wacom_wac.shared = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn wacom_add_shared_data(hdev: *mut hid_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(WACOM_DEVICETYPE_PEN: wacom_wac->features.device_type &) -> else {
    else if (wacom_wac.features.device_type & WACOM_DEVICETYPE_PEN)
    wacom_wac.shared.pen = hdev;
    return retval;
    }
#[no_mangle]
unsafe extern "C" fn wacom_led_control(wacom: *mut wacom) -> c_int {
    static int wacom_led_control(struct wacom *wacom)
    {
    unsigned char *buf;
    int retval;
    let mut report_id: c_uchar = WAC_CMD_LED_CONTROL;
    let mut buf_size: c_int = 9;
    if (!wacom.led.groups)
    return -ENOTSUPP;
...
#[no_mangle]
pub unsafe extern "C" fn if(INTUOSP2_BT: wacom->wacom_wac.features.type ==) -> else {
...
//
    let mut ring_led: c_int = wacom.led.groups[0].select & 0x03;
    let mut ring_lum: c_int = (((wacom.led.llv & 0x60) >> 5) - 1) & 0x03;
    let mut crop_lum: c_int = 0;
    let mut led_bits: c_uchar = (crop_lum << 4) | (ring_lum << 2) | (ring_led);
    buf[0] = report_id;
...
pub unsafe extern "C" fn if(INTUOSP2_BT: wacom->wacom_wac.features.type ==) -> else {
    ...
    let mut led: c_int = wacom.led.groups[0].select | 0x4;
...
#[no_mangle]
pub unsafe extern "C" fn if(&wacom->led.hlv: dest ==) -> else {
    ...
    core::ptr::null_mut(), wacom_btnimg##BUTTON_ID##_store)
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wacom_sysfs_group_devres {
    pub group: *const attribute_group,
    pub root: *mut kobject,
}

#[no_mangle]
unsafe extern "C" fn wacom_devm_sysfs_group_release(dev: *mut device, res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn wacom_devm_kfifo_release(dev: *mut device, res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn wacom_devm_kfifo_alloc(wacom: *mut wacom) -> c_int {
    static int wacom_devm_kfifo_alloc(struct wacom *wacom)
    {
    struct wacom_wac *wacom_wac = &wacom.wacom_wac;
    let mut fifo_size: c_int = min(PAGE_SIZE, 10 * wacom_wac.features.pktlen);
...
#[no_mangle]
pub unsafe extern "C" fn wacom_leds_brightness_get(led: *mut wacom_led) -> enum led_brightness {
...
#[no_mangle]
unsafe extern "C" fn __wacom_led_brightness_get(cdev: *mut led_classdev) -> enum led_brightness {
    ...
    led.cdev.name, error);
    led.cdev.name = core::ptr::null_mut();
    return error;
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn wacom_led_groups_release_one(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn wacom_led_groups_release(data: *mut c_void) {
    static void wacom_led_groups_release(void *data)
    {
    struct wacom *wacom = data;
    wacom.led.groups = core::ptr::null_mut();
    wacom.led.count = 0;
    }
#[no_mangle]
unsafe extern "C" fn wacom_led_groups_allocate(wacom: *mut wacom, count: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wacom_initialize_leds(wacom: *mut wacom) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wacom_init_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn wacom_query_tablet_data(wacom: *mut wacom) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: battery->bat_charging) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: battery->ps_connected) -> else {
    ...
    struct device *dev = &wacom.hdev.dev;
    let mut psy_cfg: power_supply_config = { .drv_data = battery, };
...
#[no_mangle]
unsafe extern "C" fn wacom_initialize_battery(wacom: *mut wacom) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wacom_destroy_battery(wacom: *mut wacom) {
    static void wacom_destroy_battery(struct wacom *wacom)
    {
    if (wacom.battery.battery) {
    devres_release_group(&wacom.hdev.dev,
    &wacom.battery.bat_desc);
    wacom.battery.battery = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn wacom_aes_battery_handler(work: *mut work_struct) {
...
// attr, char *buf)
    ...
    if (error) {
    remote.remotes[index].group.name = core::ptr::null_mut();
    hid_err(wacom.hdev,
    "cannot create sysfs group err: %d\n", error);
    return error;
    }
...
#[no_mangle]
unsafe extern "C" fn wacom_cmd_unpair_remote(wacom: *mut wacom, selector: c_uchar) -> c_int {
    static int wacom_cmd_unpair_remote(struct wacom *wacom, unsigned char selector)
    {
    let mut buf_size: usize = 2;
    unsigned char *buf;
    int retval;
    buf = kzalloc(buf_size, GFP_KERNEL);
    if (!buf)
...
#[no_mangle]
unsafe extern "C" fn wacom_remotes_destroy(data: *mut c_void) {
    ...
    wacom.remote = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn wacom_initialize_remotes(wacom: *mut wacom) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wacom_allocate_inputs(wacom: *mut wacom) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wacom_setup_inputs(wacom: *mut wacom) -> c_int {
...
// no pen in use on this interface
    ...
    wacom_wac.pen_input = core::ptr::null_mut();
    pen_input_dev = core::ptr::null_mut();
    }
    error = wacom_setup_touch_input_capabilities(touch_input_dev, wacom_wac);
    if (error) {
// no touch in use on this interface
    input_free_device(touch_input_dev);
    wacom_wac.touch_input = core::ptr::null_mut();
    touch_input_dev = core::ptr::null_mut();
    }
    error = wacom_setup_pad_input_capabilities(pad_input_dev, wacom_wac);
    if (error) {
// no pad events using this interface
    input_free_device(pad_input_dev);
    wacom_wac.pad_input = core::ptr::null_mut();
    pad_input_dev = core::ptr::null_mut();
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn wacom_register_inputs(wacom: *mut wacom) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn wacom_set_default_phy(features: *mut wacom_features) {
...
#[no_mangle]
unsafe extern "C" fn wacom_calculate_res(features: *mut wacom_features) {
...
#[no_mangle]
pub unsafe extern "C" fn wacom_battery_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn wacom_compute_pktlen(hdev: *mut hid_device) -> usize {
    ...
    let mut report_size: usize = hid_report_len(report);
    if (report_size > size)
    size = report_size;
    }
    return size;
...
#[no_mangle]
unsafe extern "C" fn wacom_update_name(wacom: *mut wacom, suffix: *const c_char) {
...
// Generic devices name unspecified
    ...
    if (dev.product != core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn wacom_release_resources(wacom: *mut wacom) {
    ...
    wacom.wacom_wac.pen_input = core::ptr::null_mut();
    wacom.wacom_wac.touch_input = core::ptr::null_mut();
    wacom.wacom_wac.pad_input = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn wacom_set_shared_values(wacom_wac: *mut wacom_wac) {
...
#[no_mangle]
unsafe extern "C" fn wacom_parse_and_register(wacom: *mut wacom, wireless: bool) -> c_int {
    ...
    struct hid_device *hdev = wacom.hdev;
    int error;
    let mut connect_mask: c_uint = HID_CONNECT_HIDRAW;
...
#[no_mangle]
unsafe extern "C" fn wacom_wireless_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn wacom_remote_destroy_battery(wacom: *mut wacom, index: c_int) {
    ...
    remote.remotes[index].battery.battery = core::ptr::null_mut();
    remote.remotes[index].active_time = 0;
    }
    }
#[no_mangle]
unsafe extern "C" fn wacom_remote_destroy_one(wacom: *mut wacom, index: c_uint) {
    static void wacom_remote_destroy_one(struct wacom *wacom, unsigned int index)
    {
    struct wacom_remote *remote = wacom.remote;
    let mut serial: u32 = remote.remotes[index].serial;
    int i;
    unsigned long flags;
    for (i = 0; i < WACOM_MAX_REMOTES; i++) {
    ...
    &remote.remotes[i]);
    remote.remotes[i].serial = 0;
    remote.remotes[i].group.name = core::ptr::null_mut();
    wacom.led.groups[i].select = WACOM_STATUS_UNKNOWN;
    }
    }
    }
...
#[no_mangle]
unsafe extern "C" fn wacom_remote_attach_battery(wacom: *mut wacom, index: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wacom_remote_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn wacom_mode_change_work(work: *mut work_struct) {
    ...
    struct wacom *wacom1 = core::ptr::null_mut();
    struct wacom *wacom2 = core::ptr::null_mut();
    let mut is_direct: bool = wacom.wacom_wac.is_direct_mode;
    let mut error: c_int = 0;
    if (shared.pen) {
...
#[no_mangle]
unsafe extern "C" fn wacom_remove(hdev: *mut hid_device) {
...
#[no_mangle]
unsafe extern "C" fn wacom_resume(hdev: *mut hid_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wacom_reset_resume(hdev: *mut hid_device) -> c_int {
