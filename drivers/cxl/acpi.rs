...
#[no_mangle]
pub unsafe extern "C" fn cxl_do_xormap_calc(cximsd: *mut cxl_cxims_data, addr: u64, hbiw: c_int) -> u64 {
    u64 cxl_do_xormap_calc(struct cxl_cxims_data *cximsd, u64 addr, int hbiw)
    {
    let mut nr_maps_to_apply: c_int = -1;
    u64 val;
    int pos;
//
...
#[no_mangle]
unsafe extern "C" fn cxl_apply_xor_maps(cxlrd: *mut cxl_root_decoder, addr: u64) -> u64 {
    static u64 cxl_apply_xor_maps(struct cxl_root_decoder *cxlrd, u64 addr)
    {
    let mut hbiw: c_int = cxlrd.cxlsd.nr_targets;
...
#[no_mangle]
unsafe extern "C" fn cfmws_to_decoder_flags(restrictions: c_int) -> c_ulong {
    static unsigned long cfmws_to_decoder_flags(int restrictions)
    {
    let mut flags: c_ulong = CXL_DECODER_F_ENABLE;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxl_cfmws_context {
    pub dev: *mut device,
    pub root_port: *mut cxl_port,
    pub cxl_res: *mut resource,
    pub id: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn del_cxl_resource(res: *mut resource) {
...
#[no_mangle]
unsafe extern "C" fn add_or_reset_cxl_resource(parent: *mut resource, res: *mut resource) -> c_int {
    static int add_or_reset_cxl_resource(struct resource *parent, struct resource *res)
    {
    let mut rc: c_int = insert_resource(parent, res);
    if (rc)
    del_cxl_resource(res);
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn cxl_setup_extended_linear_cache(cxlrd: *mut cxl_root_decoder) {
    static void cxl_setup_extended_linear_cache(struct cxl_root_decoder *cxlrd)
    {
    struct cxl_decoder *cxld = &cxlrd.cxlsd.cxld;
    struct range *hpa = &cxld.hpa_range;
    let mut size: resource_size_t = range_len(hpa);
    let mut start: resource_size_t = hpa.start;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxl_chbs_context {
    pub dev: *mut device,
    pub uid: c_ulonglong,
    pub base: resource_size_t,
    pub cxl_version: u32,
    pub nr_versions: c_int,
...
//
    ...
    int rc;
    rc = acpi_evaluate_integer(hb.handle, METHOD_NAME__UID, core::ptr::null_mut(), &uid);
    if (rc != AE_OK) {
...
#[no_mangle]
unsafe extern "C" fn get_genport_coordinates(dev: *mut device, dport: *mut cxl_dport) -> c_int {
    static int get_genport_coordinates(struct device *dev, struct cxl_dport *dport)
    {
    struct acpi_device *hb = to_cxl_host_bridge(core::ptr::null_mut(), dev);
    u32 uid;
...
#[no_mangle]
unsafe extern "C" fn add_host_bridge_dport(match: *mut device, arg: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn add_host_bridge_uport(match: *mut device, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn add_root_nvdimm_bridge(match: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_acpi_lock_reset_class(dev: *mut c_void) {
    static void cxl_acpi_lock_reset_class(void *dev)
    {
    device_lock_reset_class(dev);
    }
#[no_mangle]
unsafe extern "C" fn cxl_set_public_resource(priv: *mut resource, pub: *mut resource) {
...
#[no_mangle]
unsafe extern "C" fn remove_cxl_resources(data: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn add_cxl_resources(cxl_res: *mut resource) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pair_cxl_resource(dev: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_acpi_probe(pdev: *mut platform_device) -> c_int {
    ...
    rc = bus_for_each_dev(adev.dev.bus, core::ptr::null_mut(), root_port,
...
#[no_mangle]
unsafe extern "C" fn cxl_acpi_init() -> int __init {
    static int __init cxl_acpi_init(void)
    {
    return platform_driver_register(&cxl_acpi_driver);
    }
#[no_mangle]
unsafe extern "C" fn cxl_acpi_exit() -> void __exit {
