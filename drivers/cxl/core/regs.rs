...
//! Source: drivers/cxl/core/regs.c
#![no_std]
...
// map = (struct cxl_device_reg_map){ 0 };
    ...
    if (!res) {
    let mut end: resource_size_t = addr + length - 1;
...
// (mi->addr) = devm_cxl_iomap_block(host, addr, length);
    ...
    {
    struct device *host = map.host;
    let mut phys_addr: resource_size_t = map.resource;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mapinfo {
    pub rmap: *const cxl_reg_map,
...
// (mi->addr) = devm_cxl_iomap_block(host, addr, length);
    ...
    {
    let mut reg_type: u8 = FIELD_GET(PCI_DVSEC_CXL_REG_LOCATOR_BLOCK_ID, reg_lo);
    let mut bar: c_int = FIELD_GET(PCI_DVSEC_CXL_REG_LOCATOR_BIR, reg_lo);
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_count_regblock(pdev: *mut pci_dev, type: enum cxl_regloc_type) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_map_pmu_regs(map: *mut cxl_register_map, regs: *mut cxl_pmu_regs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_map_regblock(map: *mut cxl_register_map) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_unmap_regblock(map: *mut cxl_register_map) {
...
#[no_mangle]
unsafe extern "C" fn cxl_probe_regs(map: *mut cxl_register_map) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_setup_regs(map: *mut cxl_register_map) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_rcrb_to_aer(dev: *mut device, rcrb: resource_size_t) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn cxl_rcrb_to_linkcap(dev: *mut device, dport: *mut cxl_dport) -> resource_size_t {
    static resource_size_t cxl_rcrb_to_linkcap(struct device *dev, struct cxl_dport *dport)
    {
    let mut rcrb: resource_size_t = dport.rcrb.base;
    void __iomem *addr;
    u32 cap_hdr;
    u16 offset;
...
#[no_mangle]
pub unsafe extern "C" fn cxl_dport_map_rcd_linkcap(pdev: *mut pci_dev, dport: *mut cxl_dport) -> c_int {
    int cxl_dport_map_rcd_linkcap(struct pci_dev *pdev, struct cxl_dport *dport)
    {
    void __iomem *dport_pcie_cap = core::ptr::null_mut();
    resource_size_t pos;
    ...
    let mut rcrb: resource_size_t = ri.base;
    void __iomem *addr;
    u32 bar0, bar1;
    u32 id;
