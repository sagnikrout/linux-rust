...
//! Source: drivers/cxl/core/cdat.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cdat_normalize(entry: u16, base: u64, type: u8) -> u32 {
...
//
    ...
    let mut size: c_int = sizeof(*hdr) + sizeof(*dsmas);
    struct xarray *dsmas_xa = arg;
    struct dsmas_entry *dent;
    u16 len;
    int rc;
    len = le16_to_cpu(( __le16)hdr.length);
...
// Skip common header
    ...
    dent.dpa_range.start = le64_to_cpu(( __le64)dsmas.dpa_base_address);
    dent.dpa_range.end = le64_to_cpu(( __le64)dsmas.dpa_base_address) +
    le64_to_cpu(( __le64)dsmas.dpa_length) - 1;
    ...
    let mut size: c_int = sizeof(*hdr) + sizeof(*dslbis);
    struct xarray *dsmas_xa = arg;
    struct dsmas_entry *dent;
    __le64 le_base;
    __le16 le_val;
...
// Not a memory type, skip
    ...
    le_base = ( __le64)dslbis.entry_base_unit;
    le_val = ( __le16)dslbis.entry[0];
...
#[no_mangle]
unsafe extern "C" fn cdat_table_parse_output(rc: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn match_cxlrd_qos_class(dev: *mut device, data: *mut c_void) -> c_int {
    static int match_cxlrd_qos_class(struct device *dev, void *data)
    {
    let mut dev_qos_class: c_int = *(int *)data;
...
#[no_mangle]
unsafe extern "C" fn reset_dpa_perf(dpa_perf: *mut cxl_dpa_perf) {
...
#[no_mangle]
unsafe extern "C" fn match_cxlrd_hb(dev: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxl_qos_class_verify(cxlmd: *mut cxl_memdev) {
...
#[no_mangle]
unsafe extern "C" fn discard_dsmas(xa: *mut xarray) {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_endpoint_parse_cdat(port: *mut cxl_port) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct acpi_cdat_sslbis_table {
    pub header: acpi_cdat_header,
    pub sslbis_header: acpi_cdat_sslbis,
    pub entries: [acpi_cdat_sslbe; ],
    pub )header: *mut *mut } tbl = (struct acpi_cdat_sslbis_table,
    pub sizeof(tbl->sslbis_header): int size = sizeof(header->cdat) +,
    pub sslbis: *mut acpi_cdat_sslbis,
    pub arg: *mut *mut cxl_dport dport =,
    pub &dport->port->dev: *mut *mut device dev =,
    pub i: int remain, entries,,
    pub len: u16,
    pub __le16)header->cdat.length): len = le16_to_cpu((,
    pub size: remain = len -,
    if (!remain || remain % sizeof(tbl.entries[0]) ||
    (unsigned long)header + len > end) {
    pub len): dev_warn(dev, "Malformed SSLBIS table length: (%u)\n",,
    pub -EINVAL: return,
    }
    pub &tbl->sslbis_header: sslbis =,
// Unrecognized data type, we can skip
    if (sslbis.data_type > ACPI_HMAT_WRITE_BANDWIDTH)
    pub 0: return,
    pub sizeof(tbl->entries[0]): entries = remain /,
    if (struct_size(tbl, entries, entries) != len)
    pub -EINVAL: return,
    pub {: for (i = 0; i < entries; i++),
    pub __le16)tbl->entries[i].portx_id): u16 x = le16_to_cpu((,
    pub __le16)tbl->entries[i].porty_id): u16 y = le16_to_cpu((,
    pub le_base: __le64,
    pub le_val: __le16,
    pub dsp_id: u16,
    pub val: u64,
    ...
    pub x: dsp_id =,
    case ACPI_CDAT_SSLBIS_ANY_PORT:
    pub ACPI_CDAT_SSLBIS_ANY_PORT: dsp_id =,
    default:
    pub y: dsp_id =,
    }
    default:
    ...
    pub __le64)tbl->sslbis_header.entry_base_unit: le_base = (,
    pub __le16)tbl->entries[i].latency_or_bandwidth: le_val = (,
    val = cdat_normalize(le16_to_cpu(le_val), le64_to_cpu(le_base),
    if (dsp_id == ACPI_CDAT_SSLBIS_ANY_PORT ||
    dsp_id == dport.port_id) {
    cxl_access_coordinate_set(dport.coord,
    pub val): sslbis->data_type,,
    pub 0: return,
    }
    }
    pub 0: return,
...
#[no_mangle]
pub unsafe extern "C" fn cxl_switch_parse_cdat(dport: *mut cxl_dport) {
    void cxl_switch_parse_cdat(struct cxl_dport *dport)
    {
    pub dport->port: *mut *mut cxl_port port =,
    pub rc: c_int,
    if (!port.cdat.table)
    rc = cdat_table_parse(ACPI_CDAT_TYPE_SSLBIS, cdat_sslbis_handler,
    pub port->cdat.length): dport, port->cdat.table,,
    pub cdat_table_parse_output(rc): rc =,
    if (rc)
    pub rc): dev_dbg(&port->dev, "Failed to parse SSLBIS: %d\n",,
    }
    pub "CXL"): EXPORT_SYMBOL_NS_GPL(cxl_switch_parse_cdat,,
    ...
    pub c2->write_latency: out->write_latency = c1->write_latency +,
    if (c1.read_bandwidth && c2.read_bandwidth)
    out.read_bandwidth = min(c1.read_bandwidth,
    pub c2->read_latency: out->read_latency = c1->read_latency +,
    }
//
...
//
    ...
    struct access_coordinate *c2)
    {
    pub i++): for (int i = 0; i < ACCESS_COORDINATE_MAX;,
    pub &c2[i]): __cxl_coordinates_combine(&out[i], &c1[i],,
    }
    static void cxl_bandwidth_add(struct access_coordinate *coord,
    struct access_coordinate *c1,
    struct access_coordinate *c2)
    {
    pub {: for (int i = 0; i < ACCESS_COORDINATE_MAX; i++),
...
#[no_mangle]
unsafe extern "C" fn free_perf_xa(xa: *mut xarray) {
...
//
    ...
    xa_for_each(xa, index, ctx) {
    struct cxl_port *port = ctx.port;
    let mut hb_index: c_ulong = (unsigned long)port.uport_dev;
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_region_shared_upstream_bandwidth_update(cxlr: *mut cxl_region) {
    void cxl_region_shared_upstream_bandwidth_update(struct cxl_region *cxlr)
    {
    struct xarray *working_xa;
    let mut root_count: c_int = 0;
    bool is_root;
    int rc;
