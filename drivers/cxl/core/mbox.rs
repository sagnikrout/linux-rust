...
//! Source: drivers/cxl/core/mbox.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cxl_is_security_command(opcode: u16) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cxl_is_poison_command(opcode: u16) -> bool {
    static bool cxl_is_poison_command(u16 opcode)
    {
pub const CXL_MBOX_OP_POISON_CMDS: c_uint = 0x43;
...
#[no_mangle]
unsafe extern "C" fn cxl_mem_raw_command_allowed(opcode: u16) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cxl_mbox_cmd_dtor(mbox: *mut cxl_mbox_cmd) {
...
//
    ...
    let mut info: cxl_command_info = cmd.info;
    if (test_bit(info.id, cxl_mbox.enabled_cmds))
...
#[no_mangle]
pub unsafe extern "C" fn cxl_send_cmd(cxl_mbox: *mut cxl_mailbox, s: *mut cxl_send_command __user) -> c_int {
    ...
    let mut remaining: u32 = *size;
    let mut offset: u32 = 0;
    while (remaining) {
    let mut xfer_size: u32 = min_t(u32, remaining, cxl_mbox.payload_size);
    struct cxl_mbox_cmd mbox_cmd;
...
#[no_mangle]
unsafe extern "C" fn check_features_opcodes(opcode: u16, ro_cmds: *mut c_int, wr_cmds: *mut c_int) -> c_int {
...
pub const MAX_FEATURES_READ_CMDS: c_int = 2;
...
//
#[no_mangle]
unsafe extern "C" fn cxl_walk_cel(mds: *mut cxl_memdev_state, size: usize, cel: *mut u8) {
    static void cxl_walk_cel(struct cxl_memdev_state *mds, size_t size, u8 *cel)
    {
    struct cxl_mailbox *cxl_mbox = &mds.cxlds.cxl_mbox;
    struct cxl_cel_entry *cel_entry;
    let mut cel_entries: c_int = size / sizeof(*cel_entry);
    struct device *dev = mds.cxlds.dev;
    int i, ro_cmds = 0, wr_cmds = 0;
    cel_entry = (struct cxl_cel_entry *) cel;
    for (i = 0; i < cel_entries; i++) {
    let mut opcode: u16 = le16_to_cpu(cel_entry[i].opcode);
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_enumerate_cmds(mds: *mut cxl_memdev_state) -> c_int {
    ...
    let mut size: u32 = le32_to_cpu(gsl.entry[i].size);
    let mut uuid: uuid_t = gsl.entry[i].uuid;
    u8 *log;
...
//
    ...
    if (cxlr) {
    let mut cache_size: u64 = cxlr.params.cache_size;
    ...
    {
    let mut ev_type: enum cxl_event_type = CXL_CPER_EVENT_GENERIC;
    const uuid_t *uuid = &record.id;
    if (uuid_equal(uuid, &CXL_EVENT_GEN_MEDIA_UUID))
    ev_type = CXL_CPER_EVENT_GEN_MEDIA;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: uuid_equal(uuid, _arg: &CXL_EVENT_DRAM_UUID)) -> else {
    else if (uuid_equal(uuid, &CXL_EVENT_DRAM_UUID))
    ev_type = CXL_CPER_EVENT_DRAM;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: uuid_equal(uuid, _arg: &CXL_EVENT_MEM_MODULE_UUID)) -> else {
    else if (uuid_equal(uuid, &CXL_EVENT_MEM_MODULE_UUID))
    ev_type = CXL_CPER_EVENT_MEM_MODULE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: uuid_equal(uuid, _arg: &CXL_EVENT_MEM_SPARING_UUID)) -> else {
    ...
    let mut total: u16 = le16_to_cpu(get_pl.record_count);
    let mut max_handles: u8 = CXL_CLEAR_EVENT_MAX_HANDLES;
    let mut pl_size: usize = struct_size(payload, handles, max_handles);
    struct cxl_mbox_cmd mbox_cmd;
    u16 cnt;
    let mut rc: c_int = 0;
    int i;
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_mem_get_event_records(mds: *mut cxl_memdev_state, status: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_mem_get_partition_info(mds: *mut cxl_memdev_state) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_dev_state_identify(mds: *mut cxl_memdev_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __cxl_mem_sanitize(mds: *mut cxl_memdev_state, cmd: u16) -> c_int {
    static int __cxl_mem_sanitize(struct cxl_memdev_state *mds, u16 cmd)
    {
    struct cxl_mailbox *cxl_mbox = &mds.cxlds.cxl_mbox;
    int rc;
    let mut sec_out: u32 = 0;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxl_get_security_output {
    pub flags: __le32,
    pub out: },
    ...
    let mut mbox_cmd: cxl_mbox_cmd = { .opcode = cmd };
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_mem_sanitize(cxlmd: *mut cxl_memdev, cmd: u16) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_get_dirty_count(mds: *mut cxl_memdev_state, count: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_arm_dirty_shutdown(mds: *mut cxl_memdev_state) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_set_timestamp(mds: *mut cxl_memdev_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_poison_buf(buf: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn cxl_poison_alloc_buf(mds: *mut cxl_memdev_state) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_poison_state_init(mds: *mut cxl_memdev_state) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_mailbox_init(cxl_mbox: *mut cxl_mailbox, host: *mut device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_mbox_init() -> void __init {
