...
#[no_mangle]
unsafe extern "C" fn add_hdm_decoder(port: *mut cxl_port, cxld: *mut cxl_decoder) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn devm_cxl_add_passthrough_decoder(port: *mut cxl_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parse_hdm_decoder_caps(cxlhdm: *mut cxl_hdm) {
...
#[no_mangle]
unsafe extern "C" fn should_emulate_decoders(info: *mut cxl_endpoint_dvsec_info) -> bool {
...
#[no_mangle]
unsafe extern "C" fn __cxl_dpa_debug(file: *mut seq_file, r: *mut resource, depth: c_int) {
    static void __cxl_dpa_debug(struct seq_file *file, struct resource *r, int depth)
    {
    let mut start: c_ulonglong = r.start, end = r.end;
    seq_printf(file, "%*s%08llx-%08llx : %s\n", depth * 2, "", start, end,
    r.name);
    }
#[no_mangle]
pub unsafe extern "C" fn cxl_dpa_debug(file: *mut seq_file, cxlds: *mut cxl_dev_state) {
...
// See request_skip() kernel-doc
    ...
    {
    let mut skip_end: resource_size_t = skip_base + skip_len - 1;
...
//
#[no_mangle]
unsafe extern "C" fn __cxl_dpa_release(cxled: *mut cxl_endpoint_decoder) {
...
// save @skip_start, before @res is released
    ...
    cxled.skip = 0;
    cxled.dpa_res = core::ptr::null_mut();
    put_device(&cxled.cxld.dev);
    port.hdm_end--;
    }
#[no_mangle]
unsafe extern "C" fn cxl_dpa_release(cxled: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn devm_cxl_dpa_release(cxled: *mut cxl_endpoint_decoder) {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_dpa_setup(cxlds: *mut cxl_dev_state, info: *const cxl_dpa_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_dpa_size(cxled: *mut cxl_endpoint_decoder) -> resource_size_t {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_dpa_resource_start(cxled: *mut cxl_endpoint_decoder) -> resource_size_t {
    resource_size_t cxl_dpa_resource_start(struct cxl_endpoint_decoder *cxled)
    {
    let mut base: resource_size_t = RESOURCE_SIZE_MAX;
...
#[no_mangle]
pub unsafe extern "C" fn cxl_resource_contains_addr(res: *const resource, addr: resource_size_t) -> bool {
    bool cxl_resource_contains_addr(const struct resource *res, const resource_size_t addr)
    {
    let mut _addr: resource = DEFINE_RES_MEM(addr, 1);
    return resource_contains(res, &_addr);
    }
#[no_mangle]
pub unsafe extern "C" fn cxl_dpa_free(cxled: *mut cxl_endpoint_decoder) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __cxl_dpa_alloc(cxled: *mut cxl_endpoint_decoder, size: u64) -> c_int {
    static int __cxl_dpa_alloc(struct cxl_endpoint_decoder *cxled, u64 size)
    {
    struct cxl_memdev *cxlmd = cxled_to_memdev(cxled);
    struct cxl_dev_state *cxlds = cxlmd.cxlds;
    struct device *dev = &cxled.cxld.dev;
    struct resource *res, *prev = core::ptr::null_mut();
...
//
    ...
    for (p = prev.child, last = core::ptr::null_mut(); p; p = p.sibling)
    last = p;
    if (last) {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_dpa_alloc(cxled: *mut cxl_endpoint_decoder, size: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxld_set_interleave(cxld: *mut cxl_decoder, ctrl: *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn cxld_set_type(cxld: *mut cxl_decoder, ctrl: *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn cxlsd_set_targets(cxlsd: *mut cxl_switch_decoder, tgt: *mut u64) {
    static void cxlsd_set_targets(struct cxl_switch_decoder *cxlsd, u64 *tgt)
    {
    struct cxl_dport **t = &cxlsd.target[0];
    let mut ways: c_int = cxlsd.cxld.interleave_ways;
...
//
pub const COMMIT_TIMEOUT_MS: c_int = 20;
#[no_mangle]
unsafe extern "C" fn cxld_await_commit(hdm: *mut void __iomem, id: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn setup_hw_decoder(cxld: *mut cxl_decoder, hdm: *mut void __iomem) {
...
#[no_mangle]
unsafe extern "C" fn cxl_decoder_commit(cxld: *mut cxl_decoder) -> c_int {
    static int cxl_decoder_commit(struct cxl_decoder *cxld)
    {
    struct cxl_port *port = to_cxl_port(cxld.dev.parent);
    struct cxl_hdm *cxlhdm = dev_get_drvdata(&port.dev);
    void __iomem *hdm = cxlhdm.regs.hdm_decoder;
    let mut id: c_int = cxld.id, rc;
...
#[no_mangle]
unsafe extern "C" fn commit_reap(dev: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxl_port_commit_reap(cxld: *mut cxl_decoder) {
...
//
    ...
    dev_dbg(&port.dev, "reap: %s commit_end: %d\n", dev_name(&cxld.dev),
    port.commit_end);
    device_for_each_child_reverse_from(&port.dev, &cxld.dev, core::ptr::null_mut(),
    commit_reap);
    }
    EXPORT_SYMBOL_NS_GPL(cxl_port_commit_reap, "CXL");
#[no_mangle]
unsafe extern "C" fn cxl_decoder_reset(cxld: *mut cxl_decoder) {
...
// Userspace is now responsible for reconfiguring this decoder
    ...
    return -ENOENT;
    cxld.target_type = CXL_DECODER_HOSTONLYMEM;
    cxld.commit = core::ptr::null_mut();
    cxld.reset = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn cxl_settle_decoders(cxlhdm: *mut cxl_hdm) {
...
//
#[no_mangle]
pub unsafe extern "C" fn devm_cxl_switch_port_decoders_setup(port: *mut cxl_port) -> c_int {
    int devm_cxl_switch_port_decoders_setup(struct cxl_port *port)
    {
    struct cxl_hdm *cxlhdm;
    if (is_cxl_root(port) || is_cxl_endpoint(port))
    return -EOPNOTSUPP;
    cxlhdm = devm_cxl_setup_hdm(port, core::ptr::null_mut());
    if (!IS_ERR(cxlhdm))
    return devm_cxl_enumerate_decoders(cxlhdm, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn devm_cxl_endpoint_decoders_setup(port: *mut cxl_port) -> c_int {
    int devm_cxl_endpoint_decoders_setup(struct cxl_port *port)
    {
    struct cxl_memdev *cxlmd = to_cxl_memdev(port.uport_dev);
    let mut info: cxl_endpoint_dvsec_info = { .port = port };
    struct cxl_dev_state *cxlds = cxlmd.cxlds;
    struct cxl_hdm *cxlhdm;
    int rc;
    if (!is_cxl_endpoint(port))
