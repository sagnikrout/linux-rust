...
//! Source: drivers/uio/uio_hv_generic.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hv_uio_private_data {
    pub info: uio_info,
    pub device: *mut hv_device,
    pub refcnt: core::sync::atomic::AtomicI32,
    pub recv_buf: *mut c_void,
    pub recv_gpadl: vmbus_gpadl,
    pub /: *mut *mut char recv_name[32]; / "recv_4294967295",
    pub send_buf: *mut c_void,
    pub send_gpadl: vmbus_gpadl,
    pub send_name: [c_char; 32],
}

#[no_mangle]
unsafe extern "C" fn set_event(channel: *mut vmbus_channel, irq_state: i32) {
...
//
#[no_mangle]
unsafe extern "C" fn hv_uio_channel_cb(context: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn hv_uio_rescind(channel: *mut vmbus_channel) {
...
// Callback from VMBUS subsystem when new channel created.
    ...
    let mut ring_bytes: usize = SZ_2M;
    int ret;
// Create host communication ring
    ret = vmbus_open(new_sc, ring_bytes, ring_bytes, core::ptr::null_mut(), 0,
...
// VMBus primary channel is closed on last close
    ...
    void *ring_buffer;
    int ret;
    let mut ring_size: usize = hv_dev_ring_size(channel);
    if (!ring_size)
    ring_size = SZ_2M;
