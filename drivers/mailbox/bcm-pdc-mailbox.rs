...
//! Source: drivers/mailbox/bcm-pdc-mailbox.c
#![no_std]
...
//
pub const PDC_RINGSET: c_int = 0;
//
// Interrupt mask and status definitions. Enable interrupts for tx and rx on
// ring 0
//

pub const PDC_LAZY_FRAMECOUNT: c_int = 1;
pub const PDC_LAZY_TIMEOUT: c_int = 10000;

pub const PDC_INTMASK_OFFSET: c_uint = 0x24;
pub const PDC_INTSTATUS_OFFSET: c_uint = 0x20;

pub const FA_RCVLAZY0_OFFSET: c_uint = 0x100;
//
// For SPU2, configure MDE_CKSUM_CONTROL to write 17 bytes of metadata
// before frame
//
pub const PDC_SPU2_RESP_HDR_LEN: c_int = 17;

pub const PDC_CKSUM_CTRL_OFFSET: c_uint = 0x400;
pub const PDC_SPUM_RESP_HDR_LEN: c_int = 32;
//
...
//
pub const PDC_TX_CTL: c_uint = 0x000C0800;
// Bit in tx control reg to enable tx channel
pub const PDC_TX_ENABLE: c_uint = 0x1;
//
...
// 20:18 - BurstLen = 3 -> 2^7 = 128 byte data reads from memory
//
pub const PDC_RX_CTL: c_uint = 0x000C0E00;
// Bit in rx control reg to enable rx channel
pub const PDC_RX_ENABLE: c_uint = 0x1;

// descriptor flags

pub const RX_STATUS_OVERFLOW: c_uint = 0x00800000;
pub const RX_STATUS_LEN: c_uint = 0x0000FFFF;
pub const PDC_TXREGS_OFFSET: c_uint = 0x200;
pub const PDC_RXREGS_OFFSET: c_uint = 0x220;
// Maximum size buffer the DMA engine can handle
pub const PDC_DMA_BUF_MAX: c_int = 16384;
    enum pdc_hw {
...
#[derive(Copy, Clone)]
pub struct dma64dd {
    pub /: *mut *mut u32 ctrl1; / misc control bits,
    pub /: *mut *mut u32 ctrl2; / buffer count and address extension,
    pub /: *mut *mut u32 addrlow; / memory address of the date buffer, bits 31:0,
    pub /: *mut *mut u32 addrhigh; / memory address of the date buffer, bits 63:32,
}

// dma registers per channel(xmt or rcv)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dma64_regs {
    pub /: *mut *mut u32 control; / enable, et al,
    pub /: *mut *mut u32 ptr; / last descriptor posted to chip,
    pub /: *mut *mut u32 addrlow; / descriptor ring base address low 32-bits,
    pub /: *mut *mut u32 addrhigh; / descriptor ring base address bits 63:32,
    pub /: *mut *mut u32 status0; / last rx descriptor written by hw,
    pub /: *mut *mut u32 status1; / driver does not use,
}
...
#[derive(Copy, Clone)]
pub struct dma64 {
    pub /: *mut *mut dma64_regs dmaxmt; / dma tx,
    pub PAD: [u32; 2],
    pub /: *mut *mut dma64_regs dmarcv; / dma rx,
    pub PAD: [u32; 2],
}
...
#[derive(Copy, Clone)]
pub struct pdc_regs {
    pub /: *mut *mut u32 devcontrol; / 0x000,
    pub /: *mut *mut u32 devstatus; / 0x004,
    pub PAD: u32,
    pub /: *mut *mut u32 biststatus; / 0x00c,
    pub PAD: [u32; 4],
    pub /: *mut *mut u32 intstatus; / 0x020,
    pub /: *mut *mut u32 intmask; / 0x024,
    pub /: *mut *mut u32 gptimer; / 0x028,
    pub PAD: u32,
    pub /: *mut *mut u32 intrcvlazy_0; / 0x030 (Only in PDC, not FA2),
    pub /: *mut *mut u32 intrcvlazy_1; / 0x034 (Only in PDC, not FA2),
    pub /: *mut *mut u32 intrcvlazy_2; / 0x038 (Only in PDC, not FA2),
    pub /: *mut *mut u32 intrcvlazy_3; / 0x03c (Only in PDC, not FA2),
    pub PAD: [u32; 48],
    pub /: *mut *mut u32 fa_intrecvlazy; / 0x100 (Only in FA2, not PDC),
    pub /: *mut *mut u32 flowctlthresh; / 0x104,
    pub /: *mut *mut u32 wrrthresh; / 0x108,
    pub /: *mut *mut u32 gmac_idle_cnt_thresh; / 0x10c,
    pub PAD: [u32; 4],
    pub /: *mut *mut u32 ifioaccessaddr; / 0x120,
    pub /: *mut *mut u32 ifioaccessbyte; / 0x124,
    pub /: *mut *mut u32 ifioaccessdata; / 0x128,
    pub PAD: [u32; 21],
    pub /: *mut *mut u32 phyaccess; / 0x180,
    pub PAD: u32,
    pub /: *mut *mut u32 phycontrol; / 0x188,
    pub /: *mut *mut u32 txqctl; / 0x18c,
    pub /: *mut *mut u32 rxqctl; / 0x190,
    pub /: *mut *mut u32 gpioselect; / 0x194,
    pub /: *mut *mut u32 gpio_output_en; / 0x198,
    pub /: *mut *mut u32 PAD; / 0x19c,
    pub /: *mut *mut u32 txq_rxq_mem_ctl; / 0x1a0,
    pub /: *mut *mut u32 memory_ecc_status; / 0x1a4,
    pub /: *mut *mut u32 serdes_ctl; / 0x1a8,
    pub /: *mut *mut u32 serdes_status0; / 0x1ac,
    pub /: *mut *mut u32 serdes_status1; / 0x1b0,
    pub /: *mut *mut u32 PAD[11]; / 0x1b4-1dc,
    pub /: *mut *mut u32 clk_ctl_st; / 0x1e0,
    pub /: *mut *mut u32 hw_war; / 0x1e4 (Only in PDC, not FA2),
    pub /: *mut *mut u32 pwrctl; / 0x1e8,
    pub PAD: [u32; 5],
pub const PDC_NUM_DMA_RINGS: c_int = 4;
    pub /: *mut *mut dma64 dmaregs[PDC_NUM_DMA_RINGS]; / 0x0200 - 0x2fc,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pdc_ring_alloc {
    pub /: *mut *mut dma_addr_t dmabase; / DMA address of start of ring,
    pub /: *mut *mut *mut void vbase; / base kernel virtual address of ring,
    pub /: *mut *mut u32 size; / ring allocation size in bytes,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pdc_rx_ctx {
    pub rxp_ctx: *mut c_void,
    pub dst_sg: *mut scatterlist,
    pub rxin_numd: u32,
    pub resp_hdr: *mut c_void,
    pub resp_hdr_daddr: dma_addr_t,
}

// PDC state structure
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pdc_state {
...
// Whether a BCM header is prepended to each frame
    pub use_bcm_hdr: bool,
// Sum of length of BCM header and rx status header
    pub pdc_resp_hdr_len: u32,
// The base virtual address of DMA hw registers
    pub pdc_reg_vbase: *mut void __iomem,
// Pool for allocation of DMA rings
    pub ring_pool: *mut dma_pool,
// Pool for allocation of metadata buffers for response messages
    pub rx_buf_pool: *mut dma_pool,
//
// The base virtual address of DMA tx/rx descriptor rings. Corresponding
// DMA address and size of ring allocation.
//
    pub tx_ring_alloc: pdc_ring_alloc,
    pub rx_ring_alloc: pdc_ring_alloc,
    pub /: *mut *mut *mut pdc_regs regs; / start of PDC registers,
    pub /: *mut *mut *mut dma64_regs txregs_64; / dma tx engine registers,
    pub /: *mut *mut *mut dma64_regs rxregs_64; / dma rx engine registers,
//
// Arrays of PDC_RING_ENTRIES descriptors
// To use multiple ringsets, this needs to be extended
//
    pub /: *mut *mut *mut dma64dd txd_64; / tx descriptor ring,
    pub /: *mut *mut *mut dma64dd rxd_64; / rx descriptor ring,
// descriptor ring sizes
    pub /: *mut *mut u32 ntxd; / # tx descriptors,
    pub /: *mut *mut u32 nrxd; / # rx descriptors,
    pub /: *mut *mut u32 nrxpost; / # rx buffers to keep posted,
    pub /: *mut *mut u32 ntxpost; / max number of tx buffers that can be posted,
//
...
//
    pub txin_numd: [u32; PDC_RING_ENTRIES],
//
...
// is processed
//
    pub src_sg: [*mut scatterlist; PDC_RING_ENTRIES],
// counters
    pub /: *mut *mut u32 pdc_requests; / number of request messages submitted,
    pub /: *mut *mut u32 pdc_replies; / number of reply messages received,
    pub /: *mut *mut u32 last_tx_not_done; / too few tx descriptors to indicate done,
    pub /: *mut *mut u32 tx_ring_full; / unable to accept msg because tx ring full,
    pub /: *mut *mut u32 rx_ring_full; / unable to accept msg because rx ring full,
    pub /: *mut *mut u32 txnobuf; / unable to create tx descriptor,
    pub /: *mut *mut u32 rxnobuf; / unable to create rx descriptor,
    pub /: *mut *mut u32 rx_oflow; / count of rx overflows,
// hardware type - FA2 or PDC/MDE
    pub hw_type: enum pdc_hw,
}

// Global variables
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pdc_globals {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_setup_debugfs(pdcs: *mut pdc_state) {
    ...
    if (!debugfs_dir)
    debugfs_dir = debugfs_create_dir(KBUILD_MODNAME, core::ptr::null_mut());
// S_IRUSR == 0400
    debugfs_create_file(spu_stats_name, 0400, debugfs_dir, pdcs,
    &pdc_debugfs_stats);
    }
#[no_mangle]
unsafe extern "C" fn pdc_free_debugfs() {
    static void pdc_free_debugfs(void)
    {
    debugfs_remove_recursive(debugfs_dir);
    debugfs_dir = core::ptr::null_mut();
    }
//
...
// Return opaque context with result
    ...
    rx_ctx.rxp_ctx = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn pdc_tx_list_sg_add(pdcs: *mut pdc_state, sg: *mut scatterlist) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_tx_list_final(pdcs: *mut pdc_state) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_rx_list_sg_add(pdcs: *mut pdc_state, sg: *mut scatterlist) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_irq_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t pdc_irq_handler(int irq, void *data)
    {
    struct device *dev = (struct device *)data;
    struct pdc_state *pdcs = dev_get_drvdata(dev);
    let mut intstatus: u32 = ioread32(pdcs.pdc_reg_vbase + PDC_INTSTATUS_OFFSET);
...
//
#[no_mangle]
unsafe extern "C" fn pdc_work_cb(t: *mut work_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_ring_init(pdcs: *mut pdc_state, ringset: c_int) -> c_int {
    static int pdc_ring_init(struct pdc_state *pdcs, int ringset)
    {
    int i;
    let mut err: c_int = PDC_SUCCESS;
...
#[no_mangle]
unsafe extern "C" fn pdc_ring_free(pdcs: *mut pdc_state) {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_desc_count(sg: *mut scatterlist) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_rings_full(pdcs: *mut pdc_state, tx_cnt: c_int, rx_cnt: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_last_tx_done(chan: *mut mbox_chan) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_send_data(chan: *mut mbox_chan, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pdc_startup(chan: *mut mbox_chan) -> c_int {
    static int pdc_startup(struct mbox_chan *chan)
    {
    return pdc_ring_init(chan.con_priv, PDC_RINGSET);
    }
#[no_mangle]
unsafe extern "C" fn pdc_shutdown(chan: *mut mbox_chan) {
...
//
    static
#[no_mangle]
pub unsafe extern "C" fn pdc_hw_init(pdcs: *mut pdc_state) {
    void pdc_hw_init(struct pdc_state *pdcs)
    {
    struct platform_device *pdev;
    struct device *dev;
    struct dma64 *dma_reg;
    let mut ringset: c_int = PDC_RINGSET;
    pdev = pdcs.pdev;
    dev = &pdev.dev;
...
//
#[no_mangle]
unsafe extern "C" fn pdc_hw_disable(pdcs: *mut pdc_state) {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_rx_buf_pool_create(pdcs: *mut pdc_state) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_interrupts_init(pdcs: *mut pdc_state) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_mb_init(pdcs: *mut pdc_state) -> c_int {
...
// Device tree API
    let mut pdc_hw: static int = PDC_HW;
    let mut fa_hw: static int = FA_HW;
...
//
#[no_mangle]
unsafe extern "C" fn pdc_dt_read(pdev: *mut platform_device, pdcs: *mut pdc_state) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pdc_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pdc_remove(pdev: *mut platform_device) {
