...
//! Source: drivers/pmdomain/governor.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dev_update_qos_constraint(dev: *mut device, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn default_suspend_ok(dev: *mut device) -> bool {
    ...
    let mut ret: bool = td.cached_suspend_ok;
...
#[no_mangle]
unsafe extern "C" fn update_domain_next_wakeup(genpd: *mut generic_pm_domain, now: ktime_t) {
    static void update_domain_next_wakeup(struct generic_pm_domain *genpd, ktime_t now)
    {
    let mut domain_wakeup: ktime_t = KTIME_MAX;
    ktime_t next_wakeup;
...
//
    ...
    {
    let mut domain_wakeup: ktime_t = genpd.gd.next_wakeup;
...
//
    list_for_each_entry(link, &genpd.parent_links, parent_node) {
    struct genpd_governor_data *cgd = link.child.gd;
    let mut sd_max_off_ns: i64 = cgd ? cgd.max_off_time_ns : -1;
    if (sd_max_off_ns < 0)
    continue;
//
...
//
#[no_mangle]
unsafe extern "C" fn _default_power_down_ok(pd: *mut dev_pm_domain, now: ktime_t) -> bool {
    static bool _default_power_down_ok(struct dev_pm_domain *pd, ktime_t now)
    {
    struct generic_pm_domain *genpd = pd_to_genpd(pd);
    struct genpd_governor_data *gd = genpd.gd;
    let mut state_idx: c_int = genpd.state_count - 1;
    struct gpd_link *link;
//
...
#[no_mangle]
unsafe extern "C" fn default_power_down_ok(pd: *mut dev_pm_domain) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cpu_power_down_ok(pd: *mut dev_pm_domain) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cpu_system_power_down_ok(pd: *mut dev_pm_domain) -> bool {
    static bool cpu_system_power_down_ok(struct dev_pm_domain *pd)
    {
    let mut constraint_ns: i64 = cpu_wakeup_latency_qos_limit() * NSEC_PER_USEC;
    struct generic_pm_domain *genpd = pd_to_genpd(pd);
    let mut state_idx: c_int = genpd.state_count - 1;
    if (!(genpd.flags & GENPD_FLAG_CPU_DOMAIN)) {
    genpd.state_idx = state_idx;
