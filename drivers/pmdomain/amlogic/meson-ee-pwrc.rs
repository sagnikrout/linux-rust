...
//! Source: drivers/pmdomain/amlogic/meson-ee-pwrc.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct meson_ee_pwrc_mem_domain {
    pub reg: c_uint,
    pub mask: c_uint,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct meson_ee_pwrc_top_domain {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct meson_ee_pwrc_domain_desc {
    pub name: *mut c_char,
    pub reset_names_count: c_uint,
    pub clk_names_count: c_uint,
    pub top_pd: *const meson_ee_pwrc_top_domain,
    pub mem_pd_count: c_uint,
    pub mem_pd: *const meson_ee_pwrc_mem_domain,
    pub pwrc_domain): *mut *mut bool (is_powered_off)(struct meson_ee_pwrc_domain,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct meson_ee_pwrc_domain_data {
    pub count: c_uint,
    pub domains: *const meson_ee_pwrc_domain_desc,
}
...
// TOP Power Domains
    ...
    let mut sm1_pwrc_vpu: static struct meson_ee_pwrc_top_domain = SM1_EE_PD(8);
    let mut sm1_pwrc_nna: static struct meson_ee_pwrc_top_domain = SM1_EE_PD(16);
    let mut sm1_pwrc_usb: static struct meson_ee_pwrc_top_domain = SM1_EE_PD(17);
    let mut sm1_pwrc_pci: static struct meson_ee_pwrc_top_domain = SM1_EE_PD(18);
    let mut sm1_pwrc_ge2d: static struct meson_ee_pwrc_top_domain = SM1_EE_PD(19);
...
// Memory PD Domains
    ...
    TOP_PD(__name, core::ptr::null_mut(), __mem, core::ptr::null_mut())
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct meson_ee_pwrc_domain {
    pub base: generic_pm_domain,
    pub enabled: bool,
    pub pwrc: *mut meson_ee_pwrc,
    pub desc: meson_ee_pwrc_domain_desc,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct meson_ee_pwrc {
    pub regmap_ao: *mut regmap,
    pub regmap_hhi: *mut regmap,
    pub domains: *mut meson_ee_pwrc_domain,
    pub xlate: genpd_onecell_data,
}

#[no_mangle]
unsafe extern "C" fn pwrc_ee_is_powered_off(pwrc_domain: *mut meson_ee_pwrc_domain) -> bool {
    static bool pwrc_ee_is_powered_off(struct meson_ee_pwrc_domain *pwrc_domain)
    {
    u32 reg;
...
#[no_mangle]
unsafe extern "C" fn meson_ee_pwrc_off(domain: *mut generic_pm_domain) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn meson_ee_pwrc_on(domain: *mut generic_pm_domain) -> c_int {
    ...
    if (dom.num_rstc) {
    let mut count: c_int = reset_control_get_count(&pdev.dev);
    ...
    if (dom.num_clks) {
    let mut ret: c_int = devm_clk_bulk_get_all(&pdev.dev, &dom.clks);
    if (ret < 0)
    return ret;
    if (dom.num_clks != ret) {
...
//
    ...
    ret = pm_genpd_init(&dom.base, core::ptr::null_mut(), false);
    if (ret)
    return ret;
    } else {
    ret = pm_genpd_init(&dom.base, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn meson_ee_pwrc_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn meson_ee_pwrc_shutdown(pdev: *mut platform_device) {
