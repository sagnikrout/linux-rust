...
//! Source: drivers/target/iscsi/cxgbit/cxgbit_main.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn _cxgbit_free_cdev(kref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_set_mdsl(cdev: *mut cxgbit_device) {
    static void cxgbit_set_mdsl(struct cxgbit_device *cdev)
    {
    struct cxgb4_lld_info *lldi = &cdev.lldi;
    u32 mdsl;
pub const CXGBIT_T5_MAX_PDU_LEN: c_int = 16224;
...
#[no_mangle]
unsafe extern "C" fn cxgbit_close_conn(cdev: *mut cxgbit_device) {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_detach_cdev(cdev: *mut cxgbit_device) {
    static void cxgbit_detach_cdev(struct cxgbit_device *cdev)
    {
    let mut free_cdev: bool = false;
    spin_lock_bh(&cdev.cskq.lock);
...
#[no_mangle]
unsafe extern "C" fn cxgbit_uld_state_change(handle: *mut c_void, state: enum cxgb4_state) -> c_int {
    ...
    unsigned int offset)
    {
    let mut skb_frag_idx: u8 = skb_shinfo(skb).nr_frags;
    u8 i;
...
#[no_mangle]
unsafe extern "C" fn cxgbit_queue_lro_skb(csk: *mut cxgbit_sock, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_uld_lro_flush(lro_mgr: *mut t4_lro_mgr) {
    ...
    struct cxgbit_lro_cb *lro_cb;
    if (!csk) {
    pr_err("%s: csk core::ptr::null_mut(), op 0x%x.\n", __func__, op);
    goto out;
    }
    if (csk.lro_skb)
    goto add_packet;
    ...
    struct cxgb4_lld_info *lldi = &cdev.lldi;
    struct cpl_tx_data *rpl = core::ptr::null_mut();
    struct cxgbit_sock *csk = core::ptr::null_mut();
    let mut tid: c_uint = 0;
    struct sk_buff *skb;
    let mut op: c_uint = *(u8 *)rsp;
    let mut lro_flush: bool = true;
    switch (op) {
    case CPL_ISCSI_HDR:
    case CPL_ISCSI_DATA:
    ...
    if (!cxgbit_lro_receive(csk, op, rsp, core::ptr::null_mut(), lro_mgr,
    napi))
    return 0;
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxgbit_dcb_work {
    pub dcb_app: dcb_app_type,
...
#[no_mangle]
unsafe extern "C" fn cxgbit_dcb_workfn(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_get_sup_prot_ops(conn: *mut iscsit_conn) -> enum target_prot_op {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_exit() -> void __exit {
