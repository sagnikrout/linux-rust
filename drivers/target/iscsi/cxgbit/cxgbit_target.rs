...
//! Source: drivers/target/iscsi/cxgbit/cxgbit_target.c
#![no_std]
...
pub struct sge_opaque_hdr {
    ...
    int errcode;
    let mut hdr_len: static u32 = TX_HDR_LEN + ISCSI_HDR_LEN;
    if (len) {
    ...
    let mut iso_len: u32 = iso ? sizeof(struct cpl_tx_data_iso) : 0;
...
//
#[no_mangle]
unsafe extern "C" fn cxgbit_is_ofld_imm(skb: *const sk_buff) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn cxgbit_sgl_len(n: c_uint) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn cxgbit_calc_tx_flits_ofld(skb: *const sk_buff) -> c_uint {
...
pub const CXGBIT_ISO_FSLICE: c_uint = 0x1;
pub const CXGBIT_ISO_LSLICE: c_uint = 0x2;
    static void
    cxgbit_cpl_tx_data_iso(struct sk_buff *skb, struct cxgbit_iso_info *iso_info)
    {
    struct cpl_tx_data_iso *cpl;
    let mut submode: c_uint = cxgbit_skcb_submode(skb);
    let mut fslice: c_uint = !!(iso_info.flags & CXGBIT_ISO_FSLICE);
    let mut lslice: c_uint = !!(iso_info.flags & CXGBIT_ISO_LSLICE);
    ...
    let mut submode: u32 = cxgbit_skcb_submode(skb);
    let mut wr_ulp_mode: u32 = 0;
    let mut hdr_size: u32 = sizeof(*req);
    let mut opcode: u32 = FW_OFLD_TX_DATA_WR;
    let mut immlen: u32 = 0;
...
#[no_mangle]
unsafe extern "C" fn cxgbit_arp_failure_skb_discard(handle: *mut c_void, skb: *mut sk_buff) {
    static void cxgbit_arp_failure_skb_discard(void *handle, struct sk_buff *skb)
    {
    kfree_skb(skb);
    }
#[no_mangle]
pub unsafe extern "C" fn cxgbit_push_tx_frames(csk: *mut cxgbit_sock) {
    void cxgbit_push_tx_frames(struct cxgbit_sock *csk)
    {
    struct sk_buff *skb;
    while (csk.wr_cred && ((skb = skb_peek(&csk.txq)) != core::ptr::null_mut())) {
    let mut dlen: u32 = skb.len;
    let mut len: u32 = skb.len;
    u32 credits_needed;
    let mut compl: u32 = 0;
    let mut flowclen16: u32 = 0;
    let mut iso_cpl_len: u32 = 0;
    if (cxgbit_skcb_flags(skb) & SKCBF_TX_ISO)
...
//
    ...
    skb.csum = ( __wsum)(credits_needed + flowclen16);
...
#[no_mangle]
unsafe extern "C" fn cxgbit_unlock_sock(csk: *mut cxgbit_sock) {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_queue_skb(csk: *mut cxgbit_sock, skb: *mut sk_buff) -> c_int {
    ...
    u32 data_length)
    {
    let mut i: u32 = 0, nr_frags = MAX_SKB_FRAGS;
    let mut padding: u32 = ((-data_length) & 3);
...
//
    ...
    let mut cur_len: u32 = min_t(u32, data_length, sg.length - page_off);
    page = sg_page(sg);
    get_page(page);
    ...
    let mut data_length: u32 = cmd.se_cmd.data_length;
    let mut mrdsl: u32 = conn.conn_ops.MaxRecvDataSegmentLength;
    u32 num_pdu, plen, tx_data = 0;
    bool task_sense = !!(cmd.se_cmd.se_cmd_flags &
    SCF_TRANSPORT_TASK_SENSE);
    let mut set_statsn: bool = false;
    let mut ret: c_int = -1;
    while (data_length) {
    ...
    struct cxgbit_sock *csk = conn.context;
    struct sk_buff *skb;
    let mut padding: u32 = ((-data_buf_len) & 3);
    skb = cxgbit_alloc_skb(csk, data_buf_len + padding);
    if (unlikely(!skb))
    return -ENOMEM;
    memcpy(skb.data, cmd.pdu, ISCSI_HDR_LEN);
    if (data_buf_len) {
    let mut pad_bytes: u32 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn cxgbit_validate_params(conn: *mut iscsit_conn) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_set_digest(csk: *mut cxgbit_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_set_iso_npdu(csk: *mut cxgbit_sock) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn cxgbit_seq_pdu_inorder(csk: *mut cxgbit_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_set_params(conn: *mut iscsit_conn) -> c_int {
    ...
    struct cxgbit_sock *csk = conn.context;
    struct sk_buff *skb;
    let mut padding_buf: u32 = 0;
    let mut padding: u8 = ((-length) & 3);
    ...
    const u8 *buf;
    let mut consumed: c_uint = 0, buf_len;
    ...
    let mut sg_nents: u32 = max(1UL, DIV_ROUND_UP(pdu_cb.dlen, PAGE_SIZE));
    ...
    {
    struct iscsit_conn *conn = cmd.conn;
    let mut cmdsn_ret: c_int = 0, immed_ret = IMMEDIATE_DATA_NORMAL_OPERATION;
//
...
//
    ...
    int rc;
    let mut dump_payload: bool = false;
...
#[no_mangle]
unsafe extern "C" fn cxgbit_handle_iscsi_dataout(csk: *mut cxgbit_sock) -> c_int {
    static int cxgbit_handle_iscsi_dataout(struct cxgbit_sock *csk)
    {
    struct scatterlist *sg_start;
    struct iscsit_conn *conn = csk.conn;
    struct iscsit_cmd *cmd = core::ptr::null_mut();
    struct cxgbit_cmd *ccmd;
    struct cxgbi_task_tag_info *ttinfo;
    struct cxgbit_lro_pdu_cb *pdu_cb = cxgbit_rx_pdu_cb(csk.skb);
    struct iscsi_data *hdr = (struct iscsi_data *)pdu_cb.hdr;
    let mut data_offset: u32 = be32_to_cpu(hdr.offset);
    let mut data_len: u32 = ntoh24(hdr.dlength);
    int rc, sg_nents, sg_off;
    let mut dcrc_err: bool = false;
    if (pdu_cb.flags & PDUCBF_RX_DDP_CMP) {
    let mut offset: u32 = be32_to_cpu(hdr.offset);
    u32 ddp_data_len;
    ...
    return rc;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !success) -> else {
    else if (!success)
    return 0;
    } else {
    ...
    return rc;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !cmd) -> else {
    else if (!cmd)
    return 0;
    }
    if (pdu_cb.flags & PDUCBF_RX_DCRC_ERR) {
    ...
    let mut skip: u32 = data_offset % PAGE_SIZE;
...
#[no_mangle]
unsafe extern "C" fn cxgbit_handle_nop_out(csk: *mut cxgbit_sock, cmd: *mut iscsit_cmd) -> c_int {
    ...
    unsigned char *ping_data = core::ptr::null_mut();
    let mut payload_length: u32 = pdu_cb.dlen;
    int ret;
...
//
    ...
    let mut payload_length: u32 = pdu_cb.dlen;
    int rc;
    unsigned char *text_in = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn cxgbit_target_rx_opcode(csk: *mut cxgbit_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_rx_opcode(csk: *mut cxgbit_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_rx_login_pdu(csk: *mut cxgbit_sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_lro_skb_dump(skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_lro_hskb_reset(csk: *mut cxgbit_sock) {
    ...
    let mut hfrag_idx: u8 = hssi.nr_frags;
    ...
    let mut dfrag_idx: u8 = hssi.nr_frags, i;
    hpdu_cb.flags |= pdu_cb.flags;
...
#[no_mangle]
unsafe extern "C" fn cxgbit_process_lro_skb(csk: *mut cxgbit_sock, skb: *mut sk_buff) -> c_int {
    static int cxgbit_process_lro_skb(struct cxgbit_sock *csk, struct sk_buff *skb)
    {
    struct cxgbit_lro_cb *lro_cb = cxgbit_skb_lro_cb(skb);
    struct cxgbit_lro_pdu_cb *pdu_cb = cxgbit_skb_lro_pdu_cb(skb, 0);
    let mut pdu_idx: u8 = 0, last_idx = 0;
    let mut ret: c_int = 0;
    if (!pdu_cb.complete) {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_t5_rx_lro_skb(csk: *mut cxgbit_sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_rx_lro_skb(csk: *mut cxgbit_sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_rx_skb(csk: *mut cxgbit_sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_rxq_len(csk: *mut cxgbit_sock, rxq: *mut sk_buff_head) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cxgbit_wait_rxq(csk: *mut cxgbit_sock) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxgbit_get_login_rx(conn: *mut iscsit_conn, login: *mut iscsi_login) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cxgbit_get_rx_pdu(conn: *mut iscsit_conn) {
