...
//! Source: drivers/target/target_core_spc.c
#![no_std]
...
pub const PD_TEXT_ID_INFO_HDR_LEN: c_int = 4;
#[no_mangle]
unsafe extern "C" fn spc_fill_alua_data(lun: *mut se_lun, buf: *mut c_uchar) {
...
// unit serial number
    ...
    len++; /* Extra Byte for core::ptr::null_mut() Terminator */
    buf[3] = len;
    }
    return 0;
    }
...
//
    ...
    let mut company_id: u32 = dev.t10_wwn.company_id;
    int cnt, off = 0;
...
//
    ...
    let mut val: c_int = hex_to_bin(*p);
    if (val < 0)
    continue;
    if (next) {
    next = false;
...
//
    ...
    struct se_device *dev = cmd.se_dev;
    struct se_lun *lun = cmd.se_lun;
    struct se_portal_group *tpg = core::ptr::null_mut();
    struct t10_alua_lu_gp_member *lu_gp_mem;
    struct t10_alua_tg_pt_gp *tg_pt_gp;
    unsigned char *prod = &dev.t10_wwn.model[0];
    let mut off: u32 = 0;
    let mut len: u16 = 0, id_len;
    off = 4;
//
...
// Header size for Designation descriptor
    ...
    let mut lu_gp_id: u16 = 0;
    let mut tg_pt_gp_id: u16 = 0;
    u16 tpgt;
    tpg = lun.lun_tpg;
//
...
//
    ...
    scsi_name_len += 1 /* Include  core::ptr::null_mut() terminator */;
//
...
//
    ...
    scsi_target_len += 1 /* Include  core::ptr::null_mut() terminator */;
//
// The null-terminated, null-padded (see 4.4.2) SCSI
// NAME STRING field contains a UTF-8 format string.
// The number of bytes in the SCSI NAME STRING field
...
// Block Limits VPD page
    ...
    struct se_device *dev = cmd.se_dev;
    let mut mtl: u32 = 0;
    let mut have_tp: c_int = 0, opt, min;
    u32 io_max_blocks;
//
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_rwrecovery(cmd: *mut se_cmd, pc: u8, p: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_control(cmd: *mut se_cmd, pc: u8, p: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_caching(cmd: *mut se_cmd, pc: u8, p: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_informational_exceptions(cmd: *mut se_cmd, pc: u8, p: *mut c_uchar) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_write_protect(buf: *mut c_uchar, type: c_int) {
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_dpofua(buf: *mut c_uchar, type: c_int) {
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_blockdesc(buf: *mut c_uchar, blocks: u64, block_size: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spc_modesense_long_blockdesc(buf: *mut c_uchar, blocks: u64, block_size: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spc_emulate_modesense(cmd: *mut se_cmd) -> sense_reason_t {
    static sense_reason_t spc_emulate_modesense(struct se_cmd *cmd)
    {
    struct se_device *dev = cmd.se_dev;
    char *cdb = cmd.t_task_cdb;
    unsigned char buf[SE_MODE_PAGE_BUF], *rbuf;
    let mut type: c_int = dev.transport.get_device_type(dev);
    let mut ten: c_int = (cmd.t_task_cdb[0] == MODE_SENSE_10);
    let mut dbd: bool = !!(cdb[1] & 0x08);
    let mut llba: bool = ten ? !!(cdb[1] & 0x10) : false;
    let mut pc: u8 = cdb[2] >> 6;
    let mut page: u8 = cdb[2] & 0x3f;
    let mut subpage: u8 = cdb[3];
...
//
    ...
    let mut blocks: u64 = dev.transport.get_blocks(dev);
    let mut block_size: u32 = dev.dev_attrib.block_size;
    if (ten) {
    if (llba) {
...
#[no_mangle]
unsafe extern "C" fn spc_emulate_modeselect(cmd: *mut se_cmd) -> sense_reason_t {
    static sense_reason_t spc_emulate_modeselect(struct se_cmd *cmd)
    {
    char *cdb = cmd.t_task_cdb;
    let mut ten: bool = cdb[0] == MODE_SELECT_10;
    let mut off: c_int = ten ? 8 : 4;
    let mut pf: bool = !!(cdb[1] & 0x10);
    u8 page, subpage;
    unsigned char *buf;
    unsigned char tbuf[SE_MODE_PAGE_BUF];
    int length;
    let mut ret: sense_reason_t = 0;
    int i;
    if (!cmd.data_length) {
...
#[no_mangle]
unsafe extern "C" fn spc_emulate_request_sense(cmd: *mut se_cmd) -> sense_reason_t {
    static sense_reason_t spc_emulate_request_sense(struct se_cmd *cmd)
    {
    unsigned char *cdb = cmd.t_task_cdb;
    unsigned char *rbuf;
    let mut ua_asc: u8 = 0, ua_ascq = 0;
...
#[no_mangle]
pub unsafe extern "C" fn spc_emulate_report_luns(cmd: *mut se_cmd) -> sense_reason_t {
    ...
    unsigned char *buf;
    let mut lun_count: u32 = 0, offset = 8;
    __be32 len;
    buf = transport_kmap_data_sg(cmd);
...
#[no_mangle]
unsafe extern "C" fn set_dpofua_usage_bits(usage_bits: *mut u8, dev: *mut se_device) {
...
#[no_mangle]
unsafe extern "C" fn set_dpofua_usage_bits32(usage_bits: *mut u8, dev: *mut se_device) {
...
// opcode = descr;
    ...
    {
    let mut descr_num: c_int = ARRAY_SIZE(tcm_supported_opcodes);
    const struct target_opcode_descriptor *descr = core::ptr::null_mut();
    unsigned char *cdb = cmd.t_task_cdb;
    let mut rctd: u8 = (cdb[2] >> 7) & 0x1;
