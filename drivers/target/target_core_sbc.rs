...
//! Source: drivers/target/target_core_sbc.c
#![no_std]
...
//
    ...
    let mut blocks_long: c_ulonglong = dev.transport.get_blocks(dev);
    unsigned char *rbuf;
    unsigned char buf[8];
    u32 blocks;
//
...
//
    ...
    let mut pi_prot_type: c_int = dev.dev_attrib.pi_prot_type;
    unsigned char *rbuf;
    unsigned char buf[32];
    let mut blocks: c_ulonglong = dev.transport.get_blocks(dev);
...
//
    ...
    let mut lalba: u16 = dev.transport.get_alignment_offset_lbas(dev);
...
#[no_mangle]
pub unsafe extern "C" fn sbc_get_write_same_sectors(cmd: *mut se_cmd) -> sector_t {
...
#[no_mangle]
pub unsafe extern "C" fn if(WRITE_SAME_16: cmd->t_task_cdb[0] ==) -> else {
...
//
    ...
    let mut nolb: sector_t = sbc_get_write_same_sectors(cmd);
    sense_reason_t ret;
    if (nolb) {
...
#[no_mangle]
pub unsafe extern "C" fn sbc_get_size(cmd: *mut se_cmd, sectors: u32) -> u32 {
    static inline u32 sbc_get_size(struct se_cmd *cmd, u32 sectors)
    {
    return cmd.se_dev.dev_attrib.block_size * sectors;
    }
#[no_mangle]
pub unsafe extern "C" fn transport_get_sectors_6(cdb: *mut c_uchar) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn transport_get_sectors_10(cdb: *mut c_uchar) -> u32 {
    static inline u32 transport_get_sectors_10(unsigned char *cdb)
    {
    return get_unaligned_be16(&cdb[7]);
    }
#[no_mangle]
pub unsafe extern "C" fn transport_get_sectors_12(cdb: *mut c_uchar) -> u32 {
    static inline u32 transport_get_sectors_12(unsigned char *cdb)
    {
    return get_unaligned_be32(&cdb[6]);
    }
#[no_mangle]
pub unsafe extern "C" fn transport_get_sectors_16(cdb: *mut c_uchar) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn transport_get_sectors_32(cdb: *mut c_uchar) -> u32 {
    static inline u32 transport_get_sectors_32(unsigned char *cdb)
    {
    return get_unaligned_be32(&cdb[28]);
    }
#[no_mangle]
pub unsafe extern "C" fn transport_lba_21(cdb: *mut c_uchar) -> u32 {
    static inline u32 transport_lba_21(unsigned char *cdb)
    {
    return get_unaligned_be24(&cdb[1]) & 0x1fffff;
    }
#[no_mangle]
pub unsafe extern "C" fn transport_lba_32(cdb: *mut c_uchar) -> u32 {
    static inline u32 transport_lba_32(unsigned char *cdb)
    {
    return get_unaligned_be32(&cdb[2]);
    }
#[no_mangle]
pub unsafe extern "C" fn transport_lba_64(cdb: *mut c_uchar) -> c_ulonglong {
    ...
    {
    struct se_device *dev = cmd.se_dev;
    let mut end_lba: sector_t = dev.transport.get_blocks(dev) + 1;
    let mut sectors: c_uint = sbc_get_write_same_sectors(cmd);
    sense_reason_t ret;
...
//
    ...
    {
    struct se_device *dev = cmd.se_dev;
    let mut ret: sense_reason_t = TCM_NO_SENSE;
...
//
    ...
    let mut len: c_uint = min(sg.length, cmp_len);
...
// miscmp_off);
    ...
    {
    struct se_device *dev = cmd.se_dev;
    let mut write_tbl: sg_table = { };
    struct scatterlist *write_sg;
    struct sg_mapping_iter m;
    unsigned int len;
    let mut block_size: c_uint = dev.dev_attrib.block_size;
    let mut compare_len: c_uint = (cmd.t_task_nolb * block_size);
    let mut miscmp_off: c_uint = 0;
    let mut ret: sense_reason_t = TCM_NO_SENSE;
    int i;
    if (!success) {
//
...
//
    ...
    if (rc != 0) {
    cmd.transport_complete_callback = core::ptr::null_mut();
...
//
    ...
    {
    let mut sp_ops: c_int = cmd.se_sess.sup_prot_ops;
    let mut pi_prot_type: c_int = dev.dev_attrib.pi_prot_type;
    let mut fabric_prot: bool = false;
...
// see explanation in spc_emulate_modesense
    ...
    {
    let mut service_action: u16 = get_unaligned_be16(&cdb[8]);
...
#[no_mangle]
pub unsafe extern "C" fn sbc_get_device_type(dev: *mut se_device) -> u32 {
    ...
    struct exec_cmd_ops *ops = cmd.protocol_data;
    struct se_device *dev = cmd.se_dev;
    unsigned char *buf, *ptr = core::ptr::null_mut();
    sector_t lba;
    int size;
    u32 range;
...
// First UNMAP block descriptor starts at 8 byte offset
    ...
    let mut sector: sector_t = cmd.t_task_lba;
    void *daddr, *paddr;
    int i, j, offset = 0;
    let mut block_size: c_uint = dev.dev_attrib.block_size;
    ...
    let mut sector: sector_t = start;
    void *daddr, *paddr;
    int i;
    sense_reason_t rc;
    let mut dsg_off: c_int = 0;
    let mut block_size: c_uint = dev.dev_attrib.block_size;
