...
//! Source: drivers/pinctrl/renesas/pinctrl-rzv2m.c
#![no_std]
...
// PIN capabilities
pub const PIN_CFG_GRP_1_8V_2: c_int = 1;
pub const PIN_CFG_GRP_1_8V_3: c_int = 2;
pub const PIN_CFG_GRP_SWIO_1: c_int = 3;
pub const PIN_CFG_GRP_SWIO_2: c_int = 4;
pub const PIN_CFG_GRP_3_3V: c_int = 5;
...
pub const RZV2M_DEDICATED_PORT_IDX: c_int = 22;
//
...
pub const PFC_MASK: c_uint = 0x07;
pub const PUPD_MASK: c_uint = 0x03;
pub const DRV_MASK: c_uint = 0x03;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rzv2m_dedicated_configs {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rzv2m_pinctrl_data {
    pub port_pins: *const *const c_char,
    pub port_pin_configs: *const u32,
    pub dedicated_pins: *const rzv2m_dedicated_configs,
...
#[no_mangle]
unsafe extern "C" fn rzv2m_writel_we(addr: *mut void __iomem, shift: u8, value: u8) {
...
// Unmask input/output
    ...
    ret = pinconf_generic_parse_dt_config(np, core::ptr::null_mut(), &configs, &num_configs);
    if (ret < 0)
    return ret;
...
// num_maps = 0;
    ...
    if (*num_maps == 0) {
    ret = rzv2m_dt_subnode_to_map(pctldev, np, core::ptr::null_mut(), map,
    num_maps, &index);
    if (ret < 0)
    goto done;
    }
    ...
    {
    let mut pincount: u8 = RZV2M_GPIO_PORT_GET_PINCNT(cfg);
    let mut port_index: u32 = RZV2M_GPIO_PORT_GET_INDEX(cfg);
    u32 data;
...
// PUPD uses 2-bits per pin
    ...
    break;
    case PIN_CONFIG_DRIVE_STRENGTH_UA: {
    let mut arg: c_uint = pinconf_to_config_argument(_configs[i]);
...
#[no_mangle]
unsafe extern "C" fn rzv2m_gpio_request(chip: *mut gpio_chip, offset: c_uint) -> c_int {
    static int rzv2m_gpio_request(struct gpio_chip *chip, unsigned int offset)
    {
    struct rzv2m_pinctrl *pctrl = gpiochip_get_data(chip);
    let mut port: u32 = RZV2M_PIN_ID_TO_PORT(offset);
    let mut bit: u8 = RZV2M_PIN_ID_TO_PIN(offset);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn rzv2m_gpio_get_direction(chip: *mut gpio_chip, offset: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rzv2m_gpio_get(chip: *mut gpio_chip, offset: c_uint) -> c_int {
    static int rzv2m_gpio_get(struct gpio_chip *chip, unsigned int offset)
    {
    struct rzv2m_pinctrl *pctrl = gpiochip_get_data(chip);
    let mut port: u32 = RZV2M_PIN_ID_TO_PORT(offset);
    let mut bit: u8 = RZV2M_PIN_ID_TO_PIN(offset);
    let mut direction: c_int = rzv2m_gpio_get_direction(chip, offset);
...
#[no_mangle]
unsafe extern "C" fn rzv2m_gpio_free(chip: *mut gpio_chip, offset: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn rzv2m_gpio_register(pctrl: *mut rzv2m_pinctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rzv2m_pinctrl_register(pctrl: *mut rzv2m_pinctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rzv2m_pinctrl_probe(pdev: *mut platform_device) -> c_int {
    ...
    clk = devm_clk_get_enabled(pctrl.dev, core::ptr::null_mut());
    if (IS_ERR(clk))
...
#[no_mangle]
unsafe extern "C" fn rzv2m_pinctrl_init() -> int __init {
