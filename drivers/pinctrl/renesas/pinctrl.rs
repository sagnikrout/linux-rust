...
//! Source: drivers/pinctrl/renesas/pinctrl.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sh_pfc_pin_config {
    pub gpio_enabled:1: u16,
    pub mux_mark:15: u16,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sh_pfc_pinctrl {
    pub pctl: *mut pinctrl_dev,
    pub pctl_desc: pinctrl_desc,
    pub pfc: *mut sh_pfc,
    pub pins: *mut pinctrl_pin_desc,
    pub configs: *mut sh_pfc_pin_config,
}

#[no_mangle]
unsafe extern "C" fn sh_pfc_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sh_pfc_get_functions_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// num_groups = pmx->pfc->info->functions[selector].nr_groups;
    ...
    let mut idx: c_int = sh_pfc_get_pin_index(pfc, grp.pins[i]);
...
// If mux is already set, this configures it here
    ...
    struct sh_pfc_pinctrl *pmx = pinctrl_dev_get_drvdata(pctldev);
    struct sh_pfc *pfc = pmx.pfc;
    let mut new_type: c_int = input ? PINMUX_TYPE_INPUT : PINMUX_TYPE_OUTPUT;
    let mut idx: c_int = sh_pfc_get_pin_index(pfc, offset);
    const struct sh_pfc_pin *pin = &pfc.info.pins[idx];
    unsigned long flags;
    unsigned int dir;
...
// Check whether the requested parameter is supported for a pin.
    ...
    {
    let mut idx: c_int = sh_pfc_get_pin_index(pfc, _pin);
    const struct sh_pfc_pin *pin = &pfc.info.pins[idx];
    switch (param) {
...
// config = pinconf_to_config_packed(param, arg);
    ...
    case PIN_CONFIG_POWER_SOURCE: {
    let mut mV: c_uint = pinconf_to_config_argument(configs[i]);
    let mut idx: c_int = sh_pfc_get_pin_index(pfc, _pin);
    const struct sh_pfc_pin *pin = &pfc.info.pins[idx];
    unsigned int mode, lo, hi;
    u32 pocctrl, val;
    ...
    let mut num_pins: c_uint = pmx.pfc.info.groups[group].nr_pins;
...
#[no_mangle]
unsafe extern "C" fn sh_pfc_map_pins(pfc: *mut sh_pfc, pmx: *mut sh_pfc_pinctrl) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sh_pfc_register_pinctrl(pfc: *mut sh_pfc) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rcar_pinmux_get_bias(pfc: *mut sh_pfc, pin: c_uint) -> c_uint {
    ...
    return PIN_CONFIG_BIAS_DISABLE;
#[no_mangle]
pub unsafe extern "C" fn if((sh_pfc_read(pfc: !reg->pud ||, BIT(bit)): reg->pud) &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn rmobile_pinmux_get_bias(pfc: *mut sh_pfc, pin: c_uint) -> c_uint {
    ...
    let mut value: u32 = ioread8(reg) & PORTnCR_PULMD_MASK;
    switch (value) {
