...
//! Source: drivers/pinctrl/pinctrl-stmfx.c
#![no_std]
...
pub const NR_GPIO_REGS: c_int = 3;
pub const NR_GPIOS_PER_REG: c_int = 8;
...
// Cache of IRQ_GPI_* registers for bus_lock
    pub irq_gpi_src: [u8; NR_GPIO_REGS],
    pub irq_gpi_type: [u8; NR_GPIO_REGS],
    pub irq_gpi_evt: [u8; NR_GPIO_REGS],
    pub irq_toggle_edge: [u8; NR_GPIO_REGS],
// Backup of GPIO_* registers for suspend/resume
    pub bkp_gpio_state: [u8; NR_GPIO_REGS],
    pub bkp_gpio_dir: [u8; NR_GPIO_REGS],
    pub bkp_gpio_type: [u8; NR_GPIO_REGS],
    pub bkp_gpio_pupd: [u8; NR_GPIO_REGS],
}

#[no_mangle]
unsafe extern "C" fn stmfx_gpio_get(gc: *mut gpio_chip, offset: c_uint) -> c_int {
    static int stmfx_gpio_get(struct gpio_chip *gc, unsigned int offset)
    {
    struct stmfx_pinctrl *pctl = gpiochip_get_data(gc);
    let mut reg: u32 = STMFX_REG_GPIO_STATE + get_reg(offset);
    let mut mask: u32 = get_mask(offset);
    u32 value;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn stmfx_gpio_set(gc: *mut gpio_chip, offset: c_uint, value: c_int) -> c_int {
    static int stmfx_gpio_set(struct gpio_chip *gc, unsigned int offset, int value)
    {
    struct stmfx_pinctrl *pctl = gpiochip_get_data(gc);
    let mut reg: u32 = value ? STMFX_REG_GPO_SET : STMFX_REG_GPO_CLR;
    let mut mask: u32 = get_mask(offset);
    return regmap_write_bits(pctl.stmfx.map, reg + get_reg(offset),
    mask, mask);
    }
#[no_mangle]
unsafe extern "C" fn stmfx_gpio_get_direction(gc: *mut gpio_chip, offset: c_uint) -> c_int {
    static int stmfx_gpio_get_direction(struct gpio_chip *gc, unsigned int offset)
    {
    struct stmfx_pinctrl *pctl = gpiochip_get_data(gc);
    let mut reg: u32 = STMFX_REG_GPIO_DIR + get_reg(offset);
    let mut mask: u32 = get_mask(offset);
    u32 val;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn stmfx_gpio_direction_input(gc: *mut gpio_chip, offset: c_uint) -> c_int {
    ...
    unsigned int offset)
    {
    let mut reg: u32 = STMFX_REG_GPIO_PUPD + get_reg(offset);
    ...
    unsigned int offset)
    {
    let mut reg: u32 = STMFX_REG_GPIO_TYPE + get_reg(offset);
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_irq_mask(data: *mut irq_data) {
    static void stmfx_pinctrl_irq_mask(struct irq_data *data)
    {
    struct gpio_chip *gpio_chip = irq_data_get_irq_chip_data(data);
    struct stmfx_pinctrl *pctl = gpiochip_get_data(gpio_chip);
    let mut reg: u32 = get_reg(data.hwirq);
    let mut mask: u32 = get_mask(data.hwirq);
    pctl.irq_gpi_src[reg] &= ~mask;
    gpiochip_disable_irq(gpio_chip, irqd_to_hwirq(data));
    }
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_irq_unmask(data: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_irq_set_type(data: *mut irq_data, type: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_irq_bus_lock(data: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_irq_bus_sync_unlock(data: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn stmfx_gpio_irq_request_resources(data: *mut irq_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stmfx_gpio_irq_release_resources(data: *mut irq_data) {
    ...
    unsigned int offset)
    {
    let mut reg: u32 = get_reg(offset);
    let mut mask: u32 = get_mask(offset);
    int val;
    if (!(pctl.irq_toggle_edge[reg] & mask))
    return;
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_irq_thread_fn(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_irq_print_chip(d: *mut irq_data, p: *mut seq_file) {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_probe(pdev: *mut platform_device) -> c_int {
...
// This will let us handle the parent IRQ in the driver
    ...
    if (ret)
    return ret;
    ret = devm_request_threaded_irq(pctl.dev, irq, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_backup_regs(pctl: *mut stmfx_pinctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_restore_regs(pctl: *mut stmfx_pinctrl) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stmfx_pinctrl_resume(dev: *mut device) -> c_int {
