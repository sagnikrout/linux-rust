...
//! Source: drivers/pinctrl/sophgo/pinctrl-sophgo-common.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn sophgo_dt_get_pin(value: u32) -> u16 {
    static u16 sophgo_dt_get_pin(u32 value)
    {
    return value;
    }
#[no_mangle]
unsafe extern "C" fn sophgo_cmp_pin(key: *const c_void, pivot: *const c_void) -> c_int {
    static int sophgo_cmp_pin(const void *key, const void *pivot)
    {
    const struct sophgo_pin *pin = pivot;
    let mut pin_id: c_int = (long)key;
    let mut pivid: c_int = pin.id;
    return pin_id - pivid;
    }
    ...
    for_each_available_child_of_node(np, child) {
    let mut npins: c_int = of_property_count_u32_elems(child, "pinmux");
    unsigned int *pins;
...
// don't create a map if there are no pinconf settings
    ...
    grpnames, ngroups, core::ptr::null_mut());
    if (ret < 0) {
...
// num_maps = nmaps;
    ...
    let mut value: u32 = configs[i].config;
...
#[no_mangle]
pub unsafe extern "C" fn sophgo_pinctrl_probe(pdev: *mut platform_device) -> c_int {
