...
//! Source: drivers/pinctrl/freescale/pinctrl-imx-scmi.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_pinctrl_imx {
    pub dev: *mut device,
    pub ph: *mut scmi_protocol_handle,
    pub pctldev: *mut pinctrl_dev,
    pub pctl_desc: pinctrl_desc,
    pub ops: *const scmi_pinctrl_proto_ops,
}

// SCMI pin control types, aligned with SCMI firmware
pub const IMX_SCMI_NUM_CFG: c_int = 5;
pub const IMX_SCMI_PIN_MUX: c_int = 192;
pub const IMX_SCMI_PIN_CONFIG: c_int = 193;
pub const IMX_SCMI_PIN_DAISY_ID: c_int = 194;
pub const IMX_SCMI_PIN_DAISY_CFG: c_int = 195;
pub const IMX_SCMI_PIN_EXT: c_int = 196;

pub const IMX_SCMI_PIN_SIZE: c_int = 24;
pub const IMX94_DAISY_OFF: c_uint = 0x608;
pub const IMX95_DAISY_OFF: c_uint = 0x408;
pub const IMX952_DAISY_OFF: c_uint = 0x460;
...
// create config map
    ...
    if (mux_val & 0xFF00) {
    let mut ext_val: c_int = (mux_val & 0xFF00) >> 8;
...
// cast to avoid build warning
    ...
    {
    let mut config: c_ulong = pinconf_to_config_packed(IMX_SCMI_PIN_CONFIG, 0);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn scmi_pinctrl_imx_probe(sdev: *mut scmi_device) -> c_int {
