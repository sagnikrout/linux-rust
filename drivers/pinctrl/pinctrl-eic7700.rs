...
//! Source: drivers/pinctrl/pinctrl-eic7700.c
#![no_std]
...
pub const EIC7700_MS_3V3: c_uint = 0x0;
pub const EIC7700_MS_1V8: c_uint = 0x3;
pub const EIC7700_FUNCTIONS_PER_PIN: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct eic7700_pin {
    pub functions: [u8; EIC7700_FUNCTIONS_PER_PIN],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct eic7700_pinctrl {
    pub base: *mut void __iomem,
    pub desc: pinctrl_desc,
    pub functions_count: c_uint,
    pub __counted_by(functions_count): pinfunction functions[],
}
...
#[no_mangle]
unsafe extern "C" fn eic7700_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// All pins are persistent over suspend
    ...
    {
    struct eic7700_pinctrl *pc = pinctrl_dev_get_drvdata(pctldev);
    let mut value: u32 = readl_relaxed(pc.base + EIC7700_PIN_REG(pin)) & EIC7700_PINCONF;
...
#[no_mangle]
unsafe extern "C" fn eic7700_get_functions_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// Count the number of groups for each function
    ...
    const struct eic7700_pin *pin_data = pc.desc.pins[pin].drv_data;
    let mut found_disabled: bool = false;
    for (unsigned int fs = 0; fs < EIC7700_FUNCTIONS_PER_PIN; fs++) {
    let mut selector: c_uint = pin_data.functions[fs];
...
#[no_mangle]
unsafe extern "C" fn eic7700_pinctrl_probe(pdev: *mut platform_device) -> c_int {
