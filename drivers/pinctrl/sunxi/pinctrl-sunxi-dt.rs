...
//! Source: drivers/pinctrl/sunxi/pinctrl-sunxi-dt.c
#![no_std]
...
pub const INVALID_MUX: c_uint = 0xff;
//
...
//
    ...
    let mut name_size: c_int = 0;
    let mut port_base: c_int = desc.pin_base / PINS_PER_BANK;
...
//
    ...
    let mut bank: c_int = (pin.pin.number - pin_base) / PINS_PER_BANK;
...
//
    ...
    struct sunxi_desc_pin *pin = &pins[i];
    let mut bank: c_int = (pin.pin.number - pin_base) / PINS_PER_BANK;
    let mut lastfunc: c_int = pin.variant + 1;
    let mut irq_mux: c_int = irq_bank_muxes[bank];
    func[0].name = "gpio_in";
    func[0].muxval = 0;
