...
//! Source: drivers/pinctrl/pinctrl-k210.c
#![no_std]
...
pub const K210_PC_DRIVE_SHIFT: c_int = 8;

pub const K210_PC_DRIVE_MAX: c_int = 7;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct k210_fpioa_data {
    pub dev: *mut device,
    pub pctl: *mut pinctrl_dev,
    pub fpioa: *mut k210_fpioa __iomem,
    pub sysctl_map: *mut regmap,
...
#[no_mangle]
unsafe extern "C" fn k210_pinconf_get_drive(max_strength_ua: c_uint) -> c_int {
    ...
    let mut mode: u32 = k210_pinconf_mode_id_to_mode[info.mode_id];
    let mut val: u32 = func | mode;
    ...
    {
    struct k210_fpioa_data *pdata = pinctrl_dev_get_drvdata(pctldev);
    let mut val: u32 = readl(&pdata.fpioa.pins[pin]);
    int drive;
...
#[no_mangle]
unsafe extern "C" fn k210_pinmux_get_function_count(pctldev: *mut pinctrl_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn k210_pinctrl_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// Ignore this node
    ...
    let mut pin: u32 = FIELD_GET(K210_PG_PIN, pinmux_group);
    let mut func: u32 = FIELD_GET(K210_PG_FUNC, pinmux_group);
    if (pin >= K210_NPINS) {
    ret = -EINVAL;
    goto exit;
    }
...
#[no_mangle]
unsafe extern "C" fn k210_fpioa_init_ties(pdata: *mut k210_fpioa_data) {
...
#[no_mangle]
unsafe extern "C" fn k210_fpioa_probe(pdev: *mut platform_device) -> c_int {
