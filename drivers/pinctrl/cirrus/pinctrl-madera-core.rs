...
//! Source: drivers/pinctrl/cirrus/pinctrl-madera-core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn madera_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// alt function
    ...
    struct madera_pin_private *priv = pinctrl_dev_get_drvdata(pctldev);
    unsigned int conf[2];
    let mut reg: c_uint = MADERA_GPIO1_CTRL_1 + (2 * pin);
    unsigned int fn;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn madera_mux_get_funcs_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// num_groups = priv->chip->n_pins;
    ...
    let mut n_chip_groups: c_uint = priv.chip.n_pin_groups;
...
//
    ...
    struct madera_pin_private *priv = pinctrl_dev_get_drvdata(pctldev);
    struct madera *madera = priv.madera;
    let mut reg: c_uint = MADERA_GPIO1_CTRL_2 + (2 * offset);
    unsigned int val;
    int ret;
    if (input)
    val = MADERA_GP1_DIR;
    ...
    struct madera_pin_private *priv = pinctrl_dev_get_drvdata(pctldev);
    struct madera *madera = priv.madera;
    let mut reg: c_uint = MADERA_GPIO1_CTRL_1 + (2 * offset);
    int ret;
...
//
    ...
    let mut n_groups: c_uint = priv.chip.n_pin_groups;
    int i, ret;
...
#[no_mangle]
unsafe extern "C" fn madera_pin_probe(pdev: *mut platform_device) -> c_int {
