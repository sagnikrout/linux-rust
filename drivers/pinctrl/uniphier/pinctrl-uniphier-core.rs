...
//! Source: drivers/pinctrl/uniphier/pinctrl-uniphier-core.c
#![no_std]
...
pub const UNIPHIER_PINCTRL_PINMUX_BASE: c_uint = 0x1000;
pub const UNIPHIER_PINCTRL_LOAD_PINMUX: c_uint = 0x1700;
pub const UNIPHIER_PINCTRL_DRVCTRL_BASE: c_uint = 0x1800;
pub const UNIPHIER_PINCTRL_DRV2CTRL_BASE: c_uint = 0x1900;
pub const UNIPHIER_PINCTRL_DRV3CTRL_BASE: c_uint = 0x1980;
pub const UNIPHIER_PINCTRL_PUPDCTRL_BASE: c_uint = 0x1a00;
pub const UNIPHIER_PINCTRL_IECTRL_BASE: c_uint = 0x1d00;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct uniphier_pinctrl_reg_region {
    pub node: list_head,
    pub base: c_uint,
    pub nregs: c_uint,
    pub __counted_by(nregs): u32 vals[],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct uniphier_pinctrl_priv {
    pub pctldesc: pinctrl_desc,
    pub pctldev: *mut pinctrl_dev,
    pub regmap: *mut regmap,
    pub socdata: *const uniphier_pinctrl_socdata,
    pub reg_regions: list_head,
}

#[no_mangle]
unsafe extern "C" fn uniphier_pctl_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// strength = strengths[(val >> shift) & mask];
    ...
    struct uniphier_pinctrl_priv *priv = pinctrl_dev_get_drvdata(pctldev);
    const struct pin_desc *desc = pin_desc_get(pctldev, pin);
    let mut iectrl: c_uint = uniphier_pin_get_iectrl(desc.drv_data);
...
// This pin is always input-enabled.
    ...
    {
    let mut param: enum pin_config_param = pinconf_to_config_param(*configs);
    let mut has_arg: bool = false;
    u32 arg;
    int ret;
    switch (param) {
    case PIN_CONFIG_BIAS_DISABLE:
...
// UNIPHIER_PIN_IECTRL_NONE means the pin is always input-enabled
    ...
    let mut num_pins: unsigned = priv.socdata.groups[selector].num_pins;
    int i, ret;
...
#[no_mangle]
unsafe extern "C" fn uniphier_pmx_get_functions_count(pctldev: *mut pinctrl_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uniphier_pinctrl_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uniphier_pinctrl_resume(dev: *mut device) -> c_int {
    ...
    let mut num_drvctrl: c_uint = 0;
    let mut num_drv2ctrl: c_uint = 0;
    let mut num_drv3ctrl: c_uint = 0;
    let mut num_pupdctrl: c_uint = 0;
    let mut num_iectrl: c_uint = 0;
