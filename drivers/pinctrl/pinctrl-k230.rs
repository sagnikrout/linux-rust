...
//! Source: drivers/pinctrl/pinctrl-k230.c
#![no_std]
...
pub const K230_NPINS: c_int = 64;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct k230_pin_conf {
    pub func: c_uint,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct k230_pin_group {
    pub name: *const c_char,
    pub pins: *mut c_uint,
    pub num_pins: c_uint,
    pub data: *mut k230_pin_conf,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct k230_pmx_func {
    pub name: *const c_char,
    pub groups: *const c_char,
    pub group_idx: *mut c_uint,
    pub ngroups: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct k230_pinctrl {
    pub dev: *mut device,
    pub pctl: pinctrl_desc,
    pub pctl_dev: *mut pinctrl_dev,
    pub regmap_base: *mut regmap,
    pub base: *mut void __iomem,
    pub groups: *mut k230_pin_group,
    pub ngroups: c_uint,
    pub functions: *mut k230_pmx_func,
...
#[no_mangle]
unsafe extern "C" fn k230_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn k230_get_functions_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// num_groups = info->functions[selector].ngroups;
    ...
    return -ENOMEM;
    for (i = 0; i < size; i++) {
    let mut mux_data: c_uint = be32_to_cpu(*list++);
    grp.pins[i] = (mux_data >> 8);
    grp.data[i].func = (mux_data & 0xff);
    ret = pinconf_generic_parse_dt_config(np, core::ptr::null_mut(),
    &grp.data[i].configs,
    &grp.data[i].nconfigs);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn k230_pinctrl_probe(pdev: *mut platform_device) -> c_int {
