...
//! Source: drivers/pinctrl/pinctrl-eyeq5.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct eq5p_bank {
    pub npins: c_uint,
    pub regs: [c_uint; EQ5P_REG_COUNT],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct eq5p_match_data {
    pub npins: c_uint,
    pub nfunctions: c_uint,
    pub nbanks: c_uint,
    pub pins: *const pinctrl_pin_desc,
    pub functions: *const pinfunction,
    pub banks: *const eq5p_bank,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct eq5p_pinctrl {
    pub desc: pinctrl_desc,
    pub base: *mut void __iomem,
    pub data: *const eq5p_match_data,
}
...
//
pub const EQ5P_GPIO_FUNC_SELECTOR: c_int = 0;
...
pub const EQ5P_EYEQ5_PIN_OFFSET_BANK_B: c_int = 29;
// Bank B
...
// Bank A alternate functions
    ...
    {
    let mut val: u32 = readl(pctrl.base + bank.regs[reg]);
    ...
    let mut npins: c_uint = _bank.npins;
    if (pin < npins) {
// bank = _bank;
// offset = pin;
    return 0;
...
#[no_mangle]
unsafe extern "C" fn eq5p_pinctrl_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// Second, we retrieve the bias.
    ...
    bias = "both";
#[no_mangle]
pub unsafe extern "C" fn if(!pu: pd &&) -> else {
    else if (pd && !pu)
    bias = "pulldown";
#[no_mangle]
pub unsafe extern "C" fn if(pu: !pd &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn eq5p_pinmux_get_functions_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// num_groups = pctrl->data->functions[selector].ngroups;
    ...
    let mut is_gpio: bool = func_selector == EQ5P_GPIO_FUNC_SELECTOR;
    const struct eq5p_bank *bank;
    unsigned int offset;
    u32 mask, val;
    int ret;
...
// Pin numbers and group selectors are the same thing in our case.
    ...
    let mut param: enum pin_config_param = pinconf_to_config_param(configs[i]);
