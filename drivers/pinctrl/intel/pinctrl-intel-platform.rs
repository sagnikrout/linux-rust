...
//! Source: drivers/pinctrl/intel/pinctrl-intel-platform.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct intel_platform_pins {
    ...
    return -ENOMEM;
    for (i = 0; i < size; i++) {
    let mut pin_number: c_uint = base + i;
...
// Unify delimiter for pin name
    ...
    {
    let mut pins: intel_platform_pins = {};
...
#[no_mangle]
unsafe extern "C" fn intel_platform_pinctrl_probe(pdev: *mut platform_device) -> c_int {
