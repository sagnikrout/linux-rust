...
//! Source: drivers/pinctrl/pinctrl-th1520.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct th1520_pinctrl {
    pub desc: pinctrl_desc,
    pub /: *mut *mut mutex mutex; / serialize adding functions,
    pub /: *mut *mut raw_spinlock_t lock; / serialize register access,
...
#[no_mangle]
unsafe extern "C" fn th1520_padcfg_shift(pin: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn th1520_muxcfg_shift(pin: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn th1520_muxtype_get(str: *const c_char) -> enum th1520_muxtype {
...
#[no_mangle]
unsafe extern "C" fn th1520_pad_muxdata(drv_data: *mut c_void) -> c_ulong {
    static unsigned long th1520_pad_muxdata(void *drv_data)
    {
    return (uintptr_t)drv_data & TH1520_PAD_MUXDATA;
    }
#[no_mangle]
unsafe extern "C" fn th1520_pad_no_padcfg(drv_data: *mut c_void) -> bool {
...
#[no_mangle]
unsafe extern "C" fn th1520_pinctrl_get_groups_count(pctldev: *mut pinctrl_dev) -> c_int {
...
// npins = 1;
    ...
    {
    unsigned long *seen = core::ptr::null_mut();
    unsigned int i;
    for (i = 0; i < nmaps; i++) {
    ...
    let mut npins: c_int = of_property_count_strings(child, "pins");
    if (npins <= 0) {
...
#[no_mangle]
unsafe extern "C" fn th1520_drive_strength_from_ma(arg: u32) -> u16 {
    ...
    {
    void __iomem *padcfg = th1520_padcfg(thp, pin);
    let mut shift: c_uint = th1520_padcfg_shift(pin);
    u32 tmp;
    mask <<= shift;
    value <<= shift;
    scoped_guard(raw_spinlock_irqsave, &thp.lock) {
...
// config = pinconf_to_config_packed(param, arg);
    ...
    {
    struct th1520_pinctrl *thp = pinctrl_dev_get_drvdata(pctldev);
    let mut pin: c_uint = thp.desc.pins[gsel].number;
    ...
    {
    struct th1520_pinctrl *thp = pinctrl_dev_get_drvdata(pctldev);
    let mut value: u32 = readl_relaxed(th1520_padcfg(thp, pin));
    value = (value >> th1520_padcfg_shift(pin)) & GENMASK(9, 0);
    ...
    {
    void __iomem *muxcfg = th1520_muxcfg(thp, pin);
    let mut shift: c_uint = th1520_muxcfg_shift(pin);
    u32 mask, value, tmp;
...
#[no_mangle]
unsafe extern "C" fn th1520_pinctrl_probe(pdev: *mut platform_device) -> c_int {
    ...
    group = &th1520_group1;
#[no_mangle]
pub unsafe extern "C" fn if(2: pin_group ==) -> else {
    else if (pin_group == 2)
    group = &th1520_group2;
#[no_mangle]
pub unsafe extern "C" fn if(3: pin_group ==) -> else {
    else if (pin_group == 3)
    group = &th1520_group3;
    else
