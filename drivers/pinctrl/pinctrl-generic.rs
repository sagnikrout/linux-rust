...
//! Source: drivers/pinctrl/pinctrl-generic.c
#![no_std]
...
//
    ...
    ret = pinmux_generic_add_function(pctldev, np.name, group_names, 1, core::ptr::null_mut());
    if (ret < 0) {
    pinctrl_utils_free_map(pctldev, *maps, *num_maps);
    ...
    ret = pinmux_generic_add_function(pctldev, np.name, group_names, ngroups, core::ptr::null_mut());
    if (ret < 0) {
    pinctrl_utils_free_map(pctldev, *maps, *num_maps);
    return dev_err_probe(dev, ret, "error adding function %s\n", np.name);
    }
