...
//! Source: drivers/pci/pci-sysfs.c
#![no_std]
...
// Macro flag: #define ARCH_PCI_DEV_GROUPS
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: pci_is_enabled(pdev)) -> else {
...
unsafe extern "C" fn rescan_store(bus: *const bus_type, buf: *const c_char, count: usize) -> isize {
    ...
    static struct device_attribute dev_attr_dev_rescan = __ATTR(rescan, 0200, core::ptr::null_mut(),
    dev_rescan_store);
    ...
    static DEVICE_ATTR_IGNORE_LOCKDEP(remove, 0220, core::ptr::null_mut(),
    remove_store);
    ...
    static struct device_attribute dev_attr_bus_rescan = __ATTR(rescan, 0200, core::ptr::null_mut(),
    bus_rescan_store);
    ...
    return -EINVAL;
    if (val) {
    let mut ret: c_int = pci_try_reset_bridge(pdev);
    if (ret)
    return ret;
    }
    return count;
...
pub unsafe extern "C" fn if(PCI_HEADER_TYPE_CARDBUS: dev->hdr_type ==) -> else {
    ...
    if ((off & 3) && size > 2) {
    let mut val: u16 = data[off - init_off];
    ...
    while (size > 3) {
    let mut val: u32 = data[off - init_off];
...
//
    ...
    {
    struct pci_dev *pdev = to_pci_dev(kobj_to_dev(kobj));
    let mut bar: c_int = (unsigned long)attr.private;
...
#[no_mangle]
unsafe extern "C" fn reset_method_lookup(name: *const c_char) -> c_int {
    ...
    n = 0;
    tmp_options = options;
    while ((name = strsep(&tmp_options, " ")) != core::ptr::null_mut()) {
    if (sysfs_streq(name, ""))
    continue;
    name = strim(name);
...
#[no_mangle]
unsafe extern "C" fn __resource_resize_show(dev: *mut device, n: c_int, buf: *mut c_char) -> isize {
...
#[no_mangle]
pub unsafe extern "C" fn DEVICE_ATTR_RW(_arg: resource##n##_resize) -> static {
