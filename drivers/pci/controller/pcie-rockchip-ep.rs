...
//! Source: drivers/pci/controller/pcie-rockchip-ep.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rockchip_pcie_ep {
    pub rockchip: rockchip_pcie,
    pub epc: *mut pci_epc,
    pub max_regions: u32,
    ...
    pub irq_pending: u8,
    pub perst_irq: c_int,
    pub perst_asserted: bool,
    pub link_up: bool,
    pub link_training: delayed_work,
}
    ...
    {
    let mut num_pass_bits: c_int = fls64(pci_addr ^ (pci_addr + size - 1));
...
// All functions share the same vendor ID with function 0
    ...
    struct rockchip_pcie_ep *ep = epc_get_drvdata(epc);
    struct rockchip_pcie *rockchip = &ep.rockchip;
    let mut bar_phys: dma_addr_t = epf_bar.phys_addr;
    let mut bar: enum pci_barno = epf_bar.barno;
    let mut flags: c_int = epf_bar.flags;
...
//
    ...
    let mut is_prefetch: bool = !!(flags & PCI_BASE_ADDRESS_MEM_PREFETCH);
    let mut is_64bits: bool = !!(flags & PCI_BASE_ADDRESS_MEM_TYPE_64);
    if (is_64bits && (bar & 1))
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_prefetch) -> else {
    else if (is_prefetch)
    ctrl =
    ROCKCHIP_PCIE_CORE_BAR_CFG_CTRL_PREFETCH_MEM_32BITS;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_64bits) -> else {
    else if (is_64bits)
...
#[no_mangle]
pub unsafe extern "C" fn rockchip_ob_region(addr: phys_addr_t) -> u32 {
...
// addr_offset = offset;
    ...
    let mut r: u32 = rockchip_ob_region(addr);
    ...
    struct rockchip_pcie_ep *ep = epc_get_drvdata(epc);
    struct rockchip_pcie *rockchip = &ep.rockchip;
    let mut mmc: u8 = order_base_2(nr_irqs);
    u32 flags;
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_get_msi(epc: *mut pci_epc, fn: u8, vfn: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_start(epc: *mut pci_epc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_stop(epc: *mut pci_epc) {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_retrain_link(rockchip: *mut rockchip_pcie) {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_link_up(rockchip: *mut rockchip_pcie) -> bool {
    static bool rockchip_pcie_ep_link_up(struct rockchip_pcie *rockchip)
    {
    let mut val: u32 = rockchip_pcie_read(rockchip, PCIE_CLIENT_BASIC_STATUS1);
    return PCIE_LINK_UP(val);
    }
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_link_training(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_perst_assert(ep: *mut rockchip_pcie_ep) {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_perst_deassert(ep: *mut rockchip_pcie_ep) {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_perst_irq_thread(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t rockchip_pcie_ep_perst_irq_thread(int irq, void *data)
    {
    struct pci_epc *epc = data;
    struct rockchip_pcie_ep *ep = epc_get_drvdata(epc);
    struct rockchip_pcie *rockchip = &ep.rockchip;
    let mut perst: u32 = gpiod_get_value(rockchip.perst_gpio);
    if (perst)
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_setup_irq(epc: *mut pci_epc) -> c_int {
...
//
    ...
    ret = devm_request_threaded_irq(dev, ep.perst_irq, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_init_ob_mem(ep: *mut rockchip_pcie_ep) -> c_int {
    static int rockchip_pcie_ep_init_ob_mem(struct rockchip_pcie_ep *ep)
    {
    struct rockchip_pcie *rockchip = &ep.rockchip;
    struct device *dev = rockchip.dev;
    struct pci_epc_mem_window *windows = core::ptr::null_mut();
    int err, i;
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_exit_ob_mem(ep: *mut rockchip_pcie_ep) {
    static void rockchip_pcie_ep_exit_ob_mem(struct rockchip_pcie_ep *ep)
    {
    pci_epc_mem_exit(ep.epc);
    }
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_hide_broken_msix_cap(rockchip: *mut rockchip_pcie) {
...
#[no_mangle]
unsafe extern "C" fn rockchip_pcie_ep_probe(pdev: *mut platform_device) -> c_int {
