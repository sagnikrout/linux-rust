...
//! Source: drivers/pci/controller/pci-xgene.c
#![no_std]
...
pub const PCIECORE_CTLANDSTATUS: c_uint = 0x50;
pub const PIM1_1L: c_uint = 0x80;
pub const IBAR2: c_uint = 0x98;
pub const IR2MSK: c_uint = 0x9c;
pub const PIM2_1L: c_uint = 0xa0;
pub const IBAR3L: c_uint = 0xb4;
pub const IR3MSKL: c_uint = 0xbc;
pub const PIM3_1L: c_uint = 0xc4;
pub const OMR1BARL: c_uint = 0x100;
pub const OMR2BARL: c_uint = 0x118;
pub const OMR3BARL: c_uint = 0x130;
pub const CFGBARL: c_uint = 0x154;
pub const CFGBARH: c_uint = 0x158;
pub const CFGCTL: c_uint = 0x15c;
pub const RTDID: c_uint = 0x160;
pub const BRIDGE_CFG_0: c_uint = 0x2000;
pub const BRIDGE_CFG_4: c_uint = 0x2010;
pub const BRIDGE_STATUS_0: c_uint = 0x2600;
pub const LINK_UP_MASK: c_uint = 0x00000100;
pub const AXI_EP_CFG_ACCESS: c_uint = 0x10000;
pub const EN_COHERENCY: c_uint = 0xF0000000;
pub const EN_REG: c_uint = 0x00000001;
pub const OB_LO_IO: c_uint = 0x00000002;
pub const XGENE_PCIE_DEVICEID: c_uint = 0xE004;

pub const XGENE_V1_PCI_EXP_CAP: c_uint = 0x40;
// PCIe IP version
pub const XGENE_PCIE_IP_VER_1: c_int = 1;
pub const XGENE_PCIE_IP_VER_2: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xgene_pcie {
    pub dev: *mut device,
    pub clk: *mut clk,
    pub csr_base: *mut void __iomem,
    pub cfg_base: *mut void __iomem,
    pub cfg_addr: c_ulong,
    pub link_up: bool,
    pub version: u32,
}

#[no_mangle]
unsafe extern "C" fn xgene_pcie_readl(port: *mut xgene_pcie, reg: u32) -> u32 {
    static u32 xgene_pcie_readl(struct xgene_pcie *port, u32 reg)
    {
    return readl(port.csr_base + reg);
    }
#[no_mangle]
unsafe extern "C" fn xgene_pcie_writel(port: *mut xgene_pcie, reg: u32, val: u32) {
    static void xgene_pcie_writel(struct xgene_pcie *port, u32 reg, u32 val)
    {
    writel(val, port.csr_base + reg);
    }
#[no_mangle]
pub unsafe extern "C" fn pcie_bar_low_val(addr: u32, flags: u32) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn xgene_pcie_set_rtdid_reg(bus: *mut pci_bus, devfn: c_uint) {
    static void xgene_pcie_set_rtdid_reg(struct pci_bus *bus, uint devfn)
    {
    struct xgene_pcie *port = pcie_bus_to_port(bus);
    unsigned int b, d, f;
    let mut rtdid_val: u32 = 0;
    b = bus.number;
    d = PCI_SLOT(devfn);
    f = PCI_FUNC(devfn);
...
//
#[no_mangle]
unsafe extern "C" fn xgene_pcie_hide_rc_bars(bus: *mut pci_bus, offset: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn xgene_pcie_ecam_init(cfg: *mut pci_config_window, ipversion: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xgene_v1_pcie_ecam_init(cfg: *mut pci_config_window) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xgene_v2_pcie_ecam_init(cfg: *mut pci_config_window) -> c_int {
    ...
    u32 flags, u64 size)
    {
    let mut mask: u64 = (~(size - 1) & PCI_BASE_ADDRESS_MEM_MASK) | flags;
    let mut val32: u32 = 0;
    u32 val;
...
#[no_mangle]
unsafe extern "C" fn xgene_pcie_init_port(port: *mut xgene_pcie) -> c_int {
    static int xgene_pcie_init_port(struct xgene_pcie *port)
    {
    struct device *dev = port.dev;
    int rc;
    port.clk = clk_get(dev, core::ptr::null_mut());
    if (IS_ERR(port.clk)) {
    ...
    let mut restype: u64 = resource_type(res);
    let mut mask: u64 = 0;
    u32 min_size;
    let mut flag: u32 = EN_REG;
...
//
#[no_mangle]
unsafe extern "C" fn xgene_pcie_select_ib_reg(ib_reg_mask: *mut u8, size: u64) -> c_int {
...
// ib_reg_mask |= (1 << 2);
    ...
    u32 pim_reg;
    let mut cpu_addr: u64 = range.cpu_addr;
    let mut pci_addr: u64 = range.pci_addr;
    let mut size: u64 = range.size;
    let mut mask: u64 = ~(size - 1) | EN_REG;
    let mut flags: u32 = PCI_BASE_ADDRESS_MEM_TYPE_64;
    u32 bar_low;
    int region;
...
#[no_mangle]
unsafe extern "C" fn xgene_pcie_parse_map_dma_ranges(port: *mut xgene_pcie) -> c_int {
...
// Get the dma-ranges from DT
    ...
    let mut end: u64 = range.cpu_addr + range.size - 1;
...
#[no_mangle]
unsafe extern "C" fn xgene_pcie_clear_config(port: *mut xgene_pcie) {
...
#[no_mangle]
unsafe extern "C" fn xgene_pcie_setup(port: *mut xgene_pcie) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xgene_check_pcie_msi_ready() -> bool {
    ...
    return true;
    np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "apm,xgene1-msi");
    if (!np)
    return true;
...
#[no_mangle]
unsafe extern "C" fn xgene_pcie_probe(pdev: *mut platform_device) -> c_int {
