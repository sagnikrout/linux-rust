...
//! Source: drivers/pci/controller/pci-mvebu.c
#![no_std]
...
//
pub const PCIE_DEV_ID_OFF: c_uint = 0x0000;
pub const PCIE_CMD_OFF: c_uint = 0x0004;
pub const PCIE_DEV_REV_OFF: c_uint = 0x0008;

pub const PCIE_SSDEV_ID_OFF: c_uint = 0x002c;
pub const PCIE_CAP_PCIEXP: c_uint = 0x0060;
pub const PCIE_CAP_PCIERR_OFF: c_uint = 0x0100;

pub const PCIE_WIN5_CTRL_OFF: c_uint = 0x1880;
pub const PCIE_WIN5_BASE_OFF: c_uint = 0x1884;
pub const PCIE_WIN5_REMAP_OFF: c_uint = 0x188c;
pub const PCIE_CONF_ADDR_OFF: c_uint = 0x18f8;
pub const PCIE_CONF_ADDR_EN: c_uint = 0x80000000;

    (PCIE_CONF_BUS(bus) | PCIE_CONF_DEV(PCI_SLOT(devfn))    | \
    PCIE_CONF_FUNC(PCI_FUNC(devfn)) | PCIE_CONF_REG(where) | \
    PCIE_CONF_ADDR_EN)
pub const PCIE_CONF_DATA_OFF: c_uint = 0x18fc;
pub const PCIE_INT_CAUSE_OFF: c_uint = 0x1900;
pub const PCIE_INT_UNMASK_OFF: c_uint = 0x1910;

pub const PCIE_CTRL_OFF: c_uint = 0x1a00;
pub const PCIE_CTRL_X1_MODE: c_uint = 0x0001;

pub const PCIE_STAT_OFF: c_uint = 0x1a04;
pub const PCIE_STAT_BUS: c_uint = 0xff00;
pub const PCIE_STAT_DEV: c_uint = 0x1f0000;

pub const PCIE_SSPL_OFF: c_uint = 0x1a0c;
pub const PCIE_SSPL_VALUE_SHIFT: c_int = 0;

pub const PCIE_SSPL_SCALE_SHIFT: c_int = 8;

pub const PCIE_RC_RTSTA: c_uint = 0x1a14;
pub const PCIE_DEBUG_CTRL: c_uint = 0x1a60;

    struct mvebu_pcie_port;
// Structure representing all PCIe interfaces
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_pcie {
    pub pdev: *mut platform_device,
    pub ports: *mut mvebu_pcie_port,
    pub io: resource,
    pub realio: resource,
    pub mem: resource,
    pub nports: c_int,
...
pub struct mvebu_pcie_port {
    ...
    pub is_x4: bool,
    pub devfn: c_int,
    pub mem_target: c_uint,
    pub mem_attr: c_uint,
    pub io_target: c_uint,
    pub io_attr: c_uint,
    pub clk: *mut clk,
    pub reset_gpio: *mut gpio_desc,
    pub reset_name: *mut c_char,
    pub bridge: pci_bridge_emul,
    pub dn: *mut device_node,
    pub pcie: *mut mvebu_pcie,
    pub memwin: mvebu_pcie_window,
    pub iowin: mvebu_pcie_window,
    pub saved_pcie_stat: u32,
    pub regs: resource,
...
#[no_mangle]
pub unsafe extern "C" fn mvebu_writel(port: *mut mvebu_pcie_port, val: u32, reg: u32) {
    static inline void mvebu_writel(struct mvebu_pcie_port *port, u32 val, u32 reg)
    {
    writel(val, port.base + reg);
    }
#[no_mangle]
pub unsafe extern "C" fn mvebu_readl(port: *mut mvebu_pcie_port, reg: u32) -> u32 {
    static inline u32 mvebu_readl(struct mvebu_pcie_port *port, u32 reg)
    {
    return readl(port.base + reg);
    }
#[no_mangle]
pub unsafe extern "C" fn mvebu_has_ioport(port: *mut mvebu_pcie_port) -> bool {
    static inline bool mvebu_has_ioport(struct mvebu_pcie_port *port)
    {
    return port.io_target != -1 && port.io_attr != -1;
    }
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_link_up(port: *mut mvebu_pcie_port) -> bool {
    static bool mvebu_pcie_link_up(struct mvebu_pcie_port *port)
    {
    return !(mvebu_readl(port, PCIE_STAT_OFF) & PCIE_STAT_LINK_DOWN);
    }
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_get_local_bus_nr(port: *mut mvebu_pcie_port) -> u8 {
    static u8 mvebu_pcie_get_local_bus_nr(struct mvebu_pcie_port *port)
    {
    return (mvebu_readl(port, PCIE_STAT_OFF) & PCIE_STAT_BUS) >> 8;
    }
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_set_local_bus_nr(port: *mut mvebu_pcie_port, nr: c_int) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_set_local_dev_nr(port: *mut mvebu_pcie_port, nr: c_int) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_disable_wins(port: *mut mvebu_pcie_port) {
...
//
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_setup_wins(port: *mut mvebu_pcie_port) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_setup_hw(port: *mut mvebu_pcie_port) {
...
//
    ...
    sz, remap);
    if (ret) {
    let mut end: phys_addr_t = base + sz - 1;
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_handle_iobase_change(port: *mut mvebu_pcie_port) -> c_int {
    static int mvebu_pcie_handle_iobase_change(struct mvebu_pcie_port *port)
    {
    let mut desired: mvebu_pcie_window = {};
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_handle_membase_change(port: *mut mvebu_pcie_port) -> c_int {
    static int mvebu_pcie_handle_membase_change(struct mvebu_pcie_port *port)
    {
    let mut desired: mvebu_pcie_window = {.remap = MVEBU_MBUS_NO_REMAP};
    struct pci_bridge_emul_conf *conf = &port.bridge.conf;
...
//
    ...
    let mut val: u32 = le32_to_cpu(cfgspace[PCI_PRIMARY_BUS / 4]);
    val &= ~0xff00;
...
// value = mvebu_readl(port, PCIE_CAP_PCIEXP + PCI_EXP_LNKCTL) |
    ...
    break;
    case PCI_EXP_SLTCTL: {
    let mut slotctl: u16 = le16_to_cpu(bridge.pcie_conf.slotctl);
    let mut slotsta: u16 = le16_to_cpu(bridge.pcie_conf.slotsta);
    let mut val: u32 = 0;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !(mvebu_readl(port, PCIE_SSPL_ENABLE): PCIE_SSPL_OFF) &) -> else {
...
// On error disable IO range
    ...
    let mut ctrl: u32 = mvebu_readl(port, PCIE_CTRL_OFF);
...
//
    ...
    let mut sspl: u32 = mvebu_readl(port, PCIE_SSPL_OFF);
...
//
#[no_mangle]
unsafe extern "C" fn mvebu_pci_bridge_emul_init(port: *mut mvebu_pcie_port) -> c_int {
    static int mvebu_pci_bridge_emul_init(struct mvebu_pcie_port *port)
    {
    let mut bridge_flags: c_uint = PCI_BRIDGE_EMUL_NO_PREFMEM_FORWARD;
    struct pci_bridge_emul *bridge = &port.bridge;
    let mut dev_id: u32 = mvebu_readl(port, PCIE_DEV_ID_OFF);
    let mut dev_rev: u32 = mvebu_readl(port, PCIE_DEV_REV_OFF);
    let mut ssdev_id: u32 = mvebu_readl(port, PCIE_SSDEV_ID_OFF);
    let mut pcie_cap: u32 = mvebu_readl(port, PCIE_CAP_PCIEXP);
    let mut pcie_cap_ver: u8 = ((pcie_cap >> 16) & PCI_EXP_FLAGS_VERS);
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_intx_irq_mask(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_intx_irq_unmask(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_init_irq_domain(port: *mut mvebu_pcie_port) -> c_int {
    static int mvebu_pcie_init_irq_domain(struct mvebu_pcie_port *port)
    {
    struct device *dev = &port.pcie.pdev.dev;
    struct device_node *pcie_intc_node;
    raw_spin_lock_init(&port.irq_lock);
    pcie_intc_node = of_get_next_child(port.dn, core::ptr::null_mut());
    if (!pcie_intc_node) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_irq_handler(desc: *mut irq_desc) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_map_irq(dev: *const pci_dev, slot: u8, pin: u8) -> c_int {
...
// attr = -1;
    ...
    let mut slot: u32 = upper_32_bits(range.bus_addr);
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_port_clk_put(data: *mut c_void) {
...
// reset gpio is optional
    port.reset_gpio = core::ptr::null_mut();
    devm_kfree(dev, port.reset_name);
    port.reset_name = core::ptr::null_mut();
    }
    ...
    port.clk = of_clk_get_by_name(child, core::ptr::null_mut());
    if (IS_ERR(port.clk)) {
...
//
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_powerup(port: *mut mvebu_pcie_port) -> c_int {
    ...
    return ret;
    if (port.reset_gpio) {
    let mut reset_udelay: u32 = PCI_PM_D3COLD_WAIT * 1000;
...
//
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_powerdown(port: *mut mvebu_pcie_port) {
...
//
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_parse_request_resources(pcie: *mut mvebu_pcie) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_probe(pdev: *mut platform_device) -> c_int {
    ...
    for (i = 0; i < pcie.nports; i++) {
    struct mvebu_pcie_port *port = &pcie.ports[i];
    let mut irq: c_int = port.intx_irq;
    child = port.dn;
    if (!child)
    continue;
...
#[no_mangle]
unsafe extern "C" fn mvebu_pcie_remove(pdev: *mut platform_device) {
...
// Clear all interrupt causes.
    mvebu_writel(port, ~PCIE_INT_ALL_MASK, PCIE_INT_CAUSE_OFF);
    if (irq > 0)
    irq_set_chained_handler_and_data(irq, core::ptr::null_mut(), core::ptr::null_mut());
// Remove IRQ domains.
    if (port.intx_irq_domain)
