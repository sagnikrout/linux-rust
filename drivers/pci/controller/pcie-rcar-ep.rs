...
//! Source: drivers/pci/controller/pcie-rcar-ep.c
#![no_std]
...
pub const RCAR_EPC_MAX_FUNCTIONS: c_int = 1;
// Structure representing the PCIe interface
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rcar_pcie_endpoint {
    pub pcie: rcar_pcie,
    pub ob_mapped_addr: *mut phys_addr_t,
    pub ob_window: *mut pci_epc_mem_window,
    pub max_functions: u8,
    pub bar_to_atu: [c_uint; MAX_NR_INBOUND_MAPS],
...
#[no_mangle]
unsafe extern "C" fn rcar_pcie_ep_hw_init(pcie: *mut rcar_pcie) {
...
//
    ...
    {
    let mut flags: c_int = epf_bar.flags | LAR_ENABLE | LAM_64BIT;
    struct rcar_pcie_endpoint *ep = epc_get_drvdata(epc);
    let mut size: u64 = 1ULL << fls64(epf_bar.size - 1);
    let mut cpu_addr: dma_addr_t = epf_bar.phys_addr;
...
// use 64-bit BARs
    ...
    struct rcar_pcie_endpoint *ep = epc_get_drvdata(epc);
    let mut bar: enum pci_barno = epf_bar.barno;
    let mut atu_index: u32 = ep.bar_to_atu[bar];
    rcar_pcie_set_inbound(&ep.pcie, 0x0, 0x0, 0x0, bar, false);
    clear_bit(atu_index, ep.ib_window_map);
    clear_bit(atu_index + 1, ep.ib_window_map);
    }
#[no_mangle]
unsafe extern "C" fn rcar_pcie_ep_set_msi(epc: *mut pci_epc, fn: u8, vfn: u8, nr_irqs: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rcar_pcie_ep_get_msi(epc: *mut pci_epc, fn: u8, vfn: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rcar_pcie_ep_start(epc: *mut pci_epc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rcar_pcie_ep_stop(epc: *mut pci_epc) {
...
#[no_mangle]
unsafe extern "C" fn rcar_pcie_ep_probe(pdev: *mut platform_device) -> c_int {
