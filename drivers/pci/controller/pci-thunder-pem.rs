...
//! Source: drivers/pci/controller/pci-thunder-pem.c
#![no_std]
...
pub const PEM_CFG_WR: c_uint = 0x28;
pub const PEM_CFG_RD: c_uint = 0x30;
//
...
//
pub const THUNDER_PCIE_ECAM_BUS_SHIFT: c_int = 24;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thunder_pem_pci {
    pub ea_entry: [u32; 3],
    pub pem_reg_base: *mut void __iomem,
}
...
//
#[no_mangle]
unsafe extern "C" fn thunder_pem_bridge_w1c_bits(where_aligned: u64) -> u32 {
    static u32 thunder_pem_bridge_w1c_bits(u64 where_aligned)
    {
    let mut w1c_bits: u32 = 0;
...
#[no_mangle]
unsafe extern "C" fn thunder_pem_bridge_w1_bits(where_aligned: u64) -> u32 {
...
// Force 64-bit addressing
    ...
    let mut where_aligned: u64 = where & ~3ull;
    let mut mask: u32 = 0;
    if (devfn != 0 || where >= 2048)
    return PCIBIOS_DEVICE_NOT_FOUND;
//
...
//
    if (mask) {
    let mut w1c_bits: u32 = thunder_pem_bridge_w1c_bits(where);
    if (w1c_bits) {
    mask &= w1c_bits;
    val &= ~mask;
    }
...
pub const PEM_RES_BASE: c_uint = 0x87e0c0000000ULL;

pub const PEM_MIN_DOM_IN_NODE: c_int = 4;
pub const PEM_MAX_DOM_IN_NODE: c_int = 10;
    ...
    struct resource *res_pem)
    {
    let mut node: c_int = acpi_get_node(root.device.handle);
    int index;
...
#[no_mangle]
unsafe extern "C" fn thunder_pem_acpi_init(cfg: *mut pci_config_window) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn thunder_pem_platform_init(cfg: *mut pci_config_window) -> c_int {
