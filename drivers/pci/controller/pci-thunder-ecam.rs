...
//! Source: drivers/pci/controller/pci-thunder-ecam.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn set_val(v: u32, where: c_int, size: c_int, val: *mut u32) {
    static void set_val(u32 v, int where, int size, u32 *val)
    {
    let mut shift: c_int = (where & 3) * 8;
...
// Entries are 16-byte aligned; bits[2,3] select word in entry
    let mut where_a: c_int = where & 0xc;
    if (where_a == 0) {
...
// Check for non type-00 header
    if (cfg_type == 0) {
    bool has_msix;
    let mut is_nic: bool = (vendor_device == 0xa01e177d);
    let mut is_tns: bool = (vendor_device == 0xa01f177d);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_tns) -> else {
    else if (is_tns)
    v = 0x30014; /* EA last in chain, 3 entries */
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_msix) -> else {
    else if (has_msix)
...
// EA entry-3. PP=4, VF_BAR4 (d), Size:3
    ...
    let mut is_rsl_bridge: bool = devfn == 0x08;
    let mut is_rad_bridge: bool = devfn == 0xa0;
    let mut is_zip_bridge: bool = devfn == 0xa8;
    let mut is_dfa_bridge: bool = devfn == 0xb0;
    let mut is_nic_bridge: bool = devfn == 0x10;
    if (where_a == 0x70) {
    addr = bus.ops.map_bus(bus, devfn, where_a);
    if (!addr)
    return PCIBIOS_DEVICE_NOT_FOUND;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_rad_bridge) -> else {
    else if (is_rad_bridge)
    v = 0x0202; /* subordinate:secondary = 2:2 */
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_zip_bridge) -> else {
    else if (is_zip_bridge)
    v = 0x0303; /* subordinate:secondary = 3:3 */
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_dfa_bridge) -> else {
