...
//! Source: drivers/pci/hotplug/acpiphp_ibm.c
#![no_std]
...
pub const FOUND_APCI: c_uint = 0x61504349;
...
//
    ...
    {
    let mut ind: c_int = 0, size;
    union apci_descriptor *ret = core::ptr::null_mut(), *des;
    char *table;
...
//
#[no_mangle]
unsafe extern "C" fn ibm_set_attention_status(slot: *mut hotplug_slot, status: u8) -> c_int {
    static int ibm_set_attention_status(struct hotplug_slot *slot, u8 status)
    {
    union acpi_object args[2];
    let mut params: acpi_object_list = { .pointer = args, .count = 2 };
    acpi_status stat;
    unsigned long long rc;
    union apci_descriptor *ibm_slot;
    let mut id: c_int = hpslot_to_sun(slot);
...
//
#[no_mangle]
unsafe extern "C" fn ibm_get_attention_status(slot: *mut hotplug_slot, status: *mut u8) -> c_int {
    static int ibm_get_attention_status(struct hotplug_slot *slot, u8 *status)
    {
    union apci_descriptor *ibm_slot;
    let mut id: c_int = hpslot_to_sun(slot);
...
//
#[no_mangle]
unsafe extern "C" fn ibm_handle_events(handle: acpi_handle, event: u32, context: *mut c_void) {
    static void ibm_handle_events(acpi_handle handle, u32 event, void *context)
    {
    let mut detail: u8 = event & 0x0f;
    let mut subevent: u8 = event & 0xf0;
...
//
#[no_mangle]
unsafe extern "C" fn ibm_get_table_from_acpi(bufp: *mut c_char) -> c_int {
    static int ibm_get_table_from_acpi(char **bufp)
    {
    union acpi_object *package;
    let mut buffer: acpi_buffer = { ACPI_ALLOCATE_BUFFER, core::ptr::null_mut() };
    acpi_status status;
    char *lbuf = core::ptr::null_mut();
    int i, size = -EIO;
    status = acpi_evaluate_object(ibm_acpi_handle, "APCI", core::ptr::null_mut(), &buffer);
...
//
    ...
    char *buffer, loff_t pos, size_t size)
    {
    let mut bytes_read: c_int = -EINVAL;
    char *table = core::ptr::null_mut();
...
//
    ...
    let mut current_status: c_ulonglong = 0;
    acpi_status status;
...
#[no_mangle]
unsafe extern "C" fn ibm_acpiphp_init() -> int __init {
    static int __init ibm_acpiphp_init(void)
    {
    let mut retval: c_int = 0;
    acpi_status status;
    ...
    ACPI_UINT32_MAX, ibm_find_acpi_device, core::ptr::null_mut(),
    &ibm_acpi_handle, core::ptr::null_mut()) != FOUND_APCI) {
    ...
    ibm_apci_table_attr.size = ibm_get_table_from_acpi(core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn ibm_acpiphp_exit() -> void __exit {
    static void __exit ibm_acpiphp_exit(void)
    {
    acpi_status status;
    struct kobject *sysdir = &pci_slots_kset.kobj;
