...
//! Source: drivers/pci/hotplug/cpci_hotplug_core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn release_slot(slot: *mut slot) {
    static void release_slot(struct slot *slot)
    {
    pci_dev_put(slot.dev);
    kfree(slot);
    }
pub const SLOT_NAME_SIZE: c_int = 6;
    int
...
//
    ...
    return extracted;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !atomic_read(&extracting)) -> else {
...
// Give userspace a chance to handle extraction
    ...
    {
    if (controller.irq)
    cpci_thread = kthread_run(event_thread, core::ptr::null_mut(), "cpci_hp_eventd");
    else
    cpci_thread = kthread_run(poll_thread, core::ptr::null_mut(), "cpci_hp_polld");
