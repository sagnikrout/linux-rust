...
//! Source: drivers/pci/hotplug/ibmphp_res.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn alloc_error_bus(curr: *mut ebda_pci_rsrc, busno: u8, flag: c_int) -> *mut bus_node  __init {
    static struct bus_node * __init alloc_error_bus(struct ebda_pci_rsrc *curr, u8 busno, int flag)
    {
    struct bus_node *newbus;
    if (!(curr) && !(flag)) {
    err("core::ptr::null_mut() pointer passed\n");
...
#[no_mangle]
unsafe extern "C" fn alloc_resources(curr: *mut ebda_pci_rsrc) -> *mut resource_node  __init {
    static struct resource_node * __init alloc_resources(struct ebda_pci_rsrc *curr)
    {
    struct resource_node *rs;
    if (!curr) {
    err("core::ptr::null_mut() passed to allocate\n");
    return core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn alloc_bus_range(new_bus: *mut bus_node, new_range: *mut range_node, curr: *mut ebda_pci_rsrc, flag: c_int, first_bus: u8) -> int __init {
    static int __init alloc_bus_range(struct bus_node **new_bus, struct range_node **new_range, struct ebda_pci_rsrc *curr, int flag, u8 first_bus)
    {
    struct bus_node *newbus;
    struct range_node *newrange;
    let mut num_ranges: u8 = 0;
    if (first_bus) {
    newbus = kzalloc_obj(struct bus_node);
    if (!newbus)
    return -ENOMEM;
...
//
#[no_mangle]
pub unsafe extern "C" fn ibmphp_rsrc_init() -> int __init {
    int __init ibmphp_rsrc_init(void)
    {
    struct ebda_pci_rsrc *curr;
    struct range_node *newrange = core::ptr::null_mut();
    struct bus_node *newbus = core::ptr::null_mut();
    struct bus_node *bus_cur;
    struct bus_node *bus_prev;
    struct resource_node *new_io = core::ptr::null_mut();
    struct resource_node *new_mem = core::ptr::null_mut();
    struct resource_node *new_pfmem = core::ptr::null_mut();
    int rc;
...
// no bus structure exists in place yet
    ...
    return rc;
    list_add_tail(&newbus.bus_list, &gbuses);
    debug("gbuses = core::ptr::null_mut(), Memory Primary Bus %x [%x - %x]\n", newbus.busno, newrange.start, newrange.end);
    } else {
...
// no bus structure exists in place yet
    ...
    return rc;
    list_add_tail(&newbus.bus_list, &gbuses);
    debug("gbuses = core::ptr::null_mut(), PFMemory Primary Bus %x [%x - %x]\n", newbus.busno, newrange.start, newrange.end);
    } else {
    bus_cur = find_bus_wprev(curr.bus_num, &bus_prev, 1);
    if (bus_cur) {
// found our bus
...
// no bus structure exists in place yet
    ...
    return rc;
    list_add_tail(&newbus.bus_list, &gbuses);
    debug("gbuses = core::ptr::null_mut(), IO Primary Bus %x [%x - %x]\n", newbus.busno, newrange.start, newrange.end);
    } else {
    bus_cur = find_bus_wprev(curr.bus_num, &bus_prev, 1);
    if (bus_cur) {
...
//
#[no_mangle]
unsafe extern "C" fn add_bus_range(type: c_int, range: *mut range_node, bus_cur: *mut bus_node) -> c_int {
    static int add_bus_range(int type, struct range_node *range, struct bus_node *bus_cur)
    {
    struct range_node *range_cur = core::ptr::null_mut();
    struct range_node *range_prev;
    let mut count: c_int = 0, i_init;
    let mut noRanges: c_int = 0;
    switch (type) {
    case MEM:
...
//
#[no_mangle]
unsafe extern "C" fn update_resources(bus_cur: *mut bus_node, type: c_int, rangeno: c_int) {
    static void update_resources(struct bus_node *bus_cur, int type, int rangeno)
    {
    struct resource_node *res = core::ptr::null_mut();
    ...
    res = res.next;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: res->nextRange) -> else {
...
#[no_mangle]
unsafe extern "C" fn fix_me(res: *mut resource_node, bus_cur: *mut bus_node, range: *mut range_node) {
...
//
#[no_mangle]
unsafe extern "C" fn fix_resources(bus_cur: *mut bus_node) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ibmphp_add_resource(res: *mut resource_node) -> c_int {
    ...
    struct bus_node *bus_cur;
    struct range_node *range_cur = core::ptr::null_mut();
    struct resource_node *res_start = core::ptr::null_mut();
    debug("%s - enter\n", __func__);
    if (!res) {
    err("core::ptr::null_mut() passed to add\n");
    return -ENODEV;
    }
    bus_cur = find_bus_wprev(res.busno, core::ptr::null_mut(), 0);
    if (!bus_cur) {
...
// no first{IO,Mem,Pfmem} on the bus, 1st IO/Mem/Pfmem resource ever
    ...
    res.nextRange = core::ptr::null_mut();
    } else {
    res_cur = res_start;
    res_prev = core::ptr::null_mut();
...
// the last resource in this range
    ...
    res_prev.nextRange = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn ibmphp_remove_resource(res: *mut resource_node) -> c_int {
    int ibmphp_remove_resource(struct resource_node *res)
    {
    struct bus_node *bus_cur;
    struct resource_node *res_cur = core::ptr::null_mut();
    struct resource_node *res_prev;
    struct resource_node *mem_cur;
    char *type = "";
    if (!res)  {
    err("resource to remove is core::ptr::null_mut()\n");
    return -ENODEV;
    }
    bus_cur = find_bus_wprev(res.busno, core::ptr::null_mut(), 0);
    if (!bus_cur) {
...
// first device to be deleted
    ...
    switch (res.type) {
    case IO:
    bus_cur.firstIO = core::ptr::null_mut();
    break;
    case MEM:
    bus_cur.firstMem = core::ptr::null_mut();
    break;
    case PFMEM:
    bus_cur.firstPFMem = core::ptr::null_mut();
    break;
    }
    }
    kfree(res_cur);
    ...
    res_prev.next = core::ptr::null_mut();
    ...
    {
    struct range_node *range = core::ptr::null_mut();
    switch (res.type) {
    case IO:
    range = bus_cur.rangeIO;
    break;
...
//
#[no_mangle]
pub unsafe extern "C" fn ibmphp_check_resource(res: *mut resource_node, bridge: u8) -> c_int {
    ...
    struct resource_node *res_prev;
    struct resource_node *res_cur = core::ptr::null_mut();
    let mut len_cur: u32 = 0, start_cur = 0, len_tmp = 0;
    let mut noranges: c_int = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn ibmphp_remove_bus(bus: *mut bus_node, parent_busno: u8) -> c_int {
    ...
    int rc;
    prev_bus = find_bus_wprev(parent_busno, core::ptr::null_mut(), 0);
    if (!prev_bus) {
    ...
    res_cur = res_cur.nextRange;
    kfree(res_tmp);
    res_tmp = core::ptr::null_mut();
    }
    bus.firstIO = core::ptr::null_mut();
    }
    if (bus.firstMem) {
    ...
    bus.firstMem = core::ptr::null_mut();
    }
    if (bus.firstPFMem) {
    ...
    bus.firstPFMem = core::ptr::null_mut();
    }
    ...
    bus.firstPFMemFromMem = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn remove_ranges(bus_cur: *mut bus_node, bus_prev: *mut bus_node) -> c_int {
    ...
    range_cur = range_cur.next;
    kfree(range_tmp);
    range_tmp = core::ptr::null_mut();
    }
    bus_cur.rangeIO = core::ptr::null_mut();
    }
    ...
    bus_cur.rangeMem = core::ptr::null_mut();
    }
    ...
    bus_cur.rangePFMem = core::ptr::null_mut();
    }
    return 0;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ibmphp_find_resource(bus: *mut bus_node, start_address: u32, res: *mut resource_node, flag: c_int) -> c_int {
    int ibmphp_find_resource(struct bus_node *bus, u32 start_address, struct resource_node **res, int flag)
    {
    struct resource_node *res_cur = core::ptr::null_mut();
    char *type = "";
    if (!bus) {
    err("The bus passed in core::ptr::null_mut() to find resource\n");
    return -ENODEV;
    }
    switch (flag) {
    case IO:
...
//
#[no_mangle]
pub unsafe extern "C" fn ibmphp_free_resources() {
    void ibmphp_free_resources(void)
    {
    struct bus_node *bus_cur = core::ptr::null_mut(), *next;
    ...
    bus_cur.firstPFMemFromMem = core::ptr::null_mut();
    }
    bus_tmp = bus_cur;
    list_del(&bus_cur.bus_list);
    kfree(bus_tmp);
    bus_tmp = core::ptr::null_mut();
    }
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn once_over() -> int __init {
    ...
    for (pfmem_cur = bus_cur.firstPFMem, pfmem_prev = core::ptr::null_mut(); pfmem_cur; pfmem_prev = pfmem_cur, pfmem_cur = pfmem_cur.next) {
    pfmem_cur.fromMem = 1;
    if (pfmem_prev)
    ...
    if (!bus_cur.firstPFMemFromMem)
    pfmem_cur.next = core::ptr::null_mut();
    else
...
#[no_mangle]
pub unsafe extern "C" fn ibmphp_add_pfmem_from_mem(pfmem: *mut resource_node) -> c_int {
    int ibmphp_add_pfmem_from_mem(struct resource_node *pfmem)
    {
    struct bus_node *bus_cur = find_bus_wprev(pfmem.busno, core::ptr::null_mut(), 0);
    if (!bus_cur) {
...
//
    ...
    {
    return find_bus_wprev(bus_number, core::ptr::null_mut(), 0);
    }
...
#[no_mangle]
pub unsafe extern "C" fn ibmphp_print_test() {
    void ibmphp_print_test(void)
    {
    let mut i: c_int = 0;
    struct bus_node *bus_cur = core::ptr::null_mut();
    struct range_node *range;
    struct resource_node *res;
    debug_pci("*****************START**********************\n");
    if ((!list_empty(&gbuses)) && flags) {
    err("The GBUSES is not core::ptr::null_mut()?!?!?!?!?\n");
    return;
    }
    list_for_each_entry(bus_cur, &gbuses, bus_list) {
...
#[no_mangle]
unsafe extern "C" fn range_exists_already(range: *mut range_node, bus_cur: *mut bus_node, type: u8) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn update_bridge_ranges(bus: *mut bus_node) -> int __init {
...
//
    ...
    bus_sec = find_bus_wprev(sec_busno, core::ptr::null_mut(), 0);
// this bus structure doesn't exist yet, PPB was configured during previous loading of ibmphp
    if (!bus_sec) {
    alloc_error_bus(core::ptr::null_mut(), sec_busno, 1);
