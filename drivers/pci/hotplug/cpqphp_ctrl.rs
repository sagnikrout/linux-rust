...
//! Source: drivers/pci/hotplug/cpqphp_ctrl.c
#![no_std]
...
//
    ...
    static struct timer_list *pushbutton_pending;	/* = core::ptr::null_mut() */
// delay is in jiffies to wait for
#[no_mangle]
unsafe extern "C" fn long_delay(delay: c_int) {
...
pub const WRONG_BUS_FREQUENCY: c_uint = 0x07;
#[no_mangle]
unsafe extern "C" fn handle_switch_change(change: u8, ctrl: *mut controller) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn handle_presence_change(change: u16, ctrl: *mut controller) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn handle_power_fault(change: u8, ctrl: *mut controller) -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn sort_by_size(head: *mut pci_resource) -> c_int {
    static int sort_by_size(struct pci_resource **head)
    {
    struct pci_resource *current_res;
    struct pci_resource *next_res;
    let mut out_of_order: c_int = 1;
    if (!(*head))
    return 1;
    if (!((*head).next))
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn sort_by_max_size(head: *mut pci_resource) -> c_int {
...
//
    ...
    {
    struct pci_resource *prevnode = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn cpqhp_resource_sort_and_combine(head: *mut pci_resource) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cpqhp_ctrl_intr(IRQ: c_int, data: *mut c_void) -> irqreturn_t {
    irqreturn_t cpqhp_ctrl_intr(int IRQ, void *data)
    {
    struct controller *ctrl = data;
    let mut schedule_flag: u8 = 0;
    u8 reset;
    u16 misc;
    u32 Diff;
...
//
    ...
    if (new_slot == core::ptr::null_mut())
    return new_slot;
    new_slot.next = core::ptr::null_mut();
    new_slot.configured = 1;
    if (cpqhp_slot_list[busnumber] == core::ptr::null_mut()) {
    cpqhp_slot_list[busnumber] = new_slot;
    } else {
    next = cpqhp_slot_list[busnumber];
    while (next.next != core::ptr::null_mut())
    next = next.next;
    next.next = new_slot;
    }
    return new_slot;
...
//
#[no_mangle]
unsafe extern "C" fn slot_remove(old_slot: *mut pci_func) -> c_int {
    static int slot_remove(struct pci_func *old_slot)
    {
    struct pci_func *next;
    if (old_slot == core::ptr::null_mut())
    return 1;
    ...
    while ((next.next != old_slot) && (next.next != core::ptr::null_mut()))
    next = next.next;
...
//
#[no_mangle]
unsafe extern "C" fn bridge_slot_remove(bridge: *mut pci_func) -> c_int {
    ...
    while ((next.next != bridge) && (next.next != core::ptr::null_mut()))
    next = next.next;
    if (next.next != bridge)
    return 2;
...
//
    ...
    if ((func == core::ptr::null_mut()) || ((func.device == device) && (index == 0)))
    return func;
    if (func.device == device)
    found++;
    while (func.next != core::ptr::null_mut()) {
    func = func.next;
    if (func.device == device)
    found++;
    if (found == index)
...
// FIXME
#[no_mangle]
unsafe extern "C" fn is_bridge(func: *mut pci_func) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn set_controller_speed(ctrl: *mut controller, adapter_speed: u8, hp_slot: u8) -> u8 {
    static u8 set_controller_speed(struct controller *ctrl, u8 adapter_speed, u8 hp_slot)
    {
    struct slot *slot;
    struct pci_bus *bus = ctrl.pci_bus;
    u8 reg;
    let mut slot_power: u8 = readb(ctrl.hpc_reg + SLOT_POWER);
    u16 reg16;
    let mut leds: u32 = readl(ctrl.hpc_reg + LED_CONTROL);
...
//
#[no_mangle]
unsafe extern "C" fn board_replaced(func: *mut pci_func, ctrl: *mut controller) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_slot_enabled(ctrl, _arg: hp_slot)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn board_added(func: *mut pci_func, ctrl: *mut controller) -> u32 {
    ...
    u8 adapter_speed;
    int index;
    let mut temp_register: u32 = 0xFFFFFFFF;
    let mut rc: u32 = 0;
    struct pci_func *new_slot = core::ptr::null_mut();
    struct pci_bus *bus = ctrl.pci_bus;
...
// All F's is an empty slot or an invalid board
    ...
    res_lists.irqs = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn remove_board(func: *mut pci_func, replace_flag: u32, ctrl: *mut controller) -> u32 {
...
// Setup slot structure with entry for empty slot
    ...
    func.switch_save = 0x10;
    func.is_a_board = 0;
    func.p_task_event = core::ptr::null_mut();
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn pushbutton_helper_thread(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn event_thread(data: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cpqhp_event_start_thread() -> c_int {
    int cpqhp_event_start_thread(void)
    {
    cpqhp_event_thread = kthread_run(event_thread, core::ptr::null_mut(), "phpd_event");
...
#[no_mangle]
pub unsafe extern "C" fn cpqhp_event_stop_thread() {
    void cpqhp_event_stop_thread(void)
    {
    kthread_stop(cpqhp_event_thread);
    }
#[no_mangle]
unsafe extern "C" fn interrupt_event_handler(ctrl: *mut controller) {
    static void interrupt_event_handler(struct controller *ctrl)
    {
    int loop;
    let mut change: c_int = 1;
    struct pci_func *func;
    u8 hp_slot;
    struct slot *p_slot;
    while (change) {
...
#[no_mangle]
pub unsafe extern "C" fn if(INT_BUTTON_RELEASE: ctrl->event_queue[loop].event_type ==) -> else {
    dbg("button release\n");
...
// POWER FAULT
#[no_mangle]
pub unsafe extern "C" fn if(INT_POWER_FAULT: ctrl->event_queue[loop].event_type ==) -> else {
    dbg("power fault\n");
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn cpqhp_pushbutton_thread(t: *mut timer_list) {
    ...
    pushbutton_pending = core::ptr::null_mut();
...
// power Down board
    ...
    if (!func) {
    dbg("Error! func core::ptr::null_mut() in %s\n", __func__);
    return;
    }
...
#[no_mangle]
pub unsafe extern "C" fn cpqhp_process_SI(ctrl: *mut controller, func: *mut pci_func) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cpqhp_process_SS(ctrl: *mut controller, func: *mut pci_func) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn cpqhp_hardware_test(ctrl: *mut controller, test_num: c_int) -> c_int {
...
//
    ...
    bus_node.base += 1;
    bus_node.length -= 1;
    bus_node.next = core::ptr::null_mut();
// If we have IO resources copy them and fill in the bridge's
// IO range registers
    memcpy(hold_IO_node, io_node, sizeof(struct pci_resource));
    io_node.next = core::ptr::null_mut();
...
//
    ...
    mem_node.next = core::ptr::null_mut();
// set Mem base and Limit registers
    ...
    p_mem_node.next = core::ptr::null_mut();
...
// set subordinate bus
    ...
    temp_resources.bus_head = core::ptr::null_mut();
    } else {
