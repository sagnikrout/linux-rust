...
//
    ...
    while ((bus = pci_find_next_bus(bus)) != core::ptr::null_mut())  {
...
#[no_mangle]
unsafe extern "C" fn match_pci_dev_by_id(dev: *mut device, data: *const c_void) -> c_int {
...
//
    ...
    {
    struct device *dev;
    struct device *dev_start = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn pci_dev_present(ids: *const pci_device_id) -> c_int {
    int pci_dev_present(const struct pci_device_id *ids)
    {
    struct pci_dev *found = core::ptr::null_mut();
    while (ids.vendor || ids.subvendor || ids.class_mask) {
    found = pci_get_dev_by_id(ids, core::ptr::null_mut());
    if (found) {
    pci_dev_put(found);
    return 1;
    }
