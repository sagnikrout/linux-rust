...
pub const PCI_ROM_HEADER_SIZE: c_uint = 0x1A;
pub const PCI_ROM_POINTER_TO_DATA_STRUCT: c_uint = 0x18;
pub const PCI_ROM_LAST_IMAGE_INDICATOR: c_uint = 0x15;

pub const PCI_ROM_IMAGE_LEN: c_uint = 0x10;

pub const PCI_ROM_IMAGE_SIGNATURE: c_uint = 0xAA55;
// Data structure signature is "PCIR" in ASCII representation
pub const PCI_ROM_DATA_STRUCT_SIGNATURE: c_uint = 0x52494350;
pub const PCI_ROM_DATA_STRUCT_LEN: c_uint = 0x0A;
//
...
//
pub const PCI_ROM_DATA_STRUCT_MIN_LEN: c_uint = 0x18;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn pci_enable_rom(pdev: *mut pci_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn pci_disable_rom(pdev: *mut pci_dev) {
    ...
    bool expect_valid)
    {
    let mut rom_end: c_ulong = (unsigned long)rom + size - 1;
...
// assign the ROM an address if it doesn't have one
    if (res.parent == core::ptr::null_mut() && pci_assign_resource(pdev, PCI_ROM_RESOURCE))
...
//
#[no_mangle]
pub unsafe extern "C" fn pci_unmap_rom(pdev: *mut pci_dev, rom: *mut void __iomem) {
