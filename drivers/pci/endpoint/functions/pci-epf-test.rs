...
//! Source: drivers/pci/endpoint/functions/pci-epf-test.c
#![no_std]
...
pub const IRQ_TYPE_INTX: c_int = 0;
pub const IRQ_TYPE_MSI: c_int = 1;
pub const IRQ_TYPE_MSIX: c_int = 2;

pub const TIMER_RESOLUTION: c_int = 1;

pub const PCI_EPF_TEST_BAR_SUBRANGE_NSUB: c_int = 2;
    static struct workqueue_struct *kpcitest_workqueue;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pci_epf_test {
    pub reg: [*mut c_void; PCI_STD_NUM_BARS],
    pub epf: *mut pci_epf,
    pub group: config_group,
    pub test_reg_bar: enum pci_barno,
    pub msix_table_offset: usize,
    pub cmd_handler: delayed_work,
    pub dma_chan_tx: *mut dma_chan,
    pub dma_chan_rx: *mut dma_chan,
    pub transfer_chan: *mut dma_chan,
    pub transfer_cookie: dma_cookie_t,
    pub transfer_status: enum dma_status,
    pub transfer_complete: completion,
    pub dma_supported: bool,
    pub dma_private: bool,
    pub epc_features: *const pci_epc_features,
    pub db_bar: pci_epf_bar,
    pub db_bar_programmed: bool,
    pub bar_size: [usize; PCI_STD_NUM_BARS],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct pci_epf_test_reg {
    pub magic: __le32,
    pub command: __le32,
    pub status: __le32,
    pub src_addr: __le64,
    ...
    pub checksum: __le32,
    pub irq_type: __le32,
    pub irq_number: __le32,
    pub flags: __le32,
    pub caps: __le32,
    pub doorbell_bar: __le32,
    pub doorbell_offset: __le32,
    pub doorbell_data: __le32,
    pub __packed: },
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_dma_callback(param: *mut c_void) {
...
//
    ...
    let mut dma_local: dma_addr_t = (dir == DMA_MEM_TO_DEV) ? dma_src : dma_dst;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct epf_dma_filter {
    pub dev: *mut device,
    pub dma_mask: u32,
}

#[no_mangle]
unsafe extern "C" fn epf_dma_filter_fn(chan: *mut dma_chan, node: *mut c_void) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn pci_epf_test_init_dma_chan(epf_test: *mut pci_epf_test) -> c_int {
    ...
    epf_test.dma_chan_rx = core::ptr::null_mut();
    fail_back_tx:
    dma_cap_zero(mask);
...
//
#[no_mangle]
unsafe extern "C" fn pci_epf_test_clean_dma_chan(epf_test: *mut pci_epf_test) {
    ...
    epf_test.dma_chan_tx = core::ptr::null_mut();
    epf_test.dma_chan_rx = core::ptr::null_mut();
    return;
    }
    epf_test.dma_chan_tx = core::ptr::null_mut();
    ...
    {
    let mut ts: timespec64 = timespec64_sub(*end, *start);
    let mut rate: u64 = 0, ns;
// calculate the rate
    ...
    let mut src_addr: u64 = le64_to_cpu(reg.src_addr);
    let mut dst_addr: u64 = le64_to_cpu(reg.dst_addr);
    size_t orig_size, copy_size;
    let mut map_size: isize = 0;
    let mut flags: u32 = le32_to_cpu(reg.flags);
    let mut status: u32 = 0;
    void *copy_buf = core::ptr::null_mut(), *buf;
    ...
    let mut map_size: isize = 0;
    let mut flags: u32 = le32_to_cpu(reg.flags);
    let mut checksum: u32 = le32_to_cpu(reg.checksum);
    let mut status: u32 = 0;
...
//
    ...
    struct device *dev = &epf.dev;
    struct pci_epc *epc = epf.epc;
    let mut status: u32 = le32_to_cpu(reg.status);
    let mut irq_number: u32 = le32_to_cpu(reg.irq_number);
    let mut irq_type: u32 = le32_to_cpu(reg.irq_type);
    int count;
//
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_doorbell_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t pci_epf_test_doorbell_handler(int irq, void *data)
    {
    struct pci_epf_test *epf_test = data;
    let mut test_reg_bar: enum pci_barno = epf_test.test_reg_bar;
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_doorbell_cleanup(epf_test: *mut pci_epf_test) {
...
//
    ...
    ret = request_threaded_irq(epf.db_msg[0].virq, core::ptr::null_mut(),
    ...
    struct pci_epf_test_reg *reg)
    {
    let mut bar: enum pci_barno = le32_to_cpu(reg.doorbell_bar);
    let mut status: u32 = le32_to_cpu(reg.status);
    struct pci_epf *epf = epf_test.epf;
    struct pci_epc *epc = epf.epc;
    int ret;
...
//
    ...
    let mut nsub: c_uint = PCI_EPF_TEST_BAR_SUBRANGE_NSUB, old_nsub;
// reg->size carries BAR number for BAR_SUBRANGE_* commands.
    let mut barno: enum pci_barno = le32_to_cpu(reg.size);
    let mut status: u32 = le32_to_cpu(reg.status);
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_cmd_handler(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_set_bar(epf: *mut pci_epf) -> c_int {
    ...
    PRIMARY_INTERFACE);
    epf_test.reg[bar] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_clear_bar(epf: *mut pci_epf) {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_set_capabilities(epf: *mut pci_epf) {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_epc_init(epf: *mut pci_epf) -> c_int {
    ...
    struct pci_epc *epc = epf.epc;
    struct device *dev = &epf.dev;
    let mut linkup_notifier: bool = false;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_epc_deinit(epf: *mut pci_epf) {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_link_up(epf: *mut pci_epf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_link_down(epf: *mut pci_epf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_alloc_space(epf: *mut pci_epf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_free_space(epf: *mut pci_epf) {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_bind(epf: *mut pci_epf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_unbind(epf: *mut pci_epf) {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn pci_epf_test_exit() -> void __exit {
