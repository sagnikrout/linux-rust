...
pub const EDR_PORT_DPC_ENABLE_DSM: c_uint = 0x0C;
pub const EDR_PORT_LOCATE_DSM: c_uint = 0x0D;
pub const EDR_OST_SUCCESS: c_uint = 0x80;
pub const EDR_OST_FAILED: c_uint = 0x81;
//
...
//
#[no_mangle]
unsafe extern "C" fn acpi_enable_dpc(pdev: *mut pci_dev) -> c_int {
...
//
    ...
    EDR_PORT_LOCATE_DSM, core::ptr::null_mut());
    if (!obj)
    return pci_dev_get(pdev);
    if (obj.type != ACPI_TYPE_INTEGER) {
    ACPI_FREE(obj);
...
//
    ...
    ost_status, core::ptr::null_mut());
    if (ACPI_FAILURE(status))
    return -EINVAL;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn edr_handle_event(handle: acpi_handle, event: u32, data: *mut c_void) {
    static void edr_handle_event(acpi_handle handle, u32 event, void *data)
    {
    struct pci_dev *pdev = data, *edev;
    let mut estate: pci_ers_result_t = PCI_ERS_RESULT_DISCONNECT;
    u16 status;
...
#[no_mangle]
pub unsafe extern "C" fn pci_acpi_add_edr_notifier(pdev: *mut pci_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn pci_acpi_remove_edr_notifier(pdev: *mut pci_dev) {
