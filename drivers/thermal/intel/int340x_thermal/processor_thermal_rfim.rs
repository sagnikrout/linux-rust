...
//! Source: drivers/thermal/intel/int340x_thermal/processor_thermal_rfim.c
#![no_std]
...
// These will represent sysfs attribute names
    ...
    {
    let mut attr_matched: bool = false;
    let mut i: c_int = 0;
    if (!table)
    return -EOPNOTSUPP;
...
// If attribute name is matched, then the user space value is invalid
    ...
    {
    return match_mapping_table(table, attr_name, true, value, core::ptr::null_mut(), result, core::ptr::null_mut());
    }
    static int get_mapped_value(const struct mapping_table *table, const char *attr_name,
    const char *value, unsigned int *result)
    {
    return match_mapping_table(table, attr_name, false, 0, value, core::ptr::null_mut(), result);
    }
// These will represent sysfs attribute names
...
// ddr_data_rate
    let mut nvl_ddr_data_rate_reg: static struct mmio_reg = { 1, 0xE0, 10, 0x3FF, 2};
...
#[no_mangle]
pub unsafe extern "C" fn proc_thermal_rfim_add(pdev: *mut pci_dev, proc_priv: *mut proc_thermal_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn proc_thermal_rfim_remove(pdev: *mut pci_dev) {
