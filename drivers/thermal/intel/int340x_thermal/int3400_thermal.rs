...
//! Source: drivers/thermal/intel/int340x_thermal/int3400_thermal.c
#![no_std]
...
pub const INT3400_THERMAL_TABLE_CHANGED: c_uint = 0x83;
pub const INT3400_ODVP_CHANGED: c_uint = 0x88;
pub const INT3400_KEEP_ALIVE: c_uint = 0xA0;

// UUID prefix length for comparison - sufficient for all UUIDs
pub const INT3400_UUID_PREFIX_LEN: c_int = 7;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct int3400_thermal_priv {
    ...
    pub trts: *mut trt,
    pub uuid_bitmap: u32,
    pub rel_misc_dev_res: c_int,
    pub current_uuid_index: c_int,
    pub data_vault: *mut c_char,
    pub odvp_count: c_int,
    pub odvp: *mut c_int,
    pub os_uuid_mask: u32,
    pub production_mode: c_int,
    pub odvp_attrs: *mut odvp_attr,
}

    static int evaluate_odvp(struct int3400_thermal_priv *priv);
#[repr(C)]
#[derive(Copy, Clone)]
pub struct odvp_attr {
    pub odvp: c_int,
    pub priv: *mut int3400_thermal_priv,
...
#[no_mangle]
unsafe extern "C" fn int3400_thermal_run_osc(handle: acpi_handle, uuid_str: *mut c_char, enable: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_os_uuid_mask(priv: *mut int3400_thermal_priv, mask: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn int3400_thermal_get_uuids(priv: *mut int3400_thermal_priv) -> c_int {
    ...
    let mut result: c_int = 0;
    acpi_status status;
    status = acpi_evaluate_object(priv.adev.handle, "IDSP", core::ptr::null_mut(), &buf);
    if (ACPI_FAILURE(status))
    return -ENODEV;
...
#[no_mangle]
unsafe extern "C" fn production_mode_init(priv: *mut int3400_thermal_priv) -> c_int {
    ...
    status = acpi_evaluate_integer(priv.adev.handle, "DCFG", core::ptr::null_mut(), &mode);
...
#[no_mangle]
unsafe extern "C" fn production_mode_exit(priv: *mut int3400_thermal_priv) {
...
#[no_mangle]
unsafe extern "C" fn cleanup_odvp(priv: *mut int3400_thermal_priv) {
    ...
    priv.odvp_attrs = core::ptr::null_mut();
    }
    kfree(priv.odvp);
    priv.odvp = core::ptr::null_mut();
    priv.odvp_count = 0;
    }
#[no_mangle]
unsafe extern "C" fn evaluate_odvp(priv: *mut int3400_thermal_priv) -> c_int {
    static int evaluate_odvp(struct int3400_thermal_priv *priv)
    {
    let mut odvp: acpi_buffer = { ACPI_ALLOCATE_BUFFER, core::ptr::null_mut() };
    union acpi_object *obj = core::ptr::null_mut();
    acpi_status status;
    int i, ret;
    status = acpi_evaluate_object(priv.adev.handle, "ODVP", core::ptr::null_mut(), &odvp);
    if (ACPI_FAILURE(status)) {
    ret = -EINVAL;
    goto out_err;
    }
    ...
    if (priv.odvp == core::ptr::null_mut()) {
    ...
    if (priv.odvp_attrs == core::ptr::null_mut()) {
    ...
    odvp.attr.store = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn int3400_setup_gddv(priv: *mut int3400_thermal_priv) {
    static void int3400_setup_gddv(struct int3400_thermal_priv *priv)
    {
    let mut buffer: acpi_buffer = { ACPI_ALLOCATE_BUFFER, core::ptr::null_mut() };
    union acpi_object *obj;
    acpi_status status;
    status = acpi_evaluate_object(priv.adev.handle, "GDDV", core::ptr::null_mut(),
    &buffer);
...
#[no_mangle]
unsafe extern "C" fn int3400_thermal_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn int3400_thermal_remove(pdev: *mut platform_device) {
