...
//! Source: drivers/acpi/mipi-disco-img.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn acpi_mipi_data_tag(handle: acpi_handle, context: *mut c_void) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct crs_csi2_connection {
    pub entry: list_head,
    pub csi2_data: acpi_resource_csi2_serialbus,
    pub remote_handle: acpi_handle,
    pub remote_name: [c_char; ],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct csi2_resources_walk_data {
    pub handle: acpi_handle,
    pub connections: list_head,
}
...
unsafe extern "C" fn parse_csi2_resource(res: *mut acpi_resource, context: *mut c_void) -> acpi_status {
    ...
    if (ACPI_FAILURE(acpi_get_handle(core::ptr::null_mut(), csi2_res_src.string_ptr,
    &remote_handle))) {
    ...
    (void **)&csi2, core::ptr::null_mut())))
    return core::ptr::null_mut();
    return csi2;
    }
#[no_mangle]
unsafe extern "C" fn csi_csr2_release_connections(list: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn acpi_mipi_del_crs_csi2(csi2: *mut crs_csi2) {
...
#[no_mangle]
unsafe extern "C" fn alloc_crs_csi2_swnodes(csi2: *mut crs_csi2) {
    static void alloc_crs_csi2_swnodes(struct crs_csi2 *csi2)
    {
    let mut port_count: usize = csi2.port_count;
...
//
    ...
    swnodes.nodeptrs[i] = core::ptr::null_mut();
    for (i = 0; i < port_count; i++)
    swnodes.ports[i].port_nr = NO_CSI2_PORT;
    csi2.swnodes = swnodes;
    }
pub const ACPI_CRS_CSI2_PHY_TYPE_C: c_int = 0;
pub const ACPI_CRS_CSI2_PHY_TYPE_D: c_int = 1;
...
//
    ...
    {
    let mut ep_prop_index: c_uint = ACPI_DEVICE_SWNODE_EP_CLOCK_LANES;
    let mut handle: acpi_handle = acpi_device_handle(adev);
    u8 val[ACPI_DEVICE_CSI2_DATA_LANES];
    let mut num_lanes: c_int = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn init_crs_csi2_swnodes(csi2: *mut crs_csi2) {
    static void init_crs_csi2_swnodes(struct crs_csi2 *csi2)
    {
    let mut buffer: acpi_buffer = { .length = ACPI_ALLOCATE_BUFFER };
    struct acpi_device_software_nodes *swnodes = csi2.swnodes;
    let mut handle: acpi_handle = csi2.handle;
    let mut prop_index: c_uint = 0;
...
//
    ...
    swnodes.nodes[ACPI_DEVICE_SWNODE_ROOT] =
    SOFTWARE_NODE(buffer.pointer, swnodes.dev_props, core::ptr::null_mut());
    for (i = 0; i < swnodes.num_ports; i++) {
    struct acpi_device_software_node_port *port = &swnodes.ports[i];
    struct fwnode_handle *port_fwnode;
//
...
//
    csi2.swnodes = core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_graph_ignore_port(handle: acpi_handle) -> bool {
    bool acpi_graph_ignore_port(acpi_handle handle)
    {
    const char *path = core::ptr::null_mut(), *orig_path;
