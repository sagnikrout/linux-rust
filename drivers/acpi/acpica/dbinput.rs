...
//! Source: drivers/acpi/acpica/dbinput.c
#![no_std]
...
pub const CMD_FIRST_VALID: c_int = 2;
...
//
#[no_mangle]
unsafe extern "C" fn acpi_db_display_command_info(command: *const c_char, display_all: u8) {
    static void acpi_db_display_command_info(const char *command, u8 display_all)
    {
...
//
#[no_mangle]
unsafe extern "C" fn acpi_db_display_help(command: *mut c_char) {
    static void acpi_db_display_help(char *command)
    {
...
//
    ...
    char *start;
    u32 depth;
    let mut type: acpi_object_type = ACPI_TYPE_INTEGER;
// At end of buffer?
    if (!string || !(*string)) {
...
//
#[no_mangle]
unsafe extern "C" fn acpi_db_get_line(input_buffer: *mut c_char) -> u32 {
    static u32 acpi_db_get_line(char *input_buffer)
    {
    u32 i;
    u32 count;
...
//
#[no_mangle]
unsafe extern "C" fn acpi_db_match_command(user_command: *mut c_char) -> u32 {
    static u32 acpi_db_match_command(char *user_command)
    {
    u32 i;
...
//
    ...
    u32 param_count;
    char *command_line;
    let mut status: acpi_status = AE_CTRL_TRUE;
...
// Decode and dispatch the command
    ...
    acpi_ut_dump_allocations((u32)-1, core::ptr::null_mut());
    ...
    command_line = acpi_db_get_from_history(core::ptr::null_mut());
    if (!command_line) {
    return (AE_CTRL_TRUE);
    }
    status = acpi_db_command_dispatch(command_line, walk_state, op);
    ...
    acpi_gbl_db_console_debug_level =
    strtoul(acpi_gbl_db_args[1], core::ptr::null_mut(), 16);
    acpi_os_printf
    ...
    break;
    case CMD_NOTIFY:
    temp = strtoul(acpi_gbl_db_args[2], core::ptr::null_mut(), 0);
...
// File I/O commands.
    ...
    break;
    case CMD_LOAD:{
    struct acpi_new_table_desc *list_head = core::ptr::null_mut();
    status =
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_db_execute_thread(context: *mut c_void) -> void ACPI_SYSTEM_XFACE {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_db_user_commands() -> acpi_status {
...
// Just call to the command line interpreter
    ...
    (void)acpi_db_command_dispatch(acpi_gbl_db_line_buf, core::ptr::null_mut(),
    core::ptr::null_mut());
