...
//! Source: drivers/acpi/acpica/dbtest.c
#![no_std]
...
pub const CMD_TEST_OBJECTS: c_int = 0;
pub const CMD_TEST_PREDEFINED: c_int = 1;
pub const BUFFER_FILL_VALUE: c_uint = 0xFF;
//
...
//
    ...
    let mut read_handle: static acpi_handle = core::ptr::null_mut();
    let mut write_handle: static acpi_handle = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_db_execute_test(type_arg: *mut c_char) {
    ...
    acpi_db_evaluate_all_predefined_names(core::ptr::null_mut());
    break;
    default:
    break;
    }
...
//
#[no_mangle]
unsafe extern "C" fn acpi_db_test_all_objects() {
    static void acpi_db_test_all_objects(void)
    {
    acpi_status status;
// Install the debugger read-object control method if necessary
    ...
    status =
    acpi_get_handle(core::ptr::null_mut(), ACPI_DB_READ_METHOD, &read_handle);
    if (ACPI_FAILURE(status)) {
    acpi_os_printf
...
// Install the debugger write-object control method if necessary
    ...
    status =
    acpi_get_handle(core::ptr::null_mut(), ACPI_DB_WRITE_METHOD, &write_handle);
    if (ACPI_FAILURE(status)) {
    acpi_os_printf
    ("Could not obtain handle for debug method %s\n",
...
//
    ...
    acpi_db_test_integer_type(struct acpi_namespace_node *node, u32 bit_length)
    {
    union acpi_object *temp1 = core::ptr::null_mut();
    union acpi_object *temp2 = core::ptr::null_mut();
    union acpi_object *temp3 = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn acpi_db_test_package_type(node: *mut acpi_namespace_node) -> acpi_status {
    static acpi_status acpi_db_test_package_type(struct acpi_namespace_node *node)
    {
    union acpi_object *temp1 = core::ptr::null_mut();
    acpi_status status;
...
// Exercise read-then-write
    status =
    acpi_ex_read_data_from_field(core::ptr::null_mut(), obj_desc,
    &ret_buffer_desc);
    if (status == AE_OK) {
...
//
    ...
    status = acpi_evaluate_object(read_handle, core::ptr::null_mut(),
...
// Copy the incoming user parameter
    ...
    param_objects.pointer = params;
    acpi_gbl_method_executing = TRUE;
    status = acpi_evaluate_object(write_handle, core::ptr::null_mut(), &param_objects, core::ptr::null_mut());
    acpi_gbl_method_executing = FALSE;
    if (ACPI_FAILURE(status)) {
...
//
#[no_mangle]
unsafe extern "C" fn acpi_db_evaluate_all_predefined_names(count_arg: *mut c_char) {
    static void acpi_db_evaluate_all_predefined_names(char *count_arg)
    {
    ...
    if (count_arg) {
    info.max_count = strtoul(count_arg, core::ptr::null_mut(), 0);
    }
// Search all nodes in namespace
    (void)acpi_walk_namespace(ACPI_TYPE_ANY, ACPI_ROOT_OBJECT,
    ACPI_UINT32_MAX,
    acpi_db_evaluate_one_predefined_name, core::ptr::null_mut(),
...
// Get the object info for number of method parameters
    ...
    param_objects.pointer = core::ptr::null_mut();
...
//
    ...
    break;
    case ACPI_TYPE_PACKAGE:
    this_param.package.elements = core::ptr::null_mut();
    ...
    ACPI_FREE(obj_info);
    return_obj.pointer = core::ptr::null_mut();
    return_obj.length = ACPI_ALLOCATE_BUFFER;
// Do the actual method execution
    acpi_gbl_method_executing = TRUE;
    status = acpi_evaluate_object(node, core::ptr::null_mut(), &param_objects, &return_obj);
