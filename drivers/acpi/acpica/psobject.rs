...
//! Source: drivers/acpi/acpica/psobject.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn acpi_ps_get_aml_opcode(walk_state: *mut acpi_walk_state) -> acpi_status {
    static acpi_status acpi_ps_get_aml_opcode(struct acpi_walk_state *walk_state)
    {
...
//
    ...
    union acpi_parse_object *arg = core::ptr::null_mut();
...
// are there any inline comments associated with the name_seg?? If so, save this.
    ...
    if (acpi_gbl_current_inline_comment != core::ptr::null_mut()) {
    unnamed_op.common.name_comment =
    acpi_gbl_current_inline_comment;
    acpi_gbl_current_inline_comment = core::ptr::null_mut();
    }
...
// save any comments that might be associated with unnamed_op.
    ...
    unnamed_op.common.inline_comment = core::ptr::null_mut();
    unnamed_op.common.end_node_comment = core::ptr::null_mut();
    unnamed_op.common.close_brace_comment = core::ptr::null_mut();
    unnamed_op.common.name_comment = core::ptr::null_mut();
    unnamed_op.common.comment_list = core::ptr::null_mut();
    unnamed_op.common.end_blk_comment = core::ptr::null_mut();
...
//
    ...
    let mut status: acpi_status = AE_OK;
    union acpi_parse_object *op;
    union acpi_parse_object *named_op = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn if(AML_INCREMENT_OP: (parent_scope->common.aml_opcode ==) -> else {
    ...
    if (walk_state.descending_callback != core::ptr::null_mut()) {
//
// Find the object. This will either insert the object into
// the namespace or simply look it up
//
...
// We are about to transfer to a called method
    walk_state.prev_op = core::ptr::null_mut();
...
//
    ...
    (*op).common.node = core::ptr::null_mut();
...
//
    ...
    union acpi_parse_object *op, acpi_status status)
    {
    let mut return_status: acpi_status = status;
    let mut ascending: u8 = TRUE;
...
//
    ...
    do {
    if (op) {
    if (ascending && walk_state.ascending_callback != core::ptr::null_mut()) {
    walk_state.op = op;
    walk_state.op_info =
