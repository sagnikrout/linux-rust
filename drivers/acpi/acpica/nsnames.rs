...
//! Source: drivers/acpi/acpica/nsnames.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_ns_get_pathname_length(node: *mut acpi_namespace_node) -> acpi_size {
...
// Validate the Node
    ...
    size = acpi_ns_build_normalized_path(node, core::ptr::null_mut(), 0, FALSE);
    return (size);
    }
//
...
// Determine size required for the caller buffer
    required_size =
    acpi_ns_build_normalized_path(node, core::ptr::null_mut(), 0, no_trailing);
    if (!required_size) {
    return_ACPI_STATUS(AE_BAD_PARAMETER);
    }
// Validate/Allocate/Clear caller buffer
...
// Calculate required buffer size based on depth below root
    size = acpi_ns_build_normalized_path(node, core::ptr::null_mut(), 0, no_trailing);
    if (!size) {
...
//
    ...
    char *external_path = core::ptr::null_mut();
    char *prefix_path = core::ptr::null_mut();
...
// If there is a prefix, get the pathname to it
    ...
    core::ptr::null_mut(), &external_path);
    if (ACPI_FAILURE(status)) {
    goto cleanup;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_ns_normalize_pathname(original_path: *mut c_char) {
