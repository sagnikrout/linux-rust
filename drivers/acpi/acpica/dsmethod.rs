...
//! Source: drivers/acpi/acpica/dsmethod.c
#![no_std]
...
//
    ...
    {
    acpi_status status;
    union acpi_parse_object *op = core::ptr::null_mut();
...
// Create and initialize a new walk state
    walk_state =
    acpi_ds_create_walk_state(node.owner_id, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    if (!walk_state) {
    acpi_ps_free_op(op);
    return_ACPI_STATUS(AE_NO_MEMORY);
    }
    ...
    obj_desc.method.aml_length, core::ptr::null_mut(), 0);
...
//
    ...
    struct acpi_walk_state *next_walk_state = core::ptr::null_mut();
    union acpi_operand_object *obj_desc;
...
#[no_mangle]
pub unsafe extern "C" fn if(obj_desc->method.param_count: this_walk_state->num_operands >) -> else {
...
// Begin method parse/execution. Create a new walk state
    next_walk_state =
    acpi_ds_create_walk_state(obj_desc.method.owner_id, core::ptr::null_mut(), obj_desc,
    thread);
    if (!next_walk_state) {
    status = AE_NO_MEMORY;
    goto cleanup;
...
//
    this_walk_state.operands[this_walk_state.num_operands] = core::ptr::null_mut();
//
...
//
    ...
    status = acpi_ds_init_aml_walk(next_walk_state, core::ptr::null_mut(), method_node,
    obj_desc.method.aml_start,
