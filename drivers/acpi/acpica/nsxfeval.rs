...
//! Source: drivers/acpi/acpica/nsxfeval.c
#![no_std]
...
//
    ...
    {
    acpi_status status;
    let mut free_buffer_on_error: u8 = FALSE;
...
// Evaluate the object
    status = acpi_evaluate_object(target_handle, core::ptr::null_mut(), external_params,
    return_buffer);
    if (ACPI_FAILURE(status)) {
    goto exit;
    }
...
//
    ...
    "Both Handle and Pathname are core::ptr::null_mut()"));
    } else {
    ACPI_DEBUG_PRINT((ACPI_DB_INFO,
...
// Convert each external object in the list to an internal object
    ...
    info.parameters[info.param_count] = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(info->param_count: info->obj_desc->method.param_count <) -> else {
...
//
    status = AE_TYPE;
    info.return_object = core::ptr::null_mut();	/* No need to delete a NS Node */
    return_buffer.length = 0;
    }
    if (ACPI_FAILURE(status)) {
    goto cleanup_return_object;
...
//
#[no_mangle]
unsafe extern "C" fn acpi_ns_resolve_references(info: *mut acpi_evaluate_info) {
    static void acpi_ns_resolve_references(struct acpi_evaluate_info *info)
    {
    union acpi_operand_object *obj_desc = core::ptr::null_mut();
...
//
    ...
    acpi_ns_get_device_callback, core::ptr::null_mut(),
    &info, return_value);
    (void)acpi_ut_release_mutex(ACPI_MTX_NAMESPACE);
    return_ACPI_STATUS(status);
    }
...
//
    ...
    {
    return acpi_get_data_full(obj_handle, handler, data, core::ptr::null_mut());
    }
