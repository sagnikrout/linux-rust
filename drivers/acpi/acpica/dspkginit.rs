...
//! Source: drivers/acpi/acpica/dspkginit.c
#![no_std]
...
// Element must be a reference object of correct type
    ...
    core::ptr::null_mut(), &resolved_node);
...
// element_ptr = NULL;
    ...
    aml, core::ptr::null_mut(), &external_path);
...
//
    ...
    status = acpi_ex_resolve_node_to_value(&resolved_node, core::ptr::null_mut());
    if (ACPI_FAILURE(status)) {
    return_VOID;
    }
    switch (type) {
