...
#[no_mangle]
unsafe extern "C" fn acpi_ged_irq_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t acpi_ged_irq_handler(int irq, void *data)
    {
    struct acpi_ged_event *event = data;
    acpi_status acpi_ret;
    acpi_ret = acpi_execute_simple_method(event.handle, core::ptr::null_mut(), event.gsi);
    ...
    unsigned int irq;
    unsigned int gsi;
    let mut irqflags: c_uint = IRQF_ONESHOT;
    ...
    if (request_threaded_irq(irq, core::ptr::null_mut(), acpi_ged_irq_handler,
...
#[no_mangle]
unsafe extern "C" fn ged_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ged_remove(pdev: *mut platform_device) {
