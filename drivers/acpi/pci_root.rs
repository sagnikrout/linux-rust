...
//! Source: drivers/acpi/pci_root.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn acpi_pci_root_scan_dependent(adev: *mut acpi_device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_is_root_bridge(handle: acpi_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn decode_osc_support(root: *mut acpi_pci_root, msg: *mut c_char, word: u32) {
...
#[no_mangle]
unsafe extern "C" fn decode_osc_control(root: *mut acpi_pci_root, msg: *mut c_char, word: u32) {
...
#[no_mangle]
unsafe extern "C" fn decode_cxl_osc_support(root: *mut acpi_pci_root, msg: *mut c_char, word: u32) {
...
#[no_mangle]
unsafe extern "C" fn decode_cxl_osc_control(root: *mut acpi_pci_root, msg: *mut c_char, word: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn is_pcie(root: *mut acpi_pci_root) -> bool {
    static inline bool is_pcie(struct acpi_pci_root *root)
    {
    return root.bridge_type == ACPI_BRIDGE_TYPE_PCIE;
    }
#[no_mangle]
pub unsafe extern "C" fn is_cxl(root: *mut acpi_pci_root) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cap_length(root: *mut acpi_pci_root) -> c_int {
...
//
    ...
    u32 cxl_support)
    {
    let mut req: u32 = OSC_PCI_EXPRESS_CAPABILITY_CONTROL;
    struct acpi_pci_root *root;
    acpi_status status;
...
// Need to check the available controls bits before requesting them.
    do {
    let mut pci_missing: u32 = 0, cxl_missing = 0;
...
#[no_mangle]
unsafe extern "C" fn calculate_support() -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn calculate_cxl_support() -> u32 {
...
#[no_mangle]
unsafe extern "C" fn calculate_control() -> u32 {
...
#[no_mangle]
unsafe extern "C" fn calculate_cxl_control() -> u32 {
...
#[no_mangle]
unsafe extern "C" fn os_control_query_checks(root: *mut acpi_pci_root, support: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn negotiate_os_control(root: *mut acpi_pci_root, no_aspm: *mut c_int) {
    static void negotiate_os_control(struct acpi_pci_root *root, int *no_aspm)
    {
    u32 support, control = 0, requested = 0;
    let mut cxl_support: u32 = 0, cxl_control = 0, cxl_requested = 0;
    acpi_status status;
    struct acpi_device *device = root.device;
...
// no_aspm = 1;
    ...
    struct acpi_pci_root *root;
    let mut handle: acpi_handle = device.handle;
    let mut no_aspm: c_int = 0;
    let mut hotadd: bool = system_state == SYSTEM_RUNNING;
    const char *acpi_hid;
    root = kzalloc_obj(struct acpi_pci_root);
    if (!root)
    return -ENOMEM;
    segment = 0;
    status = acpi_evaluate_integer(handle, METHOD_NAME__SEG, core::ptr::null_mut(),
    &segment);
...
//
    ...
    core::ptr::null_mut(), &bus);
    if (ACPI_SUCCESS(status))
    root.secondary.start = bus;
#[no_mangle]
pub unsafe extern "C" fn if(AE_NOT_FOUND: status ==) -> else {
    ...
    root.bridge_type = ACPI_BRIDGE_TYPE_PCIE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(acpi_hid, 0: "ACPI0016") ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn acpi_pci_root_remove(device: *mut acpi_device) {
    static void acpi_pci_root_remove(struct acpi_device *device)
    {
...
//
    ...
    {
    LIST_HEAD(list);
    struct resource *res1, *res2, *root = core::ptr::null_mut();
...
//
    ...
    let mut cpu_addr: resource_size_t = res.start;
    let mut pci_addr: resource_size_t = cpu_addr - entry.offset;
    let mut length: resource_size_t = resource_size(res);
    unsigned long port;
...
#[no_mangle]
pub unsafe extern "C" fn acpi_pci_probe_root_resources(info: *mut acpi_pci_root_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pci_acpi_root_add_resources(info: *mut acpi_pci_root_info) {
    static void pci_acpi_root_add_resources(struct acpi_pci_root_info *info)
    {
    struct resource_entry *entry, *tmp;
    struct resource *res, *conflict, *root = core::ptr::null_mut();
    resource_list_for_each_entry_safe(entry, tmp, &info.resources) {
    res = entry.res;
    if (res.flags & IORESOURCE_MEM)
    root = &iomem_resource;
#[no_mangle]
pub unsafe extern "C" fn if(IORESOURCE_IO: res->flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn __acpi_pci_root_release_info(info: *mut acpi_pci_root_info) {
...
#[no_mangle]
unsafe extern "C" fn acpi_pci_root_release_info(bridge: *mut pci_host_bridge) {
    ...
    struct acpi_device *device = root.device;
    let mut node: c_int = acpi_get_node(device.handle);
    struct pci_bus *bus;
    ...
    bus = pci_create_root_bus(core::ptr::null_mut(), busnum, ops.pci_ops,
...
#[no_mangle]
pub unsafe extern "C" fn acpi_pci_root_init() -> void __init {
