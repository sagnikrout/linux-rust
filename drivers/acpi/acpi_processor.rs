...
//! Source: drivers/acpi/acpi_processor.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn acpi_get_processor_handle(cpu: c_int) -> acpi_handle {
...
#[no_mangle]
unsafe extern "C" fn acpi_processor_errata_piix4(dev: *mut pci_dev) -> c_int {
...
//
    ...
    PCI_ANY_ID, PCI_ANY_ID, core::ptr::null_mut());
    if (dev) {
...
#[no_mangle]
unsafe extern "C" fn acpi_processor_errata() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpufreq_add_device(name: *const c_char) {
    static void cpufreq_add_device(const char *name)
    {
    struct platform_device *pdev;
    pdev = platform_device_register_simple(name, PLATFORM_DEVID_NONE, core::ptr::null_mut(), 0);
    if (IS_ERR(pdev))
...
#[no_mangle]
unsafe extern "C" fn acpi_pcc_cpufreq_init() -> void __init {
...
//
    if (per_cpu(processor_device_array, pr.id) != core::ptr::null_mut() &&
...
// Leave the processor device array in place to detect buggy bios
    per_cpu(processors, pr.id) = core::ptr::null_mut();
    acpi_unmap_cpu(pr.id);
    goto out;
    }
//
...
#[no_mangle]
unsafe extern "C" fn acpi_processor_get_info(device: *mut acpi_device) -> c_int {
    static int acpi_processor_get_info(struct acpi_device *device)
    {
    let mut object: union acpi_object = { .processor = { 0 } };
    let mut buffer: acpi_buffer = { sizeof(union acpi_object), &object };
    struct acpi_processor *pr = acpi_driver_data(device);
    let mut device_declaration: c_int = 0;
...
// Declared with "Processor" statement; match ProcessorID
    status = acpi_evaluate_object(pr.handle, core::ptr::null_mut(), core::ptr::null_mut(), &buffer);
    if (ACPI_FAILURE(status)) {
    dev_err(&device.dev,
...
//
    ...
    dev_dbg(&device.dev, "No PBLK (core::ptr::null_mut() address)\n");
#[no_mangle]
pub unsafe extern "C" fn if(6: object.processor.pblk_length !=) -> else {
...
// of /proc/cpuinfo
//
    status = acpi_evaluate_integer(pr.handle, "_SUN", core::ptr::null_mut(), &value);
...
// Removal
#[no_mangle]
unsafe extern "C" fn acpi_processor_post_eject(device: *mut acpi_device) {
...
// Remove the CPU.
    ...
    acpi_unmap_cpu(pr.id);
// Clean up.
    per_cpu(processor_device_array, pr.id) = core::ptr::null_mut();
    per_cpu(processors, pr.id) = core::ptr::null_mut();
    cpus_write_unlock();
    cpu_maps_update_done();
...
#[no_mangle]
pub unsafe extern "C" fn processor_physically_present(handle: acpi_handle) -> bool __init {
    ...
    let mut object: union acpi_object = {};
    let mut buffer: acpi_buffer = { sizeof(union acpi_object), &object };
...
#[no_mangle]
unsafe extern "C" fn acpi_early_processor_osc() -> bool __init {
    static bool __init acpi_early_processor_osc(void)
    {
    acpi_status status;
    acpi_proc_quirk_mwait_check();
    status = acpi_walk_namespace(ACPI_TYPE_PROCESSOR, ACPI_ROOT_OBJECT,
    ACPI_UINT32_MAX, acpi_processor_osc, core::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn acpi_early_processor_control_setup() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn processor_validated_ids_update(proc_id: c_int) -> void __init {
...
//
    ...
    break;
    case ACPI_TYPE_DEVICE:
    status = acpi_evaluate_integer(handle, "_UID", core::ptr::null_mut(), &uid);
    if (ACPI_FAILURE(status))
    goto err;
    break;
    default:
...
#[no_mangle]
unsafe extern "C" fn acpi_processor_check_duplicates() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn acpi_duplicate_processor_id(proc_id: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn acpi_processor_init() -> void __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_processor_claim_cst_control() -> bool {
...
//
    ...
    acpi_status status;
    u64 count;
    let mut last_index: c_int = 0;
    int i, ret = 0;
    status = acpi_evaluate_object(handle, "_CST", core::ptr::null_mut(), &buffer);
    if (ACPI_FAILURE(status)) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct acpi_lpi_states_array {
    pub size: c_uint,
    pub composite_states_size: c_uint,
    pub entries: *mut acpi_lpi_state,
    pub composite_states: [*mut acpi_lpi_state; ACPI_PROCESSOR_MAX_POWER],
}

#[no_mangle]
unsafe extern "C" fn obj_get_integer(obj: *mut union acpi_object, value: *mut u32) -> c_int {
...
//
    ...
    acpi_status status;
    let mut ret: c_int = 0;
    status = acpi_evaluate_object(handle, "_LPI", core::ptr::null_mut(), &buffer);
    if (ACPI_FAILURE(status)) {
...
#[no_mangle]
unsafe extern "C" fn too_many_states(handle: acpi_handle, state_count: c_uint) -> bool {
...
//
    ...
    let mut handle: acpi_handle = pr_handle;
    let mut state_count: c_uint = 0;
    unsigned int i;
    int ret;
