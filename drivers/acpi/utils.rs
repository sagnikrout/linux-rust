...
// --------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------- */
#[no_mangle]
unsafe extern "C" fn acpi_util_eval_error(h: acpi_handle, p: acpi_string, s: acpi_status) {
    ...
    char *format_string = core::ptr::null_mut();
    let mut format_count: u32 = 0;
    let mut i: u32 = 0;
    u8 *head = core::ptr::null_mut();
    u8 *tail = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn acpi_get_local_u64_address(handle: acpi_handle, addr: *mut u64) -> c_int {
    int acpi_get_local_u64_address(acpi_handle handle, u64 *addr)
    {
    acpi_status status;
    status = acpi_evaluate_integer(handle, METHOD_NAME__ADR, core::ptr::null_mut(), addr);
    if (ACPI_FAILURE(status))
    return -ENODATA;
    return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn acpi_get_local_address(handle: acpi_handle, addr: *mut u32) -> c_int {
...
pub const ACPI_MAX_SUB_BUF_SIZE: c_int = 9;
    ...
    const char *sub;
    size_t len;
    status = acpi_evaluate_object(handle, METHOD_NAME__SUB, core::ptr::null_mut(), &buffer);
    if (ACPI_FAILURE(status)) {
...
// Get the  acpi_handle.
    ...
    err_free:
    kfree(list.handles);
    list.handles = core::ptr::null_mut();
    err_clear:
    list.count = 0;
    err:
    acpi_util_eval_error(handle, pathname, status);
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_handle_list_free(list: *mut acpi_handle_list) {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_device_dep(target: acpi_handle, match: acpi_handle) -> bool {
    ...
    return false;
    if (!acpi_evaluate_reference(target, "_DEP", core::ptr::null_mut(), &dep_devices)) {
    ...
    status = acpi_evaluate_object(handle, "_PLD", core::ptr::null_mut(), &buffer);
    if (ACPI_FAILURE(status))
    return false;
...
//
    ...
    let mut arg_list: acpi_object_list = {3, params};
    params[0].integer.value = source_event;
    params[1].integer.value = status_code;
    if (status_buf != core::ptr::null_mut()) {
    params[2].buffer.pointer = status_buf.pointer;
    params[2].buffer.length = status_buf.length;
    } else {
    params[2].buffer.pointer = core::ptr::null_mut();
    params[2].buffer.length = 0;
    }
    return acpi_evaluate_object(handle, "_OST", &arg_list, core::ptr::null_mut());
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_has_method(handle: acpi_handle, name: *mut c_char) -> bool {
    ...
    let mut arg_list: acpi_object_list = { .count = 1, .pointer = &obj, };
    obj.integer.value = arg;
    return acpi_evaluate_object(handle, method, &arg_list, core::ptr::null_mut());
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_evaluate_ej0(handle: acpi_handle) -> acpi_status {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_evaluate_lck(handle: acpi_handle, lock: c_int) -> acpi_status {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_evaluate_reg(handle: acpi_handle, space_id: u8, function: u32) -> acpi_status {
    ...
    return acpi_evaluate_object(handle, "_REG", &arg_list, core::ptr::null_mut());
    }
...
//
    ...
    params[3].package.elements = core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_check_dsm(handle: acpi_handle, guid: *const guid_t, rev: u64, funcs: u64) -> bool {
    ...
    if (funcs == 0)
    return false;
    obj = acpi_evaluate_dsm(handle, guid, rev, 0, core::ptr::null_mut());
    if (!obj)
    return false;
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_dev_uid_to_integer(adev: *mut acpi_device, integer: *mut u64) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_dev_found(hid: *const c_char) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct acpi_dev_match_info {
    pub hid: [acpi_device_id; 2],
    pub uid: *const c_char,
    pub hrv: i64,
}

#[no_mangle]
unsafe extern "C" fn acpi_dev_match_cb(dev: *mut device, data: *const c_void) -> c_int {
    ...
    if (match.hrv == -1)
    return 1;
    status = acpi_evaluate_integer(adev.handle, "_HRV", core::ptr::null_mut(), &hrv);
    if (ACPI_FAILURE(status))
    return 0;
    let mut hrv: return = = match.hrv;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_dev_present(hid: *const c_char, uid: *const c_char, hrv: i64) -> bool {
    bool acpi_dev_present(const char *hid, const char *uid, s64 hrv)
    {
    let mut match: acpi_dev_match_info = {};
    struct device *dev;
    strscpy(match.hid[0].id, hid, sizeof(match.hid[0].id));
    match.uid = uid;
    match.hrv = hrv;
    dev = bus_find_device(&acpi_bus_type, core::ptr::null_mut(), &match, acpi_dev_match_cb);
    put_device(dev);
    return !!dev;
    }
...
//
    ...
    {
    struct device *start = adev ? &adev.dev : core::ptr::null_mut();
    let mut match: acpi_dev_match_info = {};
    struct device *dev;
    strscpy(match.hid[0].id, hid, sizeof(match.hid[0].id));
    match.uid = uid;
    ...
    acpi_dev_put(adev);
    return dev ? to_acpi_device(dev) : core::ptr::null_mut();
    }
...
//
    ...
    {
    return acpi_dev_get_next_match_dev(core::ptr::null_mut(), hid, uid, hrv);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_reduced_hardware() -> bool {
...
#[no_mangle]
unsafe extern "C" fn acpi_backlight(str: *mut c_char) -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_dev_is_video_device(adev: *mut acpi_device) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_match_platform_list(plat: *const acpi_platform_list) -> c_int {
