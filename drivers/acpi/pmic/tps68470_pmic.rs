...
//! Source: drivers/acpi/pmic/tps68470_pmic.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tps68470_pmic_table {
    pub /: *mut *mut u32 address; / operation region address,
    pub /: *mut *mut u32 reg; / corresponding register,
    pub /: *mut *mut u32 bitmask; / bit mask for power, clock,
}

pub const TI_PMIC_POWER_OPREGION_ID: c_uint = 0xB0;
pub const TI_PMIC_VR_VAL_OPREGION_ID: c_uint = 0xB1;
pub const TI_PMIC_CLOCK_OPREGION_ID: c_uint = 0xB2;
pub const TI_PMIC_CLKFREQ_OPREGION_ID: c_uint = 0xB3;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tps68470_pmic_opregion {
    pub lock: mutex,
...
#[no_mangle]
unsafe extern "C" fn tps68470_pmic_opregion_probe(pdev: *mut platform_device) -> c_int {
    ...
    let mut handle: acpi_handle = ACPI_HANDLE(pdev.dev.parent);
    ...
    if (!handle) {
    dev_warn(dev, "acpi handle is core::ptr::null_mut()\n");
    return -ENODEV;
    }
