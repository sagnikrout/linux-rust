...
//! Source: drivers/acpi/platform_profile.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct platform_profile_handler {
    pub name: *const c_char,
    pub dev: device,
    pub minor: c_int,
    pub choices: [c_ulong; BITS_TO_LONGS(PLATFORM_PROFILE_LAST)],
    pub hidden_choices: [c_ulong; BITS_TO_LONGS(PLATFORM_PROFILE_LAST)],
    pub ops: *const platform_profile_ops,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct aggregate_choices_data {
    pub aggregate: [c_ulong; BITS_TO_LONGS(PLATFORM_PROFILE_LAST)],
    pub count: c_int,
}
...
//
#[no_mangle]
unsafe extern "C" fn _commmon_choices_show(choices: *mut c_ulong, buf: *mut c_char) -> isize {
...
//
#[no_mangle]
unsafe extern "C" fn _store_class_profile(dev: *mut device, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn _notify_class_profile(dev: *mut device, data: *mut c_void) -> c_int {
    static int _notify_class_profile(struct device *dev, void *data)
    {
    struct platform_profile_handler *handler = to_pprof_handler(dev);
    lockdep_assert_held(&profile_lock);
    sysfs_notify(&handler.dev.kobj, core::ptr::null_mut(), "profile");
...
//
    ...
    char *buf)
    {
    let mut profile: enum platform_profile_option = PLATFORM_PROFILE_LAST;
    int err;
...
//
    ...
    sysfs_notify(acpi_kobj, core::ptr::null_mut(), "platform_profile");
    return count;
    }
...
#[no_mangle]
unsafe extern "C" fn pprof_device_release(dev: *mut device) {
...
//
#[no_mangle]
unsafe extern "C" fn _aggregate_choices(dev: *mut device, arg: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn _remove_hidden_choices(dev: *mut device, arg: *mut c_void) -> c_int {
...
//
    ...
    int err;
    scoped_cond_guard(mutex_intr, return -ERESTARTSYS, &profile_lock) {
    err = class_for_each_device(&platform_profile_class, core::ptr::null_mut(),
...
//
#[no_mangle]
unsafe extern "C" fn _aggregate_profiles(dev: *mut device, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn _store_and_notify(dev: *mut device, data: *mut c_void) -> c_int {
    ...
    if (err)
    return err;
    return _notify_class_profile(dev, core::ptr::null_mut());
    }
//
...
// Scan for a matching profile
    ...
    return -EINVAL;
    scoped_cond_guard(mutex_intr, return -ERESTARTSYS, &profile_lock) {
    ret = class_for_each_device(&platform_profile_class, core::ptr::null_mut(),
    &data, _aggregate_choices);
    if (ret)
    return ret;
    if (!test_bit(i, data.aggregate))
    return -EOPNOTSUPP;
    ret = class_for_each_device(&platform_profile_class, core::ptr::null_mut(), &i,
    _store_and_notify);
    if (ret)
    return ret;
    }
    ...
    let mut attr_platform_profile_choices: static struct kobj_attribute = __ATTR_RO(platform_profile_choices);
    let mut attr_platform_profile: static struct kobj_attribute = __ATTR_RW(platform_profile);
...
#[no_mangle]
unsafe extern "C" fn profile_class_registered(dev: *mut device, data: *const c_void) -> c_int {
    static int profile_class_registered(struct device *dev, const void *data)
    {
    return 1;
    }
#[no_mangle]
unsafe extern "C" fn profile_class_is_visible(kobj: *mut kobject, attr: *mut attribute, idx: c_int) -> umode_t {
    static umode_t profile_class_is_visible(struct kobject *kobj, struct attribute *attr, int idx)
    {
    struct device *dev;
    dev = class_find_device(&platform_profile_class, core::ptr::null_mut(), core::ptr::null_mut(), profile_class_registered);
    if (!dev)
    return 0;
    put_device(dev);
    return attr.mode;
...
//
#[no_mangle]
pub unsafe extern "C" fn platform_profile_notify(dev: *mut device) {
    void platform_profile_notify(struct device *dev)
    {
    scoped_cond_guard(mutex_intr, return, &profile_lock) {
    _notify_class_profile(dev, core::ptr::null_mut());
    }
    sysfs_notify(acpi_kobj, core::ptr::null_mut(), "platform_profile");
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn platform_profile_cycle() -> c_int {
    ...
    let mut next: enum platform_profile_option = PLATFORM_PROFILE_LAST;
    let mut profile: enum platform_profile_option = PLATFORM_PROFILE_LAST;
    int err;
    scoped_cond_guard(mutex_intr, return -ERESTARTSYS, &profile_lock) {
    err = class_for_each_device(&platform_profile_class, core::ptr::null_mut(),
...
// never iterate into a custom or max power if all drivers supported it
    ...
    profile + 1);
    err = class_for_each_device(&platform_profile_class, core::ptr::null_mut(), &next,
    _store_and_notify);
    if (err)
    return err;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn platform_profile_remove(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn devm_platform_profile_release(dev: *mut device, res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn platform_profile_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn platform_profile_exit() -> void __exit {
