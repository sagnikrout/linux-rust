...
//! Source: drivers/acpi/processor_core.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn acpi_map_madt_entry(acpi_id: u32) -> phys_cpuid_t __init {
...
#[no_mangle]
pub unsafe extern "C" fn acpi_get_madt_revision() -> int __init {
    int __init acpi_get_madt_revision(void)
    {
    struct acpi_table_header *madt = core::ptr::null_mut();
    int revision;
...
#[no_mangle]
unsafe extern "C" fn map_mat_entry(handle: acpi_handle, type: c_int, acpi_id: u32) -> phys_cpuid_t {
    static phys_cpuid_t map_mat_entry(acpi_handle handle, int type, u32 acpi_id)
    {
    let mut buffer: acpi_buffer = { ACPI_ALLOCATE_BUFFER, core::ptr::null_mut() };
    union acpi_object *obj;
    struct acpi_subtable_header *header;
    let mut phys_id: phys_cpuid_t = PHYS_CPUID_INVALID;
    if (ACPI_FAILURE(acpi_evaluate_object(handle, "_MAT", core::ptr::null_mut(), &buffer)))
    goto exit;
...
#[no_mangle]
pub unsafe extern "C" fn if(ACPI_MADT_TYPE_LOCAL_SAPIC: header->type ==) -> else {
    else if (header.type == ACPI_MADT_TYPE_LOCAL_SAPIC)
    map_lsapic_id(header, type, acpi_id, &phys_id);
#[no_mangle]
pub unsafe extern "C" fn if(ACPI_MADT_TYPE_LOCAL_X2APIC: header->type ==) -> else {
    else if (header.type == ACPI_MADT_TYPE_LOCAL_X2APIC)
    map_x2apic_id(header, type, acpi_id, &phys_id);
#[no_mangle]
pub unsafe extern "C" fn if(ACPI_MADT_TYPE_GENERIC_INTERRUPT: header->type ==) -> else {
    else if (header.type == ACPI_MADT_TYPE_GENERIC_INTERRUPT)
    map_gicc_mpidr(header, type, acpi_id, &phys_id);
#[no_mangle]
pub unsafe extern "C" fn if(ACPI_MADT_TYPE_CORE_PIC: header->type ==) -> else {
    else if (header.type == ACPI_MADT_TYPE_CORE_PIC)
...
#[no_mangle]
pub unsafe extern "C" fn acpi_get_phys_id(handle: acpi_handle, type: c_int, acpi_id: u32) -> phys_cpuid_t {
...
#[no_mangle]
pub unsafe extern "C" fn acpi_map_cpuid(phys_id: phys_cpuid_t, acpi_id: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn acpi_get_cpuid(handle: acpi_handle, type: c_int, acpi_id: u32) -> c_int {
    ...
    unsigned long end)
    {
    let mut start: c_ulong = (unsigned long)entry;
...
#[no_mangle]
unsafe extern "C" fn parse_madt_ioapic_entry(gsi_base: u32, phys_addr: *mut u64) -> c_int {
    static int parse_madt_ioapic_entry(u32 gsi_base, u64 *phys_addr)
    {
    struct acpi_subtable_header *hdr;
    unsigned long madt_end, entry;
    struct acpi_table_madt *madt;
    let mut apic_id: c_int = -1;
    madt = get_madt_table();
    if (!madt)
    return apic_id;
    entry = (unsigned long)madt;
...
//
#[no_mangle]
pub unsafe extern "C" fn acpi_get_ioapic_id(handle: acpi_handle, gsi_base: u32, phys_addr: *mut u64) -> c_int {
