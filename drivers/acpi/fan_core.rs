...
//! Source: drivers/acpi/fan_core.c
#![no_std]
...
pub const ACPI_FAN_NOTIFY_STATE_CHANGED: c_uint = 0x80;
//
...
pub const ACPI_FAN_DSM_GET_TRIP_POINT_GRANULARITY: c_int = 1;
pub const ACPI_FAN_DSM_SET_TRIP_POINTS: c_int = 2;
pub const ACPI_FAN_DSM_GET_OPERATING_RANGES: c_int = 3;
//
// Ensures that fans with a very low trip point granularity
// do not send too many notifications.
//
    let mut min_trip_distance: static uint = 100;
...
#[no_mangle]
pub unsafe extern "C" fn acpi_fan_get_fst(handle: acpi_handle, fst: *mut acpi_fan_fst) -> c_int {
    ...
    acpi_status status;
    let mut ret: c_int = 0;
    status = acpi_evaluate_object(handle, "_FST", core::ptr::null_mut(), &buffer);
...
#[no_mangle]
unsafe extern "C" fn fan_get_state_acpi4(device: *mut acpi_device, state: *mut c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fan_get_state(device: *mut acpi_device, state: *mut c_ulong) -> c_int {
    static int fan_get_state(struct acpi_device *device, unsigned long *state)
    {
    int result;
    let mut acpi_state: c_int = ACPI_STATE_D0;
...
#[no_mangle]
unsafe extern "C" fn fan_set_state(device: *mut acpi_device, state: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fan_set_state_acpi4(device: *mut acpi_device, state: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn acpi_fan_get_fif(device: *mut acpi_device) -> c_int {
    static int acpi_fan_get_fif(struct acpi_device *device)
    {
    let mut buffer: acpi_buffer = { ACPI_ALLOCATE_BUFFER, core::ptr::null_mut() };
    struct acpi_fan *fan = acpi_driver_data(device);
    let mut format: acpi_buffer = { sizeof("NNNN"), "NNNN" };
    u64 fields[4];
    let mut fif: acpi_buffer = { sizeof(fields), fields };
    union acpi_object *obj;
    acpi_status status;
    status = acpi_evaluate_object(device.handle, "_FIF", core::ptr::null_mut(), &buffer);
    if (ACPI_FAILURE(status))
    return status;
    obj = buffer.pointer;
...
#[no_mangle]
pub unsafe extern "C" fn if(9: fan->fif.step_size >) -> else {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_speed_cmp(a: *const c_void, b: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_get_fps(device: *mut acpi_device) -> c_int {
    ...
    acpi_status status;
    int i;
    status = acpi_evaluate_object(device.handle, "_FPS", core::ptr::null_mut(), &buffer);
    if (ACPI_FAILURE(status))
    return status;
    obj = buffer.pointer;
...
// sort the state array according to fan speed in increase order
    ...
    acpi_fan_speed_cmp, core::ptr::null_mut());
    err:
    kfree(obj);
    return status;
    }
#[no_mangle]
unsafe extern "C" fn acpi_fan_dsm_init(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_dsm_set_trip_points(dev: *mut device, upper: u64, lower: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_dsm_start(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_dsm_update_trips_points(dev: *mut device, fst: *mut acpi_fan_fst) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_notify_handler(handle: acpi_handle, event: u32, context: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (!fan.acpi4) {
    result = acpi_device_update_power(device, core::ptr::null_mut());
    if (result) {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acpi_fan_resume(dev: *mut device) -> c_int {
    ...
    if (fan.acpi4)
    return 0;
    result = acpi_device_update_power(ACPI_COMPANION(dev), core::ptr::null_mut());
    if (result)
