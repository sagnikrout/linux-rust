...
//! Source: drivers/acpi/numa/srat.c
#![no_std]
...
//
    ...
    let mut nodes_found_map: static nodemask_t = NODE_MASK_NONE;
...
#[no_mangle]
pub unsafe extern "C" fn disable_srat() -> void __init {
    void __init disable_srat(void)
    {
    acpi_numa = -1;
    }
#[no_mangle]
pub unsafe extern "C" fn pxm_to_node(pxm: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn node_to_pxm(node: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __acpi_map_pxm_to_node(pxm: c_int, node: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn acpi_map_pxm_to_node(pxm: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn fix_pxm_node_maps(max_nid: c_int) -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn slit_valid(slit: *mut acpi_table_slit) -> int __init {
    static int __init slit_valid(struct acpi_table_slit *slit)
    {
    int i, j;
    let mut d: c_int = slit.locality_count;
    for (i = 0; i < d; i++) {
    for (j = 0; j < d; j++) {
    let mut val: u8 = slit.entry[d*i + j];
    if (i == j) {
...
#[no_mangle]
pub unsafe extern "C" fn bad_srat() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn srat_disabled() -> int __init {
    int __init srat_disabled(void)
    {
    return acpi_numa < 0;
    }
#[no_mangle]
pub unsafe extern "C" fn numa_fill_memblks(start: u64, end: u64) -> __weak int __init {
...
//
#[no_mangle]
unsafe extern "C" fn acpi_parse_slit(table: *mut acpi_table_header) -> int __init {
    ...
    let mut from_node: c_int = pxm_to_node(i);
    if (from_node == NUMA_NO_NODE)
    continue;
    for (j = 0; j < slit.locality_count; j++) {
    let mut to_node: c_int = pxm_to_node(j);
...
#[no_mangle]
unsafe extern "C" fn acpi_parse_srat(table: *mut acpi_table_header) -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn acpi_numa_init() -> int __init {
...
// No need to expand numa nodes if CXL is disabled
    ...
    return cnt;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !parsed_numa_memblks) -> else {
    else if (!parsed_numa_memblks)
    return -ENOENT;
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn acpi_node_backed_by_real_pxm(nid: c_int) -> bool {
    bool acpi_node_backed_by_real_pxm(int nid)
    {
    let mut pxm: c_int = node_to_pxm(nid);
    return pxm <= last_real_pxm;
    }
    EXPORT_SYMBOL_GPL(acpi_node_backed_by_real_pxm);
#[no_mangle]
unsafe extern "C" fn acpi_get_pxm(h: acpi_handle) -> c_int {
    static int acpi_get_pxm(acpi_handle h)
    {
    unsigned long long pxm;
    acpi_status status;
    acpi_handle handle;
    let mut phandle: acpi_handle = h;
    do {
    handle = phandle;
    status = acpi_evaluate_integer(handle, "_PXM", core::ptr::null_mut(), &pxm);
...
#[no_mangle]
pub unsafe extern "C" fn acpi_get_node(handle: acpi_handle) -> c_int {
