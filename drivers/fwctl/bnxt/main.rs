...
//! Source: drivers/fwctl/bnxt/main.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bnxtctl_dev {
    pub fwctl: fwctl_device,
    pub aux_priv: *mut bnxt_aux_priv,
}

    DEFINE_FREE(bnxtctl, struct bnxtctl_dev *, if (_T) fwctl_put(&_T.fwctl))
#[no_mangle]
unsafe extern "C" fn bnxtctl_open_uctx(uctx: *mut fwctl_uctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bnxtctl_close_uctx(uctx: *mut fwctl_uctx) {
...
pub const BNXTCTL_MAX_DMA_FIELDS: c_int = 4;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bnxtctl_dma_field {
    pub /: *mut *mut size_t offset; / offsetof(hwrm_xxx_input, addr_field),
    pub dir: enum dma_data_direction,
    pub the: *mut *mut size_t len_offset; / offsetof(hwrm_xxx_input, len_field); 0 if,
// command carries no transfer-length field
//
    pub /: *mut *mut u8 len_width; / byte width of the length field: 2 or 4,
    pub /: *mut *mut u8 len_unit; / bytes represented by one unit of the length field,
    pub /: *mut *mut u32 buf_len; / for commands with no length in payload,
}
...
pub struct bnxtctl_cmd_dma_desc {
    ...
    pub num_fields: u8,
    pub scope_min: u8,
    pub /: *mut *mut size_t req_size; / sizeof(struct hwrm_xxx_input),
...
#[no_mangle]
unsafe extern "C" fn bnxtctl_read_len_field(cmd: *mut c_void, f: *const bnxtctl_dma_field) -> u32 {
...
// Caller must hold edev->en_dev_lock
    ...
    let mut req_type: u16 = le16_to_cpu(req.req_type);
    const struct bnxtctl_cmd_dma_desc *desc;
...
#[no_mangle]
unsafe extern "C" fn bnxtctl_get_timeout(req: *mut input) -> c_uint {
    ...
    let mut rpc_in: bnxt_fw_msg = {};
    let mut num_mapped: c_uint = 0;
    struct input *req = in;
    int rc;
...
#[no_mangle]
unsafe extern "C" fn bnxtctl_remove(adev: *mut auxiliary_device) {
