...
//! Source: drivers/crypto/caam/caamhash.c
#![no_std]
...
pub const CAAM_CRA_PRIORITY: c_int = 3000;
// max hash key is max split key size

    CAAM_MAX_HASH_KEY_SIZE)

// caam context sizes for hashes: running digest + 8
pub const HASH_MSG_LEN: c_int = 8;

    static struct list_head hash_list;
// ahash per-session context
#[repr(C)]
#[derive(Copy, Clone)]
pub struct caam_hash_ctx {
    pub ____cacheline_aligned: u32 sh_desc_update[DESC_HASH_MAX_USED_LEN],
    pub ____cacheline_aligned: u32 sh_desc_update_first[DESC_HASH_MAX_USED_LEN],
    pub ____cacheline_aligned: u32 sh_desc_fin[DESC_HASH_MAX_USED_LEN],
    pub ____cacheline_aligned: u32 sh_desc_digest[DESC_HASH_MAX_USED_LEN],
    pub ____cacheline_aligned: u8 key[CAAM_MAX_HASH_KEY_SIZE],
    pub ____cacheline_aligned: dma_addr_t sh_desc_update_dma,
    pub sh_desc_update_first_dma: dma_addr_t,
    pub sh_desc_fin_dma: dma_addr_t,
    pub sh_desc_digest_dma: dma_addr_t,
    pub dir: enum dma_data_direction,
    pub key_dir: enum dma_data_direction,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct caam_hash_state {
    pub buf_dma: dma_addr_t,
    pub ctx_dma: dma_addr_t,
    pub ctx_dma_len: c_int,
    pub ____cacheline_aligned: u8 buf[CAAM_MAX_HASH_BLOCK_SIZE],
    pub buflen: c_int,
    pub next_buflen: c_int,
    pub ____cacheline_aligned: u8 caam_ctx[MAX_CTX_LEN],
    pub ____cacheline_aligned: *mut *mut *mut int (update)(struct ahash_request req),
    pub req): *mut *mut int (final)(struct ahash_request,
    pub req): *mut *mut int (finup)(struct ahash_request,
    pub edesc: *mut ahash_edesc,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct caam_export_state {
    pub buf: [u8; CAAM_MAX_HASH_BLOCK_SIZE],
    pub caam_ctx: [u8; MAX_CTX_LEN],
    pub buflen: c_int,
    pub req): *mut *mut int (update)(struct ahash_request,
    pub req): *mut *mut int (final)(struct ahash_request,
    pub req): *mut *mut int (finup)(struct ahash_request,
}

#[no_mangle]
pub unsafe extern "C" fn is_cmac_aes(algtype: u32) -> bool {
...
// Map current buffer in state (if length > 0) and put it in link table
    ...
    struct caam_hash_state *state)
    {
    let mut buflen: c_int = state.buflen;
    if (!buflen)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn ahash_set_sh_desc(ahash: *mut crypto_ahash) -> c_int {
    static int ahash_set_sh_desc(struct crypto_ahash *ahash)
    {
    struct caam_hash_ctx *ctx = crypto_ahash_ctx_dma(ahash);
    let mut digestsize: c_int = crypto_ahash_digestsize(ahash);
...
#[no_mangle]
unsafe extern "C" fn axcbc_set_sh_desc(ahash: *mut crypto_ahash) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acmac_set_sh_desc(ahash: *mut crypto_ahash) -> c_int {
...
// keylen = digestsize;
    ...
    struct caam_hash_ctx *ctx = crypto_ahash_ctx_dma(ahash);
    struct device *jrdev = ctx.jrdev;
    let mut blocksize: c_int = crypto_tfm_alg_blocksize(&ahash.base);
    let mut digestsize: c_int = crypto_ahash_digestsize(ahash);
    struct caam_drv_private *ctrlpriv = dev_get_drvdata(ctx.jrdev.parent);
    int ret;
    u8 *hashed_key = core::ptr::null_mut();
...
pub struct ahash_edesc {
    ...
    pub bklog: bool,
    pub ____cacheline_aligned: u32 hw_desc[DESC_JOB_IO_LEN_MAX / sizeof(u32)],
    pub sec4_sg: [sec4_sg_entry; ],
}
...
#[no_mangle]
unsafe extern "C" fn ahash_do_one_req(engine: *mut crypto_engine, areq: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_update_ctx(req: *mut ahash_request) -> c_int {
    ...
    let mut blocksize: c_int = crypto_ahash_blocksize(ahash);
    let mut in_len: c_int = *buflen + req.nbytes, to_hash;
    u32 *desc;
...
// next_buflen = blocksize;
    ...
    if (to_hash) {
    int pad_nents;
    let mut src_len: c_int = req.nbytes - *next_buflen;
...
#[no_mangle]
unsafe extern "C" fn ahash_final_ctx(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_finup_ctx(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_digest(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_final_no_ctx(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_update_no_ctx(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_finup_no_ctx(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_update_first(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_finup_first(req: *mut ahash_request) -> c_int {
    static int ahash_finup_first(struct ahash_request *req)
    {
    return ahash_digest(req);
    }
#[no_mangle]
unsafe extern "C" fn ahash_init(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_update(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_finup(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_final(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ahash_export(req: *mut ahash_request, out: *mut c_void) -> c_int {
    static int ahash_export(struct ahash_request *req, void *out)
    {
    struct caam_hash_state *state = ahash_request_ctx_dma(req);
    struct caam_export_state *export = out;
    u8 *buf = state.buf;
    let mut len: c_int = state.buflen;
...
#[no_mangle]
unsafe extern "C" fn ahash_import(req: *mut ahash_request, in: *const c_void) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct caam_hash_template {
    pub name: [c_char; CRYPTO_MAX_ALG_NAME],
    pub driver_name: [c_char; CRYPTO_MAX_ALG_NAME],
    pub hmac_name: [c_char; CRYPTO_MAX_ALG_NAME],
    pub hmac_driver_name: [c_char; CRYPTO_MAX_ALG_NAME],
    pub blocksize: c_uint,
    pub template_ahash: ahash_alg,
    pub alg_type: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct caam_hash_alg {
    pub entry: list_head,
    pub alg_type: c_int,
    pub is_hmac: bool,
    pub ahash_alg: ahash_engine_alg,
}

#[no_mangle]
unsafe extern "C" fn caam_hash_cra_init(tfm: *mut crypto_tfm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn caam_hash_cra_exit(tfm: *mut crypto_tfm) {
...
#[no_mangle]
pub unsafe extern "C" fn caam_algapi_hash_exit() {
...
#[no_mangle]
pub unsafe extern "C" fn caam_algapi_hash_init(ctrldev: *mut device) -> c_int {
    int caam_algapi_hash_init(struct device *ctrldev)
    {
    let mut i: c_int = 0, err = 0;
    struct caam_drv_private *priv = dev_get_drvdata(ctrldev);
    let mut md_limit: c_uint = SHA512_DIGEST_SIZE;
    u32 md_inst, md_vid;
//
...
//
    ...
    let mut mdha: u32 = rd_reg32(&priv.jr[0].vreg.mdha);
