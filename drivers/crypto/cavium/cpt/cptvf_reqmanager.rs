...
//! Source: drivers/crypto/cavium/cpt/cptvf_reqmanager.c
#![no_std]
...
//
    ...
    int qlen)
    {
    struct pending_entry *ent = core::ptr::null_mut();
    ent = &q.head[q.rear];
    ...
    int components;
    struct sglist_component *sg_ptr = core::ptr::null_mut();
    struct pci_dev *pdev = cptvf.pdev;
    if (unlikely(!list)) {
    dev_err(&pdev.dev, "Input List pointer is core::ptr::null_mut()\n");
    return -EFAULT;
    }
    ...
    {
    let mut g_sz_bytes: u16 = 0, s_sz_bytes = 0;
    let mut ret: c_int = 0;
    struct pci_dev *pdev = cptvf.pdev;
...
// ((u64 *)info->out_buffer) = ~((u64)COMPLETION_CODE_INIT);
    ...
    {
    struct pci_dev *pdev = cptvf.pdev;
    struct command_qinfo *qinfo = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn do_post_process(cptvf: *mut cpt_vf, info: *mut cpt_info_buffer) {
    ...
    struct pending_entry *pentry = core::ptr::null_mut();
    struct cpt_info_buffer *info = core::ptr::null_mut();
    union cpt_res_s *status = core::ptr::null_mut();
    unsigned char ccode;
    while (1) {
    ...
    pentry.post_arg = core::ptr::null_mut();
    pending_queue_inc_front(pqinfo, qno);
    do_request_cleanup(cptvf, info);
    spin_unlock_bh(&pqueue.lock);
    break;
...
#[no_mangle]
pub unsafe extern "C" fn process_request(cptvf: *mut cpt_vf, req: *mut cpt_request_info) -> c_int {
    int process_request(struct cpt_vf *cptvf, struct cpt_request_info *req)
    {
    let mut ret: c_int = 0, clear = 0, queue = 0;
    struct cpt_info_buffer *info = core::ptr::null_mut();
    struct cptvf_request *cpt_req = core::ptr::null_mut();
    union ctrl_info *ctrl = core::ptr::null_mut();
    union cpt_res_s *result = core::ptr::null_mut();
    struct pending_entry *pentry = core::ptr::null_mut();
    struct pending_queue *pqueue = core::ptr::null_mut();
    struct pci_dev *pdev = cptvf.pdev;
    let mut group: u8 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn vq_post_process(cptvf: *mut cpt_vf, qno: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn cptvf_do_request(vfdev: *mut c_void, req: *mut cpt_request_info) -> c_int {
