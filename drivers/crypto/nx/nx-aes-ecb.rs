...
//! Source: drivers/crypto/nx/nx-aes-ecb.c
#![no_std]
...
//
    ...
    rc = nx_build_sg_lists(nx_ctx, core::ptr::null_mut(), req.dst, req.src,
    &to_process, processed, core::ptr::null_mut());
    if (rc)
    goto out;
...
#[no_mangle]
unsafe extern "C" fn ecb_aes_nx_encrypt(req: *mut skcipher_request) -> c_int {
    static int ecb_aes_nx_encrypt(struct skcipher_request *req)
    {
    return ecb_aes_nx_crypt(req, 1);
    }
#[no_mangle]
unsafe extern "C" fn ecb_aes_nx_decrypt(req: *mut skcipher_request) -> c_int {
