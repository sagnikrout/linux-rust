...
//! Source: drivers/crypto/nx/nx-common-powernv.c
#![no_std]
...
// Below fields must be properly aligned
    pub /: *mut *mut coprocessor_request_block crb; / CRB_ALIGN align,
    pub /: *mut *mut data_descriptor_entry ddl_in[DDL_LEN_MAX]; / DDE_ALIGN align,
    pub /: *mut *mut data_descriptor_entry ddl_out[DDL_LEN_MAX]; / DDE_ALIGN align,
// Above fields must be properly aligned
    pub start: ktime_t,
    pub /: *mut *mut char padding[WORKMEM_ALIGN]; / unused, to allow alignment,
    pub __aligned(WORKMEM_ALIGN): } __packed,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nx_coproc {
    pub chip_id: c_uint,
    pub high/normal*/: *mut *mut unsigned int ct; / Can be 842 or GZIP,
    pub /: *mut *mut unsigned int ci; / Coprocessor instance, used with icswx,
    struct {
    pub rxwin: *mut vas_window,
    pub id: c_int,
    pub vas: },
    pub list: list_head,
}
...
//
    ...
    {
    let mut l: c_uint = min_t(unsigned int, len, LEN_ON_PAGE(pa));
    dde.flags = 0;
    dde.count = 0;
    dde.index = 0;
...
//
    ...
    bool in)
    {
    let mut pa: c_ulong = nx842_get_pa(buf);
...
// use the DDL
    ...
    {
    let mut start: ktime_t = wmem.start, now = ktime_get();
    let mut timeout: ktime_t = ktime_add_ms(start, CSB_WAIT_MAX);
...
//
    ...
    {
    struct vas_window *txwin = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn nx_open_percpu_txwins() -> c_int {
...
// shouldn't happen, Each chip will have NX engine
    ...
    coproc.ct = high;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(priority, _arg: "Normal")) -> else {
    ...
    {
    struct vas_window *rxwin = core::ptr::null_mut();
    ...
    VAS_COP_TYPE_842);
#[no_mangle]
pub unsafe extern "C" fn if(NX_CT_GZIP: type ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn nx_coproc_init(chip_id: c_int, ct_842: c_int, ct_gzip: c_int) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn nx_powernv_probe_vas(pn: *mut device_node) -> int __init {
    static int __init nx_powernv_probe_vas(struct device_node *pn)
    {
    int chip_id, vasid, ret = 0;
    let mut ct_842: c_int = 0, ct_gzip = 0;
...
#[no_mangle]
unsafe extern "C" fn nx842_powernv_probe(dn: *mut device_node) -> int __init {
    ...
    nx842_ct = ct;
#[no_mangle]
pub unsafe extern "C" fn if(ct: nx842_ct !=) -> else {
    else if (nx842_ct != ct)
...
#[no_mangle]
unsafe extern "C" fn nx_delete_coprocs() {
...
//
    ...
    if (txwin)
    vas_win_close(txwin);
    per_cpu(cpu_txwin, i) = core::ptr::null_mut();
    }
    list_for_each_entry_safe(coproc, n, &nx_coprocs, list) {
    if (coproc.vas.rxwin)
...
#[no_mangle]
unsafe extern "C" fn nx_compress_powernv_init() -> __init int {
...
// verify buffer size/align restrictions
    ...
    for_each_compatible_node(dn, core::ptr::null_mut(), "ibm,power9-nx") {
    ...
    for_each_compatible_node(dn, core::ptr::null_mut(), "ibm,power-nx")
...
#[no_mangle]
unsafe extern "C" fn nx_compress_powernv_exit() -> void __exit {
