...
//! Source: drivers/crypto/nx/nx-common-pseries.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hv_nx_cop_caps {
    pub descriptor: __be64,
    pub /: *mut *mut __be64 req_max_processed_len; / Max bytes in one GZIP request,
    pub /: *mut *mut __be64 min_compress_len; / Min compression size in bytes,
    pub /: *mut *mut __be64 min_decompress_len; / Min decompression size in bytes,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nx_cop_caps {
    pub descriptor: u64,
    pub /: *mut *mut u64 req_max_processed_len; / Max bytes in one GZIP request,
    pub /: *mut *mut u64 min_compress_len; / Min compression in bytes,
    pub /: *mut *mut u64 min_decompress_len; / Min decompression in bytes,
}
...
#[no_mangle]
unsafe extern "C" fn check_constraints(buf: c_ulong, len: *mut c_uint, in: bool) -> c_int {
...
// scatterlist
    pub slin: [c_char; 4096],
    pub slout: [c_char; 4096],
// coprocessor status/parameter block
    pub csbcpb: nx_csbcpb,
    pub padding: [c_char; WORKMEM_ALIGN],
    pub __aligned(WORKMEM_ALIGN): },
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ibm_nx842_counters {
    pub comp_complete: core::sync::atomic::AtomicI64,
    pub comp_failed: core::sync::atomic::AtomicI64,
    pub decomp_complete: core::sync::atomic::AtomicI64,
    pub decomp_failed: core::sync::atomic::AtomicI64,
    pub swdecomp: core::sync::atomic::AtomicI64,
    pub comp_times: [core::sync::atomic::AtomicI64; 32],
    pub decomp_times: [core::sync::atomic::AtomicI64; 32],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct nx842_devdata {
    pub vdev: *mut vio_dev,
    pub dev: *mut device,
    pub counters: *mut ibm_nx842_counters,
    pub max_sg_len: c_uint,
    pub max_sync_size: c_uint,
    pub max_sync_sg: c_uint,
}
...
pub const NX842_HIST_SLOTS: c_int = 16;
#[no_mangle]
unsafe extern "C" fn ibm_nx842_incr_hist(times: *mut core::sync::atomic::AtomicI64, time: c_uint) {
    static void ibm_nx842_incr_hist(atomic64_t *times, unsigned int time)
    {
    let mut bucket: c_int = fls(time);
    if (bucket)
    bucket = min((NX842_HIST_SLOTS - 1), bucket - 1);
    atomic64_inc(&times[bucket]);
    }
// NX unit operation flags
pub const NX842_OP_COMPRESS: c_uint = 0x0;
pub const NX842_OP_CRC: c_uint = 0x1;
pub const NX842_OP_DECOMPRESS: c_uint = 0x2;

#[no_mangle]
unsafe extern "C" fn nx842_get_desired_dma(viodev: *mut vio_dev) -> c_ulong {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nx842_slentry {
    pub /: *mut *mut __be64 ptr; / Real address (use __pa()),
    pub len: __be64,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nx842_scatterlist {
    pub /: *mut *mut int entry_nr; / number of slentries,
    pub /: *mut *mut *mut nx842_slentry entries; / ptr to array of slentries,
}
...
//
#[no_mangle]
unsafe extern "C" fn nx842_OF_set_defaults(devdata: *mut nx842_devdata) -> c_int {
...
//
    ...
    struct property *prop) {
    let mut ret: c_int = 0;
    let mut maxsglen: c_uint = of_read_number(prop.value, 1);
...
//
#[no_mangle]
unsafe extern "C" fn nx842_OF_upd(new_prop: *mut property) -> c_int {
    static int nx842_OF_upd(struct property *new_prop)
    {
    struct nx842_devdata *old_devdata = core::ptr::null_mut();
    struct nx842_devdata *new_devdata = core::ptr::null_mut();
    struct device_node *of_node = core::ptr::null_mut();
    struct property *status = core::ptr::null_mut();
    struct property *maxsglen = core::ptr::null_mut();
    struct property *maxsyncop = core::ptr::null_mut();
    let mut ret: c_int = 0;
    unsigned long flags;
...
// Set ptrs for existing properties
    status = of_find_property(of_node, "status", core::ptr::null_mut());
    maxsglen = of_find_property(of_node, "ibm,max-sg-len", core::ptr::null_mut());
    maxsyncop = of_find_property(of_node, "ibm,max-sync-cop", core::ptr::null_mut());
...
//
    ...
    nx842_timehist_show, core::ptr::null_mut());
    static struct device_attribute dev_attr_decomp_times = __ATTR(decomp_times,
    0444, nx842_timehist_show, core::ptr::null_mut());
    static ssize_t nx842_timehist_show(struct device *dev,
    struct device_attribute *attr, char *buf) {
    char *p = buf;
    struct nx842_devdata *local_devdata;
    atomic64_t *times;
    let mut bytes_remain: c_int = PAGE_SIZE;
    int bytes;
    int i;
    rcu_read_lock();
    local_devdata = rcu_dereference(devdata);
...
#[no_mangle]
pub unsafe extern "C" fn if(&dev_attr_decomp_times: attr ==) -> else {
...
// 2<<(NX842_HIST_SLOTS - 2) us
    ...
    {
    struct nx842_devdata *old_devdata, *new_devdata = core::ptr::null_mut();
    unsigned long flags;
    let mut ret: c_int = 0;
    new_devdata = kzalloc_obj(*new_devdata, GFP_NOFS);
    if (!new_devdata)
    ...
    old_devdata = rcu_dereference_check(devdata,
    lockdep_is_held(&devdata_spinlock));
    if (old_devdata && old_devdata.vdev != core::ptr::null_mut()) {
    dev_err(&viodev.dev, "%s: Attempt to register more than one instance of the hardware\n", __func__);
    ret = -1;
    goto error_unlock;
    }
    dev_set_drvdata(&viodev.dev, core::ptr::null_mut());
    ...
    ret = nx842_OF_upd(core::ptr::null_mut());
    if (ret)
    goto error;
...
#[no_mangle]
unsafe extern "C" fn nx842_remove(viodev: *mut vio_dev) {
    ...
    old_devdata = rcu_dereference_check(devdata,
    lockdep_is_held(&devdata_spinlock));
    RCU_INIT_POINTER(devdata, core::ptr::null_mut());
    spin_unlock_irqrestore(&devdata_spinlock, flags);
    synchronize_rcu();
    dev_set_drvdata(&viodev.dev, core::ptr::null_mut());
    if (old_devdata)
...
//
#[no_mangle]
unsafe extern "C" fn nxcop_get_capabilities() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn nx842_pseries_init() -> int __init {
    static int __init nx842_pseries_init(void)
    {
    struct nx842_devdata *new_devdata;
    struct device_node *np;
    int ret;
    np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "ibm,compression");
    if (!np)
    return -ENODEV;
    of_node_put(np);
    RCU_INIT_POINTER(devdata, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn nx842_pseries_exit() -> void __exit {
    ...
    dev_set_drvdata(old_devdata.dev, core::ptr::null_mut());
    kfree(old_devdata);
