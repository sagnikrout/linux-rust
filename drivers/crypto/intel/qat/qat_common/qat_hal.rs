...
//! Source: drivers/crypto/intel/qat/qat_common/qat_hal.c
#![no_std]
...
pub const BAD_REGADDR: c_uint = 0xffff;
pub const MAX_RETRY_TIMES: c_int = 10000;
pub const INIT_CTX_ARB_VALUE: c_uint = 0x0;
pub const INIT_CTX_ENABLE_VALUE: c_uint = 0x0;
pub const INIT_PC_VALUE: c_uint = 0x0;
pub const INIT_WAKEUP_EVENTS_VALUE: c_uint = 0x1;
pub const INIT_SIG_EVENTS_VALUE: c_uint = 0x1;
pub const INIT_CCENABLE_VALUE: c_uint = 0x2000;
pub const RST_CSR_QAT_LSB: c_int = 20;
pub const RST_CSR_AE_LSB: c_int = 0;

pub const MIN_RESET_DELAY_US: c_int = 3;
...
// events = qat_hal_rd_ae_csr(handle, ae, CTX_WAKEUP_EVENTS_INDIRECT);
    ...
    int chk_inactive)
    {
    let mut base_cnt: c_uint = 0, cur_cnt = 0;
    let mut csr: c_uint = (1 << ACS_ABO_BITPOS);
    let mut times: c_int = MAX_RETRY_TIMES;
    let mut elapsed_cycles: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn qat_hal_reset(handle: *mut icp_qat_fw_loader_handle) {
    void qat_hal_reset(struct icp_qat_fw_loader_handle *handle)
    {
    let mut reset_mask: c_uint = handle.chip_info.icp_rst_mask;
    let mut reset_csr: c_uint = handle.chip_info.icp_rst_csr;
...
#[no_mangle]
unsafe extern "C" fn qat_hal_check_ae_alive(handle: *mut icp_qat_fw_loader_handle) -> c_int {
    ...
    unsigned int ae)
    {
    let mut enable: c_uint = 0, active = 0;
...
#[no_mangle]
unsafe extern "C" fn qat_hal_reset_timestamp(handle: *mut icp_qat_fw_loader_handle) {
...
pub const ESRAM_AUTO_INIT_CSR_OFFSET: c_uint = 0xC1C;
#[no_mangle]
unsafe extern "C" fn qat_hal_init_esram(handle: *mut icp_qat_fw_loader_handle) -> c_int {
...
pub const SHRAM_INIT_CYCLES: c_int = 2060;
#[no_mangle]
pub unsafe extern "C" fn qat_hal_clr_reset(handle: *mut icp_qat_fw_loader_handle) -> c_int {
    int qat_hal_clr_reset(struct icp_qat_fw_loader_handle *handle)
    {
    let mut clk_csr: c_uint = handle.chip_info.glb_clk_enable_csr;
    let mut reset_mask: c_uint = handle.chip_info.icp_rst_mask;
    let mut reset_csr: c_uint = handle.chip_info.icp_rst_csr;
    let mut ae_mask: c_ulong = handle.hal_handle.ae_mask;
    let mut ae: c_uchar = 0;
    let mut times: c_uint = 100;
    unsigned int csr_val;
...
#[no_mangle]
unsafe extern "C" fn qat_hal_parity_64bit(word: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn qat_hal_set_uword_ecc(uword: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn qat_hal_clear_xfer(handle: *mut icp_qat_fw_loader_handle) {
...
#[no_mangle]
unsafe extern "C" fn qat_hal_clear_gpr(handle: *mut icp_qat_fw_loader_handle) -> c_int {
    static int qat_hal_clear_gpr(struct icp_qat_fw_loader_handle *handle)
    {
    let mut ae_mask: c_ulong = handle.hal_handle.ae_mask;
    unsigned char ae;
    let mut ctx_mask: c_uint = ICP_QAT_UCLO_AE_ALL_CTX;
    let mut times: c_int = MAX_RETRY_TIMES;
    let mut csr_val: c_uint = 0;
    let mut savctx: c_uint = 0;
    let mut ret: c_int = 0;
    for_each_set_bit(ae, &ae_mask, handle.hal_handle.ae_max_num) {
...
#[no_mangle]
pub unsafe extern "C" fn qat_hal_init(accel_dev: *mut adf_accel_dev) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qat_hal_deinit(handle: *mut icp_qat_fw_loader_handle) {
...
#[no_mangle]
pub unsafe extern "C" fn qat_hal_start(handle: *mut icp_qat_fw_loader_handle) -> c_int {
    int qat_hal_start(struct icp_qat_fw_loader_handle *handle)
    {
    let mut ae_mask: c_ulong = handle.hal_handle.ae_mask;
    let mut wakeup_val: u32 = handle.chip_info.wakeup_event_val;
    u32 fcu_ctl_csr, fcu_sts_csr;
    unsigned int fcu_sts;
    unsigned char ae;
    let mut ae_ctr: u32 = 0;
...
pub const MAX_EXEC_INST: c_int = 100;
    ...
    unsigned int *endpc)
    {
    let mut ind_lm_addr_byte0: c_uint = 0, ind_lm_addr_byte1 = 0;
    let mut ind_lm_addr_byte2: c_uint = 0, ind_lm_addr_byte3 = 0;
    let mut ind_t_index: c_uint = 0, ind_t_index_byte = 0;
    let mut ind_lm_addr0: c_uint = 0, ind_lm_addr1 = 0;
    let mut ind_lm_addr2: c_uint = 0, ind_lm_addr3 = 0;
    u64 savuwords[MAX_EXEC_INST];
    unsigned int ind_cnt_sig;
    unsigned int ind_sig, act_sig;
    let mut csr_val: c_uint = 0, newcsr_val;
    unsigned int savctx;
...
// data = qat_hal_rd_ae_csr(handle, ae, ALU_OUT);
    ...
    let mut num_inst: c_int = ARRAY_SIZE(insts), code_off = 1;
    let mut imm_w1: c_int = 0, imm_w0 = 1;
    ...
    code_off, num_inst * 0x5, core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn qat_hal_get_ins_num() -> c_int {
    ...
    let mut gpra0: c_uint = 0, gpra1 = 0, gpra2 = 0;
    let mut gprb0: c_uint = 0, gprb1 = 0;
    if (*pfirst_exec) {
...
// pfirst_exec = 0;
    ...
    inst_num * 0x5, core::ptr::null_mut());
    if (stat != 0)
    return -EFAULT;
...
// exec micro codes
    ...
    unsigned short mask;
    let mut dr_offset: c_ushort = 0x10;
    ctx_enables = qat_hal_rd_ae_csr(handle, ae, CTX_ENABLES);
    ...
    let mut num_inst: c_int = ARRAY_SIZE(micro_inst), code_off = 1;
    let mut gprnum: c_ushort = 0, dly = num_inst * 0x5;
    ctx_enables = qat_hal_rd_ae_csr(handle, ae, CTX_ENABLES);
    if (CE_INUSE_CONTEXTS & ctx_enables) {
    if (ctx & 0x1) {
    ...
    code_off, dly, core::ptr::null_mut());
