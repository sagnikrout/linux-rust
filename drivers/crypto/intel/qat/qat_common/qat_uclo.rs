...
//! Source: drivers/crypto/intel/qat/qat_common/qat_uclo.c
#![no_std]
...
pub const INVLD_UWORD: c_uint = 0xffffffffffull;
pub const PID_MINOR_REV: c_uint = 0xf;
    ...
    struct icp_qat_uclo_page *page = core::ptr::null_mut();
    struct icp_qat_uclo_aeslice *ae_slice = core::ptr::null_mut();
    ...
    ae_slice.region = core::ptr::null_mut();
    return -ENOMEM;
    }
#[no_mangle]
unsafe extern "C" fn qat_uclo_free_ae_data(ae_data: *mut icp_qat_uclo_aedata) -> c_int {
    static int qat_uclo_free_ae_data(struct icp_qat_uclo_aedata *ae_data)
    {
    unsigned int i;
    if (!ae_data) {
    pr_err("bad argument, ae_data is core::ptr::null_mut()\n");
    return -EINVAL;
    }
    for (i = 0; i < ae_data.slice_num; i++) {
    kfree(ae_data.ae_slices[i].region);
    ae_data.ae_slices[i].region = core::ptr::null_mut();
    kfree(ae_data.ae_slices[i].page);
    ae_data.ae_slices[i].page = core::ptr::null_mut();
    }
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_check_uof_format(hdr: *mut icp_qat_uof_filehdr) -> c_int {
    static int qat_uclo_check_uof_format(struct icp_qat_uof_filehdr *hdr)
    {
    let mut maj: c_int = hdr.maj_ver & 0xff;
    let mut min: c_int = hdr.min_ver & 0xff;
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_check_suof_format(suof_hdr: *mut icp_qat_suof_filehdr) -> c_int {
    static int qat_uclo_check_suof_format(struct icp_qat_suof_filehdr *suof_hdr)
    {
    let mut maj: c_int = suof_hdr.maj_ver & 0xff;
    let mut min: c_int = suof_hdr.min_ver & 0xff;
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_parse_num(str: *mut c_char, num: *mut c_uint) -> c_int {
...
// num = (unsigned int)ae;
    ...
    if (!str) {
    pr_err("AE name assigned in UOF init table is core::ptr::null_mut()\n");
    return -EINVAL;
    }
...
// init_tab_base = init_header;
    ...
    mem_init.size = 4;
    mem_init.next = core::ptr::null_mut();
    tail.next = mem_init;
    tail = mem_init;
...
// set the highest ustore address referenced
    ...
    struct icp_qat_uclo_objhandle *obj_handle = handle.obj_handle;
    let mut ae_mask: c_ulong = handle.hal_handle.ae_mask;
    let mut cfg_ae_mask: c_ulong = handle.cfg_ae_mask;
    u64 *fill_data;
    ...
    let mut ae_assigned: c_ulong = uof_image.ae_assigned;
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_init_memory(handle: *mut icp_qat_fw_loader_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_calc_checksum(reg: c_uint, ch: c_int) -> c_uint {
    static unsigned int qat_uclo_calc_checksum(unsigned int reg, int ch)
    {
    int i;
    let mut topbit: c_uint = 1 << 0xF;
    let mut inbyte: c_uint = (unsigned int)((reg >> 0x18) ^ ch);
    reg ^= inbyte << 0x8;
    for (i = 0; i < 0x8; i++) {
    if (reg & topbit)
    reg = (reg << 1) ^ 0x1021;
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_calc_str_checksum(ptr: *mut c_char, num: c_int) -> c_uint {
...
// encap_uof_obj,
    ...
    {
    int i, j;
    struct icp_qat_uof_chunkhdr *chunk_hdr = core::ptr::null_mut();
    struct icp_qat_uof_image *image;
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_map_ae(handle: *mut icp_qat_fw_loader_handle, max_ae: c_int) -> c_int {
    ...
    let mut ae_assigned: c_ulong = obj_handle.ae_uimage[i].img_ptr.ae_assigned;
    if (!test_bit(ae, &ae_assigned))
    continue;
    mflag = 1;
    ...
    obj_hdr.file_buff, tab_name, core::ptr::null_mut());
    if (chunk_hdr) {
    int hdr_size;
    ...
    struct icp_qat_uof_chunkhdr *chunk_hdr;
    chunk_hdr = qat_uclo_find_chunk(encap_uof_obj.obj_hdr,
    ICP_QAT_UOF_IMEM, core::ptr::null_mut());
    if (chunk_hdr) {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_check_uof_compat(obj_handle: *mut icp_qat_uclo_objhandle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_init_globals(handle: *mut icp_qat_fw_loader_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_set_ae_mode(handle: *mut icp_qat_fw_loader_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_init_uword_num(handle: *mut icp_qat_fw_loader_handle) {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_parse_uof_obj(handle: *mut icp_qat_fw_loader_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_simg_hdr2sign_len(handle: *mut icp_qat_fw_loader_handle) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_simg_hdr2cont_len(handle: *mut icp_qat_fw_loader_handle) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_simg_fw_type(handle: *mut icp_qat_fw_loader_handle, img_ptr: *mut c_void) -> c_uint {
    ...
    int suof_size)
    {
    let mut check_sum: c_uint = 0;
    let mut min_ver_offset: c_uint = 0;
    ...
    {
    struct icp_qat_suof_handle *suof_handle = handle.sobj_handle;
    let mut offset: c_uint = qat_uclo_simg_hdr2cont_len(handle);
...
// sym_str = (char *)(uintptr_t)
    ...
    {
    struct icp_qat_simg_ae_mode *img_ae_mode = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_del_suof(handle: *mut icp_qat_fw_loader_handle) {
    ...
    sobj_handle.img_table.simg_hdr = core::ptr::null_mut();
    kfree(handle.sobj_handle);
    handle.sobj_handle = core::ptr::null_mut();
    }
    ...
    {
    struct icp_qat_suof_handle *suof_handle = handle.sobj_handle;
    struct icp_qat_suof_chunk_hdr *suof_chunk_hdr = core::ptr::null_mut();
    struct icp_qat_suof_img_hdr *suof_img_hdr = core::ptr::null_mut();
    let mut ret: c_int = 0, ae0_img = ICP_QAT_UCLO_MAX_AE;
    let mut i: c_uint = 0;
    struct icp_qat_suof_img_hdr img_header;
    if (!suof_ptr || suof_size == 0) {
    pr_err("input parameter SUOF pointer/size is core::ptr::null_mut()\n");
    return -EINVAL;
    }
    ...
    {
    let mut retries: c_uint = FW_AUTH_MAX_RETRY;
    ...
    {
    let mut ae_mask: c_ulong = handle.hal_handle.ae_mask;
    let mut desc_ae_mask: c_ulong = desc.ae_mask;
    ...
    let mut css_dword_size: c_uint = sizeof(u32);
...
// desc = auth_desc;
    ...
    void *addr_ptr, int mem_size)
    {
    struct icp_qat_fw_auth_desc *desc = core::ptr::null_mut();
    ...
    return 0;
    out_overlay_obj_err:
    handle.obj_handle = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_del_mof(handle: *mut icp_qat_fw_loader_handle) {
    static void qat_uclo_del_mof(struct icp_qat_fw_loader_handle *handle)
    {
    struct icp_qat_mof_handle *mobj_handle = handle.mobj_handle;
    kfree(mobj_handle.obj_table.obj_hdr);
    mobj_handle.obj_table.obj_hdr = core::ptr::null_mut();
    kfree(handle.mobj_handle);
    handle.mobj_handle = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_map_objs_from_mof(mobj_handle: *mut icp_qat_mof_handle) -> c_int {
    ...
    struct icp_qat_mof_objhdr *mobj_hdr;
    let mut uobj_chunk_num: c_uint = 0;
    let mut sobj_chunk_num: c_uint = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(chunk_id, _arg: ICP_QAT_UOF_OBJS, _arg: ICP_QAT_MOF_OBJ_ID_LEN)) -> else {
    else if (!strncmp(chunk_id, ICP_QAT_UOF_OBJS, ICP_QAT_MOF_OBJ_ID_LEN))
    mobj_handle.uobjs_hdr = mobj_handle.mof_buf +
    mof_chunkhdr.offset;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(chunk_id, _arg: ICP_QAT_SUOF_OBJS, _arg: ICP_QAT_MOF_OBJ_ID_LEN)) -> else {
    else if (!strncmp(chunk_id, ICP_QAT_SUOF_OBJS, ICP_QAT_MOF_OBJ_ID_LEN))
    mobj_handle.sobjs_hdr = mobj_handle.mof_buf +
    mof_chunkhdr.offset;
    }
#[no_mangle]
unsafe extern "C" fn qat_uclo_check_mof_format(mof_hdr: *mut icp_qat_mof_file_hdr) -> c_int {
    static int qat_uclo_check_mof_format(struct icp_qat_mof_file_hdr *mof_hdr)
    {
    let mut maj: c_int = mof_hdr.maj_ver & 0xff;
    let mut min: c_int = mof_hdr.min_ver & 0xff;
    ...
    {
    struct icp_qat_mof_chunkhdr *mof_chunkhdr;
    let mut file_id: c_uint = mof_ptr.file_id;
...
#[no_mangle]
pub unsafe extern "C" fn qat_uclo_del_obj(handle: *mut icp_qat_fw_loader_handle) {
...
// copy the buffer to ustore
    ...
    let mut ae_mask: c_ulong = handle.hal_handle.ae_mask;
    let mut cfg_ae_mask: c_ulong = handle.cfg_ae_mask;
    let mut ae_assigned: c_ulong = image.ae_assigned;
    struct icp_qat_uclo_aedata *aed;
...
// find the slice to which this image is assigned
    ...
    (ctx_mask & (1 << ctx)) ? page : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_wr_suof_img(handle: *mut icp_qat_fw_loader_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qat_uclo_wr_uof_img(handle: *mut icp_qat_fw_loader_handle) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qat_uclo_wr_all_uimage(handle: *mut icp_qat_fw_loader_handle) -> c_int {
