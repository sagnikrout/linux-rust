...
//! Source: drivers/crypto/ccp/ccp-crypto-sha.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ccp_sha_complete(async_req: *mut crypto_async_request, ret: c_int) -> c_int {
    static int ccp_sha_complete(struct crypto_async_request *async_req, int ret)
    {
    struct ahash_request *req = ahash_request_cast(async_req);
    struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
    struct ccp_sha_req_ctx *rctx = ahash_request_ctx_dma(req);
    let mut digest_size: c_uint = crypto_ahash_digestsize(tfm);
    if (ret)
    goto e_free;
    if (rctx.hash_rem) {
// Save remaining data to buffer
    let mut offset: c_uint = rctx.nbytes - rctx.hash_rem;
...
// Should never get here
    ...
    &ctx.u.sha.opad_sg : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ccp_sha_init(req: *mut ahash_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccp_sha_export(req: *mut ahash_request, out: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccp_sha_import(req: *mut ahash_request, in: *const c_void) -> c_int {
...
// 'in' may not be aligned so memcpy to local variable
    ...
    let mut block_size: c_uint = crypto_shash_blocksize(shash);
    let mut digest_size: c_uint = crypto_shash_digestsize(shash);
    int i, ret;
...
#[no_mangle]
unsafe extern "C" fn ccp_sha_cra_init(tfm: *mut crypto_tfm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccp_sha_cra_exit(tfm: *mut crypto_tfm) {
    static void ccp_sha_cra_exit(struct crypto_tfm *tfm)
    {
    }
#[no_mangle]
unsafe extern "C" fn ccp_hmac_sha_cra_init(tfm: *mut crypto_tfm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccp_hmac_sha_cra_exit(tfm: *mut crypto_tfm) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ccp_sha_def {
    pub version: c_uint,
    pub name: *const c_char,
    pub drv_name: *const c_char,
    pub type: enum ccp_sha_type,
    pub digest_size: u32,
    pub block_size: u32,
}
...
#[no_mangle]
pub unsafe extern "C" fn ccp_register_sha_algs(head: *mut list_head) -> c_int {
