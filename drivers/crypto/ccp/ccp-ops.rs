...
//! Source: drivers/crypto/ccp/ccp-ops.c
#![no_std]
...
// SHA initial context values
    ...
    ccp_gen_jobid(ccp) : 0)
#[no_mangle]
unsafe extern "C" fn ccp_gen_jobid(ccp: *mut ccp_device) -> u32 {
    static u32 ccp_gen_jobid(struct ccp_device *ccp)
    {
    return atomic_inc_return(&ccp.current_id) & CCP_JOBID_MASK;
    }
#[no_mangle]
unsafe extern "C" fn ccp_sg_free(wa: *mut ccp_sg_workarea) {
...
#[no_mangle]
unsafe extern "C" fn ccp_update_sg_workarea(wa: *mut ccp_sg_workarea, len: c_uint) {
    static void ccp_update_sg_workarea(struct ccp_sg_workarea *wa, unsigned int len)
    {
    let mut nbytes: c_uint = min_t(u64, len, wa.bytes_left);
    let mut sg_combined_len: c_uint = 0;
    if (!wa.sg)
    return;
    wa.sg_used += nbytes;
...
#[no_mangle]
unsafe extern "C" fn ccp_dm_free(wa: *mut ccp_dm_workarea) {
...
#[no_mangle]
unsafe extern "C" fn ccp_free_data(data: *mut ccp_data, cmd_q: *mut ccp_cmd_queue) {
...
#[no_mangle]
unsafe extern "C" fn ccp_queue_buf(data: *mut ccp_data, from: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn ccp_fill_queue_buf(data: *mut ccp_data) -> c_uint {
    static unsigned int ccp_fill_queue_buf(struct ccp_data *data)
    {
    return ccp_queue_buf(data, 0);
    }
#[no_mangle]
unsafe extern "C" fn ccp_empty_queue_buf(data: *mut ccp_data) -> c_uint {
...
//
    let mut cp_len: c_int = ccp_fill_queue_buf(src);
    op.soc = 1;
...
// Send data to the CCP AES engine
    ...
    ccp_prepare_data(&src, core::ptr::null_mut(), &op, AES_BLOCK_SIZE, true);
...
// Push the K1/K2 key to the CCP now
    ...
    ccp_process_data(&src, core::ptr::null_mut(), &op);
    }
...
// Step 1: Run a GHASH over the Additional Authenticated Data
    ...
    ccp_prepare_data(&wa.aad, core::ptr::null_mut(), &wa.op, AES_BLOCK_SIZE, true);
    ret = cmd_q.ccp.vdata.perform.aes(&wa.op);
    if (ret) {
    cmd.engine_error = cmd_q.cmd_error;
    goto e_aad;
    }
    ccp_process_data(&wa.aad, core::ptr::null_mut(), &wa.op);
    wa.op.init = 0;
    }
    }
...
// Step 2: Run a GCTR over the plaintext
    ...
    let mut nbytes: c_uint = ilen % AES_BLOCK_SIZE;
    if (nbytes) {
    wa.op.eom = 1;
...
#[no_mangle]
pub unsafe extern "C" fn if(AES_KEYSIZE_256: xts->key_len ==) -> else {
...
// Send data to the CCP SHA engine; block_size is set above
    ...
    goto e_ctx;
    while (src.sg_wa.bytes_left) {
    ccp_prepare_data(&src, core::ptr::null_mut(), &op, block_size, false);
...
// HMAC operation, recursively perform final SHA
    ...
    hmac_cmd.u.sha.opad = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ccp_run_ecc_mm_cmd(cmd_q: *mut ccp_cmd_queue, cmd: *mut ccp_cmd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccp_run_ecc_pm_cmd(cmd_q: *mut ccp_cmd_queue, cmd: *mut ccp_cmd) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ccp_run_cmd(cmd_q: *mut ccp_cmd_queue, cmd: *mut ccp_cmd) -> c_int {
