...
//! Source: drivers/crypto/marvell/cesa/tdma.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn mv_cesa_dma_step(dreq: *mut mv_cesa_req) {
...
#[no_mangle]
pub unsafe extern "C" fn mv_cesa_dma_cleanup(dreq: *mut mv_cesa_req) {
    void mv_cesa_dma_cleanup(struct mv_cesa_req *dreq)
    {
    struct mv_cesa_tdma_desc *tdma;
    for (tdma = dreq.chain.first; tdma;) {
    struct mv_cesa_tdma_desc *old_tdma = tdma;
    let mut type: u32 = tdma.flags & CESA_TDMA_TYPE_MSK;
...
#[no_mangle]
pub unsafe extern "C" fn mv_cesa_tdma_process(engine: *mut mv_cesa_engine, status: u32) -> c_int {
    int mv_cesa_tdma_process(struct mv_cesa_engine *engine, u32 status)
    {
    struct crypto_async_request *req = core::ptr::null_mut();
    struct mv_cesa_tdma_desc *tdma = core::ptr::null_mut(), *next = core::ptr::null_mut();
    dma_addr_t tdma_cur;
    let mut res: c_int = 0;
    ...
    struct crypto_async_request *backlog = core::ptr::null_mut();
...
// Re-chaining to the next request
    ...
    tdma.next = core::ptr::null_mut();
// If this is the last request, clear the chain
    if (engine.chain_hw.first == core::ptr::null_mut())
    engine.chain_hw.last  = core::ptr::null_mut();
    spin_unlock_bh(&engine.lock);
...
//
    ...
    {
    struct mv_cesa_tdma_desc *new_tdma = core::ptr::null_mut();
...
//
    ...
    let mut type: u32 = op_desc.flags & CESA_TDMA_TYPE_MSK;
    if (type == CESA_TDMA_OP)
    break;
    }
    if (!op_desc)
...
#[no_mangle]
pub unsafe extern "C" fn mv_cesa_dma_add_dummy_launch(chain: *mut mv_cesa_tdma_chain, flags: gfp_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mv_cesa_dma_add_dummy_end(chain: *mut mv_cesa_tdma_chain, flags: gfp_t) -> c_int {
