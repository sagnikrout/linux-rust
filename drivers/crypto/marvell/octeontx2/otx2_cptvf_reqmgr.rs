...
//! Source: drivers/crypto/marvell/octeontx2/otx2_cptvf_reqmgr.c
#![no_std]
...
pub const CPT_PENTRY_TIMEOUT: c_int = 1000;
pub const CPT_PENTRY_STEP: c_int = 50;
// Default threshold for stopping and resuming sender requests
pub const CPT_IQ_STOP_MARGIN: c_int = 128;
pub const CPT_IQ_RESUME_MARGIN: c_int = 512;
// Default command timeout in seconds
pub const CPT_COMMAND_TIMEOUT: c_int = 4;
pub const CPT_TIME_IN_RESET_COUNT: c_int = 5;
    ...
    int qlen)
    {
    struct otx2_cpt_pending_entry *ent = core::ptr::null_mut();
    ent = &q.head[q.rear];
...
#[no_mangle]
pub unsafe extern "C" fn modulo_inc(index: u32, length: u32, inc: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn free_pentry(pentry: *mut otx2_cpt_pending_entry) {
    ...
    pentry.callback = core::ptr::null_mut();
    ...
    struct otx2_cpt_pending_entry *pentry = core::ptr::null_mut();
    union otx2_cpt_ctrl_info *ctrl = &req.ctrl;
    struct otx2_cpt_inst_info *info = core::ptr::null_mut();
    union otx2_cpt_res_s *result = core::ptr::null_mut();
...
//
    ...
    u32 *res_code)
    {
    let mut uc_ccode: u8 = lfs.ops.cpt_get_uc_compcode(cpt_status);
    let mut ccode: u8 = lfs.ops.cpt_get_compcode(cpt_status);
...
#[no_mangle]
pub unsafe extern "C" fn if(CPT_TIME_IN_RESET_COUNT: info->extra_time <) -> else {
    info.time_in = jiffies;
    info.extra_time++;
    }
    return 1;
...
// res_code = 0;
    ...
    struct otx2_cpt_pending_queue *pqueue)
    {
    struct otx2_cpt_pending_entry *resume_pentry = core::ptr::null_mut();
    void (*callback)(int status, void *arg, void *req);
    struct otx2_cpt_pending_entry *pentry = core::ptr::null_mut();
    union otx2_cpt_res_s *cpt_status = core::ptr::null_mut();
    struct otx2_cpt_inst_info *info = core::ptr::null_mut();
    struct otx2_cpt_req_info *req = core::ptr::null_mut();
    ...
    dev_err(&pdev.dev, "Callback core::ptr::null_mut()\n");
    goto process_pentry;
    }
    info = pentry.info;
    if (unlikely(!info)) {
    dev_err(&pdev.dev, "Pending entry post arg core::ptr::null_mut()\n");
    goto process_pentry;
    }
    req = info.req;
    if (unlikely(!req)) {
    dev_err(&pdev.dev, "Request core::ptr::null_mut()\n");
    goto process_pentry;
    }
    cpt_status = pentry.completion_addr;
    if (unlikely(!cpt_status)) {
    dev_err(&pdev.dev, "Completion address core::ptr::null_mut()\n");
    goto process_pentry;
    }
...
#[no_mangle]
pub unsafe extern "C" fn otx2_cpt_post_process(wqe: *mut otx2_cptlf_wqe) {
