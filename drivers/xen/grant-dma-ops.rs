...
//! Source: drivers/xen/grant-dma-ops.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xen_grant_dma_data {
// The ID of backend domain
    pub backend_domid: domid_t,
...
#[no_mangle]
pub unsafe extern "C" fn grant_to_dma(grant: grant_ref_t) -> dma_addr_t {
    static inline dma_addr_t grant_to_dma(grant_ref_t grant)
    {
    return XEN_GRANT_DMA_ADDR_OFF | ((dma_addr_t)grant << XEN_PAGE_SHIFT);
    }
#[no_mangle]
pub unsafe extern "C" fn dma_to_grant(dma: dma_addr_t) -> grant_ref_t {
...
// dma_handle = grant_to_dma(grant);
    ...
    {
    struct xen_grant_dma_data *data;
    let mut offset: c_ulong = offset_in_page(phys);
    ...
    {
    struct xen_grant_dma_data *data;
    let mut dma_offset: c_ulong = xen_offset_in_page(dma_handle);
    unsigned int i, n_pages = XEN_PFN_UP(dma_offset + size);
    grant_ref_t grant;
    if (WARN_ON(dir == DMA_NONE))
    return;
...
#[no_mangle]
unsafe extern "C" fn xen_grant_dma_supported(dev: *mut device, mask: u64) -> c_int {
...
// Walk up to the root bus to look for PCI Host controller
    ...
    if (dev_is_pci(dev)) {
    struct pci_dev *pdev = to_pci_dev(dev);
    let mut rid: u32 = PCI_DEVID(pdev.bus.number, pdev.devfn);
...
#[no_mangle]
unsafe extern "C" fn xen_grant_setup_dma_ops(dev: *mut device, backend_domid: domid_t) {
...
#[no_mangle]
pub unsafe extern "C" fn xen_virtio_restricted_mem_acc(dev: *mut virtio_device) -> bool {
