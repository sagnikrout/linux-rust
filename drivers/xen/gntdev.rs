...
pub const GNTDEV_COPY_BATCH: c_int = 16;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gntdev_copy_batch {
    pub ops: [gnttab_copy; GNTDEV_COPY_BATCH],
    pub pages: [*mut page; GNTDEV_COPY_BATCH],
    pub status: [*mut s16 __user; GNTDEV_COPY_BATCH],
    pub nr_ops: c_uint,
    pub nr_pages: c_uint,
    pub writeable: bool,
    pub next: *mut gntdev_copy_batch,
}

    let mut limit: static unsigned int = 64*1024;
...
// ------------------------------------------------------------------
#[no_mangle]
pub unsafe extern "C" fn gntdev_test_page_count(count: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn gntdev_free_map(map: *mut gntdev_grant_map) {
    ...
    if (core::ptr::null_mut() == add.grants    ||
    core::ptr::null_mut() == add.map_ops   ||
    core::ptr::null_mut() == add.unmap_ops ||
    core::ptr::null_mut() == add.pages     ||
    core::ptr::null_mut() == add.being_removed)
    goto err;
    if (xen_pv_domain()) {
    add.kmap_ops   = kvmalloc_objs(add.kmap_ops[0], count);
    add.kunmap_ops = kvmalloc_objs(add.kunmap_ops[0], count);
    if (core::ptr::null_mut() == add.kmap_ops || core::ptr::null_mut() == add.kunmap_ops)
    goto err;
    }
...
#[no_mangle]
pub unsafe extern "C" fn gntdev_add_map(priv: *mut gntdev_priv, add: *mut gntdev_grant_map) {
...
#[no_mangle]
pub unsafe extern "C" fn gntdev_put_map(priv: *mut gntdev_priv, map: *mut gntdev_grant_map) {
...
// ------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn find_grant_ptes(pte: *mut pte_t, addr: c_ulong, data: *mut c_void) -> c_int {
    static int find_grant_ptes(pte_t *pte, unsigned long addr, void *data)
    {
    struct gntdev_grant_map *map = data;
    let mut pgnr: c_uint = (addr - map.pages_vm_start) >> PAGE_SHIFT;
...
#[no_mangle]
pub unsafe extern "C" fn gntdev_map_grant_pages(map: *mut gntdev_grant_map) -> c_int {
    int gntdev_map_grant_pages(struct gntdev_grant_map *map)
    {
    let mut alloced: usize = 0;
    int i, err = 0;
    if (!xen_pv_domain()) {
...
//
    ...
    let mut offset: c_uint = data.unmap_ops - map.unmap_ops;
    let mut successful_unmaps: c_int = 0;
    int live_grants;
...
// Release reference taken by __unmap_grant_pages
    gntdev_put_map(core::ptr::null_mut(), map);
    }
    static void __unmap_grant_pages(struct gntdev_grant_map *map, int offset,
    int pages)
    {
    if (map.notify.flags & UNMAP_NOTIFY_CLEAR_BYTE) {
    let mut pgno: c_int = (map.notify.addr >> PAGE_SHIFT);
...
// No need for kmap, pages are in lowmem
    ...
    map.unmap_data.kunmap_ops = xen_pv_domain() ? map.kunmap_ops + offset : core::ptr::null_mut();
...
// ------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn gntdev_vma_open(vma: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn gntdev_vma_close(vma: *mut vm_area_struct) {
...
// ------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn gntdev_open(inode: *mut inode, flip: *mut file) -> c_int {
    ...
    let mut ret: c_int = PTR_ERR(priv.dmabuf_priv);
    kfree(priv);
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn gntdev_release(inode: *mut inode, flip: *mut file) -> c_int {
    ...
    list_del(&map.next);
    gntdev_put_map(core::ptr::null_mut() /* already removed */, map);
    }
...
#[no_mangle]
unsafe extern "C" fn gntdev_ioctl_notify(priv: *mut gntdev_priv, u: *mut void __user) -> c_long {
...
//
    ...
    mutex_lock(&priv.lock);
    list_for_each_entry(map, &priv.maps, next) {
    let mut begin: u64 = map.index << PAGE_SHIFT;
    let mut end: u64 = (map.index + map.count) << PAGE_SHIFT;
...
// Drop the reference to the event channel we did not save in the map
    ...
    unsigned long *gfn)
    {
    let mut addr: c_ulong = (unsigned long)virt;
    struct page *page;
...
#[no_mangle]
unsafe extern "C" fn gntdev_put_pages(batch: *mut gntdev_copy_batch) {
...
#[no_mangle]
unsafe extern "C" fn gntdev_copy(batch: *mut gntdev_copy_batch) -> c_int {
...
//
    ...
    let mut status: i16 = batch.ops[i].status;
    s16 old_status;
...
#[no_mangle]
unsafe extern "C" fn gntdev_ioctl_grant_copy(priv: *mut gntdev_priv, u: *mut void __user) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn gntdev_mmap(flip: *mut file, vma: *mut vm_area_struct) -> c_int {
    static int gntdev_mmap(struct file *flip, struct vm_area_struct *vma)
    {
    struct gntdev_priv *priv = flip.private_data;
    let mut index: c_int = vma.vm_pgoff;
    let mut count: c_int = vma_pages(vma);
    struct gntdev_grant_map *map;
...
// ------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn gntdev_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn gntdev_exit() -> void __exit {
