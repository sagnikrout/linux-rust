...
#[no_mangle]
unsafe extern "C" fn xen_efi_get_time(tm: *mut efi_time_t, tc: *mut efi_time_cap_t) -> efi_status_t {
    static efi_status_t xen_efi_get_time(efi_time_t *tm, efi_time_cap_t *tc)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(get_time);
...
#[no_mangle]
unsafe extern "C" fn xen_efi_set_time(tm: *mut efi_time_t) -> efi_status_t {
    static efi_status_t xen_efi_set_time(efi_time_t *tm)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(set_time);
    ...
    efi_time_t *tm)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(get_wakeup_time);
    if (HYPERVISOR_platform_op(&op) < 0)
    return EFI_UNSUPPORTED;
    if (tm) {
...
#[no_mangle]
unsafe extern "C" fn xen_efi_set_wakeup_time(enabled: efi_bool_t, tm: *mut efi_time_t) -> efi_status_t {
    static efi_status_t xen_efi_set_wakeup_time(efi_bool_t enabled, efi_time_t *tm)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(set_wakeup_time);
    ...
    void *data)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(get_variable);
...
// attr = efi_data(op).misc;
    ...
    efi_guid_t *vendor)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(get_next_variable_name);
...
// name_size = efi_data(op).u.get_next_variable_name.size;
    ...
    void *data)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(set_variable);
    ...
    {
    let mut op: xen_platform_op = INIT_EFI_OP(query_variable_info);
...
#[no_mangle]
unsafe extern "C" fn xen_efi_get_next_high_mono_count(count: *mut u32) -> efi_status_t {
    static efi_status_t xen_efi_get_next_high_mono_count(u32 *count)
    {
    let mut op: xen_platform_op = INIT_EFI_OP(get_next_high_monotonic_count);
    if (HYPERVISOR_platform_op(&op) < 0)
    return EFI_UNSUPPORTED;
// count = efi_data(op).misc;
    ...
    {
    let mut op: xen_platform_op = INIT_EFI_OP(update_capsule);
    if (efi.runtime_version < EFI_2_00_SYSTEM_TABLE_REVISION)
    return EFI_UNSUPPORTED;
    ...
    {
    let mut op: xen_platform_op = INIT_EFI_OP(query_capsule_capabilities);
    if (efi.runtime_version < EFI_2_00_SYSTEM_TABLE_REVISION)
    return EFI_UNSUPPORTED;
...
//
#[no_mangle]
pub unsafe extern "C" fn xen_efi_runtime_setup() -> void __init {
