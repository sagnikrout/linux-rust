...
//! Source: drivers/accessibility/speakup/speakup_soft.c
#![no_std]
...
pub const CLEAR_SYNTH: c_uint = 0x18;
    ...
    [RATE_ID]  = { RATE, .u.n = {"\x01%ds", 2, 0, 9, 0, 0, core::ptr::null_mut() } },
    [PITCH_ID]  = { PITCH, .u.n = {"\x01%dp", 5, 0, 9, 0, 0, core::ptr::null_mut() } },
    [INFLECTION_ID]  = { INFLECTION, .u.n = {"\x01%dr", 5, 0, 9, 0, 0, core::ptr::null_mut() } },
    [VOL_ID]  = { VOL, .u.n = {"\x01%dv", 5, 0, 9, 0, 0, core::ptr::null_mut() } },
    [TONE_ID]  = { TONE, .u.n = {"\x01%dx", 1, 0, 2, 0, 0, core::ptr::null_mut() } },
    [PUNCT_ID]  = { PUNCT, .u.n = {"\x01%db", 0, 0, 3, 0, 0, core::ptr::null_mut() } },
...
// &lang_attribute.attr,
    ...
    .vars = vars,
    .io_ops = core::ptr::null_mut(),
    .probe = softsynth_probe,
    .release = softsynth_release,
    .synth_immediate = core::ptr::null_mut(),
    .catch_up = core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn softsynth_open(inode: *mut inode, fp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn softsynth_close(inode: *mut inode, fp: *mut file) -> c_int {
...
// Make sure we let applications go before leaving
    ...
    {
    let mut chars_sent: c_int = 0;
    char __user *cp;
    char *init;
    let mut bytes_per_ch: usize = unicode ? 3 : 1;
    u16 ch;
    int empty;
    unsigned long flags;
    DEFINE_WAIT(wait);
...
// pos = 0;
    ...
    {
    let mut supplied_index: c_ulong = 0;
    int converted;
...
#[no_mangle]
unsafe extern "C" fn softsynth_poll(fp: *mut file, wait: *mut poll_table_struct) -> __poll_t {
...
#[no_mangle]
unsafe extern "C" fn get_index(synth: *mut spk_synth) -> c_uchar {
    static unsigned char get_index(struct spk_synth *synth)
    {
    int rv;
    rv = last_index;
...
#[no_mangle]
unsafe extern "C" fn softsynth_probe(synth: *mut spk_synth) -> c_int {
    static int softsynth_probe(struct spk_synth *synth)
    {
...
#[no_mangle]
unsafe extern "C" fn softsynth_release(synth: *mut spk_synth) {
    static void softsynth_release(struct spk_synth *synth)
    {
    misc_deregister(&synth_device);
...
#[no_mangle]
unsafe extern "C" fn softsynth_is_alive(synth: *mut spk_synth) -> c_int {
    static int softsynth_is_alive(struct spk_synth *synth)
    {
    if (synth_soft.alive)
    return 1;
...
#[no_mangle]
unsafe extern "C" fn softsynth_adjust(synth: *mut spk_synth, var: *mut st_var_header) -> c_int {
    static int softsynth_adjust(struct spk_synth *synth, struct st_var_header *var)
    {
