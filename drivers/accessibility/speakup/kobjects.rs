...
//! Source: drivers/accessibility/speakup/kobjects.c
#![no_std]
...
//
    ...
    let mut bufsize: usize = PAGE_SIZE;
    unsigned long flags;
...
// buf_pointer = '\0';
    ...
    cp = "B_CTL";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: WDLM)) -> else {
    else if (IS_TYPE(i, WDLM))
    cp = "WDLM";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: A_PUNC)) -> else {
    else if (IS_TYPE(i, A_PUNC))
    cp = "A_PUNC";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: PUNC)) -> else {
    else if (IS_TYPE(i, PUNC))
    cp = "PUNC";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: NUM)) -> else {
    else if (IS_TYPE(i, NUM))
    cp = "NUM";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: A_CAP)) -> else {
    else if (IS_TYPE(i, A_CAP))
    cp = "A_CAP";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: ALPHA)) -> else {
    else if (IS_TYPE(i, ALPHA))
    cp = "ALPHA";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: B_CAPSYM)) -> else {
    else if (IS_TYPE(i, B_CAPSYM))
    cp = "B_CAPSYM";
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_TYPE(i, _arg: B_SYM)) -> else {
...
//
    ...
    char *linefeed = core::ptr::null_mut();
    char keyword[MAX_DESC_LEN + 1];
    char *outptr = core::ptr::null_mut();	/* Will hold keyword or desc. */
    char *temp = core::ptr::null_mut();
    char *desc = core::ptr::null_mut();
    let mut retval: isize = count;
    unsigned long flags;
    let mut index: c_ulong = 0;
    let mut charclass: u16 = 0;
    let mut received: c_int = 0;
    let mut used: c_int = 0;
    let mut rejected: c_int = 0;
    let mut reset: c_int = 0;
    let mut do_characters: c_int = !strcmp(attr.attr.name, "characters");
    let mut desc_length: usize = 0;
    int i;
    spin_lock_irqsave(&speakup_info.spinlock, flags);
    while (cp < end) {
    while ((cp < end) && (*cp == ' ' || *cp == '\t'))
...
//
    ...
    char *in_buff = core::ptr::null_mut();
    char *cp;
    u_char *cp1;
    unsigned long flags;
    spin_lock_irqsave(&speakup_info.spinlock, flags);
...
//
    ...
    x = spk_set_mask_bits(core::ptr::null_mut(), var.value, 3);
    else
...
// buf_pointer = '\0'; /* buf_pointer always looking at a NUL byte.
    ...
    let mut rejected: c_int = 0;
    let mut reset: c_int = 0;
    let mut firstmessage: enum msg_index_t = group.start;
    let mut lastmessage: enum msg_index_t = group.end;
...
#[no_mangle]
pub unsafe extern "C" fn speakup_kobj_init() -> c_int {
...
//
    accessibility_kobj = kobject_create_and_add("accessibility", core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn speakup_kobj_exit() {
