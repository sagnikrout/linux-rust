...
//! Source: drivers/accessibility/speakup/speakup_acntpc.c
#![no_std]
...
//
    ...
    [VOL_ID] = { VOL, .u.n = {"\033A%d", 5, 0, 9, 0, 0, core::ptr::null_mut() } },
...
pub unsafe extern "C" fn synth_full() -> bool {
    ...
    u_char ch;
    while ((ch = *buf)) {
    let mut timeout: c_int = SPK_XMITR_TIMEOUT;
    if (ch == '\n')
    ch = PROCSPEECH;
    if (synth_full())
    return buf;
...
#[no_mangle]
unsafe extern "C" fn accent_release(synth: *mut spk_synth) {
    static void accent_release(struct spk_synth *synth)
    {
