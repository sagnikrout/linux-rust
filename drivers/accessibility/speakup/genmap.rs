...
//! Source: drivers/accessibility/speakup/genmap.c
#![no_std]
...
pub struct st_key_init {
    ...
    let mut max_states: static int = 1, flags;
// flags reserved for later, maybe for individual console maps
#[no_mangle]
unsafe extern "C" fn get_shift_value(state: c_int) -> c_int {
    ...
    if (shift_table[i] == -1) {
    if (i >= 16)
    oops("too many shift states", core::ptr::null_mut());
    shift_table[i] = state;
    max_states = i+1;
    break;
    }
    ...
    let mut max_key_used: c_int = 0, num_keys_used = 0;
    struct st_key *this;
    ...
    open_input(core::ptr::null_mut(), argv[1]);
    ...
    if (!cp)
    oops("no = found", core::ptr::null_mut());
    cp = strtok(0, delims);
    if (!cp)
    oops("no speakup function after =", core::ptr::null_mut());
    this = find_key(cp);
    if (this == core::ptr::null_mut() || this.shift != is_spk)
