...
//! Source: drivers/ata/libata-pmp.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_read(link: *mut ata_link, reg: c_int, r_val: *mut u32) -> c_uint {
    ...
    tf.feature = reg;
    tf.device = link.pmp;
    err_mask = ata_exec_internal(pmp_dev, &tf, core::ptr::null_mut(), DMA_NONE, core::ptr::null_mut(), 0,
    SATA_PMP_RW_TIMEOUT);
    if (err_mask)
    return err_mask;
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_write(link: *mut ata_link, reg: c_int, val: u32) -> c_uint {
    ...
    tf.lbam = (val >> 16) & 0xff;
    tf.lbah = (val >> 24) & 0xff;
    return ata_exec_internal(pmp_dev, &tf, core::ptr::null_mut(), DMA_NONE, core::ptr::null_mut(), 0,
    SATA_PMP_RW_TIMEOUT);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn sata_pmp_qc_defer_cmd_switch(qc: *mut ata_queued_cmd) -> c_int {
    int sata_pmp_qc_defer_cmd_switch(struct ata_queued_cmd *qc)
    {
    struct ata_link *link = qc.dev.link;
    struct ata_port *ap = link.ap;
    int ret;
    if (ap.excl_link == core::ptr::null_mut() || ap.excl_link == link) {
...
//
#[no_mangle]
pub unsafe extern "C" fn sata_pmp_scr_read(link: *mut ata_link, reg: c_int, r_val: *mut u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn sata_pmp_scr_write(link: *mut ata_link, reg: c_int, val: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_read_gscr(dev: *mut ata_device, gscr: *mut u32) -> c_int {
    static int sata_pmp_read_gscr(struct ata_device *dev, u32 *gscr)
    {
    static const int gscr_to_read[] = { 0, 1, 2, 32, 33, 64, 96 };
    int i;
    for (i = 0; i < ARRAY_SIZE(gscr_to_read); i++) {
    let mut reg: c_int = gscr_to_read[i];
    unsigned int err_mask;
    ...
    {
    let mut rev: u32 = gscr[SATA_PMP_GSCR_REV];
    if (rev & (1 << 3))
    return "1.2";
    if (rev & (1 << 2))
    return "1.1";
...
pub const PMP_GSCR_SII_POL: c_int = 129;
#[no_mangle]
unsafe extern "C" fn sata_pmp_configure(dev: *mut ata_device, print_info: c_int) -> c_int {
    static int sata_pmp_configure(struct ata_device *dev, int print_info)
    {
    struct ata_port *ap = dev.link.ap;
    u32 *gscr = dev.gscr;
    let mut vendor: u16 = sata_pmp_gscr_vendor(gscr);
    let mut devid: u16 = sata_pmp_gscr_devid(gscr);
...
#[no_mangle]
unsafe extern "C" fn sata_pmp_init_links(ap: *mut ata_port, nr_ports: c_int) -> c_int {
    ...
    kfree(pmp_link);
    ap.pmp_link = core::ptr::null_mut();
    return err;
    }
#[no_mangle]
unsafe extern "C" fn sata_pmp_quirks(ap: *mut ata_port) {
...
//
#[no_mangle]
pub unsafe extern "C" fn sata_pmp_attach(dev: *mut ata_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_detach(dev: *mut ata_device) {
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_same_pmp(dev: *mut ata_device, new_gscr: *const u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_revalidate(dev: *mut ata_device, new_class: c_uint) -> c_int {
    static int sata_pmp_revalidate(struct ata_device *dev, unsigned int new_class)
    {
    struct ata_link *link = dev.link;
    u32 *gscr = (void *)dev.sector_buf;
    int rc;
    ata_eh_about_to_do(link, core::ptr::null_mut(), ATA_EH_REVALIDATE);
...
// is the pmp still there?
    ...
    if (rc)
    goto fail;
    ata_eh_done(link, core::ptr::null_mut(), ATA_EH_REVALIDATE);
    return 0;
    fail:
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_revalidate_quick(dev: *mut ata_device) -> c_int {
...
//
    ...
    let mut tries: c_int = ATA_EH_PMP_TRIES;
    let mut detach: c_int = 0, rc = 0;
    let mut reval_failed: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn sata_pmp_eh_handle_disabled_links(ap: *mut ata_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sata_pmp_handle_link_fail(link: *mut ata_link, link_tries: *mut c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sata_pmp_eh_recover(ap: *mut ata_port) -> c_int {
...
// PMP attached?
    ...
    rc = ata_eh_recover(ap, &ops.reset, core::ptr::null_mut());
    if (rc) {
...
//
#[no_mangle]
pub unsafe extern "C" fn sata_pmp_error_handler(ap: *mut ata_port) {
