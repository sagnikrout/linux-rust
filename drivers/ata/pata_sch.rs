...
//
#[no_mangle]
unsafe extern "C" fn sch_set_piomode(ap: *mut ata_port, adev: *mut ata_device) {
    static void sch_set_piomode(struct ata_port *ap, struct ata_device *adev)
    {
    let mut pio: c_uint = adev.pio_mode - XFER_PIO_0;
    struct pci_dev *dev	= to_pci_dev(ap.host.dev);
    let mut port: c_uint = adev.devno ? D1TIM : D0TIM;
    unsigned int data;
...
//
#[no_mangle]
unsafe extern "C" fn sch_set_dmamode(ap: *mut ata_port, adev: *mut ata_device) {
    static void sch_set_dmamode(struct ata_port *ap, struct ata_device *adev)
    {
...
//
#[no_mangle]
unsafe extern "C" fn sch_init_one(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int sch_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
    const struct ata_port_info *ppi[] = { &sch_port_info, core::ptr::null_mut() };
    ata_print_version_once(&pdev.dev, DRV_VERSION);
    return ata_pci_bmdma_init_one(pdev, ppi, &sch_sht, core::ptr::null_mut(), 0);
    }
