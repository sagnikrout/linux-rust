...
//! Source: drivers/ata/pata_cmd64x.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cmd648_cable_detect(ap: *mut ata_port) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn cmd64x_set_timing(ap: *mut ata_port, adev: *mut ata_device, mode: u8) {
    static void cmd64x_set_timing(struct ata_port *ap, struct ata_device *adev, u8 mode)
    {
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    struct ata_timing t;
    let mut T: c_ulong = 1000000 / 33;
...
// Port layout is not logical so use a table
    ...
    let mut arttim: c_int = arttim_port[ap.port_no][adev.devno];
    let mut drwtim: c_int = drwtim_port[ap.port_no][adev.devno];
...
// Now convert the clocks into values we can actually stuff into
    ...
    t.recover = 0;
#[no_mangle]
pub unsafe extern "C" fn if(1: t.recover >) -> else {
    else if (t.recover > 1)
    t.recover--;
    else
    t.recover = 15;
...
//
#[no_mangle]
unsafe extern "C" fn cmd64x_set_piomode(ap: *mut ata_port, adev: *mut ata_device) {
...
//
#[no_mangle]
unsafe extern "C" fn cmd64x_set_dmamode(ap: *mut ata_port, adev: *mut ata_device) {
    ...
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    u8 regU, regD;
    let mut pciU: c_int = UDIDETCR0 + 8 * ap.port_no;
    let mut pciD: c_int = BMIDESR0 + 8 * ap.port_no;
    let mut shift: c_int = 2 * adev.devno;
...
//
#[no_mangle]
unsafe extern "C" fn cmd64x_sff_irq_check(ap: *mut ata_port) -> bool {
    static bool cmd64x_sff_irq_check(struct ata_port *ap)
    {
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    let mut irq_mask: c_int = ap.port_no ? ARTTIM23_INTR_CH1 : CFR_INTR_CH0;
    let mut irq_reg: c_int = ap.port_no ? ARTTIM23 : CFR;
    u8 irq_stat;
...
//
#[no_mangle]
unsafe extern "C" fn cmd64x_sff_irq_clear(ap: *mut ata_port) {
...
//
#[no_mangle]
unsafe extern "C" fn cmd648_sff_irq_check(ap: *mut ata_port) -> bool {
    static bool cmd648_sff_irq_check(struct ata_port *ap)
    {
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    let mut base: c_ulong = pci_resource_start(pdev, 4);
    let mut irq_mask: c_int = ap.port_no ? MRDMODE_INTR_CH1 : MRDMODE_INTR_CH0;
    let mut mrdmode: u8 = inb(base + 1);
...
//
#[no_mangle]
unsafe extern "C" fn cmd648_sff_irq_clear(ap: *mut ata_port) {
...
//
#[no_mangle]
unsafe extern "C" fn cmd646r1_bmdma_stop(qc: *mut ata_queued_cmd) {
...
#[no_mangle]
unsafe extern "C" fn cmd64x_fixup(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn cmd64x_init_one(pdev: *mut pci_dev, id: *const pci_device_id) -> c_int {
...
// all (with exceptions below) apart from 643 have CNTRL_CH0 bit
    let mut cntrl_ch0_ok: c_int = (id.driver_data != 0);
...
// check for enabled ports
    ...
    return ata_pci_bmdma_init_one(pdev, ppi, &cmd64x_sht, core::ptr::null_mut(), 0);
    }

#[no_mangle]
unsafe extern "C" fn cmd64x_reinit_one(pdev: *mut pci_dev) -> c_int {
