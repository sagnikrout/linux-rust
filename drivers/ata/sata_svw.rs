...
#[no_mangle]
unsafe extern "C" fn k2_sata_check_atapi_dma(qc: *mut ata_queued_cmd) -> c_int {
    static int k2_sata_check_atapi_dma(struct ata_queued_cmd *qc)
    {
    let mut cmnd: u8 = qc.scsicmd.cmnd[0];
...
#[no_mangle]
unsafe extern "C" fn k2_sata_tf_load(ap: *mut ata_port, tf: *const ata_taskfile) {
...
#[no_mangle]
unsafe extern "C" fn k2_sata_tf_read(ap: *mut ata_port, tf: *mut ata_taskfile) {
...
//
#[no_mangle]
unsafe extern "C" fn k2_bmdma_setup_mmio(qc: *mut ata_queued_cmd) {
...
//
#[no_mangle]
unsafe extern "C" fn k2_bmdma_start_mmio(qc: *mut ata_queued_cmd) {
...
#[no_mangle]
unsafe extern "C" fn k2_stat_check_status(ap: *mut ata_port) -> u8 {
    static u8 k2_stat_check_status(struct ata_port *ap)
    {
    return readl(ap.ioaddr.status_addr);
    }
#[no_mangle]
unsafe extern "C" fn k2_sata_show_info(m: *mut seq_file, shost: *mut Scsi_Host) -> c_int {
...
// Match it to a port node
    ...
    for (np = np.child; np != core::ptr::null_mut(); np = np.sibling) {
    u64 reg;
    if (of_property_read_reg(np, 0, &reg, core::ptr::null_mut()))
    continue;
    if (index == reg) {
...
#[no_mangle]
unsafe extern "C" fn k2_sata_setup_port(port: *mut ata_ioports, base: *mut void __iomem) {
...
#[no_mangle]
unsafe extern "C" fn k2_sata_init_one(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int k2_sata_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
    const struct ata_port_info *ppi[] =
    { &k2_port_info[ent.driver_data], core::ptr::null_mut() };
...
// longer available. This should not be seen in any system.
    ...
    let mut offset: c_uint = i * K2_SATA_PORT_OFFSET;
