...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct svia_priv {
    pub wd_workaround: bool,
}
...
#[no_mangle]
unsafe extern "C" fn svia_scr_read(link: *mut ata_link, sc_reg: c_uint, val: *mut u32) -> c_int {
    static int svia_scr_read(struct ata_link *link, unsigned int sc_reg, u32 *val)
    {
...
#[no_mangle]
unsafe extern "C" fn svia_scr_write(link: *mut ata_link, sc_reg: c_uint, val: u32) -> c_int {
    static int svia_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val)
    {
    if (sc_reg > SCR_CONTROL)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn vt8251_scr_read(link: *mut ata_link, scr: c_uint, val: *mut u32) -> c_int {
    static int vt8251_scr_read(struct ata_link *link, unsigned int scr, u32 *val)
    {
    static const u8 ipm_tbl[] = { 1, 2, 6, 0 };
    struct pci_dev *pdev = to_pci_dev(link.ap.host.dev);
    let mut slot: c_int = 2 * link.ap.port_no + link.pmp;
    let mut v: u32 = 0;
    u8 raw;
    switch (scr) {
    case SCR_STATUS:
...
#[no_mangle]
unsafe extern "C" fn vt8251_scr_write(link: *mut ata_link, scr: c_uint, val: u32) -> c_int {
    static int vt8251_scr_write(struct ata_link *link, unsigned int scr, u32 val)
    {
    struct pci_dev *pdev = to_pci_dev(link.ap.host.dev);
    let mut slot: c_int = 2 * link.ap.port_no + link.pmp;
...
//
#[no_mangle]
unsafe extern "C" fn svia_tf_load(ap: *mut ata_port, tf: *const ata_taskfile) {
    static void svia_tf_load(struct ata_port *ap, const struct ata_taskfile *tf)
    {
...
#[no_mangle]
unsafe extern "C" fn svia_noop_freeze(ap: *mut ata_port) {
    static void svia_noop_freeze(struct ata_port *ap)
    {
...
//
#[no_mangle]
unsafe extern "C" fn vt6420_prereset(link: *mut ata_link, deadline: c_ulong) -> c_int {
    static int vt6420_prereset(struct ata_link *link, unsigned long deadline)
    {
    struct ata_port *ap = link.ap;
    struct ata_eh_context *ehc = &ap.link.eh_context;
    let mut timeout: c_ulong = jiffies + (HZ * 5);
...
#[no_mangle]
unsafe extern "C" fn vt6420_bmdma_start(qc: *mut ata_queued_cmd) {
    static void vt6420_bmdma_start(struct ata_queued_cmd *qc)
    {
...
#[no_mangle]
unsafe extern "C" fn vt6421_pata_cable_detect(ap: *mut ata_port) -> c_int {
    static int vt6421_pata_cable_detect(struct ata_port *ap)
    {
...
#[no_mangle]
unsafe extern "C" fn vt6421_set_pio_mode(ap: *mut ata_port, adev: *mut ata_device) {
    static void vt6421_set_pio_mode(struct ata_port *ap, struct ata_device *adev)
    {
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
...
#[no_mangle]
unsafe extern "C" fn vt6421_set_dma_mode(ap: *mut ata_port, adev: *mut ata_device) {
    static void vt6421_set_dma_mode(struct ata_port *ap, struct ata_device *adev)
    {
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
...
#[no_mangle]
unsafe extern "C" fn vt6421_init_addrs(ap: *mut ata_port) {
    static void vt6421_init_addrs(struct ata_port *ap)
    {
    let mut iomap: *const *const void __iomem = ap.host.iomap;
...
#[no_mangle]
unsafe extern "C" fn vt6420_prepare_host(pdev: *mut pci_dev, r_host: *mut ata_host) -> c_int {
    static int vt6420_prepare_host(struct pci_dev *pdev, struct ata_host **r_host)
    {
    const struct ata_port_info *ppi[] = { &vt6420_port_info, core::ptr::null_mut() };
...
#[no_mangle]
unsafe extern "C" fn vt6421_prepare_host(pdev: *mut pci_dev, r_host: *mut ata_host) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vt8251_prepare_host(pdev: *mut pci_dev, r_host: *mut ata_host) -> c_int {
    static int vt8251_prepare_host(struct pci_dev *pdev, struct ata_host **r_host)
    {
    const struct ata_port_info *ppi[] = { &vt8251_port_info, core::ptr::null_mut() };
    struct ata_host *host;
    int i, rc;
    rc = ata_pci_bmdma_prepare_host(pdev, ppi, &host);
    if (rc)
...
#[no_mangle]
unsafe extern "C" fn svia_wd_fix(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn vt642x_interrupt(irq: c_int, dev_instance: *mut c_void) -> irqreturn_t {
    static irqreturn_t vt642x_interrupt(int irq, void *dev_instance)
    {
    struct ata_host *host = dev_instance;
    let mut rc: irqreturn_t = ata_bmdma_interrupt(irq, dev_instance);
...
#[no_mangle]
unsafe extern "C" fn vt6421_error_handler(ap: *mut ata_port) {
    static void vt6421_error_handler(struct ata_port *ap)
...
#[no_mangle]
unsafe extern "C" fn svia_init_one(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int svia_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
    unsigned int i;
    int rc;
    struct ata_host *host = core::ptr::null_mut();
    let mut board_id: c_int = (int) ent.driver_data;
...
#[no_mangle]
unsafe extern "C" fn svia_pci_device_resume(pdev: *mut pci_dev) -> c_int {
    static int svia_pci_device_resume(struct pci_dev *pdev)
    {
