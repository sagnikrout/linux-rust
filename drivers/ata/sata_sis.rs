...
#[no_mangle]
unsafe extern "C" fn get_scr_cfg_addr(link: *mut ata_link, sc_reg: c_uint) -> c_uint {
    static unsigned int get_scr_cfg_addr(struct ata_link *link, unsigned int sc_reg)
    {
    struct ata_port *ap = link.ap;
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    let mut addr: c_uint = SIS_SCR_BASE + (4 * sc_reg);
    u8 pmr;
    if (ap.port_no)  {
    switch (pdev.device) {
    case 0x0180:
    ...
    let mut cfg_addr: c_uint = get_scr_cfg_addr(link, sc_reg);
...
#[no_mangle]
unsafe extern "C" fn sis_scr_read(link: *mut ata_link, sc_reg: c_uint, val: *mut u32) -> c_int {
    static int sis_scr_read(struct ata_link *link, unsigned int sc_reg, u32 *val)
    {
    struct ata_port *ap = link.ap;
...
#[no_mangle]
unsafe extern "C" fn sis_scr_write(link: *mut ata_link, sc_reg: c_uint, val: u32) -> c_int {
    static int sis_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val)
    {
    struct ata_port *ap = link.ap;
    void __iomem *base = ap.ioaddr.scr_addr + link.pmp * 0x10;
...
unsafe extern "C" fn sis_init_one(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int sis_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
    let mut pi: ata_port_info = sis_port_info;
