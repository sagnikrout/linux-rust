...
pub const SIL_DMA_BOUNDARY: c_uint = 0x7fffffffUL;
    enum {
    SIL_MMIO_BAR		= 5,
//
// host flags
...
#[no_mangle]
unsafe extern "C" fn sil_bmdma_stop(qc: *mut ata_queued_cmd) {
    static void sil_bmdma_stop(struct ata_queued_cmd *qc)
    {
...
#[no_mangle]
unsafe extern "C" fn sil_bmdma_setup(qc: *mut ata_queued_cmd) {
    static void sil_bmdma_setup(struct ata_queued_cmd *qc)
    {
    struct ata_port *ap = qc.ap;
...
#[no_mangle]
unsafe extern "C" fn sil_bmdma_start(qc: *mut ata_queued_cmd) {
    static void sil_bmdma_start(struct ata_queued_cmd *qc)
    {
    ...
    void __iomem *mmio_base = ap.host.iomap[SIL_MMIO_BAR];
    void __iomem *bmdma2 = mmio_base + sil_port[ap.port_no].bmdma2;
    let mut dmactl: u8 = ATA_DMA_START;
...
#[no_mangle]
unsafe extern "C" fn sil_fill_sg(qc: *mut ata_queued_cmd) {
    static void sil_fill_sg(struct ata_queued_cmd *qc)
    {
    struct scatterlist *sg;
    struct ata_port *ap = qc.ap;
    struct ata_bmdma_prd *prd, *last_prd = core::ptr::null_mut();
    unsigned int si;
    prd = &ap.bmdma_prd[0];
...
//
    let mut addr: u32 = (u32) sg_dma_address(sg);
...
#[no_mangle]
unsafe extern "C" fn sil_qc_prep(qc: *mut ata_queued_cmd) -> enum ata_completion_errors {
    static enum ata_completion_errors sil_qc_prep(struct ata_queued_cmd *qc)
    {
...
#[no_mangle]
unsafe extern "C" fn sil_get_device_cache_line(pdev: *mut pci_dev) -> c_uchar {
    static unsigned char sil_get_device_cache_line(struct pci_dev *pdev)
    {
    let mut cache_line: u8 = 0;
...
//
#[no_mangle]
unsafe extern "C" fn sil_set_mode(link: *mut ata_link, r_failed: *mut ata_device) -> c_int {
    static int sil_set_mode(struct ata_link *link, struct ata_device **r_failed)
    {
...
#[no_mangle]
pub unsafe extern "C" fn if(ATA_DFLAG_PIO: dev->flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn sil_scr_read(link: *mut ata_link, sc_reg: c_uint, val: *mut u32) -> c_int {
    static int sil_scr_read(struct ata_link *link, unsigned int sc_reg, u32 *val)
    {
...
#[no_mangle]
unsafe extern "C" fn sil_scr_write(link: *mut ata_link, sc_reg: c_uint, val: u32) -> c_int {
    static int sil_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val)
    {
    void __iomem *mmio = sil_scr_addr(link.ap, sc_reg);
    if (mmio) {
...
#[no_mangle]
unsafe extern "C" fn sil_host_intr(ap: *mut ata_port, bmdma2: u32) {
...
#[no_mangle]
unsafe extern "C" fn sil_interrupt(irq: c_int, dev_instance: *mut c_void) -> irqreturn_t {
    ...
    let mut bmdma2: u32 = readl(mmio_base + sil_port[ap.port_no].bmdma2);
...
#[no_mangle]
unsafe extern "C" fn sil_freeze(ap: *mut ata_port) {
    static void sil_freeze(struct ata_port *ap)
    {
    void __iomem *mmio_base = ap.host.iomap[SIL_MMIO_BAR];
    u32 tmp;
...
#[no_mangle]
unsafe extern "C" fn sil_thaw(ap: *mut ata_port) {
    static void sil_thaw(struct ata_port *ap)
    {
    void __iomem *mmio_base = ap.host.iomap[SIL_MMIO_BAR];
    u32 tmp;
...
//
#[no_mangle]
unsafe extern "C" fn sil_dev_config(dev: *mut ata_device) {
    static void sil_dev_config(struct ata_device *dev)
    {
    struct ata_port *ap = dev.link.ap;
    let mut print_info: c_int = ap.link.eh_context.i.flags & ATA_EHI_PRINTINFO;
...
#[no_mangle]
unsafe extern "C" fn sil_init_controller(host: *mut ata_host) {
...
#[no_mangle]
unsafe extern "C" fn sil_broken_system_poweroff(pdev: *mut pci_dev) -> bool {
...
#[no_mangle]
unsafe extern "C" fn sil_init_one(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int sil_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
    let mut board_id: c_int = ent.driver_data;
    let mut pi: ata_port_info = sil_port_info[board_id];
...
#[no_mangle]
unsafe extern "C" fn sil_pci_device_resume(pdev: *mut pci_dev) -> c_int {
    static int sil_pci_device_resume(struct pci_dev *pdev)
    {
