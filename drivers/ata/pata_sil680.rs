...
//! Source: drivers/ata/pata_sil680.c
#![no_std]
...
pub const SIL680_MMIO_BAR: c_int = 5;
//
...
//
#[no_mangle]
unsafe extern "C" fn sil680_selreg(ap: *mut ata_port, r: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sil680_seldev(ap: *mut ata_port, adev: *mut ata_device, r: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sil680_cable_detect(ap: *mut ata_port) -> c_int {
    static int sil680_cable_detect(struct ata_port *ap)
    {
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    let mut addr: c_int = sil680_selreg(ap, 0);
    u8 ata66;
...
//
#[no_mangle]
unsafe extern "C" fn sil680_set_piomode(ap: *mut ata_port, adev: *mut ata_device) {
    ...
    let mut tfaddr: c_int = sil680_selreg(ap, 0x02);
    let mut addr: c_int = sil680_seldev(ap, adev, 0x04);
    let mut addr_mask: c_int = 0x80 + 4 * ap.port_no;
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    let mut pio: c_int = adev.pio_mode - XFER_PIO_0;
    let mut lowest_pio: c_int = pio;
    let mut port_shift: c_int = 4 * adev.devno;
    u16 reg;
    u8 mode;
    struct ata_device *pair = ata_dev_pair(adev);
    if (pair != core::ptr::null_mut() && adev.pio_mode > pair.pio_mode)
...
//
#[no_mangle]
unsafe extern "C" fn sil680_set_dmamode(ap: *mut ata_port, adev: *mut ata_device) {
    ...
    struct pci_dev *pdev = to_pci_dev(ap.host.dev);
    let mut ma: c_int = sil680_seldev(ap, adev, 0x08);
    let mut ua: c_int = sil680_seldev(ap, adev, 0x0C);
    let mut addr_mask: c_int = 0x80 + 4 * ap.port_no;
    let mut port_shift: c_int = adev.devno * 4;
    u8 scsc, mode;
    u16 multi, ultra;
...
#[no_mangle]
unsafe extern "C" fn sil680_sff_irq_check(ap: *mut ata_port) -> bool {
    static bool sil680_sff_irq_check(struct ata_port *ap)
    {
    struct pci_dev *pdev	= to_pci_dev(ap.host.dev);
    let mut addr: c_int = sil680_selreg(ap, 1);
    u8 val;
...
//
#[no_mangle]
unsafe extern "C" fn sil680_init_chip(pdev: *mut pci_dev, try_mmio: *mut c_int) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn sil680_init_one(pdev: *mut pci_dev, id: *const pci_device_id) -> c_int {
...
// Register & activate
    ...
    use_ioports:
    return ata_pci_bmdma_init_one(pdev, ppi, &sil680_sht, core::ptr::null_mut(), 0);
    }

#[no_mangle]
unsafe extern "C" fn sil680_reinit_one(pdev: *mut pci_dev) -> c_int {
