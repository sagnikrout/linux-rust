...
//! Source: drivers/ata/ata_generic.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn generic_set_mode(link: *mut ata_link, unused: *mut ata_device) -> c_int {
    static int generic_set_mode(struct ata_link *link, struct ata_device **unused)
    {
    struct ata_port *ap = link.ap;
    let mut driver_data: c_ulong = (unsigned long)ap.host.private_data;
    let mut dma_enabled: c_int = 0;
...
// We do need the right mode information for DMA or PIO
    ...
    let mut xfer_mask: c_uint = ata_id_xfermask(dev.id);
    const char *name;
...
//
#[no_mangle]
unsafe extern "C" fn is_intel_ider(dev: *mut pci_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ata_generic_init_one(dev: *mut pci_dev, id: *const pci_device_id) -> c_int {
...
// Don't re-enable devices in generic mode or we will break some
    ...
    let mut rc: c_int = pcim_enable_device(dev);
    if (rc < 0)
    return rc;
    pcim_pin_device(dev);
    }
