...
//! Source: drivers/ata/libata-acpi.c
#![no_std]
...
//
    ...
    let mut ata_acpi_gtf_filter: c_uint = ATA_ACPI_FILTER_DEFAULT;
    module_param_named(acpi_gtf_filter, ata_acpi_gtf_filter, int, 0644);
    MODULE_PARM_DESC(acpi_gtf_filter, "filter mask for ACPI _GTF commands, set to filter out (0x1=set xfermode, 0x2=lock/freeze lock, 0x4=DIPM, 0x8=FPDMA non-zero offset, 0x10=FPDMA DMA Setup FIS auto-activate)");
pub const NO_PORT_MULT: c_uint = 0xffff;

pub const REGS_PER_GTF: c_int = 7;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ata_acpi_gtf {
    pub /: *mut *mut u8 tf[REGS_PER_GTF]; / regs. 0x1f1 - 0x1f7,
    pub __packed: },
#[no_mangle]
unsafe extern "C" fn ata_acpi_clear_gtf(dev: *mut ata_device) {
    static void ata_acpi_clear_gtf(struct ata_device *dev)
    {
    pub NULL: dev->gtf_cache =,
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ata_acpi_hotplug_context {
...
pub unsafe extern "C" fn ata_dev_acpi_handle(dev: *mut ata_device) -> acpi_handle {
    ...
    core::ptr::null_mut() : ACPI_HANDLE(&dev.tdev);
    }
// @ap and @dev are the same as ata_acpi_handle_hotplug()
#[no_mangle]
unsafe extern "C" fn ata_acpi_detach_device(ap: *mut ata_port, dev: *mut ata_device) {
...
#[no_mangle]
unsafe extern "C" fn ata_acpi_dev_notify_dock(adev: *mut acpi_device, event: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ata_acpi_ap_notify_dock(adev: *mut acpi_device, event: u32) -> c_int {
    static int ata_acpi_ap_notify_dock(struct acpi_device *adev, u32 event)
    {
    ata_acpi_handle_hotplug(ata_hotplug_data(adev.hp).ap, core::ptr::null_mut(), event);
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn ata_acpi_ap_uevent(adev: *mut acpi_device, event: u32) {
    static void ata_acpi_ap_uevent(struct acpi_device *adev, u32 event)
    {
    ata_acpi_uevent(ata_hotplug_data(adev.hp).ap, core::ptr::null_mut(), event);
    }
#[no_mangle]
unsafe extern "C" fn ata_acpi_dev_uevent(adev: *mut acpi_device, event: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_dev_manage_restart(dev: *mut ata_device) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_port_power_on(ap: *mut ata_port) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_dissociate(host: *mut ata_host) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_gtm(ap: *mut ata_port, gtm: *mut ata_acpi_gtm) -> c_int {
    int ata_acpi_gtm(struct ata_port *ap, struct ata_acpi_gtm *gtm)
    {
    let mut output: acpi_buffer = { .length = ACPI_ALLOCATE_BUFFER };
    union acpi_object *out_obj;
    acpi_status status;
    let mut rc: c_int = 0;
    let mut handle: acpi_handle = ACPI_HANDLE(&ap.tdev);
    if (!handle)
    return -EINVAL;
    status = acpi_evaluate_object(handle, "_GTM", core::ptr::null_mut(), &output);
    rc = -ENOENT;
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_stm(ap: *mut ata_port, stm: *const ata_acpi_gtm) -> c_int {
    int ata_acpi_stm(struct ata_port *ap, const struct ata_acpi_gtm *stm)
    {
    acpi_status status;
    let mut stm_buf: ata_acpi_gtm = *stm;
...
//
#[no_mangle]
unsafe extern "C" fn ata_dev_get_GTF(dev: *mut ata_device, gtf: *mut ata_acpi_gtf) -> c_int {
...
// set up output buffer
    ...
    output.pointer = core::ptr::null_mut();	/* ACPI-CA sets this; save/free it later */
// _GTF has no input parameters
    status = acpi_evaluate_object(ata_dev_acpi_handle(dev), "_GTF", core::ptr::null_mut(),
    &output);
    ...
    ata_dev_dbg(dev, "Run _GTF: length or ptr is core::ptr::null_mut() (0x%llx, 0x%p)\n",
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_cbl_pata_type(ap: *mut ata_port) -> c_int {
    int ata_acpi_cbl_pata_type(struct ata_port *ap)
    {
    struct ata_device *dev;
    let mut ret: c_int = ATA_CBL_PATA_UNK;
    const struct ata_acpi_gtm *gtm = ata_acpi_init_gtm(ap);
    if (!gtm)
    ...
    ata_unpack_xfermask(xfer_mask, core::ptr::null_mut(), core::ptr::null_mut(), &udma_mask);
    ret = ATA_CBL_PATA40;
...
//
    ...
    {
    struct ata_taskfile *pptf = core::ptr::null_mut();
    ...
    rtf = tf;
    err_mask = ata_exec_internal(dev, &rtf, core::ptr::null_mut(),
    DMA_NONE, core::ptr::null_mut(), 0, 0);
    switch (err_mask) {
    case 0:
    ata_dev_dbg(dev,
...
//
#[no_mangle]
unsafe extern "C" fn ata_acpi_exec_tfs(dev: *mut ata_device, nr_executed: *mut c_int) -> c_int {
    static int ata_acpi_exec_tfs(struct ata_device *dev, int *nr_executed)
    {
    struct ata_acpi_gtf *gtf = core::ptr::null_mut(), *pgtf = core::ptr::null_mut();
    int gtf_count, i, rc;
// get taskfiles
...
//
#[no_mangle]
unsafe extern "C" fn ata_acpi_push_id(dev: *mut ata_device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_on_resume(ap: *mut ata_port) {
...
//
    ...
    ata_dev_get_GTF(dev, core::ptr::null_mut()) >= 0)
...
#[no_mangle]
unsafe extern "C" fn ata_acpi_choose_suspend_state(dev: *mut ata_device, runtime: bool) -> c_int {
    static int ata_acpi_choose_suspend_state(struct ata_device *dev, bool runtime)
    {
    let mut d_max_in: c_int = ACPI_STATE_D3_COLD;
    if (!runtime)
    goto out;
//
...
//
    ...
    out:
    return acpi_pm_device_sleep_state(&dev.tdev, core::ptr::null_mut(), d_max_in);
    }
#[no_mangle]
unsafe extern "C" fn sata_acpi_set_state(ap: *mut ata_port, state: pm_message_t) {
    static void sata_acpi_set_state(struct ata_port *ap, pm_message_t state)
    {
    let mut runtime: bool = PMSG_IS_AUTO(state);
    struct ata_device *dev;
    acpi_handle handle;
    int acpi_state;
    ata_for_each_dev(dev, &ap.link, ENABLED) {
...
#[no_mangle]
unsafe extern "C" fn pata_acpi_set_state(ap: *mut ata_port, state: pm_message_t) {
...
// channel first and then drives for power on and vica versa
    ...
    ata_for_each_dev(dev, &ap.link, ENABLED) {
    let mut dev_handle: acpi_handle = ata_dev_acpi_handle(dev);
    if (!dev_handle)
    continue;
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_set_state(ap: *mut ata_port, state: pm_message_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_on_devcfg(dev: *mut ata_device) -> c_int {
    int ata_acpi_on_devcfg(struct ata_device *dev)
    {
    struct ata_port *ap = dev.link.ap;
    struct ata_eh_context *ehc = &ap.link.eh_context;
    let mut acpi_sata: c_int = ap.flags & ATA_FLAG_ACPI_SATA;
    let mut nr_executed: c_int = 0;
    int rc;
...
//
#[no_mangle]
pub unsafe extern "C" fn ata_acpi_on_disable(dev: *mut ata_device) {
