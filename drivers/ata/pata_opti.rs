...
//! Source: drivers/ata/pata_opti.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn opti_pre_reset(link: *mut ata_link, deadline: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn opti_write_reg(ap: *mut ata_port, val: u8, reg: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn opti_set_piomode(ap: *mut ata_port, adev: *mut ata_device) {
...
//
    ...
    if (pair) {
// Hardware constraint
    let mut pair_addr: u8 = addr_timing[clock][pair.pio_mode - XFER_PIO_0];
    if (pair_addr > addr)
    addr = pair_addr;
    }
...
#[no_mangle]
unsafe extern "C" fn opti_init_one(dev: *mut pci_dev, id: *const pci_device_id) -> c_int {
    ...
    return ata_pci_sff_init_one(dev, ppi, &opti_sht, core::ptr::null_mut(), 0);
    }
