...
//! Source: drivers/tee/optee/protmem.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct optee_protmem_dyn_pool {
    pub pool: tee_protmem_pool,
    pub gen_pool: *mut gen_pool,
    pub optee: *mut optee,
    pub page_count: usize,
    pub mem_attrs: *mut u32,
    pub mem_attr_count: u_int,
    pub refcount: refcount_t,
    pub use_case: u32,
    pub protmem: *mut tee_shm,
...
#[no_mangle]
unsafe extern "C" fn init_dyn_protmem(rp: *mut optee_protmem_dyn_pool) -> c_int {
...
//
    ...
    rp.gen_pool = core::ptr::null_mut();
    err_reclaim:
    rp.optee.ops.reclaim_protmem(rp.optee, rp.protmem);
    err_put_shm:
    tee_shm_put(rp.protmem);
    err_null_protmem:
    rp.protmem = core::ptr::null_mut();
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn get_dyn_protmem(rp: *mut optee_protmem_dyn_pool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn release_dyn_protmem(rp: *mut optee_protmem_dyn_pool) {
...
#[no_mangle]
unsafe extern "C" fn put_dyn_protmem(rp: *mut optee_protmem_dyn_pool) {
    ...
    let mut sz: usize = ALIGN(size, PAGE_SIZE);
    phys_addr_t pa;
    int rc;
...
#[no_mangle]
unsafe extern "C" fn pool_op_dyn_destroy_pool(pool: *mut tee_protmem_pool) {
    ...
    struct tee_shm *shm_param = core::ptr::null_mut();
...
// ma_count = params[1].u.memref.size / sizeof(*mem_attrs);
    ...
    rp.use_case = id;
    rc = get_protmem_config(optee, id, &min_size, &pa_width, core::ptr::null_mut(),
    &rp.mem_attr_count);
    if (rc) {
    if (rc != -ENOSPC)
    goto err;
