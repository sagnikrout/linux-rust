...
//! Source: drivers/tee/amdtee/call.c
#![no_std]
...
// It is assumed that all values are within 2^32-1
    ...
    let mut buf_id: u32 = (u32)tee[i].u.memref.shm.sec_world_id;
...
#[no_mangle]
unsafe extern "C" fn get_ta_refcount(ta_handle: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn put_ta_refcount(ta_handle: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn handle_unload_ta(ta_handle: u32) -> c_int {
    int handle_unload_ta(u32 ta_handle)
    {
    let mut cmd: tee_cmd_unload_ta = {0};
    u32 status, count;
    int ret;
    if (!ta_handle)
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn handle_close_session(ta_handle: u32, info: u32) -> c_int {
    int handle_close_session(u32 ta_handle, u32 info)
    {
    let mut cmd: tee_cmd_close_session = {0};
    u32 status;
    int ret;
    if (ta_handle == 0)
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn handle_unmap_shmem(buf_id: u32) {
    void handle_unmap_shmem(u32 buf_id)
    {
    let mut cmd: tee_cmd_unmap_shared_mem = {0};
    u32 status;
    int ret;
    cmd.buf_id = buf_id;
    ...
    struct tee_param *p)
    {
    let mut cmd: tee_cmd_invoke_cmd = {0};
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn handle_map_shmem(count: u32, start: *mut shmem_desc, buf_id: *mut u32) -> c_int {
...
// buf_id = cmd->buf_id;
    ...
    struct tee_param *p)
    {
    let mut cmd: tee_cmd_open_session = {0};
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn handle_load_ta(data: *mut c_void, size: u32, arg: *mut tee_ioctl_open_session_arg) -> c_int {
    int handle_load_ta(void *data, u32 size, struct tee_ioctl_open_session_arg *arg)
    {
    let mut unload_cmd: tee_cmd_unload_ta = {};
    let mut load_cmd: tee_cmd_load_ta = {};
    phys_addr_t blob;
    int ret;
    if (size == 0 || !data || !arg)
    return -EINVAL;
