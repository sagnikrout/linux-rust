...
//
    ...
    let mut test_buf_size: static unsigned int = 16384;
    ...
    let mut threads_per_chan: static unsigned int = 1;
    ...
    let mut xor_sources: static unsigned int = 3;
    module_param(xor_sources, uint, 0644);
    MODULE_PARM_DESC(xor_sources,
    "Number of xor source buffers (default: 3)");
    let mut pq_sources: static unsigned int = 3;
    module_param(pq_sources, uint, 0644);
    MODULE_PARM_DESC(pq_sources,
    "Number of p+q source buffers (default: 3)");
    let mut timeout: static int = 3000;
    ...
    let mut alignment: static int = -1;
...
pub struct dmatest_params {
    pub nobounce: bool,
    pub buf_size: c_uint,
    pub channel: [c_char; 20],
    pub device: [c_char; 32],
    pub threads_per_chan: c_uint,
    pub max_channels: c_uint,
    pub iterations: c_uint,
    pub xor_sources: c_uint,
    pub pq_sources: c_uint,
    pub timeout: c_int,
    pub noverify: bool,
    pub norandom: bool,
    pub alignment: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dmatest_info {
// Test parameters
    pub params: dmatest_params,
// Internal state
    ...
    module_param_cb(test_list, &test_list_ops, core::ptr::null_mut(), 0444);
    MODULE_PARM_DESC(test_list, "Print current test list");
// Maximum amount of mismatched bytes in buffer to print
pub const MAX_ERROR_COUNT: c_int = 32;
//
...
//
pub const PATTERN_SRC: c_uint = 0x80;
pub const PATTERN_DST: c_uint = 0x00;
pub const PATTERN_COPY: c_uint = 0x40;
pub const PATTERN_OVERWRITE: c_uint = 0x20;
pub const PATTERN_COUNT_MASK: c_uint = 0x1f;
pub const PATTERN_MEMSET_IDX: c_uint = 0x01;
// Fixed point arithmetic ops
pub const FIXPT_SHIFT: c_int = 8;
pub const FIXPNT_MASK: c_uint = 0xFF;

// poor man's completion - we want to use wait_event_freezable() on it
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dmatest_done {
    pub done: bool,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dmatest_thread {
    pub node: list_head,
    pub info: *mut dmatest_info,
    pub task: *mut task_struct,
    pub chan: *mut dma_chan,
    pub src: dmatest_data,
    pub dst: dmatest_data,
    pub type: enum dma_transaction_type,
    pub done_wait: wait_queue_head_t,
    pub test_done: dmatest_done,
    pub done: bool,
    pub pending: bool,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dmatest_chan {
    pub node: list_head,
    pub chan: *mut dma_chan,
    pub threads: list_head,
}

    static DECLARE_WAIT_QUEUE_HEAD(thread_wait);
    static bool wait;
#[no_mangle]
unsafe extern "C" fn is_threaded_test_run(info: *mut dmatest_info) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_threaded_test_pending(info: *mut dmatest_info) -> bool {
...
#[no_mangle]
unsafe extern "C" fn dmatest_wait_get(val: *mut c_char, kp: *const kernel_param) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmatest_random() -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn gen_inv_idx(index: u8, is_memset: bool) -> u8 {
    static inline u8 gen_inv_idx(u8 index, bool is_memset)
    {
    let mut val: u8 = is_memset ? PATTERN_MEMSET_IDX : index;
    return ~val & PATTERN_COUNT_MASK;
    }
#[no_mangle]
pub unsafe extern "C" fn gen_src_value(index: u8, is_memset: bool) -> u8 {
    static inline u8 gen_src_value(u8 index, bool is_memset)
    {
    return PATTERN_SRC | gen_inv_idx(index, is_memset);
    }
#[no_mangle]
pub unsafe extern "C" fn gen_dst_value(index: u8, is_memset: bool) -> u8 {
    ...
    {
    let mut diff: u8 = actual ^ pattern;
    let mut expected: u8 = pattern | gen_inv_idx(counter, is_memset);
    const char	*thread_name = current.comm;
    if (is_srcbuf)
    pr_warn("%s: srcbuf[0x%x] overwritten! Expected %02x, got %02x\n",
    thread_name, index, expected, actual);
#[no_mangle]
pub unsafe extern "C" fn if(PATTERN_COPY: (pattern &) -> else {
    else if ((pattern & PATTERN_COPY)
    && (diff & (PATTERN_COPY | PATTERN_OVERWRITE)))
    pr_warn("%s: dstbuf[0x%x] not copied! Expected %02x, got %02x\n",
    thread_name, index, expected, actual);
#[no_mangle]
pub unsafe extern "C" fn if(PATTERN_SRC: diff &) -> else {
    ...
    {
    unsigned int i;
    let mut error_count: c_uint = 0;
    u8 actual;
    u8 expected;
    u8 *buf;
    let mut counter_orig: c_uint = counter;
    for (; (buf = *bufs); bufs++) {
...
#[no_mangle]
unsafe extern "C" fn dmatest_callback(arg: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn min_odd(x: c_uint, y: c_uint) -> c_uint {
    static unsigned int min_odd(unsigned int x, unsigned int y)
    {
    let mut val: c_uint = min(x, y);
...
#[no_mangle]
unsafe extern "C" fn dmatest_persec(runtime: i64, val: c_uint) -> c_ulonglong {
    static unsigned long long dmatest_persec(s64 runtime, unsigned int val)
    {
    let mut per_sec: c_ulonglong = 1000000;
    if (runtime <= 0)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn dmatest_KBs(runtime: i64, len: c_ulonglong) -> c_ulonglong {
    static unsigned long long dmatest_KBs(s64 runtime, unsigned long long len)
    {
    return FIXPT_TO_INT(dmatest_persec(runtime, len >> 10));
    }
#[no_mangle]
unsafe extern "C" fn __dmatest_free_test_data(d: *mut dmatest_data, cnt: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn dmatest_free_test_data(d: *mut dmatest_data) {
...
//
#[no_mangle]
unsafe extern "C" fn dmatest_func(data: *mut c_void) -> c_int {
    ...
    let mut failed_tests: c_uint = 0;
    let mut total_tests: c_uint = 0;
    dma_cookie_t		cookie;
    enum dma_status		status;
    enum dma_ctrl_flags	flags;
    u8			*pq_coefs = core::ptr::null_mut();
    int			ret;
    ...
    let mut filltime: ktime_t = 0;
    let mut comparetime: ktime_t = 0;
    let mut runtime: i64 = 0;
    let mut total_len: c_ulonglong = 0;
    let mut iops: c_ulonglong = 0;
    let mut align: u8 = 0;
    let mut is_memset: bool = false;
    dma_addr_t		*srcs;
    dma_addr_t		*dma_pq;
    set_freezable();
    ret = -ENOMEM;
...
// Do not alter transfer size explicitly defined by user
    ...
    if (!um) {
    failed_tests++;
    result("unmap data core::ptr::null_mut()", total_tests,
    ...
    let mut pg_off: c_ulong = offset_in_page(buf);
...
// map with DMA_BIDIRECTIONAL to force writeback/invalidate
    ...
    srcs[0], len, flags);
#[no_mangle]
pub unsafe extern "C" fn if(DMA_MEMSET: thread->type ==) -> else {
    else if (thread.type == DMA_MEMSET)
...
// (src->aligned[0] + src->off),
    len, flags);
#[no_mangle]
pub unsafe extern "C" fn if(DMA_XOR: thread->type ==) -> else {
    else if (thread.type == DMA_XOR)
    ...
    len, flags);
#[no_mangle]
pub unsafe extern "C" fn if(DMA_PQ: thread->type ==) -> else {
    for (i = 0; i < dst.cnt; i++)
    ...
    status = dma_async_is_tx_complete(chan, cookie, core::ptr::null_mut(),
    core::ptr::null_mut());
    }
    if (!done.done) {
...
#[no_mangle]
unsafe extern "C" fn dmatest_cleanup_channel(dtc: *mut dmatest_chan) {
...
// terminate all transfers on specified channels
    ...
    op = "copy";
#[no_mangle]
pub unsafe extern "C" fn if(DMA_MEMSET: type ==) -> else {
    else if (type == DMA_MEMSET)
    op = "set";
#[no_mangle]
pub unsafe extern "C" fn if(DMA_XOR: type ==) -> else {
    else if (type == DMA_XOR)
    op = "xor";
#[no_mangle]
pub unsafe extern "C" fn if(DMA_PQ: type ==) -> else {
    else if (type == DMA_PQ)
    op = "pq";
    else
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn add_threaded_test(info: *mut dmatest_info) {
...
#[no_mangle]
unsafe extern "C" fn run_pending_tests(info: *mut dmatest_info) {
...
#[no_mangle]
unsafe extern "C" fn stop_threaded_test(info: *mut dmatest_info) {
...
#[no_mangle]
unsafe extern "C" fn start_threaded_tests(info: *mut dmatest_info) {
...
#[no_mangle]
unsafe extern "C" fn dmatest_run_get(val: *mut c_char, kp: *const kernel_param) -> c_int {
    static int dmatest_run_get(char *val, const struct kernel_param *kp)
    {
    struct dmatest_info *info = &test_info;
...
#[no_mangle]
unsafe extern "C" fn dmatest_run_set(val: *const c_char, kp: *const kernel_param) -> c_int {
    static int dmatest_run_set(const char *val, const struct kernel_param *kp)
    {
    struct dmatest_info *info = &test_info;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn dmatest_chan_set(val: *const c_char, kp: *const kernel_param) -> c_int {
    static int dmatest_chan_set(const char *val, const struct kernel_param *kp)
    {
    struct dmatest_info *info = &test_info;
    struct dmatest_chan *dtc;
...
#[no_mangle]
unsafe extern "C" fn dmatest_chan_get(val: *mut c_char, kp: *const kernel_param) -> c_int {
    static int dmatest_chan_get(char *val, const struct kernel_param *kp)
    {
    struct dmatest_info *info = &test_info;
    mutex_lock(&info.lock);
...
#[no_mangle]
unsafe extern "C" fn dmatest_test_list_get(val: *mut c_char, kp: *const kernel_param) -> c_int {
    static int dmatest_test_list_get(char *val, const struct kernel_param *kp)
    {
    struct dmatest_info *info = &test_info;
    struct dmatest_chan *dtc;
...
#[no_mangle]
unsafe extern "C" fn dmatest_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn dmatest_exit() -> void __exit {
