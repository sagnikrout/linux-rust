...
//! Source: drivers/dma/ioat/prep.c
#![no_std]
...
pub const MAX_SCF: c_int = 256;
// provide a lookup table for setting the source address in the base or
// extended descriptor of an xor or pq descriptor
//
    let mut xor_idx_to_desc: static u8 = 0xe0;
    static const u8 xor_idx_to_field[] = { 1, 4, 5, 6, 7, 0, 1, 2 };
    let mut pq_idx_to_desc: static u8 = 0xf8;
...
#[no_mangle]
unsafe extern "C" fn pq_get_src(descs[2]: *mut ioat_raw_descriptor, idx: c_int) -> dma_addr_t {
...
#[no_mangle]
unsafe extern "C" fn pq16_get_src(desc[3]: *mut ioat_raw_descriptor, idx: c_int) -> dma_addr_t {
    ...
    let mut flags: gfp_t = __GFP_ZERO | GFP_ATOMIC;
    ...
    let mut dst: dma_addr_t = dma_dest;
    let mut src: dma_addr_t = dma_src;
    let mut total_len: usize = len;
    int num_descs, idx, i;
    ...
    i = 0;
    do {
    let mut copy: usize = min_t(size_t, len, 1 << ioat_chan.xfercap_log);
...
// we leave the channel locked to ensure in order submission
    ...
    struct ioat_xor_ext_descriptor *xor_ex = core::ptr::null_mut();
    struct ioat_dma_descriptor *hw;
    int num_descs, with_ext, idx, i;
    let mut offset: u32 = 0;
    let mut op: u8 = result ? IOAT_OP_XOR_VAL : IOAT_OP_XOR;
    BUG_ON(src_cnt < 2);
    num_descs = ioat_xferlen_to_descs(ioat_chan, len);
...
// we leave the channel locked to ensure in order submission
    ...
    if (test_bit(IOAT_CHAN_DOWN, &ioat_chan.state))
    return core::ptr::null_mut();
    return __ioat_prep_xor_lock(chan, core::ptr::null_mut(), dest, src, src_cnt, len, flags);
    }
    struct dma_async_tx_descriptor *
...
// result = 0;
    ...
    struct ioat_pq_ext_descriptor *pq_ex = ext ? ext.pq_ex : core::ptr::null_mut();
    struct ioat_raw_descriptor *descs[] = { (void *) pq, (void *) pq_ex };
    let mut src_cnt: c_int = src_cnt_to_sw(pq.ctl_f.src_cnt);
    int i;
    ...
    (void *)pq,
    (void *)pq };
    let mut src_cnt: c_int = src16_cnt_to_sw(pq.ctl_f.src_cnt);
    int i;
    if (desc.sed) {
    ...
    struct ioat_pq_ext_descriptor *pq_ex = core::ptr::null_mut();
    struct ioat_dma_descriptor *hw;
    let mut offset: u32 = 0;
    let mut op: u8 = result ? IOAT_OP_PQ_VAL : IOAT_OP_PQ;
    int i, s, idx, with_ext, num_descs;
    let mut cb32: c_int = (ioat_dma.version < IOAT_VER_3_3) ? 1 : 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: dmaf_continue(flags)) -> else {
...
#[no_mangle]
unsafe extern "C" fn src_cnt_flags(src_cnt: c_uint, flags: c_ulong) -> c_int {
...
//
    ...
    __ioat_prep_pq16_lock(chan, core::ptr::null_mut(), dst, single_source,
    2, single_source_coef, len,
    flags) :
    __ioat_prep_pq_lock(chan, core::ptr::null_mut(), dst, single_source, 2,
    single_source_coef, len, flags);
    } else {
    return src_cnt_flags(src_cnt, flags) > 8 ?
    __ioat_prep_pq16_lock(chan, core::ptr::null_mut(), dst, src, src_cnt,
    scf, len, flags) :
    __ioat_prep_pq_lock(chan, core::ptr::null_mut(), dst, src, src_cnt,
    scf, len, flags);
    }
    }
    struct dma_async_tx_descriptor *
...
// pqres = 0;
    ...
    return src_cnt_flags(src_cnt, flags) > 8 ?
    __ioat_prep_pq16_lock(chan, core::ptr::null_mut(), pq, src, src_cnt, scf, len,
    flags) :
    __ioat_prep_pq_lock(chan, core::ptr::null_mut(), pq, src, src_cnt, scf, len,
    flags);
    }
    struct dma_async_tx_descriptor *
