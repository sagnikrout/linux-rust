...
//! Source: drivers/dma/qcom/bam_dma.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bam_desc_hw {
    pub /: *mut *mut __le32 addr; / Buffer physical address,
    pub /: *mut *mut __le16 size; / Buffer size in bytes,
    pub flags: __le16,
}

pub const BAM_DMA_AUTOSUSPEND_DELAY: c_int = 100;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct bam_async_desc {
    pub vd: virt_dma_desc,
    pub num_desc: u32,
    pub xfer_len: u32,
// transaction flags, EOT|EOB|NWD
    pub flags: u16,
    pub curr_desc: *mut bam_desc_hw,
// list node for the desc in the bam_chan list of descriptors
    pub desc_node: list_head,
    pub dir: enum dma_transfer_direction,
    pub length: usize,
    pub __counted_by(num_desc): bam_desc_hw desc[],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct reg_offset_data {
    pub base_offset: u32,
    pub ee_mult: unsigned int pipe_mult, evnt_mult,,
}
...
pub const BAM_TESTBUS_SEL_SHIFT: c_int = 5;
pub const BAM_TESTBUS_SEL_MASK: c_uint = 0x3F;
pub const BAM_DESC_CACHE_SEL_SHIFT: c_int = 13;
pub const BAM_DESC_CACHE_SEL_MASK: c_uint = 0x3;

// BAM REVISION
pub const REVISION_SHIFT: c_int = 0;
pub const REVISION_MASK: c_uint = 0xFF;
pub const NUM_EES_SHIFT: c_int = 8;
pub const NUM_EES_MASK: c_uint = 0xF;

pub const DESC_CACHE_DEPTH_SHIFT: c_int = 21;

pub const INACTIV_TMR_BASE_SHIFT: c_int = 24;
pub const INACTIV_TMR_BASE_MASK: c_uint = 0xFF;
// BAM NUM PIPES
pub const BAM_NUM_PIPES_SHIFT: c_int = 0;
pub const BAM_NUM_PIPES_MASK: c_uint = 0xFF;
pub const PERIPH_NON_PIPE_GRP_SHIFT: c_int = 16;
pub const PERIPH_NON_PIP_GRP_MASK: c_uint = 0xFF;
pub const BAM_NON_PIPE_GRP_SHIFT: c_int = 24;
pub const BAM_NON_PIPE_GRP_MASK: c_uint = 0xFF;
// BAM CNFG BITS
...
pub const P_AUTO_EOB_SEL_SHIFT: c_int = 7;

pub const P_PREFETCH_LIMIT_SHIFT: c_int = 9;

pub const P_LOCK_GROUP_SHIFT: c_int = 16;
pub const P_LOCK_GROUP_MASK: c_uint = 0x1F;
// BAM_DESC_CNT_TRSHLD
pub const CNT_TRSHLD: c_uint = 0xffff;
pub const DEFAULT_CNT_THRSHLD: c_uint = 0x4;
// BAM_IRQ_SRCS

pub const P_IRQ: c_uint = 0x7fffffff;
// BAM_IRQ_SRCS_MSK
...
// BAM_P_SW_OFSTS
pub const P_SW_OFSTS_MASK: c_uint = 0xffff;
...
pub struct bam_chan {
    ...
    pub bdev: *mut bam_device,
// configuration from device tree
    pub id: u32,
// runtime configuration
    pub slave: dma_slave_config,
// fifo storage
    pub fifo_virt: *mut bam_desc_hw,
    pub fifo_phys: dma_addr_t,
// fifo markers
    pub /: *mut *mut unsigned short head; / start of active descriptor entries,
    pub /: *mut *mut unsigned short tail; / end of active descriptor entries,
    pub /: *mut *mut unsigned int initialized; / is the channel hw initialized?,
    pub /: *mut *mut unsigned int paused; / is the channel paused?,
    pub /: *mut *mut unsigned int reconfigure; / new slave config?,
// list of descriptors currently processed
    pub desc_list: list_head,
    pub node: list_head,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bam_device {
    pub regs: *mut void __iomem,
    pub dev: *mut device,
    pub common: dma_device,
    pub channels: *mut bam_chan,
    pub num_channels: u32,
    pub num_ees: u32,
// execution environment ID, from DT
    pub ee: u32,
    pub controlled_remotely: bool,
    pub powered_remotely: bool,
    pub active_channels: u32,
    pub layout: *const reg_offset_data,
    pub bamclk: *mut clk,
    pub irq: c_int,
...
//
    ...
    enum bam_reg reg)
    {
    let mut r: reg_offset_data = bdev.layout[reg];
...
//
#[no_mangle]
unsafe extern "C" fn bam_reset(bdev: *mut bam_device) {
...
//
#[no_mangle]
unsafe extern "C" fn bam_reset_channel(bchan: *mut bam_chan) {
...
//
#[no_mangle]
unsafe extern "C" fn bam_alloc_chan(chan: *mut dma_chan) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bam_free_chan(chan: *mut dma_chan) {
    ...
    bchan.fifo_phys);
    bchan.fifo_virt = core::ptr::null_mut();
...
// fill in temporary descriptors
    ...
    let mut remainder: c_uint = sg_dma_len(sg);
    let mut curr_offset: c_uint = 0;
    do {
...
//
#[no_mangle]
unsafe extern "C" fn bam_dma_terminate_all(chan: *mut dma_chan) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bam_pause(chan: *mut dma_chan) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bam_resume(chan: *mut dma_chan) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn process_channel_irqs(bdev: *mut bam_device) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn bam_dma_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t bam_dma_irq(int irq, void *data)
    {
    struct bam_device *bdev = data;
    let mut clr_mask: u32 = 0, srcs = 0;
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn bam_start_dma(bchan: *mut bam_chan) {
    static void bam_start_dma(struct bam_chan *bchan)
    {
    struct virt_dma_desc *vd = vchan_next_desc(&bchan.vc);
    struct bam_device *bdev = bchan.bdev;
    struct bam_async_desc *async_desc = core::ptr::null_mut();
    struct bam_desc_hw *desc;
...
//
    ...
    let mut partial: u32 = MAX_DESCRIPTORS - bchan.tail;
...
//
#[no_mangle]
unsafe extern "C" fn dma_tasklet(t: *mut tasklet_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn bam_issue_pending(chan: *mut dma_chan) {
...
//
#[no_mangle]
unsafe extern "C" fn bam_dma_free_desc(vd: *mut virt_dma_desc) {
...
//
#[no_mangle]
unsafe extern "C" fn bam_init(bdev: *mut bam_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bam_dma_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bam_dma_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn bam_dma_runtime_suspend(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn bam_dma_runtime_resume(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn bam_dma_suspend(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn bam_dma_resume(dev: *mut device) -> int __maybe_unused {
