...
pub const TRE_TYPE_DMA: c_uint = 0x10;
pub const TRE_TYPE_IMMEDIATE_DMA: c_uint = 0x11;
pub const TRE_TYPE_GO: c_uint = 0x20;
pub const TRE_TYPE_CONFIG0: c_uint = 0x22;
// TRE flags
...
pub struct gpi_dev {
    ...
    pub address*/: *mut *mut *mut void __iomem ee_base; /ee register base,
    pub /: *mut *mut u32 max_gpii; / maximum # of gpii instances available per gpi block,
    pub /: *mut *mut u32 gpii_mask; / gpii instances available for apps,
    pub /: *mut *mut u32 ev_factor; / ev ring length factor,
    pub gpiis: *mut gpii,
}
...
pub struct gchan {
    ...
    pub protocol: u32,
    pub gpii: *mut gpii,
    pub ch_state: enum gpi_ch_state,
    pub pm_state: enum gpi_pm_state,
    pub ch_cntxt_base_reg: *mut void __iomem,
    pub ch_cntxt_db_reg: *mut void __iomem,
    pub ch_cmd_reg: *mut void __iomem,
    pub dir: u32,
    pub ch_ring: gpi_ring,
...
pub struct gpii {
    pub gpii_id: u32,
    pub gchan: [gchan; MAX_CHANNELS_PER_GPII],
    pub gpi_dev: *mut gpi_dev,
    pub irq: c_int,
    pub /: *mut *mut *mut void __iomem regs; / points to gpi top,
    pub ev_cntxt_base_reg: *mut void __iomem,
    pub ev_cntxt_db_reg: *mut void __iomem,
    pub ev_ring_rp_lsb_reg: *mut void __iomem,
    pub ev_cmd_reg: *mut void __iomem,
    pub ieob_clr_reg: *mut void __iomem,
    pub ctrl_lock: mutex,
    pub ev_state: enum gpi_ev_state,
    pub configured_irq: bool,
    pub pm_state: enum gpi_pm_state,
    pub pm_lock: rwlock_t,
    pub ev_ring: gpi_ring,
    pub /: *mut *mut tasklet_ev_task; / event processing tasklet,
    pub cmd_completion: completion,
    pub gpi_cmd: enum gpi_cmd,
    pub cntxt_type_irq_msk: u32,
    pub ieob_set: bool,
}

pub const MAX_TRE: c_int = 3;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gpi_desc {
    pub vd: virt_dma_desc,
    pub len: usize,
    pub /: *mut *mut *mut void db; / DB register to program,
    pub gchan: *mut gchan,
    pub tre: [gpi_tre; MAX_TRE],
    pub num_tre: u32,
}
...
#[no_mangle]
pub unsafe extern "C" fn gpi_read_reg(gpii: *mut gpii, addr: *mut void __iomem) -> u32 {
    static inline u32 gpi_read_reg(struct gpii *gpii, void __iomem *addr)
    {
    return readl_relaxed(addr);
    }
#[no_mangle]
pub unsafe extern "C" fn gpi_write_reg(gpii: *mut gpii, addr: *mut void __iomem, val: u32) {
    ...
    let mut tmp: u32 = gpi_read_reg(gpii, addr);
    tmp &= ~mask;
    tmp |= u32_encode_bits(val, mask);
    gpi_write_reg(gpii, addr, tmp);
    }
#[no_mangle]
unsafe extern "C" fn gpi_disable_interrupts(gpii: *mut gpii) {
...
#[no_mangle]
unsafe extern "C" fn gpi_config_interrupts(gpii: *mut gpii, settings: enum gpii_irq_settings, mask: bool) -> c_int {
...
// Sends gpii event or channel command
    ...
    enum gpi_cmd gpi_cmd)
    {
    let mut chid: u32 = MAX_CHANNELS_PER_GPII;
...
#[no_mangle]
unsafe extern "C" fn gpi_process_ieob(gpii: *mut gpii) {
...
#[no_mangle]
unsafe extern "C" fn gpi_process_ch_ctrl_irq(gpii: *mut gpii) {
    static void gpi_process_ch_ctrl_irq(struct gpii *gpii)
    {
    let mut gpii_id: u32 = gpii.gpii_id;
    let mut offset: u32 = GPII_n_CNTXT_SRC_GPII_CH_IRQ_OFFS(gpii_id);
    let mut ch_irq: u32 = gpi_read_reg(gpii, gpii.regs + offset);
    struct gchan *gchan;
    u32 chid, state;
// clear the status
...
#[no_mangle]
unsafe extern "C" fn gpi_process_gen_err_irq(gpii: *mut gpii) {
    static void gpi_process_gen_err_irq(struct gpii *gpii)
    {
    let mut gpii_id: u32 = gpii.gpii_id;
    let mut offset: u32 = GPII_n_CNTXT_GPII_IRQ_STTS_OFFS(gpii_id);
    let mut irq_stts: u32 = gpi_read_reg(gpii, gpii.regs + offset);
// clear the status
...
#[no_mangle]
unsafe extern "C" fn gpi_process_glob_err_irq(gpii: *mut gpii) {
    static void gpi_process_glob_err_irq(struct gpii *gpii)
    {
    let mut gpii_id: u32 = gpii.gpii_id;
    let mut offset: u32 = GPII_n_CNTXT_GLOB_IRQ_STTS_OFFS(gpii_id);
    let mut irq_stts: u32 = gpi_read_reg(gpii, gpii.regs + offset);
...
#[no_mangle]
unsafe extern "C" fn gpi_handle_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t gpi_handle_irq(int irq, void *data)
    {
...
// process all events
#[no_mangle]
unsafe extern "C" fn gpi_process_events(gpii: *mut gpii) {
    static void gpi_process_events(struct gpii *gpii)
    {
...
#[no_mangle]
unsafe extern "C" fn gpi_ev_tasklet(data: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn gpi_mark_stale_events(gchan: *mut gchan) {
    ...
    let mut chid: u32 = gpi_event.xfer_compl_event.chid;
    if (chid == gchan.chid)
...
#[no_mangle]
unsafe extern "C" fn gpi_reset_chan(gchan: *mut gchan, gpi_cmd: enum gpi_cmd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_start_chan(gchan: *mut gchan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_stop_chan(gchan: *mut gchan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_alloc_chan(chan: *mut gchan, send_alloc_cmd: bool) -> c_int {
    ...
    int ret;
    let mut id: u32 = gpii.gpii_id;
    let mut chid: u32 = chan.chid;
    let mut pair_chid: u32 = !chid;
    if (send_alloc_cmd) {
...
#[no_mangle]
unsafe extern "C" fn gpi_ring_num_elements_avail(ring: *const *const gpi_ring) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_ring_add_element(ring: *mut gpi_ring, wp: *mut c_void) -> c_int {
    static int gpi_ring_add_element(struct gpi_ring *ring, void **wp)
    {
...
#[no_mangle]
unsafe extern "C" fn gpi_ring_recycle_ev_element(ring: *mut gpi_ring) {
    static void gpi_ring_recycle_ev_element(struct gpi_ring *ring)
    {
// Update the WP
    ring.wp += ring.el_size;
...
// allocate memory for transfer and event rings
    ...
    {
    let mut len: u64 = elements * el_size;
    int bit;
...
#[no_mangle]
unsafe extern "C" fn gpi_terminate_all(chan: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_pause(chan: *mut dma_chan) -> c_int {
...
// resume dma transfer
#[no_mangle]
unsafe extern "C" fn gpi_resume(chan: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_desc_free(vd: *mut virt_dma_desc) {
...
#[no_mangle]
unsafe extern "C" fn gpi_issue_pending(chan: *mut dma_chan) {
    static void gpi_issue_pending(struct dma_chan *chan)
    {
    struct gchan *gchan = to_gchan(chan);
    struct gpii *gpii = gchan.gpii;
    unsigned long flags, pm_lock_flags;
    struct virt_dma_desc *vd = core::ptr::null_mut();
    struct gpi_desc *gpi_desc;
    struct gpi_ring *ch_ring = &gchan.ch_ring;
    void *tre, *wp = core::ptr::null_mut();
    int i;
...
#[no_mangle]
unsafe extern "C" fn gpi_ch_init(gchan: *mut gchan) -> c_int {
    static int gpi_ch_init(struct gchan *gchan)
    {
    struct gpii *gpii = gchan.gpii;
    let mut ev_factor: c_int = gpii.gpi_dev.ev_factor;
    u32 elements;
...
#[no_mangle]
unsafe extern "C" fn gpi_free_chan_resources(chan: *mut dma_chan) {
...
// send command to de allocate event ring
    if (cur_state == ACTIVE_STATE)
    gpi_send_cmd(gpii, core::ptr::null_mut(), GPI_EV_CMD_DEALLOC);
    gpi_free_ring(&gpii.ev_ring, gpii);
// disable interrupts
    if (cur_state == ACTIVE_STATE)
    gpi_disable_interrupts(gpii);
...
#[no_mangle]
unsafe extern "C" fn gpi_alloc_chan_resources(chan: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_find_avail_gpii(gpi_dev: *mut gpi_dev, seid: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpi_init() -> int __init {
