...
//! Source: drivers/dma/bcm-sba-raid.c
#![no_std]
...
pub const SBA_TYPE_SHIFT: c_int = 48;

pub const SBA_TYPE_A: c_uint = 0x0;
pub const SBA_TYPE_B: c_uint = 0x2;
pub const SBA_TYPE_C: c_uint = 0x3;
pub const SBA_USER_DEF_SHIFT: c_int = 32;

pub const SBA_R_MDATA_SHIFT: c_int = 24;

pub const SBA_C_MDATA_MS_SHIFT: c_int = 18;

pub const SBA_INT_SHIFT: c_int = 17;

pub const SBA_RESP_SHIFT: c_int = 16;

pub const SBA_C_MDATA_SHIFT: c_int = 8;

pub const SBA_C_MDATA_DNUM_SHIFT: c_int = 5;

pub const SBA_CMD_SHIFT: c_int = 0;

pub const SBA_CMD_ZERO_BUFFER: c_uint = 0x4;
pub const SBA_CMD_ZERO_ALL_BUFFERS: c_uint = 0x8;
pub const SBA_CMD_LOAD_BUFFER: c_uint = 0x9;
pub const SBA_CMD_XOR: c_uint = 0xa;
pub const SBA_CMD_GALOIS_XOR: c_uint = 0xb;
pub const SBA_CMD_WRITE_BUFFER: c_uint = 0xc;
pub const SBA_CMD_GALOIS: c_uint = 0xe;
pub const SBA_MAX_REQ_PER_MBOX_CHANNEL: c_int = 8192;
pub const SBA_MAX_MSG_SEND_PER_MBOX_CHANNEL: c_int = 8;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sba_request {
// Global state
    pub node: list_head,
    pub sba: *mut sba_device,
    pub flags: u32,
// Chained requests management
    pub first: *mut sba_request,
    pub next: list_head,
    pub next_pending_count: core::sync::atomic::AtomicI32,
// BRCM message data
    pub msg: brcm_message,
    pub tx: dma_async_tx_descriptor,
// SBA commands
    pub cmds: [brcm_sba_command; ],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sba_device {
// Underlying device
    pub dev: *mut device,
// DT configuration parameters
    pub ver: enum sba_version,
// Derived configuration parameters
    pub max_req: u32,
    pub hw_buf_size: u32,
    pub hw_resp_size: u32,
    pub max_pq_coefs: u32,
    pub max_pq_srcs: u32,
    pub max_cmd_per_req: u32,
    pub max_xor_srcs: u32,
    pub max_resp_pool_size: u32,
    pub max_cmds_pool_size: u32,
// Mailbox client and Mailbox channels
    pub client: mbox_client,
    pub mchan: *mut mbox_chan,
    pub mbox_dev: *mut device,
// DMA device and DMA channel
    pub dma_dev: dma_device,
    pub dma_chan: dma_chan,
// DMA channel resources
    pub resp_base: *mut c_void,
    pub resp_dma_base: dma_addr_t,
    pub cmds_base: *mut c_void,
    pub cmds_dma_base: dma_addr_t,
    pub reqs_lock: spinlock_t,
    pub reqs_fence: bool,
    pub reqs_alloc_list: list_head,
    pub reqs_pending_list: list_head,
    pub reqs_active_list: list_head,
    pub reqs_aborted_list: list_head,
    pub reqs_free_list: list_head,
...
#[no_mangle]
pub unsafe extern "C" fn sba_cmd_enc(cmd: u64, val: u32, shift: u32, mask: u32) -> u64 __pure {
...
#[no_mangle]
pub unsafe extern "C" fn sba_cmd_load_c_mdata(b0: u32) -> u32 __pure {
    static inline u32 __pure sba_cmd_load_c_mdata(u32 b0)
    {
    return b0 & SBA_C_MDATA_BNUMx_MASK;
    }
#[no_mangle]
pub unsafe extern "C" fn sba_cmd_write_c_mdata(b0: u32) -> u32 __pure {
    static inline u32 __pure sba_cmd_write_c_mdata(u32 b0)
    {
    return b0 & SBA_C_MDATA_BNUMx_MASK;
    }
#[no_mangle]
pub unsafe extern "C" fn sba_cmd_xor_c_mdata(b1: u32, b0: u32) -> u32 __pure {
...
#[no_mangle]
pub unsafe extern "C" fn sba_cmd_pq_c_mdata(d: u32, b1: u32, b0: u32) -> u32 __pure {
...
// ====== General helper routines =====
    ...
    unsigned long flags;
    struct sba_request *req = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn sba_free_chained_requests(req: *mut sba_request) {
...
#[no_mangle]
unsafe extern "C" fn sba_cleanup_nonpending_requests(sba: *mut sba_device) {
...
#[no_mangle]
unsafe extern "C" fn sba_cleanup_pending_requests(sba: *mut sba_device) {
...
// Note: Must be called with sba->reqs_lock held
#[no_mangle]
unsafe extern "C" fn _sba_process_pending_requests(sba: *mut sba_device) {
...
// Process pending requests
    ...
    unsigned long flags;
    struct sba_request *req;
    let mut free_count: u32 = 0, alloced_count = 0;
    let mut pending_count: u32 = 0, active_count = 0, aborted_count = 0;
    spin_lock_irqsave(&sba.reqs_lock, flags);
    list_for_each_entry(req, &sba.reqs_free_list, node)
    if (async_tx_test_ack(&req.tx))
    free_count++;
...
// ====== DMAENGINE callbacks =====
#[no_mangle]
unsafe extern "C" fn sba_free_chan_resources(dchan: *mut dma_chan) {
...
#[no_mangle]
unsafe extern "C" fn sba_device_terminate_all(dchan: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sba_issue_pending(dchan: *mut dma_chan) {
...
#[no_mangle]
unsafe extern "C" fn sba_tx_submit(tx: *mut dma_async_tx_descriptor) -> dma_cookie_t {
...
// Assign cookie and mark all chained requests pending
    ...
    u64 cmd;
    u32 c_mdata;
    let mut resp_dma: dma_addr_t = req.tx.phys;
...
// Init async_tx descriptor
    ...
    {
    size_t req_len;
    let mut off: dma_addr_t = 0;
    struct sba_device *sba = to_sba_device(dchan);
    struct sba_request *first = core::ptr::null_mut(), *req;
// Create chained requests where each request is upto hw_buf_size
    ...
    return (first) ? &first.tx : core::ptr::null_mut();
    }
...
// Init async_tx descriptor
    ...
    let mut off: dma_addr_t = 0;
    dma_addr_t *dst_p = core::ptr::null_mut(), *dst_q = core::ptr::null_mut();
    struct sba_device *sba = to_sba_device(dchan);
    struct sba_request *first = core::ptr::null_mut(), *req;
// Sanity checks
...
// ====== Mailbox callbacks =====
#[no_mangle]
unsafe extern "C" fn sba_receive_message(cl: *mut mbox_client, msg: *mut c_void) {
...
// ====== Debugfs callbacks ======
#[no_mangle]
unsafe extern "C" fn sba_debugfs_stats_show(file: *mut seq_file, offset: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sba_prealloc_channel_resources(sba: *mut sba_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sba_freeup_channel_resources(sba: *mut sba_device) {
    ...
    sba.resp_base, sba.resp_dma_base);
    sba.resp_base = core::ptr::null_mut();
    sba.resp_dma_base = 0;
    }
#[no_mangle]
unsafe extern "C" fn sba_async_register(sba: *mut sba_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sba_probe(pdev: *mut platform_device) -> c_int {
...
// Create debugfs root entry
    sba.root = debugfs_create_dir(dev_name(sba.dev), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn sba_remove(pdev: *mut platform_device) {
