...
//! Source: drivers/dma/stm32/stm32-dma3.c
#![no_std]
...
pub const STM32_DMA3_SECCFGR: c_uint = 0x00;
pub const STM32_DMA3_PRIVCFGR: c_uint = 0x04;
pub const STM32_DMA3_RCFGLOCKR: c_uint = 0x08;
pub const STM32_DMA3_MISR: c_uint = 0x0c;
pub const STM32_DMA3_SMISR: c_uint = 0x10;

pub const STM32_DMA3_HWCFGR13: c_uint = 0xfc0 /* G_PER_CTRL(X) x=8..15 */;
pub const STM32_DMA3_HWCFGR12: c_uint = 0xfc4 /* G_PER_CTRL(X) x=0..7 */;
pub const STM32_DMA3_HWCFGR4: c_uint = 0xfe4 /* G_FIFO_SIZE(X) x=8..15 */;
pub const STM32_DMA3_HWCFGR3: c_uint = 0xfe8 /* G_FIFO_SIZE(X) x=0..7 */;
pub const STM32_DMA3_HWCFGR2: c_uint = 0xfec /* G_MAX_REQ_ID */;
pub const STM32_DMA3_HWCFGR1: c_uint = 0xff0 /* G_MASTER_PORTS, G_NUM_CHANNELS, G_Mx_DATA_WIDTH */;
pub const STM32_DMA3_VERR: c_uint = 0xff4;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dma3_lli {
    pub hwdesc: *mut stm32_dma3_hwdesc,
    pub hwdesc_addr: dma_addr_t,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dma3_swdesc {
    pub vdesc: virt_dma_desc,
    pub ccr: u32,
    pub cyclic: bool,
    pub lli_size: u32,
    pub __counted_by(lli_size): stm32_dma3_lli lli[],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dma3_dt_conf {
    pub ch_id: u32,
    pub req_line: u32,
    pub ch_conf: u32,
    pub tr_conf: u32,
...
pub struct stm32_dma3_chan {
    ...
    pub semaphore_taken: bool,
    pub dt_config: stm32_dma3_dt_conf,
    pub dma_config: dma_slave_config,
    pub config_set: u8,
    pub lli_pool: *mut dma_pool,
    pub swdesc: *mut stm32_dma3_swdesc,
    pub tcem: enum ctr2_tcem,
    pub dma_status: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dma3_pdata {
    pub axi_max_burst_len: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dma3_ddata {
    pub dma_dev: dma_device,
    pub base: *mut void __iomem,
    pub clk: *mut clk,
    pub chans: *mut stm32_dma3_chan,
    pub dma_channels: u32,
    pub dma_requests: u32,
    pub ports_max_dw: [enum stm32_dma3_port_data_width; 2],
    pub axi_max_burst_len: u32,
}
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_vdesc_free(vdesc: *mut virt_dma_desc) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_check_user_setting(chan: *mut stm32_dma3_chan) {
    static void stm32_dma3_check_user_setting(struct stm32_dma3_chan *chan)
    {
    struct stm32_dma3_ddata *ddata = to_stm32_dma3_ddata(chan);
    struct device *dev = chan2dev(chan);
    let mut ctr1: u32 = readl_relaxed(ddata.base + STM32_DMA3_CTR1(chan.id));
    let mut cbr1: u32 = readl_relaxed(ddata.base + STM32_DMA3_CBR1(chan.id));
    let mut csar: u32 = readl_relaxed(ddata.base + STM32_DMA3_CSAR(chan.id));
    let mut cdar: u32 = readl_relaxed(ddata.base + STM32_DMA3_CDAR(chan.id));
    let mut cllr: u32 = readl_relaxed(ddata.base + STM32_DMA3_CLLR(chan.id));
    let mut bndt: u32 = FIELD_GET(CBR1_BNDT, cbr1);
    let mut sdw: u32 = 1 << FIELD_GET(CTR1_SDW_LOG2, ctr1);
    let mut ddw: u32 = 1 << FIELD_GET(CTR1_DDW_LOG2, ctr1);
    let mut sap: u32 = FIELD_GET(CTR1_SAP, ctr1);
    let mut dap: u32 = FIELD_GET(CTR1_DAP, ctr1);
...
//
    ...
    {
    let mut max_dw: enum dma_slave_buswidth = get_chan_max_dw(port_max_dw, chan_max_burst);
// len and addr must be a multiple of dw
    ...
    {
    let mut max_burst: u32 = chan_max_burst ? chan_max_burst / dw : 1;
...
//
    ...
    {
    struct stm32_dma3_ddata *ddata = to_stm32_dma3_ddata(chan);
    let mut dma_device: dma_device = ddata.dma_dev;
    let mut src_max_burst: u32 = STM32_DMA3_MAX_BURST_LEN, dst_max_burst = STM32_DMA3_MAX_BURST_LEN;
    u32 sdw, ddw, sbl_max, dbl_max, tcem, init_dw, init_bl_max;
    let mut _ctr1: u32 = 0, _ctr2 = 0;
    let mut ch_conf: u32 = chan.dt_config.ch_conf;
    let mut tr_conf: u32 = chan.dt_config.tr_conf;
    let mut sap: u32 = FIELD_GET(STM32_DMA3_DT_SAP, tr_conf), sap_max_dw;
    let mut dap: u32 = FIELD_GET(STM32_DMA3_DT_DAP, tr_conf), dap_max_dw;
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_start(chan: *mut stm32_dma3_chan) {
    static void stm32_dma3_chan_start(struct stm32_dma3_chan *chan)
    {
    struct stm32_dma3_ddata *ddata = to_stm32_dma3_ddata(chan);
    struct virt_dma_desc *vdesc;
    struct stm32_dma3_hwdesc *hwdesc;
    let mut id: u32 = chan.id;
    u32 csr, ccr;
    vdesc = vchan_next_desc(&chan.vchan);
    if (!vdesc) {
    chan.swdesc = core::ptr::null_mut();
    return;
    }
    list_del(&vdesc.node);
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_suspend(chan: *mut stm32_dma3_chan, susp: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_reset(chan: *mut stm32_dma3_chan) {
    static void stm32_dma3_chan_reset(struct stm32_dma3_chan *chan)
    {
    struct stm32_dma3_ddata *ddata = to_stm32_dma3_ddata(chan);
    let mut ccr: u32 = readl_relaxed(ddata.base + STM32_DMA3_CCR(chan.id)) & ~CCR_EN;
    writel_relaxed(ccr |= CCR_RESET, ddata.base + STM32_DMA3_CCR(chan.id));
    }
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_get_curr_hwdesc(swdesc: *mut stm32_dma3_swdesc, cllr: u32, residue: *mut u32) -> c_int {
...
//
    ...
    let mut bytes_read: c_int = FIELD_GET(CBR1_BNDT, hwdesc.cbr1) - bndt;
    let mut bytes_written: c_int = cdar - hwdesc.cdar;
    if (bytes_read > 0)
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_stop(chan: *mut stm32_dma3_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_complete(chan: *mut stm32_dma3_chan) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_chan_irq(irq: c_int, devid: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_get_chan_sem(chan: *mut stm32_dma3_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_put_chan_sem(chan: *mut stm32_dma3_chan) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_alloc_chan_resources(c: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_free_chan_resources(c: *mut dma_chan) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_get_ll_count(chan: *mut stm32_dma3_chan, len: usize, prevent_refactor: bool) -> u32 {
...
// Unaligned remainder fits in one extra item
    ...
    let mut burst: u32 = chan.max_burst / dw;
...
// sg_len and i correspond to the initial sgl; count and j correspond to the hwdesc LL
    ...
    len = sg_dma_len(sg);
    do {
    let mut chunk: usize = min_t(size_t, len, STM32_DMA3_MAX_BLOCK_SIZE);
    if (!prevent_refactor &&
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_caps(c: *mut dma_chan, caps: *mut dma_slave_caps) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_config(c: *mut dma_chan, config: *mut dma_slave_config) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_pause(c: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_resume(c: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_terminate_all(c: *mut dma_chan) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_synchronize(c: *mut dma_chan) {
    ...
    {
    struct stm32_dma3_chan *chan = to_stm32_dma3_chan(c);
    struct stm32_dma3_swdesc *swdesc = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(cookie: chan->swdesc && chan->swdesc->vdesc.tx.cookie ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_issue_pending(c: *mut dma_chan) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_filter_fn(c: *mut dma_chan, fn_param: *mut c_void) -> bool {
...
// Check if chan fifo fits well
    ...
    let mut mask: dma_cap_mask_t = ddata.dma_dev.cap_mask;
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_check_rif(ddata: *mut stm32_dma3_ddata) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_probe(pdev: *mut platform_device) -> c_int {
    ...
    reset = devm_reset_control_get_optional(&pdev.dev, core::ptr::null_mut());
    if (IS_ERR(reset))
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_remove(pdev: *mut platform_device) {
    static void stm32_dma3_remove(struct platform_device *pdev)
    {
    pm_runtime_disable(&pdev.dev);
    }
#[no_mangle]
unsafe extern "C" fn stm32_dma3_runtime_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_runtime_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_pm_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dma3_pm_resume(dev: *mut device) -> c_int {
...
//
    ...
    RUNTIME_PM_OPS(stm32_dma3_runtime_suspend, stm32_dma3_runtime_resume, core::ptr::null_mut())
    };
