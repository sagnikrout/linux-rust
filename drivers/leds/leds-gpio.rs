...
//! Source: drivers/leds/leds-gpio.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gpio_led_data {
    pub cdev: led_classdev,
    pub gpiod: *mut gpio_desc,
    pub can_sleep: u8,
    pub blinking: u8,
    pub platform_gpio_blink_set: gpio_blink_set_t,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gpio_leds_priv {
    pub num_leds: c_int,
    pub __counted_by(num_leds): gpio_led_data leds[],
}
    ...
    let mut led: gpio_led = {};
//
// Acquire gpiod from DT with uninitialized label, which
// will be updated after LED class device is registered,
// Only then the final LED name is known.
//
    led.gpiod = devm_fwnode_gpiod_get(dev, child, core::ptr::null_mut(), GPIOD_ASIS,
    core::ptr::null_mut());
    if (IS_ERR(led.gpiod)) {
    ...
    ret = create_gpio_led(&led, led_dat, dev, child, core::ptr::null_mut());
    if (ret < 0)
    return ERR_PTR(ret);
...
// the GPIO from there.
//
    gpiod = devm_gpiod_get_index_optional(dev, core::ptr::null_mut(), idx, GPIOD_OUT_LOW);
    if (IS_ERR(gpiod))
    return gpiod;
...
#[no_mangle]
unsafe extern "C" fn gpio_led_probe(pdev: *mut platform_device) -> c_int {
    ...
    template.name);
    led_dat.gpiod = core::ptr::null_mut();
    continue;
    }
    ret = create_gpio_led(template, led_dat, dev, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn gpio_led_shutdown(pdev: *mut platform_device) {
