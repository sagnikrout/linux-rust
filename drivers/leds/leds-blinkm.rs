...
//! Source: drivers/leds/leds-blinkm.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct blinkm_data {
    pub i2c_client: *mut i2c_client,
    pub update_lock: mutex,
// used for led class interface
    pub blinkm_leds: [blinkm_led; NUM_LEDS],
// used for "blinkm" sysfs interface
    pub /: *mut *mut u8 red; / color red,
    pub /: *mut *mut u8 green; / color green,
    pub /: *mut *mut u8 blue; / color blue,
// next values to use for transfer
    pub /: *mut *mut u8 next_red; / color red,
    pub /: *mut *mut u8 next_green; / color green,
    pub /: *mut *mut u8 next_blue; / color blue,
// internal use
    pub /: *mut *mut u8 args[7]; / set of args for transmission,
    pub /: *mut *mut u8 i2c_addr; / i2c addr,
    pub /: *mut *mut u8 fw_ver; / firmware version,
// used, but not from userspace
    pub /: *mut *mut u8 hue; / HSB hue,
    pub /: *mut *mut u8 saturation; / HSB saturation,
    pub /: *mut *mut u8 brightness; / HSB brightness,
    pub /: *mut *mut u8 next_hue; / HSB hue,
    pub /: *mut *mut u8 next_saturation; / HSB saturation,
    pub /: *mut *mut u8 next_brightness; / HSB brightness,
// currently unused / todo
    pub /: *mut *mut u8 fade_speed; / fade speed 1 - 255,
    pub /: *mut *mut s8 time_adjust; / time adjust -128 - 127,
    pub /: *mut *mut u8 fade:1; / fade on = 1, off = 0,
    pub /: *mut *mut u8 rand:1; / rand fade mode on = 1,
    pub /: *mut *mut u8 script_id; / script ID,
    pub /: *mut *mut u8 script_repeats; / repeats of script,
    pub /: *mut *mut u8 script_startline; / line to start,
}
...
pub const BLM_GO_RGB: c_int = 0;
pub const BLM_FADE_RGB: c_int = 1;
pub const BLM_FADE_HSB: c_int = 2;
pub const BLM_FADE_RAND_RGB: c_int = 3;
pub const BLM_FADE_RAND_HSB: c_int = 4;
pub const BLM_PLAY_SCRIPT: c_int = 5;
pub const BLM_STOP_SCRIPT: c_int = 6;
pub const BLM_SET_FADE_SPEED: c_int = 7;
pub const BLM_SET_TIME_ADJ: c_int = 8;
pub const BLM_GET_CUR_RGB: c_int = 9;
pub const BLM_WRITE_SCRIPT_LINE: c_int = 10;
pub const BLM_READ_SCRIPT_LINE: c_int = 11;

pub const BLM_SET_ADDR: c_int = 13;
pub const BLM_GET_ADDR: c_int = 14;
pub const BLM_GET_FW_VER: c_int = 15;
pub const BLM_SET_STARTUP_PARAM: c_int = 16;
// BlinkM Commands
...
#[no_mangle]
unsafe extern "C" fn show_color_common(dev: *mut device, buf: *mut c_char, color: c_int) -> isize {
...
#[no_mangle]
unsafe extern "C" fn store_color_common(dev: *mut device, buf: *const c_char, color: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn blinkm_write(client: *mut i2c_client, cmd: c_int, arg: *mut u8) -> c_int {
    static int blinkm_write(struct i2c_client *client, int cmd, u8 *arg)
    {
    int result;
    int i;
    let mut arglen: c_int = blinkm_cmds[cmd].nr_args;
...
#[no_mangle]
unsafe extern "C" fn blinkm_read(client: *mut i2c_client, cmd: c_int, arg: *mut u8) -> c_int {
    static int blinkm_read(struct i2c_client *client, int cmd, u8 *arg)
    {
    int result;
    int i;
    let mut retlen: c_int = blinkm_cmds[cmd].nr_ret;
    for (i = 0; i < retlen; i++) {
// repeat for retlen
...
#[no_mangle]
unsafe extern "C" fn blinkm_transfer_hw(client: *mut i2c_client, cmd: c_int) -> c_int {
    static int blinkm_transfer_hw(struct i2c_client *client, int cmd)
    {
...
// switch cmd - usually write before reads
    ...
    blinkm_write(client, cmd, core::ptr::null_mut());
    break;
...
#[no_mangle]
unsafe extern "C" fn blinkm_init_hw(client: *mut i2c_client) {
...
#[no_mangle]
unsafe extern "C" fn blinkm_test_run(client: *mut i2c_client) -> c_int {
    static int blinkm_test_run(struct i2c_client *client)
    {
    int ret;
    struct blinkm_data *data = i2c_get_clientdata(client);
...
#[no_mangle]
unsafe extern "C" fn blinkm_detect(client: *mut i2c_client, info: *mut i2c_board_info) -> c_int {
...
// make sure the blinkM is balanced (read/writes)
    while (count > 0) {
    ret = blinkm_write(client, BLM_GET_ADDR, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn register_separate_colors(client: *mut i2c_client, data: *mut blinkm_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn register_multicolor(client: *mut i2c_client, data: *mut blinkm_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn blinkm_probe(client: *mut i2c_client) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn blinkm_remove(client: *mut i2c_client) {
