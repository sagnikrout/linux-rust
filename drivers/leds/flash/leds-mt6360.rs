...
//! Source: drivers/leds/flash/leds-mt6360.c
#![no_std]
...
pub const MT6360_REG_RGBEN: c_uint = 0x380;

// Virtual definition for multicolor

pub const MULTICOLOR_NUM_CHANNELS: c_int = 3;
pub const MT6360_REG_FLEDEN: c_uint = 0x37E;
pub const MT6360_REG_STRBTO: c_uint = 0x373;

pub const MT6360_REG_CHGSTAT2: c_uint = 0x3E1;
pub const MT6360_REG_FLEDSTAT1: c_uint = 0x3E9;

pub const MT6360_ISNKRGB_STEPUA: c_int = 2000;
pub const MT6360_ISNKRGB_MAXUA: c_int = 24000;
pub const MT6360_ISNKML_STEPUA: c_int = 5000;
pub const MT6360_ISNKML_MAXUA: c_int = 150000;
pub const MT6360_ITORCH_MINUA: c_int = 25000;
pub const MT6360_ITORCH_STEPUA: c_int = 12500;
pub const MT6360_ITORCH_MAXUA: c_int = 400000;
pub const MT6360_ISTRB_MINUA: c_int = 50000;
pub const MT6360_ISTRB_STEPUA: c_int = 12500;
pub const MT6360_ISTRB_MAXUA: c_int = 1500000;
pub const MT6360_STRBTO_MINUS: c_int = 64000;
pub const MT6360_STRBTO_STEPUS: c_int = 32000;
pub const MT6360_STRBTO_MAXUS: c_int = 2432000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mt6360_led {
    union {
    pub isnk: led_classdev,
    pub mc: led_classdev_mc,
...
pub struct mt6360_priv {
    ...
    pub __counted_by(leds_count): mt6360_led leds[],
}
    ...
    struct mt6360_priv *priv = led.priv;
    let mut enable_mask: u32 = MT6360_ISNK_ENMASK(led.led_no);
    let mut val: u32 = level ? MT6360_ISNK_ENMASK(led.led_no) : 0;
    int ret;
    mutex_lock(&priv.lock);
    ...
    struct mt6360_priv *priv = led.priv;
    let mut enable_mask: u32 = MT6360_TORCHEN_MASK | MT6360_FLCSEN_MASK(led.led_no);
    let mut val: u32 = level ? MT6360_FLCSEN_MASK(led.led_no) : 0;
    let mut prev: u32 = priv.fled_torch_used, curr;
    int ret;
    mutex_lock(&priv.lock);
//
...
#[no_mangle]
unsafe extern "C" fn mt6360_strobe_set(fl_cdev: *mut led_classdev_flash, state: bool) -> c_int {
    ...
    struct led_flash_setting *s = &fl_cdev.brightness;
    let mut enable_mask: u32 = MT6360_STROBEN_MASK | MT6360_FLCSEN_MASK(led.led_no);
    let mut val: u32 = state ? MT6360_FLCSEN_MASK(led.led_no) : 0;
    let mut prev: u32 = priv.fled_strobe_used, curr;
    let mut ret: c_int = 0;
    mutex_lock(&priv.lock);
//
...
#[no_mangle]
pub unsafe extern "C" fn if(!curr: prev &&) -> else {
    else if (prev && !curr)
    udelay(500);
...
#[no_mangle]
unsafe extern "C" fn mt6360_strobe_get(fl_cdev: *mut led_classdev_flash, state: *mut bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt6360_timeout_set(fl_cdev: *mut led_classdev_flash, timeout: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt6360_fault_get(fl_cdev: *mut led_classdev_flash, fault: *mut u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt6360_isnk_init_default_state(led: *mut mt6360_led) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt6360_flash_init_default_state(led: *mut mt6360_led) -> c_int {
    ...
    struct mt6360_priv *priv = led.priv;
    let mut mask: u32 = MT6360_FLCSEN_MASK(led.led_no);
    ...
    {
    struct mt6360_priv *priv = led.priv;
    let mut v4l2_config: v4l2_flash_config = {0};
    int ret;
...
#[no_mangle]
unsafe extern "C" fn clamp_align(val: u32, min: u32, max: u32, step: u32) -> u32 {
    ...
    let mut step_uA: u32 = MT6360_ISNKRGB_STEPUA, max_uA = MT6360_ISNKRGB_MAXUA;
    u32 val;
    let mut num_color: c_int = 0, ret;
...
#[no_mangle]
unsafe extern "C" fn mt6360_v4l2_flash_release(priv: *mut mt6360_priv) {
...
#[no_mangle]
unsafe extern "C" fn mt6360_led_probe(pdev: *mut platform_device) -> c_int {
    ...
    struct mt6360_led *led = priv.leds + i;
    let mut init_data: led_init_data = { .fwnode = child, };
    u32 reg, led_color;
...
#[no_mangle]
unsafe extern "C" fn mt6360_led_remove(pdev: *mut platform_device) {
