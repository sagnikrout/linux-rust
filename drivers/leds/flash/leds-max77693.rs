...
//! Source: drivers/leds/flash/leds-max77693.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct max77693_led_config_data {
    pub label: [*const c_char; 2],
    pub iout_torch_max: [u32; 2],
    pub iout_flash_max: [u32; 2],
    pub flash_timeout_max: [u32; 2],
    pub num_leds: u32,
    pub boost_mode: u32,
    pub boost_vout: u32,
    pub low_vsys: u32,
...
// brightness cache
    pub torch_brightness: c_uint,
// flash timeout cache
    pub flash_timeout: c_uint,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct max77693_led_device {
// parent mfd regmap
...
#[no_mangle]
unsafe extern "C" fn max77693_led_iout_to_reg(ua: u32) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn max77693_flash_timeout_to_reg(us: u32) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn max77693_led_vsys_to_reg(mv: u32) -> u8 {
    static inline u8 max77693_led_vsys_to_reg(u32 mv)
    {
    return ((mv - MAX_FLASH1_VSYS_MIN) / MAX_FLASH1_VSYS_STEP) << 2;
    }
#[no_mangle]
pub unsafe extern "C" fn max77693_led_vout_to_reg(mv: u32) -> u8 {
    ...
    int fled_id)
    {
    let mut fled_bit: u8 = (fled_id == FLED1) ? FLED1_IOUT : FLED2_IOUT;
    return led.fled_mask & fled_bit;
    }
#[no_mangle]
unsafe extern "C" fn max77693_set_mode_reg(led: *mut max77693_led_device, mode: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn max77693_add_mode(led: *mut max77693_led_device, mode: u8) -> c_int {
...
// Clear mode also on FLED2 for joint iouts case
    ...
    {
    struct regmap *rmap = led.regmap;
    let mut iout1_reg: u8 = 0, iout2_reg = 0;
    u32 iout[2];
...
#[no_mangle]
unsafe extern "C" fn max77693_set_timeout(led: *mut max77693_led_device, microsec: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn max77693_get_flash_faults(sub_led: *mut max77693_sub_led) -> c_int {
...
// LED subsystem callbacks
    ...
    struct max77693_led_device *led = sub_led_to_led(sub_led);
    let mut fled_id: c_int = sub_led.fled_id, ret;
    mutex_lock(&led.lock);
    if (value == 0) {
    ...
    struct max77693_sub_led *sub_led = flcdev_to_sub_led(fled_cdev);
    struct max77693_led_device *led = sub_led_to_led(sub_led);
    let mut fled_id: c_int = sub_led.fled_id;
    int ret;
    mutex_lock(&led.lock);
    if (!state) {
...
// state = !!(*state && (led->strobing_sub_led_id == sub_led->fled_id));
    ...
    prop = of_find_property(child_node, "led-sources", core::ptr::null_mut());
    if (prop) {
    const __be32 *srcs = core::ptr::null_mut();
    ...
    cfg.label[fled_id] =
    of_get_property(child_node, "label", core::ptr::null_mut()) ? :
    child_node.name;
...
#[no_mangle]
unsafe extern "C" fn clamp_align(v: *mut u32, min: u32, max: u32, step: u32) {
...
#[no_mangle]
unsafe extern "C" fn max77693_led_probe(pdev: *mut platform_device) -> c_int {
    ...
    let mut led_cfg: max77693_led_config_data = {};
...
#[no_mangle]
unsafe extern "C" fn max77693_led_remove(pdev: *mut platform_device) {
