...
//! Source: drivers/leds/rgb/leds-s2m-rgb.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn s2mu005_rgb_apply_params(rgb: *mut s2m_rgb) -> c_int {
    static int s2mu005_rgb_apply_params(struct s2m_rgb *rgb)
    {
    struct regmap *regmap = rgb.regmap;
    let mut ramp_val: c_uint = 0;
    let mut stay_val: c_uint = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn s2mu005_rgb_reset_params(rgb: *mut s2m_rgb) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn s2m_rgb_lut_get_closest_duration(lut: *const u32, len: usize, duration: u32) -> c_int {
    static int s2m_rgb_lut_get_closest_duration(const u32 *lut, const size_t len, const u32 duration)
    {
    let mut closest_distance: u32 = abs(duration - lut[0]);
    let mut closest_index: c_int = 0;
    let mut lo: c_int = 0;
    let mut hi: c_int = len - 1;
//
...
// Narrow down search window as per binary-search algorithm.
    ...
    let mut brightness_peak: c_int = 0;
    let mut time_hi: u32 = 0, time_lo = 0;
    let mut ramp_up_en: bool = false, ramp_dn_en = false;
    int ret;
//
...
// Pattern should consist of at least two tuples.
    ...
    return -EINVAL;
    for (int i = 0; i < len; i++) {
    let mut brightness: c_int = pattern[i].brightness;
    let mut delta_t: u32 = pattern[i].delta_t;
    if (brightness) {
//
...
#[no_mangle]
unsafe extern "C" fn s2m_rgb_pattern_clear(cdev: *mut led_classdev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s2m_rgb_brightness_set(cdev: *mut led_classdev, value: enum led_brightness) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s2m_rgb_probe(pdev: *mut platform_device) -> c_int {
