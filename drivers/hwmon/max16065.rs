...
//! Source: drivers/hwmon/max16065.c
#![no_std]
...
pub const MAX16065_CURR_SENSE: c_uint = 0x18;
pub const MAX16065_CSP_ADC: c_uint = 0x19;

pub const MAX16065_CURR_CONTROL: c_uint = 0x47;
...
// x: ADC index
//
pub const MAX16065_SW_ENABLE: c_uint = 0x73;

    warning */

pub const MAX16065_NUM_LIMIT: c_int = 3;
...
pub struct max16065_data {
    ...
    pub num_adc: c_int,
    pub have_current: bool,
    pub curr_gain: c_int,
// limits are in mV
    pub limit: [c_int; MAX16065_NUM_LIMIT][MAX16065_NUM_ADC],
    pub /: *mut *mut int range[MAX16065_NUM_ADC + 1];/ voltage range,
    pub /: *mut *mut int adc[MAX16065_NUM_ADC + 1]; / adc values (raw) including csp_adc,
    pub curr_sense: c_int,
    pub fault: [c_int; 2],
}
...
#[no_mangle]
pub unsafe extern "C" fn ADC_TO_MV(adc: c_int, range: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn LIMIT_TO_MV(limit: c_int, range: c_int) -> c_int {
    static inline int LIMIT_TO_MV(int limit, int range)
    {
    return limit * range / 256;
    }
#[no_mangle]
pub unsafe extern "C" fn MV_TO_LIMIT(mv: c_ulong, range: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ADC_TO_CURR(adc: c_int, gain: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn max16065_read_adc(client: *mut i2c_client, reg: c_int) -> c_int {
...
//
    ...
    let mut val: c_int = READ_ONCE(data.fault[attr2.nr]);
    if (val < 0)
    return val;
    val &= (1 << attr2.index);
    if (val)
    ...
    struct max16065_data *data = max16065_update_device(dev);
    let mut adc: c_int = READ_ONCE(data.adc[attr.index]);
    if (unlikely(adc < 0))
    return adc;
    return sysfs_emit(buf, "%d\n",
    ...
    {
    struct max16065_data *data = max16065_update_device(dev);
    let mut curr_sense: c_int = READ_ONCE(data.curr_sense);
...
#[no_mangle]
unsafe extern "C" fn max16065_probe(client: *mut i2c_client) -> c_int {
    ...
    let mut chip: enum chips = (uintptr_t)i2c_get_match_data(client);
