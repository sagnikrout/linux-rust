...
// LM73 registers
pub const LM73_REG_INPUT: c_uint = 0x00;
pub const LM73_REG_CONF: c_uint = 0x01;
pub const LM73_REG_MAX: c_uint = 0x02;
pub const LM73_REG_MIN: c_uint = 0x03;
pub const LM73_REG_CTRL: c_uint = 0x04;
pub const LM73_REG_ID: c_uint = 0x07;
pub const LM73_ID: c_uint = 0x9001	/* 0x0190, byte-swapped */;

pub const LM73_CTRL_RES_SHIFT: c_int = 5;

pub const LM73_CTRL_HI_SHIFT: c_int = 2;
pub const LM73_CTRL_LO_SHIFT: c_int = 1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lm73_data {
    pub client: *mut i2c_client,
    pub lock: mutex,
    pub /: *mut *mut u8 ctrl; / control register value,
}
...
// -----------------------------------------------------------------------
    ...
    short value;
    s32 err;
    let mut status: c_int = kstrtol(buf, 10, &temp);
    if (status < 0)
    return status;
// Write value
    ...
    struct lm73_data *data = dev_get_drvdata(dev);
    int temp;
    let mut err: i32 = i2c_smbus_read_word_swapped(data.client, attr.index);
    if (err < 0)
    return err;
