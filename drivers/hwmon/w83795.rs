...
pub const W83795_REG_BANKSEL: c_uint = 0x00;
pub const W83795_REG_VENDORID: c_uint = 0xfd;
pub const W83795_REG_CHIPID: c_uint = 0xfe;
pub const W83795_REG_DEVICEID: c_uint = 0xfb;
pub const W83795_REG_DEVICEID_A: c_uint = 0xff;
pub const W83795_REG_I2C_ADDR: c_uint = 0xfc;
pub const W83795_REG_CONFIG: c_uint = 0x01;
pub const W83795_REG_CONFIG_CONFIG48: c_uint = 0x04;
pub const W83795_REG_CONFIG_START: c_uint = 0x01;
// Multi-Function Pin Ctrl Registers
pub const W83795_REG_VOLT_CTRL1: c_uint = 0x02;
pub const W83795_REG_VOLT_CTRL2: c_uint = 0x03;
pub const W83795_REG_TEMP_CTRL1: c_uint = 0x04;
pub const W83795_REG_TEMP_CTRL2: c_uint = 0x05;
pub const W83795_REG_FANIN_CTRL1: c_uint = 0x06;
pub const W83795_REG_FANIN_CTRL2: c_uint = 0x07;
pub const W83795_REG_VMIGB_CTRL: c_uint = 0x08;
...
pub const W83795_REG_VRLSB: c_uint = 0x3C;
...
pub const IN_LSB_SHIFT: c_int = 0;
pub const IN_LSB_IDX: c_int = 1;
...
// High/Low LSB shift, LSB No.
    ...
    (((index) & 1) ? 4 : 0)
pub const W83795_REG_VID_CTRL: c_uint = 0x6A;
pub const W83795_REG_ALARM_CTRL: c_uint = 0x40;

pub const W83795_REG_CLR_CHASSIS: c_uint = 0x4D;

pub const W83795_REG_OVT_CFG: c_uint = 0x58;

pub const W83795_REG_FCMS1: c_uint = 0x201;
pub const W83795_REG_FCMS2: c_uint = 0x208;

pub const W83795_REG_FOMC: c_uint = 0x20F;

pub const TSS_MAP_RESERVED: c_uint = 0xff;
...
pub const PWM_OUTPUT: c_int = 0;
pub const PWM_FREQ: c_int = 1;
pub const PWM_START: c_int = 2;
pub const PWM_NONSTOP: c_int = 3;
pub const PWM_STOP_TIME: c_int = 4;

pub const W83795_REG_TFTS: c_uint = 0x250;
pub const TEMP_PWM_TTTI: c_int = 0;
pub const TEMP_PWM_CTFS: c_int = 1;
pub const TEMP_PWM_HCT: c_int = 2;
pub const TEMP_PWM_HOT: c_int = 3;

pub const SF4_TEMP: c_int = 0;
pub const SF4_PWM: c_int = 1;

    (0x280 + 0x10 * (temp_num) + (index))

    (0x288 + 0x10 * (temp_num) + (index))
pub const W83795_REG_DTSC: c_uint = 0x301;
pub const W83795_REG_DTSE: c_uint = 0x302;

pub const DTS_CRIT: c_int = 0;
pub const DTS_CRIT_HYST: c_int = 1;
pub const DTS_WARN: c_int = 2;
pub const DTS_WARN_HYST: c_int = 3;

pub const SETUP_PWM_DEFAULT: c_int = 0;
pub const SETUP_PWM_UPTIME: c_int = 1;
pub const SETUP_PWM_DOWNTIME: c_int = 2;

#[no_mangle]
pub unsafe extern "C" fn in_from_reg(index: u8, val: u16) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn in_to_reg(index: u8, val: u16) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn pwm_freq_from_reg(reg: u8, clkin: u16) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn pwm_freq_to_reg(val: c_ulong, clkin: u16) -> u8 {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct w83795_data {
    pub hwmon_dev: *mut device,
    pub update_lock: mutex,
    pub /: *mut *mut unsigned long last_updated; / In jiffies,
    pub chip_type: enum chip_types,
    pub bank: u8,
    pub /: *mut *mut u32 has_in; / Enable monitor VIN or not,
    pub /: *mut *mut u8 has_dyn_in; / Only in2-0 can have this,
    pub /: *mut *mut u16 in[21][3]; / Register value, read/high/low,
    pub /: *mut *mut u8 in_lsb[10][3]; / LSB Register value, high/low,
    pub /: *mut *mut *mut u8 has_gain; / has gain: in17-20  8,
    pub /: *mut *mut u16 has_fan; / Enable fan14-1 or not,
    pub /: *mut *mut u16 fan[14]; / Register value combine,
    pub /: *mut *mut u16 fan_min[14]; / Register value combine,
    pub /: *mut *mut u8 has_temp; / Enable monitor temp6-1 or not,
    pub /: *mut *mut s8 temp[6][5]; / current, crit, crit_hyst, warn, warn_hyst,
    pub temp_read_vrlsb: [u8; 6],
    pub /: *mut *mut u8 temp_mode; / Bit vector, 0 = TR, 1 = TD,
    pub /: *mut *mut u8 temp_src[3]; / Register value,
    pub /*: *mut u8 enable_dts;,
// Enable PECI and SB-TSI,
// bit 0: =1 enable, =0 disable,
// bit 1: =1 AMD SB-TSI, =0 Intel PECI
//
    pub /: *mut *mut u8 has_dts; / Enable monitor DTS temp,
    pub /: *mut *mut s8 dts[8]; / Register value,
    pub /: *mut *mut u8 dts_read_vrlsb[8]; / Register value,
    pub /: *mut *mut s8 dts_ext[4]; / Register value,
    pub /*: *mut u8 has_pwm;,
...
// non stop, stop time
//
    pub /: *mut *mut u16 clkin; / CLKIN frequency in kHz,
    pub /: *mut *mut u8 pwm_fcms[2]; / Register value,
    pub /: *mut *mut u8 pwm_tfmr[6]; / Register value,
    pub /: *mut *mut u8 pwm_fomc; / Register value,
    pub /*: *mut u16 target_speed[8];,
// Register value, target speed for speed
// cruise
//
    pub /: *mut *mut u8 tol_speed; / tolerance of target speed,
    pub /: *mut *mut u8 pwm_temp[6][4]; / TTTI, CTFS, HCT, HOT,
    pub /: *mut *mut u8 sf4_reg[6][2][7]; / 6 temp, temp/dcpwm, 7 registers,
    pub /: *mut *mut u8 setup_pwm[3]; / Register value,
    pub /: *mut *mut u8 alarms[6]; / Register value,
    pub enable_beep: u8,
    pub /: *mut *mut u8 beeps[6]; / Register value,
    pub valid: bool,
    pub valid_limits: c_char,
    pub valid_pwm_config: c_char,
}
...
#[no_mangle]
unsafe extern "C" fn w83795_set_bank(client: *mut i2c_client, bank: u8) -> c_int {
...
// Must be called with data->update_lock held, except during initialization
#[no_mangle]
unsafe extern "C" fn w83795_read(client: *mut i2c_client, reg: u16) -> u8 {
...
// Must be called with data->update_lock held, except during initialization
#[no_mangle]
unsafe extern "C" fn w83795_write(client: *mut i2c_client, reg: u16, value: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn w83795_update_limits(client: *mut i2c_client) {
...
// in0-2 can have dynamic limits (W83795G only)
    if (data.has_dyn_in) {
    let mut lsb_max: u8 = w83795_read(client, IN_LSB_REG(0, IN_MAX));
    let mut lsb_low: u8 = w83795_read(client, IN_LSB_REG(0, IN_LOW));
    for (i = 0; i < 3; i++) {
...
//
#[no_mangle]
unsafe extern "C" fn w83795_tss_useful(data: *const w83795_data, tsrc: c_int) -> c_int {
    static int w83795_tss_useful(const struct w83795_data *data, int tsrc)
    {
    let mut useful: c_int = 0, i;
    for (i = 0; i < 4; i++) {
    ...
    struct w83795_data *data = w83795_update_pwm_config(dev);
    let mut index: c_int = sensor_attr.index;
    let mut tmp: u8 = data.temp_src[index / 2];
    if (index & 1)
...
pub const TEMP_PWM_ENABLE: c_int = 0;
pub const TEMP_PWM_FAN_MAP: c_int = 1;
    static ssize_t
...
pub const FANIN_TARGET: c_int = 0;
pub const FANIN_TOL: c_int = 1;
    static ssize_t
    ...
    let mut nr: c_int = sensor_attr.nr;
    let mut index: c_int = sensor_attr.index;
    let mut tmp: c_long = temp_from_reg(data.pwm_temp[index][nr]);
    ...
    let mut index: c_int = sensor_attr.index;
    struct w83795_data *data = w83795_update_device(dev);
    let mut temp: c_long = temp_from_reg(data.dts[index]);
    ...
    let mut nr: c_int = sensor_attr.nr;
    struct w83795_data *data = dev_get_drvdata(dev);
    let mut temp: c_long = temp_from_reg(data.dts_ext[nr]);
    return sprintf(buf, "%ld\n", temp);
    }
    static ssize_t
...
// calculate this value again by sensors as sensors3.conf
    ...
    let mut nr: c_int = sensor_attr.nr;
    struct w83795_data *data = w83795_update_pwm_config(dev);
    let mut val: u16 = data.setup_pwm[nr];
    switch (nr) {
...
//
    ...
    core::ptr::null_mut(), ALARM_STATUS, index + ((index > 14) ? 1 : 0)), \
...
//
    ...
    core::ptr::null_mut(), ALARM_STATUS, index + 31),			\
    ...
    show_pwm_mode, core::ptr::null_mut(), NOT_USED, index - 1),		\
...
//
    ...
    show_dts_mode, core::ptr::null_mut(), NOT_USED, index - 7),	\
    SENSOR_ATTR_2(temp##index##_input, S_IRUGO, show_dts,		\
    core::ptr::null_mut(), NOT_USED, index - 7),				\
    ...
    show_alarm_beep, core::ptr::null_mut(), ALARM_STATUS, index + 17),	\
...
//
    ...
    SENSOR_ATTR_2(temp##index##_alarm, S_IRUGO,			\
    show_alarm_beep, core::ptr::null_mut(), ALARM_STATUS,			\
    index + (index > 4 ? 11 : 17)),				\
    SENSOR_ATTR_2(temp##index##_beep, S_IWUSR | S_IRUGO,		\
    show_alarm_beep, store_beep, BEEP_ENABLE,		\
    index + (index > 4 ? 11 : 17)),				\
...
//
#[no_mangle]
unsafe extern "C" fn w83795_init_client(client: *mut i2c_client) {
...
#[no_mangle]
unsafe extern "C" fn w83795_get_device_id(client: *mut i2c_client) -> c_int {
...
pub const NUM_PWM_ATTRIBUTES: c_int = 4;
pub const NUM_TEMP_ATTRIBUTES: c_int = 8;
...
#[no_mangle]
unsafe extern "C" fn w83795_check_dynamic_in_limits(client: *mut i2c_client) {
...
#[no_mangle]
unsafe extern "C" fn w83795_probe(client: *mut i2c_client) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn w83795_remove(client: *mut i2c_client) {
