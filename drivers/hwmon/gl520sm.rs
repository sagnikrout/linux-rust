...
//! Source: drivers/hwmon/gl520sm.c
#![no_std]
...
//
// The GL520 registers
pub const GL520_REG_CHIP_ID: c_uint = 0x00;
pub const GL520_REG_REVISION: c_uint = 0x01;
pub const GL520_REG_CONF: c_uint = 0x03;
pub const GL520_REG_MASK: c_uint = 0x11;
pub const GL520_REG_VID_INPUT: c_uint = 0x02;
...
pub const GL520_REG_FAN_INPUT: c_uint = 0x07;
pub const GL520_REG_FAN_MIN: c_uint = 0x08;
pub const GL520_REG_FAN_DIV: c_uint = 0x0f;

pub const GL520_REG_ALARMS: c_uint = 0x12;
pub const GL520_REG_BEEP_MASK: c_uint = 0x10;

// Client data
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gl520_data {
    pub client: *mut i2c_client,
    pub groups: [*const attribute_group; 3],
    pub update_lock: mutex,
    pub /: *mut *mut bool valid; / false until the following fields are valid,
    pub /: *mut *mut unsigned long last_updated; / in jiffies,
    pub vid: u8,
    pub vrm: u8,
    pub /: *mut *mut u8 in_input[5]; / [0] = VVD,
    pub /: *mut *mut u8 in_min[5]; / [0] = VDD,
    pub /: *mut *mut u8 in_max[5]; / [0] = VDD,
    pub fan_input: [u8; 2],
    pub fan_min: [u8; 2],
    pub fan_div: [u8; 2],
    pub fan_off: u8,
...
//
#[no_mangle]
unsafe extern "C" fn gl520_read_value(client: *mut i2c_client, reg: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gl520_write_value(client: *mut i2c_client, reg: u8, value: u16) -> c_int {
...
//
    ...
    struct device_attribute *attr, char *buf)
    {
    let mut n: c_int = to_sensor_dev_attr(attr).index;
    struct gl520_data *data = gl520_update_device(dev);
    let mut r: u8 = data.in_input[n];
    if (n == 0)
    ...
    char *buf)
    {
    let mut bit_nr: c_int = to_sensor_dev_attr(attr).index;
    struct gl520_data *data = gl520_update_device(dev);
...
#[no_mangle]
unsafe extern "C" fn gl520_detect(client: *mut i2c_client, info: *mut i2c_board_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gl520_init_client(client: *mut i2c_client) {
    ...
    conf &= ~0x10;
#[no_mangle]
pub unsafe extern "C" fn if(2: extra_sensor_type ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn gl520_probe(client: *mut i2c_client) -> c_int {
