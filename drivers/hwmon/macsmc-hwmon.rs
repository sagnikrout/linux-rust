...
//! Source: drivers/hwmon/macsmc-hwmon.c
#![no_std]
...
pub const MAX_LABEL_LENGTH: c_int = 32;
// Temperature, voltage, current, power, fan(s)
pub const NUM_SENSOR_TYPES: c_int = 5;
pub const FLT_EXP_BIAS: c_int = 127;

pub const FLT_MANT_BIAS: c_int = 23;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct macsmc_hwmon_sensor {
    pub info: apple_smc_key_info,
    pub macsmc_key: smc_key,
    pub label: [c_char; MAX_LABEL_LENGTH],
    pub attrs: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct macsmc_hwmon_fan {
    pub now: macsmc_hwmon_sensor,
    pub min: macsmc_hwmon_sensor,
    pub max: macsmc_hwmon_sensor,
    pub set: macsmc_hwmon_sensor,
    pub mode: macsmc_hwmon_sensor,
    pub label: [c_char; MAX_LABEL_LENGTH],
    pub attrs: u32,
    pub manual: bool,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct macsmc_hwmon_sensors {
    pub channel_info: hwmon_channel_info,
    pub sensors: *mut macsmc_hwmon_sensor,
    pub count: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct macsmc_hwmon_fans {
    pub channel_info: hwmon_channel_info,
    pub fans: *mut macsmc_hwmon_fan,
    pub count: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct macsmc_hwmon {
    pub dev: *mut device,
    pub smc: *mut apple_smc,
    pub hwmon_dev: *mut device,
    pub chip_info: hwmon_chip_info,
// Chip + sensor types + NULL
    pub 1]: *const *const hwmon_channel_info channel_infos[1 + NUM_SENSOR_TYPES +,
    pub temp: macsmc_hwmon_sensors,
    pub volt: macsmc_hwmon_sensors,
    pub curr: macsmc_hwmon_sensors,
    pub power: macsmc_hwmon_sensors,
    pub fan: macsmc_hwmon_fans,
}
...
// We never have negatively scaled SMC floats
    ...
    val = U64_MAX;
#[no_mangle]
pub unsafe extern "C" fn if(-63: exp <) -> else {
    else if (exp < -63)
    val = 0;
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn macsmc_hwmon_write_f32(smc: *mut apple_smc, key: smc_key, value: c_long) -> c_int {
...
// Initialise fan control mode to automatic
    ...
    struct macsmc_hwmon_sensor *sensor;
    let mut n_current: u32 = 0, n_fan = 0, n_power = 0, n_temperature = 0, n_voltage = 0;
...
#[no_mangle]
unsafe extern "C" fn macsmc_hwmon_populate_configs(configs: *mut u32, sensors: *const macsmc_hwmon_sensors) {
...
#[no_mangle]
unsafe extern "C" fn macsmc_hwmon_populate_fan_configs(configs: *mut u32, fans: *const macsmc_hwmon_fans) {
...
#[no_mangle]
unsafe extern "C" fn macsmc_hwmon_create_infos(hwmon: *mut macsmc_hwmon) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn macsmc_hwmon_probe(pdev: *mut platform_device) -> c_int {
...
//
    ...
    "macsmc_hwmon", hwmon,
    &hwmon.chip_info, core::ptr::null_mut());
