...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hwmon_device {
    pub name: *const c_char,
    pub label: *const c_char,
    pub dev: device,
    pub chip: *const hwmon_chip_info,
    pub lock: mutex,
    pub tzdata: list_head,
    pub group: attribute_group,
    pub groups: *const attribute_group,
}

pub const MAX_SYSFS_ATTR_NAME_LENGTH: c_int = 32;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hwmon_device_attribute {
    pub dev_attr: device_attribute,
    pub ops: *const hwmon_ops,
    pub type: enum hwmon_sensor_types,
    pub attr: u32,
    pub index: c_int,
    pub name: [c_char; MAX_SYSFS_ATTR_NAME_LENGTH],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hwmon_thermal_data {
    pub /: *mut *mut list_head node; / hwmon tzdata list entry,
    pub /: *mut *mut *mut device dev; / Reference to hwmon device,
    pub /: *mut *mut int index; / sensor index,
    pub /: *mut *mut *mut thermal_zone_device tzd;/ thermal zone device,
}
    ...
    if (attr == &dev_attr_name.attr && hdev.name == core::ptr::null_mut())
    return 0;
    if (attr == &dev_attr_label.attr && hdev.label == core::ptr::null_mut())
    return 0;
    return attr.mode;
    }
...
#[no_mangle]
unsafe extern "C" fn hwmon_free_attrs(attrs: *const *const attribute) {
...
#[no_mangle]
unsafe extern "C" fn hwmon_dev_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn hwmon_thermal_get_temp(tz: *mut thermal_zone_device, temp: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hwmon_thermal_set_trips(tz: *mut thermal_zone_device, low: c_int, high: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hwmon_thermal_remove_sensor(data: *mut c_void) {
    static void hwmon_thermal_remove_sensor(void *data)
    {
    list_del(data);
    }
#[no_mangle]
unsafe extern "C" fn hwmon_thermal_add_sensor(dev: *mut device, index: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hwmon_thermal_register_sensors(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hwmon_thermal_notify(dev: *mut device, index: c_int) {
...
#[no_mangle]
unsafe extern "C" fn hwmon_attr_base(type: enum hwmon_sensor_types) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hwmon_match_device(dev: *mut device, data: *const c_void) -> c_int {
    ...
    if (err < 0)
    return err;
    hdev = device_find_child(dev, core::ptr::null_mut(), hwmon_match_device);
    if (!hdev)
    return -ENODEV;
//
...
#[no_mangle]
unsafe extern "C" fn hwmon_remove_pec(dev: *mut c_void) {
    static void hwmon_remove_pec(void *dev)
    {
    device_remove_file(dev, &dev_attr_pec);
    }
#[no_mangle]
unsafe extern "C" fn hwmon_pec_register(hdev: *mut device) -> c_int {
...
// sysfs attribute management
    ...
    const struct hwmon_device_attribute *hattr = to_hwmon_attr(devattr);
    struct hwmon_device *hwdev = to_hwmon_device(dev);
    let mut type: enum hwmon_sensor_types = hattr.type;
    const char *s;
    int ret;
    guard(mutex)(&hwdev.lock);
...
#[no_mangle]
unsafe extern "C" fn is_string_attr(type: enum hwmon_sensor_types, attr: u32) -> bool {
    ...
    umode_t mode;
    const char *name;
    let mut is_string: bool = is_string_attr(type, attr);
...
//
    ...
    sysfs_notify(&dev.kobj, core::ptr::null_mut(), sattr);
...
#[no_mangle]
pub unsafe extern "C" fn hwmon_lock(dev: *mut device) {
...
#[no_mangle]
pub unsafe extern "C" fn hwmon_unlock(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn hwmon_num_channel_attrs(info: *const hwmon_channel_info) -> c_int {
    ...
    let mut attr_mask: u32 = info.config[i];
    u32 attr;
    while (attr_mask) {
...
// Complain about invalid characters in hwmon name attribute
    ...
    if (chip) {
    const struct attribute **attrs;
    int ngroups = 2; /* terminating core::ptr::null_mut() plus &hwdev.groups */
    if (groups)
    for (i = 0; groups[i]; i++)
    ngroups++;
    ...
    if (hwdev.label == core::ptr::null_mut()) {
    err = -ENOMEM;
    goto free_hwmon;
    }
    }
    ...
    tdev = tdev.parent;
    hdev.of_node = tdev ? tdev.of_node : core::ptr::null_mut();
    hwdev.chip = chip;
    ...
    let mut config: u32 = chip.info[0].config[0];
...
//
    ...
    if (!name)
    return ERR_PTR(-EINVAL);
    return __hwmon_device_register(dev, name, drvdata, core::ptr::null_mut(), groups);
    }
...
//
    ...
    if (!name || !dev)
    return ERR_PTR(-EINVAL);
    return __hwmon_device_register(dev, name, drvdata, core::ptr::null_mut(), core::ptr::null_mut());
    }
...
//
    ...
    return __hwmon_device_register(dev, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn hwmon_device_unregister(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn devm_hwmon_release(dev: *mut device, res: *mut c_void) {
...
//
    ...
    if (!name)
    return ERR_PTR(-EINVAL);
    return __hwmon_sanitize_name(core::ptr::null_mut(), name);
    }
...
#[no_mangle]
unsafe extern "C" fn hwmon_pci_quirks() -> void __init {
...
// Open access to 0x295-0x296 on MSI MS-7031
    sb = pci_get_device(PCI_VENDOR_ID_ATI, 0x436c, core::ptr::null_mut());
    if (sb) {
...
#[no_mangle]
unsafe extern "C" fn hwmon_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn hwmon_exit() -> void __exit {
