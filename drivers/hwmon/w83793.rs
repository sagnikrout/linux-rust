...
//
pub const W83793_REG_BANKSEL: c_uint = 0x00;
pub const W83793_REG_VENDORID: c_uint = 0x0d;
pub const W83793_REG_CHIPID: c_uint = 0x0e;
pub const W83793_REG_DEVICEID: c_uint = 0x0f;
pub const W83793_REG_CONFIG: c_uint = 0x40;
pub const W83793_REG_MFC: c_uint = 0x58;
pub const W83793_REG_FANIN_CTRL: c_uint = 0x5c;
pub const W83793_REG_FANIN_SEL: c_uint = 0x5d;
pub const W83793_REG_I2C_ADDR: c_uint = 0x0b;
pub const W83793_REG_I2C_SUBADDR: c_uint = 0x0c;
pub const W83793_REG_VID_INA: c_uint = 0x05;
pub const W83793_REG_VID_INB: c_uint = 0x06;
pub const W83793_REG_VID_LATCHA: c_uint = 0x07;
pub const W83793_REG_VID_LATCHB: c_uint = 0x08;
pub const W83793_REG_VID_CTRL: c_uint = 0x59;
pub const W83793_REG_WDT_LOCK: c_uint = 0x01;
pub const W83793_REG_WDT_ENABLE: c_uint = 0x02;
pub const W83793_REG_WDT_STATUS: c_uint = 0x03;
pub const W83793_REG_WDT_TIMEOUT: c_uint = 0x04;
    static u16 W83793_REG_TEMP_MODE[2] = { 0x5e, 0x5f };
pub const TEMP_READ: c_int = 0;
pub const TEMP_CRIT: c_int = 1;
pub const TEMP_CRIT_HYST: c_int = 2;
pub const TEMP_WARN: c_int = 3;
pub const TEMP_WARN_HYST: c_int = 4;
//
...
pub const W83793_REG_TEMP_LOW_BITS: c_uint = 0x22;

pub const W83793_REG_CLR_CHASSIS: c_uint = 0x4a	/* SMI MASK4 */;
pub const W83793_REG_IRQ_CTRL: c_uint = 0x50;
pub const W83793_REG_OVT_CTRL: c_uint = 0x51;
pub const W83793_REG_OVT_BEEP: c_uint = 0x52;
pub const IN_READ: c_int = 0;
pub const IN_MAX: c_int = 1;
pub const IN_LOW: c_int = 2;
...
pub const W83793_REG_PWM_DEFAULT: c_uint = 0xb2;
pub const W83793_REG_PWM_ENABLE: c_uint = 0x207;
pub const W83793_REG_PWM_UPTIME: c_uint = 0xc3	/* Unit in 0.1 second */;
pub const W83793_REG_PWM_DOWNTIME: c_uint = 0xc4	/* Unit in 0.1 second */;
pub const W83793_REG_TEMP_CRITICAL: c_uint = 0xc5;
pub const PWM_DUTY: c_int = 0;
pub const PWM_START: c_int = 1;
pub const PWM_NONSTOP: c_int = 2;
pub const PWM_STOP_TIME: c_int = 3;

    (nr) == 1 ? 0x220 : 0x218) + (index))
// bit field, fan1 is bit0, fan2 is bit1 ...

#[no_mangle]
pub unsafe extern "C" fn FAN_FROM_REG(val: u16) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn FAN_TO_REG(rpm: c_long) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn TIME_FROM_REG(reg: u8) -> c_ulong {
    static inline unsigned long TIME_FROM_REG(u8 reg)
    {
    return reg * 100;
    }
#[no_mangle]
pub unsafe extern "C" fn TIME_TO_REG(val: c_ulong) -> u8 {
    static inline u8 TIME_TO_REG(unsigned long val)
    {
    return clamp_val((val + 50) / 100, 0, 0xff);
    }
#[no_mangle]
pub unsafe extern "C" fn TEMP_FROM_REG(reg: i8) -> c_long {
    static inline long TEMP_FROM_REG(s8 reg)
    {
    return reg * 1000;
    }
#[no_mangle]
pub unsafe extern "C" fn TEMP_TO_REG(val: c_long, min: i8, max: i8) -> i8 {
...
pub struct w83793_data {
    ...
    pub the: *mut *mut unsigned long last_nonvolatile; / In jiffies, last time we update,
// nonvolatile registers
//
    pub bank: u8,
    pub vrm: u8,
    pub vid: [u8; 2],
    pub /: *mut *mut u8 in[10][3]; / Register value, read/high/low,
    pub /: *mut *mut u8 in_low_bits[3]; / Additional resolution for VCore A/B Vtt,
    pub /: *mut *mut u16 has_fan; / Only fan1- fan5 has own pins,
    pub /: *mut *mut u16 fan[12]; / Register value combine,
    pub /: *mut *mut u16 fan_min[12]; / Register value combine,
    pub /: *mut *mut s8 temp[6][5]; / current, crit, crit_hyst,warn, warn_hyst,
    pub /: *mut *mut u8 temp_low_bits; / Additional resolution TD1-TD4,
    pub bits: *mut *mut u8 temp_mode[2]; / byte 0: Temp D1-D4 mode each has 2,
// byte 1: Temp R1,R2 mode, each has 1 bit
//
    pub /: *mut *mut u8 temp_critical; / If reached all fan will be at full speed,
    pub /: *mut *mut u8 temp_fan_map[6]; / Temp controls which pwm fan, bit field,
    pub has_pwm: u8,
    pub has_temp: u8,
    pub has_vid: u8,
    pub /: *mut *mut u8 pwm_enable; / Register value, each Temp has 1 bit,
    pub /: *mut *mut u8 pwm_uptime; / Register value,
    pub /: *mut *mut u8 pwm_downtime; / Register value,
    pub /: *mut *mut u8 pwm_default; / All fan default pwm, next poweron valid,
    pub /: *mut *mut u8 pwm[8][3]; / Register value,
    pub pwm_stop_time: [u8; 8],
    pub temp_cruise: [u8; 6],
    pub /: *mut *mut u8 alarms[5]; / realtime status registers,
    pub beeps: [u8; 5],
    pub beep_enable: u8,
    pub /: *mut *mut u8 tolerance[3]; / Temp tolerance(Smart Fan I/II),
    pub /: *mut *mut u8 sf2_pwm[6][7]; / Smart FanII: Fan duty cycle,
    pub /: *mut *mut u8 sf2_temp[6][7]; / Smart FanII: Temp level point,
// watchdog
    pub client: *mut i2c_client,
    pub watchdog_lock: mutex,
    pub /: *mut *mut list_head list; / member of the watchdog_data_list,
    pub kref: kref,
    pub watchdog_miscdev: miscdevice,
    pub watchdog_is_open: c_ulong,
    pub watchdog_expect_close: c_char,
    pub /: *mut *mut char watchdog_name[10]; / must be unique to avoid sysfs conflict,
    pub watchdog_caused_reboot: c_uint,
    pub /: *mut *mut int watchdog_timeout; / watchdog timeout in minutes,
}
...
//
#[no_mangle]
unsafe extern "C" fn w83793_release_resources(ref: *mut kref) {
...
pub const ALARM_STATUS: c_int = 0;
pub const BEEP_ENABLE: c_int = 1;
    static ssize_t
    ...
    let mut index: c_int = sensor_attr.index >> 3;
    let mut bit: c_int = sensor_attr.index & 0x07;
    u8 val;
    if (nr == ALARM_STATUS) {
    ...
    to_sensor_dev_attr_2(attr);
    let mut index: c_int = sensor_attr.index >> 3;
    let mut shift: c_int = sensor_attr.index & 0x07;
    let mut beep_bit: u8 = 1 << shift;
    unsigned long val;
    int err;
    err = kstrtoul(buf, 10, &val);
    if (err)
...
pub const FAN_INPUT: c_int = 0;
pub const FAN_MIN: c_int = 1;
    static ssize_t
    ...
    let mut index: c_int = sensor_attr.index;
    struct w83793_data *data = w83793_update_device(dev);
    let mut temp: c_long = TEMP_FROM_REG(data.temp[index][nr]);
    if (nr == TEMP_READ && index < 4) {	/* Only TD1-TD4 have low bits */
    let mut low: c_int = ((data.temp_low_bits >> (index * 2)) & 0x03) * 250;
    temp += temp > 0 ? low : -low;
    }
...
// 0 disable, 6 PECI
    ...
    to_sensor_dev_attr_2(attr);
    let mut index: c_int = sensor_attr.index;
    let mut mask: u8 = (index < 4) ? 0x03 : 0x01;
    let mut shift: u8 = (index < 4) ? (2 * index) : (index - 4);
    u8 tmp;
    index = (index < 4) ? 0 : 1;
...
pub const SETUP_PWM_DEFAULT: c_int = 0;

pub const SETUP_TEMP_CRITICAL: c_int = 3;
    static ssize_t
...
#[no_mangle]
pub unsafe extern "C" fn if(SETUP_PWM_UPTIME: nr ==) -> else {
    else if (nr == SETUP_PWM_UPTIME)
    val = TIME_FROM_REG(data.pwm_uptime);
#[no_mangle]
pub unsafe extern "C" fn if(SETUP_PWM_DOWNTIME: nr ==) -> else {
    else if (nr == SETUP_PWM_DOWNTIME)
    val = TIME_FROM_REG(data.pwm_downtime);
#[no_mangle]
pub unsafe extern "C" fn if(SETUP_TEMP_CRITICAL: nr ==) -> else {
...
//
pub const TEMP_FAN_MAP: c_int = 0;
pub const TEMP_PWM_ENABLE: c_int = 1;
pub const TEMP_CRUISE: c_int = 2;
pub const TEMP_TOLERANCE: c_int = 3;
    static ssize_t
...
// +2 to transform into 2 and 3 to conform with sysfs intf
    ...
    let mut shift: u8 = (index & 0x01) ? 4 : 0;
    data.tolerance[i] =
...
// only Vcore A/B and Vtt have additional 2 bits precision
    ...
    let mut index: c_int = sensor_attr.index;
    struct w83793_data *data = w83793_update_device(dev);
    let mut val: u16 = data.in[index][nr];
    if (index < 3) {
    val <<= 2;
...
// fix the limit values of 5VDD and 5VSB to ALARM mechanism
    ...
    SENSOR_ATTR_2(in##index##_input, S_IRUGO, show_in, core::ptr::null_mut(),	\
    IN_READ, index),					\
    ...
    core::ptr::null_mut(), ALARM_STATUS, index + ((index > 2) ? 1 : 0)),	\
    SENSOR_ATTR_2(in##index##_beep, S_IWUSR | S_IRUGO,		\
    show_alarm_beep, store_beep, BEEP_ENABLE,		\
    index + ((index > 2) ? 1 : 0))

    SENSOR_ATTR_2(fan##index##_alarm, S_IRUGO, show_alarm_beep,	\
    core::ptr::null_mut(), ALARM_STATUS, index + 17),			\
    SENSOR_ATTR_2(fan##index##_beep, S_IWUSR | S_IRUGO,		\
    show_alarm_beep, store_beep, BEEP_ENABLE, index + 17),	\
    SENSOR_ATTR_2(fan##index##_input, S_IRUGO, show_fan,		\
    core::ptr::null_mut(), FAN_INPUT, index - 1),				\
    ...
    core::ptr::null_mut(), TEMP_READ, index - 1),				\
    ...
    show_alarm_beep, core::ptr::null_mut(), ALARM_STATUS, index + 11),	\
...
// Pwm4-Pwm8
    ...
    SENSOR_ATTR_2(cpu0_vid, S_IRUGO, show_vid, core::ptr::null_mut(), NOT_USED, 0),
    SENSOR_ATTR_2(cpu1_vid, S_IRUGO, show_vid, core::ptr::null_mut(), NOT_USED, 1),
    };
...
#[no_mangle]
unsafe extern "C" fn w83793_init_client(client: *mut i2c_client) {
    static void w83793_init_client(struct i2c_client *client)
    {
    if (reset)
...
//
#[no_mangle]
unsafe extern "C" fn watchdog_set_timeout(data: *mut w83793_data, timeout: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_get_timeout(data: *mut w83793_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_trigger(data: *mut w83793_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_enable(data: *mut w83793_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_disable(data: *mut w83793_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_open(inode: *mut inode, filp: *mut file) -> c_int {
    static int watchdog_open(struct inode *inode, struct file *filp)
    {
    struct w83793_data *pos, *data = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn watchdog_close(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(WDIOS_ENABLECARD: val &) -> else {
...
//
    ...
    void *unused)
    {
    struct w83793_data *data = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn w83793_remove(client: *mut i2c_client) {
    static void w83793_remove(struct i2c_client *client)
    {
    struct w83793_data *data = i2c_get_clientdata(client);
...
// Return 0 if detection is successful, -ENODEV otherwise
    ...
    u8 tmp, bank, chip_id;
    struct i2c_adapter *adapter = client.adapter;
    let mut address: c_ushort = client.addr;
...
#[no_mangle]
unsafe extern "C" fn w83793_probe(client: *mut i2c_client) -> c_int {
    static int w83793_probe(struct i2c_client *client)
    {
    struct device *dev = &client.dev;
    ...
    int i, tmp, val, err;
    let mut files_fan: c_int = ARRAY_SIZE(w83793_left_fan) / 7;
    let mut files_pwm: c_int = ARRAY_SIZE(w83793_left_pwm) / 5;
    let mut files_temp: c_int = ARRAY_SIZE(w83793_temp) / 6;
...
#[no_mangle]
unsafe extern "C" fn w83793_update_nonvolatile(dev: *mut device) {
    static void w83793_update_nonvolatile(struct device *dev)
    {
    struct i2c_client *client = to_i2c_client(dev);
    struct w83793_data *data = i2c_get_clientdata(client);
...
//
#[no_mangle]
unsafe extern "C" fn w83793_read_value(client: *mut i2c_client, reg: u16) -> u8 {
    static u8 w83793_read_value(struct i2c_client *client, u16 reg)
    {
    struct w83793_data *data = i2c_get_clientdata(client);
    u8 res;
    let mut new_bank: u8 = reg >> 8;
...
// Must be called with data->update_lock held, except during initialization
#[no_mangle]
unsafe extern "C" fn w83793_write_value(client: *mut i2c_client, reg: u16, value: u8) -> c_int {
    static int w83793_write_value(struct i2c_client *client, u16 reg, u8 value)
    {
    struct w83793_data *data = i2c_get_clientdata(client);
    int res;
