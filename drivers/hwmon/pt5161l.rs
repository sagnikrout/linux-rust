...
//! Source: drivers/hwmon/pt5161l.c
#![no_std]
...
pub const ARIES_CURRENT_AVG_TEMP_ADC_CSR: c_uint = 0x42c;
// Device Load check register
pub const ARIES_CODE_LOAD_REG: c_uint = 0x605;
// Value indicating FW was loaded properly, [3:1] = 3'b111
pub const ARIES_LOAD_CODE: c_uint = 0xe;
// Main Micro Heartbeat register
pub const ARIES_MM_HEARTBEAT_ADDR: c_uint = 0x923;
// Reg offset to specify Address for MM assisted accesses
pub const ARIES_MM_ASSIST_REG_ADDR_OFFSET: c_uint = 0xd99;
// Reg offset to specify Command for MM assisted accesses
pub const ARIES_MM_ASSIST_CMD_OFFSET: c_uint = 0xd9d;
// Reg offset to MM SPARE 0 used specify Address[7:0]
pub const ARIES_MM_ASSIST_SPARE_0_OFFSET: c_uint = 0xd9f;
// Reg offset to MM SPARE 3 used specify Data Byte 0
pub const ARIES_MM_ASSIST_SPARE_3_OFFSET: c_uint = 0xda2;
// Wide register reads
pub const ARIES_MM_RD_WIDE_REG_2B: c_uint = 0x1d;
pub const ARIES_MM_RD_WIDE_REG_3B: c_uint = 0x1e;
pub const ARIES_MM_RD_WIDE_REG_4B: c_uint = 0x1f;
pub const ARIES_MM_RD_WIDE_REG_5B: c_uint = 0x20;
// Time delay between checking MM status of EEPROM write (microseconds)
pub const ARIES_MM_STATUS_TIME: c_int = 5000;
// AL Main SRAM DMEM offset (A0)

// SRAM read command
pub const AL_TG_RD_LOC_IND_SRAM: c_uint = 0x16;
// Offset for main micro FW info

// FW Info (Major) offset location in struct
pub const ARIES_MM_FW_VERSION_MAJOR: c_int = 0;
// FW Info (Minor) offset location in struct
pub const ARIES_MM_FW_VERSION_MINOR: c_int = 1;
// FW Info (Build no.) offset location in struct
pub const ARIES_MM_FW_VERSION_BUILD: c_int = 2;
pub const ARIES_TEMP_CAL_CODE_DEFAULT: c_int = 84;
// Struct defining FW version loaded on an Aries device
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pt5161l_fw_ver {
    pub major: u8,
    pub minor: u8,
    pub build: u16,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pt5161l_data {
    pub client: *mut i2c_client,
    pub fw_ver: pt5161l_fw_ver,
    pub /: *mut *mut mutex lock; / for atomic I2C transactions,
    pub init_done: bool,
    pub /: *mut *mut bool code_load_okay; / indicate if code load reg value is expected,
    pub /: *mut *mut bool mm_heartbeat_okay; / indicate if Main Micro heartbeat is good,
    pub /: *mut *mut bool mm_wide_reg_access; / MM assisted wide register access,
}
...
//
    ...
    u8 buf[8];
    u8 i, status;
    let mut uind_offs: u32 = ARIES_MM_ASSIST_REG_ADDR_OFFSET;
...
//
#[no_mangle]
unsafe extern "C" fn pt5161l_fw_load_check(data: *mut pt5161l_data) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pt5161l_heartbeat_check(data: *mut pt5161l_data) -> c_int {
    static int pt5161l_heartbeat_check(struct pt5161l_data *data)
    {
    int ret, tries;
    u8 buf[8];
    u8 heartbeat;
    let mut hb_changed: bool = false;
...
//
#[no_mangle]
unsafe extern "C" fn pt5161l_fwsts_check(data: *mut pt5161l_data) -> c_int {
    static int pt5161l_fwsts_check(struct pt5161l_data *data)
    {
    int ret;
    u8 buf[8];
    let mut major: u8 = 0, minor = 0;
    let mut build: u16 = 0;
    ...
    u16 build)
    {
    let mut ver: u32 = major << 24 | minor << 16 | build;
...
#[no_mangle]
unsafe extern "C" fn pt5161l_init_dev(data: *mut pt5161l_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pt5161l_init_debugfs(client: *mut i2c_client, data: *mut pt5161l_data) {
...
#[no_mangle]
unsafe extern "C" fn pt5161l_probe(client: *mut i2c_client) -> c_int {
