...
pub struct adcxx {
    ...
    pub lock: mutex,
    pub channels: u32,
    pub /: *mut *mut u32 reference; / in millivolts,
}
...
// ----------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn adcxx_probe(spi: *mut spi_device) -> c_int {
    static int adcxx_probe(struct spi_device *spi)
    {
    let mut channels: c_int = spi_get_device_id(spi).driver_data;
    struct adcxx *adc;
    int status;
    int i;
...
#[no_mangle]
unsafe extern "C" fn adcxx_remove(spi: *mut spi_device) {
