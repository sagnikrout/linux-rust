...
//! Source: drivers/hwmon/arctic_fan_controller.c
#![no_std]
...
pub const ARCTIC_VID: c_uint = 0x3904;
pub const ARCTIC_PID: c_uint = 0xF001;
pub const ARCTIC_NUM_FANS: c_int = 10;
pub const ARCTIC_OUTPUT_REPORT_ID: c_uint = 0x01;
pub const ARCTIC_REPORT_LEN: c_int = 32;

// ACK report: device sends Report ID 0x02, 2 bytes (ID + status) after applying OUT report
pub const ARCTIC_ACK_REPORT_ID: c_uint = 0x02;
pub const ARCTIC_ACK_REPORT_LEN: c_int = 2;
//
// Time to wait for ACK report after send.
// Measured over 500 iterations: max ~563 ms. Keep 1 s as margin.
//
pub const ARCTIC_ACK_TIMEOUT_MS: c_int = 1000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct arctic_fan_data {
    pub hdev: *mut hid_device,
    pub /: *mut *mut *mut device hwmon_dev; / stored for explicit unregister in remove(),
    pub /: *mut *mut spinlock_t in_report_lock; / protects fan_rpm, ack_status, write_pending, pwm_duty,
    pub /: *mut *mut completion in_report_received; / ACK (ID 0x02) received in raw_event,
    pub /: *mut *mut int ack_status; / 0 = OK, negative errno on device error,
    pub /: *mut *mut bool write_pending; / true while an OUT report ACK is in flight,
    pub fan_rpm: [u32; ARCTIC_NUM_FANS],
    pub /: *mut *mut u8 pwm_duty[ARCTIC_NUM_FANS]; / 0-255 matching sysfs range; converted to 0-100 on send,
//
...
// val = priv->pwm_duty[channel];
    ...
    {
    struct arctic_fan_data *priv = dev_get_drvdata(dev);
    let mut new_duty: u8 = (u8)clamp_val(val, 0, 255);
    unsigned long flags;
    unsigned long t;
    int i, ret;
//
...
//
    ...
    for (i = 0; i < ARCTIC_NUM_FANS; i++) {
    let mut d: u8 = i == channel ? new_duty : priv.pwm_duty[i];
...
#[no_mangle]
unsafe extern "C" fn arctic_fan_reset_resume(hdev: *mut hid_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn arctic_fan_remove(hdev: *mut hid_device) {
