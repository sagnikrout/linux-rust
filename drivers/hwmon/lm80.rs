...
pub const LM80_REG_FAN1: c_uint = 0x28;
pub const LM80_REG_FAN2: c_uint = 0x29;

pub const LM80_REG_TEMP: c_uint = 0x27;
pub const LM80_REG_TEMP_HOT_MAX: c_uint = 0x38;
pub const LM80_REG_TEMP_HOT_HYST: c_uint = 0x39;
pub const LM80_REG_TEMP_OS_MAX: c_uint = 0x3a;
pub const LM80_REG_TEMP_OS_HYST: c_uint = 0x3b;
pub const LM80_REG_CONFIG: c_uint = 0x00;
pub const LM80_REG_ALARM1: c_uint = 0x01;
pub const LM80_REG_ALARM2: c_uint = 0x02;
pub const LM80_REG_MASK1: c_uint = 0x03;
pub const LM80_REG_MASK2: c_uint = 0x04;
pub const LM80_REG_FANDIV: c_uint = 0x05;
pub const LM80_REG_RES: c_uint = 0x06;
pub const LM96080_REG_CONV_RATE: c_uint = 0x07;
pub const LM96080_REG_MAN_ID: c_uint = 0x3e;
pub const LM96080_REG_DEV_ID: c_uint = 0x3f;
//
...
#[no_mangle]
pub unsafe extern "C" fn FAN_TO_REG(rpm: unsigned, div: unsigned) -> c_uchar {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lm80_data {
    pub client: *mut i2c_client,
    pub update_lock: mutex,
    pub /: *mut *mut char error; / !=0 if error occurred during last update,
    pub /: *mut *mut bool valid; / true if following fields are valid,
    pub /: *mut *mut unsigned long last_updated; / In jiffies,
    pub /: *mut *mut u8 in[i_num_in][7]; / Register value, 1st index is enum in_index,
    pub /: *mut *mut u8 fan[f_num_fan][2]; / Register value, 1st index enum fan_index,
    pub /: *mut *mut u8 fan_div[2]; / Register encoding, shifted right,
    pub /: *mut *mut s16 temp[t_num_temp]; / Register values, normalized to 16 bit,
    pub /: *mut *mut u16 alarms; / Register encoding, combined,
}

#[no_mangle]
unsafe extern "C" fn lm80_read_value(client: *mut i2c_client, reg: u8) -> c_int {
    static int lm80_read_value(struct i2c_client *client, u8 reg)
    {
    return i2c_smbus_read_byte_data(client, reg);
    }
#[no_mangle]
unsafe extern "C" fn lm80_write_value(client: *mut i2c_client, reg: u8, value: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lm80_init_client(client: *mut i2c_client) {
...
//
    ...
    long val;
    u8 reg;
    let mut err: c_int = kstrtol(buf, 10, &val);
    if (err < 0)
    return err;
    ...
    struct i2c_client *client = data.client;
    unsigned long val;
    let mut err: c_int = kstrtoul(buf, 10, &val);
    if (err < 0)
    return err;
    mutex_lock(&data.update_lock);
...
#[no_mangle]
unsafe extern "C" fn lm80_detect(client: *mut i2c_client, info: *mut i2c_board_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lm80_probe(client: *mut i2c_client) -> c_int {
