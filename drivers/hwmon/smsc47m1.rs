...
//! Source: drivers/hwmon/smsc47m1.c
#![no_std]
...
pub const SUPERIO_REG_ACT: c_uint = 0x30;
pub const SUPERIO_REG_BASE: c_uint = 0x60;
pub const SUPERIO_REG_DEVID: c_uint = 0x20;
pub const SUPERIO_REG_DEVREV: c_uint = 0x21;
// Logical device registers
pub const SMSC_EXTENT: c_uint = 0x80;
// nr is 0 or 1 in the macros below
pub const SMSC47M1_REG_ALARM: c_uint = 0x04;

pub const SMSC47M1_REG_FANDIV: c_uint = 0x58;
...
pub const SMSC47M2_REG_ALARM6: c_uint = 0x09;
pub const SMSC47M2_REG_TPIN1: c_uint = 0x38;
pub const SMSC47M2_REG_TPIN2: c_uint = 0x37;
pub const SMSC47M2_REG_TPIN3: c_uint = 0x2d;
pub const SMSC47M2_REG_PPIN3: c_uint = 0x2c;
pub const SMSC47M2_REG_FANDIV3: c_uint = 0x6a;
...
pub struct smsc47m1_data {
    ...
    pub /: *mut *mut u8 fan_preload[3]; / Register value,
    pub /: *mut *mut u8 fan_div[3]; / Register encoding, shifted right,
    pub /: *mut *mut u8 alarms; / Register encoding,
    pub /: *mut *mut u8 pwm[3]; / Register value (bit 0 is disable),
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct smsc47m1_sio_data {
    pub type: enum chips,
    pub /: *mut *mut u8 activate; / Remember initial device state,
}

#[no_mangle]
pub unsafe extern "C" fn smsc47m1_read_value(data: *mut smsc47m1_data, reg: u8) -> c_int {
...
//
    ...
    struct device_attribute *devattr, char *buf)
    {
    let mut bitnr: c_int = to_sensor_dev_attr(devattr).index;
    struct smsc47m1_data *data = smsc47m1_update_device(dev, 0);
...
//
    ...
    int err;
    long tmp;
    let mut old_div: u8 = DIV_FROM_REG(data.fan_div[nr]);
...
#[no_mangle]
unsafe extern "C" fn smsc47m1_find(sio_data: *mut smsc47m1_sio_data) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn smsc47m1_restore(sio_data: *const smsc47m1_sio_data) {
...
pub const CHECK: c_int = 1;
pub const REQUEST: c_int = 2;
//
...
// register, region length
    ...
    let mut start: c_ushort = address + ports[i];
    let mut len: c_ushort = ports[i + 1];
    switch (action) {
    case CHECK:
...
#[no_mangle]
unsafe extern "C" fn smsc47m1_remove_files(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn smsc47m1_probe(pdev: *mut platform_device) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn smsc47m1_remove(pdev: *mut platform_device) -> void __exit {
...
//
    ...
    int err;
    err = smsc47m1_handle_resources(address, sio_data.type, CHECK, core::ptr::null_mut());
    if (err)
    return err;
...
#[no_mangle]
unsafe extern "C" fn sm_smsc47m1_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn sm_smsc47m1_exit() -> void __exit {
