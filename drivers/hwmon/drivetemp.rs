...
//! Source: drivers/hwmon/drivetemp.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drivetemp_data {
    pub /: *mut *mut list_head list; / list of instantiated devices,
    pub /: *mut *mut *mut scsi_device sdev; / SCSI device,
    pub /: *mut *mut *mut device dev; / instantiating device,
    pub /: *mut *mut *mut device hwdev; / hardware monitoring device,
    pub /: *mut *mut u8 smartdata[ATA_SECT_SIZE]; / local buffer,
    pub val): *mut *mut *mut int (get_temp)(struct drivetemp_data st, u32 attr, long,
    pub /: *mut *mut bool have_temp_lowest; / lowest temp in SCT status,
    pub /: *mut *mut bool have_temp_highest; / highest temp in SCT status,
    pub /: *mut *mut bool have_temp_min; / have min temp,
    pub /: *mut *mut bool have_temp_max; / have max temp,
    pub /: *mut *mut bool have_temp_lcrit; / have lower critical limit,
    pub /: *mut *mut bool have_temp_crit; / have critical limit,
    pub /: *mut *mut int temp_min; / min temp,
    pub /: *mut *mut int temp_max; / max temp,
    pub /: *mut *mut int temp_lcrit; / lower critical limit,
    pub /: *mut *mut int temp_crit; / critical limit,
}

    static LIST_HEAD(drivetemp_devlist);
pub const ATA_MAX_SMART_ATTRS: c_int = 30;
pub const SMART_TEMP_PROP_190: c_int = 190;
pub const SMART_TEMP_PROP_194: c_int = 194;
pub const SCT_STATUS_REQ_ADDR: c_uint = 0xe0;

pub const SCT_STATUS_VERSION_HIGH: c_int = 1;
pub const SCT_STATUS_TEMP: c_int = 200;
pub const SCT_STATUS_TEMP_LOWEST: c_int = 201;
pub const SCT_STATUS_TEMP_HIGHEST: c_int = 202;
pub const SCT_READ_LOG_ADDR: c_uint = 0xe1;
pub const SMART_READ_LOG: c_uint = 0xd5;
pub const SMART_WRITE_LOG: c_uint = 0xd6;
pub const INVALID_TEMP: c_uint = 0x80;

#[no_mangle]
pub unsafe extern "C" fn ata_id_smart_supported(id: *mut u16) -> bool {
    static inline bool ata_id_smart_supported(u16 *id)
    {
    return id[ATA_ID_COMMAND_SET_1] & BIT(0);
    }
#[no_mangle]
pub unsafe extern "C" fn ata_id_smart_enabled(id: *mut u16) -> bool {
...
//
    ...
    ATA_SECT_SIZE, 10 * HZ, 5, core::ptr::null_mut());
    if (err > 0)
    err = -EIO;
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn drivetemp_get_scttemp(st: *mut drivetemp_data, attr: u32, val: *mut c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drivetemp_sct_avoid(st: *mut drivetemp_data) -> bool {
...
#[no_mangle]
unsafe extern "C" fn drivetemp_identify_sata(st: *mut drivetemp_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drivetemp_identify(st: *mut drivetemp_data) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn drivetemp_add(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drivetemp_remove(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn drivetemp_init() -> int __init {
    static int __init drivetemp_init(void)
    {
    return scsi_register_interface(&drivetemp_interface);
    }
#[no_mangle]
unsafe extern "C" fn drivetemp_exit() -> void __exit {
