...
//! Source: drivers/hwmon/asus_rog_ryujin.c
#![no_std]
...
pub const USB_VENDOR_ID_ASUS_ROG: c_uint = 0x0b05;
pub const USB_PRODUCT_ID_RYUJIN_AIO: c_uint = 0x1988	/* ASUS ROG RYUJIN II 360 */;
pub const USB_PRODUCT_ID_RYUJIN_III_EXTREME: c_uint = 0x1bcb;
pub const USB_PRODUCT_ID_RYUJIN_III_EVA: c_uint = 0x1ade;
pub const USB_PRODUCT_ID_RYUJIN_III_WHITE: c_uint = 0x1ada;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rog_ryujin_device_info {
    pub temp_offset: u8,
    pub pump_speed_offset: u8,
    pub fan_speed_offset: u8,
    pub duty_channel: u8,
...
pub const MAX_REPORT_LENGTH: c_int = 65;
// Cooler duty report offsets
pub const RYUJIN_PUMP_DUTY: c_int = 4;
pub const RYUJIN_INTERNAL_FAN_DUTY: c_int = 5;
// Controller status (speeds) report offsets
pub const RYUJIN_CONTROLLER_SPEED_1: c_int = 5;
pub const RYUJIN_CONTROLLER_SPEED_2: c_int = 7;
pub const RYUJIN_CONTROLLER_SPEED_3: c_int = 9;
pub const RYUJIN_CONTROLLER_SPEED_4: c_int = 3;
// Controller duty report offsets
pub const RYUJIN_CONTROLLER_DUTY: c_int = 4;
// Control commands and their inner offsets
pub const RYUJIN_CMD_PREFIX: c_uint = 0xEC;
...
pub const RYUJIN_SET_COOLER_PUMP_DUTY_OFFSET: c_int = 3;
pub const RYUJIN_SET_COOLER_FAN_DUTY_OFFSET: c_int = 4;
    static const u8 set_cooler_duty_cmd[] = { RYUJIN_CMD_PREFIX, 0x1A, 0x00, 0x00, 0x00 };
pub const RYUJIN_SET_CONTROLLER_FAN_DUTY_OFFSET: c_int = 4;
    static const u8 set_controller_duty_cmd[] = { RYUJIN_CMD_PREFIX, 0x21, 0x00, 0x00, 0x00 };
// Command lengths

// Command response headers
pub const RYUJIN_GET_COOLER_STATUS_CMD_RESPONSE: c_uint = 0x19;
pub const RYUJIN_GET_COOLER_DUTY_CMD_RESPONSE: c_uint = 0x1A;
pub const RYUJIN_GET_CONTROLLER_SPEED_CMD_RESPONSE: c_uint = 0x20;
pub const RYUJIN_GET_CONTROLLER_DUTY_CMD_RESPONSE: c_uint = 0x21;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rog_ryujin_data {
    pub hdev: *mut hid_device,
    pub hwmon_dev: *mut device,
    pub info: *const rog_ryujin_device_info,
// For reinitializing the completions below
    pub status_report_request_lock: spinlock_t,
    pub cooler_status_received: completion,
    pub controller_status_received: completion,
    pub cooler_duty_received: completion,
    pub controller_duty_received: completion,
    pub cooler_duty_set: completion,
    pub controller_duty_set: completion,
// Sensor data
    pub temp_input: [i32; 1],
    pub /: *mut *mut u16 speed_input[6]; / Pump, internal fan and four controller fan speeds in RPM,
    pub /: *mut *mut u8 duty_input[3]; / Pump, internal fan and controller fan duty in PWM,
    pub buffer: *mut u8,
    pub /: *mut *mut unsigned long updated; / jiffies,
}

#[no_mangle]
unsafe extern "C" fn rog_ryujin_percent_to_pwm(val: u16) -> c_int {
    static int rog_ryujin_percent_to_pwm(u16 val)
    {
    return DIV_ROUND_CLOSEST(val * 255, 100);
    }
#[no_mangle]
unsafe extern "C" fn rog_ryujin_pwm_to_percent(val: c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rog_ryujin_write_expanded(priv: *mut rog_ryujin_data, cmd: *const u8, cmd_length: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rog_ryujin_get_status(priv: *mut rog_ryujin_data) -> c_int {
...
// Retrieve controller duty
    ...
    let mut ret: c_int = rog_ryujin_get_status(priv);
    if (ret < 0)
    return ret;
    switch (type) {
    case hwmon_temp:
...
#[no_mangle]
unsafe extern "C" fn rog_ryujin_write_fixed_duty(priv: *mut rog_ryujin_data, channel: c_int, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !completion_done(&priv->cooler_duty_received)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !completion_done(&priv->controller_duty_received)) -> else {
...
#[no_mangle]
unsafe extern "C" fn rog_ryujin_probe(hdev: *mut hid_device, id: *const hid_device_id) -> c_int {
...
// Enable hidraw so existing user-space tools can continue to work
    ...
    priv, &rog_ryujin_chip_info, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn rog_ryujin_remove(hdev: *mut hid_device) {
...
#[no_mangle]
unsafe extern "C" fn rog_ryujin_init() -> int __init {
    static int __init rog_ryujin_init(void)
    {
    return hid_register_driver(&rog_ryujin_driver);
    }
#[no_mangle]
unsafe extern "C" fn rog_ryujin_exit() -> void __exit {
