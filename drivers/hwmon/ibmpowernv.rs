...
//! Source: drivers/hwmon/ibmpowernv.c
#![no_std]
...
pub const MAX_ATTR_LEN: c_int = 32;
pub const MAX_LABEL_LEN: c_int = 64;
...
pub struct sensor_data {
    pub /: *mut *mut u32 id; / An opaque id of the firmware for each sensor,
    pub hwmon_index: u32,
    pub opal_index: u32,
    pub type: enum sensors,
    pub label: [c_char; MAX_LABEL_LEN],
    pub name: [c_char; MAX_ATTR_LEN],
    pub dev_attr: device_attribute,
    pub sgrp_data: *mut sensor_group_data,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sensor_group_data {
    pub mutex: mutex,
    pub gid: u32,
    pub enable: bool,
}
...
pub struct platform_data {
    pub 1]: *const *const attribute_group attr_groups[MAX_SENSOR_TYPE +,
    pub sgrp_data: *mut sensor_group_data,
    pub /: *mut *mut u32 sensors_count; / Total count of sensors from each group,
    pub /: *mut *mut u32 nr_sensor_groups; / Total number of sensor groups,
}
...
#[no_mangle]
pub unsafe extern "C" fn if(POWER_INPUT: sdata->type ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn get_logical_cpu(hwcpu: c_int) -> c_int {
...
//
    ...
    let mut cpuid: c_int = get_logical_cpu(id);
    if (cpuid >= 0)
//
...
#[no_mangle]
unsafe extern "C" fn get_sensor_index_attr(name: *const c_char, index: *mut u32, attr: *mut c_char) -> c_int {
    ...
    {
    const char *attr_name = core::ptr::null_mut();
    ...
    attr_name = "max";
#[no_mangle]
pub unsafe extern "C" fn if(FAN: type ==) -> else {
    else if (type == FAN)
    attr_name = "min";
    }
    return attr_name;
...
#[no_mangle]
unsafe extern "C" fn get_sensor_type(np: *mut device_node) -> c_int {
...
//
    ...
    let mut count: c_int = 0, ret = 0;
    enum sensors type;
    groups = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "ibm,opal-sensor-group");
    if (!groups)
    return ret;
    ...
    continue;
    if (of_count_phandle_with_args(sgrp, "sensors", core::ptr::null_mut()) <= 0)
    continue;
    ...
    if (of_property_read_u32(sgrp, "sensor-group-id", &gid))
    continue;
    of_for_each_phandle(&it, rc, sgrp, "sensors", core::ptr::null_mut(), 0)
...
#[no_mangle]
unsafe extern "C" fn populate_attr_groups(pdev: *mut platform_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn create_device_attrs(pdev: *mut platform_device) -> c_int {
...
//
    ...
    core::ptr::null_mut(), show_label, core::ptr::null_mut());
    count++;
    }
...
#[no_mangle]
unsafe extern "C" fn ibmpowernv_probe(pdev: *mut platform_device) -> c_int {
