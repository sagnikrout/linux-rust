...
//! Source: drivers/hwmon/axi-fan-control.c
#![no_std]
...
// register map
pub const ADI_REG_RSTN: c_uint = 0x0080;
pub const ADI_REG_PWM_WIDTH: c_uint = 0x0084;
pub const ADI_REG_TACH_PERIOD: c_uint = 0x0088;
pub const ADI_REG_TACH_TOLERANCE: c_uint = 0x008c;
pub const ADI_REG_PWM_PERIOD: c_uint = 0x00c0;
pub const ADI_REG_TACH_MEASUR: c_uint = 0x00c4;
pub const ADI_REG_TEMPERATURE: c_uint = 0x00c8;
pub const ADI_REG_TEMP_00_H: c_uint = 0x0100;
pub const ADI_REG_TEMP_25_L: c_uint = 0x0104;
pub const ADI_REG_TEMP_25_H: c_uint = 0x0108;
pub const ADI_REG_TEMP_50_L: c_uint = 0x010c;
pub const ADI_REG_TEMP_50_H: c_uint = 0x0110;
pub const ADI_REG_TEMP_75_L: c_uint = 0x0114;
pub const ADI_REG_TEMP_75_H: c_uint = 0x0118;
pub const ADI_REG_TEMP_100_L: c_uint = 0x011c;
pub const ADI_REG_IRQ_MASK: c_uint = 0x0040;
pub const ADI_REG_IRQ_PENDING: c_uint = 0x0044;
pub const ADI_REG_IRQ_SRC: c_uint = 0x0048;
// IRQ sources

pub const ADI_IRQ_MASK_OUT_ALL: c_uint = 0xFFFFFFFFU;
pub const SYSFS_PWM_MAX: c_int = 255;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct axi_fan_control_data {
...
//
#[no_mangle]
unsafe extern "C" fn axi_fan_control_show(dev: *mut device, da: *mut device_attribute, buf: *mut c_char) -> isize {
    static ssize_t axi_fan_control_show(struct device *dev, struct device_attribute *da, char *buf)
    {
    struct axi_fan_control_data *ctl = dev_get_drvdata(dev);
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    let mut temp: u32 = axi_ioread(attr.index, ctl);
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_get_pwm_duty(ctl: *const axi_fan_control_data) -> c_long {
    static long axi_fan_control_get_pwm_duty(const struct axi_fan_control_data *ctl)
    {
    let mut pwm_width: u32 = axi_ioread(ADI_REG_PWM_WIDTH, ctl);
    let mut pwm_period: u32 = axi_ioread(ADI_REG_PWM_PERIOD, ctl);
//
...
//
    ...
    let mut pwm_period: u32 = axi_ioread(ADI_REG_PWM_PERIOD, ctl);
    u32 new_width;
    let mut __val: c_long = clamp_val(val, 0, SYSFS_PWM_MAX);
    new_width = DIV_ROUND_CLOSEST(__val * pwm_period, SYSFS_PWM_MAX);
    axi_iowrite(new_width, ADI_REG_PWM_WIDTH, ctl);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn axi_fan_control_get_fan_rpm(ctl: *const axi_fan_control_data) -> c_long {
    static long axi_fan_control_get_fan_rpm(const struct axi_fan_control_data *ctl)
    {
    let mut tach: u32 = axi_ioread(ADI_REG_TACH_MEASUR, ctl);
    if (tach == 0)
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_read_temp(dev: *mut device, attr: u32, val: *mut c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_read_fan(dev: *mut device, attr: u32, val: *mut c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_read_pwm(dev: *mut device, attr: u32, val: *mut c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_write_pwm(dev: *mut device, attr: u32, val: c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_fan_is_visible(attr: u32) -> umode_t {
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_pwm_is_visible(attr: u32) -> umode_t {
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_temp_is_visible(attr: u32) -> umode_t {
...
//
#[no_mangle]
unsafe extern "C" fn axi_fan_control_irq_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
    ...
    let mut irq_pending: u32 = axi_ioread(ADI_REG_IRQ_PENDING, ctl);
    u32 clear_mask;
...
//
    ...
    let mut new_tach: u32 = axi_ioread(ADI_REG_TACH_MEASUR, ctl);
// get 25% tolerance
    let mut tach_tol: u32 = DIV_ROUND_CLOSEST(new_tach * 25, 100);
...
#[no_mangle]
unsafe extern "C" fn axi_fan_control_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (ctl.irq < 0)
    return ctl.irq;
    ret = devm_request_threaded_irq(&pdev.dev, ctl.irq, core::ptr::null_mut(),
    axi_fan_control_irq_handler,
    IRQF_ONESHOT | IRQF_TRIGGER_HIGH,
    core::ptr::null_mut(), ctl);
    if (ret)
    return ret;
    return 0;
    }
    let mut version_1_0_0: static u32 = ADI_AXI_PCORE_VER(1, 0, 'a');
