...
//! Source: drivers/hwmon/asc7621.c
#![no_std]
...
pub const PRI_NONE: c_int = 0;
pub const PRI_LOW: c_int = 1;
pub const PRI_HIGH: c_int = 2;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct asc7621_chip {
    pub name: *mut c_char,
    pub chip_type: enum asc7621_type,
    pub company_reg: u8,
    pub company_id: u8,
    pub verstep_reg: u8,
    pub verstep_id: u8,
    pub addresses: *const c_ushort,
}
...
//
pub const LAST_REGISTER: c_uint = 0xff;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct asc7621_data {
    ...
    pub /: *mut *mut unsigned long last_high_reading; / In jiffies,
    pub /: *mut *mut unsigned long last_low_reading; / In jiffies,
//
// Registers we care about occupy the corresponding index
// in the array.  Registers we don't care about are left
// at 0.
//
    pub 1]: u8 reg[LAST_REGISTER +,
}
...
#[no_mangle]
pub unsafe extern "C" fn read_byte(client: *mut i2c_client, reg: u8) -> u8 {
    static inline u8 read_byte(struct i2c_client *client, u8 reg)
    {
    let mut res: c_int = i2c_smbus_read_byte_data(client, reg);
    if (res < 0) {
    dev_err(&client.dev,
...
#[no_mangle]
pub unsafe extern "C" fn write_byte(client: *mut i2c_client, reg: u8, data: u8) -> c_int {
    static inline int write_byte(struct i2c_client *client, u8 reg, u8 data)
    {
    let mut res: c_int = i2c_smbus_write_byte_data(client, reg, data);
    if (res < 0) {
    dev_err(&client.dev,
...
// mmmmmm.ll
    ...
    {
    SETUP_SHOW_DATA_PARAM(dev, attr);
    let mut regval: u8 = data.reg[param.msb[0]];
    let mut temp: c_int = ((s8) (regval & 0xfc) * 1000) + ((regval & 0x03) * 250);
    return sprintf(buf, "%d\n", temp);
    }
...
pub unsafe extern "C" fn if(7: val ==) -> else {
    ...
    newval = 1;
#[no_mangle]
pub unsafe extern "C" fn if(1: minoff ==) -> else {
    else if (minoff == 1)
    newval = 2;
    else
    newval = 3;
...
//
    ...
    {.sda = SENSOR_ATTR(name, S_IRUGO, show_##r, core::ptr::null_mut(), n), \
...
//
#[no_mangle]
pub unsafe extern "C" fn valid_address_for_chip(chip_type: c_int, address: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn asc7621_init_client(client: *mut i2c_client) {
...
#[no_mangle]
unsafe extern "C" fn asc7621_remove(client: *mut i2c_client) {
...
#[no_mangle]
unsafe extern "C" fn sm_asc7621_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn sm_asc7621_exit() -> void __exit {
