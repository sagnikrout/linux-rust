...
//! Source: drivers/hwmon/ftsteutates.c
#![no_std]
...
pub const FTS_DEVICE_ID_REG: c_uint = 0x0000;
pub const FTS_DEVICE_REVISION_REG: c_uint = 0x0001;
pub const FTS_DEVICE_STATUS_REG: c_uint = 0x0004;
pub const FTS_SATELLITE_STATUS_REG: c_uint = 0x0005;
pub const FTS_EVENT_STATUS_REG: c_uint = 0x0006;
pub const FTS_GLOBAL_CONTROL_REG: c_uint = 0x0007;
pub const FTS_DEVICE_DETECT_REG_1: c_uint = 0x0C;
pub const FTS_DEVICE_DETECT_REG_2: c_uint = 0x0D;
pub const FTS_DEVICE_DETECT_REG_3: c_uint = 0x0E;
pub const FTS_SENSOR_EVENT_REG: c_uint = 0x0010;
pub const FTS_FAN_EVENT_REG: c_uint = 0x0014;
pub const FTS_FAN_PRESENT_REG: c_uint = 0x0015;
pub const FTS_POWER_ON_TIME_COUNTER_A: c_uint = 0x007A;
pub const FTS_POWER_ON_TIME_COUNTER_B: c_uint = 0x007B;
pub const FTS_POWER_ON_TIME_COUNTER_C: c_uint = 0x007C;
pub const FTS_PAGE_SELECT_REG: c_uint = 0x007F;
pub const FTS_WATCHDOG_TIME_PRESET: c_uint = 0x000B;
pub const FTS_WATCHDOG_CONTROL: c_uint = 0x5081;
pub const FTS_NO_FAN_SENSORS: c_uint = 0x08;
pub const FTS_NO_TEMP_SENSORS: c_uint = 0x10;
pub const FTS_NO_VOLT_SENSORS: c_uint = 0x04;
pub const FTS_FAN_SOURCE_INVALID: c_uint = 0xff;
...
pub struct fts_data {
    ...
    pub resolution: enum WATCHDOG_RESOLUTION,
    pub /: *mut *mut bool valid; / false until following fields are valid,
    pub volt: [u8; FTS_NO_VOLT_SENSORS],
    pub temp_input: [u8; FTS_NO_TEMP_SENSORS],
    pub temp_alarm: u8,
    pub fan_present: u8,
    pub /: *mut *mut u8 fan_input[FTS_NO_FAN_SENSORS]; / in rps,
    pub fan_source: [u8; FTS_NO_FAN_SENSORS],
    pub fan_alarm: u8,
}
...
//
#[no_mangle]
unsafe extern "C" fn fts_read_byte(client: *mut i2c_client, reg: c_ushort) -> c_int {
    static int fts_read_byte(struct i2c_client *client, unsigned short reg)
    {
    int ret;
    let mut page: c_uchar = reg >> 8;
...
//
#[no_mangle]
unsafe extern "C" fn fts_update_device(data: *mut fts_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fts_wd_set_timeout(wdd: *mut watchdog_device, timeout: c_uint) -> c_int {
    static int fts_wd_set_timeout(struct watchdog_device *wdd, unsigned int timeout)
    {
    struct fts_data *data;
    let mut resolution: enum WATCHDOG_RESOLUTION = seconds;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn fts_wd_start(wdd: *mut watchdog_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fts_wd_stop(wdd: *mut watchdog_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fts_watchdog_init(data: *mut fts_data) -> c_int {
...
// max timeout 255 minutes.
    ...
    let mut ret: c_int = fts_update_device(data);
    if (ret < 0)
    return ret;
    switch (type) {
    case hwmon_temp:
...
// val = !(data->fan_present & BIT(channel));
    ...
    let mut fan_source: u8 = data.fan_source[channel];
...
#[no_mangle]
unsafe extern "C" fn fts_probe(client: *mut i2c_client) -> c_int {
...
// Baseboard Management Controller check
    ...
    &fts_chip_info, core::ptr::null_mut());
    if (IS_ERR(hwmon_dev))
