...
//
// chip identification
pub const FSCHMD_REG_IDENT_0: c_uint = 0x00;
pub const FSCHMD_REG_IDENT_1: c_uint = 0x01;
pub const FSCHMD_REG_IDENT_2: c_uint = 0x02;
pub const FSCHMD_REG_REVISION: c_uint = 0x03;
// global control and status
pub const FSCHMD_REG_EVENT_STATE: c_uint = 0x04;
pub const FSCHMD_REG_CONTROL: c_uint = 0x05;
pub const FSCHMD_CONTROL_ALERT_LED: c_uint = 0x01;
// watchdog
...
pub const FSCHMD_WDOG_CONTROL_TRIGGER: c_uint = 0x10;
pub const FSCHMD_WDOG_CONTROL_STARTED: c_uint = 0x10 /* the same as trigger */;
pub const FSCHMD_WDOG_CONTROL_STOP: c_uint = 0x20;
pub const FSCHMD_WDOG_CONTROL_RESOLUTION: c_uint = 0x40;
pub const FSCHMD_WDOG_STATE_CARDRESET: c_uint = 0x02;
...
pub const FSCHMD_FAN_ALARM: c_uint = 0x04 /* called fault by FSC! */;
pub const FSCHMD_FAN_NOT_PRESENT: c_uint = 0x08;
pub const FSCHMD_FAN_DISABLED: c_uint = 0x80;
...
pub const FSCHMD_TEMP_WORKING: c_uint = 0x01;
pub const FSCHMD_TEMP_ALERT: c_uint = 0x02;
pub const FSCHMD_TEMP_DISABLED: c_uint = 0x80;
...
// register values
    pub /: *mut *mut u8 revision; / chip revision,
    pub /: *mut *mut u8 global_control; / global control register,
    pub /: *mut *mut u8 watchdog_control; / watchdog control register,
    pub /: *mut *mut u8 watchdog_state; / watchdog status register,
    pub /: *mut *mut u8 watchdog_preset; / watchdog counter preset on trigger val,
    pub /: *mut *mut u8 volt[6]; / voltage,
    pub /: *mut *mut u8 temp_act[11]; / temperature,
    pub /: *mut *mut u8 temp_status[11]; / status of sensor,
    pub /: *mut *mut u8 temp_max[11]; / high temp limit, notice: undocumented!,
    pub /: *mut *mut u8 fan_act[7]; / fans revolutions per second,
    pub /: *mut *mut u8 fan_status[7]; / fan status,
    pub /: *mut *mut u8 fan_min[7]; / fan min value for rps,
    pub /: *mut *mut u8 fan_ripple[7]; / divider for rps,
}
...
//
#[no_mangle]
unsafe extern "C" fn fschmd_release_resources(ref: *mut kref) {
...
//
    ...
    let mut index: c_int = to_sensor_dev_attr(devattr).index;
...
// bits 2..7 reserved => mask with 0x03
    ...
    let mut index: c_int = to_sensor_dev_attr(devattr).index;
    struct fschmd_data *data = fschmd_update_device(dev);
    let mut val: c_int = data.fan_min[index];
...
//
#[no_mangle]
unsafe extern "C" fn watchdog_set_timeout(data: *mut fschmd_data, timeout: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_get_timeout(data: *mut fschmd_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_trigger(data: *mut fschmd_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn watchdog_stop(data: *mut fschmd_data) -> c_int {
...
unsafe extern "C" fn watchdog_open(inode: *mut inode, filp: *mut file) -> c_int {
    ...
    {
    struct fschmd_data *pos, *data = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn watchdog_release(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(WDIOS_ENABLECARD: i &) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn fschmd_dmi_decode(header: *const dmi_header, dummy: *mut c_void) {
...
// Our in sensors order and the DMI order differ
    ...
    let mut in: c_int = shuffle[dmi_data[i] - 1];
...
// Detect & Identify the chip
    ...
    kind = fscpos;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(id, _arg: "HER")) -> else {
    else if (!strcmp(id, "HER"))
    kind = fscher;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(id, _arg: "SCY")) -> else {
    else if (!strcmp(id, "SCY"))
    kind = fscscy;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(id, _arg: "HRC")) -> else {
    else if (!strcmp(id, "HRC"))
    kind = fschrc;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(id, _arg: "HMD")) -> else {
    else if (!strcmp(id, "HMD"))
    kind = fschmd;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(id, _arg: "HDS")) -> else {
    else if (!strcmp(id, "HDS"))
    kind = fschds;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strcmp(id, _arg: "SYL")) -> else {
...
#[no_mangle]
unsafe extern "C" fn fschmd_probe(client: *mut i2c_client) -> c_int {
    static int fschmd_probe(struct i2c_client *client)
    {
    ...
    int i, err;
    let mut kind: enum chips = (uintptr_t)i2c_get_match_data(client);
...
// Read the special DMI table for fscher and newer chips
    ...
    dmi_walk(fschmd_dmi_decode, core::ptr::null_mut());
    if (dmi_vref == -1) {
    dev_warn(&client.dev,
...
#[no_mangle]
unsafe extern "C" fn fschmd_remove(client: *mut i2c_client) {
    static void fschmd_remove(struct i2c_client *client)
    {
