...
//! Source: drivers/hwmon/ntc_thermistor.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct ntc_type {
    pub comp: *const ntc_compensation,
    pub n_comp: c_int,
}
...
pub struct ntc_data {
    pub comp: *const ntc_compensation,
    pub n_comp: c_int,
    pub pullup_uv: c_uint,
    pub pullup_ohm: c_uint,
    pub pulldown_ohm: c_uint,
    pub connect: enum { NTC_CONNECTED_POSITIVE, NTC_CONNECTED_GROUND },
    pub chan: *mut iio_channel,
}

#[no_mangle]
unsafe extern "C" fn ntc_adc_iio_read(data: *mut ntc_data) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn div64_u64_safe(dividend: u64, divisor: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn get_ohm_of_thermistor(data: *mut ntc_data, uv: c_uint) -> c_int {
    static int get_ohm_of_thermistor(struct ntc_data *data, unsigned int uv)
    {
    let mut puv: u32 = data.pullup_uv;
    u64 n, puo, pdo;
    puo = data.pullup_ohm;
...
#[no_mangle]
pub unsafe extern "C" fn if(0: data->connect == NTC_CONNECTED_GROUND && pdo ==) -> else {
    else if (data.connect == NTC_CONNECTED_GROUND && pdo == 0)
    n = div_u64(puo * uv, puv - uv);
#[no_mangle]
pub unsafe extern "C" fn if(NTC_CONNECTED_POSITIVE: data->connect ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn get_temp_mc(data: *mut ntc_data, ohm: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntc_thermistor_get_ohm(data: *mut ntc_data) -> c_int {
...
// val = 4;
    ...
    int ret;
    chan = devm_iio_channel_get(dev, core::ptr::null_mut());
    if (IS_ERR(chan))
    return PTR_ERR(chan);
...
#[no_mangle]
unsafe extern "C" fn ntc_thermistor_probe(pdev: *mut platform_device) -> c_int {
