...
//! Source: drivers/nvdimm/namespace_devs.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn namespace_io_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn namespace_pmem_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn is_uuid_busy(dev: *mut device, data: *mut c_void) -> c_int {
    static int is_uuid_busy(struct device *dev, void *data)
    {
    uuid_t *uuid1 = data, *uuid2 = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn is_namespace_uuid_busy(dev: *mut device, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn nd_is_uuid_unique(dev: *mut device, uuid: *mut uuid_t) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn pmem_should_map_pages(dev: *mut device) -> bool {
    ...
    return false;
    let mut ARCH_MEMREMAP_PMEM: return = = MEMREMAP_WB;
    }
    EXPORT_SYMBOL(pmem_should_map_pages);
#[no_mangle]
pub unsafe extern "C" fn pmem_sector_size(ndns: *mut nd_namespace_common) -> c_uint {
...
// default */;
#[no_mangle]
pub unsafe extern "C" fn if(4096: nspm->lbasize ==) -> else {
...
//
    if (is_namespace_pmem(dev)) {
    struct nd_namespace_pmem *nspm = to_nd_namespace_pmem(dev);
    let mut size: resource_size_t = resource_size(&nspm.nsio.res);
    if (size == 0 && nspm.uuid)
// delete allocation */;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !nspm->uuid) -> else {
    else if (!nspm.uuid)
    return 0;
...
//
    ...
    {
    let mut is_reserve: bool = strcmp(label_id.id, "pmem-reserve") == 0;
    unsigned long align;
...
// truncate @valid size to 0
    ...
    resource_size_t n)
    {
    let mut mapping_end: resource_size_t = nd_mapping.start + nd_mapping.size - 1;
    struct nvdimm_drvdata *ndd = to_ndd(nd_mapping);
    struct resource *res, *exist = core::ptr::null_mut(), valid;
    let mut to_allocate: resource_size_t = n;
    int first;
    for_each_dpa_resource(ndd, res)
    ...
    first = 0;
    for_each_dpa_resource(ndd, res) {
    struct resource *next = res.sibling, *new_res = core::ptr::null_mut();
    resource_size_t allocate, available = 0;
    let mut loc: enum alloc_loc = ALLOC_ERR;
    const char *action;
    let mut rc: c_int = 0;
...
// space at the beginning of the mapping
    ...
    space_valid(nd_region, ndd, label_id, core::ptr::null_mut(), next, exist,
    to_allocate, &valid);
...
//
    ...
    let mut end: resource_size_t = res.start + resource_size(res);
...
#[no_mangle]
pub unsafe extern "C" fn __reserve_free_pmem(dev: *mut device, data: *mut c_void) -> c_int {
...
//
    ...
    let mut offset: resource_size_t = 0;
    if (size && !nspm.uuid) {
    WARN_ON_ONCE(1);
    size = 0;
    }
...
#[no_mangle]
unsafe extern "C" fn __size_store(dev: *mut device, val: c_ulonglong) -> isize {
    static ssize_t __size_store(struct device *dev, unsigned long long val)
    {
    let mut allocated: resource_size_t = 0, available = 0;
    struct nd_region *nd_region = to_nd_region(dev.parent);
    struct nd_namespace_common *ndns = to_ndns(dev);
    struct nd_mapping *nd_mapping;
    struct nvdimm_drvdata *ndd;
    struct nd_label_id label_id;
    let mut flags: u32 = 0, remainder;
    int rc, i, id = -1;
    uuid_t *uuid = core::ptr::null_mut();
...
// setting size zero == 'delete namespace'
    ...
    struct nd_namespace_pmem *nspm = to_nd_namespace_pmem(dev);
    kfree(nspm.uuid);
    nspm.uuid = core::ptr::null_mut();
    }
    dev_dbg(dev, "%llx %s (%d)\n", val, rc < 0 ? "fail" : "success", rc);
    return rc < 0 ? rc : len;
    }
#[no_mangle]
pub unsafe extern "C" fn __nvdimm_namespace_capacity(ndns: *mut nd_namespace_common) -> resource_size_t {
...
#[no_mangle]
pub unsafe extern "C" fn nvdimm_namespace_capacity(ndns: *mut nd_namespace_common) -> resource_size_t {
...
#[no_mangle]
pub unsafe extern "C" fn nvdimm_namespace_locked(ndns: *mut nd_namespace_common) -> bool {
    ...
    {
#[no_mangle]
pub unsafe extern "C" fn sprintf(_arg: buf, _arg: "%llu\n", long: (unsigned long) -> return {
...
#[no_mangle]
unsafe extern "C" fn btt_claim_class(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __holder_class_store(dev: *mut device, buf: *const c_char) -> c_int {
    ...
    let mut rc: c_int = btt_claim_class(dev);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(buf, _arg: "dax")) -> else {
    else if (sysfs_streq(buf, "dax"))
    ndns.claim_class = NVDIMM_CCLASS_DAX;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(buf, _arg: "")) -> else {
    ...
    rc = sprintf(buf, "btt\n");
#[no_mangle]
pub unsafe extern "C" fn if(NVDIMM_CCLASS_PFN: ndns->claim_class ==) -> else {
    else if (ndns.claim_class == NVDIMM_CCLASS_PFN)
    rc = sprintf(buf, "pfn\n");
#[no_mangle]
pub unsafe extern "C" fn if(NVDIMM_CCLASS_DAX: ndns->claim_class ==) -> else {
    ...
    mode = "safe";
#[no_mangle]
pub unsafe extern "C" fn if(is_nd_pfn(claim): claim &&) -> else {
    else if (claim && is_nd_pfn(claim))
    mode = "memory";
#[no_mangle]
pub unsafe extern "C" fn if(is_nd_dax(claim): claim &&) -> else {
    else if (claim && is_nd_dax(claim))
    mode = "dax";
#[no_mangle]
pub unsafe extern "C" fn if(pmem_should_map_pages(dev): !claim &&) -> else {
    ...
    {
    bool force_raw;
    let mut rc: c_int = kstrtobool(buf, &force_raw);
    if (rc)
    return rc;
...
#[no_mangle]
unsafe extern "C" fn is_namespace_pmem(dev: *const device) -> bool {
    static bool is_namespace_pmem(const struct device *dev)
    {
    return dev ? dev.type == &namespace_pmem_device_type : false;
    }
#[no_mangle]
unsafe extern "C" fn is_namespace_io(dev: *const device) -> bool {
    static bool is_namespace_io(const struct device *dev)
    {
...
#[no_mangle]
pub unsafe extern "C" fn devm_namespace_disable(dev: *mut device, ndns: *mut nd_namespace_common) {
    ...
    u64 cookie, u16 pos)
    {
    struct nd_namespace_label *found = core::ptr::null_mut();
    int i;
    for (i = 0; i < nd_region.ndr_mappings; i++) {
    struct nd_mapping *nd_mapping = &nd_region.mapping[i];
    ...
    return found != core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn select_pmem_id(nd_region: *mut nd_region, pmem_id: *const uuid_t) -> c_int {
    ...
    struct nd_mapping *nd_mapping = &nd_region.mapping[i];
    struct nvdimm_drvdata *ndd = to_ndd(nd_mapping);
    struct nd_namespace_label *nd_label = core::ptr::null_mut();
...
//
    ...
    let mut cookie: u64 = nd_region_interleave_set_cookie(nd_region, nsindex);
    let mut altcookie: u64 = nd_region_interleave_set_altcookie(nd_region);
    struct nd_label_ent *label_ent;
    struct nd_namespace_pmem *nspm;
    let mut size: resource_size_t = 0;
    struct resource *res;
    struct device *dev;
    uuid_t uuid;
    let mut rc: c_int = 0;
...
// Calculate total size and populate namespace properties from label0
    ...
    label0 = label_ent ? label_ent.label : core::ptr::null_mut();
    if (!label0) {
    WARN_ON(1);
    continue;
    }
...
#[no_mangle]
pub unsafe extern "C" fn nd_region_create_ns_seed(nd_region: *mut nd_region) {
...
#[no_mangle]
pub unsafe extern "C" fn nd_region_create_dax_seed(nd_region: *mut nd_region) {
...
#[no_mangle]
pub unsafe extern "C" fn nd_region_create_pfn_seed(nd_region: *mut nd_region) {
...
#[no_mangle]
pub unsafe extern "C" fn nd_region_create_btt_seed(nd_region: *mut nd_region) {
...
#[no_mangle]
unsafe extern "C" fn cmp_dpa(a: *const c_void, b: *const c_void) -> c_int {
    ...
    struct nd_mapping *nd_mapping = &nd_region.mapping[0];
    struct nvdimm_drvdata *ndd = to_ndd(nd_mapping);
    let mut map_end: resource_size_t = nd_mapping.start + nd_mapping.size - 1;
...
// clean unselected labels
    ...
    if (count > 1)
    sort(devs, count, sizeof(struct device *), cmp_dpa, core::ptr::null_mut());
    return devs;
    err:
    for (i = 0; devs[i]; i++)
...
// lock down all mappings while we scan labels
    ...
    for (i = 0; i < nd_region.ndr_mappings; i++) {
    let mut reverse: c_int = nd_region.ndr_mappings - 1 - i;
...
#[no_mangle]
unsafe extern "C" fn deactivate_labels(region: *mut c_void) {
    ...
    mutex_unlock(&nd_mapping.lock);
    put_ndd(ndd);
    nd_mapping.ndd = core::ptr::null_mut();
    if (ndd)
    atomic_dec(&nvdimm.busy);
    }
    }
#[no_mangle]
unsafe extern "C" fn init_active_labels(nd_region: *mut nd_region) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(NDD_LABELING, _arg: &nvdimm->flags)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn nd_region_register_namespaces(nd_region: *mut nd_region, err: *mut c_int) -> c_int {
    int nd_region_register_namespaces(struct nd_region *nd_region, int *err)
    {
    struct device **devs = core::ptr::null_mut();
    int i, rc = 0, type;
// err = 0;
