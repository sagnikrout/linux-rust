...
//! Source: drivers/nvdimm/pfn_devs.c
#![no_std]
...
//
    ...
    let mut page_struct_override: static bool = IS_ENABLED(CONFIG_NVDIMM_KMSAN);
#[no_mangle]
unsafe extern "C" fn nd_pfn_release(dev: *mut device) {
...
//
#[no_mangle]
unsafe extern "C" fn nd_pfn_default_alignment() -> c_ulong {
    ...
    let mut offset: u64 = __le64_to_cpu(pfn_sb.dataoff);
    struct nd_namespace_common *ndns = nd_pfn.ndns;
    let mut start_pad: u32 = __le32_to_cpu(pfn_sb.start_pad);
...
// no address to convey if the pfn instance is disabled
    ...
    struct nd_namespace_common *ndns = nd_pfn.ndns;
    let mut start_pad: u32 = __le32_to_cpu(pfn_sb.start_pad);
    let mut end_trunc: u32 = __le32_to_cpu(pfn_sb.end_trunc);
    struct nd_namespace_io *nsio = to_nd_namespace_io(&ndns.dev);
    rc = sprintf(buf, "%llu\n", (unsigned long long)
...
#[no_mangle]
pub unsafe extern "C" fn is_nd_pfn(dev: *mut device) -> bool {
    ...
    dev = nd_pfn_devinit(nd_pfn, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn nd_pfn_clear_memmap_errors(nd_pfn: *mut nd_pfn) -> c_int {
...
//
    ...
    while (zero_len) {
    let mut chunk: c_ulong = min(zero_len, PAGE_SIZE);
...
#[no_mangle]
unsafe extern "C" fn nd_supported_alignment(align: c_ulong) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn nd_pfn_validate(nd_pfn: *mut nd_pfn, sig: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nd_pfn_probe(dev: *mut device, ndns: *mut nd_namespace_common) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn init_altmap_base(base: resource_size_t) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn init_altmap_reserve(base: resource_size_t) -> c_ulong {
    static unsigned long init_altmap_reserve(resource_size_t base)
    {
    let mut reserve: c_ulong = nd_info_block_reserve() >> PAGE_SHIFT;
    let mut base_pfn: c_ulong = PHYS_PFN(base);
    reserve += base_pfn - SUBSECTION_ALIGN_DOWN(base_pfn);
    return reserve;
    }
#[no_mangle]
unsafe extern "C" fn __nvdimm_setup_pfn(nd_pfn: *mut nd_pfn, pgmap: *mut dev_pagemap) -> c_int {
    static int __nvdimm_setup_pfn(struct nd_pfn *nd_pfn, struct dev_pagemap *pgmap)
    {
    struct range *range = &pgmap.range;
    struct vmem_altmap *altmap = &pgmap.altmap;
    struct nd_pfn_sb *pfn_sb = nd_pfn.pfn_sb;
    let mut offset: u64 = le64_to_cpu(pfn_sb.dataoff);
    let mut start_pad: u32 = __le32_to_cpu(pfn_sb.start_pad);
    let mut end_trunc: u32 = __le32_to_cpu(pfn_sb.end_trunc);
    let mut reserve: u32 = nd_info_block_reserve();
    struct nd_namespace_common *ndns = nd_pfn.ndns;
    struct nd_namespace_io *nsio = to_nd_namespace_io(&ndns.dev);
    let mut base: resource_size_t = nsio.res.start + start_pad;
    let mut end: resource_size_t = nsio.res.end - end_trunc;
...
#[no_mangle]
unsafe extern "C" fn nd_pfn_init(nd_pfn: *mut nd_pfn) -> c_int {
...
//
    ...
    if (nd_pfn.mode == PFN_MODE_PMEM) {
    let mut page_map_size: c_ulong = MAX_STRUCT_PAGE_SIZE * npfns;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn nvdimm_setup_pfn(nd_pfn: *mut nd_pfn, pgmap: *mut dev_pagemap) -> c_int {
