...
//! Source: drivers/edac/mem_repair.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct edac_mem_repair_dev_attr {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct edac_mem_repair_context {
    pub name: [c_char; EDAC_FEAT_NAME_LEN],
    pub mem_repair_dev_attr: [edac_mem_repair_dev_attr; MR_MAX_ATTRS],
    pub 1]: *mut *mut attribute mem_repair_attrs[MR_MAX_ATTRS +,
...
#[no_mangle]
unsafe extern "C" fn mem_repair_attr_visible(kobj: *mut kobject, a: *mut attribute, attr_id: c_int) -> umode_t {
    ...
    struct edac_dev_feat_ctx *ctx = dev_get_drvdata(ras_feat_dev);
    let mut inst: u8 = TO_MR_DEV_ATTR(dev_attr).instance;
    const struct edac_mem_repair_ops *ops = ctx.mem_repair[inst].mem_repair_ops;
    switch (attr_id) {
    case MR_TYPE:
