...
//! Source: drivers/edac/i7300_edac.c
#![no_std]
...
pub const MAX_CH_PER_BRANCH: c_int = 2;

pub const MAX_MIR: c_int = 3;

    (to_channel(ch, branch) | ((slot) << 2))
// Device name and register DID (Device ID)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct i7300_dev_info {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct i7300_pvt {
    pub /: *mut *mut *mut pci_dev pci_dev_16_0_fsb_ctlr; / 16.0,
    pub /: *mut *mut *mut pci_dev pci_dev_16_1_fsb_addr_map; / 16.1,
    pub /: *mut *mut *mut pci_dev pci_dev_16_2_fsb_err_regs; / 16.2,
    pub /: *mut *mut *mut pci_dev pci_dev_2x_0_fbd_branch[MAX_BRANCHES]; / 21.0 and 22.0,
    pub /: *mut *mut u16 tolm; / top of low memory,
    pub /: *mut *mut u64 ambase; / AMB BAR,
    pub /: *mut *mut u32 mc_settings; / Report several settings,
    pub mc_settings_a: u32,
    pub Reg*/: *mut *mut u16 mir[MAX_MIR]; / Memory Interleave,
    pub /: *mut *mut u16 mtr[MAX_SLOTS][MAX_BRANCHES]; / Memory Technlogy Reg,
    pub /: *mut *mut u16 ambpresent[MAX_CHANNELS]; / AMB present regs,
// DIMM information matrix, allocating architecture maximums
    pub dimm_info: [i7300_dimm_info; MAX_SLOTS][MAX_CHANNELS],
// Temporary buffer for use when preparing error messages
    pub tmp_prt_buffer: *mut c_char,
// Hardware error reporting status
    pub enabled_error_reporting: bool,
}
...
pub const MC_SETTINGS: c_uint = 0x40;

pub const MC_SETTINGS_A: c_uint = 0x58;
...
pub const MTR_DRAM_BANKS_ADDR_BITS: c_int = 2;
...
//
pub const FERR_GLOBAL_HI: c_uint = 0x48;
...
pub const ferr_global_hi_is_fatal(errno): c_int = 1;
pub const FERR_GLOBAL_LO: c_uint = 0x40;
...
//
#[no_mangle]
unsafe extern "C" fn i7300_process_error_global(mci: *mut mem_ctl_info) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_process_fbd_error(mci: *mut mem_ctl_info) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_check_error(mci: *mut mem_ctl_info) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_clear_error(mci: *mut mem_ctl_info) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_set_error_reporting(mci: *mut mem_ctl_info, enable: bool) {
...
//
#[no_mangle]
unsafe extern "C" fn print_dimm_size(pvt: *mut i7300_pvt) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_init_csrows(mci: *mut mem_ctl_info) -> c_int {
...
// Get the set of MTR[0-7] regs by each branch
    for (slot = 0; slot < MAX_SLOTS; slot++) {
    let mut where: c_int = mtr_regs[slot];
    for (branch = 0; branch < max_branch; branch++) {
    pci_read_config_word(pvt.pci_dev_2x_0_fbd_branch[branch],
    where,
    &pvt.mtr[slot][branch]);
    for (ch = 0; ch < max_channel; ch++) {
    let mut channel: c_int = to_channel(ch, branch);
...
//
#[no_mangle]
unsafe extern "C" fn decode_mir(mir_no: c_int, mir[MAX_MIR]: u16) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_get_mc_regs(mci: *mut mem_ctl_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_put_devices(mci: *mut mem_ctl_info) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_get_devices(mci: *mut mem_ctl_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_init_one(pdev: *mut pci_dev, id: *const pci_device_id) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_remove_one(pdev: *mut pci_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_init() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn i7300_exit() -> void __exit {
