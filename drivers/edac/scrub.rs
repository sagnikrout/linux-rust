...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct edac_scrub_dev_attr {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct edac_scrub_context {
    pub name: [c_char; EDAC_FEAT_NAME_LEN],
    pub scrub_dev_attr: [edac_scrub_dev_attr; SCRUB_MAX_ATTRS],
    pub 1]: *mut *mut attribute scrub_attrs[SCRUB_MAX_ATTRS +,
...
#[no_mangle]
unsafe extern "C" fn scrub_attr_visible(kobj: *mut kobject, a: *mut attribute, attr_id: c_int) -> umode_t {
    ...
    let mut inst: u8 = TO_SCRUB_DEV_ATTR(dev_attr).instance;
    struct edac_dev_feat_ctx *ctx = dev_get_drvdata(ras_feat_dev);
    const struct edac_scrub_ops *ops = ctx.scrub[inst].scrub_ops;
    switch (attr_id) {
    case SCRUB_ADDRESS:
