...
// mapping is only set on the head
    ...
    phys_addr_t phys;
    unsigned long pfn;
    let mut fault_size: c_uint = PAGE_SIZE;
    ...
    pgoff_t pgoff;
    unsigned long pfn;
    let mut fault_size: c_uint = PMD_SIZE;
    if (check_vma(dev_dax, vmf.vma, __func__))
    return VM_FAULT_SIGBUS;
    ...
    return VM_FAULT_SIGBUS;
#[no_mangle]
pub unsafe extern "C" fn if(dev_dax->align: fault_size >) -> else {
...
// if we are outside of the VMA
    ...
    struct vm_fault *vmf)
    {
    let mut pud_addr: c_ulong = vmf.address & PUD_MASK;
    struct device *dev = &dev_dax.dev;
    phys_addr_t phys;
    pgoff_t pgoff;
    unsigned long pfn;
    let mut fault_size: c_uint = PUD_SIZE;
    if (check_vma(dev_dax, vmf.vma, __func__))
    return VM_FAULT_SIGBUS;
...
#[no_mangle]
unsafe extern "C" fn dev_dax_huge_fault(vmf: *mut vm_fault, order: c_uint) -> vm_fault_t {
    static vm_fault_t dev_dax_huge_fault(struct vm_fault *vmf, unsigned int order)
    {
    struct file *filp = vmf.vma.vm_file;
    let mut rc: vm_fault_t = VM_FAULT_SIGBUS;
    int id;
    struct dev_dax *dev_dax = filp.private_data;
...
#[no_mangle]
pub unsafe extern "C" fn if(PUD_ORDER: order ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn dev_dax_fault(vmf: *mut vm_fault) -> vm_fault_t {
    static vm_fault_t dev_dax_fault(struct vm_fault *vmf)
    {
    return dev_dax_huge_fault(vmf, 0);
    }
#[no_mangle]
unsafe extern "C" fn dev_dax_may_split(vma: *mut vm_area_struct, addr: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dev_dax_pagesize(vma: *mut vm_area_struct) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn dax_mmap_prepare(desc: *mut vm_area_desc) -> c_int {
...
// return an unmapped area aligned to the dax region specified alignment
    ...
    struct dev_dax *dev_dax = filp ? filp.private_data : core::ptr::null_mut();
    if (!dev_dax || addr)
    goto out;
    align = dev_dax.align;
...
#[no_mangle]
unsafe extern "C" fn dax_open(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dax_release(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dev_dax_cdev_del(cdev: *mut c_void) {
    static void dev_dax_cdev_del(void *cdev)
    {
    cdev_del(cdev);
    }
#[no_mangle]
unsafe extern "C" fn dev_dax_kill(dev_dax: *mut c_void) {
    static void dev_dax_kill(void *dev_dax)
    {
    kill_dev_dax(dev_dax);
    }
#[no_mangle]
unsafe extern "C" fn dev_dax_probe(dev_dax: *mut dev_dax) -> c_int {
