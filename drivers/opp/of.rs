...
// Returns opp descriptor node for a device, caller must do of_node_put()
    ...
    {
    struct opp_table *opp_table, *managed_table = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn _opp_table_free_required_tables(opp_table: *mut opp_table) {
    ...
    opp_table.required_opp_tables = core::ptr::null_mut();
    guard(mutex)(&opp_table_lock);
...
// Traversing the first OPP node is all we need
    struct device_node *np __free(device_node) =
    of_get_next_available_child(opp_np, core::ptr::null_mut());
    if (!np) {
    dev_warn(dev, "Empty OPP table\n");
    return;
    }
    count = of_count_phandle_with_args(np, "required-opps", core::ptr::null_mut());
    if (count <= 0)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn _of_clear_opp_table(opp_table: *mut opp_table) {
...
// Put the reference back
    ...
    opp.required_opps = core::ptr::null_mut();
    kfree(required_opps);
    }
#[no_mangle]
pub unsafe extern "C" fn _of_clear_opp(opp_table: *mut opp_table, opp: *mut dev_pm_opp) {
...
#[no_mangle]
unsafe extern "C" fn lazy_link_required_opp_table(new_table: *mut opp_table) {
...
// opp_np can't be invalid here
    ...
    of_get_next_available_child(opp_table.np, core::ptr::null_mut());
    for (i = 0; i < opp_table.required_opp_count; i++) {
...
#[no_mangle]
unsafe extern "C" fn _bandwidth_supported(dev: *mut device, opp_table: *mut opp_table) -> c_int {
    static int _bandwidth_supported(struct device *dev, struct opp_table *opp_table)
    {
    struct device_node *opp_np __free(device_node) = core::ptr::null_mut();
...
// Checking only first OPP is sufficient
    ...
    prop = of_find_property(np, "opp-peak-kBps", core::ptr::null_mut());
    if (!prop || !prop.length)
    return 0;
    return 1;
    }
...
// two phandles when #interconnect-cells = <1>
    ...
    {
    let mut levels: c_uint = opp_table.supported_hw_count;
...
// Search for "opp-<prop_type>-<name>"
    ...
    opp_table.prop_name);
    prop = of_find_property(opp.np, name, core::ptr::null_mut());
    }
    if (!prop) {
...
//
    ...
    let mut ret: c_int = 0, i, j;
    bool triplet;
    microvolt = opp_parse_microvolt(opp, dev, opp_table, &triplet);
    if (IS_ERR(microvolt))
    return PTR_ERR(microvolt);
    microamp = _parse_named_prop(opp, dev, opp_table, "microamp", core::ptr::null_mut());
    if (IS_ERR(microamp)) {
    ret = PTR_ERR(microamp);
    goto free_microvolt;
    }
    microwatt = _parse_named_prop(opp, dev, opp_table, "microwatt", core::ptr::null_mut());
    if (IS_ERR(microwatt)) {
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_pm_opp_of_remove_table(dev: *mut device) {
    ...
    int i, count, ret;
    u64 *rates;
    prop = of_find_property(np, "opp-hz", core::ptr::null_mut());
    if (!prop)
    return -ENODEV;
...
#[no_mangle]
unsafe extern "C" fn _of_add_opp_table_v2(dev: *mut device, opp_table: *mut opp_table) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn _of_add_opp_table_v1(dev: *mut device, opp_table: *mut opp_table) -> c_int {
    ...
    prop = of_find_property(dev.of_node, "operating-points", core::ptr::null_mut());
    if (!prop) {
    ret = -ENODEV;
    goto remove_static_opp;
    }
...
//
    ...
    val = prop.value;
    while (nr) {
    let mut freq: c_ulong = be32_to_cpup(val++) * 1000;
    let mut volt: c_ulong = be32_to_cpup(val++);
...
#[no_mangle]
unsafe extern "C" fn _of_add_table_indexed(dev: *mut device, index: c_int) -> c_int {
...
//
    ...
    "operating-points-v2", core::ptr::null_mut());
    if (count == 1)
    index = 0;
    }
...
#[no_mangle]
unsafe extern "C" fn devm_pm_opp_of_table_release(data: *mut c_void) {
    static void devm_pm_opp_of_table_release(void *data)
    {
    dev_pm_opp_of_remove_table(data);
    }
#[no_mangle]
unsafe extern "C" fn _devm_of_add_table_indexed(dev: *mut device, index: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn devm_pm_opp_of_add_table(dev: *mut device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_pm_opp_of_add_table(dev: *mut device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_pm_opp_of_add_table_indexed(dev: *mut device, index: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn devm_pm_opp_of_add_table_indexed(dev: *mut device, index: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_pm_opp_of_cpumask_remove_table(cpumask: *const cpumask) {
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_pm_opp_of_cpumask_add_table(cpumask: *const cpumask) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_get_required_opp_performance_state(np: *mut device_node, index: c_int) -> c_int {
    int of_get_required_opp_performance_state(struct device_node *np, int index)
    {
    let mut pstate: c_int = -EINVAL;
    struct device_node *required_np __free(device_node) =
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_pm_opp_of_has_required_opp(dev: *mut device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn _of_has_opp_microwatt_property(dev: *mut device) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn dev_pm_opp_of_register_em(dev: *mut device, cpus: *mut cpumask) -> c_int {
