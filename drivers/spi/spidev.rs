...
// TX/RX buffers are NULL unless this device is open (users > 0)
    ...
    let mut bufsiz: static unsigned = 4096;
...
// Read-only message with current device setup
    ...
    let mut status: isize = -ESHUTDOWN;
// chipselect only toggles at start or end of operation
    if (count > bufsiz)
    return -EMSGSIZE;
    spidev = filp.private_data;
    mutex_lock(&spidev.spi_lock);
    if (spidev.spi == core::ptr::null_mut())
...
//
    let mut len_aligned: c_uint = ALIGN(u_tmp.len, ARCH_DMA_MINALIGN);
    k_tmp.len = u_tmp.len;
    total += k_tmp.len;
...
// write requests
    ...
    retval = get_user(tmp, (__u8 __user *)arg);
    if (retval == 0) {
    let mut save: u8 = spi.bits_per_word;
...
#[no_mangle]
unsafe extern "C" fn spidev_open(inode: *mut inode, filp: *mut file) -> c_int {
    static int spidev_open(struct inode *inode, struct file *filp)
    {
    struct spidev_data	*spidev = core::ptr::null_mut(), *iter;
    ...
    spidev.tx_buffer = core::ptr::null_mut();
    err_find_dev:
    mutex_unlock(&device_list_lock);
    return status;
    }
#[no_mangle]
unsafe extern "C" fn spidev_release(inode: *mut inode, filp: *mut file) -> c_int {
...
// ... after we unbound from the underlying device?
    dofree = (spidev.spi == core::ptr::null_mut());
    mutex_unlock(&spidev.spi_lock);
// last close?
    spidev.users--;
    if (!spidev.users) {
    ...
    spidev.rx_buffer = core::ptr::null_mut();
    if (dofree)
    kfree(spidev);
    else
...
//
#[no_mangle]
unsafe extern "C" fn spidev_of_check(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spidev_acpi_check(dev: *mut device) -> c_int {
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn spidev_probe(spi: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spidev_remove(spi: *mut spi_device) {
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn spidev_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn spidev_exit() -> void __exit {
