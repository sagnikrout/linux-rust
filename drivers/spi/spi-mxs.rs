...
pub const SSP_TIMEOUT: c_int = 10000;
pub const SG_MAXLEN: c_uint = 0xff00;
//
...
pub struct mxs_spi {
    pub ssp: mxs_ssp,
    pub c: completion,
    pub /: *mut *mut unsigned int sck; / Rate requested (vs actual),
}
    ...
    let mut hz: c_uint = min(dev.max_speed_hz, t.speed_hz);
    if (hz == 0) {
...
#[no_mangle]
unsafe extern "C" fn mxs_spi_cs_to_reg(cs: unsigned) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn mxs_ssp_wait(spi: *mut mxs_spi, offset: c_int, mask: c_int, set: bool) -> c_int {
    static int mxs_ssp_wait(struct mxs_spi *spi, int offset, int mask, bool set)
    {
    let mut timeout: c_ulong = jiffies + msecs_to_jiffies(SSP_TIMEOUT);
    struct mxs_ssp *ssp = &spi.ssp;
    u32 reg;
    do {
...
#[no_mangle]
unsafe extern "C" fn mxs_ssp_dma_irq_callback(param: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn mxs_ssp_irq_handler(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    ...
    let mut desc_len: c_int = vmalloced_buf ? PAGE_SIZE : SG_MAXLEN;
    let mut sgs: c_int = DIV_ROUND_UP(len, desc_len);
    int sg_count;
    int min, ret;
    u32 ctrl0;
    struct page *vm_page;
...
#[no_mangle]
unsafe extern "C" fn mxs_spi_runtime_suspend(dev: *mut device) -> c_int {
    ...
    if (ret) {
    let mut ret2: c_int = clk_prepare_enable(ssp.clk);
    if (ret2)
...
#[no_mangle]
unsafe extern "C" fn mxs_spi_runtime_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mxs_spi_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mxs_spi_resume(dev: *mut device) -> c_int {
    ...
    RUNTIME_PM_OPS(mxs_spi_runtime_suspend, mxs_spi_runtime_resume, core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn mxs_spi_probe(pdev: *mut platform_device) -> c_int {
...
//
    let mut clk_freq_default: c_int = 160000000;
...
#[no_mangle]
unsafe extern "C" fn mxs_spi_remove(pdev: *mut platform_device) {
