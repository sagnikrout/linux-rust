...
pub const LJCA_SPI_BUS_MAX_HZ: c_int = 48000000;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ljca_spi_init_packet {
    pub index: u8,
    pub speed: u8,
    pub mode: u8,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ljca_spi_xfer_packet {
    pub indicator: u8,
    pub len: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ljca_spi_dev {
    pub ljca: *mut ljca_client,
    pub controller: *mut spi_controller,
    pub spi_info: *mut ljca_spi_info,
    pub speed: u8,
    pub mode: u8,
    pub obuf: [u8; LJCA_SPI_BUF_SIZE],
    pub ibuf: [u8; LJCA_SPI_BUF_SIZE],
}
...
// (__le16 *)&w_packet->data[0] = cpu_to_le16(len);
    ...
    return ret;
#[no_mangle]
pub unsafe extern "C" fn if(0: *mut *mut ret < sizeof(r_packet) || r_packet->len <=) -> else {
...
#[no_mangle]
unsafe extern "C" fn ljca_spi_init(ljca_spi: *mut ljca_spi_dev, div: u8, mode: u8) -> c_int {
    static int ljca_spi_init(struct ljca_spi_dev *ljca_spi, u8 div, u8 mode)
    {
    let mut w_packet: ljca_spi_init_packet = {};
    int ret;
    ...
    sizeof(w_packet), core::ptr::null_mut(), 0);
    if (ret < 0)
    return ret;
    ljca_spi.mode = mode;
    ljca_spi.speed = div;
...
#[no_mangle]
unsafe extern "C" fn ljca_spi_deinit(ljca_spi: *mut ljca_spi_dev) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tx_data) -> else {
    else if (tx_data)
    cmd = LJCA_SPI_WRITE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: rx_data) -> else {
    else if (rx_data)
    cmd = LJCA_SPI_READ;
    else
    return -EINVAL;
    ...
    tx_data ? tx_data + offset : core::ptr::null_mut(),
    rx_data ? rx_data + offset : core::ptr::null_mut(),
    ...
    {
    let mut div: u8 = DIV_ROUND_UP(controller.max_speed_hz, xfer.speed_hz) / 2 - 1;
...
#[no_mangle]
unsafe extern "C" fn ljca_spi_dev_remove(auxdev: *mut auxiliary_device) {
...
#[no_mangle]
unsafe extern "C" fn ljca_spi_dev_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ljca_spi_dev_resume(dev: *mut device) -> c_int {
