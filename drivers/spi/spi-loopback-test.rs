...
//! Source: drivers/spi/spi-loopback-test.c
#![no_std]
...
// run tests only for a specific length
    let mut run_only_iter_len: static int = -1;
...
// run only a specific test
    let mut run_only_test: static int = -1;
...
// check rx ranges
    let mut check_ranges: static int = 1;
    module_param(check_ranges, int, 0644);
    MODULE_PARM_DESC(check_ranges,
    "checks rx_buffer pattern are valid");
    let mut delay_ms: static unsigned int = 100;
...
#[no_mangle]
unsafe extern "C" fn spi_loopback_test_probe(spi: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spi_test_print_hex_dump(pre: *mut c_char, ptr: *const c_void, len: usize) {
...
// if no ranges, then we can return and avoid the checks...
    ...
    return 0;
// sort the list
    list_sort(core::ptr::null_mut(), &ranges_list, rx_ranges_cmp);
...
//
    ...
    {
    int i;
    let mut estimated_time: c_ulonglong = 0;
    let mut delay_usecs: c_ulonglong = 0;
...
//
    ...
    "%s: iterate_tx_off configured with tx_buf==core::ptr::null_mut() - ignoring\n",
    test.description);
    return 0;
    }
    if (rx_off && (!rx_count)) {
    dev_warn_once(&spi.dev,
    "%s: iterate_rx_off configured with rx_buf==core::ptr::null_mut() - ignoring\n",
    test.description);
    return 0;
    }
// write out info
...
//
    ...
    {
    char *rx = core::ptr::null_mut(), *tx = core::ptr::null_mut();
