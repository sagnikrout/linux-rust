...
//! Source: drivers/spi/spi-spacemit-k1.c
#![no_std]
...
pub const K1_SPI_MIN_SPEED_HZ: c_int = 6250;
pub const K1_SPI_MAX_SPEED_HZ: c_int = 51200000;
// DMA constraints
pub const K1_SPI_DMA_ALIGNMENT: c_int = 64;

// SSP Top Control Register
pub const SSP_TOP_CTRL: c_uint = 0x00;

// SSP FIFO Control Register
pub const SSP_FIFO_CTRL: c_uint = 0x04;

// SSP Interrupt Enable Register
pub const SSP_INT_EN: c_uint = 0x08;
...
pub const SSP_TIMEOUT: c_uint = 0x0c;

// SSP Data Register
pub const SSP_DATAR: c_uint = 0x10;
// SSP Status Register
pub const SSP_STATUS: c_uint = 0x14;

// Error status mask

    (SSP_STATUS_TUR | SSP_STATUS_ROR | SSP_STATUS_BCE)
// The FIFO sizes and thresholds are the same for RX and TX
pub const K1_SPI_FIFO_SIZE: c_int = 32;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct k1_spi_driver_data {
    pub host: *mut spi_controller,
    pub base: *mut void __iomem,
    pub base_addr: phys_addr_t,
    pub bus_rate: c_ulong,
    pub clk: *mut clk,
    pub rate: c_ulong,
    pub irq: c_int,
// Current transfer information; not valid if message is null
    pub /: *mut *mut u32 bytes; / Bytes used for bits_per_word,
    pub /: *mut *mut unsigned int rx_resid; / RX bytes left in transfer,
    pub /: *mut *mut unsigned int tx_resid; / TX bytes left in transfer,
...
//
#[no_mangle]
unsafe extern "C" fn k1_spi_setup(spi: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn k1_spi_cleanup(spi: *mut spi_device) {
...
#[no_mangle]
unsafe extern "C" fn k1_spi_dma_callback(param: *mut c_void) {
...
// Disable the port
    ...
    val &= ~TOP_SSE;
    writel(val, drv_data.base + SSP_TOP_CTRL);
    drv_data.transfer = core::ptr::null_mut();
...
// Prepare a descriptor for TX or RX DMA
    ...
    {
    let mut addr: phys_addr_t = drv_data.base_addr + SSP_DATAR;
    let mut burst_size: u32 = K1_SPI_THRESH * drv_data.bytes;
...
// Flush the RX FIFO of any leftover data before processing a message
    ...
    {
    struct k1_spi_driver_data *drv_data = spi_controller_get_devdata(host);
    let mut val: u32 = readl(drv_data.base + SSP_STATUS);
    u32 count;
...
#[no_mangle]
unsafe extern "C" fn k1_spi_set_cs(spi: *mut spi_device, high: bool) {
...
unsafe extern "C" fn k1_spi_write_word(drv_data: *mut k1_spi_driver_data) {
    ...
    val = *(u8 *)buf;
#[no_mangle]
pub unsafe extern "C" fn if(2: bytes ==) -> else {
    else if (bytes == 2)
    val = *(u16 *)buf;
    else	/* bytes == 4 */
    val = *(u32 *)buf;
...
#[no_mangle]
unsafe extern "C" fn k1_spi_write(drv_data: *mut k1_spi_driver_data, val: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn k1_spi_read_word(drv_data: *mut k1_spi_driver_data) {
...
#[no_mangle]
unsafe extern "C" fn k1_spi_read(drv_data: *mut k1_spi_driver_data, val: u32) -> bool {
    static bool k1_spi_read(struct k1_spi_driver_data *drv_data, u32 val)
    {
    do {
    let mut count: c_uint = FIELD_GET(SSP_STATUS_RFL, val) + 1;
...
#[no_mangle]
unsafe extern "C" fn k1_spi_ssp_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn k1_spi_dma_cleanup(dev: *mut device, res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn k1_spi_probe(pdev: *mut platform_device) -> c_int {
