...
//! Source: drivers/spi/spi-qpic-snand.c
#![no_std]
...
pub const NAND_FLASH_SPI_CFG: c_uint = 0xc0;
pub const NAND_NUM_ADDR_CYCLES: c_uint = 0xc4;
pub const NAND_BUSY_CHECK_WAIT_CNT: c_uint = 0xc8;
pub const NAND_FLASH_FEATURES: c_uint = 0xf64;
// QSPI NAND config reg bits

pub const CLK_CNTR_INIT_VAL_VEC: c_uint = 0x924;

pub const FEA_STATUS_DEV_ADDR: c_uint = 0xc0;

pub const SPI_NUM_ADDR: c_uint = 0xDA4DB;
pub const SPI_WAIT_CNT: c_uint = 0x10;
pub const QPIC_QSPI_NUM_CS: c_int = 1;

pub const SPINAND_RESET: c_uint = 0xff;
pub const SPINAND_READID: c_uint = 0x9f;
pub const SPINAND_FEATURE_ADDR: c_uint = 0xb0;
pub const SPINAND_GET_FEATURE: c_uint = 0x0f;
pub const SPINAND_SET_FEATURE: c_uint = 0x1f;
pub const SPINAND_READ_CACHE: c_uint = 0x0b;
pub const SPINAND_READ: c_uint = 0x13;
pub const SPINAND_READ_QUAD: c_uint = 0xeb;
pub const SPINAND_READ_MACRONIX: c_uint = 0x6b;
pub const SPINAND_ERASE: c_uint = 0xd8;
pub const SPINAND_WRITE_EN: c_uint = 0x06;
pub const SPINAND_PROGRAM_EXECUTE: c_uint = 0x10;
pub const SPINAND_PROGRAM_LOAD: c_uint = 0x84;
pub const SPINAND_PROGRAM_LOAD_QUAD: c_uint = 0x34;
pub const QUAD_WIDTH: c_uint = 0x4;
pub const ACC_FEATURE: c_uint = 0xe;
pub const BAD_BLOCK_MARKER_SIZE: c_uint = 0x2;
pub const OOB_BUF_SIZE: c_int = 128;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct snandc_read_status {
    pub snandc_flash: __le32,
    pub snandc_buffer: __le32,
    pub snandc_erased_cw: __le32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qpic_spi_nand {
    pub snandc: *mut qcom_nand_controller,
    pub ctlr: *mut spi_controller,
    pub mtd: *mut mtd_info,
    pub iomacro_clk: *mut clk,
    pub ecc: *mut qpic_ecc,
    pub ecc_stats: qcom_ecc_stats,
...
#[no_mangle]
pub unsafe extern "C" fn if(NAND_READ_LOCATION_1: reg ==) -> else {
    else if (reg == NAND_READ_LOCATION_1)
    snandc.regs.read_location1 = locreg_val;
#[no_mangle]
pub unsafe extern "C" fn if(NAND_READ_LOCATION_2: reg ==) -> else {
    else if (reg == NAND_READ_LOCATION_2)
    snandc.regs.read_location2 = locreg_val;
#[no_mangle]
pub unsafe extern "C" fn if(NAND_READ_LOCATION_3: reg ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(NAND_READ_LOCATION_LAST_CW_1: reg ==) -> else {
    else if (reg == NAND_READ_LOCATION_LAST_CW_1)
    snandc.regs.read_location_last1 = locreg_val;
#[no_mangle]
pub unsafe extern "C" fn if(NAND_READ_LOCATION_LAST_CW_2: reg ==) -> else {
    else if (reg == NAND_READ_LOCATION_LAST_CW_2)
    snandc.regs.read_location_last2 = locreg_val;
#[no_mangle]
pub unsafe extern "C" fn if(NAND_READ_LOCATION_LAST_CW_3: reg ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_init(snandc: *mut qcom_nand_controller) -> c_int {
    static int qcom_spi_init(struct qcom_nand_controller *snandc)
    {
    let mut snand_cfg_val: u32 = 0x0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_ecc_init_ctx_pipelined(nand: *mut nand_device) -> c_int {
...
//
    ...
    snandc.qspi.oob_buf = core::ptr::null_mut();
    kfree(ecc_cfg);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn qcom_spi_ecc_cleanup_ctx_pipelined(nand: *mut nand_device) {
...
// helper to configure location register values
    ...
    let mut num_cw: c_int = snandc.qspi.num_cw;
    if (cw == (num_cw - 1))
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_block_erase(snandc: *mut qcom_nand_controller) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_check_raw_flash_errors(snandc: *mut qcom_nand_controller, cw_cnt: c_int) -> c_int {
    static int qcom_spi_check_raw_flash_errors(struct qcom_nand_controller *snandc, int cw_cnt)
    {
    int i;
    qcom_nandc_dev_to_mem(snandc, true);
    for (i = 0; i < cw_cnt; i++) {
    let mut flash: u32 = le32_to_cpu(snandc.reg_read_buf[i]);
    ...
    let mut num_cw: u32 = snandc.qspi.num_cw;
    qcom_clear_bam_transaction(snandc);
    qcom_clear_read_regs(snandc);
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_check_error(snandc: *mut qcom_nand_controller) -> c_int {
    ...
    let mut uncorrectable_cws: c_uint = 0;
...
//
    ...
    {
    struct qpic_ecc *ecc_cfg = snandc.qspi.ecc;
    u8 *data_buf = core::ptr::null_mut(), *oob_buf = core::ptr::null_mut();
    int ret, cw;
    let mut num_cw: u32 = snandc.qspi.num_cw;
...
// oob_buf++ = 0xff;
    ...
    {
    let mut opcode: u32 = op.cmd.opcode;
    let mut transfer_mode: u32 = SPI_TRANSFER_MODE_x1;
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_config_page_write(snandc: *mut qcom_nand_controller) {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_config_cw_write(snandc: *mut qcom_nand_controller) {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_io_op(snandc: *mut qcom_nand_controller, op: *const spi_mem_op) -> c_int {
    static int qcom_spi_io_op(struct qcom_nand_controller *snandc, const struct spi_mem_op *op)
    {
    int ret, val, opcode;
    let mut copy: bool = false, copy_ftr = false;
...
//
    ...
    let mut quad_enabled: bool = !!((u8)val & BIT(0));
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_is_page_op(op: *const spi_mem_op) -> bool {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_supports_op(mem: *mut spi_mem, op: *const spi_mem_op) -> bool {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_exec_op(mem: *mut spi_mem, op: *const spi_mem_op) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qcom_spi_remove(pdev: *mut platform_device) {
