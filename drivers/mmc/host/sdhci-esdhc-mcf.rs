...
//! Source: drivers/mmc/host/sdhci-esdhc-mcf.c
#![no_std]
...
pub const ESDHC_PROCTL_D3CD: c_uint = 0x08;
pub const ESDHC_SYS_CTRL_DTOCV_MASK: c_uint = 0x0f;
pub const ESDHC_DEFAULT_HOST_CONTROL: c_uint = 0x28;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pltfm_mcf_data {
    pub clk_ipg: *mut clk,
    pub clk_ahb: *mut clk,
    pub clk_per: *mut clk,
    pub aside: c_int,
    pub current_bus_width: c_int,
}

#[no_mangle]
pub unsafe extern "C" fn esdhc_mcf_buffer_swap32(buf: *mut u32, len: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_writeb_be(host: *mut sdhci_host, val: u8, reg: c_int) {
    ...
    if (reg == SDHCI_HOST_CONTROL) {
    let mut host_ctrl: u32 = ESDHC_DEFAULT_HOST_CONTROL;
    let mut dma_bits: u8 = (val & SDHCI_CTRL_DMA_MASK) >> 3;
    let mut tmp: u8 = mcf_read8(host.ioaddr + SDHCI_HOST_CONTROL + 1);
    tmp &= ~0x03;
    tmp |= dma_bits;
//
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_writew_be(host: *mut sdhci_host, val: u16, reg: c_int) {
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_writel_be(host: *mut sdhci_host, val: u32, reg: c_int) {
    static void esdhc_mcf_writel_be(struct sdhci_host *host, u32 val, int reg)
    {
    mcf_write32(val, host.ioaddr + reg);
    }
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_readb_be(host: *mut sdhci_host, reg: c_int) -> u8 {
    static u8 esdhc_mcf_readb_be(struct sdhci_host *host, int reg)
    {
    if (reg == SDHCI_HOST_CONTROL) {
    u8 __iomem *base = host.ioaddr + (reg & ~3);
    let mut val: u16 = mcf_read16(base + 2);
    let mut dma_bits: u8 = (val >> 5) & SDHCI_CTRL_DMA_MASK;
    let mut host_ctrl: u8 = val & 0xff;
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_readw_be(host: *mut sdhci_host, reg: c_int) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_readl_be(host: *mut sdhci_host, reg: c_int) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_get_max_timeout_count(host: *mut sdhci_host) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_reset(host: *mut sdhci_host, mask: u8) {
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_pltfm_get_max_clock(host: *mut sdhci_host) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_pltfm_get_min_clock(host: *mut sdhci_host) -> c_uint {
...
//
    ...
    for (i = 0; i < 8; ++i) {
    let mut result: c_int = fesdhc / sdclkfs[i];
    for (q = 1; q < 17; ++q) {
    let mut finale: c_int = result / q;
...
#[no_mangle]
unsafe extern "C" fn esdhc_mcf_pltfm_set_bus_width(host: *mut sdhci_host, width: c_int) {
...
#[no_mangle]
unsafe extern "C" fn sdhci_esdhc_mcf_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sdhci_esdhc_mcf_remove(pdev: *mut platform_device) {
