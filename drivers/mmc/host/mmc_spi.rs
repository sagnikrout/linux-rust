...
//! Source: drivers/mmc/host/mmc_spi.c
#![no_std]
...
//
pub const SPI_TOKEN_SINGLE: c_uint = 0xfe	/* single block r/w, multiblock read */;
pub const SPI_TOKEN_MULTI_WRITE: c_uint = 0xfc	/* multiblock write */;
pub const SPI_TOKEN_STOP_TRAN: c_uint = 0xfd	/* terminate multiblock write */;
pub const MMC_SPI_BLOCKSIZE: c_int = 512;
pub const MMC_SPI_R1B_TIMEOUT_MS: c_int = 3000;
pub const MMC_SPI_INIT_TIMEOUT_MS: c_int = 3000;
...
//
pub const MMC_SPI_BLOCKSATONCE: c_int = 128;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mmc_spi_host {
    pub mmc: *mut mmc_host,
    pub spi: *mut spi_device,
    pub power_mode: c_uchar,
    pub powerup_msecs: u16,
    pub pdata: *mut mmc_spi_platform_data,
// for bulk data transfers
    pub early_status: spi_transfer token, t, crc,,
    pub m: spi_message,
// for status readback
    pub status: spi_transfer,
    pub readback: spi_message,
// buffer used for commands and for message "overhead"
    pub data: *mut scratch,
...
//
#[no_mangle]
pub unsafe extern "C" fn mmc_cs_off(host: *mut mmc_spi_host) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mmc_spi_readbytes(host: *mut mmc_spi_host, len: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mmc_spi_readtoken(host: *mut mmc_spi_host, timeout: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(cmd->resp[0]: R1_SPI_ILLEGAL_COMMAND &) -> else {
    else if (R1_SPI_ILLEGAL_COMMAND & cmd.resp[0])
    value = -ENOSYS; /* Function not implemented */
#[no_mangle]
pub unsafe extern "C" fn if(cmd->resp[0]: R1_SPI_COM_CRC &) -> else {
    else if (R1_SPI_COM_CRC & cmd.resp[0])
    value = -EILSEQ; /* Illegal byte sequence */
#[no_mangle]
pub unsafe extern "C" fn if(R1_SPI_ERASE_RESET: (R1_SPI_ERASE_SEQ |) -> else {
...
//
    u8 *cp = t.rx_buf;
    unsigned int len;
    let mut bitright: c_uint = 8 - bitshift;
    u8 temp;
...
// cp = leftover | (temp >> bitshift);
    ...
    if (host.mmc.use_spi_crc) {
    let mut crc: u16 = crc_itu_t(0, t.rx_buf, t.len);
...
//
    ...
    unsigned		n_sg;
    let mut multiple: bool = (data.blocks > 1);
    let mut write: bool = (data.flags & MMC_DATA_WRITE);
...
//
    ...
    void			*kmap_addr;
    let mut length: unsigned = sg.length;
...
//
    ...
    struct scratch	*scratch = host.data;
    int		tmp;
    let mut statlen: unsigned = sizeof(scratch.status);
...
//
#[no_mangle]
unsafe extern "C" fn mmc_spi_request(mmc: *mut mmc_host, mrq: *mut mmc_request) {
    static void mmc_spi_request(struct mmc_host *mmc, struct mmc_request *mrq)
    {
    struct mmc_spi_host	*host = mmc_priv(mmc);
    let mut status: c_int = -EINVAL;
    let mut crc_retry: c_int = 5;
...
// issue command; then optionally data and stop
    status = mmc_spi_command_send(host, mrq, mrq.cmd, mrq.data != core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn mmc_spi_initsequence(host: *mut mmc_spi_host) {
...
#[no_mangle]
unsafe extern "C" fn mmc_spi_set_ios(mmc: *mut mmc_host, ios: *mut mmc_ios) {
...
//
    ...
    {
    struct mmc_spi_host *host = mmc_priv(mmc);
    let mut delay_msec: u16 = max(host.pdata.detect_delay, (u16)100);
    mmc_detect_change(mmc, msecs_to_jiffies(delay_msec));
    return IRQ_HANDLED;
    }
#[no_mangle]
unsafe extern "C" fn mmc_spi_probe(spi: *mut spi_device) -> c_int {
...
//
    status = mmc_gpiod_request_cd(mmc, core::ptr::null_mut(), 0, false, 1000);
...
// Index 1 is write protect/read only
    status = mmc_gpiod_request_ro(mmc, core::ptr::null_mut(), 1, 0);
    if (status == -EPROBE_DEFER)
    goto fail_gpiod_request;
    if (!status)
    has_ro = true;
...
#[no_mangle]
unsafe extern "C" fn mmc_spi_remove(spi: *mut spi_device) {
