...
//! Source: drivers/gpio/gpio-mpsse.c
#![no_std]
...
pub struct mpsse_priv {
    pub gpio: gpio_chip,
    pub /: *mut *mut *mut usb_device udev; / USB device encompassing all MPSSEs,
    pub /: *mut *mut *mut usb_interface intf; / USB interface for this MPSSE,
    pub /: *mut *mut u8 intf_id; / USB interface number for this MPSSE,
    pub /: *mut *mut list_head workers; / polling work threads,
    pub /: *mut *mut mutex irq_mutex; / lock over irq_data,
    pub /: *mut *mut mutex irq_race; / race for polling worker teardown,
    pub /: *mut *mut raw_spinlock_t irq_spin; / protects worker list,
    pub /: *mut *mut atomic_t irq_type[16]; / pin -> edge detection type,
    pub irq_enabled: core::sync::atomic::AtomicI32,
    pub id: c_int,
    pub /: *mut *mut u8 gpio_outputs[2]; / Output states for GPIOs [L, H],
    pub /: *mut *mut u8 gpio_dir[2]; / Directions for GPIOs [L, H],
    pub /: *mut *mut unsigned long dir_in; / Bitmask of valid input pins,
    pub /: *mut *mut unsigned long dir_out; / Bitmask of valid output pins,
    pub /: *mut *mut *mut u8 bulk_in_buf; / Extra recv buffer to grab status bytes,
    pub bulk_in: *mut usb_endpoint_descriptor,
    pub bulk_out: *mut usb_endpoint_descriptor,
    pub /: *mut *mut mutex io_mutex; / sync I/O with disconnect,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mpsse_worker {
    pub priv: *mut mpsse_priv,
    pub work: work_struct,
    pub cancelled: core::sync::atomic::AtomicI32,
    pub /: *mut *mut list_head list; / linked list,
    pub /: *mut *mut list_head destroy; / teardown linked list,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct bulk_desc {
    pub /: *mut *mut bool tx; / direction of bulk transfer,
    pub /: *mut *mut *mut u8 data; / input (tx) or output (rx),
    pub /: *mut *mut int len; / Length of `data` if tx, or length of,
// Data to read if rx
    pub /: *mut *mut int len_actual; / Length successfully transferred,
    pub timeout: c_int,
}

pub const MPSSE_NGPIO: c_int = 16;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mpsse_quirk {
    pub /: *const *const *const char names[MPSSE_NGPIO]; / Pin names, if applicable,
    pub /: *mut *mut unsigned long dir_in; / Bitmask of valid input pins,
    pub /: *mut *mut unsigned long dir_out; / Bitmask of valid output pins,
}
...
// MPSSE commands
pub const SET_BITS_CMD: c_uint = 0x80;
pub const GET_BITS_CMD: c_uint = 0x81;
pub const SET_BITMODE_REQUEST: c_uint = 0x0B;

pub const MODE_RESET: c_int = 0;
// Arbitrarily decided. This could probably be much less
pub const MPSSE_WRITE_TIMEOUT: c_int = 5000;
pub const MPSSE_READ_TIMEOUT: c_int = 5000;
// 1 millisecond, also pretty arbitrary
pub const MPSSE_POLL_INTERVAL: c_int = 1000;
#[no_mangle]
unsafe extern "C" fn mpsse_bulk_xfer(intf: *mut usb_interface, desc: *mut bulk_desc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mpsse_read(intf: *mut usb_interface, buf: *mut u8, len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_set_bank(priv: *mut mpsse_priv, bank: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_get_bank(priv: *mut mpsse_priv, bank: u8) -> c_int {
    static int gpio_mpsse_get_bank(struct mpsse_priv *priv, u8 bank)
    {
    int ret;
    let mut buf: u8 = GET_BITS_CMD | (bank << 1);
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_gpio_get(chip: *mut gpio_chip, offset: c_uint) -> c_int {
    static int gpio_mpsse_gpio_get(struct gpio_chip *chip, unsigned int offset)
    {
    int err;
    let mut mask: c_ulong = 0, bits = 0;
...
// == is not guaranteed to give 1 if true
    ...
    int ret;
    struct mpsse_priv *priv = gpiochip_get_data(chip);
    let mut bank: c_int = (offset & 8) >> 3;
    let mut bank_offset: c_int = offset & 7;
...
//
    ...
    let mut destructors: list_head = LIST_HEAD_INIT(destructors);
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_poll(my_work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_set_irq_type(irqd: *mut irq_data, type: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_irq_disable(irqd: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_irq_enable(irqd: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_ida_remove(data: *mut c_void) {
    ...
    {
    struct mpsse_priv *priv = gpiochip_get_data(chip);
    if (WARN_ON(priv == core::ptr::null_mut()))
    return -ENODEV;
...
// Reset mode, needed to correctly enter MPSSE mode
    ...
    MODE_RESET, priv.intf_id + 1, core::ptr::null_mut(), 0,
...
// Enter MPSSE mode
    ...
    SET_BITMODE_REQUEST,
    USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_DIR_OUT,
    MODE_MPSSE, priv.intf_id + 1, core::ptr::null_mut(), 0,
    USB_CTRL_SET_TIMEOUT);
    if (err)
    return err;
    gpio_irq_chip_set_chip(&priv.gpio.irq, &gpio_mpsse_irq_chip);
    priv.gpio.irq.parent_handler = core::ptr::null_mut();
    priv.gpio.irq.num_parents = 0;
    priv.gpio.irq.parents = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn gpio_mpsse_disconnect(intf: *mut usb_interface) {
...
//
    ...
    gpio_mpsse_stop_all_except(priv, core::ptr::null_mut());
    priv.intf = core::ptr::null_mut();
