...
//! Source: drivers/gpio/gpio-xtensa.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn enable_cp(cpenable: *mut c_ulong) -> c_ulong {
...
#[no_mangle]
pub unsafe extern "C" fn disable_cp(flags: c_ulong, cpenable: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn xtensa_impwire_get_direction(gc: *mut gpio_chip, offset: unsigned) -> c_int {
    static int xtensa_impwire_get_direction(struct gpio_chip *gc, unsigned offset)
    {
    return GPIO_LINE_DIRECTION_IN; /* input only */
    }
#[no_mangle]
unsafe extern "C" fn xtensa_impwire_get_value(gc: *mut gpio_chip, offset: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xtensa_expstate_get_direction(gc: *mut gpio_chip, offset: unsigned) -> c_int {
    static int xtensa_expstate_get_direction(struct gpio_chip *gc, unsigned offset)
    {
    return GPIO_LINE_DIRECTION_OUT; /* output only */
    }
#[no_mangle]
unsafe extern "C" fn xtensa_expstate_get_value(gc: *mut gpio_chip, offset: unsigned) -> c_int {
    ...
    {
    unsigned long flags, saved_cpenable;
    let mut mask: u32 = BIT(offset);
    let mut val: u32 = value ? BIT(offset) : 0;
    flags = enable_cp(&saved_cpenable);
...
#[no_mangle]
unsafe extern "C" fn xtensa_gpio_probe(pdev: *mut platform_device) -> c_int {
    static int xtensa_gpio_probe(struct platform_device *pdev)
    {
    int ret;
    ret = gpiochip_add_data(&impwire_chip, core::ptr::null_mut());
    if (ret)
    return ret;
    return gpiochip_add_data(&expstate_chip, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn xtensa_gpio_init() -> int __init {
    static int __init xtensa_gpio_init(void)
    {
    struct platform_device *pdev;
    pdev = platform_device_register_simple("xtensa-gpio", 0, core::ptr::null_mut(), 0);
    if (IS_ERR(pdev))
    return PTR_ERR(pdev);
