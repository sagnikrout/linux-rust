...
//! Source: drivers/gpio/gpio-usbio.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbio_gpio_bank {
    pub config: [u8; USBIO_GPIOSPERBANK],
    pub bitmap: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbio_gpio {
    pub /: *mut *mut mutex config_mutex; / Protects banks[x].config,
    pub banks: [usbio_gpio_bank; USBIO_MAX_GPIOBANKS],
    pub gc: gpio_chip,
...
unsafe extern "C" fn usbio_gpio_set(gc: *mut gpio_chip, offset: c_uint, value: c_int) -> c_int {
    ...
    &gbuf, sizeof(gbuf), core::ptr::null_mut(), 0);
    }
