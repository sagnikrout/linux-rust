...
//! Source: drivers/gpio/gpio-realtek-otto.c
#![no_std]
...
//
pub const REALTEK_GPIO_REG_CNR: c_uint = 0x00;
// Clear bit (0) for input, set bit (1) for output
pub const REALTEK_GPIO_REG_DIR: c_uint = 0x08;
pub const REALTEK_GPIO_REG_DATA: c_uint = 0x0C;
// Read bit for IRQ status, write 1 to clear IRQ
pub const REALTEK_GPIO_REG_ISR: c_uint = 0x10;
// Two bits per GPIO in IMR registers
pub const REALTEK_GPIO_REG_IMR: c_uint = 0x14;
pub const REALTEK_GPIO_REG_IMR_AB: c_uint = 0x14;
pub const REALTEK_GPIO_REG_IMR_CD: c_uint = 0x18;

pub const REALTEK_GPIO_IRQ_EDGE_FALLING: c_int = 1;
pub const REALTEK_GPIO_IRQ_EDGE_RISING: c_int = 2;
pub const REALTEK_GPIO_IRQ_EDGE_BOTH: c_int = 3;
pub const REALTEK_GPIO_MAX: c_int = 32;
pub const REALTEK_GPIO_PORTS_PER_BANK: c_int = 4;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct realtek_gpio_ctrl {
    pub chip: gpio_generic_chip,
    pub base: *mut void __iomem,
    pub cpumask_base: *mut void __iomem,
    pub cpu_irq_maskable: cpumask,
    pub lock: raw_spinlock_t,
    pub intr_mask: [u8; REALTEK_GPIO_MAX],
    pub intr_type: [u8; REALTEK_GPIO_MAX],
    pub reg): *mut *mut u32 (bank_read)(void __iomem,
    pub value): *mut *mut *mut void (bank_write)(void __iomem reg, u32,
    pub line): *mut *mut unsigned int (line_imr_pos)(unsigned int,
}
...
//
#[no_mangle]
unsafe extern "C" fn realtek_gpio_bank_read_swapped(reg: *mut void __iomem) -> u32 {
    static u32 realtek_gpio_bank_read_swapped(void __iomem *reg)
    {
    return ioread32be(reg);
    }
#[no_mangle]
unsafe extern "C" fn realtek_gpio_bank_write_swapped(reg: *mut void __iomem, value: u32) {
    static void realtek_gpio_bank_write_swapped(void __iomem *reg, u32 value)
    {
    iowrite32be(value, reg);
    }
#[no_mangle]
unsafe extern "C" fn realtek_gpio_line_imr_pos_swapped(line: c_uint) -> c_uint {
    static unsigned int realtek_gpio_line_imr_pos_swapped(unsigned int line)
    {
    let mut port_pin: c_uint = line % 8;
    let mut port: c_uint = line / 8;
...
//
#[no_mangle]
unsafe extern "C" fn realtek_gpio_bank_read(reg: *mut void __iomem) -> u32 {
    static u32 realtek_gpio_bank_read(void __iomem *reg)
    {
    return ioread32(reg);
    }
#[no_mangle]
unsafe extern "C" fn realtek_gpio_bank_write(reg: *mut void __iomem, value: u32) {
    static void realtek_gpio_bank_write(void __iomem *reg, u32 value)
    {
    iowrite32(value, reg);
    }
#[no_mangle]
unsafe extern "C" fn realtek_gpio_line_imr_pos(line: c_uint) -> c_uint {
    static unsigned int realtek_gpio_line_imr_pos(unsigned int line)
    {
    return 2 * line;
    }
#[no_mangle]
unsafe extern "C" fn realtek_gpio_clear_isr(ctrl: *mut realtek_gpio_ctrl, mask: u32) {
...
#[no_mangle]
unsafe extern "C" fn realtek_gpio_read_isr(ctrl: *mut realtek_gpio_ctrl) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn realtek_gpio_update_line_imr(ctrl: *mut realtek_gpio_ctrl, line: c_uint) {
    ...
    let mut line_shift: c_uint = ctrl.line_imr_pos(line);
    let mut shift: c_uint = line_shift % 32;
    let mut irq_type: u32 = ctrl.intr_type[line];
    let mut irq_mask: u32 = ctrl.intr_mask[line];
    u32 reg_val;
    reg += 4 * (line_shift / 32);
...
unsafe extern "C" fn realtek_gpio_irq_ack(data: *mut irq_data) {
    ...
    let mut line: irq_hw_number_t = irqd_to_hwirq(data);
...
unsafe extern "C" fn realtek_gpio_irq_unmask(data: *mut irq_data) {
    ...
    {
    struct realtek_gpio_ctrl *ctrl = irq_data_to_ctrl(data);
    let mut line: c_uint = irqd_to_hwirq(data);
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn realtek_gpio_irq_set_type(data: *mut irq_data, flow_type: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn realtek_gpio_irq_init(gc: *mut gpio_chip) -> c_int {
    static int realtek_gpio_irq_init(struct gpio_chip *gc)
    {
    struct realtek_gpio_ctrl *ctrl = gpiochip_get_data(gc);
    let mut mask_all: u32 = GENMASK(gc.ngpio - 1, 0);
    unsigned int line;
    int cpu;
...
#[no_mangle]
unsafe extern "C" fn realtek_gpio_probe(pdev: *mut platform_device) -> c_int {
