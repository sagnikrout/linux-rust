...
//! Source: drivers/gpio/gpio-pcie-idio-24.c
#![no_std]
...
//
pub const PLX_PEX8311_PCI_LCS_INTCSR: c_uint = 0x68;

pub const IDIO_24_OUT_BASE: c_uint = 0x0;
pub const IDIO_24_TTLCMOS_OUT_REG: c_uint = 0x3;
pub const IDIO_24_IN_BASE: c_uint = 0x4;
pub const IDIO_24_TTLCMOS_IN_REG: c_uint = 0x7;
pub const IDIO_24_COS_STATUS_BASE: c_uint = 0x8;
pub const IDIO_24_CONTROL_REG: c_uint = 0xC;
pub const IDIO_24_COS_ENABLE: c_uint = 0xE;
pub const IDIO_24_SOFT_RESET: c_uint = 0xF;
...
pub const IDIO_24_NGPIO_PER_REG: c_int = 8;
...
pub struct idio_24_gpio {
    ...
    {
    let mut type_mask: c_uint = COS_ENABLE_BOTH << index;
    let mut idio24gpio: *mut idio_24_gpio const = irq_drv_data;
    u8 type;
    int ret;
...
// if all are masked, then disable interrupts, else set to type
    ...
    {
    let mut offset: c_uint = irq_data.reg_offset;
    let mut rising: c_uint = COS_ENABLE_RISING << offset;
    let mut falling: c_uint = COS_ENABLE_FALLING << offset;
    let mut mask: c_uint = COS_ENABLE_BOTH << offset;
    let mut idio24gpio: *mut idio_24_gpio const = irq_drv_data;
    unsigned int new;
...
// if COS is currently enabled then update the edge type
    ...
    {
    let mut out_stride: c_uint = offset / IDIO_24_NGPIO_PER_REG;
    let mut in_stride: c_uint = (offset - 24) / IDIO_24_NGPIO_PER_REG;
    let mut map: *mut regmap const = gpio_regmap_get_drvdata(gpio);
    int err;
...
pub const IDIO_24_NGPIO: c_int = 56;
...
#[no_mangle]
unsafe extern "C" fn idio_24_probe(pdev: *mut pci_dev, id: *const pci_device_id) -> c_int {
    static int idio_24_probe(struct pci_dev *pdev, const struct pci_device_id *id)
    {
    let mut dev: *mut device const = &pdev.dev;
    struct idio_24_gpio *idio24gpio;
    int err;
    let mut pci_plx_bar_index: usize = 1;
    let mut pci_bar_index: usize = 2;
    let mut name: *const char const = pci_name(pdev);
