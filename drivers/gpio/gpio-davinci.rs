...
//! Source: drivers/gpio/gpio-davinci.c
#![no_std]
...
pub const MAX_REGS_BANKS: c_int = 5;
pub const MAX_INT_PER_BANK: c_int = 32;
#[repr(C)]
#[derive(Copy, Clone)]
...
pub const BINTEN: c_uint = 0x8 /* GPIO Interrupt Per-Bank Enable Register */;
...
// Serialize access to GPIO registers
    pub lock: spinlock_t,
    pub regs: [*mut void __iomem; MAX_REGS_BANKS],
    pub gpio_unbanked: c_int,
    pub irqs: [c_int; MAX_INT_PER_BANK],
    pub context: [davinci_gpio_regs; MAX_REGS_BANKS],
    pub binten_context: u32,
}

#[no_mangle]
pub unsafe extern "C" fn __gpio_mask(gpio: unsigned) -> u32 {
...
// board setup code *MUST* setup pinmux and enable the GPIO clock.
    ...
    unsigned long flags;
    u32 temp;
    let mut bank: c_int = offset / 32;
    let mut mask: u32 = __gpio_mask(offset);
    g = d.regs[bank];
...
#[no_mangle]
unsafe extern "C" fn davinci_direction_in(chip: *mut gpio_chip, offset: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn davinci_get_direction(chip: *mut gpio_chip, offset: c_uint) -> c_int {
    static int davinci_get_direction(struct gpio_chip *chip, unsigned int offset)
    {
    struct davinci_gpio_controller *d = gpiochip_get_data(chip);
    struct davinci_gpio_regs __iomem *g;
    let mut mask: u32 = __gpio_mask(offset), val;
    let mut bank: c_int = offset / 32;
    g = d.regs[bank];
...
//
#[no_mangle]
unsafe extern "C" fn davinci_gpio_get(chip: *mut gpio_chip, offset: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn davinci_gpio_probe(pdev: *mut platform_device) -> c_int {
...
unsafe extern "C" fn gpio_irq_mask(d: *mut irq_data) {
    ...
    let mut mask: uintptr_t = (uintptr_t)irq_data_get_irq_handler_data(d);
...
unsafe extern "C" fn gpio_irq_unmask(d: *mut irq_data) {
    ...
    struct davinci_gpio_regs __iomem *g = chips.regs[hwirq / 32];
    let mut mask: uintptr_t = (uintptr_t)irq_data_get_irq_handler_data(d);
    let mut status: unsigned = irqd_get_trigger_type(d);
...
#[no_mangle]
unsafe extern "C" fn gpio_irq_type(d: *mut irq_data, trigger: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpio_to_irq_banked(chip: *mut gpio_chip, offset: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpio_to_irq_unbanked(chip: *mut gpio_chip, offset: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gpio_irq_type_unbanked(data: *mut irq_data, trigger: unsigned) -> c_int {
    ...
    irq_set_chip_data(irq, ( void *)chips);
...
//
#[no_mangle]
unsafe extern "C" fn davinci_gpio_irq_setup(pdev: *mut platform_device) -> c_int {
    static int davinci_gpio_irq_setup(struct platform_device *pdev)
    {
    unsigned	gpio, bank;
    int		irq;
    struct clk	*clk;
    let mut binten: u32 = 0;
    unsigned	ngpio;
    struct device *dev = &pdev.dev;
    struct davinci_gpio_controller *chips = platform_get_drvdata(pdev);
    struct davinci_gpio_regs __iomem *g;
    struct irq_domain	*irq_domain = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn davinci_gpio_suspend(dev: *mut device) -> c_int {
    static int davinci_gpio_suspend(struct device *dev)
    {
    struct davinci_gpio_controller *chips = dev_get_drvdata(dev);
    let mut nbank: u32 = DIV_ROUND_UP(chips.chip.ngpio, 32);
    davinci_gpio_save_context(chips, nbank);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn davinci_gpio_resume(dev: *mut device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn davinci_gpio_drv_reg() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn davinci_gpio_exit() -> void __exit {
