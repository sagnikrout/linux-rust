...
//! Source: drivers/gpio/gpio-max3191x.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct max3191x_chip {
    pub gpio: gpio_chip,
    pub lock: mutex,
    pub nchips: u32,
    pub mode: enum max3191x_mode,
    pub modesel_pins: *mut gpio_descs,
    pub fault_pins: *mut gpio_descs,
    pub db0_pins: *mut gpio_descs,
    pub db1_pins: *mut gpio_descs,
    pub mesg: spi_message,
    pub xfer: spi_transfer,
    pub crc_error: *mut c_ulong,
    pub overtemp: *mut c_ulong,
    pub undervolt1: *mut c_ulong,
    pub undervolt2: *mut c_ulong,
    pub fault: *mut c_ulong,
    pub ignore_uv: bool,
}

pub const MAX3191X_NGPIO: c_int = 8;
pub const MAX3191X_CRC8_POLYNOMIAL: c_uint = 0xa8 /* (x^5) + x^4 + x^2 + x^0 */;
    DECLARE_CRC8_TABLE(max3191x_crc8);
#[no_mangle]
unsafe extern "C" fn max3191x_get_direction(gpio: *mut gpio_chip, offset: c_uint) -> c_int {
    static int max3191x_get_direction(struct gpio_chip *gpio, unsigned int offset)
    {
    return GPIO_LINE_DIRECTION_IN; /* always in */
    }
#[no_mangle]
unsafe extern "C" fn max3191x_direction_input(gpio: *mut gpio_chip, offset: c_uint) -> c_int {
    static int max3191x_direction_input(struct gpio_chip *gpio, unsigned int offset)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn max3191x_wordlen(max3191x: *mut max3191x_chip) -> c_uint {
    static unsigned int max3191x_wordlen(struct max3191x_chip *max3191x)
    {
    return max3191x.mode == STATUS_BYTE_ENABLED ? 2 : 1;
    }
#[no_mangle]
unsafe extern "C" fn max3191x_readout_locked(max3191x: *mut max3191x_chip) -> c_int {
    ...
    let mut in: u8 = ((u8 *)max3191x.xfer.rx_buf)[i * 2];
    let mut status: u8 = ((u8 *)max3191x.xfer.rx_buf)[i * 2 + 1];
...
#[no_mangle]
unsafe extern "C" fn max3191x_get(gpio: *mut gpio_chip, offset: c_uint) -> c_int {
    ...
    {
    struct max3191x_chip *max3191x = gpiochip_get_data(gpio);
    let mut wordlen: c_uint = max3191x_wordlen(max3191x);
    int ret;
    unsigned long bit;
    ...
    let mut chipnum: c_uint = bit / MAX3191X_NGPIO;
    if (max3191x_chip_is_faulting(max3191x, chipnum)) {
    ret = -EIO;
    goto out_unlock;
    }
    ...
    let mut found: c_int = gpiod_count(dev, con_id);
    if (found == -ENOENT)
...
#[no_mangle]
unsafe extern "C" fn max3191x_probe(spi: *mut spi_device) -> c_int {
    ...
    max3191x.db0_pins = core::ptr::null_mut();
    max3191x.db1_pins = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn max3191x_remove(spi: *mut spi_device) {
...
#[no_mangle]
unsafe extern "C" fn max3191x_register_driver(sdrv: *mut spi_driver) -> int __init {
