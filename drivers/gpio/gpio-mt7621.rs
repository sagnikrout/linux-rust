...
//! Source: drivers/gpio/gpio-mt7621.c
#![no_std]
...
pub const MTK_BANK_CNT: c_int = 3;
pub const MTK_BANK_WIDTH: c_int = 32;
pub const GPIO_BANK_STRIDE: c_uint = 0x04;
pub const GPIO_REG_CTRL: c_uint = 0x00;
pub const GPIO_REG_POL: c_uint = 0x10;
pub const GPIO_REG_DATA: c_uint = 0x20;
pub const GPIO_REG_DSET: c_uint = 0x30;
pub const GPIO_REG_DCLR: c_uint = 0x40;
pub const GPIO_REG_REDGE: c_uint = 0x50;
pub const GPIO_REG_FEDGE: c_uint = 0x60;
pub const GPIO_REG_HLVL: c_uint = 0x70;
pub const GPIO_REG_LLVL: c_uint = 0x80;
pub const GPIO_REG_STAT: c_uint = 0x90;
pub const GPIO_REG_EDGE: c_uint = 0xA0;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_gc {
    pub chip: gpio_generic_chip,
    pub parent_priv: *mut mtk,
    pub bank: c_int,
    pub rising: u32,
    pub falling: u32,
    pub hlevel: u32,
...
pub struct mtk {
    ...
    pub gpio_irq: c_int,
    pub num_gpios: c_int,
    pub gc_map: [mtk_gc; MTK_BANK_CNT],
}
    ...
    let mut mask: u32 = mt7621_gpio_hwirq_to_offset(d.hwirq, rg);
    ...
    struct gpio_chip *gc = irq_data_get_irq_chip_data(d);
    struct mtk_gc *rg = gpiochip_get_data(gc);
    let mut mask: u32 = BIT(mt7621_gpio_hwirq_to_offset(d.hwirq, rg));
    guard(gpio_generic_lock_irqsave)(&rg.chip);
    ...
    struct gpio_chip *gc = irq_data_get_irq_chip_data(d);
    struct mtk_gc *rg = gpiochip_get_data(gc);
    let mut irq: c_uint = mt7621_gpio_hwirq_to_offset(d.hwirq, rg);
    ...
    {
    let mut gpio: c_int = spec.args[0];
...
// flags = spec->args[1];
    ...
    int offset, virq;
    if (priv.gpio_irq > 0)
    irq_set_chained_handler_and_data(priv.gpio_irq, core::ptr::null_mut(), core::ptr::null_mut());
