...
#[no_mangle]
pub unsafe extern "C" fn __printf(_arg: 1, fmt: *const 2) __cold void vringh_bad(char, ...) -> static {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_kiov_advance(iov: *mut vringh_kiov, len: usize) {
    void vringh_kiov_advance(struct vringh_kiov *iov, size_t len)
    {
    while (len && iov.i < iov.used) {
    let mut partlen: usize = min(iov.iov[iov.i].iov_len, len);
...
#[no_mangle]
unsafe extern "C" fn resize_iovec(iov: *mut vringh_kiov, gfp: gfp_t) -> c_int {
...
// desc_max = vrh->vring.num;
    ...
    let mut range: vringh_range = { -1ULL, 0 }, slowrange;
...
// You must want something!
    ...
    let mut a: u64 = vringh64_to_cpu(vrh, desc.addr);
...
// Compiler knows num_used == 1 sometimes, hence extra check
    ...
    let mut part: u16 = vrh.vring.num - off;
...
// notification anyway).
    let mut avail: return = = vrh.last_avail_idx;
    }
...
#[no_mangle]
pub unsafe extern "C" fn getu16_user(vrh: *const vringh, val: *mut u16, p: *const __virtio16) -> c_int {
    static inline int getu16_user(const struct vringh *vrh, u16 *val, const __virtio16 *p)
    {
    let mut v: __virtio16 = 0;
    let mut rc: c_int = get_user(v, ( __virtio16 __user *)p);
// val = vringh16_to_cpu(vrh, v);
    return rc;
    }
#[no_mangle]
pub unsafe extern "C" fn putu16_user(vrh: *const vringh, p: *mut __virtio16, val: u16) -> c_int {
    static inline int putu16_user(const struct vringh *vrh, __virtio16 *p, u16 val)
    {
    let mut v: __virtio16 = cpu_to_vringh16(vrh, val);
    return put_user(v, ( __virtio16 __user *)p);
    }
    ...
    unsigned int num)
    {
    return copy_to_user(( void __user *)dst, src,
...
// vring expects kernel addresses, but only used via accessors.
    vrh.vring.desc = ( struct vring_desc *)desc;
    vrh.vring.avail = ( struct vring_avail *)avail;
    vrh.vring.used = ( struct vring_used *)used;
    return 0;
    }
...
// We need the layouts to be the identical for this to work
    ...
    BUILD_BUG_ON(sizeof(((struct iovec *)core::ptr::null_mut()).iov_base)
    != sizeof(((struct kvec *)core::ptr::null_mut()).iov_base));
    BUILD_BUG_ON(sizeof(((struct iovec *)core::ptr::null_mut()).iov_len)
    != sizeof(((struct kvec *)core::ptr::null_mut()).iov_len));
// head = err;
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_iov_pull_user(riov: *mut vringh_iov, dst: *mut c_void, len: usize) -> isize {
    ssize_t vringh_iov_pull_user(struct vringh_iov *riov, void *dst, size_t len)
    {
    return vringh_iov_xfer(core::ptr::null_mut(), (struct vringh_kiov *)riov,
...
//
    ...
    {
    return vringh_iov_xfer(core::ptr::null_mut(), (struct vringh_kiov *)wiov,
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_complete_user(vrh: *mut vringh, head: u16, len: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_notify_enable_user(vrh: *mut vringh) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_notify_disable_user(vrh: *mut vringh) {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_need_notify_user(vrh: *mut vringh) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn putu16_kern(vrh: *const vringh, p: *mut __virtio16, val: u16) -> c_int {
...
// Empty...
    ...
    return 0;
// head = err;
    err = __vringh_iov(vrh, *head, riov, wiov, no_range_check, core::ptr::null_mut(),
    gfp, copydesc_kern);
    if (err)
    return err;
    return 1;
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_complete_kern(vrh: *mut vringh, head: u16, len: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_notify_enable_kern(vrh: *mut vringh) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_notify_disable_kern(vrh: *mut vringh) {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_need_notify_kern(vrh: *mut vringh) -> c_int {
...
pub struct iotlb_vec {
    ...
    let mut ret: c_int = 0;
    let mut s: u64 = 0, last = addr + len - 1;
    ...
    let mut pfn: u64 = io_addr >> PAGE_SHIFT;
...
pub const IOTLB_IOV_STRIDE: c_int = 16;
...
// Atomic read is needed for getu16
    ...
    core::ptr::null_mut(), &ivec, VHOST_MAP_RO);
    if (ret < 0)
    return ret;
    if (vrh.use_va) {
...
// Atomic write is needed for putu16
    ...
    unsigned int num)
    {
    let mut size: c_int = num * sizeof(*dst);
    int ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_complete_iotlb(vrh: *mut vringh, head: u16, len: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn vringh_need_notify_iotlb(vrh: *mut vringh) -> c_int {
