...
//! Source: drivers/dma-buf/heaps/system_heap.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct system_heap_priv {
    pub cc_shared: bool,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct system_heap_buffer {
...
#[no_mangle]
unsafe extern "C" fn system_heap_set_page_decrypted(page: *mut page) -> c_int {
    static int system_heap_set_page_decrypted(struct page *page)
    {
    let mut addr: c_ulong = (unsigned long)page_address(page);
    let mut nr_pages: c_uint = 1 << compound_order(page);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn system_heap_set_page_encrypted(page: *mut page) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dup_sg_table(from: *mut sg_table, to: *mut sg_table) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn system_heap_mmap(dmabuf: *mut dma_buf, vma: *mut vm_area_struct) -> c_int {
    static int system_heap_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
    {
    struct system_heap_buffer *buffer = dmabuf.priv;
    struct sg_table *table = &buffer.sg_table;
    let mut addr: c_ulong = vma.vm_start;
    let mut pgoff: c_ulong = vma.vm_pgoff;
    ...
    let mut n: c_ulong = sg.length >> PAGE_SHIFT;
    if (pgoff < n)
    break;
    pgoff -= n;
    }
    for (; sg && addr < vma.vm_end; sg = sg_next(sg)) {
    let mut n: c_ulong = (sg.length >> PAGE_SHIFT) - pgoff;
    struct page *page = sg_page(sg) + pgoff;
    let mut size: c_ulong = n << PAGE_SHIFT;
    ...
    {
    struct sg_table *table = &buffer.sg_table;
    let mut npages: c_int = PAGE_ALIGN(buffer.len) / PAGE_SIZE;
...
#[no_mangle]
unsafe extern "C" fn system_heap_vmap(dmabuf: *mut dma_buf, map: *mut iosys_map) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn system_heap_vunmap(dmabuf: *mut dma_buf, map: *mut iosys_map) {
...
#[no_mangle]
unsafe extern "C" fn system_heap_dma_buf_release(dmabuf: *mut dma_buf) {
...
//
    ...
    let mut size_remaining: c_ulong = len;
    let mut max_order: c_uint = orders[0];
    struct system_heap_priv *priv = dma_heap_get_drvdata(heap);
    let mut cc_shared: bool = priv.cc_shared;
...
#[no_mangle]
unsafe extern "C" fn system_heap_create() -> int __init {
