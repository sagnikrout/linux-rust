...
//! Source: drivers/dma-buf/st-dma-resv.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dma_resv_usage_param {
    pub usage: enum dma_resv_usage,
...
unsafe extern "C" fn test_sanitycheck(test: *mut kunit) {
    ...
    dma_fence_put(f);
    dma_resv_init(&resv);
    r = dma_resv_lock(&resv, core::ptr::null_mut());
    if (r)
...
unsafe extern "C" fn test_signaling(test: *mut kunit) {
    ...
    let mut usage: enum dma_resv_usage = param.usage;
    struct dma_resv resv;
    struct dma_fence *f;
    int r;
    f = alloc_fence();
...
#[no_mangle]
unsafe extern "C" fn test_for_each(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_for_each_unlocked(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_get_fences(test: *mut kunit) {
    static void test_get_fences(struct kunit *test)
    {
    const struct dma_resv_usage_param *param = test.param_value;
    let mut usage: enum dma_resv_usage = param.usage;
    struct dma_fence *f, **fences = core::ptr::null_mut();
    struct dma_resv resv;
    int r, i;
    f = alloc_fence();
    KUNIT_ASSERT_NOT_NULL(test, f);
