...
//! Source: drivers/i2c/i2c-slave-eeprom.c
#![no_std]
...
pub struct eeprom_data {
    pub bin: bin_attribute,
    pub buffer_lock: spinlock_t,
    pub buffer_idx: u16,
    pub address_mask: u16,
    pub num_address_bytes: u8,
    pub idx_write_cnt: u8,
    pub read_only: bool,
    pub buffer: [u8; ],
}
...
//
    ...
    let mut ret: c_int = device_property_read_string(&client.dev, "firmware-name", &eeprom_data);
    if (!ret) {
...
#[no_mangle]
unsafe extern "C" fn i2c_slave_eeprom_probe(client: *mut i2c_client) -> c_int {
    static int i2c_slave_eeprom_probe(struct i2c_client *client)
    {
    const struct i2c_device_id *id = i2c_client_get_device_id(client);
    struct eeprom_data *eeprom;
    int ret;
    let mut size: c_uint = FIELD_GET(I2C_SLAVE_BYTELEN, id.driver_data) + 1;
    let mut flag_addr16: c_uint = FIELD_GET(I2C_SLAVE_FLAG_ADDR16, id.driver_data);
...
#[no_mangle]
unsafe extern "C" fn i2c_slave_eeprom_remove(client: *mut i2c_client) {
