...
//! Source: drivers/i2c/busses/i2c-cbus-gpio.c
#![no_std]
...
//
pub const CBUS_ADDR_BITS: c_int = 3;
pub const CBUS_REG_BITS: c_int = 5;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cbus_host {
    pub /: *mut *mut spinlock_t lock; / host lock,
    pub dev: *mut device,
    pub clk: *mut gpio_desc,
    pub dat: *mut gpio_desc,
    pub sel: *mut gpio_desc,
}
...
//
#[no_mangle]
unsafe extern "C" fn cbus_send_bit(host: *mut cbus_host, bit: unsigned) {
...
//
#[no_mangle]
unsafe extern "C" fn cbus_send_data(host: *mut cbus_host, data: unsigned, len: unsigned) {
...
//
#[no_mangle]
unsafe extern "C" fn cbus_receive_bit(host: *mut cbus_host) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn cbus_receive_word(host: *mut cbus_host) -> c_int {
    static int cbus_receive_word(struct cbus_host *host)
    {
    let mut ret: c_int = 0;
    int i;
    for (i = 16; i > 0; i--) {
    let mut bit: c_int = cbus_receive_bit(host);
    if (bit < 0)
    return bit;
    if (bit)
    ret |= 1 << (i - 1);
...
#[no_mangle]
unsafe extern "C" fn cbus_i2c_func(adapter: *mut i2c_adapter) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn cbus_i2c_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn cbus_i2c_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (!chost)
    return -ENOMEM;
    if (gpiod_count(&pdev.dev, core::ptr::null_mut()) != 3)
    return -ENODEV;
    chost.clk = devm_gpiod_get_index(&pdev.dev, core::ptr::null_mut(), 0, GPIOD_OUT_LOW);
    if (IS_ERR(chost.clk))
    return PTR_ERR(chost.clk);
    chost.dat = devm_gpiod_get_index(&pdev.dev, core::ptr::null_mut(), 1, GPIOD_IN);
    if (IS_ERR(chost.dat))
    return PTR_ERR(chost.dat);
    chost.sel = devm_gpiod_get_index(&pdev.dev, core::ptr::null_mut(), 2, GPIOD_OUT_HIGH);
    if (IS_ERR(chost.sel))
