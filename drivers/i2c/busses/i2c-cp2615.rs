...
//! Source: drivers/i2c/busses/i2c-cp2615.c
#![no_std]
...
pub const CP2615_VID: c_uint = 0x10c4;
pub const CP2615_PID: c_uint = 0xeac1;
pub const IOP_EP_IN: c_uint = 0x82;
pub const IOP_EP_OUT: c_uint = 0x02;
pub const IOP_IFN: c_int = 1;
pub const IOP_ALTSETTING: c_int = 2;
pub const MAX_IOP_SIZE: c_int = 64;
...
pub const PART_ID_A01: c_uint = 0x1400;
pub const PART_ID_A02: c_uint = 0x1500;
...
#[no_mangle]
unsafe extern "C" fn cp2615_init_i2c_msg(ret: *mut cp2615_iop_msg, data: *const cp2615_i2c_transfer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cp2615_check_status(status: enum cp2615_i2c_status) -> c_int {
...
// Driver code
    ...
    let mut res: c_int = cp2615_init_i2c_msg(msg, i2c_w);
    if (!res)
    res = usb_bulk_msg(usbdev, usb_sndbulkpipe(usbdev, IOP_EP_OUT),
    msg, ntohs(msg.length), core::ptr::null_mut(), 0);
    kfree(msg);
    return res;
    }
    static int
    ...
    sizeof(struct cp2615_iop_msg), core::ptr::null_mut(), 0);
    if (res < 0) {
    kfree(msg);
    return res;
    }
...
#[no_mangle]
unsafe extern "C" fn cp2615_check_iop(usbif: *mut usb_interface) -> c_int {
    ...
    struct usb_device *usbdev = interface_to_usbdev(usbif);
    let mut res: c_int = cp2615_init_iop_msg(msg, iop_GetAccessoryInfo, core::ptr::null_mut(), 0);
    if (res)
    goto out;
    res = usb_bulk_msg(usbdev, usb_sndbulkpipe(usbdev, IOP_EP_OUT),
    msg, ntohs(msg.length), core::ptr::null_mut(), 0);
    ...
    msg, sizeof(struct cp2615_iop_msg), core::ptr::null_mut(), 0);
    if (res)
    goto out;
    ...
    struct i2c_msg *msg;
    let mut i2c_w: cp2615_i2c_transfer = {0};
