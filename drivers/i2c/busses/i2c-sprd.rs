...
//! Source: drivers/i2c/busses/i2c-sprd.c
#![no_std]
...
pub const I2C_CTL: c_uint = 0x00;
pub const I2C_ADDR_CFG: c_uint = 0x04;
pub const I2C_COUNT: c_uint = 0x08;
pub const I2C_RX: c_uint = 0x0c;
pub const I2C_TX: c_uint = 0x10;
pub const I2C_STATUS: c_uint = 0x14;
pub const I2C_HSMODE_CFG: c_uint = 0x18;
pub const I2C_VERSION: c_uint = 0x1c;
pub const ADDR_DVD0: c_uint = 0x20;
pub const ADDR_DVD1: c_uint = 0x24;
pub const ADDR_STA0_DVD: c_uint = 0x28;
pub const ADDR_RST: c_uint = 0x2c;
// I2C_CTL

pub const FIFO_AF_LVL: c_int = 16;

pub const FIFO_AE_LVL: c_int = 12;

// I2C_STATUS

// ADDR_RST

pub const I2C_FIFO_DEEP: c_int = 12;
pub const I2C_FIFO_FULL_THLD: c_int = 15;
pub const I2C_FIFO_EMPTY_THLD: c_int = 4;
pub const I2C_DATA_STEP: c_int = 8;

    ((((high) & GENMASK(15, 0)) << 16) | ((low) & GENMASK(15, 0)))

    (((high) & GENMASK(31, 16)) | (((low) & GENMASK(31, 16)) >> 16))
// timeout (ms) for pm runtime autosuspend
pub const SPRD_I2C_PM_TIMEOUT: c_int = 1000;
// timeout (ms) for transfer message
pub const I2C_XFER_TIMEOUT: c_int = 1000;
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_set_count(i2c_dev: *mut sprd_i2c, count: u32) {
    static void sprd_i2c_set_count(struct sprd_i2c *i2c_dev, u32 count)
    {
    writel(count, i2c_dev.base + I2C_COUNT);
    }
#[no_mangle]
unsafe extern "C" fn sprd_i2c_send_stop(i2c_dev: *mut sprd_i2c, stop: c_int) {
    static void sprd_i2c_send_stop(struct sprd_i2c *i2c_dev, int stop)
    {
    let mut tmp: u32 = readl(i2c_dev.base + I2C_CTL);
    if (stop)
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_clear_start(i2c_dev: *mut sprd_i2c) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_clear_ack(i2c_dev: *mut sprd_i2c) {
    static void sprd_i2c_clear_ack(struct sprd_i2c *i2c_dev)
    {
    let mut tmp: u32 = readl(i2c_dev.base + I2C_STATUS);
    writel(tmp & ~I2C_RX_ACK, i2c_dev.base + I2C_STATUS);
    }
#[no_mangle]
unsafe extern "C" fn sprd_i2c_clear_irq(i2c_dev: *mut sprd_i2c) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_reset_fifo(i2c_dev: *mut sprd_i2c) {
    static void sprd_i2c_reset_fifo(struct sprd_i2c *i2c_dev)
    {
    writel(I2C_RST, i2c_dev.base + ADDR_RST);
    }
#[no_mangle]
unsafe extern "C" fn sprd_i2c_set_devaddr(i2c_dev: *mut sprd_i2c, m: *mut i2c_msg) {
    static void sprd_i2c_set_devaddr(struct sprd_i2c *i2c_dev, struct i2c_msg *m)
    {
    writel(m.addr << 1, i2c_dev.base + I2C_ADDR_CFG);
    }
#[no_mangle]
unsafe extern "C" fn sprd_i2c_write_bytes(i2c_dev: *mut sprd_i2c, buf: *mut u8, len: u32) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_read_bytes(i2c_dev: *mut sprd_i2c, buf: *mut u8, len: u32) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_set_full_thld(i2c_dev: *mut sprd_i2c, full_thld: u32) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_set_empty_thld(i2c_dev: *mut sprd_i2c, empty_thld: u32) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_set_fifo_full_int(i2c_dev: *mut sprd_i2c, enable: c_int) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_set_fifo_empty_int(i2c_dev: *mut sprd_i2c, enable: c_int) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_opt_start(i2c_dev: *mut sprd_i2c) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_opt_mode(i2c_dev: *mut sprd_i2c, rw: c_int) {
    static void sprd_i2c_opt_mode(struct sprd_i2c *i2c_dev, int rw)
    {
    let mut cmd: u32 = readl(i2c_dev.base + I2C_CTL) & ~I2C_MODE;
    writel(cmd | rw << 3, i2c_dev.base + I2C_CTL);
    }
#[no_mangle]
unsafe extern "C" fn sprd_i2c_data_transfer(i2c_dev: *mut sprd_i2c) {
    static void sprd_i2c_data_transfer(struct sprd_i2c *i2c_dev)
    {
    let mut i2c_count: u32 = i2c_dev.count;
    let mut need_tran: u32 = i2c_count <= I2C_FIFO_DEEP ? i2c_count : I2C_FIFO_DEEP;
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_func(adap: *mut i2c_adapter) -> u32 {
...
// low = (prescale * 2 * 3) / 5
//
    let mut high: u32 = ((i2c_dvd << 1) * 2) / 5;
    let mut low: u32 = ((i2c_dvd << 1) * 3) / 5;
    let mut div0: u32 = I2C_ADDR_DVD0_CALC(high, low);
    let mut div1: u32 = I2C_ADDR_DVD1_CALC(high, low);
...
#[no_mangle]
pub unsafe extern "C" fn if(I2C_MAX_STANDARD_MODE_FREQ: freq ==) -> else {
    else if (freq == I2C_MAX_STANDARD_MODE_FREQ)
    writel((4 * apb_clk) / 1000000, i2c_dev.base + ADDR_STA0_DVD);
    }
#[no_mangle]
unsafe extern "C" fn sprd_i2c_enable(i2c_dev: *mut sprd_i2c) {
    static void sprd_i2c_enable(struct sprd_i2c *i2c_dev)
    {
    let mut tmp: u32 = I2C_DVD_OPT;
    writel(tmp, i2c_dev.base + I2C_CTL);
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_isr_thread(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t sprd_i2c_isr_thread(int irq, void *dev_id)
    {
    struct sprd_i2c *i2c_dev = dev_id;
    struct i2c_msg *msg = i2c_dev.msg;
    let mut ack: bool = !(readl(i2c_dev.base + I2C_STATUS) & I2C_RX_ACK);
    u32 i2c_tran;
    if (msg.flags & I2C_M_RD)
...
//
    ...
    i2c_dev.err = -EIO;
#[no_mangle]
pub unsafe extern "C" fn if(i2c_dev->count: msg->flags & I2C_M_RD &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_clk_init(i2c_dev: *mut sprd_i2c) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_suspend_noirq(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_resume_noirq(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_runtime_suspend(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn sprd_i2c_runtime_resume(dev: *mut device) -> int __maybe_unused {
    ...
    sprd_i2c_runtime_resume, core::ptr::null_mut())
