...
//! Source: drivers/i2c/busses/i2c-brcmstb.c
#![no_std]
...
pub const BSC_CNT_REG1_SHIFT: c_int = 0;
// BSC CTL register field definitions
pub const BSC_CTL_REG_DTF_MASK: c_uint = 0x00000003;
pub const BSC_CTL_REG_SCL_SEL_MASK: c_uint = 0x00000030;
pub const BSC_CTL_REG_SCL_SEL_SHIFT: c_int = 4;
pub const BSC_CTL_REG_INT_EN_MASK: c_uint = 0x00000040;
pub const BSC_CTL_REG_INT_EN_SHIFT: c_int = 6;
pub const BSC_CTL_REG_DIV_CLK_MASK: c_uint = 0x00000080;
// BSC_IIC_ENABLE r/w enable and interrupt field definitions
pub const BSC_IIC_EN_RESTART_MASK: c_uint = 0x00000040;
pub const BSC_IIC_EN_NOSTART_MASK: c_uint = 0x00000020;
pub const BSC_IIC_EN_NOSTOP_MASK: c_uint = 0x00000010;
pub const BSC_IIC_EN_NOACK_MASK: c_uint = 0x00000004;
pub const BSC_IIC_EN_INTRP_MASK: c_uint = 0x00000002;
pub const BSC_IIC_EN_ENABLE_MASK: c_uint = 0x00000001;
// BSC_CTLHI control register field definitions
pub const BSC_CTLHI_REG_INPUT_SWITCHING_LEVEL_MASK: c_uint = 0x00000080;
pub const BSC_CTLHI_REG_DATAREG_SIZE_MASK: c_uint = 0x00000040;
pub const BSC_CTLHI_REG_IGNORE_ACK_MASK: c_uint = 0x00000002;
pub const BSC_CTLHI_REG_WAIT_DIS_MASK: c_uint = 0x00000001;

// Condition mask used for non combined transfer

// BSC data transfer direction
pub const DTF_WR_MASK: c_uint = 0x00000000;
pub const DTF_RD_MASK: c_uint = 0x00000001;
// BSC data transfer direction combined format
pub const DTF_RD_WR_MASK: c_uint = 0x00000002;
pub const DTF_WR_RD_MASK: c_uint = 0x00000003;

// BSC block register map structure to cache fields to be written
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bsc_regs {
    pub /: *mut *mut u32 chip_address; / target address,
    pub buffer*/: *mut *mut u32 data_in[N_DATA_REGS]; / tx data,
    pub /: *mut *mut u32 cnt_reg; / rx/tx data length,
    pub /: *mut *mut u32 ctl_reg; / control register,
    pub /: *mut *mut u32 iic_enable; / xfer enable and status,
    pub /: *mut *mut u32 data_out[N_DATA_REGS]; / rx data buffer,
    pub /: *mut *mut u32 ctlhi_reg; / more control fields,
    pub /: *mut *mut u32 scl_param; / reserved,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct bsc_clk_param {
    pub hz: u32,
    pub scl_mask: u32,
    pub div_mask: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct brcmstb_i2c_dev {
    pub device: *mut device,
    pub base: *mut void __iomem,
    pub irq: c_int,
    pub bsc_regmap: *mut bsc_regs,
    pub adapter: i2c_adapter,
    pub done: completion,
    pub clk_freq_hz: u32,
    pub data_regsz: c_int,
    pub atomic: bool,
}
...
#[no_mangle]
pub unsafe extern "C" fn brcmstb_i2c_get_xfersz(dev: *mut brcmstb_i2c_dev) -> c_int {
    static inline int brcmstb_i2c_get_xfersz(struct brcmstb_i2c_dev *dev)
    {
    return (N_DATA_REGS * dev.data_regsz);
    }
#[no_mangle]
pub unsafe extern "C" fn brcmstb_i2c_get_data_regsz(dev: *mut brcmstb_i2c_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_isr(irq: c_int, devid: *mut c_void) -> irqreturn_t {
    static irqreturn_t brcmstb_i2c_isr(int irq, void *devid)
    {
    struct brcmstb_i2c_dev *dev = devid;
    let mut status_bsc_ctl: u32 = bsc_readl(dev, ctl_reg);
    let mut status_iic_intrp: u32 = bsc_readl(dev, iic_enable);
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_wait_if_busy(dev: *mut brcmstb_i2c_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_wait_for_completion(dev: *mut brcmstb_i2c_dev) -> c_int {
    static int brcmstb_i2c_wait_for_completion(struct brcmstb_i2c_dev *dev)
    {
    let mut ret: c_int = 0;
    let mut timeout: c_ulong = msecs_to_jiffies(I2C_TIMEOUT);
...
// Set xfer START/STOP conditions for subsequent transfer
    ...
    u32 cond_flag)
    {
    let mut regval: u32 = dev.bsc_regmap.iic_enable;
...
// Actual data transfer through the BSC controller
    ...
    u32 ctl_reg;
    struct bsc_regs *pi2creg = dev.bsc_regmap;
    let mut no_ack: c_int = pmsg.flags & I2C_M_IGNORE_NAK;
    let mut data_regsz: c_int = brcmstb_i2c_get_data_regsz(dev);
...
// Read data from data_out register
    ...
    for (cnt = 0, i = 0; cnt < len; cnt += data_regsz, i++) {
    let mut data: u32 = bsc_readl(dev, data_out[i]);
...
// Write a single byte of data to the i2c bus
    ...
    {
    let mut cmd: enum bsc_xfer_cmd = nak_expected ? CMD_WR : CMD_WR_NOACK;
...
// Then re-send the first byte with the read bit set
    ...
    u8 *tmp_buf;
    let mut len: c_int = 0;
    let mut xfersz: c_int = brcmstb_i2c_get_xfersz(dev);
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_functionality(adap: *mut i2c_adapter) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_set_bus_speed(dev: *mut brcmstb_i2c_dev) {
    static void brcmstb_i2c_set_bus_speed(struct brcmstb_i2c_dev *dev)
    {
    let mut i: c_int = 0, num_speeds = ARRAY_SIZE(bsc_clk);
    let mut clk_freq_hz: u32 = dev.clk_freq_hz;
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_set_bsc_reg_defaults(dev: *mut brcmstb_i2c_dev) {
...
pub const AUTOI2C_CTRL0: c_uint = 0x26c;

#[no_mangle]
unsafe extern "C" fn bcm2711_release_bsc(dev: *mut brcmstb_i2c_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn brcmstb_i2c_resume(dev: *mut device) -> c_int {
