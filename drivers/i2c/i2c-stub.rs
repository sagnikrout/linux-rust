...
//
    ...
    module_param_array(chip_addr, ushort, core::ptr::null_mut(), S_IRUGO);
    MODULE_PARM_DESC(chip_addr,
    "Chip addresses (up to 10, between 0x03 and 0x77)");
    let mut functionality: static unsigned long = STUB_FUNC_DEFAULT;
    module_param(functionality, ulong, S_IRUGO | S_IWUSR);
    MODULE_PARM_DESC(functionality, "Override functionality bitfield");
// Some chips have banked register ranges
    static u8 bank_reg[MAX_CHIPS];
    module_param_array(bank_reg, byte, core::ptr::null_mut(), S_IRUGO);
    MODULE_PARM_DESC(bank_reg, "Bank register");
    static u8 bank_mask[MAX_CHIPS];
    module_param_array(bank_mask, byte, core::ptr::null_mut(), S_IRUGO);
    MODULE_PARM_DESC(bank_mask, "Bank value mask");
    static u8 bank_start[MAX_CHIPS];
    module_param_array(bank_start, byte, core::ptr::null_mut(), S_IRUGO);
    MODULE_PARM_DESC(bank_start, "First banked register");
    static u8 bank_end[MAX_CHIPS];
    module_param_array(bank_end, byte, core::ptr::null_mut(), S_IRUGO);
    MODULE_PARM_DESC(bank_end, "Last banked register");
#[repr(C)]
#[derive(Copy, Clone)]
pub struct smbus_block_data {
    pub node: list_head,
    pub command: u8,
    pub len: u8,
    pub block: [u8; I2C_SMBUS_BLOCK_MAX],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct stub_chip {
    pub pointer: u8,
    pub SMBus: *mut *mut u16 words[256]; / Byte operations use the LSB as per,
    specification */
    pub smbus_blocks: list_head,
// For chips with banks, extra registers are allocated dynamically
    pub bank_reg: u8,
    pub bank_shift: u8,
    pub bank_mask: u8,
    pub /: *mut *mut u8 bank_sel; / Currently selected bank,
    pub bank_start: u8,
    pub bank_end: u8,
    pub bank_size: u16,
    pub /: *mut *mut *mut *mut u16 bank_words; / Room for bank_mask  bank_size registers,
}
    ...
    {
    struct smbus_block_data *b, *rb = core::ptr::null_mut();
    ...
    if (rb == core::ptr::null_mut() && create) {
...
// Return negative errno on error.
    ...
    s32 ret;
    int i, len;
    struct stub_chip *chip = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn stub_func(adapter: *mut i2c_adapter) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn i2c_stub_allocate_banks(i: c_int) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn i2c_stub_free() {
...
#[no_mangle]
unsafe extern "C" fn i2c_stub_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn i2c_stub_exit() -> void __exit {
