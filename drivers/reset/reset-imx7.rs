...
//! Source: drivers/reset/reset-imx7.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct imx7_src_signal {
    pub bit: unsigned int offset,,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct imx7_src_variant {
    pub signals: *const imx7_src_signal,
    pub signals_num: c_uint,
    pub ops: reset_control_ops,
}
...
pub struct imx7_src {
    ...
    let mut bit: c_uint = imx7src.signals[id].bit;
    let mut value: c_uint = assert ? bit : 0;
    switch (id) {
...
#[no_mangle]
unsafe extern "C" fn imx7_reset_probe(pdev: *mut platform_device) -> c_int {
    static int imx7_reset_probe(struct platform_device *pdev)
    {
    struct imx7_src *imx7src;
    struct device *dev = &pdev.dev;
    let mut config: regmap_config = { .name = "src" };
