...
//! Source: drivers/reset/reset-eyeq.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct eqr_busy_wait_timings {
    pub sleep_us: c_ulong,
...
pub const EQR_MAX_DOMAIN_COUNT: c_int = 3;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct eqr_domain_descriptor {
    pub type: enum eqr_domain_type,
    pub valid_mask: u32,
    pub offset: c_uint,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct eqr_match_data {
    pub domain_count: c_uint,
    pub domains: *const eqr_domain_descriptor,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct eqr_private {
//
...
//
    pub mutexes: [mutex; EQR_MAX_DOMAIN_COUNT],
    pub base: *mut void __iomem,
    pub data: *const eqr_match_data,
...
// dest_b = readl(addr_b);
    ...
    let mut domain_type: enum eqr_domain_type = priv.data.domains[domain].type;
    let mut timeout_us: c_ulong = eqr_timings[domain_type].timeout_us;
    let mut sleep_us: c_ulong = eqr_timings[domain_type].sleep_us;
...
#[no_mangle]
unsafe extern "C" fn eqr_assert_locked(priv: *mut eqr_private, domain: u32, offset: u32) {
...
#[no_mangle]
unsafe extern "C" fn eqr_assert(rcdev: *mut reset_controller_dev, id: c_ulong) -> c_int {
    static int eqr_assert(struct reset_controller_dev *rcdev, unsigned long id)
    {
    struct eqr_private *priv = eqr_rcdev_to_priv(rcdev);
    let mut domain: u32 = FIELD_GET(ID_DOMAIN_MASK, id);
    let mut offset: u32 = FIELD_GET(ID_OFFSET_MASK, id);
...
#[no_mangle]
unsafe extern "C" fn eqr_deassert(rcdev: *mut reset_controller_dev, id: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn eqr_status(rcdev: *mut reset_controller_dev, id: c_ulong) -> c_int {
