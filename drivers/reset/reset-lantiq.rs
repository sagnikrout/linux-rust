...
//! Source: drivers/reset/reset-lantiq.c
#![no_std]
...
pub const LANTIQ_RCU_RESET_TIMEOUT: c_int = 10000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lantiq_rcu_reset_priv {
    ...
    let mut status: c_uint = (id >> 8) & 0x1f;
    u32 val;
    int ret;
    ...
    {
    int ret;
    let mut retry: c_int = LANTIQ_RCU_RESET_TIMEOUT;
    do {
    ...
    {
    struct lantiq_rcu_reset_priv *priv = to_lantiq_rcu_reset_priv(rcdev);
    let mut set: c_uint = id & 0x1f;
    let mut val: u32 = assert ? BIT(set) : 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn lantiq_rcu_reset_probe(pdev: *mut platform_device) -> c_int {
