...
//! Source: drivers/block/ataflop.c
#![no_std]
...
pub const FD_MAX_UNITS: c_int = 2;
...
pub const FDCREG_CMD: c_int = 0;
pub const FDCREG_STATUS: c_int = 0;
pub const FDCREG_TRACK: c_int = 2;
pub const FDCREG_SECTOR: c_int = 4;
pub const FDCREG_DATA: c_int = 6;
// command opcodes
...
// step rates
pub const FDCSTEP_6: c_uint = 0x00;
pub const FDCSTEP_12: c_uint = 0x01;
pub const FDCSTEP_2: c_uint = 0x02;
pub const FDCSTEP_3: c_uint = 0x03;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct atari_format_descr {
    pub /: *mut *mut int track; / to be formatted,
    pub /: *mut *mut int head; / "" "",
    pub /: *mut *mut int sect_offset; / offset of first sector,
}
...
pub const TYPE_DD: c_int = 0;
pub const TYPE_HD: c_int = 1;
pub const TYPE_ED: c_int = 2;
    let mut DriveType: static int = TYPE_HD;
...
//
pub const MAX_DISK_SIZE: c_int = 3280;
//
...
// request.
//
    let mut SelectedDrive: static int = 0;
    static int ReqCmd, ReqBlock;
    static int ReqSide, ReqTrack, ReqSector, ReqCnt;
    let mut HeadSettleFlag: static int = 0;
    static unsigned char *ReqData, *ReqBuffer;
    let mut MotorOn: static int = 0, MotorOffTrys;
    let mut IsFormatting: static int = 0, FormatError;
    static int UserSteprate[FD_MAX_UNITS] = { -1, -1 };
    module_param_array(UserSteprate, int, core::ptr::null_mut(), 0);
    static DECLARE_COMPLETION(format_wait);
    let mut changed_floppies: static unsigned long = 0xff, fake_change = 0;
...
// successful access.
//
    let mut Probing: static int = 0;
// This flag is set when a dummy seek is necessary to make the WP
// status bit accessible.
//
    let mut NeedSeek: static int = 0;

// Macro flag: #define DPRINT(a)
...
#[no_mangle]
unsafe extern "C" fn fd_end_request_cur(err: blk_status_t) {
...
#[no_mangle]
pub unsafe extern "C" fn start_motor_off_timer() {
...
#[no_mangle]
unsafe extern "C" fn fd_select_side(side: c_int) {
    static void fd_select_side( int side )
    {
    unsigned long flags;
...
//
#[no_mangle]
unsafe extern "C" fn fd_select_drive(drive: c_int) {
    static void fd_select_drive( int drive )
    {
    unsigned long flags;
    unsigned char tmp;
...
#[no_mangle]
unsafe extern "C" fn fd_deselect() {
    static void fd_deselect( void )
    {
    unsigned long flags;
// protect against various other ints mucking around with the PSG
...
//
#[no_mangle]
unsafe extern "C" fn fd_motor_off_timer(unused: *mut timer_list) {
    static void fd_motor_off_timer(struct timer_list *unused)
    {
...
//
#[no_mangle]
unsafe extern "C" fn check_change(unused: *mut timer_list) {
    static void check_change(struct timer_list *unused)
    {
    let mut drive: static int = 0;
    unsigned long flags;
...
//
#[no_mangle]
pub unsafe extern "C" fn set_head_settle_flag() {
    static inline void set_head_settle_flag(void)
    {
    HeadSettleFlag = FDCCMDADD_E;
    }
#[no_mangle]
pub unsafe extern "C" fn get_head_settle_flag() -> c_int {
    static inline int get_head_settle_flag(void)
    {
    let mut tmp: c_int = HeadSettleFlag;
    HeadSettleFlag = 0;
    return( tmp );
    }
#[no_mangle]
pub unsafe extern "C" fn copy_buffer(from: *mut c_void, to: *mut c_void) {
...
// General Interrupt Handling
    static void (*FloppyIRQHandler)( int status ) = core::ptr::null_mut();
#[no_mangle]
unsafe extern "C" fn floppy_irq(irq: c_int, dummy: *mut c_void) -> irqreturn_t {
    static irqreturn_t floppy_irq (int irq, void *dummy)
    {
    unsigned char status;
    void (*handler)( int );
    handler = xchg(&FloppyIRQHandler, core::ptr::null_mut());
    if (handler) {
    nop();
    status = FDC_READ( FDCREG_STATUS );
...
#[no_mangle]
pub unsafe extern "C" fn if(RECALIBRATE_ERRORS: unit[SelectedDrive].error_count ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn do_format(drive: c_int, type: c_int, desc: *mut atari_format_descr) -> c_int {
    static int do_format(int drive, int type, struct atari_format_descr *desc)
    {
    ...
    local_irq_save(flags);
    stdma_lock(floppy_irq, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn do_fd_action(drive: c_int) {
    static void do_fd_action( int drive )
    {
    DPRINT(("do_fd_action\n"));
...
// consistency!
    ...
    fd_calibrate();
#[no_mangle]
pub unsafe extern "C" fn if(UDT->stretch: UD.track != ReqTrack <<) -> else {
    else if (UD.track != ReqTrack << UDT.stretch)
    fd_seek();
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IsFormatting) -> else {
...
#[no_mangle]
unsafe extern "C" fn fd_calibrate() {
    static void fd_calibrate( void )
    {
    if (SUD.track >= 0) {
    fd_calibrate_done( 0 );
...
#[no_mangle]
unsafe extern "C" fn fd_calibrate_done(status: c_int) {
    static void fd_calibrate_done( int status )
    {
...
#[no_mangle]
unsafe extern "C" fn fd_seek_done(status: c_int) {
    static void fd_seek_done( int status )
    {
    DPRINT(("fd_seek_done()\n"));
    stop_timeout();
...
//
    let mut MultReadInProgress: static int = 0;
#[no_mangle]
unsafe extern "C" fn fd_rwsec() {
    static void fd_rwsec( void )
    {
...
#[no_mangle]
unsafe extern "C" fn fd_readtrack_check(unused: *mut timer_list) {
    static void fd_readtrack_check(struct timer_list *unused)
    {
...
// the read operation
//
    SET_IRQ_HANDLER( core::ptr::null_mut() );
...
#[no_mangle]
unsafe extern "C" fn fd_rwsec_done(status: c_int) {
    static void fd_rwsec_done( int status )
    {
...
#[no_mangle]
unsafe extern "C" fn fd_rwsec_done1(status: c_int) {
    static void fd_rwsec_done1(int status)
    {
    unsigned int track;
    stop_timeout();
...
#[no_mangle]
unsafe extern "C" fn fd_writetrack() {
    static void fd_writetrack( void )
    {
    unsigned long paddr, flags;
    unsigned int track;
...
#[no_mangle]
unsafe extern "C" fn fd_writetrack_done(status: c_int) {
    static void fd_writetrack_done( int status )
    {
...
#[no_mangle]
unsafe extern "C" fn fd_times_out(unused: *mut timer_list) {
    static void fd_times_out(struct timer_list *unused)
    {
    atari_disable_irq( IRQ_MFP_FDC );
...
#[no_mangle]
unsafe extern "C" fn finish_fdc_done(dummy: c_int) {
    static void finish_fdc_done( int dummy )
    {
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn setup_req_params(drive: c_int) {
    static void setup_req_params( int drive )
    {
    let mut block: c_int = ReqBlock + ReqCnt;
    ...
    let mut type: c_int = floppy.type;
    ...
    if (!stdma_try_lock(floppy_irq, core::ptr::null_mut()))  {
    spin_unlock_irq(&ataflop_lock);
...
#[no_mangle]
unsafe extern "C" fn fd_probe(drive: c_int) -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn fd_test_drive_present(drive: c_int) -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn config_types() -> void __init {
...
unsafe extern "C" fn floppy_open(disk: *mut gendisk, mode: blk_mode_t) -> c_int {
    ...
    {
    struct atari_floppy_struct *p = disk.private_data;
    let mut type: c_int = disk.first_minor >> 2;
...
unsafe extern "C" fn floppy_release(disk: *mut gendisk) {
    ...
    p.ref = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !p->ref--) -> else {
...
#[no_mangle]
unsafe extern "C" fn ataflop_alloc_disk(drive: c_uint, type: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ataflop_probe(dev: dev_t) {
    static void ataflop_probe(dev_t dev)
    {
    let mut drive: c_int = MINOR(dev) & 3;
    let mut type: c_int = MINOR(dev) >> 2;
    if (type)
    type--;
    ...
    unit[drive].disk[type] = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn atari_floppy_cleanup() {
...
#[no_mangle]
unsafe extern "C" fn atari_cleanup_floppy_disk(fs: *mut atari_floppy_struct) {
...
#[no_mangle]
unsafe extern "C" fn atari_floppy_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn atari_floppy_setup(str: *mut c_char) -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(2+FD_MAX_UNITS: ints[0] >) -> else {
...
#[no_mangle]
unsafe extern "C" fn atari_floppy_exit() -> void __exit {
