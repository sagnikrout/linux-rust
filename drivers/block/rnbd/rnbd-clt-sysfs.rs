...
//! Source: drivers/block/rnbd/rnbd-clt-sysfs.c
#![no_std]
...
//
    ...
    {RNBD_OPT_ERR,			core::ptr::null_mut()			},
    };
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rnbd_map_options {
    pub sessname: *mut c_char,
    pub paths: *mut rtrs_addr,
    pub path_cnt: *mut usize,
    pub pathname: *mut c_char,
    pub dest_port: *mut u16,
    pub access_mode: *mut enum rnbd_access_mode,
    pub nr_poll_queues: *mut u32,
}
    ...
    let mut opt_mask: c_int = 0;
    int token;
    let mut ret: c_int = -EINVAL;
    let mut nr_poll_queues: c_int = 0;
    let mut dest_port: c_int = 0;
    let mut p_cnt: c_int = 0;
    int i;
    options = kstrdup(buf, GFP_KERNEL);
    if (!options)
    return -ENOMEM;
    sep_opt = strstrip(options);
    while ((p = strsep(&sep_opt, " ")) != core::ptr::null_mut()) {
    if (!*p)
    continue;
...
// TODO fix cli tool before changing to proper state
    ...
    let mut rnbd_clt_state_attr: static struct kobj_attribute = __ATTR_RO(state);
...
#[no_mangle]
pub unsafe extern "C" fn rnbd_clt_remove_dev_symlink(dev: *mut rnbd_clt_dev) {
...
// It should be freed always.
    ...
    dev.blk_symlink_name = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn rnbd_dev_release(kobj: *mut kobject) {
...
#[no_mangle]
unsafe extern "C" fn rnbd_clt_add_dev_kobj(dev: *mut rnbd_clt_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rnbd_clt_add_dev_symlink(dev: *mut rnbd_clt_dev) -> c_int {
    ...
    let mut access_mode: enum rnbd_access_mode = RNBD_ACCESS_RW;
    let mut port_nr: u16 = RTRS_PORT;
    ...
    return count;
    unmap_dev:
    rnbd_clt_unmap_device(dev, true, core::ptr::null_mut());
    out:
    kfree(addrs);
    return ret;
    }
...
#[no_mangle]
pub unsafe extern "C" fn rnbd_clt_create_sysfs_files() -> c_int {
    ...
    if (err)
    return err;
    rnbd_dev = device_create_with_groups(&rnbd_dev_class, core::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn rnbd_clt_destroy_sysfs_files() {
