...
//! Source: drivers/block/drbd/drbd_proc.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn seq_printf_with_thousands_grouping(seq: *mut seq_file, v: c_long) {
...
//
    let mut shift: c_uint = *rs_total > UINT_MAX ? 16 : 10;
    let mut left: c_ulong = *bits_left >> shift;
    let mut total: c_ulong = 1UL + (*rs_total >> shift);
    let mut tmp: c_ulong = 1000UL - left * 1000UL/total;
// per_mil_done = tmp;
    }
    }
// lge
...
// we convert to sectors in the display below.
    let mut bm_bits: c_ulong = drbd_bm_bits(device);
    unsigned long bit_pos;
    let mut stop_sector: c_ulonglong = 0;
...
#[no_mangle]
pub unsafe extern "C" fn drbd_seq_show(seq: *mut seq_file, v: *mut c_void) -> c_int {
