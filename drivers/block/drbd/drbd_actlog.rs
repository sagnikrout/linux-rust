...
//! Source: drivers/block/drbd/drbd_actlog.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn drbd_md_put_buffer(device: *mut drbd_device) {
...
// we do all our meta data IO in aligned 4k blocks.
    ...
    if (op != REQ_OP_WRITE && device.state.disk == D_DISKLESS && device.ldev == core::ptr::null_mut())
// special case, drbd_md_read() during drbd_adm_attach(): no get_ldev
    ;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !get_ldev_if_state(device, _arg: D_ATTACHING)) -> else {
// Corresponding put_ldev in drbd_md_endio()
    ...
    if (unlikely(tmp != core::ptr::null_mut())) {
...
#[no_mangle]
pub unsafe extern "C" fn drbd_al_begin_io_fastpath(device: *mut drbd_device, i: *mut drbd_interval) -> bool {
    bool drbd_al_begin_io_fastpath(struct drbd_device *device, struct drbd_interval *i)
    {
// for bios crossing activity log extent boundaries,
// we may need to activate two extents in one go
    let mut first: unsigned = i.sector >> (AL_EXTENT_SHIFT-9);
    let mut last: unsigned = i.size == 0 ? first : (i.sector + (i.size >> 9) - 1) >> (AL_EXTENT_SHIFT-9);
...
#[no_mangle]
pub unsafe extern "C" fn drbd_al_begin_io_prepare(device: *mut drbd_device, i: *mut drbd_interval) -> bool {
...
// we may need to activate two extents in one go
    ...
    let mut last: unsigned = i.size == 0 ? first : (i.sector + (i.size >> 9) - 1) >> (AL_EXTENT_SHIFT-9);
    unsigned enr;
    let mut need_transaction: bool = false;
    D_ASSERT(device, first <= last);
    D_ASSERT(device, atomic_read(&device.local_cnt) > 0);
    for (enr = first; enr <= last; enr++) {
    struct lc_element *al_ext;
    wait_event(device.al_wait,
    (al_ext = _al_get(device, enr, false)) != core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn al_extent_to_bm_page(al_enr: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn al_tr_number_to_on_disk_sector(device: *mut drbd_device) -> sector_t {
    static sector_t al_tr_number_to_on_disk_sector(struct drbd_device *device)
    {
    let mut stripes: c_uint = device.ldev.md.al_stripes;
    let mut stripe_size_4kB: c_uint = device.ldev.md.al_stripe_size_4k;
// transaction number, modulo on-disk ring buffer wrap around
    let mut t: c_uint = device.al_tr_number % (device.ldev.md.al_size_4k);
...
#[no_mangle]
unsafe extern "C" fn __al_write_transaction(device: *mut drbd_device, buffer: *mut al_transaction_on_disk) -> c_int {
...
// be in the process of changing it.
    ...
    for (i = 0; i < mx; i++) {
    let mut idx: unsigned = device.al_tr_cycle + i;
...
#[no_mangle]
unsafe extern "C" fn al_write_transaction(device: *mut drbd_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn drbd_al_begin_io_commit(device: *mut drbd_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drbd_al_begin_io(device: *mut drbd_device, i: *mut drbd_interval) {
...
#[no_mangle]
pub unsafe extern "C" fn drbd_al_begin_io_nonblock(device: *mut drbd_device, i: *mut drbd_interval) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn drbd_al_complete_io(device: *mut drbd_device, i: *mut drbd_interval) {
...
#[no_mangle]
unsafe extern "C" fn _try_lc_del(device: *mut drbd_device, al_ext: *mut lc_element) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drbd_al_shrink(device: *mut drbd_device) {
...
#[no_mangle]
pub unsafe extern "C" fn drbd_al_initialize(device: *mut drbd_device, buffer: *mut c_void) -> c_int {
    int drbd_al_initialize(struct drbd_device *device, void *buffer)
    {
    struct al_transaction_on_disk *al = buffer;
    struct drbd_md *md = &device.ldev.md;
    let mut al_size_4k: c_int = md.al_stripes * md.al_stripe_size_4k;
    int i;
...
// on-disk ring buffer.
    ...
    let mut err: c_int = __al_write_transaction(device, al);
    if (err)
    return err;
    }
    return 0;
...
// then update and check rs_left and rs_failed.
    ...
    ext.rs_left -= count;
#[no_mangle]
pub unsafe extern "C" fn if(SET_OUT_OF_SYNC: mode ==) -> else {
...
//
    let mut rs_left: c_int = drbd_bm_e_weight(device, enr);
    if (ext.flags != 0) {
...
#[no_mangle]
pub unsafe extern "C" fn drbd_advance_rs_marks(peer_device: *mut drbd_peer_device, still_to_go: c_ulong) {
    ...
    let mut last: c_ulong = device.rs_mark_time[device.rs_last_mark];
    let mut next: c_int = (device.rs_last_mark + 1) % DRBD_SYNC_MARKS;
...
#[no_mangle]
unsafe extern "C" fn lazy_bitmap_update_due(device: *mut drbd_device) -> bool {
    static bool lazy_bitmap_update_due(struct drbd_device *device)
    {
    return time_after(jiffies, device.rs_last_bcast + 2*HZ);
    }
#[no_mangle]
unsafe extern "C" fn maybe_schedule_on_disk_bitmap_update(device: *mut drbd_device, rs_done: bool) {
...
// but cap at provided end bit number
    let mut tbnr: c_ulong = min(ebnr, sbnr | BM_BLOCKS_PER_BM_EXT_MASK);
    unsigned long c;
...
#[no_mangle]
pub unsafe extern "C" fn if(SET_IN_SYNC: mode ==) -> else {
    ...
    if (count) {
    if (mode == SET_IN_SYNC) {
    let mut still_to_go: c_ulong = drbd_bm_total_weight(device);
    let mut rs_is_done: bool = (still_to_go <= device.rs_failed);
...
#[no_mangle]
unsafe extern "C" fn plausible_request_size(size: c_int) -> bool {
...
// Should not round anything here.
    ...
    e = lc_get(device.resync, enr);
    bm_ext = e ? lc_entry(e, struct bm_extent, lce) : core::ptr::null_mut();
    if (bm_ext) {
...
#[no_mangle]
unsafe extern "C" fn _is_in_al(device: *mut drbd_device, enr: c_uint) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drbd_rs_begin_io(device: *mut drbd_device, sector: sector_t) -> c_int {
    int drbd_rs_begin_io(struct drbd_device *device, sector_t sector)
    {
    let mut enr: c_uint = BM_SECT_TO_EXT(sector);
    struct bm_extent *bm_ext;
    int i, sig;
    bool sa;
    retry:
...
//
#[no_mangle]
pub unsafe extern "C" fn drbd_try_rs_begin_io(peer_device: *mut drbd_peer_device, sector: sector_t) -> c_int {
    int drbd_try_rs_begin_io(struct drbd_peer_device *peer_device, sector_t sector)
    {
    struct drbd_device *device = peer_device.device;
    let mut enr: c_uint = BM_SECT_TO_EXT(sector);
    let mut al_enr: c_uint = enr*AL_EXT_PER_BM_SECT;
    struct lc_element *e;
    struct bm_extent *bm_ext;
    int i;
    let mut throttle: bool = drbd_rs_should_slow_down(peer_device, sector, true);
...
// Do or do not. There is no try. -- Yoda
    ...
    bm_ext = e ? lc_entry(e, struct bm_extent, lce) : core::ptr::null_mut();
    if (!bm_ext) {
    let mut rs_flags: c_ulong = device.resync.flags;
    if (rs_flags & LC_STARVING)
    drbd_warn(device, "Have to wait for element"
    " (resync LRU too small?)\n");
    BUG_ON(rs_flags & LC_LOCKED);
...
#[no_mangle]
pub unsafe extern "C" fn drbd_rs_complete_io(device: *mut drbd_device, sector: sector_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drbd_rs_cancel_all(device: *mut drbd_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drbd_rs_del_all(device: *mut drbd_device) -> c_int {
