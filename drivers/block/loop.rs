...
pub struct loop_device {
    pub lo_number: c_int,
    pub lo_offset: loff_t,
    pub lo_sizelimit: loff_t,
    pub lo_flags: c_int,
    pub lo_file_name: [c_char; LO_NAME_SIZE],
    pub lo_backing_file: *mut file,
    pub lo_min_dio_size: c_uint,
    pub lo_dio_mem_align: c_uint,
...
pub struct loop_cmd {
    ...
    pub /: *mut *mut bool use_aio; / use AIO interface to handle I/O,
    pub /: *mut *mut atomic_t ref; / only for aio,
    pub ret: c_long,
    pub iocb: kiocb,
...
pub const LOOP_DEFAULT_HW_Q_DEPTH: c_int = 128;
...
//
#[no_mangle]
unsafe extern "C" fn loop_global_lock_killable(lo: *mut loop_device, global: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn loop_global_unlock(lo: *mut loop_device, global: bool) {
...
#[no_mangle]
unsafe extern "C" fn lo_calculate_size(lo: *mut loop_device, file: *mut file) -> loff_t {
...
//
#[no_mangle]
unsafe extern "C" fn lo_can_use_dio(lo: *mut loop_device) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn loop_update_dio(lo: *mut loop_device) {
...
//
#[no_mangle]
unsafe extern "C" fn loop_set_size(lo: *mut loop_device, size: loff_t) {
...
#[no_mangle]
unsafe extern "C" fn loop_clear_limits(lo: *mut loop_device, mode: c_int) {
    static void loop_clear_limits(struct loop_device *lo, int mode)
    {
    let mut lim: queue_limits = queue_limits_start_update(lo.lo_queue);
...
#[no_mangle]
unsafe extern "C" fn lo_req_flush(lo: *mut loop_device, rq: *mut request) -> c_int {
    static int lo_req_flush(struct loop_device *lo, struct request *rq)
    {
    let mut ret: c_int = vfs_fsync(lo.lo_backing_file, 0);
    if (unlikely(ret && ret != -EINVAL))
    ret = -EIO;
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn lo_complete_rq(rq: *mut request) {
...
#[no_mangle]
unsafe extern "C" fn lo_rw_aio_do_completion(cmd: *mut loop_cmd) {
...
#[no_mangle]
unsafe extern "C" fn lo_rw_aio_complete(iocb: *mut kiocb, ret: c_long) {
...
//
    ...
    cmd.iocb.ki_complete = core::ptr::null_mut();
    cmd.iocb.ki_flags = 0;
    }
    if (rw == ITER_SOURCE) {
...
#[no_mangle]
unsafe extern "C" fn do_req_filebacked(lo: *mut loop_device, rq: *mut request) -> c_int {
    static int do_req_filebacked(struct loop_device *lo, struct request *rq)
    {
    struct loop_cmd *cmd = blk_mq_rq_to_pdu(rq);
    let mut pos: loff_t = ((loff_t) blk_rq_pos(rq) << 9) + lo.lo_offset;
    switch (req_op(rq)) {
    case REQ_OP_FLUSH:
...
#[no_mangle]
unsafe extern "C" fn loop_reread_partitions(lo: *mut loop_device) {
...
#[no_mangle]
unsafe extern "C" fn loop_update_dio_alignment(lo: *mut loop_device) {
...
#[no_mangle]
pub unsafe extern "C" fn is_loop_device(file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loop_validate_file(file: *mut file, bdev: *mut block_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loop_assign_backing_file(lo: *mut loop_device, file: *mut file) {
...
#[no_mangle]
unsafe extern "C" fn loop_check_backing_file(file: *mut file) -> c_int {
...
// loop sysfs attributes
    ...
    __ATTR(_name, 0444, loop_attr_do_show_##_name, core::ptr::null_mut());
#[no_mangle]
unsafe extern "C" fn loop_attr_backing_file_show(lo: *mut loop_device, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn loop_attr_offset_show(lo: *mut loop_device, buf: *mut c_char) -> isize {
    static ssize_t loop_attr_offset_show(struct loop_device *lo, char *buf)
    {
    return sysfs_emit(buf, "%llu\n", (unsigned long long)lo.lo_offset);
    }
#[no_mangle]
unsafe extern "C" fn loop_attr_sizelimit_show(lo: *mut loop_device, buf: *mut c_char) -> isize {
    static ssize_t loop_attr_sizelimit_show(struct loop_device *lo, char *buf)
    {
    return sysfs_emit(buf, "%llu\n", (unsigned long long)lo.lo_sizelimit);
    }
#[no_mangle]
unsafe extern "C" fn loop_attr_autoclear_show(lo: *mut loop_device, buf: *mut c_char) -> isize {
    static ssize_t loop_attr_autoclear_show(struct loop_device *lo, char *buf)
    {
    let mut autoclear: c_int = (lo.lo_flags & LO_FLAGS_AUTOCLEAR);
    return sysfs_emit(buf, "%s\n", autoclear ? "1" : "0");
    }
#[no_mangle]
unsafe extern "C" fn loop_attr_partscan_show(lo: *mut loop_device, buf: *mut c_char) -> isize {
    static ssize_t loop_attr_partscan_show(struct loop_device *lo, char *buf)
    {
    let mut partscan: c_int = (lo.lo_flags & LO_FLAGS_PARTSCAN);
    return sysfs_emit(buf, "%s\n", partscan ? "1" : "0");
    }
#[no_mangle]
unsafe extern "C" fn loop_attr_dio_show(lo: *mut loop_device, buf: *mut c_char) -> isize {
    static ssize_t loop_attr_dio_show(struct loop_device *lo, char *buf)
    {
    let mut dio: c_int = (lo.lo_flags & LO_FLAGS_DIRECT_IO);
    return sysfs_emit(buf, "%s\n", dio ? "1" : "0");
    }
...
#[no_mangle]
unsafe extern "C" fn loop_sysfs_init(lo: *mut loop_device) {
...
#[no_mangle]
unsafe extern "C" fn loop_sysfs_exit(lo: *mut loop_device) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct loop_worker {
    pub rb_node: rb_node,
    pub work: work_struct,
    pub cmd_list: list_head,
    pub idle_list: list_head,
    pub lo: *mut loop_device,
    pub blkcg_css: *mut cgroup_subsys_state,
    pub last_ran_at: c_ulong,
}

    static void loop_workfn(struct work_struct *work);

#[no_mangle]
pub unsafe extern "C" fn queue_on_root_worker(css: *mut cgroup_subsys_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loop_queue_work(lo: *mut loop_device, cmd: *mut loop_cmd) {
    static void loop_queue_work(struct loop_device *lo, struct loop_cmd *cmd)
    {
    struct rb_node **node, *parent = core::ptr::null_mut();
    struct loop_worker *cur_worker, *worker = core::ptr::null_mut();
...
//
    if (!worker) {
    cmd.blkcg_css = core::ptr::null_mut();
    if (cmd.memcg_css)
    css_put(cmd.memcg_css);
    cmd.memcg_css = core::ptr::null_mut();
    goto queue_work;
    }
...
#[no_mangle]
unsafe extern "C" fn loop_set_timer(lo: *mut loop_device) {
    static void loop_set_timer(struct loop_device *lo)
    {
    timer_reduce(&lo.timer, jiffies + LOOP_IDLE_WORKER_TIMEOUT);
    }
#[no_mangle]
unsafe extern "C" fn loop_free_idle_workers(lo: *mut loop_device, delete_all: bool) {
...
#[no_mangle]
unsafe extern "C" fn loop_free_idle_workers_timer(timer: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn loop_default_blocksize(lo: *mut loop_device) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn loop_set_dma_limit(lo: *mut loop_device, lim: *mut queue_limits) {
...
//
    ...
    struct file *file = lo.lo_backing_file;
    struct inode *inode = file.f_mapping.host;
    struct block_device *backing_bdev = core::ptr::null_mut();
    let mut granularity: u32 = 0, max_discard_sectors = 0;
    if (S_ISBLK(inode.i_mode))
    backing_bdev = I_BDEV(inode);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: inode->i_sb->s_bdev) -> else {
...
//
    ...
    error = bd_prepare_to_claim(bdev, loop_configure, core::ptr::null_mut());
    if (error)
    goto out_putf;
    }
    error = loop_global_lock_killable(lo, is_loop);
...
#[no_mangle]
unsafe extern "C" fn __loop_clr_fd(lo: *mut loop_device) {
    static void __loop_clr_fd(struct loop_device *lo)
    {
    struct queue_limits lim;
    struct file *filp;
    let mut gfp: gfp_t = lo.old_gfp_mask;
    int err;
    spin_lock_irq(&lo.lo_lock);
    filp = lo.lo_backing_file;
    lo.lo_backing_file = core::ptr::null_mut();
    spin_unlock_irq(&lo.lo_lock);
    lo.lo_device = core::ptr::null_mut();
    lo.lo_offset = 0;
    lo.lo_sizelimit = 0;
...
#[no_mangle]
unsafe extern "C" fn loop_clr_fd(lo: *mut loop_device) -> c_int {
...
// update the direct I/O flag if lo_offset changed
    ...
    clear_bit(GD_SUPPRESS_PART_SCAN, &lo.lo_disk.state);
    if (!err && size_changed) {
    let mut new_size: loff_t = lo_calculate_size(lo, lo.lo_backing_file);
...
#[no_mangle]
unsafe extern "C" fn loop_set_capacity(lo: *mut loop_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loop_set_dio(lo: *mut loop_device, arg: c_ulong) -> c_int {
    static int loop_set_dio(struct loop_device *lo, unsigned long arg)
    {
    let mut use_dio: bool = !!arg;
    unsigned int memflags;
    struct queue_limits lim;
    if (lo.lo_state != Lo_bound)
    return -ENXIO;
...
//
    if (!(mode & BLK_OPEN_EXCL)) {
    err = bd_prepare_to_claim(bdev, loop_set_block_size, core::ptr::null_mut());
    if (err)
    return err;
    }
    err = mutex_lock_killable(&lo.lo_mutex);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_loop_info {
    pub /: *mut *mut compat_int_t lo_number; / ioctl r/o,
    pub /: *mut *mut compat_dev_t lo_device; / ioctl r/o,
    pub /: *mut *mut compat_ulong_t lo_inode; / ioctl r/o,
    pub /: *mut *mut compat_dev_t lo_rdevice; / ioctl r/o,
    pub lo_offset: compat_int_t,
    pub /: *mut *mut compat_int_t lo_encrypt_type; / obsolete, ignored,
    pub /: *mut *mut compat_int_t lo_encrypt_key_size; / ioctl w/o,
    pub /: *mut *mut compat_int_t lo_flags; / ioctl r/o,
    pub lo_name: [c_char; LO_NAME_SIZE],
    pub /: *mut *mut unsigned char lo_encrypt_key[LO_KEY_SIZE]; / ioctl w/o,
    pub lo_init: [compat_ulong_t; 2],
...
#[no_mangle]
unsafe extern "C" fn lo_open(disk: *mut gendisk, mode: blk_mode_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lo_release(disk: *mut gendisk) {
...
#[no_mangle]
unsafe extern "C" fn lo_free_disk(disk: *mut gendisk) {
...
// a 'dead' device node.
//
    let mut max_loop: static int = CONFIG_BLK_DEV_LOOP_MIN_COUNT;
    ...
    let mut hw_queue_depth: static int = LOOP_DEFAULT_HW_Q_DEPTH;
#[no_mangle]
unsafe extern "C" fn loop_set_hw_queue_depth(s: *const c_char, p: *const kernel_param) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loop_handle_cmd(cmd: *mut loop_cmd) {
    static void loop_handle_cmd(struct loop_cmd *cmd)
    {
    struct cgroup_subsys_state *cmd_blkcg_css = cmd.blkcg_css;
    struct cgroup_subsys_state *cmd_memcg_css = cmd.memcg_css;
    struct request *rq = blk_mq_rq_from_pdu(cmd);
    let mut write: bool = op_is_write(req_op(rq));
    struct loop_device *lo = rq.q.queuedata;
    let mut ret: c_int = 0;
    struct mem_cgroup *old_memcg = core::ptr::null_mut();
...
// complete non-aio request
    ...
    {
    let mut orig_flags: c_int = current.flags;
    struct loop_cmd *cmd;
...
#[no_mangle]
unsafe extern "C" fn loop_workfn(work: *mut work_struct) {
    static void loop_workfn(struct work_struct *work)
    {
...
#[no_mangle]
unsafe extern "C" fn loop_rootcg_workfn(work: *mut work_struct) {
    static void loop_rootcg_workfn(struct work_struct *work)
    {
    struct loop_device *lo =
    container_of(work, struct loop_device, rootcg_work);
    loop_process_work(core::ptr::null_mut(), &lo.rootcg_cmd_list, lo);
    }
...
#[no_mangle]
unsafe extern "C" fn loop_add(i: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loop_remove(lo: *mut loop_device) {
...
#[no_mangle]
unsafe extern "C" fn loop_probe(dev: dev_t) {
    static void loop_probe(dev_t dev)
    {
    let mut idx: c_int = MINOR(dev) >> part_shift;
...
#[no_mangle]
unsafe extern "C" fn loop_control_remove(idx: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn loop_control_get_free(idx: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn max_loop_setup(str: *mut c_char) -> int __init {
    static int __init max_loop_setup(char *str)
    {
    max_loop = simple_strtol(str, core::ptr::null_mut(), 0);
