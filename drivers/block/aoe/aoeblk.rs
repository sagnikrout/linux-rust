...
//! Source: drivers/block/aoe/aoeblk.c
#![no_std]
...
// random default picked from the historic block max_sectors cap
    let mut aoe_maxsectors: static int = 2560;
...
#[no_mangle]
unsafe extern "C" fn aoe_debugfs_show(s: *mut seq_file, ignored: *mut c_void) -> c_int {
    ...
    list_empty(&(*t).ffree) ? core::ptr::null_mut() : (*t).ffree.next);
    ...
    static DEVICE_ATTR(state, 0444, aoedisk_show_state, core::ptr::null_mut());
    static DEVICE_ATTR(mac, 0444, aoedisk_show_mac, core::ptr::null_mut());
    static DEVICE_ATTR(netif, 0444, aoedisk_show_netif, core::ptr::null_mut());
    static struct device_attribute dev_attr_firmware_version = {
    .attr = { .name = "firmware-version", .mode = 0444 },
    .show = aoedisk_show_fwver,
    };
    static DEVICE_ATTR(payload, 0444, aoedisk_show_payload, core::ptr::null_mut());
    ...
    {
    char *p;
    if (aoe_debugfs_dir == core::ptr::null_mut())
    return;
...
// blk_mq_alloc_disk and add_disk can sleep
    ...
    err = device_add_disk(core::ptr::null_mut(), gd, aoe_attr_groups);
    if (err)
    ...
    aoe_debugfs_dir = core::ptr::null_mut();
    ...
    if (buf_pool_cache == core::ptr::null_mut())
    return -ENOMEM;
    aoe_debugfs_dir = debugfs_create_dir("aoe", core::ptr::null_mut());
    return 0;
    }
