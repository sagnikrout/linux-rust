...
//! Source: drivers/block/aoe/aoedev.c
#![no_std]
...
//
    ...
    let mut aoe_dyndevs: static int = 1;
...
// UP is cleared, freeze+quiesce to insure all are errored
    let mut memflags: c_uint = blk_mq_freeze_queue(d.blkq);
...
// pass three: remove aoedevs marked with DEVFL_FREED
    ...
    struct aoedev *doomed = core::ptr::null_mut();
    spin_lock(&d.lock);
...
// find it or allocate it
    ...
    d.timer.expires = jiffies + HZ;
    add_timer(&d.timer);
    d.bufpool = core::ptr::null_mut();	/* defer to aoeblk_gdalloc */
    d.tgt = d.targets;
    d.ref = 1;
    for (i = 0; i < NFACTIVE; i++)
    ...
    {
    flush_workqueue(aoe_wq);
    flush(core::ptr::null_mut(), 0, EXITING);
    }
    int __init
    aoedev_init(void)
    {
