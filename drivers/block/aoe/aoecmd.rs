...
//! Source: drivers/block/aoe/aoecmd.c
#![no_std]
...
//
    ...
    let mut aoe_deadsecs: static int = 60 * 3;
    module_param(aoe_deadsecs, int, 0644);
    MODULE_PARM_DESC(aoe_deadsecs, "After aoe_deadsecs seconds, give up and fail dev.");
    let mut aoe_maxout: static int = 64;
...
//
    ...
    ifp = t.ifp;
    ifp++;
    if (ifp >= &t.ifs[NAOEIFS] || ifp.nd == core::ptr::null_mut())
    ifp = t.ifs;
    if (ifp.nd == core::ptr::null_mut())
    return core::ptr::null_mut();
    return t.ifp = ifp;
    }
    static void
    ...
    struct frame *f;
    struct aoetgt *t, **tt;
    let mut totout: c_int = 0;
    int use_tainted;
    int has_untainted;
    if (!d.targets || !d.targets[0]) {
    printk(KERN_ERR "aoe: core::ptr::null_mut() TARGETS!\n");
    return core::ptr::null_mut();
    }
...
//
    ...
    n = newtag(d);
    skb = f.skb;
    if (ifrotate(t) == core::ptr::null_mut()) {
// probably can't happen, but set it up to fail anyway
    pr_info("aoe: resend: no interfaces to rotate to.\n");
    ktcomplete(f, core::ptr::null_mut());
    return;
    }
    h = (struct aoe_hdr *) skb_mac_header(skb);
    ...
    {
    let mut delta: u64 = ktime_to_ns(ktime_sub(ktime_get(), f.sent));
...
// timeout based on observed timings and variations
    timeout = rto(d);
    utgts = count_targets(d, core::ptr::null_mut());
...
//
    ...
    return core::ptr::null_mut();	/* initializing */
    if (d.ip.buf)
    return d.ip.buf;
    rq = d.ip.rq;
...
// cf. Congestion Avoidance and Control, Jacobson & Karels, 1988
    ...
    t.maxout += 1;
#[no_mangle]
pub unsafe extern "C" fn if(0: t->nout == t->maxout && t->next_cwnd-- ==) -> else {
    t.maxout += 1;
...
// Figure out if extra threads are required.
    ...
    if (task == core::ptr::null_mut() || IS_ERR(task))
    return -ENOMEM;
    k.task = task;
...
//
    ...
    if (f) {
    calc_rttavg(d, core::ptr::null_mut(), tsince_hr(f));
    } else {
    calc_rttavg(d, core::ptr::null_mut(), tsince(n));
    spin_unlock_irqrestore(&d.lock, flags);
    aoedev_put(d);
