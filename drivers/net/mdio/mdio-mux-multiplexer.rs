...
//! Source: drivers/net/mdio/mdio-mux-multiplexer.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mdio_mux_multiplexer_state {
    pub muxc: *mut mux_control,
    pub do_deselect: bool,
...
#[no_mangle]
unsafe extern "C" fn mdio_mux_multiplexer_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (!s)
    return -ENOMEM;
    s.muxc = devm_mux_control_get(dev, core::ptr::null_mut());
    if (IS_ERR(s.muxc))
...
#[no_mangle]
unsafe extern "C" fn mdio_mux_multiplexer_remove(pdev: *mut platform_device) {
