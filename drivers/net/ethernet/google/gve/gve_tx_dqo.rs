...
//! Source: drivers/net/ethernet/google/gve/gve_tx_dqo.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn gve_has_free_tx_qpl_bufs(tx: *mut gve_tx_ring, count: c_int) -> bool {
...
// Create a linked list of buffers to be added to the free list
    ...
    while (true) {
    let mut old_head: i16 = atomic_read_acquire(&tx.dqo_compl.free_tx_qpl_buf_head);
...
#[no_mangle]
unsafe extern "C" fn gve_has_pending_packet(tx: *mut gve_tx_ring) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn gve_xdp_tx_flush_dqo(priv: *mut gve_priv, xdp_qid: u32) {
    void gve_xdp_tx_flush_dqo(struct gve_priv *priv, u32 xdp_qid)
    {
    let mut tx_qid: u32 = gve_xdp_tx_queue_id(priv, xdp_qid);
...
// Remove pending_packet from free list
    ...
    struct gve_tx_pending_packet_dqo *pending_packet)
    {
    let mut index: i16 = pending_packet - tx.dqo.pending_packets;
    pending_packet.state = GVE_PACKET_STATE_UNALLOCATED;
    while (true) {
    let mut old_head: i16 = atomic_read_acquire(&tx.dqo_compl.free_pending_packets);
...
//
#[no_mangle]
unsafe extern "C" fn gve_tx_clean_pending_packets(tx: *mut gve_tx_ring) {
    ...
    cur_state.skb = core::ptr::null_mut();
    }
    }
    }
#[no_mangle]
pub unsafe extern "C" fn gve_tx_stop_ring_dqo(priv: *mut gve_priv, idx: c_int) {
    ...
    gve_clean_tx_done_dqo(priv, tx, /*napi=*/core::ptr::null_mut());
    if (tx.netdev_txq)
    ...
    let mut idx: c_int = tx.q_num;
    size_t bytes;
    u32 qpl_id;
    if (tx.q_resources) {
    dma_free_coherent(hdev, sizeof(*tx.q_resources),
    tx.q_resources, tx.q_resources_bus);
    tx.q_resources = core::ptr::null_mut();
    }
    ...
    tx.complq_bus_dqo);
    tx.dqo.compl_ring = core::ptr::null_mut();
    }
    if (tx.dqo.tx_ring) {
    bytes = sizeof(tx.dqo.tx_ring[0]) * (tx.mask + 1);
    dma_free_coherent(hdev, bytes, tx.dqo.tx_ring, tx.bus);
    tx.dqo.tx_ring = core::ptr::null_mut();
    }
    kvfree(tx.dqo.xsk_reorder_queue);
    tx.dqo.xsk_reorder_queue = core::ptr::null_mut();
    kvfree(tx.dqo.pending_packets);
    tx.dqo.pending_packets = core::ptr::null_mut();
    kvfree(tx.dqo.tx_qpl_buf_next);
    tx.dqo.tx_qpl_buf_next = core::ptr::null_mut();
    if (tx.dqo.qpl) {
    qpl_id = gve_tx_qpl_id(priv, tx.q_num);
    gve_free_queue_page_list(priv, tx.dqo.qpl, qpl_id);
    tx.dqo.qpl = core::ptr::null_mut();
    }
    netif_dbg(priv, drv, priv.dev, "freed tx queue %d\n", idx);
    }
#[no_mangle]
unsafe extern "C" fn gve_tx_qpl_buf_init(tx: *mut gve_tx_ring) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn gve_tx_start_ring_dqo(priv: *mut gve_priv, idx: c_int) {
...
// Only alloc xsk pool for XDP queues
    ...
    kvfree(tx);
    cfg.tx = core::ptr::null_mut();
    }
// Returns the number of slots available in the ring
#[no_mangle]
unsafe extern "C" fn num_avail_tx_slots(tx: *const gve_tx_ring) -> u32 {
    static u32 num_avail_tx_slots(const struct gve_tx_ring *tx)
    {
    let mut num_used: u32 = (tx.dqo_tx.tail - tx.dqo_tx.head) & tx.mask;
    return tx.mask - num_used;
    }
// Checks if the requested number of slots are available in the ring
#[no_mangle]
unsafe extern "C" fn gve_has_tx_slots_available(tx: *mut gve_tx_ring, slots_req: u32) -> bool {
    static bool gve_has_tx_slots_available(struct gve_tx_ring *tx, u32 slots_req)
    {
    let mut num_avail: u32 = num_avail_tx_slots(tx);
...
//
    ...
    if (skb.l4_hash) {
    let mut path_hash: u16 = skb.hash ^ (skb.hash >> 16);
    path_hash &= (1 << 15) - 1;
    ...
    let mut cur_len: u32 = min_t(u32, len, GVE_TX_MAX_BUF_SIZE_DQO);
    let mut cur_eop: bool = eop && cur_len == len;
...
//
#[no_mangle]
unsafe extern "C" fn gve_prep_tso(skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gve_tx_update_tail(tx: *mut gve_tx_ring, desc_idx: u32) {
    static void gve_tx_update_tail(struct gve_tx_ring *tx, u32 desc_idx)
    {
    let mut last_desc_idx: u32 = (desc_idx - 1) & tx.mask;
...
//
    ...
    bool is_gso)
    {
    let mut enable_csum: bool = skb.ip_summed == CHECKSUM_PARTIAL;
...
// Map the linear portion of skb
    {
    let mut len: u32 = skb_headlen(skb);
    dma_addr_t addr;
...
// eop=*/shinfo->nr_frags == 0, is_gso);
    ...
    let mut is_eop: bool = i == (shinfo.nr_frags - 1);
...
//
    ...
    {
    let mut page_id: c_int = index >> (PAGE_SHIFT - GVE_TX_BUF_SHIFT_DQO);
    let mut offset: c_int = (index & (GVE_TX_BUFS_PER_PAGE_DQO - 1)) << GVE_TX_BUF_SHIFT_DQO;
...
//
    ...
    struct sk_buff *skb)
    {
    let mut is_gso: bool = skb_is_gso(skb);
    let mut desc_idx: u32 = tx.dqo_tx.tail;
    struct gve_tx_pending_packet_dqo *pkt;
    struct gve_tx_metadata_dqo metadata;
    s16 completion_tag;
    ...
    if (is_gso) {
    let mut header_len: c_int = gve_prep_tso(skb);
    ...
    return 0;
    err:
    pkt.skb = core::ptr::null_mut();
    gve_free_pending_packet(tx, pkt);
    return -1;
    }
#[no_mangle]
unsafe extern "C" fn gve_num_descs_per_buf(size: usize) -> c_int {
    static int gve_num_descs_per_buf(size_t size)
    {
    return DIV_ROUND_UP(size, GVE_TX_MAX_BUF_SIZE_DQO);
    }
#[no_mangle]
unsafe extern "C" fn gve_num_buffer_descs_needed(skb: *const sk_buff) -> c_int {
    ...
    for (i = 0; i < shinfo.nr_frags; i++) {
    let mut frag_size: c_uint = skb_frag_size(&shinfo.frags[i]);
...
//
#[no_mangle]
unsafe extern "C" fn gve_can_send_tso(skb: *const sk_buff) -> bool {
    static bool gve_can_send_tso(const struct sk_buff *skb)
    {
    let mut max_bufs_per_seg: c_int = GVE_TX_MAX_DATA_DESCS - 1;
    const struct skb_shared_info *shinfo = skb_shinfo(skb);
    let mut header_len: c_int = skb_tcp_all_headers(skb);
    let mut gso_size: c_int = shinfo.gso_size;
...
// Metadata + (optional TSO) + data descriptors.
    ...
    u16 completion_tag)
    {
    let mut tail: u32 = atomic_read(&tx.dqo_tx.xsk_reorder_queue_tail);
    ...
    {
    let mut head: u32 = tx.dqo_compl.xsk_reorder_queue_head;
...
#[no_mangle]
unsafe extern "C" fn gve_xsk_reorder_queue_pop_dqo(tx: *mut gve_tx_ring) {
...
#[no_mangle]
pub unsafe extern "C" fn gve_tx_dqo(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
...
// bytes += pending_packet->skb->len;
    ...
    pending_packet.skb = core::ptr::null_mut();
    gve_free_pending_packet(tx, pending_packet);
    break;
...
// bytes += pending_packet->xdpf->len;
    ...
    pending_packet.xdpf = core::ptr::null_mut();
    gve_free_pending_packet(tx, pending_packet);
    break;
...
#[no_mangle]
unsafe extern "C" fn gve_tx_process_xsk_completions(tx: *mut gve_tx_ring) {
    static void gve_tx_process_xsk_completions(struct gve_tx_ring *tx)
    {
    let mut num_xsks: u32 = 0;
    while (true) {
    struct gve_tx_pending_packet_dqo *pending_packet =
    ...
    {
    let mut reinject_compl_bytes: u64 = 0;
    let mut reinject_compl_pkts: u64 = 0;
    let mut num_descs_cleaned: c_int = 0;
    let mut miss_compl_bytes: u64 = 0;
    let mut miss_compl_pkts: u64 = 0;
    let mut pkt_compl_bytes: u64 = 0;
    let mut pkt_compl_pkts: u64 = 0;
...
// This is the last descriptor fetched by HW plus one
    let mut tx_head: u16 = le16_to_cpu(compl_desc.tx_head);
    atomic_set_release(&tx.dqo_compl.hw_tx_head, tx_head);
    } else if (type == GVE_COMPL_TYPE_DQO_PKT) {
    let mut compl_tag: u16 = le16_to_cpu(compl_desc.completion_tag);
...
#[no_mangle]
pub unsafe extern "C" fn gve_tx_poll_dqo(block: *mut gve_notify_block, do_clean: bool) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn gve_xsk_tx_poll_dqo(rx_block: *mut gve_notify_block, budget: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn gve_xdp_poll_dqo(block: *mut gve_notify_block) -> bool {
