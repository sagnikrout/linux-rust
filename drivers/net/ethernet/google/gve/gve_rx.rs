...
//! Source: drivers/net/ethernet/google/gve/gve_rx.c
#![no_std]
...
//
    ...
    rx.data.page_info = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn gve_rx_ctx_clear(ctx: *mut gve_rx_ctx) {
    static void gve_rx_ctx_clear(struct gve_rx_ctx *ctx)
    {
    ctx.skb_head = core::ptr::null_mut();
    ctx.skb_tail = core::ptr::null_mut();
    ctx.total_size = 0;
    ctx.frag_cnt = 0;
    ctx.drop_pkt = false;
    }
#[no_mangle]
unsafe extern "C" fn gve_rx_init_ring_state_gqi(rx: *mut gve_rx_ring) {
...
#[no_mangle]
unsafe extern "C" fn gve_rx_reset_ring_gqi(priv: *mut gve_priv, idx: c_int) {
    static void gve_rx_reset_ring_gqi(struct gve_priv *priv, int idx)
    {
    struct gve_rx_ring *rx = &priv.rx[idx];
    let mut slots: u32 = priv.rx_desc_cnt;
    size_t size;
// Reset desc ring
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_stop_ring_gqi(priv: *mut gve_priv, idx: c_int) {
    ...
    let mut slots: u32 = rx.mask + 1;
    let mut idx: c_int = rx.q_num;
    size_t bytes;
    u32 qpl_id;
    if (rx.desc.desc_ring) {
    bytes = sizeof(struct gve_rx_desc) * cfg.ring_size;
    dma_free_coherent(dev, bytes, rx.desc.desc_ring, rx.desc.bus);
    rx.desc.desc_ring = core::ptr::null_mut();
    }
    if (rx.q_resources) {
    dma_free_coherent(dev, sizeof(*rx.q_resources),
    rx.q_resources, rx.q_resources_bus);
    rx.q_resources = core::ptr::null_mut();
    }
    ...
    rx.data.data_bus);
    rx.data.data_ring = core::ptr::null_mut();
    }
    kvfree(rx.qpl_copy_pool);
    rx.qpl_copy_pool = core::ptr::null_mut();
    if (rx.data.qpl) {
    qpl_id = gve_get_rx_qpl_id(cfg.qcfg_tx, idx);
    gve_free_queue_page_list(priv, rx.data.qpl, qpl_id);
    rx.data.qpl = core::ptr::null_mut();
    }
...
//
    ...
    let mut addr: dma_addr_t = i * PAGE_SIZE;
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_start_ring_gqi(priv: *mut gve_priv, idx: c_int) {
    ...
    let mut slots: u32 = cfg.ring_size;
    int filled_pages;
    let mut qpl_id: u32 = 0;
    size_t bytes;
    int err;
...
// alloc rx desc ring
    ...
    kvfree(rx);
    cfg.rx = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn gve_rx_write_doorbell(priv: *mut gve_priv, rx: *mut gve_rx_ring) {
    void gve_rx_write_doorbell(struct gve_priv *priv, struct gve_rx_ring *rx)
    {
    let mut db_idx: u32 = be32_to_cpu(rx.q_resources.db_index);
    iowrite32be(rx.fill_cnt, &priv.db_bar2[db_idx]);
    }
#[no_mangle]
unsafe extern "C" fn gve_rss_type(pkt_flags: __be16) -> enum pkt_hash_types {
    ...
    {
    let mut offset: u32 = page_info.page_offset + page_info.pad;
    struct sk_buff *skb = ctx.skb_tail;
    let mut num_frags: c_int = 0;
    if (!skb) {
...
#[no_mangle]
unsafe extern "C" fn gve_rx_flip_buff(page_info: *mut gve_rx_slot_page_info, slot_addr: *mut __be64) {
    static void gve_rx_flip_buff(struct gve_rx_slot_page_info *page_info, __be64 *slot_addr)
    {
    let mut offset: __be64 = cpu_to_be64(GVE_DEFAULT_RX_BUFFER_OFFSET);
// "flip" to other packet buffer on this page
    page_info.page_offset ^= GVE_DEFAULT_RX_BUFFER_OFFSET;
// (slot_addr) ^= offset;
    }
#[no_mangle]
unsafe extern "C" fn gve_rx_can_recycle_buffer(page_info: *mut gve_rx_slot_page_info) -> c_int {
    static int gve_rx_can_recycle_buffer(struct gve_rx_slot_page_info *page_info)
    {
    let mut pagecount: c_int = page_count(page_info.page);
// This page is not being used by any SKBs - reuse
    if (pagecount == page_info.pagecnt_bias)
    return 1;
// This page is still being used by an SKB - we can't reuse
#[no_mangle]
pub unsafe extern "C" fn if(page_info->pagecnt_bias: pagecount >) -> else {
...
// Just copy small packets
    ...
    let mut recycle: c_int = gve_rx_can_recycle_buffer(page_info);
    if (unlikely(recycle < 0)) {
    ...
    let mut headroom: c_int = XDP_PACKET_HEADROOM;
    struct xdp_buff new;
    void *frame;
    int err;
    if (rx.xsk_pool)
    ...
    xdp.data_end - xdp.data, core::ptr::null_mut());
    ...
    {
    let mut is_last_frag: bool = !GVE_PKTCONT_BIT_IS_SET(desc.flags_seq);
    struct gve_rx_slot_page_info *page_info;
    let mut frag_size: u16 = be16_to_cpu(desc.len);
    struct gve_rx_ctx *ctx = &rx.ctx;
    union gve_rx_data_slot *data_slot;
    struct gve_priv *priv = rx.gve;
    struct sk_buff *skb = core::ptr::null_mut();
    ...
    dma_addr_t page_bus;
    void *va;
    let mut len: u16 = frag_size;
    struct napi_struct *napi = &priv.ntfy_blocks[rx.ntfy_id].napi;
    let mut is_first_frag: bool = ctx.frag_cnt == 0;
    let mut is_only_frag: bool = is_first_frag && is_last_frag;
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_work_pending(rx: *mut gve_rx_ring) -> bool {
...
#[no_mangle]
unsafe extern "C" fn gve_rx_refill_buffers(priv: *mut gve_priv, rx: *mut gve_rx_ring) -> bool {
    static bool gve_rx_refill_buffers(struct gve_priv *priv, struct gve_rx_ring *rx)
    {
    let mut refill_target: c_int = rx.mask + 1;
    let mut fill_cnt: u32 = rx.fill_cnt;
    while (fill_cnt - rx.cnt < refill_target) {
    struct gve_rx_slot_page_info *page_info;
    let mut idx: u32 = fill_cnt & rx.mask;
    page_info = &rx.data.page_info[idx];
    if (page_info.can_flip) {
...
// We can't reuse the buffer - alloc a new one
    ...
    {
    let mut xdp_redirects: u64 = rx.xdp_actions[XDP_REDIRECT];
    let mut xdp_txs: u64 = rx.xdp_actions[XDP_TX];
    struct gve_rx_ctx *ctx = &rx.ctx;
    struct gve_priv *priv = rx.gve;
    let mut cnts: gve_rx_cnts = {0};
    struct gve_rx_desc *next_desc;
    let mut idx: u32 = rx.cnt & rx.mask;
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_poll(block: *mut gve_notify_block, budget: c_int) -> c_int {
