...
//! Source: drivers/net/ethernet/google/gve/gve_rx_dqo.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn gve_rx_starvation_timer(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn gve_rx_free_hdr_bufs(priv: *mut gve_priv, rx: *mut gve_rx_ring) {
    ...
    rx.dqo.hdr_bufs.data = core::ptr::null_mut();
    }
    }
...
// Set RX SKB context
    rx.ctx.skb_head = core::ptr::null_mut();
    rx.ctx.skb_tail = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn gve_rx_reset_ring_dqo(priv: *mut gve_priv, idx: c_int) {
    static void gve_rx_reset_ring_dqo(struct gve_priv *priv, int idx)
    {
    struct gve_rx_ring *rx = &priv.rx[idx];
    size_t size;
    int i;
    let mut buffer_queue_slots: u32 = priv.rx_desc_cnt;
    let mut completion_queue_slots: u32 = priv.rx_desc_cnt;
// Reset buffer queue
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_stop_ring_dqo(priv: *mut gve_priv, idx: c_int) {
    ...
    rx.dqo.complq.bus);
    rx.dqo.complq.desc_ring = core::ptr::null_mut();
    }
    kvfree(rx.dqo.buf_states);
    rx.dqo.buf_states = core::ptr::null_mut();
    if (rx.dqo.page_pool) {
    page_pool_destroy(rx.dqo.page_pool);
    rx.dqo.page_pool = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_start_ring_dqo(priv: *mut gve_priv, idx: c_int) {
    ...
    size_t size;
    u32 qpl_id;
    let mut buffer_queue_slots: u32 = cfg.ring_size;
    let mut completion_queue_slots: u32 = cfg.ring_size;
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_write_doorbell_dqo(priv: *const gve_priv, queue_idx: c_int) {
    void gve_rx_write_doorbell_dqo(const struct gve_priv *priv, int queue_idx)
    {
    const struct gve_rx_ring *rx = &priv.rx[queue_idx];
    let mut index: u64 = be32_to_cpu(rx.q_resources.db_index);
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_post_buffers_dqo(rx: *mut gve_rx_ring) {
    ...
    if (rx.dqo.hdr_bufs.data) {
    let mut buf_id: u16 = le16_to_cpu(desc.buf_id);
...
// Checksum should be skipped if this flag is set.
    ...
    struct gve_ptype ptype)
    {
    let mut hash_type: enum pkt_hash_types = PKT_HASH_TYPE_L2;
    if (ptype.l4_type != GVE_L4_TYPE_UNKNOWN)
    hash_type = PKT_HASH_TYPE_L4;
#[no_mangle]
pub unsafe extern "C" fn if(GVE_L3_TYPE_UNKNOWN: ptype.l3_type !=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn gve_rx_get_hwtstamp(gve: *mut gve_priv, hwts: u32) -> ktime_t {
    static ktime_t gve_rx_get_hwtstamp(struct gve_priv *gve, u32 hwts)
    {
    let mut last_read: u64 = READ_ONCE(gve.last_sync_nic_counter);
    let mut low: u32 = (u32)last_read;
    let mut diff: i32 = hwts - low;
...
#[no_mangle]
pub unsafe extern "C" fn gve_xdp_rx_timestamp(_ctx: *const xdp_md, timestamp: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gve_rx_free_skb(napi: *mut napi_struct, rx: *mut gve_rx_ring) {
    static void gve_rx_free_skb(struct napi_struct *napi, struct gve_rx_ring *rx)
    {
    if (!rx.ctx.skb_head)
    return;
    if (rx.ctx.skb_head == napi.skb)
    napi.skb = core::ptr::null_mut();
    dev_kfree_skb_any(rx.ctx.skb_head);
    rx.ctx.skb_head = core::ptr::null_mut();
    rx.ctx.skb_tail = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn gve_rx_should_trigger_copy_ondemand(rx: *mut gve_rx_ring) -> bool {
...
//
    ...
    {
    let mut num_frags: c_int = skb_shinfo(rx.ctx.skb_tail).nr_frags;
...
// Trigger ondemand page allocation if we are running low on buffers
    ...
    rx.xdp_tx_errors++;
#[no_mangle]
pub unsafe extern "C" fn if(XDP_REDIRECT: xdp_act ==) -> else {
    ...
    let mut buf_len: c_int = compl_desc.packet_len;
    struct gve_priv *priv = rx.gve;
...
//
    ...
    {
    let mut buffer_id: u16 = le16_to_cpu(compl_desc.buf_id);
    let mut hbo: bool = compl_desc.header_buffer_overflow;
    let mut eop: bool = compl_desc.end_of_packet != 0;
    let mut hsplit: bool = compl_desc.split_header;
    struct gve_rx_buf_state_dqo *buf_state;
    struct gve_priv *priv = rx.gve;
    struct bpf_prog *xprog;
    u16 buf_len;
...
#[no_mangle]
pub unsafe extern "C" fn gve_rx_poll_dqo(block: *mut gve_notify_block, budget: c_int) -> c_int {
