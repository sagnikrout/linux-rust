...
//! Source: drivers/net/ethernet/google/gve/gve_tx.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn gve_xdp_tx_flush(priv: *mut gve_priv, xdp_qid: u32) {
...
//
#[no_mangle]
unsafe extern "C" fn gve_tx_fifo_init(priv: *mut gve_priv, fifo: *mut gve_tx_fifo) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gve_tx_fifo_release(priv: *mut gve_priv, fifo: *mut gve_tx_fifo) {
...
#[no_mangle]
unsafe extern "C" fn gve_tx_fifo_can_alloc(fifo: *mut gve_tx_fifo, bytes: usize) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn gve_tx_free_fifo(fifo: *mut gve_tx_fifo, bytes: usize) {
    static void gve_tx_free_fifo(struct gve_tx_fifo *fifo, size_t bytes)
    {
    atomic_add(bytes, &fifo.available);
    }
#[no_mangle]
unsafe extern "C" fn gve_tx_clear_buffer_state(info: *mut gve_tx_buffer_state) -> usize {
    static size_t gve_tx_clear_buffer_state(struct gve_tx_buffer_state *info)
    {
    let mut space_freed: usize = 0;
    int i;
    ...
    let mut pkts: u64 = 0, bytes = 0;
    let mut space_freed: usize = 0;
    let mut xsk_complete: u32 = 0;
    u32 idx;
    int i;
    for (i = 0; i < to_do; i++) {
    idx = tx.done & tx.mask;
    ...
    info.xdp_frame = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn gve_tx_stop_ring_gqi(priv: *mut gve_priv, idx: c_int) {
    ...
    tx.tx_fifo.qpl = core::ptr::null_mut();
    }
    bytes = sizeof(*tx.desc) * slots;
    dma_free_coherent(hdev, bytes, tx.desc, tx.bus);
    tx.desc = core::ptr::null_mut();
    vfree(tx.info);
    tx.info = core::ptr::null_mut();
    netif_dbg(priv, drv, priv.dev, "freed tx queue %d\n", idx);
    }
#[no_mangle]
pub unsafe extern "C" fn gve_tx_start_ring_gqi(priv: *mut gve_priv, idx: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn gve_tx_avail(tx: *mut gve_tx_ring) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn gve_tx_unmap_buf(dev: *mut device, info: *mut gve_tx_buffer_state) {
...
//
#[no_mangle]
pub unsafe extern "C" fn gve_can_tx(tx: *mut gve_tx_ring, bytes_required: c_int) -> bool {
    static inline bool gve_can_tx(struct gve_tx_ring *tx, int bytes_required)
    {
    let mut can_alloc: bool = true;
    if (!tx.raw_addressing)
...
// Stops the queue if the skb cannot be transmitted.
    ...
    struct sk_buff *skb)
    {
    let mut bytes_required: c_int = 0;
    u32 nic_done;
    u32 to_do;
    int ret;
    if (!tx.raw_addressing)
...
// l4_hdr_offset and csum_offset are in units of 16-bit words
    ...
    {
    let mut last_page: u64 = (iov_offset + iov_len - 1) / PAGE_SIZE;
    let mut first_page: u64 = iov_offset / PAGE_SIZE;
    u64 page;
    for (page = first_page; page <= last_page; page++)
    dma_sync_single_for_device(dev, page_buses[page], PAGE_SIZE, DMA_TO_DEVICE);
    }
#[no_mangle]
unsafe extern "C" fn gve_tx_add_skb_copy(priv: *mut gve_priv, tx: *mut gve_tx_ring, skb: *mut sk_buff) -> c_int {
    static int gve_tx_add_skb_copy(struct gve_priv *priv, struct gve_tx_ring *tx, struct sk_buff *skb)
    {
    int pad_bytes, hlen, hdr_nfrags, payload_nfrags, l4_hdr_offset;
    union gve_tx_desc *pkt_desc, *seg_desc;
    struct gve_tx_buffer_state *info;
    let mut mtd_desc_nr: c_int = !!skb.l4_hash;
    let mut is_gso: bool = skb_is_gso(skb);
    let mut idx: u32 = tx.req & tx.mask;
    let mut payload_iov: c_int = 2;
    int copy_offset;
    u32 next_idx;
    int i;
    info = &tx.info[idx];
...
#[no_mangle]
pub unsafe extern "C" fn gve_tx(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
...
pub const GVE_TX_START_THRESH: c_int = 4096;
    static int gve_clean_tx_done(struct gve_priv *priv, struct gve_tx_ring *tx,
    u32 to_do, bool try_to_wake)
    {
    struct gve_tx_buffer_state *info;
...
// Make sure that the doorbells are synced
    ...
    struct gve_tx_ring *tx)
    {
    let mut counter_index: u32 = be32_to_cpu(tx.q_resources.counter_index);
    let mut counter: __be32 = READ_ONCE(priv.counter_array[counter_index]);
    ...
    nsegs = gve_tx_fill_xdp(priv, tx, data, desc.len, core::ptr::null_mut(), true);
    tx.req += nsegs;
    sent++;
    }
    out:
...
#[no_mangle]
pub unsafe extern "C" fn gve_xsk_tx_poll(rx_block: *mut gve_notify_block, budget: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn gve_xdp_poll(block: *mut gve_notify_block, budget: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn gve_tx_poll(block: *mut gve_notify_block, budget: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn gve_tx_clean_pending(priv: *mut gve_priv, tx: *mut gve_tx_ring) -> bool {
    bool gve_tx_clean_pending(struct gve_priv *priv, struct gve_tx_ring *tx)
    {
    let mut nic_done: u32 = gve_tx_load_event_counter(priv, tx);
    return nic_done != tx.done;
    }
