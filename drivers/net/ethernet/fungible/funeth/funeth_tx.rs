...
//! Source: drivers/net/ethernet/fungible/funeth/funeth_tx.c
#![no_std]
...
pub const FUN_XDP_CLEAN_THRES: c_int = 32;
pub const FUN_XDP_CLEAN_BATCH: c_int = 16;
...
//
#[no_mangle]
unsafe extern "C" fn txq_to_end(q: *const funeth_txq, p: *mut c_void) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn tx_req_ndesc(req: *const fun_eth_tx_req) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn tcp_hdr_doff_flags(th: *const tcphdr) -> __be16 {
...
//
    ...
    unsigned int tls_len)
    {
    let mut extra_bytes: c_uint = 0, extra_pkts = 0;
    let mut idx: c_uint = q.prod_cnt & q.mask;
...
//
#[no_mangle]
unsafe extern "C" fn fun_txq_avail(q: *const funeth_txq) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn fun_tx_check_stop(q: *mut funeth_txq) {
...
//
#[no_mangle]
unsafe extern "C" fn fun_txq_may_restart(q: *mut funeth_txq) -> bool {
    static bool fun_txq_may_restart(struct funeth_txq *q)
    {
    return fun_txq_avail(q) >= q.mask / 4;
    }
#[no_mangle]
pub unsafe extern "C" fn fun_start_xmit(skb: *mut sk_buff, netdev: *mut net_device) -> netdev_tx_t {
    netdev_tx_t fun_start_xmit(struct sk_buff *skb, struct net_device *netdev)
    {
    struct funeth_priv *fp = netdev_priv(netdev);
    let mut qid: c_uint = skb_get_queue_mapping(skb);
    struct funeth_txq *q = fp.txqs[qid];
    let mut tls_len: c_uint = 0;
    unsigned int ndesc;
...
#[no_mangle]
unsafe extern "C" fn txq_hw_head(q: *const funeth_txq) -> u16 {
...
//
#[no_mangle]
unsafe extern "C" fn fun_unmap_pkt(q: *const funeth_txq, idx: c_uint) -> c_uint {
    ...
    let mut ngle: c_uint = req.dataop.ngather;
    struct fun_dataop_gl *gle;
    if (ngle) {
...
//
#[no_mangle]
unsafe extern "C" fn fun_txq_reclaim(q: *mut funeth_txq, budget: c_int) -> bool {
    static bool fun_txq_reclaim(struct funeth_txq *q, int budget)
    {
    let mut npkts: c_uint = 0, nbytes = 0, ndesc = 0;
...
//
    rmb();
    do {
    let mut pkt_desc: c_uint = fun_unmap_pkt(q, reclaim_idx);
...
#[no_mangle]
pub unsafe extern "C" fn fun_txq_napi_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_xdpq_clean(q: *mut funeth_txq, budget: c_uint) -> c_uint {
    static unsigned int fun_xdpq_clean(struct funeth_txq *q, unsigned int budget)
    {
    let mut npkts: c_uint = 0, ndesc = 0, head, reclaim_idx;
    for (head = txq_hw_head(q), reclaim_idx = q.cons_cnt & q.mask;
...
#[no_mangle]
pub unsafe extern "C" fn fun_xdp_tx(q: *mut funeth_txq, xdpf: *mut xdp_frame) -> bool {
    bool fun_xdp_tx(struct funeth_txq *q, struct xdp_frame *xdpf)
    {
    unsigned int idx, nfrags = 1, ndesc = 1, tot_len = xdpf.len;
    const struct skb_shared_info *si = core::ptr::null_mut();
    unsigned int lens[MAX_SKB_FRAGS + 1];
...
//
#[no_mangle]
unsafe extern "C" fn fun_txq_purge(q: *mut funeth_txq) {
    static void fun_txq_purge(struct funeth_txq *q)
    {
    while (q.cons_cnt != q.prod_cnt) {
    let mut idx: c_uint = q.cons_cnt & q.mask;
...
#[no_mangle]
unsafe extern "C" fn fun_xdpq_purge(q: *mut funeth_txq) {
...
#[no_mangle]
unsafe extern "C" fn fun_txq_free_sw(q: *mut funeth_txq) {
...
#[no_mangle]
pub unsafe extern "C" fn fun_txq_create_dev(q: *mut funeth_txq, irq: *mut fun_irq) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_txq_free_dev(q: *mut funeth_txq) {
