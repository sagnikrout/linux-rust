...
//! Source: drivers/net/ethernet/fungible/funeth/funeth_rx.c
#![no_std]
...
//
pub const RX_MAX_FRAGS: c_int = 4;
...
//
pub const EXTRA_PAGE_REFS: c_int = 1000000;
pub const MIN_PAGE_REFS: c_int = 1000;
    enum {
...
#[no_mangle]
unsafe extern "C" fn refresh_refs(buf: *mut funeth_rxbuf) {
...
//
#[no_mangle]
unsafe extern "C" fn cache_offer(q: *mut funeth_rxq, buf: *const funeth_rxbuf) {
...
//
#[no_mangle]
unsafe extern "C" fn cache_get(q: *mut funeth_rxq, rb: *mut funeth_rxbuf) -> bool {
...
#[no_mangle]
unsafe extern "C" fn funeth_free_page(q: *mut funeth_rxq, rb: *mut funeth_rxbuf) {
...
//
#[no_mangle]
unsafe extern "C" fn cqe_to_pkt_hash_type(pkt_parse: u16) -> enum pkt_hash_types {
...
//
    ...
    cache_offer(q, buf);
// buf = q->spare_buf;
    q.spare_buf.page = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn rx_hwtstamp_enabled(dev: *const net_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn advance_cq(q: *mut funeth_rxq) {
...
//
#[no_mangle]
unsafe extern "C" fn fun_handle_cqe_pkt(q: *mut funeth_rxq, xdp_q: *mut funeth_txq) {
...
#[no_mangle]
pub unsafe extern "C" fn if(GRO_HELD: gro_res ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn cqe_phase_mismatch(ci: *const fun_cqe_info, phase: u16) -> u16 {
    static u16 cqe_phase_mismatch(const struct fun_cqe_info *ci, u16 phase)
    {
    let mut sf_p: u16 = be16_to_cpu(ci.sf_p);
    return (sf_p & 1) ^ phase;
    }
...
//
#[no_mangle]
unsafe extern "C" fn fun_process_cqes(q: *mut funeth_rxq, budget: c_int) -> c_int {
    static int fun_process_cqes(struct funeth_rxq *q, int budget)
    {
    struct funeth_priv *fp = netdev_priv(q.netdev);
    struct funeth_txq **xdpqs, *xdp_q = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn fun_rxq_napi_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    int fun_rxq_napi_poll(struct napi_struct *napi, int budget)
    {
    struct fun_irq *irq = container_of(napi, struct fun_irq, napi);
    struct funeth_rxq *q = irq.rxq;
    let mut work_done: c_int = budget - fun_process_cqes(q, budget);
    let mut cq_db_val: u32 = q.cq_head;
    if (unlikely(work_done >= budget))
    FUN_QSTAT_INC(q, rx_budget);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: napi_complete_done(napi, _arg: work_done)) -> else {
...
#[no_mangle]
unsafe extern "C" fn fun_rxq_free_bufs(q: *mut funeth_rxq) {
    ...
    q.cur_buf = core::ptr::null_mut();
    }
// Initially provision an Rx queue with Rx buffers.
#[no_mangle]
unsafe extern "C" fn fun_rxq_alloc_bufs(q: *mut funeth_rxq, node: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_rxq_free_cache(q: *mut funeth_rxq) {
    ...
    funeth_free_page(q, b);
    kvfree(q.cache.bufs);
    q.cache.bufs = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn fun_rxq_set_bpf(q: *mut funeth_rxq, prog: *mut bpf_prog) -> c_int {
    ...
    err = fun_submit_admin_sync_cmd(fp.fdev, &cmd.common, core::ptr::null_mut(), 0,
    0);
    if (err)
    return err;
    q.headroom = headroom;
...
// Create an Rx queue, allocating the host memory it needs.
    ...
    &q.rq_dma_addr, (void **)&q.bufs, core::ptr::null_mut());
    if (!q.rqes)
    goto free_q;
    q.cqes = fun_alloc_ring_mem(q.dma_dev, ncqe, FUNETH_CQE_SIZE, 0,
    false, numa_node, &q.cq_dma_addr, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (!q.cqes)
    goto free_rqes;
...
#[no_mangle]
unsafe extern "C" fn fun_rxq_free_sw(q: *mut funeth_rxq) {
...
#[no_mangle]
pub unsafe extern "C" fn fun_rxq_create_dev(q: *mut funeth_rxq, irq: *mut fun_irq) -> c_int {
    int fun_rxq_create_dev(struct funeth_rxq *q, struct fun_irq *irq)
    {
    struct funeth_priv *fp = netdev_priv(q.netdev);
    let mut ncqe: c_uint = q.cq_mask + 1;
    let mut nrqe: c_uint = q.rq_mask + 1;
    int err;
...
#[no_mangle]
unsafe extern "C" fn fun_rxq_free_dev(q: *mut funeth_rxq) {
