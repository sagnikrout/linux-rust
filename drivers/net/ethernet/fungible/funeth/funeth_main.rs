...
//! Source: drivers/net/ethernet/fungible/funeth/funeth_main.c
#![no_std]
...
pub const ADMIN_SQ_DEPTH: c_int = 32;
pub const ADMIN_CQ_DEPTH: c_int = 64;
pub const ADMIN_RQ_DEPTH: c_int = 16;
...
#[no_mangle]
pub unsafe extern "C" fn fun_port_write_cmd(fp: *mut funeth_priv, key: c_int, data: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn fun_port_read_cmd(fp: *mut funeth_priv, key: c_int, data: *mut u64) -> c_int {
    int fun_port_read_cmd(struct funeth_priv *fp, int key, u64 *data)
    {
    return fun_port_read_cmds(fp, 1, &key, data);
    }
#[no_mangle]
unsafe extern "C" fn fun_report_link(netdev: *mut net_device) {
    static void fun_report_link(struct net_device *netdev)
    {
    if (netif_carrier_ok(netdev)) {
    const struct funeth_priv *fp = netdev_priv(netdev);
    const char *fec = "", *pause = "";
    let mut speed: c_int = fp.link_speed;
    let mut unit: c_char = 'M';
    ...
    fec = ", RS-FEC";
#[no_mangle]
pub unsafe extern "C" fn if(FUN_PORT_FEC_FC: fp->active_fec &) -> else {
    else if (fp.active_fec & FUN_PORT_FEC_FC)
    fec = ", BASER-FEC";
    if ((fp.active_fc & FUN_PORT_CAP_PAUSE_MASK) == FUN_PORT_CAP_PAUSE_MASK)
    pause = ", Tx/Rx PAUSE";
#[no_mangle]
pub unsafe extern "C" fn if(FUN_PORT_CAP_RX_PAUSE: fp->active_fc &) -> else {
    else if (fp.active_fc & FUN_PORT_CAP_RX_PAUSE)
    pause = ", Rx PAUSE";
#[no_mangle]
pub unsafe extern "C" fn if(FUN_PORT_CAP_TX_PAUSE: fp->active_fc &) -> else {
    ...
    return fun_submit_admin_sync_cmd(fdev, &req.common, core::ptr::null_mut(), 0, 0);
    }
...
//
    ...
    let mut table_len: c_uint = fp.indir_table_nentries;
    let mut len: c_uint = FUN_ETH_RSS_MAX_KEY_SIZE + sizeof(u32) * table_len;
...
//
#[no_mangle]
unsafe extern "C" fn fun_destroy_rss(fp: *mut funeth_priv) {
...
#[no_mangle]
unsafe extern "C" fn fun_irq_aff_release(ref: *mut kref __always_unused) {
...
#[no_mangle]
unsafe extern "C" fn fun_free_qirq(fp: *mut funeth_priv, irq: *mut fun_irq) {
...
#[no_mangle]
unsafe extern "C" fn fun_prune_queue_irqs(dev: *mut net_device) {
    static void fun_prune_queue_irqs(struct net_device *dev)
    {
    struct funeth_priv *fp = netdev_priv(dev);
    let mut nreleased: c_uint = 0;
    struct fun_irq *irq;
    unsigned long idx;
...
//
    ...
    {
    struct funeth_priv *fp = netdev_priv(dev);
    let mut node: c_int = dev_to_node(&fp.pdev.dev);
    struct fun_irq *irq;
    unsigned int i;
    ...
    return ERR_PTR(-ENOMEM);
    for (i = start; i < nqs; i++) {
    err = funeth_txq_create(dev, i, depth, core::ptr::null_mut(), state, &xdpqs[i]);
    if (err) {
...
#[no_mangle]
unsafe extern "C" fn fun_free_rings(netdev: *mut net_device, qset: *mut fun_qset) {
...
//
    ...
    rcu_assign_pointer(fp.rxqs, core::ptr::null_mut());
    rcu_assign_pointer(fp.xdpqs, core::ptr::null_mut());
    synchronize_net();
    fp.txqs = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn fun_alloc_rings(netdev: *mut net_device, qset: *mut fun_qset) -> c_int {
    static int fun_alloc_rings(struct net_device *netdev, struct fun_qset *qset)
    {
    struct funeth_txq **xdpqs = core::ptr::null_mut(), **txqs;
...
//
#[no_mangle]
unsafe extern "C" fn fun_advance_ring_state(dev: *mut net_device, qset: *mut fun_qset) -> c_int {
    ...
    err = fun_txq_create_dev(qset.xdpqs[i], core::ptr::null_mut());
    if (err)
    goto out;
    }
    return 0;
...
#[no_mangle]
unsafe extern "C" fn fun_port_create(netdev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_port_destroy(netdev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_eth_create(fp: *mut funeth_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_vi_create(fp: *mut funeth_priv) -> c_int {
    ...
    return fun_submit_admin_sync_cmd(fp.fdev, &req.common, core::ptr::null_mut(), 0, 0);
    }
// Helper to create an ETH flow and bind an SQ to it.
// Returns the ETH id (>= 0) on success or a negative error.
//
#[no_mangle]
pub unsafe extern "C" fn fun_create_and_bind_tx(fp: *mut funeth_priv, sqid: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_queue_irq_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn fun_enable_irqs(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_disable_one_irq(irq: *mut fun_irq) {
    static void fun_disable_one_irq(struct fun_irq *irq)
    {
    napi_disable(&irq.napi);
    irq_set_affinity_notifier(irq.irq, core::ptr::null_mut());
    irq_update_affinity_hint(irq.irq, core::ptr::null_mut());
    free_irq(irq.irq, irq);
    irq.state = FUN_IRQ_INIT;
    }
#[no_mangle]
unsafe extern "C" fn fun_disable_irqs(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn fun_down(dev: *mut net_device, qset: *mut fun_qset) {
...
#[no_mangle]
unsafe extern "C" fn fun_up(dev: *mut net_device, qset: *mut fun_qset) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn funeth_open(netdev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn funeth_close(netdev: *mut net_device) -> c_int {
    static int funeth_close(struct net_device *netdev)
    {
    let mut qset: fun_qset = { .state = FUN_QSTATE_DESTROYED };
...
#[no_mangle]
unsafe extern "C" fn fun_change_mtu(netdev: *mut net_device, new_mtu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_set_macaddr(netdev: *mut net_device, addr: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_get_port_attributes(netdev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_enter_xdp(dev: *mut net_device, prog: *mut bpf_prog) -> c_int {
    ...
    out:
    while (i--)
    fun_rxq_set_bpf(rxqs[i], core::ptr::null_mut());
    free_xdpqs(xdpqs, nqs, 0, FUN_QSTATE_DESTROYED);
    return err;
    }
// Set the queues for non-XDP operation.
#[no_mangle]
unsafe extern "C" fn fun_end_xdp(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn fun_xdp_setup(dev: *mut net_device, xdp: *mut netdev_bpf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_xdp(dev: *mut net_device, xdp: *mut netdev_bpf) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_init_vports(ed: *mut fun_ethdev, n: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_free_vports(ed: *mut fun_ethdev) {
    static void fun_free_vports(struct fun_ethdev *ed)
    {
    kvfree(ed.vport_info);
    ed.vport_info = core::ptr::null_mut();
    ed.num_vports = 0;
    }
...
#[no_mangle]
unsafe extern "C" fn fun_set_vf_mac(dev: *mut net_device, vf: c_int, mac: *mut u8) -> c_int {
    static int fun_set_vf_mac(struct net_device *dev, int vf, u8 *mac)
    {
    struct funeth_priv *fp = netdev_priv(dev);
    let mut mac_param: fun_adi_param = {};
    ...
    {
    struct funeth_priv *fp = netdev_priv(dev);
    let mut vlan_param: fun_adi_param = {};
    struct fun_dev *fdev = fp.fdev;
    struct fun_ethdev *ed = to_fun_ethdev(fdev);
    struct fun_vport_info *vi;
    let mut rc: c_int = -EINVAL;
    ...
    {
    struct funeth_priv *fp = netdev_priv(dev);
    let mut rate_param: fun_adi_param = {};
    struct fun_dev *fdev = fp.fdev;
    struct fun_ethdev *ed = to_fun_ethdev(fdev);
    struct fun_vport_info *vi;
    let mut rc: c_int = -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn fun_uninit(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn fun_dflt_rss_indir(fp: *mut funeth_priv, nrx: c_uint) {
...
//
#[no_mangle]
unsafe extern "C" fn fun_reset_rss_indir(dev: *mut net_device, nrx: c_uint) {
...
//
#[no_mangle]
unsafe extern "C" fn fun_init_rss(dev: *mut net_device) -> c_int {
    static int fun_init_rss(struct net_device *dev)
    {
    struct funeth_priv *fp = netdev_priv(dev);
    let mut size: usize = sizeof(fp.rss_key) + sizeof(fp.indir_table);
    fp.rss_hw_id = FUN_HCI_ID_INVALID;
...
#[no_mangle]
unsafe extern "C" fn fun_free_rss(fp: *mut funeth_priv) {
...
#[no_mangle]
unsafe extern "C" fn fun_init_stats_area(fp: *mut funeth_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fun_free_stats_area(fp: *mut funeth_priv) {
...
#[no_mangle]
unsafe extern "C" fn fun_dl_port_register(netdev: *mut net_device) -> c_int {
    static int fun_dl_port_register(struct net_device *netdev)
    {
    struct funeth_priv *fp = netdev_priv(netdev);
    struct devlink *dl = priv_to_devlink(fp.fdev);
    let mut attrs: devlink_port_attrs = {};
    unsigned int idx;
    if (fp.port_caps & FUN_PORT_CAP_VPORT) {
...
#[no_mangle]
unsafe extern "C" fn fun_queue_defaults(dev: *mut net_device, nsqs: c_uint) {
...
//
    ...
    struct netlink_ext_ack *extack)
    {
    let mut oldqs: fun_qset = { .state = FUN_QSTATE_DESTROYED };
    struct funeth_priv *fp = netdev_priv(dev);
    int err;
...
//
    ...
    unsigned int nrx)
    {
    let mut keep_tx: c_uint = min(dev.real_num_tx_queues, ntx);
    let mut keep_rx: c_uint = min(dev.real_num_rx_queues, nrx);
    struct funeth_priv *fp = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn fun_create_netdev(ed: *mut fun_ethdev, portid: c_uint) -> c_int {
...
// configurable parameters
    ...
    return 0;
    unreg_devlink:
    ed.netdevs[portid] = core::ptr::null_mut();
    fun_ktls_cleanup(fp);
...
#[no_mangle]
unsafe extern "C" fn fun_destroy_netdev(netdev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn fun_create_ports(ed: *mut fun_ethdev, nports: c_uint) -> c_int {
...
// The admin queue takes 1 IRQ and 2 SQs.
    ...
    kfree(ed.netdevs);
    ed.netdevs = core::ptr::null_mut();
    ed.num_ports = 0;
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn fun_destroy_ports(ed: *mut fun_ethdev) {
    ...
    {
    let mut port_idx: c_uint = be16_to_cpu(notif.id);
    struct net_device *netdev;
    struct funeth_priv *fp;
...
#[no_mangle]
unsafe extern "C" fn fun_event_cb(fdev: *mut fun_dev, entry: *mut c_void) {
    static void fun_event_cb(struct fun_dev *fdev, void *entry)
    {
    let mut op: u8 = ((struct fun_admin_rsp_common *)entry).op;
...
#[no_mangle]
unsafe extern "C" fn fun_service_cb(fdev: *mut fun_dev) {
...
#[no_mangle]
unsafe extern "C" fn funeth_sriov_configure(pdev: *mut pci_dev, nvfs: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn funeth_probe(pdev: *mut pci_dev, id: *const pci_device_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn funeth_remove(pdev: *mut pci_dev) {
