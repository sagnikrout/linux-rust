...
//! Source: drivers/net/ethernet/smsc/epic100.c
#![no_std]
...
pub const TX_FIFO_THRESH: c_int = 256;
...
pub const EPIC_TOTAL_SIZE: c_uint = 0x100;
pub const USE_IO_OPS: c_int = 1;

pub const EPIC_BAR: c_int = 0;

pub const EPIC_BAR: c_int = 1;
...
pub const EpicRemoved: c_uint = 0xffffffff	/* Chip failed or removed (CardBus) */;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct epic_private {
    pub rx_ring: *mut epic_rx_desc,
    pub tx_ring: *mut epic_tx_desc,
...
// Ring pointers.
    pub /: *mut *mut spinlock_t lock; / Group with Tx control cache line.,
    ...
    pub /: *mut *mut *mut pci_dev pci_dev; / PCI bus location.,
    pub chip_flags: int chip_id,,
    pub /: *mut *mut timer_list timer; / Media selection timer.,
    pub tx_threshold: c_int,
    pub mc_filter: [c_uchar; 8],
    pub /: *mut *mut signed char phys[4]; / MII device addresses.,
    pub /: *mut *mut u16 advertising; / NWay media advertisement,
    pub mii_phy_cnt: c_int,
    pub ethtool_ops_nesting: u32,
    pub mii: mii_if_info,
    pub /: *mut *mut unsigned int tx_full:1; / The Tx queue is full.,
...
#[no_mangle]
unsafe extern "C" fn epic_init_one(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int epic_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
    let mut card_idx: static int = -1;
    void __iomem *ioaddr;
    let mut chip_idx: c_int = (int) ent.driver_data;
    struct net_device *dev;
...
pub const EE_CS: c_uint = 0x02	/* EEPROM chip select. */;
pub const EE_DATA_WRITE: c_uint = 0x08	/* EEPROM chip data in. */;
pub const EE_WRITE_0: c_uint = 0x01;
pub const EE_WRITE_1: c_uint = 0x09;
pub const EE_DATA_READ: c_uint = 0x10	/* EEPROM chip data out. */;
...
#[no_mangle]
unsafe extern "C" fn epic_disable_int(dev: *mut net_device, ep: *mut epic_private) {
...
#[no_mangle]
pub unsafe extern "C" fn __epic_pci_commit(ioaddr: *mut void __iomem) {
...
#[no_mangle]
unsafe extern "C" fn read_eeprom(ep: *mut epic_private, location: c_int) -> c_int {
...
// Shift the read command bits out.
    for (i = 12; i >= 0; i--) {
    let mut dataval: c_short = (read_cmd & (1 << i)) ? EE_WRITE_1 : EE_WRITE_0;
...
pub const MII_READOP: c_int = 1;
pub const MII_WRITEOP: c_int = 2;
#[no_mangle]
unsafe extern "C" fn mdio_read(dev: *mut net_device, phy_id: c_int, location: c_int) -> c_int {
    ...
    void __iomem *ioaddr = ep.ioaddr;
    let mut read_cmd: c_int = (phy_id << 9) | (location << 4) | MII_READOP;
    int i;
...
#[no_mangle]
unsafe extern "C" fn mdio_write(dev: *mut net_device, phy_id: c_int, loc: c_int, value: c_int) {
...
#[no_mangle]
unsafe extern "C" fn epic_open(dev: *mut net_device) -> c_int {
    static int epic_open(struct net_device *dev)
    {
    struct epic_private *ep = netdev_priv(dev);
    void __iomem *ioaddr = ep.ioaddr;
    let mut irq: c_int = ep.pci_dev.irq;
    int rc, i;
// Soft reset the chip.
    ew32(GENCTL, 0x4001);
    napi_enable(&ep.napi);
...
// Tell the chip to byteswap descriptors on big-endian hosts
    ...
    let mut mii_lpa: c_int = mdio_read(dev, ep.phys[0], MII_LPA);
    if (mii_lpa != 0xffff) {
    if ((mii_lpa & LPA_100FULL) || (mii_lpa & 0x01C0) == LPA_10FULL)
    ep.mii.full_duplex = 1;
#[no_mangle]
pub unsafe extern "C" fn if(LPA_LPACK): ! (mii_lpa &) -> else {
...
#[no_mangle]
unsafe extern "C" fn epic_pause(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn epic_restart(dev: *mut net_device) {
    static void epic_restart(struct net_device *dev)
    {
    struct epic_private *ep = netdev_priv(dev);
    void __iomem *ioaddr = ep.ioaddr;
...
#[no_mangle]
unsafe extern "C" fn check_media(dev: *mut net_device) {
    static void check_media(struct net_device *dev)
    {
    struct epic_private *ep = netdev_priv(dev);
    void __iomem *ioaddr = ep.ioaddr;
    let mut mii_lpa: c_int = ep.mii_phy_cnt ? mdio_read(dev, ep.phys[0], MII_LPA) : 0;
    let mut negotiated: c_int = mii_lpa & ep.mii.advertising;
    let mut duplex: c_int = (negotiated & 0x0100) || (negotiated & 0x01C0) == 0x0040;
...
#[no_mangle]
unsafe extern "C" fn epic_timer(t: *mut timer_list) {
    static void epic_timer(struct timer_list *t)
    {
    struct epic_private *ep = timer_container_of(ep, t, timer);
    struct net_device *dev = ep.mii.dev;
    void __iomem *ioaddr = ep.ioaddr;
    let mut next_tick: c_int = 5*HZ;
    if (debug > 3) {
...
#[no_mangle]
unsafe extern "C" fn epic_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void epic_tx_timeout(struct net_device *dev, unsigned int txqueue)
    {
    struct epic_private *ep = netdev_priv(dev);
    void __iomem *ioaddr = ep.ioaddr;
...
#[no_mangle]
unsafe extern "C" fn epic_init_ring(dev: *mut net_device) {
    static void epic_init_ring(struct net_device *dev)
    {
    struct epic_private *ep = netdev_priv(dev);
    int i;
...
// Initialize all Rx descriptors.
    ...
    ep.rx_skbuff[i] = core::ptr::null_mut();
    }
...
// The Tx buffer descriptor is filled in as needed, but we
    ...
    ep.tx_skbuff[i] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn epic_start_xmit(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
...
#[no_mangle]
unsafe extern "C" fn epic_tx(dev: *mut net_device, ep: *mut epic_private) {
...
//
    ...
    let mut txstatus: c_int = ep.tx_ring[entry].txstatus;
    if (txstatus & DescOwn)
...
// Free the original skb.
    ...
    ep.tx_skbuff[entry] = core::ptr::null_mut();
    }
...
// The interrupt handler does all of the Rx thread work and cleans up
    after the Tx thread. */
#[no_mangle]
unsafe extern "C" fn epic_interrupt(irq: c_int, dev_instance: *mut c_void) -> irqreturn_t {
    static irqreturn_t epic_interrupt(int irq, void *dev_instance)
    {
...
#[no_mangle]
unsafe extern "C" fn epic_rx(dev: *mut net_device, budget: c_int) -> c_int {
    static int epic_rx(struct net_device *dev, int budget)
    {
    struct epic_private *ep = netdev_priv(dev);
    let mut entry: c_int = ep.cur_rx % RX_RING_SIZE;
    let mut rx_work_limit: c_int = ep.dirty_rx + RX_RING_SIZE - ep.cur_rx;
...
// If we own the next entry, it's a new packet. Send it up.
    ...
    let mut status: c_int = ep.rx_ring[entry].rxstatus;
    if (debug > 4)
...
// Omit the four octet CRC from the length.
    let mut pkt_len: c_short = (status >> 16) - 4;
    struct sk_buff *skb;
...
// Check if the packet is long enough to accept without copying
    ...
    ep.rx_skbuff[entry] = core::ptr::null_mut();
    }
...
// Refill the Rx ring buffers.
    ...
    if (ep.rx_skbuff[entry] == core::ptr::null_mut()) {
    struct sk_buff *skb;
...
#[no_mangle]
unsafe extern "C" fn epic_rx_err(dev: *mut net_device, ep: *mut epic_private) {
...
#[no_mangle]
unsafe extern "C" fn epic_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    static int epic_poll(struct napi_struct *napi, int budget)
    {
...
#[no_mangle]
unsafe extern "C" fn epic_close(dev: *mut net_device) -> c_int {
    static int epic_close(struct net_device *dev)
    {
    struct epic_private *ep = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn ethtool_begin(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ethtool_complete(dev: *mut net_device) {
...
// all non-ethtool ioctls (the SIOC[GS]MIIxxx ioctls)
    spin_lock_irq(&np.lock);
    rc = generic_mii_ioctl(&np.mii, data, cmd, core::ptr::null_mut());
    spin_unlock_irq(&np.lock);
// power-down, if interface is down
    if (! netif_running(dev)) {
    ew32(GENCTL, 0x0008);
...
#[no_mangle]
unsafe extern "C" fn epic_remove_one(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn epic_suspend(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn epic_resume(dev_d: *mut device) -> int __maybe_unused {
