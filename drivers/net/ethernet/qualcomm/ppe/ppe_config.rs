...
//! Source: drivers/net/ethernet/qualcomm/ppe/ppe_config.h
#![no_std]
...
//
pub const PPE_QUEUE_BASE_DEST_PORT: c_int = 0;
pub const PPE_QUEUE_BASE_CPU_CODE: c_int = 1024;
pub const PPE_QUEUE_BASE_SERVICE_CODE: c_int = 2048;
pub const PPE_QUEUE_INTER_PRI_NUM: c_int = 16;
pub const PPE_QUEUE_HASH_NUM: c_int = 256;
// The service code is used by EDMA port to transmit packet to PPE.
pub const PPE_EDMA_SC_BYPASS_ID: c_int = 1;
// The PPE RSS hash configured for IPv4 and IPv6 packet separately.

pub const PPE_RSS_HASH_IP_LENGTH: c_int = 4;
pub const PPE_RSS_HASH_TUPLES: c_int = 5;
// PPE supports 300 queues, each bit presents as one queue.
pub const PPE_RING_TO_QUEUE_BITMAP_WORD_CNT: c_int = 10;
//
...
pub enum ppe_scheduler_frame_mode {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ppe_scheduler_cfg {
    pub flow_id: c_int,
    pub pri: c_int,
    pub drr_node_id: c_int,
    pub drr_node_wt: c_int,
    pub unit_is_packet: bool,
    pub frame_mode: ppe_scheduler_frame_mode,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ppe_queue_ucast_dest {
    pub src_profile: c_int,
    pub service_code_en: bool,
    pub service_code: c_int,
    pub cpu_code_en: bool,
    pub cpu_code: c_int,
    pub dest_port: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ppe_sc_bypass {
    pub PPE_SC_BYPASS_INGRESS_SIZE): DECLARE_BITMAP(ingress,,
    pub PPE_SC_BYPASS_EGRESS_SIZE): DECLARE_BITMAP(egress,,
    pub PPE_SC_BYPASS_COUNTER_SIZE): DECLARE_BITMAP(counter,,
    pub PPE_SC_BYPASS_TUNNEL_SIZE): DECLARE_BITMAP(tunnel,,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ppe_sc_cfg {
    pub dest_port_valid: bool,
    pub dest_port: c_int,
    pub bitmaps: ppe_sc_bypass,
    pub is_src: bool,
    pub next_service_code: c_int,
    pub eip_field_update_bitmap: c_int,
    pub eip_hw_service: c_int,
    pub eip_offset_sel: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ppe_rss_hash_cfg {
    pub hash_mask: u32,
    pub hash_fragment_mode: bool,
    pub hash_seed: u32,
    pub hash_sip_mix: [u8; PPE_RSS_HASH_IP_LENGTH],
    pub hash_dip_mix: [u8; PPE_RSS_HASH_IP_LENGTH],
    pub hash_protocol_mix: u8,
    pub hash_sport_mix: u8,
    pub hash_dport_mix: u8,
    pub hash_fin_inner: [u8; PPE_RSS_HASH_TUPLES],
    pub hash_fin_outer: [u8; PPE_RSS_HASH_TUPLES],
}

extern "C" {
    pub fn ppe_hw_config(ppe_dev: *mut ppe_device) -> c_int;
}
extern "C" {
    pub fn ppe_counter_enable_set(ppe_dev: *mut ppe_device, port: c_int) -> c_int;
}
