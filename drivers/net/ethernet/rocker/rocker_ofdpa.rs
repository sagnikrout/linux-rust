...
//! Source: drivers/net/ethernet/rocker/rocker_ofdpa.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_flow_tbl_key {
    pub priority: u32,
    pub tbl_id: enum rocker_of_dpa_table_id,
    union {
    struct {
    pub in_pport: u32,
    pub in_pport_mask: u32,
    pub goto_tbl: enum rocker_of_dpa_table_id,
    pub ig_port: },
    struct {
    pub in_pport: u32,
    pub vlan_id: __be16,
    pub vlan_id_mask: __be16,
    pub goto_tbl: enum rocker_of_dpa_table_id,
    pub untagged: bool,
    pub new_vlan_id: __be16,
    pub vlan: },
    struct {
    pub in_pport: u32,
    pub in_pport_mask: u32,
    ...
    pub eth_dst: [u8; ETH_ALEN],
    pub eth_dst_mask: [u8; ETH_ALEN],
    pub has_eth_dst: c_int,
    pub has_eth_dst_mask: c_int,
    pub vlan_id: __be16,
    pub tunnel_id: u32,
    pub goto_tbl: enum rocker_of_dpa_table_id,
    pub group_id: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_flow_tbl_entry {
    pub entry: hlist_node,
    pub cmd: u32,
    pub cookie: u64,
    pub key: ofdpa_flow_tbl_key,
    pub key_len: usize,
    pub /: *mut *mut u32 key_crc32; / key,
    pub fi: *mut fib_info,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_group_tbl_entry {
    pub entry: hlist_node,
    pub cmd: u32,
    pub /: *mut *mut u32 group_id; / key,
    pub group_count: u16,
    union {
    struct {
    pub pop_vlan: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_fdb_tbl_entry {
    pub entry: hlist_node,
    pub /: *mut *mut u32 key_crc32; / key,
    pub learned: bool,
    pub touched: c_ulong,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_fdb_tbl_key {
    pub ofdpa_port: *mut ofdpa_port,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_internal_vlan_tbl_entry {
    pub entry: hlist_node,
    pub /: *mut *mut int ifindex; / key,
    pub ref_count: u32,
    pub vlan_id: __be16,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_neigh_tbl_entry {
    pub entry: hlist_node,
    pub /: *mut *mut __be32 ip_addr; / key,
...
pub const OFDPA_INTERNAL_VLAN_ID_BASE: c_uint = 0x0f00;
pub const OFDPA_N_INTERNAL_VLANS: c_int = 255;

pub const OFDPA_UNTAGGED_VID: c_int = 0;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa {
    pub rocker: *mut rocker,
    pub 16): DECLARE_HASHTABLE(flow_tbl,,
    pub /: *mut *mut spinlock_t flow_tbl_lock; / for flow tbl accesses,
    pub flow_tbl_next_cookie: u64,
    pub 16): DECLARE_HASHTABLE(group_tbl,,
    pub /: *mut *mut spinlock_t group_tbl_lock; / for group tbl accesses,
    pub fdb_cleanup_timer: timer_list,
    pub 16): DECLARE_HASHTABLE(fdb_tbl,,
    pub /: *mut *mut spinlock_t fdb_tbl_lock; / for fdb tbl accesses,
    pub internal_vlan_bitmap: [c_ulong; OFDPA_INTERNAL_VLAN_BITMAP_LEN],
    pub 8): DECLARE_HASHTABLE(internal_vlan_tbl,,
    pub /: *mut *mut spinlock_t internal_vlan_tbl_lock; / for vlan tbl accesses,
    pub 16): DECLARE_HASHTABLE(neigh_tbl,,
    pub /: *mut *mut spinlock_t neigh_tbl_lock; / for neigh tbl accesses,
    pub neigh_tbl_next_index: u32,
    pub ageing_time: c_ulong,
    pub fib_aborted: bool,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_port {
    pub ofdpa: *mut ofdpa,
    pub rocker_port: *mut rocker_port,
    pub dev: *mut net_device,
    pub pport: u32,
    pub bridge_dev: *mut net_device,
    pub internal_vlan_id: __be16,
    pub stp_state: c_int,
    pub brport_flags: u32,
    pub ageing_time: c_ulong,
    pub ctrls: [bool; OFDPA_CTRL_MAX],
    pub vlan_bitmap: [c_ulong; OFDPA_VLAN_BITMAP_LEN],
}
...
#[no_mangle]
unsafe extern "C" fn ofdpa_vlan_id_is_internal(vlan_id: __be16) -> bool {
    static bool ofdpa_vlan_id_is_internal(__be16 vlan_id)
    {
    let mut start: u16 = OFDPA_INTERNAL_VLAN_ID_BASE;
    let mut end: u16 = 0xffe;
    let mut _vlan_id: u16 = ntohs(vlan_id);
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_is_bridged(ofdpa_port: *const ofdpa_port) -> bool {
    static bool ofdpa_port_is_bridged(const struct ofdpa_port *ofdpa_port)
    {
    return ofdpa_port_is_slave(ofdpa_port, "bridge");
    }
#[no_mangle]
unsafe extern "C" fn ofdpa_port_is_ovsed(ofdpa_port: *const ofdpa_port) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_flags_nowait(flags: c_int) -> bool {
...
//
    ...
    let mut key_len: usize = match.key_len ? match.key_len : sizeof(found.key);
    ...
    found, core::ptr::null_mut(), core::ptr::null_mut());
    }
    ...
    struct ofdpa_flow_tbl_entry *entry;
    u32 priority;
    let mut vlan_bridging: bool = !!vlan_id;
    let mut dflt: bool = !eth_dst || eth_dst_mask;
...
#[no_mangle]
pub unsafe extern "C" fn if(!wild: vlan_bridging && dflt &&) -> else {
    else if (vlan_bridging && dflt && !wild)
    priority = OFDPA_PRIORITY_BRIDGING_VLAN_DFLT_EXACT;
#[no_mangle]
pub unsafe extern "C" fn if(!dflt: vlan_bridging &&) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_link_local_ether_addr(eth_dst)) -> else {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_neigh_del(entry: *mut ofdpa_neigh_tbl_entry) {
    ...
    struct ofdpa_neigh_tbl_entry *found;
    unsigned long lock_flags;
    let mut eth_type: __be16 = htons(ETH_P_IP);
    enum rocker_of_dpa_table_id goto_tbl =
    ROCKER_OF_DPA_TABLE_ID_ACL_POLICY;
    u32 group_id;
    let mut priority: u32 = 0;
    let mut adding: bool = !(flags & OFDPA_OP_FLAG_REMOVE);
    bool updating;
    bool removing;
    let mut err: c_int = 0;
    entry = kzalloc_obj(*entry, GFP_ATOMIC);
...
//
    ...
    inet_make_mask(32),
    priority, goto_tbl,
    group_id, core::ptr::null_mut(), flags);
    if (err)
...
// index = found->index;
    ...
    ofdpa_neigh_update(found, core::ptr::null_mut(), false);
...
// Resolved means neigh ip_addr is resolved to neigh mac.
    ...
    return rocker_port ? rocker_port.wpriv : core::ptr::null_mut();
    }
    ...
    let mut port_count: c_uint = ofdpa.rocker.port_count;
    let mut group_id: u32 = ROCKER_GROUP_L2_FLOOD(vlan_id, 0);
    u32 *group_ids;
...
// pass all pkts up to CPU
    ...
    {
    let mut in_pport: u32 = ofdpa_port.pport;
    let mut in_pport_mask: u32 = 0xffffffff;
    let mut out_pport: u32 = 0;
    const u8 *eth_src = core::ptr::null_mut();
    const u8 *eth_src_mask = core::ptr::null_mut();
    let mut vlan_id_mask: __be16 = htons(0xffff);
    let mut ip_proto: u8 = 0;
    let mut ip_proto_mask: u8 = 0;
    let mut ip_tos: u8 = 0;
    let mut ip_tos_mask: u8 = 0;
    let mut group_id: u32 = ROCKER_GROUP_L2_INTERFACE(vlan_id, out_pport);
    int err;
    ...
    let mut in_pport: u32 = ofdpa_port.pport;
    let mut vlan_id: __be16 = htons(vid);
    let mut vlan_id_mask: __be16 = htons(0xffff);
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_ig_tbl(ofdpa_port: *mut ofdpa_port, flags: c_int) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ofdpa_fdb_learn_work {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_fdb_learn_work(work: *mut work_struct) {
    static void ofdpa_port_fdb_learn_work(struct work_struct *work)
    {
    const struct ofdpa_fdb_learn_work *lw =
    container_of(work, struct ofdpa_fdb_learn_work, work);
    let mut removing: bool = (lw.flags & OFDPA_OP_FLAG_REMOVE);
    ...
    rtnl_lock();
    call_switchdev_notifiers(event, lw.ofdpa_port.dev, &info.info, core::ptr::null_mut());
    rtnl_unlock();
    kfree(work);
    }
    ...
    enum rocker_of_dpa_table_id goto_tbl =
    ROCKER_OF_DPA_TABLE_ID_ACL_POLICY;
    let mut out_pport: u32 = ofdpa_port.pport;
    let mut tunnel_id: u32 = 0;
    let mut group_id: u32 = ROCKER_GROUP_NONE;
    let mut copy_to_cpu: bool = false;
    int err;
    if (ofdpa_port_is_bridged(ofdpa_port))
    group_id = ROCKER_GROUP_L2_INTERFACE(vlan_id, out_pport);
    if (!(flags & OFDPA_OP_FLAG_REFRESH)) {
    err = ofdpa_flow_tbl_bridge(ofdpa_port, flags, addr,
    core::ptr::null_mut(), vlan_id, tunnel_id, goto_tbl,
    ...
    struct ofdpa_fdb_tbl_entry *found;
    let mut removing: bool = (flags & OFDPA_OP_FLAG_REMOVE);
    unsigned long lock_flags;
    fdb = kzalloc_obj(*fdb);
    if (!fdb)
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_fdb_flush(ofdpa_port: *mut ofdpa_port, flags: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_fdb_cleanup(t: *mut timer_list) {
    ...
    struct hlist_node *tmp;
    let mut next_timer: c_ulong = jiffies + ofdpa.ageing_time;
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_fwding(ofdpa_port: *mut ofdpa_port, flags: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ofdpa_port_is_ovsed(ofdpa_port)) -> else {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_fwd_enable(ofdpa_port: *mut ofdpa_port, flags: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_fwd_disable(ofdpa_port: *mut ofdpa_port, flags: c_int) -> c_int {
...
// XXX deal with flags for PVID and untagged
    ...
    let mut eth_type: __be16 = htons(ETH_P_IP);
    let mut dst_mask: __be32 = inet_make_mask(dst_len);
    let mut internal_vlan_id: __be16 = ofdpa_port.internal_vlan_id;
    let mut priority: u32 = fi.fib_priority;
    enum rocker_of_dpa_table_id goto_tbl =
    ROCKER_OF_DPA_TABLE_ID_ACL_POLICY;
    u32 group_id;
    bool nh_on_port;
...
//
#[no_mangle]
unsafe extern "C" fn ofdpa_init(rocker: *mut rocker) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_fini(rocker: *mut rocker) {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_pre_init(rocker_port: *mut rocker_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_init(rocker_port: *mut rocker_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_fini(rocker_port: *mut rocker_port) {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_open(rocker_port: *mut rocker_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ofdpa_port_stop(rocker_port: *mut rocker_port) {
...
// p_brport_flags_support = BR_LEARNING;
    ...
    {
    struct ofdpa_port *ofdpa_port = rocker_port.wpriv;
    let mut vlan_id: __be16 = ofdpa_port_vid_to_vlan(ofdpa_port, vid, core::ptr::null_mut());
    if (!ofdpa_port_is_bridged(ofdpa_port))
    return -EINVAL;
    ...
    struct ofdpa_port *ofdpa_port = rocker_port.wpriv;
    let mut vlan_id: __be16 = ofdpa_port_vid_to_vlan(ofdpa_port, vid, core::ptr::null_mut());
    let mut flags: c_int = OFDPA_OP_FLAG_REMOVE;
    if (!ofdpa_port_is_bridged(ofdpa_port))
    return -EINVAL;
...
//
    ...
    if (err)
    return err;
    return switchdev_bridge_port_offload(dev, dev, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(),
    false, extack);
    }
#[no_mangle]
unsafe extern "C" fn ofdpa_port_bridge_leave(ofdpa_port: *mut ofdpa_port) -> c_int {
    static int ofdpa_port_bridge_leave(struct ofdpa_port *ofdpa_port)
    {
    struct net_device *dev = ofdpa_port.dev;
    int err;
    switchdev_bridge_port_unoffload(dev, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    err = ofdpa_port_vlan_del(ofdpa_port, OFDPA_UNTAGGED_VID, 0);
    if (err)
    return err;
    ofdpa_port_internal_vlan_id_put(ofdpa_port,
    ...
    ofdpa_port_internal_vlan_id_get(ofdpa_port,
    ofdpa_port.dev.ifindex);
    ofdpa_port.bridge_dev = core::ptr::null_mut();
    err = ofdpa_port_vlan_add(ofdpa_port, OFDPA_UNTAGGED_VID, 0);
    if (err)
    return err;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: netif_is_ovs_master(master)) -> else {
...
pub unsafe extern "C" fn if(_arg: ofdpa_port_is_ovsed(ofdpa_port)) -> else {
    ...
    err = ofdpa_port_ovs_changed(ofdpa_port, core::ptr::null_mut());
    return err;
    }
    ...
    let mut ip_addr: __be32 = *(__be32 *) n.primary_key;
    ...
    {
    struct ofdpa_port *ofdpa_port = rocker_port.wpriv;
    let mut flags: c_int = OFDPA_OP_FLAG_REMOVE | OFDPA_OP_FLAG_NOWAIT;
    let mut ip_addr: __be32 = *(__be32 *) n.primary_key;
    return ofdpa_port_ipv4_neigh(ofdpa_port, flags, ip_addr, n.ha);
    }
    ...
    {
    struct ofdpa_port *ofdpa_port = rocker_port.wpriv;
    let mut flags: c_int = OFDPA_OP_FLAG_NOWAIT | OFDPA_OP_FLAG_LEARNED;
    if (ofdpa_port.stp_state != BR_STATE_LEARNING &&
    ofdpa_port.stp_state != BR_STATE_FORWARDING)
    return 0;
    return ofdpa_port_fdb(ofdpa_port, addr, vlan_id, flags);
...
#[no_mangle]
unsafe extern "C" fn ofdpa_fib4_abort(rocker: *mut rocker) {
