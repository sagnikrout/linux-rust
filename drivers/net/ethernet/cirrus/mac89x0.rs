...
//! Source: drivers/net/ethernet/cirrus/mac89x0.c
#![no_std]
...
//
    ...
    : request_irq() and free_irq(). Just core::ptr::null_mut() for now.
...
pub struct net_local {
    ...
    pub /: *mut *mut int chip_type; / one of: CS8900, CS8920, CS8920M,
    pub /: *mut *mut char chip_revision; / revision letter of the chip ('A'...),
    pub /: *mut *mut int send_cmd; / the propercommand used to send a packet.,
    pub rx_mode: c_int,
    pub curr_rx_cfg: c_int,
    pub /: *mut *mut int send_underrun; / keep track of how many underruns in a row we get,
}
...
#[no_mangle]
unsafe extern "C" fn mac89x0_device_probe(pdev: *mut platform_device) -> c_int {
    static int mac89x0_device_probe(struct platform_device *pdev)
    {
    struct net_device *dev;
    struct net_local *lp;
    int i, slot;
    let mut rev_type: unsigned = 0;
...
// Big-endian (why??!)
    let mut s: c_ushort = readreg(dev, PP_IA + i);
    addr[i] = s >> 8;
    addr[i+1] = s & 0xff;
    }
...
#[no_mangle]
unsafe extern "C" fn net_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t net_interrupt(int irq, void *dev_id)
    {
...
#[no_mangle]
unsafe extern "C" fn set_mac_address(dev: *mut net_device, addr: *mut c_void) -> c_int {
    static int set_mac_address(struct net_device *dev, void *addr)
    {
...
// set the Ethernet address
    ...
    MODULE_LICENSE("GPL");
#[no_mangle]
unsafe extern "C" fn mac89x0_device_remove(pdev: *mut platform_device) {
