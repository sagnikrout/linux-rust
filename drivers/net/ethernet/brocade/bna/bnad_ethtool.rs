...
//! Source: drivers/net/ethernet/brocade/bna/bnad_ethtool.c
#![no_std]
...
pub const BNAD_NUM_TXF_COUNTERS: c_int = 12;
pub const BNAD_NUM_RXF_COUNTERS: c_int = 10;

pub const BNAD_NUM_RXQ_COUNTERS: c_int = 7;
pub const BNAD_NUM_TXQ_COUNTERS: c_int = 5;
...
#[no_mangle]
unsafe extern "C" fn bnad_get_txf_strings(string: *mut u8, f_num: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bnad_get_rxf_strings(string: *mut u8, f_num: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bnad_get_cq_strings(string: *mut u8, q_num: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bnad_get_rxq_strings(string: *mut u8, q_num: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bnad_get_txq_strings(string: *mut u8, q_num: c_int) {
    ...
    {
    int i, j;
    struct bna_rcb *rcb = core::ptr::null_mut();
    struct bna_tcb *tcb = core::ptr::null_mut();
    for (i = 0; i < bnad.num_rx; i++) {
    if (!bnad.rx_info[i].rx)
    continue;
    for (j = 0; j < bnad.num_rxp_per_rx; j++)
...
// base_offset = flash_attr->part[i].part_off;
    ...
    struct bnad *bnad = netdev_priv(netdev);
    struct bnad_iocmd_comp fcomp;
    let mut flash_part: u32 = 0, base_offset = 0;
    let mut flags: c_ulong = 0;
    let mut ret: c_int = 0;
