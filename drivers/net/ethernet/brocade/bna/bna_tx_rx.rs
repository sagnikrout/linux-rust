...
//! Source: drivers/net/ethernet/brocade/bna/bna_tx_rx.c
#![no_std]
...
// No more pending config updates
    ...
    bfa_msgq_cmd_set(&rxf.msgq_cmd, core::ptr::null_mut(), core::ptr::null_mut(),
...
// This function gets the multicast MAC that has already been added to CAM
    ...
    mac.handle = core::ptr::null_mut();
    return ret;
    }
    static int
    bna_rxf_mcast_cfg_apply(struct bna_rxf *rxf)
    {
    struct bna_mac *mac = core::ptr::null_mut();
    int ret;
...
// Clear ucast from cache
    ...
    rxf.ucast_pending_mac = core::ptr::null_mut();
    ...
    if (rxf.ucast_pending_mac == core::ptr::null_mut()) {
    ...
    rxf.cam_fltr_cbfn = core::ptr::null_mut();
...
// Add the new entries
    ...
    struct bna_rxf *rxf = &rx.rxf;
    struct bna_mac *mac, *del_mac;
    let mut need_hw_config: c_int = 0;
...
// Schedule all entries in active_q for deletion
    ...
    {
    struct bna_rxf *rxf = &rx.rxf;
    let mut index: c_int = (vlan_id >> BFI_VLAN_WORD_SHIFT);
    let mut bit: c_int = BIT(vlan_id & BFI_VLAN_WORD_MASK);
    let mut group_id: c_int = (vlan_id >> BFI_VLAN_BLOCK_SHIFT);
...
// RX
    ...
    cbfn = (rx).stop_cbfn;				 \
    cbarg = (rx).stop_cbarg;			       \
    (rx).stop_cbfn = core::ptr::null_mut();					\
    (rx).stop_cbarg = core::ptr::null_mut();				\
    cbfn(cbarg, rx);					\
    }							       \
    } while (0)
...
// ((struct bna_dma_addr *)((bna_qpt)->kv_qpt_ptr));	\
    ...
    struct bna_rx, enum bna_rx_event);
#[no_mangle]
unsafe extern "C" fn bna_rx_sm_stopped_entry(rx: *mut bna_rx) {
...
#[no_mangle]
unsafe extern "C" fn bna_rx_sm_start_wait_entry(rx: *mut bna_rx) {
...
#[no_mangle]
unsafe extern "C" fn bna_rx_sm_rxf_start_wait_entry(rx: *mut bna_rx) {
    ...
    {
    struct bna_rxp *rxp;
    let mut is_regular: c_int = (rx.type == BNA_RX_T_REGULAR);
// Start IB
    list_for_each_entry(rxp, &rx.rxp_q, qe)
...
// No-op
    ...
    struct bna_rxp *rxp = core::ptr::null_mut();
    struct bna_rxq *q0 = core::ptr::null_mut(), *q1 = core::ptr::null_mut();
    int i;
...
//
    ...
    bfa_msgq_cmd_set(&rx.msgq_cmd, core::ptr::null_mut(), core::ptr::null_mut(),
    ...
    bfa_msgq_cmd_set(&rx.msgq_cmd, core::ptr::null_mut(), core::ptr::null_mut(), sizeof(struct bfi_enet_req),
    &req.mh);
    bfa_msgq_cmd_post(&rx.bna.msgq, &rx.msgq_cmd);
    }
    static void
...
// Stop IB
    ...
    {
    struct bna_rxq *rxq = core::ptr::null_mut();
    ...
    {
    struct bna_rx *rx = core::ptr::null_mut();
    ...
    rxp.rxq.single.reserved = core::ptr::null_mut();
    break;
    case BNA_RXP_SLR:
    ...
    rx_mod.stop_cbfn = core::ptr::null_mut();
    }
    static void
...
// Build RX queues
    ...
    rx_ptr.bna = core::ptr::null_mut();
    rx_ptr.rid = index;
    rx_ptr.stop_cbfn = core::ptr::null_mut();
    rx_ptr.stop_cbarg = core::ptr::null_mut();
...
//
    ...
    struct bna_rxq *q0 = core::ptr::null_mut();
    struct bna_rxq *q1 = core::ptr::null_mut();
    struct bna_rxp *rxp;
    struct list_head *qe;
    ...
    q0.rcb = core::ptr::null_mut();
    q0.rxp = core::ptr::null_mut();
    q0.rx = core::ptr::null_mut();
    bna_rxq_put(rx_mod, q0);
    if (q1) {
    if (rx.rcb_destroy_cbfn)
    rx.rcb_destroy_cbfn(rx.bna.bnad, q1.rcb);
    q1.rcb = core::ptr::null_mut();
    q1.rxp = core::ptr::null_mut();
    q1.rx = core::ptr::null_mut();
    bna_rxq_put(rx_mod, q1);
    }
    rxp.rxq.slr.large = core::ptr::null_mut();
    rxp.rxq.slr.small = core::ptr::null_mut();
    if (rx.ccb_destroy_cbfn)
    rx.ccb_destroy_cbfn(rx.bna.bnad, rxp.cq.ccb);
    rxp.cq.ccb = core::ptr::null_mut();
    rxp.rx = core::ptr::null_mut();
    ...
    rx_mod.rid_mask &= ~BIT(rx.rid);
    rx.bna = core::ptr::null_mut();
    rx.priv = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(BNA_PKT_RATE_20K: pkt_rt <) -> else {
    else if (pkt_rt < BNA_PKT_RATE_20K)
    load = BNA_LOAD_T_LOW_3;
#[no_mangle]
pub unsafe extern "C" fn if(BNA_PKT_RATE_30K: pkt_rt <) -> else {
    else if (pkt_rt < BNA_PKT_RATE_30K)
    load = BNA_LOAD_T_LOW_2;
#[no_mangle]
pub unsafe extern "C" fn if(BNA_PKT_RATE_40K: pkt_rt <) -> else {
    else if (pkt_rt < BNA_PKT_RATE_40K)
    load = BNA_LOAD_T_LOW_1;
#[no_mangle]
pub unsafe extern "C" fn if(BNA_PKT_RATE_50K: pkt_rt <) -> else {
    else if (pkt_rt < BNA_PKT_RATE_50K)
    load = BNA_LOAD_T_HIGH_1;
#[no_mangle]
pub unsafe extern "C" fn if(BNA_PKT_RATE_60K: pkt_rt <) -> else {
    else if (pkt_rt < BNA_PKT_RATE_60K)
    load = BNA_LOAD_T_HIGH_2;
#[no_mangle]
pub unsafe extern "C" fn if(BNA_PKT_RATE_80K: pkt_rt <) -> else {
...
// TX
    ...
    cbfn = (tx).stop_cbfn;					\
    cbarg = (tx).stop_cbarg;				\
    (tx).stop_cbfn = core::ptr::null_mut();					\
    (tx).stop_cbarg = core::ptr::null_mut();				\
    cbfn(cbarg, (tx));					\
    }								\
    } while (0)
...
// No-op
    ...
    {
    struct bna_txq *txq;
    let mut is_regular: c_int = (tx.type == BNA_TX_T_REGULAR);
...
// No-op
    ...
    struct bna_txq *txq = core::ptr::null_mut();
    int i;
    bfi_msgq_mhdr_set(cfg_req.mh, BFI_MC_ENET,
    ...
    bfa_msgq_cmd_set(&tx.msgq_cmd, core::ptr::null_mut(), core::ptr::null_mut(),
    ...
    req.mh.num_entries = htons(
    bfi_msgq_num_cmd_entries(sizeof(struct bfi_enet_req)));
    bfa_msgq_cmd_set(&tx.msgq_cmd, core::ptr::null_mut(), core::ptr::null_mut(), sizeof(struct bfi_enet_req),
    &req.mh);
    bfa_msgq_cmd_post(&tx.bna.msgq, &tx.msgq_cmd);
    }
    static void
...
// Stop IB
    ...
    {
    struct bna_tx *tx = core::ptr::null_mut();
    ...
    txq.tcb = core::ptr::null_mut();
    txq.tx = core::ptr::null_mut();
    ...
    tx.bna = core::ptr::null_mut();
    tx.priv = core::ptr::null_mut();
...
// Callback to bnad for setting up TCB
    ...
    tx_mod.stop_cbfn = core::ptr::null_mut();
    }
    void
