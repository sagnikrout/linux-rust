...
//! Source: drivers/net/ethernet/marvell/octeontx2/nic/otx2_tc.c
#![no_std]
...
pub const CN10K_MAX_BURST_MANTISSA: c_uint = 0x7FFFULL;

pub const RATE_MANTISSA_BITS: c_int = 8;
...
// Send message to AF
    ...
    struct otx2_tc_flow *node)
    {
    let mut dummy_grp_update_req: nix_mcast_grp_update_req = { 0 };
    struct netlink_ext_ack *extack = f.common.extack;
    let mut pps: bool = false, mcast = false;
    struct flow_action_entry *act;
    struct net_device *target;
    struct otx2_nic *priv;
    struct rep_dev *rdev;
    u32 burst, mark = 0;
    let mut nr_police: u8 = 0;
    let mut num_intf: u8 = 1;
    int err, i;
    u64 rate;
...
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_TCP: ip_proto ==) -> else {
    else if (ip_proto == IPPROTO_TCP)
    req.features |= BIT_ULL(NPC_IPPROTO_TCP);
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_SCTP: ip_proto ==) -> else {
    else if (ip_proto == IPPROTO_SCTP)
    req.features |= BIT_ULL(NPC_IPPROTO_SCTP);
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_ICMP: ip_proto ==) -> else {
    else if (ip_proto == IPPROTO_ICMP)
    req.features |= BIT_ULL(NPC_IPPROTO_ICMP);
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_ICMPV6: ip_proto ==) -> else {
    else if (ip_proto == IPPROTO_ICMPV6)
    req.features |= BIT_ULL(NPC_IPPROTO_ICMP6);
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_ESP: ip_proto ==) -> else {
    else if (ip_proto == IPPROTO_ESP)
    req.features |= BIT_ULL(NPC_IPPROTO_ESP);
#[no_mangle]
pub unsafe extern "C" fn if(IPPROTO_AH: ip_proto ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn otx2_destroy_tc_flow_list(pfvf: *mut otx2_nic) {
...
#[no_mangle]
pub unsafe extern "C" fn otx2_del_mcam_flow_entry(nic: *mut otx2_nic, entry: u16, cntr_val: *mut u16) -> c_int {
...
// cntr_val = rsp->cntr_val;
    ...
    struct list_head *pos, *n;
    struct otx2_tc_flow *tmp;
    let mut i: c_int = 0, index = 0;
    let mut cntr_val: u16 = 0;
...
//
    ...
    struct otx2_tc_flow *node)
    {
    let mut mcam_idx: c_int = flow_cfg.max_flows - flow_cfg.nr_flows - 1;
    struct otx2_tc_flow *tmp;
    int list_idx, i;
    let mut cntr_val: u16 = 0;
    if (is_cn20k(nic.pdev))
...
// Remove the multicast/mirror related nodes
    ...
    free_mcam_flow:
    otx2_del_mcam_flow_entry(nic, flow_node.entry, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn otx2_init_tc(nic: *mut otx2_nic) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn otx2_shutdown_tc(nic: *mut otx2_nic) {
...
#[no_mangle]
pub unsafe extern "C" fn otx2_tc_apply_ingress_police_rules(nic: *mut otx2_nic) {
