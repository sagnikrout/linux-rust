...
//! Source: drivers/net/ethernet/marvell/octeon_ep/octep_ethtool.c
#![no_std]
...
//
    ...
    {
    struct octep_device *oct = netdev_priv(netdev);
    let mut num_queues: u16 = CFG_GET_PORTS_ACTIVE_IO_RINGS(oct.conf);
    const char *str;
    int i, j;
    switch (stringset) {
    case ETH_SS_STATS:
...
#[no_mangle]
unsafe extern "C" fn octep_get_sset_count(netdev: *mut net_device, sset: c_int) -> c_int {
...
// Per Rx Queue stats
    ...
    struct octep_iface_link_info *link_info;
    let mut advertised: u64 = 0;
    let mut autoneg: u8 = 0;
    int err;
    link_info = &oct.link_info;
...
#[no_mangle]
pub unsafe extern "C" fn octep_set_ethtool_ops(netdev: *mut net_device) {
