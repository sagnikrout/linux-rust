...
//! Source: drivers/net/ethernet/broadcom/sb1250-mac.c
#![no_std]
...
// Macro flag: #define CONFIG_SBMAC_COALESCE
...
// 1 normal messages, 0 quiet .. 7 verbose.
    ...
    let mut int_pktcnt_tx: static int = 255;
    module_param(int_pktcnt_tx, int, 0444);
    MODULE_PARM_DESC(int_pktcnt_tx, "TX packet count");
    let mut int_timeout_tx: static int = 255;
    module_param(int_timeout_tx, int, 0444);
    MODULE_PARM_DESC(int_timeout_tx, "TX timeout value");
    let mut int_pktcnt_rx: static int = 64;
    module_param(int_pktcnt_rx, int, 0444);
    MODULE_PARM_DESC(int_pktcnt_rx, "RX packet count");
    let mut int_timeout_rx: static int = 64;
...
pub const SBMAC_MAX_TXDESCR: c_int = 256;
pub const SBMAC_MAX_RXDESCR: c_int = 256;
pub const ENET_PACKET_SIZE: c_int = 1518;
...
// associated with it.
//
    pub associated: *mut *mut *mut sbmac_softc sbdma_eth; / back pointer to,
    MAC */
    pub /: *mut *mut int sbdma_channel; / channel number,
    pub /: *mut *mut int sbdma_txdir; / direction (1=transmit),
    pub descriptors: *mut *mut int sbdma_maxdescr; / total # of,
    in ring */

    pub sbdma_int_pktcnt: c_int,
// # descriptors rx/tx
    before interrupt */
    pub sbdma_int_timeout: c_int,
// # usec rx/tx interrupt

    pub /: *mut *mut *mut void __iomem sbdma_config0; / DMA config register 0,
    pub /: *mut *mut *mut void __iomem sbdma_config1; / DMA config register 1,
    pub sbdma_dscrbase: *mut void __iomem,
// descriptor base address
    pub /: *mut *mut *mut void __iomem sbdma_dscrcnt; / descriptor count register,
    pub descriptor: *mut *mut *mut void __iomem sbdma_curdscr; / current,
    address */
    pub sbdma_oodpktlost: *mut void __iomem,
// pkt drop (rx only)
//
// This stuff is for maintenance of the ring
//
    pub sbdma_dscrtable_unaligned: *mut c_void,
    pub sbdma_dscrtable: *mut sbdmadscr,
// base of descriptor table
    pub sbdma_dscrtable_end: *mut sbdmadscr,
// end of descriptor table
    pub sbdma_ctxtable: *mut sk_buff,
// context table, one
    per descr */
    pub sbdma_dscrtable_phys: dma_addr_t,
// and also the phys addr
    pub /: *mut *mut *mut sbdmadscr sbdma_addptr; / next dscr for sw to add,
    pub sw: *mut *mut *mut sbdmadscr sbdma_remptr; / next dscr for,
    to remove */
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sbmac_softc {
//
// Linux-specific things
//
    pub /: *mut *mut *mut net_device sbm_dev; / pointer to linux device,
    pub napi: napi_struct,
    pub /: *mut *mut *mut phy_device phy_dev; / the associated PHY device,
    pub /: *mut *mut *mut mii_bus mii_bus; / the MII bus,
    pub /: *mut *mut spinlock_t sbm_lock; / spin lock,
    pub /: *mut *mut int sbm_devflags; / current device flags,
//
// Controller-specific things
//
    pub /: *mut *mut *mut void __iomem sbm_base; / MAC's base address,
    pub /: *mut *mut enum sbmac_state sbm_state; / current state,
    pub /: *mut *mut *mut void __iomem sbm_macenable; / MAC Enable Register,
    pub /: *mut *mut *mut void __iomem sbm_maccfg; / MAC Config Register,
    pub /: *mut *mut *mut void __iomem sbm_fifocfg; / FIFO Config Register,
    pub /: *mut *mut *mut void __iomem sbm_framecfg; / Frame Config Register,
    pub /: *mut *mut *mut void __iomem sbm_rxfilter; / Receive Filter Register,
    pub /: *mut *mut *mut void __iomem sbm_isr; / Interrupt Status Register,
    pub /: *mut *mut *mut void __iomem sbm_imr; / Interrupt Mask Register,
    pub /: *mut *mut *mut void __iomem sbm_mdio; / MDIO Register,
    pub /: *mut *mut enum sbmac_speed sbm_speed; / current speed,
    pub /: *mut *mut enum sbmac_duplex sbm_duplex; / current duplex,
    pub /: *mut *mut enum sbmac_fc sbm_fc; / cur. flow control setting,
    pub /: *mut *mut int sbm_pause; / current pause setting,
    pub /: *mut *mut int sbm_link; / current link state,
    pub sbm_hwaddr: [c_uchar; ETH_ALEN],
    pub /: *mut *mut sbmacdma sbm_txdma; / only channel 0 for now,
    pub sbm_rxdma: sbmacdma,
    pub rx_hw_checksum: c_int,
    pub sbe_idx: c_int,
}
...
// MDIO constants
//
pub const MII_COMMAND_START: c_uint = 0x01;
pub const MII_COMMAND_READ: c_uint = 0x02;
pub const MII_COMMAND_WRITE: c_uint = 0x01;
pub const MII_COMMAND_ACK: c_uint = 0x02;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_mii_sync(sbm_mdio: *mut void __iomem) {
    static void sbmac_mii_sync(void __iomem *sbm_mdio)
    {
    int cnt;
    uint64_t bits;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_mii_read(bus: *mut mii_bus, phyaddr: c_int, regidx: c_int) -> c_int {
    static int sbmac_mii_read(struct mii_bus *bus, int phyaddr, int regidx)
    {
...
//
    ...
    if (d.sbdma_txdir)
    d.sbdma_oodpktlost = core::ptr::null_mut();
    else
    d.sbdma_oodpktlost =
...
//
#[no_mangle]
unsafe extern "C" fn sbdma_channel_start(d: *mut sbmacdma, rxtx: c_int) {
    static void sbdma_channel_start(struct sbmacdma *d, int rxtx)
    {
//
...
//
#[no_mangle]
unsafe extern "C" fn sbdma_channel_stop(d: *mut sbmacdma) {
...
// Zero ring pointers
//
    d.sbdma_addptr = core::ptr::null_mut();
    d.sbdma_remptr = core::ptr::null_mut();
    }
...
//
    ...
    struct sk_buff *sb_new = core::ptr::null_mut();
    let mut pktsize: c_int = ENET_PACKET_SIZE;
...
//
#[no_mangle]
unsafe extern "C" fn sbdma_add_txbuffer(d: *mut sbmacdma, sb: *mut sk_buff) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sbdma_emptyring(d: *mut sbmacdma) {
    static void sbdma_emptyring(struct sbmacdma *d)
    {
    int idx;
    struct sk_buff *sb;
    ...
    if (sb) {
    dev_kfree_skb(sb);
    d.sbdma_ctxtable[idx] = core::ptr::null_mut();
    }
    }
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn sbdma_fillring(sc: *mut sbmac_softc, d: *mut sbmacdma) {
    static void sbdma_fillring(struct sbmac_softc *sc, struct sbmacdma *d)
    {
    int idx;
    for (idx = 0; idx < SBMAC_MAX_RXDESCR - 1; idx++) {
    if (sbdma_add_rcvbuffer(sc, d, core::ptr::null_mut()) != 0)
    break;
    }
    }

#[no_mangle]
unsafe extern "C" fn sbmac_netpoll(netdev: *mut net_device) {
    static void sbmac_netpoll(struct net_device *netdev)
    {
    struct sbmac_softc *sc = netdev_priv(netdev);
    let mut irq: c_int = sc.sbm_dev.irq;
...
//
    ...
    d.sbdma_ctxtable[curidx] = core::ptr::null_mut();
...
//
    if (unlikely(sbdma_add_rcvbuffer(sc, d, core::ptr::null_mut()) ==
    -ENOBUFS)) {
...
//
    ...
    struct sk_buff *sb;
    unsigned long flags;
    let mut packets_handled: c_int = 0;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_initctx(s: *mut sbmac_softc) -> c_int {
    static int sbmac_initctx(struct sbmac_softc *s)
    {
//
...
#[no_mangle]
unsafe extern "C" fn sbdma_uninitctx(d: *mut sbmacdma) {
    static void sbdma_uninitctx(struct sbmacdma *d)
    {
    kfree(d.sbdma_dscrtable_unaligned);
    d.sbdma_dscrtable_unaligned = d.sbdma_dscrtable = core::ptr::null_mut();
    kfree(d.sbdma_ctxtable);
    d.sbdma_ctxtable = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn sbmac_uninitctx(sc: *mut sbmac_softc) {
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_channel_start(s: *mut sbmac_softc) {
    static void sbmac_channel_start(struct sbmac_softc *s)
    {
    uint64_t reg;
    void __iomem *port;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_channel_stop(s: *mut sbmac_softc) {
    static void sbmac_channel_stop(struct sbmac_softc *s)
    {
...
//
    ...
    {
    let mut oldstate: enum sbmac_state = sc.sbm_state;
//
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_promiscuous_mode(sc: *mut sbmac_softc, onoff: c_int) {
    static void sbmac_promiscuous_mode(struct sbmac_softc *sc,int onoff)
    {
    uint64_t reg;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_set_iphdr_offset(sc: *mut sbmac_softc) {
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_addr2reg(ptr: *mut c_uchar) -> u64 {
    static uint64_t sbmac_addr2reg(unsigned char *ptr)
    {
    let mut reg: u64 = 0;
    ptr += 6;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_set_speed(s: *mut sbmac_softc, speed: enum sbmac_speed) -> c_int {
    static int sbmac_set_speed(struct sbmac_softc *s, enum sbmac_speed speed)
    {
    uint64_t cfg;
    uint64_t framecfg;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_intr(irq: c_int, dev_instance: *mut c_void) -> irqreturn_t {
    static irqreturn_t sbmac_intr(int irq,void *dev_instance)
    {
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_start_tx(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
    static netdev_tx_t sbmac_start_tx(struct sk_buff *skb, struct net_device *dev)
    {
    struct sbmac_softc *sc = netdev_priv(dev);
    unsigned long flags;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_setmulti(sc: *mut sbmac_softc) {
    static void sbmac_setmulti(struct sbmac_softc *sc)
    {
    uint64_t reg;
    void __iomem *port;
...
//
#[no_mangle]
unsafe extern "C" fn sbmac_init(pldev: *mut platform_device, base: c_longlong) -> c_int {
    static int sbmac_init(struct platform_device *pldev, long long base)
    {
    struct net_device *dev = platform_get_drvdata(pldev);
    let mut idx: c_int = pldev.id;
    struct sbmac_softc *sc = netdev_priv(dev);
    unsigned char *eaddr;
    uint64_t ea_reg;
    int i;
...
// This is needed for PASS2 for Rx H/W checksum feature
    ...
    if (sc.mii_bus == core::ptr::null_mut()) {
    err = -ENOMEM;
    goto uninit_ctx;
    }
...
#[no_mangle]
unsafe extern "C" fn sbmac_open(dev: *mut net_device) -> c_int {
    static int sbmac_open(struct net_device *dev)
    {
    struct sbmac_softc *sc = netdev_priv(dev);
    int err;
...
#[no_mangle]
unsafe extern "C" fn sbmac_mii_probe(dev: *mut net_device) -> c_int {
    static int sbmac_mii_probe(struct net_device *dev)
    {
    struct sbmac_softc *sc = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn sbmac_mii_poll(dev: *mut net_device) {
    static void sbmac_mii_poll(struct net_device *dev)
    {
    struct sbmac_softc *sc = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn sbmac_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void sbmac_tx_timeout (struct net_device *dev, unsigned int txqueue)
    {
    struct sbmac_softc *sc = netdev_priv(dev);
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn sbmac_set_rx_mode(dev: *mut net_device) {
    static void sbmac_set_rx_mode(struct net_device *dev)
    {
    unsigned long flags;
    struct sbmac_softc *sc = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn sbmac_mii_ioctl(dev: *mut net_device, rq: *mut ifreq, cmd: c_int) -> c_int {
    static int sbmac_mii_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
    {
    struct sbmac_softc *sc = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn sbmac_close(dev: *mut net_device) -> c_int {
    static int sbmac_close(struct net_device *dev)
    {
    struct sbmac_softc *sc = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn sbmac_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    static int sbmac_poll(struct napi_struct *napi, int budget)
    {
...
#[no_mangle]
unsafe extern "C" fn sbmac_probe(pldev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sbmac_remove(pldev: *mut platform_device) {
