...
//! Source: drivers/net/ethernet/emulex/benet/be_ethtool.c
#![no_std]
...
pub const BE_MAC_LOOPBACK: c_uint = 0x0;
pub const BE_PHY_LOOPBACK: c_uint = 0x1;
pub const BE_ONE_PORT_EXT_LOOPBACK: c_uint = 0x2;
pub const BE_NO_LOOPBACK: c_uint = 0xff;
...
#[no_mangle]
unsafe extern "C" fn lancer_cmd_get_file_len(adapter: *mut be_adapter, file_name: *mut u8) -> u32 {
    static u32 lancer_cmd_get_file_len(struct be_adapter *adapter, u8 *file_name)
    {
    let mut data_read: u32 = 0, eof;
    u8 addn_status;
...
#[no_mangle]
unsafe extern "C" fn be_get_dump_len(adapter: *mut be_adapter) -> c_int {
    static int be_get_dump_len(struct be_adapter *adapter)
    {
    let mut dump_size: u32 = 0;
    ...
    let mut read_len: u32 = 0, total_read_len = 0, chunk_size;
    let mut eof: u32 = 0;
    u8 addn_status;
...
#[no_mangle]
unsafe extern "C" fn be_get_sset_count(netdev: *mut net_device, stringset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn be_get_port_type(adapter: *mut be_adapter) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn convert_to_et_setting(adapter: *mut be_adapter, if_speeds: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn be_pause_supported(adapter: *mut be_adapter) -> bool {
    ...
    u32 auto_speeds;
    u32 fixed_speeds;
    let mut supported: u32 = 0, advertising = 0;
...
#[no_mangle]
unsafe extern "C" fn be_set_dump(netdev: *mut net_device, dump: *mut ethtool_dump) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn be_get_wol(netdev: *mut net_device, wol: *mut ethtool_wolinfo) {
...
#[no_mangle]
unsafe extern "C" fn be_set_wol(netdev: *mut net_device, wol: *mut ethtool_wolinfo) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn be_test_ddr_dma(adapter: *mut be_adapter) -> c_int {
...
// link status was down prior to test
    ...
    for (cnt = 10; cnt; cnt--) {
    status = be_cmd_link_status_query(adapter, core::ptr::null_mut(), &link_status,
    0);
    if (status) {
    test.flags |= ETH_TEST_FL_FAILED;
    data[4] = -1;
...
#[no_mangle]
unsafe extern "C" fn be_do_flash(netdev: *mut net_device, efl: *mut ethtool_flash) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn be_get_eeprom_len(netdev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn be_get_msg_level(netdev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn be_set_msg_level(netdev: *mut net_device, level: u32) {
...
#[no_mangle]
unsafe extern "C" fn be_get_rx_ring_count(netdev: *mut net_device) -> u32 {
    ...
    {
    struct be_adapter *adapter = netdev_priv(netdev);
    let mut flow_type: u64 = cmd.flow_type;
    let mut data: u64 = 0;
    ...
    {
    struct be_adapter *adapter = netdev_priv(netdev);
    let mut rss_flags: u32 = adapter.rss_info.rss_flags;
    int status;
    if (!be_multi_rxq(adapter)) {
    dev_err(&adapter.pdev.dev,
...
#[no_mangle]
pub unsafe extern "C" fn if(L4_RSS_FLAGS): cmd->data == (L3_RSS_FLAGS |) -> else {
...
pub unsafe extern "C" fn if(L4_RSS_FLAGS): cmd->data == (L3_RSS_FLAGS |) -> else {
    ...
    {
    struct be_adapter *adapter = netdev_priv(netdev);
    let mut num_rx_irqs: u16 = max_t(u16, adapter.num_rss_qs, 1);
...
#[no_mangle]
unsafe extern "C" fn be_get_rxfh_indir_size(netdev: *mut net_device) -> u32 {
    static u32 be_get_rxfh_indir_size(struct net_device *netdev)
    {
    return RSS_INDIR_TABLE_LEN;
    }
#[no_mangle]
unsafe extern "C" fn be_get_rxfh_key_size(netdev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn be_get_priv_flags(netdev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn be_set_priv_flags(netdev: *mut net_device, flags: u32) -> c_int {
    static int be_set_priv_flags(struct net_device *netdev, u32 flags)
    {
    struct be_adapter *adapter = netdev_priv(netdev);
    let mut tpe_old: bool = !!(adapter.priv_flags & BE_DISABLE_TPE_RECOVERY);
    let mut tpe_new: bool = !!(flags & BE_DISABLE_TPE_RECOVERY);
    if (tpe_old != tpe_new) {
    if (tpe_new) {
