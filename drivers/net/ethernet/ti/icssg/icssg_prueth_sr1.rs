...
//! Source: drivers/net/ethernet/ti/icssg/icssg_prueth_sr1.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn emac_send_command_sr1(emac: *mut prueth_emac, cmd: u32) -> c_int {
    static int emac_send_command_sr1(struct prueth_emac *emac, u32 cmd)
    {
    struct cppi5_host_desc_t *first_desc;
    let mut pkt_len: u32 = sizeof(emac.cmd_data);
...
#[no_mangle]
unsafe extern "C" fn icssg_config_set_speed_sr1(emac: *mut prueth_emac) {
    static void icssg_config_set_speed_sr1(struct prueth_emac *emac)
    {
    let mut cmd: u32 = ICSSG_PSTATE_SPEED_DUPLEX_CMD_SR1, val;
    struct prueth *prueth = emac.prueth;
    let mut slice: c_int = prueth_emac_slice(emac);
...
#[no_mangle]
unsafe extern "C" fn emac_adjust_link_sr1(ndev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn emac_phy_connect(emac: *mut prueth_emac) -> c_int {
...
//
    ...
    flow_id);
    new_page = page;
    page = core::ptr::null_mut();	/* return core::ptr::null_mut() */
    }
// queue another DMA
    ...
    emac.tx_ts_skb[cookie] = core::ptr::null_mut();	/* free slot */
...
#[no_mangle]
unsafe extern "C" fn prueth_rx_mgm_ts_thread_sr1(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn prueth_rx_mgm_rsp_thread(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn prueth_emac_start(prueth: *mut prueth, emac: *mut prueth_emac) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn prueth_emac_stop(emac: *mut prueth_emac) {
...
//
#[no_mangle]
unsafe extern "C" fn emac_ndo_open(ndev: *mut net_device) -> c_int {
    static int emac_ndo_open(struct net_device *ndev)
    {
    struct prueth_emac *emac = netdev_priv(ndev);
    let mut num_data_chn: c_int = emac.tx_ch_num - 1;
    struct prueth *prueth = emac.prueth;
    let mut slice: c_int = prueth_emac_slice(emac);
    struct device *dev = prueth.dev;
...
// we use only the highest priority flow for now i.e. @irq[3]
    ...
    core::ptr::null_mut(), prueth_rx_mgm_rsp_thread,
    ...
    core::ptr::null_mut(), prueth_rx_mgm_ts_thread_sr1,
    IRQF_ONESHOT | IRQF_TRIGGER_HIGH,
    dev_name(dev), emac);
    if (ret) {
...
//
#[no_mangle]
unsafe extern "C" fn emac_ndo_stop(ndev: *mut net_device) -> c_int {
    static int emac_ndo_stop(struct net_device *ndev)
    {
    struct prueth_emac *emac = netdev_priv(ndev);
    let mut rx_flow: c_int = PRUETH_RX_FLOW_DATA_SR1;
    struct prueth *prueth = emac.prueth;
    int max_rx_flows;
    int ret, i;
...
#[no_mangle]
unsafe extern "C" fn emac_ndo_set_rx_mode_sr1(ndev: *mut net_device) {
    static void emac_ndo_set_rx_mode_sr1(struct net_device *ndev)
    {
    struct prueth_emac *emac = netdev_priv(ndev);
    let mut allmulti: bool = ndev.flags & IFF_ALLMULTI;
    let mut promisc: bool = ndev.flags & IFF_PROMISC;
    struct prueth *prueth = emac.prueth;
    let mut slice: c_int = prueth_emac_slice(emac);
    if (promisc) {
...
// get mac address from DT and set private and netdev addr
    ...
    free_ndev:
    emac.ndev = core::ptr::null_mut();
    prueth.emac[mac] = core::ptr::null_mut();
    free_netdev(ndev);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn prueth_probe(pdev: *mut platform_device) -> c_int {
    static int prueth_probe(struct platform_device *pdev)
    {
    struct device_node *eth_node, *eth_ports_node;
    struct device_node  *eth0_node = core::ptr::null_mut();
    struct device_node  *eth1_node = core::ptr::null_mut();
    ...
    eth0_node = core::ptr::null_mut();
    }
    } else if (reg == 1) {
    eth1_node = eth_node;
    if (!of_device_is_available(eth1_node)) {
    of_node_put(eth1_node);
    eth1_node = core::ptr::null_mut();
    }
    } else {
...
// At least one node must be present and available else we fail
    ...
    ret = icss_iep_init(prueth.iep0, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (ret) {
    dev_err_probe(dev, ret, "failed to init iep0\n");
    goto put_iep;
    }
    ret = icss_iep_init(prueth.iep1, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (ret) {
...
// register the network devices
    ...
    prueth.emac[i].ndev.phydev = core::ptr::null_mut();
    }
    ...
    prueth.iep0 = core::ptr::null_mut();
    prueth.iep1 = core::ptr::null_mut();
    free_pool:
...
#[no_mangle]
unsafe extern "C" fn prueth_remove(pdev: *mut platform_device) {
