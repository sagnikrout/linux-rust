...
//! Source: drivers/net/ethernet/netronome/nfp/crypto/ipsec.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_ipsec_cfg_add_sa {
    pub /: *mut *mut u32 ciph_key[8]; / Cipher Key,
    union {
    pub /: *mut *mut u32 auth_key[16]; / Authentication Key,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_ipsec_aesgcm {
    pub /: *mut *mut u32 salt; / Initialized with SA,
    pub resv: [u32; 15],
    pub aesgcm_fields: },
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sa_ctrl_word {
    pub /: *mut *mut uint32_t hash :4; / From nfp_ipsec_sa_hash_type,
    pub /: *mut *mut uint32_t cimode :4; / From nfp_ipsec_sa_cipher_mode,
    pub /: *mut *mut uint32_t cipher :4; / From nfp_ipsec_sa_cipher,
    pub /: *mut *mut uint32_t mode :2; / From nfp_ipsec_sa_mode,
    pub /: *mut *mut uint32_t proto :2; / From nfp_ipsec_sa_prot,
    pub /: *mut *mut uint32_t dir :1; / SA direction,
    pub :12: uint32_t resv0,
    pub /: *mut *mut uint32_t encap_dsbl:1; / Encap/Decap disable,
    pub /: *mut *mut uint32_t resv1 :2; / Must be set to 0,
    pub ctrl_word: },
    pub /: *mut *mut u32 spi; / SPI Value,
    pub /: *mut *mut uint32_t pmtu_limit :16; / PMTU Limit,
    pub :5: uint32_t resv0,
    pub /: *mut *mut uint32_t ipv6 :1; / Outbound IPv6 addr format,
    pub :10: uint32_t resv1,
    pub resv2: [u32; 2],
    pub /: *mut *mut u32 src_ip[4]; / Src IP addr,
    pub /: *mut *mut u32 dst_ip[4]; / Dst IP addr,
    pub resv3: [u32; 6],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_ipsec_cfg_mssg {
    union {
    struct{
    pub /: *mut *mut uint32_t cmd:16; / One of nfp_ipsec_cfg_mssg_cmd_codes,
    pub /: *mut *mut uint32_t rsp:16; / One of nfp_ipsec_cfg_mssg_rsp_codes,
    pub /: *mut *mut uint32_t sa_idx:16; / SA table index,
    pub spare0:16: u32,
    pub cfg_add_sa: nfp_ipsec_cfg_add_sa,
}
...
#[no_mangle]
unsafe extern "C" fn nfp_net_ipsec_cfg(nn: *mut nfp_net, entry: *mut nfp_mbox_amsg_entry) -> c_int {
    static int nfp_net_ipsec_cfg(struct nfp_net *nn, struct nfp_mbox_amsg_entry *entry)
    {
    let mut offset: c_uint = nn.tlv_caps.mbox_off + NFP_NET_CFG_MBOX_SIMPLE_VAL;
...
#[no_mangle]
unsafe extern "C" fn set_aes_keylen(cfg: *mut nfp_ipsec_cfg_add_sa, alg: c_int, keylen: c_int) -> c_int {
    static int set_aes_keylen(struct nfp_ipsec_cfg_add_sa *cfg, int alg, int keylen)
    {
    let mut aes_gmac: bool = (alg == SADB_X_EALG_NULL_AES_GMAC);
    switch (keylen) {
    case 128:
...
#[no_mangle]
unsafe extern "C" fn set_md5hmac(cfg: *mut nfp_ipsec_cfg_add_sa, trunc_len: *mut c_int) {
...
#[no_mangle]
unsafe extern "C" fn set_sha1hmac(cfg: *mut nfp_ipsec_cfg_add_sa, trunc_len: *mut c_int) {
...
#[no_mangle]
unsafe extern "C" fn set_sha2_256hmac(cfg: *mut nfp_ipsec_cfg_add_sa, trunc_len: *mut c_int) {
...
#[no_mangle]
unsafe extern "C" fn set_sha2_384hmac(cfg: *mut nfp_ipsec_cfg_add_sa, trunc_len: *mut c_int) {
...
#[no_mangle]
unsafe extern "C" fn set_sha2_512hmac(cfg: *mut nfp_ipsec_cfg_add_sa, trunc_len: *mut c_int) {
...
// trunc_len = 0;
    ...
    {
    let mut msg: nfp_ipsec_cfg_mssg = {};
...
// Aead->alg_key_len includes 32-bit salt
    ...
    if (x.aead) {
    let mut key_offset: c_int = 0;
    let mut salt_len: c_int = 4;
...
//
    ...
    let mut index: c_int = (i + key_offset) % (key_len / sizeof(cfg.ciph_key[0]));
...
#[no_mangle]
unsafe extern "C" fn nfp_net_xfrm_del_state(dev: *mut net_device, x: *mut xfrm_state) {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_net_ipsec_init(nn: *mut nfp_net) {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_net_ipsec_clean(nn: *mut nfp_net) {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_net_ipsec_rx(meta: *mut nfp_meta_parsed, skb: *mut sk_buff) -> c_int {
