...
//! Source: drivers/net/ethernet/netronome/nfp/nfpcore/nfp_rtsym.c
#![no_std]
...
pub const SYM_TGT_LMEM: c_int = 0;
pub const SYM_TGT_EMU_CACHE: c_uint = 0x17;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_rtsym_entry {
    pub type: u8,
    pub target: u8,
    pub island: u8,
    pub addr_hi: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_rtsym_table {
    pub cpp: *mut nfp_cpp,
    pub num: c_int,
    pub strtab: *mut c_char,
    pub symtab: [nfp_rtsym; ],
}

#[no_mangle]
unsafe extern "C" fn nfp_meid(island_id: u8, menum: u8) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(0xff: fw->island !=) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn nfp_rtsym_count(rtbl: *mut nfp_rtsym_table) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_rtsym_size(sym: *const nfp_rtsym) -> u64 {
...
// addr = sym->addr + off;
    ...
    let mut locality_off: c_int = nfp_cpp_mu_locality_lsb(cpp);
...
// cpp_id = NFP_CPP_ISLAND_ID(sym->target, action, token,
    ...
    {
    let mut sym_size: u64 = nfp_rtsym_size(sym);
    u32 cpp_id;
    u64 addr;
    int err;
    if (off > sym_size) {
