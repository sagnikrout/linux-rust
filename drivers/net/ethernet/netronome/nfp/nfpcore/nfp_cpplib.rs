...
//! Source: drivers/net/ethernet/netronome/nfp/nfpcore/nfp_cpplib.c
#![no_std]
...
// NFP6000 PL
pub const NFP_PL_DEVICE_PART_NFP6000: c_uint = 0x6200;
pub const NFP_PL_DEVICE_ID: c_uint = 0x00000004;
...
//
#[no_mangle]
pub unsafe extern "C" fn nfp_cpp_model_autodetect(cpp: *mut nfp_cpp, model: *mut u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfp_bytemask(width: c_int, addr: u64) -> u8 {
    static u8 nfp_bytemask(int width, u64 addr)
    {
    if (width == 8)
    return 0xff;
#[no_mangle]
pub unsafe extern "C" fn if(4: width ==) -> else {
    else if (width == 4)
    return 0x0f << (addr & 4);
#[no_mangle]
pub unsafe extern "C" fn if(2: width ==) -> else {
    else if (width == 2)
    return 0x03 << (addr & 6);
#[no_mangle]
