...
//! Source: drivers/net/ethernet/netronome/nfp/flower/offload.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_flower_merge_check {
    union {
    struct {
    pub tci: __be16,
    pub l2: nfp_flower_mac_mpls,
    pub l4: nfp_flower_tp_ports,
    union {
    pub ipv4: nfp_flower_ipv4,
    pub ipv6: nfp_flower_ipv6,
}
...
#[no_mangle]
unsafe extern "C" fn nfp_flower_check_higher_than_mac(rule: *mut flow_rule) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nfp_flower_check_higher_than_l3(rule: *mut flow_rule) -> bool {
...
// key_size += sizeof(struct nfp_flower_ipv4_udp_tun);
    ...
    let mut basic: flow_match_basic = { core::ptr::null_mut(), core::ptr::null_mut()};
...
// If any tun dissector is used then the required set must be used.
    ...
    let mut enc_op: flow_match_enc_opts = { core::ptr::null_mut(), core::ptr::null_mut() };
    struct flow_match_ipv4_addrs ipv4_addrs;
    struct flow_match_ipv6_addrs ipv6_addrs;
    struct flow_match_control enc_ctl;
    struct flow_match_ports enc_ports;
    let mut ipv6_tun: bool = false;
...
//
    ...
    flow_pay.nfp_tun_ipv6 = core::ptr::null_mut();
    flow_pay.meta.flags = 0;
    INIT_LIST_HEAD(&flow_pay.linked_flows);
    flow_pay.in_hw = false;
    flow_pay.pre_tun_rule.dev = core::ptr::null_mut();
    return flow_pay;
    err_free_mask:
...
//
    ...
    bool *tunnel_act)
    {
    let mut act_off: c_uint = 0, act_len;
    struct nfp_fl_act_head *a;
    let mut act_id: u8 = 0;
    while (act_off < len) {
...
#[no_mangle]
pub unsafe extern "C" fn if(NFP_FL_ACTION_OPCODE_OUTPUT: a->jump_id !=) -> else {
...
// Return error if no tunnel action is found.
    ...
    struct nfp_fl_push_vlan *post_tun_push_vlan = core::ptr::null_mut();
    let mut tunnel_act: bool = false;
    char *merge_act;
    int err;
...
// Copy any pre-actions to the start of merge flow action list.
    ...
    sub2_act_len, core::ptr::null_mut());
    merge_act += pre_off2;
...
#[no_mangle]
unsafe extern "C" fn nfp_flower_unlink_flow(link: *mut nfp_fl_payload_link) {
...
// Check if the two flows are already merged
    ...
    merge_flow.ingress_dev, core::ptr::null_mut());
    if (err)
...
//
    ...
    let mut vlan_tci: u16 = be16_to_cpu(meta_tci.tci);
...
// Flags and proto fields have same offset in IPv4 and IPv6.
    let mut ip_flags: c_int = offsetof(struct nfp_flower_ipv4, ip_ext.flags);
    let mut ip_proto: c_int = offsetof(struct nfp_flower_ipv4, ip_ext.proto);
    int size;
    int i;
...
#[no_mangle]
unsafe extern "C" fn offload_pre_check(flow: *mut flow_cls_offload) -> bool {
...
//
    ...
    {
    struct flow_rule *rule = flow_cls_offload_flow_rule(flow);
    let mut tun_type: enum nfp_flower_tun_type = NFP_FL_TUNNEL_NONE;
    struct nfp_flower_priv *priv = app.priv;
    struct netlink_ext_ack *extack = core::ptr::null_mut();
    struct nfp_fl_payload *flow_pay;
    struct nfp_fl_key_ls *key_layer;
    struct nfp_port *port = core::ptr::null_mut();
    int err;
    extack = flow.common.extack;
    if (nfp_netdev_is_nfp_repr(netdev))
    port = nfp_port_from_netdev(netdev);
    if (is_pre_ct_flow(flow))
    return nfp_fl_ct_handle_pre_ct(priv, netdev, flow, extack, core::ptr::null_mut());
...
// Clean any links connected with the merged flow.
    ...
    merge_flow.list) {
    let mut ctx_id: u32 = be32_to_cpu(link.sub_flow.flow.meta.host_ctx_id);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_flower_indr_block_cb_priv {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_flower_setup_indr_tc_release(cb_priv: *mut c_void) {
