...
//! Source: drivers/net/ethernet/netronome/nfp/flower/action.c
#![no_std]
...
//
    ...
    {
    let mut act_size: usize = sizeof(struct nfp_fl_push_mpls);
    let mut mpls_lse: u32 = 0;
...
// Proto, label and TTL are enforced and verified for MPLS push.
    ...
    const struct flow_action_entry *act)
    {
    let mut act_size: usize = sizeof(struct nfp_fl_pop_mpls);
    ...
    const struct flow_action_entry *act)
    {
    let mut act_size: usize = sizeof(struct nfp_fl_set_mpls);
    let mut mpls_lse: u32 = 0, mpls_mask = 0;
...
#[no_mangle]
unsafe extern "C" fn nfp_fl_pop_vlan(pop_vlan: *mut nfp_fl_pop_vlan) {
    static void nfp_fl_pop_vlan(struct nfp_fl_pop_vlan *pop_vlan)
    {
    let mut act_size: usize = sizeof(struct nfp_fl_pop_vlan);
    ...
    const struct flow_action_entry *act)
    {
    let mut act_size: usize = sizeof(struct nfp_fl_push_vlan);
    ...
    struct netlink_ext_ack *extack)
    {
    let mut act_size: usize = sizeof(struct nfp_fl_pre_lag);
...
//
    ...
    {
    let mut act_size: usize = sizeof(struct nfp_fl_output);
...
// Confirm ingress and egress are on same device.
    ...
    let mut num_act: c_int = rule.action.num_entries;
    int act_idx;
...
//
    ...
    {
    let mut act_size: usize = sizeof(struct nfp_fl_pre_tunnel);
...
// list_len += tot_push_len;
    ...
    let mut ipv6: bool = ip_tunnel_info_af(ip_tun) == AF_INET6;
    let mut act_size: usize = sizeof(struct nfp_fl_set_tun);
    struct nfp_flower_priv *priv = app.priv;
    let mut tmp_set_ip_tun_type_index: u32 = 0;
// Currently support one pre-tunnel so index is always 0.
    let mut pretun_idx: c_int = 0;
    __be16 tun_flags;
...
// Set tunnel type and pre-tunnel index.
    ...
    dst = ip6_dst_lookup_flow(net, core::ptr::null_mut(), &flow, core::ptr::null_mut());
    if (!IS_ERR(dst)) {
...
#[no_mangle]
unsafe extern "C" fn nfp_fl_set_helper32(value: u32, mask: u32, p_exact: *mut u8, p_mask: *mut u8) {
    static void nfp_fl_set_helper32(u32 value, u32 mask, u8 *p_exact, u8 *p_mask)
    {
    let mut oldvalue: u32 = get_unaligned((u32 *)p_exact);
    let mut oldmask: u32 = get_unaligned((u32 *)p_mask);
    value &= mask;
    value |= oldvalue & ~mask;
...
// We are expecting tcf_pedit to return a big endian value
    mask = ( __be32)~act.mangle.mask;
    exact = ( __be32)act.mangle.val;
    if (exact & ~mask) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ipv6_hop_limit_word {
...
#[no_mangle]
unsafe extern "C" fn nfp_fl_csum_l4_to_flag(ip_proto: u8) -> u32 {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_flower_pedit_acts {
    pub set_ip6_src: nfp_fl_set_ipv6_addr set_ip6_dst,,
    pub set_ip6_tc_hl_fl: nfp_fl_set_ipv6_tc_hl_fl,
    pub set_ip_ttl_tos: nfp_fl_set_ip4_ttl_tos,
    pub set_ip_addr: nfp_fl_set_ip4_addrs,
    pub set_tport: nfp_fl_set_tport,
    pub set_eth: nfp_fl_set_eth,
}
...
// csum_updated |= nfp_fl_csum_l4_to_flag(ip_proto);
    ...
    {
    let mut act_size: usize = sizeof(struct nfp_fl_meter);
...
// Currently we do not handle any other actions.
    ...
    let mut pkt_host: bool = false;
    let mut csum_updated: u32 = 0;
