...
//! Source: drivers/net/ethernet/netronome/nfp/flower/lag_conf.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_flower_cmsg_lag_config {
    pub ctrl_flags: u8,
    pub reserved: [u8; 2],
    pub ttl: u8,
    pub pkt_number: __be32,
    pub batch_ver: __be32,
    pub group_id: __be32,
    pub group_inst: __be32,
    pub members: [__be32; ],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfp_fl_lag_group {
    pub group_id: c_uint,
    pub group_inst: u8,
    pub list: list_head,
    pub master_ndev: *mut net_device,
    pub dirty: bool,
    pub offloaded: bool,
    pub to_remove: bool,
    pub to_destroy: bool,
    pub slave_cnt: c_uint,
}

pub const NFP_FL_LAG_HOST_TTL: c_uint = 0xff;
// Use this ID with zero members to ack a batch config
pub const NFP_FL_LAG_SYNC_ID: c_int = 0;

// wait for more config

#[no_mangle]
unsafe extern "C" fn nfp_fl_get_next_pkt_number(lag: *mut nfp_fl_lag) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn nfp_fl_increment_version(lag: *mut nfp_fl_lag) {
...
// Zero is reserved by firmware.
    ...
    struct nfp_fl_lag_group *group = core::ptr::null_mut();
    __be32 temp_vers;
...
// group_inst = group->group_inst;
    ...
    {
    nfp_fl_lag_get_group_info(app, netdev, core::ptr::null_mut(),
    lag.lag_version, &lag.lag_instance);
    }
#[no_mangle]
pub unsafe extern "C" fn nfp_flower_lag_get_output_id(app: *mut nfp_app, master: *mut net_device) -> c_int {
    int nfp_flower_lag_get_output_id(struct nfp_app *app, struct net_device *master)
    {
    struct nfp_flower_priv *priv = app.priv;
    struct nfp_fl_lag_group *group = core::ptr::null_mut();
    let mut group_id: c_int = -ENOENT;
    mutex_lock(&priv.nfp_lag.lock);
    group = nfp_fl_lag_find_group_for_master_with_lag(&priv.nfp_lag,
    master);
    if (group)
...
#[no_mangle]
unsafe extern "C" fn nfp_fl_lag_do_work(work: *mut work_struct) {
    static void nfp_fl_lag_do_work(struct work_struct *work)
    {
    let mut batch: enum nfp_fl_lag_batch = NFP_FL_LAG_BATCH_FIRST;
    ...
    let mut active_count: c_int = 0, slaves = 0;
    struct nfp_repr *repr;
    unsigned long *flags;
    if (entry.to_remove) {
// Active count of 0 deletes group on hw.
    err = nfp_fl_lag_config_group(lag, entry, core::ptr::null_mut(), 0,
    &batch);
    if (!err) {
...
// End the config batch if at least one packet has been batched.
    ...
    batch = NFP_FL_LAG_BATCH_FINISHED;
    err = nfp_fl_lag_config_group(lag, core::ptr::null_mut(), core::ptr::null_mut(), 0, &batch);
    if (err)
    nfp_flower_cmsg_warn(priv.app,
...
#[no_mangle]
unsafe extern "C" fn nfp_fl_send_unprocessed(lag: *mut nfp_fl_lag) {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_flower_lag_unprocessed_msg(app: *mut nfp_app, skb: *mut sk_buff) -> bool {
...
//
    ...
    struct nfp_fl_lag_group *group;
    struct nfp_flower_priv *priv;
    let mut slave_count: c_uint = 0;
    let mut can_offload: bool = true;
    struct nfp_repr *repr;
...
#[no_mangle]
pub unsafe extern "C" fn nfp_flower_lag_reset(lag: *mut nfp_fl_lag) -> c_int {
    int nfp_flower_lag_reset(struct nfp_fl_lag *lag)
    {
    let mut batch: enum nfp_fl_lag_batch = NFP_FL_LAG_BATCH_FIRST;
    lag.rst_cfg = true;
    return nfp_fl_lag_config_group(lag, core::ptr::null_mut(), core::ptr::null_mut(), 0, &batch);
    }
#[no_mangle]
pub unsafe extern "C" fn nfp_flower_lag_init(lag: *mut nfp_fl_lag) {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_flower_lag_cleanup(lag: *mut nfp_fl_lag) {
