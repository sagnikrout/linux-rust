...
//! Source: drivers/net/ethernet/netronome/nfp/nfdk/dp.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn nfp_nfdk_tx_ring_should_wake(tx_ring: *mut nfp_net_tx_ring) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfp_nfdk_tx_ring_should_stop(tx_ring: *mut nfp_net_tx_ring) -> c_int {
...
// Under count by 1 (don't count meta) for the round down to work out
    ...
    tx_ring.ktxbufs[wr_idx].skb = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn nfp_nfdk_tx(skb: *mut sk_buff, netdev: *mut net_device) -> netdev_tx_t {
...
#[no_mangle]
pub unsafe extern "C" fn if(NFDK_TX_MAX_DATA_PER_HEAD: !nr_frags && dma_len <=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn nfp_nfdk_tx_complete(tx_ring: *mut nfp_net_tx_ring, budget: c_int) {
    static void nfp_nfdk_tx_complete(struct nfp_net_tx_ring *tx_ring, int budget)
    {
    struct nfp_net_r_vector *r_vec = tx_ring.r_vec;
    struct nfp_net_dp *dp = &r_vec.nfp_net.dp;
    let mut done_pkts: u32 = 0, done_bytes = 0;
...
//
#[no_mangle]
unsafe extern "C" fn nfp_nfdk_rx_csum_has_errors(flags: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfp_nfdk_xdp_complete(tx_ring: *mut nfp_net_tx_ring) -> bool {
...
// Work out how many descriptors have been transmitted
    ...
    while (todo > 0) {
    let mut idx: c_int = D_IDX(tx_ring, tx_ring.rd_p + done);
    struct nfp_nfdk_tx_buf *txbuf;
...
// Note: tx_bytes not accumulated.
    ...
    {
    let mut dma_map_sz: c_uint = dp.fl_bufsz - NFP_NET_RX_BUF_NON_DATA;
...
// Check if cross block boundary
    ...
    let mut nop_slots: c_uint = D_BLOCK_CPL(tx_ring.wr_p);
    wr_idx = D_IDX(tx_ring, tx_ring.wr_p);
    txd = &tx_ring.ktxds[wr_idx];
    memset(txd, 0,
...
//
#[no_mangle]
unsafe extern "C" fn nfp_nfdk_rx(rx_ring: *mut nfp_net_rx_ring, budget: c_int) -> c_int {
    ...
    let mut xdp_tx_cmpl: bool = false;
    unsigned int true_bufsz;
    struct sk_buff *skb;
    let mut pkts_polled: c_int = 0;
    struct xdp_buff xdp;
    int idx;
    ...
    let mut redir_egress: bool = false;
    struct net_device *netdev;
    dma_addr_t new_dma_addr;
    let mut meta_len_xdp: u32 = 0;
    void *new_frag;
...
// Stats update
    ...
    meta_len);
    nfp_nfdk_rx_drop(dp, r_vec, rx_ring, rxbuf, core::ptr::null_mut());
    continue;
    }
    ...
    nfp_nfdk_rx_drop(dp, r_vec, rx_ring, core::ptr::null_mut(), skb);
    continue;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn nfp_nfdk_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    int nfp_nfdk_poll(struct napi_struct *napi, int budget)
    {
    struct nfp_net_r_vector *r_vec =
    container_of(napi, struct nfp_net_r_vector, napi);
    let mut pkts_polled: c_uint = 0;
    if (r_vec.tx_ring)
...
#[no_mangle]
unsafe extern "C" fn __nfp_ctrl_tx_queued(r_vec: *mut nfp_net_r_vector) {
...
#[no_mangle]
unsafe extern "C" fn nfp_ctrl_rx(r_vec: *mut nfp_net_r_vector) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn nfp_nfdk_ctrl_poll(t: *mut tasklet_struct) {
