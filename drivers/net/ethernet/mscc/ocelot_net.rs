...
//! Source: drivers/net/ethernet/mscc/ocelot_net.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn devlink_port_to_port(dlp: *mut devlink_port) -> c_int {
    ...
    {
    struct ocelot *ocelot = devlink_port_to_ocelot(dlp);
    let mut port: c_int = devlink_port_to_port(dlp);
    ...
    let mut id_len: c_int = sizeof(ocelot.base_mac);
...
#[no_mangle]
pub unsafe extern "C" fn ocelot_port_devlink_teardown(ocelot: *mut ocelot, port: c_int) {
    ...
    let mut port: c_int = priv.port.index;
    if (!ingress)
    return -EOPNOTSUPP;
    switch (f.command) {
    ...
    return -EBUSY;
    block_cb = flow_block_cb_alloc(cb, priv, priv, core::ptr::null_mut());
    if (IS_ERR(block_cb))
...
#[no_mangle]
unsafe extern "C" fn ocelot_vlan_vid_del(dev: *mut net_device, vid: u16) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_port_open(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_port_stop(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_port_xmit(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ocelot_mact_work_ctx {
    pub work: work_struct,
    pub ocelot: *mut ocelot,
    pub type: enum ocelot_action_type,
    union {
// OCELOT_MACT_LEARN
    struct {
    pub addr: [c_uchar; ETH_ALEN],
    pub vid: u16,
    pub entry_type: enum macaccess_entry_type,
    pub pgid: c_int,
    pub learn: },
// OCELOT_MACT_FORGET
    struct {
...
#[no_mangle]
unsafe extern "C" fn ocelot_mact_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn ocelot_mc_unsync(dev: *mut net_device, addr: *const c_uchar) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_mc_sync(dev: *mut net_device, addr: *const c_uchar) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_set_rx_mode(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn ocelot_port_set_mac_address(dev: *mut net_device, p: *mut c_void) -> c_int {
...
// Then forget the previous one.
    ...
    let mut portid: u32 = NETLINK_CB(dump.cb.skb).portid;
    let mut seq: u32 = dump.cb.nlh.nlmsg_seq;
    struct nlmsghdr *nlh;
    struct ndmsg *ndm;
...
#[no_mangle]
unsafe extern "C" fn ocelot_ioctl(dev: *mut net_device, ifr: *mut ifreq, cmd: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_netdevice_dev_check(dev: *const net_device) -> bool {
    static bool ocelot_netdevice_dev_check(const struct net_device *dev)
    {
    return dev.netdev_ops == &ocelot_port_netdev_ops;
    }
#[no_mangle]
pub unsafe extern "C" fn ocelot_netdev_to_port(ocelot: *mut ocelot, dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ocelot_port_get_sset_count(dev: *mut net_device, sset: c_int) -> c_int {
    ...
    {
    let mut ageing_jiffies: c_ulong = clock_t_to_jiffies(ageing_clock_t);
    let mut ageing_time: u32 = jiffies_to_msecs(ageing_jiffies);
    ocelot_set_ageing_time(ocelot, ageing_time);
    }
#[no_mangle]
unsafe extern "C" fn ocelot_port_attr_mc_set(ocelot: *mut ocelot, port: c_int, mc: bool) {
    ...
    {
    let mut flags: switchdev_brport_flags = {0};
    int flag;
...
#[no_mangle]
unsafe extern "C" fn ocelot_clear_brport_flags(ocelot: *mut ocelot, port: c_int) {
...
#[no_mangle]
unsafe extern "C" fn ocelot_switchdev_unsync(ocelot: *mut ocelot, port: c_int) -> c_int {
    static int ocelot_switchdev_unsync(struct ocelot *ocelot, int port)
    {
    int err;
    err = ocelot_port_vlan_filtering(ocelot, port, false, core::ptr::null_mut());
    if (err)
    return err;
    ...
    {
    let mut bridge_num: c_int = ocelot_bridge_num_find(ocelot, bridge_dev);
    if (bridge_num < 0) {
...
//
    ...
    struct ocelot_port *ocelot_port = &priv.port;
    struct ocelot *ocelot = ocelot_port.ocelot;
    let mut bridge_num: c_int = ocelot_port.bridge_num;
    let mut port: c_int = priv.port.index;
    int err;
...
//
    ...
    {
    struct ocelot_port_private *priv = netdev_priv(dev);
    let mut is_active: bool = info.link_up && info.tx_enabled;
    struct ocelot_port *ocelot_port = &priv.port;
    struct ocelot *ocelot = ocelot_port.ocelot;
    let mut port: c_int = priv.port.index;
    if (!ocelot_port.bond)
...
//
    ...
    priv.phylink = core::ptr::null_mut();
    return err;
    }
    return 0;
    }
    ...
    out:
    ocelot.ports[port] = core::ptr::null_mut();
    free_netdev(dev);
    return err;
    }
#[no_mangle]
pub unsafe extern "C" fn ocelot_release_port(ocelot_port: *mut ocelot_port) {
