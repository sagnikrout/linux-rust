...
//! Source: drivers/net/ethernet/mscc/ocelot_flower.c
#![no_std]
...
//
pub const VCAP_BLOCK: c_int = 10000;
pub const VCAP_LOOKUP: c_int = 1000;
pub const VCAP_IS1_NUM_LOOKUPS: c_int = 3;
pub const VCAP_IS2_NUM_LOOKUPS: c_int = 2;
pub const VCAP_IS2_NUM_PAG: c_int = 256;
...
#[no_mangle]
unsafe extern "C" fn ocelot_chain_to_block(chain: c_int, ingress: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ocelot_chain_to_lookup(chain: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ocelot_chain_to_pag(chain: c_int) -> c_int {
...
// VCAP IS2 lookup 1 can goto to PSFP block if hardware support
    ...
    struct netlink_ext_ack *extack)
    {
    let mut egress_port: c_int = ocelot.ops.netdev_to_port(ocelot, a.dev);
...
// Mirroring towards foreign interfaces is handled in software
    ...
    let mut allow_missing_goto_target: bool = false;
    const struct flow_action_entry *a;
    enum ocelot_tag_tpid_sel tpid;
    ...
    let mut pag: c_int = ocelot_chain_to_pag(filter.goto_target);
    ...
    let mut key_length: c_int = vcap.keys[VCAP_ES0_IGR_PORT].length;
    struct netlink_ext_ack *extack = f.common.extack;
    ...
    struct netlink_ext_ack *extack = f.common.extack;
    let mut proto: u16 = ntohs(f.common.protocol);
    let mut match_protocol: bool = true;
    int ret;
...
// ocelot_vcap_filter_create(struct ocelot *ocelot, int port, bool ingress,
    ...
    const struct vcap_props *vcap = &ocelot.vcap[VCAP_ES0];
    let mut key_length: c_int = vcap.keys[VCAP_ES0_EGR_PORT].length;
...
//
    ...
    struct netlink_ext_ack *extack = f.common.extack;
    struct ocelot_vcap_filter *filter;
    let mut chain: c_int = f.common.chain_index;
    int block_id, ret;
