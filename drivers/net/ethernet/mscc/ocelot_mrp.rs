...
//! Source: drivers/net/ethernet/mscc/ocelot_mrp.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ocelot_mrp_del_vcap(ocelot: *mut ocelot, id: c_int) -> c_int {
    ...
    err = ocelot_vcap_filter_add(ocelot, filter, core::ptr::null_mut());
    if (err)
    kfree(filter);
    return err;
    }
#[no_mangle]
unsafe extern "C" fn ocelot_populate_mrp_trap_key(filter: *mut ocelot_vcap_filter) {
...
#[no_mangle]
unsafe extern "C" fn ocelot_mrp_trap_add(ocelot: *mut ocelot, port: c_int) -> c_int {
    static int ocelot_mrp_trap_add(struct ocelot *ocelot, int port)
    {
    let mut cookie: c_ulong = OCELOT_VCAP_IS2_MRP_TRAP(ocelot);
    return ocelot_trap_add(ocelot, port, cookie, false,
    ocelot_populate_mrp_trap_key);
    }
#[no_mangle]
unsafe extern "C" fn ocelot_mrp_trap_del(ocelot: *mut ocelot, port: c_int) -> c_int {
