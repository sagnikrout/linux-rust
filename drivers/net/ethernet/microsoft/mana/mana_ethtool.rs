...
//! Source: drivers/net/ethernet/microsoft/mana/mana_ethtool.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mana_get_sset_count(ndev: *mut net_device, stringset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mana_get_strings_stats(apc: *mut mana_port_context, data: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn mana_get_strings_priv_flags(data: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn mana_get_strings(ndev: *mut net_device, stringset: u32, data: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn mana_get_rx_ring_count(ndev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn mana_get_rxfh_key_size(ndev: *mut net_device) -> u32 {
    static u32 mana_get_rxfh_key_size(struct net_device *ndev)
    {
    return MANA_HASH_KEY_SIZE;
    }
#[no_mangle]
unsafe extern "C" fn mana_rss_indir_size(ndev: *mut net_device) -> u32 {
    ...
    {
    struct mana_port_context *apc = netdev_priv(ndev);
    let mut update_hash: bool = false, update_table = false;
...
pub const MANA_RX_CQE_NSEC_DEF: c_int = 2048;
...
//
    ...
    let mut modr_changed: bool = false;
    let mut dim_changed: bool = false;
...
// CQE coalescing setting is applied via RSS configuration.
    ...
    let mut new_rx_dim: bool = !!ec.use_adaptive_rx_coalesce;
    let mut new_tx_dim: bool = !!ec.use_adaptive_tx_coalesce;
    let mut disable_rx_dim: bool = saved.rx_dim_enabled && !new_rx_dim;
    let mut disable_tx_dim: bool = saved.tx_dim_enabled && !new_tx_dim;
    let mut enable_rx_dim: bool = !saved.rx_dim_enabled && new_rx_dim;
    let mut enable_tx_dim: bool = !saved.tx_dim_enabled && new_tx_dim;
    int q;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: enable_rx_dim) -> else {
    else if (enable_rx_dim)
    mana_dim_change(rx_cq, true);
#[no_mangle]
pub unsafe extern "C" fn if(modr_changed: !new_rx_dim &&) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: enable_tx_dim) -> else {
    else if (enable_tx_dim)
    mana_dim_change(tx_cq, true);
#[no_mangle]
pub unsafe extern "C" fn if(modr_changed: !new_tx_dim &&) -> else {
...
//
    ...
    {
    struct mana_port_context *apc = netdev_priv(ndev);
    let mut new_count: c_uint = channels.combined_count;
    let mut old_count: c_uint = apc.num_queues;
    int err;
...
#[no_mangle]
unsafe extern "C" fn mana_get_priv_flags(ndev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn mana_set_priv_flags(ndev: *mut net_device, priv_flags: u32) -> c_int {
    static int mana_set_priv_flags(struct net_device *ndev, u32 priv_flags)
    {
    struct mana_port_context *apc = netdev_priv(ndev);
    let mut changed: u32 = apc.priv_flags ^ priv_flags;
    let mut old_priv_flags: u32 = apc.priv_flags;
    let mut err: c_int = 0;
    if (!changed)
    return 0;
// Reject unknown bits
