...
//! Source: drivers/net/ethernet/mediatek/mtk_ppe_offload.c
#![no_std]
...
pub struct mtk_flow_data {
    ...
    struct mtk_flow_data *data)
    {
    let mut val: u32 = ntohl(act.mangle.val);
...
#[no_mangle]
pub unsafe extern "C" fn if(eth->netdev[1]: dev ==) -> else {
    else if (dev == eth.netdev[1])
    pse_port = PSE_GDM2_PORT;
#[no_mangle]
pub unsafe extern "C" fn if(eth->netdev[2]: dev ==) -> else {
    ...
    struct net_device *idev = core::ptr::null_mut(), *odev = core::ptr::null_mut();
    struct flow_action_entry *act;
    let mut data: mtk_flow_data = {};
    struct mtk_foe_entry foe;
    struct mtk_flow_entry *entry;
    let mut offload_type: c_int = 0;
    let mut wed_index: c_int = -1;
...
// handled earlier
    ...
    block_cb = flow_block_cb_alloc(cb, dev, dev, core::ptr::null_mut());
    if (IS_ERR(block_cb))
...
#[no_mangle]
pub unsafe extern "C" fn mtk_eth_offload_init(eth: *mut mtk_eth, id: u8) -> c_int {
