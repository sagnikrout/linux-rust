...
//! Source: drivers/net/ethernet/intel/igb/igb_xsk.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn igb_realloc_rx_buffer_info(ring: *mut igb_ring, pool_present: bool) -> c_int {
    ...
    ring.rx_buffer_info = core::ptr::null_mut();
    ring.rx_buffer_info_zc = buff_info;
    } else {
    vfree(ring.rx_buffer_info_zc);
    ring.rx_buffer_info_zc = core::ptr::null_mut();
    ring.rx_buffer_info = buff_info;
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn igb_txrx_ring_disable(adapter: *mut igb_adapter, qid: u16) {
...
#[no_mangle]
unsafe extern "C" fn igb_txrx_ring_enable(adapter: *mut igb_adapter, qid: u16) {
...
// Rx/Tx share the same napi context.
    ...
    return (pool && pool.dev) ? pool : core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn igb_xsk_pool_disable(adapter: *mut igb_adapter, qid: u16) -> c_int {
...
// nothing to do
    ...
    {
    let mut nb_buffs_extra: u32 = 0, nb_buffs = 0;
    union e1000_adv_rx_desc *rx_desc;
    let mut ntu: u16 = rx_ring.next_to_use;
    let mut total_count: u16 = count;
    struct xdp_buff **xdp;
...
//
    ...
    let mut total_count: return = = (nb_buffs + nb_buffs_extra);
    }
#[no_mangle]
pub unsafe extern "C" fn igb_clean_rx_ring_zc(rx_ring: *mut igb_ring) {
    void igb_clean_rx_ring_zc(struct igb_ring *rx_ring)
    {
    let mut ntc: u16 = rx_ring.next_to_clean;
    let mut ntu: u16 = rx_ring.next_to_use;
    while (ntc != ntu) {
...
// allocate a skb to store the frags
    ...
    let mut ntc: u32 = rx_ring.next_to_clean;
    struct bpf_prog *xdp_prog;
...
#[no_mangle]
pub unsafe extern "C" fn igb_xmit_zc(tx_ring: *mut igb_ring, xsk_pool: *mut xsk_buff_pool) -> bool {
    bool igb_xmit_zc(struct igb_ring *tx_ring, struct xsk_buff_pool *xsk_pool)
    {
    let mut budget: c_uint = igb_desc_unused(tx_ring);
    ...
    tx_buffer_info.xdpf = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn igb_sw_irq_prep(q_vector: *mut igb_q_vector) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn igb_xsk_wakeup(dev: *mut net_device, qid: u32, flags: u32) -> c_int {
