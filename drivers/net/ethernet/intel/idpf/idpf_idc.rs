...
//! Source: drivers/net/ethernet/intel/idpf/idpf_idc.c
#![no_std]
...
pub const IDPF_IDC_MAX_ADEV_NAME_LEN: c_int = 15;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn idpf_idc_init(adapter: *mut idpf_adapter) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn idpf_vport_adev_release(dev: *mut device) {
...
//
    ...
    vdev_info.adev = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn idpf_idc_init_aux_vport_dev(vport: *mut idpf_vport) -> c_int {
    ...
    if (err) {
    vport.vdev_info = core::ptr::null_mut();
    kfree(vdev_info);
    return err;
    }
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn idpf_core_adev_release(dev: *mut device) {
...
//
#[no_mangle]
unsafe extern "C" fn idpf_plug_core_aux_dev(cdev_info: *mut iidc_rdma_core_dev_info) -> c_int {
    ...
    err_aux_dev_add:
    ida_free(&idpf_idc_ida, adev.id);
    cdev_info.adev = core::ptr::null_mut();
    auxiliary_device_uninit(adev);
    return ret;
    err_aux_dev_init:
    ida_free(&idpf_idc_ida, adev.id);
...
//
#[no_mangle]
unsafe extern "C" fn idpf_unplug_aux_dev(adev: *mut auxiliary_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn idpf_idc_issue_reset_event(cdev_info: *mut iidc_rdma_core_dev_info) {
    void idpf_idc_issue_reset_event(struct iidc_rdma_core_dev_info *cdev_info)
    {
    let mut event_type: enum iidc_rdma_event_type = IIDC_RDMA_EVENT_WARN_RESET;
...
//
#[no_mangle]
unsafe extern "C" fn idpf_idc_vport_dev_up(adapter: *mut idpf_adapter) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn idpf_idc_vport_dev_down(adapter: *mut idpf_adapter) {
    ...
    vport.vdev_info.adev = core::ptr::null_mut();
    }
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn idpf_idc_vport_dev_ctrl(cdev_info: *mut iidc_rdma_core_dev_info, up: bool) -> c_int {
...
//
    ...
    privd.mapped_mem_regions = core::ptr::null_mut();
    err_plug_aux_dev:
    kfree(privd);
    err_privd_alloc:
    kfree(cdev_info);
    adapter.cdev_info = core::ptr::null_mut();
    return err;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn idpf_idc_deinit_core_aux_device(adapter: *mut idpf_adapter) {
...
//
#[no_mangle]
pub unsafe extern "C" fn idpf_idc_deinit_vport_aux_device(vdev_info: *mut iidc_rdma_vport_dev_info) {
