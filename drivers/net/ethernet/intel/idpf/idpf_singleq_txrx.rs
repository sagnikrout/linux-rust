...
//! Source: drivers/net/ethernet/intel/idpf/idpf_singleq_txrx.c
#![no_std]
...
//
    ...
    {
    let mut ss: libeth_sq_napi_stats = { };
...
//
    ...
    {
    let mut offsets: u32 = offloads.hdr_offsets;
    struct idpf_tx_buf *tx_buf = first;
    struct idpf_base_tx_desc *tx_desc;
    struct sk_buff *skb = first.skb;
    let mut td_cmd: u64 = offloads.td_cmd;
    unsigned int data_len, size;
    let mut i: u16 = tx_q.next_to_use;
    struct netdev_queue *nq;
    skb_frag_t *frag;
    dma_addr_t dma;
    let mut td_tag: u64 = 0;
...
// write each descriptor with CRC bit
    ...
    let mut max_data: c_uint = IDPF_TX_MAX_DESC_DATA_ALIGNED;
...
//
    ...
    let mut ntu: c_int = txq.next_to_use;
...
//
    ...
    let mut qw1: u64 = (u64)IDPF_TX_DESC_DTYPE_CTX;
    if (offload.tso_segs) {
...
//
    ...
    {
    let mut offload: idpf_tx_offload_params = { };
    struct idpf_tx_buf *first;
...
#[no_mangle]
pub unsafe extern "C" fn if(htons(ETH_P_IPV6): protocol ==) -> else {
...
//
    ...
    let mut ss: libeth_sq_napi_stats = { };
    struct idpf_base_tx_desc *tx_desc;
    let mut budget: u32 = tx_q.clean_budget;
    let mut ntc: i16 = tx_q.next_to_clean;
    struct libeth_cq_pp cp = {
    .dev	= tx_q.dev,
    .ss	= &ss,
    .napi	= napi_budget,
...
//
    ...
    int *cleaned)
    {
    let mut num_txq: u16 = q_vec.num_txq;
...
//
#[no_mangle]
unsafe extern "C" fn idpf_rx_singleq_is_non_eop(rx_desc: *const union virtchnl2_rx_desc) -> bool {
...
//
    ...
    {
    let mut csum_bits: libeth_rx_csum = { };
...
//
    ...
    let mut hash: u32 = le32_to_cpu(rx_desc.base_wb.qword0.hi_dword.rss);
...
//
    ...
    let mut hash: u32 = le32_to_cpu(rx_desc.flex_nic_wb.rss_hash);
    libeth_rx_pt_set_hash(skb, hash, decoded);
    }
    }
//
...
//
    ...
    u16 ptype)
    {
    let mut decoded: libeth_rx_pt = rx_q.rx_ptype_lkup[ptype];
    struct libeth_rx_csum csum_bits;
...
//
#[no_mangle]
unsafe extern "C" fn idpf_rx_buf_hw_update(rxq: *mut idpf_rx_queue, val: u32) {
...
//
    ...
    let mut nta: u16 = rx_q.next_to_alloc;
    if (!cleaned_count)
    return false;
...
//
    ...
    {
    libeth_xdp_run_pass(xdp, core::ptr::null_mut(), napi, rs, desc, core::ptr::null_mut(),
...
//
#[no_mangle]
unsafe extern "C" fn idpf_rx_singleq_clean(rx_q: *mut idpf_rx_queue, budget: c_int) -> c_int {
    static int idpf_rx_singleq_clean(struct idpf_rx_queue *rx_q, int budget)
    {
    let mut rs: libeth_rq_napi_stats = { };
    let mut ntc: u16 = rx_q.next_to_clean;
    LIBETH_XDP_ONSTACK_BUFF(xdp);
    let mut cleaned_count: u16 = 0;
    libeth_xdp_init_buff(xdp, &rx_q.xdp, &rx_q.xdp_rxq);
// Process Rx packets bounded by budget
    while (likely(rs.packets < budget)) {
    let mut fields: libeth_rqe_info = { };
...
//
    ...
    int *cleaned)
    {
    let mut num_rxq: u16 = q_vec.num_rxq;
    let mut clean_complete: bool = true;
    int budget_per_q, i;
...
//
#[no_mangle]
pub unsafe extern "C" fn idpf_vport_singleq_napi_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
