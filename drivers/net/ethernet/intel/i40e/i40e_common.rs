...
//! Source: drivers/net/ethernet/intel/i40e/i40e_common.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_set_mac_type(hw: *mut i40e_hw) -> c_int {
...
//
    ...
    let mut effective_mask: u32 = hw.debug_mask & mask;
    char prefix[27];
    u16 len;
    u8 *buf = (u8 *)buffer;
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_check_asq_alive(hw: *mut i40e_hw) -> bool {
...
//
    ...
    status = i40e_asq_send_command(hw, &desc, core::ptr::null_mut(), 0, core::ptr::null_mut());
    return status;
    }
//
...
// Indirect command
    ...
    status = i40e_asq_send_command(hw, &desc, lut, lut_size, core::ptr::null_mut());
    return status;
    }
//
// i40e_aq_get_rss_lut
...
//
    ...
    bool set)
    {
    let mut key_size: u16 = sizeof(struct i40e_aqc_get_set_rss_key_data);
...
// Indirect command
    ...
    cmd_resp.vsi_id = cpu_to_le16(vsi_id);
    status = i40e_asq_send_command(hw, &desc, key, key_size, core::ptr::null_mut());
    return status;
    }
//
// i40e_aq_get_rss_key
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_init_shared_code(hw: *mut i40e_hw) -> c_int {
...
//
    ...
    ((u32)mac_addr[4] << 8) |
    mac_addr[5]);
    status = i40e_asq_send_command(hw, &desc, core::ptr::null_mut(), 0, cmd_details);
    return status;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_get_mac_addr(hw: *mut i40e_hw, mac_addr: *mut u8) -> c_int {
    int i40e_get_mac_addr(struct i40e_hw *hw, u8 *mac_addr)
    {
    struct i40e_aqc_mac_address_read_data addrs;
    let mut flags: u16 = 0;
    int status;
    status = i40e_aq_mac_address_read(hw, &flags, &addrs, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_get_port_mac_addr(hw: *mut i40e_hw, mac_addr: *mut u8) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_pre_tx_queue_cfg(hw: *mut i40e_hw, queue: u32, enable: bool) {
    void i40e_pre_tx_queue_cfg(struct i40e_hw *hw, u32 queue, bool enable)
    {
    let mut abs_queue_idx: u32 = hw.func_caps.base_queue + queue;
    let mut reg_block: u32 = 0;
    u32 reg_val;
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_get_pba_string(hw: *mut i40e_hw) {
    void i40e_get_pba_string(struct i40e_hw *hw)
    {
pub const I40E_NVM_PBA_FLAGS_BLK_PRESENT: c_uint = 0xFAFA;
    let mut pba_word: u16 = 0;
    let mut pba_size: u16 = 0;
    let mut pba_ptr: u16 = 0;
    int status;
    char *ptr;
    u16 i;
...
//
#[no_mangle]
unsafe extern "C" fn i40e_get_media_type(hw: *mut i40e_hw) -> enum i40e_media_type {
...
pub const I40E_PF_RESET_WAIT_COUNT_A0: c_int = 200;
pub const I40E_PF_RESET_WAIT_COUNT: c_int = 200;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_pf_reset(hw: *mut i40e_hw) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_clear_hw(hw: *mut i40e_hw) {
...
// warn the HW of the coming Tx disables
    ...
    let mut abs_queue_idx: u32 = base_queue + i;
    let mut reg_block: u32 = 0;
    if (abs_queue_idx >= 128) {
    reg_block = abs_queue_idx / 128;
    abs_queue_idx %= 128;
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_clear_pxe_mode(hw: *mut i40e_hw) {
    void i40e_clear_pxe_mode(struct i40e_hw *hw)
    {
    u32 reg;
    if (i40e_check_asq_alive(hw))
    i40e_aq_clear_pxe_mode(hw, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn i40e_led_is_mine(hw: *mut i40e_hw, idx: c_int) -> u32 {
    static u32 i40e_led_is_mine(struct i40e_hw *hw, int idx)
    {
    let mut gpio_val: u32 = 0;
    u32 port;
...
pub const I40E_LED0: c_int = 22;
pub const I40E_PIN_FUNC_SDP: c_uint = 0x0;
pub const I40E_PIN_FUNC_LED: c_uint = 0x1;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_led_get(hw: *mut i40e_hw) -> u32 {
...
//
    ...
    let mut gpio_val: u32 = i40e_led_is_mine(hw, i);
    if (!gpio_val)
    continue;
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_led_set(hw: *mut i40e_hw, mode: u32, blink: bool) {
...
//
    ...
    struct i40e_asq_cmd_details *cmd_details)
    {
    let mut abilities_size: u16 = sizeof(struct i40e_aq_get_phy_abilities_resp);
    let mut max_delay: u16 = I40E_MAX_PHY_TIMEOUT, total_delay = 0;
    struct libie_aq_desc desc;
    int status;
    if (!abilities)
    return -EINVAL;
...
// also covers LIBIE_AQ_RC_OK
    ...
    status = i40e_aq_get_link_info(hw, true, core::ptr::null_mut(), core::ptr::null_mut());
    } else {
...
// cmd = *config;
    ...
    {
    struct i40e_aq_set_phy_config config;
    let mut fc_mode: enum i40e_fc_mode = hw.fc.requested_mode;
    let mut pause_mask: u8 = 0x0;
    switch (fc_mode) {
    case I40E_FC_FULL:
...
// Copy over all the old settings
    ...
    return i40e_aq_set_phy_config(hw, &config, core::ptr::null_mut());
    }
//
// i40e_set_fc
// @hw: pointer to the hw struct
...
pub const I40E_AQ_SET_MAC_CONFIG_FC_DEFAULT_THRESHOLD: c_uint = 0x7FFF;
    cmd.fc_refresh_threshold =
    cpu_to_le16(I40E_AQ_SET_MAC_CONFIG_FC_DEFAULT_THRESHOLD);
    return i40e_asq_send_command(hw, &desc, core::ptr::null_mut(), 0, cmd_details);
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tx_pause) -> else {
    else if (tx_pause)
    hw.fc.current_mode = I40E_FC_TX_PAUSE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: rx_pause) -> else {
    else if (rx_pause)
...
//
    ...
    status = i40e_asq_send_command_atomic(hw, &desc, core::ptr::null_mut(), 0,
    cmd_details, true);
    return status;
    }
//
...
//
    ...
    sizeof(vsi_ctx.info), core::ptr::null_mut());
    if (status)
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_get_link_status(hw: *mut i40e_hw, link_up: *mut bool) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_update_link_info(hw: *mut i40e_hw) -> noinline_for_stack int {
...
// vebs_free = le16_to_cpu(cmd_resp->vebs_free);
    if (floating) {
    let mut flags: u16 = le16_to_cpu(cmd_resp.veb_flags);
...
//
    ...
    p = &hw.dev_caps;
#[no_mangle]
pub unsafe extern "C" fn if(i40e_aqc_opc_list_func_capabilities: list_type_opc ==) -> else {
...
// count the enabled ports (aka the "not disabled" ports)
    hw.num_ports = 0;
    for (i = 0; i < 4; i++) {
    let mut port_cfg_reg: u32 = I40E_PRTGEN_CNF + (4 * i);
    let mut port_cfg: u64 = 0;
// use AQ read to get the physical register offset instead
// of the port relative offset
//
    i40e_aq_debug_read_register(hw, port_cfg_reg, &port_cfg, core::ptr::null_mut());
...
//
    ...
    &ocp_cfg_word0, true, core::ptr::null_mut());
    if (!status &&
...
#[no_mangle]
pub unsafe extern "C" fn if(I40E_NVM_PRESERVATION_FLAGS_ALL: preservation_flags ==) -> else {
...
// Indirect Command
    ...
    status = i40e_asq_send_command(hw, &desc, buff, buff_size, cmd_details);
    if (!status) {
    if (local_len != core::ptr::null_mut())
// local_len = le16_to_cpu(resp->local_len);
    if (remote_len != core::ptr::null_mut())
...
//
    ...
    struct libie_aq_desc desc;
    int status;
    let mut cmd_param_flag: bool = false;
    switch (opcode) {
...
//
    ...
    u16 seid)
    {
pub const I40E_FLOW_CONTROL_ETHTYPE: c_uint = 0x8808;
    u16 flag = I40E_AQC_ADD_CONTROL_PACKET_FLAGS_IGNORE_MAC |
    I40E_AQC_ADD_CONTROL_PACKET_FLAGS_DROP |
    I40E_AQC_ADD_CONTROL_PACKET_FLAGS_TX;
    let mut ethtype: u16 = I40E_FLOW_CONTROL_ETHTYPE;
    int status;
    status = i40e_aq_add_rem_control_packet_filter(hw, core::ptr::null_mut(), ethtype, flag,
    seid, 0, true, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (status)
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_set_pci_config_data(hw: *mut i40e_hw, link_status: u16) {
...
//
    ...
    struct i40e_asq_cmd_details *cmd_details)
    {
    let mut bwd_size: u16 = sizeof(*bw_data);
    struct libie_aq_desc desc;
    int status;
    i40e_fill_default_direct_cmd_desc(&desc,
...
//
    ...
    {
    let mut port_num: u8 = (u8)hw.func_caps.mdio_port_num;
...
//
    ...
    {
    let mut port_num: u8 = hw.func_caps.mdio_port_num;
    let mut status: c_int = -EIO;
    let mut command: u32 = 0;
    let mut retry: u16 = 1000;
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_get_phy_address(hw: *mut i40e_hw, dev_num: u8) -> u8 {
    u8 i40e_get_phy_address(struct i40e_hw *hw, u8 dev_num)
    {
    let mut port_num: u8 = hw.func_caps.mdio_port_num;
    let mut reg_val: u32 = rd32(hw, I40E_GLGEN_MDIO_I2C_SEL(port_num));
...
//
    ...
    {
    let mut led_ctl: u32 = 0;
    let mut led_reg: u32 = 0;
    let mut status: c_int = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_read_rx_ctl(hw: *mut i40e_hw, reg_addr: u32) -> u32 {
    u32 i40e_read_rx_ctl(struct i40e_hw *hw, u32 reg_addr)
    {
    let mut use_register: bool = false;
    let mut status: c_int = 0;
    let mut retry: c_int = 5;
    let mut val: u32 = 0;
    if (i40e_is_aq_api_ver_lt(hw, 1, 5) || hw.mac.type == I40E_MAC_X722)
    use_register = true;
    if (!use_register) {
    do_retry:
    status = i40e_aq_rx_ctl_read_register(hw, reg_addr, &val, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_write_rx_ctl(hw: *mut i40e_hw, reg_addr: u32, reg_val: u32) {
...
//
    ...
    status = i40e_asq_send_command(hw, &desc, msg, msglen, core::ptr::null_mut());
    if (status) {
...
//
    ...
    {
    struct i40e_profile_section_header *sec = core::ptr::null_mut();
...
//
    ...
    cmd.seid = cpu_to_le16(seid);
    status = i40e_asq_send_command(hw, &desc, filters, buff_len, core::ptr::null_mut());
    return status;
    }
//
