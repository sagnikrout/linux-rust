...
//! Source: drivers/net/ethernet/intel/i40e/i40e_nvm.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_init_nvm(hw: *mut i40e_hw) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_release_nvm(hw: *mut i40e_hw) {
    ...
    if (hw.nvm.blank_nvm_mode)
    return;
    ret_code = i40e_aq_release_resource(hw, I40E_NVM_RESOURCE_ID, 0, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn i40e_poll_sr_srctl_done_bit(hw: *mut i40e_hw) -> c_int {
    static int i40e_poll_sr_srctl_done_bit(struct i40e_hw *hw)
    {
    let mut ret_code: c_int = -EIO;
    u32 srctl, wait_cnt;
...
#[no_mangle]
pub unsafe extern "C" fn if(I40E_SR_SECTOR_SIZE_IN_WORDS: words >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(I40E_SR_SECTOR_SIZE_IN_WORDS: ((offset + (words - 1)) /) -> else {
...
//
    ...
    u16 *data_ptr)
    {
    let mut specific_ptr: u16 = 0;
    let mut ptr_value: u16 = 0;
...
pub const I40E_NVM_INVALID_PTR_VAL: c_uint = 0x7FFF;
pub const I40E_NVM_INVALID_VAL: c_uint = 0xFFFF;
...
//
    ...
    let mut pcie_alt_module: u16 = 0;
    let mut checksum_local: u16 = 0;
    let mut vpd_module: u16 = 0;
    int ret_code;
    u16 *data;
    let mut i: u16 = 0;
...
// Read SR page
    ...
    let mut words: u16 = I40E_SR_SECTOR_SIZE_IN_WORDS;
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_update_nvm_checksum(hw: *mut i40e_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn i40e_nvmupd_get_module(val: u32) -> u8 {
    static u8 i40e_nvmupd_get_module(u32 val)
    {
    return (u8)(val & I40E_NVM_MOD_PNT_MASK);
    }
#[no_mangle]
pub unsafe extern "C" fn i40e_nvmupd_get_transaction(val: u32) -> u8 {
    static inline u8 i40e_nvmupd_get_transaction(u32 val)
    {
    return FIELD_GET(I40E_NVM_TRANS_MASK, val);
    }
#[no_mangle]
pub unsafe extern "C" fn i40e_nvmupd_get_preservation_flags(val: u32) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: module ==) -> else {
    else if (module == 0)
...
// check copylength range
    ...
    let mut new_len: c_int = aq_total_len - cmd.offset;
    ...
    let mut len: u32 = aq_desc_len - cmd.offset;
    ...
    if (remainder > 0) {
    let mut start_byte: c_int = buff - (u8 *)hw.nvm_buff.va;
...
//
    ...
    {
    enum i40e_nvmupd_cmd upd_cmd;
    let mut retry_attempt: bool = false;
    let mut status: c_int = 0;
    upd_cmd = i40e_nvmupd_validate_command(hw, cmd, perrno);
    retry:
    switch (upd_cmd) {
...
//
    ...
    !retry_attempt) {
    let mut old_asq_status: u32 = hw.aq.asq_last_status;
    let mut old_status: c_int = status;
    u32 gtime;
    gtime = rd32(hw, I40E_GLVFGEN_TIMER);
...
//
#[no_mangle]
pub unsafe extern "C" fn i40e_nvmupd_clear_wait_state(hw: *mut i40e_hw) {
...
//
    ...
    {
    let mut aq_desc_len: u32 = sizeof(struct libie_aq_desc);
