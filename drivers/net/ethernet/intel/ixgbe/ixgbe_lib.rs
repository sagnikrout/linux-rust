...
//! Source: drivers/net/ethernet/intel/ixgbe/ixgbe_lib.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_cache_ring_dcb_sriov(adapter: *mut ixgbe_adapter) -> bool {
...
// If we are greater than indices move to next pool
    ...
    adapter.rx_ring[i].netdev = pool ? core::ptr::null_mut() : adapter.netdev;
    }
    reg_idx = vmdq.offset * __ALIGN_MASK(1, ~vmdq.mask);
...
// The FCoE rings exist separately, we need to move their reg_idx
    if (fcoe.indices) {
    let mut queues_per_pool: u16 = __ALIGN_MASK(1, ~vmdq.mask);
    let mut fcoe_tc: u8 = ixgbe_fcoe_get_tc(adapter);
...
// ixgbe_get_first_reg_idx - Return first register index associated with ring
    ...
    let mut num_tcs: u8 = adapter.hw_tcs;
// tx = 0;
// rx = 0;
    switch (hw.mac.type) {
...
// tx = tc << 5;		/*   0,  32,  64
#[no_mangle]
pub unsafe extern "C" fn if(5: tc <) -> else {
    else if (tc < 5)
// tx = (tc + 2) << 4;	/*  80,  96
    else
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_cache_ring_dcb(adapter: *mut ixgbe_adapter) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_cache_ring_sriov(adapter: *mut ixgbe_adapter) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_cache_ring_rss(adapter: *mut ixgbe_adapter) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_cache_ring_register(adapter: *mut ixgbe_adapter) {
...
#[no_mangle]
unsafe extern "C" fn ixgbe_xdp_queues(adapter: *mut ixgbe_adapter) -> c_int {
...
pub const IXGBE_RSS_64Q_MASK: c_uint = 0x3F;
pub const IXGBE_RSS_16Q_MASK: c_uint = 0xF;
pub const IXGBE_RSS_8Q_MASK: c_uint = 0x7;
pub const IXGBE_RSS_4Q_MASK: c_uint = 0x3;
pub const IXGBE_RSS_2Q_MASK: c_uint = 0x1;
pub const IXGBE_RSS_DISABLED_MASK: c_uint = 0x0;
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_set_dcb_sriov_queues(adapter: *mut ixgbe_adapter) -> bool {
    static bool ixgbe_set_dcb_sriov_queues(struct ixgbe_adapter *adapter)
    {
    int i;
    let mut vmdq_i: u16 = adapter.ring_feature[RING_F_VMDQ].limit;
    let mut vmdq_m: u16 = 0;

    let mut fcoe_i: u16 = 0;
...
#[no_mangle]
unsafe extern "C" fn ixgbe_set_dcb_queues(adapter: *mut ixgbe_adapter) -> bool {
...
//
    if (adapter.flags & IXGBE_FLAG_FCOE_ENABLED) {
    let mut tc: u8 = ixgbe_fcoe_get_tc(adapter);
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_set_sriov_queues(adapter: *mut ixgbe_adapter) -> bool {
    static bool ixgbe_set_sriov_queues(struct ixgbe_adapter *adapter)
    {
    let mut vmdq_i: u16 = adapter.ring_feature[RING_F_VMDQ].limit;
    let mut vmdq_m: u16 = 0;
    let mut rss_i: u16 = adapter.ring_feature[RING_F_RSS].limit;
    let mut rss_m: u16 = IXGBE_RSS_DISABLED_MASK;
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_set_rss_queues(adapter: *mut ixgbe_adapter) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_set_num_queues(adapter: *mut ixgbe_adapter) {
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_acquire_msix_vectors(adapter: *mut ixgbe_adapter) -> c_int {
...
//
    ...
    {
    let mut node: c_int = dev_to_node(&adapter.pdev.dev);
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_free_q_vector(adapter: *mut ixgbe_adapter, v_idx: c_int) {
    ...
    if (ring_is_xdp(ring))
    WRITE_ONCE(adapter.xdp_ring[ring.queue_index], core::ptr::null_mut());
    else
    WRITE_ONCE(adapter.tx_ring[ring.queue_index], core::ptr::null_mut());
    }
    ixgbe_for_each_ring(ring, q_vector.rx)
    WRITE_ONCE(adapter.rx_ring[ring.queue_index], core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_alloc_q_vectors(adapter: *mut ixgbe_adapter) -> c_int {
...
// update counts and index
    ...
    let mut xqpv: c_int = DIV_ROUND_UP(xdp_remaining, q_vectors - v_idx);
    err = ixgbe_alloc_q_vector(adapter, q_vectors, v_idx,
    tqpv, txr_idx,
    xqpv, xdp_idx,
    rqpv, rxr_idx);
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_free_q_vectors(adapter: *mut ixgbe_adapter) {
...
#[no_mangle]
unsafe extern "C" fn ixgbe_reset_interrupt_capability(adapter: *mut ixgbe_adapter) {
...
//
#[no_mangle]
unsafe extern "C" fn ixgbe_set_interrupt_capability(adapter: *mut ixgbe_adapter) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ixgbe_init_interrupt_scheme(adapter: *mut ixgbe_adapter) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ixgbe_clear_interrupt_scheme(adapter: *mut ixgbe_adapter) {
