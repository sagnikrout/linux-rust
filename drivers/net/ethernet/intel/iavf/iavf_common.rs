...
//! Source: drivers/net/ethernet/intel/iavf/iavf_common.c
#![no_std]
...
//
    ...
    let mut len: u16 = le16_to_cpu(aq_desc.datalen);
...
//
#[no_mangle]
pub unsafe extern "C" fn iavf_check_asq_alive(hw: *mut iavf_hw) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn iavf_aq_queue_shutdown(hw: *mut iavf_hw, unloading: bool) -> enum iavf_status {
    ...
    status = iavf_asq_send_command(hw, &desc, core::ptr::null_mut(), 0, core::ptr::null_mut());
    return status;
    }
//
...
// Indirect command
    ...
    status = iavf_asq_send_command(hw, &desc, lut, lut_size, core::ptr::null_mut());
    return status;
    }
//
// iavf_aq_set_rss_lut
...
//
    ...
    bool set)
    {
    let mut key_size: u16 = sizeof(struct iavf_aqc_get_set_rss_key_data);
...
// Indirect command
    ...
    cmd_resp.vsi_id = cpu_to_le16(vsi_id);
    status = iavf_asq_send_command(hw, &desc, key, key_size, core::ptr::null_mut());
    return status;
    }
//
// iavf_aq_set_rss_key
