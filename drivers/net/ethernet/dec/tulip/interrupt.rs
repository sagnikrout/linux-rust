...
//! Source: drivers/net/ethernet/dec/tulip/interrupt.c
#![no_std]
...
// CRS11 21143 hardware Mitigation Control Interrupt
    ...
    TX intr. mitigation.
#[no_mangle]
pub unsafe extern "C" fn Size(control: timer) -> 31    Cycle {
...
#[no_mangle]
pub unsafe extern "C" fn Mode(_arg: CM) -> 16       Continues {
...
#[no_mangle]
pub unsafe extern "C" fn tulip_refill_rx(dev: *mut net_device) -> c_int {
...
// Refill the Rx ring buffers.
    ...
    if (tp.rx_buffers[entry].skb == core::ptr::null_mut()) {
    struct sk_buff *skb;
    dma_addr_t mapping;
    ...
    dev_kfree_skb(skb);
    tp.rx_buffers[entry].skb = core::ptr::null_mut();
    break;
    }
...
#[no_mangle]
pub unsafe extern "C" fn oom_timer(t: *mut timer_list) {
...
#[no_mangle]
pub unsafe extern "C" fn tulip_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    int tulip_poll(struct napi_struct *napi, int budget)
    {
    struct tulip_private *tp = container_of(napi, struct tulip_private, napi);
    struct net_device *dev = tp.dev;
    let mut entry: c_int = tp.cur_rx % RX_RING_SIZE;
...
// If we own the next entry, it is a new packet. Send it up.
    ...
    let mut status: i32 = le32_to_cpu(tp.rx_ring[entry].status);
    short pkt_len;
...
// If RX ring is not full we are out of memory.
    if (tp.rx_buffers[tp.dirty_rx % RX_RING_SIZE].skb == core::ptr::null_mut())
    goto oom;
...
//
    ...
    tp.rx_buffers[tp.dirty_rx % RX_RING_SIZE].skb == core::ptr::null_mut())
    tulip_refill_rx(dev);
    if (tp.rx_buffers[tp.dirty_rx % RX_RING_SIZE].skb == core::ptr::null_mut())
    goto oom;
    return work_done;
...
#[no_mangle]
unsafe extern "C" fn tulip_rx(dev: *mut net_device) -> c_int {
    static int tulip_rx(struct net_device *dev)
    {
    struct tulip_private *tp = netdev_priv(dev);
    let mut entry: c_int = tp.cur_rx % RX_RING_SIZE;
    let mut rx_work_limit: c_int = tp.dirty_rx + RX_RING_SIZE - tp.cur_rx;
    let mut received: c_int = 0;
    if (tulip_debug > 4)
    netdev_dbg(dev, "In tulip_rx(), entry %d %08x\n",
    entry, tp.rx_ring[entry].status);
...
//
#[no_mangle]
pub unsafe extern "C" fn 1518(header: 1514 + vlan) -> Maximum pkt_len is {
    Maximum pkt_len is 1518 (1514 + vlan header)
    Anything higher than this is always invalid
    regardless of RxLengthOver2047 bits
//
...
#[no_mangle]
pub unsafe extern "C" fn phy_interrupt(dev: *mut net_device) -> c_uint {
    static inline unsigned int phy_interrupt (struct net_device *dev)
    {

    struct tulip_private *tp = netdev_priv(dev);
    let mut csr12: c_int = ioread32(tp.base_addr + CSR12) & 0xff;
...
// The interrupt handler does all of the Rx thread work and cleans up
    after the Tx thread. */
#[no_mangle]
pub unsafe extern "C" fn tulip_interrupt(irq: c_int, dev_instance: *mut c_void) -> irqreturn_t {
    ...
    let mut tx: c_int = 0;
    let mut oi: c_int = 0;
    let mut maxrx: c_int = RX_RING_SIZE;
    let mut maxtx: c_int = TX_RING_SIZE;
    let mut maxoi: c_int = TX_RING_SIZE;

    let mut rxd: c_int = 0;

    int entry;

    let mut work_count: c_uint = tulip_max_interrupt_work;
