...
//! Source: drivers/net/ethernet/dec/tulip/tulip_core.c
#![no_std]
...
// Maximum events (Rx packets, etc.) to handle at each interrupt.
    let mut max_interrupt_work: static unsigned int = 25;
...
//
    ...
    let mut csr0: static int = 0x01A00000 | 0xE000;

    let mut csr0: static int = 0x01A00000 | 0x8000;

// The UltraSparc PCI controllers will disconnect at every 64-byte
// crossing anyways so it makes no sense to tell Tulip to burst
// any more than that.
//
    let mut csr0: static int = 0x01A00000 | 0x9000;

    let mut csr0: static int = 0x01A00000 | 0x4800;

    let mut csr0: static int = 0x00200000 | 0x4000;
...
// Time in jiffies before concluding the transmitter is hung.
    ...
    let mut tulip_debug: c_int = TULIP_DEBUG;

    let mut tulip_debug: c_int = 1;

#[no_mangle]
unsafe extern "C" fn tulip_timer(t: *mut timer_list) {
...
// A full-duplex map for media types.
    ...
    newtmp |= CFDD_Sleep;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: snooze) -> else {
    else if (snooze)
    newtmp |= CFDD_Snooze;
    if (tmp != newtmp)
...
#[no_mangle]
unsafe extern "C" fn tulip_up(dev: *mut net_device) {
    static void tulip_up(struct net_device *dev)
    {
    struct tulip_private *tp = netdev_priv(dev);
    void __iomem *ioaddr = tp.base_addr;
    let mut next_tick: c_int = 3*HZ;
    u32 reg;
    int i;
...
// Deassert reset.
    ...
    let mut addr_low: u32 = get_unaligned_le32(dev.dev_addr);
    let mut addr_high: u32 = get_unaligned_le16(dev.dev_addr + 4);
...
// setup_frm++ = eaddrs[2]; *setup_frm++ = eaddrs[2];
    ...
    DMA_TO_DEVICE);
    tp.tx_buffers[tp.cur_tx].skb = core::ptr::null_mut();
...
// Allow selecting a default media.
    i = 0;
    if (tp.mtable == core::ptr::null_mut())
    goto media_picked;
    if (dev.if_port) {
    ...
    let mut looking_for: c_int = tp.mtable.defaultmedia & MEDIA_MASK;
    for (i = 0; i < tp.mtable.leafcount; i++)
    if (tp.mtable.mleaf[i].media == looking_for) {
    dev_info(&dev.dev,
...
#[no_mangle]
unsafe extern "C" fn tulip_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void tulip_tx_timeout(struct net_device *dev, unsigned int txqueue)
    {
    struct tulip_private *tp = netdev_priv(dev);
    void __iomem *ioaddr = tp.base_addr;
...
#[no_mangle]
unsafe extern "C" fn tulip_init_ring(dev: *mut net_device) {
    static void tulip_init_ring(struct net_device *dev)
    {
    struct tulip_private *tp = netdev_priv(dev);
    int i;
    ...
    tp.rx_buffers[i].skb = core::ptr::null_mut();
...
// The Tx buffer descriptor is filled in as needed, but we
    ...
    for (i = 0; i < TX_RING_SIZE; i++) {
    tp.tx_buffers[i].skb = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn tulip_clean_tx_ring(tp: *mut tulip_private) {
    ...
    let mut status: c_int = le32_to_cpu(tp.tx_ring[entry].status);
    if (status < 0) {
    tp.dev.stats.tx_errors++;	/* It wasn't Txed */
    tp.tx_ring[entry].status = 0;
    }
// Check for Tx filter setup frames.
    if (tp.tx_buffers[entry].skb == core::ptr::null_mut()) {
...
// Free the original skb.
    ...
    tp.tx_buffers[entry].skb = core::ptr::null_mut();
    tp.tx_buffers[entry].mapping = 0;
    }
    }
#[no_mangle]
unsafe extern "C" fn tulip_down(dev: *mut net_device) {
    static void tulip_down (struct net_device *dev)
    {
    struct tulip_private *tp = netdev_priv(dev);
    void __iomem *ioaddr = tp.base_addr;
...
#[no_mangle]
unsafe extern "C" fn tulip_free_ring(dev: *mut net_device) {
    static void tulip_free_ring (struct net_device *dev)
    {
    struct tulip_private *tp = netdev_priv(dev);
    int i;
// Free all the skbuffs in the Rx queue.
    ...
    let mut mapping: dma_addr_t = tp.rx_buffers[i].mapping;
    tp.rx_buffers[i].skb = core::ptr::null_mut();
    tp.rx_buffers[i].mapping = 0;
...
#[no_mangle]
unsafe extern "C" fn tulip_close(dev: *mut net_device) -> c_int {
    static int tulip_close (struct net_device *dev)
    {
    struct tulip_private *tp = netdev_priv(dev);
    void __iomem *ioaddr = tp.base_addr;
...
#[no_mangle]
unsafe extern "C" fn tulip_get_drvinfo(dev: *mut net_device, info: *mut ethtool_drvinfo) {
...
#[no_mangle]
unsafe extern "C" fn private_ioctl(dev: *mut net_device, rq: *mut ifreq, cmd: c_int) -> c_int {
    static int private_ioctl (struct net_device *dev, struct ifreq *rq, int cmd)
    {
    struct tulip_private *tp = netdev_priv(dev);
    void __iomem *ioaddr = tp.base_addr;
    struct mii_ioctl_data *data = if_mii(rq);
    let mut phy_idx: c_uint = 0;
    let mut phy: c_int = tp.phys[phy_idx] & 0x1f;
    let mut regnum: c_uint = data.reg_num;
    switch (cmd) {
    case SIOCGMIIPHY:		/* Get address of MII PHY in use. */
    if (tp.mii_cnt)
    data.phy_id = phy;
#[no_mangle]
pub unsafe extern "C" fn if(HAS_NWAY: tp->flags &) -> else {
    else if (tp.flags & HAS_NWAY)
    data.phy_id = 32;
#[no_mangle]
pub unsafe extern "C" fn if(COMET: tp->chip_id ==) -> else {
    ...
    let mut csr14: c_int = ioread32 (ioaddr + CSR14);
    switch (regnum) {
    case 0:
    if (((csr14<<5) & 0x1000) ||
...
// Arbitrary non-effective limit.
    ...
    ;				/* No change. */
#[no_mangle]
pub unsafe extern "C" fn if(IS_ASIX: tp->flags &) -> else {
    ...
    unsigned long flags;
    let mut tx_flags: u32 = 0x08000000 | 192;
...
#[no_mangle]
unsafe extern "C" fn tulip_mwi_config(pdev: *mut pci_dev, dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn tulip_uli_dm_quirk(pdev: *mut pci_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tulip_init_one(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
...
// See note below on the multiport cards.
    ...
    let mut eeprom_missing: c_uint = 0;
    u8 addr[ETH_ALEN] __aligned(2);
    let mut force_csr0: c_uint = 0;
    board_idx++;
//
...
// A serial EEPROM interface, we read now and sort it out later.
    let mut sa_offset: c_int = 0;
    let mut ee_addr_size: c_int = tulip_read_eeprom(dev, 0xff, 8) & 0x40000 ? 8 : 6;
    let mut ee_max_addr: c_int = ((1 << ee_addr_size) - 1) * sizeof(u16);
    if (ee_max_addr > sizeof(tp.eeprom))
    ee_max_addr = sizeof(tp.eeprom);
    for (i = 0; i < ee_max_addr ; i += sizeof(u16)) {
    let mut data: u16 = tulip_read_eeprom(dev, i/2, ee_addr_size);
    ee_data[i] = data & 0xff;
    ee_data[i + 1] = data >> 8;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(HAS_PNICNWAY: tp->flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn tulip_set_wolopts(pdev: *mut pci_dev, wolopts: u32) {
    static void tulip_set_wolopts (struct pci_dev *pdev, u32 wolopts)
    {
...
#[no_mangle]
unsafe extern "C" fn tulip_suspend(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn tulip_resume(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn tulip_remove_one(pdev: *mut pci_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn poll_tulip(dev: *mut net_device) {
    static void poll_tulip (struct net_device *dev)
    {
    struct tulip_private *tp = netdev_priv(dev);
    let mut irq: c_int = tp.pdev.irq;
...
#[no_mangle]
unsafe extern "C" fn tulip_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn tulip_cleanup() -> void __exit {
