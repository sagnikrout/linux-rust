...
//! Source: drivers/net/ethernet/dec/tulip/eeprom.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn tulip_build_fake_mediatable(tp: *mut tulip_private) {
...
#[no_mangle]
pub unsafe extern "C" fn tulip_parse_eeprom(dev: *mut net_device) {
...
// Do a fix-up based on the vendor half of the station address prefix.
    ...
    if (eeprom_fixups[i].name == core::ptr::null_mut()) { /* No fixup found. */
    ...
    let mut csr12dir: c_uchar = 0;
    int count, new_advertise = 0;
    struct mediatable *mtable;
    let mut media: u16 = get_u16(p);
    p += 2;
...
// Davicom's media number for 100BaseTX is strange
    ...
    if (p[2] & 0x40) {
    let mut base15: u32 = get_unaligned((u16*)&p[7]);
    mtable.csr15dir =
...
// EEPROM_Ctrl bits.
pub const EE_SHIFT_CLK: c_uint = 0x02	/* EEPROM shift clock. */;
pub const EE_CS: c_uint = 0x01	/* EEPROM chip select. */;
pub const EE_DATA_WRITE: c_uint = 0x04	/* Data from the Tulip to EEPROM. */;
pub const EE_WRITE_0: c_uint = 0x01;
pub const EE_WRITE_1: c_uint = 0x05;
pub const EE_DATA_READ: c_uint = 0x08	/* Data from the EEPROM chip. */;
...
#[no_mangle]
pub unsafe extern "C" fn tulip_read_eeprom(dev: *mut net_device, location: c_int, addr_len: c_int) -> c_int {
