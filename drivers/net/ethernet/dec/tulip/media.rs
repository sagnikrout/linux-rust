...
//! Source: drivers/net/ethernet/dec/tulip/media.c
#![no_std]
...
pub const MDIO_SHIFT_CLK: c_uint = 0x10000;
pub const MDIO_DATA_WRITE0: c_uint = 0x00000;
pub const MDIO_DATA_WRITE1: c_uint = 0x20000;
pub const MDIO_ENB: c_uint = 0x00000 /* Ignore the 0x02000 databook setting. */;
pub const MDIO_ENB_IN: c_uint = 0x40000;
pub const MDIO_DATA_READ: c_uint = 0x80000;
...
//
#[no_mangle]
pub unsafe extern "C" fn tulip_mdio_read(dev: *mut net_device, phy_id: c_int, location: c_int) -> c_int {
    int tulip_mdio_read(struct net_device *dev, int phy_id, int location)
    {
    struct tulip_private *tp = netdev_priv(dev);
    int i;
    let mut read_cmd: c_int = (0xf6 << 10) | ((phy_id & 0x1f) << 5) | location;
...
// Shift the read command bits out.
    for (i = 15; i >= 0; i--) {
    let mut dataval: c_int = (read_cmd & (1 << i)) ? MDIO_DATA_WRITE1 : 0;
...
#[no_mangle]
pub unsafe extern "C" fn tulip_mdio_write(dev: *mut net_device, phy_id: c_int, location: c_int, val: c_int) {
    void tulip_mdio_write(struct net_device *dev, int phy_id, int location, int val)
    {
    struct tulip_private *tp = netdev_priv(dev);
    int i;
    let mut cmd: c_int = (0x5002 << 16) | ((phy_id & 0x1f) << 23) | (location<<18) | (val & 0xffff);
    void __iomem *ioaddr = tp.base_addr;
    void __iomem *mdio_addr = ioaddr + CSR9;
    unsigned long flags;
    if (location & ~0x1f)
...
// Shift the command bits out.
    for (i = 31; i >= 0; i--) {
    let mut dataval: c_int = (cmd & (1 << i)) ? MDIO_DATA_WRITE1 : 0;
    iowrite32(MDIO_ENB | dataval, mdio_addr);
    mdio_delay();
    iowrite32(MDIO_ENB | dataval | MDIO_SHIFT_CLK, mdio_addr);
    mdio_delay();
...
#[no_mangle]
pub unsafe extern "C" fn tulip_select_media(dev: *mut net_device, startup: c_int) {
    ...
    case 1: case 3: {
    let mut phy_num: c_int = p[0];
    let mut init_length: c_int = p[1];
    ...
    let mut reset_length: c_int = p[2 + init_length*2];
...
// and IEEE 802.3 "22.2.4.1.1 Reset"
    ...
    let mut reset_length: c_int = p[2 + init_length];
...
//
#[no_mangle]
pub unsafe extern "C" fn tulip_check_duplex(dev: *mut net_device) -> c_int {
    ...
    let mut new_bmsr: c_int = tulip_mdio_read(dev, tp.phys[0], MII_BMSR);
...
#[no_mangle]
pub unsafe extern "C" fn tulip_find_mii(dev: *mut net_device, board_idx: c_int) {
...
// Find the connected MII xcvrs.
    ...
    let mut phy: c_int = phyn & 0x1f;
    let mut mii_status: c_int = tulip_mdio_read (dev, phy, MII_BMSR);
...
//
    ...
    let mut tmpadv: c_uint = tulip_mdio_read (dev, phy, MII_BMSR);
