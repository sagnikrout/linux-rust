...
//! Source: drivers/net/ethernet/dlink/sundance.c
#![no_std]
...
// Include files, designed to support most kernel versions 2.0.0 and later.
    ...
    module_param_array(media, charp, core::ptr::null_mut(), 0);
...
// Descriptor rings first for alignment.
    ...
    pub /: *mut *mut *mut net_device ndev; / backpointer,
// ethtool extra stats
    struct {
...
// These values are keep track of the transceiver/media in use.
    pub flowctrl:1: c_uint,
    pub /: *mut *mut unsigned int default_port:4; / Last dev->if_port value.,
    pub an_enable:1: c_uint,
    pub speed: c_uint,
    pub /: *mut *mut unsigned int wol_enabled:1; / Wake on LAN enabled,
    pub rx_tasklet: tasklet_struct,
    pub tx_tasklet: tasklet_struct,
    pub budget: c_int,
    pub cur_task: c_int,
// Multicast and receive mode.
    pub /: *mut *mut spinlock_t mcastlock; / SMP lock multicast updates.,
    pub mcast_filter: [u16; 4],
// MII transceiver section.
    pub mii_if: mii_if_info,
    pub mii_preamble_required: c_int,
    pub /: *mut *mut unsigned char phys[MII_CNT]; / MII device addresses, only first one used.,
    pub pci_dev: *mut pci_dev,
    pub base: *mut void __iomem,
    pub statlock: spinlock_t,
}

// The station address location in the EEPROM.
pub const EEPROM_SA_OFFSET: c_uint = 0x10;
...
#[no_mangle]
unsafe extern "C" fn sundance_reset(dev: *mut net_device, reset_cmd: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn sundance_poll_controller(dev: *mut net_device) {
...
//
    ...
    let mut phyx: c_int = phy & 0x1f;
    let mut mii_status: c_int = mdio_read(dev, phyx, MII_BMSR);
...
// Parse override configuration
    ...
    if (media[card_idx] != core::ptr::null_mut()) {
    np.an_enable = 0;
...
#[no_mangle]
unsafe extern "C" fn change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
    static int change_mtu(struct net_device *dev, int new_mtu)
    {
...
#[no_mangle]
unsafe extern "C" fn eeprom_read(ioaddr: *mut void __iomem, location: c_int) -> c_int {
    static int eeprom_read(void __iomem *ioaddr, int location)
    {
...
#[no_mangle]
unsafe extern "C" fn mdio_wait_link(dev: *mut net_device, wait: c_int) -> c_int {
    static int mdio_wait_link(struct net_device *dev, int wait)
    {
    int bmsr;
    int phy_id;
...
unsafe extern "C" fn check_duplex(dev: *mut net_device) {
    ...
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = np.base;
    let mut mii_lpa: c_int = mdio_read(dev, np.phys[0], MII_LPA);
    let mut negotiated: c_int = mii_lpa & np.mii_if.advertising;
    int duplex;
// Force media
...
#[no_mangle]
unsafe extern "C" fn tx_poll(t: *mut tasklet_struct) {
    static void tx_poll(struct tasklet_struct *t)
    {
    struct netdev_private *np = from_tasklet(np, t, tx_tasklet);
    let mut head: unsigned = np.cur_task % TX_RING_SIZE;
    struct netdev_desc *txdesc =
    &np.tx_ring[(np.cur_tx - 1) % TX_RING_SIZE];
// Chain the next pointer
    for (; np.cur_tx - np.cur_task > 0; np.cur_task++) {
    let mut entry: c_int = np.cur_task % TX_RING_SIZE;
...
unsafe extern "C" fn intr_handler(irq: c_int, dev_instance: *mut c_void) -> irqreturn_t {
    ...
    int i;
    do {
    let mut intr_status: c_int = ioread16(ioaddr + IntrStatus);
...
#[no_mangle]
unsafe extern "C" fn rx_poll(t: *mut tasklet_struct) {
    static void rx_poll(struct tasklet_struct *t)
    {
    ...
    let mut boguscnt: c_int = np.budget;
    void __iomem *ioaddr = np.base;
    let mut received: c_int = 0;
// If EOP is set on the next entry, it's a new packet. Send it up.
    while (1) {
    struct netdev_desc *desc = &(np.rx_ring[entry]);
    let mut frame_status: u32 = le32_to_cpu(desc.status);
    int pkt_len;
    if (--boguscnt < 0) {
    goto not_done;
    }
...
#[no_mangle]
unsafe extern "C" fn __set_mac_addr(dev: *mut net_device) -> c_int {
    static int __set_mac_addr(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    u16 addr16;
...
#[no_mangle]
unsafe extern "C" fn sundance_set_mac_addr(dev: *mut net_device, data: *mut c_void) -> c_int {
    static int sundance_set_mac_addr(struct net_device *dev, void *data)
    {
...
unsafe extern "C" fn netdev_ioctl(dev: *mut net_device, rq: *mut ifreq, cmd: c_int) -> c_int {
    ...
    return -EINVAL;
    spin_lock_irq(&np.lock);
    rc = generic_mii_ioctl(&np.mii_if, if_mii(rq), cmd, core::ptr::null_mut());
    spin_unlock_irq(&np.lock);
    return rc;
    }
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn sundance_remove1(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn sundance_suspend(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn sundance_resume(dev_d: *mut device) -> int __maybe_unused {
