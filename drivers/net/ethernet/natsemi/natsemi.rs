...
//! Source: drivers/net/ethernet/natsemi/natsemi.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn copyright(2001: c), (thockin@sun.com: 2002 Sun Microsystems) -> Portions {
...
// Maximum number of multicast addresses to filter (vs. rx-all-multicast).
    ...
    let mut multicast_filter_limit: static int = 100;
// Set the copy breakpoint for the copy-only-tiny-frames scheme.
    Setting to > 1518 effectively disables this feature. */
    static int rx_copybreak;
    let mut dspcfg_workaround: static int = 1;
...
pub const RX_RING_SIZE: c_int = 32;
// Operational parameters that usually are not changed.
// Time in jiffies before concluding the transmitter is hung.

pub const NATSEMI_HW_TIMEOUT: c_int = 400;

pub const NATSEMI_PG0_NREGS: c_int = 64;
pub const NATSEMI_RFDR_NREGS: c_int = 8;
pub const NATSEMI_PG1_NREGS: c_int = 4;
...
//
pub const PHYID_AM79C874: c_uint = 0x0022561b;
    enum {
...
pub const PMDCSR_VAL: c_uint = 0x189c	/* enable preferred adaptation circuitry */;
pub const TSTDAT_VAL: c_uint = 0x0;
pub const DSPCFG_VAL: c_uint = 0x5040;
pub const SDCFG_VAL: c_uint = 0x008c	/* set voltage thresholds for Signal Detect */;
pub const DSPCFG_LOCK: c_uint = 0x20	/* coefficient lock bit in DSPCFG */;
pub const DSPCFG_COEF: c_uint = 0x1000	/* see coefficient (in TSTDAT) bit in DSPCFG */;
pub const TSTDAT_FIXED: c_uint = 0xe8	/* magic number for bad coefficients */;
...
//
pub const DEFAULT_INTR: c_uint = 0x00f1cd65;
    enum TxConfig_bits {
    TxDrthMask		= 0x3f,
    TxFlthMask		= 0x3f00,
...
pub const TX_DRTH_VAL_INC: c_int = 2;
...
pub const PHY_ADDR_NONE: c_int = 32;
pub const PHY_ADDR_INTERNAL: c_int = 1;
// values we might find in the silicon revision register
pub const SRR_DP83815_C: c_uint = 0x0302;
pub const SRR_DP83815_D: c_uint = 0x0403;
pub const SRR_DP83816_A4: c_uint = 0x0504;
pub const SRR_DP83816_A5: c_uint = 0x0505;
...
pub struct netdev_desc {
    ...
    pub cmd_status: __le32,
    pub addr: __le32,
    pub software_use: __le32,
}
...
// Frequently used values: keep some adjacent for cache effect
    ...
    pub rx_head_desc: *mut netdev_desc,
// Producer/consumer ring indices
    pub dirty_rx: unsigned int cur_rx,,
...
// Don't pay attention to the reported link state.
    pub ignore_phy: c_int,
// external phy that is used: only valid if dev->if_port != PORT_TP
    pub mii: c_int,
    pub phy_addr_external: c_int,
    pub full_duplex: c_uint,
// Rx filter
    pub cur_rx_mode: u32,
    pub rx_filter: [u32; 16],
// FIFO and PCI burst thresholds
    pub rx_config: u32 tx_config,,
...
// expected DSPCFG value
    pub dspcfg: u16,
    pub dspcfg_workaround: c_int,
// parms saved in ethtool format
    pub /: *mut *mut u16 speed; / The forced speed, 10Mb, 100Mb, gigabit,
    pub /: *mut *mut u8 duplex; / Duplex, half or full,
    pub /: *mut *mut u8 autoneg; / Autonegotiation enabled,
...
#[no_mangle]
pub unsafe extern "C" fn DEVICE_ATTR(_arg: _name, _arg: 0644, _arg: natsemi_show_##_name, _arg: natsemi_set_##_name) -> static {
...
#[no_mangle]
pub unsafe extern "C" fn natsemi_irq_enable(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn natsemi_irq_disable(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn move_int_phy(dev: *mut net_device, addr: c_int) {
    static void move_int_phy(struct net_device *dev, int addr)
    {
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = ns_ioaddr(dev);
    let mut target: c_int = 31;
//
...
#[no_mangle]
unsafe extern "C" fn natsemi_init_media(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn natsemi_probe1(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int natsemi_probe1(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
    struct net_device *dev;
    struct netdev_private *np;
    int i, option, irq, chip_idx = ent.driver_data;
    let mut find_cnt: static int = -1;
...
// D0 state, disable PME assertion
    let mut newtmp: u32 = tmp & ~PCI_PM_CTRL_STATE_MASK;
...
// Work around the dropped serial bit.
    ...
    for (i = 0; i < 3; i++) {
    let mut eedata: c_int = eeprom_read(ioaddr, i + 7);
...
// save the silicon revision for later querying
    ...
    printk(", port TP.\n");
#[no_mangle]
pub unsafe extern "C" fn if(_arg: np->ignore_phy) -> else {
...
#[no_mangle]
unsafe extern "C" fn mii_getbit(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mii_send_bits(dev: *mut net_device, data: u32, len: c_int) {
    ...
    for (i = (1 << (len-1)); i; i >>= 1)
    {
    let mut mdio_val: u32 = MII_Write | ((data & i)? MII_Data : 0);
...
#[no_mangle]
unsafe extern "C" fn miiport_read(dev: *mut net_device, phy_id: c_int, reg: c_int) -> c_int {
    static int miiport_read(struct net_device *dev, int phy_id, int reg)
    {
    u32 cmd;
    int i;
...
#[no_mangle]
unsafe extern "C" fn miiport_write(dev: *mut net_device, phy_id: c_int, reg: c_int, data: u16) {
    static void miiport_write(struct net_device *dev, int phy_id, int reg, u16 data)
    {
    u32 cmd;
// Ensure sync
...
#[no_mangle]
unsafe extern "C" fn mdio_read(dev: *mut net_device, reg: c_int) -> c_int {
    static int mdio_read(struct net_device *dev, int reg)
    {
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn mdio_write(dev: *mut net_device, reg: c_int, data: u16) {
    static void mdio_write(struct net_device *dev, int reg, u16 data)
    {
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn init_phy_fixup(dev: *mut net_device) {
    static void init_phy_fixup(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn switch_port_external(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn switch_port_internal(dev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn find_mii(dev: *mut net_device) -> c_int {
    static int find_mii(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    int tmp;
...
pub const CFG_RESET_SAVE: c_uint = 0xfde000;
// WCSR bits [0:4] [9:10]
pub const WCSR_RESET_SAVE: c_uint = 0x61f;
// RFCR bits [20] [22] [27:31]
pub const RFCR_RESET_SAVE: c_uint = 0xf8500000;
#[no_mangle]
unsafe extern "C" fn natsemi_reset(dev: *mut net_device) {
    static void natsemi_reset(struct net_device *dev)
    {
    int i;
    u32 cfg;
...
#[no_mangle]
unsafe extern "C" fn reset_rx(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn natsemi_reload_eeprom(dev: *mut net_device) {
    static void natsemi_reload_eeprom(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn natsemi_stop_rxtx(dev: *mut net_device) {
    static void natsemi_stop_rxtx(struct net_device *dev)
    {
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
    struct netdev_private *np = netdev_priv(dev);
    int i;
...
// now set the MAC address according to dev->dev_addr
    for (i = 0; i < 3; i++) {
    let mut mac: u16 = (dev.dev_addr[2*i+1]<<8) + dev.dev_addr[2*i];
    writel(i*2, ioaddr + RxFilterAddr);
...
#[no_mangle]
unsafe extern "C" fn do_cable_magic(dev: *mut net_device) {
    static void do_cable_magic(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn undo_cable_magic(dev: *mut net_device) {
    static void undo_cable_magic(struct net_device *dev)
    {
    u16 data;
    struct netdev_private *np = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn check_link(dev: *mut net_device) {
    static void check_link(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
    let mut duplex: c_int = np.duplex;
    u16 bmsr;
...
unsafe extern "C" fn netdev_timer(t: *mut timer_list) {
    ...
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
    let mut next_tick: c_int = NATSEMI_TIMER_FREQ;
    let mut irq: c_int = np.pci_dev.irq;
...
#[no_mangle]
unsafe extern "C" fn dump_ring(dev: *mut net_device) {
    static void dump_ring(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn ns_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void ns_tx_timeout(struct net_device *dev, unsigned int txqueue)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn alloc_ring(dev: *mut net_device) -> c_int {
    static int alloc_ring(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn refill_rx(dev: *mut net_device) {
    static void refill_rx(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
// Refill the Rx ring buffers.
    ...
    struct sk_buff *skb;
    let mut entry: c_int = np.dirty_rx % RX_RING_SIZE;
    if (np.rx_skbuff[entry] == core::ptr::null_mut()) {
    let mut buflen: c_uint = np.rx_buf_sz+NATSEMI_PADDING;
...
#[no_mangle]
unsafe extern "C" fn init_ring(dev: *mut net_device) {
    static void init_ring(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    int i;
...
#[no_mangle]
unsafe extern "C" fn drain_tx(dev: *mut net_device) {
    static void drain_tx(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    int i;
...
#[no_mangle]
unsafe extern "C" fn drain_rx(dev: *mut net_device) {
    static void drain_rx(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut buflen: c_uint = np.rx_buf_sz;
    int i;
...
#[no_mangle]
unsafe extern "C" fn drain_ring(dev: *mut net_device) {
    static void drain_ring(struct net_device *dev)
    {
    drain_rx(dev);
    drain_tx(dev);
...
#[no_mangle]
unsafe extern "C" fn free_ring(dev: *mut net_device) {
    static void free_ring(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn reinit_rx(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn reinit_ring(dev: *mut net_device) {
    static void reinit_ring(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    int i;
...
//
#[no_mangle]
unsafe extern "C" fn natsemi_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    static int natsemi_poll(struct napi_struct *napi, int budget)
    {
...
#[no_mangle]
unsafe extern "C" fn netdev_rx(dev: *mut net_device, work_done: *mut c_int, work_to_do: c_int) {
    static void netdev_rx(struct net_device *dev, int *work_done, int work_to_do)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut entry: c_int = np.cur_rx % RX_RING_SIZE;
    let mut boguscnt: c_int = np.dirty_rx + RX_RING_SIZE - np.cur_rx;
    let mut desc_status: i32 = le32_to_cpu(np.rx_head_desc.cmd_status);
    let mut buflen: c_uint = np.rx_buf_sz;
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
...
// without copying to a minimally-sized skbuff.
    ...
    (skb = netdev_alloc_skb(dev, pkt_len + RX_OFFSET)) != core::ptr::null_mut()) {
...
unsafe extern "C" fn netdev_error(dev: *mut net_device, intr_status: c_int) {
    ...
    let mut lpa: u16 = mdio_read(dev, MII_LPA);
...
// read MII int status to clear the flag
    ...
    let mut wol_status: c_int = readl(ioaddr + WOLCmd);
...
#[no_mangle]
unsafe extern "C" fn __get_stats(dev: *mut net_device) {
    static void __get_stats(struct net_device *dev)
    {
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn natsemi_poll_controller(dev: *mut net_device) {
    static void natsemi_poll_controller(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut irq: c_int = np.pci_dev.irq;
...
pub const HASH_TABLE: c_uint = 0x200;
#[no_mangle]
unsafe extern "C" fn __set_rx_mode(dev: *mut net_device) {
    static void __set_rx_mode(struct net_device *dev)
    {
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
    struct netdev_private *np = netdev_priv(dev);
    ...
    let mut b: c_int = (ether_crc(ETH_ALEN, ha.addr) >> 23) & 0x1ff;
...
#[no_mangle]
unsafe extern "C" fn natsemi_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
    static int natsemi_change_mtu(struct net_device *dev, int new_mtu)
    {
...
#[no_mangle]
unsafe extern "C" fn get_drvinfo(dev: *mut net_device, info: *mut ethtool_drvinfo) {
...
#[no_mangle]
unsafe extern "C" fn get_eeprom_len(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_wol(dev: *mut net_device, wol: *mut ethtool_wolinfo) {
...
#[no_mangle]
unsafe extern "C" fn set_wol(dev: *mut net_device, wol: *mut ethtool_wolinfo) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_regs(dev: *mut net_device, regs: *mut ethtool_regs, buf: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn get_msglevel(dev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn set_msglevel(dev: *mut net_device, val: u32) {
...
#[no_mangle]
unsafe extern "C" fn nway_reset(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_link(dev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn get_eeprom(dev: *mut net_device, eeprom: *mut ethtool_eeprom, data: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn netdev_set_wol(dev: *mut net_device, newval: u32) -> c_int {
    static int netdev_set_wol(struct net_device *dev, u32 newval)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
    let mut data: u32 = readl(ioaddr + WOLCmd) & ~WakeOptsSummary;
...
#[no_mangle]
unsafe extern "C" fn netdev_get_wol(dev: *mut net_device, supported: *mut u32, cur: *mut u32) -> c_int {
    static int netdev_get_wol(struct net_device *dev, u32 *supported, u32 *cur)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
    let mut regval: u32 = readl(ioaddr + WOLCmd);
...
#[no_mangle]
unsafe extern "C" fn netdev_set_sopass(dev: *mut net_device, newval: *mut u8) -> c_int {
    static int netdev_set_sopass(struct net_device *dev, u8 *newval)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
...
#[no_mangle]
unsafe extern "C" fn netdev_get_sopass(dev: *mut net_device, data: *mut u8) -> c_int {
    static int netdev_get_sopass(struct net_device *dev, u8 *data)
    {
    struct netdev_private *np = netdev_priv(dev);
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
...
// ignore maxtxpkt, maxrxpkt for now
    ...
    let mut speed: u32 = ecmd.base.speed;
...
#[no_mangle]
unsafe extern "C" fn netdev_get_regs(dev: *mut net_device, buf: *mut u8) -> c_int {
    static int netdev_get_regs(struct net_device *dev, u8 *buf)
    {
    int i;
    int j;
...
#[no_mangle]
unsafe extern "C" fn netdev_get_eeprom(dev: *mut net_device, buf: *mut u8) -> c_int {
    static int netdev_get_eeprom(struct net_device *dev, u8 *buf)
    {
    int i;
    u16 *ebuf = (u16 *)buf;
...
#[no_mangle]
unsafe extern "C" fn enable_wol_mode(dev: *mut net_device, enable_intr: c_int) {
    static void enable_wol_mode(struct net_device *dev, int enable_intr)
    {
    let mut ioaddr: *mut void __iomem = ns_ioaddr(dev);
    struct netdev_private *np = netdev_priv(dev);
...
// clear the carrier last - an interrupt could reenable it otherwise
    ...
    free_ring(dev);
    {
    let mut wol: u32 = readl(ioaddr + WOLCmd) & WakeOptsSummary;
    if (wol) {
...
#[no_mangle]
unsafe extern "C" fn natsemi_remove1(pdev: *mut pci_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn natsemi_suspend(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn natsemi_resume(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn natsemi_init_mod() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn natsemi_exit_mod() -> void __exit {
