...
//! Source: drivers/net/ethernet/natsemi/xtsonic.c
#![no_std]
...
pub const SONIC_RBSIZE: c_int = 1524;
//
// The chip provides 256 byte register space.
//
pub const SONIC_MEM_SIZE: c_uint = 0x100;
//
...
#[no_mangle]
unsafe extern "C" fn xtsonic_open(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xtsonic_close(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sonic_probe1(dev: *mut net_device) -> c_int {
    static int sonic_probe1(struct net_device *dev)
    {
    unsigned int silicon_revision;
    struct sonic_local *lp = netdev_priv(dev);
    let mut base_addr: c_uint = dev.base_addr;
    int i;
    let mut err: c_int = 0;
...
//
    ...
    SONIC_WRITE(SONIC_CEP,0);
    for (i=0; i<3; i++) {
    let mut val: c_uint = SONIC_READ(SONIC_CAP0-i);
    addr[i*2] = val;
    addr[i*2+1] = val >> 8;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn xtsonic_probe(pdev: *mut platform_device) -> c_int {
    ...
    let mut err: c_int = 0;
    if ((resmem = platform_get_resource(pdev, IORESOURCE_MEM, 0)) == core::ptr::null_mut())
    return -ENODEV;
    if ((resirq = platform_get_resource(pdev, IORESOURCE_IRQ, 0)) == core::ptr::null_mut())
    return -ENODEV;
    if ((dev = alloc_etherdev(sizeof(struct sonic_local))) == core::ptr::null_mut())
    return -ENOMEM;
    lp = netdev_priv(dev);
    lp.device = &pdev.dev;
...
#[no_mangle]
unsafe extern "C" fn xtsonic_device_remove(pdev: *mut platform_device) {
