...
//! Source: drivers/net/ethernet/airoha/airoha_ppe.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_get_num_stats_entries(ppe: *mut airoha_ppe) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_get_total_num_stats_entries(ppe: *mut airoha_ppe) -> c_int {
    static int airoha_ppe_get_total_num_stats_entries(struct airoha_ppe *ppe)
    {
    let mut num_stats: c_int = airoha_ppe_get_num_stats_entries(ppe);
    if (num_stats > 0) {
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_get_total_sram_num_entries(ppe: *mut airoha_ppe) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_get_total_num_entries(ppe: *mut airoha_ppe) -> u32 {
    u32 airoha_ppe_get_total_num_entries(struct airoha_ppe *ppe)
    {
    let mut sram_num_entries: u32 = airoha_ppe_get_total_sram_num_entries(ppe);
    return sram_num_entries + PPE_DRAM_NUM_ENTRIES;
    }
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_is_enabled(eth: *mut airoha_eth, index: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_get_timestamp(ppe: *mut airoha_ppe) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_set_cpu_port(dev: *mut airoha_gdm_dev, ppe_id: u8, fport: u8) {
    void airoha_ppe_set_cpu_port(struct airoha_gdm_dev *dev, u8 ppe_id, u8 fport)
    {
    struct airoha_qdma *qdma = airoha_qdma_deref(dev);
    struct airoha_eth *eth = dev.eth;
    let mut qdma_id: u8 = qdma - &eth.qdma[0];
    u32 fe_cpu_port;
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_set_xmit_frame_size(dev: *mut airoha_gdm_dev) {
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_hw_init(ppe: *mut airoha_ppe) {
    static void airoha_ppe_hw_init(struct airoha_ppe *ppe)
    {
    let mut sram_ppe_num_data_entries: u32 = PPE_SRAM_NUM_ENTRIES, sram_num_entries;
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_flow_mangle_eth(act: *const flow_action_entry, eth: *mut c_void) {
    ...
    {
    let mut val: u32 = be32_to_cpu(( __be32)act.mangle.val);
    switch (act.mangle.offset) {
    case 0:
    if (( __be32)act.mangle.mask == ~cpu_to_be32(0xffff))
...
#[no_mangle]
unsafe extern "C" fn airoha_get_dsa_port(dev: *mut net_device) -> c_int {
...
// dev = dsa_port_to_conduit(dp);
    ...
    {
    let mut qdata: u32 = FIELD_PREP(AIROHA_FOE_SHAPER_ID, 0x7f), ports_pad, val;
    let mut wlan_etype: c_int = -EINVAL, dsa_port = airoha_get_dsa_port(&netdev);
    struct airoha_foe_mac_info_common *l2;
    let mut smac_id: u8 = 0xf;
...
//
    ...
    bool egress)
    {
    let mut type: c_int = FIELD_GET(AIROHA_FOE_IB1_BIND_PACKET_TYPE, hwe.ib1);
    ...
    {
    let mut type: c_int = FIELD_GET(AIROHA_FOE_IB1_BIND_PACKET_TYPE, hwe.ib1);
    let mut ppe_hash_mask: u32 = airoha_ppe_get_total_num_entries(ppe) - 1;
...
// ib2 |= FIELD_PREP(AIROHA_FOE_IB2_PSE_PORT, 6) |
    ...
    let mut ppe2: bool = hash >= PPE_SRAM_NUM_ENTRIES;
    struct airoha_eth *eth = ppe.eth;
    u32 val;
    int i;
    ...
    struct airoha_foe_entry *hwe)
    {
    let mut type: c_int = FIELD_GET(AIROHA_FOE_IB1_BIND_PACKET_TYPE, e.data.ib1);
    int len;
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_foe_commit_sram_entry(ppe: *mut airoha_ppe, hash: u32) -> c_int {
    ...
    let mut sram_num_entries: u32 = airoha_ppe_get_total_sram_num_entries(ppe);
    struct airoha_foe_entry *hwe = ppe.foe + hash * sizeof(*hwe);
    let mut ts: u32 = airoha_ppe_get_timestamp(ppe);
    struct airoha_eth *eth = ppe.eth;
    struct airoha_npu *npu;
    let mut err: c_int = 0;
    ...
    u32 hash, bool rx_wlan)
    {
    let mut mask: u32 = AIROHA_FOE_IB1_BIND_PACKET_TYPE | AIROHA_FOE_IB1_BIND_UDP;
...
//
    ...
    {
    struct airoha_flow_table_entry *e;
    let mut br: airoha_foe_bridge = {};
    struct airoha_foe_entry *hwe;
    let mut commit_done: bool = false;
    struct hlist_node *n;
    u32 index, state;
    spin_lock_bh(&ppe_lock);
    hwe = airoha_ppe_foe_get_entry_locked(ppe, hash);
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_get_entry_idle_time(ppe: *mut airoha_ppe, ib1: u32) -> c_int {
    static int airoha_ppe_get_entry_idle_time(struct airoha_ppe *ppe, u32 ib1)
    {
    let mut state: u32 = FIELD_GET(AIROHA_FOE_IB1_BIND_STATE, ib1);
    ...
    struct airoha_flow_table_entry *e)
    {
    let mut min_idle: c_int = airoha_ppe_get_entry_idle_time(ppe, e.data.ib1);
    ...
    struct airoha_flow_table_entry *e;
    let mut data: airoha_flow_data = {};
    struct net_device *odev = core::ptr::null_mut();
    struct flow_action_entry *act;
    let mut l4proto: u8 = 0, priority = 0;
    struct airoha_foe_entry hwe;
...
// handled earlier
    ...
    npu = rcu_dereference(eth.npu);
    if (npu) {
    let mut packets: u64 = ppe.foe_stats[index].packets;
    let mut bytes: u64 = ppe.foe_stats[index].bytes;
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_flush_sram_entries(ppe: *mut airoha_ppe) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_wait_for_npu_init(eth: *mut airoha_eth) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn airoha_ppe_offload_setup(eth: *mut airoha_eth) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_setup_tc_block_cb(dev: *mut airoha_ppe_dev, type_data: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_init_upd_mem(dev: *mut airoha_gdm_dev, addr: *const u8) {
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_put_dev(dev: *mut airoha_ppe_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_init(eth: *mut airoha_eth) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn airoha_ppe_deinit(eth: *mut airoha_eth) {
    void airoha_ppe_deinit(struct airoha_eth *eth)
    {
    struct airoha_npu *npu;
    mutex_lock(&flow_offload_mutex);
    npu = rcu_replace_pointer(eth.npu, core::ptr::null_mut(),
