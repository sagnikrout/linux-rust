...
//! Source: drivers/net/ethernet/adaptec/starfire.c
#![no_std]
...
//
// Macro flag: #define HAS_BROKEN_FIRMWARE
//
// If using the broken firmware, data must be padded to the next 32-bit boundary.
//

pub const PADDING_MASK: c_int = 3;

//
// Define this if using the driver with the zero-copy patch
//
// Macro flag: #define ZEROCOPY

// Macro flag: #define VLAN_SUPPORT
...
// Maximum number of multicast addresses to filter (vs. rx-all-multicast).
    ...
    let mut multicast_filter_limit: static int = 512;
// Whether to do TCP/UDP checksums in hardware
    let mut enable_hw_cksum: static int = 1;
...
//
    ...
    let mut rx_copybreak: static int = PKT_BUF_SZ;

    static int rx_copybreak /* = 0 */;

// PCI DMA burst size -- on sparc64 we want to force it to 64 bytes, on the others the default of 128 is fine.

pub const DMA_BURST_SIZE: c_int = 64;

pub const DMA_BURST_SIZE: c_int = 128;
...
pub const DONE_Q_SIZE: c_int = 1024;
// All queues must be aligned on a 256-byte boundary
pub const QUEUE_ALIGN: c_int = 256;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct starfire_rx_desc {
    pub rxaddr: netdrv_addr_t,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct csum_rx_done_desc {
    pub /: *mut *mut __le32 status; / Low 16 bits is length.,
    pub /: *mut *mut __le16 csum; / Partial checksum,
    pub status2: __le16,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct full_rx_done_desc {
    pub /: *mut *mut __le32 status; / Low 16 bits is length.,
    pub status3: __le16,
    pub status2: __le16,
    pub vlanid: __le16,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct starfire_tx_desc_1 {
    pub /: *mut *mut __le32 status; / Upper bits are status, lower 16 length.,
    pub addr: __le32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tx_done_desc {
    pub /: *mut *mut __le32 status; / timestamp, index.,

    pub /: *mut *mut __le32 intrstatus; / interrupt status,
...
pub struct tx_ring_info {
    pub skb: *mut sk_buff,
    pub mapping: dma_addr_t,
    pub used_slots: c_uint,
}

pub const PHY_CNT: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct netdev_private {
// Descriptor rings first for alignment.
    pub rx_ring: *mut starfire_rx_desc,
    pub tx_ring: *mut starfire_tx_desc,
    pub rx_ring_dma: dma_addr_t,
    pub tx_ring_dma: dma_addr_t,
// The addresses of rx/tx-in-place skbuffs.
    pub rx_info: [rx_ring_info; RX_RING_SIZE],
    pub tx_info: [tx_ring_info; TX_RING_SIZE],
// Pointers to completion queues (full pages).
    pub rx_done_q: *mut rx_done_desc,
    pub rx_done_q_dma: dma_addr_t,
    pub rx_done: c_uint,
    pub tx_done_q: *mut tx_done_desc,
    pub tx_done_q_dma: dma_addr_t,
    pub tx_done: c_uint,
    pub napi: napi_struct,
    pub dev: *mut net_device,
    pub pci_dev: *mut pci_dev,
    pub active_vlans: [c_ulong; BITS_TO_LONGS(VLAN_N_VID)],
    pub queue_mem: *mut c_void,
    pub queue_mem_dma: dma_addr_t,
    pub queue_mem_size: usize,
// Frequently used values: keep some adjacent for cache effect.
    pub lock: spinlock_t,
    pub /: *mut *mut unsigned int cur_rx, dirty_rx; / Producer/consumer ring indices,
    pub reap_tx: unsigned int cur_tx, dirty_tx,,
    pub /: *mut *mut unsigned int rx_buf_sz; / Based on MTU+slack.,
// These values keep track of the transceiver/media in use.
    pub /: *mut *mut int speed100; / Set if speed == 100MBit.,
    pub tx_mode: u32,
    pub intr_timer_ctrl: u32,
    pub tx_threshold: u8,
// MII transceiver section.
    pub /: *mut *mut mii_if_info mii_if; / MII lib hooks/info,
    pub /: *mut *mut int phy_cnt; / MII device addresses.,
    pub /: *mut *mut unsigned char phys[PHY_CNT]; / MII device addresses.,
...
// Allocate the various queues.
    ...
    if (np.queue_mem == core::ptr::null_mut()) {
    free_irq(irq, dev);
    return -ENOMEM;
    }
...
#[no_mangle]
unsafe extern "C" fn check_duplex(dev: *mut net_device) {
    static void check_duplex(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    u16 reg0;
    let mut silly_count: c_int = 1000;
...
// Fill in the Rx buffers.  Handle allocation failure gracefully.
    ...
    dev_kfree_skb(skb);
    np.rx_info[i].skb = core::ptr::null_mut();
    break;
    }
...
// 4 is arbitrary, but should be ok
    ...
    np.tx_info[entry].skb = core::ptr::null_mut();
    if (i > 0) {
...
unsafe extern "C" fn intr_handler(irq: c_int, dev_instance: *mut c_void) -> irqreturn_t {
    ...
    do {
    let mut intr_status: u32 = readl(ioaddr + IntrClear);
    if (debug > 4)
...
// Scavenge the skbuff list based on the Tx-done queue.
    ...
    let mut entry: u16 = (tx_status & 0x7fff) / sizeof(starfire_tx_desc);
...
//
#[no_mangle]
unsafe extern "C" fn __netdev_rx(dev: *mut net_device, quota: *mut c_int) -> c_int {
    static int __netdev_rx(struct net_device *dev, int *quota)
    {
    struct netdev_private *np = netdev_priv(dev);
    u32 desc_status;
...
// Check if the packet is long enough to accept without copying
    ...
    skb_put(skb, pkt_len);
    np.rx_info[entry].skb = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn if(0x0040: le16_to_cpu(desc->status2) &) -> else {
    ...
    let mut vlid: u16 = le16_to_cpu(desc.vlanid);
    if (debug > 4) {
...
#[no_mangle]
unsafe extern "C" fn netdev_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    static int netdev_poll(struct napi_struct *napi, int budget)
    {
...
#[no_mangle]
unsafe extern "C" fn refill_rx_ring(dev: *mut net_device) {
    static void refill_rx_ring(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    struct sk_buff *skb;
    let mut entry: c_int = -1;
// Refill the Rx ring buffers.
    for (; np.cur_rx - np.dirty_rx > 0; np.dirty_rx++) {
    entry = np.dirty_rx % RX_RING_SIZE;
    if (np.rx_info[entry].skb == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn netdev_media_change(dev: *mut net_device) {
    static void netdev_media_change(struct net_device *dev)
    {
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = np.base;
...
#[no_mangle]
unsafe extern "C" fn set_vlan_mode(np: *mut netdev_private) -> u32 {
    static u32 set_vlan_mode(struct netdev_private *np)
    {
    let mut ret: u32 = VlanMode;
    u16 vid;
    void __iomem *filter_addr = np.base + HashTable + 8;
    let mut vlan_count: c_int = 0;
...
unsafe extern "C" fn set_rx_mode(dev: *mut net_device) {
    ...
    struct netdev_private *np = netdev_priv(dev);
    void __iomem *ioaddr = np.base;
    let mut rx_mode: u32 = MinVLANPrio;
...
// The chip uses the upper 9 CRC bits
    ...
    let mut bit_nr: c_int = ether_crc_le(ETH_ALEN, ha.addr) >> 23;
...
#[no_mangle]
unsafe extern "C" fn check_if_running(dev: *mut net_device) -> c_int {
...
unsafe extern "C" fn netdev_ioctl(dev: *mut net_device, rq: *mut ifreq, cmd: c_int) -> c_int {
    ...
    return -EINVAL;
    spin_lock_irq(&np.lock);
    rc = generic_mii_ioctl(&np.mii_if, data, cmd, core::ptr::null_mut());
    spin_unlock_irq(&np.lock);
...
// Free all the skbuffs in the Rx queue.
    ...
    if (np.rx_info[i].skb != core::ptr::null_mut()) {
    dma_unmap_single(&np.pci_dev.dev,
    ...
    dev_kfree_skb(skb);
    np.tx_info[i].skb = core::ptr::null_mut();
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn starfire_suspend(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn starfire_resume(dev_d: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn starfire_remove_one(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn starfire_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn starfire_cleanup() -> void __exit {
