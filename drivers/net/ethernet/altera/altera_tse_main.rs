...
//! Source: drivers/net/ethernet/altera/altera_tse_main.c
#![no_std]
...
//
    ...
    let mut instance_count: static atomic_t = ATOMIC_INIT(~0);
// Module parameters
    ...
    NETIF_MSG_IFDOWN);
pub const RX_DESCRIPTORS: c_int = 64;
    let mut dma_rx_num: static int = RX_DESCRIPTORS;
    module_param(dma_rx_num, int, 0644);
    MODULE_PARM_DESC(dma_rx_num, "Number of descriptors in the RX list");
pub const TX_DESCRIPTORS: c_int = 64;
    let mut dma_tx_num: static int = TX_DESCRIPTORS;
...
//
pub const ALTERA_RXDMABUFFER_SIZE: c_int = 2048;
// Allow network stack to resume queuing packets after we've
// finished transmitting at least 1/4 of the packets in the queue.
//

pub const TXQUEUESTOP_THRESHHOLD: c_int = 2;
#[no_mangle]
pub unsafe extern "C" fn tse_tx_avail(priv: *mut altera_tse_private) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn altera_tse_mdio_read(bus: *mut mii_bus, mii_id: c_int, regnum: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn altera_tse_mdio_create(dev: *mut net_device, id: c_uint) -> c_int {
    static int altera_tse_mdio_create(struct net_device *dev, unsigned int id)
    {
    struct altera_tse_private *priv = netdev_priv(dev);
    struct device_node *mdio_node = core::ptr::null_mut();
    struct device_node *child_node = core::ptr::null_mut();
    struct mii_bus *mdio = core::ptr::null_mut();
    int ret;
    ...
    out_free_mdio:
    mdiobus_free(mdio);
    mdio = core::ptr::null_mut();
    put_node:
    of_node_put(mdio_node);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn altera_tse_mdio_destroy(dev: *mut net_device) {
    static void altera_tse_mdio_destroy(struct net_device *dev)
    {
    struct altera_tse_private *priv = netdev_priv(dev);
    if (priv.mdio == core::ptr::null_mut())
    return;
    if (netif_msg_drv(priv))
    ...
    {
    let mut dma_addr: dma_addr_t = rxbuffer.dma_addr;
    ...
    rxbuffer.skb = core::ptr::null_mut();
    rxbuffer.dma_addr = 0;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn alloc_init_skbufs(priv: *mut altera_tse_private) -> c_int {
    static int alloc_init_skbufs(struct altera_tse_private *priv)
    {
    let mut rx_descs: c_uint = priv.rx_ring_size;
    let mut tx_descs: c_uint = priv.tx_ring_size;
...
#[no_mangle]
unsafe extern "C" fn free_skbufs(dev: *mut net_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tse_rx_refill(priv: *mut altera_tse_private) {
    static inline void tse_rx_refill(struct altera_tse_private *priv)
    {
    let mut rxsize: c_uint = priv.rx_ring_size;
    unsigned int entry;
    int ret;
    for (; priv.rx_cons - priv.rx_prod > 0;
    priv.rx_prod++) {
    entry = priv.rx_prod % rxsize;
    if (likely(priv.rx_ring[entry].skb == core::ptr::null_mut())) {
...
//
#[no_mangle]
pub unsafe extern "C" fn tse_rx_vlan(dev: *mut net_device, skb: *mut sk_buff) {
...
//
#[no_mangle]
unsafe extern "C" fn tse_rx(priv: *mut altera_tse_private, limit: c_int) -> c_int {
    static int tse_rx(struct altera_tse_private *priv, int limit)
    {
    let mut entry: c_uint = priv.rx_cons % priv.rx_ring_size;
...
//
    ...
    priv.rx_ring[entry].skb = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn tse_tx_complete(priv: *mut altera_tse_private) -> c_int {
    static int tse_tx_complete(struct altera_tse_private *priv)
    {
    let mut txsize: c_uint = priv.tx_ring_size;
    struct tse_buffer *tx_buff;
    unsigned int entry;
    let mut txcomplete: c_int = 0;
    u32 ready;
...
//
#[no_mangle]
unsafe extern "C" fn tse_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    static int tse_poll(struct napi_struct *napi, int budget)
    {
    struct altera_tse_private *priv =
    container_of(napi, struct altera_tse_private, napi);
    unsigned long int flags;
    let mut rxcomplete: c_int = 0;
...
//
#[no_mangle]
unsafe extern "C" fn altera_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn tse_start_xmit(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
    ...
    struct tse_buffer *buffer = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn altera_tse_phy_get_addr_mdio_create(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tse_update_mac_addr(priv: *mut altera_tse_private, addr: *const u8) {
...
//
#[no_mangle]
unsafe extern "C" fn reset_mac(priv: *mut altera_tse_private) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn init_mac(priv: *mut altera_tse_private) -> c_int {
    static int init_mac(struct altera_tse_private *priv)
    {
    let mut cmd: c_uint = 0;
    u32 frm_length;
// Setup Rx FIFO
...
//
#[no_mangle]
unsafe extern "C" fn tse_set_mac(priv: *mut altera_tse_private, enable: bool) {
    static void tse_set_mac(struct altera_tse_private *priv, bool enable)
    {
    let mut value: u32 = csrrd32(priv.mac_dev, tse_csroffs(command_config));
    if (enable)
...
//
#[no_mangle]
unsafe extern "C" fn tse_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn altera_tse_set_mcfilter(dev: *mut net_device) {
...
// clear the hash filter
    ...
    let mut xor_bit: c_uchar = 0;
    let mut octet: c_uchar = ha.addr[mac_octet];
...
#[no_mangle]
unsafe extern "C" fn altera_tse_set_mcfilterall(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn tse_set_rx_mode_hashfilter(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn tse_set_rx_mode(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn tse_open(dev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tse_shutdown(dev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn altera_tse_probe(pdev: *mut platform_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn altera_tse_remove(pdev: *mut platform_device) {
