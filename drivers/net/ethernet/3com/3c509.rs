...
//! Source: drivers/net/ethernet/3com/3c509.c
#![no_std]
...
pub const TX_FIFO: c_uint = 0x00;
pub const RX_FIFO: c_uint = 0x00;
pub const RX_STATUS: c_uint = 0x08;
pub const TX_STATUS: c_uint = 0x0B;
pub const TX_FREE: c_uint = 0x0C	/* Remaining free bytes in Tx buffer. */;
pub const WN0_CONF_CTRL: c_uint = 0x04	/* Window 0: Configuration control register. */;
pub const WN0_ADDR_CONF: c_uint = 0x06	/* Window 0: Address configuration register. */;
pub const WN0_IRQ: c_uint = 0x08	/* Window 0: Set IRQ line in bits 12-15. */;
pub const WN4_MEDIA: c_uint = 0x0A	/* Window 4: Various transcvr/media bits. */;
pub const MEDIA_TP: c_uint = 0x00C0	/* Enable link beat and jabber for 10baseT. */;
pub const WN4_NETDIAG: c_uint = 0x06	/* Window 4: Net diagnostic. */;
pub const FD_ENABLE: c_uint = 0x8000	/* Enable full-duplex ("external loopback"). */;
//
// Must be a power of two (we use a binary and in the
// circular queue).
//
pub const SKB_QUEUE_SIZE: c_int = 64;
    enum el3_cardtype { EL3_ISA, EL3_PNP, EL3_EISA };
#[repr(C)]
#[derive(Copy, Clone)]
pub struct el3_private {
// for device access
    pub lock: spinlock_t,
// skb send-queue
    pub size: int head,,
    pub queue: [*mut sk_buff; SKB_QUEUE_SIZE],
    pub type: enum el3_cardtype,
}
...
// Maximum events (Rx packets, etc.) to handle at each interrupt.
    let mut max_interrupt_work: static int = 10;
...
#[no_mangle]
unsafe extern "C" fn el3_isa_id_sequence(phys_addr: *mut __be16) -> c_int {
    static int el3_isa_id_sequence(__be16 *phys_addr)
    {
    let mut lrs_state: c_short = 0xff;
    int i;
...
#[no_mangle]
unsafe extern "C" fn el3_isa_match(pdev: *mut device, ndev: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn el3_isa_remove(pdev: *mut device, ndev: c_uint) {
    static void el3_isa_remove(struct device *pdev, unsigned int ndev)
    {
    el3_device_remove(pdev);
    dev_set_drvdata(pdev, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn el3_isa_resume(dev: *mut device, n: c_uint) -> c_int {
    static int el3_isa_resume(struct device *dev, unsigned int n)
    {
    struct net_device *ndev = dev_get_drvdata(dev);
    let mut ioaddr: c_int = ndev.base_addr, err;
    __be16 phys_addr[3];
    while ((err = el3_isa_id_sequence(phys_addr)) == 2)
    ;	/* Skip to next card when PnP card found */
    if (err == 1)
...
#[no_mangle]
unsafe extern "C" fn el3_pnp_probe(pdev: *mut pnp_dev, id: *const pnp_device_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn el3_pnp_remove(pdev: *mut pnp_dev) {
...
#[no_mangle]
unsafe extern "C" fn el3_pnp_suspend(pdev: *mut pnp_dev, state: pm_message_t) -> c_int {
    static int el3_pnp_suspend(struct pnp_dev *pdev, pm_message_t state)
    {
    return el3_suspend(&pdev.dev, state);
    }
#[no_mangle]
unsafe extern "C" fn el3_pnp_resume(pdev: *mut pnp_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn el3_common_init(dev: *mut net_device) -> c_int {
    static int el3_common_init(struct net_device *dev)
    {
...
#[no_mangle]
unsafe extern "C" fn el3_common_remove(dev: *mut net_device) {
    static void el3_common_remove(struct net_device *dev)
    {
...
#[no_mangle]
unsafe extern "C" fn el3_eisa_probe(device: *mut device) -> c_int {
    static int el3_eisa_probe(struct device *device)
    {
    struct net_device *dev = core::ptr::null_mut();
...
// Restore the "Product ID" to the EEPROM read register.
    ...
    err = el3_common_init(dev);
    if (err) {
    eisa_set_drvdata(edev, core::ptr::null_mut());
    free_netdev(dev);
    return err;
    }
    el3_devs[el3_cards++] = dev;
...
//
#[no_mangle]
unsafe extern "C" fn el3_device_remove(device: *mut device) -> c_int {
    static int el3_device_remove(struct device *device)
    {
    struct net_device *dev;
...
//
#[no_mangle]
unsafe extern "C" fn read_eeprom(ioaddr: c_int, index: c_int) -> c_ushort {
    static ushort read_eeprom(int ioaddr, int index)
    {
...
#[no_mangle]
unsafe extern "C" fn id_read_eeprom(index: c_int) -> c_ushort {
    static ushort id_read_eeprom(int index)
    {
    int bit, word = 0;
...
#[no_mangle]
unsafe extern "C" fn el3_open(dev: *mut net_device) -> c_int {
    static int el3_open(struct net_device *dev)
    {
...
#[no_mangle]
unsafe extern "C" fn el3_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void el3_tx_timeout(struct net_device *dev, unsigned int txqueue)
    {
    let mut ioaddr: c_int = dev.base_addr;
...
#[no_mangle]
unsafe extern "C" fn el3_start_xmit(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
    static netdev_tx_t el3_start_xmit(struct sk_buff *skb, struct net_device *dev)
    {
    struct el3_private *lp = netdev_priv(dev);
    let mut ioaddr: c_int = dev.base_addr;
...
#[no_mangle]
unsafe extern "C" fn el3_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t el3_interrupt(int irq, void *dev_id)
    {
    struct net_device *dev = dev_id;
    let mut i: c_int = max_interrupt_work;
...
//
#[no_mangle]
unsafe extern "C" fn el3_poll_controller(dev: *mut net_device) {
    static void el3_poll_controller(struct net_device *dev)
    {
    disable_irq(dev.irq);
...
//
#[no_mangle]
unsafe extern "C" fn update_stats(dev: *mut net_device) {
    static void update_stats(struct net_device *dev)
    {
    let mut ioaddr: c_int = dev.base_addr;
    if (el3_debug > 5)
...
#[no_mangle]
unsafe extern "C" fn el3_rx(dev: *mut net_device) -> c_int {
    static int el3_rx(struct net_device *dev)
    {
    let mut ioaddr: c_int = dev.base_addr;
    short rx_status;
    ...
    if (rx_status & 0x4000) {
// Error, update stats.
    let mut error: c_short = rx_status & 0x3800;
    ...
    let mut pkt_len: c_short = rx_status & 0x7ff;
    struct sk_buff *skb;
...
#[no_mangle]
unsafe extern "C" fn el3_close(dev: *mut net_device) -> c_int {
    static int el3_close(struct net_device *dev)
    {
    struct el3_private *lp = netdev_priv(dev);
    let mut ioaddr: c_int = dev.base_addr;
...
#[no_mangle]
unsafe extern "C" fn el3_link_ok(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn el3_get_link(dev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn el3_get_msglevel(dev: *mut net_device) -> u32 {
    static u32 el3_get_msglevel(struct net_device *dev)
    {
    return el3_debug;
    }
#[no_mangle]
unsafe extern "C" fn el3_set_msglevel(dev: *mut net_device, v: u32) {
...
#[no_mangle]
unsafe extern "C" fn el3_down(dev: *mut net_device) {
    static void el3_down(struct net_device *dev)
    {
    let mut ioaddr: c_int = dev.base_addr;
    netif_stop_queue(dev);
...
#[no_mangle]
unsafe extern "C" fn el3_up(dev: *mut net_device) {
    static void el3_up(struct net_device *dev)
    {
    let mut ioaddr: c_int = dev.base_addr;
...
#[no_mangle]
unsafe extern "C" fn el3_suspend(pdev: *mut device, state: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn el3_resume(pdev: *mut device) -> c_int {
    ...
    MODULE_LICENSE("GPL");
#[no_mangle]
unsafe extern "C" fn el3_init_module() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn el3_cleanup_module() -> void __exit {
