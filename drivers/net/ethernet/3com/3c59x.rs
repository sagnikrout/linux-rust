...
//! Source: drivers/net/ethernet/3com/3c59x.c
#![no_std]
...
// ARM systems perform better by disregarding the bus-master
    ...
    let mut rx_copybreak: static int = 1513;

// Allow setting MTU to a larger size, bypassing the normal ethernet setup.
    let mut mtu: static int = 1500;
// Maximum events (Rx packets, etc.) to handle at each interrupt.
    let mut max_interrupt_work: static int = 32;
// Tx timeout interval (millisecs)
    let mut watchdog: static int = 5000;
...
//
pub const tx_interrupt_mitigation: c_int = 1;
// Put out somewhat more debugging messages. (0: no msg, 1 minimal .. 6).

    let mut vortex_debug: static int = VORTEX_DEBUG;

    let mut vortex_debug: static int = 1;
...
pub const VORTEX_TOTAL_SIZE: c_uint = 0x20;
pub const BOOMERANG_TOTAL_SIZE: c_uint = 0x40;
...
//
    ...
    PCI_USES_MASTER, IS_TORNADO|HAS_NWAY|HAS_HWCKSM, 128, },
    {core::ptr::null_mut(),}, /* core::ptr::null_mut() terminated list. */
    };
...
//
pub const EL3_CMD: c_uint = 0x0e;
pub const EL3_STATUS: c_uint = 0x0e;
...
pub const LAST_FRAG: c_uint = 0x80000000			/* Last Addr/Len pair in descriptor. */;
pub const DN_COMPLETE: c_uint = 0x00010000			/* This packet has been downloaded */;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct boom_rx_desc {
    pub /: *mut *mut __le32 next; / Last entry points to 0.,
    pub status: __le32,
    pub /: *mut *mut __le32 addr; / Up to 63 addr/len pairs possible.,
    pub /: *mut *mut __le32 length; / Set LAST_FRAG to indicate last pair.,
}
...
pub const DO_ZEROCOPY: c_int = 1;

pub const DO_ZEROCOPY: c_int = 0;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct boom_tx_desc {
    pub /: *mut *mut __le32 next; / Last entry points to 0.,
    pub /: *mut *mut __le32 status; / bits 0:12 length, others see below.,

    struct {
    pub addr: __le32,
    pub length: __le32,
    pub frag: [}; 1+MAX_SKB_FRAGS],
    pub addr: __le32,
    pub length: __le32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vortex_extra_stats {
    pub tx_deferred: c_ulong,
    pub tx_max_collisions: c_ulong,
    pub tx_multiple_collisions: c_ulong,
    pub tx_single_collisions: c_ulong,
    pub rx_bad_ssd: c_ulong,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct vortex_private {
// The Rx and Tx rings should be quad-word-aligned.
    pub rx_ring: *mut *mut boom_rx_desc,
    pub tx_ring: *mut *mut boom_tx_desc,
...
// The addresses of transmit- and receive-in-place skbuffs.
    ...
    pub /: *mut *mut unsigned int dirty_tx; / The ring entries to be free()ed.,
    pub /: *mut *mut vortex_extra_stats xstats; / NIC-specific extra stats,
    pub /: *mut *mut *mut sk_buff tx_skb; / Packet being eaten by bus master ctrl.,
    pub /: *mut *mut dma_addr_t tx_skb_dma; / Allocated DMA address for bus master ctrl DMA.,
// PCI configuration space information.
    pub gendev: *mut device,
    pub /: *mut *mut *mut void __iomem ioaddr; / IO address space,
    pub /: *mut *mut *mut void __iomem cb_fn_base; / CardBus function status addr space.,
// Some values here only for performance evaluation and path-coverage
    pub rx_csumhits: int rx_nocopy, rx_copy, queued_packet,,
    pub card_idx: c_int,
// The remainder are related to chip state, mostly media selection.
    pub /: *mut *mut timer_list timer; / Media selection timer.,
    pub /: *mut *mut int options; / User-settable misc. driver options.,
    ...
    pub /: *mut *mut handling_irq:1; / private in_irq indicator,
// {get|set}_wol operations are already serialized by rtnl.
// no additional locking is required for the enable_wol and acpi_set_WOL()
//
    pub drv_flags: c_int,
    pub status_enable: u16,
    pub intr_enable: u16,
    pub /: *mut *mut u16 available_media; / From Wn3_Options.,
    pub /: *mut *mut u16 capabilities, info1, info2; / Various, from EEPROM.,
    pub /: *mut *mut u16 advertising; / NWay media advertisement,
    pub /: *mut *mut unsigned char phys[2]; / MII device addresses.,
    pub we: *mut *mut u16 deferred; / Resend these interrupts when,
// bale from the ISR
    pub /: *mut *mut u16 io_size; / Size of PCI region (for release_region),
// Serialises access to hardware other than MII and variables below.
// The lock hierarchy is rtnl_lock > {lock, mii_lock} > window_lock.
    pub lock: spinlock_t,
    pub /: *mut *mut spinlock_t mii_lock; / Serialises access to MII,
    pub /: *mut *mut mii_if_info mii; / MII lib hooks/info,
    pub /: *mut *mut spinlock_t window_lock; / Serialises access to windowed regs,
    pub /: *mut *mut int window; / Register window,
}

#[no_mangle]
unsafe extern "C" fn window_set(vp: *mut vortex_private, window: c_int) {
    ...
    ((struct pci_dev *) (((vp).gendev) ? DEVICE_PCI((vp).gendev) : core::ptr::null_mut()))

    ((struct eisa_device *) (((vp).gendev) ? DEVICE_EISA((vp).gendev) : core::ptr::null_mut()))
...
pub const VORTEX_NUM_STATS: c_int = 5;
...
pub const MAX_UNITS: c_int = 8;
    ...
    let mut global_options: static int = -1;
    let mut global_full_duplex: static int = -1;
    let mut global_enable_wol: static int = -1;
    let mut global_use_mmio: static int = -1;
// Variables to work-around the Compaq PCI BIOS32 problem.
    ...
    module_param_array(hw_checksums, int, core::ptr::null_mut(), 0);
    module_param_array(flow_ctrl, int, core::ptr::null_mut(), 0);
    module_param(global_enable_wol, int, 0);
    module_param_array(enable_wol, int, core::ptr::null_mut(), 0);
    ...
    module_param_array(use_mmio, int, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn poll_vortex(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn vortex_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vortex_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vortex_eisa_probe(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vortex_eisa_remove(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vortex_eisa_init() -> int __init {
    static int __init vortex_eisa_init(void)
    {
    let mut eisa_found: c_int = 0;
    let mut orig_cards_found: c_int = vortex_cards_found;
...
// Special code to work-around the Compaq PCI BIOS32 problem.
    if (compaq_ioaddr) {
    vortex_probe1(core::ptr::null_mut(), ioport_map(compaq_ioaddr, VORTEX_TOTAL_SIZE),
...
//
    ...
    let mut vci: *mut vortex_chip_info  const = &vortex_info_tbl[chip_idx];
    const char *print_name = "3c59x";
    struct pci_dev *pdev = core::ptr::null_mut();
    struct eisa_device *edev = core::ptr::null_mut();
    if (!printed_version) {
    pr_info("%s", version);
    printed_version = 1;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(MAX_UNITS: card_idx <) -> else {
...
// Read the station address from the EEPROM.
    ...
    base = 0x230;
#[no_mangle]
pub unsafe extern "C" fn if(EEPROM_OFFSET: vci->drv_flags &) -> else {
...
//
    ...
    phyx = 24;
#[no_mangle]
pub unsafe extern "C" fn if(24: phy <=) -> else {
    else if (phy <= 24)
    phyx = phy - 1;
    else
    phyx = phy;
...
#[no_mangle]
unsafe extern "C" fn vortex_check_media(dev: *mut net_device, init: c_uint) {
    static void vortex_check_media(struct net_device *dev, unsigned int init)
    {
    struct vortex_private *vp = netdev_priv(dev);
    let mut ok_to_print: c_uint = 0;
    if (vortex_debug > 3)
    ok_to_print = 1;
...
// Set the station address and mask in window 2 each time opened.
    ...
    if (vp.cb_fn_base) {
    let mut n: c_ushort = window_read16(vp, 2, Wn2_ResetOptions) & ~0x4010;
    if (vp.drv_flags & INVERT_LED_PWR)
    n |= 0x10;
    if (vp.drv_flags & INVERT_MII_PWR)
    n |= 0x4000;
...
// Clear the Rx, Tx rings.
    ...
    vp.tx_skbuff[i] = core::ptr::null_mut();
...
// Wrap the ring.
    ...
    vp.rx_skbuff[i] = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn vortex_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void vortex_tx_timeout(struct net_device *dev, unsigned int txqueue)
    {
    struct vortex_private *vp = netdev_priv(dev);
    void __iomem *ioaddr = vp.ioaddr;
...
//
    ...
    struct vortex_private *vp = netdev_priv(dev);
    void __iomem *ioaddr = vp.ioaddr;
    let mut do_tx_reset: c_int = 0, reset_mask = 0;
    let mut tx_status: c_uchar = 0;
    if (vortex_debug > 2) {
...
// Adapter failure requires Tx/Rx reset and reinit.
    if (vp.full_bus_master_tx) {
    let mut bus_status: c_int = ioread32(ioaddr + PktStatus);
...
// enable 802.1q VLAN tagged frames
    ...
    struct vortex_private *vp = netdev_priv(dev);
    void __iomem *ioaddr = vp.ioaddr;
    let mut skblen: c_int = skb.len;
// Put out the doubleword header...
    iowrite32(skb.len, ioaddr + TX_FIFO);
    if (vp.bus_master) {
// Set the bus-master controller to transfer the packet.
    let mut len: c_int = (skb.len + 3) & ~3;
...
// Calculate the next Tx descriptor entry.
    let mut entry: c_int = vp.cur_tx % TX_RING_SIZE;
    let mut skblen: c_int = skb.len;
...
//
    ...
    void __iomem *ioaddr;
    int status;
    let mut work_done: c_int = max_interrupt_work;
...
//
    ...
    let mut dirty_tx: c_uint = vp.dirty_tx;
    iowrite16(AckIntr | DownComplete, ioaddr + EL3_CMD);
    while (vp.cur_tx - dirty_tx > 0) {
    let mut entry: c_int = dirty_tx % TX_RING_SIZE;
    ...
    vp.tx_skbuff[entry] = core::ptr::null_mut();
    } else {
...
#[no_mangle]
unsafe extern "C" fn vortex_rx(dev: *mut net_device) -> c_int {
    static int vortex_rx(struct net_device *dev)
    {
    struct vortex_private *vp = netdev_priv(dev);
    void __iomem *ioaddr = vp.ioaddr;
    ...
    let mut rx_error: c_uchar = ioread8(ioaddr + RxErrors);
    if (vortex_debug > 2)
...
// The packet length: up to 4.5K!.
    let mut pkt_len: c_int = rx_status & 0x1fff;
    struct sk_buff *skb;
...
// Wait a limited time to go to next packet.
    ...
    {
    struct vortex_private *vp = netdev_priv(dev);
    let mut entry: c_int = vp.cur_rx % RX_RING_SIZE;
    void __iomem *ioaddr = vp.ioaddr;
    int rx_status;
    let mut rx_work_limit: c_int = RX_RING_SIZE;
    if (vortex_debug > 5)
    ...
    let mut rx_error: c_uchar = rx_status >> 16;
    if (vortex_debug > 2)
    pr_debug(" Rx error: status %2.2x.\n", rx_error);
    dev.stats.rx_errors++;
    if (rx_error & 0x01)  dev.stats.rx_over_errors++;
...
// The packet length: up to 4.5K!.
    ...
    dma_addr_t newdma;
    let mut dma: dma_addr_t = le32_to_cpu(vp.rx_ring[entry].addr);
    if (vortex_debug > 4)
    pr_debug("Receiving packet size %d status %4.4x.\n",
    pkt_len, rx_status);
...
// Pass up the skbuff already on the Rx ring.
    ...
    let mut csum_bits: c_int = rx_status & 0xee000000;
    if (csum_bits &&
...
//
#[no_mangle]
unsafe extern "C" fn update_stats(ioaddr: *mut void __iomem, dev: *mut net_device) {
    static void update_stats(void __iomem *ioaddr, struct net_device *dev)
    {
    struct vortex_private *vp = netdev_priv(dev);
...
// Extra stats for get_ethtool_stats()
    ...
    {
    let mut up: u8 = window_read8(vp, 4, 13);
    dev.stats.rx_bytes += (up & 0x0f) << 16;
    dev.stats.tx_bytes += (up & 0xf0) << 12;
    }
    }
#[no_mangle]
unsafe extern "C" fn vortex_nway_reset(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vortex_get_msglevel(dev: *mut net_device) -> u32 {
    static u32 vortex_get_msglevel(struct net_device *dev)
    {
    return vortex_debug;
    }
#[no_mangle]
unsafe extern "C" fn vortex_set_msglevel(dev: *mut net_device, dbg: u32) {
    static void vortex_set_msglevel(struct net_device *dev, u32 dbg)
    {
    vortex_debug = dbg;
    }
#[no_mangle]
unsafe extern "C" fn vortex_get_sset_count(dev: *mut net_device, sset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vortex_get_strings(dev: *mut net_device, stringset: u32, data: *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn vortex_get_wol(dev: *mut net_device, wol: *mut ethtool_wolinfo) {
...
#[no_mangle]
unsafe extern "C" fn vortex_set_wol(dev: *mut net_device, wol: *mut ethtool_wolinfo) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vortex_ioctl(dev: *mut net_device, rq: *mut ifreq, cmd: c_int) -> c_int {
    static int vortex_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
    {
    int err;
    struct vortex_private *vp = netdev_priv(dev);
    let mut state: pci_power_t = 0;
    if(VORTEX_PCI(vp))
    state = VORTEX_PCI(vp).current_state;
// The kernel core really should have pci_get_power_state()
    if(state != 0)
    pci_set_power_state(VORTEX_PCI(vp), PCI_D0);
    err = generic_mii_ioctl(&vp.mii, if_mii(rq), cmd, core::ptr::null_mut());
    if(state != 0)
...
pub const VLAN_ETHER_TYPE: c_uint = 0x8100;
#[no_mangle]
unsafe extern "C" fn set_8021q_mode(dev: *mut net_device, enable: c_int) {
    static void set_8021q_mode(struct net_device *dev, int enable)
    {
    struct vortex_private *vp = netdev_priv(dev);
    int mac_ctrl;
...
// The maximum data clock rate is 2.5 Mhz.  The minimum timing is usually
    ...
    "overclocking" issues. */
#[no_mangle]
unsafe extern "C" fn mdio_delay(vp: *mut vortex_private) {
    static void mdio_delay(struct vortex_private *vp)
    {
    window_read32(vp, 4, Wn4_PhysicalMgmt);
    }
pub const MDIO_SHIFT_CLK: c_uint = 0x01;
pub const MDIO_DIR_WRITE: c_uint = 0x04;

pub const MDIO_DATA_READ: c_uint = 0x02;
pub const MDIO_ENB_IN: c_uint = 0x00;
// Generate the preamble required for initial synchronization and
    a few older transceivers. */
#[no_mangle]
unsafe extern "C" fn mdio_sync(vp: *mut vortex_private, bits: c_int) {
    static void mdio_sync(struct vortex_private *vp, int bits)
    {
...
unsafe extern "C" fn mdio_read(dev: *mut net_device, phy_id: c_int, location: c_int) -> c_int {
    ...
    int i;
    struct vortex_private *vp = netdev_priv(dev);
    let mut read_cmd: c_int = (0xf6 << 10) | (phy_id << 5) | location;
...
// Shift the read command bits out.
    for (i = 14; i >= 0; i--) {
    let mut dataval: c_int = (read_cmd&(1<<i)) ? MDIO_DATA_WRITE1 : MDIO_DATA_WRITE0;
...
unsafe extern "C" fn mdio_write(dev: *mut net_device, phy_id: c_int, location: c_int, value: c_int) {
    ...
    {
    struct vortex_private *vp = netdev_priv(dev);
    let mut write_cmd: c_int = 0x50020000 | (phy_id << 23) | (location << 18) | value;
    int i;
    spin_lock_bh(&vp.mii_lock);
    if (mii_preamble_required)
    mdio_sync(vp, 32);
// Shift the command bits out.
    for (i = 31; i >= 0; i--) {
    let mut dataval: c_int = (write_cmd&(1<<i)) ? MDIO_DATA_WRITE1 : MDIO_DATA_WRITE0;
    window_write16(vp, dataval, 4, Wn4_PhysicalMgmt);
    mdio_delay(vp);
    window_write16(vp, dataval | MDIO_SHIFT_CLK,
    4, Wn4_PhysicalMgmt);
...
#[no_mangle]
unsafe extern "C" fn acpi_set_WOL(dev: *mut net_device) {
    static void acpi_set_WOL(struct net_device *dev)
    {
    struct vortex_private *vp = netdev_priv(dev);
    void __iomem *ioaddr = vp.ioaddr;
...
#[no_mangle]
unsafe extern "C" fn vortex_remove_one(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn vortex_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn vortex_eisa_cleanup() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn vortex_cleanup() -> void __exit {
