...
//! Source: drivers/net/ethernet/hisilicon/hns3/hns3vf/hclgevf_mbx.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn hclgevf_resp_to_errno(resp_code: u16) -> c_int {
    static int hclgevf_resp_to_errno(u16 resp_code)
    {
    return resp_code ? -resp_code : 0;
    }
pub const HCLGEVF_MBX_MATCH_ID_START: c_int = 1;
#[no_mangle]
unsafe extern "C" fn hclgevf_reset_mbx_resp_status(hdev: *mut hclgevf_dev) {
...
//
    ...
    {
pub const HCLGEVF_MAX_TRY_TIMES: c_int = 500;
pub const HCLGEVF_SLEEP_USECOND: c_int = 1000;
...
// ensure additional_info will be seen after received_resp
    ...
    if (!send_msg) {
    dev_err(&hdev.pdev.dev,
    "failed to send mbx, msg is core::ptr::null_mut()\n");
    return -EINVAL;
    }
...
#[no_mangle]
unsafe extern "C" fn hclgevf_cmd_crq_empty(hw: *mut hclgevf_hw) -> bool {
    static bool hclgevf_cmd_crq_empty(struct hclgevf_hw *hw)
    {
    let mut tail: u32 = hclgevf_read_dev(hw, HCLGE_COMM_NIC_CRQ_TAIL_REG);
    let mut tail: return = = (u32)hw.hw.cmq.crq.next_to_use;
    }
    static void hclgevf_handle_mbx_response(struct hclgevf_dev *hdev,
    struct hclge_mbx_pf_to_vf_cmd *req)
    {
    let mut vf_mbx_msg_subcode: u16 = le16_to_cpu(req.msg.vf_mbx_msg_subcode);
    let mut vf_mbx_msg_code: u16 = le16_to_cpu(req.msg.vf_mbx_msg_code);
    struct hclgevf_mbx_resp_status *resp = &hdev.mbx_resp;
    let mut resp_status: u16 = le16_to_cpu(req.msg.resp_status);
    let mut match_id: u16 = le16_to_cpu(req.match_id);
...
#[no_mangle]
pub unsafe extern "C" fn hclgevf_mbx_handler(hdev: *mut hclgevf_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn hclgevf_mbx_async_handler(hdev: *mut hclgevf_dev) {
