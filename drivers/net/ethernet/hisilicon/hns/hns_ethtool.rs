...
//! Source: drivers/net/ethernet/hisilicon/hns/hns_ethtool.c
#![no_std]
...
pub const HNS_PHY_PAGE_MDIX: c_int = 0;
pub const HNS_PHY_PAGE_LED: c_int = 3;
pub const HNS_PHY_PAGE_COPPER: c_int = 0;

pub const HNS_LED_FORCE_ON: c_int = 9;
pub const HNS_LED_FORCE_OFF: c_int = 8;
pub const HNS_CHIP_VERSION: c_int = 660;
pub const HNS_NET_STATS_CNT: c_int = 26;
...
//
#[no_mangle]
unsafe extern "C" fn hns_nic_get_link(net_dev: *mut net_device) -> u32 {
    static u32 hns_nic_get_link(struct net_device *net_dev)
    {
    struct hns_nic_priv *priv = netdev_priv(net_dev);
    let mut link_stat: u32 = priv.link;
    ...
    cmd.base.eth_tp_mdix = ETH_TP_MDI_INVALID;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mdix) -> else {
    else if (mdix)
...
//
    ...
    return -ESRCH;
    ret = h.dev.ops.get_info(h, core::ptr::null_mut(), &speed, &duplex);
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn hns_nic_config_phy_loopback(phy_dev: *mut phy_device, en: u8) -> c_int {
...
//
    ...
    {
pub const NIC_LB_TEST_WAIT_PHY_LINK_TIME: c_int = 300;
    struct hns_nic_priv *priv = netdev_priv(ndev);
    struct hnae_handle *h = priv.ae_handle;
    int speed, duplex;
    int ret;
...
// for mutl buffer
    ...
    let mut rx_packets: c_ulong = ndev.stats.rx_packets;
    let mut rx_bytes: c_ulong = ndev.stats.rx_bytes;
    let mut rx_frame_errors: c_ulong = ndev.stats.rx_frame_errors;
...
//
    ...
    enum hnae_loop loop_mode)
    {
pub const NIC_LB_TEST_PKT_NUM_PER_CYCLE: c_int = 1;
pub const NIC_LB_TEST_RING_ID: c_int = 0;
pub const NIC_LB_TEST_FRAME_SIZE: c_int = 128;
// nic loopback test err
pub const NIC_LB_TEST_NO_MEM_ERR: c_int = 1;
pub const NIC_LB_TEST_TX_CNT_ERR: c_int = 2;
pub const NIC_LB_TEST_RX_CNT_ERR: c_int = 3;
    struct hns_nic_priv *priv = netdev_priv(ndev);
    struct hnae_handle *h = priv.ae_handle;
...
// place data into test skb
    ...
    skb.dev = ndev;
    __lb_other_process(core::ptr::null_mut(), skb);
...
#[no_mangle]
unsafe extern "C" fn __lb_down(ndev: *mut net_device, loop: enum hnae_loop) -> c_int {
...
//
    ...
    {
    struct hns_nic_priv *priv = netdev_priv(ndev);
    let mut if_running: bool = netif_running(ndev);
pub const SELF_TEST_TPYE_NUM: c_int = 3;
    int st_param[SELF_TEST_TPYE_NUM][2];
    int i;
    let mut test_index: c_int = 0;
    ...
    if (if_running)
    (void)dev_open(ndev, core::ptr::null_mut());
    }
...
//
#[no_mangle]
unsafe extern "C" fn hns_get_strings(netdev: *mut net_device, stringset: u32, data: *mut u8) {
...
//
#[no_mangle]
unsafe extern "C" fn hns_get_sset_count(netdev: *mut net_device, stringset: c_int) -> c_int {
    ...
    if (stringset == ETH_SS_TEST) {
    let mut cnt: u32 = ARRAY_SIZE(hns_nic_test_strs);
    if (priv.ae_handle.phy_if == PHY_INTERFACE_MODE_XGMII)
    cnt--;
    if ((!netdev.phydev) || (netdev.phydev.is_c45))
    cnt--;
...
//
#[no_mangle]
unsafe extern "C" fn hns_phy_led_set(netdev: *mut net_device, value: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hns_get_regs_len(net_dev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hns_nic_nway_reset(netdev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hns_get_rx_ring_count(netdev: *mut net_device) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn hns_ethtool_set_ops(ndev: *mut net_device) {
