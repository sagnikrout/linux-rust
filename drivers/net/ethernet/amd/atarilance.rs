...
//! Source: drivers/net/ethernet/amd/atarilance.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn access(CSR0: keep AREG pointing to) -> optimized register {
...
#[derive(Copy, Clone)]
pub struct ringdesc {
    pub /: *mut *mut unsigned short adr_lo; / Low 16 bits of address,
    pub /: *mut *mut unsigned char len; / Length bits,
    pub /: *mut *mut unsigned char adr_hi; / High 8 bits of address (unused),
}
...
pub struct lance_init_block {
    ...
    pub /: *mut *mut unsigned filter[2]; / Multicast filter (unused).,
// Receive and transmit ring base, along with length bits.
    pub rx_ring: ringdesc,
    pub tx_ring: ringdesc,
}
...
pub struct lance_memory {
    ...
    pub the: *mut *mut char packet_area[]; / packet data follow after,
...
//
pub const RIEBL_RSVD_START: c_uint = 0xee70;
pub const RIEBL_RSVD_END: c_uint = 0xeec0;
pub const RIEBL_MAGIC: c_uint = 0x09051990;
...
pub struct lance_private {
    pub cardtype: enum lance_type,
    pub iobase: *mut lance_ioreg,
    pub mem: *mut lance_memory,
    pub /: *mut *mut int cur_rx, cur_tx; / The next free ring entry,
    pub /: *mut *mut int dirty_tx; / Ring entries to be freed.,
// copy function
    pub ): *const *const *const *const *const void (memcpy_f)( void , void , size_t,
...
#[no_mangle]
unsafe extern "C" fn atarilance_probe() -> *mut net_device  __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(RIEBL_MAGIC: *mut *mut RIEBL_MAGIC_ADDR ==) -> else {
...
// For VME-RieblCards, request a free VME int
    let mut irq: c_uint = atari_register_vme_int();
    if (!irq) {
...
#[no_mangle]
pub unsafe extern "C" fn if(1): lp->cardtype == PAM_CARD && (len &) -> else {
...
// Acknowledge all of the current interrupt sources ASAP.
    ...
    let mut dirty_tx: c_int = lp.dirty_tx;
    while( dirty_tx < lp.cur_tx) {
    let mut entry: c_int = dirty_tx & TX_RING_MOD_MASK;
    let mut status: c_int = MEM.tx_head[entry].flag;
    if (status & TMD1_OWN_CHIP)
    break;			/* It still hasn't been Txed */
    MEM.tx_head[entry].flag = 0;
    if (status & TMD1_ERR) {
// There was an major error, log it.
    let mut err_status: c_int = MEM.tx_head[entry].misc;
    dev.stats.tx_errors++;
...
unsafe extern "C" fn lance_rx(dev: *mut net_device) -> c_int {
    ...
    {
    struct lance_private *lp = netdev_priv(dev);
    let mut entry: c_int = lp.cur_rx & RX_RING_MOD_MASK;
    int i;
...
// Malloc up new buffer, compatible with net-3.
    let mut pkt_len: c_short = head.msg_length & 0xfff;
    struct sk_buff *skb;
    if (pkt_len < 60) {
...
#[no_mangle]
unsafe extern "C" fn lance_set_mac_address(dev: *mut net_device, addr: *mut c_void) -> c_int {
    static int lance_set_mac_address( struct net_device *dev, void *addr )
    {
    struct lance_private *lp = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn atarilance_module_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn atarilance_module_exit() -> void __exit {
