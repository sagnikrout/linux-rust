...
//! Source: drivers/net/ethernet/amd/sunlance.c
#![no_std]
...
pub const LE_R1_OWN: c_uint = 0x80    /* Who owns the entry */;
pub const LE_R1_ERR: c_uint = 0x40    /* Error: if FRA, OFL, CRC or BUF is set */;
pub const LE_R1_FRA: c_uint = 0x20    /* FRA: Frame error */;
pub const LE_R1_OFL: c_uint = 0x10    /* OFL: Frame overflow */;
pub const LE_R1_CRC: c_uint = 0x08    /* CRC error */;
pub const LE_R1_BUF: c_uint = 0x04    /* BUF: Buffer error */;
...
pub const LE_T1_OWN: c_uint = 0x80    /* Lance owns the packet */;
pub const LE_T1_ERR: c_uint = 0x40    /* Error summary */;
pub const LE_T1_EMORE: c_uint = 0x10    /* Error: more than one retry needed */;
pub const LE_T1_EONE: c_uint = 0x08    /* Error: one retry needed */;
pub const LE_T1_EDEF: c_uint = 0x04    /* Error: deferred */;
...
pub struct lance_rx_desc {
    pub /: *mut *mut u16 rmd0; / low address of packet,
    pub /: *mut *mut u8 rmd1_bits; / descriptor bits,
    pub /: *mut *mut u8 rmd1_hadr; / high address of packet,
    pub (negative)!: *mut *mut s16 length; / This length is 2s complement,
// Buffer length
//
    pub /: *mut *mut u16 mblength; / This is the actual number of bytes received,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct lance_tx_desc {
    pub /: *mut *mut u16 tmd0; / low address of packet,
    pub /: *mut *mut u8 tmd1_bits; / descriptor bits,
    pub /: *mut *mut u8 tmd1_hadr; / high address of packet,
    pub /: *mut *mut s16 length; / Length is 2s complement (negative)!,
    pub misc: u16,
}
...
pub struct lance_init_block {
    pub /: *mut *mut u16 mode; / Pre-set mode (reg. 15),
    pub /: *mut *mut u8 phys_addr[6]; / Physical ethernet address,
    pub /: *mut *mut u32 filter[2]; / Multicast filter.,
// Receive and transmit ring base, along with extra bits.
    pub /: *mut *mut u16 rx_ptr; / receive descriptor addr,
    pub /: *mut *mut u16 rx_len; / receive len and high addr,
    pub /: *mut *mut u16 tx_ptr; / transmit descriptor addr,
    pub /: *mut *mut u16 tx_len; / transmit len and high addr,
// The Tx and Rx ring entries must aligned on 8-byte boundaries.
    pub brx_ring: [lance_rx_desc; RX_RING_SIZE],
    pub btx_ring: [lance_tx_desc; TX_RING_SIZE],
    pub [TX_RING_SIZE][TX_BUFF_SIZE]: u8 tx_buf,
    pub /: *mut *mut u8 pad[2]; / align rx_buf for copy_and_sum().,
    pub [RX_RING_SIZE][RX_BUFF_SIZE]: u8 rx_buf,
}
...
pub struct lance_private {
    pub /: *mut *mut *mut void __iomem lregs; / Lance RAP/RDP regs.,
    pub /: *mut *mut *mut void __iomem dregs; / DMA controller regs.,
    pub init_block_iomem: *mut lance_init_block __iomem,
    pub init_block_mem: *mut lance_init_block,
    pub lock: spinlock_t,
    pub tx_new: int rx_new,,
    pub tx_old: int rx_old,,
    pub /: *mut *mut *mut platform_device ledma; / If set this points to ledma,
    pub /: *mut *mut char tpe; / cable-selection is TPE,
    pub /: *mut *mut char auto_select; / cable-selection by carrier,
    pub /: *mut *mut char burst_sizes; / ledma SBus burst sizes,
    pub /: *mut *mut char pio_buffer; / init block in PIO space?,
    pub busmaster_regval: c_ushort,
    pub ): *mut *mut void (init_ring)(struct net_device,
    pub ): *mut *mut void (rx)(struct net_device,
    pub ): *mut *mut void (tx)(struct net_device,
    pub name: *mut c_char,
    pub init_block_dvma: dma_addr_t,
    pub /: *mut *mut *mut net_device dev; / Backpointer,
    pub op: *mut platform_device,
    pub lebuffer: *mut platform_device,
...
// Lance registers.
pub const RDP: c_uint = 0x00UL		/* register data port		*/;
pub const RAP: c_uint = 0x02UL		/* register address port	*/;
pub const LANCE_REG_SIZE: c_uint = 0x04UL;

    do {	void __iomem *__base = (__lp).lregs; \
    sbus_writew(LE_CSR0,	__base + RAP); \
    sbus_writew(LE_C0_STOP,	__base + RDP); \
    } while (0)
    let mut sparc_lance_debug: c_int = 2;
...
#[no_mangle]
unsafe extern "C" fn lance_init_ring_dvma(dev: *mut net_device) {
    static void lance_init_ring_dvma(struct net_device *dev)
    {
    struct lance_private *lp = netdev_priv(dev);
    struct lance_init_block *ib = lp.init_block_mem;
    let mut aib: dma_addr_t = lp.init_block_dvma;
    __u32 leptr;
    int i;
...
#[no_mangle]
unsafe extern "C" fn lance_init_ring_pio(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn init_restart_ledma(lp: *mut lance_private) {
    static void init_restart_ledma(struct lance_private *lp)
    {
    let mut csr: u32 = sbus_readl(lp.dregs + DMA_CSR);
...
#[no_mangle]
unsafe extern "C" fn lance_rx_dvma(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn lance_tx_dvma(dev: *mut net_device) {
    ...
    let mut bits: u8 = td.tmd1_bits;
// If we hit a packet not owned by us, stop
    if (bits & LE_T1_OWN)
    break;
    if (bits & LE_T1_ERR) {
    let mut status: u16 = td.misc;
    dev.stats.tx_errors++;
...
#[no_mangle]
unsafe extern "C" fn lance_piocopy_to_skb(skb: *mut sk_buff, piobuf: *mut void __iomem, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn lance_rx_pio(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn lance_tx_pio(dev: *mut net_device) {
    ...
    let mut bits: u8 = sbus_readb(&td.tmd1_bits);
// If we hit a packet not owned by us, stop
    if (bits & LE_T1_OWN)
    break;
    if (bits & LE_T1_ERR) {
    let mut status: u16 = sbus_readw(&td.misc);
    dev.stats.tx_errors++;
    if (status & LE_T3_RTY)  dev.stats.tx_aborted_errors++;
    if (status & LE_T3_LCOL) dev.stats.tx_window_errors++;
    if (status & LE_T3_CLOS) {
...
// Clear the error condition
    ...
    if (csr0 & LE_C0_MERR) {
    if (lp.dregs) {
    let mut addr: u32 = sbus_readl(lp.dregs + DMA_ADDR);
    ...
    if (lp.dregs) {
    let mut dma_csr: u32 = sbus_readl(lp.dregs + DMA_CSR);
...
#[no_mangle]
unsafe extern "C" fn build_fake_packet(lp: *mut lance_private) {
...
// On the 4m, setup the ledma to provide the upper bits for buffers
    if (lp.dregs) {
    let mut regval: u32 = lp.init_block_dvma & 0xff000000;
...
#[no_mangle]
unsafe extern "C" fn lance_reset(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lance_piocopy_from_skb(dest: *mut void __iomem, src: *mut c_uchar, len: c_int) {
    ...
    p16 = (u16 *) src;
    while (len >= 4) {
    let mut val: u32 = p16[0]<<16 | p16[1];
    sbus_writel(val, piobuf);
    p16 += 2;
    piobuf += 4;
    len -= 4;
...
#[no_mangle]
unsafe extern "C" fn lance_piozero(dest: *mut void __iomem, len: c_int) {
...
// Add addresses
    ...
    let mut tmp: u16 = sbus_readw(&mcast_table[crc>>4]);
    tmp |= 1 << (crc & 0xf);
...
#[no_mangle]
unsafe extern "C" fn lance_free_hwresources(lp: *mut lance_private) {
...
// Ethtool support...
#[no_mangle]
unsafe extern "C" fn sparc_lance_get_drvinfo(dev: *mut net_device, info: *mut ethtool_drvinfo) {
...
// Get the cable-selection property
    prop = of_get_property(ledma_dp, "cable-selection", core::ptr::null_mut());
    if (!prop || prop[0] == '\0') {
    ...
    if (!nd)
    goto no_link_test;
    prop = of_get_property(nd, "tpe-link-test?", core::ptr::null_mut());
    if (!prop)
    goto node_put;
    if (strcmp(prop, "true")) {
...
#[no_mangle]
unsafe extern "C" fn sunlance_sbus_probe(op: *mut platform_device) -> c_int {
    ...
    err = sparc_lance_probe_one(op, parent, core::ptr::null_mut());
    } else if (of_node_name_eq(parent_dp, "lebuffer")) {
    err = sparc_lance_probe_one(op, core::ptr::null_mut(), parent);
    } else
    err = sparc_lance_probe_one(op, core::ptr::null_mut(), core::ptr::null_mut());
    return err;
    }
#[no_mangle]
unsafe extern "C" fn sunlance_sbus_remove(op: *mut platform_device) {
