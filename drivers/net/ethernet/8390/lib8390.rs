...
//! Source: drivers/net/ethernet/8390/lib8390.c
#![no_std]
...
// Macro flag: #define NS8390_CORE

// Macro flag: #define BUG_83C690
// These are the operational function interfaces to board-specific
    routines.
#[no_mangle]
pub unsafe extern "C" fn reset_8390(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn get_8390_hdr(dev: *mut net_device, hdr: *mut e8390_hdr, ring_page: c_int) {
    void get_8390_hdr(struct net_device *dev, struct e8390_hdr *hdr, int ring_page)
    Read the 4 byte, page aligned 8390 header. *If* there is a
    subsequent read, it will be of the rest of the packet.
#[no_mangle]
pub unsafe extern "C" fn block_input(dev: *mut net_device, count: c_int, skb: *mut sk_buff, ring_offset: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn __ei_open(dev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __ei_close(dev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __ei_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void __ei_tx_timeout(struct net_device *dev, unsigned int txqueue)
    {
    let mut e8390_base: c_ulong = dev.base_addr;
    struct ei_device *ei_local = netdev_priv(dev);
...
//
    ...
    let mut e8390_base: c_ulong = dev.base_addr;
    struct ei_device *ei_local = netdev_priv(dev);
    let mut send_length: c_int = skb.len, output_page;
    unsigned long flags;
    char buf[ETH_ZLEN];
    char *data = skb.data;
    if (skb.len < ETH_ZLEN) {
...
//
#[no_mangle]
unsafe extern "C" fn __ei_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
// rmk - acknowledge the interrupts
    ...
    ei_rx_overrun(dev);
#[no_mangle]
pub unsafe extern "C" fn if((ENISR_RX+ENISR_RX_ERR): interrupts &) -> else {
// Got a good (?) packet.
    ei_receive(dev);
    }
// Push the next to-transmit packet through.
    ...
    ei_tx_intr(dev);
#[no_mangle]
pub unsafe extern "C" fn if(ENISR_TX_ERR: interrupts &) -> else {
...
#[no_mangle]
unsafe extern "C" fn __ei_poll(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn ei_tx_err(dev: *mut net_device) {
    static void ei_tx_err(struct net_device *dev)
    {
    let mut e8390_base: c_ulong = dev.base_addr;
// ei_local is used on some platforms via the EI_SHIFT macro
    let mut __maybe_unused: *mut ei_device ei_local = netdev_priv(dev);
    let mut txsr: c_uchar = ei_inb_p(e8390_base+EN0_TSR);
    let mut tx_was_aborted: c_uchar = txsr & (ENTSR_ABT+ENTSR_FU);
...
//
#[no_mangle]
unsafe extern "C" fn ei_tx_intr(dev: *mut net_device) {
    static void ei_tx_intr(struct net_device *dev)
    {
    let mut e8390_base: c_ulong = dev.base_addr;
    struct ei_device *ei_local = netdev_priv(dev);
    let mut status: c_int = ei_inb(e8390_base + EN0_TSR);
...
//
#[no_mangle]
unsafe extern "C" fn ei_receive(dev: *mut net_device) {
    static void ei_receive(struct net_device *dev)
    {
    let mut e8390_base: c_ulong = dev.base_addr;
    struct ei_device *ei_local = netdev_priv(dev);
    ...
    let mut rx_pkt_count: c_int = 0;
    struct e8390_pkt_hdr rx_frame;
    let mut num_rx_pages: c_int = ei_local.stop_page-ei_local.rx_start_page;
...
//
#[no_mangle]
unsafe extern "C" fn ei_rx_overrun(dev: *mut net_device) {
    static void ei_rx_overrun(struct net_device *dev)
    {
    let mut e8390_base: c_ulong = dev.base_addr;
...
//
    if (was_txing) {
    let mut tx_completed: c_uchar = ei_inb_p(e8390_base+EN0_ISR) & (ENISR_TX+ENISR_TX_ERR);
    if (!tx_completed)
    must_resend = 1;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn make_mc_bits(bits: *mut u8, dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn do_set_multicast_list(dev: *mut net_device) {
    static void do_set_multicast_list(struct net_device *dev)
    {
    let mut e8390_base: c_ulong = dev.base_addr;
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(!netdev_mc_empty(dev): dev->flags & IFF_ALLMULTI ||) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn __ei_set_multicast_list(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn ethdev_setup(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn __NS8390_init(dev: *mut net_device, startp: c_int) {
    static void __NS8390_init(struct net_device *dev, int startp)
    {
    let mut e8390_base: c_ulong = dev.base_addr;
    struct ei_device *ei_local = netdev_priv(dev);
