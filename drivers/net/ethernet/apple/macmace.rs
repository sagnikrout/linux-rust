...
//! Source: drivers/net/ethernet/apple/macmace.c
#![no_std]
...
pub const N_TX_BUFF_ORDER: c_int = 0;

pub const N_RX_BUFF_ORDER: c_int = 3;

pub const MACE_BUFF_SIZE: c_uint = 0x800;
// Chip rev needs workaround on HW & multicast addr change
pub const BROKEN_ADDRCHG_REV: c_uint = 0x0941;
// The MACE is simply wired down on a Mac68K box

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mace_data {
    pub mace: *mut volatile struct mace,
    pub tx_ring: *mut c_uchar,
    pub tx_ring_phys: dma_addr_t,
    pub rx_ring: *mut c_uchar,
    pub rx_ring_phys: dma_addr_t,
    pub dma_intr: c_int,
    pub rx_tail: int rx_slot,,
    pub tx_count: int tx_slot, tx_sloti,,
    pub chipid: c_int,
...
//
#[no_mangle]
unsafe extern "C" fn mace_load_rxdma_base(dev: *mut net_device, set: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn mace_rxdma_reset(dev: *mut net_device) {
    static void mace_rxdma_reset(struct net_device *dev)
    {
    struct mace_data *mp = netdev_priv(dev);
    volatile struct mace *mace = mp.mace;
    let mut maccc: u8 = mace.maccc;
...
//
#[no_mangle]
unsafe extern "C" fn mace_txdma_reset(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn mace_dma_off(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn mace_probe(pdev: *mut platform_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mace_reset(dev: *mut net_device) {
    static void mace_reset(struct net_device *dev)
    {
    struct mace_data *mp = netdev_priv(dev);
...
//
#[no_mangle]
unsafe extern "C" fn __mace_set_address(dev: *mut net_device, addr: *const c_void) {
    static void __mace_set_address(struct net_device *dev, const void *addr)
    {
    struct mace_data *mp = netdev_priv(dev);
    volatile struct mace *mb = mp.mace;
...
#[no_mangle]
unsafe extern "C" fn mace_set_address(dev: *mut net_device, addr: *mut c_void) -> c_int {
    static int mace_set_address(struct net_device *dev, void *addr)
    {
    struct mace_data *mp = netdev_priv(dev);
    volatile struct mace *mb = mp.mace;
...
//
#[no_mangle]
unsafe extern "C" fn mace_open(dev: *mut net_device) -> c_int {
    static int mace_open(struct net_device *dev)
    {
    struct mace_data *mp = netdev_priv(dev);
    volatile struct mace *mb = mp.mace;
...
// Allocate the DMA ring buffers
    ...
    if (mp.tx_ring == core::ptr::null_mut())
    goto out1;
    mp.rx_ring = dma_alloc_coherent(mp.device,
    N_RX_RING * MACE_BUFF_SIZE,
    &mp.rx_ring_phys, GFP_KERNEL);
    if (mp.rx_ring == core::ptr::null_mut())
    goto out2;
    mace_dma_off(dev);
...
//
#[no_mangle]
unsafe extern "C" fn mace_close(dev: *mut net_device) -> c_int {
    static int mace_close(struct net_device *dev)
    {
    struct mace_data *mp = netdev_priv(dev);
    volatile struct mace *mb = mp.mace;
...
//
#[no_mangle]
unsafe extern "C" fn mace_xmit_start(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
    static netdev_tx_t mace_xmit_start(struct sk_buff *skb, struct net_device *dev)
    {
    struct mace_data *mp = netdev_priv(dev);
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn mace_set_multicast(dev: *mut net_device) {
    static void mace_set_multicast(struct net_device *dev)
    {
    struct mace_data *mp = netdev_priv(dev);
    volatile struct mace *mb = mp.mace;
...
#[no_mangle]
unsafe extern "C" fn mace_handle_misc_intrs(dev: *mut net_device, intr: c_int) {
...
#[no_mangle]
unsafe extern "C" fn mace_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t mace_interrupt(int irq, void *dev_id)
    {
...
#[no_mangle]
pub unsafe extern "C" fn if((UFLO|LCOL|RTRY): fs &) -> else {
...
#[no_mangle]
unsafe extern "C" fn mace_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
    static void mace_tx_timeout(struct net_device *dev, unsigned int txqueue)
    {
    struct mace_data *mp = netdev_priv(dev);
    volatile struct mace *mb = mp.mace;
...
//
#[no_mangle]
unsafe extern "C" fn mace_dma_rx_frame(dev: *mut net_device, mf: *mut mace_frame) {
    static void mace_dma_rx_frame(struct net_device *dev, struct mace_frame *mf)
    {
    struct sk_buff *skb;
    let mut frame_status: c_uint = mf.rcvsts;
    ...
    let mut frame_length: c_uint = mf.rcvcnt + ((frame_status & 0x0F) << 8 );
...
//
#[no_mangle]
unsafe extern "C" fn mace_dma_intr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t mace_dma_intr(int irq, void *dev_id)
    {
    struct net_device *dev = (struct net_device *) dev_id;
    struct mace_data *mp = netdev_priv(dev);
...
#[no_mangle]
unsafe extern "C" fn mac_mace_device_remove(pdev: *mut platform_device) {
