...
//! Source: drivers/net/ethernet/huawei/hinic/hinic_ethtool.c
#![no_std]
...
pub const SET_LINK_STR_MAX_LEN: c_int = 16;
pub const GET_SUPPORTED_MODE: c_int = 0;
pub const GET_ADVERTISED_MODE: c_int = 1;
...
pub const COALESCE_PENDING_LIMIT_UNIT: c_int = 8;
pub const COALESCE_TIMER_CFG_UNIT: c_int = 9;
pub const COALESCE_ALL_QUEUE: c_uint = 0xFFFF;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct hw2ethtool_link_mode {
    pub link_mode_bit: enum ethtool_link_mode_bit_indices,
    pub speed: u32,
    pub hw_link_mode: enum hinic_link_mode,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct cmd_link_settings {
    pub supported: u64,
    pub advertising: u64,
    pub speed: u32,
    pub duplex: u8,
...
pub const LP_PKT_LEN: c_int = 1514;
pub const PORT_DOWN_ERR_IDX: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn hinic_get_link_mode_index(link_mode: enum hinic_link_mode) -> c_int {
...
// link_ksettings)
    ...
    let mut link_mode: hinic_link_mode_cmd = { 0 };
    let mut pause_info: hinic_pause_config = { 0 };
    let mut settings: cmd_link_settings = { 0 };
...
#[no_mangle]
unsafe extern "C" fn hinic_ethtool_to_hw_speed_level(speed: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hinic_is_speed_legal(nic_dev: *mut hinic_dev, speed: u32) -> bool {
    static bool hinic_is_speed_legal(struct hinic_dev *nic_dev, u32 speed)
    {
    let mut link_mode: hinic_link_mode_cmd = { 0 };
    struct net_device *netdev = nic_dev.netdev;
    let mut speed_level: enum nic_speed_level = 0;
    int err;
    err = hinic_get_link_mode(nic_dev.hwdev, &link_mode);
    if (err)
    return false;
...
// set_settings |= HILINK_LINK_SET_SPEED;
    ...
    {
    let mut settings: hinic_link_ksettings_info = {0};
...
#[no_mangle]
unsafe extern "C" fn set_link_settings(netdev: *mut net_device, autoneg: u8, speed: u32) -> c_int {
    static int set_link_settings(struct net_device *netdev, u8 autoneg, u32 speed)
    {
    struct hinic_dev *nic_dev = netdev_priv(netdev);
    let mut set_settings: u32 = 0;
    int err;
...
#[no_mangle]
unsafe extern "C" fn is_coalesce_exceed_limit(coal: *const ethtool_coalesce) -> c_int {
    ...
    bool set_rx_coal)
    {
    struct hinic_intr_coal_info *intr_coal = core::ptr::null_mut();
    let mut interrupt_info: hinic_msix_config = {0};
    struct net_device *netdev = nic_dev.netdev;
    u16 msix_idx;
    int err;
...
//
    ...
    {
    struct hinic_dev *nic_dev = netdev_priv(netdev);
    let mut rx_intr_coal: hinic_intr_coal_info = {0};
    let mut tx_intr_coal: hinic_intr_coal_info = {0};
    let mut set_rx_coal: bool = false;
    let mut set_tx_coal: bool = false;
    int err;
...
//
    ...
    {
    struct hinic_dev *nic_dev = netdev_priv(netdev);
    let mut count: c_uint = channels.combined_count;
    int err;
    ...
    {
    struct hinic_dev *nic_dev = netdev_priv(netdev);
    let mut rss_type: hinic_rss_type = { 0 };
    int err;
    cmd.data = 0;
...
#[no_mangle]
unsafe extern "C" fn hinic_get_rx_ring_count(netdev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn hinic_get_rxfh_key_size(netdev: *mut net_device) -> u32 {
    static u32 hinic_get_rxfh_key_size(struct net_device *netdev)
    {
    return HINIC_RSS_KEY_SIZE;
    }
#[no_mangle]
unsafe extern "C" fn hinic_get_rxfh_indir_size(netdev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn get_drv_queue_stats(nic_dev: *mut hinic_dev, data: *mut u64) {
    static void get_drv_queue_stats(struct hinic_dev *nic_dev, u64 *data)
    {
    struct hinic_txq_stats txq_stats;
    struct hinic_rxq_stats rxq_stats;
    let mut i: u16 = 0, j = 0, qid = 0;
    char *p;
    ...
    {
    struct hinic_dev *nic_dev = netdev_priv(netdev);
    let mut vport_stats: hinic_vport_stats = {0};
...
#[no_mangle]
unsafe extern "C" fn hinic_get_sset_count(netdev: *mut net_device, sset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hinic_run_lp_test(nic_dev: *mut hinic_dev, test_time: u32) -> c_int {
    static int hinic_run_lp_test(struct hinic_dev *nic_dev, u32 test_time)
    {
    u8 *lb_test_rx_buf = nic_dev.lb_test_rx_buf;
    struct net_device *netdev = nic_dev.netdev;
    struct sk_buff *skb_tmp = core::ptr::null_mut();
    struct sk_buff *skb = core::ptr::null_mut();
    let mut cnt: u32 = test_time * 5;
    u8 *test_data = core::ptr::null_mut();
    u32 i;
    u8 j;
...
// (lb_test_rx_buf + j * LP_PKT_LEN +
    ...
    {
    struct net_device *netdev = nic_dev.netdev;
    u8 *lb_test_rx_buf = core::ptr::null_mut();
    let mut err: c_int = 0;
...
// test_index = EXTERNAL_LP_TEST;
    ...
    nic_dev.lb_test_rx_buf = core::ptr::null_mut();
    }
    if (!(flags & ETH_TEST_FL_EXTERNAL_LB)) {
    if (hinic_set_loopback_mode(nic_dev.hwdev,
    ...
    struct hinic_dev *nic_dev = netdev_priv(netdev);
    enum hinic_port_link_state link_state;
    let mut test_index: enum diag_test_index = 0;
    let mut err: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: nic_dev->module_unrecognized) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn hinic_set_ethtool_ops(netdev: *mut net_device) {
