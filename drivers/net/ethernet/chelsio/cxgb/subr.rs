...
//! Source: drivers/net/ethernet/chelsio/cxgb/subr.c
#![no_std]
...
//
    ...
    {
    while (1) {
    let mut val: u32 = readl(adapter.regs + reg) & mask;
    if (!!val == polarity)
    return 0;
    if (--attempts == 0)
    return 1;
...
pub const TPI_ATTEMPTS: c_int = 50;
//
// Write a register over the TPI interface (unlocked and locked versions).
//
#[no_mangle]
pub unsafe extern "C" fn __t1_tpi_write(adapter: *mut adapter_t, addr: u32, value: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn t1_tpi_write(adapter: *mut adapter_t, addr: u32, value: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn __t1_tpi_read(adapter: *mut adapter_t, addr: u32, valp: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn t1_tpi_read(adapter: *mut adapter_t, addr: u32, valp: *mut u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn t1_tpi_par(adapter: *mut adapter_t, value: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn t1_link_changed(adapter: *mut adapter_t, port_id: c_int) {
...
#[no_mangle]
unsafe extern "C" fn t1_pci_intr_handler(adapter: *mut adapter_t) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn fpga_phy_intr_handler(adapter: *mut adapter_t) -> c_int {
    static int fpga_phy_intr_handler(adapter_t *adapter)
    {
    int p;
    let mut cause: u32 = readl(adapter.regs + FPGA_GMAC_ADDR_INTERRUPT_CAUSE);
    for_each_port(adapter, p)
    if (cause & (1 << p)) {
    struct cphy *phy = adapter.port[p].phy;
    let mut phy_cause: c_int = phy.ops.interrupt_handler(phy);
...
//
#[no_mangle]
unsafe extern "C" fn fpga_slow_intr(adapter: *mut adapter_t) -> irqreturn_t {
    static irqreturn_t fpga_slow_intr(adapter_t *adapter)
    {
    let mut cause: u32 = readl(adapter.regs + A_PL_CAUSE);
...
//
    let mut tp_cause: u32 = readl(adapter.regs + FPGA_TP_ADDR_INTERRUPT_CAUSE);
// Clear TP interrupt
...
// Clear the interrupts just processed.
    ...
    if (ret != IRQ_NONE)
    return ret;
    let mut cause: return = = 0 ? IRQ_NONE : IRQ_HANDLED;
    }
...
//
#[no_mangle]
unsafe extern "C" fn mi1_wait_until_ready(adapter: *mut adapter_t, mi1_reg: c_int) -> c_int {
    static int mi1_wait_until_ready(adapter_t *adapter, int mi1_reg)
    {
    let mut attempts: c_int = 100, busy;
    do {
    u32 val;
...
//
#[no_mangle]
unsafe extern "C" fn mi1_mdio_init(adapter: *mut adapter_t, bi: *const board_info) {
    static void mi1_mdio_init(adapter_t *adapter, const struct board_info *bi)
    {
    let mut clkdiv: u32 = bi.clock_elmer0 / (2 * bi.mdio_mdc) - 1;
...
//
    ...
    let mut addr: u32 = V_MI1_REG_ADDR(reg_addr) | V_MI1_PHY_ADDR(phy_addr);
    unsigned int val;
    spin_lock(&adapter.tpi_lock);
    ...
    {
    struct adapter *adapter = dev.ml_priv;
    let mut addr: u32 = V_MI1_REG_ADDR(mmd_addr) | V_MI1_PHY_ADDR(phy_addr);
    unsigned int val;
    spin_lock(&adapter.tpi_lock);
...
//
    ...
    {
    return board_id < ARRAY_SIZE(t1_board) ? &t1_board[board_id] : core::ptr::null_mut();
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct chelsio_vpd_t {
    pub format_version: u32,
    pub serial_number: [u8; 16],
    pub mac_base_address: [u8; 6],
    pub /: *mut *mut u8 pad[2]; / make multiple-of-4 size requirement explicit,
}

pub const EEPROM_MAX_POLL: c_int = 4;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn t1_seeprom_read(adapter: *mut adapter_t, addr: u32, data: *mut __le32) -> c_int {
    int t1_seeprom_read(adapter_t *adapter, u32 addr, __le32 *data)
    {
    let mut i: c_int = EEPROM_MAX_POLL;
    u16 val;
    u32 v;
...
#[no_mangle]
unsafe extern "C" fn t1_eeprom_vpd_get(adapter: *mut adapter_t, vpd: *mut chelsio_vpd_t) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vpd_macaddress_get(adapter: *mut adapter_t, index: c_int, mac_addr[]: u8) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn t1_link_start(phy: *mut cphy, mac: *mut cmac, lc: *mut link_config) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn t1_elmer0_ext_intr_handler(adapter: *mut adapter_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn t1_interrupts_enable(adapter: *mut adapter_t) {
...
// Enable PCIX & external chip interrupts on ASIC boards.
    ...
    let mut pl_intr: u32 = readl(adapter.regs + A_PL_ENABLE);
// PCI-X interrupts
...
#[no_mangle]
pub unsafe extern "C" fn t1_interrupts_disable(adapter: *mut *mut adapter_t) {
...
#[no_mangle]
pub unsafe extern "C" fn t1_interrupts_clear(adapter: *mut *mut adapter_t) {
...
// Enable interrupts for external devices.
    if (t1_is_asic(adapter)) {
    let mut pl_intr: u32 = readl(adapter.regs + A_PL_CAUSE);
...
//
#[no_mangle]
unsafe extern "C" fn asic_slow_intr(adapter: *mut adapter_t) -> irqreturn_t {
...
#[no_mangle]
pub unsafe extern "C" fn t1_slow_intr_handler(adapter: *mut adapter_t) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn power_sequence_xpak(adapter: *mut *mut adapter_t) {
...
// XPAK is present
    ...
    let mut val: u32 = readl(adapter.regs + A_TP_PC_CONFIG);
    val = G_TP_PC_REV(val);
    if (val == 2)
    p.chip_revision = TERM_T1B;
#[no_mangle]
pub unsafe extern "C" fn if(3: val ==) -> else {
    else if (val == 3)
...
//
#[no_mangle]
unsafe extern "C" fn board_init(adapter: *mut adapter_t, bi: *const board_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn t1_init_hw_modules(adapter: *mut adapter_t) -> c_int {
    int t1_init_hw_modules(adapter_t *adapter)
    {
    let mut err: c_int = -EIO;
    const struct board_info *bi = board_info(adapter);
    if (!bi.clock_mc4) {
    let mut val: u32 = readl(adapter.regs + A_MC4_CFG);
...
//
#[no_mangle]
unsafe extern "C" fn get_pci_mode(adapter: *mut adapter_t, p: *mut chelsio_pci_params) {
...
//
#[no_mangle]
pub unsafe extern "C" fn t1_free_sw_modules(adapter: *mut adapter_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn t1_init_sw_modules(adapter: *mut adapter_t, bi: *const board_info) -> c_int {
    ...
    u8 hw_addr[6];
    struct cmac *mac;
    let mut phy_addr: c_int = bi.mdio_phybaseaddr + i;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: vpd_macaddress_get(adapter, _arg: i, _arg: hw_addr)) -> else {
