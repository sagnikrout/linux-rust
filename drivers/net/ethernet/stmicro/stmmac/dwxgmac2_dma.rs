...
//! Source: drivers/net/ethernet/stmicro/stmmac/dwxgmac2_dma.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_dma_reset(ioaddr: *mut void __iomem) -> c_int {
    static int dwxgmac2_dma_reset(void __iomem *ioaddr)
    {
    let mut value: u32 = readl(ioaddr + XGMAC_DMA_MODE);
// DMA SW reset
    ...
    {
    let mut value: u32 = readl(ioaddr + XGMAC_DMA_SYSBUS_MODE);
    if (dma_cfg.aal)
    value |= XGMAC_AAL;
    if (dma_cfg.eame)
    value |= XGMAC_EAME;
...
//
    ...
    {
    let mut value: u32 = readl(ioaddr + XGMAC_DMA_CH_CONTROL(chan));
    if (dma_cfg.pblx8)
    value |= XGMAC_PBLx8;
    ...
    {
    let mut rxpbl: u32 = dma_cfg.rxpbl ?: dma_cfg.pbl;
    u32 value;
    ...
    dma_addr_t phy, u32 chan)
    {
    let mut txpbl: u32 = dma_cfg.txpbl ?: dma_cfg.pbl;
    u32 value;
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_dma_axi(ioaddr: *mut void __iomem, axi: *mut stmmac_axi) {
...
//
    ...
    {
    let mut value: u32 = readl(ioaddr + XGMAC_MTL_RXQ_OPMODE(channel));
    let mut rqs: c_uint = fifosz / 256 - 1;
    unsigned int rtc;
    if (mode == SF_DMA_MODE) {
    value |= XGMAC_RSF;
    } else {
...
pub unsafe extern "C" fn if(96: mode <=) -> else {
    ...
    let mut flow: u32 = readl(ioaddr + XGMAC_MTL_RXQ_FLOW_CONTROL(channel));
...
//
    ...
    int mode, u32 channel, int fifosz, u8 qmode)
    {
    let mut value: u32 = readl(ioaddr + XGMAC_MTL_TXQ_OPMODE(channel));
    let mut tqs: c_uint = fifosz / 256 - 1;
...
pub unsafe extern "C" fn if(256: mode <=) -> else {
    ...
    ttc = 0x5;
#[no_mangle]
pub unsafe extern "C" fn if(384: mode <=) -> else {
    else if (mode <= 384)
    ttc = 0x6;
    else
    ttc = 0x7;
...
// Use static TC to Queue mapping
    ...
    bool rx, bool tx)
    {
    let mut value: u32 = readl(ioaddr + XGMAC_DMA_CH_INT_EN(chan));
    if (rx)
    ...
    let mut intr_status: u32 = readl(ioaddr + XGMAC_DMA_CH_STATUS(chan));
    let mut intr_en: u32 = readl(ioaddr + XGMAC_DMA_CH_INT_EN(chan));
    let mut ret: c_int = 0;
    if (dir == DMA_DIR_RX)
...
#[no_mangle]
pub unsafe extern "C" fn if(10: dma_cap->l3l4fnum >) -> else {
...
// MAC HW feature 4
    ...
    bool en, u32 chan)
    {
    let mut value: u32 = readl(ioaddr + XGMAC_DMA_CH_TX_CONTROL(chan));
    if (en)
    value |= XGMAC_TSE;
    else
    value &= ~XGMAC_TSE;
    ...
    {
    let mut value: u32 = readl(ioaddr + XGMAC_MTL_TXQ_OPMODE(channel));
    let mut flow: u32 = readl(ioaddr + XGMAC_RX_FLOW_CTRL);
    unsigned int txqen;
    if (qmode != MTL_QUEUE_AVB) {
    txqen = 0x2;
