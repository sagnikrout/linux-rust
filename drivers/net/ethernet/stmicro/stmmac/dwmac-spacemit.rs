...
//! Source: drivers/net/ethernet/stmicro/stmmac/dwmac-spacemit.c
#![no_std]
...
pub const MAX_DLINE_DELAY_CODE: c_uint = 0xff;
pub const MAX_WORKED_DELAY: c_int = 2800;
// Note: the delay step value is at 0.1ps
pub const K3_DELAY_STEP: c_int = 367;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct spacmit_dwmac {
    pub apmu: *mut regmap,
    pub ctrl_offset: c_uint,
    pub dline_offset: c_uint,
}
...
#[no_mangle]
unsafe extern "C" fn spacemit_dwmac_detected_delay_value(delay: c_uint) -> c_int {
...
//
    ...
    {
    let mut val: c_uint = stmmac_res.wol_irq >= 0 ? CTRL_WAKE_IRQ_EN : 0;
    let mut mask: c_uint = CTRL_WAKE_IRQ_EN;
...
#[no_mangle]
unsafe extern "C" fn spacemit_set_phy_intf_sel(bsp_priv: *mut c_void, phy_intf_sel: u8) -> c_int {
    static int spacemit_set_phy_intf_sel(void *bsp_priv, u8 phy_intf_sel)
    {
    let mut mask: c_uint = CTRL_PHY_INTF_MODE;
...
#[no_mangle]
unsafe extern "C" fn spacemit_dwmac_probe(pdev: *mut platform_device) -> c_int {
