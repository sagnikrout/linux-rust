...
//! Source: drivers/net/ethernet/stmicro/stmmac/stmmac_tc.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn tc_fill_all_pass_entry(entry: *mut stmmac_tc_entry) {
    ...
    bool free)
    {
    struct stmmac_tc_entry *entry, *first = core::ptr::null_mut(), *dup = core::ptr::null_mut();
    let mut loc: u32 = cls.knode.handle;
    int i;
...
// Unsupported
    ...
    struct tc_cls_u32_offload *cls)
    {
    struct stmmac_tc_entry *entry, *frag = core::ptr::null_mut();
    struct tc_u32_sel *sel = cls.knode.sel;
    u32 off, data, mask, real_off, rem;
    let mut prio: u32 = cls.common.prio << 16;
    int ret;
// Only 1 match per entry
    ...
    entry.frag_ptr = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn tc_rfs_init(priv: *mut stmmac_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tc_init(priv: *mut stmmac_priv) -> c_int {
...
// Transport proto is present in the key and is not TCP or UDP
    ...
    struct flow_rule *rule = flow_cls_offload_flow_rule(cls);
    struct flow_dissector *dissector = rule.match.dissector;
    let mut inv: bool = entry.action & STMMAC_FLOW_ACTION_DROP;
...
pub unsafe extern "C" fn if(-EOPNOTSUPP: ret ==) -> else {
    ...
    struct flow_rule *rule = flow_cls_offload_flow_rule(cls);
    struct flow_dissector *dissector = rule.match.dissector;
    let mut tc: c_int = tc_classid_to_hwtc(priv.dev, cls.classid);
...
// Nothing to do here
    ...
    let mut etype: u16 = ntohs(match.key.n_proto);
...
// When RSS is enabled, the filtering will be bypassed
    ...
    {
    let mut num_tc: u32 = qopt.mqprio.qopt.num_tc;
...
//
    ...
    mutex_unlock(&priv.est_lock);
    for (i = 0; i < size; i++) {
    let mut delta_ns: i64 = qopt.entries[i].interval;
    let mut gates: u32 = qopt.entries[i].gate_mask;
...
// Transmission overrun doesn't happen for stmmac, hence always 0
    ...
    let mut queue: c_int = qopt.queue_stats.queue;
...
// Transmission overrun doesn't happen for stmmac, hence always 0
    ...
    let mut num_tc: u32 = qopt.num_tc;
    int err;
    if (!num_tc) {
