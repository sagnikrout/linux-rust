...
//! Source: drivers/net/ethernet/stmicro/stmmac/dwmac1000_core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_pcs_init(priv: *mut stmmac_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_rx_ipc_enable(hw: *mut mac_device_info) -> c_int {
    static int dwmac1000_rx_ipc_enable(struct mac_device_info *hw)
    {
    void __iomem *ioaddr = hw.pcsr;
    let mut value: u32 = readl(ioaddr + GMAC_CONTROL);
    if (hw.rx_csum)
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_dump_regs(hw: *mut mac_device_info, reg_space: *mut u32) {
    ...
    let mut perfect_addr_number: c_uint = hw.unicast_filter_entries;
    u32 mc_filter[8];
...
//
    let mut flow: c_uint = GMAC_FLOW_CTRL_UP;
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_pmt(hw: *mut mac_device_info, mode: c_ulong) {
    ...
    let mut intr_mask: u32 = readl(ioaddr + GMAC_INT_MASK);
    let mut ret: c_int = 0;
// Discard masked bits
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_set_eee_pls(hw: *mut mac_device_info, link: c_int) {
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_set_eee_timer(hw: *mut mac_device_info, ls: c_int, tw: c_int) {
    static void dwmac1000_set_eee_timer(struct mac_device_info *hw, int ls, int tw)
    {
    void __iomem *ioaddr = hw.pcsr;
    let mut value: c_int = ((tw & 0xffff)) | ((ls & 0x7ff) << 16);
...
//
    ...
    {
    let mut value: u32 = readl(ioaddr + GMAC_DEBUG);
    ...
    let mut trcsts: u32 = FIELD_GET(GMAC_DEBUG_TRCSTS_MASK, value);
    if (trcsts == GMAC_DEBUG_TRCSTS_WRITE)
    x.mtl_tx_fifo_read_ctrl_write++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_TRCSTS_TXW: trcsts ==) -> else {
    else if (trcsts == GMAC_DEBUG_TRCSTS_TXW)
    x.mtl_tx_fifo_read_ctrl_wait++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_TRCSTS_READ: trcsts ==) -> else {
...
pub unsafe extern "C" fn if(GMAC_DEBUG_TFCSTS_WAIT: tfcsts ==) -> else {
    ...
    let mut rxfsts: u32 = FIELD_GET(GMAC_DEBUG_RXFSTS_MASK, value);
    if (rxfsts == GMAC_DEBUG_RXFSTS_FULL)
    x.mtl_rx_fifo_fill_level_full++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_RXFSTS_AT: rxfsts ==) -> else {
    else if (rxfsts == GMAC_DEBUG_RXFSTS_AT)
    x.mtl_rx_fifo_fill_above_thresh++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_RXFSTS_BT: rxfsts ==) -> else {
    ...
    let mut rrcsts: u32 = FIELD_GET(GMAC_DEBUG_RRCSTS_MASK, value);
    if (rrcsts == GMAC_DEBUG_RRCSTS_FLUSH)
    x.mtl_rx_fifo_read_ctrl_flush++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_RRCSTS_RSTAT: rrcsts ==) -> else {
    else if (rrcsts == GMAC_DEBUG_RRCSTS_RSTAT)
    x.mtl_rx_fifo_read_ctrl_read_data++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_RRCSTS_RDATA: rrcsts ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_set_mac_loopback(ioaddr: *mut void __iomem, enable: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn dwmac1000_setup(priv: *mut stmmac_priv) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dwmac1000_get_ptptime(ptpaddr: *mut void __iomem, ptp_time: *mut u64) {
...
#[no_mangle]
pub unsafe extern "C" fn dwmac1000_timestamp_interrupt(priv: *mut stmmac_priv) {
...
#[no_mangle]
unsafe extern "C" fn dwmac1000_timestamp_interrupt_cfg(priv: *mut stmmac_priv, en: bool) {
