...
//! Source: drivers/net/ethernet/stmicro/stmmac/dwmac4_core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dwmac4_pcs_init(priv: *mut stmmac_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_update_caps(priv: *mut stmmac_priv) {
    ...
    {
    void __iomem *ioaddr = hw.pcsr;
    let mut value: u32 = readl(ioaddr + GMAC_RXQ_CTRL0);
    value &= GMAC_RX_QUEUE_CLEAR(queue);
    if (mode == MTL_QUEUE_AVB)
    value |= GMAC_RX_AV_QUEUE_ENABLE(queue);
#[no_mangle]
pub unsafe extern "C" fn if(MTL_QUEUE_DCB: mode ==) -> else {
...
// some packets require extra ops
    ...
    {
    void __iomem *ioaddr = hw.pcsr;
    let mut value: u32 = readl(ioaddr + MTL_OPERATION_MODE);
...
#[no_mangle]
unsafe extern "C" fn dwmac4_map_mtl_dma(hw: *mut mac_device_info, queue: u32, chan: u32) {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_dump_regs(hw: *mut mac_device_info, reg_space: *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_rx_ipc_enable(hw: *mut mac_device_info) -> c_int {
    static int dwmac4_rx_ipc_enable(struct mac_device_info *hw)
    {
    void __iomem *ioaddr = hw.pcsr;
    let mut value: u32 = readl(ioaddr + GMAC_CONFIG);
    if (hw.rx_csum)
...
#[no_mangle]
unsafe extern "C" fn dwmac4_pmt(hw: *mut mac_device_info, mode: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_set_eee_pls(hw: *mut mac_device_info, link: c_int) {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_set_eee_timer(hw: *mut mac_device_info, ls: c_int, tw: c_int) {
    static void dwmac4_set_eee_timer(struct mac_device_info *hw, int ls, int tw)
    {
    void __iomem *ioaddr = hw.pcsr;
    let mut value: c_int = ((tw & 0xffff)) | ((ls & 0x3ff) << 16);
...
//
    ...
    let mut numhashregs: c_int = (hw.multicast_filter_bins >> 5);
    let mut mcbitslog2: c_int = hw.mcast_bits_log2;
    unsigned int value;
    u32 mc_filter[8];
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(NETIF_F_HW_VLAN_CTAG_FILTER: dev->features &) -> else {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_ctrl_ane(priv: *mut stmmac_priv, ane: bool, srgmi_ral: bool) {
...
// clear Interrupt
    ...
    let mut intr_status: u32 = readl(ioaddr + GMAC_INT_STATUS);
    let mut intr_enable: u32 = readl(ioaddr + GMAC_INT_EN);
    let mut ret: c_int = 0;
...
// Clear LPI interrupt by reading MAC_LPI_Control_Status
    let mut status: u32 = readl(ioaddr + GMAC4_LPI_CTRL_STATUS);
    ...
    let mut trcsts: u32 = FIELD_GET(MTL_DEBUG_TRCSTS_MASK, value);
    if (trcsts == MTL_DEBUG_TRCSTS_WRITE)
    x.mtl_tx_fifo_read_ctrl_write++;
#[no_mangle]
pub unsafe extern "C" fn if(MTL_DEBUG_TRCSTS_TXW: trcsts ==) -> else {
    else if (trcsts == MTL_DEBUG_TRCSTS_TXW)
    x.mtl_tx_fifo_read_ctrl_wait++;
#[no_mangle]
pub unsafe extern "C" fn if(MTL_DEBUG_TRCSTS_READ: trcsts ==) -> else {
    ...
    let mut rxfsts: u32 = FIELD_GET(MTL_DEBUG_RXFSTS_MASK, value);
    if (rxfsts == MTL_DEBUG_RXFSTS_FULL)
    x.mtl_rx_fifo_fill_level_full++;
#[no_mangle]
pub unsafe extern "C" fn if(MTL_DEBUG_RXFSTS_AT: rxfsts ==) -> else {
    else if (rxfsts == MTL_DEBUG_RXFSTS_AT)
    x.mtl_rx_fifo_fill_above_thresh++;
#[no_mangle]
pub unsafe extern "C" fn if(MTL_DEBUG_RXFSTS_BT: rxfsts ==) -> else {
    ...
    let mut rrcsts: u32 = FIELD_GET(MTL_DEBUG_RRCSTS_MASK, value);
    if (rrcsts == MTL_DEBUG_RRCSTS_FLUSH)
    x.mtl_rx_fifo_read_ctrl_flush++;
#[no_mangle]
pub unsafe extern "C" fn if(MTL_DEBUG_RRCSTS_RSTAT: rrcsts ==) -> else {
    else if (rrcsts == MTL_DEBUG_RRCSTS_RSTAT)
    x.mtl_rx_fifo_read_ctrl_read_data++;
#[no_mangle]
pub unsafe extern "C" fn if(MTL_DEBUG_RRCSTS_RDATA: rrcsts ==) -> else {
...
// GMAC debug
    ...
    let mut tfcsts: u32 = FIELD_GET(GMAC_DEBUG_TFCSTS_MASK, value);
    if (tfcsts == GMAC_DEBUG_TFCSTS_XFER)
    x.mac_tx_frame_ctrl_xfer++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_TFCSTS_GEN_PAUSE: tfcsts ==) -> else {
    else if (tfcsts == GMAC_DEBUG_TFCSTS_GEN_PAUSE)
    x.mac_tx_frame_ctrl_pause++;
#[no_mangle]
pub unsafe extern "C" fn if(GMAC_DEBUG_TFCSTS_WAIT: tfcsts ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_set_mac_loopback(ioaddr: *mut void __iomem, enable: bool) {
...
#[no_mangle]
unsafe extern "C" fn dwmac4_sarc_configure(ioaddr: *mut void __iomem, val: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn dwmac4_setup(priv: *mut stmmac_priv) -> c_int {
