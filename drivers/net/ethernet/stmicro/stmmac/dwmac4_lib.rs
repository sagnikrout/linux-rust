...
//! Source: drivers/net/ethernet/stmicro/stmmac/dwmac4_lib.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn dwmac4_dma_reset(ioaddr: *mut void __iomem) -> c_int {
...
// DMA SW reset
    ...
    {
    const struct dwmac4_addrs *dwmac4_addrs = priv.plat.dwmac4_addrs;
    let mut value: u32 = readl(ioaddr + DMA_CHAN_TX_CONTROL(dwmac4_addrs, chan));
    ...
    {
    const struct dwmac4_addrs *dwmac4_addrs = priv.plat.dwmac4_addrs;
    let mut value: u32 = readl(ioaddr + DMA_CHAN_RX_CONTROL(dwmac4_addrs, chan));
    ...
    {
    const struct dwmac4_addrs *dwmac4_addrs = priv.plat.dwmac4_addrs;
    let mut value: u32 = readl(ioaddr + DMA_CHAN_INTR_ENA(dwmac4_addrs, chan));
    if (rx)
    ...
    {
    const struct dwmac4_addrs *dwmac4_addrs = priv.plat.dwmac4_addrs;
    let mut intr_status: u32 = readl(ioaddr + DMA_CHAN_STATUS(dwmac4_addrs, chan));
    let mut intr_en: u32 = readl(ioaddr + DMA_CHAN_INTR_ENA(dwmac4_addrs, chan));
...
#[no_mangle]
pub unsafe extern "C" fn stmmac_dwmac4_set_mac(ioaddr: *mut void __iomem, enable: bool) {
    void stmmac_dwmac4_set_mac(void __iomem *ioaddr, bool enable)
    {
    let mut value: u32 = readl(ioaddr + GMAC_CONFIG);
    let mut old_val: u32 = value;
    if (enable)
