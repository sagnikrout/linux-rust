...
//! Source: drivers/net/ethernet/stmicro/stmmac/dwxgmac2_core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_update_caps(priv: *mut stmmac_priv) {
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_set_mac(ioaddr: *mut void __iomem, enable: bool) {
    static void dwxgmac2_set_mac(void __iomem *ioaddr, bool enable)
    {
    let mut tx: u32 = readl(ioaddr + XGMAC_TX_CONFIG);
    let mut rx: u32 = readl(ioaddr + XGMAC_RX_CONFIG);
    if (enable) {
    tx |= XGMAC_CONFIG_TE;
    rx |= XGMAC_CONFIG_RE;
    } else {
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_rx_ipc(hw: *mut mac_device_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(PACKET_MCBCQ: packet ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_dump_regs(hw: *mut mac_device_info, reg_space: *mut u32) {
    ...
    if (stat & XGMAC_LPIIS) {
    let mut lpi: u32 = readl(ioaddr + XGMAC_LPI_CTRL);
    ...
    if (status & BIT(chan)) {
    let mut chan_status: u32 = readl(ioaddr + XGMAC_MTL_QINT_STATUS(chan));
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_pmt(hw: *mut mac_device_info, mode: c_ulong) {
    ...
    if (val) {
    let mut cfg: u32 = readl(ioaddr + XGMAC_RX_CONFIG);
    cfg |= XGMAC_CONFIG_RE;
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_set_eee_pls(hw: *mut mac_device_info, link: c_int) {
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_set_eee_timer(hw: *mut mac_device_info, ls: c_int, tw: c_int) {
    ...
    let mut value: u32 = readl(ioaddr + XGMAC_PACKET_FILTER);
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_set_mac_loopback(ioaddr: *mut void __iomem, enable: bool) {
    static void dwxgmac2_set_mac_loopback(void __iomem *ioaddr, bool enable)
    {
    let mut value: u32 = readl(ioaddr + XGMAC_RX_CONFIG);
    if (enable)
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dwxgmac3_error_desc {
    pub valid: bool,
    pub desc: *const c_char,
    pub detailed_desc: *const c_char,
}
...
//
    ...
    const char **desc)
    {
    let mut module: c_int = index / 32, offset = index % 32;
...
#[no_mangle]
unsafe extern "C" fn dwxgmac3_rxp_disable(ioaddr: *mut void __iomem) -> c_int {
    static int dwxgmac3_rxp_disable(void __iomem *ioaddr)
    {
    let mut val: u32 = readl(ioaddr + XGMAC_MTL_OPMODE);
    val &= ~XGMAC_FRPE;
    writel(val, ioaddr + XGMAC_MTL_OPMODE);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn dwxgmac3_rxp_enable(ioaddr: *mut void __iomem) {
    ...
    let mut real_pos: c_int = pos * (sizeof(entry.val) / sizeof(u32)) + i;
    u32 val;
// Wait for ready
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_get_mac_tx_timestamp(hw: *mut mac_device_info, ts: *mut u64) -> c_int {
...
// ts += readl(ioaddr + XGMAC_TXTIMESTAMP_SEC) * 1000000000ULL;
    ...
    {
    let mut tnsec: u32 = readl(ioaddr + XGMAC_PPSx_TARGET_TIME_NSEC(index));
    let mut val: u32 = readl(ioaddr + XGMAC_PPS_CONTROL);
    u64 period;
...
#[no_mangle]
unsafe extern "C" fn dwxgmac2_sarc_configure(ioaddr: *mut void __iomem, val: c_int) {
    static void dwxgmac2_sarc_configure(void __iomem *ioaddr, int val)
    {
    let mut value: u32 = readl(ioaddr + XGMAC_TX_CONFIG);
    value = u32_replace_bits(value, val, XGMAC_CONFIG_SARC);
    writel(value, ioaddr + XGMAC_TX_CONFIG);
    }
#[no_mangle]
unsafe extern "C" fn dwxgmac2_filter_wait(hw: *mut mac_device_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dwxgmac2_setup(priv: *mut stmmac_priv) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dwxlgmac2_setup(priv: *mut stmmac_priv) -> c_int {
