...
//! Source: drivers/net/ethernet/cavium/liquidio/request_manager.c
#![no_std]
...
// Return 0 on success, 1 on failure
    ...
    struct octeon_iq_config *conf = core::ptr::null_mut();
    let mut iq_no: u32 = (u32)txpciq.s.q_no;
    u32 q_size;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: OCTEON_CN23XX_PF(oct)) -> else {
...
// Set the 32B/64B mode for each input queue
    ...
    iq.request_list = core::ptr::null_mut();
    lio_dma_free(oct, q_size, iq.base_addr, iq.base_addr_dma);
...
#[no_mangle]
pub unsafe extern "C" fn octeon_delete_instr_queue(oct: *mut octeon_device, iq_no: u32) -> c_int {
    int octeon_delete_instr_queue(struct octeon_device *oct, u32 iq_no)
    {
    let mut desc_size: u64 = 0, q_size;
...
pub unsafe extern "C" fn if(_arg: OCTEON_CN23XX_VF(oct)) -> else {
    ...
    oct.instr_queue[iq_no] = core::ptr::null_mut();
    oct.num_iqs--;
    return 0;
    }
    return 1;
...
#[no_mangle]
pub unsafe extern "C" fn lio_wait_for_instr_fetch(oct: *mut octeon_device) -> c_int {
...
// Can only run in process context
    ...
    int reqtype;
    void *buf;
    let mut old: u32 = iq.flush_index;
...
//
    ...
    iq.request_list[old].buf = core::ptr::null_mut();
...
// Can only be called from process context
    ...
    u32 napi_budget)
    {
    let mut inst_processed: u32 = 0;
    let mut tot_inst_processed: u32 = 0;
    let mut tx_done: c_int = 1;
...
//
#[no_mangle]
unsafe extern "C" fn __check_db_timeout(oct: *mut octeon_device, iq_no: u64) {
    static void __check_db_timeout(struct octeon_device *oct, u64 iq_no)
    {
    struct octeon_instr_queue *iq;
    u64 next_time;
...
// Flush the instruction queue
    ...
    lio_enable_irq(core::ptr::null_mut(), iq);
    }
// Called by the Poll thread at regular intervals to check the instruction
// queue for commands to be posted and for commands that were fetched by Octeon.
//
#[no_mangle]
unsafe extern "C" fn check_db_timeout(work: *mut work_struct) {
    static void check_db_timeout(struct work_struct *work)
    {
    struct cavium_wk *wk = (struct cavium_wk *)work;
    struct octeon_device *oct = (struct octeon_device *)wk.ctxptr;
    let mut iq_no: u64 = wk.ctxul;
    struct cavium_wq *db_wq = &oct.check_db_wq[iq_no];
    let mut delay: u32 = 10;
...
#[no_mangle]
pub unsafe extern "C" fn octeon_setup_sc_buffer_pool(oct: *mut octeon_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn octeon_free_sc_done_list(oct: *mut octeon_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn octeon_free_sc_zombie_list(oct: *mut octeon_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn octeon_free_sc_buffer_pool(oct: *mut octeon_device) -> c_int {
    ...
    u64 dma_addr;
    u32 size;
    let mut offset: u32 = sizeof(struct octeon_soft_command);
    struct octeon_soft_command *sc = core::ptr::null_mut();
