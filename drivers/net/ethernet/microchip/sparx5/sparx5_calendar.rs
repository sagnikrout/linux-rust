...
//! Source: drivers/net/ethernet/microchip/sparx5/sparx5_calendar.c
#![no_std]
...
pub const SPX5_DSM_CAL_TAXIS: c_int = 8;
pub const SPX5_DSM_CAL_BW_LOSS: c_int = 553;
pub const SPX5_TAXI_PORT_MAX: c_int = 70;
pub const SPEED_12500: c_int = 12500;
...
#[no_mangle]
unsafe extern "C" fn sparx5_target_bandwidth(sparx5: *mut sparx5) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn sparx5_clk_to_bandwidth(cclock: enum sparx5_core_clockfreq) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn sparx5_cal_speed_to_value(speed: enum sparx5_cal_bw) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn sparx5_bandwidth_to_calendar(bw: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn sparx5_get_port_cal_speed(sparx5: *mut sparx5, portno: u32) -> enum sparx5_cal_bw {
...
#[no_mangle]
unsafe extern "C" fn sparx5_config_auto_calendar(sparx5: *mut sparx5) -> c_int {
    static int sparx5_config_auto_calendar(struct sparx5 *sparx5)
    {
    const struct sparx5_consts *consts = sparx5.data.consts;
    u32 cal[7], value, idx, portno;
    u32 max_core_bw;
    let mut total_bw: u32 = 0, used_port_bw = 0;
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn sparx5_dsm_exb_gcd(a: u32, b: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn sparx5_dsm_cal_len(cal: *mut u32) -> u32 {
    static u32 sparx5_dsm_cal_len(u32 *cal)
    {
    let mut idx: u32 = 0, len = 0;
...
#[no_mangle]
unsafe extern "C" fn sparx5_dsm_cp_cal(sched: *mut u32) -> u32 {
    static u32 sparx5_dsm_cp_cal(u32 *sched)
    {
    let mut idx: u32 = 0, tmp;
    while (idx < SPX5_DSM_CAL_LEN) {
...
// Map ports to taxi positions
    ...
    let mut portno: u32 = data.taxi_ports[idx];
...
pub unsafe extern "C" fn if(1: idx ==) -> else {
    ...
    raw_spd = gcd / 2;
#[no_mangle]
pub unsafe extern "C" fn if(2: idx ==) -> else {
    else if (idx == 2)
    raw_spd = gcd;
    else
    raw_spd = min;
    ...
    sum = 0;
    for (idx = 0; idx < ARRAY_SIZE(data.taxi_speeds); idx++) {
    let mut spd: u32 = data.taxi_speeds[idx];
    let mut adjusted_speed: u32 = data.taxi_speeds[idx] * factor / 1000;
    if (adjusted_speed > 0) {
    ...
    let mut cal_length: u32 = sparx5_dsm_cal_len(data.schedule);
    ...
    struct sparx5_calendar_data *data)
    {
    let mut cal_len: u32 = sparx5_dsm_cal_len(data.schedule), len, idx;
    if (!is_sparx5(sparx5)) {
    u32 val, act;
...
#[no_mangle]
unsafe extern "C" fn sparx5_config_dsm_calendar(sparx5: *mut sparx5) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn sparx5_calendar_init(sparx5: *mut sparx5) -> c_int {
