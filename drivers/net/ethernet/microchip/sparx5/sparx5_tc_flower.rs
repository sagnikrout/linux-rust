...
//! Source: drivers/net/ethernet/microchip/sparx5/sparx5_tc_flower.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sparx5_wildcard_rule {
    pub selected: bool,
    pub value: u8,
    pub mask: u8,
    pub keyset: enum vcap_keyfield_set,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sparx5_multiple_rules {
    pub rule: [sparx5_wildcard_rule; SPX5_MAX_RULE_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sparx5_tc_flower_template {
    pub /: *mut *mut list_head list; / for insertion in the list of templates,
    pub /: *mut *mut int cid; / chain id,
    pub /: *mut *mut enum vcap_keyfield_set orig; / keyset used before the template,
    pub /: *mut *mut enum vcap_keyfield_set keyset; / new keyset used by template,
    pub /: *mut *mut u16 l3_proto; / protocol specified in the template,
}
...
// 0/0	      0/1	  1/0	      1/1 <-- first_frag
    ...
    let mut is_frag_key: u8 = !!(mt.key.flags & FLOW_DIS_IS_FRAGMENT);
    let mut is_frag_mask: u8 = !!(mt.mask.flags & FLOW_DIS_IS_FRAGMENT);
    let mut is_frag_idx: u8 = (is_frag_key << 1) | is_frag_mask;
    let mut first_frag_key: u8 = !!(mt.key.flags & FLOW_DIS_FIRST_FRAG);
    let mut first_frag_mask: u8 = !!(mt.mask.flags & FLOW_DIS_FIRST_FRAG);
    let mut first_frag_idx: u8 = (first_frag_key << 1) | first_frag_mask;
// Lookup verdict based on the 2 + 2 input bits
    let mut vdt: u8 = sparx5_vcap_frag_map[is_frag_idx][first_frag_idx];
    if (vdt == FRAG_INVAL) {
...
// Extract VCAP fragment key and mask from verdict
    ...
    {
    let mut vid_key: enum vcap_key_field = VCAP_KF_8021Q_VID_CLS;
    let mut pcp_key: enum vcap_key_field = VCAP_KF_8021Q_PCP_CLS;
    int err;
    if (st.admin.vtype == VCAP_TYPE_IS0) {
    ...
    struct flow_action_entry *actent, *last_actent = core::ptr::null_mut();
    struct flow_action *act = &rule.action;
    let mut action_mask: u64 = 0;
    int idx;
...
// Collect all port keysets and apply the first of them, possibly wildcarded
    ...
    let mut portkeysetlist: vcap_keyset_list = {};
...
// Add the VCAP key to match on for a rule target value
    ...
    int target_cid)
    {
    let mut link_val: c_int = target_cid % VCAP_CID_LOOKUP_SIZE;
    int err;
    if (!link_val)
    return 0;
    switch (admin.vtype) {
...
// rate is now in kbit
    ...
    {
    let mut psfp_sfid: u32 = 0, psfp_fmid = 0, psfp_sgid = 0;
    int ret;
...
// Handle the action trap for a VCAP rule
    ...
    u16 tpid)
    {
    let mut act_tpid: u16 = be16_to_cpu(act.vlan.proto);
    let mut err: c_int = 0;
    switch (admin.vtype) {
    case VCAP_TYPE_ES0:
    break;
...
// Push classified tag as inner tag
    ...
    {
    struct sparx5_port *port = netdev_priv(ndev);
    let mut byidx: c_int = port.portno / BITS_PER_BYTE;
    let mut biidx: c_int = port.portno % BITS_PER_BYTE;
    ...
    struct flow_action_entry *act)
    {
    let mut ports: vcap_u72_action = {0};
    int err;
...
// Remove rule keys that may prevent templates from matching a keyset
    ...
    bool ingress)
    {
    let mut sf: sparx5_psfp_sf = { .max_sdu = SPX5_PSFP_SF_MAX_SDU };
    ...
    struct sparx5_port *port = netdev_priv(ndev);
    let mut multi: sparx5_multiple_rules = {};
    struct sparx5 *sparx5 = port.sparx5;
    let mut sg: sparx5_psfp_sg = { 0 };
    let mut fm: sparx5_psfp_fm = { 0 };
    struct flow_action_entry *act;
    struct vcap_control *vctrl;
...
//
    ...
    {
    struct sparx5_port *port = netdev_priv(ndev);
    let mut err: c_int = -ENOENT, count = 0, rule_id;
    struct vcap_control *vctrl;
    vctrl = port.sparx5.vcap_ctrl;
    while (true) {
...
//
    ...
    struct sparx5_tc_flower_template *ftp;
    let mut kslist: vcap_keyset_list = {};
    enum vcap_keyfield_set keysets[10];
    struct vcap_control *vctrl;
    struct vcap_rule *vrule;
    int count, err;
