...
//! Source: drivers/net/ethernet/microchip/encx24j600.c
#![no_std]
...
//
pub const ENC_TX_BUF_START: c_uint = 0x0000U;
pub const ENC_RX_BUF_START: c_uint = 0x0600U;
pub const ENC_RX_BUF_END: c_uint = 0x5fffU;
pub const ENC_SRAM_SIZE: c_uint = 0x6000U;
    enum {
    RXFILTER_NORMAL,
    RXFILTER_MULTI,
    RXFILTER_PROMISC
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct encx24j600_priv {
    pub ndev: *mut net_device,
    pub /: *mut *mut mutex lock; / device access lock,
    pub ctx: encx24j600_context,
    pub tx_skb: *mut sk_buff,
    pub kworker_task: *mut task_struct,
    pub kworker: kthread_worker,
    pub tx_work: kthread_work,
    pub setrx_work: kthread_work,
    pub next_packet: u16,
    pub hw_enabled: bool,
...
#[no_mangle]
unsafe extern "C" fn encx24j600_read_reg(priv: *mut encx24j600_priv, reg: u8) -> u16 {
    static u16 encx24j600_read_reg(struct encx24j600_priv *priv, u8 reg)
    {
    struct net_device *dev = priv.ndev;
    let mut val: c_uint = 0;
    let mut ret: c_int = regmap_read(priv.ctx.regmap, reg, &val);
    if (unlikely(ret))
...
#[no_mangle]
unsafe extern "C" fn encx24j600_write_reg(priv: *mut encx24j600_priv, reg: u8, val: u16) {
    static void encx24j600_write_reg(struct encx24j600_priv *priv, u8 reg, u16 val)
    {
    struct net_device *dev = priv.ndev;
    let mut ret: c_int = regmap_write(priv.ctx.regmap, reg, val);
    if (unlikely(ret))
    ...
    {
    struct net_device *dev = priv.ndev;
    let mut ret: c_int = regmap_update_bits(priv.ctx.regmap, reg, mask, val);
    if (unlikely(ret))
    netif_err(priv, drv, dev, "%s: error %d updating reg %02x=%04x~%04x\n",
    __func__, ret, reg, val, mask);
    }
#[no_mangle]
unsafe extern "C" fn encx24j600_read_phy(priv: *mut encx24j600_priv, reg: u8) -> u16 {
    static u16 encx24j600_read_phy(struct encx24j600_priv *priv, u8 reg)
    {
    struct net_device *dev = priv.ndev;
    let mut val: c_uint = 0;
    let mut ret: c_int = regmap_read(priv.ctx.phymap, reg, &val);
    if (unlikely(ret))
...
#[no_mangle]
unsafe extern "C" fn encx24j600_write_phy(priv: *mut encx24j600_priv, reg: u8, val: u16) {
    static void encx24j600_write_phy(struct encx24j600_priv *priv, u8 reg, u16 val)
    {
    struct net_device *dev = priv.ndev;
    let mut ret: c_int = regmap_write(priv.ctx.phymap, reg, val);
    if (unlikely(ret))
    netif_err(priv, drv, dev, "%s: error %d writing reg %02x=%04x\n",
    __func__, ret, reg, val);
    }
#[no_mangle]
unsafe extern "C" fn encx24j600_clr_bits(priv: *mut encx24j600_priv, reg: u8, mask: u16) {
    static void encx24j600_clr_bits(struct encx24j600_priv *priv, u8 reg, u16 mask)
    {
    encx24j600_update_reg(priv, reg, mask, 0);
    }
#[no_mangle]
unsafe extern "C" fn encx24j600_set_bits(priv: *mut encx24j600_priv, reg: u8, mask: u16) {
    static void encx24j600_set_bits(struct encx24j600_priv *priv, u8 reg, u16 mask)
    {
    encx24j600_update_reg(priv, reg, mask, mask);
    }
#[no_mangle]
unsafe extern "C" fn encx24j600_cmd(priv: *mut encx24j600_priv, cmd: u8) {
    static void encx24j600_cmd(struct encx24j600_priv *priv, u8 cmd)
    {
    struct net_device *dev = priv.ndev;
    let mut ret: c_int = regmap_write(priv.ctx.regmap, cmd, 0);
    if (unlikely(ret))
...
#[no_mangle]
unsafe extern "C" fn encx24j600_update_phcon1(priv: *mut encx24j600_priv) {
    static void encx24j600_update_phcon1(struct encx24j600_priv *priv)
    {
    let mut phcon1: u16 = encx24j600_read_phy(priv, PHCON1);
...
#[no_mangle]
unsafe extern "C" fn encx24j600_wait_for_autoneg(priv: *mut encx24j600_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_check_link_status(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_int_link_handler(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_tx_complete(priv: *mut encx24j600_priv, err: bool) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_rx_packets(priv: *mut encx24j600_priv, packet_count: u8) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_soft_reset(priv: *mut encx24j600_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_hw_reset(priv: *mut encx24j600_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_reset_hw_tx(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_hw_init_tx(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_hw_init_rx(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_set_rxfilter_mode(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_hw_init(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_hw_enable(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_hw_disable(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_set_hw_macaddr(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_set_mac_address(dev: *mut net_device, addr: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_open(dev: *mut net_device) -> c_int {
    static int encx24j600_open(struct net_device *dev)
    {
    struct encx24j600_priv *priv = netdev_priv(dev);
    int ret = request_threaded_irq(priv.ctx.spi.irq, core::ptr::null_mut(), encx24j600_isr,
...
#[no_mangle]
unsafe extern "C" fn encx24j600_stop(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_setrx_proc(ws: *mut kthread_work) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_set_multicast_list(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_hw_tx(priv: *mut encx24j600_priv) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_tx_proc(ws: *mut kthread_work) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_tx(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_tx_timeout(dev: *mut net_device, txqueue: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_get_regs_len(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_get_msglevel(dev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_set_msglevel(dev: *mut net_device, val: u32) {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_spi_probe(spi: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn encx24j600_spi_remove(spi: *mut spi_device) {
