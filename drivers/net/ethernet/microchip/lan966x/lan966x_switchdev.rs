...
//! Source: drivers/net/ethernet/microchip/lan966x/lan966x_switchdev.c
#![no_std]
...
//
    ...
    bool enabled)
    {
    let mut val: u32 = lan_rd(port.lan966x, ANA_PGID(PGID_MC));
    ...
    bool enabled)
    {
    let mut val: u32 = lan_rd(port.lan966x, ANA_PGID(PGID_UC));
    val = ANA_PGID_PGID_GET(val);
    if (enabled)
    val |= BIT(port.chip_port);
    else
    ...
    bool enabled)
    {
    let mut val: u32 = lan_rd(port.lan966x, ANA_PGID(PGID_BC));
    val = ANA_PGID_PGID_GET(val);
    if (enabled)
    val |= BIT(port.chip_port);
    else
...
#[no_mangle]
unsafe extern "C" fn lan966x_port_set_learning(port: *mut lan966x_port, enabled: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn lan966x_update_fwd_mask(lan966x: *mut lan966x) {
...
#[no_mangle]
pub unsafe extern "C" fn lan966x_port_stp_state_set(port: *mut lan966x_port, state: u8) {
    void lan966x_port_stp_state_set(struct lan966x_port *port, u8 state)
    {
    struct lan966x *lan966x = port.lan966x;
    let mut learn_ena: bool = false;
    ...
    let mut ageing_time: u32 = jiffies_to_msecs(ageing_jiffies) / 1000;
    lan966x_mac_set_ageing(port.lan966x, ageing_time);
    }
#[no_mangle]
unsafe extern "C" fn lan966x_port_mc_set(port: *mut lan966x_port, mcast_ena: bool) {
    ...
    lan966x.bridge_mask &= ~BIT(port.chip_port);
    if (!lan966x.bridge_mask)
    lan966x.bridge = core::ptr::null_mut();
// Set the port back to host mode
    ...
    switchdev_bridge_port_unoffload(port.dev, port, core::ptr::null_mut(), core::ptr::null_mut());
    ...
    if (err || info.linking)
    return err;
    switchdev_bridge_port_unoffload(brport_dev, port, core::ptr::null_mut(), core::ptr::null_mut());
    lan966x_fdb_flush_workqueue(port.lan966x);
    }
    return err;
    }
    ...
    struct netlink_ext_ack *extack)
    {
    struct lan966x *lan966x = core::ptr::null_mut();
...
// has_foreign = true;
    ...
    struct netdev_notifier_changeupper_info *info)
    {
    let mut has_foreign: bool = false;
    let mut seen_lan966x: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn lan966x_register_notifier_blocks() {
...
#[no_mangle]
pub unsafe extern "C" fn lan966x_unregister_notifier_blocks() {
