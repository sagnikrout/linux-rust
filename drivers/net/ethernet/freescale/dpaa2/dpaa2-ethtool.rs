...
//! Source: drivers/net/ethernet/freescale/dpaa2/dpaa2-ethtool.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dpaa2_eth_nway_reset(net_dev: *mut net_device) -> c_int {
    ...
    {
    struct dpaa2_eth_priv *priv = netdev_priv(net_dev);
    let mut link_options: u64 = priv.link_state.options;
    mutex_lock(&priv.mac_lock);
    if (dpaa2_eth_is_type_phy(priv)) {
...
#[no_mangle]
unsafe extern "C" fn dpaa2_eth_get_sset_count(net_dev: *mut net_device, sset: c_int) -> c_int {
...
//
    ...
    let mut fcnt_rx_total: u32 = 0, fcnt_tx_total = 0;
    let mut bcnt_rx_total: u32 = 0, bcnt_tx_total = 0;
...
// Print FQ instantaneous counts
    err = dpaa2_io_query_fq_count(core::ptr::null_mut(), priv.fq[j].fqid,
    &fcnt, &bcnt);
    if (err) {
...
// (data + i++) = bcnt_tx_total;
    ...
    err = dpaa2_io_query_bp_count(core::ptr::null_mut(), priv.bp[j].bpid, &buf_cnt);
    if (err) {
...
// fields |= DPAA2_ETH_DIST_ETHDST;
    ...
    let mut rule_cfg: dpni_rule_cfg = { 0 };
    let mut fs_act: dpni_fs_action_cfg = { 0 };
    dma_addr_t key_iova;
...
#[no_mangle]
unsafe extern "C" fn dpaa2_eth_num_cls_rules(priv: *mut dpaa2_eth_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dpaa2_eth_get_rx_ring_count(net_dev: *mut net_device) -> u32 {
    ...
    {
    struct dpaa2_eth_priv *priv = netdev_priv(net_dev);
    let mut max_rules: c_int = dpaa2_eth_fs_count(priv);
    int i, j = 0;
    switch (rxnfc.cmd) {
    ...
    err = dpaa2_eth_update_cls_rule(net_dev, core::ptr::null_mut(), rxnfc.fs.location);
    break;
    default:
    err = -EOPNOTSUPP;
    }
...
//
    ...
    let mut dpaa2_phc_index: c_int = -1;
...
// Setup new value for rx coalescing
    ...
    {
    struct dpaa2_eth_priv *priv = netdev_priv(net_dev);
    let mut queue_count: c_int = dpaa2_eth_queue_count(priv);
