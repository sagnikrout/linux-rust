...
//! Source: drivers/net/ethernet/freescale/dpaa2/dpaa2-xsk.c
#![no_std]
...
//
    ...
    {
    let mut addr: dma_addr_t = dpaa2_fd_get_addr(fd);
    struct bpf_prog *xdp_prog;
    struct xdp_buff *xdp_buff;
    struct dpaa2_eth_swa *swa;
    let mut xdp_act: u32 = XDP_PASS;
    int err;
...
// Rx frame processing routine for the AF_XDP fast path
    ...
    {
    let mut addr: dma_addr_t = dpaa2_fd_get_addr(fd);
    let mut fd_format: u8 = dpaa2_fd_get_format(fd);
    struct rtnl_link_stats64 *percpu_stats;
    let mut fd_length: u32 = dpaa2_fd_get_len(fd);
    struct sk_buff *skb;
    void *vaddr;
    u32 xdp_act;
...
#[no_mangle]
unsafe extern "C" fn dpaa2_xsk_disable_pool(dev: *mut net_device, qid: u16) -> c_int {
    ...
    let mut pools_params: dpni_pools_cfg = { 0 };
    struct dpaa2_eth_channel *ch;
    int err;
    bool up;
    ch = priv.channel[qid];
    ...
    if (up) {
    err = dev_open(dev, core::ptr::null_mut());
    if (err)
    return err;
    }
    return 0;
    ...
    ch = priv.channel[qid];
    err = xdp_rxq_info_reg_mem_model(&ch.xdp_rxq, MEM_TYPE_XSK_BUFF_POOL, core::ptr::null_mut());
    if (err) {
    ...
    err_dma_unmap:
    if (up)
    dev_open(dev, core::ptr::null_mut());
    return err;
    }
#[no_mangle]
pub unsafe extern "C" fn dpaa2_xsk_setup_pool(dev: *mut net_device, pool: *mut xsk_buff_pool, qid: u16) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dpaa2_xsk_wakeup(dev: *mut net_device, qid: u32, flags: u32) -> c_int {
...
//
    ...
    struct dpaa2_eth_swa *swa;
    void *sgt_buf = core::ptr::null_mut();
    dma_addr_t sgt_addr;
    int sgt_buf_size;
    dma_addr_t addr;
    let mut err: c_int = 0;
...
// Initialize FD fields
    ...
    struct rtnl_link_stats64 *percpu_stats;
    let mut budget: c_int = DPAA2_ETH_TX_ZC_PER_NAPI;
...
// Update statistics
    ...
    let mut total_enqueued: return = = budget;
    }
