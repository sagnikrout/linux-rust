...
//! Source: drivers/net/ethernet/freescale/gianfar_ethtool.c
#![no_std]
...
pub const GFAR_MAX_COAL_USECS: c_uint = 0xffff;
pub const GFAR_MAX_COAL_FRAMES: c_uint = 0xff;
...
// stats
#[no_mangle]
unsafe extern "C" fn gfar_gstrings(dev: *mut net_device, stringset: u32, buf: *mut *mut u8) {
...
#[no_mangle]
unsafe extern "C" fn gfar_sset_count(dev: *mut net_device, sset: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gfar_reglen(dev: *mut net_device) -> c_int {
...
// structure.
    ...
    {
    struct gfar_private *priv = netdev_priv(dev);
    struct gfar_priv_rx_q *rx_queue = core::ptr::null_mut();
    struct gfar_priv_tx_q *tx_queue = core::ptr::null_mut();
...
// Rebuild the rings with the new size
    ...
    if (!epause.autoneg) {
    let mut tempval: u32 = gfar_read(&regs.maccfg1);
...
#[no_mangle]
pub unsafe extern "C" fn gfar_set_features(dev: *mut net_device, features: netdev_features_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gfar_get_msglevel(dev: *mut net_device) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn gfar_set_msglevel(dev: *mut net_device, data: u32) {
...
#[no_mangle]
unsafe extern "C" fn gfar_get_wol(dev: *mut net_device, wol: *mut ethtool_wolinfo) {
...
#[no_mangle]
unsafe extern "C" fn gfar_set_wol(dev: *mut net_device, wol: *mut ethtool_wolinfo) -> c_int {
    static int gfar_set_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
    {
    struct gfar_private *priv = netdev_priv(dev);
    let mut wol_opts: u16 = 0;
    int err;
...
#[no_mangle]
unsafe extern "C" fn ethflow_to_filer_rules(priv: *mut gfar_private, ethflow: u64) {
    static void ethflow_to_filer_rules (struct gfar_private *priv, u64 ethflow)
    {
    let mut fcr: u32 = 0x0, fpr = FPR_FILER_MASK;
    if (ethflow & RXH_L2DA) {
    ...
    let mut i: c_int = 0x0, k = 0x0;
    let mut j: c_int = MAX_FILER_IDX, l = 0x0;
    let mut ret: c_int = 1;
...
#[no_mangle]
unsafe extern "C" fn gfar_check_filer_hardware(priv: *mut gfar_private) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gfar_set_mask(mask: u32, tab: *mut filer_table) {
...
#[no_mangle]
unsafe extern "C" fn gfar_set_parse_bits(value: u32, mask: u32, tab: *mut filer_table) {
...
// Translates value and mask for ETHER spec
    ...
    struct filer_table *tab)
    {
    let mut upper_temp_mask: u32 = 0;
    let mut lower_temp_mask: u32 = 0;
// Source address
...
#[no_mangle]
pub unsafe extern "C" fn vlan_tci_priom(rule: *mut ethtool_rx_flow_spec) -> u32 {
...
// Convert a rule to binary filter format of gianfar
    ...
    struct filer_table *tab)
    {
    let mut vlan: u32 = 0, vlan_mask = 0;
    let mut id: u32 = 0, id_mask = 0;
    let mut cfi: u32 = 0, cfi_mask = 0;
    let mut prio: u32 = 0, prio_mask = 0;
    let mut old_index: u32 = tab.index;
...
#[no_mangle]
unsafe extern "C" fn gfar_process_filer_changes(priv: *mut gfar_private) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gfar_invert_masks(flow: *mut ethtool_rx_flow_spec) {
...
#[no_mangle]
unsafe extern "C" fn gfar_del_cls(priv: *mut gfar_private, loc: u32) -> c_int {
    static int gfar_del_cls(struct gfar_private *priv, u32 loc)
    {
    struct ethtool_flow_spec_container *comp;
    let mut ret: u32 = -EINVAL;
    if (list_empty(&priv.rx_list.list))
    return ret;
    list_for_each_entry(comp, &priv.rx_list.list, list) {
...
#[no_mangle]
unsafe extern "C" fn gfar_get_cls(priv: *mut gfar_private, cmd: *mut ethtool_rxnfc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gfar_set_nfc(dev: *mut net_device, cmd: *mut ethtool_rxnfc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gfar_get_rx_ring_count(dev: *mut net_device) -> u32 {
    ...
    struct ptp_qoriq *ptp = core::ptr::null_mut();
    if (!(priv.device_flags & FSL_GIANFAR_DEV_HAS_TIMER)) {
    info.so_timestamping = SOF_TIMESTAMPING_TX_SOFTWARE;
    return 0;
    }
    ptp_node = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "fsl,etsec-ptp");
    if (ptp_node) {
