...
//! Source: drivers/net/ethernet/qlogic/qlcnic/qlcnic_ctx.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_get_cmd_signature(ahw: *mut qlcnic_hardware_context) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_free_mbx_args(cmd: *mut qlcnic_cmd_args) {
    void qlcnic_free_mbx_args(struct qlcnic_cmd_args *cmd)
    {
    kfree(cmd.req.arg);
    cmd.req.arg = core::ptr::null_mut();
    kfree(cmd.rsp.arg);
    cmd.rsp.arg = core::ptr::null_mut();
    }
    static u32
    qlcnic_poll_rsp(struct qlcnic_adapter *adapter)
    {
    u32 rsp;
    let mut timeout: c_int = 0, err = 0;
    do {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_fw_cmd_set_drv_version(adapter: *mut qlcnic_adapter, fw_cmd: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_82xx_fw_cmd_create_rx_ctx(adapter: *mut qlcnic_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_82xx_fw_cmd_del_rx_ctx(adapter: *mut qlcnic_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_alloc_hw_resources(adapter: *mut qlcnic_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_fw_create_ctx(dev: *mut qlcnic_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_fw_destroy_ctx(adapter: *mut qlcnic_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_free_hw_resources(adapter: *mut qlcnic_adapter) {
    ...
    for (ring = 0; ring < adapter.drv_tx_rings; ring++) {
    tx_ring = &adapter.tx_ring[ring];
    if (tx_ring.hw_consumer != core::ptr::null_mut()) {
    dma_free_coherent(&adapter.pdev.dev, sizeof(u32),
    tx_ring.hw_consumer,
    tx_ring.hw_cons_phys_addr);
    tx_ring.hw_consumer = core::ptr::null_mut();
    }
    if (tx_ring.desc_head != core::ptr::null_mut()) {
    dma_free_coherent(&adapter.pdev.dev,
    TX_DESC_RINGSIZE(tx_ring),
    tx_ring.desc_head,
    tx_ring.phys_addr);
    tx_ring.desc_head = core::ptr::null_mut();
    }
    }
    for (ring = 0; ring < adapter.max_rds_rings; ring++) {
    rds_ring = &recv_ctx.rds_rings[ring];
    if (rds_ring.desc_head != core::ptr::null_mut()) {
    dma_free_coherent(&adapter.pdev.dev,
    RCV_DESC_RINGSIZE(rds_ring),
    rds_ring.desc_head,
    rds_ring.phys_addr);
    rds_ring.desc_head = core::ptr::null_mut();
    }
    }
    for (ring = 0; ring < adapter.drv_sds_rings; ring++) {
    sds_ring = &recv_ctx.sds_rings[ring];
    if (sds_ring.desc_head != core::ptr::null_mut()) {
    dma_free_coherent(&adapter.pdev.dev,
    STATUS_DESC_RINGSIZE(sds_ring),
    sds_ring.desc_head,
    sds_ring.phys_addr);
    sds_ring.desc_head = core::ptr::null_mut();
    }
    }
    }
#[no_mangle]
pub unsafe extern "C" fn qlcnic_82xx_config_intrpt(adapter: *mut qlcnic_adapter, op_type: u8) -> c_int {
...
// Get info of a NIC partition
    ...
    void *nic_info_addr;
    struct qlcnic_cmd_args cmd;
    let mut nic_size: usize = sizeof(struct qlcnic_info_le);
...
// Get PCI Info of a partition
    ...
    {
    struct qlcnic_hardware_context *ahw = adapter.ahw;
    let mut npar_size: usize = sizeof(struct qlcnic_pci_info_le);
    let mut pci_size: usize = npar_size * ahw.max_vnic_func;
    let mut nic: u16 = 0, fcoe = 0, iscsi = 0;
...
// Configure eSwitch for port mirroring
    ...
    let mut stats_size: usize = sizeof(struct qlcnic_esw_stats_le);
    ...
    struct qlcnic_cmd_args cmd;
    int err;
    if (esw_stats == core::ptr::null_mut())
    return -ENOMEM;
    if ((adapter.ahw.op_mode != QLCNIC_MGMT_FUNC) &&
...
// This routine will retrieve the MAC statistics from firmware
    ...
    struct qlcnic_cmd_args cmd;
    let mut stats_size: usize = sizeof(struct qlcnic_mac_statistics_le);
    dma_addr_t stats_dma_t;
    void *stats_addr;
    int err;
    if (mac_stats == core::ptr::null_mut())
    return -ENOMEM;
    stats_addr = dma_alloc_coherent(&adapter.pdev.dev, stats_size,
    &stats_dma_t, GFP_KERNEL);
    if (!stats_addr)
    ...
    if (adapter.ahw.op_mode != QLCNIC_MGMT_FUNC)
    return -EIO;
    if (adapter.npars == core::ptr::null_mut())
    return -EIO;
