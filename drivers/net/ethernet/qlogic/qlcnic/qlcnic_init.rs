...
//! Source: drivers/net/ethernet/qlogic/qlcnic/qlcnic_init.c
#![no_std]
...
pub const QLCNIC_MAX_CRB_XFORM: c_int = 60;
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_release_rx_buffers(adapter: *mut qlcnic_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_reset_rx_buffers_list(adapter: *mut qlcnic_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_free_sw_resources(adapter: *mut qlcnic_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_alloc_sw_resources(adapter: *mut qlcnic_adapter) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn qlcnic_decode_crb_addr(addr: u32) -> u32 {
...
pub const QLCNIC_MAX_ROM_WAIT_USEC: c_int = 100;
#[no_mangle]
unsafe extern "C" fn qlcnic_wait_rom_done(adapter: *mut qlcnic_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_rom_fast_read(adapter: *mut qlcnic_adapter, addr: u32, valp: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_pinit_from_rom(adapter: *mut qlcnic_adapter) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qlcnic_cmd_peg_ready(adapter: *mut qlcnic_adapter) -> c_int {
    ...
    {
    u32 val;
    let mut retries: c_int = QLCNIC_RCVPEG_CHECK_RETRY_COUNT;
    do {
    ...
    if (flt_entry == core::ptr::null_mut())
    return -EIO;
    ret = qlcnic_rom_fast_read_words(adapter, QLCNIC_FLT_LOCATION +
    sizeof(struct qlcnic_flt_header),
...
pub const QLCNIC_UNI_VERSION_TAIL_SIZE: c_int = 17;

    (QLCNIC_IMAGE_START - QLCNIC_BOOTLD_START)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qlcnic_uni_data {
    pub offset: u32,
    pub size: u32,
}

#[no_mangle]
unsafe extern "C" fn qlcnic_rom_range_valid(size: usize, offset: usize, len: usize) -> bool {
...
// entry_size = get_unaligned_le32(directory +
    ...
    return ret;
    for (i = 0; i < entries; i++) {
    let mut offset: usize = directory_offset + i * entry_size;
    u32 table_type;
...
#[no_mangle]
unsafe extern "C" fn qlcnic_validate_bootld(adapter: *mut qlcnic_adapter) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qlcnic_validate_fw(adapter: *mut qlcnic_adapter) -> c_int {
    ...
    const u8 *unirom = adapter.fw.data;
    let mut mn_present: c_int = qlcnic_has_mn(adapter);
    u32 entries, entry_size;
    size_t i;
    int ret;
    ...
    nomn:
    for (i = 0; i < entries; i++) {
    let mut offset: usize = table_offset + i * entry_size;
    ...
    const u8 **bootld)
    {
    let mut offs: u32 = QLCNIC_BOOTLD_START;
    struct qlcnic_uni_data data;
    int ret;
...
// bootld = adapter->fw->data + offs;
    ...
    {
    let mut offs: u32 = QLCNIC_IMAGE_START;
    struct qlcnic_uni_data data;
    int ret;
    if (adapter.ahw.fw_type == QLCNIC_UNIFIED_ROMIMAGE) {
    ret = qlcnic_get_data_desc(adapter, QLCNIC_UNI_DIR_SECT_FW,
...
#[no_mangle]
unsafe extern "C" fn qlcnic_get_fw_version(adapter: *mut qlcnic_adapter) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn qlcnic_get_bios_version(adapter: *mut qlcnic_adapter) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn qlcnic_rom_lock_recovery(adapter: *mut qlcnic_adapter) {
    ...
    let mut retries: c_int = QLCNIC_HEARTBEAT_CHECK_RETRY_COUNT;
    ...
    const struct firmware *fw = adapter.fw;
    struct pci_dev *pdev = adapter.pdev;
    let mut fw_type: u8 = adapter.ahw.fw_type;
...
#[no_mangle]
pub unsafe extern "C" fn qlcnic_request_firmware(adapter: *mut qlcnic_adapter) {
