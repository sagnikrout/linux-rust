...
//! Source: drivers/net/ethernet/qlogic/qed/qed_mng_tlv.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qed_tlv_parsed_buf {
// To be filled with the address to set in Value field
    pub p_val: *mut c_void,
// To be used internally in case the value has to be modified
    pub data: [u8; QED_TLV_DATA_MAX],
}

#[no_mangle]
unsafe extern "C" fn qed_mfw_get_tlv_group(tlv_type: u8, tlv_group: *mut u8) -> c_int {
...
// Returns size of the data buffer or, -1 in case TLV data is not available.
    ...
    case DRV_TLV_ADDITIONAL_MAC_ADDR_2:
    {
    let mut idx: c_int = p_tlv.tlv_type - DRV_TLV_LOCAL_ADMIN_ADDR;
...
// Validate numbers
    ...
    &buffer);
#[no_mangle]
pub unsafe extern "C" fn if(QED_MFW_TLV_ETH: tlv_group ==) -> else {
    else if (tlv_group == QED_MFW_TLV_ETH)
    len = qed_mfw_get_eth_tlv_value(&tlv,
    &p_tlv_data.eth,
    &buffer);
#[no_mangle]
pub unsafe extern "C" fn if(QED_MFW_TLV_FCOE: tlv_group ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn qed_mfw_process_tlv_req(p_hwfn: *mut qed_hwfn, p_ptt: *mut qed_ptt) -> c_int {
    int qed_mfw_process_tlv_req(struct qed_hwfn *p_hwfn, struct qed_ptt *p_ptt)
    {
    u32 addr, size, offset, resp, param, val, global_offsize, global_addr;
    let mut tlv_group: u8 = 0, id, *p_mfw_buf = core::ptr::null_mut(), *p_temp;
    struct qed_drv_tlv_hdr tlv;
    int rc;
