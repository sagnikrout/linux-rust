...
//! Source: drivers/net/ethernet/qlogic/netxen/netxen_nic_init.c
#![no_std]
...
pub const NETXEN_MAX_CRB_XFORM: c_int = 60;
    static unsigned int crb_addr_xform[NETXEN_MAX_CRB_XFORM];

    crb_addr_xform[NETXEN_HW_PX_MAP_CRB_##name] = \
    NETXEN_HW_CRB_HUB_AGT_ADR_##name << 20
pub const NETXEN_NIC_XDMA_RESET: c_uint = 0x8000ff;
    static void
    netxen_post_rx_buffers_nodb(struct netxen_adapter *adapter,
    struct nx_host_rds_ring *rds_ring);
    static int netxen_p3_has_mn(struct netxen_adapter *adapter);
#[no_mangle]
unsafe extern "C" fn crb_addr_transform_setup() {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_release_rx_buffers(adapter: *mut netxen_adapter) {
    ...
    if (rx_buf.skb != core::ptr::null_mut())
    dev_kfree_skb_any(rx_buf.skb);
    }
    }
    }
#[no_mangle]
pub unsafe extern "C" fn netxen_release_tx_buffers(adapter: *mut netxen_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_free_sw_resources(adapter: *mut netxen_adapter) {
    ...
    int ring;
    recv_ctx = &adapter.recv_ctx;
    if (recv_ctx.rds_rings == core::ptr::null_mut())
    goto skip_rds;
    for (ring = 0; ring < adapter.max_rds_rings; ring++) {
    rds_ring = &recv_ctx.rds_rings[ring];
    vfree(rds_ring.rx_buf_arr);
    rds_ring.rx_buf_arr = core::ptr::null_mut();
    }
    kfree(recv_ctx.rds_rings);
    skip_rds:
    if (adapter.tx_ring == core::ptr::null_mut())
    return;
    tx_ring = adapter.tx_ring;
    vfree(tx_ring.cmd_buf_arr);
    kfree(tx_ring);
    adapter.tx_ring = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn netxen_alloc_sw_resources(adapter: *mut netxen_adapter) -> c_int {
    ...
    if (rds_ring.rx_buf_arr == core::ptr::null_mut())
...
//
#[no_mangle]
unsafe extern "C" fn netxen_decode_crb_addr(addr: u32) -> u32 {
...
pub const NETXEN_MAX_ROM_WAIT_USEC: c_int = 100;
#[no_mangle]
unsafe extern "C" fn netxen_wait_rom_done(adapter: *mut netxen_adapter) -> c_int {
    static int netxen_wait_rom_done(struct netxen_adapter *adapter)
    {
    let mut timeout: c_long = 0;
    let mut done: c_long = 0;
    cond_resched();
    while (done == 0) {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_rom_fast_read(adapter: *mut netxen_adapter, addr: c_int, valp: *mut c_int) -> c_int {
...
pub const NETXEN_BOARDTYPE: c_uint = 0x4008;
pub const NETXEN_BOARDNUM: c_uint = 0x400c;
pub const NETXEN_CHIPNUM: c_uint = 0x4010;
#[no_mangle]
pub unsafe extern "C" fn netxen_pinit_from_rom(adapter: *mut netxen_adapter) -> c_int {
...
// peg_clr 3
    ...
    let mut entries: __le32 = cpu_to_le32(directory.num_entries);
    for (i = 0; i < entries; i++) {
    __le32 offs = cpu_to_le32(directory.findex) +
    (i * cpu_to_le32(directory.entry_size));
    let mut tab_type: __le32 = cpu_to_le32(*((u32 *)&unirom[offs] + 8));
    if (tab_type == section)
    ...
    struct uni_table_desc *directory = (struct uni_table_desc *) &unirom[0];
    let mut fw_file_size: u32 = adapter.fw.size;
    u32 tab_size;
    __le32 entries;
    __le32 entry_size;
    ...
    if (ptab_descr == core::ptr::null_mut())
    return -EINVAL;
    ...
    let mut chiprev: u8 = adapter.ahw.revision_id;
    uint32_t flagbit;
    ...
    {
    let mut offs: u32 = NETXEN_BOOTLD_START;
    ...
    {
    let mut offs: u32 = NETXEN_IMAGE_START;
    if (adapter.fw_type == NX_UNIFIED_ROMIMAGE)
    offs = cpu_to_le32((nx_get_data_desc(adapter,
    NX_UNI_DIR_SECT_FW,
...
// check if we have got newer or different file firmware
    ...
    size = ( u32)nx_get_fw_size(adapter) / 8;
...
// hi, lo are already in host endian byteorder
    ...
    struct pci_dev *pdev = adapter.pdev;
    const struct firmware *fw = adapter.fw;
    let mut fw_type: u8 = adapter.fw_type;
    u32 crbinit_fix_fw;
    ...
    if (( u32)val != NETXEN_BDINFO_MAGIC)
    return -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: NX_IS_REVISION_P2(adapter->ahw.revision_id)) -> else {
    else if (NX_IS_REVISION_P2(adapter.ahw.revision_id))
    fw_type = NX_P2_MN_ROMIMAGE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: netxen_p3_has_mn(adapter)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_request_firmware(adapter: *mut netxen_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_init_dummy_dma(adapter: *mut netxen_adapter) -> c_int {
    ...
    GFP_KERNEL);
    if (adapter.dummy_dma.addr == core::ptr::null_mut()) {
    dev_err(&adapter.pdev.dev,
...
//
#[no_mangle]
pub unsafe extern "C" fn netxen_free_dummy_dma(adapter: *mut netxen_adapter) {
    ...
    adapter.dummy_dma.phys_addr);
    adapter.dummy_dma.addr = core::ptr::null_mut();
    } else
    dev_err(&adapter.pdev.dev, "dma_watchdog_shutdown failed\n");
    }
#[no_mangle]
pub unsafe extern "C" fn netxen_phantom_init(adapter: *mut netxen_adapter, pegtune_val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_init_firmware(adapter: *mut netxen_adapter) -> c_int {
...
pub const TCP_HDR_SIZE: c_int = 20;
pub const TCP_TS_OPTION_SIZE: c_int = 12;
...
#[no_mangle]
pub unsafe extern "C" fn netxen_process_cmd_ring(adapter: *mut netxen_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_nic_clear_stats(adapter: *mut netxen_adapter) {
