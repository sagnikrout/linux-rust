...
//! Source: drivers/net/ethernet/qlogic/netxen/netxen_nic_ctx.c
#![no_std]
...
pub const NXHAL_VERSION: c_int = 1;
    static u32
    netxen_poll_rsp(struct netxen_adapter *adapter)
    {
    let mut rsp: u32 = NX_CDRP_RSP_OK;
...
// give atleast 1ms for firmware to respond
    ...
    let mut rcode: u32 = NX_RCODE_SUCCESS;
...
// Release semaphore
    ...
    let mut count: c_int = adapter.mdump.md_template_size/sizeof(uint32_t) ;
    while (count-- > 0)
    sum += *buff++ ;
    while (sum >> 32)
...
// sw_int_mask
    ...
    int ring;
    let mut port: c_int = adapter.portnum;
...
#[no_mangle]
pub unsafe extern "C" fn netxen_alloc_hw_resources(adapter: *mut netxen_adapter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn netxen_free_hw_resources(adapter: *mut netxen_adapter) {
...
// Allow dma queues to drain after context reset
    ...
    recv_ctx = &adapter.recv_ctx;
    if (recv_ctx.hwctx != core::ptr::null_mut()) {
    dma_free_coherent(&adapter.pdev.dev,
    sizeof(struct netxen_ring_ctx) + sizeof(uint32_t),
    recv_ctx.hwctx, recv_ctx.phys_addr);
    recv_ctx.hwctx = core::ptr::null_mut();
    }
    tx_ring = adapter.tx_ring;
