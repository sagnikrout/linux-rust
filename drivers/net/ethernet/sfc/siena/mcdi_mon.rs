...
//! Source: drivers/net/ethernet/sfc/siena/mcdi_mon.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn efx_siena_mcdi_sensor_event(efx: *mut efx_nic, ev: *mut efx_qword_t) {
    void efx_siena_mcdi_sensor_event(struct efx_nic *efx, efx_qword_t *ev)
    {
    unsigned int type, state, value;
    let mut hwmon_type: enum efx_hwmon_type = EFX_HWMON_UNKNOWN;
    const char *name = core::ptr::null_mut(), *state_txt, *unit;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct efx_mcdi_mon_attribute {
    pub dev_attr: device_attribute,
    pub index: c_uint,
    pub type: c_uint,
    pub hwmon_type: enum efx_hwmon_type,
    pub limit_value: c_uint,
    pub name: [c_char; 12],
}

#[no_mangle]
unsafe extern "C" fn efx_mcdi_mon_update(efx: *mut efx_nic) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn efx_siena_mcdi_mon_probe(efx: *mut efx_nic) -> c_int {
    int efx_siena_mcdi_mon_probe(struct efx_nic *efx)
    {
    let mut n_temp: c_uint = 0, n_cool = 0, n_in = 0, n_curr = 0, n_power = 0;
    struct efx_mcdi_mon *hwmon = efx_mcdi_mon(efx);
...
#[no_mangle]
pub unsafe extern "C" fn efx_siena_mcdi_mon_remove(efx: *mut efx_nic) {
