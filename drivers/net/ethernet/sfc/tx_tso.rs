...
//! Source: drivers/net/ethernet/sfc/tx_tso.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tso_state {
// Output position
    pub out_len: c_uint,
    pub seqnum: c_uint,
    pub ipv4_id: u16,
    pub packet_space: c_uint,
// Input position
    pub dma_addr: dma_addr_t,
    pub in_len: c_uint,
    pub unmap_len: c_uint,
    pub unmap_addr: dma_addr_t,
    pub protocol: __be16,
    pub ip_off: c_uint,
    pub tcp_off: c_uint,
    pub header_len: c_uint,
    pub ip_base_len: c_uint,
    pub header_dma_addr: dma_addr_t,
    pub header_unmap_len: c_uint,
}

#[no_mangle]
pub unsafe extern "C" fn prefetch_ptr(tx_queue: *mut efx_tx_queue) {
    static inline void prefetch_ptr(struct efx_tx_queue *tx_queue)
    {
    let mut insert_ptr: c_uint = efx_tx_queue_get_insert_index(tx_queue);
    char *ptr;
...
//
#[no_mangle]
unsafe extern "C" fn efx_tso_check_protocol(skb: *mut sk_buff) -> __be16 {
...
pub const TCP_FLAGS_OFFSET: c_int = 13;
//
...
//
    ...
    let mut is_last: bool = st.out_len <= skb_shinfo(skb).gso_size;
