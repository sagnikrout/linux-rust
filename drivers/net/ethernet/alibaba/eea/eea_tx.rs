...
//! Source: drivers/net/ethernet/alibaba/eea/eea_tx.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct eea_tx_meta {
    pub next: *mut eea_tx_meta,
    pub id: u32,
    union {
    pub skb: *mut sk_buff,
    pub data: *mut c_void,
    ...
    let mut ts: skb_shared_hwtstamps = {};
    ...
    meta.data = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn eea_clean_tx(tx: *mut eea_net_tx, budget: c_int) -> c_int {
    static int eea_clean_tx(struct eea_net_tx *tx, int budget)
    {
    let mut stats: eea_sq_free_stats = {0};
    struct eea_tx_cdesc *desc;
    struct eea_tx_meta *meta;
    int desc_n;
    u16 id;
...
#[no_mangle]
pub unsafe extern "C" fn eea_poll_tx(tx: *mut eea_net_tx, budget: c_int) -> c_int {
    int eea_poll_tx(struct eea_net_tx *tx, int budget)
    {
    struct eea_net *enet = tx.enet;
    let mut index: u32 = tx - enet.tx;
...
#[no_mangle]
pub unsafe extern "C" fn if(SKB_GSO_TCPV6: sinfo->gso_type &) -> else {
    else if (sinfo.gso_type & SKB_GSO_TCPV6)
    desc.gso_type = EEA_TX_GSO_TCPV6;
#[no_mangle]
pub unsafe extern "C" fn if(SKB_GSO_UDP_L4: sinfo->gso_type &) -> else {
...
//
    ...
    return -ENOMEM;
    eea_tx_desc_fill(tx, head_meta, addr, len, is_last, core::ptr::null_mut(), 0);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn eea_tx_post_skb(tx: *mut eea_net_tx, skb: *mut sk_buff) -> c_int {
    static int eea_tx_post_skb(struct eea_net_tx *tx, struct sk_buff *skb)
    {
    const struct skb_shared_info *shinfo = skb_shinfo(skb);
    let mut hlen: u32 = skb_headlen(skb);
    struct eea_tx_meta *meta;
    const skb_frag_t *frag;
    dma_addr_t addr;
    let mut len: u32 = hlen;
    int i, err;
    u16 flags;
    bool last;
    if (len) {
...
//
    ...
    meta = eea_tx_desc_fill(tx, core::ptr::null_mut(), addr, len, last, skb, flags);
    ...
    frag = &shinfo.frags[i];
    let mut is_last: bool = i == (shinfo.nr_frags - 1);
...
#[no_mangle]
unsafe extern "C" fn eea_tx_kick(tx: *mut eea_net_tx) {
...
#[no_mangle]
pub unsafe extern "C" fn eea_tx_xmit(skb: *mut sk_buff, netdev: *mut net_device) -> netdev_tx_t {
...
#[no_mangle]
unsafe extern "C" fn eea_free_meta(tx: *mut eea_net_tx, cfg: *mut eea_net_cfg) {
    ...
    meta.skb = core::ptr::null_mut();
    for (i = 0; i < cfg.tx_ring_depth; i++) {
    meta = &tx.meta[i];
    if (!meta.skb)
    continue;
    eea_tx_meta_put_and_unmap(tx, meta);
    eea_meta_free_xmit(tx, meta, 0, core::ptr::null_mut(), &stats);
    }
    kvfree(tx.meta);
    tx.meta = core::ptr::null_mut();
    }
// Maybe called before eea_bind_q_and_cfg. So the cfg must be passed.
#[no_mangle]
pub unsafe extern "C" fn eea_free_tx(tx: *mut eea_net_tx, cfg: *mut eea_net_cfg) {
    ...
    tx.ering = core::ptr::null_mut();
    }
    if (tx.meta)
    eea_free_meta(tx, cfg);
    }
#[no_mangle]
pub unsafe extern "C" fn eea_alloc_tx(ctx: *mut eea_net_init_ctx, tx: *mut eea_net_tx, idx: u32) -> c_int {
