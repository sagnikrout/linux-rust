...
//! Source: drivers/net/ethernet/mellanox/mlx5/core/esw/bridge_mcast.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_mdb_init(bridge: *mut mlx5_esw_bridge) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_mdb_cleanup(bridge: *mut mlx5_esw_bridge) {
    ...
    struct mlx5_esw_bridge_mdb_entry *entry)
    {
    let mut err: c_int = xa_insert(&entry.ports, mlx5_esw_bridge_port_key(port), port, GFP_KERNEL);
    if (!err)
    entry.num_ports++;
    return err;
    }
    ...
    let mut num_dests: c_int = entry.num_ports, i = 0;
    ...
    entry.egress_handle = core::ptr::null_mut();
    }
    if (IS_ERR(handle))
    ...
    {
    let mut key: mlx5_esw_bridge_mdb_key = {};
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_bridge_port_mdb_flush(port: *mut mlx5_esw_bridge_port) {
...
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_mdb_flush(bridge: *mut mlx5_esw_bridge) {
...
#[no_mangle]
pub unsafe extern "C" fn if(ETH_P_8021AD: vlan_proto ==) -> else {
    ...
    {
    let mut from: c_uint = MLX5_ESW_BRIDGE_MCAST_TABLE_VLAN_GRP_IDX_FROM;
    let mut to: c_uint = MLX5_ESW_BRIDGE_MCAST_TABLE_VLAN_GRP_IDX_TO;
    ...
    struct mlx5_flow_table *mcast_ft)
    {
    let mut from: c_uint = MLX5_ESW_BRIDGE_MCAST_TABLE_QINQ_GRP_IDX_FROM;
    let mut to: c_uint = MLX5_ESW_BRIDGE_MCAST_TABLE_QINQ_GRP_IDX_TO;
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_bridge_port_mcast_fgs_init(port: *mut mlx5_esw_bridge_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_bridge_port_mcast_fgs_cleanup(port: *mut mlx5_esw_bridge_port) {
    ...
    handle = mlx5_add_flow_rules(port.mcast.ft, rule_spec, &flow_act, core::ptr::null_mut(), 0);
    kvfree(rule_spec);
    return handle;
    }
    static struct mlx5_flow_handle *
    ...
    struct mlx5_eswitch *tmp, *peer_esw = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_vlan_mcast_cleanup(vlan: *mut mlx5_esw_bridge_vlan) {
    ...
    vlan.mcast_handle = core::ptr::null_mut();
    }
    static struct mlx5_flow_handle *
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_bridge_port_mcast_fhs_init(port: *mut mlx5_esw_bridge_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_bridge_port_mcast_fhs_cleanup(port: *mut mlx5_esw_bridge_port) {
    ...
    port.mcast.fwd_handle = core::ptr::null_mut();
    if (port.mcast.filter_handle)
    mlx5_del_flow_rules(port.mcast.filter_handle);
    port.mcast.filter_handle = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_port_mcast_init(port: *mut mlx5_esw_bridge_port) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_port_mcast_cleanup(port: *mut mlx5_esw_bridge_port) {
    ...
    br_offloads.ingress_mld_fg = core::ptr::null_mut();
    if (br_offloads.ingress_igmp_fg)
    mlx5_destroy_flow_group(br_offloads.ingress_igmp_fg);
    br_offloads.ingress_igmp_fg = core::ptr::null_mut();
    }
    static struct mlx5_flow_handle *
...
// skip_ft = br_offloads->skip_ft;
    ...
    mld_query_handle = core::ptr::null_mut();
    mld_report_handle = core::ptr::null_mut();
    mld_done_handle = core::ptr::null_mut();
    }
    ...
    br_offloads.mld_done_handle = core::ptr::null_mut();
    if (br_offloads.mld_report_handle)
    mlx5_del_flow_rules(br_offloads.mld_report_handle);
    br_offloads.mld_report_handle = core::ptr::null_mut();
    if (br_offloads.mld_query_handle)
    mlx5_del_flow_rules(br_offloads.mld_query_handle);
    br_offloads.mld_query_handle = core::ptr::null_mut();
    if (br_offloads.igmp_handle)
    mlx5_del_flow_rules(br_offloads.igmp_handle);
    br_offloads.igmp_handle = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn mlx5_esw_brige_mcast_init(bridge: *mut mlx5_esw_bridge) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_brige_mcast_cleanup(bridge: *mut mlx5_esw_bridge) {
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_brige_mcast_global_enable(br_offloads: *mut mlx5_esw_bridge_offloads) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlx5_esw_brige_mcast_global_disable(br_offloads: *mut mlx5_esw_bridge_offloads) {
...
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_mcast_enable(bridge: *mut mlx5_esw_bridge) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mlx5_esw_bridge_mcast_disable(bridge: *mut mlx5_esw_bridge) {
