...
//! Source: drivers/net/ethernet/mellanox/mlx5/core/steering/sws/dr_cmd.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn mlx5dr_cmd_sync_steering(mdev: *mut mlx5_core_dev) -> c_int {
...
// obj_id = MLX5_GET(general_obj_out_cmd_hdr, out, obj_id);
    ...
    bool *extended_dest)
    {
    let mut fw_log_max_fdb_encap_uplink: c_int = MLX5_CAP_ESW(dev, log_max_fdb_encap_uplink);
    let mut num_fwd_destinations: c_int = 0;
    let mut num_encap: c_int = 0;
    int i;
// extended_dest = false;
...
// extended_dest = true;
    ...
    let mut extended_dest: bool = false;
    void *in_match_value;
    unsigned int inlen;
    int dst_cnt_size;
    void *in_dests;
    ...
    for (i = 0; i < fte.dests_size; i++) {
    let mut type: enum mlx5_flow_destination_type = fte.dest_arr[i].type;
