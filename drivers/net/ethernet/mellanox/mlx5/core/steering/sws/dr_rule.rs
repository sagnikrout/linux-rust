...
//! Source: drivers/net/ethernet/mellanox/mlx5/core/steering/sws/dr_rule.c
#![no_std]
...
pub const DR_RULE_MAX_STES_OPTIMIZED: c_int = 0;

pub const DR_RULE_MAX_STES_OPTIMIZED: c_int = 2;
...
// Link old STEs rule to the new ste
    ...
    let mut use_update_list: bool = false;
...
// Clean all ste_info's from the new table
    ...
    return core::ptr::null_mut(); /* Skip rehash, we already at the max size */
...
#[no_mangle]
unsafe extern "C" fn dr_rule_remove_action_members(rule: *mut mlx5dr_rule) {
...
#[no_mangle]
unsafe extern "C" fn dr_get_bits_per_mask(byte_mask: u16) -> u16 {
    ...
    let mut num_of_builders: u8 = nic_matcher.num_of_builders;
    struct mlx5dr_matcher *matcher = rule.matcher;
    struct mlx5dr_domain *dmn = matcher.tbl.dmn;
...
// Point current ste to the new action
    ...
    last_ste.next_htbl = core::ptr::null_mut();
    return 0;
    err_exit:
...
// new entry -> new branch
    ...
    struct list_head *miss_list;
    let mut skip_rehash: bool = false;
    struct mlx5dr_ste *ste;
    int index;
    nic_matcher = nic_rule.nic_matcher;
    nic_dmn = nic_matcher.nic_tbl.nic_dmn;
...
// Hash table index in use, add another collision (miss)
    ...
    {
    let mut match_criteria: u8 = matcher.match_criteria;
    let mut value_size: usize = value.match_sz;
...
#[no_mangle]
unsafe extern "C" fn dr_rule_destroy_rule_fdb(rule: *mut mlx5dr_rule) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dr_rule_destroy_rule(rule: *mut mlx5dr_rule) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dr_rule_get_ipv(spec: *mut mlx5dr_match_spec) -> enum mlx5dr_ipv {
    ...
    u32 flow_source)
    {
    let mut rx: bool = nic_type == DR_DOMAIN_NIC_TYPE_RX;
    ...
    struct mlx5dr_matcher_rx_tx *nic_matcher;
    struct mlx5dr_domain_rx_tx *nic_dmn;
    struct mlx5dr_ste_htbl *htbl = core::ptr::null_mut();
    struct mlx5dr_ste_htbl *cur_htbl;
    struct mlx5dr_ste *ste = core::ptr::null_mut();
    LIST_HEAD(send_ste_list);
    bool hw_ste_arr_is_opt;
    u8 *hw_ste_arr = core::ptr::null_mut();
    u32 new_hw_ste_arr_sz;
    int ret, i;
    nic_matcher = nic_rule.nic_matcher;
    nic_dmn = nic_matcher.nic_tbl.nic_dmn;
...
// Clean all ste_info's
    ...
    struct mlx5dr_action *actions[])
    {
    let mut copy_param: mlx5dr_match_param = {};
    int ret;
...
//
    ...
    {
    struct mlx5dr_domain *dmn = matcher.tbl.dmn;
    let mut param: mlx5dr_match_param = {};
...
#[no_mangle]
pub unsafe extern "C" fn mlx5dr_rule_destroy(rule: *mut mlx5dr_rule) -> c_int {
