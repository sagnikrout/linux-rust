...
//! Source: drivers/net/ethernet/mellanox/mlxsw/spectrum_flower.c
#![no_std]
...
// Copyright (c) 2017-2018 Mellanox Technologies. All rights reserved
    ...
    {
    const struct flow_action_entry *act;
    let mut mirror_act_count: c_int = 0;
    let mut police_act_count: c_int = 0;
    let mut sample_act_count: c_int = 0;
    int err, i;
...
//
    ...
    let mut chain_index: u32 = act.chain_index;
...
//
    ...
    let mut proto: u16 = be16_to_cpu(act.vlan.proto);
    let mut prio: u8 = act.vlan.prio;
    let mut vid: u16 = act.vlan.vid;
    ...
    let mut htype: enum flow_action_mangle_base = act.mangle.htype;
    let mut be_mask: __be32 = ( __be32) act.mangle.mask;
    let mut be_val: __be32 = ( __be32) act.mangle.val;
    let mut offset: u32 = act.mangle.offset;
    let mut mask: u32 = be32_to_cpu(be_mask);
    let mut val: u32 = be32_to_cpu(be_val);
...
//
    ...
    struct flow_cls_offload *f)
    {
    let mut ingress: bool = mlxsw_sp_flow_block_is_ingress_bound(block);
...
// No matchall filters installed on this chain.
    ...
    MLXSW_SP_ACL_PROFILE_FLOWER, core::ptr::null_mut());
    if (IS_ERR(ruleset))
    return PTR_ERR(ruleset);
    rule = mlxsw_sp_acl_rule_create(mlxsw_sp, ruleset, f.cookie, core::ptr::null_mut(),
    f.common.extack);
    if (IS_ERR(rule)) {
    err = PTR_ERR(rule);
    ...
    struct flow_cls_offload *f)
    {
    let mut used_hw_stats: enum flow_action_hw_stats = FLOW_ACTION_HW_STATS_DISABLED;
    struct mlxsw_sp_acl_ruleset *ruleset;
    struct mlxsw_sp_acl_rule *rule;
    u64 packets;
    u64 lastuse;
