...
//! Source: drivers/net/ethernet/mellanox/mlxsw/spectrum_dcb.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_dcbnl_getdcbx(dev: *mut net_device __always_unused) -> u8 {
...
// Egress configuration.
    for (i = 0; i < IEEE_8021QAZ_MAX_TCS; i++) {
    let mut dwrr: bool = ets.tc_tsa[i] == IEEE_8021QAZ_TSA_ETS;
    let mut weight: u8 = ets.tc_tx_bw[i];
...
// Ingress configuration.
    ...
    err_port_ets_set:
    for (i--; i >= 0; i--) {
    let mut dwrr: bool = my_ets.tc_tsa[i] == IEEE_8021QAZ_TSA_ETS;
    let mut weight: u8 = my_ets.tc_tx_bw[i];
    err = mlxsw_sp_port_ets_set(mlxsw_sp_port,
    MLXSW_REG_QEEC_HR_SUBGROUP, i,
    0, dwrr, weight);
    }
...
// Take the highest configured priority.
    ...
    enum mlxsw_reg_qpts_trust_state ts)
    {
    let mut rewrite_dscp: bool = ts == MLXSW_REG_QPTS_TRUST_STATE_DSCP;
    int err;
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_port_dcb_app_update(mlxsw_sp_port: *mut mlxsw_sp_port) -> c_int {
...
//
    ...
    {
    struct mlxsw_sp_port *mlxsw_sp_port = netdev_priv(dev);
    let mut pause_en: bool = mlxsw_sp_port_is_pause_en(mlxsw_sp_port);
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_dcbnl_getbuffer(dev: *mut net_device, buf: *mut dcbnl_buffer) -> c_int {
    ...
    let mut bytes: u32 = mlxsw_sp_cells_bytes(mlxsw_sp, hdroom.bufs.buf[i].size_cells);
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_dcbnl_setbuffer(dev: *mut net_device, buf: *mut dcbnl_buffer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_port_ets_init(mlxsw_sp_port: *mut mlxsw_sp_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_port_ets_fini(mlxsw_sp_port: *mut mlxsw_sp_port) {
    static void mlxsw_sp_port_ets_fini(struct mlxsw_sp_port *mlxsw_sp_port)
    {
    kfree(mlxsw_sp_port.dcb.ets);
    }
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_port_maxrate_init(mlxsw_sp_port: *mut mlxsw_sp_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_port_maxrate_fini(mlxsw_sp_port: *mut mlxsw_sp_port) {
    static void mlxsw_sp_port_maxrate_fini(struct mlxsw_sp_port *mlxsw_sp_port)
    {
    kfree(mlxsw_sp_port.dcb.maxrate);
    }
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_port_pfc_init(mlxsw_sp_port: *mut mlxsw_sp_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_port_pfc_fini(mlxsw_sp_port: *mut mlxsw_sp_port) {
    static void mlxsw_sp_port_pfc_fini(struct mlxsw_sp_port *mlxsw_sp_port)
    {
    kfree(mlxsw_sp_port.dcb.pfc);
    }
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_port_dcb_init(mlxsw_sp_port: *mut mlxsw_sp_port) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_port_dcb_fini(mlxsw_sp_port: *mut mlxsw_sp_port) {
