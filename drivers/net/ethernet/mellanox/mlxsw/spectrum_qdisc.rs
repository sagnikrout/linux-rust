...
//! Source: drivers/net/ethernet/mellanox/mlxsw/spectrum_qdisc.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qdisc_ops {
    pub type: enum mlxsw_sp_qdisc_type,
    int (*check_params)(struct mlxsw_sp_port *mlxsw_sp_port,
    pub params): *mut c_void,
    int (*replace)(struct mlxsw_sp_port *mlxsw_sp_port, u32 handle,
    pub params): *mut *mut mlxsw_sp_qdisc mlxsw_sp_qdisc, void,
    int (*destroy)(struct mlxsw_sp_port *mlxsw_sp_port,
    pub mlxsw_sp_qdisc): *mut mlxsw_sp_qdisc,
    int (*get_stats)(struct mlxsw_sp_port *mlxsw_sp_port,
    struct mlxsw_sp_qdisc *mlxsw_sp_qdisc,
    pub stats_ptr): *mut tc_qopt_offload_stats,
    int (*get_xstats)(struct mlxsw_sp_port *mlxsw_sp_port,
    struct mlxsw_sp_qdisc *mlxsw_sp_qdisc,
    pub xstats_ptr): *mut c_void,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qdisc_ets_band {
    pub prio_bitmap: u8,
    pub tclass_num: c_int,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qdisc_ets_data {
    pub bands: [mlxsw_sp_qdisc_ets_band; IEEE_8021QAZ_MAX_TCS],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qdisc {
    pub handle: u32,
    union {
    pub red: red_stats,
    pub xstats_base: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qdisc_stats {
    pub tx_bytes: u64,
    pub tx_packets: u64,
    pub drops: u64,
    pub overlimits: u64,
    ...
    pub stats_base: },
    union {
    pub ets_data: *mut mlxsw_sp_qdisc_ets_data,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qdisc_state {
    pub root_qdisc: mlxsw_sp_qdisc,
...
// FIFOs.
//
    pub future_handle: u32,
    pub future_fifos: [bool; IEEE_8021QAZ_MAX_TCS],
    pub /: *mut *mut mutex lock; / Protects qdisc state.,
}
...
pub const MLXSW_SP_PORT_DEFAULT_TCLASS: c_int = 0;
    ...
    let mut err_hdroom: c_int = 0;
    let mut err: c_int = 0;
    int i;
    if (!mlxsw_sp_qdisc)
    return 0;
    if (root_qdisc == mlxsw_sp_qdisc) {
    let mut hdroom: mlxsw_sp_hdroom = *mlxsw_sp_port.hdroom;
    ...
    mlxsw_sp_qdisc.ops = core::ptr::null_mut();
    mlxsw_sp_qdisc.num_classes = 0;
    kfree(mlxsw_sp_qdisc.qdiscs);
    mlxsw_sp_qdisc.qdiscs = core::ptr::null_mut();
    return err_hdroom ?: err;
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qdisc_tree_validate {
    pub forbid_ets: bool,
    pub forbid_root_tbf: bool,
    pub forbid_tbf: bool,
    pub forbid_red: bool,
}
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_qdisc_tree_validate(mlxsw_sp_port: *mut mlxsw_sp_port) -> c_int {
    static int mlxsw_sp_qdisc_tree_validate(struct mlxsw_sp_port *mlxsw_sp_port)
    {
    let mut validate: mlxsw_sp_qdisc_tree_validate = {};
    struct mlxsw_sp_qdisc *mlxsw_sp_qdisc;
    ...
    if (root_qdisc == mlxsw_sp_qdisc) {
    let mut hdroom: mlxsw_sp_hdroom = orig_hdroom;
...
//
    let mut bs512: u32 = max_size / 64;
    let mut bs: u8 = fls(bs512);
    if (!bs)
    return -EINVAL;
    --bs;
...
//
    ...
    struct mlxsw_sp *mlxsw_sp = mlxsw_sp_port.mlxsw_sp;
    let mut rate_kbps: u64 = mlxsw_sp_qdisc_tbf_rate_kbps(p);
    u8 burst_size;
    int err;
    ...
    if (err) {
    let mut highest_shaper_bs: u8 = MLXSW_REG_QEEC_HIGHEST_SHAPER_BS;
    dev_err(mlxsw_sp.bus_info.dev,
...
#[no_mangle]
pub unsafe extern "C" fn if(TC_FIFO_DESTROY: p->command ==) -> else {
    ...
    return mlxsw_sp_qdisc_replace(mlxsw_sp_port, p.handle,
    mlxsw_sp_qdisc,
    &mlxsw_sp_qdisc_ops_fifo, core::ptr::null_mut());
    }
    if (!mlxsw_sp_qdisc_compare(mlxsw_sp_qdisc, p.handle))
    return -EOPNOTSUPP;
    switch (p.command) {
    ...
    mlxsw_sp_qdisc.ets_data = core::ptr::null_mut();
    return 0;
    }
    static int
    ...
    let mut tclass_num: c_int = MLXSW_SP_PRIO_BAND_TO_TCLASS(band);
    ...
    u32 parent)
    {
    let mut child_index: c_int = TC_H_MIN(parent);
    let mut band: c_int = child_index - 1;
    ...
    {
    let mut band: c_uint = child - mlxsw_sp_qdisc.qdiscs;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qevent_block {
    pub binding_list: list_head,
    pub mall_entry_list: list_head,
    pub mlxsw_sp: *mut mlxsw_sp,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_qevent_binding {
    pub list: list_head,
    pub mlxsw_sp_port: *mut mlxsw_sp_port,
    pub handle: u32,
    pub tclass_num: c_int,
    pub span_trigger: enum mlxsw_sp_span_trigger,
    ...
    int *p_span_id)
    {
    let mut span_trigger: enum mlxsw_sp_span_trigger = qevent_binding.span_trigger;
    struct mlxsw_sp_port *mlxsw_sp_port = qevent_binding.mlxsw_sp_port;
    let mut trigger_parms: mlxsw_sp_span_trigger_parms = {};
    bool ingress;
    int span_id;
    int err;
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_qevent_block_deconfigure(qevent_block: *mut mlxsw_sp_qevent_block) {
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_qevent_block_cb(type: enum tc_setup_type, type_data: *mut c_void, cb_priv: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp_qevent_block_release(cb_priv: *mut c_void) {
    ...
    struct flow_block_offload *f)
    {
    let mut action_mask: c_uint = BIT(MLXSW_SP_MALL_ACTION_TYPE_MIRROR);
    return mlxsw_sp_setup_tc_block_qevent(mlxsw_sp_port, f,
    MLXSW_SP_SPAN_TRIGGER_ECN,
    action_mask);
    }
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_tc_qdisc_init(mlxsw_sp_port: *mut mlxsw_sp_port) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_tc_qdisc_fini(mlxsw_sp_port: *mut mlxsw_sp_port) {
