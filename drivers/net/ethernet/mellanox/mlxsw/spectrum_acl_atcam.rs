...
//! Source: drivers/net/ethernet/mellanox/mlxsw/spectrum_acl_atcam.c
#![no_std]
...
pub const MLXSW_SP_ACL_ATCAM_LKEY_ID_BLOCK_CLEAR_START: c_int = 0;
pub const MLXSW_SP_ACL_ATCAM_LKEY_ID_BLOCK_CLEAR_END: c_int = 5;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_acl_atcam_lkey_id_ht_key {
    pub /: *mut *mut char enc_key[MLXSW_REG_PTCEX_FLEX_KEY_BLOCKS_LEN]; / MSB blocks,
    pub erp_id: u8,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_acl_atcam_lkey_id {
    pub ht_node: rhash_head,
    pub ht_key: mlxsw_sp_acl_atcam_lkey_id_ht_key,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_acl_atcam_region_ops {
    pub aregion): *mut *mut int (init)(struct mlxsw_sp_acl_atcam_region,
    pub aregion): *mut *mut void (fini)(struct mlxsw_sp_acl_atcam_region,
    struct mlxsw_sp_acl_atcam_lkey_id *
    (*lkey_id_get)(struct mlxsw_sp_acl_atcam_region *aregion,
    pub erp_id): *mut *mut char enc_key, u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_acl_atcam_region_generic {
    pub dummy_lkey_id: mlxsw_sp_acl_atcam_lkey_id,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_acl_atcam_region_12kb {
    pub lkey_ht: rhashtable,
    pub max_lkey_id: c_uint,
    pub used_lkey_id: *mut c_ulong,
}
    ...
    let mut ht_key: mlxsw_sp_acl_atcam_lkey_id_ht_key = {{ 0 } };
...
// For now, just assume that every region has 12 key blocks
    let mut hw_region: u16 = region_id * 3;
    u64 max_regions;
...
#[no_mangle]
pub unsafe extern "C" fn if(4: blocks_count <=) -> else {
    else if (blocks_count <= 4)
    region_type = MLXSW_SP_ACL_ATCAM_REGION_TYPE_4KB;
#[no_mangle]
pub unsafe extern "C" fn if(8: blocks_count <=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_acl_atcam_region_fini(aregion: *mut mlxsw_sp_acl_atcam_region) {
...
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_acl_atcam_chunk_fini(achunk: *mut mlxsw_sp_acl_atcam_chunk) {
    ...
    {
    struct mlxsw_sp_acl_tcam_region *region = aregion.region;
    let mut erp_id: u8 = mlxsw_sp_acl_erp_mask_erp_id(aentry.erp_mask);
    struct mlxsw_sp_acl_atcam_lkey_id *lkey_id;
...
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_acl_atcam_rehash_hints_put(hints_priv: *mut c_void) {
