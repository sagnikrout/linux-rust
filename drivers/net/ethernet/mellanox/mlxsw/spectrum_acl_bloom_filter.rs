...
//! Source: drivers/net/ethernet/mellanox/mlxsw/spectrum_acl_bloom_filter.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mlxsw_sp_acl_bf {
    pub /: *mut *mut mutex lock; / Protects Bloom Filter updates.,
    pub bank_size: c_uint,
...
// +-------------------------+------------------------+------------------------+
//
pub const MLXSW_BLOOM_KEY_CHUNKS: c_int = 3;
// Spectrum-2 and Spectrum-3 chunks
pub const MLXSW_SP2_BLOOM_KEY_LEN: c_int = 69;
...
// +---------+-----------+----------+-----------------------------------+
//
pub const MLXSW_SP2_BLOOM_CHUNK_PAD_BYTES: c_int = 3;
pub const MLXSW_SP2_BLOOM_CHUNK_KEY_BYTES: c_int = 18;
pub const MLXSW_SP2_BLOOM_KEY_CHUNK_BYTES: c_int = 23;
...
//
pub const MLXSW_SP2_BLOOM_CHUNK_KEY_OFFSET: c_int = 5;
...
// Spectrum-4 chunks
pub const MLXSW_SP4_BLOOM_KEY_LEN: c_int = 60;
...
// +-----------+----------+-----------------------------------+
//
pub const MLXSW_SP4_BLOOM_CHUNK_PAD_BYTES: c_int = 0;
pub const MLXSW_SP4_BLOOM_CHUNK_KEY_BYTES: c_int = 18;
pub const MLXSW_SP4_BLOOM_KEY_CHUNK_BYTES: c_int = 20;
// The offset of the key block within a chunk is 2 bytes as it comes after
// 16 bits of region ID and eRP ID.
//
pub const MLXSW_SP4_BLOOM_CHUNK_KEY_OFFSET: c_int = 2;
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp2_acl_bf_crc16_byte(crc: u16, c: u8) -> u16 {
    static u16 mlxsw_sp2_acl_bf_crc16_byte(u16 crc, u8 c)
    {
    return (crc << 8) ^ mlxsw_sp2_acl_bf_crc16_tab[(crc >> 8) ^ c];
    }
#[no_mangle]
unsafe extern "C" fn mlxsw_sp2_acl_bf_crc(buffer: *const u8, len: usize) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp4_acl_bf_crc10_byte(crc: u16, c: u8) -> u16 {
    static u16 mlxsw_sp4_acl_bf_crc10_byte(u16 crc, u8 c)
    {
    let mut index: u8 = ((crc >> 2) ^ c) & 0xff;
    return ((crc << 8) ^ mlxsw_sp4_acl_bf_crc10_tab[index]) & 0x3ff;
    }
#[no_mangle]
unsafe extern "C" fn mlxsw_sp4_acl_bf_crc6_byte(crc: u16, c: u8) -> u16 {
    static u16 mlxsw_sp4_acl_bf_crc6_byte(u16 crc, u8 c)
    {
    let mut index: u8 = (crc ^ c) & 0xff;
    return ((crc << 6) ^ (mlxsw_sp4_acl_bf_crc6_tab[index] << 2)) & 0xfc;
    }
#[no_mangle]
unsafe extern "C" fn mlxsw_sp4_acl_bf_crc(buffer: *const u8, len: usize) -> u16 {
    static u16 mlxsw_sp4_acl_bf_crc(const u8 *buffer, size_t len)
    {
    let mut crc_row: u16 = 0, crc_col = 0;
    while (len--) {
...
#[no_mangle]
unsafe extern "C" fn right_shift_array(arr: *mut c_char, len: u8, shift_bits: u8) {
    static void right_shift_array(char *arr, u8 len, u8 shift_bits)
    {
    let mut byte_mask: u8 = 0xff >> shift_bits;
    int i;
...
#[no_mangle]
unsafe extern "C" fn mlxsw_sp4_bf_key_shift_chunks(chunk_count: u8, output: *mut c_char) {
...
//
    ...
    {
    struct mlxsw_afk_key_info *key_info = aregion.region.key_info;
    let mut block_count: u8 = mlxsw_afk_key_info_blocks_count_get(key_info);
    let mut chunk_count: u8 = 1 + ((block_count - 1) >> 2);
    __mlxsw_sp_acl_bf_key_encode(aregion, aentry, output, len,
    MLXSW_BLOOM_KEY_CHUNKS,
...
#[no_mangle]
pub unsafe extern "C" fn mlxsw_sp_acl_bf_fini(bf: *mut mlxsw_sp_acl_bf) {
