...
pub const GENEVE_VER: c_int = 0;

pub const GENEVE_OPT_NETDEV_CLASS: c_uint = 0x100;
pub const GENEVE_OPT_GRO_HINT_SIZE: c_int = 8;
pub const GENEVE_OPT_GRO_HINT_TYPE: c_int = 1;
pub const GENEVE_OPT_GRO_HINT_LEN: c_int = 1;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_opt_gro_hint {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_skb_cb {
    pub gro_hint_len: c_uint,
    pub gro_hint: geneve_opt_gro_hint,
}

// per-network namespace private data for this module
#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_net {
    pub geneve_list: list_head,
    pub sock_list: list_head,
    pub lock: mutex,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_dev_node {
    pub hlist: hlist_node,
    pub geneve: *mut geneve_dev,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_config {
    pub collect_md: bool,
    pub dualstack: bool,
    pub use_udp6_rx_checksums: bool,
    pub ttl_inherit: bool,
    pub gro_hint: bool,
    pub df: enum ifla_geneve_df,
    pub inner_proto_inherit: bool,
    pub port_min: u16,
    pub port_max: u16,
    pub rcu: rcu_head,
// Must be last --ends in a flexible-array member.
    pub info: ip_tunnel_info,
}

// Pseudo network device
#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_dev {
    pub /: *mut *mut geneve_dev_node hlist4; / vni hash table for IPv4 socket,

    pub /: *mut *mut geneve_dev_node hlist6; / vni hash table for IPv6 socket,

    pub /: *mut *mut *mut net net; / netns for packet i/o,
    pub /: *mut *mut *mut net_device dev; / netdev for geneve tunnel,
    pub /: *mut *mut *mut geneve_sock __rcu sock4; / IPv4 socket used for geneve tunnel,

    pub /: *mut *mut *mut geneve_sock __rcu sock6; / IPv6 socket used for geneve tunnel,

    pub /: *mut *mut list_head next; / geneve's per namespace list,
    pub gro_cells: gro_cells,
    pub cfg: *mut geneve_config __rcu,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct geneve_sock {
    pub collect_md: bool,
    pub gro_hint: bool,
    pub list: list_head,
    pub sk: *mut sock,
    ...
    htons(ETH_P_IP) };
#[no_mangle]
unsafe extern "C" fn proto_to_id(proto: __be16) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn geneve_net_vni_hash(vni[3]: u8) -> __u32 {
...
#[no_mangle]
unsafe extern "C" fn vni_to_tunnel_id(vni: *const __u8) -> __be64 {
    static __be64 vni_to_tunnel_id(const __u8 *vni)
    {

    return (vni[0] << 16) | (vni[1] << 8) | vni[2];

    return ( __be64)((( u64)vni[0] << 40) |
    (( u64)vni[1] << 48) |
    (( u64)vni[2] << 56));
...
#[no_mangle]
unsafe extern "C" fn tunnel_id_to_vni(tun_id: __be64, vni: *mut __u8) {
    ...
    vni[0] = ( __u8)(( u64)tun_id >> 40);
    vni[1] = ( __u8)(( u64)tun_id >> 48);
    vni[2] = ( __u8)(( u64)tun_id >> 56);

    }
#[no_mangle]
unsafe extern "C" fn eq_tun_id_and_vni(tun_id: *mut u8, vni: *mut u8) -> bool {
    static bool eq_tun_id_and_vni(u8 *tun_id, u8 *vni)
    {
    return !memcmp(vni, &tun_id[5], 3);
    }
#[no_mangle]
unsafe extern "C" fn geneve_get_sk_family(gs: *mut geneve_sock) -> sa_family_t {
...
// geneve receive/decap routine
    ...
    {
    struct metadata_dst *tun_dst = core::ptr::null_mut();
    unsigned int len;
    int nh, err = 0;
    void *oiph;
...
#[no_mangle]
unsafe extern "C" fn geneve_init(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn geneve_uninit(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn geneve_hlen(gh: *const genevehdr) -> c_int {
...
// Adjust the nested IP{6} hdr to actual GSO len.
    ...
    let mut old_len: __be16 = iph.tot_len;
...
#[no_mangle]
unsafe extern "C" fn geneve_udp_encap_recv(sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn geneve_udp_encap_err_lookup(sk: *mut sock, skb: *mut sk_buff) -> c_int {
...
//
    ...
    (( u32)iph.saddr ^ ( u32)iph2.saddr) |
    (( u32)iph.daddr ^ ( u32)iph2.daddr))
    return false;
    }
...
//
    if (skb.encapsulation) {
    let mut inner_nh: c_uint = nhoff + gh_len;
...
#[no_mangle]
unsafe extern "C" fn __geneve_sock_release(geneve: *mut geneve_dev, ipv6: bool) {
    ...
    rcu_assign_pointer(geneve.sock6, core::ptr::null_mut());
    node = &geneve.hlist6;
    } else

    {
    gs = rtnl_dereference(geneve.sock4);
    rcu_assign_pointer(geneve.sock4, core::ptr::null_mut());
    node = &geneve.hlist4;
    }
    if (!gs)
    return;
...
#[no_mangle]
unsafe extern "C" fn geneve_sock_release(geneve: *mut geneve_dev) {
    ...
    struct geneve_net *gn = net_generic(net, geneve_net_id);
    const struct ip_tunnel_info *info = &cfg.info;
    let mut family: sa_family_t = ipv6 ? AF_INET6 : AF_INET;
    let mut gro_hint: bool = cfg.gro_hint;
    let mut dst_port: __be16 = info.key.tp_dst;
    struct geneve_sock *gs;
...
#[no_mangle]
unsafe extern "C" fn geneve_open(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn geneve_stop(dev: *mut net_device) -> c_int {
...
//
    ...
    if (nested_is_v6) {
    let mut start: c_int = skb_network_offset(skb) + sizeof(struct ipv6hdr);
    let mut proto: u8 = ipv6_hdr(skb).nexthdr;
    __be16 foff;
...
// geneve_build_header() did not took in account the GRO hint.
    ...
    {
    let mut udp_sum: bool = test_bit(IP_TUNNEL_CSUM_BIT, info.key.tun_flags);
    let mut inner_proto_inherit: bool = cfg.inner_proto_inherit;
    let mut xnet: bool = !net_eq(geneve.net, dev_net(geneve.dev));
    struct geneve_skb_cb *cb = GENEVE_SKB_CB(skb);
...
// use_cache = false;
    ...
    use_cache ?
    (struct dst_cache *)&info.dst_cache : core::ptr::null_mut());
    if (IS_ERR(rt))
    return PTR_ERR(rt);
...
#[no_mangle]
unsafe extern "C" fn geneve_xmit(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
    static netdev_tx_t geneve_xmit(struct sk_buff *skb, struct net_device *dev)
    {
    struct geneve_dev *geneve = netdev_priv(dev);
    const struct ip_tunnel_info *info = core::ptr::null_mut();
    const struct geneve_config *cfg;
    int err;
    rcu_read_lock();
    cfg = rcu_dereference(geneve.cfg);
...
#[no_mangle]
unsafe extern "C" fn geneve_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
    static int geneve_change_mtu(struct net_device *dev, int new_mtu)
    {
    if (new_mtu > dev.max_mtu)
    new_mtu = dev.max_mtu;
#[no_mangle]
pub unsafe extern "C" fn if(dev->min_mtu: new_mtu <) -> else {
...
#[no_mangle]
unsafe extern "C" fn geneve_fill_metadata_dst(dev: *mut net_device, skb: *mut sk_buff) -> c_int {
    ...
    tos,
    use_cache ? &info.dst_cache : core::ptr::null_mut());
    if (IS_ERR(rt))
    return PTR_ERR(rt);
    ip_rt_put(rt);
...
//
#[no_mangle]
unsafe extern "C" fn geneve_offload_rx_ports(dev: *mut net_device, push: bool) {
    ...
    if (!cfg)
    return ERR_PTR(-ENOMEM);
    cfg.info.dst_cache.cache = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn geneve_config_free(cfg: *mut geneve_config) {
...
#[no_mangle]
unsafe extern "C" fn geneve_config_free_rcu(head: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn geneve_free_dev(dev: *mut net_device) {
    static void geneve_free_dev(struct net_device *dev)
    {
    struct geneve_dev *geneve = netdev_priv(dev);
    struct geneve_config *cfg = rcu_dereference_protected(geneve.cfg, 1);
    geneve_config_free(cfg);
    RCU_INIT_POINTER(geneve.cfg, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn geneve_setup(dev: *mut net_device) {
...
//
    ...
    let mut vni: __u32 = nla_get_u32(data[IFLA_GENEVE_ID]);
    if (vni >= GENEVE_N_VID) {
    ...
    let mut df: enum ifla_geneve_df = nla_get_u8(data[IFLA_GENEVE_DF]);
...
#[no_mangle]
unsafe extern "C" fn geneve_saddr_wildcard(info: *const ip_tunnel_info) -> bool {
    ...
    bool *tun_collect_md)
    {
    struct geneve_dev *geneve, *t = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn is_tnl_info_zero(info: *const ip_tunnel_info) -> bool {
...
#[no_mangle]
unsafe extern "C" fn init_tnl_info(info: *mut ip_tunnel_info, dst_port: __u16) {
    ...
    if (changelink) {
    let mut src: __be32 = nla_get_in_addr(data[IFLA_GENEVE_LOCAL]);
    ...
    if (changelink) {
    let mut src: in6_addr = nla_get_in6_addr(data[IFLA_GENEVE_LOCAL6]);
    if (ip_tunnel_info_af(info) == AF_INET ||
    ...
    case AF_INET: {
    let mut fl4: flowi4 = { .daddr = info.key.u.ipv4.dst };
    ...
    break;
    rt = rt6_lookup(geneve.net, &info.key.u.ipv6.dst, core::ptr::null_mut(), 0,
    core::ptr::null_mut(), 0);
    if (rt && rt.dst.dev)
...
#[no_mangle]
unsafe extern "C" fn geneve_dellink(dev: *mut net_device, head: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn geneve_get_size(dev: *const net_device) -> usize {
...
#[no_mangle]
unsafe extern "C" fn geneve_fill_info(skb: *mut sk_buff, dev: *const net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn geneve_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn geneve_exit_net(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn geneve_init_module() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn geneve_cleanup_module() -> void __exit {
