...
#[no_mangle]
unsafe extern "C" fn mii_get_an(mii: *mut mii_if_info, addr: u16) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn mii_ethtool_sset(mii: *mut mii_if_info, ecmd: *mut ethtool_cmd) -> c_int {
    int mii_ethtool_sset(struct mii_if_info *mii, struct ethtool_cmd *ecmd)
    {
    struct net_device *dev = mii.dev;
    let mut speed: u32 = ethtool_cmd_speed(ecmd);
    if (speed != SPEED_10 &&
    speed != SPEED_100 &&
    speed != SPEED_1000)
    return -EINVAL;
...
//
#[no_mangle]
pub unsafe extern "C" fn mii_check_gmii_support(mii: *mut mii_if_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn mii_link_ok(mii: *mut mii_if_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn mii_nway_restart(mii: *mut mii_if_info) -> c_int {
...
pub unsafe extern "C" fn mii_check_link(mii: *mut mii_if_info) {
    ...
    {
    let mut cur_link: c_int = mii_link_ok(mii);
    let mut prev_link: c_int = netif_carrier_ok(mii.dev);
    if (cur_link && !prev_link)
    netif_carrier_on(mii.dev);
#[no_mangle]
pub unsafe extern "C" fn if(!cur_link: prev_link &&) -> else {
...
//
    ...
    {
    let mut rc: c_int = 0;
    let mut duplex_changed: c_uint = 0;
    if (duplex_chg_out)
// duplex_chg_out = 0;
