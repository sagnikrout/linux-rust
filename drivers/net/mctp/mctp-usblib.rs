...
//! Source: drivers/net/mctp/mctp-usblib.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn mctp_usblib_rx_init(rx: *mut mctp_usblib_rx, ep_pktlen: u16, span: bool) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mctp_usblib_rx_fini(rx: *mut mctp_usblib_rx) {
...
#[no_mangle]
unsafe extern "C" fn mctp_usblib_rx(netdev: *mut net_device, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn mctp_usblib_rx_stats_single_drop(dev: *mut net_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn mctp_usblib_rx_cancel(rx: *mut mctp_usblib_rx) {
...
pub struct mctp_usblib_tx_ctx {
    pub tx: *mut mctp_usblib_tx,
    pub skbs: sk_buff_head,
    pub len: unsigned int buf_len,,
    enum mctp_usblib_tx_buf_type {
    TX_SINGLE,
    TX_FLAT,
    pub buf_type: },
    pub ____cacheline_aligned: u8 buf[],
}
...
#[no_mangle]
unsafe extern "C" fn mctp_usblib_tx_avail(ctx: *mut mctp_usblib_tx_ctx) -> c_int {
    static int mctp_usblib_tx_avail(struct mctp_usblib_tx_ctx *ctx)
    {
    return ctx.buf_type == TX_SINGLE ? 0 : ctx.buf_len - ctx.len;
    }
#[no_mangle]
unsafe extern "C" fn mctp_usblib_tx_should_send(ctx: *mut mctp_usblib_tx_ctx) -> bool {
...
//
    let mut pkt_len: usize = MCTP_USB_BTU + sizeof(struct mctp_usb_hdr);
...
#[no_mangle]
unsafe extern "C" fn mctp_usblib_tx_send(ctx: *mut mctp_usblib_tx_ctx) -> c_int {
...
//
    ...
    if (!ctx)
    return;
    while ((skb = __skb_dequeue(&ctx.skbs)) != core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn mctp_usblib_tx_fini(tx: *mut mctp_usblib_tx) {
...
// transfers.
//
    let mut TX_SPAN_MAX: static unsigned int = 4096 - sizeof(struct mctp_usblib_tx_ctx);
...
// tx_skb_prepare()
//
    let mut n: u64 = ctx.skbs.qlen;
    let mut len: i64 = ctx.len - (n * sizeof(struct mctp_usb_hdr));
...
#[no_mangle]
unsafe extern "C" fn mctp_usblib_tx_stats_single_drop(dev: *mut net_device) {
...
//
    ...
    {
    struct mctp_usblib_tx_ctx *ctx, *send_ctx = core::ptr::null_mut();
    enum skb_drop_reason reason;
    let mut max_tries: c_int = 3;
    unsigned long flags;
    let mut try: c_int = 1, rc;
...
// Cancel a tx: any un-sent context is released.
    ...
    enum skb_drop_reason reason)
    {
    struct mctp_usblib_tx_ctx *ctx = core::ptr::null_mut();
    unsigned long flags;
    spin_lock_irqsave(&tx.lock, flags);
    swap(tx.cur_ctx, ctx);
    spin_unlock_irqrestore(&tx.lock, flags);
