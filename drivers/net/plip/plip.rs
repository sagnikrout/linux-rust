...
//! Source: drivers/net/plip/plip.c
#![no_std]
...
pub const PLIP_MAX: c_int = 8;
// Use 0 for production, 1 for verification, >2 for debug

pub const NET_DEBUG: c_int = 1;

    let mut net_debug: static unsigned int = NET_DEBUG;

// In micro second
pub const PLIP_DELAY_UNIT: c_int = 1;
// Connection time out = PLIP_TRIGGER_WAIT * PLIP_DELAY_UNIT usec
pub const PLIP_TRIGGER_WAIT: c_int = 500;
// Nibble time out = PLIP_NIBBLE_WAIT * PLIP_DELAY_UNIT usec
pub const PLIP_NIBBLE_WAIT: c_int = 3000;
// Bottom halves
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct plip_local {
    pub state: enum plip_packet_state,
    pub nibble: enum plip_nibble_state,
    union {
    struct {
...
pub struct net_local {
    ...
    pub timer: delayed_work,
    pub snd_data: plip_local,
    pub rcv_data: plip_local,
    pub pardev: *mut pardevice,
    pub trigger: c_ulong,
    pub nibble: c_ulong,
    pub connection: enum plip_connection_state,
    pub timeout_count: c_ushort,
    pub is_deferred: c_int,
    pub port_owner: c_int,
    pub should_relinquish: c_int,
    pub lock: spinlock_t,
    pub kill_timer: core::sync::atomic::AtomicI32,
    pub killed_timer_cmp: completion,
}

#[no_mangle]
pub unsafe extern "C" fn enable_parport_interrupts(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn disable_parport_interrupts(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn write_data(dev: *mut net_device, data: c_uchar) {
...
#[no_mangle]
pub unsafe extern "C" fn read_status(dev: *mut net_device) -> c_uchar {
...
pub const ERROR: c_int = 2;
pub const HS_TIMEOUT: c_int = 3;
...
// Try again later
    ...
    rcv.skb = core::ptr::null_mut();
    }
    snd.state = PLIP_PK_DONE;
    if (snd.skb) {
    dev_consume_skb_irq(snd.skb);
    snd.skb = core::ptr::null_mut();
    }
    spin_unlock_irq(&nl.lock);
    if (error == HS_TIMEOUT) {
    DISABLE(dev.irq);
...
//
#[no_mangle]
unsafe extern "C" fn plip_type_trans(skb: *mut sk_buff, dev: *mut net_device) -> __be16 {
...
// PLIP_RECEIVE_PACKET --- receive a packet
    ...
    struct plip_local *snd, struct plip_local *rcv)
    {
    let mut nibble_timeout: c_ushort = nl.nibble;
    unsigned char *lbuf;
    switch (rcv.state) {
...
// PLIP_SEND_PACKET --- send a packet
    ...
    unsigned char c0;
    unsigned int cx;
    if (snd.skb == core::ptr::null_mut() || (lbuf = snd.skb.data) == core::ptr::null_mut()) {
...
// Initialize the state machine.
    ...
    nl.rcv_data.skb = nl.snd_data.skb = core::ptr::null_mut();
    nl.connection = PLIP_CN_NONE;
    nl.is_deferred = 0;
...
// Clear the data port.
    ...
    static struct net_device *dev_plip[PLIP_MAX] = { core::ptr::null_mut(), };
    static inline int
...
// available to use.
#[no_mangle]
unsafe extern "C" fn plip_attach(port: *mut parport) {
...
#[no_mangle]
unsafe extern "C" fn plip_detach(port: *mut parport) {
    static void plip_detach (struct parport *port)
    {
// Nothing to do
    }
#[no_mangle]
unsafe extern "C" fn plip_probe(par_dev: *mut pardevice) -> c_int {
    static int plip_probe(struct pardevice *par_dev)
    {
    struct device_driver *drv = par_dev.dev.driver;
    let mut len: c_int = strlen(drv.name);
...
#[no_mangle]
unsafe extern "C" fn plip_cleanup_module() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn plip_setup(str: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn plip_init() -> int __init {
