...
//! Source: drivers/net/wwan/wwan_core.c
#![no_std]
...
// WWAN port flags
pub const WWAN_PORT_TX_OFF: c_int = 0;
pub const WWAN_PORT_EXCLUSIVE: c_int = 1;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wwan_device {
    pub id: c_uint,
    pub refcount: c_int,
    pub dev: device,
    pub ops: *const wwan_ops,
    pub ops_ctxt: *mut c_void,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wwan_port {
    pub type: enum wwan_port_type,
    pub start_count: c_uint,
    pub flags: c_ulong,
    pub ops: *const wwan_port_ops,
    pub /: *mut *mut mutex ops_lock; / Serialize ops + protect against removal,
    pub dev: device,
    pub rxq: sk_buff_head,
    pub waitqueue: wait_queue_head_t,
    pub /: *mut *mut mutex data_lock; / Port specific data access serialization,
    pub headroom_len: usize,
    pub frag_len: usize,
    union {
    struct {
...
#[no_mangle]
unsafe extern "C" fn index_show(dev: *mut device, attr: *mut device_attribute, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn wwan_dev_destroy(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn wwan_dev_parent_match(dev: *mut device, parent: *const c_void) -> c_int {
    ...
    {
    struct device *dev;
    dev = class_find_device(&wwan_class, core::ptr::null_mut(), parent, wwan_dev_parent_match);
    if (!dev)
    return ERR_PTR(-ENODEV);
    return to_wwan_dev(dev);
    }
#[no_mangle]
unsafe extern "C" fn wwan_dev_name_match(dev: *mut device, name: *const c_void) -> c_int {
    ...
    {
    struct device *dev;
    dev = class_find_device(&wwan_class, core::ptr::null_mut(), name, wwan_dev_name_match);
    if (!dev)
    return ERR_PTR(-ENODEV);
    return to_wwan_dev(dev);
    }
...
#[no_mangle]
unsafe extern "C" fn wwan_dev_debugfs_match(dev: *mut device, dir: *const c_void) -> c_int {
    ...
    {
    struct device *dev;
    dev = class_find_device(&wwan_class, core::ptr::null_mut(), dir, wwan_dev_debugfs_match);
    if (!dev)
    return ERR_PTR(-ENODEV);
    return to_wwan_dev(dev);
    }
#[no_mangle]
pub unsafe extern "C" fn wwan_put_debugfs_dir(dir: *mut dentry) {
...
#[no_mangle]
unsafe extern "C" fn wwan_remove_dev(wwandev: *mut wwan_device) {
...
#[no_mangle]
unsafe extern "C" fn wwan_port_destroy(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn wwan_port_minor_match(dev: *mut device, minor: *const c_void) -> c_int {
    ...
    {
    struct device *dev;
    dev = class_find_device(&wwan_class, core::ptr::null_mut(), &minor, wwan_port_minor_match);
    if (!dev)
    return ERR_PTR(-ENODEV);
...
//
#[no_mangle]
unsafe extern "C" fn __wwan_port_dev_assign_name(port: *mut wwan_port, fmt: *const c_char) -> c_int {
    static int __wwan_port_dev_assign_name(struct wwan_port *port, const char *fmt)
    {
    struct wwan_device *wwandev = to_wwan_dev(port.dev.parent);
    let mut max_ports: c_uint = PAGE_SIZE * 8;
...
// Collect ids of same name format ports
    class_dev_iter_init(&iter, &wwan_class, core::ptr::null_mut(), &wwan_port_dev_type);
...
#[no_mangle]
unsafe extern "C" fn wwan_port_register_wwan(port: *mut wwan_port) -> c_int {
...
// Serialize ports registration
    ...
    port.dev.class = core::ptr::null_mut();
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn wwan_port_unregister_wwan(port: *mut wwan_port) {
    static void wwan_port_unregister_wwan(struct wwan_port *port)
    {
    struct wwan_device *wwandev = to_wwan_dev(port.dev.parent);
    dev_set_drvdata(&port.dev, core::ptr::null_mut());
    dev_info(&wwandev.dev, "port %s disconnected\n", dev_name(&port.dev));
    device_del(&port.dev);
    }

#[no_mangle]
unsafe extern "C" fn wwan_gnss_open(gdev: *mut gnss_device) -> c_int {
    static int wwan_gnss_open(struct gnss_device *gdev)
    {
    return wwan_port_op_start(gnss_get_drvdata(gdev));
    }
#[no_mangle]
unsafe extern "C" fn wwan_gnss_close(gdev: *mut gnss_device) {
    ...
    struct sk_buff *skb, *head = core::ptr::null_mut(), *tail = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn wwan_port_register_gnss(port: *mut wwan_port) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wwan_port_unregister_gnss(port: *mut wwan_port) {
...
#[no_mangle]
pub unsafe extern "C" fn wwan_remove_port(port: *mut wwan_port) {
    ...
    port.ops = core::ptr::null_mut(); /* Prevent any new port operations (e.g. from fops) */
...
#[no_mangle]
pub unsafe extern "C" fn wwan_port_rx(port: *mut wwan_port, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn wwan_port_txon(port: *mut wwan_port) {
...
#[no_mangle]
pub unsafe extern "C" fn wwan_port_txoff(port: *mut wwan_port) {
...
#[no_mangle]
unsafe extern "C" fn wwan_port_op_start(port: *mut wwan_port) -> c_int {
    static int wwan_port_op_start(struct wwan_port *port)
    {
    let mut ret: c_int = 0;
    mutex_lock(&port.ops_lock);
...
#[no_mangle]
unsafe extern "C" fn wwan_port_op_stop(port: *mut wwan_port) {
    static void wwan_port_op_stop(struct wwan_port *port)
    {
    mutex_lock(&port.ops_lock);
    port.start_count--;
...
#[no_mangle]
unsafe extern "C" fn wwan_wait_tx(port: *mut wwan_port, nonblock: bool) -> c_int {
    static int wwan_wait_tx(struct wwan_port *port, bool nonblock)
    {
...
#[no_mangle]
unsafe extern "C" fn wwan_port_fops_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wwan_port_fops_release(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wwan_port_fops_poll(filp: *mut file, wait: *mut poll_table) -> __poll_t {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !is_write_blocked(port)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(TIOCMBIS: cmd ==) -> else {
    ...
    case TIOCGEXCL:
    {
    let mut excl: c_int = test_bit(WWAN_PORT_EXCLUSIVE, &port.flags);
...
// AT and QCDM port specific IOCTLs
    ...
    let mut wwan_type: static struct device_type = { .name = "wwan" };
...
#[no_mangle]
unsafe extern "C" fn wwan_rtnl_dellink(dev: *mut net_device, head: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn wwan_rtnl_get_size(dev: *const net_device) -> usize {
...
// The next three parsing calls can not fail
    nlmsg_parse_deprecated(nlh, 0, tb, IFLA_MAX, core::ptr::null_mut(), core::ptr::null_mut());
    nla_parse_nested_deprecated(linkinfo, IFLA_INFO_MAX, tb[IFLA_LINKINFO],
    core::ptr::null_mut(), core::ptr::null_mut());
    nla_parse_nested_deprecated(data, IFLA_WWAN_MAX,
    linkinfo[IFLA_INFO_DATA], core::ptr::null_mut(), core::ptr::null_mut());
    rtnl_lock();
    dev = rtnl_create_link(&init_net, "wwan%d", NET_NAME_ENUM,
    &wwan_rtnl_link_ops, tb, core::ptr::null_mut());
    if (WARN_ON(IS_ERR(dev)))
    goto unlock;
    if (WARN_ON(wwan_rtnl_newlink(dev, &params, core::ptr::null_mut()))) {
    free_netdev(dev);
    goto unlock;
    }
    rtnl_configure_link(dev, core::ptr::null_mut(), 0, core::ptr::null_mut()); /* Link initialized, notify new link */
    unlock:
    rtnl_unlock();
    free_attrs:
    nlmsg_free(msg);
...
#[no_mangle]
unsafe extern "C" fn wwan_child_dellink(dev: *mut device, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn wwan_unregister_ops(parent: *mut device) {
...
// Remove all child netdev(s), using batch removing
    ...
    wwandev.ops = core::ptr::null_mut();	/* Finally remove ops */
    rtnl_unlock();
    wwandev.ops_ctxt = core::ptr::null_mut();
    wwan_remove_dev(wwandev);
    }
    EXPORT_SYMBOL_GPL(wwan_unregister_ops);
#[no_mangle]
unsafe extern "C" fn wwan_init() -> int __init {
...
// chrdev used for wwan ports
    ...
    wwan_debugfs_dir = debugfs_create_dir("wwan", core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn wwan_exit() -> void __exit {
