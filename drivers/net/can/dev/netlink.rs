...
//! Source: drivers/net/can/dev/netlink.c
#![no_std]
...
// sample point is in one-tenth of a percent
    ...
    {
    let mut tdc_manual: bool = tdc_flags & CAN_CTRLMODE_TDC_MANUAL_MASK;
    let mut tdc_auto: bool = tdc_flags & CAN_CTRLMODE_TDC_AUTO_MASK;
    int err;
...
//
    ...
    let mut tms_conflicts: u32 = masked_flags & tms_conflicts_mask;
    if (tms_conflicts) {
    NL_SET_ERR_MSG_FMT(extack,
...
// Wipe potential leftovers from previous CAN FD/XL config
    ...
    return err;
    if (tb_tdc[IFLA_CAN_TDC_TDCV]) {
    let mut tdcv: u32 = nla_get_u32(tb_tdc[IFLA_CAN_TDC_TDCV]);
    if (tdcv < tdc_const.tdcv_min || tdcv > tdc_const.tdcv_max)
    return -EINVAL;
    tdc.tdcv = tdcv;
    }
    if (tb_tdc[IFLA_CAN_TDC_TDCO]) {
    let mut tdco: u32 = nla_get_u32(tb_tdc[IFLA_CAN_TDC_TDCO]);
    if (tdco < tdc_const.tdco_min || tdco > tdc_const.tdco_max)
    return -EINVAL;
    tdc.tdco = tdco;
    }
    if (tb_tdc[IFLA_CAN_TDC_TDCF]) {
    let mut tdcf: u32 = nla_get_u32(tb_tdc[IFLA_CAN_TDC_TDCF]);
    ...
    let mut need_tdc_calc: bool = false;
    u32 tdc_mask;
    int err;
    if (fd) {
...
// Finally, set the bit-timing registers
    ...
    let mut restart_ms: c_uint = nla_get_u32(data[IFLA_CAN_RESTART_MS]);
...
// CAN XL
    ...
    let mut termval: u16 = nla_get_u16(data[IFLA_CAN_TERMINATION]);
    let mut num_term: c_uint = priv.termination_const_cnt;
    unsigned int i;
...
#[no_mangle]
unsafe extern "C" fn can_ctrlmode_ext_get_size() -> usize {
...
#[no_mangle]
unsafe extern "C" fn can_get_size(dev: *const net_device) -> usize {
...
#[no_mangle]
unsafe extern "C" fn can_pwm_fill_info(skb: *mut sk_buff, priv: *const can_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn can_fill_info(skb: *mut sk_buff, dev: *const net_device) -> c_int {
    static int can_fill_info(struct sk_buff *skb, const struct net_device *dev)
    {
    struct can_priv *priv = netdev_priv(dev);
    let mut cm: can_ctrlmode = {.flags = priv.ctrlmode};
    let mut bec: can_berr_counter = { };
    let mut state: enum can_state = priv.state;
...
#[no_mangle]
unsafe extern "C" fn can_get_xstats_size(dev: *const net_device) -> usize {
    static size_t can_get_xstats_size(const struct net_device *dev)
    {
    return sizeof(struct can_device_stats);
    }
#[no_mangle]
unsafe extern "C" fn can_fill_xstats(skb: *mut sk_buff, dev: *const net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn can_dellink(dev: *mut net_device, head: *mut list_head) {
...
#[no_mangle]
pub unsafe extern "C" fn can_netlink_register() -> c_int {
    int can_netlink_register(void)
    {
    return rtnl_link_register(&can_link_ops);
    }
#[no_mangle]
pub unsafe extern "C" fn can_netlink_unregister() {
