...
//! Source: drivers/net/can/dev/bittiming.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn can_sjw_set_default(bt: *mut can_bittiming) {
...
//
    ...
    {
    let mut tseg1: c_uint = bt.prop_seg + bt.phase_seg1;
...
//
    ...
    {
    const struct can_priv *priv = netdev_priv(dev);
    let mut xl_bit_time_tqmin: u32 = can_bit_time_tqmin(&priv.xl.data_bittiming);
    let mut nom_bit_time_tqmin: u32 = can_bit_time_tqmin(&priv.bittiming);
    let mut pwms_ns: u32 = can_tqmin_to_ns(pwm.pwms, priv.clock.freq);
    let mut pwml_ns: u32 = can_tqmin_to_ns(pwm.pwml, priv.clock.freq);
