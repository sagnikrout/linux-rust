...
//! Source: drivers/net/can/usb/ucan.c
#![no_std]
...
pub const UCAN_MAX_RX_URBS: c_int = 8;
// the CAN controller needs a while to enable/disable the bus
pub const UCAN_USB_CTL_PIPE_TIMEOUT: c_int = 1000;
// this driver currently supports protocol version 3 only
pub const UCAN_PROTOCOL_VERSION_MIN: c_int = 3;
pub const UCAN_PROTOCOL_VERSION_MAX: c_int = 3;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_ctl_cmd_start {
    pub /: *mut *mut *mut __le16 mode; / OR-ing any of UCAN_MODE_,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_ctl_cmd_set_bittiming {
    pub /: *mut *mut __le32 tq; / Time quanta (TQ) in nanoseconds,
    pub /: *mut *mut __le16 brp; / TQ Prescaler,
    pub /: *mut *mut __le16 sample_point; / Samplepoint on tenth percent,
    pub /: *mut *mut u8 prop_seg; / Propagation segment in TQs,
    pub /: *mut *mut u8 phase_seg1; / Phase buffer segment 1 in TQs,
    pub /: *mut *mut u8 phase_seg2; / Phase buffer segment 2 in TQs,
    pub /: *mut *mut u8 sjw; / Synchronisation jump width in TQs,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_ctl_cmd_device_info {
    pub /: *mut *mut __le32 freq; / Clock Frequency for tq generation,
    pub /: *mut *mut u8 tx_fifo; / Size of the transmission fifo,
    pub /: *mut *mut u8 sjw_max; / can_bittiming fields...,
    pub tseg1_min: u8,
    pub tseg1_max: u8,
    pub tseg2_min: u8,
    pub tseg2_max: u8,
    pub brp_inc: __le16,
    pub brp_min: __le32,
    pub /: *mut *mut __le32 brp_max; / ...can_bittiming fields,
    pub /: *mut *mut __le16 ctrlmodes; / supported control modes,
    pub /: *mut *mut __le16 hwfilter; / Number of HW filter banks,
    pub /: *mut *mut __le16 rxmboxes; / Number of receive Mailboxes,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_ctl_cmd_get_protocol_version {
    pub version: __le32,
    pub __packed: },
...
//
    pub cmd_start: ucan_ctl_cmd_start,
// Setup Bittiming
// bmRequest == UCAN_COMMAND_SET_BITTIMING
//
    pub cmd_set_bittiming: ucan_ctl_cmd_set_bittiming,
// Get Device Information
// bmRequest == UCAN_COMMAND_GET; wValue = UCAN_COMMAND_GET_INFO
//
    pub cmd_get_device_info: ucan_ctl_cmd_device_info,
// Get Protocol Version
// bmRequest == UCAN_COMMAND_GET;
// wValue = UCAN_COMMAND_GET_PROTOCOL_VERSION
//
    pub cmd_get_protocol_version: ucan_ctl_cmd_get_protocol_version,
    pub fw_str: [u8; 128],
    pub __packed: },
    enum {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_tx_complete_entry_t {
    pub echo_index: u8,
    pub flags: u8,
    pub __aligned(0x2): } __packed,
// CAN Data message format within ucan_message_in/out
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_can_msg {
...
//
    pub id: __le32,
    union {
    pub /: *mut *mut u8 data[CAN_MAX_DLEN]; / Data of CAN frames,
    pub /: *mut *mut u8 dlc; / RTR dlc,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_message_out {
    pub /: *mut *mut __le16 len; / Length of the content include header,
    pub /: *mut *mut u8 type; / UCAN_OUT_TX and friends,
    pub /: *mut *mut u8 subtype; / command sub type,
    union {
// Transmit CAN frame
// (type == UCAN_TX) && ((msg.can_msg.id & CAN_RTR_FLAG) == 0)
// subtype stores the echo id
//
    pub can_msg: ucan_can_msg,
    pub msg: },
    pub __aligned(0x4): } __packed,
// IN Endpoint, inbound messages
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_message_in {
    pub /: *mut *mut __le16 len; / Length of the content include header,
    pub /: *mut *mut u8 type; / UCAN_IN_RX and friends,
    pub /: *mut *mut u8 subtype; / command sub type,
    union {
// CAN Frame received
// (type == UCAN_IN_RX)
...
//
    ...
    pub msg: } __aligned(0x4),
    pub __aligned(0x4): } __packed,
// Macros to calculate message lengths

    pub ucan_priv: struct,
// Context Information for transmission URBs
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_urb_context {
    pub up: *mut ucan_priv,
    pub allocated: bool,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ucan_device_info {
    pub bittiming_const: can_bittiming_const,
    pub tx_fifo: u8,
}
...
// transmission and reception buffers
    ...
    pub tx_urbs: usb_anchor,
    pub ctl_msg_buffer: *mut union ucan_ctl_payload,
    pub device_info: ucan_device_info,
// transmission control information and locks
    pub context_lock: spinlock_t,
    pub available_tx_urbs: c_uint,
    pub context_array: *mut ucan_urb_context,
}

#[no_mangle]
unsafe extern "C" fn ucan_can_cc_dlc2len(msg: *mut ucan_can_msg, len: u16) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn ucan_release_context_array(up: *mut ucan_priv) {
...
// lock is not needed because, driver is currently opening or closing
    ...
    up.context_array = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn ucan_alloc_context_array(up: *mut ucan_priv) -> c_int {
...
// lock is not needed because, driver is currently opening
    ...
    int i;
    unsigned long flags;
    struct ucan_urb_context *ret = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ucan_get_fw_str(up: *mut ucan_priv, fw_str: *mut c_char, size: usize) {
...
//
    ...
    canid_t canid)
    {
    let mut new_state: enum can_state = up.can.state;
...
// controller problems, details in data[1]
    ...
    let mut d1: u8 = m.msg.can_msg.data[1];
...
// protocol error, details in data[2]
    ...
    let mut d2: u8 = m.msg.can_msg.data[2];
...
//
#[no_mangle]
unsafe extern "C" fn ucan_rx_can_msg(up: *mut ucan_priv, m: *mut ucan_message_in) {
...
// handle error frames
    ...
    let mut busstate_changed: bool = ucan_handle_error_frame(up, m, canid);
...
// callback indicating completed transmission
    ...
    u16 count, i;
    u8 echo_index;
    let mut len: u16 = le16_to_cpu(m.len);
...
// update statistics
    ...
    can_get_echo_skb(up.netdev, echo_index, core::ptr::null_mut());
    } else {
    up.netdev.stats.tx_dropped++;
    can_free_echo_skb(up.netdev, echo_index, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn ucan_read_bulk_callback(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn ucan_write_bulk_callback(urb: *mut urb) {
...
// update counters an cleanup
    spin_lock_irqsave(&up.echo_skb_lock, flags);
    can_free_echo_skb(up.netdev, context - up.context_array, core::ptr::null_mut());
    spin_unlock_irqrestore(&up.echo_skb_lock, flags);
    up.netdev.stats.tx_dropped++;
...
#[no_mangle]
unsafe extern "C" fn ucan_cleanup_rx_urbs(up: *mut ucan_priv, urbs: *mut urb) {
...
//
#[no_mangle]
unsafe extern "C" fn ucan_submit_rx_urbs(up: *mut ucan_priv, urbs: *mut urb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ucan_open(netdev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ucan_clean_up_tx_urb(up: *mut ucan_priv, urb: *mut urb) {
...
//
#[no_mangle]
unsafe extern "C" fn ucan_close(netdev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ucan_set_bittiming(netdev: *mut net_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ucan_set_mode(netdev: *mut net_device, mode: enum can_mode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ucan_disconnect(intf: *mut usb_interface) {
