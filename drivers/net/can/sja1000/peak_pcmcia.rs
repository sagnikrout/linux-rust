...
//! Source: drivers/net/can/sja1000/peak_pcmcia.c
#![no_std]
...
pub const PCC_CHAN_MAX: c_int = 2;

pub const PCC_MANF_ID: c_uint = 0x0377;
pub const PCC_CARD_ID: c_uint = 0x0001;
pub const PCC_CHAN_SIZE: c_uint = 0x20;

pub const PCC_COMN_SIZE: c_uint = 0x40;
// common area registers
pub const PCC_CCR: c_uint = 0x00;
pub const PCC_CSR: c_uint = 0x02;
pub const PCC_CPR: c_uint = 0x04;
pub const PCC_SPI_DIR: c_uint = 0x06;
pub const PCC_SPI_DOR: c_uint = 0x08;
pub const PCC_SPI_ADR: c_uint = 0x0a;
pub const PCC_SPI_IR: c_uint = 0x0c;
pub const PCC_FW_MAJOR: c_uint = 0x10;
pub const PCC_FW_MINOR: c_uint = 0x12;
// CCR bits
pub const PCC_CCR_CLK_16: c_uint = 0x00;
pub const PCC_CCR_CLK_10: c_uint = 0x01;
pub const PCC_CCR_CLK_21: c_uint = 0x02;
pub const PCC_CCR_CLK_8: c_uint = 0x03;

// led selection bits

// led state value
pub const PCC_LED_ON: c_uint = 0x00;
pub const PCC_LED_FAST: c_uint = 0x01;
pub const PCC_LED_SLOW: c_uint = 0x02;
pub const PCC_LED_OFF: c_uint = 0x03;

    PCC_CCR_LED_OFF_CHAN(1))

// CSR bits
pub const PCC_CSR_SPI_BUSY: c_uint = 0x04;
// time waiting for SPI busy (prevent from infinite loop)
pub const PCC_SPI_MAX_BUSY_WAIT_MS: c_int = 3;
// max count of reading the SPI status register waiting for a change
// (prevent from infinite loop)
pub const PCC_WRITE_MAX_LOOP: c_int = 1000;
// max nb of int handled by that isr in one shot (prevent from infinite loop)
pub const PCC_ISR_MAX_LOOP: c_int = 10;
// EEPROM chip instruction set
// note: EEPROM Read/Write instructions include A8 bit

pub const PCC_EEP_WRDI: c_uint = 0x04	/* EEPROM Write Disable */;
pub const PCC_EEP_RDSR: c_uint = 0x05	/* EEPROM Read Status Register */;
pub const PCC_EEP_WREN: c_uint = 0x06	/* EEPROM Write Enable */;
// EEPROM Status Register bits
pub const PCC_EEP_SR_WEN: c_uint = 0x02	/* EEPROM SR Write Enable bit */;
pub const PCC_EEP_SR_WIP: c_uint = 0x01	/* EEPROM SR Write In Progress bit */;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pcan_pccard {
    pub pdev: *mut pcmcia_device,
    pub chan_count: c_int,
    pub channel: [pcan_channel; PCC_CHAN_MAX],
    pub ccr: u8,
    pub fw_major: u8,
    pub fw_minor: u8,
    pub ioport_addr: *mut void __iomem,
    pub led_timer: timer_list,
}
...
//
#[no_mangle]
unsafe extern "C" fn pcan_start_led_timer(card: *mut pcan_pccard) {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_stop_led_timer(card: *mut pcan_pccard) {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_read_canreg(priv: *const sja1000_priv, port: c_int) -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_write_canreg(priv: *const sja1000_priv, port: c_int, v: u8) {
    static void pcan_write_canreg(const struct sja1000_priv *priv, int port, u8 v)
    {
    struct pcan_pccard *card = priv.priv;
    let mut c: c_int = (priv.reg_base - card.ioport_addr) / PCC_CHAN_SIZE;
...
//
#[no_mangle]
unsafe extern "C" fn pcan_read_reg(card: *mut pcan_pccard, port: c_int) -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_write_reg(card: *mut pcan_pccard, port: c_int, v: u8) {
...
//
#[no_mangle]
pub unsafe extern "C" fn pcan_pccard_present(card: *mut pcan_pccard) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_wait_spi_busy(card: *mut pcan_pccard) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_write_eeprom(card: *mut pcan_pccard, addr: u16, v: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pcan_set_leds(card: *mut pcan_pccard, led_mask: u8, state: u8) {
    static void pcan_set_leds(struct pcan_pccard *card, u8 led_mask, u8 state)
    {
    let mut ccr: u8 = card.ccr;
    int i;
...
//
#[no_mangle]
pub unsafe extern "C" fn pcan_set_can_power(card: *mut pcan_pccard, onoff: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_led_timer(t: *mut timer_list) {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
// handle shared interrupt and next loop
    let mut nothing_to_handle: c_int = 1;
    int i;
...
//
#[no_mangle]
unsafe extern "C" fn pcan_free_channels(card: *mut pcan_pccard) {
    static void pcan_free_channels(struct pcan_pccard *card)
    {
    int i;
    let mut led_mask: u8 = 0;
    for (i = 0; i < card.chan_count; i++) {
    struct net_device *netdev;
    char name[IFNAMSIZ];
    led_mask |= PCC_LED(i);
...
//
#[no_mangle]
pub unsafe extern "C" fn pcan_channel_present(priv: *mut sja1000_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pcan_add_channels(card: *mut pcan_pccard) -> c_int {
    static int pcan_add_channels(struct pcan_pccard *card)
    {
    struct pcmcia_device *pdev = card.pdev;
    int i, err = 0;
    let mut ccr: u8 = PCC_CCR_INIT;
...
#[no_mangle]
unsafe extern "C" fn pcan_conf_check(pdev: *mut pcmcia_device, priv_data: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_free(pdev: *mut pcmcia_device) {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_probe(pdev: *mut pcmcia_device) -> c_int {
    static int pcan_probe(struct pcmcia_device *pdev)
    {
    struct pcan_pccard *card;
    int err;
    pdev.config_flags |= CONF_ENABLE_IRQ | CONF_AUTO_SET_IO;
    err = pcmcia_loop_config(pdev, pcan_conf_check, core::ptr::null_mut());
    if (err) {
...
//
#[no_mangle]
unsafe extern "C" fn pcan_remove(pdev: *mut pcmcia_device) {
