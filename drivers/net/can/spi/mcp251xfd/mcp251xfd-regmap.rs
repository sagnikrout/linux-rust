...
//! Source: drivers/net/can/spi/mcp251xfd/mcp251xfd-regmap.c
#![no_std]
...
//
    ...
    let mut reg: u16 = be16_to_cpu(*(__be16 *)reg_p) & MCP251XFD_SPI_ADDRESS_MASK;
    int ret;
...
//
    ...
    let mut len: usize = byte_exclude - reg;
// Write up to 0xe05
...
// Write from 0xe07 on
    reg += len + 1;
    reg = ( unsigned short)cpu_to_be16(MCP251XFD_SPI_INSTRUCTION_WRITE | reg);
    ...
    {
    let mut data_offset: usize = sizeof(__be16);
    ...
    struct mcp251xfd_map_buf_nocrc *buf_tx = priv.map_buf_nocrc_tx;
    let mut orig_le32: __le32 = 0, mask_le32, val_le32, tmp_le32;
...
#[no_mangle]
unsafe extern "C" fn mcp251xfd_regmap_destroy_crc(priv: *mut mcp251xfd_priv) {
    static void mcp251xfd_regmap_destroy_crc(struct mcp251xfd_priv *priv)
    {
    if (priv.map_buf_crc_rx) {
    devm_kfree(&priv.spi.dev, priv.map_buf_crc_rx);
    priv.map_buf_crc_rx = core::ptr::null_mut();
    }
    if (priv.map_buf_crc_tx) {
    devm_kfree(&priv.spi.dev, priv.map_buf_crc_tx);
    priv.map_buf_crc_tx = core::ptr::null_mut();
    }
    }
#[no_mangle]
pub unsafe extern "C" fn mcp251xfd_regmap_init(priv: *mut mcp251xfd_priv) -> c_int {
