...
//! Source: drivers/net/vmxnet3/vmxnet3_drv.c
#![no_std]
...
//
    ...
    let mut enable_mq: static int = 1;
    static void
...
//
    ...
    int i;
    unsigned long flags;
    let mut events: u32 = le32_to_cpu(adapter.shared.ecr);
    if (!events)
    return;
...
#[no_mangle]
unsafe extern "C" fn get_bitfield32(bitfield: *const __le32, pos: u32, size: u32) -> u32 {
    static u32 get_bitfield32(const __le32 *bitfield, u32 pos, u32 size)
    {
    let mut temp: u32 = le32_to_cpu(*bitfield);
    let mut mask: u32 = ((1 << size) - 1) << pos;
    temp &= mask;
    temp >>= pos;
    return temp;
    }
    ...
    struct pci_dev *pdev)
    {
    let mut map_type: u32 = tbi.map_type;
    if (map_type & VMXNET3_MAP_SINGLE)
    dma_unmap_single(&pdev.dev, tbi.dma_addr, tbi.len,
    DMA_TO_DEVICE);
#[no_mangle]
pub unsafe extern "C" fn if(VMXNET3_MAP_PAGE: map_type &) -> else {
...
//
    ...
    if (rbi.skb == core::ptr::null_mut()) {
    rbi.skb = __netdev_alloc_skb_ip_align(adapter.netdev,
    rbi.len,
    GFP_KERNEL);
    if (unlikely(rbi.skb == core::ptr::null_mut())) {
    rq.stats.rx_buf_alloc_failure++;
    break;
    }
...
// rx buffer skipped by the device
    ...
    rbi.len  != PAGE_SIZE);
    if (rbi.page == core::ptr::null_mut()) {
    rbi.page = alloc_page(GFP_ATOMIC);
    if (unlikely(rbi.page == core::ptr::null_mut())) {
    rq.stats.rx_buf_alloc_failure++;
    break;
    }
...
// so that the device can distinguish a full ring and an empty ring
    ...
    int i;
    union Vmxnet3_GenericDesc *gdesc;
    struct vmxnet3_tx_buf_info *tbi = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn txd_estimate(skb: *const sk_buff) -> c_int {
    static int txd_estimate(const struct sk_buff *skb)
    {
    let mut count: c_int = VMXNET3_TXD_NEEDED(skb_headlen(skb)) + 1;
    int i;
    for (i = 0; i < skb_shinfo(skb).nr_frags; i++) {
    const skb_frag_t *frag = &skb_shinfo(skb).frags[i];
...
//
    ...
    {
    let mut xdp_prog: bool = vmxnet3_xdp_enabled(adapter);
...
// Non-IP pkt, dont estimate header length
    ...
    {
    struct udphdr *uh = core::ptr::null_mut();
    ...
    let mut num_pkts: u32 = 0;
    let mut skip_page_frags: bool = false;
    let mut encap_lro: bool = false;
    struct Vmxnet3_RxCompDesc *rcd;
    struct vmxnet3_rx_ctx *ctx = &rq.rx_ctx;
    let mut segCnt: u16 = 0, mss = 0;
    ...
    struct vmxnet3_rx_buf_info *rbi;
    struct sk_buff *skb, *new_skb = core::ptr::null_mut();
    struct page *new_page = core::ptr::null_mut();
    dma_addr_t new_dma_addr;
    int num_to_alloc;
    struct Vmxnet3_RxDesc *rxd;
    u32 idx, ring_idx;
    struct vmxnet3_cmd_ring	*ring = core::ptr::null_mut();
    if (num_pkts >= quota) {
...
//
    ...
    BUG_ON(ctx.skb != core::ptr::null_mut() || rbi.skb == core::ptr::null_mut());
...
// Immediate refill
    ...
    BUG_ON(ctx.skb == core::ptr::null_mut() && !skip_page_frags);
...
// Immediate refill
    ...
    skb = ctx.skb;
    if (rcd.eop) {
    let mut mtu: u32 = adapter.netdev.mtu;
...
// ring has already been cleaned up
    ...
    rcu_assign_pointer(adapter.xdp_bpf_prog, core::ptr::null_mut());
    }
...
// all rx buffers must have already been freed
    ...
    BUG_ON(rq.buf_info[i][j].page != core::ptr::null_mut());
    }
    }
    for (i = 0; i < 2; i++) {
...
// sizeof(struct Vmxnet3_RxDesc),
    rq.rx_ring[i].base,
    rq.rx_ring[i].basePA);
    rq.rx_ring[i].base = core::ptr::null_mut();
    }
    }
    ...
    rq.data_ring.base = core::ptr::null_mut();
    }
    if (rq.ts_ring.base) {
    dma_free_coherent(&adapter.pdev.dev,
    rq.rx_ring[0].size * rq.rx_ts_desc_size,
    rq.ts_ring.base, rq.ts_ring.basePA);
    rq.ts_ring.base = core::ptr::null_mut();
    }
    if (rq.comp_ring.base) {
    dma_free_coherent(&adapter.pdev.dev, rq.comp_ring.size
// sizeof(struct Vmxnet3_RxCompDesc),
    rq.comp_ring.base, rq.comp_ring.basePA);
    rq.comp_ring.base = core::ptr::null_mut();
    }
    kfree(rq.buf_info[0]);
    rq.buf_info[0] = core::ptr::null_mut();
    rq.buf_info[1] = core::ptr::null_mut();
    }
    static void
...
// reset the comp ring
    ...
    rq.comp_ring.gen = VMXNET3_INIT_GEN;
// reset rxctx
    rq.rx_ctx.skb = core::ptr::null_mut();
// stats are not reset
    return 0;
    }
    static int
...
// Interrupt handler for vmxnet3
    ...
    let mut icr: u32 = VMXNET3_READ_BAR1_REG(adapter, VMXNET3_REG_ICR);
    if (unlikely(icr == 0))
// not ours
    return IRQ_NONE;
    }
...
// allow untagged pkts
    ...
    let mut sz: u32 = netdev_mc_count(netdev) * ETH_ALEN;
...
// We may be called with BH disabled
    ...
    u8 *new_table = core::ptr::null_mut();
    let mut new_table_pa: dma_addr_t = 0;
    let mut new_table_pa_valid: bool = false;
    let mut new_mode: u32 = VMXNET3_RXM_UCAST;
    if (netdev.flags & IFF_PROMISC) {
    u32 *vfTable = adapter.shared.devRead.rxFilterConf.vfTable;
    ...
    if (new_table) {
    let mut sz: usize = netdev_mc_count(netdev) * ETH_ALEN;
...
// tx queue settings
    ...
    BUG_ON(adapter.tx_queue[i].tx_ring.base == core::ptr::null_mut());
...
// of rx queues is finalized after allocating intrs
    ...
    if (VMXNET3_VERSION_GE_9(adapter)) {
    unsigned long flags;
    let mut tx_ts_desc_size: u16 = 0;
    let mut rx_ts_desc_size: u16 = 0;
    u32 ret;
    spin_lock_irqsave(&adapter.cmd_lock, flags);
    VMXNET3_WRITE_BAR1_REG(adapter, VMXNET3_REG_CMD,
...
#[no_mangle]
pub unsafe extern "C" fn if(VMXNET3_IT_MSI: adapter->intr.type ==) -> else {
...
// if the device is closed, we must leave it alone
    ...
    &adapter.pm_conf_pa,
    GFP_KERNEL);
    if (adapter.pm_conf == core::ptr::null_mut()) {
    err = -ENOMEM;
    goto err_alloc_pm;
    }
    ...
    &adapter.rss_conf_pa,
    GFP_KERNEL);
    if (adapter.rss_conf == core::ptr::null_mut()) {
    err = -ENOMEM;
    goto err_alloc_rss;
    }
...
#[no_mangle]
unsafe extern "C" fn vmxnet3_shutdown_device(pdev: *mut pci_dev) {
