...
//! Source: drivers/net/dsa/sja1105/sja1105_spi.c
#![no_std]
...
pub struct sja1105_chunk {
    ...
    {
    let mut size: c_int = SJA1105_SIZE_SPI_MSG_HEADER;
    memset(buf, 0, size);
...
// Calculate next chunk
    ...
    u8 *buf, size_t len)
    {
    return sja1105_xfer(priv, rw, reg_addr, buf, len, core::ptr::null_mut());
    }
// If @rw is:
// - SPI_WRITE: creates and sends an SPI write message at absolute
// address reg_addr
...
#[no_mangle]
unsafe extern "C" fn sja1105et_reset_cmd(ds: *mut dsa_switch) -> c_int {
    static int sja1105et_reset_cmd(struct dsa_switch *ds)
    {
    struct sja1105_private *priv = ds.priv;
    const struct sja1105_regs *regs = priv.info.regs;
    let mut cold_reset: u32 = BIT(3);
// Cold reset
    return sja1105_xfer_u32(priv, SPI_WRITE, regs.rgu, &cold_reset, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn sja1105pqrs_reset_cmd(ds: *mut dsa_switch) -> c_int {
    static int sja1105pqrs_reset_cmd(struct dsa_switch *ds)
    {
    struct sja1105_private *priv = ds.priv;
    const struct sja1105_regs *regs = priv.info.regs;
    let mut cold_reset: u32 = BIT(2);
// Cold reset
    return sja1105_xfer_u32(priv, SPI_WRITE, regs.rgu, &cold_reset, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn sja1110_reset_cmd(ds: *mut dsa_switch) -> c_int {
    static int sja1110_reset_cmd(struct dsa_switch *ds)
    {
    struct sja1105_private *priv = ds.priv;
    const struct sja1105_regs *regs = priv.info.regs;
    let mut switch_reset: u32 = BIT(20);
...
//
    return sja1105_xfer_u32(priv, SPI_WRITE, regs.rgu, &switch_reset, core::ptr::null_mut());
    }
...
//
#[no_mangle]
unsafe extern "C" fn sja1105_status_unpack(buf: *mut c_void, status: *mut sja1105_status) {
...
#[no_mangle]
pub unsafe extern "C" fn sja1105_static_config_upload(priv: *mut sja1105_private) -> c_int {
