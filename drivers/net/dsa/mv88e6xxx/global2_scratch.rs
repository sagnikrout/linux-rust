...
//! Source: drivers/net/dsa/mv88e6xxx/global2_scratch.c
#![no_std]
...
//
    ...
    int *set)
    {
    let mut reg: c_int = base_reg + (offset / 8);
    let mut mask: u8 = (1 << (offset & 0x7));
    u8 val;
    int err;
...
//
    ...
    {
    let mut mask: u8 = (1 << (pin & 0x7));
    let mut offset: c_int = (pin / 8);
    int reg;
...
//
    ...
    {
    let mut reg: c_int = MV88E6352_G2_SCRATCH_GPIO_PCTL0 + (pin / 2);
    let mut offset: c_int = (pin & 0x1) ? 4 : 0;
    let mut mask: u8 = (0x7 << offset);
    int err;
    u8 val;
    err = mv88e6xxx_g2_scratch_read(chip, reg, &val);
    if (err)
...
//
    ...
    bool external)
    {
    let mut misc_cfg: c_int = MV88E6352_G2_SCRATCH_MISC_CFG;
    let mut config_data1: c_int = MV88E6352_G2_SCRATCH_CONFIG_DATA1;
    let mut config_data2: c_int = MV88E6352_G2_SCRATCH_CONFIG_DATA2;
    bool no_cpu;
    u8 p0_mode;
    int err;
    u8 val;
...
//
#[no_mangle]
pub unsafe extern "C" fn mv88e6352_g2_cache_global_scratch_config3(chip: *mut mv88e6xxx_chip) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn mv88e6352_g2_scratch_port_has_serdes(chip: *mut mv88e6xxx_chip, port: c_int) -> c_int {
