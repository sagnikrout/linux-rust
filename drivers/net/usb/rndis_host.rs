...
//! Source: drivers/net/usb/rndis_host.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn rndis_status(dev: *mut usbnet, urb: *mut urb) {
...
//
    ...
    let mut status: u32 = le32_to_cpu(msg.status);
    switch (status) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rndis_command(dev: *mut usbnet, buf: *mut rndis_msg_hdr, buflen: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rndis_bind(dev: *mut usbnet, intf: *mut usb_interface) -> c_int {
    static int rndis_bind(struct usbnet *dev, struct usb_interface *intf)
    {
    return generic_rndis_bind(dev, intf, FLAG_RNDIS_PHYM_NOT_WIRELESS);
    }
#[no_mangle]
unsafe extern "C" fn zte_rndis_bind(dev: *mut usbnet, intf: *mut usb_interface) -> c_int {
    static int zte_rndis_bind(struct usbnet *dev, struct usb_interface *intf)
    {
    let mut status: c_int = rndis_bind(dev, intf);
    if (!status && (dev.net.dev_addr[0] & 0x02))
    eth_hw_addr_random(dev.net);
    return status;
    }
#[no_mangle]
pub unsafe extern "C" fn rndis_unbind(dev: *mut usbnet, intf: *mut usb_interface) {
...
//
#[no_mangle]
pub unsafe extern "C" fn rndis_rx_fixup(dev: *mut usbnet, skb: *mut sk_buff) -> c_int {
...
// caller will usbnet_skb_return the remaining packet
    ...
    struct sk_buff		*skb2;
    let mut len: unsigned = skb.len;
    if (likely(!skb_cloned(skb))) {
    let mut room: c_int = skb_headroom(skb);
