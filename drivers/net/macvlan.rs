...
pub const MACVLAN_HASH_BITS: c_int = 8;

pub const MACVLAN_DEFAULT_BC_QUEUE_LEN: c_int = 1000;
pub const MACVLAN_F_PASSTHRU: c_int = 1;
pub const MACVLAN_F_ADDRCHANGE: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct macvlan_port {
    pub dev: *mut net_device,
    pub vlan_hash: [hlist_head; MACVLAN_HASH_SIZE],
    pub vlans: list_head,
    pub bc_queue: sk_buff_head,
    pub bc_work: work_struct,
    pub bc_queue_len_used: u32,
    pub bc_cutoff: c_int,
    pub flags: u32,
    pub count: c_int,
    pub vlan_source_hash: [hlist_head; MACVLAN_HASH_SIZE],
    pub MACVLAN_MC_FILTER_SZ): DECLARE_BITMAP(bc_filter,,
    pub MACVLAN_MC_FILTER_SZ): DECLARE_BITMAP(mc_filter,,
    pub perm_addr: [c_uchar; ETH_ALEN],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct macvlan_source_entry {
    pub hlist: hlist_node,
    pub vlan: *mut macvlan_dev __rcu,
    pub __aligned(sizeof(u16)): unsigned char addr[6+2],
    pub rcu: rcu_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct macvlan_skb_cb {
    pub src: *const macvlan_dev,
}

    static void macvlan_port_destroy(struct net_device *dev);
    static void update_port_bc_queue_len(struct macvlan_port *port);
#[no_mangle]
pub unsafe extern "C" fn macvlan_passthru(port: *const macvlan_port) -> bool {
    static inline bool macvlan_passthru(const struct macvlan_port *port)
    {
    return port.flags & MACVLAN_F_PASSTHRU;
    }
#[no_mangle]
pub unsafe extern "C" fn macvlan_set_passthru(port: *mut macvlan_port) {
    static inline void macvlan_set_passthru(struct macvlan_port *port)
    {
    port.flags |= MACVLAN_F_PASSTHRU;
    }
#[no_mangle]
pub unsafe extern "C" fn macvlan_addr_change(port: *const macvlan_port) -> bool {
    static inline bool macvlan_addr_change(const struct macvlan_port *port)
    {
    return port.flags & MACVLAN_F_ADDRCHANGE;
    }
#[no_mangle]
pub unsafe extern "C" fn macvlan_set_addr_change(port: *mut macvlan_port) {
    static inline void macvlan_set_addr_change(struct macvlan_port *port)
    {
    port.flags |= MACVLAN_F_ADDRCHANGE;
    }
#[no_mangle]
pub unsafe extern "C" fn macvlan_clear_addr_change(port: *mut macvlan_port) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_eth_hash(addr: *const c_uchar) -> u32 {
    static u32 macvlan_eth_hash(const unsigned char *addr)
    {
    let mut value: u64 = get_unaligned((u64 *)addr);
// only want 6 bytes
    ...
    let mut idx: u32 = macvlan_eth_hash(addr);
...
#[no_mangle]
unsafe extern "C" fn macvlan_hash_add(vlan: *mut macvlan_dev) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_hash_del_source(entry: *mut macvlan_source_entry) {
    static void macvlan_hash_del_source(struct macvlan_source_entry *entry)
    {
    RCU_INIT_POINTER(entry.vlan, core::ptr::null_mut());
    hlist_del_rcu(&entry.hlist);
    kfree_rcu(entry, rcu);
    }
#[no_mangle]
unsafe extern "C" fn macvlan_hash_del(vlan: *mut macvlan_dev, sync: bool) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_hash_mix(vlan: *const macvlan_dev) -> u32 {
    ...
    const unsigned char *addr)
    {
    let mut val: u32 = get_unaligned((u32 *)(addr + 2));
...
// frame comes from an external address
    macvlan_broadcast(skb, port, core::ptr::null_mut(),
    MACVLAN_MODE_PRIVATE |
    MACVLAN_MODE_VEPA    |
    MACVLAN_MODE_PASSTHRU|
    MACVLAN_MODE_BRIDGE);
#[no_mangle]
pub unsafe extern "C" fn if(MACVLAN_MODE_VEPA: READ_ONCE(src->mode) ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn macvlan_process_broadcast(w: *mut work_struct) {
    ...
    struct sk_buff *skb)
    {
    let mut bc_queue_len_used: u32 = READ_ONCE(port.bc_queue_len_used);
    struct sk_buff *nskb;
...
#[no_mangle]
unsafe extern "C" fn macvlan_handle_frame(pskb: *mut sk_buff) -> rx_handler_result_t {
...
// pskb = skb;
    ...
    if (src) {
    let mut mode: enum macvlan_mode = READ_ONCE(src.mode);
...
// forward to original port.
    ...
    hash = mc_hash(core::ptr::null_mut(), eth.h_dest);
    if (test_bit(hash, port.bc_filter))
    macvlan_broadcast_enqueue(port, src, skb);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(hash, _arg: port->mc_filter)) -> else {
...
#[no_mangle]
unsafe extern "C" fn macvlan_queue_xmit(skb: *mut sk_buff, dev: *mut net_device) -> c_int {
...
// send to lowerdev first for its network taps
    ...
    netdev_get_sb_channel(dev) ? dev : core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn macvlan_netpoll_send_skb(vlan: *mut macvlan_dev, skb: *mut sk_buff) -> netdev_tx_t {
...
#[no_mangle]
unsafe extern "C" fn macvlan_open(dev: *mut net_device) -> c_int {
...
//
    ...
    vlan.accel_priv = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn macvlan_stop(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn macvlan_set_mac_address(dev: *mut net_device, p: *mut c_void) -> c_int {
...
// If the addresses are the same, this is a no-op
    ...
    return dev_set_mac_address(vlan.lowerdev, addr, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn macvlan_change_rx_flags(dev: *mut net_device, change: c_int) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_recompute_bc_filter(vlan: *mut macvlan_dev) {
    ...
    macvlan_compute_filter(vlan.port.bc_filter, vlan.lowerdev, core::ptr::null_mut(),
    vlan.port.bc_cutoff);
    }
#[no_mangle]
unsafe extern "C" fn macvlan_set_mac_lists(dev: *mut net_device) {
...
//
    macvlan_compute_filter(vlan.port.mc_filter, vlan.lowerdev, core::ptr::null_mut(),
    0);
    macvlan_recompute_bc_filter(vlan);
    }
#[no_mangle]
unsafe extern "C" fn update_port_bc_cutoff(vlan: *mut macvlan_dev, cutoff: c_int) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn macvlan_set_lockdep_class(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_init(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn macvlan_uninit(dev: *mut net_device) {
    ...
    let mut rx_errors: u32 = 0, tx_dropped = 0;
    unsigned int start;
    int i;
...
pub unsafe extern "C" fn if(_arg: is_multicast_ether_addr(addr)) -> else {
    ...
    {
    struct macvlan_dev *vlan = netdev_priv(dev);
    let mut lowerdev_features: netdev_features_t = vlan.lowerdev.features;
...
#[no_mangle]
unsafe extern "C" fn macvlan_dev_poll_controller(dev: *mut net_device) {
    static void macvlan_dev_poll_controller(struct net_device *dev)
    {
    return;
    }
#[no_mangle]
unsafe extern "C" fn macvlan_dev_netpoll_setup(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn macvlan_dev_netpoll_cleanup(dev: *mut net_device) {
    ...
    if (!netpoll)
    return;
    vlan.netpoll = core::ptr::null_mut();
    __netpoll_free(netpoll);
    }

#[no_mangle]
unsafe extern "C" fn macvlan_dev_get_iflink(dev: *const net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn macvlan_dev_free(dev: *mut net_device) {
...
#[no_mangle]
pub unsafe extern "C" fn macvlan_common_setup(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_setup(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn macvlan_port_create(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn macvlan_port_destroy(dev: *mut net_device) {
    static void macvlan_port_destroy(struct net_device *dev)
    {
...
//
    ...
    dev_set_mac_address(port.dev, &ss, core::ptr::null_mut());
    }
    kfree(port);
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(lowerdev->mtu: dev->mtu >) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn macvlan_dellink(dev: *mut net_device, head: *mut list_head) {
...
// Passthrough mode can't be set or cleared dynamically
    ...
    if (data && data[IFLA_MACVLAN_FLAGS]) {
    let mut flags: __u16 = nla_get_u16(data[IFLA_MACVLAN_FLAGS]);
    let mut promisc: bool = (flags ^ vlan.flags) & MACVLAN_FLAG_NOPROMISC;
...
#[no_mangle]
unsafe extern "C" fn macvlan_get_size_mac(vlan: *const macvlan_dev) -> usize {
    static size_t macvlan_get_size_mac(const struct macvlan_dev *vlan)
    {
    let mut macaddr_count: c_uint = READ_ONCE(vlan.macaddr_count);
    if (!macaddr_count)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn macvlan_get_size(dev: *const net_device) -> usize {
    ...
    const struct macvlan_dev *vlan = netdev_priv(dev);
    struct macvlan_port *port = vlan.port;
    let mut macaddr_count: c_uint = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: nla_nest_end_safe(skb, 0: nest) <) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn macvlan_link_register(ops: *mut rtnl_link_ops) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn update_port_bc_queue_len(port: *mut macvlan_port) {
    static void update_port_bc_queue_len(struct macvlan_port *port)
    {
    let mut max_bc_queue_len_req: u32 = 0;
    struct macvlan_dev *vlan;
...
#[no_mangle]
unsafe extern "C" fn macvlan_init_module() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn macvlan_cleanup_module() -> void __exit {
