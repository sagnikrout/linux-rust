...
//! Source: drivers/net/hyperv/rndis_filter.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rndis_request {
    pub list_ent: list_head,
    pub wait_event: completion,
    pub response_msg: rndis_message,
//
...
// future.
//
    pub response_ext: [u8; RNDIS_EXT_LEN],
// Simplify allocation by having a netvsc packet inline
    pub pkt: hv_netvsc_packet,
    pub request_msg: rndis_message,
//
...
//
    pub request_ext: [u8; RNDIS_EXT_LEN],
}
...
// Setup the packet to send it
    ...
    rcu_read_lock_bh();
    ret = netvsc_send(dev.ndev, packet, core::ptr::null_mut(), &pb, core::ptr::null_mut(), false);
    rcu_read_unlock_bh();
    return ret;
    }
    ...
    let mut msg_len: u32 = request.response_msg.msg_len;
// Ensure the packet is big enough to access its fields
    ...
    struct rndis_request *request = core::ptr::null_mut();
...
// ppi->size should be big enough to hold the returned object.
    ...
    void *data, u32 len)
    {
    let mut cnt: u32 = nvchan.rsc.cnt;
    if (cnt) {
...
//
    ...
    nvchan.rsc.pktlen = len;
    if (hash_info != core::ptr::null_mut()) {
...
// Identify RSC frags, drop erroneous packets
    ...
    nvchan.rsc.cnt = 0;
#[no_mangle]
pub unsafe extern "C" fn if(0: nvchan->rsc.cnt ==) -> else {
...
// notification msgs
    ...
    {
    struct rndis_request *request;
    let mut inresult_size: u32 = *result_size;
...
// Setup the rndis query
    ...
    let mut nvsp_version: u32 = nvdev.nvsp_version;
    u8 ndis_rev;
    size_t size;
...
// Get the hardware offload capabilities
    ...
    {
    let mut caps_len: u32 = sizeof(*caps);
    int ret;
    ...
    {
    let mut size: u32 = ETH_ALEN;
    return rndis_filter_query_device(dev, net_device,
    RNDIS_OID_802_3_PERMANENT_ADDRESS,
    dev.hw_mac_adr, &size);
    }

pub const NWADR_STRLEN: c_int = 14;
...
// Multiply by 4 because each MAC byte displayed as 2 utf16 chars
    ...
    struct rndis_set_complete *set_complete;
    let mut extlen: u32 = sizeof(struct ndis_offload_params);
    int ret;
    let mut vsp_version: u32 = nvdev.nvsp_version;
...
#[no_mangle]
unsafe extern "C" fn rndis_set_multicast(w: *mut work_struct) {
    static void rndis_set_multicast(struct work_struct *w)
    {
    struct rndis_device *rdev
    = container_of(w, struct rndis_device, mcast_work);
    let mut filter: u32 = NDIS_PACKET_TYPE_DIRECTED;
    let mut flags: c_uint = rdev.ndev.flags;
...
#[no_mangle]
pub unsafe extern "C" fn rndis_filter_update(nvdev: *mut netvsc_device) {
...
#[no_mangle]
unsafe extern "C" fn netvsc_device_idle(nvdev: *const netvsc_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn rndis_filter_open_device(dev: *mut rndis_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rndis_filter_close_device(dev: *mut rndis_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn netvsc_sc_open(new_sc: *mut vmbus_channel) {
    ...
    let mut chn_index: u16 = new_sc.offermsg.offer.sub_channel_index;
    struct netvsc_channel *nvchan;
    int ret;
...
// ignore the default value from host
    ...
    let mut rsscap_size: u32 = sizeof(struct ndis_recv_scale_cap);
    u32 mtu, size;
...
// Halt and release the rndis device
    ...
    ndc.rx_table_sz = 0;
    kfree(ndc.rx_table);
    ndc.rx_table = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn rndis_filter_open(nvdev: *mut netvsc_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rndis_filter_close(nvdev: *mut netvsc_device) -> c_int {
