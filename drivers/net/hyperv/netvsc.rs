...
//! Source: drivers/net/hyperv/netvsc.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn netvsc_switch_datapath(ndev: *mut net_device, vf: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn netvsc_subchan_work(w: *mut work_struct) {
...
// Avoid deadlock with device removal already under RTNL
    ...
    if (rdev) {
    ret = rndis_set_subchannel(rdev.ndev, nvdev, core::ptr::null_mut());
    if (ret == 0) {
...
#[no_mangle]
unsafe extern "C" fn __free_netvsc_device(nvdev: *mut netvsc_device) {
...
#[no_mangle]
unsafe extern "C" fn free_netvsc_device(w: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn netvsc_workqueue_init() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn netvsc_workqueue_destroy() {
...
#[no_mangle]
unsafe extern "C" fn free_netvsc_device_rcu(nvdev: *mut netvsc_device) {
...
#[no_mangle]
pub unsafe extern "C" fn netvsc_alloc_recv_comp_ring(net_device: *mut netvsc_device, q_idx: u32) -> c_int {
    int netvsc_alloc_recv_comp_ring(struct netvsc_device *net_device, u32 q_idx)
    {
    struct netvsc_channel *nvchan = &net_device.chan_table[q_idx];
    let mut node: c_int = cpu_to_node(nvchan.channel.target_cpu);
    size_t size;
...
// Ensure buffer will not overflow
    ...
    if (nvchan.recv_buf == core::ptr::null_mut()) {
    ret = -ENOMEM;
    goto cleanup;
    }
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn netvsc_device_remove(device: *mut hv_device) {
    ...
    netvsc_revoke_recv_buf(device, net_device, ndev);
    netvsc_revoke_send_buf(device, net_device, ndev);
    RCU_INIT_POINTER(net_device_ctx.nvdev, core::ptr::null_mut());
...
pub const RING_AVAIL_PERCENT_HIWATER: c_int = 20;
pub const RING_AVAIL_PERCENT_LOWATER: c_int = 10;
...
// Notify the layer above us
    ...
    let mut send_index: u32 = packet.send_buf_index;
    ...
    let mut msglen: u32 = hv_pkt_datalen(desc);
...
#[no_mangle]
unsafe extern "C" fn netvsc_get_next_send_section(net_device: *mut netvsc_device) -> u32 {
    ...
    let mut page_count: u32 = packet.cp_partial ? 1 : packet.page_buf_cnt;
    u32 remain;
// Add padding
    ...
    pb[i].offset;
    let mut len: u32 = pb[i].len;
    while (len) {
    struct page *page = phys_to_page(paddr);
    let mut off: u32 = offset_in_page(paddr);
    let mut chunk: u32 = min_t(u32, len, PAGE_SIZE - off);
...
//
    ...
    struct hv_page_buffer *pb)
    {
    let mut page_count: u32 = packet.page_buf_cnt;
    dma_addr_t dma;
    int i;
    if (!hv_is_isolation_supported())
    return 0;
...
//
    ...
    let mut offset: u32 = pb[i].offset;
    let mut len: u32 = pb[i].len;
...
// desc_size = (char *)mpb_entry - (char *)desc;
    ...
    u64 req_id;
    int ret;
    let mut ring_avail: u32 = hv_get_avail_to_write_percent(&out_channel.outbound);
    ...
    packet.dma_range = core::ptr::null_mut();
...
// msd_skb = msdp->skb;
// msd_send = msdp->pkt;
    msdp.skb = core::ptr::null_mut();
    msdp.pkt = core::ptr::null_mut();
    msdp.count = 0;
    }
...
//
    ...
    let mut pktlen: u32 = packet.total_data_buflen, msd_len = 0;
    let mut section_index: c_uint = NETVSC_INVALID_INDEX;
    struct multi_send_data *msdp;
    struct hv_netvsc_packet *msd_send = core::ptr::null_mut(), *cur_send = core::ptr::null_mut();
    struct sk_buff *msd_skb = core::ptr::null_mut();
...
// Count how many receive completions are outstanding
    ...
    {
    let mut count: u32 = nvdev.recv_completion_cnt;
...
// Add receive complete to ring to send to host.
    ...
    let mut status: u32 = NVSP_STAT_SUCCESS;
    int i;
...
// Each range represents 1 RNDIS pkt that contains 1 ethernet frame
    for (i = 0; i < count; i++) {
    let mut offset: u32 = vmxferpage_packet.ranges[i].byte_offset;
    let mut buflen: u32 = vmxferpage_packet.ranges[i].byte_count;
    void *data;
    int ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn netvsc_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn netvsc_channel_cb(context: *mut c_void) {
...
// Open the channel
    ...
    netvsc_ring_bytes,  core::ptr::null_mut(), 0,
...
// Now, we can close the channel safely
    vmbus_close(device.channel);
    cleanup:
    netif_queue_set_napi(ndev, 0, NETDEV_QUEUE_TYPE_TX, core::ptr::null_mut());
    netif_queue_set_napi(ndev, 0, NETDEV_QUEUE_TYPE_RX, core::ptr::null_mut());
