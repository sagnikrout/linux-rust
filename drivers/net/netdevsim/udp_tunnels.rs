...
//! Source: drivers/net/netdevsim/udp_tunnels.c
#![no_std]
...
// Copyright (c) 2020 Facebook Inc.
    ...
    ns.udp_ports.inject_error = 0;
    if (!ret) {
    let mut val: u32 = be16_to_cpu(ti.port) << 16 | ti.type;
...
//
    ...
    info.set_port = core::ptr::null_mut();
    info.unset_port = core::ptr::null_mut();
    } else {
    info.sync_table = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn nsim_udp_tunnels_info_destroy(dev: *mut net_device) {
    void nsim_udp_tunnels_info_destroy(struct net_device *dev)
    {
    struct netdevsim *ns = netdev_priv(dev);
    debugfs_remove_recursive(ns.udp_ports.ddir);
    kfree(dev.udp_tunnel_nic_info);
    dev.udp_tunnel_nic_info = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn nsim_udp_tunnels_debugfs_create(nsim_dev: *mut nsim_dev) {
