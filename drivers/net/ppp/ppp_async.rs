...
//! Source: drivers/net/ppp/ppp_async.c
#![no_std]
...
pub struct asyncppp {
    ...
    pub xaccm: [u32; 8],
    pub raccm: u32,
    pub tpkt: *mut sk_buff,
    pub tpkt_pos: c_int,
    pub tfcs: u16,
    pub optr: *mut c_uchar,
    pub olim: *mut c_uchar,
    pub last_xmit: c_ulong,
    pub rpkt: *mut sk_buff,
    pub lcp_fcs: c_int,
    pub rqueue: sk_buff_head,
    pub tsk: tasklet_struct,
    pub /: *mut *mut ppp_channel chan; / interface to generic ppp layer,
    pub obuf: [c_uchar; OBUFSIZE],
}

// Bit numbers in xmit_flags
pub const XMIT_WAKEUP: c_int = 0;
pub const XMIT_FULL: c_int = 1;
pub const XMIT_BUSY: c_int = 2;
// State bits
pub const SC_TOSS: c_int = 1;
pub const SC_ESCAPE: c_int = 2;
pub const SC_PREV_ERROR: c_int = 4;
// Bits in rbits

    let mut flag_time: static int = HZ;
...
//
#[no_mangle]
unsafe extern "C" fn ppp_async_process(t: *mut tasklet_struct) {
    static void ppp_async_process(struct tasklet_struct *t)
    {
    struct asyncppp *ap = from_tasklet(ap, t, tsk);
    struct sk_buff *skb;
// process received packets
    while ((skb = skb_dequeue(&ap.rqueue)) != core::ptr::null_mut()) {
    if (skb.cb[0])
...
// packet overflowed MRU
    ...
    break;
    c = buf[n];
    if (flags != core::ptr::null_mut() && flags[n] != 0) {
    ap.state |= SC_TOSS;
    } else if (c == PPP_FLAG) {
    ...
    start_tty(ap.tty);
#[no_mangle]
pub unsafe extern "C" fn if(STOP_CHAR(ap->tty): c ==) -> else {
...
pub const CONFACK: c_int = 2;

pub const LCP_ASYNCMAP: c_int = 2;
    static void async_lcp_peek(struct asyncppp *ap, unsigned char *data,
    int len, int inbound)
    {
    int dlen, fcs, i, code;
...
#[no_mangle]
unsafe extern "C" fn ppp_async_cleanup() -> void __exit {
