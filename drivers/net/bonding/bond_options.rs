...
//! Source: drivers/net/bonding/bond_options.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn bond_opt_check_range(opt: *const bond_option, val: u64) -> bool {
...
//
    ...
    const struct bond_opt_value *ret = core::ptr::null_mut();
    bool checkval;
    int i, rv;
...
//
    ...
    {
    const struct bond_opt_value *retval = core::ptr::null_mut();
    const struct bond_option *opt;
...
//
    ...
    int ret;
    ASSERT_RTNL();
    ret = __bond_opt_set(bond, option, val, core::ptr::null_mut(), core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn bond_opt_tryset_rtnl(bond: *mut bonding, option: c_uint, buf: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bond_set_xfrm_features(bond: *mut bonding) -> bool {
...
//
    ...
    int *target)
    {
    let mut value: c_int = newval.value;
    if (!bond.params.miimon) {
...
#[no_mangle]
unsafe extern "C" fn _bond_option_arp_ip_target_add(bond: *mut bonding, target: __be32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bond_option_arp_ip_target_add(bond: *mut bonding, target: __be32) -> c_int {
    static int bond_option_arp_ip_target_add(struct bonding *bond, __be32 target)
    {
    return _bond_option_arp_ip_target_add(bond, target);
    }
#[no_mangle]
unsafe extern "C" fn bond_option_arp_ip_target_rem(bond: *mut bonding, target: __be32) -> c_int {
    static int bond_option_arp_ip_target_rem(struct bonding *bond, __be32 target)
    {
    __be32 *targets = bond.params.arp_targets;
    struct list_head *iter;
...
#[no_mangle]
pub unsafe extern "C" fn bond_option_arp_ip_targets_clear(bond: *mut bonding) {
    ...
    !in4_pton(newval.string + 1, -1, (u8 *)&target, -1, core::ptr::null_mut())) {
...
#[no_mangle]
pub unsafe extern "C" fn if('-': newval->string[0] ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn slave_can_set_ns_maddr(bond: *const bonding, slave: *mut slave) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn slave_set_ns_maddrs(bond: *mut bonding, slave: *mut slave, add: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn bond_slave_ns_maddrs_add(bond: *mut bonding, slave: *mut slave) {
...
#[no_mangle]
pub unsafe extern "C" fn bond_slave_ns_maddrs_del(bond: *mut bonding, slave: *mut slave) {
...
#[no_mangle]
pub unsafe extern "C" fn bond_option_ns_ip6_targets_clear(bond: *mut bonding) {
    void bond_option_ns_ip6_targets_clear(struct bonding *bond)
    {
    let mut addr_any: in6_addr = in6addr_any;
    int i;
    for (i = 0; i < BOND_MAX_NS_TARGETS; i++)
    ...
    const struct bond_opt_value *newval)
    {
    let mut changed: bool = !!bond.params.arp_validate != !!newval.value;
    struct list_head *iter;
    struct slave *slave;
    ...
    if (!slave) {
    netdev_dbg(newval.slave_dev, "%s called on core::ptr::null_mut() slave\n", __func__);
    return -ENODEV;
    }
...
// Search for thes slave and check for duplicate qids
    update_slave = core::ptr::null_mut();
    bond_for_each_slave(bond, slave, iter) {
    if (sdev == slave.dev)
...
//
    update_slave = slave;
#[no_mangle]
pub unsafe extern "C" fn if(slave->queue_id: qid && qid ==) -> else {
    goto err_no_cmd;
    }
    }
    if (!update_slave)
...
// Actually set the qids for the slave
    ...
    ret = bond_enslave(bond.dev, dev, core::ptr::null_mut());
    break;
    case '-':
...
// should not run here.
    ...
    slave = bond_slave_get_rtnl(newval.slave_dev);
    if (!slave) {
    netdev_dbg(bond.dev, "%s called on core::ptr::null_mut() slave\n", __func__);
    return -ENODEV;
    }
