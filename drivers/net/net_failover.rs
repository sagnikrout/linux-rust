...
//! Source: drivers/net/net_failover.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn net_failover_xmit_ready(dev: *mut net_device) -> bool {
    static bool net_failover_xmit_ready(struct net_device *dev)
    {
    return netif_running(dev) && netif_carrier_ok(dev);
    }
#[no_mangle]
unsafe extern "C" fn net_failover_open(dev: *mut net_device) -> c_int {
    ...
    if (primary_dev) {
    err = dev_open(primary_dev, core::ptr::null_mut());
    if (err)
    goto err_primary_open;
    }
    standby_dev = rtnl_dereference(nfo_info.standby_dev);
    if (standby_dev) {
    err = dev_open(standby_dev, core::ptr::null_mut());
    if (err)
...
#[no_mangle]
unsafe extern "C" fn net_failover_close(dev: *mut net_device) -> c_int {
...
// Try xmit via primary netdev followed by standby netdev
    ...
    txq = netdev_pick_tx(primary_dev, skb, core::ptr::null_mut());
    } else {
...
#[no_mangle]
unsafe extern "C" fn net_failover_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn net_failover_set_rx_mode(dev: *mut net_device) {
...
//
#[no_mangle]
unsafe extern "C" fn net_failover_handle_frame(pskb: *mut sk_buff) -> rx_handler_result_t {
...
#[no_mangle]
unsafe extern "C" fn net_failover_compute_features(dev: *mut net_device) {
    static void net_failover_compute_features(struct net_device *dev)
    {
    netdev_features_t vlan_features = FAILOVER_VLAN_FEATURES &
    NETIF_F_ALL_FOR_ALL;
    let mut enc_features: netdev_features_t = FAILOVER_ENC_FEATURES;
    let mut max_hard_header_len: c_ushort = ETH_HLEN;
...
// Align MTU of slave with failover dev
    ...
    err = netif_open(slave_dev, core::ptr::null_mut());
    if (err && (err != -EBUSY)) {
    ...
    slave_is_standby = slave_dev.dev.parent == failover_dev.dev.parent;
    if (slave_is_standby) {
    RCU_INIT_POINTER(nfo_info.standby_dev, core::ptr::null_mut());
    } else {
    RCU_INIT_POINTER(nfo_info.primary_dev, core::ptr::null_mut());
    if (standby_dev) {
...
//
    dev_open(slave_dev, core::ptr::null_mut());
    return 0;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn net_failover_destroy(failover: *mut failover) {
