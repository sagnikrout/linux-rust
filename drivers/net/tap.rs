...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct major_info {
    pub rcu: rcu_head,
    pub major: dev_t,
    pub minor_idr: idr,
    pub minor_lock: spinlock_t,
...
#[no_mangle]
unsafe extern "C" fn tap_disable_queue(q: *mut tap_queue) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tap_put_queue(q: *mut tap_queue) {
    ...
    tap.numqueues--;
    RCU_INIT_POINTER(q.tap, core::ptr::null_mut());
    sock_put(&q.sk);
...
//
    ...
    struct sk_buff *skb)
    {
    struct tap_queue *queue = core::ptr::null_mut();
// Access to taps array is protected by rcu, but access to numvtaps
// isn't. Below we use it to lookup a queue, but treat it as a hint
// and validate that the result isn't NULL - in case we are
// racing against queue removal.
//
    let mut numvtaps: c_int = READ_ONCE(tap.numvtaps);
    __u32 rxq;
    if (!numvtaps)
    goto out;
    if (numvtaps == 1)
...
//
#[no_mangle]
pub unsafe extern "C" fn tap_del_queues(tap: *mut tap_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn tap_handle_frame(pskb: *mut sk_buff) -> rx_handler_result_t {
    ...
    struct tap_dev *tap;
    struct tap_queue *q;
    let mut features: netdev_features_t = TAP_FEATURES;
...
#[no_mangle]
pub unsafe extern "C" fn tap_get_minor(major: dev_t, tap: *mut tap_dev) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tap_free_minor(major: dev_t, tap: *mut tap_dev) {
...
#[no_mangle]
unsafe extern "C" fn tap_sock_write_space(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tap_sock_destruct(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn tap_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tap_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tap_poll(file: *mut file, wait: *mut poll_table) -> __poll_t {
    static __poll_t tap_poll(struct file *file, poll_table *wait)
    {
    struct tap_queue *q = file.private_data;
    let mut mask: __poll_t = EPOLLERR;
    if (!q)
    goto out;
    mask = 0;
...
// Get packet from user space buffer
    ...
    {
    let mut good_linear: c_int = SKB_MAX_HEAD(TAP_RESERVE);
    struct sk_buff *skb;
    struct tap_dev *tap;
    let mut total_len: c_ulong = iov_iter_count(from);
    let mut len: c_ulong = total_len;
    int err;
    let mut vnet_hdr: virtio_net_hdr = { 0 };
    let mut vnet_hdr_len: c_int = 0;
    let mut hdr_len: c_int = 0;
    let mut copylen: c_int = 0;
    int depth;
...
#[no_mangle]
unsafe extern "C" fn tap_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    ...
    noblock = 1;
    return tap_get_user(q, core::ptr::null_mut(), from, noblock);
    }
// Put packet to the user space buffer
    ...
    vnet_hdr_len = READ_ONCE(q.vnet_hdr_sz);
    ret = tun_vnet_hdr_from_skb(q.flags, core::ptr::null_mut(), skb, &vnet_hdr);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn tap_read_iter(iocb: *mut kiocb, to: *mut iov_iter) -> isize {
    ...
    if ((file.f_flags & O_NONBLOCK) || (iocb.ki_flags & IOCB_NOWAIT))
    noblock = 1;
    ret = tap_do_read(q, to, noblock, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn tap_put_tap_dev(tap: *mut tap_dev) {
    static void tap_put_tap_dev(struct tap_dev *tap)
    {
    dev_put(tap.dev);
    }
#[no_mangle]
unsafe extern "C" fn tap_ioctl_set_queue(file: *mut file, flags: c_uint) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(IFF_DETACH_QUEUE: flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn set_offload(q: *mut tap_queue, arg: c_ulong) -> c_int {
    static int set_offload(struct tap_queue *q, unsigned long arg)
    {
    struct tap_dev *tap;
    netdev_features_t features;
    let mut feature_mask: netdev_features_t = 0;
    tap = rtnl_dereference(q.tap);
    if (!tap)
    return -ENOLINK;
...
// let the user check for future flags
    ...
    ret = dev_set_mac_address_user(tap.dev, &ss, core::ptr::null_mut());
    tap_put_tap_dev(tap);
    rtnl_unlock();
    return ret;
    default:
...
#[no_mangle]
unsafe extern "C" fn tap_get_user_xdp(q: *mut tap_queue, xdp: *mut xdp_buff) -> c_int {
...
// Move network header to the right position for VLAN tagged packets
    ...
    return tap_get_user(q, ctl ? ctl.ptr : core::ptr::null_mut(), &m.msg_iter,
...
#[no_mangle]
unsafe extern "C" fn tap_peek_len(sock: *mut socket) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tap_queue_resize(tap: *mut tap_dev) -> c_int {
    int tap_queue_resize(struct tap_dev *tap)
    {
    struct net_device *dev = tap.dev;
    struct tap_queue *q;
    struct ptr_ring **rings;
    let mut n: c_int = tap.numqueues;
    int ret, i = 0;
...
#[no_mangle]
unsafe extern "C" fn tap_list_add(major: dev_t, device_name: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn tap_destroy_cdev(major: dev_t, tap_cdev: *mut cdev) {
