...
//! Source: drivers/net/ntb_netdev.c
#![no_std]
...
// Time in usecs for tx resource reaper
    let mut tx_time: static unsigned int = 1;
// Number of descriptors to free before resuming tx
    let mut tx_start: static unsigned int = 10;
// Number of descriptors still available before stop upper layer tx
    let mut tx_stop: static unsigned int = 5;
pub const NTB_NETDEV_MAX_QUEUES: c_int = 64;
pub const NTB_NETDEV_DEFAULT_QUEUES: c_int = 1;
    struct ntb_netdev;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ntb_netdev_queue {
    pub ntdev: *mut ntb_netdev,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ntb_netdev {
    pub pdev: *mut pci_dev,
    pub client_dev: *mut device,
    pub ndev: *mut net_device,
    pub num_queues: c_uint,
    pub queues: *mut ntb_netdev_queue,
}

pub const NTB_TX_TIMEOUT_MS: c_int = 1000;
pub const NTB_RXQ_SIZE: c_int = 100;
#[no_mangle]
unsafe extern "C" fn ntb_netdev_update_carrier(dev: *mut ntb_netdev) {
    static void ntb_netdev_update_carrier(struct ntb_netdev *dev)
    {
    struct net_device *ndev;
    let mut any_up: bool = false;
    unsigned int i;
    ndev = dev.ndev;
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_queue_rx_drain(queue: *mut ntb_netdev_queue) {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_event_handler(data: *mut c_void, link_is_up: c_int) {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_tx_timer(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_open(ndev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_close(ndev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_change_mtu(ndev: *mut net_device, new_mtu: c_int) -> c_int {
...
// Bring down the link and dispose of posted rx entries
    ...
    {
    struct ntb_netdev *dev = netdev_priv(ndev);
    let mut running: bool = netif_running(ndev);
    struct ntb_netdev_queue *queue;
...
//
    ...
    err_new:
    if (running) {
    let mut rollback: c_uint = created;
    while (rollback-- > old) {
...
//
    ...
    {
    struct ntb_netdev *dev = netdev_priv(ndev);
    let mut new: c_uint = channels.combined_count;
    let mut old: c_uint = dev.num_queues;
    if (new == old)
    return 0;
    if (new < old)
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_probe(client_dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_remove(client_dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_init_module() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ntb_netdev_exit_module() -> void __exit {
