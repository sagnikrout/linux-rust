...
//! Source: drivers/net/wireless/marvell/mwifiex/cmdevt.c
#![no_std]
...
//
    ...
    cmd_node.cmd_no = 0;
    cmd_node.cmd_flag = 0;
    cmd_node.data_buf = core::ptr::null_mut();
    cmd_node.wait_q_enabled = false;
    if (cmd_node.cmd_skb)
    skb_trim(cmd_node.cmd_skb, 0);
    if (cmd_node.resp_skb) {
    adapter.if_ops.cmdrsp_complete(adapter, cmd_node.resp_skb);
    cmd_node.resp_skb = core::ptr::null_mut();
    }
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn if(cmd_size: cmd_node->cmd_skb->len <) -> else {
...
//
    ...
    cmd_node.cmd_skb, core::ptr::null_mut());
    ...
    spin_lock_bh(&adapter.mwifiex_cmd_lock);
    adapter.curr_cmd = core::ptr::null_mut();
    spin_unlock_bh(&adapter.mwifiex_cmd_lock);
...
//
#[no_mangle]
unsafe extern "C" fn mwifiex_dnld_sleep_confirm_cmd(adapter: *mut mwifiex_adapter) -> c_int {
    ...
    ret = adapter.if_ops.host_to_card(adapter,
    MWIFIEX_USB_EP_CMD_EVENT,
    sleep_cfm_tmp, core::ptr::null_mut());
    if (ret != -EBUSY)
    dev_kfree_skb_any(sleep_cfm_tmp);
    } else {
    skb_push(adapter.sleep_cfm, adapter.intf_hdr_len);
    ret = adapter.if_ops.host_to_card(adapter, MWIFIEX_TYPE_CMD,
    adapter.sleep_cfm, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn mwifiex_alloc_cmd_buffer(adapter: *mut mwifiex_adapter) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn mwifiex_free_cmd_buffer(adapter: *mut mwifiex_adapter) {
...
// Release struct cmd_ctrl_node
    ...
    adapter.cmd_pool = core::ptr::null_mut();
    }
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn mwifiex_process_event(adapter: *mut mwifiex_adapter) -> c_int {
...
// Clear BSS_NO_BITS from event
    ...
    adapter.event_skb = core::ptr::null_mut();
...
//
    ...
    if (!adapter) {
    pr_err("PREP_CMD: adapter is core::ptr::null_mut()\n");
    return -1;
    }
...
//
    ...
    struct cmd_ctrl_node *cmd_node)
    {
    struct host_cmd_ds_command *host_cmd = core::ptr::null_mut();
    u16 command;
    let mut add_tail: bool = true;
    host_cmd = (struct host_cmd_ds_command *) (cmd_node.cmd_skb.data);
    if (!host_cmd) {
    mwifiex_dbg(adapter, ERROR, "QUEUE_CMD: host_cmd is core::ptr::null_mut()\n");
    return;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn mwifiex_exec_next_cmd(adapter: *mut mwifiex_adapter) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn mwifiex_process_cmdresp(adapter: *mut mwifiex_adapter) -> c_int {
    ...
    mwifiex_dbg(adapter, ERROR,
    "CMD_RESP: core::ptr::null_mut() curr_cmd, %#x\n",
...
// Copy original response back to response buffer
    ...
    let mut size: u16 = le16_to_cpu(resp.size);
...
#[no_mangle]
pub unsafe extern "C" fn mwifiex_process_assoc_resp(adapter: *mut mwifiex_adapter) {
...
//
    ...
    {
    struct cmd_ctrl_node *cmd_node = core::ptr::null_mut(), *tmp_node;
...
//
    ...
    {
    struct cmd_ctrl_node *cmd_node = core::ptr::null_mut();
    if ((adapter.curr_cmd) &&
...
//
    ...
    let mut conditions: u32 = le32_to_cpu(phs_cfg.params.hs_config.conditions);
...
//
    ...
    let mut result: u16 = le16_to_cpu(cmd.result);
    let mut command: u16 = le16_to_cpu(cmd.command);
    let mut seq_num: u16 = le16_to_cpu(cmd.seq_num);
    if (!upld_len) {
    mwifiex_dbg(adapter, ERROR,
    "%s: cmd size is 0\n", __func__);
    return;
...
//
    ...
    let mut action: u16 = le16_to_cpu(ps_mode.action);
    let mut ps_bitmap: u16 = le16_to_cpu(ps_mode.params.ps_bitmap);
