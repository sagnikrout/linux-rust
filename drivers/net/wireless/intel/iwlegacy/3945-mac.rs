...
//! Source: drivers/net/wireless/intel/iwlegacy/3945-mac.c
#![no_std]
...
//
    ...
    struct il_queue *q = core::ptr::null_mut();
    struct il_device_cmd *out_cmd;
    struct il_cmd_meta *out_meta;
    dma_addr_t phys_addr;
    dma_addr_t txcmd_phys;
    let mut txq_id: c_int = skb_get_queue_mapping(skb);
    u16 len, idx, hdr_len;
...
// Tell device the write idx *just past* this latest filled TFD
    ...
    let mut add_time: u32 = le64_to_cpu(params.start_time);
    int rc;
    int spectrum_resp_status;
    let mut duration: c_int = le16_to_cpu(params.duration);
...
// give the HW RF Kill time to activate...
    ...
    let mut rate: u8 = beacon.beacon_notify_hdr.rate;
...
//
    ...
    mutex_lock(&il.mutex);
    if (test_bit(S_EXIT_PENDING, &il.status) || il.txq == core::ptr::null_mut())
    goto out;
...
//
    ...
    let mut old_rfkill: bool = test_bit(S_RFKILL, &il.status);
    bool new_rfkill =
...
// restore RXON assoc
    ...
    struct il_priv *il = hw.priv;
    let mut ret: c_int = 0;
    let mut sta_id: u8 = IL_INVALID_STATION;
    u8 static_key;
    D_MAC80211("enter\n");
...
//
    ...
    static DEVICE_ATTR(temperature, 0444, il3945_show_temperature, core::ptr::null_mut());
    static ssize_t
    ...
    {
    struct il_priv *il = dev_get_drvdata(d);
    let mut flags: u32 = simple_strtoul(buf, core::ptr::null_mut(), 0);
    mutex_lock(&il.mutex);
...
// Cancel any currently running scans...
    ...
    {
    struct il_priv *il = dev_get_drvdata(d);
    let mut filter_flags: u32 = simple_strtoul(buf, core::ptr::null_mut(), 0);
    mutex_lock(&il.mutex);
...
// Cancel any currently running scans...
    ...
    let mut size: u32 = sizeof(measure_report), len = 0, ofs = 0;
    ...
    let mut type: u8 = IL_MEASURE_BASIC;
    u8 buffer[32];
    u8 channel;
    if (count) {
    char *p = buffer;
    strscpy(buffer, buf, sizeof(buffer));
    channel = simple_strtoul(p, core::ptr::null_mut(), 0);
    if (channel)
    ...
    p++;
    if (*p)
    type = simple_strtoul(p + 1, core::ptr::null_mut(), 0);
    }
    ...
    {
    struct il_priv *il = dev_get_drvdata(d);
    il.retry_rate = simple_strtoul(buf, core::ptr::null_mut(), 0);
    if (il.retry_rate <= 0)
    il.retry_rate = 1;
    return count;
    }
...
// all this shit doesn't belong into sysfs anyway
    ...
    static DEVICE_ATTR(channels, 0400, il3945_show_channels, core::ptr::null_mut());
    static ssize_t
    ...
    const char *buf, size_t count)
    {
    let mut __maybe_unused: *mut il_priv il = dev_get_drvdata(d);
    int ant;
    if (count == 0)
    return 0;
    ...
    static DEVICE_ATTR(status, 0444, il3945_show_status, core::ptr::null_mut());
    static ssize_t
    ...
    static DEVICE_ATTR(dump_errors, 0200, core::ptr::null_mut(), il3945_dump_error_log);
//
...
pub const IL3945_MAX_PROBE_REQUEST: c_int = 200;
    static int
...
//
    ...
    il3945_mac_ops.hw_scan = core::ptr::null_mut();
    }
