...
//! Source: drivers/net/wireless/intel/iwlegacy/4965-mac.c
#![no_std]
...
// to an SKB, so we need to unmap and free potential storage
    if (rxq.pool[i].page != core::ptr::null_mut()) {
    dma_unmap_page(&il.pci_dev.dev,
    rxq.pool[i].page_dma,
    PAGE_SIZE << il.hw_params.rx_page_order,
    DMA_FROM_DEVICE);
    __il_free_pages(il, rxq.pool[i].page);
    rxq.pool[i].page = core::ptr::null_mut();
    }
...
pub const SMALL_PACKET_SIZE: c_int = 256;
    static void
...
// N_RX_MPDU (HT high-throughput N frames).
    ...
    let mut rx_status: ieee80211_rx_status = {};
...
// This will be used later in il_hdl_rx() for N_RX_MPDU.
    ...
    let mut passive_dwell: u16 = 0;
    let mut active_dwell: u16 = 0;
    int added, i;
    u16 channel;
...
// ant = ind;
    ...
    let mut rate_flags: u32 = 0;
    u16 cmd_len;
    let mut rx_chain: u16 = 0;
    enum nl80211_band band;
    let mut n_probes: u8 = 0;
    let mut rx_ant: u8 = il.hw_params.valid_rx_ant;
    u8 rate;
    let mut is_active: bool = false;
    int chan_mod;
    u8 active_chains;
    let mut scan_tx_antennas: u8 = il.hw_params.valid_tx_ant;
    int ret;
    ...
    u16 interval;
    u32 extra;
    let mut suspend_time: u32 = 100;
    let mut scan_suspend_time: u32 = 100;
...
pub const IL_TX_QUEUE_MSK: c_uint = 0xfffff;
    static bool
...
pub const IL_NUM_RX_CHAINS_MULTIPLE: c_int = 3;
pub const IL_NUM_RX_CHAINS_SINGLE: c_int = 2;
pub const IL_NUM_IDLE_CHAINS_DUAL: c_int = 2;
pub const IL_NUM_IDLE_CHAINS_SINGLE: c_int = 1;
//
...
//
    ...
    {
    let mut is_single: bool = il4965_is_single_rx_stream(il);
    let mut is_cam: bool = !test_bit(S_POWER_PMI, &il.status);
...
// exactly when to expect beacons, therefore only when we're associated.
    ...
    let mut num_active_rx: c_int = 0;
    let mut total_silence: c_int = 0;
...
// reset accumulative stats for "no-counter" type stats
    ...
    {
    let mut recalib_seconds: c_int = 60;
    bool change;
    struct il_rx_pkt *pkt = rxb_addr(rxb);
...
//
    ...
    {
    let mut fc: __le16 = hdr.frame_control;
    let mut tx_flags: __le32 = tx_cmd.tx_flags;
...
//
    ...
    struct il_station_priv *sta_priv = core::ptr::null_mut();
    ...
    u8 hdr_len;
    u8 sta_id;
    let mut wait_write_ptr: u8 = 0;
    let mut tid: u8 = 0;
    u8 *qc = core::ptr::null_mut();
    unsigned long flags;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ieee80211_is_assoc_req(fc)) -> else {
    else if (ieee80211_is_assoc_req(fc))
    D_TX("Sending ASSOC frame\n");
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ieee80211_is_reassoc_req(fc)) -> else {
...
// aggregated HW queue
    ...
    let mut ssn: u16 = IEEE80211_SEQ_TO_SN(tid_data.seq_number);
    let mut tx_fifo: c_int = il4965_get_fifo_from_tid(tid);
...
// avoid atomic ops if this isn't a client
    ...
    if (WARN_ON_ONCE(skb == core::ptr::null_mut()))
    continue;
    hdr = (struct ieee80211_hdr *) skb.data;
    if (ieee80211_is_data_qos(hdr.frame_control))
    nfreed++;
    il4965_tx_status(il, skb, txq_id >= IL4965_FIRST_AMPDU_QUEUE);
    txq.skbs[txq.q.read_ptr] = core::ptr::null_mut();
...
//
    ...
    {
    int i, sh, ack;
    let mut seq_ctl: u16 = le16_to_cpu(ba_resp.seq_ctl);
    let mut scd_flow: u16 = le16_to_cpu(ba_resp.scd_flow);
...
// update driver's record of tx frame's status.
    ...
    let mut ret: c_int = IL_INVALID_STATION;
    unsigned long flags;
...
//
    ...
    struct ieee80211_tx_info *info = core::ptr::null_mut();
    struct ieee80211_hdr *hdr = core::ptr::null_mut();
    let mut rate_n_flags: u32 = le32_to_cpu(tx_resp.rate_n_flags);
    int i, sh, idx;
    u16 seq;
    if (agg.wait_for_ba)
...
// Two or more frames were attempted; expect block-ack
    ...
    let mut start: c_int = agg.start_idx;
    struct sk_buff *skb;
// Construct bit-map of pending frames within Tx win
    ...
    sh = 0xff - (start - idx);
#[no_mangle]
pub unsafe extern "C" fn if(0: sh <) -> else {
    sh = start - idx;
    start = idx;
    bitmap = bitmap << sh;
    sh = 0;
...
//
    ...
    let mut txq_id: c_int = SEQ_TO_QUEUE(sequence);
    let mut idx: c_int = SEQ_TO_IDX(sequence);
    struct il_tx_queue *txq = &il.txq[txq_id];
    struct sk_buff *skb;
    struct ieee80211_hdr *hdr;
    struct ieee80211_tx_info *info;
    struct il4965_tx_resp *tx_resp = (void *)&pkt.u.raw[0];
    let mut status: u32 = le32_to_cpu(tx_resp.u.status);
    int tid;
    int sta_id;
    int freed;
    u8 *qc = core::ptr::null_mut();
...
//
    ...
    spin_lock_irqsave(&il.sta_lock, flags);
    if (txq.sched_retry) {
    let mut scd_ssn: u32 = il4965_get_scd_ssn(tx_resp);
...
// check if BAR is needed
    ...
    il4965_free_tfds_in_queue(il, sta_id, tid, freed);
#[no_mangle]
pub unsafe extern "C" fn if(IL_INVALID_STATION: sta_id ==) -> else {
...
//
    ...
    struct il_tx_queue *txq = core::ptr::null_mut();
    struct il_ht_agg *agg;
    int idx;
    int sta_id;
    int tid;
...
// (in Tx queue's circular buffer) of first TFD/frame in win
    let mut ba_resp_scd_ssn: u16 = le16_to_cpu(ba_resp.scd_ssn);
...
// calculate mac80211 ampdu sw queue to wake
    let mut freed: c_int = il4965_tx_queue_reclaim(il, scd_flow, idx);
    il4965_free_tfds_in_queue(il, sta_id, tid, freed);
...
// sta_id_r = IL_INVALID_STATION;
    ret = il_add_station_common(il, addr, 0, core::ptr::null_mut(), &sta_id);
    if (ret) {
...
// Set up default rate scaling table in device's station table
    ...
    let mut cmd_size: usize = sizeof(struct il_wep_cmd);
    struct il_host_cmd cmd = {
    .id = C_WEPKEY,
    .data = wep_cmd,
    .flags = CMD_SYNC,
    ...
    let mut key_size: u8 = il._4965.wep_keys[i].key_size;
    ...
    {
    int ret;
    let mut idx: c_int = keyconf.keyidx;
    lockdep_assert_held(&il.mutex);
...
// but keys in device are clear anyway so return success
    ...
    {
    int ret;
    let mut len: c_int = keyconf.keylen;
    let mut idx: c_int = keyconf.keyidx;
    lockdep_assert_held(&il.mutex);
...
//
    ...
    u8 sta_id;
    spin_lock_irqsave(&il.sta_lock, flags);
    sta_id = il_prep_station(il, il_bcast_addr, false, core::ptr::null_mut());
    if (sta_id == IL_INVALID_STATION) {
...
//
    ...
    unsigned long flags;
    struct il_link_quality_cmd *link_cmd;
    let mut sta_id: u8 = il.hw_params.bcast_id;
    link_cmd = il4965_sta_alloc_lq(il, sta_id);
    if (!link_cmd) {
...
// Set up packet rate and flags
    ...
    let mut addr: dma_addr_t = get_unaligned_le32(&tb.lo);
...
//
    ...
    let mut idx: c_int = txq.q.read_ptr;
    int i;
    int num_tbs;
    tfd = &tfd_tmp[idx];
...
// dont send host command if rf-kill is on
    ...
    let mut rate: u8 = il4965_hw_get_rate(beacon.beacon_notify_hdr.rate_n_flags);
...
// due to software, hardware, or critical temperature RFKILL
    ...
    {
    struct il_rx_pkt *pkt = rxb_addr(rxb);
    let mut flags: u32 = le32_to_cpu(pkt.u.card_state_notif.flags);
    let mut status: c_ulong = il.status;
...
//
    ...
    static DEVICE_ATTR(temperature, 0444, il4965_show_temperature, core::ptr::null_mut());
    static ssize_t
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct il4965_firmware_pieces {
    pub boot: *const *const *const *const *const void inst, data, init, init_data,,
    pub boot_size: size_t inst_size, data_size, init_size, init_data_size,,
}
...
//
    ...
    int err;
    struct il4965_firmware_pieces pieces;
    let mut api_max: c_uint = il.cfg.ucode_api_max;
    let mut api_min: c_uint = il.cfg.ucode_api_min;
    u32 api_ver;
    let mut max_probe_length: u32 = 200;
...
// try next, if any
    ...
    {
    int i;
    let mut max: c_int = ARRAY_SIZE(desc_lookup_text);
    if (num < max)
...
// Map each Tx/cmd queue to its corresponding fifo
    ...
    let mut ac: c_int = default_queue_to_tx_fifo[i];
...
// Stop the device, and put it in low power state
    ...
    il.beacon_skb = core::ptr::null_mut();
...
pub const MAX_HW_RESTARTS: c_int = 5;
    static int
...
// even if interface is down
    ...
    int ret;
    u8 sta_id;
    let mut is_default_wep_key: bool = false;
    D_MAC80211("enter\n");
    if (il.cfg.mod_params.sw_crypto) {
...
//
    ...
    let mut is_ap: bool = vif.type == NL80211_IFTYPE_STATION;
    int ret;
    u8 sta_id;
...
//
    ...
    {
    struct il_priv *il = hw.priv;
    let mut filter_or: __le32 = 0, filter_nand = 0;
...
// Find out whether to activate Tx queue
    let mut active: c_int = test_bit(txq_id, &il.txq_ctx_active_msk) ? 1 : 0;
// Set up and activate
    il_wr_prph(il, IL49_SCD_QUEUE_STATUS_BITS(txq_id),
    ...
    mutex_init(&il.mutex);
    il.ieee_channels = core::ptr::null_mut();
    il.ieee_rates = core::ptr::null_mut();
...
// If platform's RF_KILL switch is NOT set to KILL
    ...
    il.workqueue = core::ptr::null_mut();
    out_free_irq:
