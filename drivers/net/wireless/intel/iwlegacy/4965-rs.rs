...
//! Source: drivers/net/wireless/intel/iwlegacy/4965-rs.c
#![no_std]
...
pub const NUM_TRY_BEFORE_ANT_TOGGLE: c_int = 1;
pub const IL_NUMBER_TRY: c_int = 1;
pub const IL_HT_NUMBER_TRY: c_int = 3;

// max allowed rate miss before sync LQ cmd
pub const IL_MISSED_RATE_MAX: c_int = 15;
...
// The oldest age we want to keep
    let mut oldest_time: u32 = curr_time - TID_MAX_TIME_DIFF;
...
//
    ...
    {
    let mut curr_time: u32 = jiffies_to_msecs(jiffies);
    u32 time_diff;
    s32 idx;
    struct il_traffic_load *tl = core::ptr::null_mut();
    u8 tid;
...
//
    ...
    {
    struct il_rate_scale_data *win = core::ptr::null_mut();
    let mut mask: static u64 = (((u64) 1) << (RATE_MAX_WINDOW - 1));
    s32 fail_count, tpt;
...
//
    ...
    {
    let mut ant_msk: u32 = (rate_n_flags & RATE_MCS_ANT_ABC_MSK);
...
//
    ...
    int rate_type)
    {
    let mut high: u8 = RATE_INVALID;
    let mut low: u8 = RATE_INVALID;
...
// Find the next rate that is in the rate mask
    ...
    u16 rate_mask;
    u16 high_low;
    let mut switch_to_legacy: u8 = 0;
    let mut is_green: u8 = lq_sta.is_green;
...
// are always supported (spec demand)
    ...
    rate_mask = il4965_rs_get_supported_rates(lq_sta, core::ptr::null_mut(), tbl.lq_type);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_siso(tbl->lq_type)) -> else {
    else if (is_siso(tbl.lq_type))
    ht_tbl_pointer = expected_tpt_siso40MHz;
#[no_mangle]
pub unsafe extern "C" fn if(lq_sta->is_dup): is_mimo2(tbl->lq_type) && (!tbl->is_ht40 ||) -> else {
...
// "active" values
    ...
    let mut active_sr: i32 = active_tbl.win[idx].success_ratio;
    let mut active_tpt: i32 = active_tbl.expected_tpt[idx];
...
#[no_mangle]
pub unsafe extern "C" fn if(RATE_INVALID: high !=) -> else {
    start_hi = high;
    rate = high;
...
//
    ...
    u16 rate_mask;
    s32 rate;
    let mut is_green: i8 = lq_sta.is_green;
...
//
    ...
    u8 start_action;
    let mut valid_tx_ant: u8 = il.hw_params.valid_tx_ant;
    let mut tx_chains_num: u8 = il.hw_params.tx_chains_num;
    let mut ret: c_int = 0;
    let mut update_search_tbl_counter: u8 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(IL_LEGACY_SWITCH_MIMO2_AC: tbl->action ==) -> else {
...
//
    ...
    search_tbl.ant_type = ANT_AB;
#[no_mangle]
pub unsafe extern "C" fn if(IL_SISO_SWITCH_MIMO2_AC: tbl->action ==) -> else {
    ...
    if (tbl.is_SGI) {
    let mut tpt: i32 = lq_sta.last_tpt / 100;
...
#[no_mangle]
pub unsafe extern "C" fn if(IL_MIMO2_SWITCH_SISO_B: tbl->action ==) -> else {
...
//
    ...
    int i;
    int active_tbl;
    let mut flush_interval_passed: c_int = 0;
    struct il_priv *il;
    il = lq_sta.drv;
...
//
    ...
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb.data;
    let mut low: c_int = RATE_INVALID;
    let mut high: c_int = RATE_INVALID;
    int idx;
    int i;
    struct il_rate_scale_data *win = core::ptr::null_mut();
    let mut current_tpt: c_int = IL_INVALID_VALUE;
    let mut low_tpt: c_int = IL_INVALID_VALUE;
    let mut high_tpt: c_int = IL_INVALID_VALUE;
    u32 fail_count;
    let mut scale_action: i8 = 0;
    u16 rate_mask;
    let mut update_lq: u8 = 0;
    struct il_scale_tbl_info *tbl, *tbl1;
    let mut rate_scale_idx_msk: u16 = 0;
    let mut is_green: u8 = 0;
    let mut active_tbl: u8 = 0;
    let mut done_search: u8 = 0;
    u16 high_low;
    s32 sr;
    u8 tid;
...
// Get expected throughput table and history win for current rate
    if (!tbl.expected_tpt) {
    IL_ERR("tbl.expected_tpt is core::ptr::null_mut()\n");
    return;
    }
...
// No throughput measured yet for adjacent rates; try increase.
    ...
    scale_action = 1;
#[no_mangle]
pub unsafe extern "C" fn if(RATE_INVALID: low !=) -> else {
...
//
    ...
    il4965_rs_set_expected_tpt_table(lq_sta, tbl);
    il4965_rs_fill_link_cmd(core::ptr::null_mut(), lq_sta, rate);
    ...
    let mut __maybe_unused: *mut il_priv il = (struct il_priv *)il_r;
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct il_lq_sta *lq_sta = il_sta;
    int rate_idx;
    D_RATE("rate scale calculate new rate for skb\n");
...
#[no_mangle]
pub unsafe extern "C" fn if(NL80211_BAND_5GHZ: sband->band ==) -> else {
...
// Set last_txrate_idx to lowest rate
    ...
    let mut idx: c_int = 0;
    int rate_idx;
    let mut repeat_rate: c_int = 0;
    let mut ant_toggle_cnt: u8 = 0;
    let mut use_ht_possible: u8 = 1;
    let mut valid_tx_ant: u8 = 0;
...
// rate scale requires free function to be implemented
    ...
    {
    let mut __maybe_unused: *mut il_priv il = il_r;
    D_RATE("enter\n");
    D_RATE("leave\n");
    }
    static void
...
// rate_n_flags = lq_sta->dbg_fixed_rate;
    ...
    if (lq_sta.dbg_fixed_rate) {
    il4965_rs_fill_link_cmd(core::ptr::null_mut(), lq_sta, lq_sta.dbg_fixed_rate);
