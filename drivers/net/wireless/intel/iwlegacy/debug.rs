...
//! Source: drivers/net/wireless/intel/iwlegacy/debug.c
#![no_std]
...
pub const BYTE1_MASK: c_uint = 0x000000ff;;
pub const BYTE2_MASK: c_uint = 0x0000ffff;;
pub const BYTE3_MASK: c_uint = 0x00ffffff;;
    static ssize_t
...
// default is to dump the entire data segment
    ...
    let mut max_sta: c_int = il.hw_params.max_stations;
    char *buf;
    int i, j, pos = 0;
    ssize_t ret;
// Add 30 for initial string
    let mut bufsz: usize = 30 + sizeof(char) * 500 * max_sta;
    buf = kmalloc(bufsz, GFP_KERNEL);
    if (!buf)
    return -ENOMEM;
    pos +=
    ...
    char *buf;
    u16 eeprom_ver;
    let mut eeprom_len: usize = il.cfg.eeprom_size;
...
// for the ACs, display the stop count too
    ...
    let mut cnt: c_int = 0;
    char *buf;
    let mut bufsz: c_int = sizeof(struct il_sensitivity_data) * 4 + 100;
    ssize_t ret;
    ...
    let mut cnt: c_int = 0;
    char *buf;
    let mut bufsz: c_int = sizeof(struct il_chain_noise_data) * 4 + 100;
    ssize_t ret;
...
//
    ...
    il.debugfs_dir = core::ptr::null_mut();
    }
