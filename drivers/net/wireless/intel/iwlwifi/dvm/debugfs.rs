...
//! Source: drivers/net/wireless/intel/iwlwifi/dvm/debugfs.c
#![no_std]
...
// file operation
    ...
    ssize_t ret;
    let mut i: c_int = 0;
    let mut device_format: bool = false;
...
// Add 30 for initial string
    let mut bufsz: usize = 30 + sizeof(char) * 500 * (priv.num_stations);
    buf = kmalloc(bufsz, GFP_KERNEL);
    if (!buf)
    return -ENOMEM;
    ...
    ssize_t ret;
    struct iwl_priv *priv = file.private_data;
    let mut pos: c_int = 0, ofs = 0, buf_size = 0;
    const u8 *ptr;
    char *buf;
    u16 nvm_ver;
    let mut eeprom_len: usize = priv.eeprom_blob_size;
...
// 4 characters for byte 0xYY
    ...
    {
    struct iwl_priv *priv = file.private_data;
    struct ieee80211_channel *channels = core::ptr::null_mut();
    const struct ieee80211_supported_band *supp_band = core::ptr::null_mut();
    let mut pos: c_int = 0, i, bufsz = PAGE_SIZE;
    char *buf;
    ssize_t ret;
    ...
    char buf[512];
    let mut pos: c_int = 0;
    let mut bufsz: usize = sizeof(buf);
...
#[no_mangle]
unsafe extern "C" fn iwl_statistics_flag(priv: *mut iwl_priv, buf: *mut c_char, bufsz: c_int) -> c_int {
...
//
    ...
    let mut pos: c_int = 0;
    char *buf;
    let mut bufsz: c_int = (sizeof(struct statistics_tx) * 48) + 250;
    ssize_t ret;
...
//
    ...
    let mut pos: c_int = 0;
    char *buf;
    let mut bufsz: c_int = sizeof(struct statistics_general) * 10 + 300;
    ssize_t ret;
...
//
    ...
    let mut pos: c_int = 0;
    char *buf;
    let mut bufsz: c_int = (sizeof(struct statistics_bt_activity) * 24) + 200;
    ssize_t ret;
...
//
    ...
    let mut cnt: c_int = 0;
    char *buf;
    let mut bufsz: c_int = sizeof(struct iwl_sensitivity_data) * 4 + 100;
    ssize_t ret;
    ...
    let mut cnt: c_int = 0;
    char *buf;
    let mut bufsz: c_int = sizeof(struct iwl_chain_noise_data) * 4 + 100;
    ssize_t ret;
...
#[no_mangle]
unsafe extern "C" fn iwl_cmd_echo_test(priv: *mut iwl_priv) -> c_int {
...
// check that the interface is up
    ...
    return -EFAULT;
    if (event_log_flag == 1)
    iwl_dump_nic_event_log(priv, true, core::ptr::null_mut());
    return count;
    }
    ...
    {
    struct iwl_priv *priv = file.private_data;
    let mut fw_restart: bool = iwlwifi_mod_params.fw_restart;
    int __maybe_unused ret;
    iwlwifi_mod_params.fw_restart = true;
    mutex_lock(&priv.mutex);
// take the return value to make compiler happy - it will fail anyway
    ret = iwl_dvm_send_cmd_pdu(priv, REPLY_ERROR, 0, 0, core::ptr::null_mut());
    mutex_unlock(&priv.mutex);
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_dbgfs_register(priv: *mut iwl_priv, dbgfs_dir: *mut dentry) {
