...
//! Source: drivers/net/wireless/intel/iwlwifi/dvm/rxon.c
#![no_std]
...
// clear both MIX and PURE40 mode flag
    ...
    {
    let mut old_filter: __le32 = send.filter_flags;
    int ret;
    ...
    let mut old_filter: __le32 = send.filter_flags;
    let mut old_dev_type: u8 = send.dev_type;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn iwl_adjust_beacon_interval(beacon_val: u16, max_beacon_val: u16) -> u16 {
...
//
    ...
    u64 tsf;
    s32 interval_tm, rem;
    struct ieee80211_conf *conf = core::ptr::null_mut();
    u16 beacon_int;
...
#[no_mangle]
unsafe extern "C" fn iwl_set_tx_power(priv: *mut iwl_priv, tx_power: i8, force: bool) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwlagn_set_pan_params(priv: *mut iwl_priv) -> c_int {
    int iwlagn_set_pan_params(struct iwl_priv *priv)
    {
    struct iwl_wipan_params_cmd cmd;
    struct iwl_rxon_context *ctx_bss, *ctx_pan;
    let mut slot0: c_int = 300, slot1 = 0;
    int ret;
...
// only 2 slots are currently allowed
    ...
    let mut bcnint: c_int = ctx_pan.beacon_int;
    let mut dtim: c_int = ctx_pan.vif.bss_conf.dtim_period ?: 1;
...
// clear the HT channel mode before set the mode
    rxon.flags &= ~(RXON_FLG_CHANNEL_MODE_MSK |
    RXON_FLG_CTRL_CHANNEL_LOC_HI_MSK);
    if (iwl_is_ht40_tx_allowed(priv, ctx, core::ptr::null_mut())) {
// pure ht40
...
#[no_mangle]
pub unsafe extern "C" fn iwl_set_rxon_ht(priv: *mut iwl_priv, ht_conf: *mut iwl_ht_config) {
...
//
    ...
    struct iwl_rxon_context *ctx)
    {
    let mut band: enum nl80211_band = ch.band;
    let mut channel: u16 = ch.hw_value;
...
// Check if we are switching association toggle
    ...
    let mut basic: c_ulong = ctx.vif.bss_conf.basic_rates;
    int i;
...
//
#[no_mangle]
pub unsafe extern "C" fn iwlagn_commit_rxon(priv: *mut iwl_priv, ctx: *mut iwl_rxon_context) -> c_int {
    int iwlagn_commit_rxon(struct iwl_priv *priv, struct iwl_rxon_context *ctx)
    {
// cast away the const for active_rxon in this function
    struct iwl_rxon_cmd *active = (void *)(uintptr_t)&ctx.active;
    let mut new_assoc: bool = !!(ctx.staging.filter_flags & RXON_FILTER_ASSOC_MSK);
    int ret;
    lockdep_assert_held(&priv.mutex);
...
#[no_mangle]
pub unsafe extern "C" fn iwlagn_mac_config(hw: *mut ieee80211_hw, radio_idx: c_int, changed: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwlagn_chain_noise_reset(priv: *mut iwl_priv) {
...
//
    ...
    if (unlikely(!ctx.vif)) {
    IWL_DEBUG_MAC80211(priv, "leave - vif is core::ptr::null_mut()\n");
    mutex_unlock(&priv.mutex);
    return;
    }
    ...
    ctx.staging.filter_flags &= ~RXON_FILTER_ASSOC_MSK;
    priv.beacon_ctx = core::ptr::null_mut();
    }
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn iwlagn_post_scan(priv: *mut iwl_priv) {
