...
//! Source: drivers/net/wireless/intel/iwlwifi/dvm/sta.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn iwl_sta_ucode_activate(priv: *mut iwl_priv, sta_id: u8) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_add_sta_callback(priv: *mut iwl_priv, rxb: *mut iwl_rx_cmd_buffer) {
    ...
    let mut __maybe_unused: u8 sta_id = sta.sta.sta_id;
    struct iwl_rx_packet *pkt;
...
// flags |= STA_FLG_HT40_EN_MSK;
    ...
    struct ieee80211_sta *sta)
    {
    let mut sta_id: u8 = iwl_sta_id(sta);
    __le32 flags, mask;
...
//
    ...
    int i;
    let mut sta_id: u8 = IWL_INVALID_STATION;
    if (is_ap)
    sta_id = ctx.ap_sta_id;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_broadcast_ether_addr(addr)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn iwl_sta_ucode_deactivate(priv: *mut iwl_priv, sta_id: u8) {
...
//
    ...
    priv.stations[sta_id].lq = core::ptr::null_mut();
    }
...
// all the way down to 1M in IEEE order, and then spin on 1M
    ...
    r = IWL_RATE_6M_INDEX;
#[no_mangle]
pub unsafe extern "C" fn if(ctx->vif->p2p: ctx && ctx->vif &&) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_restore_stations(priv: *mut iwl_priv, ctx: *mut iwl_rxon_context) {
    void iwl_restore_stations(struct iwl_priv *priv, struct iwl_rxon_context *ctx)
    {
    struct iwl_addsta_cmd sta_cmd;
    let mut zero_lq: static struct iwl_link_quality_cmd = {};
...
#[no_mangle]
pub unsafe extern "C" fn iwl_get_free_ucode_key_offset(priv: *mut iwl_priv) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_dealloc_bcast_stations(priv: *mut iwl_priv) {
    ...
    priv.stations[i].lq = core::ptr::null_mut();
    }
    spin_unlock_bh(&priv.sta_lock);
    }
...
// sta_id_r = IWL_INVALID_STATION;
    ret = iwl_add_station_common(priv, ctx, addr, 0, core::ptr::null_mut(), &sta_id);
    if (ret) {
...
//
    ...
    let mut cmd_size: usize = sizeof(struct iwl_wep_cmd);
    struct iwl_host_cmd cmd = {
    .id = ctx.wep_key_cmd,
    .data = { wep_cmd, },
    };
...
// key pointer (offset)
    ...
    {
    let mut sta_id: u8 = iwlagn_key_sta_id(priv, vif, sta);
    if (sta_id == IWL_INVALID_STATION)
    return;
...
// cancel scan failed, just live w/ bad key and rely
    ...
    {
    struct iwl_addsta_cmd sta_cmd;
    let mut sta_id: u8 = iwlagn_key_sta_id(priv, ctx.vif, sta);
    __le16 key_flags;
...
// pre-fill phase 1 key into device cache
    ...
    sta ? sta.addr : core::ptr::null_mut(), ret);
    return ret;
    }
//
...
//
    ...
    u8 sta_id;
    spin_lock_bh(&priv.sta_lock);
    sta_id = iwl_prep_station(priv, ctx, iwl_bcast_addr, false, core::ptr::null_mut());
    if (sta_id == IWL_INVALID_STATION) {
...
//
    ...
    {
    struct iwl_link_quality_cmd *link_cmd;
    let mut sta_id: u8 = ctx.bcast_sta_id;
    link_cmd = iwl_sta_alloc_lq(priv, ctx, sta_id);
    if (!link_cmd) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_update_bcast_stations(priv: *mut iwl_priv) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_sta_tx_modify_enable_tid(priv: *mut iwl_priv, sta_id: c_int, tid: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_sta_modify_sleep_tx_count(priv: *mut iwl_priv, sta_id: c_int, cnt: c_int) {
