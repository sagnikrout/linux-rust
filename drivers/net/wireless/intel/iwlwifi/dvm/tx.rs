...
//! Source: drivers/net/wireless/intel/iwlwifi/dvm/tx.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ieee80211_is_back_req(fc)) -> else {
...
//
    ...
    struct iwl_station_priv *sta_priv = core::ptr::null_mut();
    ...
    let mut is_agg: bool = false, is_data_qos = false;
    int txq_id;
    if (info.control.vif)
...
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_TX_CTL_SEND_AFTER_DTIM: info->flags &) -> else {
//
...
#[no_mangle]
unsafe extern "C" fn iwlagn_alloc_agg_txq(priv: *mut iwl_priv, mq: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwlagn_dealloc_agg_txq(priv: *mut iwl_priv, q: c_int) {
...
#[no_mangle]
unsafe extern "C" fn iwlagn_check_ratid_empty(priv: *mut iwl_priv, sta_id: c_int, tid: u8) {
...
#[no_mangle]
unsafe extern "C" fn iwlagn_count_agg_tx_err_status(priv: *mut iwl_priv, status: u16) {
...
#[no_mangle]
pub unsafe extern "C" fn iwlagn_get_scd_ssn(tx_resp: *mut iwlagn_tx_resp) -> u32 {
    ...
    let mut status: u32 = le16_to_cpu(tx_resp.status.status);
    int i;
...
// Construct bit-map of pending frames within Tx window
    ...
    let mut retry_cnt: u8 = (fstatus & AGG_TX_TRY_MSK) >> AGG_TX_TRY_POS;
...
#[no_mangle]
unsafe extern "C" fn iwlagn_count_tx_err_status(priv: *mut iwl_priv, status: u16) {
    ...
    struct iwlagn_tx_resp *tx_resp)
    {
    let mut status: u16 = le16_to_cpu(tx_resp.status.status);
    info.flags &= ~IEEE80211_TX_CTL_AMPDU;
...
#[no_mangle]
pub unsafe extern "C" fn iwlagn_rx_reply_tx(priv: *mut iwl_priv, rxb: *mut iwl_rx_cmd_buffer) {
    ...
    let mut __maybe_unused: int cmd_index = SEQ_TO_INDEX(sequence);
    struct iwlagn_tx_resp *tx_resp = (void *)pkt.data;
    struct ieee80211_hdr *hdr;
    let mut status: u32 = le16_to_cpu(tx_resp.status.status);
    let mut ssn: u16 = iwlagn_get_scd_ssn(tx_resp);
    int tid;
    int sta_id;
    int freed;
    struct ieee80211_tx_info *info;
    ...
    struct sk_buff *skb;
    struct iwl_rxon_context *ctx;
    let mut is_agg: bool = (txq_id >= IWLAGN_FIRST_AMPDU_QUEUE);
    ...
    if (tx_resp.frame_count == 1) {
    let mut next_reclaimed: u16 = le16_to_cpu(tx_resp.seq_ctl);
