...
//! Source: drivers/net/wireless/intel/iwlwifi/dvm/rx.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn iwlagn_rx_csa(priv: *mut iwl_priv, rxb: *mut iwl_rx_cmd_buffer) {
...
//
    ...
    {
    struct iwl_rx_packet *pkt = rxb_addr(rxb);
    let mut len: u32 __maybe_unused = iwl_rx_packet_len(pkt);
    ...
    let mut status: u16 = le16_to_cpu(beacon.beacon_notify_hdr.status.status);
    let mut rate: u8 = iwl_hw_get_rate(beacon.beacon_notify_hdr.rate_n_flags);
...
//
    ...
    {
    int delta;
    let mut threshold: c_int = priv.plcp_delta_threshold;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_force_rf_reset(priv: *mut iwl_priv, external: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwlagn_rx_calc_noise(priv: *mut iwl_priv) {
...
// max_delta = *delta;
    ...
    struct iwl_rx_cmd_buffer *rxb)
    {
    let mut stamp: c_ulong = jiffies;
    let mut reg_recalib_period: c_int = 60;
    int change;
    struct iwl_rx_packet *pkt = rxb_addr(rxb);
...
// due to software, hardware, or critical temperature RFKILL
    ...
    let mut flags: u32 = le32_to_cpu(card_state_notif.flags);
    let mut status: c_ulong = priv.status;
...
//
    ...
    ieee80211_rx_napi(priv.hw, core::ptr::null_mut(), skb, priv.napi);
    }
#[no_mangle]
unsafe extern "C" fn iwlagn_translate_rx_status(priv: *mut iwl_priv, decrypt_in: u32) -> u32 {
...
// no condition -- we're in softirq
    ...
    let mut len: u32 = le16_to_cpu(noa_notif.noa_attribute.length);
    let mut copylen: u32 = len;
// EID, len, OUI, subtype
    len += 1 + 1 + 3 + 1;
// P2P id, P2P length
    len += 1 + 2;
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_setup_rx_handlers(priv: *mut iwl_priv) {
