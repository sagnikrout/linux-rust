...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/ftm-initiator.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_loc_entry {
    pub list: list_head,
    pub addr: [u8; ETH_ALEN],
    pub civic_len: u8 lci_len,,
    pub buf: [u8; ],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_smooth_entry {
    pub list: list_head,
    pub addr: [u8; ETH_ALEN],
    pub rtt_avg: i64,
    pub host_time: u64,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_ftm_pasn_entry {
    pub list: list_head,
    pub addr: [u8; ETH_ALEN],
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_ftm_iter_data {
    pub cipher: *mut u8,
    pub bssid: *mut u8,
    pub tk: *mut u8,
}
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_ftm_reset(mvm: *mut iwl_mvm) {
    static void iwl_mvm_ftm_reset(struct iwl_mvm *mvm)
    {
    struct iwl_mvm_loc_entry *e, *t;
    mvm.ftm_initiator.req = core::ptr::null_mut();
    mvm.ftm_initiator.req_wdev = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_ftm_restart(mvm: *mut iwl_mvm) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_ftm_initiator_smooth_config(mvm: *mut iwl_mvm) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_ftm_initiator_smooth_stop(mvm: *mut iwl_mvm) {
...
// Don't report AP's TSF
    ...
    u8 *ctrl_ch_position)
    {
    let mut freq: u32 = peer.chandef.chan.center_freq;
...
#[no_mangle]
pub unsafe extern "C" fn if(IWL_TOF_ALGO_TYPE_FFT: IWL_MVM_FTM_INITIATOR_ALGO ==) -> else {
    else if (IWL_MVM_FTM_INITIATOR_ALGO == IWL_TOF_ALGO_TYPE_FFT)
    FTM_SET_FLAG(ALGO_FFT);
    if (peer.ftm.trigger_based)
    FTM_SET_FLAG(TB);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: peer->ftm.non_trigger_based) -> else {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_ftm_send_cmd(mvm: *mut iwl_mvm, hcmd: *mut iwl_host_cmd) -> c_int {
    static int iwl_mvm_ftm_send_cmd(struct iwl_mvm *mvm, struct iwl_host_cmd *hcmd)
    {
    u32 status;
    let mut err: c_int = iwl_mvm_send_cmd_status(mvm, hcmd, &status);
    if (!err && status) {
...
// cipher = entry->cipher;
    ...
    {
    let mut err: c_int = iwl_mvm_ftm_put_target(mvm, vif, peer, (void *)target);
    if (err)
    return err;
...
// Only 2 STS are supported on Tx
    ...
    {
    u32 flags;
    let mut ret: c_int = iwl_mvm_ftm_put_target_v7(mvm, vif, peer, (void *)target);
    if (ret)
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_ftm_abort(mvm: *mut iwl_mvm, req: *mut cfg80211_pmsr_request) {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_ftm_get_host_time(mvm: *mut iwl_mvm, fw_gp2_ts: __le32) -> u64 {
    static u64 iwl_mvm_ftm_get_host_time(struct iwl_mvm *mvm, __le32 fw_gp2_ts)
    {
    let mut gp2_ts: u32 = le32_to_cpu(fw_gp2_ts);
    u32 curr_gp2, diff;
    u64 now_from_boot_ns;
    iwl_mvm_get_sync_time(mvm, CLOCK_BOOTTIME, &curr_gp2,
    &now_from_boot_ns, core::ptr::null_mut());
    if (curr_gp2 >= gp2_ts)
...
// we found the entry we needed
    ...
    struct cfg80211_pmsr_result *res)
    {
    struct iwl_mvm_smooth_entry *resp = core::ptr::null_mut(), *iter;
...
// smooth the results
    ...
    struct cfg80211_pmsr_result *res)
    {
    let mut rtt_avg: i64 = div_s64(res.ftm.rtt_avg * 100, 6666);
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_ftm_get_range_resp_ver(mvm: *mut iwl_mvm) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_ftm_resp_size_validation(ver: u8, pkt_len: c_uint) -> bool {
    static bool iwl_mvm_ftm_resp_size_validation(u8 ver, unsigned int pkt_len)
    {
    switch (ver) {
    case 9:
    case 8:
    let mut pkt_len: return = = sizeof(struct iwl_tof_range_rsp_ntfy);
    case 7:
    let mut pkt_len: return = = sizeof(struct iwl_tof_range_rsp_ntfy_v7);
    case 6:
    let mut pkt_len: return = = sizeof(struct iwl_tof_range_rsp_ntfy_v6);
    case 5:
    let mut pkt_len: return = = sizeof(struct iwl_tof_range_rsp_ntfy_v5);
    default:
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_ftm_range_resp(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
    ...
    let mut notif_ver: u8 = iwl_mvm_ftm_get_range_resp_ver(mvm);
    lockdep_assert_held(&mvm.mutex);
    if (!mvm.ftm_initiator.req) {
    return;
    }
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_ftm_lc_notif(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
    void iwl_mvm_ftm_lc_notif(struct iwl_mvm *mvm, struct iwl_rx_cmd_buffer *rxb)
    {
    struct iwl_rx_packet *pkt = rxb_addr(rxb);
    const struct ieee80211_mgmt *mgmt = (void *)pkt.data;
    let mut len: usize = iwl_rx_packet_payload_len(pkt);
    struct iwl_mvm_loc_entry *entry;
    const u8 *ies, *lci, *civic, *msr_ie;
    size_t ies_len, lci_len = 0, civic_len = 0;
    let mut baselen: usize = IEEE80211_MIN_ACTION_SIZE(ftm);
    let mut rprt_type_lci: static u8 = IEEE80211_SPCT_MSR_RPRT_TYPE_LCI;
    let mut rprt_type_civic: static u8 = IEEE80211_SPCT_MSR_RPRT_TYPE_CIVIC;
    if (len <= baselen)
    return;
    lockdep_assert_held(&mvm.mutex);
