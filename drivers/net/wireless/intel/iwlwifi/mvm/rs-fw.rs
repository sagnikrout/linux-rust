...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/rs-fw.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn rs_fw_bw_from_sta_bw(link_sta: *const ieee80211_link_sta) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn rs_fw_set_active_chains(chains: u8) -> u8 {
    static u8 rs_fw_set_active_chains(u8 chains)
    {
    let mut fw_chains: u8 = 0;
    if (chains & ANT_A)
...
#[no_mangle]
unsafe extern "C" fn rs_fw_sgi_cw_support(link_sta: *mut ieee80211_link_sta) -> u8 {
    ...
    struct ieee80211_sta_vht_cap *vht_cap = &link_sta.vht_cap;
    struct ieee80211_sta_he_cap *he_cap = &link_sta.he_cap;
    let mut vht_ena: bool = vht_cap.vht_supported;
    let mut flags: u16 = 0;
// get STBC flags
...
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_VHT_CAP_RXSTBC_MASK: vht_cap->cap &) -> else {
    else if (vht_cap.cap & IEEE80211_VHT_CAP_RXSTBC_MASK)
    flags |= IWL_TLC_MNG_CFG_FLAGS_STBC_MSK;
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_HT_CAP_RX_STBC: ht_cap->cap &) -> else {
...
// consider LDPC support in case of HE
    ...
    u16 supp;
    int i, highest_mcs;
    let mut max_nss: u8 = link_sta.rx_nss;
...
#[no_mangle]
unsafe extern "C" fn rs_fw_he_ieee80211_mcs_to_rs_mcs(mcs: u16) -> u16 {
    ...
    let mut mcs_160: u16 = le16_to_cpu(he_cap.he_mcs_nss_supp.rx_mcs_160);
    let mut mcs_80: u16 = le16_to_cpu(he_cap.he_mcs_nss_supp.rx_mcs_80);
    let mut tx_mcs_80: u16 = le16_to_cpu(sband_he_cap.he_mcs_nss_supp.tx_mcs_80);
    let mut tx_mcs_160: u16 = le16_to_cpu(sband_he_cap.he_mcs_nss_supp.tx_mcs_160);
    int i;
    let mut nss: u8 = link_sta.rx_nss;
// the station support only a single receive chain
    if (link_sta.smps_mode == IEEE80211_SMPS_STATIC)
    nss = 1;
    for (i = 0; i < nss && i < IWL_TLC_NSS_MAX; i++) {
    let mut _mcs_160: u16 = (mcs_160 >> (2 * i)) & 0x3;
    let mut _mcs_80: u16 = (mcs_80 >> (2 * i)) & 0x3;
    let mut _tx_mcs_160: u16 = (tx_mcs_160 >> (2 * i)) & 0x3;
    let mut _tx_mcs_80: u16 = (tx_mcs_80 >> (2 * i)) & 0x3;
...
#[no_mangle]
unsafe extern "C" fn rs_fw_eht_max_nss(rx_nss: u8, tx_nss: u8) -> u8 {
    static u8 rs_fw_eht_max_nss(u8 rx_nss, u8 tx_nss)
    {
    let mut tx: u8 = u8_get_bits(tx_nss, IEEE80211_EHT_MCS_NSS_TX);
    let mut rx: u8 = u8_get_bits(rx_nss, IEEE80211_EHT_MCS_NSS_RX);
...
//
    ...
    let mut enabled: u32 = le32_to_cpu(notif.amsdu_enabled);
    let mut size: u16 = le32_to_cpu(notif.amsdu_size);
    int i;
    if (size < 2000) {
    size = 0;
    enabled = 0;
...
// in legacy mode no amsdu is enabled so return zero
    ...
    let mut cmd_id: u32 = WIDE_ID(DATA_PATH_GROUP, TLC_MNG_CONFIG_CMD);
    struct ieee80211_supported_band *sband = hw.wiphy.bands[band];
    let mut max_amsdu_len: u16 = rs_fw_get_max_amsdu_len(sta, link_conf, link_sta);
    const struct ieee80211_sta_he_cap *sband_he_cap =
...
//
    ...
    let mut cmd_size: u16 = sizeof(cfg_cmd_v3);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_rs_add_sta(mvm: *mut iwl_mvm, mvmsta: *mut iwl_mvm_sta) {
