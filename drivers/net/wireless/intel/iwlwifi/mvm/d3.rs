...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/d3.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_convert_p1k(p1k: *mut u16, out: *mut __le16) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wowlan_key_reprogram_data {
    pub error: bool,
    pub wep_key_idx: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wowlan_key_rsc_tsc_data {
    pub rsc_tsc: *mut iwl_wowlan_rsc_tsc_params_cmd_ver_2,
    pub have_rsc_tsc: bool,
}
    ...
    struct tkip_sc *tkip_sc, *tkip_tx_sc = core::ptr::null_mut();
    struct ieee80211_key_seq seq;
    int i;
    switch (key.cipher) {
    default:
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wowlan_key_rsc_v5_data {
    pub rsc: *mut iwl_wowlan_rsc_tsc_params_cmd,
    pub have_rsc: bool,
    pub gtks: c_int,
    pub gtk_ids: [c_int; 4],
}
...
// only for ciphers that can be PTK/GTK
    ...
    if (data.gtks >= 2) {
    let mut prev: c_int = data.gtks - 2;
    let mut prev_idx: c_int = data.gtk_ids[prev];
...
//
    ...
    int ret;
    if (ver == 5) {
    let mut data: wowlan_key_rsc_v5_data = {};
    int i;
    ...
    let mut data: wowlan_key_rsc_tsc_data = {};
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wowlan_key_tkip_data {
    pub tkip: iwl_wowlan_tkip_params_cmd,
    pub have_tkip_keys: bool,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wowlan_key_gtk_type_iter {
    pub kek_kck_cmd: *mut iwl_wowlan_kek_kck_material_cmd_v4,
}
    ...
    __le32 *cipher = core::ptr::null_mut();
...
// cipher = cpu_to_le32(STA_KEY_FLG_CCM);
    ...
    let mut mask_len: c_int = DIV_ROUND_UP(wowlan.patterns[i].pattern_len, 8);
    ...
    int ret, i;
    let mut binding_cmd: iwl_binding_cmd_v1 = {};
    let mut quota_cmd: iwl_time_quota_cmd = {};
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_set_last_nonqos_seq(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_switch_to_d3(mvm: *mut iwl_mvm) -> c_int {
...
// Query the last used seqno and set it
    let mut ret: c_int = iwl_mvm_get_last_nonqos_seq(mvm, vif);
    if (ret < 0)
    return ret;
...
//
    ...
    let mut key_data: wowlan_key_reprogram_data = {};
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    int ret;
    u8 cmd_ver;
    size_t cmd_size;
...
//
    ...
    IWL_FW_CMD_VER_UNKNOWN);
    let mut tkip_data: wowlan_key_tkip_data = {};
    int size;
    if (ver == 2) {
...
// configure rekey data only if offloaded rekey is supported (d3)
    ...
    let mut kek_kck_cmd: iwl_wowlan_kek_kck_material_cmd_v4 = {};
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_free_nd(mvm: *mut iwl_mvm) {
    static void iwl_mvm_free_nd(struct iwl_mvm *mvm)
    {
    kfree(mvm.nd_match_sets);
    mvm.nd_match_sets = core::ptr::null_mut();
    mvm.n_nd_match_sets = 0;
    kfree(mvm.nd_channels);
    mvm.nd_channels = core::ptr::null_mut();
    mvm.n_nd_channels = 0;
    }
    ...
    struct iwl_mvm_vif *mvmvif = core::ptr::null_mut();
    struct ieee80211_sta *ap_sta = core::ptr::null_mut();
...
//
    ...
    iwl_fw_dbg_stop_restart_recording(&mvm.fwrt, core::ptr::null_mut(), true);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_suspend(hw: *mut ieee80211_hw, wowlan: *mut cfg80211_wowlan) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_multicast_key_data {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_wowlan_status_data {
    pub replay_ctr: u64,
    pub num_of_gtk_rekeys: u32,
    pub received_beacons: u32,
    pub wakeup_reasons: u32,
    pub wake_packet_length: u32,
    pub wake_packet_bufsize: u32,
    pub pattern_number: u16,
    pub non_qos_seq_ctr: u16,
    pub qos_seq_ctr: [u16; 8],
...
//
    struct {
    pub seq: [ieee80211_key_seq; IWL_MAX_TID_COUNT],
    pub aes: } tkip,,
//
...
// Same as above
    ...
    pub aes: } tkip,,
    pub ptk: },
    pub igtk: iwl_multicast_key_data,
    pub bigtk: [iwl_multicast_key_data; WOWLAN_BIGTK_KEYS_NUM],
    pub wake_packet: *mut u8,
}
    ...
    {
    struct sk_buff *pkt = core::ptr::null_mut();
    struct cfg80211_wowlan_wakeup wakeup = {
    .pattern_idx = -1,
    };
    struct cfg80211_wowlan_wakeup *wakeup_report = &wakeup;
    let mut reasons: u32 = status.wakeup_reasons;
    if (reasons == IWL_WOWLAN_WAKEUP_BY_NON_WIRELESS) {
    wakeup_report = core::ptr::null_mut();
    goto report;
    }
    ...
    let mut pktsize: c_int = status.wake_packet_bufsize;
    let mut pktlen: c_int = status.wake_packet_length;
    const u8 *pktdata = status.wake_packet;
    const struct ieee80211_hdr *hdr = (const void *)pktdata;
    let mut truncated: c_int = pktlen - pktsize;
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_le64_to_aes_seq(le_pn: __le64, seq: *mut ieee80211_key_seq) {
    static void iwl_mvm_le64_to_aes_seq(__le64 le_pn, struct ieee80211_key_seq *seq)
    {
    let mut pn: u64 = le64_to_cpu(le_pn);
    seq.ccmp.pn[0] = pn >> 40;
    seq.ccmp.pn[1] = pn >> 32;
    seq.ccmp.pn[2] = pn >> 24;
    seq.ccmp.pn[3] = pn >> 16;
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_le64_to_tkip_seq(le_pn: __le64, seq: *mut ieee80211_key_seq) {
...
// GTK RX counters
    ...
    let mut entry: u8 = sc.mcast_key_id_map[i];
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_d3_gtk_iter_data {
    pub mvm: *mut iwl_mvm,
    pub status: *mut iwl_wowlan_status_data,
}
...
//
    ...
    if (key.keyidx == 6 || key.keyidx == 7) {
    let mut idx: u8 = key.keyidx == status.bigtk[1].id;
    iwl_mvm_d3_update_igtk_bigtk(status, key,
    &status.bigtk[idx]);
    }
    }
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_lookup_wowlan_status_ver(mvm: *mut iwl_mvm) -> c_int {
...
//
    ...
    let mut replay_ctr: __be64 = cpu_to_be64(status.replay_ctr);
...
// copy all the common fields */				\
    ...
    status.wake_packet = core::ptr::null_mut();				\
    }								\
    \
    return status;							\
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_nd_results {
    pub matched_profiles: u32,
    pub matches: [u8; ND_QUERY_BUF_LEN],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_d3_data {
    pub status: *mut iwl_wowlan_status_data,
    pub d3_end_flags: u32,
    pub /: *mut *mut u32 notif_expected; / bitmap - see &enum iwl_d3_notif,
    pub /: *mut *mut u32 notif_received; / bitmap - see &enum iwl_d3_notif,
    pub nd_results: *mut iwl_mvm_nd_results,
    pub nd_results_valid: bool,
}
    ...
    {
    struct cfg80211_wowlan_nd_info *net_detect = core::ptr::null_mut();
    struct cfg80211_wowlan_wakeup wakeup = {
    .pattern_idx = -1,
    };
    struct cfg80211_wowlan_wakeup *wakeup_report = &wakeup;
...
// if FW uses status notification, status shouldn't be NULL here
    ...
    if (WARN_ON(!status)) {
    IWL_ERR(mvm, "Got wake packet notification but wowlan status data is core::ptr::null_mut()\n");
    return -EIO;
    }
...
// no scan should be active at this point
    ...
    let mut len: u32 = iwl_rx_packet_payload_len(pkt);
    int ret;
    int wowlan_info_ver = iwl_fw_lookup_notif_ver(mvm.fw,
    PROT_OFFLOAD_GROUP,
    WOWLAN_INFO_NOTIFICATION,
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_resume_firmware(mvm: *mut iwl_mvm) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_d3_resume_notif_based(mvm: *mut iwl_mvm) -> bool {
...
#[no_mangle]
unsafe extern "C" fn __iwl_mvm_resume(mvm: *mut iwl_mvm) -> c_int {
    static int __iwl_mvm_resume(struct iwl_mvm *mvm)
    {
    struct ieee80211_vif *vif = core::ptr::null_mut();
    let mut ret: c_int = 1;
    let mut results: iwl_mvm_nd_results = {};
    struct iwl_d3_data d3_data = {
    .notif_expected =
    ...
    let mut resume_notif_based: bool = iwl_mvm_d3_resume_notif_based(mvm);
...
// nothing else to do if we already sent D0I3_END_CMD
    ...
    if (!iwl_fw_lookup_notif_ver(mvm.fw, PROT_OFFLOAD_GROUP,
    D3_END_NOTIFICATION, 0)) {
    ret = iwl_mvm_send_cmd_pdu(mvm, D0I3_END_CMD, 0, 0, core::ptr::null_mut());
    if (!ret)
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_resume(hw: *mut ieee80211_hw) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_set_wakeup(hw: *mut ieee80211_hw, enabled: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_fast_suspend(mvm: *mut iwl_mvm) {
    void iwl_mvm_fast_suspend(struct iwl_mvm *mvm)
    {
    let mut d3_cfg_cmd_data: iwl_d3_manager_config = {};
    int ret;
    lockdep_assert_held(&mvm.mutex);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_fast_resume(mvm: *mut iwl_mvm) -> c_int {
    ...
    mvm.last_reset_or_resume_time_jiffies = jiffies;
    iwl_fw_dbg_read_d3_debug_data(&mvm.fwrt);
    rt_status = iwl_mvm_check_rt_status(mvm, core::ptr::null_mut());
    if (rt_status != FW_ALIVE) {
    iwl_trans_notify_fw_error(mvm.trans);
    if (rt_status == FW_ERROR) {
    IWL_ERR(mvm,
