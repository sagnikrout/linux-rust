...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/mld-key.c
#![no_std]
...
// pass link_id to filter by it if not -1 (GTK on client)
    ...
    {
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    let mut pairwise: bool = keyconf.flags & IEEE80211_KEY_FLAG_PAIRWISE;
    let mut igtk: bool = keyconf.keyidx == 4 || keyconf.keyidx == 5;
    let mut flags: u32 = 0;
    lockdep_assert_held(&mvm.mutex);
    if (!pairwise)
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_sta_key_update_data {
    pub sta: *mut ieee80211_sta,
    pub old_sta_mask: u32,
    ...
    {
    let mut cmd_id: u32 = WIDE_ID(DATA_PATH_GROUP, SEC_KEY_CMD);
    ...
    let mut max_key_len: c_int = sizeof(cmd.u.add.key);
    int ret;
...
//
    ...
    struct ieee80211_key_conf *keyconf)
    {
    let mut sta_mask: u32 = iwl_mvm_get_sec_sta_mask(mvm, vif, sta, keyconf);
    let mut key_flags: u32 = iwl_mvm_get_sec_flags(mvm, vif, sta, keyconf);
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    struct iwl_mvm_vif_link_info *mvm_link = core::ptr::null_mut();
    int ret;
    if (keyconf.keyidx == 4 || keyconf.keyidx == 5) {
    let mut link_id: c_uint = 0;
...
// no longer in HW - mark for later
    ...
    mvm_link.igtk = core::ptr::null_mut();
    }
    }
...
// For WEP, delete the key again as unicast
    ...
    struct ieee80211_key_conf *keyconf)
    {
    u32 key_flags = iwl_mvm_get_sec_flags(mvm, vif, core::ptr::null_mut(), keyconf) |
    ...
    let mut link_id: c_uint = (uintptr_t)data;
    ...
    return;
    _iwl_mvm_sec_key_del(mvm, vif, core::ptr::null_mut(), key, CMD_ASYNC);
