...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/mac80211.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_reset_phy_ctxts(mvm: *mut iwl_mvm) {
    ...
    bool *changed)
    {
    struct ieee80211_regdomain *regd = core::ptr::null_mut();
    ...
    resp = core::ptr::null_mut();
    goto out;
    }
    if (changed) {
    let mut status: u32 = le32_to_cpu(resp.status);
// changed = (status == MCC_RESP_NEW_CHAN_PROFILE ||
    ...
    le32_to_cpu(resp.cap), resp_ver, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_update_changed_regdom(mvm: *mut iwl_mvm) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_init_fw_regd(mvm: *mut iwl_mvm, force_regd_sync: bool) -> c_int {
...
// Notify the firmware we support wifi location updates
    regd = iwl_mvm_get_current_regdomain(mvm, core::ptr::null_mut());
    if (!IS_ERR_OR_NULL(regd))
    kfree(regd);
    }
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_setup_register(mvm: *mut iwl_mvm) -> c_int {
    ...
    let mut sec_key_id: u32 = WIDE_ID(DATA_PATH_GROUP, SEC_KEY_CMD);
    let mut sec_key_ver: u8 = iwl_fw_lookup_cmd_ver(mvm.fw, sec_key_id, 0);
...
//
    ...
    let mut scan_ver: u8 = iwl_fw_lookup_cmd_ver(mvm.fw, SCAN_REQ_UMAC, 0);
    wiphy_ext_feature_set(hw.wiphy,
...
// Old firmware also supports probe deferral and suppression
    ...
    hw.wiphy.iftype_ext_capab = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_itxq_xmit(hw: *mut ieee80211_hw, txq: *mut ieee80211_txq) {
...
// recheck under lock
    ...
    u16 *ssn = &params.ssn;
    let mut buf_size: u16 = params.buf_size;
    let mut amsdu: bool = params.amsdu;
    let mut timeout: u16 = params.timeout;
    ...
    let mut macid: u16 = iwl_mvm_vif_from_mac80211(vif).id;
    ...
    mvmvif.ap_sta = core::ptr::null_mut();
    vif.driver_flags &= ~IEEE80211_VIF_EML_ACTIVE;
    for_each_mvm_vif_valid_link(mvmvif, link_id) {
    mvmvif.link[link_id].ap_sta_id = IWL_INVALID_STA;
    mvmvif.link[link_id].fw_link_id = IWL_MVM_FW_LINK_ID_INVALID;
    mvmvif.link[link_id].phy_ctxt = core::ptr::null_mut();
    mvmvif.link[link_id].active = 0;
    mvmvif.link[link_id].igtk = core::ptr::null_mut();
    ...
    RCU_INIT_POINTER(mvmvif.deflink.probe_resp_data, core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_restart_cleanup(mvm: *mut iwl_mvm) {
...
//
    ...
    mvm.p2p_device_vif = core::ptr::null_mut();
    iwl_mvm_reset_phy_ctxts(mvm);
...
#[no_mangle]
pub unsafe extern "C" fn __iwl_mvm_mac_start(mvm: *mut iwl_mvm) -> c_int {
    int __iwl_mvm_mac_start(struct iwl_mvm *mvm)
    {
    let mut fast_resume: bool = false;
    int ret;
    lockdep_assert_held(&mvm.mutex);
...
// We got the NIC, we can now free the MEI NVM data
    ...
    mvm.mei_nvm_data = core::ptr::null_mut();
//
...
//
    ...
    mvm.nvm_data = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_start(hw: *mut ieee80211_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_restart_complete(mvm: *mut iwl_mvm) {
    static void iwl_mvm_restart_complete(struct iwl_mvm *mvm)
    {
    int ret;
    guard(mvm)(mvm);
    clear_bit(IWL_MVM_STATUS_IN_HW_RESTART, &mvm.status);
    ret = iwl_mvm_update_quotas(mvm, true, core::ptr::null_mut());
    if (ret)
...
#[no_mangle]
pub unsafe extern "C" fn __iwl_mvm_mac_stop(mvm: *mut iwl_mvm, suspend: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_stop(hw: *mut ieee80211_hw, suspend: bool) {
...
//
    ...
    s16 tx_power)
    {
    let mut cmd_id: u32 = REDUCE_TX_POWER_CMD;
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(link_conf.vif);
    let mut mac_id: u32 = mvmvif.id;
    int len;
    ...
    let mut cmd_ver: u8 = iwl_fw_lookup_cmd_ver(mvm.fw, cmd_id, 3);
...
#[no_mangle]
pub unsafe extern "C" fn if(9: cmd_ver ==) -> else {
    else if (cmd_ver == 9)
    len = sizeof(cmd_v9_v10.v9);
#[no_mangle]
pub unsafe extern "C" fn if(8: cmd_ver ==) -> else {
    else if (cmd_ver == 8)
    len = sizeof(cmd.v8);
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_post_csa_tx(data: *mut c_void, sta: *mut ieee80211_sta) {
    ...
    let mut link_id: c_uint = link_conf.link_id;
    let mut ap_sta_id: u8 = mvmvif.link[link_id].ap_sta_id;
    ...
    iwl_mvm_mac_ctxt_changed(mvm, vif, false, core::ptr::null_mut());
    if (!fw_has_capa(&mvm.fw.ucode_capa,
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_channel_switch_disconnect_wk(wk: *mut work_struct) {
...
// Trigger disconnect (should clear the CSA state)
    ...
    bw = 320;
#[no_mangle]
pub unsafe extern "C" fn if(NL80211_CHAN_WIDTH_160: chandef->width ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_init_mvmvif(mvm: *mut iwl_mvm, mvmvif: *mut iwl_mvm_vif) {
...
// beacon filtering
    ...
    goto out_unlock;
    out_remove_mac:
    mvmvif.deflink.phy_ctxt = core::ptr::null_mut();
...
//
    ...
    mutex_lock(&mvm.mutex);
    if (vif == mvm.csme_vif) {
    iwl_mei_set_netdev(core::ptr::null_mut());
    mvm.csme_vif = core::ptr::null_mut();
    }
    probe_data = rcu_dereference_protected(mvmvif.deflink.probe_resp_data,
    lockdep_is_held(&mvm.mutex));
    RCU_INIT_POINTER(mvmvif.deflink.probe_resp_data, core::ptr::null_mut());
    ...
    mvm.bf_allowed_vif = core::ptr::null_mut();
    vif.driver_flags &= ~(IEEE80211_VIF_BEACON_FILTER |
    IEEE80211_VIF_SUPPORTS_CQM_RSSI);
    }
...
//
    ...
    iwl_mvm_mac_ctxt_remove(mvm, vif);
    RCU_INIT_POINTER(mvm.vif_id_to_mac[mvmvif.id], core::ptr::null_mut());
    if (vif.type == NL80211_IFTYPE_MONITOR)
    mvm.monitor_on = false;
    out:
    if (vif.type == NL80211_IFTYPE_AP ||
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_mc_iter_data {
    pub mvm: *mut iwl_mvm,
    pub port_id: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_recalc_multicast(mvm: *mut iwl_mvm) {
...
//
    ret = iwl_mvm_send_cmd_pdu(mvm, ECHO_CMD, 0, 0, core::ptr::null_mut());
    if (ret)
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_update_mu_groups(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> c_int {
    int iwl_mvm_update_mu_groups(struct iwl_mvm *mvm, struct ieee80211_vif *vif)
    {
    let mut cmd: iwl_mu_group_mgmt_cmd = {};
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_he_get_ppe_val(ppe: *mut u8, ppe_pos_bit: u8) -> u8 {
    static u8 iwl_mvm_he_get_ppe_val(u8 *ppe, u8 ppe_pos_bit)
    {
    let mut byte_num: u8 = ppe_pos_bit / 8;
    let mut bit_num: u8 = ppe_pos_bit % 8;
    u8 residue_bits;
    u8 res;
    if (bit_num <= 5)
...
//
    ...
    for (i = 0; i < nss; i++) {
    let mut ru_index_tmp: u8 = ru_index_bitmap << 1;
    let mut low_th: u8 = IWL_HE_PKT_EXT_NONE, high_th = IWL_HE_PKT_EXT_NONE;
    u8 bw;
    for (bw = 0;
...
// Starting after PPE header
    let mut ppe_pos_bit: u8 = IEEE80211_HE_PPE_THRES_INFO_HEADER_SIZE;
    ...
    u8 nominal_padding)
    {
    let mut low_th: c_int = -1;
    let mut high_th: c_int = -1;
    int i;
...
// Starting after PPE header
    let mut ppe_pos_bit: u8 = IEEE80211_EHT_PPE_THRES_INFO_HEADER_SIZE;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_is_nic_ack_enabled(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> bool {
    bool iwl_mvm_is_nic_ack_enabled(struct iwl_mvm *mvm, struct ieee80211_vif *vif)
    {
    const struct ieee80211_supported_band *sband;
    const struct ieee80211_sta_he_cap *own_he_cap = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(3: link_adap ==) -> else {
    else if (link_adap == 3)
    ...
    let mut sta_ctxt_cmd_v2: iwl_he_sta_context_cmd_v2 = {};
    let mut cmd_id: u32 = WIDE_ID(DATA_PATH_GROUP, STA_HE_CTXT_CMD);
    let mut ver: u8 = iwl_fw_lookup_cmd_ver(mvm.fw, cmd_id, 2);
    int size;
    struct ieee80211_sta *sta;
    u32 flags;
    int i;
...
// fields after pkt_ext
    ...
    {
    let mut duration: u32 = IWL_MVM_TE_SESSION_PROTECTION_MAX_TIME_MS;
    let mut min_duration: u32 = IWL_MVM_TE_SESSION_PROTECTION_MIN_TIME_MS;
...
// only anyway.
//
    let mut dur: u32 = (11 * vif.bss_conf.beacon_int) / 10;
...
// remove quota for this interface
    ret = iwl_mvm_update_quotas(mvm, false, core::ptr::null_mut());
    if (ret)
    IWL_ERR(mvm, "failed to update quotas\n");
// this will take the cleared BSSID from bss_conf
    ret = iwl_mvm_mac_ctxt_changed(mvm, vif, false, core::ptr::null_mut());
    if (ret)
    IWL_ERR(mvm,
...
// send all the early keys to the device now
    ...
    if (!key)
    continue;
    mvmvif.ap_early_keys[i] = core::ptr::null_mut();
...
// Need to update the P2P Device MAC (only GO, IBSS is single vif)
    ...
    iwl_mvm_mac_ctxt_changed(mvm, mvm.p2p_device_vif, false, core::ptr::null_mut());
    iwl_mvm_bt_coex_vif_change(mvm);
...
// Handle AP stop while in CSA
    ...
    RCU_INIT_POINTER(mvm.csa_vif, core::ptr::null_mut());
    mvmvif.csa_countdown = false;
    }
    if (rcu_access_pointer(mvm.csa_tx_blocked_vif) == vif) {
    RCU_INIT_POINTER(mvm.csa_tx_blocked_vif, core::ptr::null_mut());
...
// Need to update the P2P Device MAC (only GO, IBSS is single vif)
    if (vif.p2p && mvm.p2p_device_vif)
    iwl_mvm_mac_ctxt_changed(mvm, mvm.p2p_device_vif, false, core::ptr::null_mut());
    iwl_mvm_update_quotas(mvm, false, core::ptr::null_mut());
...
// Need to send a new beacon template to the FW
    ...
    let mut ret: c_int = iwl_mvm_ftm_start_responder(mvm, vif, &vif.bss_conf);
    if (ret)
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_sta_pm_notif(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
    ...
    struct ieee80211_sta *sta;
    struct iwl_mvm_sta *mvmsta;
    let mut sleeping: bool = (notif.type != IWL_MVM_PM_EVENT_AWAKE);
...
//
    ...
    ERR_PTR(-ENOENT));
    RCU_INIT_POINTER(mvm.fw_id_to_link_sta[sta_id], core::ptr::null_mut());
    }
    }
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_he_obss_narrow_bw_ru_data {
    pub tolerated: bool,
}
...
// TODO: add support for collocated AP data
    iwl_mei_host_associated(&conn_info, core::ptr::null_mut());
    ...
    {
    return iwl_mvm_mac_ctxt_changed(mvm, vif, force_assoc_off, core::ptr::null_mut());
    }
...
//
    ...
    if (sta.tdls &&
    (vif.p2p ||
    iwl_mvm_tdls_sta_count(mvm, core::ptr::null_mut()) == IWL_TDLS_STA_COUNT ||
...
//
    if (!sta.mfp) {
    let mut ret: c_int = callbacks.update_sta(mvm, vif, sta);
    if (ret)
    return ret;
    }
    iwl_mvm_rs_rate_init_all_links(mvm, vif, sta);
...
//
    if (vif.type == NL80211_IFTYPE_P2P_DEVICE) {
    guard(mvm)(mvm);
    return iwl_mvm_mac_ctxt_changed(mvm, vif, false, core::ptr::null_mut());
    }
    return 0;
    }
...
//
    ...
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    struct iwl_mvm *mvm = IWL_MAC80211_GET_MVM(hw);
    struct iwl_mvm_sta *mvmsta = core::ptr::null_mut();
    struct iwl_mvm_key_pn *ptk_pn = core::ptr::null_mut();
    let mut keyidx: c_int = key.keyidx;
    let mut sec_key_id: u32 = WIDE_ID(DATA_PATH_GROUP, SEC_KEY_CMD);
    let mut sec_key_ver: u8 = iwl_fw_lookup_cmd_ver(mvm.fw, sec_key_id, 0);
    int ret, i;
    u8 key_offset;
...
// in HW restart reuse the index, otherwise request a new one
    ...
    sta ? sta.addr : core::ptr::null_mut(), key.keyidx);
    if (sec_key_ver)
    ...
    key.hw_key_idx = STA_KEY_IDX_INVALID;
    if (ptk_pn) {
    RCU_INIT_POINTER(mvmsta.ptk_pn[keyidx], core::ptr::null_mut());
    kfree(ptk_pn);
    }
//
...
// When resuming from wowlan, FW already knows about the newest keys
    ...
    let mut resp_len: c_int = iwl_rx_packet_payload_len(pkt);
    ...
    let mut len: u16 = sizeof(aux_roc_req) - iwl_mvm_chan_info_padding(mvm);
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_add_aux_sta_for_hs20(mvm: *mut iwl_mvm, lmac_id: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_roc_link(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> c_int {
...
//
    ...
    {
    int ret;
    let mut cmd_id: u32 = WIDE_ID(MAC_CONF_GROUP, ROC_CMD);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_chanctx_usage_data {
    pub mvm: *mut iwl_mvm,
...
// Handle binding during CSA
    ...
    if (!fw_has_capa(&mvm.fw.ucode_capa,
    IWL_UCODE_TLV_CAPA_CHANNEL_SWITCH_CMD)) {
    let mut duration: u32 = 5 * vif.bss_conf.beacon_int;
...
// Save blocked iface, the timeout is set on the next beacon
    ...
    {
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    struct ieee80211_vif *disabled_vif = core::ptr::null_mut();
    ...
    if (!fw_has_capa(&mvm.fw.ucode_capa,
    IWL_UCODE_TLV_CAPA_CHANNEL_SWITCH_CMD))
    iwl_mvm_mac_ctxt_changed(mvm, vif, true, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_tx_last_beacon(hw: *mut ieee80211_hw) -> c_int {
...
//
    ...
    if (mvmvif.bf_enabled) {
    let mut ret: c_int = iwl_mvm_disable_beacon_filter(mvm, vif);
    if (ret)
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_csa_block_txqs(data: *mut c_void, sta: *mut ieee80211_sta) {
...
pub const IWL_MAX_CSA_BLOCK_TX: c_int = 1500;
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_flush_no_vif(mvm: *mut iwl_mvm, queues: u32, drop: bool) {
...
// we can't ask the firmware anything if it is dead
    ...
    struct iwl_mvm_sta *mvmsta;
    struct ieee80211_sta *sta;
    let mut ap_sta_done: bool = false;
    int i;
    let mut msk: u32 = 0;
    if (!vif) {
...
//
    ...
    let mut ret: c_int = iwl_mvm_request_statistics(mvm, false);
    if (ret)
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_set_sta_rate(rate_n_flags: u32, rinfo: *mut rate_info) {
    static void iwl_mvm_set_sta_rate(u32 rate_n_flags, struct rate_info *rinfo)
    {
    let mut format: u32 = rate_n_flags & RATE_MCS_MOD_TYPE_MSK;
    u32 gi_ltf;
    ...
    let mut rate: c_int = u32_get_bits(rate_n_flags, RATE_LEGACY_RATE_MSK);
...
#[no_mangle]
pub unsafe extern "C" fn if(2: gi_ltf ==) -> else {
    else if (gi_ltf == 2)
    rinfo.he_gi = NL80211_RATE_INFO_HE_GI_1_6;
#[no_mangle]
pub unsafe extern "C" fn if(3: gi_ltf ==) -> else {
    else if (gi_ltf == 3)
...
// firmware only reports a value after RXing a few beacons
    ...
    event.u.mlme.reason);
#[no_mangle]
pub unsafe extern "C" fn if(MLME_TIMEOUT: event->u.mlme.status ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_sync_rx_queues(hw: *mut ieee80211_hw) {
    void iwl_mvm_sync_rx_queues(struct ieee80211_hw *hw)
    {
    struct iwl_mvm *mvm = IWL_MAC80211_GET_MVM(hw);
    guard(mvm)(mvm);
    iwl_mvm_sync_rx_queues_internal(mvm, IWL_MVM_RXQ_EMPTY, true, core::ptr::null_mut(), 0);
    }
    int
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_can_hw_csum(skb: *mut sk_buff) -> bool {
    static bool iwl_mvm_can_hw_csum(struct sk_buff *skb)
    {
    let mut protocol: u8 = ip_hdr(skb).protocol;
    if (!IS_ENABLED(CONFIG_INET))
    return false;
    let mut protocol: return = = IPPROTO_TCP || protocol == IPPROTO_UDP;
    }
