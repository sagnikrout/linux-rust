...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/mac-ctxt.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_mac_iface_iterator_data {
    pub mvm: *mut iwl_mvm,
    pub vif: *mut ieee80211_vif,
    pub available_mac_ids: [c_ulong; BITS_TO_LONGS(NUM_MAC_INDEX_DRIVER)],
    pub available_tsf_ids: [c_ulong; BITS_TO_LONGS(NUM_TSF_IDS)],
    pub preferred_tsf: enum iwl_tsf_id,
    pub found_vif: bool,
}
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !test_bit(mvmvif->tsf_id, _arg: data.available_tsf_ids)) -> else {
    else if (!test_bit(mvmvif.tsf_id, data.available_tsf_ids))
    mvmvif.tsf_id = find_first_bit(data.available_tsf_ids,
    NUM_TSF_IDS);
    }
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_ctxt_init(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> c_int {
...
//
    ...
    let mut basic: c_ulong = vif.bss_conf.basic_rates;
    let mut lowest_present_ofdm: c_int = 100;
    let mut lowest_present_cck: c_int = 100;
    let mut cck: u8 = 0;
    let mut ofdm: u8 = 0;
    int i;
    sband = mvm.hw.wiphy.bands[band];
    for_each_set_bit(i, &basic, BITS_PER_LONG) {
    let mut hw: c_int = sband.bitrates[i].hw_value;
...
// ofdm_rates = ofdm;
    ...
    let mut cck_ack_rates: u8 = 0, ofdm_ack_rates = 0;
    let mut band: enum nl80211_band = NL80211_BAND_2GHZ;
...
// protection_flags |= cpu_to_le32(ht_flag);
    ...
    let mut txf: u8 = iwl_mvm_mac_ac_to_tx_fifo(mvm, i);
    let mut ucode_ac: u8 = iwl_mvm_mac80211_ac_to_ucode_ac(i);
    ac[ucode_ac].cw_min =
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_get_mac_type(vif: *mut ieee80211_vif) -> c_int {
    int iwl_mvm_get_mac_type(struct ieee80211_vif *vif)
    {
    let mut mac_type: u32 = FW_MAC_TYPE_BSS_STA;
    switch (vif.type) {
    case NL80211_IFTYPE_STATION:
    if (vif.p2p)
...
//
    ...
    {
    let mut cmd: iwl_mac_ctx_cmd = {};
    let mut tfd_queue_msk: u32 = 0;
    int ret;
    WARN_ON(vif.type != NL80211_IFTYPE_MONITOR);
    iwl_mvm_mac_ctxt_cmd_common(mvm, vif, &cmd, core::ptr::null_mut(), action);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_go_iterator_data {
    pub go_active: bool,
}

#[no_mangle]
unsafe extern "C" fn iwl_mvm_go_iterator(_data: *mut c_void, mac: *mut u8, vif: *mut ieee80211_vif) {
    ...
    struct ieee80211_vif *vif)
    {
    let mut data: iwl_mvm_go_iterator_data = {};
//
...
// tim_size = cpu_to_le32((u32)beacon[tim_idx + 1]);
    ...
    struct ieee80211_supported_band *sband;
    let mut basic: c_ulong = vif.bss_conf.basic_rates;
    let mut lowest_cck: u16 = IWL_RATE_COUNT, lowest_ofdm = IWL_RATE_COUNT;
    u32 link_id = u32_get_bits(info.control.flags,
    IEEE80211_TX_CTRL_MLO_LINK);
    let mut band: u8 = info.band;
    u8 rate;
    u32 i;
    ...
    sband = mvm.hw.wiphy.bands[band];
    for_each_set_bit(i, &basic, BITS_PER_LONG) {
    let mut hw: u16 = sband.bitrates[i].hw_value;
    if (hw >= IWL_FIRST_OFDM_RATE) {
    if (lowest_ofdm > hw)
    lowest_ofdm = hw;
    ...
    rate = lowest_cck;
#[no_mangle]
pub unsafe extern "C" fn if(IWL_RATE_COUNT: lowest_ofdm !=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_ctxt_get_beacon_flags(fw: *const iwl_fw, rate_idx: u8) -> u16 {
    u16 iwl_mvm_mac_ctxt_get_beacon_flags(const struct iwl_fw *fw, u8 rate_idx)
    {
    let mut is_new_rate: bool = iwl_fw_lookup_cmd_ver(fw, BEACON_TEMPLATE_CMD, 0) > 10;
    let mut flags: u16 = 0;
    ...
    let mut legacy: u32 = vif.bss_conf.beacon_tx_rate.control[info.band].legacy;
// if beacon rate was configured try using it
    if (hweight32(legacy) == 1) {
    let mut rate: u32 = ffs(legacy) - 1;
...
// Set up TX command fields
    ...
    {
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    let mut beacon_cmd: iwl_mac_beacon_cmd_v6 = {};
    ...
    {
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    let mut beacon_cmd: iwl_mac_beacon_cmd_v7 = {};
    iwl_mvm_mac_ctxt_set_tx(mvm, vif, beacon, &beacon_cmd.tx);
    beacon_cmd.template_id = cpu_to_le32((u32)mvmvif.id);
    if (vif.type == NL80211_IFTYPE_AP)
    iwl_mvm_mac_ctxt_set_tim(mvm, &beacon_cmd.tim_idx,
    ...
    let mut beacon_cmd: iwl_mac_beacon_cmd = {};
    let mut rate: u8 = iwl_mvm_mac_ctxt_get_beacon_rate(mvm, info, vif);
    u16 flags;
    struct ieee80211_chanctx_conf *ctx;
    int channel;
...
// The beacon template for the AP/GO/IBSS has changed and needs update
    ...
    vif.type != NL80211_IFTYPE_ADHOC);
    beacon = ieee80211_beacon_get_template(mvm.hw, vif, core::ptr::null_mut(),
    link_conf.link_id);
    if (!beacon)
    return -ENOMEM;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_mac_ap_iterator_data {
    pub mvm: *mut iwl_mvm,
    pub vif: *mut ieee80211_vif,
    pub beacon_device_ts: u32,
    pub beacon_int: u16,
}
...
//
    ...
    let mut rand: u32 = get_random_u32_inclusive(36, 63);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_ctxt_add(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_mac_ctxt_remove(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> c_int {
...
// Don't start to countdown from a failed beacon
    ...
    let mut c: c_int = ieee80211_beacon_update_cntdwn(csa_vif, 0);
    ...
    let mut apply_time: u32 = gp2 + rel_time * 1024;
...
// Id can be mac/link id depending on the notification version
    let mut id: u32 = le32_to_cpu(mb.link_id);
...
// TODO: implement start trigger
    ...
    iwl_fw_dbg_collect_trig(&mvm.fwrt, trigger, core::ptr::null_mut());
    }
    ...
    struct sk_buff *skb;
    u8 *data;
    let mut size: u32 = le32_to_cpu(sb.byte_count);
...
// pass it as regular rx to mac80211
    ieee80211_rx_napi(mvm.hw, core::ptr::null_mut(), skb, core::ptr::null_mut());
    }
    ...
    let mut id: u32 = le32_to_cpu(notif.mac_id);
    struct ieee80211_vif *vif;
    struct iwl_mvm_vif *mvmvif;
...
// save mac_id or link_id to use later to cancel csa if needed
    ...
    let mut link_id: u32 = le32_to_cpu(notif.link_id);
...
// should never happen
    ...
    let mut csa_err_mask: u32 = le32_to_cpu(notif.csa_err_mask);
    rcu_read_lock();
