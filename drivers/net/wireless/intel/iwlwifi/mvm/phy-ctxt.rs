...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/phy-ctxt.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_get_channel_width(chandef: *const cfg80211_chan_def) -> u8 {
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_get_ctrl_pos(chandef: *const cfg80211_chan_def) -> u8 {
    u8 iwl_mvm_get_ctrl_pos(const struct cfg80211_chan_def *chandef)
    {
    let mut offs: c_int = chandef.chan.center_freq - chandef.center_freq1;
    let mut abs_offs: c_int = abs(offs);
    u8 ret;
    if (offs == 0) {
//
...
//
    ...
    {
    int ret;
    let mut ver: c_int = iwl_fw_lookup_cmd_ver(mvm.fw, PHY_CONTEXT_CMD, 1);
    if (ver >= 3 && ver <= 4) {
    let mut cmd: iwl_phy_context_cmd = {};
...
// Set the command data
    ...
    let mut cmd: iwl_phy_context_cmd_v1 = {};
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_phy_ctxt_ref(mvm: *mut iwl_mvm, ctxt: *mut iwl_mvm_phy_ctxt) {
...
//
    ...
    u8 chains_static, u8 chains_dynamic)
    {
    let mut action: enum iwl_ctxt_action = FW_CTXT_ACTION_MODIFY;
    lockdep_assert_held(&mvm.mutex);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_phy_ctxt_unref(mvm: *mut iwl_mvm, ctxt: *mut iwl_mvm_phy_ctxt) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_phy_ctx_count(mvm: *mut iwl_mvm) -> c_int {
    int iwl_mvm_phy_ctx_count(struct iwl_mvm *mvm)
    {
    let mut phy_ctxt_counter: c_ulong = 0;
