...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/mld-mac80211.c
#![no_std]
...
//
    ...
    out_remove_mac:
    mvmvif.link[0] = core::ptr::null_mut();
...
// if not valid, mac80211 puts default (max value)
    ...
    bool is_ap)
    {
    let mut cmd: iwl_txpower_constraints_cmd = {};
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    struct iwl_mvm_vif_link_info *link_info =
    mvmvif.link[bss_conf.link_id];
    let mut cmd_id: u32 = WIDE_ID(PHY_OPS_GROUP, AP_TX_POWER_CONSTRAINTS_CMD);
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_mld_vif_have_valid_ap_sta(mvmvif: *mut iwl_mvm_vif) -> bool {
...
// FIXME: need to decide if we need FTM responder per link
    ...
    let mut ret: c_int = iwl_mvm_ftm_start_responder(mvm, vif, link_conf);
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_mld_roc_link(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> c_int {
