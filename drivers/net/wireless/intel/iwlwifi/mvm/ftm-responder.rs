...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/ftm-responder.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_pasn_sta {
    pub list: list_head,
    pub int_sta: iwl_mvm_int_sta,
    pub addr: [u8; ETH_ALEN],
// must be last as it followed by buffer holding the key
    pub keyconf: ieee80211_key_conf,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_pasn_hltk_data {
    pub addr: *mut u8,
    pub cipher: u8,
    pub hltk: *mut u8,
}
...
// Up to 2 R2I STS are allowed on the responder
    ...
    {
    let mut cmd_id: u32 = WIDE_ID(LOCATION_GROUP, TOF_RESPONDER_CONFIG_CMD);
...
//
    ...
    let mut cmd_ver: u8 = iwl_fw_lookup_cmd_ver(mvm.fw, cmd_id, 6);
    int err;
    int cmd_size;
...
// may not be able to DMA from stack
    ...
    let mut aligned_lci_len: u32 = ALIGN(params.lci_len + 2, 4);
    let mut aligned_civicloc_len: u32 = ALIGN(params.civicloc_len + 2, 4);
    u8 *pos = data;
    lockdep_assert_held(&mvm.mutex);
...
// is still protected by the mutex.
    ...
    let mut flags: u32 = le32_to_cpu(resp.flags);
    if (resp.success_ftm == resp.ftm_per_burst)
    stats.success_num++;
#[no_mangle]
pub unsafe extern "C" fn if(2: resp->success_ftm >=) -> else {
