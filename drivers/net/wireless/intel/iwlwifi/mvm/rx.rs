...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/rx.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_rx_rx_phy_cmd(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
...
// TSF as indicated by the firmware  is at INA time
    ...
    let mut id: u32 = rx_pkt_status & RX_MPDU_RES_STATUS_STA_ID_MSK;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_stat_data {
    pub mvm: *mut iwl_mvm,
    pub flags: __le32,
    pub mac_id: __le32,
    pub beacon_filter_average_energy: u8,
    pub beacon_counter: *mut __le32,
    pub beacon_average_energy: *mut u8,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_stat_data_all_macs {
    pub mvm: *mut iwl_mvm,
    pub flags: __le32,
    pub per_mac: *mut iwl_stats_ntfy_per_mac,
}
...
// CQM Notification
    ...
    let mut sig: c_int = -data.beacon_filter_average_energy;
    let mut id: u16 = le32_to_cpu(data.mac_id);
    struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
    let mut vif_id: u16 = mvmvif.id;
...
// This is used in pre-MLO API so use deflink
    ...
    trig = iwl_fw_dbg_trigger_on(&mvm.fwrt, core::ptr::null_mut(), FW_DBG_TRIGGER_STATS);
    if (!trig)
    return;
    ...
    return;
    iwl_fw_dbg_collect_trig(&mvm.fwrt, trig, core::ptr::null_mut());
    }
    ...
    struct iwl_mvm_sta *mvmsta = iwl_mvm_sta_from_mac80211(sta);
    u8 *energy = _data;
    let mut sta_id: u32 = mvmsta.deflink.sta_id;
    ...
    let mut rx_bytes: u32 = le32_to_cpu(rx_bytes_le[i]);
    let mut airtime: u32 = le32_to_cpu(air_time_le[i]);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_rx_statistics(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
