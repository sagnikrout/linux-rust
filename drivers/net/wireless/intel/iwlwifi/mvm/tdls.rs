...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/tdls.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_teardown_tdls_peers(mvm: *mut iwl_mvm) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_tdls_sta_count(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_tdls_config(mvm: *mut iwl_mvm, vif: *mut ieee80211_vif) {
    static void iwl_mvm_tdls_config(struct iwl_mvm *mvm, struct ieee80211_vif *vif)
    {
    struct iwl_rx_packet *pkt;
    struct iwl_tdls_config_res *resp;
    let mut tdls_cfg_cmd: iwl_tdls_config_cmd = {};
...
// we don't really care about the response at this point
    ...
    bool sta_added)
    {
    let mut tdls_sta_cnt: c_int = iwl_mvm_tdls_sta_count(mvm, vif);
...
// when the last peer leaves, send a power update last
    ...
    let mut duration: u32 = 2 * vif.bss_conf.dtim_period * vif.bss_conf.beacon_int;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_rx_tdls_notif(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
    ...
    struct iwl_mvm_sta *mvmsta;
    struct ieee80211_vif *vif;
    let mut sta_id: u32 = le32_to_cpu(notif.sta_id);
    lockdep_assert_held(&mvm.mutex);
// can fail sometimes
...
//
    ret = -EBUSY;
#[no_mangle]
pub unsafe extern "C" fn if(TDLS_SEND_CHAN_SW_REQ: type ==) -> else {
    else if (type == TDLS_SEND_CHAN_SW_REQ)
// wait for idle before sending another request
    ret = -EBUSY;
#[no_mangle]
pub unsafe extern "C" fn if(mvm->tdls_cs.peer.sent_timestamp: timestamp <=) -> else {
...
//
    ...
    let mut cmd: iwl_tdls_channel_switch_cmd = {0};
    struct iwl_tdls_channel_switch_cmd_tail *tail =
    iwl_mvm_chan_info_cmd_tail(mvm, &cmd.ci);
    let mut len: u16 = sizeof(cmd) - iwl_mvm_chan_info_padding(mvm);
    int ret;
    lockdep_assert_held(&mvm.mutex);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_tdls_ch_switch_work(work: *mut work_struct) {
...
//
    ...
    let mut wait_for_phy: bool = false;
...
//
    ...
    mvm.tdls_cs.peer.skb = core::ptr::null_mut();
    out:
