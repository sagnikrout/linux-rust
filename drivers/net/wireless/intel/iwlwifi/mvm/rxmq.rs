...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/rxmq.c
#![no_std]
...
// iwl_mvm_create_skb Adds the rxb to a new skb
    ...
    let mut hdrlen: c_uint = ieee80211_hdrlen(hdr.frame_control);
...
#[no_mangle]
pub unsafe extern "C" fn if(IWL_DEVICE_FAMILY_BZ: mvm->trans->mac_cfg->device_family <) -> else {
...
//
    ...
    let mut vendor_data_len: u16 = sizeof(mvm.cur_aid);
    if (!mvm.cur_aid)
    return;
...
// skip also in case we don't have it (yet)
    ...
    let mut len: u32 = le16_to_cpu(desc.mpdu_len);
...
#[no_mangle]
pub unsafe extern "C" fn if(IWL_RX_MPDU_STATUS_REPLAY_ERROR: status &) -> else {
    ...
    {
    let mut status: u32 = le32_to_cpu(desc.status);
//
...
//
    ...
    let mut hwsum: u16 = be16_to_cpu(desc.v3.raw_xsum);
    skb.ip_summed = CHECKSUM_COMPLETE;
    skb.csum = csum_unfold(~( __sum16)hwsum);
    }
    } else {
    struct iwl_mvm_sta *mvmsta = iwl_mvm_sta_from_mac80211(sta);
    struct iwl_mvm_vif *mvmvif;
    let mut flags: u16 = le16_to_cpu(desc.l3l4_flags);
...
// Allow same PN as the first subframe for following sub frames
    ...
    let mut ssn: u16 = reorder_buf.head_sn;
    lockdep_assert_held(&reorder_buf.lock);
    while (ieee80211_sn_less(ssn, nssn)) {
    let mut index: c_int = ssn % baid_data.buf_size;
...
// release all frames that are in the reorder buffer to the stack
    ...
    iwl_mvm_release_frames(mvm, sta, core::ptr::null_mut(), ba_data, reorder_buf,
...
//
    ...
    let mut reorder: u32 = le32_to_cpu(desc.reorder_data);
    let mut amsdu: bool = desc.mac_flags2 & IWL_RX_MPDU_MFLG2_AMSDU;
    bool last_subframe =
...
//
    ...
    buffer, nssn);
#[no_mangle]
pub unsafe extern "C" fn if(1: buffer->num_stored ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_flip_address(addr: *mut u8) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_mvm_rx_phy_data {
    pub info_type: enum iwl_rx_phy_info_type,
    pub d5: __le32 d0, d1, d2, d3, eht_d4,,
    pub d4: __le16,
    pub with_data: bool,
    pub first_subframe: bool,
    pub rx_vec: [__le32; 4],
    pub rate_n_flags: u32,
    pub gp2_on_air_rise: u32,
    pub phy_info: u16,
    pub energy_b: u8 energy_a,,
    pub channel: u8,
}
    ...
    {
    let mut phy_data2: u32 = le32_to_cpu(phy_data.d2);
    let mut phy_data3: u32 = le32_to_cpu(phy_data.d3);
    let mut phy_data4: u16 = le16_to_cpu(phy_data.d4);
    let mut rate_n_flags: u32 = phy_data.rate_n_flags;
...
//
    let mut ru: u8 = le32_get_bits(phy_data.d1, IWL_RX_PHY_DATA1_HE_RU_ALLOC_MASK);
    let mut rate_n_flags: u32 = phy_data.rate_n_flags;
    let mut he_type: u32 = rate_n_flags & RATE_MCS_HE_TYPE_MSK;
    let mut offs: u8 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(RATE_MCS_HE_TYPE_TRIG: he_type ==) -> else {
...
pub const IEEE80211_RADIOTAP_RU_DATA_1_1_1: c_int = 1;
pub const IEEE80211_RADIOTAP_RU_DATA_2_1_1: c_int = 2;
pub const IEEE80211_RADIOTAP_RU_DATA_1_1_2: c_int = 2;
pub const IEEE80211_RADIOTAP_RU_DATA_2_1_2: c_int = 2;
pub const IEEE80211_RADIOTAP_RU_DATA_1_2_1: c_int = 3;
pub const IEEE80211_RADIOTAP_RU_DATA_2_2_1: c_int = 3;
pub const IEEE80211_RADIOTAP_RU_DATA_1_2_2: c_int = 3;
pub const IEEE80211_RADIOTAP_RU_DATA_2_2_2: c_int = 4;
pub const IWL_RX_RU_DATA_A1: c_int = 2;
pub const IWL_RX_RU_DATA_A2: c_int = 2;
pub const IWL_RX_RU_DATA_B1: c_int = 2;
pub const IWL_RX_RU_DATA_B2: c_int = 4;
pub const IWL_RX_RU_DATA_C1: c_int = 3;
pub const IWL_RX_RU_DATA_C2: c_int = 3;
pub const IWL_RX_RU_DATA_D1: c_int = 4;
pub const IWL_RX_RU_DATA_D2: c_int = 4;
    ...
    let mut data1: __le32 = phy_data.d1;
    let mut data2: __le32 = phy_data.d2;
    let mut data3: __le32 = phy_data.d3;
    let mut data4: __le32 = phy_data.eht_d4;
    let mut data5: __le32 = phy_data.d5;
    let mut phy_bw: u32 = phy_data.rate_n_flags & RATE_MCS_CHAN_WIDTH_MSK;
...
// content channel 2
    ...
    let mut usig_a1: __le32 = phy_data.rx_vec[0];
    let mut usig_a2: __le32 = phy_data.rx_vec[1];
...
//
    ...
    struct ieee80211_radiotap_eht_usig *usig)
    {
    let mut data0: __le32 = phy_data.d0;
    let mut data1: __le32 = phy_data.d1;
    let mut usig_a1: __le32 = phy_data.rx_vec[0];
    let mut info_type: u8 = phy_data.info_type;
// Not in EHT range
...
//
    ...
    struct ieee80211_radiotap_eht *eht;
    struct ieee80211_radiotap_eht_usig *usig;
    let mut eht_len: usize = sizeof(*eht);
    let mut rate_n_flags: u32 = phy_data.rate_n_flags;
    let mut he_type: u32 = rate_n_flags & RATE_MCS_HE_TYPE_MSK;
// EHT and HE have the same valus for LTF
    let mut ltf: u8 = IEEE80211_RADIOTAP_HE_DATA5_LTF_SIZE_UNKNOWN;
    let mut phy_info: u16 = phy_data.phy_info;
    u32 bw;
// u32 for 1 user_info
    if (phy_data.with_data)
    eht_len += sizeof(u32);
...
// nothing here
    ...
    {
    struct ieee80211_rx_status *rx_status = IEEE80211_SKB_RXCB(skb);
    struct ieee80211_radiotap_he *he = core::ptr::null_mut();
    struct ieee80211_radiotap_he_mu *he_mu = core::ptr::null_mut();
    let mut rate_n_flags: u32 = phy_data.rate_n_flags;
    let mut he_type: u32 = rate_n_flags & RATE_MCS_HE_TYPE_MSK;
    u8 ltf;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_rx_sta_csa {
    pub all_sta_unblocked: bool,
    pub vif: *mut ieee80211_vif,
}

#[no_mangle]
unsafe extern "C" fn iwl_mvm_rx_get_sta_block_tx(data: *mut c_void, sta: *mut ieee80211_sta) {
...
//
    ...
    struct ieee80211_rx_status *rx_status = IEEE80211_SKB_RXCB(skb);
    let mut rate_n_flags: u32 = phy_data.rate_n_flags;
    let mut stbc: u8 = u32_get_bits(rate_n_flags, RATE_MCS_STBC_MSK);
...
// using TLV format and must be after all fixed len fields
    ...
    struct sk_buff *skb;
    let mut crypt_len: u8 = 0;
    size_t desc_size;
    let mut phy_data: iwl_mvm_rx_phy_data = {};
    u32 format;
...
//
    sta = ieee80211_find_sta_by_ifaddr(mvm.hw, hdr.addr2, core::ptr::null_mut());
    }
...
// qc &= ~IEEE80211_QOS_CTL_A_MSDU_PRESENT;
    ...
    if (baid != IWL_RX_REORDER_DATA_INVALID_BAID) {
    let mut reorder_data: u32 = le32_to_cpu(desc.reorder_data);
...
// mark as failed PLCP on any errors to skip checks in mac80211
    ...
    rx_status.band = phy_data.channel > 14 ? NL80211_BAND_5GHZ :
    NL80211_BAND_2GHZ;
    iwl_mvm_rx_fill_status(mvm, core::ptr::null_mut(), core::ptr::null_mut(), skb, &phy_data, queue);
...
// MAC or link ID in v1/v2, but driver has the IDs equal
    ...
    let mut id: u32 = le32_to_cpu(notif.link_id);
    struct iwl_mvm_vif *mvm_vif;
    struct ieee80211_vif *vif;
// >= means AUX MAC/link ID, no energy correction needed then
