...
//! Source: drivers/net/wireless/intel/iwlwifi/mvm/tx.c
#![no_std]
...
//
    ...
    trig = iwl_fw_dbg_trigger_on(&mvm.fwrt, core::ptr::null_mut(), FW_DBG_TRIGGER_BA);
    if (!trig)
    return;
    ...
    let mut mh_len: u16 = ieee80211_hdrlen(hdr.frame_control);
    let mut offload_assist: u16 = 0;
...
// We do not expect to be requested to csum stuff we do not support
    ...
    let mut off: c_uint = sizeof(*ipv6h);
...
#[no_mangle]
pub unsafe extern "C" fn if(4: ieee80211_hdrlen(hdr->frame_control) %) -> else {
...
//
    ...
    let mut tx_flags: u32 = le32_to_cpu(tx_cmd_params.tx_flags);
    let mut len: u32 = skb.len + FCS_LEN;
    let mut amsdu: bool = false;
    u8 ac;
    ...
    let mut control: u16 = le16_to_cpu(bar.control);
    let mut ssn: u16 = le16_to_cpu(bar.start_seq_num);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ieee80211_is_action(fc)) -> else {
...
// Set CCK or OFDM flag
    ...
    let mut mcs: u8 = ieee80211_rate_get_vht_mcs(rate);
    let mut nss: u8 = ieee80211_rate_get_vht_nss(rate);
...
pub unsafe extern "C" fn if(IEEE80211_TX_RC_160_MHZ_WIDTH: rate->flags &) -> else {
    ...
    let mut rate_idx: c_int = info.control.rates[0].idx;
...
//
    ...
    let mut type: enum iwl_tx_cmd_sec_ctrl = TX_CMD_SEC_CCM;
    u64 pn;
...
//
    ...
    let mut rate_n_flags: __le32 = 0;
    let mut flags: u16 = 0;
    struct iwl_mvm_sta *mvmsta = sta ?
    iwl_mvm_sta_from_mac80211(sta) : core::ptr::null_mut();
    let mut amsdu: bool = false;
...
//
    ...
    let mut base_len: c_int = (u8 *)mgmt.u.probe_resp.variable - (u8 *)mgmt;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_tx_skb_non_sta(mvm: *mut iwl_mvm, skb: *mut sk_buff) -> c_int {
    ...
    struct iwl_mvm_vif *mvmvif =
    iwl_mvm_vif_from_mac80211(info.control.vif);
    let mut p2p_aux: bool = iwl_mvm_has_p2p_over_aux(mvm);
...
//
    ...
    dev_cmd = iwl_mvm_set_tx_params(mvm, skb, &info, hdrlen, core::ptr::null_mut(), sta_id,
    core::ptr::null_mut());
    if (!dev_cmd)
    return -1;
// From now on, we cannot access info->control
    ...
    {
    struct iwl_mvm_sta *mvmsta = iwl_mvm_sta_from_mac80211(sta);
    let mut ac: u8 = tid_to_mac80211_ac[tid];
...
//
    ...
    u16 snap_ip_tcp, pad;
    let mut netdev_flags: netdev_features_t = NETIF_F_CSUM_MASK | NETIF_F_SG;
    u8 tid;
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_txq_should_update(mvm: *mut iwl_mvm, txq_id: c_int) -> bool {
    static bool iwl_mvm_txq_should_update(struct iwl_mvm *mvm, int txq_id)
    {
    let mut queue_tid_bitmap: c_ulong = mvm.queue_info[txq_id].tid_bitmap;
    ...
    int airtime)
    {
    let mut mac: c_int = mvmsta.mac_id_n_color & FW_CTXT_ID_MSK;
    ...
    {
    let mut ac: u32 = tid_to_mac80211_ac[tid];
    let mut mac: c_int = mvmsta.mac_id_n_color & FW_CTXT_ID_MSK;
    struct iwl_mvm_tcm_mac *mdata;
    if (mac >= NUM_MAC_INDEX_DRIVER)
    return -EINVAL;
...
//
    ...
    let mut tid: u8 = IWL_MAX_TID_COUNT;
    u16 txq_id;
    let mut is_ampdu: bool = false;
    int hdrlen;
    if (WARN_ON_ONCE(!sta))
    return -1;
...
//
    ...
    memcpy(&info, skb.cb, sizeof(info));
    if (!skb_is_gso(skb))
    return iwl_mvm_tx_mpdu(mvm, skb, &info, sta, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn iwl_mvm_get_hwrate_chan_width(chan_width: u32) -> c_int {
...
// struct iwl_tx_resp_v3 is almost the same
    ...
    let mut sta_id: c_int = IWL_TX_RES_GET_RA(tx_resp.ra_tid);
    let mut tid: c_int = IWL_TX_RES_GET_TID(tx_resp.ra_tid);
    struct agg_tx_status *agg_status =
    iwl_mvm_get_agg_status(mvm, tx_resp);
    let mut status: u32 = le16_to_cpu(agg_status.status);
    let mut ssn: u16 = iwl_mvm_get_scd_ssn(mvm, tx_resp);
    struct sk_buff_head skbs;
    let mut skb_freed: u8 = 0;
    u8 lq_color;
    u16 next_reclaimed, seq_ctl;
    let mut is_ndp: bool = false;
...
// W/A FW bug: seq_ctl is wrong upon failure / BAR frame
    ...
    seq_ctl = 0;
#[no_mangle]
pub unsafe extern "C" fn if(TX_STATUS_SUCCESS: status !=) -> else {
...
//
    ...
    &mvmsta.tid_data[tid];
    let mut send_eosp_ndp: bool = false;
...
//
    ...
    iwl_mvm_get_agg_status(mvm, tx_resp);
    int i;
    let mut tirgger_timepoint: bool = false;
    for (i = 0; i < tx_resp.frame_count; i++) {
    let mut fstatus: u16 = le16_to_cpu(frame_status[i].status);
// In case one frame wasn't transmitted trigger time point
    ...
    iwl_dbg_tlv_time_point(&mvm.fwrt,
    IWL_FW_INI_TIME_POINT_TX_FAILED, core::ptr::null_mut());
    }
    ...
    let mut sequence: u16 = le16_to_cpu(pkt.hdr.sequence);
    struct iwl_mvm_sta *mvmsta;
    let mut queue: c_int = SEQ_TO_QUEUE(sequence);
    struct ieee80211_sta *sta;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_rx_tx_cmd(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
    ...
    struct iwl_mvm_tid_data *tid_data = core::ptr::null_mut();
    struct ieee80211_sta *sta;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_rx_ba_notif(mvm: *mut iwl_mvm, rxb: *mut iwl_rx_cmd_buffer) {
    void iwl_mvm_rx_ba_notif(struct iwl_mvm *mvm, struct iwl_rx_cmd_buffer *rxb)
    {
    struct iwl_rx_packet *pkt = rxb_addr(rxb);
    let mut pkt_len: c_uint = iwl_rx_packet_payload_len(pkt);
    int sta_id, tid, txq, index;
    let mut ba_info: ieee80211_tx_info = {};
    ...
    (void *)pkt.data;
    let mut lq_color: u8 = TX_RES_RATE_TABLE_COL_GET(ba_res.tlc_rate_info);
    u16 tfd_cnt;
    int i;
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_flush_tx_path(mvm: *mut iwl_mvm, tfd_msk: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_flush_sta_tids(mvm: *mut iwl_mvm, sta_id: u32, tids: u16) -> c_int {
    ...
    let mut tx_info: ieee80211_tx_info = {};
    struct iwl_flush_queue_info *queue_info = &rsp.queues[i];
    let mut tid: c_int = le16_to_cpu(queue_info.tid);
    let mut read_before: c_int = le16_to_cpu(queue_info.read_before_flush);
    let mut read_after: c_int = le16_to_cpu(queue_info.read_after_flush);
    let mut queue_num: c_int = le16_to_cpu(queue_info.queue_num);
    if (tid == IWL_MGMT_TID)
    tid = IWL_MAX_TID_COUNT;
    IWL_DEBUG_TX_QUEUES(mvm,
...
#[no_mangle]
pub unsafe extern "C" fn iwl_mvm_flush_sta(mvm: *mut iwl_mvm, sta_id: u32, tfd_queue_mask: u32) -> c_int {
