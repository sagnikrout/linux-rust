...
//! Source: drivers/net/wireless/intel/iwlwifi/mld/tests/agg.c
#![no_std]
...
pub const BA_WINDOW_SIZE: c_int = 64;
pub const QUEUE: c_int = 0;
...
// We can't use for_each_sta_active_link
    ...
    let mut sn: u16 = param.reorder_buf_state.entries[i].sn;
    let mut index: c_int = sn % baid_data.buf_size;
    u8 add_subframes =
    param.reorder_buf_state.entries[i].add_subframes;
// create 1 skb per entry + additional skbs per num of
// requested subframes
//
    let mut num_skbs: u8 = 1 + add_subframes;
    ...
    const struct reorder_buffer_case *param =
    (const void *)(test.param_value);
    struct iwl_mld_baid_data *baid_data = core::ptr::null_mut();
    struct iwl_mld_reorder_buffer *buffer;
    let mut reorder_buf_size: u32 = BA_WINDOW_SIZE * sizeof(baid_data.entries[0]);
    let mut baid: u8 = param.reorder_buf_state.baid;
// Assuming only 1 RXQ
...
#[no_mangle]
unsafe extern "C" fn test_reorder_buffer(test: *mut kunit) {
    ...
    struct iwl_mld_reorder_buffer *buffer;
    enum iwl_mld_reorder_result reorder_res;
    let mut skb_release_order_count: u16 = param.expected.skb_release_order_count;
    let mut skb_idx: u16 = 0;
...
// Prepare skb, mpdu_desc, BA data and the reorder buffer
    ...
    rcu_read_lock();
    reorder_res = iwl_mld_reorder(mld, core::ptr::null_mut(), QUEUE, sta, skb, mpdu_desc);
    rcu_read_unlock();
