...
//! Source: drivers/net/wireless/intel/iwlwifi/fw/dbg-old.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iwl_fw_dump_ptrs {
    pub trans_ptr: *mut iwl_trans_dump_data,
    pub fwrt_ptr: *mut c_void,
    pub fwrt_len: u32,
}

pub const RADIO_REG_MAX_READ: c_uint = 0x2ad;
    ...
    let mut rd_cmd: u32 = RADIO_RSP_RD_CMD;
...
// Count TXF sizes
    ...
    let mut addr: dma_addr_t = fwrt.fw_paging_db[i].fw_paging_phys;
    ...
    let mut smem_len: u32 = fwrt.fw.dbg.n_mem_tlv ? 0 : fwrt.trans.mac_cfg.base.smem_len;
...
// In case we only want monitor dump, skip to dump transport data
    ...
    for (i = 0; i < fwrt.fw.dbg.n_mem_tlv; i++) {
    let mut len: u32 = le32_to_cpu(fw_dbg_mem[i].len);
    let mut ofs: u32 = le32_to_cpu(fw_dbg_mem[i].ofs);
    ...
    if (iwl_fw_dbg_is_d3_debug_enabled(fwrt) && fwrt.dump.d3_debug_data) {
    let mut addr: u32 = fwrt.trans.mac_cfg.base.d3_debug_data_base_addr;
    let mut data_size: usize = fwrt.trans.mac_cfg.base.d3_debug_data_length;
    dump_data.type = cpu_to_le32(IWL_FW_ERROR_DUMP_D3_DEBUG_DATA);
    dump_data.len = cpu_to_le32(data_size * 2);
    memcpy(dump_data.data, fwrt.dump.d3_debug_data, data_size);
    kfree(fwrt.dump.d3_debug_data);
    fwrt.dump.d3_debug_data = core::ptr::null_mut();
...
// Dump fw's virtual image
    ...
    {
    let mut fw_error_dump: iwl_fw_dump_ptrs = {};
    struct iwl_fw_error_dump_file *dump_file;
    struct scatterlist *sg_dump_data;
    u32 file_len;
    let mut dump_mask: u32 = fwrt.fw.dbg.dump_mask;
