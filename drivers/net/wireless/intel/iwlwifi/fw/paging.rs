...
//! Source: drivers/net/wireless/intel/iwlwifi/fw/paging.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn iwl_free_fw_paging(fwrt: *mut iwl_fw_runtime) {
    ...
    paging.fw_paging_block = core::ptr::null_mut();
    }
...
//
    ...
    let mut remaining: c_int = image.sec[sec_idx].len - offset;
    let mut len: c_int = block.fw_paging_size;
//
...
// loop for all paging blocks + CSS block
    for (blk_idx = 0; blk_idx < fwrt.num_of_paging_blk + 1; blk_idx++) {
    let mut addr: dma_addr_t = fwrt.fw_paging_db[blk_idx].fw_paging_phys;
    __le32 phy_addr;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_init_paging(fwrt: *mut iwl_fw_runtime, type: enum iwl_ucode_type) -> c_int {
