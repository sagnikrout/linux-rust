...
//! Source: drivers/net/wireless/intel/iwlwifi/fw/rs.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn rs_pretty_print_rate(buf: *mut c_char, bufsz: c_int, rate: u32) -> c_int {
    int rs_pretty_print_rate(char *buf, int bufsz, const u32 rate)
    {
    char *type;
    let mut mcs: u8 = 0, nss = 0;
    let mut ant: u8 = (rate & RATE_MCS_ANT_AB_MSK) >> RATE_MCS_ANT_POS;
    u32 bw = (rate & RATE_MCS_CHAN_WIDTH_MSK) >>
    RATE_MCS_CHAN_WIDTH_POS;
    let mut format: u32 = rate & RATE_MCS_MOD_TYPE_MSK;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_he_is_sgi(rate_n_flags: u32) -> bool {
    bool iwl_he_is_sgi(u32 rate_n_flags)
    {
    let mut type: u32 = rate_n_flags & RATE_MCS_HE_TYPE_MSK;
    let mut ltf_gi: u32 = rate_n_flags & RATE_MCS_HE_GI_LTF_MSK;
    if (type == RATE_MCS_HE_TYPE_SU ||
    type == RATE_MCS_HE_TYPE_EXT_SU)
    let mut ltf_gi: return = = RATE_MCS_HE_SU_4_LTF_08_GI;
    return false;
    }
