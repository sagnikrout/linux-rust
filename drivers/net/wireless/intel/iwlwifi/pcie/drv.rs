...
//! Source: drivers/net/wireless/intel/iwlwifi/pcie/drv.c
#![no_std]
...
// Killer
    ...
    let mut iwl_dev_info_table_size: c_uint = ARRAY_SIZE(iwl_dev_info_table);
...
pub const PCI_CFG_RETRY_TIMEOUT: c_uint = 0x041;
    ...
    {
    let mut num_devices: c_int = ARRAY_SIZE(iwl_dev_info_table);
    int i;
    if (!num_devices)
...
#[no_mangle]
unsafe extern "C" fn iwl_pci_probe(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwl_pci_remove(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn iwl_pci_suspend(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn _iwl_pci_resume(device: *mut device, restore: bool) -> c_int {
    static int _iwl_pci_resume(struct device *device, bool restore)
    {
    struct pci_dev *pdev = to_pci_dev(device);
    struct iwl_trans *trans = pci_get_drvdata(pdev);
    struct iwl_trans_pcie *trans_pcie = IWL_TRANS_GET_PCIE_TRANS(trans);
    let mut device_was_powered_off: bool = false;
// Before you put code here, think about WoWLAN. You cannot check here
// whether WoWLAN is enabled or not, and your code will run even if
...
//
    ...
    let mut scratch: u32 = iwl_read32(trans, CSR_FUNC_SCRATCH);
...
#[no_mangle]
unsafe extern "C" fn iwl_pci_restore(device: *mut device) -> c_int {
    static int iwl_pci_restore(struct device *device)
    {
    return _iwl_pci_resume(device, true);
    }
#[no_mangle]
unsafe extern "C" fn iwl_pci_resume(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwl_pci_dump(device: *mut device) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_pci_register_driver() -> int __must_check {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_pci_unregister_driver() {
