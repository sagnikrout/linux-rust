...
//! Source: drivers/net/wireless/intel/iwlwifi/pcie/gen1_2/tx-gen2.c
#![no_std]
...
//
    ...
    bool unmap)
    {
    let mut oldphys: dma_addr_t = phys;
    struct page *page;
    int ret;
    if (unlikely(dma_mapping_error(trans.dev, phys)))
    return -ENOMEM;
...
// this is the data left for this subframe
    let mut data_left: c_uint = min_t(unsigned int, mss, total_len);
    unsigned int tb_len;
    dma_addr_t tb_phys;
...
// Not a real mapping error, use direct comparison
    ...
    tb_phys, tso.data,
    tb_len, core::ptr::null_mut(), false);
    if (ret)
    goto out_err;
    data_left -= tb_len;
    data_offset += tb_len;
...
// re -add the WiFi header
    ...
    int tx_cmd_len)
    {
    let mut idx: c_int = iwl_txq_get_cmd_index(txq, txq.write_ptr);
...
// building the A-MSDU might have changed this data, memcpy it now
    ...
    dma_addr_t tb_phys;
    let mut fragsz: c_uint = skb_frag_size(frag);
    int ret;
    if (!fragsz)
    continue;
...
// set up TFD's third entry to point to remainder of skb's head
    ...
    ret = iwl_txq_gen2_set_tb_with_wa(trans, skb, tfd, tb_phys,
    frag.data,
    skb_headlen(frag), core::ptr::null_mut(),
    true);
    if (ret)
    goto out_err;
...
#[no_mangle]
pub unsafe extern "C" fn iwl_txq_space(trans: *mut iwl_trans, q: *const iwl_txq) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwl_txq_gen2_get_num_tbs(tfd: *mut iwl_tfh_tfd) -> u8 {
    ...
    let mut idx: c_int = iwl_txq_gen2_get_num_tbs(tfd);
...
#[no_mangle]
unsafe extern "C" fn iwl_txq_gen2_free_tfd(trans: *mut iwl_trans, txq: *mut iwl_txq) {
    static void iwl_txq_gen2_free_tfd(struct iwl_trans *trans, struct iwl_txq *txq)
    {
// rd_ptr is bounded by TFD_QUEUE_SIZE_MAX and
// idx is bounded by n_window
//
    let mut idx: c_int = iwl_txq_get_cmd_index(txq, txq.read_ptr);
    struct sk_buff *skb;
...
//
#[no_mangle]
unsafe extern "C" fn iwl_txq_inc_wr_ptr(trans: *mut iwl_trans, txq: *mut iwl_txq) {
...
// Set up first empty entry in queue's array of Tx/cmd buffers
    ...
    if (!tfd) {
    txq.entries[idx].skb = core::ptr::null_mut();
    txq.entries[idx].cmd = core::ptr::null_mut();
    spin_unlock(&txq.lock);
    return -1;
    }
    if (trans.mac_cfg.device_family >= IWL_DEVICE_FAMILY_AX210) {
...
//
#[no_mangle]
unsafe extern "C" fn iwl_txq_gen2_unmap(trans: *mut iwl_trans, txq_id: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn iwl_txq_gen2_free(trans: *mut iwl_trans, txq_id: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_txq_dyn_free(trans: *mut iwl_trans, queue: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_txq_gen2_tx_free(trans: *mut iwl_trans) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_txq_gen2_init(trans: *mut iwl_trans, txq_id: c_int, queue_size: c_int) -> c_int {
