...
//! Source: drivers/net/wireless/intel/iwlwifi/pcie/gen1_2/tx.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn iwl_pcie_free_dma_ptr(trans: *mut iwl_trans, ptr: *mut iwl_dma_ptr) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_pcie_txq_check_wrptrs(trans: *mut iwl_trans) {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_txq_gen1_tfd_get_num_tbs(tfd: *mut iwl_tfd) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn iwl_pcie_clear_cmd_in_flight(trans: *mut iwl_trans) {
...
//
    let mut idx: c_int = iwl_txq_get_cmd_index(txq, read_ptr);
    struct sk_buff *skb;
...
//
    ...
    txq.entries[idx].skb = core::ptr::null_mut();
    }
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn iwl_pcie_txq_unmap(trans: *mut iwl_trans, txq_id: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn iwl_pcie_txq_free(trans: *mut iwl_trans, txq_id: c_int) {
...
// De-alloc circular buffer of TFDs
    ...
    txq.dma_addr = 0;
    txq.tfds = core::ptr::null_mut();
    dma_free_coherent(dev,
    sizeof(*txq.first_tb_bufs) * txq.n_window,
    txq.first_tb_bufs, txq.first_tb_dma);
    }
    kfree(txq.entries);
    txq.entries = core::ptr::null_mut();
    timer_delete_sync(&txq.stuck_timer);
// 0-fill queue descriptor structure
    memset(txq, 0, sizeof(*txq));
    }
#[no_mangle]
pub unsafe extern "C" fn iwl_pcie_tx_start(trans: *mut iwl_trans) {
    void iwl_pcie_tx_start(struct iwl_trans *trans)
    {
    struct iwl_trans_pcie *trans_pcie = IWL_TRANS_GET_PCIE_TRANS(trans);
    let mut nq: c_int = trans.mac_cfg.base.num_of_queues;
    int chan;
    u32 reg_val;
...
// reset context data, TX status and translation data
    ...
    core::ptr::null_mut(), clear_dwords);
...
#[no_mangle]
pub unsafe extern "C" fn iwl_trans_pcie_tx_reset(trans: *mut iwl_trans) {
...
#[no_mangle]
unsafe extern "C" fn iwl_pcie_tx_stop_fh(trans: *mut iwl_trans) {
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_pcie_tx_stop(trans: *mut iwl_trans) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn iwl_pcie_tx_free(trans: *mut iwl_trans) {
...
// Tx queues
    ...
    trans_pcie.txqs.txq[txq_id] = core::ptr::null_mut();
    }
    }
    kfree(trans_pcie.txq_memory);
    trans_pcie.txq_memory = core::ptr::null_mut();
    iwl_pcie_free_dma_ptr(trans, &trans_pcie.kw);
    iwl_pcie_free_dma_ptr(trans, &trans_pcie.txqs.scd_bc_tbls);
    }
#[no_mangle]
pub unsafe extern "C" fn iwl_txq_log_scd_error(trans: *mut iwl_trans, txq: *mut iwl_txq) {
    void iwl_txq_log_scd_error(struct iwl_trans *trans, struct iwl_txq *txq)
    {
    let mut txq_id: u32 = txq.id;
    u32 status;
    bool active;
    u8 fifo;
    if (trans.mac_cfg.gen2) {
...
#[no_mangle]
unsafe extern "C" fn iwl_txq_stuck_timer(t: *mut timer_list) {
...
//
#[no_mangle]
unsafe extern "C" fn iwl_pcie_tx_alloc(trans: *mut iwl_trans) -> c_int {
    static int iwl_pcie_tx_alloc(struct iwl_trans *trans)
    {
    int ret;
    int txq_id, slots_num;
    struct iwl_trans_pcie *trans_pcie = IWL_TRANS_GET_PCIE_TRANS(trans);
    let mut bc_tbls_size: u16 = trans.mac_cfg.base.num_of_queues;
...
// Alloc and init all Tx queues, including the command queue (#4/#9)
    for (txq_id = 0; txq_id < trans.mac_cfg.base.num_of_queues;
    txq_id++) {
    let mut cmd_queue: bool = (txq_id == trans.conf.cmd_queue);
    if (cmd_queue)
...
//
#[no_mangle]
unsafe extern "C" fn iwl_queue_init(q: *mut iwl_txq, slots_num: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iwl_pcie_tx_init(trans: *mut iwl_trans) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iwl_txq_progress(txq: *mut iwl_txq) {
...
//
    ...
    {
    let mut index: c_int = iwl_txq_get_cmd_index(q, i);
    let mut r: c_int = iwl_txq_get_cmd_index(q, read_ptr);
    let mut w: c_int = iwl_txq_get_cmd_index(q, write_ptr);
    return w >= r ?
    (index >= r && index < w) :
    !(index < r && index >= w);
    }
...
//
#[no_mangle]
unsafe extern "C" fn iwl_pcie_cmdq_reclaim(trans: *mut iwl_trans, txq_id: c_int, idx: c_int) {
...
// combined with Traffic ID (QOS priority), in format used by Tx Scheduler
    ...
    struct iwl_trans_pcie *trans_pcie = IWL_TRANS_GET_PCIE_TRANS(trans);
    struct iwl_txq *txq;
    let mut fifo: c_int = -1;
    let mut scd_bug: bool = false;
...
// Set this queue as a chain-building queue unless it is CMD
    ...
    if (cfg.aggregate) {
    let mut ra_tid: u16 = BUILD_RAxTID(cfg.sta_id, cfg.tid);
...
// Assumes that ssn_idx is valid (!= 0xFFF)
    ...
    (ssn & 0xff) | (txq_id << 8));
    if (cfg) {
    let mut frame_limit: u8 = cfg.frame_limit;
...
#[no_mangle]
unsafe extern "C" fn iwl_trans_pcie_block_txq_ptrs(trans: *mut iwl_trans, block: bool) {
...
//
    ...
    void *dup_buf = core::ptr::null_mut();
    dma_addr_t phys_addr;
    int idx;
    u16 copy_size, cmd_size, tb0_size;
    let mut had_nocopy: bool = false;
    let mut group_id: u8 = iwl_cmd_groupid(cmd.id);
    int i, ret;
    u32 cmd_pos;
...
// need at least IWL_FIRST_TB_SIZE copied
    ...
    let mut copy: c_int = IWL_FIRST_TB_SIZE - copy_size;
    if (copy > cmdlen[i])
    copy = cmdlen[i];
    cmdlen[i] -= copy;
    cmddata[i] += copy;
...
//
    ...
    {
    struct sg_table *sgt;
    let mut n_segments: c_uint = skb_shinfo(skb).nr_frags + 1;
    int orig_nents;
...
pub const IWL_TX_CRC_SIZE: c_int = 4;
pub const IWL_TX_DELIMITER_SIZE: c_int = 4;
//
...
//
    ...
    let mut write_ptr: c_int = txq.write_ptr;
    let mut txq_id: c_int = txq.id;
    let mut sec_ctl: u8 = 0;
    let mut len: u16 = byte_cnt + IWL_TX_CRC_SIZE + IWL_TX_DELIMITER_SIZE;
    __le16 bc_ent;
    struct iwl_device_tx_cmd *dev_cmd = txq.entries[txq.write_ptr].cmd;
    struct iwl_tx_cmd_v6 *tx_cmd = (void *)dev_cmd.payload;
    let mut sta_id: u8 = tx_cmd.params.sta_id;
...
//
    ...
    txq.entries[read_ptr].skb = core::ptr::null_mut();
    if (!trans.mac_cfg.gen2)
...
#[no_mangle]
pub unsafe extern "C" fn iwl_pcie_set_q_ptrs(trans: *mut iwl_trans, txq_id: c_int, ptr: c_int) {
...
//
    ...
    IWL_ERR(trans, "Error: Response core::ptr::null_mut() in '%s'\n", cmd_str);
    ret = -EIO;
    goto cancel;
    }
    return 0;
...
//
    ...
    if (cmd.resp_pkt) {
    iwl_free_resp(cmd);
    cmd.resp_pkt = core::ptr::null_mut();
    }
    return ret;
    }
