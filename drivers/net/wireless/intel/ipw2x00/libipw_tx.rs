...
//! Source: drivers/net/wireless/intel/ipw2x00/libipw_tx.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn postfix(WEP: IV and ICV totalling 8 bytes in the case of) -> to add the prefix and {
...
#[no_mangle]
unsafe extern "C" fn libipw_copy_snap(data: *mut *mut u8, h_proto: __be16) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn libipw_txb_free(txb: *mut libipw_txb) {
...
#[no_mangle]
unsafe extern "C" fn libipw_classify(skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn libipw_xmit(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
    netdev_tx_t libipw_xmit(struct sk_buff *skb, struct net_device *dev)
    {
    struct libipw_device *ieee = netdev_priv(dev);
    struct libipw_txb *txb = core::ptr::null_mut();
    ...
    struct libipw_crypt_data *crypt;
    let mut priority: c_int = skb.priority;
    let mut snapped: c_int = 0;
...
// to insert the IV between the header and the payload
    ...
    dev_kfree_skb_any(skb);
    if (txb) {
    let mut ret: netdev_tx_t = (*ieee.hard_start_xmit)(txb, dev, priority);
    if (ret == NETDEV_TX_OK) {
