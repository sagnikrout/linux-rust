...
//! Source: drivers/net/wireless/intel/ipw2x00/libipw_wx.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn elapsed_jiffies_msecs(start: c_ulong) -> c_uint {
...
pub const MAX_CUSTOM_LEN: c_int = 64;
...
// Add quality statistics
    ...
    iwe.u.qual.qual = 100;
#[no_mangle]
pub unsafe extern "C" fn if(1: iwe.u.qual.qual <) -> else {
...
pub const SCAN_ITEM_SIZE: c_int = 128;
    ...
    let mut host_crypto: c_int = ieee.host_encrypt || ieee.host_decrypt;
...
// and if no key index was provided, de-init them all
    ...
    if (ieee.crypt_info.crypt[i] != core::ptr::null_mut()) {
    if (key_provided)
    break;
    ...
    if (*crypt != core::ptr::null_mut() && (*crypt).ops != core::ptr::null_mut() &&
    strcmp((*crypt).ops.name, "WEP") != 0) {
// changing to use WEP; deinit previously used algorithm
// on this key
    libipw_crypt_delayed_deinit(&ieee.crypt_info, crypt);
    }
    if (*crypt == core::ptr::null_mut() && host_crypto) {
    struct libipw_crypt_data *new_crypt;
// take WEP into use
    new_crypt = kzalloc_obj(struct libipw_crypt_data);
    if (new_crypt == core::ptr::null_mut())
    return -ENOMEM;
...
// If a new key was provided, set it up
    ...
    if (*crypt)
    (*crypt).ops.set_key(sec.keys[key], len, core::ptr::null_mut(),
    (*crypt).priv);
    sec.flags |= (1 << key);
...
// explicitly set
    ...
    core::ptr::null_mut(), (*crypt).priv);
    if (len == 0) {
// Set a default key of all 0
    LIBIPW_DEBUG_WX("Setting key %d to all "
    "zero.\n", key);
    memset(sec.keys[key], 0, 13);
    (*crypt).ops.set_key(sec.keys[key], 13, core::ptr::null_mut(),
    (*crypt).priv);
...
// some Cisco APs use idx>0 for unicast in dynamic WEP
    ...
    if (*crypt == core::ptr::null_mut() || (*crypt).ops != ops) {
    struct libipw_crypt_data *new_crypt;
    libipw_crypt_delayed_deinit(&ieee.crypt_info, crypt);
    ...
    if (new_crypt.priv == core::ptr::null_mut()) {
    kfree(new_crypt);
    ret = -EINVAL;
    goto done;
    }
// crypt = new_crypt;
    ...
    let mut key_len: c_int = clamp_val(ext.key_len, 0, SCM_KEY_LEN);
...
// Don't set sec level for group keys.
    ...
    ext.alg = IW_ENCODE_ALG_WEP;
#[no_mangle]
pub unsafe extern "C" fn if(SEC_ALG_TKIP: sec->encode_alg[idx] ==) -> else {
    else if (sec.encode_alg[idx] == SEC_ALG_TKIP)
    ext.alg = IW_ENCODE_ALG_TKIP;
#[no_mangle]
pub unsafe extern "C" fn if(SEC_ALG_CCMP: sec->encode_alg[idx] ==) -> else {
