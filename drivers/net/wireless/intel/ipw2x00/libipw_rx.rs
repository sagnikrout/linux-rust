...
//! Source: drivers/net/wireless/intel/ipw2x00/libipw_rx.c
#![no_std]
...
//
    ...
    let mut fc: u16 = le16_to_cpu(hdr.frame_control);
    skb.dev = ieee.dev;
...
// ieee,
    ...
    if (entry.skb != core::ptr::null_mut() &&
    ...
    if (entry.skb != core::ptr::null_mut() && entry.seq == seq &&
...
// Called only as a tasklet (software IRQ), by libipw_rx
    ...
    struct libipw_hdr_3addr *hdr;
    int res, hdrlen;
    if (crypt == core::ptr::null_mut() || crypt.ops.decrypt_mpdu == core::ptr::null_mut())
    return 0;
    hdr = (struct libipw_hdr_3addr *)skb.data;
...
// Called only as a tasklet (software IRQ), by libipw_rx
    ...
    struct libipw_hdr_3addr *hdr;
    int res, hdrlen;
    if (crypt == core::ptr::null_mut() || crypt.ops.decrypt_msdu == core::ptr::null_mut())
    return 0;
    hdr = (struct libipw_hdr_3addr *)skb.data;
    hdrlen = libipw_get_hdrlen(le16_to_cpu(hdr.frame_ctl));
    atomic_inc(&crypt.refcnt);
...
// This function is called only as a tasklet (software IRQ).
    ...
    struct net_device *wds = core::ptr::null_mut();
    struct sk_buff *skb2 = core::ptr::null_mut();
    struct net_device *wds = core::ptr::null_mut();
    let mut frame_authorized: c_int = 0;
    let mut from_assoc_ap: c_int = 0;
    void *sta = core::ptr::null_mut();

    u8 dst[ETH_ALEN];
    u8 src[ETH_ALEN];
    struct libipw_crypt_data *crypt = core::ptr::null_mut();
    let mut keyidx: c_int = 0;
    let mut can_be_decrypted: c_int = 0;
...
// for default encryption
    if (crypt && (crypt.ops == core::ptr::null_mut() ||
    crypt.ops.decrypt_mpdu == core::ptr::null_mut()))
    crypt = core::ptr::null_mut();
...
pub const MGMT_FRAME_FIXED_PART_LENGTH: c_uint = 0x24;
...
//
    ...
    {
    let mut size: usize = sizeof(*element_param);
...
//
    ...
    struct libipw_info_element *info_element)
    {
    let mut size: usize = sizeof(struct libipw_qos_information_element) - 2;
...
// info_element,
    ...
    {
    let mut rc: c_int = 0;
    struct libipw_qos_parameters *qos_param = core::ptr::null_mut();
...
// frame, struct libipw_rx_stats *stats)
    {
    let mut network_resp: libipw_network = { };
...
// for A band (No DS info)
    ...
    if (ieee.handle_assoc_response != core::ptr::null_mut())
...
#[no_mangle]
pub unsafe extern "C" fn is_beacon(fc: __le16) -> c_int {
...
// stats)
    {
    struct net_device *dev = ieee.dev;
    let mut network: libipw_network = { };
    struct libipw_network *target;
    struct libipw_network *oldest = core::ptr::null_mut();
...
// Otherwise just pull from the free list
    ...
    if (ieee.handle_beacon != core::ptr::null_mut())
    ieee.handle_beacon(dev, beacon, target);
    } else {
    if (ieee.handle_probe_response != core::ptr::null_mut())
    ...
    WLAN_FC_GET_STYPE(le16_to_cpu
    (header.frame_ctl)));
    if (ieee.handle_probe_request != core::ptr::null_mut())
    ...
    WLAN_FC_GET_STYPE(le16_to_cpu
    (header.frame_ctl)));
    if (ieee.handle_auth != core::ptr::null_mut())
    ieee.handle_auth(ieee.dev,
    (struct libipw_auth *)header);
    break;
    case IEEE80211_STYPE_DISASSOC:
    if (ieee.handle_disassoc != core::ptr::null_mut())
    ...
    ieee.dev.name);
    if (ieee.handle_reassoc_request != core::ptr::null_mut())
    ...
    ieee.dev.name);
    if (ieee.handle_assoc_request != core::ptr::null_mut())
    ieee.handle_assoc_request(ieee.dev);
    break;
    case IEEE80211_STYPE_DEAUTH:
    LIBIPW_DEBUG_MGMT("DEAUTH\n");
    if (ieee.handle_deauth != core::ptr::null_mut())
