...
//! Source: drivers/net/wireless/intersil/p54/txrx.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn p54_dump_tx_queue(priv: *mut p54_common) {
    ...
    let mut i: c_uint = 0;
    u32 prev_addr;
    let mut largest_hole: u32 = 0, free;
...
//
#[no_mangle]
unsafe extern "C" fn p54_assign_address(priv: *mut p54_common, skb: *mut sk_buff) -> c_int {
    static int p54_assign_address(struct p54_common *priv, struct sk_buff *skb)
    {
    struct sk_buff *entry, *target_skb = core::ptr::null_mut();
    struct ieee80211_tx_info *info;
    struct p54_tx_info *range;
    struct p54_hdr *data = (void *) skb.data;
    unsigned long flags;
    let mut last_addr: u32 = priv.rx_start;
    let mut target_addr: u32 = priv.rx_start;
    let mut len: u16 = priv.headroom + skb.len + priv.tailroom + 3;
    info = IEEE80211_SKB_CB(skb);
    range = (void *) info.rate_driver_data;
...
#[no_mangle]
unsafe extern "C" fn p54_tx_pending(priv: *mut p54_common) {
...
#[no_mangle]
unsafe extern "C" fn p54_wake_queues(priv: *mut p54_common) {
    ...
    queue.count++;
    if (unlikely(queue.len == queue.limit && IS_QOS_QUEUE(p54_queue))) {
    let mut ac_queue: u16 = p54_queue - P54_QUEUE_DATA;
...
#[no_mangle]
pub unsafe extern "C" fn p54_free_skb(dev: *mut ieee80211_hw, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn p54_tx(priv: *mut p54_common, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn p54_rssi_to_dbm(priv: *mut p54_common, rssi: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn p54_pspoll_workaround(priv: *mut p54_common, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn p54_rx_data(priv: *mut p54_common, skb: *mut sk_buff) -> c_int {
    static int p54_rx_data(struct p54_common *priv, struct sk_buff *skb)
    {
    struct p54_rx_data *hdr = (struct p54_rx_data *) skb.data;
    struct ieee80211_rx_status *rx_status = IEEE80211_SKB_RXCB(skb);
    let mut freq: u16 = le16_to_cpu(hdr.freq);
    let mut header_len: usize = sizeof(*hdr);
    u32 tsf32;
    __le16 fc;
    let mut rate: u8 = hdr.rate & 0xf;
//
...
#[no_mangle]
unsafe extern "C" fn p54_rx_frame_sent(priv: *mut p54_common, skb: *mut sk_buff) {
    ...
    struct sk_buff *entry;
    let mut pad: c_uint = 0, frame_len;
    int count, idx;
...
#[no_mangle]
unsafe extern "C" fn p54_rx_stats(priv: *mut p54_common, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn p54_rx_trap(priv: *mut p54_common, skb: *mut sk_buff) {
    static void p54_rx_trap(struct p54_common *priv, struct sk_buff *skb)
    {
    struct p54_hdr *hdr = (struct p54_hdr *) skb.data;
    struct p54_trap *trap = (struct p54_trap *) hdr.data;
    let mut event: u16 = le16_to_cpu(trap.event);
    let mut freq: u16 = le16_to_cpu(trap.frequency);
    switch (event) {
...
#[no_mangle]
unsafe extern "C" fn p54_rx_control(priv: *mut p54_common, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn p54_rx(dev: *mut ieee80211_hw, skb: *mut sk_buff) -> c_int {
    int p54_rx(struct ieee80211_hw *dev, struct sk_buff *skb)
    {
    struct p54_common *priv = dev.priv;
    let mut type: u16 = le16_to_cpu(*((__le16 *)skb.data));
...
#[no_mangle]
unsafe extern "C" fn p54_convert_algo(cipher: u32) -> u8 {
    ...
    int i, j, ridx;
    let mut hdr_flags: u16 = 0, aid = 0;
    u8 rate, queue = 0, crypt_offset = 0;
    let mut cts_rate: u8 = 0x20;
    u8 rc_flags;
    u8 calculated_tries[4];
    let mut nrates: u8 = 0, nremaining = 8;
    let mut burst_allowed: bool = false;
...
// if there are tries left, distribute from back to front
    ...
    let mut tmp: c_int = info.control.rates[i].count - calculated_tries[i];
    if (tmp <= 0)
    continue;
