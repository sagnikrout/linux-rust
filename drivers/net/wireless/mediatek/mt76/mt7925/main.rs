...
//! Source: drivers/net/wireless/mediatek/mt76/mt7925/main.c
#![no_std]
...
// Copyright (C) 2023 MediaTek Inc.
    ...
    let mut width: enum nl80211_chan_width = phy.mt76.chandef.width;
    let mut nss: c_int = hweight8(phy.mt76.antenna_mask);
    let mut sts: c_int = hweight16(phy.mt76.chainmask);
    u8 val;
    if (!phy.dev.has_eht)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_init_mlo_caps(phy: *mut mt792x_phy) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_set_stream_he_eht_caps(phy: *mut mt792x_phy) {
...
#[no_mangle]
pub unsafe extern "C" fn __mt7925_start(phy: *mut mt792x_phy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt7925_start(hw: *mut ieee80211_hw) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_roc_abort_sync(dev: *mut mt792x_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_roc_work(work: *mut work_struct) {
    ...
    let mut idx: c_int = key.keyidx, err = 0;
    struct mt792x_bss_conf *mconf;
    struct mt76_wcid *wcid;
    u8 *wcid_keyidx;
    link_conf = mt792x_vif_to_bss_conf(vif, link_id);
    link_sta = sta ? mt792x_sta_to_link_sta(vif, sta, link_id) : core::ptr::null_mut();
...
// wcid_keyidx = idx;
#[no_mangle]
pub unsafe extern "C" fn if(wcid_keyidx: *mut idx ==) -> else {
    else if (idx == *wcid_keyidx)
// wcid_keyidx = -1;
    else
    goto out;
    mt76_wcid_key_setup(&dev.mt76, wcid,
    cmd == SET_KEY ? key : core::ptr::null_mut());
...
//
    ...
    let mut pm_enable: bool = dev.pm.enable;
    int err;
    ...
    let mut monitor: bool = !!(hw.conf.flags & IEEE80211_CONF_MONITOR);
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_set_runtime_pm(dev: *mut mt792x_dev) {
...
#[no_mangle]
unsafe extern "C" fn mt7925_config(hw: *mut ieee80211_hw, radio_idx: c_int, changed: u32) -> c_int {
    ...
    ret = mt7925_set_tx_sar_pwr(hw, core::ptr::null_mut());
    if (ret)
    goto out;
    }
    ...
    struct mt792x_dev *dev = mt792x_hw_dev(hw);
    let mut flags: u32 = MT7925_FILTER_ENABLE;
...
// must odd index
    ...
    struct ieee80211_bss_conf *link_conf;
    struct mt792x_bss_conf *mconf;
    let mut link_id: u8 = link_sta.link_id;
    let mut wcid_published: bool = false;
    struct mt792x_sta *msta;
    struct mt76_wcid *wcid;
    let mut pm_woken: bool = false;
    int ret, idx;
...
// alternative lookup via def_wcid
    ...
    out_wcid:
    if (wcid_published) {
    let mut idx: u16 = wcid.idx;
    rcu_assign_pointer(dev.mt76.wcid[idx], core::ptr::null_mut());
...
//
    ...
    struct mt792x_link_sta *mlink;
    u16 idx;
    mlink = rcu_replace_pointer(msta.link[link_id], core::ptr::null_mut(),
    ...
    {
    struct mt792x_sta *msta = (struct mt792x_sta *)sta.drv_priv;
    let mut added_links: c_ulong = 0;
    unsigned int link_id;
    let mut err: c_int = 0;
    for_each_set_bit(link_id, &new_links, IEEE80211_MLD_MAX_NUM_LINKS) {
    struct ieee80211_link_sta *link_sta;
    struct mt792x_link_sta *mlink;
    let mut is_deflink: bool = false;
    ...
    let mut band: enum nl80211_band = chandef.chan.band, secondary_band;
    let mut sel_links: u16 = mt76_select_links(vif, 2);
    let mut secondary_link_id: u8 = __ffs(~BIT(mvif.deflink_id) & sel_links);
...
// clean up bss before starec
    ...
    continue;
    msta.valid_links &= ~BIT(link_id);
    mlink.sta = core::ptr::null_mut();
    mlink.pri_link = core::ptr::null_mut();
...
// Release NAN peer record before tearing down the STA.
    ...
    let mut ret: c_int = mt792x_nan_set_peer_rec(mdev, sta);
    if (ret)
    dev_err(mdev.dev,
...
// Release NDP context ID for NAN_DATA sta.
    ...
    if (vif.type == NL80211_IFTYPE_STATION) {
    mvif.wep_sta = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_mlo_pm_work(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_scan_work(work: *mut work_struct) {
    ...
    struct mt792x_dev *dev = mt792x_hw_dev(hw);
    struct mt792x_phy *phy = mt792x_hw_phy(hw);
    let mut max_nss: c_int = hweight8(hw.wiphy.available_antennas_tx);
...
#[no_mangle]
unsafe extern "C" fn mt7925_resume(hw: *mut ieee80211_hw) -> c_int {
    ...
    struct mt792x_vif *mvif = (struct mt792x_vif *)vif.drv_priv;
    struct mt792x_dev *dev = mt792x_hw_dev(hw);
    let mut valid: c_ulong = mvif.valid_links;
    u8 i;
    if (!msta.vif)
    return;
    mt792x_mutex_acquire(dev);
    ...
    skb = __mt76_mcu_msg_alloc(&dev.mt76, core::ptr::null_mut(), sizeof(req_hdr),
    0, GFP_ATOMIC);
    if (!skb)
    return;
    ...
    struct mt76_phy *mphy = hw.priv;
    if (sar) {
    let mut err: c_int = mt76_init_sar_power(hw, sar);
    if (err)
    return err;
    }
...
// firmware uses access class index
    ...
    link_conf, core::ptr::null_mut(), true);
    if (err)
    goto out;
    err = mt7925_mcu_set_bss_pm(dev, link_conf, true);
    if (err)
    goto out;
    err = mt7925_mcu_sta_update(dev, core::ptr::null_mut(), vif,
    ...
    mt7925_mcu_sta_update(dev, core::ptr::null_mut(), vif,
    &mvif.sta.deflink, true,
    MT76_STA_INFO_STATE_ASSOC);
...
// legacy
    ...
    let mut slottime: c_int = info.use_short_slot ? 9 : 20;
...
// Indicate the secondary setup done
    ...
    rcu_assign_pointer(mvif.link_conf[link_id], core::ptr::null_mut());
    rcu_assign_pointer(mvif.sta.link[link_id], core::ptr::null_mut());
    }
    for_each_set_bit(link_id, &add, IEEE80211_MLD_MAX_NUM_LINKS) {
    if (!old_links) {
    ...
    if (vif.type == NL80211_IFTYPE_STATION &&
    mconf == &mvif.bss_conf)
    mt7925_mcu_add_bss_info(&dev.phy, core::ptr::null_mut(), pri_link_conf,
    ...
    if (vif.type == NL80211_IFTYPE_STATION &&
    mconf == &mvif.bss_conf)
    mt7925_mcu_add_bss_info(&dev.phy, core::ptr::null_mut(), link_conf,
    core::ptr::null_mut(), false);
    } else {
    mconf = &mvif.bss_conf;
    }
    mctx.bss_conf = core::ptr::null_mut();
    mconf.mt76.ctx = core::ptr::null_mut();
    mutex_unlock(&dev.mt76.mutex);
...
#[no_mangle]
unsafe extern "C" fn mt7925_rfkill_poll(hw: *mut ieee80211_hw) {
...
#[no_mangle]
pub unsafe extern "C" fn mt7925_csa_work(work: *mut work_struct) {
    ...
    NL80211_CHAN_NO_HT);
    err = mt7925_mcu_add_bss_info(&dev.phy, core::ptr::null_mut(), link_conf,
    core::ptr::null_mut(), true);
    if (err < 0)
    goto out;
    dev.nan_vif = vif;
    ...
    rollback_bss:
    dev.nan_vif = core::ptr::null_mut();
    mt7925_mcu_add_bss_info(&dev.phy, core::ptr::null_mut(), link_conf, core::ptr::null_mut(), false);
    out:
    mt792x_mutex_release(dev);
    return err;
    }
    ...
    ret = mt7925_mcu_add_bss_info(&dev.phy, core::ptr::null_mut(), link_conf,
    core::ptr::null_mut(), false);
    if (!err)
    err = ret;
    if (dev.nan_vif == vif)
...
#[no_mangle]
unsafe extern "C" fn mt7925_stop(hw: *mut ieee80211_hw, suspend: bool) {
