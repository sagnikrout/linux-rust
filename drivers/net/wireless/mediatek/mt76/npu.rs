...
//! Source: drivers/net/wireless/mediatek/mt76/npu.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_fill_rx_queue(dev: *mut mt76_dev, q: *mut mt76_queue) -> c_int {
    int mt76_npu_fill_rx_queue(struct mt76_dev *dev, struct mt76_queue *q)
    {
    let mut nframes: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_queue_cleanup(dev: *mut mt76_dev, q: *mut mt76_queue) {
...
#[no_mangle]
unsafe extern "C" fn mt76_npu_rx_poll(napi: *mut napi_struct, budget: c_int) -> c_int {
    static int mt76_npu_rx_poll(struct napi_struct *napi, int budget)
    {
    struct mt76_dev *dev = mt76_priv(napi.dev);
    let mut qid: enum mt76_rxq_id = napi - dev.napi;
...
#[no_mangle]
unsafe extern "C" fn mt76_npu_irq_handler(irq: c_int, q_instance: *mut c_void) -> irqreturn_t {
    static irqreturn_t mt76_npu_irq_handler(int irq, void *q_instance)
    {
    struct mt76_queue *q = q_instance;
    struct mt76_dev *dev = q.dev;
    let mut qid: c_int = q - &dev.q_rx[0];
    let mut index: c_int = qid - MT_RXQ_NPU0;
    struct airoha_npu *npu;
    u32 status;
    rcu_read_lock();
    npu = rcu_dereference(dev.mmio.npu);
    ...
    void *txwi_ptr)
    {
    let mut txwi_len: u16 = min_t(u16, phy.dev.drv.txwi_size, NPU_TXWI_LEN);
...
// TODO: Take into account unlinear skbs
    ...
    q.entry[q.head].txwi = core::ptr::null_mut();
    q.entry[q.head].skb = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_txdesc_cleanup(q: *mut mt76_queue, index: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_queue_setup(dev: *mut mt76_dev, q: *mut mt76_queue) {
    void mt76_npu_queue_setup(struct mt76_dev *dev, struct mt76_queue *q)
    {
    let mut qid: c_int = FIELD_GET(MT_QFLAG_WED_RING, q.flags);
    let mut xmit: bool = mt76_queue_is_npu_tx(q);
    struct airoha_npu *npu;
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_rx_queue_init(dev: *mut mt76_dev, q: *mut mt76_queue) -> c_int {
    ...
    block_cb = flow_block_cb_alloc(cb, dev, phy, core::ptr::null_mut());
    if (IS_ERR(block_cb))
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_disable_irqs(dev: *mut mt76_dev) {
    ...
    let mut qid: c_int = i - MT_RXQ_NPU0;
    u32 status;
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_init(dev: *mut mt76_dev, phy_addr: phys_addr_t, type: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mt76_npu_deinit(dev: *mut mt76_dev) {
    void mt76_npu_deinit(struct mt76_dev *dev)
    {
    struct airoha_ppe_dev *ppe_dev;
    struct airoha_npu *npu;
    mutex_lock(&dev.mutex);
    npu = rcu_replace_pointer(dev.mmio.npu, core::ptr::null_mut(),
    lockdep_is_held(&dev.mutex));
    if (npu)
    airoha_npu_put(npu);
    ppe_dev = rcu_replace_pointer(dev.mmio.ppe_dev, core::ptr::null_mut(),
    lockdep_is_held(&dev.mutex));
    if (ppe_dev)
    airoha_ppe_put_dev(ppe_dev);
    mutex_unlock(&dev.mutex);
