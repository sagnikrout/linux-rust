...
//! Source: drivers/net/wireless/mediatek/mt76/mt7603/beacon.c
#![no_std]
...
pub struct beacon_bc_data {
    pub dev: *mut mt7603_dev,
    pub q: sk_buff_head,
    pub tail: [*mut sk_buff; MT7603_MAX_INTERFACES],
    pub count: [c_int; MT7603_MAX_INTERFACES],
}
    ...
    MT_TXQ_BEACON, skb, &mvif.sta.wcid, core::ptr::null_mut());
    out:
...
// Flush all previous CAB queue packets and beacons
    ...
    struct mt7603_vif *mvif = (struct mt7603_vif *)vif.drv_priv;
    mt76_tx_queue_skb(dev, q, MT_TXQ_CAB, skb, &mvif.sta.wcid, core::ptr::null_mut());
    }
    mt76_queue_kick(dev, q);
    spin_unlock(&q.lock);
...
#[no_mangle]
pub unsafe extern "C" fn mt7603_beacon_set_timer(dev: *mut mt7603_dev, idx: c_int, intval: c_int) {
    void mt7603_beacon_set_timer(struct mt7603_dev *dev, int idx, int intval)
    {
    let mut pre_tbtt: u32 = MT7603_PRE_TBTT_TIME / 64;
    if (idx >= 0) {
    if (intval)
