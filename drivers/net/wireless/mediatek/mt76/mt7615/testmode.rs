...
//! Source: drivers/net/wireless/mediatek/mt76/mt7615/testmode.c
#![no_std]
...
// must be last
    ...
    let mut freq: c_int = chandef.center_freq1, len, target_chains;
    u8 *data, *eep = (u8 *)dev.mt76.eeprom.data;
    let mut band: enum nl80211_band = chandef.chan.band;
    struct sk_buff *skb;
    struct {
    u8 center_chan;
    u8 dbdc_idx;
    ...
    u8 *tx_power = core::ptr::null_mut();
    if (mphy.test.state != MT76_TM_STATE_OFF)
    tx_power = mphy.test.tx_power;
    len = MT7615_EE_MAX - MT_EE_NIC_CONF_0;
    skb = mt76_mcu_msg_alloc(&dev.mt76, core::ptr::null_mut(), sizeof(req_hdr) + len);
    if (!skb)
    return -ENOMEM;
    ...
    let mut n_regs: c_int = ARRAY_SIZE(reg_backup_list);
    let mut n_rf_regs: c_int = ARRAY_SIZE(rf_backup_list);
    int i;
    ...
    let mut mask: u8 = td.tx_antenna_mask;
    int i;
    if (!mask)
    return;
    if (!en)
...
// 5 GHz band
    ...
    struct mt7615_dev *dev = phy.dev;
    struct mt76_testmode_data *td = &phy.mt76.test;
    let mut en: bool = phy.mt76.test.state != MT76_TM_STATE_OFF;
    ...
    let mut prev_state: enum mt76_testmode_state = td.state;
    mphy.test.state = state;
    if (prev_state == MT76_TM_STATE_TX_FRAMES)
    mt7615_tm_set_tx_frames(phy, false);
#[no_mangle]
pub unsafe extern "C" fn if(MT76_TM_STATE_TX_FRAMES: state ==) -> else {
    ...
    let mut cur: u16 = tm_change_map[i];
