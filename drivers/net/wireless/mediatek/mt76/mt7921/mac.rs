...
//! Source: drivers/net/wireless/mediatek/mt76/mt7921/mac.c
#![no_std]
...
pub const MT_WTBL_TXRX_CAP_RATE_OFFSET: c_int = 7;
pub const MT_WTBL_TXRX_RATE_G2_HE: c_int = 24;
pub const MT_WTBL_TXRX_RATE_G2: c_int = 12;
pub const MT_WTBL_AC0_CTT_OFFSET: c_int = 20;
#[no_mangle]
pub unsafe extern "C" fn mt7921_mac_wtbl_update(dev: *mut mt792x_dev, idx: c_int, mask: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mt7921_mac_wtbl_lmac_addr(idx: c_int, offset: u8) -> u32 {
    static u32 mt7921_mac_wtbl_lmac_addr(int idx, u8 offset)
    {
    return MT_WTBL_LMAC_OFFS(idx, 0) + offset * 4;
    }
#[no_mangle]
unsafe extern "C" fn mt7921_mac_sta_poll(dev: *mut mt792x_dev) {
    ...
    let mut tx_last: u32 = mlink.airtime_ac[i];
    let mut rx_last: u32 = mlink.airtime_ac[i + 4];
    ...
    drv_priv);
    for (i = 0; i < IEEE80211_NUM_ACS; i++) {
    let mut q: u8 = mt76_connac_lmac_mapping(i);
    let mut tx_cur: u32 = tx_time[q];
    let mut rx_cur: u32 = rx_time[q];
    let mut tid: u8 = ac_to_tid[i];
    if (!tx_cur && !rx_cur)
    continue;
...
//
    ...
    let mut offs: u8 = MT_WTBL_TXRX_RATE_G2_HE + 2 * bw;
...
// get signal strength of resp frames (CTS/BA/ACK)
    ...
    {
    let mut csum_mask: u32 = MT_RXD0_NORMAL_IP_SUM | MT_RXD0_NORMAL_UDP_TCP_SUM;
    struct mt76_rx_status *status = (struct mt76_rx_status *)skb.cb;
    bool hdr_trans, unicast, insert_ccmp_hdr = false;
    u8 chfreq, qos_ctl = 0, remove_pad, amsdu_info;
    u16 hdr_gap;
    __le32 *rxv = core::ptr::null_mut(), *rxd = (__le32 *)skb.data;
    struct mt76_phy *mphy = &dev.mt76.phy;
    struct mt792x_phy *phy = &dev.phy;
    struct ieee80211_supported_band *sband;
    let mut csum_status: u32 = *(u32 *)skb.cb;
    let mut rxd0: u32 = le32_to_cpu(rxd[0]);
    let mut rxd1: u32 = le32_to_cpu(rxd[1]);
    let mut rxd2: u32 = le32_to_cpu(rxd[2]);
    let mut rxd3: u32 = le32_to_cpu(rxd[3]);
    let mut rxd4: u32 = le32_to_cpu(rxd[4]);
    struct mt792x_sta *msta = core::ptr::null_mut();
    struct mt792x_link_sta *mlink;
...
// ICV error or CCMP/BIP/WPI MIC error
    ...
    let mut v0: u32 = le32_to_cpu(rxd[0]);
    let mut v2: u32 = le32_to_cpu(rxd[2]);
...
//
    ...
    if (insert_ccmp_hdr) {
    let mut key_id: u8 = FIELD_GET(MT_RXD1_NORMAL_KEY_ID, rxd1);
...
#[no_mangle]
pub unsafe extern "C" fn mt7921_mac_add_txs(dev: *mut mt792x_dev, data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn mt7921_mac_tx_free(dev: *mut mt792x_dev, data: *mut c_void, len: c_int) {
    ...
    struct mt76_wcid *wcid = core::ptr::null_mut();
...
//
    ...
    if (wcid) {
    let mut count: u32 = FIELD_GET(MT_TX_FREE_COUNT, info);
...
#[no_mangle]
pub unsafe extern "C" fn mt7921_rx_check(mdev: *mut mt76_dev, data: *mut c_void, len: c_int) -> bool {
    ...
    mt7921_mcu_sta_update(dev, core::ptr::null_mut(), vif, true,
...
#[no_mangle]
pub unsafe extern "C" fn mt7921_mac_reset_work(work: *mut work_struct) {
    ...
    mt7921_vif_connect_iter, core::ptr::null_mut());
    mt76_connac_power_save_sched(&dev.mt76.phy, pm);
    mt7921_regd_change(&dev.phy, "00");
    }
#[no_mangle]
pub unsafe extern "C" fn mt7921_coredump_work(work: *mut work_struct) {
...
// Release pktid in case of error.
    ...
    let mut headroom: c_uint = MT_SDIO_TXD_SIZE + MT_SDIO_HDR_SIZE;
    struct ieee80211_sta *sta;
    struct mt76_wcid *wcid;
    u16 idx;
...
#[no_mangle]
pub unsafe extern "C" fn mt7921_usb_sdio_tx_status_data(mdev: *mut mt76_dev, update: *mut u8) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn mt7921_set_ipv6_ns_work(work: *mut work_struct) {
