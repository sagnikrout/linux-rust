...
//! Source: drivers/net/wireless/mediatek/mt76/mt76_connac_mac.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn mt76_connac_gen_ppe_thresh(he_ppet: *mut u8, nss: c_int, band: enum nl80211_band) {
...
#[no_mangle]
pub unsafe extern "C" fn mt76_connac_pm_wake(phy: *mut mt76_phy, pm: *mut mt76_connac_pm) -> c_int {
    ...
    pm.tx_q[i].skb = core::ptr::null_mut();
    }
    ...
    for (i = 0; i < nbuf; i++) {
    let mut len: u16 = tx_info.buf[i + 1].len & MT_TXD_LEN_MASK;
    let mut addr: u32 = tx_info.buf[i + 1].addr;
    if (i == nbuf - 1)
    len |= last_mask;
    if (i & 1) {
...
#[no_mangle]
pub unsafe extern "C" fn mt76_connac_set_txpower_cur(phy: *mut mt76_phy, max_power: i8) {
    ...
    {
    let mut nss: u8 = 0, mode = 0, band = NL80211_BAND_2GHZ;
    let mut rateidx: c_int = 0, offset = 0, mcast_rate;
...
// pick the lowest rate for hidden nodes
    ...
    let mut multicast: bool = is_multicast_ether_addr(hdr.addr1);
    let mut tid: u8 = skb.priority & IEEE80211_QOS_CTL_TID_MASK;
    let mut fc: __le16 = hdr.frame_control;
    let mut sc: __le16 = hdr.seq_ctrl;
    u8 fc_type, fc_stype;
    u32 val;
    ...
    let mut capab: u16 = le16_to_cpu(mgmt.u.action.addba_req.capab);
...
#[no_mangle]
pub unsafe extern "C" fn if(!ieee80211_is_first_frag(sc): ieee80211_has_morefrags(fc) &&) -> else {
...
pub unsafe extern "C" fn if(!ieee80211_is_first_frag(sc): !ieee80211_has_morefrags(fc) &&) -> else {
    ...
    let mut seqno: u16 = le16_to_cpu(sc);
    ...
    {
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    let mut phy_idx: u8 = (info.hw_queue & MT_TX_HW_QUEUE_PHY) >> 2;
    struct ieee80211_vif *vif = info.control.vif;
    struct mt76_phy *mphy = &dev.phy;
    u8 p_fmt, q_idx, omac_idx = 0, wmm_idx = 0, band_idx = 0;
    u32 val, sz_txd = mt76_is_mmio(dev) ? MT_TXD_SIZE : MT_SDIO_TXD_SIZE;
    let mut is_8023: bool = info.flags & IEEE80211_TX_CTL_HW_80211_ENCAP;
    bool beacon = !!(changed & (BSS_CHANGED_BEACON |
    BSS_CHANGED_BEACON_ENABLED));
    bool inband_disc = !!(changed & (BSS_CHANGED_UNSOL_BCAST_PROBE_RESP |
    BSS_CHANGED_FILS_DISCOVERY));
    let mut amsdu_en: bool = wcid.amsdu;
    if (vif) {
...
// Fixed rata is available just for 802.11 txd
    ...
    vif ? &vif.bss_conf : core::ptr::null_mut(),
    beacon, multicast);
    let mut val: u32 = MT_TXD6_FIXED_BW;
// hardware won't add HTC for mgmt/ctrl frame
    ...
    if (!is_connac2(dev)) {
    let mut spe_idx: u8 = mt76_connac_spe_idx(mphy.antenna_mask);
    if (!spe_idx)
    spe_idx = 24 + phy_idx;
    ...
    let mut rate: rate_info = {};
    let mut cck: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn if(NL80211_BAND_6GHZ: mphy->chandef.chan->band ==) -> else {
    ...
    let mut ltf_size: u32 = le32_get_bits(rxv[2], MT_CRXV_HE_LTF_SIZE) + 1;
    struct ieee80211_radiotap_he *he;
...
#[no_mangle]
pub unsafe extern "C" fn if(ETH_P_802_3_MIN: be16_to_cpu(eth_hdr->h_proto) >=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn mt76_connac2_tx_check_aggr(sta: *mut ieee80211_sta, txwi: *mut __le32) {
    ...
    out:
    t.skb = core::ptr::null_mut();
    mt76_put_txwi(dev, t);
    }
    EXPORT_SYMBOL_GPL(mt76_connac2_txwi_free);
#[no_mangle]
pub unsafe extern "C" fn mt76_connac2_tx_token_put(dev: *mut mt76_dev) {
    ...
    mt76_connac2_txwi_free(dev, txwi, core::ptr::null_mut(), core::ptr::null_mut());
    dev.token_count--;
    }
