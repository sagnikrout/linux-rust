...
//! Source: drivers/net/wireless/mediatek/mt76/agg-rx.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mt76_aggr_tid_to_timeo(tidno: u8) -> c_ulong {
...
// for non AC_BK/AC_BE and set smaller timeout for it.
    ...
    if (!skb)
    return;
    tid.reorder_buf[idx] = core::ptr::null_mut();
    tid.nframes--;
    ...
    {
    let mut idx: c_int = tid.head % tid.size;
    ...
    mt76_aggr_tid_to_timeo(tid.num));
    mt76_rx_complete(dev, &frames, core::ptr::null_mut());
    rcu_read_unlock();
    local_bh_enable();
    }
    static void
...
#[no_mangle]
pub unsafe extern "C" fn mt76_rx_aggr_reorder(skb: *mut sk_buff, frames: *mut sk_buff_head) {
...
#[no_mangle]
unsafe extern "C" fn mt76_rx_aggr_shutdown(dev: *mut mt76_dev, tid: *mut mt76_rx_tid) {
    static void mt76_rx_aggr_shutdown(struct mt76_dev *dev, struct mt76_rx_tid *tid)
    {
    let mut size: u16 = tid.size;
    int i;
    spin_lock_bh(&tid.lock);
    tid.stopped = true;
    ...
    if (!skb)
    continue;
    tid.reorder_buf[i] = core::ptr::null_mut();
    tid.nframes--;
    dev_kfree_skb(skb);
    }
    spin_unlock_bh(&tid.lock);
...
#[no_mangle]
pub unsafe extern "C" fn mt76_rx_aggr_stop(dev: *mut mt76_dev, wcid: *mut mt76_wcid, tidno: u8) {
    void mt76_rx_aggr_stop(struct mt76_dev *dev, struct mt76_wcid *wcid, u8 tidno)
    {
    struct mt76_rx_tid *tid = core::ptr::null_mut();
