...
//! Source: drivers/net/wireless/mediatek/mt76/mt7915/main.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mt7915_dev_running(dev: *mut mt7915_dev) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn mt7915_run(hw: *mut ieee80211_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt7915_start(hw: *mut ieee80211_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt7915_stop(hw: *mut ieee80211_hw, suspend: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn get_free_idx(mask: u32, start: u8, end: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt7915_init_bitrate_mask(vif: *mut ieee80211_vif) {
    ...
    let mut ext_phy: bool = phy != &dev.phy;
    int idx, ret = 0;
    mutex_lock(&dev.mt76.mutex);
    mt76_testmode_reset(phy.mt76, true);
    ...
    mt7915_mcu_add_sta(dev, vif, core::ptr::null_mut(), CONN_STATE_PORT_SECURE, true);
    ...
    phy.monitor_vif = core::ptr::null_mut();
    mutex_unlock(&dev.mt76.mutex);
    return ret;
    }
    ...
    struct mt7915_dev *dev = mt7915_hw_dev(hw);
    struct mt7915_phy *phy = mt7915_hw_phy(hw);
    let mut idx: c_int = msta.wcid.idx;
    mt7915_mcu_add_bss_info(phy, vif, false);
    mt7915_mcu_add_sta(dev, vif, core::ptr::null_mut(), CONN_STATE_DISCONNECT, false);
    mutex_lock(&dev.mt76.mutex);
    mt76_testmode_reset(phy.mt76, true);
    mutex_unlock(&dev.mt76.mutex);
...
#[no_mangle]
pub unsafe extern "C" fn mt7915_set_channel(mphy: *mut mt76_phy) -> c_int {
...
// wcid_keyidx = -1;
    ...
    let mut enabled: bool = !!(hw.conf.flags & IEEE80211_CONF_MONITOR);
    let mut band: bool = phy.mt76.band_idx;
    let mut rxfilter: u32 = phy.rxfilter;
    if (!enabled) {
...
// total_flags = flags;
    ...
    struct mt7915_phy *phy = mt7915_hw_phy(hw);
    struct mt7915_dev *dev = mt7915_hw_dev(hw);
    let mut set_bss_info: c_int = -1, set_sta = -1;
    mutex_lock(&dev.mt76.mutex);
//
...
//
    ...
    mt7915_mcu_add_bss_info(phy, vif, true);
    if (set_sta == 1)
    mt7915_mcu_add_sta(dev, vif, core::ptr::null_mut(), CONN_STATE_PORT_SECURE, false);
    if (changed & BSS_CHANGED_HT || changed & BSS_CHANGED_ERP_CTS_PROT)
    mt7915_mcu_set_protection(phy, vif, info.ht_operation_mode,
    info.use_cts_prot);
    if (changed & BSS_CHANGED_ERP_SLOT) {
    let mut slottime: c_int = 9;
...
// ensure that enable txcmd_mode after bss_info
    ...
    if (err)
    goto out;
    err = mt7915_mcu_add_sta(dev, vif, core::ptr::null_mut(), CONN_STATE_PORT_SECURE, false);
    out:
    mutex_unlock(&dev.mt76.mutex);
    return err;
    }
    ...
    struct mt7915_vif *mvif = (struct mt7915_vif *)vif.drv_priv;
    let mut ext_phy: bool = mvif.phy != &dev.phy;
    int idx;
...
#[no_mangle]
pub unsafe extern "C" fn __mt7915_get_tsf(hw: *mut ieee80211_hw, mvif: *mut mt7915_vif) -> u64 {
...
// TSF software adjust
    ...
    let mut shift: u8 = mt7915_band_chainshift(phy);
    let mut band: u8 = phy.mt76.band_idx;
...
#[no_mangle]
unsafe extern "C" fn mt7915_sta_rc_work(data: *mut c_void, sta: *mut ieee80211_sta) {
...
//
    ...
    struct mt7915_phy *phy = mt7915_hw_phy(hw);
    struct mt7915_dev *dev = mt7915_hw_dev(hw);
    let mut txpower: i16 = sta.deflink.txpwr.power;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn mt7915_ethtool_worker(wi_data: *mut c_void, sta: *mut ieee80211_sta) {
...
// rdd2 already configured on a radar channel
    ...
    ret = mt7915_mcu_rdd_background_enable(phy, core::ptr::null_mut());
    if (ret)
    goto out;
    if (!running)
    goto update_phy;
