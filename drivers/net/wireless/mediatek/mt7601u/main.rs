...
//! Source: drivers/net/wireless/mediatek/mt7601u/main.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mt7601u_start(hw: *mut ieee80211_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mt7601u_stop(hw: *mut ieee80211_hw, suspend: bool) {
    ...
    let mut wcid: c_uint = GROUP_WCID(idx);
...
//
    ...
    struct mt7601u_dev *dev = hw.priv;
    struct mt76_vif *mvif = (struct mt76_vif *) vif.drv_priv;
    let mut wcid: c_uint = mvif.group_wcid.idx;
    dev.wcid_mask[wcid / BITS_PER_LONG] &= ~BIT(wcid % BITS_PER_LONG);
    }
#[no_mangle]
unsafe extern "C" fn mt7601u_config(hw: *mut ieee80211_hw, radio_idx: c_int, changed: u32) -> c_int {
...
//
    ...
    mutex_lock(&dev.mutex);
    rcu_assign_pointer(dev.wcid[idx], core::ptr::null_mut());
    mt76_set(dev, MT_WCID_DROP(idx), MT_WCID_DROP_MASK(idx));
    dev.wcid_mask[idx / BITS_PER_LONG] &= ~BIT(idx % BITS_PER_LONG);
    mt7601u_mac_wcid_setup(dev, idx, 0, core::ptr::null_mut());
    mt7601u_mac_set_ampdu_factor(dev);
    mutex_unlock(&dev.mutex);
    return 0;
    }
    ...
    struct mt7601u_dev *dev = hw.priv;
    struct mt76_vif *mvif = (struct mt76_vif *) vif.drv_priv;
    struct mt76_sta *msta = sta ? (struct mt76_sta *) sta.drv_priv : core::ptr::null_mut();
