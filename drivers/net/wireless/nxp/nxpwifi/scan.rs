...
//! Source: drivers/net/wireless/nxp/nxpwifi/scan.c
#![no_std]
...
pub const NXPWIFI_MAX_CHANNELS_PER_SPECIFIC_SCAN: c_int = 14;
pub const NXPWIFI_DEF_CHANNELS_PER_SCAN_CMD: c_int = 4;
...
// Check whether the RSN IE is present and if its PTK list contains the OUI
    ...
    let mut ret: u8 = NXPWIFI_OUI_NOT_PRESENT;
    let mut oui: __be32 = cpu_to_be32(cipher);
...
// scan_chan_list)
    ...
    struct nxpwifi_adapter *adapter = priv.adapter;
    let mut chan_idx: c_int = 0, i;
    let mut scan_time: u16 = 0, specific_scan_time = adapter.specific_scan_time;
    if (bgscan_cfg_in)
    ...
    cpu_to_le16(scan_time);
#[no_mangle]
pub unsafe extern "C" fn if(IEEE80211_CHAN_NO_IR: ch->flags &) -> else {
    else if (ch.flags & IEEE80211_CHAN_NO_IR)
    scan_chan_list[chan_idx].max_scan_time =
    cpu_to_le16(adapter.passive_scan_time);
    else
...
//
    ...
    u8 *ext_capab = core::ptr::null_mut();
    u8 *tlv_pos;
    u32 num_probes;
    u32 ssid_len;
    u32 chan_idx;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: *mut filtered_scan) -> else {
...
// Parse the beacon buffer and update the BSS descriptor fields.
    ...
    {
    u8 element_id;
    let mut elem_size: u16 = sizeof(struct element);
...
#[no_mangle]
unsafe extern "C" fn nxpwifi_is_hidden_ssid(ssid: *mut cfg80211_ssid) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nxpwifi_complete_scan(priv: *mut nxpwifi_private) {
...
#[no_mangle]
unsafe extern "C" fn nxpwifi_check_next_scan_command(priv: *mut nxpwifi_private) {
...
#[no_mangle]
pub unsafe extern "C" fn nxpwifi_cancel_scan(adapter: *mut nxpwifi_adapter) {
...
//
    ...
    u8 is_bgscan_resp;
    let mut fw_tsf: __le64 = 0;
    const u8 *radio_type;
    struct cfg80211_wowlan_nd_match *pmatch;
    struct cfg80211_sched_scan_request *nd_config = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn nxpwifi_stop_bg_scan(priv: *mut nxpwifi_private) -> c_int {
...
// Handle the extended scan command response.
    ...
    struct cmd_ctrl_node *cmd_node;
    let mut complete_scan: bool = false;
...
//
    ...
    u8 *radio_type;
    let mut fw_tsf: u64 = 0;
    let mut rssi: i32 = 0;
    struct nxpwifi_event_scan_result *event_scan = buf;
    let mut num_of_set: u8 = event_scan.num_of_set;
    u8 *scan_resp = buf + sizeof(struct nxpwifi_event_scan_result);
    let mut scan_resp_size: u16 = le16_to_cpu(event_scan.buf_size);
    ...
    scan_rsp_tlv = core::ptr::null_mut();
    scan_info_tlv = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn nxpwifi_cmd_802_11_bg_scan_query(cmd: *mut host_cmd_ds_command) -> c_int {
...
// Free the beacon buffer in the current BSS descriptor.
    ...
    {
    kfree(priv.curr_bcn_buf);
    priv.curr_bcn_buf = core::ptr::null_mut();
    }
