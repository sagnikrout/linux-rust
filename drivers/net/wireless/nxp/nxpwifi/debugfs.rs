...
//! Source: drivers/net/wireless/nxp/nxpwifi/debugfs.c
#![no_std]
...
//
    ...
    let mut page: c_ulong = get_zeroed_page(GFP_KERNEL);
...
//
    ...
    char *buf;
    int ret;
    let mut reg_type: u32 = 0, reg_offset = 0, reg_value = UINT_MAX;
    int rv;
...
//
    ...
    struct nxpwifi_private *priv =
    (struct nxpwifi_private *)file.private_data;
    let mut addr: c_ulong = get_zeroed_page(GFP_KERNEL);
    char *buf = (char *)addr;
    let mut pos: c_int = 0, ret = 0;
    u32 reg_value;
    if (!buf)
    return -ENOMEM;
    if (!saved_reg_type) {
...
// debugfs "memrw" read handler: show last memory access result.
    ...
    let mut saved_offset: static u32 = -1, saved_bytes = -1;
// debugfs "rdeeprom" write handler: set EEPROM offset/length to read.
    ...
    char *buf;
    let mut ret: c_int = 0;
    let mut offset: c_int = -1, bytes = -1;
    int rv;
    buf = memdup_user_nul(ubuf, min(count, (size_t)(PAGE_SIZE - 1)));
    if (IS_ERR(buf))
    return PTR_ERR(buf);
...
//
    ...
    let mut conditions: c_int = HS_CFG_COND_DEF;
    let mut gpio: u32 = HS_CFG_GPIO_DEF, gap = HS_CFG_GAP_DEF;
    buf = memdup_user_nul(ubuf, min(count, (size_t)(PAGE_SIZE - 1)));
    if (IS_ERR(buf))
    return PTR_ERR(buf);
...
// Create top-level debugfs directory.
    ...
    {
    if (!nxpwifi_dfs_dir)
    nxpwifi_dfs_dir = debugfs_create_dir("nxpwifi", core::ptr::null_mut());
    }
