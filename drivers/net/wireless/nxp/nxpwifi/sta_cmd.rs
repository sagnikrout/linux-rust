...
//! Source: drivers/net/wireless/nxp/nxpwifi/sta_cmd.c
#![no_std]
...
// If it's unidentified region code, use the default (USA/FCC)
    ...
    let mut oid: u16 = le16_to_cpu(smib.oid);
    let mut query_type: u16 = le16_to_cpu(smib.query_type);
    u32 ul_temp;
...
// (u32 *)data_buf = ul_temp;
    ...
    let mut action: u16 = le16_to_cpu(txp.action);
...
// Set AP MAC address
    ...
    let mut no_of_triplet: u8 = adapter.domain_reg.no_of_triplet;
    int triplet_size;
    nxpwifi_dbg(adapter, INFO,
    ...
    let mut action: u16 = le16_to_cpu(domain_info.action);
    u8 no_of_triplet;
...
#[no_mangle]
unsafe extern "C" fn nxpwifi_parse_cal_cfg(src: *mut u8, len: usize, dst: *mut u8) -> u32 {
...
// buffer = stack_ptr;
    ...
    struct nxpwifi_fw_mef_entry *mef_entry = core::ptr::null_mut();
    u8 *pos = (u8 *)mef_cfg;
    u16 i;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn nxpwifi_get_power_level(priv: *mut nxpwifi_private, data_buf: *mut c_void) -> c_int {
...
// At least one structure required to update power
    ...
    struct nxpwifi_types_power_group *pg_tlv_hdr;
    struct nxpwifi_power_group *pg;
    let mut action: u16 = le16_to_cpu(txp_cfg.action);
    u16 tlv_buf_left;
...
#[no_mangle]
unsafe extern "C" fn nxpwifi_ret_rate_scope(priv: *mut nxpwifi_private, tlv_buf: *mut u8) {
...
// Add RATE_DROP tlv here
    ...
    HOST_ACT_GEN_GET, 0, core::ptr::null_mut(), false);
    return 0;
    }
    static int
    ...
    bool *is_timeshare = (bool *)data_buf;
    struct nxpwifi_ie_types_robust_coex *coex_tlv;
    let mut action: u16 = le16_to_cpu(coex.action);
    u32 mode;
    if (!is_timeshare)
    return 0;
    coex_tlv = (struct nxpwifi_ie_types_robust_coex
...
// is_timeshare = false;
    ...
    let mut ps_bitmap: u16 = (u16)cmd_type;
    struct nxpwifi_ds_auto_ds *auto_ds =
...
// This section is for get power save mode
    ...
    struct nxpwifi_ps_param_in_hs *psparam_tlv = core::ptr::null_mut();
    let mut hs_activate: bool = false;
    u16 size;
    if (!hscfg_param)
...
#[no_mangle]
pub unsafe extern "C" fn if(NL80211_IFTYPE_AP: priv->bss_mode ==) -> else {
    ...
    &cmd.params.net_mon;
    struct chan_band_param *chan_band = core::ptr::null_mut();
    let mut sec_chan_offset: u8 = 0;
    let mut bw_offset: u32 = 0;
    ...
    NXPWIFI_SEC_CHAN_ABOVE;
#[no_mangle]
pub unsafe extern "C" fn if(NXPWIFI_SEC_CHAN_BELOW: sec_chan_offset ==) -> else {
...
// Add sizeof action, num_of_rules to total command length
    ...
    u8 *buf, u16 buf_len)
    {
    let mut num_chan: u16 = buf_len / 2;
    ...
    bw = MHZ_TO_KHZ(80);
#[no_mangle]
pub unsafe extern "C" fn if(NXPWIFI_CHANNEL_NOHT40): !(chflags &) -> else {
    ...
    let mut action: u16 = le16_to_cpu(reg.action);
    u16 tlv, tlv_buf_len, tlv_buf_left;
    struct nxpwifi_ie_types_header *head;
    struct ieee80211_regdomain *regd;
    u8 *tlv_buf;
...
//
    ...
    {
    struct nxpwifi_adapter *adapter = priv.adapter;
    let mut cmd_no: u16 = cmd_node.cmd_no;
    struct host_cmd_ds_command *cmd =
...
//
#[no_mangle]
pub unsafe extern "C" fn nxpwifi_sta_init_cmd(priv: *mut nxpwifi_private, first_sta: u8, init: bool) -> c_int {
    ...
    HOST_ACT_GEN_SET, 0, core::ptr::null_mut(), true);
    if (ret)
    return ret;
    if (adapter.cal_data)
...
// Read MAC address from HW
    ...
    HOST_ACT_GEN_GET, 0, core::ptr::null_mut(), true);
    if (ret)
    return ret;
...
// Enable IEEE PS by default
    ...
    EN_AUTO_PS, BITMAP_STA_PS, core::ptr::null_mut(),
    true);
    if (ret)
    return ret;
    }
