...
//! Source: drivers/net/wireless/nxp/nxpwifi/uap_cmd.c
#![no_std]
...
// Parse BSS params and append WPA/WPA2 TLVs to the command buffer.
    ...
    let mut cmd_size: u16 = *param_size;
    u8 *tlv = *tlv_buf;
...
#[no_mangle]
unsafe extern "C" fn nxpwifi_uap_custom_ie_prepare(tlv: *mut u8, cmd_buf: *mut c_void, ie_size: *mut u16) -> c_int {
...
// Build AP add-station command.
    ...
    let mut len: u16 = params.ext_capab_len;
    ...
    let mut len: u16 = params.link_sta_params.supported_rates_len;
    ...
    let mut qos_capability: u8 = params.uapsd_queues | (params.max_sp << 5);
    u8 *data = &qos_capability;
    let mut len: u16 = sizeof(u8);
    ...
    let mut len: u16 = sizeof(struct ieee80211_ht_cap);
    ...
    let mut len: u16 = sizeof(struct ieee80211_vht_cap);
    ...
    let mut len: u16 = params.link_sta_params.he_capa_len;
...
// Enable VHT only when VHT IE is present; otherwise disable VHT.
    ...
    {
    struct nxpwifi_adapter *adapter = priv.adapter;
    let mut band: u8 = bss_cfg.band_cfg & BAND_CFG_CHAN_BAND_MASK;
    if (band == BAND_2GHZ &&
    ...
    ax_cfg.band = BIT(0);
#[no_mangle]
pub unsafe extern "C" fn if(BAND_5GHZ: band ==) -> else {
...
// Copy supported rates from beacon into bss_config.
    ...
    let mut var_offset: c_int = offsetof(struct ieee80211_mgmt, u.beacon.variable);
    const u8 *var_pos = params.beacon.head + var_offset;
    let mut len: c_int = params.beacon.head_len - var_offset;
    let mut rate_len: u8 = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn nxpwifi_set_sys_config_invalid_data(config: *mut nxpwifi_uap_bss_param) {
...
// Send cmd to FW to enable 11D function
    ...
    {
    let mut config_bands: u8 = 0, old_bands = priv.config_bands;
