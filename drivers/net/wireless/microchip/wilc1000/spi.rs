...
//! Source: drivers/net/wireless/microchip/wilc1000/spi.c
#![no_std]
...
// operates at 1MHz.
//
pub const WILC_SPI_RSP_HDR_EXTRA_DATA: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wilc_spi {
    pub /: *mut *mut bool isinit; / true if wilc_spi_init was successful,
    pub /: *mut *mut bool probing_crc; / true if we're probing chip's CRC config,
    pub /: *mut *mut bool crc7_enabled; / true if crc7 is currently enabled,
    pub /: *mut *mut bool crc16_enabled; / true if crc16 is currently enabled,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wilc_gpios {
    pub /: *mut *mut *mut gpio_desc enable; / ENABLE GPIO or NULL,
    pub /: *mut *mut *mut gpio_desc reset; / RESET GPIO or NULL,
    pub gpios: },
}
...
//
pub const CMD_DMA_WRITE: c_uint = 0xc1;
pub const CMD_DMA_READ: c_uint = 0xc2;
pub const CMD_INTERNAL_WRITE: c_uint = 0xc3;
pub const CMD_INTERNAL_READ: c_uint = 0xc4;
pub const CMD_TERMINATE: c_uint = 0xc5;
pub const CMD_REPEAT: c_uint = 0xc6;
pub const CMD_DMA_EXT_WRITE: c_uint = 0xc7;
pub const CMD_DMA_EXT_READ: c_uint = 0xc8;
pub const CMD_SINGLE_WRITE: c_uint = 0xc9;
pub const CMD_SINGLE_READ: c_uint = 0xca;
pub const CMD_RESET: c_uint = 0xcf;
pub const SPI_RETRY_MAX_LIMIT: c_int = 10;
pub const SPI_ENABLE_VMM_RETRY_LIMIT: c_int = 2;
// SPI response fields (section 11.1.2 in ATWILC1000 User Guide):

// SPI response values for the response fields:
pub const RSP_START_TAG: c_uint = 0xc;
pub const RSP_TYPE_FIRST_PACKET: c_uint = 0x1;
pub const RSP_TYPE_INNER_PACKET: c_uint = 0x2;
pub const RSP_TYPE_LAST_PACKET: c_uint = 0x3;
pub const RSP_STATE_NO_ERROR: c_uint = 0x00;
...
pub const WILC_SPI_COMMAND_STAT_SUCCESS: c_int = 0;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct wilc_spi_cmd {
    pub cmd_type: u8,
    union {
    struct {
    pub addr: [u8; 3],
    pub crc: [u8; ],
    pub simple_cmd: } __packed,
    struct {
    pub addr: [u8; 3],
    pub size: [u8; 2],
    pub crc: [u8; ],
    pub dma_cmd: } __packed,
    struct {
    pub addr: [u8; 3],
    pub size: [u8; 3],
    pub crc: [u8; ],
    pub dma_cmd_ext: } __packed,
    struct {
    pub addr: [u8; 2],
    pub data: __be32,
    pub crc: [u8; ],
    pub internal_w_cmd: } __packed,
    struct {
    pub addr: [u8; 3],
    pub data: __be32,
    pub crc: [u8; ],
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wilc_spi_read_rsp_data {
    pub header: u8,
    pub data: [u8; 4],
    pub crc: [u8; ],
    pub __packed: },
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wilc_spi_special_cmd_rsp {
    pub skip_byte: u8,
    pub rsp_cmd_type: u8,
    pub status: u8,
    pub __packed: },
#[no_mangle]
unsafe extern "C" fn wilc_parse_gpios(wilc: *mut wilc) -> c_int {
    static int wilc_parse_gpios(struct wilc *wilc)
    {
    pub to_spi_device(wilc->dev): *mut *mut spi_device spi =,
    pub wilc->bus_data: *mut *mut wilc_spi spi_priv =,
    pub &spi_priv->gpios: *mut *mut wilc_gpios gpios =,
// get ENABLE pin and deassert it (if it is defined):
    gpios.enable = devm_gpiod_get_optional(&spi.dev,
    pub GPIOD_OUT_LOW): "enable",,
...
// if enable pin exists, reset must exist as well
    ...
    pub GPIOD_OUT_HIGH): "reset",,
    if (IS_ERR(gpios.reset)) {
    pub gpio.\n"): dev_err(&spi->dev, "missing reset,
    pub PTR_ERR(gpios->reset): return,
    }
    } else {
...
#[no_mangle]
unsafe extern "C" fn wilc_wlan_power(wilc: *mut wilc, on: bool) {
    ...
    if (on) {
// assert ENABLE:
    pub 1): gpiod_set_value(gpios->enable,,
// deassert RESET:
    pub 0): gpiod_set_value(gpios->reset,,
    } else {
// assert RESET:
    pub 1): gpiod_set_value(gpios->reset,,
// deassert ENABLE:
    pub 0): gpiod_set_value(gpios->enable,,
    }
    }
#[no_mangle]
unsafe extern "C" fn wilc_bus_probe(spi: *mut spi_device) -> c_int {
    static int wilc_bus_probe(struct spi_device *spi)
    {
    pub spi_priv: *mut wilc_spi,
    pub vif: *mut wilc_vif,
    pub wilc: *mut wilc,
    pub ret: c_int,
    pub kzalloc_obj(*spi_priv): *mut spi_priv =,
    if (!spi_priv)
    pub -ENOMEM: return,
    pub &wilc_hif_spi): ret = wilc_cfg80211_init(&wilc, &spi->dev, WILC_HIF_SPI,,
    if (ret)
    pub free: goto,
    pub wilc): spi_set_drvdata(spi,,
    pub &spi->dev: wilc->dev =,
    pub spi_priv: wilc->bus_data =,
    pub spi->irq: wilc->dev_irq_num =,
    pub wilc_parse_gpios(wilc): ret =,
    if (ret < 0)
    pub netdev_cleanup: goto,
    pub "rtc"): wilc->rtc_clk = devm_clk_get_optional_enabled(&spi->dev,,
    if (IS_ERR(wilc.rtc_clk)) {
    pub PTR_ERR(wilc->rtc_clk): ret =,
    pub netdev_cleanup: goto,
    }
    dev_info(&spi.dev, "Selected CRC config: crc7=%s, crc16=%s\n",
    pub "off"): enable_crc7 ? "on" : "off", enable_crc16 ? "on" :,
// we need power to configure the bus protocol and to read the chip id:
    pub true): wilc_wlan_power(wilc,,
    pub wilc_spi_configure_bus_protocol(wilc): ret =,
    if (ret)
    pub power_down: goto,
    pub wilc_get_chipid(wilc): ret =,
    if (ret)
    pub power_down: goto,
    pub wilc_cfg80211_register(wilc): ret =,
    if (ret)
    pub power_down: goto,
    pub wilc_load_mac_from_nv(wilc): ret =,
    if (ret) {
    pub chip\n"): pr_err("Can not retrieve MAC address from,
    pub unregister_wiphy: goto,
    }
    pub false): wilc_wlan_power(wilc,,
    vif = wilc_netdev_ifc_init(wilc, "wlan%d", WILC_STATION_MODE,
    pub false): NL80211_IFTYPE_STATION,,
    if (IS_ERR(vif)) {
    pub PTR_ERR(vif): ret =,
    pub unregister_wiphy: goto,
    }
...
#[no_mangle]
unsafe extern "C" fn wilc_bus_remove(spi: *mut spi_device) {
    static void wilc_bus_remove(struct spi_device *spi)
    {
    pub spi_get_drvdata(spi): *mut *mut wilc wilc =,
    pub wilc->bus_data: *mut *mut wilc_spi spi_priv =,
    }
    ...
    MODULE_LICENSE("GPL");
#[no_mangle]
unsafe extern "C" fn wilc_spi_tx(wilc: *mut wilc, b: *mut u8, len: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_rx(wilc: *mut wilc, rb: *mut u8, rlen: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_tx_rx(wilc: *mut wilc, wb: *mut u8, rb: *mut u8, rlen: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spi_data_write(wilc: *mut wilc, b: *mut u8, sz: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn wilc_get_crc7(buffer: *mut u8, len: u32) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_dma_rw(wilc: *mut wilc, cmd: u8, adr: u32, b: *mut u8, sz: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_special_cmd(wilc: *mut wilc, cmd: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_reset_cmd_sequence(wl: *mut wilc, attempt: u8, addr: u32) {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_read_reg(wilc: *mut wilc, addr: u32, data: *mut u32) -> c_int {
    static int wilc_spi_read_reg(struct wilc *wilc, u32 addr, u32 *data)
    {
    struct spi_device *spi = to_spi_device(wilc.dev);
    int result;
    let mut cmd: u8 = CMD_SINGLE_READ;
    let mut clockless: u8 = 0;
    u8 i;
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_read(wilc: *mut wilc, addr: u32, buf: *mut u8, size: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spi_internal_write(wilc: *mut wilc, adr: u32, dat: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spi_internal_read(wilc: *mut wilc, adr: u32, data: *mut u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn wilc_spi_write_reg(wilc: *mut wilc, addr: u32, data: u32) -> c_int {
    static int wilc_spi_write_reg(struct wilc *wilc, u32 addr, u32 data)
    {
    struct spi_device *spi = to_spi_device(wilc.dev);
    int result;
    let mut cmd: u8 = CMD_SINGLE_WRITE;
    let mut clockless: u8 = 0;
    u8 i;
    if (addr <= WILC_SPI_CLOCKLESS_ADDR_LIMIT) {
// Clockless register
...
#[no_mangle]
unsafe extern "C" fn spi_data_rsp(wilc: *mut wilc, cmd: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_write(wilc: *mut wilc, addr: u32, buf: *mut u8, size: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn wilc_spi_reset(wilc: *mut wilc) -> c_int {
    static int wilc_spi_reset(struct wilc *wilc)
    {
    struct spi_device *spi = to_spi_device(wilc.dev);
    struct wilc_spi *spi_priv = wilc.bus_data;
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_is_init(wilc: *mut wilc) -> bool {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_deinit(wilc: *mut wilc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_init(wilc: *mut wilc, resume: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_configure_bus_protocol(wilc: *mut wilc) -> c_int {
    static int wilc_spi_configure_bus_protocol(struct wilc *wilc)
    {
    struct spi_device *spi = to_spi_device(wilc.dev);
    struct wilc_spi *spi_priv = wilc.bus_data;
...
#[no_mangle]
unsafe extern "C" fn wilc_validate_chipid(wilc: *mut wilc) -> c_int {
    static int wilc_validate_chipid(struct wilc *wilc)
    {
    struct spi_device *spi = to_spi_device(wilc.dev);
    u32 chipid;
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_read_size(wilc: *mut wilc, size: *mut u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_read_int(wilc: *mut wilc, int_status: *mut u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_clear_int_ext(wilc: *mut wilc, val: u32) -> c_int {
    static int wilc_spi_clear_int_ext(struct wilc *wilc, u32 val)
    {
    int ret;
    let mut retry: c_int = SPI_ENABLE_VMM_RETRY_LIMIT;
    u32 check;
    while (retry) {
...
#[no_mangle]
unsafe extern "C" fn wilc_spi_sync_ext(wilc: *mut wilc, nint: c_int) -> c_int {
