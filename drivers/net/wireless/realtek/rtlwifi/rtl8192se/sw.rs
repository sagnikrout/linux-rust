...
//! Source: drivers/net/wireless/realtek/rtlwifi/rtl8192se/sw.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn rtl92s_init_aspm_vars(hw: *mut ieee80211_hw) {
...
#[no_mangle]
unsafe extern "C" fn rtl92se_fw_cb(firmware: *const firmware, context: *mut c_void) {
    static void rtl92se_fw_cb(const struct firmware *firmware, void *context)
    {
    struct ieee80211_hw *hw = context;
    struct rtl_priv *rtlpriv = rtl_priv(hw);
    struct rt_firmware *pfirmware = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn rtl92s_init_sw_vars(hw: *mut ieee80211_hw) -> c_int {
    static int rtl92s_init_sw_vars(struct ieee80211_hw *hw)
    {
    struct rtl_priv *rtlpriv = rtl_priv(hw);
    struct rtl_pci *rtlpci = rtl_pcidev(rtl_pcipriv(hw));
    let mut err: c_int = 0;
    let mut earlyrxthreshold: u16 = 7;
    char *fw_name = "rtlwifi/rtl8192sefw.bin";
...
#[no_mangle]
pub unsafe extern "C" fn if(2: rtlpriv->psc.reg_fwctrl_lps ==) -> else {
    else if (rtlpriv.psc.reg_fwctrl_lps == 2)
    rtlpriv.psc.fwctrl_psmode = FW_PS_MAX_MODE;
#[no_mangle]
pub unsafe extern "C" fn if(3: rtlpriv->psc.reg_fwctrl_lps ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn rtl92s_deinit_sw_vars(hw: *mut ieee80211_hw) {
    ...
    let mut own: u8 = (u8)rtl92se_get_desc(hw, entry, true, HW_DESC_OWN);
    if (own)
    return false;
    return true;
    }
