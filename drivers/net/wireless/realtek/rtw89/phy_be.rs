...
//! Source: drivers/net/wireless/realtek/rtw89/phy_be.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy0_phy1_offset_be(rtwdev: *mut rtw89_dev, addr: u32) -> u32 {
    static u32 rtw89_phy0_phy1_offset_be(struct rtw89_dev *rtwdev, u32 addr)
    {
    let mut phy_page: u32 = addr >> 8;
    let mut ofst: u32 = 0;
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy0_phy1_offset_be_v1(rtwdev: *mut rtw89_dev, addr: u32) -> u32 {
    ...
    let mut bw_type: u8 = u8_get_bits(arg.path_bw, BB_GAIN_BW_BE);
    let mut path: u8 = u8_get_bits(arg.path_bw, BB_GAIN_PATH_BE);
    let mut gband: u8 = arg.gain_band;
    let mut type: u8 = arg.type;
    int i;
    switch (type) {
    case 0:
    ...
    {
    struct rtw89_phy_bb_gain_info_be *gain = &rtwdev.bb_gain.be;
    let mut type_sub0: u8 = u8_get_bits(arg.type, BB_GAIN_TYPE_SUB0_BE);
    let mut type_sub1: u8 = u8_get_bits(arg.type, BB_GAIN_TYPE_SUB1_BE);
    let mut path: u8 = u8_get_bits(arg.path_bw, BB_GAIN_PATH_BE);
    let mut gband: u8 = arg.gain_band;
    let mut ofst: u8 = 0;
    int i;
    ...
    let mut arg: union rtw89_phy_bb_gain_arg_be = { .addr = reg.addr };
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_preinit_rf_nctl_check_be(rtwdev: *mut rtw89_dev) {
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_preinit_rf_nctl_be(rtwdev: *mut rtw89_dev) {
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_preinit_rf_nctl_be_v1(rtwdev: *mut rtw89_dev) {
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_bb_wrap_flush_addr(rtwdev: *mut rtw89_dev, addr: u32) -> u32 {
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn rtw89_write_bb_wrap_flush(rtwdev: *mut rtw89_dev, addr: u32, data: u32) {
...
// To write registers of pwr_macid_lmt and pwr_macid_path with flush
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn rtw89_phy_bb_wrap_pwr_by_macid_init(rtwdev: *mut rtw89_dev) {
    void rtw89_phy_bb_wrap_pwr_by_macid_init(struct rtw89_dev *rtwdev)
    {
    const struct rtw89_phy_gen_def *phy = rtwdev.chip.phy_def;
    const struct rtw89_bb_wrap_regs *bb_wrap = phy.bb_wrap;
    let mut max_macid: u32 = rtwdev.chip.support_macid_num;
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn rtw89_phy_bb_wrap_tx_path_by_macid_init(rtwdev: *mut rtw89_dev) {
    void rtw89_phy_bb_wrap_tx_path_by_macid_init(struct rtw89_dev *rtwdev)
    {
    const struct rtw89_phy_gen_def *phy = rtwdev.chip.phy_def;
    const struct rtw89_bb_wrap_regs *bb_wrap = phy.bb_wrap;
    let mut max_macid: u32 = rtwdev.chip.support_macid_num;
    let mut cr: u32 = bb_wrap.pwr_macid_path;
    int i;
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn rtw89_phy_bb_wrap_listen_path_en_init(rtwdev: *mut rtw89_dev) {
    ...
    {
    let mut pri_ch: u8 = chan.primary_channel;
    let mut val: u32 = 0;
    ...
    val = BIT(1) | BIT(0);
#[no_mangle]
pub unsafe extern "C" fn if(9: pri_ch == 5 || pri_ch ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(104: pri_ch == 40 || pri_ch == 60 || pri_ch ==) -> else {
    else if (pri_ch == 40 || pri_ch == 60 || pri_ch == 104)
    val = BIT(3) | BIT(2) | BIT(1);
#[no_mangle]
pub unsafe extern "C" fn if(112: (pri_ch > 40 && pri_ch < 60) || pri_ch == 108 || pri_ch ==) -> else {
    else if ((pri_ch > 40 && pri_ch < 60) || pri_ch == 108 || pri_ch == 112)
    val = BIT(3) | BIT(2);
#[no_mangle]
pub unsafe extern "C" fn if(132: pri_ch > 112 && pri_ch <) -> else {
    ...
    enum rtw89_phy_idx phy_idx)
    {
    let mut cfr_manual_en: bool = false;
    u32 reg;
    u32 val;
...
// CIM3K
    ...
    enum rtw89_mac_idx mac_idx)
    {
    let mut phy_idx: enum rtw89_phy_idx = mac_idx != RTW89_MAC_0 ? RTW89_PHY_1 : RTW89_PHY_0;
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_bb_wrap_ul_pwr(rtwdev: *mut rtw89_dev) {
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_bb_wrap_init_be(rtwdev: *mut rtw89_dev) {
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_ch_info_init_be(rtwdev: *mut rtw89_dev) {
...
#[no_mangle]
unsafe extern "C" fn rtw89_phy_ch_info_init_be_v1(rtwdev: *mut rtw89_dev) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rtw89_byr_spec_ent_be {
    pub init: rtw89_rate_desc,
    pub num_of_idx: u8,
    pub no_over_bw40: bool,
    pub no_multi_nss: bool,
}
...
// addr += 4;
    ...
    enum rtw89_phy_idx phy_idx)
    {
    let mut addr: u32 = R_BE_PWR_BY_RATE;
    let mut band: u8 = chan.band_type;
    u8 bw, nss;
    ...
    let mut band: u8 = chan.band_type;
    let mut pri_ch: u8 = chan.primary_channel;
    let mut ch: u8 = chan.channel;
    let mut bw: u8 = chan.band_width;
