...
//! Source: drivers/net/wireless/ath/wil6210/cfg80211.c
#![no_std]
...
pub const WIL_MAX_ROC_DURATION_MS: c_int = 5000;
...
//
pub const WIL_EDMG_BW_CONFIGURATION: c_int = 5;
...
#[no_mangle]
unsafe extern "C" fn wil_rx_cb_mode_to_n_bonded(cb_mode: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_tx_cb_mode_to_n_bonded(cb_mode: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_num_supported_channels(wil: *mut wil6210_priv) -> c_int {
    static int wil_num_supported_channels(struct wil6210_priv *wil)
    {
    let mut num_channels: c_int = ARRAY_SIZE(wil_60ghz_channels);
    if (!test_bit(WMI_FW_CAPABILITY_CHANNEL_4, wil.fw_capabilities))
    num_channels--;
    return num_channels;
    }
#[no_mangle]
pub unsafe extern "C" fn update_supported_bands(wil: *mut wil6210_priv) {
...
//
pub const QCA_NL80211_VENDOR_ID: c_uint = 0x001374;
...
#[no_mangle]
pub unsafe extern "C" fn wil_iftype_nl2wmi(type: enum nl80211_iftype) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wil_spec2wmi_ch(spec_ch: u8, wmi_ch: *mut u8) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn wil_wmi2spec_ch(wmi_ch: u8, spec_ch: *mut u8) -> c_int {
...
// spec_ch = 12;
    ...
    int rc;
    u8 tx_mcs, rx_mcs;
    let mut tx_rate_flag: u8 = RATE_INFO_FLAGS_DMG;
    let mut rx_rate_flag: u8 = RATE_INFO_FLAGS_DMG;
...
//
    ...
    int rc;
    let mut cid: c_int = wil_find_cid(wil, vif.mid, mac);
...
//
#[no_mangle]
pub unsafe extern "C" fn wil_find_cid_by_idx(wil: *mut wil6210_priv, mid: u8, idx: c_int) -> c_int {
    ...
    struct wil6210_vif *vif = wdev_to_vif(wil, wdev);
    int rc;
    let mut cid: c_int = wil_find_cid_by_idx(wil, vif.mid, idx);
    if (!wil_cid_valid(wil, cid))
    return -ENOENT;
    ...
    let mut check_combos: bool = false;
    for (i = 0; i < GET_MAX_VIFS(wil); i++) {
...
#[no_mangle]
pub unsafe extern "C" fn wil_vif_prepare_stop(vif: *mut wil6210_vif) -> c_int {
...
// social scan on P2P_DEVICE is handled as p2p search
    ...
    rc = wmi_set_ssid(vif, 0, core::ptr::null_mut());
    if (rc) {
    ...
    for (i = 0; i < n; i++) {
    let mut ch: c_int = request.channels[i].hw_value;
    if (ch == 0) {
    wil_err(wil,
...
// convert from edmg channel bitmap into edmg channel number
    ...
    int ch;
    let mut rc: c_int = 0;
    let mut is_ft_roam: bool = false;
    u8 network_type;
    let mut bss_type: enum ieee80211_bss_type = IEEE80211_BSS_TYPE_ESS;
...
// Connect can take lots of time
    ...
    WMI_DISCONNECT_EVENTID, core::ptr::null_mut(), 0,
...
// TODO: Rx GTK or Tx GTK?
    ...
    {
    let mut cid: c_int = -EINVAL;
    if (key_usage == WMI_KEY_USE_TX_GROUP)
    return core::ptr::null_mut(); /* not needed */
// supplicant provides Rx group key in STA mode with NULL MAC address
    if (mac_addr)
    cid = wil_find_cid(wil, mid, mac_addr);
#[no_mangle]
pub unsafe extern "C" fn if(WMI_KEY_USE_RX_GROUP: key_usage ==) -> else {
    ...
    struct wil6210_priv *wil = wiphy_to_wil(wiphy);
    struct wil6210_vif *vif = wdev_to_vif(wil, wdev);
    let mut key_usage: enum wmi_key_usage = wil_detect_key_usage(wdev, pairwise);
    struct wil_sta_info *cs = wil_find_sta_by_key_usage(wil, vif.mid,
    key_usage,
    mac_addr);
    if (!params) {
    wil_err(wil, "core::ptr::null_mut() params\n");
    return -EINVAL;
    }
...
#[no_mangle]
unsafe extern "C" fn wil_print_bcon_data(b: *mut cfg80211_beacon_data) {
...
// ies_len = proberesp_len - hlen;
    ...
    {
    int rc;
    let mut len: u16 = 0, proberesp_len = 0;
    u8 *ies = core::ptr::null_mut(), *proberesp;
// update local storage used for AP recovery
    ...
    let mut wmi_nettype: u8 = wil_iftype_nl2wmi(wdev.iftype);
    let mut is_go: u8 = (wdev.iftype == NL80211_IFTYPE_P2P_GO);
    let mut proberesp_len: u16 = 0;
    u8 *proberesp;
    let mut ft: bool = false;
    if (pbss)
...
#[no_mangle]
pub unsafe extern "C" fn wil_cfg80211_ap_recovery(wil: *mut wil6210_priv) {
    ...
    struct net_device *ndev;
    let mut bcon: cfg80211_beacon_data = {};
    let mut key_params: key_params = {};
    ...
    vif.gtk_index,
    false, core::ptr::null_mut(), &key_params);
    if (rc)
...
// in case privacy has changed, need to restart the AP
    ...
    clear_bit(wil_vif_ft_roam, vif.status);
    vif.ssid_len = 0;
    wil_memdup_ie(&vif.proberesp, &vif.proberesp_len, core::ptr::null_mut(), 0);
    wil_memdup_ie(&vif.proberesp_ies, &vif.proberesp_ies_len, core::ptr::null_mut(), 0);
    wil_memdup_ie(&vif.assocresp_ies, &vif.assocresp_ies_len, core::ptr::null_mut(), 0);
    ...
    int authorize;
    int cid, i;
    struct wil_ring_tx_data *txdata = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn wil_probe_client_worker(work: *mut work_struct) {
    ...
    struct wil_probe_client_req *req;
    struct list_head *lh;
    while ((lh = next_probe_client(vif)) != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn wil_probe_client_flush(vif: *mut wil6210_vif) {
    ...
    struct wil_probe_client_req *req;
    let mut cid: c_int = wil_find_cid(wil, vif.mid, peer);
...
#[no_mangle]
unsafe extern "C" fn wil_cfg80211_resume(wiphy: *mut wiphy) -> c_int {
...
//
    bss = vif.bss;
    if (!bss) {
    wil_err(wil, "FT: bss is core::ptr::null_mut()\n");
    return -EINVAL;
    }
...
#[no_mangle]
unsafe extern "C" fn wil_wiphy_init(wiphy: *mut wiphy) {
...
#[no_mangle]
pub unsafe extern "C" fn wil_cfg80211_deinit(wil: *mut wil6210_priv) {
    ...
    wiphy.iface_combinations = core::ptr::null_mut();
    wiphy_free(wiphy);
// do not access wil6210_priv after returning from here
    }
#[no_mangle]
pub unsafe extern "C" fn wil_p2p_wdev_free(wil: *mut wil6210_priv) {
    void wil_p2p_wdev_free(struct wil6210_priv *wil)
    {
    struct wireless_dev *p2p_wdev;
    mutex_lock(&wil.vif_mutex);
    p2p_wdev = wil.p2p_wdev;
    wil.p2p_wdev = core::ptr::null_mut();
    wil.radio_wdev = wil.main_ndev.ieee80211_ptr;
    mutex_unlock(&wil.vif_mutex);
    if (p2p_wdev) {
...
#[no_mangle]
unsafe extern "C" fn wil_rf_sector_status_to_rc(status: u8) -> c_int {
    ...
    data_len, wil_rf_sector_policy, core::ptr::null_mut());
    if (rc) {
