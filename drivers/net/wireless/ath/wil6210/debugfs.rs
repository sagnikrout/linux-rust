...
//! Source: drivers/net/wireless/ath/wil6210/debugfs.c
#![no_std]
...
pub struct dbg_off {
    ...
    {
    u8 num_of_descs;
    let mut has_skb: bool = false;
    if (ring.is_rx) {
    struct wil_rx_enhanced_desc *rx_d =
    (struct wil_rx_enhanced_desc *)
    &ring.va[idx].rx.enhanced;
    let mut buff_id: u16 = le16_to_cpu(rx_d.mac.buff_id);
...
#[no_mangle]
unsafe extern "C" fn ring_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    ...
    if (ring.va) {
    let mut cid: c_int = wil.ring2cid_tid[i][0];
    let mut tid: c_int = wil.ring2cid_tid[i][1];
    let mut swhead: u32 = ring.swhead;
    let mut swtail: u32 = ring.swtail;
    int used = (ring.size + swhead - swtail)
    % ring.size;
    let mut avail: c_int = ring.size - used - 1;
    char name[10];
    char sidle[10];
// performance monitoring
    let mut now: cycles_t = get_cycles();
    let mut idle: u64 = txdata.idle * 100;
    let mut total: u64 = now - txdata.begin;
    if (total != 0) {
    do_div(idle, total);
    ...
    {
    void __iomem *x;
    let mut sring_idx: c_int = sring - wil.srings;
    u32 v;
...
#[no_mangle]
unsafe extern "C" fn srings_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
//
    ...
    let mut delta: usize = i * sizeof(d);
    ...
    let mut len: u16 = le16_to_cpu(hdr.len);
...
#[no_mangle]
unsafe extern "C" fn mbox_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_debugfs_iomem_x32_set(data: *mut c_void, val: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_debugfs_iomem_x32_get(data: *mut c_void, val: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_debugfs_ulong_set(data: *mut c_void, val: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_debugfs_ulong_get(data: *mut c_void, val: *mut u64) -> c_int {
...
//
    ...
    wil6210_debugfs_init_offset(wil, d, (void * )wil.csr + off,
    isr_off);
    }
    ...
    wil6210_debugfs_init_offset(wil, d, (void * )wil.csr,
    pseudo_isr_off);
    }
    ...
    wil6210_debugfs_init_offset(wil, d, (void * )wil.csr,
    lgc_itr_cnt_off);
    wil6210_debugfs_init_offset(wil, dtx, (void * )wil.csr,
    tx_itr_cnt_off);
    wil6210_debugfs_init_offset(wil, drx, (void * )wil.csr,
    rx_itr_cnt_off);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn memread_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    ...
    let mut available: usize = wil_blob.blob.size;
    void *buf;
...
#[no_mangle]
unsafe extern "C" fn wil_pmcring_seq_open(inode: *mut inode, file: *mut file) -> c_int {
...
//
    ...
    void *cmd;
    let mut cmdlen: c_int = len - sizeof(struct wmi_cmd_hdr);
    u16 cmdid;
    int rc1;
    if (cmdlen < 0 || *ppos != 0)
    return -EINVAL;
    ...
    if (IS_ERR(wmi))
    return PTR_ERR(wmi);
    cmd = (cmdlen > 0) ? &wmi[1] : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn wil_seq_print_skb(s: *mut seq_file, skb: *mut sk_buff) {
    static void wil_seq_print_skb(struct seq_file *s, struct sk_buff *skb)
    {
    let mut i: c_int = 0;
    let mut len: c_int = skb_headlen(skb);
    void *p = skb.data;
...
// ---------Tx/Rx descriptor------------
#[no_mangle]
unsafe extern "C" fn txdesc_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    static int txdesc_show(struct seq_file *s, void *data)
    {
    struct wil6210_priv *wil = s.private;
    struct wil_ring *ring;
    bool tx;
    let mut ring_idx: c_int = dbg_ring_index;
    let mut txdesc_idx: c_int = dbg_txdesc_index;
    volatile struct vring_tx_desc *d;
    volatile u32 *u;
    struct sk_buff *skb;
    if (wil.use_enhanced_dma_hw) {
...
//
    ...
    if (wil.use_enhanced_dma_hw) {
    if (tx) {
    skb = ring.ctx ? ring.ctx[txdesc_idx].skb : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn status_msg_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    static int status_msg_show(struct seq_file *s, void *data)
    {
    struct wil6210_priv *wil = s.private;
    let mut sring_idx: c_int = dbg_sring_index;
    struct wil_status_ring *sring;
    bool tx;
    let mut status_msg_idx: u32 = dbg_status_msg_index;
    u32 *u;
...
#[no_mangle]
unsafe extern "C" fn wil_print_rx_buff(s: *mut seq_file, lh: *mut list_head) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rx_buff_mgmt_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_all_zeros(x_: *const *const c_void, sz: usize) -> bool {
...
#[no_mangle]
unsafe extern "C" fn bf_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
// ---------temp------------
#[no_mangle]
unsafe extern "C" fn print_temp(s: *mut seq_file, prefix: *const c_char, t: i32) {
...
#[no_mangle]
unsafe extern "C" fn temp_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
// ---------link------------
#[no_mangle]
unsafe extern "C" fn link_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    ...
    continue;
    vif = (mid < GET_MAX_VIFS(wil)) ? wil.vifs[mid] : core::ptr::null_mut();
    if (vif) {
...
// ---------info------------
#[no_mangle]
unsafe extern "C" fn info_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    static int info_show(struct seq_file *s, void *data)
    {
    struct wil6210_priv *wil = s.private;
    struct net_device *ndev = wil.main_ndev;
    let mut is_ac: c_int = power_supply_is_system_supplied();
    let mut rx: c_int = atomic_xchg(&wil.isr_count_rx, 0);
    let mut tx: c_int = atomic_xchg(&wil.isr_count_tx, 0);
    static ulong rxf_old, txf_old;
    let mut rxf: c_ulong = ndev.stats.rx_packets;
    let mut txf: c_ulong = ndev.stats.tx_packets;
    unsigned int i;
// >0 : AC; 0 : battery; <0 : error
    ...
    let mut state: c_ulong = txq.state;
...
// ---------Station matrix------------
#[no_mangle]
unsafe extern "C" fn wil_print_rxtid(s: *mut seq_file, r: *mut wil_tid_ampdu_rx) {
    static void wil_print_rxtid(struct seq_file *s, struct wil_tid_ampdu_rx *r)
    {
    int i;
    let mut index: u16 = ((r.head_seq_num - r.ssn) & 0xfff) % r.buf_size;
    let mut drop_dup: c_ulonglong = r.drop_dup, drop_old = r.drop_old;
    let mut drop_dup_mcast: c_ulonglong = r.drop_dup_mcast;
...
#[no_mangle]
unsafe extern "C" fn sta_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    ...
    let mut aid: u8 = 0;
    u8 mid;
    let mut sta_connected: bool = false;
    switch (p.status) {
    case wil_sta_unused:
    status = "unused   ";
    break;
...
#[no_mangle]
unsafe extern "C" fn mids_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_tx_latency_debugfs_show(s: *mut seq_file, data: *mut c_void) -> c_int {
    ...
    let mut tx_latency_avg: u64 = p.stats.tx_latency_total_us;
...
#[no_mangle]
unsafe extern "C" fn wil_tx_latency_seq_open(inode: *mut inode, file: *mut file) -> c_int {
...
// default resolution
    ...
    if (enable) {
    let mut sz: usize = sizeof(u64) * WIL_NUM_LATENCY_BINS;
...
#[no_mangle]
unsafe extern "C" fn wil_link_stats_debugfs_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wil_link_stats_seq_open(inode: *mut inode, file: *mut file) -> c_int {
...
// ---------FW capabilities------------
#[no_mangle]
unsafe extern "C" fn fw_capabilities_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
// ---------FW version------------
#[no_mangle]
unsafe extern "C" fn fw_version_show(s: *mut seq_file, data: *mut c_void) -> c_int {
...
// ----------------
    ...
    continue;
    wil_blob.wil = wil;
    blob.data = (void * )wil.csr + HOSTADDR(map.host);
...
#[no_mangle]
pub unsafe extern "C" fn wil6210_debugfs_init(wil: *mut wil6210_priv) -> c_int {
    ...
    wil6210_debugfs_init_offset(wil, dbg, (void * )wil.csr,
    dbg_wil_regs);
    wil6210_debugfs_init_offset(wil, dbg, core::ptr::null_mut(), dbg_statics);
    wil6210_debugfs_create_pseudo_ISR(wil, dbg);
    wil6210_debugfs_create_ITR_CNT(wil, dbg);
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn wil6210_debugfs_remove(wil: *mut wil6210_priv) {
