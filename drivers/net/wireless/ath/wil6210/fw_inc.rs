...
//! Source: drivers/net/wireless/ath/wil6210/fw_inc.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn wil_fw_verify(wil: *mut wil6210_priv, data: *const u8, size: usize) -> c_int {
...
// let the FW load anyway
    ...
    kfree(wil.brd_info);
    wil.brd_info = core::ptr::null_mut();
    wil_dbg_fw(wil,
...
// continue, let the FW load anyway
    ...
    void __iomem *dst;
    u32 v;
    let mut s: usize = (size_t)le32_to_cpu(d.size);
    if (size != sizeof(*d)) {
    ...
    for (i = 0; i < n; i++) {
    void __iomem *dst;
    let mut m: u32 = le32_to_cpu(block[i].mask);
    let mut v: u32 = le32_to_cpu(block[i].value);
    u32 x, y;
    ...
    for (i = 0; i < n; i++) {
    int rc;
    let mut a: u32 = le32_to_cpu(block[i].addr);
    let mut v: u32 = le32_to_cpu(block[i].value);
...
//
#[no_mangle]
pub unsafe extern "C" fn wil_request_board(wil: *mut wil6210_priv, name: *const c_char) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn wil_fw_verify_file_exists(wil: *mut wil6210_priv, name: *const c_char) -> bool {
