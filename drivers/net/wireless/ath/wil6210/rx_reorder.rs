...
//! Source: drivers/net/wireless/ath/wil6210/rx_reorder.c
#![no_std]
...
pub const SEQ_MODULO: c_uint = 0x1000;
pub const SEQ_MASK: c_uint = 0xfff;
#[no_mangle]
pub unsafe extern "C" fn seq_less(sq1: u16, sq2: u16) -> c_int {
    static inline int seq_less(u16 sq1, u16 sq2)
    {
    return ((sq1 - sq2) & SEQ_MASK) > (SEQ_MODULO >> 1);
    }
#[no_mangle]
pub unsafe extern "C" fn seq_inc(sq: u16) -> u16 {
    static inline u16 seq_inc(u16 sq)
    {
    return (sq + 1) & SEQ_MASK;
    }
#[no_mangle]
pub unsafe extern "C" fn seq_sub(sq1: u16, sq2: u16) -> u16 {
    static inline u16 seq_sub(u16 sq1, u16 sq2)
    {
    return (sq1 - sq2) & SEQ_MASK;
    }
#[no_mangle]
pub unsafe extern "C" fn reorder_index(r: *mut wil_tid_ampdu_rx, seq: u16) -> c_int {
...
// release the frame from the reorder ring buffer
    r.stored_mpdu_num--;
    r.reorder_buf[index] = core::ptr::null_mut();
...
//
    ...
    struct wil_tid_ampdu_rx *r)
    {
    let mut index: c_int = reorder_index(r, r.head_seq_num);
...
#[no_mangle]
pub unsafe extern "C" fn wil_rx_reorder(wil: *mut wil6210_priv, skb: *mut sk_buff) {
...
// ADDBA processing
#[no_mangle]
unsafe extern "C" fn wil_agg_size(wil: *mut wil6210_priv, req_agg_wsize: u16) -> u16 {
...
// Block Ack - Rx side (recipient)
    ...
    __acquires(&sta.tid_rx_lock) __releases(&sta.tid_rx_lock)
    {
    let mut param_set: u16 = le16_to_cpu(ba_param_set);
    let mut agg_timeout: u16 = le16_to_cpu(ba_timeout);
    let mut seq_ctrl: u16 = le16_to_cpu(ba_seq_ctrl);
    struct wil_sta_info *sta;
    u16 agg_wsize;
...
//
    let mut req_agg_wsize: u16 = WIL_GET_BITS(param_set, 6, 15);
    bool agg_amsdu = wil.use_enhanced_dma_hw &&
    wil.use_rx_hw_reordering &&
    test_bit(WMI_FW_CAPABILITY_AMSDU, wil.fw_capabilities) &&
    wil.amsdu_en && (param_set & BIT(0));
    let mut ba_policy: c_int = param_set & BIT(1);
    let mut ssn: u16 = seq_ctrl >> 4;
    struct wil_tid_ampdu_rx *r;
    let mut rc: c_int = 0;
    might_sleep();
// sanity checks
...
#[no_mangle]
pub unsafe extern "C" fn wil_addba_tx_request(wil: *mut wil6210_priv, ringid: u8, wsize: u16) -> c_int {
    int wil_addba_tx_request(struct wil6210_priv *wil, u8 ringid, u16 wsize)
    {
    let mut agg_wsize: u8 = wil_agg_size(wil, wsize);
    let mut agg_timeout: u16 = 0;
