...
//! Source: drivers/net/wireless/ath/ath10k/htt_rx.c
#![no_std]
...
pub const HTT_RX_RING_REFILL_RETRY_MS: c_int = 50;
pub const HTT_RX_RING_REFILL_RESCHED_MS: c_int = 5;
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_ring_free(htt: *mut ath10k_htt) {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_get_rx_ring_size_32(htt: *mut ath10k_htt) -> usize {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_get_rx_ring_size_64(htt: *mut ath10k_htt) -> usize {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_reset_paddrs_ring_32(htt: *mut ath10k_htt, idx: c_int) {
    static void ath10k_htt_reset_paddrs_ring_32(struct ath10k_htt *htt, int idx)
    {
    htt.rx_ring.paddrs_ring_32[idx] = 0;
    }
#[no_mangle]
unsafe extern "C" fn ath10k_htt_reset_paddrs_ring_64(htt: *mut ath10k_htt, idx: c_int) {
...
#[no_mangle]
unsafe extern "C" fn __ath10k_htt_rx_ring_fill_n(htt: *mut ath10k_htt, num: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_ring_fill_n(htt: *mut ath10k_htt, num: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_msdu_buff_replenish(htt: *mut ath10k_htt) {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_ring_refill_retry(t: *mut timer_list) {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_rx_ring_refill(ar: *mut ath10k) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_rx_free(htt: *mut ath10k_htt) {
    ...
    ath10k_htt_config_paddrs_ring(htt, core::ptr::null_mut());
    dma_free_coherent(htt.ar.dev,
    sizeof(*htt.rx_ring.alloc_idx.vaddr),
    htt.rx_ring.alloc_idx.vaddr,
    htt.rx_ring.alloc_idx.paddr);
    htt.rx_ring.alloc_idx.vaddr = core::ptr::null_mut();
    kfree(htt.rx_ring.netbufs_ring);
    htt.rx_ring.netbufs_ring = core::ptr::null_mut();
    }
    ...
    htt.rx_ring.netbufs_ring[idx] = core::ptr::null_mut();
    ...
    DMA_FROM_DEVICE);
    ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, core::ptr::null_mut(), "htt rx netbuf pop: ",
...
//
    ...
    let mut amsdu_len: c_int = __le16_to_cpu(ind_desc.msdu_len);
    ...
    paddr);
    prev_frag_buf.next = core::ptr::null_mut();
    return -ENOENT;
    }
    skb_put(frag_buf, min(amsdu_len, HTT_RX_BUF_SIZE));
    last_frag = ind_desc.reserved;
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_rx_alloc(htt: *mut ath10k_htt) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_bw_to_mac80211_bw(bw: u8) -> u8 {
    static inline u8 ath10k_bw_to_mac80211_bw(u8 bw)
    {
    pub 0: u8 ret =,
    switch (bw) {
    case 0:
    pub RATE_INFO_BW_20: ret =,
    case 1:
    pub RATE_INFO_BW_40: ret =,
    case 2:
    pub RATE_INFO_BW_80: ret =,
    case 3:
    pub RATE_INFO_BW_160: ret =,
    }
    pub ret: return,
    }
    ...
    struct htt_rx_desc *rxd)
    {
    pub &ar->hw_params: *mut *mut ath10k_hw_params hw =,
    pub rxd_attention: *mut rx_attention,
    pub rxd_mpdu_start: *mut rx_mpdu_start,
    pub rxd_mpdu_end: *mut rx_mpdu_end,
    pub rxd_msdu_start_common: *mut rx_msdu_start_common,
    pub rxd_msdu_end_common: *mut rx_msdu_end_common,
    pub rxd_ppdu_start: *mut rx_ppdu_start,
    pub sband: *mut ieee80211_supported_band,
    pub nss: u8 cck, rate, bw, sgi, mcs,,
    pub rxd_msdu_payload: *mut u8,
    pub 0: u8 preamble =,
    pub group_id: u8,
    pub info3: u32 info1, info2,,
    pub nsts_su: u32 stbc,,
    pub rxd): rxd_attention = ath10k_htt_rx_desc_get_attention(hw,,
    pub rxd): rxd_mpdu_start = ath10k_htt_rx_desc_get_mpdu_start(hw,,
    pub rxd): rxd_mpdu_end = ath10k_htt_rx_desc_get_mpdu_end(hw,,
    pub rxd): rxd_msdu_start_common = ath10k_htt_rx_desc_get_msdu_start(hw,,
    pub rxd): rxd_msdu_end_common = ath10k_htt_rx_desc_get_msdu_end(hw,,
    pub rxd): rxd_ppdu_start = ath10k_htt_rx_desc_get_ppdu_start(hw,,
    pub rxd): rxd_msdu_payload = ath10k_htt_rx_desc_get_msdu_payload(hw,,
    pub __le32_to_cpu(rxd_ppdu_start->info1): info1 =,
    pub __le32_to_cpu(rxd_ppdu_start->info2): info2 =,
    pub __le32_to_cpu(rxd_ppdu_start->info3): info3 =,
    pub RX_PPDU_START_INFO1_PREAMBLE_TYPE): preamble = MS(info1,,
    switch (preamble) {
    case HTT_RX_LEGACY:
...
//
    if (!status.freq)
    pub RX_PPDU_START_INFO1_L_SIG_RATE_SELECT: cck = info1 &,
    pub RX_PPDU_START_INFO1_L_SIG_RATE): rate = MS(info1,,
    pub ~RX_PPDU_START_RATE_FLAG: rate &=,
    pub &ar->mac.sbands[status->band]: sband =,
    pub cck): status->rate_idx = ath10k_mac_hw_rate_to_idx(sband, rate,,
    case HTT_RX_HT:
...
//
    ...
    pub 1: sgi = info3 &,
    pub 1: stbc = (info2 >> 3) &,
    pub 0x3F: group_id = (info2 >> 4) &,
    if (GROUP_ID_IS_SU_MIMO(group_id)) {
    pub 0x0F: mcs = (info3 >> 4) &,
    pub 0x07): nsts_su = ((info2 >> 10) &,
    if (stbc)
    pub 1: nss = (nsts_su >> 2) +,
    else
    pub 1): nss = (nsts_su +,
...
//
    ...
    if (mcs > 0x09) {
    pub mcs): ath10k_warn(ar, "invalid MCS received %u\n",,
    ...
    ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, core::ptr::null_mut(),
    "rx desc msdu payload: ",
    pub 50): rxd_msdu_payload,,
    }
    pub mcs: status->rate_idx =,
    pub nss: status->nss =,
    if (sgi)
    pub RX_ENC_FLAG_SHORT_GI: status->enc_flags |=,
    pub ath10k_bw_to_mac80211_bw(bw): status->bw =,
    pub RX_ENC_VHT: status->encoding =,
    default:
    }
    }
    ...
    pub peer_id): peer = ath10k_peer_find_by_id(ar,,
    if (!peer)
    pub NULL: return,
    pub peer->vdev_id): arvif = ath10k_get_arvif(ar,,
    ...
    void *data)
    {
    pub data: *mut *mut cfg80211_chan_def def =,
// def = conf->def;
    }
    static struct ieee80211_channel *
    ath10k_htt_rx_h_any_channel(struct ath10k *ar)
    {
    pub {}: cfg80211_chan_def def =,
    ...
    u32 vdev_id)
    {
    pub ch: *mut ieee80211_channel,
    pub ar->scan_channel: ch =,
    if (!ch)
    pub ar->rx_channel: ch =,
    if (!ch)
    pub rxd): ch = ath10k_htt_rx_h_peer_channel(ar,,
    if (!ch)
    pub vdev_id): ch = ath10k_htt_rx_h_vdev_channel(ar,,
    if (!ch)
    pub ath10k_htt_rx_h_any_channel(ar): ch =,
    if (!ch)
    pub ar->tgt_oper_chan: ch =,
    if (!ch)
    pub false: return,
    pub ch->band: status->band =,
    pub ch->center_freq: status->freq =,
    pub true: return,
    }
    ...
    {
    pub &ar->hw_params: *mut *mut ath10k_hw_params hw =,
    pub rxd): *mut *mut rx_ppdu_start rxd_ppdu_start = ath10k_htt_rx_desc_get_ppdu_start(hw,,
    pub i: c_int,
    pub {: for (i = 0; i < IEEE80211_MAX_CHAINS ; i++),
    pub ~BIT(i): status->chains &=,
...
// FIXME: Get real NF
    ...
    pub ~RX_FLAG_NO_SIGNAL_VAL: status->flag &=,
    }
    ...
    {
    pub &ar->hw_params: *mut *mut ath10k_hw_params hw =,
    pub rxd_ppdu_end_common: *mut rx_ppdu_end_common,
    pub rxd): rxd_ppdu_end_common = ath10k_htt_rx_desc_get_ppdu_end(hw,,
...
//
    pub __le32_to_cpu(rxd_ppdu_end_common->tsf_timestamp): status->mactime =,
    pub RX_FLAG_MACTIME_END: status->flag |=,
    }
    ...
    pub &ar->hw_params: *mut *mut ath10k_hw_params hw =,
    pub rxd: *mut htt_rx_desc,
    pub rxd_attention: *mut rx_attention,
    pub is_first_ppdu: bool,
    pub is_last_ppdu: bool,
    if (skb_queue_empty(amsdu))
    pub skb_peek(amsdu): first =,
    rxd = HTT_RX_BUF_TO_RX_DESC(hw,
    pub hw->rx_desc_ops->rx_desc_size): *mut *mut (void )first->data -,
    pub rxd): rxd_attention = ath10k_htt_rx_desc_get_attention(hw,,
...
// New PPDU starts so clear out the old per-PPDU status.
    pub 0: status->freq =,
    pub 0: status->rate_idx =,
    pub 0: status->nss =,
    pub RX_ENC_LEGACY: status->encoding =,
    pub RATE_INFO_BW_20: status->bw =,
    pub ~RX_FLAG_MACTIME: status->flag &=,
    pub RX_FLAG_NO_SIGNAL_VAL: status->flag |=,
    pub ~(RX_FLAG_AMPDU_IS_LAST): status->flag &=,
    pub RX_FLAG_AMPDU_LAST_KNOWN: status->flag |= RX_FLAG_AMPDU_DETAILS |,
    pub ar->ampdu_reference: status->ampdu_reference =,
    pub rxd): ath10k_htt_rx_h_signal(ar, status,,
    pub vdev_id): ath10k_htt_rx_h_channel(ar, status, rxd,,
    pub rxd): ath10k_htt_rx_h_rates(ar, status,,
    }
    if (is_last_ppdu) {
    pub rxd): ath10k_htt_rx_h_mactime(ar, status,,
...
#[no_mangle]
unsafe extern "C" fn ath10k_process_rx(ar: *mut ath10k, skb: *mut sk_buff) {
    ...
    ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, core::ptr::null_mut(), "rx skb: ",
    skb.data, skb.len);
    trace_ath10k_rx_hdr(ar, skb.data, skb.len);
    trace_ath10k_rx_payload(ar, skb.data, skb.len);
    ieee80211_rx_napi(ar.hw, core::ptr::null_mut(), skb, &ar.napi);
    }
    static int ath10k_htt_rx_nwifi_hdrlen(struct ath10k *ar,
    struct ieee80211_hdr *hdr)
    {
    let mut len: c_int = ieee80211_hdrlen(hdr.frame_control);
    ...
    bool is_last;
    bool msdu_limit_err;
    let mut bytes_aligned: c_int = ar.hw_params.decap_align_bytes;
    u8 *qos;
    rxd = HTT_RX_BUF_TO_RX_DESC(hw,
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_get_csum_state(hw: *mut ath10k_hw_params, skb: *mut sk_buff) -> c_int {
    static int ath10k_htt_rx_get_csum_state(struct ath10k_hw_params *hw, struct sk_buff *skb)
    {
    struct htt_rx_desc *rxd;
    struct rx_attention *rxd_attention;
    ...
    bool is_mgmt;
    u32 attention;
    let mut frag_pn_check: bool = true, multicast_check = true;
    if (skb_queue_empty(amsdu))
    return;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_tkip_err) -> else {
    else if (has_tkip_err)
// err = ATH10K_PKT_RX_ERR_TKIP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_crypto_err) -> else {
    else if (has_crypto_err)
// err = ATH10K_PKT_RX_ERR_CRYPT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: has_peer_idx_invalid) -> else {
...
// Setup per-MSDU flags
    ...
    first_subframe = core::ptr::null_mut();
    ...
    let mut amsdu_len: c_int = skb_queue_len(amsdu);
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_handle_amsdu(htt: *mut ath10k_htt) -> c_int {
    ...
    struct sk_buff_head amsdu;
    int ret;
    let mut drop_cnt: c_ulong = 0;
    let mut unchain_cnt: c_ulong = 0;
    let mut drop_cnt_filter: c_ulong = 0;
    unsigned long msdus_to_queue, num_msdus;
    let mut err: enum ath10k_pkt_rx_err = ATH10K_PKT_RX_ERR_MAX;
...
// only for ret = 1 indicates chained msdus
    ...
    let mut new_pn: union htt_rx_pn_t = {};
...
//
    ...
    u8 offset, *ivp, i;
    let mut keyidx: i8 = 0;
    let mut pn48: __le64 = cpu_to_le64(new_pn.pn48);
    hdr = (struct ieee80211_hdr *)skb.data;
...
// the ExtIV bit is always set to 1 for CCMP
    ...
    {
    struct ath10k *ar = htt.ar;
    let mut tkip_mic: enum htt_rx_tkip_demic_type = HTT_RX_NON_TKIP_MIC;
    enum htt_txrx_sec_cast_type sec_index;
...
//
    ...
    ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, core::ptr::null_mut(), "htt rx ind: ",
    ...
    let mut tx_done: htt_tx_done = {};
    let mut status: c_int = MS(resp.data_tx_completion.flags, HTT_DATA_TX_STATUS);
    __le16 msdu_id, *msdus;
    let mut rssi_enabled: bool = false;
    let mut msdu_count: u8 = 0, num_airtime_records, tid;
    int i, htt_pad = 0;
    struct htt_data_tx_compl_ppdu_dur *ppdu_info;
    struct ath10k_peer *peer;
    let mut ppdu_info_offset: u16 = 0, peer_id;
    u32 tx_duration;
    switch (status) {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_addba(ar: *mut ath10k, resp: *mut htt_resp) {
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_delba(ar: *mut ath10k, resp: *mut htt_resp) {
...
//
    ...
    ath10k_htt_rx_h_channel(ar, status, core::ptr::null_mut(), rx.vdev_id);
    ath10k_htt_rx_h_queue_msdu(ar, status, msdu);
    }
    }
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_in_ord_ind(ar: *mut ath10k, skb: *mut sk_buff) -> c_int {
...
//
    ...
    ath10k_htt_rx_h_filter(ar, &amsdu, status, core::ptr::null_mut());
    ath10k_htt_rx_h_mpdu(ar, &amsdu, status, false, core::ptr::null_mut(),
    core::ptr::null_mut(), peer_id, frag);
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_tx_fetch_ind(ar: *mut ath10k, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_htc_t2h_msg_handler(ar: *mut ath10k, skb: *mut sk_buff) {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_get_legacy_rate_idx(ar: *mut ath10k, rate: u8) -> i8 {
    ...
    struct ieee80211_chanctx_conf *conf = core::ptr::null_mut();
    let mut rate: u8 = 0, sgi;
    let mut rate_idx: i8 = 0;
    bool skip_auto_rate;
...
#[no_mangle]
unsafe extern "C" fn ath10k_fetch_10_2_tx_stats(ar: *mut ath10k, data: *mut u8) {
    ...
    struct ieee80211_sta *sta;
    struct ath10k_peer *peer;
    let mut log_type: u16 = __le16_to_cpu(hdr.log_type);
    let mut peer_id: u32 = 0, i;
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_pn_len(sec_type: enum htt_security_types) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_t2h_msg_handler(ar: *mut ath10k, skb: *mut sk_buff) -> bool {
...
// confirm alignment
    ...
    let mut status: c_int = __le32_to_cpu(resp.mgmt_tx_completion.status);
    let mut info: c_int = __le32_to_cpu(resp.mgmt_tx_completion.info);
    ...
    ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, core::ptr::null_mut(), "htt event: ",
    skb.data, skb.len);
...
//
    ...
    struct ath10k_htc *htc = &ar.htc;
    struct ath10k_htc_ep *ep = &ar.htc.endpoint[htt.eid];
    let mut msg_word: u32 = __le32_to_cpu(*(__le32 *)resp);
    ...
    let mut phymode: u32 = __le32_to_cpu(resp.chan_change.phymode);
    let mut freq: u32 = __le32_to_cpu(resp.chan_change.freq);
...
#[no_mangle]
unsafe extern "C" fn ath10k_htt_rx_deliver_msdu(ar: *mut ath10k, quota: c_int, budget: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_rx_hl_indication(ar: *mut ath10k, budget: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_txrx_compl_task(ar: *mut ath10k, budget: c_int) -> c_int {
    ...
    struct sk_buff *skb;
    unsigned long flags;
    let mut quota: c_int = 0, done, ret;
    let mut resched_napi: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn ath10k_htt_set_rx_ops(htt: *mut ath10k_htt) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ar->hw_params.target_64bit) -> else {
