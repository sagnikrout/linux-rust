...
//! Source: drivers/net/wireless/ath/ath10k/debugfs_sta.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn if(2: msdu_count ==) -> else {
    else if (msdu_count == 2)
    stats.rx_pkt_amsdu[ATH10K_AMSDU_SUBFRM_NUM_2]++;
#[no_mangle]
pub unsafe extern "C" fn if(3: msdu_count ==) -> else {
    else if (msdu_count == 3)
    stats.rx_pkt_amsdu[ATH10K_AMSDU_SUBFRM_NUM_3]++;
#[no_mangle]
pub unsafe extern "C" fn if(4: msdu_count ==) -> else {
...
pub unsafe extern "C" fn if(60: mpdu_count >) -> else {
    ...
    let mut non_data_frm: bool = false;
    ...
    return;
    rcu_read_lock();
    sta = ieee80211_find_sta_by_ifaddr(ar.hw, hdr.addr2, core::ptr::null_mut());
    if (!sta)
    goto exit;
...
// Valid TID values are 0 through 15
    ...
    let mut len: usize = 0, buf_len = 1048 * IEEE80211_NUM_TIDS;
    char *buf;
    int i, j;
    ssize_t ret;
    ...
    let mut len: c_int = 0, i, j, k, retval = 0;
    let mut size: c_int = 16 * 4096;
    char *buf;
