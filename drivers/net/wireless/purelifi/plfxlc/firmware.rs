...
//! Source: drivers/net/wireless/purelifi/plfxlc/firmware.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn plfxlc_download_fpga(intf: *mut usb_interface) -> c_int {
    int plfxlc_download_fpga(struct usb_interface *intf)
    {
    struct usb_device *udev = interface_to_usbdev(intf);
    unsigned char *fpga_dmabuff = core::ptr::null_mut();
    const struct firmware *fw = core::ptr::null_mut();
    let mut blk_tran_len: c_int = PLF_BULK_TLEN;
    unsigned char *fw_data;
    const char *fw_name;
    int r, actual_length;
    let mut fw_data_i: c_int = 0;
    ...
    send_vendor_command(udev, PLF_VNDR_FPGA_SET_CMD, core::ptr::null_mut(), 0);
...
// u8 bit reverse
    ...
    send_vendor_command(udev, PLF_VNDR_FPGA_STATE_CMD, core::ptr::null_mut(), 0);
...
#[no_mangle]
pub unsafe extern "C" fn plfxlc_download_xl_firmware(intf: *mut usb_interface) -> c_int {
    int plfxlc_download_xl_firmware(struct usb_interface *intf)
    {
    struct usb_device *udev = interface_to_usbdev(intf);
    const struct firmware *fwp = core::ptr::null_mut();
    let mut file: plfxlc_firmware_file = {0};
    const char *fw_pack;
    int s, r;
    u8 *buf;
    u32 i;
    r = send_vendor_command(udev, PLF_VNDR_XL_FW_CMD, core::ptr::null_mut(), 0);
    msleep(PLF_MSLEEP_TIME);
    if (r) {
...
// Code for single pack file download ends fw download finish
    r = send_vendor_command(udev, PLF_VNDR_XL_EX_CMD, core::ptr::null_mut(), 0);
    ...
    unsigned char *dma_buffer = core::ptr::null_mut();
