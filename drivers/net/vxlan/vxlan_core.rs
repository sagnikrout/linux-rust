...
//! Source: drivers/net/vxlan/vxlan_core.c
#![no_std]
...
//
    let mut __read_mostly: static unsigned short vxlan_port = 8472;
    module_param_named(udp_port, vxlan_port, ushort, 0444);
    MODULE_PARM_DESC(udp_port, "Destination UDP port");
    let mut log_ecn_error: static bool = true;
...
#[no_mangle]
pub unsafe extern "C" fn vxlan_collect_metadata(vs: *mut vxlan_sock) -> bool {
...
// Look up VNI in a per net namespace table
    ...
    if (!vs)
    return core::ptr::null_mut();
    return vxlan_vs_find_vni(vs, ifindex, vni, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn vxlan_nlmsg_size() -> usize {
...
// -EMSGSIZE implies BUG in vxlan_nlmsg_size()
    ...
    vxlan_fdb_switchdev_notifier_info(vxlan, fdb, rd, core::ptr::null_mut(), &info);
...
#[no_mangle]
unsafe extern "C" fn vxlan_ip_miss(dev: *mut net_device, ipa: *mut union vxlan_addr) {
    ...
    vxlan_fdb_notify(vxlan, &f, &remote, RTM_GETNEIGH, true, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn vxlan_fdb_miss(vxlan: *mut vxlan_dev, eth_addr[ETH_ALEN]: u8) {
    static void vxlan_fdb_miss(struct vxlan_dev *vxlan, const u8 eth_addr[ETH_ALEN])
    {
    struct vxlan_fdb f = {
    .state = NUD_STALE,
    };
    let mut remote: vxlan_rdst = { };
...
// caller should hold vxlan->hash_lock
    ...
    struct vxlan_rdst *rdst = core::ptr::null_mut();
    struct vxlan_fdb *f;
    let mut rc: c_int = 0;
    ...
    vxlan_fdb_switchdev_notifier_info(vxlan, f, rdst, core::ptr::null_mut(), fdb_info);
    out:
    rcu_read_unlock();
    return rc;
    }
...
#[no_mangle]
pub unsafe extern "C" fn vxlan_fdb_clear_offload(dev: *const net_device, vni: __be32) {
...
#[no_mangle]
unsafe extern "C" fn vxlan_parse_gpe_proto(hdr: *const vxlanhdr, protocol: *mut __be16) -> bool {
...
// protocol = tun_p_to_eth_p(gpe->next_protocol);
    ...
    struct sk_buff *skb)
    {
    struct sk_buff *pp = core::ptr::null_mut();
    struct gro_remcsum grc;
    let mut flush: c_int = 1;
...
#[no_mangle]
unsafe extern "C" fn vxlan_gro_complete(sk: *mut sock, skb: *mut sk_buff, nhoff: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vxlan_gpe_gro_complete(sk: *mut sock, skb: *mut sk_buff, nhoff: c_int) -> c_int {
    static int vxlan_gpe_gro_complete(struct sock *sk, struct sk_buff *skb, int nhoff)
    {
    struct vxlanhdr *vh = (struct vxlanhdr *)(skb.data + nhoff);
    const struct packet_offload *ptype;
    let mut err: c_int = -ENOSYS;
    __be16 protocol;
    if (!vxlan_parse_gpe_proto(vh, &protocol))
    return err;
...
// check nexthop group family
    ...
    struct netlink_ext_ack *extack)
    {
    struct vxlan_rdst *rd = core::ptr::null_mut();
    struct vxlan_fdb *f;
    int rc;
...
#[no_mangle]
unsafe extern "C" fn __vxlan_fdb_free(f: *mut vxlan_fdb) {
    ...
    if (nh) {
    rcu_assign_pointer(f.nh, core::ptr::null_mut());
    rcu_assign_pointer(f.vdev, core::ptr::null_mut());
    nexthop_put(nh);
    }
...
#[no_mangle]
unsafe extern "C" fn vxlan_fdb_free(head: *mut rcu_head) {
    ...
    if (do_notify) {
    if (rcu_access_pointer(f.nh))
    vxlan_fdb_notify(vxlan, f, core::ptr::null_mut(), RTM_DELNEIGH,
    swdev_notify, core::ptr::null_mut());
    else
    list_for_each_entry(rd, &f.remotes, list)
...
#[no_mangle]
unsafe extern "C" fn vxlan_dst_free(head: *mut rcu_head) {
    ...
    struct netlink_ext_ack *extack)
    {
    let mut fdb_flags: __u16 = (ndm_flags & ~NTF_USE);
    struct vxlan_rdst *rd = core::ptr::null_mut();
...
// rd = oldrd;
#[no_mangle]
pub unsafe extern "C" fn if(rc: (flags & NLM_F_APPEND) &&) -> else {
...
// Add new entry to forwarding table -- assumes lock held
    ...
    {
    list_del_rcu(&rd.list);
    vxlan_fdb_notify(vxlan, f, rd, RTM_DELNEIGH, swdev_notify, core::ptr::null_mut());
    call_rcu(&rd.rcu, vxlan_dst_free);
    }
...
// Dump forwarding table
    ...
    cb.nlh.nlmsg_seq,
    RTM_NEWNEIGH, NLM_F_MULTI, core::ptr::null_mut());
    if (err < 0) {
    rcu_read_unlock();
    goto out;
    }
...
// Don't migrate static entries, drop packets
    ...
    vxlan_fdb_notify(vxlan, f, rdst, RTM_NEWNEIGH, true, core::ptr::null_mut());
    } else {
// learned new entry
...
// close off race between vxlan_flush and incoming packets
    ...
    ifindex, NTF_SELF, 0, true, core::ptr::null_mut());
    spin_unlock(&vxlan.hash_lock);
    }
    return SKB_NOT_DROPPED_YET;
    }
#[no_mangle]
unsafe extern "C" fn __vxlan_sock_release_prep(vs: *mut vxlan_sock) -> bool {
...
#[no_mangle]
unsafe extern "C" fn vxlan_sock_release(vxlan: *mut vxlan_dev) {
    static void vxlan_sock_release(struct vxlan_dev *vxlan)
    {
    struct vxlan_sock *sock4 = rtnl_dereference(vxlan.vn4_sock);

    struct vxlan_sock *sock6 = rtnl_dereference(vxlan.vn6_sock);
    RCU_INIT_POINTER(vxlan.vn6_sock, core::ptr::null_mut());

    RCU_INIT_POINTER(vxlan.vn4_sock, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn vxlan_remcsum(skb: *mut sk_buff, vxflags: u32) -> enum skb_drop_reason {
...
// In flow-based mode, GBP is carried in dst_metadata
    ...
    let mut ifindex: u32 = skb.dev.ifindex;
...
#[no_mangle]
unsafe extern "C" fn vxlan_rcv(sk: *mut sock, skb: *mut sk_buff) -> c_int {
    static int vxlan_rcv(struct sock *sk, struct sk_buff *skb)
    {
    struct vxlan_vni_node *vninode = core::ptr::null_mut();
    const struct vxlanhdr *vh;
    struct vxlan_dev *vxlan;
    struct vxlan_sock *vs;
    struct vxlan_metadata _md;
    struct vxlan_metadata *md = &_md;
    let mut protocol: __be16 = htons(ETH_P_TEB);
    enum skb_drop_reason reason;
    let mut raw_proto: bool = false;
    void *oiph;
    let mut vni: __be32 = 0;
    int nh;
...
#[no_mangle]
unsafe extern "C" fn vxlan_err_lookup(sk: *mut sock, skb: *mut sk_buff) -> c_int {
    ...
    vxlan = vxlan_vs_find_vni(vs, skb.dev.ifindex, vni, core::ptr::null_mut());
    if (!vxlan)
    return -ENOENT;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn arp_reduce(dev: *mut net_device, skb: *mut sk_buff, vni: __be32) -> c_int {
    ...
    vxlan_vnifilter_count(vxlan, vni, core::ptr::null_mut(),
...
// bridge-local neighbor
    ...
    int ns_olen;
    int i, len;
    if (dev == core::ptr::null_mut() || !pskb_may_pull(request, request.len))
    return core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn neigh_reduce(dev: *mut net_device, skb: *mut sk_buff, vni: __be32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn route_shortcircuit(dev: *mut net_device, skb: *mut sk_buff) -> bool {
...
#[no_mangle]
unsafe extern "C" fn vxlan_build_gpe_hdr(vxh: *mut vxlanhdr, protocol: __be16) -> c_int {
    ...
    bool udp_sum)
    {
    let mut type: c_int = udp_sum ? SKB_GSO_UDP_TUNNEL_CSUM : SKB_GSO_UDP_TUNNEL;
    let mut inner_protocol: __be16 = htons(ETH_P_TEB);
    struct vxlanhdr *vxh;
    bool double_encap;
    int min_headroom;
    int err;
    ...
    let mut csum_start: c_int = skb_checksum_start_offset(skb);
...
// Bypass encapsulation if the destination is local
    ...
    vxlan_vnifilter_count(src_vxlan, vni, core::ptr::null_mut(), VXLAN_VNI_STATS_TX, len);
    if (__netif_rx(skb) == NET_RX_SUCCESS) {
    dev_dstats_rx_add(dst_vxlan.dev, len);
    vxlan_vnifilter_count(dst_vxlan, vni, core::ptr::null_mut(), VXLAN_VNI_STATS_RX,
    len);
    } else {
    drop:
    dev_dstats_rx_dropped(dev);
    vxlan_vnifilter_count(dst_vxlan, vni, core::ptr::null_mut(),
    VXLAN_VNI_STATS_RX_DROPS, 0);
    }
    rcu_read_unlock();
    }
...
// Bypass encapsulation if the destination is local
    ...
    struct vxlan_metadata _md;
    struct vxlan_metadata *md = &_md;
    let mut pkt_len: c_uint = skb.len;
    let mut src_port: __be16 = 0, dst_port;
    struct dst_entry *ndst = core::ptr::null_mut();
    int addr_family;
    __u8 tos, ttl;
    int ifindex;
    let mut err: c_int = 0;
    let mut flags: u32 = vxlan.cfg.flags;
    bool use_cache;
    let mut udp_sum: bool = false;
    let mut xnet: bool = !net_eq(vxlan.net, dev_net(vxlan.dev));
    enum skb_drop_reason reason;
    bool no_eth_encap;
    let mut vni: __be32 = 0;
...
// short-circuited back to local bridge
    ...
    tos, use_cache ? dst_cache : core::ptr::null_mut());
    if (IS_ERR(rt)) {
    err = PTR_ERR(rt);
...
// Bypass encapsulation if the destination is local
    ...
    use_cache ? dst_cache : core::ptr::null_mut());
    if (IS_ERR(ndst)) {
    err = PTR_ERR(ndst);
    ndst = core::ptr::null_mut();
    reason = SKB_DROP_REASON_IP_OUTNOROUTES;
    goto tx_error;
    }
    if (flags & VXLAN_F_MC_ROUTE)
    ...
    if (!info) {
    let mut rt6i_flags: u32 = dst_rt6_info(ndst).rt6i_flags;
    ...
    vxlan_vnifilter_count(vxlan, vni, core::ptr::null_mut(), VXLAN_VNI_STATS_TX, pkt_len);
    out_unlock:
    rcu_read_unlock();
    return;
    drop:
    dev_dstats_tx_dropped(dev);
    vxlan_vnifilter_count(vxlan, vni, core::ptr::null_mut(), VXLAN_VNI_STATS_TX_DROPS, 0);
    kfree_skb_reason(skb, reason);
    return;
    tx_error:
    rcu_read_unlock();
...
#[no_mangle]
pub unsafe extern "C" fn if(-ENETUNREACH: err ==) -> else {
    else if (err == -ENETUNREACH)
    DEV_STATS_INC(dev, tx_carrier_errors);
    dst_release(ndst);
    DEV_STATS_INC(dev, tx_errors);
    vxlan_vnifilter_count(vxlan, vni, core::ptr::null_mut(), VXLAN_VNI_STATS_TX_ERRORS, 0);
    kfree_skb_reason(skb, reason);
    }
    ...
    drop:
    dev_dstats_tx_dropped(dev);
    vxlan_vnifilter_count(netdev_priv(dev), vni, core::ptr::null_mut(),
    VXLAN_VNI_STATS_TX_DROPS, 0);
    dev_kfree_skb(skb);
    }
...
//
#[no_mangle]
unsafe extern "C" fn vxlan_xmit(skb: *mut sk_buff, dev: *mut net_device) -> netdev_tx_t {
    static netdev_tx_t vxlan_xmit(struct sk_buff *skb, struct net_device *dev)
    {
    struct vxlan_dev *vxlan = netdev_priv(dev);
    struct vxlan_rdst *rdst, *fdst = core::ptr::null_mut();
    ...
    vxlan_xmit_one(skb, dev, vni, core::ptr::null_mut(), false);
    else
...
#[no_mangle]
unsafe extern "C" fn vxlan_cleanup(t: *mut timer_list) {
    static void vxlan_cleanup(struct timer_list *t)
    {
    struct vxlan_dev *vxlan = timer_container_of(vxlan, t, age_timer);
    let mut next_timer: c_ulong = jiffies + FDB_AGE_INTERVAL;
    struct vxlan_fdb *f;
...
#[no_mangle]
unsafe extern "C" fn vxlan_vs_del_dev(vxlan: *mut vxlan_dev) {
    static void vxlan_vs_del_dev(struct vxlan_dev *vxlan)
    {
    ASSERT_RTNL();
    ...
    struct vxlan_dev_node *node)
    {
    let mut vni: __be32 = vxlan.default_dst.remote_vni;
    ASSERT_RTNL();
    node.vxlan = vxlan;
...
#[no_mangle]
unsafe extern "C" fn vxlan_init(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vxlan_uninit(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn vxlan_open(dev: *mut net_device) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vxlan_fdb_flush_desc {
    pub ignore_default_entry: bool,
    pub state: c_ulong,
    pub state_mask: c_ulong,
    pub flags: c_ulong,
...
#[no_mangle]
unsafe extern "C" fn vxlan_fdb_nhid_matches(f: *const vxlan_fdb, nhid: u32) -> bool {
...
// Purge the forwarding table
    ...
    const struct vxlan_fdb_flush_desc *desc)
    {
    let mut match_remotes: bool = vxlan_fdb_flush_should_match_remotes(desc);
    struct vxlan_fdb *f;
    rcu_read_lock();
    hlist_for_each_entry_rcu(f, &vxlan.fdb_list, fdb_node) {
    ...
    goto unlock;
    if (match_remotes) {
    let mut destroy_fdb: bool = false;
    ...
    {
    struct vxlan_dev *vxlan = netdev_priv(dev);
    let mut desc: vxlan_fdb_flush_desc = {};
...
#[no_mangle]
unsafe extern "C" fn vxlan_stop(dev: *mut net_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vxlan_set_multicast_list(dev: *mut net_device) {
    static void vxlan_set_multicast_list(struct net_device *dev)
    {
    }
#[no_mangle]
unsafe extern "C" fn vxlan_change_mtu(dev: *mut net_device, new_mtu: c_int) -> c_int {
...
//
    if (lowerdev) {
    let mut max_mtu: c_int = lowerdev.mtu - vxlan_headroom(vxlan.cfg.flags);
    if (new_mtu > max_mtu)
    return -EINVAL;
    }
...
#[no_mangle]
unsafe extern "C" fn vxlan_fill_metadata_dst(dev: *mut net_device, skb: *mut sk_buff) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vxlan_offload_rx_ports(dev: *mut net_device, push: bool) {
...
#[no_mangle]
unsafe extern "C" fn vxlan_setup(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn vxlan_ether_setup(dev: *mut net_device) {
...
#[no_mangle]
unsafe extern "C" fn vxlan_raw_setup(dev: *mut net_device) {
    static void vxlan_raw_setup(struct net_device *dev)
    {
    dev.header_ops = core::ptr::null_mut();
    ...
    let mut id: u32 = nla_get_u32(data[IFLA_VXLAN_ID]);
    if (id >= VXLAN_N_VID) {
    ...
    let mut df: enum ifla_vxlan_df = nla_get_u8(data[IFLA_VXLAN_DF]);
...
// Mark socket as an encapsulation socket.
    ...
    tunnel_cfg.encap_destroy = core::ptr::null_mut();
    if (vs.flags & VXLAN_F_GPE) {
...
#[no_mangle]
unsafe extern "C" fn __vxlan_sock_add(vxlan: *mut vxlan_dev, ipv6: bool) -> c_int {
    static int __vxlan_sock_add(struct vxlan_dev *vxlan, bool ipv6)
    {
    let mut metadata: bool = vxlan.cfg.flags & VXLAN_F_COLLECT_METADATA;
    struct vxlan_sock *vs = core::ptr::null_mut();
    struct vxlan_dev_node *node;
    let mut l3mdev_index: c_int = 0;
    ASSERT_RTNL();
...
#[no_mangle]
unsafe extern "C" fn vxlan_sock_add(vxlan: *mut vxlan_dev) -> c_int {
    static int vxlan_sock_add(struct vxlan_dev *vxlan)
    {
    let mut metadata: bool = vxlan.cfg.flags & VXLAN_F_COLLECT_METADATA;
    let mut ipv6: bool = vxlan.cfg.flags & VXLAN_F_IPV6 || metadata;
    let mut ipv4: bool = !ipv6 || metadata;
    let mut ret: c_int = 0;
    RCU_INIT_POINTER(vxlan.vn4_sock, core::ptr::null_mut());
...
// lower = NULL;
    ...
    struct vxlan_dev *vxlan = netdev_priv(dev);
    struct vxlan_rdst *dst = &vxlan.default_dst;
    let mut needed_headroom: c_ushort = ETH_HLEN;
    let mut max_mtu: c_int = ETH_MAX_MTU;
    let mut flags: u32 = conf.flags;
    if (!changelink) {
    ...
    struct vxlan_net *vn = net_generic(net, vxlan_net_id);
    struct vxlan_dev *vxlan = netdev_priv(dev);
    struct net_device *remote_dev = core::ptr::null_mut();
    ...
    err = rtnl_configure_link(dev, core::ptr::null_mut(), 0, core::ptr::null_mut());
    if (err < 0)
    goto unlink;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: nla_get_u8(tb[attrtype])) -> else {
...
// if changelink operation, start with old existing cfg
    ...
    if (data[IFLA_VXLAN_ID]) {
    let mut vni: __be32 = cpu_to_be32(nla_get_u32(data[IFLA_VXLAN_ID]));
...
#[no_mangle]
unsafe extern "C" fn vxlan_dellink(dev: *mut net_device, head: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn vxlan_get_size(dev: *const net_device) -> usize {
...
#[no_mangle]
unsafe extern "C" fn vxlan_fill_info(skb: *mut sk_buff, dev: *const net_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(NETDEV_UDP_TUNNEL_PUSH_INFO: event ==) -> else {
    else if (event == NETDEV_UDP_TUNNEL_PUSH_INFO)
    vxlan_offload_rx_ports(dev, true);
#[no_mangle]
pub unsafe extern "C" fn if(NETDEV_UDP_TUNNEL_DROP_INFO: event ==) -> else {
    ...
    err = -ENOENT;
#[no_mangle]
pub unsafe extern "C" fn if(NTF_EXT_LEARNED: f->flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn vxlan_fdb_nh_flush(nh: *mut nexthop) {
...
#[no_mangle]
unsafe extern "C" fn vxlan_init_net(net: *mut net) -> __net_init int {
...
#[no_mangle]
unsafe extern "C" fn vxlan_exit_net(net: *mut net) -> void __net_exit {
...
#[no_mangle]
unsafe extern "C" fn vxlan_init_module() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn vxlan_cleanup_module() -> void __exit {
