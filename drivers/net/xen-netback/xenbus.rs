...
//! Source: drivers/net/xen-netback/xenbus.c
#![no_std]
...
//
    ...
    struct dentry *xen_netback_dbg_root = core::ptr::null_mut();
#[no_mangle]
unsafe extern "C" fn xenvif_read_io_ring(m: *mut seq_file, v: *mut c_void) -> c_int {
...
pub const BUFFER_SIZE: c_int = 32;
    static ssize_t
...
#[no_mangle]
unsafe extern "C" fn xenvif_io_ring_open(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenvif_ctrl_show(m: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenvif_debugfs_addif(vif: *mut xenvif) {
...
#[no_mangle]
unsafe extern "C" fn xenvif_debugfs_delif(vif: *mut xenvif) {
    static void xenvif_debugfs_delif(struct xenvif *vif)
    {
    debugfs_remove_recursive(vif.xenvif_dbg_root);
    vif.xenvif_dbg_root = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn backend_create_xenvif(be: *mut backend_info) -> c_int {
    static int backend_create_xenvif(struct backend_info *be)
    {
    int err;
    long handle;
...
#[no_mangle]
unsafe extern "C" fn backend_disconnect(be: *mut backend_info) {
    static void backend_disconnect(struct backend_info *be)
    {
    struct xenvif *vif = be.vif;
    if (vif) {
    let mut num_queues: c_uint = vif.num_queues;
...
//
    ...
    vfree(vif.queues);
    vif.queues = core::ptr::null_mut();
    xenvif_disconnect_ctrl(vif);
    }
    }
#[no_mangle]
unsafe extern "C" fn backend_connect(be: *mut backend_info) {
...
// bytes = ~0UL;
// usec = 0;
    ratestr = xenbus_read(XBT_NIL, dev.nodename, "rate", core::ptr::null_mut());
    if (IS_ERR(ratestr))
    return;
    s = ratestr;
...
unsafe extern "C" fn xen_net_read_mac(dev: *mut xenbus_device, mac[]: u8) -> c_int {
    ...
    let mut err: c_int = 0;
    char *node;
    let mut maxlen: unsigned = strlen(dev.nodename) + sizeof("/rate");
    ...
    vif.credit_watch.will_handle = core::ptr::null_mut();
    vif.credit_watch.callback = xen_net_rate_changed;
    err = register_xenbus_watch(&vif.credit_watch);
    if (err) {
    pr_err("Failed to set watcher %s\n", vif.credit_watch.node);
    kfree(node);
    vif.credit_watch.node = core::ptr::null_mut();
    vif.credit_watch.will_handle = core::ptr::null_mut();
    vif.credit_watch.callback = core::ptr::null_mut();
    }
    return err;
    }
#[no_mangle]
unsafe extern "C" fn xen_unregister_credit_watch(vif: *mut xenvif) {
    ...
    vif.mcast_ctrl_watch.will_handle = core::ptr::null_mut();
    ...
    kfree(node);
    vif.mcast_ctrl_watch.node = core::ptr::null_mut();
    vif.mcast_ctrl_watch.will_handle = core::ptr::null_mut();
    vif.mcast_ctrl_watch.callback = core::ptr::null_mut();
    }
    return err;
    }
#[no_mangle]
unsafe extern "C" fn xen_unregister_mcast_ctrl_watch(vif: *mut xenvif) {
...
#[no_mangle]
unsafe extern "C" fn xen_unregister_watchers(vif: *mut xenvif) {
    static void xen_unregister_watchers(struct xenvif *vif)
    {
...
#[no_mangle]
unsafe extern "C" fn unregister_hotplug_status_watch(be: *mut backend_info) {
    static void unregister_hotplug_status_watch(struct backend_info *be)
    {
    if (be.have_hotplug_status_watch) {
...
#[no_mangle]
unsafe extern "C" fn connect_ctrl_ring(be: *mut backend_info) -> c_int {
...
//
    ...
    unregister_hotplug_status_watch(be);
    err = xenbus_watch_pathfmt(dev, &be.hotplug_status_watch, core::ptr::null_mut(),
    ...
    vfree(be.vif.queues);
    be.vif.queues = core::ptr::null_mut();
    be.vif.num_queues = 0;
    ...
    {
    struct xenbus_device *dev = be.dev;
    let mut num_queues: c_uint = queue.vif.num_queues;
...
//
    ...
    "rx-ring-ref", "%lu", &rx_ring_ref, core::ptr::null_mut());
    if (err) {
    xenbus_dev_fatal(dev, err,
    "reading %s/ring-ref",
    xspath);
...
// Try split event channels first, then single event channel.
    ...
    "event-channel-rx", "%u", &rx_evtchn, core::ptr::null_mut());
    if (err < 0) {
...
#[no_mangle]
unsafe extern "C" fn read_xenbus_vif_flags(be: *mut backend_info) -> c_int {
    static int read_xenbus_vif_flags(struct backend_info *be)
    {
    struct xenvif *vif = be.vif;
    struct xenbus_device *dev = be.dev;
...
#[no_mangle]
unsafe extern "C" fn netback_remove(dev: *mut xenbus_device) {
...
// Multi-queue support: This is an optional feature.
    ...
    backend_switch_state(be, XenbusStateInitWait);
    script = xenbus_read(XBT_NIL, dev.nodename, "script", core::ptr::null_mut());
    if (IS_ERR(script)) {
    err = PTR_ERR(script);
...
#[no_mangle]
pub unsafe extern "C" fn xenvif_xenbus_init() -> c_int {
    int xenvif_xenbus_init(void)
    {
    return xenbus_register_backend(&netback_driver);
    }
#[no_mangle]
pub unsafe extern "C" fn xenvif_xenbus_fini() {
