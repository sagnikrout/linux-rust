...
//
#[no_mangle]
pub unsafe extern "C" fn mdio45_probe(mdio: *mut mdio_if_info, prtad: c_int) -> c_int {
...
//
    ...
    bool sense)
    {
    let mut old_val: c_int = mdio.mdio_read(mdio.dev, prtad, devad, addr);
    int new_val;
    if (old_val < 0)
    return old_val;
    if (sense)
...
//
#[no_mangle]
pub unsafe extern "C" fn mdio45_links_ok(mdio: *const mdio_if_info, mmd_mask: u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn mdio45_nway_restart(mdio: *const mdio_if_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mdio45_get_an(mdio: *const mdio_if_info, addr: u16) -> u32 {
...
//
    ...
    {
    int prtad, devad;
    let mut addr: u16 = mii_data.reg_num;
...
// Remap commonly-used MII registers.
    ...
    if (cmd == SIOCGMIIREG) {
    let mut rc: c_int = mdio.mdio_read(mdio.dev, prtad, devad, addr);
    if (rc < 0)
    return rc;
    mii_data.val_out = rc;
    return 0;
