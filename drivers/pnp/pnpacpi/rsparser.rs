...
//! Source: drivers/pnp/pnpacpi/rsparser.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn pnpacpi_add_irqresource(dev: *mut pnp_dev, r: *mut resource) {
...
//
    ...
    int expected_len)
    {
    let mut uuid_len: c_int = sizeof(vendor.uuid);
    let mut uuid_subtype: u8 = vendor.uuid_subtype;
    u8 *uuid = vendor.uuid;
    int actual_len;
...
#[no_mangle]
pub unsafe extern "C" fn pnpacpi_parse_allocated_resource(dev: *mut pnp_dev) -> c_int {
    int pnpacpi_parse_allocated_resource(struct pnp_dev *dev)
    {
    struct acpi_device *acpi_dev = dev.data;
    let mut handle: acpi_handle = acpi_dev.handle;
    acpi_status status;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct acpipnp_parse_option_s {
    pub dev: *mut pnp_dev,
    pub option_flags: c_uint,
}
    ...
    let mut option_flags: c_uint = parse_data.option_flags;
    switch (res.type) {
...
#[no_mangle]
pub unsafe extern "C" fn pnpacpi_parse_resource_option_data(dev: *mut pnp_dev) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn pnpacpi_supported_resource(res: *mut acpi_resource) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pnpacpi_type_resources(res: *mut acpi_resource, data: *mut c_void) -> acpi_status {
...
#[no_mangle]
pub unsafe extern "C" fn pnpacpi_encode_resources(dev: *mut pnp_dev, buffer: *mut acpi_buffer) -> c_int {
    int pnpacpi_encode_resources(struct pnp_dev *dev, struct acpi_buffer *buffer)
    {
    let mut i: c_int = 0;
// pnpacpi_build_resource_template allocates extra mem
    let mut res_cnt: c_int = (buffer.length - 1) / sizeof(struct acpi_resource) - 1;
    struct acpi_resource *resource = buffer.pointer;
    let mut port: c_uint = 0, irq = 0, dma = 0, mem = 0;
