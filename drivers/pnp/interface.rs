...
//! Source: drivers/pnp/interface.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pnp_info_buffer {
    pub /: *mut *mut *mut char buffer; / pointer to begin of buffer,
    pub /: *mut *mut *mut char curr; / current position in buffer,
    pub /: *mut *mut unsigned long size; / current size,
    pub /: *mut *mut unsigned long len; / total length of buffer,
    ...
    __printf(2, 3)
#[no_mangle]
unsafe extern "C" fn pnp_printf(buffer: *mut *mut pnp_info_buffer_t, fmt: *mut c_char, ...) -> c_int {
