...
//! Source: drivers/memory/tegra/tegra114.c
#![no_std]
...
//
    ...
    let mut fifo_size: c_uint = client.fifo_size;
...
#[no_mangle]
unsafe extern "C" fn tegra114_mc_icc_set(src: *mut icc_node, dst: *mut icc_node) -> c_int {
    static int tegra114_mc_icc_set(struct icc_node *src, struct icc_node *dst)
    {
    struct tegra_mc *mc = icc_provider_to_tegra_mc(src.provider);
    const struct tegra_mc_client *client = &mc.soc.clients[src.id];
    let mut peak_bandwidth: u64 = icc_units_to_bps(src.peak_bw);
//
