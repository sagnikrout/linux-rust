...
//! Source: drivers/memory/tegra/tegra210-emc-cc-r21021.c
#![no_std]
...
pub const DVFS_CLOCK_CHANGE_VERSION: c_int = 21021;
pub const EMC_PRELOCK_VERSION: c_int = 2101;
    enum {
    DVFS_SEQUENCE = 1,
...
//
pub const PTFV_DQSOSC_MOVAVG_C0D0U0_INDEX: c_int = 0;
pub const PTFV_DQSOSC_MOVAVG_C0D0U1_INDEX: c_int = 1;
pub const PTFV_DQSOSC_MOVAVG_C0D1U0_INDEX: c_int = 2;
pub const PTFV_DQSOSC_MOVAVG_C0D1U1_INDEX: c_int = 3;
pub const PTFV_DQSOSC_MOVAVG_C1D0U0_INDEX: c_int = 4;
pub const PTFV_DQSOSC_MOVAVG_C1D0U1_INDEX: c_int = 5;
pub const PTFV_DQSOSC_MOVAVG_C1D1U0_INDEX: c_int = 6;
pub const PTFV_DQSOSC_MOVAVG_C1D1U1_INDEX: c_int = 7;
pub const PTFV_DVFS_SAMPLES_INDEX: c_int = 9;
pub const PTFV_MOVAVG_WEIGHT_INDEX: c_int = 10;
pub const PTFV_CONFIG_CTRL_INDEX: c_int = 11;

//
// Do arithmetic in fixed point.
//
pub const MOVAVG_PRECISION_FACTOR: c_int = 100;
//
...
// Access a particular average.
    ...
    let mut rate_mhz: u32 = timing.rate / 1000;
    u32 tmdel;
...
// curr = measured;
    ...
    let mut rate_mhz: u32 = curr.rate / 1000;
...
#[no_mangle]
unsafe extern "C" fn tegra210_emc_r21021_periodic_compensation(emc: *mut tegra210_emc) -> u32 {
    ...
    let mut items: c_uint = ARRAY_SIZE(list), i;
    if (last.periodic_training) {
...
//
#[no_mangle]
unsafe extern "C" fn tegra210_emc_r21021_set_clock(emc: *mut tegra210_emc, clksrc: u32) {
    static void tegra210_emc_r21021_set_clock(struct tegra210_emc *emc, u32 clksrc)
    {
// state variables
    static bool fsp_for_next_freq;
// constant configuration parameters
    let mut save_restore_clkstop_pd: bool = true;
    let mut zqcal_before_cc_cutoff: u32 = 2400;
    let mut cya_allow_ref_cc: bool = false;
    let mut cya_issue_pc_ref: bool = false;
    let mut opt_cc_short_zcal: bool = true;
    let mut ref_b4_sref_en: bool = false;
    let mut tZQCAL_lpddr4: u32 = 1000000;
    let mut opt_short_zcal: bool = true;
    let mut opt_do_sw_qrst: bool = true;
    let mut opt_dvfs_mode: u32 = MAN_SR;
//
...
//
    ...
    let mut tFC_lpddr4: u32 = 1000 * next.dram_timings[T_FC_LPDDR4];
    u32 bg_reg_mode_change, enable_bglp_reg, enable_bg_reg;
    let mut opt_zcal_en_cc: bool = false, is_lpddr3 = false;
    let mut compensate_trimmer_applicable: bool = false;
    u32 emc_dbg, emc_cfg_pipe_clk, emc_pin;
    u32 src_clk_period, dst_clk_period; /* in picoseconds */
    let mut shared_zq_resistor: bool = false;
    u32 value, dram_type;
    let mut opt_dll_mode: u32 = 0;
    unsigned long delay;
    unsigned int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(is_lpddr3: dram_type == DRAM_TYPE_LPDDR2 ||) -> else {
    else if (dram_type == DRAM_TYPE_LPDDR2 || is_lpddr3)
    zq_wait_long = max(next.min_mrs_wait,
    div_o3(360000, dst_clk_period)) + 4;
#[no_mangle]
pub unsafe extern "C" fn if(DRAM_TYPE_DDR3: dram_type ==) -> else {
...
//
    ...
    EMC_ZQ_CAL, 0);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: shared_zq_resistor) -> else {
