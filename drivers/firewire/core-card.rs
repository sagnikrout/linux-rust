...
//! Source: drivers/firewire/core-card.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn fw_compute_block_crc(block: *mut __be32) -> c_int {
...
// ROM header, bus info block, root dir header, capabilities = 7 quadlets
    let mut config_rom_length: static size_t = 1 + 4 + 1 + 1;

pub const BIB_BUS_NAME: c_uint = 0x31333934 /* "1394" */;

pub const NODE_CAPABILITIES: c_uint = 0x0c0083c0 /* per IEEE 1394 clause 8.3.2.6.5.2 */;
//
...
#[no_mangle]
unsafe extern "C" fn generate_config_rom(card: *mut fw_card, config_rom: *mut __be32) {
...
#[no_mangle]
unsafe extern "C" fn update_config_roms() {
...
#[no_mangle]
unsafe extern "C" fn required_space(desc: *mut fw_descriptor) -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn fw_core_add_descriptor(desc: *mut fw_descriptor) -> c_int {
...
//
    while (i < desc.length) {
    let mut block_len: u16 = desc.data[i] >> 16;
//
...
#[no_mangle]
pub unsafe extern "C" fn fw_core_remove_descriptor(desc: *mut fw_descriptor) {
...
#[no_mangle]
unsafe extern "C" fn reset_bus(card: *mut fw_card, short_reset: bool) -> c_int {
    static int reset_bus(struct fw_card *card, bool short_reset)
    {
    let mut reg: c_int = short_reset ? 5 : 1;
    let mut bit: c_int = short_reset ? PHY_BUS_SHORT_RESET : PHY_BUS_RESET;
    trace_bus_reset_initiate(card.index, card.generation, short_reset);
    return card.driver.update_phy_reg(card, reg, 0, bit);
    }
#[no_mangle]
pub unsafe extern "C" fn fw_schedule_bus_reset(card: *mut fw_card, delayed: bool, short_reset: bool) {
...
#[no_mangle]
unsafe extern "C" fn br_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn allocate_broadcast_channel(card: *mut fw_card, generation: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn fw_schedule_bm_work(card: *mut fw_card, delay: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn contend_for_bm(card: *mut fw_card) -> enum bm_contention_outcome {
    static enum bm_contention_outcome contend_for_bm(struct fw_card *card)
    __must_hold(&card.lock)
    {
    let mut generation: c_int = card.generation;
    let mut local_id: c_int = card.local_node.node_id;
    __be32 data[2] = {
    cpu_to_be32(BUS_MANAGER_ID_NOT_REGISTERED),
    cpu_to_be32(local_id),
    };
    let mut grace: bool = time_is_before_jiffies64(card.reset_jiffies + msecs_to_jiffies(125));
...
// NOTE: It is likely that the quirk detection for IRM device has not done yet.
    ...
    case RCODE_COMPLETE:
    {
    let mut bm_id: c_int = be32_to_cpu(data[0]);
...
#[no_mangle]
unsafe extern "C" fn bm_work(work: *mut work_struct) {
    ...
    struct fw_node *root_node __free(node_unref) = core::ptr::null_mut();
    int root_id, new_root_id, irm_id, local_id;
    int expected_gap_count, generation;
    let mut stand_for_root: bool = false;
    spin_lock_irq(&card.lock);
    if (card.local_node == core::ptr::null_mut()) {
    spin_unlock_irq(&card.lock);
    return;
    }
    ...
    let mut result: enum bm_contention_outcome = contend_for_bm(card);
    switch (result) {
...
// Now investigate root node.
    ...
    if (root_device == core::ptr::null_mut()) {
...
// setting, let's do it.
    ...
    let mut card_gap_count: c_int = card.gap_count;
    spin_unlock_irq(&card.lock);
...
// Make sure that the cycle master sends cycle start packets.
    let mut data: __be32 = cpu_to_be32(CSR_STATE_BIT_CMSTR);
    ...
    {
    let mut index: static atomic_t = ATOMIC_INIT(-1);
    ...
    {
    struct workqueue_struct *isoc_wq __free(workqueue_destroy) = core::ptr::null_mut();
    struct workqueue_struct *async_wq __free(workqueue_destroy) = core::ptr::null_mut();
    int ret;
...
// response AR/AT contexts.
    ...
    if (ret < 0) {
    card.isoc_wq = core::ptr::null_mut();
    card.async_wq = core::ptr::null_mut();
    return ret;
    }
...
//
#[no_mangle]
unsafe extern "C" fn dummy_read_phy_reg(card: *mut fw_card, address: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dummy_send_request(card: *mut fw_card, packet: *mut fw_packet) {
    static void dummy_send_request(struct fw_card *card, struct fw_packet *packet)
    {
    packet.callback(packet, card, RCODE_CANCELLED);
    }
#[no_mangle]
unsafe extern "C" fn dummy_send_response(card: *mut fw_card, packet: *mut fw_packet) {
    static void dummy_send_response(struct fw_card *card, struct fw_packet *packet)
    {
    packet.callback(packet, card, RCODE_CANCELLED);
    }
#[no_mangle]
unsafe extern "C" fn dummy_cancel_packet(card: *mut fw_card, packet: *mut fw_packet) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dummy_read_csr(card: *mut fw_card, csr_offset: c_int) -> u32 {
    static u32 dummy_read_csr(struct fw_card *card, int csr_offset)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn dummy_write_csr(card: *mut fw_card, csr_offset: c_int, value: u32) {
...
#[no_mangle]
unsafe extern "C" fn dummy_set_iso_channels(ctx: *mut fw_iso_context, channels: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dummy_flush_queue_iso(ctx: *mut fw_iso_context) {
    static void dummy_flush_queue_iso(struct fw_iso_context *ctx)
    {
    }
#[no_mangle]
unsafe extern "C" fn dummy_flush_iso_completions(ctx: *mut fw_iso_context) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn fw_card_release(kref: *mut kref) {
...
#[no_mangle]
pub unsafe extern "C" fn fw_core_remove_card(card: *mut fw_card) {
    void fw_core_remove_card(struct fw_card *card)
    {
    let mut dummy_driver: fw_card_driver = dummy_driver_template;
    might_sleep();
...
//
#[no_mangle]
pub unsafe extern "C" fn fw_card_read_cycle_time(card: *mut fw_card, cycle_time: *mut u32) -> c_int {
