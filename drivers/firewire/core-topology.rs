...
//! Source: drivers/firewire/core-topology.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn update_hop_count(node: *mut fw_node) {
    static void update_hop_count(struct fw_node *node)
    {
    int depths[2] = { -1, -1 };
    let mut max_child_hops: c_int = 0;
    int i;
    for (i = 0; i < node.port_count; i++) {
    if (node.ports[i] == core::ptr::null_mut())
    continue;
...
#[no_mangle]
unsafe extern "C" fn free_fw_node(card: *mut fw_card, node: *mut fw_node, parent: *mut fw_node) {
...
//
    ...
    local_node = core::ptr::null_mut();
    node = core::ptr::null_mut();
    INIT_LIST_HEAD(&stack);
    stack_depth = 0;
    phy_id = 0;
    irm_node = core::ptr::null_mut();
    gap_count = phy_packet_self_id_zero_get_gap_count(*sid);
    beta_repeaters_present = false;
    while (enumerator.quadlet_count > 0) {
    let mut child_port_count: c_uint = 0;
    let mut parent_port_count: c_uint = 0;
    let mut total_port_count: c_uint = 0;
...
// Topology has changed - reset bus manager retry counter
    ...
    struct fw_node *node, struct fw_node *parent)
    {
    let mut b_path: c_int = (node.phy_speed == SCODE_BETA);
...
#[no_mangle]
pub unsafe extern "C" fn fw_destroy_nodes(card: *mut fw_card) {
...
#[no_mangle]
unsafe extern "C" fn move_tree(node0: *mut fw_node, node1: *mut fw_node, port: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn update_tree(card: *mut fw_card, root: *mut fw_node) {
...
#[no_mangle]
pub unsafe extern "C" fn if(node1->link_on: !node0->link_on &&) -> else {
    else if (!node0.link_on && node1.link_on)
    event = FW_NODE_LINK_ON;
#[no_mangle]
pub unsafe extern "C" fn if(node1->link_on: node1->initiated_reset &&) -> else {
...
//
    ...
    report_lost_node);
    node0.ports[i] = core::ptr::null_mut();
...
//
    ...
    {
    __be32 *map = buffer;
    let mut next_generation: u32 = be32_to_cpu(buffer[1]) + 1;
    let mut node_count: c_int = (root_node_id & 0x3f) + 1;
...
// old and new topologies.
    if (!is_next_generation(generation, card.generation) && card.local_node != core::ptr::null_mut()) {
...
// a stale node_id together with a current generation.
    ...
    card.color++;
    if (local_node == core::ptr::null_mut()) {
    fw_err(card, "topology build failed\n");
// FIXME: We need to issue a bus reset in this case.
    } else if (card.local_node == core::ptr::null_mut()) {
