...
//! Source: drivers/firewire/core-device.c
#![no_std]
...
pub const ROOT_DIR_OFFSET: c_int = 5;
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn fw_csr_iterator_next(ci: *mut fw_csr_iterator, key: *mut c_int, value: *mut c_int) -> c_int {
...
// value = *ci->p & 0xffffff;
    ...
    {
    struct fw_csr_iterator ci;
    let mut last_key: c_int = 0, key, value;
    fw_csr_iterator_init(&ci, directory);
    while (fw_csr_iterator_next(&ci, &key, &value)) {
...
#[no_mangle]
unsafe extern "C" fn textual_leaf_to_string(block: *const u32, buf: *mut c_char, size: usize) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn fw_csr_string(directory: *const u32, key: c_int, buf: *mut c_char, size: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn get_ids(directory: *const u32, id: *mut c_int) {
...
#[no_mangle]
unsafe extern "C" fn get_modalias_ids(unit: *const fw_unit, id: *mut c_int) {
    static void get_modalias_ids(const struct fw_unit *unit, int *id)
    {
    const u32 *root_directory = &fw_parent_device(unit).config_rom[ROOT_DIR_OFFSET];
    const u32 *directories[] = {core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut()};
...
#[no_mangle]
unsafe extern "C" fn match_ids(id_table: *const ieee1394_device_id, id: *mut c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn fw_unit_match(dev: *mut device, drv: *const device_driver) -> c_int {
    static int fw_unit_match(struct device *dev, const struct device_driver *drv)
    {
// We only allow binding to fw_units.
    return is_fw_unit(dev) && unit_match(dev, drv) != core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn fw_unit_probe(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fw_unit_remove(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn get_modalias(unit: *const fw_unit, buffer: *mut c_char, buffer_size: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fw_unit_uevent(dev: *const device, env: *mut kobj_uevent_env) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn fw_device_enable_phys_dma(device: *mut fw_device) -> c_int {
    int fw_device_enable_phys_dma(struct fw_device *device)
    {
    let mut generation: c_int = device.generation;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct config_rom_attribute {
    ...
    struct fw_csr_iterator ci;
    const u32 *directories[] = {core::ptr::null_mut(), core::ptr::null_mut()};
    int i, value = -1;
...
// Note that this function is also called by init_fw_attribute_group() with NULL pointer.
    ...
    { __ATTR(name, S_IRUGO, show_immediate, core::ptr::null_mut()), key }
...
// Note that this function is also called by init_fw_attribute_group() with NULL pointer.
    ...
    for (i = 0; i < ARRAY_SIZE(directories) && !!directories[i]; ++i) {
    let mut result: c_int = fw_csr_string(directories[i], attr.key, buf, bufsize);
// Detected.
    if (result >= 0) {
    ret = result;
...
// Strip trailing whitespace and add newline.
    ...
    { __ATTR(name, S_IRUGO, show_text_leaf, core::ptr::null_mut()), key }
    ...
    int i, j;
    for (j = 0; attrs[j].attr.name != core::ptr::null_mut(); j++)
    group.attrs[j] = &attrs[j].attr;
    for (i = 0; i < ARRAY_SIZE(config_rom_attributes); i++) {
    attr = &config_rom_attributes[i].attr;
    if (attr.show(dev, attr, core::ptr::null_mut()) < 0)
    continue;
    group.attrs[j++] = &attr.attr;
    }
    group.attrs[j] = core::ptr::null_mut();
    group.groups[0] = &group.group;
    group.groups[1] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn units_sprintf(buf: *mut c_char, directory: *const u32) -> c_int {
    static int units_sprintf(char *buf, const u32 *directory)
    {
    struct fw_csr_iterator ci;
    int key, value;
    let mut specifier_id: c_int = 0;
...
pub const CANON_OUI: c_uint = 0x000085;
#[no_mangle]
unsafe extern "C" fn detect_quirks_by_bus_information_block(bus_information_block: *const u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn detect_quirks_by_root_directory(root_directory: *const u32, length: c_uint) -> c_int {
    ...
    let mut index: c_uint = entry.matches[j].index;
    let mut value: c_uint = entry.matches[j].value;
...
#[no_mangle]
unsafe extern "C" fn read_rom(device: *mut fw_device, generation: c_int, speed: c_int, index: c_int, data: *mut u32) -> c_int {
    static int read_rom(struct fw_device *device, int generation, int speed, int index, u32 *data)
    {
    let mut offset: u64 = (CSR_REGISTER_BASE | CSR_CONFIG_ROM) + index * 4;
    int i, rcode;
// device->node_id, accessed below, must not be older than generation
    smp_rmb();
    for (i = 10; i < 100; i += 10) {
...
//
#[no_mangle]
unsafe extern "C" fn read_config_rom(device: *mut fw_device, generation: c_int) -> c_int {
    static int read_config_rom(struct fw_device *device, int generation)
    {
    struct fw_card *card = device.card;
    const u32 *new_rom, *old_rom __free(kfree) = core::ptr::null_mut();
    u32 *stack, *rom __free(kfree) = core::ptr::null_mut();
    u32 sp, key;
    int i, end, length, ret, speed;
    int quirks;
    rom = kmalloc(sizeof(*rom) * MAX_CONFIG_ROM_SIZE +
    sizeof(*stack) * MAX_CONFIG_ROM_SIZE, GFP_KERNEL);
    if (rom == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn fw_unit_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn is_fw_unit(dev: *const device) -> bool {
    static bool is_fw_unit(const struct device *dev)
    {
    return dev.type == &fw_unit_type;
    }
#[no_mangle]
unsafe extern "C" fn create_units(device: *mut fw_device) {
...
#[no_mangle]
unsafe extern "C" fn shutdown_unit(device: *mut device, data: *mut c_void) -> c_int {
...
//
pub const MAX_RETRIES: c_int = 10;

#[no_mangle]
unsafe extern "C" fn fw_device_shutdown(work: *mut work_struct) {
    ...
    device_for_each_child(&device.device, core::ptr::null_mut(), shutdown_unit);
    device_unregister(&device.device);
    xa_erase(&fw_device_xa, MINOR(device.device.devt));
    fw_device_put(device);
    }
#[no_mangle]
unsafe extern "C" fn fw_device_release(dev: *mut device) {
...
//
    ...
    fw_node_set_device(device.node, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn is_fw_device(dev: *const device) -> bool {
    static bool is_fw_device(const struct device *dev)
    {
    return dev.type == &fw_device_type;
    }
#[no_mangle]
unsafe extern "C" fn update_unit(dev: *mut device, data: *mut c_void) -> c_int {
    static int update_unit(struct device *dev, void *data)
    {
    struct fw_unit *unit = fw_unit(dev);
    struct fw_driver *driver = (struct fw_driver *)dev.driver;
    if (is_fw_unit(dev) && driver != core::ptr::null_mut() && driver.update != core::ptr::null_mut()) {
    device_lock(dev);
    driver.update(unit);
    device_unlock(dev);
    }
...
#[no_mangle]
unsafe extern "C" fn fw_device_update(work: *mut work_struct) {
    static void fw_device_update(struct work_struct *work)
    {
    struct fw_device *device = from_work(device, work, work.work);
    fw_device_cdev_update(device);
    device_for_each_child(&device.device, core::ptr::null_mut(), update_unit);
    }
    enum { BC_UNKNOWN = 0, BC_UNIMPLEMENTED, BC_IMPLEMENTED, };
#[no_mangle]
unsafe extern "C" fn set_broadcast_channel(device: *mut fw_device, generation: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn fw_device_set_broadcast_channel(dev: *mut device, gen: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compare_configuration_rom(dev: *mut device, data: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fw_device_init(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn fw_device_refresh(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn fw_device_workfn(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn fw_node_event(card: *mut fw_card, node: *mut fw_node, event: c_int) {
