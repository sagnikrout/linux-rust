...
//! Source: drivers/hv/mshv_root_hv_call.c
#![no_std]
...
pub const HV_INIT_PARTITION_DEPOSIT_PAGES: c_int = 208;
pub const HV_MAP_GPA_DEPOSIT_PAGES: c_int = 256;
pub const HV_UMAP_GPA_PAGES: c_int = 512;
    ...
    sizeof(u64))
#[no_mangle]
pub unsafe extern "C" fn hv_call_withdraw_memory(count: u64, node: c_int, partition_id: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hv_call_initialize_partition(partition_id: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hv_call_finalize_partition(partition_id: u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hv_call_delete_partition(partition_id: u64) -> c_int {
...
// Ask the hypervisor to map guest ram pages or the guest mmio space
    ...
    let mut ret: c_int = 0, done = 0;
    let mut page_count: u64 = page_struct_count;
    ...
    let mut rep_count: c_int = min(remain, HV_MAP_GPA_BATCH_SIZE);
    local_irq_save(irq_flags);
    input_page = *this_cpu_ptr(hyperv_pcpu_input_arg);
    ...
    let mut index: u64 = (done + i) << large_shift;
...
#[no_mangle]
pub unsafe extern "C" fn hv_call_map_mmio_pages(partition_id: u64, gfn: u64, mmio_spa: u64, numpgs: u64) -> c_int {
    ...
    return -EINVAL;
    return hv_do_map_gpa_hcall(partition_id, gfn, numpgs, flags, core::ptr::null_mut(),
    mmio_spa);
    }
    ...
    let mut rep_count: c_int = min(remain, HV_UMAP_GPA_PAGES);
    local_irq_save(irq_flags);
    input_page = *this_cpu_ptr(hyperv_pcpu_input_arg);
    input_page.target_partition_id = partition_id;
    input_page.target_gpa_base = gfn + (done << large_shift);
    ...
    0, input_page, core::ptr::null_mut());
    local_irq_restore(irq_flags);
    completed = hv_repcomp(status);
    if (!hv_result_success(status)) {
    ret = hv_result_to_errno(status);
    ...
    let mut remaining: c_ulong = count;
    int rep_count, i;
...
//
    ...
    status = hv_do_hypercall(HVCALL_ASSERT_VIRTUAL_INTERRUPT, input, core::ptr::null_mut());
    local_irq_restore(flags);
    return hv_result_to_errno(status);
    }
#[no_mangle]
pub unsafe extern "C" fn hv_call_delete_vp(partition_id: u64, vp_index: u32) -> c_int {
    int hv_call_delete_vp(u64 partition_id, u32 vp_index)
    {
    let mut input: union hv_input_delete_vp = {};
    u64 status;
    input.partition_id = partition_id;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: page_count) -> else {
    else if (page_count)
    ...
    status = hv_do_hypercall(control, input, core::ptr::null_mut());
    if (!hv_result_needs_memory(status)) {
    local_irq_restore(flags);
    ret = hv_result_to_errno(status);
    break;
...
#[no_mangle]
unsafe extern "C" fn mshv_use_overlay_gpfn() -> bool {
    ...
    {
    let mut ret: c_int = 0;
    struct page *allocated_page = core::ptr::null_mut();
...
// state_page = NULL;
    ...
    input.type = type;
    input.input_vtl = input_vtl;
    status = hv_do_hypercall(HVCALL_UNMAP_VP_STATE_PAGE, input, core::ptr::null_mut());
    local_irq_restore(flags);
    return hv_result_to_errno(status);
    }
    ...
    {
    let mut ret: c_int = hv_call_unmap_vp_state_page(partition_id, vp_index, type, input_vtl);
    ...
    status = hv_do_hypercall(HVCALL_CREATE_PORT, input, core::ptr::null_mut());
    local_irq_restore(flags);
    if (hv_result_success(status))
    break;
    if (!hv_result_needs_memory(status)) {
    ...
    {
    let mut input: union hv_input_delete_port = { 0 };
    int status;
    input.port_partition_id = port_partition_id;
    input.port_id = port_id;
    ...
    input.proximity_domain_info = hv_numa_node_to_pxm_info(node);
    status = hv_do_hypercall(HVCALL_CONNECT_PORT, input, core::ptr::null_mut());
    local_irq_restore(flags);
    if (hv_result_success(status))
    break;
    if (!hv_result_needs_memory(status)) {
    ...
    union hv_connection_id connection_id)
    {
    let mut input: union hv_input_disconnect_port = { 0 };
    int status;
    input.connection_partition_id = connection_partition_id;
    input.connection_id = connection_id;
    input.is_doorbell = 1;
    ...
    {
    let mut input: union hv_input_notify_port_ring_empty = { 0 };
    int status;
...
//
    ...
    status = hv_do_hypercall(HVCALL_MAP_STATS_PAGE2, input, core::ptr::null_mut());
    local_irq_restore(flags);
    ret = hv_result_to_errno(status);
    if (!ret)
    break;
...
// addr = NULL;
    ...
    input.type = type;
    input.identity = *identity;
    status = hv_do_hypercall(HVCALL_UNMAP_STATS_PAGE, input, core::ptr::null_mut());
    local_irq_restore(flags);
    return hv_result_to_errno(status);
    }
