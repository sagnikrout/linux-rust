...
//! Source: drivers/hv/mshv_common.c
#![no_std]
...
//
    ...
    int rep_count, i;
    let mut status: u64 = HV_STATUS_SUCCESS;
    unsigned long flags;
...
//
#[no_mangle]
unsafe extern "C" fn hv_initialize_sleep_states() -> c_int {
    ...
    status = hv_do_hypercall(HVCALL_SET_SYSTEM_PROPERTY, in, core::ptr::null_mut());
    local_irq_restore(flags);
    if (!hv_result_success(status)) {
...
#[no_mangle]
pub unsafe extern "C" fn hv_sleep_notifiers_register() {
...
//
#[no_mangle]
pub unsafe extern "C" fn hv_machine_power_off() {
    ...
    (void)hv_do_hypercall(HVCALL_ENTER_SLEEP_STATE, in, core::ptr::null_mut());
    local_irq_restore(flags);
