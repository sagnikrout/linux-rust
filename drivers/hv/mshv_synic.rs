...
//! Source: drivers/hv/mshv_synic.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn synic_event_ring_get_queued_port(sint_index: u32) -> u32 {
...
//
    ...
    let mut ptinfo: port_table_info = { 0 };
...
#[no_mangle]
unsafe extern "C" fn mshv_async_call_completion_isr(msg: *mut hv_message) -> bool {
...
#[no_mangle]
unsafe extern "C" fn kick_vp(vp: *mut mshv_vp) {
    ...
    let mut partition_id: u64 = msg.partition_id;
    ...
    while (true) {
    let mut vp_bank_idx: c_int = -1;
    let mut vp_bank_size: c_int = sizeof(*bank_contents) * BITS_PER_BYTE;
    int vp_index;
...
// This shouldn't happen, but just in case.
    ...
    {
    struct mshv_partition *partition = core::ptr::null_mut();
    struct mshv_vp *vp;
    int idx;
    rcu_read_lock();
    for (idx = 0; idx < msg.vp_count; idx++) {
    let mut partition_id: u64 = msg.partition_ids[idx];
    let mut vp_index: u32 = msg.vp_indexes[idx];
...
#[no_mangle]
pub unsafe extern "C" fn mshv_isr() {
    void mshv_isr(void)
    {
...
#[no_mangle]
unsafe extern "C" fn mshv_synic_cpu_init(cpu: c_uint) -> c_int {
...
//
    let mut vmbus_active: bool = hv_vmbus_exists();
//
...
#[no_mangle]
unsafe extern "C" fn mshv_synic_cpu_exit(cpu: c_uint) -> c_int {
...
// When VMBus is active it owns SCONTROL — leave it.
    ...
    {
    let mut connection_info: hv_connection_info = { 0 };
    let mut connection_id: union hv_connection_id = { 0 };
    struct port_table_info *port_table_info;
    let mut port_info: hv_port_info = { 0 };
    let mut port_id: union hv_port_id = { 0 };
    int ret;
...
#[no_mangle]
unsafe extern "C" fn mshv_percpu_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn mshv_acpi_setup_sint_irq() -> int __init {
    static int __init mshv_acpi_setup_sint_irq(void)
    {
    return acpi_register_gsi(core::ptr::null_mut(), mshv_sint_vector, ACPI_EDGE_SENSITIVE,
    ACPI_ACTIVE_HIGH);
    }
#[no_mangle]
unsafe extern "C" fn mshv_acpi_cleanup_sint_irq() {
...
#[no_mangle]
unsafe extern "C" fn mshv_sint_vector_setup() -> int __init {
    ...
    let mut input_vtl: union hv_input_vtl = { 0 };
    if (acpi_disabled)
    return -ENODEV;
...
#[no_mangle]
unsafe extern "C" fn mshv_sint_vector_cleanup() {
...
#[no_mangle]
pub unsafe extern "C" fn mshv_synic_init(dev: *mut device) -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn mshv_synic_exit() {
