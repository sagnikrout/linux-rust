...
//! Source: drivers/hv/mshv_root_main.c
#![no_std]
...
pub const HV_VP_COUNTER_ROOT_DISPATCH_THREAD_BLOCKED: c_int = 202;

pub const HV_VP_COUNTER_ROOT_DISPATCH_THREAD_BLOCKED: c_int = 95;
...
#[no_mangle]
unsafe extern "C" fn mshv_hvcall_is_async(code: u16) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mshv_passthru_hvcall_allowed(code: u16, pt_id: u64) -> bool {
    static bool mshv_passthru_hvcall_allowed(u16 code, u64 pt_id)
    {
    int i;
    let mut n: c_int = ARRAY_SIZE(mshv_passthru_hvcalls);
    ...
    void *input_pg = core::ptr::null_mut();
    void *output_pg = core::ptr::null_mut();
    u16 reps_completed;
    let mut pt_id: u64 = partition ? partition.pt_id : HV_PARTITION_ID_SELF;
...
#[no_mangle]
pub unsafe extern "C" fn is_ghcb_mapping_available() -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn mshv_run_vp_with_hyp_scheduler(vp: *mut mshv_vp) -> c_long {
    ...
    let mut count: usize = ARRAY_SIZE(suspend_regs);
// Resume VP execution
    ret = mshv_set_vp_registers(vp.vp_index, vp.vp_partition.pt_id,
    count, suspend_regs);
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn mshv_vp_interrupt_pending(vp: *mut mshv_vp) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn mshv_vp_dispatch_thread_blocked(vp: *mut mshv_vp) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mshv_run_vp_with_root_scheduler(vp: *mut mshv_vp) -> c_long {
...
//
#[no_mangle]
unsafe extern "C" fn mshv_handle_gpa_intercept(vp: *mut mshv_vp) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mshv_vp_handle_intercept(vp: *mut mshv_vp) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mshv_vp_ioctl_run_vp(vp: *mut mshv_vp, ret_msg: *mut void __user) -> c_long {
...
// Pin user pages so hypervisor can copy directly to them
    ...
    let mut user_addr: c_ulong = (user_pfn + completed) * PAGE_SIZE;
    let mut remaining: c_int = page_count - completed;
    ...
    u32 data_sz;
    let mut state_data: hv_vp_state_data = {};
    if (copy_from_user(&args, user_args, sizeof(args)))
    return -EFAULT;
...
// If the data is transmitted via pfns, delegate to helper
    ...
    let mut user_pfn: c_ulong = PFN_DOWN(args.buf_ptr);
    let mut page_count: usize = PFN_DOWN(args.buf_sz);
...
#[no_mangle]
unsafe extern "C" fn mshv_vp_fault(vmf: *mut vm_fault) -> vm_fault_t {
    static vm_fault_t mshv_vp_fault(struct vm_fault *vmf)
    {
...
#[no_mangle]
unsafe extern "C" fn mshv_vp_mmap(file: *mut file, vma: *mut vm_area_struct) -> c_int {
    static int mshv_vp_mmap(struct file *file, struct vm_area_struct *vma)
    {
...
#[no_mangle]
unsafe extern "C" fn mshv_init_async_handler(partition: *mut mshv_partition) -> c_int {
    static int mshv_init_async_handler(struct mshv_partition *partition)
    {
...
#[no_mangle]
unsafe extern "C" fn mshv_async_hvcall_handler(data: *mut c_void, status: *mut u64) {
    static void mshv_async_hvcall_handler(void *data, u64 *status)
    {
...
//
    ...
    let mut nr_pages: u64 = HVPFN_DOWN(mem.size);
...
//
#[no_mangle]
unsafe extern "C" fn mshv_prepare_pinned_region(region: *mut mshv_mem_region) -> c_int {
...
//
    ...
    region.start_gfn,
    region.nr_pages,
    HV_MAP_GPA_NO_ACCESS, core::ptr::null_mut());
    break;
...
// Paranoia check
    ...
    long ret, i;
    let mut hv_flags: union hv_gpa_page_access_state_flags = {};
    u8 hv_type_mask;
...
// zero the unused bits in the last byte(s) of the returned bitmap
    ...
    void __user *user_args)
    {
    struct mshv_user_irq_entry *entries = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn drain_all_vps(partition: *const mshv_partition) {
...
//
#[no_mangle]
unsafe extern "C" fn destroy_partition(partition: *mut mshv_partition) {
...
// Remove vps
    ...
    input_vtl_zero);
    vp.vp_register_page = core::ptr::null_mut();
    }
    (void)hv_unmap_vp_state_page(partition.pt_id,
    vp.vp_index,
    HV_VP_STATE_PAGE_INTERCEPT_MESSAGE,
    ...
    input_vtl_zero);
    vp.vp_intercept_msg_page = core::ptr::null_mut();
    if (vp.vp_ghcb_page) {
    (void)hv_unmap_vp_state_page(partition.pt_id,
    vp.vp_index,
    HV_VP_STATE_PAGE_GHCB,
    ...
    input_vtl_normal);
    vp.vp_ghcb_page = core::ptr::null_mut();
    }
    kfree(vp);
    partition.pt_vp_array[i] = core::ptr::null_mut();
    }
...
// pt_flags |= HV_PARTITION_CREATION_FLAG_SMT_ENABLED_GUEST;
    ...
    misc.this_device);
    case MSHV_ROOT_HVCALL:
    return mshv_ioctl_passthru_hvcall(core::ptr::null_mut(), false,
    (void __user *)arg);
    }
    return -ENOTTY;
    }
...
#[no_mangle]
unsafe extern "C" fn l1vh_retrieve_scheduler_type(out: *mut enum hv_scheduler_type) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn hv_retrieve_scheduler_type(out: *mut enum hv_scheduler_type) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn mshv_retrieve_scheduler_type(dev: *mut device) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn mshv_root_scheduler_init(cpu: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mshv_root_scheduler_cleanup(cpu: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mshv_init_vmm_caps(dev: *mut device) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn mshv_parent_partition_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn mshv_parent_partition_exit() -> void __exit {
    static void __exit mshv_parent_partition_exit(void)
    {
    hv_setup_mshv_handler(core::ptr::null_mut());
