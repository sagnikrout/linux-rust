...
//! Source: drivers/nvme/host/auth.c
#![no_std]
...
pub const CHAP_BUF_SIZE: c_int = 4096;
    static struct kmem_cache *nvme_chap_buf_cache;
    static mempool_t *nvme_chap_buf_pool;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nvme_dhchap_queue_context {
    pub entry: list_head,
    pub auth_work: work_struct,
    pub ctrl: *mut nvme_ctrl,
    pub dh_tfm: *mut crypto_kpp,
    pub transformed_key: *mut nvme_dhchap_key,
    pub buf: *mut c_void,
    pub qid: c_int,
    pub error: c_int,
    pub s1: u32,
    ...
    pub sc_c: u8,
    pub hash_len: usize,
    pub c1: [u8; NVME_AUTH_MAX_DIGEST_SIZE],
    pub c2: [u8; NVME_AUTH_MAX_DIGEST_SIZE],
    pub response: [u8; NVME_AUTH_MAX_DIGEST_SIZE],
    pub ctrl_key: *mut u8,
    pub host_key: *mut u8,
    pub sess_key: *mut u8,
    pub ctrl_key_len: c_int,
    pub host_key_len: c_int,
    pub sess_key_len: c_int,
}

    static struct workqueue_struct *nvme_auth_wq;
#[no_mangle]
pub unsafe extern "C" fn ctrl_max_dhchaps(ctrl: *mut nvme_ctrl) -> c_int {
    ...
    let mut flags: nvme_submit_flags_t = NVME_SUBMIT_RETRY;
    ...
    ret = __nvme_submit_sync_cmd(q, &cmd, core::ptr::null_mut(), data, data_len,
...
pub unsafe extern "C" fn if(0: ret <) -> else {
    ...
    let mut size: usize = sizeof(*data) + sizeof(union nvmf_auth_protocol);
    let mut dh_list_offset: u8 = NVME_AUTH_DHCHAP_MAX_DH_IDS;
    ...
    let mut dhvlen: u16 = le16_to_cpu(data.dhvlen);
    let mut size: usize = sizeof(*data) + data.hl + dhvlen;
...
// Reset dh_tfm if it can't be reused
    ...
    let mut ret: c_int = PTR_ERR(chap.dh_tfm);
    dev_warn(ctrl.device,
    ...
    dev_warn(ctrl.device,
    "qid %d: invalid DH value for core::ptr::null_mut() DH\n",
    chap.qid);
    chap.status = NVME_AUTH_DHCHAP_FAILURE_INCORRECT_PAYLOAD;
    return -EPROTO;
    }
    ...
    let mut size: usize = sizeof(*data);
    ...
    let mut size: usize = sizeof(*data) + chap.hash_len;
    if (size > CHAP_BUF_SIZE) {
    chap.status = NVME_AUTH_DHCHAP_FAILURE_INCORRECT_PAYLOAD;
    return -EINVAL;
    }
...
// Just print out information for the admin queue
    ...
    chap.transformed_key = core::ptr::null_mut();
    return ret;
    }
    } else {
...
#[no_mangle]
unsafe extern "C" fn nvme_auth_reset_dhchap(chap: *mut nvme_dhchap_queue_context) {
    static void nvme_auth_reset_dhchap(struct nvme_dhchap_queue_context *chap)
    {
    nvme_auth_free_key(chap.transformed_key);
    chap.transformed_key = core::ptr::null_mut();
    kfree_sensitive(chap.host_key);
    chap.host_key = core::ptr::null_mut();
    chap.host_key_len = 0;
    kfree_sensitive(chap.ctrl_key);
    chap.ctrl_key = core::ptr::null_mut();
    chap.ctrl_key_len = 0;
    kfree_sensitive(chap.sess_key);
    chap.sess_key = core::ptr::null_mut();
    chap.sess_key_len = 0;
    chap.status = 0;
    chap.error = 0;
    chap.s1 = 0;
    ...
    chap.buf = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn nvme_auth_free_dhchap(chap: *mut nvme_dhchap_queue_context) {
...
#[no_mangle]
pub unsafe extern "C" fn nvme_auth_revoke_tls_key(ctrl: *mut nvme_ctrl) {
    ...
    ctrl.opts.tls_key = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn nvme_queue_auth_work(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn nvme_auth_negotiate(ctrl: *mut nvme_ctrl, qid: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nvme_auth_wait(ctrl: *mut nvme_ctrl, qid: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvme_ctrl_auth_work(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn nvme_auth_init_ctrl(ctrl: *mut nvme_ctrl) -> c_int {
    ...
    ctrl.ctrl_key = core::ptr::null_mut();
    err_free_dhchap_secret:
    nvme_auth_free_key(ctrl.host_key);
    ctrl.host_key = core::ptr::null_mut();
    return ret;
    }
    EXPORT_SYMBOL_GPL(nvme_auth_init_ctrl);
#[no_mangle]
pub unsafe extern "C" fn nvme_auth_stop(ctrl: *mut nvme_ctrl) {
...
#[no_mangle]
pub unsafe extern "C" fn nvme_auth_free(ctrl: *mut nvme_ctrl) {
...
#[no_mangle]
pub unsafe extern "C" fn nvme_init_auth() -> int __init {
    ...
    CHAP_BUF_SIZE, 0, SLAB_HWCACHE_ALIGN, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn nvme_exit_auth() -> void __exit {
