...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_ctrl_options {
    pub mask: c_int,
    pub wwnn: u64,
    pub wwpn: u64,
    pub roles: u32,
    ...
    { NVMF_OPT_ERR,		core::ptr::null_mut()		}
    };
#[no_mangle]
unsafe extern "C" fn fcloop_verify_addr(s: *mut substring_t) -> c_int {
    static int fcloop_verify_addr(substring_t *s)
    {
    let mut blen: usize = s.to - s.from + 1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_lport_priv {
    pub lport: *mut fcloop_lport,
}

// The port is already being removed, avoid double free
pub const PORT_DELETED: c_int = 0;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_rport {
    pub remoteport: *mut nvme_fc_remote_port,
    pub targetport: *mut nvmet_fc_target_port,
    pub nport: *mut fcloop_nport,
    pub lport: *mut fcloop_lport,
    pub lock: spinlock_t,
    pub ls_list: list_head,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_nport {
    pub rport: *mut fcloop_rport,
    pub tport: *mut fcloop_tport,
    pub lport: *mut fcloop_lport,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_lsreq {
    pub lsreq: *mut nvmefc_ls_req,
    pub ls_rsp: nvmefc_ls_rsp,
    pub status: c_int,
    pub /: *mut *mut list_head ls_list; / fcloop_rport->ls_list,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_rscn {
    pub tport: *mut fcloop_tport,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_fcpreq {
    pub tport: *mut fcloop_tport,
    pub fcpreq: *mut nvmefc_fcp_req,
    pub reqlock: spinlock_t,
    pub status: u16,
    pub inistate: u32,
    pub active: bool,
    pub aborted: bool,
    pub ref: refcount_t,
    pub fcp_rcv_work: work_struct,
    pub abort_rcv_work: work_struct,
    pub tio_done_work: work_struct,
    pub tgt_fcp_req: nvmefc_tgt_fcp_req,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct fcloop_ini_fcpreq {
    pub fcpreq: *mut nvmefc_fcp_req,
    pub tfcp_req: *mut fcloop_fcpreq,
    pub inilock: spinlock_t,
}
...
//
    ...
    {
    struct fcloop_ini_fcpreq *inireq = core::ptr::null_mut();
    if (fcpreq) {
    inireq = fcpreq.private;
    spin_lock(&inireq.inilock);
    inireq.tfcp_req = core::ptr::null_mut();
...
pub const DROP_OPCODE_MASK: c_uint = 0x00FF;
// fabrics opcode will have a bit set above 1st byte
    let mut drop_opcode: static int = -1;
...
//
#[no_mangle]
unsafe extern "C" fn check_for_drop(tfcp_req: *mut fcloop_fcpreq) -> c_int {
...
// the abort handler will call fcloop_call_host_done
    ...
    case INI_IO_ABORTED:
    fcpreq = tfcp_req.fcpreq;
    tfcp_req.fcpreq = core::ptr::null_mut();
    break;
...
//
    ...
    struct nvmefc_fcp_req *fcpreq;
    let mut rsplen: u32 = 0, xfrlen = 0;
    let mut fcp_err: c_int = 0, active, aborted;
    let mut op: u8 = tgt_fcpreq.op;
    unsigned long flags;
    spin_lock_irqsave(&tfcp_req.reqlock, flags);
    fcpreq = tfcp_req.fcpreq;
...
//
    ...
    struct fcloop_ini_fcpreq *inireq = fcpreq.private;
    struct fcloop_fcpreq *tfcp_req;
    let mut abortio: bool = true;
    unsigned long flags;
    spin_lock(&inireq.inilock);
    tfcp_req = inireq.tfcp_req;
    if (tfcp_req) {
    if (!fcloop_tfcp_req_get(tfcp_req))
    tfcp_req = core::ptr::null_mut();
    }
    spin_unlock(&inireq.inilock);
    if (!tfcp_req) {
...
//
    ...
    {
    struct fcloop_rport *rport = remoteport.private;
    let mut put_port: bool = false;
    unsigned long flags;
    flush_work(&rport.ls_work);
    spin_lock_irqsave(&fcloop_lock, flags);
    if (!test_and_set_bit(PORT_DELETED, &rport.flags))
    put_port = true;
    rport.nport.rport = core::ptr::null_mut();
    spin_unlock_irqrestore(&fcloop_lock, flags);
    if (put_port) {
    ...
    put_port = true;
    tport.nport.tport = core::ptr::null_mut();
    spin_unlock_irqrestore(&fcloop_lock, flags);
    if (put_port) {
...
pub const FCLOOP_HW_QUEUES: c_int = 4;
pub const FCLOOP_SGL_SEGS: c_int = 256;
pub const FCLOOP_DMABOUND_4G: c_uint = 0xFFFFFFFF;
...
// everything there ?
    ...
    ret = nvme_fc_register_localport(&pinfo, &fctemplate, core::ptr::null_mut(), &localport);
    if (!ret) {
// success
...
// free only if we're going to fail
    ...
    struct fcloop_ctrl_options *opts;
    unsigned long flags;
    let mut opts_mask: u32 = (remoteport) ? RPORT_OPTS : TGTPORT_OPTS;
    int ret;
    opts = kzalloc_obj(*opts);
    if (!opts)
    return core::ptr::null_mut();
...
// success
    ...
    rport.targetport = (nport.tport) ?  nport.tport.targetport : core::ptr::null_mut();
    if (nport.tport) {
    ...
    nport.tport.remoteport = core::ptr::null_mut();
    nport.rport = core::ptr::null_mut();
    return rport;
    }
    static int
    ...
    ret = nvmet_fc_register_targetport(&tinfo, &tgttemplate, core::ptr::null_mut(),
    &targetport);
    if (ret) {
    fcloop_nport_put(nport);
    return ret;
...
// success
    ...
    tport.remoteport = (nport.rport) ?  nport.rport.remoteport : core::ptr::null_mut();
    if (nport.rport)
    ...
    nport.rport.targetport = core::ptr::null_mut();
    nport.tport = core::ptr::null_mut();
    return tport;
    }
    static int
...
//
    ...
    static DEVICE_ATTR(add_local_port, 0200, core::ptr::null_mut(), fcloop_create_local_port);
    static DEVICE_ATTR(del_local_port, 0200, core::ptr::null_mut(), fcloop_delete_local_port);
    static DEVICE_ATTR(add_remote_port, 0200, core::ptr::null_mut(), fcloop_create_remote_port);
    static DEVICE_ATTR(del_remote_port, 0200, core::ptr::null_mut(), fcloop_delete_remote_port);
    static DEVICE_ATTR(add_target_port, 0200, core::ptr::null_mut(), fcloop_create_target_port);
    static DEVICE_ATTR(del_target_port, 0200, core::ptr::null_mut(), fcloop_delete_target_port);
    static DEVICE_ATTR(set_cmd_drop, 0200, core::ptr::null_mut(), fcloop_set_cmd_drop);
...
#[no_mangle]
unsafe extern "C" fn fcloop_init() -> int __init {
    ...
    &fcloop_class, core::ptr::null_mut(), MKDEV(0, 0), core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn fcloop_exit() -> void __exit {
