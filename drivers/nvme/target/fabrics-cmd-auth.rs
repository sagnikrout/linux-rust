...
//! Source: drivers/nvme/target/fabrics-cmd-auth.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn nvmet_auth_expired_work(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn nvmet_auth_sq_init(sq: *mut nvmet_sq) {
...
#[no_mangle]
unsafe extern "C" fn nvmet_auth_negotiate(req: *mut nvmet_req, d: *mut c_void, tl: u32) -> u8 {
...
// Secure concatenation can only be enabled on the admin queue
    ...
    let mut host_hmac_id: u8 = data.auth_protocol[0].dhchap.idlist[i];
    ...
    let mut tmp_dhgid: c_int = data.auth_protocol[0].dhchap.idlist[i + 30];
    ...
    pr_debug("%s: ctrl %d qid %d: core::ptr::null_mut() DH group invalid "
...
#[no_mangle]
unsafe extern "C" fn nvmet_auth_reply(req: *mut nvmet_req, d: *mut c_void, tl: u32) -> u8 {
...
//
    ...
    req.sq.dhchap_c2 = core::ptr::null_mut();
    } else if (!data.cvalid)
    req.sq.authenticated = true;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn nvmet_auth_failure2(d: *mut c_void) -> u8 {
...
#[no_mangle]
pub unsafe extern "C" fn nvmet_auth_send_data_len(req: *mut nvmet_req) -> u32 {
    u32 nvmet_auth_send_data_len(struct nvmet_req *req)
    {
    return le32_to_cpu(req.cmd.auth_send.tl);
    }
#[no_mangle]
pub unsafe extern "C" fn nvmet_execute_auth_send(req: *mut nvmet_req) {
...
// Validate negotiation parameters
    ...
    req.sq.dhchap_step != NVME_AUTH_DHCHAP_MESSAGE_FAILURE2) {
    let mut auth_expire_secs: c_ulong = ctrl.kato ? ctrl.kato : 120;
...
#[no_mangle]
unsafe extern "C" fn nvmet_auth_challenge(req: *mut nvmet_req, d: *mut c_void, al: c_int) -> c_int {
    static int nvmet_auth_challenge(struct nvmet_req *req, void *d, int al)
    {
    struct nvmf_auth_dhchap_challenge_data *data = d;
    struct nvmet_ctrl *ctrl = req.sq.ctrl;
    let mut ret: c_int = 0;
    let mut hash_len: c_int = nvme_auth_hmac_hash_len(ctrl.shash_id);
    let mut data_size: c_int = sizeof(*d) + hash_len;
    if (ctrl.dh_tfm)
...
#[no_mangle]
unsafe extern "C" fn nvmet_auth_success1(req: *mut nvmet_req, d: *mut c_void, al: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvmet_auth_failure1(req: *mut nvmet_req, d: *mut c_void, al: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn nvmet_auth_receive_data_len(req: *mut nvmet_req) -> u32 {
    u32 nvmet_auth_receive_data_len(struct nvmet_req *req)
    {
    struct nvmet_ctrl *ctrl = req.sq.ctrl;
    let mut al: u32 = le32_to_cpu(req.cmd.auth_receive.al);
    u32 min_len;
//
...
#[no_mangle]
pub unsafe extern "C" fn nvmet_execute_auth_receive(req: *mut nvmet_req) {
    ...
    nvmet_auth_sq_free(req.sq);
#[no_mangle]
pub unsafe extern "C" fn if(NVME_AUTH_DHCHAP_MESSAGE_FAILURE1: req->sq->dhchap_step ==) -> else {
    nvmet_auth_sq_free(req.sq);
    nvmet_ctrl_fatal_error(ctrl);
    }
    nvmet_req_complete(req, status);
