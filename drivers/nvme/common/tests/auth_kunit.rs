...
//! Source: drivers/nvme/common/tests/auth_kunit.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nvme_auth_test_values {
    pub hmac_id: u8,
    pub hash_len: usize,
    pub expected_psk: [u8; NVME_AUTH_MAX_DIGEST_SIZE],
    pub expected_psk_digest: *mut c_char,
    pub expected_tls_psk: [u8; NVME_AUTH_MAX_DIGEST_SIZE],
}

#[no_mangle]
unsafe extern "C" fn kfree_action(ptr: *mut c_void) {
    static void kfree_action(void *ptr)
    {
    kfree(ptr);
    }
#[no_mangle]
unsafe extern "C" fn kunit_add_kfree_action(test: *mut kunit, ptr: *mut c_void) {
...
//
    ...
    {
    let mut hmac_id: u8 = vals.hmac_id;
    let mut hash_len: usize = vals.hash_len;
    let mut skey_len: usize = hash_len;
    ...
    u8 *psk = core::ptr::null_mut(), *tls_psk = core::ptr::null_mut();
    char *psk_digest = core::ptr::null_mut();
    size_t psk_len;
    int ret;
    ...
    if (vals.expected_psk_digest == core::ptr::null_mut()) {
//
...
#[no_mangle]
unsafe extern "C" fn test_nvme_auth_derive_tls_psk_hmac_sha256(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_nvme_auth_derive_tls_psk_hmac_sha384(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_nvme_auth_derive_tls_psk_hmac_sha512(test: *mut kunit) {
...
// nvme_auth_generate_digest() doesn't support SHA-512 yet.
    .expected_psk_digest = core::ptr::null_mut(),
    };
    test_nvme_auth_derive_tls_psk(test, &vals);
    }
