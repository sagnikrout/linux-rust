...
//! Source: drivers/usb/phy/phy-ulpi.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ulpi_set_otg_flags(phy: *mut usb_phy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ulpi_set_fc_flags(phy: *mut usb_phy) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(ULPI_FC_LS: phy->flags &) -> else {
    else if (phy.flags & ULPI_FC_LS)
    flags |= ULPI_FUNC_CTRL_LOW_SPEED;
#[no_mangle]
pub unsafe extern "C" fn if(ULPI_FC_FS4LS: phy->flags &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(ULPI_FC_OP_DIS_NRZI: phy->flags &) -> else {
    else if (phy.flags & ULPI_FC_OP_DIS_NRZI)
    flags |= ULPI_FUNC_CTRL_OPMODE_DISABLE_NRZI;
#[no_mangle]
pub unsafe extern "C" fn if(ULPI_FC_OP_NSYNC_NEOP: phy->flags &) -> else {
...
#[no_mangle]
unsafe extern "C" fn ulpi_set_ic_flags(phy: *mut usb_phy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ulpi_set_flags(phy: *mut usb_phy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ulpi_check_integrity(phy: *mut usb_phy) -> c_int {
    static int ulpi_check_integrity(struct usb_phy *phy)
    {
    int ret, i;
    let mut val: c_uint = 0x55;
    for (i = 0; i < 2; i++) {
...
#[no_mangle]
unsafe extern "C" fn ulpi_init(phy: *mut usb_phy) -> c_int {
    static int ulpi_init(struct usb_phy *phy)
    {
    int i, vid, pid, ret;
    let mut ulpi_id: u32 = 0;
    for (i = 0; i < 4; i++) {
...
#[no_mangle]
unsafe extern "C" fn ulpi_set_host(otg: *mut usb_otg, host: *mut usb_bus) -> c_int {
    static int ulpi_set_host(struct usb_otg *otg, struct usb_bus *host)
    {
    struct usb_phy *phy = otg.usb_phy;
    let mut flags: c_uint = usb_phy_io_read(phy, ULPI_IFC_CTRL);
    if (!host) {
    ...
    flags |= ULPI_IFC_CTRL_6_PIN_SERIAL_MODE;
#[no_mangle]
pub unsafe extern "C" fn if(ULPI_IC_3PIN_SERIAL: phy->flags &) -> else {
    else if (phy.flags & ULPI_IC_3PIN_SERIAL)
    flags |= ULPI_IFC_CTRL_3_PIN_SERIAL_MODE;
#[no_mangle]
pub unsafe extern "C" fn if(ULPI_IC_CARKIT: phy->flags &) -> else {
    else if (phy.flags & ULPI_IC_CARKIT)
    flags |= ULPI_IFC_CTRL_CARKITMODE;
    return usb_phy_io_write(phy, flags, ULPI_IFC_CTRL);
    }
#[no_mangle]
unsafe extern "C" fn ulpi_set_vbus(otg: *mut usb_otg, on: bool) -> c_int {
    static int ulpi_set_vbus(struct usb_otg *otg, bool on)
    {
    struct usb_phy *phy = otg.usb_phy;
    let mut flags: c_uint = usb_phy_io_read(phy, ULPI_OTG_CTRL);
