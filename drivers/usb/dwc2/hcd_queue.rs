...
//! Source: drivers/usb/dwc2/hcd_queue.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_periodic_channel_available(hsotg: *mut dwc2_hsotg) -> c_int {
...
// At this point we have a valid point for first one
    ...
    let mut ith_start: c_int = start + interval_bits * i;
    let mut ith_end: c_int = end + interval_bits * i;
    int ret;
...
//
    ...
    let mut period_start: c_int = period * bits_per_period;
    let mut period_end: c_int = period_start + bits_per_period;
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_qh_print(str: *const c_char, data: *mut c_void) {
...
//
    ...
    struct dwc2_qh *qh)
    {
    let mut print_data: dwc2_qh_print_data = { hsotg, qh };
    int i;
//
...
//
    ...
    int search_slice)
    {
    let mut slices: c_int = DIV_ROUND_UP(qh.device_us, DWC2_US_PER_SLICE);
    unsigned long *map = dwc2_get_ls_map(hsotg, qh);
    int slice;
    if (!map)
    return -EINVAL;
...
//
    ...
    struct dwc2_qh *qh)
    {
    let mut bytecount: c_int = qh.maxp_mult * qh.maxp;
    int ls_search_slice;
    let mut err: c_int = 0;
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_uframe_schedule_hs(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_uframe_schedule_ls(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_uframe_schedule(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !qh->do_split) -> else {
    else if (!qh.do_split)
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_uframe_unschedule(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_pick_first_frame(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_do_reserve(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_do_unreserve(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_unreserve_timer_fn(t: *mut timer_list) {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_schedule_periodic(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dwc2_wait_timer_fn(t: *mut hrtimer) -> enum hrtimer_restart {
...
//
    ...
    {
    let mut dev_speed: c_int = dwc2_host_get_speed(hsotg, urb.priv);
    let mut ep_type: u8 = dwc2_hcd_get_pipe_type(&urb.pipe_info);
    let mut ep_is_in: bool = !!dwc2_hcd_is_pipe_in(&urb.pipe_info);
    let mut ep_is_isoc: bool = (ep_type == USB_ENDPOINT_XFER_ISOC);
    let mut ep_is_int: bool = (ep_type == USB_ENDPOINT_XFER_INT);
    let mut hprt: u32 = dwc2_readl(hsotg, HPRT0);
    let mut prtspd: u32 = (hprt & HPRT0_SPD_MASK) >> HPRT0_SPD_SHIFT;
    bool do_split = (prtspd == HPRT0_SPD_HIGH_SPEED &&
    dev_speed != USB_SPEED_HIGH);
    let mut maxp: c_int = dwc2_hcd_get_maxp(&urb.pipe_info);
    let mut maxp_mult: c_int = dwc2_hcd_get_maxp_mult(&urb.pipe_info);
    let mut bytecount: c_int = maxp_mult * maxp;
    char *speed, *type;
// Initialize QH
    qh.hsotg = hsotg;
...
// Compute scheduling parameters once and save them
    let mut host_speed: c_int = do_split ? USB_SPEED_HIGH : dev_speed;
...
//
#[no_mangle]
pub unsafe extern "C" fn dwc2_hcd_qh_free(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) {
...
#[no_mangle]
pub unsafe extern "C" fn if(qh->dw_align_buf: hsotg->unaligned_cache &&) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn dwc2_hcd_qh_add(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dwc2_hcd_qh_unlink(hsotg: *mut dwc2_hsotg, qh: *mut dwc2_qh) {
...
//
    ...
    {
    let mut old_frame: u16 = qh.next_active_frame;
    let mut prev_frame_number: u16 = dwc2_frame_num_dec(frame_number, 1);
    let mut missed: c_int = 0;
    u16 incr;
//
...
//
    ...
    {
    let mut missed: c_int = 0;
    let mut interval: u16 = qh.host_interval;
    let mut prev_frame_number: u16 = dwc2_frame_num_dec(frame_number, 1);
...
//
    ...
    let mut ideal_start: u16 = qh.start_active_frame;
    int periods_in_map;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn dwc2_hcd_qtd_init(qtd: *mut dwc2_qtd, urb: *mut dwc2_hcd_urb) {
