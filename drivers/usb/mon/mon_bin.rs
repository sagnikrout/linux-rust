...
//
pub const SETUP_LEN: c_int = 8;
// ioctl macros
pub const MON_IOC_MAGIC: c_uint = 0x92;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mon_bin_hdr {
    pub /: *mut *mut u64 id; / URB ID - from submission to callback,
    pub /: *mut *mut unsigned char type; / Same as in text API; extensible.,
    pub /: *mut *mut unsigned char xfer_type; / ISO, Intr, Control, Bulk,
    pub /: *mut *mut unsigned char epnum; / Endpoint number and transfer direction,
    pub /: *mut *mut unsigned char devnum; / Device address,
    pub /: *mut *mut unsigned short busnum; / Bus number,
    pub flag_setup: c_char,
    pub flag_data: c_char,
    pub /: *mut *mut s64 ts_sec; / ktime_get_real_ts64,
    pub /: *mut *mut s32 ts_usec; / ktime_get_real_ts64,
    pub status: c_int,
    pub /: *mut *mut unsigned int len_urb; / Length of data (submitted or actual),
    pub /: *mut *mut unsigned int len_cap; / Delivered length,
    union {
    pub /: *mut *mut unsigned char setup[SETUP_LEN]; / Only for Control S-type,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iso_rec {
    ...
    pub xfer_flags: c_uint,
    pub /: *mut *mut unsigned int ndesc; / Actual number of ISO descriptors,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mon_bin_isodesc {
    pub iso_status: c_int,
    pub iso_off: c_uint,
    pub iso_len: c_uint,
    pub _pad: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mon_bin_get {
    pub /: *mut *mut *mut mon_bin_hdr __user hdr; / Can be 48 bytes or 64.,
    pub data: *mut void __user,
    pub /: *mut *mut size_t alloc; / Length of data (can be zero),
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mon_bin_mfetch {
    pub /: *mut *mut *mut u32 __user offvec; / Vector of events fetched,
    pub /: *mut *mut u32 nfetch; / Number of events to fetch (out: fetched),
    pub /: *mut *mut u32 nflush; / Number of events to flush,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mon_bin_get32 {
    pub hdr32: u32,
    pub data32: u32,
    pub alloc32: u32,
}
...
pub const PKT_ALIGN: c_int = 64;
pub const PKT_SIZE: c_int = 64;

// max number of USB bus supported
pub const MON_BIN_MAX_MINOR: c_int = 128;
//
// The buffer: map of used pages.
//
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mon_pgmap {
    pub pg: *mut page,
    pub /: *mut *mut *mut unsigned char ptr; / XXX just use page_to_virt everywhere?,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mon_reader_bin {
// The buffer: one per open.
    pub /: *mut *mut spinlock_t b_lock; / Protect b_cnt, b_in,
    pub /: *mut *mut unsigned int b_size; / Current size of the buffer - bytes,
    pub /: *mut *mut unsigned int b_cnt; / Bytes used,
    pub /: *mut *mut unsigned int b_in, b_out; / Offsets into buffer - bytes,
    pub /: *mut *mut unsigned int b_read; / Amount of read data in curr. pkt.,
    pub /: *mut *mut *mut mon_pgmap b_vec; / The map array,
    pub /: *mut *mut wait_queue_head_t b_wait; / Wait for data here,
    pub /: *mut *mut mutex fetch_lock; / Protect b_read, b_out,
    pub mmap_active: c_int,
...
//
#[no_mangle]
unsafe extern "C" fn mon_buff_area_shrink(rp: *mut mon_reader_bin, size: c_uint) {
...
//
#[no_mangle]
unsafe extern "C" fn mon_buff_area_free(rp: *mut mon_reader_bin, size: c_uint) {
    ...
    {
    if (urb.setup_packet == core::ptr::null_mut())
    return 'Z';
...
#[no_mangle]
unsafe extern "C" fn mon_bin_submit(data: *mut c_void, urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn mon_bin_complete(data: *mut c_void, urb: *mut urb, status: c_int) {
...
#[no_mangle]
unsafe extern "C" fn mon_bin_error(data: *mut c_void, urb: *mut urb, error: c_int) {
...
#[no_mangle]
unsafe extern "C" fn mon_bin_open(inode: *mut inode, file: *mut file) -> c_int {
    ...
    if (mbus == core::ptr::null_mut()) {
    mutex_unlock(&mon_lock);
    return -ENODEV;
    }
    if (mbus != &mon_bus0 && mbus.u_bus == core::ptr::null_mut()) {
    ...
    if ((rp.b_vec = kzalloc(size, GFP_KERNEL)) == core::ptr::null_mut()) {
    rc = -ENOMEM;
    goto err_allocvec;
    }
...
#[no_mangle]
unsafe extern "C" fn mon_bin_release(inode: *mut inode, file: *mut file) -> c_int {
    static int mon_bin_release(struct inode *inode, struct file *file)
    {
    struct mon_reader_bin *rp = file.private_data;
    let mut mbus: *mut mon_bus = rp.r.m_bus;
    mutex_lock(&mon_lock);
    if (mbus.nreaders <= 0) {
    ...
    {
    struct mon_reader_bin *rp = file.private_data;
    let mut hdrbytes: c_uint = PKT_SZ_API0;
    unsigned long flags;
    struct mon_bin_hdr *ep;
    unsigned int offset;
    size_t step_len;
...
//
#[no_mangle]
unsafe extern "C" fn mon_bin_flush(rp: *mut mon_reader_bin, nevents: unsigned) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mon_bin_queued(rp: *mut mon_reader_bin) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mon_bin_ioctl(file: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
...
//
#[no_mangle]
unsafe extern "C" fn mon_bin_vma_open(vma: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn mon_bin_vma_close(vma: *mut vm_area_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn mon_bin_vma_fault(vmf: *mut vm_fault) -> vm_fault_t {
...
#[no_mangle]
unsafe extern "C" fn mon_bin_mmap(filp: *mut file, vma: *mut vm_area_struct) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mon_bin_wait_event(file: *mut file, rp: *mut mon_reader_bin) -> c_int {
    static int mon_bin_wait_event(struct file *file, struct mon_reader_bin *rp)
    {
...
#[no_mangle]
unsafe extern "C" fn mon_alloc_buff(map: *mut mon_pgmap, npages: c_int) -> c_int {
    static int mon_alloc_buff(struct mon_pgmap *map, int npages)
    {
    int n;
    unsigned long vaddr;
...
#[no_mangle]
unsafe extern "C" fn mon_free_buff(map: *mut mon_pgmap, npages: c_int) {
    static void mon_free_buff(struct mon_pgmap *map, int npages)
    {
    int n;
    for (n = 0; n < npages; n++)
...
#[no_mangle]
pub unsafe extern "C" fn mon_bin_add(mbus: *mut mon_bus, ubus: *const usb_bus) -> c_int {
    int mon_bin_add(struct mon_bus *mbus, const struct usb_bus *ubus)
    {
    struct device *dev;
    let mut minor: unsigned = ubus? ubus.busnum: 0;
    if (minor >= MON_BIN_MAX_MINOR)
    return 0;
    dev = device_create(&mon_bin_class, ubus ? ubus.controller : core::ptr::null_mut(),
    MKDEV(MAJOR(mon_bin_dev0), minor), core::ptr::null_mut(),
    "usbmon%d", minor);
    if (IS_ERR(dev))
    return 0;
    mbus.classdev = dev;
...
#[no_mangle]
pub unsafe extern "C" fn mon_bin_del(mbus: *mut mon_bus) {
    void mon_bin_del(struct mon_bus *mbus)
    {
    device_destroy(&mon_bin_class, mbus.classdev.devt);
    }
#[no_mangle]
pub unsafe extern "C" fn mon_bin_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn mon_bin_exit() {
