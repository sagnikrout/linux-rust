...
//! Source: drivers/usb/atm/cxacru.c
#![no_std]
...
pub const CXACRU_EP_CMD: c_uint = 0x01	/* Bulk/interrupt in/out */;
pub const CXACRU_EP_DATA: c_uint = 0x02	/* Bulk in/out */;

// Addresses
pub const PLLFCLK_ADDR: c_uint = 0x00350068;
pub const PLLBCLK_ADDR: c_uint = 0x0035006c;
pub const SDRAMEN_ADDR: c_uint = 0x00350010;
pub const FW_ADDR: c_uint = 0x00801000;
pub const BR_ADDR: c_uint = 0x00180600;
pub const SIG_ADDR: c_uint = 0x00180500;
pub const BR_STACK_ADDR: c_uint = 0x00187f10;
// Values
pub const SDRAM_ENA: c_uint = 0x1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxacru_modem_type {
    pub pll_f_clk: u32,
    pub pll_b_clk: u32,
    pub boot_rom_patch: c_int,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxacru_data {
    pub usbatm: *mut usbatm_data,
    pub modem_type: *const cxacru_modem_type,
    pub line_status: c_int,
    pub adsl_state_serialize: mutex,
    pub adsl_status: c_int,
    pub poll_work: delayed_work,
    pub card_info: [u32; CXINF_MAX],
    pub poll_state_serialize: mutex,
    pub poll_state: enum cxacru_poll_state,
// control handles
    pub cm_serialize: mutex,
    pub rcv_buf: *mut u8,
    pub snd_buf: *mut u8,
    pub rcv_urb: *mut urb,
    pub snd_urb: *mut urb,
    pub rcv_done: completion,
    pub snd_done: completion,
}
...
#[no_mangle]
unsafe extern "C" fn cxacru_sysfs_showattr_u32(value: u32, buf: *mut c_char) -> isize {
    static ssize_t cxacru_sysfs_showattr_u32(u32 value, char *buf)
    {
    return sprintf(buf, "%u\n", value);
    }
#[no_mangle]
unsafe extern "C" fn cxacru_sysfs_showattr_s8(value: i8, buf: *mut c_char) -> isize {
    static ssize_t cxacru_sysfs_showattr_s8(s8 value, char *buf)
    {
    return sprintf(buf, "%d\n", value);
    }
#[no_mangle]
unsafe extern "C" fn cxacru_sysfs_showattr_dB(value: i16, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn cxacru_sysfs_showattr_bool(value: u32, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn cxacru_sysfs_showattr_LINK(value: u32, buf: *mut c_char) -> isize {
    static ssize_t cxacru_sysfs_showattr_LINK(u32 value, char *buf)
    {
    static char *str[] = { core::ptr::null_mut(), "not connected", "connected", "lost" };
    if (unlikely(value >= ARRAY_SIZE(str) || str[value] == core::ptr::null_mut()))
    return sprintf(buf, "%u\n", value);
    return sprintf(buf, "%s\n", str[value]);
    }
#[no_mangle]
unsafe extern "C" fn cxacru_sysfs_showattr_LINE(value: u32, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn cxacru_sysfs_showattr_MODU(value: u32, buf: *mut c_char) -> isize {
...
//
    ...
    struct cxacru_data *instance = to_usbatm_driver_data(
    to_usb_interface(dev));
    if (instance == core::ptr::null_mut() || instance.usbatm.atm_dev == core::ptr::null_mut())
    return -ENODEV;
    ...
    to_usb_interface(dev));
    int ret;
    let mut poll: c_int = -1;
    char str_cmd[8];
    let mut len: c_int = strlen(buf);
    ...
    ret = cxacru_cm(instance, CM_REQUEST_CHIP_ADSL_LINE_STOP, core::ptr::null_mut(), 0, core::ptr::null_mut(), 0);
    if (ret < 0) {
...
// be sure that restart will have an effect.
    ...
    ret = cxacru_cm(instance, CM_REQUEST_CHIP_ADSL_LINE_START, core::ptr::null_mut(), 0, core::ptr::null_mut(), 0);
    if (ret < 0) {
    atm_err(instance.usbatm, "change adsl state:"
...
//
    ...
    (u8 *) data, 4 + num * 8, core::ptr::null_mut(), 0);
    if (ret < 0) {
...
#[no_mangle]
unsafe extern "C" fn cxacru_blocking_completion(urb: *mut urb) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cxacru_timer {
    pub timer: timer_list,
    pub urb: *mut urb,
}

#[no_mangle]
unsafe extern "C" fn cxacru_timeout_kill(t: *mut timer_list) {
...
// actual_length = urb->actual_length;
    ...
    int ret, actlen;
    int offb, offd;
    let mut stride: c_int = CMD_PACKET_SIZE - 4;
    u8 *wbuf = instance.snd_buf;
    u8 *rbuf = instance.rcv_buf;
    let mut wbuflen: c_int = ((wsize - 1) / stride + 1) * CMD_PACKET_SIZE;
    let mut rbuflen: c_int = ((rsize - 1) / stride + 1) * CMD_PACKET_SIZE;
...
// handle wsize == 0
    ...
    ret = cxacru_start_wait_urb(instance.snd_urb, &instance.snd_done, core::ptr::null_mut());
    if (ret < 0) {
    if (printk_ratelimit())
...
// check the return status and copy the data to the output buffer, if needed
    ...
    int offb;
    unsigned int offd;
    let mut stride: c_int = CMD_PACKET_SIZE / (4 * 2) - 1;
    let mut buflen: c_int = ((size - 1) / stride + 1 + size * 2) * 4;
    buf = kmalloc(buflen, GFP_KERNEL);
    if (!buf)
    return -ENOMEM;
    ret = cxacru_cm(instance, cm, core::ptr::null_mut(), 0, (u8 *) buf, buflen);
    if (ret < 0)
    goto cleanup;
// len > 0 && len % 4 == 0 guaranteed by cxacru_cm()
    len = ret / 4;
    for (offb = 0; offb < len; ) {
    let mut l: c_int = le32_to_cpu(buf[offb++]);
...
#[no_mangle]
unsafe extern "C" fn cxacru_card_status(instance: *mut cxacru_data) -> c_int {
    static int cxacru_card_status(struct cxacru_data *instance)
    {
    let mut ret: c_int = cxacru_cm(instance, CM_REQUEST_CARD_GET_STATUS, core::ptr::null_mut(), 0, core::ptr::null_mut(), 0);
    ...
    int ret;
    let mut start_polling: c_int = 1;
    dev_dbg(&intf.dev, "%s\n", __func__);
// Read MAC address
    ret = cxacru_cm(instance, CM_REQUEST_CARD_GET_MAC_ADDRESS, core::ptr::null_mut(), 0,
...
#[no_mangle]
unsafe extern "C" fn cxacru_poll_status(work: *mut work_struct) {
    static void cxacru_poll_status(struct work_struct *work)
    {
    ...
    u8 *buf;
    int offd, offb;
    let mut stride: c_int = CMD_PACKET_SIZE - 8;
    buf = (u8 *) __get_free_page(GFP_KERNEL);
    if (!buf)
    return -ENOMEM;
    offb = offd = 0;
    do {
    let mut l: c_int = min_t(int, stride, size - offd);
    buf[offb++] = fw;
    buf[offb++] = l;
    buf[offb++] = code1;
    buf[offb++] = code2;
    ...
    buf, offb, core::ptr::null_mut(), CMD_TIMEOUT);
    if (ret < 0) {
...
// Signature
    ...
    ret = cxacru_fw(usb_dev, FW_GOTO_MEM, 0x0, 0x0, FW_ADDR, core::ptr::null_mut(), 0);
    }
    if (ret) {
...
// Delay to allow firmware to start up.
    ...
    ret = cxacru_cm(instance, CM_REQUEST_CARD_GET_STATUS, core::ptr::null_mut(), 0, core::ptr::null_mut(), 0);
    if (ret < 0) {
...
// instance init
    ...
    if (!instance) {
    usb_dbg(usbatm_instance, "cxacru_unbind: core::ptr::null_mut() instance!\n");
    return;
    }
    mutex_lock(&instance.poll_state_serialize);
...
// stop polling from being stopped or started
    ...
    free_page((unsigned long) instance.rcv_buf);
    kfree(instance);
    usbatm_instance.driver_data = core::ptr::null_mut();
    }
