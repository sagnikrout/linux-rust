...
//! Source: drivers/usb/misc/chaoskey.c
#![no_std]
...
// Version Information
    ...
    MODULE_LICENSE("GPL");
pub const CHAOSKEY_VENDOR_ID: c_uint = 0x1d50	/* OpenMoko */;
pub const CHAOSKEY_PRODUCT_ID: c_uint = 0x60c6	/* ChaosKey */;
pub const ALEA_VENDOR_ID: c_uint = 0x12d8	/* Araneus */;
pub const ALEA_PRODUCT_ID: c_uint = 0x0001	/* Alea I */;

pub const USB_CHAOSKEY_MINOR_BASE: c_int = 0;

// IOWARRIOR_MINOR_BASE + 16, not official yet
pub const USB_CHAOSKEY_MINOR_BASE: c_int = 224;
...
pub struct chaoskey {
    ...
    pub lock: mutex,
    pub rng_lock: mutex,
    pub /: *mut *mut int open; / open count,
    pub /: *mut *mut bool present; / device not disconnected,
    pub /: *mut *mut bool reading; / ongoing IO,
    pub /: *mut *mut bool reads_started; / track first read for Alea,
    pub /: *mut *mut int size; / size of buf,
    pub /: *mut *mut int valid; / bytes of buf read,
    pub /: *mut *mut int used; / bytes of buf consumed,
    pub /: *mut *mut *mut char name; / product + serial,
    pub /: *mut *mut hwrng hwrng; / Embedded for hwrng,
    pub /: *mut *mut int hwrng_registered; / registered with hwrng API,
    pub /: *mut *mut wait_queue_head_t wait_q; / for timeouts,
    pub /: *mut *mut *mut urb urb; / for performing IO,
    pub buf: *mut c_char,
}

#[no_mangle]
unsafe extern "C" fn chaoskey_free(dev: *mut chaoskey) {
...
#[no_mangle]
unsafe extern "C" fn chaoskey_disconnect(interface: *mut usb_interface) {
...
#[no_mangle]
unsafe extern "C" fn chaoskey_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn chaoskey_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn chaos_read_callback(urb: *mut urb) {
    static void chaos_read_callback(struct urb *urb)
    {
...
//
#[no_mangle]
unsafe extern "C" fn _chaoskey_fill(dev: *mut chaoskey) -> c_int {
...
// Let the device go back to sleep eventually
    ...
    {
    struct chaoskey *dev;
    let mut read_count: isize = 0;
    int this_time;
    let mut result: c_int = 0;
    unsigned long remain;
    dev = file.private_data;
    if (dev == core::ptr::null_mut() || !dev.present)
    return -ENODEV;
...
#[no_mangle]
unsafe extern "C" fn chaoskey_resume(interface: *mut usb_interface) -> c_int {
