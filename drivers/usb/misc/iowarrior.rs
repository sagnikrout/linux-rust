...
//! Source: drivers/usb/misc/iowarrior.c
#![no_std]
...
pub const USB_VENDOR_ID_CODEMERCS: c_int = 1984;
// low speed iowarrior
pub const USB_DEVICE_ID_CODEMERCS_IOW40: c_uint = 0x1500;
pub const USB_DEVICE_ID_CODEMERCS_IOW24: c_uint = 0x1501;
pub const USB_DEVICE_ID_CODEMERCS_IOWPV1: c_uint = 0x1511;
pub const USB_DEVICE_ID_CODEMERCS_IOWPV2: c_uint = 0x1512;
// full speed iowarrior
pub const USB_DEVICE_ID_CODEMERCS_IOW56: c_uint = 0x1503;
// fuller speed iowarrior
pub const USB_DEVICE_ID_CODEMERCS_IOW28: c_uint = 0x1504;
pub const USB_DEVICE_ID_CODEMERCS_IOW28L: c_uint = 0x1505;
pub const USB_DEVICE_ID_CODEMERCS_IOW100: c_uint = 0x1506;
// OEMed devices
pub const USB_DEVICE_ID_CODEMERCS_IOW24SAG: c_uint = 0x158a;
pub const USB_DEVICE_ID_CODEMERCS_IOW56AM: c_uint = 0x158b;
// Get a minor range for your devices from the usb maintainer

pub const IOWARRIOR_MINOR_BASE: c_int = 0;

// interrupt input queue size
pub const MAX_INTERRUPT_BUFFER: c_int = 16;
//
...
//
pub const MAX_WRITES_IN_FLIGHT: c_int = 4;
...
pub struct iowarrior {
    ...
    pub /: *mut *mut *mut usb_endpoint_descriptor int_out_endpoint; / endpoint for reading (needed for IOW56 only),
    pub /: *mut *mut *mut usb_endpoint_descriptor int_in_endpoint; / endpoint for reading,
    pub /: *mut *mut *mut urb int_in_urb; / the urb for reading data,
    pub /: *mut *mut *mut unsigned char int_in_buffer; / buffer for data to be read,
    pub /: *mut *mut unsigned char serial_number; / to detect lost packages,
    pub /: *mut *mut *mut *mut unsigned char read_queue; / size is MAX_INTERRUPT_BUFFER  packet size,
    pub read_wait: wait_queue_head_t,
    pub /: *mut *mut wait_queue_head_t write_wait; / wait-queue for writing to the device,
    pub /: *mut *mut atomic_t write_busy; / number of write-urbs submitted,
    pub read_idx: core::sync::atomic::AtomicI32,
    pub intr_idx: core::sync::atomic::AtomicI32,
    pub /: *mut *mut atomic_t overflow_flag; / signals an index 'rollover',
    pub /: *mut *mut int present; / this is 1 as long as the device is connected,
    pub /: *mut *mut int opened; / this is 1 if the device is currently open,
    pub /: *mut *mut char chip_serial[9]; / the serial number string of the chip connected,
    pub /: *mut *mut int report_size; / number of bytes in a report,
    pub product_id: u16,
...
//
#[no_mangle]
unsafe extern "C" fn iowarrior_callback(urb: *mut urb) {
...
//
#[no_mangle]
unsafe extern "C" fn iowarrior_write_callback(urb: *mut urb) {
...
//
#[no_mangle]
pub unsafe extern "C" fn iowarrior_delete(kref: *mut kref) {
...
// ---------------------
#[no_mangle]
unsafe extern "C" fn read_index(dev: *mut iowarrior) -> c_int {
...
//
    ...
    struct iowarrior *dev;
    int retval;
    char *buf = core::ptr::null_mut();	/* for IOW24 and IOW56 we need a buffer */
    struct urb *int_out_urb = core::ptr::null_mut();
    dev = file.private_data;
    retval = mutex_lock_interruptible(&dev.mutex);
    if (retval < 0)
    return -EINTR;
...
//
    ...
    unsigned long arg)
    {
    struct iowarrior *dev = core::ptr::null_mut();
    __u8 *buffer;
...
// serial number string has been read earlier 8 chars or empty string
    ...
    if (cfg_descriptor == core::ptr::null_mut()) {
...
//
#[no_mangle]
unsafe extern "C" fn iowarrior_open(inode: *mut inode, file: *mut file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn iowarrior_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iowarrior_poll(file: *mut file, wait: *mut *mut poll_table) -> __poll_t {
...
//
#[no_mangle]
unsafe extern "C" fn iowarrior_disconnect(interface: *mut usb_interface) {
