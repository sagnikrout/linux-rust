...
//! Source: drivers/usb/misc/usbtest.c
#![no_std]
...
// -------------------------------------------------------------------------
    let mut override_alt: static int = -1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbtest_param_32 {
// inputs
    pub /: *mut *mut __u32 test_num; / 0..(TEST_CASES-1),
    pub iterations: __u32,
    pub length: __u32,
    pub vary: __u32,
    pub sglen: __u32,
// outputs
    pub duration_sec: __s32,
    pub duration_usec: __s32,
}
...
// inputs
    ...
    pub sglen: __u32,
// outputs
    pub duration_sec: __s64,
    pub duration_usec: __s64,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbtest_info {
    pub name: *const c_char,
    pub /: *mut *mut u8 ep_in; / bulk/intr source,
    pub /: *mut *mut u8 ep_out; / bulk/intr sink,
    pub autoconf:1: unsigned,
    pub ctrl_out:1: unsigned,
    pub /: *mut *mut unsigned iso:1; / try iso in/out,
    pub /: *mut *mut unsigned intr:1; / try interrupt in/out,
    pub alt: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbtest_dev {
    pub intf: *mut usb_interface,
    pub info: *mut usbtest_info,
    pub in_pipe: c_int,
    pub out_pipe: c_int,
    pub in_iso_pipe: c_int,
    pub out_iso_pipe: c_int,
    pub in_int_pipe: c_int,
    pub out_int_pipe: c_int,
    pub iso_out: *mut *mut usb_endpoint_descriptor iso_in,,
    pub int_out: *mut *mut usb_endpoint_descriptor int_in,,
    pub lock: mutex,
pub const TBUF_SIZE: c_int = 256;
    pub buf: *mut u8,
}
...
pub const GUARD_BYTE: c_uint = 0xA5;
pub const MAX_SGLEN: c_int = 128;
// -------------------------------------------------------------------------
...
// out = e;
    ...
    iso_in = iso_out = core::ptr::null_mut();
    int_in = int_out = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn simple_callback(urb: *mut urb) {
    ...
    return urb;
    if (bInterval)
    usb_fill_int_urb(urb, udev, pipe, core::ptr::null_mut(), bytes, complete_fn,
    core::ptr::null_mut(), bInterval);
    else
    usb_fill_bulk_urb(urb, udev, pipe, core::ptr::null_mut(), bytes, complete_fn,
    core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn get_maxpacket(udev: *mut usb_device, pipe: c_int) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn ss_isoc_get_packet_num(udev: *mut usb_device, pipe: c_int) -> c_int {
    static int ss_isoc_get_packet_num(struct usb_device *udev, int pipe)
    {
    struct usb_host_endpoint *ep = usb_pipe_endpoint(udev, pipe);
#[no_mangle]
pub unsafe extern "C" fn USB_SS_MULT(_arg: ep->ss_ep_comp.bmAttributes) -> return {
    return USB_SS_MULT(ep.ss_ep_comp.bmAttributes)
// (1 + ep->ss_ep_comp.bMaxBurst);
    }
#[no_mangle]
unsafe extern "C" fn simple_fill_buf(urb: *mut urb) {
    static void simple_fill_buf(struct urb *urb)
    {
    unsigned	i;
    u8		*buf = urb.transfer_buffer;
    let mut len: unsigned = urb.transfer_buffer_length;
    unsigned	maxpacket;
    switch (pattern) {
    default:
    fallthrough;
...
#[no_mangle]
pub unsafe extern "C" fn buffer_offset(buf: *mut c_void) -> c_ulong {
    static inline unsigned long buffer_offset(void *buf)
    {
    return (unsigned long)buf & (ARCH_KMALLOC_MINALIGN - 1);
    }
#[no_mangle]
unsafe extern "C" fn check_guard_bytes(tdev: *mut usbtest_dev, urb: *mut urb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn simple_check_buf(tdev: *mut usbtest_dev, urb: *mut urb) -> c_int {
    static int simple_check_buf(struct usbtest_dev *tdev, struct urb *urb)
    {
    unsigned	i;
    u8		expected;
    u8		*buf = urb.transfer_buffer;
    let mut len: unsigned = urb.actual_length;
    let mut maxpacket: unsigned = get_maxpacket(urb.dev, urb.pipe);
    let mut ret: c_int = check_guard_bytes(tdev, urb);
    if (ret)
    return ret;
    for (i = 0; i < len; i++, buf++) {
    switch (pattern) {
...
#[no_mangle]
unsafe extern "C" fn simple_free_urb(urb: *mut urb) {
    static void simple_free_urb(struct urb *urb)
    {
    let mut offset: c_ulong = buffer_offset(urb.transfer_buffer);
    if (urb.transfer_flags & URB_NO_TRANSFER_DMA_MAP)
    usb_free_coherent(
    urb.dev,
    ...
    let mut max: c_int = urb.transfer_buffer_length;
    ...
    if (vary) {
    let mut len: c_int = urb.transfer_buffer_length;
    len += vary;
    len %= max;
    if (len == 0)
...
//
#[no_mangle]
unsafe extern "C" fn free_sglist(sg: *mut scatterlist, nents: c_int) {
    ...
    let mut n_size: c_uint = 0;
    unsigned		i;
    let mut size: unsigned = max;
    unsigned		maxpacket =
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sg_timeout {
    pub timer: timer_list,
    pub req: *mut usb_sg_request,
}

#[no_mangle]
unsafe extern "C" fn sg_timeout(t: *mut timer_list) {
...
//
    let mut realworld: static unsigned = 1;
    module_param(realworld, uint, 0);
    MODULE_PARM_DESC(realworld, "clear to demand stricter spec compliance");
#[no_mangle]
unsafe extern "C" fn get_altsetting(dev: *mut usbtest_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_altsetting(dev: *mut usbtest_dev, alternate: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_good_config(tdev: *mut usbtest_dev, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_good_ext(tdev: *mut usbtest_dev, buf: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_good_ss_cap(tdev: *mut usbtest_dev, buf: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_good_con_id(tdev: *mut usbtest_dev, buf: *mut u8) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ch9_postconfig(dev: *mut usbtest_dev) -> c_int {
...
// [real world] get_config unimplemented if there's only one
    ...
    let mut expected: c_int = udev.actconfig.desc.bConfigurationValue;
...
//
    ...
    struct usb_bos_descriptor *bos = core::ptr::null_mut();
    struct usb_dev_cap_header *header = core::ptr::null_mut();
...
// and sometimes [9.2.6.6] speed dependent descriptors
    ...
    struct usb_qualifier_descriptor *d = core::ptr::null_mut();
...
// might not have [9.6.2] any other-speed configs [9.6.4]
    if (d) {
    let mut max: unsigned = d.bNumConfigurations;
    for (i = 0; i < max; i++) {
    retval = usb_get_descriptor(udev,
...
pub struct ctrl_ctx {
    pub lock: spinlock_t,
    pub dev: *mut usbtest_dev,
    pub complete: completion,
    pub count: unsigned,
    pub pending: unsigned,
    pub status: c_int,
    pub urb: *mut urb,
    pub param: *mut usbtest_param_32,
    pub last: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn ctrl_complete(urb: *mut urb) {
...
#[no_mangle]
pub unsafe extern "C" fn if(-ECONNRESET: status !=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(-EPIPE: subcase->number == 12 && status ==) -> else {
...
//
    ...
    let mut pipe: c_int = usb_rcvctrlpipe(udev, 0);
    unsigned		len;
    struct urb		*u;
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn unlink1_callback(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn unlink1(dev: *mut usbtest_dev, pipe: c_int, size: c_int, async: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn unlink_simple(dev: *mut usbtest_dev, pipe: c_int, len: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn unlink_queued_callback(urb: *mut urb) {
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn verify_not_halted(tdev: *mut usbtest_dev, ep: c_int, urb: *mut urb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn verify_halted(tdev: *mut usbtest_dev, ep: c_int, urb: *mut urb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_halt(tdev: *mut usbtest_dev, ep: c_int, urb: *mut urb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test_toggle_sync(tdev: *mut usbtest_dev, ep: c_int, urb: *mut urb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn halt_simple(dev: *mut usbtest_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn toggle_sync_simple(dev: *mut usbtest_dev) -> c_int {
    ...
    struct urb		*urb;
    struct usb_device	*udev = testdev_to_usbdev(dev);
    let mut maxp: unsigned = get_maxpacket(udev, dev.out_pipe);
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct transfer_context {
    pub count: unsigned,
    pub pending: unsigned,
    pub lock: spinlock_t,
    pub done: completion,
    pub submit_error: c_int,
    pub errors: c_ulong,
    pub packet_count: c_ulong,
    pub dev: *mut usbtest_dev,
    pub is_iso: bool,
}

#[no_mangle]
unsafe extern "C" fn complicated_callback(urb: *mut urb) {
    static void complicated_callback(struct urb *urb)
    {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: urb->status !=) -> else {
    else if (urb.status != 0)
    ctx.errors += (ctx.is_iso ? urb.number_of_packets : 1);
#[no_mangle]
pub unsafe extern "C" fn if(urb->transfer_buffer_length: urb->actual_length !=) -> else {
    else if (urb.actual_length != urb.transfer_buffer_length)
    ctx.errors++;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: check_guard_bytes(ctx->dev, 0: urb) !=) -> else {
    else if (check_guard_bytes(ctx.dev, urb) != 0)
    ctx.errors++;
    if (urb.status == 0 && ctx.count > (ctx.pending - 1)
    && !ctx.submit_error) {
    let mut status: c_int = usb_submit_urb(urb, GFP_ATOMIC);
    switch (status) {
    case 0:
    goto done;
    default:
...
//
    ...
    ;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: context.submit_error) -> else {
...
// We do performance test, so ignore data compare
    ...
    retval = test_queue(dev, param,
    dev.out_pipe, core::ptr::null_mut(), 0);
    break;
    case 28:
    ...
    param.sglen * param.length) / (1024 * 1024));
    retval = test_queue(dev, param,
    dev.in_pipe, core::ptr::null_mut(), 0);
    break;
...
#[no_mangle]
unsafe extern "C" fn usbtest_suspend(intf: *mut usb_interface, message: pm_message_t) -> c_int {
    static int usbtest_suspend(struct usb_interface *intf, pm_message_t message)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn usbtest_resume(intf: *mut usb_interface) -> c_int {
    static int usbtest_resume(struct usb_interface *intf)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn usbtest_disconnect(intf: *mut usb_interface) {
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn usbtest_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn usbtest_exit() -> void __exit {
