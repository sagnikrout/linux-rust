...
//! Source: drivers/usb/serial/omninet.c
#![no_std]
...
pub const ZYXEL_VENDOR_ID: c_uint = 0x0586;
pub const ZYXEL_OMNINET_ID: c_uint = 0x1000;
pub const ZYXEL_OMNI_56K_PLUS_ID: c_uint = 0x1500;
// This one seems to be a re-branded ZyXEL device
pub const BT_IGNITIONPRO_ID: c_uint = 0x2000;
pub const OMNINET_HEADERLEN: c_int = 4;
pub const OMNINET_BULKOUTSIZE: c_int = 64;
...
// function prototypes
    ...
    &zyxel_omninet_device, core::ptr::null_mut()
    };
//
// The protocol.
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct omninet_data {
    pub /: *mut *mut __u8 od_outseq; / Sequence number for bulk_out URBs,
}
...
#[no_mangle]
unsafe extern "C" fn omninet_port_probe(port: *mut usb_serial_port) -> c_int {
    static int omninet_port_probe(struct usb_serial_port *port)
    {
...
#[no_mangle]
unsafe extern "C" fn omninet_port_remove(port: *mut usb_serial_port) {
    static void omninet_port_remove(struct usb_serial_port *port)
    {
    struct omninet_data *od;
...
#[no_mangle]
unsafe extern "C" fn omninet_process_read_urb(urb: *mut urb) {
    static void omninet_process_read_urb(struct urb *urb)
    {
