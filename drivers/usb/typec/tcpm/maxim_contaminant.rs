...
//! Source: drivers/usb/typec/tcpm/maxim_contaminant.c
#![no_std]
...
pub const FLADC_1uA_LSB_MV: c_int = 25;
// High range CC
pub const FLADC_CC_HIGH_RANGE_LSB_MV: c_int = 208;
// Low range CC
pub const FLADC_CC_LOW_RANGE_LSB_MV: c_int = 126;
// 1uA current source
pub const FLADC_CC_SCALE1: c_int = 1;
// 5 uA current source
pub const FLADC_CC_SCALE2: c_int = 5;
pub const FLADC_1uA_CC_OFFSET_MV: c_int = 300;
pub const FLADC_CC_HIGH_RANGE_OFFSET_MV: c_int = 624;
pub const FLADC_CC_LOW_RANGE_OFFSET_MV: c_int = 378;
pub const CONTAMINANT_THRESHOLD_SBU_K: c_int = 1000;
pub const CONTAMINANT_THRESHOLD_CC_K: c_int = 1000;
pub const READ1_SLEEP_MS: c_int = 10;
pub const READ2_SLEEP_MS: c_int = 5;
...
#[no_mangle]
pub unsafe extern "C" fn if(CC2_SCALE1): !ua_src && (channel == CC1_SCALE1 || channel ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(CC2_SCALE2): !ua_src && (channel == CC1_SCALE2 || channel ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn max_contaminant_detect_contaminant(chip: *mut max_tcpci_chip) -> c_int {
    static int max_contaminant_detect_contaminant(struct max_tcpci_chip *chip)
    {
    int cc1_k, cc2_k, sbu1_k, sbu2_k, ret;
    let mut vendor_cc_status2_cc1: u8 = 0xff, vendor_cc_status2_cc2 = 0xff;
    let mut role_ctrl: u8 = 0, role_ctrl_backup = 0;
    let mut inferred_state: c_int = NOT_DETECTED;
...
#[no_mangle]
unsafe extern "C" fn max_contaminant_enable_dry_detection(chip: *mut max_tcpci_chip) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn max_contaminant_enable_toggling(chip: *mut max_tcpci_chip) -> c_int {
