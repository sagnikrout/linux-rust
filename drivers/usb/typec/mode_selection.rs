...
//! Source: drivers/usb/typec/mode_selection.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mode_selection {
    pub mode_list: list_head,
...
#[no_mangle]
unsafe extern "C" fn activate_altmode(dev: *mut device, data: *mut c_void) -> c_int {
    ...
    {
    let mut order: mode_order = {.svid = svid, .enter = enter, .result = -ENODEV};
//
...
#[no_mangle]
unsafe extern "C" fn mode_list_clean(sel: *mut mode_selection) {
...
//
#[no_mangle]
unsafe extern "C" fn mode_selection_work_fn(work: *mut work_struct) {
    ...
    let mut delay: c_uint = sel.delay;
    int result;
...
#[no_mangle]
unsafe extern "C" fn altmode_add_to_list(dev: *mut device, data: *mut c_void) -> c_int {
    ...
    list_sort(core::ptr::null_mut(), &sel.mode_list, compare_priorities);
    sel.partner = partner;
    sel.delay = delay;
    sel.timeout = timeout;
    mutex_init(&sel.lock);
...
#[no_mangle]
pub unsafe extern "C" fn typec_mode_selection_delete(partner: *mut typec_partner) {
    void typec_mode_selection_delete(struct typec_partner *partner)
    {
    struct mode_selection *sel = partner.sel;
    if (sel) {
    partner.sel = core::ptr::null_mut();
