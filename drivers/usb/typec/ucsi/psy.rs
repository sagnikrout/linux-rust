...
//! Source: drivers/usb/typec/ucsi/psy.c
#![no_std]
...
// Power Supply access to expose source power information
    ...
    {
    let mut scope: u8 = POWER_SUPPLY_SCOPE_UNKNOWN;
    ...
    union power_supply_propval *val)
    {
    let mut is_sink: bool = UCSI_CONSTAT(con, PWR_DIR) == TYPEC_SINK;
    let mut sink_path_enabled: bool = true;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !is_sink) -> else {
    else if (!is_sink)
    ...
    {
    u32 pdo;
    let mut max_voltage: c_int = 0;
    switch (UCSI_CONSTAT(con, PWR_OPMODE)) {
    case UCSI_CONSTAT_PWR_OPMODE_PD:
    for (int i = 0; i < con.num_pdos; i++) {
    let mut pdo_voltage: c_int = 0;
    pdo = con.src_pdos[i];
...
#[no_mangle]
unsafe extern "C" fn ucsi_psy_get_charge_type(con: *mut ucsi_connector, val: *mut union power_supply_propval) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ucsi_register_port_psy(con: *mut ucsi_connector) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ucsi_unregister_port_psy(con: *mut ucsi_connector) {
    void ucsi_unregister_port_psy(struct ucsi_connector *con)
    {
    if (IS_ERR_OR_NULL(con.psy))
    return;
    power_supply_unregister(con.psy);
    con.psy = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn ucsi_port_psy_changed(con: *mut ucsi_connector) {
