...
//! Source: drivers/usb/typec/altmodes/thunderbolt.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tbt_altmode {
    pub state: enum tbt_state,
    pub cable: *mut typec_cable,
    pub alt: *mut typec_altmode,
    pub plug: [*mut typec_altmode; 2],
    pub enter_vdo: u32,
...
#[no_mangle]
unsafe extern "C" fn tbt_enter_mode(tbt: *mut tbt_altmode) -> c_int {
    static int tbt_enter_mode(struct tbt_altmode *tbt)
    {
    return typec_altmode_enter(tbt.alt, &tbt.enter_vdo);
    }
#[no_mangle]
unsafe extern "C" fn tbt_altmode_work(work: *mut work_struct) {
    ...
    ret = typec_cable_altmode_enter(tbt.alt, TYPEC_PLUG_SOP_P, core::ptr::null_mut());
    if (ret) {
    ...
    ret = typec_cable_altmode_enter(tbt.alt, TYPEC_PLUG_SOP_PP,  core::ptr::null_mut());
    if (ret) {
...
//
#[no_mangle]
unsafe extern "C" fn tbt_enter_modes_ordered(alt: *mut typec_altmode) -> c_int {
    ...
    ret = typec_cable_altmode_enter(alt, TYPEC_PLUG_SOP_P, core::ptr::null_mut());
    if (ret < 0) {
    for (int i = TYPEC_PLUG_SOP_PP; i >= 0; --i) {
    if (tbt.plug[i])
    typec_altmode_put_plug(tbt.plug[i]);
    ...
    {
    struct tbt_altmode *tbt = typec_altmode_get_drvdata(alt);
    let mut cmd_type: c_int = PD_VDO_CMDT(hdr);
    let mut cmd: c_int = PD_VDO_CMD(hdr);
    mutex_lock(&tbt.lock);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tbt->plug[TYPEC_PLUG_SOP_P]) -> else {
...
#[no_mangle]
unsafe extern "C" fn tbt_altmode_activate(alt: *mut typec_altmode, activate: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tbt_altmode_probe(alt: *mut typec_altmode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: tbt->plug[TYPEC_PLUG_SOP_PP]) -> else {
...
#[no_mangle]
unsafe extern "C" fn tbt_altmode_remove(alt: *mut typec_altmode) {
...
#[no_mangle]
unsafe extern "C" fn tbt_ready(alt: *mut typec_altmode) -> bool {
    static bool tbt_ready(struct typec_altmode *alt)
    {
    struct tbt_altmode *tbt = typec_altmode_get_drvdata(alt);
