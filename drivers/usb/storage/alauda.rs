...
//! Source: drivers/usb/storage/alauda.c
#![no_std]
...
// Status bytes
//
pub const ALAUDA_STATUS_ERROR: c_uint = 0x01;
pub const ALAUDA_STATUS_READY: c_uint = 0x40;
//
// Control opcodes (for request field)
//
pub const ALAUDA_GET_XD_MEDIA_STATUS: c_uint = 0x08;
pub const ALAUDA_GET_SM_MEDIA_STATUS: c_uint = 0x98;
pub const ALAUDA_ACK_XD_MEDIA_CHANGE: c_uint = 0x0a;
pub const ALAUDA_ACK_SM_MEDIA_CHANGE: c_uint = 0x9a;
pub const ALAUDA_GET_XD_MEDIA_SIG: c_uint = 0x86;
pub const ALAUDA_GET_SM_MEDIA_SIG: c_uint = 0x96;
//
// Bulk command identity (byte 0)
//
pub const ALAUDA_BULK_CMD: c_uint = 0x40;
//
// Bulk opcodes (byte 1)
//
pub const ALAUDA_BULK_GET_REDU_DATA: c_uint = 0x85;
pub const ALAUDA_BULK_READ_BLOCK: c_uint = 0x94;
pub const ALAUDA_BULK_ERASE_BLOCK: c_uint = 0xa3;
pub const ALAUDA_BULK_WRITE_BLOCK: c_uint = 0xb4;
pub const ALAUDA_BULK_GET_STATUS2: c_uint = 0xb7;
pub const ALAUDA_BULK_RESET_MEDIA: c_uint = 0xe0;
//
// Port to operate on (byte 8)
//
pub const ALAUDA_PORT_XD: c_uint = 0x00;
pub const ALAUDA_PORT_SM: c_uint = 0x01;
//
// LBA and PBA are unsigned ints. Special values.
//
pub const UNDEF: c_uint = 0xffff;
pub const SPARE: c_uint = 0xfffe;
pub const UNUSABLE: c_uint = 0xfffd;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct alauda_media_info {
    pub /: *mut *mut unsigned long capacity; / total media size in bytes,
    pub /: *mut *mut unsigned int pagesize; / page size in bytes,
    pub /: *mut *mut unsigned int blocksize; / number of pages per block,
    pub /: *mut *mut unsigned int uzonesize; / number of usable blocks per zone,
    pub /: *mut *mut unsigned int zonesize; / number of blocks per zone,
    pub /: *mut *mut unsigned int blockmask; / mask to get page from address,
    pub pageshift: c_uchar,
    pub blockshift: c_uchar,
    pub zoneshift: c_uchar,
    pub /: *mut *mut *mut *mut u16 lba_to_pba; / logical to physical block map,
    pub /: *mut *mut *mut *mut u16 pba_to_lba; / physical to logical block map,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct alauda_info {
    pub port: [alauda_media_info; 2],
    pub /: *mut *mut int wr_ep; / endpoint to write data out of,
    pub sense_key: c_uchar,
    pub /: *mut *mut unsigned long sense_asc; / additional sense code,
    pub /: *mut *mut unsigned long sense_ascq; / additional sense code qualifier,
    pub media_initialized: bool,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct alauda_card_info {
    pub /: *mut *mut unsigned char id; / id byte,
    pub /: *mut *mut unsigned char chipshift; / 1<<cs bytes total capacity,
    pub /: *mut *mut unsigned char pageshift; / 1<<ps bytes in a page,
    pub /: *mut *mut unsigned char blockshift; / 1<<bs pages per block,
    pub /: *mut *mut unsigned char zoneshift; / 1<<zs blocks per zone,
}
...
#[no_mangle]
unsafe extern "C" fn nand_init_ecc() {
...
#[no_mangle]
unsafe extern "C" fn nand_compute_ecc(data: *mut c_uchar, ecc: *mut c_uchar) {
    static void nand_compute_ecc(unsigned char *data, unsigned char *ecc)
    {
    int i, j, a;
    let mut par: c_uchar = 0, bit, bits[8] = {0};
...
#[no_mangle]
unsafe extern "C" fn nand_compare_ecc(data: *mut c_uchar, ecc: *mut c_uchar) -> c_int {
    static int nand_compare_ecc(unsigned char *data, unsigned char *ecc)
    {
    return (data[0] == ecc[0] && data[1] == ecc[1] && data[2] == ecc[2]);
    }
#[no_mangle]
unsafe extern "C" fn nand_store_ecc(data: *mut c_uchar, ecc: *mut c_uchar) {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_free_maps(media_info: *mut alauda_media_info) {
    ...
    let mut num_zones: c_uint = media_info.capacity >> shift;
    unsigned int i;
    if (media_info.lba_to_pba != core::ptr::null_mut())
    for (i = 0; i < num_zones; i++) {
    kfree(media_info.lba_to_pba[i]);
    media_info.lba_to_pba[i] = core::ptr::null_mut();
    }
    if (media_info.pba_to_lba != core::ptr::null_mut())
    for (i = 0; i < num_zones; i++) {
    kfree(media_info.pba_to_lba[i]);
    media_info.pba_to_lba[i] = core::ptr::null_mut();
    }
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn alauda_get_media_status(us: *mut us_data, data: *mut c_uchar) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_ack_media(us: *mut us_data) -> c_int {
    ...
    command, 0x40, 0, 1, core::ptr::null_mut(), 0);
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn alauda_get_media_signature(us: *mut us_data, data: *mut c_uchar) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_reset_media(us: *mut us_data) -> c_int {
    ...
    command, 9, core::ptr::null_mut());
    }
//
// Examines the media and deduces capacity, etc.
//
#[no_mangle]
unsafe extern "C" fn alauda_init_media(us: *mut us_data) -> c_int {
    ...
    if (media_info == core::ptr::null_mut()) {
    ...
    if (MEDIA_INFO(us).pba_to_lba == core::ptr::null_mut() || MEDIA_INFO(us).lba_to_pba == core::ptr::null_mut())
    return USB_STOR_TRANSPORT_ERROR;
...
//
#[no_mangle]
unsafe extern "C" fn alauda_check_media(us: *mut us_data) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_check_status2(us: *mut us_data) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_get_redu_data(us: *mut us_data, pba: u16, data: *mut c_uchar) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_read_map(us: *mut us_data, zone: c_uint) -> c_int {
    static int alauda_read_map(struct us_data *us, unsigned int zone)
    {
    unsigned char *data = us.iobuf;
    int result;
    int i, j;
    let mut zonesize: c_uint = MEDIA_INFO(us).zonesize;
    let mut uzonesize: c_uint = MEDIA_INFO(us).uzonesize;
    unsigned int lba_offset, lba_real, blocknum;
    let mut zone_base_lba: c_uint = zone * uzonesize;
    let mut zone_base_pba: c_uint = zone * zonesize;
    u16 *lba_to_pba = kcalloc(zonesize, sizeof(u16), GFP_NOIO);
    u16 *pba_to_lba = kcalloc(zonesize, sizeof(u16), GFP_NOIO);
    if (lba_to_pba == core::ptr::null_mut() || pba_to_lba == core::ptr::null_mut()) {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_ensure_map_for_zone(us: *mut us_data, zone: c_uint) {
    static void alauda_ensure_map_for_zone(struct us_data *us, unsigned int zone)
    {
    if (MEDIA_INFO(us).lba_to_pba[zone] == core::ptr::null_mut()
    || MEDIA_INFO(us).pba_to_lba[zone] == core::ptr::null_mut())
...
//
#[no_mangle]
unsafe extern "C" fn alauda_erase_block(us: *mut us_data, pba: u16) -> c_int {
...
//
    ...
    return rc;
    return usb_stor_bulk_transfer_buf(us, us.recv_bulk_pipe,
    data, (MEDIA_INFO(us).pagesize + 64) * pages, core::ptr::null_mut());
    }
//
...
//
    ...
    {
    int i, rc;
    let mut pagesize: c_uint = MEDIA_INFO(us).pagesize;
    rc = alauda_read_block_raw(us, pba, page, pages, data);
    if (rc != USB_STOR_XFER_GOOD)
    return rc;
// Cut out the redundancy data
    for (i = 0; i < pages; i++) {
    let mut dest_offset: c_int = i * pagesize;
    let mut src_offset: c_int = i * (pagesize + 64);
...
//
#[no_mangle]
unsafe extern "C" fn alauda_write_block(us: *mut us_data, pba: u16, data: *mut c_uchar) -> c_int {
...
//
    ...
    let mut zonesize: c_uint = MEDIA_INFO(us).zonesize;
    let mut pagesize: c_uint = MEDIA_INFO(us).pagesize;
    let mut blocksize: c_uint = MEDIA_INFO(us).blocksize;
    let mut lba_offset: c_uint = lba % uzonesize;
    unsigned int new_pba_offset;
    let mut zone: c_uint = lba / uzonesize;
...
// copy in new stuff and compute ECC
    ...
    if (pba != UNDEF) {
    let mut pba_offset: c_uint = pba - (zone * zonesize);
...
//
    ...
    let mut blockshift: c_uint = MEDIA_INFO(us).blockshift;
    let mut pageshift: c_uint = MEDIA_INFO(us).pageshift;
    let mut blocksize: c_uint = MEDIA_INFO(us).blocksize;
    let mut pagesize: c_uint = MEDIA_INFO(us).pagesize;
    let mut uzonesize: c_uint = MEDIA_INFO(us).uzonesize;
...
// Figure out the initial LBA and page
    ...
    let mut lba_offset: c_uint = lba - (zone * uzonesize);
    unsigned int pages;
    u16 pba;
    alauda_ensure_map_for_zone(us, zone);
...
// Write as many sectors as possible in this block
    let mut pages: c_uint = min(sectors, blocksize - page);
    len = pages << pageshift;
// Not overflowing capacity?
    if (lba >= max_lba) {
...
//
#[no_mangle]
unsafe extern "C" fn alauda_info_destructor(extra: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn init_alauda(us: *mut us_data) -> c_int {
    static int init_alauda(struct us_data *us)
    {
...
#[no_mangle]
unsafe extern "C" fn alauda_transport(srb: *mut scsi_cmnd, us: *mut us_data) -> c_int {
