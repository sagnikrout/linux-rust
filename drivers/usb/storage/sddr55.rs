...
//! Source: drivers/usb/storage/sddr55.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sddr55_card_info {
    pub /: *mut *mut unsigned long capacity; / Size of card in bytes,
    pub /: *mut *mut int max_log_blks; / maximum number of logical blocks,
    pub /: *mut *mut int pageshift; / log2 of pagesize,
    pub /: *mut *mut int smallpageshift; / 1 if pagesize == 256,
    ...
    pub /: *mut *mut int read_only; / non zero if card is write protected,
    pub error*/: *mut *mut int force_read_only; / non zero if we find a map,
    pub /: *mut *mut *mut int lba_to_pba; / logical to physical map,
    pub /: *mut *mut *mut int pba_to_lba; / physical to logical map,
    pub /: *mut *mut int fatal_error; / set if we detect something nasty,
    pub /: *mut *mut unsigned long last_access; / number of jiffies since we last talked to device,
    pub sense_data: [c_uchar; 18],
}

pub const NOT_ALLOCATED: c_uint = 0xffffffff;
pub const BAD_BLOCK: c_uint = 0xffff;
pub const CIS_BLOCK: c_uint = 0x400;
pub const UNUSED_BLOCK: c_uint = 0x3ff;
    static int
    ...
    return usb_stor_bulk_transfer_buf(us, pipe, data, len, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn sddr55_status(us: *mut us_data) -> c_int {
...
// no pba allocated for this lba, find a free pba to use
    let mut max_pba: c_int = (info.max_log_blks / 250 ) * 256;
    let mut found_count: c_int = 0;
    let mut found_pba: c_int = -1;
...
#[no_mangle]
unsafe extern "C" fn sddr55_reset(us: *mut us_data) -> c_int {
    static int sddr55_reset(struct us_data *us)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn sddr55_get_capacity(us: *mut us_data) -> c_ulong {
    unsigned char manufacturerID;
    unsigned char deviceID;
    int result;
    struct sddr55_card_info *info = (struct sddr55_card_info *)us.extra;
...
#[no_mangle]
unsafe extern "C" fn sddr55_read_map(us: *mut us_data) -> c_int {
    struct sddr55_card_info *info = (struct sddr55_card_info *)(us.extra);
    int numblocks;
    unsigned char *buffer;
    unsigned char *command = us.iobuf;
...
#[no_mangle]
unsafe extern "C" fn sddr55_card_info_destructor(extra: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn sddr55_transport(srb: *mut scsi_cmnd, us: *mut us_data) -> c_int {
...
//
    if (info.lba_to_pba == core::ptr::null_mut() || time_after(jiffies, info.last_access + HZ/2)) {
