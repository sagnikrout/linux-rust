...
//! Source: drivers/usb/storage/sddr09.c
#![no_std]
...
pub struct nand_flash_dev {
    pub model_id: c_int,
    pub /: *mut *mut int chipshift; / 1<<cs bytes total capacity,
    pub /: *mut *mut char pageshift; / 1<<ps bytes in a page,
    pub /: *mut *mut char blockshift; / 1<<bs pages in an erase block,
    pub /: *mut *mut char zoneshift; / 1<<zs blocks in a zone,
// # of logical blocks is 125/128 of this
    pub /: *mut *mut char pageadrlen; / length of an address in bytes - 1,
}
...
pub const NAND_MFR_NATSEMI: c_uint = 0x8f;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sddr09_card_info {
    pub /: *mut *mut unsigned long capacity; / Size of card in bytes,
    pub /: *mut *mut int pagesize; / Size of page in bytes,
    pub /: *mut *mut int pageshift; / log2 of pagesize,
    pub /: *mut *mut int blocksize; / Size of block in pages,
    pub /: *mut *mut int blockshift; / log2 of blocksize,
    pub /: *mut *mut int blockmask; / 2^blockshift - 1,
    pub /: *mut *mut *mut int lba_to_pba; / logical to physical map,
    pub /: *mut *mut *mut int pba_to_lba; / physical to logical map,
    pub /: *mut *mut int lbact; / number of available pages,
    pub flags: c_int,
...
//
pub const CONTROL_SHIFT: c_int = 6;
//
// On my Combo CF/SM reader, the SM reader has LUN 1.
// (and things fail with LUN 0).
// It seems LUN is irrelevant for others.
//
pub const LUN: c_int = 1;

//
// LBA and PBA are unsigned ints. Special values.
//
pub const UNDEF: c_uint = 0xffffffff;
pub const SPARE: c_uint = 0xfffffffe;
pub const UNUSABLE: c_uint = 0xfffffffd;
    let mut erase_bad_lba_entries: static int = 0;
...
// called for requests 0, 1, 8
    ...
    unsigned int pipe;
    let mut requesttype: c_uchar = (0x41 | direction);
    int rc;
...
//
    ...
    sensebuf, buflen, core::ptr::null_mut());
...
//
    ...
    buf, bulklen, use_sg, core::ptr::null_mut());
...
//
    ...
    let mut bulklen: c_int = nr_of_pages << pageshift;
...
//
    ...
    let mut bulklen: c_int = (count << controlshift);
...
//
    ...
    int nr_of_pages, int pageshift, unsigned char *buf, int use_sg) {
    let mut bulklen: c_int = (nr_of_pages << pageshift) + (nr_of_pages << CONTROL_SHIFT);
...
// Store the data in the transfer buffer
    ...
    let mut lastpba: static unsigned int = 1;
    int zonestart, end, i;
...
// copy in new stuff and compute ECC
    ...
    let mut result2: c_int = sddr09_read_status(us, &status);
    if (result2)
    usb_stor_dbg(us, "cannot read status\n");
#[no_mangle]
pub unsafe extern "C" fn if(0xc0: status !=) -> else {
    else if (status != 0xc0)
    usb_stor_dbg(us, "status after write: 0x%x\n", status);
    }

    {
    let mut result2: c_int = sddr09_test_unit_ready(us);
    }
...
//
pub const SDDR09_READ_MAP_BUFSZ: c_int = 65536;
    ...
    if (info.lba_to_pba == core::ptr::null_mut() || info.pba_to_lba == core::ptr::null_mut()) {
...
//
    ...
    kfree(info.lba_to_pba);
    kfree(info.pba_to_lba);
    info.lba_to_pba = core::ptr::null_mut();
    info.pba_to_lba = core::ptr::null_mut();
    }
    kfree(buffer);
    return result;
    }
...
//
#[no_mangle]
unsafe extern "C" fn dpcm_transport(srb: *mut scsi_cmnd, us: *mut us_data) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sddr09_transport(srb: *mut scsi_cmnd, us: *mut us_data) -> c_int {
    static int sddr09_transport(struct scsi_cmnd *srb, struct us_data *us)
    {
    let mut sensekey: static unsigned char = 0, sensecode = 0;
    let mut havefakesense: static unsigned char = 0;
    int result, i;
...
// Report page size
    ...
    let mut modepage: c_int = (srb.cmnd[2] & 0x3F);
//
