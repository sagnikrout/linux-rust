...
//! Source: drivers/usb/host/max3421-hcd.c
#![no_std]
...
pub const USB_MAX_FRAME_NUMBER: c_uint = 0x7ff;
...
pub const MAX3421_GPOUT_COUNT: c_int = 8;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct max3421_dma_buf {
    pub data: [u8; 2],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct max3421_hcd {
    pub lock: spinlock_t,
    pub spi_thread: *mut task_struct,
    pub rh_state: enum max3421_rh_state,
// lower 16 bits contain port status, upper 16 bits the change mask:
    pub port_status: u32,
    pub active:1: unsigned,
    pub /: *mut *mut list_head ep_list; / list of EP's with work,
//
// The following are owned by spi_thread (may be accessed by
// SPI-thread without acquiring the HCD lock:
//
    pub /: *mut *mut u8 rev; / chip revision,
    pub frame_number: u16,
//
// kmalloc'd buffers guaranteed to be in separate (DMA)
// cache-lines:
//
    pub tx: *mut max3421_dma_buf,
    pub rx: *mut max3421_dma_buf,
//
// URB we're currently processing.  Must not be reset to NULL
// unless MAX3421E chip is idle:
//
    pub curr_urb: *mut urb,
    pub sched_pass: enum scheduling_pass,
    pub /: *mut *mut int urb_done; / > 0 -> no errors, < 0: errno,
    pub curr_len: usize,
    pub hien: u8,
    pub mode: u8,
    pub iopins: [u8; 2],
    pub todo: c_ulong,
    pub err_stat: [c_ulong; 16],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct max3421_ep {
    pub ep: *mut usb_host_endpoint,
    pub ep_list: list_head,
    pub naks: u32,
    pub /: *mut *mut u16 last_active; / frame # this ep was last active,
    pub pkt_state: enum pkt_state,
    pub retries: u8,
    pub /: *mut *mut u8 retransmit; / packet needs retransmission,
}

pub const MAX3421_FIFO_SIZE: c_int = 64;

// SPI commands:
pub const MAX3421_SPI_DIR_SHIFT: c_int = 1;
pub const MAX3421_SPI_REG_SHIFT: c_int = 3;
pub const MAX3421_REG_RCVFIFO: c_int = 1;
pub const MAX3421_REG_SNDFIFO: c_int = 2;
pub const MAX3421_REG_SUDFIFO: c_int = 4;
pub const MAX3421_REG_RCVBC: c_int = 6;
pub const MAX3421_REG_SNDBC: c_int = 7;
pub const MAX3421_REG_USBIRQ: c_int = 13;
pub const MAX3421_REG_USBIEN: c_int = 14;
pub const MAX3421_REG_USBCTL: c_int = 15;
pub const MAX3421_REG_CPUCTL: c_int = 16;
pub const MAX3421_REG_PINCTL: c_int = 17;
pub const MAX3421_REG_REVISION: c_int = 18;
pub const MAX3421_REG_IOPINS1: c_int = 20;
pub const MAX3421_REG_IOPINS2: c_int = 21;
pub const MAX3421_REG_GPINIRQ: c_int = 22;
pub const MAX3421_REG_GPINIEN: c_int = 23;
pub const MAX3421_REG_GPINPOL: c_int = 24;
pub const MAX3421_REG_HIRQ: c_int = 25;
pub const MAX3421_REG_HIEN: c_int = 26;
pub const MAX3421_REG_MODE: c_int = 27;
pub const MAX3421_REG_PERADDR: c_int = 28;
pub const MAX3421_REG_HCTL: c_int = 29;
pub const MAX3421_REG_HXFR: c_int = 30;
pub const MAX3421_REG_HRSL: c_int = 31;
    enum {
...
pub const MAX3421_HXFR_SETUP: c_uint = 0x10;

pub const MAX3421_HXFR_HS_IN: c_uint = 0x80		/* handshake in */;
pub const MAX3421_HXFR_HS_OUT: c_uint = 0xa0		/* handshake out */;
...
//
    ...
    {
    struct max3421_hcd *max3421_hcd = hcd_to_max3421(hcd);
    let mut epnum: c_int = usb_pipeendpoint(urb.pipe);
...
//
    ...
    struct spi_device *spi = to_spi_device(hcd.self.controller);
    struct max3421_hcd *max3421_hcd = hcd_to_max3421(hcd);
    struct urb *urb, *curr_urb = core::ptr::null_mut();
    struct max3421_ep *max3421_ep;
    int epnum;
...
//
    ...
    max3421_hcd.curr_urb = core::ptr::null_mut();
    }
//
// Caller must NOT hold HCD spinlock.
//
...
// ack the RCVDAV irq now that the FIFO has been read:
    ...
    struct spi_device *spi = to_spi_device(hcd.self.controller);
    struct max3421_hcd *max3421_hcd = hcd_to_max3421(hcd);
    let mut result_code: u8 = hrsl & MAX3421_HRSL_RESULT_MASK;
    struct urb *urb = max3421_hcd.curr_urb;
...
// flip the send toggle bit:
    let mut sndtog: c_int = (hrsl >> MAX3421_HRSL_SNDTOGRD_BIT) & 1;
    sndtog ^= 1;
...
//
    let mut max_packet: u32 = usb_maxpacket(urb.dev, urb.pipe);
    if (max3421_hcd.curr_len == max_packet)
    return 0;
    }
    return 1;
...
// save the old end-points toggles:
    let mut hrsl: u8 = spi_rd8(hcd, MAX3421_REG_HRSL);
    let mut rcvtog: c_int = (hrsl >> MAX3421_HRSL_RCVTOGRD_BIT) & 1;
    let mut sndtog: c_int = (hrsl >> MAX3421_HRSL_SNDTOGRD_BIT) & 1;
    let mut epnum: c_int = usb_endpoint_num(&urb.ep.desc);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: max3421_handle_irqs(hcd)) -> else {
    else if (max3421_handle_irqs(hcd))
    i_worked = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !max3421_hcd->curr_urb) -> else {
...
//
    ...
    let mut val: u8 = spi_rd8(hcd, MAX3421_REG_IOPINS1 + i);
    val = ((val & 0xf0) |
...
// (__le32 *) buf = cpu_to_le32(0);
    ...
    struct max3421_hcd_platform_data *pdata = core::ptr::null_mut();
    int retval;
    if (spi_setup(spi) < 0) {
    ...
    spi.dev.platform_data = core::ptr::null_mut();
    }
    if (hcd) {
