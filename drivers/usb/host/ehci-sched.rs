...
//! Source: drivers/usb/host/ehci-sched.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn periodic_unlink(ehci: *mut ehci_hcd, frame: unsigned, ptr: *mut c_void) {
...
// ptt = NULL;
    ...
    unsigned		start_uf;
    unsigned		i, j, m;
    let mut usecs: c_int = qh.ps.usecs;
    let mut c_usecs: c_int = qh.ps.c_usecs;
    let mut tt_usecs: c_int = qh.ps.tt_usecs;
    struct ehci_tt		*tt;
...
#[no_mangle]
pub unsafe extern "C" fn carryover_tt_bandwidth(tt_usecs[8]: c_ushort) {
...
//
    ...
    {
    let mut period: unsigned = ps.bw_period;
    let mut usecs: unsigned = ps.tt_usecs;
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn enable_periodic(ehci: *mut ehci_hcd) {
...
#[no_mangle]
unsafe extern "C" fn disable_periodic(ehci: *mut ehci_hcd) {
...
//
#[no_mangle]
unsafe extern "C" fn qh_link_periodic(ehci: *mut ehci_hcd, qh: *mut ehci_qh) {
    static void qh_link_periodic(struct ehci_hcd *ehci, struct ehci_qh *qh)
    {
    unsigned	i;
    let mut period: unsigned = qh.ps.period;
...
#[no_mangle]
unsafe extern "C" fn qh_unlink_periodic(ehci: *mut ehci_hcd, qh: *mut ehci_qh) {
...
#[no_mangle]
unsafe extern "C" fn cancel_unlink_wait_intr(ehci: *mut ehci_hcd, qh: *mut ehci_qh) {
...
#[no_mangle]
unsafe extern "C" fn start_unlink_intr(ehci: *mut ehci_hcd, qh: *mut ehci_qh) {
...
#[no_mangle]
pub unsafe extern "C" fn if(EHCI_RH_RUNNING: ehci->rh_state <) -> else {
    else if (ehci.rh_state < EHCI_RH_RUNNING)
    ehci_handle_intr_unlinks(ehci);
#[no_mangle]
pub unsafe extern "C" fn if(&qh->unlink_node: ehci->intr_unlink.next ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(&qh->unlink_node: ehci->intr_unlink_wait.next ==) -> else {
    ehci_enable_event(ehci, EHCI_HRTIMER_START_UNLINK_INTR, true);
    ++ehci.intr_unlink_wait_cycle;
    }
    }
#[no_mangle]
unsafe extern "C" fn end_unlink_intr(ehci: *mut ehci_hcd, qh: *mut ehci_qh) {
...
//
#[no_mangle]
unsafe extern "C" fn qh_schedule(ehci: *mut ehci_hcd, qh: *mut ehci_qh) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scan_intr(ehci: *mut ehci_hcd) {
...
// ehci_iso_stream ops work with both ITD and SITD
    ...
    if (likely(stream != core::ptr::null_mut())) {
...
// knows about ITD vs SITD
    ...
    let mut multi: unsigned = usb_endpoint_maxp_mult(&urb.ep.desc);
    stream.highspeed = 1;
    buf1 |= maxp;
    maxp *= multi;
...
// stream->splits gets created from cs_mask later
    ...
    spin_lock_irqsave(&ehci.lock, flags);
    stream = ep.hcpriv;
    if (unlikely(stream == core::ptr::null_mut())) {
...
// if dev->ep [epnum] is a QH, hw is set
    } else if (unlikely(stream.hw != core::ptr::null_mut())) {
...
// ehci_iso_sched ops can be ITD-only or SITD-only
    ...
    if (likely(iso_sched != core::ptr::null_mut()))
    ...
    {
    unsigned	i;
    let mut dma: dma_addr_t = urb.transfer_dma;
...
// caller must hold ehci->lock!
    ...
    if (unlikely(sched == core::ptr::null_mut()))
    return -ENOMEM;
...
// -------------------------------------------------------------------------
    ...
    let mut usecs: c_int = stream.ps.usecs;
    let mut c_usecs: c_int = stream.ps.c_usecs;
    let mut tt_usecs: c_int = stream.ps.tt_usecs;
    struct ehci_tt		*tt;
...
#[no_mangle]
pub unsafe extern "C" fn if(m: c_mask &) -> else {
    else if (c_mask & m)
    ehci.bandwidth[i+j] += c_usecs;
    }
    }
...
//
    ...
    let mut wrap: u32 = 0, skip = 0;
    let mut status: c_int = 0;
    let mut mod: unsigned = ehci.periodic_size << 3;
    struct ehci_iso_sched	*sched = urb.hcpriv;
    let mut empty: bool = list_empty(&stream.td_list);
    let mut new_stream: bool = false;
...
// fill iTDs uframe by uframe
    for (packet = iso_sched.first_packet, itd = core::ptr::null_mut();
    packet < urb.number_of_packets;) {
    if (itd == core::ptr::null_mut()) {
...
//
#[no_mangle]
unsafe extern "C" fn itd_complete(ehci: *mut ehci_hcd, itd: *mut ehci_itd) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn if(EHCI_ISOC_BABBLE: t &) -> else {
...
// OUT uses multiple start-splits
    ...
    if (iso_sched == core::ptr::null_mut())
    return -ENOMEM;
...
// fill sITDs frame by frame
    for (packet = sched.first_packet, sitd = core::ptr::null_mut();
    packet < urb.number_of_packets;
    packet++) {
...
//
#[no_mangle]
unsafe extern "C" fn sitd_complete(ehci: *mut ehci_hcd, sitd: *mut ehci_sitd) -> bool {
...
// report transfer status
    ...
    : -ECOMM; /* hc couldn't write */
#[no_mangle]
pub unsafe extern "C" fn if(SITD_STS_BABBLE: t &) -> else {
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn scan_isoc(ehci: *mut ehci_hcd) {
    static void scan_isoc(struct ehci_hcd *ehci)
    {
    unsigned		uf, now_frame, frame;
    let mut fmask: unsigned = ehci.periodic_size - 1;
    bool			modified, live;
