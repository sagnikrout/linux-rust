...
//! Source: drivers/usb/host/xen-hcd.c
#![no_std]
...
pub struct urb_priv {
    pub list: list_head,
    pub urb: *mut urb,
    pub /: *mut *mut int req_id; / RING_REQUEST id for submitting,
    pub /: *mut *mut int unlink_req_id; / RING_REQUEST id for unlinking,
    pub status: c_int,
    pub /: *mut *mut bool unlinked; / dequeued marker,
}

// virtual roothub port status
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rhport_status {
    pub status: __u32,
    pub /: *mut *mut bool resuming; / in resuming,
    pub /: *mut *mut bool c_connection; / connection changed,
    pub timeout: c_ulong,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vdevice_status {
    pub devnum: c_int,
    pub status: enum usb_device_state,
    pub speed: enum usb_device_speed,
}

// RING request shadow
#[repr(C)]
#[derive(Copy, Clone)]
pub struct usb_shadow {
    pub req: xenusb_urb_request,
    pub urb: *mut urb,
    pub in_flight: bool,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xenhcd_info {
// Virtual Host Controller has 4 urb queues
    pub pending_submit_list: list_head,
    pub pending_unlink_list: list_head,
    pub in_progress_list: list_head,
    pub giveback_waiting_list: list_head,
    pub lock: spinlock_t,
// timer that kick pending and giveback waiting urbs
    pub watchdog: timer_list,
    pub actions: c_ulong,
// virtual root hub
    pub rh_numports: c_int,
    pub ports: [rhport_status; XENUSB_MAX_PORTNR],
    pub devices: [vdevice_status; XENUSB_MAX_PORTNR],
// Xen related staff
    pub xbdev: *mut xenbus_device,
    pub urb_ring_ref: c_int,
    pub conn_ring_ref: c_int,
    pub urb_ring: xenusb_urb_front_ring,
    pub conn_ring: xenusb_conn_front_ring,
    pub evtchn: c_uint,
    pub irq: c_uint,
    pub shadow: [usb_shadow; XENUSB_URB_RING_SIZE],
    pub shadow_free: c_uint,
    pub error: bool,
}

pub const XENHCD_SCAN_JIFFIES: c_int = 1;
...
#[no_mangle]
unsafe extern "C" fn xenhcd_set_error(info: *mut xenhcd_info, msg: *const c_char) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_set_connect_state(info: *mut xenhcd_info, portnum: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_rhport_suspend(info: *mut xenhcd_info, portnum: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_rhport_resume(info: *mut xenhcd_info, portnum: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_rhport_power_on(info: *mut xenhcd_info, portnum: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_rhport_power_off(info: *mut xenhcd_info, portnum: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_rhport_disable(info: *mut xenhcd_info, portnum: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_rhport_reset(info: *mut xenhcd_info, portnum: c_int) {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_bus_suspend(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_bus_resume(hcd: *mut usb_hcd) -> c_int {
...
// resume any suspended ports
    ...
    {
    __u16 temp;
    let mut ports: c_int = info.rh_numports;
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_hub_status_data(hcd: *mut usb_hcd, buf: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_free_urb_priv(urbp: *mut urb_priv) {
    static void xenhcd_free_urb_priv(struct urb_priv *urbp)
    {
    urbp.urb.hcpriv = core::ptr::null_mut();
    kmem_cache_free(xenhcd_urbp_cachep, urbp);
    }
#[no_mangle]
pub unsafe extern "C" fn xenhcd_get_id_from_freelist(info: *mut xenhcd_info) -> c_uint {
    ...
    info.shadow[id].urb = core::ptr::null_mut();
    info.shadow_free = id;
    }
#[no_mangle]
pub unsafe extern "C" fn xenhcd_count_pages(addr: *mut c_void, length: c_int) -> c_int {
    static inline int xenhcd_count_pages(void *addr, int length)
    {
    let mut vaddr: c_ulong = (unsigned long)addr;
...
#[no_mangle]
unsafe extern "C" fn xenhcd_pipe_urb_to_xenusb(urb_pipe: __u32, port: __u8) -> __u32 {
    ...
    {
    grant_ref_t gref_head;
    let mut nr_buff_pages: c_int = 0;
    let mut nr_isodesc_pages: c_int = 0;
    let mut nr_grants: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn xenhcd_translate_status(status: c_int) -> c_int {
    ...
    let mut priv_status: c_int = urbp.status;
...
#[no_mangle]
unsafe extern "C" fn xenhcd_do_request(info: *mut xenhcd_info, urbp: *mut urb_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_kick_pending_urbs(info: *mut xenhcd_info) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_cancel_all_enqueued_urbs(info: *mut xenhcd_info) {
...
// not dequeued
    ...
    info.shadow[req_id].urb = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_giveback_unlinked_urbs(info: *mut xenhcd_info) {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_submit_urb(info: *mut xenhcd_info, urbp: *mut urb_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_unlink_urb(info: *mut xenhcd_info, urbp: *mut urb_priv) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: res->actual_length <) -> else {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_conn_notify(info: *mut xenhcd_info, eoiflag: *mut c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_int(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_destroy_rings(info: *mut xenhcd_info) {
    ...
    info.irq = err;
    err = request_threaded_irq(info.irq, core::ptr::null_mut(), xenhcd_int,
...
#[no_mangle]
unsafe extern "C" fn xenhcd_connect(dev: *mut xenbus_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_disconnect(dev: *mut xenbus_device) {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_watchdog(timer: *mut timer_list) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_setup(hcd: *mut usb_hcd) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_run(hcd: *mut usb_hcd) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_stop(hcd: *mut usb_hcd) {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_urb_dequeue(hcd: *mut usb_hcd, urb: *mut urb, status: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn xenhcd_get_frame(hcd: *mut usb_hcd) -> c_int {
...
// root hub operations
    ...
    info.shadow[i].urb = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn xenhcd_remove(dev: *mut xenbus_device) {
...
#[no_mangle]
unsafe extern "C" fn xenhcd_exit() -> void __exit {
