...
//! Source: drivers/usb/host/ohci-hub.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ohci_rh_suspend(ohci: *mut ohci_hcd, autostop: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ohci_rh_resume(ohci: *mut ohci_hcd) -> c_int {
    ...
    let mut autostopped: c_int = ohci.autostop;
    ohci.autostop = 0;
    ohci.hc_control = ohci_readl (ohci, &ohci.regs.control);
...
#[no_mangle]
unsafe extern "C" fn ohci_bus_suspend(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ohci_bus_resume(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ohci_rh_resume(ohci: *mut ohci_hcd) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ohci_hub_status_data(hcd: *mut usb_hcd, buf: *mut c_char) -> c_int {
    int ohci_hub_status_data(struct usb_hcd *hcd, char *buf)
    {
    struct ohci_hcd	*ohci = hcd_to_ohci (hcd);
    int		i, changed = 0, length = 1;
    let mut any_connected: c_int = 0;
    int		rhsc_status;
    unsigned long	flags;
...
// look at each port
    ...
    let mut status: u32 = roothub_portstatus (ohci, i);
...
#[no_mangle]
unsafe extern "C" fn ohci_start_port_reset(hcd: *mut usb_hcd, port: unsigned) -> c_int {
...
//
pub const PORT_RESET_MSEC: c_int = 50;
// this timer value might be vendor-specific ...
pub const PORT_RESET_HW_MSEC: c_int = 10;
// wrap-aware logic morphed from <linux/jiffies.h>

// called from some task, normally hub_wq
#[no_mangle]
pub unsafe extern "C" fn root_port_reset(ohci: *mut ohci_hcd, port: unsigned) -> c_int {
    static inline int root_port_reset (struct ohci_hcd *ohci, unsigned port)
    {
    __hc32 __iomem *portstat = &ohci.regs.roothub.portstatus [port];
    let mut temp: u32 = 0;
    let mut now: u16 = ohci_readl(ohci, &ohci.regs.fmnumber);
    let mut reset_done: u16 = now + PORT_RESET_MSEC;
    let mut limit_1: c_int = DIV_ROUND_UP(PORT_RESET_MSEC, PORT_RESET_HW_MSEC);
...
//
    ...
    struct ohci_hcd	*ohci = hcd_to_ohci (hcd);
    let mut ports: c_int = ohci.num_ports;
    u32		temp;
