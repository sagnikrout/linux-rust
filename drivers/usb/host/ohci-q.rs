...
//! Source: drivers/usb/host/ohci-q.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn urb_free_priv(hc: *mut ohci_hcd, urb_priv: *mut urb_priv_t) {
    static void urb_free_priv (struct ohci_hcd *hc, urb_priv_t *urb_priv)
    {
    let mut last: c_int = urb_priv.length - 1;
    if (last >= 0) {
    int		i;
    struct td	*td;
    for (i = 0; i <= last; i++) {
...
//
#[no_mangle]
unsafe extern "C" fn balance(ohci: *mut ohci_hcd, interval: c_int, load: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn periodic_link(ohci: *mut ohci_hcd, ed: *mut ed) {
...
#[no_mangle]
unsafe extern "C" fn ed_schedule(ohci: *mut ohci_hcd, ed: *mut ed) -> c_int {
    static int ed_schedule (struct ohci_hcd *ohci, struct ed *ed)
    {
    int	branch;
    ed.ed_prev = core::ptr::null_mut();
    ed.ed_next = core::ptr::null_mut();
    ed.hwNextED = 0;
    wmb ();
...
//
    switch (ed.type) {
    case PIPE_CONTROL:
    if (ohci.ed_controltail == core::ptr::null_mut()) {
    ...
    break;
    case PIPE_BULK:
    if (ohci.ed_bulktail == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn periodic_unlink(ohci: *mut ohci_hcd, ed: *mut ed) {
...
//
#[no_mangle]
unsafe extern "C" fn ed_deschedule(ohci: *mut ohci_hcd, ed: *mut ed) {
...
// remove ED from the HC's list:
    if (ed.ed_prev == core::ptr::null_mut()) {
    if (!ed.hwNextED) {
    ohci.hc_control &= ~OHCI_CTRL_CLE;
    ohci_writel (ohci, ohci.hc_control,
    &ohci.regs.control);
...
// remove ED from the HCD's list:
    ...
    ohci.ed_controltail.ed_next = core::ptr::null_mut();
    } else if (ed.ed_next) {
...
// remove ED from the HCD's list:
    ...
    if (ohci.ed_bulktail)
    ohci.ed_bulktail.ed_next = core::ptr::null_mut();
    } else if (ed.ed_next) {
    ed.ed_next.ed_prev = ed.ed_prev;
    }
    break;
...
//
#[no_mangle]
unsafe extern "C" fn start_ed_unlink(ohci: *mut ohci_hcd, ed: *mut ed) {
...
// enqueue next TD for this URB (OHCI spec 5.2.8.2)
    ...
    let mut is_iso: c_int = info & TD_ISO;
    int			hash;
...
// fill the old dummy TD
    ...
    td.ed = urb_priv.ed;
    td.next_dl_td = core::ptr::null_mut();
    td.index = index;
    td.urb = urb;
    td.data_dma = data;
    if (!len)
...
//
    ...
    struct device *dev = ohci_to_hcd(ohci).self.controller;
    dma_addr_t	data;
    let mut data_len: c_int = urb.transfer_buffer_length;
    let mut cnt: c_int = 0;
    let mut info: u32 = 0;
    let mut is_out: c_int = usb_pipeout (urb.pipe);
    let mut periodic: c_int = 0;
    int		i, this_sg_len, n;
...
//
    ...
    cnt++) {
    let mut frame: c_int = urb.start_frame;
...
#[no_mangle]
unsafe extern "C" fn td_done(ohci: *mut ohci_hcd, urb: *mut urb, td: *mut td) -> c_int {
    static int td_done(struct ohci_hcd *ohci, struct urb *urb, struct td *td)
    {
    let mut tdINFO: u32 = hc32_to_cpup (ohci, &td.hwINFO);
    let mut cc: c_int = 0;
    let mut status: c_int = -EINPROGRESS;
    list_del (&td.td_list);
// ISO ... drivers see per-TD length/status
    if (tdINFO & TD_ISO) {
    let mut tdPSW: u16 = ohci_hwPSW(ohci, td, 0);
...
//
    ...
    let mut type: c_int = usb_pipetype (urb.pipe);
    let mut tdBE: u32 = hc32_to_cpup (ohci, &td.hwBE);
    cc = TD_CC_GET (tdINFO);
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn ed_halted(ohci: *mut ohci_hcd, td: *mut td, cc: c_int) {
    ...
    let mut toggle: __hc32 = ed.hwHeadP & cpu_to_hc32 (ohci, ED_C);
...
#[no_mangle]
unsafe extern "C" fn add_to_done_list(ohci: *mut ohci_hcd, td: *mut td) {
...
#[no_mangle]
unsafe extern "C" fn finish_unlinks(ohci: *mut ohci_hcd) {
    static void finish_unlinks(struct ohci_hcd *ohci)
    {
    let mut tick: unsigned = ohci_frame_no(ohci);
    struct ed	*ed, **last;
    rescan_all:
    for (last = &ohci.ed_rm_list, ed = *last; ed != core::ptr::null_mut(); ed = *last) {
...
#[no_mangle]
pub unsafe extern "C" fn if(TD_T_DATA1: (tdINFO & TD_T) ==) -> else {
...
// maybe reenable control and bulk lists
    ...
    let mut command: u32 = 0, control = 0;
    if (ohci.ed_controltail) {
    command |= OHCI_CLF;
...
#[no_mangle]
unsafe extern "C" fn takeback_td(ohci: *mut ohci_hcd, td: *mut td) {
...
//
#[no_mangle]
unsafe extern "C" fn process_done_list(ohci: *mut ohci_hcd) {
    ...
    td = ohci.dl_start;
    if (td == ohci.dl_end)
    ohci.dl_start = ohci.dl_end = core::ptr::null_mut();
    else
...
//
#[no_mangle]
unsafe extern "C" fn ohci_work(ohci: *mut ohci_hcd) {
