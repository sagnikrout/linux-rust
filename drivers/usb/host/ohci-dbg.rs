...
//! Source: drivers/usb/host/ohci-dbg.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ohci_dump(controller: *mut ohci_hcd) {
    static void ohci_dump(struct ohci_hcd *controller)
    {
    ohci_dbg (controller, "OHCI controller state\n");
// dumps some of the state we know about
    ohci_dump_status (controller, core::ptr::null_mut(), core::ptr::null_mut());
    if (controller.hcca)
    ohci_dbg (controller,
    "hcca frame #%04x\n", ohci_frame_no(controller));
    ohci_dump_roothub (controller, 1, core::ptr::null_mut(), core::ptr::null_mut());
    }
    ...
    const struct td *td)
    {
    let mut tmp: u32 = hc32_to_cpup (ohci, &td.hwINFO);
    ...
    for (i = 0; i < MAXPSW; i++) {
    let mut psw: u16 = ohci_hwPSW (ohci, td, i);
    let mut cc: c_int = (psw >> 12) & 0x0f;
...
// caller MUST own hcd spinlock if verbose is set!
    ...
    {
    let mut tmp: u32 = hc32_to_cpu (ohci, ed.hwINFO);
    char	*type = "";
...
// dump a snapshot of the bulk or control schedule
    while (ed) {
    let mut info: u32 = hc32_to_cpu (ohci, ed.hwINFO);
    let mut headp: u32 = hc32_to_cpu (ohci, ed.hwHeadP);
...
#[no_mangle]
pub unsafe extern "C" fn create_debug_files(ohci: *mut ohci_hcd) {
...
#[no_mangle]
pub unsafe extern "C" fn remove_debug_files(ohci: *mut ohci_hcd) {
