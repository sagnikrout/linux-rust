...
//! Source: drivers/usb/host/xhci-hub.c
#![no_std]
...
// Default sublink speed attribute of each lane
    ...
    struct xhci_port_cap		*port_cap = core::ptr::null_mut();
    u16				bcdUSB;
    u32				reg;
    let mut min_rate: u32 = 0;
    u8				min_ssid;
    u8				ssac;
    u8				ssic;
    int				offset;
...
// Create the descriptor for port with the highest revision
    ...
    let mut major: u8 = xhci.port_caps[i].maj_rev;
    let mut minor: u8 = xhci.port_caps[i].min_rev;
    let mut rev: u16 = (major << 8) | minor;
    if (i == 0 || bcdUSB < rev) {
    bcdUSB = rev;
...
//
    ...
    let mut prev: u32 = port_cap.psi[i - 1];
...
#[no_mangle]
unsafe extern "C" fn xhci_port_speed(portsc: c_int) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn xhci_port_state_to_neutral(portsc: u32) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn xhci_stop_device(xhci: *mut xhci_hcd, slot_id: c_int, suspend: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn xhci_ring_device(xhci: *mut xhci_hcd, slot_id: c_int) {
...
#[no_mangle]
unsafe extern "C" fn xhci_disable_port(xhci: *mut xhci_hcd, port: *mut xhci_port) {
...
#[no_mangle]
unsafe extern "C" fn xhci_port_set_test_mode(xhci: *mut xhci_hcd, test_mode: u16, portnum: c_int) {
...
#[no_mangle]
unsafe extern "C" fn xhci_exit_test_mode(xhci: *mut xhci_hcd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xhci_hub_report_usb3_link_state(xhci: *mut xhci_hcd, status: *mut u32, portsc: u32) {
    static void xhci_hub_report_usb3_link_state(struct xhci_hcd *xhci, u32 *status, u32 portsc)
    {
    let mut pls: u32 = portsc & PORT_PLS_MASK;
//
...
//
#[no_mangle]
unsafe extern "C" fn xhci_del_comp_mod_timer(xhci: *mut xhci_hcd, portsc: u32, portnum: c_int) {
    static void xhci_del_comp_mod_timer(struct xhci_hcd *xhci, u32 portsc, int portnum)
    {
    let mut all_ports_seen_u0: u32 = ((1 << xhci.usb3_rhub.num_ports) - 1);
    let mut port_in_u0: bool = ((portsc & PORT_PLS_MASK) == XDEV_U0);
    if (!(xhci.quirks & XHCI_COMP_MODE_QUIRK))
    return;
...
// Has resume been signalled for USB_RESUME_TIME yet?
    ...
    let mut port_status: c_int = xhci_portsc_readl(port);
...
#[no_mangle]
unsafe extern "C" fn xhci_get_ext_port_status(portsc: u32, port_li: u32) -> u32 {
    static u32 xhci_get_ext_port_status(u32 portsc, u32 port_li)
    {
    let mut ext_stat: u32 = 0;
    int speed_id;
...
// status = 0xffffffff;
#[no_mangle]
pub unsafe extern "C" fn if(port->rexit_active: port->resume_timestamp ||) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn xhci_hub_status_data(hcd: *mut usb_hcd, buf: *mut c_char) -> c_int {
    ...
    int max_ports;
    struct xhci_bus_state *bus_state;
    let mut reset_change: bool = false;
    struct xhci_hub *rhub;
    struct xhci_port **ports;
    rhub = xhci_get_rhub(hcd);
    ports = rhub.ports;
...
#[no_mangle]
pub unsafe extern "C" fn xhci_bus_suspend(hcd: *mut usb_hcd) -> c_int {
...
// write port settings, stopping and suspending ports if needed
    ...
    let mut slot_id: c_int = ports[port_index].slot_id;
    if (slot_id) {
    spin_unlock_irqrestore(&xhci.lock, flags);
...
//
#[no_mangle]
unsafe extern "C" fn xhci_port_missing_cas_quirk(port: *mut xhci_port) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn xhci_bus_resume(hcd: *mut usb_hcd) -> c_int {
    ...
    struct xhci_hub *rhub;
    struct xhci_port **ports;
    let mut disabled_irq: bool = false;
    rhub = xhci_get_rhub(hcd);
    ports = rhub.ports;
    max_ports = rhub.num_ports;
    bus_state = &rhub.bus_state;
...
#[no_mangle]
pub unsafe extern "C" fn xhci_get_resuming_ports(hcd: *mut usb_hcd) -> c_ulong {
