...
//! Source: drivers/usb/host/ehci-hub.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn persist_enabled_on_companion(udev: *mut usb_device, unused: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ehci_handover_companion_ports(ehci: *mut ehci_hcd) {
...
//
    if (!usb_for_each_dev(core::ptr::null_mut(), persist_enabled_on_companion))
    return;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(port, _arg: &ehci->companion_ports)) -> else {
...
#[no_mangle]
unsafe extern "C" fn ehci_port_change(ehci: *mut ehci_hcd) -> c_int {
    static int ehci_port_change(struct ehci_hcd *ehci)
    {
    let mut i: c_int = HCS_N_PORTS(ehci.hcs_params);
...
// clear phy low-power mode before changing wakeup flags
    ...
    let mut t1: u32 = ehci_readl(ehci, reg) & ~PORT_RWC_BITS;
    let mut t2: u32 = t1 & ~PORT_WAKE_BITS;
...
#[no_mangle]
unsafe extern "C" fn ehci_bus_suspend(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(PORT_SUSPEND): (t1 & PORT_PE) && !(t1 &) -> else {
    t2 |= PORT_SUSPEND;
...
#[no_mangle]
unsafe extern "C" fn ehci_bus_resume(hcd: *mut usb_hcd) -> c_int {
    ...
    u32			power_okay;
    int			i;
    let mut resume_needed: c_ulong = 0;
    if (time_before (jiffies, ehci.next_statechange))
    msleep(5);
    spin_lock_irq (&ehci.lock);
...
#[no_mangle]
unsafe extern "C" fn ehci_get_resuming_ports(hcd: *mut usb_hcd) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn set_owner(ehci: *mut ehci_hcd, portnum: c_int, new_owner: c_int) {
...
// -------------------------------------------------------------------------
    ...
    let mut ports: c_int = HCS_N_PORTS (ehci.hcs_params);
    u16		temp;
...
#[no_mangle]
unsafe extern "C" fn ehci_relinquish_port(hcd: *mut usb_hcd, portnum: c_int) {
...
#[no_mangle]
unsafe extern "C" fn ehci_port_handed_over(hcd: *mut usb_hcd, portnum: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ehci_port_power(ehci: *mut ehci_hcd, portnum: c_int, enable: bool) -> c_int {
    static int ehci_port_power(struct ehci_hcd *ehci, int portnum, bool enable)
    {
    struct usb_hcd *hcd = ehci_to_hcd(ehci);
    u32 __iomem *status_reg = &ehci.regs.port_status[portnum];
    let mut temp: u32 = ehci_readl(ehci, status_reg) & ~PORT_RWC_BITS;
    if (enable)
