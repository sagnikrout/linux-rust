...
//! Source: drivers/usb/core/devices.c
#![no_std]
...
// Macro flag: #define ALLOW_SERIAL_NUMBER
...
// getting these some in 2.3.7; none in 2.3.6
    ...
    if (config.intf_assoc[i] == core::ptr::null_mut())
    break;
...
//
    ...
    {
    let mut bcdUSB: u16 = le16_to_cpu(desc.bcdUSB);
    let mut bcdDevice: u16 = le16_to_cpu(desc.bcdDevice);
    if (start > end)
    return start;
...
//
    ...
    int chix;
    int ret, cnt = 0;
    let mut parent_devnum: c_int = 0;
    char *pages_start, *data_end, *speed;
    unsigned int length;
    let mut total_written: isize = 0;
    struct usb_device *childdev = core::ptr::null_mut();
...
// Now look at all of this device's children.
    ...
    let mut skip_bytes: loff_t = *ppos;
    int id;
    if (*ppos < 0)
    return -EINVAL;
    if (nbytes <= 0)
