...
//! Source: drivers/usb/gadget/epautoconf.c
#![no_std]
...
// report address
    ...
    let mut num: u8 = simple_strtoul(&ep.name[2], core::ptr::null_mut(), 10);
...
//
    ...
    struct usb_ep	*ep;
    u8		type;
    ep = usb_ep_autoconfig_ss(gadget, desc, core::ptr::null_mut());
    if (!ep)
    return core::ptr::null_mut();
    type = usb_endpoint_type(desc);
// report (variable) full speed bulk maxpacket
    if (type == USB_ENDPOINT_XFER_BULK) {
    let mut size: c_int = ep.maxpacket_limit;
...
//
#[no_mangle]
pub unsafe extern "C" fn usb_ep_autoconfig_release(ep: *mut usb_ep) {
    void usb_ep_autoconfig_release(struct usb_ep *ep)
    {
    ep.claimed = false;
    ep.driver_data = core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn usb_ep_autoconfig_reset(gadget: *mut usb_gadget) {
