...
//! Source: drivers/usb/gadget/function/f_uac1_legacy.c
#![no_std]
...
//
pub const F_AUDIO_AC_INTERFACE: c_int = 0;
pub const F_AUDIO_AS_INTERFACE: c_int = 1;
pub const F_AUDIO_NUM_INTERFACES: c_int = 1;
...
pub const INPUT_TERMINAL_ID: c_int = 1;
...
pub const FEATURE_UNIT_ID: c_int = 2;
...
pub const OUTPUT_TERMINAL_ID: c_int = 3;
...
#[no_mangle]
unsafe extern "C" fn f_audio_buffer_free(audio_buf: *mut f_audio_buf) {
...
pub struct f_audio {
    pub card: gaudio,
    pub ac_alt: u8 ac_intf,,
    pub as_alt: u8 as_intf,,
// endpoints handle full and/or high speeds
    pub out_ep: *mut usb_ep,
    pub lock: spinlock_t,
    pub copy_buf: *mut f_audio_buf,
    pub playback_work: work_struct,
    pub play_queue: list_head,
// Control Set command
    pub cs: list_head,
    pub set_cmd: u8,
    pub set_con: *mut usb_audio_control,
}
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn f_audio_playback_work(data: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn f_audio_out_ep_complete(ep: *mut usb_ep, req: *mut usb_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f_audio_complete(ep: *mut usb_ep, req: *mut usb_request) {
    ...
    let mut type: u8 = con.type;
    u32 data;
    let mut valid_request: bool = false;
    switch (type) {
    case UAC_FU_MUTE: {
    u8 value;
    ...
    usb_ep_set_halt(ep);
    audio.set_con = core::ptr::null_mut();
    }
    break;
    default:
    break;
    ...
    let mut id: u8 = ((le16_to_cpu(ctrl.wIndex) >> 8) & 0xFF);
    let mut len: u16 = le16_to_cpu(ctrl.wLength);
    let mut w_value: u16 = le16_to_cpu(ctrl.wValue);
    let mut con_sel: u8 = (w_value >> 8) & 0xFF;
    let mut cmd: u8 = (ctrl.bRequest & 0x0F);
...
unsafe extern "C" fn f_audio_get_alt(f: *mut usb_function, intf: unsigned) -> c_int {
    ...
    return audio.ac_alt;
#[no_mangle]
pub unsafe extern "C" fn if(audio->as_intf: intf ==) -> else {
    else if (intf == audio.as_intf)
    return audio.as_alt;
    else
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn f_audio_build_desc(audio: *mut f_audio) {
...
// copy descriptors, and track endpoint copies
    status = usb_assign_descriptors(f, f_audio_desc, f_audio_desc, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (status)
    goto fail;
    return 0;
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn generic_set_cmd(con: *mut usb_audio_control, cmd: u8, value: c_int) -> c_int {
    static int generic_set_cmd(struct usb_audio_control *con, u8 cmd, int value)
    {
    con.data[cmd] = value;
    return 0;
...
#[no_mangle]
unsafe extern "C" fn generic_get_cmd(con: *mut usb_audio_control, cmd: u8) -> c_int {
    static int generic_get_cmd(struct usb_audio_control *con, u8 cmd)
    {
    return con.data[cmd];
    }
...
#[no_mangle]
unsafe extern "C" fn control_selector_init(audio: *mut f_audio) -> c_int {
