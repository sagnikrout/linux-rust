...
//! Source: drivers/usb/gadget/function/f_uac1.c
#![no_std]
...
pub const UAC1_CHANNEL_MASK: c_uint = 0x0FFF;
...
// Interrupt IN endpoint of AC interface
    ...
    pub /: *mut *mut int ctl_id; / EP id,
    pub /: *mut *mut int c_srate; / current capture srate,
    pub /: *mut *mut int p_srate; / current playback prate,
}
...
//
#[no_mangle]
unsafe extern "C" fn uac_cs_attr_sample_rate(ep: *mut usb_ep, req: *mut usb_request) {
...
#[no_mangle]
unsafe extern "C" fn audio_notify_complete(_ep: *mut usb_ep, req: *mut usb_request) {
...
#[no_mangle]
unsafe extern "C" fn audio_notify(audio: *mut g_audio, unit_id: c_int, cs: c_int) -> c_int {
    ...
    let mut w_length: u16 = le16_to_cpu(cr.wLength);
    let mut w_index: u16 = le16_to_cpu(cr.wIndex);
    let mut w_value: u16 = le16_to_cpu(cr.wValue);
    let mut entity_id: u8 = (w_index >> 8) & 0xff;
    let mut control_selector: u8 = w_value >> 8;
    let mut value: c_int = -EOPNOTSUPP;
    if ((FUIN_EN(opts) && (entity_id == USB_IN_FU_ID)) ||
    (FUOUT_EN(opts) && (entity_id == USB_OUT_FU_ID))) {
    let mut is_playback: c_uint = 0;
    if (FUIN_EN(opts) && (entity_id == USB_IN_FU_ID))
    is_playback = 1;
...
// (u8 *)req->buf = mute;
    ...
    let mut value: c_int = -EOPNOTSUPP;
    let mut ep: u8 = ((le16_to_cpu(ctrl.wIndex) >> 8) & 0xFF);
    let mut len: u16 = le16_to_cpu(ctrl.wLength);
    ...
    let mut value: c_int = -EOPNOTSUPP;
    let mut ep: u16 = le16_to_cpu(ctrl.wIndex);
    let mut len: u16 = le16_to_cpu(ctrl.wLength);
    let mut w_value: u16 = le16_to_cpu(ctrl.wValue);
    let mut cs: u8 = w_value >> 8;
    DBG(cdev, "bRequest 0x%x, w_value 0x%04x, len %d, endpoint %d\n",
    ctrl.bRequest, w_value, len, ep);
    switch (ctrl.bRequest) {
    case UAC_SET_CUR: {
    ...
    u8 *buf = (u8 *)req.buf;
    let mut value: c_int = -EOPNOTSUPP;
    let mut ep: u8 = le16_to_cpu(ctrl.wIndex);
    let mut len: u16 = le16_to_cpu(ctrl.wLength);
    let mut w_value: u16 = le16_to_cpu(ctrl.wValue);
    let mut cs: u8 = w_value >> 8;
    let mut val: u32 = 0;
    ...
    val = uac1.p_srate;
#[no_mangle]
pub unsafe extern "C" fn if(1): ep == (USB_DIR_OUT |) -> else {
...
#[no_mangle]
unsafe extern "C" fn f_audio_set_alt(f: *mut usb_function, intf: unsigned, alt: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f_audio_get_alt(f: *mut usb_function, intf: unsigned) -> c_int {
    ...
    return uac1.ac_alt;
#[no_mangle]
pub unsafe extern "C" fn if(uac1->as_out_intf: intf ==) -> else {
    else if (intf == uac1.as_out_intf)
    return uac1.as_out_alt;
#[no_mangle]
pub unsafe extern "C" fn if(uac1->as_in_intf: intf ==) -> else {
    else if (intf == uac1.as_in_intf)
...
#[no_mangle]
unsafe extern "C" fn f_audio_disable(f: *mut usb_function) {
...
// -------------------------------------------------------------------------
    ...
    let mut channels: c_int = num_channels(chmask);
    let mut fu_desc_size: c_int = UAC_DT_FEATURE_UNIT_SIZE(channels);
...
#[no_mangle]
unsafe extern "C" fn setup_descriptor(opts: *mut f_uac1_opts) {
...
// patch descriptors
    ...
    if (EPIN_EN(opts)) {
    let mut len: u16 = le16_to_cpu(ac_header_desc.wTotalLength);
    ...
    f_audio_desc[i] = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn f_audio_validate_opts(audio: *mut g_audio, dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f_audio_bind(c: *mut usb_configuration, f: *mut usb_function) -> c_int {
    ...
    struct usb_ep			*ep = core::ptr::null_mut();
...
// copy descriptors, and track endpoint copies
    ...
    ac_header_desc = core::ptr::null_mut();
    return status;
    }
// -------------------------------------------------------------------------
...
#[no_mangle]
unsafe extern "C" fn f_uac1_attr_release(item: *mut config_item) {
...
#[no_mangle]
unsafe extern "C" fn f_audio_free_inst(f: *mut usb_function_instance) {
...
#[no_mangle]
unsafe extern "C" fn f_audio_free(f: *mut usb_function) {
...
#[no_mangle]
unsafe extern "C" fn f_audio_unbind(c: *mut usb_configuration, f: *mut usb_function) {
    ...
    kfree(ac_header_desc);
    ac_header_desc = core::ptr::null_mut();
    audio.gadget = core::ptr::null_mut();
    }
