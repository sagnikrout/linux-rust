...
//! Source: drivers/usb/gadget/udc/aspeed-vhub/epn.c
#![no_std]
...
// Macro flag: #define EXTRA_CHECKS
...
#[no_mangle]
unsafe extern "C" fn ast_vhub_epn_kick(ep: *mut ast_vhub_ep, req: *mut ast_vhub_req) {
    static void ast_vhub_epn_kick(struct ast_vhub_ep *ep, struct ast_vhub_req *req)
    {
    let mut act: c_uint = req.req.actual;
    let mut len: c_uint = req.req.length;
    unsigned int chunk;
...
#[no_mangle]
pub unsafe extern "C" fn if(!req->req.zero: (chunk < ep->ep.maxpacket) ||) -> else {
...
#[no_mangle]
unsafe extern "C" fn ast_vhub_epn_handle_ack(ep: *mut ast_vhub_ep) {
...
#[no_mangle]
pub unsafe extern "C" fn ast_vhub_count_free_descs(ep: *mut ast_vhub_ep) -> c_uint {
...
//
    ...
    struct ast_vhub_req *req)
    {
    struct ast_vhub_desc *desc = core::ptr::null_mut();
    let mut act: c_uint = req.act_count;
    let mut len: c_uint = req.req.length;
    unsigned int chunk;
...
#[no_mangle]
unsafe extern "C" fn ast_vhub_epn_handle_ack_desc(ep: *mut ast_vhub_ep) {
...
#[no_mangle]
pub unsafe extern "C" fn ast_vhub_epn_ack_irq(ep: *mut ast_vhub_ep) {
...
#[no_mangle]
unsafe extern "C" fn ast_vhub_epn_dequeue(u_ep: *mut *mut usb_ep, u_req: *mut usb_request) -> c_int {
    static int ast_vhub_epn_dequeue(struct usb_ep* u_ep, struct usb_request *u_req)
    {
    struct ast_vhub_ep *ep = to_ast_ep(u_ep);
    struct ast_vhub *vhub = ep.vhub;
    struct ast_vhub_req *req = core::ptr::null_mut(), *iter;
    unsigned long flags;
...
#[no_mangle]
pub unsafe extern "C" fn ast_vhub_update_epn_stall(ep: *mut ast_vhub_ep) {
...
#[no_mangle]
unsafe extern "C" fn ast_vhub_epn_dispose(u_ep: *mut usb_ep) {
...
// Mark the address free in the device
    ep.dev.epns[ep.d_idx - 1] = core::ptr::null_mut();
// Free name & DMA buffers
    kfree(ep.ep.name);
    ep.ep.name = core::ptr::null_mut();
    dma_free_coherent(&ep.vhub.pdev.dev,
    AST_VHUB_EPn_MAX_PACKET +
    8 * AST_VHUB_DESCS_COUNT,
    ep.buf, ep.buf_dma);
    ep.buf = core::ptr::null_mut();
    ep.epn.descs = core::ptr::null_mut();
// Mark free
    ep.dev = core::ptr::null_mut();
    }
...
// Find a free one (no device)
    ...
    if (vhub.epns[i].dev == core::ptr::null_mut())
    break;
    if (i >= vhub.max_epns) {
    spin_unlock_irqrestore(&vhub.lock, flags);
