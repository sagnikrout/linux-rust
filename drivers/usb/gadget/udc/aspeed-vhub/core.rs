...
//! Source: drivers/usb/gadget/udc/aspeed-vhub/core.c
#![no_std]
...
//
    ...
    int status)
    {
    let mut internal: bool = req.internal;
...
#[no_mangle]
pub unsafe extern "C" fn ast_vhub_nuke(ep: *mut ast_vhub_ep, status: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn ast_vhub_free_request(u_ep: *mut usb_ep, u_req: *mut usb_request) {
...
#[no_mangle]
unsafe extern "C" fn ast_vhub_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
// Handle generic EPs first
    ...
    let mut ep_acks: u32 = readl(vhub.regs + AST_VHUB_EP_ACK_ISR);
    writel(ep_acks, vhub.regs + AST_VHUB_EP_ACK_ISR);
    for (i = 0; ep_acks && i < vhub.max_epns; i++) {
    let mut mask: u32 = VHUB_EP_IRQ(i);
    if (ep_acks & mask) {
...
#[no_mangle]
pub unsafe extern "C" fn ast_vhub_init_hw(vhub: *mut ast_vhub) {
...
#[no_mangle]
unsafe extern "C" fn ast_vhub_remove(pdev: *mut platform_device) {
...
// Pull device, leave PHY enabled
    ...
    vhub.ep0_bufs,
    vhub.ep0_bufs_dma);
    vhub.ep0_bufs = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn ast_vhub_probe(pdev: *mut platform_device) -> c_int {
    ...
    vhub.clk = devm_clk_get(&pdev.dev, core::ptr::null_mut());
    if (IS_ERR(vhub.clk)) {
    rc = PTR_ERR(vhub.clk);
    goto err;
    }
    ...
    vhub.rst = devm_reset_control_get_optional_shared(&pdev.dev, core::ptr::null_mut());
    if (IS_ERR(vhub.rst)) {
    rc = PTR_ERR(vhub.rst);
    goto err;
    }
...
// Init vHub EP0
    ast_vhub_init_ep0(vhub, &vhub.ep0, core::ptr::null_mut());
// Init devices
...
// Initialize HW
    ...
    let mut dma_mask_32: static u64 = DMA_BIT_MASK(32);
    let mut dma_mask_64: static u64 = DMA_BIT_MASK(64);
