...
//! Source: drivers/usb/gadget/udc/renesas_usbf.c
#![no_std]
...
pub const USBF_NUM_ENDPOINTS: c_int = 16;
pub const USBF_EP0_MAX_PCKT_SIZE: c_int = 64;
// EPC registers
pub const USBF_REG_USB_CONTROL: c_uint = 0x000;

pub const USBF_REG_USB_STATUS: c_uint = 0x004;

pub const USBF_REG_USB_ADDRESS: c_uint = 0x008;

pub const USBF_REG_SETUP_DATA0: c_uint = 0x018;
pub const USBF_REG_SETUP_DATA1: c_uint = 0x01C;
pub const USBF_REG_USB_INT_STA: c_uint = 0x020;

pub const USBF_REG_USB_INT_ENA: c_uint = 0x024;

pub const USBF_BASE_EP0: c_uint = 0x028;
// EP0 registers offsets from Base + USBF_BASE_EP0 (EP0 regs area)
pub const USBF_REG_EP0_CONTROL: c_uint = 0x00;

pub const USBF_REG_EP0_STATUS: c_uint = 0x04;

pub const USBF_REG_EP0_INT_ENA: c_uint = 0x08;

pub const USBF_REG_EP0_LENGTH: c_uint = 0x0C;

pub const USBF_REG_EP0_READ: c_uint = 0x10;
pub const USBF_REG_EP0_WRITE: c_uint = 0x14;

// EPn registers offsets from Base + USBF_BASE_EPN(n-1). n=1..15
pub const USBF_REG_EPN_CONTROL: c_uint = 0x000;

pub const USBF_REG_EPN_STATUS: c_uint = 0x004;

pub const USBF_REG_EPN_INT_ENA: c_uint = 0x008;

pub const USBF_REG_EPN_DMA_CTRL: c_uint = 0x00C;

pub const USBF_REG_EPN_PCKT_ADRS: c_uint = 0x010;

pub const USBF_REG_EPN_LEN_DCNT: c_uint = 0x014;

pub const USBF_REG_EPN_READ: c_uint = 0x018;
pub const USBF_REG_EPN_WRITE: c_uint = 0x01C;
// AHB-EPC Bridge registers
pub const USBF_REG_AHBSCTR: c_uint = 0x1000;
pub const USBF_REG_AHBMCTR: c_uint = 0x1004;

pub const USBF_REG_AHBBINT: c_uint = 0x1008;

pub const USBF_REG_AHBBINTEN: c_uint = 0x100C;

pub const USBF_REG_EPCTR: c_uint = 0x1010;

pub const USBF_REG_USBSSVER: c_uint = 0x1020;
pub const USBF_REG_USBSSCONF: c_uint = 0x1024;

// EPn DMA registers offsets from Base USBF_BASE_DMA_EPN(n-1). n=1..15
pub const USBF_REG_DMA_EPN_DCR1: c_uint = 0x00;

pub const USBF_REG_DMA_EPN_DCR2: c_uint = 0x04;

pub const USBF_REG_DMA_EPN_TADR: c_uint = 0x08;
// USB request
#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbf_req {
    pub req: usb_request,
    pub queue: list_head,
    pub 1: unsigned int is_zero_sent :,
    pub 1: unsigned int is_mapped :,
    enum {
    USBF_XFER_START,
    USBF_XFER_WAIT_DMA,
    USBF_XFER_SEND_NULL,
...
pub struct usbf_ep {
    ...
    pub name: [c_char; 32],
    pub queue: list_head,
    pub 1: unsigned int is_processing :,
    pub 1: unsigned int is_in :,
    pub udc: *mut usbf_udc,
    pub regs: *mut void __iomem,
    pub dma_regs: *mut void __iomem,
    pub 8: unsigned int id :,
    pub 1: unsigned int disabled :,
    pub 1: unsigned int is_wedged :,
    pub 1: unsigned int delayed_status :,
    pub status: u32,
    pub ep): *mut *mut void (bridge_on_dma_end)(struct usbf_ep,
}
...
pub struct usbf_udc {
    ...
    pub regs: *mut void __iomem,
    pub lock: spinlock_t,
    pub is_remote_wakeup: bool,
    pub is_usb_suspended: bool,
    pub ep: [usbf_ep; USBF_NUM_ENDPOINTS],
// for EP0 control messages
    pub ep0state: enum usbf_ep0state,
    pub setup_reply: usbf_req,
    pub ep0_buf: [u8; USBF_EP0_MAX_PCKT_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbf_ep_info {
    pub name: *const c_char,
    pub caps: usb_ep_caps,
    pub base_addr: u16,
    pub 1: unsigned int is_double :,
    pub maxpacket_limit: u16,
}

pub const USBF_SINGLE_BUFFER: c_int = 0;
pub const USBF_DOUBLE_BUFFER: c_int = 1;
...
#[no_mangle]
pub unsafe extern "C" fn usbf_reg_readl(udc: *mut usbf_udc, offset: c_uint) -> u32 {
    static inline u32 usbf_reg_readl(struct usbf_udc *udc, uint offset)
    {
    return readl(udc.regs + offset);
    }
#[no_mangle]
pub unsafe extern "C" fn usbf_reg_writel(udc: *mut usbf_udc, offset: c_uint, val: u32) {
    static inline void usbf_reg_writel(struct usbf_udc *udc, uint offset, u32 val)
    {
    writel(val, udc.regs + offset);
    }
#[no_mangle]
pub unsafe extern "C" fn usbf_reg_bitset(udc: *mut usbf_udc, offset: c_uint, set: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn usbf_reg_bitclr(udc: *mut usbf_udc, offset: c_uint, clr: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn usbf_ep_reg_readl(ep: *mut usbf_ep, offset: c_uint) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn usbf_ep_reg_writel(ep: *mut usbf_ep, offset: c_uint, val: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn usbf_ep_reg_bitset(ep: *mut usbf_ep, offset: c_uint, set: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn usbf_ep_reg_bitclr(ep: *mut usbf_ep, offset: c_uint, clr: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn usbf_ep_dma_reg_readl(ep: *mut usbf_ep, offset: c_uint) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep0_send_null(ep0: *mut usbf_ep, is_data1: bool) {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep0_pio_in(ep0: *mut usbf_ep, req: *mut usbf_req) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep0_pio_out(ep0: *mut usbf_ep, req: *mut usbf_req) -> c_int {
    static int usbf_ep0_pio_out(struct usbf_ep *ep0, struct usbf_req *req)
    {
    let mut req_status: c_int = 0;
    unsigned int count;
    unsigned int recv;
    unsigned int left;
    unsigned int nb;
...
#[no_mangle]
unsafe extern "C" fn usbf_ep0_fifo_flush(ep0: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_send_null(epn: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_pio_in(epn: *mut usbf_ep, req: *mut usbf_req) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_enable_in_end_int(epn: *mut usbf_ep) {
    static void usbf_epn_enable_in_end_int(struct usbf_ep *epn)
    {
    usbf_ep_reg_bitset(epn, USBF_REG_EPN_INT_ENA, USBF_EPN_IN_END_EN);
    }
#[no_mangle]
unsafe extern "C" fn usbf_epn_dma_in(epn: *mut usbf_ep, req: *mut usbf_req) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_pio_out(epn: *mut usbf_ep, req: *mut usbf_req) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_enable_out_end_int(epn: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_dma_out_send_dma(epn: *mut usbf_ep, addr: dma_addr_t, npkt: u32, is_short: bool) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_dma_out_complete_dma(epn: *mut usbf_ep, is_short: bool) -> usize {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_dma_out(epn: *mut usbf_ep, req: *mut usbf_req) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_dma_stop(epn: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_dma_abort(epn: *mut usbf_ep, req: *mut usbf_req) {
    static void usbf_epn_dma_abort(struct usbf_ep *epn,  struct usbf_req *req)
    {
    dev_dbg(epn.udc.dev, "ep%u %s dma abort\n", epn.id,
    epn.is_in ? "in" : "out");
    epn.bridge_on_dma_end = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_fifo_flush(epn: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_nuke(ep: *mut usbf_ep, status: c_int) {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_is_stalled(ep: *mut usbf_ep) -> bool {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_start_queue(epn: *mut usbf_ep) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_process_queue(ep: *mut usbf_ep) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_stall(ep: *mut usbf_ep, stall: bool) {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep0_enable(ep0: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_enable(epn: *mut usbf_ep) -> c_int {
...
// enable DMA interrupt at bridge level if DMA is used
    if (ep.dma_regs) {
    ep.bridge_on_dma_end = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_disable(epn: *mut usbf_ep) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_disable(_ep: *mut usb_ep) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_dequeue(_ep: *mut usb_ep, _req: *mut usb_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_free_request(_ep: *mut usb_ep, _req: *mut usb_request) {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_set_halt(_ep: *mut usb_ep, halt: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_set_wedge(_ep: *mut usb_ep) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep0_req_complete(_ep: *mut usb_ep, _req: *mut usb_request) {
    ...
    req.req.status = -EINPROGRESS;
    req.req.context = core::ptr::null_mut();
    req.req.actual = 0;
    }
    ...
    sizeof(status_data), core::ptr::null_mut());
...
// Queued the status request
    usbf_ep0_fill_req(&udc.ep[0], &udc.setup_reply, core::ptr::null_mut(), 0,
...
#[no_mangle]
unsafe extern "C" fn usbf_handle_ep0_setup(ep0: *mut usbf_ep) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_handle_ep0_out_status_start(ep0: *mut usbf_ep) -> c_int {
    ...
    if (!req) {
    usbf_ep0_fill_req(ep0, &udc.setup_reply, core::ptr::null_mut(), 0, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn usbf_handle_ep0_in_status_start(ep0: *mut usbf_ep) -> c_int {
...
// Queue request for status if needed
    ...
    usbf_ep0_fill_req(ep0, &udc.setup_reply, core::ptr::null_mut(),
    0, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn usbf_ep0_interrupt(ep0: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_process_queue(epn: *mut usbf_ep) {
    static void usbf_epn_process_queue(struct usbf_ep *epn)
    {
    int ret;
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_interrupt(epn: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_ep_reset(ep: *mut usbf_ep) {
...
#[no_mangle]
unsafe extern "C" fn usbf_reset(udc: *mut usbf_udc) {
...
#[no_mangle]
unsafe extern "C" fn usbf_driver_suspend(udc: *mut usbf_udc) {
...
#[no_mangle]
unsafe extern "C" fn usbf_driver_resume(udc: *mut usbf_udc) {
...
#[no_mangle]
unsafe extern "C" fn usbf_epc_irq(irq: c_int, _udc: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn usbf_ahb_epc_irq(irq: c_int, _udc: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn usbf_udc_stop(gadget: *mut usb_gadget) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_get_frame(gadget: *mut usb_gadget) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_attach(udc: *mut usbf_udc) {
...
#[no_mangle]
unsafe extern "C" fn usbf_detach(udc: *mut usbf_udc) {
...
#[no_mangle]
unsafe extern "C" fn usbf_pullup(gadget: *mut usb_gadget, is_on: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_udc_wakeup(gadget: *mut usb_gadget) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_epn_check(epn: *mut usbf_ep) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbf_remove(pdev: *mut platform_device) {
