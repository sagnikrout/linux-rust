...
//! Source: drivers/usb/usbip/stub_tx.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn stub_complete(urb: *mut urb) {
...
// link a urb to the queue of tx.
    ...
    if (sdev.ud.tcp_socket == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn setup_ret_submit_pdu(rpdu: *mut usbip_header, urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn stub_send_ret_submit(sdev: *mut stub_device) -> c_int {
    ...
    while ((priv = dequeue_from_priv_tx(sdev)) != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn if(priv->sgl: usb_pipein(urb->pipe) &&) -> else {
...
//
    ...
    urb.actual_length > 0) {
    if (urb.num_sgs) {
    let mut copy: c_uint = urb.actual_length;
    unsigned int size;
...
#[no_mangle]
unsafe extern "C" fn stub_send_ret_unlink(sdev: *mut stub_device) -> c_int {
    ...
    size_t txsize;
    let mut total_size: usize = 0;
    while ((unlink = dequeue_from_unlink_tx(sdev)) != core::ptr::null_mut()) {
    int ret;
    struct usbip_header pdu_header;
    txsize = 0;
    memset(&pdu_header, 0, sizeof(pdu_header));
...
#[no_mangle]
pub unsafe extern "C" fn stub_tx_loop(data: *mut c_void) -> c_int {
