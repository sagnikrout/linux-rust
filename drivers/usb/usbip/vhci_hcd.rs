...
//! Source: drivers/usb/usbip/vhci_hcd.c
#![no_std]
...
// See usb gadget dummy hcd
    ...
    let mut vhci_num_controllers: c_int = VHCI_NR_HCS;
    struct vhci *vhcis;
...
#[no_mangle]
unsafe extern "C" fn dump_port_status_diff(prev_status: u32, new_status: u32, usb3: bool) {
    ...
    while (bit) {
    let mut prev: u32 = prev_status & bit;
    let mut new: u32 = new_status & bit;
    char change;
    if (!prev && new)
    change = '+';
#[no_mangle]
pub unsafe extern "C" fn if(!new: prev &&) -> else {
    else if (prev && !new)
    change = '-';
    else
    change = ' ';
...
#[no_mangle]
pub unsafe extern "C" fn rh_port_connect(vdev: *mut vhci_device, speed: enum usb_device_speed) {
    void rh_port_connect(struct vhci_device *vdev, enum usb_device_speed speed)
    {
    struct vhci_hcd	*vhci_hcd = vdev_to_vhci_hcd(vdev);
    struct vhci *vhci = vhci_hcd.vhci;
    let mut rhport: c_int = vdev.rhport;
    u32		status;
    unsigned long	flags;
...
#[no_mangle]
unsafe extern "C" fn rh_port_disconnect(vdev: *mut vhci_device) {
...
//
#[no_mangle]
unsafe extern "C" fn vhci_hub_status(hcd: *mut usb_hcd, buf: *mut c_char) -> c_int {
    static int vhci_hub_status(struct usb_hcd *hcd, char *buf)
    {
    struct vhci_hcd	*vhci_hcd = hcd_to_vhci_hcd(hcd);
    struct vhci *vhci = vhci_hcd.vhci;
    let mut retval: c_int = DIV_ROUND_UP(VHCI_HC_PORTS + 1, 8);
    int		rhport;
...
pub unsafe extern "C" fn hub_descriptor(desc: *mut usb_hub_descriptor) {
    ...
    let mut rhport: c_int = -1;
    unsigned long	flags;
    let mut invalid_rhport: bool = false;
...
#[no_mangle]
unsafe extern "C" fn vhci_tx_urb(urb: *mut urb, vdev: *mut vhci_device) {
...
#[no_mangle]
unsafe extern "C" fn vhci_urb_enqueue(hcd: *mut usb_hcd, urb: *mut urb, mem_flags: gfp_t) -> c_int {
    static int vhci_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
    {
    struct vhci_hcd *vhci_hcd = hcd_to_vhci_hcd(hcd);
    struct vhci *vhci = vhci_hcd.vhci;
    struct device *dev = &urb.dev.dev;
    let mut portnum: u8 = urb.dev.portnum;
    let mut ret: c_int = 0;
...
//
    ...
    let mut type: __u8 = usb_pipetype(urb.pipe);
...
//
#[no_mangle]
unsafe extern "C" fn vhci_urb_dequeue(hcd: *mut usb_hcd, urb: *mut urb, status: c_int) -> c_int {
    static int vhci_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
    {
    struct vhci_hcd *vhci_hcd = hcd_to_vhci_hcd(hcd);
    struct vhci *vhci = vhci_hcd.vhci;
...
#[no_mangle]
unsafe extern "C" fn vhci_device_unlink_cleanup(vdev: *mut vhci_device) {
...
//
#[no_mangle]
unsafe extern "C" fn vhci_shutdown_connection(ud: *mut usbip_device) {
...
// kill threads related to this sdev
    ...
    vdev.ud.tcp_rx = core::ptr::null_mut();
    }
    if (vdev.ud.tcp_tx) {
    kthread_stop_put(vdev.ud.tcp_tx);
    vdev.ud.tcp_tx = core::ptr::null_mut();
    }
    dev_info(hcd_dev(hcd), "stop threads\n");
// active connection is closed
    if (vdev.ud.tcp_socket) {
    sockfd_put(vdev.ud.tcp_socket);
    vdev.ud.tcp_socket = core::ptr::null_mut();
    vdev.ud.sockfd = -1;
    }
...
#[no_mangle]
unsafe extern "C" fn vhci_device_reset(ud: *mut usbip_device) {
    ...
    vdev.speed  = 0;
    vdev.devid  = 0;
    vdev.udev = core::ptr::null_mut();
    usb_put_dev(old);
    if (ud.tcp_socket) {
    sockfd_put(ud.tcp_socket);
    ud.tcp_socket = core::ptr::null_mut();
    ud.sockfd = -1;
    }
...
#[no_mangle]
unsafe extern "C" fn vhci_device_unusable(ud: *mut usbip_device) {
...
#[no_mangle]
unsafe extern "C" fn vhci_device_init(vdev: *mut vhci_device) {
...
#[no_mangle]
unsafe extern "C" fn hcd_name_to_id(name: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhci_setup(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhci_start(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhci_stop(hcd: *mut usb_hcd) {
    static void vhci_stop(struct usb_hcd *hcd)
    {
    struct vhci_hcd *vhci_hcd = hcd_to_vhci_hcd(hcd);
    int id, rhport;
...
#[no_mangle]
unsafe extern "C" fn vhci_get_frame_number(hcd: *mut usb_hcd) -> c_int {
    static int vhci_get_frame_number(struct usb_hcd *hcd)
    {
...
#[no_mangle]
unsafe extern "C" fn vhci_bus_suspend(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhci_bus_resume(hcd: *mut usb_hcd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhci_hcd_probe(pdev: *mut platform_device) -> c_int {
...
//
    ...
    put_usb2_hcd:
    usb_put_hcd(hcd_hs);
    vhci.vhci_hcd_hs = core::ptr::null_mut();
    vhci.vhci_hcd_ss = core::ptr::null_mut();
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn vhci_hcd_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn vhci_hcd_suspend(pdev: *mut platform_device, state: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vhci_hcd_resume(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn del_platform_devices() {
    static void del_platform_devices(void)
    {
    int i;
    for (i = 0; i < vhci_num_controllers; i++) {
    platform_device_unregister(vhcis[i].pdev);
    vhcis[i].pdev = core::ptr::null_mut();
    }
    sysfs_remove_link(&platform_bus.kobj, driver_name);
    }
#[no_mangle]
unsafe extern "C" fn vhci_hcd_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn vhci_hcd_exit() -> void __exit {
