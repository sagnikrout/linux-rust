...
//! Source: drivers/usb/renesas_usbhs/mod_host.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbhsh_device {
    pub usbv: *mut usb_device,
    pub /: *mut *mut list_head ep_list_head; / list of usbhsh_ep,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbhsh_ep {
    pub /: *mut *mut *mut usbhs_pipe pipe; / attached pipe,
    pub /: *mut *mut *mut usbhsh_device udev; / attached udev,
    pub ep: *mut usb_host_endpoint,
    pub /: *mut *mut list_head ep_list; / list to usbhsh_device,
    pub /: *mut *mut unsigned int counter; / pipe attach counter,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct usbhsh_hpriv {
    pub mod: usbhs_mod,
    pub dcp: *mut usbhs_pipe,
    pub udev: [usbhsh_device; USBHSH_DEVICE_MAX],
    pub /: *mut *mut u32 port_stat; / USB_PORT_STAT_xxx,
    pub setup_ack_done: completion,
}
...
//
    ...
    struct usbhsh_request *ureq)
    {
    usbhsh_urb_to_ureq(ureq.urb) = core::ptr::null_mut();
    ureq.urb = core::ptr::null_mut();
    kfree(ureq);
    }
//
// status
//
#[no_mangle]
unsafe extern "C" fn usbhsh_is_running(hpriv: *mut usbhsh_hpriv) -> c_int {
...
// usbhsh_irq_dtch()
//
    return (hpriv.mod.irq_attch == core::ptr::null_mut());
    }
//
// pipe control
//
    ...
    let mut maxp: c_int = usb_endpoint_maxp(&urb.ep.desc);
    let mut t: c_int = 0;
...
//
    ...
    unsigned long flags;
    let mut dir_in_req: c_int = !!usb_pipein(urb.pipe);
    let mut is_dcp: c_int = usb_endpoint_xfer_control(desc);
    int i, dir_in;
...
// detach pipe from uep
    usbhsh_uep_to_pipe(uep)		= core::ptr::null_mut();
    usbhsh_pipe_to_uep(pipe)	= core::ptr::null_mut();
    dev_dbg(dev, "%s [%d-%d(%s)]\n", __func__,
    usbhsh_device_number(hpriv, udev),
...
// remove this endpoint from udev
    ...
    usbhsh_uep_to_udev(uep)	= core::ptr::null_mut();
    usbhsh_uep_to_ep(uep)	= core::ptr::null_mut();
    usbhsh_ep_to_uep(ep)	= core::ptr::null_mut();
    usbhs_unlock(priv, flags);
// spin unlock
    kfree(uep);
    }
...
#[no_mangle]
unsafe extern "C" fn usbhsh_device_has_endpoint(udev: *mut usbhsh_device) -> c_int {
...
// return attached device
    ...
    struct urb *urb)
    {
    struct usbhsh_device *udev = core::ptr::null_mut();
...
// will be disable
//
    dev_set_drvdata(&usbv.dev, core::ptr::null_mut());
    udev.usbv = core::ptr::null_mut();
    usbhs_unlock(priv, flags);
// spin unlock
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn usbhsh_queue_done(priv: *mut usbhs_priv, pkt: *mut usbhs_pkt) {
...
#[no_mangle]
unsafe extern "C" fn usbhsh_queue_force_pop_all(priv: *mut usbhs_priv) {
...
//
#[no_mangle]
unsafe extern "C" fn usbhsh_is_request_address(urb: *mut urb) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn usbhsh_host_start(hcd: *mut usb_hcd) -> c_int {
    static int usbhsh_host_start(struct usb_hcd *hcd)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn usbhsh_host_stop(hcd: *mut usb_hcd) {
    ...
    struct device *dev = usbhs_priv_to_dev(priv);
    struct usb_host_endpoint *ep = urb.ep;
    struct usbhsh_device *new_udev = core::ptr::null_mut();
    let mut is_dir_in: c_int = usb_pipein(urb.pipe);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn usbhsh_urb_dequeue(hcd: *mut usb_hcd, urb: *mut urb, status: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbhsh_hub_status_data(hcd: *mut usb_hcd, buf: *mut c_char) -> c_int {
...
// buf = 0;
    ...
    struct usbhs_priv *priv = usbhsh_hpriv_to_priv(hpriv);
    struct device *dev = usbhs_priv_to_dev(priv);
    let mut enable: c_int = (typeReq == SetPortFeature);
...
#[no_mangle]
unsafe extern "C" fn usbhsh_bus_nop(hcd: *mut usb_hcd) -> c_int {
...
// usbhsh_urb_enqueue()
//
    hpriv.mod.irq_attch = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn usbhsh_pipe_init_for_host(priv: *mut usbhs_priv) {
...
//
    ...
    pipe.mod_private = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn usbhsh_start(priv: *mut usbhs_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn usbhsh_stop(priv: *mut usbhs_priv) -> c_int {
...
//
    mod.irq_attch	= core::ptr::null_mut();
    mod.irq_dtch	= core::ptr::null_mut();
    mod.irq_sack	= core::ptr::null_mut();
    mod.irq_sign	= core::ptr::null_mut();
    usbhs_irq_callback_update(priv, mod);
    usb_remove_hcd(hcd);
// disable sys
...
#[no_mangle]
pub unsafe extern "C" fn usbhs_mod_host_probe(priv: *mut usbhs_priv) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn usbhs_mod_host_remove(priv: *mut usbhs_priv) -> c_int {
