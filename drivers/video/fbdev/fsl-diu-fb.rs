...
//! Source: drivers/video/fbdev/fsl-diu-fb.c
#![no_std]
...
pub const MAX_CURS: c_int = 32;
// INT_STATUS/INT_MASK field descriptions
pub const INT_VSYNC: c_uint = 0x01	/* Vsync interrupt  */;
pub const INT_VSYNC_WB: c_uint = 0x02	/* Vsync interrupt for write back operation */;
pub const INT_UNDRUN: c_uint = 0x04	/* Under run exception interrupt */;
pub const INT_PARERR: c_uint = 0x08	/* Display parameters error interrupt */;
pub const INT_LS_BF_VS: c_uint = 0x10	/* Lines before vsync. interrupt */;
//
...
//
    ...
    static char *fb_mode;
    let mut default_bpp: static unsigned long = 32;
...
#[derive(Copy, Clone)]
pub struct mfb_info {
    pub index: enum mfb_index,
    pub id: *mut c_char,
    pub registered: c_int,
    pub pseudo_palette: [c_ulong; 16],
    pub ad: *mut diu_ad,
    pub g_alpha: c_uchar,
    pub count: c_uint,
    pub /: *mut *mut int x_aoi_d; / aoi display x offset to physical screen,
    pub /: *mut *mut int y_aoi_d; / aoi display y offset to physical screen,
    pub parent: *mut fsl_diu_data,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fsl_diu_data {
    pub dma_addr: dma_addr_t,
    pub fsl_diu_info: [fb_info; NUM_AOIS],
    pub mfb: [mfb_info; NUM_AOIS],
    pub dev_attr: device_attribute,
    pub irq: c_uint,
    pub monitor_port: enum fsl_diu_monitor_port,
    pub diu_reg: *mut diu __iomem,
    pub reg_lock: spinlock_t,
    pub 4]: *mut *mut *mut u8 dummy_aoi[4  4,
    pub __aligned(8): diu_ad dummy_ad,
    pub __aligned(8): diu_ad ad[NUM_AOIS],
    pub __aligned(32): *mut *mut u8 gamma[256  3],
// It's easier to parse the cursor data as little-endian
    pub __aligned(32): *mut *mut __le16 cursor[MAX_CURS  MAX_CURS],
// Blank cursor data -- used to hide the cursor
    pub __aligned(32): *mut *mut __le16 blank_cursor[MAX_CURS  MAX_CURS],
// Scratch cursor data -- used to build new cursor
    pub __aligned(32): *mut *mut __le16 next_cursor[MAX_CURS  MAX_CURS],
    pub edid_data: [u8; EDID_LENGTH],
    pub has_edid: bool,
    pub __aligned(32): },
...
#[no_mangle]
unsafe extern "C" fn __attribute__(hw: *mut (unused)) fsl_diu_dump(struct diu __iomem) {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_name_to_port(s: *const c_char) -> enum fsl_diu_monitor_port {
    static enum fsl_diu_monitor_port fsl_diu_name_to_port(const char *s)
    {
    let mut port: enum fsl_diu_monitor_port = FSL_DIU_PORT_DVI;
    unsigned long val;
    if (s) {
    if (!kstrtoul(s, 10, &val) && (val <= 2))
    port = (enum fsl_diu_monitor_port) val;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(s, _arg: "lvds", 0: 4) ==) -> else {
    else if (strncmp(s, "lvds", 4) == 0)
    port = FSL_DIU_PORT_LVDS;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncmp(s, _arg: "dlvds", 0: 5) ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn wr_reg_wa(reg: *mut u32, val: u32) {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_enable_panel(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_disable_panel(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn enable_lcdc(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn disable_lcdc(info: *mut fb_info) {
    ...
    let mut index: enum mfb_index = mfbi.index;
...
#[no_mangle]
unsafe extern "C" fn set_fix(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn update_lcdc(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn map_video_memory(info: *mut fb_info) -> c_int {
    static int map_video_memory(struct fb_info *info)
    {
    let mut smem_len: u32 = info.fix.line_length * info.var.yres_virtual;
    void *p;
    ...
    info.screen_base = p;
    info.fix.smem_start = virt_to_phys(( const void *)info.screen_base);
...
#[no_mangle]
unsafe extern "C" fn unmap_video_memory(info: *mut fb_info) {
    static void unmap_video_memory(struct fb_info *info)
    {
    void *p = info.screen_base;
    let mut l: usize = info.fix.smem_len;
    mutex_lock(&info.mm_lock);
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_set_aoi(info: *mut fb_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_get_pixel_format(bits_per_pixel: c_uint) -> u32 {
    static u32 fsl_diu_get_pixel_format(unsigned int bits_per_pixel)
    {
pub const PF_BYTE_F: c_uint = 0x10000000;
pub const PF_ALPHA_C_MASK: c_uint = 0x0E000000;
pub const PF_ALPHA_C_SHIFT: c_int = 25;
pub const PF_BLUE_C_MASK: c_uint = 0x01800000;
pub const PF_BLUE_C_SHIFT: c_int = 23;
pub const PF_GREEN_C_MASK: c_uint = 0x00600000;
pub const PF_GREEN_C_SHIFT: c_int = 21;
pub const PF_RED_C_MASK: c_uint = 0x00180000;
pub const PF_RED_C_SHIFT: c_int = 19;
pub const PF_PALETTE: c_uint = 0x00040000;
pub const PF_PIXEL_S_MASK: c_uint = 0x00030000;
pub const PF_PIXEL_S_SHIFT: c_int = 16;
pub const PF_COMP_3_MASK: c_uint = 0x0000F000;
pub const PF_COMP_3_SHIFT: c_int = 12;
pub const PF_COMP_2_MASK: c_uint = 0x00000F00;
pub const PF_COMP_2_SHIFT: c_int = 8;
pub const PF_COMP_1_MASK: c_uint = 0x000000F0;
pub const PF_COMP_1_SHIFT: c_int = 4;
pub const PF_COMP_0_MASK: c_uint = 0x0000000F;
pub const PF_COMP_0_SHIFT: c_int = 0;
...
//
    ...
    let mut _fg: __le16 = cpu_to_le16(fg);
    let mut _bg: __le16 = cpu_to_le16(bg);
    unsigned int h, w;
    for (h = 0; h < height; h++) {
    let mut mask: u32 = 1 << 31;
    let mut line: u32 = be32_to_cpup(image);
    for (w = 0; w < width; w++) {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_cursor(info: *mut fb_info, cursor: *mut fb_cursor) -> c_int {
...
//
    ...
    let mut bg_idx: c_uint = cursor.image.bg_color;
    let mut fg_idx: c_uint = cursor.image.fg_color;
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_set_par(info: *mut fb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn CNVT_TOHW(val: __u32, width: __u32) -> __u32 {
...
#[no_mangle]
pub unsafe extern "C" fn fsl_diu_enable_interrupts(data: *mut fsl_diu_data) {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_open(info: *mut fb_info, user: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_release(info: *mut fb_info, user: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn install_fb(info: *mut fb_info) -> c_int {
    ...
    let mut dbsize: c_uint = ARRAY_SIZE(fsl_diu_mode_db);
    let mut has_default_mode: c_int = 1;
...
// Now build modedb from EDID
    ...
    rc = fb_find_mode(&info.var, info, aoi_mode, db, dbsize, core::ptr::null_mut(),
    default_bpp);
    if (!rc) {
//
...
//
    ...
    ARRAY_SIZE(fsl_diu_mode_db), core::ptr::null_mut(), default_bpp);
    if (rc)
    has_default_mode = 1;
    }
...
#[no_mangle]
unsafe extern "C" fn uninstall_fb(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t fsl_diu_isr(int irq, void *dev_id)
    {
    struct diu __iomem *hw = dev_id;
    let mut status: u32 = in_be32(&hw.int_status);
    if (status) {
...
#[no_mangle]
pub unsafe extern "C" fn if(INT_VSYNC: status &) -> else {
    unsigned int i;
...
//
#[no_mangle]
unsafe extern "C" fn fsl_diu_suspend(ofdev: *mut platform_device, state: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_resume(ofdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_setup(options: *mut c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_init() -> int __init {
...
//
    ...
    prop = of_get_property(np, "d-cache-size", core::ptr::null_mut());
...
//
    ...
    coherence_data_size);
    prop = of_get_property(np, "d-cache-line-size", core::ptr::null_mut());
    if (prop == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn fsl_diu_exit() -> void __exit {
