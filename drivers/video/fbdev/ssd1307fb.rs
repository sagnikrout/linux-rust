...
//! Source: drivers/video/fbdev/ssd1307fb.c
#![no_std]
...
pub const SSD1307FB_DATA: c_uint = 0x40;
pub const SSD1307FB_COMMAND: c_uint = 0x80;
pub const SSD1307FB_SET_ADDRESS_MODE: c_uint = 0x20;

pub const SSD1307FB_SET_COL_RANGE: c_uint = 0x21;
pub const SSD1307FB_SET_PAGE_RANGE: c_uint = 0x22;
pub const SSD1307FB_CONTRAST: c_uint = 0x81;
pub const SSD1307FB_SET_LOOKUP_TABLE: c_uint = 0x91;
pub const SSD1307FB_CHARGE_PUMP: c_uint = 0x8d;
pub const SSD1307FB_SEG_REMAP_ON: c_uint = 0xa1;
pub const SSD1307FB_DISPLAY_OFF: c_uint = 0xae;
pub const SSD1307FB_SET_MULTIPLEX_RATIO: c_uint = 0xa8;
pub const SSD1307FB_DISPLAY_ON: c_uint = 0xaf;
pub const SSD1307FB_START_PAGE_ADDRESS: c_uint = 0xb0;
pub const SSD1307FB_SET_DISPLAY_OFFSET: c_uint = 0xd3;
pub const SSD1307FB_SET_CLOCK_FREQ: c_uint = 0xd5;
pub const SSD1307FB_SET_AREA_COLOR_MODE: c_uint = 0xd8;
pub const SSD1307FB_SET_PRECHARGE_PERIOD: c_uint = 0xd9;
pub const SSD1307FB_SET_COM_PINS_CONFIG: c_uint = 0xda;
pub const SSD1307FB_SET_VCOMH: c_uint = 0xdb;
pub const MAX_CONTRAST: c_int = 255;
pub const REFRESHRATE: c_int = 1;
    let mut refreshrate: static u_int = REFRESHRATE;
    module_param(refreshrate, uint, 0);
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ssd1307fb_deviceinfo {
...
pub struct ssd1307fb_par {
    ...
    pub dclk_div: u32,
    pub dclk_frq: u32,
    pub device_info: *const ssd1307fb_deviceinfo,
...
#[no_mangle]
pub unsafe extern "C" fn ssd1307fb_write_cmd(client: *mut i2c_client, cmd: u8) -> c_int {
    ...
    u8 cols)
    {
    let mut col_end: u8 = col_start + cols - 1;
    int ret;
    ...
    u8 pages)
    {
    let mut page_end: u8 = page_start + pages - 1;
    int ret;
    ...
    let mut line_length: c_uint = par.info.fix.line_length;
    let mut pages: c_uint = DIV_ROUND_UP(y % 8 + height, 8);
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_update_display(par: *mut ssd1307fb_par) -> c_int {
    static int ssd1307fb_update_display(struct ssd1307fb_par *par)
    {
    return ssd1307fb_update_rect(par, 0, 0, par.width, par.height);
    }
#[no_mangle]
unsafe extern "C" fn ssd1307fb_blank(blank_mode: c_int, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_defio_damage_range(info: *mut fb_info, off: off_t, len: usize) {
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_deferred_io(info: *mut fb_info, pagereflist: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_init(par: *mut ssd1307fb_par) -> c_int {
    ...
    par.pwm = pwm_get(&par.client.dev, core::ptr::null_mut());
    if (IS_ERR(par.pwm)) {
...
// Set lookup table
    ...
    let mut val: u8 = par.lookup_table[i];
    if (val < 31 || val > 63)
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_update_bl(bdev: *mut backlight_device) -> c_int {
    static int ssd1307fb_update_bl(struct backlight_device *bdev)
    {
    struct ssd1307fb_par *par = bl_get_data(bdev);
    int ret;
    let mut brightness: c_int = bdev.props.brightness;
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_get_brightness(bdev: *mut backlight_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_probe(client: *mut i2c_client) -> c_int {
    ...
    if (ret == -ENODEV) {
    par.vbat_reg = core::ptr::null_mut();
    } else {
...
#[no_mangle]
unsafe extern "C" fn ssd1307fb_remove(client: *mut i2c_client) {
