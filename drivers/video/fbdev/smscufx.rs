...
//! Source: drivers/video/fbdev/smscufx.c
#![no_std]
...
pub struct urb_node {
    pub entry: list_head,
    pub dev: *mut ufx_data,
    pub release_urb_work: delayed_work,
    pub urb: *mut urb,
}
...
pub struct ufx_data {
    ...
    pub /: *mut *mut *mut device gdev; / &udev->dev,
    ...
    pub /: *mut *mut *mut u8 edid; / null until we read edid from hw or get from sysfs,
    pub edid_size: usize,
...
#[no_mangle]
unsafe extern "C" fn ufx_reg_read(dev: *mut ufx_data, index: u32, data: *mut u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_reg_write(dev: *mut ufx_data, index: u32, data: u32) -> c_int {
...
// buf = data;
    ...
    {
    u32 data;
    let mut status: c_int = ufx_reg_read(dev, index, &data);
...
#[no_mangle]
unsafe extern "C" fn ufx_blank(dev: *mut ufx_data, wait: bool) -> c_int {
    static int ufx_blank(struct ufx_data *dev, bool wait)
    {
    u32 dc_ctrl, dc_sts;
    int i;
    let mut status: c_int = ufx_reg_read(dev, 0x2004, &dc_sts);
...
#[no_mangle]
unsafe extern "C" fn ufx_unblank(dev: *mut ufx_data, wait: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_disable(dev: *mut ufx_data, wait: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_enable(dev: *mut ufx_data, wait: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_config_sys_clk(dev: *mut ufx_data) -> c_int {
    static int ufx_config_sys_clk(struct ufx_data *dev)
    {
    let mut status: c_int = ufx_reg_write(dev, 0x700C, 0x8000000F);
...
#[no_mangle]
unsafe extern "C" fn ufx_config_ddr2(dev: *mut ufx_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_calc_range(ref_freq: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn ufx_config_pix_clk(dev: *mut ufx_data, pixclock: u32) -> c_int {
    static int ufx_config_pix_clk(struct ufx_data *dev, u32 pixclock)
    {
    let mut asic_pll: pll_values = {0};
...
#[no_mangle]
unsafe extern "C" fn ufx_set_vid_mode(dev: *mut ufx_data, var: *mut fb_var_screeninfo) -> c_int {
    ...
    let mut status: c_int = ufx_reg_write(dev, 0x8028, 0);
...
#[no_mangle]
unsafe extern "C" fn ufx_ops_mmap(info: *mut fb_info, vma: *mut vm_area_struct) -> c_int {
    ...
    {
    let mut packed_line_len: usize = ALIGN((width * 2), 4);
    let mut packed_rect_len: usize = packed_line_len * height;
    int line;
    BUG_ON(!dev);
    BUG_ON(!dev.info);
// command word
...
// packed data
    ...
    let mut line_offset: c_int = dev.info.fix.line_length * (y + line);
    let mut byte_offset: c_int = line_offset + (x * BPP);
...
// grab the same mutex.
#[no_mangle]
unsafe extern "C" fn ufx_dpy_deferred_io(info: *mut fb_info, pagereflist: *mut list_head) {
...
// entire dirty framebuffer page
    let mut x: c_int = 0;
    let mut width: c_int = dev.info.var.xres;
    let mut y: c_int = pageref.offset / (width * 2);
    let mut height: c_int = (PAGE_SIZE / (width * 2)) + 1;
...
#[no_mangle]
unsafe extern "C" fn ufx_ops_open(info: *mut fb_info, user: c_int) -> c_int {
...
// If the USB device is gone, we don't accept new opens
    ...
    dev.fb_count++;
    kref_get(&dev.kref);
    if (fb_defio && (info.fbdefio == core::ptr::null_mut())) {
...
//
#[no_mangle]
unsafe extern "C" fn ufx_free(kref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn ufx_ops_destory(info: *mut fb_info) {
    static void ufx_ops_destory(struct fb_info *info)
    {
    struct ufx_data *dev = info.par;
    let mut node: c_int = info.node;
...
#[no_mangle]
unsafe extern "C" fn ufx_release_urb_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn ufx_free_framebuffer(dev: *mut ufx_data) {
...
//
#[no_mangle]
unsafe extern "C" fn ufx_ops_release(info: *mut fb_info, user: c_int) -> c_int {
...
// We can't free fb_info here - fbmem will touch it when we return
    ...
    kfree(info.fbdefio);
    info.fbdefio = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn ufx_var_color_format(var: *mut fb_var_screeninfo) {
    static void ufx_var_color_format(struct fb_var_screeninfo *var)
    {
    let mut red: fb_bitfield = { 11, 5, 0 };
    let mut green: fb_bitfield = { 5, 6, 0 };
    let mut blue: fb_bitfield = { 0, 5, 0 };
...
#[no_mangle]
unsafe extern "C" fn ufx_ops_set_par(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_ops_blank(blank_mode: c_int, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_ops_damage_range(info: *mut fb_info, off: off_t, len: usize) {
    static void ufx_ops_damage_range(struct fb_info *info, off_t off, size_t len)
    {
    struct ufx_data *dev = info.par;
    let mut start: c_int = max((int)(off / info.fix.line_length), 0);
    let mut lines: c_int = min((u32)((len / info.fix.line_length) + 1), (u32)info.var.yres);
    ufx_handle_damage(dev, 0, start, info.var.xres, lines);
    }
#[no_mangle]
unsafe extern "C" fn ufx_ops_damage_area(info: *mut fb_info, x: u32, y: u32, width: u32, height: u32) {
...
#[no_mangle]
unsafe extern "C" fn ufx_realloc_framebuffer(dev: *mut ufx_data, info: *mut fb_info) -> c_int {
    static int ufx_realloc_framebuffer(struct ufx_data *dev, struct fb_info *info)
    {
    let mut old_len: c_int = info.fix.smem_len;
    int new_len;
...
#[no_mangle]
unsafe extern "C" fn ufx_i2c_init(dev: *mut ufx_data) -> c_int {
    static int ufx_i2c_init(struct ufx_data *dev)
    {
    u32 tmp;
// disable the controller before it can be reprogrammed
    let mut status: c_int = ufx_reg_write(dev, 0x106C, 0x00);
...
#[no_mangle]
unsafe extern "C" fn ufx_i2c_configure(dev: *mut ufx_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_i2c_wait_busy(dev: *mut ufx_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufx_read_edid(dev: *mut ufx_data, edid: *mut u8, edid_len: c_int) -> c_int {
...
// Read the 128-byte EDID as 2 bursts of 64 bytes
    for (i = 0; i < 2; i++) {
    let mut temp: u32 = 0x28070000 | (63 << 20) | (((u32)(i * 64)) << 8);
    ...
    for (j = 0; j < 16; j++) {
    let mut data_reg_addr: u32 = 0x1110 + (j * 4);
...
// Returns 0 if successful
    ...
    {
    const struct fb_videomode *default_vmode = core::ptr::null_mut();
    u8 *edid;
...
// If everything else has failed, fall back to safe default mode
    if (default_vmode == core::ptr::null_mut()) {
    let mut fb_vmode: fb_videomode = {0};
...
// If we have good mode and no active clients
    if ((default_vmode != core::ptr::null_mut()) && (dev.fb_count == 0)) {
...
// allocates framebuffer driver structure, not framebuffer memory
    ...
    retval = ufx_setup_modes(dev, info, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn ufx_usb_disconnect(interface: *mut usb_interface) {
...
#[no_mangle]
unsafe extern "C" fn ufx_urb_completion(urb: *mut urb) {
    static void ufx_urb_completion(struct urb *urb)
    {
...
#[no_mangle]
unsafe extern "C" fn ufx_free_urb_list(dev: *mut ufx_data) {
    static void ufx_free_urb_list(struct ufx_data *dev)
    {
    let mut count: c_int = dev.urbs.count;
...
#[no_mangle]
unsafe extern "C" fn ufx_alloc_urb_list(dev: *mut ufx_data, count: c_int, size: usize) -> c_int {
    static int ufx_alloc_urb_list(struct ufx_data *dev, int count, size_t size)
    {
    let mut i: c_int = 0;
    struct urb *urb;
...
#[no_mangle]
unsafe extern "C" fn ufx_submit_urb(dev: *mut ufx_data, urb: *mut urb, len: usize) -> c_int {
    static int ufx_submit_urb(struct ufx_data *dev, struct urb *urb, size_t len)
    {
    int ret;
