...
//! Source: drivers/video/fbdev/nvidia/nvidia.c
#![no_std]
...
// command line data, set in nvidiafb_setup()
    ...
    let mut fpdither: static int = -1;
    let mut forceCRTC: static int = -1;
    let mut hwcur: static int = 0;
    let mut noaccel: static int = 0;
    let mut noscale: static int = 0;
    let mut paneltweak: static int = 0;
    let mut vram: static int = 0;
    let mut bpp: static int = 8;
    static int reverse_i2c;
    let mut nomtrr: static bool = false;
...
#[no_mangle]
unsafe extern "C" fn nvidia_screen_off(par: *mut nvidia_par, on: c_int) {
...
#[no_mangle]
unsafe extern "C" fn nvidia_calc_regs(info: *mut fb_info) -> c_int {
    static int nvidia_calc_regs(struct fb_info *info)
    {
    struct nvidia_par *par = info.par;
    struct _riva_hw_state *state = &par.ModeReg;
    int i, depth = fb_get_color_depth(&info.var, &info.fix);
    let mut h_display: c_int = info.var.xres / 8 - 1;
    let mut h_start: c_int = (info.var.xres + info.var.right_margin) / 8 - 1;
    int h_end = (info.var.xres + info.var.right_margin +
    info.var.hsync_len) / 8 - 1;
    int h_total = (info.var.xres + info.var.right_margin +
    info.var.hsync_len + info.var.left_margin) / 8 - 5;
    let mut h_blank_s: c_int = h_display;
    let mut h_blank_e: c_int = h_total + 4;
    let mut v_display: c_int = info.var.yres - 1;
    let mut v_start: c_int = info.var.yres + info.var.lower_margin - 1;
    int v_end = (info.var.yres + info.var.lower_margin +
    info.var.vsync_len) - 1;
    int v_total = (info.var.yres + info.var.lower_margin +
    info.var.vsync_len + info.var.upper_margin) - 2;
    let mut v_blank_s: c_int = v_display;
    let mut v_blank_e: c_int = v_total + 1;
//
// Set all CRTC values.
//
...
#[no_mangle]
unsafe extern "C" fn nvidia_init_vga(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_cursor(info: *mut fb_info, cursor: *mut fb_cursor) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_set_par(info: *mut fb_info) -> c_int {
...
// turn on LFB swapping
    ...
    nvidia_fb_ops.fb_sync = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_blank(blank: c_int, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn save_vga_x86(par: *mut nvidia_par) {
...
#[no_mangle]
unsafe extern "C" fn restore_vga_x86(par: *mut nvidia_par) {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_open(info: *mut fb_info, user: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_release(info: *mut fb_info, user: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_suspend_late(dev: *mut device, mesg: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvidia_set_fbinfo(info: *mut fb_info) -> c_int {
...
// maximize virtual vertical length
    ...
    if (!hwcur)
    nvidia_fb_ops.fb_cursor = core::ptr::null_mut();
    ...
    {
    let mut id: u32 = (pci_dev.vendor << 16) | pci_dev.device;
...
#[no_mangle]
unsafe extern "C" fn nvidia_get_arch(Chipset: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_probe(pd: *mut pci_dev, ent: *const pci_device_id) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_remove(pd: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_setup(options: *mut c_char) -> c_int {
    ...
    paneltweak = simple_strtoul(this_opt+11, core::ptr::null_mut(), 0);
    ...
    fpdither = simple_strtol(this_opt+9, core::ptr::null_mut(), 0);
    } else if (!strncmp(this_opt, "bpp:", 4)) {
    bpp = simple_strtoul(this_opt+4, core::ptr::null_mut(), 0);
    } else
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn nvidiafb_init() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvidiafb_exit() -> void __exit {
