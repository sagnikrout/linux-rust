...
//! Source: drivers/video/fbdev/nvidia/nv_hw.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn NVLockUnlock(par: *mut nvidia_par, Lock: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn NVShowHideCursor(par: *mut nvidia_par, ShowHide: c_int) -> c_int {
    int NVShowHideCursor(struct nvidia_par *par, int ShowHide)
    {
    let mut cur: c_int = par.CurrentState.cursor1;
...
#[no_mangle]
unsafe extern "C" fn nv4CalcArbitration(fifo: *mut *mut nv4_fifo_info, arb: *mut *mut nv4_sim_state) {
    ...
    if (fifo_data.valid) {
    let mut b: c_int = fifo_data.graphics_burst_size >> 4;
// burst = 0;
    while (b >>= 1)
    (*burst)++;
...
#[no_mangle]
unsafe extern "C" fn nv10CalcArbitration(fifo: *mut *mut nv10_fifo_info, arb: *mut *mut nv10_sim_state) {
...
// lwm = graphics_lwm >> 3;
    ...
    struct pci_dev *dev;
    let mut domain: c_int = pci_domain_nr(par.pci_dev.bus);
...
#[no_mangle]
pub unsafe extern "C" fn NVLoadStateExt(par: *mut nvidia_par, state: *mut *mut RIVA_HW_STATE) {
    ...
    if (!state) {
    par.CurrentState = core::ptr::null_mut();
    return;
    }
    if (par.Architecture >= NV_ARCH_10) {
    if (par.twoHeads) {
...
#[no_mangle]
pub unsafe extern "C" fn NVUnloadStateExt(par: *mut nvidia_par, state: *mut *mut RIVA_HW_STATE) {
    VGA_WR08(par.PCIO, 0x03D4, 0x19);
...
#[no_mangle]
pub unsafe extern "C" fn NVSetStartAddress(par: *mut nvidia_par, start: u32) {
