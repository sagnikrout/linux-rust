...
//! Source: drivers/video/fbdev/offb.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct offb_par {
    pub cmap_adr: *mut volatile void __iomem,
    pub cmap_data: *mut volatile void __iomem,
    pub cmap_type: c_int,
    pub blanked: c_int,
...
//
    ...
    let mut cr: u32 = red >> (16 - info.var.red.length);
    let mut cg: u32 = green >> (16 - info.var.green.length);
    let mut cb: u32 = blue >> (16 - info.var.blue.length);
    u32 value;
    if (regno >= 16)
    return -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn offb_blank(blank: c_int, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn offb_set_par(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn offb_destroy(info: *mut fb_info) {
    ...
    let mut base: c_ulong = address & 0xff000000UL;
    par.cmap_adr =
...
// Look for AVIVO initialized by SLOF
    ...
    const u32 *vid, *did;
    vid = of_get_property(pciparent, "vendor-id", core::ptr::null_mut());
    did = of_get_property(pciparent, "device-id", core::ptr::null_mut());
// This will match most R5xx
    ...
    let mut io_addr: u64 = of_translate_address(dp, io_of_addr);
    ...
    {
    let mut res_size: c_ulong = pitch * height;
    let mut res_start: c_ulong = address;
    ...
    par.cmap_adr = core::ptr::null_mut();
    framebuffer_release(info);
    release_mem_region(res_start, res_size);
    }
    ...
    let mut max_size: c_ulong = 0;
    u64 rstart, address = OF_BAD_ADDR;
    const __be32 *pp, *addrp, *up;
    u64 asize;
    let mut foreign_endian: c_int = 0;
...
// Hack for when BootX is passing us
    ...
    u32 vid, did;
    struct pci_dev *pdev;
    vidp = of_get_property(dp, "vendor-id", core::ptr::null_mut());
    didp = of_get_property(dp, "device-id", core::ptr::null_mut());
    if (vidp && didp) {
    vid = be32_to_cpup(vidp);
    did = be32_to_cpup(didp);
    pdev = pci_get_device(vid, did, core::ptr::null_mut());
    if (!pdev)
    return;
...
// kludge for valkyrie
    ...
    address += 0x1000;
    offb_init_fb(parent, no_real_node ? "bootx" : core::ptr::null_mut(),
    width, height, depth, pitch, address,
    foreign_endian, no_real_node ? core::ptr::null_mut() : dp);
    }
    }
#[no_mangle]
unsafe extern "C" fn offb_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn offb_probe_bootx_noscreen(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn offb_probe_display(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn offb_init() -> int __init {
    static int __init offb_init(void)
    {
    if (fb_get_options("offb", core::ptr::null_mut()))
    return -ENODEV;
...
#[no_mangle]
unsafe extern "C" fn offb_exit() -> void __exit {
