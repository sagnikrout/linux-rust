...
//! Source: drivers/video/fbdev/simplefb.c
#![no_std]
...
pub const PSEUDO_PALETTE_SIZE: c_int = 16;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct simplefb_par {
    pub palette: [u32; PSEUDO_PALETTE_SIZE],
    pub base: resource_size_t,
    pub size: resource_size_t,
    pub mem: *mut resource,

    pub clks_enabled: bool,
    pub clk_count: c_uint,
    pub clks: *mut clk,

    pub num_genpds: c_uint,
    pub genpds: *mut device,
    pub genpd_links: *mut device_link,

    pub regulators_enabled: bool,
    pub regulator_count: u32,
...
//
#[no_mangle]
unsafe extern "C" fn simplefb_destroy(info: *mut fb_info) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct simplefb_params {
    pub width: u32,
    pub height: u32,
    pub stride: u32,
    pub format: *mut simplefb_format,
    pub memory: resource,
}
    ...
    params.format = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn simplefb_clocks_destroy(par: *mut simplefb_par) {
    static void simplefb_clocks_destroy(struct simplefb_par *par)
    {
    int i;
    if (!par.clks)
...
// Enable all the regulators
    ...
    par.regulators[i] = core::ptr::null_mut();
    }
    }
    par.regulators_enabled = true;
    }
#[no_mangle]
unsafe extern "C" fn simplefb_regulators_destroy(par: *mut simplefb_par) {
    static void simplefb_regulators_destroy(struct simplefb_par *par)
    {
    int i;
...
#[no_mangle]
unsafe extern "C" fn simplefb_detach_genpds(res: *mut c_void) {
    static void simplefb_detach_genpds(void *res)
    {
    struct simplefb_par *par = res;
    let mut i: c_uint = par.num_genpds;
    if (par.num_genpds <= 1)
    return;
    while (i--) {
...
#[no_mangle]
unsafe extern "C" fn simplefb_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (fb_get_options("simplefb", core::ptr::null_mut()))
    return -ENODEV;
    ret = -ENODEV;
    if (dev_get_platdata(&pdev.dev))
    ret = simplefb_parse_pd(pdev, &params);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: pdev->dev.of_node) -> else {
...
#[no_mangle]
unsafe extern "C" fn simplefb_remove(pdev: *mut platform_device) {
