...
//! Source: drivers/video/fbdev/atafb_iplan2p4.c
#![no_std]
...
//
    ...
    int w, l , i, j;
    u_int colsize;
    let mut upwards: u_int = (dy < sy) || (dy == sy && dx < sx);
    colsize = height;
    if (!((sx ^ dx) & 15)) {
...
// --dst32 = pval[1] | (v1 << 8);
    ...
    if (rows) {
    u32 *d = dest;
    let mut off: u32 = next_line - rows * BPL * 2;
    for (i = height; i; i--) {
