...
//! Source: drivers/video/fbdev/s1d13xxxfb.c
#![no_std]
...
// make sure the hardware can cope with us
    ...
    {
    let mut mode: u8 = s1d13xxxfb_readreg(par, S1DREG_COM_DISP_MODE);
    if (enable)
    mode |= 0x01;
    else
    mode &= ~0x01;
...
//
    ...
    u32 dst, src;
    u32 stride;
    let mut reverse: u16 = 0;
    let mut sx: u16 = area.sx, sy = area.sy;
    let mut dx: u16 = area.dx, dy = area.dy;
    let mut width: u16 = area.width, height = area.height;
    u16 bpp;
...
//
    ...
    u32 fg;
    let mut bpp: u16 = (info.var.bits_per_pixel >> 3);
// grab spinlock
    spin_lock(&s1d13xxxfb_bitblt_lock);
// bytes per x width
...
//
#[no_mangle]
unsafe extern "C" fn s1d13xxxfb_fetch_hw_state(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn __s1d13xxxfb_remove(pdev: *mut platform_device) {
    static void __s1d13xxxfb_remove(struct platform_device *pdev)
    {
    struct fb_info *info = platform_get_drvdata(pdev);
    struct s1d13xxxfb_par *par = core::ptr::null_mut();
    if (info) {
    par = info.par;
    if (par && par.regs) {
...
#[no_mangle]
unsafe extern "C" fn s1d13xxxfb_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn s1d13xxxfb_probe(pdev: *mut platform_device) -> c_int {
    static int s1d13xxxfb_probe(struct platform_device *pdev)
    {
    struct s1d13xxxfb_par *default_par;
    struct fb_info *info;
    struct s1d13xxxfb_pdata *pdata = core::ptr::null_mut();
    let mut ret: c_int = 0;
    int i;
    u8 revision, prod_id;
...
#[no_mangle]
unsafe extern "C" fn s1d13xxxfb_suspend(dev: *mut platform_device, state: pm_message_t) -> c_int {
...
// disable display
    ...
    s1dfb.disp_save = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn s1d13xxxfb_resume(dev: *mut platform_device) -> c_int {
...
// will write RO regs, *should* get away with it :)
    ...
    if (fb_get_options("s1d13xxxfb", core::ptr::null_mut()))
    return -ENODEV;
