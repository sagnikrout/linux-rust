...
//! Source: drivers/video/fbdev/omap2/omapfb/omapfb-main.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn draw_pixel(fbi: *mut fb_info, x: c_int, y: c_int, color: unsigned) {
    static void draw_pixel(struct fb_info *fbi, int x, int y, unsigned color)
    {
    struct fb_var_screeninfo *var = &fbi.var;
    struct fb_fix_screeninfo *fix = &fbi.fix;
    void __iomem *addr = fbi.screen_base;
    let mut bytespp: unsigned = var.bits_per_pixel >> 3;
    let mut line_len: unsigned = fix.line_length / bytespp;
    let mut r: c_int = (color >> 16) & 0xff;
    let mut g: c_int = (color >> 8) & 0xff;
    let mut b: c_int = (color >> 0) & 0xff;
...
#[no_mangle]
unsafe extern "C" fn fill_fb(fbi: *mut fb_info) {
    static void fill_fb(struct fb_info *fbi)
    {
    struct fb_var_screeninfo *var = &fbi.var;
    let mut w: c_short = var.xres_virtual;
    let mut h: c_short = var.yres_virtual;
    void __iomem *addr = fbi.screen_base;
    int y, x;
    if (!addr)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn if(20): x < 20 && (y > 20 && y < h -) -> else {
    else if (x < 20 && (y > 20 && y < h - 20))
    draw_pixel(fbi, x, y, 0xff);
#[no_mangle]
pub unsafe extern "C" fn if(20): y < 20 && (x > 20 && x < w -) -> else {
    else if (y < 20 && (x > 20 && x < w - 20))
    draw_pixel(fbi, x, y, 0xff00);
#[no_mangle]
pub unsafe extern "C" fn if(20): x > w - 20 && (y > 20 && y < h -) -> else {
    else if (x > w - 20 && (y > 20 && y < h - 20))
    draw_pixel(fbi, x, y, 0xff0000);
#[no_mangle]
pub unsafe extern "C" fn if(20): y > h - 20 && (x > 20 && x < w -) -> else {
    ...
    draw_pixel(fbi, x, y, 0xffffff);
#[no_mangle]
pub unsafe extern "C" fn if(y: x == y || w - x == h -) -> else {
    else if (x == y || w - x == h - y)
    draw_pixel(fbi, x, y, 0xff00ff);
#[no_mangle]
pub unsafe extern "C" fn if(y: w - x == y || x == h -) -> else {
    else if (w - x == y || x == h - y)
    draw_pixel(fbi, x, y, 0x00ffff);
#[no_mangle]
pub unsafe extern "C" fn if(20: x > 20 && y > 20 && x < w - 20 && y < h -) -> else {
    let mut t: c_int = x * 3 / w;
    let mut r: unsigned = 0, g = 0, b = 0;
    unsigned c;
    if (var.bits_per_pixel == 16) {
    if (t == 0)
    b = (y % 32) * 256 / 32;
#[no_mangle]
pub unsafe extern "C" fn if(1: t ==) -> else {
    else if (t == 1)
    g = (y % 64) * 256 / 64;
#[no_mangle]
pub unsafe extern "C" fn if(2: t ==) -> else {
    else if (t == 2)
    r = (y % 32) * 256 / 32;
    } else {
    if (t == 0)
...
#[no_mangle]
unsafe extern "C" fn omapfb_get_vrfb_offset(ofbi: *const omapfb_info, rot: c_int) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn omapfb_get_region_rot_paddr(ofbi: *const omapfb_info, rot: c_int) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn omapfb_get_region_paddr(ofbi: *const omapfb_info) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn cmp_component(f1: *mut fb_bitfield, f2: *mut fb_bitfield) -> bool {
...
#[no_mangle]
unsafe extern "C" fn check_fb_res_bounds(var: *mut fb_var_screeninfo) -> c_int {
    static int check_fb_res_bounds(struct fb_var_screeninfo *var)
    {
    let mut xres_min: c_int = OMAPFB_PLANE_XRES_MIN;
    let mut xres_max: c_int = 2048;
    let mut yres_min: c_int = OMAPFB_PLANE_YRES_MIN;
    let mut yres_max: c_int = 2048;
// XXX: some applications seem to set virtual res to 0.
    ...
    struct fb_var_screeninfo *var)
    {
    let mut max_frame_size: c_ulong = ofbi.region.size;
    let mut bytespp: c_int = var.bits_per_pixel >> 3;
    let mut line_size: c_ulong = var.xres_virtual * bytespp;
    if (ofbi.rotation_type == OMAP_DSS_ROT_VRFB) {
...
//
#[no_mangle]
unsafe extern "C" fn setup_vrfb_rotation(fbi: *mut fb_info) -> c_int {
...
// XXX we shouldn't allow this when framebuffer is mmapped
    ...
    reconf = true;
#[no_mangle]
pub unsafe extern "C" fn if(vrfb->bytespp: bytespp !=) -> else {
    ...
    fbi.screen_base = core::ptr::null_mut();
    fix.smem_start = 0;
    fix.smem_len = 0;
    iounmap(vrfb.vaddr[0]);
    vrfb.vaddr[0] = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn set_fb_fix(fbi: *mut fb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn check_fb_var(fbi: *mut fb_info, var: *mut fb_var_screeninfo) -> c_int {
    int check_fb_var(struct fb_info *fbi, struct fb_var_screeninfo *var)
    {
    struct omapfb_info *ofbi = FB2OFB(fbi);
    struct omap_dss_device *display = fb2display(fbi);
    let mut mode: enum omap_color_mode = 0;
    int i;
    int r;
    DBG("check_fb_var %d\n", ofbi.id);
...
//
#[no_mangle]
unsafe extern "C" fn omapfb_open(fbi: *mut fb_info, user: c_int) -> c_int {
    static int omapfb_open(struct fb_info *fbi, int user)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn omapfb_release(fbi: *mut fb_info, user: c_int) -> c_int {
    ...
    fix.line_length;
#[no_mangle]
pub unsafe extern "C" fn if(FB_ROTATE_CW: rotation ==) -> else {
    ...
    var.xoffset * (var.bits_per_pixel >> 3);
#[no_mangle]
pub unsafe extern "C" fn if(FB_ROTATE_UD: rotation ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(FB_ROTATE_CCW: rotation ==) -> else {
...
// setup overlay according to the fb
    ...
    struct fb_fix_screeninfo *fix = &fbi.fix;
    let mut mode: enum omap_color_mode = 0;
    let mut data_start_p: u32 = 0;
    struct omap_overlay_info info;
    int xres, yres;
    int screen_width;
    int mirror;
    let mut rotation: c_int = var.rotate;
    int i;
    WARN_ON(!atomic_read(&ofbi.region.lock_count));
    for (i = 0; i < ofbi.num_overlays; i++) {
...
#[no_mangle]
pub unsafe extern "C" fn omapfb_apply_changes(fbi: *mut fb_info, init: c_int) -> c_int {
...
// the fb is not available. disable the overlay
    ...
    let mut rotation: c_int = (var.rotate + ofbi.rotation[i]) % 4;
...
#[no_mangle]
unsafe extern "C" fn mmap_user_open(vma: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn mmap_user_close(vma: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn omapfb_mmap(fbi: *mut fb_info, vma: *mut vm_area_struct) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn omapfb_free_fbmem(fbi: *mut fb_info) {
...
// unmap the 0 angle rotation
    ...
    rg.vrfb.vaddr[0] = core::ptr::null_mut();
    }
    omap_vrfb_release_ctx(&rg.vrfb);
    }
    dma_free_attrs(fbdev.dev, rg.size, rg.token, rg.dma_handle,
    rg.attrs);
    rg.token = core::ptr::null_mut();
    rg.vaddr = core::ptr::null_mut();
    rg.paddr = 0;
    rg.alloc = 0;
    rg.size = 0;
    }
#[no_mangle]
unsafe extern "C" fn clear_fb_info(fbi: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn omapfb_free_all_fbmem(fbdev: *mut omapfb2_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn omapfb_allocate_all_fbs(fbdev: *mut omapfb2_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn omapfb_clear_fb(fbi: *mut fb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn omapfb_realloc_fbmem(fbi: *mut fb_info, size: c_ulong, type: c_int) -> c_int {
    ...
    let mut old_paddr: c_ulong = rg.paddr;
    let mut old_type: c_int = rg.type;
    int r;
...
#[no_mangle]
unsafe extern "C" fn omapfb_auto_update_work(work: *mut work_struct) {
    ...
    {
    struct omapfb_display_data *d;
    if (fbdev.auto_update_wq == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn omapfb_fb_init(fbdev: *mut omapfb2_device, fbi: *mut fb_info) -> c_int {
    static int omapfb_fb_init(struct omapfb2_device *fbdev, struct fb_info *fbi)
    {
    struct fb_var_screeninfo *var = &fbi.var;
    struct omap_dss_device *display = fb2display(fbi);
    ...
    if (display) {
    u16 w, h;
    let mut rotation: c_int = (var.rotate + ofbi.rotation[0]) % 4;
    display.driver.get_resolution(display, &w, &h);
    if (rotation == FB_ROTATE_CW ||
    rotation == FB_ROTATE_CCW) {
    var.xres = h;
...
#[no_mangle]
unsafe extern "C" fn fbinfo_cleanup(fbdev: *mut omapfb2_device, fbi: *mut fb_info) {
    static void fbinfo_cleanup(struct omapfb2_device *fbdev, struct fb_info *fbi)
    {
    fb_dealloc_cmap(&fbi.cmap);
    }
#[no_mangle]
unsafe extern "C" fn omapfb_free_resources(fbdev: *mut omapfb2_device) {
...
// free the reserved fbmem
    ...
    fbdev.auto_update_wq = core::ptr::null_mut();
    }
    dev_set_drvdata(fbdev.dev, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn omapfb_create_framebuffers(fbdev: *mut omapfb2_device) -> c_int {
...
// bpp = 0;
    fbi = core::ptr::null_mut();
    var = core::ptr::null_mut();
    fbops = core::ptr::null_mut();
    fbi = kzalloc_obj(*fbi);
    ...
    var = kzalloc_obj(*var);
    if (var == core::ptr::null_mut()) {
    r = -ENOMEM;
    goto err;
    }
    fbops = kzalloc_obj(*fbops);
    if (fbops == core::ptr::null_mut()) {
    r = -ENOMEM;
    goto err;
    }
    fbi.fbops = fbops;
    r = fb_find_mode(var, fbi, mode_str, core::ptr::null_mut(), 0, core::ptr::null_mut(), 24);
    if (r == 0) {
    r = -EINVAL;
    goto err;
    }
...
// If check_timings is not present compare xres and yres
    ...
    {
    struct omapfb_display_data *d;
    BUG_ON(dssdev.driver.get_recommended_bpp == core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn omapfb_parse_def_modes(fbdev: *mut omapfb2_device) -> c_int {
...
// then look for DT alias display0
    ...
    int id;
    dssdev = fbdev.displays[i].dssdev;
    if (dssdev.dev.of_node == core::ptr::null_mut())
    continue;
...
unsafe extern "C" fn omapfb_probe(pdev: *mut platform_device) -> c_int {
    ...
    {
    struct omapfb2_device *fbdev = core::ptr::null_mut();
    let mut r: c_int = 0;
    int i;
    ...
    if (def_display == core::ptr::null_mut()) {
