...
//! Source: drivers/video/fbdev/omap2/omapfb/dss/manager-sysfs.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn manager_name_show(mgr: *mut omap_overlay_manager, buf: *mut c_char) -> isize {
    static ssize_t manager_name_show(struct omap_overlay_manager *mgr, char *buf)
    {
    return sysfs_emit(buf, "%s\n", mgr.name);
    }
#[no_mangle]
unsafe extern "C" fn manager_display_show(mgr: *mut omap_overlay_manager, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn manager_display_match(dssdev: *mut omap_dss_device, data: *mut c_void) -> c_int {
    ...
    if (len > 0 && dssdev == core::ptr::null_mut())
    return -EINVAL;
    if (dssdev) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct manager_attribute {
    pub attr: attribute,
    pub ): *mut *mut *mut ssize_t (show)(struct omap_overlay_manager , char,
    pub size_t): *const *const *const *const ssize_t (store)(struct omap_overlay_manager , char ,,
}

    struct manager_attribute manager_attr_##_name = \
    __ATTR(_name, _mode, _show, _store)
    static MANAGER_ATTR(name, S_IRUGO, manager_name_show, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn dss_manager_kobj_uninit(mgr: *mut omap_overlay_manager) {
