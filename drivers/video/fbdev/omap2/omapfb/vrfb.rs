...
//! Source: drivers/video/fbdev/omap2/omapfb/vrfb.c
#![no_std]
...
pub const SMS_IMAGEHEIGHT_OFFSET: c_int = 16;
pub const SMS_IMAGEWIDTH_OFFSET: c_int = 0;
pub const SMS_PH_OFFSET: c_int = 8;
pub const SMS_PW_OFFSET: c_int = 4;
pub const SMS_PS_OFFSET: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn omap2_sms_write_rot_control(val: u32, ctx: unsigned) {
    static void omap2_sms_write_rot_control(u32 val, unsigned ctx)
    {
    __raw_writel(val, vrfb_base + SMS_ROT_CONTROL(ctx));
    }
#[no_mangle]
unsafe extern "C" fn omap2_sms_write_rot_size(val: u32, ctx: unsigned) {
    static void omap2_sms_write_rot_size(u32 val, unsigned ctx)
    {
    __raw_writel(val, vrfb_base + SMS_ROT_SIZE(ctx));
    }
#[no_mangle]
unsafe extern "C" fn omap2_sms_write_rot_physical_ba(val: u32, ctx: unsigned) {
    static void omap2_sms_write_rot_physical_ba(u32 val, unsigned ctx)
    {
    __raw_writel(val, vrfb_base + SMS_ROT_PHYSICAL_BA(ctx));
    }
#[no_mangle]
pub unsafe extern "C" fn restore_hw_context(ctx: c_int) {
...
#[no_mangle]
unsafe extern "C" fn get_image_width_roundup(width: u16, bytespp: u8) -> u32 {
    static u32 get_image_width_roundup(u16 width, u8 bytespp)
    {
    let mut stride: c_ulong = width * bytespp;
...
//
#[no_mangle]
pub unsafe extern "C" fn get_extra_physical_size(image_width_roundup: u16, bytespp: u8) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn omap_vrfb_restore_context() {
    void omap_vrfb_restore_context(void)
    {
    int i;
    let mut map: c_ulong = ctx_map;
...
#[no_mangle]
pub unsafe extern "C" fn omap_vrfb_min_phys_size(width: u16, height: u16, bytespp: u8) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn omap_vrfb_max_height(phys_size: u32, width: u16, bytespp: u8) -> u16 {
...
// differently. See TRM.
    ...
    pixel_size_exp = 2;
#[no_mangle]
pub unsafe extern "C" fn if(2: bytespp ==) -> else {
    else if (bytespp == 2)
    pixel_size_exp = 1;
    else {
    BUG();
...
#[no_mangle]
pub unsafe extern "C" fn omap_vrfb_map_angle(vrfb: *mut vrfb, height: u16, rot: u8) -> c_int {
    int omap_vrfb_map_angle(struct vrfb *vrfb, u16 height, u8 rot)
    {
    let mut size: c_ulong = height * OMAP_VRFB_LINE_LEN * vrfb.bytespp;
...
#[no_mangle]
pub unsafe extern "C" fn omap_vrfb_release_ctx(vrfb: *mut vrfb) {
    void omap_vrfb_release_ctx(struct vrfb *vrfb)
    {
    int rot;
    let mut ctx: c_int = vrfb.context;
    if (ctx == 0xff)
    return;
    DBG("release ctx %d\n", ctx);
    mutex_lock(&ctx_lock);
...
#[no_mangle]
pub unsafe extern "C" fn omap_vrfb_request_ctx(vrfb: *mut vrfb) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn omap_vrfb_supported() -> bool {
...
#[no_mangle]
unsafe extern "C" fn vrfb_probe(pdev: *mut platform_device) -> int __init {
