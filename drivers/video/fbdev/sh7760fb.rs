...
//! Source: drivers/video/fbdev/sh7760fb.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sh7760fb_par {
    pub base: *mut void __iomem,
    pub irq: c_int,
    pub /: *mut *mut *mut sh7760fb_platdata pd; / display information,
    pub /: *mut *mut dma_addr_t fbdma; / physical address,
    pub /: *mut *mut int rot; / rotation enabled?,
    pub pseudo_palette: [u32; 16],
    pub dev: *mut platform_device,
    pub ioarea: *mut resource,
    pub /: *mut *mut completion vsync; / vsync irq event,
}

#[no_mangle]
unsafe extern "C" fn sh7760fb_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn wait_for_lps(par: *mut sh7760fb_par, val: c_int) -> c_int {
...
// en/disable the LCDC
#[no_mangle]
unsafe extern "C" fn sh7760fb_blank(blank: c_int, info: *mut fb_info) -> c_int {
    static int sh7760fb_blank(int blank, struct fb_info *info)
    {
    struct sh7760fb_par *par = info.par;
    struct sh7760fb_platdata *pd = par.pd;
    let mut cntr: c_ushort = ioread16(par.base + LDCNTR);
    let mut intr: c_ushort = ioread16(par.base + LDINTR);
    int lps;
...
// get color info from register value
    ret = sh7760fb_get_color_info(info, par.pd.lddfr, &bpp, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(15: var->bits_per_pixel >=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn sh7760fb_set_par(info: *mut fb_info) -> c_int {
...
// AC modulation sig
    ...
    stride >>= 3;
#[no_mangle]
pub unsafe extern "C" fn if(2: bpp ==) -> else {
    else if (bpp == 2)
    stride >>= 2;
#[no_mangle]
pub unsafe extern "C" fn if(4: bpp ==) -> else {
    else if (bpp == 4)
    stride >>= 1;
// 6 bpp == 8 bpp
    }
...
#[no_mangle]
unsafe extern "C" fn sh7760fb_free_mem(info: *mut fb_info) {
...
//
#[no_mangle]
unsafe extern "C" fn sh7760fb_alloc_mem(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sh7760fb_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sh7760fb_remove(dev: *mut platform_device) {
