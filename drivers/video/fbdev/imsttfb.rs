...
//! Source: drivers/video/fbdev/imsttfb.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct imstt_regvals {
    pub pitch: __u32,
    pub vil: __u16 hes, heb, hsb, ht, ves, veb, vsb, vt,,
    pub pclk_p: __u8 pclk_m, pclk_n,,
// Values of the tvp which change depending on colormode x resolution
    pub /: *mut *mut __u8 mlc[3]; / Memory Loop Config 0x39,
    pub /: *mut *mut __u8 lckl_p[3]; / P value of LCKL PLL,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct imstt_par {
    pub init: imstt_regvals,
    pub dc_regs: *mut __u32 __iomem,
    pub cmap_regs_phys: c_ulong,
    pub cmap_regs: *mut __u8,
    pub ramdac: __u32,
    pub palette: [__u32; 16],
}
...
pub const INIT_BPP: c_int = 8;
pub const INIT_XRES: c_int = 640;
pub const INIT_YRES: c_int = 480;
    let mut inverse: static int = 0;
    static char fontname[40] __initdata = { 0 };

    let mut init_vmode: static signed char = -1, init_cmode = -1;
...
//
#[no_mangle]
pub unsafe extern "C" fn read_reg_le32(base: *mut volatile u32 __iomem, regindex: c_int) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn write_reg_le32(base: *mut volatile u32 __iomem, regindex: c_int, val: u32) {
...
// case 0x400000:
    ...
    let mut vram: __u32 = (info.fix.smem_len - (PAGE_SIZE << 2));
    ...
    {
    struct imstt_par *par = info.par;
    let mut bpp: u_int = info.var.bits_per_pixel;
    if (regno > 255)
    return 1;
    red >>= 8;
    green >>= 8;
...
// PADDRW/PDATA are the same as TVPPADDRW/TVPPDATA
    ...
    let mut x: __u16 = d.dx + 0x40, y = d.dy + 0x40;
    ...
    {
    struct imstt_par *par = info.par;
    let mut flags: u32 = cursor.set, fg, bg, xx, yy;
    if (cursor.dest == core::ptr::null_mut() && cursor.rop == ROP_XOR)
    return 1;
    ...
    let mut fg_idx: c_int = cursor.image.fg_color;
    let mut width: c_int = (cursor.image.width+7)/8;
...
pub const FBIMSTT_SETREG: c_uint = 0x545401;
pub const FBIMSTT_GETREG: c_uint = 0x545402;
pub const FBIMSTT_SETCMAPREG: c_uint = 0x545403;
pub const FBIMSTT_GETCMAPREG: c_uint = 0x545404;
pub const FBIMSTT_SETIDXREG: c_uint = 0x545405;
pub const FBIMSTT_GETIDXREG: c_uint = 0x545406;
    static int
...
#[no_mangle]
unsafe extern "C" fn init_imstt(info: *mut fb_info) -> c_int {
...
// set default values for DAC registers
    ...
    let mut vmode: c_int = init_vmode, cmode = init_cmode;
    if (vmode == -1) {
...
#[no_mangle]
unsafe extern "C" fn imsttfb_probe(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    static int imsttfb_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
    {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ENABLED(CONFIG_OF)) -> else {
...
#[no_mangle]
unsafe extern "C" fn imsttfb_remove(pdev: *mut pci_dev) {
    static void imsttfb_remove(struct pci_dev *pdev)
    {
    struct fb_info *info = pci_get_drvdata(pdev);
    struct imstt_par *par = info.par;
    let mut size: c_int = pci_resource_len(pdev, 0);
...
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "vmode:", _arg: 6)) -> else {
    let mut vmode: c_int = simple_strtoul(this_opt+6, core::ptr::null_mut(), 0);
    if (vmode > 0 && vmode <= VMODE_MAX)
    init_vmode = vmode;
    } else if (!strncmp(this_opt, "cmode:", 6)) {
    let mut cmode: c_int = simple_strtoul(this_opt+6, core::ptr::null_mut(), 0);
    switch (cmode) {
    case CMODE_8:
    case 8:
    init_cmode = CMODE_8;
...
#[no_mangle]
unsafe extern "C" fn imsttfb_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn imsttfb_exit() -> void __exit {
