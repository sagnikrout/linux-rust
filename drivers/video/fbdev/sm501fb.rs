...
//! Source: drivers/video/fbdev/sm501fb.c
#![no_std]
...
//
    ...
    let mut default_bpp: static unsigned long = 16;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sm501_mem {
    pub size: c_ulong,
    pub /: *mut *mut unsigned long sm_addr; / offset from base of sm501 fb.,
    pub k_addr: *mut void __iomem,
}

// private data that is shared between all frambuffers*
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sm501fb_info {
    pub dev: *mut device,
    pub /: *mut *mut *mut fb_info fb[2]; / fb info for both heads,
    pub /: *mut *mut *mut resource fbmem_res; / framebuffer resource,
    pub /: *mut *mut *mut resource regs_res; / registers resource,
    pub /: *mut *mut *mut resource regs2d_res; / 2d registers resource,
    pub /: *mut *mut *mut sm501_platdata_fb pdata; / our platform data,
    pub /: *mut *mut unsigned long pm_crt_ctrl; / pm: crt ctrl save,
    pub irq: c_int,
    pub /: *mut *mut int swap_endian; / set to swap rgb=>bgr,
    pub /: *mut *mut *mut void __iomem regs; / remapped registers,
    pub /: *mut *mut *mut void __iomem regs2d; / 2d remapped registers,
    pub /: *mut *mut *mut void __iomem fbmem; / remapped framebuffer,
    pub /: *mut *mut size_t fbmem_len; / length of remapped region,
    pub edid_data: *mut u8,
    pub fb_mode: *mut c_char,
}

// per-framebuffer private data
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sm501fb_par {
    pub pseudo_palette: [u32; 16],
    pub head: enum sm501_controller,
    pub cursor: sm501_mem,
    pub screen: sm501_mem,
    pub ops: fb_ops,
    pub store_fb: *mut c_void,
    pub store_cursor: *mut c_void,
    pub cursor_regs: *mut void __iomem,
    pub info: *mut sm501fb_info,
}

// Helper functions
#[no_mangle]
pub unsafe extern "C" fn h_total(var: *mut fb_var_screeninfo) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn v_total(var: *mut fb_var_screeninfo) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn sm501fb_sync_regs(info: *mut sm501fb_info) {
...
//
#[no_mangle]
unsafe extern "C" fn sm501fb_ps_to_hz(psvalue: c_ulong) -> c_ulong {
    static unsigned long sm501fb_ps_to_hz(unsigned long psvalue)
    {
    let mut numerator: c_ulonglong = 1000000000000ULL;
...
//
    ...
    struct sm501fb_par  *par = info.par;
    struct sm501fb_info *fbi = par.info;
    let mut bytes_pixel: c_uint = info.var.bits_per_pixel / 8;
    unsigned long reg;
    unsigned long xoffs;
...
//
#[no_mangle]
unsafe extern "C" fn sm501fb_set_par_crt(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn sm501fb_panel_power(fbi: *mut sm501fb_info, to: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn sm501fb_set_par_pnl(info: *mut fb_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sm501fb_blank_pnl(blank_mode: c_int, info: *mut fb_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sm501fb_blank_crt(blank_mode: c_int, info: *mut fb_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sm501fb_cursor(info: *mut fb_info, cursor: *mut fb_cursor) -> c_int {
...
// set data
    ...
    let mut x: c_uint = cursor.image.dx;
    let mut y: c_uint = cursor.image.dy;
    if (x >= 2048 || y >= 2048 )
    return -EINVAL;
...
// y += cursor->image.height;
    ...
    let mut bg_col: c_uint = cursor.image.bg_color;
    let mut fg_col: c_uint = cursor.image.fg_color;
...
// cursor
    ...
    let mut dcol: c_uchar = 0;
    let mut dmsk: c_uchar = 0;
    unsigned int   op;
...
//
    ...
    head = HEAD_CRT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strncasecmp(buf, _arg: "panel", 0: 5) ==) -> else {
...
//
    ...
    static DEVICE_ATTR(fbregs_crt, 0444, sm501fb_debug_show_crt, core::ptr::null_mut());
...
//
    ...
    static DEVICE_ATTR(fbregs_pnl, 0444, sm501fb_debug_show_pnl, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn sm501fb_sync(info: *mut fb_info) -> c_int {
    static int sm501fb_sync(struct fb_info *info)
    {
    let mut count: c_int = 1000000;
    struct sm501fb_par  *par = info.par;
    struct sm501fb_info *fbi = par.info;
...
#[no_mangle]
unsafe extern "C" fn sm501fb_copyarea(info: *mut fb_info, area: *const fb_copyarea) {
    static void sm501fb_copyarea(struct fb_info *info, const struct fb_copyarea *area)
    {
    struct sm501fb_par  *par = info.par;
    struct sm501fb_info *fbi = par.info;
    let mut width: c_int = area.width;
    let mut height: c_int = area.height;
    let mut sx: c_int = area.sx;
    let mut sy: c_int = area.sy;
    let mut dx: c_int = area.dx;
    let mut dy: c_int = area.dy;
    let mut rtl: c_ulong = 0;
// source clip
...
#[no_mangle]
unsafe extern "C" fn sm501fb_fillrect(info: *mut fb_info, rect: *const fb_fillrect) {
    static void sm501fb_fillrect(struct fb_info *info, const struct fb_fillrect *rect)
    {
    struct sm501fb_par  *par = info.par;
    struct sm501fb_info *fbi = par.info;
    let mut width: c_int = rect.width, height = rect.height;
...
//
#[no_mangle]
unsafe extern "C" fn sm501_init_cursor(fbi: *mut fb_info, reg_base: c_uint) -> c_int {
...
// controller registers
    ...
    resource_size(res),
    pdev.name);
    if (info.regs_res == core::ptr::null_mut()) {
    dev_err(dev, "cannot claim registers\n");
    ret = -ENXIO;
    goto err_release;
    }
    info.regs = ioremap(res.start, resource_size(res));
    if (info.regs == core::ptr::null_mut()) {
...
// controller registers
    ...
    resource_size(res),
    pdev.name);
    if (info.regs2d_res == core::ptr::null_mut()) {
    dev_err(dev, "cannot claim registers\n");
    ret = -ENXIO;
    goto err_regs_map;
    }
    info.regs2d = ioremap(res.start, resource_size(res));
    if (info.regs2d == core::ptr::null_mut()) {
    dev_err(dev, "cannot remap registers\n");
    ret = -ENXIO;
    goto err_regs2d_res;
    }
// allocate, reserve resources for framebuffer
    ...
    resource_size(res),
    pdev.name);
    if (info.fbmem_res == core::ptr::null_mut()) {
    dev_err(dev, "cannot claim framebuffer\n");
    ret = -ENXIO;
    goto err_regs2d_map;
    }
    info.fbmem = ioremap(res.start, resource_size(res));
    if (info.fbmem == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn sm501fb_stop(info: *mut sm501fb_info) {
...
// ensure we set the correct source register
    ...
    break;
    default:
    pd = core::ptr::null_mut();		/* stop compiler warnings */
    ctrl = 0;
    enable = 0;
    BUG();
    }
...
// update ops dependent on what we've been passed
    ...
    par.ops.fb_cursor = core::ptr::null_mut();
    fb.fbops = &par.ops;
...
// TODO read the mode from the current display
    ...
    ret = fb_find_mode(&fb.var, fb,
    core::ptr::null_mut(), core::ptr::null_mut(), 0, core::ptr::null_mut(), 8);
    }
    switch (ret) {
    case 1:
...
#[no_mangle]
unsafe extern "C" fn sm501fb_probe(pdev: *mut platform_device) -> c_int {
...
// probe for the presence of each panel
    ...
    if (info.fb[HEAD_PANEL] == core::ptr::null_mut() &&
    info.fb[HEAD_CRT] == core::ptr::null_mut()) {
...
// These aren't needed any more
    ...
    kfree(info.fb_mode);
    info.edid_data = core::ptr::null_mut();
    info.fb_mode = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn sm501fb_remove(pdev: *mut platform_device) {
...
// backup copies in case chip is powered down over suspend
    ...
    err_nocursor:
    vfree(par.store_fb);
    par.store_fb = core::ptr::null_mut();
    return -ENOMEM;
    }
...
#[no_mangle]
unsafe extern "C" fn sm501fb_suspend(pdev: *mut platform_device, state: pm_message_t) -> c_int {
...
// turn off the clocks, in case the device is not powered down
    ...
    SM501_DC_CRT_CONTROL_SEL)
#[no_mangle]
unsafe extern "C" fn sm501fb_resume(pdev: *mut platform_device) -> c_int {
