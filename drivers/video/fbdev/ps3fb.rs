...
//! Source: drivers/video/fbdev/ps3fb.c
#![no_std]
...
pub const GPU_DRIVER_INFO_VERSION: c_uint = 0x211;
// gpu internals
#[repr(C)]
#[derive(Copy, Clone)]
pub struct display_head {
    pub be_time_stamp: u64,
    pub status: u32,
    pub offset: u32,
    pub res1: u32,
    pub res2: u32,
...
pub struct gpu_driver_info {
    ...
    pub reserved: [u32; 1063],
    pub display_head: [display_head; 8],
    pub irq: gpu_irq,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ps3fb_priv {
    pub irq_no: c_uint,
    pub memory_handle: u64 context_handle,,
    pub dinfo: *mut gpu_driver_info,
    pub /: *mut *mut u64 vblank_count; / frame count,
    pub wait_vsync: wait_queue_head_t,
    pub /: *mut *mut atomic_t ext_flip; / on/off flip with vsync,
    pub /: *mut *mut atomic_t f_count; / fb_open count,
    pub is_blanked: c_int,
    pub is_kicked: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ps3fb_par {
    pub pseudo_palette: [u32; 16],
    pub new_mode_id: int mode_id,,
    pub /: *mut *mut unsigned int num_frames; / num of frame buffers,
    pub width: c_uint,
    pub height: c_uint,
    pub ddr_line_length: c_uint,
    pub ddr_frame_size: c_uint,
    pub xdr_frame_size: c_uint,
    pub /: *mut *mut unsigned int full_offset; / start of fullscreen DDR fb,
    pub /: *mut *mut unsigned int fb_offset; / start of actual DDR fb,
    pub pan_offset: c_uint,
}

pub const FIRST_NATIVE_MODE_INDEX: c_int = 10;
...
// Macro flag: #define HEAD_A
// Macro flag: #define HEAD_B
...
// resolution difference
    ...
    {
    let mut mode: u32 = id & PS3AV_MODE_MASK;
...
#[no_mangle]
unsafe extern "C" fn ps3fb_sync(info: *mut fb_info, frame: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_open(info: *mut fb_info, user: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_release(info: *mut fb_info, user: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ps3fb_check_var(var: *mut fb_var_screeninfo, info: *mut fb_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ps3fb_set_par(info: *mut fb_info) -> c_int {
...
// Clear DDR frame buffer memory
    ...
    let mut l: c_uint = min(lines, maxlines);
...
//
#[no_mangle]
unsafe extern "C" fn ps3fb_mmap(info: *mut fb_info, vma: *mut vm_area_struct) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ps3fb_blank(blank: c_int, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_get_vblank(vblank: *mut fb_vblank) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_wait_for_vsync(crtc: u32) -> c_int {
...
//
    ...
    let mut id: u32 = ps3av_get_auto_mode();
    if (id > 0)
...
#[no_mangle]
unsafe extern "C" fn ps3fbd(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_vsync_interrupt(irq: c_int, ptr: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_probe(dev: *mut ps3_system_bus_device) -> c_int {
    static int ps3fb_probe(struct ps3_system_bus_device *dev)
    {
    struct fb_info *info;
    struct ps3fb_par *par;
    int retval;
    let mut ddr_lpar: u64 = 0;
    let mut lpar_dma_control: u64 = 0;
    let mut lpar_driver_info: u64 = 0;
    let mut lpar_reports: u64 = 0;
    let mut lpar_reports_size: u64 = 0;
    u64 xdr_lpar;
...
// vsync interrupt
    dinfo = (void  *)ioremap(lpar_driver_info, 128 * 1024);
    if (!dinfo) {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_shutdown(dev: *mut ps3_system_bus_device) {
    static void ps3fb_shutdown(struct ps3_system_bus_device *dev)
    {
    struct fb_info *info = ps3_system_bus_get_drvdata(dev);
    let mut xdr_lpar: u64 = ps3_mm_phys_to_lpar(__pa(ps3fb_videomemory.address));
    dev_dbg(&dev.core, " . %s:%d\n", __func__, __LINE__);
    atomic_inc(&ps3fb.ext_flip);	/* flip off */
    ps3fb.dinfo.irq.mask = 0;
    if (ps3fb.task) {
    struct task_struct *task = ps3fb.task;
    ps3fb.task = core::ptr::null_mut();
    kthread_stop(task);
    }
    if (ps3fb.irq_no) {
    free_irq(ps3fb.irq_no, &dev.core);
...
#[no_mangle]
unsafe extern "C" fn ps3fb_setup() -> int __init {
    ...
    ps3fb_mode = simple_strtoul(this_opt + 5, core::ptr::null_mut(), 0);
    else
...
#[no_mangle]
unsafe extern "C" fn ps3fb_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ps3fb_exit() -> void __exit {
