...
//! Source: drivers/video/fbdev/asiliantfb.c
#![no_std]
...
// Built in clock of the 69030
    let mut Fref: static unsigned = 14318180;

    writeb((num), mmio_base + (ap)); writeb((val), mmio_base + (dp)); \
    } while (0)
#[no_mangle]
unsafe extern "C" fn mm_write_xr(p: *mut fb_info, reg: u8, data: u8) {
...
#[no_mangle]
unsafe extern "C" fn mm_write_fr(p: *mut fb_info, reg: u8, data: u8) {
...
#[no_mangle]
unsafe extern "C" fn mm_write_cr(p: *mut fb_info, reg: u8, data: u8) {
...
#[no_mangle]
unsafe extern "C" fn mm_write_gr(p: *mut fb_info, reg: u8, data: u8) {
...
#[no_mangle]
unsafe extern "C" fn mm_write_sr(p: *mut fb_info, reg: u8, data: u8) {
...
#[no_mangle]
unsafe extern "C" fn mm_write_ar(p: *mut fb_info, reg: u8, data: u8) {
...
// value
#[no_mangle]
unsafe extern "C" fn asiliant_calc_dclk2(ppixclock: *mut u32, dclk2_m: *mut u8, dclk2_n: *mut u8, dclk2_div: *mut u8) {
    static void asiliant_calc_dclk2(u32 *ppixclock, u8 *dclk2_m, u8 *dclk2_n, u8 *dclk2_div)
    {
    let mut pixclock: unsigned = *ppixclock;
    unsigned Ftarget;
    unsigned n;
    let mut best_error: unsigned = 0xffffffff;
    unsigned best_m = 0xffffffff,
    best_n = 0xffffffff;
    unsigned ratio;
    unsigned remainder;
    let mut divisor: c_uchar = 0;
...
// together with 3 <= n <= 257.
    for (n = 3; n <= 257; n++) {
    let mut m: unsigned = n * ratio + (n * remainder) / Fref;
// 3 <= m <= 257
    if (m >= 3 && m <= 257) {
...
#[no_mangle]
pub unsafe extern "C" fn if(1028: m <=) -> else {
...
#[no_mangle]
unsafe extern "C" fn asiliant_set_timing(p: *mut fb_info) {
    static void asiliant_set_timing(struct fb_info *p)
    {
    let mut hd: unsigned = p.var.xres / 8;
    let mut hs: unsigned = (p.var.xres + p.var.right_margin) / 8;
    let mut he: unsigned = (p.var.xres + p.var.right_margin + p.var.hsync_len) / 8;
    let mut ht: unsigned = (p.var.left_margin + p.var.xres + p.var.right_margin + p.var.hsync_len) / 8;
    let mut vd: unsigned = p.var.yres;
    let mut vs: unsigned = p.var.yres + p.var.lower_margin;
    let mut ve: unsigned = p.var.yres + p.var.lower_margin + p.var.vsync_len;
    let mut vt: unsigned = p.var.upper_margin + p.var.yres + p.var.lower_margin + p.var.vsync_len;
    let mut wd: unsigned = (p.var.xres_virtual * ((p.var.bits_per_pixel+7)/8)) / 8;
...
#[no_mangle]
unsafe extern "C" fn asiliantfb_set_par(p: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn chips_hw_init(p: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn init_asiliant(p: *mut fb_info, addr: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn asiliantfb_remove(dp: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn asiliantfb_init() -> int __init {
    static int __init asiliantfb_init(void)
    {
    if (fb_modesetting_disabled("asiliantfb"))
    return -ENODEV;
    if (fb_get_options("asiliantfb", core::ptr::null_mut()))
    return -ENODEV;
    return pci_register_driver(&asiliantfb_driver);
    }
    module_init(asiliantfb_init);
#[no_mangle]
unsafe extern "C" fn asiliantfb_exit() -> void __exit {
