...
//! Source: drivers/video/fbdev/cirrusfb.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct zorrocl {
    pub /: *mut *mut enum cirrus_board type; / Board type,
    pub /: *mut *mut u32 regoffset; / Offset of registers in first Zorro device,
    pub /: *mut *mut u32 ramsize; / Size of video RAM in first Zorro device,
// If zero, use autoprobe on RAM device
    pub /: *mut *mut u32 ramoffset; / Offset of video RAM in first Zorro device,
    pub /: *mut *mut zorro_id ramid; / Zorro ID of RAM device,
    pub /: *mut *mut zorro_id ramid2; / Zorro ID of optional second RAM device,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cirrusfb_info {
    pub regbase: *mut u8 __iomem,
    pub laguna_mmio: *mut u8 __iomem,
    pub btype: enum cirrus_board,
    pub /: *mut *mut unsigned char SFR; / Shadow of special function register,
    pub multiplexing: c_int,
    pub doubleVCLK: c_int,
    pub blank_mode: c_int,
    pub pseudo_palette: [u32; 16],
    pub info): *mut *mut void (unmap)(struct fb_info,
}
...
#[no_mangle]
pub unsafe extern "C" fn is_laguna(cinfo: *const cirrusfb_info) -> c_int {
...
// --- Open /dev/fbx ---------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn cirrusfb_open(info: *mut fb_info, user: c_int) -> c_int {
...
// --- Close /dev/fbx --------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn cirrusfb_release(info: *mut fb_info, user: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_check_mclk(info: *mut fb_info, freq: c_long) -> c_int {
    static int cirrusfb_check_mclk(struct fb_info *info, long freq)
    {
    struct cirrusfb_info *cinfo = info.par;
    let mut mclk: c_long = vga_rseq(cinfo.regbase, CL_SEQR1F) & 0x3f;
// Read MCLK value
    mclk = (14318 * mclk) >> 3;
...
//
    ...
    long maxclock;
    struct cirrusfb_info *cinfo = info.par;
    let mut maxclockidx: unsigned = var.bits_per_pixel >> 3;
...
// truncate xoffset and yoffset to maximum if too high
    ...
    yres *= 2;
#[no_mangle]
pub unsafe extern "C" fn if(FB_VMODE_INTERLACED: var->vmode &) -> else {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_set_mclk_as_source(info: *const fb_info, div: c_int) {
    static void cirrusfb_set_mclk_as_source(const struct fb_info *info, int div)
    {
    struct cirrusfb_info *cinfo = info.par;
    unsigned char old1f, old1e;
    assert(cinfo != core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn cirrusfb_set_par_foo(info: *mut fb_info) -> c_int {
    ...
    long freq;
    int nom, den, div;
    let mut control: c_uint = 0, format = 0, threshold = 0;
...
// as clock source
//
    let mut divMCLK: c_int = cirrusfb_check_mclk(info, freq);
    if (divMCLK)
    nom = 0;
    cirrusfb_set_mclk_as_source(info, divMCLK);
    }
    if (is_laguna(cinfo)) {
    let mut pcifc: c_long = fb_readl(cinfo.laguna_mmio + 0x3fc);
    let mut tile: c_uchar = fb_readb(cinfo.laguna_mmio + 0x407);
...
//
#[no_mangle]
pub unsafe extern "C" fn if(8: var->bits_per_pixel ==) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(24: var->bits_per_pixel ==) -> else {
...
//
    ...
    cirrusfb_dbg_reg_dump(info, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_set_par(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_blank(blank_mode: c_int, info: *mut fb_info) -> c_int {
...
//
    unsigned char val;
    struct cirrusfb_info *cinfo = info.par;
    let mut current_mode: c_int = cinfo.blank_mode;
...
#[no_mangle]
unsafe extern "C" fn init_vgachip(info: *mut fb_info) {
    static void init_vgachip(struct fb_info *info)
    {
    struct cirrusfb_info *cinfo = info.par;
    const struct cirrusfb_board_info_rec *bi;
...
#[no_mangle]
unsafe extern "C" fn switch_monitor(cinfo: *mut cirrusfb_info, on: c_int) {
    static void switch_monitor(struct cirrusfb_info *cinfo, int on)
    {
...
//
#[no_mangle]
unsafe extern "C" fn cirrusfb_sync(info: *mut fb_info) -> c_int {
    ...
    int vxres, vyres;
    struct cirrusfb_info *cinfo = info.par;
    let mut m: c_int = info.var.bits_per_pixel;
    ...
    {
    struct cirrusfb_info *cinfo = info.par;
    let mut op: c_uchar = (info.var.bits_per_pixel == 24) ? 0xc : 0x4;
    if (info.state != FBINFO_STATE_RUNNING)
    return;
// Alpine/SD64 does not work at 24bpp ???
    ...
    let mut size: unsigned = ((image.width + 7) >> 3) * image.height;
    let mut m: c_int = info.var.bits_per_pixel;
    u32 fg, bg;
    if (info.var.bits_per_pixel == 8) {
    fg = image.fg_color;
...
// seem to have.
    ...
    struct cirrusfb_info *cinfo = info.par;
    if (is_laguna(cinfo)) {
    let mut SR14: c_uchar = vga_rseq(regbase, CL_SEQR14);
    mem = ((SR14 & 7) + 1) << 20;
    } else {
    let mut SRF: c_uchar = vga_rseq(regbase, CL_SEQRF);
    switch ((SRF & 0x18)) {
    case 0x08:
    mem = 512 * 1024;
    break;
...
// TODO: Handling of GD5446/5480 (see XF86 sources ...)
    ...
    assert(display != core::ptr::null_mut());
    assert(registers != core::ptr::null_mut());
// display = 0;
// registers = 0;
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_pci_unmap(info: *mut fb_info) {
    static void cirrusfb_pci_unmap(struct fb_info *info)
    {
    struct pci_dev *pdev = to_pci_dev(info.device);
    struct cirrusfb_info *cinfo = info.par;
    if (cinfo.laguna_mmio == core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_zorro_unmap(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_set_fbinfo(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_register(info: *mut fb_info) -> c_int {
...
// set all the vital stuff
    ...
    err = fb_find_mode(&info.var, info, mode_option, core::ptr::null_mut(), 0, core::ptr::null_mut(), 8);
    if (!err) {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_cleanup(info: *mut fb_info) {
...
// FIXME: this forces VGA.  alternatives?
    cinfo.regbase = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_pci_unregister(pdev: *mut pci_dev) {
...
// Quirk for 64 MiB Picasso IV
    ...
    struct zorro_dev *ram = zorro_find_device(zcl.ramid, core::ptr::null_mut());
    ...
    if (zcl.ramid2 &&
    (ram = zorro_find_device(zcl.ramid2, core::ptr::null_mut()))) {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_zorro_unregister(z: *mut zorro_dev) {
    static void cirrusfb_zorro_unregister(struct zorro_dev *z)
    {
    struct fb_info *info = zorro_get_drvdata(z);
    cirrusfb_cleanup(info);
    zorro_set_drvdata(z, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_setup(options: *mut c_char) -> int __init {
    ...
    noaccel = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "mode:", _arg: 5)) -> else {
...
//
    ...
    MODULE_LICENSE("GPL");
#[no_mangle]
unsafe extern "C" fn cirrusfb_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_exit() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn RGen(cinfo: *const cirrusfb_info, regnum: c_int) -> c_uchar {
    static unsigned char RGen(const struct cirrusfb_info *cinfo, int regnum)
    {
    let mut regofs: c_ulong = 0;
    if (cinfo.btype == BT_PICASSO) {
...
#[no_mangle]
unsafe extern "C" fn AttrOn(cinfo: *const cirrusfb_info) {
    static void AttrOn(const struct cirrusfb_info *cinfo)
    {
    assert(cinfo != core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn WHDR(cinfo: *const cirrusfb_info, val: c_uchar) {
    static void WHDR(const struct cirrusfb_info *cinfo, unsigned char val)
    {
    if (is_laguna(cinfo))
    return;
...
#[no_mangle]
unsafe extern "C" fn WSFR(cinfo: *mut cirrusfb_info, val: c_uchar) {
    static void WSFR(struct cirrusfb_info *cinfo, unsigned char val)
    {

    assert(cinfo.regbase != core::ptr::null_mut());
    cinfo.SFR = val;
...
#[no_mangle]
unsafe extern "C" fn WSFR2(cinfo: *mut cirrusfb_info, val: c_uchar) {
    static void WSFR2(struct cirrusfb_info *cinfo, unsigned char val)
    {
...
#[no_mangle]
unsafe extern "C" fn cirrusfb_WaitBLT(regbase: *mut u8 __iomem) {
    static void cirrusfb_WaitBLT(u8 __iomem *regbase)
    {
...
//
    ...
    u_short line_length)
    {
    let mut nwidth: u_short = width - 1;
    let mut nheight: u_short = height - 1;
    u_long nsrc, ndest;
    u_char bltmode;
    bltmode = 0x00;
...
//
    ...
    u_char blitmode)
    {
    let mut ndest: u_long = (y * line_length) + x;
    u_char op;
    cirrusfb_WaitBLT(regbase);
...
//
#[no_mangle]
unsafe extern "C" fn bestclock(freq: c_long, nom: *mut c_int, den: *mut c_int, div: *mut c_int) {
    static void bestclock(long freq, int *nom, int *den, int *div)
    {
    int n, d;
    long h, diff;
    assert(nom != core::ptr::null_mut());
    assert(den != core::ptr::null_mut());
    assert(div != core::ptr::null_mut());
// nom = 0;
// den = 0;
// div = 0;
    if (freq < 8000)
...
//
#[no_mangle]
unsafe extern "C" fn cirrusfb_dbg_reg_dump(info: *mut fb_info, regbase: caddr_t) {
    static void cirrusfb_dbg_reg_dump(struct fb_info *info, caddr_t regbase)
    {
