...
//! Source: drivers/video/fbdev/grvga.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct grvga_regs {
    pub /: *mut *mut u32 status; / 0x00,
    pub /: *mut *mut u32 video_length; / 0x04,
    pub /: *mut *mut u32 front_porch; / 0x08,
    pub /: *mut *mut u32 sync_length; / 0x0C,
    pub /: *mut *mut u32 line_length; / 0x10,
    pub /: *mut *mut u32 fb_pos; / 0x14,
    pub /: *mut *mut u32 clk_vector[4]; / 0x18,
    pub /: *mut *mut u32 clut; / 0x28,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct grvga_par {
    pub regs: *mut grvga_regs,
    pub /: *mut *mut u32 color_palette[16]; / 16 entry pseudo palette used by fbcon in true color mode,
    pub clk_sel: c_int,
    pub /: *mut *mut int fb_alloced; / = 1 if framebuffer is allocated in main memory,
}

    static const struct fb_videomode grvga_modedb[] = {
    {
// 640x480 @ 60 Hz
    core::ptr::null_mut(), 60, 640, 480, 40000, 48, 16, 39, 11, 96, 2,
...
#[no_mangle]
pub unsafe extern "C" fn if(32: var->bits_per_pixel <=) -> else {
...
#[no_mangle]
unsafe extern "C" fn grvga_set_par(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn grvga_setcolreg(regno: unsigned, red: unsigned, green: unsigned, blue: unsigned, transp: unsigned, info: *mut fb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(FB_VISUAL_TRUECOLOR: info->fix.visual ==) -> else {
    u32 v;
    if (regno >= 16)
    return -EINVAL;
...
// Set framebuffer base address
    ...
    switch (count) {
    case 0:
    screendata.pixclock = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 1:
    screendata.xres = screendata.xres_virtual = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 2:
    screendata.right_margin = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 3:
    screendata.hsync_len = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 4:
    screendata.left_margin = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 5:
    screendata.yres = screendata.yres_virtual = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 6:
    screendata.lower_margin = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 7:
    screendata.vsync_len = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 8:
    screendata.upper_margin = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    case 9:
    screendata.bits_per_pixel = simple_strtoul(this_opt, core::ptr::null_mut(), 0);
    count++;
    break;
    default:
    return -1;
...
#[no_mangle]
unsafe extern "C" fn grvga_probe(dev: *mut platform_device) -> c_int {
    static int grvga_probe(struct platform_device *dev)
    {
    struct fb_info *info;
    let mut retval: c_int = -ENOMEM;
    unsigned long virtual_start;
    let mut grvga_fix_addr: c_ulong = 0;
    let mut physical_start: c_ulong = 0;
    let mut grvga_mem_size: c_ulong = 0;
    struct grvga_par *par = core::ptr::null_mut();
    char *options = core::ptr::null_mut(), *mode_opt = core::ptr::null_mut();
...
//
    ...
    grvga_fix_addr = simple_strtoul(this_opt + 5, core::ptr::null_mut(), 16);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "size", _arg: 4)) -> else {
    else if (!strncmp(this_opt, "size", 4))
    grvga_mem_size = simple_strtoul(this_opt + 5, core::ptr::null_mut(), 0);
    else
    mode_opt = this_opt;
    }
    par = info.par;
...
#[no_mangle]
unsafe extern "C" fn grvga_remove(device: *mut platform_device) {
