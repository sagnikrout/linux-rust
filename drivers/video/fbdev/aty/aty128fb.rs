...
//! Source: drivers/video/fbdev/aty/aty128fb.c
#![no_std]
...
// various memory configurations
    ...
    let mut default_vmode: static int = VMODE_1024_768_60;
    let mut default_cmode: static int = CMODE_8;

    let mut default_crt_on: static int = 0;
    let mut default_lcd_on: static int = 1;
    let mut mtrr: static bool = true;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct aty128_crtc {
    pub gen_cntl: u32,
    pub h_sync_strt_wid: u32 h_total,,
    pub v_sync_strt_wid: u32 v_total,,
    pub pitch: u32,
    pub offset_cntl: u32 offset,,
    pub yoffset: u32 xoffset,,
    pub vyres: u32 vxres,,
    pub bpp: u32 depth,,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct aty128fb_par {
    pub crtc: aty128_crtc,
    pub pll: aty128_pll,
    pub fifo_reg: aty128_ddafifo,
    pub accel_flags: u32,
    pub /: *mut *mut aty128_constants constants; / PLL and others,
    pub /: *mut *mut *mut void __iomem regbase; / remapped mmio,
    pub /: *mut *mut u32 vram_size; / onboard video ram,
    pub chip_gen: c_int,
    pub /: *const *const *const aty128_meminfo mem; / onboard mem info,
    pub wc_cookie: c_int,
    pub blitter_may_be_busy: c_int,
    pub /: *mut *mut int fifo_slots; / free slots in FIFO (64 max),
    pub lcd_on: int crt_on,,
    pub pdev: *mut pci_dev,
    pub next: *mut fb_info,
    pub asleep: c_int,
    pub lock_blank: c_int,
    pub /: *mut *mut u8 red[32]; / see aty128fb_setcolreg,
    pub green: [u8; 64],
    pub blue: [u8; 32],
    pub /: *mut *mut u32 pseudo_palette[16]; / used for TRUECOLOR,
}
...
#[no_mangle]
unsafe extern "C" fn aty_pll_readupdate(par: *const aty128fb_par) -> c_int {
    static int aty_pll_readupdate(const struct aty128fb_par *par)
    {
    return !(aty_ld_pll(PPLL_REF_DIV) & PPLL_ATOMIC_UPDATE_R);
    }
#[no_mangle]
unsafe extern "C" fn aty_pll_wait_readupdate(par: *const aty128fb_par) {
...
// tell PLL to update
#[no_mangle]
unsafe extern "C" fn aty_pll_writeupdate(par: *const aty128fb_par) {
...
#[no_mangle]
unsafe extern "C" fn register_test(par: *const aty128fb_par) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn do_wait_for_fifo(entries: u16, par: *mut aty128fb_par) {
    static void do_wait_for_fifo(u16 entries, struct aty128fb_par *par)
    {
    int i;
    for (;;) {
...
#[no_mangle]
unsafe extern "C" fn wait_for_idle(par: *mut aty128fb_par) {
    static void wait_for_idle(struct aty128fb_par *par)
    {
    int i;
...
#[no_mangle]
unsafe extern "C" fn wait_for_fifo(entries: u16, par: *mut aty128fb_par) {
    static void wait_for_fifo(u16 entries, struct aty128fb_par *par)
    {
    if (par.fifo_slots < entries)
    do_wait_for_fifo(64, par);
...
#[no_mangle]
unsafe extern "C" fn aty128_flush_pixel_cache(par: *const aty128fb_par) {
    static void aty128_flush_pixel_cache(const struct aty128fb_par *par)
    {
    int i;
    u32 tmp;
...
#[no_mangle]
unsafe extern "C" fn aty128_reset_engine(par: *const aty128fb_par) {
    static void aty128_reset_engine(const struct aty128fb_par *par)
    {
...
#[no_mangle]
unsafe extern "C" fn aty128_init_engine(par: *mut aty128fb_par) {
    static void aty128_init_engine(struct aty128fb_par *par)
    {
    u32 pitch_value;
    wait_for_idle(par);
...
#[no_mangle]
unsafe extern "C" fn depth_to_dst(depth: u32) -> u32 {
    static u32 depth_to_dst(u32 depth)
    {
    if (depth <= 8)
    return DST_8BPP;
#[no_mangle]
pub unsafe extern "C" fn if(15: depth <=) -> else {
    else if (depth <= 15)
    return DST_15BPP;
#[no_mangle]
pub unsafe extern "C" fn if(16: depth ==) -> else {
    else if (depth == 16)
    return DST_16BPP;
#[no_mangle]
pub unsafe extern "C" fn if(24: depth <=) -> else {
    else if (depth <= 24)
    return DST_24BPP;
#[no_mangle]
pub unsafe extern "C" fn if(32: depth <=) -> else {
    else if (depth <= 32)
    return DST_32BPP;
    return -EINVAL;
    }
...
//
    u32  segstart;
    unsigned char __iomem *rom_base = core::ptr::null_mut();
    for (segstart=0x000c0000; segstart<0x000f0000; segstart+=0x00001000) {
    rom_base = ioremap(segstart, 0x10000);
    if (rom_base == core::ptr::null_mut())
    return core::ptr::null_mut();
    if (readb(rom_base) == 0x55 && readb(rom_base + 1) == 0xaa)
    break;
    iounmap(rom_base);
    rom_base = core::ptr::null_mut();
    }
    return rom_base;
    }
...
#[no_mangle]
unsafe extern "C" fn aty128_timings(par: *mut aty128fb_par) {
    static void aty128_timings(struct aty128fb_par *par)
    {
...
#[no_mangle]
unsafe extern "C" fn aty128_pix_width_to_var(pix_width: c_int, var: *mut fb_var_screeninfo) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn aty128_set_crt_enable(par: *mut aty128fb_par, on: c_int) {
...
#[no_mangle]
unsafe extern "C" fn aty128_set_lcd_enable(par: *mut aty128fb_par, on: c_int) {
...
// clear the reset, just in case
    ...
    const struct aty128fb_par *par)
    {
    let mut c: aty128_constants = par.constants;
...
// calculate feedback divider
    ...
    let mut xclk: u32 = par.constants.xclk;
    let mut fifo_width: u32 = par.constants.fifo_width;
    let mut fifo_depth: u32 = par.constants.fifo_depth;
    s32 x, b, p, ron, roff;
    u32 n, d, bpp;
...
//
#[no_mangle]
unsafe extern "C" fn aty128fb_set_par(info: *mut fb_info) -> c_int {
    static int aty128fb_set_par(struct fb_info *info)
    {
...
#[no_mangle]
pub unsafe extern "C" fn if(16: par->crtc.bpp ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn aty128fb_sync(info: *mut fb_info) -> c_int {
    static int aty128fb_sync(struct fb_info *info)
    {
    struct aty128fb_par *par = info.par;
    if (par.blitter_may_be_busy)
...
#[no_mangle]
unsafe extern "C" fn aty128fb_setup(options: *mut c_char) -> c_int {
    ...
    default_lcd_on = simple_strtoul(this_opt+4, core::ptr::null_mut(), 0);
    continue;
    } else if (!strncmp(this_opt, "crt:", 4)) {
    default_crt_on = simple_strtoul(this_opt+4, core::ptr::null_mut(), 0);
    continue;
...
// vmode and cmode deprecated
    ...
    let mut vmode: c_uint = simple_strtoul(this_opt+6, core::ptr::null_mut(), 0);
    if (vmode > 0 && vmode <= VMODE_MAX)
    default_vmode = vmode;
    continue;
    } else if (!strncmp(this_opt, "cmode:", 6)) {
    let mut cmode: c_uint = simple_strtoul(this_opt+6, core::ptr::null_mut(), 0);
    switch (cmode) {
    case 0:
    case 8:
...
// backlight anyway
//
// Macro flag: #define BACKLIGHT_LVDS_OFF
// That one prevents proper CRT output with LCD off

#[no_mangle]
unsafe extern "C" fn aty128_bl_update_status(bd: *mut backlight_device) -> c_int {
    static int aty128_bl_update_status(struct backlight_device *bd)
    {
    struct aty128fb_par *par = bl_get_data(bd);
    let mut reg: c_uint = aty_ld_le32(LVDS_GEN_CNTL);
    int level;
    if (!par.lcd_on)
    level = 0;
    else
...
#[no_mangle]
unsafe extern "C" fn aty128_bl_set_power(info: *mut fb_info, power: c_int) {
    static void aty128_bl_set_power(struct fb_info *info, int power)
    {
    if (info.bl_dev) {
...
#[no_mangle]
unsafe extern "C" fn aty128_bl_init(par: *mut aty128fb_par) {
...
#[no_mangle]
unsafe extern "C" fn aty128_bl_exit(bd: *mut backlight_device) {
...
#[no_mangle]
unsafe extern "C" fn aty128_early_resume(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn aty128_init(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
...
// Indicate sleep capability
    if (par.chip_gen == rage_M3) {
    pmac_call_feature(PMAC_FTR_DEVICE_CAN_WAKE, core::ptr::null_mut(), 0, 1);
...
// PowerBook Titanium
    ...
    default_cmode = CMODE_32;
#[no_mangle]
pub unsafe extern "C" fn if(8: default_cmode >) -> else {
    ...
    {
    if (mode_option)
    if (fb_find_mode(&var, info, mode_option, core::ptr::null_mut(),
    0, &defaultmode, 8) == 0)
    var = default_var;
    }
...
#[no_mangle]
unsafe extern "C" fn aty128_probe(pdev: *mut pci_dev, ent: *const pci_device_id) -> c_int {
    ...
    void __iomem *bios = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn aty128_remove(pdev: *mut pci_dev) {
    static void aty128_remove(struct pci_dev *pdev)
    {
    struct fb_info *info = pci_get_drvdata(pdev);
    struct aty128fb_par *par;
...
//
#[no_mangle]
unsafe extern "C" fn aty128fb_blank(blank: c_int, fb: *mut fb_info) -> c_int {
    static int aty128fb_blank(int blank, struct fb_info *fb)
    {
    struct aty128fb_par *par = fb.par;
    u8 state;
...
pub const ATY_MIRROR_LCD_ON: c_uint = 0x00000001;
pub const ATY_MIRROR_CRT_ON: c_uint = 0x00000002;
// out param: u32*	backlight value: 0 to 15

// in param: u32*	backlight value: 0 to 15

#[no_mangle]
unsafe extern "C" fn aty128fb_ioctl(info: *mut fb_info, cmd: u_int, arg: u_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn aty128_set_suspend(par: *mut aty128fb_par, suspend: c_int) {
...
#[no_mangle]
unsafe extern "C" fn aty128_pci_suspend_late(dev: *mut device, state: pm_message_t) -> c_int {
    static int aty128_pci_suspend_late(struct device *dev, pm_message_t state)
    {
...
#[no_mangle]
unsafe extern "C" fn aty128_pci_suspend(dev: *mut device) -> int __maybe_unused {
    static int __maybe_unused aty128_pci_suspend(struct device *dev)
    {
    return aty128_pci_suspend_late(dev, PMSG_SUSPEND);
    }
#[no_mangle]
unsafe extern "C" fn aty128_pci_hibernate(dev: *mut device) -> int __maybe_unused {
    static int __maybe_unused aty128_pci_hibernate(struct device *dev)
    {
    return aty128_pci_suspend_late(dev, PMSG_HIBERNATE);
    }
#[no_mangle]
unsafe extern "C" fn aty128_pci_freeze(dev: *mut device) -> int __maybe_unused {
    static int __maybe_unused aty128_pci_freeze(struct device *dev)
    {
    return aty128_pci_suspend_late(dev, PMSG_FREEZE);
    }
#[no_mangle]
unsafe extern "C" fn aty128_do_resume(pdev: *mut pci_dev) -> c_int {
    static int aty128_do_resume(struct pci_dev *pdev)
    {
    struct fb_info *info = pci_get_drvdata(pdev);
    struct aty128fb_par *par = info.par;
...
#[no_mangle]
unsafe extern "C" fn aty128_pci_resume(dev: *mut device) -> int __maybe_unused {
    static int __maybe_unused aty128_pci_resume(struct device *dev)
    {
    int rc;
    console_lock();
...
#[no_mangle]
unsafe extern "C" fn aty128fb_init() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn aty128fb_exit() -> void __exit {
