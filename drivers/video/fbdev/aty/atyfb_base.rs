...
//! Source: drivers/video/fbdev/aty/atyfb_base.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn aty_st_lcd(index: c_int, val: u32, par: *const atyfb_par) {
...
#[no_mangle]
pub unsafe extern "C" fn aty_ld_lcd(index: c_int, par: *const atyfb_par) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn ATIReduceRatio(Numerator: *mut c_int, Denominator: *mut c_int) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pci_mmap_map {
    pub voff: c_ulong,
    pub poff: c_ulong,
    pub size: c_ulong,
    pub prot_flag: c_ulong,
    pub prot_mask: c_ulong,
}
...
#[no_mangle]
unsafe extern "C" fn atyfb_compat_ioctl(info: *mut fb_info, cmd: u_int, arg: u_long) -> c_int {
...
// 640x480 @ 60 Hz, 31.5 kHz hsync
    ...
    static int mclk;
    static int xclk;
    let mut comp_sync: static int = -1;
    static char *mode;
    let mut backlight: static int = IS_BUILTIN(CONFIG_PMAC_BACKLIGHT);

    let mut default_vmode: static int = VMODE_CHOOSE;
    let mut default_cmode: static int = CMODE_CHOOSE;
    ...
    let mut mach64_count: static unsigned int = 0;
...
//
#[no_mangle]
unsafe extern "C" fn aty_fudge_framebuffer_len(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn correct_chipset(par: *mut atyfb_par) -> c_int {
    ...
    {
    let mut pixclock: u32 = var.pixclock;
...
//
#[no_mangle]
unsafe extern "C" fn read_aty_sense(par: *const atyfb_par) -> c_int {
    static int read_aty_sense(const struct atyfb_par *par)
    {
    int sense, i;
...
//
#[no_mangle]
unsafe extern "C" fn aty_get_crtc(par: *const atyfb_par, crtc: *mut crtc) {
    static void aty_get_crtc(const struct atyfb_par *par, struct crtc *crtc)
    {
...
#[no_mangle]
unsafe extern "C" fn aty_set_crtc(par: *const atyfb_par, crtc: *const crtc) {
    static void aty_set_crtc(const struct atyfb_par *par, const struct crtc *crtc)
    {
...
#[no_mangle]
unsafe extern "C" fn calc_line_length(par: *mut atyfb_par, vxres: u32, bpp: u32) -> u32 {
    static u32 calc_line_length(struct atyfb_par *par, u32 vxres, u32 bpp)
    {
    let mut line_length: u32 = vxres * bpp / 8;
...
// convert (and round up) and validate
    ...
    if (par.lcd_table != 0) {
    if (!M64_HAS(LT_LCD_REGS)) {
    let mut lcd_index: u32 = aty_ld_le32(LCD_INDEX, par);
...
//
    let mut HDisplay: c_int = xres & ~7;
    let mut nStretch: c_int = par.lcd_width / HDisplay;
    let mut Remainder: c_int = par.lcd_width % HDisplay;
    if ((!Remainder && ((nStretch > 2))) ||
    (((HDisplay * 16) / par.lcd_width) < 7)) {
    static const char StretchLoops[] = { 10, 12, 13, 15, 16 };
    let mut horz_stretch_loop: c_int = -1, BestRemainder;
    let mut Numerator: c_int = HDisplay, Denominator = par.lcd_width;
    let mut Index: c_int = 5;
    ...
    let mut horz_stretch_ratio: c_int = 0, Accumulator = 0;
    let mut reuse_previous: c_int = 1;
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn atyfb_set_par(info: *mut fb_info) -> c_int {
    static int atyfb_set_par(struct fb_info *info)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
    ...
    atyfb_ops.fb_sync = core::ptr::null_mut();
    info.flags |= FBINFO_HWACCEL_DISABLED;
    }
...
#[no_mangle]
unsafe extern "C" fn atyfb_check_var(var: *mut fb_var_screeninfo, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn set_off_pitch(par: *mut atyfb_par, info: *const fb_info) {
    static void set_off_pitch(struct atyfb_par *par, const struct fb_info *info)
    {
    let mut xoffset: u32 = info.var.xoffset;
    let mut yoffset: u32 = info.var.yoffset;
    let mut line_length: u32 = info.fix.line_length;
    let mut bpp: u32 = info.var.bits_per_pixel;
    par.crtc.off_pitch =
    ((yoffset * line_length + xoffset * bpp / 8) / 8) |
    ((line_length / bpp) << 22);
    }
...
//
#[no_mangle]
unsafe extern "C" fn atyfb_open(info: *mut fb_info, user: c_int) -> c_int {
    static int atyfb_open(struct fb_info *info, int user)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
    if (user) {
...
#[no_mangle]
unsafe extern "C" fn aty_irq(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn aty_enable_irq(par: *mut atyfb_par, reenable: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn aty_disable_irq(par: *mut atyfb_par) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atyfb_release(info: *mut fb_info, user: c_int) -> c_int {
    static int atyfb_release(struct fb_info *info, int user)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
...
//
    ...
    let mut videoram: u32 = (info.fix.smem_len - (PAGE_SIZE << 2));
    var.yres_virtual =
...
#[no_mangle]
unsafe extern "C" fn aty_waitforvblank(par: *mut atyfb_par, crtc: u32) -> c_int {
...
pub const ATYIO_CLKR: c_uint = 0x41545900	/* ATY\00 */;
pub const ATYIO_CLKW: c_uint = 0x41545901	/* ATY\01 */;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct atyclk {
    pub ref_clk_per: u32,
    pub pll_ref_div: u8,
    pub mclk_fb_div: u8,
    pub /: *mut *mut u8 mclk_post_div; / 1,2,3,4,8,
    pub /: *mut *mut u8 mclk_fb_mult; / 2 or 4,
    pub /: *mut *mut u8 xclk_post_div; / 1,2,3,4,8,
    pub vclk_fb_div: u8,
    pub /: *mut *mut u8 vclk_post_div; / 1,2,3,4,6,8,12,
    pub /: *mut *mut u32 dsp_xclks_per_row; / 0-16383,
    pub /: *mut *mut u32 dsp_loop_latency; / 0-15,
    pub /: *mut *mut u32 dsp_precision; / 0-7,
    pub /: *mut *mut u32 dsp_on; / 0-2047,
    pub /: *mut *mut u32 dsp_off; / 0-2047,
}

pub const ATYIO_FEATR: c_uint = 0x41545902	/* ATY\02 */;
pub const ATYIO_FEATW: c_uint = 0x41545903	/* ATY\03 */;

#[no_mangle]
unsafe extern "C" fn atyfb_ioctl(info: *mut fb_info, cmd: u_int, arg: u_long) -> c_int {
    static int atyfb_ioctl(struct fb_info *info, u_int cmd, u_long arg)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
    ...
    case ATYIO_CLKR:
    if (M64_HAS(INTEGRATED)) {
    let mut clk: atyclk = { 0 };
    union aty_pll *pll = &par.pll;
    let mut dsp_config: u32 = pll.ct.dsp_config;
    let mut dsp_on_off: u32 = pll.ct.dsp_on_off;
...
#[no_mangle]
unsafe extern "C" fn atyfb_sync(info: *mut fb_info) -> c_int {
    static int atyfb_sync(struct fb_info *info)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
    if (par.blitter_may_be_busy)
...
#[no_mangle]
unsafe extern "C" fn atyfb_mmap(info: *mut fb_info, vma: *mut vm_area_struct) -> c_int {
    static int atyfb_mmap(struct fb_info *info, struct vm_area_struct *vma)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
    unsigned int size, page, map_size = 0;
    let mut map_offset: c_ulong = 0;
    unsigned long off;
    int i;
    if (!par.mmap_map)
    return -ENXIO;
...
// Each page, see which map applies
    ...
    let mut start: c_ulong = par.mmap_map[i].voff;
    let mut end: c_ulong = start + par.mmap_map[i].size;
    let mut offset: c_ulong = off + page;
    if (start > offset)
    continue;
    if (offset >= end)
    continue;
...
//
#[no_mangle]
unsafe extern "C" fn aty_power_mgmt(sleep: c_int, par: *mut atyfb_par) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atyfb_pci_suspend_late(dev: *mut device, state: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atyfb_pci_resume(dev: *mut device) -> int __maybe_unused {
...
pub const MAX_LEVEL: c_uint = 0xFF;
#[no_mangle]
unsafe extern "C" fn aty_bl_get_level_brightness(par: *mut atyfb_par, level: c_int) -> c_int {
...
// No locking of bl_curve since we read a single value
    ...
    atylevel = 0;
#[no_mangle]
pub unsafe extern "C" fn if(MAX_LEVEL: atylevel >) -> else {
    else if (atylevel > MAX_LEVEL)
    atylevel = MAX_LEVEL;
    return atylevel;
    }
#[no_mangle]
unsafe extern "C" fn aty_bl_update_status(bd: *mut backlight_device) -> c_int {
    static int aty_bl_update_status(struct backlight_device *bd)
    {
    struct atyfb_par *par = bl_get_data(bd);
    let mut reg: c_uint = aty_ld_lcd(LCD_MISC_CNTL, par);
...
#[no_mangle]
unsafe extern "C" fn aty_bl_init(par: *mut atyfb_par) {
...
#[no_mangle]
unsafe extern "C" fn aty_bl_exit(bd: *mut backlight_device) {
...
#[no_mangle]
unsafe extern "C" fn aty_calc_mem_refresh(par: *mut atyfb_par, xclk: c_int) {
...
#[no_mangle]
unsafe extern "C" fn aty_init(info: *mut fb_info) -> c_int {
    static int aty_init(struct fb_info *info)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
    const char *ramname = core::ptr::null_mut(), *xtal;
...
// Allow command line to override clocks.
    ...
    if (M64_HAS(GTB_DSP)) {
    let mut pll_ref_div: u8 = aty_ld_pll_ct(PLL_REF_DIV, par);
    if (pll_ref_div) {
    int diff1, diff2;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: of_machine_is_compatible("iMac")) -> else {
    else if (of_machine_is_compatible("iMac"))
    default_vmode = VMODE_1024_768_75;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: of_machine_is_compatible("PowerBook2, _arg: 1")) -> else {
...
// iBook with 800x600 LCD
    ...
    if (mode && fb_find_mode(&var, info, mode, core::ptr::null_mut(), 0, &defmode, 8))
    has_var = 1;
    if (!has_var)
    var = default_var;
    if (noaccel)
...
#[no_mangle]
unsafe extern "C" fn store_video_par(video_str: *mut c_char, m64_num: c_uchar) -> c_int {
    ...
    goto mach64_invalid;
    vmembase = simple_strtoul(p, core::ptr::null_mut(), 0);
    if (!(p = strsep(&video_str, ";")) || !*p)
    goto mach64_invalid;
    size = simple_strtoul(p, core::ptr::null_mut(), 0);
    if (!(p = strsep(&video_str, ";")) || !*p)
    goto mach64_invalid;
    guiregbase = simple_strtoul(p, core::ptr::null_mut(), 0);
...
//
#[no_mangle]
unsafe extern "C" fn atyfb_blank(blank: c_int, info: *mut fb_info) -> c_int {
    static int atyfb_blank(int blank, struct fb_info *info)
    {
    struct atyfb_par *par = (struct atyfb_par *) info.par;
    u32 gen_cntl;
    ...
    let mut pm: u32 = aty_ld_lcd(POWER_MANAGEMENT, par);
    pm &= ~PWR_BLON;
    aty_st_lcd(POWER_MANAGEMENT, pm, par);
    }
...
#[no_mangle]
unsafe extern "C" fn aty_init_lcd(par: *mut atyfb_par, bios_base: u32) {
...
#[no_mangle]
unsafe extern "C" fn init_from_bios(par: *mut atyfb_par) -> c_int {
...
//
    ...
    if (par.ati_regbase == core::ptr::null_mut())
    return -ENOMEM;
...
//
    ...
    info.fix.smem_len);
    if (info.screen_base == core::ptr::null_mut()) {
    ret = -ENOMEM;
...
// devm handles cleanup automatically on probe failure
    par.ati_regbase = core::ptr::null_mut();
    info.screen_base = core::ptr::null_mut();
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn atyfb_atari_probe() -> int __init {
    ...
    info = framebuffer_alloc(sizeof(struct atyfb_par), core::ptr::null_mut());
    if (!info)
    return -ENOMEM;
    par = info.par;
    info.fix = atyfb_fix;
...
#[no_mangle]
unsafe extern "C" fn atyfb_remove(info: *mut fb_info) {
...
#[no_mangle]
unsafe extern "C" fn atyfb_pci_remove(pdev: *mut pci_dev) {
    static void atyfb_pci_remove(struct pci_dev *pdev)
    {
    struct fb_info *info = pci_get_drvdata(pdev);
    mutex_lock(&reboot_lock);
    if (reboot_info == info)
    reboot_info = core::ptr::null_mut();
    mutex_unlock(&reboot_lock);
    atyfb_remove(info);
    }
...
#[no_mangle]
unsafe extern "C" fn atyfb_setup(options: *mut c_char) -> int __init {
    ...
    vram = simple_strtoul(this_opt + 5, core::ptr::null_mut(), 0);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "pll:", _arg: 4)) -> else {
    else if (!strncmp(this_opt, "pll:", 4))
    pll = simple_strtoul(this_opt + 4, core::ptr::null_mut(), 0);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "mclk:", _arg: 5)) -> else {
    else if (!strncmp(this_opt, "mclk:", 5))
    mclk = simple_strtoul(this_opt + 5, core::ptr::null_mut(), 0);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "xclk:", _arg: 5)) -> else {
    else if (!strncmp(this_opt, "xclk:", 5))
    xclk = simple_strtoul(this_opt+5, core::ptr::null_mut(), 0);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "comp_sync:", _arg: 10)) -> else {
    else if (!strncmp(this_opt, "comp_sync:", 10))
    comp_sync = simple_strtoul(this_opt+10, core::ptr::null_mut(), 0);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "backlight:", _arg: 10)) -> else {
    else if (!strncmp(this_opt, "backlight:", 10))
    backlight = simple_strtoul(this_opt+10, core::ptr::null_mut(), 0);

#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(this_opt, _arg: "vmode:", _arg: 6)) -> else {
    unsigned int vmode =
    simple_strtoul(this_opt + 6, core::ptr::null_mut(), 0);
...
//
    ...
    let mut registered_notifier: static bool = false;
#[no_mangle]
unsafe extern "C" fn atyfb_init() -> int __init {
    static int __init atyfb_init(void)
    {
    let mut err1: c_int = 1, err2 = 1;
...
#[no_mangle]
unsafe extern "C" fn atyfb_exit() -> void __exit {
