...
//! Source: drivers/video/fbdev/tgafb.c
#![no_std]
...
pub const TGA_BUS_TC(dev): c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn tgafb_pci_unregister(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn tgafb_tc_register(dev: *mut device) -> c_int {
    static int tgafb_tc_register(struct device *dev)
    {
    let mut status: c_int = tgafb_register(dev);
    if (!status)
    get_device(dev);
    return status;
    }
#[no_mangle]
unsafe extern "C" fn tgafb_tc_unregister(dev: *mut device) -> c_int {
...
//
    ...
    struct tga_par *par = (struct tga_par *) info.par;
    let mut tga_bus_pci: c_int = dev_is_pci(par.dev);
    let mut tga_bus_tc: c_int = TGA_BUS_TC(par.dev);
...
// Finally, enable video scan (and pray for the monitor... :-)
    ...
    shift = 0;
#[no_mangle]
pub unsafe extern "C" fn if(4: f >= TGA_PLL_MAX_FREQ /) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(200000: f <=) -> else {
...
//
    ...
    if (regno < 16) {
    let mut value: u32 = (regno << 16) | (regno << 8) | regno;
...
// The image data is bit big endian; we need
    ...
    let mut pos0: c_ulong = pos;
    const unsigned char *data0 = data;
    let mut bincr: c_ulong = (is8bpp ? 8 : 8*4);
    unsigned long bwidth;
...
// Finally, handle the generic case of misaligned start.
    ...
    bwidth = (width & 15) > 8;
    for (i = 0; i < height; ++i) {
    let mut mask: u32 = bitrev8(data[0]);
    if (bwidth)
...
//
    ...
    {
    let mut is8bpp: c_uint = info.var.bits_per_pixel == 8;
...
//
    ...
    {
    struct tga_par *par = (struct tga_par *) info.par;
    let mut is8bpp: c_int = info.var.bits_per_pixel == 8;
...
// The write into the frame buffer must be aligned to 4 bytes,
    ...
    let mut Bpp: c_ulong = (is8bpp ? 1 : 4);
    let mut nwidth: c_ulong = width & -2048;
    u32 fdata, ldata;
    fdata = (2048 - 1) | align;
...
#[no_mangle]
pub unsafe extern "C" fn if(32: bpp ==) -> else {
    else if (bpp == 32)
    cfb_copyarea(info, area);
    else
...
//
    ...
    let mut tga_bus_pci: c_int = dev_is_pci(par.dev);
    let mut tga_bus_tc: c_int = TGA_BUS_TC(par.dev);
    let mut tga_type: u8 = par.tga_type;
    const char *tga_type_name = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn tgafb_pan_display(var: *mut fb_var_screeninfo, info: *mut fb_info) -> c_int {
    static int tgafb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
    {
...
#[no_mangle]
unsafe extern "C" fn tgafb_register(dev: *mut device) -> c_int {
    static int tgafb_register(struct device *dev)
    {
...
// 1280x1024 @ 72 Hz, 76.8 kHz hsync
    ...
    const struct fb_videomode *modedb_tga = core::ptr::null_mut();
    let mut bar0_start: resource_size_t = 0, bar0_len = 0;
    const char *mode_option_tga = core::ptr::null_mut();
    let mut tga_bus_pci: c_int = dev_is_pci(dev);
    let mut tga_bus_tc: c_int = TGA_BUS_TC(dev);
    let mut modedbsize_tga: c_uint = 0;
...
// This should give a reasonable default video mode.
    ...
    modedb_tga, modedbsize_tga, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn tgafb_unregister(dev: *mut device) {
    static void tgafb_unregister(struct device *dev)
    {
    let mut bar0_start: resource_size_t = 0, bar0_len = 0;
    let mut tga_bus_pci: c_int = dev_is_pci(dev);
    let mut tga_bus_tc: c_int = TGA_BUS_TC(dev);
    struct fb_info *info = core::ptr::null_mut();
    struct tga_par *par;
...
#[no_mangle]
unsafe extern "C" fn tgafb_setup(arg: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tgafb_init() -> c_int {
