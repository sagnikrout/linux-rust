...
//! Source: drivers/video/fbdev/udlfb.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_lfsr16(actual_count: u16) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn dlfb_vm_open(vma: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_vm_close(vma: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_ops_mmap(info: *mut fb_info, vma: *mut vm_area_struct) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_trim_hline(bback: *const u8, bfront: *const u8, width_bytes: *mut c_int) -> c_int {
    static int dlfb_trim_hline(const u8 *bback, const u8 **bfront, int *width_bytes)
    {
    int j, k;
    const unsigned long *back = (const unsigned long *) bback;
    const unsigned long *front = (const unsigned long *) *bfront;
    let mut width: c_int = *width_bytes / sizeof(unsigned long);
    int identical;
    let mut start: c_int = width;
    let mut end: c_int = width;
    for (j = 0; j < width; j++) {
    if (back[j] != front[j]) {
    start = j;
    break;
...
//
    ...
    let mut dev_addr: u32 = *device_address_ptr;
    uint8_t *cmd = *command_buffer_ptr;
    while ((pixel_end > pixel) &&
    (cmd_buffer_end - MIN_RLX_CMD_BYTES > cmd)) {
    uint8_t *raw_pixels_count_byte = core::ptr::null_mut();
    uint8_t *cmd_pixels_count_byte = core::ptr::null_mut();
    const uint16_t *raw_pixel_start = core::ptr::null_mut();
    const uint16_t *cmd_pixel_start, *cmd_pixel_end = core::ptr::null_mut();
...
// (cmd_pixel_end - 1) == *(u16 *)((u8 *)(cmd_pixel_end - 1) + back_buffer_offset))
    ...
    let mut repeating_pixel: *const uint16_t  const = pixel;
    let mut pixel_value: u16 = *pixel;
...
//
    ...
    let mut dev_addr: u32 = dlfb.base16 + byte_offset;
    struct urb *urb = *urb_ptr;
    u8 *cmd = *urb_buf_ptr;
    u8 *cmd_end = (u8 *) urb.transfer_buffer + urb.transfer_buffer_length;
    let mut back_buffer_offset: c_ulong = 0;
...
// ident_ptr += dlfb_trim_hline(back_start, &next_pixel,
    ...
    ident_ptr);
    if (cmd >= cmd_end) {
    let mut len: c_int = cmd - (u8 *) urb.transfer_buffer;
...
#[no_mangle]
unsafe extern "C" fn dlfb_handle_damage(dlfb: *mut dlfb_data, x: c_int, y: c_int, width: c_int, height: c_int) -> c_int {
    ...
    let mut bytes_identical: c_int = 0;
    struct urb *urb;
    int aligned_x;
    ...
    let mut line_offset: c_int = dlfb.info.fix.line_length * i;
...
#[no_mangle]
unsafe extern "C" fn dlfb_init_damage(dlfb: *mut dlfb_data) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_damage_work(w: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_offload_damage(dlfb: *mut dlfb_data, x: c_int, y: c_int, width: c_int, height: c_int) {
    static void dlfb_offload_damage(struct dlfb_data *dlfb, int x, int y, int width, int height)
    {
    unsigned long flags;
    let mut x2: c_int = x + width;
    let mut y2: c_int = y + height;
    if (x >= x2 || y >= y2)
    return;
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_dpy_deferred_io(info: *mut fb_info, pagereflist: *mut list_head) {
    ...
    let mut bytes_sent: c_int = 0;
    let mut bytes_identical: c_int = 0;
    let mut bytes_rendered: c_int = 0;
    mutex_lock(&dlfb.render_mutex);
    if (!fb_defio)
    goto unlock_ret;
    if (!atomic_read(&dlfb.usb_active))
...
#[no_mangle]
unsafe extern "C" fn dlfb_get_edid(dlfb: *mut dlfb_data, edid: *mut c_char, len: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_ops_open(info: *mut fb_info, user: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dlfb_ops_destroy(info: *mut fb_info) {
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_ops_release(info: *mut fb_info, user: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_is_valid_mode(mode: *mut fb_videomode, dlfb: *mut dlfb_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dlfb_var_color_format(var: *mut fb_var_screeninfo) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_ops_set_par(info: *mut fb_info) -> c_int {
    ...
    let mut line_length: u32 = info.var.xres * (info.var.bits_per_pixel / 8);
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_ops_blank(blank_mode: c_int, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dlfb_ops_damage_range(info: *mut fb_info, off: off_t, len: usize) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_ops_damage_area(info: *mut fb_info, x: u32, y: u32, width: u32, height: u32) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_deferred_vfree(dlfb: *mut dlfb_data, mem: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn dlfb_realloc_framebuffer(dlfb: *mut dlfb_data, info: *mut fb_info, new_len: u32) -> c_int {
    static int dlfb_realloc_framebuffer(struct dlfb_data *dlfb, struct fb_info *info, u32 new_len)
    {
    let mut old_len: u32 = info.fix.smem_len;
    const void *old_fb = info.screen_buffer;
    unsigned char *new_fb;
    unsigned char *new_back = core::ptr::null_mut();
...
// If we have good mode and no active clients
    if ((default_vmode != core::ptr::null_mut()) && (dlfb.fb_count == 0)) {
...
//
    ...
    struct fb_info *fb_info = dev_get_drvdata(fbdev);
    struct dlfb_data *dlfb = fb_info.par;
    if (dlfb.edid == core::ptr::null_mut())
    return 0;
...
// We only support write of entire EDID at once, no offset
    ...
    __ATTR(metrics_reset, S_IWUSR, core::ptr::null_mut(), metrics_reset_store),
    };
//
// This is necessary before we can communicate with the display controller.
//
#[no_mangle]
unsafe extern "C" fn dlfb_select_std_channel(dlfb: *mut dlfb_data) -> c_int {
...
// We don't register a new USB class. Our client interface is dlfbev
    ...
    retval = dlfb_setup_modes(dlfb, info, core::ptr::null_mut(), 0);
    if (retval != 0) {
    dev_err(info.device,
...
#[no_mangle]
unsafe extern "C" fn dlfb_usb_disconnect(intf: *mut usb_interface) {
...
#[no_mangle]
unsafe extern "C" fn dlfb_urb_completion(urb: *mut urb) {
    static void dlfb_urb_completion(struct urb *urb)
    {
...
#[no_mangle]
unsafe extern "C" fn dlfb_free_urb_list(dlfb: *mut dlfb_data) {
    static void dlfb_free_urb_list(struct dlfb_data *dlfb)
    {
    let mut count: c_int = dlfb.urbs.count;
...
#[no_mangle]
unsafe extern "C" fn dlfb_alloc_urb_list(dlfb: *mut dlfb_data, count: c_int, size: usize) -> c_int {
    static int dlfb_alloc_urb_list(struct dlfb_data *dlfb, int count, size_t size)
    {
    struct urb *urb;
    struct urb_node *unode;
    char *buf;
    let mut wanted_size: usize = count * size;
...
#[no_mangle]
unsafe extern "C" fn dlfb_submit_urb(dlfb: *mut dlfb_data, urb: *mut urb, len: usize) -> c_int {
    static int dlfb_submit_urb(struct dlfb_data *dlfb, struct urb *urb, size_t len)
    {
    int ret;
