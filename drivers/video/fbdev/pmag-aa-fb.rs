...
//! Source: drivers/video/fbdev/pmag-aa-fb.c
#![no_std]
...
//
pub const PMAG_AA_BT455_OFFSET: c_uint = 0x100000;
//
// Bt431 cursor generator offset (rel. to TC slot base address).
//
pub const PMAG_AA_BT431_OFFSET: c_uint = 0x180000;
//
...
//
pub const PMAG_AA_ONBOARD_FBMEM_OFFSET: c_uint = 0x200000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct aafb_par {
    pub mmio: *mut void __iomem,
    pub bt455: *mut bt455_regs __iomem,
    pub bt431: *mut bt431_regs __iomem,
}
...
#[no_mangle]
unsafe extern "C" fn aafb_cursor(info: *mut fb_info, cursor: *mut fb_cursor) -> c_int {
    ...
    let mut fg: u8 = cursor.image.fg_color ? 0xf : 0x0;
    let mut bg: u8 = cursor.image.bg_color ? 0xf : 0x0;
...
#[no_mangle]
unsafe extern "C" fn aafb_blank(blank: c_int, info: *mut fb_info) -> c_int {
    static int aafb_blank(int blank, struct fb_info *info)
    {
    struct aafb_par *par = info.par;
    let mut val: u8 = blank ? 0x00 : 0x0f;
...
#[no_mangle]
unsafe extern "C" fn pmagaafb_probe(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pmagaafb_remove(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pmagaafb_init() -> int __init {
    static int __init pmagaafb_init(void)
    {

    if (fb_get_options("pmagaafb", core::ptr::null_mut()))
    return -ENXIO;

    return tc_register_driver(&pmagaafb_driver);
    }
#[no_mangle]
unsafe extern "C" fn pmagaafb_exit() -> void __exit {
