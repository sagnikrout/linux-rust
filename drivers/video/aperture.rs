...
//! Source: drivers/video/aperture.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct aperture_range {
    pub dev: *mut device,
    pub base: resource_size_t,
    pub size: resource_size_t,
    pub lh: list_head,
    pub dev): *mut *mut void (detach)(struct device,
}
...
#[no_mangle]
unsafe extern "C" fn devm_aperture_acquire_release(data: *mut c_void) {
    static void devm_aperture_acquire_release(void *data)
    {
    struct aperture_range *ap = data;
    let mut detached: bool = !ap.dev;
    if (detached)
    return;
...
#[no_mangle]
unsafe extern "C" fn aperture_detach_platform_device(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn aperture_detach_devices(base: resource_size_t, size: resource_size_t) {
    static void aperture_detach_devices(resource_size_t base, resource_size_t size)
    {
    let mut end: resource_size_t = base + size;
    ...
    if (!overlap(base, end, ap.base, ap.base + ap.size))
    continue;
    ap.dev = core::ptr::null_mut(); /* detach from device */
    list_del(&ap.lh);
    ap.detach(dev);
    }
    mutex_unlock(&apertures_lock);
...
//
#[no_mangle]
pub unsafe extern "C" fn __aperture_remove_legacy_vga_devices(pdev: *mut pci_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn aperture_remove_conflicting_pci_devices(pdev: *mut pci_dev, name: *const c_char) -> c_int {
