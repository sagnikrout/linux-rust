...
//! Source: drivers/video/console/vgacon.c
#![no_std]
...
// Description of the hardware situation
    ...
    let mut vga_hardscroll_user_enable: static bool = true;
#[no_mangle]
unsafe extern "C" fn no_scroll(str: *mut c_char) -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn write_vga(reg: c_uchar, val: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn vga_set_mem_top(c: *mut vc_data) {
    static inline void vga_set_mem_top(struct vc_data *c)
    {
    write_vga(12, (c.vc_visible_origin - vga_vram_base) / 2);
    }
#[no_mangle]
unsafe extern "C" fn vgacon_scrolldelta(c: *mut vc_data, lines: c_int) {
    static void vgacon_scrolldelta(struct vc_data *c, int lines)
    {
    let mut scr_end: c_ulong = c.vc_scr_end - vga_vram_base;
    let mut vorigin: c_ulong = c.vc_visible_origin - vga_vram_base;
    let mut origin: c_ulong = c.vc_origin - vga_vram_base;
    let mut margin: c_int = c.vc_size_row * 4;
...
#[no_mangle]
unsafe extern "C" fn vgacon_restore_screen(c: *mut vc_data) {
...
// VGA16 modes are not handled by VGACON
    ...
    vgastate.vgabase = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn vgacon_init(c: *mut vc_data, init: bool) {
...
#[no_mangle]
unsafe extern "C" fn vgacon_deinit(c: *mut vc_data) {
...
// When closing the active console, reset video origin
    ...
    bool italic)
    {
    let mut attr: u8 = color;
    if (vga_can_do_color) {
    if (italic)
    attr = (attr & 0xF0) | c.vc_itcolor;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: underline) -> else {
    else if (underline)
    attr = (attr & 0xf0) | c.vc_ulcolor;
#[no_mangle]
pub unsafe extern "C" fn if(VCI_HALF_BRIGHT: intensity ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn vgacon_invert_region(c: *mut vc_data, p: *mut *mut u16, count: c_int) {
    static void vgacon_invert_region(struct vc_data *c, u16 * p, int count)
    {
    let mut col: bool = vga_can_do_color;
    while (count--) {
    let mut a: u16 = scr_readw(p);
    if (col)
...
#[no_mangle]
unsafe extern "C" fn vgacon_set_cursor_size(from: c_int, to: c_int) {
...
#[no_mangle]
unsafe extern "C" fn vgacon_cursor(c: *mut vc_data, enable: bool) {
    ...
    {
    unsigned long flags;
    let mut scanlines: c_uint = height * c.vc_cell_height;
    let mut scanlines_lo: u8 = 0, r7 = 0, vsync_end = 0, mode, max_scan;
    raw_spin_lock_irqsave(&vga_lock, flags);
...
#[no_mangle]
unsafe extern "C" fn vgacon_switch(c: *mut vc_data) -> bool {
    static bool vgacon_switch(struct vc_data *c)
    {
    let mut x: c_int = c.vc_cols * VGA_FONTWIDTH;
    let mut y: c_int = c.vc_rows * c.vc_cell_height;
...
#[no_mangle]
unsafe extern "C" fn vga_set_palette(vc: *mut vc_data, table: *const c_uchar) {
...
#[no_mangle]
unsafe extern "C" fn vgacon_set_palette(vc: *mut vc_data, table: *const c_uchar) {
...
#[no_mangle]
unsafe extern "C" fn vga_vesa_blank(state: *mut vgastate, mode: enum vesa_blank_mode) {
...
#[no_mangle]
unsafe extern "C" fn vga_vesa_unblank(state: *mut vgastate) {
...
#[no_mangle]
unsafe extern "C" fn vga_pal_blank(state: *mut vgastate) {
...
//
pub const colourmap: c_uint = 0xa0000;
// Pauline Middelink <middelin@polyware.iaf.nl> reports that we
    should use 0xA0000 for the bwmap as well.. */
pub const blackwmap: c_uint = 0xa0000;
pub const cmapsz: c_int = 8192;
    static int vgacon_do_font_op(struct vgastate *state, char *arg, int set,
    bool ch512)
    {
    let mut video_port_status: c_ushort = vga_video_port_reg + 6;
    let mut font_select: c_int = 0x00, beg, i;
    char *charmap;
    let mut clear_attribs: bool = false;
...
//
#[no_mangle]
unsafe extern "C" fn vgacon_adjust_height(vc: *mut vc_data, fontheight: unsigned) -> c_int {
...
// void size to cause regs to be rewritten
    ...
    {
    let mut charcount: unsigned = font.charcount;
    int rc;
    if (vga_video_type < VIDEO_TYPE_EGAM)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn vgacon_font_get(c: *mut vc_data, font: *mut console_font, vpitch: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vgacon_set_origin(c: *mut vc_data) -> bool {
    static bool vgacon_set_origin(struct vc_data *c)
    {
...
#[no_mangle]
unsafe extern "C" fn vgacon_save_screen(c: *mut vc_data) {
    static void vgacon_save_screen(struct vc_data *c)
    {
    let mut vga_bootup_console: static int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn vgacon_register_screen(si: *mut screen_info) {
