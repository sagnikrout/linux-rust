...
//! Source: drivers/gpu/tests/gpu_buddy_test.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn get_size(order: c_int, chunk_size: u64) -> u64 {
    static inline u64 get_size(int order, u64 chunk_size)
    {
    return (1 << order) * chunk_size;
    }
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_subtree_offset_alignment_stress(test: *mut kunit) {
    static void gpu_test_buddy_subtree_offset_alignment_stress(struct kunit *test)
    {
    struct gpu_buddy_block *block;
    struct rb_node *node = core::ptr::null_mut();
    let mut mm_size: u64 = SZ_2M;
...
//
    ...
    struct gpu_buddy_block *root = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_offset_aligned_allocation(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_fragmentation_performance(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_range_bias(test: *mut kunit) {
...
//
    ...
    do {
    let mut size: u32 = max(round_up(prandom_u32_state(&prng) % bias_rem, ps), ps);
    KUNIT_ASSERT_FALSE_MSG(test,
    gpu_buddy_alloc_blocks(&mm, bias_start,
    bias_end, size, ps,
    &allocated,
...
//
    do {
    let mut old_bias_start: u32 = bias_start;
    let mut old_bias_end: u32 = bias_end;
    if (bias_start)
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_range(test: *mut kunit) {
...
//
    ...
    let mut offset: u64 = gpu_buddy_block_offset(block);
    let mut bsize: u64 = gpu_buddy_block_size(&mm, block);
...
//
    ...
    {
    let mut quarter: u32 = mm_size / 4;
    int i;
    for (i = 0; i < 4; i++) {
    let mut start: u32 = i * quarter;
    let mut end: u32 = start + quarter;
...
//
    ...
    KUNIT_ASSERT_FALSE(test, gpu_buddy_init(&mm, mm_size, ps));
    {
pub const N_RAND_RANGES: c_int = 16;
    u32 ranges[N_RAND_RANGES + 1]; /* boundaries */
    u32 order_arr[N_RAND_RANGES];
    let mut remaining: u32 = mm_size;
    int i;
    ranges[0] = 0;
    for (i = 0; i < N_RAND_RANGES - 1; i++) {
    let mut max_chunk: u32 = remaining - (N_RAND_RANGES - 1 - i) * ps;
    let mut sz: u32 = max(round_up(prandom_u32_state(&prng) % max_chunk, ps), ps);
...
// Create a random order
    ...
    let mut j: u32 = prandom_u32_state(&prng) % (i + 1);
    let mut tmp_val: u32 = order_arr[i];
    order_arr[i] = order_arr[j];
    order_arr[j] = tmp_val;
    }
    for (i = 0; i < N_RAND_RANGES; i++) {
    let mut idx: u32 = order_arr[i];
    let mut start: u32 = ranges[idx];
    let mut end: u32 = ranges[idx + 1];
    let mut sz: u32 = end - start;
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_clear(test: *mut kunit) {
    static void gpu_test_buddy_alloc_clear(struct kunit *test)
    {
    unsigned long n_pages, total, i = 0;
    let mut ps: c_ulong = SZ_4K;
    struct gpu_buddy_block *block;
    let mut max_order: c_int = 12;
    LIST_HEAD(allocated);
    struct gpu_buddy mm;
    unsigned int order;
    u32 mm_size, size;
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_contiguous(test: *mut kunit) {
    static void gpu_test_buddy_alloc_contiguous(struct kunit *test)
    {
    let mut ps: c_ulong = SZ_4K, mm_size = 16 * 3 * SZ_4K;
...
//
    ...
    do {
    struct list_head *list;
    let mut slot: c_int = i % 3;
    if (slot == 0)
    list = &left;
#[no_mangle]
pub unsafe extern "C" fn if(1: slot ==) -> else {
    else if (slot == 1)
    list = &middle;
    else
    list = &right;
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_pathological(test: *mut kunit) {
    static void gpu_test_buddy_alloc_pathological(struct kunit *test)
    {
    u64 mm_size, size, start = 0;
    struct gpu_buddy_block *block;
    let mut max_order: c_int = 3;
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_pessimistic(test: *mut kunit) {
    static void gpu_test_buddy_alloc_pessimistic(struct kunit *test)
    {
    u64 mm_size, size, start = 0;
    struct gpu_buddy_block *block, *bn;
    let mut max_order: c_uint = 16;
    let mut flags: c_ulong = 0;
    struct gpu_buddy mm;
    unsigned int order;
    LIST_HEAD(blocks);
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_optimistic(test: *mut kunit) {
    static void gpu_test_buddy_alloc_optimistic(struct kunit *test)
    {
    u64 mm_size, size, start = 0;
    struct gpu_buddy_block *block;
    let mut flags: c_ulong = 0;
    let mut max_order: c_int = 16;
    struct gpu_buddy mm;
    LIST_HEAD(blocks);
    LIST_HEAD(tmp);
    int order;
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_limit(test: *mut kunit) {
    static void gpu_test_buddy_alloc_limit(struct kunit *test)
    {
    let mut size: u64 = U64_MAX, start = 0;
    struct gpu_buddy_block *block;
    let mut flags: c_ulong = 0;
    LIST_HEAD(allocated);
    struct gpu_buddy mm;
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_alloc_exceeds_max_order(test: *mut kunit) {
    static void gpu_test_buddy_alloc_exceeds_max_order(struct kunit *test)
    {
    let mut mm_size: u64 = SZ_8G + SZ_2G, size = SZ_8G + SZ_1G, min_block_size = SZ_8G;
    struct gpu_buddy mm;
    LIST_HEAD(blocks);
    int err;
    KUNIT_ASSERT_FALSE_MSG(test, gpu_buddy_init(&mm, mm_size, SZ_4K),
...
#[no_mangle]
unsafe extern "C" fn gpu_test_buddy_addr_to_block(test: *mut kunit) {
    ...
    let mut test_size: u64 = SZ_4M + SZ_2M;
    let mut alloc_start: u64 = SZ_4M;
    let mut alloc_size: u64 = SZ_4K;
    let mut chunk_size: u64 = SZ_4K;
    struct gpu_buddy mm;
...
#[no_mangle]
unsafe extern "C" fn gpu_buddy_suite_init(suite: *mut kunit_suite) -> c_int {
