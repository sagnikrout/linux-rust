...
#[no_mangle]
unsafe extern "C" fn gpu_buddy_block_offset_alignment(block: *mut gpu_buddy_block) -> c_uint {
...
//
    ...
    {
    return node ? rb_entry(node, struct gpu_buddy_block, rb) : core::ptr::null_mut();
    }
    static struct gpu_buddy_block *
...
#[no_mangle]
unsafe extern "C" fn rbtree_is_empty(root: *mut rb_root) -> bool {
...
//
    ...
    struct gpu_buddy_block *block)
    {
    let mut order: c_uint = gpu_buddy_block_order(block);
    enum gpu_buddy_free_tree tree;
    struct rb_root *root;
...
#[no_mangle]
unsafe extern "C" fn clear_reset(block: *mut gpu_buddy_block) {
    static void clear_reset(struct gpu_buddy_block *block)
    {
    block.header &= ~GPU_BUDDY_HEADER_CLEAR;
    }
#[no_mangle]
unsafe extern "C" fn mark_cleared(block: *mut gpu_buddy_block) {
...
#[no_mangle]
pub unsafe extern "C" fn overlaps(s1: u64, e1: u64, s2: u64, e2: u64) -> bool {
    static inline bool overlaps(u64 s1, u64 e1, u64 s2, u64 e2)
    {
    return s1 <= e2 && e1 >= s2;
    }
#[no_mangle]
pub unsafe extern "C" fn contains(s1: u64, e1: u64, s2: u64, e2: u64) -> bool {
...
//
    ...
    static int _merge(struct gpu_buddy *mm,
    u64 start,
    u64 end,
...
//
#[no_mangle]
pub unsafe extern "C" fn gpu_buddy_init(mm: *mut gpu_buddy, size: u64, chunk_size: u64) -> c_int {
...
//
    ...
    root = gpu_block_alloc(mm, core::ptr::null_mut(), order, offset);
    if (!root)
    goto out_free_roots;
    mark_free(mm, root);
    ...
    mm.lock_dep_map = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn gpu_buddy_fini(mm: *mut gpu_buddy) {
    ...
    _merge(mm, start, start + size, order);
    ...
    struct gpu_buddy_block *block)
    {
    let mut block_order: c_uint = gpu_buddy_block_order(block) - 1;
    let mut offset: u64 = gpu_buddy_block_offset(block);
...
//
#[no_mangle]
pub unsafe extern "C" fn gpu_buddy_reset_clear(mm: *mut gpu_buddy, is_clear: bool) {
...
//
    ...
    return block;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: gpu_buddy_block_is_free(block)) -> else {
    ...
    mark_cleared(block);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mark_dirty) -> else {
    else if (mark_dirty)
    clear_reset(block);
...
//
    ...
    unsigned int flags)
    {
    let mut mark_clear: bool = flags & GPU_BUDDY_CLEARED;
    gpu_buddy_driver_lock_held(mm);
    __gpu_buddy_free_list(mm, objects, mark_clear, !mark_clear);
    }
    EXPORT_SYMBOL(gpu_buddy_free_list);
#[no_mangle]
unsafe extern "C" fn block_incompatible(block: *mut gpu_buddy_block, flags: c_uint) -> bool {
    static bool block_incompatible(struct gpu_buddy_block *block, unsigned int flags)
    {
    let mut needs_clear: bool = flags & GPU_BUDDY_CLEAR_ALLOCATION;
    ...
    bool fallback)
    {
    let mut req_size: u64 = mm.chunk_size << order;
    struct gpu_buddy_block *block;
    LIST_HEAD(dfs);
    int err;
    int i;
    ...
    let mut adjusted_start: u64 = max(block_start, start);
    let mut adjusted_end: u64 = min(block_end, end);
...
//
    ...
    enum gpu_buddy_free_tree tree)
    {
    struct gpu_buddy_block *max_block = core::ptr::null_mut(), *block = core::ptr::null_mut();
    struct rb_root *root;
    unsigned int i;
    ...
    unsigned long flags)
    {
    struct gpu_buddy_block *block = core::ptr::null_mut();
    struct rb_root *root;
    enum gpu_buddy_free_tree tree;
    unsigned int tmp;
    int err;
...
// total_allocated_on_err = total_allocated;
    ...
    struct list_head *blocks)
    {
    let mut aligned_offset: u64 = round_down(unaligned_offset, min_block_size);
    return __gpu_buddy_alloc_range(mm, aligned_offset, size, core::ptr::null_mut(), blocks);
    }
...
// Prevent recursively freeing this node
    parent = block.parent;
    block.parent = core::ptr::null_mut();
    list_add(&block.tmp_link, &dfs);
    err =  __alloc_range(mm, &dfs, new_start, new_size, blocks, core::ptr::null_mut());
    if (err) {
    mark_allocated(mm, block);
    mm.avail -= gpu_buddy_block_size(mm, block);
    if (gpu_buddy_block_is_clear(block))
...
// Allocate traversing within the range
    ...
    order, flags);
#[no_mangle]
pub unsafe extern "C" fn if(min_block_size: size <) -> else {
...
// Actual range allocation
    ...
    return -EINVAL;
    return __gpu_buddy_alloc_range(mm, start, size, core::ptr::null_mut(), blocks);
    }
    original_size = size;
...
// Try allocation through force merge method
    if (mm.clear_avail &&
    !_merge(mm, start, end, fallback_order)) {
    block = __gpu_buddy_alloc_blocks(mm, start,
    end,
    size,
    min_block_size,
...
//
#[no_mangle]
pub unsafe extern "C" fn gpu_buddy_print(mm: *mut gpu_buddy) {
...
#[no_mangle]
unsafe extern "C" fn gpu_buddy_module_exit() {
    static void gpu_buddy_module_exit(void)
    {
    kmem_cache_destroy(slab_blocks);
    }
#[no_mangle]
unsafe extern "C" fn gpu_buddy_module_init() -> int __init {
