...
//! Source: drivers/gpu/drm/arm/malidp_planes.c
#![no_std]
...
pub const MALIDP_LAYER_FORMAT: c_uint = 0x000;
pub const LAYER_FORMAT_MASK: c_uint = 0x3f;
pub const MALIDP_LAYER_CONTROL: c_uint = 0x004;

pub const LAYER_FLOWCFG_MASK: c_int = 7;

pub const LAYER_FLOWCFG_SCALE_SE: c_int = 3;
pub const LAYER_ROT_OFFSET: c_int = 8;

pub const MALIDP_LAYER_COMPOSE: c_uint = 0x008;
pub const MALIDP_LAYER_SIZE: c_uint = 0x00c;

pub const MALIDP_LAYER_COMP_SIZE: c_uint = 0x010;
pub const MALIDP_LAYER_OFFSET: c_uint = 0x014;
pub const MALIDP550_LS_ENABLE: c_uint = 0x01c;
pub const MALIDP550_LS_R1_IN_SIZE: c_uint = 0x020;
pub const MODIFIERS_COUNT_MAX: c_int = 15;
//
...
//
pub const MALIDP_ALPHA_LUT: c_uint = 0xffaa5500;
// page sizes the MMU prefetcher can support

// readahead for partial-frame prefetch
pub const MALIDP_MMU_PREFETCH_READAHEAD: c_int = 8;
//
...
//
#[no_mangle]
unsafe extern "C" fn malidp_plane_reset(plane: *mut drm_plane) {
...
#[no_mangle]
unsafe extern "C" fn malidp_get_pgsize_bitmap(mp: *mut malidp_plane) -> u32 {
...
// get the full-frame prefetch page size(s) supported by the MMU
    ...
    while (pgsizes) {
    let mut largest_pgsize: u32 = 1 << __fls(pgsizes);
...
// get the page sizes supported by the MMU
    ...
    let mut rotated: bool = new_plane_state.rotation & MALIDP_ROTATED_MASK;
    struct drm_framebuffer *fb;
    let mut pixel_alpha: u16 = new_plane_state.pixel_blend_mode;
    int i, ret;
    ...
    for (i = 0; i < ms.n_planes; i++) {
    let mut alignment: u8 = malidp_hw_get_pitch_align(mp.hwdev, rotated);
...
// HW can't support plane + pixel blending
    ...
    {
    int i;
    let mut num_strides: c_int = num_planes;
...
//
    ...
    let mut block_h: c_uint = drm_format_info_block_height(mp.base.state.fb.format, i);
...
// check hardware supports MMU prefetch
    ...
    let mut afbc: bool = fb.modifier ? true : false;
...
#[no_mangle]
unsafe extern "C" fn malidp_de_set_plane_afbc(plane: *mut drm_plane) {
...
// convert src values from Q16 fixed point to integer
    ...
    plane);
    let mut pixel_alpha: u16 = new_state.pixel_blend_mode;
    let mut plane_alpha: u8 = new_state.alpha >> 8;
...
#[no_mangle]
pub unsafe extern "C" fn malidp_de_planes_init(drm: *mut drm_device) -> c_int {
    int malidp_de_planes_init(struct drm_device *drm)
    {
    struct malidp_drm *malidp = drm_to_malidp(drm);
    const struct malidp_hw_regmap *map = &malidp.dev.hw.map;
    struct malidp_plane *plane = core::ptr::null_mut();
    enum drm_plane_type plane_type;
    let mut crtcs: c_ulong = BIT(drm.mode_config.num_crtc);
...
//
    ...
    modifiers, plane_type, core::ptr::null_mut());
    if (IS_ERR(plane)) {
    ret = PTR_ERR(plane);
    goto cleanup;
    }
...
// default encoding for YUV->RGB is BT601 NARROW
    let mut enc: enum drm_color_encoding = DRM_COLOR_YCBCR_BT601;
    let mut range: enum drm_color_range = DRM_COLOR_YCBCR_LIMITED_RANGE;
