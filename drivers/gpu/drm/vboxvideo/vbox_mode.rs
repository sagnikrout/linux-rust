...
//! Source: drivers/gpu/drm/vboxvideo/vbox_mode.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn vbox_do_modeset(crtc: *mut drm_crtc) {
...
#[no_mangle]
unsafe extern "C" fn vbox_set_view(crtc: *mut drm_crtc) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vbox_set_up_input_mapping(vbox: *mut vbox_private) -> bool {
    static bool vbox_set_up_input_mapping(struct vbox_private *vbox)
    {
    struct drm_crtc *crtci;
    struct drm_connector *connectori;
    struct drm_framebuffer *fb, *fb1 = core::ptr::null_mut();
    let mut single_framebuffer: bool = true;
    let mut old_single_framebuffer: bool = vbox.single_framebuffer;
    let mut width: u16 = 0, height = 0;
//
...
// Otherwise calculate the total span of all screens.
    ...
    struct vbox_private *vbox = to_vbox_dev(crtc.dev);
    struct vbox_crtc *vbox_crtc = to_vbox_crtc(crtc);
    let mut needs_modeset: bool = drm_atomic_crtc_needs_modeset(crtc.state);
...
#[no_mangle]
unsafe extern "C" fn vbox_crtc_destroy(crtc: *mut drm_crtc) {
...
// Send information about dirty rectangles to VBVA.
    ...
    let mut crtc_id: c_uint = to_vbox_crtc(crtc).crtc_id;
...
// vbox_do_modeset checks plane->state->fb and will disable if NULL
    ...
    plane);
    struct drm_crtc_state *crtc_state = core::ptr::null_mut();
    let mut width: u32 = new_state.crtc_w;
    let mut height: u32 = new_state.crtc_h;
    int ret;
    if (new_state.crtc) {
    crtc_state = drm_atomic_get_new_crtc_state(state,
    new_state.crtc);
...
//
    ...
    size_t mask_size)
    {
    let mut line_size: usize = (width + 7) / 8;
    u32 i, j;
...
//
    ...
    struct drm_plane *cursor = core::ptr::null_mut();
    ...
    &vbox_crtc_funcs, core::ptr::null_mut());
    if (ret)
    goto clean_cursor;
...
#[no_mangle]
unsafe extern "C" fn vbox_encoder_destroy(encoder: *mut drm_encoder) {
...
// descriptor block 4: dummy data
    ...
    let mut clock: c_int = (width + 6) * (height + 6) * 60 / 10000;
...
#[no_mangle]
unsafe extern "C" fn vbox_get_modes(connector: *mut drm_connector) -> c_int {
    static int vbox_get_modes(struct drm_connector *connector)
    {
    struct vbox_connector *vbox_connector = core::ptr::null_mut();
    struct drm_display_mode *mode = core::ptr::null_mut();
    struct vbox_private *vbox = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn vbox_connector_destroy(connector: *mut drm_connector) {
...
#[no_mangle]
pub unsafe extern "C" fn vbox_mode_init(vbox: *mut vbox_private) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn vbox_mode_fini(vbox: *mut vbox_private) {
