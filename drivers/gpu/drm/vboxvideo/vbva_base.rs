...
//! Source: drivers/gpu/drm/vboxvideo/vbva_base.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn vbva_buffer_available(vbva: *const vbva_buffer) -> u32 {
    static u32 vbva_buffer_available(const struct vbva_buffer *vbva)
    {
    let mut diff: i32 = vbva.data_offset - vbva.free_offset;
    ...
    let mut bytes_till_boundary: u32 = vbva.data_len - offset;
    u8 *dst = &vbva.data[offset];
    let mut diff: i32 = len - bytes_till_boundary;
    if (diff <= 0) {
...
#[no_mangle]
unsafe extern "C" fn vbva_buffer_flush(ctx: *mut gen_pool) {
    ...
    vbva_ctx.record = core::ptr::null_mut();
    vbva_ctx.vbva = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn vbva_buffer_end_update(vbva_ctx: *mut vbva_buf_ctx) {
