...
//! Source: drivers/gpu/drm/drm_plane.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn drm_num_planes(dev: *mut drm_device) -> c_uint {
...
//
    ...
    blob = drm_property_create_blob(dev, blob_size, core::ptr::null_mut());
    if (IS_ERR(blob))
    return blob;
    blob_data = blob.data;
...
//
#[no_mangle]
unsafe extern "C" fn drm_plane_create_hotspot_properties(plane: *mut drm_plane) -> c_int {
    ...
    let mut format_modifier_count: c_uint = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn drmm_universal_plane_alloc_release(dev: *mut drm_device, ptr: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn drm_plane_register_all(dev: *mut drm_device) -> c_int {
    int drm_plane_register_all(struct drm_device *dev)
    {
    let mut num_planes: c_uint = 0;
    let mut num_zpos: c_uint = 0;
    struct drm_plane *plane;
    let mut ret: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn drm_plane_unregister_all(dev: *mut drm_device) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_plane_cleanup(plane: *mut drm_plane) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_plane_force_disable(plane: *mut drm_plane) {
    ...
    ret = plane.funcs.disable_plane(plane, core::ptr::null_mut());
    if (ret) {
    DRM_ERROR("failed to disable plane with busy fb\n");
    plane.old_fb = core::ptr::null_mut();
    return;
    }
// disconnect the plane from the fb and crtc:
    drm_framebuffer_put(plane.old_fb);
    plane.old_fb = core::ptr::null_mut();
    plane.fb = core::ptr::null_mut();
    plane.crtc = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(drm_lease_held(file_priv: !plane->state && plane->crtc &&, _arg: plane->crtc->base.id)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(plane->fb: !plane->state &&) -> else {
...
//
    ...
    let mut crtc_w: u32 = 0, crtc_h = 0;
    let mut src_w: u32 = 0, src_h = 0;
    let mut ret: c_int = 0;
    BUG_ON(!plane);
    WARN_ON(plane.crtc != crtc && plane.crtc != core::ptr::null_mut());
//
...
//
    ...
    struct drm_crtc *crtc;
    struct drm_plane *plane;
    struct drm_framebuffer *fb = core::ptr::null_mut(), *old_fb;
    struct drm_pending_vblank_event *e = core::ptr::null_mut();
    let mut target_vblank: u32 = page_flip.sequence;
    struct drm_modeset_acquire_ctx ctx;
    let mut ret: c_int = -EINVAL;
    if (!drm_core_check_feature(dev, DRIVER_MODESET))
    return -EOPNOTSUPP;
...
//
    ...
    } else if (crtc.funcs.page_flip == core::ptr::null_mut() ||
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_plane_enable_fb_damage_clips(plane: *mut drm_plane) {
...
//
    ...
    state.fb_damage_clips.data : core::ptr::null_mut());
    }
//
