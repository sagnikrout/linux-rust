...
//! Source: drivers/gpu/drm/i915/gt/selftest_hangcheck.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn hang_init(h: *mut hang, gt: *mut intel_gt) -> c_int {
    ...
    memset(h, 0, sizeof(*h));
    h.gt = gt;
    h.ctx = kernel_context(gt.i915, core::ptr::null_mut());
    if (IS_ERR(h.ctx))
    ...
    h.obj = obj;
    h.batch = vaddr;
    vma = i915_vma_instance(h.obj, vm, core::ptr::null_mut());
    if (IS_ERR(vma)) {
    i915_vm_put(vm);
    return ERR_CAST(vma);
    }
    hws = i915_vma_instance(h.hws, vm, core::ptr::null_mut());
    if (IS_ERR(hws)) {
    i915_vm_put(vm);
    return ERR_CAST(hws);
    }
...
#[no_mangle]
unsafe extern "C" fn hws_seqno(h: *const hang, rq: *const i915_request) -> u32 {
    static u32 hws_seqno(const struct hang *h, const struct i915_request *rq)
    {
    return READ_ONCE(h.seqno[rq.fence.context % (PAGE_SIZE/sizeof(u32))]);
    }
#[no_mangle]
unsafe extern "C" fn hang_fini(h: *mut hang) {
...
#[no_mangle]
unsafe extern "C" fn wait_until_running(h: *mut hang, rq: *mut i915_request) -> bool {
...
#[no_mangle]
unsafe extern "C" fn igt_hang_sanitycheck(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn wait_for_idle(engine: *mut intel_engine_cs) -> bool {
    static bool wait_for_idle(struct intel_engine_cs *engine)
    {
    return wait_for(intel_engine_is_idle(engine), IGT_IDLE_TIMEOUT) == 0;
    }
#[no_mangle]
unsafe extern "C" fn igt_reset_nop(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn igt_reset_nop_engine(arg: *mut c_void) -> c_int {
...
//
    ...
    err = intel_engine_reset(engine, core::ptr::null_mut());
    if (err) {
...
#[no_mangle]
unsafe extern "C" fn igt_reset_fail_engine(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __igt_reset_engine(gt: *mut intel_gt, active: bool) -> c_int {
...
// Check that we can issue an engine reset on an idle engine (no-op)
    ...
    unsigned long count;
    let mut using_guc: bool = intel_engine_uses_guc(engine);
    IGT_TIMEOUT(end_time);
    if (using_guc && !active)
    continue;
...
#[no_mangle]
unsafe extern "C" fn igt_reset_idle_engine(arg: *mut c_void) -> c_int {
    static int igt_reset_idle_engine(void *arg)
    {
    return __igt_reset_engine(arg, false);
    }
#[no_mangle]
unsafe extern "C" fn igt_reset_active_engine(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn active_request_put(rq: *mut i915_request) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn active_engine(work: *mut kthread_work) {
    ...
    let mut idx: c_uint = count++ & (ARRAY_SIZE(rq) - 1);
    ...
    let mut err__: c_int = active_request_put(rq[count]);
    if (err)
...
//
    ...
    return -ENOMEM;
    for_each_engine(engine, gt, id) {
    let mut device: c_ulong = i915_reset_count(global);
    let mut count: c_ulong = 0, reported;
    let mut using_guc: bool = intel_engine_uses_guc(engine);
    IGT_TIMEOUT(end_time);
    if (flags & TEST_ACTIVE) {
    if (!intel_engine_can_store_dword(engine))
...
#[no_mangle]
unsafe extern "C" fn igt_reset_engines(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fake_hangcheck(gt: *mut intel_gt, mask: intel_engine_mask_t) -> u32 {
    static u32 fake_hangcheck(struct intel_gt *gt, intel_engine_mask_t mask)
    {
    let mut count: u32 = i915_reset_count(&gt.i915.gpu_error);
    intel_gt_reset(gt, mask, core::ptr::null_mut());
    return count;
    }
#[no_mangle]
unsafe extern "C" fn igt_reset_wait(arg: *mut c_void) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct evict_vma {
    pub completion: completion,
    pub vma: *mut i915_vma,
}

#[no_mangle]
unsafe extern "C" fn evict_vma(data: *mut c_void) -> c_int {
    static int evict_vma(void *data)
    {
    struct evict_vma *arg = data;
    struct i915_address_space *vm = arg.vma.vm;
    let mut evict: drm_mm_node = arg.vma.node;
    int err;
    complete(&arg.completion);
    mutex_lock(&vm.mutex);
    err = i915_gem_evict_for_node(vm, core::ptr::null_mut(), &evict, 0);
    mutex_unlock(&vm.mutex);
    return err;
    }
#[no_mangle]
unsafe extern "C" fn evict_fence(data: *mut c_void) -> c_int {
...
// Check that we can recover an unbind stuck on a hanging request
    ...
    arg.vma = i915_vma_instance(obj, vm, core::ptr::null_mut());
    if (IS_ERR(arg.vma)) {
    err = PTR_ERR(arg.vma);
...
#[no_mangle]
unsafe extern "C" fn igt_reset_evict_ggtt(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn igt_reset_evict_ppgtt(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn igt_reset_evict_fence(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn igt_reset_queue(arg: *mut c_void) -> c_int {
...
// h.batch = MI_BATCH_BUFFER_END;
    ...
    restore:
    if (using_guc) {
    let mut err2: c_int = intel_selftest_restore_policy(engine, &saved);
    if (err2)
...
#[no_mangle]
unsafe extern "C" fn igt_handle_error(arg: *mut c_void) -> c_int {
...
// Temporarily disable error capture
    ...
    intel_gt_handle_error(gt, engine.mask, 0, core::ptr::null_mut());
    ...
    const char *mode)
    {
    let mut t: *mut tasklet_ const = &engine.sched_engine.tasklet;
    int err;
    ...
    err = __intel_engine_reset_bh(engine, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn igt_reset_engines_atomic(arg: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn intel_hangcheck_live_selftests(i915: *mut drm_i915_private) -> c_int {
