...
//! Source: drivers/gpu/drm/i915/gt/selftest_tlb.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn vma_set_qw(vma: *mut i915_vma, addr: u64, val: u64) {
    ...
    vma = i915_vma_instance(batch, ce.vm, core::ptr::null_mut());
    if (IS_ERR(vma)) {
    err = PTR_ERR(vma);
    goto out;
    }
...
// cs = MI_BATCH_BUFFER_END;
    ...
    let mut size: resource_size_t = SZ_1G;
//
...
//
    ...
    void (*tlbinv)(struct i915_address_space *vm, u64 addr, u64 length))
    {
    let mut ppgtt_size: c_uint = RUNTIME_INFO(gt.i915).ppgtt_size;
...
//
    ...
    va = i915_vma_instance(A, &ppgtt.vm, core::ptr::null_mut());
    if (IS_ERR(va)) {
    err = PTR_ERR(va);
    goto out_vm;
    }
    vb = i915_vma_instance(B, &ppgtt.vm, core::ptr::null_mut());
    if (IS_ERR(vb)) {
    err = PTR_ERR(vb);
    goto out_vm;
    }
...
#[no_mangle]
unsafe extern "C" fn tlbinv_full(vm: *mut i915_address_space, addr: u64, length: u64) {
...
#[no_mangle]
unsafe extern "C" fn invalidate_full(arg: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn intel_tlb_live_selftests(i915: *mut drm_i915_private) -> c_int {
