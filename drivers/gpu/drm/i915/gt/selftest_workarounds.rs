...
//! Source: drivers/gpu/drm/i915/gt/selftest_workarounds.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct wa_lists {
    pub gt_wa_list: i915_wa_list,
    struct {
    pub wa_list: i915_wa_list,
    pub ctx_wa_list: i915_wa_list,
    pub engine: [}; I915_NUM_ENGINES],
}

#[no_mangle]
unsafe extern "C" fn request_add_sync(rq: *mut i915_request, err: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn request_add_spin(rq: *mut i915_request, spin: *mut igt_spinner) -> c_int {
    ...
    let mut base: u32 = engine.mmio_base;
    ...
    vma = i915_vma_instance(result, &engine.gt.ggtt.vm, core::ptr::null_mut());
    if (IS_ERR(vma)) {
    err = PTR_ERR(vma);
    goto err_obj;
    }
...
// cs++ = 0;
    ...
    unsigned int i;
    for (i = 0; i < RING_MAX_NONPRIV_SLOTS; i++) {
    let mut expected: u32 = get_whitelist_reg(engine, i);
    let mut actual: u32 = results[i];
    pr_info("RING_NONPRIV[%d]: expected 0x%08x, found 0x%08x\n",
    i, expected, actual);
    }
    }
#[no_mangle]
unsafe extern "C" fn check_whitelist(ce: *mut intel_context) -> c_int {
    ...
    err = 0;
    i915_gem_object_lock(results, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn do_device_reset(engine: *mut intel_engine_cs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_engine_reset(engine: *mut intel_engine_cs) -> c_int {
    static int do_engine_reset(struct intel_engine_cs *engine)
    {
    return intel_engine_reset(engine, "live_workarounds");
    }
#[no_mangle]
unsafe extern "C" fn do_guc_reset(engine: *mut intel_engine_cs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn reg_write(old: u32, new: u32, rsvd: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn wo_register(engine: *mut intel_engine_cs, reg: u32) -> bool {
    static bool wo_register(struct intel_engine_cs *engine, u32 reg)
    {
    let mut platform: enum intel_platform = INTEL_INFO(engine.i915).platform;
    int i;
...
#[no_mangle]
unsafe extern "C" fn timestamp(engine: *const intel_engine_cs, reg: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ro_register(reg: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn whitelist_writable_count(engine: *mut intel_engine_cs) -> c_int {
    static int whitelist_writable_count(struct intel_engine_cs *engine)
    {
    let mut count: c_int = engine.whitelist.count;
    int i;
    for (i = 0; i < engine.whitelist.count; i++) {
    let mut reg: u32 = i915_mmio_reg_offset(engine.whitelist.list[i].reg);
    if (ro_register(reg))
    count--;
    }
    return count;
...
#[no_mangle]
unsafe extern "C" fn check_dirty_whitelist(ce: *mut intel_context) -> c_int {
    ...
    let mut addr: u64 = i915_vma_offset(scratch);
    struct i915_request *rq;
    u32 srm, lrm, rsvd;
    u32 expect;
    int idx;
...
#[no_mangle]
unsafe extern "C" fn live_dirty_whitelist(arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn live_reset_whitelist(arg: *mut c_void) -> c_int {
...
// If we reset the gpu, we should not lose the RING_NONPRIV
    ...
    for (i = 0; i < engine.whitelist.count; i++) {
    let mut offset: u64 = i915_vma_offset(results) + sizeof(u32) * i;
    let mut reg: u32 = i915_mmio_reg_offset(engine.whitelist.list[i].reg);
// Clear non priv flags
    reg &= RING_FORCE_TO_NONPRIV_ADDRESS_MASK;
// cs++ = srm;
...
#[no_mangle]
unsafe extern "C" fn scrub_whitelisted_registers(ce: *mut intel_context) -> c_int {
...
pub struct regmask {
    ...
    unsigned long count)
    {
    let mut offset: u32 = i915_mmio_reg_offset(reg);
    while (count--) {
...
#[no_mangle]
unsafe extern "C" fn pardon_reg(i915: *mut drm_i915_private, reg: i915_reg_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn writeonly_reg(i915: *mut drm_i915_private, reg: i915_reg_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn live_isolated_whitelist(arg: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn intel_workarounds_live_selftests(i915: *mut drm_i915_private) -> c_int {
