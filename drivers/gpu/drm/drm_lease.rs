...
//! Source: drivers/gpu/drm/drm_lease.c
#![no_std]
...
//
    ...
    {
    while (master.lessor != core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn _drm_lease_held_master(master: *mut drm_master, id: c_int) -> c_int {
    static int _drm_lease_held_master(struct drm_master *master, int id)
    {
    lockdep_assert_held(&master.dev.mode_config.idr_mutex);
    if (master.lessor)
    return idr_find(&master.leases, id) != core::ptr::null_mut();
    return true;
    }
// Checks if the given object has been leased to some lessee of drm_master
#[no_mangle]
unsafe extern "C" fn _drm_has_leased(master: *mut drm_master, id: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn _drm_lease_held(file_priv: *mut drm_file, id: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn drm_lease_held(file_priv: *mut drm_file, id: c_int) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_lease_filter_crtcs(file_priv: *mut drm_file, crtcs_in: u32) -> u32 {
    ...
    let mut mask_in: u32 = 1ul << count_in;
    if ((crtcs_in & mask_in) != 0) {
    let mut mask_out: u32 = 1ul << count_out;
    crtcs_out |= mask_out;
    }
    count_out++;
    }
...
//
    ...
    error = -ENOENT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: _drm_has_leased(lessor, _arg: object)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn drm_lease_destroy(master: *mut drm_master) {
...
#[no_mangle]
unsafe extern "C" fn _drm_lease_revoke(top: *mut drm_master) {
...
#[no_mangle]
pub unsafe extern "C" fn drm_lease_revoke(top: *mut drm_master) {
    ...
    {
    int o;
    let mut has_crtc: c_int = -1;
    let mut has_connector: c_int = -1;
    let mut has_plane: c_int = -1;
// we want to confirm that there is at least one crtc, plane
    ...
    u32 o;
    int ret;
    let mut universal_planes: bool = READ_ONCE(lessor_priv.universal_planes);
...
// add their IDs to the lease request - taking into account
    ...
    let mut object_id: u32 = objects[o].id;
...
//
    ...
    struct drm_master *lessee = core::ptr::null_mut();
    struct file *lessee_file = core::ptr::null_mut();
...
// Hook up the fd
    ...
    let mut count_lessees: __u32 = arg.count_lessees;
...
// Return the list of leased objects for the specified lessee
    ...
    let mut count_objects: __u32 = arg.count_objects;
    struct drm_master *lessee;
...
// Can't lease without MODESET
    ...
    mutex_lock(&dev.mode_config.idr_mutex);
    if (lessee.lessor == core::ptr::null_mut())
