...
//! Source: drivers/gpu/drm/sysfb/simpledrm.c
#![no_std]
...
//
    ...
    {
    let mut ret: c_int = of_property_read_string(of_node, name, value);
    if (ret)
    drm_err(dev, "simplefb: cannot parse framebuffer %s: error %d\n",
    name, ret);
    return ret;
    ...
    {
    u32 width;
    let mut ret: c_int = simplefb_read_u32_of(dev, of_node, "width", &width);
    if (ret)
    return ret;
    ...
    {
    u32 height;
    let mut ret: c_int = simplefb_read_u32_of(dev, of_node, "height", &height);
    if (ret)
    return ret;
    ...
    {
    u32 stride;
    let mut ret: c_int = simplefb_read_u32_of(dev, of_node, "stride", &stride);
    if (ret)
    return ret;
    ...
    {
    const char *format;
    let mut ret: c_int = simplefb_read_string_of(dev, of_node, "format", &format);
    if (ret)
    return ERR_PTR(ret);
...
// regulators
    ...
    pub regulator_count: c_uint,
    pub regulators: *mut regulator,

// power-domains

    pub pwr_dom_count: c_int,
    pub pwr_dom_devs: *mut device,
    pub pwr_dom_links: *mut device_link,
...
//
#[no_mangle]
unsafe extern "C" fn simpledrm_device_release_clocks(res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn simpledrm_device_init_clocks(sdev: *mut simpledrm_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn simpledrm_device_release_regulators(res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn simpledrm_device_init_regulators(sdev: *mut simpledrm_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn simpledrm_device_detach_genpd(res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn simpledrm_device_attach_genpd(sdev: *mut simpledrm_device) -> c_int {
...
//
    ...
    let mut ret: c_int = PTR_ERR(sdev.pwr_dom_devs[i]);
    if (ret == -EPROBE_DEFER) {
...
//
    ...
    let mut width_mm: u16 = 0, height_mm = 0;
    struct device_node *panel_node;
    let mut orientation: enum drm_panel_orientation = DRM_MODE_PANEL_ORIENTATION_UNKNOWN;
    const struct drm_format_info *format;
    u64 size;
    struct resource *res, *mem = core::ptr::null_mut();
...
//
    ...
    if (!stride) {
    let mut pitch: u64 = drm_format_info_min_pitch(format, 0, width);
...
// CRTC
    ...
    &simpledrm_crtc_funcs, core::ptr::null_mut());
    if (ret)
    return ERR_PTR(ret);
...
//
#[no_mangle]
unsafe extern "C" fn simpledrm_pm_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn simpledrm_pm_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn simpledrm_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn simpledrm_remove(pdev: *mut platform_device) {
