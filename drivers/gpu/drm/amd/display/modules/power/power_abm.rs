...
//! Source: drivers/gpu/drm/amd/display/modules/power/power_abm.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct abm_parameters {
    pub min_reduction: c_uchar,
    pub max_reduction: c_uchar,
    pub bright_pos_gain: c_uchar,
    pub dark_pos_gain: c_uchar,
    pub brightness_gain: c_uchar,
    pub contrast_factor: c_uchar,
    pub deviation_gain: c_uchar,
    pub min_knee: c_uchar,
    pub max_knee: c_uchar,
    pub blRampReduction: c_ushort,
    pub blRampStart: c_ushort,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct custom_backlight_profile {
    pub ac_level_percentage: u8,
    pub dc_level_percentage: u8,
    pub min_input_signal: u8,
    pub max_input_signal: u8,
    pub num_data_points: u8,
    pub data_points: *const dm_bl_data_point,
}
...
pub const IRAM_SIZE: c_int = 256;
pub const IRAM_RESERVE_AREA_START_V2: c_uint = 0xF0  // reserve 0xF0~0xF6 are write by DMCU only;
pub const IRAM_RESERVE_AREA_END_V2: c_uint = 0xF6  // reserve 0xF0~0xF6 are write by DMCU only;
pub const IRAM_RESERVE_AREA_START_V2_2: c_uint = 0xF0  // reserve 0xF0~0xFF are write by DMCU only;
pub const IRAM_RESERVE_AREA_END_V2_2: c_uint = 0xFF  // reserve 0xF0~0xFF are write by DMCU only;

// NOTE: iRAM is 256B in size
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iram_table_v_2 {
// flags
    pub /: *mut *mut uint16_t min_abm_backlight; / 0x00 U16,
// parameters for ABM2.0 algorithm
    pub /: *mut *mut uint8_t min_reduction[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x02 U0.8,
    pub /: *mut *mut uint8_t max_reduction[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x16 U0.8,
    pub /: *mut *mut uint8_t bright_pos_gain[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x2a U2.6,
    pub /: *mut *mut uint8_t bright_neg_gain[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x3e U2.6,
    pub /: *mut *mut uint8_t dark_pos_gain[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x52 U2.6,
    pub /: *mut *mut uint8_t dark_neg_gain[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x66 U2.6,
    pub /: *mut *mut uint8_t iir_curve[NUM_AMBI_LEVEL]; / 0x7a U0.8,
    pub /: *mut *mut uint8_t deviation_gain; / 0x7f U0.8,
// parameters for crgb conversion
    pub /: *mut *mut uint16_t crgb_thresh[NUM_POWER_FN_SEGS]; / 0x80 U3.13,
    pub /: *mut *mut uint16_t crgb_offset[NUM_POWER_FN_SEGS]; / 0x90 U1.15,
    pub /: *mut *mut uint16_t crgb_slope[NUM_POWER_FN_SEGS]; / 0xa0 U4.12,
// parameters for custom curve
// thresholds for brightness --> backlight
    pub /: *mut *mut uint16_t backlight_thresholds[NUM_BL_CURVE_SEGS]; / 0xb0 U16.0,
// offsets for brightness --> backlight
    pub /: *mut *mut uint16_t backlight_offsets[NUM_BL_CURVE_SEGS]; / 0xd0 U16.0,
// For reading PSR State directly from IRAM
    pub /: *mut *mut uint8_t psr_state; / 0xf0,
    pub /: *mut *mut uint8_t dmcu_mcp_interface_version; / 0xf1,
    pub /: *mut *mut uint8_t dmcu_abm_feature_version; / 0xf2,
    pub /: *mut *mut uint8_t dmcu_psr_feature_version; / 0xf3,
    pub /: *mut *mut uint16_t dmcu_version; / 0xf4,
    pub /: *mut *mut uint8_t dmcu_state; / 0xf6,
    pub /: *mut *mut uint16_t blRampReduction; / 0xf7,
    pub /: *mut *mut uint16_t blRampStart; / 0xf9,
    pub /: *mut *mut uint8_t dummy5; / 0xfb,
    pub /: *mut *mut uint8_t dummy6; / 0xfc,
    pub /: *mut *mut uint8_t dummy7; / 0xfd,
    pub /: *mut *mut uint8_t dummy8; / 0xfe,
    pub /: *mut *mut uint8_t dummy9; / 0xff,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct iram_table_v_2_2 {
// flags
    pub /: *mut *mut uint16_t flags; / 0x00 U16,
// parameters for ABM2.2 algorithm
    pub /: *mut *mut uint8_t min_reduction[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x02 U0.8,
    pub /: *mut *mut uint8_t max_reduction[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x16 U0.8,
    pub /: *mut *mut uint8_t bright_pos_gain[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x2a U2.6,
    pub /: *mut *mut uint8_t dark_pos_gain[NUM_AMBI_LEVEL][NUM_AGGR_LEVEL]; / 0x3e U2.6,
    pub /: *mut *mut uint8_t hybrid_factor[NUM_AGGR_LEVEL]; / 0x52 U0.8,
    pub /: *mut *mut uint8_t contrast_factor[NUM_AGGR_LEVEL]; / 0x56 U0.8,
    pub /: *mut *mut uint8_t deviation_gain[NUM_AGGR_LEVEL]; / 0x5a U0.8,
    pub /: *mut *mut uint8_t iir_curve[NUM_AMBI_LEVEL]; / 0x5e U0.8,
    pub /: *mut *mut uint8_t min_knee[NUM_AGGR_LEVEL]; / 0x63 U0.8,
    pub /: *mut *mut uint8_t max_knee[NUM_AGGR_LEVEL]; / 0x67 U0.8,
    pub /: *mut *mut uint16_t min_abm_backlight; / 0x6b U16,
    pub /: *mut *mut uint8_t pad[19]; / 0x6d U0.8,
// parameters for crgb conversion
    pub /: *mut *mut uint16_t crgb_thresh[NUM_POWER_FN_SEGS]; / 0x80 U3.13,
    pub /: *mut *mut uint16_t crgb_offset[NUM_POWER_FN_SEGS]; / 0x90 U1.15,
    pub /: *mut *mut uint16_t crgb_slope[NUM_POWER_FN_SEGS]; / 0xa0 U4.12,
...
// For reading PSR State directly from IRAM
    ...
    pub /: *mut *mut uint16_t dmcu_version; / 0xf4,
    pub /: *mut *mut uint8_t dmcu_state; / 0xf6,
    pub /: *mut *mut uint8_t dummy1; / 0xf7,
    pub /: *mut *mut uint8_t dummy2; / 0xf8,
    pub /: *mut *mut uint8_t dummy3; / 0xf9,
    pub /: *mut *mut uint8_t dummy4; / 0xfa,
    pub /: *mut *mut uint8_t dummy5; / 0xfb,
    pub /: *mut *mut uint8_t dummy6; / 0xfc,
    pub /: *mut *mut uint8_t dummy7; / 0xfd,
    pub /: *mut *mut uint8_t dummy8; / 0xfe,
...
pub const MOD_POWER_MAX_CONCURRENT_STREAMS: c_int = 32;
pub const SMOOTH_BRIGHTNESS_ADJUSTMENT_TIME_IN_MS: c_int = 500;
...
// Max = 300 nits
//
// Macro flag: #define MOD_POWER_TO_CORE(mod_power)\
    container_of(mod_power, struct core_power, mod_public)
// Maximum brightness expressed in millipercent (100% * 1000).

#[no_mangle]
unsafe extern "C" fn backlight_8_to_16(backlight_8bit: c_uint) -> u16 {
...
//
    ...
    {
    if (core_power == core::ptr::null_mut())
    return;
...
//
#[no_mangle]
unsafe extern "C" fn is_vesa_abc(core_power: *mut core_power, inst: c_uint) -> bool {
...
//
    ...
    {
    let mut millipercent: c_uint = 0;
    let mut numerator: c_ulonglong = 0;
    if (core_power == core::ptr::null_mut())
    return 0;
...
//
    ...
    let mut max: c_uint = max_index;
    let mut mid: c_uint = 0;
    while (max >= min) {
    mid = (min + max) / 2; /* floor of half range */
    if (core_power.bl_prop[inst].backlight_lut[mid] < pwm)
    min = mid + 1;
#[no_mangle]
pub unsafe extern "C" fn if(pwm: core_power->bl_prop[inst].backlight_lut[mid] >) -> else {
...
//
    ...
    struct core_power *core_power, unsigned int millipercent, unsigned int inst)
    {
    let mut pwm: c_uint = (unsigned int)-1;
...
// Null check otherwise eDP doesn't lightup when connected to DP1
    if (core_power.bl_prop[inst].backlight_lut == core::ptr::null_mut())
    return pwm;
...
//
    ...
    return core_power.bl_prop[inst].max_backlight_pwm;
#[no_mangle]
pub unsafe extern "C" fn if(core_power->bl_prop[inst].min_brightness_millinits: millinit <=) -> else {
...
//
    ...
    {
    unsigned int i;
    let mut num_of_data_points: c_uint = 0;
    let mut last_signal_level: c_uint = 0;
    let mut last_luminance: c_uint = 0;
...
//
    ...
    let mut luminance: c_uint = ext_backlight_caps.data_points[i].luminance;
    let mut signal_level: c_uint = ext_backlight_caps.data_points[i].signal_level;
...
#[no_mangle]
pub unsafe extern "C" fn initialize_backlight_caps(core_power: *mut core_power, inst: c_uint) {
    void initialize_backlight_caps(struct core_power *core_power, unsigned int inst)
    {
    unsigned int i;
    struct dm_acpi_atif_backlight_caps *ext_backlight_caps = core::ptr::null_mut();
    let mut custom_curve_present: bool = false;
    let mut num_levels: c_uint = 0;
    struct dc *dc = core::ptr::null_mut();
...
//
    ...
    if (ext_backlight_caps == core::ptr::null_mut())
    return;
// Retrieve ACPI extended brightness caps
    ...
    let mut index: c_uint = 1;
    let mut num_of_data_points: c_uint = ext_backlight_caps.num_data_points;
...
// Lineary interpolate missing values
    ...
    luminance - index + 1;
    let mut step: c_uint = delta_signal;
...
//
    ...
    core_power.bl_prop[inst].backlight_lut[num_levels - 1] -
    base_value;
    let mut delta_luma: c_uint = num_levels - index;
    let mut step: c_uint = delta_signal;
...
//
    ...
    let mut backlight_curve_coeff_b: c_uint = num_levels;
    unsigned int delta =
    core_power.bl_prop[inst].backlight_lut[num_levels - 1] -
    core_power.bl_prop[inst].backlight_lut[0];
    let mut coeffC: c_uint = core_power.bl_prop[inst].backlight_lut[0];
    unsigned int coeffB =
...
#[no_mangle]
unsafe extern "C" fn varibright_set_level(core_power: *mut core_power) {
...
#[no_mangle]
pub unsafe extern "C" fn mod_power_hw_init_backlight(mod_power: *mut mod_power) -> bool {
    bool mod_power_hw_init_backlight(struct mod_power *mod_power)
    {
    struct core_power *core_power = core::ptr::null_mut();
    struct dc *dc = core::ptr::null_mut();
    struct dmcu *dmcu = core::ptr::null_mut();
    struct dmcu_iram_parameters params;
    unsigned int i;
    if (mod_power == core::ptr::null_mut())
    return false;
...
// DMCU does not support multiple eDP
    ...
    bool is_hdr)
    {
    let mut backlight_level_params: set_backlight_level_params = { 0 };
...
// only use internal backlight control if dmub capabilities are not present
    ...
    let mut frame_ramp: c_uint = 0;
    unsigned int vsync_rate_hz;
    union dmcu_abm_set_bl_params params;
    const struct dc_link *link = core::ptr::null_mut();
    let mut backlight_pwm_u16_16: c_uint = backlight_level_params.backlight_pwm_u16_16;
    let mut transition_time_millisec: c_uint = backlight_level_params.transition_time_in_ms;
    if (core_power == core::ptr::null_mut())
    return false;
...
// backlight_millipercent = core_power->bl_state[inst].backlight_millipercent;
    ...
    {
    struct core_power *core_power = core::ptr::null_mut();
    let mut backlight_u16_16: c_uint = 0;
    if (mod_power == core::ptr::null_mut())
    return false;
    core_power = MOD_POWER_TO_CORE(mod_power);
...
// varibright_enable = core_power->varibright_prop.varibright_user_enable;
    ...
    unsigned int inst)
    {
    let mut user_backlight: c_uint = 0;
    let mut current_backlight: c_uint = 0;
...
// It's only implemented on dmcub.
    ...
    {
    unsigned int i;
    let mut num_entries: c_uint = NUM_BL_CURVE_SEGS;
...
//
    ...
    let mut threshold_val: c_uint = DIV_ROUNDUP((i * 65536), num_entries);
    let mut offset_val: c_uint = params.backlight_lut_array[lut_index];
...
#[no_mangle]
unsafe extern "C" fn fill_iram_v_2(ram_table: *mut iram_table_v_2, params: dmcu_iram_parameters) {
    static void fill_iram_v_2(struct iram_table_v_2 *ram_table, struct dmcu_iram_parameters params)
    {
    let mut set: c_uint = params.set;
...
#[no_mangle]
unsafe extern "C" fn fill_iram_v_2_2(ram_table: *mut iram_table_v_2_2, params: dmcu_iram_parameters) {
...
#[no_mangle]
unsafe extern "C" fn fill_iram_v_2_3(ram_table: *mut iram_table_v_2_2, params: dmcu_iram_parameters, big_endian: bool) {
...
// Gamma 2.2
    ...
    uint32_t i, j = 0;
    if (res_pool.abm == core::ptr::null_mut() && res_pool.multiple_abms[inst] == core::ptr::null_mut())
    return false;
...
#[no_mangle]
pub unsafe extern "C" fn dmub_is_abm_supported(res_pool: *mut resource_pool, inst: c_uint) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn fill_custom_backlight_caps(config_no: c_uint, caps: *mut dm_acpi_atif_backlight_caps) -> bool {
