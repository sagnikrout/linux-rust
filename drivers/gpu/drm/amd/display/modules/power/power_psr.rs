...
//! Source: drivers/gpu/drm/amd/display/modules/power/power_psr.c
#![no_std]
...
//
    ...
    let mut psr_config: psr_config = {0};
    let mut psr_context: psr_context = {0};
    let mut active_psr_events: c_int = 0;
    if ((mod_power == core::ptr::null_mut()) || (stream == core::ptr::null_mut()) || (link == core::ptr::null_mut()))
    return false;
...
//
    ...
    dc_link_set_psr_allow_active(link, core::ptr::null_mut(), false, false, &power_opt);
    ...
    {
    struct core_power *core_power = core::ptr::null_mut();
    let mut state: enum dc_psr_state = PSR_STATE0;
    unsigned int retry_count;
    let mut max_retry: c_uint = 1000;
    struct dc_link *link = core::ptr::null_mut();
    ...
    let mut params: dc_static_screen_params = {0};
    ...
    link = dc_stream_get_link(stream);
    if (!dc_link_set_psr_allow_active(link, &psr_enable, false, force_static, core::ptr::null_mut())) {
    DC_TRACE_LEVEL_MESSAGE(DAL_TRACE_LEVEL_ERROR,
    WPP_BIT_FLAG_Firmware_PsrState,
...
// active_psr_events = core_power->map[stream_index].psr_events;
    ...
    struct core_power *core_power = core::ptr::null_mut();
    let mut stream_index: c_uint = 0;
    let mut active_psr_events: c_uint = 0;
    let mut psr_enable_request: bool = false;
    let mut force_static: bool = false;
    if (mod_power == core::ptr::null_mut() || stream == core::ptr::null_mut())
    return false;
    core_power = MOD_POWER_TO_CORE(mod_power);
    stream_index = map_index_from_stream(core_power, stream);
    if (core_power.num_entities == 0) {
...
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_pause: active_psr_events &) -> else {
    else if (active_psr_events & psr_event_pause)
    psr_enable_request = false;
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_test_harness_enable_psr: active_psr_events &) -> else {
    else if (active_psr_events & psr_event_test_harness_enable_psr)
    psr_enable_request = true;
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_dynamic_display_switch: active_psr_events &) -> else {
    psr_enable_request = true;
    force_static = true;
    ...
    psr_enable_request = false;
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_big_screen_video: active_psr_events &) -> else {
    else if (active_psr_events & psr_event_big_screen_video)
    psr_enable_request = true;
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_full_screen: active_psr_events &) -> else {
    else if (active_psr_events & psr_event_full_screen)
    psr_enable_request = false;
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_mpo_video_selective_update: active_psr_events &) -> else {
    else if (active_psr_events & psr_event_mpo_video_selective_update)
    psr_enable_request = true;
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_vsync: active_psr_events &) -> else {
    else if (active_psr_events & psr_event_vsync)
    psr_enable_request = false;
#[no_mangle]
pub unsafe extern "C" fn if(psr_event_crc_window_active: active_psr_events &) -> else {
...
// psr_enabled = core_power->map[stream_index].psr_enabled;
    ...
    if (mod_power == core::ptr::null_mut())
    return false;
    if (active_psr_events == core::ptr::null_mut())
    return false;
    core_power = MOD_POWER_TO_CORE(mod_power);
    if (core_power.num_entities == 0)
    return false;
...
//
#[no_mangle]
pub unsafe extern "C" fn is_psr_su_specific_panel(link: *mut dc_link) -> bool {
    bool is_psr_su_specific_panel(struct dc_link *link)
    {
    let mut isPSRSUSupported: bool = false;
...
//
    ...
    isPSRSUSupported = false;
#[no_mangle]
pub unsafe extern "C" fn if(0x03: dpcd_caps->sink_dev_id_str[1] == 0x08 && dpcd_caps->sink_dev_id_str[0] ==) -> else {
    else if (dpcd_caps.sink_dev_id_str[1] == 0x08 && dpcd_caps.sink_dev_id_str[0] == 0x03)
    isPSRSUSupported = false;
#[no_mangle]
pub unsafe extern "C" fn if(0x01: dpcd_caps->sink_dev_id_str[1] == 0x08 && dpcd_caps->sink_dev_id_str[0] ==) -> else {
    else if (dpcd_caps.sink_dev_id_str[1] == 0x08 && dpcd_caps.sink_dev_id_str[0] == 0x01)
    isPSRSUSupported = false;
#[no_mangle]
pub unsafe extern "C" fn if(0x1: dpcd_caps->psr_info.force_psrsu_cap ==) -> else {
...
//
    ...
    const struct dc_stream_state *stream)
    {
    let mut num_vblank_lines: c_uint = 0;
    let mut vblank_time_in_us: c_uint = 0;
    let mut sdp_tx_deadline_in_us: c_uint = 0;
    let mut line_time_in_us: c_uint = 0;
    struct dpcd_caps *dpcd_caps = &link.dpcd_caps;
