...
//! Source: drivers/gpu/drm/amd/display/modules/power/power_replay.c
#![no_std]
...
pub const LOW_REFRESH_RATE_DURATION_US_UPPER_BOUND: c_int = 25000;
    ...
    if (!link)
    return false;
    if (!dc_link_set_replay_allow_active(link, &replay_active, false, force_static, core::ptr::null_mut()))
    return false;
    if (wait == true) {
...
//
    ...
    if (!dc_link_set_replay_allow_active(link, core::ptr::null_mut(), false, false, &power_opt))
    return false;
    return true;
    }
...
// General UI scenario
    ...
    struct dc_link *link = core::ptr::null_mut();
    let mut stream_index: c_uint = 0;
    let mut cmd_data: union dmub_replay_cmd_set = { 0 };
    if (!stream || mod_power == core::ptr::null_mut())
    return;
    core_power = MOD_POWER_TO_CORE(mod_power);
    if (core_power.num_entities == 0)
    return;
    ...
    {
    struct dc_link *link = core::ptr::null_mut();
    union replay_debug_flags *pDebug = core::ptr::null_mut();
...
// Check if LIVE_CAPTURE_WITH_CVT bit is enabled in replay optimization config
    ...
    struct dc_link *link = core::ptr::null_mut();
    let mut stream_index: c_uint = 0;
    let mut active_replay_events: c_uint = 0;
    let mut replay_active_request: bool = false;
    let mut force_static: bool = false;
    let mut coasting_vtotal: u32 = 0;
    let mut current_timing_sync_status: bool = false;
    let mut is_full_screen_video: bool = false;
    let mut is_ultra_sleep_mode: bool = false;
    let mut sink_duration_us: c_uint = 0;
    let mut low_rr_active: bool = false;
    let mut frame_skip_number: u16 = 0;
    let mut is_video_playback: bool = false;
    if (!stream)
    return false;
    if (mod_power == core::ptr::null_mut())
    return false;
...
// replay_active = link->replay_settings.replay_allow_active;
    ...
    {
    struct core_power *core_power = core::ptr::null_mut();
    let mut active_replay_events: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn init_replay_config(link: *mut dc_link, pr_config: *mut replay_config) {
    ...
    bool is_defer)
    {
    uint32_t *frame_skip_number_array = core::ptr::null_mut();
    let mut frame_skip_number: u32 = 0;
    ...
    if (frame_skip_number_array == core::ptr::null_mut())
    return;
...
#[no_mangle]
pub unsafe extern "C" fn set_replay_low_rr_full_screen_video_src_vtotal(link: *mut dc_link, vtotal: u16) {
    ...
    {
    let mut max_link_off_frame_count: u32 = 0;
    let mut max_deviation_line: u16 = 0,  pixel_deviation_per_line = 0;
...
#[no_mangle]
pub unsafe extern "C" fn reset_replay_dsync_error_count(link: *mut dc_link) {
