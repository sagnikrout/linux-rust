...
//! Source: drivers/gpu/drm/msm/msm_gem_vma.c
#![no_std]
...
//
    ...
    let mut vm_log_shift: static uint = 0;
...
// @map: Parameters used if op == MSM_VMA_OP_MAP
    pub map: msm_vm_map_op,
// @unmap: Parameters used if op == MSM_VMA_OP_UNMAP
    pub unmap: msm_vm_unmap_op,
}
...
// @prealloc: Tracking for pre-allocated MMU pgtable pages
    pub prealloc: msm_mmu_prealloc,
// @vm_ops: a list of struct msm_vm_op
    pub vm_ops: list_head,
...
//
    ...
    let mut vm_log_len: u32 = (1 << vm.log_shift);
    let mut vm_log_mask: u32 = vm_log_len - 1;
    uint32_t nr_vm_logs;
    int first;
    vm.unusable = true;
...
//
    ...
    let mut idx: c_int = (i + first) & vm_log_mask;
...
#[no_mangle]
pub unsafe extern "C" fn msm_gem_vma_unmap(vma: *mut drm_gpuva, reason: *const c_char) {
...
#[no_mangle]
pub unsafe extern "C" fn msm_gem_vma_close(vma: *mut drm_gpuva) {
...
// _NO_SHARE objs cannot be mapped outside of their "host" vm:
    ...
    if (vm.managed) {
    BUG_ON(offset != 0);
    BUG_ON(!obj);  /* core::ptr::null_mut() mappings not valid for kernel managed VM */
...
pub struct op_arg {
    pub flags: unsigned,
    pub job: *mut msm_vm_bind_job,
    pub op: *const msm_vm_bind_op,
    pub kept: bool,
}
...
// op = _op;
    ...
    struct drm_gpuva *prev_vma = core::ptr::null_mut(), *next_vma = core::ptr::null_mut();
    struct drm_gpuvm_bo *vm_bo = orig_vma.vm_bo;
    let mut mapped: bool = to_msm_vma(orig_vma).mapped;
    unsigned flags;
    int ret;
...
// Only flags are changing, so update that in-place:
    let mut orig_flags: unsigned = vma.flags & (DRM_GPUVA_USERBITS - 1);
    ...
    struct drm_gem_object *obj;
    let mut ret: c_int = vm.unusable ? -EINVAL : 0;
    vm_dbg("");
    mutex_lock(&vm.mmu_lock);
...
//
    ...
    vm.mmu.prealloc = core::ptr::null_mut();
    mutex_unlock(&vm.mmu_lock);
//
...
//
    let mut flags: enum drm_gpuvm_flags = 0;
...
// Kill the scheduler now, so we aren't racing with it for cleanup:
    drm_sched_stop(&vm.sched, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn invalid_alignment(addr: u64) -> bool {
...
// nr_bos = cnt;
    ...
    if (!first)
    return;
    let mut start_iova: u64 = first.iova;
    let mut end_iova: u64 = last.iova + last.range;
...
//
    ...
    {
    struct msm_vm_bind_op *first = core::ptr::null_mut(), *last = core::ptr::null_mut();
    struct msm_gem_vm *vm = to_msm_vm(job.vm);
    int ret;
...
//
    ...
    struct msm_vm_bind_job *job = core::ptr::null_mut();
    struct msm_gpu *gpu = priv.gpu;
    struct msm_gpu_submitqueue *queue;
    struct msm_syncobj_post_dep *post_deps = core::ptr::null_mut();
    struct drm_syncobj **syncobjs_to_reset = core::ptr::null_mut();
...
//
    ...
    goto out_unlock;
    struct drm_exec exec;
    let mut flags: unsigned = DRM_EXEC_IGNORE_DUPLICATES | DRM_EXEC_INTERRUPTIBLE_WAIT;
