...
//! Source: drivers/gpu/drm/msm/msm_gem_submit.c
#![no_std]
...
//
    ...
    {
    let mut ident: static atomic_t = ATOMIC_INIT(0);
...
#[no_mangle]
pub unsafe extern "C" fn __msm_gem_submit_destroy(kref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn submit_lock_objects_vmbind(submit: *mut msm_gem_submit) -> c_int {
    static int submit_lock_objects_vmbind(struct msm_gem_submit *submit)
    {
    let mut flags: unsigned = DRM_EXEC_INTERRUPTIBLE_WAIT | DRM_EXEC_IGNORE_DUPLICATES;
...
#[no_mangle]
unsafe extern "C" fn submit_lock_objects(submit: *mut msm_gem_submit) -> c_int {
    static int submit_lock_objects(struct msm_gem_submit *submit)
    {
    let mut flags: unsigned = DRM_EXEC_INTERRUPTIBLE_WAIT;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn submit_fence_sync(submit: *mut msm_gem_submit) -> c_int {
    static int submit_fence_sync(struct msm_gem_submit *submit)
    {
    int i, ret = 0;
    for (i = 0; i < submit.nr_bos; i++) {
    struct drm_gem_object *obj = submit.bos[i].obj;
    let mut write: bool = submit.bos[i].flags & MSM_SUBMIT_BO_WRITE;
...
#[no_mangle]
unsafe extern "C" fn submit_pin_objects(submit: *mut msm_gem_submit) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn submit_unpin_objects(submit: *mut msm_gem_submit) {
...
#[no_mangle]
unsafe extern "C" fn submit_attach_object_fences(submit: *mut msm_gem_submit) {
...
#[no_mangle]
pub unsafe extern "C" fn if(MSM_SUBMIT_BO_READ: submit->bos[i].flags &) -> else {
...
//
    ...
    let mut submit_reloc: drm_msm_gem_submit_reloc = relocs[i];
    uint32_t off;
    uint64_t iova;
...
// offset in dwords:
    ...
    ret = submit_bo(submit, submit_reloc.reloc_idx, core::ptr::null_mut(), &iova);
    if (ret)
    goto out;
...
//
#[no_mangle]
unsafe extern "C" fn submit_cleanup(submit: *mut msm_gem_submit, error: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn msm_submit_retire(submit: *mut msm_gem_submit) {
    ...
    struct msm_gem_submit *submit = core::ptr::null_mut();
...
// The scheduler owns a ref now:
    ...
    msm_rd_dump_submit(priv.rd, submit, core::ptr::null_mut());
