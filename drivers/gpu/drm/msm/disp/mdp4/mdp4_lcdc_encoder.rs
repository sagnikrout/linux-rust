...
//! Source: drivers/gpu/drm/msm/disp/mdp4/mdp4_lcdc_encoder.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mdp4_lcdc_encoder_disable(encoder: *mut drm_encoder) {
...
#[no_mangle]
unsafe extern "C" fn mdp4_lcdc_encoder_enable(encoder: *mut drm_encoder) {
    static void mdp4_lcdc_encoder_enable(struct drm_encoder *encoder)
    {
    struct drm_device *dev = encoder.dev;
    struct mdp4_lcdc_encoder *mdp4_lcdc_encoder =
    to_mdp4_lcdc_encoder(encoder);
    let mut pc: c_ulong = mdp4_lcdc_encoder.pixclock;
    struct mdp4_kms *mdp4_kms = get_kms(encoder);
    uint32_t config;
    int ret;
    if (WARN_ON(mdp4_lcdc_encoder.enabled))
...
// initialize encoder
    ...
    core::ptr::null_mut(), DRM_MODE_ENCODER_LVDS, core::ptr::null_mut());
