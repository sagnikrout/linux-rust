...
//! Source: drivers/gpu/drm/msm/disp/mdp4/mdp4_plane.c
#![no_std]
...
pub const DOWN_SCALE_MAX: c_int = 8;
pub const UP_SCALE_MAX: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mdp4_plane {
    pub base: drm_plane,
    pub name: *const c_char,
    pub pipe: enum mdp4_pipe,
}

// MDP format helper functions
    static inline
#[no_mangle]
pub unsafe extern "C" fn mdp4_get_frame_format(fb: *mut drm_framebuffer) -> enum mdp4_frame_format {
    enum mdp4_frame_format mdp4_get_frame_format(struct drm_framebuffer *fb)
    {
    let mut is_tile: bool = false;
...
// atomic_check should have ensured that this doesn't fail
    ...
    let mut pipe: enum mdp4_pipe = mdp4_plane.pipe;
...
pub const MDP4_VG_PHASE_STEP_DEFAULT: c_uint = 0x20000000;
    static int mdp4_plane_mode_set(struct drm_plane *plane,
    struct drm_crtc *crtc, struct drm_framebuffer *fb,
    int crtc_x, int crtc_y,
    unsigned int crtc_w, unsigned int crtc_h,
    ...
    let mut phasex_step: u32 = MDP4_VG_PHASE_STEP_DEFAULT;
    let mut phasey_step: u32 = MDP4_VG_PHASE_STEP_DEFAULT;
...
// src values are in Q16 fixed point, convert to integer:
    ...
    if (src_w != crtc_w) {
    let mut sel_unit: u32 = SCALE_FIR;
    op_mode |= MDP4_PIPE_OP_MODE_SCALEX_EN;
    if (MSM_FORMAT_IS_YUV(format)) {
    if (crtc_w > src_w)
    sel_unit = SCALE_PIXEL_RPT;
#[no_mangle]
pub unsafe extern "C" fn if(4): crtc_w <= (src_w /) -> else {
    ...
    sel_unit = SCALE_PIXEL_RPT;
#[no_mangle]
pub unsafe extern "C" fn if(4): crtc_h <= (src_h /) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn mdp4_plane_pipe(plane: *mut drm_plane) -> enum mdp4_pipe {
