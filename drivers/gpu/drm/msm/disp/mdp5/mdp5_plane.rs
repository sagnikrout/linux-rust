...
//! Source: drivers/gpu/drm/msm/disp/mdp5/mdp5_plane.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mdp5_plane_reset(plane: *mut drm_plane) {
    ...
    {
    let mut needs_dirtyfb: bool = to_mdp5_plane_state(new_state).needs_dirtyfb;
    if (!new_state.fb)
    return 0;
    ...
    let mut needed_dirtyfb: bool = to_mdp5_plane_state(old_state).needs_dirtyfb;
    if (!fb)
    return;
    ...
    let mut new_hwpipe: bool = false;
    let mut need_right_hwpipe: bool = false;
...
//
    ...
    mdp5_state.r_hwpipe;
    struct mdp5_hw_pipe *new_hwpipe = core::ptr::null_mut();
    struct mdp5_hw_pipe *new_right_hwpipe = core::ptr::null_mut();
    ret = mdp5_pipe_assign(state.state, plane, caps,
    blkcfg, &new_hwpipe,
    need_right_hwpipe ?
    &new_right_hwpipe : core::ptr::null_mut());
    if (ret) {
...
// new state
//
    mdp5_state.r_hwpipe = core::ptr::null_mut();
    ...
    if (ret)
    return ret;
    mdp5_state.hwpipe = mdp5_state.r_hwpipe = core::ptr::null_mut();
    }
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn csc_disable(mdp5_kms: *mut mdp5_kms, pipe: enum mdp5_pipe) {
...
pub const PHASE_STEP_SHIFT: c_int = 21;

#[no_mangle]
unsafe extern "C" fn calc_phase_step(src: u32, dst: u32, out_phase: *mut u32) -> c_int {
...
// out_phase = mult_frac(unit, src, dst);
    ...
    let mut yuv: bool = MSM_FORMAT_IS_YUV(format);
    let mut scaling: bool = yuv ? true : (src != dst);
    uint32_t sub;
    ...
    bool horz)
    {
    let mut scaling: bool = MSM_FORMAT_IS_YUV(format) ? true : (src != dst);
    int i;
//
// Note:
// We assume here that:
...
//
    ...
    int i;
    for (i = 0; i < COMP_MAX; i++) {
    let mut roi_w: u32 = src_w;
    let mut roi_h: u32 = src_h;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pixel_ext {
    pub left: [c_int; COMP_MAX],
    pub right: [c_int; COMP_MAX],
    pub top: [c_int; COMP_MAX],
    pub bottom: [c_int; COMP_MAX],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct phase_step {
    pub x: [u32; COMP_MAX],
    pub y: [u32; COMP_MAX],
}
    ...
    u32 src_w, u32 src_h)
    {
    let mut pipe: enum mdp5_pipe = hwpipe.pipe;
    let mut has_pe: bool = hwpipe.caps & MDP_PIPE_CAP_SW_PIX_EXT;
    const struct msm_format *format =
...
// not using secure mode:
    ...
    let mut step: phase_step = { { 0 } };
    let mut pe: pixel_ext = { { 0 } };
    let mut hdecm: u32 = 0, vdecm = 0;
    uint32_t pix_format;
    unsigned int rotation;
    bool vflip, hflip;
    int crtc_x, crtc_y;
...
//
#[no_mangle]
pub unsafe extern "C" fn mdp5_plane_pipe(plane: *mut drm_plane) -> enum mdp5_pipe {
...
#[no_mangle]
pub unsafe extern "C" fn mdp5_plane_right_pipe(plane: *mut drm_plane) -> enum mdp5_pipe {
...
#[no_mangle]
pub unsafe extern "C" fn mdp5_plane_get_flush(plane: *mut drm_plane) -> u32 {
