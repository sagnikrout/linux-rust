...
//! Source: drivers/gpu/drm/msm/adreno/adreno_device.c
#![no_std]
...
//
    ...
    let mut hang_debug: bool = false;
    MODULE_PARM_DESC(hang_debug, "Dump registers when hang is detected (can be slow!)");
    module_param_named(hang_debug, hang_debug, bool, 0600);
    let mut snapshot_debugbus: bool = false;
    MODULE_PARM_DESC(snapshot_debugbus, "Include debugbus sections in GPU devcoredump (if not fused off)");
    module_param_named(snapshot_debugbus, snapshot_debugbus, bool, 0600);
    let mut enable_preemption: c_int = -1;
...
// identify gpu:
    ...
    struct msm_gpu *gpu = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn find_chipid(node: *mut device_node, chipid: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn adreno_has_gpu(node: *mut device_node) -> bool {
...
#[no_mangle]
unsafe extern "C" fn adreno_bind(dev: *mut device, master: *mut device, data: *mut c_void) -> c_int {
    static int adreno_bind(struct device *dev, struct device *master, void *data)
    {
    let mut config: static struct adreno_platform_config = {};
    const struct adreno_info *info;
...
// We shouldn't have gotten this far if we don't recognize the GPU:
    ...
    ret = dev_pm_opp_of_find_icc_paths(dev, core::ptr::null_mut());
    if (ret)
    return ret;
    return 0;
    }
    ...
    priv.gpu_pdev = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn adreno_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn adreno_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn adreno_shutdown(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn adreno_runtime_resume(dev: *mut device) -> c_int {
    static int adreno_runtime_resume(struct device *dev)
    {
    struct msm_gpu *gpu = dev_to_gpu(dev);
    let mut ret: c_int = gpu.funcs.pm_resume(gpu);
    if (!ret)
    ret = msm_perfcntr_resume(gpu);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn adreno_runtime_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn suspend_scheduler(gpu: *mut msm_gpu) {
...
#[no_mangle]
unsafe extern "C" fn resume_scheduler(gpu: *mut msm_gpu) {
...
#[no_mangle]
unsafe extern "C" fn adreno_system_suspend(dev: *mut device) -> c_int {
    static int adreno_system_suspend(struct device *dev)
    {
    struct msm_gpu *gpu = dev_to_gpu(dev);
    int remaining, ret;
...
#[no_mangle]
unsafe extern "C" fn adreno_system_resume(dev: *mut device) -> c_int {
    ...
    RUNTIME_PM_OPS(adreno_runtime_suspend, adreno_runtime_resume, core::ptr::null_mut())
    };
...
#[no_mangle]
pub unsafe extern "C" fn adreno_register() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn adreno_unregister() -> void __exit {
