...
//! Source: drivers/gpu/drm/msm/msm_rd.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct msm_rd_state {
    pub dev: *mut drm_device,
    pub open: bool,
// fifo access is synchronized on the producer side by
// write_lock.  And read_lock synchronizes the reads
//
    pub write_lock: mutex read_lock,,
    pub fifo_event: wait_queue_head_t,
    pub fifo: circ_buf,
    pub buf: [c_char; BUF_SZ],
}

#[no_mangle]
unsafe extern "C" fn rd_write(rd: *mut msm_rd_state, buf: *const c_void, sz: c_int) {
...
#[no_mangle]
unsafe extern "C" fn rd_open(inode: *mut inode, file: *mut file) -> c_int {
...
//
    gpu.funcs.get_param(gpu, core::ptr::null_mut(), MSM_PARAM_GPU_ID, &val, &zero);
    gpu_id = val;
    rd_write_section(rd, RD_GPU_ID, &gpu_id, sizeof(gpu_id));
    gpu.funcs.get_param(gpu, core::ptr::null_mut(), MSM_PARAM_CHIP_ID, &val, &zero);
...
#[no_mangle]
unsafe extern "C" fn rd_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rd_cleanup(rd: *mut msm_rd_state) {
...
#[no_mangle]
pub unsafe extern "C" fn msm_rd_debugfs_init(minor: *mut drm_minor) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn msm_rd_debugfs_cleanup(priv: *mut msm_drm_private) {
    void msm_rd_debugfs_cleanup(struct msm_drm_private *priv)
    {
    rd_cleanup(priv.rd);
    priv.rd = core::ptr::null_mut();
    rd_cleanup(priv.hangrd);
    priv.hangrd = core::ptr::null_mut();
    }
...
// called under gpu->lock
    ...
    let mut dump: bool = rd_full || (vma.flags & MSM_VMA_DUMP);
// Skip MAP_NULL/PRR VMAs:
    ...
    let mut dump: bool = rd_full || (submit.bos[i].flags & MSM_SUBMIT_BO_DUMP);
    snapshot_buf(rd, obj, submit.bos[i].iova, dump, 0, obj.size);
    }
    for (i = 0; i < submit.nr_cmds; i++) {
    uint32_t szd  = submit.cmd[i].size; /* in dwords */
    let mut idx: c_int = submit.cmd[i].idx;
    let mut dump: bool = rd_full || (submit.bos[idx].flags & MSM_SUBMIT_BO_DUMP);
// snapshot cmdstream bo's (if we haven't already):
    if (!dump) {
    struct drm_gem_object *obj = submit.bos[idx].obj;
    let mut offset: usize = submit.cmd[i].iova - submit.bos[idx].iova;
    ...
    for (i = 0; i < submit.nr_cmds; i++) {
    let mut iova: u64 = submit.cmd[i].iova;
    uint32_t szd  = submit.cmd[i].size; /* in dwords */
