...
//! Source: drivers/gpu/drm/msm/msm_iommu.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct msm_iommu {
    pub base: msm_mmu,
    pub domain: *mut iommu_domain,
    pub /: *mut *mut mutex init_lock; / protects pagetables counter and prr_page,
    pub pagetables: c_int,
    pub prr_page: *mut page,
    pub pt_cache: *mut kmem_cache,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct msm_iommu_pagetable {
    pub base: msm_mmu,
    pub parent: *mut msm_mmu,
    ...
    pub ttbr: phys_addr_t,
    pub asid: u32,
// @root_page_table: Stores the root page table pointer.
    pub root_page_table: *mut c_void,
}
...
// count = size >> pgsize_idx;
    ...
    unmapped = ops.unmap_pages(ops, iova, pgsize, count, core::ptr::null_mut());
    if (unmapped <= 0) {
    ret = -EINVAL;
//
...
#[no_mangle]
unsafe extern "C" fn msm_iommu_pagetable_map_prr(mmu: *mut msm_mmu, iova: u64, len: usize, prot: c_int) -> c_int {
    static int msm_iommu_pagetable_map_prr(struct msm_mmu *mmu, u64 iova, size_t len, int prot)
    {
    struct msm_iommu_pagetable *pagetable = to_pagetable(mmu);
    struct io_pgtable_ops *ops = pagetable.pgtbl_ops;
    struct msm_iommu *iommu = to_msm_iommu(pagetable.parent);
    let mut phys: phys_addr_t = page_to_phys(iommu.prr_page);
    let mut addr: u64 = iova;
    while (len) {
...
//
    ...
    let mut size: usize = sg.length;
    let mut phys: phys_addr_t = sg_phys(sg);
    if (!len)
    break;
    if (size <= off) {
    off -= size;
...
#[no_mangle]
unsafe extern "C" fn msm_iommu_pagetable_destroy(mmu: *mut msm_mmu) {
...
//
    ...
    adreno_smmu.set_ttbr0_cfg(adreno_smmu.cookie, core::ptr::null_mut());
    if (adreno_smmu.set_prr_bit) {
    adreno_smmu.set_prr_bit(adreno_smmu.cookie, false);
    __free_page(iommu.prr_page);
    iommu.prr_page = core::ptr::null_mut();
    }
    }
...
// asid = pagetable->asid;
    ...
    {
    struct msm_iommu_pagetable *pagetable;
    let mut wd: arm_lpae_io_pgtable_walk_data = {};
    if (mmu.type != MSM_MMU_IOMMU_PAGETABLE)
    return -EINVAL;
    pagetable = to_pagetable(mmu);
...
//
    ...
    {
    struct kmem_cache *pt_cache = get_pt_cache(mmu);
    let mut remaining_pt_count: u32 = p.count - p.ptr;
    if (!p.pages)
    return;
    if (p.count > 0)
...
// Allocation of the root page table happening during init.
    ...
    page = p ? page_address(p) : core::ptr::null_mut();
...
//
    ...
    pagetable.root_page_table = core::ptr::null_mut();
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn msm_iommu_tlb_flush_all(cookie: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn get_tblsz(cfg: *const io_pgtable_cfg) -> usize {
...
// arm_lpae_iopte is u64:
    ...
    struct msm_iommu_pagetable *pagetable;
    const struct io_pgtable_cfg *ttbr1_cfg = core::ptr::null_mut();
...
//
    ...
    struct adreno_smmu_fault_info info, *ptr = core::ptr::null_mut();
    ...
    struct msm_iommu *iommu = arg;
    if (iommu.base.handler)
    return iommu.base.handler(iommu.base.arg, iova, flags, core::ptr::null_mut());
    return -ENOSYS;
    }
#[no_mangle]
unsafe extern "C" fn msm_iommu_set_stall(mmu: *mut msm_mmu, enable: bool) {
...
#[no_mangle]
unsafe extern "C" fn msm_iommu_detach(mmu: *mut msm_mmu) {
...
#[no_mangle]
unsafe extern "C" fn msm_iommu_unmap(mmu: *mut msm_mmu, iova: u64, len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn msm_iommu_destroy(mmu: *mut msm_mmu) {
...
//
    ...
    let mut tblsz: usize = get_tblsz(cfg);
    iommu.pt_cache =
    kmem_cache_create("msm-mmu-pt", tblsz, tblsz, 0, core::ptr::null_mut());
    }
