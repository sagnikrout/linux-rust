...
//! Source: drivers/gpu/drm/drm_gpusvm.c
#![no_std]
...
// No full SVM mode, only core drm_gpusvm_pages API.
    ...
    gpusvm.lock_dep_map = core::ptr::null_mut();
...
//
    ...
    unsigned long *pfns;
    let mut npages: c_ulong = npages_in_range(start, end);
    int err, i;
...
//
    ...
    let mut start: c_ulong = drm_gpusvm_range_start(range);
    let mut end: c_ulong = drm_gpusvm_range_end(range);
    struct hmm_range hmm_range = {
    .default_flags = 0,
    .notifier = notifier,
    .start = start,
    ...
    let mut timeout: c_ulong = msecs_to_jiffies(HMM_RANGE_DEFAULT_TIMEOUT);
    let mut state: enum drm_gpusvm_scan_result = DRM_GPUSVM_SCAN_UNPOPULATED, new_state;
    unsigned long *pfns;
    let mut npages: c_ulong = npages_in_range(start, end);
    const struct dev_pagemap *other = core::ptr::null_mut();
    int err, i;
    ...
    for (i = 0; i < npages;) {
    struct page *page;
    const struct dev_pagemap *cur = core::ptr::null_mut();
    if (!(pfns[i] & HMM_PFN_VALID)) {
...
#[no_mangle]
pub unsafe extern "C" fn if(DRM_GPUSVM_SCAN_MIXED: state !=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn drm_gpusvm_driver_lock_held(gpusvm: *mut drm_gpusvm) {
...
//
    ...
    let mut addr: c_ulong = ULONG_MAX;
...
//
    ...
    struct mm_struct *mm = gpusvm.mm;
    struct vm_area_struct *vas;
    let mut notifier_alloc: bool = false;
...
//
    ...
    let mut use_iova: bool = dma_use_iova(&svm_pages.state);
//
...
// WRITE_ONCE pairs with READ_ONCE for opportunistic checks
    ...
    svm_pages.dpagemap = core::ptr::null_mut();
    }
    }
//
...
//
    ...
    svm_pages.dma_addr = core::ptr::null_mut();
    }
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn drm_gpusvm_range_destroy(refcount: *mut kref) {
...
//
    ...
    unsigned long i, j;
    let mut npages: c_ulong = npages_in_range(pages_start, pages_end);
...
// Unlock and restart mapping to allocate memory.
    ...
    zdd = core::ptr::null_mut();
    pagemap = core::ptr::null_mut();
    num_dma_mapped = 0;
    for (i = 0, j = 0; i < npages; ++j) {
...
//
    ...
    .dev_private_owner = core::ptr::null_mut(),
    };
    let mut timeout: c_ulong = msecs_to_jiffies(HMM_RANGE_DEFAULT_TIMEOUT);
    unsigned long *pfns;
    ...
    kvfree(pfns);
    mmput(mm);
    let mut err: return = = -EBUSY ? -ETIME : err;
    }
...
//
    ...
    struct drm_gpusvm_range *range = core::ptr::null_mut();
