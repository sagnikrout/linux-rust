...
//! Source: drivers/gpu/drm/panel/panel-synaptics-tddi.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tddi_panel_data {
    pub lanes: u8,
// wait timings for panel enable
    pub delay_ms_sleep_exit: u8,
    pub delay_ms_display_on: u8,
// wait timings for panel disable
    pub delay_ms_display_off: u8,
    pub delay_ms_sleep_enter: u8,
}
...
pub struct tddi_ctx {
    ...
    pub data: *const tddi_panel_data,
    pub supplies: *mut regulator_bulk_data,
    pub reset_gpio: *mut gpio_desc,
    pub backlight_gpio: *mut gpio_desc,
}
...
#[no_mangle]
unsafe extern "C" fn tddi_update_status(backlight: *mut backlight_device) -> c_int {
    static int tddi_update_status(struct backlight_device *backlight)
    {
    struct tddi_ctx *ctx = bl_get_data(backlight);
    let mut dsi: mipi_dsi_multi_context = { .dsi = ctx.dsi };
    let mut brightness: u8 = backlight_get_brightness(backlight);
...
#[no_mangle]
unsafe extern "C" fn tddi_prepare(panel: *mut drm_panel) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tddi_unprepare(panel: *mut drm_panel) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tddi_enable(panel: *mut drm_panel) -> c_int {
    static int tddi_enable(struct drm_panel *panel)
    {
    struct tddi_ctx *ctx = to_tddi_ctx(panel);
    let mut dsi: mipi_dsi_multi_context = { .dsi = ctx.dsi };
    let mut brightness: u8 = ctx.backlight.props.brightness;
...
#[no_mangle]
unsafe extern "C" fn tddi_disable(panel: *mut drm_panel) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tddi_probe(dsi: *mut mipi_dsi_device) -> c_int {
    ...
    ret = of_get_drm_panel_display_mode(dev.of_node, &ctx.mode, core::ptr::null_mut());
    if (ret < 0)
...
#[no_mangle]
unsafe extern "C" fn tddi_remove(dsi: *mut mipi_dsi_device) {
