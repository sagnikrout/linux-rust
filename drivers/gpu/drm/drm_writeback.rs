...
//! Source: drivers/gpu/drm/drm_writeback.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn create_writeback_properties(dev: *mut drm_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn delete_writeback_properties(dev: *mut drm_device) {
    ...
    dev.mode_config.writeback_pixel_formats_property = core::ptr::null_mut();
    }
    if (dev.mode_config.writeback_out_fence_ptr_property) {
    drm_property_destroy(dev, dev.mode_config.writeback_out_fence_ptr_property);
    dev.mode_config.writeback_out_fence_ptr_property = core::ptr::null_mut();
    }
    if (dev.mode_config.writeback_fb_id_property) {
    drm_property_destroy(dev, dev.mode_config.writeback_fb_id_property);
    dev.mode_config.writeback_fb_id_property = core::ptr::null_mut();
    }
    }
//
...
//
    ...
    let mut ret: c_int = create_writeback_properties(dev);
    if (ret != 0)
...
//
    ...
    DRM_MODE_CONNECTOR_WRITEBACK, core::ptr::null_mut());
    if (ret)
    return ret;
    ret = __drm_writeback_connector_init(dev, wb_connector, enc, formats,
    n_formats);
...
#[no_mangle]
pub unsafe extern "C" fn drm_writeback_prepare_job(job: *mut drm_writeback_job) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn drm_writeback_cleanup_job(job: *mut drm_writeback_job) {
...
//
#[no_mangle]
unsafe extern "C" fn cleanup_work(work: *mut work_struct) {
...
//
    ...
    job.out_fence = core::ptr::null_mut();
    }
