...
//! Source: drivers/gpu/drm/vmwgfx/vmwgfx_surface.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmw_user_surface {
    pub prime: ttm_prime_object,
    pub srf: vmw_surface,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmw_surface_dirty {
    pub cache: vmw_surface_cache,
    pub num_subres: u32,
    pub __counted_by(num_subres): SVGA3dBox boxes[],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmw_surface_dma {
    pub header: SVGA3dCmdHeader,
    pub body: SVGA3dCmdSurfaceDMA,
    pub cb: SVGA3dCopyBox,
    pub suffix: SVGA3dCmdSurfaceDMASuffix,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmw_surface_define {
    pub header: SVGA3dCmdHeader,
    pub body: SVGA3dCmdDefineSurface,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmw_surface_destroy {
    pub header: SVGA3dCmdHeader,
    pub body: SVGA3dCmdDestroySurface,
}
...
//
#[no_mangle]
pub unsafe extern "C" fn vmw_surface_dma_size(srf: *const vmw_surface) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn vmw_surface_define_size(srf: *const vmw_surface) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn vmw_surface_destroy_size() -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn vmw_hw_surface_destroy(res: *mut vmw_resource) {
...
//
#[no_mangle]
unsafe extern "C" fn vmw_legacy_srf_create(res: *mut vmw_resource) -> c_int {
    static int vmw_legacy_srf_create(struct vmw_resource *res)
    {
    struct vmw_private *dev_priv = res.dev_priv;
...
//
    (void) vmw_execbuf_fence_commands(core::ptr::null_mut(), dev_priv,
    &fence, core::ptr::null_mut());
    vmw_bo_fence_single(val_buf.bo, fence);
    if (likely(fence != core::ptr::null_mut()))
...
//
#[no_mangle]
unsafe extern "C" fn vmw_legacy_srf_destroy(res: *mut vmw_resource) -> c_int {
    static int vmw_legacy_srf_destroy(struct vmw_resource *res)
    {
    struct vmw_private *dev_priv = res.dev_priv;
    uint32_t submit_size;
...
//
#[no_mangle]
unsafe extern "C" fn vmw_user_surface_free(res: *mut vmw_resource) {
    static void vmw_user_surface_free(struct vmw_resource *res)
    {
    struct vmw_surface *srf = vmw_res_to_srf(res);
...
//
#[no_mangle]
unsafe extern "C" fn vmw_user_surface_base_release(p_base: *mut ttm_base_object) {
...
//
    ...
    u32 handle)
    {
    struct vmw_user_surface *user_srf = core::ptr::null_mut();
    ...
    struct vmw_surface *surf = core::ptr::null_mut();
    struct ttm_base_object *base;
    if (user_srf) {
    ...
    base = &user_srf.prime.base;
    ret = ttm_ref_object_add(tfile, base, core::ptr::null_mut(), false);
    if (ret) {
    drm_warn(&dev_priv.drm,
...
// base_p = base;
    ...
    if (handle_type != DRM_VMW_HANDLE_PRIME) {
    let mut require_exist: bool = false;
...
//
    ...
    require_exist = true;
    ret = ttm_ref_object_add(tfile, base, core::ptr::null_mut(), require_exist);
    if (unlikely(ret != 0)) {
...
//
#[no_mangle]
unsafe extern "C" fn vmw_gb_surface_create(res: *mut vmw_resource) -> c_int {
    static int vmw_gb_surface_create(struct vmw_resource *res)
    {
    struct vmw_private *dev_priv = res.dev_priv;
    struct vmw_surface *srf = vmw_res_to_srf(res);
...
#[no_mangle]
unsafe extern "C" fn vmw_gb_surface_destroy(res: *mut vmw_resource) -> c_int {
    static int vmw_gb_surface_destroy(struct vmw_resource *res)
    {
    struct vmw_private *dev_priv = res.dev_priv;
    struct vmw_surface *srf = vmw_res_to_srf(res);
...
//
    ...
    struct vmw_private *dev_priv = vmw_priv(dev);
    struct vmw_user_surface *user_srf;
    let mut metadata: vmw_surface_metadata = {0};
    struct vmw_surface *srf;
    struct vmw_resource *res;
    struct vmw_resource *tmp;
    let mut ret: c_int = 0;
    let mut backup_handle: u32 = 0;
...
// Define a surface based on the parameters.
    ...
    ttm_bo_reserve(&backup.tbo, false, false, core::ptr::null_mut());
...
//
    ...
    let mut mip: u32 = loc_start.sub_resource % cache.num_mip_levels;
    const struct drm_vmw_size *size = &cache.mip[mip].size;
    let mut box_c2: u32 = box.z + box.d;
...
//
#[no_mangle]
unsafe extern "C" fn vmw_subres_dirty_full(dirty: *mut vmw_surface_dirty, subres: u32) {
    static void vmw_subres_dirty_full(struct vmw_surface_dirty *dirty, u32 subres)
    {
    const struct vmw_surface_cache *cache = &dirty.cache;
    let mut mip: u32 = subres % cache.num_mip_levels;
    const struct drm_vmw_size *size = &cache.mip[mip].size;
...
//
    ...
    let mut backup_end: usize = res.guest_memory_offset + res.guest_memory_size;
...
//
    ...
    (struct vmw_surface_dirty *) res.dirty;
    const struct vmw_surface_cache *cache = &dirty.cache;
    let mut backup_end: usize = res.guest_memory_offset + cache.mip_chain_bytes;
...
//
#[no_mangle]
unsafe extern "C" fn vmw_surface_dirty_sync(res: *mut vmw_resource) -> c_int {
    static int vmw_surface_dirty_sync(struct vmw_resource *res)
    {
    struct vmw_private *dev_priv = res.dev_priv;
    u32 i, num_dirty;
...
//
#[no_mangle]
unsafe extern "C" fn vmw_surface_dirty_alloc(res: *mut vmw_resource) -> c_int {
    static int vmw_surface_dirty_alloc(struct vmw_resource *res)
    {
    struct vmw_surface *srf = vmw_res_to_srf(res);
    const struct vmw_surface_metadata *metadata = &srf.metadata;
    struct vmw_surface_dirty *dirty;
    let mut num_layers: u32 = 1;
    u32 num_mip;
    u32 num_subres;
    u32 num_samples;
    size_t dirty_size;
...
#[no_mangle]
pub unsafe extern "C" fn if(SVGA3D_SURFACE_CUBEMAP: metadata->flags &) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn vmw_surface_dirty_free(res: *mut vmw_resource) {
    static void vmw_surface_dirty_free(struct vmw_resource *res)
    {
    struct vmw_surface_dirty *dirty =
    (struct vmw_surface_dirty *) res.dirty;
    kvfree(dirty);
    res.dirty = core::ptr::null_mut();
    }
//
// vmw_surface_clean - The surface's clean callback
//
#[no_mangle]
unsafe extern "C" fn vmw_surface_clean(res: *mut vmw_resource) -> c_int {
    static int vmw_surface_clean(struct vmw_resource *res)
    {
    struct vmw_private *dev_priv = res.dev_priv;
    size_t alloc_size;
...
// srf_out = NULL;
    ...
    if (req.num_sizes != 1)
    return -EINVAL;
    if (req.sizes != core::ptr::null_mut())
    return -EINVAL;
    user_srf = kzalloc_obj(*user_srf);
    if (unlikely(!user_srf)) {
    ret = -ENOMEM;
...
//
    ...
    struct vmw_private *dev_priv = vmw_priv(dev);
    struct ttm_object_file *tfile = vmw_fpriv(file_priv).tfile;
    struct vmw_bo *vbo = core::ptr::null_mut();
    struct vmw_resource *res = core::ptr::null_mut();
    let mut arg: union drm_vmw_gb_surface_create_ext_arg = { 0 };
    struct drm_vmw_gb_surface_create_ext_req *req = &arg.req;
    int ret;
    ...
    let mut format: SVGA3dSurfaceFormat = vmw_format_bpp_to_svga(dev_priv, args.bpp);
...
//
    ...
    usurf.prime.base.refcount_release = core::ptr::null_mut();
    err:
    if (res)
    vmw_resource_unreference(&res);
