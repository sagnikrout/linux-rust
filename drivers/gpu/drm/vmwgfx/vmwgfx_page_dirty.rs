...
//! Source: drivers/gpu/drm/vmwgfx/vmwgfx_page_dirty.c
#![no_std]
...
// changing method?
//
pub const VMW_DIRTY_NUM_CHANGE_TRIGGERS: c_int = 2;
// Percentage to trigger a transition to the _PAGETABLE method
pub const VMW_DIRTY_PERCENTAGE: c_int = 10;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmw_bo_dirty {
    pub ref_count: kref,
    pub start: c_ulong,
    pub end: c_ulong,
    pub method: enum vmw_bo_dirty_method,
    pub change_count: c_uint,
    pub bitmap_size: c_ulong,
    pub bitmap: [c_ulong; ],
}

#[no_mangle]
pub unsafe extern "C" fn vmw_bo_is_dirty(vbo: *mut vmw_bo) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn vmw_bo_dirty_scan_pagetable(vbo: *mut vmw_bo) {
    static void vmw_bo_dirty_scan_pagetable(struct vmw_bo *vbo)
    {
    struct vmw_bo_dirty *dirty = vbo.dirty;
    let mut offset: pgoff_t = drm_vma_node_start(&vbo.tbo.base.vma_node);
...
//
#[no_mangle]
unsafe extern "C" fn vmw_bo_dirty_scan_mkwrite(vbo: *mut vmw_bo) {
    static void vmw_bo_dirty_scan_mkwrite(struct vmw_bo *vbo)
    {
    struct vmw_bo_dirty *dirty = vbo.dirty;
    let mut offset: c_ulong = drm_vma_node_start(&vbo.tbo.base.vma_node);
    struct address_space *mapping = vbo.tbo.bdev.dev_mapping;
    pgoff_t num_marked;
    ...
    let mut end: pgoff_t = dirty.bitmap_size;
...
//
#[no_mangle]
pub unsafe extern "C" fn vmw_bo_dirty_scan(vbo: *mut vmw_bo) {
...
//
#[no_mangle]
pub unsafe extern "C" fn vmw_bo_dirty_add(vbo: *mut vmw_bo) -> c_int {
    int vmw_bo_dirty_add(struct vmw_bo *vbo)
    {
    struct vmw_bo_dirty *dirty = vbo.dirty;
    let mut num_pages: pgoff_t = PFN_UP(vbo.tbo.resource.size);
    size_t size;
    int ret;
    if (dirty) {
...
#[no_mangle]
unsafe extern "C" fn vmw_bo_dirty_free(kref: *mut kref) {
...
//
#[no_mangle]
pub unsafe extern "C" fn vmw_bo_dirty_transfer_to_res(res: *mut vmw_resource) {
    void vmw_bo_dirty_transfer_to_res(struct vmw_resource *res)
    {
    struct vmw_bo *vbo = res.guest_memory_bo;
    struct vmw_bo_dirty *dirty = vbo.dirty;
    pgoff_t start, cur, end;
    let mut res_start: c_ulong = res.guest_memory_offset;
    let mut res_end: c_ulong = res.guest_memory_offset + res.guest_memory_size;
...
#[no_mangle]
pub unsafe extern "C" fn vmw_bo_dirty_clear(vbo: *mut vmw_bo) {
    void vmw_bo_dirty_clear(struct vmw_bo *vbo)
    {
    struct vmw_bo_dirty *dirty = vbo.dirty;
    pgoff_t start, cur, end;
    let mut res_start: c_ulong = 0;
    let mut res_end: c_ulong = vbo.tbo.base.size;
    WARN_ON_ONCE(res_start & ~PAGE_MASK);
    res_start >>= PAGE_SHIFT;
    res_end = DIV_ROUND_UP(res_end, PAGE_SIZE);
    if (res_start >= dirty.end || res_end <= dirty.start)
...
//
#[no_mangle]
pub unsafe extern "C" fn vmw_bo_dirty_clear_res(res: *mut vmw_resource) {
...
#[no_mangle]
pub unsafe extern "C" fn vmw_bo_vm_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
...
#[no_mangle]
pub unsafe extern "C" fn vmw_bo_vm_fault(vmf: *mut vm_fault) -> vm_fault_t {
