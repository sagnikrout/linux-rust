...
//! Source: drivers/gpu/drm/bridge/simple-bridge.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct simple_bridge {
    pub bridge: drm_bridge,
    pub connector: drm_connector,
    pub info: *const simple_bridge_info,
...
#[no_mangle]
unsafe extern "C" fn simple_bridge_get_modes(connector: *mut drm_connector) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn simple_bridge_probe(pdev: *mut platform_device) -> c_int {
...
// Get the regulator and GPIO resources.
    ...
    let mut ret: c_int = PTR_ERR(sbridge.vdd);
    if (ret == -EPROBE_DEFER)
    return -EPROBE_DEFER;
    sbridge.vdd = core::ptr::null_mut();
