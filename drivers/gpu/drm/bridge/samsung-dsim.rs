...
//! Source: drivers/gpu/drm/bridge/samsung-dsim.c
#![no_std]
...
pub const DSI_MAX_BUS_WIDTH: c_int = 4;
pub const DSI_NUM_VIRTUAL_CHANNELS: c_int = 4;
pub const DSI_TX_FIFO_SIZE: c_int = 2048;
pub const DSI_RX_FIFO_SIZE: c_int = 256;
pub const DSI_XFER_TIMEOUT_MS: c_int = 100;
pub const DSI_RX_FIFO_EMPTY: c_uint = 0x30800002;
...
#[no_mangle]
pub unsafe extern "C" fn samsung_dsim_read(dsi: *mut samsung_dsim, idx: enum reg_idx) -> u32 {
    static inline u32 samsung_dsim_read(struct samsung_dsim *dsi, enum reg_idx idx)
    {
    return readl(dsi.reg_base + dsi.driver_data.reg_ofs[idx]);
    }
#[no_mangle]
unsafe extern "C" fn samsung_dsim_wait_for_reset(dsi: *mut samsung_dsim) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_reset(dsi: *mut samsung_dsim) {
    static void samsung_dsim_reset(struct samsung_dsim *dsi)
    {
    let mut reset_val: u32 = dsi.driver_data.reg_values[RESET_TYPE];
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_enable_clock(dsi: *mut samsung_dsim) -> c_int {
    ...
    let mut bpp: c_int = mipi_dsi_pixel_format_to_bpp(dsi.format);
// m->clock is in KHz
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_set_phy_ctrl(dsi: *mut samsung_dsim) {
    ...
    let mut byte_clock: c_ulonglong = dsi.hs_clock / 8;
    if (driver_data.has_freqband)
    return;
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_disable_clock(dsi: *mut samsung_dsim) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_enable_lane(dsi: *mut samsung_dsim, lane: u32) {
    static void samsung_dsim_enable_lane(struct samsung_dsim *dsi, u32 lane)
    {
    let mut reg: u32 = samsung_dsim_read(dsi, DSIM_CONFIG_REG);
    reg |= (DSIM_NUM_OF_DATA_LANE(dsi.lanes - 1) | DSIM_LANE_EN_CLK |
    DSIM_LANE_EN(lane));
    samsung_dsim_write(dsi, DSIM_CONFIG_REG, reg);
    }
#[no_mangle]
unsafe extern "C" fn samsung_dsim_init_link(dsi: *mut samsung_dsim) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_set_display_mode(dsi: *mut samsung_dsim) {
    static void samsung_dsim_set_display_mode(struct samsung_dsim *dsi)
    {
    struct drm_display_mode *m = &dsi.mode;
    let mut num_bits_resol: c_uint = dsi.driver_data.num_bits_resol;
    let mut main_vsa_offset: c_uint = dsi.driver_data.main_vsa_offset;
    u32 reg;
    if (dsi.mode_flags & MIPI_DSI_MODE_VIDEO) {
    let mut byte_clk: u64 = dsi.hs_clock / 8;
    let mut pix_clk: u64 = m.clock * 1000;
    let mut hfp: c_int = DIV64_U64_ROUND_UP((m.hsync_start - m.hdisplay) * byte_clk, pix_clk);
    let mut hbp: c_int = DIV64_U64_ROUND_UP((m.htotal - m.hsync_end) * byte_clk, pix_clk);
    let mut hsa: c_int = DIV64_U64_ROUND_UP((m.hsync_end - m.hsync_start) * byte_clk, pix_clk);
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_set_display_enable(dsi: *mut samsung_dsim, enable: bool) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_wait_for_hdr_fifo(dsi: *mut samsung_dsim) -> c_int {
    static int samsung_dsim_wait_for_hdr_fifo(struct samsung_dsim *dsi)
    {
    let mut timeout: c_int = 2000;
    do {
    let mut reg: u32 = samsung_dsim_read(dsi, DSIM_FIFOCTRL_REG);
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_set_cmd_lpm(dsi: *mut samsung_dsim, lpm: bool) {
    static void samsung_dsim_set_cmd_lpm(struct samsung_dsim *dsi, bool lpm)
    {
    let mut v: u32 = samsung_dsim_read(dsi, DSIM_ESCMODE_REG);
    if (lpm)
    v |= DSIM_CMD_LPDT_LP;
    else
    v &= ~DSIM_CMD_LPDT_LP;
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_force_bta(dsi: *mut samsung_dsim) {
    ...
    let mut length: u16 = pkt.payload_length - xfer.tx_done;
    let mut first: bool = !xfer.tx_done;
    u32 reg;
...
// Send packet header
    ...
    let mut first: bool = !xfer.rx_done;
    struct device *dev = dsi.dev;
    u16 length;
    u32 reg;
    if (first) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_transfer_start(dsi: *mut samsung_dsim) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_transfer_finish(dsi: *mut samsung_dsim) -> bool {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_irq(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_enable_irq(dsi: *mut samsung_dsim) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_disable_irq(dsi: *mut samsung_dsim) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_init(dsi: *mut samsung_dsim) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_pixel_output_fmt_supported(fmt: u32) -> bool {
...
//
    if (dsi.lanes > 1) {
    let mut hfp: c_int = adjusted_mode.hsync_start - adjusted_mode.hdisplay;
    let mut remainder: c_int = hfp % dsi.lanes;
    if (remainder) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_te_irq_handler(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_register_te_irq(dsi: *mut samsung_dsim, dev: *mut device) -> c_int {
    ...
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ERR(dsi->te_gpio)) -> else {
    else if (IS_ERR(dsi.te_gpio))
    return dev_err_probe(dev, PTR_ERR(dsi.te_gpio), "failed to get te GPIO\n");
    te_gpio_irq = gpiod_to_irq(dsi.te_gpio);
    ret = request_threaded_irq(te_gpio_irq, samsung_dsim_te_irq_handler, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_unregister_te_irq(dsi: *mut samsung_dsim) {
    ...
    const struct samsung_dsim_plat_data *pdata = dsi.plat_data;
    struct drm_bridge *next_bridge __free(drm_bridge_put) = core::ptr::null_mut();
    struct device *dev = dsi.dev;
...
//
    ...
    next_bridge = core::ptr::null_mut(); // Inhibit the cleanup action on an ERR_PTR
    } else {
...
// The next bridge can be used by host_ops->attach
    ...
    {
    let mut ret: c_int = of_property_read_u32(np, propname, out_value);
    if (ret < 0 && !optional)
    pr_err("%pOF: failed to get '%s' property\n", np, propname);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn samsung_dsim_parse_dt(dsi: *mut samsung_dsim) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn generic_dsim_register_host(dsi: *mut samsung_dsim) -> c_int {
    static int generic_dsim_register_host(struct samsung_dsim *dsi)
    {
    return mipi_dsi_host_register(&dsi.dsi_host);
    }
#[no_mangle]
unsafe extern "C" fn generic_dsim_unregister_host(dsi: *mut samsung_dsim) {
...
#[no_mangle]
pub unsafe extern "C" fn samsung_dsim_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (dsi.irq < 0)
    return dsi.irq;
    ret = devm_request_threaded_irq(dev, dsi.irq, core::ptr::null_mut(),
    samsung_dsim_irq,
...
#[no_mangle]
pub unsafe extern "C" fn samsung_dsim_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn samsung_dsim_resume(dev: *mut device) -> c_int {
    ...
    RUNTIME_PM_OPS(samsung_dsim_suspend, samsung_dsim_resume, core::ptr::null_mut())
