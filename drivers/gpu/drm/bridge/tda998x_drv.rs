...
//! Source: drivers/gpu/drm/bridge/tda998x_drv.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tda998x_audio_route {
    pub ena_aclk: u8,
    pub mux_ap: u8,
    pub aip_clksel: u8,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tda998x_audio_settings {
    pub route: *const tda998x_audio_route,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tda998x_priv {
    pub cec: *mut i2c_client,
    pub hdmi: *mut i2c_client,
    pub mutex: mutex,
    pub rev: u16,
    pub cec_addr: u8,
    pub current_page: u8,
    pub is_on: bool,
    pub supports_infoframes: bool,
    pub sink_has_audio: bool,
    pub rgb_quant_range: enum hdmi_quantization_range,
    pub vip_cntrl_0: u8,
    pub vip_cntrl_1: u8,
    pub vip_cntrl_2: u8,
    pub tmds_clock: c_ulong,
    pub audio: tda998x_audio_settings,
    pub audio_pdev: *mut platform_device,
    pub audio_mutex: mutex,
    pub edid_mutex: mutex,
    pub wq_edid: wait_queue_head_t,
    pub wq_edid_wait: volatile int,
    pub detect_work: work_struct,
    pub edid_delay_timer: timer_list,
    pub edid_delay_waitq: wait_queue_head_t,
    pub edid_delay_active: bool,
    pub encoder: drm_encoder,
    pub bridge: drm_bridge,
    pub connector: drm_connector,
    pub audio_port_enable: [u8; AUDIO_ROUTE_NUM],
    pub cec_glue: tda9950_glue,
    pub calib: *mut gpio_desc,
    pub cec_notify: *mut cec_notifier,
}
...
pub const REG_CURPAGE: c_uint = 0xff                /* write */;
...
//
pub const REG_CEC_INTSTATUS: c_uint = 0xee		      /* read */;

pub const REG_CEC_CAL_XOSC_CTRL1: c_uint = 0xf2;

pub const REG_CEC_DES_FREQ2: c_uint = 0xf5;

pub const REG_CEC_CLK: c_uint = 0xf6;

pub const REG_CEC_FRO_IM_CLK_CTRL: c_uint = 0xfb                /* read/write */;

pub const REG_CEC_RXSHPDINTENA: c_uint = 0xfc		      /* read/write */;
pub const REG_CEC_RXSHPDINT: c_uint = 0xfd		      /* read */;

pub const REG_CEC_RXSHPDLEV: c_uint = 0xfe                /* read */;

pub const REG_CEC_ENAMODS: c_uint = 0xff                /* read/write */;

// Device versions:
pub const TDA9989N2: c_uint = 0x0101;
pub const TDA19989: c_uint = 0x0201;
pub const TDA19989N2: c_uint = 0x0202;
pub const TDA19988: c_uint = 0x0301;
    static void
...
#[no_mangle]
unsafe extern "C" fn cec_enamods(priv: *mut tda998x_priv, mods: u8, enable: bool) {
    static void cec_enamods(struct tda998x_priv *priv, u8 mods, bool enable)
    {
    let mut val: c_int = cec_read(priv, REG_CEC_ENAMODS);
    if (val < 0)
    return;
    if (enable)
    val |= mods;
...
#[no_mangle]
unsafe extern "C" fn tda998x_cec_set_calibration(priv: *mut tda998x_priv, enable: bool) {
...
//
#[no_mangle]
unsafe extern "C" fn tda998x_cec_calibration(priv: *mut tda998x_priv) {
...
#[no_mangle]
unsafe extern "C" fn tda998x_cec_hook_init(data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda998x_cec_hook_exit(data: *mut c_void) {
    static void tda998x_cec_hook_exit(void *data)
    {
    struct tda998x_priv *priv = data;
    gpiod_put(priv.calib);
    priv.calib = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn tda998x_cec_hook_open(data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda998x_cec_hook_release(data: *mut c_void) {
    ...
    let mut ret: c_int = i2c_master_send(client, buf, sizeof(buf));
    if (ret < 0) {
...
pub const MAX_WRITE_RANGE_BUF: c_int = 32;
    static void
...
//
#[no_mangle]
unsafe extern "C" fn tda998x_edid_delay_done(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn tda998x_edid_delay_start(priv: *mut tda998x_priv) {
...
#[no_mangle]
unsafe extern "C" fn tda998x_edid_delay_wait(priv: *mut tda998x_priv) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tda998x_detect_work(work: *mut work_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn tda998x_irq_thread(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn tda998x_get_adiv(priv: *mut tda998x_priv, fs: c_uint) -> u8 {
    static u8 tda998x_get_adiv(struct tda998x_priv *priv, unsigned int fs)
    {
    let mut min_audio_clk: c_ulong = fs * 128;
    let mut ser_clk: c_ulong = priv.tmds_clock * 1000;
    u8 adiv;
...
#[no_mangle]
unsafe extern "C" fn tda998x_audio_mute(priv: *mut tda998x_priv, on: bool) {
...
#[no_mangle]
unsafe extern "C" fn tda998x_configure_audio(priv: *mut tda998x_priv) {
...
//
    ...
    let mut spdif: bool = daifmt.fmt == HDMI_SPDIF;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn tda998x_audio_shutdown(dev: *mut device, data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn tda998x_conn_detect(priv: *mut tda998x_priv) -> enum drm_connector_status {
    static enum drm_connector_status tda998x_conn_detect(struct tda998x_priv *priv)
    {
    let mut val: u8 = cec_read(priv, REG_CEC_RXSHPDLEV);
...
#[no_mangle]
unsafe extern "C" fn read_edid_block(data: *mut c_void, buf: *mut u8, blk: c_uint, length: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda998x_connector_get_modes(connector: *mut drm_connector) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tda998x_bridge_detach(bridge: *mut drm_bridge) {
...
#[no_mangle]
unsafe extern "C" fn tda998x_bridge_hpd_enable(bridge: *mut drm_bridge) {
...
#[no_mangle]
unsafe extern "C" fn tda998x_bridge_hpd_disable(bridge: *mut drm_bridge) {
...
// I2C driver functions
    ...
    for (i = 0; i < size; i++) {
    unsigned int route;
    let mut afmt: u8 = be32_to_cpup(&port_data[2*i]);
    let mut ena_ap: u8 = be32_to_cpup(&port_data[2*i+1]);
    switch (afmt) {
    case TDA998x_I2S:
...
// init read EDID waitqueue and HDP work
    ...
    ret = request_threaded_irq(client.irq, core::ptr::null_mut(),
...
// enable HPD irq
    ...
    priv.cec_notify = cec_notifier_conn_register(dev, core::ptr::null_mut(), core::ptr::null_mut());
    if (!priv.cec_notify) {
    ret = -ENOMEM;
    goto free_irq;
    }
...
#[no_mangle]
unsafe extern "C" fn tda998x_remove(client: *mut i2c_client) {
