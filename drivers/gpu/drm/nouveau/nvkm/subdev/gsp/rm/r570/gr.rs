...
//! Source: drivers/gpu/drm/nouveau/nvkm/subdev/gsp/rm/r570/gr.c
#![no_std]
...
// Scrubber channel only required on TU10x.
    ...
    gr.scrubber.chid = nvkm_chid_get(device.fifo.chid, core::ptr::null_mut());
    if (gr.scrubber.chid < 0)
...
// Allocate scrubber channel.
    ...
    if (ret)
    goto done;
    ret = nvkm_vmm_new(device, 0x1000, 0, core::ptr::null_mut(), 0, core::ptr::null_mut(), "grScrubberVmm",
    &gr.scrubber.vmm);
    if (ret)
    goto done;
