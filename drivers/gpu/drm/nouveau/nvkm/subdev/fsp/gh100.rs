...
//! Source: drivers/gpu/drm/nouveau/nvkm/subdev/fsp/gh100.c
#![no_std]
...
pub const MCTP_MSG_HEADER_TYPE_VENDOR_PCI: c_uint = 0x7e;
pub const MCTP_MSG_HEADER_VENDOR_ID_NV: c_uint = 0x10de;
pub const NVDM_TYPE_COT: c_uint = 0x14;
pub const NVDM_TYPE_FSP_RESPONSE: c_uint = 0x15;
...
// Authentication related fields
    ...
    {
    struct nvkm_device *device = fsp.subdev.device;
    let mut time: c_int = 1000, ret;
    if (WARN_ON(packet_size % sizeof(u32)))
    return -EINVAL;
// Ensure any previously sent message has been consumed.
    do {
    let mut head: u32 = nvkm_rd32(device, NV_PFSP_QUEUE_HEAD(0));
    let mut tail: u32 = nvkm_rd32(device, NV_PFSP_QUEUE_TAIL(0));
    if (tail == head)
    break;
    usleep_range(1000, 2000);
    } while(time--);
