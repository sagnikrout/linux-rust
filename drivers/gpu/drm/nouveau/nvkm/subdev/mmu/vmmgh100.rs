...
//! Source: drivers/gpu/drm/nouveau/nvkm/subdev/mmu/vmmgh100.c
#![no_std]
...
//
    ...
    {
    let mut data: u64 = addr | map.type;
    while (ptes--) {
    ...
    {
    let mut data: u64 = NVDEF(NV_MMU, VER3_PTE, PCF, SPARSE);
    ...
    struct nvkm_mmu_pt *pt, u32 ptei, u32 ptes)
    {
    let mut data: u64 = NVDEF(NV_MMU, VER3_PTE, PCF, NO_VALID_4KB_PAGE);
    VMM_FO064(pt, vmm, ptei * NV_MMU_VER3_PTE__SIZE, data, ptes);
    }
    static const struct nvkm_vmm_desc_func
    gh100_vmm_desc_lpt = {
...
// data |= pt->addr;
    ...
    {
    let mut data: u64 = NVDEF(NV_MMU, VER3_DUAL_PDE, PCF_BIG, SPARSE_ATS_ALLOWED);
    ...
    let mut vol: bool = target == NVKM_MEM_TARGET_HOST;
