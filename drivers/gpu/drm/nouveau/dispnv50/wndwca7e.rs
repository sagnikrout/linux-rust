...
//! Source: drivers/gpu/drm/nouveau/dispnv50/wndwca7e.c
#![no_std]
...
//
    ...
    {
    let mut iso0_hi: u32 = upper_32_bits(asyw.image.offset[0]);
    let mut iso0_lo: u32 = lower_32_bits(asyw.image.offset[0]);
    struct nvif_push *push = &wndw.wndw.push;
    int ret, kind;
    if (asyw.image.kind)
    ...
    {
    let mut ilut_hi: u32 = upper_32_bits(asyw.xlut.i.offset);
    let mut ilut_lo: u32 = lower_32_bits(asyw.xlut.i.offset);
    struct nvif_push *push = &wndw.wndw.push;
    int ret;
    ...
    let mut ntfy_addr: u64 = disp.sync.offset + asyw.ntfy.offset;
