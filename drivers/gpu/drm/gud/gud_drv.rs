...
//! Source: drivers/gpu/drm/gud/gud_drv.c
#![no_std]
...
// Only used internally
    ...
    {
    let mut requesttype: u8 = USB_TYPE_VENDOR | USB_RECIP_INTERFACE;
    let mut ifnum: u8 = intf.cur_altsetting.desc.bInterfaceNumber;
...
#[no_mangle]
unsafe extern "C" fn gud_status_to_errno(status: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gud_usb_get_status(intf: *mut usb_interface) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn gud_usb_get(gdrm: *mut gud_device, request: u8, index: u16, buf: *mut c_void, max_len: usize) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn gud_usb_set(gdrm: *mut gud_device, request: u8, index: u16, buf: *mut c_void, len: usize) -> c_int {
    int gud_usb_set(struct gud_device *gdrm, u8 request, u16 index, void *buf, size_t len)
    {
    void *trbuf = core::ptr::null_mut();
    int ret;
    if (buf && len) {
...
//
#[no_mangle]
pub unsafe extern "C" fn gud_usb_get_u8(gdrm: *mut gud_device, request: u8, index: u16, val: *mut u8) -> c_int {
...
// Returns zero on success or negative error code on failure.
#[no_mangle]
pub unsafe extern "C" fn gud_usb_set_u8(gdrm: *mut gud_device, request: u8, val: u8) -> c_int {
    int gud_usb_set_u8(struct gud_device *gdrm, u8 request, u8 val)
    {
    return gud_usb_set(gdrm, request, 0, &val, sizeof(val));
    }
#[no_mangle]
unsafe extern "C" fn gud_plane_add_properties(gdrm: *mut gud_device) -> c_int {
    ...
    let mut prop: u16 = le16_to_cpu(properties[i].prop);
    let mut val: u64 = le64_to_cpu(properties[i].val);
    switch (prop) {
...
#[no_mangle]
unsafe extern "C" fn gud_stats_debugfs(m: *mut seq_file, data: *mut c_void) -> c_int {
    ...
    &remainder);
    let mut ratio_frac: u64 = div64_u64(remainder * 10, gdrm.stats_actual_length);
...
#[no_mangle]
unsafe extern "C" fn gud_alloc_bulk_buffer(gdrm: *mut gud_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gud_free_buffers_and_mutex(data: *mut c_void) {
    static void gud_free_buffers_and_mutex(void *data)
    {
    struct gud_device *gdrm = data;
    vfree(gdrm.compress_buf);
    gdrm.compress_buf = core::ptr::null_mut();
    sg_free_table(&gdrm.bulk_sgt);
    vfree(gdrm.bulk_buf);
    gdrm.bulk_buf = core::ptr::null_mut();
    mutex_destroy(&gdrm.ctrl_lock);
    }
#[no_mangle]
unsafe extern "C" fn gud_probe(intf: *mut usb_interface, id: *const usb_device_id) -> c_int {
    static int gud_probe(struct usb_interface *intf, const struct usb_device_id *id)
    {
    const struct drm_format_info *xrgb8888_emulation_format = core::ptr::null_mut();
    let mut rgb565_supported: bool = false, xrgb8888_supported = false;
...
#[no_mangle]
pub unsafe extern "C" fn if(GUD_DRM_FORMAT_XRGB1111: format ==) -> else {
...
// Pipeline init
    ...
    ret = drm_crtc_init_with_planes(drm, &gdrm.crtc, &gdrm.plane, core::ptr::null_mut(),
    &gud_crtc_funcs, core::ptr::null_mut());
    if (ret)
    return ret;
    ...
    drm_debugfs_add_file(drm, "stats", gud_stats_debugfs, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn gud_disconnect(interface: *mut usb_interface) {
...
#[no_mangle]
unsafe extern "C" fn gud_suspend(intf: *mut usb_interface, message: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gud_resume(intf: *mut usb_interface) -> c_int {
