...
//! Source: drivers/gpu/drm/gud/gud_pipe.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn gud_is_big_endian() -> bool {
    ...
    {
    let mut block_width: c_uint = drm_format_info_block_width(format, 0);
    let mut bits_per_pixel: c_uint = 8 / block_width;
...
// Start on a byte boundary
    ...
    drm_fb_xrgb8888_to_gray8(&dst_map, core::ptr::null_mut(), &vmap, fb, rect, fmtcnv_state);
    pix8 = buf;
    for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
    unsigned int pixpos = x % block_width; /* within byte from the left */
    let mut pixshift: c_uint = (block_width - pixpos - 1) * bits_per_pixel;
    if (!pixpos) {
    block = dst++;
// block = 0;
    }
...
// block |= pix << pixshift;
    ...
    struct drm_format_conv_state *fmtcnv_state)
    {
    let mut compression: u8 = gdrm.compression;
...
//
    ...
    drm_fb_xrgb8888_to_gray8(&dst, core::ptr::null_mut(), src, fb, rect, fmtcnv_state);
    } else if (format.format == DRM_FORMAT_RGB332) {
    drm_fb_xrgb8888_to_rgb332(&dst, core::ptr::null_mut(), src, fb, rect, fmtcnv_state);
    } else if (format.format == DRM_FORMAT_RGB565) {
    if (gud_is_big_endian()) {
    drm_fb_xrgb8888_to_rgb565be(&dst, core::ptr::null_mut(), src, fb, rect,
    fmtcnv_state);
    } else {
    drm_fb_xrgb8888_to_rgb565(&dst, core::ptr::null_mut(), src, fb, rect,
    fmtcnv_state);
    }
    } else if (format.format == DRM_FORMAT_RGB888) {
    drm_fb_xrgb8888_to_rgb888(&dst, core::ptr::null_mut(), src, fb, rect, fmtcnv_state);
    } else {
    len = gud_xrgb8888_to_color(buf, format, vaddr, fb, rect);
    }
    } else if (gud_is_big_endian() && format.cpp[0] > 1) {
    drm_fb_swab(&dst, core::ptr::null_mut(), src, fb, rect, cached_reads, fmtcnv_state);
    } else if (compression && cached_reads && pitch == fb.pitches[0]) {
// can compress directly from the framebuffer
    buf = vaddr + rect.y1 * pitch;
    } else {
    drm_fb_memcpy(&dst, core::ptr::null_mut(), src, fb, rect);
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gud_usb_bulk_context {
    pub timer: timer_list,
    pub sgr: usb_sg_request,
}

#[no_mangle]
unsafe extern "C" fn gud_usb_bulk_timeout(t: *mut timer_list) {
...
#[no_mangle]
unsafe extern "C" fn gud_usb_bulk(gdrm: *mut gud_device, len: usize) -> c_int {
    ...
    ret = -ETIMEDOUT;
#[no_mangle]
pub unsafe extern "C" fn if(0: ctx.sgr.status <) -> else {
    else if (ctx.sgr.status < 0)
    ret = ctx.sgr.status;
#[no_mangle]
pub unsafe extern "C" fn if(len: ctx.sgr.bytes !=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn gud_clear_damage(gdrm: *mut gud_device) {
    ...
    {
    let mut fmtcnv_state: drm_format_conv_state = DRM_FORMAT_CONV_STATE_INIT;
...
#[no_mangle]
pub unsafe extern "C" fn gud_flush_work(work: *mut work_struct) {
...
// Imported buffers are assumed to be WriteCombined with uncached reads
    ...
    struct drm_connector_state *connector_state = core::ptr::null_mut();
...
//
    ...
    let mut prop: u16 = gdrm.properties[i];
    u64 val;
    switch (prop) {
...
// DRM UAPI matches the protocol so use value directly
    ...
    gud_usb_set(gdrm, GUD_REQ_SET_STATE_COMMIT, 0, core::ptr::null_mut(), 0);
    ...
    gdrm.shadow_buf = core::ptr::null_mut();
    mutex_unlock(&gdrm.damage_lock);
    }
