...
//! Source: drivers/gpu/drm/vc4/vc4_crtc.c
#![no_std]
...
pub const HVS_FIFO_LATENCY_PIX: c_int = 6;
...
//
    ...
    let mut channel: c_uint = vc4_crtc_state.assigned_channel;
...
#[no_mangle]
unsafe extern "C" fn vc4_get_fifo_full_level(vc4_crtc: *mut vc4_crtc, format: u32) -> u32 {
...
// size?
//
    let mut fifo_len_bytes: u32 = pv_data.fifo_depth;
//
...
//
    ...
    u32 format)
    {
    let mut level: u32 = vc4_get_fifo_full_level(vc4_crtc, format);
    let mut ret: u32 = 0;
...
#[no_mangle]
unsafe extern "C" fn vc4_crtc_pixelvalve_reset(crtc: *mut drm_crtc) {
...
// The PV needs to be disabled before it can be flushed
    ...
    let mut pixel_rep: u32 = ((mode.flags & DRM_MODE_FLAG_DBLCLK) && !is_hdmi) ? 2 : 1;
    bool is_dsi = (vc4_encoder.type == VC4_ENCODER_TYPE_DSI0 ||
    vc4_encoder.type == VC4_ENCODER_TYPE_DSI1);
    let mut is_dsi1: bool = vc4_encoder.type == VC4_ENCODER_TYPE_DSI1;
    let mut is_vec: bool = vc4_encoder.type == VC4_ENCODER_TYPE_VEC;
    let mut format: u32 = is_dsi1 ? PV_CONTROL_FORMAT_DSIV_24 : PV_CONTROL_FORMAT_24;
    let mut ppc: u8 = pv_data.pixels_per_clock;
    let mut vert_bp: u16 = mode.crtc_vtotal - mode.crtc_vsync_end;
    let mut vert_sync: u16 = mode.crtc_vsync_end - mode.crtc_vsync_start;
    let mut vert_fp: u16 = mode.crtc_vsync_start - mode.crtc_vdisplay;
    let mut debug_dump_regs: bool = false;
    int idx;
    if (!drm_dev_enter(dev, &idx))
    return;
    if (debug_dump_regs) {
    let mut p: drm_printer = drm_info_printer(&vc4_crtc.pdev.dev);
    ...
    PV_HORZB_HACTIVE));
    if (interlace) {
    let mut odd_field_first: bool = false;
    let mut field_delay: u32 = mode.htotal * pixel_rep / (2 * ppc);
    let mut vert_bp_even: u16 = vert_bp;
    let mut vert_fp_even: u16 = vert_fp;
    if (is_vec) {
...
#[no_mangle]
unsafe extern "C" fn require_hvs_enabled(dev: *mut drm_device) {
...
#[no_mangle]
pub unsafe extern "C" fn vc4_crtc_disable_at_boot(crtc: *mut drm_crtc) -> c_int {
    ...
    if (ret)
    return ret;
    ret = vc4_crtc_disable(crtc, encoder, core::ptr::null_mut(), channel);
    if (ret)
    return ret;
//
...
#[no_mangle]
pub unsafe extern "C" fn vc4_crtc_send_vblank(crtc: *mut drm_crtc) {
...
#[no_mangle]
unsafe extern "C" fn vc4_enable_vblank(crtc: *mut drm_crtc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vc4_disable_vblank(crtc: *mut drm_crtc) {
...
#[no_mangle]
unsafe extern "C" fn vc4_crtc_handle_page_flip(vc4_crtc: *mut vc4_crtc) {
    ...
    let mut chan: u32 = vc4_crtc.current_hvs_channel;
    unsigned long flags;
    spin_lock_irqsave(&dev.event_lock, flags);
    ...
    vc4_crtc.event = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn vc4_crtc_handle_vblank(crtc: *mut vc4_crtc) {
...
#[no_mangle]
unsafe extern "C" fn vc4_crtc_irq_handler(irq: c_int, data: *mut c_void) -> irqreturn_t {
    static irqreturn_t vc4_crtc_irq_handler(int irq, void *data)
    {
    struct vc4_crtc *vc4_crtc = data;
    let mut stat: u32 = CRTC_READ(PV_INTSTAT);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vc4_async_flip_state {
...
//
    ...
    struct vc4_bo *bo = core::ptr::null_mut();
    if (flip_state.old_fb) {
...
#[no_mangle]
pub unsafe extern "C" fn vc4_crtc_reset(crtc: *mut drm_crtc) {
...
#[no_mangle]
pub unsafe extern "C" fn vc4_crtc_late_register(crtc: *mut drm_crtc) -> c_int {
    ...
    .set_property = core::ptr::null_mut(),
    .cursor_set = core::ptr::null_mut(), /* handled by drm_mode_cursor_universal */
    .cursor_move = core::ptr::null_mut(), /* handled by drm_mode_cursor_universal */
...
//
    ...
    ret = drmm_crtc_init_with_planes(drm, crtc, primary_plane, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn vc4_crtc_bind(dev: *mut device, master: *mut device, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vc4_crtc_dev_probe(pdev: *mut platform_device) -> c_int {
    static int vc4_crtc_dev_probe(struct platform_device *pdev)
    {
    return component_add(&pdev.dev, &vc4_crtc_ops);
    }
#[no_mangle]
unsafe extern "C" fn vc4_crtc_dev_remove(pdev: *mut platform_device) {
