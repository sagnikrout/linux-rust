...
#[no_mangle]
unsafe extern "C" fn vc4_get_scaling_mode(src: u32, dst: u32) -> enum vc4_scaling_mode {
...
#[no_mangle]
unsafe extern "C" fn plane_enabled(state: *mut drm_plane_state) -> bool {
...
#[no_mangle]
unsafe extern "C" fn vc4_plane_release_upm_ida(hvs: *mut vc4_hvs, upm_handle: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn vc4_plane_reset(plane: *mut drm_plane) {
...
#[no_mangle]
unsafe extern "C" fn vc4_dlist_counter_increment(vc4_state: *mut vc4_plane_state) {
    static void vc4_dlist_counter_increment(struct vc4_plane_state *vc4_state)
    {
    if (vc4_state.dlist_count == vc4_state.dlist_size) {
    let mut new_size: u32 = max(4u, vc4_state.dlist_count * 2);
...
#[no_mangle]
unsafe extern "C" fn vc4_dlist_write(vc4_state: *mut vc4_plane_state, val: u32) {
    static void vc4_dlist_write(struct vc4_plane_state *vc4_state, u32 val)
    {
    let mut idx: c_uint = vc4_state.dlist_count;
...
//
#[no_mangle]
unsafe extern "C" fn vc4_get_scl_field(state: *mut drm_plane_state, plane: c_int) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn vc4_plane_margins_adj(pstate: *mut drm_plane_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vc4_plane_setup_clipping_and_scaling(state: *mut drm_plane_state) -> c_int {
    static int vc4_plane_setup_clipping_and_scaling(struct drm_plane_state *state)
    {
    struct vc4_plane_state *vc4_state = to_vc4_plane_state(state);
    struct drm_framebuffer *fb = state.fb;
    let mut num_planes: c_int = fb.format.num_planes;
    struct drm_crtc_state *crtc_state;
    let mut h_subsample: u32 = fb.format.hsub;
    let mut v_subsample: u32 = fb.format.vsub;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn vc4_write_tpz(vc4_state: *mut vc4_plane_state, src: u32, dst: u32) {
...
pub const PHASE_BITS: c_int = 6;
    static void vc4_write_ppf(struct vc4_plane_state *vc4_state, u32 src, u32 dst,
    u32 xy, int channel)
    {
    struct vc4_dev *vc4 = to_vc4_dev(vc4_state.base.plane.dev);
    let mut scale: u32 = src / dst;
    s32 offset, offset2;
    s32 phase;
    WARN_ON_ONCE(vc4.gen > VC4_GEN_6_D);
//
...
#[no_mangle]
unsafe extern "C" fn __vc4_lbm_size(state: *mut drm_plane_state) -> u32 {
...
// Each "word" of the LBM memory contains 2 or 4 (hvs5) pixels
    ...
    const struct drm_format_info *info = state.fb.format;
    const struct vc4_plane_state *vc4_state = to_vc4_plane_state(state);
    let mut channels_scaled: c_uint = 0;
...
#[no_mangle]
unsafe extern "C" fn __vc6_lbm_size(state: *const drm_plane_state) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn vc4_lbm_size(state: *mut drm_plane_state) -> u32 {
...
// LBM is not needed when there's no vertical scaling.
    ...
    {
    const struct vc4_plane_state *vc4_state = to_vc4_plane_state(state);
    let mut stride: c_uint = state.fb.pitches[plane];
//
// TODO: This only works for raster formats, and is sub-optimal
// for buffers with a stride aligned on 32 bytes.
//
    let mut words_per_line: c_uint = (stride + 62) / 32;
    let mut fetch_region_size: c_uint = words_per_line * 32;
    let mut buffer_lines: c_uint = 2 << vc4_state.upm_buffer_lines;
    let mut buffer_size: c_uint = fetch_region_size * buffer_lines;
...
#[no_mangle]
unsafe extern "C" fn vc4_plane_calc_load(state: *mut drm_plane_state) {
...
#[no_mangle]
unsafe extern "C" fn vc4_plane_allocate_lbm(state: *mut drm_plane_state) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(VC4_GEN_4: vc4->gen ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn vc6_plane_allocate_upm(state: *mut drm_plane_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vc6_plane_free_upm(state: *mut drm_plane_state) {
...
#[no_mangle]
unsafe extern "C" fn vc4_hvs4_get_alpha_blend_mode(state: *mut drm_plane_state) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn vc4_hvs5_get_alpha_blend_mode(state: *mut drm_plane_state) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn vc4_hvs6_get_alpha_mask_mode(state: *mut drm_plane_state) -> u32 {
...
//
    ...
    struct vc4_plane_state *vc4_state = to_vc4_plane_state(state);
    struct drm_framebuffer *fb = state.fb;
    let mut ctl0_offset: u32 = vc4_state.dlist_count;
    const struct hvs_format *format = vc4_get_hvs_format(fb.format.format);
    let mut base_format_mod: u64 = fourcc_mod_broadcom_mod(fb.modifier);
    let mut num_planes: c_int = fb.format.num_planes;
    let mut h_subsample: u32 = fb.format.hsub;
    let mut v_subsample: u32 = fb.format.vsub;
    ...
    u32 width, height;
    let mut hvs_format: u32 = format.hvs;
...
// Whole-tile offsets, mostly for setting the pitch.
    let mut tile_w_shift: u32 = fb.format.cpp[0] == 2 ? 6 : 5;
    u32 tile_h_shift = 5; /* 16 and 32bpp are 32 pixels high */
    let mut tile_w_mask: u32 = (1 << tile_w_shift) - 1;
// The height mask on 32-bit-per-pixel tiles is 63, i.e. twice
// the height (in pixels) of a 4k tile.
//
    let mut tile_h_mask: u32 = (2 << tile_h_shift) - 1;
// For T-tiled, the FB pitch is "how many bytes from one row to
// the next, such that
//
// pitch * tile_h == tile_size * tiles_per_row
//
    let mut tiles_w: u32 = fb.pitches[0] >> (tile_size_shift - tile_h_shift);
    let mut tiles_l: u32 = src_x >> tile_w_shift;
    let mut tiles_r: u32 = tiles_w - tiles_l;
    let mut tiles_t: u32 = src_y >> tile_h_shift;
// Intra-tile offsets, which modify the base address (the
// SCALER_PITCH0_TILE_Y_OFFSET tells HVS how to walk from that
// base address).
//
    let mut tile_y: u32 = (src_y >> 4) & 1;
    let mut subtile_y: u32 = (src_y >> 2) & 3;
    let mut utile_y: u32 = src_y & 3;
    let mut x_off: u32 = src_x & tile_w_mask;
    let mut y_off: u32 = src_y & tile_h_mask;
...
// Rows of tiles alternate left-to-right and right-to-left.
    ...
    let mut param: u32 = fourcc_mod_broadcom_param(fb.modifier);
...
//
    let mut remaining_pixels: u32 = src_x % 96;
    let mut aligned: u32 = remaining_pixels / 12;
    let mut last_bits: u32 = remaining_pixels % 12;
...
// Colorspace conversion words
    if (vc4_state.is_yuv) {
    let mut color_encoding: enum drm_color_encoding = state.color_encoding;
    let mut color_range: enum drm_color_range = state.color_range;
    const u32 *ccm;
...
#[no_mangle]
unsafe extern "C" fn vc6_plane_get_csc_mode(vc4_state: *mut vc4_plane_state) -> u32 {
...
//
    for (i = 0; i < num_planes; i++) {
    struct drm_gem_dma_object *bo = drm_fb_dma_get_gem_obj(fb, i);
    let mut paddr: dma_addr_t = bo.dma_addr + fb.offsets[i] + offsets[i];
// Pointer Word 0
...
#[no_mangle]
pub unsafe extern "C" fn vc4_plane_write_dlist(plane: *mut drm_plane, dlist: *mut u32 __iomem) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn vc4_plane_dlist_size(state: *const drm_plane_state) -> u32 {
...
//
#[no_mangle]
pub unsafe extern "C" fn vc4_plane_async_set_fb(plane: *mut drm_plane, fb: *mut drm_framebuffer) {
    void vc4_plane_async_set_fb(struct drm_plane *plane, struct drm_framebuffer *fb)
    {
    struct vc4_plane_state *vc4_state = to_vc4_plane_state(plane.state);
    struct drm_gem_dma_object *bo = drm_fb_dma_get_gem_obj(fb, 0);
    struct vc4_dev *vc4 = to_vc4_dev(plane.dev);
    let mut dma_addr: dma_addr_t = bo.dma_addr + fb.offsets[0];
    int idx;
    if (!drm_dev_enter(plane.dev, &idx))
    return;
...
// Support T_TILING for RGB formats only.
    ...
    &vc4_plane_funcs,
    formats, num_formats,
    modifiers, type, core::ptr::null_mut());
    if (IS_ERR(vc4_plane))
...
pub const VC4_NUM_OVERLAY_PLANES: c_int = 16;
#[no_mangle]
pub unsafe extern "C" fn vc4_plane_create_additional_planes(drm: *mut drm_device) -> c_int {
