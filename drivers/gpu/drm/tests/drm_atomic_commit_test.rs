...
//! Source: drivers/gpu/drm/tests/drm_atomic_commit_test.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_clone_mode_test {
...
pub struct drm_atomic_test_priv {
    ...
    pub encoders: [drm_encoder; 3],
    pub connectors: [drm_connector; 2],
}
...
#[no_mangle]
unsafe extern "C" fn drm_atomic_test_dummy_get_modes(connector: *mut drm_connector) -> c_int {
    ...
    enc = &priv.encoders[i];
    ret = drmm_encoder_init(drm, enc, core::ptr::null_mut(),
    ...
    DRM_MODE_CONNECTOR_DSI, core::ptr::null_mut());
    KUNIT_ASSERT_EQ(test, ret, 0);
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_check_connector_changed_modeset(test: *mut kunit) {
...
// first modeset to enable
    ...
    ret = drm_atomic_set_crtc_for_connector(old_conn_state, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_check_in_clone_mode(test: *mut kunit) {
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_check_valid_clones(test: *mut kunit) {
    ...
    drm_modeset_acquire_init(&ctx, 0);
    retry_set_up:
    ret = set_up_atomic_state(test, priv, core::ptr::null_mut(), &ctx);
    if (ret == -EDEADLK) {
    ret = drm_modeset_backoff(&ctx);
    if (!ret)
    goto retry_set_up;
