...
//! Source: drivers/gpu/drm/tests/drm_framebuffer_test.c
#![no_std]
...
pub const MIN_WIDTH: c_int = 4;
pub const MAX_WIDTH: c_int = 4096;
pub const MIN_HEIGHT: c_int = 4;
pub const MAX_HEIGHT: c_int = 4096;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_framebuffer_test {
    pub buffer_created: c_int,
    pub cmd: drm_mode_fb_cmd2,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_framebuffer_test_priv {
    pub dev: drm_device,
...
#[no_mangle]
unsafe extern "C" fn drm_framebuffer_test_init(test: *mut kunit) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_create(test: *mut kunit) {
    ...
    drm_internal_framebuffer_create(dev, &params.cmd, core::ptr::null_mut());
    KUNIT_EXPECT_EQ(test, params.buffer_created, priv.buffer_created);
    }
#[no_mangle]
unsafe extern "C" fn drm_framebuffer_test_to_desc(t: *const drm_framebuffer_test, desc: *mut c_char) {
...
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_modifiers_not_supported(test: *mut kunit) {
...
// A valid cmd with modifier
    ...
    fb = drm_internal_framebuffer_create(dev, &cmd, core::ptr::null_mut());
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_framebuffer_check_src_coords_case {
    pub name: *const c_char,
    pub expect: c_int,
    pub fb_size: c_uint,
    pub src_x: u32,
...
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_check_src_coords(test: *mut kunit) {
    ...
    let mut src_x: u32 = params.src_x;
    let mut src_y: u32 = params.src_y;
    let mut src_w: u32 = (params.fb_size << 16) + params.dsrc_w;
    let mut src_h: u32 = (params.fb_size << 16) + params.dsrc_h;
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_cleanup(test: *mut kunit) {
    static void drm_test_framebuffer_cleanup(struct kunit *test)
    {
    struct drm_framebuffer_test_priv *priv = test.priv;
    struct drm_device *dev = &priv.dev;
    struct list_head *fb_list = &dev.mode_config.fb_list;
    let mut format: drm_format_info = { };
    let mut fb1: drm_framebuffer = { .dev = dev, .format = &format };
    let mut fb2: drm_framebuffer = { .dev = dev, .format = &format };
// This will result on [fb_list] -> fb2 -> fb1
    drm_framebuffer_init(dev, &fb1, core::ptr::null_mut());
    drm_framebuffer_init(dev, &fb2, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_lookup(test: *mut kunit) {
    static void drm_test_framebuffer_lookup(struct kunit *test)
    {
    struct drm_framebuffer_test_priv *priv = test.priv;
    struct drm_device *dev = &priv.dev;
    let mut format: drm_format_info = { };
    let mut expected_fb: drm_framebuffer = { .dev = dev, .format = &format };
    struct drm_framebuffer *returned_fb;
    let mut id: u32 = 0;
    int ret;
    ret = drm_framebuffer_init(dev, &expected_fb, core::ptr::null_mut());
    KUNIT_ASSERT_EQ(test, ret, 0);
    id = expected_fb.base.id;
// Looking for expected_fb
    returned_fb = drm_framebuffer_lookup(dev, core::ptr::null_mut(), id);
...
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_lookup_inexistent(test: *mut kunit) {
...
// Looking for an inexistent framebuffer
    fb = drm_framebuffer_lookup(dev, core::ptr::null_mut(), id);
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_init(test: *mut kunit) {
    ...
    let mut format: drm_format_info = { };
    let mut fb1: drm_framebuffer = { .dev = dev, .format = &format };
    let mut funcs: drm_framebuffer_funcs = { };
    int ret;
...
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_init_bad_format(test: *mut kunit) {
    static void drm_test_framebuffer_init_bad_format(struct kunit *test)
    {
    struct drm_framebuffer_test_priv *priv = test.priv;
    struct drm_device *dev = &priv.dev;
    let mut fb1: drm_framebuffer = { .dev = dev, .format = core::ptr::null_mut() };
    let mut funcs: drm_framebuffer_funcs = { };
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_init_dev_mismatch(test: *mut kunit) {
    ...
    let mut format: drm_format_info = { };
    let mut fb1: drm_framebuffer = { .dev = right_dev, .format = &format };
    let mut funcs: drm_framebuffer_funcs = { };
    int ret;
...
#[no_mangle]
unsafe extern "C" fn destroy_free_mock(fb: *mut drm_framebuffer) {
...
//
#[no_mangle]
unsafe extern "C" fn drm_test_framebuffer_free(test: *mut kunit) {
...
// The framebuffer object must be unregistered
    obj = drm_mode_object_find(dev, core::ptr::null_mut(), id, DRM_MODE_OBJECT_FB);
    KUNIT_EXPECT_PTR_EQ(test, obj, core::ptr::null_mut());
