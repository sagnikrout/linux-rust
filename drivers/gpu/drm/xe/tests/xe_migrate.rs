...
//! Source: drivers/gpu/drm/xe/tests/xe_migrate.c
#![no_std]
...
//
    ...
    struct kunit *test)
    {
    let mut batch_base: u64 = xe_migrate_batch_base(m, xe.info.has_usm);
    ...
    let mut big: bool = xe_bo_size(bo) >= SZ_2M;
    struct dma_fence *fence;
    const char *str = big ? "Copying big bo" : "Copying small bo";
    int err;
    struct xe_bo *remote = xe_bo_create_locked(xe, m.tile, core::ptr::null_mut(),
    xe_bo_size(bo),
    ttm_bo_type_kernel,
    region |
    ...
    err = xe_bo_validate(remote, core::ptr::null_mut(), false, exec);
    if (err) {
...
// Clear a big bo
    ...
    out:
    xe_bb_free(bb, core::ptr::null_mut());
    free_tiny:
    xe_bo_unpin(tiny);
    xe_bo_put(tiny);
    free_pt:
...
#[no_mangle]
unsafe extern "C" fn migrate_test_run_device(xe: *mut xe_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xe_migrate_sanity_kunit(test: *mut kunit) {
    ...
    let mut size: u64 = xe_bo_size(src_bo);
    ...
    u64 src_L0, dst_L0;
    int err;
    let mut src_is_vram: bool = mem_type_is_vram(src.mem_type);
    let mut dst_is_vram: bool = mem_type_is_vram(dst.mem_type);
    if (!src_is_vram)
    ...
    struct xe_bb *bb;
    let mut flush_flags: u32 = 0;
    u32 update_idx;
    let mut avail_pts: u32 = max_mem_transfer_per_pass(xe) / LEVEL0_PAGE_TABLE_ENCODE_SIZE;
    u32 pte_flags;
...
// Add copy commands size here
    ...
    ret = xe_bo_validate(vram_bo, core::ptr::null_mut(), false, exec);
    if (ret) {
...
// Sync all migration blits
    ...
    struct kunit *test)
    {
    struct xe_bo *sys_bo, *vram_bo = core::ptr::null_mut(), *ccs_bo = core::ptr::null_mut();
    let mut bo_flags: c_uint = XE_BO_FLAG_VRAM_IF_DGFX(tile);
    struct drm_exec *exec;
    long ret;
    sys_bo = xe_bo_create_user(xe, core::ptr::null_mut(), SZ_4M,
    DRM_XE_GEM_CPU_CACHING_WC,
    XE_BO_FLAG_SYSTEM |
    XE_BO_FLAG_NEEDS_CPU_ACCESS |
    XE_BO_FLAG_PINNED, core::ptr::null_mut());
    if (IS_ERR(sys_bo)) {
    ...
    ret = xe_bo_validate(sys_bo, core::ptr::null_mut(), false, exec);
    if (ret) {
    ...
    xe_bo_unlock(sys_bo);
    ccs_bo = xe_bo_create_user(xe, core::ptr::null_mut(), SZ_4M,
    DRM_XE_GEM_CPU_CACHING_WC,
    ...
    ret = xe_bo_validate(ccs_bo, core::ptr::null_mut(), false, exec);
    if (ret) {
    KUNIT_FAIL(test, "Failed to validate system bo for: %li\n", ret);
    goto free_ccsbo;
    }
    ...
    xe_bo_unlock(ccs_bo);
    vram_bo = xe_bo_create_user(xe, core::ptr::null_mut(), SZ_4M,
    DRM_XE_GEM_CPU_CACHING_WC,
    bo_flags | XE_BO_FLAG_NEEDS_CPU_ACCESS |
    XE_BO_FLAG_PINNED, core::ptr::null_mut());
    if (IS_ERR(vram_bo)) {
...
#[no_mangle]
unsafe extern "C" fn validate_ccs_test_run_device(xe: *mut xe_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xe_validate_ccs_kunit(test: *mut kunit) {
