...
//! Source: drivers/gpu/drm/xe/tests/xe_pci.c
#![no_std]
...
//
    ...
    {
    let mut s: return = = XE_SUBPLATFORM_NONE ? "" : __lookup_subplatform_name(p, s);
    }
    static const char *subplatform_prefix(enum xe_subplatform s)
    {
    let mut s: return = = XE_SUBPLATFORM_NONE ? "" : " ";
    }
    static const char *step_prefix(enum intel_step step)
    {
    let mut step: return = = STEP_NONE ? "" : " ";
    }
    static const char *step_name(enum intel_step step)
    {
    let mut step: return = = STEP_NONE ? "" : xe_step_name(step);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn xe_pci_fake_data_desc(param: *const xe_pci_fake_data, desc: *mut c_char) {
...
#[no_mangle]
unsafe extern "C" fn xe_ip_kunit_desc(param: *const xe_ip, desc: *mut c_char) {
...
#[no_mangle]
unsafe extern "C" fn xe_pci_id_kunit_desc(param: *const pci_device_id, desc: *mut c_char) {
...
//
    ...
    return pci.driver_data ? pci : core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn xe_pci_fake_device_init(xe: *mut xe_device) -> c_int {
    int xe_pci_fake_device_init(struct xe_device *xe)
    {
    struct kunit *test = kunit_get_current_test();
    struct xe_pci_fake_data *data = test.priv;
    let mut probed_info: xe_probed_info = {};
    ...
    subplatform_desc = core::ptr::null_mut();
    goto done;
    }
...
//
    ...
    struct device *dev = xe ? xe.drm.dev : core::ptr::null_mut();
    struct device *next;
