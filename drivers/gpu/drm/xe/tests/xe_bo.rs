...
//! Source: drivers/gpu/drm/xe/tests/xe_bo.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct page_size_alloc_saved {
    pub mode: enum xe_page_size_alloc_ctrl_mode,
    pub cur_index: u32,
}
...
pub struct leaf_info {
    ...
    mutex_unlock(&xe.page_size_alloc_ctrl.lock);
    bo = xe_bo_create_user(xe, core::ptr::null_mut(), li.alloc_size,
    DRM_XE_GEM_CPU_CACHING_WC,
    XE_BO_FLAG_VRAM0, core::ptr::null_mut());
    if (IS_ERR(bo)) {
...
#[no_mangle]
unsafe extern "C" fn xe_bo_page_size_alloc_only_2m(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn xe_bo_page_size_alloc_only_1g(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn xe_bo_page_size_alloc_mixed_bos(test: *mut kunit) {
...
// slot.
//
    bo = xe_bo_create_user(xe, core::ptr::null_mut(), SZ_1G,
    DRM_XE_GEM_CPU_CACHING_WC,
    XE_BO_FLAG_VRAM0, core::ptr::null_mut());
    if (IS_ERR(bo)) {
    let mut err: c_int = PTR_ERR(bo);
    page_size_alloc_restore(xe, &saved);
    if (err == -ENOSPC) {
    kunit_skip(test,
...
// Move bo to VRAM if not already there.
    ret = xe_bo_validate(bo, core::ptr::null_mut(), false, exec);
    if (ret) {
...
// Check last CCS value, or at least last value in page.
    ...
    int ret;
// TODO: Sanity check
    let mut bo_flags: c_uint = XE_BO_FLAG_VRAM_IF_DGFX(tile);
    struct drm_exec *exec = XE_VALIDATION_OPT_OUT;
    if (IS_DGFX(xe))
    kunit_info(test, "Testing vram id %u\n", tile.id);
    else
    kunit_info(test, "Testing system memory\n");
    bo = xe_bo_create_user(xe, core::ptr::null_mut(), SZ_1M, DRM_XE_GEM_CPU_CACHING_WC,
    bo_flags, exec);
    if (IS_ERR(bo)) {
...
#[no_mangle]
unsafe extern "C" fn ccs_test_run_device(xe: *mut xe_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xe_ccs_migrate_kunit(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn evict_test_run_tile(xe: *mut xe_device, tile: *mut xe_tile, test: *mut kunit) -> c_int {
    ...
    external = xe_bo_create_user(xe, core::ptr::null_mut(), 0x10000,
    DRM_XE_GEM_CPU_CACHING_WC,
    bo_flags, core::ptr::null_mut());
    if (IS_ERR(external)) {
...
//
    ...
    xe_bo_lock(external, false);
    err = xe_bo_validate(external, core::ptr::null_mut(), false, exec);
    xe_bo_unlock(external);
    if (err) {
...
#[no_mangle]
unsafe extern "C" fn evict_test_run_device(xe: *mut xe_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xe_bo_evict_kunit(test: *mut kunit) {
...
pub struct xe_bo_link {
    ...
    let mut ret: c_int = ttm_bo_vmap(&bo.ttm, &map);
    size_t __maybe_unused i;
    if (ret)
    return ret;
    for (i = 0; i < bo.ttm.base.size; i += sizeof(u32)) {
    let mut val: u32 = prandom_u32_state(state);
...
//
#[no_mangle]
unsafe extern "C" fn shrink_test_run_device(xe: *mut xe_device) -> c_int {
    ...
    let mut interrupted: c_uint = 0, successful = 0, count = 0;
...
// We can create bos using WC caching here. But it is slower.
    bo = xe_bo_create_user(xe, core::ptr::null_mut(), XE_BO_SHRINK_SIZE,
    DRM_XE_GEM_CPU_CACHING_WB,
    XE_BO_FLAG_SYSTEM, core::ptr::null_mut());
    if (IS_ERR(bo)) {
...
//
    ...
    let mut ret: c_int = shrink_test_fill_random(bo, &prng, link);
    if (ret) {
    xe_bo_unlock(bo);
    xe_bo_put(bo);
...
//
    ...
    let mut ctx: static struct ttm_operation_ctx = {.interruptible = true};
    ...
    xe_bo_lock(bo, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn xe_bo_shrink_kunit(test: *mut kunit) {
