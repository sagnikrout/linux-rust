...
//! Source: drivers/gpu/drm/gma500/fbdev.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn psb_fbdev_vm_fault(vmf: *mut vm_fault) -> vm_fault_t {
    static vm_fault_t psb_fbdev_vm_fault(struct vm_fault *vmf)
    {
    struct vm_area_struct *vma = vmf.vma;
    struct fb_info *info = vma.vm_private_data;
    let mut address: c_ulong = vmf.address - (vmf.pgoff << PAGE_SHIFT);
    let mut pfn: c_ulong = info.fix.smem_start >> PAGE_SHIFT;
    let mut err: vm_fault_t = VM_FAULT_SIGBUS;
    let mut page_num: c_ulong = vma_pages(vma);
    unsigned long i;
...
//
#[no_mangle]
unsafe extern "C" fn psb_fbdev_fb_mmap(info: *mut fb_info, vma: *mut vm_area_struct) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn psb_fbdev_fb_destroy(info: *mut fb_info) {
