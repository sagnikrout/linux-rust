...
//! Source: drivers/gpu/drm/gma500/cdv_intel_display.c
#![no_std]
...
pub const CDV_LIMIT_SINGLE_LVDS_96: c_int = 0;
pub const CDV_LIMIT_SINGLE_LVDS_100: c_int = 1;
pub const CDV_LIMIT_DAC_HDMI_27: c_int = 2;
pub const CDV_LIMIT_DAC_HDMI_96: c_int = 3;
pub const CDV_LIMIT_DP_27: c_int = 4;
pub const CDV_LIMIT_DP_100: c_int = 5;
...
#[no_mangle]
pub unsafe extern "C" fn cdv_sb_read(dev: *mut drm_device, reg: u32, val: *mut u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cdv_sb_write(dev: *mut drm_device, reg: u32, val: u32) -> c_int {
    int cdv_sb_write(struct drm_device *dev, u32 reg, u32 val)
    {
    int ret;
    let mut dpio_debug: static bool = true;
    u32 temp;
    if (dpio_debug) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cdv_sb_reset(dev: *mut drm_device) {
...
//
    ...
    u32 m, n_vco, p;
    let mut ret: c_int = 0;
    let mut dpll_reg: c_int = (pipe == 0) ? DPLL_A : DPLL_B;
    let mut ref_sfr: c_int = (pipe == 0) ? SB_REF_DPLLA : SB_REF_DPLLB;
    u32 ref_value;
...
#[no_mangle]
unsafe extern "C" fn cdv_intel_clock(refclk: c_int, clock: *mut gma_clock_t) {
...
#[no_mangle]
unsafe extern "C" fn cdv_intel_pipe_enabled(dev: *mut drm_device, pipe: c_int) -> bool {
    static bool cdv_intel_pipe_enabled(struct drm_device *dev, int pipe)
    {
    struct drm_crtc *crtc;
    struct drm_psb_private *dev_priv = to_drm_psb_private(dev);
    struct gma_crtc *gma_crtc = core::ptr::null_mut();
    crtc = dev_priv.pipe_to_crtc_mapping[pipe];
    gma_crtc = to_gma_crtc(crtc);
    if (crtc.primary.fb == core::ptr::null_mut() || !gma_crtc.active)
    return false;
    return true;
    }
#[no_mangle]
pub unsafe extern "C" fn cdv_disable_sr(dev: *mut drm_device) {
...
#[no_mangle]
pub unsafe extern "C" fn cdv_update_wm(dev: *mut drm_device, crtc: *mut drm_crtc) {
...
//
#[no_mangle]
unsafe extern "C" fn cdv_intel_panel_fitter_pipe(dev: *mut drm_device) -> c_int {
...
// See if the panel fitter is in use
    ...
    int refclk;
    struct gma_clock_t clock;
    let mut dpll: u32 = 0, dspcntr, pipeconf;
    bool ok;
    let mut is_lvds: bool = false;
    let mut is_dp: bool = false;
    struct drm_connector_list_iter conn_iter;
    struct drm_connector *connector;
    const struct gma_limit_t *limit;
    let mut ddi_select: u32 = 0;
    let mut is_edp: bool = false;
...
//
    if (is_lvds) {
    let mut lvds: u32 = REG_READ(LVDS);
    lvds |=
...
// Wait for the clocks to stabilize.
    ...
    {
    let mut sdvo_pixel_multiply: c_int = adjusted_mode.clock / mode.clock;
...
#[no_mangle]
unsafe extern "C" fn i8xx_clock(refclk: c_int, clock: *mut gma_clock_t) {
