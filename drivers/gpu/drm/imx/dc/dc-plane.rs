...
//! Source: drivers/gpu/drm/imx/dc/dc-plane.c
#![no_std]
...
pub const DC_PLANE_MAX_PITCH: c_uint = 0x10000;
pub const DC_PLANE_MAX_PIX_CNT: c_int = 8192;
...
#[no_mangle]
unsafe extern "C" fn dc_plane_check_max_source_resolution(state: *mut drm_plane_state) -> c_int {
    static int dc_plane_check_max_source_resolution(struct drm_plane_state *state)
    {
    let mut src_h: c_int = drm_rect_height(&state.src) >> 16;
    let mut src_w: c_int = drm_rect_width(&state.src) >> 16;
...
#[no_mangle]
unsafe extern "C" fn dc_plane_check_fb(state: *mut drm_plane_state) -> c_int {
    static int dc_plane_check_fb(struct drm_plane_state *state)
    {
    struct drm_framebuffer *fb = state.fb;
    let mut baseaddr: dma_addr_t = drm_fb_dma_get_gem_addr(fb, state, 0);
...
#[no_mangle]
pub unsafe extern "C" fn dc_plane_init(dc_drm: *mut dc_drm_device, dc_plane: *mut dc_plane) -> c_int {
