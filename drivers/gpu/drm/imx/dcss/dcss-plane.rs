...
//! Source: drivers/gpu/drm/imx/dcss/dcss-plane.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn dcss_plane_fb_is_linear(fb: *const drm_framebuffer) -> bool {
...
#[no_mangle]
unsafe extern "C" fn dcss_plane_destroy(plane: *mut drm_plane) {
    ...
    {
    let mut linear_format: bool = !mod_present || modifier == DRM_FORMAT_MOD_LINEAR;
    let mut supported_rotation: u32 = DRM_MODE_ROTATE_0;
...
#[no_mangle]
unsafe extern "C" fn dcss_plane_is_source_size_allowed(src_w: u16, src_h: u16, pix_fmt: u32) -> bool {
    ...
    let mut is_primary_plane: bool = plane.type == DRM_PLANE_TYPE_PRIMARY;
...
#[no_mangle]
unsafe extern "C" fn dcss_plane_atomic_set_base(dcss_plane: *mut dcss_plane) {
    ...
    let mut p1_ba: c_ulong = 0, p2_ba = 0;
    if (!format.is_yuv ||
    format.format == DRM_FORMAT_NV12 ||
    format.format == DRM_FORMAT_NV21)
...
//
    ...
    format_modifiers, type, core::ptr::null_mut());
    if (ret) {
