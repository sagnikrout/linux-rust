...
//! Source: drivers/gpu/drm/ttm/ttm_backup.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn ttm_backup_shmem_idx_to_handle(idx: pgoff_t) -> c_ulong {
    static unsigned long ttm_backup_shmem_idx_to_handle(pgoff_t idx)
    {
    return (unsigned long)idx + 1;
    }
#[no_mangle]
unsafe extern "C" fn ttm_backup_handle_to_shmem_idx(handle: pgoff_t) -> pgoff_t {
...
//
#[no_mangle]
pub unsafe extern "C" fn ttm_backup_drop(backup: *mut file, handle: pgoff_t) {
    void ttm_backup_drop(struct file *backup, pgoff_t handle)
    {
    let mut start: loff_t = ttm_backup_handle_to_shmem_idx(handle);
    start <<= PAGE_SHIFT;
...
//
    ...
    let mut idx: pgoff_t = ttm_backup_handle_to_shmem_idx(handle);
...
//
    ...
    {
    struct address_space *mapping = backup.f_mapping;
    let mut nr_pages: c_int = 1 << order;
...
//
    ...
    if (IS_ERR(to_folio)) {
    let mut err: c_int = PTR_ERR(to_folio);
...
//
#[no_mangle]
pub unsafe extern "C" fn ttm_backup_fini(backup: *mut file) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ttm_backup_bytes_avail() -> u64 {
