...
//! Source: drivers/gpu/drm/virtio/virtgpu_submit.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct virtio_gpu_submit_post_dep {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct virtio_gpu_submit {
    pub post_deps: *mut virtio_gpu_submit_post_dep,
    pub num_out_syncobjs: c_uint,
    pub in_syncobjs: *mut drm_syncobj,
    pub num_in_syncobjs: c_uint,
    pub buflist: *mut virtio_gpu_object_array,
    pub exbuf: *mut drm_virtgpu_execbuffer,
    pub out_fence: *mut virtio_gpu_fence,
    pub vfpriv: *mut virtio_gpu_fpriv,
    pub vgdev: *mut virtio_gpu_device,
    ...
    {
    let mut context: u64 = submit.fence_ctx + submit.ring_idx;
    ...
    let mut syncobj_stride: usize = exbuf.syncobj_stride;
    let mut num_in_syncobjs: u32 = exbuf.num_in_syncobjs;
...
//
    ...
    let mut address: u64 = exbuf.in_syncobjs + i * syncobj_stride;
    struct dma_fence *fence;
    ...
    if (syncobjs[i])
    drm_syncobj_replace_fence(syncobjs[i], core::ptr::null_mut());
    }
    }
    static void
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_parse_post_deps(submit: *mut virtio_gpu_submit) -> c_int {
    static int virtio_gpu_parse_post_deps(struct virtio_gpu_submit *submit)
    {
    struct drm_virtgpu_execbuffer *exbuf = submit.exbuf;
    struct drm_virtgpu_execbuffer_syncobj syncobj_desc;
    struct virtio_gpu_submit_post_dep *post_deps;
    let mut num_out_syncobjs: u32 = exbuf.num_out_syncobjs;
    let mut syncobj_stride: usize = exbuf.syncobj_stride;
    let mut ret: c_int = 0, i;
    if (!num_out_syncobjs)
    return 0;
    ...
    let mut address: u64 = exbuf.out_syncobjs + i * syncobj_stride;
    memset(&syncobj_desc, 0, sizeof(syncobj_desc));
    if (copy_from_user(&syncobj_desc,
    u64_to_user_ptr(address),
    min(syncobj_stride, sizeof(syncobj_desc)))) {
    ...
    post_deps[i].chain = core::ptr::null_mut();
    } else {
    ...
    u32 ring_idx)
    {
    struct virtio_gpu_fence_event *e = core::ptr::null_mut();
    int ret;
    e = kzalloc_obj(*e);
    if (!e)
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_init_submit_buflist(submit: *mut virtio_gpu_submit) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_cleanup_submit(submit: *mut virtio_gpu_submit) {
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_submit(submit: *mut virtio_gpu_submit) {
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_complete_submit(submit: *mut virtio_gpu_submit) {
    static void virtio_gpu_complete_submit(struct virtio_gpu_submit *submit)
    {
    submit.buf = core::ptr::null_mut();
    submit.buflist = core::ptr::null_mut();
    submit.sync_file = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_wait_in_fence(submit: *mut virtio_gpu_submit) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_install_out_fence_fd(submit: *mut virtio_gpu_submit) {
...
#[no_mangle]
unsafe extern "C" fn virtio_gpu_lock_buflist(submit: *mut virtio_gpu_submit) -> c_int {
    ...
    struct virtio_gpu_device *vgdev = dev.dev_private;
    struct virtio_gpu_fpriv *vfpriv = file.driver_priv;
    let mut fence_ctx: u64 = vgdev.fence_drv.context;
    struct drm_virtgpu_execbuffer *exbuf = data;
    struct virtio_gpu_submit submit;
    let mut ring_idx: u32 = 0;
