...
//! Source: drivers/gpu/drm/display/drm_dp_mst_topology.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_dp_pending_up_req {
    pub hdr: drm_dp_sideband_msg_hdr,
    pub msg: drm_dp_sideband_msg_req_body,
    pub next: list_head,
}
    ...
    {
    let mut idx: c_int = (lct / 2) - 1;
    let mut shift: c_int = (lct % 2) ? 0 : 4;
    u8 ufp_num;
...
#[no_mangle]
unsafe extern "C" fn drm_dp_msg_header_crc4(data: *const u8, num_nibbles: usize) -> u8 {
    static u8 drm_dp_msg_header_crc4(const uint8_t *data, size_t num_nibbles)
    {
    let mut bitmask: u8 = 0x80;
    let mut bitshift: u8 = 7;
    let mut array_index: u8 = 0;
    let mut number_of_bits: c_int = num_nibbles * 4;
...
#[no_mangle]
unsafe extern "C" fn drm_dp_msg_data_crc4(data: *const u8, number_of_bytes: u8) -> u8 {
    static u8 drm_dp_msg_data_crc4(const uint8_t *data, u8 number_of_bytes)
    {
    let mut bitmask: u8 = 0x80;
    let mut bitshift: u8 = 7;
    let mut array_index: u8 = 0;
    let mut number_of_bits: c_int = number_of_bytes * 8;
...
#[no_mangle]
pub unsafe extern "C" fn drm_dp_calc_sb_hdr_size(hdr: *mut drm_dp_sideband_msg_hdr) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_crc_sideband_chunk_req(msg: *mut u8, len: u8) {
...
#[no_mangle]
unsafe extern "C" fn build_link_address(msg: *mut drm_dp_sideband_msg_tx) {
...
#[no_mangle]
unsafe extern "C" fn build_clear_payload_id_table(msg: *mut drm_dp_sideband_msg_tx) {
...
//
    ...
    let mut wait_timeout: c_ulong = msecs_to_jiffies(4000);
    let mut wait_expires: c_ulong = jiffies + wait_timeout;
    int ret;
    for (;;) {
//
...
#[no_mangle]
unsafe extern "C" fn drm_dp_free_mst_branch_device(kref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_free_mst_port(kref: *mut kref) {
...
pub const STACK_DEPTH: c_int = 8;
    static noinline void
    __topology_ref_save(struct drm_dp_mst_topology_mgr *mgr,
    struct drm_dp_mst_topology_ref_history *history,
    enum drm_dp_mst_topology_ref_type type)
    {
    struct drm_dp_mst_topology_ref_entry *entry = core::ptr::null_mut();
...
// Otherwise add one
    ...
    let mut new_len: c_int = history.len + 1;
    ...
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(entry_b->ts_nsec: entry_a->ts_nsec <) -> else {
    ...
    {
    let mut p: drm_printer = drm_dbg_printer(drm, DRM_UT_DP, DBG_PREFIX);
...
//
    ...
    topology_ref_history_cmp, core::ptr::null_mut());
    drm_printf(&p, "%s (%p) topology count reached 0, dumping history:\n",
    type_str, ptr);
    for (i = 0; i < history.len; i++) {
    const struct drm_dp_mst_topology_ref_entry *entry =
    &history.entries[i];
    let mut ts_nsec: u64 = entry.ts_nsec;
    let mut rem_nsec: u32 = do_div(ts_nsec, 1000000000);
...
#[no_mangle]
unsafe extern "C" fn drm_dp_destroy_mst_branch_device(kref: *mut kref) {
...
//
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_topology_get_mstb(mstb: *mut drm_dp_mst_branch) {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_destroy_port(kref: *mut kref) {
...
//
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_topology_get_port(port: *mut drm_dp_mst_port) {
...
//
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_topology_put_port(port: *mut drm_dp_mst_port) {
    static void drm_dp_mst_topology_put_port(struct drm_dp_mst_port *port)
    {
    topology_ref_history_lock(port.mgr);
    ...
    struct drm_dp_mst_branch *mstb)
    {
    struct drm_dp_mst_branch *rmstb = core::ptr::null_mut();
    mutex_lock(&mgr.lock);
    if (mgr.mst_primary) {
    rmstb = drm_dp_mst_topology_get_mstb_validated_locked(
    mgr.mst_primary, mstb);
    if (rmstb && !drm_dp_mst_topology_try_get_mstb(rmstb))
    rmstb = core::ptr::null_mut();
    }
    ...
    struct drm_dp_mst_port *port)
    {
    struct drm_dp_mst_port *rport = core::ptr::null_mut();
    mutex_lock(&mgr.lock);
    if (mgr.mst_primary) {
    rport = drm_dp_mst_topology_get_port_validated_locked(
    mgr.mst_primary, port);
    if (rport && !drm_dp_mst_topology_try_get_port(rport))
    rport = core::ptr::null_mut();
    }
    mutex_unlock(&mgr.lock);
    return rport;
    }
    ...
    return ret ? port : core::ptr::null_mut();
    }
    }
    return core::ptr::null_mut();
    }
...
//
    ...
    u8 *rad)
    {
    let mut parent_lct: c_int = port.parent.lct;
    let mut shift: c_int = 4;
    let mut idx: c_int = (parent_lct - 1) / 2;
    if (parent_lct > 1) {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_is_end_device(pdt: u8, mcs: bool) -> bool {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_check_mstb_guid(mstb: *mut drm_dp_mst_branch, guid: *mut guid_t) -> c_int {
    ...
    let mut shift: c_int = (i % 2) ? 0 : 4;
    let mut port_num: c_int = (mstb.rad[i / 2] >> shift) & 0xf;
...
//
    ...
    struct drm_dp_mst_port *port;
    int ret;
    let mut new_pdt: u8 = DP_PEER_DEVICE_NONE;
    let mut new_mcs: bool = 0;
    let mut created: bool = false, send_link_addr = false, changed = false;
...
// under lock
//
    drm_modeset_lock(&mgr.base.lock, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !port->input) -> else {
    else if (!port.input)
...
// put reference to this port
    ...
    u8 new_pdt;
    bool new_mcs;
    let mut dowork: bool = false, create_connector = false;
...
// Reprobe link address so we get num_sdp_streams
    ...
    drm_modeset_unlock(&mgr.base.lock);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: create_connector) -> else {
...
// find the port by iterating down
    ...
    goto out;
    for (i = 1; i < lct; i++) {
    let mut port_num: c_int = drm_dp_mst_get_ufp_num_at_lct_from_rad(i + 1, rad);
    list_for_each_entry(port, &mstb.ports, next) {
    if (port.port_num == port_num) {
    mstb = port.mstb;
    if (!mstb) {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_link_probe_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_queue_probe_work(mgr: *mut drm_dp_mst_topology_mgr) {
    ...
    {
    int ret;
    let mut regbase: c_int = up ? DP_SIDEBAND_MSG_UP_REP_BASE : DP_SIDEBAND_MSG_DOWN_REQ_BASE;
...
#[no_mangle]
unsafe extern "C" fn process_single_down_tx_qlock(mgr: *mut drm_dp_mst_topology_mgr) {
...
// Remove remote payload allocation
    let mut send_remove: bool = false;
    mutex_lock(&mgr.lock);
...
#[no_mangle]
unsafe extern "C" fn drm_dp_encode_up_ack_reply(msg: *mut drm_dp_sideband_msg_tx, req_type: u8) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_get_vc_payload_bw(link_rate: c_int, link_lane_count: c_int) -> fixed20_12 {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_topology_mgr_set_mst(mgr: *mut drm_dp_mst_topology_mgr, mst_state: bool) -> c_int {
    int drm_dp_mst_topology_mgr_set_mst(struct drm_dp_mst_topology_mgr *mgr, bool mst_state)
    {
    let mut ret: c_int = 0;
    struct drm_dp_mst_branch *mstb = core::ptr::null_mut();
    mutex_lock(&mgr.lock);
...
// add initial branch device at LCT 1
    mstb = drm_dp_add_mst_branch_device(1, core::ptr::null_mut());
    if (mstb == core::ptr::null_mut()) {
    ret = -ENOMEM;
    goto out_unlock;
    }
    mstb.mgr = mgr;
...
// disable MST on the device
    mstb = mgr.mst_primary;
    mgr.mst_primary = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_topology_queue_probe(mgr: *mut drm_dp_mst_topology_mgr) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_topology_mgr_suspend(mgr: *mut drm_dp_mst_topology_mgr) {
...
#[no_mangle]
unsafe extern "C" fn reset_msg_rx_state(msg: *mut drm_dp_sideband_msg_rx) {
...
#[no_mangle]
unsafe extern "C" fn get_msg_request_type(data: u8) -> c_int {
    ...
    let mut tx_req_type: c_int = get_msg_request_type(txmsg.msg[0]);
    let mut rx_req_type: c_int = get_msg_request_type(rxmsg.msg[0]);
    char rad_str[64];
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_handle_down_rep(mgr: *mut drm_dp_mst_topology_mgr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn primary_mstb_probing_is_done(mgr: *mut drm_dp_mst_topology_mgr) -> bool {
    static bool primary_mstb_probing_is_done(struct drm_dp_mst_topology_mgr *mgr)
    {
    let mut probing_done: bool = false;
    mutex_lock(&mgr.lock);
    ...
    let mut hotplug: bool = false, dowork = false;
    if (hdr.broadcast) {
    const guid_t *guid = core::ptr::null_mut();
    if (msg.req_type == DP_CONNECTION_STATUS_NOTIFY)
    guid = &msg.u.conn_stat.guid;
#[no_mangle]
pub unsafe extern "C" fn if(DP_RESOURCE_STATUS_NOTIFY: msg->req_type ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_up_req_work(work: *mut work_struct) {
    ...
    up_req_work);
    struct drm_dp_pending_up_req *up_req;
    let mut send_hotplug: bool = false;
    mutex_lock(&mgr.probe_lock);
    while (true) {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_handle_up_req(mgr: *mut drm_dp_mst_topology_mgr) -> c_int {
    static int drm_dp_mst_handle_up_req(struct drm_dp_mst_topology_mgr *mgr)
    {
    struct drm_dp_pending_up_req *up_req;
    struct drm_dp_mst_branch *mst_primary;
    let mut ret: c_int = 0;
    if (!drm_dp_get_one_sb_msg(mgr, true, core::ptr::null_mut()))
    goto out_clear_reply;
...
#[no_mangle]
unsafe extern "C" fn update_msg_rx_state(mgr: *mut drm_dp_mst_topology_mgr) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_hpd_irq_send_new_request(mgr: *mut drm_dp_mst_topology_mgr) {
    void drm_dp_mst_hpd_irq_send_new_request(struct drm_dp_mst_topology_mgr *mgr)
    {
    struct drm_dp_sideband_msg_tx *txmsg;
    let mut kick: bool = true;
    mutex_lock(&mgr.qlock);
    txmsg = list_first_entry_or_null(&mgr.tx_msg_downq,
    struct drm_dp_sideband_msg_tx, next);
...
//
    ...
    struct drm_dp_mst_atomic_payload *payload = core::ptr::null_mut();
    struct drm_connector_state *conn_state;
    let mut prev_slots: c_int = 0, prev_bw = 0, req_slots;
...
//
    ...
    let mut update_payload: bool = true;
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_atomic_setup_commit(state: *mut drm_atomic_commit) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_atomic_wait_for_dependencies(state: *mut drm_atomic_commit) {
...
//
    ...
    struct drm_crtc_state *crtc_state;
    struct drm_dp_mst_topology_state *mst_state = core::ptr::null_mut();
    if (new_conn_state.crtc) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_update_slots(mst_state: *mut drm_dp_mst_topology_state, link_encoding_cap: u8) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_check_act_status(mgr: *mut drm_dp_mst_topology_mgr) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_calc_pbn_mode(clock: c_int, bpp: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_kick_tx(mgr: *mut drm_dp_mst_topology_mgr) {
    static void drm_dp_mst_kick_tx(struct drm_dp_mst_topology_mgr *mgr)
    {
...
pub const DP_PAYLOAD_TABLE_SIZE: c_int = 64;
    static bool dump_dp_payload_table(struct drm_dp_mst_topology_mgr *mgr,
    char *buf)
    {
    int i;
...
#[no_mangle]
unsafe extern "C" fn drm_dp_tx_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn drm_dp_delayed_destroy_work(work: *mut work_struct) {
    static void drm_dp_delayed_destroy_work(struct work_struct *work)
    {
    struct drm_dp_mst_topology_mgr *mgr =
    container_of(work, struct drm_dp_mst_topology_mgr,
    delayed_destroy_work);
    let mut send_hotplug: bool = false, go_again;
//
...
//
    ...
    state.commit_deps = core::ptr::null_mut();
...
// failing_port = port;
    ...
    {
    struct drm_dp_mst_atomic_payload *payload;
    let mut avail_slots: c_int = mst_state.total_avail_slots, payload_count = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_add_affected_dsc_crtcs(state: *mut drm_atomic_commit, mgr: *mut drm_dp_mst_topology_mgr) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_atomic_check(state: *mut drm_atomic_commit) -> c_int {
...
//
    ...
    return old_priv_state ? to_dp_mst_topology_state(old_priv_state) : core::ptr::null_mut();
    }
...
//
    ...
    return new_priv_state ? to_dp_mst_topology_state(new_priv_state) : core::ptr::null_mut();
    }
...
//
    ...
    if (mgr.delayed_destroy_wq == core::ptr::null_mut())
    return -ENOMEM;
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_dp_mst_topology_mgr_destroy(mgr: *mut drm_dp_mst_topology_mgr) {
...
// The following will also drain any requeued work on the WQ.
    ...
    mgr.delayed_destroy_wq = core::ptr::null_mut();
    }
    mgr.dev = core::ptr::null_mut();
    mgr.aux = core::ptr::null_mut();
    drm_atomic_private_obj_fini(&mgr.base);
    mgr.funcs = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn remote_i2c_read_ok(msgs[]: i2c_msg, num: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn remote_i2c_write_ok(msgs[]: i2c_msg, num: c_int) -> bool {
    ...
    unsigned int i;
    struct drm_dp_sideband_msg_req_body msg;
    struct drm_dp_sideband_msg_tx *txmsg = core::ptr::null_mut();
    int ret;
    memset(&msg, 0, sizeof(msg));
...
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_i2c_functionality(adapter: *mut i2c_adapter) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_register_i2c_bus(port: *mut drm_dp_mst_port) -> c_int {
    static int drm_dp_mst_register_i2c_bus(struct drm_dp_mst_port *port)
    {
...
//
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_unregister_i2c_bus(port: *mut drm_dp_mst_port) {
    static void drm_dp_mst_unregister_i2c_bus(struct drm_dp_mst_port *port)
    {
...
//
#[no_mangle]
unsafe extern "C" fn drm_dp_mst_is_virtual_dpcd(port: *mut drm_dp_mst_port) -> bool {
...
//
    ...
    immediate_upstream_port = core::ptr::null_mut();
