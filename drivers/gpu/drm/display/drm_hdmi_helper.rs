...
//! Source: drivers/gpu/drm/display/drm_hdmi_helper.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn is_eotf_supported(output_eotf: u8, sink_eotf: u8) -> bool {
...
pub const FULL_COLORIMETRY_MASK: c_uint = 0x1FF;
pub const NORMAL_COLORIMETRY_MASK: c_uint = 0x3;
pub const EXTENDED_COLORIMETRY_MASK: c_uint = 0x7;
pub const EXTENDED_ACE_COLORIMETRY_MASK: c_uint = 0xF;

pub const HDMI_COLORIMETRY_NO_DATA: c_uint = 0x0;
...
//
    ...
    {
    u32 colorimetry_val;
    let mut colorimetry_index: u32 = conn_state.colorspace & FULL_COLORIMETRY_MASK;
...
//
    ...
    {
    let mut clock: c_ulonglong = mode.clock * 1000ULL;
    let mut vic: c_uint = drm_match_cea_mode(mode);
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_hdmi_acr_n_cts_entry {
    pub n: c_uint,
    pub cts: c_uint,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_hdmi_acr_data {
    pub tmds_clock_khz: c_ulong,
...
#[no_mangle]
unsafe extern "C" fn drm_hdmi_acr_find_tmds_entry(tmds_clock_khz: c_ulong) -> c_int {
...
// be a bit more tolerant, especially for the 1.001 entries
    let mut tmds_clock_khz: c_ulong = DIV_ROUND_CLOSEST_ULL(tmds_char_rate, 1000);
