...
//! Source: drivers/gpu/drm/tiny/cirrus-qemu.c
#![no_std]
...
//
pub const SEQ_INDEX: c_int = 4;
pub const SEQ_DATA: c_int = 5;
#[no_mangle]
unsafe extern "C" fn rreg_seq(cirrus: *mut cirrus_device, reg: u8) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn wreg_seq(cirrus: *mut cirrus_device, reg: u8, val: u8) {
    static void wreg_seq(struct cirrus_device *cirrus, u8 reg, u8 val)
    {
    iowrite8(reg, cirrus.mmio + SEQ_INDEX);
    iowrite8(val, cirrus.mmio + SEQ_DATA);
    }
pub const CRT_INDEX: c_uint = 0x14;
pub const CRT_DATA: c_uint = 0x15;
#[no_mangle]
unsafe extern "C" fn rreg_crt(cirrus: *mut cirrus_device, reg: u8) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn wreg_crt(cirrus: *mut cirrus_device, reg: u8, val: u8) {
    static void wreg_crt(struct cirrus_device *cirrus, u8 reg, u8 val)
    {
    iowrite8(reg, cirrus.mmio + CRT_INDEX);
    iowrite8(val, cirrus.mmio + CRT_DATA);
    }
pub const GFX_INDEX: c_uint = 0xe;
pub const GFX_DATA: c_uint = 0xf;
#[no_mangle]
unsafe extern "C" fn wreg_gfx(cirrus: *mut cirrus_device, reg: u8, val: u8) {
...
pub const VGA_DAC_MASK: c_uint = 0x06;
#[no_mangle]
unsafe extern "C" fn wreg_hdr(cirrus: *mut cirrus_device, val: u8) {
...
#[no_mangle]
unsafe extern "C" fn cirrus_set_start_address(cirrus: *mut cirrus_device, offset: u32) {
...
#[no_mangle]
unsafe extern "C" fn cirrus_pitch_set(cirrus: *mut cirrus_device, pitch: c_uint) {
...
// validate size constraints
    ...
    return -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(fb->height: fb->pitches[0] > CIRRUS_VRAM_SIZE /) -> else {
    ...
    let mut vaddr: iosys_map = IOSYS_MAP_INIT_VADDR_IOMEM(cirrus.vram);
    ...
    let mut offset: c_uint = drm_fb_clip_offset(fb.pitches[0], fb.format, &damage);
    let mut dst: iosys_map = IOSYS_MAP_INIT_OFFSET(&vaddr, offset);
...
#[no_mangle]
unsafe extern "C" fn cirrus_crtc_helper_atomic_check(crtc: *mut drm_crtc, state: *mut drm_atomic_commit) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cirrus_connector_helper_get_modes(connector: *mut drm_connector) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cirrus_pipe_init(cirrus: *mut cirrus_device) -> c_int {
    ...
    &cirrus_crtc_funcs, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn cirrus_mode_config_init(cirrus: *mut cirrus_device) -> c_int {
...
// ------------------------------------------------------------------
    ...
    if (cirrus.vram == core::ptr::null_mut())
    return -ENOMEM;
    cirrus.mmio = devm_ioremap(&pdev.dev, pci_resource_start(pdev, 1),
    pci_resource_len(pdev, 1));
    if (cirrus.mmio == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn cirrus_pci_remove(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn cirrus_pci_shutdown(pdev: *mut pci_dev) {
