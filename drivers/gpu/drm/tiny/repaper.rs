...
//! Source: drivers/gpu/drm/tiny/repaper.c
#![no_std]
...
pub const REPAPER_RID_G2_COG_ID: c_uint = 0x12;
    enum repaper_model {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct repaper_epd {
    pub drm: drm_device,
    pub pipe: drm_simple_display_pipe,
    pub mode: *const drm_display_mode,
    pub connector: drm_connector,
    pub spi: *mut spi_device,
    pub panel_on: *mut gpio_desc,
    pub border: *mut gpio_desc,
    pub discharge: *mut gpio_desc,
    pub reset: *mut gpio_desc,
    pub busy: *mut gpio_desc,
    pub thermal: *mut thermal_zone_device,
    pub height: c_uint,
    pub width: c_uint,
    pub bytes_per_scan: c_uint,
    pub channel_select: *const u8,
    pub stage_time: c_uint,
    pub factored_stage_time: c_uint,
    pub middle_scan: bool,
    pub pre_border_byte: bool,
    pub border_byte: enum repaper_epd_border_byte,
    pub line_buffer: *mut u8,
    pub current_frame: *mut c_void,
    pub cleared: bool,
    pub partial: bool,
}
    ...
    {
    void *txbuf = core::ptr::null_mut(), *rxbuf = core::ptr::null_mut();
...
// Stack allocated tx?
    ...
    {
    int ret;
    ret = repaper_spi_transfer(spi, 0x70, &reg, core::ptr::null_mut(), 1);
    if (ret)
    return ret;
    return repaper_spi_transfer(spi, 0x72, buf, core::ptr::null_mut(), len);
    }
#[no_mangle]
unsafe extern "C" fn repaper_write_val(spi: *mut spi_device, reg: u8, val: u8) -> c_int {
    static int repaper_write_val(struct spi_device *spi, u8 reg, u8 val)
    {
    return repaper_write_buf(spi, reg, &val, 1);
    }
#[no_mangle]
unsafe extern "C" fn repaper_read_val(spi: *mut spi_device, reg: u8) -> c_int {
    ...
    if (ret)
    return ret;
    ret = repaper_spi_transfer(spi, 0x73, core::ptr::null_mut(), &val, 1);
    return ret ? ret : val;
    }
#[no_mangle]
unsafe extern "C" fn repaper_read_id(spi: *mut spi_device) -> c_int {
    static int repaper_read_id(struct spi_device *spi)
    {
    int ret;
    u8 id;
    ret = repaper_spi_transfer(spi, 0x71, core::ptr::null_mut(), &id, 1);
    return ret ? ret : id;
    }
#[no_mangle]
unsafe extern "C" fn repaper_spi_mosi_low(spi: *mut spi_device) {
...
// pixels on display are numbered from 1 so even is actually bits 1,3,5,...
    ...
    if (data) {
    let mut pixels: u8 = data[b] & 0xaa;
    let mut pixel_mask: u8 = 0xff;
    u8 p1, p2, p3, p4;
    if (mask) {
...
// pixels on display are numbered from 1 so odd is actually bits 0,2,4,...
    ...
    if (data) {
    let mut pixels: u8 = data[b - 1] & 0x55;
    let mut pixel_mask: u8 = 0xff;
    if (mask) {
...
#[no_mangle]
pub unsafe extern "C" fn repaper_interleave_bits(value: u16) -> u16 {
...
// pixels on display are numbered from 1
    ...
    for (b = epd.width / 8; b > 0; b--) {
    if (data) {
    let mut pixels: u16 = repaper_interleave_bits(data[b - 1]);
    let mut pixel_mask: u16 = 0xffff;
    if (mask) {
...
// Output data to panel
    ...
    repaper_one_line(epd, line, core::ptr::null_mut(), fixed_value, core::ptr::null_mut(), stage);
    }
    ...
    0, core::ptr::null_mut(), stage);
    }
    } else {
    for (line = 0; line < epd.height; line++) {
    let mut n: usize = line * epd.width / 8;
    ...
    enum repaper_stage stage)
    {
    let mut start: u64 = local_clock();
    let mut end: u64 = start + ((u64)epd.factored_stage_time * 1000 * 1000);
    do {
...
#[no_mangle]
unsafe extern "C" fn repaper_get_temperature(epd: *mut repaper_epd) {
    ...
    factor10x = 170;
#[no_mangle]
pub unsafe extern "C" fn if(-5: temperature <=) -> else {
...
pub unsafe extern "C" fn if(5: temperature <=) -> else {
    ...
    factor10x = 80;
#[no_mangle]
pub unsafe extern "C" fn if(10: temperature <=) -> else {
    else if (temperature <= 10)
    factor10x = 40;
#[no_mangle]
pub unsafe extern "C" fn if(15: temperature <=) -> else {
    else if (temperature <= 15)
    factor10x = 30;
#[no_mangle]
pub unsafe extern "C" fn if(20: temperature <=) -> else {
    else if (temperature <= 20)
    factor10x = 20;
#[no_mangle]
pub unsafe extern "C" fn if(40: temperature <=) -> else {
...
// repaper can't do partial updates
    ...
    repaper_frame_data_repeat(epd, epd.current_frame, core::ptr::null_mut(),
    REPAPER_COMPENSATE);
    repaper_frame_data_repeat(epd, epd.current_frame, core::ptr::null_mut(),
    REPAPER_WHITE);
    repaper_frame_data_repeat(epd, buf, core::ptr::null_mut(), REPAPER_INVERSE);
    repaper_frame_data_repeat(epd, buf, core::ptr::null_mut(), REPAPER_NORMAL);
    epd.partial = true;
    } else {
...
#[no_mangle]
unsafe extern "C" fn power_off(epd: *mut repaper_epd) {
...
#[no_mangle]
unsafe extern "C" fn repaper_pipe_disable(pipe: *mut drm_simple_display_pipe) {
...
// Nothing frame
    for (line = 0; line < epd.height; line++)
    repaper_one_line(epd, 0x7fffu, core::ptr::null_mut(), 0x00, core::ptr::null_mut(),
    REPAPER_COMPENSATE);
// 2.7"
    if (epd.border) {
// Dummy line
...
#[no_mangle]
unsafe extern "C" fn repaper_connector_get_modes(connector: *mut drm_connector) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn repaper_probe(spi: *mut spi_device) -> c_int {
...
// The SPI device is used to allocate dma memory
    ...
    core::ptr::null_mut(), &epd.connector);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn repaper_remove(spi: *mut spi_device) {
...
#[no_mangle]
unsafe extern "C" fn repaper_shutdown(spi: *mut spi_device) {
