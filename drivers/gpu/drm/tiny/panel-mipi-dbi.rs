...
//! Source: drivers/gpu/drm/tiny/panel-mipi-dbi.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct panel_mipi_dbi_format {
...
#[no_mangle]
unsafe extern "C" fn panel_mipi_dbi_get_format(dev: *mut device, formats: *mut u32, bpp: *mut c_uint) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct panel_mipi_dbi_config {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct panel_mipi_dbi_commands {
    pub buf: *const u8,
    pub len: usize,
}
    ...
    let mut size: usize = fw.size, commands_len;
    let mut i: c_uint = 0;
    ...
    let mut command: u8 = config.commands[i++];
    let mut num_parameters: u8 = config.commands[i++];
    ...
    let mut command: u8 = commands.buf[i++];
    let mut num_parameters: u8 = commands.buf[i++];
    const u8 *parameters = &commands.buf[i];
    if (command == 0x00 && num_parameters == 1)
    msleep(parameters[0]);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: num_parameters) -> else {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct panel_mipi_dbi_device {
...
#[no_mangle]
unsafe extern "C" fn panel_mipi_dbi_get_mode(dbidev: *mut mipi_dbi_dev, mode: *mut drm_display_mode) -> c_int {
    ...
    int ret;
    ret = of_get_drm_panel_display_mode(dev.of_node, mode, core::ptr::null_mut());
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn panel_mipi_dbi_spi_probe(spi: *mut spi_device) -> c_int {
...
// Multiple panels can share the "dc" GPIO, but only if they are on the same SPI bus!
    ...
    ret = drm_crtc_init_with_planes(drm, crtc, plane, core::ptr::null_mut(), &panel_mipi_dbi_crtc_funcs,
    core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn panel_mipi_dbi_spi_remove(spi: *mut spi_device) {
...
#[no_mangle]
unsafe extern "C" fn panel_mipi_dbi_spi_shutdown(spi: *mut spi_device) {
    static void panel_mipi_dbi_spi_shutdown(struct spi_device *spi)
    {
    drm_atomic_helper_shutdown(spi_get_drvdata(spi));
    }
#[no_mangle]
unsafe extern "C" fn panel_mipi_dbi_pm_suspend(dev: *mut device) -> int __maybe_unused {
    static int __maybe_unused panel_mipi_dbi_pm_suspend(struct device *dev)
    {
    return drm_mode_config_helper_suspend(dev_get_drvdata(dev));
    }
#[no_mangle]
unsafe extern "C" fn panel_mipi_dbi_pm_resume(dev: *mut device) -> int __maybe_unused {
