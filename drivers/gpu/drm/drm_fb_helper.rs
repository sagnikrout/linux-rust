...
//! Source: drivers/gpu/drm/drm_fb_helper.c
#![no_std]
...
//
    ...
    let mut drm_fbdev_emulation: static bool = true;
    module_param_named(fbdev_emulation, drm_fbdev_emulation, bool, 0600);
    MODULE_PARM_DESC(fbdev_emulation,
    "Enable legacy fbdev emulation [default=true]");
    let mut drm_fbdev_overalloc: static int = CONFIG_DRM_FBDEV_OVERALLOC;
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_restore_fbdev_mode_unlocked(fb_helper: *mut drm_fb_helper, force: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_fb_helper_dpms(info: *mut fb_info, dpms_mode: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_blank(blank: c_int, info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_fb_helper_resume_worker(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn find_crtc_index_atomic(helper: *mut drm_fb_helper) -> c_int {
    static int find_crtc_index_atomic(struct drm_fb_helper *helper)
    {
    struct drm_device *dev = helper.dev;
    let mut crtc_index: c_int = -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn find_crtc_index_legacy(helper: *mut drm_fb_helper) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_fb_helper_find_crtc_index(helper: *mut drm_fb_helper) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_fb_helper_fb_dirty(helper: *mut drm_fb_helper) {
...
#[no_mangle]
unsafe extern "C" fn drm_fb_helper_damage_work(work: *mut work_struct) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_unprepare(fb_helper: *mut drm_fb_helper) {
...
#[no_mangle]
unsafe extern "C" fn drm_fb_helper_release_info(fb_helper: *mut drm_fb_helper) {
    static void drm_fb_helper_release_info(struct drm_fb_helper *fb_helper)
    {
    struct fb_info *info = fb_helper.info;
    if (!info)
    return;
    fb_helper.info = core::ptr::null_mut();
    if (info.cmap.len)
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_unregister_info(fb_helper: *mut drm_fb_helper) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_fini(fb_helper: *mut drm_fb_helper) {
    void drm_fb_helper_fini(struct drm_fb_helper *fb_helper)
    {
    if (!fb_helper)
    return;
    fb_helper.dev.fb_helper = core::ptr::null_mut();
...
//
    ...
    let mut fb_height: u32 = info.var.yres;
    let mut end: off_t = off + len;
    let mut x1: u32 = 0;
    let mut y1: u32 = off / line_length;
    let mut x2: u32 = info.var.xres;
    let mut y2: u32 = DIV_ROUND_UP(end, line_length);
...
//
    let mut bit_off: off_t = (off % line_length) * 8;
    let mut bit_end: off_t = bit_off + len * 8;
...
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_damage_range(info: *mut fb_info, off: off_t, len: usize) {
...
// Don't use in new code.
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_damage_area(info: *mut fb_info, x: u32, y: u32, width: u32, height: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_deferred_io(info: *mut fb_info, pagereflist: *mut list_head) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_set_suspend(fb_helper: *mut drm_fb_helper, suspend: bool) {
...
#[no_mangle]
unsafe extern "C" fn setcmap_pseudo_palette(cmap: *mut fb_cmap, info: *mut fb_info) -> c_int {
    ...
    let mut red: u16 = cmap.red[i];
    let mut green: u16 = cmap.green[i];
    let mut blue: u16 = cmap.blue[i];
    u32 value;
    ...
    let mut mask: u32 = (1 << info.var.transp.length) - 1;
...
#[no_mangle]
unsafe extern "C" fn setcmap_legacy(cmap: *mut fb_cmap, info: *mut fb_info) -> c_int {
    ...
    crtc.gamma_size, core::ptr::null_mut());
    if (ret)
    goto out;
    }
    out:
    ...
    let mut size: c_int = crtc.gamma_size;
    int i;
    if (!size || cmap.start + cmap.len > size)
    return ERR_PTR(-EINVAL);
    gamma_lut = drm_property_create_blob(dev, sizeof(*lut) * size, core::ptr::null_mut());
    if (IS_ERR(gamma_lut))
    return gamma_lut;
    lut = gamma_lut.data;
...
#[no_mangle]
unsafe extern "C" fn setcmap_atomic(cmap: *mut fb_cmap, info: *mut fb_info) -> c_int {
    static int setcmap_atomic(struct fb_cmap *cmap, struct fb_info *info)
    {
    struct drm_fb_helper *fb_helper = info.par;
    struct drm_device *dev = fb_helper.dev;
    struct drm_property_blob *gamma_lut = core::ptr::null_mut();
    struct drm_modeset_acquire_ctx ctx;
...
//
    ...
    core::ptr::null_mut());
    replaced |= drm_property_replace_blob(&crtc_state.ctm, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_setcmap(cmap: *mut fb_cmap, info: *mut fb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: drm_drv_uses_atomic_modeset(fb_helper->dev)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_set_par(info: *mut fb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pan_set(fb_helper: *mut drm_fb_helper, dx: c_int, dy: c_int) {
...
//
    ...
    struct drm_client_dev *client = &fb_helper.client;
    struct drm_device *dev = fb_helper.dev;
    let mut crtc_count: c_int = 0;
    struct drm_connector_list_iter conn_iter;
    struct drm_connector *connector;
    struct drm_mode_set *mode_set;
    let mut surface_format: u32 = DRM_FORMAT_INVALID;
...
// and horizontally
//
    let mut lastv: bool = true, lasth = true;
...
//
#[no_mangle]
unsafe extern "C" fn drm_fb_helper_single_fb_probe(fb_helper: *mut drm_fb_helper) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn drm_setup_crtcs_fb(fb_helper: *mut drm_fb_helper) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_initial_config(fb_helper: *mut drm_fb_helper) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_fb_helper_hotplug_event(fb_helper: *mut drm_fb_helper) -> c_int {
