...
//! Source: drivers/gpu/drm/udl/udl_transfer.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn pixel32_to_be16(pixel: u32) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn get_pixel_val16(pixel: *const u8, log_bpp: c_int) -> u16 {
...
//
    ...
    const u8 *raw_pixel_start = core::ptr::null_mut();
    const u8 *cmd_pixel_start, *cmd_pixel_end = core::ptr::null_mut();
...
// cmd++ = (uint8_t) ((dev_addr) & 0xFF);
    ...
    let mut start: *const u8 const = pixel;
    let mut repeating_pixel_val16: u16 = pixel_val16;
...
//
    ...
    let mut base16: u32 = 0 + (device_byte_offset >> log_bpp) * 2;
...
// need to finish URB at error from this function
    ...
    let mut ret: c_int = udl_submit_urb(udl, urb, len);
    if (ret)
    return ret;
    urb = udl_get_urb(udl);
    if (!urb)
