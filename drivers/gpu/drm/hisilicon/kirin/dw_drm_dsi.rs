...
//! Source: drivers/gpu/drm/hisilicon/kirin/dw_drm_dsi.c
#![no_std]
...
pub const MAX_TX_ESC_CLK: c_int = 10;

    ((x) % (y) * 10 / (y) >= 5 ? 1 : 0))
pub const PHY_REF_CLK_RATE: c_int = 19200000;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mipi_phy_params {
    pub clk_t_lpx: u32,
    pub clk_t_hs_prepare: u32,
    pub clk_t_hs_zero: u32,
    pub clk_t_hs_trial: u32,
    pub clk_t_wakeup: u32,
    pub data_t_lpx: u32,
    pub data_t_hs_prepare: u32,
    pub data_t_hs_zero: u32,
    pub data_t_hs_trial: u32,
    pub data_t_ta_go: u32,
    pub data_t_ta_get: u32,
    pub data_t_wakeup: u32,
    pub hstx_ckg_sel: u32,
    pub pll_fbd_div5f: u32,
    pub pll_fbd_div1f: u32,
    pub pll_fbd_2p: u32,
    pub pll_enbwt: u32,
    pub pll_fbd_p: u32,
    pub pll_fbd_s: u32,
    pub pll_pre_div1p: u32,
    pub pll_pre_p: u32,
    pub pll_vco_750M: u32,
    pub pll_lpf_rs: u32,
    pub pll_lpf_cs: u32,
    pub clklp2hs_time: u32,
    pub clkhs2lp_time: u32,
    pub lp2hs_time: u32,
    pub hs2lp_time: u32,
    pub clk_to_data_delay: u32,
    pub data_to_clk_delay: u32,
    pub lane_byte_clk_kHz: u32,
    pub clk_division: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct dsi_hw_ctx {
...
pub struct dw_dsi {
    ...
    pub cur_mode: drm_display_mode,
    pub ctx: *mut dsi_hw_ctx,
    pub phy: mipi_phy_params,
    pub lanes: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dsi_phy_range {
    pub min_range_kHz: u32,
    pub max_range_kHz: u32,
    pub pll_vco_750M: u32,
    pub hstx_ckg_sel: u32,
}
...
#[no_mangle]
unsafe extern "C" fn dsi_calc_phy_rate(req_kHz: u32, phy: *mut mipi_phy_params) -> u32 {
    static u32 dsi_calc_phy_rate(u32 req_kHz, struct mipi_phy_params *phy)
    {
    let mut ref_clk_ps: u32 = PHY_REF_CLK_PERIOD_PS;
    let mut tmp_kHz: u32 = req_kHz;
    let mut i: u32 = 0;
    let mut q_pll: u32 = 1;
    let mut m_pll: u32 = 0;
    let mut n_pll: u32 = 0;
...
#[no_mangle]
unsafe extern "C" fn dsi_get_dpi_color_coding(format: enum mipi_dsi_pixel_format) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn dsi_phy_tst_set(base: *mut void __iomem, reg: u32, val: u32) {
    static void dsi_phy_tst_set(void __iomem *base, u32 reg, u32 val)
    {
    let mut reg_write: u32 = 0x10000 + reg;
//
// latch reg first
//
...
#[no_mangle]
unsafe extern "C" fn dsi_set_video_mode(base: *mut void __iomem, flags: c_ulong) {
    ...
    MIPI_DSI_MODE_VIDEO_SYNC_PULSE;
    let mut non_burst_sync_event: u32 = MIPI_DSI_MODE_VIDEO;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(non_burst_sync_event: (flags & mode_mask) ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn dsi_mipi_init(dsi: *mut dw_dsi) {
    ...
    let mut bpp: u32 = mipi_dsi_pixel_format_to_bpp(dsi.format);
...
#[no_mangle]
unsafe extern "C" fn dsi_encoder_disable(encoder: *mut drm_encoder) {
...
#[no_mangle]
unsafe extern "C" fn dsi_encoder_enable(encoder: *mut drm_encoder) {
...
//
    ...
    const struct drm_display_mode *mode)
    {
    const struct drm_crtc_helper_funcs *crtc_funcs = core::ptr::null_mut();
...
// do nothing
    ...
    {
    int ret;
    let mut crtc_mask: u32 = drm_of_find_possible_crtcs(drm_dev, dev.of_node);
    if (!crtc_mask) {
...
#[no_mangle]
unsafe extern "C" fn dsi_host_init(dev: *mut device, dsi: *mut dw_dsi) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dsi_bridge_init(dev: *mut drm_device, dsi: *mut dw_dsi) -> c_int {
...
// associate the bridge to dsi encoder
    return drm_bridge_attach(encoder, bridge, core::ptr::null_mut(), 0);
    }
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn dsi_parse_dt(pdev: *mut platform_device, dsi: *mut dw_dsi) -> c_int {
