...
//! Source: drivers/gpu/drm/v3d/v3d_submit.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn v3d_job_cleanup(job: *mut v3d_job) {
...
#[no_mangle]
pub unsafe extern "C" fn v3d_job_put(job: *mut v3d_job) {
    ...
    {
    let mut has_multisync: bool = se && (se.flags & DRM_V3D_EXT_ID_MULTI_SYNC);
...
//
    ...
    struct drm_file *file_priv)
    {
    let mut submit: v3d_submit = { .v3d = to_v3d_dev(dev), .file_priv = file_priv };
    struct drm_v3d_submit_cl *args = data;
    struct drm_syncobj *sync_out = core::ptr::null_mut();
    let mut se: v3d_submit_ext = {0};
    struct v3d_bin_job *bin = core::ptr::null_mut();
    ...
    ret = v3d_get_extensions(file_priv, args.extensions, &se, core::ptr::null_mut());
    if (ret) {
...
// Reject a query CPU job whose writes would land outside their BO.
    ...
    let mut elem: u32 = copy.do_64bit ? sizeof(u64) : sizeof(u32);
...
// 3 is the three dimensions (x, y, z) of the workgroup counts.
    ...
    for (i = 0; i < 3; i++) {
    let mut uidx: u32 = indirect_csd.wg_uniform_offsets[i];
//
...
//
    ...
    let mut se: v3d_submit_ext = {0};
    struct v3d_cpu_job *cpu_job = core::ptr::null_mut();
    int ret;
    if (args.flags && !(args.flags & DRM_V3D_SUBMIT_EXTENSION)) {
    drm_dbg(dev, "Invalid flags: %d\n", args.flags);
    return -EINVAL;
...
// The CSD job was appended at jobs[1]
    ...
    if (ret)
    goto fail;
    ret = v3d_submit_jobs(&submit, core::ptr::null_mut(), &se);
    if (ret)
    goto fail_submit;
    return 0;
    fail:
    v3d_submit_cleanup_jobs(&submit);
    fail_submit:
    v3d_submit_put_post_deps(core::ptr::null_mut(), &se);
    return ret;
    }
