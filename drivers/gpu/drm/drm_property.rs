...
//! Source: drivers/gpu/drm/drm_property.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn drm_property_flags_valid(flags: u32) -> bool {
    static bool drm_property_flags_valid(u32 flags)
    {
    let mut legacy_type: u32 = flags & DRM_MODE_PROP_LEGACY_TYPE;
    let mut ext_type: u32 = flags & DRM_MODE_PROP_EXTENDED_TYPE;
...
//
    ...
    int num_values)
    {
    struct drm_property *property = core::ptr::null_mut();
    int ret;
...
//
    ...
    struct drm_property *property;
    int i, ret;
    let mut num_values: c_int = hweight64(supported_bits);
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_property_destroy(dev: *mut drm_device, property: *mut drm_property) {
...
#[no_mangle]
unsafe extern "C" fn drm_property_free_blob(kref: *mut kref) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_property_blob_put(blob: *mut drm_property_blob) {
...
//
    ...
    struct drm_property_blob *blob = core::ptr::null_mut();
    obj = __drm_mode_object_find(dev, core::ptr::null_mut(), id, DRM_MODE_OBJECT_BLOB);
    if (obj)
...
//
    ...
    {
    struct drm_property_blob *new_blob = core::ptr::null_mut();
    struct drm_property_blob *old_blob = core::ptr::null_mut();
    int ret;
    WARN_ON(replace == core::ptr::null_mut());
    old_blob = *replace;
    if (length && data) {
...
// replaced |= drm_property_replace_blob(blob, new_blob);
    ...
    if (!drm_core_check_feature(dev, DRIVER_MODESET))
    return -EOPNOTSUPP;
    blob = drm_property_create_blob(dev, out_resp.length, core::ptr::null_mut());
    if (IS_ERR(blob))
    return PTR_ERR(blob);
...
// not associated with any file_priv.
    ...
    struct drm_property_blob *blob = core::ptr::null_mut(), *bt;
...
// ref = NULL;
    ...
    let mut svalue: i64 = U642I64(value);
