...
//! Source: drivers/gpu/drm/drm_client_modeset.c
#![no_std]
...
pub const DRM_CLIENT_MAX_CLONED_CONNECTORS: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct drm_client_offset {
    pub y: int x,,
}

#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_create(client: *mut drm_client_dev) -> c_int {
    int drm_client_modeset_create(struct drm_client_dev *client)
    {
    struct drm_device *dev = client.dev;
    let mut num_crtc: c_uint = dev.mode_config.num_crtc;
    let mut max_connector_count: c_uint = 1;
...
#[no_mangle]
unsafe extern "C" fn drm_client_modeset_release(client: *mut drm_client_dev) {
    ...
    modeset.mode = core::ptr::null_mut();
    modeset.fb = core::ptr::null_mut();
    for (i = 0; i < modeset.num_connectors; i++) {
    drm_connector_put(modeset.connectors[i]);
    modeset.connectors[i] = core::ptr::null_mut();
    }
    modeset.num_connectors = 0;
    }
    }
#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_free(client: *mut drm_client_dev) {
...
#[no_mangle]
unsafe extern "C" fn drm_connector_enabled(connector: *mut drm_connector, strict: bool) -> bool {
    ...
    bool enabled[])
    {
    let mut any_enabled: bool = false;
...
// dst = src ? drm_mode_duplicate(dev, src) : NULL;
    ...
    int i;
    for (i = 0; i < count; i++)
    mode_replace(dev, &modes[i], core::ptr::null_mut());
    }
...
// try and find a 1024x768 mode on each connector
    ...
    {
    int i;
    let mut hoffset: c_int = 0, voffset = 0;
    for (i = 0; i < connector_count; i++) {
    ...
    bool enabled[], int width, int height)
    {
    let mut mask: u64 = BIT_ULL(connector_count) - 1;
    let mut conn_configured: u64 = 0;
    let mut tile_pass: c_int = 0;
    let mut num_tiled_conns: c_int = 0;
    int i;
    for (i = 0; i < connector_count; i++) {
...
// Try to read the BIOS display configuration and use it for the initial config
    ...
    bool enabled[], int width, int height)
    {
    let mut count: c_int = min_t(unsigned int, connector_count, BITS_PER_LONG);
    unsigned long conn_configured, conn_seq, mask;
    struct drm_device *dev = client.dev;
    int i;
    bool *save_enabled;
    let mut fallback: bool = true, ret = true;
    let mut num_connectors_enabled: c_int = 0;
    let mut num_connectors_detected: c_int = 0;
    let mut num_tiled_conns: c_int = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_probe(client: *mut drm_client_dev, width: c_uint, height: c_uint) -> c_int {
    int drm_client_modeset_probe(struct drm_client_dev *client, unsigned int width, unsigned int height)
    {
    struct drm_connector *connector, **connectors = core::ptr::null_mut();
    struct drm_connector_list_iter conn_iter;
    struct drm_device *dev = client.dev;
    let mut total_modes_count: c_uint = 0;
    struct drm_client_offset *offsets;
    let mut connector_count: c_uint = 0;
    const struct drm_display_mode **modes;
    struct drm_crtc **crtcs;
    int i, ret = 0;
    bool *enabled;
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_client_rotation(modeset: *mut drm_mode_set, rotation: *mut c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn drm_client_modeset_commit_atomic(client: *mut drm_client_dev, active: bool, check: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_client_modeset_commit_legacy(client: *mut drm_client_dev) -> c_int {
    ...
    ret = crtc.funcs.cursor_set2(crtc, core::ptr::null_mut(), 0, 0, 0, 0, 0);
    if (ret)
    goto out;
    } else if (crtc.funcs.cursor_set) {
    ret = crtc.funcs.cursor_set(crtc, core::ptr::null_mut(), 0, 0, 0);
    if (ret)
    goto out;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_check(client: *mut drm_client_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_commit_locked(client: *mut drm_client_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_commit(client: *mut drm_client_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn drm_client_modeset_dpms_legacy(client: *mut drm_client_dev, dpms_mode: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_dpms(client: *mut drm_client_dev, mode: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn drm_client_modeset_wait_for_vblank(client: *mut drm_client_dev, crtc_index: c_uint) -> c_int {
