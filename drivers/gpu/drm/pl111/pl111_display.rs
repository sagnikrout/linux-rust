...
//! Source: drivers/gpu/drm/pl111/pl111_display.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn pl111_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
// Clear the interrupt once done
    ...
    let mut cpp: u32 = DIV_ROUND_UP(priv.variant.fb_depth, 8);
    u64 bw;
//
...
// Only supported on the ST Micro variant
    ...
    cntl |= CNTL_LCDBPP16;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: priv->variant->st_bitmux_control) -> else {
...
#[no_mangle]
unsafe extern "C" fn pl111_display_disable(pipe: *mut drm_simple_display_pipe) {
...
// Disable
    ...
    struct drm_framebuffer *fb = pstate.fb;
    if (fb) {
    let mut addr: u32 = drm_fb_dma_get_gem_addr(fb, pstate, 0);
...
#[no_mangle]
unsafe extern "C" fn pl111_display_enable_vblank(pipe: *mut drm_simple_display_pipe) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pl111_display_disable_vblank(pipe: *mut drm_simple_display_pipe) {
    ...
    let mut max_div: c_int = (1 << (TIM2_PCD_LO_BITS + TIM2_PCD_HI_BITS)) - 1;
...
// prate = best_prate;
    ...
    let mut tim2: u32 = readl(priv.regs + CLCD_TIM2);
    int div;
    if (tim2 & TIM2_BCD)
    return prate;
    ...
    struct pl111_drm_dev_private *priv =
    container_of(hw, struct pl111_drm_dev_private, clk_div);
    let mut div: c_int = pl111_clk_div_choose_div(hw, rate, &prate, false);
    u32 tim2;
    spin_lock(&priv.tim2_lock);
    tim2 = readl(priv.regs + CLCD_TIM2);
    tim2 &= ~(TIM2_BCD | TIM2_PCD_LO_MASK | TIM2_PCD_HI_MASK);
...
#[no_mangle]
pub unsafe extern "C" fn pl111_display_init(drm: *mut drm_device) -> c_int {
