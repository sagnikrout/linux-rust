...
//! Source: drivers/gpu/drm/hyperv/hyperv_drm_modeset.c
#![no_std]
...
//
    ...
    let mut dst: iosys_map = IOSYS_MAP_INIT_VADDR_IOMEM(hv.vram);
    int idx;
...
#[no_mangle]
unsafe extern "C" fn hv_drm_connector_get_modes(connector: *mut drm_connector) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hv_drm_conn_init(hv: *mut hv_drm_device) -> c_int {
    ...
    {
    let mut pitch: u32 = w * (hv.screen_depth / 8);
    if (fb)
    pitch = fb.pitches[0];
    ...
    {
    struct hv_drm_device *hv = to_hv_drm(plane.dev);
    let mut map: iosys_map = IOSYS_MAP_INIT_VADDR_IOMEM(hv.vram);
...
#[no_mangle]
unsafe extern "C" fn hv_drm_plane_panic_flush(plane: *mut drm_plane) {
...
#[no_mangle]
pub unsafe extern "C" fn hv_drm_pipe_init(hv: *mut hv_drm_device) -> c_int {
    ...
    &hv_drm_crtc_funcs, core::ptr::null_mut());
    if (ret)
    return ret;
    ...
    if (hv_drm_check_size(hv, mode.hdisplay, mode.vdisplay, core::ptr::null_mut()))
    return MODE_BAD;
    return MODE_OK;
    }
...
#[no_mangle]
pub unsafe extern "C" fn hv_drm_mode_config_init(hv: *mut hv_drm_device) -> c_int {
