...
//! Source: drivers/s390/scsi/zfcp_scsi.c
#![no_std]
...
//
    ...
    let mut default_depth: static unsigned int = 32;
    ...
    let mut allow_lun_scan: static bool = true;
    module_param(allow_lun_scan, bool, 0600);
    MODULE_PARM_DESC(allow_lun_scan, "For NPIV, scan and attach all storage LUNs");
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_sdev_destroy(sdev: *mut scsi_device) {
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_command_fail(scpnt: *mut scsi_cmnd, result: c_int) {
...
// reset the status for this request
    scpnt.result = 0;
    scpnt.host_scribble = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(0): unlikely(ret <) -> else {
    else if (unlikely(ret < 0))
    return SCSI_MLQUEUE_HOST_BUSY;
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_sdev_init(sdev: *mut scsi_device) -> c_int {
    ...
    let mut npiv: c_int = adapter.connection_features & FSF_FEATURE_NPIV_MODE;
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_eh_abort_handler(scpnt: *mut scsi_cmnd) -> c_int {
    ...
    unsigned long flags;
    let mut old_reqid: u64 = (u64) scpnt.host_scribble;
    let mut retval: c_int = SUCCESS, ret;
...
// avoid race condition between late normal completion and abort
    ...
    zfcp_dbf_scsi_abort("abrt_or", scpnt, core::ptr::null_mut());
    return FAILED; /* completion could be in progress */
    }
    old_req.data = core::ptr::null_mut();
// don't access old fsf_req after releasing the abort_lock
    ...
    if (abrt_req)
    break;
    zfcp_dbf_scsi_abort("abrt_wt", scpnt, core::ptr::null_mut());
    zfcp_erp_wait(adapter);
    ret = fc_block_scsi_eh(scpnt);
    if (ret) {
    zfcp_dbf_scsi_abort("abrt_bl", scpnt, core::ptr::null_mut());
    return ret;
    }
    if (!(atomic_read(&adapter.status) &
    ZFCP_STATUS_COMMON_RUNNING)) {
    zfcp_dbf_scsi_abort("abrt_ru", scpnt, core::ptr::null_mut());
    return SUCCESS;
    }
    }
    if (!abrt_req) {
    zfcp_dbf_scsi_abort("abrt_ar", scpnt, core::ptr::null_mut());
    return FAILED;
    }
    ...
    dbf_tag = "abrt_ok";
#[no_mangle]
pub unsafe extern "C" fn if(ZFCP_STATUS_FSFREQ_ABORTNOTNEEDED: abrt_req->status &) -> else {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct zfcp_scsi_req_filter {
    pub tmf_scope: u8,
    pub lun_handle: u32,
    pub port_handle: u32,
}
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_forget_cmnd(old_req: *mut zfcp_fsf_req, data: *mut c_void) {
    static void zfcp_scsi_forget_cmnd(struct zfcp_fsf_req *old_req, void *data)
    {
    struct zfcp_scsi_req_filter *filter =
    (struct zfcp_scsi_req_filter *)data;
// already aborted - prevent side-effects - or not a SCSI command
    if (old_req.data == core::ptr::null_mut() ||
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_forget_cmnds(zsdev: *mut zfcp_scsi_dev, tm_flags: u8) {
...
//
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_task_mgmt_function(sdev: *mut scsi_device, tm_flags: u8) -> c_int {
    static int zfcp_scsi_task_mgmt_function(struct scsi_device *sdev, u8 tm_flags)
    {
    struct zfcp_scsi_dev *zfcp_sdev = sdev_to_zfcp(sdev);
    struct zfcp_adapter *adapter = zfcp_sdev.port.adapter;
    struct fc_rport *rport = starget_to_rport(scsi_target(sdev));
    struct zfcp_fsf_req *fsf_req = core::ptr::null_mut();
    let mut retval: c_int = SUCCESS, ret;
    let mut retry: c_int = 3;
    while (retry--) {
    ...
    if (fsf_req)
    break;
    zfcp_dbf_scsi_devreset("wait", sdev, tm_flags, core::ptr::null_mut());
    zfcp_erp_wait(adapter);
    ret = fc_block_rport(rport);
    if (ret) {
    zfcp_dbf_scsi_devreset("fiof", sdev, tm_flags, core::ptr::null_mut());
    return ret;
    }
    if (!(atomic_read(&adapter.status) &
    ZFCP_STATUS_COMMON_RUNNING)) {
    zfcp_dbf_scsi_devreset("nres", sdev, tm_flags, core::ptr::null_mut());
    return SUCCESS;
    }
    }
    if (!fsf_req) {
    zfcp_dbf_scsi_devreset("reqf", sdev, tm_flags, core::ptr::null_mut());
    return FAILED;
    }
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_eh_device_reset_handler(scpnt: *mut scsi_cmnd) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_eh_target_reset_handler(scpnt: *mut scsi_cmnd) -> c_int {
    static int zfcp_scsi_eh_target_reset_handler(struct scsi_cmnd *scpnt)
    {
    struct scsi_target *starget = scsi_target(scpnt.device);
    struct fc_rport *rport = starget_to_rport(starget);
    struct Scsi_Host *shost = rport_to_shost(rport);
    struct scsi_device *sdev = core::ptr::null_mut(), *tmp_sdev;
    struct zfcp_adapter *adapter =
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_eh_host_reset_handler(scpnt: *mut scsi_cmnd) -> c_int {
    static int zfcp_scsi_eh_host_reset_handler(struct scsi_cmnd *scpnt)
    {
    struct zfcp_scsi_dev *zfcp_sdev = sdev_to_zfcp(scpnt.device);
    struct zfcp_adapter *adapter = zfcp_sdev.port.adapter;
    let mut ret: c_int = SUCCESS, fc_ret;
...
//
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_sysfs_host_reset(shost: *mut Scsi_Host, reset_type: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_adapter_register(adapter: *mut zfcp_adapter) -> c_int {
...
// make all basic properties known at registration time
    ...
    return 0;
    err_out:
    adapter.scsi_host = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_adapter_unregister(adapter: *mut zfcp_adapter) {
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_reset_fc_host_stats(shost: *mut Scsi_Host) {
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_get_host_port_state(shost: *mut Scsi_Host) {
    static void zfcp_scsi_get_host_port_state(struct Scsi_Host *shost)
    {
    struct zfcp_adapter *adapter =
    (struct zfcp_adapter *)shost.hostdata[0];
    let mut status: c_int = atomic_read(&adapter.status);
    if ((status & ZFCP_STATUS_COMMON_RUNNING) &&
    !(status & ZFCP_STATUS_ADAPTER_LINK_UNPLUGGED))
    fc_host_port_state(shost) = FC_PORTSTATE_ONLINE;
#[no_mangle]
pub unsafe extern "C" fn if(ZFCP_STATUS_ADAPTER_LINK_UNPLUGGED: status &) -> else {
    else if (status & ZFCP_STATUS_ADAPTER_LINK_UNPLUGGED)
    fc_host_port_state(shost) = FC_PORTSTATE_LINKDOWN;
#[no_mangle]
pub unsafe extern "C" fn if(ZFCP_STATUS_COMMON_ERP_FAILED: status &) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_terminate_rport_io(rport: *mut fc_rport) {
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_rport_register(port: *mut zfcp_port) {
    ...
    zfcp_dbf_rec_trig_lock("scpaddy", port.adapter, port, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn zfcp_scsi_rport_block(port: *mut zfcp_port) {
    static void zfcp_scsi_rport_block(struct zfcp_port *port)
    {
    struct fc_rport *rport = port.rport;
    if (rport) {
    zfcp_dbf_rec_trig_lock("scpdely", port.adapter, port, core::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_schedule_rport_register(port: *mut zfcp_port) {
...
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_schedule_rport_block(port: *mut zfcp_port) {
...
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_schedule_rports_block(adapter: *mut zfcp_adapter) {
...
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_rport_work(work: *mut work_struct) {
...
//
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_set_prot(adapter: *mut zfcp_adapter) {
...
//
#[no_mangle]
pub unsafe extern "C" fn zfcp_scsi_dif_sense_error(scmd: *mut scsi_cmnd, ascq: c_int) {
    ...
    {
    let mut shost: *mut Scsi_Host const = adapter.scsi_host;
