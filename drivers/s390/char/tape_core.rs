...
//! Source: drivers/s390/char/tape_core.c
#![no_std]
...
//
    debug_info_t *TAPE_DBF_AREA = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn devid_to_int(dev_id: *mut ccw_dev_id) -> c_int {
...
//
    ...
    static
    DEVICE_ATTR(medium_state, 0444, tape_medium_state_show, core::ptr::null_mut());
    static ssize_t
    ...
    static
    DEVICE_ATTR(first_minor, 0444, tape_first_minor_show, core::ptr::null_mut());
    static ssize_t
    ...
    static
    DEVICE_ATTR(state, 0444, tape_state_show, core::ptr::null_mut());
    static ssize_t
    ...
    static
    DEVICE_ATTR(operation, 0444, tape_operation_show, core::ptr::null_mut());
    static ssize_t
    ...
    static
    DEVICE_ATTR(blocksize, 0444, tape_blocksize_show, core::ptr::null_mut());
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tape_med_state_work_data {
    pub device: *mut tape_device,
    pub state: enum tape_medium_state,
    ...
    char *envp[] = { core::ptr::null_mut(), core::ptr::null_mut() };
    switch (p.state) {
    case MS_UNLOADED:
...
// Check if interrupt has already been processed
    if (request.callback == core::ptr::null_mut())
    return 0;
    rc = 0;
...
//
    ...
    if (device.modeset_byte == core::ptr::null_mut()) {
    DBF_EXCEPTION(2, "ti:no mem\n");
    kfree(device);
    return ERR_PTR(-ENOMEM);
    }
...
// Decrease ref_count for removed request.
    request.device = core::ptr::null_mut();
    tape_put_device(device);
    request.rc = -EIO;
    if (request.callback != core::ptr::null_mut())
...
// allocate channel program
    ...
    GFP_ATOMIC | GFP_DMA);
    if (request.cpaddr == core::ptr::null_mut()) {
    DBF_EXCEPTION(1, "cqra nomem\n");
    kfree(request);
    return ERR_PTR(-ENOMEM);
    }
...
// alloc small kernel buffer
    ...
    if (request.cpdata == core::ptr::null_mut()) {
    DBF_EXCEPTION(1, "cqra nomem\n");
...
#[no_mangle]
unsafe extern "C" fn tape_long_busy_timeout(t: *mut timer_list) {
    static void tape_long_busy_timeout(struct timer_list *t)
    {
...
//
    ...
    {
    request.callback = core::ptr::null_mut();
...
// Request added to the queue. Wait for its completion.
    wait_event(device.wait_queue, (request.callback == core::ptr::null_mut()));
// Get rc from request
    return request.rc;
    }
//
...
//
    if(request != core::ptr::null_mut() && request.status == TAPE_REQUEST_DONE) {
    __tape_end_request(device, request, -EIO);
    return;
    }
...
//
    ...
    DBF_EVENT(6, "TAPE:dbusy\n");
    rc = -EBUSY;
    } else if (device.discipline != core::ptr::null_mut() &&
