...
//! Source: drivers/s390/char/sclp_sd.c
#![no_std]
...
pub const SD_EQ_STORE_DATA: c_int = 0;
pub const SD_EQ_HALT: c_int = 1;
pub const SD_EQ_SIZE: c_int = 2;
pub const SD_DI_CONFIG: c_int = 3;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct sclp_sd_evbuf {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sclp_sd_sccb {
    pub hdr: sccb_header,
    pub evbuf: sclp_sd_evbuf,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sclp_sd_data {
    pub esize_bytes: usize,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct sclp_sd_file {
    pub kobj: kobject,
    pub data_attr: bin_attribute,
    pub data_mutex: mutex,
    pub data: sclp_sd_data,
    pub di: u8,
}
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_listener_add(listener: *mut sclp_sd_listener) {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_listener_remove(listener: *mut sclp_sd_listener) {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_listener_init(listener: *mut sclp_sd_listener, id: u32) {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_receiver(evbuf_hdr: *mut evbuf_header) {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_store_data(result: *mut sclp_sd_data, di: u8) -> c_int {
    static int sclp_sd_store_data(struct sclp_sd_data *result, u8 di)
    {
    let mut dsize: u32 = 0, esize = 0;
...
// Cancel running request if interrupted or timed out
    ...
    if (sclp_sd_sync(page, SD_EQ_HALT, di, 0, 0, core::ptr::null_mut(), core::ptr::null_mut())) {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_data_reset(data: *mut sclp_sd_data) {
    static void sclp_sd_data_reset(struct sclp_sd_data *data)
    {
    vfree(data.data);
    data.data = core::ptr::null_mut();
    data.dsize_bytes = 0;
    data.esize_bytes = 0;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_file_release(kobj: *mut kobject) {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_file_update(sd_file: *mut sclp_sd_file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_file_update_async(data: *mut c_void, cookie: async_cookie_t) {
...
//
    ...
    let mut reload_attr: static struct kobj_attribute = __ATTR_WO(reload);
...
// Create kobject located under /sys/firmware/sclp_sd/
    ...
    rc = kobject_init_and_add(&sd_file.kobj, &sclp_sd_file_ktype, core::ptr::null_mut(),
    "%s", name);
    if (rc) {
...
//
#[no_mangle]
unsafe extern "C" fn sclp_sd_init() -> __init int {
...
// Create kset named "sclp_sd" located under /sys/firmware/
    rc = -ENOMEM;
    sclp_sd_kset = kset_create_and_add("sclp_sd", core::ptr::null_mut(), firmware_kobj);
    if (!sclp_sd_kset)
    goto err_kset;
    rc = -EINVAL;
