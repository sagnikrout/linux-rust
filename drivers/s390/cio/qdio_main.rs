...
//! Source: drivers/s390/cio/qdio_main.c
#![no_std]
...
//
    ...
    {
    let mut tmp_count: c_int = count, tmp_start = start, nr = q.nr;
    let mut ccq: c_uint = 0;
    qperf_inc(q, eqbs);
    if (!q.is_input_q)
...
//
    ...
    {
    let mut ccq: c_uint = 0;
    let mut tmp_count: c_int = count, tmp_start = start;
    let mut nr: c_int = q.nr;
    qperf_inc(q, sqbs);
    if (!q.is_input_q)
    nr += q.irq_ptr.nr_input_qs;
    again:
...
#[no_mangle]
unsafe extern "C" fn qdio_init_buf_states(irq_ptr: *mut qdio_irq) {
    ...
    unsigned int input)
    {
    let mut schid: c_ulong = *((u32 *) &q.irq_ptr.schid);
    let mut fc: c_uint = QDIO_SIGA_SYNC;
    int cc;
...
#[no_mangle]
pub unsafe extern "C" fn qdio_sync_input_queue(q: *mut qdio_q) -> c_int {
    static inline int qdio_sync_input_queue(struct qdio_q *q)
    {
    return qdio_siga_sync(q, 0, q.mask);
    }
#[no_mangle]
pub unsafe extern "C" fn qdio_sync_output_queue(q: *mut qdio_q) -> c_int {
    static inline int qdio_sync_output_queue(struct qdio_q *q)
    {
    return qdio_siga_sync(q, q.mask, 0);
    }
#[no_mangle]
pub unsafe extern "C" fn qdio_siga_sync_q(q: *mut qdio_q) -> c_int {
    ...
    {
    let mut schid: c_ulong = *((u32 *) &q.irq_ptr.schid);
    let mut fc: c_uint = QDIO_SIGA_WRITE;
    let mut start_time: u64 = 0;
    let mut retries: c_int = 0, cc;
    if (queue_type(q) == QDIO_IQDIO_QFMT && !multicast_outbound(q)) {
    if (count > 1)
    fc = QDIO_SIGA_WRITEM;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: aob) -> else {
    else if (aob)
    fc = QDIO_SIGA_WRITEQ;
    }
    if (is_qebsm(q)) {
...
#[no_mangle]
pub unsafe extern "C" fn qdio_siga_input(q: *mut qdio_q) -> c_int {
    static inline int qdio_siga_input(struct qdio_q *q)
    {
    let mut schid: c_ulong = *((u32 *) &q.irq_ptr.schid);
    let mut fc: c_uint = QDIO_SIGA_READ;
    int cc;
...
#[no_mangle]
pub unsafe extern "C" fn qdio_stop_polling(q: *mut qdio_q) {
...
#[no_mangle]
pub unsafe extern "C" fn account_sbals(q: *mut qdio_q, count: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn qdio_inbound_q_done(q: *mut qdio_q, start: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qdio_irq_check_sense(irq_ptr: *mut qdio_irq, irb: *mut irb) {
...
#[no_mangle]
unsafe extern "C" fn qdio_int_handler_pci(irq_ptr: *mut qdio_irq) {
    ...
    int dstat)
    {
    let mut first_to_check: c_uint = 0;
...
// qdio interrupt handler
    ...
    dstat);
#[no_mangle]
pub unsafe extern "C" fn if(1: dcc ==) -> else {
    else if (dcc == 1)
    rc = -EAGAIN;
    break;
...
//
    ...
    return qdio_setup_get_ssqd(core::ptr::null_mut(), &schid, data);
    }
    EXPORT_SYMBOL_GPL(qdio_get_ssqd_desc);
#[no_mangle]
unsafe extern "C" fn qdio_cancel_ccw(irq: *mut qdio_irq, how: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn qdio_shutdown(cdev: *mut ccw_device, how: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn qdio_free(cdev: *mut ccw_device) -> c_int {
    ...
    mutex_lock(&irq_ptr.setup_mutex);
    irq_ptr.debug_area = core::ptr::null_mut();
    cdev.private.qdio_data = core::ptr::null_mut();
    mutex_unlock(&irq_ptr.setup_mutex);
...
//
#[no_mangle]
pub unsafe extern "C" fn qdio_activate(cdev: *mut ccw_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn handle_inbound(q: *mut qdio_q, bufnr: c_int, count: c_int) -> c_int {
...
//
    ...
    let mut phys_aob: dma64_t = aob ? virt_to_dma64(aob) : 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn qdio_start_irq(cdev: *mut ccw_device) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn qdio_stop_irq(cdev: *mut ccw_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn init_QDIO() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_QDIO() -> void __exit {
