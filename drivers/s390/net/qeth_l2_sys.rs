...
//! Source: drivers/s390/net/qeth_l2_sys.c
#![no_std]
...
//
    ...
    let mut state: enum qeth_sbp_states = QETH_SBP_STATE_INACTIVE;
    let mut rc: c_int = 0;
    char *word;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(buf, _arg: "secondary")) -> else {
    else if (sysfs_streq(buf, "secondary"))
    role = QETH_SBP_ROLE_SECONDARY;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(buf, _arg: "none")) -> else {
    ...
    rc = -EBUSY;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: card->options.sbp.reflect_promisc) -> else {
    else if (card.options.sbp.reflect_promisc)
// Forbid direct manipulation
    rc = -EPERM;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: qeth_card_hw_is_reachable(card)) -> else {
...
// sbp_lock ensures ordering vs notifications-stopped events
    ...
    rc = -EBUSY;
#[no_mangle]
pub unsafe extern "C" fn if(QETH_SBP_ROLE_NONE: card->options.sbp.role !=) -> else {
...
#[no_mangle]
unsafe extern "C" fn qeth_l2_vnicc_sysfs_attr_to_char(attr_name: *const c_char) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(attr_name, _arg: "mcast_flooding")) -> else {
    else if (sysfs_streq(attr_name, "mcast_flooding"))
    return QETH_VNICC_MCAST_FLOODING;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(attr_name, _arg: "learning")) -> else {
    else if (sysfs_streq(attr_name, "learning"))
    return QETH_VNICC_LEARNING;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(attr_name, _arg: "takeover_setvmac")) -> else {
    else if (sysfs_streq(attr_name, "takeover_setvmac"))
    return QETH_VNICC_TAKEOVER_SETVMAC;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(attr_name, _arg: "takeover_learning")) -> else {
    else if (sysfs_streq(attr_name, "takeover_learning"))
    return QETH_VNICC_TAKEOVER_LEARNING;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(attr_name, _arg: "bridge_invisible")) -> else {
    else if (sysfs_streq(attr_name, "bridge_invisible"))
    return QETH_VNICC_BRIDGE_INVISIBLE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq(attr_name, _arg: "rx_bcast")) -> else {
