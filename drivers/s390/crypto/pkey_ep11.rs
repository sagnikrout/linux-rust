...
//! Source: drivers/s390/crypto/pkey_ep11.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn is_ep11_key(key: *const u8, keylen: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_ep11_keytype(key_type: enum pkey_key_type) -> bool {
    ...
    let mut minhwtype: c_int = 0, api = 0;
...
// check keytype, subtype, keybitsize
    ...
    apqns[0].card == 0xFFFF && apqns[0].domain == 0xFFFF)) {
    nr_apqns = MAXAPQNSINLIST;
    rc = ep11_apqns4type(subtype, core::ptr::null_mut(), core::ptr::null_mut(), 0,
...
// try two times in case of failure
    ...
    rc = ep11_clr2key(core::ptr::null_mut(), 0, t.keytype, PKEY_TYPE_EP11,
    8 * keysize, 0, t.clearkey, t.len,
    tmpbuf, &tmplen, core::ptr::null_mut(), pflags);
    pr_debug("ep11_clr2key()=%d\n", rc);
    if (rc)
    continue;
    rc = ep11_key2protkey(core::ptr::null_mut(), 0, tmpbuf, tmplen,
...
//
#[no_mangle]
unsafe extern "C" fn pkey_ep11_init() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn pkey_ep11_exit() -> void __exit {
