...
//! Source: drivers/s390/crypto/pkey_cca.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn is_cca_key(key: *const u8, keylen: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn is_cca_keytype(key_type: enum pkey_key_type) -> bool {
    ...
    const u8 *ptr_cur_mkvp = core::ptr::null_mut();
    const u8 *ptr_old_mkvp = core::ptr::null_mut();
    let mut minhwtype: c_int = ZCRYPT_CEX3C;
...
// check keytype, subtype, keybitsize
    ...
    apqns[0].card == 0xFFFF && apqns[0].domain == 0xFFFF)) {
    nr_apqns = MAXAPQNSINLIST;
    rc = cca_apqns4type(subtype, core::ptr::null_mut(), core::ptr::null_mut(), 0,
...
// keybitsize = t->bitsize;
    ...
    t.mkvp, core::ptr::null_mut(), xflags);
    if (!rc)
// flags = PKEY_FLAGS_MATCH_CUR_MKVP;
    ...
    rc = cca_findcard2(apqns, &nr_apqns, *card, *dom,
    ZCRYPT_CEX3C, AES_MK_SET,
    core::ptr::null_mut(), t.mkvp, xflags);
    if (!rc)
...
#[no_mangle]
pub unsafe extern "C" fn if(576: !t->plfver && t->wpllen ==) -> else {
    else if (!t.plfver && t.wpllen == 576)
// keybitsize = PKEY_SIZE_AES_192;
#[no_mangle]
pub unsafe extern "C" fn if(640: !t->plfver && t->wpllen ==) -> else {
    else if (!t.plfver && t.wpllen == 640)
// keybitsize = PKEY_SIZE_AES_256;
    rc = cca_findcard2(apqns, &nr_apqns, *card, *dom,
    ZCRYPT_CEX6, AES_MK_SET,
    t.mkvp0, core::ptr::null_mut(), xflags);
    if (!rc)
// flags = PKEY_FLAGS_MATCH_CUR_MKVP;
    if (rc == -ENODEV) {
    nr_apqns = ARRAY_SIZE(apqns);
    rc = cca_findcard2(apqns, &nr_apqns, *card, *dom,
    ZCRYPT_CEX6, AES_MK_SET,
    core::ptr::null_mut(), t.mkvp0, xflags);
    if (!rc)
// flags = PKEY_FLAGS_MATCH_ALT_MKVP;
    }
    if (rc)
...
// try two times in case of failure
    ...
    rc = cca_clr2key(core::ptr::null_mut(), 0, t.keytype, PKEY_TYPE_CCA_DATA,
    8 * keysize, 0, t.clearkey, t.len,
    tmpbuf, &tmplen, core::ptr::null_mut(), pflags);
    pr_debug("cca_clr2key()=%d\n", rc);
    if (rc)
    continue;
    rc = cca_key2protkey(core::ptr::null_mut(), 0, tmpbuf, tmplen,
...
//
#[no_mangle]
unsafe extern "C" fn pkey_cca_init() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn pkey_cca_exit() -> void __exit {
