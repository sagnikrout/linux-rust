...
//! Source: drivers/s390/block/dasd_devmap.c
#![no_std]
...
pub const DASD_MAX_PARAMS: c_int = 256;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dasd_devmap {
    pub list: list_head,
    pub bus_id: [c_char; DASD_BUS_ID_SIZE],
    pub devindex: c_uint,
    pub features: c_ushort,
...
//
    ...
    module_param_array(dasd, charp, core::ptr::null_mut(), S_IRUGO);
//
...
#[no_mangle]
unsafe extern "C" fn dasd_call_setup(opt: *mut c_char) -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn dasd_busid(str: *mut c_char, id0: *mut c_int, id1: *mut c_int, devno: *mut c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dasd_feature_list(str: *mut c_char) -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn if(!strncmp(str: len == 4 &&, _arg: "diag", _arg: 4)) -> else {
    else if (len == 4 && !strncmp(str, "diag", 4))
    features |= DASD_FEATURE_USEDIAG;
#[no_mangle]
pub unsafe extern "C" fn if(!strncmp(str: len == 3 &&, _arg: "raw", _arg: 3)) -> else {
    else if (len == 3 && !strncmp(str, "raw", 3))
    features |= DASD_FEATURE_USERAW;
#[no_mangle]
pub unsafe extern "C" fn if(!strncmp(str: len == 6 &&, _arg: "erplog", _arg: 6)) -> else {
    else if (len == 6 && !strncmp(str, "erplog", 6))
    features |= DASD_FEATURE_ERPLOG;
#[no_mangle]
pub unsafe extern "C" fn if(!strncmp(str: len == 8 &&, _arg: "failfast", _arg: 8)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn dasd_parse_keyword(keyword: *mut c_char) -> int __init {
    static int __init dasd_parse_keyword(char *keyword)
    {
    let mut length: c_int = strlen(keyword);
...
//
#[no_mangle]
unsafe extern "C" fn dasd_parse_range(range: *const c_char) -> int __init {
    ...
    char *features_str = core::ptr::null_mut();
    char *from_str = core::ptr::null_mut();
    char *to_str = core::ptr::null_mut();
    let mut rc: c_int = 0;
    char *tmp;
...
//
#[no_mangle]
pub unsafe extern "C" fn dasd_parse() -> int __init {
...
//
    ...
    BUG_ON(devmap.device != core::ptr::null_mut());
...
// Someone else was faster.
    ...
    device.paths_info = kset_create_and_add("paths_info", core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn dasd_devmap_delete_copy_relation_device(device: *mut dasd_device) {
    ...
    copy.entry[i].device = core::ptr::null_mut();
    }
    dasd_put_device(device);
    device.copy = core::ptr::null_mut();
    }
//
...
//
    ...
    bool pprc_enabled)
    {
    struct dasd_pprc_data_sc4 *data = core::ptr::null_mut();
    struct dasd_copy_entry *entry = core::ptr::null_mut();
    struct dasd_copy_relation *copy;
    struct dasd_devmap *devmap;
    struct dasd_device *device;
    int i, rc = 0;
...
// First remove device pointer from devmap.
    ...
    devmap.device = core::ptr::null_mut();
    spin_unlock(&dasd_devmap_lock);
// Disconnect dasd_device structure from ccw_device structure.
    spin_lock_irqsave(get_ccwdev_lock(device.cdev), flags);
    dev_set_drvdata(&device.cdev.dev, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn dasd_add_link_to_gendisk(gdp: *mut gendisk, device: *mut dasd_device) {
...
// Already doing offline processing
    ...
    static DEVICE_ATTR(safe_offline, 0200, core::ptr::null_mut(), dasd_safe_offline_store);
    static ssize_t
    ...
    count = -ENODEV;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !device->discipline->host_access_count) -> else {
    ...
    static DEVICE_ATTR(host_access_count, 0444, dasd_access_show, core::ptr::null_mut());
    static ssize_t
    ...
    goto out;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !device->discipline) -> else {
    dasd_put_device(device);
    goto out;
    } else {
    ...
    static DEVICE_ATTR(discipline, 0444, dasd_discipline_show, core::ptr::null_mut());
    static ssize_t
    ...
    static DEVICE_ATTR(status, 0444, dasd_device_status_show, core::ptr::null_mut());
    ...
    static DEVICE_ATTR(alias, 0444, dasd_alias_show, core::ptr::null_mut());
    ...
    static DEVICE_ATTR(vendor, 0444, dasd_vendor_show, core::ptr::null_mut());
    static ssize_t
...
// should not happen, treat like base device
    ...
    static DEVICE_ATTR(uid, 0444, dasd_uid_show, core::ptr::null_mut());
//
...
// adaptive range but not ESE: the heuristic does not run
    ...
    static DEVICE_ATTR(path_reset, 0200, core::ptr::null_mut(), dasd_path_reset_store);
    ...
    static DEVICE_ATTR(hpf, 0444, dasd_hpf_show, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sysfs_streq("fail", _arg: buf)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(DASD_FLAG_LOCK_STOLEN, _arg: &device->flags)) -> else {
    ...
    static DEVICE_ATTR(path_masks, 0444, dasd_pm_show, core::ptr::null_mut());
//
...
//
    ...
    static DEVICE_ATTR(fc_security, 0444, dasd_device_fcs_show, core::ptr::null_mut());
    static ssize_t
    dasd_path_fcs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
    {
    struct dasd_path *path = to_dasd_path(kobj);
    let mut fc_sec: c_uint = path.fc_security;
    return sysfs_emit(buf, "%s\n", dasd_path_get_fcs_str(fc_sec));
    }
    static struct kobj_attribute path_fcs_attribute =
    __ATTR(fc_security, 0444, dasd_path_fcs_show, core::ptr::null_mut());
//
...
// find entry
    ...
    copy.active = core::ptr::null_mut();
    copy.entry[i].primary = false;
    }
    spin_unlock(&dasd_devmap_lock);
    }
#[no_mangle]
unsafe extern "C" fn dasd_devmap_clear_copy_relation(dev: *mut device) -> c_int {
...
// clear all devmap entries
    ...
    copy.entry[i].device.copy = core::ptr::null_mut();
    copy.entry[i].device = core::ptr::null_mut();
    }
    devmap = dasd_find_busid_locked(copy.entry[i].busid);
    devmap.copy = core::ptr::null_mut();
    memset(copy.entry[i].busid, 0, DASD_BUS_ID_SIZE);
    }
    kfree(copy);
    rc = 0;
...
// not in the list, no COPY role
    ...
    static DEVICE_ATTR(copy_role, 0444, dasd_copy_role_show, core::ptr::null_mut());
...
//
    ...
    static DEVICE_ATTR(ping, 0200, core::ptr::null_mut(), dasd_device_ping);
    ...
    static DEVICE_ATTR(_name, 0444, dasd_##_name##_show, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn dasd_path_init_kobj(device: *mut dasd_device, chp: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn dasd_path_create_kobj(device: *mut dasd_device, chp: c_int) {
    ...
    rc = kobject_add(&device.path[chp].kobj, core::ptr::null_mut(), "%x.%02x",
...
#[no_mangle]
pub unsafe extern "C" fn dasd_path_create_kobjects(device: *mut dasd_device) {
...
#[no_mangle]
unsafe extern "C" fn dasd_path_remove_kobj(device: *mut dasd_device, chp: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn dasd_path_remove_kobjects(device: *mut dasd_device) {
