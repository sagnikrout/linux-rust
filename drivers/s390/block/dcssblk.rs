...
//! Source: drivers/s390/block/dcssblk.c
#![no_std]
...
pub const DCSSBLK_MINORS_PER_DISK: c_int = 1;
pub const DCSSBLK_PARM_LEN: c_int = 400;
pub const DCSS_BUS_ID_SIZE: c_int = 20;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dcssblk_dev_info {
    pub lh: list_head,
    pub dev: device,
    pub segment_name: [c_char; DCSS_BUS_ID_SIZE],
    pub use_count: core::sync::atomic::AtomicI32,
    pub gd: *mut gendisk,
    pub start: c_ulong,
    pub end: c_ulong,
    pub segment_type: c_int,
    pub save_pending: c_uchar,
    pub is_shared: c_uchar,
    pub num_of_segments: c_int,
    pub seg_list: list_head,
    pub dax_dev: *mut dax_device,
    pub pgmap: dev_pagemap,
    pub pgmap_addr: *mut c_void,
}
...
pub struct segment_info {
    ...
    size_t count);
    static DEVICE_ATTR(add, S_IWUSR, core::ptr::null_mut(), dcssblk_add_store);
    static DEVICE_ATTR(remove, S_IWUSR, core::ptr::null_mut(), dcssblk_remove_store);
...
//
    ...
    if (sort_list == core::ptr::null_mut())
    return -ENOMEM;
    i = 0;
    list_for_each_entry(entry, &dev_info.seg_list, lh) {
...
//
    ...
    static DEVICE_ATTR(seglist, S_IRUSR, dcssblk_seglist_show, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn dcssblk_setup_dax(dev_info: *mut dcssblk_dev_info) -> c_int {
...
//
    ...
    dev_info = core::ptr::null_mut();
    seg_info = core::ptr::null_mut();
    ...
    if (local_buf == core::ptr::null_mut()) {
    rc = -ENOMEM;
    goto out_nobuf;
    }
//
...
//
    ...
    rc = device_add_disk(&dev_info.dev, dev_info.gd, core::ptr::null_mut());
    if (rc)
    goto out_dax_host;
    switch (dev_info.segment_type) {
    case SEG_TYPE_SR:
...
// More paranoia.
    ...
    {
    let mut offset: resource_size_t = pgoff * PAGE_SIZE;
...
// pfn = PFN_DOWN(dev_info->start + offset);
    ...
    buf[j-i] = '\0';
    rc = dcssblk_add_store(dcssblk_root_dev, core::ptr::null_mut(), buf, j-i);
