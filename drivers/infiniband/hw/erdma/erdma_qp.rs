...
//! Source: drivers/infiniband/hw/erdma/erdma_qp.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn erdma_qp_llp_close(qp: *mut erdma_qp) {
...
// rsvd tcp seq for mpa-rsp in server.
    ...
    ret = erdma_post_cmd_wait(&dev.cmdq, &req, sizeof(req), core::ptr::null_mut(), core::ptr::null_mut(),
    true);
    if (ret)
    return ret;
    ...
    int mask)
    {
    let mut need_reflush: bool = false;
    int drop_conn, ret = 0;
    if (!mask)
    return 0;
    ...
    return erdma_post_cmd_wait(&qp.dev.cmdq, &req, sizeof(req), core::ptr::null_mut(),
    core::ptr::null_mut(), true);
    }
#[no_mangle]
unsafe extern "C" fn erdma_reset_qp(qp: *mut erdma_qp) {
...
#[no_mangle]
unsafe extern "C" fn erdma_qp_safe_free(ref: *mut kref) {
...
#[no_mangle]
pub unsafe extern "C" fn erdma_qp_put(qp: *mut erdma_qp) {
...
#[no_mangle]
pub unsafe extern "C" fn erdma_qp_get(qp: *mut erdma_qp) {
...
// Not allowed to send control qkey
    ...
    let mut idx: u32 = *pi & (qp.attrs.sq_size - 1);
    let mut op: enum ib_wr_opcode = send_wr.opcode;
...
#[no_mangle]
unsafe extern "C" fn kick_sq_db(qp: *mut erdma_qp, pi: u16) {
