...
//! Source: drivers/infiniband/hw/irdma/uk.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn irdma_nop_1(qp: *mut irdma_qp_uk) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_clr_wqes(qp: *mut irdma_qp_uk, qp_wqe_idx: u32) {
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_uk_qp_post_wr(qp: *mut irdma_qp_uk) {
...
//
    ...
    {
    __le64 *wqe;
    __le64 *wqe_0 = core::ptr::null_mut();
    u16 avail_quanta;
    u16 i;
...
//
    ...
    u32 i, wqe_idx;
    let mut total_size: u32 = 0, byte_off;
    int ret_code;
    u32 frag_cnt, addl_frag_cnt;
    let mut read_fence: bool = false;
    u16 quanta;
...
// if not an odd number set valid bit in next fragment
    ...
    frag_cnt) {
    qp.wqe_ops.iw_set_fragment(wqe, byte_off, core::ptr::null_mut(),
    qp.swqe_polarity);
...
// if not an odd number set valid bit in next fragment
    ...
    info.num_sges) {
    srq.wqe_ops.iw_set_fragment(wqe, byte_off, core::ptr::null_mut(),
    srq.srwqe_polarity);
...
//
    ...
    let mut local_fence: bool = false;
    u32 addl_frag_cnt;
    __le64 *wqe;
    u32 wqe_idx;
    u16 quanta;
...
//
    ...
    qp.wqe_ops.iw_set_fragment(wqe, 0,
    frag_cnt ? op_info.sg_list : core::ptr::null_mut(),
    qp.swqe_polarity);
    i = 1;
    }
...
//
    ...
    {
    let mut quanta_bytes_remaining: u32 = 16;
    int i;
    for (i = 0; i < num_sges; i++) {
    u8 *cur_sge = (u8 *)(uintptr_t)sge_list[i].addr;
    let mut sge_len: u32 = sge_list[i].length;
    while (sge_len) {
    u32 bytes_copied;
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_inline_data_size_to_quanta_gen_1(data_size: u32) -> u16 {
...
//
    ...
    u32 num_sges, u8 polarity)
    {
    let mut inline_valid: u8 = polarity << IRDMA_INLINE_VALID_S;
    let mut quanta_bytes_remaining: u32 = 8;
    let mut first_quanta: bool = true;
    int i;
    wqe += 8;
    for (i = 0; i < num_sges; i++) {
    u8 *cur_sge = (u8 *)(uintptr_t)sge_list[i].addr;
...
//
#[no_mangle]
unsafe extern "C" fn irdma_inline_data_size_to_quanta(data_size: u32) -> u16 {
    static u16 irdma_inline_data_size_to_quanta(u32 data_size)
    {
    if (data_size <= 8)
    return IRDMA_QP_WQE_MIN_QUANTA;
#[no_mangle]
pub unsafe extern "C" fn if(39: data_size <=) -> else {
    else if (data_size <= 39)
    return 2;
#[no_mangle]
pub unsafe extern "C" fn if(70: data_size <=) -> else {
    else if (data_size <= 70)
    return 3;
#[no_mangle]
pub unsafe extern "C" fn if(101: data_size <=) -> else {
    else if (data_size <= 101)
    return 4;
#[no_mangle]
pub unsafe extern "C" fn if(132: data_size <=) -> else {
    else if (data_size <= 132)
    return 5;
#[no_mangle]
pub unsafe extern "C" fn if(163: data_size <=) -> else {
    else if (data_size <= 163)
    return 6;
#[no_mangle]
pub unsafe extern "C" fn if(194: data_size <=) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_uk_cq_resize(cq: *mut irdma_cq_uk, cq_base: *mut c_void, cq_size: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_uk_cq_set_resized_cnt(cq: *mut irdma_cq_uk, cq_cnt: u16) {
...
//
    ...
    u64 temp_val;
    u16 sw_cq_sel;
    let mut arm_next_se: u8 = 0;
    let mut arm_next: u8 = 0;
    u8 arm_seq_num;
    get_64bit_val(cq.shadow_area, 32, &temp_val);
    arm_seq_num = (u8)FIELD_GET(IRDMA_CQ_DBSA_ARM_SEQ_NUM, temp_val);
    arm_seq_num++;
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_uk_cq_empty(cq: *mut irdma_cq_uk) -> bool {
...
//
    ...
    u8 is_srq;
    struct irdma_ring *pring = core::ptr::null_mut();
    u32 wqe_idx;
    int ret_code;
    let mut move_cq_head: bool = true;
    u8 polarity;
    bool ext_valid;
    __le64 *ext_cqe;
    if (cq.avoid_mem_cflct)
...
// Set the min error to standard flush error code for remaining cqes
    ...
    break;
    default:
pub const IRDMA_CIE_SIGNATURE: c_uint = 0xE;
...
//
#[no_mangle]
unsafe extern "C" fn irdma_round_up_wq(wqdepth: u64) -> u64 {
    static u64 irdma_round_up_wq(u64 wqdepth)
    {
    let mut scount: c_int = 1;
...
// shift = 1;
#[no_mangle]
pub unsafe extern "C" fn if(101: sge < 8 && inline_data <=) -> else {
...
//
    ...
    u32 *sqdepth)
    {
    let mut min_hw_quanta: u32 = (u32)uk_attrs.min_hw_wq_size << shift;
    u64 hw_quanta =
    irdma_round_up_wq(((u64)sq_size << shift) + IRDMA_SQ_RSVD);
    if (hw_quanta < min_hw_quanta)
    hw_quanta = min_hw_quanta;
#[no_mangle]
pub unsafe extern "C" fn if(uk_attrs->max_hw_wq_quanta: hw_quanta >) -> else {
...
//
    ...
    hw_quanta = min_hw_quanta;
#[no_mangle]
pub unsafe extern "C" fn if(uk_attrs->max_hw_rq_quanta: hw_quanta >) -> else {
...
//
    ...
    hw_quanta = min_hw_quanta;
#[no_mangle]
pub unsafe extern "C" fn if(uk_attrs->max_hw_srq_quanta: hw_quanta >) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn irdma_setup_connection_wqes(qp: *mut irdma_qp_uk) {
    static void irdma_setup_connection_wqes(struct irdma_qp_uk *qp)
    {
    let mut move_cnt: u16 = 1;
...
//
    ...
    u8 *rq_shift)
    {
    let mut imm_support: bool = ukinfo.uk_attrs.hw_rev >= IRDMA_GEN_2;
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_uk_qp_init(qp: *mut irdma_qp_uk, info: *mut irdma_qp_uk_init_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_uk_clean_cq(q: *mut c_void, cq: *mut irdma_cq_uk) {
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_nop(qp: *mut irdma_qp_uk, wr_id: u64, signaled: bool, post_sq: bool) -> c_int {
    int irdma_nop(struct irdma_qp_uk *qp, u64 wr_id, bool signaled, bool post_sq)
    {
    __le64 *wqe;
    u64 hdr;
    u32 wqe_idx;
    let mut info: irdma_post_sq_info = {};
    info.wr_id = wr_id;
    wqe = irdma_qp_get_next_send_wqe(qp, &wqe_idx, IRDMA_QP_WQE_MIN_QUANTA,
    0, &info);
    if (!wqe)
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_fragcnt_to_quanta_sq(frag_cnt: u32, quanta: *mut u16) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn irdma_fragcnt_to_wqesize_rq(frag_cnt: u32, wqe_size: *mut u16) -> c_int {
