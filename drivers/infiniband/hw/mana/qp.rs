...
//! Source: drivers/infiniband/hw/mana/qp.c
#![no_std]
...
//
    ...
    let mut resp: mana_ib_create_qp_rss_resp = {};
...
//
    ...
    let mut wq_spec: mana_obj_spec = {};
    let mut cq_spec: mana_obj_spec = {};
...
// Create CQ table entry
    ...
    ibucontext);
    let mut resp: mana_ib_create_qp_resp = {};
    let mut ucmd: mana_ib_create_qp = {};
    let mut wq_spec: mana_obj_spec = {};
    let mut cq_spec: mana_obj_spec = {};
    struct mana_port_context *mpc;
    struct net_device *ndev;
...
#[no_mangle]
unsafe extern "C" fn mana_ib_wqe_size(sge: u32, oob_size: u32) -> u32 {
    static u32 mana_ib_wqe_size(u32 sge, u32 oob_size)
    {
    let mut wqe_size: u32 = sge * sizeof(struct gdma_sge) + sizeof(struct gdma_wqe) + oob_size;
    return ALIGN(wqe_size, GDMA_WQE_BU_SIZE);
    }
#[no_mangle]
unsafe extern "C" fn mana_ib_queue_size(attr: *mut ib_qp_init_attr, queue_type: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn mana_ib_queue_type(attr: *mut ib_qp_init_attr, queue_type: u32) -> enum gdma_queue_type {
...
#[no_mangle]
unsafe extern "C" fn mana_table_drain_qp_ref(qp: *mut mana_ib_qp) {
...
#[no_mangle]
unsafe extern "C" fn mana_table_remove_qp_qid(mdev: *mut mana_ib_dev, qid: u32, is_sq: bool) {
...
#[no_mangle]
unsafe extern "C" fn mana_table_store_qp(mdev: *mut mana_ib_dev, qp: *mut mana_ib_qp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mana_table_remove_qp(mdev: *mut mana_ib_dev, qp: *mut mana_ib_qp) {
    ...
    struct mana_ib_qp *qp = container_of(ibqp, struct mana_ib_qp, ibqp);
    let mut resp: mana_ib_create_rc_qp_resp = {};
    struct mana_ib_ucontext *mana_ucontext;
    let mut ucmd: mana_ib_create_rc_qp = {};
    int i, err, j;
    let mut flags: u64 = 0;
    u32 doorbell;
    if (!udata)
...
// skip FMR for user-level RC QPs
    ...
    struct mana_ib_dev *mdev = container_of(ibpd.device, struct mana_ib_dev, ib_dev);
    struct mana_ib_qp *qp = container_of(ibqp, struct mana_ib_qp, ibqp);
    let mut resp: mana_ib_create_uc_qp_resp = {};
    struct mana_ib_ucontext *mana_ucontext;
...
#[no_mangle]
unsafe extern "C" fn mana_add_qp_to_cqs(qp: *mut mana_ib_qp) {
...
#[no_mangle]
unsafe extern "C" fn mana_remove_qp_from_cqs(qp: *mut mana_ib_qp) {
...
// When rwq_ind_tbl is used, it's for creating WQs for RSS
    ...
    struct mana_ib_qp *qp = container_of(ibqp, struct mana_ib_qp, ibqp);
    let mut resp: mana_rnic_set_qp_state_resp = {};
    let mut req: mana_rnic_set_qp_state_req = {};
...
#[no_mangle]
unsafe extern "C" fn mana_ib_destroy_qp_raw(qp: *mut mana_ib_qp, udata: *mut ib_udata) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mana_ib_destroy_rc_qp(qp: *mut mana_ib_qp, udata: *mut ib_udata) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mana_ib_destroy_uc_qp(qp: *mut mana_ib_qp, udata: *mut ib_udata) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mana_ib_destroy_ud_qp(qp: *mut mana_ib_qp, udata: *mut ib_udata) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mana_ib_destroy_qp(ibqp: *mut ib_qp, udata: *mut ib_udata) -> c_int {
