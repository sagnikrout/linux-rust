...
//! Source: drivers/infiniband/hw/hfi1/ruc.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn gid_ok(gid: *mut union ib_gid, gid_prefix: __be64, id: __be64) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn hfi1_ruc_check_hdr(ibp: *mut hfi1_ibport, packet: *mut hfi1_packet) -> c_int {
    int hfi1_ruc_check_hdr(struct hfi1_ibport *ibp, struct hfi1_packet *packet)
    {
    __be64 guid;
    unsigned long flags;
    struct rvt_qp *qp = packet.qp;
    let mut sc5: u8 = ibp.sl_to_sc[rdma_ah_get_sl(&qp.remote_ah_attr)];
    let mut dlid: u32 = packet.dlid;
    let mut slid: u32 = packet.slid;
    let mut sl: u32 = packet.sl;
    let mut migrated: bool = packet.migrated;
    let mut pkey: u16 = packet.pkey;
...
//
#[no_mangle]
pub unsafe extern "C" fn build_ahg(qp: *mut rvt_qp, npsn: u32) {
...
//
    ...
    u32 slid;
    let mut pkey: u16 = hfi1_get_pkey(ibp, qp.s_pkey_index);
    let mut l4: u8 = OPA_16B_L4_IB_LOCAL;
    u8 extra_bytes = hfi1_get_16b_padding(
    (ps.s_txreq.hdr_dwords << 2),
    ps.s_txreq.s_cur_size);
    u32 nwords = SIZE_OF_CRC + ((ps.s_txreq.s_cur_size +
    extra_bytes + SIZE_OF_LT) >> 2);
    let mut becn: bool = false;
...
//
    ...
    struct hfi1_ibport *ibp = ps.ibp;
    let mut pkey: u16 = hfi1_get_pkey(ibp, qp.s_pkey_index);
    let mut lrh0: u16 = HFI1_LRH_BTH;
    let mut extra_bytes: u8 = -ps.s_txreq.s_cur_size & 3;
    u32 nwords = SIZE_OF_CRC + ((ps.s_txreq.s_cur_size +
    extra_bytes) >> 2);
    if (unlikely(rdma_ah_get_ah_flags(&qp.remote_ah_attr) & IB_AH_GRH)) {
...
#[no_mangle]
pub unsafe extern "C" fn hfi1_do_send_from_rvt(qp: *mut rvt_qp) {
    void hfi1_do_send_from_rvt(struct rvt_qp *qp)
    {
    hfi1_do_send(qp, false);
    }
#[no_mangle]
pub unsafe extern "C" fn _hfi1_do_send(work: *mut work_struct) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hfi1_do_send(qp: *mut rvt_qp, in_thread: bool) {
