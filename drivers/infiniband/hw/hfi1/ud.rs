...
//! Source: drivers/infiniband/hw/hfi1/ud.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn ud_loopback(sqp: *mut rvt_qp, swqe: *mut rvt_swqe) {
    ...
    u16 pkey;
    u32 slid;
    let mut sc5: u8 = ibp.sl_to_sc[rdma_ah_get_sl(ah_attr)];
...
// Silently drop packets which are too big.
    ...
    struct ib_grh grh;
    let mut grd: ib_global_route = *(rdma_ah_read_grh(ah_attr));
//
...
//
    ...
    sge = &ssge.sge;
    while (length) {
    let mut len: u32 = rvt_get_sge_length(sge, length);
    WARN_ON_ONCE(len == 0);
...
// Setup the packet
    ...
    let mut dest_qp: u32 = rvt_get_swqe_remote_qpn(wqe);
    let mut src_qp: u32 = qp.ibqp.qp_num;
    u16 len, pkey;
    u8 l4, sc5;
    let mut is_mgmt: bool = false;
    ibp = to_iport(qp.ibqp.device, qp.port_num);
    ppd = ppd_from_ibp(ibp);
    ah_attr = rvt_get_swqe_ah_attr(wqe);
//
...
//
#[no_mangle]
pub unsafe extern "C" fn hfi1_make_ud_req(qp: *mut rvt_qp, ps: *mut hfi1_pkt_state) -> c_int {
...
// Construct the header.
    ...
    let mut tflags: c_ulong = ps.flags;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn hfi1_lookup_pkey_idx(ibp: *mut hfi1_ibport, pkey: u16) -> c_int {
    int hfi1_lookup_pkey_idx(struct hfi1_ibport *ibp, u16 pkey)
    {
    struct hfi1_pportdata *ppd = ppd_from_ibp(ibp);
    unsigned i;
    if (pkey == FULL_MGMT_P_KEY || pkey == LIM_MGMT_P_KEY) {
    let mut lim_idx: unsigned = -1;
...
// Convert dwords to flits
    ...
    if (ctxt) {
    pbuf = sc_buffer_alloc(ctxt, plen, core::ptr::null_mut(), core::ptr::null_mut());
    ...
    u16 lrh0;
    let mut sl: u8 = ibp.sc_to_sl[sc5];
    struct hfi1_opa_header hdr;
    struct ib_other_headers *ohdr;
    struct pio_buf *pbuf;
    struct send_context *ctxt = qp_to_send_context(qp, sc5);
...
//
#[no_mangle]
pub unsafe extern "C" fn hfi1_ud_rcv(packet: *mut hfi1_packet) {
    void hfi1_ud_rcv(struct hfi1_packet *packet)
    {
    let mut hdrsize: u32 = packet.hlen;
    struct ib_wc wc;
    u32 src_qp;
    u16 pkey;
    let mut mgmt_pkey_idx: c_int = -1;
    struct hfi1_ibport *ibp = rcd_to_iport(packet.rcd);
    struct hfi1_pportdata *ppd = ppd_from_ibp(ibp);
    void *data = packet.payload;
    let mut tlen: u32 = packet.tlen;
    struct rvt_qp *qp = packet.qp;
    let mut sc5: u8 = packet.sc;
    u8 sl_from_sc;
    let mut opcode: u8 = packet.opcode;
    let mut sl: u8 = packet.sl;
    let mut dlid: u32 = packet.dlid;
    let mut slid: u32 = packet.slid;
    u8 extra_bytes;
    let mut l4: u8 = 0;
    bool dlid_is_permissive;
    bool slid_is_permissive;
    let mut solicited: bool = false;
