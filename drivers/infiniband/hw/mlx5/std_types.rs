...
//! Source: drivers/infiniband/hw/mlx5/std_types.c
#![no_std]
...
//
    ...
    struct mlx5_ib_uapi_query_port *info)
    {
    let mut err: c_int = mlx5_vport_get_vhca_id(mdev, vport, &info.vport_vhca_id);
    if (err)
    return err;
    ...
    mdev = mlx5_ib_get_native_port_mdev(dev, port_num, core::ptr::null_mut());
    if (!mdev)
    return -EINVAL;
    ...
    struct uverbs_attr_bundle *attrs)
    {
    let mut info: mlx5_ib_uapi_query_port = {};
