...
//! Source: drivers/infiniband/sw/siw/siw_qp_tx.c
#![no_std]
...
// Copyright (c) 2008-2019, IBM Corporation
    ...
    let mut offset: u64 = addr - mem.va;
    let mut paddr: dma_addr_t = siw_pbl_get_buffer(pbl, offset, core::ptr::null_mut(), idx);
    if (paddr)
...
//
#[no_mangle]
unsafe extern "C" fn siw_try_1seg(c_tx: *mut siw_iwarp_tx, paddr: *mut c_void) -> c_int {
    static int siw_try_1seg(struct siw_iwarp_tx *c_tx, void *paddr)
    {
    struct siw_wqe *wqe = &c_tx.wqe_active;
    struct siw_sge *sge = &wqe.sqe.sge[0];
    let mut bytes: u32 = sge.length;
...
// Kernel client using kva
    ...
    let mut off: c_uint = sge.laddr & ~PAGE_MASK;
    struct page *p;
    char *buffer;
    let mut pbl_idx: c_int = 0;
    ...
    let mut part: c_ulong = bytes - (PAGE_SIZE - off);
...
pub const PKT_FRAGMENTED: c_int = 1;
pub const PKT_COMPLETE: c_int = 0;
//
// siw_qp_prepare_tx()
//
...
//
#[no_mangle]
unsafe extern "C" fn siw_qp_prepare_tx(c_tx: *mut siw_iwarp_tx) -> c_int {
    static int siw_qp_prepare_tx(struct siw_iwarp_tx *c_tx)
    {
    struct siw_wqe *wqe = &c_tx.wqe_active;
    char *crc = core::ptr::null_mut();
...
//
    ...
    let mut rv: c_int = siw_sendmsg(s, flags, &iov, 1, iov.iov_len);
    if (rv >= 0) {
    c_tx.ctrl_sent += rv;
...
//
    ...
    struct sock *sk = s.sk;
    let mut i: c_int = 0, rv = 0, sent = 0;
    while (size) {
    let mut bytes: usize = min_t(size_t, PAGE_SIZE - offset, size);
...
//
    ...
    unsigned int size)
    {
    let mut i: c_int = 0, sent = 0, rv;
    let mut sge_bytes: c_int = min(sge.length - offset, size);
...
#[no_mangle]
unsafe extern "C" fn siw_unmap_pages(iov: *mut kvec, kmap_mask: c_ulong, len: c_int) {
...
//
    for (i = (len-1); i >= 0; i--) {
    if (kmap_mask & BIT(i)) {
    let mut addr: c_ulong = (unsigned long)iov[i].iov_base;
...
//
    ...
    let mut seg: c_int = 0, do_crc = c_tx.do_crc, is_kva = 0, rv;
    unsigned int data_len = c_tx.bytes_unsent, hdr_len = 0, trl_len = 0,
    sge_off = c_tx.sge_off, sge_idx = c_tx.sge_idx,
    pbl_idx = c_tx.pbl_idx;
    let mut kmap_mask: c_ulong = 0L;
    ...
    let mut sge_len: c_uint = min(sge.length - sge_off, data_len);
    let mut fp_off: c_uint = (sge.laddr + sge_off) & ~PAGE_MASK;
    struct siw_mem *mem;
    if (!(tx_flags(wqe) & SIW_WQE_INLINE)) {
    mem = wqe.mem[sge_idx];
    is_kva = mem.mem_obj == core::ptr::null_mut() ? 1 : 0;
    } else {
    is_kva = 1;
    }
...
//
    ...
    while (sge_len) {
    let mut plen: usize = min((int)PAGE_SIZE - fp_off, sge_len);
    void *kaddr;
    if (!is_kva) {
    struct page *p;
...
// in sge->laddr.
//
    let mut va: u64 = sge.laddr + sge_off;
...
// (u32 *)c_tx->trailer.pad = 0;
    ...
    c_tx.trailer.crc = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: do_crc) -> else {
    else if (do_crc)
...
//
#[no_mangle]
unsafe extern "C" fn siw_prepare_fpdu(qp: *mut siw_qp, wqe: *mut siw_wqe) {
...
//
#[no_mangle]
unsafe extern "C" fn siw_qp_sq_proc_tx(qp: *mut siw_qp, wqe: *mut siw_wqe) -> c_int {
    static int siw_qp_sq_proc_tx(struct siw_qp *qp, struct siw_wqe *wqe)
    {
    struct siw_iwarp_tx *c_tx = &qp.tx_ctx;
    struct socket *s = qp.attrs.sk;
    let mut rv: c_int = 0, burst_len = qp.tx_ctx.burst;
    let mut ecode: enum rdmap_ecode = RDMAP_ECODE_CATASTROPHIC_STREAM;
...
//
    ...
    let mut tx_type: enum siw_opcode = tx_type(wqe);
...
#[no_mangle]
unsafe extern "C" fn siw_fastreg_mr(pd: *mut ib_pd, sqe: *mut siw_sqe) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn siw_qp_sq_proc_local(qp: *mut siw_qp, wqe: *mut siw_wqe) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_qp_sq_process(qp: *mut siw_qp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn siw_sq_resume(qp: *mut siw_qp) {
    static void siw_sq_resume(struct siw_qp *qp)
    {
    if (down_read_trylock(&qp.state_lock)) {
    if (likely(qp.attrs.state == SIW_QP_STATE_RTS &&
    !qp.tx_ctx.tx_suspend)) {
    let mut rv: c_int = siw_qp_sq_process(qp);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tx_task_t {
    pub active: llist_head,
    pub waiting: wait_queue_head_t,
}

    static DEFINE_PER_CPU(struct tx_task_t, siw_tx_task_g);
#[no_mangle]
pub unsafe extern "C" fn siw_create_tx_threads() -> c_int {
...
// Skip HT cores
    ...
    siw_tx_thread[cpu] = core::ptr::null_mut();
    continue;
    }
    assigned++;
    }
...
#[no_mangle]
pub unsafe extern "C" fn siw_stop_tx_threads() {
...
#[no_mangle]
pub unsafe extern "C" fn siw_run_sq(data: *mut c_void) -> c_int {
    int siw_run_sq(void *data)
    {
    let mut nr_cpu: c_int = (unsigned int)(long)data;
    struct llist_node *active;
    struct siw_qp *qp;
    struct tx_task_t *tx_task = &per_cpu(siw_tx_task_g, nr_cpu);
    while (1) {
    struct llist_node *fifo_list = core::ptr::null_mut();
...
//
    ...
    qp.tx_list.next = core::ptr::null_mut();
    siw_sq_resume(qp);
    }
    }
    active = llist_del_all(&tx_task.active);
...
#[no_mangle]
pub unsafe extern "C" fn siw_sq_start(qp: *mut siw_qp) -> c_int {
