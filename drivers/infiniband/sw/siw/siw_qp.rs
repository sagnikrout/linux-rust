...
//! Source: drivers/infiniband/sw/siw/siw_qp.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn siw_qp_llp_data_ready(sk: *mut sock) {
    ...
    let mut rd_desc: read_descriptor_t = { .arg.data = qp, .count = 1 };
...
#[no_mangle]
pub unsafe extern "C" fn siw_qp_llp_close(qp: *mut siw_qp) {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_qp_llp_write_space(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn siw_qp_readq_init(qp: *mut siw_qp, irq_size: c_int, orq_size: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_qp_mpa_rts(qp: *mut siw_qp, ctrl: enum mpa_v2_ctrl) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(MPA_V2_RDMA_READ_RTR: ctrl &) -> else {
    struct siw_sqe *rreq = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_tagged_error(state: enum siw_access_state) -> enum ddp_ecode {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_rdmap_error(state: enum siw_access_state) -> enum rdmap_ecode {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_send_terminate(qp: *mut siw_qp) {
    void siw_send_terminate(struct siw_qp *qp)
    {
    struct kvec iov[3];
    let mut msg: msghdr = { .msg_flags = MSG_DONTWAIT | MSG_EOR };
    struct iwarp_terminate *term = core::ptr::null_mut();
    union iwarp_hdr *err_hdr = core::ptr::null_mut();
...
// Adjust DDP Segment Length parameter, if valid
    if (term.flag_m) {
    let mut real_ddp_len: u32 = be16_to_cpu(rx_hdr.ctrl.mpa_len);
    let mut op: enum rdma_opcode = __rdmap_get_opcode(&rx_hdr.ctrl);
...
//
    ...
    enum siw_qp_attr_mask mask)
    {
    let mut drop_conn: c_int = 0, rv = 0;
    if (!mask)
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn siw_read_to_orq(rreq: *mut siw_sqe, sqe: *mut siw_sqe) {
...
#[no_mangle]
unsafe extern "C" fn siw_activate_tx_from_sq(qp: *mut siw_qp) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_activate_tx(qp: *mut siw_qp) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn siw_cq_notify_now(cq: *mut siw_cq, flags: u32) -> bool {
...
//
    ...
    let mut rv: c_int = 0;
    if (cq) {
    let mut sqe_flags: u32 = sqe.flags;
    struct siw_cqe *cqe;
    u32 idx;
    unsigned long flags;
...
// recycle SQE
    ...
    if (!READ_ONCE(cqe.flags)) {
    bool notify;
    let mut cqe_flags: u8 = SIW_WQE_VALID;
    cqe.id = rqe.id;
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_sq_flush(qp: *mut siw_qp) {
    void siw_sq_flush(struct siw_qp *qp)
    {
    struct siw_sqe *sqe;
    struct siw_wqe *wqe = tx_wqe(qp);
    let mut async_event: c_int = 0;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_rq_flush(qp: *mut siw_qp) {
...
#[no_mangle]
pub unsafe extern "C" fn siw_qp_add(sdev: *mut siw_device, qp: *mut siw_qp) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn siw_free_qp(ref: *mut kref) {
