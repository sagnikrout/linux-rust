...
//! Source: drivers/infiniband/sw/siw/siw_qp_rx.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn siw_rx_kva(srx: *mut siw_rx_stream, kva: *mut c_void, len: c_int) -> c_int {
...
//
    ...
    let mut sink_stag: u32 = be32_to_cpu(rresp.sink_stag);
    let mut sink_to: u64 = be64_to_cpu(rresp.sink_to);
...
//
    ...
    enum ddp_ecode ecode;
    let mut sink_stag: u32 = be32_to_cpu(write.sink_stag);
    let mut sink_to: u64 = be64_to_cpu(write.sink_to);
    if (frx.first_ddp_seg) {
...
//
    ...
    struct siw_wqe *wqe = &frx.wqe_active;
    enum ddp_ecode ecode;
    let mut ddp_msn: u32 = be32_to_cpu(send.ddp_msn);
    let mut ddp_mo: u32 = be32_to_cpu(send.ddp_mo);
    let mut ddp_qn: u32 = be32_to_cpu(send.ddp_qn);
...
// only valid for SEND_INV and SEND_SE_INV operations
    ...
    struct siw_rqe *rqe;
    struct siw_srq *srq;
    struct siw_wqe *wqe = core::ptr::null_mut();
    let mut srq_event: bool = false;
    unsigned long flags;
    srq = qp.srq;
    if (srq) {
    ...
    let mut num_sge: c_int = rqe.num_sge;
...
// Test SRQ limit
    ...
    {
    int rv;
    if (mem_p.mem_obj == core::ptr::null_mut())
    rv = siw_rx_kva(srx, ib_virt_dma_to_ptr(addr), bytes);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !mem_p->is_pbl) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_proc_send(qp: *mut siw_qp) -> c_int {
...
//
    pd = qp.srq == core::ptr::null_mut() ? qp.pd : qp.srq.base_srq.pd;
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_proc_write(qp: *mut siw_qp) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_proc_rreq(qp: *mut siw_qp) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn siw_init_rresp(qp: *mut siw_qp, srx: *mut siw_rx_stream) -> c_int {
...
//
    tx_work.processed = 0;
    tx_work.mem[0] = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn siw_orqe_start_rx(qp: *mut siw_qp) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn siw_proc_rresp(qp: *mut siw_qp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn siw_update_skb_rcvd(srx: *mut siw_rx_stream, length: u16) {
...
#[no_mangle]
pub unsafe extern "C" fn siw_proc_terminate(qp: *mut siw_qp) -> c_int {
    ...
    enum rdma_opcode op;
    let mut to_copy: u16 = sizeof(struct iwarp_ctrl);
...
#[no_mangle]
unsafe extern "C" fn siw_get_trailer(qp: *mut siw_qp, srx: *mut siw_rx_stream) -> c_int {
    static int siw_get_trailer(struct siw_qp *qp, struct siw_rx_stream *srx)
    {
    struct sk_buff *skb = srx.skb;
    let mut avail: c_int = min(srx.skb_new, srx.fpdu_part_rem);
...
#[no_mangle]
unsafe extern "C" fn siw_get_hdr(srx: *mut siw_rx_stream) -> c_int {
...
//
    ...
    let mut hdrlen: c_int = iwarp_pktinfo[opcode].hdr_len;
...
#[no_mangle]
unsafe extern "C" fn siw_check_tx_fence(qp: *mut siw_qp) -> c_int {
    static int siw_check_tx_fence(struct siw_qp *qp)
    {
    struct siw_wqe *tx_waiting = tx_wqe(qp);
    struct siw_sqe *rreq;
    let mut resume_tx: c_int = 0, rv = 0;
    unsigned long flags;
...
//
#[no_mangle]
unsafe extern "C" fn siw_rdmap_complete(qp: *mut siw_qp, error: c_int) -> c_int {
    static int siw_rdmap_complete(struct siw_qp *qp, int error)
    {
    struct siw_rx_stream *srx = &qp.rx_stream;
    struct siw_wqe *wqe = rx_wqe(qp.rx_fpdu);
    let mut wc_status: enum siw_wc_status = wqe.wc_status;
    let mut opcode: u8 = __rdmap_get_opcode(&srx.hdr.ctrl);
    let mut rv: c_int = 0;
    switch (opcode) {
    case RDMAP_SEND_SE:
...
//
    ...
    rx_mem(&qp.rx_tagged) = core::ptr::null_mut();
    }
    break;
    default:
    break;
...
//
    ...
    while (srx.skb_new) {
    let mut run_completion: c_int = 1;
