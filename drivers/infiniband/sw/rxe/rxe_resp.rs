...
//! Source: drivers/infiniband/sw/rxe/rxe_resp.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn rxe_resp_queue_pkt(qp: *mut rxe_qp, skb: *mut sk_buff) {
...
// pkt_p = SKB_TO_PKT(skb);
    ...
    {
    let mut diff: c_int = psn_compare(pkt.psn, qp.resp.psn);
    ...
    let mut flush_type: u32 = feth_plt(pkt);
...
#[no_mangle]
unsafe extern "C" fn get_srq_wqe(qp: *mut rxe_qp) -> enum resp_states {
...
#[no_mangle]
unsafe extern "C" fn rxe_get_recv_wqe(qp: *mut rxe_qp) -> enum resp_states {
...
//
    ...
    let mut payload: c_uint = payload_size(pkt);
    let mut recv_buffer_len: c_uint = 0;
    int i;
...
//
#[no_mangle]
unsafe extern "C" fn qp_resp_from_reth(qp: *mut rxe_qp, pkt: *mut rxe_pkt_info) {
    static void qp_resp_from_reth(struct rxe_qp *qp, struct rxe_pkt_info *pkt)
    {
    let mut length: c_uint = reth_len(pkt);
...
#[no_mangle]
unsafe extern "C" fn qp_resp_from_atmeth(qp: *mut rxe_qp, pkt: *mut rxe_pkt_info) {
...
//
#[no_mangle]
unsafe extern "C" fn get_rkey_violation_state(pkt: *mut rxe_pkt_info) -> enum resp_states {
...
//
    ...
    struct rxe_mw *mw = core::ptr::null_mut();
    u64 va;
    u32 rkey;
    u32 resid;
    u32 pktlen;
    let mut mtu: c_int = qp.mtu;
...
//
    ...
    struct rxe_pkt_info *pkt)
    {
    let mut rc: enum resp_states = RESPST_NONE;
    int	err;
    let mut data_len: c_int = payload_size(pkt);
...
// next expected psn, read handles this separately
    ...
    if (!res.replay) {
    let mut iova: u64 = qp.resp.va + qp.resp.offset;
    if (is_odp_mr(mr))
...
#[no_mangle]
unsafe extern "C" fn invalidate_rkey(qp: *mut rxe_qp, rkey: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn execute(qp: *mut rxe_qp, pkt: *mut rxe_pkt_info) -> enum resp_states {
...
// Unreachable
    ...
    let mut rkey: u32 = ieth_rkey(pkt);
...
#[no_mangle]
pub unsafe extern "C" fn if(IB_QPT_RC: qp_type(qp) ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn send_ack(qp: *mut rxe_qp, syndrome: u8, psn: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn send_atomic_ack(qp: *mut rxe_qp, syndrome: u8, psn: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn send_read_response_ack(qp: *mut rxe_qp, syndrome: u8, psn: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(RXE_ATOMIC_MASK: pkt->mask &) -> else {
    else if (pkt.mask & RXE_ATOMIC_MASK)
    send_atomic_ack(qp, AETH_ACK_UNLIMITED, pkt.psn);
#[no_mangle]
pub unsafe extern "C" fn if(RXE_ATOMIC_WRITE_MASK): pkt->mask & (RXE_FLUSH_MASK |) -> else {
    else if (pkt.mask & (RXE_FLUSH_MASK | RXE_ATOMIC_WRITE_MASK))
    send_read_response_ack(qp, AETH_ACK_UNLIMITED, pkt.psn);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: bth_ack(pkt)) -> else {
    else if (bth_ack(pkt))
    ...
    {
    enum resp_states rc;
    let mut prev_psn: u32 = (qp.resp.ack_psn - 1) & BTH_PSN_MASK;
    if (pkt.mask & RXE_SEND_MASK ||
    pkt.mask & RXE_WRITE_MASK) {
...
//
    let mut iova: u64 = reth_va(pkt);
    let mut resid: u32 = reth_len(pkt);
...
#[no_mangle]
unsafe extern "C" fn do_qp_event(qp: *mut rxe_qp, etype: enum ib_event_type) {
...
#[no_mangle]
unsafe extern "C" fn do_class_d1e_error(qp: *mut rxe_qp) -> enum resp_states {
...
#[no_mangle]
unsafe extern "C" fn drain_req_pkts(qp: *mut rxe_qp) {
...
#[no_mangle]
unsafe extern "C" fn flush_recv_wqe(qp: *mut rxe_qp, wqe: *mut rxe_recv_wqe) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn flush_recv_queue(qp: *mut rxe_qp, notify: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn rxe_receiver(qp: *mut rxe_qp) -> c_int {
    int rxe_receiver(struct rxe_qp *qp)
    {
    struct rxe_dev *rxe = to_rdev(qp.ibqp.device);
    enum resp_states state;
    struct rxe_pkt_info *pkt = core::ptr::null_mut();
    int ret;
    unsigned long flags;
    spin_lock_irqsave(&qp.state_lock, flags);
    if (!qp.valid || qp_state(qp) == IB_QPS_ERR ||
    qp_state(qp) == IB_QPS_RESET) {
    let mut notify: bool = qp.valid && (qp_state(qp) == IB_QPS_ERR);
    drain_req_pkts(qp);
