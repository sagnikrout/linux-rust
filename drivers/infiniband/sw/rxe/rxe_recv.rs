...
//! Source: drivers/infiniband/sw/rxe/rxe_recv.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn set_bad_pkey_cntr(port: *mut rxe_port) {
...
#[no_mangle]
unsafe extern "C" fn set_qkey_viol_cntr(port: *mut rxe_port) {
    ...
    let mut qkey: u32 = (qpn == 1) ? GSI_QKEY : qp.attr.qkey;
...
#[no_mangle]
unsafe extern "C" fn hdr_check(pkt: *mut rxe_pkt_info) -> c_int {
    static int hdr_check(struct rxe_pkt_info *pkt)
    {
    struct rxe_dev *rxe = pkt.rxe;
    struct rxe_port *port = &rxe.port;
    struct rxe_qp *qp = core::ptr::null_mut();
    let mut qpn: u32 = bth_qpn(pkt);
    int index;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn rxe_rcv_pkt(pkt: *mut rxe_pkt_info, skb: *mut sk_buff) {
...
#[no_mangle]
unsafe extern "C" fn rxe_rcv_mcast_pkt(rxe: *mut rxe_dev, skb: *mut sk_buff) {
...
//
    ...
    rxe_get(qp);
    rxe_rcv_pkt(pkt, skb);
    skb = core::ptr::null_mut();	/* mark consumed */
    }
    }
...
//
#[no_mangle]
unsafe extern "C" fn rxe_chk_dgid(rxe: *mut rxe_dev, skb: *mut sk_buff) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rxe_rcv(skb: *mut sk_buff) {
    void rxe_rcv(struct sk_buff *skb)
    {
    int err;
    struct rxe_pkt_info *pkt = SKB_TO_PKT(skb);
    struct rxe_dev *rxe = pkt.rxe;
    let mut skblen: c_uint = skb.len - skb_network_offset(skb);
