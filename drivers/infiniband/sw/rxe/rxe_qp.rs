...
//! Source: drivers/infiniband/sw/rxe/rxe_qp.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn rxe_reclassify_send_socket(sock: *mut socket) {
...
#[no_mangle]
pub unsafe extern "C" fn rxe_qp_chk_init(rxe: *mut rxe_dev, init: *mut ib_qp_init_attr) -> c_int {
    int rxe_qp_chk_init(struct rxe_dev *rxe, struct ib_qp_init_attr *init)
    {
    struct ib_qp_cap *cap = &init.cap;
    struct rxe_port *port;
    let mut port_num: c_int = init.port_num;
    switch (init.qp_type) {
    case IB_QPT_GSI:
    case IB_QPT_RC:
    case IB_QPT_UC:
...
#[no_mangle]
unsafe extern "C" fn alloc_rd_atomic_resources(qp: *mut rxe_qp, n: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn free_rd_atomic_resources(qp: *mut rxe_qp) {
    ...
    qp.resp.resources = core::ptr::null_mut();
    qp.resp.res = core::ptr::null_mut();
    }
    }
#[no_mangle]
pub unsafe extern "C" fn free_rd_atomic_resource(res: *mut resp_res) {
    void free_rd_atomic_resource(struct resp_res *res)
    {
    res.type = 0;
    }
#[no_mangle]
unsafe extern "C" fn cleanup_rd_atomic_resources(qp: *mut rxe_qp) {
...
// prepare info for caller to mmap send queue if user space qp
    err = do_mmap_info(rxe, uresp ? &uresp.sq_mi : core::ptr::null_mut(), udata,
...
// prepare info for caller to mmap recv queue if user space qp
    err = do_mmap_info(rxe, uresp ? &uresp.rq_mi : core::ptr::null_mut(), udata,
...
// called by the create qp verb
    ...
    struct rxe_srq *srq = init.srq ? to_rsrq(init.srq) : core::ptr::null_mut();
    unsigned long flags;
    rxe_get(pd);
    rxe_get(rcq);
    rxe_get(scq);
    ...
    qp.rcq = core::ptr::null_mut();
    qp.scq = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn rxe_qp_to_init(qp: *mut rxe_qp, init: *mut ib_qp_init_attr) -> c_int {
    ...
    let mut max_mtu: enum ib_mtu = port.attr.max_mtu;
    let mut mtu: enum ib_mtu = attr.path_mtu;
    if (mtu > max_mtu) {
...
#[no_mangle]
unsafe extern "C" fn rxe_qp_reset(qp: *mut rxe_qp) {
...
#[no_mangle]
pub unsafe extern "C" fn rxe_qp_error(qp: *mut rxe_qp) {
...
// According to the spec, timeout = 4.096 * 2 ^ attr->timeout [us]
    let mut j: c_int = nsecs_to_jiffies(4096ULL << attr.timeout);
...
// called by the query qp verb
#[no_mangle]
pub unsafe extern "C" fn rxe_qp_to_attr(qp: *mut rxe_qp, attr: *mut ib_qp_attr, mask: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn rxe_qp_chk_destroy(qp: *mut rxe_qp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rxe_qp_do_cleanup(work: *mut work_struct) {
...
// called when the last reference to the qp is dropped
#[no_mangle]
pub unsafe extern "C" fn rxe_qp_cleanup(elem: *mut rxe_pool_elem) {
