...
//! Source: drivers/infiniband/sw/rxe/rxe_srq.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn rxe_srq_chk_init(rxe: *mut rxe_dev, init: *mut ib_srq_init_attr) -> c_int {
    ...
    err = do_mmap_info(rxe, uresp ? &uresp.mi : core::ptr::null_mut(), udata, q.buf,
    q.buf_size, &q.ip);
    if (err) {
    ...
    struct mminfo __user *mi = core::ptr::null_mut();
    int wqe_size;
    int err;
    if (mask & IB_SRQ_MAX_WR) {
//
...
#[no_mangle]
pub unsafe extern "C" fn rxe_srq_cleanup(elem: *mut rxe_pool_elem) {
