...
//! Source: drivers/infiniband/core/uverbs_uapi.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ib_uverbs_notsupp(attrs: *mut uverbs_attr_bundle) -> c_int {
...
// exists = true;
    ...
    let mut method_key: u32 = obj_key;
    bool exists;
    if (def.write.is_ex)
...
// cur_method_key = method_key;
    ...
    bool is_driver)
    {
    let mut method_key: u32 = obj_key | uapi_key_ioctl_method(method.id);
...
//
    ...
    let mut access: u8 = attr.attr.u2.objs_arr.access;
...
//
    ...
    let mut cur_obj_key: u32 = UVERBS_API_KEY_ERR;
    let mut cur_method_key: u32 = UVERBS_API_KEY_ERR;
    bool exists;
    int rc;
    if (!def_list)
    return 0;
    ...
    let mut max_bkey: c_uint = 0;
    let mut single_uobj: bool = false;
    void __rcu **slot;
    ...
    let mut attr_key: u32 = iter.index & UVERBS_API_ATTR_KEY_MASK;
    let mut attr_bkey: u32 = uapi_bkey_attr(attr_key);
    let mut type: u8 = elm.spec.type;
    ...
    let mut access: u8 = elm.spec.u.obj.access;
//
...
#[no_mangle]
unsafe extern "C" fn uapi_finalize(uapi: *mut uverbs_api) -> c_int {
    static int uapi_finalize(struct uverbs_api *uapi)
    {
    const struct uverbs_api_write_method **data;
    let mut max_write_ex: c_ulong = 0;
    let mut max_write: c_ulong = 0;
    struct radix_tree_iter iter;
    void __rcu **slot;
    int rc;
    int i;
...
#[no_mangle]
unsafe extern "C" fn uapi_remove_range(uapi: *mut uverbs_api, start: u32, last: u32) {
...
#[no_mangle]
unsafe extern "C" fn uapi_remove_object(uapi: *mut uverbs_api, obj_key: u32) {
...
#[no_mangle]
unsafe extern "C" fn uapi_remove_method(uapi: *mut uverbs_api, method_key: u32) {
...
#[no_mangle]
unsafe extern "C" fn uapi_get_obj_id(spec: *mut uverbs_attr_spec) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn uapi_key_okay(key: u32) {
...
#[no_mangle]
unsafe extern "C" fn uapi_finalize_disable(uapi: *mut uverbs_api) {
    static void uapi_finalize_disable(struct uverbs_api *uapi)
    {
    struct radix_tree_iter iter;
    let mut starting_key: u32 = 0;
    let mut scan_again: bool = false;
    void __rcu **slot;
    again:
...
#[no_mangle]
pub unsafe extern "C" fn uverbs_destroy_api(uapi: *mut uverbs_api) {
...
//
#[no_mangle]
pub unsafe extern "C" fn uverbs_disassociate_api_pre(uverbs_dev: *mut ib_uverbs_device) {
    void uverbs_disassociate_api_pre(struct ib_uverbs_device *uverbs_dev)
    {
    struct uverbs_api *uapi = uverbs_dev.uapi;
    struct radix_tree_iter iter;
    void __rcu **slot;
    rcu_assign_pointer(uverbs_dev.ib_dev, core::ptr::null_mut());
    radix_tree_for_each_slot (slot, &uapi.radix, &iter, 0) {
    if (uapi_key_is_ioctl_method(iter.index)) {
    struct uverbs_api_ioctl_method *method_elm =
    rcu_dereference_protected(*slot, true);
    if (method_elm.driver_method)
    rcu_assign_pointer(method_elm.handler, core::ptr::null_mut());
    }
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn uverbs_disassociate_api(uapi: *mut uverbs_api) {
...
//
    ...
    continue;
    object_elm.type_attrs = core::ptr::null_mut();
    } else if (uapi_key_is_attr(iter.index)) {
    struct uverbs_api_attr *elm =
    rcu_dereference_protected(*slot, true);
    if (elm.spec.type == UVERBS_ATTR_TYPE_ENUM_IN)
    elm.spec.u2.enum_def.ids = core::ptr::null_mut();
    }
    }
    }
