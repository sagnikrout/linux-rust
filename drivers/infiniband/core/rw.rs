...
//! Source: drivers/infiniband/core/rw.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn rdma_rw_can_use_mr(dev: *mut ib_device, port_num: u32) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn rdma_rw_inv_key(reg: *mut rdma_rw_reg_ctx) -> c_int {
    ...
    reg.inv_wr.next = core::ptr::null_mut();
    }
    return count;
    }
// Caller must have zero-initialized *reg.
    ...
    qp.integrity_en);
    let mut nents: u32 = min(sg_cnt, pages_per_mr);
    ...
    u64 remote_addr, u32 rkey, enum dma_data_direction dir)
    {
    struct rdma_rw_reg_ctx *prev = core::ptr::null_mut();
    u32 pages_per_mr = rdma_rw_fr_page_list_len(qp.pd.device,
    qp.integrity_en);
    ...
    if (prev)
    prev.wr.wr.next = core::ptr::null_mut();
    ctx.type = RDMA_RW_MR;
    return count;
    out_free:
    while (--i >= 0)
    ...
    struct rdma_rw_reg_ctx *prev = core::ptr::null_mut();
    let mut pages_per_mr: u32 = rdma_rw_fr_page_list_len(dev, qp.integrity_en);
...
//
    ...
    let mut bv: bio_vec = mp_bvec_iter_bvec(bvecs, *iter);
    if (nents >= nr_bvec) {
    ret = -EINVAL;
    goto out_free_sgl;
    }
...
// DMA map the scatterlist
    ...
    for (i = 0; i < ctx.nr_ops; i++) {
    struct rdma_rw_reg_ctx *reg = &ctx.reg[i];
    let mut sge_cnt: u32 = min(nents, pages_per_mr);
    ...
    let mut nr_sge: u32 = min(sg_cnt, max_sge);
    if (dir == DMA_TO_DEVICE)
    ...
    &ctx.map.wrs[i + 1].wr : core::ptr::null_mut();
    }
    ...
    struct ib_sge *sge;
    let mut total_len: u32 = 0, i, j;
    let mut mapped_bvecs: u32 = 0;
    let mut nr_ops: u32 = DIV_ROUND_UP(nr_bvec, max_sge);
    let mut sges_size: usize = array_size(nr_bvec, sizeof(*ctx.map.sges));
    let mut wrs_offset: usize = ALIGN(sges_size, __alignof__(*ctx.map.wrs));
    let mut wrs_size: usize = array_size(nr_ops, sizeof(*ctx.map.wrs));
    void *mem;
    ...
    for (i = 0; i < nr_ops; i++) {
    struct ib_rdma_wr *rdma_wr = &ctx.map.wrs[i];
    let mut nr_sge: u32 = min(nr_bvec - mapped_bvecs, max_sge);
    if (dir == DMA_TO_DEVICE)
    rdma_wr.wr.opcode = IB_WR_RDMA_WRITE;
    else
    rdma_wr.wr.opcode = IB_WR_RDMA_READ;
...
//
    ...
    let mut total_len: usize = iter.bi_size;
...
// Link all bvecs into the IOVA space
    while (iter.bi_size) {
    let mut bv: bio_vec = mp_bvec_iter_bvec(bvec, *iter);
...
//
    ...
    ret = ib_map_mr_sg_pi(ctx.reg.mr, sg, sgt.nents, core::ptr::null_mut(), prot_sg,
    prot_sgt.nents, core::ptr::null_mut(), SZ_4K);
    if (unlikely(ret)) {
    ...
    ctx.reg.reg_wr.wr.wr_cqe = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn rdma_rw_update_lkey(reg: *mut rdma_rw_reg_ctx, need_inval: bool) {
...
//
    ...
    return ib_post_send(qp, first_wr, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn rdma_rw_init_qp(dev: *mut ib_device, attr: *mut ib_qp_init_attr) {
...
#[no_mangle]
pub unsafe extern "C" fn rdma_rw_init_mrs(qp: *mut ib_qp, attr: *mut ib_qp_init_attr) -> c_int {
    int rdma_rw_init_mrs(struct ib_qp *qp, struct ib_qp_init_attr *attr)
    {
    struct ib_device *dev = qp.pd.device;
    let mut nr_mrs: u32 = 0, nr_sig_mrs = 0, max_num_sg = 0;
    let mut ret: c_int = 0;
    if (attr.create_flags & IB_QP_CREATE_INTEGRITY_EN) {
...
#[no_mangle]
pub unsafe extern "C" fn rdma_rw_cleanup_mrs(qp: *mut ib_qp) {
