...
//! Source: drivers/infiniband/core/uverbs_std_types_dmabuf.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn uverbs_dmabuf_pin(attach: *mut dma_buf_attachment) -> c_int {
    static int uverbs_dmabuf_pin(struct dma_buf_attachment *attach)
    {
    return -EOPNOTSUPP;
    }
#[no_mangle]
unsafe extern "C" fn uverbs_dmabuf_unpin(attach: *mut dma_buf_attachment) {
    static void uverbs_dmabuf_unpin(struct dma_buf_attachment *attach)
    {
    }
#[no_mangle]
unsafe extern "C" fn uverbs_dmabuf_release(dmabuf: *mut dma_buf) {
...
//
    ...
    struct ib_uverbs_dmabuf_file *uverbs_dmabuf =
    container_of(uobj, struct ib_uverbs_dmabuf_file, uobj);
    let mut wait_for_comp: bool = false;
...
// Matches the get done as part of pgoff_to_mmap_entry()
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), O_RDONLY),
