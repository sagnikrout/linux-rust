...
//! Source: drivers/infiniband/core/uverbs_std_types_device.c
#![no_std]
...
//
    ...
    return -ENOSPC;
    attrs.uobject = core::ptr::null_mut();
    ...
    u64 *total)
    {
    let mut max_count: u64 = out_len / sizeof(u32);
...
// This ABI needs to be extended to provide any speed more than IB_SPEED_NDR
    ...
    let mut resp: ib_uverbs_query_port_resp_ex = {};
...
// FIXME: Extend the UAPI_DEF_OBJ_NEEDS_FN stuff..
    ...
    struct uverbs_attr_bundle *attrs)
    {
    let mut num_comp: u32 = attrs.ufile.device.num_comp_vectors;
    let mut core_support: u64 = IB_UVERBS_CORE_SUPPORT_OPTIONAL_MR_ACCESS;
    struct ib_device *ib_dev;
    int ret;
    ...
    struct uverbs_attr_bundle *attrs)
    {
    let mut entry: ib_uverbs_gid_entry = {};
    ...
    struct uverbs_attr_bundle *attrs)
    {
    let mut caps: ib_comp_cntr_caps = {};
    struct ib_ucontext *ucontext;
    struct ib_device *ib_dev;
    int ret;
    ucontext = ib_uverbs_get_ucontext(attrs);
