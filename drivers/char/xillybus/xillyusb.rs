...
//! Source: drivers/char/xillybus/xillyusb.c
#![no_std]
...
pub const BUF_SIZE_ORDER: c_int = 4;
pub const BUFNUM: c_int = 8;
pub const LOG2_IDT_FIFO_SIZE: c_int = 16;
pub const LOG2_INITIAL_FIFO_BUF_SIZE: c_int = 16;
pub const MSG_EP_NUM: c_int = 1;
pub const IN_EP_NUM: c_int = 1;
    static const char xillyname[] = "xillyusb";
    static unsigned int fifo_buf_order;
    static struct workqueue_struct *wakeup_wq;
pub const USB_VENDOR_ID_XILINX: c_uint = 0x03fd;
pub const USB_VENDOR_ID_ALTERA: c_uint = 0x09fb;
pub const USB_PRODUCT_ID_XILLYUSB: c_uint = 0xebbe;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xillyfifo {
    pub /: *mut *mut unsigned int bufsize; / In bytes, always a power of 2,
    pub bufnum: c_uint,
    pub /: *mut *mut *mut unsigned int size; / Lazy: Equals bufsize  bufnum,
    pub buf_order: c_uint,
    pub /: *mut *mut int fill; / Number of bytes in the FIFO,
    pub lock: spinlock_t,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xillyusb_endpoint {
    pub xdev: *mut xillyusb_dev,
    pub /: *mut *mut mutex ep_mutex; / serialize operations on endpoint,
    pub buffers: list_head,
    pub filled_buffers: list_head,
    pub /: *mut *mut spinlock_t buffers_lock; / protect these two lists,
    pub order: c_uint,
    pub buffer_size: c_uint,
    pub fill_mask: c_uint,
    pub outstanding_urbs: c_int,
    pub anchor: usb_anchor,
    pub fifo: xillyfifo,
    pub workitem: work_struct,
    pub shutting_down: bool,
    pub drained: bool,
    pub wake_on_drain: bool,
    pub ep_num: u8,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct xillyusb_channel {
    pub xdev: *mut xillyusb_dev,
    pub in_fifo: *mut xillyfifo,
    pub out_ep: *mut xillyusb_endpoint,
    pub /: *mut *mut mutex lock; / protect @out_ep, @in_fifo, bit fields below,
    pub /: *mut *mut mutex in_mutex; / serialize fops on FPGA to host stream,
    pub /: *mut *mut mutex out_mutex; / serialize fops on host to FPGA stream,
    pub flushq: wait_queue_head_t,
    pub chan_idx: c_int,
    pub in_consumed_bytes: u32,
    pub in_current_checkpoint: u32,
    pub out_bytes: u32,
    pub in_log2_element_size: c_uint,
    pub out_log2_element_size: c_uint,
    pub in_log2_fifo_size: c_uint,
    pub out_log2_fifo_size: c_uint,
    pub /: *mut *mut unsigned int read_data_ok; / EOF not arrived (yet),
    pub poll_used: c_uint,
    pub flushing: c_uint,
    pub flushed: c_uint,
    pub canceled: c_uint,
// Bit fields protected by @lock except for initialization
    pub readable:1: unsigned,
    pub writable:1: unsigned,
    pub open_for_read:1: unsigned,
    pub open_for_write:1: unsigned,
    pub in_synchronous:1: unsigned,
    pub out_synchronous:1: unsigned,
    pub in_seekable:1: unsigned,
    pub out_seekable:1: unsigned,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xillyusb_dev {
    pub channels: *mut xillyusb_channel,
    pub udev: *mut usb_device,
    pub /: *mut *mut *mut device dev; / For dev_err() and such,
    pub kref: kref,
    pub workq: *mut workqueue_struct,
    pub error: c_int,
    pub /: *mut *mut spinlock_t error_lock; / protect @error,
    pub wakeup_workitem: work_struct,
    pub num_channels: c_int,
    pub msg_ep: *mut xillyusb_endpoint,
    pub in_ep: *mut xillyusb_endpoint,
    pub /: *mut *mut mutex msg_mutex; / serialize opcode transmission,
    pub in_bytes_left: c_int,
    pub leftover_chan_num: c_int,
    pub in_counter: c_uint,
    pub /: *mut *mut mutex process_in_mutex; / synchronize wakeup_all(),
}
...
//
    ...
    let mut todo: c_uint = len;
    unsigned int nmax;
    let mut writepos: c_uint = fifo.writepos;
    let mut writebuf: c_uint = fifo.writebuf;
    unsigned long flags;
    int rc;
    nmax = fifo.size - READ_ONCE(fifo.fill);
    while (1) {
    let mut nrail: c_uint = fifo.bufsize - writepos;
    let mut n: c_uint = min(todo, nmax);
    if (n == 0) {
    ...
    let mut todo: c_uint = len;
    unsigned int fill;
    let mut readpos: c_uint = fifo.readpos;
    let mut readbuf: c_uint = fifo.readbuf;
    unsigned long flags;
    int rc;
//
...
//
    ...
    spin_unlock_irqrestore(&fifo.lock, flags);
    while (1) {
    let mut nrail: c_uint = fifo.bufsize - readpos;
    let mut n: c_uint = min(todo, fill);
    if (n == 0) {
    spin_lock_irqsave(&fifo.lock, flags);
    fifo.fill -= done;
    spin_unlock_irqrestore(&fifo.lock, flags);
...
//
#[no_mangle]
unsafe extern "C" fn xilly_copy_from_user(dst: *mut c_void, src: *const c_void, n: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xilly_copy_to_user(dst: *mut c_void, src: *const c_void, n: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xilly_memcpy(dst: *mut c_void, src: *const c_void, n: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fifo_mem_release(fifo: *mut xillyfifo) {
...
//
#[no_mangle]
unsafe extern "C" fn endpoint_quiesce(ep: *mut xillyusb_endpoint) {
...
//
#[no_mangle]
unsafe extern "C" fn endpoint_dealloc(ep: *mut xillyusb_endpoint) {
...
#[no_mangle]
unsafe extern "C" fn cleanup_dev(kref: *mut kref) {
    ...
    kfree(xdev.channels); /* Argument may be core::ptr::null_mut(), and that's fine */
    kfree(xdev);
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn wakeup_all(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn bulk_in_completer(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn bulk_out_completer(urb: *mut urb) {
...
#[no_mangle]
unsafe extern "C" fn try_queue_bulk_in(ep: *mut xillyusb_endpoint) {
    ...
    int rc;
    unsigned long flags;
    let mut bufsize: c_uint = ep.buffer_size;
    mutex_lock(&ep.ep_mutex);
...
#[no_mangle]
unsafe extern "C" fn try_queue_bulk_out(ep: *mut xillyusb_endpoint) {
...
#[no_mangle]
unsafe extern "C" fn bulk_out_work(work: *mut work_struct) {
    ...
    let mut chan_idx: c_int = chan_num >> 1;
...
#[no_mangle]
unsafe extern "C" fn process_bulk_in(xb: *mut xillybuffer) -> c_int {
    static int process_bulk_in(struct xillybuffer *xb)
    {
    struct xillyusb_endpoint *ep = xb.ep;
    struct xillyusb_dev *xdev = ep.xdev;
    struct device *dev = xdev.dev;
    let mut dws: c_int = xb.len >> 2;
    __le32 *p = xb.buf;
    u32 ctrlword;
    struct xillyusb_channel *chan;
    struct xillyfifo *fifo;
    let mut chan_num: c_int = 0, opcode;
    int chan_idx;
    int bytes, count, dwconsume;
    let mut in_bytes_left: c_int = 0;
    int rc;
    if ((dws << 2) != xb.len) {
    ...
    let mut in_counter: c_uint = xdev.in_counter++ & 0x3ff;
    if (count != in_counter) {
...
#[no_mangle]
unsafe extern "C" fn bulk_in_work(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: consumed) -> else {
    else if (consumed)
...
//
    ...
    let mut chan_num: c_int = chan.chan_idx << 1;
    ...
    if (chan.flushing) {
    let mut cancel_deadline: c_long = jiffies + 1 + XILLY_RESPONSE_TIMEOUT;
    chan.canceled = 0;
...
// request_read_anything(): Ask the FPGA for any little amount of data
    ...
    {
    struct xillyusb_dev *xdev = chan.xdev;
    let mut sh: c_uint = chan.in_log2_element_size;
    let mut chan_num: c_int = (chan.chan_idx << 1) | 1;
    let mut mercy: u32 = chan.in_consumed_bytes + (2 << sh) - 1;
    return xillyusb_send_opcode(xdev, chan_num, opcode, mercy >> sh);
    }
#[no_mangle]
unsafe extern "C" fn xillyusb_open(inode: *mut inode, filp: *mut file) -> c_int {
    static int xillyusb_open(struct inode *inode, struct file *filp)
    {
    struct xillyusb_dev *xdev;
    struct xillyusb_channel *chan;
    struct xillyfifo *in_fifo = core::ptr::null_mut();
    struct xillyusb_endpoint *out_ep = core::ptr::null_mut();
    int rc;
    int index;
...
//
    ...
    unfifo:
    chan.read_data_ok = 0;
    safely_assign_in_fifo(chan, core::ptr::null_mut());
    fifo_mem_release(in_fifo);
    kfree(in_fifo);
    if (out_ep) {
    mutex_lock(&chan.lock);
    chan.out_ep = core::ptr::null_mut();
    mutex_unlock(&chan.lock);
    }
    late_unopen:
    if (out_ep)
    ...
    let mut sent_set_push: bool = false;
    int rc;
    ...
    rc = fifo_read(fifo, ( void *)userbuf + bytes_done,
...
#[no_mangle]
unsafe extern "C" fn xillyusb_flush(filp: *mut file, id: fl_owner_t) -> c_int {
...
// The things you do to use dev_warn() and not pr_warn()
    ...
    if (count == 0)
    break;
    rc = fifo_write(fifo, ( void *)userbuf, count,
    ...
    let mut flush_rc: c_int = flush_downstream(chan, 0, true);
    if (flush_rc && !rc)
    rc = flush_rc;
    }
    done:
...
#[no_mangle]
unsafe extern "C" fn xillyusb_release(inode: *mut inode, filp: *mut file) -> c_int {
    static int xillyusb_release(struct inode *inode, struct file *filp)
    {
    struct xillyusb_channel *chan = filp.private_data;
    struct xillyusb_dev *xdev = chan.xdev;
    let mut rc_read: c_int = 0, rc_write = 0;
    if (filp.f_mode & FMODE_READ) {
...
//
#[no_mangle]
unsafe extern "C" fn xillyusb_llseek(filp: *mut file, offset: loff_t, whence: c_int) -> loff_t {
    static loff_t xillyusb_llseek(struct file *filp, loff_t offset, int whence)
    {
    struct xillyusb_channel *chan = filp.private_data;
    struct xillyusb_dev *xdev = chan.xdev;
    let mut pos: loff_t = filp.f_pos;
    let mut rc: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn xillyusb_poll(filp: *mut file, wait: *mut poll_table) -> __poll_t {
...
#[no_mangle]
unsafe extern "C" fn xillyusb_setup_base_eps(xdev: *mut xillyusb_dev) -> c_int {
...
// Verify that device has the two fundamental bulk in/out endpoints
    ...
    let mut in_desc: c_uint = le16_to_cpu(*chandesc++);
    let mut out_desc: c_uint = le16_to_cpu(*chandesc++);
    chan.xdev = xdev;
...
#[no_mangle]
unsafe extern "C" fn xillyusb_discovery(interface: *mut usb_interface) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xillyusb_disconnect(interface: *mut usb_interface) {
...
#[no_mangle]
unsafe extern "C" fn xillyusb_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn xillyusb_exit() -> void __exit {
