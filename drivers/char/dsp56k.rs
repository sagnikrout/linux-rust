...
#[no_mangle]
unsafe extern "C" fn dsp56k_reset() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dsp56k_upload(bin: *mut u_char __user, len: c_int) -> c_int {
    ...
    int i;
    dsp56k_reset();
    pdev = platform_device_register_simple("dsp56k", 0, core::ptr::null_mut(), 0);
    if (IS_ERR(pdev)) {
...
// tx_wait(10);
    ...
    let mut dev: c_int = iminor(inode) & 0x0f;
    switch(dev)
    {
...
// Don't do anything if nothing is to be done
    ...
    unsigned long arg)
    {
    let mut dev: c_int = iminor(file_inode(file)) & 0x0f;
...
#[no_mangle]
pub unsafe extern "C" fn if(0x1: dir &) -> else {
    else if (dir & 0x1)
    dsp56k_host_interface.icr &= ~DSP56K_ICR_HF0;
    if ((dir & 0x2) && (out & 0x2))
    dsp56k_host_interface.icr |= DSP56K_ICR_HF1;
#[no_mangle]
pub unsafe extern "C" fn if(0x2: dir &) -> else {
    else if (dir & 0x2)
    dsp56k_host_interface.icr &= ~DSP56K_ICR_HF1;
    status = 0;
...
#[no_mangle]
unsafe extern "C" fn dsp56k_poll(file: *mut file, wait: *mut poll_table) -> __poll_t {
...
#[no_mangle]
unsafe extern "C" fn dsp56k_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dsp56k_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dsp56k_init_driver() -> int __init {
    ...
    if (err)
    goto out_chrdev;
    device_create(&dsp56k_class, core::ptr::null_mut(), MKDEV(DSP56K_MAJOR, 0), core::ptr::null_mut(),
    "dsp56k");
    printk(banner);
    goto out;
    out_chrdev:
...
#[no_mangle]
unsafe extern "C" fn dsp56k_cleanup_driver() -> void __exit {
