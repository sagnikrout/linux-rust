...
pub const LP_NO: c_int = 8;
    static DEFINE_MUTEX(lp_mutex);
    static struct lp_struct lp_table[LP_NO];
    static int port_num[LP_NO];
    let mut lp_count: static unsigned int = 0;
...
pub const LP_PREEMPT_REQUEST: c_int = 1;
pub const LP_PARPORT_CLAIMED: c_int = 2;
// --- low-level port access -----------------------------------

// Claim the parport or block trying unless we've already claimed it
#[no_mangle]
unsafe extern "C" fn lp_claim_parport_or_block(this_lp: *mut lp_struct) {
...
// Claim the parport or block trying unless we've already claimed it
#[no_mangle]
unsafe extern "C" fn lp_release_parport(this_lp: *mut lp_struct) {
...
#[no_mangle]
unsafe extern "C" fn lp_preempt(handle: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn lp_negotiate(port: *mut parport, mode: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lp_reset(minor: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lp_error(minor: c_int) {
...
#[no_mangle]
unsafe extern "C" fn lp_check_status(minor: c_int) -> c_int {
    static int lp_check_status(int minor)
    {
    let mut error: c_int = 0;
    let mut last: c_uint = lp_table[minor].last_error;
    let mut status: c_uchar = r_str(minor);
    if ((status & LP_PERRORP) && !(LP_F(minor) & LP_CAREFUL))
// No error.
    last = 0;
#[no_mangle]
pub unsafe extern "C" fn if(LP_POUTPA): (status &) -> else {
    if (last != LP_POUTPA) {
    last = LP_POUTPA;
...
#[no_mangle]
unsafe extern "C" fn lp_wait_ready(minor: c_int, nonblock: c_int) -> c_int {
...
// If we're not in compatibility mode, we're ready now!
    ...
    let mut retv: isize = 0;
    ssize_t written;
    let mut copy_size: usize = count;
...
#[no_mangle]
unsafe extern "C" fn lp_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lp_release(inode: *mut inode, file: *mut file) -> c_int {
    ...
    lp_table[minor].lp_buffer = core::ptr::null_mut();
    LP_F(minor) &= ~LP_BUSY;
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn lp_set_timeout(minor: c_uint, tv_sec: i64, tv_usec: c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lp_set_timeout32(minor: c_uint, arg: *mut void __user) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn lp_set_timeout64(minor: c_uint, arg: *mut void __user) -> c_int {
...
pub const CONSOLE_LP: c_int = 0;
// If the printer is out of paper, we can either lose the messages or
// stall until the printer is happy again.  Define CONSOLE_LP_STRICT
// non-zero to get the latter behaviour.
pub const CONSOLE_LP_STRICT: c_int = 1;
...
// --- initialisation code -------------------------------------
    ...
    static bool reset;
    module_param_array(parport, charp, core::ptr::null_mut(), 0);
    module_param(reset, bool, 0);

#[no_mangle]
unsafe extern "C" fn lp_setup(str: *mut c_char) -> int __init {
...
// disable driver on "lp=" or "lp=0"
    ...
    let mut n: c_int = simple_strtoul(str+7, core::ptr::null_mut(), 10);
...
#[no_mangle]
unsafe extern "C" fn lp_register(nr: c_int, port: *mut parport) -> c_int {
    ...
    &ppdev_cb, nr);
    if (lp_table[nr].dev == core::ptr::null_mut())
    return 1;
    lp_table[nr].flags |= LP_EXIST;
    if (reset)
    lp_reset(nr);
    device_create(&lp_class, port.dev, MKDEV(LP_MAJOR, nr), core::ptr::null_mut(),
    "lp%d", nr);
...
#[no_mangle]
unsafe extern "C" fn lp_attach(port: *mut parport) {
...
#[no_mangle]
unsafe extern "C" fn lp_detach(port: *mut parport) {
...
// Write this some day.
    ...
    console_registered = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn lp_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn lp_init_module() -> int __init {
...
// The user gave some parameters.  Let's see what they were.
    ...
    char *ep;
    let mut r: c_ulong = simple_strtoul(parport[n], &ep, 0);
    if (ep != parport[n])
    parport_nr[n] = r;
    else {
...
#[no_mangle]
unsafe extern "C" fn lp_cleanup_module() {
