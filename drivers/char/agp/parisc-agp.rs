...
//! Source: drivers/char/agp/parisc-agp.c
#![no_std]
...
pub const AGP8X_MODE_BIT: c_int = 3;
...
// return native (big-endian) PDIR entry
    ...
    {
    struct pci_dev *fake_bridge_dev = core::ptr::null_mut();
    struct agp_bridge_data *bridge;
    let mut error: c_int = 0;
    fake_bridge_dev = pci_alloc_dev(core::ptr::null_mut());
    if (!fake_bridge_dev) {
    error = -ENOMEM;
    goto fail;
    }
...
// lba = padev;
    ...
    {
    let mut err: c_int = -1;
    struct parisc_device *sba = core::ptr::null_mut(), *lba = core::ptr::null_mut();
    struct lba_device *lbadev = core::ptr::null_mut();
    if (!sba_list)
    goto out;
