...
//! Source: drivers/char/tpm/tpm_tis_i2c_cr50.c
#![no_std]
...
pub const TPM_CR50_MAX_BUFSIZE: c_int = 64;

pub const TPM_CR50_I2C_DID_VID: c_uint = 0x00281ae0L	/* Device and vendor ID for Cr50 H1 */;
pub const TPM_TI50_DT_I2C_DID_VID: c_uint = 0x504a6666L	/* Device and vendor ID for Ti50 DT */;
pub const TPM_TI50_OT_I2C_DID_VID: c_uint = 0x50666666L	/* Device and vendor ID for TI50 OT */;

pub const TPM_CR50_I2C_DEFAULT_LOC: c_int = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tpm_i2c_cr50_priv_data {
    pub irq: c_int,
    pub tpm_ready: completion,
    pub buf: [u8; TPM_CR50_MAX_BUFSIZE],
}
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_int_handler(dummy: c_int, tpm_info: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_wait_tpm_ready(chip: *mut tpm_chip) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_enable_tpm_irq(chip: *mut tpm_chip) {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_disable_tpm_irq(chip: *mut tpm_chip) {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_read(chip: *mut tpm_chip, addr: u8, buffer: *mut u8, len: usize) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_check_locality(chip: *mut tpm_chip, loc: c_int) -> c_int {
    static int tpm_cr50_check_locality(struct tpm_chip *chip, int loc)
    {
    let mut mask: u8 = TPM_ACCESS_VALID | TPM_ACCESS_ACTIVE_LOCALITY;
    u8 buf;
    int rc;
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_release_locality(chip: *mut tpm_chip, loc: c_int) -> c_int {
    static int tpm_cr50_release_locality(struct tpm_chip *chip, int loc)
    {
    struct i2c_client *client = to_i2c_client(chip.dev.parent);
    let mut mask: u8 = TPM_ACCESS_VALID | TPM_ACCESS_REQUEST_PENDING;
    let mut addr: u8 = TPM_I2C_ACCESS(loc);
    u8 buf;
    int rc;
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_request_locality(chip: *mut tpm_chip, loc: c_int) -> c_int {
    static int tpm_cr50_request_locality(struct tpm_chip *chip, int loc)
    {
    struct i2c_client *client = to_i2c_client(chip.dev.parent);
    let mut buf: u8 = TPM_ACCESS_REQUEST_USE;
    unsigned long stop;
    int rc;
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_tis_status(chip: *mut tpm_chip) -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_tis_set_ready(chip: *mut tpm_chip) {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_tis_recv(chip: *mut tpm_chip, buf: *mut u8, buf_len: usize) -> c_int {
    static int tpm_cr50_i2c_tis_recv(struct tpm_chip *chip, u8 *buf, size_t buf_len)
    {
    let mut mask: u8 = TPM_STS_VALID | TPM_STS_DATA_AVAIL;
    size_t burstcnt, cur, len, expected;
    let mut addr: u8 = TPM_I2C_DATA_FIFO(chip.locality);
    u32 status;
    int rc;
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_req_canceled(chip: *mut tpm_chip, status: u8) -> bool {
    static bool tpm_cr50_i2c_req_canceled(struct tpm_chip *chip, u8 status)
    {
    let mut status: return = = TPM_STS_COMMAND_READY;
    }
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_is_firmware_power_managed(dev: *mut device) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_probe(client: *mut i2c_client) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tpm_cr50_i2c_remove(client: *mut i2c_client) {
