...
//! Source: drivers/char/tpm/tpm_tis_spi_main.c
#![no_std]
...
pub const MAX_SPI_FRAMESIZE: c_int = 64;
pub const SPI_HDRSIZE: c_int = 4;
//
...
//
    ...
    if (in) {
    spi_xfer[2].tx_buf = core::ptr::null_mut();
...
// Flow control transfers are receive only
    spi_xfer.tx_buf = core::ptr::null_mut();
    ...
    if (out) {
    spi_xfer.tx_buf = phy.iobuf;
    spi_xfer.rx_buf = core::ptr::null_mut();
...
//
    ...
    {
    return tpm_tis_spi_transfer(data, addr, len, result, core::ptr::null_mut());
    }
    static int tpm_tis_spi_write_bytes(struct tpm_tis_data *data, u32 addr,
    u16 len, const u8 *value, enum tpm_tis_io_mode io_mode)
    {
    return tpm_tis_spi_transfer(data, addr, len, core::ptr::null_mut(), value);
    }
    ...
    return -ENOMEM;
    phy.spi_device = spi;
    return tpm_tis_core_init(&spi.dev, &phy.priv, irq, phy_ops, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn tpm_tis_spi_probe(dev: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tpm_tis_spi_driver_probe(spi: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tpm_tis_spi_remove(dev: *mut spi_device) {
