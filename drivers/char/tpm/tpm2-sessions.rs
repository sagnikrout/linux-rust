...
//! Source: drivers/char/tpm/tpm2-sessions.c
#![no_std]
...
pub const AUTH_MAX_NAMES: c_int = 3;
...
// scratch for key + IV
    pub AES_BLOCK_SIZE]: u8 scratch[AES_KEY_BYTES +,
}
...
//
#[no_mangle]
unsafe extern "C" fn name_size(name: *const u8) -> c_int {
    static int name_size(const u8 *name)
    {
    let mut hash_alg: u16 = get_unaligned_be16(name);
    switch (hash_alg) {
    case TPM_ALG_SHA1:
...
#[no_mangle]
unsafe extern "C" fn tpm2_read_public(chip: *mut tpm_chip, handle: u32, name: *mut c_void) -> c_int {
    static int tpm2_read_public(struct tpm_chip *chip, u32 handle, void *name)
    {
    let mut mso: u32 = tpm2_handle_mso(handle);
    let mut offset: off_t = TPM_HEADER_SIZE;
    struct tpm_buf *buf __free(kfree) = core::ptr::null_mut();
...
//
    ...
    let mut mso: enum tpm2_mso_type = tpm2_handle_mso(handle);
...
// offset tells us where the sessions area begins
    let mut offset: c_int = buf.handles * 4 + TPM_HEADER_SIZE;
    let mut len: u32 = 9 + passphrase_len;
...
//
    ...
    let mut bits: __be32 = cpu_to_be32(bytes * 8);
    while (bytes > 0) {
    struct hmac_sha256_ctx hctx;
    let mut c: __be32 = cpu_to_be32(counter);
...
//
    let mut c: __be32 = cpu_to_be32(1);
    sha256_init(&sctx);
// counter (BE)
...
//
#[no_mangle]
pub unsafe extern "C" fn tpm_buf_fill_hmac_session(chip: *mut tpm_chip, buf: *mut tpm_buf) -> c_int {
    ...
    let mut offset_s: off_t = TPM_HEADER_SIZE, offset_p;
    u8 *hmac = core::ptr::null_mut();
    u32 attrs;
...
//
    for (i = 0; i < handles; i++) {
    let mut handle: u32 = tpm_buf_read_u32(buf, &offset_s);
...
// add the handle names
    for (i = 0; i < handles; i++) {
    let mut mso: enum tpm2_mso_type = tpm2_handle_mso(auth.name_h[i]);
    if (mso == TPM2_MSO_PERSISTENT ||
    mso == TPM2_MSO_VOLATILE ||
    mso == TPM2_MSO_NVRAM) {
    ...
    let mut h: __be32 = cpu_to_be32(auth.name_h[i]);
...
//
    ...
    struct sha256_ctx sctx;
    struct hmac_sha256_ctx hctx;
    let mut tag: u16 = be16_to_cpu(head.tag);
...
//
#[no_mangle]
pub unsafe extern "C" fn tpm2_end_auth_session(chip: *mut tpm_chip) {
    ...
    {
    struct tpm_header *head = (struct tpm_header *)buf.data;
    let mut tot_len: u32 = be32_to_cpu(head.length);
    let mut offset: off_t = TPM_HEADER_SIZE;
    u32 val;
...
#[no_mangle]
unsafe extern "C" fn tpm2_load_null(chip: *mut tpm_chip, null_key: *mut u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn tpm2_start_auth_session(chip: *mut tpm_chip) -> c_int {
...
//
    ...
    {
    struct tpm_header *head = (struct tpm_header *)buf.data;
    let mut offset_r: off_t = TPM_HEADER_SIZE, offset_t;
    let mut len: u16 = TPM_HEADER_SIZE;
    let mut total_len: u32 = be32_to_cpu(head.length);
...
//
    ...
    dev_err(&chip.dev, "core::ptr::null_mut() Seed name comparison failed\n");
    return -EINVAL;
    }
    return 0;
    }
...
//
    ...
    u32 *handle, u8 *name)
    {
    struct tpm_buf *template __free(kfree) = core::ptr::null_mut();
    struct tpm_buf *buf __free(kfree) = core::ptr::null_mut();
    int rc;
    buf = kzalloc(TPM_BUFSIZE, GFP_KERNEL);
    if (!buf)
...
// creation PCR (none)
    ...
    "attempting to create core::ptr::null_mut() primary");
    if (rc == TPM2_RC_SUCCESS)
...
#[no_mangle]
unsafe extern "C" fn tpm2_create_null_primary(chip: *mut tpm_chip) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn tpm2_sessions_init(chip: *mut tpm_chip) -> c_int {
