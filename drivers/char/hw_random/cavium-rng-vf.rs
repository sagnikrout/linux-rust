...
//! Source: drivers/char/hw_random/cavium-rng-vf.c
#![no_std]
...
// PCI device IDs
pub const PCI_DEVID_CAVIUM_RNG_PF: c_uint = 0xA018;
pub const PCI_DEVID_CAVIUM_RNG_VF: c_uint = 0xA033;
pub const HEALTH_STATUS_REG: c_uint = 0x38;
// RST device info
pub const PCI_DEVICE_ID_RST_OTX2: c_uint = 0xA085;
pub const RST_BOOT_REG: c_uint = 0x1600ULL;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct cavium_rng {
    pub ops: hwrng,
    pub result: *mut void __iomem,
    pub pf_regbase: *mut void __iomem,
...
#[no_mangle]
pub unsafe extern "C" fn is_octeontx(pdev: *mut pci_dev) -> bool {
...
#[no_mangle]
unsafe extern "C" fn rng_get_coprocessor_clkrate() -> u64 {
    ...
    PCI_DEVICE_ID_RST_OTX2, core::ptr::null_mut());
    if (!pdev)
    goto error;
...
#[no_mangle]
unsafe extern "C" fn check_rng_health(rng: *mut cavium_rng) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cavium_rng_read(rng: *mut hwrng, dat: *mut c_void, max: usize, wait: bool) -> c_int {
    static int cavium_rng_read(struct hwrng *rng, void *dat, size_t max, bool wait)
    {
    struct cavium_rng *p = container_of(rng, struct cavium_rng, ops);
    let mut size: c_uint = max;
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn cavium_map_pf_regs(rng: *mut cavium_rng) -> c_int {
...
// Health status is not supported on 83xx, skip mapping PF CSRs
    ...
    rng.pf_regbase = core::ptr::null_mut();
    return 0;
    }
    pdev = pci_get_device(PCI_VENDOR_ID_CAVIUM,
    PCI_DEVID_CAVIUM_RNG_PF, core::ptr::null_mut());
    if (!pdev) {
