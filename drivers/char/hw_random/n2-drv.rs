...
//! Source: drivers/char/hw_random/n2-drv.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn n2rng_hv_err_trans(hv_err: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn n2rng_control_settle_v2(np: *mut n2rng, unit: c_int) -> c_ulong {
    static unsigned long n2rng_control_settle_v2(struct n2rng *np, int unit)
    {
    let mut ra: c_ulong = __pa(&np.scratch_control[0]);
...
#[no_mangle]
unsafe extern "C" fn n2rng_generic_read_data(data_ra: c_ulong) -> c_int {
    static int n2rng_generic_read_data(unsigned long data_ra)
    {
    unsigned long ticks, hv_err;
    let mut block: c_int = 0, hcheck = 0;
    while (1) {
...
//
#[no_mangle]
unsafe extern "C" fn n2rng_try_read_ctl(np: *mut n2rng) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn n2rng_control_default(np: *mut n2rng, ctl: c_int) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn n2rng_control_swstate_init(np: *mut n2rng) {
...
#[no_mangle]
unsafe extern "C" fn n2rng_grab_diag_control(np: *mut n2rng) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn n2rng_init_control(np: *mut n2rng) -> c_int {
    static int n2rng_init_control(struct n2rng *np)
    {
    let mut err: c_int = n2rng_grab_diag_control(np);
...
#[no_mangle]
unsafe extern "C" fn n2rng_data_read(rng: *mut hwrng, data: *mut u32) -> c_int {
    static int n2rng_data_read(struct hwrng *rng, u32 *data)
    {
    struct n2rng *np = (struct n2rng *) rng.priv;
    let mut ra: c_ulong = __pa(&np.test_data);
    int len;
...
// data = np->buffer;
    ...
    let mut err: c_int = n2rng_generic_read_data(ra);
    if (!err) {
    np.flags |= N2RNG_FLAG_BUFFER_VALID;
...
//
#[no_mangle]
unsafe extern "C" fn n2rng_guest_check(np: *mut n2rng) -> c_int {
    ...
    {
    let mut post_ctl_ra: c_ulong = __pa(post_control);
    let mut pre_ctl_ra: c_ulong = __pa(pre_control);
    let mut buffer_ra: c_ulong = __pa(buffer);
    int err;
...
#[no_mangle]
unsafe extern "C" fn advance_polynomial(poly: u64, val: u64, count: c_int) -> u64 {
    static u64 advance_polynomial(u64 poly, u64 val, int count)
    {
    int i;
    for (i = 0; i < count; i++) {
    let mut highbit_set: c_int = ((s64)val < 0);
    val <<= 1;
    if (highbit_set)
    val ^= poly;
    }
...
#[no_mangle]
unsafe extern "C" fn n2rng_test_buffer_find(np: *mut n2rng, val: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn n2rng_dump_test_buffer(np: *mut n2rng) {
...
#[no_mangle]
unsafe extern "C" fn n2rng_check_selftest_buffer(np: *mut n2rng, unit: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn n2rng_control_selftest(np: *mut n2rng, unit: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn n2rng_control_check(np: *mut n2rng) -> c_int {
    static int n2rng_control_check(struct n2rng *np)
    {
    int i;
    for (i = 0; i < np.num_units; i++) {
    let mut err: c_int = n2rng_control_selftest(np, i);
    if (err)
    return err;
    }
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn n2rng_control_configure_units(np: *mut n2rng) -> c_int {
    ...
    let mut ctl_ra: c_ulong = __pa(&up.control[0]);
    int esrc;
    u64 base, shift;
    if (np.data.chip_version == 1) {
...
#[no_mangle]
unsafe extern "C" fn n2rng_work(work: *mut work_struct) {
    static void n2rng_work(struct work_struct *work)
    {
    struct n2rng *np = container_of(work, struct n2rng, work.work);
    let mut err: c_int = 0;
    let mut retries: static int = 4;
...
#[no_mangle]
pub unsafe extern "C" fn if(N2RNG_FLAG_SHUTDOWN): err && !(np->flags &) -> else {
    else if (err && !(np.flags & N2RNG_FLAG_SHUTDOWN))
    schedule_delayed_work(&np.work, HZ * 2);
    }
#[no_mangle]
unsafe extern "C" fn n2rng_driver_version() {
...
#[no_mangle]
unsafe extern "C" fn n2rng_probe(op: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn n2rng_remove(op: *mut platform_device) {
