...
//! Source: drivers/iio/common/cros_ec_sensors/cros_ec_sensors_core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cros_ec_sensors_core_clean(arg: *mut c_void) {
    ...
    struct cros_ec_sensors_core_state *st = iio_priv(indio_dev);
    let mut sensor_num: u8 = st.param.info.sensor_num;
...
#[no_mangle]
pub unsafe extern "C" fn if(MOTIONSENSE_LOC_LID: loc ==) -> else {
    else if (loc == MOTIONSENSE_LOC_LID)
    indio_dev.label = "accel-display";
#[no_mangle]
pub unsafe extern "C" fn if(MOTIONSENSE_LOC_CAMERA: loc ==) -> else {
...
//
    ret = devm_iio_kfifo_buffer_setup_ext(dev, indio_dev, core::ptr::null_mut(),
...
//
    ...
    core::ptr::null_mut(), trigger_capture, core::ptr::null_mut());
    if (ret)
    return ret;
    }
    }
...
//
    ...
    {
    __le16 tmp;
    let mut ret: c_int = ec.cmd_readmem(ec, offset, 2, &tmp);
    if (ret >= 0)
...
//
    ...
    struct cros_ec_sensors_core_state *st = iio_priv(indio_dev);
    struct cros_ec_device *ec = st.ec;
    let mut samp_id: u8 = 0xff, status = 0;
    int ret, attempts = 0;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn cros_ec_sensors_capture(irq: c_int, p: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn cros_ec_sensors_resume(dev: *mut device) -> int __maybe_unused {
    ...
    SIMPLE_DEV_PM_OPS(cros_ec_sensors_pm_ops, core::ptr::null_mut(), cros_ec_sensors_resume);
