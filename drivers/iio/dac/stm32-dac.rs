...
//! Source: drivers/iio/dac/stm32-dac.c
#![no_std]
...
pub const STM32_DAC_CHANNEL_1: c_int = 1;
pub const STM32_DAC_CHANNEL_2: c_int = 2;

pub const STM32_DAC_AUTO_SUSPEND_DELAY_MS: c_int = 2000;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dac {
    pub common: *mut stm32_dac_common,
    pub lock: mutex,
}

#[no_mangle]
unsafe extern "C" fn stm32_dac_is_enabled(indio_dev: *mut iio_dev, channel: c_int) -> c_int {
    ...
    let mut msk: u32 = STM32_DAC_IS_CHAN_1(ch) ? STM32_DAC_CR_EN1 : STM32_DAC_CR_EN2;
    let mut en: u32 = enable ? msk : 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn stm32_dac_get_value(dac: *mut stm32_dac, channel: c_int, val: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dac_set_value(dac: *mut stm32_dac, channel: c_int, val: c_int) -> c_int {
...
// val2 = chan->scan_type.realbits;
    ...
    char *buf)
    {
    let mut ret: c_int = stm32_dac_is_enabled(indio_dev, chan.channel);
    if (ret < 0)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn stm32_dac_chan_of_init(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dac_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dac_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dac_suspend(dev: *mut device) -> c_int {
    static int stm32_dac_suspend(struct device *dev)
    {
    struct iio_dev *indio_dev = dev_get_drvdata(dev);
    let mut channel: c_int = indio_dev.channels[0].channel;
    int ret;
