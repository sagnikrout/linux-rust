...
//! Source: drivers/iio/pressure/ms5611_core.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ms5611_prom_is_valid(prom: *mut u16, len: usize) -> bool {
    static bool ms5611_prom_is_valid(u16 *prom, size_t len)
    {
    int i, j;
    let mut crc: u16 = 0, crc_orig = prom[7] & 0x000F;
    prom[7] &= 0xFF00;
    for (i = 0; i < len * 2; i++) {
    if (i % 2 == 1)
    crc ^= prom[i >> 1] & 0x00FF;
    ...
    crc = (crc >> 12) & 0x000F;
    let mut crc: return = = crc_orig;
    }
#[no_mangle]
unsafe extern "C" fn ms5611_read_prom(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ms5611_reset(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ms5611_trigger_handler(irq: c_int, p: *mut c_void) -> irqreturn_t {
...
// val = (int)st->pressure_osr->rate;
    ...
    {
    struct ms5611_state *st = iio_priv(indio_dev);
    const struct ms5611_osr *osr = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ms5611_init(indio_dev: *mut iio_dev) -> c_int {
...
// Enable attached regulator if any.
    ...
    ms5611_trigger_handler, core::ptr::null_mut());
    if (ret < 0) {
