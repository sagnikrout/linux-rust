...
//! Source: drivers/iio/potentiometer/ds1803.c
#![no_std]
...
pub const DS1803_WIPER_0: c_uint = 0xA9;
pub const DS1803_WIPER_1: c_uint = 0xAA;
pub const DS3502_WR_IVR: c_uint = 0x00;
    enum ds1803_type {
    DS1803_010,
    DS1803_050,
    DS1803_100,
...
pub struct ds1803_cfg {
    ...
    pub val): *const *const iio_chan_spec chan, int,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ds1803_data {
    pub client: *mut i2c_client,
    pub cfg: *const ds1803_cfg,
}
...
// val2 = data->cfg->avail[2]; /* Max wiper position
    ...
    {
    struct ds1803_data *data = iio_priv(indio_dev);
    let mut addr: u8 = chan.address;
    let mut max_pos: c_int = data.cfg.avail[2];
    if (val2 != 0)
    return -EINVAL;
    switch (mask) {
    case IIO_CHAN_INFO_RAW:
...
#[no_mangle]
unsafe extern "C" fn ds1803_probe(client: *mut i2c_client) -> c_int {
