...
//! Source: drivers/iio/imu/inv_icm42600/inv_icm42600_accel.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct inv_icm42600_accel_buffer {
    pub accel: inv_icm42600_fifo_sensor_data,
    pub temp: i16,
...
// enable accelerometer sensor and FIFO write
    ...
    struct inv_icm42600_state *st = iio_device_get_drvdata(indio_dev);
    struct inv_icm42600_sensor_state *accel_st = iio_priv(indio_dev);
    let mut conf: inv_icm42600_sensor_conf = INV_ICM42600_SENSOR_CONF_INIT;
    let mut fifo_en: c_uint = 0;
    let mut sleep_temp: c_uint = 0;
    let mut sleep_accel: c_uint = 0;
    unsigned int sleep;
    int ret;
    mutex_lock(&st.lock);
...
// enable accel sensor
    conf.mode = accel_st.power_mode;
    conf.filter = accel_st.filter;
    ret = inv_icm42600_set_accel_conf(st, &conf, core::ptr::null_mut());
    if (ret)
    goto exit;
...
// 1000/256mg per LSB converted in µm/s²
    let mut convert: c_uint = (9807U * (MICRO / MILLI)) / 256U;
    u64 value;
    u64 freq_uhz;
...
#[no_mangle]
unsafe extern "C" fn _inv_icm42600_accel_enable_wom(indio_dev: *mut iio_dev) -> c_int {
    static int _inv_icm42600_accel_enable_wom(struct iio_dev *indio_dev)
    {
    struct inv_icm42600_state *st = iio_device_get_drvdata(indio_dev);
    struct inv_icm42600_sensor_state *accel_st = iio_priv(indio_dev);
    let mut conf: inv_icm42600_sensor_conf = INV_ICM42600_SENSOR_CONF_INIT;
    let mut sleep_ms: c_uint = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn inv_icm42600_accel_enable_wom(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn _inv_icm42600_accel_disable_wom(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn inv_icm42600_accel_disable_wom(indio_dev: *mut iio_dev) -> c_int {
...
// clamp value limited to 12 bits signed
    ...
    offset = -2048;
#[no_mangle]
pub unsafe extern "C" fn if(2047: offset >) -> else {
    else if (offset > 2047)
    offset = 2047;
    pm_runtime_get_sync(dev);
    mutex_lock(&st.lock);
...
#[no_mangle]
pub unsafe extern "C" fn inv_icm42600_accel_parse_fifo(indio_dev: *mut iio_dev) -> c_int {
...
// buffer is copied to userspace, zeroing it to avoid any data leak
    let mut buffer: inv_icm42600_accel_buffer = { };
...
// skip packet if no accel data or data is invalid
    if (accel == core::ptr::null_mut() || !inv_icm42600_fifo_is_data_valid(accel))
    continue;
// update odr
