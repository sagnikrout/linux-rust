...
//! Source: drivers/iio/imu/inv_icm45600/inv_icm45600_core.c
#![no_std]
...
//
    ...
    {
    let mut reg: c_uint = be16_to_cpup(reg_buf);
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_write(context: *mut c_void, data: *const c_void, count: usize) -> c_int {
    static int inv_icm45600_write(void *context, const void *data, size_t count)
    {
    const u8 *d = data;
    let mut reg: c_uint = be16_to_cpup(data);
    struct regmap *map = context;
    if (FIELD_GET(INV_ICM45600_REG_BANK_MASK, reg))
...
#[no_mangle]
pub unsafe extern "C" fn inv_icm45600_odr_to_period(odr: enum inv_icm45600_odr) -> u32 {
...
// 3 first values are reserved, left to 0
    ...
    {
    let mut oldgyro: enum inv_icm45600_sensor_mode = st.conf.gyro.mode;
    let mut oldaccel: enum inv_icm45600_sensor_mode = st.conf.accel.mode;
...
#[no_mangle]
pub unsafe extern "C" fn if(INV_ICM45600_SENSOR_MODE_OFF: gyro ==) -> else {
...
// sleep_ms = sleepval;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: sleepval) -> else {
    else if (sleepval)
    msleep(sleepval);
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_irq_timestamp(irq: c_int, _data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_irq_handler(irq: c_int, _data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_timestamp_setup(st: *mut inv_icm45600_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_enable_regulator_vddio(st: *mut inv_icm45600_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_disable_vddio_reg(_data: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn inv_icm45600_suspend(dev: *mut device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn inv_icm45600_resume(dev: *mut device) -> c_int {
...
// Restore sensors state.
    ...
    st.suspended.accel, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_runtime_suspend(dev: *mut device) -> c_int {
...
// disable all sensors
    ...
    INV_ICM45600_SENSOR_MODE_OFF, core::ptr::null_mut());
    if (ret)
    return ret;
    regulator_disable(st.vddio_supply);
    return 0;
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_runtime_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn _inv_icm45600_temp_read(st: *mut inv_icm45600_state, temp: *mut i16) -> c_int {
    static int _inv_icm45600_temp_read(struct inv_icm45600_state *st, s16 *temp)
    {
    let mut conf: inv_icm45600_sensor_conf = INV_ICM45600_SENSOR_CONF_KEEP_VALUES;
    int ret;
// Make sure a sensor is on.
    if (st.conf.gyro.mode == INV_ICM45600_SENSOR_MODE_OFF &&
    st.conf.accel.mode == INV_ICM45600_SENSOR_MODE_OFF) {
    conf.mode = INV_ICM45600_SENSOR_MODE_LOW_POWER;
    ret = inv_icm45600_set_accel_conf(st, &conf, core::ptr::null_mut());
    if (ret)
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn inv_icm45600_temp_read(st: *mut inv_icm45600_state, temp: *mut i16) -> c_int {
...
// val = 3200;
    ...
    inv_icm45600_runtime_resume, core::ptr::null_mut())
    };
