...
//! Source: drivers/iio/imu/bno055/bno055_ser_core.c
#![no_std]
...
//
pub const BNO055_SER_XFER_BURST_BREAK_THRESHOLD: c_int = 22;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bno055_ser_priv {
    enum {
    CMD_NONE,
    CMD_READ,
    CMD_WRITE,
    pub expect_response: },
    pub expected_data_len: c_int,
    pub response_buf: *mut u8,
//
...
// Only accessed in serdev RX callback context
    ...
    RX_DATA,
    pub state: },
    pub databuf_count: c_int,
    pub expected_len: c_int,
    pub type: c_int,
    pub rx: },
...
#[no_mangle]
unsafe extern "C" fn bno055_ser_send_chunk(priv: *mut bno055_ser_priv, data: *const u8, len: c_int) -> c_int {
...
// waiting more than 30mS should clear the BNO055 internal state
    ...
    bool read, int addr, int len, const u8 *data)
    {
    let mut retry_max: c_int = 5;
    let mut retry: c_int = retry_max;
    let mut ret: c_int = 0;
//
...
#[no_mangle]
unsafe extern "C" fn bno055_ser_write_reg(context: *mut c_void, _data: *const c_void, count: usize) -> c_int {
    ...
    mutex_unlock(&priv.lock);
    ret = bno055_ser_send_cmd(priv, 1, reg_addr, val_size, core::ptr::null_mut());
    mutex_lock(&priv.lock);
    priv.response_buf = core::ptr::null_mut();
    mutex_unlock(&priv.lock);
    return ret;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn bno055_ser_handle_rx(priv: *mut bno055_ser_priv, status: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bno055_ser_probe(serdev: *mut serdev_device) -> c_int {
