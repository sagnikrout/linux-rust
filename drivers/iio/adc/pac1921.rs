...
//! Source: drivers/iio/adc/pac1921.c
#![no_std]
...
// pac1921 registers
pub const PAC1921_REG_GAIN_CFG: c_uint = 0x00;
pub const PAC1921_REG_INT_CFG: c_uint = 0x01;
pub const PAC1921_REG_CONTROL: c_uint = 0x02;
pub const PAC1921_REG_VBUS: c_uint = 0x10;
pub const PAC1921_REG_VSENSE: c_uint = 0x12;
pub const PAC1921_REG_OVERFLOW_STS: c_uint = 0x1C;
pub const PAC1921_REG_VPOWER: c_uint = 0x1D;
...
pub const PAC1921_RES_RESOLUTION: c_int = 1023;
// pac1921 overflow status bits

// pac1921 constants
pub const PAC1921_MAX_VSENSE_MV: c_int = 100;
pub const PAC1921_MAX_VBUS_V: c_int = 32;
// Time to first communication after power up (tINT_T)
pub const PAC1921_POWERUP_TIME_MS: c_int = 20;
// Time from Sleep State to Start of Integration Period (tSLEEP_TO_INT)
pub const PAC1921_SLEEP_TO_INT_TIME_US: c_int = 86;
// pac1921 defaults

pub const PAC1921_ACPI_GET_uOHMS_VALS: c_int = 0;
pub const PAC1921_ACPI_GET_LABEL: c_int = 1;
...
pub const PAC1921_NUM_MEAS_CHANS: c_int = 4;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pac1921_priv {
...
//
    pub lock: mutex,
    pub /: *mut *mut u32 rshunt_uohm; / uOhm,
    pub dv_gain: u8,
    pub di_gain: u8,
    pub n_samples: u8,
    pub prev_ovf_flags: u8,
    pub ovf_enabled_events: u8,
    pub first_integr_started: bool,
    pub first_integr_done: bool,
    pub integr_started_time_jiffies: c_ulong,
    pub integr_period_usecs: c_uint,
    pub current_scales: [c_int; ARRAY_SIZE(pac1921_vsense_scales)][2],
    struct {
    pub chan: [u16; PAC1921_NUM_MEAS_CHANS],
...
//
#[no_mangle]
unsafe extern "C" fn pac1921_data_ready(priv: *mut pac1921_priv) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn pac1921_calc_current_scales(priv: *mut pac1921_priv) {
    static void pac1921_calc_current_scales(struct pac1921_priv *priv)
    {
    for (unsigned int i = 0; i < ARRAY_SIZE(priv.current_scales); i++) {
    let mut max: c_int = (PAC1921_MAX_VSENSE_MV * MICRO) >> i;
    let mut vsense_lsb: c_int = DIV_ROUND_CLOSEST(max, PAC1921_RES_RESOLUTION);
...
//
#[no_mangle]
unsafe extern "C" fn pac1921_check_push_overflow(indio_dev: *mut iio_dev, timestamp: i64) -> c_int {
...
//
    ...
    u16 *val)
    {
    let mut ret: c_int = regmap_bulk_read(priv.regmap, reg, val, sizeof(*val));
    if (ret)
    return ret;
...
// Convert current_scale from INT_PLUS_NANO to INT
    let mut tmp: i64 = curr_scale[0] * (s64)NANO + curr_scale[1];
...
//
#[no_mangle]
unsafe extern "C" fn pac1921_lookup_int_num_samples(num_samples: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pac1921_trigger_handler(irq: c_int, p: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn pac1921_init(priv: *mut pac1921_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pac1921_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pac1921_resume(dev: *mut device) -> c_int {
    ...
    pac1921_resume);
#[no_mangle]
unsafe extern "C" fn pac1921_regulator_disable(data: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn pac1921_match_acpi_device(indio_dev: *mut iio_dev) -> c_int {
    ...
    PAC1921_ACPI_GET_uOHMS_VALS, core::ptr::null_mut());
    if (!status)
    ...
    ACPI_FREE(status);
    status = acpi_evaluate_dsm(handle, &pac1921_guid, 1,
    PAC1921_ACPI_GET_LABEL, core::ptr::null_mut());
    if (!status)
    return dev_err_probe(dev, -EINVAL,
...
#[no_mangle]
unsafe extern "C" fn pac1921_parse_of_fw(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pac1921_probe(client: *mut i2c_client) -> c_int {
    ...
    &pac1921_trigger_handler, core::ptr::null_mut());
    if (ret)
    return dev_err_probe(dev, ret,
