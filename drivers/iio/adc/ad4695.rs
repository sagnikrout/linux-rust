...
//! Source: drivers/iio/adc/ad4695.c
#![no_std]
...
// AD4695 registers
pub const AD4695_REG_SPI_CONFIG_A: c_uint = 0x0000;

pub const AD4695_REG_SPI_CONFIG_B: c_uint = 0x0001;

pub const AD4695_REG_DEVICE_TYPE: c_uint = 0x0003;
pub const AD4695_REG_SCRATCH_PAD: c_uint = 0x000A;
pub const AD4695_REG_VENDOR_L: c_uint = 0x000C;
pub const AD4695_REG_VENDOR_H: c_uint = 0x000D;
pub const AD4695_REG_LOOP_MODE: c_uint = 0x000E;
pub const AD4695_REG_SPI_CONFIG_C: c_uint = 0x0010;

pub const AD4695_REG_SPI_STATUS: c_uint = 0x0011;
pub const AD4695_REG_STATUS: c_uint = 0x0014;
pub const AD4695_REG_ALERT_STATUS1: c_uint = 0x0015;
pub const AD4695_REG_ALERT_STATUS2: c_uint = 0x0016;
pub const AD4695_REG_CLAMP_STATUS: c_uint = 0x001A;
pub const AD4695_REG_SETUP: c_uint = 0x0020;

pub const AD4695_REG_REF_CTRL: c_uint = 0x0021;

pub const AD4695_REG_SEQ_CTRL: c_uint = 0x0022;

pub const AD4695_REG_AC_CTRL: c_uint = 0x0023;
pub const AD4695_REG_STD_SEQ_CONFIG: c_uint = 0x0024;
pub const AD4695_REG_GPIO_CTRL: c_uint = 0x0026;
pub const AD4695_REG_GP_MODE: c_uint = 0x0027;

pub const AD4695_REG_TEMP_CTRL: c_uint = 0x0029;

// Conversion mode commands
pub const AD4695_CMD_EXIT_CNV_MODE: c_uint = 0x0A;
pub const AD4695_CMD_TEMP_CHAN: c_uint = 0x0F;

// timing specs
pub const AD4695_T_CONVERT_NS: c_int = 415;
pub const AD4695_T_WAKEUP_HW_MS: c_int = 3;
pub const AD4695_T_WAKEUP_SW_MS: c_int = 3;
pub const AD4695_T_REFBUF_MS: c_int = 100;
pub const AD4695_T_REGCONFIG_NS: c_int = 20;
pub const AD4695_T_SCK_CNV_DELAY_NS: c_int = 80;
pub const AD4695_T_CNVL_NS: c_int = 80;
pub const AD4695_T_CNVH_NS: c_int = 10;

// Max number of voltage input channels.
pub const AD4695_MAX_VIN_CHANNELS: c_int = 16;
    enum ad4695_in_pair {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ad4695_chip_info {
    pub name: *const c_char,
    pub max_sample_rate: c_int,
    pub t_acq_ns: u32,
    pub num_voltage_inputs: u8,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ad4695_channel_config {
    pub channel: c_uint,
    pub highz_en: bool,
    pub bipolar: bool,
    pub pin_pairing: enum ad4695_in_pair,
    pub common_mode_mv: c_uint,
    pub oversampling_ratio: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ad4695_state {
...
// currently PWM CNV only supported with SPI offload use
    pub cnv_pwm: *mut pwm_device,
// protects against concurrent use of cnv_pwm
    pub cnv_pwm_lock: mutex,
// offload also requires separate gpio to manually control CNV
    pub cnv_gpio: *mut gpio_desc,
// voltages channels plus temperature and timestamp
    pub 2]: iio_chan_spec iio_chan[AD4695_MAX_VIN_CHANNELS +,
    pub channels_cfg: [ad4695_channel_config; AD4695_MAX_VIN_CHANNELS],
    pub chip_info: *const ad4695_chip_info,
    pub sample_freq_range: [c_int; 3],
// Reference voltage.
    pub vref_mv: c_uint,
...
// CNV rising edges.
//
    pub 3]: *mut *mut spi_transfer buf_read_xfer[AD4695_MAX_VIN_CHANNELS  2 +,
    pub buf_read_msg: spi_message,
// Raw conversion data received.
    pub 1): IIO_DECLARE_DMA_BUFFER_WITH_TS(u16, buf, AD4695_MAX_VIN_CHANNELS +,
    pub raw_data: u16,
// Commands to send for single conversion.
    pub cnv_cmd: u16,
    pub cnv_cmd2: u8,
// Buffer for storing data from regmap bus reads/writes
    pub regmap_bus_data: [u8; 4],
}
...
#[no_mangle]
unsafe extern "C" fn ad4695_cnv_manual_trigger(st: *mut ad4695_state) {
...
//
#[no_mangle]
unsafe extern "C" fn ad4695_enter_advanced_sequencer_mode(st: *mut ad4695_state, n: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ad4695_exit_conversion_mode(st: *mut ad4695_state) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ad4695_set_ref_voltage(st: *mut ad4695_state, vref_mv: c_int) -> c_int {
    ...
    val = 0;
#[no_mangle]
pub unsafe extern "C" fn if(3250: vref_mv > 2750 && vref_mv <=) -> else {
    else if (vref_mv > 2750 && vref_mv <= 3250)
    val = 1;
#[no_mangle]
pub unsafe extern "C" fn if(3750: vref_mv > 3250 && vref_mv <=) -> else {
    else if (vref_mv > 3250 && vref_mv <= 3750)
    val = 2;
#[no_mangle]
pub unsafe extern "C" fn if(4500: vref_mv > 3750 && vref_mv <=) -> else {
    else if (vref_mv > 3750 && vref_mv <= 4500)
    val = 3;
#[no_mangle]
pub unsafe extern "C" fn if(5100: vref_mv > 4500 && vref_mv <=) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn ad4695_osr_to_regval(ratio: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ad4695_buffer_preenable(indio_dev: *mut iio_dev) -> c_int {
    static int ad4695_buffer_preenable(struct iio_dev *indio_dev)
    {
    struct ad4695_state *st = iio_priv(indio_dev);
    struct spi_transfer *xfer;
    let mut temp_chan_bit: u8 = st.chip_info.num_voltage_inputs;
    u32 bit, num_xfer, num_slots;
    let mut temp_en: u32 = 0;
...
#[no_mangle]
unsafe extern "C" fn ad4695_buffer_postdisable(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ad4695_trigger_handler(irq: c_int, p: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ad4695_offload_buffer_postenable(indio_dev: *mut iio_dev) -> c_int {
    ...
    let mut temp_chan_bit: u8 = st.chip_info.num_voltage_inputs;
    let mut num_slots: u8 = 0;
    let mut temp_en: u8 = 0;
    unsigned int bit;
    int ret;
    iio_for_each_active_channel(indio_dev, bit) {
    if (bit == temp_chan_bit) {
...
#[no_mangle]
unsafe extern "C" fn ad4695_offload_buffer_predisable(indio_dev: *mut iio_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ad4695_read_one_sample(st: *mut ad4695_state, address: c_uint) -> c_int {
...
// Then read the result and exit conversion mode.
    ...
    int *val)
    {
    let mut realbits: u8 = chan.scan_type.realbits;
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(AD4695_IN_PAIR_EVEN_ODD: cfg->pin_pairing ==) -> else {
...
// val = DIV_ROUND_UP_ULL(NSEC_PER_SEC, state.period * osr);
    ...
    int val)
    {
    let mut osr: c_int = ad4695_osr_to_regval(val);
    if (osr < 0)
    return osr;
    switch (chan.type) {
    case IIO_VOLTAGE:
...
#[no_mangle]
unsafe extern "C" fn ad4695_get_calibbias(val: c_int, val2: c_int, osr: c_int) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: val2 <) -> else {
    else if (val2 < 0)
...
// type = IIO_VAL_INT;
    ...
    {
    struct ad4695_state *st = iio_priv(indio_dev);
    let mut osr: c_uint = st.channels_cfg[chan.scan_index].oversampling_ratio;
    switch (osr) {
    case 1:
...
#[no_mangle]
unsafe extern "C" fn ad4695_parse_channel_cfg(st: *mut ad4695_state) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(AD4695_COMMON_MODE_COM: val ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn ad4695_pwm_disable(pwm: *mut c_void) {
    ...
    if (ret)
    return ret;
    st.cnv_pwm = devm_pwm_get(dev, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn ad4695_probe(spi: *mut spi_device) -> c_int {
