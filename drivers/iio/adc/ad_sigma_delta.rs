...
//! Source: drivers/iio/adc/ad_sigma_delta.c
#![no_std]
...
pub const AD_SD_COMM_CHAN_MASK: c_uint = 0x3;
pub const AD_SD_REG_COMM: c_uint = 0x00;
pub const AD_SD_REG_STATUS: c_uint = 0x00;
pub const AD_SD_REG_DATA: c_uint = 0x03;
pub const AD_SD_REG_STATUS_RDY: c_uint = 0x80;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ad_sd_set_comm(sigma_delta: *mut ad_sigma_delta, comm: u8) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ad_sd_reset(sigma_delta: *mut ad_sigma_delta) -> c_int {
    int ad_sd_reset(struct ad_sigma_delta *sigma_delta)
    {
    let mut reset_length: c_uint = sigma_delta.info.num_resetclks;
    unsigned int size;
    u8 *buf;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn ad_sd_disable_irq(sigma_delta: *mut ad_sigma_delta) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ad_sd_enable_irq(sigma_delta: *mut ad_sigma_delta) {
...
pub const AD_SD_CLEAR_DATA_BUFLEN: c_int = 9;
// Called with `sigma_delta->bus_locked == true` only.
#[no_mangle]
unsafe extern "C" fn ad_sigma_delta_clear_pending_event(sigma_delta: *mut ad_sigma_delta) -> c_int {
    static int ad_sigma_delta_clear_pending_event(struct ad_sigma_delta *sigma_delta)
    {
    bool pending_event;
    let mut data_read_len: c_uint = BITS_TO_BYTES(sigma_delta.info.num_resetclks);
    u8 *data;
    struct spi_transfer t[] = {
    {
    .len = 1,
...
//
    ...
    spi_message_init(&m);
    if (sigma_delta.info.has_registers) {
    let mut data_reg: c_uint = sigma_delta.info.data_reg ?: AD_SD_REG_DATA;
...
#[no_mangle]
unsafe extern "C" fn ad_sd_buffer_postenable(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ad_sd_buffer_predisable(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ad_sd_trigger_handler(irq: c_int, p: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ad_sd_validate_scan_mask(indio_dev: *mut iio_dev, mask: *const c_ulong) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ad_sd_data_rdy_trig_poll(irq: c_int, private: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
pub unsafe extern "C" fn ad_sd_validate_trigger(indio_dev: *mut iio_dev, trig: *mut iio_trigger) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn devm_ad_sd_probe_trigger(dev: *mut device, indio_dev: *mut iio_dev) -> c_int {
    static int devm_ad_sd_probe_trigger(struct device *dev, struct iio_dev *indio_dev)
    {
    struct ad_sigma_delta *sigma_delta = iio_device_get_drvdata(indio_dev);
    let mut irq_flags: c_ulong = irq_get_trigger_type(sigma_delta.irq_line);
    int ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn devm_ad_sd_setup_buffer_and_trigger(dev: *mut device, indio_dev: *mut iio_dev) -> c_int {
