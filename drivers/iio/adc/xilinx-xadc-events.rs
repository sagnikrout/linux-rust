...
//! Source: drivers/iio/adc/xilinx-xadc-events.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn xadc_handle_event(indio_dev: *mut iio_dev, event: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn xadc_handle_events(indio_dev: *mut iio_dev, events: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn xadc_get_alarm_mask(chan: *const iio_chan_spec) -> c_uint {
...
// We will never get here
    ...
    {
    let mut alarm: c_uint = xadc_get_alarm_mask(chan);
    struct xadc *xadc = iio_priv(indio_dev);
    ...
    int *val, int *val2)
    {
    let mut offset: c_uint = xadc_get_threshold_offset(chan, dir);
    struct xadc *xadc = iio_priv(indio_dev);
    switch (info) {
