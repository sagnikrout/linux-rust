...
//! Source: drivers/iio/adc/ti-ads112c14.c
#![no_std]
...
pub const ADS112C14_MAX_MEASUREMENT_CHANNELS: c_int = 16;
// Datasheet t_d(RST) - time to wait after reset before next I2C use.
pub const ADS112C14_DELAY_RESET_US: c_int = 500;
pub const ADS112C14_CMD_RDATA: c_uint = 0x00;
pub const ADS112C14_CMD_RREG: c_uint = 0x40;
pub const ADS112C14_CMD_WREG: c_uint = 0x80;
pub const ADS112C14_REG_DEVICE_ID: c_uint = 0x00;

pub const ADS112C14_REG_REVISION_ID: c_uint = 0x01;
pub const ADS112C14_REG_STATUS_MSB: c_uint = 0x02;

pub const ADS112C14_REG_STATUS_LSB: c_uint = 0x03;

pub const ADS112C14_REG_CONVERSION_CTRL: c_uint = 0x04;

pub const ADS112C14_REG_DEVICE_CFG: c_uint = 0x05;

pub const ADS112C14_DEVICE_CFG_CONV_MODE_CONTINUOUS: c_int = 0;
pub const ADS112C14_DEVICE_CFG_CONV_MODE_SINGLE_SHOT: c_int = 1;

pub const ADS112C14_REG_DATA_RATE_CFG: c_uint = 0x06;

pub const ADS112C14_REG_MUX_CFG: c_uint = 0x07;

pub const ADS112C14_MUX_CFG_AIN_GND: c_int = 8;
pub const ADS112C14_REG_GAIN_CFG: c_uint = 0x08;

pub const ADS112C14_REG_REFERENCE_CFG: c_uint = 0x09;

pub const ADS112C14_REFERENCE_CFG_REF_VAL_1_25V: c_int = 0;
pub const ADS112C14_REFERENCE_CFG_REF_VAL_2_5V: c_int = 1;

pub const ADS112C14_REFERENCE_CFG_REF_SEL_INTERNAL: c_int = 0;
pub const ADS112C14_REFERENCE_CFG_REF_SEL_EXTERNAL: c_int = 1;
pub const ADS112C14_REFERENCE_CFG_REF_SEL_AVDD: c_int = 2;
pub const ADS112C14_REG_DIGITAL_CFG: c_uint = 0x0A;

pub const ADS112C14_REG_GPIO_CFG: c_uint = 0x0B;

pub const ADS112C14_REG_GPIO_DATA_OUTPUT: c_uint = 0x0C;

pub const ADS112C14_REG_IDAC_MAG_CFG: c_uint = 0x0D;

pub const ADS112C14_REG_IDAC_MUX_CFG: c_uint = 0x0E;

pub const ADS112C14_REG_REG_MAP_CRC: c_uint = 0x0F;
pub const ADS112C14_INT_REF0_mV: c_int = 1250;
pub const ADS112C14_INT_REF1_mV: c_int = 2500;
    enum {
...
pub const ADS112C14_I2C_CRC8_POLYNOMIAL: c_uint = 0x07;
    DECLARE_CRC8_TABLE(ads112c14_crc8_table);
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ads112c14_chip_info {
...
pub const ADS112C14_SYS_MON_CHANNEL_BASE: c_int = 100;
    enum {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ads112c14_measurement {
    ...
    pub iadc_count: u8,
    pub gain_val: u8,
    pub global_chop: bool,
    pub bipolar: bool,
    pub scale_available: [c_int; ARRAY_SIZE(ads112c14_pga_gains_x10)][2],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ads112c14_data {
    pub chip_info: *const ads112c14_chip_info,
...
// Synchronizes access to register value fields.
    ...
    pub avdd_uV: u32,
    pub ext_ref_uV: u32,
    pub refp_is_avdd: bool,
    pub refn_is_gnd: bool,
    pub ext_ref_ohms: u32,
    pub measurements: *mut ads112c14_measurement,
    pub num_measurements: u32,
    pub sys_mon_chan_short_gain_val: u8,
    pub sys_mon_chan_short_scale_available: [c_int; ARRAY_SIZE(ads112c14_pga_gains_x10)][2],
    IIO_DECLARE_BUFFER_WITH_TS(__be32, scan, ADS112C14_MAX_MEASUREMENT_CHANNELS +
}

#[no_mangle]
unsafe extern "C" fn ads112c14_writeable_reg(dev: *mut device, reg: c_uint) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ads112c14_volatile_reg(dev: *mut device, reg: c_uint) -> bool {
...
//
    ...
    return ret;
    if (use_crc) {
    let mut crc: u8 = crc8(ads112c14_crc8_table, rx_buf, len, CRC8_INIT_VALUE);
    if (crc != rx_buf[len])
    return -EBADMSG;
    }
...
//
    ...
    struct ads112c14_measurement *measurement = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ads112c14_trigger_handler(irq: c_int, private: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ads112c14_populate_idac_mag(current_nA: u32, idac_mag: *mut u8) -> c_int {
    static int ads112c14_populate_idac_mag(u32 current_nA, u8 *idac_mag)
    {
    let mut current_uA: u32 = current_nA / (NANO / MICRO);
// Convert microamps to IMAG bits
    if (current_uA == 1)
// idac_mag = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: in_range(current_uA, _arg: 10, 0: 100) && current_uA % 10 ==) -> else {
    else if (in_range(current_uA, 10, 100) && current_uA % 10 == 0)
// idac_mag = current_uA / 10 + 1;
    else
    return dev_err_probe(core::ptr::null_mut(), -EINVAL,
...
// Update the template that was already copied with dynamic values.
    ...
    {
    for (u32 i = 0; i < ARRAY_SIZE(ads112c14_pga_gains_x10); i++) {
    let mut gain_x10: u64 = ads112c14_pga_gains_x10[i];
    s64 scale;
...
#[no_mangle]
unsafe extern "C" fn ads112c14_populate_tables(data: *mut ads112c14_data) {
...
#[no_mangle]
unsafe extern "C" fn ads112c14_probe(client: *mut i2c_client) -> c_int {
    ...
    let mut refp_uV: u32 = 0;
    let mut refn_uV: u32 = 0;
    u32 reg_val;
    int ret;
...
//
    ...
    ads112c14_trigger_handler, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn ads112c14_i2c_add_driver(driver: *mut i2c_driver) -> c_int {
