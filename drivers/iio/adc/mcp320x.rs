...
//! Source: drivers/iio/adc/mcp320x.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mcp320x_chip_info {
    pub channels: *const iio_chan_spec,
    pub num_channels: c_uint,
    pub resolution: c_uint,
    pub /: *mut *mut unsigned int conv_time; / usec,
}
...
pub struct mcp320x {
    ...
    pub msg: spi_message,
    pub transfer: [spi_transfer; 2],
    pub start_conv_msg: spi_message,
    pub start_conv_transfer: spi_transfer,
    pub reg: *mut regulator,
    pub lock: mutex,
    pub chip_info: *const mcp320x_chip_info,
...
// val = sign_extend32((adc->rx_buf[0] & 0x1f) << 8
    ...
    case mcp3551:
    case mcp3553: {
    let mut raw: u32 = be32_to_cpup((__be32 *)adc.rx_buf);
...
#[no_mangle]
pub unsafe extern "C" fn if(BIT(22): raw &) -> else {
    else if (raw & BIT(22))
    raw &= ~BIT(22); /* overrange */
#[no_mangle]
pub unsafe extern "C" fn if(BIT(21): raw & BIT(23) || raw &) -> else {
...
#[no_mangle]
unsafe extern "C" fn mcp320x_regulator_disable(reg: *mut c_void) {
    static void mcp320x_regulator_disable(void *reg)
    {
    regulator_disable(reg);
    }
#[no_mangle]
unsafe extern "C" fn mcp320x_probe(spi: *mut spi_device) -> c_int {
