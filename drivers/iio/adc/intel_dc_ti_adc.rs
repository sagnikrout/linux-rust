...
//! Source: drivers/iio/adc/intel_dc_ti_adc.c
#![no_std]
...
pub const DC_TI_ADC_CNTL_REG: c_uint = 0x50;

pub const DC_TI_VBAT_ZSE_GE_REG: c_uint = 0x53;

// VBAT GE gain correction is in 0.0015 increments, ZSE is in 1.0 increments
pub const DC_TI_VBAT_GE_STEP: c_int = 15;
pub const DC_TI_VBAT_GE_DIV: c_int = 10000;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dc_ti_adc_info {
    pub /: *mut *mut mutex lock; / Protects against concurrent accesses to the ADC,
...
#[no_mangle]
unsafe extern "C" fn dc_ti_adc_isr(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn dc_ti_adc_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (ret)
    return ret;
    ret = devm_request_threaded_irq(dev, irq, core::ptr::null_mut(), dc_ti_adc_isr,
