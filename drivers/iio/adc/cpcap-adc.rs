...
//! Source: drivers/iio/adc/cpcap-adc.c
#![no_std]
...
// Register CPCAP_REG_ADCC2 bits
    ...
    CPCAP_BIT_TS_M2 | \
    CPCAP_BIT_TS_M1)
pub const CPCAP_MAX_TEMP_LVL: c_int = 27;
pub const CPCAP_FOUR_POINT_TWO_ADC: c_int = 801;
pub const ST_ADC_CAL_CHRGI_HIGH_THRESHOLD: c_int = 530;
pub const ST_ADC_CAL_CHRGI_LOW_THRESHOLD: c_int = 494;
pub const ST_ADC_CAL_BATTI_HIGH_THRESHOLD: c_int = 530;
pub const ST_ADC_CAL_BATTI_LOW_THRESHOLD: c_int = 494;
pub const ST_ADC_CALIBRATE_DIFF_THRESHOLD: c_int = 3;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cpcap_adc_ato {
    pub ato_in: c_ushort,
    pub atox_in: c_ushort,
    pub adc_ps_factor_in: c_ushort,
    pub atox_ps_factor_in: c_ushort,
    pub ato_out: c_ushort,
    pub atox_out: c_ushort,
    pub adc_ps_factor_out: c_ushort,
    pub atox_ps_factor_out: c_ushort,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cpcap_adc {
    pub reg: *mut regmap,
    pub dev: *mut device,
    pub vendor: u16,
    pub irq: c_int,
    pub /: *mut *mut mutex lock; / ADC register access lock,
    pub ato: *const cpcap_adc_ato,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cpcap_adc_phasing_tbl {
    pub offset: c_short,
    pub multiplier: c_ushort,
    pub divider: c_ushort,
    pub min: c_short,
    pub max: c_short,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cpcap_adc_conversion_tbl {
    pub conv_type: enum iio_chan_info_enum,
    pub align_offset: c_int,
    pub conv_offset: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cpcap_adc_request {
    pub channel: c_int,
    pub phase_tbl: *const cpcap_adc_phasing_tbl,
    pub conv_tbl: *const cpcap_adc_conversion_tbl,
    pub bank_index: c_int,
    pub timing: enum cpcap_adc_timing,
    pub result: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn cpcap_adc_irq_thread(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
// ADC calibration functions
    ...
    let mut timeout: c_ulong = jiffies + msecs_to_jiffies(3000);
    int error;
...
#[no_mangle]
unsafe extern "C" fn cpcap_adc_calibrate(ddata: *mut cpcap_adc) -> c_int {
...
// ADC setup, read and scale functions
    ...
    let mut value1: c_ushort = 0;
    let mut value2: c_ushort = 0;
    int error;
    if (!ato)
    return;
    switch (req.channel) {
...
#[no_mangle]
unsafe extern "C" fn cpcap_adc_stop_bank(ddata: *mut cpcap_adc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpcap_adc_phase(req: *mut cpcap_adc_request) {
    ...
    let mut index: c_int = req.channel;
...
#[no_mangle]
pub unsafe extern "C" fn if(phase_tbl[index].max: req->result >) -> else {
...
#[no_mangle]
unsafe extern "C" fn cpcap_adc_table_to_millicelcius(value: c_ushort) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cpcap_adc_convert(req: *mut cpcap_adc_request) {
...
#[no_mangle]
unsafe extern "C" fn cpcap_adc_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (ddata.irq < 0)
    return -ENODEV;
    error = devm_request_threaded_irq(&pdev.dev, ddata.irq, core::ptr::null_mut(),
