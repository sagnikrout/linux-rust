...
//! Source: drivers/iio/adc/mcp3422.c
#![no_std]
...
pub const MCP3422_SRATE_240: c_uint = 0x0;
pub const MCP3422_SRATE_60: c_uint = 0x1;
pub const MCP3422_SRATE_15: c_uint = 0x2;
pub const MCP3422_SRATE_3: c_uint = 0x3;
pub const MCP3422_PGA_1: c_int = 0;
pub const MCP3422_PGA_2: c_int = 1;
pub const MCP3422_PGA_4: c_int = 2;
pub const MCP3422_PGA_8: c_int = 3;
...
#[no_mangle]
unsafe extern "C" fn mcp3422_update_config(adc: *mut mcp3422, newconfig: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mcp3422_read(adc: *mut mcp3422, value: *mut c_int, config: *mut u8) -> c_int {
    static int mcp3422_read(struct mcp3422 *adc, int *value, u8 *config)
    {
    let mut ret: c_int = 0;
    let mut sample_rate: u8 = FIELD_GET(MCP3422_SRATE_MASK, adc.config);
    u8 buf[4] = {0, 0, 0, 0};
    u32 temp;
...
// value = sign_extend32(temp, mcp3422_sign_extend[sample_rate]);
    ...
    int ret;
    u8 config;
    let mut req_channel: u8 = channel.channel;
    mutex_lock(&adc.lock);
    ...
    int err;
    let mut sample_rate: u8 = FIELD_GET(MCP3422_SRATE_MASK, adc.config);
    let mut pga: u8 = FIELD_GET(MCP3422_PGA_MASK, adc.config);
    switch (mask) {
...
// val1 = mcp3422_sample_rates[FIELD_GET(MCP3422_SRATE_MASK, adc->config)];
    ...
    struct mcp3422 *adc = iio_priv(iio);
    u8 temp;
    let mut config: u8 = adc.config;
    let mut req_channel: u8 = channel.channel;
    let mut sample_rate: u8 = FIELD_GET(MCP3422_SRATE_MASK, config);
    u8 i;
    switch (mask) {
    case IIO_CHAN_INFO_SCALE:
    if (val1 != 0)
    ...
    mcp3422_show_samp_freqs, core::ptr::null_mut(), 0);
    static IIO_DEVICE_ATTR(in_voltage_scale_available, S_IRUGO,
    mcp3422_show_scales, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn mcp3422_probe(client: *mut i2c_client) -> c_int {
