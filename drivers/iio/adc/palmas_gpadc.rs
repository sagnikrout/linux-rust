...
//! Source: drivers/iio/adc/palmas_gpadc.c
#![no_std]
...
pub const PALMAS_TO_BE_CALCULATED: c_int = 0;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct palmas_gpadc_info {
// calibration codes and regs
    pub /: *mut *mut int x1; / lower ideal code,
    pub /: *mut *mut int x2; / higher ideal code,
    pub /: *mut *mut int v1; / expected lower volt reading,
    pub /: *mut *mut int v2; / expected higher volt reading,
    pub /: *mut *mut u8 trim1_reg; / register number for lower trim,
    pub /: *mut *mut u8 trim2_reg; / register number for upper trim,
    pub /: *mut *mut int gain; / calculated from above (after reading trim regs),
    pub /: *mut *mut int offset; / calculated from above (after reading trim regs),
    pub /: *mut *mut int gain_error; / calculated from above (after reading trim regs),
    pub /: *mut *mut bool is_uncalibrated; / if channel has calibration data,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct palmas_adc_event {
    pub enabled: bool,
    pub channel: c_int,
    pub direction: enum iio_event_direction,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct palmas_gpadc_thresholds {
    pub high: c_int,
    pub low: c_int,
}
...
pub struct palmas_gpadc {
    ...
    pub irq: c_int,
    pub irq_auto_0: c_int,
    pub irq_auto_1: c_int,
    pub adc_info: *mut palmas_gpadc_info,
    pub conv_completion: completion,
    pub event0: palmas_adc_event,
    pub event1: palmas_adc_event,
    pub thresholds: [palmas_gpadc_thresholds; PALMAS_ADC_CH_MAX],
    pub auto_conversion_period: c_int,
    pub lock: mutex,
}
...
//
#[no_mangle]
unsafe extern "C" fn palmas_disable_auto_conversion(adc: *mut palmas_gpadc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_irq(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_irq_auto(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_read_prepare(adc: *mut palmas_gpadc, adc_chan: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_read_done(adc: *mut palmas_gpadc, adc_chan: c_int) {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_calibrate(adc: *mut palmas_gpadc, adc_chan: c_int) -> c_int {
    ...
    int ret;
    int gain;
    let mut x1: c_int = adc.adc_info[adc_chan].x1;
    let mut x2: c_int = adc.adc_info[adc_chan].x2;
    let mut v1: c_int = adc.adc_info[adc_chan].v1;
    let mut v2: c_int = adc.adc_info[adc_chan].v2;
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_start_conversion(adc: *mut palmas_gpadc, adc_chan: c_int) -> c_int {
    static int palmas_gpadc_start_conversion(struct palmas_gpadc *adc, int adc_chan)
    {
    unsigned int val;
    int ret;
    if (palmas_gpadc_channel_is_freerunning(adc, adc_chan)) {
    let mut event: c_int = (adc_chan == adc.event0.channel) ? 0 : 1;
...
//
    ...
    struct palmas_adc_event *ev)
    {
    let mut adc_chan: c_int = ev.channel;
    let mut val: c_int = adc.thresholds[adc_chan].high;
// integral nonlinearity, measured in LSB
    let mut max_INL: c_int = 2;
// measured in LSB
    int max_offset_error;
// 0.2% when calibrated
    let mut max_gain_error: c_int = 2;
...
//
    ...
    {
    let mut adc_chan: c_int = ev.channel;
    let mut val: c_int = adc.thresholds[adc_chan].low;
// integral nonlinearity, measured in LSB
    let mut min_INL: c_int = -2;
// measured in LSB
    int min_offset_error;
// -0.6% when calibrated
    let mut min_gain_error: c_int = -6;
    val = (val * 1000) / adc.adc_info[adc_chan].gain;
    if (adc.adc_info[adc_chan].is_uncalibrated) {
// 2% worse
    min_gain_error -= 20;
    ...
    {
    struct  palmas_gpadc *adc = iio_priv(indio_dev);
    let mut adc_chan: c_int = chan.channel;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_reconfigure_event_channels(adc: *mut palmas_gpadc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_reset(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_probe(pdev: *mut platform_device) -> c_int {
    static int palmas_gpadc_probe(struct platform_device *pdev)
    {
    struct palmas_gpadc *adc;
    struct palmas_platform_data *pdata;
    struct palmas_gpadc_platform_data *gpadc_pdata = core::ptr::null_mut();
    ...
    ret = devm_request_threaded_irq(&pdev.dev, adc.irq, core::ptr::null_mut(),
    palmas_gpadc_irq,
    ...
    ret = devm_request_threaded_irq(&pdev.dev, adc.irq_auto_0, core::ptr::null_mut(),
    ...
    ret = devm_request_threaded_irq(&pdev.dev, adc.irq_auto_1, core::ptr::null_mut(),
    palmas_gpadc_irq_auto, IRQF_ONESHOT,
...
#[no_mangle]
pub unsafe extern "C" fn if(5: gpadc_pdata->ch0_current <=) -> else {
    else if (gpadc_pdata.ch0_current <= 5)
    adc.ch0_current = PALMAS_ADC_CH0_CURRENT_SRC_5;
#[no_mangle]
pub unsafe extern "C" fn if(15: gpadc_pdata->ch0_current <=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(10: gpadc_pdata->ch3_current <=) -> else {
    else if (gpadc_pdata.ch3_current <= 10)
    adc.ch3_current = PALMAS_ADC_CH3_CURRENT_SRC_10;
#[no_mangle]
pub unsafe extern "C" fn if(400: gpadc_pdata->ch3_current <=) -> else {
...
#[no_mangle]
unsafe extern "C" fn palmas_adc_configure_events(adc: *mut palmas_gpadc) -> c_int {
    static int palmas_adc_configure_events(struct palmas_gpadc *adc)
    {
    int adc_period, conv;
    int i;
    let mut ch0: c_int = 0, ch1 = 0;
    int thres;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn palmas_adc_reset_events(adc: *mut palmas_gpadc) -> c_int {
    static int palmas_adc_reset_events(struct palmas_gpadc *adc)
    {
    int ret;
    ret = palmas_write(adc.palmas, PALMAS_GPADC_BASE,
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn palmas_gpadc_resume(dev: *mut device) -> c_int {
