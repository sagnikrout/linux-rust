...
//! Source: drivers/iio/adc/stm32-dfsdm-adc.c
#![no_std]
...
// Conversion timeout
pub const DFSDM_TIMEOUT_US: c_int = 100000;

// Oversampling attribute default
pub const DFSDM_DEFAULT_OVERSAMPLING: c_int = 100;
// Oversampling max values
pub const DFSDM_MAX_INT_OVERSAMPLING: c_int = 256;
pub const DFSDM_MAX_FL_OVERSAMPLING: c_int = 1024;
...
//
pub const DFSDM_DATA_RES: c_int = 24;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dfsdm_dev_data {
    pub type: c_int,
    pub indio_dev): *mut *mut *mut int (init)(struct device dev, struct iio_dev,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dfsdm_adc {
    pub dfsdm: *mut stm32_dfsdm,
    pub dev_data: *const stm32_dfsdm_dev_data,
    pub fl_id: c_uint,
    pub nconv: c_uint,
    pub smask: c_ulong,
// ADC specific
    pub oversamp: c_uint,
    pub hwc: *mut iio_hw_consumer,
    pub backend: *mut iio_backend,
    pub completion: completion,
    pub buffer: *mut u32,
// Audio specific
    pub /: *mut *mut unsigned int spi_freq; / SPI bus clock frequency,
    pub /: *mut *mut unsigned int sample_freq; / Sample frequency after filter decimation,
    pub cb_priv): *const *const *const int (cb)(void data, size_t size, void,
    pub cb_priv: *mut c_void,
// DMA
    pub rx_buf: *mut u8,
    pub /: *mut *mut unsigned int bufi; / Buffer current position,
    pub /: *mut *mut unsigned int buf_sz; / Buffer size,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dfsdm_str2field {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct stm32_dfsdm_trig_info {
    pub name: *const c_char,
    pub jextsel: c_uint,
}
...
//
    ...
    d = fosr * iosr;
#[no_mangle]
pub unsafe extern "C" fn if(DFSDM_FASTSINC_ORDER: fl->ford ==) -> else {
    ...
    break;
#[no_mangle]
pub unsafe extern "C" fn if(oversamp: d !=) -> else {
    else if (d != oversamp)
    continue;
//
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_start_channel(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_stop_channel(indio_dev: *mut iio_dev) {
...
// Disable conversion
    ...
    struct stm32_dfsdm_adc *adc = iio_priv(indio_dev);
    struct regmap *regmap = adc.dfsdm.regmap;
    let mut jextsel: u32 = 0, jexten = STM32_DFSDM_JEXTEN_DISABLED;
    int ret;
    if (trig) {
...
//
    ...
    const char *of_str;
    let mut chan_idx: c_int = ch.scan_index;
    int ret, val;
...
// label is optional
    ...
    backend = devm_iio_backend_fwnode_get(&indio_dev.dev, core::ptr::null_mut(), node);
    if (IS_ERR(backend))
    ...
    let mut sample_freq: c_uint = adc.sample_freq;
    unsigned int spi_freq;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_stop_conv(indio_dev: *mut iio_dev) {
    ...
    {
    struct stm32_dfsdm_adc *adc = iio_priv(indio_dev);
    let mut watermark: c_uint = DFSDM_DMA_BUFFER_SIZE / 2;
    let mut rx_buf_sz: c_uint = DFSDM_DMA_BUFFER_SIZE;
//
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_dma_residue(adc: *mut stm32_dfsdm_adc) -> c_uint {
...
// Residue is size in bytes from end of buffer
    let mut i: c_uint = adc.buf_sz - state.residue;
    unsigned int size;
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_dma_buffer_done(data: *mut c_void) {
    static void stm32_dfsdm_dma_buffer_done(void *data)
    {
    struct iio_dev *indio_dev = data;
    struct stm32_dfsdm_adc *adc = iio_priv(indio_dev);
    let mut available: c_int = stm32_dfsdm_adc_dma_residue(adc);
    size_t old_pos;
//
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_dma_start(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_dma_stop(indio_dev: *mut iio_dev) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_postenable(indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_predisable(indio_dev: *mut iio_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn stm32_dfsdm_release_buff_cb(iio_dev: *mut iio_dev) -> c_int {
    ...
    return -EINVAL;
    adc = iio_priv(iio_dev);
    adc.cb = core::ptr::null_mut();
    adc.cb_priv = core::ptr::null_mut();
    return 0;
    }
    ...
    ret = stm32_dfsdm_start_conv(indio_dev, core::ptr::null_mut());
    if (ret < 0) {
...
// Mask IRQ for regular conversion achievement
    ...
    ret = -ETIMEDOUT;
#[no_mangle]
pub unsafe extern "C" fn if(0: time_left <) -> else {
    else if (time_left < 0)
    ret = time_left;
    else
    ret = IIO_VAL_INT;
    ...
    struct stm32_dfsdm_filter *fl = &adc.dfsdm.fl_list[adc.fl_id];
    struct stm32_dfsdm_filter_osr *flo = &fl.flo[fl.fast];
    let mut max: u32 = flo.max << (flo.lshift - chan.scan_type.shift);
...
//
    if (adc.backend) {
    ret = iio_backend_read_scale(adc.backend[idx], chan, val, core::ptr::null_mut());
    if (ret < 0)
    return ret;
...
//
    if (adc.backend) {
    ret = iio_backend_read_offset(adc.backend[idx], chan, val, core::ptr::null_mut());
    if (ret < 0)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_irq(irq: c_int, arg: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_dma_release(indio_dev: *mut iio_dev) {
    ...
    let mut ret: c_int = PTR_ERR(adc.dma_chan);
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_chan_init(indio_dev: *mut iio_dev, channels: *mut iio_chan_spec) -> c_int {
    static int stm32_dfsdm_chan_init(struct iio_dev *indio_dev, struct iio_chan_spec *channels)
    {
    let mut num_ch: c_int = indio_dev.num_channels;
    let mut chan_idx: c_int = 0;
    int ret;
    for (chan_idx = 0; chan_idx < num_ch; chan_idx++) {
    channels[chan_idx].scan_index = chan_idx;
    ret = stm32_dfsdm_adc_chan_init_one(indio_dev, &channels[chan_idx], core::ptr::null_mut());
    if (ret < 0)
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_generic_chan_init(indio_dev: *mut iio_dev, channels: *mut iio_chan_spec) -> c_int {
    static int stm32_dfsdm_generic_chan_init(struct iio_dev *indio_dev, struct iio_chan_spec *channels)
    {
    let mut chan_idx: c_int = 0, ret;
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_audio_init(dev: *mut device, indio_dev: *mut iio_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_init(dev: *mut device, indio_dev: *mut iio_dev) -> c_int {
...
// Optionally request DMA
    ...
    &iio_pollfunc_store_time, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stm32_dfsdm_adc_resume(dev: *mut device) -> c_int {
