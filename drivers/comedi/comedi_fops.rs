...
//! Source: drivers/comedi/comedi_fops.c
#![no_std]
...
pub struct comedi_file {
    ...
    pub last_detach_count: c_uint,
    pub last_attached:1: c_uint,
}

pub const COMEDI_NUM_MINORS: c_uint = 0x100;
    ...
    let mut comedi_default_buf_size_kb: c_uint = CONFIG_COMEDI_DEFAULT_BUF_SIZE_KB;
...
#[no_mangle]
unsafe extern "C" fn comedi_device_init(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn comedi_dev_kref_release(kref: *mut kref) {
...
//
#[no_mangle]
pub unsafe extern "C" fn comedi_dev_put(dev: *mut comedi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn comedi_device_cleanup(dev: *mut comedi_device) {
    static void comedi_device_cleanup(struct comedi_device *dev)
    {
    struct module *driver_module = core::ptr::null_mut();
    if (!dev)
    return;
...
#[no_mangle]
unsafe extern "C" fn comedi_clear_board_dev(dev: *mut comedi_device) -> bool {
    static bool comedi_clear_board_dev(struct comedi_device *dev)
    {
    let mut i: c_uint = dev.minor;
    let mut cleared: bool = false;
    lockdep_assert_held(&dev.mutex);
    mutex_lock(&comedi_board_minor_table_lock);
    if (dev == comedi_board_minor_table[i]) {
    comedi_board_minor_table[i] = core::ptr::null_mut();
    cleared = true;
    }
    ...
    comedi_board_minor_table[minor] = core::ptr::null_mut();
    mutex_unlock(&comedi_board_minor_table_lock);
    return dev;
    }
    static struct comedi_subdevice *
    ...
    let mut i: c_uint = minor - COMEDI_NUM_BOARD_MINORS;
    ...
    mutex_lock(&comedi_subdevice_minor_table_lock);
    s = comedi_subdevice_minor_table[i];
    dev = comedi_dev_get(s ? s.device : core::ptr::null_mut());
    mutex_unlock(&comedi_subdevice_minor_table_lock);
    return dev;
    }
//
...
#[no_mangle]
unsafe extern "C" fn comedi_file_check(file: *mut file) {
...
// sysfs attribute files
    ...
    {
    let mut minor: c_uint = MINOR(csdev.devt);
    struct comedi_device *dev;
    struct comedi_subdevice *s;
...
#[no_mangle]
unsafe extern "C" fn comedi_free_board_dev(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn __comedi_get_subdevice_runflags(s: *mut comedi_subdevice) -> c_uint {
    static unsigned int __comedi_get_subdevice_runflags(struct comedi_subdevice *s)
    {
    return s.runflags;
    }
#[no_mangle]
unsafe extern "C" fn comedi_get_subdevice_runflags(s: *mut comedi_subdevice) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn comedi_is_runflags_running(runflags: c_uint) -> bool {
    static bool comedi_is_runflags_running(unsigned int runflags)
    {
    return runflags & COMEDI_SRF_RUNNING;
    }
#[no_mangle]
unsafe extern "C" fn comedi_is_runflags_in_error(runflags: c_uint) -> bool {
    static bool comedi_is_runflags_in_error(unsigned int runflags)
    {
    return runflags & COMEDI_SRF_ERROR;
    }
#[no_mangle]
unsafe extern "C" fn comedi_is_runflags_busy(runflags: c_uint) -> bool {
...
pub unsafe extern "C" fn comedi_is_subdevice_running(s: *mut comedi_subdevice) -> bool {
    ...
    {
    let mut runflags: c_uint = comedi_get_subdevice_runflags(s);
    return comedi_is_runflags_running(runflags);
    }
    EXPORT_SYMBOL_GPL(comedi_is_subdevice_running);
#[no_mangle]
unsafe extern "C" fn __comedi_is_subdevice_running(s: *mut comedi_subdevice) -> bool {
    static bool __comedi_is_subdevice_running(struct comedi_subdevice *s)
    {
    let mut runflags: c_uint = __comedi_get_subdevice_runflags(s);
    return comedi_is_runflags_running(runflags);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn comedi_put_is_subdevice_running(s: *mut comedi_subdevice) {
...
//
    ...
    comedi_buf_reset(s);
    async.inttrig = core::ptr::null_mut();
    kfree(async.cmd.chanlist);
    async.cmd.chanlist = core::ptr::null_mut();
    s.busy = core::ptr::null_mut();
    wake_up_interruptible_all(&async.wait_head);
    }
    }
#[no_mangle]
unsafe extern "C" fn do_cancel(dev: *mut comedi_device, s: *mut comedi_subdevice) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn comedi_device_cancel_all(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn is_device_busy(dev: *mut comedi_device) -> c_int {
...
//
    ...
    let mut become_nonbusy: bool = false;
    bool attach_locked;
...
// command was set up in "write" direction
    ...
    if (become_nonbusy) {
    struct comedi_subdevice *new_s = core::ptr::null_mut();
//
...
//
#[no_mangle]
unsafe extern "C" fn get_valid_routes(dev: *mut comedi_device, data: *mut c_uint) -> c_int {
...
//
    let mut orig_mask: c_uint = data[0];
...
//
// arbitrary limits
pub const MIN_SAMPLES: c_int = 16;
pub const MAX_SAMPLES: c_int = 65536;
    static int do_insnlist_ioctl(struct comedi_device *dev,
    struct comedi_insn *insns,
    unsigned int n_insns,
    void *file)
    {
    unsigned int *data = core::ptr::null_mut();
    let mut max_n_data_required: c_uint = MIN_SAMPLES;
    let mut i: c_int = 0;
    let mut ret: c_int = 0;
    lockdep_assert_held(&dev.mutex);
...
// Allocate scratch space for all instruction data.
    ...
    for (i = 0; i < n_insns; ++i) {
    let mut n: c_uint = insns[i].n;
    if (insns[i].insn & INSN_MASK_WRITE) {
    if (copy_from_user(data, insns[i].data,
#[no_mangle]
pub unsafe extern "C" fn sizeof(int)): unsigned) -> *mut n {
    dev_dbg(dev.class_dev,
...
#[no_mangle]
unsafe extern "C" fn check_insnlist_len(dev: *mut comedi_device, n_insns: c_uint) -> c_int {
...
//
    ...
    {
    unsigned int *data = core::ptr::null_mut();
    let mut n_data: c_uint = MIN_SAMPLES;
    let mut ret: c_int = 0;
    lockdep_assert_held(&dev.mutex);
...
//
    ...
    int ret;
    lockdep_assert_held(&dev.mutex);
    cmd.chanlist = core::ptr::null_mut();
...
// make sure channel/gain list isn't too short
    ...
    async.cmd = *cmd;
    async.cmd.data = core::ptr::null_mut();
...
// restore chanlist pointer before copying back
    cmd.chanlist = (unsigned int  *)user_chanlist;
    cmd.data = core::ptr::null_mut();
// copy = true;
    ret = -EAGAIN;
    goto cleanup;
    }
...
//
    ...
    struct comedi_insn *insns = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn comedi_vm_open(area: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn comedi_vm_close(area: *mut vm_area_struct) {
...
#[no_mangle]
unsafe extern "C" fn comedi_mmap(file: *mut file, vma: *mut vm_area_struct) -> c_int {
    ...
    struct comedi_subdevice *s;
    struct comedi_async *async;
    struct comedi_buf_map *bm = core::ptr::null_mut();
    struct comedi_buf_page *buf;
    let mut start: c_ulong = vma.vm_start;
    unsigned long size;
    int n_pages;
    int i;
    let mut retval: c_int = 0;
...
//
    ...
    up_read(&dev.attach_lock);
    comedi_buf_map_put(bm);	/* put reference to buf map - okay if core::ptr::null_mut() */
    return retval;
    }
#[no_mangle]
unsafe extern "C" fn comedi_poll(file: *mut file, wait: *mut poll_table) -> __poll_t {
    ...
    s = comedi_file_write_subdevice(file);
    if (s && s.async) {
    let mut bps: c_uint = comedi_bytes_per_sample(s);
    if (s != s_read)
    poll_wait(file, &s.async.wait_head, wait);
    if (s.busy != file || !comedi_is_subdevice_running(s) ||
    !(s.async.cmd.flags & CMDF_WRITE) ||
    ...
    let mut page: c_uint = src_offset >> PAGE_SHIFT;
    let mut offset: c_uint = offset_in_page(src_offset);
    while (n) {
    let mut copy_amount: c_uint = min(n, PAGE_SIZE - offset);
    ...
    struct comedi_buf_map *bm = s.async.buf_map;
    struct comedi_buf_page *buf_page_list = bm.page_list;
    let mut page: c_uint = dst_offset >> PAGE_SHIFT;
    let mut offset: c_uint = offset_in_page(dst_offset);
    while (n) {
    let mut copy_amount: c_uint = min(n, PAGE_SIZE - offset);
    unsigned int uncopied;
...
#[no_mangle]
unsafe extern "C" fn comedi_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn comedi_fasync(fd: c_int, file: *mut file, on: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn comedi_close(inode: *mut inode, file: *mut file) -> c_int {
    static int comedi_close(struct inode *inode, struct file *file)
    {
    struct comedi_file *cfp = file.private_data;
    struct comedi_device *dev = cfp.dev;
    struct comedi_subdevice *s = core::ptr::null_mut();
    int i;
    mutex_lock(&dev.mutex);
    if (dev.subdevices) {
    for (i = 0; i < dev.n_subdevices; i++) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct comedi32_chaninfo_struct {
    pub subdev: c_uint,
    pub /: *mut *mut *mut compat_uptr_t maxdata_list; / 32-bit 'unsigned int ',
    pub /: *mut *mut *mut compat_uptr_t flaglist; / 32-bit 'unsigned int ',
    pub /: *mut *mut *mut compat_uptr_t rangelist; / 32-bit 'unsigned int ',
    pub unused: [c_uint; 4],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct comedi32_rangeinfo_struct {
    pub range_type: c_uint,
    pub /: *mut *mut *mut compat_uptr_t range_ptr; / 32-bit 'void ',
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct comedi32_cmd_struct {
    pub subdev: c_uint,
    pub flags: c_uint,
    ...
    pub stop_src: c_uint,
    pub stop_arg: c_uint,
    pub /: *mut *mut *mut compat_uptr_t chanlist; / 32-bit 'unsigned int ',
    pub chanlist_len: c_uint,
    pub /: *mut *mut *mut compat_uptr_t data; / 32-bit 'short ',
    pub data_len: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct comedi32_insn_struct {
    pub insn: c_uint,
    pub n: c_uint,
    pub /: *mut *mut *mut compat_uptr_t data; / 32-bit 'unsigned int ',
    pub subdev: c_uint,
    pub chanspec: c_uint,
    pub unused: [c_uint; 3],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct comedi32_insnlist_struct {
    pub n_insns: c_uint,
    pub /: *mut *mut *mut compat_uptr_t insns; / 32-bit 'struct comedi_insn ',
}

// Handle 32-bit COMEDI_CHANINFO ioctl.
#[no_mangle]
unsafe extern "C" fn compat_chaninfo(file: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compat_rangeinfo(file: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compat_cmd(file: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compat_cmdtest(file: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compat_insnlist(file: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn compat_insn(file: *mut file, arg: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn comedi_compat_ioctl(file: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
...
#[no_mangle]
pub unsafe extern "C" fn _comedi_event(dev: *mut comedi_device, s: *mut comedi_subdevice) {
...
// Note: the ->mutex is pre-locked on successful return
    ...
    MKDEV(COMEDI_MAJOR, i), core::ptr::null_mut(), "comedi%i", i);
    if (!IS_ERR(csdev))
...
#[no_mangle]
pub unsafe extern "C" fn comedi_release_hardware_device(hardware_device: *mut device) {
...
#[no_mangle]
pub unsafe extern "C" fn comedi_alloc_subdevice_minor(s: *mut comedi_subdevice) -> c_int {
    ...
    MKDEV(COMEDI_MAJOR, i), core::ptr::null_mut(), "comedi%i_subd%i",
    dev.minor, s.index);
    if (!IS_ERR(csdev))
    s.class_dev = csdev;
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn comedi_free_subdevice_minor(s: *mut comedi_subdevice) {
    ...
    comedi_subdevice_minor_table[i] = core::ptr::null_mut();
    mutex_unlock(&comedi_subdevice_minor_table_lock);
    if (s.class_dev) {
    device_destroy(&comedi_class, MKDEV(COMEDI_MAJOR, s.minor));
    s.class_dev = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn comedi_cleanup_board_minors() {
...
#[no_mangle]
unsafe extern "C" fn comedi_init() -> int __init {
...
// create devices files for legacy/manual use
    ...
    struct comedi_device *dev;
    dev = comedi_alloc_board_minor(core::ptr::null_mut());
    if (IS_ERR(dev)) {
    retval = PTR_ERR(dev);
...
#[no_mangle]
unsafe extern "C" fn comedi_cleanup() -> void __exit {
