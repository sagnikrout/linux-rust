...
//! Source: drivers/comedi/drivers/addi_apci_1500.c
#![no_std]
...
//
pub const APCI1500_Z8536_PORTC_REG: c_uint = 0x00;
pub const APCI1500_Z8536_PORTB_REG: c_uint = 0x01;
pub const APCI1500_Z8536_PORTA_REG: c_uint = 0x02;
pub const APCI1500_Z8536_CTRL_REG: c_uint = 0x03;
//
// PCI Bar 2 Register map (devpriv->addon)
//
pub const APCI1500_CLK_SEL_REG: c_uint = 0x00;
pub const APCI1500_DI_REG: c_uint = 0x00;
pub const APCI1500_DO_REG: c_uint = 0x02;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct apci1500_private {
    pub amcc: c_ulong,
    pub addon: c_ulong,
    pub clk_src: c_uint,
// Digital trigger configuration [0]=AND [1]=OR
    pub /: *mut *mut unsigned int pm[2]; / Pattern Mask,
    pub /: *mut *mut unsigned int pt[2]; / Pattern Transition,
    pub /: *mut *mut unsigned int pp[2]; / Pattern Polarity,
}

#[no_mangle]
unsafe extern "C" fn z8536_read(dev: *mut comedi_device, reg: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn z8536_reset(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn apci1500_port_enable(dev: *mut comedi_device, enable: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn if(1: chan ==) -> else {
    else if (chan == 1)
...
#[no_mangle]
unsafe extern "C" fn apci1500_interrupt(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
// Enable Ports A & B
    ...
    let mut pa_mode: c_uint = Z8536_PAB_MODE_PMS_DISABLE;
    let mut pb_mode: c_uint = Z8536_PAB_MODE_PMS_DISABLE;
    let mut pa_trig: c_uint = trig_num & 0x01;
    let mut pb_trig: c_uint = (trig_num >> 1) & 0x01;
    let mut valid_trig: bool = false;
    unsigned int val;
...
//
    ...
    {
    struct apci1500_private *devpriv = dev.private;
    let mut trig: c_uint = data[1];
    let mut shift: c_uint = data[3];
...
// Hardware Triggered Strobe (watchdog)
    ...
    val |= Z8536_CT_MODE_EGE;
#[no_mangle]
pub unsafe extern "C" fn if(1: data[1] >) -> else {
    else if (data[1] > 1)
    return -EINVAL;
    z8536_write(dev, val, Z8536_CT_MODE_REG(chan));
    break;
...
#[no_mangle]
unsafe extern "C" fn apci1500_detach(dev: *mut comedi_device) {
