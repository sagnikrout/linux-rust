...
//! Source: drivers/comedi/drivers/c6xdigio.c
#![no_std]
...
// Register I/O map
//
pub const C6XDIGIO_DATA_REG: c_uint = 0x00;

pub const C6XDIGIO_STATUS_REG: c_uint = 0x01;
pub const C6XDIGIO_CTRL_REG: c_uint = 0x02;
pub const C6XDIGIO_TIME_OUT: c_int = 20;
#[no_mangle]
unsafe extern "C" fn c6xdigio_chk_status(dev: *mut comedi_device, context: c_ulong) -> c_int {
...
// bits = val;
    ...
    {
    let mut cmd: c_uint = C6XDIGIO_DATA_PWM | C6XDIGIO_DATA_CHAN(chan);
    unsigned int bits;
    if (val > 498)
    val = 498;
    if (val < 2)
    ...
    unsigned int chan)
    {
    let mut cmd: c_uint = C6XDIGIO_DATA_ENCODER | C6XDIGIO_DATA_CHAN(chan);
    ...
    let mut val: c_uint = (s.state >> (16 * chan)) & 0xffff;
    int i;
    for (i = 0; i < insn.n; i++) {
    val = data[i];
...
#[no_mangle]
unsafe extern "C" fn c6xdigio_init(dev: *mut comedi_device) {
...
// will not be connected when device driver is loaded.
    ...
    let mut c6xdigio_pnp_registered: static bool = false;
#[no_mangle]
unsafe extern "C" fn c6xdigio_module_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn c6xdigio_module_exit() -> void __exit {
