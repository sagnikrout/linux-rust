...
//! Source: drivers/comedi/drivers/quatech_daqp_cs.c
#![no_std]
...
//
pub const DAQP_AI_FIFO_REG: c_uint = 0x00;
pub const DAQP_SCANLIST_REG: c_uint = 0x01;

pub const DAQP_CTRL_REG: c_uint = 0x02;

pub const DAQP_STATUS_REG: c_uint = 0x02;

// these bits clear when the status register is read

    DAQP_STATUS_END_OF_SCAN |	\
    DAQP_STATUS_FIFO_THRESHOLD)
pub const DAQP_DI_REG: c_uint = 0x03;
pub const DAQP_DO_REG: c_uint = 0x03;
pub const DAQP_PACER_LOW_REG: c_uint = 0x04;
pub const DAQP_PACER_MID_REG: c_uint = 0x05;
pub const DAQP_PACER_HIGH_REG: c_uint = 0x06;
pub const DAQP_CMD_REG: c_uint = 0x07;
// the monostable bits are self-clearing after the function is complete

pub const DAQP_AO_REG: c_uint = 0x08	/* and 0x09 (16-bit) */;
pub const DAQP_TIMER_REG: c_uint = 0x0a	/* and 0x0b (16-bit) */;
pub const DAQP_AUX_REG: c_uint = 0x0f;
// Auxiliary Control register bits (write)

// Auxiliary Status register bits (read)

pub const DAQP_FIFO_SIZE: c_int = 4096;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct daqp_private {
    pub pacer_div: c_uint,
    pub stop: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn daqp_clear_events(dev: *mut comedi_device, loops: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn daqp_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t daqp_interrupt(int irq, void *dev_id)
    {
    struct comedi_device *dev = dev_id;
    struct comedi_subdevice *s = dev.read_subdev;
    struct comedi_cmd *cmd;
    let mut loop_limit: c_int = 10000;
    int status;
    if (!dev.attached)
    return IRQ_NONE;
    cmd = &s.async.cmd;
...
//
#[no_mangle]
unsafe extern "C" fn daqp_ns_to_timer(ns: *mut c_uint, flags: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn daqp_set_pacer(dev: *mut comedi_device, val: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn daqp_ai_cmd(dev: *mut comedi_device, s: *mut comedi_subdevice) -> c_int {
...
// Program scan list
    ...
    let mut start: c_int = (i == 0 || scanlist_start_on_every_entry);
...
#[no_mangle]
unsafe extern "C" fn daqp_cs_suspend(link: *mut pcmcia_device) -> c_int {
    static int daqp_cs_suspend(struct pcmcia_device *link)
    {
    struct comedi_device *dev = link.priv;
    struct daqp_private *devpriv = dev ? dev.private : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn daqp_cs_resume(link: *mut pcmcia_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn daqp_cs_attach(link: *mut pcmcia_device) -> c_int {
