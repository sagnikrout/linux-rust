...
//! Source: drivers/comedi/drivers/ni_labpc_common.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn labpc_inb(dev: *mut comedi_device, reg: c_ulong) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn labpc_readb(dev: *mut comedi_device, reg: c_ulong) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn labpc_cancel(dev: *mut comedi_device, s: *mut comedi_subdevice) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn labpc_read_adc_fifo(dev: *mut comedi_device) -> c_uint {
    static unsigned int labpc_read_adc_fifo(struct comedi_device *dev)
    {
    struct labpc_private *devpriv = dev.private;
    let mut lsb: c_uint = devpriv.read_byte(dev, ADC_FIFO_REG);
    let mut msb: c_uint = devpriv.read_byte(dev, ADC_FIFO_REG);
    return (msb << 8) | lsb;
    }
#[no_mangle]
unsafe extern "C" fn labpc_clear_adc_fifo(dev: *mut comedi_device) {
...
// figures out what counter values to use based on command
    ...
    let mut convert_period: c_uint = labpc_ai_convert_period(cmd, mode);
    let mut scan_period: c_uint = labpc_ai_scan_period(cmd, mode);
...
#[no_mangle]
unsafe extern "C" fn labpc_ai_scan_mode(cmd: *const comedi_cmd) -> enum scan_mode {
...
// chanlist may be NULL during cmdtest
    ...
    {
    let mut mode: enum scan_mode = labpc_ai_scan_mode(cmd);
...
#[no_mangle]
unsafe extern "C" fn labpc_ai_cmd(dev: *mut comedi_device, s: *mut comedi_subdevice) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn labpc_drain_fifo(dev: *mut comedi_device) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn labpc_drain_dregs(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn labpc_interrupt(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn labpc_serial_in(dev: *mut comedi_device) -> c_uint {
...
// bits to tell eeprom to expect a read
    let mut read_instruction: c_int = 0x3;
// 8 bit write lengths to eeprom
    let mut write_length: c_int = 8;
...
#[no_mangle]
unsafe extern "C" fn labpc_eeprom_read_status(dev: *mut comedi_device) -> c_uint {
    static unsigned int labpc_eeprom_read_status(struct comedi_device *dev)
    {
    struct labpc_private *devpriv = dev.private;
    unsigned int value;
    let mut read_status_instruction: c_int = 0x5;
...
// disable read/write to eeprom
    ...
    {
    struct labpc_private *devpriv = dev.private;
    let mut write_enable_instruction: c_int = 0x6;
    let mut write_instruction: c_int = 0x2;
    const int write_length = 8;	/*  8 bit write lengths to eeprom */
// enable read/write to eeprom
    devpriv.cmd5 &= ~CMD5_EEPROMCS;
    udelay(1);
...
#[no_mangle]
pub unsafe extern "C" fn labpc_common_detach(dev: *mut comedi_device) {
