...
//! Source: drivers/comedi/drivers/adv_pci1710.c
#![no_std]
...
//
pub const PCI171X_AD_DATA_REG: c_uint = 0x00	/* R:   A/D data */;
pub const PCI171X_SOFTTRG_REG: c_uint = 0x00	/* W:   soft trigger for A/D */;
pub const PCI171X_RANGE_REG: c_uint = 0x02	/* W:   A/D gain/range register */;

pub const PCI171X_MUX_REG: c_uint = 0x04	/* W:   A/D multiplexor control */;

pub const PCI171X_STATUS_REG: c_uint = 0x06	/* R:   status register */;

pub const PCI171X_CTRL_REG: c_uint = 0x06	/* W:   control register */;

pub const PCI171X_CLRINT_REG: c_uint = 0x08	/* W:   clear interrupts request */;
pub const PCI171X_CLRFIFO_REG: c_uint = 0x09	/* W:   clear FIFO */;

pub const PCI171X_DAREF_REG: c_uint = 0x0e	/* W:   D/A reference control */;

pub const PCI171X_DI_REG: c_uint = 0x10	/* R:   digital inputs */;
pub const PCI171X_DO_REG: c_uint = 0x10	/* W:   digital outputs */;
pub const PCI171X_TIMER_BASE: c_uint = 0x18	/* R/W: 8254 timer */;
...
pub struct boardtype {
    ...
    pub is_pci1711:1: c_uint,
    pub is_pci1713:1: c_uint,
    pub has_ao:1: c_uint,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pci1710_private {
    pub max_samples: c_uint,
    pub /: *mut *mut unsigned int ctrl; / control register value,
    pub /: *mut *mut unsigned int ctrl_ext; / used to switch from TRIG_EXT to TRIG_xxx,
    pub /: *mut *mut unsigned int mux_scan; / used to set the channel interval to scan,
    pub ai_et: c_uchar,
    pub /: *mut *mut unsigned int act_chanlist[32]; / list of scanned channel,
    pub /: *mut *mut unsigned char saved_seglen; / len of the non-repeating chanlist,
    pub /: *mut *mut unsigned char da_ranges; / copy of D/A outpit range register,
    pub /: *mut *mut unsigned char unipolar_gain; / adjust for unipolar gain codes,
}
    ...
    let mut last_aref: c_uint = CR_AREF(cmd.chanlist[0]);
    let mut next_chan: c_uint = (chan0 + 1) % s.n_chan;
...
// next correct channel in list
    ...
    let mut last_chan: c_uint = CR_CHAN(chanlist[seglen - 1]);
    unsigned int i;
    for (i = 0; i < seglen; i++) {	/*  store range list to card */
    let mut chan: c_uint = CR_CHAN(chanlist[i]);
    let mut range: c_uint = CR_RANGE(chanlist[i]);
    let mut aref: c_uint = CR_AREF(chanlist[i]);
    let mut rangeval: c_uint = 0;
    if (aref == AREF_DIFF)
...
#[no_mangle]
unsafe extern "C" fn pci1710_irq_handler(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn pci1710_ai_cmd(dev: *mut comedi_device, s: *mut comedi_subdevice) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pci1710_reset(dev: *mut comedi_device) {
...
// set digital outputs to 0
    ...
    const struct boardtype *board = core::ptr::null_mut();
