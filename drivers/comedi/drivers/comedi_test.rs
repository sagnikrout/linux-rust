...
//! Source: drivers/comedi/drivers/comedi_test.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct waveform_private {
    pub /: *mut *mut timer_list ai_timer; / timer for AI commands,
    pub /: *mut *mut u64 ai_convert_time; / time of next AI conversion in usec,
    pub /: *mut *mut unsigned int wf_amplitude; / waveform amplitude in microvolts,
    pub /: *mut *mut unsigned int wf_period; / waveform period in microseconds,
    pub /: *mut *mut unsigned int wf_current; / current time in waveform period,
    pub /: *mut *mut unsigned int ai_scan_period; / AI scan period in usec,
    pub /: *mut *mut unsigned int ai_convert_period; / AI conversion period in usec,
    pub /: *mut *mut timer_list ao_timer; / timer for AO commands,
    pub /: *mut *mut *mut comedi_device dev; / parent comedi device,
    pub /: *mut *mut u64 ao_last_scan_time; / time of previous AO scan in usec,
    pub /: *mut *mut unsigned int ao_scan_period; / AO scan period in usec,
    pub /: *mut *mut bool ai_timer_enable:1; / should AI timer be running?,
    pub /: *mut *mut bool ao_timer_enable:1; / should AO timer be running?,
    pub ao_loopbacks: [c_ushort; N_CHANS],
}
...
// fake analog input ranges
    ...
    let mut offset: c_uint = s.maxdata / 2;
    u64 value;
...
unsafe extern "C" fn waveform_ai_timer(t: *mut timer_list) {
    ...
    let mut chanspec: c_uint = cmd.chanlist[async.cur_chan];
...
//
#[no_mangle]
unsafe extern "C" fn waveform_ao_timer(t: *mut timer_list) {
...
// digital input/output subdevice
    ...
    {
    let mut amplitude: c_int = it.options[0];
    let mut period: c_int = it.options[1];
// set default amplitude and period
    ...
    {
    let mut amplitude: c_int = set_amplitude;
    let mut period: c_int = set_period;
// set default amplitude and period
    if (!amplitude)
    amplitude = 1000000;	/* 1 volt */
    if (!period)
...
#[no_mangle]
unsafe extern "C" fn waveform_detach(dev: *mut comedi_device) {
...
//
#[no_mangle]
unsafe extern "C" fn comedi_test_init() -> int __init {
    ...
    ctdev = device_create(&ctcls, core::ptr::null_mut(), MKDEV(0, 0), core::ptr::null_mut(), DEV_NAME);
    if (IS_ERR(ctdev)) {
...
#[no_mangle]
unsafe extern "C" fn comedi_test_exit() -> void __exit {
