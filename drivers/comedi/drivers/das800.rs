...
//! Source: drivers/comedi/drivers/das800.c
#![no_std]
...
pub const DAS800_LSB: c_int = 0;
pub const FIFO_EMPTY: c_uint = 0x1;
pub const FIFO_OVF: c_uint = 0x2;
pub const DAS800_MSB: c_int = 1;
pub const DAS800_CONTROL1: c_int = 2;
pub const CONTROL1_INTE: c_uint = 0x8;
pub const DAS800_CONV_CONTROL: c_int = 2;
pub const ITE: c_uint = 0x1;
pub const CASC: c_uint = 0x2;
pub const DTEN: c_uint = 0x4;
pub const IEOC: c_uint = 0x8;
pub const EACS: c_uint = 0x10;
pub const CONV_HCEN: c_uint = 0x80;
pub const DAS800_SCAN_LIMITS: c_int = 2;
pub const DAS800_STATUS: c_int = 2;
pub const IRQ: c_uint = 0x8;
pub const BUSY: c_uint = 0x80;
pub const DAS800_GAIN: c_int = 3;
pub const CIO_FFOV: c_uint = 0x8   /* cio-das802/16 fifo overflow */;
pub const CIO_ENHF: c_uint = 0x90  /* cio-das802/16 fifo half full int ena */;
pub const CONTROL1: c_uint = 0x80;
pub const CONV_CONTROL: c_uint = 0xa0;
pub const SCAN_LIMITS: c_uint = 0xc0;
pub const ID: c_uint = 0xe0;
pub const DAS800_8254: c_int = 4;
pub const DAS800_STATUS2: c_int = 7;
pub const STATUS2_HCEN: c_uint = 0x80;

pub const DAS800_ID: c_int = 7;
pub const DAS802_16_HALF_FIFO_SZ: c_int = 128;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct das800_board {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct das800_private {
    pub /: *mut *mut unsigned int do_bits; / digital output bits,
}
...
#[no_mangle]
unsafe extern "C" fn das800_ind_read(dev: *mut comedi_device, reg: c_uint) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn das800_enable(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn das800_disable(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn das800_cancel(dev: *mut comedi_device, s: *mut comedi_subdevice) -> c_int {
...
// Step 5: check channel list if it exists
    ...
    let mut gain: c_uint = CR_RANGE(cmd.chanlist[0]);
    let mut start_chan: c_uint = CR_CHAN(cmd.chanlist[0]);
    let mut end_chan: c_uint = (start_chan + cmd.chanlist_len - 1) % 8;
    let mut scan_chans: c_uint = (end_chan << 3) | start_chan;
    int conv_bits;
    unsigned long irq_flags;
    das800_disable(dev);
    spin_lock_irqsave(&dev.spinlock, irq_flags);
...
#[no_mangle]
unsafe extern "C" fn das800_ai_get_sample(dev: *mut comedi_device) -> c_uint {
    static unsigned int das800_ai_get_sample(struct comedi_device *dev)
    {
    let mut lsb: c_uint = inb(dev.iobase + DAS800_LSB);
    let mut msb: c_uint = inb(dev.iobase + DAS800_MSB);
    return (msb << 8) | lsb;
    }
#[no_mangle]
unsafe extern "C" fn das800_interrupt(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
// trigger conversion
    ...
    {
    const struct das800_board *board = dev.board_ptr;
    let mut index: c_int = board ? board - das800_boards : -EINVAL;
    int id_bits;
    unsigned long irq_flags;
//
...
#[no_mangle]
unsafe extern "C" fn das800_attach(dev: *mut comedi_device, it: *mut comedi_devconfig) -> c_int {
