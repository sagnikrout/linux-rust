...
//! Source: drivers/comedi/drivers/ni_atmio.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ni_isapnp_find_board(dev: *mut pnp_dev) -> c_int {
    static int ni_isapnp_find_board(struct pnp_dev **dev)
    {
    struct pnp_dev *isapnp_dev = core::ptr::null_mut();
    int i;
    for (i = 0; i < ARRAY_SIZE(ni_boards); i++) {
    isapnp_dev =
    pnp_find_dev(core::ptr::null_mut(),
    ISAPNP_VENDOR('N', 'I', 'C'),
...
// dev = isapnp_dev;
    ...
    {
    let mut device_id: c_int = ni_read_eeprom(dev, 511);
    int i;
    for (i = 0; i < ARRAY_SIZE(ni_boards); i++) {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: device_id ==) -> else {
    else if (device_id == 0)
...
#[no_mangle]
unsafe extern "C" fn ni_atmio_detach(dev: *mut comedi_device) {
    static void ni_atmio_detach(struct comedi_device *dev)
    {
    struct pnp_dev *isapnp_dev;
    mio_common_detach(dev);
    comedi_legacy_detach(dev);
    isapnp_dev = dev.hw_dev ? to_pnp_dev(dev.hw_dev) : core::ptr::null_mut();
    if (isapnp_dev)
    pnp_device_detach(isapnp_dev);
    }
