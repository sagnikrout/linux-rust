...
//! Source: drivers/comedi/drivers/cb_pcidas.c
#![no_std]
...
//
pub const PCIDAS_CTRL_REG: c_uint = 0x00	/* INTERRUPT / ADC FIFO register */;

    PCIDAS_CTRL_ADHFI | PCIDAS_CTRL_ADNEI | \
    PCIDAS_CTRL_LADFUL)

pub const PCIDAS_AI_REG: c_uint = 0x02	/* ADC CHANNEL MUX AND CONTROL reg */;

pub const PCIDAS_TRIG_REG: c_uint = 0x04	/* TRIGGER CONTROL/STATUS register */;

pub const PCIDAS_CALIB_REG: c_uint = 0x06	/* CALIBRATION register */;

pub const PCIDAS_AO_REG: c_uint = 0x08	/* dac control and status register */;

//
// PCI BAR2 Register map (devpriv->pcibar2)
//
pub const PCIDAS_AI_DATA_REG: c_uint = 0x00;
pub const PCIDAS_AI_FIFO_CLR_REG: c_uint = 0x02;
//
// PCI BAR3 Register map (dev->iobase)
//
pub const PCIDAS_AI_8254_BASE: c_uint = 0x00;
pub const PCIDAS_8255_BASE: c_uint = 0x04;
pub const PCIDAS_AO_8254_BASE: c_uint = 0x08;
//
// PCI BAR4 Register map (devpriv->pcibar4)
//

pub const PCIDAS_AO_FIFO_REG: c_uint = 0x00;
pub const PCIDAS_AO_FIFO_CLR_REG: c_uint = 0x02;
...
pub struct cb_pcidas_board {
    ...
    pub /: *mut *mut int ao_scan_speed; / analog output scan speed for 1602 series,
    pub /: *mut *mut int fifo_size; / number of samples fifo can hold,
    pub /: *mut *mut unsigned int is_16bit; / ai/ao is 1=16-bit; 0=12-bit,
    pub /: *mut *mut unsigned int use_alt_range:1; / use alternate ai range table,
    pub /: *mut *mut unsigned int has_ao:1; / has 2 analog output channels,
    pub /: *mut *mut unsigned int has_ao_fifo:1; / analog output has fifo,
    pub /: *mut *mut unsigned int has_ad8402:1; / trimpot type 1=AD8402; 0=AD7376,
    pub has_dac08:1: c_uint,
    pub is_1602:1: c_uint,
}
...
pub struct cb_pcidas_private {
    pub ao_pacer: *mut comedi_8254,
// base addresses
    pub /: *mut *mut unsigned long amcc; / pcibar0,
    pub pcibar1: c_ulong,
    pub pcibar2: c_ulong,
    pub pcibar4: c_ulong,
// bits to write to registers
    pub ctrl: c_uint,
    pub amcc_intcsr: c_uint,
    pub ao_ctrl: c_uint,
// fifo buffers
    pub ai_buffer: [c_ushort; AI_BUFFER_SIZE],
    pub ao_buffer: [c_ushort; AO_BUFFER_SIZE],
    pub calib_src: c_uint,
}
...
#[no_mangle]
unsafe extern "C" fn cb_pcidas_dac08_write(dev: *mut comedi_device, val: c_uint) {
...
// set start trigger and burst mode
    ...
    let mut chan1: c_uint = CR_CHAN(cmd.chanlist[1]);
    if (chan0 != 0 || chan1 != 1) {
    dev_dbg(dev.class_dev,
...
// step 4: fix up any arguments
    if (cmd.scan_begin_src == TRIG_TIMER) {
    let mut arg: c_uint = cmd.scan_begin_arg;
...
#[no_mangle]
unsafe extern "C" fn cb_pcidas_interrupt(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
// handle analog input interrupts
    ...
    const struct cb_pcidas_board *board = core::ptr::null_mut();
...
// Calibration subdevice - 8800 caldac
    ...
    let mut val: c_uint = s.maxdata / 2;
// write 11-bit channel/value to caldac
...
#[no_mangle]
unsafe extern "C" fn cb_pcidas_detach(dev: *mut comedi_device) {
