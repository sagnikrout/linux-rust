...
//! Source: drivers/comedi/drivers/ni_65xx.c
#![no_std]
...
pub const NI_65XX_ID_REG: c_uint = 0x00;
pub const NI_65XX_CLR_REG: c_uint = 0x01;

pub const NI_65XX_STATUS_REG: c_uint = 0x02;

pub const NI_65XX_CTRL_REG: c_uint = 0x03;

pub const NI_65XX_REV_REG: c_uint = 0x04 /* 32-bit */;
pub const NI_65XX_FILTER_REG: c_uint = 0x08 /* 32-bit */;
pub const NI_65XX_RTSI_ROUTE_REG: c_uint = 0x0c /* 16-bit */;
pub const NI_65XX_RTSI_EDGE_REG: c_uint = 0x0e /* 16-bit */;
pub const NI_65XX_RTSI_WDOG_REG: c_uint = 0x10 /* 16-bit */;
pub const NI_65XX_RTSI_TRIG_REG: c_uint = 0x12 /* 16-bit */;
pub const NI_65XX_AUTO_CLK_SEL_REG: c_uint = 0x14 /* PXI-6528 only */;

pub const NI_65XX_WDOG_CTRL_REG: c_uint = 0x15;

pub const NI_65XX_RTSI_CFG_REG: c_uint = 0x16;

pub const NI_65XX_WDOG_STATUS_REG: c_uint = 0x17;

pub const NI_65XX_WDOG_INTERVAL_REG: c_uint = 0x18 /* 32-bit */;
// Recurring port registers (8-bit)

pub const NI_65XX_IO_SEL_OUTPUT: c_int = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ni_65xx_board {
    pub name: *const c_char,
    pub num_dio_ports: c_uint,
    pub num_di_ports: c_uint,
    pub num_do_ports: c_uint,
    pub legacy_invert:1: c_uint,
}
...
#[no_mangle]
unsafe extern "C" fn ni_65xx_num_ports(dev: *mut comedi_device) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn ni_65xx_disable_input_filters(dev: *mut comedi_device) {
    static void ni_65xx_disable_input_filters(struct comedi_device *dev)
    {
    let mut num_ports: c_uint = ni_65xx_num_ports(dev);
    int i;
...
// updates edge detection for base_chan to base_chan+31
    ...
    let mut bitshift: c_int = (int)(NI_65XX_PORT_TO_CHAN(port) - base_chan);
...
#[no_mangle]
unsafe extern "C" fn ni_65xx_disable_edge_detection(dev: *mut comedi_device) {
...
// clear edge detection for channels 64 to 95
    ...
    unsigned int *data)
    {
    let mut base_port: c_ulong = (unsigned long)s.private;
    let mut chan: c_uint = CR_CHAN(insn.chanspec);
    let mut chan_mask: c_uint = NI_65XX_CHAN_TO_MASK(chan);
    let mut port: c_uint = base_port + NI_65XX_CHAN_TO_PORT(chan);
...
//
    ...
    {
    let mut base_port: c_ulong = (unsigned long)s.private;
    let mut base_chan: c_uint = CR_CHAN(insn.chanspec);
    let mut last_port_offset: c_int = NI_65XX_CHAN_TO_PORT(s.n_chan - 1);
    let mut read_bits: c_uint = 0;
    int port_offset;
    for (port_offset = NI_65XX_CHAN_TO_PORT(base_chan);
    port_offset <= last_port_offset; port_offset++) {
    let mut port: c_uint = base_port + port_offset;
    let mut base_port_channel: c_int = NI_65XX_PORT_TO_CHAN(port_offset);
    unsigned int port_mask, port_data, bits;
    let mut bitshift: c_int = base_port_channel - base_chan;
    if (bitshift >= 32)
    break;
    port_mask = data[0];
    port_data = data[1];
...
#[no_mangle]
unsafe extern "C" fn ni_65xx_interrupt(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ni_65xx_mite_init(pcidev: *mut pci_dev) -> c_int {
...
// finished with MITE registers
    ...
    const struct ni_65xx_board *board = core::ptr::null_mut();
    struct comedi_subdevice *s;
    unsigned int i;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn ni_65xx_detach(dev: *mut comedi_device) {
