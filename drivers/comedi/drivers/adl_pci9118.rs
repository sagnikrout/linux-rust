...
//! Source: drivers/comedi/drivers/adl_pci9118.c
#![no_std]
...
//
pub const PCI9118_TIMER_BASE: c_uint = 0x00;
pub const PCI9118_AI_FIFO_REG: c_uint = 0x10;

pub const PCI9118_AI_STATUS_REG: c_uint = 0x18;

pub const PCI9118_AI_CTRL_REG: c_uint = 0x18;

pub const PCI9118_DIO_REG: c_uint = 0x1c;
pub const PCI9118_SOFTTRG_REG: c_uint = 0x20;
pub const PCI9118_AI_CHANLIST_REG: c_uint = 0x24;

pub const PCI9118_AI_BURST_NUM_REG: c_uint = 0x28;
pub const PCI9118_AI_AUTOSCAN_MODE_REG: c_uint = 0x2c;
pub const PCI9118_AI_CFG_REG: c_uint = 0x30;

pub const PCI9118_FIFO_RESET_REG: c_uint = 0x34;
pub const PCI9118_INT_CTRL_REG: c_uint = 0x38;

pub const START_AI_EXT: c_uint = 0x01	/* start measure on external trigger */;
pub const STOP_AI_EXT: c_uint = 0x02	/* stop measure on external trigger */;
pub const STOP_AI_INT: c_uint = 0x08	/* stop measure on internal trigger */;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pci9118_boardinfo {
    pub name: *const c_char,
    pub ai_is_16bit:1: c_uint,
    pub is_hg:1: c_uint,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pci9118_dmabuf {
    pub /: *mut *mut *mut unsigned short virt; / virtual address of buffer,
    pub /: *mut *mut dma_addr_t hw; / hardware (bus) address of buffer,
    pub /: *mut *mut unsigned int size; / size of dma buffer in bytes,
    pub /: *mut *mut unsigned int use_size; / which size we may now use for transfer,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct pci9118_private {
    pub /: *mut *mut unsigned long iobase_a; / base+size for AMCC chip,
    pub master:1: c_uint,
    pub dma_doublebuf:1: c_uint,
    pub ai_neverending:1: c_uint,
    pub usedma:1: c_uint,
    pub usemux:1: c_uint,
    pub ai_ctrl: c_uchar,
    pub int_ctrl: c_uchar,
    pub ai_cfg: c_uchar,
    pub /: *mut *mut unsigned int ai_do; / what do AI? 0=nothing, 1 to 4 mode,
    pub /*: *mut unsigned int ai_n_realscanlen;,
// what we must transfer for one
// outgoing scan include front/back adds
//
    pub /: *mut *mut unsigned int ai_act_dmapos; / position in actual real stream,
    pub /*: *mut unsigned int ai_add_front;,
// how many channels we must add
// before scan to satisfy S&H?
//
    pub /*: *mut unsigned int ai_add_back;,
// how many channels we must add
// before scan to satisfy DMA?
//
    pub ai_flags: c_uint,
    pub /*: *mut char ai12_startstop;,
// measure can start/stop
// on external trigger
//
    pub /: *mut *mut unsigned int dma_actbuf; / which buffer is used now,
    pub dmabuf: [pci9118_dmabuf; 2],
    pub /*: *mut int softsshdelay;,
// >0 use software S&H,
// number is requested delay in ns
//
    pub /*: *mut unsigned char softsshsample;,
// polarity of S&H signal
// in sample state
//
    pub /*: *mut unsigned char softsshhold;,
// polarity of S&H signal
// in hold state
//
    pub ai_ns_min: c_uint,
}

#[no_mangle]
unsafe extern "C" fn pci9118_amcc_setup_dma(dev: *mut comedi_device, buf: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn pci9118_amcc_dma_ena(dev: *mut comedi_device, enable: bool) {
...
#[no_mangle]
unsafe extern "C" fn pci9118_amcc_int_ena(dev: *mut comedi_device, enable: bool) {
...
#[no_mangle]
unsafe extern "C" fn pci9118_ai_reset_fifo(dev: *mut comedi_device) {
...
// single channel scans are always ok
    ...
    {
    struct pci9118_private *devpriv = dev.private;
    let mut chan0: c_uint = CR_CHAN(chanlist[0]);
    let mut range0: c_uint = CR_RANGE(chanlist[0]);
    let mut aref0: c_uint = CR_AREF(chanlist[0]);
    let mut ssh: c_uint = 0x00;
    unsigned int val;
    int i;
//
...
// store chanlist
    for (i = 0; i < n_chan; i++) {
    let mut chan: c_uint = CR_CHAN(chanlist[i]);
    let mut range: c_uint = CR_RANGE(chanlist[i]);
...
// udelay(100); important delay, or first sample will be crippled
    ...
    let mut start_pos: c_uint = devpriv.ai_add_front;
    let mut stop_pos: c_uint = start_pos + cmd.chanlist_len;
    let mut span_len: c_uint = stop_pos + devpriv.ai_add_back;
    let mut dma_pos: c_uint = devpriv.ai_act_dmapos;
...
#[no_mangle]
unsafe extern "C" fn pci9118_exttrg_enable(dev: *mut comedi_device, enable: bool) {
...
#[no_mangle]
unsafe extern "C" fn pci9118_start_pacer(dev: *mut comedi_device, mode: c_int) {
...
// reset scan queue
    ...
    let mut num_samples: c_uint = comedi_bytes_to_samples(s, num_bytes);
    unsigned int i;
    __be16 *barray = data;
    ...
    let mut n_all: c_uint = comedi_bytes_to_samples(s, dmabuf.use_size);
...
#[no_mangle]
unsafe extern "C" fn pci9118_interrupt(irq: c_int, d: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn pci9118_ai_cmd_start(dev: *mut comedi_device) {
    ...
    let mut dmalen0: c_uint = dmabuf0.size;
    let mut dmalen1: c_uint = dmabuf1.size;
...
#[no_mangle]
unsafe extern "C" fn pci9118_ai_cmd(dev: *mut comedi_device, s: *mut comedi_subdevice) -> c_int {
    static int pci9118_ai_cmd(struct comedi_device *dev, struct comedi_subdevice *s)
    {
    struct pci9118_private *devpriv = dev.private;
    struct comedi_8254 *pacer = dev.pacer;
    struct comedi_cmd *cmd = &s.async.cmd;
    let mut addchans: c_uint = 0;
...
// start async command now or wait for internal trigger
    ...
    pci9118_ai_cmd_start(dev);
#[no_mangle]
pub unsafe extern "C" fn if(TRIG_INT: cmd->start_src ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn pci9118_ai_start_conv(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn pci9118_reset(dev: *mut comedi_device) {
...
// reset analog outputs to 0V
    ...
    let mut bus: c_int = it.options[0];
    let mut slot: c_int = it.options[1];
...
#[no_mangle]
unsafe extern "C" fn pci9118_alloc_dma(dev: *mut comedi_device) {
...
#[no_mangle]
unsafe extern "C" fn pci9118_free_dma(dev: *mut comedi_device) {
...
// get the current state of the digital outputs
    ...
    {
    struct pci_dev *pcidev = comedi_to_pci_dev(dev);
    const struct pci9118_boardinfo *board = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn pci9118_detach(dev: *mut comedi_device) {
