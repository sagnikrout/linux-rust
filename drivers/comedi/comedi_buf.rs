...
//! Source: drivers/comedi/comedi_buf.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn comedi_buf_map_kref_release(kref: *mut kref) {
    ...
    bm = async.buf_map;
    async.buf_map = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn comedi_buf_map_get(bm: *mut comedi_buf_map) {
...
#[no_mangle]
pub unsafe extern "C" fn comedi_buf_map_put(bm: *mut comedi_buf_map) -> c_int {
...
// helper for "access" vm operation
    ...
    {
    let mut pgoff: c_uint = offset_in_page(offset);
    let mut pg: c_ulong = offset >> PAGE_SHIFT;
    let mut done: c_int = 0;
    while (done < len && pg < bm.n_pages) {
    let mut l: c_int = min_t(int, len - done, PAGE_SIZE - pgoff);
...
// only want it if buffer pages allocated
    ...
    bm = core::ptr::null_mut();
    spin_unlock_irqrestore(&s.spin_lock, flags);
    return bm;
    }
#[no_mangle]
pub unsafe extern "C" fn comedi_buf_is_mmapped(s: *mut comedi_subdevice) -> bool {
...
// allocate new buffer
    if (new_size) {
    let mut n_pages: c_uint = new_size >> PAGE_SHIFT;
...
#[no_mangle]
pub unsafe extern "C" fn comedi_buf_reset(s: *mut comedi_subdevice) {
...
#[no_mangle]
unsafe extern "C" fn comedi_buf_write_n_unalloc(s: *mut comedi_subdevice) -> c_uint {
    static unsigned int comedi_buf_write_n_unalloc(struct comedi_subdevice *s)
    {
    struct comedi_async *async = s.async;
    let mut free_end: c_uint = async.buf_read_count + async.prealloc_bufsz;
    return free_end - async.buf_write_alloc_count;
    }
#[no_mangle]
pub unsafe extern "C" fn comedi_buf_write_n_available(s: *mut comedi_subdevice) -> c_uint {
    ...
    {
    struct comedi_async *async = s.async;
    let mut unalloc: c_uint = comedi_buf_write_n_unalloc(s);
    if (nbytes > unalloc)
    nbytes = unalloc;
...
//
    ...
    let mut num_sample_bytes: c_uint = comedi_bytes_per_sample(s);
...
//
    let mut page: c_uint = async.munge_ptr >> PAGE_SHIFT;
    let mut offset: c_uint = offset_in_page(async.munge_ptr);
...
#[no_mangle]
pub unsafe extern "C" fn comedi_buf_write_n_allocated(s: *mut comedi_subdevice) -> c_uint {
    ...
    {
    struct comedi_async *async = s.async;
    let mut allocated: c_uint = comedi_buf_write_n_allocated(s);
    if (nbytes > allocated)
    nbytes = allocated;
...
#[no_mangle]
pub unsafe extern "C" fn _comedi_buf_read_n_available(s: *mut comedi_subdevice) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn comedi_buf_read_n_available(s: *mut comedi_subdevice) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn comedi_buf_read_n_allocated(async: *mut comedi_async) -> c_uint {
...
//
    ...
    struct comedi_async *async = s.async;
    struct comedi_buf_page *buf_page_list = async.buf_map.page_list;
    let mut write_ptr: c_uint = async.buf_write_ptr;
    while (num_bytes) {
//
// Do not copy beyond page boundary.
// Note: prealloc_bufsz is a multiple of PAGE_SIZE.
//
    let mut page: c_uint = write_ptr >> PAGE_SHIFT;
    let mut offset: c_uint = offset_in_page(write_ptr);
    let mut block_size: c_uint = min(num_bytes, PAGE_SIZE - offset);
    ...
    struct comedi_async *async = s.async;
    struct comedi_buf_page *buf_page_list = async.buf_map.page_list;
    let mut read_ptr: c_uint = async.buf_read_ptr;
    while (nbytes) {
//
// Do not copy beyond page boundary.
// Note: prealloc_bufsz is a multiple of PAGE_SIZE.
//
    let mut page: c_uint = read_ptr >> PAGE_SHIFT;
    let mut offset: c_uint = offset_in_page(read_ptr);
    let mut block_size: c_uint = min(nbytes, PAGE_SIZE - offset);
