...
//! Source: drivers/pps/clients/pps_parport.c
#![no_std]
...
// module parameters
pub const CLEAR_WAIT_MAX: c_int = 100;
pub const CLEAR_WAIT_MAX_ERRORS: c_int = 5;
    let mut clear_wait: static unsigned int = 100;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pps_client_pp {
    pub /: *mut *mut *mut pardevice pardev; / parport device,
    pub /: *mut *mut *mut pps_device pps; / PPS device,
    pub /: *mut *mut unsigned int cw; / port clear timeout,
    pub /: *mut *mut unsigned int cw_err; / number of timeouts,
    pub /: *mut *mut int index; / device number,
}

#[no_mangle]
pub unsafe extern "C" fn signal_is_set(port: *mut parport) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn parport_irq(handle: *mut c_void) {
...
// fire assert event
    ...
    PPS_CAPTUREASSERT, core::ptr::null_mut());
    return;
    out_both:
// fire assert event
    pps_event(dev.pps, &ts_assert,
...
// fire clear event
    ...
    PPS_CAPTURECLEAR, core::ptr::null_mut());
    return;
    }
#[no_mangle]
unsafe extern "C" fn parport_attach(port: *mut parport) {
...
#[no_mangle]
unsafe extern "C" fn parport_detach(port: *mut parport) {
