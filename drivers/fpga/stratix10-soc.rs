...
//! Source: drivers/fpga/stratix10-soc.c
#![no_std]
...
// design.
//
pub const NUM_SVC_BUFS: c_int = 4;

// Indicates buffer is in use if set
pub const SVC_BUF_LOCK: c_int = 0;
...
//
#[no_mangle]
unsafe extern "C" fn s10_free_buffers(mgr: *mut fpga_manager) -> bool {
    static bool s10_free_buffers(struct fpga_manager *mgr)
    {
    struct s10_priv *priv = mgr.priv;
    let mut num_free: c_uint = 0;
    uint i;
    ...
    priv.svc_bufs[i].buf);
    priv.svc_bufs[i].buf = core::ptr::null_mut();
    num_free++;
    }
    }
    let mut num_free: return = = NUM_SVC_BUFS;
    }
//
// Returns count of how many buffers are not in use.
//
#[no_mangle]
unsafe extern "C" fn s10_free_buffer_count(mgr: *mut fpga_manager) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn s10_unlock_bufs(priv: *mut s10_priv, kaddr: *mut c_void) {
...
//
    ...
    u32 status;
    int i;
    WARN_ONCE(!data, "%s: stratix10_svc_rc_data = core::ptr::null_mut()", __func__);
    status = data.status;
//
...
//
#[no_mangle]
unsafe extern "C" fn s10_send_buf(mgr: *mut fpga_manager, buf: *const c_char, count: usize) -> c_int {
...
//
    ...
    do {
    reinit_completion(&priv.status_return_completion);
    ret = s10_svc_send_msg(priv, COMMAND_RECONFIG_STATUS, core::ptr::null_mut(), 0);
    if (ret < 0)
    break;
...
#[no_mangle]
unsafe extern "C" fn s10_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s10_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn s10_init() -> int __init {
    static int __init s10_init(void)
    {
    struct device_node *fw_np;
    struct device_node *np;
    int ret;
    fw_np = of_find_node_by_name(core::ptr::null_mut(), "svc");
    if (!fw_np)
    return -ENODEV;
    of_node_get(fw_np);
    ...
    of_node_put(np);
    ret = of_platform_populate(fw_np, s10_of_match, core::ptr::null_mut(), core::ptr::null_mut());
    of_node_put(fw_np);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn s10_exit() -> void __exit {
