...
//! Source: drivers/fpga/altera-ps-spi.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct altera_ps_data {
    pub devtype: enum altera_ps_devtype,
    pub status_wait_min_us: c_int,
    pub status_wait_max_us: c_int,
    pub t_cfg_us: c_int,
    pub t_st2ck_us: c_int,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct altera_ps_conf {
    pub config: *mut gpio_desc,
    pub confd: *mut gpio_desc,
    pub status: *mut gpio_desc,
    pub spi: *mut spi_device,
    pub data: *const altera_ps_data,
    pub info_flags: u32,
...
#[no_mangle]
unsafe extern "C" fn altera_ps_state(mgr: *mut fpga_manager) -> enum fpga_mgr_states {
...
#[no_mangle]
pub unsafe extern "C" fn altera_ps_delay(delay_us: c_int) {
...
#[no_mangle]
unsafe extern "C" fn rev_buf(buf: *mut c_char, len: usize) {
    static void rev_buf(char *buf, size_t len)
    {
    u32 *fw32 = (u32 *)buf;
    let mut extra_bytes: usize = (len & 0x03);
...
// buf = bitrev8(*buf);
    ...
    int ret;
    let mut stride: usize = min_t(size_t, fw_data_end - fw_data, SZ_4K);
...
#[no_mangle]
unsafe extern "C" fn altera_ps_probe(spi: *mut spi_device) -> c_int {
