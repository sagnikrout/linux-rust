...
//! Source: drivers/i3c/master/mipi-i3c-hci/pio.c
#![no_std]
...
pub const PIO_COMMAND_QUEUE_PORT: c_uint = 0x00;
pub const PIO_RESPONSE_QUEUE_PORT: c_uint = 0x04;
pub const PIO_XFER_DATA_PORT: c_uint = 0x08;
pub const PIO_IBI_PORT: c_uint = 0x0c;
pub const PIO_QUEUE_THLD_CTRL: c_uint = 0x10;

pub const PIO_DATA_BUFFER_THLD_CTRL: c_uint = 0x14;

pub const PIO_QUEUE_SIZE: c_uint = 0x18;

pub const PIO_ALT_QUEUE_SIZE: c_uint = 0x1C;

pub const PIO_INTR_STATUS: c_uint = 0x20;
pub const PIO_INTR_STATUS_ENABLE: c_uint = 0x24;
pub const PIO_INTR_SIGNAL_ENABLE: c_uint = 0x28;
pub const PIO_INTR_FORCE: c_uint = 0x2c;

pub const PIO_CONTROL: c_uint = 0x30;

pub const PIO_QUEUE_CUR_STATUS: c_uint = 0x38;

pub const PIO_DATA_BUFFER_CUR_STATUS: c_uint = 0x3c;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hci_pio_dev_ibi_data {
    pub pool: *mut i3c_generic_ibi_pool,
    pub max_len: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct hci_pio_ibi_data {
    pub slot: *mut i3c_ibi_slot,
    pub data_ptr: *mut c_void,
    pub addr: c_uint,
    pub seg_cnt: unsigned int seg_len,,
    pub max_len: c_uint,
    pub last_seg: bool,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hci_pio_data {
    pub xfer_queue: *mut *mut hci_xfer curr_xfer,,
    pub rx_queue: *mut *mut hci_xfer curr_rx,,
    pub tx_queue: *mut *mut hci_xfer curr_tx,,
    pub resp_queue: *mut *mut hci_xfer curr_resp,,
    pub ibi: hci_pio_ibi_data,
    pub tx_thresh_size: unsigned int rx_thresh_size,,
    pub max_ibi_thresh: c_uint,
    pub reg_queue_thresh: u32,
    pub enabled_irqs: u32,
}

#[no_mangle]
unsafe extern "C" fn __hci_pio_init(hci: *mut i3c_hci, size_val_ptr: *mut u32) {
...
// Adjust actual IBI queue size based on v1.2 ALT_QUEUE_SIZE
    ...
    let mut alt_val: u32 = pio_reg_read(ALT_QUEUE_SIZE);
...
// MIPI I3C HCI v1.2 requires explicitly enabling and starting PIO queues
    if (hci_version_at_least(hci, 1, 2)) {
    let mut ctl_val: u32 = pio_reg_read(CONTROL);
...
#[no_mangle]
unsafe extern "C" fn hci_pio_suspend(hci: *mut i3c_hci) {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_resume(hci: *mut i3c_hci) {
    static void hci_pio_resume(struct i3c_hci *hci)
    {
    __hci_pio_init(hci, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn hci_pio_init(hci: *mut i3c_hci) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_cleanup(hci: *mut i3c_hci) {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_write_cmd(hci: *mut i3c_hci, xfer: *mut hci_xfer) {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_do_rx(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
...
// trailing data is retrieved upon response reception
    ...
    p += (xfer.data_len - xfer.data_left) / 4;
    if (count >= 4) {
    let mut nr_words: c_uint = count / 4;
// extract data from FIFO
    xfer.data_left -= nr_words * 4;
    dev_dbg(&hci.master.dev, "now %d left %d",
    nr_words * 4, xfer.data_left);
...
//
    u8 *p_byte = (u8 *)p;
    let mut data: u32 = pio_reg_read(XFER_DATA_PORT);
...
#[no_mangle]
unsafe extern "C" fn hci_pio_do_tx(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_process_rx(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_process_tx(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_queue_data(hci: *mut i3c_hci, pio: *mut hci_pio_data) {
...
//
    ...
    last_word = ( u32) cpu_to_le32(last_word);
...
// p_byte++ = last_word;
    ...
    u32 status);
#[no_mangle]
unsafe extern "C" fn hci_pio_process_resp(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
    ...
    let mut resp: u32 = pio_reg_read(RESPONSE_QUEUE_PORT);
    let mut tid: c_uint = RESP_TID(resp);
...
#[no_mangle]
unsafe extern "C" fn hci_pio_queue_resp(hci: *mut i3c_hci, pio: *mut hci_pio_data) {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_process_cmd(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_queue_xfer(hci: *mut i3c_hci, xfer: *mut hci_xfer, n: c_int) -> c_int {
...
// link xfer instances together and initialize data count
    for (i = 0; i < n; i++) {
    xfer[i].next_xfer = (i + 1 < n) ? &xfer[i + 1] : core::ptr::null_mut();
    xfer[i].next_data = core::ptr::null_mut();
    xfer[i].next_resp = core::ptr::null_mut();
...
//
    ...
    pio.curr_xfer = pio.curr_rx = pio.curr_tx = pio.curr_resp = core::ptr::null_mut();
    return true;
    }
#[no_mangle]
unsafe extern "C" fn hci_pio_dequeue_xfer(hci: *mut i3c_hci, xfer: *mut hci_xfer, n: c_int) -> bool {
...
// dump states on programming errors
    ...
    let mut queue: u32 = pio_reg_read(QUEUE_CUR_STATUS);
    let mut data: u32 = pio_reg_read(DATA_BUFFER_CUR_STATUS);
    dev_err(&hci.master.dev,
...
// MIPI I3C HCI v1.2 requires explicitly restarting PIO queues after error/abort
    ...
    {
    let mut regval: u32 = pio.reg_queue_thresh;
...
#[no_mangle]
unsafe extern "C" fn hci_pio_prep_new_ibi(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_free_ibi_slot(hci: *mut i3c_hci, pio: *mut hci_pio_data) {
    ...
    ibi.slot = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn hci_pio_process_ibi(hci: *mut i3c_hci, pio: *mut hci_pio_data) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hci_pio_free_ibi(hci: *mut i3c_hci, dev: *mut i3c_dev_desc) {
    ...
    dev_data.ibi_data = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn hci_pio_irq_handler(hci: *mut i3c_hci) -> bool {
