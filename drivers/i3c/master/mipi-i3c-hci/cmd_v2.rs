...
//! Source: drivers/i3c/master/mipi-i3c-hci/cmd_v2.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn get_i3c_rate_idx(hci: *mut i3c_hci) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn get_i2c_rate_idx(hci: *mut i3c_hci) -> c_uint {
...
// we consumed all the data with the cmd descriptor
    ...
    {
    let mut mode: c_uint = XFERMODE_IDX_I3C_SDR;
    let mut rate: c_uint = get_i3c_rate_idx(hci);
    u8 *data = xfer.data;
    let mut data_len: c_uint = xfer.data_len;
    let mut rnw: bool = xfer.rnw;
...
// we consumed all the data with the cmd descriptor
    ...
    let mut mode: c_uint = XFERMODE_IDX_I3C_SDR;
    let mut rate: c_uint = get_i3c_rate_idx(hci);
    let mut addr: u8 = dev.info.dyn_addr;
    ...
    struct hci_xfer *xfer)
    {
    let mut mode: c_uint = XFERMODE_IDX_I2C;
    let mut rate: c_uint = get_i2c_rate_idx(hci);
    let mut addr: u8 = dev.addr;
    hci_cmd_v2_prep_private_xfer(hci, xfer, addr, mode, rate);
    }
#[no_mangle]
unsafe extern "C" fn hci_cmd_v2_daa(hci: *mut i3c_hci) -> c_int {
