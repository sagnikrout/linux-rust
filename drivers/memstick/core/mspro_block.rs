...
//! Source: drivers/memstick/core/mspro_block.c
#![no_std]
...
pub const MSPRO_BLOCK_MAX_SEGS: c_int = 32;

pub const MSPRO_BLOCK_SIGNATURE: c_uint = 0xa5c3;
pub const MSPRO_BLOCK_MAX_ATTRIBUTES: c_int = 41;
pub const MSPRO_BLOCK_PART_SHIFT: c_int = 3;
    enum {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mspro_attribute {
    pub signature: __be16,
    pub version: c_ushort,
    pub count: c_uchar,
    pub reserved: [c_uchar; 11],
    pub entries: [mspro_attr_entry; ],
    pub __attribute__((packed)): },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mspro_sys_info {
    pub class: c_uchar,
    pub reserved0: c_uchar,
    pub block_size: __be16,
    pub block_count: __be16,
    pub user_block_count: __be16,
    pub page_size: __be16,
    pub reserved1: [c_uchar; 2],
    pub assembly_date: [c_uchar; 8],
    pub serial_number: __be32,
    pub assembly_maker_code: c_uchar,
    pub assembly_model_code: [c_uchar; 3],
    pub memory_maker_code: __be16,
    pub memory_model_code: __be16,
    pub reserved2: [c_uchar; 4],
    pub vcc: c_uchar,
    pub vpp: c_uchar,
    pub controller_number: __be16,
    pub controller_function: __be16,
    pub start_sector: __be16,
    pub unit_size: __be16,
    pub ms_sub_class: c_uchar,
    pub reserved3: [c_uchar; 4],
    pub interface_type: c_uchar,
    pub controller_code: __be16,
    pub format_type: c_uchar,
    pub reserved4: c_uchar,
    pub device_type: c_uchar,
    pub reserved5: [c_uchar; 7],
    pub mspro_id: [c_uchar; 16],
    pub reserved6: [c_uchar; 16],
    pub __attribute__((packed)): },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mspro_mbr {
    pub boot_partition: c_uchar,
    pub start_head: c_uchar,
    pub start_sector: c_uchar,
    pub start_cylinder: c_uchar,
    pub partition_type: c_uchar,
    pub end_head: c_uchar,
    pub end_sector: c_uchar,
    pub end_cylinder: c_uchar,
    pub start_sectors: c_uint,
    pub sectors_per_partition: c_uint,
    pub __attribute__((packed)): },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mspro_specfile {
    pub name: [c_char; 8],
    pub ext: [c_char; 3],
    pub attr: c_uchar,
    pub reserved: [c_uchar; 10],
    pub time: c_ushort,
    pub date: c_ushort,
    pub cluster: c_ushort,
    pub size: c_uint,
    pub __attribute__((packed)): },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mspro_devinfo {
    pub cylinders: __be16,
    pub heads: __be16,
    pub bytes_per_track: __be16,
    pub bytes_per_sector: __be16,
    pub sectors_per_track: __be16,
    pub reserved: [c_uchar; 6],
    pub __attribute__((packed)): },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mspro_block_data {
    pub card: *mut memstick_dev,
    pub caps: c_uint,
    pub disk: *mut gendisk,
    pub queue: *mut request_queue,
    pub block_req: *mut request,
    pub tag_set: blk_mq_tag_set,
    pub q_lock: spinlock_t,
    pub page_size: c_ushort,
    pub cylinders: c_ushort,
    pub heads: c_ushort,
    pub sectors_per_track: c_ushort,
    pub system: c_uchar,
    unsigned char         read_only:1,
    eject:1,
    data_dir:1,
    pub transfer_cmd: c_uchar,
...
//
    ...
    pub length): u64 offset, size_t,
    pub attr_group: attribute_group,
    pub req_sg: [scatterlist; MSPRO_BLOCK_MAX_SEGS],
    pub seg_count: c_uint,
    pub current_seg: c_uint,
...
// Block device
#[no_mangle]
unsafe extern "C" fn mspro_block_bd_free_disk(disk: *mut gendisk) {
    static void mspro_block_bd_free_disk(struct gendisk *disk)
    {
    struct mspro_block_data *msb = disk.private_data;
    let mut disk_id: c_int = MINOR(disk_devt(disk)) >> MSPRO_BLOCK_PART_SHIFT;
...
// Information
    ...
    let mut rc: isize = 0;
    let mut date_tz: c_int = 0, date_tz_f = 0;
...
#[no_mangle]
unsafe extern "C" fn mspro_block_attr_show(tag: c_uchar) -> sysfs_show_t {
...
// mrq = &card->current_mrq;
    ...
    (*mrq).error = -EFAULT;
#[no_mangle]
pub unsafe extern "C" fn if(MEMSTICK_INT_CED): *mut *mut !((mrq)->data[0] &) -> else {
    ...
    {
    struct mspro_block_data *msb = memstick_get_drvdata(card);
    let mut t_val: c_uchar = 0;
    let mut t_sg: scatterlist = { 0 };
    size_t t_offset;
    if ((*mrq).error)
    return mspro_block_complete_req(card, (*mrq).error);
    switch ((*mrq).tpc) {
    ...
    case MS_TPC_SET_CMD:
    t_val = (*mrq).int_reg;
    memstick_init_req(*mrq, MS_TPC_GET_INT, core::ptr::null_mut(), 1);
...
// Data transfer
#[no_mangle]
unsafe extern "C" fn mspro_block_issue_req(card: *mut memstick_dev) -> c_int {
    ...
    if (!msb.seg_count) {
    let mut bytes: c_uint = blk_rq_cur_bytes(msb.block_req);
    bool chunk;
    ...
    BLK_STS_RESOURCE);
    msb.block_req = core::ptr::null_mut();
    return -EAGAIN;
    }
...
#[no_mangle]
unsafe extern "C" fn mspro_block_complete_req(card: *mut memstick_dev, error: c_int) -> c_int {
    static int mspro_block_complete_req(struct memstick_dev *card, int error)
    {
    struct mspro_block_data *msb = memstick_get_drvdata(card);
    int cnt;
    bool chunk;
    let mut t_len: c_uint = 0;
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn mspro_block_stop(card: *mut memstick_dev) {
...
#[no_mangle]
unsafe extern "C" fn mspro_block_start(card: *mut memstick_dev) {
...
// Initialization
#[no_mangle]
unsafe extern "C" fn mspro_block_wait_for_ced(card: *mut memstick_dev) -> c_int {
    static int mspro_block_wait_for_ced(struct memstick_dev *card)
    {
    struct mspro_block_data *msb = memstick_get_drvdata(card);
    card.next_request = h_mspro_block_req_init;
    msb.mrq_handler = h_mspro_block_wait_for_ced;
    memstick_init_req(&card.current_mrq, MS_TPC_GET_INT, core::ptr::null_mut(), 1);
    memstick_new_req(card.host);
    wait_for_completion(&card.mrq_complete);
...
#[no_mangle]
unsafe extern "C" fn mspro_block_switch_interface(card: *mut memstick_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mspro_block_read_attributes(card: *mut memstick_dev) -> c_int {
    static int mspro_block_read_attributes(struct memstick_dev *card)
    {
    struct mspro_block_data *msb = memstick_get_drvdata(card);
    struct mspro_attribute *attr = core::ptr::null_mut();
    struct mspro_sys_attr *s_attr = core::ptr::null_mut();
    unsigned char *buffer = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn mspro_block_init_card(card: *mut memstick_dev) -> c_int {
    ...
    memstick_init_req(&card.current_mrq, MS_TPC_READ_REG, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn mspro_block_init_disk(card: *mut memstick_dev) -> c_int {
    ...
    struct mspro_devinfo *dev_info = core::ptr::null_mut();
    struct mspro_sys_info *sys_info = core::ptr::null_mut();
    struct mspro_sys_attr *s_attr = core::ptr::null_mut();
    int rc, disk_id;
...
#[no_mangle]
pub unsafe extern "C" fn if(MSPRO_BLOCK_ID_SYSINFO: s_attr->id ==) -> else {
    ...
    rc = device_add_disk(&card.dev, msb.disk, core::ptr::null_mut());
    if (rc)
...
#[no_mangle]
unsafe extern "C" fn mspro_block_data_clear(msb: *mut mspro_block_data) {
    ...
    msb.card = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn mspro_block_check_card(card: *mut memstick_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mspro_block_probe(card: *mut memstick_dev) -> c_int {
    ...
    out_free:
    memstick_set_drvdata(card, core::ptr::null_mut());
    mspro_block_data_clear(msb);
    kfree(msb);
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn mspro_block_remove(card: *mut memstick_dev) {
...
#[no_mangle]
unsafe extern "C" fn mspro_block_suspend(card: *mut memstick_dev, state: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mspro_block_resume(card: *mut memstick_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mspro_block_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn mspro_block_exit() -> void __exit {
