...
//! Source: drivers/cdx/controller/mcdi.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_rpc_timeout(cdx: *mut cdx_mcdi, cmd: c_uint) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_flushed(mcdi: *mut cdx_mcdi_iface, ignore_cleanups: bool) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_wait_for_cleanup(cdx: *mut cdx_mcdi) {
    static void cdx_mcdi_wait_for_cleanup(struct cdx_mcdi *cdx)
    {
...
// MCDI v2
    ...
    hdr_len = 8;
    hdr[0].cdx_u32 |= ( __le32)(cdx_mcdi_payload_csum(hdr, hdr_len, inbuf, inlen) <<
...
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_errno(cdx: *mut cdx_mcdi, mcdi_err: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_cancel_cmd(cdx: *mut cdx_mcdi, cmd: *mut cdx_mcdi_cmd) {
    static void cdx_mcdi_cancel_cmd(struct cdx_mcdi *cdx, struct cdx_mcdi_cmd *cmd)
    {
    struct cdx_mcdi_iface *mcdi = cdx_mcdi_if(cdx);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cdx_mcdi_blocking_data {
    pub ref: kref,
    pub done: bool,
    ...
    pub outlen: usize,
    pub outlen_actual: usize,
}

#[no_mangle]
unsafe extern "C" fn cdx_mcdi_blocking_data_release(ref: *mut kref) {
...
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_get_seq(mcdi: *mut cdx_mcdi_iface, seq: *mut c_uchar) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn cdx_mcdi_process_cmd(cdx: *mut cdx_mcdi, outbuf: *mut cdx_dword, len: c_int) {
...
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_cmd_work(context: *mut work_struct) {
    static void cdx_mcdi_cmd_work(struct work_struct *context)
    {
    struct cdx_mcdi_cmd *cmd =
...
// MC rebooted during command
    ...
    if (!cmd.quiet) {
    let mut err_arg: c_int = 0;
    if (resp_data_len >= MC_CMD_ERR_ARG_OFST + 4) {
    let mut offset: c_int = (resp_hdr_len + MC_CMD_ERR_ARG_OFST) / 4;
...
// free doorbell
    ...
    mcdi.db_held_by = core::ptr::null_mut();
    if (cdx_cmd_cancelled(cmd)) {
    list_del(&cmd.list);
    kref_put(&cmd.ref, cdx_mcdi_cmd_release);
    completed = true;
...
// free sequence number and buffer
    mcdi.seq_held_by[cmd.seq] = core::ptr::null_mut();
...
// inbuf is probably not valid after return, so take a copy
    ...
    return cdx_mcdi_rpc_async_internal(cdx, cmd_item, core::ptr::null_mut());
    }
    static void _cdx_mcdi_display_error(struct cdx_mcdi *cdx, unsigned int cmd,
    size_t inlen, int raw, int arg, int err_no)
    {
...
//
#[no_mangle]
unsafe extern "C" fn cdx_mcdi_mode_fail(cdx: *mut cdx_mcdi, cleanup_list: *mut list_head) {
    static void cdx_mcdi_mode_fail(struct cdx_mcdi *cdx, struct list_head *cleanup_list)
    {
    struct cdx_mcdi_iface *mcdi = cdx_mcdi_if(cdx);
    if (!mcdi)
