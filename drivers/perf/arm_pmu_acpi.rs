...
//! Source: drivers/perf/arm_pmu_acpi.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn arm_pmu_acpi_register_irq(cpu: c_int) -> c_int {
...
//
    return acpi_register_gsi(core::ptr::null_mut(), gsi, ACPI_LEVEL_SENSITIVE, ACPI_ACTIVE_HIGH);
    }
#[no_mangle]
unsafe extern "C" fn arm_pmu_acpi_unregister_irq(cpu: c_int) {
...
//
    ...
    if (!this_gsi)
    return 0;
    irq = acpi_register_gsi(core::ptr::null_mut(), gsi, ACPI_LEVEL_SENSITIVE, ACPI_ACTIVE_HIGH);
    if (irq < 0) {
...
#[no_mangle]
unsafe extern "C" fn arm_spe_parse_gsi(gicc: *mut acpi_madt_generic_interrupt) -> u16 {
...
//
#[no_mangle]
unsafe extern "C" fn arm_spe_acpi_register_device() {
...
#[no_mangle]
pub unsafe extern "C" fn arm_spe_acpi_register_device() {
...
#[no_mangle]
unsafe extern "C" fn arm_trbe_parse_gsi(gicc: *mut acpi_madt_generic_interrupt) -> u16 {
    static u16 arm_trbe_parse_gsi(struct acpi_madt_generic_interrupt *gicc)
    {
    return gicc.trbe_interrupt;
    }
#[no_mangle]
unsafe extern "C" fn arm_trbe_acpi_register_device() {
...
#[no_mangle]
pub unsafe extern "C" fn arm_trbe_acpi_register_device() {
    static inline void arm_trbe_acpi_register_device(void)
    {
    }

#[no_mangle]
unsafe extern "C" fn arm_pmu_acpi_parse_irqs() -> c_int {
...
//
    ...
    {
    let mut cpuid: c_ulong = read_cpuid_id();
    struct arm_pmu *pmu;
    int cpu;
    for_each_possible_cpu(cpu) {
...
//
#[no_mangle]
unsafe extern "C" fn pmu_irq_matches(pmu: *mut arm_pmu, irq: c_int) -> bool {
    ...
    let mut other_irq: c_int = per_cpu(hw_events.irq, cpu);
    if (!other_irq)
    continue;
    if (irq == other_irq)
    continue;
    ...
    unsigned int cpu)
    {
    let mut irq: c_int = per_cpu(pmu_irqs, cpu);
...
//
#[no_mangle]
unsafe extern "C" fn arm_pmu_acpi_cpu_starting(cpu: c_uint) -> c_int {
...
// If we've already probed this CPU, we have nothing to do
    ...
    let mut cpu_cpuid: c_ulong = per_cpu(cpu_data, cpu).reg_midr;
    if (cpu_cpuid == cpuid)
    arm_pmu_acpi_associate_pmu_cpu(pmu, cpu);
    }
    }
#[no_mangle]
pub unsafe extern "C" fn arm_pmu_acpi_probe(init_fn: armpmu_init_fn) -> c_int {
    int arm_pmu_acpi_probe(armpmu_init_fn init_fn)
    {
    let mut pmu_idx: c_int = 0;
    unsigned int cpu;
    int ret;
    ...
    arm_pmu_acpi_cpu_starting, core::ptr::null_mut());
    if (ret)
    return ret;
//
...
#[no_mangle]
unsafe extern "C" fn arm_pmu_acpi_init() -> c_int {
