...
//! Source: drivers/perf/fsl_imx9_ddr_perf.c
#![no_std]
...
pub const PMCFG1: c_uint = 0x00;

pub const PMCFG2: c_uint = 0x04;

pub const PMCFG3: c_uint = 0x08;
pub const PMCFG4: c_uint = 0x0C;
pub const PMCFG5: c_uint = 0x10;
pub const PMCFG6: c_uint = 0x14;

// Global control register affects all counters and takes priority over local control registers
pub const PMGC0: c_uint = 0x40;
...
pub const NUM_COUNTERS: c_int = 11;
pub const CYCLES_COUNTER: c_int = 0;
pub const CYCLES_EVENT_ID: c_int = 0;
...
//
pub const DDR_PERF_AXI_FILTER_V1: c_uint = 0x1;
//
// V2 support 1 read beats and 3 write beats events which corresponding
// respecitively to counter 2-5.
//
pub const DDR_PERF_AXI_FILTER_V2: c_uint = 0x2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct imx_ddr_devtype_data {
    pub /: *const *const *const char identifier; / system PMU identifier for userspace,
    pub /: *mut *mut unsigned int filter_ver; / AXI filter version,
}
...
pub struct ddr_pmu {
    ...
    pub devtype_data: *const imx_ddr_devtype_data,
    pub irq: c_int,
    pub id: c_int,
}
...
#[no_mangle]
pub unsafe extern "C" fn axi_filter_v1(pmu: *mut ddr_pmu) -> bool {
    static inline bool axi_filter_v1(struct ddr_pmu *pmu)
    {
    return pmu.devtype_data.filter_ver == DDR_PERF_AXI_FILTER_V1;
    }
#[no_mangle]
pub unsafe extern "C" fn axi_filter_v2(pmu: *mut ddr_pmu) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct imx9_pmu_events_attr {
...
pub const COUNTER_OFFSET_IN_EVENT: c_int = 8;

    (&((struct imx9_pmu_events_attr[]) {				\
    { .attr = __ATTR(_name, 0444, ddr_pmu_event_show, core::ptr::null_mut()),\
    .id = _id,						\
    .devtype_data = _data, }				\
    })[0].attr.attr)

    DDR_PMU_EVENT_ATTR_COMM(_name, _id, core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn ddr_perf_clear_counter(pmu: *mut ddr_pmu, counter: c_int) {
...
#[no_mangle]
unsafe extern "C" fn ddr_perf_read_counter(pmu: *mut ddr_pmu, counter: c_int) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn ddr_perf_counter_global_config(pmu: *mut ddr_pmu, enable: bool) {
...
#[no_mangle]
unsafe extern "C" fn ddr_perf_alloc_counter(pmu: *mut ddr_pmu, event: c_int, counter: c_int) -> c_int {
    static int ddr_perf_alloc_counter(struct ddr_pmu *pmu, int event, int counter)
    {
    int i;
    if (event == CYCLES_EVENT_ID) {
// Cycles counter is dedicated for cycle event.
    if (pmu.events[CYCLES_COUNTER] == core::ptr::null_mut())
    return CYCLES_COUNTER;
    } else if (counter != 0) {
// Counter specific event use specific counter.
    if (pmu.events[counter] == core::ptr::null_mut())
    return counter;
    } else {
...
unsafe extern "C" fn ddr_perf_probe(pdev: *mut platform_device) -> c_int {
    ...
    core::ptr::null_mut(), ddr_perf_offline_cpu);
    if (ret < 0) {
