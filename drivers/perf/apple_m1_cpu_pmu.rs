...
//! Source: drivers/perf/apple_m1_cpu_pmu.c
#![no_std]
...
pub const M1_PMU_NR_COUNTERS: c_int = 10;
...
// Low level accessors. No synchronisation.
    ...
    return
#[no_mangle]
unsafe extern "C" fn m1_pmu_read_hw_counter(index: c_uint) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_write_hw_counter(val: u64, index: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn __m1_pmu_enable_counter(index: c_uint, en: bool) {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_enable_counter(index: c_uint) {
    static void m1_pmu_enable_counter(unsigned int index)
    {
    __m1_pmu_enable_counter(index, true);
    }
#[no_mangle]
unsafe extern "C" fn m1_pmu_disable_counter(index: c_uint) {
    static void m1_pmu_disable_counter(unsigned int index)
    {
    __m1_pmu_enable_counter(index, false);
    }
#[no_mangle]
unsafe extern "C" fn __m1_pmu_enable_counter_interrupt(index: c_uint, en: bool) {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_enable_counter_interrupt(index: c_uint) {
    static void m1_pmu_enable_counter_interrupt(unsigned int index)
    {
    __m1_pmu_enable_counter_interrupt(index, true);
    }
#[no_mangle]
unsafe extern "C" fn m1_pmu_disable_counter_interrupt(index: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_kernel_in_hyp_mode()) -> else {
    else if (is_kernel_in_hyp_mode())
    sysreg_clear_set_s(SYS_IMP_APL_PMCR1_EL12, clear, set);
    }
#[no_mangle]
unsafe extern "C" fn __m1_pmu_configure_eventsel(index: c_uint, event: u8) {
    static void __m1_pmu_configure_eventsel(unsigned int index, u8 event)
    {
    let mut clear: u64 = 0, set = 0;
    int shift;
//
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_configure_counter(index: c_uint, config_base: c_ulong) {
    static void m1_pmu_configure_counter(unsigned int index, unsigned long config_base)
    {
    let mut kernel: bool = config_base & M1_PMU_CFG_COUNT_KERNEL;
    let mut guest: bool = config_base & M1_PMU_CFG_COUNT_GUEST;
    let mut host: bool = config_base & M1_PMU_CFG_COUNT_HOST;
    let mut user: bool = config_base & M1_PMU_CFG_COUNT_USER;
    let mut evt: u8 = config_base & M1_PMU_CFG_EVENT;
...
// arm_pmu backend
#[no_mangle]
unsafe extern "C" fn m1_pmu_enable_event(event: *mut perf_event) {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_disable_event(event: *mut perf_event) {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_handle_irq(cpu_pmu: *mut arm_pmu) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_read_counter(event: *mut perf_event) -> u64 {
    static u64 m1_pmu_read_counter(struct perf_event *event)
    {
    return m1_pmu_read_hw_counter(event.hw.idx);
    }
#[no_mangle]
unsafe extern "C" fn m1_pmu_write_counter(event: *mut perf_event, value: u64) {
    ...
    {
    let mut evtype: c_ulong = event.hw.config_base & M1_PMU_CFG_EVENT;
    let mut affinity: c_ulong = m1_pmu_event_affinity[evtype];
    int idx;
//
...
#[no_mangle]
unsafe extern "C" fn __m1_pmu_set_mode(mode: u8) {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_start(cpu_pmu: *mut arm_pmu) {
    static void m1_pmu_start(struct arm_pmu *cpu_pmu)
    {
    __m1_pmu_set_mode(PMCR0_IMODE_FIQ);
    }
#[no_mangle]
unsafe extern "C" fn m1_pmu_stop(cpu_pmu: *mut arm_pmu) {
    static void m1_pmu_stop(struct arm_pmu *cpu_pmu)
    {
    __m1_pmu_set_mode(PMCR0_IMODE_OFF);
    }
#[no_mangle]
unsafe extern "C" fn m1_pmu_map_event(event: *mut perf_event) -> c_int {
...
//
    ...
    return armpmu_map_event(event, &m1_pmu_perf_map, core::ptr::null_mut(), M1_PMU_CFG_EVENT);
    }
#[no_mangle]
unsafe extern "C" fn m2_pmu_map_event(event: *mut perf_event) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_map_pmuv3_event(eventsel: c_uint) -> c_int {
    static int m1_pmu_map_pmuv3_event(unsigned int eventsel)
    {
    let mut m1_event: u16 = HW_OP_UNSUPPORTED;
    if (eventsel < ARMV8_PMUV3_MAX_COMMON_EVENTS)
    m1_event = m1_pmu_pmceid_map[eventsel];
    let mut m1_event: return = = HW_OP_UNSUPPORTED ? -EOPNOTSUPP : m1_event;
    }
#[no_mangle]
unsafe extern "C" fn m1_pmu_init_pmceid(pmu: *mut arm_pmu) {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_reset(info: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_init(cpu_pmu: *mut arm_pmu, flags: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(ARMPMU_EVT_63BIT: flags &) -> else {
...
// Device driver gunk
#[no_mangle]
unsafe extern "C" fn m1_pmu_ice_init(cpu_pmu: *mut arm_pmu) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_fire_init(cpu_pmu: *mut arm_pmu) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn m2_pmu_avalanche_init(cpu_pmu: *mut arm_pmu) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn m2_pmu_blizzard_init(cpu_pmu: *mut arm_pmu) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn m1_pmu_device_probe(pdev: *mut platform_device) -> c_int {
    static int m1_pmu_device_probe(struct platform_device *pdev)
    {
    return arm_pmu_device_probe(pdev, m1_pmu_of_device_ids, core::ptr::null_mut());
    }
