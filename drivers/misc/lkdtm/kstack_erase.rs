...
//! Source: drivers/misc/lkdtm/kstack_erase.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn check_stackleak_irqoff() -> void noinstr {
    static void noinstr check_stackleak_irqoff(void)
    {
    let mut task_stack_base: c_ulong = (unsigned long)task_stack_page(current);
    let mut task_stack_low: c_ulong = stackleak_task_low_bound(current);
    let mut task_stack_high: c_ulong = stackleak_task_high_bound(current);
    let mut current_sp: c_ulong = current_stack_pointer;
    let mut lowest_sp: c_ulong = current.lowest_stack;
...
#[no_mangle]
unsafe extern "C" fn lkdtm_KSTACK_ERASE() {
