...
//! Source: drivers/misc/lkdtm/heap.c
#![no_std]
...
//
    let mut __offset: static volatile int = 1;
//
...
//
#[no_mangle]
unsafe extern "C" fn lkdtm_VMALLOC_LINEAR_OVERFLOW() {
...
//
#[no_mangle]
unsafe extern "C" fn lkdtm_SLAB_LINEAR_OVERFLOW() {
    static void lkdtm_SLAB_LINEAR_OVERFLOW(void)
    {
    let mut len: usize = 1020;
...
#[no_mangle]
unsafe extern "C" fn lkdtm_WRITE_AFTER_FREE() {
...
//
    let mut offset: usize = (len / sizeof(*base)) / 2;
...
#[no_mangle]
unsafe extern "C" fn lkdtm_READ_AFTER_FREE() {
...
//
    let mut offset: usize = sizeof(*base);
    base = kmalloc(len, GFP_KERNEL);
    if (!base) {
...
#[no_mangle]
unsafe extern "C" fn lkdtm_KFENCE_READ_AFTER_FREE() {
...
#[no_mangle]
unsafe extern "C" fn lkdtm_WRITE_BUDDY_AFTER_FREE() {
    static void lkdtm_WRITE_BUDDY_AFTER_FREE(void)
    {
    let mut p: c_ulong = __get_free_page(GFP_KERNEL);
    if (!p) {
...
#[no_mangle]
unsafe extern "C" fn lkdtm_READ_BUDDY_AFTER_FREE() {
...
#[no_mangle]
unsafe extern "C" fn lkdtm_SLAB_INIT_ON_ALLOC() {
    ...
    if (memchr(val, 0xAB, 512) == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn lkdtm_BUDDY_INIT_ON_ALLOC() {
    ...
    if (memchr(val, 0xAB, PAGE_SIZE) == core::ptr::null_mut()) {
    pr_info("Memory appears initialized (%x, no earlier values)\n", *val);
    } else {
    pr_err("FAIL: Slab was not initialized\n");
    pr_expected_config_param(CONFIG_INIT_ON_ALLOC_DEFAULT_ON, "init_on_alloc");
...
#[no_mangle]
unsafe extern "C" fn lkdtm_SLAB_FREE_DOUBLE() {
...
#[no_mangle]
unsafe extern "C" fn lkdtm_SLAB_FREE_CROSS() {
...
#[no_mangle]
unsafe extern "C" fn lkdtm_SLAB_FREE_PAGE() {
    static void lkdtm_SLAB_FREE_PAGE(void)
    {
    let mut p: c_ulong = __get_free_page(GFP_KERNEL);
    pr_info("Attempting non-Slab slab free ...\n");
    kmem_cache_free(core::ptr::null_mut(), (void *)p);
    free_page(p);
    }
#[no_mangle]
pub unsafe extern "C" fn lkdtm_heap_init() -> void __init {
    void __init lkdtm_heap_init(void)
    {
    double_free_cache = kmem_cache_create("lkdtm-heap-double_free",
    64, 0, SLAB_NO_MERGE, core::ptr::null_mut());
    a_cache = kmem_cache_create("lkdtm-heap-a", 64, 0, SLAB_NO_MERGE, core::ptr::null_mut());
    b_cache = kmem_cache_create("lkdtm-heap-b", 64, 0, SLAB_NO_MERGE, core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn lkdtm_heap_exit() -> void __exit {
