...
//! Source: drivers/misc/altera-stapl/altera.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct altera_procinfo {
    pub name: *mut c_char,
    pub attrs: u8,
    pub next: *mut altera_procinfo,
}

// This function checks if enough parameters are available on the stack.
#[no_mangle]
unsafe extern "C" fn altera_check_stack(stack_ptr: c_int, count: c_int, status: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn altera_export_int(key: *mut c_char, value: i32) {
    static void altera_export_int(char *key, s32 value)
    {
    dprintk("Export: key = \"%s\", value = %d\n", key, value);
    }
pub const HEX_LINE_CHARS: c_int = 72;

#[no_mangle]
unsafe extern "C" fn altera_export_bool_array(key: *mut c_char, data: *mut u8, count: i32) {
    ...
    let mut first_word: u32 = 0L;
    let mut action_table: u32 = 0L;
    let mut proc_table: u32 = 0L;
    let mut str_table: u32 = 0L;
    let mut sym_table: u32 = 0L;
    let mut data_sect: u32 = 0L;
    let mut code_sect: u32 = 0L;
    let mut debug_sect: u32 = 0L;
    let mut action_count: u32 = 0L;
    let mut proc_count: u32 = 0L;
    let mut sym_count: u32 = 0L;
    long *vars = core::ptr::null_mut();
    s32 *var_size = core::ptr::null_mut();
    char *attrs = core::ptr::null_mut();
    u8 *proc_attributes = core::ptr::null_mut();
    u32 pc;
    u32 opcode_address;
    u32 args[3];
    u32 opcode;
    ...
    let mut stack_ptr: c_int = 0;
    u32 arg_count;
    let mut done: c_int = 0;
    let mut bad_opcode: c_int = 0;
    u32 count;
    u32 index;
    u32 index2;
    s32 long_count;
...
// format_version = version + 1;
    ...
    if (proc_attributes == core::ptr::null_mut())
    status = -ENOMEM;
    }
    if (status != 0)
    goto exit_done;
...
// initialized scalar variable
    vars[i] = value;
#[no_mangle]
pub unsafe extern "C" fn if(0x0e: (attrs[i] & 0x1e) ==) -> else {
...
//
    if (version > 0) {
    if (aconf.action == core::ptr::null_mut()) {
    status = -EINVAL;
    done = 1;
    } else {
    let mut action_found: c_int = 0;
...
//
    ...
    let mut a: i32 = stack[--stack_ptr];
    let mut b: i32 = stack[--stack_ptr];
...
//
    ...
    break;
    count = stack[--stack_ptr];
    status = altera_set_dr_pre(&astate.js, count, 0, core::ptr::null_mut());
    break;
    case OP_DPRL:
//
...
//
    ...
    let mut step: i32 = stack[stack_ptr - 1];
    let mut end: i32 = stack[stack_ptr - 2];
    let mut top: i32 = stack[stack_ptr - 3];
    let mut iterator: i32 = vars[args[0]];
    let mut break_out: c_int = 0;
    if (step < 0) {
    if (iterator <= end)
    break_out = 1;
...
//
    ...
    GFP_KERNEL);
    if (charptr_tmp == core::ptr::null_mut()) {
    status = -ENOMEM;
    break;
    }
...
// reverse indices not supported
    ...
    charptr_tmp = (u8 *)vars[variable_id];
    charptr_tmp2 = core::ptr::null_mut();
    if ((long_idx & 7L) != 0) {
    let mut k: i32 = long_idx;
    charptr_tmp2 =
    kzalloc(((long_count + 7L) / 8L),
    GFP_KERNEL);
    if (charptr_tmp2 == core::ptr::null_mut()) {
    status = -ENOMEM;
    break;
    }
    for (i = 0; i < long_count; ++i) {
...
//
    ...
    s32 src_count;
    s32 dest_count;
    let mut src_reverse: c_int = 0;
    let mut dest_reverse: c_int = 0;
    if (!altera_check_stack(stack_ptr, 3, &status))
    break;
...
//
    let mut mask_right: i32 = stack[--stack_ptr];
    let mut mask_left: i32 = stack[--stack_ptr];
// source 1 count
    a = 1 + index2 - index1;
// source 2 count
...
// Free all dynamically allocated arrays
    if ((attrs != core::ptr::null_mut()) && (vars != core::ptr::null_mut()))
    for (i = 0; i < sym_count; ++i)
    if (attrs[i] & 0x80)
    kfree((void *)vars[i]);
    kfree(vars);
...
//
    {
    let mut status: c_int = -ENODATA;
    let mut note_strings: u32 = 0L;
    let mut note_table: u32 = 0L;
    let mut note_count: u32 = 0L;
    let mut first_word: u32 = 0L;
    let mut version: c_int = 0;
    let mut delta: c_int = 0;
    char *key_ptr;
...
#[no_mangle]
unsafe extern "C" fn altera_check_crc(p: *mut u8, program_size: i32) -> c_int {
...
// procedure_count = get_unaligned_be32(&p[52]);
    ...
    {
    let mut status: c_int = -EIO;
    struct altera_procinfo *procptr = core::ptr::null_mut();
    struct altera_procinfo *tmpptr = core::ptr::null_mut();
    let mut first_word: u32 = 0L;
    let mut action_table: u32 = 0L;
    let mut proc_table: u32 = 0L;
    let mut str_table: u32 = 0L;
    ...
    let mut action_count: u32 = 0L;
    let mut proc_count: u32 = 0L;
    let mut act_name_id: u32 = 0L;
    let mut act_desc_id: u32 = 0L;
    let mut act_proc_id: u32 = 0L;
    let mut act_proc_name: u32 = 0L;
    let mut act_proc_attribute: u8 = 0;
    if (program_size <= 52L)
    return status;
// Read header information
    first_word = get_unaligned_be32(&p[0]);
...
// description = &p[str_table + act_desc_id];
    ...
    if (procptr == core::ptr::null_mut())
    status = -ENOMEM;
    else {
    procptr.name = &p[str_table + act_proc_name];
    procptr.attrs = act_proc_attribute;
    procptr.next = core::ptr::null_mut();
// add record to end of linked list
    if (*proc_list == core::ptr::null_mut())
// proc_list = procptr;
    else {
    tmpptr = *proc_list;
    while (tmpptr.next != core::ptr::null_mut())
    tmpptr = tmpptr.next;
    tmpptr.next = procptr;
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn altera_init(config: *mut altera_config, fw: *const firmware) -> c_int {
    int altera_init(struct altera_config *config, const struct firmware *fw)
    {
    struct altera_state *astate = core::ptr::null_mut();
    struct altera_procinfo *proc_list = core::ptr::null_mut();
    struct altera_procinfo *procptr = core::ptr::null_mut();
    char *key = core::ptr::null_mut();
    char *value = core::ptr::null_mut();
    char *action_name = core::ptr::null_mut();
    char *description = core::ptr::null_mut();
    let mut exec_result: c_int = 0;
    let mut exit_code: c_int = 0;
    let mut format_version: c_int = 0;
    let mut action_count: c_int = 0;
    let mut procedure_count: c_int = 0;
    let mut index: c_int = 0;
    let mut offset: i32 = 0L;
    let mut error_address: i32 = 0L;
    ...
    &description,
    &proc_list);
    if (description == core::ptr::null_mut())
    ...
    description);
    procptr = proc_list;
    while (procptr != core::ptr::null_mut()) {
    if (procptr.attrs != 0)
    printk(KERN_INFO "%s:    %s (%s)\n",
    __func__,
    procptr.name,
    ...
    if (astate.config.action == core::ptr::null_mut())
