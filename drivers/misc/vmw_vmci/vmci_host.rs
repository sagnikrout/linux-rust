...
//! Source: drivers/misc/vmw_vmci/vmci_host.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmci_qp_alloc_info_vmvm {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmci_set_notify_info {
    pub notify_uva: u64,
    pub result: i32,
    pub _pad: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmci_host_dev {
    pub context: *mut vmci_ctx,
    pub user_version: c_int,
    pub ct_type: enum vmci_obj_type,
    pub /: *mut *mut mutex lock; / Mutex lock for vmci context access,
}

    static struct vmci_ctx *host_context;
    static bool vmci_host_device_initialized;
    let mut vmci_host_active_users: static atomic_t = ATOMIC_INIT(0);
//
...
//
#[no_mangle]
pub unsafe extern "C" fn vmci_host_code_active() -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn vmci_host_users() -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vmci_host_close(inode: *mut inode, filp: *mut file) -> c_int {
    static int vmci_host_close(struct inode *inode, struct file *filp)
    {
    struct vmci_host_dev *vmci_host_dev = filp.private_data;
    if (vmci_host_dev.ct_type == VMCIOBJ_CONTEXT) {
    vmci_ctx_destroy(vmci_host_dev.context);
    vmci_host_dev.context = core::ptr::null_mut();
//
...
//
#[no_mangle]
unsafe extern "C" fn vmci_host_poll(filp: *mut file, wait: *mut poll_table) -> __poll_t {
...
//
    ...
    struct vmci_datagram *dg = core::ptr::null_mut();
    u32 cid;
    if (vmci_host_dev.ct_type != VMCIOBJ_CONTEXT) {
...
//
    ...
    let mut flags: u32 = VMCI_NO_PRIVILEGE_FLAGS;
...
#[no_mangle]
pub unsafe extern "C" fn vmci_host_exit() -> void __exit {
