...
//! Source: drivers/misc/vmw_vmci/vmci_guest.c
#![no_std]
...
pub const PCI_DEVICE_ID_VMWARE_VMCI: c_uint = 0x0740;
pub const VMCI_UTIL_NUM_RESOURCES: c_int = 1;
//
...
//
    ...
    let mut ctx_update_sub_id: static u32 = VMCI_INVALID_ID;
    let mut vm_context_id: static u32 = VMCI_INVALID_ID;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vmci_guest_device {
    pub /: *mut *mut *mut device dev; / PCI device we are attached to,
    pub iobase: *mut void __iomem,
    pub mmio_base: *mut void __iomem,
    pub exclusive_vectors: bool,
    pub inout_wq: wait_queue_head,
    pub data_buffer: *mut c_void,
    pub data_buffer_base: dma_addr_t,
    pub tx_buffer: *mut c_void,
    pub tx_buffer_base: dma_addr_t,
    pub notification_bitmap: *mut c_void,
    pub notification_base: dma_addr_t,
}

    static bool use_ppn64;
#[no_mangle]
pub unsafe extern "C" fn vmci_use_ppn64() -> bool {
...
// vmci_dev singleton device and supporting data
    ...
    let mut vmci_num_guest_devices: static atomic_t = ATOMIC_INIT(0);
#[no_mangle]
pub unsafe extern "C" fn vmci_guest_code_active() -> bool {
    bool vmci_guest_code_active(void)
    {
    return atomic_read(&vmci_num_guest_devices) != 0;
    }
#[no_mangle]
pub unsafe extern "C" fn vmci_get_vm_context_id() -> u32 {
...
#[no_mangle]
unsafe extern "C" fn vmci_read_reg(dev: *mut vmci_guest_device, reg: u32) -> c_uint {
    static unsigned int vmci_read_reg(struct vmci_guest_device *dev, u32 reg)
    {
    if (dev.mmio_base != core::ptr::null_mut())
    return readl(dev.mmio_base + reg);
    return ioread32(dev.iobase + reg);
    }
#[no_mangle]
unsafe extern "C" fn vmci_write_reg(dev: *mut vmci_guest_device, val: u32, reg: u32) {
    ...
    {
    if (vmci_dev.mmio_base == core::ptr::null_mut())
...
//
    ...
    let mut buffer_offset: usize = dest - vmci_dev.data_buffer;
...
//
#[no_mangle]
pub unsafe extern "C" fn vmci_send_datagram(dg: *mut vmci_datagram) -> c_int {
    int vmci_send_datagram(struct vmci_datagram *dg)
    {
    unsigned long flags;
    int result;
// Check args.
    if (dg == core::ptr::null_mut())
    return VMCI_ERROR_INVALID_ARGS;
//
...
//
#[no_mangle]
unsafe extern "C" fn vmci_check_host_caps(pdev: *mut pci_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn vmci_dispatch_dgs(vmci_dev: *mut vmci_guest_device) {
    static void vmci_dispatch_dgs(struct vmci_guest_device *vmci_dev)
    {
    u8 *dg_in_buffer = vmci_dev.data_buffer;
    struct vmci_datagram *dg;
    let mut dg_in_buffer_size: usize = VMCI_MAX_DG_SIZE;
    size_t current_dg_in_buffer_size;
    size_t remaining_bytes;
    let mut is_io_port: bool = vmci_dev.mmio_base == core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn vmci_process_bitmap(dev: *mut vmci_guest_device) {
...
//
#[no_mangle]
unsafe extern "C" fn vmci_interrupt(irq: c_int, _dev: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn vmci_interrupt_bm(irq: c_int, _dev: *mut c_void) -> irqreturn_t {
...
//
#[no_mangle]
unsafe extern "C" fn vmci_interrupt_dma_datagram(irq: c_int, _dev: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn vmci_free_dg_buffers(vmci_dev: *mut vmci_guest_device) {
    static void vmci_free_dg_buffers(struct vmci_guest_device *vmci_dev)
    {
    if (vmci_dev.mmio_base != core::ptr::null_mut()) {
    if (vmci_dev.tx_buffer != core::ptr::null_mut())
    dma_free_coherent(vmci_dev.dev,
    VMCI_DMA_DG_BUFFER_SIZE,
    vmci_dev.tx_buffer,
    vmci_dev.tx_buffer_base);
    if (vmci_dev.data_buffer != core::ptr::null_mut())
    dma_free_coherent(vmci_dev.dev,
    VMCI_DMA_DG_BUFFER_SIZE,
...
//
    ...
    void __iomem *mmio_base = core::ptr::null_mut();
...
// If the map fails, we fall back to IOIO access.
    ...
    if (mmio_base != core::ptr::null_mut()) {
...
//
    ...
    vmci_guest_cid_update, core::ptr::null_mut(),
    &ctx_update_sub_id);
...
// MSI-X vector.
//
    error = request_threaded_irq(pci_irq_vector(pdev, 0), core::ptr::null_mut(),
...
//
    ...
    error = request_threaded_irq(pci_irq_vector(pdev, 1), core::ptr::null_mut(),
    vmci_interrupt_bm, 0,
    KBUILD_MODNAME, vmci_dev);
    if (error) {
    dev_err(&pdev.dev,
...
// Enable interrupts.
    ...
    spin_lock_irq(&vmci_dev_spinlock);
    vmci_pdev = core::ptr::null_mut();
    vmci_dev_g = core::ptr::null_mut();
    spin_unlock_irq(&vmci_dev_spinlock);
...
#[no_mangle]
unsafe extern "C" fn vmci_guest_remove_device(pdev: *mut pci_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn vmci_guest_init() -> int __init {
    int __init vmci_guest_init(void)
    {
    return pci_register_driver(&vmci_guest_driver);
    }
#[no_mangle]
pub unsafe extern "C" fn vmci_guest_exit() -> void __exit {
