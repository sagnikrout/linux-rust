...
//! Source: drivers/misc/genwqe/card_utils.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn __genwqe_writeq(cd: *mut genwqe_dev, byte_offs: u64, val: u64) -> c_int {
    int __genwqe_writeq(struct genwqe_dev *cd, u64 byte_offs, u64 val)
    {
    struct pci_dev *pci_dev = cd.pci_dev;
    if (cd.err_inject & GENWQE_INJECT_HARDWARE_FAILURE)
    return -EIO;
    if (cd.mmio == core::ptr::null_mut())
    return -EIO;
    if (pci_channel_offline(pci_dev))
    return -EIO;
    __raw_writeq(( u64)cpu_to_be64(val), cd.mmio + byte_offs);
    return 0;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn __genwqe_readq(cd: *mut genwqe_dev, byte_offs: u64) -> u64 {
    ...
    if (cd.mmio == core::ptr::null_mut())
    return 0xffffffffffffffffull;
    return be64_to_cpu(( __be64)__raw_readq(cd.mmio + byte_offs));
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn __genwqe_writel(cd: *mut genwqe_dev, byte_offs: u64, val: u32) -> c_int {
    ...
    if (pci_channel_offline(pci_dev))
    return -EIO;
    __raw_writel(( u32)cpu_to_be32(val), cd.mmio + byte_offs);
    return 0;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn __genwqe_readl(cd: *mut genwqe_dev, byte_offs: u64) -> u32 {
    ...
    if (cd.mmio == core::ptr::null_mut())
    return 0xffffffff;
    return be32_to_cpu(( __be32)__raw_readl(cd.mmio + byte_offs));
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_read_app_id(cd: *mut genwqe_dev, app_name: *mut c_char, len: c_int) -> c_int {
    int genwqe_read_app_id(struct genwqe_dev *cd, char *app_name, int len)
    {
    int i, j;
    let mut app_id: u32 = (u32)cd.app_unitcfg;
    memset(app_name, 0, len);
    for (i = 0, j = 0; j < min(len, 4); j++) {
    let mut ch: c_char = (char)((app_id >> (24 - j*8)) & 0xff);
    if (ch == ' ')
    continue;
...
pub const CRC32_POLYNOMIAL: c_uint = 0x20044009;
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_init_crc32() {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_crc32(buff: *mut u8, len: usize, init: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn genwqe_sgl_size(num_pages: c_int) -> c_int {
...
// Only use buffering on incomplete pages
    ...
    &sgl.fpage_dma_addr);
    if (sgl.fpage == core::ptr::null_mut())
    goto err_out;
// Sync with user memory
    ...
    &sgl.lpage_dma_addr);
    if (sgl.lpage == core::ptr::null_mut())
    goto err_out1;
// Sync with user memory
    ...
    sgl.lpage_dma_addr);
    sgl.lpage = core::ptr::null_mut();
    sgl.lpage_dma_addr = 0;
    err_out1:
    __genwqe_free_consistent(cd, PAGE_SIZE, sgl.fpage,
    sgl.fpage_dma_addr);
    sgl.fpage = core::ptr::null_mut();
    sgl.fpage_dma_addr = 0;
    err_out:
    __genwqe_free_consistent(cd, sgl.sgl_size, sgl.sgl,
    sgl.sgl_dma_addr);
    sgl.sgl = core::ptr::null_mut();
    sgl.sgl_dma_addr = 0;
    sgl.sgl_size = 0;
    return ret;
    }
    ...
    dma_addr_t *dma_list)
    {
    let mut i: c_int = 0, j = 0, p;
    unsigned long dma_offs, map_offs;
    let mut prev_daddr: dma_addr_t = 0;
    struct sg_entry *s, *last_s = core::ptr::null_mut();
    let mut size: usize = sgl.user_size;
...
// DMA mapping for requested page, offs, size
    ...
    if ((p == 0) && (sgl.fpage != core::ptr::null_mut())) {
    ...
    let mut prev_len: u32 = be32_to_cpu(last_s.len);
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_free_sync_sgl(cd: *mut genwqe_dev, sgl: *mut genwqe_sgl) -> c_int {
...
//
    ...
    struct pci_dev *pci_dev = cd.pci_dev;
    if ((uaddr == core::ptr::null_mut()) || (size == 0)) {
...
// assumption: pin_user_pages can be killed by signals.
    ...
    fail_pin_user_pages:
    kfree(m.page_list);
    m.page_list = core::ptr::null_mut();
    m.dma_list = core::ptr::null_mut();
    m.nr_pages = 0;
    m.u_vaddr = core::ptr::null_mut();
    m.size = 0;		/* mark unused and not added */
    return rc;
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_user_vunmap(cd: *mut genwqe_dev, m: *mut dma_mapping) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_card_type(cd: *mut genwqe_dev) -> u8 {
    u8 genwqe_card_type(struct genwqe_dev *cd)
    {
    let mut card_type: u64 = cd.slu_unitcfg;
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_card_reset(cd: *mut genwqe_dev) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn genwqe_read_softreset(cd: *mut genwqe_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_set_interrupt_capability(cd: *mut genwqe_dev, count: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_reset_interrupt_capability(cd: *mut genwqe_dev) {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_ffdc_buff_size(cd: *mut genwqe_dev, uid: c_int) -> c_int {
    int genwqe_ffdc_buff_size(struct genwqe_dev *cd, int uid)
    {
    let mut entries: c_int = 0, ring, traps, traces, trace_entries;
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_write_vreg(cd: *mut genwqe_dev, reg: u32, val: u64, func: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_read_vreg(cd: *mut genwqe_dev, reg: u32, func: c_int) -> u64 {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_base_clock_frequency(cd: *mut genwqe_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_stop_traps(cd: *mut genwqe_dev) {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_start_traps(cd: *mut genwqe_dev) {
