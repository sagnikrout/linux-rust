...
//! Source: drivers/misc/genwqe/card_dev.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn genwqe_open_files(cd: *mut genwqe_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn genwqe_add_file(cd: *mut genwqe_dev, cfile: *mut genwqe_file) {
...
#[no_mangle]
unsafe extern "C" fn genwqe_del_file(cd: *mut genwqe_dev, cfile: *mut genwqe_file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn genwqe_add_pin(cfile: *mut genwqe_file, m: *mut dma_mapping) {
...
#[no_mangle]
unsafe extern "C" fn genwqe_del_pin(cfile: *mut genwqe_file, m: *mut dma_mapping) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn genwqe_remove_mappings(cfile: *mut genwqe_file) {
...
#[no_mangle]
unsafe extern "C" fn genwqe_remove_pinnings(cfile: *mut genwqe_file) {
...
//
#[no_mangle]
unsafe extern "C" fn genwqe_kill_fasync(cd: *mut genwqe_dev, sig: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn genwqe_terminate(cd: *mut genwqe_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn genwqe_open(inode: *mut inode, filp: *mut file) -> c_int {
    ...
    cfile.cd = cd;
    cfile.filp = filp;
    cfile.client = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn genwqe_fasync(fd: c_int, filp: *mut file, mode: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn genwqe_release(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn genwqe_vma_open(vma: *mut vm_area_struct) {
...
//
#[no_mangle]
unsafe extern "C" fn genwqe_mmap(filp: *mut file, vma: *mut vm_area_struct) -> c_int {
    ...
    &dma_map.dma_addr);
    if (dma_map.k_vaddr == core::ptr::null_mut()) {
    rc = -ENOMEM;
    goto free_dma_map;
    }
...
pub const FLASH_BLOCK: c_uint = 0x40000	/* we use 256k blocks */;
//
...
#[no_mangle]
unsafe extern "C" fn genwqe_pin_mem(cfile: *mut genwqe_file, m: *mut genwqe_mem) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn genwqe_unpin_mem(cfile: *mut genwqe_file, m: *mut genwqe_mem) -> c_int {
    ...
    map_addr = (m.addr & PAGE_MASK);
    map_size = round_up(m.size + (m.addr & ~PAGE_MASK), PAGE_SIZE);
    dma_map = genwqe_search_pin(cfile, map_addr, map_size, core::ptr::null_mut());
    if (dma_map == core::ptr::null_mut())
    return -ENOENT;
...
//
#[no_mangle]
unsafe extern "C" fn ddcb_cmd_cleanup(cfile: *mut genwqe_file, req: *mut ddcb_requ) -> c_int {
    ...
    if (req.sgls[i].sgl != core::ptr::null_mut())
...
//
#[no_mangle]
unsafe extern "C" fn ddcb_cmd_fixups(cfile: *mut genwqe_file, req: *mut ddcb_requ) -> c_int {
...
// ((__be64 *)&cmd->asiv[asiv_offs]) =
    ...
    m = genwqe_search_pin(cfile, u_addr, u_size, core::ptr::null_mut());
    if (m != core::ptr::null_mut()) {
    page_offs = (u_addr -
...
#[no_mangle]
unsafe extern "C" fn genwqe_device_initialized(cd: *mut genwqe_dev) -> c_int {
    static int genwqe_device_initialized(struct genwqe_dev *cd)
    {
    return cd.dev != core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_device_create(cd: *mut genwqe_dev) -> c_int {
...
//
    ...
    err_dev:
    cd.dev = core::ptr::null_mut();
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn genwqe_inform_and_stop_processes(cd: *mut genwqe_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn genwqe_device_remove(cd: *mut genwqe_dev) -> c_int {
