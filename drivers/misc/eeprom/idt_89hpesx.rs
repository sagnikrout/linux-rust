...
//! Source: drivers/misc/eeprom/idt_89hpesx.c
#![no_std]
...
pub struct idt_89hpesx_dev {
    ...
    pub iniccode: u8,
    pub csr: u16,
    pub ): *const *const *const int (smb_write)(struct idt_89hpesx_dev , struct idt_smb_seq,
    pub ): *mut *mut *mut int (smb_read)(struct idt_89hpesx_dev , struct idt_smb_seq,
    pub smb_mtx: mutex,
    pub client: *mut i2c_client,
    pub ee_file: *mut bin_attribute,
    pub csr_dir: *mut dentry,
}
...
//
    ...
    pub \: int __retry = RETRY_CNT;,
    pub \: s32 __sts;,
    do { \
    pub \: __sts = i2c_smbus_ ## ops ## _data(args);,
    pub \: } while (__retry-- && __sts < 0);,
    pub \: __sts;,
    })
// ===========================================================================
...
// Loop over the supplied data sending byte one-by-one
    pub {: for (idx = 0; idx < seq->bytecnt; idx++),
// Collect the command code byte
    pub CCODE_BYTE: ccode = seq->ccode |,
    if (idx == 0)
    pub CCODE_START: ccode |=,
...
// Read data from the device
    pub ccode): sts = idt_smb_safe(read_byte, pdev->client,,
    if (sts < 0)
    pub (int)sts: return,
    pub (u8)sts: seq->data[idx] =,
    }
    pub 0: return,
    }
//
...
// Calculate the even count of data to send
    pub 2): evencnt = seq->bytecnt - (seq->bytecnt %,
// Loop over the supplied data sending two bytes at a time
    pub {: for (idx = 0; idx < evencnt; idx += 2),
// Collect the command code byte
    pub CCODE_WORD: ccode = seq->ccode |,
    if (idx == 0)
    pub CCODE_START: ccode |=,
    if (idx == evencnt - 2)
    pub CCODE_END: ccode |=,
...
// Collect the command code byte
    pub CCODE_END: ccode = seq->ccode | CCODE_BYTE |,
    if (idx == 0)
    pub CCODE_START: ccode |=,
...
// Read word data from the device
    pub ccode): sts = idt_smb_safe(read_word, pdev->client,,
    if (sts < 0)
    pub (int)sts: return,
...
// Return error if too much data passed to send
    ...
    pub -EINVAL: return,
// Collect the command code byte
    pub CCODE_END: ccode = seq->ccode | CCODE_BLOCK | CCODE_START |,
...
// Read block of data from the device
    pub seq->data): sts = idt_smb_safe(read_block, pdev->client, ccode,,
    if (sts != seq.bytecnt)
    pub -ENODATA): return (sts < 0 ? sts :,
    pub 0: return,
    }
//
...
// Copy retrieved data to the output data buffer
    pub seq->bytecnt): memcpy(seq->data, &buf[1],,
    pub 0: return,
    }
// ===========================================================================
...
//
    ...
    u8 *data)
    {
    pub &pdev->client->dev: *mut *mut device dev =,
    pub eeseq: idt_eeprom_seq,
    pub smbseq: idt_smb_seq,
    pub retry: int ret,,
// Initialize SMBus sequence fields
    pub CCODE_EEPROM: smbseq.ccode = pdev->iniccode |,
    pub )&eeseq: *mut smbseq.data = (u8,
//
...
// Send EEPROM memory address to read data from
    pub EEPROM_WRRD_CNT: smbseq.bytecnt =,
    pub EEPROM_OP_READ: eeseq.cmd = pdev->inieecmd |,
    pub pdev->eeaddr: eeseq.eeaddr =,
    pub cpu_to_le16(memaddr): eeseq.memaddr =,
    pub &smbseq): ret = pdev->smb_write(pdev,,
    if (ret != 0) {
    dev_err(dev, "Failed to init eeprom addr 0x%02x",
    }
// Perform read operation
    pub EEPROM_RD_CNT: smbseq.bytecnt =,
    pub &smbseq): ret = pdev->smb_read(pdev,,
    if (ret != 0) {
...
// Send data byte-by-byte, checking if it is successfully written
    pub {: for (idx = 0; idx < len; idx++, memaddr++),
// Lock IDT SMBus device
// Perform write operation
    pub EEPROM_WR_CNT: smbseq.bytecnt =,
    pub EEPROM_OP_WRITE: eeseq.cmd = pdev->inieecmd |,
    pub pdev->eeaddr: eeseq.eeaddr =,
    pub cpu_to_le16(memaddr): eeseq.memaddr =,
    pub data: [eeseq.data =; idx],
    pub &smbseq): ret = pdev->smb_write(pdev,,
    if (ret != 0) {
    dev_err(dev,
    "Failed to write 0x%04hx:0x%02hhx to eeprom",
    pub data[idx]): memaddr,,
    pub err_mutex_unlock: goto,
    }
//
// Check whether the data is successfully written by reading
// from the same EEPROM memory address.
//
    pub ~data[idx]: eeseq.data =,
    pub &eeseq.data): ret = idt_eeprom_read_byte(pdev, memaddr,,
    if (ret != 0)
    pub err_mutex_unlock: goto,
...
// Just read the byte to the buffer
    pub &buf[idx]): ret = idt_eeprom_read_byte(pdev, memaddr,,
// Unlock IDT SMBus device
...
//
    ...
    {
    pub &pdev->client->dev: *mut *mut device dev =,
    pub csrseq: idt_csr_seq,
    pub smbseq: idt_smb_seq,
    pub ret: c_int,
// Initialize SMBus sequence fields
    pub CCODE_CSR: smbseq.ccode = pdev->iniccode |,
    pub )&csrseq: *mut smbseq.data = (u8,
// Lock IDT SMBus device
// Perform write operation
    pub CSR_WR_CNT: smbseq.bytecnt =,
    pub CSR_OP_WRITE: csrseq.cmd = pdev->inicsrcmd |,
    pub cpu_to_le16(csraddr): csrseq.csraddr =,
    pub cpu_to_le32(data): csrseq.data =,
    pub &smbseq): ret = pdev->smb_write(pdev,,
    if (ret != 0) {
    dev_err(dev, "Failed to write 0x%04x: 0x%04x to csr",
    pub data): CSR_REAL_ADDR(csraddr),,
    pub err_mutex_unlock: goto,
    }
// Send CSR address to read data from
    pub CSR_WRRD_CNT: smbseq.bytecnt =,
    pub CSR_OP_READ: csrseq.cmd = pdev->inicsrcmd |,
    pub &smbseq): ret = pdev->smb_write(pdev,,
    if (ret != 0) {
...
// Check whether IDT successfully retrieved CSR data
    ...
    pub r/w"): dev_err(dev, "IDT failed to perform CSR,
    pub -EREMOTEIO: ret =,
    pub err_mutex_unlock: goto,
    }
// Unlock IDT SMBus device
...
//
#[no_mangle]
unsafe extern "C" fn idt_csr_read(pdev: *mut idt_89hpesx_dev, csraddr: u16, data: *mut u32) -> c_int {
...
//
    ...
    {
    pub pdev: *mut idt_89hpesx_dev,
    pub ret: c_int,
// Retrieve driver data
    pub dev_get_drvdata(kobj_to_dev(kobj)): pdev =,
// Perform EEPROM write operation
    pub )buf): *mut ret = idt_eeprom_write(pdev, (u16)off, (u16)count, (u8,
    pub count): return (ret != 0 ? ret :,
    }
//
...
// Perform EEPROM read operation
    pub )buf): *mut ret = idt_eeprom_read(pdev, (u16)off, (u16)count, (u8,
    pub count): return (ret != 0 ? ret :,
    }
//
...
//
    ...
    {
    pub filep->private_data: *mut *mut idt_89hpesx_dev pdev =,
    pub csrval_str: *mut *mut *mut char colon_ch, csraddr_str,,
    pub ret: c_int,
    pub csrval: u32 csraddr,,
    pub buf: *mut c_char,
    if (*offp)
    pub 0: return,
// Copy data from User-space
    pub count): buf = memdup_user_nul(ubuf,,
    if (IS_ERR(buf))
    pub PTR_ERR(buf): return,
// Find position of colon in the buffer
    pub ':'): colon_ch = strnchr(buf, count,,
//
...
// no new CSR value
//
    if (colon_ch != core::ptr::null_mut()) {
// Copy the register address to the substring buffer
    pub GFP_KERNEL): csraddr_str = kmemdup_nul(buf, colon_ch - buf,,
    if (csraddr_str == core::ptr::null_mut()) {
    pub -ENOMEM: ret =,
    pub free_buf: goto,
    }
// Register value must follow the colon
    pub 1: csrval_str = colon_ch +,
    } else /* if (str_colon == core::ptr::null_mut()) */ {
    pub /: *mut *mut *mut csraddr_str = (char )buf; / Just to shut warning up,
    pub NULL: csrval_str =,
    }
// Convert CSR address to u32 value
    pub &csraddr): ret = kstrtou32(csraddr_str, 0,,
    if (ret != 0)
    pub free_csraddr_str: goto,
...
// Shift register address to the right so to have u16 address
    pub 2): pdev->csr = (csraddr >>,
// Parse new CSR value and send it to IDT, if colon has been found
    if (colon_ch != core::ptr::null_mut()) {
    pub &csrval): ret = kstrtou32(csrval_str, 0,,
    if (ret != 0)
    pub free_csraddr_str: goto,
    pub csrval): ret = idt_csr_write(pdev, pdev->csr,,
    if (ret != 0)
    pub free_csraddr_str: goto,
    }
...
//
    ...
    pub filep->private_data: *mut *mut idt_89hpesx_dev pdev =,
    pub csrval: u32 csraddr,,
    pub buf: [c_char; CSRBUF_SIZE],
    pub size: int ret,,
// Perform CSR read operation
    pub &csrval): ret = idt_csr_read(pdev, pdev->csr,,
    if (ret != 0)
    pub ret: return,
// Shift register address to the left so to have real address
    pub 2): csraddr = ((u32)pdev->csr <<,
// Print the "0x<reg addr>:0x<value>" to buffer
    size = snprintf(buf, CSRBUF_SIZE, "0x%05x:0x%08x\n",
    pub int)csrval): (unsigned int)csraddr, (unsigned,
// Copy data to User-space
    pub size): return simple_read_from_buffer(ubuf, count, offp, buf,,
    }
//
...
//
    pub EEPROM_DEF_SIZE): static BIN_ATTR_RW(eeprom,,
//
...
//
#[no_mangle]
unsafe extern "C" fn idt_set_defval(pdev: *mut idt_89hpesx_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn idt_get_fw_data(pdev: *mut idt_89hpesx_dev) {
    static void idt_get_fw_data(struct idt_89hpesx_dev *pdev)
    {
    struct device *dev = &pdev.client.dev;
    struct fwnode_handle *fwnode;
    const struct i2c_device_id *ee_id = core::ptr::null_mut();
    u32 eeprom_addr;
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn idt_free_pdev(pdev: *mut idt_89hpesx_dev) {
    static void idt_free_pdev(struct idt_89hpesx_dev *pdev)
    {
// Clear driver data from device private field
    i2c_set_clientdata(pdev.client, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn idt_set_smbus_ops(pdev: *mut idt_89hpesx_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn idt_check_dev(pdev: *mut idt_89hpesx_dev) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn idt_create_sysfs_files(pdev: *mut idt_89hpesx_dev) -> c_int {
...
// In case of read-only EEPROM get rid of write ability
    ...
    pdev.ee_file.write = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn idt_remove_sysfs_files(pdev: *mut idt_89hpesx_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn idt_probe(client: *mut i2c_client) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn idt_remove(client: *mut i2c_client) {
