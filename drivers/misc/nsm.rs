...
#[derive(Copy, Clone)]
pub struct nsm_msg {
    pub req: nsm_data_req,
    pub resp: nsm_data_resp,
}
...
pub const CBOR_TYPE_MASK: c_uint = 0xE0;
pub const CBOR_TYPE_MAP: c_uint = 0xA0;
pub const CBOR_TYPE_TEXT: c_uint = 0x60;
pub const CBOR_TYPE_ARRAY: c_uint = 0x40;
pub const CBOR_HEADER_SIZE_SHORT: c_int = 1;
pub const CBOR_SHORT_SIZE_MAX_VALUE: c_int = 23;
pub const CBOR_LONG_SIZE_U8: c_int = 24;
pub const CBOR_LONG_SIZE_U16: c_int = 25;
pub const CBOR_LONG_SIZE_U32: c_int = 26;
pub const CBOR_LONG_SIZE_U64: c_int = 27;
#[no_mangle]
unsafe extern "C" fn cbor_object_is_array(cbor_object: *const u8, cbor_object_size: usize) -> bool {
    static bool cbor_object_is_array(const u8 *cbor_object, size_t cbor_object_size)
    {
    if (cbor_object_size == 0 || cbor_object == core::ptr::null_mut())
    return false;
    return (cbor_object[0] & CBOR_TYPE_MASK) == CBOR_TYPE_ARRAY;
    }
#[no_mangle]
unsafe extern "C" fn cbor_object_get_array(cbor_object: *mut u8, cbor_object_size: usize, cbor_array: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nsm_vq_callback(vq: *mut virtqueue) {
...
#[no_mangle]
unsafe extern "C" fn nsm_sendrecv_msg_locked(nsm: *mut nsm) -> c_int {
...
// If the kick succeeded, wait for the device's response.
    ...
    if (rc) {
// Clean the virtqueue.
    while (virtqueue_get_buf(vq, &len) != core::ptr::null_mut())
    ;
    }
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn fill_req_get_random(nsm: *mut nsm, req: *mut nsm_data_req) -> c_int {
...
//
    ...
    'r', 'a', 'n', 'd', 'o', 'm' };
    struct device *dev = &nsm.vdev.dev;
    u8 *rand_data = core::ptr::null_mut();
    u8 *resp_ptr = resp.data;
    let mut resp_len: u64 = resp.len;
    int rc;
...
//
#[no_mangle]
unsafe extern "C" fn nsm_rng_read(rng: *mut hwrng, data: *mut c_void, max: usize, wait: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nsm_device_init_vq(vdev: *mut virtio_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nsm_device_probe(vdev: *mut virtio_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nsm_device_remove(vdev: *mut virtio_device) {
