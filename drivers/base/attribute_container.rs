...
//! Source: drivers/base/attribute_container.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct internal_container {
    pub node: klist_node,
    pub cont: *mut attribute_container,
    pub classdev: device,
}
...
//
    ...
    ({ klist_iter_exit(iter) ; core::ptr::null_mut(); }); \
    })) != core::ptr::null_mut();)
//
...
//
    ...
    struct klist_iter iter;
    if (attribute_container_no_classdevs(cont))
    return fn(cont, dev, core::ptr::null_mut());
    klist_for_each_entry(ic, &cont.containers, node, &iter) {
    if (dev == ic.classdev.parent) {
...
//
    ...
    struct device *))
    {
    struct attribute_container *cont, *failed = core::ptr::null_mut();
    let mut ret: c_int = 0;
    mutex_lock(&attribute_container_mutex);
    list_for_each_entry(cont, &attribute_container_list, node) {
    if (!cont.match(cont, dev))
...
//
    ...
    {
    let mut error: c_int = device_add(classdev);
    if (error)
    return error;
...
//
    ...
    struct device *dev)
    {
    struct device *cdev = core::ptr::null_mut();
    struct internal_container *ic;
    struct klist_iter iter;
    klist_for_each_entry(ic, &cont.containers, node, &iter) {
