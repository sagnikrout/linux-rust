...
//! Source: drivers/base/test/property-entry-test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn pe_test_uints(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn pe_test_uint_arrays(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn pe_test_strings(test: *mut kunit) {
...
// NULL argument -> returns size
    error = fwnode_property_read_string_array(node, "strs", core::ptr::null_mut(), 0);
    KUNIT_EXPECT_EQ(test, error, 2);
...
#[no_mangle]
unsafe extern "C" fn pe_test_bool(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn pe_test_move_inline_u8(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn pe_test_move_inline_str(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn pe_test_reference(test: *mut kunit) {
    static void pe_test_reference(struct kunit *test)
    {
    let mut node1: static struct software_node = { .name = "1" };
    let mut node2: static struct software_node = { .name = "2" };
    static const struct software_node *group[] = { &node1, &node2, core::ptr::null_mut() };
    ...
    node = fwnode_create_software_node(entries, core::ptr::null_mut());
    KUNIT_ASSERT_NOT_ERR_OR_NULL(test, node);
    error = fwnode_property_get_reference_args(node, "ref-1", core::ptr::null_mut(),
    0, 0, &ref);
    KUNIT_ASSERT_EQ(test, error, 0);
...
// wrong index
    ...
    0, 1, &ref);
    KUNIT_EXPECT_NE(test, error, 0);
    error = fwnode_property_get_reference_args(node, "ref-2", core::ptr::null_mut(),
    1, 0, &ref);
    KUNIT_ASSERT_EQ(test, error, 0);
...
// wrong index
    ...
    KUNIT_EXPECT_NE(test, error, 0);
// array of references
    error = fwnode_property_get_reference_args(node, "ref-3", core::ptr::null_mut(),
    0, 0, &ref);
    KUNIT_ASSERT_EQ(test, error, 0);
    KUNIT_EXPECT_PTR_EQ(test, to_software_node(ref.fwnode), &node1);
    KUNIT_EXPECT_EQ(test, ref.nargs, 0U);
...
#[no_mangle]
unsafe extern "C" fn pe_test_child_iteration(test: *mut kunit) {
    ...
    let mut node: static struct software_node = { .name = "sw" };
    let mut node1: static struct software_node = { .name = "sw-1", .parent = &node};
    let mut node2: static struct software_node = { .name = "sw-2", .parent = &node};
    let mut node3: static struct software_node = { .name = "sw-3", .parent = &node};
    static const struct software_node *group[] = { &node, &node1, &node2, &node3, core::ptr::null_mut() };
...
// 1. Test OF node child iteration
    of_node = create_device_node(test, "of", "of", core::ptr::null_mut());
    ...
    KUNIT_EXPECT_PTR_EQ(test, child, core::ptr::null_mut());
...
// 6. Test SW (primary) + OF (secondary, but no children) node child iteration
    ...
    KUNIT_EXPECT_PTR_EQ(test, child, core::ptr::null_mut());
    of_node.secondary = core::ptr::null_mut();
    sw_node.secondary = core::ptr::null_mut();
    software_node_unregister_node_group(group);
    }
