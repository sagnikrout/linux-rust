...
//! Source: drivers/virtio/virtio_mem.c
#![no_std]
...
//
    ...
    let mut unplug_online: static bool = true;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct virtio_mem {
    pub vdev: *mut virtio_device,
// We might first have to unplug all memory when starting up.
    pub unplug_all_required: bool,
// Workqueue that processes the plug/unplug requests.
    pub wq: work_struct,
    pub wq_active: core::sync::atomic::AtomicI32,
    pub config_changed: core::sync::atomic::AtomicI32,
// Virtqueue for guest->host requests.
    pub vq: *mut virtqueue,
// Wait for a host response to a guest request.
    pub host_resp: wait_queue_head_t,
// Space for one guest request and the host response.
    pub req: virtio_mem_req,
    pub resp: virtio_mem_resp,
...
// The parent resource for all memory added via this device.
    pub parent_resource: *mut resource,
//
...
//
    ...
    pub offline_size: core::sync::atomic::AtomicI64,
...
// Id of the first memory block of this device.
    pub first_mb_id: c_ulong,
// Id of the last usable memory block of this device.
    pub last_usable_mb_id: c_ulong,
// Id of the next memory bock to prepare when needed.
    pub next_mb_id: c_ulong,
// The subblock size.
    pub sb_size: u64,
...
//
    pub have_unplugged_mb: bool,
// Summary of all memory block states.
    pub mb_count: [c_ulong; VIRTIO_MEM_SBM_MB_COUNT],
//
...
// in one 4 KiB page.
//
    pub sb_states: *mut c_ulong,
    pub sbm: },
    struct {
// Id of the first big block of this device.
    pub first_bb_id: c_ulong,
// Id of the last usable big block of this device.
    pub last_usable_bb_id: c_ulong,
// Id of the next device bock to prepare when needed.
    pub next_bb_id: c_ulong,
// Summary of all big block states.
    pub bb_count: [c_ulong; VIRTIO_MEM_BBM_BB_COUNT],
// One byte state per big block. See sbm.mb_states.
    pub bb_states: *mut u8,
...
pub const VIRTIO_MEM_RETRY_TIMER_MIN_MS: c_int = 50000;
pub const VIRTIO_MEM_RETRY_TIMER_MAX_MS: c_int = 300000;
...
//
#[no_mangle]
unsafe extern "C" fn register_virtio_mem_device(vm: *mut virtio_mem) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn unregister_virtio_mem_device(vm: *mut virtio_mem) {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_phys_to_mb_id(addr: c_ulong) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_mb_id_to_phys(mb_id: c_ulong) -> c_ulong {
...
//
    ...
    unsigned long addr)
    {
    let mut mb_id: c_ulong = virtio_mem_phys_to_mb_id(addr);
    let mut mb_addr: c_ulong = virtio_mem_mb_id_to_phys(mb_id);
...
//
    ...
    {
    let mut idx: c_ulong = bb_id - vm.bbm.first_bb_id;
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_bbm_bb_states_prepare_next_bb(vm: *mut virtio_mem) -> c_int {
    static int virtio_mem_bbm_bb_states_prepare_next_bb(struct virtio_mem *vm)
    {
    let mut old_bytes: c_ulong = vm.bbm.next_bb_id - vm.bbm.first_bb_id;
    let mut new_bytes: c_ulong = old_bytes + 1;
    let mut old_pages: c_int = PFN_UP(old_bytes);
    let mut new_pages: c_int = PFN_UP(new_bytes);
    uint8_t *new_array;
...
//
    ...
    {
    let mut idx: c_ulong = mb_id - vm.sbm.first_mb_id;
    uint8_t old_state;
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_sbm_mb_states_prepare_next_mb(vm: *mut virtio_mem) -> c_int {
    static int virtio_mem_sbm_mb_states_prepare_next_mb(struct virtio_mem *vm)
    {
    let mut old_pages: c_int = PFN_UP(vm.sbm.next_mb_id - vm.sbm.first_mb_id);
    let mut new_pages: c_int = PFN_UP(vm.sbm.next_mb_id - vm.sbm.first_mb_id + 1);
    uint8_t *new_array;
...
//
    ...
    int count)
    {
    let mut bit: c_int = virtio_mem_sbm_sb_state_bit_nr(vm, mb_id, sb_id);
...
//
    ...
    unsigned long mb_id)
    {
    let mut bit: c_int = virtio_mem_sbm_sb_state_bit_nr(vm, mb_id, 0);
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_sbm_sb_states_prepare_next_mb(vm: *mut virtio_mem) -> c_int {
    static int virtio_mem_sbm_sb_states_prepare_next_mb(struct virtio_mem *vm)
    {
    let mut old_nb_mb: c_ulong = vm.sbm.next_mb_id - vm.sbm.first_mb_id;
    let mut old_nb_bits: c_ulong = old_nb_mb * vm.sbm.sbs_per_mb;
    let mut new_nb_bits: c_ulong = (old_nb_mb + 1) * vm.sbm.sbs_per_mb;
    let mut old_pages: c_int = PFN_UP(BITS_TO_LONGS(old_nb_bits) * sizeof(long));
    let mut new_pages: c_int = PFN_UP(BITS_TO_LONGS(new_nb_bits) * sizeof(long));
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_could_add_memory(vm: *mut virtio_mem, size: u64) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_sbm_add_mb(vm: *mut virtio_mem, mb_id: c_ulong) -> c_int {
    static int virtio_mem_sbm_add_mb(struct virtio_mem *vm, unsigned long mb_id)
    {
    let mut addr: u64 = virtio_mem_mb_id_to_phys(mb_id);
    let mut size: u64 = memory_block_size_bytes();
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_bbm_add_bb(vm: *mut virtio_mem, bb_id: c_ulong) -> c_int {
    static int virtio_mem_bbm_add_bb(struct virtio_mem *vm, unsigned long bb_id)
    {
    let mut addr: u64 = virtio_mem_bb_id_to_phys(vm, bb_id);
    let mut size: u64 = vm.bbm.bb_size;
    return virtio_mem_add_memory(vm, addr, size);
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_sbm_remove_mb(vm: *mut virtio_mem, mb_id: c_ulong) -> c_int {
...
//
    ...
    let mut rc: return = = -ENOMEM ? -ENOMEM : -EBUSY;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_retry(vm: *mut virtio_mem) {
    static void virtio_mem_retry(struct virtio_mem *vm)
    {
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_translate_node_id(vm: *mut virtio_mem, node_id: u16) -> c_int {
...
//
    ...
    {
    let mut is_movable: bool = is_zone_movable_page(pfn_to_page(start_pfn));
    int new_state;
    switch (virtio_mem_sbm_get_mb_state(vm, mb_id)) {
    case VIRTIO_MEM_SBM_MB_OFFLINE_PARTIAL:
    ...
    unsigned long mb_id)
    {
    let mut nr_pages: c_ulong = PFN_DOWN(vm.sbm.sb_size);
    unsigned long pfn;
    int sb_id;
...
//
    ...
    let mut start: c_ulong = PFN_PHYS(mhp.start_pfn);
    let mut size: c_ulong = PFN_PHYS(mhp.nr_pages);
    let mut rc: c_int = NOTIFY_OK;
    unsigned long id;
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_fake_online(pfn: c_ulong, nr_pages: c_ulong) {
    static void virtio_mem_fake_online(unsigned long pfn, unsigned long nr_pages)
    {
    let mut order: c_ulong = MAX_PAGE_ORDER;
    unsigned long i;
//
...
//
    ...
    unsigned long nr_pages)
    {
    let mut is_movable: bool = is_zone_movable_page(pfn_to_page(pfn));
    int rc, retry_count;
//
...
// whoops, out of memory
    return rc;
#[no_mangle]
pub unsafe extern "C" fn if(!is_movable: rc &&) -> else {
...
//
    ...
    {
    let mut start: c_ulong = page_to_phys(page);
    let mut end: c_ulong = start + PFN_PHYS(1 << order);
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_online_page_cb(page: *mut page, order: c_uint) {
    static void virtio_mem_online_page_cb(struct page *page, unsigned int order)
    {
...
// wait for a response
    ...
    uint64_t size)
    {
    let mut nb_vm_blocks: u64 = size / vm.device_block_size;
    const struct virtio_mem_req req = {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_send_unplug_all_request(vm: *mut virtio_mem) -> c_int {
...
//
    ...
    sb_id * vm.sbm.sb_size;
    let mut size: u64 = count * vm.sbm.sb_size;
    int rc;
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_bbm_unplug_bb(vm: *mut virtio_mem, bb_id: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_bbm_plug_bb(vm: *mut virtio_mem, bb_id: c_ulong) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_sbm_unplug_mb(vm: *mut virtio_mem, mb_id: c_ulong) -> c_int {
    static int virtio_mem_sbm_unplug_mb(struct virtio_mem *vm, unsigned long mb_id)
    {
    let mut nb_sb: u64 = vm.sbm.sbs_per_mb;
...
//
    ...
    unsigned long mb_id, uint64_t *nb_sb)
    {
    let mut count: c_int = min_t(int, *nb_sb, vm.sbm.sbs_per_mb);
    int rc;
...
// Add the memory block to linux - if that fails, try to unplug.
    ...
    if (rc) {
    let mut new_state: c_int = VIRTIO_MEM_SBM_MB_UNUSED;
...
//
    ...
    unsigned long mb_id, uint64_t *nb_sb)
    {
    let mut old_state: c_int = virtio_mem_sbm_get_mb_state(vm, mb_id);
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_sbm_plug_request(vm: *mut virtio_mem, diff: u64) -> c_int {
    ...
    let mut nb_sb: u64 = diff / vm.sbm.sb_size;
    unsigned long mb_id;
    int rc, i;
    if (!nb_sb)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_bbm_plug_request(vm: *mut virtio_mem, diff: u64) -> c_int {
    static int virtio_mem_bbm_plug_request(struct virtio_mem *vm, uint64_t diff)
    {
    let mut nb_bb: u64 = diff / vm.bbm.bb_size;
    unsigned long bb_id;
    int rc;
    if (!nb_bb)
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_plug_request(vm: *mut virtio_mem, diff: u64) -> c_int {
...
//
    ...
    int count)
    {
    let mut nr_pages: c_ulong = PFN_DOWN(vm.sbm.sb_size) * count;
    let mut old_state: c_int = virtio_mem_sbm_get_mb_state(vm, mb_id);
    unsigned long start_pfn;
    int rc;
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_sbm_unplug_request(vm: *mut virtio_mem, diff: u64) -> c_int {
...
//
    ...
    unsigned long bb_id)
    {
    let mut start_pfn: c_ulong = PFN_DOWN(virtio_mem_bb_id_to_phys(vm, bb_id));
    let mut nr_pages: c_ulong = PFN_DOWN(vm.bbm.bb_size);
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_bbm_unplug_request(vm: *mut virtio_mem, diff: u64) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_unplug_request(vm: *mut virtio_mem, diff: u64) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_cleanup_pending_mb(vm: *mut virtio_mem) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_refresh_config(vm: *mut virtio_mem) {
    static void virtio_mem_refresh_config(struct virtio_mem *vm)
    {
    let mut pluggable_range: range = mhp_get_pluggable_range(true);
...
//
#[no_mangle]
unsafe extern "C" fn virtio_mem_run_wq(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_timer_expired(timer: *mut hrtimer) -> enum hrtimer_restart {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_handle_response(vq: *mut virtqueue) {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_init_vq(vm: *mut virtio_mem) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_init_hotplug(vm: *mut virtio_mem) -> c_int {
...
// On any kind of error, we're going to signal !ram.
    ...
    struct virtio_mem *vm = container_of(cb, struct virtio_mem,
    vmcore_cb);
    let mut device_start: u64 = vm.addr;
    let mut device_end: u64 = vm.addr + vm.usable_region_size;
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_init_kdump(vm: *mut virtio_mem) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_init(vm: *mut virtio_mem) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_create_resource(vm: *mut virtio_mem) -> c_int {
    static int virtio_mem_create_resource(struct virtio_mem *vm)
    {
//
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_delete_resource(vm: *mut virtio_mem) {
    static void virtio_mem_delete_resource(struct virtio_mem *vm)
    {
    const char *name;
    if (!vm.parent_resource)
    ...
    kfree(name);
    vm.parent_resource = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn virtio_mem_range_has_system_ram(res: *mut resource, arg: *mut c_void) -> c_int {
    static int virtio_mem_range_has_system_ram(struct resource *res, void *arg)
    {
    return 1;
    }
#[no_mangle]
unsafe extern "C" fn virtio_mem_has_memory_added(vm: *mut virtio_mem) -> bool {
    ...
    vm.addr + vm.region_size, core::ptr::null_mut(),
    virtio_mem_range_has_system_ram) == 1;
    }
#[no_mangle]
unsafe extern "C" fn virtio_mem_probe(vdev: *mut virtio_device) -> c_int {
...
// trigger a config update to start processing the requested_size
    ...
    kfree(vm);
    vdev.priv = core::ptr::null_mut();
    return rc;
    }
#[no_mangle]
unsafe extern "C" fn virtio_mem_deinit_hotplug(vm: *mut virtio_mem) {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_deinit_kdump(vm: *mut virtio_mem) {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_remove(vdev: *mut virtio_device) {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_config_changed(vdev: *mut virtio_device) {
...
#[no_mangle]
unsafe extern "C" fn virtio_mem_freeze(vdev: *mut virtio_device) -> c_int {
...
//
    ...
    virtio_reset_device(vdev);
    vdev.config.del_vqs(vdev);
    vm.vq = core::ptr::null_mut();
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn virtio_mem_restore(vdev: *mut virtio_device) -> c_int {
