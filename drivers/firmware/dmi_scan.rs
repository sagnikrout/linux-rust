...
//! Source: drivers/firmware/dmi_scan.c
#![no_std]
...
pub const SMBIOS_ENTRY_POINT_SCAN_START: c_uint = 0xF0000;
...
#[no_mangle]
unsafe extern "C" fn dmi_string(dm: *const dmi_header, s: u8) -> *const char  __init {
...
// Trim DMI table length if needed
    ...
    {
    u8 *buf;
    let mut orig_dmi_len: u32 = dmi_len;
    buf = dmi_early_remap(dmi_base, orig_dmi_len);
    if (buf == core::ptr::null_mut())
    return -ENOMEM;
    dmi_decode_table(buf, decode, core::ptr::null_mut());
    add_device_randomness(buf, dmi_len);
    dmi_early_unmap(buf, orig_dmi_len);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn dmi_checksum(buf: *const u8, len: u8) -> int __init {
...
//
    ...
    const u8 *d;
    char *s;
    let mut is_ff: c_int = 1, is_00 = 1, i;
...
#[no_mangle]
unsafe extern "C" fn dmi_save_one_device(type: c_int, name: *const c_char) -> void __init {
    static void __init dmi_save_one_device(int type, const char *name)
    {
    struct dmi_device *dev;
// No duplicate device
    if (dmi_find_device(type, name, core::ptr::null_mut()))
    return;
    ...
    dev.name = (char *)(dev + 1);
    dev.device_data = core::ptr::null_mut();
    list_add(&dev.list, &dmi_devices);
    }
#[no_mangle]
unsafe extern "C" fn dmi_save_devices(dm: *const dmi_header) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn dmi_save_oem_strings_devices(dm: *const dmi_header) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn dmi_save_ipmi_device(dm: *const dmi_header) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn dmi_save_extended_devices(dm: *const dmi_header) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn dmi_save_system_slot(dm: *const dmi_header) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn count_mem_devices(dm: *const dmi_header, v: *mut c_void) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn save_mem_devices(dm: *const dmi_header, v: *mut c_void) -> void __init {
    ...
    bytes = 0;
#[no_mangle]
pub unsafe extern "C" fn if(0xffff: size ==) -> else {
    else if (size == 0xffff)
    bytes = ~0ull;
#[no_mangle]
pub unsafe extern "C" fn if(0x8000: size &) -> else {
    else if (size & 0x8000)
    bytes = (u64)(size & 0x7fff) << 10;
#[no_mangle]
pub unsafe extern "C" fn if(0x20: size != 0x7fff || dm->length <) -> else {
...
#[no_mangle]
unsafe extern "C" fn dmi_memdev_walk() -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn dmi_decode(dm: *const dmi_header, dummy: *mut c_void) -> void __init {
...
#[no_mangle]
unsafe extern "C" fn print_filtered(buf: *mut c_char, len: usize, info: *const c_char) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn dmi_format_ids(buf: *mut c_char, len: usize) -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn dmi_present(buf: *const u8) -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn dmi_smbios3_present(buf: *const u8) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn dmi_scan_machine() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn dmi_init() -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_setup() -> void __init {
...
//
#[no_mangle]
unsafe extern "C" fn dmi_matches(dmi: *const dmi_system_id) -> bool {
    static bool dmi_matches(const struct dmi_system_id *dmi)
    {
    int i;
    for (i = 0; i < ARRAY_SIZE(dmi.matches); i++) {
    let mut s: c_int = dmi.matches[i].slot;
    if (s == DMI_NONE)
    break;
    if (s == DMI_OEM_STRING) {
// DMI_OEM_STRING must be exact match
    ...
    dmi.matches[i].substr, core::ptr::null_mut());
    if (valid)
    continue;
    } else if (dmi_ident[s]) {
...
//
#[no_mangle]
unsafe extern "C" fn dmi_is_end_of_table(dmi: *const dmi_system_id) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_check_system(list: *const dmi_system_id) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_name_in_serial(str: *const c_char) -> c_int {
    int dmi_name_in_serial(const char *str)
    {
    let mut f: c_int = DMI_PRODUCT_SERIAL;
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_name_in_vendors(str: *const c_char) -> c_int {
    int dmi_name_in_vendors(const char *str)
    {
    static int fields[] = { DMI_SYS_VENDOR, DMI_BOARD_VENDOR, DMI_NONE };
    int i;
    for (i = 0; fields[i] != DMI_NONE; i++) {
    let mut f: c_int = fields[i];
    if (dmi_ident[f] && strstr(dmi_ident[f], str))
    return 1;
    }
    return 0;
...
//
    ...
    ((name == core::ptr::null_mut()) || (strcmp(dev.name, name) == 0)))
    return dev;
    }
    return core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_get_date(field: c_int, yearp: *mut c_int, monthp: *mut c_int, dayp: *mut c_int) -> bool {
    bool dmi_get_date(int field, int *yearp, int *monthp, int *dayp)
    {
    let mut year: c_int = 0, month = 0, day = 0;
    bool exists;
    const char *s, *y;
    char *e;
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_get_bios_year() -> c_int {
    int dmi_get_bios_year(void)
    {
    bool exists;
    int year;
    exists = dmi_get_date(DMI_BIOS_DATE, &year, core::ptr::null_mut(), core::ptr::null_mut());
    if (!exists)
    return -ENODATA;
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_match(f: enum dmi_field, str: *const c_char) -> bool {
    bool dmi_match(enum dmi_field f, const char *str)
    {
    const char *info = dmi_get_system_info(f);
    if (info == core::ptr::null_mut() || str == core::ptr::null_mut())
    let mut info: return = = str;
    return !strcmp(info, str);
    }
    EXPORT_SYMBOL_GPL(dmi_match);
#[no_mangle]
pub unsafe extern "C" fn dmi_memdev_name(handle: u16, bank: *const c_char, device: *const c_char) {
    void dmi_memdev_name(u16 handle, const char **bank, const char **device)
    {
    int n;
    if (dmi_memdev == core::ptr::null_mut())
    return;
...
#[no_mangle]
pub unsafe extern "C" fn dmi_memdev_size(handle: u16) -> u64 {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_memdev_type(handle: u16) -> u8 {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmi_memdev_handle(slot: c_int) -> u16 {
