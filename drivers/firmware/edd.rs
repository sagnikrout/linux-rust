...
pub struct edd_attribute {
    pub attr: attribute,
    pub buf): *mut *mut *mut ssize_t(show) (struct edd_device  edev, char,
    pub edev): *mut *mut *mut int (test) (struct edd_device,
}
...
//
    ...
    {
    struct edd_info *info;
    let mut len: isize = sizeof (info.params);
    if (!edev)
    return -EINVAL;
    info = edd_dev_get_info(edev);
    if (!info || !buf)
...
//
#[no_mangle]
unsafe extern "C" fn edd_release(kobj: *mut *mut kobject) {
    static void edd_release(struct kobject * kobj)
    {
    let mut dev: *mut edd_device = to_edd_device(kobj);
    kfree(dev);
    }
...
#[no_mangle]
unsafe extern "C" fn edd_populate_dir(edev: *mut *mut edd_device) {
    ...
    error = kobject_init_and_add(&edev.kobj, &edd_ktype, core::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn edd_num_devices() -> c_int {
...
//
    ...
    edd_kset = kset_create_and_add("edd", core::ptr::null_mut(), firmware_kobj);
    if (!edd_kset)
    return -ENOMEM;
