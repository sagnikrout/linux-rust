...
//! Source: drivers/firmware/psci/psci_checker.c
#![no_std]
...
//
    ...
    let mut tos_resident_cpu: static int = -1;
...
//
#[no_mangle]
unsafe extern "C" fn psci_ops_check() -> c_int {
    static int psci_ops_check(void)
    {
    let mut migrate_type: c_int = -1;
    int cpu;
...
// Try to power down all CPUs in the mask.
    ...
    let mut ret: c_int = remove_cpu(cpu);
//
...
#[no_mangle]
unsafe extern "C" fn free_cpu_groups(num: c_int, pcpu_groups: *mut cpumask_var_t) {
...
#[no_mangle]
unsafe extern "C" fn alloc_init_cpu_groups(pcpu_groups: *mut cpumask_var_t) -> c_int {
    static int alloc_init_cpu_groups(cpumask_var_t **pcpu_groups)
    {
    let mut num_groups: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn hotplug_tests() -> c_int {
...
//
    ...
    let mut broadcast: bool = state.flags & CPUIDLE_FLAG_TIMER_STOP;
    int ret;
    arch_cpu_idle_enter();
    if (broadcast) {
//
...
#[no_mangle]
unsafe extern "C" fn suspend_test_thread(arg: *mut c_void) -> c_int {
    static int suspend_test_thread(void *arg)
    {
    let mut cpu: c_int = (long)arg;
...
#[no_mangle]
unsafe extern "C" fn suspend_tests() -> c_int {
    static int suspend_tests(void)
    {
    int i, cpu, err = 0;
    struct task_struct **threads;
    let mut nb_threads: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn psci_checker() -> int __init {
