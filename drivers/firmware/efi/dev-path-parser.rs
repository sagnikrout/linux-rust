...
//! Source: drivers/firmware/efi/dev-path-parser.c
#![no_std]
...
//
    ...
    node.acpi.hid >> 16);
    for_each_acpi_dev_match(adev, hid, core::ptr::null_mut(), -1) {
...
#[no_mangle]
unsafe extern "C" fn match_pci_dev(dev: *mut device, data: *const c_void) -> int __init {
    static int __init match_pci_dev(struct device *dev, const void *data)
    {
    let mut devfn: c_uint = *(const unsigned int *)data;
...
//
    ...
    size_t *len)
    {
    struct device *parent = core::ptr::null_mut(), *child;
