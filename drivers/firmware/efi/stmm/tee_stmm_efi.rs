...
//! Source: drivers/firmware/efi/stmm/tee_stmm_efi.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tee_stmm_efi_private {
...
#[no_mangle]
unsafe extern "C" fn tee_ctx_match(ver: *mut tee_ioctl_version_data, data: *const c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tee_mm_communicate(comm_buf: *mut c_void, dsize: usize) -> efi_status_t {
...
//
#[no_mangle]
unsafe extern "C" fn mm_communicate(comm_buf: *mut u8, payload_size: usize) -> efi_status_t {
...
//
    ...
    {
    let mut mm_var_guid: efi_guid_t = EFI_MM_VARIABLE_GUID;
    struct efi_mm_communicate_header *mm_hdr;
    struct smm_variable_communicate_header *var_hdr;
    u8 *comm_buf;
...
//
#[no_mangle]
unsafe extern "C" fn get_max_payload(size: *mut usize) -> efi_status_t {
    static efi_status_t get_max_payload(size_t *size)
    {
    struct smm_variable_payload_size *var_payload = core::ptr::null_mut();
    size_t payload_size;
    u8 *comm_buf = core::ptr::null_mut();
    efi_status_t ret;
    if (!size)
    return EFI_INVALID_PARAMETER;
...
#[no_mangle]
unsafe extern "C" fn tee_stmm_efi_close_context(data: *mut c_void) {
    static void tee_stmm_efi_close_context(void *data)
    {
    tee_client_close_context(pvt_data.ctx);
    }
#[no_mangle]
unsafe extern "C" fn tee_stmm_efi_close_session(data: *mut c_void) {
    static void tee_stmm_efi_close_session(void *data)
    {
    tee_client_close_session(pvt_data.ctx, pvt_data.session);
    }
#[no_mangle]
unsafe extern "C" fn tee_stmm_restore_efivars_generic_ops() {
...
#[no_mangle]
unsafe extern "C" fn tee_stmm_efi_probe(tee_dev: *mut tee_client_device) -> c_int {
    ...
    efi_status_t ret;
    int rc;
    pvt_data.ctx = tee_client_open_context(core::ptr::null_mut(), tee_ctx_match, core::ptr::null_mut(), core::ptr::null_mut());
    if (IS_ERR(pvt_data.ctx))
    return -ENODEV;
    rc = devm_add_action_or_reset(dev, tee_stmm_efi_close_context, core::ptr::null_mut());
    if (rc)
    return rc;
// Open session with StMM PTA
    memset(&sess_arg, 0, sizeof(sess_arg));
    export_uuid(sess_arg.uuid, &tee_stmm_efi_id_table[0].uuid);
    rc = tee_client_open_session(pvt_data.ctx, &sess_arg, core::ptr::null_mut());
    ...
    pvt_data.dev = dev;
    rc = devm_add_action_or_reset(dev, tee_stmm_efi_close_session, core::ptr::null_mut());
    if (rc)
    return rc;
...
#[no_mangle]
unsafe extern "C" fn tee_stmm_efi_remove(dev: *mut tee_client_device) {
