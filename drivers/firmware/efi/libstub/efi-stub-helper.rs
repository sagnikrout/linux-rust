...
//! Source: drivers/firmware/efi/libstub/efi-stub-helper.c
#![no_std]
...
//
    ...
    bool efi_nochunk;
    let mut efi_nokaslr: bool = !IS_ENABLED(CONFIG_RANDOMIZE_BASE);
    bool efi_novamap;
    static bool efi_noinitrd;
    static bool efi_nosoftreserve;
    let mut efi_disable_pci_dma: static bool = IS_ENABLED(CONFIG_EFI_DISABLE_PCI_DMA);
    int efi_mem_encrypt;
#[no_mangle]
...
//
#[no_mangle]
pub unsafe extern "C" fn efi_parse_options(cmdline: *const c_char) -> efi_status_t {
    efi_status_t efi_parse_options(char const *cmdline)
    {
    char *buf __free(efi_pool) = core::ptr::null_mut();
    efi_status_t status;
    size_t len;
    char *str;
    if (!cmdline)
    ...
    efi_mem_encrypt = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: parse_option_str(val, _arg: "off")) -> else {
...
// Scan file_path_list.
    ...
    dest.optional_data = size ? pos : core::ptr::null_mut();
    return true;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn efi_apply_loadoptions_quirk(load_options: *const c_void, load_options_size: *mut u32) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct efistub_measured_event {
    pub event_data: union efistub_event,
    pub __packed: TCG_PCClientTaggedEvent tagged_event,
}
    ...
    struct efistub_measured_event *evt __free(efi_pool) = core::ptr::null_mut();
    int size = struct_size(evt, tagged_event.tagged_event_data,
    events[event].event_data_len);
    let mut tcg2_guid: efi_guid_t = EFI_TCG2_PROTOCOL_GUID;
    efi_tcg2_protocol_t *tcg2 = core::ptr::null_mut();
    union efistub_event ev;
    efi_status_t status;
    void *protocol;
    efi_bs_call(locate_protocol, &tcg2_guid, core::ptr::null_mut(), (void **)&tcg2);
    if (tcg2) {
    ...
    efi_bs_call(locate_protocol, &cc_guid, core::ptr::null_mut(), (void **)&cc);
    if (!cc)
...
//
    ...
    let mut options_size: u32 = efi_table_attr(image, load_options_size);
    int options_bytes = 0, safe_options_bytes = 0;  /* UTF-8 bytes */
    let mut cmdline_addr: c_ulong = 0;
    ...
    in_quote = !in_quote;
#[no_mangle]
pub unsafe extern "C" fn if(isspace((char)c): !in_quote &&) -> else {
...
//
    ...
    {
    let mut tables: c_ulong = efi_table_attr(efi_system_table, tables);
    let mut nr_tables: c_int = efi_table_attr(efi_system_table, nr_tables);
    int i;
...
//
    ...
    unsigned long max)
    {
    let mut lf2_proto_guid: efi_guid_t = EFI_LOAD_FILE2_PROTOCOL_GUID;
    ...
    return status;
    initrd.size = 0;
    status = efi_call_proto(lf2, load_file, dp, false, &initrd.size, core::ptr::null_mut());
...
//
    ...
    {
    let mut tbl_guid: efi_guid_t = LINUX_EFI_INITRD_MEDIA_GUID;
    let mut status: efi_status_t = EFI_SUCCESS;
...
//
#[no_mangle]
pub unsafe extern "C" fn efi_wait_for_key(usec: c_ulong, key: *mut efi_input_key_t) -> efi_status_t {
    ...
    status = efi_bs_call(create_event, EFI_EVT_TIMER, 0, core::ptr::null_mut(), core::ptr::null_mut(), &timer);
    if (status != EFI_SUCCESS)
    return status;
...
//
    ...
    {
    let mut guid: efi_guid_t = EFI_MEMORY_ATTRIBUTE_PROTOCOL_GUID;
...
//
    status = efi_bs_call(locate_protocol, &guid, core::ptr::null_mut(), (void **)&memattr);
    if (status != EFI_SUCCESS)
    return;
