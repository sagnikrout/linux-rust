...
//! Source: drivers/firmware/efi/libstub/file.c
#![no_std]
...
pub const MAX_FILENAME_SIZE: c_int = 256;
//
...
#[derive(Copy, Clone)]
pub struct finfo {
    pub info: efi_file_info_t,
    pub filename: [efi_char16_t; MAX_FILENAME_SIZE],
}
...
// file_size = fi->info.file_size;
    ...
    {
    let mut prefix_len: c_int = prefix_size / 2;
...
// Skip any leading slashes
    ...
    let mut c: efi_char16_t = cmdline[i++];
    if (c == L'\0' || c == L'\n' || c == L' ')
    break;
// result++ = c;
    }
// result = L'\0';
    ...
    static efi_device_path_from_text_protocol_t *text_to_dp = core::ptr::null_mut();
    let mut fs_proto: efi_guid_t = EFI_FILE_SYSTEM_GUID;
...
// See if the text to device path protocol exists
    if (!text_to_dp &&
    efi_bs_call(locate_protocol, &text_to_dp_guid, core::ptr::null_mut(),
...
// Macro flag: #define CONFIG_CMDLINE
...
//
    ...
    let mut cmdline_len: u32 = efi_table_attr(image, load_options_size);
    let mut efi_chunk_size: c_ulong = ULONG_MAX;
    efi_file_protocol_t *volume = core::ptr::null_mut();
...
//
    ...
    let mut old_addr: c_ulong = alloc_addr;
...
//
    ...
    alloc_size += size;
    while (size) {
    let mut chunksize: c_ulong = min(size, efi_chunk_size);
