...
//! Source: drivers/firmware/efi/libstub/random.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn efi_get_random_bytes(size: c_ulong, out: *mut u8) -> efi_status_t {
    efi_status_t efi_get_random_bytes(unsigned long size, u8 *out)
    {
    let mut rng_proto: efi_guid_t = EFI_RNG_PROTOCOL_GUID;
    efi_status_t status;
    efi_rng_protocol_t *rng = core::ptr::null_mut();
    status = efi_bs_call(locate_protocol, &rng_proto, core::ptr::null_mut(), (void **)&rng);
    if (status != EFI_SUCCESS)
    return status;
    return efi_call_proto(rng, get_rng, core::ptr::null_mut(), size, out);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn efi_random_get_seed() -> efi_status_t {
    efi_status_t efi_random_get_seed(void)
    {
    let mut rng_proto: efi_guid_t = EFI_RNG_PROTOCOL_GUID;
    let mut rng_algo_raw: efi_guid_t = EFI_RNG_ALGORITHM_RAW;
    let mut rng_table_guid: efi_guid_t = LINUX_EFI_RANDOM_SEED_TABLE_GUID;
    struct linux_efi_random_seed *prev_seed, *seed = core::ptr::null_mut();
    let mut prev_seed_size: c_int = 0, seed_size = EFI_RANDOM_SEED_SIZE;
    let mut nv_seed_size: c_ulong = 0, offset = 0;
    efi_rng_protocol_t *rng = core::ptr::null_mut();
    efi_status_t status;
    status = efi_bs_call(locate_protocol, &rng_proto, core::ptr::null_mut(), (void **)&rng);
    if (status != EFI_SUCCESS)
...
// Call GetVariable() with a zero length buffer to obtain the size
    get_efi_var(L"RandomSeed", &rng_table_guid, core::ptr::null_mut(), &nv_seed_size, core::ptr::null_mut());
...
//
    ...
    if (nv_seed_size) {
    status = get_efi_var(L"RandomSeed", &rng_table_guid, core::ptr::null_mut(),
