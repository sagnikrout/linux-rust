...
//! Source: drivers/firmware/efi/libstub/mem.c
#![no_std]
...
//
    ...
    {
    struct efi_boot_memmap tmp, *m __free(efi_pool) = core::ptr::null_mut();
    ...
    unsigned long size;
    tmp.map_size = 0;
    status = efi_bs_call(get_memory_map, &tmp.map_size, core::ptr::null_mut(), &tmp.map_key,
...
//
    ...
    if (status != EFI_SUCCESS) {
    if (install_cfg_tbl)
    efi_bs_call(install_configuration_table, &tbl_guid, core::ptr::null_mut());
    return status;
    }
// map = no_free_ptr(m);
    return EFI_SUCCESS;
...
//
#[no_mangle]
pub unsafe extern "C" fn efi_free(size: c_ulong, addr: c_ulong) {
