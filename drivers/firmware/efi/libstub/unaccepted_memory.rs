...
//! Source: drivers/firmware/efi/libstub/unaccepted_memory.c
#![no_std]
...
// SPDX-License-Identifier: GPL-2.0-only
    ...
    {
    let mut unaccepted_table_guid: efi_guid_t = LINUX_EFI_UNACCEPTED_MEM_TABLE_GUID;
    let mut unaccepted_start: u64 = ULLONG_MAX, unaccepted_end = 0, bitmap_size;
    efi_status_t status;
    int i;
...
// Check if there's any unaccepted memory and find the max address
    for (i = 0; i < nr_desc; i++) {
    efi_memory_desc_t *d;
    let mut m: c_ulong = (unsigned long)map.map;
...
//
#[no_mangle]
pub unsafe extern "C" fn process_unaccepted_memory(start: u64, end: u64) {
    void process_unaccepted_memory(u64 start, u64 end)
    {
    let mut unit_size: u64 = unaccepted_table.unit_size;
    let mut unit_mask: u64 = unaccepted_table.unit_size - 1;
    let mut bitmap_size: u64 = unaccepted_table.size;
//
...
#[no_mangle]
pub unsafe extern "C" fn accept_memory(start: phys_addr_t, size: c_ulong) {
