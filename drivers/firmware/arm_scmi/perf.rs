...
//! Source: drivers/firmware/arm_scmi/perf.c
#![no_std]
...
pub const SCMI_PROTOCOL_SUPPORTED_VERSION: c_uint = 0x40000;
pub const MAX_OPPS: c_int = 64;
...
pub struct scmi_opp {
    pub perf: u32,
    pub power: u32,
    pub trans_latency_us: u32,
    pub indicative_freq: u32,
    pub level_index: u32,
    pub hash: hlist_node,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_resp_perf_attributes {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_resp_perf_domain_attributes {
    pub flags: __le32,

    pub rate_limit_us: __le32,
    pub sustained_freq_khz: __le32,
    pub sustained_perf_level: __le32,
    pub name: [u8; SCMI_SHORT_NAME_MAX_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_perf_describe_levels {
    pub domain: __le32,
    pub level_index: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_perf_set_limits {
    pub domain: __le32,
    pub max_level: __le32,
    pub min_level: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_perf_get_limits {
    pub max_level: __le32,
    pub min_level: __le32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_perf_set_level {
    pub domain: __le32,
    pub level: __le32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_perf_notify_level_or_limits {
    pub domain: __le32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_perf_limits_notify_payld {
    pub agent_id: __le32,
    pub domain_id: __le32,
    pub range_max: __le32,
    pub range_min: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_perf_level_notify_payld {
    pub agent_id: __le32,
    pub domain_id: __le32,
    pub performance_level: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_resp_perf_describe_levels {
    pub num_returned: __le16,
    pub num_remaining: __le16,
    struct {
    pub perf_val: __le32,
    pub power: __le32,
    pub transition_latency_us: __le16,
    pub reserved: __le16,
    pub opp: [}; ],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_resp_perf_describe_levels_v4 {
    pub num_returned: __le16,
    pub num_remaining: __le16,
    struct {
    pub perf_val: __le32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct perf_dom_info {
    pub id: u32,
    pub set_limits: bool,
    pub perf_limit_notify: bool,
    pub perf_level_notify: bool,
    pub perf_fastchannels: bool,
    pub level_indexing_mode: bool,
    pub opp_count: u32,
    pub rate_limit_us: u32,
    pub sustained_freq_khz: u32,
    pub sustained_perf_level: u32,
    pub mult_factor: c_ulong,
    pub info: scmi_perf_domain_info,
    pub opp: [scmi_opp; MAX_OPPS],
    pub fc_info: *mut scmi_fc_info,
    pub opps_by_idx: xarray,
    pub opps_by_lvl: xarray,
    pub ilog2(MAX_OPPS)): DECLARE_HASHTABLE(opps_by_freq,,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_perf_info {
    pub num_domains: u16,
    pub power_scale: enum scmi_power_scale,
    pub stats_addr: u64,
    pub stats_size: u32,
    pub notify_lvl_cmd: bool,
    pub notify_lim_cmd: bool,
    pub dom_info: *mut perf_dom_info,
}
    ...
    if (!ret) {
    let mut flags: u16 = le16_to_cpu(attr.flags);
    ...
    if (!ret) {
    if (!ph.hops.protocol_msg_check(ph, PERF_NOTIFY_LEVEL, core::ptr::null_mut()))
    pi.notify_lvl_cmd = true;
    if (!ph.hops.protocol_msg_check(ph, PERF_NOTIFY_LIMITS, core::ptr::null_mut()))
    pi.notify_lim_cmd = true;
    }
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn scmi_perf_xa_destroy(data: *mut c_void) {
...
//
    ...
    dom_info.id, core::ptr::null_mut(), dom_info.info.name,
    SCMI_MAX_STR_SIZE);
...
// Skip BAD duplicates received from firmware
    if (ret)
    let mut ret: return = = -EBUSY ? 0 : ret;
    perf_dom.opp_count++;
    ...
    sizeof(struct scmi_opp), opp_cmp_func, core::ptr::null_mut());
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn scmi_perf_num_domains_get(ph: *const scmi_protocol_handle) -> c_int {
...
// level = opp->perf;
    ...
    &fc[PERF_FC_LEVEL].get_addr, core::ptr::null_mut(),
    &fc[PERF_FC_LEVEL].rate_limit);
    ph.hops.fastchannel_init(ph, PERF_DESCRIBE_FASTCHANNEL,
    PERF_LIMITS_GET, 8, dom.id,
    &fc[PERF_FC_LIMIT].get_addr, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn scmi_perf_get_num_sources(ph: *const scmi_protocol_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_perf_protocol_init(ph: *const scmi_protocol_handle) -> c_int {
