...
//! Source: drivers/firmware/arm_scmi/driver.c
#![no_std]
...
pub struct scmi_xfers_info {
    pub xfer_alloc_table: *mut c_ulong,
    pub xfer_lock: spinlock_t,
    pub max_msg: c_int,
    pub free_xfers: hlist_head,
    pub SCMI_PENDING_XFERS_HT_ORDER_SZ): DECLARE_HASHTABLE(pending_xfers,,
}
...
// Ensure mutual exclusive access to protocols instance array
    pub protocols_mtx: mutex,
    pub protocols_imp: *mut u8,
    pub active_protocols: idr,
    pub notify_priv: *mut c_void,
    pub node: list_head,
    pub users: c_int,
    pub bus_nb: notifier_block,
    pub dev_req_nb: notifier_block,
// Serialize device creation process for this instance
    pub devreq_mtx: mutex,
...
// vendor_id/sub_vendor_id guaranteed <= SCMI_SHORT_NAME_MAX_SIZE
    ...
    {
    unsigned long key;
    struct scmi_protocol *proto = core::ptr::null_mut();
    ...
    char *sub_vendor_id, u32 impl_ver)
    {
    const struct scmi_protocol *proto = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn scmi_protocol_put(proto: *const scmi_protocol) {
...
#[no_mangle]
unsafe extern "C" fn scmi_vendor_protocol_check(proto: *const scmi_protocol) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn scmi_protocol_register(proto: *const scmi_protocol) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn scmi_protocol_unregister(proto: *const scmi_protocol) {
...
//
#[no_mangle]
pub unsafe extern "C" fn scmi_xfer_raw_put(handle: *const scmi_handle, xfer: *mut scmi_xfer) {
...
//
    ...
    {
    struct scmi_xfer *xfer = core::ptr::null_mut();
...
//
    ...
    MSG_XTRACT_TOKEN(msg_hdr), err, core::ptr::null_mut(), 0);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn scmi_xfer_state_update(xfer: *mut scmi_xfer, msg_type: u8) {
...
//
    ...
    let mut msg_type: u8 = MSG_XTRACT_TYPE(msg_hdr);
    let mut xfer_id: u16 = MSG_XTRACT_TOKEN(msg_hdr);
...
//
    ...
    let mut ooo: bool = false;
//
// Poll on xfer using transport provided .poll_done();
// assumes no completion interrupt was available.
//
    let mut stop: ktime_t = ktime_add_ms(ktime_get(), timeout_ms);
...
//
    ...
    xfer.async_done = core::ptr::null_mut();
    return ret;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn version_get(ph: *const scmi_protocol_handle, version: *mut u32) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_resp_domain_name_get {
    pub flags: __le32,
    pub name: [u8; SCMI_MAX_STR_SIZE],
}
...
//
#[no_mangle]
unsafe extern "C" fn scmi_common_get_max_msg_size(ph: *const scmi_protocol_handle) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_iterator {
    pub msg: *mut c_void,
    pub resp: *mut c_void,
    pub t: *mut scmi_xfer,
    pub ph: *const scmi_protocol_handle,
    pub ops: *mut scmi_iterator_ops,
    pub state: scmi_iterator_state,
    pub priv: *mut c_void,
}
...
#[no_mangle]
unsafe extern "C" fn __scmi_iterator_run(iter: *mut c_void, start: *mut c_uint, end: *mut c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_iterator_bound_cleanup(iter: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn scmi_iterator_run(iter: *mut c_void) -> c_int {
    static int scmi_iterator_run(void *iter)
    {
    int ret;
    ret = __scmi_iterator_run(iter, core::ptr::null_mut(), core::ptr::null_mut());
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_get_fc_info {
    pub domain: __le32,
    pub message_id: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_msg_resp_desc_fc {
    pub attr: __le32,

    pub rate_limit: __le32,
    pub chan_addr_low: __le32,
    pub chan_addr_high: __le32,
    pub chan_size: __le32,
    pub db_addr_low: __le32,
    pub db_addr_high: __le32,
    pub db_set_lmask: __le32,
    pub db_set_hmask: __le32,
    pub db_preserve_lmask: __le32,
    pub db_preserve_hmask: __le32,
}
    ...
    void __iomem *addr;
    struct scmi_xfer *t;
    struct scmi_fc_db_info *db = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn scmi_common_fastchannel_db_ring(db: *mut scmi_fc_db_info) {
...
#[no_mangle]
pub unsafe extern "C" fn if(2: db->width ==) -> else {
    else if (db.width == 2)
    SCMI_PROTO_FC_RING_DB(16);
#[no_mangle]
pub unsafe extern "C" fn if(4: db->width ==) -> else {
    else if (db.width == 4)
...
//
#[no_mangle]
unsafe extern "C" fn scmi_protocol_version_negotiate(ph: *mut scmi_protocol_handle) -> c_int {
...
// At first check if NEGOTIATE_PROTOCOL_VERSION is supported ...
    ret = scmi_protocol_msg_check(ph, NEGOTIATE_PROTOCOL_VERSION, core::ptr::null_mut());
    if (ret)
    return ret;
...
// Protocol specific devres group
    gid = devres_open_group(handle.dev, core::ptr::null_mut(), GFP_KERNEL);
    if (!gid) {
...
//
#[no_mangle]
pub unsafe extern "C" fn scmi_protocol_acquire(handle: *const scmi_handle, protocol_id: u8) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn scmi_protocol_release(handle: *const scmi_handle, protocol_id: u8) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_protocol_devres {
    pub handle: *const scmi_handle,
    pub protocol_id: u8,
}

#[no_mangle]
unsafe extern "C" fn scmi_devm_release_protocol(dev: *mut device, res: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn scmi_devm_protocol_match(dev: *mut device, res: *mut c_void, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn scmi_devm_protocol_put(sdev: *mut scmi_device, protocol_id: u8) {
...
//
    ...
    {
    struct scmi_info *info;
    struct scmi_handle *handle = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn scmi_handle_put(handle: *const scmi_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_clear_handle(scmi_dev: *mut scmi_device) {
    static void scmi_clear_handle(struct scmi_device *scmi_dev)
    {
    if (!scmi_dev.handle)
    return;
    scmi_handle_put(scmi_dev.handle);
    scmi_dev.handle = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn scmi_set_handle(scmi_dev: *mut scmi_device) {
...
#[no_mangle]
unsafe extern "C" fn scmi_channels_max_msg_configure(sinfo: *mut scmi_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_xfer_info_init(sinfo: *mut scmi_info) -> c_int {
    ...
    struct scmi_chan_info *cinfo;
    struct idr *idr;
    struct scmi_device *tdev = core::ptr::null_mut();
...
// Destroy channel and device only if created by this call.
    ...
    int prot_id)
    {
    let mut ret: c_int = scmi_chan_setup(info, of_node, prot_id, true);
    if (!ret) {
...
//
#[no_mangle]
unsafe extern "C" fn scmi_channels_setup(info: *mut scmi_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_chan_destroy(id: c_int, p: *mut c_void, data: *mut c_void) -> c_int {
    ...
    cinfo.dev = core::ptr::null_mut();
    }
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn scmi_cleanup_channels(info: *mut scmi_info, idr: *mut idr) {
...
#[no_mangle]
unsafe extern "C" fn scmi_cleanup_txrx_channels(info: *mut scmi_info) {
...
#[no_mangle]
unsafe extern "C" fn scmi_debugfs_common_cleanup(d: *mut c_void) {
    ...
    const char *c_ptr = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn scmi_debugfs_raw_mode_setup(info: *mut scmi_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_enable_matching_quirks(info: *mut scmi_info) {
...
#[no_mangle]
unsafe extern "C" fn scmi_probe(pdev: *mut platform_device) -> c_int {
    ...
    struct scmi_info *info;
    let mut coex: bool = IS_ENABLED(CONFIG_ARM_SCMI_RAW_MODE_SUPPORT_COEX);
...
//
    ...
    of_node_get(child);
    scmi_create_protocol_devices(child, info, prot_id, core::ptr::null_mut());
    }
    return 0;
    raw_mode_cleanup:
    if (IS_ENABLED(CONFIG_ARM_SCMI_RAW_MODE_SUPPORT))
...
#[no_mangle]
unsafe extern "C" fn scmi_remove(pdev: *mut platform_device) {
...
// Stop transport callbacks before tearing down notifications.
    ...
    {
    struct dentry *d;
    d = debugfs_create_dir("scmi", core::ptr::null_mut());
    if (IS_ERR(d)) {
...
#[no_mangle]
pub unsafe extern "C" fn scmi_inflight_count(handle: *const scmi_handle) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_driver_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn scmi_driver_exit() -> void __exit {
