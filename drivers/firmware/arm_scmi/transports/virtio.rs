...
//! Source: drivers/firmware/arm_scmi/transports/virtio.c
#![no_std]
...
pub const VIRTIO_MAX_RX_TIMEOUT_MS: c_int = 60000;

    ((ci).max_msg_size + SCMI_MSG_MAX_PROT_OVERHEAD)
pub const DESCRIPTORS_PER_TX_MSG: c_int = 2;
//
...
pub struct scmi_vio_msg {
    ...
    pub max_len: c_uint,
    pub poll_idx: c_uint,
    pub poll_status: enum poll_states,
...
#[no_mangle]
pub unsafe extern "C" fn scmi_vio_channel_acquire(vioch: *mut scmi_vio_channel) -> bool {
    static inline bool scmi_vio_channel_acquire(struct scmi_vio_channel *vioch)
    {
    return refcount_inc_not_zero(&vioch.users);
    }
#[no_mangle]
pub unsafe extern "C" fn scmi_vio_channel_release(vioch: *mut scmi_vio_channel) {
...
#[no_mangle]
unsafe extern "C" fn scmi_vio_channel_cleanup_sync(vioch: *mut scmi_vio_channel) {
...
// Cannot be kicked anymore after this...
    vioch.deferred_tx_wq = core::ptr::null_mut();
    spin_unlock_irqrestore(&vioch.lock, flags);
...
#[no_mangle]
pub unsafe extern "C" fn scmi_vio_msg_acquire(msg: *mut scmi_vio_msg) -> bool {
...
#[no_mangle]
unsafe extern "C" fn scmi_vio_have_vq_rx(vdev: *mut virtio_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn scmi_vio_complete_cb(vqueue: *mut virtqueue) {
...
#[no_mangle]
unsafe extern "C" fn scmi_vio_deferred_tx_worker(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn virtio_get_max_msg(base_cinfo: *mut scmi_chan_info) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn virtio_chan_available(of_node: *mut device_node, idx: c_int) -> bool {
    static bool virtio_chan_available(struct device_node *of_node, int idx)
    {
    struct scmi_vio_channel *channels, *vioch = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn scmi_destroy_tx_workqueue(deferred_tx_wq: *mut c_void) {
    ...
    {
    struct scmi_vio_channel *vioch;
    let mut index: c_int = tx ? VIRTIO_SCMI_VQ_TX : VIRTIO_SCMI_VQ_RX;
    int i;
    if (!scmi_vdev)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn virtio_chan_free(id: c_int, p: *mut c_void, data: *mut c_void) -> c_int {
...
// Ensure order between xfer->priv clear and vq feeding
    smp_store_mb(xfer.priv, core::ptr::null_mut());
    if (xfer.hdr.poll_completion)
    scmi_vio_msg_release(vioch, msg);
    scmi_vio_msg_release(vioch, msg);
    }
...
// Do not free timedout polled messages only if still inflight
    ...
    scmi_vio_msg_release(vioch, msg);
#[no_mangle]
pub unsafe extern "C" fn if(VIO_MSG_POLLING: msg->poll_status ==) -> else {
...
//
    ...
    let mut next_msg_done: bool = false;
//
...
// for non-realtime virtio devices
    ...
    scmi_of_match, core);
#[no_mangle]
unsafe extern "C" fn scmi_vio_probe(vdev: *mut virtio_device) -> c_int {
...
// Only one SCMI VirtiO device allowed
    ...
    ret = virtio_find_vqs(vdev, vq_cnt, vqs, scmi_vio_vqs_info, core::ptr::null_mut());
    if (ret) {
...
// Ensure NULLified scmi_vdev is visible
    smp_store_mb(scmi_vdev, core::ptr::null_mut());
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn scmi_vio_remove(vdev: *mut virtio_device) {
...
#[no_mangle]
unsafe extern "C" fn scmi_vio_validate(vdev: *mut virtio_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_transport_virtio_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn scmi_transport_virtio_exit() -> void __exit {
