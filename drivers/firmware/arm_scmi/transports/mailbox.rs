...
//! Source: drivers/firmware/arm_scmi/transports/mailbox.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn tx_prepare(cl: *mut mbox_client, m: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn rx_callback(cl: *mut mbox_client, m: *mut c_void) {
...
//
    ...
    core.shmem.read_header(smbox.shmem), core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn mailbox_chan_available(of_node: *mut device_node, idx: c_int) -> bool {
...
//
    ...
    "#mbox-cells", idx, core::ptr::null_mut());
    }
//
...
//
    ...
    num_sh = of_count_phandle_with_args(np, "shmem", core::ptr::null_mut());
...
// p2a_rx_chan = 3;
    ...
    if (!smbox)
    return -ENOMEM;
    smbox.shmem = core.shmem.setup_iomap(cinfo, dev, tx, core::ptr::null_mut(),
    &smbox.io_ops);
    if (IS_ERR(smbox.shmem))
    return PTR_ERR(smbox.shmem);
    cl = &smbox.cl;
    cl.dev = cdev;
    cl.tx_prepare = tx ? tx_prepare : core::ptr::null_mut();
...
// Additional unidirectional channel for TX if needed
    ...
    smbox.chan_receiver = core::ptr::null_mut();
    if (ret != -EPROBE_DEFER)
    ...
    smbox.chan_platform_receiver = core::ptr::null_mut();
    if (ret != -EPROBE_DEFER)
    ...
    smbox.cinfo = core::ptr::null_mut();
    devm_iounmap(dev, smbox.shmem);
    devm_kfree(dev, smbox);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn mailbox_chan_free(id: c_int, p: *mut c_void, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mailbox_clear_channel(cinfo: *mut scmi_chan_info) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: smbox->chan) -> else {
    else if (smbox.chan)
    intr_chan = smbox.chan;
    else
    return;
    ret = mbox_send_message(intr_chan, core::ptr::null_mut());
