...
//! Source: drivers/firmware/arm_scmi/scmi_power_control.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_syspower_conf {
    pub dev: *mut device,
    pub state: enum scmi_syspower_state,
// Protect access to state
    pub state_mtx: mutex,
    pub required_transition: enum scmi_system_events,
    pub userspace_nb: notifier_block,
    pub reboot_nb: notifier_block,
    pub forceful_work: delayed_work,
...
//
    ...
    {
    let mut adj_timeout_ms: c_uint = 0;
    if (timeout_ms) {
    int ret;
...
#[no_mangle]
unsafe extern "C" fn scmi_suspend_work_func(work: *mut work_struct) {
    static void scmi_suspend_work_func(struct work_struct *work)
    {
    pm_suspend(PM_SUSPEND_MEM);
    }
#[no_mangle]
unsafe extern "C" fn scmi_syspower_probe(sdev: *mut scmi_device) -> c_int {
    ...
    core::ptr::null_mut(), &sc.userspace_nb);
    }
#[no_mangle]
unsafe extern "C" fn scmi_system_power_resume(dev: *mut device) -> c_int {
    ...
    SYSTEM_SLEEP_PM_OPS(core::ptr::null_mut(), scmi_system_power_resume)
    };
