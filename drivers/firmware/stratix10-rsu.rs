...
//! Source: drivers/firmware/stratix10-rsu.c
#![no_std]
...
pub const INVALID_RETRY_COUNTER: c_uint = 0xFF;
pub const INVALID_DCMF_VERSION: c_uint = 0xFF;
pub const INVALID_DCMF_STATUS: c_uint = 0xFFFFFFFF;
pub const INVALID_SPT_ADDRESS: c_uint = 0x0;

pub const RSU_GET_SPT_CMD: c_uint = 0x5A;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct flash_device_info {
    pub size: c_uint,
...
pub struct stratix10_rsu_priv {
    ...
    pub async: bool,
    struct {
    pub current_image: c_ulong,
    pub fail_image: c_ulong,
    pub version: c_uint,
    pub state: c_uint,
    pub error_details: c_uint,
    pub error_location: c_uint,
    pub status: },
    struct {
    pub dcmf0: c_uint,
    pub dcmf1: c_uint,
    ...
    pub dcmf3: c_uint,
    pub dcmf_status: },
    pub device_info: [flash_device_info; 4],
    pub retry_counter: c_uint,
    pub max_retry: c_uint,
    pub spt0_address: c_ulong,
    pub spt1_address: c_ulong,
    pub get_spt_response_buf: *mut c_uint,
}
...
//
#[no_mangle]
unsafe extern "C" fn rsu_device_info_invalidate(priv: *mut stratix10_rsu_priv) {
...
#[no_mangle]
pub unsafe extern "C" fn if(BIT(SVC_STATUS_ERROR): data->status ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(BIT(SVC_STATUS_NO_SUPPORT): data->status ==) -> else {
...
//
    ...
    priv.get_spt_response_buf = core::ptr::null_mut();
    complete(&priv.completion);
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn soc64_async_callback(ptr: *mut c_void) {
...
//
    ...
    {
    let mut msg: stratix10_svc_client_msg = {0};
    let mut data: stratix10_svc_cb_data = {0};
...
//
    ...
    return ret;
    if (priv.async)
    ret = rsu_send_async_msg(dev, priv, COMMAND_RSU_NOTIFY, status, core::ptr::null_mut());
    else
...
#[no_mangle]
unsafe extern "C" fn stratix10_rsu_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn stratix10_rsu_remove(pdev: *mut platform_device) {
