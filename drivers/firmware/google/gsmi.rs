...
//! Source: drivers/firmware/google/gsmi.c
#![no_std]
...
pub const GSMI_GUID_SIZE: c_int = 16;
pub const GSMI_BUF_SIZE: c_int = 1024;

pub const GSMI_CALLBACK: c_uint = 0xef;
// SMI return codes
pub const GSMI_SUCCESS: c_uint = 0x00;
pub const GSMI_UNSUPPORTED2: c_uint = 0x03;
pub const GSMI_LOG_FULL: c_uint = 0x0b;
pub const GSMI_VAR_NOT_FOUND: c_uint = 0x0e;
pub const GSMI_HANDSHAKE_SPIN: c_uint = 0x7d;
pub const GSMI_HANDSHAKE_CF: c_uint = 0x7e;
pub const GSMI_HANDSHAKE_NONE: c_uint = 0x7f;
pub const GSMI_INVALID_PARAMETER: c_uint = 0x82;
pub const GSMI_UNSUPPORTED: c_uint = 0x83;
pub const GSMI_BUFFER_TOO_SMALL: c_uint = 0x85;
pub const GSMI_NOT_READY: c_uint = 0x86;
pub const GSMI_DEVICE_ERROR: c_uint = 0x87;
pub const GSMI_NOT_FOUND: c_uint = 0x8e;
pub const QUIRKY_BOARD_HASH: c_uint = 0x78a30a50;
// Internally used commands passed to the firmware
pub const GSMI_CMD_GET_NVRAM_VAR: c_uint = 0x01;
pub const GSMI_CMD_GET_NEXT_VAR: c_uint = 0x02;
pub const GSMI_CMD_SET_NVRAM_VAR: c_uint = 0x03;
pub const GSMI_CMD_SET_EVENT_LOG: c_uint = 0x08;
pub const GSMI_CMD_CLEAR_EVENT_LOG: c_uint = 0x09;
pub const GSMI_CMD_LOG_S0IX_SUSPEND: c_uint = 0x0a;
pub const GSMI_CMD_LOG_S0IX_RESUME: c_uint = 0x0b;
pub const GSMI_CMD_CLEAR_CONFIG: c_uint = 0x20;
pub const GSMI_CMD_HANDSHAKE_TYPE: c_uint = 0xC1;
pub const GSMI_CMD_RESERVED: c_uint = 0xff;
// Magic entry type for kernel events
pub const GSMI_LOG_ENTRY_TYPE_KERNEL: c_uint = 0xDEAD;
// SMI buffers must be in 32bit physical address space
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gsmi_buf {
    pub /: *mut *mut *mut u8 start; / start of buffer,
    pub /: *mut *mut size_t length; / length of buffer,
    pub /: *mut *mut u32 address; / physical address of buffer,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gsmi_nvram_var_param {
    pub guid: efi_guid_t,
    pub name_ptr: u32,
    pub attributes: u32,
    pub data_len: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gsmi_get_next_var_param {
    pub guid: [u8; GSMI_GUID_SIZE],
    pub name_ptr: u32,
    pub name_len: u32,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gsmi_set_eventlog_param {
    pub data_ptr: u32,
    pub data_len: u32,
    pub type: u32,
    pub __packed: },
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct gsmi_log_entry_type_1 {
    pub type: u16,
    pub instance: u32,
    pub __packed: },
//
...
//
pub const GSMI_DEFAULT_SPINCOUNT: c_uint = 0x10000;
    pub GSMI_DEFAULT_SPINCOUNT: static unsigned int spincount =,
    pub 0600): module_param(spincount, uint,,
    MODULE_PARM_DESC(spincount,
    pub handshake."): "The number of loop iterations to use when using the spin,
//
...
// entirely.
//
    pub true: static bool s0ix_logging_enable =,
    pub 0600): module_param(s0ix_logging_enable, bool,,
    static struct gsmi_buf *gsmi_buf_alloc(void)
    {
    pub smibuf: *mut gsmi_buf,
    pub kzalloc_obj(*smibuf): *mut smibuf =,
    if (!smibuf) {
    pub memory\n"): printk(KERN_ERR "gsmi: out of,
    pub NULL: return,
    }
// allocate buffer in 32bit address space
    pub GFP_KERNEL): smibuf->start = kmem_cache_alloc(gsmi_dev.mem_pool,,
    if (!smibuf.start) {
    pub buffer\n"): printk(KERN_ERR "gsmi: failed to allocate name,
    pub NULL: return,
    }
// fill in the buffer handle
    pub GSMI_BUF_SIZE: smibuf->length =,
    pub (u32)virt_to_phys(smibuf->start): smibuf->address =,
    pub smibuf: return,
    }
#[no_mangle]
unsafe extern "C" fn gsmi_buf_free(smibuf: *mut gsmi_buf) {
    static void gsmi_buf_free(struct gsmi_buf *smibuf)
    {
    if (smibuf) {
    if (smibuf.start)
    pub smibuf->start): kmem_cache_free(gsmi_dev.mem_pool,,
    }
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn gsmi_exec(func: u8, sub: u8) -> c_int {
    static int gsmi_exec(u8 func, u8 sub)
    {
    pub func: u16 cmd = (sub << 8) |,
    pub 0: u16 result =,
    pub 0: int rc =,
//
...
// not really an error, but let the caller know
    ...
    pub cmd): printk(KERN_ERR "gsmi: exec 0x%04x: Invalid parameter\n",,
    pub -EINVAL: rc =,
    case GSMI_BUFFER_TOO_SMALL:
    pub cmd): printk(KERN_ERR "gsmi: exec 0x%04x: Buffer too small\n",,
    pub -ENOMEM: rc =,
    ...
    pub -ENOSYS: rc =,
    case GSMI_NOT_READY:
    pub cmd): printk(KERN_ERR "gsmi: exec 0x%04x: Not ready\n",,
    pub -EBUSY: rc =,
    case GSMI_DEVICE_ERROR:
    pub cmd): printk(KERN_ERR "gsmi: exec 0x%04x: Device error\n",,
    pub -EFAULT: rc =,
    case GSMI_NOT_FOUND:
    pub cmd): printk(KERN_ERR "gsmi: exec 0x%04x: Data not found\n",,
    pub -ENOENT: rc =,
    case GSMI_LOG_FULL:
    pub cmd): printk(KERN_ERR "gsmi: exec 0x%04x: Log full\n",,
    pub -ENOSPC: rc =,
    ...
    pub efivars: static struct efivars,
    ...
    let mut ret: efi_status_t = EFI_SUCCESS;
    unsigned long flags;
    let mut name_len: usize = ucs2_strnlen(name, GSMI_BUF_SIZE / 2);
    int rc;
...
#[no_mangle]
unsafe extern "C" fn gsmi_shutdown_reason(reason: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn local_hash_64(val: u64, bits: unsigned) -> u64 __init {
    static u64 __init local_hash_64(u64 val, unsigned bits)
    {
    let mut hash: u64 = val;
...
#[no_mangle]
unsafe extern "C" fn hash_oem_table_id(s[8]: c_char) -> u32 __init {
...
#[no_mangle]
unsafe extern "C" fn gsmi_system_valid() -> __init int {
...
#[no_mangle]
unsafe extern "C" fn gsmi_log_s0ix_info(cmd: u8) {
...
#[no_mangle]
unsafe extern "C" fn gsmi_log_s0ix_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gsmi_log_s0ix_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gsmi_platform_driver_probe(dev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gsmi_init() -> __init int {
...
//
    ...
    GSMI_BUF_ALIGN,
    SLAB_CACHE_DMA32, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn gsmi_exit() -> void __exit {
