...
//! Source: drivers/mtd/hyperbus/hbmc-am654.c
#![no_std]
...
pub const AM654_HBMC_CALIB_COUNT: c_int = 25;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct am654_hbmc_device_priv {
    pub rx_dma_complete: completion,
    pub device_base: phys_addr_t,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct am654_hbmc_priv {
    pub ctlr: hyperbus_ctlr,
    pub hbdev: hyperbus_device,
    pub mux_ctrl: *mut mux_control,
}

#[no_mangle]
unsafe extern "C" fn am654_hbmc_calibrate(hbdev: *mut hyperbus_device) -> c_int {
    static int am654_hbmc_calibrate(struct hyperbus_device *hbdev)
    {
    struct map_info *map = &hbdev.map;
    struct cfi_private cfi;
    let mut count: c_int = AM654_HBMC_CALIB_COUNT;
    let mut pass_count: c_int = 0;
    int ret;
    cfi.interleave = 1;
    cfi.device_type = CFI_DEVICETYPE_X16;
    cfi_send_gen_cmd(0xF0, 0, 0, map, &cfi, cfi.device_type, core::ptr::null_mut());
    cfi_send_gen_cmd(0x98, 0x55, 0, map, &cfi, cfi.device_type, core::ptr::null_mut());
    while (count--) {
...
#[no_mangle]
unsafe extern "C" fn am654_hbmc_dma_callback(param: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn am654_hbmc_request_mmap_dma(priv: *mut am654_hbmc_device_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn am654_hbmc_probe(pdev: *mut platform_device) -> c_int {
    ...
    priv.hbdev.np = of_get_next_child(np, core::ptr::null_mut());
    ret = of_address_to_resource(priv.hbdev.np, 0, &res);
    if (ret)
    goto put_node;
    if (of_property_present(dev.of_node, "mux-controls")) {
    struct mux_control *control = devm_mux_control_get(dev, core::ptr::null_mut());
    if (IS_ERR(control)) {
    ret = PTR_ERR(control);
    goto put_node;
    }
...
#[no_mangle]
unsafe extern "C" fn am654_hbmc_remove(pdev: *mut platform_device) {
