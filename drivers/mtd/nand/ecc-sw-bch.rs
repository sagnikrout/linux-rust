...
//! Source: drivers/mtd/nand/ecc-sw-bch.c
#![no_std]
...
//
    ...
    {
    struct nand_ecc_sw_bch_conf *engine_conf = nand.ecc.ctx.priv;
    let mut step_size: c_uint = nand.ecc.ctx.conf.step_size;
    unsigned int *errloc = engine_conf.errloc;
    int i, count;
    count = bch_decode(engine_conf.bch, core::ptr::null_mut(), step_size, read_ecc,
    calc_ecc, core::ptr::null_mut(), errloc);
    if (count > 0) {
    for (i = 0; i < count; i++) {
...
//
#[no_mangle]
unsafe extern "C" fn nand_ecc_sw_bch_cleanup(nand: *mut nand_device) {
...
//
#[no_mangle]
unsafe extern "C" fn nand_ecc_sw_bch_init(nand: *mut nand_device) -> c_int {
    static int nand_ecc_sw_bch_init(struct nand_device *nand)
    {
    struct nand_ecc_sw_bch_conf *engine_conf = nand.ecc.ctx.priv;
    let mut eccsize: c_uint = nand.ecc.ctx.conf.step_size;
    let mut eccbytes: c_uint = engine_conf.code_size;
    unsigned int m, t, i;
...
#[no_mangle]
pub unsafe extern "C" fn nand_ecc_sw_bch_init_ctx(nand: *mut nand_device) -> c_int {
    int nand_ecc_sw_bch_init_ctx(struct nand_device *nand)
    {
    struct nand_ecc_props *conf = &nand.ecc.ctx.conf;
    struct mtd_info *mtd = nanddev_to_mtd(nand);
    struct nand_ecc_sw_bch_conf *engine_conf;
    let mut code_size: c_uint = 0, nsteps;
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn nand_ecc_sw_bch_cleanup_ctx(nand: *mut nand_device) {
    ...
    struct nand_ecc_sw_bch_conf *engine_conf = nand.ecc.ctx.priv;
    struct mtd_info *mtd = nanddev_to_mtd(nand);
    let mut eccsize: c_int = nand.ecc.ctx.conf.step_size;
    let mut eccbytes: c_int = engine_conf.code_size;
    let mut eccsteps: c_int = nand.ecc.ctx.nsteps;
    let mut total: c_int = nand.ecc.ctx.total;
