...
//! Source: drivers/mtd/nand/raw/fsl_ifc_nand.c
#![no_std]
...
pub const ERR_BYTE: c_uint = 0xFF /* Value returned for read;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fsl_ifc_mtd {
    pub chip: nand_chip,
    pub ctrl: *mut fsl_ifc_ctrl,
    pub dev: *mut device,
    pub /: *mut *mut int bank; / Chip select bank number,
    pub /: *mut *mut unsigned int bufnum_mask; / bufnum = page & bufnum_mask,
    pub /: *mut *mut *mut u8 __iomem vbase; / Chip select base virtual address,
}

// overview of the fsl ifc controller
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fsl_ifc_nand_ctrl {
    pub controller: nand_controller,
    pub chips: [*mut fsl_ifc_mtd; FSL_IFC_BANK_COUNT],
    pub /: *mut *mut *mut void __iomem addr; / Address of assigned IFC buffer,
    pub /: *mut *mut unsigned int page; / Last page written to / read from,
    pub /: *mut *mut unsigned int read_bytes;/ Number of bytes read during command,
    pub /: *mut *mut unsigned int column; / Saved column from SEQIN,
    pub /: *mut *mut unsigned int index; / Pointer to next byte to 'read',
    pub /: *mut *mut unsigned int oob; / Non zero if operating on OOB data,
    pub /: *mut *mut unsigned int eccread; / Non zero for a full-page ECC read,
    pub /: *mut *mut unsigned int counter; / counter for the initializations,
    pub /: *mut *mut unsigned int max_bitflips; / Saved during READ0 cmd,
}
...
//
#[no_mangle]
unsafe extern "C" fn set_addr(mtd: *mut mtd_info, column: c_int, page_addr: c_int, oob: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_ifc_run_command(mtd: *mut mtd_info) {
...
// ctrl->nand_stat will be updated from IRQ context
    ...
    if (nctrl.eccread) {
    int errors;
    let mut bufnum: c_int = nctrl.page & priv.bufnum_mask;
    let mut sector_start: c_int = bufnum * chip.ecc.steps;
    let mut sector_end: c_int = sector_start + chip.ecc.steps - 1;
...
//
    let mut len: c_int = 8;
    let mut timing: c_int = IFC_FIR_OP_RB;
...
#[no_mangle]
unsafe extern "C" fn fsl_ifc_select_chip(chip: *mut nand_chip, cs: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_ifc_write_buf(chip: *mut nand_chip, buf: *const u8, len: c_int) {
    static void fsl_ifc_write_buf(struct nand_chip *chip, const u8 *buf, int len)
    {
    struct mtd_info *mtd = nand_to_mtd(chip);
    struct fsl_ifc_mtd *priv = nand_get_controller_data(chip);
    let mut bufsize: c_uint = mtd.writesize + mtd.oobsize;
    if (len <= 0) {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_ifc_read_byte(chip: *mut nand_chip) -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_ifc_read_byte16(chip: *mut nand_chip) -> u8 {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_ifc_read_buf(chip: *mut nand_chip, buf: *mut u8, len: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn fsl_ifc_wait(chip: *mut nand_chip) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn check_erased_page(chip: *mut nand_chip, buf: *mut u8) -> c_int {
    static int check_erased_page(struct nand_chip *chip, u8 *buf)
    {
    struct mtd_info *mtd = nand_to_mtd(chip);
    u8 *ecc = chip.oob_poi;
    let mut ecc_size: c_int = chip.ecc.bytes;
    let mut pkt_size: c_int = chip.ecc.size;
...
#[no_mangle]
unsafe extern "C" fn fsl_ifc_attach_chip(chip: *mut nand_chip) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fsl_ifc_sram_init(priv: *mut fsl_ifc_mtd) -> c_int {
    static int fsl_ifc_sram_init(struct fsl_ifc_mtd *priv)
    {
    struct fsl_ifc_ctrl *ctrl = priv.ctrl;
    struct fsl_ifc_runtime __iomem *ifc_runtime = ctrl.rregs;
    struct fsl_ifc_global __iomem *ifc_global = ctrl.gregs;
    let mut csor: u32 = 0, csor_8k = 0, csor_ext = 0;
    let mut cs: u32 = priv.bank;
...
#[no_mangle]
unsafe extern "C" fn fsl_ifc_chip_init(priv: *mut fsl_ifc_mtd) -> c_int {
...
// Fill in fsl_ifc_mtd structure
    ...
    of_get_next_child_with_prefix(priv.dev.of_node, core::ptr::null_mut(), "nand");
    if (np)
...
#[no_mangle]
unsafe extern "C" fn fsl_ifc_chip_remove(priv: *mut fsl_ifc_mtd) -> c_int {
    ...
    if (priv.vbase)
    iounmap(priv.vbase);
    ifc_nand_ctrl.chips[priv.bank] = core::ptr::null_mut();
    return 0;
    }
    static int match_bank(struct fsl_ifc_global __iomem *ifc_global, int bank,
    phys_addr_t addr)
    {
    let mut cspr: u32 = ifc_in32(&ifc_global.cspr_cs[bank].cspr);
    if (!(cspr & CSPR_V))
    return 0;
...
#[no_mangle]
unsafe extern "C" fn fsl_ifc_nand_probe(dev: *mut platform_device) -> c_int {
...
// find which chip select it is connected to
    ...
    ifc_nand_ctrl.read_bytes = 0;
    ifc_nand_ctrl.index = 0;
    ifc_nand_ctrl.addr = core::ptr::null_mut();
...
// line, these take precedence over device tree information
    ret = mtd_device_parse_register(mtd, part_probe_types, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (ret)
    goto cleanup_nand;
...
#[no_mangle]
unsafe extern "C" fn fsl_ifc_nand_remove(dev: *mut platform_device) {
    ...
    fsl_ifc_ctrl_dev.nand = core::ptr::null_mut();
    kfree(ifc_nand_ctrl);
    }
    mutex_unlock(&fsl_ifc_nand_mutex);
    }
