...
//! Source: drivers/mtd/nand/raw/technologic-nand-controller.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn ts72xx_nand_attach_chip(chip: *mut nand_chip) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ts72xx_nand_ctrl(chip: *mut nand_chip, value: u8) {
    static void ts72xx_nand_ctrl(struct nand_chip *chip, u8 value)
    {
    struct ts72xx_nand_data *data = chip_to_ts72xx(chip);
    let mut bits: c_uchar = ioread8(data.ctrl) & ~GENMASK(2, 0);
...
#[no_mangle]
unsafe extern "C" fn ts72xx_nand_probe(pdev: *mut platform_device) -> c_int {
    ...
    child = fwnode_get_next_child_node(dev_fwnode(&pdev.dev), core::ptr::null_mut());
    if (!child)
...
// Scan to find existence of the device
    ...
    if (err)
    goto err_handle_put;
    err = mtd_device_parse_register(mtd, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (err)
    goto err_clean_nand;
    return 0;
    err_clean_nand:
...
#[no_mangle]
unsafe extern "C" fn ts72xx_nand_remove(pdev: *mut platform_device) {
