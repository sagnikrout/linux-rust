...
//! Source: drivers/mtd/nand/raw/gpio.c
#![no_std]
...
pub struct gpiomtd {
    ...
    pub io_sync: *mut void __iomem,
    pub nand_chip: nand_chip,
    pub plat: gpio_nand_platdata,
    pub /: *mut *mut *mut gpio_desc nce; / Optional chip enable,
    pub cle: *mut gpio_desc,
    pub ale: *mut gpio_desc,
    pub rdy: *mut gpio_desc,
    pub /: *mut *mut *mut gpio_desc nwp; / Optional write protection,
}
...
//
#[no_mangle]
unsafe extern "C" fn gpio_nand_dosync(gpiomtd: *mut gpiomtd) {
...
unsafe extern "C" fn gpio_nand_attach_chip(chip: *mut nand_chip) -> c_int {
    ...
    struct gpio_nand_platdata *plat)
    {
    let mut ret: c_int = gpio_nand_get_config_of(dev, plat);
    if (!ret)
    return ret;
