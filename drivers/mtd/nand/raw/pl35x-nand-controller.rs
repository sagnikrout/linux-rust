...
//! Source: drivers/mtd/nand/raw/pl35x-nand-controller.c
#![no_std]
...
pub const PL35X_SMC_MEMC_STATUS: c_uint = 0x0;

// SMC clear config register (WO)
pub const PL35X_SMC_MEMC_CFG_CLR: c_uint = 0xC;

// SMC direct command register (WO)
pub const PL35X_SMC_DIRECT_CMD: c_uint = 0x10;

// SMC set cycles register (WO)
pub const PL35X_SMC_CYCLES: c_uint = 0x14;

// SMC set opmode register (WO)
pub const PL35X_SMC_OPMODE: c_uint = 0x18;
pub const PL35X_SMC_OPMODE_BW_8: c_int = 0;
pub const PL35X_SMC_OPMODE_BW_16: c_int = 1;
// SMC ECC status register (RO)
pub const PL35X_SMC_ECC_STATUS: c_uint = 0x400;

// SMC ECC configuration register
pub const PL35X_SMC_ECC_CFG: c_uint = 0x404;
pub const PL35X_SMC_ECC_CFG_MODE_MASK: c_uint = 0xC;
pub const PL35X_SMC_ECC_CFG_MODE_BYPASS: c_int = 0;

pub const PL35X_SMC_ECC_CFG_PGSIZE_MASK: c_uint = 0x3;
// SMC ECC command 1 register
pub const PL35X_SMC_ECC_CMD1: c_uint = 0x408;

// SMC ECC command 2 register
pub const PL35X_SMC_ECC_CMD2: c_uint = 0x40C;

// SMC ECC value registers (RO)

// NAND AXI interface
pub const PL35X_SMC_CMD_PHASE: c_int = 0;

pub const PL35X_NAND_MAX_CS: c_int = 1;
pub const PL35X_NAND_LAST_XFER_SZ: c_int = 4;

pub const PL35X_NAND_ECC_BITS_MASK: c_uint = 0xFFF;
pub const PL35X_NAND_ECC_BYTE_OFF_MASK: c_uint = 0x1FF;
pub const PL35X_NAND_ECC_BIT_OFF_MASK: c_uint = 0x7;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pl35x_nand_timings {
    pub t_rc:4: c_uint,
    pub t_wc:4: c_uint,
    pub t_rea:3: c_uint,
    pub t_wp:3: c_uint,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pl35x_nand {
    pub node: list_head,
    pub chip: nand_chip,
    pub cs: c_uint,
    pub addr_cycles: c_uint,
    pub ecc_cfg: u32,
    pub timings: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pl35x_nandc {
    pub dev: *mut device,
    pub conf_regs: *mut void __iomem,
...
#[no_mangle]
unsafe extern "C" fn pl35x_smc_update_regs(nfc: *mut pl35x_nandc) {
...
#[no_mangle]
unsafe extern "C" fn pl35x_smc_set_buswidth(nfc: *mut pl35x_nandc, bw: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pl35x_smc_clear_irq(nfc: *mut pl35x_nandc) {
...
#[no_mangle]
unsafe extern "C" fn pl35x_smc_wait_for_irq(nfc: *mut pl35x_nandc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pl35x_smc_wait_for_ecc_done(nfc: *mut pl35x_nandc) -> c_int {
...
// Configure the ECC engine
    ...
    {
    struct pl35x_nandc *nfc = to_pl35x_nandc(chip.controller);
    let mut buf_end: c_uint = len / 4;
    let mut in_start: c_uint = round_down(len, 4);
...
// One error in the ECC data; no action needed
    ...
    let mut max_bitflips: c_uint = 0, chunk;
    u8 calc_ecc[3];
    u32 ecc_value;
    int stats;
    for (chunk = 0; chunk < chip.ecc.steps;
...
// Read ECC value for each chunk
    ...
    struct pl35x_nand *plnand = to_pl35x_nand(chip);
    struct mtd_info *mtd = nand_to_mtd(chip);
    let mut first_row: c_uint = (mtd.writesize <= 512) ? 1 : 2;
    let mut nrows: c_uint = plnand.addr_cycles;
    let mut addr1: u32 = 0, addr2 = 0, row;
    u32 cmd_addr;
    int i, ret;
    u8 status;
...
//
    ...
    let mut first_row: c_uint = (mtd.writesize <= 512) ? 1 : 2;
    let mut nrows: c_uint = plnand.addr_cycles;
    let mut addr1: c_uint = 0, addr2 = 0, row;
    u32 cmd_addr;
    int i, ret;
    ret = pl35x_smc_set_ecc_mode(nfc, chip, PL35X_SMC_ECC_CFG_MODE_APB);
    if (ret)
...
// Correct the data and report failures
    ...
    {
    struct pl35x_nandc *nfc = to_pl35x_nandc(chip.controller);
    const struct nand_op_instr *instr, *data_instr = core::ptr::null_mut();
    let mut rdy_tim_ms: c_uint = 0, naddrs = 0, cmds = 0, last_flags = 0;
    let mut addr1: u32 = 0, addr2 = 0, cmd0 = 0, cmd1 = 0, cmd_addr = 0;
    unsigned int op_id, len, offset, rdy_del_ns;
    let mut last_instr_type: c_int = -1;
    let mut cmd1_valid: bool = false;
    const u8 *addrs;
    int i, ret;
...
// Data phase
    ...
    struct pl35x_nandc *nfc = to_pl35x_nandc(chip.controller);
    struct pl35x_nand *plnand = to_pl35x_nand(chip);
    let mut tmgs: pl35x_nand_timings = {};
    const struct nand_sdr_timings *sdr;
...
#[no_mangle]
unsafe extern "C" fn pl35x_nand_attach_chip(chip: *mut nand_chip) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pl35x_nand_reset_state(nfc: *mut pl35x_nandc) -> c_int {
...
// Ensure the ECC controller is bypassed by default
    ret = pl35x_smc_set_ecc_mode(nfc, core::ptr::null_mut(), PL35X_SMC_ECC_CFG_MODE_BYPASS);
    if (ret)
    return ret;
//
...
#[no_mangle]
unsafe extern "C" fn pl35x_nand_chips_cleanup(nfc: *mut pl35x_nandc) {
...
#[no_mangle]
unsafe extern "C" fn pl35x_nand_chips_init(nfc: *mut pl35x_nandc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pl35x_nand_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pl35x_nand_remove(pdev: *mut platform_device) {
