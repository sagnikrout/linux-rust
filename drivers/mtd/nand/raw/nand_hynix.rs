...
//! Source: drivers/mtd/nand/raw/nand_hynix.c
#![no_std]
...
pub const NAND_HYNIX_CMD_SET_PARAMS: c_uint = 0x36;
pub const NAND_HYNIX_CMD_APPLY_PARAMS: c_uint = 0x16;
pub const NAND_HYNIX_1XNM_RR_REPEAT: c_int = 8;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hynix_nand {
    pub read_retry: *const hynix_read_retry,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hynix_read_retry_otp {
    pub nregs: c_int,
    pub regs: *const u8,
    pub values: *const u8,
    pub page: c_int,
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_has_valid_jedecid(chip: *mut nand_chip) -> bool {
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_cmd_op(chip: *mut nand_chip, cmd: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_reg_write_op(chip: *mut nand_chip, addr: u8, val: u8) -> c_int {
    static int hynix_nand_reg_write_op(struct nand_chip *chip, u8 addr, u8 val)
    {
    let mut column: u16 = ((u16)addr << 8) | addr;
    if (nand_has_exec_op(chip)) {
    struct nand_op_instr instrs[] = {
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_setup_read_retry(chip: *mut nand_chip, retry_mode: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hynix_get_majority(in: *const u8, repeat: c_int, out: *mut u8) -> c_int {
...
// Put everything back to normal
    ...
    if (ret)
    return ret;
    return nand_read_page_op(chip, 0, 0, core::ptr::null_mut(), 0);
    }
pub const NAND_HYNIX_1XNM_RR_COUNT_OFFS: c_int = 0;
pub const NAND_HYNIX_1XNM_RR_REG_COUNT_OFFS: c_int = 8;
    ...
    let mut val_offs: c_int = (mode * nregs) + reg;
    let mut set_size: c_int = nmodes * nregs;
    int i, ret;
    for (i = 0; i < NAND_HYNIX_1XNM_RR_REPEAT; i++) {
    let mut set_offs: c_int = NAND_HYNIX_1XNM_RR_SET_OFFS(i, set_size, inv);
...
// val = ~*val;
    ...
    {
    struct hynix_nand *hynix = nand_get_manufacturer_data(chip);
    struct hynix_read_retry *rr = core::ptr::null_mut();
    int ret, i, j;
    u8 nregs, nmodes;
    u8 *buf;
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_rr_init(chip: *mut nand_chip) -> c_int {
...
//
    ...
    let mut nand_tech: u8 = chip.id.data[5] >> 4;
// 1xnm technology
    if (nand_tech == 4) {
...
//
    ...
    let mut ecc_level: u8 = (chip.id.data[4] >> 4) & 0x7;
    if (valid_jedecid) {
...
// NAND technology.
//
    let mut nand_tech: u8 = chip.id.data[5] & 0x7;
    if (nand_tech < 3) {
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_decode_id(chip: *mut nand_chip) {
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_cleanup(chip: *mut nand_chip) {
    ...
    kfree(hynix);
    nand_set_manufacturer_data(chip, core::ptr::null_mut());
    }
    static int
...
#[no_mangle]
unsafe extern "C" fn h27ucg8t2etrbc_init(chip: *mut nand_chip) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hynix_nand_init(chip: *mut nand_chip) -> c_int {
