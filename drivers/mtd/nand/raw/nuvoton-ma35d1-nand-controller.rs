...
//! Source: drivers/mtd/nand/raw/nuvoton-ma35d1-nand-controller.c
#![no_std]
...
// NFI Registers
pub const MA35_NFI_REG_DMACTL: c_uint = 0x400;

pub const MA35_NFI_REG_DMASA: c_uint = 0x408;
pub const MA35_NFI_REG_GCTL: c_uint = 0x800;

pub const MA35_NFI_REG_NANDCTL: c_uint = 0x8A0;

pub const MA35_NFI_REG_NANDINTEN: c_uint = 0x8A8;
pub const MA35_NFI_REG_NANDINTSTS: c_uint = 0x8AC;

pub const MA35_NFI_REG_NANDCMD: c_uint = 0x8B0;
pub const MA35_NFI_REG_NANDADDR: c_uint = 0x8B4;

pub const MA35_NFI_REG_NANDDATA: c_uint = 0x8B8;
pub const MA35_NFI_REG_NANDRACTL: c_uint = 0x8BC;
pub const MA35_NFI_REG_NANDECTL: c_uint = 0x8C0;
pub const ENABLE_WP: c_uint = 0x0;

pub const MA35_NFI_REG_NANDECCES0: c_uint = 0x8D0;

pub const MA35_NFI_REG_NANDECCEA0: c_uint = 0x900;
pub const MA35_NFI_REG_NANDECCED0: c_uint = 0x960;
pub const MA35_NFI_REG_NANDRA0: c_uint = 0xA00;
// Define for the BCH hardware ECC engine
// define the total padding bytes for 512/1024 data segment
pub const MA35_BCH_PADDING_512: c_int = 32;
pub const MA35_BCH_PADDING_1024: c_int = 64;
// define the BCH parity code length for 512 bytes data pattern
pub const MA35_PARITY_BCH8: c_int = 15;
pub const MA35_PARITY_BCH12: c_int = 23;
// define the BCH parity code length for 1024 bytes data pattern
pub const MA35_PARITY_BCH24: c_int = 45;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ma35_nand_chip {
    pub node: list_head,
    pub chip: nand_chip,
    pub eccstatus: u32,
    pub nsels: u8,
...
#[no_mangle]
pub unsafe extern "C" fn ma35_clear_spare(chip: *mut nand_chip, size: c_int) {
    ...
    {
    let mut value: u32 = readl(nand.regs + MA35_NFI_REG_NANDRA0 + offset);
    u8 *ptr = (u8 *)buf;
    int i, shift;
    for (i = 0; i < size; i++) {
    shift = (swap ? 3 - i : i) * 8;
...
#[no_mangle]
pub unsafe extern "C" fn ma35_read_spare(chip: *mut nand_chip, size: c_int, buf: *mut u32, offset: u32) {
    static inline void ma35_read_spare(struct nand_chip *chip, int size, u32 *buf, u32 offset)
    {
    struct ma35_nand_info *nand = nand_get_controller_data(chip);
    let mut off: u32 = round_down(offset, 4);
    let mut len: c_int = offset % 4;
    int i;
    if (len) {
...
#[no_mangle]
pub unsafe extern "C" fn ma35_write_spare(chip: *mut nand_chip, size: c_int, buf: *mut u32) {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_target_enable(chip: *mut nand_chip, cs: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_hwecc_init(chip: *mut nand_chip, nand: *mut ma35_nand_info) -> c_int {
...
// correct each error bytes
    for (i = 0; i < err_cnt; i++) {
    let mut corrected_index: u32 = temp_addr[i];
...
#[no_mangle]
unsafe extern "C" fn ma35_nfi_ecc_check(chip: *mut nand_chip, addr: *mut u8) -> c_int {
    static int ma35_nfi_ecc_check(struct nand_chip *chip, u8 *addr)
    {
    struct ma35_nand_info *nand = nand_get_controller_data(chip);
    struct ma35_nand_chip *nvtnand = to_ma35_nand(chip);
    struct mtd_info *mtd = nand_to_mtd(chip);
    let mut maxbitflips: c_int = 0;
    let mut cnt: c_int = 0;
    u32 status;
    int i, j;
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_dmac_init(nand: *mut ma35_nand_info) {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_do_write(chip: *mut nand_chip, addr: *const u8, len: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_do_read(chip: *mut nand_chip, addr: *mut u8, len: u32) -> c_int {
    static int ma35_nand_do_read(struct nand_chip *chip, u8 *addr, u32 len)
    {
    struct ma35_nand_info *nand = nand_get_controller_data(chip);
    struct mtd_info *mtd = nand_to_mtd(chip);
    let mut ret: c_int = 0, cnt = 0, i;
    dma_addr_t dma_addr;
    u32 reg;
    if (len != mtd.writesize) {
    for (i = 0; i < len; i++)
...
// Setup and start DMA using dma_addr
    ...
    struct ma35_nand_info *nand = nand_get_controller_data(chip);
    struct mtd_info *mtd = nand_to_mtd(chip);
    let mut page_off: u32 = round_down(offset, chip.ecc.size);
    let mut end: u32 = DIV_ROUND_UP(page_off + len, chip.ecc.size);
    let mut start: u32 = page_off / chip.ecc.size;
    u32 reg;
    int i;
...
// Enable HW ECC engine
    ...
    nand_read_page_op(chip, page, offset, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_read_oob_hwecc(chip: *mut nand_chip, page: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ma35_hw_init(nand: *mut ma35_nand_info) {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_irq(irq: c_int, id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_attach_chip(chip: *mut nand_chip) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ma35_chips_cleanup(nand: *mut ma35_nand_info) {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_chips_init(dev: *mut device, nand: *mut ma35_nand_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ma35_nand_remove(pdev: *mut platform_device) {
