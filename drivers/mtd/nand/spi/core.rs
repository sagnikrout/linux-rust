...
//! Source: drivers/mtd/nand/spi/core.c
#![no_std]
...
//
    ...
    {
    let mut op: spi_mem_op = spinand.op_templates.readid;
    ...
    {
    let mut op: spi_mem_op = spinand.op_templates.set_feature;
    ...
    {
    let mut op: spi_mem_op = spinand.op_templates.get_feature;
    if (op.cmd.dtr && op.cmd.buswidth == 8)
    reg |= reg << 8;
    op.addr.val = reg;
    ...
    {
    let mut op: spi_mem_op = spinand.op_templates.blk_erase;
    op.addr.val = addr;
    return op;
    }
    static struct spi_mem_op
    ...
    {
    let mut op: spi_mem_op = spinand.op_templates.page_read;
    op.addr.val = addr;
    return op;
    }
    static struct spi_mem_op
    ...
    {
    let mut op: spi_mem_op = spinand.op_templates.prog_exec;
    op.addr.val = addr;
    return op;
    }
#[no_mangle]
pub unsafe extern "C" fn spinand_read_reg_op(spinand: *mut spinand_device, reg: u8, val: *mut u8) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn spinand_write_reg_op(spinand: *mut spinand_device, reg: u8, val: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_read_status(spinand: *mut spinand_device, status: *mut u8) -> c_int {
    static int spinand_read_status(struct spinand_device *spinand, u8 *status)
    {
    return spinand_read_reg_op(spinand, REG_STATUS, status);
    }
#[no_mangle]
unsafe extern "C" fn spinand_get_cfg(spinand: *mut spinand_device, cfg: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_set_cfg(spinand: *mut spinand_device, cfg: u8) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn spinand_upd_cfg(spinand: *mut spinand_device, mask: u8, val: u8) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn spinand_select_target(spinand: *mut spinand_device, target: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_read_cfg(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_init_cfg_cache(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_check_ecc_status(spinand: *mut spinand_device, status: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_ondie_ecc_init_ctx(nand: *mut nand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_ondie_ecc_cleanup_ctx(nand: *mut nand_device) {
    ...
    {
    struct spinand_device *spinand = nand_to_spinand(nand);
    let mut enable: bool = (req.mode != MTD_OPS_RAW);
...
#[no_mangle]
unsafe extern "C" fn spinand_ondie_ecc_save_status(nand: *mut nand_device, status: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn spinand_write_enable_op(spinand: *mut spinand_device) -> c_int {
    int spinand_write_enable_op(struct spinand_device *spinand)
    {
    let mut op: spi_mem_op = SPINAND_OP(spinand, wr_en);
    return spi_mem_exec_op(spinand.spimem, &op);
    }
    ...
    {
    struct nand_device *nand = spinand_to_nand(spinand);
    let mut row: c_uint = nanddev_pos_to_row(nand, &req.pos);
    let mut packed: bool = spinand.flags & SPINAND_ODTR_PACKED_PAGE_READ;
...
//
    ...
    struct nand_device *nand = spinand_to_nand(spinand);
    let mut row: c_uint = nanddev_pos_to_row(nand, &req.pos);
    let mut op: spi_mem_op = SPINAND_OP(spinand, prog_exec, row);
    return spi_mem_exec_op(spinand.spimem, &op);
    }
    ...
    {
    struct nand_device *nand = spinand_to_nand(spinand);
    let mut row: c_uint = nanddev_pos_to_row(nand, pos);
    let mut op: spi_mem_op = SPINAND_OP(spinand, blk_erase, row);
    return spi_mem_exec_op(spinand.spimem, &op);
    }
//
...
#[no_mangle]
unsafe extern "C" fn spinand_reset_op(spinand: *mut spinand_device) -> c_int {
    static int spinand_reset_op(struct spinand_device *spinand)
    {
    let mut op: spi_mem_op = SPINAND_OP(spinand, reset);
    int ret;
    ret = spi_mem_exec_op(spinand.spimem, &op);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn spinand_lock_block(spinand: *mut spinand_device, lock: u8) -> c_int {
...
//
    ...
    let mut disable_ecc: bool = false;
    let mut ecc_failed: bool = false;
    let mut retry_mode: c_uint = 0;
    let mut ret: c_int = 0;
...
//
    ...
    SPINAND_READ_POLL_DELAY_US, core::ptr::null_mut());
    if (ret < 0)
    goto end_cont_read;
...
#[no_mangle]
unsafe extern "C" fn spinand_cont_read_init(spinand: *mut spinand_device) {
    static void spinand_cont_read_init(struct spinand_device *spinand)
    {
    struct nand_device *nand = spinand_to_nand(spinand);
    let mut engine_type: enum nand_ecc_engine_type = nand.ecc.ctx.conf.engine_type;
...
#[no_mangle]
unsafe extern "C" fn spinand_isbad(nand: *mut nand_device, pos: *const nand_pos) -> bool {
...
#[no_mangle]
unsafe extern "C" fn spinand_mtd_block_isbad(mtd: *mut mtd_info, offs: loff_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_markbad(nand: *mut nand_device, pos: *const nand_pos) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_mtd_block_markbad(mtd: *mut mtd_info, offs: loff_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_erase(nand: *mut nand_device, pos: *const nand_pos) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_mtd_block_isreserved(mtd: *mut mtd_info, offs: loff_t) -> c_int {
    ...
    {
    struct nand_device *nand = spinand_to_nand(spinand);
    struct spi_mem_dirmap_desc *desc = core::ptr::null_mut();
    if (spinand.cont_read_possible) {
//
...
//
    ...
    {
    struct nand_device *nand = spinand_to_nand(spinand);
    let mut info: spi_mem_dirmap_info = { 0 };
    struct spi_mem_dirmap_desc *desc;
    let mut enable_ecc: bool = false, secondary_op = false;
    if (nand.ecc.engine.integration == NAND_ECC_ENGINE_INTEGRATION_PIPELINED)
    enable_ecc = true;
...
#[no_mangle]
unsafe extern "C" fn spinand_create_dirmaps(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_randomizer_init(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_id_detect(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_manufacturer_init(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_manufacturer_cleanup(spinand: *mut spinand_device) {
...
#[no_mangle]
pub unsafe extern "C" fn spinand_op_is_odtr(op: *const spi_mem_op) -> bool {
    bool spinand_op_is_odtr(const struct spi_mem_op *op)
    {
    return op.cmd.dtr && op.cmd.buswidth == 8;
    }
#[no_mangle]
unsafe extern "C" fn spinand_init_ssdr_templates(spinand: *mut spinand_device) {
    ...
    tmpl.readid = (struct spi_mem_op)SPINAND_READID_1S_1S_1S_OP(0, 0, core::ptr::null_mut(), 0);
    tmpl.wr_en = (struct spi_mem_op)SPINAND_WR_EN_1S_0_0_OP;
    tmpl.wr_dis = (struct spi_mem_op)SPINAND_WR_DIS_1S_0_0_OP;
    tmpl.set_feature = (struct spi_mem_op)SPINAND_SET_FEATURE_1S_1S_1S_OP(0, core::ptr::null_mut());
    tmpl.get_feature = (struct spi_mem_op)SPINAND_GET_FEATURE_1S_1S_1S_OP(0, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn spinand_init_odtr_instruction_set(spinand: *mut spinand_device) -> c_int {
    ...
    return -EOPNOTSUPP;
    tmpl.readid = (struct spi_mem_op)SPINAND_READID_8D_8D_8D_OP(0, 0, core::ptr::null_mut(), 0);
    ...
    return -EOPNOTSUPP;
    tmpl.set_feature = (struct spi_mem_op)SPINAND_SET_FEATURE_8D_8D_8D_OP(0, core::ptr::null_mut());
    if (!spi_mem_supports_op(spinand.spimem, &tmpl.set_feature))
    return -EOPNOTSUPP;
    tmpl.get_feature = (struct spi_mem_op)SPINAND_GET_FEATURE_8D_8D_8D_OP(0, core::ptr::null_mut());
    ...
    {
    struct nand_device *nand = spinand_to_nand(spinand);
    const struct spi_mem_op *best_variant = core::ptr::null_mut();
    let mut best_op_duration_ns: u64 = ULLONG_MAX;
    unsigned int i;
    for (i = 0; i < variants.nops; i++) {
    let mut op: spi_mem_op = variants.ops[i];
    let mut op_duration_ns: u64 = 0;
    unsigned int nbytes;
    int ret;
    if ((iface == SSDR && spinand_op_is_odtr(&op)) ||
    (iface == ODTR && !spinand_op_is_odtr(&op)))
...
#[no_mangle]
unsafe extern "C" fn spinand_detect(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_configure_chip(spinand: *mut spinand_device) -> c_int {
    static int spinand_configure_chip(struct spinand_device *spinand)
    {
    let mut odtr: bool = false, quad_enable = false;
    int ret;
...
// ODTR bus interface configuration happens here
    ...
    if (ret) {
    spinand.odtr_op_templates.read_cache = core::ptr::null_mut();
    spinand.odtr_op_templates.write_cache = core::ptr::null_mut();
    spinand.odtr_op_templates.update_cache = core::ptr::null_mut();
    goto try_ssdr;
    }
...
#[no_mangle]
unsafe extern "C" fn spinand_init_flash(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_mtd_resume(mtd: *mut mtd_info) {
...
#[no_mangle]
unsafe extern "C" fn spinand_mtd_suspend(mtd: *mut mtd_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_init(spinand: *mut spinand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_cleanup(spinand: *mut spinand_device) {
...
#[no_mangle]
unsafe extern "C" fn spinand_probe(mem: *mut spi_mem) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn spinand_remove(mem: *mut spi_mem) -> c_int {
