...
//! Source: drivers/mtd/nand/ecc-realtek.c
#![no_std]
...
// +------+------+------+------+-----+------+------+------+------+-----+-----+-----+-----+
//
pub const RTL_ECC_ALLOWED_PAGE_SIZE: c_int = 2048;
pub const RTL_ECC_ALLOWED_MIN_OOB_SIZE: c_int = 64;
pub const RTL_ECC_ALLOWED_STRENGTH: c_int = 6;
pub const RTL_ECC_BLOCK_SIZE: c_int = 512;
pub const RTL_ECC_FREE_SIZE: c_int = 6;
pub const RTL_ECC_PARITY_SIZE_BCH6: c_int = 10;
pub const RTL_ECC_PARITY_SIZE_BCH12: c_int = 20;
//
...
pub const RTL_ECC_CFG: c_uint = 0x00;
pub const RTL_ECC_BCH6: c_int = 0;

pub const RTL_ECC_DMA_TRIGGER: c_uint = 0x08;
pub const RTL_ECC_OP_DECODE: c_int = 0;

pub const RTL_ECC_DMA_START: c_uint = 0x0c;
pub const RTL_ECC_DMA_TAG: c_uint = 0x10;
pub const RTL_ECC_STATUS: c_uint = 0x14;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct rtl_ecc_engine {
    pub dev: *mut device,
    pub engine: nand_ecc_engine,
    pub lock: mutex,
    pub buf: *mut c_char,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rtl_ecc_ctx {
    pub rtlc: *mut *mut rtl_ecc_engine,
    pub req_ctx: nand_ecc_req_tweak_ctx,
    pub steps: c_int,
    pub bch_mode: c_int,
    pub strength: c_int,
    pub parity_size: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_kick_engine(ctx: *mut rtl_ecc_ctx, operation: c_int) {
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_wait_for_engine(ctx: *mut rtl_ecc_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_prepare_io_req(nand: *mut nand_device, req: *mut nand_page_io_req) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_finish_io_req(nand: *mut nand_device, req: *mut nand_page_io_req) -> c_int {
    ...
    parity = req.oobbuf.in + ctx.steps * RTL_ECC_FREE_SIZE;
    for (int i = 0 ; i < ctx.steps; i++) {
    let mut ret: c_int = rtl_ecc_run_engine(ctx, data, free, parity, RTL_ECC_OP_DECODE);
    if (unlikely(ret < 0))
...
#[no_mangle]
pub unsafe extern "C" fn if(2): unlikely(bitflips >) -> else {
    else if (unlikely(bitflips > 2))
    dev_dbg(rtlc.dev, "%d bitflips detected\n", bitflips);
    return failure ? -EBADMSG : bitflips;
    }
#[no_mangle]
unsafe extern "C" fn rtl_ecc_check_support(nand: *mut nand_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_init_ctx(nand: *mut nand_device) -> c_int {
    static int rtl_ecc_init_ctx(struct nand_device *nand)
    {
    struct nand_ecc_props *conf = &nand.ecc.ctx.conf;
    struct rtl_ecc_engine *rtlc = nand_to_rtlc(nand);
    struct mtd_info *mtd = nanddev_to_mtd(nand);
    let mut strength: c_int = nand.ecc.user_conf.strength;
    struct device *dev = nand.ecc.engine.dev;
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_cleanup_ctx(nand: *mut nand_device) {
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rtl_ecc_remove(pdev: *mut platform_device) {
