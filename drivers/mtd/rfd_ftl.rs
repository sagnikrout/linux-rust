...
pub struct block {
    ...
    BLOCK_FAILED
    pub state: },
    pub free_sectors: c_int,
    pub used_sectors: c_int,
    pub erases: c_int,
    pub offset: u_long,
}
...
pub struct partition {
    ...
    pub /: *mut *mut u_int block_size; / size of erase unit,
    pub /: *mut *mut u_int total_blocks; / number of erase units,
    pub /: *mut *mut u_int header_sectors_per_block; / header sectors in erase unit,
    pub /: *mut *mut u_int data_sectors_per_block; / data sectors in erase unit,
    pub /: *mut *mut u_int sector_count; / sectors in translated disk,
    pub /: *mut *mut u_int header_size; / bytes in header sector,
    pub /: *mut *mut int reserved_block; / block next up for reclaim,
    pub /: *mut *mut int current_block; / block to write to,
    pub /: *mut *mut *mut u16 header_cache; / cached header,
    pub is_reclaiming: c_int,
    pub cylinders: c_int,
    pub errors: c_int,
    pub sector_map: *mut u_long,
    pub blocks: *mut block,
}

    static int rfd_ftl_writesect(struct mtd_blktrans_dev *dev, u_long sector, char *buf);
#[no_mangle]
unsafe extern "C" fn build_block_map(part: *mut partition, block_no: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scan_header(part: *mut partition) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rfd_ftl_readsect(dev: *mut mtd_blktrans_dev, sector: u_long, buf: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn erase_block(part: *mut partition, block: c_int) -> c_int {
    ...
    let mut magic: u16 = cpu_to_le16(RFD_MAGIC);
    size_t retlen;
...
#[no_mangle]
unsafe extern "C" fn move_block_contents(part: *mut partition, block_no: c_int, old_sector: *mut u_long) -> c_int {
    ...
    for (i=0; i<part.data_sectors_per_block; i++) {
    let mut entry: u16 = le16_to_cpu(map[HEADER_MAP_OFFSET + i]);
    u_long addr;
...
#[no_mangle]
unsafe extern "C" fn reclaim_block(part: *mut partition, old_sector: *mut u_long) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn find_free_block(part: *mut partition) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn find_writable_block(part: *mut partition, old_sector: *mut u_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mark_sector_deleted(part: *mut partition, old_addr: u_long) -> c_int {
    static int mark_sector_deleted(struct partition *part, u_long old_addr)
    {
    int block, offset, rc;
    u_long addr;
    size_t retlen;
    let mut del: u16 = cpu_to_le16(SECTOR_DELETED);
...
#[no_mangle]
unsafe extern "C" fn find_free_sector(part: *const partition, block: *const block) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_writesect(dev: *mut mtd_blktrans_dev, sector: u_long, buf: *mut c_char, old_addr: *mut c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rfd_ftl_writesect(dev: *mut mtd_blktrans_dev, sector: u_long, buf: *mut c_char) -> c_int {
    static int rfd_ftl_writesect(struct mtd_blktrans_dev *dev, u_long sector, char *buf)
    {
    struct partition *part = container_of(dev, struct partition, mbd);
    u_long old_addr;
...
#[no_mangle]
unsafe extern "C" fn rfd_ftl_getgeo(dev: *mut mtd_blktrans_dev, geo: *mut hd_geometry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rfd_ftl_add_mtd(tr: *mut mtd_blktrans_ops, mtd: *mut mtd_info) {
    ...
    part.mbd.readonly = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: part->errors) -> else {
...
#[no_mangle]
unsafe extern "C" fn rfd_ftl_remove_dev(dev: *mut mtd_blktrans_dev) {
