...
//! Source: drivers/mtd/parsers/afs.c
#![no_std]
...
pub const AFSV1_FOOTER_MAGIC: c_uint = 0xA0FFFF9F;
pub const AFSV2_FOOTER_MAGIC1: c_uint = 0x464C5348 /* "FLSH" */;
pub const AFSV2_FOOTER_MAGIC2: c_uint = 0x464F4F54 /* "FOOT" */;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct footer_v1 {
    pub /: *mut *mut u32 image_info_base; / Address of first word of ImageFooter,
    pub /: *mut *mut u32 image_start; / Start of area reserved by this footer,
    pub /: *mut *mut u32 signature; / 'Magic' number proves it's a footer,
    pub /: *mut *mut u32 type; / Area type: ARM Image, SIB, customer,
    pub /: *mut *mut u32 checksum; / Just this structure,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct image_info_v1 {
    pub /: *mut *mut u32 bootFlags; / Boot flags, compression etc.,
    pub /: *mut *mut u32 imageNumber; / Unique number, selects for boot etc.,
    pub /: *mut *mut u32 loadAddress; / Address program should be loaded to,
    pub /: *mut *mut u32 length; / Actual size of image,
    pub /: *mut *mut u32 address; / Image is executed from here,
    pub /: *mut *mut char name[16]; / Null terminated,
    pub /: *mut *mut u32 headerBase; / Flash Address of any stripped header,
    pub /: *mut *mut u32 header_length; / Length of header in memory,
    pub /: *mut *mut u32 headerType; / AIF, RLF, s-record etc.,
    pub /: *mut *mut u32 checksum; / Image checksum (inc. this struct),
}

#[no_mangle]
unsafe extern "C" fn word_sum(words: *mut c_void, num: c_int) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn word_sum_v2(p: *mut u32, num: u32) -> u32 {
...
// 64bit platforms have 8 bytes padding
    ...
    let mut region_load_addr: u32 = imginfo[pad + 3 + i*4];
    let mut region_size: u32 = imginfo[pad + 4 + i*4];
    let mut region_offset: u32 = imginfo[pad + 5 + i*4];
    u32 region_start;
    u32 region_end;
