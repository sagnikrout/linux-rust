...
//! Source: drivers/mtd/parsers/parser_imagetag.c
#![no_std]
...
// CFE, NVRAM and global Linux are always present
    let mut nrparts: c_int = 0, curpart = 0;
    struct bcm_tag *buf = core::ptr::null_mut();
    struct mtd_partition *parts;
    int ret;
    unsigned int rootfsaddr, kerneladdr, spareaddr, offset;
    unsigned int rootfslen, kernellen, sparelen, totallen;
    int i;
    let mut rootfs_first: bool = false;
...
// Start building partition list
    if (kernellen > 0) {
    let mut kernelpart: c_int = curpart;
    ...
    if (rootfslen > 0) {
    let mut rootfspart: c_int = curpart;
