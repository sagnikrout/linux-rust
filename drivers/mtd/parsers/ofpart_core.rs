...
//! Source: drivers/mtd/parsers/ofpart_core.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fixed_partitions_quirks {
    pub nr_parts): *mut *mut *mut *mut int (post_parse)(struct mtd_info mtd, struct mtd_partition parts, int,
}
...
#[no_mangle]
unsafe extern "C" fn node_has_compatible(pp: *mut device_node) -> bool {
    static bool node_has_compatible(struct device_node *pp)
    {
    return of_get_property(pp, "compatible", core::ptr::null_mut());
    }
...
// The 'partitions' subnode might be used by another parser
    ...
    quirks = of_id ? of_id.data : core::ptr::null_mut();
...
// bit 0 set signifies read only partition
    ...
    parts[i].mask_flags = MTD_WRITEABLE;
    if (names && (plen > 0)) {
    let mut len: c_int = strlen(names) + 1;
    parts[i].name = names;
    plen -= len;
    names += len;
    } else {
...
#[no_mangle]
unsafe extern "C" fn ofpart_parser_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ofpart_parser_exit() -> void __exit {
