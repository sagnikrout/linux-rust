...
// Device attributes corresponding to files in '/<sysfs>/class/ubi/ubiX_Y'
    ...
    __ATTR(reserved_ebs, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
    static struct device_attribute attr_vol_type =
    __ATTR(type, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
    static struct device_attribute attr_vol_name =
    __ATTR(name, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
    static struct device_attribute attr_vol_corrupted =
    __ATTR(corrupted, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
    static struct device_attribute attr_vol_alignment =
    __ATTR(alignment, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
    static struct device_attribute attr_vol_usable_eb_size =
    __ATTR(usable_eb_size, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
    static struct device_attribute attr_vol_data_bytes =
    __ATTR(data_bytes, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
    static struct device_attribute attr_vol_upd_marker =
    __ATTR(upd_marker, S_IRUGO, vol_attribute_show, core::ptr::null_mut());
//
...
// Take a reference to prevent volume removal
    ...
    ret = sprintf(buf, "%d\n", vol.reserved_pebs);
#[no_mangle]
pub unsafe extern "C" fn if(&attr_vol_type: attr ==) -> else {
    const char *tp;
    ...
    ret = sprintf(buf, "%s\n", vol.name);
#[no_mangle]
pub unsafe extern "C" fn if(&attr_vol_corrupted: attr ==) -> else {
    else if (attr == &attr_vol_corrupted)
    ret = sprintf(buf, "%d\n", vol.corrupted);
#[no_mangle]
pub unsafe extern "C" fn if(&attr_vol_alignment: attr ==) -> else {
    else if (attr == &attr_vol_alignment)
    ret = sprintf(buf, "%d\n", vol.alignment);
#[no_mangle]
pub unsafe extern "C" fn if(&attr_vol_usable_eb_size: attr ==) -> else {
    else if (attr == &attr_vol_usable_eb_size)
    ret = sprintf(buf, "%d\n", vol.usable_leb_size);
#[no_mangle]
pub unsafe extern "C" fn if(&attr_vol_data_bytes: attr ==) -> else {
    else if (attr == &attr_vol_data_bytes)
    ret = sprintf(buf, "%lld\n", vol.used_bytes);
#[no_mangle]
pub unsafe extern "C" fn if(&attr_vol_upd_marker: attr ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn vol_release(dev: *mut device) {
    static void vol_release(struct device *dev)
    {
    struct ubi_volume *vol = container_of(dev, struct ubi_volume, dev);
    ubi_eba_replace_table(vol, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_create_volume(ubi: *mut ubi_device, req: *mut ubi_mkvol_req) -> c_int {
    int ubi_create_volume(struct ubi_device *ubi, struct ubi_mkvol_req *req)
    {
    int i, err, vol_id = req.vol_id;
    struct ubi_volume *vol;
    struct ubi_vtbl_record vtbl_rec;
    struct ubi_eba_table *eba_tbl = core::ptr::null_mut();
    if (ubi.ro_mode)
    return -EROFS;
...
//
    ...
    out_mapping:
    spin_lock(&ubi.volumes_lock);
    ubi.volumes[vol_id] = core::ptr::null_mut();
    ubi.vol_count -= 1;
    spin_unlock(&ubi.volumes_lock);
    out_acc:
    spin_lock(&ubi.volumes_lock);
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_remove_volume(desc: *mut ubi_volume_desc, no_vtbl: c_int) -> c_int {
...
//
    ...
    spin_unlock(&ubi.volumes_lock);
    if (!no_vtbl) {
    err = ubi_change_vtbl_record(ubi, vol_id, core::ptr::null_mut());
    if (err)
    goto out_err;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_resize_volume(desc: *mut ubi_volume_desc, reserved_pebs: c_int) -> c_int {
    ...
    struct ubi_device *ubi = vol.ubi;
    struct ubi_vtbl_record vtbl_rec;
    struct ubi_eba_table *new_eba_tbl = core::ptr::null_mut();
    struct ubi_eba_table *old_eba_tbl = core::ptr::null_mut();
    let mut vol_id: c_int = vol.vol_id;
    if (ubi.ro_mode)
    return -EROFS;
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_rename_volumes(ubi: *mut ubi_device, rename_list: *mut list_head) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_add_volume(ubi: *mut ubi_device, vol: *mut ubi_volume) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_free_volume(ubi: *mut ubi_device, vol: *mut ubi_volume) {
    void ubi_free_volume(struct ubi_device *ubi, struct ubi_volume *vol)
    {
    dbg_gen("free volume %d", vol.vol_id);
    ubi.volumes[vol.vol_id] = core::ptr::null_mut();
    cdev_del(&vol.cdev);
...
//
#[no_mangle]
unsafe extern "C" fn self_check_volume(ubi: *mut ubi_device, vol_id: c_int) -> c_int {
    static int self_check_volume(struct ubi_device *ubi, int vol_id)
    {
    let mut idx: c_int = vol_id2idx(ubi, vol_id);
...
//
#[no_mangle]
unsafe extern "C" fn self_check_volumes(ubi: *mut ubi_device) -> c_int {
    static int self_check_volumes(struct ubi_device *ubi)
    {
    int i, err = 0;
