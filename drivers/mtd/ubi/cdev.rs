...
//
#[no_mangle]
unsafe extern "C" fn get_exclusive(desc: *mut ubi_volume_desc) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn revoke_exclusive(desc: *mut ubi_volume_desc, mode: c_int) {
    ...
    vol.readers = 1;
#[no_mangle]
pub unsafe extern "C" fn if(UBI_READWRITE: mode ==) -> else {
    else if (mode == UBI_READWRITE)
    vol.writers = 1;
#[no_mangle]
pub unsafe extern "C" fn if(UBI_METAONLY: mode ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn vol_cdev_open(inode: *mut inode, file: *mut file) -> c_int {
    static int vol_cdev_open(struct inode *inode, struct file *file)
    {
    struct ubi_volume_desc *desc;
    let mut vol_id: c_int = iminor(inode) - 1, mode, ubi_num;
...
#[no_mangle]
unsafe extern "C" fn vol_cdev_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vol_cdev_llseek(file: *mut file, offset: loff_t, origin: c_int) -> loff_t {
...
// Update is in progress, seeking is prohibited
    ...
    let mut count_save: usize = count;
    void *tbuf;
...
// Create the re-name list
    ...
    for (i = 0; i < req.count; i++) {
    let mut vol_id: c_int = req.ents[i].vol_id;
    let mut name_len: c_int = req.ents[i].name_len;
...
// Find out the volumes which have to be removed
    ...
    struct ubi_volume_desc *desc;
    let mut no_remove_needed: c_int = 0;
//
...
#[no_mangle]
unsafe extern "C" fn ubi_get_ec_info(ubi: *mut ubi_device, ureq: *mut ubi_ecinfo_req __user) -> c_int {
...
// Attach an MTD device command
    ...
    mtd = get_mtd_device(core::ptr::null_mut(), req.mtd_num);
    if (IS_ERR(mtd)) {
    err = PTR_ERR(mtd);
    break;
    }
