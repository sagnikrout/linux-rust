...
//! Source: drivers/mtd/ubi/block.c
#![no_std]
...
pub const UBIBLOCK_MAX_DEVICES: c_int = 32;
// Maximum length of the 'block=' parameter
pub const UBIBLOCK_PARAM_LEN: c_int = 63;
// Maximum number of comma-separated items in the 'block=' parameter
pub const UBIBLOCK_PARAM_COUNT: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ubiblock_param {
    pub ubi_num: c_int,
    pub vol_id: c_int,
    pub name: [c_char; UBIBLOCK_PARAM_LEN+1],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ubiblock_pdu {
    pub usgl: ubi_sgl,
}
...
// One parameter: must be device path
    ...
    module_param_cb(block, &ubiblock_param_ops, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn ubiblock_read(req: *mut request) -> blk_status_t {
    static blk_status_t ubiblock_read(struct request *req)
    {
    struct ubiblock_pdu *pdu = blk_mq_rq_to_pdu(req);
    struct ubiblock *dev = req.q.queuedata;
    let mut pos: u64 = blk_rq_pos(req) << 9;
    let mut to_read: c_int = blk_rq_bytes(req);
    let mut bytes_left: c_int = to_read;
// Get LEB:offset address to read from
    let mut offset: c_int = do_div(pos, dev.leb_size);
    let mut leb: c_int = pos;
...
#[no_mangle]
unsafe extern "C" fn ubiblock_open(disk: *mut gendisk, mode: blk_mode_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ubiblock_release(gd: *mut gendisk) {
...
#[no_mangle]
unsafe extern "C" fn ubiblock_getgeo(disk: *mut gendisk, geo: *mut hd_geometry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn calc_disk_capacity(vi: *mut ubi_volume_info, disk_capacity: *mut u64) -> c_int {
    static int calc_disk_capacity(struct ubi_volume_info *vi, u64 *disk_capacity)
    {
    let mut size: u64 = vi.used_bytes >> 9;
...
#[no_mangle]
pub unsafe extern "C" fn ubiblock_create(vi: *mut ubi_volume_info) -> c_int {
...
// Must be the last step: anyone can call file ops from now on
    ret = device_add_disk(vi.dev, dev.gd, core::ptr::null_mut());
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn ubiblock_cleanup(dev: *mut ubiblock) {
    static void ubiblock_cleanup(struct ubiblock *dev)
    {
    let mut id: c_int = dev.gd.first_minor;
...
#[no_mangle]
pub unsafe extern "C" fn ubiblock_remove(vi: *mut ubi_volume_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ubiblock_resize(vi: *mut ubi_volume_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ubiblock_remove_all() {
...
#[no_mangle]
pub unsafe extern "C" fn ubiblock_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn ubiblock_exit() {
