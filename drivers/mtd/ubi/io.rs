...
//
#[no_mangle]
unsafe extern "C" fn do_sync_erase(ubi: *mut ubi_device, pnum: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn torture_peb(ubi: *mut ubi_device, pnum: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn nor_erase_prepare(ubi: *mut ubi_device, pnum: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_io_sync_erase(ubi: *mut ubi_device, pnum: c_int, torture: *mut c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_io_is_bad(ubi: *const ubi_device, pnum: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_io_mark_bad(ubi: *const ubi_device, pnum: c_int) -> c_int {
...
//
    ...
    const struct ubi_vid_hdr *vid_hdr)
    {
    let mut vol_type: c_int = vid_hdr.vol_type;
    let mut copy_flag: c_int = vid_hdr.copy_flag;
    let mut vol_id: c_int = be32_to_cpu(vid_hdr.vol_id);
    let mut lnum: c_int = be32_to_cpu(vid_hdr.lnum);
    let mut compat: c_int = vid_hdr.compat;
    let mut data_size: c_int = be32_to_cpu(vid_hdr.data_size);
    let mut used_ebs: c_int = be32_to_cpu(vid_hdr.used_ebs);
    let mut data_pad: c_int = be32_to_cpu(vid_hdr.data_pad);
    let mut data_crc: c_int = be32_to_cpu(vid_hdr.data_crc);
    let mut usable_leb_size: c_int = ubi.leb_size - data_pad;
...
//
#[no_mangle]
unsafe extern "C" fn self_check_not_bad(ubi: *const ubi_device, pnum: c_int) -> c_int {
    static int self_check_not_bad(const struct ubi_device *ubi, int pnum)
    {
    int err;
...
//
#[no_mangle]
unsafe extern "C" fn self_check_peb_ec_hdr(ubi: *const ubi_device, pnum: c_int) -> c_int {
    static int self_check_peb_ec_hdr(const struct ubi_device *ubi, int pnum)
    {
    int err;
...
//
#[no_mangle]
unsafe extern "C" fn self_check_peb_vid_hdr(ubi: *const ubi_device, pnum: c_int) -> c_int {
    static int self_check_peb_vid_hdr(const struct ubi_device *ubi, int pnum)
    {
    int err;
    uint32_t crc, hdr_crc;
...
//
    ...
    size_t read;
    void *buf1;
    let mut addr: loff_t = (loff_t)pnum * ubi.peb_size + offset;
    if (!ubi_dbg_chk_io(ubi))
    return 0;
    ...
    goto out_free;
    for (i = 0; i < len; i++) {
    let mut c: u8 = ((uint8_t *)buf)[i];
    let mut c1: u8 = ((uint8_t *)buf1)[i];
    int dump_len;
    if (c == c1)
    continue;
...
//
#[no_mangle]
pub unsafe extern "C" fn ubi_self_check_all_ff(ubi: *mut ubi_device, pnum: c_int, offset: c_int, len: c_int) -> c_int {
