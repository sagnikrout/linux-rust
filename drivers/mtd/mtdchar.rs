...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtd_file_info {
    pub mtd: *mut mtd_info,
    pub mode: enum mtd_file_modes,
}

#[no_mangle]
unsafe extern "C" fn mtdchar_lseek(file: *mut file, offset: loff_t, orig: c_int) -> loff_t {
...
#[no_mangle]
unsafe extern "C" fn mtdchar_open(inode: *mut inode, file: *mut file) -> c_int {
    static int mtdchar_open(struct inode *inode, struct file *file)
    {
    let mut minor: c_int = iminor(inode);
    let mut devnum: c_int = minor >> 1;
    let mut ret: c_int = 0;
    struct mtd_info *mtd;
...
// You can't open the RO devices RW
    ...
    return -EACCES;
    mtd = get_mtd_device(core::ptr::null_mut(), devnum);
    if (IS_ERR(mtd))
    return PTR_ERR(mtd);
...
// ====================================================================
#[no_mangle]
unsafe extern "C" fn mtdchar_close(inode: *mut inode, file: *mut file) -> c_int {
...
// ======================================================================
    ...
    ======================================================================*/
#[no_mangle]
unsafe extern "C" fn otp_select_filemode(mfi: *mut mtd_file_info, mode: c_int) -> c_int {
    ...
    if (mtd_read_fact_prot_reg(mtd, -1, 0, &retlen, core::ptr::null_mut()) ==
    -EOPNOTSUPP)
    return -EOPNOTSUPP;
    mfi.mode = MTD_FILE_MODE_OTP_FACTORY;
    break;
    case MTD_OTP_USER:
    if (mtd_read_user_prot_reg(mtd, -1, 0, &retlen, core::ptr::null_mut()) ==
    -EOPNOTSUPP)
    return -EOPNOTSUPP;
...
#[no_mangle]
unsafe extern "C" fn get_oobinfo(mtd: *mut mtd_info, to: *mut nand_oobinfo) -> c_int {
...
// Sanitize user input
    ...
    uint8_t *datbuf = core::ptr::null_mut(), *oobbuf = core::ptr::null_mut();
...
//
    ...
    ret = -EBADMSG;
#[no_mangle]
pub unsafe extern "C" fn if(0: req.ecc_stats.corrected_bitflips >) -> else {
...
#[no_mangle]
unsafe extern "C" fn mtdchar_ioctl(file: *mut file, cmd: u_int, arg: u_long) -> c_int {
...
// Legacy interface
    ...
    let mut nbr: c_int = retlen / sizeof(struct otp_info);
...
#[no_mangle]
unsafe extern "C" fn mtdchar_unlocked_ioctl(file: *mut file, cmd: u_int, arg: u_long) -> c_long {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtd_oob_buf32 {
    pub start: u_int32_t,
    pub length: u_int32_t,
    pub /: *mut *mut *mut compat_caddr_t ptr; / unsigned char,
}
...
#[no_mangle]
unsafe extern "C" fn mtdchar_mmap_capabilities(file: *mut file) -> unsigned {
...
//
#[no_mangle]
unsafe extern "C" fn mtdchar_mmap_prepare(desc: *mut vm_area_desc) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn init_mtdchar() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn cleanup_mtdchar() -> void __exit {
