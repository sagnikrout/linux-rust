...
//! Source: drivers/mtd/chips/cfi_cmdset_0020.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn cfi_staa_read(mtd: *mut mtd_info, from: loff_t, len: usize, retlen: *mut usize, buf: *mut u_char) -> c_int {
...
//
    ...
    {
    unsigned long i;
    let mut totlen: usize = 0, thislen;
    let mut ret: c_int = 0;
...
//
    ...
    return -ENOMEM;
    for (i=0; i<count; i++) {
    let mut elem_len: usize = vecs[i].iov_len;
...
#[no_mangle]
pub unsafe extern "C" fn do_erase_oneblock(map: *mut map_info, chip: *mut flchip, adr: c_ulong) -> c_int {
...
// check for lock bit
    if (map_word_bitsset(map, status, CMD(0x3a))) {
    let mut chipstatus: c_uchar = status.x[0];
...
#[no_mangle]
unsafe extern "C" fn cfi_staa_sync(mtd: *mut mtd_info) {
...
#[no_mangle]
pub unsafe extern "C" fn do_lock_oneblock(map: *mut map_info, chip: *mut flchip, adr: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cfi_staa_lock(mtd: *mut mtd_info, ofs: loff_t, len: u64) -> c_int {
    static int cfi_staa_lock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
    {
    struct map_info *map = mtd.priv;
    struct cfi_private *cfi = map.fldrv_priv;
    ...
    cfi_send_gen_cmd(0x90, 0x55, 0, map, cfi, cfi.device_type, core::ptr::null_mut());
    printk("before lock: block status register is %x\n",cfi_read_query(map, adr+(2*ofs_factor)));
    cfi_send_gen_cmd(0xff, 0x55, 0, map, cfi, cfi.device_type, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn do_unlock_oneblock(map: *mut map_info, chip: *mut flchip, adr: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cfi_staa_unlock(mtd: *mut mtd_info, ofs: loff_t, len: u64) -> c_int {
    static int cfi_staa_unlock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
    {
    struct map_info *map = mtd.priv;
    struct cfi_private *cfi = map.fldrv_priv;
    ...
    {
    let mut temp_adr: c_ulong = adr;
    let mut temp_len: c_ulong = len;
    cfi_send_gen_cmd(0x90, 0x55, 0, map, cfi, cfi.device_type, core::ptr::null_mut());
    while (temp_len) {
...
#[no_mangle]
unsafe extern "C" fn cfi_staa_suspend(mtd: *mut mtd_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cfi_staa_resume(mtd: *mut mtd_info) {
...
#[no_mangle]
unsafe extern "C" fn cfi_staa_destroy(mtd: *mut mtd_info) {
