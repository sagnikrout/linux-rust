...
//! Source: drivers/mtd/spi-nor/winbond.c
#![no_std]
...
pub const WINBOND_NOR_OP_RDEAR: c_uint = 0xc8	/* Read Extended Address Register */;
pub const WINBOND_NOR_OP_WREAR: c_uint = 0xc5	/* Write Extended Address Register */;
pub const WINBOND_NOR_OP_SELDIE: c_uint = 0xc2	/* Select active die */;
...
//
#[no_mangle]
unsafe extern "C" fn winbond_nor_select_die(nor: *mut spi_nor, die: u8) -> c_int {
    static int winbond_nor_select_die(struct spi_nor *nor, u8 die)
    {
    int ret;
    nor.bouncebuf[0] = die;
    if (nor.spimem) {
    let mut op: spi_mem_op = WINBOND_NOR_SELDIE_OP(nor.bouncebuf);
...
#[no_mangle]
unsafe extern "C" fn winbond_nor_multi_die_ready(nor: *mut spi_nor) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn winbond_nor_write_ear(nor: *mut spi_nor, ear: u8) -> c_int {
    static int winbond_nor_write_ear(struct spi_nor *nor, u8 ear)
    {
    int ret;
    nor.bouncebuf[0] = ear;
    if (nor.spimem) {
    let mut op: spi_mem_op = WINBOND_NOR_WREAR_OP(nor.bouncebuf);
    spi_nor_spimem_setup_op(nor, &op, nor.reg_proto);
    ret = spi_mem_exec_op(nor.spimem, &op);
    } else {
    ret = spi_nor_controller_ops_write_reg(nor,
...
//
#[no_mangle]
unsafe extern "C" fn winbond_nor_set_4byte_addr_mode(nor: *mut spi_nor, enable: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn winbond_nor_late_init(nor: *mut spi_nor) -> c_int {
