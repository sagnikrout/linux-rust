...
//! Source: drivers/irqchip/irq-hip04.c
#![no_std]
...
pub const HIP04_MAX_IRQS: c_int = 510;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hip04_irq_data {
...
// by the GIC itself.
//
pub const NR_HIP04_CPU_IF: c_int = 16;
...
#[no_mangle]
pub unsafe extern "C" fn hip04_irq(d: *mut irq_data) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn hip04_mask_irq(d: *mut irq_data) {
    static void hip04_mask_irq(struct irq_data *d)
    {
    let mut mask: u32 = 1 << (hip04_irq(d) % 32);
...
#[no_mangle]
unsafe extern "C" fn hip04_unmask_irq(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn hip04_ipi_send_mask(d: *mut irq_data, mask: *const cpumask) {
...
#[no_mangle]
unsafe extern "C" fn hip04_handle_irq(regs: *mut pt_regs) -> void __exception_irq_entry {
...
#[no_mangle]
unsafe extern "C" fn hip04_get_cpumask(intc: *mut hip04_irq_data) -> u16 {
...
#[no_mangle]
unsafe extern "C" fn hip04_irq_dist_init(intc: *mut hip04_irq_data) -> void __init {
    static void __init hip04_irq_dist_init(struct hip04_irq_data *intc)
    {
    unsigned int i;
    u32 cpumask;
    let mut nr_irqs: c_uint = intc.nr_irqs;
    void __iomem *base = intc.dist_base;
...
#[no_mangle]
unsafe extern "C" fn hip04_irq_cpu_init(intc: *mut hip04_irq_data) {
...
#[no_mangle]
unsafe extern "C" fn hip04_irq_starting_cpu(cpu: c_uint) -> c_int {
...
//
    ...
    hip04_irq_starting_cpu, core::ptr::null_mut());
    return 0;
    }
