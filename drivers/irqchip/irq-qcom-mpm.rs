...
//! Source: drivers/irqchip/irq-qcom-mpm.c
#![no_std]
...
//
pub const MPM_REG_ENABLE: c_int = 0;
pub const MPM_REG_FALLING_EDGE: c_int = 1;
pub const MPM_REG_RISING_EDGE: c_int = 2;
pub const MPM_REG_POLARITY: c_int = 3;
pub const MPM_REG_STATUS: c_int = 4;
// MPM pin map to GIC hwirq
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mpm_gic_map {
    pub pin: c_int,
...
pub struct qcom_mpm_priv {
    ...
    unsigned int index)
    {
    let mut offset: c_uint = (reg * priv.reg_stride + index + 2) * 4;
...
#[no_mangle]
unsafe extern "C" fn qcom_mpm_enable_irq(d: *mut irq_data, en: bool) {
    static void qcom_mpm_enable_irq(struct irq_data *d, bool en)
    {
    struct qcom_mpm_priv *priv = d.chip_data;
    let mut pin: c_int = d.hwirq;
    let mut index: c_uint = pin / 32;
    let mut shift: c_uint = pin % 32;
...
#[no_mangle]
unsafe extern "C" fn qcom_mpm_mask(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn qcom_mpm_unmask(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn qcom_mpm_set_type(d: *mut irq_data, type: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qcom_mpm_handler(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn mpm_pd_power_off(genpd: *mut generic_pm_domain) -> c_int {
...
// Notify RPM to write vMPM into HW
    ret = mbox_send_message(priv.mbox_chan, core::ptr::null_mut());
    if (ret < 0)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn gic_hwirq_is_mapped(maps: *mut mpm_gic_map, cnt: c_int, hwirq: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn qcom_mpm_probe(pdev: *mut platform_device, parent: *mut device_node) -> c_int {
...
// Otherwise, fall back to simple MMIO.
    ...
    if (!genpd.name)
    return -ENOMEM;
    ret = pm_genpd_init(genpd, core::ptr::null_mut(), false);
    if (ret) {
