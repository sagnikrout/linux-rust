...
//! Source: drivers/irqchip/irq-mvebu-sei.c
#![no_std]
...
pub const GICP_SET_SEI_OFFSET: c_uint = 0x30;
pub const SEI_IRQ_COUNT_PER_REG: c_int = 32;
pub const SEI_IRQ_REG_COUNT: c_int = 2;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_sei_interrupt_range {
    pub first: u32,
    pub size: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_sei_caps {
    pub ap_range: mvebu_sei_interrupt_range,
    pub cp_range: mvebu_sei_interrupt_range,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_sei {
    pub dev: *mut device,
    pub base: *mut void __iomem,
    pub res: *mut resource,
    pub sei_domain: *mut irq_domain,
    pub ap_domain: *mut irq_domain,
    pub cp_domain: *mut irq_domain,
    pub caps: *const mvebu_sei_caps,
// Lock on MSI allocations/releases
    pub cp_msi_lock: mutex,
    pub SEI_IRQ_COUNT): DECLARE_BITMAP(cp_msi_bitmap,,
// Lock on IRQ masking register
    pub mask_lock: raw_spinlock_t,
}

#[no_mangle]
unsafe extern "C" fn mvebu_sei_ack_irq(d: *mut irq_data) {
    static void mvebu_sei_ack_irq(struct irq_data *d)
    {
    struct mvebu_sei *sei = irq_data_get_irq_chip_data(d);
    let mut reg_idx: u32 = SEI_IRQ_REG_IDX(d.hwirq);
    writel_relaxed(BIT(SEI_IRQ_REG_BIT(d.hwirq)),
    sei.base + GICP_SECR(reg_idx));
    }
#[no_mangle]
unsafe extern "C" fn mvebu_sei_mask_irq(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_sei_unmask_irq(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_sei_ap_set_type(data: *mut irq_data, type: c_uint) -> c_int {
    ...
    let mut set: phys_addr_t = sei.res.start + GICP_SET_SEI_OFFSET;
    msg.data = data.hwirq + sei.caps.cp_range.first;
    msg.address_lo = lower_32_bits(set);
    msg.address_hi = upper_32_bits(set);
    }
#[no_mangle]
unsafe extern "C" fn mvebu_sei_cp_set_type(data: *mut irq_data, type: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mvebu_sei_cp_release_irq(sei: *mut mvebu_sei, hwirq: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_sei_handle_cascade_irq(desc: *mut irq_desc) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_sei_reset(sei: *mut mvebu_sei) {
...
#[no_mangle]
unsafe extern "C" fn mvebu_sei_probe(pdev: *mut platform_device) -> c_int {
