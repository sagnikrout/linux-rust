...
//! Source: drivers/irqchip/irq-pic32-evic.c
#![no_std]
...
pub const REG_INTCON: c_uint = 0x0000;
pub const REG_INTSTAT: c_uint = 0x0020;
pub const REG_IFS_OFFSET: c_uint = 0x0040;
pub const REG_IEC_OFFSET: c_uint = 0x00C0;
pub const REG_IPC_OFFSET: c_uint = 0x0140;
pub const REG_OFF_OFFSET: c_uint = 0x0540;
pub const MAJPRI_MASK: c_uint = 0x07;
pub const SUBPRI_MASK: c_uint = 0x03;
pub const PRIORITY_MASK: c_uint = 0x1F;

    ((((pri) & MAJPRI_MASK) << 2) | ((subpri) & SUBPRI_MASK))
#[repr(C)]
#[derive(Copy, Clone)]
pub struct evic_chip_data {
    pub irq_types: [u32; NR_IRQS],
    pub ext_irqs: [u32; 8],
}
...
#[no_mangle]
unsafe extern "C" fn pic32_set_ext_polarity(bit: c_int, type: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pic32_bind_evic_interrupt(irq: c_int, set: c_int) {
    static void pic32_bind_evic_interrupt(int irq, int set)
    {
    writel(set, evic_base + REG_OFF_OFFSET + irq * 4);
    }
#[no_mangle]
unsafe extern "C" fn pic32_set_irq_priority(irq: c_int, priority: c_int) {
...
#[no_mangle]
unsafe extern "C" fn pic32_ext_irq_of_init(domain: *mut irq_domain) -> void __init {
...
//
    ...
    for (i = 0; i < nchips; i++) {
    let mut ifsclr: u32 = PIC32_CLR(REG_IFS_OFFSET + (i * 0x10));
    let mut iec: u32 = REG_IEC_OFFSET + (i * 0x10);
