...
//! Source: drivers/irqchip/irq-loongarch-cpu.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn lpic_gsi_to_irq(gsi: u32) -> u32 {
...
// Only pch irqdomain transferring is required for LoongArch.
    ...
    {
    int id;
    struct fwnode_handle *domain_handle = core::ptr::null_mut();
    switch (gsi) {
...
#[no_mangle]
unsafe extern "C" fn mask_loongarch_irq(d: *mut irq_data) {
    static void mask_loongarch_irq(struct irq_data *d)
    {
    clear_csr_ecfg(ECFGF(d.hwirq));
    }
#[no_mangle]
unsafe extern "C" fn unmask_loongarch_irq(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn handle_cpu_irq(regs: *mut pt_regs) {
    static void handle_cpu_irq(struct pt_regs *regs)
    {
    int hwirq;
    let mut estat: c_uint = read_csr_estat() & CSR_ESTAT_IS;
    ...
    &loongarch_cpu_intc_irq_domain_ops, core::ptr::null_mut());
    if (!irq_domain)
...
// Mask interrupts.
    ...
    panic("Failed to add irqdomain for LoongArch CPU");
    set_handle_irq(&handle_cpu_irq);
    acpi_set_irq_model(ACPI_IRQ_MODEL_LPIC, lpic_get_gsi_domain_id, core::ptr::null_mut());
    acpi_set_gsi_to_irq_fallback(lpic_gsi_to_irq);
    ret = acpi_cascade_irqdomain_init();
    return ret;
    }
    IRQCHIP_ACPI_DECLARE(cpuintc_v1, ACPI_MADT_TYPE_CORE_PIC,
    core::ptr::null_mut(), ACPI_MADT_CORE_PIC_VERSION_V1, cpuintc_acpi_init);
