...
//! Source: drivers/irqchip/irq-ast2700-intc0.c
#![no_std]
...
pub const INT_NUM: c_int = 480;
pub const INTM_NUM: c_int = 50;
pub const SWINT_NUM: c_int = 16;

pub const INTC0_IN_NUM: c_int = 480;
pub const INTC0_ROUTE_NUM: c_int = 5;
pub const INTC0_INTM_NUM: c_int = 50;
pub const INTC0_ROUTE_BITS: c_int = 3;
pub const GIC_P2P_SPI_END: c_int = 128;
pub const INTC0_SWINT_OUT_BASE: c_int = 144;
pub const INTC0_SWINT_IER: c_uint = 0x10;
pub const INTC0_SWINT_ISR: c_uint = 0x14;
pub const INTC0_INTBANKX_IER: c_uint = 0x1000;
pub const INTC0_INTBANK_SIZE: c_uint = 0x100;
pub const INTC0_INTBANK_GROUPS: c_int = 11;
pub const INTC0_INTBANKS_PER_GRP: c_int = 3;
pub const INTC0_INTMX_IER: c_uint = 0x1b00;
pub const INTC0_INTMX_ISR: c_uint = 0x1b04;
pub const INTC0_INTMX_BANK_SIZE: c_uint = 0x10;
pub const INTC0_IRQS_PER_BANK: c_int = 32;
pub const INTM_IRQS_PER_BANK: c_int = 10;

pub const INTC0_SEL_BASE: c_uint = 0x200;
pub const INTC0_SEL_BANK_SIZE: c_uint = 0x4;
pub const INTC0_SEL_ROUTE_SIZE: c_uint = 0x100;
#[no_mangle]
unsafe extern "C" fn aspeed_swint_irq_mask(data: *mut irq_data) {
    static void aspeed_swint_irq_mask(struct irq_data *data)
    {
    struct aspeed_intc0 *intc0 = irq_data_get_irq_chip_data(data);
    let mut bit: c_int = data.hwirq - SWINT_BASE;
    u32 ier;
...
#[no_mangle]
unsafe extern "C" fn aspeed_swint_irq_unmask(data: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn aspeed_swint_irq_eoi(data: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn aspeed_intc0_irq_mask(data: *mut irq_data) {
    static void aspeed_intc0_irq_mask(struct irq_data *data)
    {
    struct aspeed_intc0 *intc0 = irq_data_get_irq_chip_data(data);
    let mut bank: c_int = (data.hwirq - INTM_BASE) / INTM_IRQS_PER_BANK;
    let mut bit: c_int = (data.hwirq - INTM_BASE) % INTM_IRQS_PER_BANK;
    u32 ier;
    guard(raw_spinlock)(&intc0.intc_lock);
...
#[no_mangle]
unsafe extern "C" fn aspeed_intc0_irq_unmask(data: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn aspeed_intc0_irq_eoi(data: *mut irq_data) {
    ...
    let mut range: aspeed_intc_interrupt_range = intc0.ranges.ranges[i];
...
//
    ...
    let mut c1o: u32 = c1outs[i];
    if (c1o == AST2700_INTC_INVALID_ROUTE)
    continue;
    for (size_t j = 0; j < nc1ranges; j++) {
    let mut c1r: aspeed_intc_interrupt_range = c1ranges[j];
    u32 input;
//
...
//
    if (input < GIC_P2P_SPI_END)
    continue;
    ret = resolve_parent_route_for_input(intc0, parent_fwnode, input, core::ptr::null_mut());
    if (ret < 0)
    continue;
...
#[no_mangle]
pub unsafe extern "C" fn if(INTM_BASE: hwirq <) -> else {
    else if (hwirq < INTM_BASE)
    return -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(SWINT_BASE: hwirq <) -> else {
    else if (hwirq < SWINT_BASE)
    irq_set_chip_and_handler(irq, &aspeed_intm_chip, handle_level_irq);
#[no_mangle]
pub unsafe extern "C" fn if(INT0_NUM: hwirq <) -> else {
...
pub unsafe extern "C" fn if(SWINT_BASE: hwirq <) -> else {
    ...
    {
    struct aspeed_intc0 *intc0 = irq_data_get_irq_chip_data(data);
    let mut hwirq: c_ulong = data.hwirq;
    int route, bank, bit;
    u32 mask;
    if (hwirq >= INT0_NUM)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn aspeed_intc0_disable_swint(intc0: *mut aspeed_intc0) {
    static void aspeed_intc0_disable_swint(struct aspeed_intc0 *intc0)
    {
    writel(0, intc0.base + INTC0_SWINT_IER);
    }
#[no_mangle]
unsafe extern "C" fn aspeed_intc0_disable_intbank(intc0: *mut aspeed_intc0) {
...
#[no_mangle]
unsafe extern "C" fn aspeed_intc0_disable_intm(intc0: *mut aspeed_intc0) {
