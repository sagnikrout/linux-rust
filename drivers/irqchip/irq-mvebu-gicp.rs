...
//! Source: drivers/irqchip/irq-mvebu-gicp.c
#![no_std]
...
pub const GICP_SETSPI_NSR_OFFSET: c_uint = 0x0;
pub const GICP_CLRSPI_NSR_OFFSET: c_uint = 0x8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_gicp_spi_range {
    pub start: c_uint,
    pub count: c_uint,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mvebu_gicp {
    pub spi_ranges: *mut mvebu_gicp_spi_range,
    pub spi_ranges_cnt: c_uint,
    pub spi_cnt: c_uint,
    pub spi_bitmap: *mut c_ulong,
    pub spi_lock: spinlock_t,
    pub res: *mut resource,
    pub dev: *mut device,
}

#[no_mangle]
unsafe extern "C" fn gicp_idx_to_spi(gicp: *mut mvebu_gicp, idx: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gicp_compose_msi_msg(data: *mut irq_data, msg: *mut msi_msg) {
    static void gicp_compose_msi_msg(struct irq_data *data, struct msi_msg *msg)
    {
    struct mvebu_gicp *gicp = data.chip_data;
    let mut setspi: phys_addr_t = gicp.res.start + GICP_SETSPI_NSR_OFFSET;
    let mut clrspi: phys_addr_t = gicp.res.start + GICP_CLRSPI_NSR_OFFSET;
...
#[no_mangle]
unsafe extern "C" fn mvebu_gicp_probe(pdev: *mut platform_device) -> c_int {
