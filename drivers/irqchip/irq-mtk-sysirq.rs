...
//! Source: drivers/irqchip/irq-mtk-sysirq.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_sysirq_chip_data {
    pub lock: raw_spinlock_t,
    pub nr_intpol_bases: u32,
    pub intpol_bases: *mut void __iomem,
    pub intpol_words: *mut u32,
    pub intpol_idx: *mut u8,
    pub which_word: *mut u16,
}

#[no_mangle]
unsafe extern "C" fn mtk_sysirq_set_type(data: *mut irq_data, type: c_uint) -> c_int {
    static int mtk_sysirq_set_type(struct irq_data *data, unsigned int type)
    {
    let mut hwirq: irq_hw_number_t = data.hwirq;
    struct mtk_sysirq_chip_data *chip_data = data.chip_data;
    let mut intpol_idx: u8 = chip_data.intpol_idx[hwirq];
    void __iomem *base;
...
// type = fwspec->param[2] & IRQ_TYPE_SENSE_MASK;
    ...
    let mut gic_fwspec: irq_fwspec = *fwspec;
    if (fwspec.param_count != 3)
    return -EINVAL;
// sysirq doesn't support PPI
    ...
    if (!chip_data)
    return -ENOMEM;
    while (of_get_address(node, i++, core::ptr::null_mut(), core::ptr::null_mut()))
    nr_intpol_bases++;
