...
//! Source: drivers/irqchip/irq-crossbar.c
#![no_std]
...
pub const GIC_IRQ_START: c_int = 32;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct crossbar_device {
    pub lock: raw_spinlock_t,
    pub int_max: c_uint,
    pub safe_map: c_uint,
    pub max_crossbar_sources: c_uint,
    pub irq_map: *mut c_uint,
    pub crossbar_base: *mut void __iomem,
    pub register_offsets: *mut c_int,
    pub int): *mut *mut void (write)(int,,
}

    static struct crossbar_device *cb;
#[no_mangle]
unsafe extern "C" fn crossbar_writel(irq_no: c_int, cb_no: c_int) {
    static void crossbar_writel(int irq_no, int cb_no)
    {
    writel(cb_no, cb.crossbar_base + cb.register_offsets[irq_no]);
    }
#[no_mangle]
unsafe extern "C" fn crossbar_writew(irq_no: c_int, cb_no: c_int) {
    static void crossbar_writew(int irq_no, int cb_no)
    {
    writew(cb_no, cb.crossbar_base + cb.register_offsets[irq_no]);
    }
#[no_mangle]
unsafe extern "C" fn crossbar_writeb(irq_no: c_int, cb_no: c_int) {
    ...
    for (i = 0; i < nr_irqs; i++) {
    let mut err: c_int = allocate_gic_irq(d, virq + i, hwirq + i);
    if (err)
    return err;
    irq_domain_set_hwirq_and_chip(d, virq + i, hwirq + i,
    &crossbar_chip, core::ptr::null_mut());
    }
    return 0;
    }
//
...
#[no_mangle]
unsafe extern "C" fn crossbar_of_init(node: *mut device_node) -> int __init {
    static int __init crossbar_of_init(struct device_node *node)
    {
    let mut max: u32 = 0, entry, reg_size;
...
// Initialize the crossbar with safe map to start with
    ...
    of_fwnode_handle(node), &crossbar_domain_ops, core::ptr::null_mut());
    if (!domain) {
