...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct at25_data {
    pub chip: spi_eeprom,
    pub spimem: *mut spi_mem,
    pub lock: mutex,
    pub addrlen: unsigned,
    pub nvmem_config: nvmem_config,
    pub nvmem: *mut nvmem_device,
    pub sernum: [u8; FM25_SN_LEN],
    pub id: [u8; FM25_MAX_ID_LEN],
    pub id_len: u8,
}

pub const AT25_WREN: c_uint = 0x06		/* latch the write enable */;
pub const AT25_WRDI: c_uint = 0x04		/* reset the write enable */;
pub const AT25_RDSR: c_uint = 0x05		/* read status register */;
pub const AT25_WRSR: c_uint = 0x01		/* write status register */;
pub const AT25_READ: c_uint = 0x03		/* read byte(s) */;
pub const AT25_WRITE: c_uint = 0x02		/* write byte(s)/sector */;
pub const FM25_SLEEP: c_uint = 0xb9		/* enter sleep mode */;
pub const FM25_RDID: c_uint = 0x9f		/* read device ID */;
pub const FM25_RDSN: c_uint = 0xc3		/* read S/N */;
pub const AT25_SR_nRDY: c_uint = 0x01		/* nRDY = write-in-progress */;
pub const AT25_SR_WEN: c_uint = 0x02		/* write enable (latched) */;
pub const AT25_SR_BP0: c_uint = 0x04		/* BP for software writeprotect */;
pub const AT25_SR_BP1: c_uint = 0x08;
pub const AT25_SR_WPEN: c_uint = 0x80		/* writeprotect enable */;
pub const AT25_INSTR_BIT3: c_uint = 0x08		/* additional address bit in instr */;
//
// Specs often allow 5ms for a page write, sometimes 20ms;
// it's important to recover from write timeouts.
//
pub const EE_TIMEOUT: c_int = 25;
// -------------------------------------------------------------------------

// Handle the address MSB as part of instruction byte
#[no_mangle]
unsafe extern "C" fn at25_instr(at25: *mut at25_data, instr: u8, off: c_uint) -> u8 {
    ...
    char *buf = val;
    let mut msg_offset: c_uint = offset;
    let mut bytes_left: usize = count;
    size_t segment;
    int status;
    if (!bounce)
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn sernum_show(dev: *mut device, attr: *mut device_attribute, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn jedec_id_show(dev: *mut device, attr: *mut device_attribute, buf: *mut c_char) -> isize {
...
//
#[no_mangle]
unsafe extern "C" fn at25_wait_ready(at25: *mut at25_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn at25_ee_write(priv: *mut c_void, off: c_uint, val: *mut c_void, count: usize) -> c_int {
...
// -------------------------------------------------------------------------
#[no_mangle]
unsafe extern "C" fn at25_fw_to_chip(dev: *mut device, chip: *mut spi_eeprom) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn at25_fram_to_chip(dev: *mut device, chip: *mut spi_eeprom) -> c_int {
...
// There are inside-out FRAM variations, detect them and reverse the ID bytes
    ...
    let mut tmp: u8 = id[i];
    let mut j: c_int = ARRAY_SIZE(id) - i - 1;
    id[i] = id[j];
    id[j] = tmp;
    }
    if (id[6] == 0xc2) {
...
#[no_mangle]
unsafe extern "C" fn at25_probe(mem: *mut spi_mem) -> c_int {
...
// For now we only support 8/16/24 bit addressing
    ...
    at25.addrlen = 1;
#[no_mangle]
pub unsafe extern "C" fn if(EE_ADDR2: at25->chip.flags &) -> else {
    else if (at25.chip.flags & EE_ADDR2)
    at25.addrlen = 2;
#[no_mangle]
pub unsafe extern "C" fn if(EE_ADDR3: at25->chip.flags &) -> else {
