...
pub struct m24lr {
    ...
    pub page_size: c_uint,
    pub eeprom_size: c_uint,
    pub ctl_regmap: *mut regmap,
    pub eeprom_regmap: *mut regmap,
    pub /: *mut *mut mutex lock; / synchronize operations to the device,
}
...
#[no_mangle]
unsafe extern "C" fn m24lr_probe(client: *mut i2c_client) -> c_int {
    static int m24lr_probe(struct i2c_client *client)
    {
    let mut eeprom_regmap_conf: regmap_config = {0};
    let mut nvmem_conf: nvmem_config = {0};
...
#[no_mangle]
unsafe extern "C" fn m24lr_remove(client: *mut i2c_client) {
