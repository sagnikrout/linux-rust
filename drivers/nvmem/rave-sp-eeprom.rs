...
//! Source: drivers/nvmem/rave-sp-eeprom.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rave_sp_eeprom_page {
    pub type: u8,
    pub success: u8,
    pub data: [u8; RAVE_SP_EEPROM_PAGE_SIZE],
    pub __packed: },
//
...
//
    ...
    {
    let mut is_write: bool = type == RAVE_SP_EEPROM_WRITE;
    let mut data_size: c_uint = is_write ? sizeof(page.data) : 0;
    let mut cmd_size: c_uint = eeprom.header_size + data_size;
...
//
    ...
    size_t data_len)
    {
    let mut page_offset: c_uint = offset % RAVE_SP_EEPROM_PAGE_SIZE;
    let mut page_nr: c_uint = offset / RAVE_SP_EEPROM_PAGE_SIZE;
    struct rave_sp_eeprom_page page;
    int ret;
//
...
#[no_mangle]
unsafe extern "C" fn rave_sp_eeprom_probe(pdev: *mut platform_device) -> c_int {
    static int rave_sp_eeprom_probe(struct platform_device *pdev)
    {
    struct device *dev = &pdev.dev;
    struct rave_sp *sp = dev_get_drvdata(dev.parent);
    struct device_node *np = dev.of_node;
    let mut config: nvmem_config = { 0 };
