...
//! Source: drivers/iommu/iommufd/iova_bitmap.c
#![no_std]
...
// IOVA range representing the currently mapped bitmap data
    pub mapped: iova_bitmap_map,
// userspace address of the bitmap
    pub bitmap: *mut u8 __user,
// u64 index that @mapped points to
    pub mapped_base_index: c_ulong,
// how many u64 can we walk in total
    pub mapped_total_index: c_ulong,
...
//
    ...
    unsigned long index)
    {
    let mut pgshift: c_ulong = bitmap.mapped.pgshift;
...
//
#[no_mangle]
unsafe extern "C" fn iova_bitmap_mapped_iova(bitmap: *mut iova_bitmap) -> c_ulong {
    static unsigned long iova_bitmap_mapped_iova(struct iova_bitmap *bitmap)
    {
    let mut skip: c_ulong = bitmap.mapped_base_index;
...
//
#[no_mangle]
unsafe extern "C" fn iova_bitmap_get(bitmap: *mut iova_bitmap) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn iova_bitmap_put(bitmap: *mut iova_bitmap) {
...
//
#[no_mangle]
pub unsafe extern "C" fn iova_bitmap_free(bitmap: *mut iova_bitmap) {
    ...
    mapped.pages = core::ptr::null_mut();
    }
    kfree(bitmap);
    }
...
//
#[no_mangle]
unsafe extern "C" fn iova_bitmap_mapped_remaining(bitmap: *mut iova_bitmap) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn iova_bitmap_mapped_length(bitmap: *mut iova_bitmap) -> c_ulong {
    static unsigned long iova_bitmap_mapped_length(struct iova_bitmap *bitmap)
    {
    let mut max_iova: c_ulong = bitmap.iova + bitmap.length - 1;
    let mut iova: c_ulong = iova_bitmap_mapped_iova(bitmap);
...
//
    ...
    mapped.pgshift) + mapped.pgoff * BITS_PER_BYTE;
    do {
    let mut page_idx: c_uint = cur_bit / BITS_PER_PAGE;
    let mut offset: c_uint = cur_bit % BITS_PER_PAGE;
