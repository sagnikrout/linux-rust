...
//! Source: drivers/iommu/iommufd/eventq.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_fault_destroy(obj: *mut iommufd_object) {
...
// Fetch the first node out of the fault->deliver list
    ...
    struct iopf_group *group = core::ptr::null_mut();
    spin_lock(&fault.common.lock);
    if (!list_empty(list)) {
...
// Restore a node back to the head of the fault->deliver list
    ...
    {
    let mut fault_size: usize = sizeof(struct iommu_hwpt_pgfault);
    struct iommufd_eventq *eventq = filep.private_data;
    struct iommufd_fault *fault = eventq_to_fault(eventq);
    let mut data: iommu_hwpt_pgfault = {};
    struct iommufd_device *idev;
    struct iopf_group *group;
...
// Read count doesn't fit the first fault group
    ...
    mutex_unlock(&fault.mutex);
    let mut done: return = = 0 ? rc : done;
    }
    static ssize_t iommufd_fault_fops_write(struct file *filep, const char __user *buf,
    size_t count, loff_t *ppos)
    {
    let mut response_size: usize = sizeof(struct iommu_hwpt_page_response);
    struct iommufd_eventq *eventq = filep.private_data;
    struct iommufd_fault *fault = eventq_to_fault(eventq);
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_veventq_abort(obj: *mut iommufd_object) {
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_veventq_destroy(obj: *mut iommufd_object) {
    ...
    struct iommufd_vevent *vevent = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn iommufd_eventq_fops_release(inode: *mut inode, filep: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_fault_alloc(ucmd: *mut iommufd_ucmd) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_fault_iopf_handler(group: *mut iopf_group) -> c_int {
    ...
    INIT_EVENTQ_FOPS(iommufd_veventq_fops_read, core::ptr::null_mut());
// An arbitrary upper bound for veventq_depth that fits all existing HWs

#[no_mangle]
pub unsafe extern "C" fn iommufd_veventq_alloc(ucmd: *mut iommufd_ucmd) -> c_int {
