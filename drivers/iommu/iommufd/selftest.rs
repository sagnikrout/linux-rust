...
//! Source: drivers/iommu/iommufd/selftest.c
#![no_std]
...
//
    ...
    let mut iommufd_test_memory_limit: usize = 65536;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mock_bus_type {
    pub bus: bus_type,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct syz_layout {
    pub nth_area: __u32,
    pub offset: __u32,
}

    struct syz_layout *syz = (void *)iova;
    let mut nth: c_uint = syz.nth_area;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mock_iommu_domain_nested {
    pub domain: iommu_domain,
    pub mock_viommu: *mut mock_viommu,
    pub iotlb: [u32; MOCK_NESTED_DOMAIN_IOTLB_NUM],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mock_viommu {
    pub core: iommufd_viommu,
    pub s2_parent: *mut mock_iommu_domain,
    pub hw_queue: [*mut mock_hw_queue; IOMMU_TEST_HW_QUEUE_MAX],
    pub queue_mutex: mutex,
    pub mmap_offset: c_ulong,
    pub /: *mut *mut *mut u32 page; / Mmap page to test u32 type of in_data,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mock_hw_queue {
    pub core: iommufd_hw_queue,
    pub mock_viommu: *mut mock_viommu,
    pub prev: *mut mock_hw_queue,
    pub index: u16,
}
...
pub struct mock_dev {
    pub dev: device,
    pub viommu: *mut mock_viommu,
    pub viommu_rwsem: rw_semaphore,
    pub iopf_rwsem: rw_semaphore,
    pub flags: c_ulong,
    pub vdev_id: c_ulong,
    pub id: c_int,
    pub cache: [u32; MOCK_DEV_CACHE_NUM],
    pub pasid_1024_fake_error: core::sync::atomic::AtomicI32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct selftest_obj {
    pub obj: iommufd_object,
    pub type: enum selftest_obj_type,
    union {
    struct {
    pub idev: *mut iommufd_device,
    pub ictx: *mut iommufd_ctx,
    pub mock_dev: *mut mock_dev,
    pub idev: },
}
    ...
    struct mock_viommu *new_viommu = core::ptr::null_mut();
    let mut vdev_id: c_ulong = 0;
    int rc;
...
// type = IOMMU_HW_INFO_TYPE_SELFTEST;
    ...
    let mut flags: c_ulong = mock.flags;
...
#[no_mangle]
unsafe extern "C" fn mock_domain_free(domain: *mut iommu_domain) {
...
//
    ...
    {
    let mut has_dirty_flag: bool = flags & IOMMU_HWPT_ALLOC_DIRTY_TRACKING;
    const u32 PAGING_FLAGS = IOMMU_HWPT_ALLOC_DIRTY_TRACKING |
    IOMMU_HWPT_ALLOC_NEST_PARENT |
    IOMMU_HWPT_ALLOC_PASID;
    struct mock_dev *mdev = to_mock_dev(dev);
    let mut no_dirty_ops: bool = mdev.flags & MOCK_FLAGS_DEVICE_NO_DIRTY;
    let mut user_cfg: iommu_hwpt_selftest = {};
    struct mock_iommu_domain *mock;
    int rc;
...
#[no_mangle]
unsafe extern "C" fn mock_domain_capable(dev: *mut device, cap: enum iommu_cap) -> bool {
...
#[no_mangle]
unsafe extern "C" fn mock_dev_enable_iopf(dev: *mut device, domain: *mut iommu_domain) -> c_int {
    static int mock_dev_enable_iopf(struct device *dev, struct iommu_domain *domain)
    {
    struct mock_dev *mdev = to_mock_dev(dev);
    int ret;
...
#[no_mangle]
unsafe extern "C" fn mock_dev_disable_iopf(dev: *mut device, domain: *mut iommu_domain) {
    static void mock_dev_disable_iopf(struct device *dev, struct iommu_domain *domain)
    {
    struct mock_dev *mdev = to_mock_dev(dev);
    if (!domain || !domain.iopf_handler)
...
#[no_mangle]
unsafe extern "C" fn mock_viommu_destroy(viommu: *mut iommufd_viommu) {
...
#[no_mangle]
unsafe extern "C" fn mock_hw_queue_destroy(hw_queue: *mut iommufd_hw_queue) {
    ...
    mock_viommu.hw_queue[mock_hw_queue.index] = core::ptr::null_mut();
...
// Test iommufd_hw_queue_depend/undepend()
    ...
    struct mock_hw_queue *mock_hw_queue = to_mock_hw_queue(hw_queue);
    struct mock_hw_queue *prev = core::ptr::null_mut();
    let mut rc: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn mock_domain_free_nested(domain: *mut iommu_domain) {
...
#[no_mangle]
unsafe extern "C" fn mock_dev_release(dev: *mut device) {
    ...
    rc = device_create_managed_software_node(&mdev.dev, prop, core::ptr::null_mut());
    if (rc) {
...
#[no_mangle]
unsafe extern "C" fn mock_dev_destroy(mdev: *mut mock_dev) {
    static void mock_dev_destroy(struct mock_dev *mdev)
    {
    device_unregister(&mdev.dev);
    }
#[no_mangle]
pub unsafe extern "C" fn iommufd_selftest_is_mock_dev(dev: *mut device) -> bool {
...
// Add an additional reserved IOVA to the IOAS
    ...
    rc = iopt_reserve_iova(&ioas.iopt, start, last, core::ptr::null_mut());
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct selftest_access_item {
    pub items_elm: list_head,
    pub iova: c_ulong,
    pub length: usize,
    pub id: c_uint,
}
    ...
    {
    let mut iova_last: c_ulong = iova + length - 1;
...
// Prevent syzkaller from triggering a WARN_ON in kvzalloc()
    ...
    struct iommu_test_cmd *cmd)
    {
    let mut test: iommu_viommu_event_selftest = {};
    struct iommufd_device *idev;
    struct mock_dev *mdev;
    ...
    struct iommu_test_cmd *cmd)
    {
    let mut hwpt_id: u32 = cmd.pasid_check.hwpt_id;
    ...
    if (IS_ERR(handle))
    attached_domain = core::ptr::null_mut();
    else
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_selftest_destroy(obj: *mut iommufd_object) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iommufd_test_dma_buf {
...
#[no_mangle]
unsafe extern "C" fn iommufd_test_dma_buf_release(dmabuf: *mut dma_buf) {
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_test(ucmd: *mut iommufd_ucmd) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_should_fail() -> bool {
    bool iommufd_should_fail(void)
    {
    return should_fail(&fail_iommufd, 1);
    }
#[no_mangle]
pub unsafe extern "C" fn iommufd_test_init() -> int __init {
    ...
    int rc;
    dbgfs_root =
    fault_create_debugfs_attr("fail_iommufd", core::ptr::null_mut(), &fail_iommufd);
    ...
    &selftest_iommu_dev.dev, core::ptr::null_mut(), "%s",
...
#[no_mangle]
unsafe extern "C" fn iommufd_test_wait_for_users() {
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_test_exit() {
    void iommufd_test_exit(void)
    {
    if (mock_iommu_iopf_queue) {
    iopf_queue_free(mock_iommu_iopf_queue);
    mock_iommu_iopf_queue = core::ptr::null_mut();
    }
