...
//! Source: drivers/iommu/iommufd/pages.c
#![no_std]
...
pub const TEMP_MEMORY_LIMIT: c_int = 65536;

pub const BATCH_BACKUP_SIZE: c_int = 32;
//
...
// size = backup_len;
    ...
    {
    let mut last_hole: c_ulong = ULONG_MAX;
    unsigned int i;
...
#[no_mangle]
unsafe extern "C" fn iopt_pages_add_npinned(pages: *mut iopt_pages, npages: usize) {
...
#[no_mangle]
unsafe extern "C" fn iopt_pages_sub_npinned(pages: *mut iopt_pages, npages: usize) {
    ...
    {
    let mut npages: c_ulong = last_index - start_index + 1;
...
//
    ...
    unsigned long last_index)
    {
    let mut start_iova: c_ulong = iopt_area_index_to_iova(area, start_index);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pfn_batch {
    pub pfns: *mut c_ulong,
    pub npfns: *mut u32,
    pub array_size: c_uint,
    pub end: c_uint,
    pub total_pfns: c_uint,
    pub kind: enum batch_kind,
}

    enum { MAX_NPFNS = type_max(typeof(((struct pfn_batch *)0).npfns[0])) };
#[no_mangle]
unsafe extern "C" fn batch_clear(batch: *mut pfn_batch) {
...
//
#[no_mangle]
unsafe extern "C" fn batch_clear_carry(batch: *mut pfn_batch, keep_pfns: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn batch_skip_carry(batch: *mut pfn_batch, skip_pfns: c_uint) {
    ...
    size_t backup_len)
    {
    let mut elmsz: usize = sizeof(*batch.pfns) + sizeof(*batch.npfns);
    let mut size: usize = max_pages * elmsz;
...
#[no_mangle]
unsafe extern "C" fn batch_init(batch: *mut pfn_batch, max_pages: usize) -> c_int {
    static int batch_init(struct pfn_batch *batch, size_t max_pages)
    {
    return __batch_init(batch, max_pages, core::ptr::null_mut(), 0);
    }
...
#[no_mangle]
unsafe extern "C" fn batch_destroy(batch: *mut pfn_batch, backup: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn batch_remove_pfn_num(batch: *mut pfn_batch, nr: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn batch_add_pfn(batch: *mut pfn_batch, pfn: c_ulong) -> bool {
...
// Continues reading a domain until we reach a discontinuity in the pfns.
    ...
    unsigned long last_index)
    {
    let mut array_size: c_uint = batch.array_size;
...
//
    ...
    {
    let mut start_iova: c_ulong = iova;
    int rc;
    if (IS_ENABLED(CONFIG_IOMMUFD_TEST))
    ...
    struct iopt_area *area, unsigned long start_index)
    {
    let mut disable_large_pages: bool = area.iopt.disable_large_pages;
    let mut last_iova: c_ulong = iopt_area_last_iova(area);
    let mut iommu_prot: c_int = area.iommu_prot;
    let mut page_offset: c_uint = 0;
...
// aka xas_destroy()
    ...
    let mut offset: c_ulong = *offset_p;
    while (npages) {
    struct folio *folio = *folios;
    let mut nr: c_ulong = folio_nr_pages(folio) - offset;
    let mut pfn: c_ulong = page_to_pfn(folio_page(folio, offset));
    nr = min(nr, npages);
    npages -= nr;
...
// offset_p = offset;
    ...
    unsigned int flags)
    {
    let mut copied: c_ulong = 0;
    let mut npage: c_uint = 0;
    let mut cur: c_uint = 0;
    while (cur < batch.end) {
    let mut bytes: c_ulong = min(length, PAGE_SIZE - offset);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pfn_reader_user {
    pub upages: *mut page,
    pub upages_len: usize,
    pub upages_start: c_ulong,
    pub upages_end: c_ulong,
    pub gup_flags: c_uint,
//
...
// The following are only valid if file != NULL.
    pub file: *mut file,
    pub ufolios: *mut folio,
    pub ufolios_len: usize,
    pub ufolios_offset: c_ulong,
    pub ufolios_next: *mut folio,
}

    static void pfn_reader_user_init(struct pfn_reader_user *user,
    struct iopt_pages *pages)
    {
    user.upages = core::ptr::null_mut();
    user.upages_len = 0;
    user.upages_start = 0;
    user.upages_end = 0;
    user.locked = -1;
    ...
    user.file = (pages.type == IOPT_ADDRESS_FILE) ? pages.file : core::ptr::null_mut();
    user.ufolios = core::ptr::null_mut();
    user.ufolios_len = 0;
    user.ufolios_next = core::ptr::null_mut();
    ...
    let mut npages_out: c_ulong = 0;
    struct page **upages = user.upages;
    let mut end: c_ulong = start + (npages << PAGE_SHIFT) - 1;
    let mut nfolios: c_long = user.ufolios_len / sizeof(*user.ufolios);
//
...
//
    ...
    let mut npin: c_ulong = min(nr - offset, npages);
    npages -= npin;
    npages_out += npin;
    if (upages) {
    if (npin == 1) {
// upages++ = folio_page(folio, offset);
    ...
    let mut rc: c_int = folio_add_pins(folio, npin - 1);
    if (rc)
    return rc;
    while (npin--)
// upages++ = folio_page(folio, offset++);
    ...
    unsigned long last_index)
    {
    let mut remote_mm: bool = pages.source_mm != current.mm;
    let mut npages: c_ulong = last_index - start_index + 1;
    unsigned long start;
    unsigned long unum;
    uintptr_t uptr;
...
// All undone in pfn_reader_destroy()
    ...
    user.upages = temp_kmalloc(&user.upages_len, core::ptr::null_mut(), 0);
    if (!user.upages)
    return -ENOMEM;
    }
    if (user.file && !user.ufolios) {
    user.ufolios_len = npages * sizeof(*user.ufolios);
    user.ufolios = temp_kmalloc(&user.ufolios_len, core::ptr::null_mut(), 0);
    if (!user.ufolios)
    return -ENOMEM;
    }
    if (user.locked == -1) {
...
#[no_mangle]
unsafe extern "C" fn incr_user_locked_vm(pages: *mut iopt_pages, npages: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn decr_user_locked_vm(pages: *mut iopt_pages, npages: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn update_unpinned(pages: *mut iopt_pages) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pfn_reader_dmabuf {
    pub phys: phys_vec,
    pub start_offset: c_ulong,
}
...
// Callers must not get here if the dmabuf was already revoked
    ...
    unsigned long last_index)
    {
    let mut start: c_ulong = dmabuf.start_offset + start_index * PAGE_SIZE;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pfn_reader {
    pub pages: *mut iopt_pages,
    pub span: interval_tree_double_span_iter,
    pub batch: pfn_batch,
    pub batch_start_index: c_ulong,
    pub batch_end_index: c_ulong,
    pub last_index: c_ulong,
    union {
    pub user: pfn_reader_user,
    pub dmabuf: pfn_reader_dmabuf,
}

    };
#[no_mangle]
unsafe extern "C" fn pfn_reader_update_pinned(pfns: *mut pfn_reader) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pfn_reader_unpin(pfns: *mut pfn_reader) {
    static void pfn_reader_unpin(struct pfn_reader *pfns)
    {
    let mut last: c_ulong = pfns.batch_end_index - 1;
    let mut start: c_ulong = pfns.batch_start_index;
...
#[no_mangle]
unsafe extern "C" fn pfn_reader_fill_span(pfns: *mut pfn_reader) -> c_int {
    static int pfn_reader_fill_span(struct pfn_reader *pfns)
    {
    struct interval_tree_double_span_iter *span = &pfns.span;
    let mut start_index: c_ulong = pfns.batch_end_index;
...
#[no_mangle]
unsafe extern "C" fn pfn_reader_done(pfns: *mut pfn_reader) -> bool {
    static bool pfn_reader_done(struct pfn_reader *pfns)
    {
    return pfns.batch_start_index == pfns.last_index + 1;
    }
#[no_mangle]
unsafe extern "C" fn pfn_reader_next(pfns: *mut pfn_reader) -> c_int {
    ...
    let mut npfns: c_uint = pfns.batch.total_pfns;
    if (IS_ENABLED(CONFIG_IOMMUFD_TEST) &&
...
//
#[no_mangle]
unsafe extern "C" fn pfn_reader_release_pins(pfns: *mut pfn_reader) {
...
// Any pages not transferred to the batch are just unpinned
    let mut npages: c_ulong = user.upages_end - pfns.batch_end_index;
    unsigned long start_index = pfns.batch_end_index -
    user.upages_start;
    if (!user.file) {
    unpin_user_pages(user.upages + start_index, npages);
    } else {
    let mut n: c_long = user.ufolios_len / sizeof(*user.ufolios);
...
#[no_mangle]
unsafe extern "C" fn pfn_reader_destroy(pfns: *mut pfn_reader) {
    ...
    batch_destroy(&pfns.batch, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn iopt_revoke_notify(attach: *mut dma_buf_attachment) {
...
//
    ...
    dma_resv_lock(dmabuf.resv, core::ptr::null_mut());
//
...
#[no_mangle]
pub unsafe extern "C" fn iopt_release_pages(kref: *mut kref) {
...
// unmapped_end_index - batch_last_index - 1);
    ...
    {
    struct interval_tree_double_span_iter span;
    let mut start_index: c_ulong = iopt_area_index(area);
    let mut unmapped_end_index: c_ulong = start_index;
...
//
#[no_mangle]
pub unsafe extern "C" fn iopt_area_unmap_domain(area: *mut iopt_area, domain: *mut iommu_domain) {
...
//
#[no_mangle]
pub unsafe extern "C" fn iopt_area_fill_domain(area: *mut iopt_area, domain: *mut iommu_domain) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn iopt_area_fill_domains(area: *mut iopt_area, pages: *mut iopt_pages) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn iopt_area_unfill_domains(area: *mut iopt_area, pages: *mut iopt_pages) {
    ...
    if (iopt_is_dmabuf(pages))
    iopt_dmabuf_untrack_all_domains(area, pages);
    area.storage_domain = core::ptr::null_mut();
    out_unlock:
    mutex_unlock(&pages.mutex);
    }
...
//
    ...
    u64 backup[BATCH_BACKUP_SIZE];
    struct pfn_batch batch;
    let mut batch_inited: bool = false;
    lockdep_assert_held(&pages.mutex);
    interval_tree_for_each_double_span(&span, &pages.access_itree,
    &pages.domains_itree, start_index,
    last_index) {
...
// (out_pages++) = pfn_to_page(xa_to_value(entry));
    ...
    struct page **out_pages)
    {
    let mut cur_index: c_ulong = start_index;
    int rc;
...
//
    ...
    {
    struct interval_tree_double_span_iter span;
    let mut xa_end: c_ulong = start_index;
    struct pfn_reader_user user;
    int rc;
    lockdep_assert_held(&pages.mutex);
...
//
    ...
    {
    let mut start_index: c_ulong = start_byte / PAGE_SIZE;
    let mut last_index: c_ulong = (start_byte + length - 1) / PAGE_SIZE;
    let mut change_mm: bool = current.mm != pages.source_mm;
    let mut rc: c_int = 0;
    if (IS_ENABLED(CONFIG_IOMMUFD_TEST) &&
