...
//! Source: drivers/iommu/iommufd/vfio_compat.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn iommufd_vfio_compat_ioas_get_id(ictx: *mut iommufd_ctx, out_ioas_id: *mut u32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn iommufd_vfio_compat_set_no_iommu(ictx: *mut iommufd_ctx) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn iommufd_vfio_compat_ioas_create(ictx: *mut iommufd_ctx) -> c_int {
    int iommufd_vfio_compat_ioas_create(struct iommufd_ctx *ictx)
    {
    struct iommufd_ioas *ioas = core::ptr::null_mut();
    int ret;
...
#[no_mangle]
pub unsafe extern "C" fn iommufd_vfio_ioas(ucmd: *mut iommufd_ucmd) -> c_int {
    ...
    xa_lock(&ucmd.ictx.objects);
    ucmd.ictx.vfio_ioas = core::ptr::null_mut();
    xa_unlock(&ucmd.ictx.objects);
    return 0;
    default:
    return -EOPNOTSUPP;
    ...
    void __user *arg)
    {
    let mut supported_flags: u32 = VFIO_DMA_MAP_FLAG_READ | VFIO_DMA_MAP_FLAG_WRITE;
    let mut minsz: usize = offsetofend(struct vfio_iommu_type1_dma_map, size);
...
//
    ...
    void __user *arg)
    {
    let mut minsz: usize = offsetofend(struct vfio_iommu_type1_dma_unmap, size);
//
...
//
    let mut supported_flags: u32 = VFIO_DMA_UNMAP_FLAG_ALL;
...
#[no_mangle]
unsafe extern "C" fn iommufd_vfio_cc_iommu(ictx: *mut iommufd_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn iommufd_vfio_set_iommu(ictx: *mut iommufd_ctx, type: c_ulong) -> c_int {
    static int iommufd_vfio_set_iommu(struct iommufd_ctx *ictx, unsigned long type)
    {
    let mut no_iommu_mode: bool = READ_ONCE(ictx.no_iommu_mode);
    struct iommufd_ioas *ioas = core::ptr::null_mut();
    let mut rc: c_int = 0;
//
...
#[no_mangle]
unsafe extern "C" fn iommufd_get_pagesizes(ioas: *mut iommufd_ioas) -> c_ulong {
    static unsigned long iommufd_get_pagesizes(struct iommufd_ioas *ioas)
    {
    struct io_pagetable *iopt = &ioas.iopt;
    let mut pgsize_bitmap: c_ulong = ULONG_MAX;
...
//
    ...
    let mut minsz: usize = offsetofend(struct vfio_iommu_type1_info, iova_pgsizes);
    struct vfio_info_cap_header __user *last_cap = core::ptr::null_mut();
    ...
    cap_size = fill_fns[i](ioas, core::ptr::null_mut(), 0);
    if (cap_size < 0) {
    rc = cap_size;
    goto out_put;
    }
