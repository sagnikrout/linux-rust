...
//! Source: drivers/iommu/amd/nested.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn validate_gdte_nested(gdte: *mut iommu_hwpt_amd_guest) -> c_int {
    static int validate_gdte_nested(struct iommu_hwpt_amd_guest *gdte)
    {
    let mut gpt_level: u32 = FIELD_GET(DTE_GPT_LEVEL_MASK, gdte.dte[2]);
...
//
    ...
    res = __xa_cmpxchg(xa, index, core::ptr::null_mut(), elm, GFP_ATOMIC);
    if (xa_is_err(res))
...
// The gDomID does not exist. We allocate new hdom_id
    ...
    ndom.gdom_id, gdom_info, core::ptr::null_mut(), GFP_ATOMIC);
    xa_unlock_irqrestore(&aviommu.gdomid_array, irqflags);
    ret = -ENOSPC;
...
#[no_mangle]
unsafe extern "C" fn nested_domain_free(dom: *mut iommu_domain) {
...
//
    ...
    ndom.gdom_info, core::ptr::null_mut(), GFP_ATOMIC);
    xa_unlock_irqrestore(&aviommu.gdomid_array, irqflags);
