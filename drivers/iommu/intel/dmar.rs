...
//! Source: drivers/iommu/intel/dmar.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dmar_res_callback {
    pub cb: [dmar_res_handler_t; ACPI_DMAR_TYPE_RESERVED],
    pub arg: [*mut c_void; ACPI_DMAR_TYPE_RESERVED],
...
//
    ...
    let mut dmar_dev_scope_status: static int = 1;
    static DEFINE_IDA(dmar_seq_ids);
    static int alloc_iommu(struct dmar_drhd_unit *drhd);
    static void free_iommu(struct intel_iommu *iommu);
#[no_mangle]
unsafe extern "C" fn dmar_register_drhd_unit(drhd: *mut dmar_drhd_unit) {
...
#[no_mangle]
pub unsafe extern "C" fn dmar_free_dev_scope(devices: *mut dmar_dev_scope, cnt: *mut c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn dmar_free_pci_notify_info(info: *mut dmar_pci_notify_info) {
...
//
    ...
    if (tmp == &info.dev.dev) {
    RCU_INIT_POINTER(devices[index].dev, core::ptr::null_mut());
    synchronize_rcu();
    put_device(tmp);
    return 1;
    }
...
#[no_mangle]
unsafe extern "C" fn dmar_pci_bus_add_dev(info: *mut dmar_pci_notify_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmar_pci_bus_del_dev(info: *mut dmar_pci_notify_info) {
...
#[no_mangle]
pub unsafe extern "C" fn vf_inherit_msi_domain(pdev: *mut pci_dev) {
...
#[no_mangle]
pub unsafe extern "C" fn if(BUS_NOTIFY_REMOVED_DEVICE: action ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn dmar_parse_one_drhd(header: *mut acpi_dmar_header, arg: *mut c_void) -> c_int {
...
// The size of the register set is 2 ^ N 4 KB pages.
    ...
    ((void *)drhd) + drhd.header.length,
    &dmaru.devices_cnt);
    if (dmaru.devices_cnt && dmaru.devices == core::ptr::null_mut()) {
    kfree(dmaru);
    return -ENOMEM;
    }
...
#[no_mangle]
unsafe extern "C" fn dmar_free_drhd(dmaru: *mut dmar_drhd_unit) {
...
#[no_mangle]
unsafe extern "C" fn dmar_parse_one_rhsa(header: *mut acpi_dmar_header, arg: *mut c_void) -> c_int {
    ...
    let mut node: c_int = pxm_to_node(rhsa.proximity_domain);
...
//
#[no_mangle]
unsafe extern "C" fn dmar_table_detect() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn dmar_acpi_dev_scope_init() -> int __init {
    static int __init dmar_acpi_dev_scope_init(void)
    {
    struct acpi_dmar_andd *andd;
    if (dmar_tbl == core::ptr::null_mut())
    return -ENODEV;
...
#[no_mangle]
pub unsafe extern "C" fn dmar_dev_scope_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn dmar_register_bus_notifier() -> void __init {
    void __init dmar_register_bus_notifier(void)
    {
    bus_register_notifier(&pci_bus_type, &dmar_pci_bus_nb);
    }
#[no_mangle]
pub unsafe extern "C" fn dmar_table_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn warn_invalid_dmar(addr: u64, message: *const c_char) {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmar_can_force_on(force_on: enum dmar_force_on) -> bool {
...
#[no_mangle]
unsafe extern "C" fn dmar_required() -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn detect_intel_iommu() -> void __init {
...
// Make sure ACS will be enabled
    ...
    dmar_tbl = core::ptr::null_mut();
    }
    up_write(&dmar_global_lock);
    }
#[no_mangle]
unsafe extern "C" fn unmap_iommu(iommu: *mut intel_iommu) {
...
//
#[no_mangle]
unsafe extern "C" fn map_iommu(iommu: *mut intel_iommu, drhd: *mut dmar_drhd_unit) -> c_int {
    static int map_iommu(struct intel_iommu *iommu, struct dmar_drhd_unit *drhd)
    {
    let mut phys_addr: u64 = drhd.reg_base_addr;
    int map_size, err=0;
...
#[no_mangle]
unsafe extern "C" fn alloc_iommu(drhd: *mut dmar_drhd_unit) -> c_int {
    static int alloc_iommu(struct dmar_drhd_unit *drhd)
    {
    struct intel_iommu *iommu;
    u32 ver, sts;
    let mut agaw: c_int = -1;
    let mut msagaw: c_int = -1;
    int err;
...
//
    ...
    err = iommu_device_sysfs_add(&iommu.iommu, core::ptr::null_mut(),
    intel_iommu_groups,
    "%s", iommu.name);
    if (err)
    goto err_unmap;
    err = iommu_device_register(&iommu.iommu, &intel_iommu_ops, core::ptr::null_mut());
    if (err)
    goto err_sysfs;
...
#[no_mangle]
unsafe extern "C" fn free_iommu(iommu: *mut intel_iommu) {
    static void free_iommu(struct intel_iommu *iommu)
    {
...
//
#[no_mangle]
pub unsafe extern "C" fn reclaim_free_desc(qi: *mut q_inval) {
...
#[no_mangle]
unsafe extern "C" fn qi_dump_fault(iommu: *mut intel_iommu, fault: u32) {
    static void qi_dump_fault(struct intel_iommu *iommu, u32 fault)
    {
    let mut head: c_uint = readl(iommu.reg + DMAR_IQH_REG);
    let mut iqe_err: u64 = readq(iommu.reg + DMAR_IQER_REG);
...
#[no_mangle]
unsafe extern "C" fn qi_check_fault(iommu: *mut intel_iommu, index: c_int, wait_index: c_int) -> c_int {
    ...
    let mut shift: c_int = qi_shift(iommu);
...
//
    ...
    {
    struct q_inval *qi = iommu.qi;
    let mut devtlb_start_ktime: i64 = 0;
    let mut iotlb_start_ktime: i64 = 0;
    let mut iec_start_ktime: i64 = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn qi_global_iec(iommu: *mut intel_iommu) {
...
#[no_mangle]
pub unsafe extern "C" fn qi_flush_piotlb_all(iommu: *mut intel_iommu, did: u16, pasid: u32) {
...
// PASID-based device IOTLB Invalidate
    ...
    {
    let mut desc: qi_desc = {.qw1 = 0, .qw2 = 0, .qw3 = 0};
//
// VT-d spec, section 4.3:
//
// Software is recommended to not submit any Device-TLB invalidation
...
//
#[no_mangle]
pub unsafe extern "C" fn dmar_disable_qi(iommu: *mut intel_iommu) {
    void dmar_disable_qi(struct intel_iommu *iommu)
    {
    unsigned long flags;
    u32 sts;
    let mut start_time: cycles_t = get_cycles();
...
//
#[no_mangle]
unsafe extern "C" fn __dmar_enable_qi(iommu: *mut intel_iommu) {
    static void __dmar_enable_qi(struct intel_iommu *iommu)
    {
    u32 sts;
    unsigned long flags;
    struct q_inval *qi = iommu.qi;
    let mut val: u64 = virt_to_phys(qi.desc);
...
//
#[no_mangle]
pub unsafe extern "C" fn dmar_enable_qi(iommu: *mut intel_iommu) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dmar_msi_reg(iommu: *mut intel_iommu, irq: c_int) -> c_int {
    static inline int dmar_msi_reg(struct intel_iommu *iommu, int irq)
    {
    if (iommu.irq == irq)
    return DMAR_FECTL_REG;
#[no_mangle]
pub unsafe extern "C" fn if(irq: iommu->pr_irq ==) -> else {
    else if (iommu.pr_irq == irq)
    return DMAR_PECTL_REG;
#[no_mangle]
pub unsafe extern "C" fn if(irq: iommu->perf_irq ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn dmar_msi_unmask(data: *mut irq_data) {
    void dmar_msi_unmask(struct irq_data *data)
    {
    struct intel_iommu *iommu = irq_data_get_irq_handler_data(data);
    let mut reg: c_int = dmar_msi_reg(iommu, data.irq);
    unsigned long flag;
// unmask it
...
#[no_mangle]
pub unsafe extern "C" fn dmar_msi_mask(data: *mut irq_data) {
...
#[no_mangle]
pub unsafe extern "C" fn dmar_msi_write(irq: c_int, msg: *mut msi_msg) {
    void dmar_msi_write(int irq, struct msi_msg *msg)
    {
    struct intel_iommu *iommu = irq_get_handler_data(irq);
    let mut reg: c_int = dmar_msi_reg(iommu, irq);
    unsigned long flag;
    raw_spin_lock_irqsave(&iommu.register_lock, flag);
...
#[no_mangle]
pub unsafe extern "C" fn dmar_fault(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
// Disable printing, simply clear the fault when ratelimited
    let mut ratelimited: bool = !__ratelimit(&rs);
    u8 fault_reason;
    u16 source_id;
    u64 guest_addr;
    u32 pasid;
...
#[no_mangle]
pub unsafe extern "C" fn dmar_set_interrupt(iommu: *mut intel_iommu) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn enable_drhd_fault_handling(cpu: c_uint) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmar_reenable_qi(iommu: *mut intel_iommu) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmar_ir_support() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn dmar_in_use() -> bool {
    static inline bool dmar_in_use(void)
    {
    return irq_remapping_enabled || intel_iommu_enabled;
    }
#[no_mangle]
unsafe extern "C" fn dmar_free_unused_resources() -> int __init {
...
// so use 0 for safety.
//
pub const DMAR_DSM_REV_ID: c_int = 0;
pub const DMAR_DSM_FUNC_DRHD: c_int = 1;
pub const DMAR_DSM_FUNC_ATSR: c_int = 2;
pub const DMAR_DSM_FUNC_RHSA: c_int = 3;
pub const DMAR_DSM_FUNC_SATC: c_int = 4;
#[no_mangle]
pub unsafe extern "C" fn dmar_detect_dsm(handle: acpi_handle, func: c_int) -> bool {
    ...
    func, core::ptr::null_mut(), ACPI_TYPE_BUFFER);
    if (!obj)
    return -ENODEV;
...
#[no_mangle]
unsafe extern "C" fn dmar_hp_add_drhd(header: *mut acpi_dmar_header, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmar_hp_remove_drhd(header: *mut acpi_dmar_header, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmar_hp_release_drhd(header: *mut acpi_dmar_header, arg: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dmar_hotplug_insert(handle: acpi_handle) -> c_int {
    ...
    &dmar_parse_one_rhsa, core::ptr::null_mut());
    if (ret)
    goto release_drhd;
    ret = dmar_walk_dsm_resource(handle, DMAR_DSM_FUNC_ATSR,
    &dmar_parse_one_atsr, core::ptr::null_mut());
    if (ret)
    goto release_atsr;
    ret = dmar_walk_dsm_resource(handle, DMAR_DSM_FUNC_DRHD,
    &dmar_hp_add_drhd, core::ptr::null_mut());
    if (!ret)
    return 0;
    dmar_walk_dsm_resource(handle, DMAR_DSM_FUNC_DRHD,
    &dmar_hp_remove_drhd, core::ptr::null_mut());
    release_atsr:
    dmar_walk_dsm_resource(handle, DMAR_DSM_FUNC_ATSR,
    &dmar_release_one_atsr, core::ptr::null_mut());
    release_drhd:
    dmar_walk_dsm_resource(handle, DMAR_DSM_FUNC_DRHD,
    &dmar_hp_release_drhd, core::ptr::null_mut());
    out:
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn dmar_hotplug_remove(handle: acpi_handle) -> c_int {
    static int dmar_hotplug_remove(acpi_handle handle)
    {
    int ret;
    ret = dmar_walk_dsm_resource(handle, DMAR_DSM_FUNC_ATSR,
    &dmar_check_one_atsr, core::ptr::null_mut());
    if (ret)
    return ret;
    ret = dmar_walk_dsm_resource(handle, DMAR_DSM_FUNC_DRHD,
    &dmar_hp_remove_drhd, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn dmar_device_hotplug(handle: acpi_handle, insert: bool) -> c_int {
    static int dmar_device_hotplug(acpi_handle handle, bool insert)
    {
    int ret;
    let mut tmp: acpi_handle = core::ptr::null_mut();
    acpi_status status;
    if (!dmar_in_use())
    return 0;
    if (dmar_detect_dsm(handle, DMAR_DSM_FUNC_DRHD)) {
    ...
    ACPI_UINT32_MAX,
    dmar_get_dsm_handle,
    core::ptr::null_mut(), core::ptr::null_mut(), &tmp);
...
#[no_mangle]
pub unsafe extern "C" fn dmar_device_add(handle: acpi_handle) -> c_int {
    int dmar_device_add(acpi_handle handle)
    {
    return dmar_device_hotplug(handle, true);
    }
#[no_mangle]
pub unsafe extern "C" fn dmar_device_remove(handle: acpi_handle) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn dmar_platform_optin() -> bool {
