...
//! Source: drivers/iommu/io-pgtable-arm-v7s.c
#![no_std]
...
//
pub const ARM_V7S_ADDR_BITS: c_int = 32;

pub const ARM_V7S_TABLE_SHIFT: c_int = 10;
...
//
pub const ARM_V7S_CONT_PAGES: c_int = 16;
// PTE type bits: these are all mixed up with XN/PXN bits in most cases
pub const ARM_V7S_PTE_TYPE_TABLE: c_uint = 0x1;
pub const ARM_V7S_PTE_TYPE_PAGE: c_uint = 0x2;
pub const ARM_V7S_PTE_TYPE_CONT_PAGE: c_uint = 0x1;

    ((lvl) == 1 && (((pte) & 0x3) == ARM_V7S_PTE_TYPE_TABLE))
// Page table bits

pub const ARM_V7S_CONT_PAGE_XN_SHIFT: c_int = 15;
//
...
pub const ARM_V7S_ATTR_MASK: c_uint = 0xff;

pub const ARM_V7S_TEX_SHIFT: c_int = 2;
pub const ARM_V7S_TEX_MASK: c_uint = 0x7;

// MediaTek extend the bits below for PA 32bit/33bit/34bit

// *well, except for TEX on level 2 large pages, of course :(
pub const ARM_V7S_CONT_PAGE_TEX_SHIFT: c_int = 6;

// Simplified access permissions

// Register bits
pub const ARM_V7S_RGN_NC: c_int = 0;
pub const ARM_V7S_RGN_WBWA: c_int = 1;
pub const ARM_V7S_RGN_WT: c_int = 2;
pub const ARM_V7S_RGN_WB: c_int = 3;
pub const ARM_V7S_PRRR_TYPE_DEVICE: c_int = 1;
pub const ARM_V7S_PRRR_TYPE_NORMAL: c_int = 2;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct arm_v7s_io_pgtable {
    pub iop: io_pgtable,
    pub pgd: *mut arm_v7s_iopte,
    pub l2_tables: *mut kmem_cache,
}

    static bool arm_v7s_pte_is_cont(arm_v7s_iopte pte, int lvl);
#[no_mangle]
unsafe extern "C" fn __arm_v7s_dma_addr(pages: *mut c_void) -> dma_addr_t {
    static dma_addr_t __arm_v7s_dma_addr(void *pages)
    {
    return (dma_addr_t)virt_to_phys(pages);
    }
#[no_mangle]
unsafe extern "C" fn arm_v7s_is_mtk_enabled(cfg: *mut io_pgtable_cfg) -> bool {
...
#[no_mangle]
unsafe extern "C" fn to_mtk_iopte(paddr: phys_addr_t, pte: arm_v7s_iopte) -> arm_v7s_iopte {
    ...
    {
    let mut pte: arm_v7s_iopte = paddr & ARM_V7S_LVL_MASK(lvl);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: arm_v7s_pte_is_cont(pte, _arg: lvl)) -> else {
    ...
    phys_addr_t phys;
    dma_addr_t dma;
    let mut size: usize = ARM_V7S_TABLE_SIZE(lvl, cfg);
    void *table = core::ptr::null_mut();
    gfp_t gfp_l1;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(2: lvl ==) -> else {
    else if (lvl == 2)
...
//
    ...
    struct io_pgtable_cfg *cfg)
    {
    let mut ap: bool = !(cfg.quirks & IO_PGTABLE_QUIRK_NO_PERMS);
    let mut pte: arm_v7s_iopte = ARM_V7S_ATTR_NG | ARM_V7S_ATTR_S;
    if (!(prot & IOMMU_MMIO))
    ...
    pte |= ARM_V7S_ATTR_B;
#[no_mangle]
pub unsafe extern "C" fn if(IOMMU_CACHE: prot &) -> else {
...
#[no_mangle]
unsafe extern "C" fn arm_v7s_pte_to_cont(pte: arm_v7s_iopte, lvl: c_int) -> arm_v7s_iopte {
    static arm_v7s_iopte arm_v7s_pte_to_cont(arm_v7s_iopte pte, int lvl)
    {
    if (lvl == 1) {
    pte |= ARM_V7S_CONT_SECTION;
    } else if (lvl == 2) {
    let mut xn: arm_v7s_iopte = pte & ARM_V7S_ATTR_XN(lvl);
    let mut tex: arm_v7s_iopte = pte & ARM_V7S_CONT_PAGE_TEX_MASK;
...
#[no_mangle]
unsafe extern "C" fn arm_v7s_pte_is_cont(pte: arm_v7s_iopte, lvl: c_int) -> bool {
    static bool arm_v7s_pte_is_cont(arm_v7s_iopte pte, int lvl)
    {
...
//
    arm_v7s_iopte *tblp;
    let mut sz: usize = ARM_V7S_BLOCK_SIZE(lvl);
    tblp = ptep - ARM_V7S_LVL_IDX(iova, lvl, cfg);
    if (WARN_ON(__arm_v7s_unmap(data, core::ptr::null_mut(), iova + i * sz,
    sz, lvl, tblp) != sz))
    return -EINVAL;
    } else if (ptep[i]) {
// We require an unmap first
    ...
    struct io_pgtable_cfg *cfg)
    {
    let mut phys: phys_addr_t = virt_to_phys(table);
...
//
    ...
    let mut num_entries: c_int = size >> ARM_V7S_LVL_SHIFT(lvl);
...
#[no_mangle]
unsafe extern "C" fn arm_v7s_free_pgtable(iop: *mut io_pgtable) {
    static void arm_v7s_free_pgtable(struct io_pgtable *iop)
    {
    struct arm_v7s_io_pgtable *data = io_pgtable_to_data(iop);
    int i;
    for (i = 0; i < ARM_V7S_PTES_PER_LVL(1, &data.iop.cfg); i++) {
    let mut pte: arm_v7s_iopte = data.pgd[i];
...
// If the size matches this level, we're in the right place
    if (num_entries) {
    let mut blk_size: usize = ARM_V7S_BLOCK_SIZE(lvl);
...
//
#[no_mangle]
unsafe extern "C" fn dummy_tlb_flush_all(cookie: *mut c_void) -> __noipa void __init {
...
#[no_mangle]
unsafe extern "C" fn arm_v7s_do_selftests() -> int __init {
...
// Full unmap
    ...
    for_each_set_bit(i, &cfg.pgsize_bitmap, BITS_PER_LONG) {
    size = 1UL << i;
    if (ops.unmap_pages(ops, iova, size, 1, core::ptr::null_mut()) != size)
    return __FAIL(ops);
