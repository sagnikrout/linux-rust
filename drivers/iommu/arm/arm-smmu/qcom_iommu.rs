...
//! Source: drivers/iommu/arm/arm-smmu/qcom_iommu.c
#![no_std]
...
pub const SMMU_INTR_SEL_NS: c_uint = 0x2000;
    enum qcom_iommu_clk {
    CLK_IFACE,
    CLK_BUS,
    CLK_TBU,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qcom_iommu_dev {
// IOMMU core code handle
    pub iommu: iommu_device,
    pub dev: *mut device,
    pub clks: [clk_bulk_data; CLK_NUM],
    pub local_base: *mut void __iomem,
    pub sec_id: u32,
    pub max_asid: u8,
    pub /: *mut *mut *mut qcom_iommu_ctx ctxs[]; / indexed by asid,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct qcom_iommu_ctx {
    pub dev: *mut device,
    pub base: *mut void __iomem,
    pub secure_init: bool,
    pub secured_ctx: bool,
    pub /: *mut *mut u8 asid; / asid and ctx bank # are 1:1,
    pub domain: *mut iommu_domain,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct qcom_iommu_domain {
    pub pgtbl_ops: *mut io_pgtable_ops,
    pub pgtbl_lock: spinlock_t,
    pub /: *mut *mut mutex init_mutex; / Protects iommu pointer,
    pub domain: iommu_domain,
    pub iommu: *mut qcom_iommu_dev,
...
#[no_mangle]
unsafe extern "C" fn to_ctx(d: *mut qcom_iommu_domain, asid: unsigned) -> *mut qcom_iommu_ctx {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_tlb_sync(cookie: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_tlb_inv_context(cookie: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_fault(irq: c_int, dev: *mut c_void) -> irqreturn_t {
...
// Publish page table ops for map/unmap
    ...
    qcom_domain.iommu = core::ptr::null_mut();
    out_unlock:
    mutex_unlock(&qcom_domain.init_mutex);
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_domain_free(domain: *mut iommu_domain) {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_flush_iotlb_all(domain: *mut iommu_domain) {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_capable(dev: *mut device, cap: enum iommu_cap) -> bool {
...
//
    ...
    let mut asid: unsigned = args.args[0];
...
//
    ...
    WARN_ON(qcom_iommu.ctxs[asid] == core::ptr::null_mut()))
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_sec_ptbl_init(dev: *mut device) -> c_int {
    static int qcom_iommu_sec_ptbl_init(struct device *dev)
    {
    let mut psize: usize = 0;
    let mut spare: c_uint = 0;
    void *cpu_addr;
    dma_addr_t paddr;
    unsigned long attrs;
    let mut allocated: static bool = false;
    int ret;
    if (allocated)
    return 0;
...
#[no_mangle]
unsafe extern "C" fn get_asid(np: *const device_node) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_ctx_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_ctx_remove(pdev: *mut platform_device) {
    ...
    qcom_iommu.ctxs[ctx.asid] = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_has_secure_context(qcom_iommu: *mut qcom_iommu_dev) -> bool {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_device_probe(pdev: *mut platform_device) -> c_int {
...
// register context bank devices, which are child nodes:
    ...
    ret = iommu_device_sysfs_add(&qcom_iommu.iommu, dev, core::ptr::null_mut(),
    dev_name(dev));
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_device_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_resume(dev: *mut device) -> int __maybe_unused {
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_suspend(dev: *mut device) -> int __maybe_unused {
    ...
    SET_RUNTIME_PM_OPS(qcom_iommu_suspend, qcom_iommu_resume, core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn qcom_iommu_init() -> int __init {
