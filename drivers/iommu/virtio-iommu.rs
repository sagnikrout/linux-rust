...
//! Source: drivers/iommu/virtio-iommu.c
#![no_std]
...
pub const VIOMMU_REQUEST_VQ: c_int = 0;
pub const VIOMMU_EVENT_VQ: c_int = 1;
pub const VIOMMU_NR_VQS: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct viommu_dev {
    pub iommu: iommu_device,
    pub dev: *mut device,
    pub vdev: *mut virtio_device,
    pub domain_ids: ida,
    pub vqs: [*mut virtqueue; VIOMMU_NR_VQS],
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct viommu_mapping {
    pub paddr: phys_addr_t,
    pub iova: interval_tree_node,
    pub flags: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct viommu_domain {
    pub domain: iommu_domain,
    pub viommu: *mut viommu_dev,
    pub id: c_uint,
    pub map_flags: u32,
    pub mappings_lock: spinlock_t,
    pub mappings: rb_root_cached,
    pub nr_endpoints: c_ulong,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct viommu_endpoint {
    pub dev: *mut device,
    pub viommu: *mut viommu_dev,
    pub vdomain: *mut viommu_domain,
    pub resv_regions: list_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct viommu_request {
    pub list: list_head,
    pub writeback: *mut c_void,
    pub write_offset: c_uint,
    pub len: c_uint,
    pub __counted_by(len): char buf[],
}

pub const VIOMMU_FAULT_RESV_MASK: c_uint = 0xffffff00;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct viommu_event {
    union {
    pub head: u32,
    pub fault: virtio_iommu_fault,
}
...
#[no_mangle]
unsafe extern "C" fn viommu_get_req_errno(buf: *mut c_void, len: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn viommu_set_req_status(buf: *mut c_void, len: usize, status: c_int) {
    ...
    size_t len)
    {
    let mut tail_size: usize = sizeof(struct virtio_iommu_req_tail);
...
//
#[no_mangle]
unsafe extern "C" fn __viommu_sync_req(viommu: *mut viommu_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn viommu_sync_req(viommu: *mut viommu_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn viommu_add_req(viommu: *mut viommu_dev, buf: *mut c_void, len: usize) -> c_int {
...
//
    ...
    unsigned long flags;
    struct viommu_mapping *mapping = core::ptr::null_mut();
...
//
    ...
    let mut iova: u64 = vdomain.domain.geometry.aperture_start;
    let mut limit: u64 = vdomain.domain.geometry.aperture_end;
    let mut flags: u32 = VIRTIO_IOMMU_MAP_F_READ | VIRTIO_IOMMU_MAP_F_WRITE;
    let mut granule: c_ulong = 1UL << __ffs(vdomain.domain.pgsize_bitmap);
    iova = ALIGN(iova, granule);
    limit = ALIGN_DOWN(limit + 1, granule) - 1;
    list_for_each_entry(resv, &vdev.resv_regions, list) {
    let mut resv_start: u64 = ALIGN_DOWN(resv.start, granule);
    let mut resv_end: u64 = ALIGN(resv.start + resv.length, granule) - 1;
...
//
#[no_mangle]
unsafe extern "C" fn viommu_replay_mappings(vdomain: *mut viommu_domain) -> c_int {
    ...
    struct iommu_resv_region *region = core::ptr::null_mut(), *next;
    let mut prot: c_ulong = IOMMU_WRITE | IOMMU_NOEXEC | IOMMU_MMIO;
...
#[no_mangle]
unsafe extern "C" fn viommu_probe_endpoint(viommu: *mut viommu_dev, dev: *mut device) -> c_int {
...
//
    ...
    {
    char *reason_str;
    let mut reason: u8 = fault.reason;
    let mut flags: u32 = le32_to_cpu(fault.flags);
    let mut endpoint: u32 = le32_to_cpu(fault.endpoint);
    let mut address: u64 = le64_to_cpu(fault.address);
    switch (reason) {
...
#[no_mangle]
unsafe extern "C" fn viommu_event_handler(vq: *mut virtqueue) {
    ...
    while ((evt = virtqueue_get_buf(vq, &len)) != core::ptr::null_mut()) {
    if (len > sizeof(*evt)) {
    dev_err(viommu.dev,
...
#[no_mangle]
unsafe extern "C" fn viommu_domain_free(domain: *mut iommu_domain) {
...
#[no_mangle]
unsafe extern "C" fn viommu_detach_dev(vdev: *mut viommu_endpoint) {
    ...
    vdomain.nr_endpoints--;
    vdev.vdomain = core::ptr::null_mut();
    }
    ...
    int ret;
    u32 flags;
    let mut size: usize = pgsize * pgcount;
    let mut end: u64 = iova + size - 1;
    struct virtio_iommu_req_map map;
    struct viommu_domain *vdomain = to_viommu_domain(domain);
...
#[no_mangle]
unsafe extern "C" fn viommu_flush_iotlb_all(domain: *mut iommu_domain) {
...
#[no_mangle]
unsafe extern "C" fn viommu_get_resv_regions(dev: *mut device, head: *mut list_head) {
    static void viommu_get_resv_regions(struct device *dev, struct list_head *head)
    {
    struct iommu_resv_region *entry, *new_entry, *msi = core::ptr::null_mut();
    struct viommu_endpoint *vdev = dev_iommu_priv_get(dev);
...
#[no_mangle]
unsafe extern "C" fn viommu_match_node(dev: *mut device, data: *const c_void) -> c_int {
    ...
    {
    struct device *dev = bus_find_device(virtio_bus_type, core::ptr::null_mut(), fwnode,
    viommu_match_node);
    put_device(dev);
    return dev ? dev_to_virtio(dev).priv : core::ptr::null_mut();
    }
    static struct iommu_device *viommu_probe_device(struct device *dev)
    {
    int ret;
    struct viommu_endpoint *vdev;
    struct viommu_dev *viommu = core::ptr::null_mut();
    struct iommu_fwspec *fwspec = dev_iommu_fwspec_get(dev);
...
#[no_mangle]
unsafe extern "C" fn viommu_release_device(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn viommu_capable(dev: *mut device, cap: enum iommu_cap) -> bool {
...
#[no_mangle]
unsafe extern "C" fn viommu_init_vqs(viommu: *mut viommu_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn viommu_fill_evtq(viommu: *mut viommu_dev) -> c_int {
    ...
    let mut nr_evts: usize = vq.num_free;
...
#[no_mangle]
unsafe extern "C" fn viommu_probe(vdev: *mut virtio_device) -> c_int {
    static int viommu_probe(struct virtio_device *vdev)
    {
    struct device *parent_dev = vdev.dev.parent;
    struct viommu_dev *viommu = core::ptr::null_mut();
    struct device *dev = &vdev.dev;
    let mut input_start: u64 = 0;
    let mut input_end: u64 = -1UL;
    int ret;
...
// Populate the event queue with buffers
    ...
    if (ret)
    goto err_free_vqs;
    ret = iommu_device_sysfs_add(&viommu.iommu, dev, core::ptr::null_mut(), "%s",
...
#[no_mangle]
unsafe extern "C" fn viommu_remove(vdev: *mut virtio_device) {
...
#[no_mangle]
unsafe extern "C" fn viommu_config_changed(vdev: *mut virtio_device) {
