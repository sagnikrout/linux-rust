...
//! Source: drivers/iommu/of_iommu.c
#![no_std]
...
//
    ...
    {
    struct of_phandle_args iommu_spec;
    let mut err: c_int = -ENODEV, idx = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct of_pci_iommu_alias_info {
    pub dev: *mut device,
    pub np: *mut device_node,
}

#[no_mangle]
unsafe extern "C" fn of_pci_iommu_init(pdev: *mut pci_dev, alias: u16, data: *mut c_void) -> c_int {
    static int of_pci_iommu_init(struct pci_dev *pdev, u16 alias, void *data)
    {
    struct of_pci_iommu_alias_info *info = data;
    let mut input_id: u32 = alias;
...
#[no_mangle]
unsafe extern "C" fn of_pci_check_device_ats(dev: *mut device, np: *mut device_node) {
...
#[no_mangle]
pub unsafe extern "C" fn if(dev->iommu: err &&) -> else {
...
//
    ...
    {
    let mut end: phys_addr_t = start + length - 1;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn of_iommu_get_resv_regions(dev: *mut device, list: *mut list_head) {
    void of_iommu_get_resv_regions(struct device *dev, struct list_head *list)
    {

    struct of_phandle_iterator it;
    int err;
    of_for_each_phandle(&it, err, dev.of_node, "memory-region", core::ptr::null_mut(), 0) {
