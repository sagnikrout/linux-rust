...
//! Source: drivers/counter/microchip-tcb-capture.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mchp_tc_data {
    pub tc_cfg: *const atmel_tcb_config,
    ...
    {
    let mut priv: *mut mchp_tc_data const = counter_priv(counter);
    if (priv.qdec_mode)
...
// action = COUNTER_SYNAPSE_ACTION_BOTH_EDGES;
    ...
    {
    let mut priv: *mut mchp_tc_data const = counter_priv(counter);
    let mut edge: u32 = ATMEL_TC_ETRGEDG_NONE;
...
#[no_mangle]
unsafe extern "C" fn mchp_tc_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
    static irqreturn_t mchp_tc_isr(int irq, void *dev_id)
    {
    let mut counter: *mut counter_device const = dev_id;
    let mut priv: *mut mchp_tc_data const = counter_priv(counter);
    u32 sr, mask;
    regmap_read(priv.regmap, ATMEL_TC_REG(priv.channel[0], SR), &sr);
...
#[no_mangle]
unsafe extern "C" fn mchp_tc_irq_remove(ptr: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn mchp_tc_irq_enable(counter: *const *const counter_device, irq: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mchp_tc_clk_remove(ptr: *mut c_void) {
    static void mchp_tc_clk_remove(void *ptr)
    {
    clk_disable_unprepare((struct clk *)ptr);
    }
#[no_mangle]
unsafe extern "C" fn mchp_tc_probe(pdev: *mut platform_device) -> c_int {
