...
//! Source: drivers/regulator/of_regulator.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn fill_limit(limit: *mut c_int, val: c_int) {
...
// Protection limits:
    ...
    constraints.name = of_get_property(np, "regulator-name", core::ptr::null_mut());
...
// Current change possible?
    ...
    of_node_put(suspend_np);
    suspend_state = core::ptr::null_mut();
    suspend_np = core::ptr::null_mut();
    }
    return 0;
    }
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct devm_of_regulator_matches {
    pub matches: *mut of_regulator_match,
    pub num_matches: c_uint,
}

#[no_mangle]
unsafe extern "C" fn devm_of_regulator_put_matches(dev: *mut device, res: *mut c_void) {
...
//
    ...
    match.init_data = core::ptr::null_mut();
    match.of_node = core::ptr::null_mut();
    }
    for_each_child_of_node(node, child) {
    name = of_get_property(child,
    "regulator-compatible", core::ptr::null_mut());
    if (!name)
    name = child.name;
    for (i = 0; i < num_matches; i++) {
    struct of_regulator_match *match = &matches[i];
    ...
    name = of_get_property(child, "regulator-compatible", core::ptr::null_mut());
    if (!name) {
...
//
    ...
    {
    struct device_node *regnode = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn of_get_n_coupled(rdev: *mut regulator_dev) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn of_check_coupling_data(rdev: *mut regulator_dev) -> bool {
    bool of_check_coupling_data(struct regulator_dev *rdev)
    {
    struct device_node *node = rdev.dev.of_node;
    let mut n_phandles: c_int = of_get_n_coupled(rdev);
...
// iterate over rdev's phandles
    for (i = 0; i < n_phandles; i++) {
    let mut max_spread: c_int = rdev.constraints.max_spread[i];
...
//
#[no_mangle]
unsafe extern "C" fn is_supply_name(name: *const c_char) -> c_int {
...
//
    ...
    {
    let mut num_consumers: c_int = 0;
    struct regulator *tmp;
    struct regulator_bulk_data *_consumers = core::ptr::null_mut();
