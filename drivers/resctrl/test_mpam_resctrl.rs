...
//! Source: drivers/resctrl/test_mpam_resctrl.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct percent_value_test_info {
    pub /: *mut *mut u32 pc; / result of value-to-percent conversion,
    pub /: *mut *mut u32 value; / result of percent-to-value conversion,
    pub /: *mut *mut u32 max_value; / maximum raw value allowed by test params,
    pub /: *mut *mut unsigned int shift; / promotes raw testcase value to 16 bits,
}
...
#[no_mangle]
unsafe extern "C" fn test_get_mba_granularity(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_mbw_max_to_percent(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_percent_to_mbw_max(test: *mut kunit) {
...
// No flexibility allowed for 0% and 100%!
    ...
    char *desc)
    {
    let mut param: uintptr_t = (uintptr_t)prev;
    if (param > 15)
...
#[no_mangle]
unsafe extern "C" fn test_get_bwa_wd(test: *mut kunit) -> c_uint {
    static unsigned int test_get_bwa_wd(struct kunit *test)
    {
    let mut param: uintptr_t = (uintptr_t)test.param_value;
    KUNIT_ASSERT_GE(test, param, 1);
    KUNIT_ASSERT_LE(test, param, 16);
    return param;
    }
#[no_mangle]
unsafe extern "C" fn test_mbw_max_to_percent_limits(test: *mut kunit) {
    static void test_mbw_max_to_percent_limits(struct kunit *test)
    {
    let mut fake_props: mpam_props = {0};
    u32 max_value;
    mpam_set_feature(mpam_feat_mbw_max, &fake_props);
...
//
#[no_mangle]
unsafe extern "C" fn test_percent_max_roundtrip_stability(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_percent_to_max_rounding(test: *mut kunit) {
    static void test_percent_to_max_rounding(struct kunit *test)
    {
    const struct percent_value_case *param = test.param_value;
    let mut num_rounded_up: c_uint = 0, total = 0;
    struct percent_value_test_info res;
