...
//! Source: drivers/platform/raspberrypi/vchiq-interface/vchiq_core.c
#![no_std]
...
pub const VCHIQ_SLOT_HANDLER_STACK: c_int = 8192;

pub const TYPE_SHIFT: c_int = 24;

pub const VCHIQ_PORT_FREE: c_uint = 0x1000;
...
pub const PAGELIST_WRITE: c_int = 0;
pub const PAGELIST_READ: c_int = 1;
pub const PAGELIST_READ_WITH_FRAGMENTS: c_int = 2;
pub const BELL2: c_uint = 0x08;
...
pub const VCHIQ_MSGID_CLAIMED: c_uint = 0x40000000;
pub const VCHIQ_FOURCC_INVALID: c_uint = 0x00000000;

    (service.stats. stat += addend)

pub const HANDLE_STATE_SHIFT: c_int = 12;
...
pub const NO_CLOSE_RECVD: c_int = 0;
pub const CLOSE_RECVD: c_int = 1;
pub const NO_RETRY_POLL: c_int = 0;
pub const RETRY_POLL: c_int = 1;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vchiq_open_payload {
    pub fourcc: c_int,
    pub client_id: c_int,
    pub version: c_short,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vchiq_openack_payload {
    pub version: c_short,
}
...
#[no_mangle]
pub unsafe extern "C" fn check_sizes() {
    ...
    {
    let mut idx: c_int = handle & (VCHIQ_MAX_SERVICES - 1);
...
// pidx = idx;
    ...
    {
    if (!service) {
    WARN(1, "%s service is core::ptr::null_mut()\n", __func__);
    return;
    }
    kref_get(&service.ref_count);
    }
#[no_mangle]
unsafe extern "C" fn service_release(kref: *mut kref) {
    ...
    rcu_assign_pointer(state.services[service.localport], core::ptr::null_mut());
    ...
    rcu_read_lock();
    service = handle_to_service(instance, handle);
    userdata = service ? service.base.userdata : core::ptr::null_mut();
    rcu_read_unlock();
    return userdata;
    }
...
// Unblock any sending thread.
    ...
    {
    void *cb_data = core::ptr::null_mut();
    void __user *cb_userdata = core::ptr::null_mut();
    int status;
//
...
//
    ...
    {
    let mut oldstate: enum vchiq_connstate = state.conn_state;
...
//
    ...
    let mut tx_pos: c_int = state.local_tx_pos;
    let mut slot_space: c_int = VCHIQ_SLOT_SIZE - (tx_pos & VCHIQ_SLOT_MASK);
    if (space > slot_space) {
...
// But first, flush through the last slot.
    ...
    return core::ptr::null_mut(); /* No space available */
    }
    ...
    struct vchiq_header *header)
    {
    let mut msgid: c_int = header.msgid;
    let mut port: c_int = VCHIQ_MSG_SRCPORT(msgid);
...
//
    ...
    let mut max_bytes: usize = size - pos;
...
// Called by the slot handler and application threads
    ...
    struct vchiq_service_quota *quota = core::ptr::null_mut();
    struct vchiq_header *header;
    let mut type: c_int = VCHIQ_MSG_TYPE(msgid);
    int svc_fourcc;
    size_t stride;
    local = state.local;
...
// Ensure that reads don't overtake the remote_event_wait.
    ...
    local.slot_sync);
    {
    let mut oldmsgid: c_int = header.msgid;
...
//
    ...
    let mut reason: enum vchiq_reason = get_bulk_reason(bulk);
    return make_service_callback(service, reason, core::ptr::null_mut(), bulk);
    }
    return 0;
    }
...
//
    ...
    {
    let mut flags: u32 = atomic_xchg(&state.poll_services[group], 0);
    int i;
    for (i = 0; flags; i++) {
    struct vchiq_service *service;
    u32 service_flags;
...
// Called by the slot handler thread
    ...
    let mut tmp: return = = (addr & PAGE_MASK);
    }
...
// Populate the fields of the pagelistinfo structure
    ...
    if (bulk.offset) {
    let mut length: c_ulong = count;
    let mut off: c_uint = offset;
    for (actual_pages = 0; actual_pages < num_pages;
    actual_pages++) {
    struct page *pg =
    vmalloc_to_page(((unsigned int *)bulk.offset +
    (actual_pages * PAGE_SIZE)));
    let mut bytes: usize = PAGE_SIZE - off;
    if (!pg) {
...
// Now set the pages for each scatterlist
    ...
    let mut len: c_uint = PAGE_SIZE - offset;
    if (len > count)
    len = count;
...
// Partial cache lines (fragments) require special measures
    ...
    let mut num_pages: c_uint = pagelistinfo.num_pages;
    unsigned int cache_line_size;
...
// Called with the bulk_mutex held
    ...
    {
    let mut is_tx: c_int = (queue == &service.bulk_tx);
    dev_dbg(service.state.dev,
...
// fabricate a matching dummy bulk
    bulk.remote_data = core::ptr::null_mut();
    bulk.remote_size = 0;
    queue.remote_insert++;
    }
...
// Acknowledge the OPEN
    if (service.sync) {
    if (queue_message_sync(state, core::ptr::null_mut(), openack_id,
...
// The service is now open
    ...
    if (queue_message(state, core::ptr::null_mut(), openack_id,
...
// No available service, or an invalid request - send a CLOSE
    if (queue_message(state, core::ptr::null_mut(), MAKE_CLOSE(0, VCHIQ_MSG_SRCPORT(msgid)),
    core::ptr::null_mut(), core::ptr::null_mut(), 0, 0) == -EINTR)
    goto bail_not_ready;
    return 1;
    bail_not_ready:
    if (service)
...
// Send a PAUSE in response
    if (queue_message(state, core::ptr::null_mut(), MAKE_PAUSE, core::ptr::null_mut(), core::ptr::null_mut(), 0,
...
// Remove the extra reference count.
    release_slot(state, state.rx_info, core::ptr::null_mut(), core::ptr::null_mut());
    state.rx_data = core::ptr::null_mut();
    }
    }
    }
//
...
// Retry later
    ...
    if (queue_message(state, core::ptr::null_mut(), MAKE_RESUME, core::ptr::null_mut(), core::ptr::null_mut(), 0,
...
// Ensure that reads don't overtake the remote_event_wait.
    ...
    let mut num_slots: c_int = (mem_size - mem_align) / VCHIQ_SLOT_SIZE;
    let mut first_data_slot: c_int = VCHIQ_SLOT_ZERO_SLOTS;
    check_sizes();
...
// Called from application thread when a client or server service is created.
    ...
    {
    struct vchiq_service *service;
    struct vchiq_service __rcu **pservice = core::ptr::null_mut();
    struct vchiq_service_quota *quota;
    int ret;
    int i;
...
// Don't unlock the service - leave it with a ref_count of 1.
    ...
    core::ptr::null_mut(), MAKE_OPEN(service.localport),
    memcpy_copy_callback,
    &payload,
    sizeof(payload),
    QMFLAGS_IS_BLOCKING);
...
// Wait for the ACK/NAK
    ...
    {
    struct vchiq_state *state = service.state;
    let mut slot_last: c_int = state.remote.slot_last;
    int i;
...
//
    ...
    (struct vchiq_header *)(data + pos);
    let mut msgid: c_int = header.msgid;
    let mut port: c_int = VCHIQ_MSG_DSTPORT(msgid);
    if ((port == service.localport) && (msgid & VCHIQ_MSGID_CLAIMED)) {
    dev_dbg(state.dev, "core:  fsi - hdr %p\n", header);
    release_slot(state, slot_info, header, core::ptr::null_mut());
    }
    pos += calc_stride(header.size);
    if (pos > VCHIQ_SLOT_SIZE) {
    dev_err(state.dev,
...
// Abort any outstanding bulk transfers
    ...
    {
    int status;
    let mut is_server: c_int = (service.public_fourcc != VCHIQ_FOURCC_INVALID);
    int newstate;
    ...
    status = make_service_callback(service, VCHIQ_SERVICE_CLOSED, core::ptr::null_mut(), core::ptr::null_mut());
    if (status != -EAGAIN) {
    let mut uc: c_int = service.service_use_count;
    int i;
...
//
    ...
    {
    struct vchiq_bulk_queue *queue;
    struct bulk_waiter *bulk_waiter = core::ptr::null_mut();
    struct vchiq_bulk *bulk;
    struct vchiq_state *state = service.state;
    let mut dir_char: c_char = (bulk_params.dir == VCHIQ_BULK_TRANSMIT) ? 't' : 'r';
    ...
    bulk_waiter.bulk = core::ptr::null_mut();
    }
...
//
    ...
    status = -EINTR;
#[no_mangle]
pub unsafe extern "C" fn if(VCHIQ_BULK_ACTUAL_ABORTED: bulk_waiter->actual ==) -> else {
...
// Shutdown mid-open - let the other side know
    status = queue_message(state, service, close_id, core::ptr::null_mut(), core::ptr::null_mut(), 0, 0);
    }
    break;
    ...
    if (!status)
    status = queue_message(state, service, close_id, core::ptr::null_mut(),
    core::ptr::null_mut(), 0, QMFLAGS_NO_MUTEX_UNLOCK);
    if (status) {
...
// Find all services registered to this client and enable them.
    i = 0;
    while ((service = next_service_by_instance(state, instance, &i)) != core::ptr::null_mut()) {
    if (service.srvstate == VCHIQ_SRVSTATE_HIDDEN)
    set_service_state(service, VCHIQ_SRVSTATE_LISTENING);
    vchiq_service_put(service);
    }
    if (state.conn_state == VCHIQ_CONNSTATE_DISCONNECTED) {
    status = queue_message(state, core::ptr::null_mut(), MAKE_CONNECT, core::ptr::null_mut(), core::ptr::null_mut(), 0,
    QMFLAGS_IS_BLOCKING);
    if (status)
    return status;
...
#[no_mangle]
pub unsafe extern "C" fn vchiq_get_config(config: *mut vchiq_config) {
...
// Don't include the lock just taken
    ...
    let mut fourcc: c_int = service.base.fourcc;
    ...
    if (tx_pending) {
    let mut i: c_uint = BULK_INDEX(service.bulk_tx.remove);
    tx_size = service.bulk_tx.bulks[i].size;
    }
    rx_pending = service.bulk_rx.local_insert -
    service.bulk_rx.remote_insert;
    if (rx_pending) {
    let mut i: c_uint = BULK_INDEX(service.bulk_rx.remove);
...
#[no_mangle]
pub unsafe extern "C" fn vchiq_dump_state(f: *mut seq_file, state: *mut vchiq_state) {
...
#[no_mangle]
pub unsafe extern "C" fn vchiq_send_remote_use(state: *mut vchiq_state) -> c_int {
    int vchiq_send_remote_use(struct vchiq_state *state)
    {
    if (state.conn_state == VCHIQ_CONNSTATE_DISCONNECTED)
    return -ENOTCONN;
    return queue_message(state, core::ptr::null_mut(), MAKE_REMOTE_USE, core::ptr::null_mut(), core::ptr::null_mut(), 0, 0);
    }
#[no_mangle]
pub unsafe extern "C" fn vchiq_send_remote_use_active(state: *mut vchiq_state) -> c_int {
    int vchiq_send_remote_use_active(struct vchiq_state *state)
    {
    if (state.conn_state == VCHIQ_CONNSTATE_DISCONNECTED)
    return -ENOTCONN;
    return queue_message(state, core::ptr::null_mut(), MAKE_REMOTE_USE_ACTIVE,
