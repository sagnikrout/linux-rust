...
//! Source: drivers/platform/chrome/chromeos_laptop.c
#![no_std]
...
pub const ATMEL_TP_I2C_ADDR: c_uint = 0x4b;
pub const ATMEL_TP_I2C_BL_ADDR: c_uint = 0x25;
pub const ATMEL_TS_I2C_ADDR: c_uint = 0x4a;
pub const ATMEL_TS_I2C_BL_ADDR: c_uint = 0x26;
pub const CYAPA_TP_I2C_ADDR: c_uint = 0x67;
pub const ELAN_TP_I2C_ADDR: c_uint = 0x15;
pub const ISL_ALS_I2C_ADDR: c_uint = 0x44;
pub const TAOS_ALS_I2C_ADDR: c_uint = 0x29;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct i2c_peripheral {
    pub board_info: i2c_board_info,
    pub alt_addr: c_ushort,
    pub dmi_name: *const c_char,
    pub irqflags: c_ulong,
    pub irq_resource: resource,
    pub type: enum i2c_adapter_type,
    pub pci_devid: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct acpi_peripheral {
    pub hid: [c_char; ACPI_ID_LEN],
    pub swnode: software_node,
    pub client: *mut i2c_client,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct chromeos_laptop {
//
// Note that we can't mark this pointer as const because
// i2c_new_scanned_device() changes passed in I2C board info, so.
//
    pub i2c_peripherals: *mut i2c_peripheral,
    pub num_i2c_peripherals: c_uint,
    pub acpi_peripherals: *mut acpi_peripheral,
    pub num_acpi_peripherals: c_uint,
}
...
//
    client = i2c_new_scanned_device(adapter, info, addr_list, core::ptr::null_mut());
    ...
    alt_addr_list, core::ptr::null_mut());
    if (!IS_ERR(dummy)) {
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_match_adapter_devid(dev: *mut device, devid: u32) -> bool {
    ...
    return false;
    pdev = to_pci_dev(dev);
    let mut devid: return = = pci_dev_id(pdev);
    }
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_check_adapter(adapter: *mut i2c_adapter) {
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_adjust_client(client: *mut i2c_client) -> bool {
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_detach_i2c_client(client: *mut i2c_client) {
    ...
    acpi_dev.client = core::ptr::null_mut();
    return;
    }
    }
    else
    ...
    i2c_dev.client = core::ptr::null_mut();
    return;
    }
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(&i2c_client_type: dev->type ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_scan_peripherals(dev: *mut device, data: *mut c_void) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_get_irq_from_dmi(dmi_name: *const c_char) -> int __init {
    static int __init chromeos_laptop_get_irq_from_dmi(const char *dmi_name)
    {
    const struct dmi_device *dmi_dev;
    const struct dmi_dev_onboard *dev_data;
    dmi_dev = dmi_find_device(DMI_DEV_TYPE_DEV_ONBOARD, dmi_name, core::ptr::null_mut());
    if (!dmi_dev) {
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_setup_irq(i2c_dev: *mut i2c_peripheral) -> int __init {
...
// Create primary fwnode for the device - copies everything
    ...
    info.fwnode = fwnode_create_software_node(i2c_dev.properties, core::ptr::null_mut());
    ...
    let mut n_peripherals: c_int = 0;
    int i;
    int error;
    for (i = 0; i < src.num_acpi_peripherals; i++) {
    if (acpi_dev_present(src.acpi_peripherals[i].hid, core::ptr::null_mut(), -1))
    n_peripherals++;
    }
    if (!n_peripherals)
    return 0;
    ...
    if (!acpi_dev_present(src_dev.hid, core::ptr::null_mut(), -1))
    continue;
// acpi_dev = *src_dev;
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_destroy(cros_laptop: *const chromeos_laptop) {
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_init() -> int __init {
...
//
    i2c_for_each_dev(core::ptr::null_mut(), chromeos_laptop_scan_peripherals);
    return 0;
...
#[no_mangle]
unsafe extern "C" fn chromeos_laptop_exit() -> void __exit {
