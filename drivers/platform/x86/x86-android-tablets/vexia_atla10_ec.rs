...
//! Source: drivers/platform/x86/x86-android-tablets/vexia_atla10_ec.c
#![no_std]
...
pub const ATLA10_EC_BATTERY_STATE_COMMAND: c_uint = 0x87;
pub const ATLA10_EC_BATTERY_INFO_COMMAND: c_uint = 0x88;
// From broken ACPI battery device in DSDT
pub const ATLA10_EC_VOLTAGE_MIN_DESIGN_uV: c_int = 3750000;
// Update data every 5 seconds

#[repr(C)]
#[derive(Copy, Clone)]
pub struct atla10_ec_battery_state {
    pub /: *mut *mut u8 status; / Using ACPI Battery spec status bits,
    pub /: *mut *mut u8 capacity; / Percent,
    pub charge_now_mAh: __le16,
    pub voltage_now_mV: __le16,
    pub current_now_mA: __le16,
    pub charge_full_mAh: __le16,
    pub /: *mut *mut __le16 temp; / centi degrees Celsius,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct atla10_ec_battery_info {
    pub charge_full_design_mAh: __le16,
    pub /: *mut *mut __le16 voltage_now_mV; / Should be design voltage, but is not ?,
    pub charge_full_design2_mAh: __le16,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct atla10_ec_data {
    pub client: *mut i2c_client,
    pub psy: *mut power_supply,
    pub work: delayed_work,
    pub update_lock: mutex,
    pub info: atla10_ec_battery_info,
    pub state: atla10_ec_battery_state,
    pub /: *mut *mut bool valid; / true if state is valid,
    pub /: *mut *mut unsigned long last_update; / In jiffies,
}

#[no_mangle]
unsafe extern "C" fn atla10_ec_cmd(data: *mut atla10_ec_data, cmd: u8, len: u8, values: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atla10_ec_update(data: *mut atla10_ec_data) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(ACPI_BATTERY_STATE_CHARGING: data->state.status &) -> else {
    else if (data.state.status & ACPI_BATTERY_STATE_CHARGING)
    val.intval = POWER_SUPPLY_STATUS_CHARGING;
#[no_mangle]
pub unsafe extern "C" fn if(100: data->state.capacity ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn atla10_ec_external_power_changed_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn atla10_ec_external_power_changed(psy: *mut power_supply) {
...
#[no_mangle]
unsafe extern "C" fn atla10_ec_probe(client: *mut i2c_client) -> c_int {
