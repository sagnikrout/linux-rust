...
//! Source: drivers/platform/x86/x86-android-tablets/core.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn x86_acpi_irq_helper_get(data: *const x86_acpi_irq_data) -> c_int {
    int x86_acpi_irq_helper_get(const struct x86_acpi_irq_data *data)
    {
    let mut fwspec: irq_fwspec = { };
...
//
    ...
    irq = acpi_register_gsi(core::ptr::null_mut(), data.index, data.trigger, data.polarity);
    if (irq < 0)
...
// Like acpi_dev_gpio_irq_get(), but without parsing ACPI resources
    ...
    status = acpi_get_handle(core::ptr::null_mut(), data.chip, &handle);
    ...
    acpi_handle handle;
    acpi_status status;
    status = acpi_get_handle(core::ptr::null_mut(), client_info.adapter_path, &handle);
    if (ACPI_FAILURE(status)) {
...
#[no_mangle]
unsafe extern "C" fn match_parent(dev: *mut device, data: *const c_void) -> __init int {
    ...
    pdev = bus_find_device_by_name(&pci_bus_type, core::ptr::null_mut(), client_info.adapter_path);
    if (!pdev) {
    pr_err("Error could not find %s PCI device\n", client_info.adapter_path);
    return core::ptr::null_mut();
    }
    adap_dev = bus_find_device(&i2c_bus_type, core::ptr::null_mut(), pdev, match_parent);
    if (adap_dev) {
    ...
    let mut board_info: i2c_board_info = client_info.board_info;
...
#[no_mangle]
unsafe extern "C" fn x86_instantiate_spi_dev(dev_info: *const x86_dev_info, idx: c_int) -> __init int {
    ...
    let mut board_info: spi_board_info = spi_dev_info.board_info;
    ...
    if (board_info.irq < 0)
    return board_info.irq;
    status = acpi_get_handle(core::ptr::null_mut(), spi_dev_info.ctrl_path, &handle);
    if (ACPI_FAILURE(status)) {
...
#[no_mangle]
unsafe extern "C" fn x86_instantiate_serdev(dev_info: *const x86_dev_info, idx: c_int) -> __init int {
    ...
    serdev_adev = acpi_dev_get_first_match_dev(info.serdev_hid, core::ptr::null_mut(), -1);
    if (!serdev_adev) {
...
#[no_mangle]
unsafe extern "C" fn gpio_secondary_unset(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn gpio_secondary_unregister_node_group(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn gpio_secondary_fwnode_init(parent: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn x86_android_tablet_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn x86_android_tablet_probe(pdev: *mut platform_device) -> __init int {
...
//
    ...
    gpiochip_node_group = core::ptr::null_mut();
    break;
    }
...
#[no_mangle]
unsafe extern "C" fn x86_android_tablet_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn x86_android_tablet_exit() -> void __exit {
