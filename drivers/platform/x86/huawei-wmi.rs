...
//! Source: drivers/platform/x86/huawei-wmi.c
#![no_std]
...
pub struct quirk_entry {
    pub battery_reset: bool,
    pub ec_micmute: bool,
    pub report_brightness: bool,
}

    static struct quirk_entry *quirks;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct huawei_wmi_debug {
    pub root: *mut dentry,
    pub arg: u64,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct huawei_wmi {
    pub battery_available: bool,
    pub fn_lock_available: bool,
    pub debug: huawei_wmi_debug,
...
// Ignore Ambient Light Sensoring
    ...
    let mut battery_reset: static int = -1;
    let mut report_brightness: static int = -1;
...
//
#[no_mangle]
unsafe extern "C" fn huawei_wmi_cmd(arg: u64, buf: *mut u8, buflen: usize) -> c_int {
...
// This is a workaround until the "legacy" interface is implemented.
    ...
    status = acpi_evaluate_object(handle, acpi_method, &arg_list, core::ptr::null_mut());
    if (ACPI_FAILURE(status))
    return -ENODEV;
    return 0;
    } else {
    ...
    return huawei_wmi_cmd(arg.cmd, core::ptr::null_mut(), 0);
    }
    }
#[no_mangle]
unsafe extern "C" fn huawei_wmi_leds_setup(dev: *mut device) {
...
// Battery protection
#[no_mangle]
unsafe extern "C" fn huawei_wmi_battery_get(start: *mut c_int, end: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_battery_set(start: c_int, end: c_int) -> c_int {
...
//
    ...
    err = huawei_wmi_cmd(arg.cmd, core::ptr::null_mut(), 0);
    return err;
    }
    ...
    {
    int err, start;
    err = huawei_wmi_battery_get(&start, core::ptr::null_mut());
    if (err)
    return err;
    ...
    {
    int err, end;
    err = huawei_wmi_battery_get(core::ptr::null_mut(), &end);
    if (err)
    return err;
    return sysfs_emit(buf, "%d\n", end);
    }
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_battery_add(battery: *mut power_supply, hook: *mut acpi_battery_hook) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_battery_remove(battery: *mut power_supply, hook: *mut acpi_battery_hook) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_battery_setup(dev: *mut device) {
    static void huawei_wmi_battery_setup(struct device *dev)
    {
    struct huawei_wmi *huawei = dev_get_drvdata(dev);
    huawei.battery_available = true;
    if (huawei_wmi_battery_get(core::ptr::null_mut(), core::ptr::null_mut())) {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_battery_exit(dev: *mut device) {
...
pub const FN_LOCK_ACPI_OFF: c_int = 1;
pub const FN_LOCK_ACPI_ON: c_int = 2;
pub const FN_LOCK_ACPI_STAT_OK: c_int = 0;
//
...
#[no_mangle]
unsafe extern "C" fn huawei_acpi_fn_lock_set(on: c_int) -> c_int {
...
//
    ...
    status = acpi_evaluate_object(core::ptr::null_mut(), "\\SFRS", &arg_list, &output);
    if (ACPI_FAILURE(status))
    return -EIO;
    union acpi_object *obj __free(kfree) = output.pointer;
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_fn_lock_get(on: *mut c_int) -> c_int {
    static int huawei_wmi_fn_lock_get(int *on)
    {
    u8 ret[0x100] = { 0 };
    int err, i;
// Newer models: use direct ACPI \GFRS method
    if (acpi_has_method(core::ptr::null_mut(), "\\GFRS"))
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_fn_lock_set(on: c_int) -> c_int {
    static int huawei_wmi_fn_lock_set(int on)
    {
    union hwmi_arg arg;
// Newer models: use direct ACPI \SFRS method
    if (acpi_has_method(core::ptr::null_mut(), "\\SFRS"))
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_fn_lock_setup(dev: *mut device) {
    static void huawei_wmi_fn_lock_setup(struct device *dev)
    {
    struct huawei_wmi *huawei = dev_get_drvdata(dev);
    huawei.fn_lock_available = true;
    if (huawei_wmi_fn_lock_get(core::ptr::null_mut())) {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_fn_lock_exit(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_debugfs_call_show(m: *mut seq_file, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_debugfs_setup(dev: *mut device) {
    static void huawei_wmi_debugfs_setup(struct device *dev)
    {
    struct huawei_wmi *huawei = dev_get_drvdata(dev);
    huawei.debug.root = debugfs_create_dir("huawei-wmi", core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_debugfs_exit(dev: *mut device) {
...
// Input
#[no_mangle]
unsafe extern "C" fn huawei_wmi_process_key(idev: *mut input_dev, code: c_int) {
...
//
    if (code == 0x80) {
    let mut response: acpi_buffer = { ACPI_ALLOCATE_BUFFER, core::ptr::null_mut() };
    union acpi_object *obj;
    acpi_status status;
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_input_notify(obj: *mut union acpi_object, context: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_input_setup(dev: *mut device, guid: *const c_char) -> c_int {
    ...
    idev.dev.parent = dev;
    err = sparse_keymap_setup(idev, huawei_wmi_keymap, core::ptr::null_mut());
    if (err)
    return err;
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_input_exit(dev: *mut device, guid: *const c_char) {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_init() -> __init int {
    ...
    if (err)
    goto pdrv_err;
    pdev = platform_device_register_simple("huawei-wmi", PLATFORM_DEVID_NONE, core::ptr::null_mut(), 0);
    if (IS_ERR(pdev)) {
    err = PTR_ERR(pdev);
    goto pdev_err;
    }
...
#[no_mangle]
unsafe extern "C" fn huawei_wmi_exit() -> __exit void {
