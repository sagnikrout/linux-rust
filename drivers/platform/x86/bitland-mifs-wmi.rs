...
//! Source: drivers/platform/x86/bitland-mifs-wmi.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bitland_mifs_output {
    pub reserved1: u8,
    pub operation: u8,
    pub reserved2: u8,
    pub function: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bitland_mifs_event {
    pub event_type: u8,
    pub event_id: u8,
    pub /: *mut *mut u8 value_low; / For most events, this is the value,
    pub /: *mut *mut u8 value_high; / For fan speed events, combined with value_low,
    pub reserved: [u8; 4],
    pub __packed: },
    pub BLOCKING_NOTIFIER_HEAD(bitland_notifier_list): static,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bitland_fan_notify_data {
    pub /: *mut *mut int channel; / 0 = CPU, 1 = GPU,
    pub speed: u16,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct bitland_mifs_wmi_data {
    pub wdev: *mut wmi_device,
    pub /: *mut *mut mutex lock; / Protects WMI calls,
    pub kbd_led: led_classdev,
    pub notifier: notifier_block,
    pub input_dev: *mut input_dev,
    pub hwmon_dev: *mut device,
    pub pp_dev: *mut device,
    pub saved_profile: enum platform_profile_option,
}
    ...
    {
    let mut in_buf: wmi_buffer = { .length = sizeof(*input), .data = (void *)input };
    let mut out_buf: wmi_buffer = { 0 };
    int ret;
...
#[no_mangle]
unsafe extern "C" fn bitland_check_performance_capability(data: *mut bitland_mifs_wmi_data) -> c_int {
...
// Full-speed/performance mode requires DC power (not USB-C)
    ...
    input.payload[0] = val;
    return bitland_mifs_wmi_call(data, &input, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn platform_profile_probe(drvdata: *mut c_void, choices: *mut c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bitland_mifs_wmi_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bitland_mifs_wmi_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn laptop_kbd_led_get(led_cdev: *mut led_classdev) -> enum led_brightness {
...
// GPU Mode: 0:Hybrid, 1:Discrete, 2:UMA
    ...
    ret = bitland_mifs_wmi_call(data, &input, core::ptr::null_mut());
    if (ret)
    return ret;
    return count;
    }
...
#[no_mangle]
unsafe extern "C" fn bitland_notifier_unregister(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn bitland_mifs_wmi_probe(wdev: *mut wmi_device, context: *const c_void) -> c_int {
...
// Register input device for hotkeys
    ...
    bitland_mifs_wmi_keymap, core::ptr::null_mut());
    if (ret)
    return ret;
