...
//! Source: drivers/platform/x86/dell/dell-wmi-sysman/sysman.c
#![no_std]
...
// reset bios to defaults
    ...
    let mut reset_option: static int = -1;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn populate_string_buffer(buffer: *mut u8, buffer_len: usize, str: *const c_char) -> isize {
...
//
#[no_mangle]
pub unsafe extern "C" fn calculate_string_buffer(str: *const c_char) -> usize {
...
//
#[no_mangle]
pub unsafe extern "C" fn calculate_security_buffer(authentication: *const c_char) -> usize {
...
//
#[no_mangle]
pub unsafe extern "C" fn populate_security_buffer(buffer: *mut u8, authentication: *const c_char) {
    void populate_security_buffer(u8 *buffer, const char *authentication)
    {
    let mut seclen: usize = strlen(authentication);
...
//
#[no_mangle]
pub unsafe extern "C" fn map_wmi_error(error_code: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn reset_bios_show(kobj: *mut kobject, attr: *mut kobj_attribute, buf: *mut c_char) -> isize {
...
//
    ...
    {
    let mut type: c_int = sysfs_match_string(reset_types, buf);
    int ret;
    if (type < 0)
    return type;
...
//
    ...
    let mut reset_bios: static struct kobj_attribute = __ATTR_RW(reset_bios);
    let mut pending_reboot: static struct kobj_attribute = __ATTR_RO(pending_reboot);
//
// create_attributes_level_sysfs_files() - Creates reset_bios and
// pending_reboot attributes
//
#[no_mangle]
unsafe extern "C" fn create_attributes_level_sysfs_files() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn attr_name_release(kobj: *mut kobject) {
...
//
#[no_mangle]
pub unsafe extern "C" fn strlcpy_attr(dest: *mut c_char, src: *mut c_char) {
    void strlcpy_attr(char *dest, char *src)
    {
    let mut len: usize = strlen(src) + 1;
...
//
    ...
    return ACPI_SUCCESS(status) ? (union acpi_object *)out.pointer : core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn get_instance_count(guid_string: *const c_char) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn alloc_attributes_data(attr_type: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn destroy_attribute_objs(kset: *mut kset) {
...
//
#[no_mangle]
unsafe extern "C" fn release_attributes_data() {
    ...
    wmi_priv.authentication_dir_kset = core::ptr::null_mut();
    }
    ...
    wmi_priv.main_dir_kset = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn init_bios_attributes(attr_type: c_int, guid: *const c_char) -> c_int {
...
// build attribute
    ...
    retval = kobject_init_and_add(attr_name_kobj, &attr_name_ktype, core::ptr::null_mut(), "%s",
    elements[ATTR_NAME].string.pointer);
    if (retval) {
...
#[no_mangle]
unsafe extern "C" fn sysman_init() -> int __init {
    static int __init sysman_init(void)
    {
    let mut ret: c_int = 0;
    if (!dmi_find_device(DMI_DEV_TYPE_OEM_STRING, "Dell System", core::ptr::null_mut()) &&
    !dmi_find_device(DMI_DEV_TYPE_OEM_STRING, "Alienware", core::ptr::null_mut()) &&
    !dmi_find_device(DMI_DEV_TYPE_OEM_STRING, "www.dell.com", core::ptr::null_mut())) {
    ...
    wmi_priv.class_dev = device_create(&firmware_attributes_class, core::ptr::null_mut(), MKDEV(0, 0),
    core::ptr::null_mut(), "%s", DRIVER_NAME);
    if (IS_ERR(wmi_priv.class_dev)) {
    ret = PTR_ERR(wmi_priv.class_dev);
    goto err_exit_bios_attr_pass_interface;
    }
    wmi_priv.main_dir_kset = kset_create_and_add("attributes", core::ptr::null_mut(),
    &wmi_priv.class_dev.kobj);
    if (!wmi_priv.main_dir_kset) {
    ret = -ENOMEM;
    goto err_destroy_classdev;
    }
    wmi_priv.authentication_dir_kset = kset_create_and_add("authentication", core::ptr::null_mut(),
    &wmi_priv.class_dev.kobj);
    if (!wmi_priv.authentication_dir_kset) {
    ret = -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn sysman_exit() -> void __exit {
