...
//! Source: drivers/platform/x86/intel/vsec.c
#![no_std]
...
pub const PMT_XA_START: c_int = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vsec_priv {
    pub info: *const intel_vsec_platform_info,
    pub suppliers: [*mut device; VSEC_FEATURE_COUNT],
    pub plat_info: oobmsm_plat_info,
    pub state: [enum vsec_device_state; VSEC_FEATURE_COUNT],
    pub found_caps: c_ulong,
}
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_supported(id: u16, caps: c_ulong) -> bool {
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_remove_aux(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_dev_free(intel_vsec_dev: *mut intel_vsec_device) {
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_dev_release(dev: *mut device) {
    ...
    let mut consumer_id: c_int = priv.info.num_deps;
    if (!deps)
    return core::ptr::null_mut();
    while (consumer_id--)
...
#[no_mangle]
unsafe extern "C" fn vsec_driver_present(cap_id: c_int) -> bool {
    static bool vsec_driver_present(int cap_id)
    {
    let mut bit: c_ulong = BIT(cap_id);
    switch (bit) {
...
//
    ...
    {
    struct intel_vsec_device __free(kfree) *intel_vsec_dev = core::ptr::null_mut();
    struct resource *res;
    struct resource *tmp;
    struct device *parent;
    let mut quirks: c_ulong = info.quirks;
    int i;
    if (info.parent)
    parent = info.parent;
    else
...
#[no_mangle]
unsafe extern "C" fn get_cap_id(header_id: u32, cap_id: *mut c_ulong) -> c_int {
...
//
    ...
    const struct intel_vsec_platform_info *info)
    {
    let mut have_devices: bool = false;
    let mut pos: c_int = 0;
    do {
    struct intel_vsec_header header;
    u32 table, hdr;
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_skip_missing_dependencies(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_pci_init(pdev: *mut pci_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_pci_probe(pdev: *mut pci_dev, id: *const pci_device_id) -> c_int {
...
pub const PCI_DEVICE_ID_INTEL_VSEC_ADL: c_uint = 0x467d;
pub const PCI_DEVICE_ID_INTEL_VSEC_DG1: c_uint = 0x490e;
pub const PCI_DEVICE_ID_INTEL_VSEC_MTL_M: c_uint = 0x7d0d;
pub const PCI_DEVICE_ID_INTEL_VSEC_MTL_S: c_uint = 0xad0d;
pub const PCI_DEVICE_ID_INTEL_VSEC_OOBMSM: c_uint = 0x09a7;
pub const PCI_DEVICE_ID_INTEL_VSEC_OOBMSM_DMR: c_uint = 0x09a1;
pub const PCI_DEVICE_ID_INTEL_VSEC_RPL: c_uint = 0xa77d;
pub const PCI_DEVICE_ID_INTEL_VSEC_TGL: c_uint = 0x9a0d;
pub const PCI_DEVICE_ID_INTEL_VSEC_LNL_M: c_uint = 0x647d;
pub const PCI_DEVICE_ID_INTEL_VSEC_PTL: c_uint = 0xb07d;
pub const PCI_DEVICE_ID_INTEL_VSEC_WCL: c_uint = 0xfd7d;
pub const PCI_DEVICE_ID_INTEL_VSEC_NVL: c_uint = 0xd70d;
    static const struct pci_device_id intel_vsec_pci_ids[] = {
    ...
    {
    let mut status: pci_ers_result_t = PCI_ERS_RESULT_NEED_RESET;
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_pci_slot_reset(pdev: *mut pci_dev) -> pci_ers_result_t {
    static pci_ers_result_t intel_vsec_pci_slot_reset(struct pci_dev *pdev)
    {
    struct intel_vsec_device *intel_vsec_dev;
    let mut status: pci_ers_result_t = PCI_ERS_RESULT_DISCONNECT;
    unsigned long index;
...
#[no_mangle]
unsafe extern "C" fn intel_vsec_pci_resume(pdev: *mut pci_dev) {
