...
//! Source: drivers/platform/x86/intel/vbtn.c
#![no_std]
...
pub const VGBS_TABLET_MODE_FLAG_ALT: c_uint = 0x10;
// When NOT in tablet mode, VGBS returns with the flag 0x40
pub const VGBS_TABLET_MODE_FLAG: c_uint = 0x40;
pub const VGBS_DOCK_MODE_FLAG: c_uint = 0x80;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct intel_vbtn_priv {
    pub /: *mut *mut mutex mutex; / Avoid notify_handler() racing with itself,
    pub buttons_dev: *mut input_dev,
    pub switches_dev: *mut input_dev,
    pub dual_accel: bool,
...
#[no_mangle]
unsafe extern "C" fn detect_tablet_mode(dev: *mut device) {
    ...
    acpi_status status;
    int m;
    status = acpi_evaluate_integer(handle, "VGBS", core::ptr::null_mut(), &vgbs);
...
//
#[no_mangle]
unsafe extern "C" fn intel_vbtn_input_setup(device: *mut platform_device) -> c_int {
    ...
    if (!priv.buttons_dev)
    return -ENOMEM;
    ret = sparse_keymap_setup(priv.buttons_dev, intel_vbtn_keymap, core::ptr::null_mut());
    if (ret)
    return ret;
    ...
    return -ENOMEM;
    ret = sparse_keymap_setup(priv.switches_dev, intel_vbtn_switchmap, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn intel_vbtn_has_switches(handle: acpi_handle, dual_accel: bool) -> bool {
...
#[no_mangle]
unsafe extern "C" fn intel_vbtn_probe(device: *mut platform_device) -> c_int {
    ...
    return -EBUSY;
    if (has_buttons) {
    status = acpi_evaluate_object(handle, "VBDL", core::ptr::null_mut(), core::ptr::null_mut());
    if (ACPI_FAILURE(status))
...
#[no_mangle]
unsafe extern "C" fn intel_vbtn_remove(device: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn intel_vbtn_pm_prepare(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_vbtn_pm_complete(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn intel_vbtn_pm_resume(dev: *mut device) -> c_int {
