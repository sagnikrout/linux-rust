...
//! Source: drivers/platform/x86/intel/pmc/tgl.c
#![no_std]
...
pub const ACPI_GET_LOW_MODE_REGISTERS: c_int = 1;
    enum pch_type {
    PCH_H,
    PCH_LP
    };
...
#[no_mangle]
pub unsafe extern "C" fn pmc_core_get_tgl_lpm_reqs(pdev: *mut platform_device) {
    void pmc_core_get_tgl_lpm_reqs(struct platform_device *pdev)
    {
    struct pmc_dev *pmcdev = platform_get_drvdata(pdev);
    struct pmc *pmc = pmcdev.pmcs[PMC_IDX_MAIN];
    let mut num_maps: c_int = pmc.map.lpm_num_maps;
    let mut lpm_size: u32 = LPM_MAX_NUM_MODES * num_maps * 4;
    ...
    ACPI_GET_LOW_MODE_REGISTERS, core::ptr::null_mut(), ACPI_TYPE_BUFFER);
    if (out_obj) {
    let mut size: u32 = out_obj.buffer.length;
    if (size != lpm_size) {
...
#[no_mangle]
unsafe extern "C" fn tgl_core_init(pmcdev: *mut pmc_dev, pmc_dev_info: *mut pmc_dev_info) -> c_int {
