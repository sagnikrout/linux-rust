...
//! Source: drivers/platform/x86/intel/pmc/pwrm_telemetry.c
#![no_std]
...
// DWORD3
    ...
    0x56, 0xb0, 0x4f, 0xc1);
#[no_mangle]
unsafe extern "C" fn is_valid_entry(pkg: *mut union acpi_object) -> bool {
...
#[no_mangle]
unsafe extern "C" fn pmc_pwrm_acpi_probe(pdev: *mut platform_device) -> c_int {
    static int pmc_pwrm_acpi_probe(struct platform_device *pdev)
    {
    struct intel_vsec_header header;
    struct intel_vsec_header *headers[2] = { &header, core::ptr::null_mut() };
    let mut buf: acpi_buffer = { ACPI_ALLOCATE_BUFFER, core::ptr::null_mut() };
    let mut info: intel_vsec_platform_info = { };
    ...
    if (!handle)
    return -ENODEV;
    status = acpi_evaluate_object(handle, "_DSD", core::ptr::null_mut(), &buf);
