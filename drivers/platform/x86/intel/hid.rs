...
//! Source: drivers/platform/x86/intel/hid.c
#![no_std]
...
//
    ...
    let mut enable_sw_tablet_mode: static int = TABLET_SW_AUTO;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct intel_hid_priv {
    pub /: *mut *mut mutex mutex; / Avoid notify_handler() racing with itself,
    pub input_dev: *mut input_dev,
    pub array: *mut input_dev,
    pub switches: *mut input_dev,
    pub wakeup_mode: bool,
}
...
// result = obj->integer.value;
    ...
    skip_dsm_eval:
    status = acpi_evaluate_integer(handle, method_name, core::ptr::null_mut(), result);
    if (ACPI_SUCCESS(status))
    return true;
    return false;
    }
#[no_mangle]
unsafe extern "C" fn intel_hid_init_dsm(handle: acpi_handle) {
    static void intel_hid_init_dsm(acpi_handle handle)
    {
    union acpi_object *obj;
    guid_parse(HID_EVENT_FILTER_UUID, &intel_dsm_guid);
    obj = acpi_evaluate_dsm_typed(handle, &intel_dsm_guid, 1, 0, core::ptr::null_mut(),
    ACPI_TYPE_BUFFER);
    if (obj) {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_set_enable(device: *mut device, enable: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_button_array_enable(device: *mut device, enable: bool) {
...
// Query supported platform features
    status = acpi_evaluate_integer(handle, "BTNC", core::ptr::null_mut(), &button_cap);
...
#[no_mangle]
unsafe extern "C" fn intel_hid_pm_prepare(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_pm_complete(device: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_pl_suspend_handler(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_pl_freeze_handler(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_pl_resume_handler(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_input_setup(device: *mut platform_device) -> c_int {
    ...
    if (!priv.input_dev)
    return -ENOMEM;
    ret = sparse_keymap_setup(priv.input_dev, intel_hid_keymap, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn intel_button_array_input_setup(device: *mut platform_device) -> c_int {
...
// Setup input device for 5 button array
    ...
    if (!priv.array)
    return -ENOMEM;
    ret = sparse_keymap_setup(priv.array, intel_array_keymap, core::ptr::null_mut());
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn intel_hid_switches_setup(device: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn report_tablet_mode_state(device: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn report_tablet_mode_event(input_dev: *mut input_dev, event: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn notify_handler(handle: acpi_handle, event: u32, context: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn button_array_present(device: *mut platform_device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_probe(device: *mut platform_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(!dual_accel_detect(): dmi_check_system(dmi_auto_add_switch) &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn intel_hid_remove(device: *mut platform_device) {
