...
//! Source: drivers/platform/x86/intel/telemetry/pltdrv.c
#![no_std]
...
pub const TELEM_TRC_VERBOSITY_MASK: c_uint = 0x3;

pub const TELEM_SAMPLING_DEFAULT_PERIOD: c_uint = 0xD;
pub const TELEM_MAX_EVENTS_SRAM: c_int = 28;
pub const TELEM_SSRAM_STARTTIME_OFFSET: c_int = 8;
pub const TELEM_SSRAM_EVTLOG_OFFSET: c_int = 16;
pub const IOSS_TELEM: c_uint = 0xeb;
pub const IOSS_TELEM_EVENT_READ: c_uint = 0x0;
pub const IOSS_TELEM_EVENT_WRITE: c_uint = 0x1;
pub const IOSS_TELEM_INFO_READ: c_uint = 0x2;
pub const IOSS_TELEM_TRACE_CTL_READ: c_uint = 0x5;
pub const IOSS_TELEM_TRACE_CTL_WRITE: c_uint = 0x6;
pub const IOSS_TELEM_EVENT_CTL_READ: c_uint = 0x7;
pub const IOSS_TELEM_EVENT_CTL_WRITE: c_uint = 0x8;
pub const IOSS_TELEM_EVT_WRITE_SIZE: c_uint = 0x3;
...
pub const TELEM_INFO_NENABLES_MASK: c_uint = 0xFF;
pub const TELEM_EVENT_ENABLE: c_uint = 0x8000;
pub const TELEM_MASK_BIT: c_int = 1;
pub const TELEM_MASK_BYTE: c_uint = 0xFF;
pub const BYTES_PER_LONG: c_int = 8;
pub const TELEM_MASK_PCS_STATE: c_uint = 0xF;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct telem_ssram_region {
    pub timestamp: u64,
    pub start_time: u64,
    pub events: [u64; TELEM_MAX_EVENTS_SRAM],
}
...
// unit_config = &(telm_conf->ioss_config);
    ...
    if (len > TELEM_MAX_EVENTS_SRAM)
    return -EINVAL;
    if ((len > 0) && (evtmap == core::ptr::null_mut()))
    return -EINVAL;
    break;
    case TELEM_ADD:
...
#[no_mangle]
pub unsafe extern "C" fn telemetry_plt_config_ioss_event(evt_id: u32, index: c_int) -> c_int {
    ...
    IOSS_TELEM_EVT_WRITE_SIZE, core::ptr::null_mut(), 0);
    }
#[no_mangle]
pub unsafe extern "C" fn telemetry_plt_config_pss_event(evt_id: u32, index: c_int) -> c_int {
    ...
    index, 0, &write_buf, core::ptr::null_mut());
    return ret;
    }
...
// Get telemetry EVENT CTL
    ...
    IOSS_TELEM_EVENT_CTL_READ, core::ptr::null_mut(), 0,
...
// Disable Telemetry
    ...
    sizeof(telem_ctrl), core::ptr::null_mut(), 0);
    if (ret) {
...
// Enable Periodic Telemetry Events and enable SRAM trace
    ...
    ret = intel_scu_ipc_dev_command(scu, IOSS_TELEM,
    IOSS_TELEM_EVENT_CTL_WRITE,
    &telem_ctrl, sizeof(telem_ctrl), core::ptr::null_mut(), 0);
    if (ret) {
...
// Get telemetry EVENT CTL
    ...
    0, 0, core::ptr::null_mut(), &telem_ctrl);
    if (ret) {
...
// Disable Telemetry
    ...
    0, 0, &telem_ctrl, core::ptr::null_mut());
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn telemetry_setup(pdev: *mut platform_device) -> c_int {
    ...
    IOSS_TELEM_INFO_READ, core::ptr::null_mut(), 0,
...
// Elements matched
    ...
    let mut temp: u32 = 0;
    int ret;
    if (verbosity == core::ptr::null_mut())
    return -EINVAL;
    ...
    core::ptr::null_mut(), 0, &temp, sizeof(temp));
    if (ret) {
    ...
    &temp, sizeof(temp), core::ptr::null_mut(), 0);
    if (ret) {
...
#[no_mangle]
unsafe extern "C" fn telemetry_pltdrv_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn telemetry_pltdrv_remove(pdev: *mut platform_device) {
