...
//! Source: drivers/platform/x86/hp/hp-bioscfg/order-list-attributes.c
#![no_std]
...
unsafe extern "C" fn current_value_show(kobj: *mut kobject, attr: *mut kobj_attribute, buf: *mut c_char) -> isize {
    ...
    {
    let mut instance_id: c_int = get_ordered_list_instance_id(kobj);
    if (instance_id < 0)
    return -EIO;
...
#[no_mangle]
unsafe extern "C" fn replace_char_str(buffer: *mut u8, repl_char: *mut c_char, repl_with: *mut c_char) -> c_int {
    static int replace_char_str(u8 *buffer, char *repl_char, char *repl_with)
    {
    char *src = buffer;
    let mut buflen: c_int = strlen(buffer);
    int item;
    if (buflen < 1)
    return -EINVAL;
...
//
#[no_mangle]
unsafe extern "C" fn validate_ordered_list_input(instance: c_int, buf: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn update_ordered_list_value(instance: c_int, attr_value: *mut c_char) {
...
#[no_mangle]
pub unsafe extern "C" fn hp_alloc_ordered_list_data() -> c_int {
...
// Expected Values types associated with each element
    ...
    int reqs;
    int eloc;
    char *tmpstr = core::ptr::null_mut();
    char *part_tmp = core::ptr::null_mut();
    let mut tmp_len: c_int = 0;
    char *part = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn hp_exit_ordered_list_attributes() {
    ...
    bioscfg_drv.ordered_list_data = core::ptr::null_mut();
    }
