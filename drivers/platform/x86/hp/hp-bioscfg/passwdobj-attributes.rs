...
//! Source: drivers/platform/x86/hp/hp-bioscfg/passwdobj-attributes.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn clear_passwords(instance: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn hp_clear_all_credentials() -> c_int {
    int hp_clear_all_credentials(void)
    {
    let mut count: c_int = bioscfg_drv.password_instances_count;
    int instance;
// clear all passwords
...
#[no_mangle]
pub unsafe extern "C" fn hp_get_password_instance_for_type(name: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn validate_password_input(instance_id: c_int, buf: *const c_char) -> c_int {
    ...
    let mut password_is_password_set: static struct kobj_attribute = __ATTR_RO(is_enabled);
    ...
    let mut password_current_password: static struct kobj_attribute = __ATTR_WO(current_password);
    ...
    let mut password_new_password: static struct kobj_attribute = __ATTR_WO(new_password);
    ATTRIBUTE_N_PROPERTY_SHOW(min_password_length, password);
    let mut password_min_password_length: static struct kobj_attribute = __ATTR_RO(min_password_length);
    ATTRIBUTE_N_PROPERTY_SHOW(max_password_length, password);
    let mut password_max_password_length: static struct kobj_attribute = __ATTR_RO(max_password_length);
#[no_mangle]
unsafe extern "C" fn role_show(kobj: *mut kobject, attr: *mut kobj_attribute, buf: *mut c_char) -> isize {
    ...
    let mut password_role: static struct kobj_attribute = __ATTR_RO(role);
    static ssize_t mechanism_show(struct kobject *kobj, struct kobj_attribute *attr,
    char *buf)
    {
    let mut i: c_int = get_password_instance_id(kobj);
    if (i < 0)
    return i;
    ...
    let mut password_mechanism: static struct kobj_attribute = __ATTR_RO(mechanism);
    ATTRIBUTE_VALUES_PROPERTY_SHOW(encodings, password, SEMICOLON_SEP);
    let mut password_encodings_val: static struct kobj_attribute = __ATTR_RO(encodings);
...
#[no_mangle]
pub unsafe extern "C" fn hp_alloc_password_data() -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn hp_exit_password_attributes() {
    ...
    bioscfg_drv.password_data = core::ptr::null_mut();
    }
