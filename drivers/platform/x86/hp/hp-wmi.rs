...
//! Source: drivers/platform/x86/hp/hp-wmi.c
#![no_std]
...
pub const HP_FAN_SPEED_AUTOMATIC: c_uint = 0x00;
pub const HP_POWER_LIMIT_DEFAULT: c_uint = 0x00;
pub const HP_POWER_LIMIT_NO_CHANGE: c_uint = 0xFF;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct thermal_profile_params {
    pub performance: u8,
    pub balanced: u8,
    pub low_power: u8,
    pub ec_tp_offset: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hp_wmi_fan_profile_params {
    pub fan): *mut *mut int (get_fan_speed)(int,
    pub fan_table: bool,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct hp_wmi_board_params {
    pub thermal_profile: *const thermal_profile_params,
    pub fan_profile: *const hp_wmi_fan_profile_params,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct victus_power_limits {
    pub pl1: u8,
    pub pl2: u8,
    pub pl4: u8,
    pub cpu_gpu_concurrent_limit: u8,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct victus_gpu_power_modes {
    pub ctgp_enable: u8,
    pub ppab_enable: u8,
    pub dstate: u8,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bios_rfkill2_device_state {
    pub radio_type: u8,
    pub bus_type: u8,
    pub vendor_id: u16,
    pub product_id: u16,
    pub subsys_vendor_id: u16,
    pub subsys_product_id: u16,
    pub rfkill_id: u8,
    pub power: u8,
    pub unknown: [u8; 4],
}
...
pub const HPWMI_MAX_RFKILL2_DEVICES: c_int = 7;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bios_rfkill2_state {
    pub unknown: [u8; 7],
    pub count: u8,
    pub pad: [u8; 8],
    pub device: [bios_rfkill2_device_state; HPWMI_MAX_RFKILL2_DEVICES],
}
...
pub const DEVICE_MODE_TABLET: c_uint = 0x06;
pub const CPU_FAN: c_int = 0;
pub const GPU_FAN: c_int = 1;
    enum pwm_modes {
    PWM_MODE_MAX		= 0,
    PWM_MODE_MANUAL		= 1,
    PWM_MODE_AUTO		= 2,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hp_wmi_hwmon_priv {
    pub /: *mut *mut mutex lock; / protects mode, pwm,
    pub min_rpm: u8,
    pub max_rpm: u8,
    pub mode: u8,
    pub cpu_pwm: u8,
    pub gpu_pwm: u8,
    pub keep_alive_dwork: delayed_work,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct victus_s_fan_table_header {
    pub num_fans: u8,
    pub unknown: u8,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct victus_s_fan_table_entry {
    pub cpu_rpm: u8,
    pub gpu_rpm: u8,
    pub noise_db: u8,
    pub __packed: },
#[repr(C)]
#[derive(Copy, Clone)]
pub struct victus_s_fan_table {
    pub header: victus_s_fan_table_header,
    pub entries: [victus_s_fan_table_entry; ],
    pub __packed: },
//
// 90s delay to prevent the firmware from resetting fan mode after fixed
// 120s timeout
//
pub const KEEP_ALIVE_DELAY_SECS: c_int = 90;
#[no_mangle]
pub unsafe extern "C" fn rpm_to_pwm(rpm: u8, priv: *mut hp_wmi_hwmon_priv) -> u8 {
    static inline u8 rpm_to_pwm(u8 rpm, struct hp_wmi_hwmon_priv *priv)
    {
    return fixp_linear_interpolate(0, 0, priv.max_rpm, U8_MAX,
    pub priv->max_rpm)): clamp_val(rpm, 0,,
    }
#[no_mangle]
pub unsafe extern "C" fn pwm_to_rpm(pwm: u8, priv: *mut hp_wmi_hwmon_priv) -> u8 {
    static inline u8 pwm_to_rpm(u8 pwm, struct hp_wmi_hwmon_priv *priv)
    {
    return fixp_linear_interpolate(0, 0, U8_MAX, priv.max_rpm,
    pub U8_MAX)): clamp_val(pwm, 0,,
    }
// map output size to the corresponding WMI method id
#[no_mangle]
pub unsafe extern "C" fn encode_outsize_for_pvsz(outsize: c_int) -> c_int {
...
//
    ...
    {
    pub }: acpi_buffer input, output = { ACPI_ALLOCATE_BUFFER, NULL,
    pub bios_return: *mut bios_return,
    pub NULL: *mut *mut union acpi_object obj =,
    pub NULL: *mut *mut bios_args args =,
    pub actual_outsize: int mid, actual_insize,,
    pub bios_args_size: usize,
    pub ret: c_int,
    pub encode_outsize_for_pvsz(outsize): mid =,
    if (WARN_ON(mid < 0))
    pub mid: return,
    pub 128): actual_insize = max(insize,,
    pub actual_insize): bios_args_size = struct_size(args, data,,
    pub GFP_KERNEL): args = kmalloc(bios_args_size,,
    if (!args)
    pub -ENOMEM: return,
    pub bios_args_size: input.length =,
    pub args: input.pointer =,
    pub 0x55434553: args->signature =,
    pub command: args->command =,
    pub query: args->commandtype =,
    pub insize: args->datasize =,
    pub insize)): memcpy(args->data, buffer, flex_array_size(args, data,,
    pub &output): ret = wmi_evaluate_method(HPWMI_BIOS_GUID, 0, mid, &input,,
    if (ret)
    pub out_free: goto,
    pub output.pointer: obj =,
    if (!obj) {
    pub -EINVAL: ret =,
    pub out_free: goto,
    }
    if (obj.type != ACPI_TYPE_BUFFER) {
    pub ret): pr_warn("query 0x%x returned an invalid object 0x%x\n", query,,
    pub -EINVAL: ret =,
    pub out_free: goto,
    }
    pub )obj->buffer.pointer: *mut bios_return = (struct bios_return,
    pub bios_return->return_code: ret =,
    if (ret) {
    if (ret != HPWMI_RET_UNKNOWN_COMMAND &&
    ret != HPWMI_RET_UNKNOWN_CMDTYPE)
    pub ret): pr_warn("query 0x%x returned error 0x%x\n", query,,
    pub out_free: goto,
    }
// Ignore output data of zero size
    if (!outsize)
    pub out_free: goto,
    pub sizeof(*bios_return))): *mut actual_outsize = min(outsize, (int)(obj->buffer.length -,
    pub actual_outsize): *mut *mut memcpy(buffer, obj->buffer.pointer + sizeof(bios_return),,
    pub actual_outsize): memset(buffer + actual_outsize, 0, outsize -,
    out_free:
    pub ret: return,
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_fan_count_userdefine_trigger() -> c_int {
    ...
    if (ret != 0)
    pub -EINVAL: return,
    pub /: *mut *mut return fan_data[0]; / Others bytes aren't providing fan count,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_fan_speed(fan: c_int) -> c_int {
    static int hp_wmi_get_fan_speed(int fan)
    {
    pub fsl: u8 fsh,,
    pub }: char fan_data[4] = { fan, 0, 0, 0,
    ...
    pub fan_data: [fsh =; 2],
    pub fan_data: [fsl =; 3],
    pub fsl: return (fsh << 8) |,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_fan_speed_victus_s(fan: c_int) -> c_int {
    static int hp_wmi_get_fan_speed_victus_s(int fan)
    {
    pub {}: u8 fan_data[128] =,
    pub ret: c_int,
    ...
    if (ret != 0)
    pub -EINVAL: return,
    pub 100: *mut *mut return fan_data[fan],
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_read_int(query: c_int) -> c_int {
    static int hp_wmi_read_int(int query)
    {
    pub ret: int val = 0,,
    ret = hp_wmi_perform_query(query, HPWMI_READ, &val,
    pub sizeof(val)): zero_if_sup(val),,
    if (ret)
    pub -EINVAL: return ret < 0 ? ret :,
    pub val: return,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_dock_state() -> c_int {
    static int hp_wmi_get_dock_state(void)
    {
    pub hp_wmi_read_int(HPWMI_HARDWARE_QUERY): int state =,
    if (state < 0)
    pub state: return,
    pub HPWMI_DOCK_MASK): return !!(state &,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_tablet_mode() -> c_int {
    static int hp_wmi_get_tablet_mode(void)
    {
    pub }: char system_device_mode[4] = { 0,
    pub chassis_type: *const c_char,
    pub tablet_found: bool,
    pub ret: c_int,
    pub dmi_get_system_info(DMI_CHASSIS_TYPE): chassis_type =,
    if (!chassis_type)
    pub -ENODEV: return,
    ...
    if (ret < 0)
    pub ret: return,
    pub DEVICE_MODE_TABLET: return system_device_mode[0] ==,
    }
#[no_mangle]
unsafe extern "C" fn omen_thermal_profile_set(mode: c_int) -> c_int {
...
//
    pub mode}: char buffer[2] = {-1,,
    pub ret: c_int,
    ret = hp_wmi_perform_query(HPWMI_SET_PERFORMANCE_MODE, HPWMI_GM,
    pub 0): &buffer, sizeof(buffer),,
    if (ret)
    pub -EINVAL: return ret < 0 ? ret :,
    pub mode: return,
    }
#[no_mangle]
unsafe extern "C" fn is_omen_thermal_profile() -> bool {
    static bool is_omen_thermal_profile(void)
    {
    pub dmi_get_system_info(DMI_BOARD_NAME): *const *const char board_name =,
    if (!board_name)
    pub false: return,
...
#[no_mangle]
unsafe extern "C" fn omen_get_thermal_policy_version() -> c_int {
    static int omen_get_thermal_policy_version(void)
    {
    pub }: unsigned char buffer[8] = { 0,
    pub ret: c_int,
    pub dmi_get_system_info(DMI_BOARD_NAME): *const *const char board_name =,
    if (board_name) {
    ...
    pub sizeof(buffer)): &buffer, sizeof(buffer),,
    if (ret)
    pub -EINVAL: return ret < 0 ? ret :,
    pub buffer: [return; 3],
    }
#[no_mangle]
unsafe extern "C" fn omen_thermal_profile_get() -> c_int {
    static int omen_thermal_profile_get(void)
    {
    pub data: u8,
    pub &data): int ret = ec_read(HP_OMEN_EC_THERMAL_PROFILE_OFFSET,,
    if (ret)
    pub ret: return,
    pub data: return,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_fan_speed_max_set(enabled: c_int) -> c_int {
    static int hp_wmi_fan_speed_max_set(int enabled)
    {
    pub ret: c_int,
    ret = hp_wmi_perform_query(HPWMI_FAN_SPEED_MAX_SET_QUERY, HPWMI_GM,
    pub 0): &enabled, sizeof(enabled),,
    if (ret)
    pub -EINVAL: return ret < 0 ? ret :,
    pub enabled: return,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_fan_speed_set(priv: *mut hp_wmi_hwmon_priv) -> c_int {
    static int hp_wmi_fan_speed_set(struct hp_wmi_hwmon_priv *priv)
    {
    pub fan_speed: [u8; 2],
    pub ret: c_int,
    if (priv.cpu_pwm == HP_FAN_SPEED_AUTOMATIC)
    pub HP_FAN_SPEED_AUTOMATIC: fan_speed[CPU_FAN] =,
    else
    pub priv): fan_speed[CPU_FAN] = pwm_to_rpm(priv->cpu_pwm,,
    if (priv.gpu_pwm == HP_FAN_SPEED_AUTOMATIC)
    pub HP_FAN_SPEED_AUTOMATIC: fan_speed[GPU_FAN] =,
    else
    pub priv): fan_speed[GPU_FAN] = pwm_to_rpm(priv->gpu_pwm,,
    pub hp_wmi_get_fan_count_userdefine_trigger(): ret =,
    if (ret < 0)
    pub ret: return,
// Max fans need to be explicitly disabled
    pub hp_wmi_fan_speed_max_set(0): ret =,
    if (ret < 0)
    pub ret: return,
    ret = hp_wmi_perform_query(HPWMI_VICTUS_S_FAN_SPEED_SET_QUERY, HPWMI_GM,
    pub 0): &fan_speed, sizeof(fan_speed),,
    pub ret: return,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_fan_speed_reset(priv: *mut hp_wmi_hwmon_priv) -> c_int {
    static int hp_wmi_fan_speed_reset(struct hp_wmi_hwmon_priv *priv)
    {
    pub HP_FAN_SPEED_AUTOMATIC: priv->cpu_pwm =,
    pub HP_FAN_SPEED_AUTOMATIC: priv->gpu_pwm =,
    pub hp_wmi_fan_speed_set(priv): return,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_fan_speed_max_reset(priv: *mut hp_wmi_hwmon_priv) -> c_int {
...
// Disabling max fan speed on Victus s1xxx laptops needs a 2nd step:
    pub hp_wmi_fan_speed_reset(priv): return,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_bios_2008_later() -> int __init {
    static int __init hp_wmi_bios_2008_later(void)
    {
    pub 0: int state =,
    int ret = hp_wmi_perform_query(HPWMI_FEATURE_QUERY, HPWMI_READ, &state,
    pub sizeof(state)): zero_if_sup(state),,
    if (!ret)
    pub 1: return,
    pub -ENXIO: return (ret == HPWMI_RET_UNKNOWN_CMDTYPE) ? 0 :,
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_bios_2009_later() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_enable_hotkeys() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_set_block(data: *mut c_void, blocked: bool) -> c_int {
    static int hp_wmi_set_block(void *data, bool blocked)
    {
    pub (long)data: enum hp_wmi_radio r =,
    pub r): int query = BIT(r + 8) | ((!blocked) <<,
    pub ret: c_int,
    ret = hp_wmi_perform_query(HPWMI_WIRELESS_QUERY, HPWMI_WRITE,
    pub 0): &query, sizeof(query),,
    pub -EINVAL: return ret <= 0 ? ret :,
    }
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_sw_state(r: enum hp_wmi_radio) -> bool {
    static bool hp_wmi_get_sw_state(enum hp_wmi_radio r)
    {
    let mut mask: c_int = 0x200 << (r * 8);
    let mut wireless: c_int = hp_wmi_read_int(HPWMI_WIRELESS_QUERY);
// TBD: Pass error
    WARN_ONCE(wireless < 0, "error executing HPWMI_WIRELESS_QUERY");
    return !(wireless & mask);
    }
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_hw_state(r: enum hp_wmi_radio) -> bool {
    static bool hp_wmi_get_hw_state(enum hp_wmi_radio r)
    {
    let mut mask: c_int = 0x800 << (r * 8);
    let mut wireless: c_int = hp_wmi_read_int(HPWMI_WIRELESS_QUERY);
// TBD: Pass error
    WARN_ONCE(wireless < 0, "error executing HPWMI_WIRELESS_QUERY");
    return !(wireless & mask);
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_rfkill2_set_block(data: *mut c_void, blocked: bool) -> c_int {
    static int hp_wmi_rfkill2_set_block(void *data, bool blocked)
    {
    let mut rfkill_id: c_int = (int)(long)data;
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_rfkill2_refresh() -> c_int {
    ...
    let mut num: c_int = rfkill2[i].num;
    ...
    char *buf)
    {
    let mut value: c_int = hp_wmi_read_int(HPWMI_DISPLAY_QUERY);
    if (value < 0)
    return value;
    ...
    char *buf)
    {
    let mut value: c_int = hp_wmi_read_int(HPWMI_HDDTEMP_QUERY);
    if (value < 0)
    return value;
    return sysfs_emit(buf, "%d\n", value);
    }
    ...
    char *buf)
    {
    let mut value: c_int = hp_wmi_read_int(HPWMI_ALS_QUERY);
    if (value < 0)
    return value;
    return sysfs_emit(buf, "%d\n", value);
    }
    ...
    char *buf)
    {
    let mut value: c_int = hp_wmi_get_dock_state();
    if (value < 0)
    return value;
    return sysfs_emit(buf, "%d\n", value);
    }
    ...
    char *buf)
    {
    let mut value: c_int = hp_wmi_get_tablet_mode();
    if (value < 0)
    return value;
    return sysfs_emit(buf, "%d\n", value);
    }
...
// Get the POST error code of previous boot failure.
    let mut value: c_int = hp_wmi_read_int(HPWMI_POSTCODEERROR_QUERY);
    if (value < 0)
    return value;
...
#[no_mangle]
unsafe extern "C" fn camera_shutter_input_setup() -> c_int {
    ...
    camera_shutter_input_dev = core::ptr::null_mut();
    return err;
    }
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_mux_supported_modes(supported: *mut u8) -> c_int {
    static int hp_wmi_get_mux_supported_modes(u8 *supported)
    {
    u8 legacy_buffer[4] = {};
    u8 buffer[128] = {};
    let mut req_packet: u32 = 0;
    int ret;
    if (!supported)
    return -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_mux_mode(mode: *mut u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_set_mux_mode(mode: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_notify(obj: *mut union acpi_object, context: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn if(0xfe: event_data ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_input_setup() -> int __init {
...
// Tablet mode
    ...
    err = sparse_keymap_setup(hp_wmi_input_dev, hp_wmi_keymap, core::ptr::null_mut());
    if (err)
    goto err_free_dev;
// Set initial hardware state
    ...
    status = wmi_install_notify_handler(HPWMI_EVENT_GUID, hp_wmi_notify, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_input_destroy() {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_rfkill_setup(device: *mut platform_device) -> int __init {
    ...
    bluetooth_rfkill = core::ptr::null_mut();
    if (wifi_rfkill)
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_rfkill2_setup(device: *mut platform_device) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn platform_profile_omen_get_ec(profile: *mut enum platform_profile_option) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn has_omen_thermal_profile_ec_timer() -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn omen_thermal_profile_ec_flags_set(flags: enum hp_thermal_profile_omen_flags) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn omen_thermal_profile_ec_timer_set(value: u8) -> c_int {
    inline int omen_thermal_profile_ec_timer_set(u8 value)
    {
    return ec_write(HP_OMEN_EC_THERMAL_PROFILE_TIMER_OFFSET, value);
    }
#[no_mangle]
unsafe extern "C" fn platform_profile_omen_set_ec(profile: enum platform_profile_option) -> c_int {
    static int platform_profile_omen_set_ec(enum platform_profile_option profile)
    {
    int err, tp, tp_version;
    let mut flags: enum hp_thermal_profile_omen_flags = 0;
...
#[no_mangle]
unsafe extern "C" fn thermal_profile_get() -> c_int {
    static int thermal_profile_get(void)
    {
    return hp_wmi_read_int(HPWMI_THERMAL_PROFILE_QUERY);
    }
#[no_mangle]
unsafe extern "C" fn thermal_profile_set(thermal_profile: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_victus_thermal_profile() -> bool {
...
#[no_mangle]
unsafe extern "C" fn platform_profile_victus_get_ec(profile: *mut enum platform_profile_option) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn platform_profile_victus_set_ec(profile: enum platform_profile_option) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_victus_s_thermal_profile() -> bool {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_fan_control_supported() -> bool {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_fan_table_supported() -> bool {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_get_active_fan_speed(fan: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn victus_s_set_cpu_pl1_pl2(pl1: u8, pl2: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn platform_profile_victus_s_get_ec(profile: *mut enum platform_profile_option) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(1: current_ctgp_state == 0 && current_ppab_state ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn platform_profile_victus_s_set_ec(profile: enum platform_profile_option) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_platform_profile_probe(drvdata: *mut c_void, choices: *mut c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn omen_register_powersource_event_handler() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn victus_s_register_powersource_event_handler() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn omen_unregister_powersource_event_handler() {
    static inline void omen_unregister_powersource_event_handler(void)
    {
    unregister_acpi_notifier(&platform_power_source_nb);
    }
#[no_mangle]
pub unsafe extern "C" fn victus_s_unregister_powersource_event_handler() {
...
#[no_mangle]
unsafe extern "C" fn thermal_profile_setup(device: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_bios_setup(device: *mut platform_device) -> int __init {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_bios_remove(device: *mut platform_device) -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_resume_handler(device: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_apply_fan_settings(priv: *mut hp_wmi_hwmon_priv) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(GPU_FAN: channel ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_hwmon_keep_alive_handler(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_setup_fan_settings(priv: *mut hp_wmi_hwmon_priv) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_hwmon_init() -> c_int {
    static int hp_wmi_hwmon_init(void)
    {
...
#[no_mangle]
unsafe extern "C" fn setup_active_board_params() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_init() -> int __init {
    static int __init hp_wmi_init(void)
    {
    let mut event_capable: c_int = wmi_has_guid(HPWMI_EVENT_GUID);
    let mut bios_capable: c_int = wmi_has_guid(HPWMI_BIOS_GUID);
    int err, tmp = 0;
    ...
    if (bios_capable) {
    hp_wmi_platform_dev =
    platform_device_register_simple("hp-wmi", PLATFORM_DEVID_NONE, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn hp_wmi_exit() -> void __exit {
