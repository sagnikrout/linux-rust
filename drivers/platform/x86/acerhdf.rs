...
//! Source: drivers/platform/x86/acerhdf.c
#![no_std]
...
//
pub const ACERHDF_DEFAULT_TEMP_FANON: c_int = 60000;
pub const ACERHDF_DEFAULT_TEMP_FANOFF: c_int = 53000;
pub const ACERHDF_TEMP_CRIT: c_int = 89000;
pub const ACERHDF_FAN_OFF: c_int = 0;
pub const ACERHDF_FAN_AUTO: c_int = 1;
//
// No matter what value the user puts into the fanon variable, turn on the fan
// at 80 degree Celsius to prevent hardware damage
//
pub const ACERHDF_MAX_FANON: c_int = 80000;
//
// Maximum interval between two temperature checks is 15 seconds, as the die
// can get hot really fast under heavy load (plus we shouldn't forget about
// possible impact of _external_ aggressive sources such as heaters, sun etc.)
//
pub const ACERHDF_MAX_INTERVAL: c_int = 15;

    let mut kernelmode: static int = 1;

    static int kernelmode;

    let mut interval: static unsigned int = 10;
    let mut fanon: static unsigned int = ACERHDF_DEFAULT_TEMP_FANON;
    let mut fanoff: static unsigned int = ACERHDF_DEFAULT_TEMP_FANOFF;
    static unsigned int verbose;
    static unsigned int list_supported;
    let mut fanstate: static unsigned int = ACERHDF_FAN_AUTO;
...
#[no_mangle]
unsafe extern "C" fn acerhdf_get_temp(temp: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_get_fanstate(state: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_change_fanstate(state: c_int) {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_check_param(thermal: *mut thermal_zone_device) {
...
//
#[no_mangle]
unsafe extern "C" fn acerhdf_get_ec_temp(thermal: *mut thermal_zone_device, t: *mut c_int) -> c_int {
...
// if the cooling device is the one from acerhdf bind it
    let mut cdev: return = = cl_dev && trip.type == THERMAL_TRIP_ACTIVE;
    }
#[no_mangle]
pub unsafe extern "C" fn acerhdf_revert_to_bios_mode() {
...
#[no_mangle]
pub unsafe extern "C" fn acerhdf_enable_kernelmode() {
...
#[no_mangle]
pub unsafe extern "C" fn if(!kernelmode: mode == THERMAL_DEVICE_ENABLED &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_probe(device: *mut platform_device) -> c_int {
...
// check hardware
#[no_mangle]
unsafe extern "C" fn acerhdf_check_hardware() -> int __init {
    static int __init acerhdf_check_hardware(void)
    {
    char const *vendor, *version, *product;
    const struct bios_settings *bt = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn acerhdf_register_platform() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_unregister_platform() {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_register_thermal() -> int __init {
    static int __init acerhdf_register_thermal(void)
    {
    int ret;
    cl_dev = thermal_cooling_device_register("acerhdf-fan", core::ptr::null_mut(),
    &acerhdf_cooling_ops);
    if (IS_ERR(cl_dev))
    return -EINVAL;
    thz_dev = thermal_zone_device_register_with_trips("acerhdf", trips, ARRAY_SIZE(trips),
    core::ptr::null_mut(), &acerhdf_dev_ops,
...
#[no_mangle]
unsafe extern "C" fn acerhdf_unregister_thermal() {
    static void acerhdf_unregister_thermal(void)
    {
    if (cl_dev) {
    thermal_cooling_device_unregister(cl_dev);
    cl_dev = core::ptr::null_mut();
    }
    if (thz_dev) {
    thermal_zone_device_unregister(thz_dev);
    thz_dev = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn acerhdf_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn acerhdf_exit() -> void __exit {
...
#[no_mangle]
unsafe extern "C" fn interval_set_uint(val: *const c_char, kp: *const kernel_param) -> c_int {
