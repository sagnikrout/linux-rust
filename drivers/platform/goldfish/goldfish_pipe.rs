...
//! Source: drivers/platform/goldfish/goldfish_pipe.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct goldfish_pipe_command {
    pub /: *mut *mut s32 cmd; / PipeCmdCode, guest -> host,
    pub /: *mut *mut s32 id; / pipe id, guest -> host,
    pub /: *mut *mut s32 status; / command execution status, host -> guest,
    pub /: *mut *mut s32 reserved; / to pad to 64-bit boundary,
    union {
...
// number of consumed bytes, host -> guest
    pub consumed_size: i32,
// buffer pointers, guest -> host
    pub ptrs: [u64; MAX_BUFFERS_PER_COMMAND],
// buffer sizes, guest -> host
    pub sizes: [u32; MAX_BUFFERS_PER_COMMAND],
    pub rw_params: },
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct signalled_pipe_buffer {
    pub id: u32,
    pub flags: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct open_command_param {
    pub command_buffer_ptr: u64,
    pub rw_params_max_count: u32,
}

// Device-level set of buffers shared with the host
#[repr(C)]
#[derive(Copy, Clone)]
pub struct goldfish_pipe_dev_buffers {
    pub open_command_params: open_command_param,
...
//
    pub signalled_flags: c_ulong,
// A pointer to command buffer
    pub command_buffer: *mut goldfish_pipe_command,
// doubly linked list of signalled pipes, protected by
// goldfish_pipe_dev::lock
//
    pub prev_signalled: *mut goldfish_pipe,
    pub next_signalled: *mut goldfish_pipe,
//
...
// A wake queue for sleeping until host signals an event
    pub wake_queue: wait_queue_head_t,
// Pointer to the parent goldfish_pipe_dev instance
    pub dev: *mut goldfish_pipe_dev,
// A buffer of pages, too large to fit into a stack frame
    pub pages: [*mut page; MAX_BUFFERS_PER_COMMAND],
}
...
//
    pub pipes: *mut goldfish_pipe,
    pub pipes_capacity: u32,
// Pointers to the buffers host uses for interaction with this driver
    pub buffers: *mut goldfish_pipe_dev_buffers,
// Head of a doubly linked list of signalled pipes
    pub first_signalled_pipe: *mut goldfish_pipe,
...
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_cmd(pipe: *mut goldfish_pipe, cmd: enum PipeCmdCode) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_error_convert(status: c_int) -> c_int {
    ...
    {
    int ret;
    let mut requested_pages: c_int = ((last_page - first_page) >> PAGE_SHIFT) + 1;
...
//
    let mut xaddr: c_ulong = page_to_phys(pages[0]);
    let mut xaddr_prev: c_ulong = xaddr;
    let mut buffer_idx: c_int = 0;
    let mut i: c_int = 1;
    ...
    int *status)
    {
    let mut first_page: c_ulong = address & PAGE_MASK;
    unsigned int iter_last_page_size;
    int pages_count;
...
#[no_mangle]
unsafe extern "C" fn wait_for_host_signal(pipe: *mut goldfish_pipe, is_write: c_int) -> c_int {
    static int wait_for_host_signal(struct goldfish_pipe *pipe, int is_write)
    {
    let mut wake_bit: u32 = is_write ? BIT_WAKE_ON_WRITE : BIT_WAKE_ON_READ;
...
// Tell the emulator we're going to wait for a wake event
    ...
    let mut count: c_int = 0, ret = -EINVAL;
...
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_poll(filp: *mut file, wait: *mut poll_table) -> __poll_t {
    ...
    pipe.prev_signalled = core::ptr::null_mut();
    pipe.next_signalled = core::ptr::null_mut();
    }
...
//
    dev.first_signalled_pipe = pipe.next_signalled;
    if (dev.first_signalled_pipe)
    dev.first_signalled_pipe.prev_signalled = core::ptr::null_mut();
    pipe.next_signalled = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn goldfish_interrupt_task(irq: c_int, dev_addr: *mut c_void) -> irqreturn_t {
...
// Iterate over the signalled pipes and wake them one by one
    ...
    struct goldfish_pipe *pipe;
    int wakes;
    while ((pipe = signalled_pipes_pop_front(dev, &wakes)) != core::ptr::null_mut()) {
...
//
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_interrupt(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn get_free_pipe_id_locked(dev: *mut goldfish_pipe_dev) -> c_int {
...
//
    let mut new_capacity: u32 = 2 * dev.pipes_capacity;
...
//
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_open(inode: *mut inode, file: *mut file) -> c_int {
...
// All is done, save the pipe into the file's private data field
    ...
    err_cmd:
    spin_lock_irqsave(&dev.lock, flags);
    dev.pipes[id] = core::ptr::null_mut();
    err_id_locked:
    spin_unlock_irqrestore(&dev.lock, flags);
...
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_release(inode: *mut inode, filp: *mut file) -> c_int {
...
// The guest is closing the channel, so tell the emulator right now
    ...
    spin_lock_irqsave(&dev.lock, flags);
    dev.pipes[pipe.id] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn init_miscdevice(miscdev: *mut miscdevice) {
...
#[no_mangle]
unsafe extern "C" fn write_pa_addr(addr: *mut c_void, portl: *mut void __iomem, porth: *mut void __iomem) {
    static void write_pa_addr(void *addr, void __iomem *portl, void __iomem *porth)
    {
    let mut paddr: c_ulong = __pa(addr);
    ...
    dev.first_signalled_pipe = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn goldfish_pipe_remove(pdev: *mut platform_device) {
