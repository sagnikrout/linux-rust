...
//! Source: drivers/phy/mediatek/phy-mtk-xfi-tphy.c
#![no_std]
...
pub const MTK_XFI_TPHY_NUM_CLOCKS: c_int = 2;
pub const REG_DIG_GLB_70: c_uint = 0x0070;

pub const REG_DIG_GLB_F4: c_uint = 0x00f4;

pub const REG_DIG_LN_TRX_40: c_uint = 0x3040;

pub const REG_DIG_LN_TRX_B0: c_uint = 0x30b0;

pub const REG_ANA_GLB_D0: c_uint = 0x90d0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mtk_xfi_tphy {
    pub base: *mut void __iomem,
    pub dev: *mut device,
    pub reset: *mut reset_control,
    pub clocks: [clk_bulk_data; MTK_XFI_TPHY_NUM_CLOCKS],
    pub da_war: bool,
}
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_5g) -> else {
    else if (is_5g)
    writel(0x00a132a1, xfi_tphy.base + 0x00f8);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_2p5g) -> else {
    else if (is_2p5g)
...
//
#[no_mangle]
unsafe extern "C" fn mtk_xfi_tphy_reset(phy: *mut phy) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mtk_xfi_tphy_power_on(phy: *mut phy) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mtk_xfi_tphy_power_off(phy: *mut phy) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn mtk_xfi_tphy_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (ret)
    return ret;
    xfi_tphy.reset = devm_reset_control_get_exclusive(&pdev.dev, core::ptr::null_mut());
    ...
    phy = devm_phy_create(&pdev.dev, core::ptr::null_mut(), &mtk_xfi_tphy_ops);
    if (IS_ERR(phy))
    return PTR_ERR(phy);
