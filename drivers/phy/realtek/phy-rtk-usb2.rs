...
//! Source: drivers/phy/realtek/phy-rtk-usb2.c
#![no_std]
...
pub const PHY_VCTRL_SHIFT: c_int = 8;
pub const PHY_REG_DATA_MASK: c_uint = 0xff;

pub const EFUS_USB_DC_CAL_RATE: c_int = 2;
pub const EFUS_USB_DC_CAL_MAX: c_int = 7;
pub const EFUS_USB_DC_DIS_RATE: c_int = 1;
pub const EFUS_USB_DC_DIS_MAX: c_int = 7;
pub const MAX_PHY_DATA_SIZE: c_int = 20;
pub const OFFEST_PHY_READ: c_uint = 0x20;
pub const MAX_USB_PHY_NUM: c_int = 4;
pub const MAX_USB_PHY_PAGE0_DATA_SIZE: c_int = 16;
pub const MAX_USB_PHY_PAGE1_DATA_SIZE: c_int = 16;
pub const MAX_USB_PHY_PAGE2_DATA_SIZE: c_int = 8;
pub const SET_PAGE_OFFSET: c_uint = 0xf4;
pub const SET_PAGE_0: c_uint = 0x9b;
pub const SET_PAGE_1: c_uint = 0xbb;
pub const SET_PAGE_2: c_uint = 0xdb;
pub const PAGE_START: c_uint = 0xe0;
pub const PAGE0_0XE4: c_uint = 0xe4;
pub const PAGE0_0XE6: c_uint = 0xe6;
pub const PAGE0_0XE7: c_uint = 0xe7;
pub const PAGE1_0XE0: c_uint = 0xe0;
pub const PAGE1_0XE2: c_uint = 0xe2;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct phy_reg {
    pub reg_wrap_vstatus: *mut void __iomem,
    pub reg_gusb2phyacc0: *mut void __iomem,
    pub vstatus_index: c_int,
}
...
#[derive(Copy, Clone)]
pub struct phy_cfg {
    pub page0_size: c_int,
    pub page0: [phy_data; MAX_USB_PHY_PAGE0_DATA_SIZE],
    pub page1_size: c_int,
    pub page1: [phy_data; MAX_USB_PHY_PAGE1_DATA_SIZE],
    pub page2_size: c_int,
    pub page2: [phy_data; MAX_USB_PHY_PAGE2_DATA_SIZE],
    pub num_phy: c_int,
    pub check_efuse: bool,
    pub check_efuse_version: c_int,
pub const CHECK_EFUSE_V1: c_int = 1;
pub const CHECK_EFUSE_V2: c_int = 2;
    pub efuse_dc_driving_rate: c_int,
    pub efuse_dc_disconnect_rate: c_int,
    pub dc_driving_mask: c_int,
    pub dc_disconnect_mask: c_int,
    pub usb_dc_disconnect_at_page0: bool,
    pub driving_updated_for_dev_dis: c_int,
    pub do_toggle: bool,
    pub do_toggle_driving: bool,
    pub use_default_parameter: bool,
    pub is_double_sensitivity_mode: bool,
}
...
pub struct phy_parameter {
    pub phy_reg: phy_reg,
// Get from efuse
    pub efuse_usb_dc_cal: i8,
    pub efuse_usb_dc_dis: i8,
// Get from dts
    pub inverse_hstx_sync_clock: bool,
    pub driving_level: u32,
    pub driving_level_compensate: i32,
    pub disconnection_compensate: i32,
}
...
#[no_mangle]
pub unsafe extern "C" fn page_addr_to_array_index(addr: u8) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn array_index_to_page_addr(index: c_int) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn rtk_phy_read(phy_reg: *mut phy_reg, addr: c_char) -> c_char {
...
#[no_mangle]
unsafe extern "C" fn rtk_phy_write(phy_reg: *mut phy_reg, addr: c_char, data: c_char) -> c_int {
    ...
    void __iomem *reg_gusb2phyacc0 = phy_reg.reg_gusb2phyacc0;
    let mut shift_bits: c_int = phy_reg.vstatus_index * 8;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn rtk_phy_set_page(phy_reg: *mut phy_reg, page: c_int) -> c_int {
    ...
    u8 ret;
    s32 val;
    let mut dc_disconnect_mask: i32 = phy_cfg.dc_disconnect_mask;
...
// Set page 1
    ...
    struct phy_parameter *phy_parameter, u8 data)
    {
    let mut driving_level_compensate: i32 = phy_parameter.driving_level_compensate;
    let mut dc_driving_mask: i32 = phy_cfg.dc_driving_mask;
    s32 val;
    u8 ret;
    if (phy_cfg.check_efuse_version == CHECK_EFUSE_V1) {
...
#[no_mangle]
unsafe extern "C" fn rtk_usb2_parameter_show(s: *mut seq_file, unused: *mut c_void) -> c_int {
...
// Set page 0
    ...
    for (i = 0; i < phy_cfg.page0_size; i++) {
    struct phy_data *phy_data = phy_data_page + i;
    let mut addr: u8 = array_index_to_page_addr(i);
    let mut data: u8 = phy_data.data;
    let mut value: u8 = rtk_phy_read(phy_reg, addr);
    if (phy_data.addr)
...
// Read efuse for usb dc cal
    ...
    if (phy_cfg.check_efuse_version == CHECK_EFUSE_V1) {
    let mut rate: c_int = phy_cfg.efuse_dc_driving_rate;
...
// Read efuse for usb dc disconnect level
    ...
    if (phy_cfg.check_efuse_version == CHECK_EFUSE_V1) {
    let mut rate: c_int = phy_cfg.efuse_dc_disconnect_rate;
...
#[no_mangle]
unsafe extern "C" fn rtk_usb2phy_probe(pdev: *mut platform_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rtk_usb2phy_remove(pdev: *mut platform_device) {
