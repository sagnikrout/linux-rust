...
//! Source: drivers/phy/broadcom/phy-brcm-usb.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct match_chip_info {
    pub params): *mut *mut void (init_func)(struct brcm_usb_init_params,
    pub 1]: u8 required_regs[BRCM_REGS_MAX +,
    pub optional_reg: u8,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct brcm_usb_phy_data {
    pub ini: brcm_usb_init_params,
    pub has_eohci: bool,
    pub has_xhci: bool,
    pub usb_20_clk: *mut clk,
    pub usb_30_clk: *mut clk,
    pub suspend_clk: *mut clk,
    pub /: *mut *mut mutex mutex; / serialize phy init,
    pub init_count: c_int,
    pub wake_irq: c_int,
    pub phys: [brcm_usb_phy; BRCM_USB_PHY_ID_MAX],
...
#[no_mangle]
unsafe extern "C" fn brcm_usb_phy_wake_isr(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn brcm_usb_phy_init(gphy: *mut phy) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(BRCM_USB_PHY_3_0: phy->id ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn brcm_usb_phy_exit(gphy: *mut phy) -> c_int {
...
// XHCI_EC registers are optional
    ...
    struct phy *gphy = core::ptr::null_mut();
    int err;
    ...
    priv.usb_20_clk = core::ptr::null_mut();
    }
    err = clk_prepare_enable(priv.usb_20_clk);
    if (err)
    return err;
    if (priv.has_eohci) {
    gphy = devm_phy_create(dev, core::ptr::null_mut(), &brcm_usb_phy_ops);
    if (IS_ERR(gphy)) {
    ...
    priv.usb_30_clk = core::ptr::null_mut();
    }
    ...
    priv.suspend_clk = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn brcm_usb_phy_probe(pdev: *mut platform_device) -> c_int {
...
//
    ...
    brcm_usb_phy_attrs[1] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn brcm_usb_phy_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn brcm_usb_phy_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn brcm_usb_phy_resume(dev: *mut device) -> c_int {
