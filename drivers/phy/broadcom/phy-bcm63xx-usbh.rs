...
//! Source: drivers/phy/broadcom/phy-bcm63xx-usbh.c
#![no_std]
...
// USBH control register offsets
    ...
    USBH_PLL_CONTROL1,
pub const USBH_PLLC_REFCLKSEL_SHIFT: c_int = 0;

pub const USBH_PLLC_CLKSEL_SHIFT: c_int = 2;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct bcm63xx_usbh_phy_variant {
// Registers
    pub regs: [c_long; __USBH_ENUM_SIZE],
// PLLC bits to set/clear for power on
    pub power_pllc_clr: u32,
    pub power_pllc_set: u32,
// Setup bits to set/clear for power on
    pub setup_clr: u32,
    pub setup_set: u32,
// Swap Control bits to set
    pub swapctl_dev_set: u32,
// Test Port Control value to set if non-zero
    pub tpc_val: u32,
// USB Sim Control bits to set
    pub usc_set: u32,
// UTMI Control 1 bits to set
    pub utmictl1_dev_set: u32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct bcm63xx_usbh_phy {
    pub base: *mut void __iomem,
    pub usbh_clk: *mut clk,
    pub usb_ref_clk: *mut clk,
    pub reset: *mut reset_control,
    pub variant: *const bcm63xx_usbh_phy_variant,
...
#[no_mangle]
pub unsafe extern "C" fn usbh_has_reg(usbh: *mut bcm63xx_usbh_phy, reg: c_int) -> bool {
    static inline bool usbh_has_reg(struct bcm63xx_usbh_phy *usbh, int reg)
    {
    return (usbh.variant.regs[reg] >= 0);
    }
#[no_mangle]
pub unsafe extern "C" fn usbh_readl(usbh: *mut bcm63xx_usbh_phy, reg: c_int) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn bcm63xx_usbh_phy_init(phy: *mut phy) -> c_int {
...
// Configure to work in native CPU endian
    ...
    let mut val: u32 = usbh_readl(usbh, USBH_SWAP_CONTROL);
    ...
    let mut val: u32 = usbh_readl(usbh, USBH_SETUP);
    ...
    let mut val: u32 = usbh_readl(usbh, USBH_USB_SIM_CONTROL);
    ...
    let mut val: u32 = usbh_readl(usbh, USBH_UTMI_CONTROL1);
...
#[no_mangle]
unsafe extern "C" fn bcm63xx_usbh_phy_power_on(phy: *mut phy) -> c_int {
    static int bcm63xx_usbh_phy_power_on(struct phy *phy)
    {
    struct bcm63xx_usbh_phy *usbh = phy_get_drvdata(phy);
    if (usbh_has_reg(usbh, USBH_PLL_CONTROL1)) {
    let mut val: u32 = usbh_readl(usbh, USBH_PLL_CONTROL1);
...
#[no_mangle]
unsafe extern "C" fn bcm63xx_usbh_phy_power_off(phy: *mut phy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bcm63xx_usbh_phy_exit(phy: *mut phy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bcm63xx_usbh_phy_probe(pdev: *mut platform_device) -> c_int {
    ...
    usbh.reset = devm_reset_control_get_exclusive(dev, core::ptr::null_mut());
    ...
    phy = devm_phy_create(dev, core::ptr::null_mut(), &bcm63xx_usbh_phy_ops);
    if (IS_ERR(phy)) {
