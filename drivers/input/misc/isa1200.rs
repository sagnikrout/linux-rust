...
//! Source: drivers/input/misc/isa1200.c
#![no_std]
...
//
pub const ISA1200_SCTRL: c_uint = 0x00;
pub const ISA1200_LDO_VOLTAGE_BASE: c_uint = 0x08;
pub const ISA1200_LDO_VOLTAGE_STEP: c_int = 100000;
pub const ISA1200_LDO_VOLTAGE_2V3: c_int = 23;
pub const ISA1200_LDO_VOLTAGE_3V1: c_int = 31;
pub const ISA1200_LDO_VOLTAGE_MIN: c_int = 2300000;
pub const ISA1200_LDO_VOLTAGE_MAX: c_int = 3800000;
//
...
// the clock input.
//
pub const ISA1200_HCTRL0: c_uint = 0x30;

pub const ISA1200_HCTRL0_CLKDIV_128: c_int = 128;
//
// HCTRL1 configures the motor type and clock sourse
//
pub const ISA1200_HCTRL1: c_uint = 0x31;

// HCTRL2 controls software reset of the chip
pub const ISA1200_HCTRL2: c_uint = 0x32;
...
//
pub const ISA1200_HCTRL3: c_uint = 0x33;
pub const ISA1200_HCTRL3_DEFAULT: c_uint = 0x03;

// HCTRL4 controls the PWM frequency of external channel
pub const ISA1200_HCTRL4: c_uint = 0x34;
// HCTRL5 controls the PWM high duty cycle of internal channel
pub const ISA1200_HCTRL5: c_uint = 0x35;
// HCTRL6 controls the PWM period of internal channel
pub const ISA1200_HCTRL6: c_uint = 0x36;
pub const ISA1200_HCTRL6_PERIOD_SCALE: c_int = 100;
// The use for these registers is unknown but they exist
pub const ISA1200_HCTRL7: c_uint = 0x37;
pub const ISA1200_HCTRL8: c_uint = 0x38;
pub const ISA1200_HCTRL9: c_uint = 0x39;
pub const ISA1200_HCTRLA: c_uint = 0x3a;
pub const ISA1200_HCTRLB: c_uint = 0x3b;
pub const ISA1200_HCTRLC: c_uint = 0x3c;
pub const ISA1200_HCTRLD: c_uint = 0x3d;
pub const ISA1200_EN_PINS_MAX: c_int = 2;
    static const struct regulator_bulk_data isa1200_supplies[] = {
    { .supply = "vdd" }, { .supply = "vddp" },
    };
#[repr(C)]
#[derive(Copy, Clone)]
pub struct isa1200_config {
    pub ldo_voltage: u32,
    pub mode: u32,
    pub clkdiv: u32,
    pub plldiv: u32,
...
#[no_mangle]
unsafe extern "C" fn isa1200_stop(isa: *mut isa1200) {
...
#[no_mangle]
unsafe extern "C" fn isa1200_play_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn isa1200_vibrator_close(input: *mut input_dev) {
...
#[no_mangle]
unsafe extern "C" fn isa1200_of_probe(client: *mut i2c_client) -> c_int {
    ...
    int err;
    isa.clk = devm_clk_get_optional(dev, core::ptr::null_mut());
    if (IS_ERR(isa.clk))
    return dev_err_probe(dev, PTR_ERR(isa.clk),
    "failed to get clock\n");
    isa.pwm = devm_pwm_get(dev, core::ptr::null_mut());
    if (IS_ERR(isa.pwm)) {
    err = PTR_ERR(isa.pwm);
    if (err == -ENODEV || err == -EINVAL)
    isa.pwm = core::ptr::null_mut();
    else
...
#[no_mangle]
unsafe extern "C" fn isa1200_probe(client: *mut i2c_client) -> c_int {
...
// TODO: this hardware can likely support more than rumble
    ...
    err = input_ff_create_memless(isa.input, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn isa1200_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn isa1200_resume(dev: *mut device) -> c_int {
