...
//! Source: drivers/input/misc/soc_button_array.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct soc_button_info {
    pub name: *const c_char,
    pub acpi_index: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct soc_device_data {
    pub button_info: *const soc_button_info,
    pub dev): *mut *mut int (check)(struct device,
}
...
//
pub const BUTTON_TYPES: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct soc_button_data {
    pub children: [*mut platform_device; BUTTON_TYPES],
}
...
//
    ...
    desc = gpiod_get_index(dev, core::ptr::null_mut(), acpi_index, GPIOD_ASIS);
    if (IS_ERR(desc))
    return PTR_ERR(desc);
...
// irq_ret = gpiod_to_irq(desc);
    ...
    let mut invalid_acpi_index: c_int = -1;
    int error, gpio, irq;
    let mut n_buttons: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn soc_button_get_acpi_object_int(obj: *const union acpi_object) -> c_int {
...
// Parse ACPI0011 _DSD button descriptors
    ...
    const union acpi_object *desc, *el0, *uuid, *btns_desc = core::ptr::null_mut();
    struct soc_button_info *button_info;
    acpi_status status;
    int i, btn, collection_uid = -1;
    status = acpi_evaluate_object_typed(ACPI_HANDLE(dev), "_DSD", core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn soc_button_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn soc_button_probe(pdev: *mut platform_device) -> c_int {
    ...
    error = gpiod_count(dev, core::ptr::null_mut());
    if (error < 0) {
...
pub const MSHW0040_DSM_GET_OMPR: c_uint = 0x02	// get OEM Platform Revision;
    ...
    0x49, 0x80, 0x35);
#[no_mangle]
unsafe extern "C" fn soc_device_check_MSHW0040(dev: *mut device) -> c_int {
...
// get OEM platform revision
    ...
    MSHW0040_DSM_GET_OMPR, core::ptr::null_mut(),
    ACPI_TYPE_INTEGER);
    if (result) {
