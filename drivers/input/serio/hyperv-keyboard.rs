...
//! Source: drivers/input/serio/hyperv-keyboard.c
#![no_std]
...
// Current version 1.0
//
pub const SYNTH_KBD_VERSION_MAJOR: c_int = 1;
pub const SYNTH_KBD_VERSION_MINOR: c_int = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct synth_kbd_msg_hdr {
    pub type: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct synth_kbd_msg {
    pub header: synth_kbd_msg_hdr,
    pub /: *mut *mut char data[]; / Enclosed message,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct synth_kbd_protocol_request {
    pub header: synth_kbd_msg_hdr,
    pub version_requested: union synth_kbd_version,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct synth_kbd_protocol_response {
    pub header: synth_kbd_msg_hdr,
    pub proto_status: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct synth_kbd_keystroke {
    pub header: synth_kbd_msg_hdr,
    pub make_code: __le16,
    pub reserved0: __le16,
    pub /: *mut *mut __le32 info; / Additional information,
}

pub const HK_MAXIMUM_MESSAGE_SIZE: c_int = 256;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hv_kbd_dev {
    pub hv_dev: *mut hv_device,
    pub hv_serio: *mut serio,
    pub protocol_req: synth_kbd_protocol_request,
    pub protocol_resp: synth_kbd_protocol_response,
// Synchronize the request/response if needed
    pub wait_event: completion,
    pub /: *mut *mut spinlock_t lock; / protects 'started' field,
    pub started: bool,
}
    ...
    let mut msg_type: u32 = __le32_to_cpu(msg.header.type);
    u32 info;
    u16 scan_code;
    switch (msg_type) {
...
#[no_mangle]
unsafe extern "C" fn hv_kbd_on_channel_callback(context: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn hv_kbd_connect_to_vsp(hv_dev: *mut hv_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hv_kbd_start(serio: *mut serio) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hv_kbd_stop(serio: *mut serio) {
...
#[no_mangle]
unsafe extern "C" fn hv_kbd_remove(hv_dev: *mut hv_device) {
    ...
    vmbus_close(hv_dev.channel);
    kfree(kbd_dev);
    hv_set_drvdata(hv_dev, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn hv_kbd_suspend(hv_dev: *mut hv_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hv_kbd_resume(hv_dev: *mut hv_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hv_kbd_init() -> int __init {
    static int __init hv_kbd_init(void)
    {
    return vmbus_driver_register(&hv_kbd_drv);
    }
#[no_mangle]
unsafe extern "C" fn hv_kbd_exit() -> void __exit {
