...
//! Source: drivers/input/keyboard/pinephone-keyboard.c
#![no_std]
...
pub const PPKB_CRC8_POLYNOMIAL: c_uint = 0x07;
pub const PPKB_DEVICE_ID_HI: c_uint = 0x00;

pub const PPKB_DEVICE_ID_LO: c_uint = 0x01;

pub const PPKB_FW_REVISION: c_uint = 0x02;
pub const PPKB_FW_FEATURES: c_uint = 0x03;
pub const PPKB_MATRIX_SIZE: c_uint = 0x06;
pub const PPKB_SCAN_CRC: c_uint = 0x07;
pub const PPKB_SCAN_DATA: c_uint = 0x08;
pub const PPKB_SYS_CONFIG: c_uint = 0x20;

pub const PPKB_SYS_SMBUS_COMMAND: c_uint = 0x21;
pub const PPKB_SYS_SMBUS_DATA: c_uint = 0x22;
pub const PPKB_SYS_COMMAND: c_uint = 0x23;
pub const PPKB_SYS_COMMAND_SMBUS_READ: c_uint = 0x91;
pub const PPKB_SYS_COMMAND_SMBUS_WRITE: c_uint = 0xa1;
pub const PPKB_ROWS: c_int = 6;
pub const PPKB_COLS: c_int = 12;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pinephone_keyboard {
    pub adapter: i2c_adapter,
    pub input: *mut input_dev,
    pub buf: [u8; 2][PPKB_BUF_LEN],
    pub crc_table: [u8; CRC8_TABLE_SIZE],
    pub fn_state: [u8; PPKB_COLS],
    pub buf_swap: bool,
    pub fn_pressed: bool,
}
...
#[no_mangle]
unsafe extern "C" fn ppkg_adap_functionality(adap: *mut i2c_adapter) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn ppkb_update(client: *mut i2c_client) {
    ...
    let mut row_shift: c_int = get_count_order(PPKB_COLS);
    ...
    let mut old: u8 = old_buf[1 + col];
    let mut new: u8 = new_buf[1 + col];
    let mut changed: u8 = old ^ new;
    if (!changed)
    continue;
    for (row = 0; row < PPKB_ROWS; ++row) {
    let mut mask: u8 = BIT(row);
    let mut value: u8 = new & mask;
    unsigned short code;
    bool fn_state;
    if (!(changed & mask))
    continue;
...
#[no_mangle]
unsafe extern "C" fn ppkb_irq_thread(irq: c_int, data: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
unsafe extern "C" fn ppkb_set_scan(client: *mut i2c_client, enable: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ppkb_open(input: *mut input_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ppkb_close(input: *mut input_dev) {
...
#[no_mangle]
unsafe extern "C" fn ppkb_probe(client: *mut i2c_client) -> c_int {
...
// Disable scan by default to save power.
    ...
    error = matrix_keypad_build_keymap(&ppkb_keymap_data, core::ptr::null_mut(),
    2 * PPKB_ROWS, PPKB_COLS, core::ptr::null_mut(),
    ppkb.input);
    if (error) {
    ...
    core::ptr::null_mut(), ppkb_irq_thread,
