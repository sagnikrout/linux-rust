...
//! Source: drivers/input/keyboard/cros_ec_keyb.c
#![no_std]
...
//
pub const CROS_EC_KEYBOARD_COLS_MAX: c_int = 18;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cros_ec_keyb {
    pub rows: c_uint,
    pub cols: c_uint,
    pub row_shift: c_int,
    pub ghost_filter: bool,
    pub valid_keys: [u8; CROS_EC_KEYBOARD_COLS_MAX],
    pub old_kb_state: [u8; CROS_EC_KEYBOARD_COLS_MAX],
    pub dev: *mut device,
    pub ec: *mut cros_ec_device,
    pub idev: *mut input_dev,
    pub bs_idev: *mut input_dev,
    pub notifier: notifier_block,
    pub vdata: vivaldi_data,
    pub has_fn_map: bool,
    pub fn_active: bool,
    pub fn_combo_active: bool,
}
...
//
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_has_ghosting(ckdev: *mut cros_ec_keyb, buf: *mut u8) -> bool {
...
#[no_mangle]
unsafe extern "C" fn cros_ec_emit_fn_key(input: *mut input_dev, pos: c_uint) {
    ...
    let mut pos: c_int = MATRIX_SCAN_CODE(row, col, ckdev.row_shift);
...
//
    ...
    if (changed & BIT(row)) {
    let mut key_state: u8 = col_state & BIT(row);
...
//
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_process(ckdev: *mut cros_ec_keyb, kb_state: *mut u8, len: c_int) {
...
//
    ...
    let mut changed: u8 = kb_state[col] ^ ckdev.old_kb_state[col];
    if (changed)
...
//
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_compute_valid_keys(ckdev: *mut cros_ec_keyb) {
    static void cros_ec_keyb_compute_valid_keys(struct cros_ec_keyb *ckdev)
    {
    int row, col;
    let mut row_shift: c_int = ckdev.row_shift;
...
//
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_query_switches(ckdev: *mut cros_ec_keyb) -> c_int {
    static int cros_ec_keyb_query_switches(struct cros_ec_keyb *ckdev)
    {
    struct cros_ec_device *ec_dev = ckdev.ec;
    let mut event_data: union ec_response_get_next_data = {};
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_parse_vivaldi_physmap(ckdev: *mut cros_ec_keyb) {
...
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_has_fn_key(ckdev: *mut cros_ec_keyb) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_has_fn_map(ckdev: *mut cros_ec_keyb) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_register_matrix(ckdev: *mut cros_ec_keyb) -> c_int {
...
//
    ...
    err = matrix_keypad_build_keymap(core::ptr::null_mut(), core::ptr::null_mut(), ckdev.rows * 2, ckdev.cols,
    core::ptr::null_mut(), idev);
    if (err) {
...
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_probe(pdev: *mut platform_device) -> c_int {
    static int cros_ec_keyb_probe(struct platform_device *pdev)
    {
    struct cros_ec_device *ec;
    struct device *dev = &pdev.dev;
    struct cros_ec_keyb *ckdev;
    let mut buttons_switches_only: bool = device_get_match_data(dev);
    int err;
//
...
#[no_mangle]
unsafe extern "C" fn cros_ec_keyb_remove(pdev: *mut platform_device) {
    ...
    static DEFINE_SIMPLE_DEV_PM_OPS(cros_ec_keyb_pm_ops, core::ptr::null_mut(), cros_ec_keyb_resume);
