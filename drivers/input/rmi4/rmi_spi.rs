...
//! Source: drivers/input/rmi4/rmi_spi.c
#![no_std]
...
pub const RMI_SPI_DEFAULT_XFER_BUF_SIZE: c_int = 64;
pub const RMI_PAGE_SELECT_REGISTER: c_uint = 0x00FF;

pub const RMI_SPI_XFER_SIZE_LIMIT: c_int = 255;
pub const BUFFER_SIZE_INCREMENT: c_int = 32;
    enum rmi_spi_op {
    RMI_SPI_WRITE = 0,
    RMI_SPI_READ,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct rmi_spi_cmd {
    pub op: enum rmi_spi_op,
    pub addr: u16,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct rmi_spi_xport {
    ...
    pub rx_buf: *mut u8,
    pub tx_buf: *mut u8,
    pub xfer_buf_size: c_int,
    pub rx_xfers: *mut spi_transfer,
    pub tx_xfers: *mut spi_transfer,
    pub rx_xfer_count: c_int,
    pub tx_xfer_count: c_int,
}

#[no_mangle]
unsafe extern "C" fn rmi_spi_manage_pools(rmi_spi: *mut rmi_spi_xport, len: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rmi_set_page(rmi_spi: *mut rmi_spi_xport, page: u8) -> c_int {
    ...
    ret = rmi_spi_xfer(rmi_spi, &cmd, &page, 1, core::ptr::null_mut(), 0);
    if (ret)
    rmi_spi.page = page;
    return ret;
    }
    ...
    cmd.op = RMI_SPI_WRITE;
    cmd.addr = addr;
    ret = rmi_spi_xfer(rmi_spi, &cmd, buf, len, core::ptr::null_mut(), 0);
    exit:
    ...
    cmd.op = RMI_SPI_READ;
    cmd.addr = addr;
    ret = rmi_spi_xfer(rmi_spi, &cmd, core::ptr::null_mut(), 0, buf, len);
    exit:
    mutex_unlock(&rmi_spi.page_mutex);
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn rmi_spi_unregister_transport(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn rmi_spi_probe(spi: *mut spi_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rmi_spi_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rmi_spi_resume(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rmi_spi_runtime_suspend(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rmi_spi_runtime_resume(dev: *mut device) -> c_int {
    ...
    RUNTIME_PM_OPS(rmi_spi_runtime_suspend, rmi_spi_runtime_resume, core::ptr::null_mut())
    };
