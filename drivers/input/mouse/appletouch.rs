...
//! Source: drivers/input/mouse/appletouch.c
#![no_std]
...
#[derive(Copy, Clone)]
pub struct atp_info {
    pub /: *mut *mut int xsensors; / number of X sensors,
    pub /: *mut *mut int xsensors_17; / 17" models have more sensors,
    pub /: *mut *mut int ysensors; / number of Y sensors,
    pub /: *mut *mut int xfact; / X multiplication factor,
    pub /: *mut *mut int yfact; / Y multiplication factor,
    pub /: *mut *mut int datalen; / size of USB transfers,
    pub /: *mut *mut *mut *mut void (callback)(struct urb ); / callback function,
    pub /: *mut *mut int fuzz; / fuzz touchpad generates,
}
...
pub const ATP_XSENSORS: c_int = 26;
pub const ATP_YSENSORS: c_int = 16;
//
...
//
pub const ATP_SMOOTHSIZE: c_int = 34;
// maximum pressure this driver will report
pub const ATP_PRESSURE: c_int = 300;
//
// Threshold for the touchpad sensors. Any change less than ATP_THRESHOLD is
// ignored.
//
pub const ATP_THRESHOLD: c_int = 5;
//
// How far we'll bitshift our sensor values before averaging them. Mitigates
// rounding errors.
//
pub const ATP_SCALE: c_int = 12;
// Geyser initialization constants
pub const ATP_GEYSER_MODE_READ_REQUEST_ID: c_int = 1;
pub const ATP_GEYSER_MODE_WRITE_REQUEST_ID: c_int = 9;
pub const ATP_GEYSER_MODE_REQUEST_VALUE: c_uint = 0x300;
pub const ATP_GEYSER_MODE_REQUEST_INDEX: c_int = 0;
pub const ATP_GEYSER_MODE_VENDOR_VALUE: c_uint = 0x04;
//
...
pub struct atp {
    ...
    pub /: *mut *mut *mut u8 data; / transferred data,
    pub /: *mut *mut *mut input_dev input; / input dev,
    pub /: *const *const *const atp_info info; / touchpad model,
    pub /: *mut *mut bool valid; / are the samples valid?,
    pub size_detect_done: bool,
    pub overflow_warned: bool,
    pub /: *mut *mut int fingers_old; / last reported finger count,
    pub /: *mut *mut int x_old; / last reported x/y,,
    pub /: *mut *mut int y_old; / used for smoothing,
    pub ATP_YSENSORS]: signed char xy_cur[ATP_XSENSORS +,
    pub ATP_YSENSORS]: signed char xy_old[ATP_XSENSORS +,
    pub ATP_YSENSORS]: int xy_acc[ATP_XSENSORS +,
    pub smooth: [c_int; ATP_SMOOTHSIZE],
    pub smooth_tmp: [c_int; ATP_SMOOTHSIZE],
    pub /: *mut *mut int idlecount; / number of empty packets,
...
//
    let mut threshold: static int = ATP_THRESHOLD;
...
//
#[no_mangle]
unsafe extern "C" fn atp_geyser_init(dev: *mut atp) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn atp_reinit(work: *mut work_struct) {
...
// values to calculate mean
    let mut pcum: c_int = 0, psum = 0;
    let mut is_increasing: c_int = 0;
// fingers = 0;
...
#[no_mangle]
pub unsafe extern "C" fn atp_report_fingers(input: *mut input_dev, fingers: c_int) {
...
pub const ATP_URB_STATUS_SUCCESS: c_int = 0;
pub const ATP_URB_STATUS_ERROR: c_int = 1;
pub const ATP_URB_STATUS_ERROR_FATAL: c_int = 2;
#[no_mangle]
unsafe extern "C" fn atp_status_check(urb: *mut urb) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atp_detect_size(dev: *mut atp) {
...
#[no_mangle]
unsafe extern "C" fn atp_complete_geyser_1_2(urb: *mut urb) {
    static void atp_complete_geyser_1_2(struct urb *urb)
    {
    ...
    int key, fingers;
    struct atp *dev = urb.context;
    let mut status: c_int = atp_status_check(urb);
    if (status == ATP_URB_STATUS_ERROR_FATAL)
    return;
#[no_mangle]
pub unsafe extern "C" fn if(ATP_URB_STATUS_ERROR: status ==) -> else {
...
// accumulate the change
    let mut change: signed char = dev.xy_old[i] - dev.xy_cur[i];
    dev.xy_acc[i] -= change;
...
#[no_mangle]
unsafe extern "C" fn atp_complete_geyser_3_4(urb: *mut urb) {
    static void atp_complete_geyser_3_4(struct urb *urb)
    {
    int x, y, x_z, y_z, x_f, y_f;
    int retval, i, j;
...
#[no_mangle]
unsafe extern "C" fn atp_open(input: *mut input_dev) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atp_close(input: *mut input_dev) {
...
#[no_mangle]
unsafe extern "C" fn atp_handle_geyser(dev: *mut atp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atp_disconnect(iface: *mut usb_interface) {
...
#[no_mangle]
unsafe extern "C" fn atp_recover(dev: *mut atp) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atp_suspend(iface: *mut usb_interface, message: pm_message_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atp_resume(iface: *mut usb_interface) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn atp_reset_resume(iface: *mut usb_interface) -> c_int {
