...
//! Source: drivers/input/input-mt.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn copy_abs(dev: *mut input_dev, dst: c_uint, src: c_uint) {
...
// Arbitrary limit for avoiding too large memory allocation.
    ...
    let mut n2: c_uint = num_slots * num_slots;
...
//
#[no_mangle]
pub unsafe extern "C" fn input_mt_destroy_slots(dev: *mut input_dev) {
...
//
#[no_mangle]
pub unsafe extern "C" fn input_mt_report_finger_count(dev: *mut input_dev, count: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn input_mt_report_pointer_emulation(dev: *mut input_dev, use_count: bool) {
    ...
    reported_p += p;
#[no_mangle]
pub unsafe extern "C" fn if(id: oldid ==) -> else {
    else if (oldid == id)
    reported_p = p;
    }
    count++;
...
//
    ...
    if (oldest) {
    let mut x: c_int = input_mt_get_value(oldest, ABS_MT_POSITION_X);
    let mut y: c_int = input_mt_get_value(oldest, ABS_MT_POSITION_Y);
...
#[no_mangle]
unsafe extern "C" fn __input_mt_drop_unused(dev: *mut input_dev, mt: *mut input_mt) {
...
//
#[no_mangle]
pub unsafe extern "C" fn input_mt_drop_unused(dev: *mut input_dev) {
...
//
#[no_mangle]
pub unsafe extern "C" fn input_mt_release_slots(dev: *mut input_dev) {
...
//
#[no_mangle]
pub unsafe extern "C" fn input_mt_sync_frame(dev: *mut input_dev) {
    void input_mt_sync_frame(struct input_dev *dev)
    {
    struct input_mt *mt = dev.mt;
    let mut use_count: bool = false;
    if (!mt)
    return;
...
#[no_mangle]
unsafe extern "C" fn adjust_dual(begin: *mut c_int, step: c_int, end: *mut c_int, eq: c_int, mu: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn find_reduced_matrix(w: *mut c_int, nr: c_int, nc: c_int, nrc: c_int, mu: c_int) {
    ...
    let mut dx: c_int = x - p.x, dy = y - p.y;
// w++ = dx * dx + dy * dy - mu;
    }
    }
    return w - mt.red;
...
//
#[no_mangle]
pub unsafe extern "C" fn input_mt_get_slot_by_key(dev: *mut input_dev, key: c_int) -> c_int {
