...
//! Source: drivers/input/touchscreen/goodix_berlin_spi.c
#![no_std]
...
pub const GOODIX_BERLIN_SPI_TRANS_PREFIX_LEN: c_int = 1;
pub const GOODIX_BERLIN_REGISTER_WIDTH: c_int = 4;
pub const GOODIX_BERLIN_SPI_READ_DUMMY_LEN_A: c_int = 4;
pub const GOODIX_BERLIN_SPI_READ_DUMMY_LEN_D: c_int = 3;
    ...
    GOODIX_BERLIN_REGISTER_WIDTH)
pub const GOODIX_BERLIN_SPI_WRITE_FLAG: c_uint = 0xF0;
pub const GOODIX_BERLIN_SPI_READ_FLAG: c_uint = 0xF1;
...
// buffer format: 0xF1 + addr(4bytes) + dummy(3/4bytes) + data
    ...
    size_t count)
    {
    let mut len: c_uint = count - GOODIX_BERLIN_REGISTER_WIDTH;
    struct spi_device *spi = context;
    struct spi_transfer xfers;
    struct spi_message spi_msg;
...
#[no_mangle]
unsafe extern "C" fn goodix_berlin_spi_probe(spi: *mut spi_device) -> c_int {
    ...
    regmap = devm_regmap_init(&spi.dev, core::ptr::null_mut(), spi, &regmap_config);
    if (IS_ERR(regmap))
