...
//! Source: drivers/clk/renesas/r9a09g077-cpg.c
#![no_std]
...
pub const RZT2H_REG_BLOCK_SHIFT: c_int = 11;
...
pub struct pll_clk {
    ...
    pub dev: *mut device,
    pub pll_parameters: rzv2h_pll_pars,
    pub hw: clk_hw,
    pub cur_rate: c_ulong,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct r9a09g077_lcdc_div_clk {
...
pub const RZT2H_MAX_LCDC_DIV_TABLES: c_int = 16;
...
// index 1 is always .pll3 in sel_clk_pll3[]
    ...
    struct r9a09g077_lcdc_div_clk *dsi_div = to_lcdc_div_clk(hw);
    const struct clk_div_table *clkt;
    let mut divider_found: bool = false;
    u32 val, shift;
    ...
    struct cpg_mssr_pub *pub)
    {
    let mut offset: u32 = GET_REG_OFFSET(core.conf);
