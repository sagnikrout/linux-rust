...
//! Source: drivers/clk/at91/sama7d65.c
#![no_std]
...
pub const PMC_INDEX_MAX: c_int = 25;
//
...
#[no_mangle]
unsafe extern "C" fn sama7d65_pmc_setup(np: *mut device_node) -> void __init {
...
// Should not happen.
    ...
    &pmc_pll_lock, sama7d65_plls[i][j].n,
    core::ptr::null_mut(), sama7d65_plls[i][0].hw, i,
    sama7d65_plls[i][j].c,
    sama7d65_plls[i][j].l,
    sama7d65_plls[i][j].f,
    ...
    let mut num_parents: u8 = 3 + sama7d65_mckx[i].ep_count;
    ...
    let mut pll_id: u8 = sama7d65_mckx[i].ep[j].pll_id;
    let mut pll_compid: u8 = sama7d65_mckx[i].ep[j].pll_compid;
    tmp_parent_hws[j] = sama7d65_plls[pll_id][pll_compid].hw;
    }
    PMC_FILL_TABLE(&parent_hws[3], tmp_parent_hws,
    sama7d65_mckx[i].ep_count);
    hw = at91_clk_sama7g5_register_master(regmap, sama7d65_mckx[i].n,
    num_parents, core::ptr::null_mut(), parent_hws,
    ...
    sama7d65_systemck[i].p, core::ptr::null_mut(),
    ...
    let mut num_parents: u8 = 4 + sama7d65_gck[i].pp_count;
    struct clk_hw *tmp_parent_hws[8];
    u32 *mux_table;
    mux_table = kmalloc_array(num_parents, sizeof(*mux_table),
    GFP_KERNEL);
    ...
    let mut pll_id: u8 = sama7d65_gck[i].pp[j].pll_id;
    let mut pll_compid: u8 = sama7d65_gck[i].pp[j].pll_compid;
    tmp_parent_hws[j] = sama7d65_plls[pll_id][pll_compid].hw;
    }
    ...
    &sama7d65_pcr_layout,
    sama7d65_gck[i].n, core::ptr::null_mut(),
