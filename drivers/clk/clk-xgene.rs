...
//! Source: drivers/clk/clk-xgene.c
#![no_std]
...
pub const REGSPEC_RESET_F1_MASK: c_uint = 0x00010000;

    static DEFINE_SPINLOCK(clk_lock);
#[no_mangle]
pub unsafe extern "C" fn xgene_clk_read(csr: *mut void __iomem) -> u32 {
    static inline u32 xgene_clk_read(void __iomem *csr)
    {
    return readl_relaxed(csr);
    }
#[no_mangle]
pub unsafe extern "C" fn xgene_clk_write(data: u32, csr: *mut void __iomem) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xgene_clk_pll {
    pub hw: clk_hw,
    pub reg: *mut void __iomem,
    pub lock: *mut spinlock_t,
    pub pll_offset: u32,
    pub type: enum xgene_pll_type,
    pub version: c_int,
}

#[no_mangle]
unsafe extern "C" fn xgene_clk_pll_is_enabled(hw: *mut clk_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xgene_pllclk_version(np: *mut device_node) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xgene_pllclk_init(np: *mut device_node, pll_type: enum xgene_pll_type) {
    static void xgene_pllclk_init(struct device_node *np, enum xgene_pll_type pll_type)
    {
    const char *clk_name = np.full_name;
    struct clk *clk;
    void __iomem *reg;
    let mut version: c_int = xgene_pllclk_version(np);
    reg = of_iomap(np, 0);
    if (!reg) {
    ...
    clk = xgene_register_clk_pll(core::ptr::null_mut(),
    clk_name, of_clk_get_parent_name(np, 0),
    0, reg, 0, pll_type, &clk_lock,
    version);
    if (!IS_ERR(clk)) {
    of_clk_add_provider(np, of_clk_src_simple_get, clk);
    clk_register_clkdev(clk, clk_name, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn xgene_socpllclk_init(np: *mut device_node) {
    static void xgene_socpllclk_init(struct device_node *np)
    {
    xgene_pllclk_init(np, PLL_TYPE_SOC);
    }
#[no_mangle]
unsafe extern "C" fn xgene_pcppllclk_init(np: *mut device_node) {
...
pub const XGENE_CLK_PMD_SHIFT: c_int = 8;
pub const XGENE_CLK_PMD_WIDTH: c_int = 3;
...
#[no_mangle]
unsafe extern "C" fn xgene_pmdclk_init(np: *mut device_node) {
...
// Parse the DTS register for resource
    ...
    clk = xgene_register_clk_pmd(core::ptr::null_mut(), clk_name,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xgene_dev_parameters {
    pub /: *mut *mut *mut void __iomem csr_reg; / CSR for IP clock,
    pub /: *mut *mut u32 reg_clk_offset; / Offset to clock enable CSR,
    pub /: *mut *mut u32 reg_clk_mask; / Mask bit for clock enable,
    pub /: *mut *mut u32 reg_csr_offset; / Offset to CSR reset,
    pub /: *mut *mut u32 reg_csr_mask; / Mask bit for disable CSR reset,
    pub /: *mut *mut *mut void __iomem divider_reg; / CSR for divider,
    pub /: *mut *mut u32 reg_divider_offset; / Offset to divider register,
    pub /: *mut *mut u32 reg_divider_shift; / Bit shift to divider field,
    pub /: *mut *mut u32 reg_divider_width; / Width of the bit to divider field,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct xgene_clk {
    pub hw: clk_hw,
    pub lock: *mut spinlock_t,
    pub param: xgene_dev_parameters,
}

#[no_mangle]
unsafe extern "C" fn xgene_clk_enable(hw: *mut clk_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xgene_clk_disable(hw: *mut clk_hw) {
...
#[no_mangle]
unsafe extern "C" fn xgene_clk_is_enabled(hw: *mut clk_hw) -> c_int {
...
// Register the clock for lookup
    rc = clk_register_clkdev(clk, name, core::ptr::null_mut());
    if (rc != 0) {
...
#[no_mangle]
unsafe extern "C" fn xgene_devclk_init(np: *mut device_node) -> void __init {
...
// Check if the entry is disabled
    ...
    return;
// Parse the DTS register for resource
    parameters.csr_reg = core::ptr::null_mut();
    parameters.divider_reg = core::ptr::null_mut();
    for (i = 0; i < 2; i++) {
    ...
    of_property_read_string(np, "clock-output-names", &clk_name);
    clk = xgene_register_clk(core::ptr::null_mut(), clk_name,
