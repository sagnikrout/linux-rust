...
//! Source: drivers/clk/zynqmp/clkc.c
#![no_std]
...
pub const MAX_PARENT: c_int = 100;
pub const MAX_NODES: c_int = 6;
pub const MAX_NAME_LEN: c_int = 50;
// Flags for parents
pub const PARENT_CLK_SELF: c_int = 0;
pub const PARENT_CLK_NODE1: c_int = 1;
pub const PARENT_CLK_NODE2: c_int = 2;
pub const PARENT_CLK_NODE3: c_int = 3;
pub const PARENT_CLK_NODE4: c_int = 4;
pub const PARENT_CLK_EXTERNAL: c_int = 5;

pub const END_OF_TOPOLOGY_NODE: c_int = 1;
pub const END_OF_PARENTS: c_int = 1;

pub const CLK_GET_NAME_RESP_LEN: c_int = 16;
pub const CLK_GET_TOPOLOGY_RESP_WORDS: c_int = 3;
pub const CLK_GET_PARENTS_RESP_WORDS: c_int = 3;
pub const CLK_GET_ATTR_RESP_WORDS: c_int = 1;
    enum clk_type {
    CLK_TYPE_OUTPUT,
    CLK_TYPE_EXTERNAL,
    };
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct zynqmp_clock {
    pub clk_name: [c_char; MAX_NAME_LEN],
    pub valid: u32,
    pub type: enum clk_type,
    pub node: [clock_topology; MAX_NODES],
    pub num_nodes: u32,
    pub parent: [clock_parent; MAX_PARENT],
    pub num_parents: u32,
    pub clk_id: u32,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct name_resp {
    pub name: [c_char; CLK_GET_NAME_RESP_LEN],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct topology_resp {
    pub topology: [u32; CLK_GET_TOPOLOGY_RESP_WORDS],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct parents_resp {
pub const NA_PARENT: c_uint = 0xFFFFFFFF;
pub const DUMMY_PARENT: c_uint = 0xFFFFFFFE;
    pub parents: [u32; CLK_GET_PARENTS_RESP_WORDS],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct attr_resp {
    pub attr: [u32; CLK_GET_ATTR_RESP_WORDS],
}
    ...
    = {
    [TYPE_INVALID] = core::ptr::null_mut(),
...
//
#[no_mangle]
pub unsafe extern "C" fn zynqmp_is_valid_clock(clk_id: u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn zynqmp_get_clock_name(clk_id: u32, clk_name: *mut c_char) -> c_int {
    ...
    let mut ret: return = = 0 ? -EINVAL : ret;
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn zynqmp_get_clock_type(clk_id: u32, type: *mut u32) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn zynqmp_pm_clock_get_num_clocks(nclocks: *mut u32) -> c_int {
    static int zynqmp_pm_clock_get_num_clocks(u32 *nclocks)
    {
    let mut qdata: zynqmp_pm_query_data = {0};
...
#[no_mangle]
pub unsafe extern "C" fn zynqmp_clk_map_common_ccf_flags(zynqmp_flag: u32) -> c_ulong {
...
//
    ...
    hw = clk_hw_register_fixed_factor(core::ptr::null_mut(), name,
    parents[0],
    flag, mult,
    div);
    return hw;
...
//
    ...
    {
    int j, ret;
    let mut response: topology_resp = { };
// num_nodes = 0;
...
//
    ...
    u32 *num_parents)
    {
    let mut j: c_int = 0, ret;
    let mut response: parents_resp = { };
// num_parents = 0;
    do {
...
//
    ...
    let mut total_parents: u32 = clock[clk_id].num_parents;
...
//
    ...
    struct clk_hw *hw = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn zynqmp_register_clocks(np: *mut device_node) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn zynqmp_get_clock_info() {
...
//
#[no_mangle]
unsafe extern "C" fn zynqmp_clk_setup(np: *mut device_node) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn zynqmp_clock_probe(pdev: *mut platform_device) -> c_int {
