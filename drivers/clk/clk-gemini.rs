...
//! Source: drivers/clk/clk-gemini.c
#![no_std]
...
pub const GEMINI_GLOBAL_STATUS: c_uint = 0x04;

pub const AHBSPEED_MASK: c_uint = 0x07;

pub const CPU_AHB_RATIO_MASK: c_uint = 0x03;
pub const GEMINI_GLOBAL_PLL_CONTROL: c_uint = 0x08;
pub const GEMINI_GLOBAL_SOFT_RESET: c_uint = 0x0c;
pub const GEMINI_GLOBAL_MISC_CONTROL: c_uint = 0x30;

pub const GEMINI_GLOBAL_CLOCK_CONTROL: c_uint = 0x34;

pub const TVC_HALFDIV_MASK: c_uint = 0x1f;

pub const GEMINI_GLOBAL_PCI_DLL_CONTROL: c_uint = 0x44;

pub const PCI_DLL_TAP_SEL_MASK: c_uint = 0x1f;
//
...
#[no_mangle]
unsafe extern "C" fn gemini_pci_enable(hw: *mut clk_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn gemini_pci_disable(hw: *mut clk_hw) {
...
#[no_mangle]
unsafe extern "C" fn gemini_pci_is_enabled(hw: *mut clk_hw) -> c_int {
    ...
    pciclk.map = map;
    pciclk.hw.init = &init;
    ret = clk_hw_register(core::ptr::null_mut(), &pciclk.hw);
    if (ret) {
    kfree(pciclk);
    return ERR_PTR(ret);
    }
...
#[no_mangle]
unsafe extern "C" fn gemini_clk_probe(pdev: *mut platform_device) -> c_int {
...
// RTC clock 32768 Hz
    hw = clk_hw_register_fixed_rate(core::ptr::null_mut(), "rtc", core::ptr::null_mut(), 0, 32768);
...
// CPU clock derived as a fixed ratio from the AHB clock
    ...
    hw = clk_hw_register_fixed_factor(core::ptr::null_mut(), "cpu", "ahb", 0,
    cpu_ahb_mult[val],
    cpu_ahb_div[val]);
...
// Security clock is 1:1 or 0.75 of APB
    ...
    hw = clk_hw_register_fixed_factor(core::ptr::null_mut(), "secdiv", "ahb", 0, mult, div);
//
...
//
    ...
    div += 1;
    hw = clk_hw_register_fixed_rate(core::ptr::null_mut(), "tvcdiv", "xtal", 0, 27000000);
...
// FIXME: very unclear what the parent is
    hw = clk_hw_register_fixed_rate(core::ptr::null_mut(), "uart", "xtal", 0, 48000000);
...
#[no_mangle]
unsafe extern "C" fn gemini_cc_init(np: *mut device_node) -> void __init {
...
//
    ...
    freq = 60000000;
    hw = clk_hw_register_fixed_rate(core::ptr::null_mut(), "xtal", core::ptr::null_mut(), 0, freq);
...
// If we run on 30 MHz crystal we have to multiply with two
    if (val & PLL_OSC_SEL)
    mult *= 2;
    hw = clk_hw_register_fixed_factor(core::ptr::null_mut(), "vco", "xtal", 0, mult, div);
// The AHB clock is always 1/3 of the VCO
    hw = clk_hw_register_fixed_factor(core::ptr::null_mut(), "ahb", "vco", 0, 1, 3);
    gemini_clk_data.hws[GEMINI_CLK_AHB] = hw;
// The APB clock is always 1/6 of the AHB
    hw = clk_hw_register_fixed_factor(core::ptr::null_mut(), "apb", "ahb", 0, 1, 6);
