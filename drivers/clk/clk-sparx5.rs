...
//! Source: drivers/clk/clk-sparx5.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct s5_clk_data {
    pub base: *mut void __iomem,
    pub s5_hw: [s5_hw_clk; N_CLOCKS],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct s5_pll_conf {
    pub freq: c_ulong,
    pub div: u8,
    pub rot_ena: bool,
    pub rot_sel: u8,
    ...
    {
    let mut rate: c_ulong = parent_rate / conf.div;
    if (conf.rot_ena) {
    let mut sign: c_int = conf.rot_dir ? -1 : 1;
    let mut divt: c_int = sel_rates[conf.rot_sel] * (1 + conf.pre_div);
    let mut divb: c_int = divt + sign;
...
#[no_mangle]
unsafe extern "C" fn s5_pll_enable(hw: *mut clk_hw) -> c_int {
    static int s5_pll_enable(struct clk_hw *hw)
    {
    struct s5_hw_clk *pll = to_s5_pll(hw);
    let mut val: u32 = readl(pll.reg);
    val |= PLL_CLK_ENA;
    writel(val, pll.reg);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn s5_pll_disable(hw: *mut clk_hw) {
...
#[no_mangle]
unsafe extern "C" fn s5_clk_probe(pdev: *mut platform_device) -> c_int {
