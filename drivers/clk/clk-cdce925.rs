...
//! Source: drivers/clk/clk-cdce925.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct clk_cdce925_chip_info {
    pub num_plls: c_int,
    pub num_outputs: c_int,
}

pub const MAX_NUMBER_OF_PLLS: c_int = 4;
pub const MAX_NUMBER_OF_OUTPUTS: c_int = 9;
pub const CDCE925_REG_GLOBAL1: c_uint = 0x01;
pub const CDCE925_REG_Y1SPIPDIVH: c_uint = 0x02;
pub const CDCE925_REG_PDIVL: c_uint = 0x03;
pub const CDCE925_REG_XCSEL: c_uint = 0x05;
// PLL parameters start at 0x10, steps of 0x10
pub const CDCE925_OFFSET_PLL: c_uint = 0x10;
// Add CDCE925_OFFSET_PLL * (pll) to these registers before sending
pub const CDCE925_PLL_MUX_OUTPUTS: c_uint = 0x14;
pub const CDCE925_PLL_MULDIV: c_uint = 0x18;
...
pub struct clk_cdce925_output {
    ...
    pub index: u8,
    pub /: *mut *mut u16 pdiv; / 1..127 for Y2-Y9; 1..1023 for Y1,
}
...
pub struct clk_cdce925_pll {
    ...
    pub chip: *mut clk_cdce925_chip,
    pub index: u8,
    pub /: *mut *mut u16 m; / 1..511,
    pub /: *mut *mut u16 n; / 1..4095,
}
...
#[no_mangle]
pub unsafe extern "C" fn if(CDCE925_PLL_FREQUENCY_MAX: rate >) -> else {
...
#[no_mangle]
unsafe extern "C" fn cdce925_pll_calc_p(n: u16, m: u16) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn cdce925_pll_calc_range_bits(hw: *mut clk_hw, n: u16, m: u16) -> u8 {
    static u8 cdce925_pll_calc_range_bits(struct clk_hw *hw, u16 n, u16 m)
    {
    struct clk *parent = clk_get_parent(hw.clk);
    let mut rate: c_ulong = clk_get_rate(parent);
...
// may sleep
#[no_mangle]
unsafe extern "C" fn cdce925_pll_prepare(hw: *mut clk_hw) -> c_int {
    static int cdce925_pll_prepare(struct clk_hw *hw)
    {
    struct clk_cdce925_pll *data = to_clk_cdce925_pll(hw);
    let mut n: u16 = data.n;
    let mut m: u16 = data.m;
    u16 r;
    u8 q;
    u8 p;
    u16 nn;
    u8 pll[4]; /* Bits are spread out over 4 byte registers */
    let mut reg_ofs: u8 = data.index * CDCE925_OFFSET_PLL;
    unsigned i;
    if ((!m || !n) || (m == n)) {
...
#[no_mangle]
unsafe extern "C" fn cdce925_pll_unprepare(hw: *mut clk_hw) {
...
#[no_mangle]
unsafe extern "C" fn cdce925_clk_set_pdiv(data: *mut clk_cdce925_output, pdiv: u16) {
...
#[no_mangle]
unsafe extern "C" fn cdce925_clk_activate(data: *mut clk_cdce925_output) {
...
#[no_mangle]
unsafe extern "C" fn cdce925_clk_prepare(hw: *mut clk_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cdce925_clk_unprepare(hw: *mut clk_hw) {
...
// Disable clock by setting divider to "0"
    ...
    let mut root_rate: c_ulong = clk_get_rate(root);
    let mut best_rate_error: c_ulong = rate;
    u16 pdiv_min;
    u16 pdiv_max;
    u16 pdiv_best;
    u16 pdiv_now;
    ...
    let mut target_rate: c_ulong = rate * pdiv_now;
    let mut pll_rate: c_long = clk_round_rate(pll, target_rate);
...
// and pick the better one if the error is equal
    ...
    struct clk_rate_request *req)
    {
    let mut l_parent_rate: c_ulong = req.best_parent_rate;
    let mut divider: u16 = cdce925_calc_divider(req.rate, l_parent_rate);
    ...
    {
    let mut l_parent_rate: c_ulong = req.best_parent_rate;
    let mut divider: u16 = cdce925_y1_calc_divider(req.rate, l_parent_rate);
    if (divider)
    req.rate = (long)(l_parent_rate / divider);
    else
    req.rate = 0;
...
pub const CDCE925_I2C_COMMAND_BLOCK_TRANSFER: c_uint = 0x00;
pub const CDCE925_I2C_COMMAND_BYTE_TRANSFER: c_uint = 0x80;
...
#[no_mangle]
unsafe extern "C" fn cdce925_regulator_enable(dev: *mut device, name: *const c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cdce925_probe(client: *mut i2c_client) -> c_int {
    static int cdce925_probe(struct i2c_client *client)
    {
    struct clk_cdce925_chip *data;
    struct device_node *node = client.dev.of_node;
    const char *parent_name;
    const char *pll_clk_name[MAX_NUMBER_OF_PLLS] = {core::ptr::null_mut(),};
