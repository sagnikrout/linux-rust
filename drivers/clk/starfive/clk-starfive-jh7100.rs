...
//! Source: drivers/clk/starfive/clk-starfive-jh7100.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn clk_starfive_jh7100_probe(pdev: *mut platform_device) -> int __init {
    ...
    let mut max: u32 = jh7100_clk_data[idx].max;
    ...
    let mut pidx: c_uint = jh7100_clk_data[idx].parents[i];
    if (pidx < JH7100_CLK_PLL0_OUT)
    parents[i].hw = &priv.reg[pidx].hw;
#[no_mangle]
pub unsafe extern "C" fn if(JH7100_CLK_END: pidx <) -> else {
    else if (pidx < JH7100_CLK_END)
    parents[i].hw = priv.pll[pidx - JH7100_CLK_PLL0_OUT];
#[no_mangle]
pub unsafe extern "C" fn if(JH7100_CLK_OSC_SYS: pidx ==) -> else {
    else if (pidx == JH7100_CLK_OSC_SYS)
    parents[i].fw_name = "osc_sys";
#[no_mangle]
pub unsafe extern "C" fn if(JH7100_CLK_OSC_AUD: pidx ==) -> else {
    else if (pidx == JH7100_CLK_OSC_AUD)
    parents[i].fw_name = "osc_aud";
#[no_mangle]
pub unsafe extern "C" fn if(JH7100_CLK_GMAC_RMII_REF: pidx ==) -> else {
    else if (pidx == JH7100_CLK_GMAC_RMII_REF)
    parents[i].fw_name = "gmac_rmii_ref";
#[no_mangle]
pub unsafe extern "C" fn if(JH7100_CLK_GMAC_GR_MII_RX: pidx ==) -> else {
