...
//! Source: drivers/clk/starfive/clk-starfive-jh7110-sys.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn jh7110_reset_unregister_adev(_adev: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn jh7110_reset_adev_release(dev: *mut device) {
...
#[no_mangle]
unsafe extern "C" fn jh7110_syscrg_probe(pdev: *mut platform_device) -> int __init {
...
// 24MHz -> 1188.0MHz
    ...
    clk_put(pllclk);
    priv.pll[2] = core::ptr::null_mut();
    }
    for (idx = 0; idx < JH7110_SYSCLK_END; idx++) {
    let mut max: u32 = jh7110_sysclk_data[idx].max;
    ...
    let mut pidx: c_uint = jh7110_sysclk_data[idx].parents[i];
    if (pidx < JH7110_SYSCLK_END)
    parents[i].hw = &priv.reg[pidx].hw;
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_OSC: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_OSC)
    parents[i].fw_name = "osc";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_GMAC1_RMII_REFIN: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_GMAC1_RMII_REFIN)
    parents[i].fw_name = "gmac1_rmii_refin";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_GMAC1_RGMII_RXIN: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_GMAC1_RGMII_RXIN)
    parents[i].fw_name = "gmac1_rgmii_rxin";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_I2STX_BCLK_EXT: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_I2STX_BCLK_EXT)
    parents[i].fw_name = "i2stx_bclk_ext";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_I2STX_LRCK_EXT: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_I2STX_LRCK_EXT)
    parents[i].fw_name = "i2stx_lrck_ext";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_I2SRX_BCLK_EXT: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_I2SRX_BCLK_EXT)
    parents[i].fw_name = "i2srx_bclk_ext";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_I2SRX_LRCK_EXT: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_I2SRX_LRCK_EXT)
    parents[i].fw_name = "i2srx_lrck_ext";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_TDM_EXT: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_TDM_EXT)
    parents[i].fw_name = "tdm_ext";
#[no_mangle]
pub unsafe extern "C" fn if(JH7110_SYSCLK_MCLK_EXT: pidx ==) -> else {
    else if (pidx == JH7110_SYSCLK_MCLK_EXT)
    parents[i].fw_name = "mclk_ext";
#[no_mangle]
pub unsafe extern "C" fn if(!priv->pll[0]: pidx == JH7110_SYSCLK_PLL0_OUT &&) -> else {
    else if (pidx == JH7110_SYSCLK_PLL0_OUT && !priv.pll[0])
    parents[i].fw_name = "pll0_out";
#[no_mangle]
pub unsafe extern "C" fn if(!priv->pll[1]: pidx == JH7110_SYSCLK_PLL1_OUT &&) -> else {
    else if (pidx == JH7110_SYSCLK_PLL1_OUT && !priv.pll[1])
    parents[i].fw_name = "pll1_out";
#[no_mangle]
pub unsafe extern "C" fn if(!priv->pll[2]: pidx == JH7110_SYSCLK_PLL2_OUT &&) -> else {
