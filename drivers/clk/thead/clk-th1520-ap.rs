...
//! Source: drivers/clk/thead/clk-th1520-ap.c
#![no_std]
...
pub const TH1520_PLL_STS: c_uint = 0x80;

pub const TH1520_PLL_FRAC_BITS: c_int = 24;
//
// All PLLs in TH1520 take 21250ns at maximum to lock, let's take its double
// for safety.
//
pub const TH1520_PLL_LOCK_TIMEOUT_US: c_int = 44;
pub const TH1520_PLL_STABLE_DELAY_US: c_int = 30;
// c910_bus_clk must be kept below 750MHz for stability

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ccu_internal {
    pub shift: u8,
    pub width: u8,
}
...
pub struct ccu_div {
    ...
    pub mux: ccu_internal,
    pub common: ccu_common,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ccu_pll_cfg {
    pub freq: c_ulong,
    pub fbdiv: u32,
    pub frac: u32,
    pub postdiv1: u32,
...
pub struct ccu_pll {
    pub common: ccu_common,
    pub lock_sts_mask: u32,
    pub cfgnum: c_int,
    pub cfgs: *const ccu_pll_cfg,
}
...
#[no_mangle]
unsafe extern "C" fn ccu_disable_helper(common: *mut ccu_common, gate: u32) {
...
#[no_mangle]
unsafe extern "C" fn ccu_enable_helper(common: *mut ccu_common, gate: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccu_is_enabled_helper(common: *mut ccu_common, gate: u32) -> c_int {
    ...
    rate = divider_recalc_rate(hw, parent_rate, val, core::ptr::null_mut(),
    ...
    unsigned int val;
    if (cd.div_en)
    return divider_determine_rate(hw, req, core::ptr::null_mut(),
    cd.div.width, cd.div.flags);
    regmap_read(cd.common.map, cd.common.cfg0, &val);
    val = val >> cd.div.shift;
    val &= GENMASK(cd.div.width - 1, 0);
    return divider_ro_determine_rate(hw, req, core::ptr::null_mut(), cd.div.width,
    cd.div.flags, val);
    }
    ...
    {
    struct ccu_div *cd = hw_to_ccu_div(hw);
    int val = divider_get_val(rate, parent_rate, core::ptr::null_mut(),
    cd.div.width, cd.div.flags);
...
#[no_mangle]
unsafe extern "C" fn ccu_div_get_parent(hw: *mut clk_hw) -> u8 {
...
#[no_mangle]
unsafe extern "C" fn ccu_div_set_parent(hw: *mut clk_hw, index: u8) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccu_div_disable(hw: *mut clk_hw) {
...
#[no_mangle]
unsafe extern "C" fn ccu_div_enable(hw: *mut clk_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccu_div_is_enabled(hw: *mut clk_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccu_pll_disable(hw: *mut clk_hw) {
...
#[no_mangle]
unsafe extern "C" fn ccu_pll_enable(hw: *mut clk_hw) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ccu_pll_is_enabled(hw: *mut clk_hw) -> c_int {
    ...
    unsigned long rate)
    {
    let mut best_delta: c_ulong = ULONG_MAX;
...
//
#[no_mangle]
unsafe extern "C" fn c910_clk_get_parent(hw: *mut clk_hw) -> u8 {
    static u8 c910_clk_get_parent(struct clk_hw *hw)
    {
    return clk_mux_ops.get_parent(hw);
    }
#[no_mangle]
unsafe extern "C" fn c910_clk_set_parent(hw: *mut clk_hw, index: u8) -> c_int {
    ...
    struct clk_rate_request *req)
    {
    let mut alt_parent_index: u8 = !c910_clk_get_parent(hw);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct th1520_plat_data {
    pub th1520_pll_clks: *mut ccu_common,
    pub th1520_div_clks: *mut ccu_common,
    pub th1520_mux_clks: *mut ccu_mux,
    pub th1520_gate_clks: *mut ccu_gate,
    pub nr_clks: c_int,
    pub nr_pll_clks: c_int,
    pub nr_div_clks: c_int,
    pub nr_mux_clks: c_int,
    pub nr_gate_clks: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn th1520_clk_probe(pdev: *mut platform_device) -> c_int {
