...
//! Source: drivers/clk/rockchip/clk-cpu.c
#![no_std]
...
pub struct rockchip_cpuclk {
    ...
    let mut clksel0: u32 = readl_relaxed(cpuclk.reg_base + reg_data.core_reg[0]);
...
#[no_mangle]
pub unsafe extern "C" fn if(POST_RATE_CHANGE: event ==) -> else {
...
// disallow automatic parent changes by ccf
    ...
    cclk = clk_register(core::ptr::null_mut(), &cpuclk.hw);
    if (IS_ERR(cclk)) {
    ...
    struct clk_notifier_data *ndata)
    {
    let mut new_rate: c_ulong = roundup(ndata.new_rate, 1000);
    const struct rockchip_cpuclk_rate_table *rate;
    unsigned long flags;
