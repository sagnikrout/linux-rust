...
//! Source: drivers/clk/clk-divider.c
#![no_std]
...
//
#[no_mangle]
pub unsafe extern "C" fn clk_div_readl(divider: *mut clk_divider) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn clk_div_writel(divider: *mut clk_divider, val: u32) {
    ...
    u8 width)
    {
    let mut maxdiv: c_uint = 0, mask = clk_div_mask(width);
...
#[no_mangle]
unsafe extern "C" fn _get_table_mindiv(table: *const clk_div_table) -> c_uint {
    static unsigned int _get_table_mindiv(const struct clk_div_table *table)
    {
    let mut mindiv: c_uint = UINT_MAX;
    const struct clk_div_table *clkt;
    for (clkt = table; clkt.div; clkt++)
    if (clkt.div < mindiv)
    mindiv = clkt.div;
...
#[no_mangle]
unsafe extern "C" fn _round_up_table(table: *const clk_div_table, div: c_int) -> c_int {
    static int _round_up_table(const struct clk_div_table *table, int div)
    {
    const struct clk_div_table *clkt;
    let mut up: c_int = INT_MAX;
    for (clkt = table; clkt.div; clkt++) {
    if (clkt.div == div)
    return clkt.div;
#[no_mangle]
pub unsafe extern "C" fn if(div: clkt->div <) -> else {
...
#[no_mangle]
unsafe extern "C" fn _round_down_table(table: *const clk_div_table, div: c_int) -> c_int {
    static int _round_down_table(const struct clk_div_table *table, int div)
    {
    const struct clk_div_table *clkt;
    let mut down: c_int = _get_table_mindiv(table);
    for (clkt = table; clkt.div; clkt++) {
    if (clkt.div == div)
    return clkt.div;
#[no_mangle]
pub unsafe extern "C" fn if(div: clkt->div >) -> else {
    ...
    unsigned long flags)
    {
    let mut div: c_int = DIV_ROUND_UP_ULL((u64)parent_rate, rate);
    if (flags & CLK_DIVIDER_POWER_OF_TWO)
    ...
    let mut overflow: bool = check_mul_overflow(rate, (unsigned long)i, &target_parent_rate);
...
//
    ...
    {
    struct clk_hw *hw;
    hw =  __clk_hw_register_divider(dev, core::ptr::null_mut(), name, parent_name, core::ptr::null_mut(),
    core::ptr::null_mut(), flags, reg, shift, width, clk_divider_flags,
    table, lock);
    if (IS_ERR(hw))
    return ERR_CAST(hw);
    return hw.clk;
...
#[no_mangle]
pub unsafe extern "C" fn clk_unregister_divider(clk: *mut clk) {
...
//
#[no_mangle]
pub unsafe extern "C" fn clk_hw_unregister_divider(hw: *mut clk_hw) {
...
#[no_mangle]
unsafe extern "C" fn devm_clk_hw_release_divider(dev: *mut device, res: *mut c_void) {
