...
//! Source: drivers/clk/spear/clk-aux-synth.c
#![no_std]
...
//
    ...
    let mut eq: u8 = rtbl[index].eq ? 1 : 2;
    ...
    {
    struct clk_aux *aux = to_clk_aux(hw);
    let mut num: c_uint = 1, den = 1, val, eqn;
...
// struct clk_aux assignments
    ...
    init.num_parents = 1;
    clk = clk_register(core::ptr::null_mut(), &aux.hw);
    if (IS_ERR_OR_NULL(clk))
    goto free_aux;
    if (gate_name) {
    struct clk *tgate_clk;
    tgate_clk = clk_register_gate(core::ptr::null_mut(), gate_name, aux_name,
