...
//
#[no_mangle]
pub unsafe extern "C" fn clk_mux_readl(mux: *mut clk_mux) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn clk_mux_writel(mux: *mut clk_mux, val: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn clk_mux_index_to_val(table: *const u32, flags: c_uint, index: u8) -> c_uint {
    unsigned int clk_mux_index_to_val(const u32 *table, unsigned int flags, u8 index)
    {
    let mut val: c_uint = index;
    if (table) {
    val = table[index];
    } else {
...
unsafe extern "C" fn clk_mux_set_parent(hw: *mut clk_hw, index: u8) -> c_int {
    ...
    let mut width: u8 = fls(mask) - ffs(mask) + 1;
    if (width + shift > 16) {
...
#[no_mangle]
unsafe extern "C" fn devm_clk_hw_release_mux(dev: *mut device, res: *mut c_void) {
