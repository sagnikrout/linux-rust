...
//! Source: drivers/power/supply/rx51_battery.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn rx51_battery_read_adc(channel: *mut iio_channel) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn rx51_battery_read_voltage(di: *mut rx51_device_info) -> c_int {
    static int rx51_battery_read_voltage(struct rx51_device_info *di)
    {
    let mut voltage: c_int = rx51_battery_read_adc(di.channel_vbat);
    if (voltage < 0) {
...
//
pub const rx51_temp_table2_first: c_int = 53;
...
//
#[no_mangle]
unsafe extern "C" fn rx51_battery_read_temperature(di: *mut rx51_device_info) -> c_int {
    static int rx51_battery_read_temperature(struct rx51_device_info *di)
    {
    let mut min: c_int = 0;
    let mut max: c_int = ARRAY_SIZE(rx51_temp_table2) - 1;
    let mut raw: c_int = rx51_battery_read_adc(di.channel_temp);
    if (raw < 0)
    dev_err(di.dev, "Could not read ADC: %d\n", raw);
...
//
#[no_mangle]
unsafe extern "C" fn rx51_battery_read_capacity(di: *mut rx51_device_info) -> c_int {
    static int rx51_battery_read_capacity(struct rx51_device_info *di)
    {
    let mut capacity: c_int = rx51_battery_read_adc(di.channel_bsi);
    if (capacity < 0) {
    dev_err(di.dev, "Could not read ADC: %d\n", capacity);
    return capacity;
    }
...
#[no_mangle]
unsafe extern "C" fn rx51_battery_probe(pdev: *mut platform_device) -> c_int {
