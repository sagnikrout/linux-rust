...
//! Source: drivers/power/supply/max8903_charger.c
#![no_std]
...
//
pub const MAX8903_USB_CURRENT_LIMIT_LOW_UA: c_int = 100000;
pub const MAX8903_USB_CURRENT_LIMIT_HIGH_UA: c_int = 500000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct max8903_current_limit_mapping {
    pub /: *mut *mut u32 limit_ua; / Current limit in microamps,
    pub /: *mut *mut u32 gpio_value; / GPIO bit pattern,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct max8903_data {
    pub dev: *mut device,
...
//
    pub /: *mut *mut *mut gpio_desc cen; / Charger Enable input,
    pub /: *mut *mut *mut gpio_desc dok; / DC (Adapter) Power OK output,
    pub /: *mut *mut *mut gpio_desc uok; / USB Power OK output,
    pub /: *mut *mut *mut gpio_desc chg; / Charger status output,
    pub /: *mut *mut *mut gpio_desc flt; / Fault output,
    pub /: *mut *mut *mut gpio_desc dcm; / Current-Limit Mode input (1: DC, 2: USB),
    pub /: *mut *mut *mut gpio_desc usus; / USB Suspend Input (1: suspended),
// DC current limit control (ISET pins)
    pub dc_current_limit_gpios: *mut gpio_descs,
    pub dc_current_limit_map: *mut max8903_current_limit_mapping,
    pub dc_current_limit_map_size: u32,
    pub /: *mut *mut u32 dc_current_limit_ua; / Current setting in uA,
// USB current limit control (IUSB pin)
    pub usb_current_limit_gpio: *mut gpio_desc,
    pub /: *mut *mut u32 usb_current_limit_ua; / Current setting in uA,
//
...
#[no_mangle]
pub unsafe extern "C" fn if(ta_in: usb_in ||) -> else {
...
#[no_mangle]
unsafe extern "C" fn max8903_set_dc_current_limit(data: *mut max8903_data, limit_ua: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn max8903_set_usb_current_limit(data: *mut max8903_data, limit_ua: u32) -> c_int {
...
//
    ...
    -ENODEV;
#[no_mangle]
pub unsafe extern "C" fn if(data->usb_current_limit_gpio: data->usb_in &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn max8903_dcin(irq: c_int, _data: *mut c_void) -> irqreturn_t {
...
// Certainly enable if DOK is asserted
    val = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: data->usb_in) -> else {
    else if (data.usb_in)
...
#[no_mangle]
unsafe extern "C" fn max8903_usbin(irq: c_int, _data: *mut c_void) -> irqreturn_t {
...
// Certainly enable if UOK is asserted
    val = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: data->ta_in) -> else {
    else if (data.ta_in)
...
#[no_mangle]
unsafe extern "C" fn max8903_fault(irq: c_int, _data: *mut c_void) -> irqreturn_t {
...
//
    ...
    for (i = 0; i < data.dc_current_limit_map_size; i++) {
    let mut gpio_value: u32 = map[i * 2 + 1];
//
...
#[no_mangle]
unsafe extern "C" fn max8903_setup_gpios(pdev: *mut platform_device) -> c_int {
    static int max8903_setup_gpios(struct platform_device *pdev)
    {
    struct max8903_data *data = platform_get_drvdata(pdev);
    struct device *dev = &pdev.dev;
    let mut ta_in: bool = false;
    let mut usb_in: bool = false;
...
#[no_mangle]
unsafe extern "C" fn max8903_probe(pdev: *mut platform_device) -> c_int {
    ...
    core::ptr::null_mut(), max8903_dcin,
    ...
    core::ptr::null_mut(), max8903_usbin,
    IRQF_TRIGGER_FALLING |
    IRQF_TRIGGER_RISING | IRQF_ONESHOT,
    "MAX8903 USB IN", data);
    if (ret)
    ...
    core::ptr::null_mut(), max8903_fault,
    IRQF_TRIGGER_FALLING |
    IRQF_TRIGGER_RISING | IRQF_ONESHOT,
    "MAX8903 Fault", data);
    if (ret)
