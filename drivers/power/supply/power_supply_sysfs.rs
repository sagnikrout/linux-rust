...
//! Source: drivers/power/supply/power_supply_sysfs.c
#![no_std]
...
pub const MAX_PROP_NAME_LEN: c_int = 30;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct power_supply_attr {
    pub prop_name: *const c_char,
    pub 1]: char attr_name[MAX_PROP_NAME_LEN +,
    pub dev_attr: device_attribute,
    pub text_values: *const *const c_char,
    pub text_values_len: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn dev_attr_psp(attr: *const device_attribute) -> enum power_supply_property {
    static enum power_supply_property dev_attr_psp(const struct device_attribute *attr)
    {
    return  to_ps_attr(attr) - power_supply_attrs;
    }
#[no_mangle]
unsafe extern "C" fn power_supply_escape_spaces(str: *const c_char, buf: *mut c_char, bufsize: usize) {
...
#[no_mangle]
unsafe extern "C" fn power_supply_match_string(array: *const *const c_char, n: usize, s: *const c_char) -> c_int {
...
// Second round, try matching with spaces replaced by '_'
    ...
    {
    let mut match: bool = false, available, active;
    ...
    let mut psp: enum power_supply_property = dev_attr_psp(attr);
    union power_supply_propval value;
    ...
    attr.attr.name);
#[no_mangle]
pub unsafe extern "C" fn if(-EINVAL: ret != -ENODEV && ret != -EAGAIN && ret !=) -> else {
...
//
    ...
    struct power_supply *psy = dev_to_psy(dev);
    let mut mode: umode_t = S_IRUSR | S_IRGRP | S_IROTH;
...
#[no_mangle]
pub unsafe extern "C" fn power_supply_init_attrs() -> void __init {
...
#[no_mangle]
pub unsafe extern "C" fn power_supply_uevent(dev: *const device, env: *mut kobj_uevent_env) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn power_supply_charge_behaviour_parse(available_behaviours: c_uint, buf: *const c_char) -> c_int {
    int power_supply_charge_behaviour_parse(unsigned int available_behaviours, const char *buf)
    {
    let mut i: c_int = sysfs_match_string(POWER_SUPPLY_CHARGE_BEHAVIOUR_TEXT, buf);
    if (i < 0)
    return i;
...
#[no_mangle]
pub unsafe extern "C" fn power_supply_charge_types_parse(available_types: c_uint, buf: *const c_char) -> c_int {
