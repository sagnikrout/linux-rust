...
//! Source: drivers/power/supply/mp2629_charger.c
#![no_std]
...
pub const MP2629_REG_INPUT_ILIM: c_uint = 0x00;
pub const MP2629_REG_INPUT_VLIM: c_uint = 0x01;
pub const MP2629_REG_CHARGE_CTRL: c_uint = 0x04;
pub const MP2629_REG_CHARGE_ILIM: c_uint = 0x05;
pub const MP2629_REG_PRECHARGE: c_uint = 0x06;
pub const MP2629_REG_TERM_CURRENT: c_uint = 0x06;
pub const MP2629_REG_CHARGE_VLIM: c_uint = 0x07;
pub const MP2629_REG_TIMER_CTRL: c_uint = 0x08;
pub const MP2629_REG_IMPEDANCE_COMP: c_uint = 0x09;
pub const MP2629_REG_INTERRUPT: c_uint = 0x0b;
pub const MP2629_REG_STATUS: c_uint = 0x0c;
pub const MP2629_REG_FAULT: c_uint = 0x0d;

pub const MP2629_MAX_BATT_CAPACITY: c_int = 100;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mp2629_charger {
    pub dev: *mut device,
    pub status: c_int,
    pub fault: c_int,
    pub regmap: *mut regmap,
    pub regmap_fields: [*mut regmap_field; MP2629_MAX_FIELD],
    pub lock: mutex,
    pub usb: *mut power_supply,
    pub battery: *mut power_supply,
    pub iiochan: [*mut iio_channel; MP2629_ADC_CHAN_END],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct mp2629_prop {
    pub reg: c_int,
    pub mask: c_int,
    pub min: c_int,
    pub max: c_int,
...
#[no_mangle]
pub unsafe extern "C" fn if(charger->fault: MP2629_FAULT_THERMAL &) -> else {
    else if (MP2629_FAULT_THERMAL & charger.fault)
    val.intval = POWER_SUPPLY_HEALTH_OVERHEAT;
#[no_mangle]
pub unsafe extern "C" fn if(charger->fault: MP2629_FAULT_INPUT &) -> else {
...
#[no_mangle]
unsafe extern "C" fn mp2629_irq_handler(irq: c_int, dev_id: *mut c_void) -> irqreturn_t {
...
#[no_mangle]
pub unsafe extern "C" fn if(rval: MP2629_FAULT_THERMAL &) -> else {
    else if (MP2629_FAULT_THERMAL & rval)
    dev_err(charger.dev, "Thermal shutdown fault\n");
#[no_mangle]
pub unsafe extern "C" fn if(rval: MP2629_FAULT_INPUT &) -> else {
    else if (MP2629_FAULT_INPUT & rval)
    dev_err(charger.dev, "no input or input OVP\n");
#[no_mangle]
pub unsafe extern "C" fn if(rval: MP2629_FAULT_OTG &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(MP2629_CHARGING_CHANGE: rval &) -> else {
...
#[no_mangle]
unsafe extern "C" fn mp2629_charger_disable(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn mp2629_charger_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (ret)
    return ret;
    charger.usb = devm_power_supply_register(dev, &mp2629_usb_desc, core::ptr::null_mut());
    ...
    ret = devm_request_threaded_irq(dev, irq, core::ptr::null_mut(),	mp2629_irq_handler,
