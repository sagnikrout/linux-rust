...
//! Source: drivers/vfio/pci/vfio_pci_igd.c
#![no_std]
...
pub const OPREGION_PCI_ADDR: c_uint = 0xfc;
pub const OPREGION_RVDA: c_uint = 0x3ba;
pub const OPREGION_RVDS: c_uint = 0x3c2;
pub const OPREGION_VERSION: c_uint = 0x16;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct igd_opregion_vbt {
    pub opregion: *mut c_void,
    pub vbt_ex: *mut c_void,
}
...
// remaining -= bytes;
    ...
    bool iswrite)
    {
    let mut i: c_uint = VFIO_PCI_OFFSET_TO_INDEX(*ppos) - VFIO_PCI_NUM_REGIONS;
    struct igd_opregion_vbt *opregionvbt = vdev.region[i].data;
    let mut pos: loff_t = *ppos & VFIO_PCI_OFFSET_MASK, off = 0;
    size_t remaining;
...
// Copy until OpRegion version
    ...
    let mut bytes: usize = min_t(size_t, remaining, OPREGION_VERSION - pos);
...
// Copy until RVDA
    ...
    let mut bytes: usize = min_t(size_t, remaining, OPREGION_RVDA - pos);
    if (igd_opregion_shift_copy(buf, &off,
    opregionvbt.opregion + pos, &pos,
    &remaining, bytes))
    return -EFAULT;
...
// Copy the rest of OpRegion
    ...
    let mut bytes: usize = min_t(size_t, remaining, OPREGION_SIZE - pos);
    if (igd_opregion_shift_copy(buf, &off,
    opregionvbt.opregion + pos, &pos,
    &remaining, bytes))
    return -EFAULT;
...
#[no_mangle]
unsafe extern "C" fn vfio_pci_igd_opregion_init(vdev: *mut vfio_pci_core_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vfio_pci_igd_cfg_init(vdev: *mut vfio_pci_core_device) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn vfio_pci_is_intel_display(pdev: *mut pci_dev) -> bool {
    bool vfio_pci_is_intel_display(struct pci_dev *pdev)
    {
    return (pdev.vendor == PCI_VENDOR_ID_INTEL) && pci_is_display(pdev);
    }
#[no_mangle]
pub unsafe extern "C" fn vfio_pci_igd_init(vdev: *mut vfio_pci_core_device) -> c_int {
