...
//! Source: drivers/vfio/pci/vfio_pci_rdwr.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn if(x_end: off >=) -> else {
    else if (off >= x_end)
    fillable = count;
    else
    fillable = 0;
...
// Fill reads with -1, drop writes
    ...
    let mut bar: c_int = VFIO_PCI_OFFSET_TO_INDEX(*ppos);
    let mut x_start: usize = 0, x_end = 0;
    resource_size_t end;
    void __iomem *io;
    struct resource *res = &vdev.pdev.resource[bar];
    ssize_t done;
    let mut max_width: enum vfio_pci_io_width = VFIO_PCI_IO_WIDTH_8;
    if (pci_resource_start(pdev, bar))
    end = pci_resource_len(pdev, bar);
#[no_mangle]
pub unsafe extern "C" fn if(pdev->romlen: bar == PCI_ROM_RESOURCE && pdev->rom &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn vfio_pci_ioeventfd_handler(opaque: *mut c_void, unused: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vfio_pci_ioeventfd_thread(opaque: *mut c_void, unused: *mut c_void) {
    ...
    {
    struct pci_dev *pdev = vdev.pdev;
    let mut pos: loff_t = offset & VFIO_PCI_OFFSET_MASK;
...
// Disallow ioeventfds working around MSI-X table writes
    ...
    vfio_pci_ioeventfd_thread, core::ptr::null_mut(),
    &ioeventfd.virqfd, fd);
    if (ret) {
    kfree(ioeventfd);
    goto out_unlock;
