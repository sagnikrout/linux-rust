...
//! Source: drivers/vfio/pci/nvgrace-gpu/main.c
#![no_std]
...
pub const DVSEC_BITMAP_OFFSET: c_uint = 0xA;

pub const GPU_CAP_DVSEC_REGISTER: c_int = 3;
pub const C2C_LINK_BAR0_OFFSET: c_uint = 0x1498;
pub const HBM_TRAINING_BAR0_OFFSET: c_uint = 0x200BC;
pub const STATUS_READY: c_uint = 0xFF;
pub const POLL_QUANTUM_MS: c_int = 1000;
...
pub struct mem_region {
    pub /: *mut *mut phys_addr_t memphys; / Base physical address of the region,
    pub /: *mut *mut size_t memlength; / Region size,
    pub /: *mut *mut size_t bar_size; / Reported region BAR size,
    pub /: *mut *mut __le64 bar_val; / Emulated BAR offset registers,
    union {
...
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_init_fake_bar_emu_regs(core_vdev: *mut vfio_device) {
...
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_open_device(core_vdev: *mut vfio_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_close_device(core_vdev: *mut vfio_device) {
    static void nvgrace_gpu_close_device(struct vfio_device *core_vdev)
    {
    struct nvgrace_gpu_pci_core_device *nvdev =
    container_of(core_vdev, struct nvgrace_gpu_pci_core_device,
    core_device.vdev);
    nvdev.bar0_base = core::ptr::null_mut();
    if (nvdev.resmem.memlength)
    unregister_pfn_address_space(&nvdev.resmem.pfn_address_space);
...
// Unmap the mapping to the device memory cached region
    ...
    nvdev.usemem.memaddr = core::ptr::null_mut();
    }
// Unmap the mapping to the device memory non-cached region
    if (nvdev.resmem.ioaddr) {
    iounmap(nvdev.resmem.ioaddr);
    nvdev.resmem.ioaddr = core::ptr::null_mut();
    }
    mutex_destroy(&nvdev.remap_lock);
    vfio_pci_core_close_device(core_vdev);
    }
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_wait_device_ready_legacy(io: *mut void __iomem) -> c_int {
    static int nvgrace_gpu_wait_device_ready_legacy(void __iomem *io)
    {
    let mut timeout: c_ulong = jiffies + msecs_to_jiffies(POLL_TIMEOUT_MS);
    do {
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_mem_active_timeout_ms(timeout: u8) -> c_ulong {
...
//
#[no_mangle]
pub unsafe extern "C" fn cxl_dvsec_mem_is_active(status: u32) -> bool {
    ...
    let mut cxl_dvsec: c_int = nvdev.cxl_dvsec;
    u32 val;
...
//
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_wait_device_ready_cxl(nvdev: *mut nvgrace_gpu_pci_core_device) -> c_int {
    static int nvgrace_gpu_wait_device_ready_cxl(struct nvgrace_gpu_pci_core_device *nvdev)
    {
    let mut deadline: c_ulong = jiffies + msecs_to_jiffies(POLL_QUANTUM_MS);
    let mut active_phase: bool = false;
    u32 status;
    int ret;
    for (;;) {
    ...
    let mut t: u8 = FIELD_GET(PCI_DVSEC_CXL_MEM_ACTIVE_TIMEOUT, status);
    deadline = jiffies +
...
//
    ...
    return -EIO;
    if (nvdev.cxl_dvsec)
    ret = nvgrace_gpu_test_device_ready_cxl(nvdev, core::ptr::null_mut());
    else
...
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_vfio_pci_fault(vmf: *mut vm_fault) -> vm_fault_t {
...
//
    ...
    struct mem_region *memregion = core::ptr::null_mut();
    __le64 val64;
...
//
    ...
    let mut offset: u64 = *ppos & VFIO_PCI_OFFSET_MASK;
    int ret;
    if (!mem_count)
    return 0;
//
...
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_has_mig_hw_bug(pdev: *mut pci_dev) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_probe_check_device_ready(nvdev: *mut nvgrace_gpu_pci_core_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_remove(pdev: *mut pci_dev) {
...
//
#[no_mangle]
unsafe extern "C" fn nvgrace_gpu_vfio_pci_reset_done(pdev: *mut pci_dev) {
