...
//! Source: drivers/vfio/pci/mlx5/main.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_disable_fd(migf: *mut mlx5_vf_migration_file) {
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_release_file(inode: *mut inode, filp: *mut file) -> c_int {
...
//
    ...
    return found ? buf : core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn mlx5vf_buf_read_done(vhca_buf: *mut mlx5_vhca_data_buffer) {
    ...
    let mut is_header: bool = vhca_buf.dma_dir == DMA_NONE;
    let mut chunk_num: u8 = vhca_buf.stop_copy_chunk_num;
    let mut next_required_umem_size: usize = 0;
    if (is_header)
...
// buf += page_len;
    ...
    let mut first_loop_call: bool = true;
    bool end_of_data;
    let mut done: isize = 0;
    if (pos)
    return -ESPIPE;
...
//
#[no_mangle]
unsafe extern "C" fn mlx5vf_mark_err(migf: *mut mlx5_vf_migration_file) {
    ...
    {
    let mut npages: u32 = DIV_ROUND_UP(required_length, PAGE_SIZE);
    struct mlx5_vhca_data_buffer *buf = migf.buf[index];
    u8 chunk_num;
    WARN_ON(!buf);
    chunk_num = buf.stop_copy_chunk_num;
    buf.migf.buf[index] = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_mig_file_save_work(_work: *mut work_struct) {
    ...
    let mut data: mlx5_vf_migration_tag_stop_copy_data = {};
    struct mlx5_vhca_data_buffer *header_buf = core::ptr::null_mut();
    let mut header: mlx5_vf_migration_header = {};
    unsigned long flags;
    struct page *page;
    u8 *to_buff;
    int ret;
    ...
    int i;
    if (mvdev.chunk_mode) {
    let mut chunk_size: usize = min_t(size_t, MAX_CHUNK_SIZE, full_size);
...
// let's not overflow the device specification max SAVE size
    ...
    migf.buf[i] = core::ptr::null_mut();
    }
    if (migf.buf_header[i]) {
    mlx5vf_put_data_buffer(migf.buf_header[i]);
    migf.buf_header[i] = core::ptr::null_mut();
    }
    }
    return ret;
    }
    ...
    let mut migration_state: u8 = 0;
    let mut inc_length: usize = 0;
    bool reinit_state;
    bool end_of_data;
    int ret;
...
//
    ...
    core::ptr::null_mut(), &migration_state,
    MLX5VF_QUERY_INC);
    if (ret)
    goto err_state_unlock;
    }
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_pci_save_device_inc_data(mvdev: *mut mlx5vf_pci_core_device) -> c_int {
    ...
    if (migf.state == MLX5_MIGF_STATE_ERROR)
    return -ENODEV;
    ret = mlx5vf_cmd_query_vhca_migration_state(mvdev, &length, core::ptr::null_mut(), core::ptr::null_mut(),
...
//
    ...
    if (ret)
    goto out;
    ret = mlx5vf_cmd_query_vhca_migration_state(mvdev, &length, &full_size, core::ptr::null_mut(), 0);
    if (ret)
    goto out_pd;
...
// has_work = true;
    ...
    {
    let mut npages: u32 = DIV_ROUND_UP(migf.record_size, PAGE_SIZE);
    ...
    migf.buf_header[0] = core::ptr::null_mut();
    goto out_unlock;
    }
    ...
    let mut npages: u32 = DIV_ROUND_UP(size, PAGE_SIZE);
...
// prep data buf for next image
    ...
    mvdev.resuming_migf = core::ptr::null_mut();
    }
...
// last_save_state = mvdev->saving_migf->state;
    ...
    mvdev.saving_migf = core::ptr::null_mut();
    }
    }
    static struct file *
    ...
    u32 new)
    {
    let mut cur: u32 = mvdev.mig_state;
    int ret;
    ...
    mlx5vf_disable_fds(mvdev, core::ptr::null_mut());
    return core::ptr::null_mut();
    }
    ...
    size_t size;
    ret = mlx5vf_cmd_query_vhca_migration_state(mvdev, &size, core::ptr::null_mut(), core::ptr::null_mut(),
...
// pre_copy cleanup
    ...
    return (state != MLX5_MIGF_STATE_ERROR) ? core::ptr::null_mut() : ERR_PTR(-EIO);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn mlx5vf_state_mutex_unlock(mvdev: *mut mlx5vf_pci_core_device) {
    ...
    struct file *res = core::ptr::null_mut();
    int ret;
    mutex_lock(&mvdev.state_mutex);
    ...
    &total_size, core::ptr::null_mut(), 0);
    if (!ret)
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_pci_aer_reset_done(pdev: *mut pci_dev) {
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_pci_open_device(core_vdev: *mut vfio_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_pci_close_device(core_vdev: *mut vfio_device) {
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_pci_init_dev(core_vdev: *mut vfio_device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_pci_release_dev(core_vdev: *mut vfio_device) {
...
#[no_mangle]
unsafe extern "C" fn mlx5vf_pci_remove(pdev: *mut pci_dev) {
