...
//! Source: drivers/vfio/vfio_iommu_type1.c
#![no_std]
...
//
    ...
    let mut __read_mostly: static unsigned int dma_entry_limit = U16_MAX;
    module_param_named(dma_entry_limit, dma_entry_limit, uint, 0644);
    MODULE_PARM_DESC(dma_entry_limit,
    "Maximum number of user DMA mappings per container (65535).");
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vfio_iommu {
    pub domain_list: list_head,
    pub iova_list: list_head,
    pub lock: mutex,
    pub dma_list: rb_root,
    pub device_list: list_head,
    pub device_list_lock: mutex,
    pub dma_avail: c_uint,
    pub vaddr_invalid_count: c_uint,
    pub pgsize_bitmap: u64,
    pub num_non_pinned_groups: u64,
    pub v2: bool,
    pub dirty_page_tracking: bool,
    pub emulated_iommu_groups: list_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct vfio_domain {
...
pub struct vfio_dma {
    pub node: rb_node,
    pub /: *mut *mut dma_addr_t iova; / Device address,
    pub /: *mut *mut unsigned long vaddr; / Process virtual addr,
    pub /: *mut *mut size_t size; / Map size (bytes),
    pub /: *mut *mut int prot; / IOMMU_READ/WRITE,
    pub iommu_mapped: bool,
    pub /: *mut *mut bool lock_cap; / capable(CAP_IPC_LOCK),
    pub vaddr_invalid: bool,
    pub /: *mut *mut bool has_rsvd; / has 1 or more rsvd pfns,
    pub task: *mut task_struct,
    pub /: *mut *mut rb_root pfn_list; / Ex-user pinned pfn list,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct vfio_batch {
    pub /: *mut *mut *mut *mut page pages; / for pin_user_pages_remote,
    pub /: *mut *mut *mut page fallback_page; / if pages alloc fails,
    pub /: *mut *mut unsigned int capacity; / length of pages array,
    pub /: *mut *mut unsigned int size; / of batch currently,
    pub /: *mut *mut unsigned int offset; / of next entry in pages,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct vfio_iommu_group {
    pub iommu_group: *mut iommu_group,
    pub next: list_head,
    pub pinned_page_dirty_scope: bool,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct vfio_iova {
    pub list: list_head,
...
pub struct vfio_pfn {
    pub node: rb_node,
    pub /: *mut *mut dma_addr_t iova; / Device address,
    pub /: *mut *mut unsigned long pfn; / Host pfn,
    pub ref_count: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct vfio_regions {
    pub list: list_head,
    pub iova: dma_addr_t,
...
//
    ...
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(1: start > dma->iova + dma->size -) -> else {
    ...
    dma_addr_t end)
    {
    struct rb_node *res = core::ptr::null_mut();
    struct rb_node *node = iommu.dma_list.rb_node;
    struct vfio_dma *dma_res = core::ptr::null_mut();
    WARN_ON(end < start);
    while (node) {
    struct vfio_dma *dma = rb_entry(node, struct vfio_dma, node);
...
#[no_mangle]
unsafe extern "C" fn vfio_link_dma(iommu: *mut vfio_iommu, new: *mut vfio_dma) {
    static void vfio_link_dma(struct vfio_iommu *iommu, struct vfio_dma *new)
    {
    struct rb_node **link = &iommu.dma_list.rb_node, *parent = core::ptr::null_mut();
    struct vfio_dma *dma;
    WARN_ON(new.size != 0);
    while (*link) {
    parent = *link;
...
#[no_mangle]
unsafe extern "C" fn vfio_unlink_dma(iommu: *mut vfio_iommu, old: *mut vfio_dma) {
    static void vfio_unlink_dma(struct vfio_iommu *iommu, struct vfio_dma *old)
    {
    rb_erase(&old.node, &iommu.dma_list);
    }
#[no_mangle]
unsafe extern "C" fn vfio_dma_bitmap_alloc(dma: *mut vfio_dma, pgsize: usize) -> c_int {
    static int vfio_dma_bitmap_alloc(struct vfio_dma *dma, size_t pgsize)
    {
    let mut npages: u64 = dma.size / pgsize;
...
#[no_mangle]
unsafe extern "C" fn vfio_dma_bitmap_free(dma: *mut vfio_dma) {
    static void vfio_dma_bitmap_free(struct vfio_dma *dma)
    {
    kvfree(dma.bitmap);
    dma.bitmap = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn vfio_dma_populate_bitmap(dma: *mut vfio_dma, pgsize: usize) {
    static void vfio_dma_populate_bitmap(struct vfio_dma *dma, size_t pgsize)
    {
    struct rb_node *p;
    let mut pgshift: c_ulong = __ffs(pgsize);
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_populate_bitmap_full(iommu: *mut vfio_iommu) {
    static void vfio_iommu_populate_bitmap_full(struct vfio_iommu *iommu)
    {
    struct rb_node *n;
    let mut pgshift: c_ulong = __ffs(iommu.pgsize_bitmap);
...
#[no_mangle]
unsafe extern "C" fn vfio_dma_bitmap_alloc_all(iommu: *mut vfio_iommu, pgsize: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vfio_dma_bitmap_free_all(iommu: *mut vfio_iommu) {
...
//
    ...
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(vpfn->iova: iova_start >) -> else {
    ...
    struct vfio_pfn *new)
    {
    struct rb_node **link, *parent = core::ptr::null_mut();
    struct vfio_pfn *vpfn;
...
#[no_mangle]
unsafe extern "C" fn vfio_unlink_pfn(dma: *mut vfio_dma, old: *mut vfio_pfn) {
...
#[no_mangle]
unsafe extern "C" fn vfio_iova_put_vfio_pfn(dma: *mut vfio_dma, vpfn: *mut vfio_pfn) -> c_int {
    ...
    {
    let mut ret: c_int = mmap_write_lock_killable(mm);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn vfio_lock_acct(dma: *mut vfio_dma, npage: c_long, async: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn is_invalid_reserved_pfn(pfn: c_ulong) -> bool {
...
#[no_mangle]
unsafe extern "C" fn put_pfn(pfn: c_ulong, prot: c_int) -> c_int {
    static int put_pfn(unsigned long pfn, int prot)
    {
...
#[no_mangle]
unsafe extern "C" fn __vfio_batch_init(batch: *mut vfio_batch, single: bool) {
...
#[no_mangle]
unsafe extern "C" fn vfio_batch_init(batch: *mut vfio_batch) {
    static void vfio_batch_init(struct vfio_batch *batch)
    {
    __vfio_batch_init(batch, false);
    }
#[no_mangle]
unsafe extern "C" fn vfio_batch_init_single(batch: *mut vfio_batch) {
    static void vfio_batch_init_single(struct vfio_batch *batch)
    {
    __vfio_batch_init(batch, true);
    }
#[no_mangle]
unsafe extern "C" fn vfio_batch_unpin(batch: *mut vfio_batch, dma: *mut vfio_dma) {
    static void vfio_batch_unpin(struct vfio_batch *batch, struct vfio_dma *dma)
    {
    while (batch.size) {
    let mut pfn: c_ulong = page_to_pfn(batch.pages[batch.offset]);
...
#[no_mangle]
unsafe extern "C" fn vfio_batch_fini(batch: *mut vfio_batch) {
    ...
    {
    let mut args: follow_pfnmap_args = { .vma = vma, .address = vaddr };
    int ret;
...
//
    ...
    {
    let mut pin_pages: c_ulong = min_t(unsigned long, npages, batch.capacity);
...
// pfn = page_to_pfn(batch->pages[0]);
    ...
    {
    let mut iova_end: dma_addr_t = iova_start + (nr_pages << PAGE_SHIFT);
...
//
    ...
    bool rsvd;
    let mut iova: dma_addr_t = vaddr - dma.vaddr + dma.iova;
...
//
    ...
    bool do_accounting)
    {
    let mut unlocked: c_long = 0, locked = vpfn_pages(dma, iova, npage);
    if (dma.has_rsvd) {
    unsigned long i;
    for (i = 0; i < npage; i++)
...
// Supported for v2 version only
    ...
    do_accounting = list_empty(&iommu.domain_list);
    for (i = 0; i < npage; i++) {
    let mut iova: dma_addr_t = user_iova + PAGE_SIZE * i;
    struct vfio_dma *dma;
    dma = vfio_find_dma(iommu, iova, PAGE_SIZE);
    if (!dma)
    break;
...
//
pub const VFIO_IOMMU_TLB_SYNC_MAX: c_int = 512;
...
// unmapped_cnt = 0;
    ...
    long *unlocked)
    {
    let mut unmapped: usize = iommu_unmap(domain.domain, iova, len);
    if (unmapped) {
// unlocked += vfio_unpin_pages_remote(dma, iova,
    ...
    struct iommu_iotlb_gather iotlb_gather;
    let mut unmapped_region_cnt: c_int = 0;
    let mut unlocked: c_long = 0;
    let mut pos: usize = 0;
    if (!dma.size)
    return 0;
...
//
    ...
    let mut iova: dma_addr_t = dma.iova + pos;
...
#[no_mangle]
unsafe extern "C" fn vfio_remove_dma(iommu: *mut vfio_iommu, dma: *mut vfio_dma) {
...
#[no_mangle]
unsafe extern "C" fn vfio_update_pgsize_bitmap(iommu: *mut vfio_iommu) {
...
//
    ...
    {
    let mut pgshift: c_ulong = __ffs(pgsize);
    let mut nbits: c_ulong = dma.size >> pgshift;
    let mut bit_offset: c_ulong = (dma.iova - base_iova) >> pgshift;
    let mut copy_offset: c_ulong = bit_offset / BITS_PER_LONG;
    let mut shift: c_ulong = bit_offset % BITS_PER_LONG;
...
#[no_mangle]
unsafe extern "C" fn verify_bitmap_size(npages: u64, bitmap_size: u64) -> c_int {
...
//
    ...
    {
    struct vfio_dma *dma, *dma_last = core::ptr::null_mut();
    let mut unmapped: usize = 0, pgsize;
    let mut ret: c_int = -EINVAL, retries = 0;
    unsigned long pgshift;
    let mut iova: dma_addr_t = unmap.iova;
    dma_addr_t iova_end;
    let mut size: usize = unmap.size;
    let mut unmap_all: bool = unmap.flags & VFIO_DMA_UNMAP_FLAG_ALL;
    let mut invalidate_vaddr: bool = unmap.flags & VFIO_DMA_UNMAP_FLAG_VADDR;
...
// Report how much was unmapped
    ...
    size_t map_size)
    {
    let mut iova: dma_addr_t = dma.iova;
    let mut vaddr: c_ulong = dma.vaddr;
    struct vfio_batch batch;
    let mut size: usize = map_size;
    long npage;
...
#[no_mangle]
unsafe extern "C" fn vfio_change_dma_owner(dma: *mut vfio_dma) -> c_int {
    static int vfio_change_dma_owner(struct vfio_dma *dma)
    {
    struct task_struct *task = current.group_leader;
    struct mm_struct *mm = current.mm;
    let mut npage: c_long = dma.locked_vm;
    bool lock_cap;
    int ret;
    if (mm == dma.mm)
    return 0;
    ...
    {
    let mut set_vaddr: bool = map.flags & VFIO_DMA_MAP_FLAG_VADDR;
    let mut iova: dma_addr_t = map.iova;
    dma_addr_t iova_end;
    let mut vaddr: c_ulong = map.vaddr;
    unsigned long vaddr_end;
    let mut size: usize = map.size;
    let mut ret: c_int = 0, prot = 0;
    size_t pgsize;
    struct vfio_dma *dma;
...
// Don't pin and map if container doesn't contain IOMMU capable domain
    ...
    {
    struct vfio_batch batch;
    struct vfio_domain *d = core::ptr::null_mut();
    struct rb_node *n;
    let mut limit: c_ulong = rlimit(RLIMIT_MEMLOCK) >> PAGE_SHIFT;
    int ret;
// Arbitrarily pick the first domain in the list for lookups
    ...
    unsigned long pfn;
    let mut vaddr: c_ulong = dma.vaddr + pos;
    let mut n: usize = dma.size - pos;
    long npage;
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_resv_free(resv_regions: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_iova_free(iova: *mut list_head) {
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_domain_alloc(dev: *mut device, data: *mut c_void) -> c_int {
...
// domain = iommu_paging_domain_alloc(dev);
    ...
    struct vfio_domain *domain, *d;
    bool resv_msi;
    let mut resv_msi_base: phys_addr_t = 0;
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_unmap_unpin_all(iommu: *mut vfio_iommu) {
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_unmap_unpin_reaccount(iommu: *mut vfio_iommu) {
    ...
    for (; n; n = rb_next(n)) {
    struct vfio_dma *dma;
    let mut locked: c_long = 0, unlocked = 0;
    dma = rb_entry(n, struct vfio_dma, node);
...
//
    ...
    struct vfio_domain *domain;
    struct vfio_iova *node;
    let mut start: dma_addr_t = 0;
    let mut end: dma_addr_t = (dma_addr_t)~0;
...
// Exclude current reserved regions from iova ranges
    ...
    struct vfio_domain *domain;
    struct vfio_iommu_group *group;
    let mut update_dirty_scope: bool = false;
    LIST_HEAD(iova_copy);
    mutex_lock(&iommu.lock);
    list_for_each_entry(group, &iommu.emulated_iommu_groups, next) {
    if (group.iommu_group != iommu_group)
...
#[no_mangle]
unsafe extern "C" fn vfio_release_domain(domain: *mut vfio_domain) {
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_type1_release(iommu_data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn vfio_domains_have_enforce_cache_coherency(iommu: *mut vfio_iommu) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_has_emulated(iommu: *mut vfio_iommu) -> bool {
...
//
    ...
    struct vfio_info_cap *caps)
    {
    let mut cap_mig: vfio_iommu_type1_info_cap_migration = {};
...
// only one flag should be set at a time
    ...
    unsigned long pgshift;
    let mut data_size: usize = dirty.argsz - minsz;
...
//
    ...
    unsigned long vaddr;
    struct vfio_dma *dma;
    let mut kthread: bool = current.mm == core::ptr::null_mut();
    size_t offset;
// copied = 0;
    ...
    kthread_use_mm(mm);
#[no_mangle]
pub unsafe extern "C" fn if(mm: current->mm !=) -> else {
...
#[no_mangle]
unsafe extern "C" fn vfio_iommu_type1_init() -> int __init {
    static int __init vfio_iommu_type1_init(void)
    {
    return vfio_register_iommu_driver(&vfio_iommu_driver_ops_type1);
    }
#[no_mangle]
unsafe extern "C" fn vfio_iommu_type1_cleanup() -> void __exit {
