...
//! Source: drivers/vfio/vfio_iommu_spapr_tce.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tce_iommu_group {
    pub next: list_head,
    pub grp: *mut iommu_group,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tce_iommu_prereg {
    pub next: list_head,
    pub mem: *mut mm_iommu_table_group_mem_t,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct tce_container {
    pub lock: mutex,
    pub enabled: bool,
    pub v2: bool,
    pub def_window_pending: bool,
    pub locked_pages: c_ulong,
    pub mm: *mut mm_struct,
    pub tables: [*mut iommu_table; IOMMU_TABLE_GROUP_MAX_TABLES],
    pub group_list: list_head,
    pub prereg_list: list_head,
}

#[no_mangle]
unsafe extern "C" fn tce_iommu_mm_set(container: *mut tce_container) -> c_long {
    ...
    struct tce_iommu_prereg *tcemem;
    let mut entries: c_ulong = size >> PAGE_SHIFT;
    if ((vaddr & ~PAGE_MASK) || (size & ~PAGE_MASK) ||
    ((vaddr + size) < vaddr))
    return -EINVAL;
    ...
    let mut size: return = = (1UL << it_page_shift);
...
#[no_mangle]
pub unsafe extern "C" fn tce_groups_attached(container: *mut tce_container) -> bool {
    ...
    if (tbl) {
    let mut entry: c_ulong = ioba >> tbl.it_page_shift;
    let mut start: c_ulong = tbl.it_offset;
    let mut end: c_ulong = start + tbl.it_size;
...
#[no_mangle]
unsafe extern "C" fn tce_iommu_find_free_table(container: *mut tce_container) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tce_iommu_enable(container: *mut tce_container) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn tce_iommu_disable(container: *mut tce_container) {
    ...
    struct iommu_table *tbl);
#[no_mangle]
unsafe extern "C" fn tce_iommu_release(iommu_data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn tce_iommu_unuse_page(hpa: c_ulong) {
...
// pua = cpu_to_be64(0);
    ...
    let mut lastentry: c_ulong = entry + pages, firstentry = entry;
...
#[no_mangle]
unsafe extern "C" fn tce_iommu_use_page(tce: c_ulong, hpa: *mut c_ulong) -> c_int {
    static int tce_iommu_use_page(unsigned long tce, unsigned long *hpa)
    {
    struct page *page = core::ptr::null_mut();
    let mut direction: enum dma_data_direction = iommu_tce_direction(tce);
...
// hpa = __pa((unsigned long) page_address(page));
    ...
    for (i = 0; i < pages; ++i) {
    let mut offset: c_ulong = tce & IOMMU_PAGE_MASK(tbl) & ~PAGE_MASK;
...
// pua = cpu_to_be64(tce);
    ...
    struct iommu_table *tbl)
    {
    let mut pages: c_ulong = tbl.it_allocated_size >> PAGE_SHIFT;
...
// Free table
    tce_iommu_free_table(container, tbl);
    container.tables[num] = core::ptr::null_mut();
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn tce_iommu_create_default_window(container: *mut tce_container) -> c_long {
    static long tce_iommu_create_default_window(struct tce_container *container)
    {
    long ret;
    let mut start_addr: __u64 = 0;
    struct tce_iommu_group *tcegrp;
    struct iommu_table_group *table_group;
...
// Set all windows to the new group
    ...
    struct tce_container *container = iommu_data;
    struct iommu_table_group *table_group;
    struct tce_iommu_group *tcegrp = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn tce_iommu_init() -> int __init {
    static int __init tce_iommu_init(void)
    {
    return vfio_register_iommu_driver(&tce_iommu_driver_ops);
    }
#[no_mangle]
unsafe extern "C" fn tce_iommu_cleanup() -> void __exit {
