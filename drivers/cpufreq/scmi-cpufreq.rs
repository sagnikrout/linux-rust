...
//! Source: drivers/cpufreq/scmi-cpufreq.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct scmi_data {
    pub domain_id: c_int,
    pub nr_opp: c_int,
    pub cpu_dev: *mut device,
    pub opp_shared_cpus: cpumask_var_t,
    pub limit_notify_nb: notifier_block,
    pub limits_freq_req: freq_qos_request,
}
...
#[no_mangle]
unsafe extern "C" fn scmi_cpufreq_get_rate(cpu: c_uint) -> c_uint {
...
//
    ...
    let mut freq: u64 = policy.freq_table[index].frequency;
    ...
    {
    struct scmi_data *priv = policy.driver_data;
    let mut freq: c_ulong = target_freq;
    if (!perf_ops.freq_set(ph, priv.domain_id, freq * 1000, true))
    return target_freq;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn scmi_cpu_domain_id(cpu_dev: *mut device) -> c_int {
...
// Find the corresponding index for power-domain "perf".
    ...
    unsigned long *KHz)
    {
    let mut power_scale: enum scmi_power_scale = perf_ops.power_scale_get(ph);
    unsigned long Hz;
    int ret, domain;
...
#[no_mangle]
unsafe extern "C" fn scmi_limit_notify_cb(nb: *mut notifier_block, event: c_ulong, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_cpufreq_init(policy: *mut cpufreq_policy) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn scmi_cpufreq_exit(policy: *mut cpufreq_policy) {
...
#[no_mangle]
unsafe extern "C" fn scmi_cpufreq_register_em(policy: *mut cpufreq_policy) {
    static void scmi_cpufreq_register_em(struct cpufreq_policy *policy)
    {
    let mut em_cb: em_data_callback = EM_DATA_CB(scmi_get_cpu_power);
    let mut power_scale: enum scmi_power_scale = perf_ops.power_scale_get(ph);
    struct scmi_data *priv = policy.driver_data;
    let mut em_power_scale: bool = false;
//
...
#[no_mangle]
unsafe extern "C" fn scmi_dev_used_by_cpus(scmi_dev: *mut device) -> bool {
...
#[no_mangle]
unsafe extern "C" fn scmi_cpufreq_probe(sdev: *mut scmi_device) -> c_int {
...
// dummy clock provider as needed by OPP if clocks property is used
    ...
    ret = devm_of_clk_add_hw_provider(dev, of_clk_hw_simple_get, core::ptr::null_mut());
    if (ret)
...
#[no_mangle]
unsafe extern "C" fn scmi_cpufreq_remove(sdev: *mut scmi_device) {
