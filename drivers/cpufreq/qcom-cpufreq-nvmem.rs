...
//! Source: drivers/cpufreq/qcom-cpufreq-nvmem.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qcom_cpufreq_match_data {
    int (*get_version)(struct device *cpu_dev,
    struct nvmem_cell *speedbin_nvmem,
    char **pvs_name,
    pub drv): *mut qcom_cpufreq_drv,
    pub pd_names: *const c_char,
    pub num_pd_names: c_uint,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct qcom_cpufreq_drv_cpu {
    pub opp_token: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct qcom_cpufreq_drv {
    pub versions: u32,
    pub data: *const qcom_cpufreq_match_data,
    pub cpus: [qcom_cpufreq_drv_cpu; ],
}
    ...
    u8 *speedbin;
// pvs_name = NULL;
    speedbin = nvmem_cell_read(speedbin_nvmem, core::ptr::null_mut());
    if (IS_ERR(speedbin))
...
// pvs_name = NULL;
    ...
    struct qcom_cpufreq_drv *drv)
    {
    let mut speed: c_int = 0, pvs = 0, pvs_ver = 0;
    u8 *speedbin;
    size_t len;
    let mut ret: c_int = 0;
    speedbin = nvmem_cell_read(speedbin_nvmem, &len);
    ...
    struct qcom_cpufreq_drv *drv)
    {
    let mut msm_id: c_int = -1, ret = 0;
    let mut speed: c_int = 0, pvs = 0;
    u8 *speedbin;
    size_t len;
    speedbin = nvmem_cell_read(speedbin_nvmem, &len);
    if (IS_ERR(speedbin))
...
#[no_mangle]
unsafe extern "C" fn qcom_cpufreq_suspend_pd_devs(drv: *mut qcom_cpufreq_drv, cpu: c_uint) {
...
#[no_mangle]
unsafe extern "C" fn qcom_cpufreq_probe(pdev: *mut platform_device) -> c_int {
    ...
    .supported_hw = core::ptr::null_mut(),
    };
    cpu_dev = get_cpu_device(cpu);
    if (core::ptr::null_mut() == cpu_dev) {
    ret = -ENODEV;
    goto free_opp;
    }
    if (drv.data.get_version) {
...
#[no_mangle]
unsafe extern "C" fn qcom_cpufreq_remove(pdev: *mut platform_device) {
...
#[no_mangle]
unsafe extern "C" fn qcom_cpufreq_suspend(dev: *mut device) -> c_int {
    ...
    static DEFINE_SIMPLE_DEV_PM_OPS(qcom_cpufreq_pm_ops, qcom_cpufreq_suspend, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn qcom_cpufreq_init() -> int __init {
    ...
    if (unlikely(ret < 0))
    return ret;
    cpufreq_pdev = platform_device_register_data(core::ptr::null_mut(), "qcom-cpufreq-nvmem",
...
#[no_mangle]
unsafe extern "C" fn qcom_cpufreq_exit() -> void __exit {
