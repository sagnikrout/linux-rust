...
//! Source: drivers/cpufreq/s5pv210-cpufreq.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct dram_conf {
    pub /: *mut *mut unsigned long freq; / HZ,
    pub /: *mut *mut *mut unsigned long refresh; / DRAM refresh counter  1000,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct s5pv210_dvs_conf {
    pub /: *mut *mut int arm_volt; / uV,
    pub /: *mut *mut int int_volt; / uV,
}

    let mut arm_volt_max: static int = 1350000;
    let mut int_volt_max: static int = 1250000;
...
//
#[no_mangle]
unsafe extern "C" fn s5pv210_set_refresh(ch: enum s5pv210_dmc_port, freq: c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn s5pv210_target(policy: *mut cpufreq_policy, index: c_uint) -> c_int {
    static int s5pv210_target(struct cpufreq_policy *policy, unsigned int index)
    {
    unsigned long reg;
    unsigned int priv_index;
    let mut pll_changing: c_uint = 0;
    let mut bus_speed_changing: c_uint = 0;
...
#[no_mangle]
unsafe extern "C" fn check_mem_type(dmc_reg: *mut void __iomem) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn s5pv210_cpu_init(policy: *mut cpufreq_policy) -> c_int {
    ...
    dmc0_clk = clk_get(core::ptr::null_mut(), "sclk_dmc0");
    if (IS_ERR(dmc0_clk)) {
    ret = PTR_ERR(dmc0_clk);
    goto out_dmc0;
    }
    dmc1_clk = clk_get(core::ptr::null_mut(), "hclk_msys");
    if (IS_ERR(dmc1_clk)) {
...
#[no_mangle]
unsafe extern "C" fn s5pv210_cpufreq_probe(pdev: *mut platform_device) -> c_int {
...
// cpufreq-dt driver.
//
    arm_regulator = regulator_get(core::ptr::null_mut(), "vddarm");
    if (IS_ERR(arm_regulator))
    return dev_err_probe(dev, PTR_ERR(arm_regulator),
    "failed to get regulator vddarm\n");
    int_regulator = regulator_get(core::ptr::null_mut(), "vddint");
    if (IS_ERR(int_regulator)) {
    result = dev_err_probe(dev, PTR_ERR(int_regulator),
    "failed to get regulator vddint\n");
    goto err_int_regulator;
    }
    np = of_find_compatible_node(core::ptr::null_mut(), core::ptr::null_mut(), "samsung,s5pv210-clock");
    if (!np) {
    ...
    for_each_compatible_node_scoped(dmc, core::ptr::null_mut(), "samsung,s5pv210-dmc") {
