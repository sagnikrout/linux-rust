...
//! Source: drivers/cpufreq/amd-pstate-ut.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct amd_pstate_ut_struct {
    pub name: *const c_char,
    pub index): *mut *mut int (func)(u32,
}
...
#[no_mangle]
unsafe extern "C" fn test_in_list(list: *const c_char, name: *const c_char) -> bool {
    ...
    let mut token_len: usize = sep ? sep - p : strlen(p);
...
#[no_mangle]
unsafe extern "C" fn get_shared_mem() -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_acpi_cpc_valid(index: u32) -> c_int {
    static int amd_pstate_ut_acpi_cpc_valid(u32 index)
    {
    if (!acpi_cpc_valid()) {
...
//
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_check_enabled(index: u32) -> c_int {
    static int amd_pstate_ut_check_enabled(u32 index)
    {
    let mut cppc_enable: u64 = 0;
    int ret;
    if (get_shared_mem())
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_check_perf(index: u32) -> c_int {
    static int amd_pstate_ut_check_perf(u32 index)
    {
    let mut cpu: c_int = 0, ret = 0;
    let mut highest_perf: u32 = 0, nominal_perf = 0, lowest_nonlinear_perf = 0, lowest_perf = 0;
    let mut cap1: u64 = 0;
    struct cppc_perf_caps cppc_perf;
    union perf_cached cur_perf;
    for_each_online_cpu(cpu) {
    struct cpufreq_policy *policy __free(put_cpufreq_policy) = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_check_freq(index: u32) -> c_int {
    static int amd_pstate_ut_check_freq(u32 index)
    {
    let mut cpu: c_int = 0;
    for_each_online_cpu(cpu) {
...
#[no_mangle]
unsafe extern "C" fn amd_pstate_set_mode(mode: enum amd_pstate_mode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_epp(index: u32) -> c_int {
    static int amd_pstate_ut_epp(u32 index)
    {
    ...
    char *buf __free(cleanup_page) = core::ptr::null_mut();
    struct cpufreq_policy *policy = core::ptr::null_mut();
    let mut orig_dynamic_epp: c_ulong = 0;
...
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_check_driver(index: u32) -> c_int {
    static int amd_pstate_ut_check_driver(u32 index)
    {
    enum amd_pstate_mode mode1, mode2 = AMD_PSTATE_DISABLE;
    let mut orig_mode: enum amd_pstate_mode = amd_pstate_get_status();
    int ret;
...
#[no_mangle]
unsafe extern "C" fn attr_in_driver(driver_attrs: *mut freq_attr, name: *const c_char) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_check_freq_attrs(index: u32) -> c_int {
    static int amd_pstate_ut_check_freq_attrs(u32 index)
    {
    let mut orig_mode: enum amd_pstate_mode = amd_pstate_get_status();
...
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_check_floor_freq(index: u32) -> c_int {
    static int amd_pstate_ut_check_floor_freq(u32 index)
    {
    struct cpufreq_policy *policy __free(put_cpufreq_policy) = core::ptr::null_mut();
    char *buf __free(cleanup_page) = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_init() -> int __init {
    static int __init amd_pstate_ut_init(void)
    {
    let mut i: u32 = 0, arr_size = ARRAY_SIZE(amd_pstate_ut_cases);
    let mut mode: enum amd_pstate_mode = amd_pstate_get_status();
...
#[no_mangle]
unsafe extern "C" fn amd_pstate_ut_exit() -> void __exit {
