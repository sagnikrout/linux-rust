...
//! Source: drivers/rtc/rtc-optee.c
#![no_std]
...
pub const RTC_INFO_VERSION: c_uint = 0x1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct optee_rtc_alarm {
    pub enabled: u8,
    pub pending: u8,
    pub time: optee_rtc_time,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct optee_rtc_info {
    pub version: u64,
    pub features: u64,
    pub range_min: optee_rtc_time,
    pub range_max: optee_rtc_time,
}
...
pub struct optee_rtc {
    ...
    pub shm: *mut tee_shm,
    pub features: u64,
    pub alarm_task: *mut task_struct,
    pub rtc: *mut rtc_device,
}

#[no_mangle]
unsafe extern "C" fn optee_rtc_readtime(dev: *mut device, tm: *mut rtc_time) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_settime(dev: *mut device, tm: *mut rtc_time) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_readoffset(dev: *mut device, offset: *mut c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_setoffset(dev: *mut device, offset: c_long) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_read_alarm(dev: *mut device, alarm: *mut rtc_wkalrm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_set_alarm(dev: *mut device, alarm: *mut rtc_wkalrm) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_enable_alarm(dev: *mut device, enabled: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_wait_alarm(dev: *mut device, return_status: *mut c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_cancel_wait_alarm(dev: *mut device) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_set_alarm_wake_status(dev: *mut device, status: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_handle_alarm_event(data: *mut c_void) -> c_int {
    static int optee_rtc_handle_alarm_event(void *data)
    {
    struct optee_rtc *priv = (struct optee_rtc *)data;
    let mut wait_alarm_return_status: c_int = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_probe(rtc_device: *mut tee_client_device) -> c_int {
    static int optee_rtc_probe(struct tee_client_device *rtc_device)
    {
    struct device *dev = &rtc_device.dev;
    let mut sess2_arg: tee_ioctl_open_session_arg = {0};
    let mut sess_arg: tee_ioctl_open_session_arg = {0};
...
// Open first session with rtc Pseudo Trusted App
    ...
    ret = tee_client_open_session(priv.ctx, &sess_arg, core::ptr::null_mut());
...
// Open second session with rtc Trusted App
    ...
    ret = tee_client_open_session(priv.ctx, &sess2_arg, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_remove(rtc_device: *mut tee_client_device) {
...
#[no_mangle]
unsafe extern "C" fn optee_rtc_suspend(dev: *mut device) -> c_int {
    static int optee_rtc_suspend(struct device *dev)
    {
    let mut res: c_int = optee_rtc_set_alarm_wake_status(dev, device_may_wakeup(dev));
    if (res) {
    ...
    static DEFINE_SIMPLE_DEV_PM_OPS(optee_rtc_pm_ops, optee_rtc_suspend, core::ptr::null_mut());
