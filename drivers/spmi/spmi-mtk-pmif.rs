...
//! Source: drivers/spmi/spmi-mtk-pmif.c
#![no_std]
...
pub const SWINF_IDLE: c_uint = 0x00;
pub const SWINF_WFVLDCLR: c_uint = 0x06;

pub const PMIF_CMD_REG_0: c_int = 0;
pub const PMIF_CMD_REG: c_int = 1;
pub const PMIF_CMD_EXT_REG: c_int = 2;
pub const PMIF_CMD_EXT_REG_LONG: c_int = 3;
pub const PMIF_DELAY_US: c_int = 2;

pub const PMIF_CHAN_OFFSET: c_uint = 0x5;

pub const PMIF_MAX_BUSES: c_int = 2;
pub const PMIF_MAX_CLKS: c_int = 3;
pub const SPMI_OP_ST_BUSY: c_int = 1;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ch_reg {
    pub ch_sta: u32,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pmif_data {
    pub regs: *const u32,
    pub spmimst_regs: *const u32,
    pub soc_chan: u32,
    pub spmi_ver: u8,
    pub num_spmi_buses: u32,
}
...
pub struct pmif_bus {
    ...
    pub spmimst_base: *mut void __iomem,
    pub ctrl: *mut spmi_controller,
    pub dom: *mut irq_domain,
    pub irq: c_int,
    pub clks: [clk_bulk_data; PMIF_MAX_CLKS],
    pub nclks: usize,
    pub irq_min_sid: u8,
    pub irq_max_sid: u8,
    pub irq_en: u16,
...
#[derive(Copy, Clone)]
pub struct pmif {
    pub bus: [pmif_bus; PMIF_MAX_BUSES],
    pub chan: ch_reg,
    pub data: *const pmif_data,
}
...
#[no_mangle]
unsafe extern "C" fn pmif_readl(arb: *mut pmif, pbus: *mut pmif_bus, reg: enum pmif_regs) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_readl(arb: *mut pmif, pbus: *mut pmif_bus, reg: enum spmi_regs) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn pmif_is_fsm_vldclr(arb: *mut pmif, pbus: *mut pmif_bus) -> bool {
...
#[no_mangle]
unsafe extern "C" fn pmif_arb_cmd(ctrl: *mut spmi_controller, opc: u8, sid: u8) -> c_int {
...
// Check for argument validation.
    ...
    opc = PMIF_CMD_REG;
#[no_mangle]
pub unsafe extern "C" fn if(0x3f): (opc >= 0x20 && opc <= 0x2f) || (opc >= 0x38 && opc <=) -> else {
...
// Check the opcode
    ...
    opc = PMIF_CMD_REG;
#[no_mangle]
pub unsafe extern "C" fn if(0x37): (opc <= 0xF) || (opc >= 0x30 && opc <=) -> else {
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_handle_chained_irq(desc: *mut irq_desc) {
    ...
    let mut val: u32 = mtk_spmi_readl(arb, pbus, i);
    while (val) {
    let mut bit: u8 = __ffs(val);
    let mut bank: u8 = bit / 7;
    let mut sid: u8 = ((i - SPMI_SLV_3_0_EINT) * 4) + bank;
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_rcs_irq_eoi(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_rcs_irq_enable(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_rcs_irq_disable(d: *mut irq_data) {
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_rcs_irq_set_wake(d: *mut irq_data, on: c_uint) -> c_int {
...
// out_type = intspec[2] & IRQ_TYPE_SENSE_MASK;
    ...
    pbus, handle_level_irq, core::ptr::null_mut(), core::ptr::null_mut());
    }
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_irq_remove(pbus: *mut pmif_bus) {
    static void mtk_spmi_irq_remove(struct pmif_bus *pbus)
    {
    if (!pbus.dom)
    return;
    irq_set_chained_handler_and_data(pbus.irq, core::ptr::null_mut(), core::ptr::null_mut());
    ...
    if (idx < 0)
    return -EINVAL;
    pbus.base = devm_of_iomap(&pdev.dev, node, idx, core::ptr::null_mut());
    if (IS_ERR(pbus.base))
    return PTR_ERR(pbus.base);
    idx = of_property_match_string(node, "reg-names", "spmimst");
    if (idx < 0)
    return -EINVAL;
    pbus.spmimst_base = devm_of_iomap(&pdev.dev, node, idx, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_probe(pdev: *mut platform_device) -> c_int {
    static int mtk_spmi_probe(struct platform_device *pdev)
    {
    struct device_node *node = pdev.dev.of_node;
    struct pmif *arb;
    u32 chan_offset;
    let mut cur_bus: u8 = 0;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn mtk_spmi_remove(pdev: *mut platform_device) {
