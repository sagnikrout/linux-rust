...
//! Source: crypto/jitterentropy-testing.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct jent_testing {
    pub jent_testing_rb: [u64; JENT_TEST_RINGBUFFER_SIZE],
    pub rb_reader: u32,
    pub rb_writer: core::sync::atomic::AtomicI32,
    pub jent_testing_enabled: core::sync::atomic::AtomicI32,
    pub lock: spinlock_t,
    pub read_wait: wait_queue_head_t,
}

    static struct dentry *jent_raw_debugfs_root = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn jent_testing_reset(data: *mut jent_testing) {
...
#[no_mangle]
unsafe extern "C" fn jent_testing_data_init(data: *mut jent_testing, boot: u32) {
...
#[no_mangle]
unsafe extern "C" fn jent_testing_fini(data: *mut jent_testing, boot: u32) {
...
#[no_mangle]
unsafe extern "C" fn jent_testing_have_data(data: *mut jent_testing) -> bool {
    ...
    while (outbuflen) {
    let mut writer: u32 = (u32)atomic_read(&data.rb_writer);
    spin_lock_irqsave(&data.lock, flags);
...
// We copy out word-wise
    ...
    {
    u8 *tmp, *tmp_aligned;
    let mut ret: c_int = 0, large_request = (nbytes > 256);
    if (!nbytes)
    return 0;
//
...
// Raw High-Resolution Timer Entropy Data Handling
    let mut boot_raw_hires_test: static u32 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn jent_raw_hires_entropy_store(value: __u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn jent_raw_hires_entropy_reader(outbuf: *mut u8, outbuflen: u32) -> c_int {
...
pub unsafe extern "C" fn jent_testing_init() {
    ...
    jent_raw_debugfs_root, core::ptr::null_mut(),
    &jent_raw_hires_fops);
    }
