...
#[no_mangle]
pub unsafe extern "C" fn scatterwalk_skip(walk: *mut scatter_walk, nbytes: c_uint) {
    ...
    if (unlikely(nbytes == 0)) /* in case sg == core::ptr::null_mut() */
    return;
...
//
    ...
    if (unlikely(nbytes == 0)) /* in case src and/or dst is core::ptr::null_mut() */
    return;
...
// HIGHMEM: we may have to actually map the pages.
    let mut src_oip: c_uint = offset_in_page(src_offset);
    let mut dst_oip: c_uint = offset_in_page(dst_offset);
    let mut limit: c_uint = PAGE_SIZE;
