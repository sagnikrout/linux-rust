...
//! Source: crypto/async_tx/async_raid6_recov.c
#![no_std]
...
//
    ...
    &dest, 1, srcs, 2, len);
    struct dma_device *dma = chan ? chan.device : core::ptr::null_mut();
    ...
    let mut dma_flags: enum dma_ctrl_flags = DMA_PREP_PQ_DISABLE_P;
...
// d++ = qmul[*s++];
    ...
    let mut flags: enum async_tx_flags = submit.flags;
    let mut cb_fn: dma_async_tx_callback = submit.cb_fn;
...
// Dx = A*(P+Pxy) + B*(Q+Qxy)
    ...
    init_async_submit(submit, ASYNC_TX_FENCE, tx, core::ptr::null_mut(), core::ptr::null_mut(), scribble);
...
//
    dp = blocks[faila];
    dp_off = offs[faila];
    blocks[faila] = core::ptr::null_mut();
    blocks[disks-2] = dp;
    offs[disks-2] = dp_off;
    dq = blocks[failb];
    dq_off = offs[failb];
    blocks[failb] = core::ptr::null_mut();
    blocks[disks-1] = dq;
    offs[disks-1] = dq_off;
    init_async_submit(submit, ASYNC_TX_FENCE, tx, core::ptr::null_mut(), core::ptr::null_mut(), scribble);
...
//
    ...
    let mut g_off: c_uint = offs[good];
    init_async_submit(submit, ASYNC_TX_FENCE, tx, core::ptr::null_mut(), core::ptr::null_mut(),
    scribble);
