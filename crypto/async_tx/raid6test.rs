...
//! Source: crypto/async_tx/raid6test.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn callback(param: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn makedata(disks: c_int) {
...
#[no_mangle]
unsafe extern "C" fn disk_type(d: c_int, disks: c_int) -> c_char {
    static char disk_type(int d, int disks)
    {
    if (d == disks - 2)
    return 'P';
#[no_mangle]
pub unsafe extern "C" fn if(1: d == disks -) -> else {
    else if (d == disks - 1)
    return 'Q';
    else
    return 'D';
...
// Recover two failed blocks.
    ...
    let mut result: enum sum_check_flags = ~0;
    if (faila > failb)
    swap(faila, failb);
    if (failb == disks-1) {
    if (faila == disks-2) {
// P+Q failure.  Just rebuild the syndrome.
    init_async_submit(&submit, 0, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), addr_conv);
...
//
    ...
    core::ptr::null_mut(), core::ptr::null_mut(), addr_conv);
    tx = async_xor(dest, blocks, 0, count, bytes, &submit);
    init_async_submit(&submit, 0, tx, core::ptr::null_mut(), core::ptr::null_mut(), addr_conv);
    tx = async_gen_syndrome(ptrs, offs,
    disks, bytes, &submit);
    }
    } else {
...
#[no_mangle]
unsafe extern "C" fn test_disks(i: c_int, j: c_int, disks: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn test(disks: c_int, tests: *mut c_int) -> c_int {
...
// Generate assumed good syndrome
    init_completion(&cmp);
    init_async_submit(&submit, ASYNC_TX_ACK, core::ptr::null_mut(), callback, &cmp, addr_conv);
...
#[no_mangle]
unsafe extern "C" fn raid6_test() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn raid6_test_exit() -> void __exit {
