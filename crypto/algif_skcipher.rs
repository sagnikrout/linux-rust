...
//! Source: crypto/algif_skcipher.c
#![no_std]
...
//
    ...
    let mut ivsize: unsigned = crypto_skcipher_ivsize(tfm);
    return af_alg_sendmsg(sock, msg, size, ivsize);
    }
#[no_mangle]
unsafe extern "C" fn algif_skcipher_export(sk: *mut sock, req: *mut skcipher_request) -> c_int {
    ...
    struct crypto_skcipher *tfm = pask.private;
    let mut bs: c_uint = crypto_skcipher_chunksize(tfm);
    struct af_alg_async_req *areq;
    let mut cflags: unsigned = 0;
    let mut err: c_int = 0;
    let mut len: usize = 0;
...
// Initialize the crypto operation
    ...
    let mut err: c_int = _skcipher_recvmsg(sock, msg, ignored, flags);
//
...
#[no_mangle]
unsafe extern "C" fn skcipher_check_key(sock: *mut socket) -> c_int {
    ...
    {
    let mut mask: u32 = AF_ALG_CRYPTOAPI_MASK;
    int err;
...
#[no_mangle]
unsafe extern "C" fn skcipher_release(private: *mut c_void) {
    static void skcipher_release(void *private)
    {
    crypto_free_skcipher(private);
    }
#[no_mangle]
unsafe extern "C" fn skcipher_setkey(private: *mut c_void, key: *const u8, keylen: c_uint) -> c_int {
    static int skcipher_setkey(void *private, const u8 *key, unsigned int keylen)
    {
    return crypto_skcipher_setkey(private, key, keylen);
    }
#[no_mangle]
unsafe extern "C" fn skcipher_sock_destruct(sk: *mut sock) {
...
#[no_mangle]
unsafe extern "C" fn skcipher_accept_parent_nokey(private: *mut c_void, sk: *mut sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn skcipher_accept_parent(private: *mut c_void, sk: *mut sock) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn algif_skcipher_init() -> int __init {
    static int __init algif_skcipher_init(void)
    {
    return af_alg_register_type(&algif_type_skcipher);
    }
#[no_mangle]
unsafe extern "C" fn algif_skcipher_exit() -> void __exit {
    static void __exit algif_skcipher_exit(void)
    {
    let mut err: c_int = af_alg_unregister_type(&algif_type_skcipher);
    BUG_ON(err);
    }
