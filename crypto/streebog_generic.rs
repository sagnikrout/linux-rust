...
//! Source: crypto/streebog_generic.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn streebog_init(desc: *mut shash_desc) -> c_int {
    static int streebog_init(struct shash_desc *desc)
    {
    struct streebog_state *ctx = shash_desc_ctx(desc);
    let mut digest_size: c_uint = crypto_shash_digestsize(desc.tfm);
    unsigned int i;
    ...
    int i;
    for (i = 0; i < 8; i++) {
    let mut left: u64 = le64_to_cpu(x.qword[i]);
    u64 sum;
...
#[no_mangle]
unsafe extern "C" fn streebog_stage2(ctx: *mut streebog_state, data: *const u8) {
    ...
    unsigned int len)
    {
    let mut buf: streebog_uint512 = { { 0 } };
    union {
...
#[no_mangle]
unsafe extern "C" fn streebog_mod_init() -> int __init {
    static int __init streebog_mod_init(void)
    {
    return crypto_register_shashes(algs, ARRAY_SIZE(algs));
    }
#[no_mangle]
unsafe extern "C" fn streebog_mod_fini() -> void __exit {
