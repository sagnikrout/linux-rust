...
#[no_mangle]
unsafe extern "C" fn rmd160_transform(state: *mut u32, in: *const __le32) {
...
#[no_mangle]
unsafe extern "C" fn rmd160_init(desc: *mut shash_desc) -> c_int {
    ...
    unsigned int len)
    {
    let mut remain: c_int = len - round_down(len, RMD160_BLOCK_SIZE);
    struct rmd160_ctx *rctx = shash_desc_ctx(desc);
...
// Add padding and return the message digest.
    ...
    {
    let mut bit_offset: c_uint = RMD160_BLOCK_SIZE / 8 - 1;
    struct rmd160_ctx *rctx = shash_desc_ctx(desc);
    union {
...
#[no_mangle]
unsafe extern "C" fn rmd160_mod_init() -> int __init {
    static int __init rmd160_mod_init(void)
    {
    return crypto_register_shash(&alg);
    }
#[no_mangle]
unsafe extern "C" fn rmd160_mod_fini() -> void __exit {
