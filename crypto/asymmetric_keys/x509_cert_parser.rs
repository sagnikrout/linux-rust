...
//! Source: crypto/asymmetric_keys/x509_cert_parser.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct x509_parse_context {
    pub /: *mut *mut *mut x509_certificate cert; / Certificate being constructed,
    pub /: *mut *mut unsigned long data; / Start of data,
    pub /: *const *const *const void key; / Key data,
    pub /: *mut *mut size_t key_size; / Size of key data,
    pub /: *const *const *const void params; / Key parameters,
    pub /: *mut *mut size_t params_size; / Size of key parameters,
    pub /: *mut *mut enum OID key_algo; / Algorithm used by the cert's key,
    pub /: *mut *mut enum OID last_oid; / Last OID encountered,
    pub /: *mut *mut enum OID sig_algo; / Algorithm used to sign the cert,
    pub /: *mut *mut u8 o_size; / Size of organizationName (O),
    pub /: *mut *mut u8 cn_size; / Size of commonName (CN),
    pub /: *mut *mut u8 email_size; / Size of emailAddress,
    pub /: *mut *mut u16 o_offset; / Offset of organizationName (O),
    pub /: *mut *mut u16 cn_offset; / Offset of commonName (CN),
    pub /: *mut *mut u16 email_offset; / Offset of emailAddress,
    pub raw_akid_size: unsigned,
    pub /: *const *const *const void raw_akid; / Raw authorityKeyId in ASN.1,
    pub /: *const *const *const void akid_raw_issuer; / Raw directoryName in authorityKeyId,
    pub akid_raw_issuer_size: unsigned,
}
...
//
#[no_mangle]
pub unsafe extern "C" fn x509_free_certificate(cert: *mut x509_certificate) {
...
//
    ...
    struct x509_parse_context *ctx __free(kfree) = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(0x04): vlen > 4 && v[1] == 0x03 && (v[3] &) -> else {
