...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xts_instance_ctx {
    pub spawn: crypto_skcipher_spawn,
    pub tweak_spawn: crypto_cipher_spawn,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct xts_request_ctx {
    pub t: le128,
    pub tail: *mut scatterlist,
...
//
    ...
    let mut cts: bool = (req.cryptlen % XTS_BLOCK_SIZE);
    let mut bs: c_int = XTS_BLOCK_SIZE;
    struct skcipher_walk w;
    let mut t: le128 = rctx.t;
    int err;
    if (second_pass) {
    req = &rctx.subreq;
...
#[no_mangle]
unsafe extern "C" fn xts_xor_tweak_pre(req: *mut skcipher_request, enc: bool) -> c_int {
    static int xts_xor_tweak_pre(struct skcipher_request *req, bool enc)
    {
    return xts_xor_tweak(req, false, enc);
    }
#[no_mangle]
unsafe extern "C" fn xts_xor_tweak_post(req: *mut skcipher_request, enc: bool) -> c_int {
...
unsafe extern "C" fn xts_cts_done(data: *mut c_void, err: c_int) {
    ...
    let mut offset: c_int = req.cryptlen & ~(XTS_BLOCK_SIZE - 1);
    struct xts_request_ctx *rctx = skcipher_request_ctx(req);
    struct skcipher_request *subreq = &rctx.subreq;
    let mut tail: c_int = req.cryptlen % XTS_BLOCK_SIZE;
    le128 b[2];
    int err;
    ...
    XTS_BLOCK_SIZE, core::ptr::null_mut());
    err = crypt(subreq);
    if (err)
    return err;
    scatterwalk_map_and_copy(b, rctx.tail, 0, XTS_BLOCK_SIZE, 0);
...
#[no_mangle]
unsafe extern "C" fn xts_encrypt_done(data: *mut c_void, err: c_int) {
...
#[no_mangle]
unsafe extern "C" fn xts_decrypt_done(data: *mut c_void, err: c_int) {
    ...
    req.cryptlen & ~(XTS_BLOCK_SIZE - 1), core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn xts_init_tfm(tfm: *mut crypto_skcipher) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xts_exit_tfm(tfm: *mut crypto_skcipher) {
...
#[no_mangle]
unsafe extern "C" fn xts_free_instance(inst: *mut skcipher_instance) {
...
#[no_mangle]
unsafe extern "C" fn xts_create(tmpl: *mut crypto_template, tb: *mut rtattr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn xts_module_init() -> int __init {
    static int __init xts_module_init(void)
    {
    return crypto_register_template(&xts_tmpl);
    }
#[no_mangle]
unsafe extern "C" fn xts_module_exit() -> void __exit {
