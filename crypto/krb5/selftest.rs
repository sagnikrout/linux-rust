...
#[no_mangle]
unsafe extern "C" fn dump_sg(sg: *mut scatterlist, limit: c_uint) {
    static void dump_sg(struct scatterlist *sg, unsigned int limit)
    {
    let mut index: c_uint = 0, n = 0;
    for (; sg && limit > 0; sg = sg_next(sg)) {
    let mut off: c_uint = sg.offset, len = umin(sg.length, limit);
    const void *p = kmap_local_page(sg_page(sg));
    limit -= len;
    while (len > 0) {
    let mut part: c_uint = umin(len, 32);
    pr_notice("[%x] %04x: %*phN\n", n, index, part, p + off);
    index += part;
    off += part;
    len -= part;
...
#[no_mangle]
unsafe extern "C" fn prep_buf(buf: *mut krb5_buffer) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn load_buf(buf: *mut krb5_buffer, from: *const c_char) -> c_int {
    static int load_buf(struct krb5_buffer *buf, const char *from)
    {
    let mut len: usize = strlen(from);
    int ret;
    if (len > 1 && from[0] == '\'') {
    PREP_BUF(buf, len - 1);
...
#[no_mangle]
unsafe extern "C" fn clear_buf(buf: *mut krb5_buffer) {
...
//
#[no_mangle]
unsafe extern "C" fn krb5_test_one_prf(test: *const krb5_prf_test) -> c_int {
    ...
    let mut key: krb5_buffer = {}, octet = {}, result = {}, prf = {};
    int ret;
    if (!krb5)
    return -EOPNOTSUPP;
...
//
    ...
    {
    let mut key: krb5_buffer = {}, result = {};
    int ret;
    LOAD_BUF(&key,   test.key);
...
#[no_mangle]
unsafe extern "C" fn krb5_test_one_key(test: *const krb5_key_test) -> c_int {
    static int krb5_test_one_key(const struct krb5_key_test *test)
    {
    const struct krb5_enctype *krb5 = crypto_krb5_find_enctype(test.etype);
    let mut base_key: krb5_buffer = {};
    int ret;
    if (!krb5)
    return -EOPNOTSUPP;
    pr_notice("Running %s %s\n", krb5.name, test.name);
...
//
#[no_mangle]
unsafe extern "C" fn krb5_test_one_enc(test: *const krb5_enc_test, buf: *mut c_void) -> c_int {
    static int krb5_test_one_enc(const struct krb5_enc_test *test, void *buf)
    {
    const struct krb5_enctype *krb5 = crypto_krb5_find_enctype(test.etype);
    struct crypto_aead *ci = core::ptr::null_mut();
    let mut K0: krb5_buffer = {}, Ke = {}, Ki = {}, keys = {};
    let mut conf: krb5_buffer = {}, plain = {}, ct = {};
...
//
#[no_mangle]
unsafe extern "C" fn krb5_test_one_mic(test: *const krb5_mic_test, buf: *mut c_void) -> c_int {
    static int krb5_test_one_mic(const struct krb5_mic_test *test, void *buf)
    {
    const struct krb5_enctype *krb5 = crypto_krb5_find_enctype(test.etype);
    struct crypto_shash *ci = core::ptr::null_mut();
    struct scatterlist sg[1];
    let mut K0: krb5_buffer = {}, Kc = {}, keys = {}, plain = {}, mic = {};
...
// Generate a MIC generation request.
    ...
    ret = crypto_krb5_get_mic(krb5, ci, core::ptr::null_mut(), sg, 1, 1024,
...
// Generate a verification request.
    offset = 0;
    ret = crypto_krb5_verify_mic(krb5, ci, core::ptr::null_mut(), sg, 1, &offset, &len);
    if (ret < 0) {
    CHECK(1);
...
#[no_mangle]
pub unsafe extern "C" fn krb5_selftest() -> c_int {
