...
#[no_mangle]
unsafe extern "C" fn hfs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    static int hfs_read_folio(struct file *file, struct folio *folio)
    {
    return block_read_full_folio(folio, hfs_get_block);
    }
#[no_mangle]
unsafe extern "C" fn hfs_write_failed(mapping: *mut address_space, to: loff_t) {
...
#[no_mangle]
unsafe extern "C" fn hfs_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
    static sector_t hfs_bmap(struct address_space *mapping, sector_t block)
    {
    return generic_block_bmap(mapping, block, hfs_get_block);
    }
#[no_mangle]
unsafe extern "C" fn hfs_release_folio(folio: *mut folio, mask: gfp_t) -> bool {
    ...
    ;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: atomic_read(&node->refcnt)) -> else {
...
#[no_mangle]
unsafe extern "C" fn hfs_direct_IO(iocb: *mut kiocb, iter: *mut iov_iter) -> isize {
...
//
    ...
    HFS_I(inode).flags = 0;
    HFS_I(inode).rsrc_inode = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn hfs_delete_inode(inode: *mut inode) {
    ...
    {
    struct super_block *sb = inode.i_sb;
    let mut log_size: u32 = be32_to_cpu(__log_size);
    u16 count;
    int i;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hfs_iget_data {
    pub key: *mut hfs_cat_key,
    pub rec: *mut hfs_cat_rec,
}

#[no_mangle]
unsafe extern "C" fn hfs_test_inode(inode: *mut inode, data: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn hfs_read_inode(inode: *mut inode, data: *mut c_void) -> c_int {
...
//
    ...
    {
    let mut data: hfs_iget_data = { key, rec };
    struct inode *inode;
    u32 cnid;
    switch (rec.type) {
    case HFS_CDR_DIR:
...
#[no_mangle]
pub unsafe extern "C" fn hfs_write_inode(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
...
// panic?
    ...
    if (!res) {
    let mut idata: hfs_iget_data = { core::ptr::null_mut(), &rec };
...
#[no_mangle]
pub unsafe extern "C" fn hfs_evict_inode(inode: *mut inode) {
    void hfs_evict_inode(struct inode *inode)
    {
    truncate_inode_pages_final(&inode.i_data);
    clear_inode(inode);
    if (HFS_IS_RSRC(inode) && HFS_I(inode).rsrc_inode) {
    HFS_I(HFS_I(inode).rsrc_inode).rsrc_inode = core::ptr::null_mut();
    iput(HFS_I(inode).rsrc_inode);
    }
    }
#[no_mangle]
unsafe extern "C" fn hfs_file_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hfs_file_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hfs_fileattr_get(dentry: *mut dentry, fa: *mut file_kattr) -> c_int {
