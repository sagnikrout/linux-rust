...
//
#[no_mangle]
pub unsafe extern "C" fn free_ea_wmap(inode: *mut inode) {
...
//
    ...
    struct inode *ip = core::ptr::null_mut();	/* child directory inode */
    ino_t ino;
...
// update parent directory inode
    ...
    return rc ? ERR_PTR(rc) : core::ptr::null_mut();
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn jfs_rmdir(dip: *mut inode, dentry: *mut dentry) -> c_int {
...
// free EA pages
    txEA(tid, ip, &JFS_IP(ip).ea, core::ptr::null_mut());
    }
    JFS_IP(ip).ea.flag = 0;
// free ACL from both persistent and working map
    if (JFS_IP(ip).acl.flag & DXD_EXTENT) {
// free ACL pages
    txEA(tid, ip, &JFS_IP(ip).acl, core::ptr::null_mut());
    }
...
//
#[no_mangle]
unsafe extern "C" fn jfs_unlink(dip: *mut inode, dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn commitZeroLink(tid: tid_t, ip: *mut inode) -> i64 {
...
//
#[no_mangle]
pub unsafe extern "C" fn jfs_free_zero_link(ip: *mut inode) {
...
//
    if (JFS_IP(ip).ea.flag & DXD_EXTENT) {
    let mut xaddr: i64 = addressDXD(&JFS_IP(ip).ea);
    let mut xlen: c_int = lengthDXD(&JFS_IP(ip).ea);
...
// free EA extent from working block map
    ...
    txFreeMap(ip, pxdlock, core::ptr::null_mut(), COMMIT_WMAP);
    }
//
// free ACL
//
    if (JFS_IP(ip).acl.flag & DXD_EXTENT) {
    let mut xaddr: i64 = addressDXD(&JFS_IP(ip).acl);
    let mut xlen: c_int = lengthDXD(&JFS_IP(ip).acl);
    struct maplock maplock;	/* maplock for COMMIT_WMAP */
    struct pxd_lock *pxdlock;	/* maplock for COMMIT_WMAP */
...
// This is kind of silly since PATH_MAX == 4K
    let mut copy_size: u32 = min_t(u32, ssize, PSIZE);
...
//
    ...
    goto out3;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: new_ip) -> else {
...
#[no_mangle]
unsafe extern "C" fn jfs_ci_hash(dir: *const dentry, this: *mut qstr) -> c_int {
