...
pub struct ea_buffer {
    pub /: *mut *mut int flag; / Indicates what storage xattr points to,
    pub /: *mut *mut int max_size; / largest xattr that fits in current buffer,
    pub /: *mut *mut dxd_t new_ea; / dxd to replace ea when modifying xattr,
    pub /: *mut *mut *mut metapage mp; / metapage containing ea list,
    pub /: *mut *mut *mut jfs_ea_list xattr; / buffer containing ea list,
}

//
// ea_buffer.flag values
//
pub const EA_INLINE: c_uint = 0x0001;
pub const EA_EXTENT: c_uint = 0x0002;
pub const EA_NEW: c_uint = 0x0004;
pub const EA_MALLOC: c_uint = 0x0008;
//
...
//
#[no_mangle]
unsafe extern "C" fn is_known_namespace(name: *const c_char) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn name_size(ea: *mut jfs_ea) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn copy_name(buffer: *mut c_char, ea: *mut jfs_ea) -> c_int {
    static inline int copy_name(char *buffer, struct jfs_ea *ea)
    {
    let mut len: c_int = ea.namelen;
    if (!is_known_namespace(ea.name)) {
...
//
#[no_mangle]
unsafe extern "C" fn ea_read_inline(ip: *mut inode, ealist: *mut jfs_ea_list) -> c_int {
    static int ea_read_inline(struct inode *ip, struct jfs_ea_list *ealist)
    {
    struct jfs_inode_info *ji = JFS_IP(ip);
    let mut ea_size: c_int = sizeDXD(&ji.ea);
    if (ea_size == 0) {
    ealist.size = 0;
    return 0;
    }
...
//
#[no_mangle]
unsafe extern "C" fn ea_read(ip: *mut inode, ealist: *mut jfs_ea_list) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ea_get(inode: *mut inode, ea_buf: *mut ea_buffer, min_size: c_int) -> c_int {
    ...
    s64 blkno;
    int rc;
    let mut quota_allocation: c_int = 0;
...
// When fsck.jfs clears a bad ea, it doesn't clear the size
    ...
    ea_buf.flag = 0;
    ea_buf.max_size = 0;
    ea_buf.xattr = core::ptr::null_mut();
    return 0;
    }
...
//
    ...
    if (ea_buf.xattr == core::ptr::null_mut())
    return -ENOMEM;
...
// Allocate new blocks to quota.
    ...
    1);
    if (ea_buf.mp == core::ptr::null_mut()) {
    ...
    let mut size: c_int = clamp_t(int, ea_size, 0, EALIST_SIZE(ea_buf.xattr));
...
#[no_mangle]
unsafe extern "C" fn ea_release(inode: *mut inode, ea_buf: *mut ea_buffer) {
    static void ea_release(struct inode *inode, struct ea_buffer *ea_buf)
    {
    if (ea_buf.flag & EA_MALLOC)
    kfree(ea_buf.xattr);
#[no_mangle]
pub unsafe extern "C" fn if(EA_EXTENT: ea_buf->flag &) -> else {
    assert(ea_buf.mp);
...
// ->xattr must point to original ea's metapage
    ...
    txEA(tid, inode, &ji.ea, core::ptr::null_mut());
    if (ji.ea.flag & DXD_INLINE)
    ji.mode2 |= INLINEEA;
    ji.ea.flag = 0;
    ji.ea.size = 0;
...
// If old blocks exist, they must be removed from quota allocation.
    ...
    struct jfs_ea *ea, *old_ea = core::ptr::null_mut(), *next_ea = core::ptr::null_mut();
    struct ea_buffer ea_buf;
    let mut old_ea_size: c_int = 0;
    int xattr_size;
    int new_size;
...
// Found it
    ...
    goto release;
#[no_mangle]
pub unsafe extern "C" fn if(buf_size: size >) -> else {
    size = -ERANGE;
    goto release;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn can_list(ea: *mut jfs_ea) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn jfs_listxattr(dentry: *mut *mut dentry, data: *mut c_char, buf_size: usize) -> isize {
...
// Copy attribute names to buffer
    ...
    if (can_list(ea)) {
    let mut namelen: c_int = copy_name(buffer, ea);
    buffer += namelen + 1;
    }
    }
    release:
