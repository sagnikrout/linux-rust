...
pub const L2MEGABYTE: c_int = 20;

// convert block number to bmap file page number
// Macro flag: #define BLKTODMAPN(b)\
    (((b) >> 13) + ((b) >> 23) + ((b) >> 33) + 3 + 1)
//
// jfs_extendfs()
//
...
//
#[no_mangle]
pub unsafe extern "C" fn jfs_extendfs(sb: *mut super_block, newLVSize: i64, newLogSize: c_int) -> c_int {
    ...
    int newFSCKSize;
    let mut newMapSize: i64 = 0, mapSize;
    s64 XAddress, XSize, nblocks, xoff, xaddr, t64;
    s64 oldLVSize;
    s64 newFSSize;
    s64 VolumeSize;
    let mut newNpages: c_int = 0, nPages, newPage, xlen, t32;
    int tid;
    let mut log_formatted: c_int = 0;
    struct inode *iplist[1];
    struct jfs_superblock *j_sb, *j_sb2;
    s64 old_agsize;
    let mut agsizechanged: c_int = 0;
