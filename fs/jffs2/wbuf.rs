...
pub const MAX_ERASE_FAILURES: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
...
#[no_mangle]
unsafe extern "C" fn jffs2_wbuf_pending_for_ino(c: *mut jffs2_sb_info, ino: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn jffs2_clear_wbuf_ino_list(c: *mut jffs2_sb_info) {
    ...
    c.wbuf_inodes = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn jffs2_wbuf_dirties_inode(c: *mut jffs2_sb_info, ino: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_refile_wbuf_blocks(c: *mut jffs2_sb_info) {
...
pub const REFILE_NOTEMPTY: c_int = 0;
pub const REFILE_ANYWAY: c_int = 1;
#[no_mangle]
unsafe extern "C" fn jffs2_block_refile(c: *mut jffs2_sb_info, jeb: *mut jffs2_eraseblock, allow_empty: c_int) {
...
// It has to have had some nodes or we couldn't be here
    ...
    let mut oldfree: u32 = jeb.free_size;
    jffs2_link_node_ref(c, jeb,
    (jeb.offset+c.sector_size-oldfree) | REF_OBSOLETE,
    oldfree, core::ptr::null_mut());
// convert to wasted
...
// Find a frag which refers to the full_dnode we want to modify
    ...
    eccstr = "corrected";
#[no_mangle]
pub unsafe extern "C" fn if(-EBADMSG: ret ==) -> else {
    else if (ret == -EBADMSG)
...
#[no_mangle]
unsafe extern "C" fn jffs2_wbuf_recover(c: *mut jffs2_sb_info) {
    static void jffs2_wbuf_recover(struct jffs2_sb_info *c)
    {
    struct jffs2_eraseblock *jeb, *new_jeb;
    struct jffs2_raw_node_ref *raw, *next, *first_raw = core::ptr::null_mut();
    size_t retlen;
    int ret;
    let mut nr_refile: c_int = 0;
    unsigned char *buf;
...
// Need to do another write immediately, but it's possible
    ...
    let mut towrite: u32 = (end-start) - ((end-start)%c.wbuf_pagesize);
...
// Argh. We tried. Really we did.
    ...
    kfree(buf);
    if (retlen)
    jffs2_add_physical_node_ref(c, ofs | REF_OBSOLETE, ref_totlen(c, jeb, first_raw), core::ptr::null_mut());
    return;
    }
...
// Now sort out the jffs2_raw_node_refs, moving them from the old to the next block
    ...
    let mut rawlen: u32 = ref_totlen(c, jeb, raw);
    struct jffs2_inode_cache *ic;
    struct jffs2_raw_node_ref *new_ref;
    struct jffs2_raw_node_ref **adjust_ref = core::ptr::null_mut();
    struct jffs2_inode_info *f = core::ptr::null_mut();
...
// Ick. This XATTR mess should be fixed shortly...
    ...
    BUG_ON(xd.node != raw);
    adjust_ref = &xd.node;
    raw.next_in_ino = core::ptr::null_mut();
    ic = core::ptr::null_mut();
...
//
pub const NOPAD: c_int = 0;
pub const PAD_NOACCOUNT: c_int = 1;
pub const PAD_ACCOUNTING: c_int = 2;
#[no_mangle]
unsafe extern "C" fn __jffs2_flush_wbuf(c: *mut jffs2_sb_info, pad: c_int) -> c_int {
...
// Adjust free size of the block if we padded.
    if (pad) {
    let mut waste: u32 = c.wbuf_pagesize - c.wbuf_len;
...
// wbuf_pagesize - wbuf_len is the amount of space that's to be
    ...
    spin_lock(&c.erase_completion_lock);
    jffs2_link_node_ref(c, wbuf_jeb, (c.wbuf_ofs + c.wbuf_len) | REF_OBSOLETE, waste, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_flush_wbuf_gc(c: *mut jffs2_sb_info, ino: u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_flush_wbuf_pad(c: *mut jffs2_sb_info) -> c_int {
...
// take care of alignment to next page
    ...
    let mut vlen: c_int = invecs[invec].iov_len;
...
// retlen = donelen;
    if (jffs2_sum_active()) {
    let mut res: c_int = jffs2_sum_add_kvec(c, invecs, count, (uint32_t) to);
    if (res)
    return res;
    }
    if (c.wbuf_len && ino)
...
//
#[no_mangle]
pub unsafe extern "C" fn jffs2_flash_read(c: *mut jffs2_sb_info, ofs: loff_t, len: usize, retlen: *mut usize, buf: *mut u_char) -> c_int {
    int jffs2_flash_read(struct jffs2_sb_info *c, loff_t ofs, size_t len, size_t *retlen, u_char *buf)
    {
    let mut orbf: loff_t = 0, owbf = 0, lwbf = 0;
    int	ret;
    if (!jffs2_is_writebuffered(c))
...
pub const NR_OOB_SCAN_PAGES: c_int = 4;
// For historical reasons we use only 8 bytes for OOB clean marker
pub const OOB_CM_SIZE: c_int = 8;
...
//
    ...
    {
    int i, ret;
    let mut cmlen: c_int = min_t(int, c.oobavail, OOB_CM_SIZE);
...
//
#[no_mangle]
pub unsafe extern "C" fn jffs2_write_nand_badblock(c: *mut jffs2_sb_info, jeb: *mut jffs2_eraseblock, bad_offset: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn delayed_wbuf_sync(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_dirty_trigger(c: *mut jffs2_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_nand_flash_setup(c: *mut jffs2_sb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_nand_flash_cleanup(c: *mut jffs2_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_dataflash_setup(c: *mut jffs2_sb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_dataflash_cleanup(c: *mut jffs2_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_nor_wbuf_flash_setup(c: *mut jffs2_sb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_nor_wbuf_flash_cleanup(c: *mut jffs2_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_ubivol_setup(c: *mut jffs2_sb_info) -> c_int {
    c.cleanmarker_size = 0;
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_ubivol_cleanup(c: *mut jffs2_sb_info) {
    kfree(c.wbuf);
    }
