...
//
#[no_mangle]
unsafe extern "C" fn check_node_data(c: *mut jffs2_sb_info, tn: *mut jffs2_tmp_dnode_info) -> c_int {
    static int check_node_data(struct jffs2_sb_info *c, struct jffs2_tmp_dnode_info *tn)
    {
    struct jffs2_raw_node_ref *ref = tn.fn.raw;
    let mut err: c_int = 0, pointed = 0;
...
// Calculate how many bytes were already checked
    ...
    let mut adj: c_int = ofs % c.wbuf_pagesize;
    if (likely(adj))
...
// adding and jffs2_flash_read_end() interface.
    err = mtd_point(c.mtd, ofs, len, &retlen, (void **)&buffer, core::ptr::null_mut());
    if (!err && retlen < len) {
...
//
#[no_mangle]
unsafe extern "C" fn check_tn_node(c: *mut jffs2_sb_info, tn: *mut jffs2_tmp_dnode_info) -> c_int {
...
// We only check the data CRC of unchecked nodes
    ...
    {
    struct rb_node *next;
    struct jffs2_tmp_dnode_info *tn = core::ptr::null_mut();
    ...
    next = tn.rb.rb_right;
#[no_mangle]
pub unsafe extern "C" fn if(offset: tn->fn->ofs >=) -> else {
...
#[no_mangle]
unsafe extern "C" fn jffs2_kill_tn(c: *mut jffs2_sb_info, tn: *mut jffs2_tmp_dnode_info) {
...
//
    ...
    {
    let mut fn_end: u32 = tn.fn.ofs + tn.fn.size;
...
// We neither completely obsoleted nor were completely
    ...
    struct jffs2_tmp_dnode_info *insert_point = core::ptr::null_mut();
    while (*link) {
    parent = *link;
...
#[no_mangle]
unsafe extern "C" fn eat_last(root: *mut rb_root, node: *mut rb_node) {
...
// LAST!
    ...
    link = &root.rb_node;
#[no_mangle]
pub unsafe extern "C" fn if(parent->rb_left: node ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn ver_insert(ver_root: *mut rb_root, tn: *mut jffs2_tmp_dnode_info) {
...
// Build final, normal fragtree from tn tree. It doesn't matter which order
    ...
    let mut ver_root: rb_root = RB_ROOT;
    let mut high_ver: u32 = 0;
    if (rii.mdata_tn) {
...
#[no_mangle]
unsafe extern "C" fn jffs2_free_tmp_dnode_info_list(list: *mut rb_root) {
...
#[no_mangle]
unsafe extern "C" fn jffs2_free_full_dirent_list(fd: *mut jffs2_full_dirent) {
...
// FIXME: point()
    int err;
    let mut already: c_int = read - sizeof(*rd);
...
//
    ...
    fd.nhash = full_name_hash(core::ptr::null_mut(), fd.name, rd.nsize);
    fd.next = core::ptr::null_mut();
    fd.name[rd.nsize] = '\0';
//
...
//
#[no_mangle]
pub unsafe extern "C" fn read_unknown(c: *mut jffs2_sb_info, ref: *mut jffs2_raw_node_ref, un: *mut jffs2_unknown_node) -> c_int {
...
//
    ...
    uint32_t offs;
    if (jffs2_is_writebuffered(c)) {
    let mut rem: c_int = to_read % c.wbuf_pagesize;
    if (rem)
...
// FIXME: We could at least crc-check them all
    ...
    rii.mdata_tn = core::ptr::null_mut();
    }
    return ret;
    }
    if (rii.mdata_tn) {
...
// operation.
    let mut csize: u32 = je32_to_cpu(latest_node.csize);
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_do_crccheck_inode(c: *mut jffs2_sb_info, ic: *mut jffs2_inode_cache) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_do_clear_inode(c: *mut jffs2_sb_info, f: *mut jffs2_inode_info) {
    ...
    jffs2_kill_fragtree(&f.fragtree, deleted?c:core::ptr::null_mut());
    fds = f.dents;
    while(fds) {
    fd = fds;
    fds = fd.next;
