...
// Called with erase_completion_lock held
    ...
    struct list_head *nextlist = core::ptr::null_mut();
    let mut n: c_int = jiffies % 128;
...
// Eep. All were empty
    ...
    if (!ret.gc_node) {
    pr_warn("Eep. ret.gc_node for block at 0x%08x is core::ptr::null_mut()\n",
    ret.offset);
    BUG();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn jffs2_garbage_collect_pass(c: *mut jffs2_sb_info) -> c_int {
    ...
    let mut ret: c_int = 0, inum, nlink;
    let mut xattr: c_int = 0;
...
// It's an unknown node with JFFS2_FEATURE_RWCOMPAT_COPY
    jffs2_garbage_collect_pristine(c, core::ptr::null_mut(), raw);
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn block(_arg: s) -> followed by freeing of the ic because the erased {
...
// We're GC'ing an empty block?
    ...
    struct jffs2_full_dnode *fn = core::ptr::null_mut();
    struct jffs2_full_dirent *fd;
    let mut start: u32 = 0, end = 0, nrfrags = 0;
    let mut ret: c_int = 0;
    mutex_lock(&f.sem);
...
// OK, all the CRCs are good; this node can just be copied as-is.
    ...
    if (retlen) {
    jffs2_add_physical_node_ref(c, phys_ofs | REF_OBSOLETE, rawlen, core::ptr::null_mut());
    } else {
...
// this is not the exact summary size of it,
    ...
    char *mdata = core::ptr::null_mut();
    let mut mdatalen: c_int = 0;
...
// On a medium where we can't actually mark nodes obsolete
    ...
    int ret;
    size_t retlen;
    let mut name_len: c_int = strlen(fd.name);
    let mut name_crc: u32 = crc32(0, fd.name, name_len);
    let mut rawlen: u32 = ref_totlen(c, jeb, fd.raw);
...
// Any dirent with the same name is going to have the same length...
    if (ref_totlen(c, core::ptr::null_mut(), raw) != rawlen)
    continue;
...
// from i_size since i_size may have not been updated yet
    ...
    new_fn = jffs2_write_dnode(c, f, &ri, core::ptr::null_mut(), 0, ALLOC_GC);
    if (IS_ERR(new_fn)) {
...
// This is a partially-overlapped hole node. Mark it REF_NORMAL not REF_PRISTINE
    ...
    let mut ret: c_int = 0;
    unsigned char *comprbuf = core::ptr::null_mut(), *writebuf;
    struct folio *folio;
...
// trying to write out, read_cache_folio() will not deadlock.
    ...
    __jffs2_read_folio, core::ptr::null_mut());
    if (IS_ERR(folio)) {
    ...
    uint32_t datalen;
    uint32_t cdatalen;
    let mut comprtype: u16 = JFFS2_COMPR_NONE;
