...
//
    ...
    struct jffs2_full_dirent *fd = core::ptr::null_mut(), *fd_list;
    let mut ino: u32 = 0;
...
// The 'nhash' on the fd_list is not the same as the dentry hash
    nhash = full_name_hash(core::ptr::null_mut(), target.d_name.name, target.d_name.len);
...
//
#[no_mangle]
unsafe extern "C" fn jffs2_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    static int jffs2_readdir(struct file *file, struct dir_context *ctx)
    {
    struct inode *inode = file_inode(file);
    struct jffs2_inode_info *f = JFFS2_INODE_INFO(inode);
    struct jffs2_full_dirent *fd;
    let mut curofs: c_ulong = 1;
...
//
#[no_mangle]
unsafe extern "C" fn jffs2_unlink(dir_i: *mut inode, dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn jffs2_link(old_dentry: *mut dentry, dir_i: *mut inode, dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn jffs2_rmdir(dir_i: *mut inode, dentry: *mut dentry) -> c_int {
...
// Link the fd into the inode's list, obsoleting an old
    ...
    struct jffs2_inode_info *victim_f = core::ptr::null_mut();
    uint8_t type;
    uint32_t now;
...
// Unlink the original
    ...
    old_dentry.d_name.name, old_dentry.d_name.len, core::ptr::null_mut(), now);
