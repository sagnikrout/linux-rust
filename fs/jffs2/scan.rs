...
pub const DEFAULT_EMPTY_SCAN_SIZE: c_int = 256;
...
#[no_mangle]
pub unsafe extern "C" fn min_free(c: *mut jffs2_sb_info) -> c_int {
    static inline int min_free(struct jffs2_sb_info *c)
    {
    let mut min: u32 = 2 * sizeof(struct jffs2_raw_inode);
...
#[no_mangle]
pub unsafe extern "C" fn EMPTY_SCAN_SIZE(sector_size: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn file_dirty(c: *mut jffs2_sb_info, jeb: *mut jffs2_eraseblock) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_scan_medium(c: *mut jffs2_sb_info) -> c_int {
    int jffs2_scan_medium(struct jffs2_sb_info *c)
    {
    int i, ret;
    let mut empty_blocks: u32 = 0, bad_blocks = 0;
    unsigned char *flashbuf = core::ptr::null_mut();
    let mut buf_size: u32 = 0;
    struct jffs2_summary *s = core::ptr::null_mut(); /* summary info collected by the scan process */

    size_t pointlen, try_size;
    ret = mtd_point(c.mtd, 0, c.mtd.size, &pointlen,
    (void **)&flashbuf, core::ptr::null_mut());
    if (!ret && pointlen < c.mtd.size) {
// Don't muck about if it won't let us point to the whole flash
    jffs2_dbg(1, "MTD point returned len too short: 0x%zx\n",
    pointlen);
    mtd_unpoint(c.mtd, 0, pointlen);
    flashbuf = core::ptr::null_mut();
    }
...
// If we're going to start writing into a block which already
    ...
    let mut skip: u32 = c.nextblock.free_size % c.wbuf_pagesize;
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_scan_classify_jeb(c: *mut jffs2_sb_info, jeb: *mut jffs2_eraseblock) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(jeb->unchecked_size)): !ISDIRTY(c->sector_size - (jeb->used_size +) -> else {
    ...
    = jffs2_link_node_ref(c, jeb, ofs | REF_PRISTINE, totlen, core::ptr::null_mut());
...
// Called with 'buf_size == 0' if buf is in fact a pointer _directly_ into
    ...
    let mut cleanmarkerfound: c_int = 0;
...
// scan oob, take care of cleanmarker
    let mut ret: c_int = jffs2_check_oob_empty(c, jeb, cleanmarkerfound);
...
// Wheee. This is an obsoleted node
    ...
    jffs2_link_node_ref(c, jeb, ofs | REF_NORMAL, c.cleanmarker_size, core::ptr::null_mut());
    ...
    jffs2_dbg(1, "Unknown but compatible feature node (0x%04x) found at offset 0x%08x\n",
    je16_to_cpu(node.nodetype), ofs);
    jffs2_link_node_ref(c, jeb, ofs | REF_PRISTINE, PAD(je32_to_cpu(node.totlen)), core::ptr::null_mut());
...
// We believe totlen because the CRC on the node _header_ was OK, just the name failed.
    ...
    fd.nhash = full_name_hash(core::ptr::null_mut(), fd.name, checkedlen);
    fd.type = rd.type;
...
#[no_mangle]
unsafe extern "C" fn count_list(l: *mut list_head) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn rotate_list(head: *mut list_head, count: u32) {
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_rotate_lists(c: *mut jffs2_sb_info) {
