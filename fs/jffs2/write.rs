...
// Don't change raw->size to match retlen. We may have
    ...
    this node */
    jffs2_add_physical_node_ref(c, flash_ofs | REF_OBSOLETE, PAD(sizeof(*ri)+datalen), core::ptr::null_mut());
    } else {
...
// Release the full_dnode which is now useless, and return
    ...
    if (retried) {
    jffs2_dbg_acct_sanity_check(c,core::ptr::null_mut());
    }
    return fn;
    }
...
// This should never happen, but seems to have done on at least one
    ...
    fd.nhash = full_name_hash(core::ptr::null_mut(), name, namelen);
    fd.type = rd.type;
...
// Mark the space as dirtied
    if (retlen) {
    jffs2_add_physical_node_ref(c, flash_ofs | REF_OBSOLETE, PAD(sizeof(*rd)+namelen), core::ptr::null_mut());
    } else {
    pr_notice("Not marking the space at 0x%08x as dirty because the flash driver returned retlen zero\n",
    flash_ofs);
    }
...
//
    ...
    ri.node_crc = cpu_to_je32(crc32(0, ri, sizeof(*ri)-8));
    fn = jffs2_write_dnode(c, f, ri, core::ptr::null_mut(), 0, ALLOC_NORMAL);
...
// File it. This will mark the old one obsolete.
    ...
    let mut nhash: u32 = full_name_hash(core::ptr::null_mut(), name, namelen);
    fd = dir_f.dents;
...
#[no_mangle]
pub unsafe extern "C" fn jffs2_do_link(c: *mut jffs2_sb_info, dir_f: *mut jffs2_inode_info, ino: u32, type: u8, name: *const c_char, namelen: c_int, time: u32) -> c_int {
