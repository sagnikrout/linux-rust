...
//
#[no_mangle]
pub unsafe extern "C" fn lookup_constant(tbl: *const constant_table, name: *const c_char, not_found: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn is_flag(p: *const fs_parameter_spec) -> bool {
    static inline bool is_flag(const struct fs_parameter_spec *p)
    {
    return p.type == core::ptr::null_mut();
    }
    static const struct fs_parameter_spec *fs_lookup_key(
    const struct fs_parameter_spec *desc,
    struct fs_parameter *param, bool *negated)
    {
    const struct fs_parameter_spec *p, *other = core::ptr::null_mut();
    const char *name = param.key;
    let mut want_flag: bool = param.type == fs_value_is_flag;
// negated = false;
    for (p = desc; p.name; p++) {
...
//
    ...
    let mut ret: c_int = p.type(log, p, param, result);
    if (ret)
    return ret;
    }
    return p.opt;
...
//
    ...
    ret = filename_lookup(param.dirfd, f, flags, _path, core::ptr::null_mut());
    if (ret < 0) {
    ...
    path_put(_path);
    _path.dentry = core::ptr::null_mut();
    _path.mnt = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn fs_param_bad_value(log: *mut p_log, param: *mut fs_parameter) -> c_int {
    ...
    struct fs_parameter *param, struct fs_parse_result *result)
    {
    let mut base: c_int = (unsigned long)p.data;
    if (param.type != fs_value_is_string)
    return fs_param_bad_value(log, param);
    if (!*param.string && (p.flags & fs_param_can_be_empty))
    return 0;
