...
// lock cache
    ...
    if (create) {
    let mut blocknr: u32 = affs_alloc_block(inode, ext_bh.b_blocknr);
    if (!blocknr)
    goto err_alloc;
...
// insert first block into header block
    let mut tmp: u32 = be32_to_cpu(AFFS_HEAD(ext_bh).first_data);
    if (tmp)
...
// unlock cache
    ...
    bh_result.b_bdev = core::ptr::null_mut();
// unlock cache
    affs_unlock_ext(inode);
    return -ENOSPC;
    }
...
#[no_mangle]
unsafe extern "C" fn affs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    static int affs_read_folio(struct file *file, struct folio *folio)
    {
    return block_read_full_folio(folio, affs_get_block);
    }
#[no_mangle]
unsafe extern "C" fn affs_write_failed(mapping: *mut address_space, to: loff_t) {
...
#[no_mangle]
unsafe extern "C" fn _affs_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
...
#[no_mangle]
unsafe extern "C" fn affs_do_read_folio_ofs(folio: *mut folio, to: usize, create: c_int) -> c_int {
    ...
    mark_buffer_dirty(bh);
    if (prev_bh) {
    let mut tmp_next: u32 = be32_to_cpu(AFFS_DATA_HEAD(prev_bh).next);
    if (tmp_next)
...
#[no_mangle]
unsafe extern "C" fn affs_read_folio_ofs(file: *mut file, folio: *mut folio) -> c_int {
...
// Truncate (or enlarge) a file to the requested size.
    ...
    int res;
    res = mapping.a_ops.write_begin(core::ptr::null_mut(), mapping, isize, 0, &folio, &fsdata);
    if (!res)
    res = mapping.a_ops.write_end(core::ptr::null_mut(), mapping, isize, 0, 0, folio, fsdata);
    else
...
#[no_mangle]
pub unsafe extern "C" fn affs_file_fsync(filp: *mut file, start: loff_t, end: loff_t, datasync: c_int) -> c_int {
