...
#[no_mangle]
unsafe extern "C" fn journal_brelse_array(b[]: *mut buffer_head, n: c_int) {
...
// fixed value.
//
pub const MAXBUF: c_int = 8;
#[no_mangle]
unsafe extern "C" fn do_readahead(journal: *mut journal_t, start: c_uint) {
...
// a time to the block device IO layer.
    ...
    let mut err: c_int = jbd2_journal_bmap(journal, next, &blocknr);
    if (err) {
...
//
    let mut need_readahead: bool = !buffer_req(bh);
    bh_read_nowait(bh, 0);
    if (need_readahead)
...
#[no_mangle]
unsafe extern "C" fn jbd2_descriptor_block_csum_verify(j: *mut journal_t, buf: *mut c_void) -> c_int {
    ...
    let mut provided: return = = cpu_to_be32(calculated);
    }
//
// Count the number of in-use tags in a journal descriptor block.
//
#[no_mangle]
unsafe extern "C" fn count_tags(journal: *mut journal_t, bh: *mut buffer_head) -> c_int {
    static int count_tags(journal_t *journal, struct buffer_head *bh)
    {
    char *			tagp;
    journal_block_tag_t	tag;
    let mut nr: c_int = 0, size = journal.j_blocksize;
    let mut tag_bytes: c_int = journal_tag_bytes(journal);
...
// Make sure we wrap around the log correctly!
    ...
    struct recovery_info *info, enum passtype pass)
    {
    let mut expected_commit_id: c_uint = info.end_transaction;
...
//
#[no_mangle]
pub unsafe extern "C" fn jbd2_journal_recover(journal: *mut journal_t) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn jbd2_journal_skip_recovery(journal: *mut journal_t) -> c_int {
    ...
    journal_block_tag_t *tag)
    {
    let mut block: c_ulonglong = be32_to_cpu(tag.t_blocknr);
...
#[no_mangle]
unsafe extern "C" fn jbd2_commit_block_csum_verify(j: *mut journal_t, buf: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn jbd2_commit_block_csum_verify_partial(j: *mut journal_t, buf: *mut c_void) -> bool {
    ...
    let mut ret: c_int = 0;
    let mut tag_bytes: c_int = journal_tag_bytes(journal);
    let mut descr_csum_size: c_int = 0;
    unsigned long io_block;
    journal_block_tag_t tag;
    struct buffer_head *obh;
...
// Recover what we can, but report failure at the end.
    ...
    unsigned long long blocknr;
    J_ASSERT(obh != core::ptr::null_mut());
...
// ((__be32 *)nbh->b_data) =
    ...
    let mut last_trans_commit_time: __u64 = 0, commit_time;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(PASS_REVOKE: pass ==) -> else {
//
...
// Scan a revoke record, marking all blocks mentioned as revoked.
    ...
    int offset, max;
    let mut csum_size: unsigned = 0;
    __u32 rcount;
    let mut record_len: c_int = 4;
