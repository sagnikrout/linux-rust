...
#[no_mangle]
unsafe extern "C" fn __first_visible(grp: *const attribute_group, kobj: *mut kobject) -> umode_t {
...
//
    ...
    let mut mode: umode_t = (*bin_attr).attr.mode;
    let mut size: usize = (*bin_attr).size;
    if (update)
...
// Updates may happen before the object has been instantiated
    ...
    if (grp.name) {
    let mut mode: umode_t = __first_visible(grp, kobj);
...
// may have been invisible prior to this update
    ...
    uid, gid, kobj, core::ptr::null_mut());
    if (IS_ERR(kn)) {
