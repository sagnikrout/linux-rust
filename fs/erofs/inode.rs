...
#[no_mangle]
unsafe extern "C" fn erofs_fill_symlink(inode: *mut inode, bptr: *mut c_void, ofs: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn erofs_read_inode(inode: *mut inode) -> c_int {
    static int erofs_read_inode(struct inode *inode)
    {
    struct super_block *sb = inode.i_sb;
    let mut blkaddr: erofs_blk_t = erofs_blknr(sb, erofs_iloc(inode));
    let mut ofs: c_uint = erofs_blkoff(sb, erofs_iloc(inode));
    let mut in_mbox: bool = erofs_inode_in_metabox(inode);
    let mut buf: erofs_buf = __EROFS_BUF_INITIALIZER;
    struct erofs_sb_info *sbi = EROFS_SB(sb);
    let mut addrmask: erofs_blk_t = BIT_ULL(48) - 1;
    struct erofs_inode *vi = EROFS_I(inode);
...
// check if the extended inode acrosses block boundary
    ...
    let mut gotten: c_uint = sb.s_blocksize - ofs;
...
#[no_mangle]
unsafe extern "C" fn erofs_fill_inode(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn erofs_squash_ino(sb: *mut super_block, nid: erofs_nid_t) -> ino_t {
    static ino_t erofs_squash_ino(struct super_block *sb, erofs_nid_t nid)
    {
    let mut ino64: u64 = erofs_nid_to_ino64(EROFS_SB(sb), nid);
    if (sizeof(ino_t) < sizeof(erofs_nid_t))
    ino64 ^= ino64 >> (sizeof(erofs_nid_t) - sizeof(ino_t)) * 8;
    return (ino_t)ino64;
    }
#[no_mangle]
unsafe extern "C" fn erofs_iget5_eq(inode: *mut inode, opaque: *mut c_void) -> c_int {
    static int erofs_iget5_eq(struct inode *inode, void *opaque)
    {
    return EROFS_I(inode).nid == *(erofs_nid_t *)opaque;
    }
#[no_mangle]
unsafe extern "C" fn erofs_iget5_set(inode: *mut inode, opaque: *mut c_void) -> c_int {
    static int erofs_iget5_set(struct inode *inode, void *opaque)
    {
    let mut nid: erofs_nid_t = *(erofs_nid_t *)opaque;
    ...
    let mut err: c_int = erofs_fill_inode(inode);
    if (err) {
    iget_failed(inode);
    return ERR_PTR(err);
    }
    ...
    {
    let mut inode: *mut inode const = d_inode(path.dentry);
...
#[no_mangle]
unsafe extern "C" fn erofs_ioctl_get_volume_label(inode: *mut inode, arg: *mut void __user) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn erofs_ioctl(filp: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
