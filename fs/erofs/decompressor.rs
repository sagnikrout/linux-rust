...
//! Source: fs/erofs/decompressor.c
#![no_std]
...
//
    ...
    {
    struct page *availables[LZ4_MAX_DISTANCE_PAGES] = { core::ptr::null_mut() };
...
//
    ...
    u8 *dst)
    {
    let mut may_inplace: bool = false;
...
// general decoding path which can be used for all cases
    ...
    kunmap_local(dst);
#[no_mangle]
pub unsafe extern "C" fn if(2: dst_maptype ==) -> else {
    ...
    struct page **pagepool)
    {
    let mut nrpages_in: c_uint = rq.inpages, nrpages_out = rq.outpages;
    let mut bs: c_uint = rq.sb.s_blocksize;
    let mut cur: c_uint = 0, ni = 0, no, pi, po, insz, cnt;
    u8 *kin;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: rq->out[no]) -> else {
    else if (rq.out[no])
...
#[no_mangle]
pub unsafe extern "C" fn z_erofs_parse_cfgs(sb: *mut super_block, dsb: *mut erofs_super_block) -> c_int {
    ...
    return z_erofs_load_lz4_config(sb, dsb, core::ptr::null_mut(), 0);
...
#[no_mangle]
pub unsafe extern "C" fn z_erofs_init_decompressor() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn z_erofs_exit_decompressor() {
