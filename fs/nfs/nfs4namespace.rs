...
//
#[no_mangle]
unsafe extern "C" fn nfs4_pathname_len(pathname: *const nfs4_pathname) -> isize {
...
// parse [] escaped IPv6 addrs
    p = strchr(nfspath, ']');
    if (p != core::ptr::null_mut() && ++p < end && *p == ':')
    return p + 1;
    } else {
// otherwise split on first colon
    p = strchr(nfspath, ':');
    if (p != core::ptr::null_mut() && p < end)
    return p + 1;
    }
...
//
    ...
    struct nfs_fs_context *ctx = nfs_fc2context(fc);
    int loc, error;
    if (locations == core::ptr::null_mut() || locations.nlocations <= 0)
    return -ENOENT;
...
// Ensure fs path is a prefix of current dentry path
    ...
    if (location == core::ptr::null_mut() || location.nservers <= 0 ||
...
//
#[no_mangle]
unsafe extern "C" fn nfs_do_refmount(fc: *mut fs_context, client: *mut rpc_clnt) -> c_int {
    static int nfs_do_refmount(struct fs_context *fc, struct rpc_clnt *client)
    {
    struct nfs_fs_context *ctx = nfs_fc2context(fc);
    struct dentry *dentry, *parent;
    struct nfs4_fs_locations *fs_locations = core::ptr::null_mut();
    struct page *page;
...
#[no_mangle]
pub unsafe extern "C" fn nfs4_submount(fc: *mut fs_context, server: *mut nfs_server) -> c_int {
...
//
    ...
    continue;
    if (memchr(buf.data, IPV6_SCOPE_DELIMITER, buf.len) != core::ptr::null_mut())
    continue;
