...
//
pub const NFS4_DEVICE_ID_HASH_BITS: c_int = 5;
    ...
    let mut nbytes: c_uint = NFS4_DEVICEID4_SIZE;
    let mut x: u32 = 0;
    while (nbytes--) {
    x *= 37;
    x += *cptr++;
    ...
    {
    struct nfs4_deviceid_node *d = core::ptr::null_mut();
    struct pnfs_device *pdev = core::ptr::null_mut();
...
//
    ...
    hash);
    if (d != core::ptr::null_mut() && !atomic_inc_not_zero(&d.ref))
    d = core::ptr::null_mut();
    rcu_read_unlock();
    return d;
    }
    ...
    gfp_t gfp_mask)
    {
    let mut hash: c_long = nfs4_deviceid_hash(id);
