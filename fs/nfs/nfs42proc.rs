...
//
    ...
    u64 *copied);
#[no_mangle]
unsafe extern "C" fn nfs42_set_netaddr(filep: *mut file, naddr: *mut nfs42_netaddr) {
    static void nfs42_set_netaddr(struct file *filep, struct nfs42_netaddr *naddr)
    {
    struct nfs_client *clp = (NFS_SERVER(file_inode(filep))).nfs_client;
    let mut port: c_ushort = 2049;
    rcu_read_lock();
...
#[no_mangle]
pub unsafe extern "C" fn nfs42_proc_allocate(filep: *mut file, offset: loff_t, len: loff_t) -> c_int {
    ...
    offset + len);
#[no_mangle]
pub unsafe extern "C" fn if(-EOPNOTSUPP: err ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn nfs42_proc_deallocate(filep: *mut file, offset: loff_t, len: loff_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nfs42_proc_zero_range(filep: *mut file, offset: loff_t, len: loff_t) -> c_int {
    ...
    bool *restart)
    {
    struct nfs4_copy_state *copy, *tmp_copy = core::ptr::null_mut(), *iter;
    struct nfs_open_context *dst_ctx = nfs_file_open_context(dst);
    struct nfs_open_context *src_ctx = nfs_file_open_context(src);
    struct nfs_client *clp = dst_server.nfs_client;
    let mut timeout: c_ulong = 3 * HZ;
    let mut status: c_int = NFS4_OK;
    u64 copied;
...
//
    ...
    let mut newsize: loff_t = pos + len;
    let mut end: loff_t = newsize - 1;
    ...
    let mut pos_src: loff_t = args.src_pos;
    let mut pos_dst: loff_t = args.dst_pos;
    loff_t oldsize_dst;
    let mut count: usize = args.count;
    ssize_t status;
    if (nss) {
    args.cp_src = nss;
    ...
    if (status)
    return status;
    res.commit_res.verf = core::ptr::null_mut();
    if (args.sync) {
    res.commit_res.verf =
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfs42_offload_data {
    pub seq_server: *mut nfs_server,
    pub args: nfs42_offload_status_args,
    pub res: nfs42_offload_status_res,
}

#[no_mangle]
unsafe extern "C" fn nfs42_offload_prepare(task: *mut rpc_task, calldata: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn nfs42_offload_cancel_done(task: *mut rpc_task, calldata: *mut c_void) {
    ...
    if (task.tk_status &&
    nfs4_async_handle_error(task, data.seq_server, core::ptr::null_mut(),
    core::ptr::null_mut()) == -EAGAIN)
    rpc_restart_call_prepare(task);
    }
#[no_mangle]
unsafe extern "C" fn nfs42_offload_release(data: *mut c_void) {
    ...
    struct nfs42_offload_data *data = core::ptr::null_mut();
...
// copied = data->res.osr_count;
    ...
    status = -EINPROGRESS;
#[no_mangle]
pub unsafe extern "C" fn if(NFS_OK: data->res.osr_complete !=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn nfs42_proc_llseek(filep: *mut file, offset: loff_t, whence: c_int) -> loff_t {
...
// Do we need to delay before resending?
    ...
    let mut oldsize_dst: loff_t = i_size_read(dst_inode);
    int status;
    msg.rpc_argp = &args;
    msg.rpc_resp = &res;
...
// a zero-length count means clone to EOF in src
    ...
    struct nfs_lock_context *src_lock;
    struct nfs_lock_context *dst_lock;
    let mut src_exception: nfs4_exception = { };
    let mut dst_exception: nfs4_exception = { };
    int err, err2;
...
#[no_mangle]
unsafe extern "C" fn _nfs42_proc_removexattr(inode: *mut inode, name: *const c_char) -> c_int {
...
//
    nfs4_xattr_cache_add(inode, name, core::ptr::null_mut(), pages, res.xattr_len);
    if (buflen) {
...
#[no_mangle]
pub unsafe extern "C" fn nfs42_proc_removexattr(inode: *mut inode, name: *const c_char) -> c_int {
