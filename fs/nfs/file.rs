...
#[no_mangle]
pub unsafe extern "C" fn nfs_check_flags(flags: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn nfs_revalidate_file_size(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nfs_file_llseek(filp: *mut file, offset: loff_t, whence: c_int) -> loff_t {
...
//
    ...
    let mut retval: c_int = nfs_revalidate_file_size(inode, filp);
    if (retval < 0)
...
//
    ...
    let mut save_nredirtied: c_long = atomic_long_read(&nfsi.redirtied_pages);
    long nredirtied;
    int ret;
    ...
    let mut fpos: loff_t = folio_pos(folio);
    let mut offset: usize = from - fpos;
    let mut end: usize = folio_size(folio);
    if (to - fpos < end)
    end = to - fpos;
...
//
    ...
    unsigned int len)
    {
    let mut pglen: c_uint = nfs_folio_length(folio);
    let mut offset: c_uint = offset_in_folio(folio, pos);
    let mut end: c_uint = offset + len;
...
// foliop = folio;
    ...
    let mut offset: unsigned = offset_in_folio(folio, pos);
    int status;
...
//
    ...
    let mut pglen: unsigned = nfs_folio_length(folio);
    let mut end: unsigned = offset + copied;
    if (pglen == 0) {
...
//
#[no_mangle]
unsafe extern "C" fn nfs_release_folio(folio: *mut folio, gfp: gfp_t) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn nfs_launder_folio(folio: *mut folio) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfs_swap_submit_write(ctx: *mut swap_io_ctx) {
...
#[no_mangle]
unsafe extern "C" fn nfs_swap_submit_read(ctx: *mut swap_io_ctx) {
...
#[no_mangle]
unsafe extern "C" fn nfs_swap_deactivate(file: *mut file) {
...
//
#[no_mangle]
unsafe extern "C" fn nfs_vm_page_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
...
#[no_mangle]
pub unsafe extern "C" fn nfs_file_write(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
// Return error values
    ...
    let mut saved_type: c_uint = fl.c.flc_type;
...
//
#[no_mangle]
pub unsafe extern "C" fn nfs_lock(filp: *mut file, cmd: c_int, fl: *mut file_lock) -> c_int {
    int nfs_lock(struct file *filp, int cmd, struct file_lock *fl)
    {
    struct inode *inode = filp.f_mapping.host;
    let mut ret: c_int = -ENOLCK;
    let mut is_local: c_int = 0;
    ...
    is_local = 1;
    if (NFS_PROTO(inode).lock_check_bounds != core::ptr::null_mut()) {
...
//
#[no_mangle]
pub unsafe extern "C" fn nfs_flock(filp: *mut file, cmd: c_int, fl: *mut file_lock) -> c_int {
