...
// Is this is an attribute revalidation, subject to softreval?
    ...
    if (res.dir_attr == core::ptr::null_mut())
    return -ENOMEM;
    nfs_fattr_init(fattr);
...
// Is this is an attribute revalidation, subject to softreval?
    ...
    {
    const char dotdot[] = "..";
    let mut len: usize = strlen(dotdot);
    let mut task_flags: c_ushort = 0;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfs3_createdata {
    pub msg: rpc_message,
    union {
    pub create: nfs3_createargs,
    pub mkdir: nfs3_mkdirargs,
    pub symlink: nfs3_symlinkargs,
    pub mknod: nfs3_mknodargs,
    pub arg: },
    pub res: nfs3_diropres,
    pub fh: nfs_fh,
    pub fattr: nfs_fattr,
...
#[no_mangle]
unsafe extern "C" fn nfs3_free_createdata(data: *mut nfs3_createdata) {
...
#[no_mangle]
unsafe extern "C" fn nfs3_proc_unlink_rpc_prepare(task: *mut rpc_task, data: *mut nfs_unlinkdata) {
...
#[no_mangle]
unsafe extern "C" fn nfs3_proc_rename_rpc_prepare(task: *mut rpc_task, data: *mut nfs_renamedata) {
    ...
    res.fattr = nfs_alloc_fattr();
    res.dir_attr = nfs_alloc_fattr();
    if (res.fattr == core::ptr::null_mut() || res.dir_attr == core::ptr::null_mut())
    goto out;
    status = rpc_call_sync(NFS_CLIENT(inode), &msg, 0);
    nfs_post_op_update_inode(dir, res.dir_attr);
...
//
    ...
    let mut __read_mostly: static unsigned nfs3_localio_probe_throttle = 0;
...
#[no_mangle]
unsafe extern "C" fn nfs3_localio_probe(server: *mut nfs_server) {
...
#[no_mangle]
unsafe extern "C" fn nfs3_read_done(task: *mut rpc_task, hdr: *mut nfs_pgio_header) -> c_int {
    static int nfs3_read_done(struct rpc_task *task, struct nfs_pgio_header *hdr)
    {
    struct inode *inode = hdr.inode;
    struct nfs_server *server = NFS_SERVER(inode);
    if (hdr.pgio_done_cb != core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn nfs3_write_done(task: *mut rpc_task, hdr: *mut nfs_pgio_header) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfs3_proc_commit_rpc_prepare(task: *mut rpc_task, data: *mut nfs_commit_data) {
    static void nfs3_proc_commit_rpc_prepare(struct rpc_task *task, struct nfs_commit_data *data)
    {
    rpc_call_start(task);
    }
#[no_mangle]
unsafe extern "C" fn nfs3_commit_done(task: *mut rpc_task, data: *mut nfs_commit_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfs3_nlm_alloc_call(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn nfs3_nlm_unlock_prepare(task: *mut rpc_task, data: *mut c_void) -> bool {
...
#[no_mangle]
unsafe extern "C" fn nfs3_nlm_release_call(data: *mut c_void) {
    ...
    struct nfs_lock_context *l_ctx = core::ptr::null_mut();
    struct nfs_open_context *ctx = nfs_file_open_context(filp);
    int status;
    if (fl.c.flc_flags & FL_CLOSE) {
    l_ctx = nfs_get_lock_context(ctx);
    if (IS_ERR(l_ctx))
    l_ctx = core::ptr::null_mut();
    else
...
#[no_mangle]
unsafe extern "C" fn nfs3_have_delegation(inode: *mut inode, type: fmode_t, flags: c_int) -> c_int {
    static int nfs3_have_delegation(struct inode *inode, fmode_t type, int flags)
    {
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn nfs3_return_delegation(inode: *mut inode) {
