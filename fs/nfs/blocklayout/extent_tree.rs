...
//! Source: fs/nfs/blocklayout/extent_tree.c
#![no_std]
...
//
    ...
    return node ? ext_node(node) : core::ptr::null_mut();
    }
    static struct pnfs_block_extent *
    ...
    struct pnfs_block_extent *be = core::ptr::null_mut();
    while (node) {
    be = ext_node(node);
    if (start < be.be_f_offset)
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(ext_f_end(be): start >=) -> else {
...
#[no_mangle]
unsafe extern "C" fn __ext_put_deviceids(head: *mut list_head) {
    ...
    {
    struct pnfs_block_extent *be;
    let mut len1: sector_t = 0, len2 = 0;
    ...
    let mut new_len: sector_t = ext_f_end(new) - ext_f_end(be);
    let mut diff: sector_t = new.be_length - new_len;
    ...
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(ext_f_end(be): isect >=) -> else {
...
// ret = *be;
    ...
    {
    struct pnfs_block_extent *new;
    let mut orig_len: sector_t = be.be_length;
    new = kzalloc_obj(*new, GFP_ATOMIC);
    if (!new)
    return -ENOMEM;
    ...
    let mut end: sector_t = start + len;
    struct pnfs_block_extent *be;
    let mut err: c_int = 0;
    LIST_HEAD(tmp);
    spin_lock(&bl.bl_ext_lock);
...
//
    ...
    struct pnfs_block_extent *left = ext_tree_prev(be);
    if (left && ext_can_merge(left, be)) {
    let mut diff: sector_t = start - be.be_f_offset;
    left.be_length += diff;
    be.be_f_offset += diff;
    be.be_v_offset += diff;
    be.be_length -= diff;
    ...
    struct pnfs_block_extent *right = ext_tree_next(be);
    if (right && ext_can_merge(be, right)) {
    let mut diff: sector_t = end - be.be_f_offset;
    be.be_length -= diff;
...
#[no_mangle]
unsafe extern "C" fn ext_tree_layoutupdate_size(bl: *mut pnfs_block_layout, count: usize) -> usize {
    ...
    let mut nr_pages: c_int = DIV_ROUND_UP(buffer_size, PAGE_SIZE), i;
...
//
    ...
    let mut count: usize = 0, buffer_size = PAGE_SIZE;
    __be32 *start_p;
    int ret;
