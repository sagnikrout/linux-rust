...
//
    ...
    trace_nfs4_cb_getattr(cps.clp, &args.fh, core::ptr::null_mut(),
    -ntohl(res.status));
    goto out;
    }
    ...
    trace_nfs4_cb_recall(cps.clp, &args.fh, core::ptr::null_mut(),
...
// Crazy server!
    ...
    struct inode *ino;
    struct pnfs_layout_hdr *lo;
    let mut rv: u32 = NFS4ERR_NOMATCHING_LAYOUT;
    LIST_HEAD(free_me_list);
    let mut return_range: bool = false;
...
//
    ...
    pnfs_set_layout_stateid(lo, &args.cbl_stateid, core::ptr::null_mut(), true);
...
// Free all lsegs that are attached to commit buckets
    ...
    let mut res: u32 = NFS4ERR_OP_NOT_IN_SESSION;
    if (cps.clp)
...
// FIXME we ignore errors, what should we do?
    ...
    const struct pnfs_layoutdriver_type *ld = core::ptr::null_mut();
    uint32_t i;
    let mut res: __be32 = 0;
    if (!cps.clp) {
...
//
    ...
    int ret;
    int i;
    let mut status: __be32 = htonl(NFS4ERR_BADSESSION);
...
//
    ...
    __be32 status;
    let mut flags: fmode_t = 0;
    let mut schedule_manager: bool = false;
