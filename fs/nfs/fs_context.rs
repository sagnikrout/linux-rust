...
pub const NFS_DEFAULT_VERSION: c_int = 3;

pub const NFS_DEFAULT_VERSION: c_int = 2;

pub const NFS_MAX_CONNECTIONS: c_int = 16;
    enum nfs_param {
    Opt_ac,
    Opt_acdirmax,
    Opt_acdirmin,
    ...
    __fsparam(core::ptr::null_mut(), "intr",		Opt_intr,
    fs_param_neg_with_no|fs_param_deprecated, core::ptr::null_mut()),
...
//
#[no_mangle]
unsafe extern "C" fn nfs_verify_server_address(addr: *mut sockaddr_storage) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn nfs_server_transport_udp_invalid(ctx: *const nfs_fs_context) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn nfs_set_mount_transport_protocol(ctx: *mut nfs_fs_context) {
...
//
    ...
    {
    unsigned int i;
    let mut max_flavor_len: c_uint = ARRAY_SIZE(auth_info.flavors);
...
//
    ...
    while ((p = strsep(&string, ":")) != core::ptr::null_mut()) {
    if (!*p)
    continue;
...
#[no_mangle]
unsafe extern "C" fn nfs_tls_key_verify(key_id: key_serial_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nfs_tls_key_verify(key_id: key_serial_t) -> c_int {
...
//
    ...
    ctx.fscache_uniq = core::ptr::null_mut();
    break;
    case Opt_fscache:
    trace_nfs_mount_assign(param.key, param.string);
    ctx.options |= NFS_OPTION_FSCACHE;
...
// Is the host name protected with square brakcets?
    ...
    if (end == core::ptr::null_mut() || end[1] != ':')
...
#[no_mangle]
pub unsafe extern "C" fn is_remount_fc(fc: *mut fs_context) -> bool {
    static inline bool is_remount_fc(struct fs_context *fc)
    {
    return fc.root != core::ptr::null_mut();
    }
//
...
//
    ...
    let mut extra_flags: c_int = NFS_MOUNT_LEGACY_INTERFACE;
    int ret;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_nfs4_mount_data_v1 {
    pub version: compat_int_t,
    pub flags: compat_int_t,
    pub rsize: compat_int_t,
    pub wsize: compat_int_t,
    pub timeo: compat_int_t,
    pub retrans: compat_int_t,
    pub acregmin: compat_int_t,
    pub acregmax: compat_int_t,
    pub acdirmin: compat_int_t,
    pub acdirmax: compat_int_t,
    pub client_addr: compat_nfs_string,
    pub mnt_path: compat_nfs_string,
    pub hostname: compat_nfs_string,
    pub host_addrlen: compat_uint_t,
    pub host_addr: compat_uptr_t,
    pub proto: compat_int_t,
    pub auth_flavourlen: compat_int_t,
    pub auth_flavours: compat_uptr_t,
}

#[no_mangle]
unsafe extern "C" fn nfs4_compat_mount_data_conv(data: *mut nfs4_mount_data) {
...
//
#[no_mangle]
unsafe extern "C" fn nfs_fs_context_validate(fc: *mut fs_context) -> c_int {
    static int nfs_fs_context_validate(struct fs_context *fc)
    {
    struct nfs_fs_context *ctx = nfs_fc2context(fc);
    struct nfs_subversion *nfs_mod;
    struct sockaddr_storage *sap = &ctx.nfs_server._address;
    let mut max_namelen: c_int = PAGE_SIZE;
    let mut max_pathlen: c_int = NFS_MAXPATHLEN;
...
//
#[no_mangle]
unsafe extern "C" fn nfs_get_tree(fc: *mut fs_context) -> c_int {
    static int nfs_get_tree(struct fs_context *fc)
    {
    struct nfs_fs_context *ctx = nfs_fc2context(fc);
    let mut err: c_int = nfs_fs_context_validate(fc);
    if (err)
    return err;
    if (!ctx.internal)
...
//
#[no_mangle]
unsafe extern "C" fn nfs_fs_context_dup(fc: *mut fs_context, src_fc: *mut fs_context) -> c_int {
    ...
    ctx.client_address		= core::ptr::null_mut();
    ctx.mount_server.hostname	= core::ptr::null_mut();
    ctx.nfs_server.export_path	= core::ptr::null_mut();
    ctx.nfs_server.hostname	= core::ptr::null_mut();
    ctx.fscache_uniq		= core::ptr::null_mut();
    ctx.clone_data.fattr		= core::ptr::null_mut();
    fc.fs_private = ctx;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn nfs_fs_context_free(fc: *mut fs_context) {
...
//
#[no_mangle]
unsafe extern "C" fn nfs_init_fs_context(fc: *mut fs_context) -> c_int {
