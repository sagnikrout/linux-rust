...
//
#[no_mangle]
pub unsafe extern "C" fn allocate_compression_buffers() -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn free_compression_buffers() {
    ...
    ntfs_compression_buffer = core::ptr::null_mut();
    mutex_unlock(&ntfs_cb_lock);
    }
//
...
//
    ...
    {
    let mut pos: loff_t = page_offset(page);
...
// Variables for tag and token parsing.
    ...
    let mut nr_completed_pages: c_int = 0;
// Default error code.
    let mut err: c_int = -EOVERFLOW;
    dp_kaddr = core::ptr::null_mut();
...
// Second stage: finalize completed pages.
    ...
    let mut di: c_int = completed_pages[i];
    dp = dest_pages[di];
//
...
// xpage_done = 1;
    ...
    put_page(dp);
    dest_pages[di] = core::ptr::null_mut();
    }
    }
    return err;
    }
...
// Check if the decompressed sub-block was not full-length.
    ...
    let mut nr_bytes: c_int = do_sb_end - *dest_ofs;
...
// No tokens left in the current tag. Continue with the next tag.
    ...
    if (dp_kaddr)
    kunmap_local(dp_kaddr);
    ntfs_error(core::ptr::null_mut(), "Failed. Returning -EOVERFLOW.");
    goto return_error;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_read_compressed_block(folio: *mut folio) -> c_int {
    ...
    let mut cb_size: u32 = ni.itype.compressed.block_size;
    let mut cb_size_mask: u64 = cb_size - 1UL;
    s64 vcn;
    s64 lcn;
...
//
    let mut nr_pages: c_uint = ntfs_cluster_to_pidx(vol, end_vcn - start_vcn);
    unsigned int xpage, max_page, cur_page, cur_ofs, i, page_ofs, page_index;
    unsigned int cb_clusters, cb_max_ofs;
    int cb_max_page, err = 0;
    struct page **pages;
    int *completed_pages;
    let mut xpage_done: c_uchar = 0;
    struct page *lpage;
...
//
    ...
    if (likely(rl != core::ptr::null_mut())) {
...
// We can sleep from now on, so we drop lock.
    ...
    put_page(page);
    pages[cur_page] = core::ptr::null_mut();
    }
...
// We can't sleep so we need two stages.
    let mut cur2_page: c_uint = cur_page;
    let mut cur_ofs2: c_uint = cur_ofs;
    u8 *cb_pos2 = cb_pos;
...
//
    ...
    put_page(page);
    pages[cur2_page] = core::ptr::null_mut();
    }
...
// Compressed cb, decompress it into the destination page(s).
    let mut prev_cur_page: c_uint = cur_page;
...
// Release the unfinished pages.
    ...
    put_page(page);
    pages[prev_cur_page] = core::ptr::null_mut();
    }
    }
    }
    }
...
// utilize this hash.
//
pub const HASH_MULTIPLIER: c_uint = 0x1E35A7BD;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compress_context {
    ...
    pub rel: c_int,
    pub mxsz: c_int,
    pub HASH_SHIFT]: s16 head[1 <<,
    pub prev: [i16; NTFS_SB_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ntfs_compress_workspace {
    pub pages: *mut page,
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_hash(p: *const u8) -> c_uint {
...
//
    ...
    int best_len)
    {
    let mut inbuf: *const u8  const = pctx.inbuf;
    const u8 * const strptr = &inbuf[i]; /* String we're matching against */
    let mut prev: *mut s16  const = pctx.prev;
    let mut max_len: c_int = min(pctx.bufsize - i, pctx.mxsz);
    let mut nice_len: c_int = min(NICE_MATCH_LEN, max_len);
    let mut depth_remaining: c_int = MAX_SEARCH_DEPTH;
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_skip_position(pctx: *mut compress_context, i: c_int) {
...
#[no_mangle]
unsafe extern "C" fn ntfs_compress_workspace_free(ws: *mut ntfs_compress_workspace) {
    ...
    {
    let mut copied: c_uint = 0, i;
    for (i = 0; i < pages_per_cb && copied < bytes; i++) {
    let mut offset: c_uint = i ? 0 : page_offset;
    let mut len: c_uint = min(bytes - copied, PAGE_SIZE - offset);
    ...
    int sz;
    unsigned int bsz;
    let mut fail: bool = false, allzeroes;
...
// more compressed zeroes, to be followed by some count
    ...
    int i, err;
    let mut cb_clusters: u32 = ni.itype.compressed.block_clusters;
    size_t new_rl_count;
    ...
    let mut input_offset: c_uint = page_offset + p;
    let mut page_idx: c_uint = input_offset >> PAGE_SHIFT;
    const char *input;
    void *addr;
...
// check whether this is all zeroes
    ...
    for (i = 0; bio_size; i++) {
    let mut len: c_uint = min_t(unsigned int, bio_size, PAGE_SIZE);
...
// Do not discard the old compression block until the new one is safe.
    ...
    {
    let mut ws: ntfs_compress_workspace = {};
    struct compress_context *ctx;
    struct folio *folio;
    struct page **pages = core::ptr::null_mut(), *page;
    int pages_per_cb;
    let mut cb_size: c_int = ni.itype.compressed.block_size, cb_off, err = 0;
    int i, ip;
    ...
    folio = read_mapping_folio(mapping, index + i, core::ptr::null_mut());
    if (IS_ERR(folio)) {
    for (ip = 0; ip < i; ip++) {
