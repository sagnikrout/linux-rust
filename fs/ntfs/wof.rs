...
pub const WOF_NAME_LEN: c_int = 17;

    DIV_ROUND_UP(NTFS_WOF_MAX_COMP_UNIT + PAGE_SIZE - 1, PAGE_SIZE)
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ntfs_wof_workspace {
    pub lock: *mut mutex,
    pub codec: *const ntfs_codec_ops,
    pub comp_unit: u32,
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_wof_input_size(ws: *const ntfs_wof_workspace) -> usize {
    static size_t ntfs_wof_input_size(const struct ntfs_wof_workspace *ws)
    {
    return round_up((size_t)ws.comp_unit + 511, 512);
    }
#[no_mangle]
unsafe extern "C" fn ntfs_wof_workspace_prepare(ws: *mut ntfs_wof_workspace) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_wof_free_workspaces() {
    ...
    kvfree(ws.output);
    kvfree(ws.scratch);
    ws.output = core::ptr::null_mut();
    ws.scratch = core::ptr::null_mut();
...
// chunk_size = chunk_data_size;
    ...
    let mut start_sector: sector_t = byte_off >> 9;
    let mut sector_off: u32 = byte_off & ((1 << 9) - 1);
    let mut sectors: u32 = DIV_ROUND_UP(sector_off + bytes_to_read, 512);
    ...
    ctx = ntfs_attr_get_search_ctx(base_ni, core::ptr::null_mut());
    if (!ctx) {
    ret = -ENOMEM;
    goto out_unlock_mrec;
    }
    ret = ntfs_attr_lookup(ni.type, ni.name, ni.name_len,
    CASE_SENSITIVE, 0, core::ptr::null_mut(), 0, ctx);
    if (ret)
    goto out_put_ctx;
    value_length =
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: chunk_idx) -> else {
    else if (chunk_idx)
...
// chunk_size = off[1] - off[0];
    ...
    struct ntfs_attr_search_ctx *ctx;
    let mut input_offset: u32 = chunk_offset & 511;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ntfs_wof_dest {
    pub folios: [*mut folio; NTFS_WOF_MAX_PAGES],
    pub pages: [*mut page; NTFS_WOF_MAX_PAGES],
    ...
    u32 dst_len)
    {
    let mut page_offset: c_uint = offset_in_page(chunk_start);
    struct page *page;
    pgoff_t page_index;
    void *addr;
    int err;
...
//
    ...
    u32 decomp_size)
    {
    let mut chunk_end: loff_t = chunk_file_offset + decomp_size;
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_read_wof_compressed_block(folio: *mut folio) -> c_int {
    ...
    let mut i_size: loff_t = i_size_read(VFS_I(ni));
    let mut folio_start: loff_t = folio_pos(folio);
    let mut folio_end: loff_t = folio_next_pos(folio);
    char *chunk_mem;
    void *input;
    size_t input_size;
    u32 decomp_size;
