...
#[no_mangle]
pub unsafe extern "C" fn ntfs_check_bad_char(wc: *const __le16, wc_len: c_uint) -> c_int {
    static inline int ntfs_check_bad_char(const __le16 *wc, unsigned int wc_len)
    {
    int i;
    for (i = 0; i < wc_len; i++) {
    let mut c: u16 = le16_to_cpu(wc[i]);
    if (c < 0x0020 ||
...
// Check for trailing space or dot.
    ...
    let mut size: u32 = vol.upcase_len;
    ...
    __le16 *upcase = vol.upcase;
    let mut size: u32 = vol.upcase_len, port;
...
// The dcache will handle negative entries.
    ntfs_debug("Done.");
    return d_splice_alias(core::ptr::null_mut(), dent);
    }
    ...
    nls_name.name = core::ptr::null_mut();
...
// Find the WIN32 name corresponding to the matched DOS name.
    ...
    err = ntfs_attr_lookup(AT_FILE_NAME, core::ptr::null_mut(), 0, 0, 0,
    core::ptr::null_mut(), 0, ctx);
    if (unlikely(err)) {
    ntfs_error(vol.sb,
...
#[no_mangle]
unsafe extern "C" fn ntfs_sd_add_everyone(ni: *mut ntfs_inode) -> c_int {
...
//
    ...
    let mut rollback_data: bool = false, rollback_sd = false, rollback_reparse = false;
    struct file_name_attr *fn = core::ptr::null_mut();
    struct standard_information *si = core::ptr::null_mut();
    let mut err: c_int = 0, fn_len, si_len;
    struct inode *vi;
...
//
    ...
    err = ntfs_mft_record_alloc(dir_ni.vol, mode, &ni, core::ptr::null_mut(),
    &ni_mrec);
    if (err) {
    iput(vi);
    return ERR_PTR(err);
...
// Add STANDARD_INFORMATION to inode.
    ...
    rollback_sd = true;
    if (S_ISDIR(mode)) {
    struct index_root *ir = core::ptr::null_mut();
...
// Add DATA attribute to inode.
    err = ntfs_attr_add(ni, AT_DATA, AT_UNNAMED, 0, core::ptr::null_mut(), 0);
    if (err) {
...
//
    ...
    ni = __ntfs_create(idmap, dir, uname, uname_len, S_IFREG | mode, 0, core::ptr::null_mut(), 0);
    kmem_cache_free(ntfs_name_cache, uname);
    if (IS_ERR(ni))
    return PTR_ERR(ni);
...
#[no_mangle]
unsafe extern "C" fn ntfs_check_unlinkable_dir(ctx: *mut ntfs_attr_search_ctx, fn: *mut file_name_attr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntfs_test_inode_attr(vi: *mut inode, data: *mut c_void) -> c_int {
    static int ntfs_test_inode_attr(struct inode *vi, void *data)
    {
    struct ntfs_inode *ni = NTFS_I(vi);
    let mut mft_no: u64 = (u64)(uintptr_t)data;
    if (ni.mft_no != mft_no)
    return 0;
...
//
    ...
    {
    struct ntfs_attr_search_ctx *actx = core::ptr::null_mut();
    struct file_name_attr *fn = core::ptr::null_mut();
    let mut looking_for_dos_name: bool = false, looking_for_win32_name = false;
    let mut case_sensitive_match: bool = true;
    let mut err: c_int = 0;
    struct mft_record *ni_mrec;
    struct super_block *sb;
    let mut link_count_zero: bool = false;
    ntfs_debug("Entering.\n");
    if (need_lock == true) {
    mutex_lock_nested(&ni.mrec_lock, NTFS_INODE_MUTEX_NORMAL);
    mutex_lock_nested(&dir_ni.mrec_lock, NTFS_INODE_MUTEX_PARENT);
...
//
    actx = ntfs_attr_get_search_ctx(ni, core::ptr::null_mut());
    if (!actx) {
    ...
    0, core::ptr::null_mut(), 0, actx)) == 0) {

    unsigned char *s;

    let mut case_sensitive: bool = IGNORE_CASE;
...
//
    ...
    (void *)(uintptr_t)ni.mft_no)) != core::ptr::null_mut()) {
    clear_nlink(attr_vi);
    iput(attr_vi);
    }
    }
...
#[no_mangle]
unsafe extern "C" fn ntfs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
    ...
    let mut err: c_int = 0;
    struct ntfs_inode *ni = NTFS_I(vi);
    __le16 *uname = core::ptr::null_mut();
    int uname_len;
    if (NVolShutdown(vol))
    return -EIO;
    uname_len = ntfs_nlstoucs(vol, dentry.d_name.name, dentry.d_name.len,
    ...
    if (!(vol.vol_flags & VOLUME_IS_DIRTY))
    ntfs_set_volume_flags(vol, VOLUME_IS_DIRTY);
    ni = __ntfs_create(idmap, dir, uname, uname_len, mode, 0, core::ptr::null_mut(), 0);
    kmem_cache_free(ntfs_name_cache, uname);
    if (IS_ERR(ni)) {
    err = PTR_ERR(ni);
    return ERR_PTR(err);
...
#[no_mangle]
unsafe extern "C" fn ntfs_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
//
    ...
    struct file_name_attr *fn = core::ptr::null_mut();
    int fn_len, err = 0;
    struct mft_record *dir_mrec = core::ptr::null_mut(), *ni_mrec = core::ptr::null_mut();
    ntfs_debug("Entering.\n");
    sb = dir_ni.vol.sb;
...
// Done!
    ...
    {
    struct inode *old_inode, *new_inode = core::ptr::null_mut();
    let mut err: c_int = 0;
    int is_dir;
    struct super_block *sb = old_dir.i_sb;
    __le16 *uname_new = core::ptr::null_mut();
    __le16 *uname_old = core::ptr::null_mut();
    int new_name_len;
    int old_name_len;
    struct ntfs_volume *vol = NTFS_SB(sb);
    struct ntfs_inode *old_ni, *new_ni = core::ptr::null_mut();
    struct ntfs_inode *old_dir_ni = NTFS_I(old_dir), *new_dir_ni = NTFS_I(new_dir);
    let mut new_dir_first: bool = false;
    ...
    __le16 *usrc;
    int usrc_len;
    let mut symlen: c_int = strlen(symname);
    if (NVolShutdown(vol))
    return -EIO;
    ...
    mode, rdev, core::ptr::null_mut(), 0);
    break;
    default:
    ni = __ntfs_create(idmap, dir, uname, uname_len,
    mode, 0, core::ptr::null_mut(), 0);
    }
    kmem_cache_free(ntfs_name_cache, uname);
    if (IS_ERR(ni)) {
    err = PTR_ERR(ni);
...
// Find the first file name attribute in the mft record.
    ...
    try_next:
    err = ntfs_attr_lookup(AT_FILE_NAME, core::ptr::null_mut(), 0, CASE_SENSITIVE, 0, core::ptr::null_mut(),
    0, ctx);
    if (unlikely(err)) {
    ntfs_attr_put_search_ctx(ctx);
    unmap_mft_record(ni);
