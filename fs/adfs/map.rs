...
//
    ...
    {
    let mut endbit: c_uint = dm.dm_endbit;
    let mut idmask: u32 = (1 << idlen) - 1;
    unsigned char *map = dm.dm_bh.b_data;
    let mut start: c_uint = dm.dm_startbit;
    ...
    let mut length: c_uint = fragend + 1 - start;
    if (*offset < length)
    return start + *offset;
// offset -= length;
    }
...
//
    ...
    {
    let mut endbit: c_uint = dm.dm_endbit;
    let mut idlen: c_uint = asb.s_idlen;
    let mut frag_idlen: c_uint = idlen <= 15 ? idlen : 15;
    let mut idmask: u32 = (1 << frag_idlen) - 1;
    unsigned char *map = dm.dm_bh.b_data;
    let mut start: c_uint = 8, fragend;
    u32 frag;
...
//
#[no_mangle]
pub unsafe extern "C" fn adfs_map_statfs(sb: *mut super_block, buf: *mut kstatfs) {
...
#[no_mangle]
pub unsafe extern "C" fn adfs_map_lookup(sb: *mut super_block, frag_id: u32, offset: c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn adfs_calczonecheck(sb: *mut super_block, map: *mut c_uchar) -> c_uchar {
...
#[no_mangle]
unsafe extern "C" fn adfs_checkmap(sb: *mut super_block, dm: *mut adfs_discmap) -> c_int {
    static int adfs_checkmap(struct super_block *sb, struct adfs_discmap *dm)
    {
    let mut crosscheck: c_uchar = 0, zonecheck = 1;
    int i;
    ...
    let mut crosscheck: return = = 0xff && zonecheck;
    }
//
...
#[no_mangle]
unsafe extern "C" fn adfs_map_relse(dm: *mut adfs_discmap, nzones: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn adfs_free_map(sb: *mut super_block) {
