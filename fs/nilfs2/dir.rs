...
#[no_mangle]
pub unsafe extern "C" fn nilfs_rec_len_from_disk(dlen: __le16) -> c_uint {
    static inline unsigned int nilfs_rec_len_from_disk(__le16 dlen)
    {
    let mut len: c_uint = le16_to_cpu(dlen);
...
#[no_mangle]
pub unsafe extern "C" fn nilfs_rec_len_to_disk(len: c_uint) -> __le16 {
...
//
#[no_mangle]
pub unsafe extern "C" fn nilfs_chunk_size(inode: *mut inode) -> c_uint {
...
//
#[no_mangle]
unsafe extern "C" fn nilfs_last_byte(inode: *mut inode, page_nr: c_ulong) -> c_uint {
    static unsigned int nilfs_last_byte(struct inode *inode, unsigned long page_nr)
    {
    let mut last_byte: u64 = inode.i_size;
    ...
    unsigned int to)
    {
    let mut pos: loff_t = folio_pos(folio) + from;
...
#[no_mangle]
unsafe extern "C" fn nilfs_check_folio(folio: *mut folio, kaddr: *mut c_char) -> bool {
    static bool nilfs_check_folio(struct folio *folio, char *kaddr)
    {
    struct inode *dir = folio.mapping.host;
    struct super_block *sb = dir.i_sb;
    let mut chunk_size: c_uint = nilfs_chunk_size(dir);
    size_t offs, rec_len;
    let mut limit: usize = folio_size(folio);
...
#[no_mangle]
unsafe extern "C" fn nilfs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
...
//
    ...
    let mut reclen: c_uint = NILFS_DIR_REC_LEN(namelen);
...
#[no_mangle]
pub unsafe extern "C" fn nilfs_inode_by_name(dir: *mut inode, qstr: *const qstr, ino: *mut u64) -> c_int {
...
// ino = le64_to_cpu(de->inode);
    ...
    {
    let mut from: usize = offset_in_folio(folio, de);
    let mut to: usize = from + nilfs_rec_len_from_disk(de.rec_len);
...
//
#[no_mangle]
pub unsafe extern "C" fn nilfs_add_link(dentry: *mut dentry, inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn nilfs_delete_entry(dir: *mut nilfs_dir_entry, folio: *mut folio) -> c_int {
    ...
    size_t from, to;
    struct nilfs_dir_entry *de, *pde = core::ptr::null_mut();
    int err;
...
//
#[no_mangle]
pub unsafe extern "C" fn nilfs_make_empty(inode: *mut inode, parent: *mut inode) -> c_int {
    int nilfs_make_empty(struct inode *inode, struct inode *parent)
    {
    struct address_space *mapping = inode.i_mapping;
    struct folio *folio = filemap_grab_folio(mapping, 0);
    let mut chunk_size: c_uint = nilfs_chunk_size(inode);
    struct nilfs_dir_entry *de;
    int err;
    void *kaddr;
    if (IS_ERR(folio))
...
//
#[no_mangle]
pub unsafe extern "C" fn nilfs_empty_dir(inode: *mut inode) -> c_int {
