...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nilfs_recovery_block {
    pub /*: *mut u64 ino;,
// Inode number of the file that this block
// belongs to
//
    pub /: *mut *mut sector_t blocknr; / block number,
    pub /: *mut *mut __u64 vblocknr; / virtual block number,
    pub /: *mut *mut unsigned long blkoff; / File offset of the data block (per block),
    pub list: list_head,
}

#[no_mangle]
unsafe extern "C" fn nilfs_warn_segment_error(sb: *mut super_block, err: c_int) -> c_int {
...
//
    ...
    {
    let mut blocksize: c_uint = nilfs.ns_blocksize;
    unsigned long size;
    u32 crc;
...
// pbh = NULL;
    ...
    if (check) {
    let mut bytes: c_uint = le16_to_cpu(sr.sr_bytes);
...
// offset += bytes * count;
    ...
    let mut blocknr: sector_t = (*pbh).b_blocknr;
    let mut nitem_per_block: c_uint = (*pbh).b_size / bytes;
    unsigned int bcnt;
...
// INIT_LIST_HEAD(&rb->list);
    ...
    err = 0;
    out:
    brelse(bh);   /* brelse(core::ptr::null_mut()) is just ignored */
    return err;
    }
#[no_mangle]
unsafe extern "C" fn dispose_recovery_list(head: *mut list_head) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nilfs_segment_entry {
    pub list: list_head,
    pub segnum: __u64,
}

#[no_mangle]
unsafe extern "C" fn nilfs_segment_list_add(head: *mut list_head, segnum: __u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn nilfs_dispose_segment_list(head: *mut list_head) {
...
// No need to recover sufile because it will be destroyed on error
    ...
    err2 = err;
    next:
    iput(inode); /* iput(core::ptr::null_mut()) is just ignored */
...
//
    ...
    struct nilfs_recovery_info *ri)
    {
    struct buffer_head *bh_sum = core::ptr::null_mut();
    struct nilfs_segment_summary *sum = core::ptr::null_mut();
    sector_t pseg_start;
    sector_t seg_start, seg_end;  /* Starting/ending DBN of full segment */
    let mut nsalvaged_blocks: c_ulong = 0;
    unsigned int flags;
    u64 seg_seq;
    __u64 segnum, nextnum = 0;
    let mut empty_seg: c_int = 0;
    ...
    let mut state: c_int = RF_INIT_ST;
...
//
#[no_mangle]
unsafe extern "C" fn nilfs_abort_roll_forward(nilfs: *mut the_nilfs) {
...
//
    ...
    __u64 cno;
    LIST_HEAD(segments);
    let mut empty_seg: c_int = 0, scan_newer = 0;
    int ret;
