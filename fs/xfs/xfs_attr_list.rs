...
//
    ...
    int				i;
    let mut error: c_int = 0;
    let mut expected_level: c_uint = 0;
    uint16_t			magic;
    ASSERT(*pbp == core::ptr::null_mut());
    cursor.blkno = 0;
    for (;;) {
...
#[no_mangle]
pub unsafe extern "C" fn if(nodehdr.level: expected_level !=) -> else {
...
// Prerequisite for xfs_attr_is_leaf
    error = xfs_iread_extents(core::ptr::null_mut(), dp, XFS_ATTR_FORK);
    if (error)
    return error;
