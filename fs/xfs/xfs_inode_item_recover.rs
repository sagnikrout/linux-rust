...
//! Source: fs/xfs/xfs_inode_item_recover.c
#![no_std]
...
// instantiate the inode
    ...
    error = xfs_bmbt_change_owner(core::ptr::null_mut(), ip, XFS_DATA_FORK,
    ...
    error = xfs_bmbt_change_owner(core::ptr::null_mut(), ip, XFS_ATTR_FORK,
    I_INO(ip), buffer_list);
    if (error)
    goto out_free_ip;
    }
...
#[no_mangle]
pub unsafe extern "C" fn xfs_log_dinode_has_bigtime(ld: *const xfs_log_dinode) -> bool {
...
// also covers the di_used_blocks union arm:
    ...
    let mut dsize: c_uint = XFS_DFORK_DSIZE(dip, mp);
...
//
    if (dip.di_version >= 3) {
    let mut lsn: xfs_lsn_t = be64_to_cpu(dip.di_lsn);
