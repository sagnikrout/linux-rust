...
//
    ...
    lv.lv_item.li_lv_shadow = core::ptr::null_mut();
...
//
    ...
    struct xfs_log_item	*lip;
    let mut len: c_int = 0;
    let mut iovhdr_res: c_int = 0, split_res = 0, ctx_res = 0;
    int			space_used;
    int			order;
    unsigned int		cpu_nr;
    struct xlog_cil_pcp	*cilpcp;
...
//
    ...
    bool			aborted)
    {
pub const LOG_ITEM_BATCH_SIZE: c_int = 32;
...
//
    ...
    let mut abort: bool = xlog_is_shutdown(ctx.cil.xc_log);
//
...
//
    ...
    let mut lsn: xfs_lsn_t = be64_to_cpu(iclog.ic_header.h_lsn);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xlog_cil_trans_hdr {
    pub oph: [xlog_op_header; 2],
    pub thdr: xfs_trans_header,
    pub lhdr: [xfs_log_iovec; 2],
}
...
//
    ...
    let mut tid: __be32 = cpu_to_be32(tic.t_tid);
...
//
    ...
    let mut num_iovecs: c_int = 0;
    let mut num_bytes: c_int = 0;
    let mut error: c_int = 0;
    struct xlog_cil_trans_hdr thdr;
    let mut lvhdr: xfs_log_vec = {};
    xfs_csn_t		push_seq;
...
// of the chain.
//
    list_sort(core::ptr::null_mut(), &ctx.lv_chain, xlog_cil_order_cmp);
//
...
//
    ...
    {
    struct xfs_cil	*cil = log.l_cilp;
    let mut space_used: c_int = atomic_read(&cil.xc_ctx.space_used);
//
...
//
    ...
    struct xlog	*log)
    {
    let mut seq: xfs_csn_t = log.l_cilp.xc_current_sequence;
...
//
    ...
    struct xfs_cil		*cil = log.l_cilp;
    struct xfs_cil_ctx	*ctx;
    let mut commit_lsn: xfs_lsn_t = NULLCOMMITLSN;
