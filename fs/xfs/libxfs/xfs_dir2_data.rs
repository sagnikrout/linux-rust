...
//! Source: fs/xfs/libxfs/xfs_dir2_data.c
#![no_std]
...
//
    ...
    {
    if (xfs_has_ftype(mp)) {
    let mut ftype: u8 = dep.name[dep.namelen];
...
//
    ...
    xfs_dir2_block_tail_t	*btp=core::ptr::null_mut();	/* block tail */
    ...
    xfs_dir2_leaf_entry_t	*lep=core::ptr::null_mut();	/* block leaf entries */
...
//
    ...
    return __xfs_dir3_data_check(core::ptr::null_mut(), bp);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn if(&bf[1]: dfp ==) -> else {
    else if (dfp == &bf[1])
    bf[1] = bf[2];
//
...
//
    ...
    void				*addr = hdr;
    let mut offset: c_uint = geo.data_entry_offset;
    unsigned int			end;
    ASSERT(hdr.magic == cpu_to_be32(XFS_DIR2_DATA_MAGIC) ||
    hdr.magic == cpu_to_be32(XFS_DIR3_DATA_MAGIC) ||
    hdr.magic == cpu_to_be32(XFS_DIR2_BLOCK_MAGIC) ||
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: prevdup) -> else {
    dfp = xfs_dir2_data_freefind(hdr, bf, prevdup);
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: postdup) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: matchfront) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: matchback) -> else {
    newdup = dup;
