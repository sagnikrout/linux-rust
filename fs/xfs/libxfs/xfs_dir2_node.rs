...
//! Source: fs/xfs/libxfs/xfs_dir2_node.c
#![no_std]
...
// Everything ok in the free block header?
    ...
    {
    struct xfs_mount	*mp = bp.b_mount;
    let mut maxbests: c_int = mp.m_dir_geo.free_max_bests;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: leafhdr.stale) -> else {
//
...
//
    ...
    {
    struct xfs_buf		*curbp = core::ptr::null_mut();	/* current data/free buffer */
    xfs_dir2_db_t		curdb = -1;	/* current data block number */
    xfs_dir2_db_t		curfdb = -1;	/* current free block number */
    xfs_inode_t		*dp;		/* incore directory inode */
    int			error;		/* error return value */
    int			fi;		/* free entry index */
    xfs_dir2_free_t		*free = core::ptr::null_mut();	/* free block structure */
...
#[no_mangle]
pub unsafe extern "C" fn if(hdr2.count: hdr1.count >) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(0): !(error == -ENOSPC && args->total ==) -> else {
    else if (!(error == -ENOSPC && args.total == 0))
    return error;
    }
//
...
//
    oldblk.hashval = xfs_dir2_leaf_lasthash(dp, oldblk.bp, core::ptr::null_mut());
    newblk.hashval = xfs_dir2_leaf_lasthash(dp, newblk.bp, core::ptr::null_mut());
...
//
    ...
    for (i = 0, bp = core::ptr::null_mut(); i < 2; forward = !forward, i++) {
    struct xfs_dir3_icleaf_hdr hdr2;
...
//
    ...
    xfs_alert(mp, " ... fblk is core::ptr::null_mut()");
    }
    xfs_da_mark_sick(args);
    return -EFSCORRUPTED;
    }
...
// fbpp = fbp;
    ...
    struct xfs_inode	*dp = args.dp;
    struct xfs_trans	*tp = args.trans;
    struct xfs_buf		*fbp = core::ptr::null_mut();
    xfs_dir2_db_t		firstfbno;
    xfs_dir2_db_t		lastfbno;
    let mut ifbno: xfs_dir2_db_t = -1;
    let mut dbno: xfs_dir2_db_t = -1;
    xfs_dir2_db_t		fbno;
    xfs_fileoff_t		fo;
    let mut findex: c_int = 0;
    int			error;
//
...
//
    ...
    state.extravalid ? &state.extrablk : core::ptr::null_mut());
    if (rval) {
    goto done;
    }
    blk = &state.path.blk[state.path.active - 1];
...
//
    ...
    rval = error;
#[no_mangle]
pub unsafe extern "C" fn if(XFS_CMP_CASE: rval == -ENOENT && args->cmpresult ==) -> else {
...
//
    ...
    state.path.blk[i].bp = core::ptr::null_mut();
    }
//
...
//
    ...
    state.extrablk.bp = core::ptr::null_mut();
    }
    xfs_da_state_free(state);
    return rval;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: state->extravalid) -> else {
    xfs_trans_brelse(args.trans, state.extrablk.bp);
    state.extrablk.bp = core::ptr::null_mut();
    }
//
