...
//! Source: fs/xfs/libxfs/xfs_dir2_leaf.c
#![no_std]
...
//
    ...
    use_block = i;
#[no_mangle]
pub unsafe extern "C" fn if(length: be16_to_cpu(bestsp[i]) >=) -> else {
    use_block = i;
    break;
    }
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn if(needbytes: *mut *mut *mut (char )bestsp - (char )&ents[leafhdr.count] <) -> else {
//
...
//
    if (args.op_flags & XFS_DA_OP_JUSTCHECK) {
    xfs_trans_brelse(tp, lbp);
    let mut use_block: return = = -1 ? -ENOSPC : 0;
    }
//
...
//
    ...
    struct xfs_buf		*dbp = core::ptr::null_mut();	/* data buffer */
...
//
#[no_mangle]
pub unsafe extern "C" fn if(geo->datablk: db !=) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn if(hashwant: hash <) -> else {
...
//
    error = xfs_dir2_leaf_to_block(args, lbp, core::ptr::null_mut());
    state.path.blk[0].bp = core::ptr::null_mut();
    return error;
    }
