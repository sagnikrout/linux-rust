...
//! Source: fs/xfs/libxfs/xfs_iext_tree.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn xfs_iext_rec_is_empty(rec: *mut xfs_iext_rec) -> bool {
    static bool xfs_iext_rec_is_empty(struct xfs_iext_rec *rec)
    {
    return rec.hi == 0;
    }
#[no_mangle]
pub unsafe extern "C" fn xfs_iext_rec_clear(rec: *mut xfs_iext_rec) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xfs_iext_node {
    pub keys: [u64; KEYS_PER_NODE],    pub ptrs: [*mut c_void; KEYS_PER_NODE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct xfs_iext_leaf {
    pub recs: [xfs_iext_rec; RECS_PER_LEAF],
    pub prev: *mut xfs_iext_leaf,
    pub next: *mut xfs_iext_leaf,
}

#[no_mangle]
pub unsafe extern "C" fn xfs_iext_count(ifp: *mut xfs_ifork) -> xfs_extnum_t {
    inline xfs_extnum_t xfs_iext_count(struct xfs_ifork *ifp)
    {
    return ifp.if_bytes / sizeof(struct xfs_iext_rec);
    }
#[no_mangle]
pub unsafe extern "C" fn xfs_iext_max_recs(ifp: *mut xfs_ifork) -> c_int {
    ...
    xfs_fileoff_t		offset)
    {
    let mut rec_offset: u64 = rec.lo & XFS_IEXT_STARTOFF_MASK;
    let mut rec_len: u32 = rec.hi & XFS_IEXT_LENGTH_MASK;
    ...
    let mut nr_move: c_int = KEYS_PER_NODE / 2;
    let mut nr_keep: c_int = nr_move + (KEYS_PER_NODE & 1);
    let mut i: c_int = 0;
...
// nr_entries = 0;
    ...
    node.ptrs[nr_keep + i] = core::ptr::null_mut();
    }
    if (*pos >= nr_keep) {
// nodep = new;
// pos -= nr_keep;
...
//
    ...
    let mut nr_move: c_int = RECS_PER_LEAF / 2;
    let mut nr_keep: c_int = nr_move + (RECS_PER_LEAF & 1);
    int			i;
// for sequential append operations just spill over into the new node
...
// now that we have a node step into it
    ...
    struct xfs_iext_cursor	*cur)
    {
    let mut new_size: i64 = ifp.if_bytes + sizeof(struct xfs_iext_rec);
    void *new;
...
//
#[no_mangle]
pub unsafe extern "C" fn xfs_iext_inc_seq(ifp: *mut xfs_ifork) {
    ...
    struct xfs_bmbt_irec	*irec)
    {
    let mut offset: xfs_fileoff_t = irec.br_startoff;
    struct xfs_iext_leaf	*new = core::ptr::null_mut();
    int			nr_entries, i;
    xfs_iext_inc_seq(ifp);
    if (ifp.if_height == 0)
    xfs_iext_alloc_root(ifp, cur);
#[no_mangle]
pub unsafe extern "C" fn if(1: ifp->if_height ==) -> else {
...
//
    ...
    let mut nr_prev: c_int = xfs_iext_node_nr_entries(prev, 0), i;
    ...
    let mut nr_next: c_int = xfs_iext_node_nr_entries(next, 0), i;
...
//
    ...
    let mut level: c_int = 2, pos, nr_entries, i;
    ...
    node.ptrs[nr_entries] = core::ptr::null_mut();
...
//
    ...
    goto remove_node;
    if (leaf.prev) {
    let mut nr_prev: c_int = xfs_iext_leaf_nr_entries(ifp, leaf.prev, 0), i;
    ...
    if (leaf.next) {
    let mut nr_next: c_int = xfs_iext_leaf_nr_entries(ifp, leaf.next, 0), i;
...
//
    ...
    struct xfs_ifork	*ifp = xfs_iext_state_to_fork(ip, state);
    struct xfs_iext_leaf	*leaf = cur.leaf;
    let mut offset: xfs_fileoff_t = xfs_iext_leaf_key(leaf, 0);
    int			i, nr_entries;
    trace_xfs_iext_remove(ip, cur, state, _RET_IP_);
    ASSERT(ifp.if_height > 0);
    ASSERT(ifp.if_data != core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn if(0: nr_entries ==) -> else {
