...
//! Source: fs/xfs/xfs_dquot_item_recover.c
#![no_std]
...
//
    ...
    recddq = item.ri_buf[1].iov_base;
    if (recddq == core::ptr::null_mut()) {
    xfs_alert(log.l_mp, "core::ptr::null_mut() dquot in %s.", __func__);
    return -EFSCORRUPTED;
    }
    if (item.ri_buf[1].iov_len < sizeof(struct xfs_disk_dquot)) {
...
//
    error = xfs_trans_read_buf(mp, core::ptr::null_mut(), mp.m_ddev_targp, dq_f.qlf_blkno,
...
//
    if (xfs_has_crc(mp)) {
    let mut lsn: xfs_lsn_t = be64_to_cpu(dqb.dd_lsn);
