...
//! Source: fs/xfs/scrub/quota_repair.c
#![no_std]
...
// Make sure there's a written block backing this dquot
    ...
    let mut offset: xfs_fileoff_t = dq.q_id / qi.qi_dqperchunk;
    let mut nmaps: c_int = 1;
    int			error;
...
// Fix a quota timer so that we can pass the verifier.
    ...
    time64_t		timelimit)
    {
    let mut soft: u64 = be64_to_cpu(softlimit);
    let mut count: u64 = be64_to_cpu(countnow);
    time64_t		new_timer;
    uint32_t		t;
...
// Fix anything the verifiers complain about.
    ...
    let mut buftype: enum xfs_blft = 0;
    int			i;
    int			error;
...
//
    ...
    xfs_dqtype_t		dqtype)
    {
    let mut cursor: xchk_dqiter = { };
    let mut rqi: xrep_quota_info = { .sc = sc };
    struct xfs_dquot	*dq;
    int			error;
