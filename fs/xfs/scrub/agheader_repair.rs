...
//! Source: fs/xfs/scrub/agheader_repair.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_agf_allocbt {
    pub sc: *mut xfs_scrub,
    pub freeblks: xfs_agblock_t,
...
// Update all AGF fields which derive from btree contents.
    ...
    struct xfs_buf		*agf_bp)
    {
    let mut raa: xrep_agf_allocbt = { .sc = sc };
    struct xfs_btree_cur	*cur = core::ptr::null_mut();
    struct xfs_agf		*agf = agf_bp.b_addr;
    struct xfs_mount	*mp = sc.mp;
...
//
    ...
    XFS_AGF_DADDR(mp)),
    XFS_FSS_TO_BB(mp, 1), 0, &agf_bp, core::ptr::null_mut());
    if (error)
    return error;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_agfl {
// Bitmap of alleged AGFL blocks that we're not going to add.
    pub crossed: xagb_bitmap,
// Bitmap of other OWN_AG metadata blocks.
    pub agmetablocks: xagb_bitmap,
// Bitmap of free space.
    pub freesp: *mut xagb_bitmap,
...
// Strike out the blocks that are cross-linked according to the rmapbt.
    ...
    {
    struct xrep_agfl	*ra = priv;
    let mut last_agbno: xfs_agblock_t = agbno + len - 1;
    int			error;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_agfl_fill {
    pub used_extents: xagb_bitmap,
    pub sc: *mut xfs_scrub,
    pub agfl_bno: *mut __be32,
    pub flcount: xfs_agblock_t,
    pub fl_off: c_uint,
}
...
// Fill the AGFL with whatever blocks are in this extent.
    ...
    let mut agbno: xfs_agblock_t = start;
    int			error;
...
//
    ...
    XFS_AG_DADDR(mp, pag_agno(sc.sa.pag),
    XFS_AGFL_DADDR(mp)),
    XFS_FSS_TO_BB(mp, 1), 0, &agfl_bp, core::ptr::null_mut());
    if (error)
    return error;
...
pub const XREP_AGI_LOOKUP_BATCH: c_int = 32;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_agi {
    pub sc: *mut xfs_scrub,
...
// Map of ino -> prev_ino for unlinked inode processing.
    ...
    ragi.iunlink_prev = core::ptr::null_mut();
    if (ragi.iunlink_next)
    xfarray_destroy(ragi.iunlink_next);
    ragi.iunlink_next = core::ptr::null_mut();
...
// Find the btree roots.
    error = xrep_find_ag_btree_roots(sc, agf_bp, fab, core::ptr::null_mut());
    if (error)
    return error;
...
//
    ...
    struct xfs_scrub	*sc = ragi.sc;
    struct xfs_agi		*agi = ragi.agi_bp.b_addr;
    let mut prev_agino: xfs_agino_t = NULLAGINO;
    xfs_agino_t		next_agino;
    let mut error: c_int = 0;
    xagino_bitmap_init(&seen);
    next_agino = be32_to_cpu(agi.agi_unlinked[bucket]);
    while (next_agino != NULLAGINO) {
    let mut agino: xfs_agino_t = next_agino;
...
//
    ...
    let mut agino: xfs_agino_t = XFS_INODE_TO_AGINO(ip);
    let mut bucket: c_uint = agino % XFS_AGI_UNLINKED_BUCKETS;
    int			error;
...
//
    ...
    ragi.lookup_batch[i] = core::ptr::null_mut();
//
...
//
    ...
    struct xfs_inode	*ip;
    let mut prev_agino: xfs_agino_t = NULLAGINO;
    let mut next_agino: xfs_agino_t = ragi.iunlink_heads[bucket];
    let mut error: c_int = 0;
    xagino_bitmap_init(&seen);
    while (next_agino != NULLAGINO) {
    let mut len: c_uint = 1;
...
// Update i_next_iunlinked for the inode @agino.
    ...
    struct xfs_perag	*pag = sc.sa.pag;
    struct xfs_inode	*ip;
    let mut agino: xfarray_idx_t = idx - 1;
    let mut want_rele: bool = false;
    let mut error: c_int = 0;
...
// Log all the iunlink updates we need to finish regenerating the AGI.
    ...
    {
    struct xfs_agi		*agi = ragi.agi_bp.b_addr;
    let mut idx: xfarray_idx_t = XFARRAY_CURSOR_INIT;
    xfs_agino_t		agino;
    unsigned int		i;
    int			error;
...
//
    ...
    XFS_AG_DADDR(mp, pag_agno(sc.sa.pag),
    XFS_AGI_DADDR(mp)),
    XFS_FSS_TO_BB(mp, 1), 0, &ragi.agi_bp, core::ptr::null_mut());
    if (error)
    return error;
