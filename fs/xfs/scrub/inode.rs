...
//
    ...
    let mut agno: xfs_agnumber_t = XFS_INO_TO_AGNO(mp, sc.sm.sm_ino);
    int			error;
...
// Ensure that we have protected against inode allocation/freeing.
    if (agi_bp == core::ptr::null_mut()) {
    ASSERT(agi_bp != core::ptr::null_mut());
    error = -ECANCELED;
    goto out_cancel;
    }
//
...
// Validate di_extsize hint.
    ...
    {
    xfs_failaddr_t		fa;
    let mut value: u32 = be32_to_cpu(dip.di_extsize);
...
// Validate di_cowextsize hint.
    ...
    {
    xfs_failaddr_t		fa;
    let mut value: u32 = be32_to_cpu(dip.di_cowextsize);
//
...
// Walk all the extents to check nextents/naextents/nblocks.
    ...
    if (!xchk_should_check_xref(sc, &error, core::ptr::null_mut()))
    return;
...
//
    ...
    xchk_ino_set_preen(sc, ino);
#[no_mangle]
pub unsafe extern "C" fn if(has_shared: !xfs_is_reflink_inode(sc->ip) &&) -> else {
