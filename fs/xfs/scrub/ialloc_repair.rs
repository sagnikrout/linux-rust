...
//! Source: fs/xfs/scrub/ialloc_repair.c
#![no_std]
...
// Stash the accumulated inobt record for rebuilding.
    ...
    if (xfs_inobt_check_irec(ri.sc.sa.pag, &ri.rie) != core::ptr::null_mut())
    return -EFSCORRUPTED;
...
// Update the AGI counters.
    ...
    let mut freecount: c_uint = ri.icount - ri.iused;
...
// Make sure the records do not overlap in inumber address space.
    ...
    xfarray_idx_t			cur;
    let mut next_agino: xfs_agino_t = 0;
    let mut error: c_int = 0;
...
// Build new inode btrees and dispose of the old one.
    ...
    struct xfs_btree_cur	*fino_cur = core::ptr::null_mut();
    bool			need_finobt;
    int			error;
...
//
    ...
    ino_cur = xfs_inobt_init_cursor(sc.sa.pag, core::ptr::null_mut(), core::ptr::null_mut());
...
// Set up finobt staging cursor.
    if (need_finobt) {
    let mut resv: enum xfs_ag_resv_type = XFS_AG_RESV_METADATA;
    ...
    fino_cur = xfs_finobt_init_cursor(sc.sa.pag, core::ptr::null_mut(), core::ptr::null_mut());
