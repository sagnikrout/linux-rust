...
//! Source: fs/xfs/scrub/parent_repair.c
#![no_std]
...
// Scratch buffers for saving extended attributes
    pub xattr_name: *mut c_uchar,
    pub xattr_value: *mut c_void,
    pub xattr_value_sz: c_uint,
//
...
//
    ...
    int			error;
// inop = NULLFSINO;
    error = xchk_xattr_walk(sc, sc.ip, xrep_parent_lookup_pptr, core::ptr::null_mut(),
    inop);
...
//
    ...
    {
    struct xfs_scrub	*sc = rp.sc;
    let mut parent_ino: xfs_ino_t = NULLFSINO;
    int			error;
//
...
// Count parent pointers so we can reset the file link count.
    rp.parents = 0;
    error = xchk_xattr_walk(sc, ip, xrep_parent_count_pptr, core::ptr::null_mut(), rp);
    if (error)
    return error;
//
