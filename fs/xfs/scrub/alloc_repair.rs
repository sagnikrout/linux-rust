...
//! Source: fs/xfs/scrub/alloc_repair.c
#![no_std]
...
// Check for any obvious conflicts in the free extent.
    ...
    int			error;
    if (xfs_alloc_check_irec(sc.sa.pag, rec) != core::ptr::null_mut())
    return -EFSCORRUPTED;
...
//
    ...
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(bp->ar_startblock: ap->ar_startblock <) -> else {
...
//
    ...
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(bp->ar_blockcount: ap->ar_blockcount <) -> else {
...
//
    ...
    let mut free_agbno: xfs_agblock_t = resv.agbno + resv.used;
    let mut free_aglen: xfs_extlen_t = resv.len - resv.used;
    int			error;
    ASSERT(pag == resv.pag);
...
//
    ...
    struct xfs_perag	*pag = sc.sa.pag;
    struct xfs_agf		*agf = sc.sa.agf_bp.b_addr;
    let mut freesp_btreeblks: c_uint = 0;
//
...
//
    ...
    struct xfs_btree_cur	*cnt_cur;
    struct xfs_perag	*pag = sc.sa.pag;
    let mut needs_resort: bool = false;
    int			error;
//
...
// Allocate cursors for the staged btrees.
    bno_cur = xfs_bnobt_init_cursor(sc.mp, core::ptr::null_mut(), core::ptr::null_mut(), pag);
    xfs_btree_stage_afakeroot(bno_cur, &ra.new_bnobt.afake);
    cnt_cur = xfs_cntbt_init_cursor(sc.mp, core::ptr::null_mut(), core::ptr::null_mut(), pag);
...
// Make sure both btrees are ok after we've rebuilt them.
    ...
    struct xfs_scrub	*sc)
    {
    let mut old_type: __u32 = sc.sm.sm_type;
    int			error;
//
