...
//! Source: fs/xfs/scrub/bmap_repair.c
#![no_std]
...
// What d the REFLINK flag be set when the repair is over?
    pub reflink_scan: enum reflink_scan_state,
...
//
    ...
    let mut ao: xfs_fileoff_t = xfs_bmbt_disk_get_startoff(ba);
    let mut bo: xfs_fileoff_t = xfs_bmbt_disk_get_startoff(bb);
    if (ao > bo)
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(bo: ao <) -> else {
    else if (ao < bo)
    return -1;
    return 0;
    }
...
//
    ...
    {
    struct xfs_bmbt_irec	irec;
    let mut next_off: xfs_fileoff_t = 0;
...
// data structure.
//
    bmap_cur = xfs_bmbt_init_cursor(sc.mp, core::ptr::null_mut(), sc.ip, XFS_STAGING_FORK);
