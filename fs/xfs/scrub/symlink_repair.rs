...
//! Source: fs/xfs/scrub/symlink_repair.c
#![no_std]
...
//
    ...
    bool			hdr_ok;
    int			n;
    let mut nmaps: c_int = XFS_SYMLINK_MAPS;
    int			error;
...
//
    ...
    if (!hdr_ok || (fa != core::ptr::null_mut() && !magic_ok))
    break;
...
//
    if (temp_local) {
    let mut logflags: c_int = XFS_ILOG_CORE;
