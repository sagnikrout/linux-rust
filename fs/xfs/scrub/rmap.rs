...
// Cross-reference with the other btrees.
    ...
    struct xfs_rmap_irec	*irec)
    {
    let mut agbno: xfs_agblock_t = irec.rm_startblock;
    let mut len: xfs_extlen_t = irec.rm_blockcount;
...
//
    ...
    let mut badflag: __be64 = cpu_to_be64(XFS_RMAP_OFF_UNWRITTEN);
    unsigned int		level;
...
// Compare an rmap for AG metadata against the metadata walk.
    ...
    {
    struct xfs_scrub		*sc = bs.sc;
    struct xagb_bitmap		*bmp = core::ptr::null_mut();
    let mut fsbcount: xfs_extlen_t = irec.rm_blockcount;
//
...
// Scrub an rmapbt record.
    ...
    struct xfs_rmap_irec	irec;
    if (xfs_rmap_btrec_to_irec(rec, &irec) != core::ptr::null_mut() ||
    xfs_rmap_check_irec(to_perag(bs.cur.bc_group), &irec) != core::ptr::null_mut()) {
    xchk_btree_set_corrupt(bs.sc, bs.cur, 0);
    return 0;
    }
