...
//! Source: fs/xfs/scrub/dir_repair.c
#![no_std]
...
// Directory entry name, plus the trailing null.
    pub xname: xfs_name,
    pub namebuf: [c_uchar; MAXNAMELEN],
}
...
// Tear down all the incore stuff we created.
    ...
    rd.dir_names = core::ptr::null_mut();
    if (rd.dir_entries)
    xfarray_destroy(rd.dir_entries);
    rd.dir_entries = core::ptr::null_mut();
    }
...
//
    ...
    xfs_ino_t		ino;
    int			error;
    error = xfs_dir_lookup(sc.tp, sc.ip, &xfs_name_dotdot, &ino, core::ptr::null_mut());
    if (error)
    return NULLFSINO;
...
//
    ...
    XFS_DABUF_MAP_HOLE_OK, &bp, XFS_DATA_FORK, core::ptr::null_mut());
    if (error || !bp)
    return error;
    hdr = bp.b_addr;
    oldmagic = hdr.magic;
...
//
    ...
    goto out;
    if (xfs_dir3_block_header_check(bp, I_INO(rd.sc.ip)) != core::ptr::null_mut())
    goto out;
    break;
    ...
    goto out;
    if (xfs_dir3_data_header_check(bp, I_INO(rd.sc.ip)) != core::ptr::null_mut())
    goto out;
    break;
    default:
    goto out;
...
//
    ...
    error = xchk_xattr_walk(rd.sc, ip, xrep_dir_scan_pptr, core::ptr::null_mut(), rd);
    if (error)
    goto scan_done;
...
// Reinitialize the short form directory.
    xrep_dir_init_args(rd, sc.tempip, core::ptr::null_mut());
