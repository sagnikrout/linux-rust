...
//! Source: fs/xfs/scrub/cow_repair.c
#![no_std]
...
//
    ...
    struct xrep_cow		*xc)
    {
    let mut fsbno: xfs_fsblock_t = xc.irec.br_startblock;
    let mut len: xfs_extlen_t = xc.irec.br_blockcount;
    uint32_t		trim;
...
//
    ...
    struct xrep_cow			*xc)
    {
    let mut rc_low: xfs_refcount_irec = { 0 };
    let mut rc_high: xfs_refcount_irec = { 0 };
    let mut rm_low: xfs_rmap_irec = { 0 };
    let mut rm_high: xfs_rmap_irec = { 0 };
...
#[no_mangle]
pub unsafe extern "C" fn if(XFS_SCRUB_IFLAG_FORCE_REBUILD: sc->sm->sm_flags &) -> else {
...
//
    ...
    goto bad;
    if (got.br_startoff < badoff) {
    let mut delta: i64 = badoff - got.br_startoff;
