...
//! Source: fs/xfs/scrub/rtbitmap_repair.c
#![no_std]
...
// Set up to repair the realtime bitmap for this group.
    ...
    let mut blocks: c_ulonglong = mp.m_sb.sb_rbmblocks;
    int			error;
...
// Set bits as needed to round startrtx up to the nearest word.
    ...
    if (bit) {
    let mut len: xfs_rtblock_t = nextrtx - startrtx;
...
//
    ...
    int			error;
    ASSERT(sc.ip != core::ptr::null_mut());
    while (off < len) {
...
//
    ...
    let mut delta: u64 = blocks - rtb.resblks;
    if (delta > UINT_MAX)
    return -EOPNOTSUPP;
