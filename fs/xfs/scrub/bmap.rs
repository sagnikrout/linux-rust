...
//
    ...
    let mut is_repair: bool = xchk_could_repair(sc);
...
// Make sure that we have rmapbt records for this data/attr fork extent.
    ...
    let mut owner: u64 = I_INO(info.sc.ip);
...
// Make sure that we have rmapbt records for this COW fork extent.
    ...
    struct xfs_rmap_irec	rmap;
    unsigned long long	rmap_end;
    let mut owner: u64 = XFS_RMAP_OWN_COW;
...
// Scrub a bmbt record.
    ...
    struct xfs_buf		*bp = core::ptr::null_mut();
...
//
    ...
    if (xfs_bmap_validate_extent(ip, info.whichfork, &irec) != core::ptr::null_mut()) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xchk_bmap_check_rmap_info {
    pub sc: *mut xfs_scrub,
    pub whichfork: c_int,
    pub icur: xfs_iext_cursor,
}
...
//
    ...
    {
    struct xfs_bmbt_irec	irec;
    let mut info: xchk_bmap_info = { core::ptr::null_mut() };
    struct xfs_mount	*mp = sc.mp;
    struct xfs_inode	*ip = sc.ip;
