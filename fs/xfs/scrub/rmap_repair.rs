...
//! Source: fs/xfs/scrub/rmap_repair.c
#![no_std]
...
// Number of non-freespace records found.
    pub nr_records: c_ulonglong,
// bnobt/cntbt contribution to btreeblks
    pub freesp_btblocks: xfs_agblock_t,
// old agf_rmap_blocks counter
    pub old_rmapbt_fsbcount: c_uint,
}
...
// Make sure there's nothing funny about this mapping.
    ...
    int			error;
    if (xfs_rmap_check_irec(sc.sa.pag, rec) != core::ptr::null_mut())
    return -EFSCORRUPTED;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_rmap_stash_run {
    pub rr: *mut xrep_rmap,
    pub owner: u64,
    pub rmap_flags: c_uint,
}
...
//
    ...
    continue;
    error = xrep_rmap_visit_bmbt(core::ptr::null_mut(), &rec, rf);
    if (error)
    return error;
    }
    ...
    struct xfs_rtgroup	*rtg = core::ptr::null_mut();
...
//
    ...
    struct xfs_inode	*ip)
    {
    let mut lock_mode: c_uint = XFS_ILOCK_SHARED;
...
// Record reverse mappings for a file.
    ...
    struct xfs_inode	*ip)
    {
    let mut lock_mode: c_uint = xrep_rmap_scan_ilock(ip);
    int			error;
// Check the data fork.
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_rmap_inodes {
    pub rr: *mut xrep_rmap,
    pub /: *mut *mut xagb_bitmap inobt_blocks; / INOBIT,
    pub /: *mut *mut xagb_bitmap ichunk_blocks; / ICHUNKBIT,
}
...
// Record the inobt blocks.
    ...
    if (xfs_inobt_check_irec(to_perag(cur.bc_group), &irec) != core::ptr::null_mut())
    return -EFSCORRUPTED;
...
//
    sa.agf_bp = core::ptr::null_mut();
    sa.agi_bp = core::ptr::null_mut();
...
// a bnobt cursor.
//
    mcur = xfs_rmapbt_mem_cursor(rr.sc.sa.pag, core::ptr::null_mut(), &rr.rmap_btree);
    ...
    sc.sa.bno_cur = core::ptr::null_mut();
    xfs_btree_del_cursor(mcur, error);
    return error;
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_rmap_agfl {
    pub bitmap: *mut xagb_bitmap,
...
// Set all the cntbt blocks in the bitmap.
    ...
    sc.sa.cnt_cur = core::ptr::null_mut();
    if (error)
    return error;
...
//
    ...
    rmap_cur = xfs_rmapbt_init_cursor(sc.mp, core::ptr::null_mut(), core::ptr::null_mut(), pag);
...
//
    rr.mcur = xfs_rmapbt_mem_cursor(rr.sc.sa.pag, core::ptr::null_mut(),
    &rr.rmap_btree);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_rmap_find_gaps {
    pub rmap_gaps: xagb_bitmap,
