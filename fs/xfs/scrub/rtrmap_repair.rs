...
//! Source: fs/xfs/scrub/rtrmap_repair.c
#![no_std]
...
// Make sure there's nothing funny about this mapping.
    ...
    {
    if (xfs_rtrmap_check_irec(sc.sr.rtg, rec) != core::ptr::null_mut())
    return -EFSCORRUPTED;
...
//
    ...
    continue;
    error = xrep_rtrmap_visit_bmbt(core::ptr::null_mut(), &rec, rf);
    if (error)
    return error;
    }
    return xrep_rtrmap_stash_accumulated(rf);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xrep_rtrmap_stash_run {
    pub rr: *mut xrep_rtrmap,
    pub owner: u64,
}
    ...
    let mut rgbno: xfs_rgblock_t = start;
...
//
    mcur = xfs_rtrmapbt_mem_cursor(rr.sc.sr.rtg, core::ptr::null_mut(), &rr.rtrmap_btree);
    rr.nr_records = 0;
...
//
    ...
    rmap_cur = xfs_rtrmapbt_init_cursor(core::ptr::null_mut(), rtg);
...
// new btree.
//
    rr.mcur = xfs_rtrmapbt_mem_cursor(sc.sr.rtg, core::ptr::null_mut(), &rr.rtrmap_btree);
