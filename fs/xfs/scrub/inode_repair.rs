...
//! Source: fs/xfs/scrub/inode_repair.c
#![no_std]
...
//
    ...
    let mut magic_ok: bool = false;
    let mut unlinked_ok: bool = false;
...
//
    ...
    {
    let mut deadline: c_ulong = jiffies + msecs_to_jiffies(30000);
    unsigned int		lock_mode;
    let mut error: c_int = 0;
    do {
    if (xchk_should_terminate(ri.sc, &error))
...
// Turn di_mode into /something/ recognizable.  Returns true if we succeed.
    ...
    {
    struct xfs_scrub	*sc = ri.sc;
    let mut mode: u16 = be16_to_cpu(dip.di_mode);
    int			error;
...
// Fix any conflicting flags that the verifiers complain about.
    ...
    {
    struct xfs_mount	*mp = sc.mp;
    let mut flags2: u64 = be64_to_cpu(dip.di_flags2);
    let mut flags: u16 = be16_to_cpu(dip.di_flags);
    let mut mode: u16 = be16_to_cpu(dip.di_mode);
...
#[no_mangle]
pub unsafe extern "C" fn if(3: dip->di_version >=) -> else {
...
// Make sure we don't have a garbage file size.
    ...
    {
    struct xfs_scrub	*sc = ri.sc;
    let mut size: u64 = be64_to_cpu(dip.di_size);
    let mut mode: u16 = be16_to_cpu(dip.di_mode);
...
// directory structure must pass verification.
    if (S_ISDIR(mode) &&
    xfs_dir2_sf_verify(sc.mp, dfork_ptr, data_size) != core::ptr::null_mut())
    return true;
// symlink structure must pass verification.
    if (S_ISLNK(mode) &&
    xfs_symlink_shortform_verify(dfork_ptr, data_size) != core::ptr::null_mut())
    return true;
    break;
    case XFS_DINODE_FMT_EXTENTS:
...
// xattr structure must pass verification.
    return xfs_attr_shortform_verify(afork_ptr, attr_size) != core::ptr::null_mut();
    case XFS_DINODE_FMT_EXTENTS:
...
// Make sure the fork offset is a sensible value.
    ...
    let mut bmdr_minsz: usize = xfs_bmdr_space_calc(1);
    let mut lit_sz: c_uint = XFS_LITINO(sc.mp);
...
//
    ...
    uint16_t		mode;
    let mut zap_datafork: bool = false;
    let mut zap_attrfork: bool = ri.zap_acls;
...
// Inode didn't pass dinode verifiers, so fix the raw buffer and retry iget.
    ...
    struct xfs_buf		*bp;
    struct xfs_dinode	*dip;
    let mut ino: xfs_ino_t = sc.sm.sm_ino;
    int			error;
    int			iget_error;
...
//
    ...
    struct xfs_scrub	*sc)
    {
    let mut nlink: c_uint = VFS_I(sc.ip).i_nlink;
    int			error;
//
