...
// Check free space info in a directory data block.
    ...
    struct xfs_mount		*mp = sc.mp;
    u16				tag;
    let mut nr_bestfrees: c_uint = 0;
    let mut nr_frees: c_uint = 0;
...
// Check free space info in a directory leaf1 block.
    ...
    __u16				best;
    __u32				hash;
    let mut lasthash: __u32 = 0;
    __u32				bestcount;
    let mut stale: c_uint = 0;
    int				i;
    int				error;
...
// Check free space information in directories.
    ...
    xfs_dablk_t		dabno;
    let mut last_data_db: xfs_dir2_db_t = 0;
    bool			found;
...
// Check directory tree structure
    error = xchk_da_btree(sc, XFS_DATA_FORK, xchk_dir_rec, core::ptr::null_mut());
    if (error)
    return error;
    if (sc.sm.sm_flags & XFS_SCRUB_OFLAG_CORRUPT)
    return 0;
