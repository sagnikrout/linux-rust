...
//! Source: fs/xfs/xfs_buf_item_recover.c
#![no_std]
...
// recovery.
//
pub const XLOG_BC_TABLE_SIZE: c_int = 64;
...
//
    ...
    uint			len)
    {
    return xlog_find_buffer_cancelled(log, blkno, len) != core::ptr::null_mut();
    }
//
// Check if there is and entry for blkno, len in the buffer cancel record table,
...
//
    ...
    {
    struct xfs_buf_log_format	*buf_f = item.ri_buf[0].iov_base;
    xlog_buf_readahead(log, buf_f.blf_blkno, buf_f.blf_len, core::ptr::null_mut());
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: xlog_add_buffer_cancelled(log, _arg: bf->blf_blkno, _arg: bf->blf_len)) -> else {
...
//
    ...
    uint16_t			magic16;
    uint16_t			magicda;
    char				*warnmsg = core::ptr::null_mut();
//
...
//
    ...
    int			nbits;
    xfs_failaddr_t		fa;
    let mut size_disk_dquot: usize = sizeof(struct xfs_disk_dquot);
    ...
    buf_f.blf_map_size, bit);
    ASSERT(nbits > 0);
    ASSERT(item.ri_buf[i].iov_base != core::ptr::null_mut());
...
//
    fa = core::ptr::null_mut();
    if (buf_f.blf_flags &
    (XFS_BLF_UDQUOT_BUF|XFS_BLF_PDQUOT_BUF|XFS_BLF_GDQUOT_BUF)) {
    if (item.ri_buf[i].iov_base == core::ptr::null_mut()) {
    xfs_alert(mp,
    "XFS: core::ptr::null_mut() dquot in %s.", __func__);
    goto next;
    }
...
//
    ...
    {
    int				i;
    let mut item_index: c_int = 0;
    let mut bit: c_int = 0;
    let mut nbits: c_int = 0;
    let mut reg_buf_offset: c_int = 0;
    let mut reg_buf_bytes: c_int = 0;
...
//
    ...
    continue;
    ASSERT(item.ri_buf[item_index].iov_base != core::ptr::null_mut());
...
//
    ...
    let mut orig_agcount: xfs_agnumber_t = mp.m_sb.sb_agcount;
    let mut orig_rgcount: xfs_rgnumber_t = mp.m_sb.sb_rgcount;
    int				error;
...
//
    ...
    void		*p;
    int		i;
    ASSERT(log.l_buf_cancel_table == core::ptr::null_mut());
    ...
    log.l_buf_cancel_table = core::ptr::null_mut();
    }
