...
//
    ...
    let mut prev_lsn: xfs_lsn_t = NULLCOMMITLSN;
    let mut next_lsn: xfs_lsn_t = NULLCOMMITLSN;
    xfs_lsn_t		lsn;
    bool			in_ail;
...
// Macro flag: #define	xfs_ail_check(a,l)
...
//
    lip = cur ? cur.item : core::ptr::null_mut();
...
// If the existing target is higher than the new target, keep it.
    ...
    let mut type: c_uint = lip.li_type;
    let mut flags: c_ulong = lip.li_flags;
    let mut item_lsn: xfs_lsn_t = lip.li_lsn;
    int			lock_result;
//
...
//
    ...
    DEFINE_WAIT(wait);
    spin_lock(&ailp.ail_lock);
    while (xfs_ail_max(ailp) != core::ptr::null_mut()) {
...
// Insert a log item into the AIL.
    ...
    {
    spin_lock(&ailp.ail_lock);
    xfs_trans_ail_update_bulk(ailp, core::ptr::null_mut(), &lip, 1, lsn);
    }
//
...
//
    ...
    let mut lsn: xfs_lsn_t = lip.li_lsn;
