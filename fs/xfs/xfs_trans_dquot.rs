...
//! Source: fs/xfs/xfs_trans_dquot.c
#![no_std]
...
//
    ...
    blk_res_used = 0;
    if (oqa[i].qt_dquot == core::ptr::null_mut())
    break;
    oq = &oqa[i];
    nq = &nqa[i];
...
//
    ...
    for (i = 0; i < XFS_QM_TRANS_MAXDQS; i++) {
    if (qa[i].qt_dquot == core::ptr::null_mut() ||
    qa[i].qt_dquot == dqp)
    return &qa[i];
    }
    return core::ptr::null_mut();
...
//
    ...
    qtrx = core::ptr::null_mut();
    if (!delta)
    return;
    if (tp.t_dqinfo == core::ptr::null_mut())
...
//
    ...
    ASSERT(qtrx);
    if (qtrx.qt_dquot == core::ptr::null_mut())
    qtrx.qt_dquot = dqp;
...
//
    ...
    {
    unsigned int		i;
    ASSERT(q[0].qt_dquot != core::ptr::null_mut());
    if (q[1].qt_dquot == core::ptr::null_mut()) {
    mutex_lock(&q[0].qt_dquot.q_qlock);
    xfs_trans_dqjoin(tp, q[0].qt_dquot);
    } else if (q[2].qt_dquot == core::ptr::null_mut()) {
    ...
    xfs_dqlockn(q);
    for (i = 0; i < XFS_QM_TRANS_MAXDQS; i++) {
    if (q[i].qt_dquot == core::ptr::null_mut())
    break;
...
//
    ...
    if (qa[0].qt_dquot == core::ptr::null_mut())
    continue;
//
...
//
    if ((dqp = qtrx.qt_dquot) == core::ptr::null_mut())
    break;
    ASSERT(XFS_DQ_IS_LOCKED(dqp));
...
//
    ...
    bool			*fatal)
    {
    let mut hardlimit: xfs_qcnt_t = res.hardlimit;
    let mut softlimit: xfs_qcnt_t = res.softlimit;
    let mut total_count: xfs_qcnt_t = res.reserved + delta;
...
// Change the quota reservations for an inode creation activity.
    ...
    return xfs_trans_reserve_quota_nblks(core::ptr::null_mut(), ip, 0, blocks,
    false);
    return xfs_trans_reserve_quota_nblks(core::ptr::null_mut(), ip, blocks, 0, false);
    }
