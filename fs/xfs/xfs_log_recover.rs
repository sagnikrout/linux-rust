...
//! Source: fs/xfs/xfs_log_recover.c
#![no_std]
...
// new_blk = -1;
    ...
    let mut h_size: c_int = be32_to_cpu(rh.h_size);
...
//
    ...
    xfs_daddr_t		i;
    char			*buffer;
    char			*offset = core::ptr::null_mut();
    struct xlog_rec_header	*head = core::ptr::null_mut();
    let mut error: c_int = 0;
    let mut smallmem: c_int = 0;
    let mut num_blks: c_int = *last_blk - start_blk;
    int			xhdrs;
...
//
    ...
    let mut orig_tail: xfs_daddr_t = *tail_blk;
    buffer = xlog_alloc_buffer(log, 1);
    if (!buffer)
    return -ENOMEM;
//
...
//
    ...
    char			*tmp_buffer;
    let mut first_bad: xfs_daddr_t = XFS_BUF_DADDR_NULL;
...
//
    if (!clean) {
    let mut orig_head: xfs_daddr_t = *head_blk;
...
//
    ...
    char		*buffer;
    int		balign, ealign;
    let mut sectbb: c_int = log.l_sectBBsize;
    let mut end_block: c_int = start_block + blocks;
    int		bufblks;
    let mut error: c_int = 0;
    int		i, j = 0;
//
...
//
    ...
    {
    int			error;
    error = xfs_iget(mp, core::ptr::null_mut(), ino, 0, 0, ipp);
    if (error)
    return error;
...
//
    ...
    let mut fate: enum xlog_recover_reorder = XLOG_REORDER_ITEM_LIST;
...
//
    ...
    {
    let mut error: c_int = 0;
    let mut items_queued: c_int = 0;
    struct xlog_recover_item	*item;
    struct xlog_recover_item	*next;
    LIST_HEAD			(ra_list);
    LIST_HEAD			(done_list);
pub const XLOG_RECOVER_COMMIT_QUEUE_MAX: c_int = 100;
...
//
    ...
    {
    struct xfs_mount	*mp = pag_mount(pag);
    struct xfs_inode	*prev_ip = core::ptr::null_mut();
    struct xfs_inode	*ip;
    xfs_agino_t		prev_agino, agino;
    let mut error: c_int = 0;
    agino = be32_to_cpu(agi.agi_unlinked[bucket]);
    while (agino != NULLAGINO) {
    error = xfs_iget(mp, core::ptr::null_mut(), xfs_agino_to_ino(pag, agino), 0, 0,
    &ip);
    if (error)
    break;
...
//
    ...
    int			bucket;
    int			error;
    error = xfs_read_agi(pag, core::ptr::null_mut(), 0, &agibp);
    if (error) {
//
...
//
    ...
    struct xlog	*log)
    {
    struct xfs_perag	*pag = core::ptr::null_mut();
...
//
    ...
    struct list_head	*buffer_list)
    {
    let mut expected_crc: __le32 = rhead.h_crc, crc, other_crc;
...
//
    ...
    let mut h_version: u32 = be32_to_cpu(rhead.h_version);
    int			hlen;
...
//
    ...
    char			*offset;
    char			*hbp, *dbp;
    let mut error: c_int = 0, h_size, h_len;
    let mut error2: c_int = 0;
    int			bblks, split_bblks;
    let mut hblks: c_int = 1, split_hblks, wrapped_hblks;
    int			i;
...
//
    ...
    return error;
    error = xlog_do_recovery_pass(log, head_blk, tail_blk,
    XLOG_RECOVER_PASS1, core::ptr::null_mut());
    if (error != 0)
    goto out_cancel;
//
...
//
    error = xlog_do_recovery_pass(log, head_blk, tail_blk,
    XLOG_RECOVER_PASS2, core::ptr::null_mut());
    if (!error)
...
//
    ...
    struct xlog	*log)
    {
    let mut nofs_flags: c_uint = memalloc_nofs_save();
    int		error;
