...
pub const XFS_GC_NR_BUFS: c_int = 2;
...
//
    pub old_startblock: xfs_fsblock_t,
    pub new_daddr: xfs_daddr_t,
// Are we writing to a sequential write required zone?
    pub is_seq: bool,
// Open Zone being written to
    pub oz: *mut xfs_open_zone,
// Realtime group currently being reclaimed
    pub victim_rtg: *mut xfs_rtgroup,
// Bio used for reads and writes, including the bvec used by it
    pub /: *mut *mut bio bio; / must be last,
}

pub const XFS_ZONE_GC_RECS: c_int = 1024;
// iterator, needs to be reinitialized for each victim zone
#[repr(C)]
#[derive(Copy, Clone)]
pub struct xfs_zone_gc_iter {
    pub victim_rtg: *mut xfs_rtgroup,
    pub rec_count: c_uint,
    pub rec_idx: c_uint,
    pub next_startblock: xfs_agblock_t,
    pub recs: *mut xfs_rmap_irec,
}
...
// the tail.
//
    pub scratch_folios: [*mut folio; XFS_GC_NR_BUFS],
    pub scratch_size: c_uint,
    pub scratch_available: c_uint,
    pub scratch_head: c_uint,
    pub scratch_tail: c_uint,
//
...
//
    pub reading: list_head,
    pub writing: list_head,
    pub resetting: list_head,
//
...
//
    ...
    xfs_zone_gc_rmap_rec_cmp, core::ptr::null_mut());
    if (error == 0) {
//
...
//
    ...
    iter.victim_rtg = core::ptr::null_mut();
    return 0;
    }
    static bool
    ...
    error = xfs_iget(mp, core::ptr::null_mut(), irec.rm_owner,
...
// chunk_rec = *irec;
    ...
    let mut victim_used: u32 = U32_MAX;
    struct xfs_rtgroup	*victim_rtg = core::ptr::null_mut();
    uint32_t		bit;
...
//
    ...
    struct xfs_open_zone	*oz, *found = core::ptr::null_mut();
...
// daddr += XFS_FSB_TO_BB(mp, oz->oz_allocated);
    ...
    {
    struct xfs_zone_gc_data	*data = chunk.data;
    let mut len: c_uint = chunk.len;
    let mut off: c_uint = data.scratch_head;
    do {
    let mut this_off: c_uint = off % XFS_GC_BUF_SIZE;
    let mut this_len: c_uint = min(len, XFS_GC_BUF_SIZE - this_off);
...
//
    ...
    struct xfs_gc_bio	*chunk)
    {
    let mut iolock: c_uint = XFS_IOLOCK_EXCL | XFS_MMAPLOCK_EXCL;
    struct xfs_zone_gc_data	*data = chunk.data;
...
//
    ...
    struct bio		bio;
    int			error;
    bio_init(&bio, rtg_mount(rtg).m_rtdev_targp.bt_bdev, core::ptr::null_mut(), 0,
    ...
    rtg_group(rtg).xg_next_reset = core::ptr::null_mut();
...
//
    ...
    zi.zi_reset_list = core::ptr::null_mut();
