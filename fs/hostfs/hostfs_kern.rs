...
//! Source: fs/hostfs/hostfs_kern.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct hostfs_fs_info {
    pub host_root_path: *mut c_char,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct hostfs_inode_info {
    pub fd: c_int,
    pub mode: fmode_t,
    pub vfs_inode: inode,
...
// Changed in hostfs_args before the kernel starts running
    ...
    let mut append: static int = 0;
    static const struct inode_operations hostfs_iops;
    static const struct inode_operations hostfs_dir_iops;
    static const struct inode_operations hostfs_link_iops;

#[no_mangle]
unsafe extern "C" fn hostfs_args(options: *mut c_char, add: *mut c_int) -> int __init {
...
// ptr++ = '\0';
    ...
    goto out_free;
#[no_mangle]
pub unsafe extern "C" fn if(PATH_MAX: n ==) -> else {
    n = -E2BIG;
    goto out_free;
    }
    if (*name == '/')
    ...
    if (end == core::ptr::null_mut())
    return name;
// (end + 1) = '\0';
    resolved = kasprintf(GFP_KERNEL, "%s%s", link, name);
    if (resolved == core::ptr::null_mut()) {
    n = -ENOMEM;
    goto out_free;
    }
    kfree(name);
...
#[no_mangle]
unsafe extern "C" fn hostfs_statfs(dentry: *mut dentry, sf: *mut kstatfs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hostfs_evict_inode(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn hostfs_free_inode(inode: *mut inode) {
    static void hostfs_free_inode(struct inode *inode)
    {
    kmem_cache_free(hostfs_inode_cache, HOSTFS_I(inode));
    }
#[no_mangle]
unsafe extern "C" fn hostfs_show_options(seq: *mut seq_file, root: *mut dentry) -> c_int {
    ...
    let mut offset: usize = strlen(root_ino) + 1;
    if (strlen(root_path) > offset)
    seq_show_option(seq, root_path + offset, core::ptr::null_mut());
    if (append)
    seq_puts(seq, ",append");
    return 0;
    }
...
unsafe extern "C" fn hostfs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    ...
    next = ctx.pos;
    seek_dir(dir, next);
    while ((name = read_dir(dir, &next, &ino, &len, &type)) != core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn hostfs_open(ino: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hostfs_file_release(inode: *mut inode, file: *mut file) -> c_int {
    ...
    let mut pos: loff_t = folio_pos(folio);
    let mut count: usize = folio_size(folio);
    char *buffer;
    int ret;
    if (count > i_size - pos)
    count = i_size - pos;
...
#[no_mangle]
unsafe extern "C" fn hostfs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    static int hostfs_read_folio(struct file *file, struct folio *folio)
    {
    char *buffer;
    let mut start: loff_t = folio_pos(folio);
...
#[no_mangle]
unsafe extern "C" fn hostfs_inode_update(ino: *mut inode, st: *const hostfs_stat) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hostfs_inode_set(ino: *mut inode, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hostfs_inode_test(inode: *mut inode, data: *mut c_void) -> c_int {
    static int hostfs_inode_test(struct inode *inode, void *data)
    {
    const struct hostfs_stat *st = data;
    let mut dev: dev_t = MKDEV(st.dev.maj, st.dev.min);
    ...
    let mut err: c_int = stat_file(name, &st, -1);
    if (err)
    return ERR_PTR(err);
    ...
    int err;
    if ((from_name = dentry_name(from)) == core::ptr::null_mut())
    return -ENOMEM;
    to_name = dentry_name(to);
    if (to_name == core::ptr::null_mut()) {
    __putname(from_name);
    return -ENOMEM;
    }
...
#[no_mangle]
unsafe extern "C" fn hostfs_unlink(ino: *mut inode, dentry: *mut dentry) -> c_int {
    ...
    if (append)
    return -EPERM;
    if ((file = dentry_name(dentry)) == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn hostfs_rmdir(ino: *mut inode, dentry: *mut dentry) -> c_int {
    ...
    if (old_name == core::ptr::null_mut())
    return -ENOMEM;
    new_name = dentry_name(new_dentry);
    if (new_name == core::ptr::null_mut()) {
    __putname(old_name);
    return -ENOMEM;
    }
    if (!flags)
    ...
    char *name;
    int err;
    let mut fd: c_int = HOSTFS_I(inode).fd;
...
#[no_mangle]
unsafe extern "C" fn hostfs_fill_super(sb: *mut super_block, fc: *mut fs_context) -> c_int {
    ...
    if (sb.s_root == core::ptr::null_mut())
    return -ENOMEM;
    return 0;
    }
    enum hostfs_parma {
...
#[no_mangle]
unsafe extern "C" fn hostfs_parse_param(fc: *mut fs_context, param: *mut fs_parameter) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hostfs_parse_monolithic(fc: *mut fs_context, data: *mut c_void) -> c_int {
    static int hostfs_parse_monolithic(struct fs_context *fc, void *data)
    {
    struct hostfs_fs_info *fsi = fc.s_fs_info;
    char *tmp_root, *host_root = (char *)data;
// NULL is printed as '(null)' by printf(): avoid that.
    if (host_root == core::ptr::null_mut())
    return 0;
...
#[no_mangle]
unsafe extern "C" fn hostfs_fc_get_tree(fc: *mut fs_context) -> c_int {
    static int hostfs_fc_get_tree(struct fs_context *fc)
    {
    return get_tree_nodev(fc, hostfs_fill_super);
    }
#[no_mangle]
unsafe extern "C" fn hostfs_fc_free(fc: *mut fs_context) {
...
#[no_mangle]
unsafe extern "C" fn hostfs_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hostfs_kill_sb(s: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn init_hostfs() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_hostfs() -> void __exit {
