...
#[no_mangle]
pub unsafe extern "C" fn buffer_size(prz: *mut persistent_ram_zone) -> usize {
    static inline size_t buffer_size(struct persistent_ram_zone *prz)
    {
    return atomic_read(&prz.buffer.size);
    }
#[no_mangle]
pub unsafe extern "C" fn buffer_start(prz: *mut persistent_ram_zone) -> usize {
...
#[no_mangle]
unsafe extern "C" fn buffer_start_add(prz: *mut persistent_ram_zone, a: usize) -> usize {
...
#[no_mangle]
unsafe extern "C" fn buffer_size_add(prz: *mut persistent_ram_zone, a: usize) {
...
// Initialize the parity buffer
    ...
    uint8_t *block;
    uint8_t *par;
    let mut ecc_block_size: c_int = prz.ecc_info.block_size;
    let mut ecc_size: c_int = prz.ecc_info.ecc_size;
    let mut size: c_int = ecc_block_size;
    if (!ecc_size)
    return;
...
#[no_mangle]
unsafe extern "C" fn persistent_ram_update_header_ecc(prz: *mut persistent_ram_zone) {
...
#[no_mangle]
unsafe extern "C" fn persistent_ram_ecc_old(prz: *mut persistent_ram_zone) {
    ...
    int numerr;
    let mut size: c_int = prz.ecc_info.block_size;
...
//
    ...
    if (prz.rs_decoder == core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn persistent_ram_save_old(prz: *mut persistent_ram_zone) {
    void persistent_ram_save_old(struct persistent_ram_zone *prz)
    {
    struct persistent_ram_buffer *buffer = prz.buffer;
    let mut size: usize = buffer_size(prz);
    let mut start: usize = buffer_start(prz);
    if (!size)
    return;
//
...
#[no_mangle]
pub unsafe extern "C" fn persistent_ram_old_size(prz: *mut persistent_ram_zone) -> usize {
...
#[no_mangle]
pub unsafe extern "C" fn persistent_ram_free_old(prz: *mut persistent_ram_zone) {
    void persistent_ram_free_old(struct persistent_ram_zone *prz)
    {
    kvfree(prz.old_log);
    prz.old_log = core::ptr::null_mut();
    prz.old_log_size = 0;
    }
#[no_mangle]
pub unsafe extern "C" fn persistent_ram_zap(prz: *mut persistent_ram_zone) {
...
pub const MEM_TYPE_WCOMBINE: c_int = 0;
pub const MEM_TYPE_NONCACHED: c_int = 1;
pub const MEM_TYPE_NORMAL: c_int = 2;
    ...
    let mut addr: phys_addr_t = page_start + i * PAGE_SIZE;
...
//
    ...
    {
    int ret;
    let mut zap: bool = !!(prz.flags & PRZ_FLAG_ZAP_OLD);
...
#[no_mangle]
pub unsafe extern "C" fn persistent_ram_free(_prz: *mut persistent_ram_zone) {
...
// We must vunmap() at page-granularity.
    ...
    prz.vaddr = core::ptr::null_mut();
    }
    if (prz.rs_decoder) {
    free_rs(prz.rs_decoder);
    prz.rs_decoder = core::ptr::null_mut();
    }
    kfree(prz.ecc_info.par);
    prz.ecc_info.par = core::ptr::null_mut();
    persistent_ram_free_old(prz);
    kfree(prz.label);
    kfree(prz);
// _prz = NULL;
