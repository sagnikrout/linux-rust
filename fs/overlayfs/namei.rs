...
#[no_mangle]
unsafe extern "C" fn ovl_acceptable(ctx: *mut c_void, dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ovl_check_fb_len(fb: *mut ovl_fb, fb_len: c_int) -> c_int {
...
// Treat endianness mismatch as "origin unknown"
    ...
    res = ovl_getxattr_upper(ofs, upperdentry, ox, core::ptr::null_mut(), 0);
    if (res < 0) {
...
//
    ...
    connected ? ovl_acceptable : core::ptr::null_mut(), mnt);
    if (IS_ERR(real)) {
//
...
// Recheck condition under lock
    ...
    struct path path;
    int err;
    let mut last_element: bool = !post[0];
    let mut is_upper: bool = d.layer.idx == 0;
    char val;
//
...
//
    ...
    err = ovl_check_metacopy_xattr(ofs, &path, core::ptr::null_mut());
    if (err < 0)
    goto out_err;
    d.metacopy = err;
    d.stop = !d.metacopy;
...
// Counting down from the end, since the prefix can change
    let mut rem: usize = d.name.len - 1;
    ...
    let mut thislen: usize = next - s;
    let mut end: bool = !next[0];
...
// Lookup in data-only layers by absolute redirect to layer root
    ...
    {
    struct dentry *origin = core::ptr::null_mut();
    int i;
...
//
    ...
    return -ESTALE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: IS_ERR(origin)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn ovl_verify_index(ofs: *mut ovl_fs, index: *mut dentry) -> c_int {
    int ovl_verify_index(struct ovl_fs *ofs, struct dentry *index)
    {
    struct ovl_fh *fh = core::ptr::null_mut();
    size_t len;
    let mut origin: ovl_path = { };
    struct ovl_path *stack = &origin;
    struct dentry *upper = core::ptr::null_mut();
    int err;
    if (!d_inode(index))
    return 0;
    err = -EINVAL;
...
#[no_mangle]
pub unsafe extern "C" fn ovl_get_index_name_fh(fh: *const ovl_fh, name: *mut qstr) -> c_int {
...
// Lookup index by file handle for NFS export
    ...
    err = -ESTALE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ovl_dentry_weird(index)) -> else {
    ...
    struct inode *inode;
    struct qstr name;
    let mut is_dir: bool = d_is_dir(origin);
    int err;
...
#[no_mangle]
unsafe extern "C" fn ovl_maybe_validate_verity(dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_maybe_lookup_lowerdata(dentry: *mut dentry) -> c_int {
    static int ovl_maybe_lookup_lowerdata(struct dentry *dentry)
    {
    struct inode *inode = d_inode(dentry);
    const char *redirect = ovl_lowerdata_redirect(inode);
    let mut datapath: ovl_path = {};
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn ovl_verify_lowerdata(dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ovl_check_follow_redirect(d: *mut ovl_lookup_data) -> bool {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ovl_lookup_ctx {
    pub dentry: *mut dentry,
    pub oe: *mut ovl_entry,
    pub stack: *mut ovl_path,
    pub origin_path: *mut ovl_path,
    pub upperdentry: *mut dentry,
    pub index: *mut dentry,
    pub inode: *mut inode,
    pub ctr: c_uint,
}

#[no_mangle]
unsafe extern "C" fn ovl_lookup_layers(ctx: *mut ovl_lookup_ctx, d: *mut ovl_lookup_data) -> c_int {
    ...
    let mut check_redirect: bool = (ovl_redirect_follow(ofs) || ofs.numdatalayer);
    struct dentry *upperdir;
    struct dentry *this;
    struct dentry *origin = core::ptr::null_mut();
    let mut upperopaque: bool = false;
    let mut uppermetacopy: bool = false;
    let mut metacopy_size: c_int = 0;
    unsigned int i;
    int err;
...
//
    ...
    let mut lower: ovl_path = ovl_lowerstack(poe)[i];
...
#[no_mangle]
pub unsafe extern "C" fn if(!ofs->numdatalayer: d->is_dir ||) -> else {
...
//
    ...
    if (WARN_ON(ctx.stack != core::ptr::null_mut())) {
    err = -EIO;
    return err;
    }
    ...
    ctx.origin_path = core::ptr::null_mut();
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ctx->index) -> else {
    char *upperredirect;
...
//
    ...
    err = ovl_check_metacopy_xattr(ofs, &upperpath, core::ptr::null_mut());
    if (err < 0)
    return err;
...
#[no_mangle]
pub unsafe extern "C" fn ovl_lower_positive(dentry: *mut dentry) -> bool {
