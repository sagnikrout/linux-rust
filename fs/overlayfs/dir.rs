...
//
    ...
    let mut ovl_redirect_max: static unsigned short = 256;
...
#[no_mangle]
pub unsafe extern "C" fn ovl_tempname(name[OVL_TEMPNAME_SIZE]: c_char) {
    void ovl_tempname(char name[OVL_TEMPNAME_SIZE])
    {
    let mut temp_id: static atomic_t = ATOMIC_INIT(0);
// counter is allowed to wrap, since temp dentries are ephemeral
    ...
    ofs.whiteout = core::ptr::null_mut();
    return whiteout;
    }
...
#[no_mangle]
unsafe extern "C" fn ovl_set_opaque(dentry: *mut dentry, upperdentry: *mut dentry) -> c_int {
...
//
    ...
    ovl_dentry_init_reval(dentry, newdentry, core::ptr::null_mut());
    if (!hardlink) {
//
...
#[no_mangle]
unsafe extern "C" fn ovl_type_merge(dentry: *mut dentry) -> bool {
    static bool ovl_type_merge(struct dentry *dentry)
    {
    return OVL_TYPE_MERGE(ovl_path_type(dentry));
    }
#[no_mangle]
unsafe extern "C" fn ovl_type_origin(dentry: *mut dentry) -> bool {
...
// Setting opaque here is just an optimization, allow to fail
    ...
    err = ovl_instantiate(dentry, inode, newdentry, !!attr.hardlink, core::ptr::null_mut());
    if (err)
    goto out_cleanup;
    return 0;
    out_cleanup:
...
// dentry's upper doesn't match now, get rid of it
    ...
    let mut hardlink: bool = !!cattr.hardlink;
    if (WARN_ON(!workdir))
    return -EROFS;
    if (!hardlink) {
...
//
    ...
    ovl_dir_modified(dentry.d_parent, false);
    err = ovl_instantiate(dentry, inode, newdentry, hardlink, core::ptr::null_mut());
    if (err) {
    ovl_cleanup(ofs, upperdir, newdentry);
    dput(newdentry);
    }
...
#[no_mangle]
unsafe extern "C" fn ovl_revert_creator_creds(old_cred: *const cred) {
...
// Did we end up using the preallocated inode?
    ...
    {
    return ovl_create_object(idmap, dentry, (mode & 07777) | S_IFREG, 0, core::ptr::null_mut());
    }
    static struct dentry *ovl_mkdir(struct mnt_idmap *idmap, struct inode *dir,
    struct dentry *dentry, umode_t mode)
    {
    return ERR_PTR(ovl_create_object(idmap, dentry, (mode & 07777) | S_IFDIR, 0, core::ptr::null_mut()));
    }
...
// Don't allow creation of "whiteout" on overlay
    ...
    return -EPERM;
    return ovl_create_object(idmap, dentry, mode, rdev, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn ovl_set_link_redirect(dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_matches_upper(dentry: *mut dentry, upper: *mut dentry) -> bool {
    ...
    struct dentry *upperdir = ovl_dentry_upper(dentry.d_parent);
    struct dentry *upper;
    struct dentry *opaquedir = core::ptr::null_mut();
    int err;
    if (WARN_ON(!workdir))
    return -EROFS;
    if (!list_empty(list)) {
...
#[no_mangle]
unsafe extern "C" fn ovl_pure_upper(dentry: *mut dentry) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ovl_drop_nlink(dentry: *mut dentry) {
...
#[no_mangle]
unsafe extern "C" fn ovl_do_remove(dentry: *mut dentry, is_dir: bool) -> c_int {
    static int ovl_do_remove(struct dentry *dentry, bool is_dir)
    {
    int err;
    let mut lower_positive: bool = ovl_lower_positive(dentry);
    LIST_HEAD(list);
...
#[no_mangle]
unsafe extern "C" fn ovl_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
    static int ovl_unlink(struct inode *dir, struct dentry *dentry)
    {
    return ovl_do_remove(dentry, false);
    }
#[no_mangle]
unsafe extern "C" fn ovl_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
    static int ovl_rmdir(struct inode *dir, struct dentry *dentry)
    {
    return ovl_do_remove(dentry, true);
    }
#[no_mangle]
unsafe extern "C" fn ovl_type_merge_or_lower(dentry: *mut dentry) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ovl_can_move(dentry: *mut dentry) -> bool {
    ...
    char *buf, *ret;
    struct dentry *d, *tmp;
    let mut buflen: c_int = ovl_redirect_max + 1;
    if (!abs_redirect) {
...
#[no_mangle]
unsafe extern "C" fn ovl_need_absolute_redirect(dentry: *mut dentry, samedir: bool) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ovl_set_redirect(dentry: *mut dentry, samedir: bool) -> c_int {
    static int ovl_set_redirect(struct dentry *dentry, bool samedir)
    {
    int err;
    struct ovl_fs *ofs = OVL_FS(dentry.d_sb);
    const char *redirect = ovl_dentry_get_redirect(dentry);
    let mut absolute_redirect: bool = ovl_need_absolute_redirect(dentry, samedir);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ovl_renamedata {
    pub renamedata: struct,
    pub opaquedir: *mut dentry,
    pub cleanup_whiteout: bool,
    pub update_nlink: bool,
    pub overwrite: bool,
}

#[no_mangle]
unsafe extern "C" fn ovl_rename_start(ovlrd: *mut ovl_renamedata, list: *mut list_head) -> c_int {
    ...
    let mut is_dir: bool = d_is_dir(old);
    let mut new_is_dir: bool = d_is_dir(new);
    int err;
...
#[no_mangle]
unsafe extern "C" fn ovl_rename_upper(ovlrd: *mut ovl_renamedata, list: *mut list_head) -> c_int {
    ...
    let mut is_dir: bool = d_is_dir(old);
    let mut new_is_dir: bool = d_is_dir(new);
    let mut samedir: bool = old.d_parent == new.d_parent;
    let mut rd: renamedata = {};
    struct dentry *de;
    struct dentry *whiteout = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(ovl_type_merge(new->d_parent): is_dir && !old_opaque &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn ovl_rename_end(ovlrd: *mut ovl_renamedata) {
...
// It's okay to set O_NOATIME, since the owner will be current fsuid
    let mut flags: c_int = file.f_flags | OVL_OPEN_FLAGS;
    int err;
    scoped_class(override_creds_ovl, original_creds, dentry.d_sb) {
...
#[no_mangle]
unsafe extern "C" fn ovl_dummy_open(inode: *mut inode, file: *mut file) -> c_int {
