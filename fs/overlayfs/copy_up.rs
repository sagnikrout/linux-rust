...
#[no_mangle]
unsafe extern "C" fn ovl_ccup_set(buf: *const c_char, param: *const kernel_param) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_ccup_get(buf: *mut c_char, param: *const kernel_param) -> c_int {
    static int ovl_ccup_get(char *buf, const struct kernel_param *param)
    {
    return sprintf(buf, "N\n");
    }
    module_param_call(check_copy_up, ovl_ccup_set, ovl_ccup_get, core::ptr::null_mut(), 0644);
    MODULE_PARM_DESC(check_copy_up, "Obsolete; does nothing");
#[no_mangle]
unsafe extern "C" fn ovl_must_copy_xattr(name: *const c_char) -> bool {
    ...
    {
    int err;
    struct posix_acl *clone, *real_acl = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn ovl_copy_xattr(sb: *mut super_block, oldpath: *const path, new: *mut dentry) -> c_int {
    int ovl_copy_xattr(struct super_block *sb, const struct path *oldpath, struct dentry *new)
    {
    struct dentry *old = oldpath.dentry;
    ssize_t list_size, size, value_size = 0;
    char *buf, *name, *value = core::ptr::null_mut();
    let mut error: c_int = 0;
    size_t slen;
    if (!old.d_inode.i_op.listxattr || !new.d_inode.i_op.listxattr)
    return 0;
    list_size = vfs_listxattr(old, core::ptr::null_mut(), 0);
    if (list_size <= 0) {
...
// POSIX ACLs must be copied.
    ...
    if (size == -ERANGE)
    size = ovl_do_getxattr(oldpath, name, core::ptr::null_mut(), 0);
    if (size < 0) {
    error = size;
    break;
    }
...
// Ignore failure to copy unknown xattrs
    ...
    {
    let mut oldfa: file_kattr = { .flags_valid = true };
    let mut newfa: file_kattr = { .flags_valid = true };
    int err;
...
#[no_mangle]
unsafe extern "C" fn ovl_verify_area(pos: loff_t, pos2: loff_t, len: loff_t, totlen: loff_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_sync_file(path: *const path) -> c_int {
    ...
    let mut old_pos: loff_t = 0;
    let mut new_pos: loff_t = 0;
    loff_t cloned;
    let mut data_pos: loff_t = -1;
    loff_t hole_len;
    let mut skip_hole: bool = false;
    let mut error: c_int = 0;
    ovl_path_lowerdata(dentry, &datapath);
    if (WARN_ON_ONCE(datapath.dentry == core::ptr::null_mut()) ||
    WARN_ON_ONCE(len < 0))
    return -EIO;
...
// Check if lower fs supports seek operation
    ...
    skip_hole = true;
    while (len) {
    let mut this_len: usize = OVL_COPY_UP_CHUNK_SIZE;
    ssize_t bytes;
    if (len < this_len)
    this_len = len;
...
// call fsync once, either now or later along with metadata
    ...
    struct ovl_fh *fh;
    int fh_type, dwords;
    let mut buflen: c_int = MAX_HANDLE_SZ;
...
// Ignore -EPERM from setting "user.*" on symlink/special
    let mut err: return = = -EPERM ? 0 : err;
    }
...
//
    ...
    struct dentry *temp = core::ptr::null_mut();
...
pub struct ovl_copy_up_ctx {
    ...
    pub destdir: *mut dentry,
    pub destname: qstr,
    pub workdir: *mut dentry,
    pub origin_fh: *const ovl_fh,
    pub origin: bool,
    pub indexed: bool,
    pub metacopy: bool,
    pub metacopy_digest: bool,
    pub metadata_fsync: bool,
}

#[no_mangle]
unsafe extern "C" fn ovl_link_up(c: *mut ovl_copy_up_ctx) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_copy_up_data(c: *mut ovl_copy_up_ctx, temp: *const path) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_copy_up_metadata(c: *mut ovl_copy_up_ctx, temp: *mut dentry) -> c_int {
    static int ovl_copy_up_metadata(struct ovl_copy_up_ctx *c, struct dentry *temp)
    {
    struct ovl_fs *ofs = OVL_FS(c.dentry.d_sb);
    struct inode *inode = d_inode(c.dentry);
    let mut upperpath: path = { .mnt = ovl_upper_mnt(ofs), .dentry = temp };
    int err;
...
//
    ...
    let mut metacopy_data: ovl_metacopy = OVL_METACOPY_INIT;
    ovl_path_lowerdata(c.dentry, &lowerdatapath);
    if (WARN_ON_ONCE(lowerdatapath.dentry == core::ptr::null_mut()))
    return -EIO;
...
// fsync metadata before moving it into upper dir
    ...
    {
    struct cred *copy_up_cred = core::ptr::null_mut();
    int err;
...
#[no_mangle]
unsafe extern "C" fn ovl_revert_copy_up_creds(orig_cred: *const cred) {
...
//
#[no_mangle]
unsafe extern "C" fn ovl_copy_up_workdir(c: *mut ovl_copy_up_ctx) -> c_int {
    static int ovl_copy_up_workdir(struct ovl_copy_up_ctx *c)
    {
    struct ovl_fs *ofs = OVL_FS(c.dentry.d_sb);
    struct inode *inode;
    let mut path: path = { .mnt = ovl_upper_mnt(ofs) };
    let mut rd: renamedata = {};
    struct dentry *temp;
    int err;
...
#[no_mangle]
unsafe extern "C" fn ovl_copy_up_tmpfile(c: *mut ovl_copy_up_ctx) -> c_int {
    static int ovl_copy_up_tmpfile(struct ovl_copy_up_ctx *c)
    {
    struct ovl_fs *ofs = OVL_FS(c.dentry.d_sb);
    struct inode *udir = d_inode(c.destdir);
    struct dentry *temp, *upper, *newdentry = core::ptr::null_mut();
    struct file *tmpfile;
    int err;
    scoped_class(copy_up_creds, copy_up_creds, c.dentry) {
    if (IS_ERR(copy_up_creds))
...
//
#[no_mangle]
unsafe extern "C" fn ovl_do_copy_up(c: *mut ovl_copy_up_ctx) -> c_int {
    static int ovl_do_copy_up(struct ovl_copy_up_ctx *c)
    {
    int err;
    struct ovl_fs *ofs = OVL_FS(c.dentry.d_sb);
    struct dentry *origin = c.lowerpath.dentry;
    struct ovl_fh *fh = core::ptr::null_mut();
    let mut to_index: bool = false;
//
...
// Fall back to full copy if no fsverity on source data and we require verity
    ...
    if (WARN_ON_ONCE(lowerdata.dentry == core::ptr::null_mut()) ||
...
#[no_mangle]
unsafe extern "C" fn ovl_getxattr_value(path: *const path, name: *mut c_char, value: *mut c_char) -> isize {
    static ssize_t ovl_getxattr_value(const struct path *path, char *name, char **value)
    {
    ssize_t res;
    char *buf;
    res = ovl_do_getxattr(path, name, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn ovl_copy_up_meta_inode_data(c: *mut ovl_copy_up_ctx) -> c_int {
    static int ovl_copy_up_meta_inode_data(struct ovl_copy_up_ctx *c)
    {
    struct ovl_fs *ofs = OVL_FS(c.dentry.d_sb);
    struct path upperpath;
    int err;
    char *capability = core::ptr::null_mut();
    ssize_t cap_size;
    ovl_path_upper(c.dentry, &upperpath);
    if (WARN_ON(upperpath.dentry == core::ptr::null_mut()))
    return -EIO;
    if (c.stat.size) {
...
#[no_mangle]
unsafe extern "C" fn ovl_copy_up_flags(dentry: *mut dentry, flags: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_open_need_copy_up(dentry: *mut dentry, flags: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ovl_maybe_copy_up(dentry: *mut dentry, flags: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ovl_copy_up_with_data(dentry: *mut dentry) -> c_int {
    int ovl_copy_up_with_data(struct dentry *dentry)
    {
    return ovl_copy_up_flags(dentry, O_WRONLY);
    }
#[no_mangle]
pub unsafe extern "C" fn ovl_copy_up(dentry: *mut dentry) -> c_int {
