...
#[no_mangle]
unsafe extern "C" fn ovl_encode_maybe_copy_up(dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ovl_connectable_layer(dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ovl_connect_layer(dentry: *mut dentry) -> c_int {
    static int ovl_connect_layer(struct dentry *dentry)
    {
    struct dentry *next, *parent = core::ptr::null_mut();
    struct ovl_entry *oe = OVL_E(dentry);
    int origin_layer;
    let mut err: c_int = 0;
...
//
#[no_mangle]
unsafe extern "C" fn ovl_check_encode_origin(inode: *mut inode) -> c_int {
    static int ovl_check_encode_origin(struct inode *inode)
    {
    struct ovl_fs *ofs = OVL_FS(inode.i_sb);
    let mut decodable: bool = ofs.config.nfs_export;
...
//
    ...
    struct dentry *index)
    {
    struct dentry *lower = lowerpath ? lowerpath.dentry : core::ptr::null_mut();
...
//
    ...
    index = ovl_lookup_index(ofs, core::ptr::null_mut(), real, false);
    if (IS_ERR(index))
    return index;
    }
...
//
    ...
    if (IS_ERR_OR_NULL(upper))
    return upper;
    dentry = ovl_get_dentry(sb, upper, core::ptr::null_mut(), core::ptr::null_mut());
    dput(upper);
    return dentry;
    }
...
// First lookup overlay inode in inode cache by origin fh
    err = ovl_check_origin_fh(ofs, fh, false, core::ptr::null_mut(), &stack);
    if (err)
    return ERR_PTR(err);
...
// Find origin.dentry again with ovl_acceptable() layer check
    if (d_is_dir(origin.dentry)) {
    dput(origin.dentry);
    origin.dentry = core::ptr::null_mut();
    err = ovl_check_origin_fh(ofs, fh, true, core::ptr::null_mut(), &stack);
    if (err)
    goto out_err;
    }
    if (index) {
...
// Get a connected non-upper dir or disconnected non-dir
    dentry = ovl_get_dentry(sb, core::ptr::null_mut(), &origin, index);
    out:
    dput(origin.dentry);
    dput(index);
    return dentry;
