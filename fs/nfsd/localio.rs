...
//
    ...
    const fmode_t fmode)
    {
    let mut mayflags: c_int = NFSD_MAY_LOCALIO;
...
// nfs_fh -> svc_fh
    ...
    again:
    rcu_read_lock();
    new = unrcu_pointer(cmpxchg(pnf, core::ptr::null_mut(), RCU_INITIALIZER(localio)));
    if (new) {
// Some other thread installed an nfsd_file
    if (nfsd_file_get(new) == core::ptr::null_mut()) {
    rcu_read_unlock();
    goto again;
    }
    rcu_read_unlock();
...
#[no_mangle]
pub unsafe extern "C" fn nfsd_localio_ops_init() {
...
//
#[no_mangle]
unsafe extern "C" fn localio_proc_null(rqstp: *mut svc_rqst) -> __be32 {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct localio_uuidarg {
    pub uuid: uuid_t,
}

#[no_mangle]
unsafe extern "C" fn localio_proc_uuid_is_local(rqstp: *mut svc_rqst) -> __be32 {
