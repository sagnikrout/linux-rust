...
#[no_mangle]
unsafe extern "C" fn nfsd_map_status(status: __be32) -> __be32 {
...
//
    let mut delta: time64_t = iap.ia_atime.tv_sec - ktime_get_real_seconds();
...
//
    ...
    resp.status = nfsd_setattr(rqstp, fhp, &attrs, core::ptr::null_mut());
    if (resp.status != nfs_ok)
    goto out;
    resp.status = fh_getattr(&resp.fh, &resp.stat);
    out:
...
//
    ...
    resp.status = fh_getattr(&resp.fh, &resp.stat);
#[no_mangle]
pub unsafe extern "C" fn if(nfserr_jukebox: resp->status ==) -> else {
...
//
    ...
    &argp.payload, &cnt, NFS_DATA_SYNC, core::ptr::null_mut());
    if (resp.status == nfs_ok)
    resp.status = fh_getattr(&resp.fh, &resp.stat);
#[no_mangle]
pub unsafe extern "C" fn if(nfserr_jukebox: resp->status ==) -> else {
...
//
    ...
    int		type, mode;
    int		hosterr;
    let mut rdev: dev_t = 0, wanted = new_decode_dev(attr.ia_size);
