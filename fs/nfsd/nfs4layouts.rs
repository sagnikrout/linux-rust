...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfs4_layout {
    pub lo_perstate: list_head,
    pub lo_state: *mut nfs4_layout_stateid,
    pub lo_seg: nfsd4_layout_seg,
}
...
pub const DEVID_HASH_BITS: c_int = 8;

    let mut nfsd_devid_seq: static u64 = 1;
    static struct list_head nfsd_devid_hash[DEVID_HASH_SIZE];
    static DEFINE_SPINLOCK(nfsd_devid_lock);
#[no_mangle]
pub unsafe extern "C" fn devid_hashfn(idx: u64) -> u32 {
    ...
    let mut fsid_len: usize = key_len(fh.fh_fsid_type);
    ...
    {
    struct nfsd4_deviceid_map *map, *ret = core::ptr::null_mut();
    rcu_read_lock();
...
#[no_mangle]
pub unsafe extern "C" fn nfsd4_setup_layout_type(exp: *mut svc_export) {
    void nfsd4_setup_layout_type(struct svc_export *exp)
    {
    struct super_block *sb = exp.ex_path.mnt.mnt_sb;
    let mut block_supported: expfs_block_layouts_t = exportfs_layouts_supported(sb);
...
#[no_mangle]
pub unsafe extern "C" fn nfsd4_close_layout(ls: *mut nfs4_layout_stateid) {
    void nfsd4_close_layout(struct nfs4_layout_stateid *ls)
    {
    struct nfsd_file *fl;
    spin_lock(&ls.ls_stid.sc_file.fi_lock);
    fl = ls.ls_file;
    ls.ls_file = core::ptr::null_mut();
    spin_unlock(&ls.ls_stid.sc_file.fi_lock);
    if (fl) {
    if (!nfsd4_layout_ops[ls.ls_layout_type].disable_recalls)
    kernel_setlease(fl.nf_file, F_UNLCK, core::ptr::null_mut(),
    (void **)&ls);
    nfsd_file_put(fl);
    }
    }
    ...
    status = kernel_setlease(fl.c.flc_file, fl.c.flc_type, &fl, core::ptr::null_mut());
    if (status) {
    locks_free_lease(fl);
    return status;
    }
    BUG_ON(fl != core::ptr::null_mut());
    return 0;
    }
    ...
    let mut typemask: c_ushort = SC_TYPE_LAYOUT;
    __be32 status;
    if (create)
...
// lsp = ls;
    ...
    {
    let mut end: u64 = seg.offset + seg.length;
    ...
    struct nfs4_file *fp = ls.ls_stid.sc_file;
    struct nfs4_layout *lp, *new = core::ptr::null_mut();
    __be32 nfserr;
    spin_lock(&fp.fi_lock);
