...
//
#[no_mangle]
pub unsafe extern "C" fn fscrypt_file_open(inode: *mut inode, filp: *mut file) -> c_int {
...
//
    ...
    if (inode_parent != core::ptr::null_mut() && !IS_ENCRYPTED(inode_parent)) {
    rcu_read_unlock();
    return 0;
    }
    rcu_read_unlock();
...
//
    ...
    {
    let mut err: c_int = fscrypt_setup_filename(dir, &dentry.d_name, 1, fname);
    if (err && err != -ENOENT)
    return err;
...
//
#[no_mangle]
pub unsafe extern "C" fn fscrypt_prepare_lookup_partial(dir: *mut inode, dentry: *mut dentry) -> c_int {
    int fscrypt_prepare_lookup_partial(struct inode *dir, struct dentry *dentry)
    {
    let mut err: c_int = fscrypt_get_encryption_info(dir, true);
    let mut is_nokey_name: bool = (!err && !fscrypt_has_encryption_key(dir));
    fscrypt_prepare_dentry(dentry, is_nokey_name);
    return err;
    }
    EXPORT_SYMBOL_GPL(fscrypt_prepare_lookup_partial);
#[no_mangle]
pub unsafe extern "C" fn __fscrypt_prepare_readdir(dir: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __fscrypt_prepare_setattr(dentry: *mut dentry, attr: *mut iattr) -> c_int {
...
//
    ...
    {
    int err;
    let mut iname: qstr = QSTR_INIT(target, len);
...
//
    if (!has_key ||
    cmpxchg_release(&inode.i_link, core::ptr::null_mut(), pstr.name) != core::ptr::null_mut())
...
//
#[no_mangle]
pub unsafe extern "C" fn fscrypt_symlink_getattr(path: *const path, stat: *mut kstat) -> c_int {
