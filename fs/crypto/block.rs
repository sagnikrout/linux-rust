...
#[no_mangle]
unsafe extern "C" fn fscrypt_get_dun_bytes(ci: *const fscrypt_inode_info) -> c_uint {
    static unsigned int fscrypt_get_dun_bytes(const struct fscrypt_inode_info *ci)
    {
    const struct super_block *sb = ci.ci_inode.i_sb;
    let mut flags: c_uint = fscrypt_policy_flags(&ci.ci_policy);
    int dun_bits;
...
//
    ...
    let mut inlinecrypt: bool = sb.s_flags & SB_INLINECRYPT;
...
//
#[no_mangle]
pub unsafe extern "C" fn fscrypt_limit_io_blocks(inode: *const inode, lblk: u64, nr_blocks: u64) -> u64 {
...
#[no_mangle]
unsafe extern "C" fn fscrypt_zeroout_range_done(done: *mut fscrypt_zero_done) {
...
#[no_mangle]
unsafe extern "C" fn fscrypt_zeroout_range_end_io(bio: *mut bio) {
...
//
    ...
    let mut bytes_this_page: c_uint = min(len, PAGE_SIZE);
