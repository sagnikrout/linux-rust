...
//! Source: fs/crypto/keysetup_v1.c
#![no_std]
...
//
    ...
    key = request_key(&key_type_logon, description, core::ptr::null_mut());
    kfree(description);
    if (IS_ERR(key))
    return key;
    down_read(&key.sem);
...
pub struct fscrypt_direct_key {
    ...
    pub dk_descriptor: [u8; FSCRYPT_KEY_DESCRIPTOR_SIZE],
    pub dk_raw: [u8; FSCRYPT_MAX_RAW_KEY_SIZE],
}

#[no_mangle]
unsafe extern "C" fn free_direct_key(dk: *mut fscrypt_direct_key) {
...
#[no_mangle]
pub unsafe extern "C" fn fscrypt_put_direct_key(dk: *mut fscrypt_direct_key) {
...
// Is there already a tfm for this key?
    dk = find_or_insert_direct_key(core::ptr::null_mut(), raw_key, ci);
    if (dk)
    return dk;
// Nope, allocate one.
    dk = kzalloc_obj(*dk);
...
//
    ...
    const u8 *raw_master_key)
    {
    let mut derived_keysize: c_uint = ci.ci_mode.keysize;
