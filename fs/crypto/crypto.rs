...
//
    ...
    let mut num_prealloc_crypto_pages: static unsigned int = 32;
    module_param(num_prealloc_crypto_pages, uint, 0444);
    MODULE_PARM_DESC(num_prealloc_crypto_pages,
    "Number of crypto pages to preallocate");
    static mempool_t *fscrypt_bounce_page_pool = core::ptr::null_mut();
...
pub unsafe extern "C" fn fscrypt_free_bounce_page(bounce_page: *mut page) {
    ...
    if (!bounce_page)
    return;
    set_page_private(bounce_page, (unsigned long)core::ptr::null_mut());
...
//
    ...
    {
    let mut flags: u8 = fscrypt_policy_flags(&ci.ci_policy);
...
// Encrypt or decrypt a single "data unit" of file contents.
    ...
    int err;
    if (WARN_ON_ONCE(ci == core::ptr::null_mut())) /* File hasn't been opened yet? */
    return -ENOKEY;
    tfm = ci.ci_enc_key.tfm;
    if (WARN_ON_ONCE(tfm == core::ptr::null_mut())) /* Called on block-based filesystem? */
    return -ENOKEY;
...
//
#[no_mangle]
pub unsafe extern "C" fn fscrypt_initialize(sb: *mut super_block) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: inode) -> else {
    else if (inode)
...
#[no_mangle]
unsafe extern "C" fn fscrypt_init() -> int __init {
