...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct binfmt_misc_entry {
    pub node: hlist_node,
    pub /: *mut *mut unsigned long flags; / type, status, etc.,
    pub /: *mut *mut int offset; / offset of magic,
    pub /: *mut *mut int size; / size of magic/mask,
    pub /: *mut *mut *mut char magic; / magic or filename extension,
    pub /: *mut *mut *mut char mask; / mask, NULL for exact match,
    pub /: *const *const *const char interpreter; / filename of interpreter,
    pub name: *mut c_char,
    pub dentry: *mut dentry,
    pub /: *const *const *const binfmt_misc_ops bpf_ops; / bpf-backed handler ('B'),
    pub bpf_ops_name: *const c_char,
    pub /: *mut *mut list_head interps; / the interpreters it bound,
    pub /: *mut *mut refcount_t users; / sync removal with load_misc_binary(),
    pub rcu: rcu_head,
    pub /: *mut *mut char buf[]; / register string, fields point in here,
}
...
//
pub const MAX_REGISTER_LENGTH: c_int = 1920;
// Trailing delimiter pad so field parsing always terminates at a delimiter.
pub const MISC_DELIM_PAD: c_int = 8;
...
//
    ...
    const char *ext = dot ? dot + 1 : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn close_interp_file(f: *mut file) {
...
#[no_mangle]
unsafe extern "C" fn entry_put_interpreters(e: *mut binfmt_misc_entry) {
...
//
    ...
    struct file *f)
    {
    let mut nlen: usize = strlen(name), plen = strlen(path);
    struct binfmt_misc_interp *interp;
...
#[no_mangle]
unsafe extern "C" fn bm_entry_free_rcu(rcu: *mut rcu_head) {
...
//
#[no_mangle]
unsafe extern "C" fn put_binfmt_handler(e: *mut binfmt_misc_entry) {
...
#[no_mangle]
unsafe extern "C" fn drop_staged_selection(bprm: *mut linux_binprm) {
    static void drop_staged_selection(struct linux_binprm *bprm)
    {
    kfree(bprm.bpf_interp);
    bprm.bpf_interp = core::ptr::null_mut();
    kfree(bprm.bpf_interp_arg);
    bprm.bpf_interp_arg = core::ptr::null_mut();
    if (bprm.bpf_interp_file) {
    fput(bprm.bpf_interp_file);
    bprm.bpf_interp_file = core::ptr::null_mut();
    }
    bprm.bpf_flags = 0;
    }
//
...
// The interpreters this entry lets the program choose from.
    ...
    bprm.bpf_interps = core::ptr::null_mut();
    if (retval) {
...
//
    ...
    {
    struct file *interp_file __free(fput) = core::ptr::null_mut();
    struct binfmt_misc_interp *interp;
    struct file *bound;
    int retval;
    if (bprm.bpf_interp_file) {
...
//
#[no_mangle]
unsafe extern "C" fn load_misc_binary(bprm: *mut linux_binprm) -> c_int {
    static int load_misc_binary(struct linux_binprm *bprm)
    {
    struct binfmt_misc_entry *fmt __free(put_binfmt_handler) = core::ptr::null_mut();
    const char *interpreter;
...
// Declining here would run the binary's own PT_INTERP.
    let mut retval: return = = -ENOEXEC ? -EACCES : retval;
    }
...
//
    ...
    size_t count)
    {
    struct binfmt_misc_entry *e __free(kfree) = core::ptr::null_mut();
    char *buf, *p;
    char del;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(MISC_FMT_MAGIC_BIT, _arg: &e->flags)) -> else {
    else if (test_bit(MISC_FMT_MAGIC_BIT, &e.flags))
...
// The 'interpreter' field carries the handler name.
    ...
    e.interpreter = core::ptr::null_mut();
    if (!e.bpf_ops_name[0])
    return ERR_PTR(-EINVAL);
...
#[no_mangle]
unsafe extern "C" fn read_command(buffer: *const char __user, count: usize) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bm_seq_hex(m: *mut seq_file, data: *const u8, size: c_int) {
...
#[no_mangle]
unsafe extern "C" fn bm_entry_show(m: *mut seq_file, unused: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn bm_evict_inode(inode: *mut inode) {
...
//
    ...
    locked_recursive_removal(e.dentry, core::ptr::null_mut());
    }
// Remove @e unless it was already removed.
#[no_mangle]
unsafe extern "C" fn bm_remove_entry(e: *mut binfmt_misc_entry, sb: *mut super_block) {
...
// Remove all entries of the binfmt_misc instance @misc belonging to @sb.
    ...
    {
    struct inode *root = bm_root_inode(sb);
    struct dentry *child = core::ptr::null_mut();
    inode_lock_nested(root, I_MUTEX_PARENT);
//
...
//
#[no_mangle]
unsafe extern "C" fn bm_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
// /<entry>
#[no_mangle]
unsafe extern "C" fn bm_entry_open(inode: *mut inode, file: *mut file) -> c_int {
...
//
    ...
    {
    struct file *f __free(close_interp_file) = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !hlist_unhashed(&e->node)) -> else {
...
// add to filesystem
#[no_mangle]
unsafe extern "C" fn add_entry(e: *mut binfmt_misc_entry, sb: *mut super_block) -> c_int {
...
// A 'D' entry stays out of the search list until its first enable.
    ...
    size_t count, loff_t *ppos)
    {
    struct binfmt_misc_entry *e __free(put_binfmt_handler) = core::ptr::null_mut();
...
// /status
    ...
    {
    struct binfmt_misc *misc;
    let mut res: c_int = read_command(buffer, count);
    misc = i_binfmt_misc(file_inode(file));
    switch (res) {
    case BM_CMD_DISABLE:
...
#[no_mangle]
unsafe extern "C" fn bm_fill_super(sb: *mut super_block, fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bm_free(fc: *mut fs_context) {
...
#[no_mangle]
unsafe extern "C" fn bm_get_tree(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn bm_kill_sb(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn bm_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn init_misc_binfmt() -> int __init {
    static int __init init_misc_binfmt(void)
    {
    let mut err: c_int = register_filesystem(&bm_fs_type);
    if (!err)
    insert_binfmt(&misc_format);
    return err;
    }
#[no_mangle]
unsafe extern "C" fn exit_misc_binfmt() -> void __exit {
