...
#[no_mangle]
unsafe extern "C" fn hpfs_claim_alloc(s: *mut super_block, sec: secno) {
...
#[no_mangle]
unsafe extern "C" fn hpfs_claim_free(s: *mut super_block, sec: secno) {
...
#[no_mangle]
unsafe extern "C" fn hpfs_claim_dirband_alloc(s: *mut super_block, sec: secno) {
...
#[no_mangle]
unsafe extern "C" fn hpfs_claim_dirband_free(s: *mut super_block, sec: secno) {
...
//
#[no_mangle]
unsafe extern "C" fn chk_if_allocated(s: *mut super_block, sec: secno, msg: *mut c_char) -> c_int {
    ...
    let mut ssec: unsigned = (sec - hpfs_sb(s).sb_dirband_start) / 4;
...
//
#[no_mangle]
pub unsafe extern "C" fn hpfs_chk_sectors(s: *mut super_block, start: secno, len: c_int, msg: *mut c_char) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn alloc_in_bmp(s: *mut super_block, near: secno, n: unsigned, forward: unsigned) -> secno {
    static secno alloc_in_bmp(struct super_block *s, secno near, unsigned n, unsigned forward)
    {
    struct quad_buffer_head qbh;
    __le32 *bmp;
    let mut bs: unsigned = near & ~0x3fff;
    let mut nr: unsigned = (near & 0x3fff) & ~(n - 1);
// unsigned mnr;
    unsigned i, q;
    int a, b;
    let mut ret: secno = 0;
...
// for (i = nr + 1; i != nr; i++, i &= 0x1ff)
    ...
    q = i<<5;
    if (i > 0) {
    let mut k: unsigned = le32_to_cpu(bmp[i-1]);
    while (k & 0x80000000) {
    q--; k <<= 1;
    }
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn hpfs_alloc_sector(s: *mut super_block, near: secno, n: unsigned, forward: c_int) -> secno {
...
#[no_mangle]
unsafe extern "C" fn alloc_in_dirband(s: *mut super_block, near: secno) -> secno {
    static secno alloc_in_dirband(struct super_block *s, secno near)
    {
    let mut nr: unsigned = near;
    secno sec;
    struct hpfs_sb_info *sbi = hpfs_sb(s);
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_alloc_if_possible(s: *mut super_block, sec: secno) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_free_sectors(s: *mut super_block, sec: secno, n: unsigned) {
...
//
#[no_mangle]
pub unsafe extern "C" fn hpfs_check_free_dnodes(s: *mut super_block, n: c_int) -> c_int {
    int hpfs_check_free_dnodes(struct super_block *s, int n)
    {
    let mut n_bmps: c_int = (hpfs_sb(s).sb_fs_size + 0x4000 - 1) >> 14;
    let mut b: c_int = hpfs_sb(s).sb_c_bitmap & 0x0fffffff;
    int i, j;
    __le32 *bmp;
    struct quad_buffer_head qbh;
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_free_dnode(s: *mut super_block, dno: dnode_secno) {
    ...
    struct quad_buffer_head qbh;
    __le32 *bmp;
    let mut ssec: unsigned = (dno - hpfs_sb(s).sb_dirband_start) / 4;
    if (!(bmp = hpfs_map_dnode_bitmap(s, &qbh))) {
    return;
    }
    ...
    if (!(*dno = hpfs_alloc_sector(s, near, 4, 0))) return core::ptr::null_mut();
    } else {
    if (!(*dno = hpfs_alloc_sector(s, near, 4, 0)))
    if (!(*dno = alloc_in_dirband(s, near))) return core::ptr::null_mut();
    }
    ...
    {
    struct fnode *f;
    if (!(*fno = hpfs_alloc_sector(s, near, 1, FNODE_ALLOC_FWD))) return core::ptr::null_mut();
    ...
    {
    struct anode *a;
    if (!(*ano = hpfs_alloc_sector(s, near, 1, ANODE_ALLOC_FWD))) return core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn find_run(bmp: *mut __le32, idx: *mut unsigned) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn do_trim(s: *mut super_block, start: secno, len: unsigned, limit_start: secno, limit_end: secno, minlen: unsigned, result: *mut unsigned) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_trim_fs(s: *mut super_block, start: u64, end: u64, minlen: u64, result: *mut unsigned) -> c_int {
