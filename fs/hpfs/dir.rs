...
#[no_mangle]
unsafe extern "C" fn hpfs_dir_release(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hpfs_dir_lseek(filp: *mut file, off: loff_t, whence: c_int) -> loff_t {
    static loff_t hpfs_dir_lseek(struct file *filp, loff_t off, int whence)
    {
    let mut new_off: loff_t = off + (whence == 1 ? filp.f_pos : 0);
    loff_t pos;
...
#[no_mangle]
unsafe extern "C" fn hpfs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
...
//
    de = map_dirent(dir, hpfs_i(dir).i_dno, name, len, core::ptr::null_mut(), &qbh);
//
...
#[no_mangle]
pub unsafe extern "C" fn if(hpfs_sb(dir->i_sb)->sb_eas: le32_to_cpu(de->ea_size) &&) -> else {
...
//
    ...
    let mut csec: time64_t = local_to_gmt(dir.i_sb, le32_to_cpu(de.creation_date));
