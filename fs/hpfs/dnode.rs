...
#[no_mangle]
unsafe extern "C" fn get_pos(d: *mut dnode, fde: *mut hpfs_dirent) -> loff_t {
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_add_pos(inode: *mut inode, pos: *mut loff_t) -> c_int {
    ...
    hpfs_inode.i_rddir_off[i + 1] = core::ptr::null_mut();
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn hpfs_del_pos(inode: *mut inode, pos: *mut loff_t) {
...
#[no_mangle]
unsafe extern "C" fn hpfs_pos_subst(p: *mut loff_t, f: loff_t, t: loff_t) {
...
#[no_mangle]
unsafe extern "C" fn hpfs_pos_ins(p: *mut loff_t, d: loff_t, c: loff_t) {
    static void hpfs_pos_ins(loff_t *p, loff_t d, loff_t c)
    {
    if ((*p & ~0x3f) == (d & ~0x3f) && (*p & 0x3f) >= (d & 0x3f)) {
    let mut n: c_int = (*p & 0x3f) + c;
    if (n > 0x3f)
    pr_err("%s(): %08x + %d\n",
...
#[no_mangle]
unsafe extern "C" fn hpfs_pos_del(p: *mut loff_t, d: loff_t, c: loff_t) {
...
// p = (*p & ~0x3f) | n;
    ...
    {
    struct hpfs_dirent *de, *de_end, *dee = core::ptr::null_mut(), *deee = core::ptr::null_mut();
    ...
    {
    struct hpfs_dirent *de, *de_end, *dee = core::ptr::null_mut();
    de_end = dnode_end_de(d);
    for (de = dnode_first_de(d); de < de_end; de = de_next_de(de)) {
    dee = de;
    }
...
#[no_mangle]
unsafe extern "C" fn set_last_pointer(s: *mut super_block, d: *mut dnode, ptr: dnode_secno) {
...
// Add an entry to dnode and don't care if it grows over 2048 bytes
    ...
    struct hpfs_dirent *de;
    struct hpfs_dirent *de_end = dnode_end_de(d);
    let mut d_size: unsigned = de_size(namelen, down_ptr);
    for (de = dnode_first_de(d); de < de_end; de = de_next_de(de)) {
    let mut c: c_int = hpfs_compare_names(s, name, namelen, de.name, de.namelen, de.last);
    if (!c) {
...
#[no_mangle]
unsafe extern "C" fn fix_up_ptrs(s: *mut super_block, d: *mut dnode) {
    static void fix_up_ptrs(struct super_block *s, struct dnode *d)
    {
    struct hpfs_dirent *de;
    struct hpfs_dirent *de_end = dnode_end_de(d);
    let mut dno: dnode_secno = le32_to_cpu(d.self);
    for (de = dnode_first_de(d); de < de_end; de = de_next_de(de))
    if (de.down) {
...
// Add an entry to dnode and do dnode splitting if required
    ...
    struct dnode *d, *ad, *rd, *nd = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn move_to_top(i: *mut inode, from: dnode_secno, to: dnode_secno) -> secno {
    static secno move_to_top(struct inode *i, dnode_secno from, dnode_secno to)
    {
    dnode_secno dno, ddno;
    let mut chk_up: dnode_secno = to;
    struct dnode *dnode;
    struct quad_buffer_head qbh;
    ...
    let mut up: dnode_secno = le32_to_cpu(dnode.up);
    hpfs_brelse4(&qbh);
...
//
#[no_mangle]
unsafe extern "C" fn delete_empty_dnode(i: *mut inode, dno: dnode_secno) {
    ...
    let mut root: c_int = dnode.root_dnode;
...
// Delete dirent from directory
    ...
    hpfs_brelse4(qbh);
    if (down) {
    let mut a: dnode_secno = move_to_top(i, down, dno);
    ...
    if (!dnode) return core::ptr::null_mut();
    if (dn) *dn=dnode;
    de = dnode_first_de(dnode);
    de_end = dnode_end_de(dnode);
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_de_as_down_as_possible(s: *mut super_block, dno: dnode_secno) -> dnode_secno {
    dnode_secno hpfs_de_as_down_as_possible(struct super_block *s, dnode_secno dno)
    {
    struct quad_buffer_head qbh;
    let mut d: dnode_secno = dno;
    let mut up: dnode_secno = 0;
    struct hpfs_dirent *de;
    int c1, c2 = 0;
    again:
    if (hpfs_sb(s).sb_chk)
    ...
    return d;
    if (!(de = map_nth_dirent(s, d, 1, &qbh, core::ptr::null_mut()))) return dno;
    if (hpfs_sb(s).sb_chk)
...
// Find a dirent in tree
    ...
    if (hpfs_stop_cycles(inode.i_sb, dno, &c1, &c2, "map_dirent")) return core::ptr::null_mut();
    if (!(dnode = hpfs_map_dnode(inode.i_sb, dno, qbh))) return core::ptr::null_mut();
    de_end = dnode_end_de(dnode);
    for (de = dnode_first_de(dnode); de < de_end; de = de_next_de(de)) {
    let mut t: c_int = hpfs_compare_names(inode.i_sb, name, len, de.name, de.namelen, de.last);
    if (!t) {
    if (dd) *dd = dno;
    return de;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn hpfs_remove_dtree(s: *mut super_block, dno: dnode_secno) {
