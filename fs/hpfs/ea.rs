...
#[no_mangle]
pub unsafe extern "C" fn hpfs_ea_ext_remove(s: *mut super_block, a: secno, ano: c_int, len: unsigned) {
...
// Read an extended attribute named 'key'
    ...
    if (hpfs_ea_read(s, a, ano, pos, 4, ex)) return core::ptr::null_mut();
    if (hpfs_ea_read(s, a, ano, pos + 4, ea.namelen + 1 + (ea_indirect(ea) ? 8 : 0), ex + 4))
    return core::ptr::null_mut();
    if (!strcmp(ea.name, key)) {
    if (ea_indirect(ea))
...
//
    ...
    const char *data, int size)
    {
    let mut fno: fnode_secno = inode.i_ino;
...
// Most the code here is 99.9993422% unused. I hope there are no bugs.
    ...
    while (((pos + 511) >> 9) > len) {
    if (!len) {
    let mut q: secno = hpfs_alloc_sector(s, fno, 1, 0);
    if (!q) goto bail;
