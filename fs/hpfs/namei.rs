...
#[no_mangle]
unsafe extern "C" fn hpfs_update_directory_times(dir: *mut inode) {
    static void hpfs_update_directory_times(struct inode *dir)
    {
    let mut t: time64_t = local_to_gmt(dir.i_sb, local_get_seconds(dir.i_sb));
    ...
    let mut len: unsigned = dentry.d_name.len;
...
// de->hidden = de->system = 0;
    ...
    const unsigned char *name = dentry.d_name.name;
    let mut len: unsigned = dentry.d_name.len;
    struct inode *result = core::ptr::null_mut();
    struct buffer_head *bh;
    struct fnode *fnode;
    fnode_secno fno;
    int r;
    ...
    let mut err: return = =-ENOENT ? -EINVAL : err;
    hpfs_lock(dir.i_sb);
    err = -ENOSPC;
    fnode = hpfs_alloc_fnode(dir.i_sb, hpfs_i(dir).i_dno, &fno, &bh);
    if (!fnode)
...
#[no_mangle]
unsafe extern "C" fn hpfs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn hpfs_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
    ...
    if (!de.directory)
    goto out1;
    hpfs_count_dnodes(dir.i_sb, hpfs_i(inode).i_dno, core::ptr::null_mut(), core::ptr::null_mut(), &n_items);
    err = -ENOTEMPTY;
    if (n_items)
    goto out1;
    r = hpfs_remove_dirent(dir, dno, de, &qbh, 1);
...
#[no_mangle]
unsafe extern "C" fn hpfs_symlink_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    ...
    let mut old_len: unsigned = old_dentry.d_name.len;
    const unsigned char *new_name = new_dentry.d_name.name;
    let mut new_len: unsigned = new_dentry.d_name.len;
...
// Erm? Moving over the empty non-busy directory is perfectly legal
    ...
    if ((nde = map_dirent(new_dir, hpfs_i(new_dir).i_dno, new_name, new_len, core::ptr::null_mut(), &qbh1))) {
