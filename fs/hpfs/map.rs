...
//
    ...
    secno sec;
    __le32 *ret;
    let mut n_bands: unsigned = (hpfs_sb(s).sb_fs_size + 0x3fff) >> 14;
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_prefetch_bitmap(s: *mut super_block, bmp_block: unsigned) {
...
//
    ...
    if (!cp) return core::ptr::null_mut();
    ...
    if (!(cpd = hpfs_map_sector(s, cpds, &bh, 0))) return core::ptr::null_mut();
...
// Try to build lowercasing table from uppercasing one
    ...
    {
    struct buffer_head *bh;
    let mut n: c_int = (hpfs_sb(s).sb_fs_size + 0x200000 - 1) >> 21;
    int i;
    __le32 *b;
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_load_hotfix_map(s: *mut super_block, spareblock: *mut hpfs_spare_block) {
...
//
    ...
    {
    struct anode *anode;
    if (hpfs_sb(s).sb_chk) if (hpfs_chk_sectors(s, ano, 1, "anode")) return core::ptr::null_mut();
...
//
    ...
    struct dnode *dnode;
    if (hpfs_sb(s).sb_chk) {
    if (hpfs_chk_sectors(s, secno, 4, "dnode")) return core::ptr::null_mut();
    if (secno & 3) {
...
#[no_mangle]
pub unsafe extern "C" fn hpfs_fnode_dno(s: *mut super_block, ino: ino_t) -> dnode_secno {
