...
//
    ...
    {
    let mut remaining: c_uint = flags;
    let mut separator: bool = false;
    int i;
    for (i = 0; i < count; i++) {
    let mut flag: c_uint = table[i].const_value;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !separator) -> else {
    else if (!separator)
    seq_puts(m, "none");
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ksmbd_const_smb2_process_req {
...
#[no_mangle]
unsafe extern "C" fn proc_show_runtime_totals(m: *mut seq_file) {
    static void proc_show_runtime_totals(struct seq_file *m)
    {
    struct ksmbd_conn *conn;
    let mut clients: c_uint = 0;
    let mut open_files: c_uint = 0;
    int i;
...
#[no_mangle]
unsafe extern "C" fn proc_show_ksmbd_stats(m: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ksmbd_proc_cleanup() {
    ...
    ksmbd_proc_fs = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn ksmbd_proc_reset() {
...
#[no_mangle]
pub unsafe extern "C" fn ksmbd_proc_init() -> c_int {
    int ksmbd_proc_init(void)
    {
    int i;
    let mut retval: c_int = -ENOMEM;
    ksmbd_proc_fs = proc_mkdir("fs/ksmbd", core::ptr::null_mut());
    if (!ksmbd_proc_fs)
    return retval;
    ...
    if (!ksmbd_proc_create("server", proc_show_ksmbd_stats, core::ptr::null_mut())) {
    retval = -ENOMEM;
    goto err_out;
    }
    ksmbd_proc_reset();
