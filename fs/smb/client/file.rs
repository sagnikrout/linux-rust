...
//
#[no_mangle]
unsafe extern "C" fn cifs_prepare_write(subreq: *mut netfs_io_subrequest) {
    ...
    let mut wsize: usize = req.rreq.wsize;
    int rc;
    if (!wdata.have_xid) {
    wdata.xid = get_xid();
    wdata.have_xid = true;
...
//
#[no_mangle]
unsafe extern "C" fn cifs_issue_write(subreq: *mut netfs_io_subrequest) {
...
#[no_mangle]
unsafe extern "C" fn cifs_netfs_invalidate_cache(wreq: *mut netfs_io_request) {
...
//
#[no_mangle]
unsafe extern "C" fn cifs_prepare_read(subreq: *mut netfs_io_subrequest) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn cifs_issue_read(subreq: *mut netfs_io_subrequest) {
...
//
#[no_mangle]
unsafe extern "C" fn cifs_begin_writeback(wreq: *mut netfs_io_request) {
...
//
#[no_mangle]
unsafe extern "C" fn cifs_init_request(rreq: *mut netfs_io_request, file: *mut file) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn cifs_rreq_done(rreq: *mut netfs_io_request) {
...
#[no_mangle]
unsafe extern "C" fn cifs_free_request(rreq: *mut netfs_io_request) {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_convert_flags(oflags: c_uint, rdwr_for_fscache: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn cifs_posix_convert_flags(flags: c_uint) -> u32 {
    static u32 cifs_posix_convert_flags(unsigned int flags)
    {
    let mut posix_flags: u32 = 0;
    if ((flags & O_ACCMODE) == O_RDONLY)
    posix_flags = SMB_O_RDONLY;
#[no_mangle]
pub unsafe extern "C" fn if(O_WRONLY: (flags & O_ACCMODE) ==) -> else {
    else if ((flags & O_ACCMODE) == O_WRONLY)
    posix_flags = SMB_O_WRONLY;
#[no_mangle]
pub unsafe extern "C" fn if(O_RDWR: (flags & O_ACCMODE) ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_get_disposition(flags: c_uint) -> c_int {
    static inline int cifs_get_disposition(unsigned int flags)
    {
    if ((flags & (O_CREAT | O_EXCL)) == (O_CREAT | O_EXCL))
    return FILE_CREATE;
#[no_mangle]
pub unsafe extern "C" fn if(O_TRUNC): (flags & (O_CREAT | O_TRUNC)) == (O_CREAT |) -> else {
    else if ((flags & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC))
    return FILE_OVERWRITE_IF;
#[no_mangle]
pub unsafe extern "C" fn if(O_CREAT: (flags & O_CREAT) ==) -> else {
    else if ((flags & O_CREAT) == O_CREAT)
    return FILE_OPEN_IF;
#[no_mangle]
pub unsafe extern "C" fn if(O_TRUNC: (flags & O_TRUNC) ==) -> else {
    ...
    if (presp_data == core::ptr::null_mut())
    return -ENOMEM;
...
// pinode = cifs_iget(sb, &fattr);
    ...
    int desired_access;
    int disposition;
    let mut create_options: c_int = CREATE_NOT_DIR;
    struct TCP_Server_Info *server = tcon.ses.server;
    struct cifs_open_parms oparms;
    let mut rdwr_for_fscache: c_int = 0;
    if (!server.ops.open)
    return -ENOSYS;
...
#[no_mangle]
unsafe extern "C" fn cifsFileInfo_put_final(cifs_file: *mut cifsFileInfo) {
...
#[no_mangle]
unsafe extern "C" fn cifsFileInfo_put_work(work: *mut work_struct) {
    static void cifsFileInfo_put_work(struct work_struct *work)
    {
...
#[no_mangle]
pub unsafe extern "C" fn serverclose_work(work: *mut work_struct) {
    void serverclose_work(struct work_struct *work)
    {
    struct cifsFileInfo *cifs_file = container_of(work,
    ...
    let mut MAX_RETRIES: c_int = 4;
    do {
    if (server.ops.close_getattr)
    rc = server.ops.close_getattr(0, tcon, cifs_file);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: server->ops->close) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn cifsFileInfo_put(cifs_file: *mut cifsFileInfo) {
...
//
    ...
    struct cifs_sb_info *cifs_sb = CIFS_SB(sb);
    let mut fid: cifs_fid = {};
    struct cifs_pending_open open;
    bool oplock_break_cancelled;
    let mut serverclose_offloaded: bool = false;
    spin_lock(&tcon.open_file_lock);
...
#[no_mangle]
unsafe extern "C" fn cifs_do_truncate(xid: c_uint, dentry: *mut dentry) -> c_int {
    static int cifs_do_truncate(const unsigned int xid, struct dentry *dentry)
    {
    struct cifsInodeInfo *cinode = CIFS_I(d_inode(dentry));
    struct inode *inode = d_inode(dentry);
    struct cifsFileInfo *cfile = core::ptr::null_mut();
    struct TCP_Server_Info *server;
    struct cifs_tcon *tcon;
    int rc;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_open(inode: *mut inode, file: *mut file) -> c_int {
    int cifs_open(struct inode *inode, struct file *file)
    {
    struct cifs_sb_info *cifs_sb = CIFS_SB(inode);
    let mut data: cifs_open_info_data = {};
    struct cifsFileInfo *cfile = core::ptr::null_mut();
    struct TCP_Server_Info *server;
    struct cifs_pending_open open;
    let mut posix_open_ok: bool = false;
    let mut fid: cifs_fid = {};
    struct tcon_link *tlink;
    struct cifs_tcon *tcon;
...
// can cache locks - no need to relock
    ...
    const char *full_path;
    int desired_access;
    let mut disposition: c_int = FILE_OPEN;
    let mut create_options: c_int = CREATE_NOT_DIR;
    struct cifs_open_parms oparms;
    let mut rdwr_for_fscache: c_int = 0;
    xid = get_xid();
...
//
    ...
    rc = cifs_posix_open(full_path, core::ptr::null_mut(), inode.i_sb,
    cifs_sb.ctx.file_mode /* ignored */,
...
//
    rc = server.ops.open(xid, &oparms, &oplock, core::ptr::null_mut());
...
// indicate that we need to relock the file
    ...
    core::ptr::null_mut(), inode.i_sb, xid);
    } else if (tcon.unix_ext) {
    rc = cifs_get_inode_info_unix(&inode, full_path,
    inode.i_sb, xid);
    } else {
    rc = cifs_get_inode_info(&inode, full_path, core::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn smb2_deferred_work_close(work: *mut work_struct) {
...
pub unsafe extern "C" fn cifs_close(inode: *mut inode, file: *mut file) -> c_int {
    ...
    if (file.private_data != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_closedir(inode: *mut inode, file: *mut file) -> c_int {
...
// not much we can do if it fails anyway, ignore rc
    ...
    cfile.srch_inf.ntwrk_buf_start = core::ptr::null_mut();
    if (cfile.srch_inf.smallBuf)
    cifs_small_buf_release(buf);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: cfile->srch_inf.is_dynamic_buf) -> else {
...
pub const CIFS_LOCK_OP: c_int = 0;
pub const CIFS_READ_OP: c_int = 1;
pub const CIFS_WRITE_OP: c_int = 2;
...
//
    ...
    rc = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !wait) -> else {
    else if (!wait)
    rc = -EACCES;
    else {
...
//
    ...
    let mut saved_type: c_uchar = flock.c.flc_type;
...
//
    ...
    {
    struct cifsInodeInfo *cinode = CIFS_I(file_inode(file));
    let mut rc: c_int = FILE_LOCK_DEFERRED + 1;
    if ((flock.c.flc_flags & FL_POSIX) == 0)
    return rc;
    cifs_down_write(&cinode.lock_sem);
    if (!cinode.can_cache_brlcks) {
    ...
    rc = posix_lock_file(file, flock, core::ptr::null_mut());
    up_write(&cinode.lock_sem);
    return rc;
    }
    int
...
//
    ...
    let mut ftype: c_uchar = flock.c.flc_type;
    if (el == &locks_to_send) {
//
...
//
    ...
    lck.offset, lck.length, core::ptr::null_mut(),
    lck.type, 0);
    if (stored_rc)
    rc = stored_rc;
    list_del(&lck.llist);
...
// lock = 1;
    ...
    {
    let mut rc: c_int = 0;
    let mut length: __u64 = cifs_flock_len(flock);
    struct cifsFileInfo *cfile = (struct cifsFileInfo *)file.private_data;
    struct cifs_tcon *tcon = tlink_tcon(cfile.tlink);
    struct TCP_Server_Info *server = tcon.ses.server;

    let mut netfid: __u16 = cfile.fid.netfid;
    if (posix_lck) {
    int posix_lock_type;
...
//
    ...
    hash_lockowner(flock.c.flc_owner),
    flock.fl_start, length,
    core::ptr::null_mut(), posix_lock_type, wait_flag);
    goto out;
    }
...
#[no_mangle]
pub unsafe extern "C" fn cifs_flock(file: *mut file, cmd: c_int, fl: *mut file_lock) -> c_int {
    int cifs_flock(struct file *file, int cmd, struct file_lock *fl)
    {
    int rc, xid;
    let mut lock: c_int = 0, unlock = 0;
    let mut wait_flag: bool = false;
    let mut posix_lck: bool = false;
    struct cifs_sb_info *cifs_sb;
    struct cifs_tcon *tcon;
    struct cifsFileInfo *cfile;
    __u32 type;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_lock(file: *mut file, cmd: c_int, flock: *mut file_lock) -> c_int {
...
//
    ...
    {
    struct cifsInodeInfo *cinode = CIFS_I(inode);
    let mut allocated_end: u64 = CIFS_INO_BYTES(inode.i_blocks);
    u64 blocks;
...
//
    ...
    let mut end: loff_t = iocb.ki_pos;
    if (written <= 0)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_write_subrequest_terminated(wdata: *mut cifs_io_subrequest, result: isize) {
    ...
    let mut fsuid_only: bool = find_flags & FIND_FSUID_ONLY;
    struct cifsFileInfo *open_file = core::ptr::null_mut();
...
// Return -EBADF if no handle is found and general rc otherwise
    ...
    {
    struct cifsFileInfo *open_file, *inv_file = core::ptr::null_mut();
    bool fsuid_only, with_delete;
    struct cifs_sb_info *cifs_sb;
    let mut any_available: bool = false;
    let mut refind: c_uint = 0;
// ret_file = NULL;
    let mut rc: c_int = -EBADF;
//
// Having a null inode here (because mapping->host was set to zero by
// the VFS or MM) should not happen but we had reports of on oops (due
// to it being zero) during stress testcases so we need to check for it
//
    if (cifs_inode == core::ptr::null_mut()) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cifs_fsync(file: *mut file, start: loff_t, end: loff_t, datasync: c_int) -> c_int {
    ...
    if (!(cifs_sb_flags(cifs_sb) & CIFS_MOUNT_NOSSYNC)) {
    server = tcon.ses.server;
    if (server.ops.flush == core::ptr::null_mut()) {
    rc = -ENOSYS;
    goto fsync_exit;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn cifs_flush(file: *mut file, id: fl_owner_t) -> c_int {
...
//
    ...
    core::ptr::null_mut(), CIFS_WRITE_OP))) {
    rc = -EACCES;
    goto out;
    }
    rc = netfs_buffered_write_iter_locked(iocb, from, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn cifs_direct_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_loose_read_iter(iocb: *mut kiocb, iter: *mut iov_iter) -> isize {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_file_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
//
    ...
    0, core::ptr::null_mut(), CIFS_READ_OP))
...
#[no_mangle]
unsafe extern "C" fn cifs_page_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_file_strict_mmap_prepare(desc: *mut vm_area_desc) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_file_mmap_prepare(desc: *mut vm_area_desc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_inode_writable(cifs_inode: *mut cifsInodeInfo) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_oplock_break(work: *mut work_struct) {
    ...
    unsigned int oplock;
    let mut rc: c_int = 0;
    let mut purge_cache: bool = false, oplock_break_cancelled;
    ...
    server = tcon.ses.server;
    scoped_guard(spinlock, &cinode.open_file_lock) {
    let mut sbflags: c_uint = cifs_sb_flags(cifs_sb);
...
#[no_mangle]
unsafe extern "C" fn cifs_swap_submit_write(ctx: *mut swap_io_ctx) {
    ...
    ret = netfs_unbuffered_write_iter_locked(&sio.iocb, &iter, core::ptr::null_mut());
    if (ret != -EIOCBQUEUED)
    sio.iocb.ki_complete(&sio.iocb, ret);
    }
#[no_mangle]
unsafe extern "C" fn cifs_swap_submit_read(ctx: *mut swap_io_ctx) {
...
#[no_mangle]
unsafe extern "C" fn cifs_swap_deactivate(file: *mut file) {
