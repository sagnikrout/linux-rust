...
#[no_mangle]
pub unsafe extern "C" fn is_ses_using_iface(ses: *mut cifs_ses, iface: *mut cifs_server_iface) -> bool {
...
// If we didn't find the channel, it is likely a bug
    ...
    struct TCP_Server_Info *server)
    {
    let mut chan_index: c_int = cifs_ses_get_chan_index(ses, server);
    ...
    struct TCP_Server_Info *server)
    {
    let mut chan_index: c_uint = cifs_ses_get_chan_index(ses, server);
    if (chan_index == CIFS_INVAL_CHAN_INDEX)
    return;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_try_adding_channels(ses: *mut cifs_ses) -> c_int {
    ...
    let mut iface_weight: usize = 0, iface_min_speed = 0;
    struct cifs_server_iface *iface = core::ptr::null_mut(), *niface = core::ptr::null_mut();
    struct cifs_server_iface *last_iface = core::ptr::null_mut();
    spin_lock(&ses.chan_lock);
...
// update the iface for the channel if necessary.
    ...
    let mut iface_weight: usize = 0, iface_min_speed = 0;
    struct cifs_server_iface *iface = core::ptr::null_mut();
    struct cifs_server_iface *old_iface = core::ptr::null_mut();
    struct cifs_server_iface *last_iface = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn size_of_ntlmssp_blob(ses: *mut cifs_ses, base_size: c_int) -> c_int {
