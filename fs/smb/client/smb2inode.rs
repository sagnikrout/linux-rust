...
//! Source: fs/smb/client/smb2inode.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn file_create_options(dentry: *mut dentry) -> __u32 {
...
// Parse owner and group from SMB3.1.1 POSIX query info
    ...
    let mut out_len: c_uint = le32_to_cpu(qi.OutputBufferLength);
    let mut qi_len: c_uint = sizeof(data.posix_fi);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct wsl_query_ea {
    pub next: __le32,
    pub name_len: __u8,
    pub 1]: __u8 name[SMB2_WSL_XATTR_NAME_LEN +,
    pub __packed: },
...
#[no_mangle]
unsafe extern "C" fn check_wsl_eas(rsp_iov: *mut kvec) -> c_int {
...
//
    ...
    {
    struct smb2_create_rsp *create_rsp = core::ptr::null_mut();
    struct smb2_query_info_rsp *qi_rsp = core::ptr::null_mut();
    struct smb2_compound_vars *vars = core::ptr::null_mut();
...
// Operation
    ...
    PATH_MAX * 2, 0, core::ptr::null_mut());
    if (rc)
    goto finished;
...
// TBD: fix following to allow for longer SIDs
    ...
    (PATH_MAX * 2) +
    (sizeof(struct smb_sid) * 2), 0, core::ptr::null_mut());
    if (rc)
    goto finished;
    set_next_compound(tcon, cfile, i, num_cmds, rqst, &num_rqst);
...
//
    ...
    core::ptr::null_mut(), 0, CIFSMaxBufSize);
    if (rc)
    goto finished;
    set_next_compound(tcon, cfile, i, num_cmds, rqst, &num_rqst);
...
// smb2_parse_contexts() fills idata->fi.IndexNumber
    ...
    oparms.fid.lease_key, &oplock, &idata.fi, core::ptr::null_mut());
    if (rc)
    ...
    let mut reparse_point: bool = false;
    let mut tag: u32 = 0;
    let mut rc: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn free_rsp_iov(iovs: *mut kvec, buftype: *mut c_int, count: c_int) {
    ...
    struct cifsFileInfo *cfile;
    let mut create_options: __u32 = 0;
    int out_buftype[5] = {};
    struct smb2_hdr *hdr;
...
// If it is a root and its handle is cached then use it
    ...
    cfile, out_iov, out_buftype, core::ptr::null_mut());
...
//
    ...
    cfile, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    if (tmprc == 0)
    ...
    __le16 *utf16_path __free(kfree) = core::ptr::null_mut();
    let mut retries: c_int = 0, cur_sleep = 0;
    struct TCP_Server_Info *server;
    struct cifs_open_parms oparms;
...
//
    ...
    struct cifs_open_parms oparms;
    struct kvec in_iov;
    __le16 *smb2_to_name = core::ptr::null_mut();
    int rc;
    smb2_to_name = cifs_convert_path_to_utf16(to_name, cifs_sb);
    if (smb2_to_name == core::ptr::null_mut()) {
    rc = -ENOMEM;
    if (cfile)
    cifsFileInfo_put(cfile);
    ...
    cfile, core::ptr::null_mut(), core::ptr::null_mut(), dentry);
    smb2_rename_path:
    kfree(smb2_to_name);
    return rc;
    }
    ...
    struct cifs_sb_info *cifs_sb)
    {
    struct inode *inode = source_dentry ? d_inode(source_dentry) : core::ptr::null_mut();
    struct cifsFileInfo *cfile;
    let mut co: __u32 = file_create_options(source_dentry);
    ...
    co, DELETE, SMB2_OP_RENAME, cfile, core::ptr::null_mut());
    }
    return rc;
    }
    ...
    SMB2_OP_HARDLINK, cfile, core::ptr::null_mut());
    }
    int
    ...
    struct dentry *dentry)
    {
    struct inode *inode = dentry ? d_inode(dentry) : core::ptr::null_mut();
    let mut eof: __le64 = cpu_to_le64(size);
    struct cifs_open_parms oparms;
    struct cifsFileInfo *cfile;
    struct kvec in_iov;
    int rc;
    ...
    {
    let mut in_iov: kvec = { .iov_base = buf, .iov_len = sizeof(*buf), };
...
//
    ...
    cifs_get_writable_path(tcon, full_path, core::ptr::null_mut(), FIND_ANY, &cfile);
    rc = smb2_compound_op(xid, tcon, cifs_sb, full_path, &oparms,
    in_iov, cmds, 2, cfile, out_iov, out_buftype, core::ptr::null_mut());
    if (!rc) {
...
//
    if (rc &&
    out_iov[0].iov_base != core::ptr::null_mut() && out_buftype[0] != CIFS_NO_BUFFER &&
    ((struct smb2_hdr *)out_iov[0].iov_base).Status == STATUS_SUCCESS &&
    (out_iov[1].iov_base == core::ptr::null_mut() || out_buftype[1] == CIFS_NO_BUFFER ||
    ((struct smb2_hdr *)out_iov[1].iov_base).Status != STATUS_SUCCESS))
    smb2_unlink(xid, tcon, full_path, cifs_sb, core::ptr::null_mut());
    ...
    struct cifsFileInfo *cfile;
    let mut in_iov: kvec = { .iov_base = &data, .iov_len = sizeof(data), };
    int rc;
...
// rsp_buftype = data.reparse.io.buftype;
    ...
    __le16 *utf16_path __free(kfree) = core::ptr::null_mut();
    let mut co: __u32 = file_create_options(dentry);
    int cmds[] = {
    SMB2_OP_SET_INFO,
    SMB2_OP_RENAME,
    SMB2_OP_UNLINK,
    };
    let mut num_cmds: c_int = ARRAY_SIZE(cmds);
    char *to_name __free(kfree) = core::ptr::null_mut();
    let mut attrs: __u32 = cinode.cifsAttrs;
    struct cifs_open_parms oparms;
    struct cifsFileInfo *cfile;
    struct tcon_link *tlink;
    struct cifs_tcon *tcon;
    ...
    cmds, num_cmds, cfile, core::ptr::null_mut(), core::ptr::null_mut(), dentry);
    if (rc == -EINVAL) {
    cifs_dbg(FYI, "invalid lease key, resending request without lease\n");
    cifs_get_writable_path(tcon, full_path, d_inode(dentry),
    FIND_WITH_DELETE, &cfile);
    rc = smb2_compound_op(xid, tcon, cifs_sb, full_path, &oparms, iov,
    cmds, num_cmds, cfile, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    }
    if (!rc) {
