...
//! Source: fs/smb/client/smb2transport.c
#![no_std]
...
//
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn smb3_get_sign_key(ses_id: __u64, server: *mut TCP_Server_Info, key: *mut u8) -> c_int {
...
// tcon already has a ref to ses, so we don't need ses anymore
    ...
    let mut sid: __u64 = le64_to_cpu(shdr.SessionId);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct derivation_triplet {
    pub signing: derivation,
    pub encryption: derivation,
    pub decryption: derivation,
}
    ...
    {
    let mut full_key_size: c_uint = SMB2_NTLMV2_SESSKEY_SIZE;
    let mut is_binding: bool = false;
    let mut chan_index: c_int = 0;
    spin_lock(&ses.ses_lock);
    spin_lock(&ses.chan_lock);
    is_binding = (cifs_chan_needs_reconnect(ses, server) &&
    ses.ses_status == SES_GOOD);
...
// skip message numbers according to CreditCharge field
    ...
    let mut credits: c_uint = le16_to_cpu(shdr.CreditCharge);
...
// mid = smb2_mid_entry_alloc(shdr, server);
    ...
    bool log_error)
    {
    let mut len: c_uint = mid.resp_buf_size;
    struct kvec iov[1];
