...
//! Source: fs/smb/client/netmisc.c
#![no_std]
...
// calculate length by finding first slash or NULL
    ...
    ret = in4_pton(cp, len, dst, '\\', core::ptr::null_mut());
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: address_family ==) -> else {
    else if (address_family == AF_INET6)
    ret = in6_pton(cp, len, dst , '\\', core::ptr::null_mut());
...
// Subtract the NTFS time offset, then convert to 1s intervals.
    let mut t: i64 = le64_to_cpu(ntutc) - NTFS_TIME_OFFSET;
    u64 abs_t;
//
...
#[no_mangle]
pub unsafe extern "C" fn cnvrtDosUnixTm(le_date: __le16, le_time: __le16, offset: c_int) -> timespec64 {
    struct timespec64 cnvrtDosUnixTm(__le16 le_date, __le16 le_time, int offset)
    {
    struct timespec64 ts;
    time64_t sec, days;
    int min, day, month, year;
    let mut date: u16 = le16_to_cpu(le_date);
    let mut time: u16 = le16_to_cpu(le_time);
