...
//! Source: fs/smb/client/smb2ops.c
#![no_std]
...
// Change credits for different ops and return the total number of credits
    ...
    let mut add: c_uint = credits.value;
    let mut instance: c_uint = credits.instance;
    let mut reconnect_detected: bool = false;
    let mut reconnect_with_invalid_credits: bool = false;
...
// for SMB2 we need the current value
    ...
    let mut wire_mid: __u64 = le64_to_cpu(shdr.MessageId);
...
//
    ...
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(0: cmp_ret >) -> else {
    else if (cmp_ret > 0)
    return 1;
    else
    return -1;
    ...
    struct cifs_server_iface *info = core::ptr::null_mut(), *iface = core::ptr::null_mut(), *niface = core::ptr::null_mut();
    struct cifs_server_iface tmp_iface;
    __be16 port;
    ssize_t bytes_left;
    let mut next: usize = 0;
    let mut nb_iface: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(AF_INET6: ses->server->dstaddr.ss_family ==) -> else {
...
//
    ...
    {
    int rc;
    let mut ret_data_len: c_uint = 0;
    struct network_interface_info_ioctl_rsp *out_buf = core::ptr::null_mut();
...
// do not query too frequently
    ...
    core::ptr::null_mut() /* no data input */, 0 /* no data input */,
...
// check if iface is still active
    ...
    let mut oplock: u8 = SMB2_OPLOCK_LEVEL_NONE;
    struct cifs_open_parms oparms;
    struct cifs_fid fid;
    struct cached_fid *cfid = core::ptr::null_mut();
    ...
    rc = SMB2_open(xid, &oparms, &srch_path, &oplock, core::ptr::null_mut(), core::ptr::null_mut(),
    ...
    {
    __le16 *utf16_path;
    let mut oplock: __u8 = SMB2_OPLOCK_LEVEL_NONE;
    let mut err_buftype: c_int = CIFS_NO_BUFFER;
    struct cifs_open_parms oparms;
    let mut err_iov: kvec = {};
    struct cifs_fid fid;
    ...
    rc = SMB2_open(xid, &oparms, utf16_path, &oplock, core::ptr::null_mut(), core::ptr::null_mut(),
    &err_iov, &err_buftype);
    if (rc) {
...
// uniqueid = le64_to_cpu(data->fi.IndexNumber);
    ...
    {
    let mut rc: c_int = 0;
    let mut ea_name_len: c_uint = ea_name ? strlen(ea_name) : 0;
    char *name, *value;
    let mut buf_size: usize = dst_size;
...
// didn't find the named attribute
    ...
    {
    int rc;
    let mut rsp_iov: kvec = {core::ptr::null_mut(), 0};
    let mut buftype: c_int = CIFS_NO_BUFFER;
    struct smb2_query_info_rsp *rsp;
    struct smb2_file_full_ea_info *info = core::ptr::null_mut();
...
//
    ...
    struct kvec *rsp_iov;
    __le16 *utf16_path = core::ptr::null_mut();
    let mut ea_name_len: c_int = strlen(ea_name);
    let mut flags: c_int = CIFS_CP_CREATE_CLOSE_OP;
    int len;
    int resp_buftype[3];
    struct cifs_open_parms oparms;
    let mut oplock: __u8 = SMB2_OPLOCK_LEVEL_NONE;
    ...
    struct smb2_query_info_rsp *rsp;
    int rc, used_len = 0;
    let mut retries: c_int = 0, cur_sleep = 0;
    replay_again:
...
// End of file and Attributes should not have to be updated on close
    ...
    FSCTL_SRV_REQUEST_RESUME_KEY, core::ptr::null_mut(), 0 /* no input */,
    ...
    let mut rc: c_int = 0;
    let mut flags: c_int = CIFS_CP_CREATE_CLOSE_OP;
    struct smb2_query_info_rsp *qi_rsp = core::ptr::null_mut();
    struct smb2_ioctl_rsp *io_rsp = core::ptr::null_mut();
    ...
    unsigned int size[2];
    void *data[2];
    let mut create_options: c_int = is_dir ? CREATE_NOT_FILE : CREATE_NOT_DIR;
...
//
    ...
    {
    let mut max_chunks: u32 = READ_ONCE(tcon.max_chunks);
    let mut max_bytes_copy: u32 = READ_ONCE(tcon.max_bytes_copy);
    u64 need;
    u32 allowed;
...
//
    ...
    let mut rc: c_int = 0;
    let mut ret_data_len: c_uint = 0;
    struct copychunk_ioctl_req *cc_req = core::ptr::null_mut();
    struct copychunk_ioctl_rsp *cc_rsp = core::ptr::null_mut();
    struct cifs_tcon *tcon;
    struct srv_copychunk *chunk;
    u32 chunks, chunk_count, chunk_bytes, chunk_size;
    u32 copy_bytes, copy_bytes_left;
    u32 chunks_written, bytes_written;
    let mut total_bytes_left: u64 = len;
...
//
    ...
    let mut delta: u64 = dst_off - src_off;
    if (delta < len) {
    reverse = true;
    max_chunk = delta;
    }
...
// Partial write: rewind
    ...
    let mut delta: u32 = copy_bytes - bytes_written;
    src_off -= delta;
    dst_off -= delta;
    }
...
//
    ...
    &setsparse, 1, CIFSMaxBufSize, core::ptr::null_mut(), core::ptr::null_mut());
    if (rc) {
    if (rc == -EOPNOTSUPP)
...
// server fileays advertise duplicate extent support with this flag
    ...
    CIFSMaxBufSize, core::ptr::null_mut(),
    &ret_data_len);
    if (ret_data_len > 0)
...
pub const GMT_TOKEN_SIZE: c_int = 50;
...
//
    ...
    core::ptr::null_mut(), 0 /* no input data */, max_response_size,
    (char **)&retbuf,
    &ret_data_len);
    ...
    if (rc)
    return rc;
    if (ret_data_len && (ioc_buf != core::ptr::null_mut()) && (retbuf != core::ptr::null_mut())) {
// Fixup buffer
...
//
    ...
    char *returned_ioctl_info = core::ptr::null_mut();
    void *page = alloc_dentry_path();
    __le16 *utf16_path = core::ptr::null_mut();
    let mut oplock: u8 = SMB2_OPLOCK_LEVEL_NONE;
    let mut rc: c_int = 0;
    let mut ret_len: __u32 = 0;
    ...
    utf16_path = cifs_convert_path_to_utf16(path, cifs_sb);
    if (utf16_path == core::ptr::null_mut()) {
    rc = -ENOMEM;
    goto notify_exit;
    }
    if (return_changes) {
    ...
    rc = SMB2_open(xid, &oparms, utf16_path, &oplock, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(),
    core::ptr::null_mut());
    if (rc)
    goto notify_exit;
    ...
    rc = -EFAULT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: copy_to_user(&pnotify_buf->data_len, _arg: &ret_len, _arg: sizeof(ret_len))) -> else {
    ...
    let mut oplock: u8 = SMB2_OPLOCK_LEVEL_NONE;
    struct cifs_open_parms oparms;
    struct smb2_query_directory_rsp *qd_rsp = core::ptr::null_mut();
    struct smb2_create_rsp *op_rsp = core::ptr::null_mut();
    struct TCP_Server_Info *server;
    let mut retries: c_int = 0, cur_sleep = 0;
    replay_again:
// reinitialize for possible replay
...
// If the open failed there is nothing to do
    ...
    if (op_rsp == core::ptr::null_mut() || op_rsp.hdr.Status != STATUS_SUCCESS) {
...
// If server is a channel, select the primary channel
    ...
    {
    let mut sbflags: c_uint = cifs_sb_flags(CIFS_SB(cinode));
    __u8 op;
    ...
    if (shdr == core::ptr::null_mut()) {
    cifs_dbg(FYI, "shdr core::ptr::null_mut() in smb2_set_related\n");
    return;
    }
    ...
    let mut len: c_ulong = smb_rqst_len(server, rqst);
    int num_padding;
    shdr = (struct smb2_hdr *)(rqst.rq_iov[0].iov_base);
    if (shdr == core::ptr::null_mut()) {
    cifs_dbg(FYI, "shdr core::ptr::null_mut() in smb2_set_next_command\n");
    return;
    }
...
// buftype = resp_buftype[1];
    ...
    {
    struct smb2_query_info_rsp *rsp;
    struct smb2_fs_full_size_info *info = core::ptr::null_mut();
    let mut rsp_iov: kvec = {core::ptr::null_mut(), 0};
    let mut buftype: c_int = CIFS_NO_BUFFER;
    int rc;
    rc = smb2_query_info_compound(xid, tcon, path,
    ...
    int rc;
    __le16 *utf16_path = core::ptr::null_mut();
    let mut utf16_path_len: c_int = 0;
    struct cifs_tcon *tcon;
    struct fsctl_get_dfs_referral_req *dfs_req = core::ptr::null_mut();
    struct get_dfs_referral_rsp *dfs_rsp = core::ptr::null_mut();
    let mut dfs_req_size: u32 = 0, dfs_rsp_size = 0;
    let mut retry_once: c_int = 0;
    cifs_dbg(FYI, "%s: path: %s\n", __func__, search_name);
//
// Try to use the IPC tcon, otherwise just use any
//
    tcon = ses.tcon_ipc;
    if (tcon == core::ptr::null_mut()) {
    spin_lock(&cifs_tcp_ses_lock);
...
// ipc tcons are not refcounted
    ...
    {
    struct smb_ntsd *pntsd = core::ptr::null_mut();
    unsigned int xid;
...
// Retrieve an ACL from the server
    ...
    {
    struct smb_ntsd *pntsd = core::ptr::null_mut();
    struct cifsFileInfo *open_file = core::ptr::null_mut();
    ...
    let mut top: c_ulonglong = umin(offset + len, i_size);
...
// if file not oplocked can't be sure whether asking to extend size
    ...
    goto zero_range_exit;
    fscache_invalidate(cifs_inode_cookie(inode), core::ptr::null_mut(),
    i_size_read(inode), 0);
...
//
    ...
    struct cifsFileInfo *cfile = file.private_data;
    struct file_zero_data_information fsctl_buf;
    let mut end: c_ulonglong = offset + len, i_size, remote_i_size, zero_point;
    long rc;
    unsigned int xid;
    let mut set_sparse: __u8 = 1;
    xid = get_xid();
...
//
    ...
    (char *)&fsctl_buf,
    sizeof(struct file_zero_data_information),
    CIFSMaxBufSize, core::ptr::null_mut(), core::ptr::null_mut());
    if (rc)
    goto unlock;
...
//
    ...
    let mut extend_to: c_ulonglong = umin(end, i_size);
    rc = SMB2_set_eof(xid, tcon, cfile.fid.persistent_fid,
    ...
    char *buf)
    {
    let mut io_parms: cifs_io_parms = {0};
    unsigned int nbytes;
    let mut rc: c_int = 0;
    struct kvec iov[2];
...
// iov[0] is reserved for smb header
    ...
    loff_t off, loff_t len)
    {
    struct file_allocated_range_buffer in_data, *out_data = core::ptr::null_mut(), *tmp_data;
...
//
    ...
    fscache_invalidate(cifs_inode_cookie(inode), core::ptr::null_mut(), old_eof, 0);
    spin_lock(&inode.i_lock);
...
#[no_mangle]
unsafe extern "C" fn smb3_llseek(file: *mut file, tcon: *mut cifs_tcon, offset: loff_t, whence: c_int) -> loff_t {
    ...
    struct inode *inode;
    let mut rc: c_int = 0;
    struct file_allocated_range_buffer in_data, *out_data = core::ptr::null_mut();
    u32 out_data_len;
    unsigned int xid;
...
#[no_mangle]
pub unsafe extern "C" fn if(FALLOC_FL_ZERO_RANGE: mode &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(FALLOC_FL_COLLAPSE_RANGE: mode ==) -> else {
    else if (mode == FALLOC_FL_COLLAPSE_RANGE)
    return smb3_collapse_range(file, tcon, off, len);
#[no_mangle]
pub unsafe extern "C" fn if(FALLOC_FL_INSERT_RANGE: mode ==) -> else {
    else if (mode == FALLOC_FL_INSERT_RANGE)
    return smb3_insert_range(file, tcon, off, len);
#[no_mangle]
pub unsafe extern "C" fn if(FALLOC_FL_ALLOCATE_RANGE: mode ==) -> else {
    ...
    {
    lockdep_assert_held(&cinode.open_file_lock);
    server.ops.set_oplock_level(cinode, oplock, 0, core::ptr::null_mut());
    }
    static void
    ...
    __u16 epoch, bool *purge_cache)
    {
    let mut old_state: c_uint = cinode.oplock;
    let mut old_epoch: __u16 = cinode.epoch;
    unsigned int new_state;
    if (epoch > old_epoch) {
    smb21_set_oplock_level(cinode, oplock, 0, core::ptr::null_mut());
    cinode.epoch = epoch;
    }
...
// purge_cache = true;
#[no_mangle]
pub unsafe extern "C" fn if(1): old_state == new_state && (epoch - old_epoch >) -> else {
...
// Check if the server granted an oplock rather than a lease
    ...
    __u16 epoch, bool *purge_cache)
    {
    let mut old_oplock: c_uint = READ_ONCE(cinode.oplock);
...
// purge_cache = true;
    ...
    {
    let mut oplock: return = = SMB2_OPLOCK_LEVEL_II;
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(SMB2_OPLOCK_LEVEL_II: oplock ==) -> else {
    else if (oplock == SMB2_OPLOCK_LEVEL_II)
    return SMB2_LEASE_READ_CACHING_LE | SMB2_LEASE_HANDLE_CACHING_LE;
#[no_mangle]
pub unsafe extern "C" fn if(SMB2_OPLOCK_LEVEL_BATCH: oplock ==) -> else {
...
// epoch = le16_to_cpu(lc->lcontext.Epoch);
    ...
    {
    let mut req_size: c_uint = sizeof(**req) + crypto_aead_reqsize(tfm);
    let mut iv_size: c_uint = crypto_aead_ivsize(tfm);
    unsigned int len;
    int ret;
    u8 *p;
...
//
    ...
    let mut assoc_data_len: c_uint = sizeof(struct smb2_transform_hdr) - 20;
    let mut rc: c_int = 0;
    ...
    let mut crypt_len: c_uint = le32_to_cpu(tr_hdr.OriginalMessageSize);
    void *creq;
...
//
    ...
    let mut part: usize = folioq_folio_size(buffer, s);
    part = umin(part, size);
...
//
    ...
    let mut orig_len: c_uint = 0;
    let mut rc: c_int = -ENOMEM;
    for (int i = 1; i < num_rqst; i++) {
    struct smb_rqst *old = &old_rq[i - 1];
    struct smb_rqst *new = &new_rq[i];
    struct folio_queue *buffer = core::ptr::null_mut();
    let mut size: usize = iov_iter_count(&old.rq_iter);
    orig_len += smb_rqst_len(server, old);
    new.rq_iov = old.rq_iov;
    new.rq_nvec = old.rq_nvec;
    if (size > 0) {
    let mut cur_size: usize = 0;
    rc = netfs_alloc_folioq_buffer(core::ptr::null_mut(), &buffer, &cur_size,
    size, GFP_NOFS);
...
// fill the 1st iov with a transform header
    ...
    let mut rqst: smb_rqst = {core::ptr::null_mut()};
    struct kvec iov[2];
    let mut iter_size: usize = 0;
    int rc;
    iov[0].iov_base = buf;
    ...
    struct smb2_hdr *shdr = (struct smb2_hdr *)buf;
    size_t copied;
    let mut use_rdma_mr: bool = false;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct smb2_decrypt_work {
    pub decrypt: work_struct,
    pub server: *mut TCP_Server_Info,
    pub buffer: *mut folio_queue,
    pub buf: *mut c_char,
    pub len: c_uint,
}

#[no_mangle]
unsafe extern "C" fn smb2_decrypt_offload(work: *mut work_struct) {
    ...
    struct iov_iter iter;
    unsigned int len;
    let mut buflen: c_uint = server.pdu_size;
    int rc;
...
// num_mids = 1;
    ...
    let mut cur_size: usize = 0;
    rc = netfs_alloc_folioq_buffer(core::ptr::null_mut(), &dw.buffer, &cur_size, len, GFP_NOFS);
    if (rc < 0)
    goto discard_data;
...
// mid = smb2_find_mid(server, buf);
    ...
    char *buf = server.smallbuf;
    struct smb2_hdr *shdr;
    let mut pdu_length: c_uint = server.pdu_size;
    unsigned int buf_size;
    unsigned int next_cmd;
    struct mid_q_entry *mid_entry;
    int next_is_large;
    char *next_buffer = core::ptr::null_mut();
// num_mids = 0;
...
// now read the rest
    ...
    length = decrypt_raw_data(server, buf, buf_size, core::ptr::null_mut(), false);
    if (length)
    return length;
    ...
    if (mid_entry == core::ptr::null_mut())
    cifs_dbg(FYI, "mid not found\n");
    else {
    cifs_dbg(FYI, "mid found\n");
...
//
    ...
    let mut pdu_length: c_uint = server.pdu_size;
    struct smb2_transform_hdr *tr_hdr = (struct smb2_transform_hdr *)buf;
    let mut orig_len: c_uint = le32_to_cpu(tr_hdr.OriginalMessageSize);
...
// noff = le32_to_cpu(hdr->NextCommand);
    ...
    struct TCP_Server_Info *server = tcon.ses.server;
    struct cifs_open_parms oparms;
    let mut idata: cifs_open_info_data = {};
    let mut io_parms: cifs_io_parms = {};
    struct cifs_sb_info *cifs_sb = CIFS_SB(inode.i_sb);
    struct cifs_fid fid;
    unsigned int bytes_written;
    u8 type[8];
    let mut type_len: c_int = 0;
    struct {
    __le64 major;
    __le64 minor;
    } __packed pdev = {};
    __le16 *symname_utf16 = core::ptr::null_mut();
    u8 *data = core::ptr::null_mut();
    let mut data_len: c_int = 0;
    struct kvec iov[3];
    let mut oplock: __u32 = server.oplocks ? REQ_OPLOCK : 0;
    int rc;
    switch (mode & S_IFMT) {
    case S_IFCHR:
    type_len = 8;
...
//
    if (rc)
    server.ops.unlink(xid, tcon, full_path, cifs_sb, core::ptr::null_mut());
    out:
    kfree(symname_utf16);
    return rc;
    }
    ...
    const char *full_path, umode_t mode, dev_t dev)
    {
    struct inode *new = core::ptr::null_mut();
    int rc;
    rc = __cifs_sfu_make_node(xid, inode, dentry, tcon,
    full_path, mode, dev, core::ptr::null_mut());
    if (rc)
    return rc;
    if (tcon.posix_extensions) {
    rc = smb311_posix_get_inode_info(&new, full_path, core::ptr::null_mut(),
    inode.i_sb, xid);
    } else if (tcon.unix_ext) {
    rc = cifs_get_inode_info_unix(&new, full_path,
    inode.i_sb, xid);
    } else {
    rc = cifs_get_inode_info(&new, full_path, core::ptr::null_mut(),
    inode.i_sb, xid, core::ptr::null_mut());
    }
    if (!rc)
    ...
    const char *full_path, umode_t mode, dev_t dev)
    {
    let mut sbflags: c_uint = cifs_sb_flags(CIFS_SB(inode));
    let mut rc: c_int = -EOPNOTSUPP;
//
