...
//! Source: fs/smb/client/smb2misc.c
#![no_std]
...
// length of negcontexts including pad from end of sec blob to them
    ...
    let mut hdr_size: c_int = sizeof(struct smb2_hdr);
    let mut pdu_size: c_int = sizeof(struct smb2_pdu);
    int command;
...
//
    ...
    {
    let mut max_off: c_int = 4096;
    let mut max_len: c_int = 128 * 1024;
// off = 0;
// len = 0;
...
// Structure Size has already been checked to make sure it is 64
    let mut len: c_int = le16_to_cpu(shdr.StructureSize);
    if (have_data)
// have_data = false;
    if (data_area_overlap)
// data_area_overlap = false;
...
// have_data = (data_length > 0);
    ...
    {
    return __smb2_calc_size(buf, core::ptr::null_mut(), core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn smb2_get_lease_state(cinode: *mut cifsInodeInfo, oplock: c_uint) -> __le32 {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct smb2_lease_break_work {
    pub lease_break: work_struct,
    pub tlink: *mut tcon_link,
    pub lease_key: [__u8; 16],
    pub lease_state: __le32,
}
    ...
    {
    let mut lease_state: __u8 = le32_to_cpu(rsp.NewLeaseState);
    int ack_req = le32_to_cpu(rsp.Flags &
    SMB2_NOTIFY_BREAK_LEASE_FLAG_ACK_REQUIRED);
    struct cifs_pending_open *open;
    struct cifs_pending_open *found = core::ptr::null_mut();
