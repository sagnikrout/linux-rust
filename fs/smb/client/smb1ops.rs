...
//! Source: fs/smb/client/smb1ops.c
#![no_std]
...
//
    let mut saved_cap: __u64 = le64_to_cpu(tcon.fsUnixInfo.Capability);
    ...
    let mut cap: __u64 = le64_to_cpu(tcon.fsUnixInfo.Capability);
...
//
    ...
    cap &= ~CIFS_UNIX_POSIX_ACL_CAP;
#[no_mangle]
pub unsafe extern "C" fn if(cap: CIFS_UNIX_POSIX_ACL_CAP &) -> else {
    ...
    cap &= ~CIFS_UNIX_POSIX_PATHNAMES_CAP;
#[no_mangle]
pub unsafe extern "C" fn if(CIFS_UNIX_POSIX_PATHNAMES_CAP: cap &) -> else {
...
//
    ...
    struct kvec iov[1];
    let mut crqst: smb_rqst = { .rq_iov = iov, .rq_nvec = 1 };
    let mut rc: c_int = 0;
// +2 for BCC field
...
//
    ...
    {
    struct smb_hdr *in_buf = (struct smb_hdr *)rqst.rq_iov[0].iov_base;
    let mut in_len: c_uint = rqst.rq_iov[0].iov_len;
...
//
    ...
    rc = SendReceive(xid, ses, in_buf, in_len, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (rc == -ENOLCK)
...
//
    ...
    {
    let mut unix_cap: __u64 = le64_to_cpu(tcon.fsUnixInfo.Capability);
...
// start with specified wsize, or default
    ...
    wsize = ctx.vol_wsize;
#[no_mangle]
pub unsafe extern "C" fn if(CIFS_UNIX_LARGE_WRITE_CAP): tcon->unix_ext && (unix_cap &) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(CAP_LARGE_READ_X: server->capabilities &) -> else {
...
//
    ...
    {
    let mut rc: c_int = -EOPNOTSUPP;
    let mut fi: FILE_ALL_INFO = {};
    let mut search_info: cifs_search_info = {};
    let mut non_unicode_wildcard: bool = false;
...
//
    ...
    rc = CIFSFindFirst(xid, tcon, full_path, cifs_sb, core::ptr::null_mut(),
    CIFS_SEARCH_CLOSE_ALWAYS | CIFS_SEARCH_CLOSE_AT_END,
    &search_info, false);
    if (rc == 0) {
    if (!(tcon.ses.capabilities & tcon.ses.server.vals.cap_nt_find)) {
    FIND_FILE_STANDARD_INFO *di;
    let mut offset: c_int = tcon.ses.server.timeAdj;
...
//
    ...
    let mut query_data: cifs_open_info_data = {};
    let mut write_time: __le64 = buf.LastWriteTime;
...
//
    rc = cifs_open_file(xid, &oparms, &oplock, core::ptr::null_mut());
//
...
// Fallback to SMB_COM_SETATTR command when absolutely needed.
    ...
    if (!rc)
    cinode.cifsAttrs = le32_to_cpu(buf.Attributes);
    if (open_file == core::ptr::null_mut())
...
//
    ...
    int rc;
    let mut referral: dfs_info3_param = {0};
...
// plen = le16_to_cpu(io->ByteCount);
    ...
    {
    let mut oplock: return = = OPLOCK_READ;
    }
    static unsigned int
