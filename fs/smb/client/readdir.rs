...
//! Source: fs/smb/client/readdir.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn dump_cifs_file_struct(file: *mut file, label: *mut c_char) {
    static void dump_cifs_file_struct(struct file *file, char *label)
    {
    struct cifsFileInfo *cf;
    if (file) {
    cf = file.private_data;
    if (cf == core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn dump_cifs_file_struct(file: *mut file, label: *mut c_char) {
...
//
    ...
    let mut posix: bool = cifs_sb_master_tcon(cifs_sb).posix_extensions;
    let mut reparse_need_reval: bool = false;
    int rc;
...
// Fill a cifs_fattr struct with info from SMB_FIND_FILE_POSIX_INFO.
    ...
    posix_info_parse(info, core::ptr::null_mut(), &parsed);
...
#[no_mangle]
unsafe extern "C" fn __dir_info_to_fattr(fattr: *mut cifs_fattr, info: *const c_void) {
...
// See MS-FSCC 2.4.14, 2.4.19
    ...
    struct cifs_sb_info *cifs_sb)
    {
    let mut offset: c_int = cifs_sb_master_tcon(cifs_sb).ses.server.timeAdj;
    memset(fattr, 0, sizeof(*fattr));
...
#[no_mangle]
unsafe extern "C" fn cifs_unicode_bytelen(str: *const c_char) -> c_int {
    ...
    let mut next_offset: u32 = le32_to_cpu(pDirInfo.NextEntryOffset);
...
// payload should have already been checked at this point
    if (posix_info_parse(info, core::ptr::null_mut(), &parsed) < 0) {
...
#[no_mangle]
unsafe extern "C" fn cifs_entry_is_dot(de: *mut cifs_dirent, is_unicode: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_dir_changed(file: *mut file) -> c_int {
...
//
    ...
    __u16 search_flags;
    let mut rc: c_int = 0;
    let mut pos_in_buf: c_int = 0;
    loff_t first_entry_in_buffer;
    let mut index_to_find: loff_t = pos;
...
// check if index in the buffer
    ...
    return -ENOSYS;
    if ((cfile == core::ptr::null_mut()) || (current_entry == core::ptr::null_mut()) || (num_to_ret == core::ptr::null_mut()))
    return -ENOENT;
// current_entry = NULL;
...
// Reset all pointers to the network buffer to prevent stale references
    ...
    cfile.srch_inf.srch_entries_start = core::ptr::null_mut();
    cfile.srch_inf.last_entry = core::ptr::null_mut();
    }
...
// scan and find it
    ...
    char *cur_ent;
    char *end_of_smb;
    if (cfile.srch_inf.ntwrk_buf_start == core::ptr::null_mut()) {
    cifs_dbg(VFS, "ntwrk_buf_start is core::ptr::null_mut() during readdir\n");
    ...
    for (i = 0; (i < (pos_in_buf)) && (cur_ent != core::ptr::null_mut()); i++) {
// go entry by entry figuring out which is first
    cur_ent = nxt_dir_entry(cur_ent, end_of_smb,
    cfile.srch_inf.info_level);
    }
    if ((cur_ent == core::ptr::null_mut()) && (i < pos_in_buf)) {
...
//
    ...
    if (de == core::ptr::null_mut()) {
    cde.is_failed = 1;
    return false;
    }
    de.namelen = namelen;
    de.name = kstrndup(name, namelen, GFP_ATOMIC);
    if (de.name == core::ptr::null_mut()) {
    kfree(de);
    cde.is_failed = 1;
    return false;
    }
...
// update accounting
    ...
    bool rc, added = false;
    let mut ino: ino_t = cifs_uniqueid_to_ino_t(fattr.cf_uniqueid);
...
// per-tcon then global for consistency with free path
    ...
    struct cifs_sb_info *cifs_sb = CIFS_SB(sb);
    let mut de: cifs_dirent = { core::ptr::null_mut(), };
    struct cifs_fattr fattr;
    unsigned int sbflags;
    struct qstr name;
    let mut rc: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
...
// directory content.
//
    if (ctx.pos == 0 && cfid.dirents.file == core::ptr::null_mut()) {
...
// 1) If search is active,
    ...
    goto rddir2_exit;
    } else if (current_entry != core::ptr::null_mut()) {
    ...
    if (current_entry == core::ptr::null_mut()) {
