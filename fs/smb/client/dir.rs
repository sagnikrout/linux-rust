...
// BB check if there is a way to get the kernel to do this or if we
    ...
    {
    let mut pplen: c_int = ctx.prepath ? strlen(ctx.prepath) + 1 : 0;
    int dfsplen;
    char *full_path = core::ptr::null_mut();
...
// Note: caller must free return buffer
    ...
    let mut prefix: bool = tcon.Flags & SMB_SHARE_IS_IN_DFS;
    ...
    let mut dirsep: c_char = CIFS_DIR_SEP(cifs_sb);
    let mut pplen: c_int = 0;
    int dfsplen;
    char *s;
    if (unlikely(!page))
...
#[no_mangle]
pub unsafe extern "C" fn if(O_TRUNC: oflags &) -> else {
...
//
    ...
    if (file_info == core::ptr::null_mut()) {
    if (server.ops.close)
    server.ops.close(xid, tcon, &fid);
    cifs_del_pending_open(&open);
    rc = -ENOMEM;
...
// request so much.
//
    let mut oflags: unsigned = O_EXCL | O_CREAT | O_RDWR;
    struct tcon_link *tlink;
    struct cifs_tcon *tcon;
    struct TCP_Server_Info *server;
    struct inode *inode;
...
#[no_mangle]
pub unsafe extern "C" fn cases(itself: beginning of sys_rename) -> deadlock in the {
    ...
    cifs_dbg(FYI, "non-core::ptr::null_mut() inode in lookup\n");
    } else {
    struct cached_fid *cfid = core::ptr::null_mut();
    cifs_dbg(FYI, "core::ptr::null_mut() inode in lookup\n");
//
...
//
    ...
    rc = smb311_posix_get_inode_info(&newInode, full_path, core::ptr::null_mut(),
    parent_dir_inode.i_sb, xid);
    } else if (pTcon.unix_ext) {
    rc = cifs_get_inode_info_unix(&newInode, full_path,
    parent_dir_inode.i_sb, xid);
    } else {
    rc = cifs_get_inode_info(&newInode, full_path, core::ptr::null_mut(),
    parent_dir_inode.i_sb, xid, core::ptr::null_mut());
    }
    if (rc == 0) {
...
#[no_mangle]
unsafe extern "C" fn cifs_ci_hash(dentry: *const dentry, q: *mut qstr) -> c_int {
...
//
    ...
    struct cifs_sb_info *cifs_sb = CIFS_SB(dir);
    let mut namesize: usize = CIFS_TMPNAME_LEN + 1;
    char *path __free(kfree) = core::ptr::null_mut(), *name;
    let mut oflags: c_uint = file.f_flags;
    let mut retries: c_int = 0, max_retries = 16;
    struct TCP_Server_Info *server;
    struct cifs_pending_open open;
...
// Append tmpfile name to @path
    ...
    mode, &oplock, &fid, core::ptr::null_mut(), &inode);
    if (!rc) {
    ...
    cfile = cifs_new_fileinfo(&fid, file, tlink, oplock, core::ptr::null_mut());
    if (!cfile) {
    rc = -ENOMEM;
    goto err_open;
    }
    ...
    let mut retries: c_int = 0, max_retries = 16;
    let mut namesize: usize = sizeof(name);
    struct dentry *sdentry = core::ptr::null_mut();
    char *path;
    do {
    dput(sdentry);
