...
// BB need to add server (Samba e.g) support for security and trusted prefix
    ...
    {
    let mut rc: isize = -EOPNOTSUPP;
    __u32 *pattrib = (__u32 *)value;
    __u32 attrib;
    FILE_BASIC_INFO info_buf;
    if ((value == core::ptr::null_mut()) || (size != sizeof(__u32)))
    return -ERANGE;
    ...
    __u64 creation_time;
    FILE_BASIC_INFO info_buf;
    if ((value == core::ptr::null_mut()) || (size != sizeof(__u64)))
    return -ERANGE;
    memset(&info_buf, 0, sizeof(info_buf));
...
// if proc/fs/cifs/streamstoxattr is set then
    ...
    if (rc)
    return rc;
    if ((value == core::ptr::null_mut()) || (size == 0))
    return sizeof(__u32);
#[no_mangle]
pub unsafe extern "C" fn if(sizeof(__u32): size <) -> else {
...
// pattribute = CIFS_I(inode)->cifsAttrs;
    ...
    return sizeof(__u64);
#[no_mangle]
pub unsafe extern "C" fn if(sizeof(__u64): size <) -> else {
...
//
    ...
    struct smb_ntsd *pacl;
    if (pTcon.ses.server.ops.get_acl == core::ptr::null_mut())
...
// if proc/fs/cifs/streamstoxattr is set then
    ...
    if (pTcon.ses.server.ops.query_all_EAs)
    rc = pTcon.ses.server.ops.query_all_EAs(xid, pTcon,
    full_path, core::ptr::null_mut(), data, buf_size, cifs_sb);
    list_ea_exit:
    free_dentry_path(page);
    free_xid(xid);
    cifs_put_tlink(tlink);
