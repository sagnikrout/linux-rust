...
//! Source: fs/smb/client/connect.c
#![no_std]
...
pub const MAX_STATUS_IO_TIMEOUT: c_int = 5;
...
//
#[no_mangle]
unsafe extern "C" fn reconn_set_ipaddr_from_hostname(server: *mut TCP_Server_Info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn smb2_query_server_interfaces(work: *mut work_struct) {
...
// do not want to be sending data on a socket we are freeing
    ...
    server.ssocket = core::ptr::null_mut();
    ...
    server.session_key.response = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn cifs_tcp_ses_needs_reconnect(server: *mut TCP_Server_Info, num_targets: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn reconnect_dfs_server(server: *mut TCP_Server_Info) -> c_int {
    static int reconnect_dfs_server(struct TCP_Server_Info *server)
    {
    struct dfs_cache_tgt_iterator *target_hint = core::ptr::null_mut();
    const char *ref_path = server.leaf_fullpath + 1;
    DFS_CACHE_TGT_LIST(tl);
    let mut num_targets: c_int = 0;
    let mut rc: c_int = 0;
//
...
//
    if (!dfs_cache_noreq_find(ref_path, core::ptr::null_mut(), &tl))
...
//
    ...
    {
    let mut smb_msg: msghdr = {};
    let mut iov: kvec = {.iov_base = buf, .iov_len = to_read};
...
//
    ...
    unsigned int to_read)
    {
    let mut smb_msg: msghdr = { .msg_iter = *iter };
...
// smb buffer will be freed by user thread
    if (server.large_buf)
    server.bigbuf = core::ptr::null_mut();
    else
    server.smallbuf = core::ptr::null_mut();
    }
    ...
    {
    let mut srv_sign_required: bool = server.sec_mode & server.vals.signing_required;
    let mut srv_sign_enabled: bool = server.sec_mode & server.vals.signing_enabled;
...
//
    ...
    struct task_struct *task_to_wake = core::ptr::null_mut();
    struct mid_q_entry *mids[MAX_COMPOUND];
    char *bufs[MAX_COMPOUND];
    unsigned int noreclaim_flag, num_io_timeout = 0;
    let mut pending_reconnect: bool = false;
...
// read down to the MID
    ...
    server.lstrp = jiffies;
    for (i = 0; i < num_mids; i++) {
    if (mids[i] != core::ptr::null_mut()) {
    mids[i].resp_buf_size = server.pdu_size;
    if (bufs[i] != core::ptr::null_mut()) {
...
// buffer usually freed in free_mid - need to free it here on exit
    ...
    if (server.smallbuf) /* no sense logging a debug message if core::ptr::null_mut() */
    cifs_small_buf_release(server.smallbuf);
    task_to_wake = xchg(&server.tsk, core::ptr::null_mut());
...
// SMBDirect manages its own ports, don't match it here
    ...
    let mut port: return = = *sport;
    }
#[no_mangle]
unsafe extern "C" fn match_server_address(server: *mut TCP_Server_Info, addr: *mut sockaddr) -> bool {
...
// For secondary channels, we pick up ref-count on the primary server
    ...
    server.session_key.response = core::ptr::null_mut();
    server.session_key.len = 0;
    task = xchg(&server.tsk, core::ptr::null_mut());
    if (task)
    ...
    {
    struct TCP_Server_Info *tcp_ses = core::ptr::null_mut();
    int rc;
    cifs_dbg(FYI, "UNC: %s\n", ctx.UNC);
...
// NULL username means anonymous session
    if (ses.user_name == core::ptr::null_mut()) {
    if (!ctx.nullauth)
    return 0;
    break;
    }
// anything else takes username/password
    ...
    ses.password != core::ptr::null_mut()) {
...
// mount
//
    if (ses.password2 != core::ptr::null_mut() && ctx.password2 != core::ptr::null_mut()) {
    ...
    ctx.password : "", CIFS_MAX_PASSWORD_LEN) == 0)))
    return 0;
    } else if ((ses.password2 == core::ptr::null_mut() && ctx.password2 != core::ptr::null_mut()) ||
    (ses.password2 != core::ptr::null_mut() && ctx.password2 == core::ptr::null_mut())) {
    return 0;
    } else {
    if (strncmp(ses.password, ctx.password ?
...
// no need to setup directory caching on IPC share, so pass in false
    ...
    {
    struct cifs_ses *ses, *ret = core::ptr::null_mut();
    spin_lock(&cifs_tcp_ses_lock);
    list_for_each_entry(ses, &server.smb_ses_list, smb_ses_list) {
    spin_lock(&ses.ses_lock);
    if (ses.ses_status == SES_EXITING) {
...
#[no_mangle]
pub unsafe extern "C" fn __cifs_put_smb_ses(ses: *mut cifs_ses) {
...
// ses_count can never go negative
    ...
    tcon = ses.tcon_ipc;
    ses.tcon_ipc = core::ptr::null_mut();
    spin_unlock(&ses.ses_lock);
    spin_unlock(&cifs_tcp_ses_lock);
//
...
// close any extra channels
    ...
    ses.chans[i].iface = core::ptr::null_mut();
    }
    cifs_put_tcp_session(ses.chans[i].server, 0);
    ses.chans[i].server = core::ptr::null_mut();
    }
// we now account for primary channel in iface->refcount
    if (ses.chans[0].iface) {
    kref_put(&ses.chans[0].iface.refcount, release_iface);
    ses.chans[0].server = core::ptr::null_mut();
    }
    sesInfoFree(ses);
    cifs_put_tcp_session(server, 0);
    }
...
// try to find an address key first
    ...
    if (IS_ERR(key)) {
    if (!ses.domainName) {
    cifs_dbg(FYI, "domainName is core::ptr::null_mut()\n");
    rc = PTR_ERR(key);
    goto out_err;
    }
...
// find first : in payload
    ...
    rc = -EINVAL;
    kfree(ctx.username);
    ctx.username = core::ptr::null_mut();
    goto out_key_put;
    }
    ++delim;
...
// no need to free ctx->password2 since not allocated in this path
    ctx.password = core::ptr::null_mut();
    goto out_key_put;
    }
    }
...
//
    ...
    spin_lock(&cifs_tcp_ses_lock);
    spin_lock(&ses.ses_lock);
    ses.tcon_ipc = !IS_ERR(ipc) ? ipc : core::ptr::null_mut();
    spin_unlock(&ses.ses_lock);
    spin_unlock(&cifs_tcp_ses_lock);
    free_xid(xid);
    return ses;
...
#[no_mangle]
unsafe extern "C" fn match_tcon(tcon: *mut cifs_tcon, ctx: *mut smb3_fs_context) -> c_int {
...
//
    if (tcon == core::ptr::null_mut() || tcon.ipc)
    return;
    ses = tcon.ses;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ctx->cache_rw) -> else {
...
// schedule query interfaces poll
    ...
    let mut oldflags: c_uint = cifs_sb_flags(old) & CIFS_MOUNT_MASK;
    let mut newflags: c_uint = cifs_sb_flags(new) & CIFS_MOUNT_MASK;
...
//
    ...
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(!new_set: !old_set &&) -> else {
...
#[no_mangle]
unsafe extern "C" fn rfc1002mangle(target: *mut c_char, source: *mut c_char, length: c_uint) {
...
// Bind to the specified local IP address
    ...
    let mut rc: c_int = 0;
    let mut retries: c_int = 0;
    let mut msg_flags: c_int = server.noblocksnd ? MSG_DONTWAIT : 0;
// recv = 0;
...
//
    let mut req: rfc1002_session_packet = {};
    let mut resp: rfc1002_session_packet = {};
    let mut msg: msghdr = {};
    let mut iov: kvec = {};
    unsigned int len;
    size_t sent;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_setup_cifs_sb(cifs_sb: *mut cifs_sb_info) -> c_int {
...
// this is needed for ASCII cp to Unicode converts
    if (ctx.iocharset == core::ptr::null_mut()) {
// load_nls_default cannot return null
    cifs_sb.local_nls = load_nls_default();
    } else {
    cifs_sb.local_nls = load_nls(ctx.iocharset);
    if (cifs_sb.local_nls == core::ptr::null_mut()) {
    ...
    if (cifs_sb.prepath == core::ptr::null_mut())
    rc = -ENOMEM;
    else
...
#[no_mangle]
pub unsafe extern "C" fn cifs_mount_put_conns(mnt_ctx: *mut cifs_mount_ctx) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mnt_ctx->ses) -> else {
    else if (mnt_ctx.ses)
    cifs_put_smb_ses(mnt_ctx.ses);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: mnt_ctx->server) -> else {
    else if (mnt_ctx.server)
    cifs_put_tcp_session(mnt_ctx.server, 0);
    mnt_ctx.ses = core::ptr::null_mut();
    mnt_ctx.tcon = core::ptr::null_mut();
    mnt_ctx.server = core::ptr::null_mut();
    atomic_andnot(CIFS_MOUNT_POSIX_PATHS, &cifs_sb.mnt_cifs_flags);
    free_xid(mnt_ctx.xid);
    }
#[no_mangle]
pub unsafe extern "C" fn cifs_mount_get_session(mnt_ctx: *mut cifs_mount_ctx) -> c_int {
    int cifs_mount_get_session(struct cifs_mount_ctx *mnt_ctx)
    {
    struct TCP_Server_Info *server = core::ptr::null_mut();
    struct smb3_fs_context *ctx;
    struct cifs_ses *ses = core::ptr::null_mut();
    unsigned int xid;
    let mut rc: c_int = 0;
    xid = get_xid();
...
// get a reference to a tcp session
    server = cifs_get_tcp_session(ctx, core::ptr::null_mut());
    if (IS_ERR(server)) {
    rc = PTR_ERR(server);
...
#[no_mangle]
pub unsafe extern "C" fn cifs_mount_get_tcon(mnt_ctx: *mut cifs_mount_ctx) -> c_int {
    int cifs_mount_get_tcon(struct cifs_mount_ctx *mnt_ctx)
    {
    struct TCP_Server_Info *server;
    struct cifs_tcon *tcon = core::ptr::null_mut();
    struct cifs_sb_info *cifs_sb;
    struct smb3_fs_context *ctx;
    unsigned int sbflags;
    let mut rc: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(CIFS_MOUNT_RW_CACHE): !(sbflags &) -> else {
...
// hang the tcon off of the superblock
    ...
    char *s;
    char sep, tmp;
    let mut skip: c_int = added_treename ? 1 : 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn cifs_is_path_remote(mnt_ctx: *mut cifs_mount_ctx) -> c_int {
    int cifs_is_path_remote(struct cifs_mount_ctx *mnt_ctx)
    {
    int rc;
    struct cifs_sb_info *cifs_sb = mnt_ctx.cifs_sb;
    struct TCP_Server_Info *server = mnt_ctx.server;
    let mut xid: c_uint = mnt_ctx.xid;
...
//
    ...
    if (full_path == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_mount(cifs_sb: *mut cifs_sb_info, ctx: *mut smb3_fs_context) -> c_int {
    int cifs_mount(struct cifs_sb_info *cifs_sb, struct smb3_fs_context *ctx)
    {
    let mut mnt_ctx: cifs_mount_ctx = { .cifs_sb = cifs_sb, .fs_ctx = ctx, };
    struct mchan_mount *mchan_mount = core::ptr::null_mut();
    int rc;
...
//
    ...
    rc = -EHOSTDOWN;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: WARN_ON(!mnt_ctx.ses)) -> else {
    else if (WARN_ON(!mnt_ctx.ses))
    rc = -EACCES;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: WARN_ON(!mnt_ctx.tcon)) -> else {
...
#[no_mangle]
unsafe extern "C" fn delayed_free(p: *mut rcu_head) {
...
//
    ...
    ses.auth_key.response = core::ptr::null_mut();
    ses.auth_key.len = 0;
    }
    }
...
//
    ...
    struct cifs_tcon *tcon = core::ptr::null_mut();
    struct smb3_fs_context *ctx;
    char *origin_fullpath = core::ptr::null_mut();
    ctx = kzalloc_obj(*ctx);
...
// get a reference for the same TCP session
    ...
    origin_fullpath = core::ptr::null_mut();
    cifs_put_smb_ses(ses);
    goto out;
    }
    } else {
    ...
    if (cap_unix(ses))
    reset_cifs_unix_caps(0, tcon, core::ptr::null_mut(), ctx);
...
// find and return a tlink with given uid
    ...
    node = node.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: uid_lt(tlink->tl_uid, _arg: uid)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_tree_connect(xid: c_uint, tcon: *mut cifs_tcon) -> c_int {
