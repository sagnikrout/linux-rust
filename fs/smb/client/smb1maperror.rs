...
//! Source: fs/smb/client/smb1maperror.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn smb1_posix_error_cmp(_key: *const c_void, _pivot: *const c_void) -> __always_inline int {
    static __always_inline int smb1_posix_error_cmp(const void *_key, const void *_pivot)
    {
    let mut key: __u16 = *(__u16 *)_key;
...
//
#[no_mangle]
unsafe extern "C" fn ntstatus_to_dos_cmp(_key: *const c_void, _pivot: *const c_void) -> __always_inline int {
...
//
    ...
    __u8 smberrclass;
    __u16 smberrcode;
    const struct smb_to_posix_error *err_map = core::ptr::null_mut();
...
// and then to POSIX errors
    let mut err: __u32 = le32_to_cpu(smb.Status.CifsError);
...
// special cases for NT status codes which cannot be translated to DOS codes
    ...
    rc = -ENODATA;
#[no_mangle]
pub unsafe extern "C" fn if((NT_STATUS_PRIVILEGE_NOT_HELD): err ==) -> else {
...
// possible ERRBaduid
    let mut class: __u8 = smb.Status.DosError.ErrorClass;
    let mut code: __u16 = le16_to_cpu(smb.Status.DosError.Error);
...
#[no_mangle]
pub unsafe extern "C" fn smb1_init_maperror() -> int __init {
    ...
    let mut ntstatus_to_dos_num: c_uint = ARRAY_SIZE(ntstatus_to_dos_map);
    ...
    let mut mapping_table_ERRDOS_num: c_uint = ARRAY_SIZE(mapping_table_ERRDOS);
    ...
    let mut mapping_table_ERRSRV_num: c_uint = ARRAY_SIZE(mapping_table_ERRSRV);
