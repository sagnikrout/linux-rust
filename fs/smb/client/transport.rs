...
//! Source: fs/smb/client/transport.c
#![no_std]
...
#[no_mangle]
pub unsafe extern "C" fn __release_mid(server: *mut TCP_Server_Info, midEntry: *mut mid_q_entry) {
    void __release_mid(struct TCP_Server_Info *server, struct mid_q_entry *midEntry)
    {

    let mut command: __le16 = server.vals.lock_cmd;
    let mut smb_cmd: __u16 = le16_to_cpu(midEntry.command);
    unsigned long now;
    ...
    server.slowest_cmd[smb_cmd] = roundtrip_time;
#[no_mangle]
pub unsafe extern "C" fn if(roundtrip_time: server->fastest_cmd[smb_cmd] >) -> else {
...
// total up iov array first
    ...
    struct kvec *iov;
    int n_vec;
    let mut send_length: c_uint = 0;
    unsigned int i, j;
    sigset_t mask, oldmask;
    let mut total_len: usize = 0, sent, size;
    struct socket *ssocket = server.ssocket;
...
// Generate a rfc1002 marker
    ...
    unmask:
    sigprocmask(SIG_SETMASK, &oldmask, core::ptr::null_mut());
//
...
#[no_mangle]
pub unsafe extern "C" fn wait_for_response(server: *mut TCP_Server_Info, mid: *mut mid_q_entry) -> c_int {
    int wait_for_response(struct TCP_Server_Info *server, struct mid_q_entry *mid)
    {
    let mut sleep_state: c_uint = TASK_KILLABLE;
    int error;
...
//
    ...
    let mut credits: cifs_credits = { .value = 0, .instance = 0 };
    unsigned int instance;
    int optype;
    optype = flags & CIFS_OP_MASK;
...
#[no_mangle]
pub unsafe extern "C" fn cifs_sync_mid_result(mid: *mut mid_q_entry, server: *mut TCP_Server_Info) -> c_int {
...
//
    ...
    let mut min_in_flight: c_uint = UINT_MAX;
...
// mark it so buf will not be freed by delete_mid
    ...
    mid[i].resp_buf = core::ptr::null_mut();
    }
    }
//
// Compounding is never used during session establish.
...
//
    ...
    {
    let mut rfclen: c_uint = server.pdu_size;
    let mut remaining: usize = rfclen - server.total_read;
    while (remaining > 0) {
    ssize_t length;
