...
//
    ...
    {
    struct cached_fid *parent_cfid = core::ptr::null_mut();
    if (!tcon || !parent)
    return;
...
//
#[no_mangle]
unsafe extern "C" fn cifs_set_netfs_context(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn cifs_set_ops(inode: *mut inode) {
...
//
    ...
    fattr.cf_symlink_target = core::ptr::null_mut();
    }
...
// safest to call it a file if we do not know
    ...
    let mut id: u64 = le64_to_cpu(info.Uid);
    if (id < ((uid_t)-1)) {
    let mut uid: kuid_t = make_kuid(&init_user_ns, id);
    if (uid_valid(uid))
    fattr.cf_uid = uid;
    }
    }
    ...
    let mut id: u64 = le64_to_cpu(info.Gid);
    if (id < ((gid_t)-1)) {
    let mut gid: kgid_t = make_kgid(&init_user_ns, id);
    if (gid_valid(gid))
    fattr.cf_gid = gid;
    }
    }
...
// check for Minshall+French symlinks
    ...
    char *pbuf;
    let mut buf_type: c_int = CIFS_NO_BUFFER;
    pbuf = buf;
    fattr.cf_mode &= ~S_IFMT;
    if (fattr.cf_eof == 0) {
    cifs_dbg(FYI, "Fifo\n");
    ...
    oplock = 0;
    rc = tcon.ses.server.ops.open(xid, &oparms, &oplock, core::ptr::null_mut());
    if (rc) {
...
//
    ...
    return PTR_ERR(tlink);
    tcon = tlink_tcon(tlink);
    if (tcon.ses.server.ops.query_all_EAs == core::ptr::null_mut()) {
    cifs_put_tlink(tlink);
    return -EOPNOTSUPP;
    }
    ...
    return (int)rc;
#[no_mangle]
pub unsafe extern "C" fn if(3: rc >) -> else {
...
pub const POSIX_X_OTH: c_int = 0000001;
pub const POSIX_W_OTH: c_int = 0000002;
pub const POSIX_R_OTH: c_int = 0000004;
pub const POSIX_X_GRP: c_int = 0000010;
pub const POSIX_W_GRP: c_int = 0000020;
pub const POSIX_R_GRP: c_int = 0000040;
pub const POSIX_X_USR: c_int = 0000100;
pub const POSIX_W_USR: c_int = 0000200;
pub const POSIX_R_USR: c_int = 0000400;
pub const POSIX_STICKY: c_int = 0001000;
pub const POSIX_SET_GID: c_int = 0002000;
pub const POSIX_SET_UID: c_int = 0004000;
pub const POSIX_OTH_MASK: c_int = 0000007;
pub const POSIX_GRP_MASK: c_int = 0000070;
pub const POSIX_USR_MASK: c_int = 0000700;
pub const POSIX_PERM_MASK: c_int = 0000777;
pub const POSIX_FILETYPE_MASK: c_int = 0070000;
pub const POSIX_FILETYPE_SHIFT: c_int = 12;
#[no_mangle]
unsafe extern "C" fn wire_perms_to_posix(wire: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn wire_filetype_to_posix(wire_type: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn wire_mode_to_posix(wire: u32, is_dir: bool) -> umode_t {
...
//
    ...
    data.symlink_target = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn simple_hashstr(str: *const c_char) -> __u64 {
...
//
    ...
    {
    struct cifs_sb_info *cifs_sb = CIFS_SB(sb);
    let mut info: cifs_search_info = {0};
    u16 flags;
    int rc;
// resp_buf = NULL;
...
#[no_mangle]
pub unsafe extern "C" fn if(CIFS_MOUNT_SERVER_INUM: cifs_sb_flags(cifs_sb) &) -> else {
    ...
    cifs_sb, core::ptr::null_mut(), flags, &info, false);
    if (rc)
    return rc;
...
//
    rc = server.ops.get_srv_inum(xid, *inode ? core::ptr::null_mut() : tcon, cifs_sb, full_path,
...
#[no_mangle]
pub unsafe extern "C" fn is_inode_cache_good(ino: *mut inode) -> bool {
    ...
    struct TCP_Server_Info *server = tcon.ses.server;
    struct cifs_sb_info *cifs_sb = CIFS_SB(sb);
    struct kvec rsp_iov, *iov = core::ptr::null_mut();
    let mut rsp_buftype: c_int = CIFS_NO_BUFFER;
    let mut tag: u32 = data.reparse.tag;
    let mut rc: c_int = 0;
...
//
    ...
    let mut directory: bool = cifs_open_data_attrs(data) & ATTR_DIRECTORY;
    ...
    {
    struct cifs_sb_info *cifs_sb = CIFS_SB(sb);
    let mut tmp_data: cifs_open_info_data = {};
    void *smb1_backup_rsp_buf = core::ptr::null_mut();
    struct TCP_Server_Info *server;
    struct cifs_tcon *tcon;
    struct tcon_link *tlink;
    unsigned int sbflags;
    let mut tmprc: c_int = 0;
    let mut rc: c_int = 0;
    tlink = cifs_sb_tlink(cifs_sb);
    if (IS_ERR(tlink))
    return PTR_ERR(tlink);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !(tcon->posix_extensions)) -> else {
...
// some servers mistakenly claim POSIX support
    ...
    rc = smb311_posix_get_fattr(core::ptr::null_mut(), &fattr, path, sb, xid);
    else
    rc = cifs_get_fattr(core::ptr::null_mut(), sb, xid, core::ptr::null_mut(), &fattr, &inode, path);
    iget_root:
    if (!rc) {
    if (fattr.cf_flags & CIFS_FATTR_JUNCTION) {
...
//
    ...
    FILE_BASIC_INFO *info_buf = core::ptr::null_mut();
    tlink = cifs_sb_tlink(cifs_sb);
    if (IS_ERR(tlink))
    return PTR_ERR(tlink);
    tcon = tlink_tcon(tlink);
...
// rename the file
    rc = CIFSSMBRenameOpenFile(xid, tcon, fid.netfid, core::ptr::null_mut(),
    cifs_sb.local_nls,
    cifs_remap(cifs_sb));
    if (rc != 0) {
    rc = -EBUSY;
...
//
#[no_mangle]
unsafe extern "C" fn __cifs_unlink(dir: *mut inode, dentry: *mut dentry, sillyrename: bool) -> c_int {
    ...
    struct tcon_link *tlink;
    struct cifs_tcon *tcon;
    let mut dosattr: __u32 = 0, origattr = 0;
    struct TCP_Server_Info *server;
    struct iattr *attrs = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn cifs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
    ...
    core::ptr::null_mut(), parent.i_sb, xid);
    ...
    rc = cifs_get_inode_info(&inode, full_path, core::ptr::null_mut(), parent.i_sb,
    xid, core::ptr::null_mut());
    }
    if (rc)
    return rc;
...
// must turn on setgid bit if parent dir has it
    ...
    FILE_UNIX_BASIC_INFO *info = core::ptr::null_mut();
    struct inode *newinode = core::ptr::null_mut();
    struct cifs_fattr fattr;
    ...
    core::ptr::null_mut() /* netfid */, info, &oplock, full_path,
    cifs_sb.local_nls, cifs_remap(cifs_sb));
    if (rc == -EOPNOTSUPP)
    goto posix_mkdir_out;
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn cifs_rmdir(inode: *mut inode, direntry: *mut dentry) -> c_int {
...
// open the file to be renamed -- we need DELETE perms
    ...
    FILE_UNIX_BASIC_INFO *info_buf_source = core::ptr::null_mut();
...
//
    ...
    if (d_inode(target_dentry) != core::ptr::null_mut()) {
...
//
    ...
    if (info_buf_source == core::ptr::null_mut()) {
    rc = -ENOMEM;
    goto cifs_rename_exit;
    }
...
#[no_mangle]
pub unsafe extern "C" fn cifs_revalidate_file_attr(filp: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_revalidate_dentry_attr(dentry: *mut dentry) -> c_int {
    ...
    rc = smb311_posix_get_inode_info(&inode, full_path,
    core::ptr::null_mut(), sb, xid);
    } else if (cifs_sb_master_tcon(CIFS_SB(sb)).unix_ext) {
    rc = cifs_get_inode_info_unix(&inode, full_path, sb, xid);
    } else {
    rc = cifs_get_inode_info(&inode, full_path, core::ptr::null_mut(), sb,
    xid, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn cifs_revalidate_file(filp: *mut file) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_revalidate_dentry(dentry: *mut dentry) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn cifs_setsize(inode: *mut inode, offset: loff_t) {
    void cifs_setsize(struct inode *inode, loff_t offset)
    {
    loff_t old_size;
    let mut blocks: u64 = CIFS_INO_BLOCKS(offset);
    spin_lock(&inode.i_lock);
...
#[no_mangle]
pub unsafe extern "C" fn cifs_resize_file_locked(inode: *mut inode, offset: loff_t) {
    ...
    struct cifs_sb_info *cifs_sb = CIFS_SB(inode.i_sb);
    struct cifsInodeInfo *cifsInode = CIFS_I(inode);
    struct tcon_link *tlink = core::ptr::null_mut();
...
//
    ...
    struct cifs_unix_set_info_args *args = core::ptr::null_mut();
...
//
    ...
    void *page = alloc_dentry_path();
    let mut mode: __u64 = NO_CHANGE_64;
    let mut uid: kuid_t = INVALID_UID;
    let mut gid: kgid_t = INVALID_GID;
    const char *full_path;
    let mut dosattr: __u32 = 0;
