...
//! Source: fs/smb/client/cifssmb.c
#![no_std]
...
pub const CIFS_NUM_PROT: c_int = 2;

pub const CIFS_NUM_PROT: c_int = 1;
...
// tell server Unix caps we support
    if (cap_unix(ses))
    reset_cifs_unix_caps(0, tcon, core::ptr::null_mut(), core::ptr::null_mut());
//
...
// request_buf = cifs_small_buf_get();
    if (*request_buf == core::ptr::null_mut()) {
// BB should we add a retry in here if not a writepage?
    ...
    rc = small_smb_init(smb_command, wct, core::ptr::null_mut(), request_buf);
    if (rc < 0)
    return rc;
...
#[no_mangle]
unsafe extern "C" fn validate_t2(pSMB: *mut smb_t2_rsp) -> c_int {
...
//
    ...
    u16 count;
    if (!server) {
    WARN(1, "%s: server is core::ptr::null_mut()!\n", __func__);
    return smb_EIO(smb_eio_trace_null_pointers);
    }
    rc = smb_init(SMB_COM_NEGOTIATE, 0, core::ptr::null_mut() /* no tcon yet */ ,
...
//
    ...
    let mut len: usize = strlen(protocols[i].name) + 1;
...
//
    ...
    reset_cifs_unix_caps(xid, tcon, core::ptr::null_mut(), core::ptr::null_mut());
    }
    }
    out:
...
// BB: do we need to check this? These should never be NULL.
    if ((tcon.ses == core::ptr::null_mut()) || (tcon.ses.server == core::ptr::null_mut()))
    return smb_EIO(smb_eio_trace_null_pointers);
//
...
//
    ...
    {
    let mut credits: cifs_credits = { .value = 1, .instance = 0 };
    ...
    rc = small_smb_init(SMB_COM_ECHO, 0, core::ptr::null_mut(), (void **)&smb);
    if (rc < 0)
    return rc;
    in_len = rc;
...
// set up echo request
    ...
    iov[0].iov_base = smb;
    rc = cifs_call_async(server, &rqst, core::ptr::null_mut(), cifs_echo_callback, core::ptr::null_mut(),
    server, CIFS_NON_BLOCKING | CIFS_ECHO_OP, core::ptr::null_mut());
    if (rc)
...
//
    ...
    spin_unlock(&ses.chan_lock);
    rc = small_smb_init(SMB_COM_LOGOFF_ANDX, 2, core::ptr::null_mut(), (void **)&pSMB);
    if (rc < 0) {
    mutex_unlock(&ses.session_mutex);
    return rc;
    }
...
// if session dead then we do not need to do ulogoff,
    ...
    {
    TRANSACTION2_SPI_REQ *pSMB = core::ptr::null_mut();
    TRANSACTION2_SPI_RSP *pSMBr = core::ptr::null_mut();
    struct unlink_psx_rq *pRqD;
    unsigned int in_len;
    int name_len;
    let mut rc: c_int = 0;
    let mut bytes_returned: c_int = 0;
...
// Setup pointer to Request Data (inode type).
    ...
    {
    DELETE_FILE_REQ *pSMB = core::ptr::null_mut();
    DELETE_FILE_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    let mut rc: c_int = 0;
    int bytes_returned;
    int name_len;
    let mut remap: c_int = cifs_remap(cifs_sb);
    DelFileRetry:
    ...
    {
    DELETE_DIRECTORY_REQ *pSMB = core::ptr::null_mut();
    DELETE_DIRECTORY_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    let mut rc: c_int = 0;
    int bytes_returned;
    int name_len;
    ...
    {
    let mut rc: c_int = 0;
    CREATE_DIRECTORY_REQ *pSMB = core::ptr::null_mut();
    CREATE_DIRECTORY_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    int bytes_returned;
    int name_len;
    let mut remap: c_int = cifs_remap(cifs_sb);
...
#[no_mangle]
unsafe extern "C" fn convert_disposition(disposition: c_int) -> __u16 {
    static __u16 convert_disposition(int disposition)
    {
    let mut ofun: __u16 = 0;
    switch (disposition) {
    case FILE_SUPERSEDE:
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: with_write_flags) -> else {
    else if (with_write_flags)
    return SMBOPEN_WRITE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: with_execute_flags) -> else {
    ...
    {
    int rc;
    OPENX_REQ *pSMB = core::ptr::null_mut();
    OPENX_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    int bytes_returned;
    int name_len;
    __u16 count;
...
#[no_mangle]
pub unsafe extern "C" fn if(REQ_BATCHOPLOCK: *mut *mut pOplock &) -> else {
...
// the file_info buf is endian converted by caller
    ...
    {
    int rc;
    OPEN_REQ *req = core::ptr::null_mut();
    OPEN_RSP *rsp = core::ptr::null_mut();
    int bytes_returned;
    int name_len;
    __u16 count;
    ...
    let mut create_options: c_int = oparms.create_options;
    let mut desired_access: c_int = oparms.desired_access;
    let mut disposition: c_int = oparms.disposition;
...
#[no_mangle]
pub unsafe extern "C" fn if(REQ_BATCHOPLOCK: *mut *mut oplock &) -> else {
...
// the file_info buf is endian converted by caller
    ...
    let mut rreq_debug_id: c_uint = rdata.rreq.debug_id;
    let mut subreq_debug_index: c_uint = rdata.subreq.debug_index;
...
// FIXME: should this be counted toward the initiating task?
    ...
    let mut trans: usize = rdata.subreq.transferred + rdata.got_bytes;
...
// cifs_async_readv - send an async write, and set up mid to handle result
    ...
    {
    int rc;
    READ_REQ *smb = core::ptr::null_mut();
    int wct;
    struct cifs_tcon *tcon = tlink_tcon(rdata.req.cfile.tlink);
    struct smb_rqst rqst = { .rq_iov = rdata.iov,
    .rq_nvec = 1 };
...
// 4 for RFC1001 length + 1 for BCC
    ...
    cifs_readv_callback, core::ptr::null_mut(), rdata, 0, core::ptr::null_mut());
    if (rc == 0)
    ...
    READ_REQ *pSMB = core::ptr::null_mut();
    READ_RSP *pSMBr = core::ptr::null_mut();
    char *pReadData = core::ptr::null_mut();
    int wct;
    let mut resp_buf_type: c_int = 0;
    struct kvec iov[1];
    struct kvec rsp_iov;
    let mut pid: __u32 = io_parms.pid;
    let mut netfid: __u16 = io_parms.netfid;
    let mut offset: __u64 = io_parms.offset;
    struct cifs_tcon *tcon = io_parms.tcon;
    unsigned int in_len;
    let mut count: c_uint = io_parms.length;
...
// old style read
    ...
    let mut data_length: c_int = le16_to_cpu(pSMBr.DataLengthHigh);
...
#[no_mangle]
pub unsafe extern "C" fn if(CIFS_LARGE_BUFFER: resp_buf_type ==) -> else {
...
// Note: On -EAGAIN error only caller can retry on handle based calls
    ...
    {
    let mut rc: c_int = -EACCES;
    WRITE_REQ *pSMB = core::ptr::null_mut();
    WRITE_RSP *pSMBr = core::ptr::null_mut();
    ...
    let mut offset: __u64 = io_parms.offset;
    struct cifs_tcon *tcon = io_parms.tcon;
    let mut count: c_uint = io_parms.length, in_len;
// nbytes = 0;
// cifs_dbg(FYI, "write at %lld %d bytes\n", offset, count);
    if (tcon.ses == core::ptr::null_mut())
    return -ECONNABORTED;
    if (tcon.ses.capabilities & CAP_LARGE_FILES)
    wct = 14;
    else {
...
#[no_mangle]
pub unsafe extern "C" fn if(0: count !=) -> else {
// No buffer
    cifs_buf_release(pSMB);
    return -EINVAL;
...
// cifs_async_writev - send an async write, and set up mid to handle result
    ...
    {
    let mut rc: c_int = -EACCES;
    WRITE_REQ *req = core::ptr::null_mut();
    int wct;
    struct cifs_tcon *tcon = tlink_tcon(wdata.req.cfile.tlink);
    struct kvec iov[1];
    let mut rqst: smb_rqst = { };
    unsigned int in_len;
    if (tcon.ses.capabilities & CAP_LARGE_FILES) {
    wct = 14;
    } else {
...
// wct == 12
    ...
    rc = cifs_call_async(tcon.ses.server, &rqst, core::ptr::null_mut(),
    cifs_writev_callback, core::ptr::null_mut(), wdata, 0, core::ptr::null_mut());
...
// Note: On -EAGAIN error only caller can retry on handle based calls
    ...
    {
    let mut rc: c_int = 0;
    LOCK_REQ *pSMB = core::ptr::null_mut();
    struct kvec iov[2];
    struct kvec rsp_iov;
    unsigned int in_len;
    int resp_buf_type;
...
// LOCK_RSP *pSMBr = NULL; */ /* No response data other than rc to parse
    unsigned int in_len;
    int bytes_returned;
    let mut flags: c_int = CIFS_WINDOWS_LOCK | CIFS_INTERRUPTIBLE_WAIT;
    __u16 count;
...
// Note: On -EAGAIN error only caller can retry on handle based calls
    ...
    const bool waitFlag)
    {
    struct smb_com_transaction2_sfi_req *pSMB  = core::ptr::null_mut();
    struct smb_com_transaction2_sfi_rsp *pSMBr = core::ptr::null_mut();
    struct cifs_posix_lock *parm_data;
    unsigned int in_len;
    let mut rc: c_int = 0;
    let mut sr_flags: c_int = CIFS_INTERRUPTIBLE_WAIT;
    let mut bytes_returned: c_int = 0;
    let mut resp_buf_type: c_int = 0;
    __u16 params, param_offset, offset, byte_count, count;
    struct kvec iov[1];
...
// Note: On -EAGAIN error only caller can retry on handle based calls
    ...
    {
    let mut rc: c_int = 0;
    CLOSE_REQ *pSMB = core::ptr::null_mut();
    unsigned int in_len;
...
// Since session is dead, file will be closed on server already
    ...
    {
    let mut rc: c_int = 0;
    FLUSH_REQ *pSMB = core::ptr::null_mut();
    unsigned int in_len;
    ...
    {
    let mut rc: c_int = 0;
    RENAME_REQ *pSMB = core::ptr::null_mut();
    RENAME_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    int bytes_returned;
...
// unicode only call
    if (target_name == core::ptr::null_mut()) {
...
// BB find exact max on data count below from sess
    ...
    {
    let mut rc: c_int = 0;
    NT_RENAME_REQ *pSMB = core::ptr::null_mut();
    RENAME_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    int bytes_returned;
    int name_len, name_len2;
...
// SMB_QUERY_FILE_UNIX_LINK
    TRANSACTION2_QPI_REQ *pSMB = core::ptr::null_mut();
    TRANSACTION2_QPI_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    let mut rc: c_int = 0;
    int bytes_returned;
    int name_len;
...
// BB also check enough total bytes returned
    ...
    bool is_unicode;
    let mut count: u16 = le16_to_cpu(pSMBr.t2.DataCount);
...
// symlinkinfo = cifs_strndup_from_utf16(data_start,
    ...
    TRANSACT_IOCTL_REQ *io_req = core::ptr::null_mut();
    TRANSACT_IOCTL_RSP *io_rsp = core::ptr::null_mut();
    struct cifs_fid fid;
    unsigned int in_len;
    __u32 data_offset, data_count, len;
    __u8 *start, *end;
    int io_rsp_len;
    let mut oplock: c_int = 0;
    int rc;
    ...
    rc = CIFS_open(xid, &oparms, &oplock, core::ptr::null_mut());
    if (rc)
    return rc;
...
//
    ...
    rc = smb_init(SMB_COM_NT_TRANSACT, 23, tcon, (void **)&io_req, core::ptr::null_mut());
    if (rc < 0)
    goto out_close;
    in_len = rc;
...
// FSCTL_SET_REPARSE_POINT response contains empty output data.
    ...
    cifs_buf_release(io_req);
    if (!rc)
    rc = cifs_get_inode_info(&new, full_path, data, sb, xid, core::ptr::null_mut());
    out_close:
    CIFSSMBClose(xid, tcon, fid.netfid);
//
...
//
    if (rc)
    CIFSSMBDelFile(xid, tcon, full_path, cifs_sb, core::ptr::null_mut());
...
//
    ...
    struct posix_acl *kacl = core::ptr::null_mut();
...
//
    ...
    int count;
    let mut i: c_int = 0;
    if ((acl == core::ptr::null_mut()) || (cifs_acl == core::ptr::null_mut()))
    return 0;
    count = acl.a_count;
...
// BB also check enough total bytes returned
    ...
    let mut data_offset: __u16 = le16_to_cpu(pSMBr.t2.DataOffset);
    let mut count: __u16 = le16_to_cpu(pSMBr.t2.DataCount);
...
//
    ...
    int remap)
    {
    struct smb_com_transaction2_spi_req *pSMB = core::ptr::null_mut();
    struct smb_com_transaction2_spi_rsp *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    char *parm_data;
    int name_len;
    let mut rc: c_int = 0;
...
// convert to on the wire format for POSIX ACL
    ...
    {
    let mut rc: c_int = 0;
    struct smb_t2_qfi_req *pSMB = core::ptr::null_mut();
    struct smb_t2_qfi_rsp *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    int bytes_returned;
    __u16 params, byte_count;
...
// pbuflen = 0;
    ...
    let mut rc: c_int = 0;
    let mut bytes_returned: c_int = 0;
    SET_SEC_DESC_REQ *pSMB = core::ptr::null_mut();
    unsigned int in_len;
    void *pSMBr;
    setCifsAclRetry:
...
// Legacy Query Path Information call for lookup to old servers such
    ...
    let mut time: __u32 = le32_to_cpu(pSMBr.last_write_time);
// decode response
...
#[no_mangle]
pub unsafe extern "C" fn if(40: get_bcc(&pSMBr->hdr) <) -> else {
    else if (get_bcc(&pSMBr.hdr) < 40)
    rc = smb_EIO2(smb_eio_trace_qfileinfo_bcc_too_small,
    get_bcc(&pSMBr.hdr), 40);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: pFindData) -> else {
    let mut data_offset: __u16 = le16_to_cpu(pSMBr.t2.DataOffset);
...
// BB find exact max SMB PDU from sess structure BB
    ...
    get_bcc(&pSMBr.hdr), 40);
#[no_mangle]
pub unsafe extern "C" fn if(40: !legacy && get_bcc(&pSMBr->hdr) <) -> else {
    else if (!legacy && get_bcc(&pSMBr.hdr) < 40)
    rc = smb_EIO2(smb_eio_trace_qpathinfo_bcc_too_small,
    get_bcc(&pSMBr.hdr), 40);
#[no_mangle]
pub unsafe extern "C" fn if(24: legacy && get_bcc(&pSMBr->hdr) <) -> else {
...
// xid, tcon, searchName and codepage are input parms, rest are returned
    ...
    {
// level 257 SMB_
    TRANSACTION2_FFIRST_REQ *pSMB = core::ptr::null_mut();
    TRANSACTION2_FFIRST_RSP *pSMBr = core::ptr::null_mut();
...
// decode response
    ...
    psrch_inf.last_entry = core::ptr::null_mut();
    } else {
...
// pnetfid = parms->SearchHandle;
    ...
    {
    TRANSACTION2_FNEXT_REQ *pSMB = core::ptr::null_mut();
    TRANSACTION2_FNEXT_RSP *pSMBr = core::ptr::null_mut();
...
//
    ...
    {
    let mut rc: c_int = 0;
    FINDCLOSE_REQ *pSMB = core::ptr::null_mut();
    unsigned int in_len;
...
// TRANS2_GET_DFS_REFERRAL
    TRANSACTION2_GET_DFS_REFER_REQ *pSMB = core::ptr::null_mut();
    TRANSACTION2_GET_DFS_REFER_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    let mut rc: c_int = 0;
    int bytes_returned;
    int name_len;
...
// target_nodes = NULL;
    ...
    if (ses == core::ptr::null_mut() || ses.tcon_ipc == core::ptr::null_mut())
    return -ENODEV;
    getDFSRetry:
//
...
// level 0x01 SMB_QUERY_FILE_SYSTEM_INFO
    TRANSACTION2_QFSI_REQ *pSMB = core::ptr::null_mut();
    TRANSACTION2_QFSI_RSP *pSMBr = core::ptr::null_mut();
...
// level 0x200  SMB_SET_CIFS_UNIX_INFO
    TRANSACTION2_SETFSI_REQ *pSMB = core::ptr::null_mut();
    TRANSACTION2_SETFSI_RSP *pSMBr = core::ptr::null_mut();
    unsigned int in_len;
    let mut rc: c_int = 0;
    let mut bytes_returned: c_int = 0;
    __u16 params, param_offset, offset, byte_count;
...
// BB find max SMB PDU from sess structure BB
    ...
    {
    let mut uid: u64 = NO_CHANGE_64, gid = NO_CHANGE_64;
    let mut mode: u64 = args.mode;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISDIR(mode)) -> else {
    else if (S_ISDIR(mode))
    data_offset.Type = cpu_to_le32(UNIX_DIR);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISLNK(mode)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISFIFO(mode)) -> else {
    else if (S_ISFIFO(mode))
    data_offset.Type = cpu_to_le32(UNIX_FIFO);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISSOCK(mode)) -> else {
