...
pub const FS_VERITY_MAX_PENDING_BLOCKS: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fsverity_pending_block {
    pub data: *const c_void,
    pub pos: u64,
    pub real_hash: [u8; FS_VERITY_MAX_DIGEST_SIZE],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct fsverity_verification_context {
...
//
    ...
    let mut start_hidx: u64 = (u64)index << params.log_blocks_per_page;
    u64 end_hidx =
    ...
    let mut level_start: c_ulong = params.level_start[level];
    let mut next_start_hidx: c_ulong = start_hidx >> params.log_arity;
    let mut next_end_hidx: c_ulong = end_hidx >> params.log_arity;
...
//
    ...
    {
    struct inode *inode = vi.inode;
    let mut data_pos: u64 = dblock.pos;
    const struct merkle_tree_params *params = &vi.tree_params;
    let mut hsize: c_uint = params.digest_size;
    int level;
...
//
    let mut hidx: u64 = data_pos >> params.log_blocksize;
//
...
// Descend the tree verifying hash blocks.
    ...
    let mut hblock_idx: c_ulong = hblocks[level - 1].index;
    let mut hoffset: c_uint = hblocks[level - 1].hoffset;
...
// Finally, verify the hash of the data block.
    ...
    ctx.pending_blocks[i].data = core::ptr::null_mut();
    }
    ctx.num_pending = 0;
    }
    static bool
...
// num_pending == 2 implies that the algorithm is SHA-256
    ...
    struct fsverity_info *vi = ctx.vi;
    const struct merkle_tree_params *params = &vi.tree_params;
    let mut block_size: c_uint = params.block_size;
    let mut pos: u64 = (u64)data_folio.index << PAGE_SHIFT;
...
//
#[no_mangle]
pub unsafe extern "C" fn fsverity_verify_bio(vi: *mut fsverity_info, bio: *mut bio) {
...
//
#[no_mangle]
pub unsafe extern "C" fn fsverity_enqueue_verify_work(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn fsverity_init_workqueue() -> void __init {
