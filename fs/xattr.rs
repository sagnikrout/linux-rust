...
pub struct sx_key {
    ...
    return *a_prefix ? core::ptr::null_mut() : a;
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn may_write_xattr(idmap: *mut mnt_idmap, inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn xattr_permission_error(mask: c_int) -> c_int {
...
//
    ...
    if (!handler.get)
    return -EOPNOTSUPP;
    error = handler.get(handler, dentry, inode, name, core::ptr::null_mut(), 0);
    if (error < 0)
    return error;
...
//
    ...
    if (!handler.set)
    return -EOPNOTSUPP;
    return handler.set(handler, idmap, dentry, inode, name, core::ptr::null_mut(), 0,
    XATTR_REPLACE);
    }
...
#[no_mangle]
pub unsafe extern "C" fn import_xattr_name(kname: *mut xattr_name, name: *const char __user) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn setxattr_copy(name: *const char __user, ctx: *mut kernel_xattr_ctx) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn file_setxattr(f: *mut file, ctx: *mut kernel_xattr_ctx) -> c_int {
    int file_setxattr(struct file *f, struct kernel_xattr_ctx *ctx)
    {
    let mut error: c_int = mnt_want_write_file(f);
    if (!error) {
    audit_file(f);
    ...
    int error;
    retry:
    error = filename_lookup(dfd, filename, lookup_flags, &path, core::ptr::null_mut());
    if (error)
    return error;
    ...
    const void __user *,value, size_t, size, int, flags)
    {
    return path_setxattrat(fd, core::ptr::null_mut(), AT_EMPTY_PATH, name,
    value, size, flags);
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn file_getxattr(f: *mut file, ctx: *mut kernel_xattr_ctx) -> isize {
    ...
    void __user *, value, size_t, size)
    {
    return path_getxattrat(fd, core::ptr::null_mut(), AT_EMPTY_PATH, name, value, size);
    }
//
...
// The file system tried to returned a list bigger
    ...
    static
#[no_mangle]
pub unsafe extern "C" fn file_listxattr(f: *mut file, list: *mut char __user, size: usize) -> isize {
    ...
    {
    return path_listxattrat(fd, core::ptr::null_mut(), AT_EMPTY_PATH, list, size);
    }
//
...
#[no_mangle]
unsafe extern "C" fn file_removexattr(f: *mut file, kname: *mut xattr_name) -> c_int {
    ...
    {
    return path_removexattrat(fd, core::ptr::null_mut(), AT_EMPTY_PATH, name);
    }
#[no_mangle]
pub unsafe extern "C" fn xattr_list_one(buffer: *mut c_char, remaining_size: *mut isize, name: *const c_char) -> c_int {
...
//
    ...
    let mut remaining_size: isize = buffer_size;
    for_each_xattr_handler(handlers, handler) {
    int err;
...
//
    ...
    const char *name)
    {
    let mut prefix_len: usize = strlen(xattr_prefix(handler));
...
//
#[no_mangle]
pub unsafe extern "C" fn simple_xattr_space(name: *const c_char, size: usize) -> usize {
...
//
#[no_mangle]
pub unsafe extern "C" fn simple_xattr_free(xattr: *mut simple_xattr) {
...
#[no_mangle]
unsafe extern "C" fn simple_xattr_rcu_free(head: *mut rcu_head) {
...
//
#[no_mangle]
pub unsafe extern "C" fn simple_xattr_free_rcu(xattr: *mut simple_xattr) {
...
#[no_mangle]
unsafe extern "C" fn sx_hashfn(name: *const c_char, parent: *const list_head, seed: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn simple_xattr_hashfn(data: *const c_void, len: u32, seed: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn simple_xattr_obj_hashfn(obj: *const c_void, len: u32, seed: u32) -> u32 {
...
//
    ...
    let mut key: sx_key = { .parent = xattrs, .name = name };
    ...
    if (unlikely(!ht)) {
    if (!value)
    return (flags & XATTR_REPLACE) ? ERR_PTR(-ENODATA) : core::ptr::null_mut();
    ht = kzalloc_obj(*ht);
    if (!ht)
    return ERR_PTR(-ENOMEM);
...
//
    oldht = cmpxchg_release(&cache.ht, core::ptr::null_mut(), ht);
    if (oldht) {
// Race lost
...
//
    ...
    {
    let mut key: sx_key = { .parent = xattrs, .name = name };
    struct simple_xattr *old_xattr = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn xattr_is_trusted(name: *const c_char) -> bool {
    static bool xattr_is_trusted(const char *name)
    {
    return !strncmp(name, XATTR_TRUSTED_PREFIX, XATTR_TRUSTED_PREFIX_LEN);
    }
#[no_mangle]
unsafe extern "C" fn xattr_is_maclabel(name: *const c_char) -> bool {
...
//
    ...
    {
    let mut trusted: bool = ns_capable_noaudit(&init_user_ns, CAP_SYS_ADMIN);
    struct simple_xattr *xattr;
    let mut remaining_size: isize = size;
    let mut err: c_int = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn simple_xattr_cache_cleanup(cache: *mut simple_xattr_cache) {
