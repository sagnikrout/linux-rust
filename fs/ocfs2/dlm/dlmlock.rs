...
//
    ...
    let mut dlm_next_cookie: static u64 = 1;
...
#[no_mangle]
pub unsafe extern "C" fn dlm_init_lock_cache() -> c_int {
    int dlm_init_lock_cache(void)
    {
    dlm_lock_cache = kmem_cache_create("o2dlm_lock",
    sizeof(struct dlm_lock),
    0, SLAB_HWCACHE_ALIGN, core::ptr::null_mut());
    if (dlm_lock_cache == core::ptr::null_mut())
    return -ENOMEM;
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn dlm_destroy_lock_cache() {
...
//
    ...
    struct dlm_lock *lock, int flags)
    {
    let mut call_ast: c_int = 0, kick_thread = 0;
    let mut status: enum dlm_status = DLM_NORMAL;
    mlog(0, "type=%d\n", lock.ml.type);
...
//
    ...
    struct dlm_lock *lock, int flags)
    {
    let mut status: enum dlm_status = DLM_DENIED;
    let mut lockres_changed: c_int = 1;
...
#[no_mangle]
pub unsafe extern "C" fn dlm_lock_get(lock: *mut dlm_lock) {
    void dlm_lock_get(struct dlm_lock *lock)
    {
    kref_get(&lock.lock_refs);
    }
#[no_mangle]
pub unsafe extern "C" fn dlm_lock_put(lock: *mut dlm_lock) {
    void dlm_lock_put(struct dlm_lock *lock)
    {
    kref_put(&lock.lock_refs, dlm_lock_release);
    }
#[no_mangle]
unsafe extern "C" fn dlm_lock_release(kref: *mut kref) {
    static void dlm_lock_release(struct kref *kref)
    {
    struct dlm_lock *lock;
...
#[no_mangle]
unsafe extern "C" fn dlm_lock_detach_lockres(lock: *mut dlm_lock) {
    static void dlm_lock_detach_lockres(struct dlm_lock *lock)
    {
    struct dlm_lock_resource *res;
    res = lock.lockres;
    if (res) {
    lock.lockres = core::ptr::null_mut();
    ...
    newlock.ml.list = 0;
    newlock.ml.flags = 0;
    newlock.ast = core::ptr::null_mut();
    newlock.bast = core::ptr::null_mut();
    newlock.astdata = core::ptr::null_mut();
    ...
    {
    struct dlm_lock *lock;
    let mut kernel_allocated: c_int = 0;
...
//
    ...
    create.node_idx,
    be64_to_cpu(create.cookie), core::ptr::null_mut());
    if (!newlock) {
    dlm_error(status);
    goto leave;
    }
...
#[no_mangle]
pub unsafe extern "C" fn dlm_get_next_cookie(node_num: u8, cookie: *mut u64) {
    static inline void dlm_get_next_cookie(u8 node_num, u64 *cookie)
    {
    let mut tmpnode: u64 = node_num;
...
// cookie = (dlm_next_cookie | tmpnode);
    ...
    struct dlm_lock_resource *res = core::ptr::null_mut();
    struct dlm_lock *lock = core::ptr::null_mut();
    let mut convert: c_int = 0, recovery = 0;
...
// if converting, must pass in a valid dlm_lock
    lock = lksb.lockid;
    if (!lock) {
    mlog(ML_ERROR, "core::ptr::null_mut() lock pointer in convert "
    "request\n");
    goto error;
    }
    res = lock.lockres;
    if (!res) {
    mlog(ML_ERROR, "core::ptr::null_mut() lockres pointer in convert "
    "request\n");
    goto error;
    }
    dlm_lockres_get(res);
