...
//! Source: fs/ocfs2/dlm/dlmmaster.c
#![no_std]
...
//
    ...
    struct dlm_lock_resource *res);
#[no_mangle]
pub unsafe extern "C" fn dlm_is_host_down(errno: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn dlm_get_mle_inuse(mle: *mut dlm_master_list_entry) {
...
#[no_mangle]
unsafe extern "C" fn dlm_put_mle_inuse(mle: *mut dlm_master_list_entry) {
...
#[no_mangle]
unsafe extern "C" fn __dlm_put_mle(mle: *mut dlm_master_list_entry) {
    static void __dlm_put_mle(struct dlm_master_list_entry *mle)
    {
    struct dlm_ctxt *dlm;
    dlm = mle.dlm;
...
#[no_mangle]
unsafe extern "C" fn dlm_put_mle(mle: *mut dlm_master_list_entry) {
    static void dlm_put_mle(struct dlm_master_list_entry *mle)
    {
    struct dlm_ctxt *dlm;
    dlm = mle.dlm;
...
#[no_mangle]
pub unsafe extern "C" fn dlm_get_mle(mle: *mut dlm_master_list_entry) {
...
#[no_mangle]
pub unsafe extern "C" fn __dlm_unlink_mle(dlm: *mut dlm_ctxt, mle: *mut dlm_master_list_entry) {
...
#[no_mangle]
pub unsafe extern "C" fn __dlm_insert_mle(dlm: *mut dlm_ctxt, mle: *mut dlm_master_list_entry) {
...
#[no_mangle]
pub unsafe extern "C" fn dlm_hb_event_notify_attached(dlm: *mut dlm_ctxt, idx: c_int, node_up: c_int) {
    ...
    if (node_up)
    dlm_mle_node_up(dlm, mle, core::ptr::null_mut(), idx);
    else
    dlm_mle_node_down(dlm, mle, core::ptr::null_mut(), idx);
    }
    }
    static void dlm_mle_node_down(struct dlm_ctxt *dlm,
    struct dlm_master_list_entry *mle,
...
#[no_mangle]
pub unsafe extern "C" fn dlm_init_mle_cache() -> c_int {
    ...
    core::ptr::null_mut());
    if (dlm_mle_cache == core::ptr::null_mut())
    return -ENOMEM;
    return 0;
    }
#[no_mangle]
pub unsafe extern "C" fn dlm_destroy_mle_cache() {
    void dlm_destroy_mle_cache(void)
    {
    kmem_cache_destroy(dlm_mle_cache);
    }
#[no_mangle]
unsafe extern "C" fn dlm_mle_release(kref: *mut kref) {
    static void dlm_mle_release(struct kref *kref)
    {
    struct dlm_master_list_entry *mle;
    struct dlm_ctxt *dlm;
...
//
#[no_mangle]
pub unsafe extern "C" fn dlm_init_master_caches() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dlm_destroy_master_caches() {
    void dlm_destroy_master_caches(void)
    {
    kmem_cache_destroy(dlm_lockname_cache);
    dlm_lockname_cache = core::ptr::null_mut();
    kmem_cache_destroy(dlm_lockres_cache);
    dlm_lockres_cache = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn dlm_lockres_release(kref: *mut kref) {
...
#[no_mangle]
pub unsafe extern "C" fn dlm_lockres_put(res: *mut dlm_lock_resource) {
...
//
    ...
    int flags)
    {
    struct dlm_lock_resource *tmpres=core::ptr::null_mut(), *res=core::ptr::null_mut();
    struct dlm_master_list_entry *mle = core::ptr::null_mut();
    struct dlm_master_list_entry *alloc_mle = core::ptr::null_mut();
...
// make sure this does not get freed below
    alloc_mle = core::ptr::null_mut();
    dlm_init_mle(mle, DLM_MLE_MASTER, dlm, res, core::ptr::null_mut(), 0);
...
pub const DLM_MASTERY_TIMEOUT_MS: c_int = 5000;
    static int dlm_wait_for_lock_mastery(struct dlm_ctxt *dlm,
    struct dlm_lock_resource *res,
    struct dlm_master_list_entry *mle,
    int *blocked)
...
// sleep if we haven't finished voting yet
    if (sleep) {
    let mut timeo: c_ulong = msecs_to_jiffies(DLM_MASTERY_TIMEOUT_MS);
    atomic_set(&mle.woken, 0);
...
//
    ...
    void **ret_data)
    {
    let mut response: u8 = DLM_MASTER_RESP_MAYBE;
    struct dlm_ctxt *dlm = data;
    struct dlm_lock_resource *res = core::ptr::null_mut();
    struct dlm_master_request *request = (struct dlm_master_request *) msg.buf;
    struct dlm_master_list_entry *mle = core::ptr::null_mut(), *tmpmle = core::ptr::null_mut();
    char *name;
    unsigned int namelen, hash;
    int found, ret;
    int set_maybe;
    let mut dispatch_assert: c_int = 0;
...
// this lockid has never been seen on this node yet
    ...
    dlm_init_mle(mle, DLM_MLE_BLOCK, dlm, core::ptr::null_mut(), name, namelen);
    ...
    response = DLM_MASTER_RESP_NO;
#[no_mangle]
pub unsafe extern "C" fn if(DLM_MLE_MIGRATION: tmpmle->type ==) -> else {
...
//
    ...
    let mut namelen: c_uint = res.lockname.len;
...
//
    ...
    unsigned int namelen, hash;
    u32 flags;
    let mut master_request: c_int = 0, have_lockres_ref = 0;
    let mut ret: c_int = 0;
    if (!dlm_grab(dlm))
    return 0;
    name = assert.name;
...
// not an error, could be master just re-asserting
    ...
    let mut bit: c_int = find_first_bit(mle.maybe_map, O2NM_MAX_NODES);
    if (bit >= O2NM_MAX_NODES) {
...
// on this mle.
    ...
    err = 1;
#[no_mangle]
pub unsafe extern "C" fn if(2: !extra_ref && rr <) -> else {
    ...
    err = 1;
#[no_mangle]
pub unsafe extern "C" fn if(1: !extra_ref && rr <) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn dlm_assert_master_post_handler(status: c_int, data: *mut c_void, ret_data: *mut c_void) {
...
// queue up work for dlm_assert_master_worker
    dlm_init_work_item(dlm, item, dlm_assert_master_worker, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn dlm_assert_master_worker(item: *mut dlm_work_item, data: *mut c_void) {
    static void dlm_assert_master_worker(struct dlm_work_item *item, void *data)
    {
    struct dlm_ctxt *dlm = data;
    let mut ret: c_int = 0;
...
// the node returns a live node or no node.
    ...
    int nodenum;
    let mut ret: c_int = 0;
    let mut master: u8 = DLM_LOCK_RES_OWNER_UNKNOWN;
    spin_lock(&dlm.spinlock);
...
//
#[no_mangle]
pub unsafe extern "C" fn dlm_drop_lockres_ref(dlm: *mut dlm_ctxt, res: *mut dlm_lock_resource) -> c_int {
...
// BAD.  other node says I did not have a ref.
    ...
    dlm_init_work_item(dlm, item, dlm_deref_lockres_worker, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn dlm_deref_lockres_worker(item: *mut dlm_work_item, data: *mut c_void) {
    static void dlm_deref_lockres_worker(struct dlm_work_item *item, void *data)
    {
    struct dlm_ctxt *dlm;
    struct dlm_lock_resource *res;
...
//
    ...
    let mut nonlocal: c_int = 0, node_ref;
...
//
    ...
    {
    struct dlm_master_list_entry *mle = core::ptr::null_mut();
    struct dlm_master_list_entry *oldmle = core::ptr::null_mut();
    struct dlm_migratable_lockres *mres = core::ptr::null_mut();
    let mut ret: c_int = 0;
    const char *name;
    unsigned int namelen;
    let mut mle_added: c_int = 0;
    let mut wake: c_int = 0;
    if (!dlm_grab(dlm))
    return -EINVAL;
...
//
#[no_mangle]
pub unsafe extern "C" fn dlm_empty_lockres(dlm: *mut dlm_ctxt, res: *mut dlm_lock_resource) -> c_int {
    int dlm_empty_lockres(struct dlm_ctxt *dlm, struct dlm_lock_resource *res)
    __must_hold(&dlm.spinlock)
    {
    int ret;
    let mut lock_dropped: c_int = 0;
    let mut target: u8 = O2NM_MAX_NODES;
    assert_spin_locked(&dlm.spinlock);
    spin_lock(&res.spinlock);
...
#[no_mangle]
pub unsafe extern "C" fn dlm_lock_basts_flushed(dlm: *mut dlm_ctxt, lock: *mut dlm_lock) -> c_int {
...
//
    ...
    struct dlm_lock *lock;
    int noderef;
    let mut nodenum: u8 = O2NM_MAX_NODES;
    assert_spin_locked(&dlm.spinlock);
    assert_spin_locked(&res.spinlock);
...
// list.
    ...
    struct dlm_master_list_entry *mle = core::ptr::null_mut(), *oldmle = core::ptr::null_mut();
    const char *name;
    unsigned int namelen, hash;
    let mut ret: c_int = 0;
    if (!dlm_grab(dlm))
...
#[no_mangle]
pub unsafe extern "C" fn dlm_clean_master_list(dlm: *mut dlm_ctxt, dead_node: u8) {
...
#[no_mangle]
pub unsafe extern "C" fn __dlm_lockres_reserve_ast(res: *mut dlm_lock_resource) {
...
#[no_mangle]
pub unsafe extern "C" fn dlm_force_free_mles(dlm: *mut dlm_ctxt) {
