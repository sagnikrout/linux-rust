...
#[no_mangle]
unsafe extern "C" fn dlm_should_cancel_bast(dlm: *mut dlm_ctxt, lock: *mut dlm_lock) -> c_int {
    static int dlm_should_cancel_bast(struct dlm_ctxt *dlm, struct dlm_lock *lock)
    {
...
// EX blocks anything left, any bast still valid
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(LKM_NLMODE: lock->ml.type ==) -> else {
    else if (lock.ml.type == LKM_NLMODE)
// NL blocks nothing, no reason to send any bast, cancel it
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(LKM_EXMODE: lock->ml.highest_blocked !=) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn __dlm_queue_ast(dlm: *mut dlm_ctxt, lock: *mut dlm_lock) {
...
#[no_mangle]
pub unsafe extern "C" fn dlm_queue_ast(dlm: *mut dlm_ctxt, lock: *mut dlm_lock) {
...
#[no_mangle]
pub unsafe extern "C" fn __dlm_queue_bast(dlm: *mut dlm_ctxt, lock: *mut dlm_lock) {
...
// go do the ast over there
    ...
    char *name;
    struct list_head *head = core::ptr::null_mut();
    __be64 cookie;
    u32 flags;
    u8 node;
    if (!dlm_grab(dlm)) {
