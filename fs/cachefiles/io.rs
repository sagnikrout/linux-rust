...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cachefiles_kiocb {
    pub iocb: kiocb,
    pub ki_refcnt: refcount_t,
    pub start: loff_t,
    union {
    pub skipped: usize,
    pub len: usize,
...
#[no_mangle]
pub unsafe extern "C" fn cachefiles_put_kiocb(ki: *mut cachefiles_kiocb) {
...
//
#[no_mangle]
unsafe extern "C" fn cachefiles_read_complete(iocb: *mut kiocb, ret: c_long) {
...
//
    ...
    let mut ret: isize = -ENOBUFS;
    let mut len: usize = iov_iter_count(iter), skipped = 0;
...
//
    ...
    let mut off: loff_t = start_pos, off2;
...
//
#[no_mangle]
unsafe extern "C" fn cachefiles_write_complete(iocb: *mut kiocb, ret: c_long) {
...
//
    ...
    struct cachefiles_object *object = core::ptr::null_mut();
    struct cachefiles_cache *cache;
    struct fscache_cookie *cookie = fscache_cres_cookie(cres);
    const struct cred *saved_cred;
    struct file *file = cachefiles_cres_file(cres);
    let mut ret: enum netfs_io_source = NETFS_DOWNLOAD_FROM_SERVER;
    let mut len: usize = *_len;
    loff_t off, to;
    let mut ino: ino_t = file ? file_inode(file).i_ino : 0;
...
//
    ...
    let mut start: loff_t = *_start, pos;
    let mut len: usize = *_len;
    int ret;
// Round to DIO size
...
#[no_mangle]
unsafe extern "C" fn cachefiles_prepare_write_subreq(subreq: *mut netfs_io_subrequest) {
...
#[no_mangle]
unsafe extern "C" fn cachefiles_issue_write(subreq: *mut netfs_io_subrequest) {
    ...
    let mut start: loff_t = subreq.start;
    int ret;
...
// We also need to end on the cache granularity boundary
    ...
    let mut part: usize = len % CACHEFILES_DIO_BLOCK_SIZE;
    let mut need: usize = CACHEFILES_DIO_BLOCK_SIZE - part;
...
//
#[no_mangle]
unsafe extern "C" fn cachefiles_end_operation(cres: *mut netfs_cache_resources) {
