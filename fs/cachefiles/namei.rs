...
// search the current directory for the element name
    ...
    if (IS_ERR(subdir)) {
    trace_cachefiles_vfs_error(core::ptr::null_mut(), d_backing_inode(dir),
    PTR_ERR(subdir),
...
// we need to create the subdir if it doesn't exist yet
    ...
    if (ret == 0) {
    subdir = vfs_mkdir(&nop_mnt_idmap, d_inode(dir), subdir, 0700, core::ptr::null_mut());
    if (IS_ERR(subdir))
    ret = PTR_ERR(subdir);
    } else {
    end_creating(subdir);
    ...
    if (IS_ERR(subdir)) {
    trace_cachefiles_vfs_error(core::ptr::null_mut(), d_inode(dir), ret,
...
// Tell rmdir() it's not allowed to delete the subdir
    ...
    if (!__cachefiles_mark_inode_in_use(core::ptr::null_mut(), d_inode(subdir))) {
...
//
#[no_mangle]
pub unsafe extern "C" fn cachefiles_put_directory(dir: *mut dentry) {
    void cachefiles_put_directory(struct dentry *dir)
    {
    if (dir) {
    cachefiles_do_unmark_inode_in_use(core::ptr::null_mut(), d_inode(dir));
    dput(dir);
    }
    }
//
...
//
    ...
    if (ret == 0) {
    ret = vfs_unlink(&nop_mnt_idmap, d_backing_inode(dir), dentry, core::ptr::null_mut());
    if (ret == -EIO)
...
//
    ...
    struct dentry *fan = volume.fanout[(u8)object.cookie.key_hash];
    struct file *file;
    let mut parentpath: path = { .mnt = cache.mnt, .dentry = fan };
    uint64_t ni_size;
    long ret;
...
//
#[no_mangle]
unsafe extern "C" fn cachefiles_create_file(object: *mut cachefiles_object) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn cachefiles_look_up_object(object: *mut cachefiles_object) -> bool {
...
//
    ...
    d_inode(fan), dentry, core::ptr::null_mut());
    if (ret < 0) {
    trace_cachefiles_vfs_error(object, d_inode(fan), ret,
...
//
    dget(victim);
    ret = cachefiles_bury_object(cache, core::ptr::null_mut(), dir, victim,
