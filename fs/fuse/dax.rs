...
//
pub const FUSE_DAX_SHIFT: c_int = 21;
...
// Return fuse_dax_mapping to free list
    ...
    let mut offset: loff_t = start_idx << FUSE_DAX_SHIFT;
    FUSE_ARGS(args);
    ssize_t err;
    WARN_ON(fcd.nr_free_ranges < 0);
...
//
    ...
    int err, num = 0;
    LIST_HEAD(to_remove);
    let mut start_idx: c_ulong = start >> FUSE_DAX_SHIFT;
    let mut end_idx: c_ulong = end >> FUSE_DAX_SHIFT;
...
//
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_inode_cleanup(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn fuse_fill_iomap_hole(iomap: *mut iomap, length: loff_t) {
...
// Mapping beyond end of file is hole
    ...
    struct fuse_dax_mapping *dmap, *alloc_dmap = core::ptr::null_mut();
    int ret;
    let mut writable: bool = flags & IOMAP_WRITE;
    let mut start_idx: c_ulong = pos >> FUSE_DAX_SHIFT;
    struct interval_tree_node *node;
//
...
// Setup one mapping
    ...
    struct fuse_dax_mapping *dmap;
    int ret;
    let mut idx: c_ulong = pos >> FUSE_DAX_SHIFT;
    struct interval_tree_node *node;
//
...
#[no_mangle]
unsafe extern "C" fn fuse_wait_dax_page(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_read_iter(iocb: *mut kiocb, to: *mut iov_iter) -> isize {
...
#[no_mangle]
unsafe extern "C" fn file_extending_write(iocb: *mut kiocb, from: *mut iov_iter) -> bool {
...
#[no_mangle]
unsafe extern "C" fn fuse_dax_direct_write(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
#[no_mangle]
unsafe extern "C" fn fuse_dax_fault(vmf: *mut vm_fault) -> vm_fault_t {
    static vm_fault_t fuse_dax_fault(struct vm_fault *vmf)
    {
    return __fuse_dax_fault(vmf, 0, vmf.flags & FAULT_FLAG_WRITE);
    }
#[no_mangle]
unsafe extern "C" fn fuse_dax_huge_fault(vmf: *mut vm_fault, order: c_uint) -> vm_fault_t {
...
#[no_mangle]
unsafe extern "C" fn fuse_dax_page_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
    static vm_fault_t fuse_dax_page_mkwrite(struct vm_fault *vmf)
    {
    return __fuse_dax_fault(vmf, 0, true);
    }
#[no_mangle]
unsafe extern "C" fn fuse_dax_pfn_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_mmap(file: *mut file, vma: *mut vm_area_struct) -> c_int {
    ...
    {
    int ret;
    let mut start_pos: loff_t = dmap.itn.start << FUSE_DAX_SHIFT;
    let mut end_pos: loff_t = (start_pos + FUSE_DAX_SZ - 1);
...
//
    ...
    int ret;
    struct fuse_inode *fi = get_fuse_inode(inode);
    let mut dmap_start: loff_t = start_idx << FUSE_DAX_SHIFT;
    let mut dmap_end: loff_t = (dmap_start + FUSE_DAX_SZ) - 1;
    filemap_invalidate_lock(inode.i_mapping);
    ret = fuse_dax_break_layouts(inode, dmap_start, dmap_end);
    if (ret) {
    ...
    int ret, nr_freed = 0;
    let mut start_idx: c_ulong = 0, end_idx = 0;
...
#[no_mangle]
unsafe extern "C" fn fuse_dax_free_mem_worker(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn fuse_free_dax_mem_ranges(mem_list: *mut list_head) {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_conn_free(fc: *mut fuse_conn) {
    void fuse_dax_conn_free(struct fuse_conn *fc)
    {
    if (fc.dax) {
    fuse_free_dax_mem_ranges(&fc.dax.free_ranges);
    kfree(fc.dax);
    fc.dax = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn fuse_dax_mem_range_init(fcd: *mut fuse_conn_dax) -> c_int {
    static int fuse_dax_mem_range_init(struct fuse_conn_dax *fcd)
    {
    long nr_pages, nr_ranges;
    struct fuse_dax_mapping *range;
    int ret, id;
    let mut dax_size: usize = -1;
    unsigned long i;
    ...
    DAX_ACCESS, core::ptr::null_mut(), core::ptr::null_mut());
    dax_read_unlock(id);
    if (nr_pages < 0) {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_inode_alloc(sb: *mut super_block, fi: *mut fuse_inode) -> bool {
    bool fuse_dax_inode_alloc(struct super_block *sb, struct fuse_inode *fi)
    {
    struct fuse_conn *fc = get_fuse_conn_super(sb);
    fi.dax = core::ptr::null_mut();
    if (fc.dax) {
...
#[no_mangle]
unsafe extern "C" fn fuse_should_enable_dax(inode: *mut inode, flags: c_uint) -> bool {
    static bool fuse_should_enable_dax(struct inode *inode, unsigned int flags)
    {
    struct fuse_conn *fc = get_fuse_conn(inode);
    let mut dax_mode: enum fuse_dax_mode = fc.dax_mode;
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_inode_init(inode: *mut inode, flags: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_dontcache(inode: *mut inode, flags: c_uint) {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_check_alignment(fc: *mut fuse_conn, map_alignment: c_uint) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn fuse_dax_cancel_work(fc: *mut fuse_conn) {
