...
#[no_mangle]
unsafe extern "C" fn __estimate_accuracy(tv: *mut timespec64) -> c_long {
    static long __estimate_accuracy(struct timespec64 *tv)
    {
    long slack;
    let mut divfactor: c_int = 1000;
    if (tv.tv_sec < 0)
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn select_estimate_accuracy(tv: *mut timespec64) -> u64 {
    u64 select_estimate_accuracy(struct timespec64 *tv)
    {
    u64 ret;
    struct timespec64 now;
    let mut slack: u64 = current.timer_slack_ns;
    if (slack == 0)
    return 0;
    ktime_get_ts64(&now);
...
pub struct poll_table_page {
    pub next: *mut *mut poll_table_page,
    pub entry: *mut *mut poll_table_entry,
    pub entries: [poll_table_entry; ],
}
...
//
    ...
    poll_table *p);
#[no_mangle]
pub unsafe extern "C" fn poll_initwait(pwq: *mut poll_wqueues) {
    ...
    pwq.triggered = 0;
    pwq.error = 0;
    pwq.table = core::ptr::null_mut();
    pwq.inline_index = 0;
    }
    EXPORT_SYMBOL(poll_initwait);
#[no_mangle]
unsafe extern "C" fn free_poll_entry(entry: *mut poll_table_entry) {
...
#[no_mangle]
pub unsafe extern "C" fn poll_freewait(pwq: *mut poll_wqueues) {
    void poll_freewait(struct poll_wqueues *pwq)
    {
    let mut p: *mut poll_table_page = pwq.table;
    int i;
...
#[no_mangle]
unsafe extern "C" fn __pollwake(wait: *mut wait_queue_entry_t, mode: unsigned, sync: c_int, key: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn pollwake(wait: *mut wait_queue_entry_t, mode: unsigned, sync: c_int, key: *mut c_void) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn poll_select_set_timeout(to: *mut timespec64, sec: time64_t, nsec: c_long) -> c_int {
    int poll_select_set_timeout(struct timespec64 *to, time64_t sec, long nsec)
    {
    let mut ts: timespec64 = {.tv_sec = sec, .tv_nsec = nsec};
...
//
    static inline
#[no_mangle]
pub unsafe extern "C" fn get_fd_set(nr: c_ulong, ufdset: *mut void __user, fdset: *mut c_ulong) -> c_int {
    ...
    static inline
#[no_mangle]
pub unsafe extern "C" fn zero_fd_set(nr: c_ulong, fdset: *mut c_ulong) {
...
#[no_mangle]
unsafe extern "C" fn max_select_fd(n: c_ulong, fds: *mut fd_set_bits) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_select(n: c_int, fds: *mut fd_set_bits, end_time: *mut timespec64) -> noinline_for_stack int {
    static noinline_for_stack int do_select(int n, fd_set_bits *fds, struct timespec64 *end_time)
    {
    ktime_t expire, *to = core::ptr::null_mut();
    struct poll_wqueues table;
    poll_table *wait;
    int retval, i, timed_out = 0;
    let mut slack: u64 = 0;
    let mut busy_flag: __poll_t = net_busy_loop_on() ? POLL_BUSY_LOOP : 0;
    let mut busy_start: c_ulong = 0;
    rcu_read_lock();
    ...
    let mut can_busy_loop: bool = false;
    inp = fds.in; outp = fds.out; exp = fds.ex;
    rinp = fds.res_in; routp = fds.res_out; rexp = fds.res_ex;
    for (i = 0; i < n; ++rinp, ++routp, ++rexp) {
    unsigned long in, out, ex, all_bits, bit = 1, j;
    let mut res_in: c_ulong = 0, res_out = 0, res_ex = 0;
    __poll_t mask;
...
// Not enough space in on-stack array; must use kmalloc
    ...
    {
    struct timespec64 end_time, *to = core::ptr::null_mut();
...
//
    ...
    {
    struct timespec64 ts, end_time, *to = core::ptr::null_mut();
    int ret;
    if (tsp) {
    switch (type) {
    case PT_TIMESPEC:
...
// the path is hot enough for overhead of copy_from_user() to matter
    ...
    void __user *, sig)
    {
    let mut x: sigset_argpack = {core::ptr::null_mut(), 0};
...
pub struct sel_arg_struct {
    pub n: c_ulong,
    pub exp: *mut *mut *mut fd_set __user inp, outp,,
    pub tvp: *mut __kernel_old_timeval __user,
}
...
pub struct poll_list {
    ...
    pub len: c_uint,
    pub __counted_by(len): pollfd entries[],
}
...
// can_busy_poll = true;
    ...
    {
    let mut pt: *mut poll_table = &wait.pt;
    ktime_t expire, *to = core::ptr::null_mut();
    let mut timed_out: c_int = 0, count = 0;
    let mut slack: u64 = 0;
    let mut busy_flag: __poll_t = net_busy_loop_on() ? POLL_BUSY_LOOP : 0;
    let mut busy_start: c_ulong = 0;
// Optimise the no-wait case
    if (end_time && !end_time.tv_sec && !end_time.tv_nsec) {
    pt._qproc = core::ptr::null_mut();
    timed_out = 1;
    }
    if (end_time && !timed_out)
    slack = select_estimate_accuracy(end_time);
    ...
    let mut can_busy_loop: bool = false;
    for (walk = list; walk != core::ptr::null_mut(); walk = walk.next) {
...
//
    ...
    {
    struct poll_wqueues table;
    let mut err: c_int = -EFAULT, fdcount;
// Allocate small arguments on the stack to save memory and be
    faster - use long to make sure the buffer is aligned properly
    on 64 bit archs to avoid unaligned access */
    long stack_pps[POLL_STACK_ALLOC/sizeof(long)];
    let mut head: *mut poll_list const = (struct poll_list *)stack_pps;
    struct poll_list *walk = head;
    let mut todo: c_uint = nfds;
    unsigned int len;
    if (nfds > rlimit(RLIMIT_NOFILE))
    return -EINVAL;
    len = min_t(unsigned int, nfds, N_STACK_PPS);
    for (;;) {
    walk.next = core::ptr::null_mut();
    walk.len = len;
    if (!len)
    break;
...
#[no_mangle]
unsafe extern "C" fn do_restart_poll(restart_block: *mut restart_block) -> c_long {
    static long do_restart_poll(struct restart_block *restart_block)
    {
    struct pollfd __user *ufds = restart_block.poll.ufds;
    let mut nfds: c_int = restart_block.poll.nfds;
    struct timespec64 *to = core::ptr::null_mut();
    int ret;
...
pub struct compat_sel_arg_struct {
    pub n: compat_ulong_t,
    pub inp: compat_uptr_t,
    pub outp: compat_uptr_t,
    pub exp: compat_uptr_t,
    pub tvp: compat_uptr_t,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_sigset_argpack {
    pub p: compat_uptr_t,
    pub size: compat_size_t,
}
    ...
    {
    let mut x: compat_sigset_argpack = {0, 0};
