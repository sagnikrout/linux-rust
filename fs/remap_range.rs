...
//
    ...
    let mut bs: loff_t = inode_out.i_sb.s_blocksize;
    int ret;
...
pub unsafe extern "C" fn remap_verify_area(file: *mut file, pos: loff_t, len: loff_t, write: bool) -> c_int {
    ...
    {
    let mut mask: c_int = write ? MAY_WRITE : MAY_READ;
    loff_t tmp;
    int ret;
...
//
    ...
    {
    let mut blkmask: u64 = i_blocksize(inode_in) - 1;
    let mut new_len: loff_t = *len;
    if ((*len & blkmask) == 0)
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn vfs_lock_two_folios(folio1: *mut folio, folio2: *mut folio) {
...
#[no_mangle]
unsafe extern "C" fn vfs_unlock_two_folios(folio1: *mut folio, folio2: *mut folio) {
...
//
    ...
    let mut same_inode: bool = (inode_in == inode_out);
    int ret;
...
// Zero length dedupe exits immediately; reflink goes to EOF.
    if (*len == 0) {
    let mut isize: loff_t = i_size_read(inode_in);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: dax_read_ops) -> else {
...
// If can't alter the file contents, we're done.
    ...
    pos_out, len, remap_flags, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn may_dedupe_file(file: *mut file) -> bool {
...
pub unsafe extern "C" fn vfs_dedupe_file_range(file: *mut file, same: *mut file_dedupe_range) -> c_int {
    ...
    int i;
    int ret;
    let mut count: u16 = same.dest_count;
    loff_t deduped;
...
#[no_mangle]
pub unsafe extern "C" fn if(0: deduped <) -> else {
    else if (deduped < 0)
    info.status = deduped;
    else
