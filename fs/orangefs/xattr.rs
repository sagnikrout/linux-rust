...
pub const SYSTEM_ORANGEFS_KEY_LEN: c_int = 13;
//
...
//
#[no_mangle]
unsafe extern "C" fn is_reserved_key(key: *const c_char, size: usize) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn convert_to_internal_xattr_flags(setxattr_flags: c_int) -> c_int {
    static inline int convert_to_internal_xattr_flags(int setxattr_flags)
    {
    let mut internal_flag: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn xattr_key(key: *const c_char) -> c_uint {
...
pub unsafe extern "C" fn orangefs_listxattr(dentry: *mut dentry, buffer: *mut c_char, size: usize) -> isize {
    ...
    struct orangefs_inode_s *orangefs_inode = ORANGEFS_I(inode);
    struct orangefs_kernel_op_s *new_op;
    let mut token: __u64 = ORANGEFS_ITERATE_START;
    let mut ret: isize = -ENOMEM;
    ...
    if (size > 0 && !buffer) {
    gossip_err("%s: bogus core::ptr::null_mut() pointers\n", __func__);
    return -EINVAL;
    }
    down_read(&orangefs_inode.xattr_sem);
