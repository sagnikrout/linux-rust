...
//! Source: fs/orangefs/devorangefs-req.c
#![no_std]
...
#[no_mangle]
unsafe extern "C" fn hash_func(tag: __u64, table_size: c_int) -> c_int {
    static int hash_func(__u64 tag, int table_size)
    {
    return do_div(tag, (unsigned int)table_size);
    }
#[no_mangle]
unsafe extern "C" fn orangefs_devreq_add_op(op: *mut orangefs_kernel_op_s) {
    static void orangefs_devreq_add_op(struct orangefs_kernel_op_s *op)
    {
    let mut index: c_int = hash_func(op.tag, hash_table_size);
...
#[no_mangle]
unsafe extern "C" fn mark_all_pending_mounts() -> c_int {
    static int mark_all_pending_mounts(void)
    {
    let mut unmounted: c_int = 1;
    struct orangefs_sb_info_s *orangefs_sb = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn fs_mount_pending(fsid: __s32) -> c_int {
    static int fs_mount_pending(__s32 fsid)
    {
    let mut mount_pending: c_int = -1;
    struct orangefs_sb_info_s *orangefs_sb = core::ptr::null_mut();
    spin_lock(&orangefs_superblocks_lock);
    list_for_each_entry(orangefs_sb, &orangefs_superblocks, list) {
...
#[no_mangle]
unsafe extern "C" fn orangefs_devreq_open(inode: *mut inode, file: *mut file) -> c_int {
...
// Function for read() callers into the device
    ...
    let mut proto_ver: __s32 = ORANGEFS_KERNEL_PROTO_VERSION;
    let mut magic: static __s32 = ORANGEFS_DEVREQ_MAGIC;
...
//
    ...
    {
    ssize_t ret;
    struct orangefs_kernel_op_s *op = core::ptr::null_mut();
    struct {
    __u32 version;
    __u32 magic;
    __u64 tag;
    } head;
    let mut total: c_int = ret = iov_iter_count(iter);
    let mut downcall_size: c_int = sizeof(struct orangefs_downcall_s);
    let mut head_size: c_int = sizeof(head);
...
// READDIR operations should always have trailers.
    ...
    op.downcall.trailer_buf = core::ptr::null_mut();
    goto Efault;
    }
    wakeup:
//
...
//
#[no_mangle]
unsafe extern "C" fn orangefs_devreq_release(inode: *mut inode, file: *mut file) -> c_int {
    static int orangefs_devreq_release(struct inode *inode, struct file *file)
    {
    let mut unmounted: c_int = 0;
    gossip_debug(GOSSIP_DEV_DEBUG,
...
#[no_mangle]
pub unsafe extern "C" fn is_daemon_in_service() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __is_daemon_in_service() -> bool {
    bool __is_daemon_in_service(void)
    {
    let mut open_access_count: return = = 1;
    }
#[no_mangle]
pub unsafe extern "C" fn check_ioctl_command(command: c_uint) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn dispatch_ioctl_command(command: c_uint, arg: c_ulong) -> c_long {
    static long dispatch_ioctl_command(unsigned int command, unsigned long arg)
    {
    let mut magic: static __s32 = ORANGEFS_DEVREQ_MAGIC;
    let mut max_up_size: static __s32 = MAX_DEV_REQ_UPSIZE;
    let mut max_down_size: static __s32 = MAX_DEV_REQ_DOWNSIZE;
    struct ORANGEFS_dev_map_desc user_desc;
    let mut ret: c_int = 0;
    let mut upstream_kmod: c_int = 1;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ORANGEFS_dev_map_desc32 {
    pub ptr: compat_uptr_t,
...
// no other ioctl requires translation
    ...
    {
    let mut poll_revent_mask: __poll_t = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn orangefs_dev_init() -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn orangefs_dev_cleanup() {
