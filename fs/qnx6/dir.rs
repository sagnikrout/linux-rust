...
#[no_mangle]
unsafe extern "C" fn qnx6_lfile_checksum(name: *mut c_char, size: unsigned) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn last_entry(inode: *mut inode, page_nr: c_ulong) -> unsigned {
    static unsigned last_entry(struct inode *inode, unsigned long page_nr)
    {
    let mut last_byte: c_ulong = inode.i_size;
...
#[no_mangle]
unsafe extern "C" fn qnx6_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    static int qnx6_readdir(struct file *file, struct dir_context *ctx)
    {
    struct inode *inode = file_inode(file);
    struct super_block *s = inode.i_sb;
    struct qnx6_sb_info *sbi = QNX6_SB(s);
    let mut pos: loff_t = ctx.pos & ~(QNX6_DIR_ENTRY_SIZE - 1);
    let mut npages: c_ulong = dir_pages(inode);
    let mut n: c_ulong = pos >> PAGE_SHIFT;
    let mut offset: unsigned = (pos & ~PAGE_MASK) / QNX6_DIR_ENTRY_SIZE;
    ...
    let mut size: c_int = de.de_size;
    let mut no_inode: u32 = fs32_to_cpu(sbi, de.de_inode);
    if (!no_inode || !size)
    continue;
...
#[no_mangle]
pub unsafe extern "C" fn qnx6_find_ino(len: c_int, dir: *mut inode, name: *const c_char) -> unsigned {
    ...
    if (!IS_ERR(de)) {
    let mut limit: c_int = last_entry(dir, n);
    int i;
