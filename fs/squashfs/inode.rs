...
//
#[no_mangle]
pub unsafe extern "C" fn squashfs_read_inode(inode: *mut inode, ino: c_longlong) -> c_int {
    int squashfs_read_inode(struct inode *inode, long long ino)
    {
    struct super_block *sb = inode.i_sb;
    struct squashfs_sb_info *msblk = sb.s_fs_info;
    let mut block: u64 = SQUASHFS_INODE_BLK(ino) + msblk.inode_table;
    int err, type, offset = SQUASHFS_INODE_OFFSET(ino);
    union squashfs_inode squashfs_ino;
    struct squashfs_base_inode *sqshb_ino = &squashfs_ino.base;
    let mut xattr_id: c_int = SQUASHFS_INVALID_XATTR;
...
//
    ...
    __le32 xattr;
    err = squashfs_read_metadata(sb, core::ptr::null_mut(), &block,
    &offset, inode.i_size);
    if (err < 0)
    goto failed_read;
