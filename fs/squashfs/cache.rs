...
#[no_mangle]
unsafe extern "C" fn squashfs_cache_wake_block(cache: *mut squashfs_cache, block: u64) {
...
//
    ...
    let mut capacity_wake: bool = false;
...
//
#[no_mangle]
pub unsafe extern "C" fn squashfs_cache_put(entry: *mut squashfs_cache_entry) {
...
//
#[no_mangle]
pub unsafe extern "C" fn squashfs_cache_delete(cache: *mut squashfs_cache) {
    void squashfs_cache_delete(struct squashfs_cache *cache)
    {
    int i, j;
    if (IS_ERR(cache) || cache == core::ptr::null_mut())
    return;
...
//
    ...
    if (cache.entry == core::ptr::null_mut()) {
    ERROR("Failed to allocate %s cache\n", name);
    goto cleanup;
    }
    cache.curr_blk = 0;
    ...
    if (entry.data == core::ptr::null_mut()) {
    ERROR("Failed to allocate %s cache entry\n", name);
    goto cleanup;
    }
    for (j = 0; j < cache.pages; j++) {
    entry.data[j] = kmalloc(PAGE_SIZE, GFP_KERNEL);
    if (entry.data[j] == core::ptr::null_mut()) {
    ...
    cache.pages, 0);
    if (entry.actor == core::ptr::null_mut()) {
    ERROR("Failed to allocate %s cache entry\n", name);
    goto cleanup;
    }
    }
...
//
    ...
    {
    let mut remaining: c_int = length;
    if (length == 0 || offset < 0)
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(NULL: buffer ==) -> else {
    else if (buffer == core::ptr::null_mut())
...
//
    ...
    {
    let mut pages: c_int = (length + PAGE_SIZE - 1) >> PAGE_SHIFT;
    int i, res;
    ...
    SQUASHFS_COMPRESSED_BIT_BLOCK, core::ptr::null_mut(), actor);
    kfree(data);
    kfree(actor);
    if (res < 0)
    goto failed;
