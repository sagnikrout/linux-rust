...
// size should never be larger than SQUASHFS_NAME_LEN
    ...
    break;
    err = squashfs_read_metadata(sb, core::ptr::null_mut(), &index_start,
    &index_offset, size);
    if (err < 0)
    break;
    length = index;
...
unsafe extern "C" fn squashfs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    ...
    let mut block: u64 = squashfs_i(inode).start + msblk.directory_table;
    let mut offset: c_int = squashfs_i(inode).offset, length, err;
    unsigned int inode_number, dir_count, size, type;
    struct squashfs_dir_header dirh;
    struct squashfs_dir_entry *dire;
    TRACE("Entered squashfs_readdir [%llx:%x]\n", block, offset);
    dire = kmalloc(sizeof(*dire) + SQUASHFS_NAME_LEN + 1, GFP_KERNEL);
    if (dire == core::ptr::null_mut()) {
