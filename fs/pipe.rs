...
// emptied.
//
pub const PIPE_MIN_DEF_BUFFERS: c_int = 2;
//
// The max size that a non-root user is allowed to grow the pipe. Can
// be set by root in /proc/sys/fs/pipe-max-size
//
    let mut pipe_max_size: static unsigned int = 1048576;
// Maximum allocatable pages per user. Hard limit is unset by default, soft
// matches default values.
//
    static unsigned long pipe_user_pages_hard;
    let mut pipe_user_pages_soft: static unsigned long = PIPE_DEF_BUFFERS * INR_OPEN_CUR;
//
...
#[no_mangle]
pub unsafe extern "C" fn pipe_lock(pipe: *mut pipe_inode_info) {
...
#[no_mangle]
pub unsafe extern "C" fn pipe_unlock(pipe: *mut pipe_inode_info) {
...
//
#[no_mangle]
unsafe extern "C" fn anon_pipe_prefill_and_lock(pipe: *mut pipe_inode_info, total_len: usize) {
...
//
#[no_mangle]
pub unsafe extern "C" fn generic_pipe_buf_get(pipe: *mut pipe_inode_info, buf: *mut pipe_buffer) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn pipe_readable(pipe: *const pipe_inode_info) -> bool {
    static inline bool pipe_readable(const struct pipe_inode_info *pipe)
    {
    let mut idx: union pipe_index = { .head_tail = READ_ONCE(pipe.head_tail) };
    let mut writers: c_uint = READ_ONCE(pipe.writers);
...
//
    ...
    {
    let mut total_len: usize = iov_iter_count(to);
    struct file *filp = iocb.ki_filp;
    struct pipe_inode_info *pipe = filp.private_data;
    let mut wake_writer: bool = false, wake_next_reader = false;
    ssize_t ret;
// Null read succeeds.
...
// Read ->head with a barrier vs post_one_notification()
    let mut head: c_uint = smp_load_acquire(&pipe.head);
    let mut tail: c_uint = pipe.tail;
...
// Consumed buffers may have refilled the pool; trim it and unlock.
    ...
    {
    let mut ret: c_int = anon_pipe_read(iocb, to);
    if (ret > 0)
    file_accessed(iocb.ki_filp);
    return ret;
    }
#[no_mangle]
pub unsafe extern "C" fn is_packetized(file: *mut file) -> c_int {
...
// Done while waiting without holding the pipe lock - thus the READ_ONCE()
#[no_mangle]
pub unsafe extern "C" fn pipe_writable(pipe: *const pipe_inode_info) -> bool {
    static inline bool pipe_writable(const struct pipe_inode_info *pipe)
    {
    let mut idx: union pipe_index = { .head_tail = READ_ONCE(pipe.head_tail) };
    let mut max_usage: c_uint = READ_ONCE(pipe.max_usage);
    ...
    let mut wake_next_writer: bool = false;
//
...
//
    ...
    let mut offset: c_int = buf.offset + buf.len;
...
//
    ...
    {
    let mut ret: c_int = anon_pipe_write(iocb, from);
    if (ret > 0) {
    struct file *filp = iocb.ki_filp;
    if (sb_start_write_trylock(file_inode(filp).i_sb)) {
    let mut err: c_int = file_update_time(filp);
    if (err)
    ret = err;
...
#[no_mangle]
unsafe extern "C" fn pipe_ioctl(filp: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn put_pipe_info(inode: *mut inode, pipe: *mut pipe_inode_info) {
    static void put_pipe_info(struct inode *inode, struct pipe_inode_info *pipe)
    {
    let mut kill: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn too_many_pipe_buffers_soft(user_bufs: c_ulong) -> bool {
    bool too_many_pipe_buffers_soft(unsigned long user_bufs)
    {
    let mut soft_limit: c_ulong = READ_ONCE(pipe_user_pages_soft);
    return soft_limit && user_bufs > soft_limit;
    }
#[no_mangle]
pub unsafe extern "C" fn too_many_pipe_buffers_hard(user_bufs: c_ulong) -> bool {
    bool too_many_pipe_buffers_hard(unsigned long user_bufs)
    {
    let mut hard_limit: c_ulong = READ_ONCE(pipe_user_pages_hard);
    return hard_limit && user_bufs > hard_limit;
    }
#[no_mangle]
pub unsafe extern "C" fn pipe_is_unprivileged_user() -> bool {
    ...
    let mut pipe_bufs: c_ulong = PIPE_DEF_BUFFERS;
    struct user_struct *user = get_current_user();
    unsigned long user_bufs;
    let mut max_size: c_uint = READ_ONCE(pipe_max_size);
    ...
    lock_set_cmp_fn(&pipe.mutex, pipe_lock_cmp_fn, core::ptr::null_mut());
    return pipe;
    }
    out_revert_acct:
...
#[no_mangle]
pub unsafe extern "C" fn free_pipe_info(pipe: *mut pipe_inode_info) {
...
#[no_mangle]
unsafe extern "C" fn get_pipe_inode() -> *mut inode {
...
#[no_mangle]
pub unsafe extern "C" fn create_pipe_files(res: *mut file, flags: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __do_pipe_flags(fd: *mut c_int, files: *mut file, flags: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn do_pipe_flags(fd: *mut c_int, flags: c_int) -> c_int {
    int do_pipe_flags(int *fd, int flags)
    {
    struct file *files[2];
    let mut error: c_int = __do_pipe_flags(fd, files, flags);
    if (!error) {
...
//
#[no_mangle]
unsafe extern "C" fn do_pipe2(fildes: *mut int __user, flags: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn pipe_wait_readable(pipe: *mut pipe_inode_info) {
...
#[no_mangle]
pub unsafe extern "C" fn pipe_wait_writable(pipe: *mut pipe_inode_info) {
...
//
#[no_mangle]
unsafe extern "C" fn wait_for_partner(pipe: *mut pipe_inode_info, cnt: *mut c_uint) -> c_int {
    static int wait_for_partner(struct pipe_inode_info *pipe, unsigned int *cnt)
    {
    DEFINE_WAIT(rdwait);
    let mut cur: c_int = *cnt;
    while (cur == *cnt) {
    ...
    let mut cur: return = = *cnt ? -ERESTARTSYS : 0;
    }
#[no_mangle]
unsafe extern "C" fn wake_up_partner(pipe: *mut pipe_inode_info) {
    static void wake_up_partner(struct pipe_inode_info *pipe)
    {
    wake_up_interruptible_all(&pipe.rd_wait);
    }
#[no_mangle]
unsafe extern "C" fn fifo_open(inode: *mut inode, filp: *mut file) -> c_int {
    static int fifo_open(struct inode *inode, struct file *filp)
    {
    let mut is_pipe: bool = inode.i_fop == &pipeanon_fops;
    struct pipe_inode_info *pipe;
    int ret;
    filp.f_pipe = 0;
    spin_lock(&inode.i_lock);
...
//
#[no_mangle]
pub unsafe extern "C" fn round_pipe_size(size: c_uint) -> c_uint {
...
//
#[no_mangle]
pub unsafe extern "C" fn pipe_resize_ring(pipe: *mut pipe_inode_info, nr_slots: c_uint) -> c_int {
...
//
    if (n > 0) {
    let mut h: c_uint = head & mask;
    let mut t: c_uint = tail & mask;
    if (h > t) {
    memcpy(bufs, pipe.bufs + t,
    n * sizeof(struct pipe_buffer));
    } else {
    let mut tsize: c_uint = pipe.ring_size - t;
    if (h > 0)
...
//
#[no_mangle]
unsafe extern "C" fn pipe_set_size(pipe: *mut pipe_inode_info, arg: c_uint) -> c_long {
...
#[no_mangle]
pub unsafe extern "C" fn pipe_fcntl(file: *mut file, cmd: c_uint, arg: c_uint) -> c_long {
...
//
#[no_mangle]
unsafe extern "C" fn pipefs_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn round_pipe_size_ul(size: c_ulong) -> c_ulong {
    static ulong round_pipe_size_ul(ulong size)
    {
    return round_pipe_size(size);
    }
#[no_mangle]
unsafe extern "C" fn u2k_pipe_maxsz(u_ptr: *const c_ulong, k_ptr: *mut c_uint) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn init_pipe_fs() -> int __init {
    static int __init init_pipe_fs(void)
    {
    let mut err: c_int = register_filesystem(&pipe_fs_type);
    if (!err) {
