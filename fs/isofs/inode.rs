...
#[no_mangle]
unsafe extern "C" fn isofs_put_super(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn isofs_free_inode(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn isofs_reconfigure(fc: *mut fs_context) -> c_int {
...
pub struct isofs_options {
    ...
    pub cruft:1: c_uint,
    pub hide:1: c_uint,
    pub showassoc:1: c_uint,
    pub nocompress:1: c_uint,
    pub overriderockperm:1: c_uint,
    pub uid_set:1: c_uint,
    pub gid_set:1: c_uint,
    pub map: c_uchar,
    pub check: c_uchar,
...
//
#[no_mangle]
unsafe extern "C" fn isofs_show_options(m: *mut seq_file, root: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn isofs_get_last_session(sb: *mut super_block, session: i32) -> c_uint {
    static unsigned int isofs_get_last_session(struct super_block *sb, s32 session)
    {
    struct cdrom_device_info *cdi = disk_to_cdi(sb.s_bdev.bd_disk);
    let mut vol_desc_start: c_uint = 0;
    if (session > 0) {
...
//
#[no_mangle]
unsafe extern "C" fn rootdir_empty(sb: *mut super_block, block: c_ulong) -> bool {
    static bool rootdir_empty(struct super_block *sb, unsigned long block)
    {
    let mut offset: c_int = 0, files = 0, de_len;
...
//
#[no_mangle]
unsafe extern "C" fn isofs_fill_super(s: *mut super_block, fc: *mut fs_context) -> c_int {
    static int isofs_fill_super(struct super_block *s, struct fs_context *fc)
    {
    struct buffer_head *bh = core::ptr::null_mut(), *pri_bh = core::ptr::null_mut();
    struct hs_primary_descriptor *h_pri = core::ptr::null_mut();
    struct iso_primary_descriptor *pri = core::ptr::null_mut();
    struct iso_supplementary_descriptor *sec = core::ptr::null_mut();
    struct iso_directory_record *rootp;
    struct inode *inode;
    struct isofs_options *opt = fc.fs_private;
    struct isofs_sb_info *sbi;
    unsigned long first_data_zone;
    let mut joliet_level: c_int = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(ISO_VD_SUPPLEMENTARY: isonum_711(vdp->type) ==) -> else {
    ...
    joliet_level = 1;
#[no_mangle]
pub unsafe extern "C" fn if(0x43: sec->escape[2] ==) -> else {
    else if (sec.escape[2] == 0x43)
    joliet_level = 2;
#[no_mangle]
pub unsafe extern "C" fn if(0x45: sec->escape[2] ==) -> else {
...
//
    ...
    goto out_freesbi;
    sbi.s_nls_iocharset = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn isofs_statfs(dentry: *mut dentry, buf: *mut kstatfs) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn isofs_bmap(inode: *mut inode, block: sector_t) -> c_int {
    ...
    {
    let mut blknr: sector_t = isofs_bmap(inode, block);
    if (!blknr)
    return core::ptr::null_mut();
    return sb_bread(inode.i_sb, blknr);
    }
#[no_mangle]
unsafe extern "C" fn isofs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    static int isofs_read_folio(struct file *file, struct folio *folio)
    {
    return mpage_read_folio(folio, isofs_get_block);
    }
#[no_mangle]
unsafe extern "C" fn isofs_readahead(rac: *mut readahead_control) {
    static void isofs_readahead(struct readahead_control *rac)
    {
    mpage_readahead(rac, isofs_get_block);
    }
#[no_mangle]
unsafe extern "C" fn _isofs_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
...
#[no_mangle]
unsafe extern "C" fn isofs_read_level3_size(inode: *mut inode) -> c_int {
    static int isofs_read_level3_size(struct inode *inode)
    {
    let mut bufsize: c_ulong = ISOFS_BUFFER_SIZE(inode);
    let mut high_sierra: c_int = ISOFS_SB(inode.i_sb).s_high_sierra;
    struct buffer_head *bh = core::ptr::null_mut();
    unsigned long block, offset, block_saved, offset_saved;
    let mut i: c_int = 0;
    let mut more_entries: c_int = 0;
    struct iso_directory_record *tmpde = core::ptr::null_mut();
    struct iso_inode_info *ei = ISOFS_I(inode);
    inode.i_size = 0;
...
// Make sure we have a full directory entry
    if (offset >= bufsize) {
    let mut slop: c_int = bufsize - offset + de_len;
    if (!tmpde) {
...
#[no_mangle]
unsafe extern "C" fn isofs_read_inode(inode: *mut inode, relocated: c_int) -> c_int {
    ...
    let mut bufsize: c_ulong = ISOFS_BUFFER_SIZE(inode);
    unsigned long block;
    let mut high_sierra: c_int = sbi.s_high_sierra;
    struct buffer_head *bh;
    struct iso_directory_record *de;
    struct iso_directory_record *tmpde = core::ptr::null_mut();
    unsigned int de_len;
    ...
    let mut frag1: c_int = bufsize - offset;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct isofs_iget5_callback_data {
    pub block: c_ulong,
    pub offset: c_ulong,
}

#[no_mangle]
unsafe extern "C" fn isofs_iget5_test(ino: *mut inode, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn isofs_iget5_set(ino: *mut inode, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn isofs_get_tree(fc: *mut fs_context) -> c_int {
    static int isofs_get_tree(struct fs_context *fc)
    {
    return get_tree_bdev(fc, isofs_fill_super);
    }
#[no_mangle]
unsafe extern "C" fn isofs_free_fc(fc: *mut fs_context) {
...
#[no_mangle]
unsafe extern "C" fn isofs_init_fs_context(fc: *mut fs_context) -> c_int {
    ...
    opt.iocharset = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn init_iso9660_fs() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_iso9660_fs() -> void __exit {
