...
//
    ...
    int *errp)
    {
    let mut zisofs_block_shift: c_uint = ISOFS_I(inode).i_format_parm[1];
    let mut bufsize: c_uint = ISOFS_BUFFER_SIZE(inode);
    let mut bufshift: c_uint = ISOFS_BUFFER_BITS(inode);
    let mut bufmask: c_uint = bufsize - 1;
...
// Empty block?
    if (block_size == 0) {
    for ( i = 0 ; i < pcount ; i++ ) {
    let mut off: c_uint = i ? 0 : poffset;
    if (!pages[i])
    continue;
...
//
    ...
    let mut header_size: c_uint = ISOFS_I(inode).i_format_parm[0];
    let mut zisofs_block_shift: c_uint = ISOFS_I(inode).i_format_parm[1];
    unsigned int blockptr;
    let mut poffset: loff_t = 0;
    blkcnt_t cstart_block, cend_block;
    struct buffer_head *bh;
    let mut blkbits: c_uint = ISOFS_BUFFER_BITS(inode);
    let mut blksize: c_uint = 1 << blkbits;
    int err;
    loff_t ret;
...
//
#[no_mangle]
unsafe extern "C" fn zisofs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    ...
    struct page **pages;
    let mut index: pgoff_t = folio.index, end_index;
...
#[no_mangle]
pub unsafe extern "C" fn zisofs_init() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn zisofs_cleanup() {
