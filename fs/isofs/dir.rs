...
#[no_mangle]
pub unsafe extern "C" fn isofs_name_translate(de: *mut iso_directory_record, new: *mut c_char, inode: *mut inode) -> c_int {
    int isofs_name_translate(struct iso_directory_record *de, char *new, struct inode *inode)
    {
    let mut old: *mut c_char = de.name;
    let mut len: c_int = de.name_len[0];
    int i;
    for (i = 0; i < len; i++) {
    let mut c: c_uchar = old[i];
    if (!c)
    break;
    if (c >= 'A' && c <= 'Z')
    c |= 0x20;	/* lower case */
...
// Acorn extensions written by Matthew Wilcox <willy@infradead.org> 1998
    ...
    int std;
    unsigned char *chr;
    let mut retnamlen: c_int = isofs_name_translate(de, retname, inode);
    if (retnamlen == 0)
    return 0;
...
//
    ...
    let mut bufbits: c_uchar = ISOFS_BUFFER_BITS(inode);
    unsigned long block, offset, block_saved, offset_saved;
    unsigned long inode_number = 0;	/* Quiet GCC */
    struct buffer_head *bh = core::ptr::null_mut();
    int len;
    int map;
    let mut first_de: c_int = 1;
    char *p = core::ptr::null_mut();		/* Quiet GCC */
...
//
#[no_mangle]
unsafe extern "C" fn isofs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn isofs_fileattr_get(dentry: *mut dentry, fa: *mut file_kattr) -> c_int {
