...
#[no_mangle]
pub unsafe extern "C" fn f2fs_space_for_roll_forward(sbi: *mut f2fs_sb_info) -> bool {
    bool f2fs_space_for_roll_forward(struct f2fs_sb_info *sbi)
    {
    let mut nalloc: i64 = percpu_counter_sum_positive(&sbi.alloc_valid_block_count);
    ...
    GFP_F2FS_ZERO, true, core::ptr::null_mut());
    entry.inode = inode;
...
#[no_mangle]
unsafe extern "C" fn del_fsync_inode(entry: *mut fsync_inode_entry, drop: c_int) {
...
// name_len = min_t(unsigned int, le32_to_cpu(raw->i_namelen),
    ...
    let mut pino: nid_t = le32_to_cpu(raw_inode.i_pino);
...
#[no_mangle]
unsafe extern "C" fn recover_quota_data(inode: *mut inode, folio: *mut folio) -> c_int {
    static int recover_quota_data(struct inode *inode, struct folio *folio)
    {
    struct f2fs_inode *raw = F2FS_INODE(folio);
    struct iattr attr;
    let mut i_uid: uid_t = le32_to_cpu(raw.i_uid);
    let mut i_gid: gid_t = le32_to_cpu(raw.i_gid);
    int err;
...
#[no_mangle]
unsafe extern "C" fn recover_inline_flags(inode: *mut inode, ri: *mut f2fs_inode) {
...
#[no_mangle]
unsafe extern "C" fn recover_inode(inode: *mut inode, folio: *mut folio) -> c_int {
    ...
    ra_blocks * 2);
#[no_mangle]
pub unsafe extern "C" fn if(BLKS_PER_SEG(sbi): next_blkaddr %) -> else {
...
// Detect looped node chain with Floyd's cycle detection algorithm.
    ...
    {
    let mut ra_blocks: c_uint = RECOVERY_MAX_RA_BLOCKS;
    int i;
    if (!*is_detecting)
    return 0;
    for (i = 0; i < 2; i++) {
...
// blkaddr_fast = next_blkaddr_of_node(folio);
    ...
    let mut is_detecting: bool = true;
    let mut err: c_int = 0;
// get node pages in the current segment
    ...
    if (!entry) {
    let mut quota_inode: bool = false;
    if (!check_only &&
    IS_INODE(folio) &&
    is_dent_dnode(folio)) {
...
#[no_mangle]
unsafe extern "C" fn destroy_fsync_dnodes(head: *mut list_head, drop: c_int) {
    ...
    let mut segno: c_uint = GET_SEGNO(sbi, blkaddr);
    let mut blkoff: c_ushort = GET_BLKOFF_FROM_SEG0(sbi, blkaddr);
    struct f2fs_summary_block *sum_node;
    struct f2fs_summary sum;
    struct folio *sum_folio, *node_folio;
    let mut tdn: dnode_of_data = *dn;
    nid_t ino, nid;
    struct inode *inode;
...
//
    ...
    set_new_dnode(&tdn, inode, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    ...
    iput(inode);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: dn->inode_folio_locked) -> else {
...
#[no_mangle]
unsafe extern "C" fn f2fs_reserve_new_block_retry(dn: *mut dnode_of_data) -> c_int {
    ...
    struct node_info ni;
    let mut start: c_uint = 0, end = 0, index;
    let mut err: c_int = 0, recovered = 0;
...
// write dummy data page
    ...
    block_t blkaddr;
    let mut ra_blocks: c_uint = RECOVERY_MAX_RA_BLOCKS;
    let mut recoverable_dnode: c_uint = 0;
    let mut fsynced_dnode: c_uint = 0;
    let mut total_dnode: c_uint = 0;
    let mut recovered_inode: c_uint = 0;
    let mut recovered_dentry: c_uint = 0;
    let mut recovered_dnode: c_uint = 0;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_recover_fsync_data(sbi: *mut f2fs_sb_info, check_only: bool) -> c_int {
    ...
    int err;
    let mut ret: c_int = 0;
    let mut s_flags: c_ulong = sbi.sb.s_flags;
    let mut need_writecp: bool = false;
    let mut new_inode: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_create_recovery_cache() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_destroy_recovery_cache() {
