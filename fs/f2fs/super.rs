...
#[no_mangle]
unsafe extern "C" fn inject_timeout(sbi: *mut f2fs_sb_info) {
    static void inject_timeout(struct f2fs_sb_info *sbi)
    {
    struct f2fs_fault_info *ffi = &F2FS_OPTION(sbi).fault_info;
    let mut type: enum f2fs_timeout_type = ffi.inject_lock_timeout;
    let mut start_time: c_ulong = jiffies;
    let mut timeout: c_ulong = HZ;
    switch (type) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_simulate_lock_timeout(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_init_shrinker() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn f2fs_exit_shrinker() {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct f2fs_fs_context {
    pub info: f2fs_mount_info,
    pub /: *mut *mut unsigned long long opt_mask; / Bits changed,
    pub spec_mask: c_uint,
    pub qname_mask: c_ushort,
}
    ...
    {
    let mut magic: __u16 = le16_to_cpu(sb.s_encoding);
    int i;
...
#[no_mangle]
unsafe extern "C" fn f2fs_create_casefold_cache() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn f2fs_destroy_casefold_cache() {
...
#[no_mangle]
pub unsafe extern "C" fn limit_reserve_root(sbi: *mut f2fs_sb_info) {
    static inline void limit_reserve_root(struct f2fs_sb_info *sbi)
    {
    block_t block_limit = min((sbi.user_block_count >> 3),
    sbi.user_block_count - sbi.reserved_blocks);
    let mut node_limit: block_t = sbi.total_node_count >> 3;
// limit is 12.5%
...
#[no_mangle]
pub unsafe extern "C" fn adjust_unusable_cap_perc(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn adjust_pinned_area_boundary(sbi: *mut f2fs_sb_info) {
...
unsafe extern "C" fn init_once(foo: *mut c_void) {
    ...
    fi.i_crypt_info = core::ptr::null_mut();
...
//
    ...
    char *qname;
    if (param.size < 1) {
    f2fs_err(core::ptr::null_mut(), "Missing quota name");
    return -EINVAL;
    }
    if (strchr(param.string, '/')) {
    f2fs_err(core::ptr::null_mut(), "quotafile must be on filesystem root");
    return -EINVAL;
    }
    if (ctx.info.s_qf_names[qtype]) {
    if (strcmp(ctx.info.s_qf_names[qtype], param.string) != 0) {
    f2fs_err(core::ptr::null_mut(), "Quota file already specified");
    return -EINVAL;
    }
    return 0;
    }
    ...
    if (!qname) {
    f2fs_err(core::ptr::null_mut(), "Not enough memory for storing quotafile name");
    return -ENOMEM;
    }
...
//
#[no_mangle]
unsafe extern "C" fn f2fs_unnote_qf_name(fc: *mut fs_context, qtype: c_int) -> c_int {
    static int f2fs_unnote_qf_name(struct fs_context *fc, int qtype)
    {
    struct f2fs_fs_context *ctx = fc.fs_private;
    kfree(ctx.info.s_qf_names[qtype]);
    ctx.info.s_qf_names[qtype] = core::ptr::null_mut();
    ctx.qname_mask |= 1 << qtype;
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn f2fs_unnote_qf_name_all(fc: *mut fs_context) {
    ...
    f2fs_warn(core::ptr::null_mut(), "test_dummy_encryption option not supported");
    return -EINVAL;
    }
    ...
    if (err) {
    if (err == -EINVAL)
    f2fs_warn(core::ptr::null_mut(), "Value of option \"%s\" is unrecognized",
    param.key);
#[no_mangle]
pub unsafe extern "C" fn if(-EEXIST: err ==) -> else {
    else if (err == -EEXIST)
    f2fs_warn(core::ptr::null_mut(), "Conflicting test_dummy_encryption options");
    else
    f2fs_warn(core::ptr::null_mut(), "Error processing option \"%s\" [%d]",
    param.key, err);
    return -EINVAL;
    }
    return 0;
...
//
    ...
    int ext_cnt)
    {
    let mut index: c_int = 0, no_index = 0;
    if (!noext_cnt)
    return 0;
    ...
    f2fs_info(core::ptr::null_mut(), "Don't allow the nocompress extension specifies all files");
    return -EINVAL;
    }
    ...
    f2fs_info(core::ptr::null_mut(), "Don't allow the same extension %s appear in both compress and nocompress extension",
    ext[index]);
    return -EINVAL;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn f2fs_set_lz4hc_level(ctx: *mut f2fs_fs_context, str: *const c_char) -> c_int {
    ...
    str += 3;
    if (str[0] != ':') {
    f2fs_info(core::ptr::null_mut(), "wrong format, e.g. <alg_name>:<compr_level>");
    return -EINVAL;
    }
    if (kstrtouint(str + 1, 10, &level))
    return -EINVAL;
    if (!f2fs_is_compress_level_valid(COMPRESS_LZ4, level)) {
    f2fs_info(core::ptr::null_mut(), "invalid lz4hc compress level: %d", level);
    return -EINVAL;
    }
    ...
    f2fs_info(core::ptr::null_mut(), "kernel doesn't support lz4hc compression");
    return -EINVAL;

    }

#[no_mangle]
unsafe extern "C" fn f2fs_set_zstd_level(ctx: *mut f2fs_fs_context, str: *const c_char) -> c_int {
...
// f2fs does not support negative compress level now
    if (level < 0) {
    f2fs_info(core::ptr::null_mut(), "do not support negative compress level: %d", level);
    return -ERANGE;
    }
    if (!f2fs_is_compress_level_valid(COMPRESS_ZSTD, level)) {
    f2fs_info(core::ptr::null_mut(), "invalid zstd compress level: %d", level);
    return -EINVAL;
    }
    F2FS_CTX_INFO(ctx).compress_level = level;
    ctx.spec_mask |= F2FS_SPEC_compress_level;
...
#[no_mangle]
unsafe extern "C" fn f2fs_parse_param(fc: *mut fs_context, param: *mut fs_parameter) -> c_int {
...
// requires ro mount, checked in f2fs_validate_options
    ...
    case Opt_noheap:
    case Opt_heap:
    f2fs_warn(core::ptr::null_mut(), "heap/no_heap options were deprecated");
    break;
    ...
    f2fs_err(core::ptr::null_mut(), "inline xattr size is out of range: %u ~ %u",
    ...
    case Opt_inline_xattr:
    case Opt_inline_xattr_size:
    f2fs_info(core::ptr::null_mut(), "%s options not supported", param.key);
    break;
    ...
    case Opt_grpjquota:
    case Opt_prjjquota:
    f2fs_info(core::ptr::null_mut(), "quota operations not supported");
    break;
    ...
    f2fs_info(core::ptr::null_mut(), "inline encryption not supported");
...
//
    args[0].from = args[0].to = core::ptr::null_mut();
    arg = 0;
// revert to match_table for checkpoint= options
    ...
    f2fs_info(core::ptr::null_mut(), "kernel doesn't support lzo compression");
    ...
    f2fs_info(core::ptr::null_mut(), "kernel doesn't support lz4 compression");
    ...
    f2fs_info(core::ptr::null_mut(), "kernel doesn't support zstd compression");
    ...
    f2fs_info(core::ptr::null_mut(), "kernel doesn't support lzorle compression");
    ...
    f2fs_err(core::ptr::null_mut(), "invalid extension length/number");
    return -EINVAL;
    }
    ...
    case Opt_compress_mode:
    case Opt_compress_cache:
    f2fs_info(core::ptr::null_mut(), "compression options not supported");
    break;
...
//
    ...
    struct f2fs_fs_context *ctx = fc.fs_private;
    let mut quota_feature: bool = f2fs_sb_has_quota_ino(sbi);
    let mut quota_turnon: bool = sb_any_quota_loaded(sb);
...
//
    ...
    F2FS_CTX_INFO(ctx).s_qf_names[i] = core::ptr::null_mut();
    }
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn test_compression_spec(mask: c_uint) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn clear_compression_spec(ctx: *mut f2fs_fs_context) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_apply_options(fc: *mut fs_context, sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_sanity_check_options(sbi: *mut f2fs_sb_info, remount: bool) -> c_int {
    static int f2fs_sanity_check_options(struct f2fs_sb_info *sbi, bool remount)
    {
    let mut total_sections: c_uint = le32_to_cpu(sbi.raw_super.section_count);
...
#[no_mangle]
unsafe extern "C" fn f2fs_drop_inode(inode: *mut inode) -> c_int {
...
// should remain fi->extent_tree for writepage
    ...
    inode, core::ptr::null_mut(), 0, DATA);
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_inode_dirtied(inode: *mut inode, sync: bool) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_inode_synced(inode: *mut inode) {
...
//
#[no_mangle]
unsafe extern "C" fn f2fs_dirty_inode(inode: *mut inode, flags: c_int) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_free_inode(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn destroy_percpu_info(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
unsafe extern "C" fn destroy_device_list(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_put_super(sb: *mut super_block) {
...
// our cp_error case, we can wait for any writeback page
    ...
    iput(sbi.node_inode);
    sbi.node_inode = core::ptr::null_mut();
    iput(sbi.meta_inode);
    sbi.meta_inode = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_sync_fs(sb: *mut super_block, sync: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_freeze(sb: *mut super_block) -> c_int {
...
// Let's flush checkpoints and stop the thread.
    ...
    sbi.umount_lock_holder = core::ptr::null_mut();
// to avoid deadlock on f2fs_evict_inode->SB_FREEZE_FS
    set_sbi_flag(sbi, SBI_IS_FREEZING);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn f2fs_unfreeze(sb: *mut super_block) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_statfs(dentry: *mut dentry, buf: *mut kstatfs) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(COMPR_MODE_USER: F2FS_OPTION(sbi).compress_mode ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn f2fs_show_options(seq: *mut seq_file, root: *mut dentry) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(BGGC_MODE_ON: F2FS_OPTION(sbi).bggc_mode ==) -> else {
    else if (F2FS_OPTION(sbi).bggc_mode == BGGC_MODE_ON)
    seq_printf(seq, ",background_gc=%s", "on");
#[no_mangle]
pub unsafe extern "C" fn if(BGGC_MODE_OFF: F2FS_OPTION(sbi).bggc_mode ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(DISCARD_UNIT_SEGMENT: F2FS_OPTION(sbi).discard_unit ==) -> else {
    else if (F2FS_OPTION(sbi).discard_unit == DISCARD_UNIT_SEGMENT)
    seq_printf(seq, ",discard_unit=%s", "segment");
#[no_mangle]
pub unsafe extern "C" fn if(DISCARD_UNIT_SECTION: F2FS_OPTION(sbi).discard_unit ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(FS_MODE_LFS: F2FS_OPTION(sbi).fs_mode ==) -> else {
    else if (F2FS_OPTION(sbi).fs_mode == FS_MODE_LFS)
    seq_puts(seq, "lfs");
#[no_mangle]
pub unsafe extern "C" fn if(_arg: f2fs_need_rand_seg(sbi, _arg: NO_CHECK_TYPE)) -> else {
    else if (f2fs_need_rand_seg(sbi, NO_CHECK_TYPE))
    seq_puts(seq, "fragment:segment");
#[no_mangle]
pub unsafe extern "C" fn if(_arg: f2fs_need_rand_blk(sbi, _arg: NO_CHECK_TYPE)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(ALLOC_MODE_REUSE: F2FS_OPTION(sbi).alloc_mode ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(FSYNC_MODE_STRICT: F2FS_OPTION(sbi).fsync_mode ==) -> else {
    else if (F2FS_OPTION(sbi).fsync_mode == FSYNC_MODE_STRICT)
    seq_printf(seq, ",fsync_mode=%s", "strict");
#[no_mangle]
pub unsafe extern "C" fn if(FSYNC_MODE_NOBARRIER: F2FS_OPTION(sbi).fsync_mode ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(MEMORY_MODE_LOW: F2FS_OPTION(sbi).memory_mode ==) -> else {
    else if (F2FS_OPTION(sbi).memory_mode == MEMORY_MODE_LOW)
    seq_printf(seq, ",memory=%s", "low");
    if (F2FS_OPTION(sbi).errors == MOUNT_ERRORS_READONLY)
    seq_printf(seq, ",errors=%s", "remount-ro");
#[no_mangle]
pub unsafe extern "C" fn if(MOUNT_ERRORS_CONTINUE: F2FS_OPTION(sbi).errors ==) -> else {
    else if (F2FS_OPTION(sbi).errors == MOUNT_ERRORS_CONTINUE)
    seq_printf(seq, ",errors=%s", "continue");
#[no_mangle]
pub unsafe extern "C" fn if(MOUNT_ERRORS_PANIC: F2FS_OPTION(sbi).errors ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(LOOKUP_COMPAT: F2FS_OPTION(sbi).lookup_mode ==) -> else {
    else if (F2FS_OPTION(sbi).lookup_mode == LOOKUP_COMPAT)
    seq_show_option(seq, "lookup_mode", "compat");
#[no_mangle]
pub unsafe extern "C" fn if(LOOKUP_AUTO: F2FS_OPTION(sbi).lookup_mode ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn default_options(sbi: *mut f2fs_sb_info, remount: bool) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_disable_checkpoint(sbi: *mut f2fs_sb_info) -> c_int {
    static int f2fs_disable_checkpoint(struct f2fs_sb_info *sbi)
    {
    let mut s_flags: c_uint = sbi.sb.s_flags;
    struct cp_control cpc;
    struct f2fs_lock_context lc;
    let mut gc_mode: c_uint = sbi.gc_mode;
    let mut err: c_int = 0;
    int ret;
    block_t unusable;
...
#[no_mangle]
unsafe extern "C" fn f2fs_enable_checkpoint(sbi: *mut f2fs_sb_info) -> c_int {
    static int f2fs_enable_checkpoint(struct f2fs_sb_info *sbi)
    {
    let mut retry: c_int = MAX_FLUSH_RETRY_COUNT;
...
#[no_mangle]
unsafe extern "C" fn __f2fs_remount(fc: *mut fs_context, sb: *mut super_block) -> c_int {
    static int __f2fs_remount(struct fs_context *fc, struct super_block *sb)
    {
    struct f2fs_sb_info *sbi = F2FS_SB(sb);
    struct f2fs_mount_info org_mount_opt;
    unsigned long old_sb_flags;
    let mut flags: c_uint = fc.sb_flags;
    int err;
    let mut need_restart_gc: bool = false, need_stop_gc = false;
    let mut need_restart_flush: bool = false, need_stop_flush = false;
    let mut need_restart_discard: bool = false, need_stop_discard = false;
    let mut need_enable_checkpoint: bool = false, need_disable_checkpoint = false;
    let mut no_read_extent_cache: bool = !test_opt(sbi, READ_EXTENT_CACHE);
    let mut no_age_extent_cache: bool = !test_opt(sbi, AGE_EXTENT_CACHE);
    let mut enable_checkpoint: bool = !test_opt(sbi, DISABLE_CHECKPOINT);
    let mut no_atgc: bool = !test_opt(sbi, ATGC);
    let mut no_discard: bool = !test_opt(sbi, DISCARD);
    let mut no_compress_cache: bool = !test_opt(sbi, COMPRESS_CACHE);
    let mut block_unit_discard: bool = f2fs_block_unit_discard(sbi);
    let mut no_nat_bits: bool = !test_opt(sbi, NAT_BITS);
...
//
    ...
    org_mount_opt.s_qf_names[i] = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn f2fs_shutdown(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_need_recovery(sbi: *mut f2fs_sb_info) -> bool {
...
#[no_mangle]
unsafe extern "C" fn f2fs_recover_quota_begin(sbi: *mut f2fs_sb_info) -> bool {
    static bool f2fs_recover_quota_begin(struct f2fs_sb_info *sbi)
    {
    let mut readonly: bool = f2fs_readonly(sbi.sb);
...
// Write to quotafile
    ...
    towrite);
    retry:
    err = a_ops.write_begin(core::ptr::null_mut(), mapping, off, tocopy,
    &folio, &fsdata);
    if (unlikely(err)) {
    if (err == -ENOMEM) {
    memalloc_retry_wait(GFP_NOFS);
    ...
    a_ops.write_end(core::ptr::null_mut(), mapping, off, tocopy, tocopy,
    folio, fsdata);
    offset = 0;
    towrite -= tocopy;
    off += tocopy;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_dquot_initialize(inode: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_quota_on_mount(sbi: *mut f2fs_sb_info, type: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_enable_quota_files(sbi: *mut f2fs_sb_info, rdonly: bool) -> c_int {
    ...
    let mut qf_flag: c_ulong = F2FS_QUOTA_DEFAULT_FL;
    int err;
...
#[no_mangle]
unsafe extern "C" fn f2fs_enable_quotas(sb: *mut super_block) -> c_int {
    static int f2fs_enable_quotas(struct super_block *sb)
    {
    struct f2fs_sb_info *sbi = F2FS_SB(sb);
    int type, err = 0;
...
#[no_mangle]
unsafe extern "C" fn f2fs_quota_sync_file(sbi: *mut f2fs_sb_info, type: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_do_quota_sync(sb: *mut super_block, type: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_quota_sync(sb: *mut super_block, type: c_int) -> c_int {
    static int f2fs_quota_sync(struct super_block *sb, int type)
    {
    int ret;
    F2FS_SB(sb).umount_lock_holder = current;
    ret = f2fs_do_quota_sync(sb, type);
    F2FS_SB(sb).umount_lock_holder = core::ptr::null_mut();
    return ret;
    }
...
#[no_mangle]
unsafe extern "C" fn __f2fs_quota_off(sb: *mut super_block, type: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_quota_off(sb: *mut super_block, type: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_quota_off_umount(sb: *mut super_block) {
    ...
    err = __f2fs_quota_off(sb, type);
    if (err) {
    let mut ret: c_int = dquot_quota_off(sb, type);
...
#[no_mangle]
unsafe extern "C" fn f2fs_truncate_quota_inode_pages(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_dquot_commit(dquot: *mut dquot) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_dquot_acquire(dquot: *mut dquot) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_dquot_release(dquot: *mut dquot) -> c_int {
    static int f2fs_dquot_release(struct dquot *dquot)
    {
    struct f2fs_sb_info *sbi = F2FS_SB(dquot.dq_sb);
    let mut ret: c_int = dquot_release(dquot);
    if (ret < 0)
    set_sbi_flag(sbi, SBI_QUOTA_NEED_REPAIR);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn f2fs_dquot_mark_dquot_dirty(dquot: *mut dquot) -> c_int {
    static int f2fs_dquot_mark_dquot_dirty(struct dquot *dquot)
    {
    struct super_block *sb = dquot.dq_sb;
    struct f2fs_sb_info *sbi = F2FS_SB(sb);
    let mut ret: c_int = dquot_mark_dquot_dirty(dquot);
// if we are using journalled quota
    if (is_journalled_quota(sbi))
...
#[no_mangle]
unsafe extern "C" fn f2fs_dquot_commit_info(sb: *mut super_block, type: c_int) -> c_int {
    static int f2fs_dquot_commit_info(struct super_block *sb, int type)
    {
    struct f2fs_sb_info *sbi = F2FS_SB(sb);
    let mut ret: c_int = dquot_commit_info(sb, type);
    if (ret < 0)
    set_sbi_flag(sbi, SBI_QUOTA_NEED_REPAIR);
    return ret;
    }
#[no_mangle]
unsafe extern "C" fn f2fs_get_projid(inode: *mut inode, projid: *mut kprojid_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_get_context(inode: *mut inode, ctx: *mut c_void, len: usize) -> c_int {
    static int f2fs_get_context(struct inode *inode, void *ctx, size_t len)
    {
    return f2fs_getxattr(inode, F2FS_XATTR_INDEX_ENCRYPTION,
    F2FS_XATTR_NAME_ENCRYPTION_CONTEXT,
    ctx, len, core::ptr::null_mut());
    }
...
#[no_mangle]
unsafe extern "C" fn f2fs_has_stable_inodes(sb: *mut super_block) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn max_file_blocks(inode: *mut inode) -> loff_t {
    loff_t max_file_blocks(struct inode *inode)
    {
    let mut result: loff_t = 0;
    loff_t leaf_count;
//
...
// it's rare case, we can do fua all the time
    let mut opf: blk_opf_t = REQ_OP_WRITE | REQ_SYNC | REQ_PREFLUSH | REQ_FUA;
    int ret;
    folio_lock(folio);
...
// it doesn't need to set crypto context for superblock update
    ...
    let mut segment0_blkaddr: u32 = le32_to_cpu(raw_super.segment0_blkaddr);
    let mut cp_blkaddr: u32 = le32_to_cpu(raw_super.cp_blkaddr);
    let mut sit_blkaddr: u32 = le32_to_cpu(raw_super.sit_blkaddr);
    let mut nat_blkaddr: u32 = le32_to_cpu(raw_super.nat_blkaddr);
    let mut ssa_blkaddr: u32 = le32_to_cpu(raw_super.ssa_blkaddr);
    let mut main_blkaddr: u32 = le32_to_cpu(raw_super.main_blkaddr);
    let mut segment_count_ckpt: u32 = le32_to_cpu(raw_super.segment_count_ckpt);
    let mut segment_count_sit: u32 = le32_to_cpu(raw_super.segment_count_sit);
    let mut segment_count_nat: u32 = le32_to_cpu(raw_super.segment_count_nat);
    let mut segment_count_ssa: u32 = le32_to_cpu(raw_super.segment_count_ssa);
    let mut segment_count_main: u32 = le32_to_cpu(raw_super.segment_count_main);
    let mut segment_count: u32 = le32_to_cpu(raw_super.segment_count);
    let mut log_blocks_per_seg: u32 = le32_to_cpu(raw_super.log_blocks_per_seg);
...
// blocks_per_seg should be 512, given the above check
    ...
    if (RDEV(0).path[0]) {
    let mut dev_seg_count: block_t = le32_to_cpu(RDEV(0).total_segments);
    let mut i: c_int = 1;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_sanity_check_ckpt(sbi: *mut f2fs_sb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn init_sb_info(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
unsafe extern "C" fn init_percpu_info(sbi: *mut f2fs_sb_info) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct f2fs_report_zones_args {
    pub sbi: *mut f2fs_sb_info,
    pub dev: *mut f2fs_dev_info,
}
...
#[no_mangle]
unsafe extern "C" fn init_blkz_info(sbi: *mut f2fs_sb_info, devi: c_int) -> c_int {
    static int init_blkz_info(struct f2fs_sb_info *sbi, int devi)
    {
    struct block_device *bdev = FDEV(devi).bdev;
    let mut nr_sectors: sector_t = bdev_nr_sectors(bdev);
...
//
    ...
    folio = read_mapping_folio(sb.s_bdev.bd_mapping, block, core::ptr::null_mut());
    if (IS_ERR(folio)) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_commit_super(sbi: *mut f2fs_sb_info, recover: bool) -> c_int {
...
// write back-up superblock first
    ...
    folio = read_mapping_folio(sbi.sb.s_bdev.bd_mapping, index, core::ptr::null_mut());
    if (IS_ERR(folio))
    return PTR_ERR(folio);
...
#[no_mangle]
unsafe extern "C" fn save_stop_reason(sbi: *mut f2fs_sb_info, reason: c_uchar) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_record_stop_reason(sbi: *mut f2fs_sb_info) {
    static void f2fs_record_stop_reason(struct f2fs_sb_info *sbi)
    {
    struct f2fs_super_block *raw_super = F2FS_RAW_SUPER(sbi);
    unsigned long flags;
    let mut report_shutdown: bool = false;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_save_errors(sbi: *mut f2fs_sb_info, flag: c_uchar) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_report_fserror(sbi: *mut f2fs_sb_info, error: c_uchar) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_handle_error(sbi: *mut f2fs_sb_info, error: c_uchar) {
...
unsafe extern "C" fn system_going_down() -> bool {
    ...
    {
    struct super_block *sb = sbi.sb;
    let mut shutdown: bool = reason == STOP_CP_REASON_SHUTDOWN;
...
#[no_mangle]
unsafe extern "C" fn f2fs_record_error_work(work: *mut work_struct) {
...
#[no_mangle]
pub unsafe extern "C" fn get_first_seq_zone_segno(sbi: *mut f2fs_sb_info) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn f2fs_scan_devices(sbi: *mut f2fs_sb_info) -> c_int {
    static int f2fs_scan_devices(struct f2fs_sb_info *sbi)
    {
    struct f2fs_super_block *raw_super = F2FS_RAW_SUPER(sbi);
    let mut max_devices: c_uint = MAX_DEVICES;
    unsigned int logical_blksize;
    let mut mode: blk_mode_t = sb_open_mode(sbi.sb.s_flags);
    int i;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !RDEV(i).path[0]) -> else {
...
#[no_mangle]
unsafe extern "C" fn f2fs_setup_casefold(sbi: *mut f2fs_sb_info) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_tuning_parameters(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_restore_device_alias(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_fill_super(sb: *mut super_block, fc: *mut fs_context) -> c_int {
    ...
    struct inode *root;
    int err;
    let mut skip_recovery: bool = false, need_fsck = false;
    int recovery, i, valid_super_block;
    struct curseg_info *seg_i;
    let mut retry_cnt: c_int = 1;

    let mut quota_enabled: bool = false;

    try_onemore:
    err = -EINVAL;
    raw_super = core::ptr::null_mut();
    valid_super_block = -1;
    recovery = 0;
...
// precompute checksum seed for metadata
    ...
    if (err)
    goto free_options;
    sb.s_maxbytes = max_file_blocks(core::ptr::null_mut()) <<
    le32_to_cpu(raw_super.log_blocksize);
...
// f2fs_recover_fsync_data() cleared this already
    ...
    err = f2fs_disable_checkpoint(sbi);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_set_ckpt_flags(sbi, _arg: CP_DISABLED_FLAG)) -> else {
...
// flush s_error_work before sbi destroy
    ...
    utf8_unload(sb.s_encoding);
    sb.s_encoding = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn f2fs_get_tree(fc: *mut fs_context) -> c_int {
    static int f2fs_get_tree(struct fs_context *fc)
    {
    return get_tree_bdev(fc, f2fs_fill_super);
    }
#[no_mangle]
unsafe extern "C" fn f2fs_reconfigure(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_fc_free(fc: *mut fs_context) {
...
#[no_mangle]
unsafe extern "C" fn kill_f2fs_super(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_init_fs_context(fc: *mut fs_context) -> c_int {
...
unsafe extern "C" fn init_inodecache() -> int __init {
    ...
    SLAB_RECLAIM_ACCOUNT|SLAB_ACCOUNT, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn init_f2fs_fs() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_f2fs_fs() -> void __exit {
