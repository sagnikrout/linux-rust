...
pub const NUM_PREALLOC_POST_READ_CTXS: c_int = 128;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct f2fs_folio_state {
    pub state_lock: spinlock_t,
    pub read_pages_pending: c_uint,
}
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_init_bioset() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_destroy_bioset() {
    void f2fs_destroy_bioset(void)
    {
    bioset_exit(&f2fs_bioset);
    }
#[no_mangle]
pub unsafe extern "C" fn f2fs_is_cp_guaranteed(folio: *const folio) -> bool {
...
#[no_mangle]
unsafe extern "C" fn __read_io_type(folio: *mut folio) -> enum count_type {
...
//
#[no_mangle]
unsafe extern "C" fn f2fs_finish_read_bio(bio: *mut bio, in_task: bool) {
    ...
    let mut nr_pages: unsigned = fi.length >> PAGE_SHIFT;
...
#[no_mangle]
unsafe extern "C" fn f2fs_verify_bio(work: *mut work_struct) {
    static void f2fs_verify_bio(struct work_struct *work)
    {
    struct bio_post_read_ctx *ctx =
    container_of(work, struct bio_post_read_ctx, work);
    struct bio *bio = ctx.bio;
    let mut may_have_compressed_pages: bool = (ctx.enabled_steps & STEP_DECOMPRESS);
...
//
#[no_mangle]
unsafe extern "C" fn f2fs_verify_and_finish_bio(bio: *mut bio, in_task: bool) {
...
//
    ...
    {
    struct folio_iter fi;
    let mut all_compressed: bool = true;
    let mut blkaddr: block_t = ctx.fs_blkaddr;
...
#[no_mangle]
unsafe extern "C" fn f2fs_post_read_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_read_end_io(bio: *mut bio) {
    static void f2fs_read_end_io(struct bio *bio)
    {
    struct f2fs_sb_info *sbi = F2FS_F_SB(bio_first_folio_all(bio));
    struct bio_post_read_ctx *ctx;
    let mut intask: bool = in_task() && !irqs_disabled();
...
#[no_mangle]
unsafe extern "C" fn f2fs_write_end_bio(bio: *mut bio) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_write_end_io_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_write_end_io(bio: *mut bio) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_zone_write_end_io(bio: *mut bio) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_target_device_index(sbi: *mut f2fs_sb_info, blkaddr: block_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_io_flags(fio: *mut f2fs_io_info) -> blk_opf_t {
    static blk_opf_t f2fs_io_flags(struct f2fs_io_info *fio)
    {
    let mut temp_mask: c_uint = GENMASK(NR_TEMP_TYPE - 1, 0);
    unsigned int fua_flag, meta_flag, io_flag;
    let mut op_flags: blk_opf_t = 0;
    if (fio.op != REQ_OP_WRITE)
    return 0;
    if (fio.type == DATA)
    io_flag = fio.sbi.data_io_flag;
#[no_mangle]
pub unsafe extern "C" fn if(NODE: fio->type ==) -> else {
...
//
    ...
    iostat_alloc_and_bind_ctx(sbi, bio, core::ptr::null_mut());
    if (fio.io_wbc)
...
#[no_mangle]
unsafe extern "C" fn __submit_merged_bio(io: *mut f2fs_bio_info) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_init_write_merge_io(sbi: *mut f2fs_sb_info) -> c_int {
    int f2fs_init_write_merge_io(struct f2fs_sb_info *sbi)
    {
    int i;
    for (i = 0; i < NR_PAGE_TYPE; i++) {
    let mut n: c_int = (i == META) ? 1 : NR_TEMP_TYPE;
    int j;
    ...
    io.zone_pending_bio = core::ptr::null_mut();
    io.bi_private = core::ptr::null_mut();
    ...
    {
    let mut btype: enum page_type = PAGE_TYPE_OF_BIO(type);
...
// change META to META_FLUSH in the checkpoint procedure
    ...
    let mut force: bool = !inode && !folio && !ino;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_submit_merged_write(sbi: *mut f2fs_sb_info, type: enum page_type) {
    void f2fs_submit_merged_write(struct f2fs_sb_info *sbi, enum page_type type)
    {
    __submit_merged_write_cond(sbi, core::ptr::null_mut(), core::ptr::null_mut(), 0, type, false);
    }
    ...
    {
    __submit_merged_write_cond(sbi, core::ptr::null_mut(), folio, 0, type, true);
    }
#[no_mangle]
pub unsafe extern "C" fn f2fs_flush_merged_writes(sbi: *mut f2fs_sb_info) {
...
//
#[no_mangle]
pub unsafe extern "C" fn f2fs_submit_page_bio(fio: *mut f2fs_io_info) -> c_int {
...
// Allocate a new bio
    ...
    return false;
    return bio.bi_bdev == f2fs_target_device(sbi, cur_blkaddr, core::ptr::null_mut());
    }
    static bool io_type_is_mergeable(struct f2fs_bio_info *io,
    struct f2fs_io_info *fio)
    {
    let mut mask: blk_opf_t = ~(REQ_PREFLUSH | REQ_FUA);
    if (io.fio.op != fio.op)
    return false;
    ...
    struct bio_entry *be;
    be = f2fs_kmem_cache_alloc(bio_entry_slab, GFP_NOFS, true, core::ptr::null_mut());
    be.bio = bio;
    bio_get(bio);
...
#[no_mangle]
unsafe extern "C" fn del_bio_entry(be: *mut bio_entry) {
...
// bio = NULL;
    ...
    enum temp_type temp;
    let mut found: bool = false;
    struct bio *target = bio ? *bio : core::ptr::null_mut();
    ...
    found = __has_merged_page(be.bio, core::ptr::null_mut(),
    folio, 0);
    if (found)
    break;
    }
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_submit_all_merged_ipu_writes(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_merge_page_bio(fio: *mut f2fs_io_info) -> c_int {
    ...
    fio.new_blkaddr))
    f2fs_submit_merged_ipu_write(fio.sbi, &bio, core::ptr::null_mut());
    alloc_new:
    if (!bio) {
...
#[no_mangle]
unsafe extern "C" fn is_end_zone_blkaddr(sbi: *mut f2fs_sb_info, blkaddr: block_t) -> bool {
    static bool is_end_zone_blkaddr(struct f2fs_sb_info *sbi, block_t blkaddr)
    {
    struct block_device *bdev = sbi.sb.s_bdev;
    let mut devi: c_int = 0;
    if (f2fs_is_multi_device(sbi)) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_submit_page_write(fio: *mut f2fs_io_info) {
    void f2fs_submit_page_write(struct f2fs_io_info *fio)
    {
    struct f2fs_sb_info *sbi = fio.sbi;
    let mut btype: enum page_type = PAGE_TYPE_OF_BIO(fio.type);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: fio->compressed_page) -> else {
...
// set submitted = true as a return value
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct bio *bio;
    struct bio_post_read_ctx *ctx = core::ptr::null_mut();
    let mut post_read_steps: c_uint = 0;
    sector_t sector;
    ...
    bio.bi_iter.bi_sector = sector;
    f2fs_set_bio_crypt_ctx(bio, inode, first_idx, core::ptr::null_mut(), GFP_NOFS);
    bio.bi_end_io = f2fs_read_end_io;
    if (vi)
...
// wait for GCed page writeback via META_MAPPING
    ...
    f2fs_update_iostat(sbi, core::ptr::null_mut(), FS_DATA_READ_IO, F2FS_BLKSIZE);
    f2fs_submit_read_bio(sbi, bio, DATA);
    }
#[no_mangle]
unsafe extern "C" fn __set_data_blkaddr(dn: *mut dnode_of_data, blkaddr: block_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn f2fs_set_data_blkaddr(dn: *mut dnode_of_data, blkaddr: block_t) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_update_data_blkaddr(dn: *mut dnode_of_data, blkaddr: block_t) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_reserve_new_blocks(dn: *mut dnode_of_data, count: blkcnt_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_reserve_new_block(dn: *mut dnode_of_data) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_reserve_block(dn: *mut dnode_of_data, index: pgoff_t) -> c_int {
    int f2fs_reserve_block(struct dnode_of_data *dn, pgoff_t index)
    {
    let mut need_put: bool = dn.inode_folio ? false : true;
    int err;
    ...
    return folio;
    if (folio_test_large(folio)) {
    let mut folio_index: pgoff_t = mapping_align_index(mapping, index);
...
//
    ...
    struct address_space *mapping = inode.i_mapping;
    struct folio *folio;
    folio = f2fs_get_read_data_folio(inode, index, 0, for_write, core::ptr::null_mut());
    if (IS_ERR(folio))
    return folio;
...
#[no_mangle]
unsafe extern "C" fn __allocate_data_block(dn: *mut dnode_of_data, seg_type: c_int) -> c_int {
    ...
    err = f2fs_allocate_data_block(sbi, core::ptr::null_mut(), old_blkaddr,
    &dn.data_blkaddr, &sum, seg_type, core::ptr::null_mut());
    if (err) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_get_block_locked(dn: *mut dnode_of_data, index: pgoff_t) -> c_int {
...
// map->m_next_extent = f2fs_get_next_page_offset(dn, pgoff);
    ...
    {
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    let mut maxblocks: c_uint = map.m_len;
    let mut pgoff: pgoff_t = (pgoff_t)map.m_lblk;
...
// for hardware encryption, but to avoid potential issue in future
    ...
    let mut bidx: c_int = f2fs_target_device_index(sbi, map.m_pblk);
...
//
#[no_mangle]
pub unsafe extern "C" fn f2fs_map_blocks(inode: *mut inode, map: *mut f2fs_map_blocks, flag: c_int) -> c_int {
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct f2fs_lock_context lc;
    let mut mode: c_int = map.m_may_create ? ALLOC_NODE : LOOKUP_NODE;
    pgoff_t pgofs, end_offset, end;
    let mut err: c_int = 0, ofs = 1;
    unsigned int ofs_in_node, last_ofs_in_node;
    blkcnt_t prealloc;
    block_t blkaddr;
    unsigned int start_pgofs;
    let mut bidx: c_int = 0;
    bool is_hole;
    bool lfs_dio_write;
    if (!maxblocks)
    return 0;
...
// preallocate blocks in batch for one dnode page
    ...
    goto sync_out;
#[no_mangle]
pub unsafe extern "C" fn if(end_offset: dn.ofs_in_node <) -> else {
    else if (dn.ofs_in_node < end_offset)
    goto next_block;
    if (flag == F2FS_GET_BLOCK_PRECACHE) {
    if (map.m_flags & F2FS_MAP_MAPPED) {
    let mut ofs: c_uint = start_pgofs - map.m_lblk;
...
//
    ...
    map.m_pblk, map.m_len);
    if (map.m_multidev_dio) {
    let mut blk_addr: block_t = map.m_pblk;
...
// map->m_next_extent = is_hole ? pgofs + 1 : pgofs;
    ...
    map.m_next_extent = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_overwrite_io(inode: *mut inode, pos: loff_t, len: usize) -> bool {
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct node_info ni;
    let mut phys: __u64 = 0, len;
    __u32 flags;
    let mut xnid: nid_t = F2FS_I(inode).i_xattr_nid;
    let mut err: c_int = 0;
    ...
    pgoff_t next_pgofs;
    let mut logical: u64 = 0, phys = 0, size = 0;
    let mut flags: u32 = 0;
    let mut ret: c_int = 0;
    let mut compr_cluster: bool = false, compr_appended;
    let mut cluster_size: c_uint = F2FS_I(inode).i_cluster_size;
    let mut count_in_cluster: c_uint = 0;
    loff_t maxbytes;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_readpage_limit(inode: *mut inode) -> loff_t {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_ra_op_flags(rac: *mut readahead_control) -> blk_opf_t {
    ...
    let mut blocksize: c_uint = F2FS_BLKSIZE;
    sector_t block_in_file;
    sector_t last_block;
    sector_t last_block_in_file;
    sector_t block_nr;
    let mut index: pgoff_t = folio.index;
    let mut ret: c_int = 0;
...
// last_block_in_bio, block_nr) ||
    !f2fs_crypt_mergeable_bio(bio, inode, index, core::ptr::null_mut()))) {
    submit_and_realloc:
...
//
    ...
    f2fs_update_iostat(F2FS_I_SB(inode), core::ptr::null_mut(), FS_DATA_READ_IO,
    F2FS_BLKSIZE);
// last_block_in_bio = block_nr;
    out:
// bio_ret = bio;
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct bio *bio = *bio_ret;
    let mut start_idx: c_uint = cc.cluster_idx << cc.log_cluster_size;
    sector_t last_block_in_file;
    let mut blocksize: c_uint = F2FS_BLKSIZE;
    struct decompress_io_ctx *dic = core::ptr::null_mut();
    let mut ei: extent_info = {};
    let mut from_dnode: bool = true;
    int i;
    let mut ret: c_int = 0;
    if (unlikely(f2fs_cp_error(sbi))) {
    ret = -EIO;
...
// last_block_in_bio, blkaddr) ||
    !f2fs_crypt_mergeable_bio(bio, inode, folio.index, core::ptr::null_mut()))) {
    submit_and_realloc:
    f2fs_submit_read_bio(sbi, bio, DATA);
    bio = core::ptr::null_mut();
    }
...
// bio_ret = bio;
    ...
    GFP_NOIO | __GFP_ZERO, true, core::ptr::null_mut());
    spin_lock_init(&ffs.state_lock);
    folio_attach_private(folio, ffs);
    return ffs;
    }
#[no_mangle]
unsafe extern "C" fn ffs_detach_free(folio: *mut folio) {
    ...
    struct f2fs_folio_state *ffs;
    let mut ret: c_int = 0;
    let mut folio_in_bio: bool = false;
...
//
    ...
    let mut page_offset: usize = offset << PAGE_SHIFT;
...
//
    ...
    .rpages = core::ptr::null_mut(),
    .cpages = core::ptr::null_mut(),
    .nr_rpages = 0,
    .nr_cpages = 0,
    };
    let mut nc_cluster_idx: pgoff_t = NULL_CLUSTER;
    pgoff_t index;

    let mut next_pgofs: pgoff_t = 0;
    let mut nr_pages: unsigned = rac ? readahead_count(rac) : 1;
    struct address_space *mapping = rac ? rac.mapping : folio.mapping;
    let mut max_nr_pages: unsigned = nr_pages;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn f2fs_read_data_folio(file: *mut file, folio: *mut folio) -> c_int {
    static int f2fs_read_data_folio(struct file *file, struct folio *folio)
    {
    struct inode *inode = folio.mapping.host;
    struct fsverity_info *vi = core::ptr::null_mut();
    int ret;
...
// If the file has inline data, try to read it directly
    ...
    return f2fs_mpage_readpages(inode, vi, core::ptr::null_mut(), folio);
    }
#[no_mangle]
unsafe extern "C" fn f2fs_readahead(rac: *mut readahead_control) {
...
// If the file has inline data, skip readahead
    ...
    readahead_count(rac));
    f2fs_mpage_readpages(inode, vi, rac, core::ptr::null_mut());
    }
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_should_update_inplace(inode: *mut inode, fio: *mut f2fs_io_info) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_should_update_outplace(inode: *mut inode, fio: *mut f2fs_io_info) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn need_inplace_update(fio: *mut f2fs_io_info) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_do_write_data_page(fio: *mut f2fs_io_info) -> c_int {
...
// Use COW inode to make dnode_of_data for atomic write
    ...
    if (atomic_commit)
    set_new_dnode(&dn, F2FS_I(inode).cow_inode, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    else
    set_new_dnode(&dn, inode, core::ptr::null_mut(), core::ptr::null_mut(), 0);
...
// LFS mode write path
    ...
    >> PAGE_SHIFT;
    let mut psize: loff_t = (loff_t)(folio.index + 1) << PAGE_SHIFT;
    let mut offset: unsigned = 0;
    let mut need_balance_fs: bool = false;
    let mut quota_inode: bool = IS_NOQUOTA(inode);
    let mut err: c_int = 0;
    struct f2fs_io_info fio = {
    .sbi = sbi,
    .ino = inode.i_ino,
    ...
    .folio = folio,
    .encrypted_page = core::ptr::null_mut(),
    .submitted = 0,
...
//
    ...
    f2fs_submit_merged_write(sbi, DATA);
    if (bio && *bio)
    f2fs_submit_merged_ipu_write(sbi, bio, core::ptr::null_mut());
    submitted = core::ptr::null_mut();
    }
    if (submitted)
...
//
    ...
    int nr_folios, p, idx;
    int nr_pages;
    let mut max_pages: c_uint = F2FS_ONSTACK_PAGES;
    pgoff_t index;
...
#[no_mangle]
pub unsafe extern "C" fn account_writeback(inode: *mut inode, inc: bool) {
    ...
    struct writeback_control *wbc)
    {
    let mut skipped: c_long = wbc.pages_skipped;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: atomic_read(&sbi->wb_sync_req[DATA])) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_write_failed(inode: *mut inode, to: loff_t) {
...
// In the fs-verity case, f2fs_end_enable_verity() does the truncate
    ...
    struct folio *ifolio;
    let mut locked: bool = false;
    let mut flag: c_int = F2FS_GET_BLOCK_PRE_AIO;
    let mut err: c_int = 0;
...
// node_changed = dn.node_changed;
    ...
    let mut index: pgoff_t = folio.index;
    let mut err: c_int = 0;
    let mut ori_blk_addr: block_t = NULL_ADDR;
    let mut cow_has_reserved_block: bool = false;
...
// blk_addr = ori_blk_addr;
    ...
    let mut blkaddr: block_t = NULL_ADDR;
    let mut err: c_int = 0;
...
//
    ...
    core::ptr::null_mut(), /* can't write to fsverity files */
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_invalidate_folio(folio: *mut folio, offset: usize, length: usize) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_release_folio(folio: *mut folio, wait: gfp_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn f2fs_bmap_compress(inode: *mut inode, block: sector_t) -> sector_t {
...
#[no_mangle]
unsafe extern "C" fn f2fs_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
...
// Block number less than F2FS MAX BLOCKS
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    unsigned int blkofs;
    let mut blk_per_sec: c_uint = BLKS_PER_SEC(sbi);
    let mut end_blk: c_uint = start_blk + blkcnt - 1;
    let mut secidx: c_uint = start_blk / blk_per_sec;
    ...
    struct folio *folio;
    let mut blkidx: c_uint = secidx * blk_per_sec + blkofs;
    ...
    block_t last_lblock;
    block_t pblock;
    let mut lowest_pblock: block_t = -1;
    let mut highest_pblock: block_t = 0;
    let mut nr_extents: c_int = 0;
    unsigned int nr_pblocks;
    let mut blks_per_sec: c_uint = BLKS_PER_SEC(sbi);
    let mut not_aligned: c_uint = 0;
    let mut ret: c_int = 0;
//
...
//
    ...
    struct f2fs_map_blocks map;
    let mut last_extent: bool = false;
    retry:
    cond_resched();
    memset(&map, 0, sizeof(map));
...
#[no_mangle]
unsafe extern "C" fn f2fs_swap_deactivate(file: *mut file) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_clear_page_cache_dirty_tag(folio: *mut folio) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_init_post_read_processing() -> int __init {
    int __init f2fs_init_post_read_processing(void)
    {
    bio_post_read_ctx_cache =
    kmem_cache_create("f2fs_bio_post_read_ctx",
    sizeof(struct bio_post_read_ctx), 0, 0, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_destroy_post_read_processing() {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_init_wq(sbi: *mut f2fs_sb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_destroy_wq(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_init_bio_entry_cache() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_destroy_bio_entry_cache() {
    ...
    struct iomap *srcmap)
    {
    let mut map: f2fs_map_blocks = { core::ptr::null_mut(), };
    let mut next_pgofs: pgoff_t = 0;
    int err;
    ...
    iomap.private = core::ptr::null_mut();
    }
//
