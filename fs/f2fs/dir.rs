...
#[no_mangle]
pub unsafe extern "C" fn f2fs_should_fallback_to_linear(dir: *mut inode) -> bool {
...
#[no_mangle]
unsafe extern "C" fn dir_blocks(inode: *mut inode) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn dir_buckets(level: c_uint, dir_level: c_int) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn bucket_blocks(level: c_uint) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_free_casefolded_name(fname: *mut f2fs_filename) {
    void f2fs_free_casefolded_name(struct f2fs_filename *fname)
    {
    unsigned char *buf = (unsigned char *)fname.cf_name.name;
    if (buf) {
    kmem_cache_free(f2fs_cf_name_slab, buf);
    fname.cf_name.name = core::ptr::null_mut();
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_free_filename(fname: *mut f2fs_filename) {
...
// max_slots = max_len;
    ...
    bool use_hash)
    {
    let mut s: c_int = GET_DENTRY_SLOTS(fname.disk_name.len);
    unsigned int nbucket, nblock;
    unsigned int bidx, end_block, bucket_no;
    struct f2fs_dir_entry *de = core::ptr::null_mut();
    pgoff_t next_pgofs;
    let mut room: bool = false;
    int max_slots;
...
// res_folio = dentry_folio;
    ...
    struct folio **res_folio)
    {
    let mut npages: c_ulong = dir_blocks(dir);
    struct f2fs_dir_entry *de = core::ptr::null_mut();
    unsigned int max_depth;
    unsigned int level;
    let mut use_hash: bool = true;
// res_folio = NULL;
...
// res_folio = ERR_PTR(err);
    ...
    {
    let mut type: enum page_type = f2fs_has_inline_dentry(dir) ? NODE : DATA;
    folio_lock(folio);
...
//
    ...
    struct f2fs_dentry_ptr *d)
    {
    let mut dot: fscrypt_str = FSTR_INIT(".", 1);
    let mut dotdot: fscrypt_str = FSTR_INIT("..", 2);
// update dirent of "."
...
// update dirent of ".."
    ...
    dentry_blk = folio_address(dentry_folio);
    make_dentry_ptr_block(core::ptr::null_mut(), &d, dentry_blk);
...
// in order to handle error case
    ...
    fname ? fname.usr_fname : core::ptr::null_mut(),
    folio);
    if (err)
    goto put_error;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_room_for_filename(bitmap: *const c_void, slots: c_int, max_slots: c_int) -> c_int {
    int f2fs_room_for_filename(const void *bitmap, int slots, int max_slots)
    {
    let mut bit_start: c_int = 0;
    ...
    let mut slots: c_int = GET_DENTRY_SLOTS(fname.disk_name.len);
    ...
    {
    struct f2fs_dir_entry *de;
    let mut slots: c_int = GET_DENTRY_SLOTS(name.len);
    int i;
    de = &d.dentry[bit_pos];
...
// avoid wrong garbage data for readdir
    ...
    unsigned int nbucket, nblock;
    struct folio *dentry_folio = core::ptr::null_mut();
    struct f2fs_dentry_block *dentry_blk = core::ptr::null_mut();
    struct f2fs_dentry_ptr d;
...
// Increase the depth, if required
    ...
    dentry_folio = f2fs_get_new_data_folio(dir, core::ptr::null_mut(), block, true);
    if (IS_ERR(dentry_folio))
    return PTR_ERR(dentry_folio);
    dentry_blk = folio_address(dentry_folio);
...
// Move to next level to find the empty slot for new dentry
    ...
    folio = f2fs_init_inode_metadata(inode, dir, fname, core::ptr::null_mut());
    if (IS_ERR(folio)) {
    err = PTR_ERR(folio);
    goto fail;
    }
...
//
    ...
    F2FS_I(dir).task = core::ptr::null_mut();
    }
    if (de) {
    f2fs_folio_put(folio, false);
    err = -EEXIST;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_drop_nlink(dir: *mut inode, inode: *mut inode) {
...
//
    ...
    struct f2fs_dentry_block *dentry_blk;
    unsigned int bit_pos;
    let mut slots: c_int = GET_DENTRY_SLOTS(le16_to_cpu(dentry.name_len));
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_empty_dir(dir: *mut inode) -> bool {
    bool f2fs_empty_dir(struct inode *dir)
    {
    let mut bidx: c_ulong = 0;
    unsigned int bit_pos;
    struct f2fs_dentry_block *dentry_blk;
    let mut nblock: c_ulong = dir_blocks(dir);
    if (f2fs_has_inline_dentry(dir))
    ...
    unsigned int bit_pos;
    struct f2fs_dir_entry *de = core::ptr::null_mut();
    let mut de_name: fscrypt_str = FSTR_INIT(core::ptr::null_mut(), 0);
    struct f2fs_sb_info *sbi = F2FS_I_SB(d.inode);
    struct blk_plug plug;
    let mut readdir_ra: bool = sbi.readdir_ra;
    let mut found_valid_dirent: bool = false;
    let mut err: c_int = 0;
...
// check memory boundary before moving forward
    ...
    let mut save_len: c_int = fstr.len;
...
#[no_mangle]
unsafe extern "C" fn f2fs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    static int f2fs_readdir(struct file *file, struct dir_context *ctx)
    {
    struct inode *inode = file_inode(file);
    let mut npages: c_ulong = dir_blocks(inode);
    struct f2fs_dentry_block *dentry_blk = core::ptr::null_mut();
    struct file_ra_state *ra = &file.f_ra;
    let mut start_pos: loff_t = ctx.pos;
    let mut n: c_uint = ((unsigned long)ctx.pos / NR_DENTRY_IN_BLOCK);
    struct f2fs_dentry_ptr d;
    let mut fstr: fscrypt_str = FSTR_INIT(core::ptr::null_mut(), 0);
    let mut err: c_int = 0;
    if (IS_ENCRYPTED(inode)) {
