...
//
    ...
    {
    let mut slen: usize = strlen(s);
    let mut sublen: usize = strlen(sub);
    int i;
    if (sublen == 1 && *sub == '*')
    return true;
//
...
#[no_mangle]
pub unsafe extern "C" fn is_temperature_extension(s: *const c_uchar, sub: *const c_char) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn is_compress_extension(s: *const c_uchar, sub: *const c_char) -> bool {
    ...
    let mut cold_count: c_int = le32_to_cpu(sbi.raw_super.extension_count);
    let mut hot_count: c_int = sbi.raw_super.hot_ext_count;
    let mut total_count: c_int = cold_count + hot_count;
    int start, count;
    int i;
    if (set) {
    ...
    let mut ext_cnt: c_uchar = F2FS_OPTION(sbi).compress_ext_cnt;
    let mut noext_cnt: c_uchar = F2FS_OPTION(sbi).nocompress_ext_cnt;
...
//
    ...
    nid_t ino;
    struct inode *inode;
    let mut nid_free: bool = false;
    let mut encrypt: bool = false;
    let mut xattr_size: c_int = 0;
    int err;
...
// Should enable inline_data after compression set
    ...
    {
    struct folio *folio;
    let mut ino: c_ulong = f2fs_inode_by_name(d_inode(child), &dotdot_name, &folio);
    if (!ino) {
    if (IS_ERR(folio))
...
#[no_mangle]
unsafe extern "C" fn f2fs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
// this is broken symlink case
    ...
    struct f2fs_lock_context lc;
    struct inode *inode;
    let mut len: usize = strlen(symname);
    struct fscrypt_str disk_link;
    let mut orphan_free: bool = true;
    ...
    if (err)
    return err;
    inode = f2fs_new_inode(idmap, dir, S_IFLNK | S_IRWXUGO, core::ptr::null_mut());
    if (IS_ERR(inode))
    return PTR_ERR(inode);
...
// Give up and leave a broken symlink.
    ...
    if (err)
    return ERR_PTR(err);
    inode = f2fs_new_inode(idmap, dir, mode, core::ptr::null_mut());
    if (IS_ERR(inode))
...
#[no_mangle]
unsafe extern "C" fn f2fs_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
// new_inode = inode;
    ...
    if (!f2fs_is_checkpoint_ready(sbi))
    return -ENOSPC;
    err = __f2fs_tmpfile(idmap, dir, file, mode, false, core::ptr::null_mut(), core::ptr::null_mut());
    ...
    struct f2fs_filename *fname)
    {
    return __f2fs_tmpfile(idmap, dir, core::ptr::null_mut(), S_IFCHR | WHITEOUT_MODE,
    true, whiteout, fname);
    }
    int f2fs_get_tmpfile(struct mnt_idmap *idmap, struct inode *dir,
    struct inode **new_inode)
    {
    return __f2fs_tmpfile(idmap, dir, core::ptr::null_mut(), S_IFREG,
    false, new_inode, core::ptr::null_mut());
    }
    ...
    struct folio *old_dir_folio = core::ptr::null_mut();
    struct folio *old_folio, *new_folio = core::ptr::null_mut();
    struct f2fs_dir_entry *old_dir_entry = core::ptr::null_mut();
    struct f2fs_dir_entry *old_entry;
    struct f2fs_dir_entry *new_entry;
    struct f2fs_lock_context lc;
    let mut old_is_dir: bool = S_ISDIR(old_inode.i_mode);
    int err;
...
// adjust dir's i_pino to pass fsck check
    ...
    f2fs_delete_entry(old_entry, old_folio, old_dir, core::ptr::null_mut());
    old_folio = core::ptr::null_mut();
    if (whiteout) {
    ...
    struct f2fs_dir_entry *old_dir_entry = core::ptr::null_mut(), *new_dir_entry = core::ptr::null_mut();
    struct f2fs_dir_entry *old_entry, *new_entry;
    struct f2fs_lock_context lc;
    let mut old_nlink: c_int = 0, new_nlink = 0;
    int err;
