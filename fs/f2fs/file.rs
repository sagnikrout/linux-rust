...
#[no_mangle]
unsafe extern "C" fn do_zero_post_eof_page(inode: *mut inode, new_size: loff_t) -> c_int {
    static int do_zero_post_eof_page(struct inode *inode, loff_t new_size)
    {
    let mut old_size: loff_t = i_size_read(inode);
...
#[no_mangle]
unsafe extern "C" fn f2fs_filemap_fault(vmf: *mut vm_fault) -> vm_fault_t {
    static vm_fault_t f2fs_filemap_fault(struct vm_fault *vmf)
    {
    struct inode *inode = file_inode(vmf.vma.vm_file);
    let mut flags: vm_flags_t = vmf.vma.vm_flags;
    vm_fault_t ret;
...
#[no_mangle]
unsafe extern "C" fn f2fs_vm_page_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
    ...
    let mut need_alloc: bool = !f2fs_is_pinned_file(inode);
    let mut err: c_int = 0;
    vm_fault_t ret;
//
...
//
    ...
    let mut ret: c_int = f2fs_is_compressed_cluster(inode, folio.index);
    if (ret < 0) {
    err = ret;
    goto out;
    } else if (ret) {
...
// should do out of any locked page
    ...
    set_new_dnode(&dn, inode, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (need_alloc) {
// block allocation
...
#[no_mangle]
unsafe extern "C" fn get_parent_ino(inode: *mut inode, pino: *mut nid_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn need_do_checkpoint(inode: *mut inode) -> enum cp_reason_type {
    static inline enum cp_reason_type need_do_checkpoint(struct inode *inode)
    {
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    let mut cp_reason: enum cp_reason_type = CP_NO_NEEDED;
    if (!S_ISREG(inode.i_mode))
    cp_reason = CP_NON_REGULAR;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: f2fs_compressed_file(inode)) -> else {
    else if (f2fs_compressed_file(inode))
    cp_reason = CP_COMPRESSED;
#[no_mangle]
pub unsafe extern "C" fn if(1: inode->i_nlink !=) -> else {
    else if (inode.i_nlink != 1)
    cp_reason = CP_HARDLINK;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_sbi_flag_set(sbi, _arg: SBI_NEED_CP)) -> else {
    else if (is_sbi_flag_set(sbi, SBI_NEED_CP))
    cp_reason = CP_SB_NEED_CP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: file_wrong_pino(inode)) -> else {
    else if (file_wrong_pino(inode))
    cp_reason = CP_WRONG_PINO;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !f2fs_space_for_roll_forward(sbi)) -> else {
    else if (!f2fs_space_for_roll_forward(sbi))
    cp_reason = CP_NO_SPC_ROLL;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !f2fs_is_checkpointed_node(sbi, _arg: F2FS_I(inode)->i_pino)) -> else {
    else if (!f2fs_is_checkpointed_node(sbi, F2FS_I(inode).i_pino))
    cp_reason = CP_NODE_NEED_CP;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_opt(sbi, _arg: FASTBOOT)) -> else {
    else if (test_opt(sbi, FASTBOOT))
    cp_reason = CP_FASTBOOT_MODE;
#[no_mangle]
pub unsafe extern "C" fn if(2: F2FS_OPTION(sbi).active_logs ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn need_inode_page_update(sbi: *mut f2fs_sb_info, ino: nid_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn try_to_fix_pino(inode: *mut inode) {
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    let mut ino: nid_t = inode.i_ino;
    let mut ret: c_int = 0;
    let mut cp_reason: enum cp_reason_type = 0;
    struct writeback_control wbc = {
    .sync_mode = WB_SYNC_ALL,
    .nr_to_write = LONG_MAX,
    };
    let mut seq_id: c_uint = 0;
...
// if the inode is dirty, let's recover all the time
    ...
    f2fs_write_inode(inode, core::ptr::null_mut());
    goto go_write;
    }
//
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_sync_file(file: *mut file, start: loff_t, end: loff_t, datasync: c_int) -> c_int {
    ...
    {
    let mut blkaddr: block_t = f2fs_data_blkaddr(dn);
    struct inode *inode = mapping.host;
    let mut compressed_cluster: bool = false;
    if (f2fs_compressed_file(inode)) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_seek_block(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
    static loff_t f2fs_seek_block(struct file *file, loff_t offset, int whence)
    {
    struct inode *inode = file.f_mapping.host;
    let mut maxbytes: loff_t = F2FS_BLK_TO_BYTES(max_file_blocks(inode));
    struct dnode_of_data dn;
    pgoff_t pgofs, end_offset;
    let mut data_ofs: loff_t = offset;
    loff_t isize;
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn f2fs_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
...
#[no_mangle]
unsafe extern "C" fn f2fs_file_mmap_prepare(desc: *mut vm_area_desc) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn finish_preallocate_blocks(inode: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_file_open(inode: *mut inode, filp: *mut file) -> c_int {
    static int f2fs_file_open(struct inode *inode, struct file *filp)
    {
    let mut err: c_int = fscrypt_file_open(inode, filp);
    if (err)
    return err;
    if (!f2fs_is_compress_backend_ready(inode))
    return -EOPNOTSUPP;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_truncate_data_blocks_range(dn: *mut dnode_of_data, count: c_int) {
    void f2fs_truncate_data_blocks_range(struct dnode_of_data *dn, int count)
    {
    struct f2fs_sb_info *sbi = F2FS_I_SB(dn.inode);
    let mut nr_free: c_int = 0, ofs = dn.ofs_in_node, len = count;
    __le32 *addr;
    let mut compressed_cluster: bool = false;
    let mut cluster_index: c_int = 0, valid_blocks = 0;
    let mut cluster_size: c_int = F2FS_I(dn.inode).i_cluster_size;
    let mut released: bool = !atomic_read(&F2FS_I(dn.inode).i_compr_blocks);
    block_t blkstart;
    let mut blklen: c_int = 0;
    addr = get_dnode_addr(dn.inode, dn.node_folio) + ofs;
    blkstart = le32_to_cpu(*addr);
// Assumption: truncation starts with cluster
    for (; count > 0; count--, addr++, dn.ofs_in_node++, cluster_index++) {
    let mut blkaddr: block_t = le32_to_cpu(*addr);
...
//
    ...
    bool cache_only)
    {
    let mut offset: loff_t = from & (PAGE_SIZE - 1);
    let mut index: pgoff_t = from >> PAGE_SHIFT;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_do_truncate_blocks(inode: *mut inode, from: u64, lock: bool) -> c_int {
    ...
    struct folio *ifolio;
    let mut truncate_page: bool = false;
    ...
    set_new_dnode(&dn, inode, ifolio, core::ptr::null_mut(), 0);
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_truncate_blocks(inode: *mut inode, from: u64, lock: bool) -> c_int {
    int f2fs_truncate_blocks(struct inode *inode, u64 from, bool lock)
    {
    let mut free_from: u64 = from;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_truncate(inode: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_force_buffered_io(inode: *mut inode, rw: c_int) -> bool {
...
//
    ...
    struct f2fs_inode_info *fi = F2FS_I(inode);
    struct f2fs_inode *ri = core::ptr::null_mut();
    unsigned int flags;
...
//
    ...
    let mut bsize: c_uint = i_blocksize(inode);
...
// we need to show initial sectors used for inline_data/dentries
    ...
    {
    let mut ia_valid: c_uint = attr.ia_valid;
    ...
    let mut mode: umode_t = attr.ia_mode;
...
// inode change will produce dirty node pages flushed by checkpoint
    ...
    f2fs_balance_fs(sbi, true);
    f2fs_lock_op(sbi, &lc);
    folio = f2fs_get_new_data_folio(inode, core::ptr::null_mut(), index, false);
    f2fs_unlock_op(sbi, &lc);
    if (IS_ERR(folio))
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_truncate_hole(inode: *mut inode, pg_start: pgoff_t, pg_end: pgoff_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_punch_hole(inode: *mut inode, offset: loff_t, len: loff_t) -> c_int {
...
// do_replace = 1;
    ...
    size_t new_size;
    pgoff_t ilen;
    set_new_dnode(&dn, dst_inode, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    ...
    if (IS_ERR(fsrc))
    return PTR_ERR(fsrc);
    fdst = f2fs_get_new_data_folio(dst_inode, core::ptr::null_mut(), dst + i,
    true);
    if (IS_ERR(fdst)) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_do_collapse(inode: *mut inode, offset: loff_t, len: loff_t) -> c_int {
    static int f2fs_do_collapse(struct inode *inode, loff_t offset, loff_t len)
    {
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct f2fs_lock_context lc;
    let mut nrpages: pgoff_t = DIV_ROUND_UP(i_size_read(inode), PAGE_SIZE);
    let mut start: pgoff_t = offset >> PAGE_SHIFT;
    let mut end: pgoff_t = (offset + len) >> PAGE_SHIFT;
    int ret;
    f2fs_balance_fs(sbi, true);
...
#[no_mangle]
unsafe extern "C" fn f2fs_collapse_range(inode: *mut inode, offset: loff_t, len: loff_t) -> c_int {
...
// write out all moved pages, if possible
    ...
    let mut ofs_in_node: c_uint = dn.ofs_in_node;
    let mut count: blkcnt_t = 0;
    int ret;
...
//
    ...
    let mut new_size: loff_t = i_size_read(inode);
    loff_t off_start, off_end;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn f2fs_insert_range(inode: *mut inode, offset: loff_t, len: loff_t) -> c_int {
...
// write out all moved pages, if possible
    ...
    {
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct f2fs_map_blocks map = { .m_next_pgofs = core::ptr::null_mut(),
    .m_next_extent = core::ptr::null_mut(), .m_seg_type = NO_CHECK_TYPE,
    .m_may_create = true };
    ...
    return 0;
    if (f2fs_is_pinned_file(inode)) {
    let mut sec_blks: block_t = CAP_BLKS_PER_SEC(sbi);
    block_t sec_len;
...
#[no_mangle]
unsafe extern "C" fn f2fs_release_file(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_file_flush(file: *mut file, id: fl_owner_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_setflags_common(inode: *mut inode, iflags: u32, mask: u32) -> c_int {
    static int f2fs_setflags_common(struct inode *inode, u32 iflags, u32 mask)
    {
    struct f2fs_inode_info *fi = F2FS_I(inode);
    let mut masked_flags: u32 = fi.i_flags & mask;
...
// try to convert inline_data to support compression
    let mut err: c_int = f2fs_convert_inline_inode(inode);
    if (err)
    return err;
    f2fs_down_write(&fi.i_sem);
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_iflags_to_fsflags(iflags: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_fsflags_to_iflags(fsflags: u32) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_getversion(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_start_atomic_write(filp: *mut file, truncate: bool) -> c_int {
...
// Check if the inode already has a COW inode
    if (fi.cow_inode == core::ptr::null_mut()) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_commit_atomic_write(filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_abort_atomic_write(filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_shutdown(filp: *mut file, arg: c_ulong) -> c_int {
    ...
    __u32 in;
    int ret;
    let mut need_drop: bool = false, readonly = false;
...
// fallback to nosync shutdown for readonly fs
    ...
    {
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    let mut max_bytes: u64 = F2FS_BLK_TO_BYTES(max_file_blocks(inode));
    u64 start, end;
    let mut ret: c_int = 0;
    if (!S_ISREG(inode.i_mode))
    return 0;
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_fitrim(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn uuid_is_nonzero(u[16]: __u8) -> bool {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_set_encryption_policy(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_encryption_policy(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_dev_alias_status(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_encryption_pwsalt(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_add_encryption_key(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_remove_encryption_key(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_encryption_nonce(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_gc(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __f2fs_ioc_gc_range(filp: *mut file, range: *mut f2fs_gc_range) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_gc_range(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_write_checkpoint(filp: *mut file) -> c_int {
    ...
    {
    struct inode *inode = file_inode(filp);
    struct f2fs_map_blocks map = { .m_next_extent = core::ptr::null_mut(),
    .m_seg_type = NO_CHECK_TYPE,
    .m_may_create = false };
    let mut ei: extent_info = {};
    pgoff_t pg_start, pg_end, next_pgofs;
    let mut total: c_uint = 0, sec_num;
    let mut blk_end: block_t = 0;
    let mut fragmented: bool = false;
    int err;
    f2fs_balance_fs(sbi, true);
    inode_lock(inode);
...
// if in-place-update policy is enabled, don't waste time here
    ...
    if (f2fs_should_update_inplace(inode, core::ptr::null_mut())) {
    err = -EINVAL;
    goto out;
    }
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_defragment(filp: *mut file, arg: c_ulong) -> c_int {
...
// verify alignment of offset & size
    ...
    struct f2fs_lock_context lc;
    let mut olen: usize = len;
    let mut dst_max_i_size: loff_t = 0;
...
#[no_mangle]
pub unsafe extern "C" fn if(dst->i_size: dst_osize !=) -> else {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_move_range(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_flush_device(filp: *mut file, arg: c_ulong) -> c_int {
    static int f2fs_ioc_flush_device(struct file *filp, unsigned long arg)
    {
    struct inode *inode = file_inode(filp);
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct sit_info *sm = SIT_I(sbi);
    let mut start_segno: c_uint = 0, end_segno = 0;
    let mut dev_start_segno: c_uint = 0, dev_end_segno = 0;
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_features(filp: *mut file, arg: c_ulong) -> c_int {
    static int f2fs_ioc_get_features(struct file *filp, unsigned long arg)
    {
    struct inode *inode = file_inode(filp);
    let mut sb_feature: u32 = le32_to_cpu(F2FS_I_SB(inode).raw_super.feature);
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_transfer_project_quota(inode: *mut inode, kprojid: kprojid_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_setproject(inode: *mut inode, projid: __u32) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_fileattr_get(dentry: *mut dentry, fa: *mut file_kattr) -> c_int {
    int f2fs_fileattr_get(struct dentry *dentry, struct file_kattr *fa)
    {
    struct inode *inode = d_inode(dentry);
    struct f2fs_inode_info *fi = F2FS_I(inode);
    let mut fsflags: u32 = f2fs_iflags_to_fsflags(fi.i_flags);
    if (IS_ENCRYPTED(inode))
    ...
    {
    struct inode *inode = d_inode(dentry);
    let mut fsflags: u32 = fa.flags, mask = F2FS_SETTABLE_FS_FL;
    u32 iflags;
    int err;
    if (unlikely(f2fs_cp_error(F2FS_I_SB(inode))))
    return -EIO;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_pin_file_control(inode: *mut inode, inc: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_set_pin_file(filp: *mut file, arg: c_ulong) -> c_int {
...
// Let's allow file pinning on zoned device.
    ...
    f2fs_should_update_outplace(inode, core::ptr::null_mut())) {
    ret = -EINVAL;
    goto out;
    }
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_pin_file(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_dev_alias_file(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_reserve_dev_alias(filp: *mut file) -> c_int {
    ...
    struct extent_tree *et = F2FS_I(inode).extent_tree[EX_READ];
    struct extent_info ei;
    let mut cpc: cp_control = { CP_SYNC, 0, 0, 0 };
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_release_dev_alias(filp: *mut file) -> c_int {
    static int f2fs_ioc_release_dev_alias(struct file *filp)
    {
    struct inode *inode = file_inode(filp);
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct extent_tree *et = F2FS_I(inode).extent_tree[EX_READ];
    let mut ei: extent_info = {0, };
    let mut cpc: cp_control = { CP_SYNC, 0, 0, 0 };
    struct f2fs_lock_context lc, glc;
    int err;
    if (!capable(CAP_SYS_ADMIN))
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_io_prio(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_precache_extents(inode: *mut inode) -> c_int {
    ...
    map.m_lblk = 0;
    map.m_pblk = 0;
    map.m_next_pgofs = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_precache_extents(filp: *mut file) -> c_int {
    static int f2fs_ioc_precache_extents(struct file *filp)
    {
    return f2fs_precache_extents(file_inode(filp));
    }
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_resize_fs(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_enable_verity(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_measure_verity(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_read_verity_metadata(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_getfslabel(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_setfslabel(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_get_compress_blocks(inode: *mut inode, blocks: *mut __u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_compress_blocks(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn release_compress_blocks(dn: *mut dnode_of_data, count: pgoff_t) -> c_int {
    static int release_compress_blocks(struct dnode_of_data *dn, pgoff_t count)
    {
    struct f2fs_sb_info *sbi = F2FS_I_SB(dn.inode);
    let mut released_blocks: c_uint = 0;
    let mut cluster_size: c_int = F2FS_I(dn.inode).i_cluster_size;
    block_t blkaddr;
    int i;
    for (i = 0; i < count; i++) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_release_compress_blocks(filp: *mut file, arg: c_ulong) -> c_int {
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct f2fs_lock_context lc;
    let mut page_idx: pgoff_t = 0, last_idx;
    let mut released_blocks: c_uint = 0;
    int ret;
    int writecount;
...
#[no_mangle]
unsafe extern "C" fn f2fs_reserve_compress_blocks(filp: *mut file, arg: c_ulong) -> c_int {
    ...
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    let mut page_idx: pgoff_t = 0, last_idx;
    let mut reserved_blocks: c_uint = 0;
    int ret;
    if (!f2fs_sb_has_compression(sbi))
    return -EOPNOTSUPP;
    if (f2fs_readonly(sbi.sb))
    ...
    {
    let mut sector: sector_t = SECTOR_FROM_BLOCK(block);
    let mut nr_sects: sector_t = SECTOR_FROM_BLOCK(len);
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn f2fs_sec_trim_file(filp: *mut file, arg: c_ulong) -> c_int {
    static int f2fs_sec_trim_file(struct file *filp, unsigned long arg)
    {
    struct inode *inode = file_inode(filp);
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct address_space *mapping = inode.i_mapping;
    struct block_device *prev_bdev = core::ptr::null_mut();
    struct f2fs_sectrim_range range;
    pgoff_t index, pg_end, prev_index = 0;
    let mut prev_block: block_t = 0, len = 0;
    loff_t end_addr;
    let mut to_end: bool = false;
    let mut ret: c_int = 0;
    if (!(filp.f_mode & FMODE_WRITE))
    return -EBADF;
    ...
    cur_bdev = f2fs_target_device(sbi, blkaddr, core::ptr::null_mut());
    if (f2fs_is_multi_device(sbi)) {
    let mut di: c_int = f2fs_target_device_index(sbi, blkaddr);
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_get_compress_option(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_set_compress_option(filp: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn redirty_blocks(inode: *mut inode, page_idx: pgoff_t, len: c_int) -> c_int {
    static int redirty_blocks(struct inode *inode, pgoff_t page_idx, int len)
    {
    DEFINE_READAHEAD(ractl, core::ptr::null_mut(), core::ptr::null_mut(), inode.i_mapping, page_idx);
    struct address_space *mapping = inode.i_mapping;
    struct folio *folio;
    let mut redirty_idx: pgoff_t = page_idx;
    let mut page_len: c_int = 0, ret = 0;
    filemap_invalidate_lock_shared(mapping);
    page_cache_ra_unbounded(&ractl, len, 0);
    filemap_invalidate_unlock_shared(mapping);
    do {
    folio = read_cache_folio(mapping, page_idx, core::ptr::null_mut(), core::ptr::null_mut());
    if (IS_ERR(folio)) {
    ret = PTR_ERR(folio);
    break;
    }
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_decompress_file(filp: *mut file) -> c_int {
    static int f2fs_ioc_decompress_file(struct file *filp)
    {
    struct inode *inode = file_inode(filp);
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    struct f2fs_inode_info *fi = F2FS_I(inode);
    let mut page_idx: pgoff_t = 0, last_idx, cluster_idx;
    int ret;
    if (!f2fs_sb_has_compression(sbi) ||
...
#[no_mangle]
unsafe extern "C" fn f2fs_ioc_compress_file(filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __f2fs_ioctl(filp: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_ioctl(filp: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
...
#[no_mangle]
unsafe extern "C" fn f2fs_dio_end_bio(bio: *mut bio) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_dio_iostat_start(sbi: *mut f2fs_sb_info, bio: *mut bio) {
    ...
    if (error)
    return error;
    f2fs_update_iostat(sbi, core::ptr::null_mut(), APP_DIRECT_READ_IO, size);
    return 0;
    }
...
#[no_mangle]
unsafe extern "C" fn f2fs_dio_read_iter(iocb: *mut kiocb, to: *mut iov_iter) -> isize {
...
//
    ...
    &f2fs_iomap_dio_read_ops, 0, core::ptr::null_mut(), 0);
...
#[no_mangle]
unsafe extern "C" fn f2fs_file_read_iter(iocb: *mut kiocb, to: *mut iov_iter) -> isize {
...
#[no_mangle]
unsafe extern "C" fn f2fs_write_checks(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
// -ENOSPC|-EDQUOT are fine to report the number of allocated blocks.
    ...
    return error;
    f2fs_update_time(sbi, REQ_TIME);
    f2fs_update_iostat(sbi, core::ptr::null_mut(), APP_DIRECT_IO, size);
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn f2fs_valid_write_stream(sbi: *mut f2fs_sb_info, write_stream: u8) -> bool {
    ...
    let mut type: enum log_type = f2fs_rw_hint_to_seg_type(sbi, inode.i_write_hint);
    let mut temp: enum temp_type = f2fs_get_segment_temp(sbi, type);
    ...
    struct f2fs_inode_info *fi = F2FS_I(inode);
    struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
    let mut do_opu: bool = f2fs_lfs_mode(sbi);
    let mut pos: loff_t = iocb.ki_pos;
    let mut count: isize = iov_iter_count(from);
...
//
    ...
    ssize_t ret2;
    let mut bufio_start_pos: loff_t = iocb.ki_pos;
//
...
//
    if (ret2 > 0) {
    let mut bufio_end_pos: loff_t = bufio_start_pos + ret2 - 1;
    ret += ret2;
...
#[no_mangle]
unsafe extern "C" fn f2fs_file_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    static ssize_t f2fs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
    {
    struct inode *inode = file_inode(iocb.ki_filp);
    let mut orig_pos: loff_t = iocb.ki_pos;
    let mut orig_count: usize = iov_iter_count(from);
    loff_t target_size;
    bool dio;
    let mut may_need_sync: bool = true;
    int preallocated;
    ssize_t ret;
...
#[no_mangle]
pub unsafe extern "C" fn if(POSIX_FADV_NOREUSE: advice ==) -> else {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_f2fs_gc_range {
    pub sync: u32,
    pub start: compat_u64,
    pub len: compat_u64,
}

    struct compat_f2fs_gc_range)
#[no_mangle]
unsafe extern "C" fn f2fs_compat_ioc_gc_range(file: *mut file, arg: c_ulong) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_f2fs_move_range {
    pub dst_fd: u32,
    pub pos_in: compat_u64,
    pub pos_out: compat_u64,
    pub len: compat_u64,
}

    struct compat_f2fs_move_range)
#[no_mangle]
unsafe extern "C" fn f2fs_compat_ioc_move_range(file: *mut file, arg: c_ulong) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_compat_ioctl(file: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
