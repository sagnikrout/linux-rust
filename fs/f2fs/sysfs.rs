...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct f2fs_attr {
    pub attr: attribute,
    pub buf): *mut *mut *mut *mut ssize_t (show)(struct f2fs_attr a, struct f2fs_sb_info sbi, char,
    ssize_t (*store)(struct f2fs_attr *a, struct f2fs_sb_info *sbi,
    pub len): *const *const char buf, size_t,
    pub struct_type: c_int,
    pub offset: c_int,
    pub size: c_int,
    pub id: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct f2fs_base_attr {
    pub attr: attribute,
    pub buf): *mut *mut *mut ssize_t (show)(struct f2fs_base_attr a, char,
    pub len): *const *const *const *const ssize_t (store)(struct f2fs_base_attr a, char buf, size_t,
}
...
#[no_mangle]
pub unsafe extern "C" fn if(SM_INFO: struct_type ==) -> else {
    else if (struct_type == SM_INFO)
    return (unsigned char *)SM_I(sbi);
#[no_mangle]
pub unsafe extern "C" fn if(DCC_INFO: struct_type ==) -> else {
    else if (struct_type == DCC_INFO)
    return (unsigned char *)SM_I(sbi).dcc_info;
#[no_mangle]
pub unsafe extern "C" fn if(NM_INFO: struct_type ==) -> else {
    else if (struct_type == NM_INFO)
    return (unsigned char *)NM_I(sbi);
#[no_mangle]
pub unsafe extern "C" fn if(RESERVED_BLOCKS: struct_type == F2FS_SBI || struct_type ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(STAT_INFO: struct_type ==) -> else {
    else if (struct_type == STAT_INFO)
    return (unsigned char *)F2FS_STAT(sbi);

#[no_mangle]
pub unsafe extern "C" fn if(CPRC_INFO: struct_type ==) -> else {
    else if (struct_type == CPRC_INFO)
    return (unsigned char *)&sbi.cprc_info;
#[no_mangle]
pub unsafe extern "C" fn if(ATGC_INFO: struct_type ==) -> else {
    ...
    struct f2fs_sb_info *sbi, char *buf)
    {
    unsigned char *ptr = core::ptr::null_mut();
    ...
    let mut class: c_int = IOPRIO_PRIO_CLASS(cprc.ckpt_thread_ioprio);
    let mut level: c_int = IOPRIO_PRIO_LEVEL(cprc.ckpt_thread_ioprio);
    ...
    let mut current_write: i64 = atomic64_read(&sbi.current_atomic_write);
...
// (u64 *)ui = value;
    ...
    let mut set: bool = true, hot;
    if (!strncmp(name, "[h]", 3))
    hot = true;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(name, _arg: "[c]", _arg: 3)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !strncmp(name, _arg: "be, _arg: ", _arg: 3)) -> else {
...
#[no_mangle]
unsafe extern "C" fn f2fs_sb_release(kobj: *mut kobject) {
...
//
#[no_mangle]
unsafe extern "C" fn f2fs_feature_show(a: *mut f2fs_base_attr, buf: *mut c_char) -> isize {
...
#[no_mangle]
unsafe extern "C" fn f2fs_tune_show(a: *mut f2fs_base_attr, buf: *mut c_char) -> isize {
    ...
    f2fs_sbi_show, core::ptr::null_mut(),				\
    ...
    static struct f2fs_attr f2fs_attr_##name = __ATTR(name, 0444, name##_show, core::ptr::null_mut())
...
#[no_mangle]
unsafe extern "C" fn f2fs_stat_kobj_release(kobj: *mut kobject) {
...
#[no_mangle]
unsafe extern "C" fn f2fs_feature_list_kobj_release(kobj: *mut kobject) {
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_init_sysfs() -> int __init {
    ...
    core::ptr::null_mut(), "tuning");
    if (ret)
    goto put_feat;
    f2fs_proc_root = proc_mkdir("fs/f2fs", core::ptr::null_mut());
    if (!f2fs_proc_root) {
    ret = -ENOMEM;
    goto put_tune;
    }
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_exit_sysfs() {
    void f2fs_exit_sysfs(void)
    {
    kobject_put(&f2fs_tune);
    kobject_put(&f2fs_feat);
    kset_unregister(&f2fs_kset);
    remove_proc_entry("fs/f2fs", core::ptr::null_mut());
    f2fs_proc_root = core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn f2fs_register_sysfs(sbi: *mut f2fs_sb_info) -> c_int {
    ...
    err = kobject_init_and_add(&sbi.s_kobj, &f2fs_sb_ktype, core::ptr::null_mut(),
    "%s", sb.s_id);
    if (err)
    goto put_sb_kobj;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_unregister_sysfs(sbi: *mut f2fs_sb_info) {
