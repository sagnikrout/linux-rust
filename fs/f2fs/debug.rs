...
//
#[no_mangle]
pub unsafe extern "C" fn f2fs_update_sit_info(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
unsafe extern "C" fn update_multidevice_stats(sbi: *mut f2fs_sb_info) {
...
#[no_mangle]
pub unsafe extern "C" fn if(BLKS_PER_SEG(sbi): seg_blks ==) -> else {
    else if (seg_blks == BLKS_PER_SEG(sbi))
    dev_stats[i].devstats[0][DEVSTAT_FULL]++;
#[no_mangle]
pub unsafe extern "C" fn if(0: seg_blks !=) -> else {
    else if (seg_blks != 0)
    dev_stats[i].devstats[0][DEVSTAT_DIRTY]++;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !test_bit(j, _arg: FREE_I(sbi)->free_segmap)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(BLKS_PER_SEC(sbi): sec_blks ==) -> else {
    else if (sec_blks == BLKS_PER_SEC(sbi))
    dev_stats[i].devstats[1][DEVSTAT_FULL]++;
#[no_mangle]
pub unsafe extern "C" fn if(0: sec_blks !=) -> else {
    else if (sec_blks != 0)
...
#[no_mangle]
unsafe extern "C" fn update_general_status(sbi: *mut f2fs_sb_info) {
...
// 100 / (int)(sbi->user_block_count >> sbi->log_blocks_per_seg)
    ...
    let mut blks: c_int = get_seg_entry(sbi, i).valid_blocks;
    let mut type: c_int = get_seg_entry(sbi, i).type;
    if (!blks)
    continue;
...
//
#[no_mangle]
unsafe extern "C" fn update_mem_info(sbi: *mut f2fs_sb_info) {
...
// free nids
    ...
    si.page_mem = 0;
    if (sbi.node_inode) {
    let mut npages: c_ulong = NODE_MAPPING(sbi).nrpages;
    si.page_mem += (unsigned long long)npages << PAGE_SHIFT;
    }
    if (sbi.meta_inode) {
    let mut npages: c_ulong = META_MAPPING(sbi).nrpages;
    si.page_mem += (unsigned long long)npages << PAGE_SHIFT;
    }

    if (sbi.compress_inode) {
    let mut npages: c_ulong = COMPRESS_MAPPING(sbi).nrpages;
    si.page_mem += (unsigned long long)npages << PAGE_SHIFT;
    }
...
#[no_mangle]
unsafe extern "C" fn stat_show(s: *mut seq_file, v: *mut c_void) -> c_int {
    ...
    let mut policy: c_ulong = SM_I(sbi).ipu_policy;
...
#[no_mangle]
pub unsafe extern "C" fn f2fs_create_root_stats() -> void __init {
    void __init f2fs_create_root_stats(void)
    {

    f2fs_debugfs_root = debugfs_create_dir("f2fs", core::ptr::null_mut());
    debugfs_create_file("status", 0444, f2fs_debugfs_root, core::ptr::null_mut(),
    &stat_fops);
...
pub unsafe extern "C" fn f2fs_destroy_root_stats() {
    ...
    f2fs_debugfs_root = core::ptr::null_mut();
