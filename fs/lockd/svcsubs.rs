...
//
pub const FILE_HASH_BITS: c_int = 7;

    static struct hlist_head	nlm_files[FILE_NRHASH];
    static DEFINE_MUTEX(nlm_file_mutex);

#[no_mangle]
pub unsafe extern "C" fn nlm_debug_print_fh(msg: *mut c_char, f: *mut nfs_fh) {
...
#[no_mangle]
pub unsafe extern "C" fn nlm_debug_print_file(msg: *mut c_char, file: *mut nlm_file) {
...
#[no_mangle]
pub unsafe extern "C" fn file_hash(f: *mut nfs_fh) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn lock_to_openmode(lock: *mut file_lock) -> c_int {
...
//
    ...
    let mut nlmerr: __be32 = nlm__int__failed;
...
//
#[no_mangle]
unsafe extern "C" fn nlm_release_files(file: *mut nlm_file) {
...
#[no_mangle]
unsafe extern "C" fn nlm_unlock_files(file: *mut nlm_file, fl: *const file_lock) -> c_int {
    ...
    if (lock.c.flc_file && vfs_lock_file(lock.c.flc_file, F_SETLK, &lock, core::ptr::null_mut()))
    goto out_err;
...
#[no_mangle]
unsafe extern "C" fn nlm_file_release(file: *mut nlm_file) {
...
//
    ...
    hint.net = net;
    nlm_traverse_files(&hint, nlmsvc_mark_host, core::ptr::null_mut());
    }
//
...
//
    ...
    if (nlm_traverse_files(host, nlmsvc_same_host, core::ptr::null_mut())) {
    printk(KERN_WARNING
...
//
    nlm_traverse_files(core::ptr::null_mut(), nlmsvc_is_client, core::ptr::null_mut());
    }
    static int
    nlmsvc_match_sb(void *datap, struct nlm_file *file)
    {
    struct super_block *sb = datap;
    let mut sb: return = = nlmsvc_file_inode(file).i_sb;
    }
//
...
//
    ...
    {
    int ret;
    ret = nlm_traverse_files(server_addr, nlmsvc_match_ip, core::ptr::null_mut());
    return ret ? -EIO : 0;
    }
