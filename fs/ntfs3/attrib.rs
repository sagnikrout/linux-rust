...
pub const NTFS_MIN_LOG2_OF_CLUMP: c_int = 16;

pub const NTFS_MAX_LOG2_OF_CLUMP: c_int = 26;
...
#[no_mangle]
pub unsafe extern "C" fn get_pre_allocated(size: u64) -> u64 {
...
//
    ...
    let mut evcn: CLST = le64_to_cpu(attr.nres.evcn);
    u32 asize;
    u16 run_off;
...
// Undo last 'ntfs_look_for_free_space'
    ...
    let mut shift: u8 = sbi.cluster_bits - SECTOR_SHIFT;
...
// Undo 'ntfs_look_for_free_space'
    if (vcn - vcn0) {
    run_deallocate_ex(sbi, run, vcn0, vcn - vcn0, core::ptr::null_mut(), false,
    run_da);
    run_truncate(run, vcn0);
    }
    return err;
...
// len - how many clusters required to store 'rsize' bytes
    ...
    let mut shift: u8 = sbi.cluster_bits + NTFS_LZNT_CUNIT;
...
// Empty resident -> Empty nonresident.
    ...
    ALLOCATE_DEF, &alen, 0, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (err)
    goto out1;
    if (!rsize) {
...
// Remove original attribute.
    ...
    attr_s.flags, &attr, core::ptr::null_mut(), core::ptr::null_mut());
    if (err)
    goto out3;
    kfree(attr_s);
...
//
    ...
    let mut aoff: u32 = PtrOffset(rec, attr);
    let mut rsize: u32 = le32_to_cpu(attr.res.data_size);
    let mut tail: u32 = used - aoff - asize;
    char *next = Add2Ptr(attr, asize);
    let mut dsize: i64 = ALIGN(new_size, 8) - ALIGN(rsize, 8);
    if (dsize < 0) {
    memmove(next + dsize, next, tail);
    } else if (dsize > 0) {
    if (used + dsize > sbi.max_bytes_per_attr)
    return attr_make_nonresident(ni, attr, le, mi, new_size,
    run, ins_attr, core::ptr::null_mut());
    memmove(next + dsize, next, tail);
    memset(next, 0, dsize);
    }
    if (new_size > rsize)
...
//
    ...
    let mut new_alloc: u64 = 0;
    struct ATTRIB *attr = core::ptr::null_mut(), *attr_b;
    struct ATTR_LIST_ENTRY *le, *le_b;
    struct mft_inode *mi, *mi_b;
    CLST alen, vcn, lcn, new_alen, old_alen, svcn, evcn;
    CLST next_svcn, pre_alloc = -1, done = 0;
    let mut is_ext: bool = false, is_bad = false;
    let mut dirty: bool = false;
    struct runs_tree *run_da = run == &ni.file.run ? &ni.file.run_da :
    core::ptr::null_mut();
    let mut da: bool = !is_mft && sbi.options.delalloc && run_da && !no_da;
    u32 align;
    struct MFT_REC *rec;
    again:
    alen = 0;
    le_b = core::ptr::null_mut();
    attr_b = ni_find_attr(ni, core::ptr::null_mut(), &le_b, type, name, name_len, core::ptr::null_mut(),
    &mi_b);
    if (!attr_b) {
    err = -ENOENT;
    goto bad_inode;
...
//
    next_le:
    rec = mi.mrec;
    err = attr_load_runs(attr, ni, run, core::ptr::null_mut());
    if (err)
    goto out;
...
// Get the last LCN to allocate from.
    if (old_alen &&
    !run_lookup_entry(run, vcn, &lcn, core::ptr::null_mut(), core::ptr::null_mut())) {
    lcn = SPARSE_LCN;
    }
    if (lcn == SPARSE_LCN)
    lcn = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: lcn) -> else {
    else if (lcn)
    lcn += 1;
...
// Insert new attribute segment.
    ...
    attr_b.flags, &attr, &mi, core::ptr::null_mut());
//
// Layout of records maybe changed.
// Find base attribute to update.
//
    le_b = core::ptr::null_mut();
    attr_b = ni_find_attr(ni, core::ptr::null_mut(), &le_b, type, name, name_len,
    core::ptr::null_mut(), &mi_b);
    if (!attr_b) {
    err = -EINVAL;
    goto bad_inode;
    }
...
//
    ...
    let mut le_sz: u16 = le16_to_cpu(le.size);
//
...
// dlen - really deallocated clusters.
    ...
    if (err)
    goto out;
    attr = mi_find_attr(ni, mi, core::ptr::null_mut(), type, name, name_len,
    &le.id);
    if (!attr) {
    err = -EINVAL;
    goto bad_inode;
    ...
    ok:
    if (new_valid) {
    let mut valid: __le64 = cpu_to_le64(min(*new_valid, new_size));
...
// Restore 'attr' and 'mi'.
    ...
    is_bad = true;
    undo_1:
    run_deallocate_ex(sbi, run, vcn, alen, core::ptr::null_mut(), false, run_da);
    run_truncate(run, vcn);
    out:
    if (is_bad) {
    bad_inode:
...
// Try to find in cache.
    ...
    if (run_lookup_entry_da(&ni.file.run, !no_da ? &ni.file.run_da : core::ptr::null_mut(),
    vcn, lcn, len)) {
    } else {
// len = 0;
    }
...
//
    ...
    let mut da: bool = sbi.options.delalloc && !no_da;
    u8 cluster_bits;
    ...
    int step;
    again:
    if (run_lookup_entry_da(run, da ? &ni.file.run_da : core::ptr::null_mut(), vcn, lcn,
    len)) {
    } else {
// len = 0;
    }
    ...
    step = 0;
    le_b = core::ptr::null_mut();
    attr_b = ni_find_attr(ni, core::ptr::null_mut(), &le_b, ATTR_DATA, ni.file.ads.name,
    ni.file.ads.len, core::ptr::null_mut(), &mi_b);
    if (!attr_b) {
    err = -ENOENT;
    goto out;
    }
    if (!attr_b.non_res) {
    let mut data_size: u32 = le32_to_cpu(attr_b.res.data_size);
// lcn = RESIDENT_LCN;
// len = data_size;
    if (res) {
// res = NULL;
...
// len = NTFS_LZNT_CLUSTERS - (vcn & (NTFS_LZNT_CLUSTERS - 1));
    ...
    if (!*len) {
    if (run_lookup_entry_da(run, da ? run_da : core::ptr::null_mut(), vcn, lcn,
    len)) {
    if (*lcn != SPARSE_LCN || !new)
...
// ntfs3 uses 1 cluster per frame for new created sparsed files.
    ...
    let mut clst_per_frame: CLST = 1u << attr_b.nres.c_unit;
    let mut cmask: CLST = ~(clst_per_frame - 1);
...
// Load runs for truncated vcn.
    ...
    err = attr_load_runs(attr2, ni, run, core::ptr::null_mut());
    if (err)
    goto out;
    }
...
// Get the last LCN to allocate from.
    ...
    } else if (vcn && !run_lookup_entry(run, vcn - 1, &hint, core::ptr::null_mut(), core::ptr::null_mut())) {
    hint = -1;
    }
...
// Save 'total_size0' to restore if error.
    ...
    if (vcn != vcn0) {
    if (!run_lookup_entry(run, vcn0, lcn, len, core::ptr::null_mut())) {
    err = -EINVAL;
    goto out;
    }
    if (*lcn == SPARSE_LCN) {
...
// Layout of records is changed.
    le_b = core::ptr::null_mut();
    attr_b = ni_find_attr(ni, core::ptr::null_mut(), &le_b, ATTR_DATA,
    ni.file.ads.name,
    ni.file.ads.len, core::ptr::null_mut(), &mi_b);
    if (!attr_b) {
    err = -ENOENT;
...
// Last attribute segment.
    ...
    attr = mi_find_attr(ni, mi, core::ptr::null_mut(), ATTR_DATA, ni.file.ads.name,
...
// Undo step1.
    ...
    if (run_deallocate_ex(sbi, run, vcn, *len, core::ptr::null_mut(), false, run_da) ||
...
#[no_mangle]
pub unsafe extern "C" fn attr_data_write_resident(ni: *mut ntfs_inode, folio: *mut folio) -> c_int {
    ...
    struct ATTRIB *attr;
    u32 data_size;
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_DATA, ni.file.ads.name,
    ni.file.ads.len, core::ptr::null_mut(), &mi);
    if (!attr)
    return -EINVAL;
    if (attr.non_res) {
// Return special error code to check this case.
    ...
    let mut len: usize = min(data_size - vbo, folio_size(folio));
...
// Is record corrupted?
    ...
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), type, name, name_len, &vcn, core::ptr::null_mut());
    if (!attr) {
// Is record corrupted?
    return -ENOENT;
    }
...
//
    ...
    let mut cluster_bits: u8 = sbi.cluster_bits;
    CLST vcn;
    let mut vcn_last: CLST = (to - 1) >> cluster_bits;
    CLST lcn, clen;
    let mut err: c_int = 0;
    let mut retry: c_int = 0;
    for (vcn = from >> cluster_bits; vcn <= vcn_last; vcn += clen) {
    if (run_lookup_entry(run, vcn, &lcn, &clen, core::ptr::null_mut())) {
    retry = 0;
    continue;
    }
    if (retry) {
...
// ondisk_size = off[1] - off[0];
    ...
    do {
    let mut index: pgoff_t = vbo[i] >> PAGE_SHIFT;
    if (index != folio.index) {
    let mut from: u64 = vbo[i] & ~(u64)(PAGE_SIZE - 1);
    let mut to: u64 = min(from + PAGE_SIZE, wof_size);
    if (from >= wof_size) {
    _ntfs_bad_inode(&ni.vfs_inode);
    err = -EINVAL;
    goto out1;
...
//
    ...
    core::ptr::null_mut(), true, core::ptr::null_mut());
    if (err)
    goto out;
...
// Get the last LCN to allocate from.
    if (vcn + clst_data &&
    !run_lookup_entry(run, vcn + clst_data - 1, &hint, core::ptr::null_mut(),
    core::ptr::null_mut())) {
    hint = -1;
    }
    err = attr_allocate_clusters(sbi, run, core::ptr::null_mut(), vcn + clst_data,
    hint + 1, len - clst_data, core::ptr::null_mut(),
    ALLOCATE_DEF, &alen, 0, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (err)
    goto out;
...
// Last attribute segment.
    ...
    attr = mi_find_attr(ni, mi, core::ptr::null_mut(), ATTR_DATA,
...
//
#[no_mangle]
pub unsafe extern "C" fn attr_collapse_range(ni: *mut ntfs_inode, vbo: u64, bytes: u64) -> c_int {
...
//
    ...
    ni.file.ads.len, run,
    i_size - bytes, core::ptr::null_mut(), false,
...
// Layout of records maybe changed.
    le_b = core::ptr::null_mut();
    attr_b = ni_find_attr(ni, core::ptr::null_mut(), &le_b, ATTR_DATA,
    ni.file.ads.name, ni.file.ads.len, core::ptr::null_mut(),
    &mi_b);
...
// Look for required attribute.
    ...
    valid_size -= bytes;
#[no_mangle]
pub unsafe extern "C" fn if(valid_size: vbo <) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn attr_punch_hole(ni: *mut ntfs_inode, vbo: u64, bytes: u64, frame_size: *mut u32) -> c_int {
...
//
    err = run_deallocate_ex(core::ptr::null_mut(), run, vcn1, zero, &hole2, false,
    core::ptr::null_mut());
    if (err)
    goto done;
...
// Get next attribute segment.
    ...
    goto out;
    if (!attr_b) {
    attr_b = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_DATA,
    ni.file.ads.name, ni.file.ads.len, core::ptr::null_mut(),
    &mi_b);
    if (!attr_b) {
    err = -EINVAL;
...
//
#[no_mangle]
pub unsafe extern "C" fn attr_insert_range(ni: *mut ntfs_inode, vbo: u64, bytes: u64) -> c_int {
...
// ni->valid may be not equal valid_size (temporary).
    ...
    if (attr_load_runs(attr, ni, run, core::ptr::null_mut()))
    goto bad_inode;
...
//
#[no_mangle]
pub unsafe extern "C" fn attr_force_nonresident(ni: *mut ntfs_inode) -> c_int {
    ...
    struct mft_inode *mi;
    attr = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_DATA, ni.file.ads.name,
    ni.file.ads.len, core::ptr::null_mut(), &mi);
    if (!attr) {
    _ntfs_bad_inode(&ni.vfs_inode);
    return -ENOENT;
...
// Already non resident.
    ...
    &ni.file.run, &attr, core::ptr::null_mut());
    up_write(&ni.file.run_lock);
    return err;
    }
