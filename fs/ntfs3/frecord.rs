...
//
    ...
    struct rb_node *pr = core::ptr::null_mut();
    while (*p) {
    struct mft_inode *mi;
    pr = *p;
    ...
    p = &pr.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(ino: mi->rno <) -> else {
    else if (mi.rno < ino)
    p = &pr.rb_right;
    else
    return mi;
...
//
    ...
    {
    return ni_ins_mi(ni, &ni.mi_tree, rno, core::ptr::null_mut());
    }
//
// ni_add_mi - Add new mft_inode into ntfs_inode.
//
#[no_mangle]
unsafe extern "C" fn ni_add_mi(ni: *mut ntfs_inode, mi: *mut mft_inode) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_remove_mi(ni: *mut ntfs_inode, mi: *mut mft_inode) {
...
//
    ...
    attr = mi_find_attr(ni, &ni.mi, core::ptr::null_mut(), ATTR_STD, core::ptr::null_mut(), 0, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_clear(ni: *mut ntfs_inode) {
...
// On-demand allocated page for offsets.
    ...
    ni.file.offs_folio = core::ptr::null_mut();
    }

    kfree(ni.file.ads.name);
    ni.file.ads.name = core::ptr::null_mut();
    }
    if (ni.base && ni.base != ni) {
    iput(&ni.base.vfs_inode);
    ni.base = core::ptr::null_mut();
    }
    mi_clear(&ni.mi);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_load_mi_ex(ni: *mut ntfs_inode, rno: CLST, mi: *mut mft_inode) -> c_int {
...
// First look for list entry of required type.
    le = al_find_ex(ni, le_o ? *le_o : core::ptr::null_mut(), type, name, name_len, vcn);
    if (!le)
    return core::ptr::null_mut();
    if (le_o)
// le_o = le;
...
// Look for required attribute.
    attr = mi_find_attr(ni, m, core::ptr::null_mut(), type, name, name_len, &le.id);
    if (!attr)
    goto out;
    if (!attr.non_res) {
    if (vcn && *vcn)
...
// Get next list entry.
    le2 = *le = al_enumerate(ni, attr ? *le : core::ptr::null_mut());
    if (!le2)
    return core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_load_all_mi(ni: *mut ntfs_inode) -> c_int {
    ...
    let mut rno: CLST = ino_get(&le.ref);
    if (rno == ni.mi.rno)
    continue;
    err = ni_load_mi_ex(ni, rno, core::ptr::null_mut());
    if (err)
    return err;
    }
    return 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_add_subrecord(ni: *mut ntfs_inode, rno: CLST, mi: *mut mft_inode) -> bool {
...
//
    ...
    attr = mi_find_attr(ni, &ni.mi, core::ptr::null_mut(), type, name, name_len,
    id);
    if (!attr)
    return -ENOENT;
    ...
    return err;
    al_remove_le(ni, le);
    attr = mi_find_attr(ni, mi, core::ptr::null_mut(), type, name, name_len, id);
    if (!attr)
    return -ENOENT;
...
//
#[no_mangle]
unsafe extern "C" fn ni_repack(ni: *mut ntfs_inode) -> c_int {
    ...
    struct mft_inode *mi, *mi_p = core::ptr::null_mut();
    struct ATTRIB *attr = core::ptr::null_mut(), *attr_p;
    struct ATTR_LIST_ENTRY *le = core::ptr::null_mut(), *le_p;
    let mut alloc: CLST = 0;
    let mut cluster_bits: u8 = sbi.cluster_bits;
...
// We can remove this attribute segment.
    al_remove_le(ni, le);
    mi_remove_attr(core::ptr::null_mut(), mi, attr);
    le = le_p;
    continue;
    }
...
//
#[no_mangle]
unsafe extern "C" fn ni_try_remove_attr_list(ni: *mut ntfs_inode) -> c_int {
    ...
    if (err)
    return err;
    attr_list = mi_find_attr(ni, &ni.mi, core::ptr::null_mut(), ATTR_LIST, core::ptr::null_mut(), 0, core::ptr::null_mut());
    if (!attr_list)
    return 0;
...
// Free space in primary record without attribute list.
    ...
    if (!mi)
    return 0;
    attr = mi_find_attr(ni, mi, core::ptr::null_mut(), le.type, le_name(le),
    le.name_len, &le.id);
    if (!attr)
    return 0;
...
// It seems that attribute list can be removed from primary record.
    mi_remove_attr(core::ptr::null_mut(), &ni.mi, attr_list);
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_create_attr_list(ni: *mut ntfs_inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(ATTR_STD: attr->type ==) -> else {
    else if (attr.type == ATTR_STD)
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(ATTR_LIST: attr->type ==) -> else {
    else if (attr.type == ATTR_LIST)
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(ATTR_DATA: is_mft && attr->type ==) -> else {
...
// Call mi_remove_attr() in reverse order to keep pointers 'arr_move' valid.
    ...
    let mut asize: u32 = le32_to_cpu(b.size);
    let mut name_off: u16 = le16_to_cpu(b.name_off);
...
// Remove from primary record.
    if (!mi_remove_attr(core::ptr::null_mut(), &ni.mi, b))
    goto out;
    if (to_free <= asize)
    break;
    to_free -= asize;
    ...
    attr = mi_insert_attr(ni, &ni.mi, ATTR_LIST, core::ptr::null_mut(), 0,
...
// Check each of loaded subrecord.
    ...
    if (is_mft_data &&
    (mi_enum_attr(ni, mi, core::ptr::null_mut()) ||
    vbo <= ((u64)mi.rno << sbi.record_bits))) {
// We can't accept this record 'cause MFT's bootstrapping.
    continue;
    }
    if (is_mft &&
    mi_find_attr(ni, mi, core::ptr::null_mut(), ATTR_DATA, core::ptr::null_mut(), 0, core::ptr::null_mut())) {
//
...
//
    ...
    mi_find_attr(ni, mi, core::ptr::null_mut(), type, name, name_len, core::ptr::null_mut())) {
...
// Reserve space for the ATTRIB list.
    ...
    if (asize <= free) {
    attr = ni_ins_new_attr(ni, &ni.mi, core::ptr::null_mut(), type, name, name_len,
...
// This ATTRIB will be external.
    err = ni_ins_attr_ext(ni, core::ptr::null_mut(), type, name, name_len, asize,
...
// Skip attributes that MUST be primary record.
    ...
    le = core::ptr::null_mut();
    if (ni.attr_list.size) {
    le = al_find_le(ni, core::ptr::null_mut(), attr);
    if (!le) {
// Really this is a serious bug.
    ...
    false, &eattr, core::ptr::null_mut(), core::ptr::null_mut());
    if (err)
    return err;
    id = eattr.id;
    ...
    eattr.id = id;
// Remove from primary record.
    mi_remove_attr(core::ptr::null_mut(), &ni.mi, attr);
// attr now points to next attribute.
    ...
    ;
    attr = ni_ins_new_attr(ni, &ni.mi, core::ptr::null_mut(), type, name, name_len, asize,
    name_off, svcn, ins_le);
    if (!attr) {
    err = -EINVAL;
    goto out;
...
#[no_mangle]
unsafe extern "C" fn ni_expand_mft_list(ni: *mut ntfs_inode) -> c_int {
...
// Find the nearest MFT.
    mft_min = 0;
    mft_new = 0;
    mi_min = core::ptr::null_mut();
    for (node = rb_first(&ni.mi_tree); node; node = rb_next(node)) {
    mi = rb_entry(node, struct mft_inode, node);
    attr = mi_enum_attr(ni, mi, core::ptr::null_mut());
    if (!attr) {
    mft_min = mi.rno;
    mi_min = mi;
    break;
...
// Really this is not critical.
    ...
    attr = mi_find_attr(ni, &ni.mi, core::ptr::null_mut(), ATTR_DATA, core::ptr::null_mut(), 0, core::ptr::null_mut());
    if (!attr) {
    err = -EINVAL;
    goto out;
    }
...
// Estimate packed size (run_buf=NULL).
    err = run_pack(run, svcn, evcn + 1 - svcn, core::ptr::null_mut(), sbi.record_size,
    &plen);
    if (err < 0)
    goto out;
    run_size = ALIGN(err, 8);
...
//
    attr = ni_ins_new_attr(ni, mi_min, core::ptr::null_mut(), ATTR_DATA, core::ptr::null_mut(), 0,
    SIZEOF_NONRESIDENT + run_size,
    SIZEOF_NONRESIDENT, svcn, core::ptr::null_mut());
    if (!attr) {
    err = -EINVAL;
    goto out;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_expand_list(ni: *mut ntfs_inode) -> c_int {
    int ni_expand_list(struct ntfs_inode *ni)
    {
    let mut err: c_int = 0;
    u32 asize, done = 0;
    struct ATTRIB *attr, *ins_attr = core::ptr::null_mut();
    struct ATTR_LIST_ENTRY *le;
    let mut is_mft: bool = ni.mi.rno == MFT_REC_MFT;
    struct MFT_REF ref;
    ni = ni.base;
    mi_get_ref(&ni.mi, &ref);
    le = core::ptr::null_mut();
...
// Always insert into new record to avoid collisions (deep recursive).
    ...
    &ins_attr, core::ptr::null_mut(), core::ptr::null_mut());
    if (err || !ins_attr)
    goto out;
...
//
    ...
    let mut name_off: u32 = is_ext ? SIZEOF_NONRESIDENT_EX : SIZEOF_NONRESIDENT;
    let mut run_off: u32 = name_off + name_size;
    u32 run_size, asize;
    struct ntfs_sb_info *sbi = ni.mi.sbi;
    ni = ni.base;
// Estimate packed size (run_buf=NULL).
    err = run_pack(run, svcn, len, core::ptr::null_mut(), sbi.max_bytes_per_attr - run_off,
    &plen);
    if (err < 0)
    goto out;
    run_size = ALIGN(err, 8);
...
//
    ...
    int err;
    let mut name_size: u32 = ALIGN(name_len * sizeof(short), 8);
    let mut asize: u32 = SIZEOF_RESIDENT + name_size + ALIGN(data_size, 8);
    struct ATTRIB *attr;
    ni = ni.base;
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_delete_all(ni: *mut ntfs_inode) -> c_int {
    ...
    CLST svcn, evcn;
    struct ntfs_sb_info *sbi = ni.mi.sbi;
    let mut nt3: bool = is_ntfs3(sbi);
    struct MFT_REF ref;
    ni = ni.base;
    while ((attr = ni_enum_attr_ex(ni, attr, &le, core::ptr::null_mut()))) {
...
// Enumerate all names.
    next:
    attr = ni_find_attr(ni, attr, le, ATTR_NAME, core::ptr::null_mut(), 0, core::ptr::null_mut(), mi);
    if (!attr)
    return core::ptr::null_mut();
    ...
    goto next;
    if (ntfs_cmp_names(uni.name, uni.len, fname.name, uni.len, core::ptr::null_mut(),
    false))
    goto next;
    return fname;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_new_attr_flags(ni: *mut ntfs_inode, new_fa: enum FILE_ATTRIBUTE) -> c_int {
    ...
    u32 new_asize;
    ni = ni.base;
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_DATA, core::ptr::null_mut(), 0, core::ptr::null_mut(), &mi);
    if (!attr)
    return -EINVAL;
...
//
    ...
    {
    const struct REPARSE_DATA_BUFFER *rp = core::ptr::null_mut();
    u8 bits;
    u16 len;
...
// Try to estimate reparse point.
    ...
    struct runs_tree run;
    run_init(&run);
    if (!attr_load_runs_vcn(ni, ATTR_REPARSE, core::ptr::null_mut(), 0, &run, 0) &&
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_read_folio_cmpr(ni: *mut ntfs_inode, folio: *mut folio) -> c_int {
    ...
    struct page **pages = core::ptr::null_mut(); /* Array of at most 16 pages. stack? */
    u8 frame_bits;
    CLST frame;
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_decompress_file(ni: *mut ntfs_inode) -> c_int {
    ...
    let mut gfp_mask: gfp_t = mapping_gfp_mask(mapping);
...
//
    ...
    &clen, &new, false, core::ptr::null_mut(),
    false);
    if (err)
    goto out;
    }
...
//
    err = ni_remove_attr(ni, ATTR_REPARSE, core::ptr::null_mut(), 0, false, core::ptr::null_mut());
    if (err)
    goto out;
//
...
//
    ...
    struct ntfs_sb_info *sbi = ni.mi.sbi;
    let mut cluster_bits: u8 = sbi.cluster_bits;
    char *frame_ondisk = core::ptr::null_mut();
    char *frame_mem = core::ptr::null_mut();
    struct ATTR_LIST_ENTRY *le = core::ptr::null_mut();
    struct runs_tree *run = &ni.file.run;
    let mut valid_size: u64 = ni.i_valid;
    u64 vbo_disk;
    let mut unc_size: usize = 0;
...
//
    ...
    attr = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_DATA, core::ptr::null_mut(), 0, core::ptr::null_mut(), core::ptr::null_mut());
    if (!attr) {
    err = -ENOENT;
    goto out1;
    }
    if (!attr.non_res) {
    let mut data_size: u32 = le32_to_cpu(attr.res.data_size);
    ...
    let mut i_size: loff_t = i_size_read(&ni.vfs_inode);
    let mut frame_bits: u32 = ni_ext_compress_bits(ni);
    let mut frame64: u64 = frame_vbo >> frame_bits;
    u64 frames, vbo_data;
...
// Unknown compression.
    ...
    ARRAY_SIZE(WOF_NAME), core::ptr::null_mut(), core::ptr::null_mut());
    if (!attr) {
    ntfs_inode_err(
    &ni.vfs_inode,
...
// LZNT - Native NTFS compression.
    ...
    err = unc_size;
#[no_mangle]
pub unsafe extern "C" fn if(frame_size: !unc_size || unc_size >) -> else {
    else if (!unc_size || unc_size > frame_size)
    err = -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(frame_size: unc_size <) -> else {
//
...
//
    ...
    let mut ok: usize = valid_size - frame_vbo;
...
//
    ...
    let mut frame_bits: u8 = NTFS_LZNT_CUNIT + sbi.cluster_bits;
    let mut frame_size: u32 = sbi.cluster_size << NTFS_LZNT_CUNIT;
    let mut frame_vbo: u64 = folio_pos(folio);
    let mut frame: CLST = frame_vbo >> frame_bits;
    char *frame_ondisk = core::ptr::null_mut();
    struct ATTR_LIST_ENTRY *le = core::ptr::null_mut();
    char *frame_mem;
    struct ATTRIB *attr;
    ...
    struct lznt *lznt;
    ni = ni.base;
    attr = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_DATA, core::ptr::null_mut(), 0, core::ptr::null_mut(), &mi);
    if (!attr) {
    err = -ENOENT;
    goto out;
    }
...
//
    ...
    struct ATTR_LIST_ENTRY *le;
    struct mft_inode *mi;
    let mut de_key_size: u16 = le16_to_cpu(de.key_size);
    u8 name_type;
    ni = ni.base;
// undo_step = 0;
// Find name in record.
...
// Get paired name.
    ...
    if (fname) {
    let mut de2_key_size: u16 = fname_full_size(fname);
// de2 = Add2Ptr(de, 1024);
...
//
    ...
    if (ni_insert_resident(ni, de_key_size, ATTR_NAME, core::ptr::null_mut(), 0,
    &attr, core::ptr::null_mut(), core::ptr::null_mut()))
    return false;
    ...
    de2.flags = 0;
    de2.res = 0;
    if (indx_insert_entry(&dir_ni.dir, dir_ni, de2, sbi, core::ptr::null_mut(), 1))
    return false;
    fallthrough;
    case 2:
    ...
    if (indx_insert_entry(&dir_ni.dir, dir_ni, de, sbi, core::ptr::null_mut(), 1))
    return false;
    }
    return true;
    }
...
// Fill duplicate from any ATTR_NAME.
    fname = ni_fname_name(ni, core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut(), core::ptr::null_mut());
    if (fname)
    memcpy(&de_name.dup, &fname.dup, sizeof(fname.dup));
    de_name.dup.fa = ni.std_fa;
// Insert new name into MFT.
    err = ni_insert_resident(ni, de_key_size, ATTR_NAME, core::ptr::null_mut(), 0, &attr,
    &mi, &le);
    if (err)
    return err;
    memcpy(Add2Ptr(attr, SIZEOF_RESIDENT), de_name, de_key_size);
// Insert new name into directory.
    err = indx_insert_entry(&dir_ni.dir, dir_ni, de, sbi, core::ptr::null_mut(), 0);
    if (err)
...
//
    ...
    {
    int err;
    struct NTFS_DE *de2 = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_is_dirty(inode: *mut inode) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_seek_data_or_hole(ni: *mut ntfs_inode, offset: loff_t, data: bool) -> loff_t {
    loff_t ni_seek_data_or_hole(struct ntfs_inode *ni, loff_t offset, bool data)
    {
    int err;
    let mut cluster_bits: u8 = ni.mi.sbi.cluster_bits;
    CLST vcn, lcn, clen;
    loff_t vbo;
// Enumerate all fragments.
    for (vcn = offset >> cluster_bits;; vcn += clen) {
    err = attr_data_get_block(ni, vcn, 1, &lcn, &clen, core::ptr::null_mut(), false,
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_write_parents(ni: *mut ntfs_inode, sync: c_int) -> c_int {
    ...
    if (!is_ni_base(ni))
    return 0;
    while ((attr = ni_find_attr(ni, attr, &le, ATTR_NAME, core::ptr::null_mut(), 0, core::ptr::null_mut(),
    core::ptr::null_mut()))) {
    struct inode *dir;
    struct ATTR_FILE_NAME *fname;
    fname = resident_data_ex(attr, SIZEOF_ATTRIBUTE_FILENAME);
...
// Check simple case when parent inode equals current inode.
    ...
    dir = ntfs_iget5(sb, &fname.home, core::ptr::null_mut());
    if (IS_ERR(dir)) {
    ntfs_inode_warn(
    &ni.vfs_inode,
    ...
    if (!is_bad_inode(dir)) {
    let mut err2: c_int = write_inode_now(dir, sync);
    if (!err)
    err = err2;
    }
    iput(dir);
...
//
    ...
    dup.fa &= ~FILE_ATTRIBUTE_DIRECTORY;
    attr = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_DATA, core::ptr::null_mut(), 0, core::ptr::null_mut(),
    &mi);
    if (!attr) {
    ...
    let mut new_valid: u64 = ni.i_valid;
    let mut data_size: u64 = le64_to_cpu(attr.nres.data_size);
    __le64 valid_le;
    ...
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_REPARSE, core::ptr::null_mut(), 0, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (attr) {
...
// If ATTR_REPARSE exists 'rp' can't be NULL.
    ...
    attr = ni_find_attr(ni, attr, &le, ATTR_EA_INFO, core::ptr::null_mut(), 0, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (attr) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_write_inode(inode: *mut inode, sync: c_int, hint: *const c_char) -> c_int {
...
// Update attribute list.
    ...
    if (!mi.dirty)
    continue;
    is_empty = !mi_enum_attr(ni, mi, core::ptr::null_mut());
    if (is_empty)
    clear_rec_inuse(mi.mrec);
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_allocate_da_blocks(ni: *mut ntfs_inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_allocate_da_blocks_locked(ni: *mut ntfs_inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ni_query_ads(ni: *mut ntfs_inode, pos: *mut loff_t, iter: *mut iov_iter) -> isize {
    ssize_t ni_query_ads(struct ntfs_inode *ni, loff_t *pos, struct iov_iter *iter)
    {
    let mut ret: isize = 0;
    struct ntfs_sb_info *sbi = ni.mi.sbi;
    let mut bytes: usize = iov_iter_count(iter);
    let mut end: loff_t = *pos + bytes;
