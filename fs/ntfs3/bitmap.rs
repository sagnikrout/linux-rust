...
#[derive(Copy, Clone)]
pub struct e_node {
    pub /: *mut *mut rb_node_key start; / Tree sorted by start.,
    pub /: *mut *mut rb_node_key count; / Tree sorted by len.,
}
...
#[no_mangle]
pub unsafe extern "C" fn ntfs3_init_bitmap() -> int __init {
...
#[no_mangle]
pub unsafe extern "C" fn ntfs3_exit_bitmap() {
...
//
    ...
    size_t free_len;
    u32 free_bits, end;
    let mut used: u32 = find_next_zero_bit_le(buf, wend, wpos);
    if (used >= wend) {
    if (*b_len < *prev_tail) {
...
//
#[no_mangle]
pub unsafe extern "C" fn wnd_close(wnd: *mut wnd_bitmap) {
    void wnd_close(struct wnd_bitmap *wnd)
    {
    struct rb_node *node, *next;
    kvfree(wnd.free_bits);
    wnd.free_bits = core::ptr::null_mut();
    run_close(&wnd.run);
...
//
#[no_mangle]
pub unsafe extern "C" fn rb_insert_count(root: *mut rb_root, e: *mut e_node) -> bool {
    static inline bool rb_insert_count(struct rb_root *root, struct e_node *e)
    {
    struct rb_node **p = &root.rb_node;
    struct rb_node *parent = core::ptr::null_mut();
    let mut e_ckey: usize = e.count.key;
    let mut e_skey: usize = e.start.key;
    while (*p) {
    struct e_node *k =
...
//
#[no_mangle]
pub unsafe extern "C" fn rb_insert_start(root: *mut rb_root, e: *mut e_node) -> bool {
...
//
    ...
    bool build)
    {
    struct e_node *e, *e0 = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn wnd_remove_free_ext(wnd: *mut wnd_bitmap, bit: usize, len: usize) {
    static void wnd_remove_free_ext(struct wnd_bitmap *wnd, size_t bit, size_t len)
    {
    struct rb_node *n, *n3;
    struct e_node *e, *e3;
    let mut end_in: usize = bit + len;
...
// Range [bit,end_in) must be inside 'e' or outside 'e' and 'n'.
    ...
    ;
#[no_mangle]
pub unsafe extern "C" fn if(end: end_in <=) -> else {
// Range [bit,end_in) inside 'e'.
    new_key = end_in;
    new_len = end - end_in;
    len = bit - e.start.key;
    } else if (bit > end) {
    let mut bmax: bool = false;
    n3 = rb_next(n);
    while (n3) {
...
//
#[no_mangle]
unsafe extern "C" fn wnd_rescan(wnd: *mut wnd_bitmap) -> c_int {
    static int wnd_rescan(struct wnd_bitmap *wnd)
    {
    let mut err: c_int = 0;
    ...
    let mut wbits: u32 = 8 * sb.s_blocksize;
    u32 used, frb;
...
// All zeroes.
    ...
    if (ra) {
    let mut idx: pgoff_t = lbo >> PAGE_SHIFT;
...
//
    ...
    let mut zlen: usize = wnd.zone_end - wnd.zone_bit;
...
#[no_mangle]
pub unsafe extern "C" fn wnd_init(wnd: *mut wnd_bitmap, sb: *mut super_block, nbits: usize) -> c_int {
    int wnd_init(struct wnd_bitmap *wnd, struct super_block *sb, size_t nbits)
    {
    int err;
    let mut blocksize: u32 = sb.s_blocksize;
    let mut wbits: u32 = blocksize * 8;
...
//
#[no_mangle]
pub unsafe extern "C" fn wnd_set_free(wnd: *mut wnd_bitmap, bit: usize, bits: usize) -> c_int {
    int wnd_set_free(struct wnd_bitmap *wnd, size_t bit, size_t bits)
    {
    let mut err: c_int = 0;
    struct super_block *sb = wnd.sb;
    let mut wbits: u32 = 8 * sb.s_blocksize;
    let mut iw: usize = bit >> (sb.s_blocksize_bits + 3);
    let mut wbit: u32 = bit & (wbits - 1);
    struct buffer_head *bh;
    u32 op;
...
//
#[no_mangle]
pub unsafe extern "C" fn wnd_set_used(wnd: *mut wnd_bitmap, bit: usize, bits: usize) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn wnd_is_free_hlp(wnd: *mut wnd_bitmap, bit: usize, bits: usize) -> bool {
    static bool wnd_is_free_hlp(struct wnd_bitmap *wnd, size_t bit, size_t bits)
    {
    struct super_block *sb = wnd.sb;
    let mut iw: usize = bit >> (sb.s_blocksize_bits + 3);
...
//
#[no_mangle]
pub unsafe extern "C" fn wnd_is_free(wnd: *mut wnd_bitmap, bit: usize, bits: usize) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn wnd_is_used(wnd: *mut wnd_bitmap, bit: usize, bits: usize) -> bool {
...
//
    ...
    let mut to_alloc0: usize = to_alloc;
...
// Fast checking for available free space.
    ...
    let mut zeroes: usize = wnd_zeroes(wnd);
...
// Use hint: Enumerate extents by start >= hint.
    ...
    cr = cr.rb_left;
    if (!cr) {
    e = pr ? rb_entry(pr, struct e_node, start.node) : core::ptr::null_mut();
    break;
    }
    }
    if (!e)
...
// We have found extension with 'hint' inside.
    let mut len: usize = e.start.key + e.count.key - hint;
...
// TODO: Optimize request for case nbits > wbits.
    ...
    let mut t: usize = max_alloc + wbits - 1;
...
//
    let mut nextmax: usize = hint + to_alloc;
...
//
#[no_mangle]
pub unsafe extern "C" fn wnd_extend(wnd: *mut wnd_bitmap, new_bits: usize) -> c_int {
    ...
    let mut old_bits: usize = wnd.nbits;
    u16 *new_free;
...
#[no_mangle]
pub unsafe extern "C" fn wnd_zone_set(wnd: *mut wnd_bitmap, lcn: usize, len: usize) {
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_trim_fs(sbi: *mut ntfs_sb_info, range: *mut fstrim_range) -> c_int {
    ...
    let mut wbits: u32 = 8 * sb.s_blocksize;
    let mut len: CLST = 0, lcn = 0, done = 0;
    let mut minlen: CLST = bytes_to_cluster(sbi, range.minlen);
    let mut lcn_from: CLST = bytes_to_cluster(sbi, range.start);
    let mut iw: usize = lcn_from >> (sb.s_blocksize_bits + 3);
    let mut wbit: u32 = lcn_from & (wbits - 1);
    CLST lcn_to;
    if (!minlen)
    minlen = 1;
    if (range.len == (u64)-1)
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_bitmap_set_le(map: *mut c_void, start: c_uint, len: c_int) {
    ...
    let mut mask_to_set: bitmap_ulong = cpu_to_ul(BITMAP_FIRST_WORD_MASK(start));
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_bitmap_clear_le(map: *mut c_void, start: c_uint, len: c_int) {
    ...
    let mut mask_to_clear: bitmap_ulong = cpu_to_ul(BITMAP_FIRST_WORD_MASK(start));
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_bitmap_weight_le(bitmap: *const c_void, bits: c_int) -> c_uint {
