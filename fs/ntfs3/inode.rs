...
//
    ...
    struct ATTR_STD_INFO5 *std5 = core::ptr::null_mut();
    ...
    let mut rp_fa: u32 = 0, asize, t32;
    u16 roff, rsize, names = 0, links = 0;
    const struct ATTR_FILE_NAME *fname = core::ptr::null_mut();
    const struct INDEX_ROOT *root = core::ptr::null_mut();
...
//
    ...
    err = -EINVAL;
    attr = ni_enum_attr_ex(ni, attr, &le, core::ptr::null_mut());
    if (!attr)
    goto end_enum;
    if (le && le.vcn) {
// This is non primary attribute segment. Ignore if not MFT.
    ...
    run = &ni.file.run;
#[no_mangle]
pub unsafe extern "C" fn if(ATTR_BITMAP: attr->type ==) -> else {
...
// Enumerate all attributes in record.
    while ((attr = ni_enum_attr_ex(nb, attr, &le, core::ptr::null_mut()))) {
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_test_inode(inode: *mut inode, data: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntfs_set_inode(inode: *mut inode, data: *mut c_void) -> c_int {
...
// Pack params to pass in iget5_locked.
    let mut ig5: IGET5_PARAM = { ref, name };
    let mut ino: u64 = ino_get(ref);
    struct inode *inode, *base = core::ptr::null_mut();
    let mut ads: bool = name && name.ads_len;
    struct ntfs_inode *ni;
    if (ads) {
// First get base inode
    base = ntfs_iget5_flags(sb, ref, core::ptr::null_mut(), 0);
    if (IS_ERR(base))
    return base;
    }
...
// inode + ads
    ni.base = ntfs_i(base);
    base = core::ptr::null_mut(); /* keep reference incremented (instead of ihold). */
    }
    if (ads) {
...
#[no_mangle]
unsafe extern "C" fn ntfs_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
...
#[no_mangle]
unsafe extern "C" fn ntfs_iomap_read_end_io(bio: *mut bio) {
    static void ntfs_iomap_read_end_io(struct bio *bio)
    {
    let mut error: c_int = blk_status_to_errno(bio.bi_status);
    struct folio_iter fi;
    bio_for_each_folio_all(fi, bio) {
    struct folio *folio = fi.folio;
    struct inode *inode = folio.mapping.host;
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut valid: u64 = ni.i_valid;
    let mut f_size: u32 = folio_size(folio);
    let mut f_pos: loff_t = folio_pos(folio);
...
// clang-format on
#[no_mangle]
unsafe extern "C" fn ntfs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    ...
    struct inode *inode = mapping.host;
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut vbo: loff_t = folio_pos(folio);
    struct iomap_read_folio_ctx ctx = {
    .cur_folio = folio,
...
// ni_lock is taken inside ni_read_folio_cmpr after page locks
    ...
    iomap_read_folio(&ntfs_iomap_ops, &ctx, core::ptr::null_mut());
    return 0;
    }
#[no_mangle]
unsafe extern "C" fn ntfs_readahead(rac: *mut readahead_control) {
...
// No readahead for compressed.
    ...
    iomap_readahead(&ntfs_iomap_ops, &ctx, core::ptr::null_mut());
    }
#[no_mangle]
pub unsafe extern "C" fn ntfs_set_size(inode: *mut inode, new_size: u64) -> c_int {
...
//
    ...
    let mut vcn: CLST = offset >> cluster_bits;
    let mut off: u32 = offset & sbi.cluster_mask;
    let mut rw: bool = flags & IOMAP_WRITE;
    let mut endbyte: loff_t = offset + length;
    void *res = core::ptr::null_mut();
    int err;
    CLST lcn, clen, clen_max = 1;
    let mut new_clst: bool = false;
    bool no_da;
...
//
    ...
    rw ? &new_clst : core::ptr::null_mut(), zero, &res, no_da);
    if (err) {
    return err;
    }
    if (lcn == EOF_LCN) {
...
// Avoid too large requests.
    u32 tail;
    let mut off_a: u32 = offset & (PAGE_SIZE - 1);
    if (off_a)
    ...
    let mut err: c_int = 0;
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut endbyte: loff_t = pos + written;
    if ((flags & IOMAP_WRITE) || (flags & IOMAP_ZERO)) {
    if (iomap.type == IOMAP_INLINE) {
    u32 data_size;
    struct ATTRIB *attr;
    struct mft_inode *mi;
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_DATA,
...
//
    ...
    {
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut end: loff_t = pos + len;
    let mut f_size: u32 = folio_size(folio);
    let mut f_pos: loff_t = folio_pos(folio);
    let mut f_end: loff_t = f_pos + f_size;
...
// Check iomap position.
    ...
    let mut i_size_up: loff_t = ntfs_up_cluster(sbi, inode.i_size);
    let mut len_max: loff_t = i_size_up - offset;
...
#[no_mangle]
pub unsafe extern "C" fn ntfs3_write_inode(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
    int ntfs3_write_inode(struct inode *inode, struct writeback_control *wbc)
    {
    return _ni_write_inode(inode, wbc.sync_mode == WB_SYNC_ALL);
    }
#[no_mangle]
pub unsafe extern "C" fn ntfs_sync_inode(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn inode_read_data(inode: *mut inode, data: *mut c_void, bytes: usize) -> c_int {
    int inode_read_data(struct inode *inode, void *data, size_t bytes)
    {
    pgoff_t idx;
    struct address_space *mapping = inode.i_mapping;
    for (idx = 0; bytes; idx++) {
    let mut op: usize = bytes > PAGE_SIZE ? PAGE_SIZE : bytes;
    struct page *page = read_mapping_page(mapping, idx, core::ptr::null_mut());
    void *kaddr;
    if (IS_ERR(page))
    return PTR_ERR(page);
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_reparse_bytes(uni_len: u32, is_absolute: bool) -> u32 {
...
//
    ...
    struct ntfs_inode *ni = core::ptr::null_mut();
    ...
    let mut security_id: __le32 = SECURITY_ID_INVALID;
    CLST vcn;
    const void *sd;
    u16 t16, nsize = 0, aid = 0;
    struct INDEX_ROOT *root, *dir_root;
    struct NTFS_DE *e, *new_de = core::ptr::null_mut();
    struct REPARSE_DATA_BUFFER *rp = core::ptr::null_mut();
    let mut rp_inserted: bool = false;
// New file will be resident or non resident.
    let mut new_file_resident: bool = 1;
    if (!fnd)
    ni_lock_dir(dir_ni);
    if (sbi.options.ads) {
    ...
    dir_root = indx_get_root(&dir_ni.dir, dir_ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!dir_root) {
    err = -EINVAL;
    goto out1;
    }
...
// Step 1: allocate and fill new mft record.
    err = ntfs_look_free_mft(sbi, &ino, false, core::ptr::null_mut(), core::ptr::null_mut());
    if (err)
    goto out2;
    ni = ntfs_new_inode(sbi, ino, S_ISDIR(mode) ? RECORD_FLAG_DIR : 0);
    if (IS_ERR(ni)) {
    err = PTR_ERR(ni);
    ni = core::ptr::null_mut();
    goto out3;
    }
    inode = &ni.vfs_inode;
...
//
    ...
    attr.flags = ATTR_FLAG_SPARSED;
#[no_mangle]
pub unsafe extern "C" fn if(FILE_ATTRIBUTE_COMPRESSED: fa &) -> else {
    else if (fa & FILE_ATTRIBUTE_COMPRESSED)
    attr.flags = ATTR_FLAG_COMPRESSED;
    attr.res.data_off = SIZEOF_RESIDENT_LE;
    asize = SIZEOF_RESIDENT;
...
//
    ...
    CLST alen;
    let mut clst: CLST = bytes_to_cluster(sbi, nsize);
// Bytes per runs.
    ...
    err = attr_allocate_clusters(sbi, &ni.file.run, core::ptr::null_mut(),
    0, 0, clst, core::ptr::null_mut(),
    ALLOCATE_DEF, &alen, 0,
...
//
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_NAME, core::ptr::null_mut(), 0, core::ptr::null_mut(),
    core::ptr::null_mut());
    if (attr) {
...
// Normal exit.
    goto out2;
    out6:
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_EA, core::ptr::null_mut(), 0, core::ptr::null_mut(), core::ptr::null_mut());
    if (attr && attr.non_res) {
// Delete ATTR_EA, if non-resident.
    struct runs_tree run;
    run_init(&run);
    attr_set_size(ni, ATTR_EA, core::ptr::null_mut(), 0, &run, 0, core::ptr::null_mut(), false);
    run_close(&run);
    }
    if (rp_inserted)
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_link_inode(inode: *mut inode, dentry: *mut dentry) -> c_int {
...
// Mark rw ntfs as dirty. It will be cleared at umount.
    ntfs_set_state(sbi, NTFS_DIRTY_DIRTY);
// Construct 'de'.
    err = fill_name_de(sbi, de, &dentry.d_name, core::ptr::null_mut());
    if (err)
    goto out;
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_unlink_inode(dir: *mut inode, dentry: *const dentry) -> c_int {
    ...
    struct ntfs_inode *ni = ntfs_i(inode);
    struct ntfs_inode *dir_ni = ntfs_i(dir);
    struct NTFS_DE *de, *de2 = core::ptr::null_mut();
    int undo_remove;
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_evict_inode(inode: *mut inode) {
...
//
    ...
    char *link_path_buffer = core::ptr::null_mut(), *link_path;
    char *translated = core::ptr::null_mut();
    char *target_start;
    int copy_len;
...
// buffer = 0;
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_REPARSE, core::ptr::null_mut(), 0, core::ptr::null_mut(), core::ptr::null_mut());
    if (!attr)
    goto out;
    if (!attr.non_res) {
...
// Read into temporal buffer.
    err = ntfs_read_run_nb(sbi, &ni.file.run, 0, rp, size, core::ptr::null_mut());
    if (err)
    goto out;
    }
// Microsoft Tag.
