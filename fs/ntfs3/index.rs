...
//
    ...
    let mut t1: u32 = le32_to_cpu(*k1);
    let mut t2: u32 = le32_to_cpu(*k2);
    if (t1 > t2)
    return 1;
    if (t1 < t2)
    return -1;
...
#[no_mangle]
pub unsafe extern "C" fn get_cmp_func(root: *const INDEX_ROOT) -> NTFS_CMP_FUNC {
...
pub struct bmp_buf {
    ...
    let mut vcn: CLST = off >> sbi.cluster_bits;
    struct ATTR_LIST_ENTRY *le = core::ptr::null_mut();
    struct buffer_head *bh;
    struct super_block *sb;
    u32 blocksize;
    const struct INDEX_NAMES *in = &s_index_names[indx.type];
    bbuf.bh = core::ptr::null_mut();
    b = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_BITMAP, in.name, in.name_len,
    &vcn, &bbuf.mi);
    bbuf.b = b;
    if (!b)
    return -EINVAL;
...
// Looks like filesystem error.
    ...
    bbuf.new_valid = 0;
#[no_mangle]
pub unsafe extern "C" fn if(data_size: bbuf->new_valid >) -> else {
    else if (bbuf.new_valid > data_size)
    bbuf.new_valid = data_size;
    if (vbo >= valid_size) {
    memset(bh.b_data, 0, blocksize);
    } else if (vbo + blocksize > valid_size) {
    let mut voff: u32 = valid_size & sbi.block_mask;
...
#[no_mangle]
unsafe extern "C" fn bmp_buf_put(bbuf: *mut bmp_buf, dirty: bool) {
...
//
    ...
    let mut nbits: u32 = sb.s_blocksize * 8;
    let mut blocksize: u32 = sb.s_blocksize;
    let mut valid_size: u64 = le64_to_cpu(bitmap.nres.valid_size);
    let mut data_size: u64 = le64_to_cpu(bitmap.nres.data_size);
    let mut eblock: sector_t = bytes_to_block(sb, data_size);
    let mut vbo: usize = from >> 3;
    let mut blk: sector_t = (vbo & sbi.cluster_mask) >> sb.s_blocksize_bits;
    let mut vblock: sector_t = vbo >> sb.s_blocksize_bits;
    sector_t blen, block;
...
#[no_mangle]
unsafe extern "C" fn scan_for_free(buf: *const c_ulong, bit: u32, bits: u32, ret: *mut usize) -> bool {
    static bool scan_for_free(const ulong *buf, u32 bit, u32 bits, size_t *ret)
    {
    let mut pos: usize = find_next_zero_bit_le(buf, bits, bit);
    if (pos >= bits)
    return false;
// ret = pos;
    return true;
...
// bit = MINUS_ONE_T;
    if (!b.non_res) {
    let mut nbits: u32 = 8 * le32_to_cpu(b.res.data_size);
    let mut pos: usize = find_next_zero_bit_le(resident_data(b), nbits, 0);
    if (pos < nbits)
// bit = pos;
    } else {
...
#[no_mangle]
unsafe extern "C" fn scan_for_used(buf: *const c_ulong, bit: u32, bits: u32, ret: *mut usize) -> bool {
    static bool scan_for_used(const ulong *buf, u32 bit, u32 bits, size_t *ret)
    {
    let mut pos: usize = find_next_bit_le(buf, bits, bit);
    if (pos >= bits)
    return false;
// ret = pos;
    return true;
...
//
#[no_mangle]
pub unsafe extern "C" fn indx_used_bit(indx: *mut ntfs_index, ni: *mut ntfs_inode, bit: *mut usize) -> c_int {
    int indx_used_bit(struct ntfs_index *indx, struct ntfs_inode *ni, size_t *bit)
    {
    struct ATTRIB *b;
    struct ATTR_LIST_ENTRY *le = core::ptr::null_mut();
    let mut from: usize = *bit;
    const struct INDEX_NAMES *in = &s_index_names[indx.type];
    int err;
    b = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_BITMAP, in.name, in.name_len,
    core::ptr::null_mut(), core::ptr::null_mut());
...
// bit = MINUS_ONE_T;
    if (!b.non_res) {
    let mut nbits: u32 = le32_to_cpu(b.res.data_size) * 8;
    let mut pos: usize = find_next_bit_le(resident_data(b), nbits, from);
    if (pos < nbits)
// bit = pos;
    } else {
...
//
    ...
    let mut used_2: u32 = le32_to_cpu(hdr.used) >> 1;
    u16 esize;
    if (!e || de_is_last(e))
    return core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn index_hdr_check(hdr: *const INDEX_HDR, bytes: u32) -> bool {
    static bool index_hdr_check(const struct INDEX_HDR *hdr, u32 bytes)
    {
    let mut has_subnode: bool = hdr_has_subnode(hdr);
    const u16 min_size =
    sizeof(struct NTFS_DE) + (has_subnode ? sizeof(u64) : 0);
    let mut end: u32 = le32_to_cpu(hdr.used);
    let mut tot: u32 = le32_to_cpu(hdr.total);
    let mut off: u32 = le32_to_cpu(hdr.de_off);
...
// Ensure every key stays inside its entry before lookup walks it.
    ...
    for (;;) {
    let mut e_size: u16 = le16_to_cpu(e.size);
...
#[no_mangle]
pub unsafe extern "C" fn fnd_clear(fnd: *mut ntfs_fnd) {
    ...
    continue;
    put_indx_node(n);
    fnd.nodes[i] = core::ptr::null_mut();
    }
    fnd.level = 0;
    fnd.root_de = core::ptr::null_mut();
    }
    static int fnd_push(struct ntfs_fnd *fnd, struct indx_node *n,
    struct NTFS_DE *e)
    {
    let mut i: c_int = fnd.level;
...
#[no_mangle]
unsafe extern "C" fn fnd_is_empty(fnd: *mut ntfs_fnd) -> bool {
...
//
    ...
    NTFS_CMP_FUNC cmp)
    {
    struct NTFS_DE *e, *found = core::ptr::null_mut();
    let mut min_idx: c_int = 0, mid_idx, max_idx = 0;
    int diff2;
    let mut table_size: c_int = 8;
    u32 e_size, e_key_len;
    let mut end: u32 = le32_to_cpu(hdr.used);
    let mut off: u32 = le32_to_cpu(hdr.de_off);
    let mut total: u32 = le32_to_cpu(hdr.total);
    u16 offs[128];
    fill_table:
    if (end > total)
    return core::ptr::null_mut();
...
//
    ...
    {
    int diff;
    let mut off: usize = PtrOffset(hdr, before);
    let mut used: u32 = le32_to_cpu(hdr.used);
    let mut total: u32 = le32_to_cpu(hdr.total);
    let mut de_size: u16 = le16_to_cpu(de.size);
...
//
    ...
    {
    let mut used: u32 = le32_to_cpu(hdr.used);
    let mut esize: u16 = le16_to_cpu(re.size);
    let mut off: u32 = PtrOffset(hdr, re);
    let mut bytes: c_int = used - (off + esize);
...
#[no_mangle]
pub unsafe extern "C" fn indx_clear(indx: *mut ntfs_index) {
...
// Index record must be a multiple of cluster size.
    ...
    let mut vbo: u64 = (u64)vbn << indx.vbn2vbo_bits;
    let mut bytes: u32 = 1u << indx.index_bits;
    u16 fn;
    u32 eo;
...
// Create header.
    ...
    a = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_ROOT, in.name, in.name_len, core::ptr::null_mut(),
    mi);
    if (!a)
    return core::ptr::null_mut();
    if (attr)
...
//
    ...
    NTFS_CMP_FUNC cmp;
    if (!root)
    root = indx_get_root(&ni.dir, ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!root) {
// Should not happen.
    return -EINVAL;
    }
...
// entry = e;
    ...
    {
    int err;
    struct indx_node *n = core::ptr::null_mut();
    struct NTFS_DE *e;
    let mut iter: usize = 0;
    let mut level: c_int = fnd.level;
    if (!*entry) {
// Start find.
...
// entry = e;
    ...
    int err;
    struct indx_node *n = core::ptr::null_mut();
    struct NTFS_DE *e = core::ptr::null_mut();
    struct NTFS_DE *e2;
    size_t bit;
    CLST next_used_vbn;
    CLST next_vbn;
    let mut record_size: u32 = ni.mi.sbi.record_size;
...
//
    ...
    struct ATTRIB *alloc;
    let mut data_size: u32 = 1u << indx.index_bits;
    let mut alloc_size: u32 = ntfs_up_cluster(sbi, data_size);
    let mut len: CLST = alloc_size >> sbi.cluster_bits;
    const struct INDEX_NAMES *in = &s_index_names[indx.type];
    CLST alen;
    struct runs_tree run;
    run_init(&run);
    err = attr_allocate_clusters(sbi, &run, core::ptr::null_mut(), 0, 0, len, core::ptr::null_mut(),
    ALLOCATE_DEF, &alen, 0, core::ptr::null_mut(), core::ptr::null_mut());
    if (err)
    goto out;
    err = ni_insert_nonresident(ni, ATTR_ALLOC, in.name, in.name_len,
    &run, 0, len, 0, &alloc, core::ptr::null_mut(), core::ptr::null_mut());
    if (err)
    goto out1;
    alloc.nres.valid_size = alloc.nres.data_size = cpu_to_le64(data_size);
    err = ni_insert_resident(ni, ntfs3_bitmap_size(1), ATTR_BITMAP,
    in.name, in.name_len, &bitmap, core::ptr::null_mut(), core::ptr::null_mut());
    if (err)
    goto out2;
    if (in.name == I30_NAME) {
...
// vbn = 0;
    return 0;
    out2:
    mi_remove_attr(core::ptr::null_mut(), &ni.mi, alloc);
    out1:
...
// Increase bitmap.
    ...
    &indx.bitmap_run,
    ntfs3_bitmap_size(bit + 1), core::ptr::null_mut(), true);
    if (err)
    goto out1;
    }
    alloc = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_ALLOC, in.name, in.name_len,
    core::ptr::null_mut(), &mi);
    if (!alloc) {
    err = -EINVAL;
    if (bmp)
    goto out2;
...
//
    e = hdr_insert_de(indx, hdr, new_de, core::ptr::null_mut(), ctx, cmp);
    if (!e) {
    err = -EINVAL;
    goto out_put_n;
    }
...
//
    ...
    u16 sp_size;
    void *hdr1_saved = core::ptr::null_mut();
// Try the most easy case.
    e = fnd.level - 1 == level ? fnd.de[level] : core::ptr::null_mut();
...
//
    ...
    hdr2 :
    hdr1,
    new_de, core::ptr::null_mut(), ctx, cmp);
    indx_mark_used(indx, ni, new_vbn >> indx.idx2vbn_bits);
    indx_write(indx, ni, n1, 0);
...
// Insert split_point in root.
    err = indx_insert_into_root(indx, ni, up_e, core::ptr::null_mut(), ctx, fnd, 0,
    cmp);
    } else {
//
...
//
    ...
    int diff;
    struct NTFS_DE *e;
    struct ntfs_fnd *fnd_a = core::ptr::null_mut();
    struct INDEX_ROOT *root;
    NTFS_CMP_FUNC cmp;
    if (!fnd) {
    fnd_a = fnd_get();
    ...
    root = indx_get_root(indx, ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!root) {
    err = -EINVAL;
    goto out;
    }
...
//
    if (depth > ARRAY_SIZE(((struct ntfs_fnd *)core::ptr::null_mut()).nodes))
...
//
    ...
    let mut used: usize = MINUS_ONE_T;
...
//
    ...
    struct indx_node *n, *n2d = core::ptr::null_mut();
    __le64 sub_vbn;
    int level, level2;
    struct ATTRIB *attr;
    struct mft_inode *mi;
...
// Do slow search from root.
    struct indx_node *in;
    fnd_clear(fnd);
    in = indx_find_buffer(indx, ni, root, sub_vbn, core::ptr::null_mut(), 0);
    if (IS_ERR(in)) {
    err = PTR_ERR(in);
    goto out;
    }
    if (in)
    fnd_push(fnd, in, core::ptr::null_mut());
    }
// Merge fnd2 -> fnd.
    for (level = 0; level < fnd2.level; level++) {
    fnd_push(fnd, fnd2.nodes[level], fnd2.de[level]);
    fnd2.nodes[level] = core::ptr::null_mut();
    }
    fnd2.level = 0;
...
//
    ...
    err = attr_set_size(ni, ATTR_ALLOC, in.name, in.name_len,
    &indx.alloc_run, 0, core::ptr::null_mut(), false);
    if (in.name == I30_NAME)
    ...
    run_close(&indx.alloc_run);
    err = attr_set_size(ni, ATTR_BITMAP, in.name, in.name_len,
    &indx.bitmap_run, 0, core::ptr::null_mut(), false);
...
//
    ...
    if (!fnd)
    return -ENOMEM;
    root = indx_get_root(indx, ni, core::ptr::null_mut(), &mi);
    if (!root) {
    err = -EINVAL;
    goto out;
    }
