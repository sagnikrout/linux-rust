...
//
// clang-format off
pub const MaxLogFileSize: c_uint = 0x100000000ull;
pub const DefaultLogPageSize: c_int = 4096;
pub const MinLogRecordPages: c_uint = 0x30;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct RESTART_HDR {
    pub 'RSTR': NTFS_RECORD_HEADER rhdr; //,
    pub log.: __le32 sys_page_size; // 0x10: Page size of the system which initialized the,
    pub file.: __le32 page_size; // 0x14: Log page size used for this log,
    pub 0x18:: __le16 ra_off; //,
    pub 0x1A:: __le16 minor_ver; //,
    pub 0x1C:: __le16 major_ver; //,
    pub fixups: [__le16; ],
}

pub const LFS_NO_CLIENT: c_uint = 0xffff;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct CLIENT_REC {
    pub oldest_lsn: __le64,
    pub 0x08:: __le64 restart_lsn; //,
    pub 0x10:: __le16 prev_client; //,
    pub 0x12:: __le16 next_client; //,
    pub 0x14:: __le16 seq_num; //,
    pub 0x16:: u8 align[6]; //,
    pub bytes.: __le32 name_bytes; // 0x1C: In,
    pub client.: __le16 name[64]; // 0x20: Name of,
}
...
pub struct RESTART_AREA {
    pub file.: __le64 current_lsn; // 0x00: Current logical end of log,
    pub clients.: __le16 log_clients; // 0x08: Maximum number of,
    pub arrays.: __le16 client_idx[2]; // 0x0A: Free/use index into the client record,
    pub RESTART_SINGLE_PAGE_IO.: __le16 flags; // 0x0E: See,
    pub number.: __le32 seq_num_bits; // 0x10: The number of bits in sequence,
    pub 0x14:: __le16 ra_len; //,
    pub 0x16:: __le16 client_off; //,
    pub size.: __le64 l_size; // 0x18: Usable log file,
    pub 0x20:: __le32 last_lsn_data_len; //,
    pub offset.: __le16 rec_hdr_len; // 0x24: Log page data,
    pub length.: __le16 data_off; // 0x26: Log page data,
    pub 0x28:: __le32 open_log_count; //,
    pub 0x2C:: __le32 align[5]; //,
    pub 0x40:: CLIENT_REC clients[]; //,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct LOG_REC_HDR {
    pub NTFS_LOG_OPERATION: __le16 redo_op; // 0x00:,
    pub NTFS_LOG_OPERATION: __le16 undo_op; // 0x02:,
    pub record.: __le16 redo_off; // 0x04: Offset to Redo,
    pub length.: __le16 redo_len; // 0x06: Redo,
    pub record.: __le16 undo_off; // 0x08: Offset to Undo,
    pub length.: __le16 undo_len; // 0x0A: Undo,
    pub 0x0C:: __le16 target_attr; //,
    pub 0x0E:: __le16 lcns_follow; //,
    pub 0x10:: __le16 record_off; //,
    pub 0x12:: __le16 attr_off; //,
    pub 0x14:: __le16 cluster_off; //,
    pub 0x16:: __le16 reserved; //,
    pub 0x18:: __le64 target_vcn; //,
    pub 0x20:: __le64 page_lcns[]; //,
}

    static_assert(sizeof(struct LOG_REC_HDR) == 0x20);
pub const RESTART_ENTRY_ALLOCATED: c_uint = 0xFFFFFFFF;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct RESTART_TABLE {
    pub bytes: __le16 size; // 0x00: In,
    pub Entries: __le16 used; // 0x02:,
    pub Entries: __le16 total; // 0x04:,
    pub 0x06:: __le16 res[3]; //,
    pub 0x0C:: __le32 free_goal; //,
    pub 0x10:: __le32 first_free; //,
    pub 0x14:: __le32 last_free; //,
}

    static_assert(sizeof(struct RESTART_TABLE) == 0x18);
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ATTR_NAME_ENTRY {
    pub Table.: __le16 off; // Offset in the Open attribute,
    pub name_bytes: __le16,
    pub name: [__le16; ],
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct OPEN_ATTR_ENRTY {
    pub allocated: __le32 next; // 0x00: RESTART_ENTRY_ALLOCATED if,
    pub 0x04:: __le32 bytes_per_index; //,
    pub 0x08:: enum ATTR_TYPE type; //,
    pub 0x0C:: u8 is_dirty_pages; //,
    pub 'ptr': u8 is_attr_name; // 0x0B: Faked field to manage,
    pub 'ptr': u8 name_len; // 0x0C: Faked field to manage,
    pub res: u8,
    pub attribute: MFT_REF ref; // 0x10: File Reference of file containing,
    pub 0x18:: __le64 open_record_lsn; //,
    pub 0x20:: *mut *mut void ptr; //,
}

// 32 bit version of 'struct OPEN_ATTR_ENRTY'
#[repr(C)]
#[derive(Copy, Clone)]
pub struct OPEN_ATTR_ENRTY_32 {
    pub allocated: __le32 next; // 0x00: RESTART_ENTRY_ALLOCATED if,
    pub 0x04:: __le32 ptr; //,
    pub 0x08:: MFT_REF ref; //,
    pub 0x10:: __le64 open_record_lsn; //,
    pub 0x18:: u8 is_dirty_pages; //,
    pub 0x19:: u8 is_attr_name; //,
    pub res1: [u8; 2],
    pub 0x1C:: enum ATTR_TYPE type; //,
    pub wchar: u8 name_len; // 0x20: In,
    pub res2: [u8; 3],
    pub 0x24:: __le32 AttributeName; //,
    pub 0x28:: __le32 bytes_per_index; //,
}

pub const SIZEOF_OPENATTRIBUTEENTRY0: c_uint = 0x2c;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct DIR_PAGE_ENTRY {
    pub allocated: __le32 next; // 0x00: RESTART_ENTRY_ALLOCATED if,
    pub Table: __le32 target_attr; // 0x04: Index into the Open attribute,
    pub 0x08:: __le32 transfer_len; //,
    pub 0x0C:: __le32 lcns_follow; //,
    pub page: __le64 vcn; // 0x10: Vcn of dirty,
    pub 0x18:: __le64 oldest_lsn; //,
    pub 0x20:: __le64 page_lcns[]; //,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct DIR_PAGE_ENTRY_32 {
    pub allocated: __le32 next; // 0x00: RESTART_ENTRY_ALLOCATED if,
    pub Table: __le32 target_attr; // 0x04: Index into the Open attribute,
    pub 0x08:: __le32 transfer_len; //,
    pub 0x0C:: __le32 lcns_follow; //,
    pub 0x10:: __le32 reserved; //,
    pub page: __le32 vcn_low; // 0x14: Vcn of dirty,
    pub page: __le32 vcn_hi; // 0x18: Vcn of dirty,
    pub 0x1C:: __le32 oldest_lsn_low; //,
    pub 0x1C:: __le32 oldest_lsn_hi; //,
    pub 0x24:: __le32 page_lcns_low; //,
    pub 0x24:: __le32 page_lcns_hi; //,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct TRANSACTION_ENTRY {
    pub allocated: __le32 next; // 0x00: RESTART_ENTRY_ALLOCATED if,
    pub 0x04:: u8 transact_state; //,
    pub 0x05:: u8 reserved[3]; //,
    pub 0x08:: __le64 first_lsn; //,
    pub 0x10:: __le64 prev_lsn; //,
    pub 0x18:: __le64 undo_next_lsn; //,
    pub abort: __le32 undo_records; // 0x20: Number of undo log records pending,
    pub size: __le32 undo_len; // 0x24: Total undo,
}

    static_assert(sizeof(struct TRANSACTION_ENTRY) == 0x28);
#[repr(C)]
#[derive(Copy, Clone)]
pub struct NTFS_RESTART {
    pub 0x00:: __le32 major_ver; //,
    pub 0x04:: __le32 minor_ver; //,
    pub 0x08:: __le64 check_point_start; //,
    pub 0x10:: __le64 open_attr_table_lsn; //,
    pub 0x18:: __le64 attr_names_lsn; //,
    pub 0x20:: __le64 dirty_pages_table_lsn; //,
    pub 0x28:: __le64 transact_table_lsn; //,
    pub bytes: __le32 open_attr_len; // 0x30: In,
    pub bytes: __le32 attr_names_len; // 0x34: In,
    pub bytes: __le32 dirty_pages_len; // 0x38: In,
    pub bytes: __le32 transact_table_len; // 0x3C: In,
}

    static_assert(sizeof(struct NTFS_RESTART) == 0x40);
#[repr(C)]
#[derive(Copy, Clone)]
pub struct NEW_ATTRIBUTE_SIZES {
    pub alloc_size: __le64,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct BITMAP_RANGE {
    pub bitmap_off: __le32,
    pub bits: __le32,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct LCN_RANGE {
    pub lcn: __le64,
    pub len: __le64,
}
...
pub struct LFS_RECORD_HDR {
    pub 0x00:: __le64 this_lsn; //,
    pub 0x08:: __le64 client_prev_lsn; //,
    pub 0x10:: __le64 client_undo_next_lsn; //,
    pub 0x18:: __le32 client_data_len; //,
    pub record.: CLIENT_ID client; // 0x1C: Owner of this log,
    pub LfsClientRestart.: __le32 record_type; // 0x20: LfsClientRecord or,
    pub 0x24:: __le32 transact_id; //,
    pub LOG_RECORD_MULTI_PAGE: __le16 flags; // 0x28:,
    pub 0x2A:: u8 align[6]; //,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct LFS_RECORD {
    pub page,: __le16 next_record_off; // 0x00: Offset of the free space in the,
    pub 0x02:: u8 align[6]; //,
    pub page,: __le64 last_end_lsn; // 0x08: lsn for the last log record which ends on the,
}
...
pub struct RECORD_PAGE_HDR {
    pub 'RCRD': NTFS_RECORD_HEADER rhdr; //,
    pub LOG_PAGE_LOG_RECORD_END: __le32 rflags; // 0x10: See,
    pub 0x14:: __le16 page_count; //,
    pub 0x16:: __le16 page_pos; //,
    pub 0x18:: LFS_RECORD record_hdr; //,
    pub 0x28:: __le16 fixups[10]; //,
    pub 2: __le32 file_off; // 0x3c: Used when major version >=,
}
...
#[no_mangle]
pub unsafe extern "C" fn is_log_record_end(hdr: *const RECORD_PAGE_HDR) -> bool {
...
pub const INITIAL_NUMBER_TRANSACTIONS: c_int = 5;
...
#[no_mangle]
pub unsafe extern "C" fn is_target_required(op: u16) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn can_skip_action(op: enum NTFS_LOG_OPERATION) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn bytes_per_rt(rt: *const RESTART_TABLE) -> u32 {
...
// Log record length.
#[no_mangle]
pub unsafe extern "C" fn lrh_length(lr: *const LOG_REC_HDR) -> u32 {
    static inline u32 lrh_length(const struct LOG_REC_HDR *lr)
    {
    let mut t16: u16 = le16_to_cpu(lr.lcns_follow);
    return struct_size(lr, page_lcns, max_t(u16, 1, t16));
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lcb {
    pub lsn.: *mut *mut LFS_RECORD_HDR lrh; // Log record header of the current,
    pub log_rec: *mut LOG_REC_HDR,
    pub lcb_ctx_undo_next/lcb_ctx_prev/lcb_ctx_next: u32 ctx_mode; //,
    pub client: CLIENT_ID,
    pub 'log_rec'.: bool alloc; // If true the we should deallocate,
}

#[no_mangle]
unsafe extern "C" fn lcb_put(lcb: *mut lcb) {
...
// Find the oldest lsn from active clients.
    ...
    let mut lsn: u64 = le64_to_cpu(cr.oldest_lsn);
...
// oldest_lsn = lsn;
    ...
    {
    let mut sys_page: u32 = le32_to_cpu(rhdr.sys_page_size);
    let mut page_size: u32 = le32_to_cpu(rhdr.page_size);
    u32 end_usa;
    u16 ro;
...
#[no_mangle]
pub unsafe extern "C" fn is_rst_area_valid(rhdr: *const RESTART_HDR) -> bool {
    static inline bool is_rst_area_valid(const struct RESTART_HDR *rhdr)
    {
    const struct RESTART_AREA *ra;
    u16 cl, fl, ul;
    u32 off, l_size, seq_bits;
    let mut ro: u16 = le16_to_cpu(rhdr.ra_off);
    let mut sys_page: u32 = le32_to_cpu(rhdr.sys_page_size);
...
// The log page data offset and record header length must be quad-aligned.
    ...
    let mut ra_len: u16 = le16_to_cpu(ra.ra_len);
...
//
    for (i = 0; i < 2; i++) {
    let mut client_idx: u16 = le16_to_cpu(ra.client_idx[i]);
    let mut first_client: bool = true;
    let mut clients: u16 = le16_to_cpu(ra.log_clients);
...
//
#[no_mangle]
pub unsafe extern "C" fn add_client(ca: *mut CLIENT_REC, index: u16, head: *mut __le16) {
...
//
#[no_mangle]
pub unsafe extern "C" fn dp_range_ok(j: usize, count: u32, cap: u32) -> bool {
...
//
    ...
    {
    let mut ta: __le32 = cpu_to_le32(target_attr);
    struct DIR_PAGE_ENTRY *dp = core::ptr::null_mut();
    while ((dp = enum_rstbl(dptbl, dp))) {
    let mut dp_vcn: u64 = le64_to_cpu(dp.vcn);
...
#[no_mangle]
pub unsafe extern "C" fn norm_file_page(page_size: u32, l_size: *mut u32, use_default: bool) -> u32 {
...
#[no_mangle]
unsafe extern "C" fn check_rstbl(rt: *const RESTART_TABLE, bytes: usize) -> bool {
    static bool check_rstbl(const struct RESTART_TABLE *rt, size_t bytes)
    {
    u32 ts;
    u32 i, off;
    let mut rsize: u16 = le16_to_cpu(rt.size);
    let mut ne: u16 = le16_to_cpu(rt.used);
    let mut ff: u32 = le32_to_cpu(rt.first_free);
    let mut lf: u32 = le32_to_cpu(rt.last_free);
...
#[no_mangle]
unsafe extern "C" fn check_dp_table(dptbl: *const RESTART_TABLE) -> bool {
    static bool check_dp_table(const struct RESTART_TABLE *dptbl)
    {
    let mut rsize: u32 = le16_to_cpu(dptbl.size);
    struct DIR_PAGE_ENTRY *dp = core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn free_rsttbl_idx(rt: *mut RESTART_TABLE, off: u32) {
    static inline void free_rsttbl_idx(struct RESTART_TABLE *rt, u32 off)
    {
    __le32 *e;
    let mut lf: u32 = le32_to_cpu(rt.last_free);
    let mut off_le: __le32 = cpu_to_le32(off);
    e = Add2Ptr(rt, off);
...
// e = 0;
    ...
    __le32 *e, *last_free;
    u32 off;
    let mut bytes: u32 = esize * used + sizeof(struct RESTART_TABLE);
    let mut lf: u32 = sizeof(struct RESTART_TABLE) + (used - 1) * esize;
...
// e = cpu_to_le32(off);
    ...
    {
    let mut esize: u16 = le16_to_cpu(tbl.size);
    let mut osize: __le32 = cpu_to_le32(bytes_per_rt(tbl));
    let mut used: u32 = le16_to_cpu(tbl.used);
...
//
    ...
    let mut bytes: u32 = bytes_per_rt(rt);
    let mut esize: u16 = le16_to_cpu(rt.size);
...
//
    let mut bytes2idx: u32 = vbo - bytes;
//
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct restart_info {
    pub last_lsn: u64,
    pub r_page: *mut RESTART_HDR,
    pub vbo: u32,
    pub chkdsk_was_run: bool,
    pub valid_page: bool,
    pub initialized: bool,
    pub restart: bool,
}

pub const NTFSLOG_WRAPPED: c_uint = 0x00000001;
pub const NTFSLOG_MULTIPLE_PAGE_IO: c_uint = 0x00000002;
pub const NTFSLOG_NO_LAST_LSN: c_uint = 0x00000004;
pub const NTFSLOG_REUSE_TAIL: c_uint = 0x00000010;
pub const NTFSLOG_NO_OLDEST_LSN: c_uint = 0x00000020;
...
pub struct ntfs_log {
    ...
    pub sys_page_size: u32,
    pub sys_page_mask: u32,
    pub page_size: u32,
    pub 1: u32 page_mask; // page_size -,
    pub page_bits: u8,
    pub one_page_buf: *mut RECORD_PAGE_HDR,
    pub open_attr_tbl: *mut RESTART_TABLE,
    pub transaction_id: u32,
    pub clst_per_page: u32,
    pub first_page: u32,
    pub next_page: u32,
    pub ra_off: u32,
    pub data_off: u32,
    pub restart_size: u32,
    pub data_size: u32,
    pub record_header_len: u16,
    pub seq_num: u64,
    pub seq_num_bits: u32,
    pub file_data_bits: u32,
    pub /: *mut *mut u32 seq_num_mask; / (1 << file_data_bits) - 1,
    pub /: *mut *mut *mut RESTART_AREA ra; / In-memory image of the next restart area.,
    pub /: *mut *mut u32 ra_size; / The usable size of the restart area.,
//
// If true, then the in-memory restart area is to be written
// to the first position on the disk.
//
    pub init_ra: bool,
    pub /: *mut *mut bool set_dirty; / True if we need to set dirty flag.,
    pub oldest_lsn: u64,
    pub oldest_lsn_off: u32,
    pub last_lsn: u64,
    pub total_avail: u32,
    pub total_avail_pages: u32,
    pub total_undo_commit: u32,
    pub max_current_avail: u32,
    pub current_avail: u32,
    pub reserved: u32,
    pub major_ver: c_short,
    pub minor_ver: c_short,
    pub /: *mut *mut u32 l_flags; / See NTFSLOG_XXX,
    pub /: *mut *mut u32 current_openlog_count; / On-disk value for open_log_count.,
    pub client_id: CLIENT_ID,
    pub client_undo_commit: u32,
    pub rst_info2: restart_info rst_info,,
    pub read_ahead: file_ra_state,
}

#[no_mangle]
pub unsafe extern "C" fn lsn_to_vbo(log: *mut ntfs_log, lsn: u64) -> u32 {
    static inline u32 lsn_to_vbo(struct ntfs_log *log, const u64 lsn)
    {
    let mut vbo: u32 = (lsn << log.seq_num_bits) >> (log.seq_num_bits - 3);
    return vbo;
    }
// Compute the offset in the log file of the next log page.
#[no_mangle]
pub unsafe extern "C" fn next_page_off(log: *mut ntfs_log, off: u32) -> u32 {
...
#[no_mangle]
pub unsafe extern "C" fn lsn_to_page_off(log: *mut ntfs_log, lsn: u64) -> u32 {
    static inline u32 lsn_to_page_off(struct ntfs_log *log, u64 lsn)
    {
    return (((u32)lsn) << 3) & log.page_mask;
    }
#[no_mangle]
pub unsafe extern "C" fn vbo_to_lsn(log: *mut ntfs_log, off: u32, Seq: u64) -> u64 {
    static inline u64 vbo_to_lsn(struct ntfs_log *log, u32 off, u64 Seq)
    {
    return (off >> 3) + (Seq << log.file_data_bits);
    }
#[no_mangle]
pub unsafe extern "C" fn is_lsn_in_file(log: *mut ntfs_log, lsn: u64) -> bool {
    ...
    {
    let mut h_lsn: u64 = le64_to_cpu(hdr.rhdr.lsn);
    ...
    {
    let mut err: c_int = 0;
    let mut page_idx: u32 = vbo >> log.page_bits;
    let mut page_off: u32 = vbo & log.page_mask;
    let mut bytes: u32 = log.page_size - page_off;
    void *to_free = core::ptr::null_mut();
    let mut page_vbo: u32 = page_idx << log.page_bits;
...
// buffer = to_free;
    ...
    log.page_size, core::ptr::null_mut(), &log.read_ahead);
    if (err)
    goto out;
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: bBAAD) -> else {
    else if (bBAAD)
    err = -EINVAL;
    out:
    if (err && to_free) {
...
//
    ...
    u32 skip;
    u64 vbo;
    struct RESTART_HDR *r_page = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn log_init_pg_hdr(log: *mut ntfs_log, major_ver: u16, minor_ver: u16) {
...
#[no_mangle]
unsafe extern "C" fn final_log_off(log: *mut ntfs_log, lsn: u64, data_len: u32) -> u32 {
    static u32 final_log_off(struct ntfs_log *log, u64 lsn, u32 data_len)
    {
    let mut base_vbo: u32 = lsn << 3;
    let mut final_log_off: u32 = (base_vbo & log.seq_num_mask) & ~log.page_mask;
    let mut page_off: u32 = base_vbo & log.page_mask;
    let mut tail: u32 = log.page_size - page_off;
    page_off -= 1;
...
//
    ...
    {
    int err;
    let mut this_lsn: u64 = le64_to_cpu(rh.this_lsn);
    let mut vbo: u32 = lsn_to_vbo(log, this_lsn);
    u32 end =
    final_log_off(log, this_lsn, le32_to_cpu(rh.client_data_len));
    let mut hdr_off: u32 = end & ~log.sys_page_mask;
    let mut seq: u64 = this_lsn >> log.file_data_bits;
    struct RECORD_PAGE_HDR *page = core::ptr::null_mut();
// Remember if we wrapped.
    if (end <= vbo)
    seq += 1;
// Log page header for this page.
    err = read_log_page(log, hdr_off, &page, core::ptr::null_mut());
    if (err)
    return err;
//
...
//
#[no_mangle]
unsafe extern "C" fn current_log_avail(log: *mut ntfs_log) -> u32 {
...
//
    ...
    let mut lsn: u64 = le64_to_cpu(rhdr.lsn);
...
//
#[no_mangle]
unsafe extern "C" fn last_log_lsn(log: *mut ntfs_log) -> c_int {
    static int last_log_lsn(struct ntfs_log *log)
    {
    int err;
    let mut usa_error: bool = false;
    let mut replace_page: bool = false;
    let mut reuse_page: bool = log.l_flags & NTFSLOG_REUSE_TAIL;
    bool wrapped_file, wrapped;
    let mut page_cnt: u32 = 1, page_pos = 1;
    let mut page_off: u32 = 0, page_off1 = 0, saved_off = 0;
    u32 final_off, second_off, final_off_prev = 0, second_off_prev = 0;
    let mut first_file_off: u32 = 0, second_file_off = 0;
    let mut part_io_count: u32 = 0;
    let mut tails: u32 = 0;
    ...
    let mut last_ok_lsn: u64 = reuse_page ? log.last_lsn : 0;
    u16 cur_pos, best_page_pos;
    struct RECORD_PAGE_HDR *page = core::ptr::null_mut();
    struct RECORD_PAGE_HDR *tst_page = core::ptr::null_mut();
    struct RECORD_PAGE_HDR *first_tail = core::ptr::null_mut();
    struct RECORD_PAGE_HDR *second_tail = core::ptr::null_mut();
    struct RECORD_PAGE_HDR *tail_page = core::ptr::null_mut();
    struct RECORD_PAGE_HDR *second_tail_prev = core::ptr::null_mut();
    struct RECORD_PAGE_HDR *first_tail_prev = core::ptr::null_mut();
    struct RECORD_PAGE_HDR *page_bufs = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: first_tail) -> else {
    else if (first_tail)
    best_page = 0;
    else
    best_page = 1;
    ...
    best_page_pos == 1) &&
    (page_pos >= page_cnt || best_page_pos == page_pos)) {
    let mut bppc: u16 = le16_to_cpu(best_page.page_count);
...
// We have a valid file.
    ...
    while (tails--) {
    let mut off: u64 = hdr_file_off(log, tmp_page);
    if (!page) {
...
//
    ...
    {
    int err;
    struct RECORD_PAGE_HDR *ph = core::ptr::null_mut();
    let mut lsn: u64 = le64_to_cpu(rh.this_lsn);
    let mut vbo: u32 = lsn_to_vbo(log, lsn) & ~log.page_mask;
    let mut off: u32 = lsn_to_page_off(log, lsn) + log.record_header_len;
    let mut data_len: u32 = le32_to_cpu(rh.client_data_len);
//
...
//
    ...
    {
    int err;
    struct LFS_RECORD_HDR *rh = core::ptr::null_mut();
    const struct CLIENT_REC *cr =
...
// If the client doesn't have a restart area, go ahead and exit now.
    ...
    (struct RECORD_PAGE_HDR **)&rh, core::ptr::null_mut());
    if (err)
    return err;
    rst = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn find_log_rec(log: *mut ntfs_log, lsn: u64, lcb: *mut lcb) -> c_int {
...
// If beyond the end of the current page -> an error.
    let mut page_off: u32 = lsn_to_page_off(log, lsn);
...
//
#[no_mangle]
unsafe extern "C" fn find_client_next_lsn(log: *mut ntfs_log, lcb: *mut lcb, lsn: *mut u64) -> c_int {
...
// Loop as long as another lsn can be found.
    ...
    (struct RECORD_PAGE_HDR **)&hdr, core::ptr::null_mut());
    if (err)
    goto out;
...
#[no_mangle]
pub unsafe extern "C" fn if(lcb->ctx_mode: lcb_ctx_prev ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn read_next_log_rec(log: *mut ntfs_log, lcb: *mut lcb, lsn: *mut u64) -> c_int {
    ...
    if (lcb.alloc)
    kfree(lcb.log_rec);
    lcb.log_rec = core::ptr::null_mut();
    lcb.alloc = false;
    kfree(lcb.lrh);
    lcb.lrh = core::ptr::null_mut();
    return find_log_rec(log, *lsn, lcb);
    }
#[no_mangle]
pub unsafe extern "C" fn check_index_header(hdr: *const INDEX_HDR, bytes: usize) -> bool {
    bool check_index_header(const struct INDEX_HDR *hdr, size_t bytes)
    {
    let mut has_subnode: bool = hdr_has_subnode(hdr);
    __le16 mask;
    ...
    let mut esize: u16 = le16_to_cpu(e.size);
    let mut key_size: u16 = le16_to_cpu(e.key_size);
    u16 data_size;
...
#[no_mangle]
pub unsafe extern "C" fn check_index_buffer(ib: *const INDEX_BUFFER, bytes: u32) -> bool {
    ...
    sbi.cluster_bits :
    SECTOR_SHIFT;
    let mut block_clst: u8 = root.index_block_clst;
    ...
    struct ntfs_sb_info *sbi)
    {
    let mut asize: u32 = le32_to_cpu(attr.size);
    let mut rsize: u32 = 0;
    u64 dsize, svcn, evcn;
    u16 run_off;
...
// Check the attribute fields.
    ...
    if (run_off > asize)
    return false;
    if (run_unpack(core::ptr::null_mut(), sbi, 0, svcn, evcn, svcn,
    ...
    {
    const struct ATTRIB *attr;
    let mut fo: u16 = le16_to_cpu(rec.rhdr.fix_off);
    let mut fn: u16 = le16_to_cpu(rec.rhdr.fix_num);
    let mut ao: u16 = le16_to_cpu(rec.attr_off);
    let mut rs: u32 = sbi.record_size;
    let mut used: u32 = le32_to_cpu(rec.used);
...
//
    ...
    {
    let mut ro: u16 = le16_to_cpu(lrh.record_off);
    let mut o: u16 = le16_to_cpu(rec.attr_off);
    ...
    let mut o: return = = ro && attr.type == ATTR_ROOT;
    }
    static inline bool check_if_root_index(const struct ATTRIB *attr,
    const struct INDEX_HDR *hdr,
    const struct LOG_REC_HDR *lrh)
    {
    let mut ao: u16 = le16_to_cpu(lrh.attr_off);
    let mut de_off: u32 = le32_to_cpu(hdr.de_off);
    let mut o: u32 = PtrOffset(attr, hdr) + de_off;
    const struct NTFS_DE *e = Add2Ptr(hdr, de_off);
    let mut asize: u32 = le32_to_cpu(attr.size);
    while (o < ao) {
    u16 esize;
    ...
    {
    let mut de_off: u32 = le32_to_cpu(hdr.de_off);
    let mut o: u32 = offsetof(struct INDEX_BUFFER, ihdr) + de_off;
    const struct NTFS_DE *e = Add2Ptr(hdr, de_off);
    let mut used: u32 = le32_to_cpu(hdr.used);
    while (o < attr_off) {
    u16 esize;
    if (de_off >= used)
    break;
...
//
    ...
    {
    struct OPEN_ATTR_ENRTY *oe = core::ptr::null_mut();
    ...
    {
    struct ATTRIB *attr;
    let mut name_size: u32 = ALIGN(name_len * sizeof(short), 8);
    let mut is_ext: bool = flags & (ATTR_FLAG_COMPRESSED | ATTR_FLAG_SPARSED);
    u32 asize = name_size +
...
//
#[no_mangle]
unsafe extern "C" fn update_oa_attr(oa2: *mut OpenAttr, attr: *mut ATTRIB) -> bool {
...
//
    ...
    struct inode *inode = core::ptr::null_mut(), *inode_parent;
    struct mft_inode *mi = core::ptr::null_mut(), *mi2_child = core::ptr::null_mut();
    let mut rno: CLST = 0, rno_base = 0;
    struct INDEX_BUFFER *ib = core::ptr::null_mut();
    struct MFT_REC *rec = core::ptr::null_mut();
    struct ATTRIB *attr = core::ptr::null_mut(), *attr2;
    ...
    u16 id, id2;
    let mut record_size: u32 = sbi.record_size;
    u64 t64;
    let mut roff: u16 = le16_to_cpu(lrh.record_off);
    let mut aoff: u16 = le16_to_cpu(lrh.attr_off);
    let mut lco: u64 = 0;
    let mut cbo: u64 = (u64)le16_to_cpu(lrh.cluster_off) << SECTOR_SHIFT;
    let mut tvo: u64 = le64_to_cpu(lrh.target_vcn) << sbi.cluster_bits;
    let mut vbo: u64 = cbo + tvo;
    void *buffer_le = core::ptr::null_mut();
    let mut bytes: u32 = 0;
    let mut a_dirty: bool = false;
    u16 data_off;
    oa = oe.ptr;
...
// Read from disk.
    ...
    goto out;
    if (!check_file_record(rec, core::ptr::null_mut(), sbi))
    goto dirty_vol;
    ...
    inode_parent = ntfs_iget5(sbi.sb, &rec.parent_ref, core::ptr::null_mut());
    ...
    skip_load_parent:
    inode_parent = core::ptr::null_mut();
    break;
//
...
//
    ...
    data = core::ptr::null_mut(); // To skip below memmove().
    }
...
// See UpdateRecordDataRoot for the rationale.
    ...
    if (rlsn) {
    let mut t64: __le64 = cpu_to_le64(*rlsn);
    if (rec)
    rec.rhdr.lsn = t64;
    if (ib)
    ib.rhdr.lsn = t64;
    ...
    iput(inode);
#[no_mangle]
pub unsafe extern "C" fn if(mi2_child: mi !=) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn log_replay(ni: *mut ntfs_inode, initialized: *mut bool) -> c_int {
    ...
    struct ATTR_NAME_ENTRY *attr_names = core::ptr::null_mut();
    let mut attr_names_bytes: u32 = 0;
    let mut oatbl_bytes: u32 = 0;
    struct RESTART_TABLE *dptbl = core::ptr::null_mut();
    struct RESTART_TABLE *trtbl = core::ptr::null_mut();
    const struct RESTART_TABLE *rt;
    struct RESTART_TABLE *oatbl = core::ptr::null_mut();
    struct inode *inode;
    struct OpenAttr *oa;
    struct ntfs_inode *ni_oe;
    struct ATTRIB *attr = core::ptr::null_mut();
    u64 size, vcn, undo_next_lsn;
    CLST rno, lcn, lcn0, len0, clen;
    void *data;
    struct NTFS_RESTART *rst = core::ptr::null_mut();
    struct lcb *lcb = core::ptr::null_mut();
    ...
    struct RESTART_AREA *ra2, *ra = core::ptr::null_mut();
    ...
    const struct LOG_REC_HDR *lrh;
    bool is_mapped;
    let mut is_ro: bool = sb_rdonly(sbi.sb);
    u64 t64;
    u16 t16;
    u32 t32;
...
// Determine which restart area to use.
    ...
    struct RECORD_PAGE_HDR *sp = core::ptr::null_mut();
    bool usa_error;
    ...
    log.rst_info2.r_page = core::ptr::null_mut();
    }
    use_first_page:
...
// If we have a valid page then grab a pointer to the restart area.
    ...
    let mut use_multi_page: bool = false;
    u32 open_log_count;
...
//
    ...
    let mut old_page_size: u32 = log.page_size;
...
// Convert Ra version '0' into version '1'.
    ...
    let mut lcns: u32 = le32_to_cpu(dp.lcns_follow);
    if (lcns > t32) {
    log.set_dirty = true;
    goto out;
    }
...
// Loop through all of the Lcn ranges this log record.
    ...
    let mut lcn0: u64 = le64_to_cpu(r.lcn);
    let mut lcn_e: u64 = lcn0 + le64_to_cpu(r.len) - 1;
    dp = core::ptr::null_mut();
    while ((dp = enum_rstbl(dptbl, dp))) {
    u32 j;
    t32 = le32_to_cpu(dp.lcns_follow);
...
//
    let mut new_e: u32 = t16 / le16_to_cpu(oatbl.size);
...
// Reopen all of the attributes with dirty pages.
    ...
    inode = ntfs_iget5(sbi.sb, &oe.ref, core::ptr::null_mut());
    if (IS_ERR(inode))
    goto fake_attr;
    ...
    ni_oe = ntfs_i(inode);
    oa.ni = ni_oe;
    attr = ni_find_attr(ni_oe, core::ptr::null_mut(), core::ptr::null_mut(), oe.type, oe.ptr, oe.name_len,
    ...
    if (attr.non_res) {
    let mut roff: u16 = le16_to_cpu(attr.nres.run_off);
    let mut svcn: CLST = le64_to_cpu(attr.nres.svcn);
    if (roff > t32) {
    kfree(oa.attr);
    oa.attr = core::ptr::null_mut();
    goto fake_attr;
    }
...
//
    ...
    if ((!run_lookup_entry(oa.run1, vcn, &lcn0, &len0, core::ptr::null_mut()) ||
    lcn0 != lcn) &&
...
// Ignore log records that do not update pages.
    ...
    if (!run_lookup_entry(oa.run1, vcn, &lcn, core::ptr::null_mut(), core::ptr::null_mut()) ||
    lcn == SPARSE_LCN) {
    goto read_next_log_do_action;
    }
...
//
    ...
    dlen = 0;
#[no_mangle]
pub unsafe extern "C" fn if(alen: voff + dlen >) -> else {
...
// It is time to apply the undo action.
    err = do_action(log, oe, lrh, t16, data, dlen, rec_len, core::ptr::null_mut());
...
//
    ...
    err = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: log->set_dirty) -> else {
