...
// First, compare the type codes.
    let mut diff: c_int = le32_to_cpu(left.type) - le32_to_cpu(type);
    if (diff)
    return diff;
...
//
#[no_mangle]
unsafe extern "C" fn mi_new_attt_id(ni: *mut ntfs_inode, mi: *mut mft_inode) -> __le16 {
...
#[no_mangle]
pub unsafe extern "C" fn mi_get(sbi: *mut ntfs_sb_info, rno: CLST, mi: *mut mft_inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mi_put(mi: *mut mft_inode) {
...
#[no_mangle]
pub unsafe extern "C" fn mi_init(mi: *mut mft_inode, sbi: *mut ntfs_sb_info, rno: CLST) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn mi_read(mi: *mut mft_inode, is_mft: bool) -> c_int {
    int mi_read(struct mft_inode *mi, bool is_mft)
    {
    int err;
    struct MFT_REC *rec = mi.mrec;
    struct ntfs_sb_info *sbi = mi.sbi;
    let mut bpr: u32 = sbi.record_size;
    let mut vbo: u64 = (u64)mi.rno << sbi.record_bits;
    struct ntfs_inode *mft_ni = sbi.mft.ni;
    struct runs_tree *run = mft_ni ? &mft_ni.file.run : core::ptr::null_mut();
    struct rw_semaphore *rw_lock = core::ptr::null_mut();
    if (is_mounted(sbi)) {
    if (!is_mft && mft_ni) {
    ...
    err = attr_load_runs_vcn(mft_ni, ATTR_DATA, core::ptr::null_mut(), 0, run,
...
//
    ...
    if (!attr) {
    let mut total: u32 = le32_to_cpu(rec.total);
...
#[no_mangle]
pub unsafe extern "C" fn mi_write(mi: *mut mft_inode, wait: c_int) -> c_int {
    ...
    let mut seq: u16 = 1;
    struct MFT_REC *rec;
    let mut vbo: u64 = (u64)rno << sbi.record_bits;
...
//
    ...
    let mut diff: c_int = compare_attr(attr, type, name, name_len, upcase);
    if (diff < 0)
    continue;
...
//
    ...
    let mut links: u16 = le16_to_cpu(ni.mi.mrec.hard_links);
    if (!links) {
...
#[no_mangle]
pub unsafe extern "C" fn mi_resize_attr(mi: *mut mft_inode, attr: *mut ATTRIB, bytes: c_int) -> bool {
    ...
    let mut tail: c_int = (int)(used - aoff - asize);
    int dsize;
    char *next;
    if (tail < 0 || aoff >= used)
    return false;
...
//
    ...
    let mut run_size: u32 = asize - run_off;
    let mut tail: u32 = used - aoff - asize;
    let mut dsize: u32 = sbi.record_size - used;
