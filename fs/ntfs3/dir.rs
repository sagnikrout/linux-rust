...
// clang-format off
pub const PLANE_SIZE: c_uint = 0x00010000;
pub const SURROGATE_PAIR: c_uint = 0x0000d800;
pub const SURROGATE_LOW: c_uint = 0x00000400;
pub const SURROGATE_BITS: c_uint = 0x000003ff;
// clang-format on
//
...
//
    ...
    err = indx_find(&ni.dir, ni, core::ptr::null_mut(), uni, 0, sbi, &diff, &e, fnd);
    if (err)
    goto out;
    if (diff) {
    err = -ENOENT;
    ...
    out:
    fnd_put(fnd_a);
    let mut err: return = = -ENOENT ? core::ptr::null_mut() : err ? ERR_PTR(err) : inode;
    }
...
//
    ...
    struct inode *inode = ntfs_iget5(sbi.sb, &e.ref, core::ptr::null_mut());
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    ...
    struct super_block *sb = dir.i_sb;
    struct ntfs_sb_info *sbi = sb.s_fs_info;
    let mut i_size: loff_t = i_size_read(dir);
    let mut pos: u64 = ctx.pos;
    u8 *name = core::ptr::null_mut();
    struct indx_node *node = core::ptr::null_mut();
    let mut index_bits: u8 = ni.dir.index_bits;
    let mut max_bit: usize = i_size >> ni.dir.index_bits;
    let mut eod: loff_t = i_size + sbi.record_size;
...
//
    ...
    const struct ATTR_FILE_NAME *fname;
    u32 e_size, off, end;
    let mut drs: usize = 0, fles = 0, bit = 0;
    struct indx_node *node = core::ptr::null_mut();
    let mut max_indx: usize = i_size_read(&ni.vfs_inode) >> ni.dir.index_bits;
    if (is_empty)
// is_empty = true;
    root = indx_get_root(&ni.dir, ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!root)
...
#[no_mangle]
pub unsafe extern "C" fn dir_is_empty(dir: *mut inode) -> bool {
    bool dir_is_empty(struct inode *dir)
    {
    let mut is_empty: bool = false;
    ntfs_dir_count(dir, &is_empty, core::ptr::null_mut(), core::ptr::null_mut());
    return is_empty;
    }
// clang-format off
