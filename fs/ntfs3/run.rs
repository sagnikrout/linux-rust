...
pub const NTFS3_RUN_MAX_BYTES: c_uint = 0x10000;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ntfs_run {
    pub /: *mut *mut CLST vcn; / Virtual cluster number.,
    pub /: *mut *mut CLST len; / Length in clusters.,
    pub /: *mut *mut CLST lcn; / Logical cluster number.,
}
...
//
#[no_mangle]
unsafe extern "C" fn run_lookup(run: *const runs_tree, vcn: CLST, index: *mut usize) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn run_consolidate(run: *mut runs_tree, index: usize) {
...
//
#[no_mangle]
pub unsafe extern "C" fn run_is_mapped_full(run: *const runs_tree, svcn: CLST, evcn: CLST) -> bool {
...
//
    ...
    {
    CLST vcn1, len1;
    if (run_da && run_lookup_entry(run_da, vcn, lcn, len, core::ptr::null_mut())) {
// lcn = DELALLOC_LCN;
    return true;
    }
    if (!run_lookup_entry(run, vcn, lcn, len, core::ptr::null_mut()))
    return false;
...
//
#[no_mangle]
pub unsafe extern "C" fn run_truncate_head(run: *mut runs_tree, vcn: CLST) {
...
//
#[no_mangle]
pub unsafe extern "C" fn run_truncate(run: *mut runs_tree, vcn: CLST) {
...
//
#[no_mangle]
pub unsafe extern "C" fn run_truncate_around(run: *mut runs_tree, vcn: CLST) {
...
//
    ...
    struct ntfs_run *r;
    bool inrange;
    let mut tail_vcn: CLST = 0, tail_len = 0, tail_lcn = 0;
...
// Use power of 2 for 'bytes'.
    ...
    let mut i: usize = run.count - index;
    r = run.runs + index;
...
//
    ...
    let mut to_eat: CLST = vcn - r.vcn;
    let mut Tovcn: CLST = to_eat + len;
...
//
#[no_mangle]
pub unsafe extern "C" fn run_collapse_range(run: *mut runs_tree, vcn: CLST, len: CLST, sub: CLST) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn run_insert_range(run: *mut runs_tree, vcn: CLST, len: CLST) -> c_int {
    ...
    if (vcn > r.vcn) {
// split fragment.
    let mut len1: CLST = vcn - r.vcn;
    let mut len2: CLST = r.len - len1;
    let mut lcn2: CLST = r.lcn == SPARSE_LCN ? SPARSE_LCN : (r.lcn + len1);
    r.len = len1;
...
//
#[no_mangle]
pub unsafe extern "C" fn run_insert_range_da(run: *mut runs_tree, vcn: CLST, len: CLST) -> c_int {
    int run_insert_range_da(struct runs_tree *run, CLST vcn, CLST len)
    {
    struct ntfs_run *r, *r0 = core::ptr::null_mut(), *e = run.runs + run.count;
    ;
    for (r = run.runs; r < e; r++) {
    let mut end: CLST = r.vcn + r.len;
    if (vcn >= end)
    ...
    if (r0) {
// split fragment.
    let mut len1: CLST = vcn - r0.vcn;
    let mut len2: CLST = r0.len - len1;
    r0.len = len1;
...
#[no_mangle]
pub unsafe extern "C" fn run_packed_size(n: i64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn run_pack_s64(run_buf: *mut u8, size: u8, v: i64) {
...
// Full trusted function. It does not check 'size' for errors.
#[no_mangle]
pub unsafe extern "C" fn run_unpack_s64(run_buf: *const u8, size: u8, v: i64) -> i64 {
...
//
    ...
    let mut prev_lcn: CLST = 0;
    let mut evcn1: CLST = svcn + len;
    const struct ntfs_run *r, *r_end;
    let mut packed_size: c_int = 0;
    size_t i;
    s64 dlcn;
...
//
    ...
    let mut is_mft: bool = ino == MFT_REC_MFT;
    if (run_buf_size < 0)
    return -EINVAL;
// Check for empty.
    if (evcn + 1 == svcn)
...
// size_size - How much bytes is packed len.
    let mut size_size: u8 = *run_buf & 0xF;
// offset_size - How much bytes is packed dlcn.
    let mut offset_size: u8 = *run_buf++ >> 4;
    u64 len;
    if (!size_size)
    break;
//
...
// Skip size_size.
    ...
    lcn = SPARSE_LCN64;
#[no_mangle]
pub unsafe extern "C" fn if(sizeof(s64): offset_size <=) -> else {
    s64 dlcn;
...
#[no_mangle]
pub unsafe extern "C" fn if(RUN_DEALLOCATE: run ==) -> else {
//
...
//
#[no_mangle]
pub unsafe extern "C" fn run_clone(run: *const runs_tree, new_run: *mut runs_tree) -> c_int {
    int run_clone(const struct runs_tree *run, struct runs_tree *new_run)
    {
    let mut bytes: usize = run.count * sizeof(struct ntfs_run);
...
//
#[no_mangle]
pub unsafe extern "C" fn run_remove_range(run: *mut runs_tree, vcn: CLST, len: CLST, done: *mut CLST) -> bool {
...
// No entries in this run.
    ...
    if (vcn > r.vcn) {
    let mut r_end: CLST = r.vcn + r.len;
    d = vcn - r.vcn;
    if (r_end > end) {
...
#[no_mangle]
pub unsafe extern "C" fn run_len(run: *const runs_tree) -> CLST {
...
#[no_mangle]
pub unsafe extern "C" fn run_get_max_vcn(run: *const runs_tree) -> CLST {
