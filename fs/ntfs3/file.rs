...
//
#[no_mangle]
unsafe extern "C" fn ntfs_dio_alignment(inode: *mut inode) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_should_use_dio(iocb: *mut kiocb, iter: *mut iov_iter) -> bool {
    static bool ntfs_should_use_dio(struct kiocb *iocb, struct iov_iter *iter)
    {
    struct inode *inode = file_inode(iocb.ki_filp);
    let mut dio_align: u32 = ntfs_dio_alignment(inode);
    if (!dio_align)
    return false;
    return IS_ALIGNED(iocb.ki_pos | iov_iter_alignment(iter), dio_align);
    }
#[no_mangle]
unsafe extern "C" fn ntfs_ioctl_fitrim(sbi: *mut ntfs_sb_info, arg: c_ulong) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_fileattr_get(dentry: *mut dentry, fa: *mut file_kattr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntfs_ioctl_get_volume_label(sbi: *mut ntfs_sb_info, buf: *mut u8 __user) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntfs_ioctl_set_volume_label(sbi: *mut ntfs_sb_info, buf: *mut u8 __user) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_force_shutdown(sb: *mut super_block, flags: u32) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ntfs_ioctl_shutdown(sb: *mut super_block, arg: c_ulong) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_ioctl(filp: *mut file, cmd: u32, arg: c_ulong) -> c_long {
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_compat_ioctl(filp: *mut file, cmd: u32, arg: c_ulong) -> c_long {
...
// Avoid any operation if inode is bad.
    ...
    let mut valid: loff_t = ni.i_valid;
    int err;
    if (valid >= new_valid)
    return 0;
    if (is_resident(ni)) {
    ...
    err = iomap_zero_range(inode, valid, new_valid - valid, core::ptr::null_mut(),
    &ntfs_iomap_ops, &ntfs_iomap_folio_ops, core::ptr::null_mut());
    if (err) {
    ni.i_valid = valid;
...
#[no_mangle]
unsafe extern "C" fn ntfs_zero_tail(mapping: *mut address_space, from: loff_t) {
...
#[no_mangle]
unsafe extern "C" fn ntfs_filemap_close(vma: *mut vm_area_struct) {
    static void ntfs_filemap_close(struct vm_area_struct *vma)
    {
    struct inode *inode = file_inode(vma.vm_file);
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut i_size: u64 = i_size_read(inode);
    let mut from: u64 = (u64)vma.vm_pgoff << PAGE_SHIFT;
    let mut to: u64 = min(i_size, from + vma.vm_end - vma.vm_start);
    if (ni.i_valid < to) {
    ni.i_valid = to;
    mark_inode_dirty(inode);
    }
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_file_mmap_prepare(desc: *mut vm_area_desc) -> c_int {
    static int ntfs_file_mmap_prepare(struct vm_area_desc *desc)
    {
    struct file *file = desc.file;
    struct inode *inode = file_inode(file);
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut rw: bool = vma_desc_test(desc, VMA_WRITE_BIT);
    int err;
// Avoid any operation if inode is bad.
    if (unlikely(is_bad_ni(ni)))
    return -EINVAL;
...
// Turn off readahead for compressed files.
    ...
    if (rw) {
    let mut from: u64 = (u64)desc.pgoff << PAGE_SHIFT;
...
// Allocate clusters for rw map.
    struct ntfs_sb_info *sbi = inode.i_sb.s_fs_info;
    CLST lcn, len;
    let mut vcn: CLST = from >> sbi.cluster_bits;
    let mut end: CLST = bytes_to_cluster(sbi, to);
    bool new;
    for (; vcn < end; vcn += len) {
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_fallocate(file: *mut file, mode: c_int, vbo: loff_t, len: loff_t) -> c_long {
    ...
    struct ntfs_sb_info *sbi = sb.s_fs_info;
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut end: loff_t = vbo + len;
    loff_t vbo_down = round_down(vbo, max_t(unsigned long,
    sbi.cluster_size, PAGE_SIZE));
    let mut is_supported_holes: bool = is_sparsed(ni) || is_compressed(ni);
...
// Zero head of punch.
    if (tmp > from) {
    err = iomap_zero_range(inode, from, tmp - from, core::ptr::null_mut(),
    &ntfs_iomap_ops,
    &ntfs_iomap_folio_ops, core::ptr::null_mut());
    if (err)
    goto out;
    }
// Aligned punch_hole. Deallocate clusters.
    if (end_a > vbo_a) {
    ni_lock(ni);
    err = attr_punch_hole(ni, vbo_a, end_a - vbo_a, core::ptr::null_mut());
    ni_unlock(ni);
    if (err)
    goto out;
    }
// Zero tail of punch.
    ...
    err = iomap_zero_range(inode, end_a, end - end_a, core::ptr::null_mut(),
    &ntfs_iomap_ops,
    &ntfs_iomap_folio_ops, core::ptr::null_mut());
    if (err)
    goto out;
...
// generic/213: expected -ENOSPC instead of -EFBIG.
    if (!is_supported_holes) {
    let mut to_alloc: loff_t = new_size - inode_get_bytes(inode);
    if (to_alloc > 0 &&
...
//
    ...
    if (is_supported_holes) {
    let mut vcn: CLST = vbo >> cluster_bits;
    let mut cend: CLST = bytes_to_cluster(sbi, end);
    CLST lcn, clen;
    bool new;
//
...
//
    ...
    struct inode *inode = d_inode(dentry);
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut ia_valid: u32 = attr.ia_valid;
...
// Linux 'w' -> Windows 'ro'.
    ...
    ntfs_save_wsl_perm(inode, core::ptr::null_mut());
    mark_inode_dirty(inode);
    out:
    return err;
    }
...
//
#[no_mangle]
unsafe extern "C" fn check_read_restriction(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_file_read_iter(iocb: *mut kiocb, iter: *mut iov_iter) -> isize {
...
// Direct I/O is not compatible with delalloc.
    ...
    ret = iomap_dio_rw(iocb, iter, &ntfs_iomap_ops, core::ptr::null_mut(), dio_flags,
    core::ptr::null_mut(), 0);
    if (ret <= 0)
    goto out;
    end = vbo + ret;
    if (valid < end) {
    let mut to_zero: usize = end - valid;
// Fix iter.
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_compress_write(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    ...
    let mut frame_size: u32 = 1u << frame_bits;
    let mut pages_per_frame: u32 = frame_size >> PAGE_SHIFT;
    u32 ip, off;
    CLST frame;
    u64 frame_vbo;
    pgoff_t index;
...
// Zero range [valid : pos).
    ...
    &clen, core::ptr::null_mut(), false, core::ptr::null_mut(), false);
    if (err)
    goto out;
    if (lcn == SPARSE_LCN) {
    ni.i_valid = valid =
...
//
#[no_mangle]
unsafe extern "C" fn check_write_restriction(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_file_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
// Direct I/O is not compatible with delalloc.
    ...
    ret = iomap_dio_rw(iocb, from, &ntfs_iomap_ops, core::ptr::null_mut(),
    IOMAP_DIO_FORCE_WAIT, core::ptr::null_mut(), 0);
    if (ret == -ENOTBLK) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_file_open(inode: *mut inode, file: *mut file) -> c_int {
...
// Decompress "external compressed" file if opened for rw.
    ...
    let mut err: c_int = ni_decompress_file(ni);
    if (err)
    return err;
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_file_release(inode: *mut inode, file: *mut file) -> c_int {
...
// Deallocate preallocated.
    ...
    inode.i_size, &ni.i_valid, false, core::ptr::null_mut(),
    true);
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_file_fsync(file: *mut file, start: loff_t, end: loff_t, datasync: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
    static loff_t ntfs_llseek(struct file *file, loff_t offset, int whence)
    {
    struct inode *inode = file.f_mapping.host;
    struct ntfs_inode *ni = ntfs_i(inode);
    let mut maxbytes: loff_t = ntfs_get_maxbytes(ni);
    loff_t ret;
