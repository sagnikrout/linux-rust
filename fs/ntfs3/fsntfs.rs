...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_fix_pre_write(rhdr: *mut NTFS_RECORD_HEADER, bytes: usize) -> bool {
    bool ntfs_fix_pre_write(struct NTFS_RECORD_HEADER *rhdr, size_t bytes)
    {
    u16 *fixup, *ptr;
    u16 sample;
    let mut fo: u16 = le16_to_cpu(rhdr.fix_off);
    let mut fn: u16 = le16_to_cpu(rhdr.fix_num);
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_extend_init(sbi: *mut ntfs_sb_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_loadlog_and_replay(ni: *mut ntfs_inode, sbi: *mut ntfs_sb_info) -> c_int {
...
// Check for 4GB.
    ...
    inode = ntfs_iget5(sb, &ref, core::ptr::null_mut());
    if (IS_ERR(inode))
    inode = core::ptr::null_mut();
    if (!inode) {
// Try to use MFT copy.
    let mut t64: u64 = sbi.mft.lbo;
...
//
    ...
    let mut da_clusters: CLST = ntfs_get_da(sbi);
    wnd = &sbi.used.bitmap;
...
//
#[no_mangle]
unsafe extern "C" fn ntfs_extend_mft(sbi: *mut ntfs_sb_info) -> c_int {
...
// Step 1: Resize $MFT::DATA.
    ...
    err = attr_set_size_ex(ni, ATTR_DATA, core::ptr::null_mut(), 0, &ni.file.run,
    new_mft_bytes, core::ptr::null_mut(), false, &attr, false);
    if (err) {
...
// Step 2: Resize $MFT::BITMAP.
    ...
    err = attr_set_size(ni, ATTR_BITMAP, core::ptr::null_mut(), 0, &sbi.mft.bitmap.run,
...
// Once per session create internal bitmap for 5 bits.
    ...
    i = ntfs_iget5(sb, &ref, core::ptr::null_mut());
    if (IS_ERR(i)) {
    next:
    ntfs_notice(
    sb,
    ...
    if (!ni_std(ni))
    goto next;
    if (ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_NAME,
...
#[no_mangle]
pub unsafe extern "C" fn if(sbi->mft.reserved_bitmap_inited: *mut *mut rno >= MFT_REC_RESERVED &&) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_mark_rec_free(sbi: *mut ntfs_sb_info, rno: CLST, is_mft: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn if(MFT_REC_USER: rno < sbi->mft.next_free && rno >=) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_clear_mft_tail(sbi: *mut ntfs_sb_info, from: usize, to: usize) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_refresh_zone(sbi: *mut ntfs_sb_info) -> c_int {
...
// Do not change anything unless we have non empty MFT zone.
    ...
    if (!run_lookup_entry(&ni.file.run, vcn - 1, &lcn, &len, core::ptr::null_mut()))
    lcn = SPARSE_LCN;
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_update_mftmirr(sbi: *mut ntfs_sb_info) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_bad_inode(inode: *mut inode, hint: *const c_char) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_set_state(sbi: *mut ntfs_sb_info, dirty: enum NTFS_DIRTY_FLAGS) -> c_int {
...
// Check cached value.
    ...
    attr = ni_find_attr(ni, core::ptr::null_mut(), core::ptr::null_mut(), ATTR_VOL_INFO, core::ptr::null_mut(), 0, core::ptr::null_mut(), &mi);
    if (!attr) {
    err = -EINVAL;
    goto out;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn security_hash(sd: *const c_void, bytes: usize) -> __le32 {
...
// prevent generic message "attempt to access beyond end of device"
    ...
    {
    let mut blocksize: u32 = sb.s_blocksize;
    struct block_device *bdev = sb.s_bdev;
    let mut block: sector_t = lbo >> sb.s_blocksize_bits;
    let mut off: u32 = lbo & (blocksize - 1);
    let mut op: u32 = blocksize - off;
    struct buffer_head *bh;
    ...
    unlock_buffer(bh);
    if (wait) {
    let mut err: c_int = sync_dirty_buffer(bh);
    if (err) {
    ntfs_err(
    sb,
    ...
    let mut off: u32 = vbo & sbi.cluster_mask;
    ...
    for (;;) {
    let mut op: u32 = min_t(u64, len, bytes);
    let mut err: c_int = ntfs_sb_write(sb, lbo, op, buf, sync);
    if (err)
    return err;
    bytes -= op;
    if (!bytes)
    ...
    CLST lcn;
    u64 lbo;
    if (!run_lookup_entry(run, vbo >> cluster_bits, &lcn, core::ptr::null_mut(), core::ptr::null_mut()))
...
// Use absolute boot's 'MFTCluster' to read record.
    ...
    for (;;) {
    let mut len32: u32 = len >= bytes ? bytes : len;
    let mut block: sector_t = lbo >> sb.s_blocksize_bits;
    if (ra) {
    let mut index: pgoff_t = lbo >> PAGE_SHIFT;
    if (!ra_has_index(ra, index)) {
    page_cache_sync_readahead(mapping, ra, core::ptr::null_mut(),
    index, 1);
...
//
    ...
    {
    let mut err: c_int = ntfs_read_run_nb_ra(sbi, run, vbo, rhdr, bytes, nb, ra);
    if (err)
    return err;
    ...
    for (;;) {
    let mut len32: u32 = min_t(u64, len, bytes);
    let mut block: sector_t = lbo >> sb.s_blocksize_bits;
    do {
    u32 op;
    struct buffer_head *bh;
    ...
    let mut err: c_int = 0;
    struct super_block *sb = sbi.sb;
    let mut block_size: u32 = sb.s_blocksize;
    let mut bytes: u32 = nb.bytes;
    let mut off: u32 = nb.off;
    let mut fo: u16 = le16_to_cpu(rhdr.fix_off);
    let mut fn: u16 = le16_to_cpu(rhdr.fix_num);
    u32 idx;
    __le16 *fixup = core::ptr::null_mut();
    let mut sample: __le16 = cpu_to_le16(-1u);
    if ((fo & 1) || fo + fn * sizeof(short) > SECTOR_SIZE || !fn-- ||
    fn * SECTOR_SIZE > bytes) {
    return -EINVAL;
    }
    for (idx = 0; bytes && idx < nb.nbufs; idx += 1, off = 0) {
    let mut op: u32 = block_size - off;
    char *bh_data;
...
// ptr = sample;
    ...
    unlock_buffer(bh);
    if (sync) {
    let mut err2: c_int = sync_dirty_buffer(bh);
    if (!err && err2)
    err = err2;
    }
    bytes -= op;
...
// Read range [lbo, lbo+len).
    folio = read_mapping_folio(mapping, lbo >> PAGE_SHIFT, core::ptr::null_mut());
    if (IS_ERR(folio))
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_bio_fill_1(sbi: *mut ntfs_sb_info, run: *const runs_tree) -> c_int {
    ...
    struct block_device *bdev = sb.s_bdev;
    let mut cluster_bits: u8 = sbi.cluster_bits;
    struct bio *new, *bio = core::ptr::null_mut();
    CLST lcn, clen;
    u64 lbo, len;
    size_t run_idx;
    struct page *fill;
...
//
    ...
    for (;;) {
    let mut add: u32 = len > PAGE_SIZE ? PAGE_SIZE : len;
    ...
    } while (run_get_entry(run, ++run_idx, core::ptr::null_mut(), &lcn, &clen));
    if (!err)
    ...
    CLST lcn, len;
    let mut cluster_bits: u8 = sbi.cluster_bits;
    if (!run_lookup_entry(run, vbo >> cluster_bits, &lcn, &len, core::ptr::null_mut()))
    return -ENOENT;
    off = vbo & sbi.cluster_mask;
...
#[no_mangle]
pub unsafe extern "C" fn sid_length(sid: *const SID) -> u32 {
...
//
#[no_mangle]
unsafe extern "C" fn is_acl_valid(acl: *const ACL, len: u32) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn is_sd_valid(sd: *const SECURITY_DESCRIPTOR_RELATIVE, len: u32) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_security_init(sbi: *mut ntfs_sb_info) -> c_int {
    ...
    struct ntfs_fnd *fnd_sii = core::ptr::null_mut();
    ...
    attr = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_ROOT, SDH_NAME,
    ARRAY_SIZE(SDH_NAME), core::ptr::null_mut(), core::ptr::null_mut());
    if (!attr ||
    ...
    ARRAY_SIZE(SII_NAME), core::ptr::null_mut(), core::ptr::null_mut());
    if (!attr ||
...
// sd = NULL;
    ...
    root_sii = indx_get_root(indx, ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!root_sii) {
    err = -EINVAL;
    goto out;
    }
// Try to find this SECURITY descriptor in SII indexes.
    ...
    core::ptr::null_mut(), &diff, (struct NTFS_DE **)&sii_e, fnd_sii);
    if (err)
    goto out;
    if (diff)
    goto out;
...
// size = t32 - sizeof(struct SECURITY_HDR);
    ...
    sizeof(d_security), core::ptr::null_mut());
    if (err)
    goto out;
...
//
    ...
    struct NTFS_DE_SII sii_e;
    struct SECURITY_HDR *d_security;
    let mut new_sec_size: u32 = size_sd + sizeof(struct SECURITY_HDR);
    let mut aligned_sec_size: u32 = ALIGN(new_sec_size, 16);
    struct SECURITY_KEY hash_key;
    struct ntfs_fnd *fnd_sdh = core::ptr::null_mut();
    const struct INDEX_ROOT *root_sdh;
    const struct INDEX_ROOT *root_sii;
...
// Allocate a temporal buffer.
    ...
    root_sdh = indx_get_root(indx_sdh, ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!root_sdh) {
    err = -EINVAL;
    goto out;
    }
    root_sii = indx_get_root(indx_sii, ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!root_sii) {
    err = -EINVAL;
    goto out;
    }
...
//
    ...
    d_security, new_sec_size, core::ptr::null_mut());
    if (err)
    goto out;
...
// Fill SII entry.
    ...
    err = indx_insert_entry(indx_sii, ni, &sii_e.de, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    if (err)
    goto out;
// Fill SDH entry.
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_reparse_init(sbi: *mut ntfs_sb_info) -> c_int {
    ...
    return 0;
    le = core::ptr::null_mut();
    attr = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_ROOT, SR_NAME,
    ARRAY_SIZE(SR_NAME), core::ptr::null_mut(), core::ptr::null_mut());
    if (!attr) {
    err = -EINVAL;
    goto out;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_objid_init(sbi: *mut ntfs_sb_info) -> c_int {
    ...
    return 0;
    le = core::ptr::null_mut();
    attr = ni_find_attr(ni, core::ptr::null_mut(), &le, ATTR_ROOT, SO_NAME,
    ARRAY_SIZE(SO_NAME), core::ptr::null_mut(), core::ptr::null_mut());
    if (!attr) {
    err = -EINVAL;
    goto out;
    }
...
#[no_mangle]
pub unsafe extern "C" fn ntfs_objid_remove(sbi: *mut ntfs_sb_info, guid: *mut GUID) -> c_int {
    ...
    err = indx_delete_entry(indx, ni, guid, sizeof(*guid), core::ptr::null_mut());
    mark_inode_dirty(&ni.vfs_inode);
    ni_unlock(ni);
    return err;
    }
    ...
    err = indx_insert_entry(indx, ni, &re.de, core::ptr::null_mut(), core::ptr::null_mut(), 0);
    mark_inode_dirty(&ni.vfs_inode);
    ni_unlock(ni);
    return err;
    }
    ...
    struct ntfs_inode *ni = sbi.reparse.ni;
    struct ntfs_index *indx = &sbi.reparse.index_r;
    struct ntfs_fnd *fnd = core::ptr::null_mut();
    ...
    mutex_lock_nested(&ni.ni_lock, NTFS_INODE_MUTEX_REPARSE);
    if (rtag) {
    err = indx_delete_entry(indx, ni, &rkey, sizeof(rkey), core::ptr::null_mut());
    goto out1;
    }
    fnd = fnd_get();
    if (!fnd) {
    ...
    root_r = indx_get_root(indx, ni, core::ptr::null_mut(), core::ptr::null_mut());
    if (!root_r) {
    err = -EINVAL;
    goto out;
    }
...
#[no_mangle]
pub unsafe extern "C" fn mark_as_free_ex(sbi: *mut ntfs_sb_info, lcn: CLST, len: CLST, trim: bool) {
...
// Create MFT zone only if 'zlen' is large enough.
    ...
    let mut zone_lcn: CLST = wnd_zone_bit(wnd);
    if (lcn + len == zone_lcn) {
...
//
    ...
    CLST lcn, len;
    let mut idx: usize = 0;
    while (run_get_entry(run, idx++, core::ptr::null_mut(), &lcn, &len)) {
    if (lcn == SPARSE_LCN)
    continue;
...
#[no_mangle]
pub unsafe extern "C" fn name_has_forbidden_chars(fname: *const le_str) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn valid_windows_name(sbi: *mut ntfs_sb_info, fname: *const le_str) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ntfs_set_label(sbi: *mut ntfs_sb_info, label: *mut u8, len: c_int) -> c_int {
...
// Undo delimiter parse
    ...
    ni_lock(ni);
// Ignore any errors.
    ni_remove_attr(ni, ATTR_LABEL, core::ptr::null_mut(), 0, false, core::ptr::null_mut());
    err = ni_insert_resident(ni, uni_bytes, ATTR_LABEL, core::ptr::null_mut(), 0, &attr,
