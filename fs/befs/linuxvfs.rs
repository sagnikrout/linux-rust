...
pub const VFS_BLOCK_SIZE: c_int = 512;
...
//
#[no_mangle]
unsafe extern "C" fn befs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    static int befs_read_folio(struct file *file, struct folio *folio)
    {
...
//
    ...
    let mut run: befs_block_run = BAD_IADDR;
    int res;
    ulong disk_off;
...
#[no_mangle]
unsafe extern "C" fn befs_free_inode(inode: *mut inode) {
    static void befs_free_inode(struct inode *inode)
    {
...
//
    ...
    i_data.symlink),
    init_once);
    if (befs_inode_cachep == core::ptr::null_mut())
    return -ENOMEM;
    return 0;
    }
...
//
#[no_mangle]
unsafe extern "C" fn befs_symlink_read_folio(unused: *mut file, folio: *mut folio) -> c_int {
    ...
    let mut len: befs_off_t = data.size;
...
// in special cases
//
    let mut maxlen: c_int = (3 * in_len) + 1;
    befs_debug(sb, "--. %s\n", __func__);
    if (!nls) {
...
#[no_mangle]
unsafe extern "C" fn befs_show_options(m: *mut seq_file, root: *mut dentry) -> c_int {
...
//
    ...
    BEFS_SB(sb).mount_opts.iocharset = core::ptr::null_mut();
...
//
    ...
    let mut sb_block: c_ulong = 0;
    let mut x86_sb_off: off_t = 512;
    int blocksize;
...
#[no_mangle]
unsafe extern "C" fn befs_get_tree(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn befs_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn befs_free_fc(fc: *mut fs_context) {
    static void befs_free_fc(struct fs_context *fc)
    {
    struct befs_mount_options *opts = fc.fs_private;
    kfree(opts.iocharset);
