...
//
    let mut ecryptfs_verbosity: c_int = 0;
...
//
    let mut ecryptfs_message_buf_len: c_uint = ECRYPTFS_DEFAULT_MSG_CTX_ELEMS;
...
//
    let mut ecryptfs_message_wait_timeout: signed long = ECRYPTFS_MAX_MSG_CTX_TTL / HZ;
...
//
    let mut ecryptfs_number_of_users: c_uint = ECRYPTFS_DEFAULT_NUM_USERS;
    module_param(ecryptfs_number_of_users, uint, 0);
    MODULE_PARM_DESC(ecryptfs_number_of_users, "An estimate of the number of "
    "concurrent users of eCryptfs");
#[no_mangle]
pub unsafe extern "C" fn __ecryptfs_printk(fmt: *const c_char, ...) {
...
//
    ...
    let mut path: path = ecryptfs_lower_path(dentry);
    int rc;
    ...
    (*lower_file) = core::ptr::null_mut();
    }
    return rc;
    }
#[no_mangle]
pub unsafe extern "C" fn ecryptfs_get_lower_file(dentry: *mut dentry, inode: *mut inode) -> c_int {
    ...
    rc = -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(1: count ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn ecryptfs_put_lower_file(inode: *mut inode) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ecryptfs_fs_context {
// Mount option status trackers
    pub check_ruid: bool,
    pub sig_set: bool,
    pub cipher_name_set: bool,
    pub cipher_key_bytes_set: bool,
    pub fn_cipher_name_set: bool,
    pub fn_cipher_key_bytes_set: bool,
}
...
#[no_mangle]
unsafe extern "C" fn ecryptfs_validate_options(fc: *mut fs_context) -> c_int {
    ...
    let mut cipher_name_len: c_int = strlen(ECRYPTFS_DEFAULT_CIPHER);
    ...
    core::ptr::null_mut(), mount_crypt_stat.global_default_cipher_name,
    mount_crypt_stat.global_default_cipher_key_size);
    if (rc) {
    ...
    if ((mount_crypt_stat.flags & ECRYPTFS_GLOBAL_ENCRYPT_FILENAMES)
    && !ecryptfs_tfm_exists(
    mount_crypt_stat.global_default_fn_cipher_name, core::ptr::null_mut())) {
    rc = ecryptfs_add_new_key_tfm(
    core::ptr::null_mut(), mount_crypt_stat.global_default_fn_cipher_name,
    mount_crypt_stat.global_default_fn_cipher_key_bytes);
    if (rc) {
    printk(KERN_ERR "Error attempting to initialize "
    "cipher with name = [%s] and key size = [%td]; "
...
//
#[no_mangle]
unsafe extern "C" fn ecryptfs_get_tree(fc: *mut fs_context) -> c_int {
    ...
    s = sget_fc(fc, core::ptr::null_mut(), set_anon_super_fc);
    if (IS_ERR(s)) {
    rc = PTR_ERR(s);
    goto out;
    }
...
//
#[no_mangle]
unsafe extern "C" fn ecryptfs_kill_block_super(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn ecryptfs_free_fc(fc: *mut fs_context) {
...
#[no_mangle]
unsafe extern "C" fn ecryptfs_init_fs_context(fc: *mut fs_context) -> c_int {
    static int ecryptfs_init_fs_context(struct fs_context *fc)
    {
    struct ecryptfs_fs_context *ctx;
    struct ecryptfs_sb_info *sbi = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ecryptfs_free_kmem_caches() {
...
//
#[no_mangle]
unsafe extern "C" fn ecryptfs_init_kmem_caches() -> c_int {
...
// (info->cache) = kmem_cache_create(info->name, info->size, 0,
    ...
    let mut version_attr: static struct kobj_attribute = __ATTR_RO(version);
...
#[no_mangle]
unsafe extern "C" fn do_sysfs_registration() -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_sysfs_unregistration() {
...
#[no_mangle]
unsafe extern "C" fn ecryptfs_init() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn ecryptfs_exit() -> void __exit {
