...
//! Source: fs/notify/fanotify/fanotify_user.c
#![no_std]
...
pub const FANOTIFY_DEFAULT_MAX_EVENTS: c_int = 16384;
pub const FANOTIFY_OLD_DEFAULT_MAX_MARKS: c_int = 8192;
pub const FANOTIFY_DEFAULT_MAX_GROUPS: c_int = 128;
pub const FANOTIFY_DEFAULT_FEE_POOL_SIZE: c_int = 32;
//
...
// configurable via /proc/sys/fs/fanotify/
    ...
    let mut ft_zero: static long = 0;
    let mut ft_int_max: static long = INT_MAX;
...
#[no_mangle]
unsafe extern "C" fn fanotify_sysctls_init() -> void __init {
...
#[no_mangle]
unsafe extern "C" fn perm_group_watchdog_schedule() {
    static void perm_group_watchdog_schedule(void)
    {
    let mut timeout: c_int = READ_ONCE(perm_group_timeout);
    if (!timeout)
    return;
    schedule_delayed_work(&perm_group_work, secs_to_jiffies(timeout));
    }
#[no_mangle]
unsafe extern "C" fn perm_group_watchdog(work: *mut work_struct) {
    static void perm_group_watchdog(struct work_struct *work)
    {
    struct fsnotify_group *group;
    struct fanotify_perm_event *event;
    struct task_struct *task;
    let mut failed_pid: pid_t = 0;
...
// Do not report same pid repeatedly
    ...
    event.recv_pid,
    task ? task.comm : core::ptr::null_mut(),
    perm_group_timeout);
    rcu_read_unlock();
    }
    }
...
#[no_mangle]
unsafe extern "C" fn fanotify_perm_watchdog_group_remove(group: *mut fsnotify_group) {
...
#[no_mangle]
unsafe extern "C" fn fanotify_perm_watchdog_group_add(group: *mut fsnotify_group) {
...
pub const FANOTIFY_EVENT_ALIGN: c_int = 4;
...
#[no_mangle]
unsafe extern "C" fn fanotify_fid_info_len(fh_len: c_int, name_len: c_int) -> c_int {
    static int fanotify_fid_info_len(int fh_len, int name_len)
    {
    let mut info_len: c_int = fh_len;
    if (name_len)
    info_len += name_len + 1;
...
#[no_mangle]
unsafe extern "C" fn fanotify_dir_name_info_len(event: *mut fanotify_event) -> c_int {
    static int fanotify_dir_name_info_len(struct fanotify_event *event)
    {
    struct fanotify_info *info = fanotify_event_info(event);
    let mut dir_fh_len: c_int = fanotify_event_dir_fh_len(event);
    let mut dir2_fh_len: c_int = fanotify_event_dir2_fh_len(event);
    ...
    {
    let mut event_len: usize = FAN_EVENT_METADATA_LEN;
    int fh_len;
    let mut dot_len: c_int = 0;
...
//
    ...
    {
    size_t event_size;
    struct fanotify_event *event = core::ptr::null_mut();
    struct fsnotify_event *fsn_event;
    let mut info_mode: c_uint = FAN_GROUP_FLAG(group, FANOTIFY_INFO_MODES);
...
//
    ...
    {
    struct fanotify_perm_event *event;
    let mut fd: c_int = response_struct.fd;
    let mut response: u32 = response_struct.response;
    let mut errno: c_int = fanotify_get_response_errno(response);
    let mut ret: c_int = info_len;
    struct fanotify_response_info_audit_rule friar;
...
//
    ...
    {
    let mut info: fanotify_event_info_mnt = { };
    ...
    char __user *buf, int count)
    {
    let mut info: fanotify_event_info_error = { };
    ...
    {
    let mut info: fanotify_event_info_fid = { };
    let mut handle: file_handle = { };
    unsigned char bounce[FANOTIFY_INLINE_FH_LEN], *fh_buf;
    let mut fh_len: usize = fh ? fh.len : 0;
    let mut info_len: usize = fanotify_fid_info_len(fh_len, name_len);
    let mut len: usize = info_len;
...
// Pad with 0's
    ...
    size_t count)
    {
    let mut info: fanotify_event_info_pidfd = { };
    let mut info_len: usize = FANOTIFY_PIDFD_INFO_LEN;
    ...
    let mut info: fanotify_event_info_range = { };
    let mut info_len: usize = FANOTIFY_RANGE_INFO_LEN;
    if (WARN_ON_ONCE(info_len > count))
    return -EFAULT;
    ...
    let mut fid_mode: c_uint = info_mode & FANOTIFY_FID_BITS;
    let mut pidfd_mode: c_uint = info_mode & FAN_REPORT_PIDFD;
//
...
// New dir fid+name may be reported in addition to old dir fid+name
    ...
    if (fanotify_event_has_object_fh(event)) {
    const char *dot = core::ptr::null_mut();
    let mut dot_len: c_int = 0;
...
//
    ...
    let mut info_mode: c_uint = FAN_GROUP_FLAG(group, FANOTIFY_INFO_MODES);
    let mut pidfd_mode: c_uint = info_mode & FAN_REPORT_PIDFD;
    struct file *f = core::ptr::null_mut(), *pidfd_file = core::ptr::null_mut();
...
//
    ...
    if (pidfd_mode) {
    let mut pidfd_flags: c_uint = PIDFD_STALE;
...
#[no_mangle]
unsafe extern "C" fn fanotify_poll(file: *mut file, wait: *mut poll_table) -> __poll_t {
...
//
    ...
    FANOTIFY_PERM(event), FAN_DENY, core::ptr::null_mut());
    wake_up(&group.fanotify_data.access_waitq);
    } else {
    spin_lock(&group.notification_lock);
...
#[no_mangle]
unsafe extern "C" fn fanotify_write(file: *mut file, buf: *const char __user, count: usize, pos: *mut loff_t) -> isize {
...
#[no_mangle]
unsafe extern "C" fn fanotify_release(ignored: *mut inode, file: *mut file) -> c_int {
...
//
    ...
    list_del_init(&event.fae.fse.list);
    finish_permission_event(group, event, FAN_ALLOW, core::ptr::null_mut());
    spin_lock(&group.notification_lock);
    }
//
...
#[no_mangle]
unsafe extern "C" fn fanotify_ioctl(file: *mut file, cmd: c_uint, arg: c_ulong) -> c_long {
...
// destroy = !((fsn_mark->mask | fsn_mark->ignore_mask) & ~umask);
    ...
    {
    struct fsnotify_mark *fsn_mark = core::ptr::null_mut();
    __u32 removed;
    int destroy_mark;
...
// matches the fsnotify_find_mark()
    ...
    {
    let mut want_iref: bool = !(fan_flags & FAN_MARK_EVICTABLE);
    let mut ignore: c_uint = fan_flags & FANOTIFY_MARK_IGNORE_BITS;
...
#[no_mangle]
unsafe extern "C" fn fanotify_group_init_error_pool(group: *mut fsnotify_group) -> c_int {
...
// fanotify syscalls
    ...
    int f_flags, fd;
    let mut fid_mode: c_uint = flags & FANOTIFY_FID_BITS;
    let mut class: c_uint = flags & FANOTIFY_CLASS_BITS;
    let mut internal_flags: c_uint = 0;
...
// Enforce groups limits per user in all containing user ns
    ...
    struct fan_fsid *fsid)
    {
    let mut mark_type: c_uint = flags & FANOTIFY_MARK_TYPE_BITS;
...
#[no_mangle]
unsafe extern "C" fn fanotify_test_fid(dentry: *mut dentry, flags: c_uint) -> c_int {
...
//
    ...
    {
    let mut mark_type: c_uint = flags & FANOTIFY_MARK_TYPE_BITS;
    let mut is_dir: bool = d_is_dir(path.dentry);
...
//
    ...
    struct fsnotify_group *group;
    struct path path;
    struct fan_fsid __fsid, *fsid = core::ptr::null_mut();
    struct user_namespace *user_ns = core::ptr::null_mut();
    struct mnt_namespace *mntns;
    let mut valid_mask: u32 = FANOTIFY_EVENTS | FANOTIFY_EVENT_FLAGS;
    let mut mark_type: c_uint = flags & FANOTIFY_MARK_TYPE_BITS;
    let mut mark_cmd: c_uint = flags & FANOTIFY_MARK_CMD_BITS;
    let mut ignore: c_uint = flags & FANOTIFY_MARK_IGNORE_BITS;
    unsigned int obj_type, fid_mode;
    void *obj = core::ptr::null_mut();
    let mut umask: u32 = 0;
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn fanotify_user_setup() -> int __init {
