...
#[no_mangle]
pub unsafe extern "C" fn exfat_d_version(dentry: *mut dentry) -> c_ulong {
...
//
#[no_mangle]
unsafe extern "C" fn exfat_d_hash(dentry: *const dentry, qstr: *mut qstr) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn exfat_utf8_d_hash(dentry: *const dentry, qstr: *mut qstr) -> c_int {
...
// search EMPTY CONTINUOUS "num_entries" entries
    ...
    let mut total_entries: c_uint = exfat_cluster_to_dentries(sbi, p_dir.size);
...
#[no_mangle]
unsafe extern "C" fn exfat_check_max_dentries(inode: *mut inode) -> c_int {
...
//
    ...
    {
    int namelen;
    let mut lossy: c_int = NLS_NAME_NO_LOSSY;
    struct super_block *sb = inode.i_sb;
    let mut pathlen: c_int = strlen(path);
//
...
#[no_mangle]
pub unsafe extern "C" fn exfat_make_i_pos(info: *mut exfat_dir_entry) -> loff_t {
    ...
    struct exfat_chain clu;
    let mut ts: timespec64 = current_time(inode);
    struct exfat_entry_set_cache es;
    let mut clu_size: c_int = 0;
    let mut start_clu: c_uint = EXFAT_FREE_CLUSTER;
    let mut dir_allocated: bool = false;
...
//
    ...
    exfat_init_ext_entry(&es, num_entries, &uniname, core::ptr::null_mut(), 0);
    ...
    loff_t i_pos;
    int err;
    let mut size: loff_t = i_size_read(dir);
...
#[no_mangle]
unsafe extern "C" fn exfat_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn exfat_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn exfat_count_extra_entries(es: *mut exfat_entry_set_cache) -> c_int {
    ...
    let mut sync: c_int = IS_DIRSYNC(parent_inode);
...
// rename or move a old file into a new file
    ...
    struct exfat_inode_info *new_ei = core::ptr::null_mut();
// check the validity of pointer parameters
    if (new_path == core::ptr::null_mut() || strlen(new_path) == 0)
    return -EINVAL;
    if (ei.dir.dir == DIR_DELETED) {
...
//
    ...
    loff_t i_pos;
    int err;
    let mut size: loff_t = i_size_read(new_dir);
//
