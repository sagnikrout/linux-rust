...
#[no_mangle]
unsafe extern "C" fn exfat_mirror_bh(sb: *mut super_block, bh: *mut buffer_head) -> c_int {
    static int exfat_mirror_bh(struct super_block *sb, struct buffer_head *bh)
    {
    struct buffer_head *c_bh;
    struct exfat_sb_info *sbi = EXFAT_SB(sb);
    let mut sec: sector_t = bh.b_blocknr;
    sector_t sec2;
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn exfat_end_bh(sb: *mut super_block, bh: *mut buffer_head) -> c_int {
...
// content = EXFAT_EOF_CLUSTER;
    ...
    sector_t sec;
    __le32 *fat_entry;
    struct buffer_head *bh = cache ? *cache : core::ptr::null_mut();
    unsigned int off;
    sec = FAT_ENT_OFFSET_SECTOR(sb, loc);
    off = FAT_ENT_OFFSET_BYTE_IN_SECTOR(sb, loc);
    if (!bh || bh.b_blocknr != sec || !buffer_uptodate(bh)) {
...
// fat_entry = cpu_to_le32(content);
    ...
    {
    return __exfat_ent_set(sb, loc, content, core::ptr::null_mut());
    }
//
...
#[no_mangle]
unsafe extern "C" fn __exfat_free_cluster(inode: *mut inode, p_chain: *mut exfat_chain) -> c_int {
    static int __exfat_free_cluster(struct inode *inode, struct exfat_chain *p_chain)
    {
    struct super_block *sb = inode.i_sb;
    struct exfat_sb_info *sbi = EXFAT_SB(sb);
    int cur_cmap_i, next_cmap_i;
    let mut num_clusters: c_uint = 0;
    unsigned int clu;
...
// check cluster validation
    ...
    int err;
    let mut last_cluster: c_uint = p_chain.dir + p_chain.size - 1;
    do {
...
// flush bitmap only if index would be changed or for last cluster
    ...
    let mut nr_clu: c_uint = 1;
    do {
    let mut sync: bool = false;
    let mut n_clu: c_uint = clu;
    let mut err: c_int = exfat_get_next_cluster(sb, &n_clu);
...
#[no_mangle]
pub unsafe extern "C" fn exfat_free_cluster(inode: *mut inode, p_chain: *mut exfat_chain) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn exfat_zeroed_cluster(dir: *mut inode, clu: c_uint) -> c_int {
