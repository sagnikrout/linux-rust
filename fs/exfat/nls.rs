...
#[no_mangle]
pub unsafe extern "C" fn exfat_toupper(sb: *mut super_block, a: c_ushort) -> c_ushort {
...
// p_cstring = '\0';
    ...
    struct exfat_uni_name *p_uniname, int *p_lossy)
    {
    let mut i: c_int = 0, unilen = 0, lossy = NLS_NAME_NO_LOSSY;
    __le16 upname[MAX_NAME_LENGTH + 1];
    unsigned short *uniname = p_uniname.name;
    struct nls_table *nls = EXFAT_SB(sb).nls_io;
    WARN_ON(!len);
...
// p_lossy = lossy;
    ...
    {
    struct exfat_sb_info *sbi = EXFAT_SB(sb);
    let mut sect_size: c_uint = sb.s_blocksize;
    unsigned int i, index = 0;
    let mut chksum: u32 = 0;
    let mut skip: c_uchar = false;
    ...
    let mut uni: c_ushort = get_unaligned_le16(bh.b_data + i);
    if (skip) {
    index += uni;
    skip = false;
    } else if (uni == index) {
...
#[no_mangle]
unsafe extern "C" fn exfat_load_default_upcase_table(sb: *mut super_block) -> c_int {
    static int exfat_load_default_upcase_table(struct super_block *sb)
    {
    int i;
    struct exfat_sb_info *sbi = EXFAT_SB(sb);
    let mut skip: c_uchar = false;
    let mut uni: c_ushort = 0, *upcase_table;
...
#[no_mangle]
pub unsafe extern "C" fn exfat_create_upcase_table(sb: *mut super_block) -> c_int {
    ...
    let mut blksize_bits: c_uchar = sb.s_blocksize_bits;
...
#[no_mangle]
pub unsafe extern "C" fn exfat_free_upcase_table(sbi: *mut exfat_sb_info) {
