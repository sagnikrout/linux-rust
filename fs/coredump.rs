...
// descriptor table.
//
pub const COREDUMP_PIDFD_NUMBER: c_int = 3;
    static int core_uses_pid;
    static unsigned int core_pipe_limit;
    static unsigned int core_sort_vma;
    static char core_pattern[CORENAME_MAX_SIZE] = "core";
    let mut core_name_size: static int = CORENAME_MAX_SIZE;
    let mut core_file_note_size_limit: c_uint = CORE_FILE_NOTE_SIZE_DEFAULT;
    let mut core_pipe_count: static atomic_t = ATOMIC_INIT(0);
...
pub struct core_name {
    pub __counted_by_ptr(size): *mut *mut char corename,
    pub size: int used,,
    pub core_pipe_limit: c_uint,
    pub core_dumped: bool,
    pub core_type: enum coredump_type_t,
    pub mask: u64,
}

#[no_mangle]
unsafe extern "C" fn expand_corename(cn: *mut core_name, size: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __printf(_arg: 2, cn: *mut 3) int cn_printf(struct core_name, fmt: *const c_char, ...) -> static {
...
pub unsafe extern "C" fn __printf(_arg: 2, _arg: 3) -> static {
    static __printf(2, 3)
#[no_mangle]
pub unsafe extern "C" fn cn_esc_printf(cn: *mut core_name, fmt: *const c_char, ...) -> c_int {
    int cn_esc_printf(struct core_name *cn, const char *fmt, ...)
    {
    let mut cur: c_int = cn.used;
    va_list arg;
    int ret;
    va_start(arg, fmt);
    ret = cn_vprintf(cn, fmt, arg);
...
#[no_mangle]
unsafe extern "C" fn cn_print_exe_file(cn: *mut core_name, name_only: bool) -> c_int {
...
//
    ...
    let mut was_space: bool = false;
    let mut pid_in_pattern: c_int = 0;
    let mut err: c_int = 0;
    cn.mask = COREDUMP_KERNEL;
    if (core_pipe_limit)
    cn.mask |= COREDUMP_WAIT;
    cn.used = 0;
    cn.corename = core::ptr::null_mut();
    cn.core_pipe_limit = 0;
    cn.core_dumped = false;
    if (*pat_ptr == '|')
    cn.core_type = COREDUMP_PIPE;
#[no_mangle]
pub unsafe extern "C" fn if('@': *mut *mut pat_ptr ==) -> else {
    else if (*pat_ptr == '@')
    ...
    switch (cn.core_type) {
    case COREDUMP_PIPE: {
    let mut argvs: c_int = sizeof(core_pattern) / 2;
...
#[no_mangle]
unsafe extern "C" fn zap_process(signal: *mut signal_struct, exit_code: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn coredump_wait(exit_code: c_int, core_state: *mut core_state) -> c_int {
    static int coredump_wait(int exit_code, struct core_state *core_state)
    {
    struct task_struct *tsk = current;
    let mut core_waiters: c_int = -EBUSY;
    init_completion(&core_state.startup);
    core_state.dumper.task = tsk;
    core_state.dumper.next = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn coredump_finish(core_dumped: bool) {
    ...
    current.signal.core_state = core::ptr::null_mut();
    spin_unlock_irq(&current.sighand.siglock);
    while ((curr = next) != core::ptr::null_mut()) {
    next = curr.next;
    task = curr.task;
//
...
//
    smp_mb();
    curr.task = core::ptr::null_mut();
    wake_up_process(task);
    }
    }
#[no_mangle]
unsafe extern "C" fn dump_interrupted() -> bool {
...
#[no_mangle]
unsafe extern "C" fn wait_for_dump_helpers(file: *mut file) {
...
//
#[no_mangle]
unsafe extern "C" fn umh_coredump_setup(info: *mut subprocess_info, new: *mut cred) -> c_int {
    ...
    int err;
    if (cp.pid) {
    struct file *pidfs_file __free(fput) = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn coredump_sock_connect(cn: *mut core_name, cprm: *mut coredump_params) -> bool {
...
//
    ...
    if (retval < 0)
    return false;
    file = sock_alloc_file(socket, 0, core::ptr::null_mut());
    if (IS_ERR(file))
    return false;
//
...
#[no_mangle]
pub unsafe extern "C" fn coredump_sock_recv(file: *mut file, ack: *mut coredump_ack, size: usize, flags: c_int) -> bool {
    static inline bool coredump_sock_recv(struct file *file, struct coredump_ack *ack, size_t size, int flags)
    {
    let mut msg: msghdr = {};
    let mut iov: kvec = { .iov_base = ack, .iov_len = size };
    ssize_t ret;
    memset(ack, 0, size);
    ret = kernel_recvmsg(sock_from_file(file), &msg, &iov, 1, size, flags);
    let mut ret: return = = size;
    }
#[no_mangle]
pub unsafe extern "C" fn coredump_sock_send(file: *mut file, req: *mut coredump_req) -> bool {
    static inline bool coredump_sock_send(struct file *file, struct coredump_req *req)
    {
    let mut msg: msghdr = { .msg_flags = MSG_NOSIGNAL };
    let mut iov: kvec = { .iov_base = req, .iov_len = sizeof(*req) };
    ssize_t ret;
    ret = kernel_sendmsg(sock_from_file(file), &msg, &iov, 1, sizeof(*req));
    let mut ret: return = = sizeof(*req);
    }
    static_assert(sizeof(enum coredump_mark) == sizeof(__u32));
#[no_mangle]
pub unsafe extern "C" fn coredump_sock_mark(file: *mut file, mark: enum coredump_mark) -> bool {
    static inline bool coredump_sock_mark(struct file *file, enum coredump_mark mark)
    {
    let mut msg: msghdr = { .msg_flags = MSG_NOSIGNAL };
    let mut iov: kvec = { .iov_base = &mark, .iov_len = sizeof(mark) };
    ssize_t ret;
    ret = kernel_sendmsg(sock_from_file(file), &msg, &iov, 1, sizeof(mark));
    let mut ret: return = = sizeof(mark);
    }
#[no_mangle]
pub unsafe extern "C" fn coredump_sock_wait(file: *mut file) {
...
//
    n = __kernel_read(file, &(char){ 0 }, 1, core::ptr::null_mut());
    if (n > 0)
...
#[no_mangle]
pub unsafe extern "C" fn coredump_sock_shutdown(file: *mut file) {
...
#[no_mangle]
unsafe extern "C" fn coredump_sock_request(cn: *mut core_name, cprm: *mut coredump_params) -> bool {
...
#[no_mangle]
unsafe extern "C" fn coredump_socket(cn: *mut core_name, cprm: *mut coredump_params) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn coredump_force_suid_safe(cprm: *const coredump_params) -> bool {
...
// Require nonrelative corefile path and be extra careful.
    ...
    struct inode *inode;
    struct file *file __free(fput) = core::ptr::null_mut();
    let mut open_flags: c_int = O_CREAT | O_WRONLY | O_NOFOLLOW | O_LARGEFILE | O_EXCL;
...
//
    ...
    {
    int argi;
    char **helper_argv __free(kfree) = core::ptr::null_mut();
...
//
    ...
    helper_argv[argi] = core::ptr::null_mut();
    sub_info = call_usermodehelper_setup(helper_argv[0], helper_argv, core::ptr::null_mut(),
    GFP_KERNEL, umh_coredump_setup,
    core::ptr::null_mut(), cprm);
    if (!sub_info)
    return false;
...
#[no_mangle]
unsafe extern "C" fn coredump_cleanup(cn: *mut core_name, cprm: *mut coredump_params) {
    static void coredump_cleanup(struct core_name *cn, struct coredump_params *cprm)
    {
    if (cprm.file)
    filp_close(cprm.file, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn vfs_coredump(siginfo: *const kernel_siginfo_t) {
    void vfs_coredump(const kernel_siginfo_t *siginfo)
    {
    size_t *argv __free(kfree) = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn __dump_emit(cprm: *mut coredump_params, addr: *const c_void, nr: c_int) -> c_int {
    static int __dump_emit(struct coredump_params *cprm, const void *addr, int nr)
    {
    struct file *file = cprm.file;
    let mut pos: loff_t = file.f_pos;
    ssize_t n;
...
#[no_mangle]
unsafe extern "C" fn __dump_skip(cprm: *mut coredump_params, nr: usize) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dump_emit(cprm: *mut coredump_params, addr: *const c_void, nr: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dump_skip_to(cprm: *mut coredump_params, pos: c_ulong) {
...
#[no_mangle]
pub unsafe extern "C" fn dump_skip(cprm: *mut coredump_params, nr: usize) {
...
#[no_mangle]
unsafe extern "C" fn dump_emit_page(cprm: *mut coredump_params, page: *mut page) -> c_int {
...
//
    ...
    let mut left: usize = copy_mc_to_kernel(page_address(dst), buf, PAGE_SIZE);
    kunmap_local(buf);
    return left ? core::ptr::null_mut() : dst;
    }
...
//
    ...
    let mut stop: c_int = !dump_emit_page(cprm, dump_page_copy(page, dump_page));
    put_page(page);
    if (stop)
    goto out;
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn dump_align(cprm: *mut coredump_params, align: c_int) -> c_int {
    int dump_align(struct coredump_params *cprm, int align)
    {
    let mut mod: unsigned = (cprm.pos + cprm.to_skip) & (align - 1);
    if (align & (align - 1))
    return 0;
    if (mod)
...
#[no_mangle]
pub unsafe extern "C" fn check_coredump_socket() -> bool {
...
// Must not contain ".." in the path.
    ...
    let mut core_file_note_size_min: static unsigned int = CORE_FILE_NOTE_SIZE_DEFAULT;
    let mut core_file_note_size_max: static unsigned int = CORE_FILE_NOTE_SIZE_MAX;
...
#[no_mangle]
unsafe extern "C" fn init_fs_coredump_sysctls() -> int __init {
...
//
#[no_mangle]
unsafe extern "C" fn always_dump_vma(vma: *mut vm_area_struct) -> bool {
...
pub const DUMP_SIZE_MAYBE_ELFHDR_PLACEHOLDER: c_int = 1;
//
...
// Dump segments that have been written to.
    ...
    goto whole;
    if (vma.vm_file == core::ptr::null_mut())
    return 0;
...
#[no_mangle]
unsafe extern "C" fn free_vma_snapshot(cprm: *mut coredump_params) {
    static void free_vma_snapshot(struct coredump_params *cprm)
    {
    if (cprm.vma_meta) {
    int i;
    ...
    cprm.vma_meta = core::ptr::null_mut();
    }
    }
#[no_mangle]
unsafe extern "C" fn cmp_vma_size(vma_meta_lhs_ptr: *const c_void, vma_meta_rhs_ptr: *const c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dump_vma_snapshot(cprm: *mut coredump_params) -> bool {
    static bool dump_vma_snapshot(struct coredump_params *cprm)
    {
    struct vm_area_struct *gate_vma, *vma = core::ptr::null_mut();
...
//
    ...
    while ((vma = coredump_next_vma(&vmi, vma, gate_vma)) != core::ptr::null_mut()) {
    ...
    cmp_vma_size, core::ptr::null_mut());
    return true;
    }
