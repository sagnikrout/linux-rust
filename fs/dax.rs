...
pub const DAX_WAIT_TABLE_BITS: c_int = 12;

// The 'colour' (ie low bits) within a PMD of a page offset.

    static wait_queue_head_t wait_table[DAX_WAIT_TABLE_ENTRIES];
#[no_mangle]
unsafe extern "C" fn init_dax_wait_table() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn dax_to_pfn(entry: *mut c_void) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn dax_is_locked(entry: *mut c_void) -> bool {
    static bool dax_is_locked(void *entry)
    {
    return xa_to_value(entry) & DAX_LOCKED;
    }
#[no_mangle]
unsafe extern "C" fn dax_entry_order(entry: *mut c_void) -> c_uint {
...
#[no_mangle]
unsafe extern "C" fn dax_is_pmd_entry(entry: *mut c_void) -> c_ulong {
    static unsigned long dax_is_pmd_entry(void *entry)
    {
    return xa_to_value(entry) & DAX_PMD;
    }
#[no_mangle]
unsafe extern "C" fn dax_is_pte_entry(entry: *mut c_void) -> bool {
    static bool dax_is_pte_entry(void *entry)
    {
    return !(xa_to_value(entry) & DAX_PMD);
    }
#[no_mangle]
unsafe extern "C" fn dax_is_zero_entry(entry: *mut c_void) -> c_int {
    static int dax_is_zero_entry(void *entry)
    {
    return xa_to_value(entry) & DAX_ZERO_PAGE;
    }
#[no_mangle]
unsafe extern "C" fn dax_is_empty_entry(entry: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn dax_is_conflict(entry: *mut c_void) -> bool {
    static bool dax_is_conflict(void *entry)
    {
    let mut entry: return = = XA_RETRY_ENTRY;
    }
//
...
//
    ...
    return 0;
    return autoremove_wake_function(wait, mode, sync, core::ptr::null_mut());
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn wait_entry_unlocked(xas: *mut xa_state, entry: *mut c_void) {
...
//
#[no_mangle]
unsafe extern "C" fn dax_unlock_entry(xas: *mut xa_state, entry: *mut c_void) {
...
//
    ...
    {
    let mut v: c_ulong = xa_to_value(entry);
    return xas_store(xas, xa_mk_value(v | DAX_LOCKED));
    }
#[no_mangle]
unsafe extern "C" fn dax_entry_size(entry: *mut c_void) -> c_ulong {
    static unsigned long dax_entry_size(void *entry)
    {
    if (dax_is_zero_entry(entry))
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: dax_is_empty_entry(entry)) -> else {
    else if (dax_is_empty_entry(entry))
    return 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: dax_is_pmd_entry(entry)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn dax_folio_is_shared(folio: *mut folio) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn dax_folio_make_shared(folio: *mut folio) {
...
//
#[no_mangle]
pub unsafe extern "C" fn dax_folio_reset_order(folio: *mut folio) -> c_int {
    int dax_folio_reset_order(struct folio *folio)
    {
    struct dev_pagemap *pgmap = page_pgmap(&folio.page);
    let mut order: c_int = folio_order(folio);
//
...
#[no_mangle]
pub unsafe extern "C" fn dax_folio_put(folio: *mut folio) -> c_ulong {
...
#[no_mangle]
unsafe extern "C" fn dax_folio_init(entry: *mut c_void) {
    static void dax_folio_init(void *entry)
    {
    struct folio *folio = dax_to_folio(entry);
    let mut order: c_int = dax_entry_order(entry);
//
...
//
    ...
    {
    let mut size: c_ulong = dax_entry_size(entry), index;
    struct folio *folio = dax_to_folio(entry);
...
//
#[no_mangle]
pub unsafe extern "C" fn dax_lock_folio(folio: *mut folio) -> dax_entry_t {
...
#[no_mangle]
pub unsafe extern "C" fn dax_unlock_folio(folio: *mut folio, cookie: dax_entry_t) {
...
//
    ...
    xas_store(xas, core::ptr::null_mut());	/* undo the PMD join */
    ...
    let mut flags: c_ulong = DAX_EMPTY;
    if (order > 0)
    flags |= DAX_PMD;
...
//
    ...
    {
    void *entry;
    let mut scanned: c_uint = 0;
    struct page *page = core::ptr::null_mut();
    let mut start_idx: pgoff_t = start >> PAGE_SHIFT;
    pgoff_t end_idx;
...
//
#[no_mangle]
pub unsafe extern "C" fn dax_delete_mapping_entry(mapping: *mut address_space, index: pgoff_t) -> c_int {
    int dax_delete_mapping_entry(struct address_space *mapping, pgoff_t index)
    {
    let mut ret: c_int = __dax_invalidate_entry(mapping, index, true);
//
...
#[no_mangle]
pub unsafe extern "C" fn dax_break_layout_final(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn dax_iomap_pgoff(iomap: *const iomap, pos: loff_t) -> pgoff_t {
    static pgoff_t dax_iomap_pgoff(const struct iomap *iomap, loff_t pos)
    {
    return PHYS_PFN(iomap.addr + (pos & PAGE_MASK) - iomap.offset);
    }
#[no_mangle]
unsafe extern "C" fn copy_cow_page_dax(vmf: *mut vm_fault, iter: *const iomap_iter) -> c_int {
    static int copy_cow_page_dax(struct vm_fault *vmf, const struct iomap_iter *iter)
    {
    let mut pgoff: pgoff_t = dax_iomap_pgoff(&iter.iomap, iter.pos);
    void *vto, *kaddr;
    long rc;
    int id;
    id = dax_read_lock();
...
//
    ...
    let mut write: bool = iter.flags & IOMAP_WRITE;
    let mut dirty: bool = write && !dax_fault_is_synchronous(iter, vmf.vma);
    let mut shared: bool = iter.iomap.flags & IOMAP_F_SHARED;
    if (dirty)
...
//
    ...
    let mut end_index: pgoff_t = wbc.range_end >> PAGE_SHIFT;
    void *entry;
    let mut ret: c_int = 0;
    let mut scanned: c_uint = 0;
    ...
    {
    let mut pgoff: pgoff_t = dax_iomap_pgoff(iomap, pos);
    int id, rc = 0;
    long length;
    id = dax_read_lock();
...
//
    ...
    {
    let mut head_off: loff_t = pos & (align_size - 1);
    let mut size: usize = ALIGN(head_off + length, align_size);
    let mut end: loff_t = pos + length;
    let mut pg_end: loff_t = round_up(end, align_size);
// copy_all is usually in page fault case
    let mut copy_all: bool = head_off == 0 && end == pg_end;
// zero the edges if srcmap is a HOLE or IOMAP_UNWRITTEN
    bool zero_edge = srcmap.flags & IOMAP_F_SHARED ||
    srcmap.type == IOMAP_UNWRITTEN;
    void *saddr = core::ptr::null_mut();
    let mut ret: c_int = 0;
    if (!zero_edge) {
    ret = dax_iomap_direct_access(srcmap, pos, size, &saddr, core::ptr::null_mut());
    if (ret)
...
// Copy the tail part of the range
    if (end < pg_end) {
    let mut tail_off: loff_t = head_off + length;
    let mut tail_len: loff_t = pg_end - end;
    if (zero_edge)
...
//
    ...
    let mut vaddr: c_ulong = vmf.address;
    let mut pfn: c_ulong = zero_pfn(vaddr);
    vm_fault_t ret;
...
#[no_mangle]
unsafe extern "C" fn dax_unshare_iter(iter: *mut iomap_iter) -> c_int {
    static int dax_unshare_iter(struct iomap_iter *iter)
    {
    struct iomap *iomap = &iter.iomap;
    const struct iomap *srcmap = iomap_iter_srcmap(iter);
    let mut copy_pos: loff_t = iter.pos;
    let mut copy_len: u64 = iomap_length(iter);
    u32 mod;
    let mut id: c_int = 0;
    s64 ret;
    void *daddr = core::ptr::null_mut(), *saddr = core::ptr::null_mut();
...
//
    ...
    id = dax_read_lock();
    ret = dax_iomap_direct_access(iomap, copy_pos, copy_len, &daddr, core::ptr::null_mut());
    if (ret < 0)
    goto out_unlock;
    ret = dax_iomap_direct_access(srcmap, copy_pos, copy_len, &saddr, core::ptr::null_mut());
    if (ret < 0)
    goto out_unlock;
...
#[no_mangle]
unsafe extern "C" fn dax_memzero(iter: *mut iomap_iter, pos: loff_t, size: usize) -> c_int {
    static int dax_memzero(struct iomap_iter *iter, loff_t pos, size_t size)
    {
    const struct iomap *iomap = &iter.iomap;
    const struct iomap *srcmap = iomap_iter_srcmap(iter);
    let mut offset: unsigned = offset_in_page(pos);
    let mut pgoff: pgoff_t = dax_iomap_pgoff(iomap, pos);
    void *kaddr;
    long ret;
...
#[no_mangle]
unsafe extern "C" fn dax_zero_iter(iter: *mut iomap_iter, did_zero: *mut bool) -> c_int {
    static int dax_zero_iter(struct iomap_iter *iter, bool *did_zero)
    {
    const struct iomap *iomap = &iter.iomap;
    const struct iomap *srcmap = iomap_iter_srcmap(iter);
    let mut length: u64 = iomap_length(iter);
    int ret;
...
// did_zero = true;
    ...
    let mut off: c_uint = pos & (blocksize - 1);
...
#[no_mangle]
unsafe extern "C" fn dax_iomap_iter(iomi: *mut iomap_iter, iter: *mut iov_iter) -> c_int {
    static int dax_iomap_iter(struct iomap_iter *iomi, struct iov_iter *iter)
    {
    const struct iomap *iomap = &iomi.iomap;
    const struct iomap *srcmap = iomap_iter_srcmap(iomi);
    let mut length: loff_t = iomap_length(iomi);
    let mut pos: loff_t = iomi.pos;
    struct dax_device *dax_dev = iomap.dax_dev;
    let mut end: loff_t = pos + length, done = 0;
    let mut write: bool = iov_iter_rw(iter) == WRITE;
    let mut cow: bool = write && iomap.flags & IOMAP_F_SHARED;
    let mut ret: isize = 0;
    size_t xfer;
    int id;
    if (!write) {
...
//
    ...
    let mut offset: unsigned = pos & (PAGE_SIZE - 1);
    let mut size: usize = ALIGN(length + offset, PAGE_SIZE);
    let mut pgoff: pgoff_t = dax_iomap_pgoff(iomap, pos);
    ssize_t map_len;
    ...
    DAX_ACCESS, &kaddr, core::ptr::null_mut());
    ...
    map_len, iter);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: write) -> else {
    else if (write)
...
#[no_mangle]
unsafe extern "C" fn dax_fault_return(error: c_int) -> vm_fault_t {
...
//
    ...
    const struct iomap *iomap = &iter.iomap;
    const struct iomap *srcmap = iomap_iter_srcmap(iter);
    let mut size: usize = pmd ? PMD_SIZE : PAGE_SIZE;
    let mut pos: loff_t = (loff_t)xas.xa_index << PAGE_SHIFT;
    let mut write: bool = iter.flags & IOMAP_WRITE;
    let mut entry_flags: c_ulong = pmd ? DAX_PMD : 0;
    struct folio *folio;
    int ret, err = 0;
    unsigned long pfn;
    void *kaddr;
...
// iomap_errp = error;
    ...
    pgoff_t max_pgoff)
    {
    let mut pmd_addr: c_ulong = vmf.address & PMD_MASK;
...
#[no_mangle]
pub unsafe extern "C" fn if(PMD_ORDER: order ==) -> else {
...
//
    ...
    {
    int err;
    let mut start: loff_t = ((loff_t)vmf.pgoff) << PAGE_SHIFT;
    let mut len: usize = PAGE_SIZE << order;
    ...
    let mut pos1: loff_t = it_src.pos, pos2 = it_dest.pos;
    void *saddr, *daddr;
    int id, ret;
