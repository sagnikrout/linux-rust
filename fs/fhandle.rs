...
// SPDX-License-Identifier: GPL-2.0
    ...
    struct file_handle *handle = core::ptr::null_mut();
//
...
//
    ...
    return -EINVAL;
#[no_mangle]
pub unsafe extern "C" fn if(AT_HANDLE_FID: flag &) -> else {
    else if (flag & AT_HANDLE_FID)
    fh_flags |= EXPORT_FH_FID;
#[no_mangle]
pub unsafe extern "C" fn if(AT_HANDLE_CONNECTABLE: flag &) -> else {
    else if (flag & AT_HANDLE_CONNECTABLE)
    fh_flags |= EXPORT_FH_CONNECTABLE;
    lookup_flags = (flag & AT_SYMLINK_FOLLOW) ? LOOKUP_FOLLOW : 0;
    CLASS(filename_uflags, filename)(name, flag);
    err = filename_lookup(dfd, filename, lookup_flags, &path, core::ptr::null_mut());
    if (!err) {
...
#[no_mangle]
unsafe extern "C" fn get_path_anchor(fd: c_int, root: *mut path) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn vfs_dentry_acceptable(context: *mut c_void, dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn capable_wrt_mount(mount: *mut mount) -> bool {
...
// Are we able to override DAC permissions?
    ...
    let mut retval: c_int = 0;
    struct file_handle f_handle;
    struct file_handle *handle __free(kfree) = core::ptr::null_mut();
    let mut ctx: handle_to_path_ctx = {};
