...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mext_data {
    pub /: *mut *mut *mut inode orig_inode; / Origin file inode,
    pub /: *mut *mut *mut inode donor_inode; / Donor file inode,
    pub /: *mut *mut ext4_map_blocks orig_map;/ Origin file's move mapping,
    pub /: *mut *mut ext4_lblk_t donor_lblk; / Start block of the donor file,
}
...
#[no_mangle]
unsafe extern "C" fn mext_folio_double_unlock(folio[2]: *mut folio) {
...
#[no_mangle]
unsafe extern "C" fn mext_folio_mkuptodate(folio: *mut folio, from: usize, to: usize) -> c_int {
    ...
    continue;
    if (!buffer_mapped(bh)) {
    let mut err: c_int = ext4_get_block(inode, block, bh, 0);
    if (err)
    return err;
    if (!buffer_mapped(bh)) {
    ...
    ext4_read_bh_nowait(bh, 0, core::ptr::null_mut(), false);
    nr++;
...
//
    ...
    let mut blkbits: c_uint = orig_inode.i_blkbits;
    let mut donor_map: ext4_map_blocks = {0};
...
// Adjust the moving length according to the length of shorter folio.
    ...
    donor_map.m_flags = 0;
    ret = ext4_map_blocks(core::ptr::null_mut(), donor_inode, &donor_map, 0);
    if (ret < 0)
    goto error;
...
//
#[no_mangle]
unsafe extern "C" fn mext_move_extent(mext: *mut mext_data, m_len: *mut u64) -> c_int {
    ...
    let mut blkbits: c_uint = orig_inode.i_blkbits;
    struct folio *folio[2] = {core::ptr::null_mut(), core::ptr::null_mut()};
    loff_t from, length;
    let mut move_type: enum mext_move_type = 0;
    handle_t *handle;
    let mut r_len: u64 = 0;
...
// len = 0;
#[no_mangle]
pub unsafe extern "C" fn if(1: *mut *mut orig_eof < orig_start + len -) -> else {
    else if (orig_eof < orig_start + *len - 1)
// len = orig_eof - orig_start;
    if (donor_eof <= donor_start)
// len = 0;
#[no_mangle]
pub unsafe extern "C" fn if(1: *mut *mut donor_eof < donor_start + len -) -> else {
...
// Check and adjust the specified move_extent range.
    ...
    ret = ext4_map_blocks(core::ptr::null_mut(), orig_inode, &mext.orig_map, 0);
    if (ret < 0)
    goto out;
