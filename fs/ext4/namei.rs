...
//
pub const NAMEI_RA_CHUNKS: c_int = 2;
pub const NAMEI_RA_BLOCKS: c_int = 4;
...
// directory.
//
    err = ext4_map_blocks(core::ptr::null_mut(), inode, &map, 0);
    if (err < 0)
    return ERR_PTR(err);
    if (err) {
...
//
    ...
    bh = ext4_bread(core::ptr::null_mut(), inode, block, 0);
    if (IS_ERR(bh)) {
...
// Macro flag: #define dxtrace(command)
...
pub struct dx_tail {
    pub dt_reserved: u32,
    pub /: *mut *mut __le32 dt_checksum; / crc32c(uuid+inum+dirblock),
}
...
// Walk through a dirent block to find a checksum "dirent" at the tail
    ...
    let mut blocksize: c_int = EXT4_BLOCK_SIZE(inode.i_sb);
...
#[no_mangle]
unsafe extern "C" fn ext4_dirblock_csum(inode: *mut inode, dirent: *mut c_void, size: c_int) -> __le32 {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_dirblock_csum_verify(inode: *mut inode, bh: *mut buffer_head) -> c_int {
    ...
    int count_offset;
    let mut blocksize: c_int = EXT4_BLOCK_SIZE(inode.i_sb);
    let mut rlen: c_uint = ext4_rec_len_from_disk(dirent.rec_len, blocksize);
    if (rlen == blocksize)
    count_offset = 8;
#[no_mangle]
pub unsafe extern "C" fn if(12: rlen ==) -> else {
...
// offset = count_offset;
    ...
    __u32 csum;
    int size;
    let mut dummy_csum: __u32 = 0;
    let mut offset: c_int = offsetof(struct dx_tail, dt_checksum);
...
#[no_mangle]
unsafe extern "C" fn ext4_dx_csum_set(inode: *mut inode, dirent: *mut ext4_dir_entry) {
...
//
#[no_mangle]
pub unsafe extern "C" fn dx_get_block(entry: *mut dx_entry) -> ext4_lblk_t {
    static inline ext4_lblk_t dx_get_block(struct dx_entry *entry)
    {
    return le32_to_cpu(entry.block) & 0x0fffffff;
    }
#[no_mangle]
pub unsafe extern "C" fn dx_set_block(entry: *mut dx_entry, value: ext4_lblk_t) {
    static inline void dx_set_block(struct dx_entry *entry, ext4_lblk_t value)
    {
    entry.block = cpu_to_le32(value);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_get_hash(entry: *mut dx_entry) -> unsigned {
    static inline unsigned dx_get_hash(struct dx_entry *entry)
    {
    return le32_to_cpu(entry.hash);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_set_hash(entry: *mut dx_entry, value: unsigned) {
    static inline void dx_set_hash(struct dx_entry *entry, unsigned value)
    {
    entry.hash = cpu_to_le32(value);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_get_count(entries: *mut dx_entry) -> unsigned {
    static inline unsigned dx_get_count(struct dx_entry *entries)
    {
    return le16_to_cpu(((struct dx_countlimit *) entries).count);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_get_limit(entries: *mut dx_entry) -> unsigned {
    static inline unsigned dx_get_limit(struct dx_entry *entries)
    {
    return le16_to_cpu(((struct dx_countlimit *) entries).limit);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_set_count(entries: *mut dx_entry, value: unsigned) {
    static inline void dx_set_count(struct dx_entry *entries, unsigned value)
    {
    ((struct dx_countlimit *) entries).count = cpu_to_le16(value);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_set_limit(entries: *mut dx_entry, value: unsigned) {
    static inline void dx_set_limit(struct dx_entry *entries, unsigned value)
    {
    ((struct dx_countlimit *) entries).limit = cpu_to_le16(value);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_root_limit(dir: *mut inode, infosize: unsigned) -> unsigned {
    static inline unsigned dx_root_limit(struct inode *dir, unsigned infosize)
    {
    unsigned int entry_space = dir.i_sb.s_blocksize -
    ext4_dir_rec_len(1, core::ptr::null_mut()) -
    ext4_dir_rec_len(2, core::ptr::null_mut()) - infosize;
    if (ext4_has_feature_metadata_csum(dir.i_sb))
    entry_space -= sizeof(struct dx_tail);
    return entry_space / sizeof(struct dx_entry);
    }
#[no_mangle]
pub unsafe extern "C" fn dx_node_limit(dir: *mut inode) -> unsigned {
...
#[no_mangle]
unsafe extern "C" fn dx_show_index(label: *mut *mut c_char, entries: *mut dx_entry) {
    ...
    {
    let mut names: unsigned = 0, space = 0;
    char *base = (char *) de;
    let mut h: dx_hash_info = *hinfo;
    printk("names: ");
...
// Directory is encrypted
    ...
    {
    let mut blocksize: unsigned = dir.i_sb.s_blocksize;
    let mut count: unsigned = dx_get_count(entries), names = 0, space = 0, i;
    let mut bcount: unsigned = 0;
    struct buffer_head *bh;
    printk("%i indexed blocks...\n", count);
    for (i = 0; i < count; i++, entries++)
    {
    let mut block: ext4_lblk_t = dx_get_block(entries);
    let mut hash: ext4_lblk_t = i ? dx_get_hash(entries): 0;
    let mut range: u32 = i < count - 1? (dx_get_hash(entries + 1) - hash): ~hash;
    struct stats stats;
    printk("%s%3u:%03u hash %8x/%8x ",levels?"":"   ", i, block, hash, range);
    bh = ext4_bread(core::ptr::null_mut(),dir, block, 0);
...
#[no_mangle]
unsafe extern "C" fn dx_release(frames: *mut dx_frame) {
    static void dx_release(struct dx_frame *frames)
    {
    struct dx_root_info *info;
    int i;
    unsigned int indirect_levels;
    if (frames[0].bh == core::ptr::null_mut())
    return;
    info = &((struct dx_root *)frames[0].bh.b_data).info;
// save local copy, "info" may be freed after brelse()
    indirect_levels = info.indirect_levels;
    for (i = 0; i <= indirect_levels; i++) {
    if (frames[i].bh == core::ptr::null_mut())
    break;
    brelse(frames[i].bh);
    frames[i].bh = core::ptr::null_mut();
    }
    }
//
...
//
    ...
    let mut err: c_int = 0, count = 0;
    let mut fname_crypto_str: fscrypt_str = FSTR_INIT(core::ptr::null_mut(), 0), tmp_str;
    let mut csum: c_int = ext4_has_feature_metadata_csum(dir.i_sb);
...
// Check if the directory is encrypted
    ...
    if (ext4_check_dir_entry(dir, core::ptr::null_mut(), de, bh,
    bh.b_data, bh.b_size,
...
// silently ignore the rest of the block
    ...
    let mut save_len: c_int = fname_crypto_str.len;
...
//
    ...
    let mut has_inline_data: c_int = 1;
...
// next_hash = ~0;
    ...
    hinfo.hash = start_hash;
    hinfo.minor_hash = 0;
    frame = dx_probe(core::ptr::null_mut(), dir, &hinfo, frames);
    if (IS_ERR(frame))
...
//
    ...
    let mut buflen: c_uint = bh.b_size;
    char *base = bh.b_data;
    let mut h: dx_hash_info = *hinfo;
    let mut blocksize: c_int = EXT4_BLOCK_SIZE(dir.i_sb);
    if (ext4_has_feature_metadata_csum(dir.i_sb))
    buflen -= sizeof(struct ext4_dir_entry_tail);
    while ((char *) de < base + buflen) {
    if (ext4_check_dir_entry(dir, core::ptr::null_mut(), de, bh, base, buflen,
    ((char *)de) - base))
    return -EFSCORRUPTED;
    if (de.name_len && de.inode) {
    if (ext4_hash_in_dirent(dir))
...
#[no_mangle]
unsafe extern "C" fn dx_sort_map(map: *mut dx_map_entry, count: unsigned) {
...
#[no_mangle]
unsafe extern "C" fn dx_insert_block(frame: *mut dx_frame, hash: u32, block: ext4_lblk_t) {
    static void dx_insert_block(struct dx_frame *frame, u32 hash, ext4_lblk_t block)
    {
    struct dx_entry *entries = frame.entries;
    struct dx_entry *old = frame.at, *new = old + 1;
    let mut count: c_int = dx_get_count(entries);
    ...
    cf_name.name = core::ptr::null_mut();
    return 0;
    }
...
// a full check
    if (ext4_check_dir_entry(dir, core::ptr::null_mut(), de, bh, search_buf,
    buf_size, offset))
    return -EFSCORRUPTED;
// res_dir = de;
    return 1;
...
//
    ...
    struct buffer_head *bh, *ret = core::ptr::null_mut();
...
// Refill the readahead buffer
    ...
    if ((bh = bh_use[ra_ptr++]) == core::ptr::null_mut())
    goto next;
    wait_on_buffer(bh);
...
// Clean up the read-ahead blocks
    ...
    if (err)
    return ERR_PTR(err);
    bh = __ext4_find_entry(dir, &fname, res_dir, core::ptr::null_mut());
    ext4_fname_free_filename(&fname);
    return bh;
    }
    static struct buffer_head * ext4_dx_find_entry(struct inode *dir,
...
// res_dir = NULL;
    ...
    frame = dx_probe(fname, dir, core::ptr::null_mut(), frames);
    if (IS_ERR(frame))
...
// Check to see if we should continue to search
    ...
    {
    struct inode *inode;
    struct ext4_dir_entry_2 *de = core::ptr::null_mut();
    struct buffer_head *bh;
    ...
    if (bh) {
    let mut ino: __u32 = le32_to_cpu(de.inode);
    brelse(bh);
...
//
    ...
    {
    __u32 ino;
    let mut de: *mut ext4_dir_entry_2 = core::ptr::null_mut();
    struct buffer_head *bh;
    bh = ext4_find_entry(d_inode(child), &dotdot_name, &de, core::ptr::null_mut());
    if (IS_ERR(bh))
    return ERR_CAST(bh);
    if (!bh)
...
//
    ...
    struct ext4_dir_entry_2 *de = core::ptr::null_mut(), *de2;
    let mut csum_size: c_int = 0;
...
// bh = NULL;
    ...
    {
    struct ext4_dir_entry_2 *de;
    let mut reclen: c_ushort = ext4_dir_rec_len(fname_len(fname), dir);
    int nlen, rlen;
...
//
    ...
    struct buffer_head *bh)
    {
    let mut blocksize: c_uint = dir.i_sb.s_blocksize;
    let mut csum_size: c_int = 0;
    int		err, err2;
    if (ext4_has_feature_metadata_csum(inode.i_sb))
    csum_size = sizeof(struct ext4_dir_entry_tail);
...
#[no_mangle]
unsafe extern "C" fn ext4_check_dx_root(dir: *mut inode, root: *mut dx_root) -> bool {
...
// Allocate new block for the 0th block's dirents
    ...
    if (ext4_check_dir_entry(dir, core::ptr::null_mut(), de, bh2, data2, len,
    (char *)de - data2)) {
    brelse(bh2);
    brelse(bh);
    return -EFSCORRUPTED;
...
// Initialize the root; the dot dirents already exist
    ...
    blocksize - ext4_dir_rec_len(2, core::ptr::null_mut()), blocksize);
...
// Can we just ignore htree data?
    ...
    inode, bh);
    bh = core::ptr::null_mut(); /* make_indexed_dir releases bh */
    goto out;
    }
    brelse(bh);
    }
...
//
    ...
    if (err)
    goto journal_error;
    err = add_dirent_to_buf(handle, fname, dir, inode, core::ptr::null_mut(), bh);
    if (err != -ENOSPC)
    goto cleanup;
    err = 0;
...
// Need to split index?
    ...
    ext4_lblk_t newblock;
    let mut levels: c_int = frame - frames + 1;
    unsigned int icount;
    let mut add_level: c_int = 1;
    ...
    if (!add_level) {
    let mut icount1: unsigned = icount/2, icount2 = icount - icount1;
    let mut hash2: unsigned = dx_get_hash(entries + icount1);
...
//
#[no_mangle]
unsafe extern "C" fn ext4_inc_count(inode: *mut inode) {
...
//
#[no_mangle]
unsafe extern "C" fn ext4_dec_count(inode: *mut inode) {
...
//
    ...
    let mut err: c_int = ext4_add_entry(handle, dentry, inode);
    if (!err) {
    err = ext4_mark_inode_dirty(handle, inode);
    if (IS_DIRSYNC(dir))
...
//
    ...
    0, core::ptr::null_mut(), EXT4_HT_DIR, credits);
    ...
    {
    struct ext4_dir_entry_2 *de = (struct ext4_dir_entry_2 *) bh.b_data;
    let mut blocksize: usize = bh.b_size;
    let mut csum_size: c_int = 0, header_size;
    if (ext4_has_feature_metadata_csum(inode.i_sb))
    csum_size = sizeof(struct ext4_dir_entry_tail);
    de.inode = cpu_to_le32(inode.i_ino);
    de.name_len = 1;
    de.rec_len = ext4_rec_len_to_disk(ext4_dir_rec_len(de.name_len, core::ptr::null_mut()),
    blocksize);
    memcpy(de.name, ".", 2);
    ...
    if (inline_buf) {
    de.rec_len = ext4_rec_len_to_disk(
    ext4_dir_rec_len(de.name_len, core::ptr::null_mut()),
    blocksize);
    de = ext4_next_entry(de, blocksize);
    ...
    (csum_size + ext4_dir_rec_len(1, core::ptr::null_mut())),
    blocksize);
    }
    if (csum_size)
    ext4_initialize_dirent_tail(bh, blocksize);
    ...
    struct inode *inode)
    {
    struct buffer_head *dir_block = core::ptr::null_mut();
    let mut block: ext4_lblk_t = 0;
    int err;
    ...
    err = ext4_init_dirblock(handle, inode, dir_block, dir.i_ino, core::ptr::null_mut(), 0);
    out:
    brelse(dir_block);
    return err;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_empty_dir(inode: *mut inode) -> bool {
    ...
    sb = inode.i_sb;
    if (inode.i_size < ext4_dir_rec_len(1, core::ptr::null_mut()) +
    ext4_dir_rec_len(2, core::ptr::null_mut())) {
    ...
    return false;
    de = (struct ext4_dir_entry_2 *) bh.b_data;
    if (ext4_check_dir_entry(inode, core::ptr::null_mut(), de, bh, bh.b_data, bh.b_size,
    0) ||
    ...
    if (ext4_check_dir_entry(inode, core::ptr::null_mut(), de, bh,
...
#[no_mangle]
unsafe extern "C" fn ext4_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
// separate transaction
    ...
    return retval;
    retval = -ENOENT;
    bh = ext4_find_entry(dir, &dentry.d_name, &de, core::ptr::null_mut());
    if (IS_ERR(bh))
    return PTR_ERR(bh);
    if (!bh)
    goto end_rmdir;
...
//
    ...
    struct inode *inode,
    struct dentry *dentry /* core::ptr::null_mut() during fast_commit recovery */)
    {
    let mut retval: c_int = -ENOENT;
    struct buffer_head *bh;
    struct ext4_dir_entry_2 *de = core::ptr::null_mut();
    handle_t *handle;
    let mut skip_remove_dentry: c_int = 0;
//
// Keep this outside the transaction; it may have to set up the
// directory's encryption key, which isn't GFP_NOFS-safe.
//
    bh = ext4_find_entry(dir, d_name, &de, core::ptr::null_mut());
    if (IS_ERR(bh))
    return PTR_ERR(bh);
    if (!bh)
    return -ENOENT;
...
#[no_mangle]
unsafe extern "C" fn ext4_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
//
    ...
    &dentry.d_name, 0, core::ptr::null_mut(),
    EXT4_HT_DIR, credits);
    handle = ext4_journal_current_handle();
    if (IS_ERR(inode)) {
    if (handle)
...
// retval = PTR_ERR(bh);
    ...
    de = (struct ext4_dir_entry_2 *) bh.b_data;
    if (ext4_check_dir_entry(inode, core::ptr::null_mut(), de, bh, bh.b_data,
    bh.b_size, 0) ||
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ext4_renament {
    pub dir: *mut inode,
    pub dentry: *mut dentry,
    pub inode: *mut inode,
    pub is_dir: bool,
    pub dir_nlink_delta: c_int,
// entry for "dentry"
    pub bh: *mut buffer_head,
    pub de: *mut ext4_dir_entry_2,
    pub inlined: c_int,
// entry for ".." in inode if it's a directory
    pub dir_bh: *mut buffer_head,
    pub parent_de: *mut ext4_dir_entry_2,
    pub dir_inlined: c_int,
}

#[no_mangle]
unsafe extern "C" fn ext4_rename_dir_prepare(handle: *mut handle_t, ent: *mut ext4_renament, is_cross: bool) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext4_update_dir_count(handle: *mut handle_t, ent: *mut ext4_renament) {
...
//
    ...
    &ent.dentry.d_name, 0, core::ptr::null_mut(),
    EXT4_HT_DIR, credits);
    handle = ext4_journal_current_handle();
    if (IS_ERR(wh)) {
    if (handle)
...
//
    ...
    int force_reread;
    int retval;
    struct inode *whiteout = core::ptr::null_mut();
    int credits;
    u8 old_file_type;
