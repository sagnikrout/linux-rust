...
#[no_mangle]
unsafe extern "C" fn mbt_free_inode(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn mbt_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mbt_mb_init(sb: *mut super_block) -> c_int {
...
// needed by ext4_mb_init->bdev_rot(sb->s_bdev)
    ...
    if (sb.s_bdev.bd_queue == core::ptr::null_mut()) {
    kfree(sb.s_bdev);
    return -ENOMEM;
    }
//
...
#[no_mangle]
unsafe extern "C" fn mbt_mb_release(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn mbt_set(sb: *mut super_block, fc: *mut fs_context) -> c_int {
    ...
    fsb = kzalloc_obj(*fsb);
    if (fsb == core::ptr::null_mut())
    return core::ptr::null_mut();
    fc = fs_context_for_mount(&mbt_fs_type, 0);
    if (IS_ERR(fc))
    goto out;
    sb = sget_fc(fc, core::ptr::null_mut(), mbt_set);
    put_fs_context(fc);
    if (IS_ERR(sb))
    goto out;
    sbi = &fsb.sbi;
...
#[no_mangle]
unsafe extern "C" fn mbt_ext4_free_super_block(sb: *mut super_block) {
...
pub struct mbt_ext4_block_layout {
    pub blocksize_bits: c_uchar,
    pub cluster_bits: c_uint,
    ...
    {
    let mut max: ext4_grpblk_t = EXT4_CLUSTERS_PER_GROUP(sb);
    grp_ctx.bitmap_bh.b_data = kzalloc(EXT4_BLOCK_SIZE(sb), GFP_KERNEL);
    if (grp_ctx.bitmap_bh.b_data == core::ptr::null_mut())
    return -ENOMEM;
...
#[no_mangle]
unsafe extern "C" fn mbt_grp_ctx_release(grp_ctx: *mut mbt_grp_ctx) {
    static void mbt_grp_ctx_release(struct mbt_grp_ctx *grp_ctx)
    {
    kfree(grp_ctx.bitmap_bh.b_data);
    grp_ctx.bitmap_bh.b_data = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn mbt_ctx_init(sb: *mut super_block) -> c_int {
    ...
    if (ctx.grp_ctx == core::ptr::null_mut())
    return -ENOMEM;
    for (i = 0; i < ngroups; i++)
...
#[no_mangle]
unsafe extern "C" fn mbt_ctx_release(sb: *mut super_block) {
...
pub const TEST_GOAL_GROUP: c_int = 1;
#[no_mangle]
unsafe extern "C" fn mbt_kunit_init(test: *mut kunit) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mbt_kunit_exit(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_new_blocks_simple(test: *mut kunit) {
...
pub const TEST_RANGE_COUNT: c_int = 8;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct test_range {
    pub start: ext4_grpblk_t,
    pub len: ext4_grpblk_t,
}
...
#[no_mangle]
unsafe extern "C" fn test_free_blocks_simple(test: *mut kunit) {
...
// ext4_mb_mark_diskspace_used will BUG if len is 0
    ...
    ret = ext4_mb_mark_diskspace_used_test(ac, core::ptr::null_mut());
...
#[no_mangle]
unsafe extern "C" fn test_mark_diskspace_used(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_mb_generate_buddy(test: *mut kunit) {
...
#[no_mangle]
unsafe extern "C" fn test_mb_mark_used(test: *mut kunit) {
...
// mb_free_blocks will WARN if len is 0
    ...
    mb_free_blocks_test(core::ptr::null_mut(), e4b, start, len);
...
#[no_mangle]
unsafe extern "C" fn test_mb_free_blocks(test: *mut kunit) {
...
pub const COUNT_FOR_ESTIMATE: c_int = 100000;
#[no_mangle]
unsafe extern "C" fn test_mb_mark_used_cost(test: *mut kunit) {
...
// buddy cache assumes that each page contains at least one block
    ...
    continue;
    ext4_lock_group(sb, TEST_GOAL_GROUP);
    mb_free_blocks_test(core::ptr::null_mut(), &e4b, ranges[i].start,
    ranges[i].len);
    ext4_unlock_group(sb, TEST_GOAL_GROUP);
    }
    }
