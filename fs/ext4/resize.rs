...
#[no_mangle]
pub unsafe extern "C" fn ext4_kvfree_array_rcu(to_free: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_resize_begin(sb: *mut super_block) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_resize_end(sb: *mut super_block, update_backups: bool) -> c_int {
    ...
    let mut start: ext4_fsblk_t = ext4_blocks_count(es);
    let mut end: ext4_fsblk_t = start + input.blocks_count;
    let mut group: ext4_group_t = input.group;
    let mut itend: ext4_fsblk_t = input.inode_table + sbi.s_itb_per_group;
    unsigned overhead;
    ...
    ext4_get_group_no_and_offset(sb, start, core::ptr::null_mut(), &offset);
    if (offset != 0)
    ext4_warning(sb, "Last group not full");
#[no_mangle]
pub unsafe extern "C" fn if(5: input->reserved_blocks > input->blocks_count /) -> else {
    else if (input.reserved_blocks > input.blocks_count / 5)
    ext4_warning(sb, "Reserved blocks too high (%u)",
    input.reserved_blocks);
#[no_mangle]
pub unsafe extern "C" fn if(0: free_blocks_count <) -> else {
    else if (free_blocks_count < 0)
    ext4_warning(sb, "Bad blocks count %u",
    input.blocks_count);
#[no_mangle]
pub unsafe extern "C" fn if(ext4_sb_bread(sb: IS_ERR(bh =, 1: end -, _arg: 0))) -> else {
    err = PTR_ERR(bh);
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: outside(input->inode_bitmap, _arg: start, _arg: end)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(input->block_bitmap: input->inode_bitmap ==) -> else {
    else if (input.inode_bitmap == input.block_bitmap)
    ext4_warning(sb, "Block bitmap same as inode bitmap (%llu)",
    (unsigned long long)input.block_bitmap);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: inside(input->block_bitmap, _arg: input->inode_table, _arg: itend)) -> else {
    ...
    (unsigned long long)input.inode_table, itend - 1);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: inside(input->inode_bitmap, _arg: input->inode_table, _arg: itend)) -> else {
    ...
    (unsigned long long)input.inode_table, itend - 1);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: inside(input->block_bitmap, _arg: start, _arg: metaend)) -> else {
    else if (inside(input.block_bitmap, start, metaend))
    ext4_warning(sb, "Block bitmap (%llu) in GDT table (%llu-%llu)",
    (unsigned long long)input.block_bitmap,
    start, metaend - 1);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: inside(input->inode_bitmap, _arg: start, _arg: metaend)) -> else {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ext4_new_flex_group_data {
    pub groups: *mut *mut *mut ext4_new_group_data groups; / new_group_data for,
    in the flex group */
    pub groups: *mut *mut *mut __u16 bg_flags; / block group flags of,
    in @groups */
    pub allocated: *mut *mut ext4_group_t resize_bg; / number of,
    new_group_data */
    pub @groups: *mut *mut ext4_group_t count; / number of groups in,
//
}
...
//
pub const MAX_RESIZE_BG: c_int = 16384;
//
...
#[no_mangle]
pub unsafe extern "C" fn if(flex_gd->resize_bg: n_group - last_group <) -> else {
    ...
    if (flex_gd.groups == core::ptr::null_mut())
    goto out2;
    flex_gd.bg_flags = kmalloc_array(flex_gd.resize_bg, sizeof(__u16),
    GFP_NOFS);
    if (flex_gd.bg_flags == core::ptr::null_mut())
    goto out1;
    return flex_gd;
    out1:
...
#[no_mangle]
unsafe extern "C" fn free_flex_gd(flex_gd: *mut ext4_new_flex_group_data) {
...
//
    ...
    let mut bb_index: ext4_group_t = 0;
    let mut ib_index: ext4_group_t = 0;
    let mut it_index: ext4_group_t = 0;
    ext4_group_t group;
    ext4_group_t last_group;
    unsigned overhead;
    let mut uninit_mask: __u16 = (flexbg_size > 1) ? ~EXT4_BG_BLOCK_UNINIT : ~0;
    int i;
    BUG_ON(flex_gd.count == 0 || group_data == core::ptr::null_mut());
...
// Allocate inode tables
    ...
    let mut itb: c_uint = EXT4_SB(sb).s_itb_per_group;
...
#[no_mangle]
unsafe extern "C" fn ext4_resize_ensure_credits_batch(handle: *mut handle_t, credits: c_int) -> c_int {
...
//
    ...
    {
    struct ext4_sb_info *sbi = EXT4_SB(sb);
    let mut count: ext4_group_t = last_cluster - first_cluster + 1;
    ext4_group_t count2;
...
// Copy all of the GDT blocks into the backup in this group
    ...
    err = ext4_handle_dirty_metadata(handle, core::ptr::null_mut(), gdb);
    if (unlikely(err)) {
    brelse(gdb);
    goto out;
    }
...
//
    ...
    {
    let mut blk: ext4_fsblk_t = primary.b_blocknr;
    let mut three: unsigned = 1;
    let mut five: unsigned = 5;
    let mut seven: unsigned = 7;
    unsigned grp;
    __le32 *p = (__le32 *)primary.b_data;
    let mut gdbackups: c_int = 0;
    while ((grp = ext4_list_backups(sb, &three, &five, &seven)) < end) {
    if (le32_to_cpu(*p++) !=
#[no_mangle]
pub unsafe extern "C" fn EXT4_BLOCKS_PER_GROUP(blk: sb) +) -> *mut grp {
...
//
    ...
    struct ext4_super_block *es = EXT4_SB(sb).s_es;
    let mut gdb_num: c_ulong = group / EXT4_DESC_PER_BLOCK(sb);
    let mut gdblock: ext4_fsblk_t = EXT4_SB(sb).s_sbh.b_blocknr + 1 + gdb_num;
    struct buffer_head **o_group_desc, **n_group_desc = core::ptr::null_mut();
    struct buffer_head *dind = core::ptr::null_mut();
    struct buffer_head *gdb_bh = core::ptr::null_mut();
    int gdbackups;
...
//
    ...
    err = ext4_handle_dirty_metadata(handle, core::ptr::null_mut(), dind);
    if (unlikely(err)) {
    ext4_std_error(sb, err);
    goto errout;
    }
    ...
    err = ext4_handle_dirty_metadata(handle, core::ptr::null_mut(), gdb_bh);
    if (unlikely(err)) {
    ext4_std_error(sb, err);
    ...
    err = ext4_handle_dirty_metadata(handle, core::ptr::null_mut(), EXT4_SB(sb).s_sbh);
    if (err)
    ext4_std_error(sb, err);
    return err;
    errout:
...
//
    ...
    {
    struct super_block *sb = inode.i_sb;
    let mut reserved_gdb: c_int = le16_to_cpu(EXT4_SB(sb).s_es.s_reserved_gdt_blocks);
    let mut cluster_bits: c_int = EXT4_SB(sb).s_cluster_bits;
    struct buffer_head **primary;
...
//
    ...
    err2 = ext4_handle_dirty_metadata(handle, core::ptr::null_mut(), primary[i]);
    if (!err)
    err = err2;
    }
...
//
    ...
    struct ext4_sb_info *sbi = EXT4_SB(sb);
    ext4_group_t last;
    let mut bpg: c_int = EXT4_BLOCKS_PER_GROUP(sb);
    let mut three: unsigned = 1;
    let mut five: unsigned = 5;
    let mut seven: unsigned = 7;
    let mut group: ext4_group_t = 0;
    let mut rest: c_int = sb.s_blocksize - size;
    handle_t *handle;
    ...
    let mut has_super: c_int = ext4_bg_has_super(sb, group);
    let mut first_block: ext4_fsblk_t = ext4_group_first_block_no(sb, group);
// Out of journal space, and can't get more - abort - so sad
    ...
    group = ext4_list_backups(sb, &three, &five, &seven);
#[no_mangle]
pub unsafe extern "C" fn if(last: group ==) -> else {
    else if (group == last)
    break;
    else
    group = last;
...
//
    ...
    if (ext4_read_bh(bh, 0, core::ptr::null_mut(), false) < 0) {
    brelse(bh);
    return core::ptr::null_mut();
    }
    }
...
//
    ...
    struct ext4_new_flex_group_data *flex_gd)
    {
    let mut blocks_count: ext4_fsblk_t = 0;
    let mut free_blocks: ext4_fsblk_t = 0;
    let mut reserved_blocks: ext4_fsblk_t = 0;
    struct ext4_new_group_data *group_data = flex_gd.groups;
    struct ext4_sb_info *sbi = EXT4_SB(sb);
    struct ext4_super_block *es = sbi.s_es;
    int i;
...
//
    ...
    let mut err: c_int = 0, err2 = 0, credit;
...
// GDT blocks
    ...
    err = err2;
    if (!err) {
    let mut gdb_num: c_int = group / EXT4_DESC_PER_BLOCK(sb);
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_group_add(sb: *mut super_block, input: *mut ext4_new_group_data) -> c_int {
...
// We add the blocks to the bitmap and set the group need init bit
    ...
    if (err)
    goto errout;
    ext4_handle_dirty_metadata(handle, core::ptr::null_mut(), EXT4_SB(sb).s_sbh);
...
#[no_mangle]
unsafe extern "C" fn num_desc_blocks(sb: *mut super_block, groups: ext4_group_t) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ext4_convert_meta_bg(sb: *mut super_block, inode: *mut inode) -> c_int {
...
// Do a quick sanity check of the resize inode
    ...
    ext4_free_blocks(handle, inode, core::ptr::null_mut(), nr, 1,
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_resize_fs(sb: *mut super_block, n_blocks_count: ext4_fsblk_t) -> c_int {
    int ext4_resize_fs(struct super_block *sb, ext4_fsblk_t n_blocks_count)
    {
    struct ext4_new_flex_group_data *flex_gd = core::ptr::null_mut();
    struct ext4_sb_info *sbi = EXT4_SB(sb);
    struct ext4_super_block *es = sbi.s_es;
    struct buffer_head *bh;
    struct inode *resize_inode = core::ptr::null_mut();
    ...
    ext4_group_t n_group;
    ext4_fsblk_t o_blocks_count;
    let mut n_blocks_count_retry: ext4_fsblk_t = 0;
    let mut last_update_time: c_ulong = 0;
    let mut err: c_int = 0;
    int meta_bg;
    let mut flexbg_size: c_uint = ext4_flex_bg_size(sbi);
// See if the device is actually as big as what was requested
...
// Nothing need to do
    ...
    if (resize_inode) {
    iput(resize_inode);
    resize_inode = core::ptr::null_mut();
    }
...
//
    ...
    if (flex_gd)
    free_flex_gd(flex_gd);
    if (resize_inode != core::ptr::null_mut())
    iput(resize_inode);
    if (err)
