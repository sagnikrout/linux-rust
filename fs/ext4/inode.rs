...
//
    ...
    let mut offset: c_int = offsetof(struct ext4_inode, i_checksum_lo);
    let mut csum_size: c_uint = sizeof(dummy_csum);
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_inode_is_fast_symlink(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_evict_inode(inode: *mut inode) {
...
//
    ...
    ext4_fc_mark_ineligible(inode.i_sb, EXT4_FC_REASON_NOMEM, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn ext4_check_map_extents_env(inode: *mut inode) {
...
//
    ...
    unsigned int status;
    int retval;
    let mut orig_mlen: c_uint = map.m_len;
...
// Lookup extent status tree firstly
    if (ext4_es_lookup_extent(inode, map.m_lblk, core::ptr::null_mut(), &es, &map.m_seq)) {
...
//
    ...
    let mut start_byte: loff_t = EXT4_LBLK_TO_B(inode, map.m_lblk);
    let mut length: loff_t = EXT4_LBLK_TO_B(inode, map.m_len);
    if (flags & EXT4_GET_BLOCKS_IO_SUBMIT)
...
//
#[no_mangle]
unsafe extern "C" fn ext4_update_bh_state(bh: *mut buffer_head, flags: c_ulong) {
...
pub const DIO_MAX_BLOCKS: c_int = 4096;
//
...
//
    ...
    let mut create: c_int = map_flags & EXT4_GET_BLOCKS_CREATE;
    let mut nowait: bool = map_flags & EXT4_GET_BLOCKS_CACHED_NOWAIT;
    int err;
    ASSERT((EXT4_SB(inode.i_sb).s_mount_state & EXT4_FC_REPLAY)
    || handle != core::ptr::null_mut() || create == 0);
    ASSERT(create == 0 || !nowait);
    map.m_lblk = block;
    map.m_len = 1;
    err = ext4_map_blocks(handle, inode, &map, map_flags);
    if (err == 0)
    return create ? ERR_PTR(-ENOSPC) : core::ptr::null_mut();
    if (err < 0)
    return ERR_PTR(err);
    if (nowait)
...
// Read a contiguous batch of blocks.
    ...
    bhs[i] = ext4_getblk(core::ptr::null_mut(), inode, block + i, 0 /* map_flags */);
    if (IS_ERR(bhs[i])) {
    err = PTR_ERR(bhs[i]);
    bh_count = i;
    goto out_brelse;
...
// Note that NULL bhs[i] is valid because of holes.
    ...
    let mut blocksize: unsigned = head.b_size;
    int err, ret = 0;
...
//
#[no_mangle]
unsafe extern "C" fn ext4_dirty_journalled_data(handle: *mut handle_t, bh: *mut buffer_head) -> c_int {
...
// only regular files have a_ops
    ...
    return ext4_handle_dirty_metadata(handle, core::ptr::null_mut(), bh);
    }
    ...
    {
    let mut from: c_uint = offset_in_folio(folio, pos);
    let mut to: unsigned = from + len;
    struct inode *inode = folio.mapping.host;
    unsigned block_start, block_end;
    sector_t block;
    let mut err: c_int = 0;
    let mut blocksize: c_uint = i_blocksize(inode);
    struct buffer_head *bh, *head, *wait[2];
    let mut nr_wait: c_int = 0;
    int i;
    let mut should_journal_data: bool = ext4_should_journal_data(inode);
    let mut folio_uptodate: bool = folio_test_uptodate(folio);
...
// In case writeback began while the folio was unlocked
    ...
    core::ptr::null_mut(), do_journal_get_write_access);
    }
    if (ret) {
...
//
    ...
    struct inode *inode = mapping.host;
    let mut old_size: loff_t = inode.i_size;
    let mut ret: c_int = 0, ret2;
    let mut i_size_changed: c_int = 0;
    let mut verity: bool = ext4_verity_in_progress(inode);
...
//
    ...
    unsigned from, unsigned to)
    {
    let mut block_start: c_uint = 0, block_end;
    ...
    let mut old_size: loff_t = inode.i_size;
    let mut ret: c_int = 0, ret2;
    let mut partial: c_int = 0;
    unsigned from, to;
    let mut size_changed: c_int = 0;
    let mut verity: bool = ext4_verity_in_progress(inode);
...
//
#[no_mangle]
unsafe extern "C" fn ext4_da_reserve_space(inode: *mut inode, nr_resv: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_da_release_space(inode: *mut inode, to_free: c_int) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct mpage_da_data {
// These are input fields for ext4_do_writepages()
    pub inode: *mut inode,
    pub wbc: *mut writeback_control,
    pub /: *mut *mut unsigned int can_map:1; / Can writepages call map blocks?,
// These are internal state of ext4_do_writepages()
    pub /: *mut *mut loff_t start_pos; / The start pos to write,
    pub /: *mut *mut loff_t next_pos; / Current pos to examine,
    pub /: *mut *mut loff_t end_pos; / Last pos to examine,
//
// Extent to map - this can be after start_pos because that can be
// fully mapped. We somewhat abuse m_flags to store whether the extent
// is delalloc or unwritten.
//
    pub map: ext4_map_blocks,
    pub /: *mut *mut ext4_io_submit io_submit; / IO submission data,
    pub do_map:1: c_uint,
    pub scanned_until_end:1: c_uint,
    pub journalled_more_data:1: c_uint,
}
...
#[no_mangle]
unsafe extern "C" fn ext4_print_free_blocks(inode: *mut inode) {
...
//
#[no_mangle]
unsafe extern "C" fn ext4_clu_alloc_state(inode: *mut inode, lblk: ext4_lblk_t) -> c_int {
...
//
    ...
    struct ext4_sb_info *sbi = EXT4_SB(inode.i_sb);
    int ret;
    let mut lclu_allocated: bool = false;
    let mut end_allocated: bool = false;
    ext4_lblk_t resv_clu;
    let mut end: ext4_lblk_t = lblk + len - 1;
//
...
//
#[no_mangle]
unsafe extern "C" fn ext4_da_map_blocks(inode: *mut inode, map: *mut ext4_map_blocks) -> c_int {
    ...
    ext4_check_map_extents_env(inode);
// Lookup extent status tree firstly
    if (ext4_es_lookup_extent(inode, map.m_lblk, core::ptr::null_mut(), &es, core::ptr::null_mut())) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ext4_es_is_unwritten(&es)) -> else {
    else if (ext4_es_is_unwritten(&es))
    map.m_flags |= EXT4_MAP_UNWRITTEN;
    else
    BUG();

    ext4_map_blocks_es_recheck(core::ptr::null_mut(), inode, map, &orig_map, 0);
...
//
    ...
    retval = ext4_map_query_blocks(core::ptr::null_mut(), inode, map, 0);
    up_read(&EXT4_I(inode).i_data_sem);
    if (retval)
...
//
    ...
    {
    struct ext4_map_blocks map;
    let mut invalid_block: sector_t = ~((sector_t) 0xffff);
    let mut ret: c_int = 0;
    BUG_ON(create == 0);
...
#[no_mangle]
unsafe extern "C" fn mpage_folio_done(mpd: *mut mpage_da_data, folio: *mut folio) {
...
#[no_mangle]
unsafe extern "C" fn mpage_submit_folio(mpd: *mut mpage_da_data, folio: *mut folio) {
...
//
pub const MAX_WRITEPAGES_EXTENT_LEN: c_int = 2048;
//
...
//
    ...
    let mut lblk: ext4_lblk_t = *m_lblk;
    let mut pblock: ext4_fsblk_t = *m_pblk;
    let mut err: c_int = 0;
    let mut io_end_size: isize = 0;
...
//
#[no_mangle]
unsafe extern "C" fn mpage_map_and_submit_buffers(mpd: *mut mpage_da_data) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mpage_map_one_extent(handle: *mut handle_t, mpd: *mut mpage_da_data) -> c_int {
...
//
    ...
    handle.h_rsv_handle = core::ptr::null_mut();
    }
...
//
#[no_mangle]
unsafe extern "C" fn mpage_submit_partial_folio(mpd: *mut mpage_da_data) -> c_int {
...
//
    ...
    core::ptr::null_mut(), write_end_fn);
    if (ret == 0)
    ret = err;
    ...
    let mut len: usize = folio_size(folio);
...
//
#[no_mangle]
unsafe extern "C" fn mpage_prepare_extent_to_map(mpd: *mut mpage_da_data) -> c_int {
    static int mpage_prepare_extent_to_map(struct mpage_da_data *mpd)
    {
    struct address_space *mapping = mpd.inode.i_mapping;
    struct folio_batch fbatch;
    unsigned int nr_folios;
    let mut index: pgoff_t = mpd.start_pos >> PAGE_SHIFT;
    let mut end: pgoff_t = mpd.end_pos >> PAGE_SHIFT;
    xa_mark_t tag;
    int i, err = 0;
    ext4_lblk_t lblk;
    struct buffer_head *head;
    handle_t *handle = core::ptr::null_mut();
    let mut bpp: c_int = ext4_journal_blocks_per_folio(mpd.inode);
...
#[no_mangle]
unsafe extern "C" fn ext4_do_writepages(mpd: *mut mpage_da_data) -> c_int {
    static int ext4_do_writepages(struct mpage_da_data *mpd)
    {
    struct writeback_control *wbc = mpd.wbc;
    let mut writeback_index: pgoff_t = 0;
    let mut nr_to_write: c_long = wbc.nr_to_write;
    let mut range_whole: c_int = 0;
    let mut cycled: c_int = 1;
    handle_t *handle = core::ptr::null_mut();
    struct inode *inode = mpd.inode;
    struct address_space *mapping = inode.i_mapping;
    ...
    let mut give_up_on_write: bool = false;
...
//
    ...
    if (ext4_should_dioread_nolock(inode)) {
    let mut bpf: c_int = ext4_journal_blocks_per_folio(inode);
//
...
// Submit prepared bio
    ...
    mpd.io_submit.io_end = core::ptr::null_mut();
    if (ret < 0)
    goto unplug;
...
#[no_mangle]
pub unsafe extern "C" fn ext4_normal_submit_inode_data_buffers(jinode: *mut jbd2_inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext4_nonda_switch(sb: *mut super_block) -> c_int {
...
// fsdata = (void *)((unsigned long)*fsdata | EXT4_WRITE_DATA_INLINE);
    ...
    if (len > folio_next_pos(folio) - pos)
    len = folio_next_pos(folio) - pos;
    ret = ext4_block_write_begin(core::ptr::null_mut(), folio, pos, len,
...
//
    ...
    struct inode *inode = mapping.host;
    let mut old_size: loff_t = inode.i_size;
    let mut disksize_changed: bool = false;
    loff_t new_i_size;
    handle_t *handle;
...
//
    ...
    {
    struct inode *inode = mapping.host;
    let mut write_mode: c_ulong = (unsigned long)fsdata;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_alloc_da_blocks(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ext4_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
    static sector_t ext4_bmap(struct address_space *mapping, sector_t block)
    {
    struct inode *inode = mapping.host;
    let mut ret: sector_t = 0;
...
#[no_mangle]
unsafe extern "C" fn ext4_release_folio(folio: *mut folio, wait: gfp_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ext4_inode_datasync_dirty(inode: *mut inode) -> bool {
...
// Any metadata buffers to write?
    ...
    {
    let mut blkbits: u8 = inode.i_blkbits;
//
...
//
    ...
    struct inode *inode, struct ext4_map_blocks *map)
    {
    let mut m_lblk: ext4_lblk_t = map.m_lblk;
    let mut m_len: c_uint = map.m_len;
    let mut mapped_len: c_uint = 0, m_flags = 0;
    let mut next_pblk: ext4_fsblk_t = 0;
    let mut check_next_pblk: bool = false;
    let mut ret: c_int = 0;
...
// force_commit = true;
    ...
    let mut force_commit: bool = false;
...
//
    if (flags & IOMAP_ATOMIC) {
    let mut orig_mlen: c_uint = map.m_len;
    ret = ext4_map_blocks(core::ptr::null_mut(), inode, map, 0);
    if (ret < 0)
    return ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn if(_arg: EXT4_LBLK_TO_B(inode, i_size_read(inode): map->m_lblk) >=) -> else {
    else if (EXT4_LBLK_TO_B(inode, map.m_lblk) >= i_size_read(inode))
    m_flags = EXT4_GET_BLOCKS_CREATE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ext4_test_inode_flag(inode, _arg: EXT4_INODE_EXTENTS)) -> else {
...
#[no_mangle]
unsafe extern "C" fn ext4_dirty_folio(mapping: *mut address_space, folio: *mut folio) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_set_aops(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_opt(inode->i_sb, _arg: DELALLOC)) -> else {
...
// Ok, it's mapped. Make sure it's up-to-date
    ...
    folio_unlock(folio);
    folio_put(folio);
    return err ? ERR_PTR(err) : core::ptr::null_mut();
    }
...
//
    ...
    bool *zero_written)
    {
    let mut blocksize: unsigned = inode.i_sb.s_blocksize;
    let mut max: c_uint = blocksize - (from & (blocksize - 1));
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_block_zero_eof(inode: *mut inode, from: loff_t, end: loff_t) -> c_int {
    int ext4_block_zero_eof(struct inode *inode, loff_t from, loff_t end)
    {
    let mut blocksize: c_uint = i_blocksize(inode);
    unsigned int offset;
    let mut length: loff_t = end - from;
    let mut did_zero: bool = false;
    let mut zero_written: bool = false;
    int err;
...
//
    ...
    let mut byte_end: loff_t = (lstart + length - 1);
    let mut did_zero: bool = false;
    let mut err: c_int = 0;
...
// Handle partial zero out on the end of the range
    ...
    partial_end + 1, &did_zero, core::ptr::null_mut());
    if (err)
    return err;
    if (did_zero)
...
#[no_mangle]
pub unsafe extern "C" fn ext4_can_truncate(inode: *mut inode) -> c_int {
...
//
    ...
    {
    let mut blocksize: c_ulong = i_blocksize(inode);
    struct folio *folio;
...
//
    ...
    let mut page_boundary: loff_t = round_up(start, PAGE_SIZE);
...
#[no_mangle]
unsafe extern "C" fn ext4_wait_dax_page(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_break_layouts(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_punch_hole(file: *mut file, offset: loff_t, length: loff_t) -> c_int {
    int ext4_punch_hole(struct file *file, loff_t offset, loff_t length)
    {
    struct inode *inode = file_inode(file);
    struct super_block *sb = inode.i_sb;
    ext4_lblk_t start_lblk, end_lblk;
    let mut max_end: loff_t = sb.s_maxbytes;
    let mut end: loff_t = offset + length;
    handle_t *handle;
    unsigned int credits;
    let mut partial_zeroed: c_uint = 0;
    int ret;
...
// If there are blocks to remove, do it
    ...
    let mut hole_len: ext4_lblk_t = end_lblk - start_lblk;
    ext4_fc_track_inode(handle, inode);
    ext4_check_map_extents_env(inode);
    down_write(&EXT4_I(inode).i_data_sem);
    ext4_discard_preallocations(inode);
...
#[no_mangle]
pub unsafe extern "C" fn ext4_inode_attach_jinode(inode: *mut inode) -> c_int {
...
//
    ...
    jinode = core::ptr::null_mut();
    }
    spin_unlock(&inode.i_lock);
    if (unlikely(jinode != core::ptr::null_mut()))
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_truncate(inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_inode_peek_iversion(inode: *const inode) -> u64 {
    ...
    let mut i_blocks: u64 = READ_ONCE(inode.i_blocks);
    struct super_block *sb = inode.i_sb;
    if (i_blocks <= ~0U) {
//
...
#[no_mangle]
unsafe extern "C" fn ext4_fill_raw_inode(inode: *mut inode, raw_inode: *mut ext4_inode) -> c_int {
...
//
    ...
    if (likely(!test_opt2(inode.i_sb, HURD_COMPAT))) {
    let mut ivers: u64 = ext4_inode_peek_iversion(inode);
...
//
    ...
    iloc.bh = core::ptr::null_mut();
    if (ino < EXT4_ROOT_INO ||
    ino > le32_to_cpu(EXT4_SB(sb).s_es.s_inodes_count))
    return -EFSCORRUPTED;
    iloc.block_group = (ino - 1) / EXT4_INODES_PER_GROUP(sb);
    gdp = ext4_get_group_desc(sb, iloc.block_group, core::ptr::null_mut());
    if (!gdp)
    return -EIO;
//
...
//
    ...
    unsigned num;
    let mut ra_blks: __u32 = EXT4_SB(sb).s_inode_readahead_blks;
...
//
    ...
    ext4_read_bh_nowait(bh, REQ_META | REQ_PRIO, core::ptr::null_mut(),
...
// ret_block = block;
    ...
    {
    let mut err_blk: ext4_fsblk_t = 0;
    int ret;
    ret = __ext4_get_inode_loc(inode.i_sb, inode.i_ino, core::ptr::null_mut(), iloc,
    &err_blk);
    if (ret == -EIO)
...
#[no_mangle]
pub unsafe extern "C" fn ext4_get_inode_loc(inode: *mut inode, iloc: *mut ext4_iloc) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_get_inode_loc_noio(inode: *mut inode, iloc: *mut ext4_iloc) -> c_int {
    ...
    ext4_fsblk_t block;
    int inodes_per_block, inode_offset;
    let mut ino: c_ulong = inode.i_ino;
    iloc.bh = core::ptr::null_mut();
    if (ino < EXT4_ROOT_INO ||
    ino > le32_to_cpu(EXT4_SB(sb).s_es.s_inodes_count))
    return -EFSCORRUPTED;
...
// Figure out the offset within the block group inode table.
    ...
    struct ext4_iloc *iloc)
    {
    return __ext4_get_inode_loc(sb, ino, core::ptr::null_mut(), iloc, core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn ext4_should_enable_dax(inode: *mut inode) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_set_inode_flags(inode: *mut inode, init: bool) {
    void ext4_set_inode_flags(struct inode *inode, bool init)
    {
    let mut flags: c_uint = EXT4_I(inode).i_flags;
    let mut new_fl: c_uint = 0;
...
#[no_mangle]
pub unsafe extern "C" fn ext4_get_projid(inode: *mut inode, projid: *mut kprojid_t) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_inode_set_iversion_queried(inode: *mut inode, val: u64) {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_set_inode_mapping_order(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn ext4_iget_match(inode: *mut inode, ino: u64, data: *mut c_void) -> c_int {
    ...
    inode = find_inode_nowait(sb, ino, ext4_iget_match, core::ptr::null_mut());
    if (!inode)
...
// Precompute checksum seed for inode metadata
    ...
    struct ext4_sb_info *sbi = EXT4_SB(inode.i_sb);
    __u32 csum;
    let mut inum: __le32 = cpu_to_le32(inode.i_ino);
    let mut gen: __le32 = raw_inode.i_generation;
...
// The extra space is currently unused. Use it.
    ...
    if (likely(!test_opt2(inode.i_sb, HURD_COMPAT))) {
    let mut ivers: u64 = le32_to_cpu(raw_inode.i_disk_version);
    if (EXT4_INODE_SIZE(inode.i_sb) > EXT4_GOOD_OLD_INODE_SIZE) {
    if (EXT4_FITS_IN_INODE(raw_inode, ei, i_version_hi))
    ivers |=
...
//
    ...
    let mut inode_size: c_int = EXT4_INODE_SIZE(sb);
//
...
//
    ...
    struct super_block *sb = inode.i_sb;
    int err;
    let mut need_datasync: c_int = 0, set_large_file = 0;
    spin_lock(&ei.i_raw_lock);
//
...
//
    ...
    ext4_handle_sync(handle);
    err = ext4_handle_dirty_metadata(handle, core::ptr::null_mut(),
    EXT4_SB(sb).s_sbh);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_write_inode(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_sync_inode_metadata(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ext4_wait_for_tail_page_commit(inode: *mut inode) {
...
//
    ...
    int error, rc = 0;
    let mut orphan: c_int = 0;
    let mut ia_valid: c_uint = attr.ia_valid;
    let mut inc_ivers: bool = true;
...
// one transaction
    ...
    handle_t *handle;
    let mut oldsize: loff_t = inode.i_size;
    loff_t old_disksize;
    let mut shrink: c_int = (attr.ia_size < inode.i_size);
    if (!(ext4_test_inode_flag(inode, EXT4_INODE_EXTENTS))) {
    struct ext4_sb_info *sbi = EXT4_SB(inode.i_sb);
...
#[no_mangle]
pub unsafe extern "C" fn ext4_dio_alignment(inode: *mut inode) -> u32 {
...
// Return the DIO alignment restrictions if requested.
    ...
    let mut dio_align: u32 = ext4_dio_alignment(inode);
...
// iomap defaults
    ...
    struct ext4_sb_info *sbi = EXT4_SB(inode.i_sb);
    let mut awu_min: c_uint = 0, awu_max = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_chunk_trans_extent(inode: *mut inode, nrblocks: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_chunk_trans_blocks(inode: *mut inode, nrblocks: c_int) -> c_int {
...
//
    spin_lock(&inode.i_lock);
    inode_attach_wb(inode, core::ptr::null_mut());
    spin_unlock(&inode.i_lock);
...
//
    ...
    let mut inode_size: c_uint = EXT4_INODE_SIZE(inode.i_sb);
    struct ext4_inode_info *ei = EXT4_I(inode);
    int error;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_dirty_inode(inode: *mut inode, flags: c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_change_inode_journal_flag(inode: *mut inode, val: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_page_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
...
//
    if (folio_buffers(folio)) {
    if (!ext4_walk_page_buffers(core::ptr::null_mut(), inode, folio_buffers(folio),
    0, len, core::ptr::null_mut(),
    ext4_bh_unmapped)) {
