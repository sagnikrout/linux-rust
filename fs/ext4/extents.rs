...
//
pub const EXT4_EXT_MAY_ZEROOUT: c_uint = 0x1  /* safe to zeroout if split fails \;
    due to ENOSPC */
...
#[no_mangle]
unsafe extern "C" fn ext4_ext_trunc_restart_fn(inode: *mut inode, dropped: *mut c_int) -> c_int {
...
//
    BUG_ON(EXT4_JOURNAL(inode) == core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_path_brelse(path: *mut ext4_ext_path) {
    static inline void ext4_ext_path_brelse(struct ext4_ext_path *path)
    {
    brelse(path.p_bh);
    path.p_bh = core::ptr::null_mut();
    }
#[no_mangle]
unsafe extern "C" fn ext4_ext_drop_refs(path: *mut ext4_ext_path) {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_free_ext_path(path: *mut ext4_ext_path) {
...
// path points to leaf/index in inode body
    ...
    {
    if (path) {
    let mut depth: c_int = path.p_depth;
...
//
    ex = path[depth].p_ext;
    if (ex) {
    let mut ext_pblk: ext4_fsblk_t = ext4_ext_pblock(ex);
    let mut ext_block: ext4_lblk_t = le32_to_cpu(ex.ee_block);
    if (block > ext_block)
...
//
    ...
    core::ptr::null_mut(), err);
    return newblock;
    }
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_space_block(inode: *mut inode, check: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_space_block_idx(inode: *mut inode, check: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_space_root(inode: *mut inode, check: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_space_root_idx(inode: *mut inode, check: c_int) -> c_int {
    ...
    int nofail)
    {
    let mut flags: c_int = EXT4_EX_NOCACHE | EXT4_GET_BLOCKS_SPLIT_NOMERGE;
    if (nofail)
...
#[no_mangle]
unsafe extern "C" fn ext4_valid_extent(inode: *mut inode, ext: *mut ext4_extent) -> c_int {
    static int ext4_valid_extent(struct inode *inode, struct ext4_extent *ext)
    {
    let mut block: ext4_fsblk_t = ext4_ext_pblock(ext);
    let mut len: c_int = ext4_ext_get_actual_len(ext);
    let mut lblock: ext4_lblk_t = le32_to_cpu(ext.ee_block);
//
// We allow neither:
// - zero length
...
//
    ...
    {
    let mut block: ext4_fsblk_t = ext4_idx_pblock(ext_idx);
    ...
    let mut lblock: ext4_lblk_t = 0;
    let mut cur: ext4_lblk_t = 0;
    if (eh.eh_entries == 0)
    return 1;
...
// pblk = ext4_idx_pblock(ext_idx);
    ...
    let mut max: c_int = 0, err = -EFSCORRUPTED;
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_check_inode(inode: *mut inode) -> c_int {
    ...
    let mut prev: ext4_lblk_t = 0;
    int i;
    KUNIT_STATIC_STUB_REDIRECT(ext4_cache_extents, inode, eh);
    for (i = le16_to_cpu(eh.eh_entries); i > 0; i--, ex++) {
    let mut status: c_uint = EXTENT_STATUS_WRITTEN;
    let mut lblk: ext4_lblk_t = le32_to_cpu(ex.ee_block);
    let mut len: c_int = ext4_ext_get_actual_len(ex);
    if (prev && (prev != lblk))
    ...
    int				err;
    let mut gfp_flags: gfp_t = __GFP_MOVABLE | GFP_NOFS;
    ext4_fsblk_t			pblk;
    ...
    err = ext4_read_bh(bh, 0, core::ptr::null_mut(), false);
    if (err < 0)
    goto errout;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_precache(inode: *mut inode) -> c_int {
    int ext4_ext_precache(struct inode *inode)
    {
    struct ext4_inode_info *ei = EXT4_I(inode);
    struct ext4_ext_path *path = core::ptr::null_mut();
    struct buffer_head *bh;
    let mut i: c_int = 0, depth, ret = 0;
...
#[no_mangle]
unsafe extern "C" fn ext4_ext_show_path(inode: *mut inode, path: *mut ext4_ext_path) {
...
#[no_mangle]
unsafe extern "C" fn ext4_ext_show_leaf(inode: *mut inode, path: *mut ext4_ext_path) {
    static void ext4_ext_show_leaf(struct inode *inode, struct ext4_ext_path *path)
    {
    let mut depth: c_int = ext_depth(inode);
    struct ext4_extent_header *eh;
    struct ext4_extent *ex;
    int i;
    if (IS_ERR_OR_NULL(path))
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_tree_init(handle: *mut handle_t, inode: *mut inode) {
    ...
    int ret;
    let mut gfp_flags: gfp_t = GFP_NOFS;
...
// walk through the tree
    ...
    path[ppos].p_depth = i;
    path[ppos].p_ext = core::ptr::null_mut();
    ...
    path[ppos].p_depth = i;
    path[ppos].p_ext = core::ptr::null_mut();
    path[ppos].p_idx = core::ptr::null_mut();
// find extent
...
//
    ...
    let mut i: c_int = at, k, m, a;
    ext4_fsblk_t newblock, oldblock;
    __le32 border;
    ext4_fsblk_t *ablocks = core::ptr::null_mut(); /* array of allocated blocks */
    let mut gfp_flags: gfp_t = GFP_NOFS;
    let mut err: c_int = 0;
    let mut ext_size: usize = 0;
    if (flags & EXT4_EX_NOFAIL)
    gfp_flags |= __GFP_NOFAIL;
...
// free all allocated blocks in error case
    ...
    if (!ablocks[i])
    continue;
    ext4_free_blocks(handle, inode, core::ptr::null_mut(), ablocks[i], 1,
...
//
    ...
    struct ext4_ext_path *curp;
    int depth, i, err = 0;
    let mut ee_block: ext4_lblk_t = le32_to_cpu(newext.ee_block);
    repeat:
...
//
    ...
    struct ext4_extent *ex;
    int depth, ee_len;
    if (unlikely(path == core::ptr::null_mut())) {
    EXT4_ERROR_INODE(inode, "path == core::ptr::null_mut() *logical %d!", *logical);
    return -EFSCORRUPTED;
    }
    depth = path.p_depth;
// phys = 0;
    if (depth == 0 && path.p_ext == core::ptr::null_mut())
    return 0;
...
// logical, le32_to_cpu(ex->ee_block));
    ...
    ix != core::ptr::null_mut() ? le32_to_cpu(ix.ei_block) : 0,
...
//
#[no_mangle]
unsafe extern "C" fn ext4_ext_next_leaf_block(path: *mut ext4_ext_path) -> ext4_lblk_t {
...
//
    ...
    eh = path[depth].p_hdr;
    ex = path[depth].p_ext;
    if (unlikely(ex == core::ptr::null_mut() || eh == core::ptr::null_mut())) {
    EXT4_ERROR_INODE(inode,
    "ex %p == core::ptr::null_mut() or eh %p == core::ptr::null_mut()", ex, eh);
    return -EFSCORRUPTED;
    }
    if (depth == 0) {
...
//
    ...
    let mut merge_done: c_int = 0, unwritten;
    depth = ext_depth(inode);
    BUG_ON(path[depth].p_hdr == core::ptr::null_mut());
    eh = path[depth].p_hdr;
...
//
    ...
    {
    size_t s;
    let mut max_root: unsigned = ext4_ext_space_root(inode, 0);
    ext4_fsblk_t blk;
    if ((path[0].p_depth != 1) ||
...
//
    ...
    ext4_free_blocks(handle, inode, core::ptr::null_mut(), blk, 1,
...
//
    ...
    int depth, len, err = 0;
    ext4_lblk_t next;
    let mut mb_flags: c_int = 0, unwritten;
    ...
    ex = path[depth].p_ext;
    eh = path[depth].p_hdr;
    if (unlikely(path[depth].p_hdr == core::ptr::null_mut())) {
    EXT4_ERROR_INODE(inode, "path[%d].p_hdr == core::ptr::null_mut()", depth);
    err = -EFSCORRUPTED;
    goto errout;
    }
...
// probably next leaf has space for us?
    ...
    npath = ext4_find_extent(inode, next, core::ptr::null_mut(), gb_flags);
    if (IS_ERR(npath)) {
    err = PTR_ERR(npath);
    goto errout;
    }
...
// time to correct all indexes above
    ...
    let mut blksize_bits: c_uchar = inode.i_sb.s_blocksize_bits;
    unsigned int flags;
    int err;
    while (block <= end) {
    next = 0;
    flags = 0;
    if (!ext4_es_lookup_extent(inode, block, &next, &es, core::ptr::null_mut()))
    break;
...
// free index block
    ...
    let mut len: c_int = EXT_LAST_INDEX(path[k].p_hdr) - path[k].p_idx;
    ...
    ext4_free_blocks(handle, inode, core::ptr::null_mut(), leaf, 1,
    EXT4_FREE_BLOCKS_METADATA | EXT4_FREE_BLOCKS_FORGET);
    while (--k >= 0) {
    if (path[k + 1].p_idx != EXT_FIRST_INDEX(path[k + 1].p_hdr))
    break;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_index_trans_blocks(inode: *mut inode, extents: c_int) -> c_int {
...
//
    ...
    let mut ext_max: c_int = ext4_ext_space_block(inode, 0);
...
#[no_mangle]
pub unsafe extern "C" fn get_default_free_blocks_flags(inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: ext4_should_journal_data(inode)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn ext4_rereserve_cluster(inode: *mut inode, lblk: ext4_lblk_t) {
    ...
    {
    struct ext4_sb_info *sbi = EXT4_SB(inode.i_sb);
    let mut ee_len: c_ushort = ext4_ext_get_actual_len(ex);
...
//
    ...
    ext4_free_blocks(handle, inode, core::ptr::null_mut(),
...
//
    ...
    ext4_free_blocks(handle, inode, core::ptr::null_mut(), pblk, num, flags);
...
//
    ...
    {
    struct ext4_sb_info *sbi = EXT4_SB(inode.i_sb);
    let mut err: c_int = 0, correct_index = 0;
    let mut depth: c_int = ext_depth(inode), credits, revoke_credits;
    struct ext4_extent_header *eh;
    ext4_lblk_t a, b;
    unsigned num;
    ext4_lblk_t ex_ee_block;
    unsigned short ex_ee_len;
    let mut unwritten: unsigned = 0;
    struct ext4_extent *ex;
    ext4_fsblk_t pblk;
...
//
    ...
    let mut flags: c_int = get_default_free_blocks_flags(inode);
    if (ext4_is_pending(inode, partial.lblk))
    flags |= EXT4_FREE_BLOCKS_RERESERVE_CLUSTER;
    ext4_free_blocks(handle, inode, core::ptr::null_mut(),
    EXT4_C2B(sbi, partial.pclu),
...
// remove it from index block above
    if (err == 0 && eh.eh_entries == 0 && path[depth].p_bh != core::ptr::null_mut())
...
//
    ...
    {
    BUG_ON(path.p_idx == core::ptr::null_mut());
...
//
    ...
    let mut flags: c_int = EXT4_EX_NOCACHE | EXT4_EX_NOFAIL;
    partial.pclu = 0;
    partial.lblk = 0;
    partial.state = initial;
...
// find extent for or closest extent to this block
    path = ext4_find_extent(inode, end, core::ptr::null_mut(), flags);
    if (IS_ERR(path)) {
...
// Leaf not may not exist only if inode has no blocks at all
    ...
    if (depth) {
    EXT4_ERROR_INODE(inode,
    "path[%d].p_hdr == core::ptr::null_mut()",
    depth);
    err = -EFSCORRUPTED;
    }
    goto out;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_init(sb: *mut super_block) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_release(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn ext4_zeroout_es(inode: *mut inode, ex: *mut ext4_extent) {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_zeroout(inode: *mut inode, ex: *mut ext4_extent) -> c_int {
...
//
    ...
    struct ext4_extent *ex2 = core::ptr::null_mut();
    unsigned int ee_len, depth;
    let mut err: c_int = 0, insert_err = 0, is_unwrit = 0;
...
//
    path = ext4_find_extent(inode, ee_block, core::ptr::null_mut(), flags | EXT4_EX_NOFAIL);
    if (IS_ERR(path)) {
...
// range.
//
    let mut map_end: loff_t = (loff_t) map.m_lblk + map.m_len;
    let mut ex_end: loff_t = (loff_t) ee_block + ee_len;
    if (!is_unwrit)
...
// we can't zeroout? just return the original err
    ...
    path = ext4_find_extent(inode, map.m_lblk, core::ptr::null_mut(), flags);
    if (IS_ERR(path))
    goto out_orig_err;
    depth = ext_depth(inode);
    ex = path[depth].p_ext;
...
// allocated = map->m_len;
    ...
    flags | EXT4_GET_BLOCKS_CONVERT, core::ptr::null_mut());
    if (IS_ERR(path))
    return path;
    out:
...
//
    ...
    struct ext4_extent *ex;
    unsigned int ee_len;
    let mut split_flag: c_int = 0, depth, err = 0;
    let mut did_zeroout: bool = false;
    ext_debug(inode, "logical block %llu, max_blocks %u\n",
...
//
    ...
    ext4_ext_mark_initialized(ex);
#[no_mangle]
pub unsafe extern "C" fn if(EXT4_GET_BLOCKS_CONVERT_UNWRITTEN: flags &) -> else {
    else if (flags & EXT4_GET_BLOCKS_CONVERT_UNWRITTEN)
    ext4_ext_mark_unwritten(ex);
...
//
    ...
    {
    struct ext4_sb_info *sbi = EXT4_SB(sb);
    let mut c_offset: ext4_lblk_t = EXT4_LBLK_COFF(sbi, map.m_lblk);
    ext4_lblk_t ex_cluster_start, ex_cluster_end;
    ext4_lblk_t rr_cluster_start;
    let mut ee_block: ext4_lblk_t = le32_to_cpu(ex.ee_block);
    let mut ee_start: ext4_fsblk_t = ext4_ext_pblock(ex);
    let mut ee_len: c_ushort = ext4_ext_get_actual_len(ex);
...
//
    if (map.m_lblk > ee_block) {
    let mut next: ext4_lblk_t = ext4_ext_next_allocated_block(path);
...
//
    ...
    struct ext4_sb_info *sbi = EXT4_SB(inode.i_sb);
    let mut newblock: ext4_fsblk_t = 0, pblk;
    let mut err: c_int = 0, depth;
    let mut allocated: c_uint = 0, offset = 0;
    let mut allocated_clusters: c_uint = 0;
...
//
    if (unlikely(path[depth].p_ext == core::ptr::null_mut() && depth != 0)) {
...
//
    ...
    ext4_free_blocks(handle, inode, core::ptr::null_mut(), newblock,
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_truncate(handle: *mut handle_t, inode: *mut inode) -> c_int {
...
//
    ...
    handle_t *handle;
    let mut ret: c_int = 0, ret2 = 0, ret3 = 0;
    let mut retries: c_int = 0;
    let mut depth: c_int = 0;
    ext4_lblk_t len_lblk;
    struct ext4_map_blocks map;
    unsigned int credits;
    let mut epos: loff_t = 0, old_size = i_size_read(inode);
    let mut blkbits: c_uint = inode.i_blkbits;
    let mut alloc_zero: bool = false;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_fallocate(file: *mut file, mode: c_int, offset: loff_t, len: loff_t) -> c_long {
...
//
    ...
    struct ext4_map_blocks map;
    let mut blkbits: c_uint = inode.i_blkbits;
    let mut credits: c_uint = 0;
    let mut flags: c_int = EXT4_GET_BLOCKS_IO_CONVERT_EXT | EXT4_EX_NOCACHE;
    map.m_lblk = offset >> blkbits;
...
#[no_mangle]
pub unsafe extern "C" fn ext4_convert_unwritten_io_end_vec(handle: *mut handle_t, io_end: *mut ext4_io_end_t) -> c_int {
...
//
    ...
    io_end_vec.offset,
    io_end_vec.size, core::ptr::null_mut());
    if (ret)
    break;
    }
    if (handle)
...
#[no_mangle]
unsafe extern "C" fn ext4_iomap_xattr_fiemap(inode: *mut inode, iomap: *mut iomap) -> c_int {
    static int ext4_iomap_xattr_fiemap(struct inode *inode, struct iomap *iomap)
    {
    let mut physical: __u64 = 0;
    let mut length: __u64 = 0;
    let mut blockbits: c_int = inode.i_sb.s_blocksize_bits;
...
#[no_mangle]
unsafe extern "C" fn ext4_fiemap_check_ranges(inode: *mut inode, start: u64, len: *mut u64) -> c_int {
    static int ext4_fiemap_check_ranges(struct inode *inode, u64 start, u64 *len)
    {
    let mut maxbytes: u64 = ext4_get_maxbytes(inode);
    if (*len == 0)
    return -EINVAL;
    if (start > maxbytes)
    return -EFBIG;
...
//
    ...
    {
    struct ext4_ext_path *path;
    let mut ret: c_int = 0, depth;
    struct ext4_extent *extent;
    ext4_lblk_t stop, *iterator, ex_start, ex_end;
    let mut tmp: ext4_lblk_t = EXT_MAX_BLOCKS;
// Let path point to the last extent
    path = ext4_find_extent(inode, EXT_MAX_BLOCKS - 1, core::ptr::null_mut(),
    EXT4_EX_NOCACHE);
    if (IS_ERR(path))
    return PTR_ERR(path);
    depth = path.p_depth;
...
#[no_mangle]
pub unsafe extern "C" fn if(start: le32_to_cpu(extent->ee_block) ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn ext4_collapse_range(file: *mut file, offset: loff_t, len: loff_t) -> c_int {
    static int ext4_collapse_range(struct file *file, loff_t offset, loff_t len)
    {
    struct inode *inode = file_inode(file);
    struct super_block *sb = inode.i_sb;
...
//
#[no_mangle]
unsafe extern "C" fn ext4_insert_range(file: *mut file, offset: loff_t, len: loff_t) -> c_int {
    static int ext4_insert_range(struct file *file, loff_t offset, loff_t len)
    {
    struct inode *inode = file_inode(file);
    struct super_block *sb = inode.i_sb;
...
// Expand file to avoid data loss if there is error while shifting
    ...
    down_write(&EXT4_I(inode).i_data_sem);
    ext4_discard_preallocations(inode);
    path = ext4_find_extent(inode, start_lblk, core::ptr::null_mut(), 0);
    if (IS_ERR(path)) {
    up_write(&EXT4_I(inode).i_data_sem);
    ret = PTR_ERR(path);
    goto out_handle;
...
//
    ...
    {
    struct ext4_ext_path *path1 = core::ptr::null_mut();
    struct ext4_ext_path *path2 = core::ptr::null_mut();
    let mut replaced_count: c_int = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext4_clu_mapped(inode: *mut inode, lclu: ext4_lblk_t) -> c_int {
...
// search for the extent closest to the first block in the cluster
    path = ext4_find_extent(inode, EXT4_C2B(sbi, lclu), core::ptr::null_mut(), 0);
    if (IS_ERR(path))
    return PTR_ERR(path);
    depth = ext_depth(inode);
//
...
//
    ...
    struct ext4_extent *ex;
    int ret;
    path = ext4_find_extent(inode, start, core::ptr::null_mut(), 0);
    if (IS_ERR(path))
    return PTR_ERR(path);
...
// We need to split this extent to match our extent first
    down_write(&EXT4_I(inode).i_data_sem);
    path = ext4_force_split_extent_at(core::ptr::null_mut(), inode, path, start, 1);
    up_write(&EXT4_I(inode).i_data_sem);
    if (IS_ERR(path)) {
    ret = PTR_ERR(path);
    goto out;
    ...
    down_write(&EXT4_I(inode).i_data_sem);
    path = ext4_force_split_extent_at(core::ptr::null_mut(), inode, path,
    start + len, 1);
    up_write(&EXT4_I(inode).i_data_sem);
    if (IS_ERR(path)) {
    ret = PTR_ERR(path);
    ...
    down_write(&EXT4_I(inode).i_data_sem);
    ret = ext4_ext_dirty(core::ptr::null_mut(), inode, &path[path.p_depth]);
    up_write(&EXT4_I(inode).i_data_sem);
    out:
    ext4_free_ext_path(path);
    ext4_mark_inode_dirty(core::ptr::null_mut(), inode);
    return ret;
    }
// Try to shrink the extent tree
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_replay_shrink_inode(inode: *mut inode, end: ext4_lblk_t) {
    ...
    while (cur < end) {
    path = ext4_find_extent(inode, cur, core::ptr::null_mut(), 0);
    if (IS_ERR(path))
    return;
    ex = path[path.p_depth].p_ext;
    if (!ex) {
    ...
    if (cur <= old_cur)
    cur = old_cur + 1;
    ext4_ext_try_to_merge(core::ptr::null_mut(), inode, path, ex);
    down_write(&EXT4_I(inode).i_data_sem);
    ext4_ext_dirty(core::ptr::null_mut(), inode, &path[path.p_depth]);
    up_write(&EXT4_I(inode).i_data_sem);
    ext4_mark_inode_dirty(core::ptr::null_mut(), inode);
    ext4_free_ext_path(path);
    }
...
#[no_mangle]
unsafe extern "C" fn skip_hole(inode: *mut inode, cur: *mut ext4_lblk_t) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_replay_set_iblocks(inode: *mut inode) -> c_int {
    int ext4_ext_replay_set_iblocks(struct inode *inode)
    {
    struct ext4_ext_path *path = core::ptr::null_mut(), *path2 = core::ptr::null_mut();
    struct ext4_extent *ex;
    let mut cur: ext4_lblk_t = 0, end;
    let mut numblks: c_int = 0, i, ret = 0;
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_clear_bb(inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext4_ext_space_root_idx_test(inode: *mut inode, check: c_int) -> c_int {
