...
#[no_mangle]
unsafe extern "C" fn prepend_char(p: *mut prepend_buffer, c: c_uchar) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn prepend_copy(dst: *mut c_void, src: *const c_void, len: c_int) -> bool {
...
#[no_mangle]
unsafe extern "C" fn prepend(p: *mut prepend_buffer, str: *const c_char, namelen: c_int) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn prepend_name(p: *mut prepend_buffer, name: *const qstr) -> bool {
    static bool prepend_name(struct prepend_buffer *p, const struct qstr *name)
    {
    const char *dname = smp_load_acquire(&name.name); /* ^^^ */
    let mut dlen: u32 = READ_ONCE(name.len);
...
#[no_mangle]
unsafe extern "C" fn get_fs_root_rcu(fs: *mut fs_struct, root: *mut path) {
...
//
    ...
    error = -ENAMETOOLONG;
#[no_mangle]
pub unsafe extern "C" fn if(size): unlikely(len >) -> else {
    else if (unlikely(len > size))
    error = -ERANGE;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: copy_to_user(buf, _arg: b.buf, _arg: len)) -> else {
