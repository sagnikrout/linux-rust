...
//
#[no_mangle]
pub unsafe extern "C" fn ufs_free_inode(inode: *mut *mut inode) {
...
//
    ...
    let mut end: sector_t = beg + uspi.s_fpb;
    UFSD("ENTER cgno %d\n", ucpi.c_cgx);
    for (; beg < end; ++beg) {
    bh = sb_getblk(sb, beg);
    lock_buffer(bh);
...
//
    ...
    let mut initediblk: u32 = fs32_to_cpu(sb, ucg.cg_u.cg_u2.cg_initediblk);
