...
#[no_mangle]
unsafe extern "C" fn ufs_commit_chunk(folio: *mut folio, pos: loff_t, len: unsigned) {
...
#[no_mangle]
unsafe extern "C" fn ufs_handle_dirsync(dir: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ufs_inode_by_name(dir: *mut inode, qstr: *const qstr) -> ino_t {
    ...
    let mut len: unsigned = fs16_to_cpu(dir.i_sb, de.d_reclen);
    int err;
    folio_lock(folio);
...
#[no_mangle]
unsafe extern "C" fn ufs_check_folio(folio: *mut folio, kaddr: *mut c_char) -> bool {
    ...
    let mut chunk_mask: unsigned = UFS_SB(sb).s_uspi.s_dirblksize - 1;
...
//
    ...
    let mut namelen: c_int = qstr.len;
    let mut reclen: unsigned = UFS_DIR_REC_LEN(namelen);
...
//
#[no_mangle]
pub unsafe extern "C" fn ufs_add_link(dentry: *mut dentry, inode: *mut inode) -> c_int {
    ...
    struct super_block *sb = dir.i_sb;
    let mut reclen: unsigned = UFS_DIR_REC_LEN(namelen);
    let mut chunk_size: c_uint = UFS_SB(sb).s_uspi.s_dirblksize;
...
//
    ...
    char *kaddr;
    loff_t pos;
    struct ufs_dir_entry *de, *pde = core::ptr::null_mut();
    int err;
    UFSD("ENTER\n");
...
#[no_mangle]
pub unsafe extern "C" fn ufs_make_empty(inode: *mut *mut inode, dir: *mut inode) -> c_int {
    int ufs_make_empty(struct inode * inode, struct inode *dir)
    {
    let mut sb: *mut super_block = dir.i_sb;
...
//
#[no_mangle]
pub unsafe extern "C" fn ufs_empty_dir(inode: *mut *mut inode) -> c_int {
    ...
    if (de.d_ino) {
    let mut namelen: u16 = ufs_get_de_namlen(sb, de);
// check for . and ..
    if (de.d_name[0] != '.')
    goto not_empty;
    if (namelen > 2)
...
#[no_mangle]
unsafe extern "C" fn ufs_dir_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufs_dir_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ufs_dir_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
