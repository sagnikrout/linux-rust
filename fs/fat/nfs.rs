...
pub struct fat_fid {
    ...
    pub i_pos_low: u32,
    pub i_pos_hi: u16,
    pub parent_i_pos_hi: u16,
    pub parent_i_pos_low: u32,
    pub parent_i_gen: u32,
}

pub const FAT_FID_SIZE_WITHOUT_PARENT: c_int = 3;
...
//
    ...
    if (inode == core::ptr::null_mut() && MSDOS_SB(sb).options.nfs == FAT_NFS_NOSTALE_RO) {
...
//
    ...
    loff_t i_pos;
    let mut type: c_int = FILEID_FAT_WITHOUT_PARENT;
    if (parent) {
...
//
    ...
    struct inode *dummy_grand_parent = core::ptr::null_mut();
    struct fat_slot_info sinfo;
    struct msdos_sb_info *sbi = MSDOS_SB(sb);
    let mut blknr: sector_t = fat_clus_to_blknr(sbi, parent_logstart);
...
//
    ...
    struct buffer_head *bh = core::ptr::null_mut();
    struct msdos_dir_entry *de;
    struct inode *parent_inode = core::ptr::null_mut();
    struct msdos_sb_info *sbi = MSDOS_SB(sb);
    if (!fat_get_dotdot_entry(d_inode(child_dir), &bh, &de)) {
    let mut parent_logstart: c_int = fat_get_start(sbi, de);
