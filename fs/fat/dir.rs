...
#[no_mangle]
pub unsafe extern "C" fn fat_tolower(c: c_uchar) -> c_uchar {
...
// root dir of FAT12/FAT16
    ...
    if (bh == core::ptr::null_mut() || !buffer_uptodate(bh)) {
...
// Returns the inode number of the directory entry at offset pos. If bh is
    non-core::ptr::null_mut(), it is brelse'd before. Pos is incremented. The buffer header is
    returned in bh.
    AV. Most often we do it item-by-item. Makes sense to optimize.
    AV. OK, there we go: if both bh and de are non-core::ptr::null_mut() we assume that we just
    AV. want the next entry (took one explicit de=core::ptr::null_mut() in vfat/namei.c).
    AV. It's done in fat_get_entry() (inlined), here the slow case lives.
    AV. Additionally, when we return -1 (i.e. reached the end of directory)
    AV. we make bh core::ptr::null_mut().
//
...
//
    ...
    struct msdos_dir_entry **de)
    {
    let mut err: c_int = fat_get_entry(dir, pos, bh, de);
...
//
    ...
    {
    let mut uni_xlate: c_int = MSDOS_SB(sb).options.unicode_xlate;
    const wchar_t *ip;
    wchar_t ec;
    unsigned char *op;
    int charlen;
...
// uni = 0x003f;	/* a question mark
    ...
    let mut nc: c_uchar = t.charset2lower[*c];
    if (!nc)
    nc = *c;
...
#[no_mangle]
pub unsafe extern "C" fn if(VFAT_SFN_DISPLAY_WIN95: opt &) -> else {
    else if (opt & VFAT_SFN_DISPLAY_WIN95)
    len = fat_short2uni(nls, buf, buf_size, uni_buf);
#[no_mangle]
pub unsafe extern "C" fn if(VFAT_SFN_DISPLAY_WINNT: opt &) -> else {
    if (lower)
    len = fat_short2lower_uni(nls, buf, buf_size, uni_buf);
    else
    len = fat_short2uni(nls, buf, buf_size, uni_buf);
...
//
    ...
    let mut isvfat: c_int = sbi.options.isvfat;
    let mut nocase: c_int = sbi.options.nocase;
    let mut opt_shortname: c_ushort = sbi.options.shortname;
    struct nls_table *nls_disk = sbi.nls_disk;
    wchar_t uni_name[14];
    unsigned char c, work[MSDOS_NAME];
    unsigned char *ptname = name;
    int chi, chl, i, j, k;
    let mut dotoffset: c_int = 0;
    let mut name_len: c_int = 0, uni_len = 0;
...
// Extension
    ...
    uni_len = j;
    if (isvfat) {
    let mut offset: c_int = min(chl, MSDOS_NAME-k);
    k += offset;
    i += offset;
    } else {
...
//
    ...
    wchar_t *unicode = core::ptr::null_mut();
    unsigned char bufname[FAT_MAX_SHORT_SIZE];
    let mut cpos: loff_t = 0;
    int err, len;
    err = -ENOENT;
    while (1) {
    ...
    continue;
#[no_mangle]
pub unsafe extern "C" fn if(PARSE_NOT_LONGNAME: status ==) -> else {
    else if (status == PARSE_NOT_LONGNAME)
    goto parse_record;
#[no_mangle]
pub unsafe extern "C" fn if(PARSE_EOF: status ==) -> else {
...
// Compare shortname
    ...
    let mut size: c_int = PATH_MAX - FAT_MAX_UNI_SIZE;
// Compare longname
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct fat_ioctl_filldir_callback {
    pub ctx: dir_context,
    pub dirent: *mut void __user,
    pub result: c_int,
// for dir ioctl
    ...
    unsigned char bufname[FAT_MAX_SHORT_SIZE];
    let mut isvfat: c_int = sbi.options.isvfat;
    const char *fill_name = core::ptr::null_mut();
    let mut fake_offset: c_int = 0;
    loff_t cpos;
    let mut short_len: c_int = 0, fill_len = 0;
    let mut ret: c_int = 0;
...
pub unsafe extern "C" fn if(PARSE_EOF: status ==) -> else {
    ...
    void *longname = unicode + FAT_MAX_UNI_CHARS;
    let mut size: c_int = PATH_MAX - FAT_MAX_UNI_SIZE;
    let mut len: c_int = fat_uni_to_x8(sb, unicode, longname, size);
    fill_name = longname;
    fill_len = len;
...
// hack for fat_ioctl_filldir()
    ...
    unsigned long inum;
    let mut i_pos: loff_t = fat_make_i_pos(sb, bh, de);
...
#[no_mangle]
unsafe extern "C" fn fat_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    static int fat_readdir(struct file *file, struct dir_context *ctx)
    {
    return __fat_readdir(file_inode(file), file, ctx, 0, core::ptr::null_mut());
    }
...
// dirent has short and long name */			   \
    ...
    short_only, both ? &buf : core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn fat_dir_empty(dir: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn fat_subdirs(dir: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn __fat_remove_entries(dir: *mut inode, pos: loff_t, nr_slots: c_int) -> c_int {
    static int __fat_remove_entries(struct inode *dir, loff_t pos, int nr_slots)
    {
    struct super_block *sb = dir.i_sb;
    struct buffer_head *bh;
    struct msdos_dir_entry *de, *endp;
    let mut err: c_int = 0, orig_slots;
    while (nr_slots) {
    bh = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn fat_remove_entries(dir: *mut inode, sinfo: *mut fat_slot_info) -> c_int {
    int fat_remove_entries(struct inode *dir, struct fat_slot_info *sinfo)
    {
    struct super_block *sb = dir.i_sb;
    struct msdos_dir_entry *de;
    struct buffer_head *bh;
    let mut err: c_int = 0, nr_slots;
//
...
//
    ...
    fat_truncate_time(dir, core::ptr::null_mut(), FAT_UPDATE_ATIME | FAT_UPDATE_CMTIME);
    err = 0;
    ...
    {
    struct super_block *sb = dir.i_sb;
    let mut last_blknr: sector_t = blknr + MSDOS_SB(sb).sec_per_clus;
    int err, i, n;
...
#[no_mangle]
pub unsafe extern "C" fn fat_alloc_new_dir(dir: *mut inode, ts: *mut timespec64) -> c_int {
...
//
    let mut size: c_int = free_slots * sizeof(*de);
    let mut offset: c_int = pos & (sb.s_blocksize - 1);
    let mut long_bhs: c_int = nr_bhs - (nr_slots == 0);
// Fill the long name slots.
    for (i = 0; i < long_bhs; i++) {
    let mut copy: c_int = umin(sb.s_blocksize - offset, size);
