...
//
    ...
    {
    let mut last_byte: unsigned = PAGE_SIZE;
    if (page_nr == (inode.i_size >> PAGE_SHIFT))
    last_byte = inode.i_size & (PAGE_SIZE - 1);
    return last_byte;
    }
#[no_mangle]
unsafe extern "C" fn dir_commit_chunk(folio: *mut folio, pos: loff_t, len: unsigned) {
...
#[no_mangle]
unsafe extern "C" fn minix_handle_dirsync(dir: *mut inode) -> c_int {
    ...
    struct folio **foliop)
    {
    struct folio *folio = read_mapping_folio(dir.i_mapping, n, core::ptr::null_mut());
    if (IS_ERR(folio))
...
#[no_mangle]
unsafe extern "C" fn minix_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    static int minix_readdir(struct file *file, struct dir_context *ctx)
    {
    struct inode *inode = file_inode(file);
    struct super_block *sb = inode.i_sb;
    struct minix_sb_info *sbi = minix_sb(sb);
    let mut chunk_size: unsigned = sbi.s_dirsize;
    let mut npages: c_ulong = dir_pages(inode);
    let mut pos: c_ulong = ctx.pos;
    unsigned offset;
    unsigned long n;
    ...
    if (inumber) {
    let mut l: unsigned = strnlen(name, sbi.s_namelen);
...
//
    ...
    {
    let mut name: *const c_char = dentry.d_name.name;
    let mut namelen: c_int = dentry.d_name.len;
    let mut dir: *mut inode = d_inode(dentry.d_parent);
    let mut sb: *mut super_block = dir.i_sb;
    let mut sbi: *mut minix_sb_info = minix_sb(sb);
    unsigned long n;
...
#[no_mangle]
pub unsafe extern "C" fn minix_add_link(dentry: *mut dentry, inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn minix_delete_entry(de: *mut minix_dir_entry, folio: *mut folio) -> c_int {
    ...
    let mut len: unsigned = sbi.s_dirsize;
    int err;
    folio_lock(folio);
...
#[no_mangle]
pub unsafe extern "C" fn minix_make_empty(inode: *mut inode, dir: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn minix_empty_dir(inode: *mut *mut inode) -> c_int {
...
// check for . and ..
    ...
    goto not_empty;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: name[2]) -> else {
    else if (name[2])
    goto not_empty;
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn minix_inode_by_name(dentry: *mut dentry) -> ino_t {
