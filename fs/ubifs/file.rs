...
#[no_mangle]
unsafe extern "C" fn do_readpage(folio: *mut folio) -> c_int {
    static int do_readpage(struct folio *folio)
    {
    let mut err: c_int = 0, i;
    unsigned int block, beyond;
    struct ubifs_data_node *dn = core::ptr::null_mut();
...
// Reading beyond inode
    ...
    let mut dlen: c_int = le32_to_cpu(dn.size);
    let mut ilen: c_int = i_size & (UBIFS_BLOCK_SIZE - 1);
    if (ilen && ilen < dlen)
...
//
#[no_mangle]
unsafe extern "C" fn release_new_page_budget(c: *mut ubifs_info) {
    static void release_new_page_budget(struct ubifs_info *c)
    {
    let mut req: ubifs_budget_req = { .recalculate = 1, .new_page = 1 };
...
//
#[no_mangle]
unsafe extern "C" fn release_existing_page_budget(c: *mut ubifs_info) {
    static void release_existing_page_budget(struct ubifs_info *c)
    {
    let mut req: ubifs_budget_req = { .dd_growth = c.bi.page_budget};
    ubifs_release_budget(c, &req);
    }
    ...
    let mut req: ubifs_budget_req = { .new_page = 1 };
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !folio_test_checked(folio)) -> else {
...
//
    ...
    {
    let mut req: ubifs_budget_req = { .fast = 1 };
    if (folio.private) {
    if (!appending)
//
...
//
    ...
    struct ubifs_inode *ui = ubifs_inode(inode);
    struct ubifs_info *c = inode.i_sb.s_fs_info;
    let mut end_pos: loff_t = pos + len;
    let mut appending: c_int = !!(end_pos > inode.i_size);
...
//
    ...
    {
    let mut i: c_int = 0, nn = *n, offs = bu.zbranch[0].offs, hole = 0, read = 0;
    struct inode *inode = folio.mapping.host;
    let mut i_size: loff_t = i_size_read(inode);
    ...
    let mut len: c_int = i_size & (PAGE_SIZE - 1);
    if (len && len < read)
...
//
    ...
    struct folio *folio1)
    {
    let mut offset: pgoff_t = folio1.index, end_index;
    struct address_space *mapping = folio1.mapping;
    struct inode *inode = mapping.host;
    struct ubifs_inode *ui = ubifs_inode(inode);
    int err, page_idx, page_cnt, ret = 0, n = 0;
    let mut allocate: c_int = bu.buf ? 0 : 1;
    loff_t isize;
    let mut ra_gfp_mask: gfp_t = readahead_gfp_mask(mapping) & ~__GFP_FS;
...
//
    ...
    let mut page_offset: pgoff_t = offset + page_idx;
    struct folio *folio;
...
//
#[no_mangle]
unsafe extern "C" fn ubifs_bulk_read(folio: *mut folio) -> c_int {
    static int ubifs_bulk_read(struct folio *folio)
    {
    struct inode *inode = folio.mapping.host;
    struct ubifs_info *c = inode.i_sb.s_fs_info;
    struct ubifs_inode *ui = ubifs_inode(inode);
    let mut index: pgoff_t = folio.index, last_page_read = ui.last_page_read;
    struct bu_info *bu;
    let mut err: c_int = 0, allocated = 0;
...
#[no_mangle]
unsafe extern "C" fn ubifs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn do_writepage(folio: *mut folio, len: usize) -> c_int {
    static int do_writepage(struct folio *folio, size_t len)
    {
    let mut err: c_int = 0, blen;
    unsigned int block;
    let mut offset: usize = 0;
    union ubifs_key key;
    struct inode *inode = folio.mapping.host;
    ...
    ubifs_assert(c, folio.private != core::ptr::null_mut());
    if (folio_test_checked(folio))
    release_new_page_budget(c);
    else
    release_existing_page_budget(c);
...
//
#[no_mangle]
unsafe extern "C" fn ubifs_writepage(folio: *mut folio, wbc: *mut writeback_control) -> c_int {
    static int ubifs_writepage(struct folio *folio, struct writeback_control *wbc)
    {
    struct inode *inode = folio.mapping.host;
    struct ubifs_info *c = inode.i_sb.s_fs_info;
    struct ubifs_inode *ui = ubifs_inode(inode);
    let mut i_size: loff_t = i_size_read(inode), synced_i_size;
...
// Is the folio fully inside i_size?
    ...
    err = inode.i_sb.s_op.write_inode(inode, core::ptr::null_mut());
    if (err)
    goto out_redirty;
//
...
//
#[no_mangle]
unsafe extern "C" fn do_attr_changes(inode: *mut inode, attr: *const iattr) {
...
//
    ...
    let mut old_size: loff_t = inode.i_size, new_size = attr.ia_size;
    let mut offset: c_int = new_size & (UBIFS_BLOCK_SIZE - 1), budgeted = 1;
    struct ubifs_inode *ui = ubifs_inode(inode);
...
//
    ...
    if (offset) {
    let mut index: pgoff_t = new_size >> PAGE_SHIFT;
    struct folio *folio;
...
//
    ...
    {
    int err, release;
    let mut new_size: loff_t = attr.ia_size;
    struct ubifs_inode *ui = ubifs_inode(inode);
...
#[no_mangle]
pub unsafe extern "C" fn ubifs_fsync(file: *mut file, start: loff_t, end: loff_t, datasync: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn update_mctime(inode: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ubifs_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    static ssize_t ubifs_write_iter(struct kiocb *iocb, struct iov_iter *from)
    {
    let mut err: c_int = update_mctime(file_inode(iocb.ki_filp));
    if (err)
    return err;
...
#[no_mangle]
unsafe extern "C" fn ubifs_release_folio(folio: *mut folio, unused_gfp_flags: gfp_t) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn ubifs_vm_page_mkwrite(vmf: *mut vm_fault) -> vm_fault_t {
...
#[no_mangle]
unsafe extern "C" fn ubifs_file_mmap_prepare(desc: *mut vm_area_desc) -> c_int {
