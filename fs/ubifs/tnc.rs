...
//
    ...
    p = &(*p).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(o->lnum: old_idx->lnum >) -> else {
    else if (old_idx.lnum > o.lnum)
    p = &(*p).rb_right;
#[no_mangle]
pub unsafe extern "C" fn if(o->offs: old_idx->offs <) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn insert_old_idx(c: *mut ubifs_info, lnum: c_int, offs: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn insert_old_idx_znode(c: *mut ubifs_info, znode: *mut ubifs_znode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn destroy_old_idx(c: *mut ubifs_info) {
...
//
#[no_mangle]
unsafe extern "C" fn add_idx_dirt(c: *mut ubifs_info, lnum: c_int, dirt: c_int) -> c_int {
...
//
    ...
    if (old_zn.level != 0) {
    int i;
    let mut n: c_int = new_zn.child_cnt;
...
//
#[no_mangle]
unsafe extern "C" fn lnc_free(zbr: *mut ubifs_zbranch) {
    static void lnc_free(struct ubifs_zbranch *zbr)
    {
    if (!zbr.leaf)
    return;
    kfree(zbr.leaf);
    zbr.leaf = core::ptr::null_mut();
    }
//
...
//
    ...
    struct ubifs_zbranch *zbr)
    {
    let mut len: c_int = zbr.len;
    let mut lnum: c_int = zbr.lnum;
    let mut offs: c_int = zbr.offs;
    int err, node_len;
...
// Add the node to the leaf node cache
    ...
    return NAME_MATCHES;
#[no_mangle]
pub unsafe extern "C" fn if(fname_len(nm): nlen <) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn tnc_next(c: *mut ubifs_info, zn: *mut ubifs_znode, n: *mut c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn tnc_prev(c: *mut ubifs_info, zn: *mut ubifs_znode, n: *mut c_int) -> c_int {
...
//
    ...
    int adding)
    {
    struct ubifs_znode *o_znode = core::ptr::null_mut(), *znode = *zn;
...
//
#[no_mangle]
unsafe extern "C" fn matches_position(zbr: *mut ubifs_zbranch, lnum: c_int, offs: c_int) -> c_int {
...
//
    ...
    int err, exact;
    struct ubifs_znode *znode;
    let mut time: time64_t = ktime_get_seconds();
    dbg_tnck(key, "search key ");
    ubifs_assert(c, key_type(c, key) < UBIFS_INVALID_KEY);
    znode = c.zroot.znode;
    if (unlikely(!znode)) {
    znode = ubifs_load_znode(c, &c.zroot, core::ptr::null_mut(), 0);
    if (IS_ERR(znode))
    return PTR_ERR(znode);
    }
    znode.time = time;
...
//
#[no_mangle]
unsafe extern "C" fn maybe_leb_gced(c: *mut ubifs_info, lnum: c_int, gc_seq1: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_tnc_get_bu_keys(c: *mut ubifs_info, bu: *mut bu_info) -> c_int {
    int ubifs_tnc_get_bu_keys(struct ubifs_info *c, struct bu_info *bu)
    {
    int n, err = 0, lnum = -1, offs;
    int len;
    let mut block: c_uint = key_block(c, &bu.key);
    struct ubifs_znode *znode;
    bu.cnt = 0;
    bu.blk_cnt = 0;
    bu.eof = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_tnc_bulk_read(c: *mut ubifs_info, bu: *mut bu_info) -> c_int {
    int ubifs_tnc_bulk_read(struct ubifs_info *c, struct bu_info *bu)
    {
    let mut lnum: c_int = bu.zbranch[0].lnum, offs = bu.zbranch[0].offs, len, err, i;
...
// an unmatchable name.
//
    let mut noname: fscrypt_name = { .disk_name = { .name = "", .len = 1 } };
    err = dbg_check_tnc(c, 0);
    mutex_unlock(&c.tnc_mutex);
    if (err)
    return err;
...
//
#[no_mangle]
unsafe extern "C" fn tnc_delete(c: *mut ubifs_info, znode: *mut ubifs_znode, n: c_int) -> c_int {
...
//
    ...
    if (IS_ERR(znode))
    return PTR_ERR(znode);
    znode.parent = core::ptr::null_mut();
    znode.iip = 0;
    if (c.zroot.len) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_tnc_remove(c: *mut ubifs_info, key: *const union ubifs_key) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_tnc_remove_ino(c: *mut ubifs_info, inum: ino_t) -> c_int {
    int ubifs_tnc_remove_ino(struct ubifs_info *c, ino_t inum)
    {
    union ubifs_key key1, key2;
    struct ubifs_dent_node *xent, *pxent = core::ptr::null_mut();
    let mut nm: fscrypt_name = {0};
...
//
#[no_mangle]
unsafe extern "C" fn tnc_destroy_cnext(c: *mut ubifs_info) {
    ...
    kfree(znode);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !ubifs_zn_cow(znode)) -> else {
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_tnc_close(c: *mut ubifs_info) {
...
//
    ...
    struct ubifs_znode *znode)
    {
    let mut level: c_int = znode.level;
    while (1) {
    let mut n: c_int = znode.iip - 1;
...
//
    ...
    struct ubifs_znode *znode, *zn;
    int n, found, err, nn;
    let mut unique: c_int = !is_hash_key(c, key);
    found = ubifs_lookup_level0(c, key, &znode, &n);
    if (found < 0)
...
// The index node was found but it was dirty
    err = 0;
#[no_mangle]
pub unsafe extern "C" fn if(2: err ==) -> else {
    else if (err == 2)
