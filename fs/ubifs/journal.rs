...
//
#[no_mangle]
pub unsafe extern "C" fn zero_ino_node_unused(ino: *mut ubifs_ino_node) {
...
//
#[no_mangle]
pub unsafe extern "C" fn zero_dent_node_unused(dent: *mut ubifs_dent_node) {
...
//
#[no_mangle]
pub unsafe extern "C" fn zero_trun_node_unused(trun: *mut ubifs_trun_node) {
    static inline void zero_trun_node_unused(struct ubifs_trun_node *trun)
    {
    memset(trun.padding, 0, 12);
    }
#[no_mangle]
unsafe extern "C" fn ubifs_add_auth_dirt(c: *mut ubifs_info, lnum: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn reserve_space(c: *mut ubifs_info, jhead: c_int, len: c_int) -> c_int {
    static int reserve_space(struct ubifs_info *c, int jhead, int len)
    {
    let mut err: c_int = 0, err1, retries = 0, avail, lnum, offs, squeeze;
...
//
    ...
    {
    let mut auth_node_size: c_int = ubifs_auth_node_sz(c);
    int err;
    while (1) {
    const struct ubifs_ch *ch = node;
    let mut nodelen: c_int = le32_to_cpu(ch.len);
...
//
#[no_mangle]
unsafe extern "C" fn __queue_and_wait(c: *mut ubifs_info) {
...
//
#[no_mangle]
unsafe extern "C" fn wait_for_reservation(c: *mut ubifs_info) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn wake_up_reservation(c: *mut ubifs_info) {
...
//
#[no_mangle]
unsafe extern "C" fn add_or_start_queue(c: *mut ubifs_info) {
...
//
#[no_mangle]
unsafe extern "C" fn make_reservation(c: *mut ubifs_info, jhead: c_int, len: c_int) -> c_int {
    static int make_reservation(struct ubifs_info *c, int jhead, int len)
    {
    int err, cmt_retries = 0, nospc_retries = 0;
    let mut blocked: bool = wait_for_reservation(c);
    again:
...
//
#[no_mangle]
pub unsafe extern "C" fn release_head(c: *mut ubifs_info, jhead: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn finish_reservation(c: *mut ubifs_info) {
...
//
#[no_mangle]
unsafe extern "C" fn get_dent_type(mode: c_int) -> c_int {
...
//
    ...
    {
    let mut data_len: c_int = 0, last_reference = !inode.i_nlink;
...
//
#[no_mangle]
unsafe extern "C" fn mark_inode_clean(c: *mut ubifs_info, ui: *mut ubifs_inode) {
...
#[no_mangle]
unsafe extern "C" fn set_dent_cookie(c: *mut ubifs_info, dent: *mut ubifs_dent_node) {
    static void set_dent_cookie(struct ubifs_info *c, struct ubifs_dent_node *dent)
    {
    if (c.double_hash)
    dent.cookie = ( __le32) get_random_u32();
    else
    dent.cookie = 0;
    }
//
...
//
    ...
    let mut last_reference: c_int = !!(deletion && inode.i_nlink == 0);
    struct ubifs_inode *ui = ubifs_inode(inode);
...
// Make reservation before allocating sequence numbers
    ...
    if (fname_name(nm) == core::ptr::null_mut())
...
//
    ...
    let mut dlen: c_int = COMPRESSED_DATA_NODE_BUF_SZ, allocated = 1;
    int write_len;
    struct ubifs_inode *ui = ubifs_inode(inode);
    let mut encrypted: bool = IS_ENCRYPTED(inode);
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_jnl_write_inode(c: *mut ubifs_info, inode: *const inode) -> c_int {
    int ubifs_jnl_write_inode(struct ubifs_info *c, const struct inode *inode)
    {
    int err, lnum, offs;
    struct ubifs_ino_node *ino, *ino_start;
    struct ubifs_inode *ui = ubifs_inode(inode);
    let mut sync: c_int = 0, write_len = 0, ilen = UBIFS_INO_NODE_SZ;
    let mut last_reference: c_int = !inode.i_nlink;
    let mut kill_xattrs: c_int = ui.xattr_cnt && last_reference;
    u8 hash[UBIFS_HASH_ARR_SZ];
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_jnl_delete_inode(c: *mut ubifs_info, inode: *const inode) -> c_int {
...
//
    ...
    let mut twoparents: c_int = (fst_dir != snd_dir);
    void *p;
...
//
    ...
    let mut last_reference: c_int = !!(new_inode && new_inode.i_nlink == 0);
    let mut move: c_int = (old_dir != new_dir);
...
//
    ...
    int dn_size;
    struct ubifs_inode *ui = ubifs_inode(inode);
    let mut inum: ino_t = inode.i_ino;
    unsigned int blk;
    u8 hash_ino[UBIFS_HASH_ARR_SZ];
...
pub unsafe extern "C" fn if(_arg: err) -> else {
    ...
    let mut dn_len: c_int = le32_to_cpu(dn.size);
...
//
    ...
    let mut sync: c_int = IS_DIRSYNC(host);
