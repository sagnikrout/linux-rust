...
// may move.
//
pub const SOFT_LEBS_LIMIT: c_int = 4;
pub const HARD_LEBS_LIMIT: c_int = 32;
//
...
//
#[no_mangle]
unsafe extern "C" fn switch_gc_head(c: *mut ubifs_info) -> c_int {
...
//
    ...
    if (inuma == inumb) {
    let mut blka: c_uint = key_block(c, &sa.key);
    let mut blkb: c_uint = key_block(c, &sb.key);
    if (blka <= blkb)
    return -1;
    } else if (inuma <= inumb)
    return -1;
...
// Inodes go before directory entries
    ...
    inumb = key_inum(c, &sb.key);
    if (inuma == inumb) {
    let mut hasha: u32 = key_hash(c, &sa.key);
    let mut hashb: u32 = key_hash(c, &sb.key);
    if (hasha <= hashb)
    return -1;
    } else if (inuma <= inumb)
    return -1;
...
//
#[no_mangle]
unsafe extern "C" fn move_nodes(c: *mut ubifs_info, sleb: *mut ubifs_scan_leb) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn gc_sync_wbufs(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_garbage_collect_leb(c: *mut ubifs_info, lp: *mut ubifs_lprops) -> c_int {
    int ubifs_garbage_collect_leb(struct ubifs_info *c, struct ubifs_lprops *lp)
    {
    struct ubifs_scan_leb *sleb;
    struct ubifs_scan_node *snod;
    struct ubifs_wbuf *wbuf = &c.jheads[GCHD].wbuf;
    let mut err: c_int = 0, lnum = lp.lnum;
...
//
    ...
    let mut level: c_int = le16_to_cpu(idx.level);
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_garbage_collect(c: *mut ubifs_info, anyway: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_gc_start_commit(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_gc_end_commit(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_destroy_idx_gc(c: *mut ubifs_info) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_get_idx_gc_leb(c: *mut ubifs_info) -> c_int {
