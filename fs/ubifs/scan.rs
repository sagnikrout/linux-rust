...
//
#[no_mangle]
unsafe extern "C" fn scan_padding_bytes(buf: *mut c_void, len: c_int) -> c_int {
    static int scan_padding_bytes(void *buf, int len)
    {
    let mut pad_len: c_int = 0, max_pad_len = min_t(int, UBIFS_PAD_NODE_SZ, len);
    uint8_t *p = buf;
    dbg_scan("not a node");
...
//
    ...
    let mut pad_len: c_int = le32_to_cpu(pad.pad_len);
    let mut node_len: c_int = le32_to_cpu(ch.len);
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_scan_destroy(sleb: *mut ubifs_scan_leb) {
