...
//
#[no_mangle]
unsafe extern "C" fn inherit_flags(dir: *const inode, mode: umode_t) -> c_int {
...
//
    ...
    if (encrypted) {
    err = fscrypt_set_context(inode, core::ptr::null_mut());
    if (err) {
    if (!is_xattr) {
    set_nlink(inode, 1);
    ...
    struct ubifs_dent_node *dent = core::ptr::null_mut();
...
//
    ...
    {
    int err;
    let mut mode: umode_t = S_IFCHR | WHITEOUT_MODE;
    struct inode *inode;
    struct ubifs_info *c = dir.i_sb.s_fs_info;
//
...
//
#[no_mangle]
unsafe extern "C" fn lock_2_inodes(inode1: *mut inode, inode2: *mut inode) {
...
//
#[no_mangle]
unsafe extern "C" fn unlock_2_inodes(inode1: *mut inode, inode2: *mut inode) {
    ...
    struct ubifs_budget_req req = { .new_ino = 1, .new_dent = 1,
    .dirtied_ino = 1};
    let mut ino_req: ubifs_budget_req = { .dirtied_ino = 1 };
    struct ubifs_inode *ui;
...
//
#[no_mangle]
unsafe extern "C" fn vfs_dent_type(type: u8) -> c_uint {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ubifs_dir_data {
    pub dent: *mut ubifs_dent_node,
    pub cookie: u64,
}
...
//
#[no_mangle]
unsafe extern "C" fn ubifs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    static int ubifs_readdir(struct file *file, struct dir_context *ctx)
    {
    let mut fstr_real_len: c_int = 0, err = 0;
    struct fscrypt_name nm;
    let mut fstr: fscrypt_str = {0};
    union ubifs_key key;
    struct ubifs_dent_node *dent;
    struct inode *dir = file_inode(file);
    struct ubifs_info *c = dir.i_sb.s_fs_info;
    let mut encrypted: bool = IS_ENCRYPTED(dir);
...
#[no_mangle]
unsafe extern "C" fn ubifs_dir_release(dir: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ubifs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
    ...
    let mut req: ubifs_budget_req = { .mod_dent = 1, .dirtied_ino = 2 };
    let mut saved_nlink: c_uint = inode.i_nlink;
    struct fscrypt_name nm;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_check_dir_empty(dir: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ubifs_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
//
    ...
    struct ubifs_inode *dir_ui = ubifs_inode(dir);
    struct ubifs_info *c = dir.i_sb.s_fs_info;
    union ubifs_dev_desc *dev = core::ptr::null_mut();
    int sz_change;
    int err, devlen = 0;
    struct ubifs_budget_req req = { .new_ino = 1, .new_dent = 1,
    .dirtied_ino = 1 };
...
//
    ...
    struct ubifs_inode *whiteout_ui = core::ptr::null_mut();
    int err, release, sync = 0, move = (new_dir != old_dir);
    let mut is_dir: c_int = S_ISDIR(old_inode.i_mode);
    let mut unlink: c_int = !!new_inode, new_sz, old_sz;
...
// code to userspace.
//
    old_inode.i_sb.s_op.write_inode(old_inode, core::ptr::null_mut());
    fscrypt_free_filename(&old_nm);
    fscrypt_free_filename(&new_nm);
    return 0;
    out_cancel:
    ...
    struct ubifs_budget_req req = { .new_dent = 1, .mod_dent = 1,
    .dirtied_ino = 2 };
    let mut sync: c_int = IS_DIRSYNC(old_dir) || IS_DIRSYNC(new_dir);
...
//
    ...
    if (err)
    goto out;
    lock_4_inodes(old_dir, new_dir, core::ptr::null_mut(), core::ptr::null_mut());
    simple_rename_timestamp(old_dir, old_dentry, new_dir, new_dentry);
    if (old_dir != new_dir) {
...
#[no_mangle]
pub unsafe extern "C" fn if(S_ISDIR(snd_inode->i_mode): !S_ISDIR(fst_inode->i_mode) &&) -> else {
    drop_nlink(new_dir);
    inc_nlink(old_dir);
    }
    }
    ...
    unlock_4_inodes(old_dir, new_dir, core::ptr::null_mut(), core::ptr::null_mut());
    ubifs_release_budget(c, &req);
    out:
    fscrypt_free_filename(&fst_nm);
...
#[no_mangle]
unsafe extern "C" fn ubifs_dir_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ubifs_dir_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
