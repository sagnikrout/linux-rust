...
//
#[no_mangle]
unsafe extern "C" fn do_calc_lpt_geom(c: *mut ubifs_info) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_calc_lpt_geom(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn pack_bits(c: *const ubifs_info, addr: *mut u8, pos: *mut c_int, val: u32, nrbits: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_unpack_bits(c: *const ubifs_info, addr: *mut u8, pos: *mut c_int, nrbits: c_int) -> u32 {
    uint32_t ubifs_unpack_bits(const struct ubifs_info *c, uint8_t **addr, int *pos, int nrbits)
    {
    let mut k: c_int = 32 - nrbits;
    uint8_t *p = *addr;
    let mut b: c_int = *pos;
    uint32_t val;
    let mut bytes: c_int = (nrbits + b + 7) >> 3;
    ubifs_assert(c, nrbits > 0);
    ubifs_assert(c, nrbits <= 32);
    ubifs_assert(c, *pos >= 0);
    ubifs_assert(c, *pos < 8);
...
//
    ...
    for (i = 0; i < UBIFS_LPT_FANOUT; i++) {
    let mut lnum: c_int = nnode.nbranch[i].lnum;
    if (lnum == 0)
    lnum = c.lpt_last + 1;
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_pack_lsave(c: *mut ubifs_info, buf: *mut c_void, lsave: *mut c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_add_lpt_dirt(c: *mut ubifs_info, lnum: c_int, dirty: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn set_ltab(c: *mut ubifs_info, lnum: c_int, free: c_int, dirty: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_add_nnode_dirt(c: *mut ubifs_info, nnode: *mut ubifs_nnode) {
...
//
#[no_mangle]
unsafe extern "C" fn add_pnode_dirt(c: *mut ubifs_info, pnode: *mut ubifs_pnode) {
...
//
#[no_mangle]
unsafe extern "C" fn calc_nnode_num(row: c_int, col: c_int) -> c_int {
...
//
    ...
    struct ubifs_pnode *pnode = core::ptr::null_mut();
    struct ubifs_nnode *nnode = core::ptr::null_mut();
    void *buf = core::ptr::null_mut(), *p;
    struct ubifs_lpt_lprops *ltab = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn update_cats(c: *mut ubifs_info, pnode: *mut ubifs_pnode) {
    static void update_cats(struct ubifs_info *c, struct ubifs_pnode *pnode)
    {
    int i;
    for (i = 0; i < UBIFS_LPT_FANOUT; i++) {
    let mut cat: c_int = pnode.lprops[i].flags & LPROPS_CAT_MASK;
    let mut lnum: c_int = pnode.lprops[i].lnum;
    if (!lnum)
    return;
...
//
#[no_mangle]
unsafe extern "C" fn check_lpt_crc(c: *const ubifs_info, buf: *mut c_void, len: c_int) -> c_int {
...
//
    ...
    for (i = 0; i < UBIFS_LPT_FANOUT; i++) {
    let mut lprops: *mut ubifs_lprops  const = &pnode.lprops[i];
...
//
#[no_mangle]
unsafe extern "C" fn unpack_ltab(c: *const ubifs_info, buf: *mut c_void) -> c_int {
    ...
    return err;
    for (i = 0; i < c.lpt_lebs; i++) {
    let mut free: c_int = ubifs_unpack_bits(c, &addr, &pos, c.lpt_spc_bits);
    let mut dirty: c_int = ubifs_unpack_bits(c, &addr, &pos, c.lpt_spc_bits);
...
//
#[no_mangle]
unsafe extern "C" fn unpack_lsave(c: *const ubifs_info, buf: *mut c_void) -> c_int {
    ...
    return err;
    for (i = 0; i < c.lsave_cnt; i++) {
    let mut lnum: c_int = ubifs_unpack_bits(c, &addr, &pos, c.lnum_bits);
...
//
    ...
    int i, lvl, max_offs;
    if (c.big_lpt) {
    let mut num: c_int = calc_nnode_num_from_parent(c, parent, iip);
    if (nnode.num != num)
    return -EINVAL;
    }
    ...
    for (i = 0; i < UBIFS_LPT_FANOUT; i++) {
    let mut lnum: c_int = nnode.nbranch[i].lnum;
    let mut offs: c_int = nnode.nbranch[i].offs;
    if (lnum == 0) {
    if (offs != 0)
    return -EINVAL;
    continue;
...
//
    ...
    int i;
    if (c.big_lpt) {
    let mut num: c_int = calc_pnode_num_from_parent(c, parent, iip);
    if (pnode.num != num)
    return -EINVAL;
    }
    for (i = 0; i < UBIFS_LPT_FANOUT; i++) {
    let mut free: c_int = pnode.lprops[i].free;
    let mut dirty: c_int = pnode.lprops[i].dirty;
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_read_nnode(c: *mut ubifs_info, parent: *mut ubifs_nnode, iip: c_int) -> c_int {
    int ubifs_read_nnode(struct ubifs_info *c, struct ubifs_nnode *parent, int iip)
    {
    struct ubifs_nbranch *branch = core::ptr::null_mut();
    struct ubifs_nnode *nnode = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn read_pnode(c: *mut ubifs_info, parent: *mut ubifs_nnode, iip: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn read_ltab(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn read_lsave(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_lpt_calc_hash(c: *mut ubifs_info, hash: *mut u8) -> c_int {
    int ubifs_lpt_calc_hash(struct ubifs_info *c, u8 *hash)
    {
    struct ubifs_nnode *nnode, *nn;
    struct ubifs_cnode *cnode;
    struct shash_desc *desc;
    let mut iip: c_int = 0, i;
    let mut bufsiz: c_int = max_t(int, c.nnode_sz, c.pnode_sz);
    void *buf;
    int err;
...
//
#[no_mangle]
unsafe extern "C" fn lpt_check_hash(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn lpt_init_rd(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn lpt_init_wr(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_lpt_init(c: *mut ubifs_info, rd: c_int, wr: c_int) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct lpt_scan_node {
    union {
    pub nnode: ubifs_nnode,
    pub pnode: ubifs_pnode,
    pub cnode: ubifs_cnode,
}
...
//
    ...
    {
    let mut err: c_int = 0, i, h, iip, shft;
    struct ubifs_nnode *nnode;
    struct ubifs_pnode *pnode;
    struct lpt_scan_node *path;
    if (start_lnum == -1) {
...
// Add all the nodes in path to the tree in memory
    for (h = 1; h < c.lpt_hght; h++) {
    let mut sz: usize = sizeof(struct ubifs_nnode);
    ...
    let mut sz: usize = sizeof(struct ubifs_pnode);
    struct ubifs_nnode *parent;
