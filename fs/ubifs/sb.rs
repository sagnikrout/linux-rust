...
// flash size.
//
pub const DEFAULT_JNL_PERCENT: c_int = 5;
// Default maximum journal size in bytes

// Default indexing tree fanout
pub const DEFAULT_FANOUT: c_int = 8;
// Default number of data journal heads
pub const DEFAULT_JHEADS_CNT: c_int = 1;
// Default positions of different LEBs in the main area
pub const DEFAULT_IDX_LEB: c_int = 0;
pub const DEFAULT_DATA_LEB: c_int = 1;
pub const DEFAULT_GC_LEB: c_int = 2;
// Default number of LEB numbers in LPT's save table
pub const DEFAULT_LSAVE_CNT: c_int = 256;
// Default reserved pool size as a percent of maximum free space
pub const DEFAULT_RP_PERCENT: c_int = 5;
// The default maximum size of reserved pool in bytes

// Default time granularity in nanoseconds
pub const DEFAULT_TIME_GRAN: c_int = 1000000000;
#[no_mangle]
unsafe extern "C" fn get_default_compressor(c: *mut ubifs_info) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn create_default_filesystem(c: *mut ubifs_info) -> c_int {
    ...
    let mut min_leb_cnt: c_int = UBIFS_MIN_LEB_CNT;
    int idx_node_size;
...
//
#[no_mangle]
unsafe extern "C" fn validate_sb(c: *mut ubifs_info, sup: *mut ubifs_sb_node) -> c_int {
    static int validate_sb(struct ubifs_info *c, struct ubifs_sb_node *sup)
    {
    long long max_bytes;
    let mut err: c_int = 1, min_leb_cnt;
    if (!c.key_hash) {
    err = 2;
    goto failed;
    }
...
//
    ...
    {
    let mut sup_flags: c_uint = le32_to_cpu(sup.flags);
    u8 hmac_wkm[UBIFS_HMAC_ARR_SZ];
    let mut authenticated: c_int = !!(sup_flags & UBIFS_FLG_AUTHENTICATION);
    int hash_algo;
    int err;
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_write_sb_node(c: *mut ubifs_info, sup: *mut ubifs_sb_node) -> c_int {
    int ubifs_write_sb_node(struct ubifs_info *c, struct ubifs_sb_node *sup)
    {
    let mut len: c_int = ALIGN(UBIFS_SB_NODE_SZ, c.min_io_size);
    int err;
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_read_superblock(c: *mut ubifs_info) -> c_int {
...
// Automatically increase file system size to the maximum size
    ...
    let mut old_leb_cnt: c_int = c.leb_cnt;
...
//
#[no_mangle]
unsafe extern "C" fn fixup_leb(c: *mut ubifs_info, lnum: c_int, len: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn fixup_free_space(c: *mut ubifs_info) -> c_int {
...
// Fixup LEBs in the LPT area
    ...
    let mut free: c_int = c.ltab[lnum - c.lpt_first].free;
    if (free > 0) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ubifs_fixup_free_space(c: *mut ubifs_info) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ubifs_enable_encryption(c: *mut ubifs_info) -> c_int {
