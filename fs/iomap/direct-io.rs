...
#[no_mangle]
pub unsafe extern "C" fn iomap_dio_err_type(dio: *const iomap_dio) -> enum fserror_type {
...
#[no_mangle]
pub unsafe extern "C" fn should_report_dio_fserror(error: c_int) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn iomap_dio_complete(dio: *mut iomap_dio) -> isize {
    ssize_t iomap_dio_complete(struct iomap_dio *dio)
    {
    const struct iomap_dio_ops *dops = dio.dops;
    struct kiocb *iocb = dio.iocb;
    let mut offset: loff_t = iocb.ki_pos;
    let mut ret: isize = dio.error;
    if (dops && dops.end_io)
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_complete_work(work: *mut work_struct) {
...
//
#[no_mangle]
pub unsafe extern "C" fn iomap_dio_set_error(dio: *mut iomap_dio, ret: c_int) {
...
//
#[no_mangle]
unsafe extern "C" fn iomap_dio_done(dio: *mut iomap_dio) {
...
#[no_mangle]
unsafe extern "C" fn __iomap_dio_bio_end_io(bio: *mut bio, inline_completion: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn iomap_dio_bio_end_io(bio: *mut bio) {
...
#[no_mangle]
pub unsafe extern "C" fn iomap_finish_ioend_direct(ioend: *mut iomap_ioend) -> u32 {
    u32 iomap_finish_ioend_direct(struct iomap_ioend *ioend)
    {
    struct iomap_dio *dio = ioend.io_bio.bi_private;
    let mut vec_count: u32 = ioend.io_bio.bi_vcnt;
    if (ioend.io_error)
...
//
    ...
    let mut nr_vecs: c_int = max(1, i_blocksize(inode) / folio_size(zero_folio));
    if (!len)
    return 0;
//
...
//
    ...
    while (len > 0) {
    let mut io_len: c_uint = min(len, folio_size(zero_folio));
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_bio_iter(iter: *mut iomap_iter, dio: *mut iomap_dio) -> c_int {
    static int iomap_dio_bio_iter(struct iomap_iter *iter, struct iomap_dio *dio)
    {
    const struct iomap *iomap = &iter.iomap;
    struct inode *inode = iter.inode;
    let mut fs_block_size: c_uint = i_blocksize(inode), pad;
    let mut length: loff_t = iomap_length(iter);
    let mut pos: loff_t = iter.pos;
    let mut bio_opf: blk_opf_t = REQ_SYNC | REQ_IDLE;
    let mut need_zeroout: bool = false;
...
//
    ...
    return -EINVAL;
    if (dio.flags & IOMAP_DIO_WRITE) {
    let mut need_completion_work: bool = true;
    switch (iomap.type) {
    case IOMAP_MAPPED:
//
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_hole_iter(iter: *mut iomap_iter, dio: *mut iomap_dio) -> c_int {
    static int iomap_dio_hole_iter(struct iomap_iter *iter, struct iomap_dio *dio)
    {
    let mut length: loff_t = iov_iter_zero(iomap_length(iter), dio.submit.iter);
    dio.size += length;
    if (!length)
    return -EFAULT;
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_inline_iter(iomi: *mut iomap_iter, dio: *mut iomap_dio) -> c_int {
    ...
    return -EIO;
    if (dio.flags & IOMAP_DIO_WRITE) {
    let mut size: loff_t = iomi.inode.i_size;
    if (pos > size)
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_iter(iter: *mut iomap_iter, dio: *mut iomap_dio) -> c_int {
...
//
    ...
    dio.flags &= ~IOMAP_DIO_NEED_SYNC;
#[no_mangle]
pub unsafe extern "C" fn if(IOMAP_DIO_NEED_SYNC: dio->flags &) -> else {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct iomap_dio_simple {
    pub iocb: *mut kiocb,
    pub size: usize,
    pub dio_flags: c_uint,
    pub work: work_struct,
//
...
//
    pub ____cacheline_aligned_in_smp: bio bio,
}

    static struct bio_set iomap_dio_simple_pool;
#[no_mangle]
unsafe extern "C" fn iomap_dio_simple_complete(sr: *mut iomap_dio_simple) -> isize {
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_simple_complete_work(work: *mut work_struct) {
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_simple_end_io(bio: *mut bio) {
    ...
    struct iomap_iter *iomi)
    {
    let mut gfp: gfp_t = (iomi.flags & IOMAP_NOWAIT) ? GFP_NOWAIT : GFP_KERNEL;
    struct iomap_dio_simple *sr;
    unsigned int alignment;
    struct bio *bio;
    ssize_t ret;
...
#[no_mangle]
unsafe extern "C" fn iomap_dio_init() -> int __init {
