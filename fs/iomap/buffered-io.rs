...
pub struct iomap_folio_state {
    ...
    pub read_bytes_pending: c_uint,
    pub write_bytes_pending: core::sync::atomic::AtomicI32,
//
...
//
    ...
    struct iomap_folio_state *ifs = folio.private;
    unsigned long flags;
    let mut mark_uptodate: bool = true;
...
#[no_mangle]
pub unsafe extern "C" fn iomap_folio_mark_uptodate(folio: *mut folio) {
...
//
    ...
    struct iomap_folio_state *ifs = folio.private;
    struct inode *inode = folio.mapping.host;
    let mut blks: c_uint = i_blocks_per_folio(inode, folio);
...
//
    ...
    {
    struct inode *inode = folio.mapping.host;
    let mut blks_per_folio: c_uint = i_blocks_per_folio(inode, folio);
    unsigned int first_blk = round_up(off, i_blocksize(inode)) >>
    inode.i_blkbits;
    let mut last_blk: c_uint = (off + len) >> inode.i_blkbits;
    unsigned long flags;
...
#[no_mangle]
unsafe extern "C" fn iomap_clear_range_dirty(folio: *mut folio, off: usize, len: usize) {
...
#[no_mangle]
unsafe extern "C" fn iomap_set_range_dirty(folio: *mut folio, off: usize, len: usize) {
    ...
    {
    struct iomap_folio_state *ifs = folio.private;
    let mut nr_blocks: c_uint = i_blocks_per_folio(inode, folio);
    gfp_t gfp;
    if (ifs || nr_blocks <= 1)
    return ifs;
...
#[no_mangle]
unsafe extern "C" fn ifs_free(folio: *mut folio) {
...
//
    ...
    {
    let mut block_size: unsigned = 1 << block_bits;
    let mut block_offset: unsigned = end_pos & (block_size - 1);
    if (!block_offset)
...
//
    ...
    {
    struct iomap_folio_state *ifs = folio.private;
    let mut orig_pos: loff_t = *pos;
    let mut isize: loff_t = i_size_read(inode);
    let mut block_bits: unsigned = inode.i_blkbits;
    let mut block_size: unsigned = (1 << block_bits);
    let mut poff: usize = offset_in_folio(folio, *pos);
    let mut plen: usize = min_t(loff_t, folio_size(folio) - poff, length);
    let mut orig_plen: usize = plen;
    let mut first: unsigned = poff >> block_bits;
    let mut last: unsigned = (poff + plen - 1) >> block_bits;
//
...
//
    ...
    if (blocks_skipped) {
    let mut block_offset: c_ulong = *pos & (block_size - 1);
...
//
    ...
    let mut end: unsigned = offset_in_folio(folio, isize - 1) >> block_bits;
...
//
    ...
    let mut size: usize = i_size_read(iter.inode) - iomap.offset;
    let mut offset: usize = offset_in_folio(folio, iomap.offset);
...
#[no_mangle]
unsafe extern "C" fn iomap_read_init(folio: *mut folio) {
...
//
#[no_mangle]
unsafe extern "C" fn iomap_read_end(folio: *mut folio, bytes_submitted: usize) {
...
//
    ...
    let mut folio_len: usize = folio_size(folio);
    struct iomap_folio_state *ifs;
    size_t poff, plen;
    loff_t pos_diff;
    int ret;
...
//
    ...
    ctx.cur_folio = core::ptr::null_mut();
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn iomap_is_partially_uptodate(folio: *mut folio, from: usize, count: usize) -> bool {
...
//
    ...
    {
    let mut fgp: fgf_t = FGP_WRITEBEGIN;
...
#[no_mangle]
pub unsafe extern "C" fn iomap_release_folio(folio: *mut folio, gfp_flags: gfp_t) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn iomap_invalidate_folio(folio: *mut folio, offset: usize, len: usize) {
...
#[no_mangle]
pub unsafe extern "C" fn iomap_dirty_folio(mapping: *mut address_space, folio: *mut folio) -> bool {
...
//
    ...
    struct iomap_folio_state *ifs;
    let mut pos: loff_t = iter.pos;
    let mut block_size: loff_t = i_blocksize(iter.inode);
    let mut block_start: loff_t = round_down(pos, block_size);
    let mut block_end: loff_t = round_up(pos + len, block_size);
    let mut nr_blocks: c_uint = i_blocks_per_folio(iter.inode, folio);
    let mut from: usize = offset_in_folio(folio, pos), to = from + len;
    size_t poff, plen;
//
...
//
    ...
    const struct iomap *srcmap = iomap_iter_srcmap(iter);
    loff_t pos;
    let mut len: u64 = min_t(u64, SIZE_MAX, iomap_length(iter));
    struct folio *folio;
...
#[no_mangle]
pub unsafe extern "C" fn if(IOMAP_F_BUFFER_HEAD: srcmap->flags &) -> else {
    else if (srcmap.flags & IOMAP_F_BUFFER_HEAD)
    status = __block_write_begin_int(folio, pos, len, core::ptr::null_mut(), srcmap);
    else
...
//
    ...
    let mut bh_written: return = = copied;
    }
    ...
    let mut bdp_flags: c_uint = (iter.flags & IOMAP_NOWAIT) ? BDP_ASYNC : 0;
    do {
    struct folio *folio;
    loff_t old_size;
...
//
    ...
    ret = iomap_file_buffered_write(&iocb, &iiter, ops, write_ops, core::ptr::null_mut());
    if (ret < 0)
    return ret;
    let mut ret: return = = length ? 0 : -EIO;
    }
...
//
    ...
    iomap_punch_t punch)
    {
    let mut punch_start_byte: loff_t = start_byte;
    let mut scan_end_byte: loff_t = min(i_size_read(inode), end_byte);
//
...
// The next data search starts at the end of this one.
    ...
    let mut bytes: u64 = iomap_length(iter);
    int status;
...
//
    ...
    {
    struct address_space	*mapping = iter.inode.i_mapping;
    let mut pstart: pgoff_t = *start >> PAGE_SHIFT;
    let mut pend: pgoff_t = (end - 1) >> PAGE_SHIFT;
    unsigned int		count;
    if (!iter.fbatch) {
// start = end;
    return 0;
...
// Block boundary? Nothing to do
    ...
    ret = __block_write_begin_int(folio, iter.pos, length, core::ptr::null_mut(),
    &iter.iomap);
    if (ret)
    return ret;
...
#[no_mangle]
unsafe extern "C" fn iomap_writeback_init(inode: *mut inode, folio: *mut folio) {
...
//
    ...
    let mut isize: u64 = i_size_read(inode);
    if (wpc.iomap.flags & IOMAP_F_FSVERITY) {
    WARN_ON_ONCE(folio_pos(folio) < isize);
    return true;
    }
    if (*end_pos > isize) {
    let mut poff: usize = offset_in_folio(folio, isize);
    let mut end_index: pgoff_t = isize >> PAGE_SHIFT;
//
...
#[no_mangle]
pub unsafe extern "C" fn iomap_writeback_folio(wpc: *mut iomap_writepage_ctx, folio: *mut folio) -> c_int {
    int iomap_writeback_folio(struct iomap_writepage_ctx *wpc, struct folio *folio)
    {
    struct iomap_folio_state *ifs = folio.private;
    struct inode *inode = wpc.inode;
    let mut pos: u64 = folio_pos(folio);
    let mut end_pos: u64 = pos + folio_size(folio);
    let mut end_aligned: u64 = 0;
    let mut orig_pos: loff_t = pos;
    let mut bytes_submitted: usize = 0;
