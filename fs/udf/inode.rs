...
pub const EXTENT_MERGE_SIZE: c_int = 5;
...
#[no_mangle]
unsafe extern "C" fn __udf_clear_extent_cache(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn udf_clear_extent_cache(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn udf_evict_inode(inode: *mut inode) {
    void udf_evict_inode(struct inode *inode)
    {
    struct udf_inode_info *iinfo = UDF_I(inode);
    let mut want_delete: c_int = 0;
    ...
    clear_inode(inode);
    kfree(iinfo.i_data);
    iinfo.i_data = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn udf_write_failed(mapping: *mut address_space, to: loff_t) {
    static void udf_write_failed(struct address_space *mapping, loff_t to)
    {
    struct inode *inode = mapping.host;
    struct udf_inode_info *iinfo = UDF_I(inode);
    let mut isize: loff_t = inode.i_size;
    if (to > isize) {
...
#[no_mangle]
unsafe extern "C" fn udf_adinicb_read_folio(folio: *mut folio) {
...
#[no_mangle]
unsafe extern "C" fn udf_read_folio(file: *mut file, folio: *mut folio) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn udf_readahead(rac: *mut readahead_control) {
...
#[no_mangle]
unsafe extern "C" fn udf_direct_IO(iocb: *mut kiocb, iter: *mut iov_iter) -> isize {
...
#[no_mangle]
unsafe extern "C" fn udf_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
...
pub const UDF_MAP_CREATE: c_uint = 0x01	/* Mapping can allocate new blocks */;
pub const UDF_MAP_NOPREALLOC: c_uint = 0x02	/* Do not preallocate blocks */;
pub const UDF_BLK_MAPPED: c_uint = 0x01	/* Block was successfully mapped */;
pub const UDF_BLK_NEW: c_uint = 0x02	/* Block was freshly allocated */;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct udf_map_rq {
    pub lblk: sector_t,
    pub pblk: udf_pblk_t,
    pub /: *mut *mut int iflags; / UDF_MAP_ flags determining behavior,
    pub /: *mut *mut int oflags; / UDF_BLK_ flags reporting results,
}

#[no_mangle]
unsafe extern "C" fn udf_map_block(inode: *mut inode, map: *mut udf_map_rq) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn udf_expand_file_adinicb(inode: *mut inode) -> c_int {
...
// Restore everything back so that we don't lose data...
    ...
    struct buffer_head *bh_result, int create)
    {
    let mut flags: c_int = create ? UDF_MAP_CREATE : 0;
//
...
//
    ...
    {
    uint32_t add;
    let mut count: c_int = 0, fake = !(last_ext.extLength & UDF_EXTENT_LENGTH_MASK);
...
#[no_mangle]
unsafe extern "C" fn udf_extend_file(inode: *mut inode, newsize: loff_t) -> c_int {
    ...
    int8_t etype;
    struct super_block *sb = inode.i_sb;
    let mut first_block: sector_t = newsize >> sb.s_blocksize_bits, offset;
    loff_t new_elen;
    int adsize;
    struct udf_inode_info *iinfo = UDF_I(inode);
...
#[no_mangle]
unsafe extern "C" fn inode_getblk(inode: *mut inode, map: *mut udf_map_rq) -> c_int {
    static int inode_getblk(struct inode *inode, struct udf_map_rq *map)
    {
    struct kernel_long_ad laarr[EXTENT_MERGE_SIZE];
    struct extent_position prev_epos, cur_epos, next_epos;
    let mut count: c_int = 0, startnum = 0, endnum = 0;
    let mut elen: u32 = 0, tmpelen;
    struct kernel_lb_addr eloc, tmpeloc;
    let mut c: c_int = 1;
    let mut lbcount: loff_t = 0, b_off = 0;
    udf_pblk_t newblocknum;
    let mut offset: sector_t = 0;
    int8_t etype, tmpetype;
    struct udf_inode_info *iinfo = UDF_I(inode);
    let mut goal: udf_pblk_t = 0, pgoal = 0;
    let mut lastblock: c_int = 0;
    let mut isBeyondEOF: bool = false;
    let mut ret: c_int = 0;
    prev_epos.offset = udf_file_entry_alloc_offset(inode);
    prev_epos.block = iinfo.i_location;
    prev_epos.bh = core::ptr::null_mut();
...
// the new number of extents is less than the old number
    ...
    let mut blocksize_bits: c_uchar = inode.i_sb.s_blocksize_bits;
    if ((laarr[*c].extLength >> 30) == (EXT_NOT_RECORDED_ALLOCATED >> 30) ||
    (laarr[*c].extLength >> 30) ==
    (EXT_NOT_RECORDED_NOT_ALLOCATED >> 30)) {
    let mut curr: c_int = *c;
    int blen = ((laarr[curr].extLength & UDF_EXTENT_LENGTH_MASK) +
    blocksize - 1) >> blocksize_bits;
    let mut etype: i8 = (laarr[curr].extLength >> 30);
    if (blen == 1)
    ;
#[no_mangle]
pub unsafe extern "C" fn if(1: !offset || blen == offset +) -> else {
    ...
    udf_delete_aext(inode, *epos, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn udf_setsize(inode: *mut inode, newsize: loff_t) -> c_int {
...
//
pub const UDF_MAX_ICB_NESTING: c_int = 1024;
#[no_mangle]
unsafe extern "C" fn udf_read_inode(inode: *mut inode, hidden_inode: bool) -> c_int {
    ...
    let mut bs: c_int = inode.i_sb.s_blocksize;
...
#[no_mangle]
unsafe extern "C" fn udf_alloc_i_data(inode: *mut inode, size: usize) -> c_int {
    static int udf_alloc_i_data(struct inode *inode, size_t size)
    {
    struct udf_inode_info *iinfo = UDF_I(inode);
...
#[no_mangle]
unsafe extern "C" fn udf_convert_permissions(fe: *mut fileEntry) -> umode_t {
...
#[no_mangle]
pub unsafe extern "C" fn udf_update_extra_perms(inode: *mut inode, mode: umode_t) {
...
#[no_mangle]
pub unsafe extern "C" fn udf_sync_inode_metadata(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn udf_adjust_time(iinfo: *mut udf_inode_info, time: timespec64) {
...
#[no_mangle]
pub unsafe extern "C" fn udf_write_inode(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISREG(inode->i_mode)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISBLK(inode->i_mode)) -> else {
    else if (S_ISBLK(inode.i_mode))
    fe.icbTag.fileType = ICBTAG_FILE_TYPE_BLOCK;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISCHR(inode->i_mode)) -> else {
    else if (S_ISCHR(inode.i_mode))
    fe.icbTag.fileType = ICBTAG_FILE_TYPE_CHAR;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISFIFO(inode->i_mode)) -> else {
    else if (S_ISFIFO(inode.i_mode))
    fe.icbTag.fileType = ICBTAG_FILE_TYPE_FIFO;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISSOCK(inode->i_mode)) -> else {
...
// write the data blocks
    ...
    bool hidden_inode)
    {
    let mut block: c_ulong = udf_get_lb_pblock(sb, ino, 0);
    ...
    adsize = sizeof(struct short_ad);
#[no_mangle]
pub unsafe extern "C" fn if(ICBTAG_FLAG_AD_LONG: UDF_I(inode)->i_alloc_type ==) -> else {
...
pub unsafe extern "C" fn if(ICBTAG_FLAG_AD_LONG: UDF_I(inode)->i_alloc_type ==) -> else {
    ...
    new_block = udf_new_block(sb, core::ptr::null_mut(),
...
//
pub const UDF_MAX_INDIR_EXTS: c_int = 16;
//
...
//
    ...
    {
    let mut blocksize_bits: c_uchar = inode.i_sb.s_blocksize_bits;
    let mut lbcount: loff_t = 0, bcount = (loff_t) block << blocksize_bits;
    struct udf_inode_info *iinfo;
    let mut err: c_int = 0;
    iinfo = UDF_I(inode);
