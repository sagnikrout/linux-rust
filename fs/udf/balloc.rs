...
#[no_mangle]
unsafe extern "C" fn udf_add_free_space(sb: *mut super_block, partition: u16, cnt: u32) {
...
// err = -EIO;
    ...
    epos.bh = oepos.bh = core::ptr::null_mut();
    while (count) {
...
// AED block freed by udf_delete_aext(), released after unlock
    let mut freed: kernel_lb_addr = { .partitionReferenceNum = 0xFFFF };
...
// AED block freed by udf_delete_aext(), released after unlock
    let mut freed: kernel_lb_addr = { .partitionReferenceNum = 0xFFFF };
    let mut spread: u32 = 0xFFFFFFFF, nspread = 0xFFFFFFFF;
    let mut newblock: udf_pblk_t = 0;
    uint32_t adsize;
...
//
    epos.offset = sizeof(struct unallocSpaceEntry);
    epos.block = iinfo.i_location;
    epos.bh = goal_epos.bh = core::ptr::null_mut();
    while (spread) {
    ret = udf_next_aext(table, &epos, &eloc, &elen, &etype, 1);
    if (ret <= 0)
    break;
...
// err = 0;
    ...
    uint32_t count)
    {
    let mut partition: u16 = bloc.partitionReferenceNum;
    ...
    block_count);
#[no_mangle]
pub unsafe extern "C" fn if(UDF_PART_FLAG_UNALLOC_TABLE: map->s_partition_flags &) -> else {
    else if (map.s_partition_flags & UDF_PART_FLAG_UNALLOC_TABLE)
