...
//
    let mut autofs_next_wait_queue: static autofs_wqt_t = 1;
#[no_mangle]
pub unsafe extern "C" fn autofs_catatonic_mode(sbi: *mut autofs_sb_info) {
    ...
    wq = sbi.queues;
    sbi.queues = core::ptr::null_mut();	/* Erase all wait queues */
    while (wq) {
    nwq = wq.next;
    ...
    while (bytes) {
    wr = __kernel_write(file, data, bytes, core::ptr::null_mut());
    if (wr <= 0)
    break;
    data += wr;
    bytes -= wr;
...
// if 'wr' returned 0 (impossible) we assume -EIO (safe)
    let mut bytes: return = = 0 ? 0 : wr < 0 ? wr : -EIO;
    }
    ...
    struct file *pipe = core::ptr::null_mut();
    size_t pktsz;
    int ret;
...
//
    if (notify == NFY_MOUNT) {
    struct dentry *new = core::ptr::null_mut();
    struct path this;
...
//
    ...
    return -ENOENT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !IS_ROOT(dentry->d_parent)) -> else {
...
//
    wait_event_killable(wq.queue, wq.name.name == core::ptr::null_mut());
    status = wq.status;
//
...
// Are we the last process to need status?
    ...
    mutex_lock(&sbi.wq_mutex);
    for (wql = &sbi.queues; (wq = *wql) != core::ptr::null_mut(); wql = &wq.next) {
...
// wql = wq->next;	/* Unlink from chain
    kfree(wq.name.name - wq.offset);
    wq.name.name = core::ptr::null_mut();	/* Do not wait on this queue */
    wq.status = status;
    wake_up(&wq.queue);
    if (!--wq.wait_ctr)
    kfree(wq);
