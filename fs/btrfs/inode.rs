...
//
    ...
    struct inode_fs_paths *ipath __free(inode_fs_paths) = core::ptr::null_mut();
...
// This makes the path point to (inum INODE_ITEM ioff).
    ...
    key.offset = 0;
    ret = btrfs_search_slot(core::ptr::null_mut(), local_root, &key, &warn.path, 0, 0);
    if (ret) {
...
//
    ...
    let mut found_key: btrfs_key = { 0 };
    struct extent_buffer *eb;
    struct btrfs_extent_item *ei;
    let mut csum_size: u32 = fs_info.csum_size;
    u64 logical;
    u64 flags;
    u32 item_size;
    int ret;
    ...
    let mut reloc_warn: data_reloc_warn = { 0 };
//
...
//
    ...
    let mut csum_size: u32 = root.fs_info.csum_size;
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_inode_lock(inode: *mut btrfs_inode, ilock_flags: c_uint) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_inode_unlock(inode: *mut btrfs_inode, ilock_flags: c_uint) {
...
//
    ...
    let mut sectorsize: u32 = trans.fs_info.sectorsize;
    char *kaddr;
    unsigned long ptr;
    struct btrfs_file_extent_item *ei;
    int ret;
    let mut cur_size: usize = size;
    u64 i_size;
//
...
//
    ...
    {
    struct btrfs_fs_info *fs_info = inode.root.fs_info;
    let mut data_len: u64 = (compressed_size ?: size);
...
//
    ...
    bool update_i_size)
    {
    let mut drop_args: btrfs_drop_extents_args = { 0 };
    struct btrfs_root *root = inode.root;
    struct btrfs_fs_info *fs_info = root.fs_info;
    struct btrfs_trans_handle *trans = core::ptr::null_mut();
    let mut data_len: u64 = (compressed_size ?: size);
    int ret;
    struct btrfs_path *path;
...
//
    if (ret <= 0)
    btrfs_qgroup_free_data(inode, core::ptr::null_mut(), 0, fs_info.sectorsize, core::ptr::null_mut());
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct async_extent {
    pub start: u64,
    pub ram_size: u64,
    pub cb: *mut compressed_bio,
    pub list: list_head,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct async_chunk {
    pub inode: *mut btrfs_inode,
    pub locked_folio: *mut folio,
    pub start: u64,
    pub end: u64,
    pub write_flags: blk_opf_t,
    pub extents: list_head,
    pub blkcg_css: *mut cgroup_subsys_state,
    pub work: btrfs_work,
    pub async_cow: *mut async_cow,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct async_cow {
    pub num_chunks: core::sync::atomic::AtomicI32,
    pub chunks: [async_chunk; ],
}
...
#[no_mangle]
unsafe extern "C" fn extent_range_clear_dirty_for_io(inode: *mut btrfs_inode, start: u64, end: u64) -> c_int {
    ...
    let mut end_index: pgoff_t = end >> PAGE_SHIFT;
    struct folio *folio;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn round_up_last_block(cb: *mut compressed_bio, blocksize: u32) {
    ...
    let mut bio_size: u32 = bio.bi_iter.bi_size;
    let mut foffset: u32 = offset_in_folio(last_folio, bio_size);
    let mut padding_len: u32 = round_up(foffset, blocksize) - foffset;
    bool ret;
...
//
#[no_mangle]
unsafe extern "C" fn compress_file_range(work: *mut btrfs_work) {
    ...
    struct btrfs_fs_info *fs_info = inode.root.fs_info;
    struct compressed_bio *cb = core::ptr::null_mut();
    let mut blocksize: u32 = fs_info.sectorsize;
    let mut start: u64 = async_chunk.start;
    let mut end: u64 = async_chunk.end;
    u64 actual_end;
    u64 i_size;
    u32 cur_len;
    let mut ret: c_int = 0;
    let mut total_compressed: c_ulong = 0;
    let mut total_in: c_ulong = 0;
    let mut compress_type: c_int = fs_info.compress_type;
    let mut compress_level: c_int = fs_info.compress_level;
...
//
    ...
    ret = add_async_extent(async_chunk, start, end - start + 1, core::ptr::null_mut());
    BUG_ON(ret);
    if (cb)
    ...
    {
    let mut start: u64 = async_extent.start;
    let mut end: u64 = async_extent.start + async_extent.ram_size - 1;
    int ret;
    ...
    struct folio *locked_folio = core::ptr::null_mut();
    struct extent_state *cached = core::ptr::null_mut();
...
//
    ...
    let mut locked_folio_start: u64 = folio_pos(async_chunk.locked_folio);
...
//
    ...
    async_extent.cb = core::ptr::null_mut();
    goto done;
    }
...
// Clear dirty, set writeback and unlock the pages.
    ...
    core::ptr::null_mut(), &cached, EXTENT_LOCKED | EXTENT_DELALLOC,
...
// alloc_hint = ins.objectid + ins.offset;
    ...
    if (async_chunk.blkcg_css)
    kthread_associate_blkcg(core::ptr::null_mut());
    kfree(async_extent);
    return;
    out_free_reserve:
    btrfs_dec_block_group_reservations(fs_info, ins.objectid);
...
//
    ...
    PAGE_UNLOCK | PAGE_START_WRITEBACK |
    PAGE_END_WRITEBACK);
    btrfs_qgroup_free_data(inode, core::ptr::null_mut(), file_offset, cur_len, core::ptr::null_mut());
    btrfs_dec_block_group_reservations(fs_info, ins.objectid);
    btrfs_free_reserved_extent(fs_info, ins.objectid, ins.offset, true);
    mapping_set_error(inode.vfs_inode.i_mapping, -EIO);
// ret_alloc_size = cur_len;
...
//
    ...
    struct extent_state *cached = core::ptr::null_mut();
    let mut alloc_hint: u64 = 0;
    let mut orig_start: u64 = start;
    u64 num_bytes;
    u32 min_alloc_size;
    let mut blocksize: u32 = fs_info.sectorsize;
    let mut cur_alloc_size: u32 = 0;
    struct btrfs_key ins;
    unsigned clear_bits;
...
//
    ...
    locked_folio, core::ptr::null_mut(), clear_bits, page_ops);
    }
...
//
    ...
    btrfs_qgroup_free_data(inode, core::ptr::null_mut(), start + cur_alloc_size,
    end - start - cur_alloc_size + 1, core::ptr::null_mut());
    }
    btrfs_err(fs_info,
...
//
#[no_mangle]
unsafe extern "C" fn submit_compressed_extents(work: *mut btrfs_work, do_free: bool) -> noinline void {
...
// atomic_sub_return implies a barrier
    ...
    struct async_chunk *async_chunk;
    unsigned long nr_pages;
    let mut num_chunks: u64 = DIV_ROUND_UP(end - start, BTRFS_COMPRESSION_CHUNK_SIZE);
    int i;
    unsigned nofs_flag;
    let mut write_flags: blk_opf_t = wbc_to_write_flags(wbc);
    nofs_flag = memalloc_nofs_save();
    ...
    let mut cur_end: u64 = min(end, start + BTRFS_COMPRESSION_CHUNK_SIZE - 1);
//
...
//
    ...
    locked_folio = core::ptr::null_mut();
    } else {
    async_chunk[i].locked_folio = core::ptr::null_mut();
    }
    ...
    async_chunk[i].blkcg_css = core::ptr::null_mut();
    }
...
//
    ...
    const u64 end)
    {
    let mut is_space_ino: bool = btrfs_is_free_space_inode(inode);
    let mut is_reloc_ino: bool = btrfs_is_data_reloc_root(inode.root);
    let mut range_bytes: u64 = end + 1 - start;
    struct extent_io_tree *io_tree = &inode.io_tree;
    struct extent_state *cached_state = core::ptr::null_mut();
    let mut range_start: u64 = start;
    u64 count;
    int ret;
//
...
//
    ...
    EXTENT_NORESERVE, false, core::ptr::null_mut());
    if (count > 0 || is_space_ino || is_reloc_ino) {
    let mut bytes: u64 = count;
    struct btrfs_fs_info *fs_info = inode.root.fs_info;
...
//
    ret = cow_file_range(inode, locked_folio, start, end, core::ptr::null_mut(),
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct can_nocow_file_extent_args {
// Input fields.
// Start file offset of the range we want to NOCOW.
    pub start: u64,
// End file offset (inclusive) of the range we want to NOCOW.
    pub end: u64,
    pub writeback_path: bool,
//
...
//
    pub file_extent: btrfs_file_extent,
}
...
//
    ...
    {
    let mut is_freespace_inode: bool = btrfs_is_free_space_inode(inode);
    ...
    u64 extent_end;
    u8 extent_type;
    let mut can_nocow: c_int = 0;
    let mut ret: c_int = 0;
    let mut nowait: bool = path.nowait;
...
//
    ...
    {
    struct btrfs_ordered_extent *ordered;
    let mut len: u64 = nocow_args.file_extent.num_bytes;
    let mut end: u64 = file_pos + len - 1;
    let mut ret: c_int = 0;
...
//
    let mut cow_end: u64 = 0;
    let mut nocow_end: u64 = 0;
    let mut cur_offset: u64 = start;
    int ret;
    let mut check_prev: bool = true;
    let mut ino: u64 = btrfs_ino(inode);
    let mut nocow_args: can_nocow_file_extent_args = { 0 };
// The range that has ordered extent(s).
    u64 oe_cleanup_start;
    let mut oe_cleanup_len: u64 = 0;
// The range that is untouched.
    u64 untouched_start;
    let mut untouched_len: u64 = 0;
//
...
//
    ...
    struct btrfs_block_group *nocow_bg = core::ptr::null_mut();
    ...
    u64 extent_end;
    int extent_type;
    ret = btrfs_lookup_file_extent(core::ptr::null_mut(), root, path, ino,
    cur_offset, 0);
    if (ret < 0)
    goto error;
//
...
//
    extent_clear_unlock_delalloc(inode, start, end, locked_folio, core::ptr::null_mut(), 0, PAGE_UNLOCK);
    btrfs_free_path(path);
    return 0;
    error:
    if (cow_start == (u64)-1) {
...
//
    ...
    if (oe_cleanup_len) {
    let mut oe_cleanup_end: u64 = oe_cleanup_start + oe_cleanup_len - 1;
    ...
    if (untouched_len) {
    struct extent_state *cached = core::ptr::null_mut();
    let mut untouched_end: u64 = untouched_start + untouched_len - 1;
//
...
//
    ...
    PAGE_END_WRITEBACK);
    btrfs_qgroup_free_data(inode, core::ptr::null_mut(), untouched_start, untouched_len, core::ptr::null_mut());
    }
    btrfs_free_path(path);
    btrfs_err(fs_info,
...
#[no_mangle]
unsafe extern "C" fn should_nocow(inode: *mut btrfs_inode, start: u64, end: u64) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn run_delalloc_inline(inode: *mut btrfs_inode, locked_folio: *mut folio) -> c_int {
    static int run_delalloc_inline(struct btrfs_inode *inode, struct folio *locked_folio)
    {
    struct btrfs_fs_info *fs_info = inode.root.fs_info;
    struct compressed_bio *cb = core::ptr::null_mut();
    struct extent_state *cached = core::ptr::null_mut();
    let mut i_size: u64 = i_size_read(&inode.vfs_inode);
    let mut blocksize: u32 = fs_info.sectorsize;
    let mut compress_type: c_int = fs_info.compress_type;
    let mut compress_level: c_int = fs_info.compress_level;
...
//
    ...
    {
    let mut zoned: bool = btrfs_is_zoned(inode.root.fs_info);
//
...
//
    ...
    return cow_file_range(inode, locked_folio, start, end, core::ptr::null_mut(), 0);
    }
...
#[no_mangle]
unsafe extern "C" fn btrfs_add_delalloc_inode(inode: *mut btrfs_inode) {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_del_delalloc_inode(inode: *mut btrfs_inode) {
...
//
    ...
    let mut len: u64 = state.end + 1 - state.start;
    u64 prev_delalloc_bytes;
    let mut num_extents: u32 = count_max_extents(fs_info, len);
    spin_lock(&inode.lock);
...
//
    ...
    struct btrfs_root *csum_root = core::ptr::null_mut();
    int ret;
    ...
    {
    let mut search_start: u64 = start;
    let mut end: u64 = start + len - 1;
    while (search_start < end) {
    let mut search_len: u64 = end - search_start + 1;
    struct extent_map *em;
    u64 em_len;
    let mut ret: c_int = 0;
    em = btrfs_get_extent(inode, core::ptr::null_mut(), search_start, search_len);
    if (IS_ERR(em))
    return PTR_ERR(em);
    ...
    struct extent_state **cached_state)
    {
    let mut blocksize: u32 = inode.root.fs_info.sectorsize;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct btrfs_writepage_fixup {
    pub folio: *mut folio,
    pub inode: *mut btrfs_inode,
...
//
#[no_mangle]
unsafe extern "C" fn btrfs_writepage_fixup_worker(work: *mut work_struct) {
    static void btrfs_writepage_fixup_worker(struct work_struct *work)
    {
    struct btrfs_writepage_fixup *fixup =
    container_of(work, struct btrfs_writepage_fixup, work);
    struct extent_state *cached_state = core::ptr::null_mut();
    struct extent_changeset *data_reserved = core::ptr::null_mut();
    unsigned long delalloc_bitmap[BITS_TO_LONGS(BTRFS_MAX_BLOCKS_PER_FOLIO)] = { 0 };
    struct folio *folio = fixup.folio;
    struct btrfs_inode *inode = fixup.inode;
    struct btrfs_fs_info *fs_info = inode.root.fs_info;
    let mut blocks_per_folio: c_uint = btrfs_blocks_per_folio(fs_info, folio);
    let mut sectorsize: u32 = fs_info.sectorsize;
    let mut page_start: u64 = folio_pos(folio);
    let mut page_end: u64 = folio_next_pos(folio) - 1;
...
//
    ...
    struct btrfs_ordered_extent *ordered;
    let mut start: u64 = page_start + (bit << fs_info.sectorsize_bits);
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_queue_writepage_fixup(inode: *mut btrfs_inode, folio: *mut folio) {
...
// Clear the old accounting as the range may already be dirty.
    ...
    {
    struct btrfs_root *root = inode.root;
    let mut sectorsize: u32 = root.fs_info.sectorsize;
    BTRFS_PATH_AUTO_FREE(path);
    struct extent_buffer *leaf;
    struct btrfs_key ins;
    let mut disk_num_bytes: u64 = btrfs_stack_file_extent_disk_num_bytes(stack_fi);
    let mut disk_bytenr: u64 = btrfs_stack_file_extent_disk_bytenr(stack_fi);
    let mut offset: u64 = btrfs_stack_file_extent_offset(stack_fi);
    let mut num_bytes: u64 = btrfs_stack_file_extent_num_bytes(stack_fi);
    let mut ram_bytes: u64 = btrfs_stack_file_extent_ram_bytes(stack_fi);
    let mut drop_args: btrfs_drop_extents_args = { 0 };
    int ret;
    path = btrfs_alloc_path();
    if (!path)
...
//
    ...
    let mut inline_size: u64 = round_down(drop_args.bytes_found, sectorsize);
    ...
    let mut num_bytes: u64 = oe.num_bytes;
    let mut ram_bytes: u64 = oe.ram_bytes;
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_finish_one_ordered(ordered_extent: *mut btrfs_ordered_extent) -> c_int {
    ...
    struct extent_state *cached_state = core::ptr::null_mut();
    u64 start, end;
    let mut compress_type: c_int = 0;
    let mut ret: c_int = 0;
    let mut logical_len: u64 = ordered_extent.num_bytes;
    bool freespace_inode;
    let mut truncated: bool = false;
    let mut clear_reserved_extent: bool = true;
    let mut clear_bits: c_uint = 0;
...
//
    if (!btrfs_is_free_space_inode(inode)) {
    let mut unwritten_start: u64 = start;
    if (truncated)
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_finish_ordered_io(ordered: *mut btrfs_ordered_extent) -> c_int {
...
//
    ...
    let mut blocksize: u32 = fs_info.sectorsize;
    let mut step: u32 = min(blocksize, PAGE_SIZE);
    let mut nr_steps: u32 = blocksize / step;
    phys_addr_t paddrs[BTRFS_MAX_BLOCKSIZE / PAGE_SIZE];
// The full block must be inside the folio.
    ASSERT(offset_in_folio(folio, paddr) + blocksize <= folio_size(folio));
    for (int i = 0; i < nr_steps; i++) {
    let mut pindex: u32 = offset_in_folio(folio, paddr + i * step) >> PAGE_SHIFT;
//
...
//
    ...
    for (int i = 0; i < nr_steps; i++) {
    let mut paddr: phys_addr_t = paddrs[i];
    void *kaddr;
...
//
    ...
    let mut step: u32 = min(blocksize, PAGE_SIZE);
    let mut nr_steps: u32 = blocksize / step;
    let mut file_offset: u64 = bbio.file_offset + bio_offset;
    let mut end: u64 = file_offset + blocksize - 1;
    u8 *csum_expected;
...
// Skip the range without csum for data reloc inode
    ...
    EXTENT_NODATASUM, core::ptr::null_mut());
    return true;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_add_delayed_iput(inode: *mut btrfs_inode) {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_run_delayed_iputs(fs_info: *mut btrfs_fs_info) {
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_wait_on_delayed_iputs(fs_info: *mut btrfs_fs_info) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_orphan_cleanup(root: *mut btrfs_root) -> c_int {
    ...
    let mut last_objectid: u64 = 0;
    let mut ret: c_int = 0, nr_unlink = 0;
...
//
    ...
    let mut is_dead_root: bool = false;
//
...
//
    ...
    {
    let mut nritems: u32 = btrfs_header_nritems(leaf);
    struct btrfs_key found_key;
    let mut xattr_access: static u64 = 0;
    let mut xattr_default: static u64 = 0;
...
#[no_mangle]
unsafe extern "C" fn btrfs_init_file_extent_tree(inode: *mut btrfs_inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btrfs_add_inode_to_root(inode: *mut btrfs_inode, prealloc: bool) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn btrfs_read_locked_inode(inode: *mut btrfs_inode, path: *mut btrfs_path) -> c_int {
    ...
    ret = btrfs_lookup_inode(core::ptr::null_mut(), root, path, &location, 0);
    if (ret) {
//
...
#[no_mangle]
unsafe extern "C" fn update_time_after_link_or_unlink(dir: *mut btrfs_inode) {
...
//
    ...
    u64 index;
    let mut ino: u64 = btrfs_ino(inode);
    let mut dir_ino: u64 = btrfs_ino(dir);
    path = btrfs_alloc_path();
    if (!path)
    return -ENOMEM;
...
//
    ...
    {
    int ret;
    ret = __btrfs_unlink_inode(trans, dir, inode, name, core::ptr::null_mut());
    if (!ret) {
...
#[no_mangle]
unsafe extern "C" fn btrfs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn may_destroy_subvol(root: *mut btrfs_root) -> noinline int {
    ...
    struct btrfs_dir_item *di;
    struct btrfs_key key;
    let mut name: fscrypt_str = FSTR_INIT("default", 7);
    u64 dir_id;
    int ret;
    path = btrfs_alloc_path();
    if (!path)
...
// Make sure this root isn't set as the default subvol
    ...
    di = btrfs_lookup_dir_item(core::ptr::null_mut(), fs_info.tree_root, path,
    dir_id, &name, 0);
    if (!IS_ERR_OR_NULL(di)) {
    ...
    key.offset = (u64)-1;
    ret = btrfs_search_slot(core::ptr::null_mut(), fs_info.tree_root, &key, path, 0, 0);
    if (ret < 0)
    return ret;
    if (unlikely(ret == 0)) {
//
...
#[no_mangle]
unsafe extern "C" fn btrfs_prune_dentries(root: *mut btrfs_root) {
    static void btrfs_prune_dentries(struct btrfs_root *root)
    {
    struct btrfs_fs_info *fs_info = root.fs_info;
    struct btrfs_inode *inode;
    let mut min_ino: u64 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_delete_subvolume(dir: *mut btrfs_inode, dentry: *mut dentry) -> c_int {
...
//
    ...
    dest.anon_dev = 0;
    out_end_trans:
    trans.block_rsv = core::ptr::null_mut();
    trans.bytes_reserved = 0;
    ret = btrfs_end_transaction(trans);
    inode.i_flags |= S_DEAD;
    out_release:
    btrfs_block_rsv_release(fs_info, &block_rsv, (u64)-1, core::ptr::null_mut());
    if (qgroup_reserved)
...
#[no_mangle]
unsafe extern "C" fn btrfs_rmdir(vfs_dir: *mut inode, dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_inside_block(bytenr: u64, blockstart: u64, blocksize: u32) -> bool {
...
#[no_mangle]
unsafe extern "C" fn truncate_block_zero_beyond_eof(inode: *mut btrfs_inode, start: u64) -> c_int {
...
// No folio present.
    ...
    ret = btrfs_read_folio(core::ptr::null_mut(), folio);
    folio_lock(folio);
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_truncate_block(inode: *mut btrfs_inode, offset: u64, start: u64, end: u64) -> c_int {
    ...
    struct extent_state *cached_state = core::ptr::null_mut();
    struct extent_changeset *data_reserved = core::ptr::null_mut();
    let mut only_release_metadata: bool = false;
    let mut blocksize: u32 = fs_info.sectorsize;
    let mut index: pgoff_t = (offset >> PAGE_SHIFT);
    struct folio *folio;
    let mut mask: gfp_t = btrfs_alloc_write_mask(mapping);
    let mut ret: c_int = 0;
    ...
    blocksize);
    let mut need_truncate_head: bool = false;
    let mut need_truncate_tail: bool = false;
    u64 zero_start;
    u64 zero_end;
    u64 block_start;
    u64 block_end;
...
//
    ...
    blocksize, false);
    if (ret < 0) {
    let mut write_bytes: usize = blocksize;
...
#[no_mangle]
unsafe extern "C" fn maybe_insert_hole(inode: *mut btrfs_inode, offset: u64, len: u64) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_cont_expand(inode: *mut btrfs_inode, oldsize: loff_t, size: loff_t) -> c_int {
    int btrfs_cont_expand(struct btrfs_inode *inode, loff_t oldsize, loff_t size)
    {
    struct btrfs_root *root = inode.root;
    struct btrfs_fs_info *fs_info = root.fs_info;
    struct extent_io_tree *io_tree = &inode.io_tree;
    struct extent_map *em = core::ptr::null_mut();
    struct extent_state *cached_state = core::ptr::null_mut();
    let mut hole_start: u64 = ALIGN(oldsize, fs_info.sectorsize);
    let mut block_end: u64 = ALIGN(size, fs_info.sectorsize);
    u64 last_byte;
    u64 cur_offset;
    u64 hole_size;
    let mut ret: c_int = 0;
...
//
    ...
    while (1) {
    em = btrfs_get_extent(inode, core::ptr::null_mut(), cur_offset, block_end - cur_offset);
    if (IS_ERR(em)) {
    ret = PTR_ERR(em);
    em = core::ptr::null_mut();
    break;
    }
...
#[no_mangle]
unsafe extern "C" fn btrfs_setsize(inode: *mut inode, attr: *mut iattr) -> c_int {
    static int btrfs_setsize(struct inode *inode, struct iattr *attr)
    {
    struct btrfs_root *root = BTRFS_I(inode).root;
    struct btrfs_trans_handle *trans;
    let mut oldsize: loff_t = i_size_read(inode);
    let mut newsize: loff_t = attr.ia_size;
    let mut mask: c_int = attr.ia_valid;
    int ret;
//
...
//
#[no_mangle]
unsafe extern "C" fn evict_inode_truncate_pages(inode: *mut inode) {
...
//
    ...
    btrfs_qgroup_free_data(BTRFS_I(inode), core::ptr::null_mut(), start,
    end - start + 1, core::ptr::null_mut());
    ...
    struct btrfs_fs_info *fs_info = root.fs_info;
    struct btrfs_trans_handle *trans;
    let mut delayed_refs_extra: u64 = btrfs_calc_delayed_ref_bytes(fs_info, 1);
    int ret;
//
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_evict_inode(inode: *mut inode) {
...
//
    ...
    out_release:
    btrfs_block_rsv_release(fs_info, &rsv, (u64)-1, core::ptr::null_mut());
    out:
//
...
//
    ASSERT(ret == 0);
// This needs to handle no-key deletions later on
    di = btrfs_lookup_dir_item(core::ptr::null_mut(), root, path, btrfs_ino(dir),
    &fname.disk_name, 0);
    if (IS_ERR_OR_NULL(di)) {
    ret = di ? PTR_ERR(di) : -ENOENT;
    goto out;
...
#[no_mangle]
unsafe extern "C" fn btrfs_del_inode_from_root(inode: *mut btrfs_inode) {
...
// don't really matter.
//
    entry = __xa_cmpxchg(&root.inodes, btrfs_ino(inode), inode, core::ptr::null_mut(),
    GFP_ATOMIC);
    if (entry == inode)
...
#[no_mangle]
unsafe extern "C" fn btrfs_init_locked_inode(inode: *mut inode, p: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btrfs_find_actor(inode: *mut inode, opaque: *mut c_void) -> c_int {
    ...
    let mut hashval: c_ulong = btrfs_inode_hash(ino, root);
    args.ino = ino;
    args.root = root;
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_inode_type(inode: *const btrfs_inode) -> u8 {
    ...
    let mut location: btrfs_key = { 0 };
    let mut di_type: u8 = 0;
    let mut ret: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn btrfs_dentry_delete(dentry: *const dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn btrfs_set_inode_index_count(inode: *mut btrfs_inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btrfs_get_dir_last_index(dir: *mut btrfs_inode, index: *mut u64) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn btrfs_opendir(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btrfs_dir_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
...
#[no_mangle]
unsafe extern "C" fn btrfs_filldir(addr: *mut c_void, entries: c_int, ctx: *mut dir_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btrfs_real_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn btrfs_dirty_inode(inode: *mut btrfs_inode) -> c_int {
    static int btrfs_dirty_inode(struct btrfs_inode *inode)
    {
    struct btrfs_root *root = inode.root;
    struct btrfs_fs_info *fs_info = root.fs_info;
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_set_inode_index(dir: *mut btrfs_inode, index: *mut u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn btrfs_insert_inode_locked(inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_new_inode_args_destroy(args: *mut btrfs_new_inode_args) {
...
//
#[no_mangle]
unsafe extern "C" fn btrfs_inherit_iflags(inode: *mut btrfs_inode, dir: *mut btrfs_inode) {
    ...
    struct inode *dir = args.dir;
    struct inode *inode = args.inode;
    const struct fscrypt_str *name = args.orphan ? core::ptr::null_mut() : &args.fname.disk_name;
    struct btrfs_fs_info *fs_info = inode_to_fs_info(dir);
    struct btrfs_root *root;
    struct btrfs_inode_item *inode_item;
    struct btrfs_path *path;
    ...
    unsigned long ptr;
    int ret;
    let mut xa_reserved: bool = false;
...
//
    ...
    let mut ino: u64 = btrfs_ino(inode);
    let mut parent_ino: u64 = btrfs_ino(parent_inode);
...
// Nothing to clean up yet
    ...
    goto fail_dir_item;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: unlikely(ret)) -> else {
    btrfs_abort_transaction(trans, ret);
    return ret;
    }
    ...
    int ret2;
    ret2 = btrfs_del_inode_ref(trans, root, name, ino, parent_ino, core::ptr::null_mut());
    if (ret2)
    btrfs_abort_transaction(trans, ret2);
    }
...
// Grab reference for the new dentry passed to d_instantiate().
    ...
    btrfs_log_new_name(trans, old_dentry, core::ptr::null_mut(), 0, dentry.d_parent);
    fail:
    fscrypt_free_filename(&fname);
    if (trans)
    btrfs_end_transaction(trans);
    ...
    int ret;
    struct extent_buffer *leaf = path.nodes[0];
    let mut blocksize: u32 = leaf.fs_info.sectorsize;
    char *tmp;
    size_t max_size;
...
#[no_mangle]
unsafe extern "C" fn read_inline_extent(path: *mut btrfs_path, folio: *mut folio) -> c_int {
    static int read_inline_extent(struct btrfs_path *path, struct folio *folio)
    {
    let mut blocksize: u32 = path.nodes[0].fs_info.sectorsize;
    struct btrfs_file_extent_item *fi;
    void *kaddr;
    size_t copy_size;
...
//
    ...
    let mut extent_end: u64 = 0;
    let mut objectid: u64 = btrfs_ino(inode);
    let mut extent_type: c_int = -1;
    struct btrfs_path *path = core::ptr::null_mut();
    struct btrfs_root *root = inode.root;
    struct btrfs_file_extent_item *item;
    struct extent_buffer *leaf;
    ...
    btrfs_free_extent_map(em);
#[no_mangle]
pub unsafe extern "C" fn if(folio: em->disk_bytenr == EXTENT_MAP_INLINE &&) -> else {
...
//
    ...
    ret = btrfs_lookup_file_extent(core::ptr::null_mut(), root, path, objectid, start, 0);
    if (ret < 0) {
    goto out;
    } else if (ret > 0) {
    if (path.slots[0] == 0)
...
#[no_mangle]
unsafe extern "C" fn btrfs_extent_readonly(fs_info: *mut btrfs_fs_info, bytenr: u64) -> bool {
...
//
    ...
    return -ENOMEM;
    path.nowait = nowait;
    ret = btrfs_lookup_file_extent(core::ptr::null_mut(), root, path, btrfs_ino(inode),
    offset, 0);
    if (ret < 0)
    return ret;
    if (ret == 1) {
...
//
#[no_mangle]
unsafe extern "C" fn wait_subpage_spinlock(folio: *mut folio) {
...
#[no_mangle]
unsafe extern "C" fn btrfs_launder_folio(folio: *mut folio) -> c_int {
    static int btrfs_launder_folio(struct folio *folio)
    {
    return btrfs_qgroup_free_data(folio_to_inode(folio), core::ptr::null_mut(), folio_pos(folio),
    folio_size(folio), core::ptr::null_mut());
    }
#[no_mangle]
unsafe extern "C" fn __btrfs_release_folio(folio: *mut folio, gfp_flags: gfp_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn btrfs_release_folio(folio: *mut folio, gfp_flags: gfp_t) -> bool {
    ...
    {
    let mut ret: c_int = filemap_migrate_folio(mapping, dst, src, mode);
    if (ret)
    return ret;
    return 0;
    }
    ...
    struct extent_state *cached_state = core::ptr::null_mut();
    let mut page_start: u64 = folio_pos(folio);
    let mut page_end: u64 = page_start + folio_size(folio) - 1;
    u64 cur;
    let mut inode_evicting: c_int = inode_state_read_once(&inode.vfs_inode) & I_FREEING;
//
...
//
    btrfs_qgroup_free_data(inode, core::ptr::null_mut(), cur, range_end + 1 - cur, core::ptr::null_mut());
    if (!inode_evicting)
...
#[no_mangle]
unsafe extern "C" fn btrfs_truncate(inode: *mut btrfs_inode, skip_writeback: bool) -> c_int {
    static int btrfs_truncate(struct btrfs_inode *inode, bool skip_writeback)
    {
    struct btrfs_truncate_control control = {
    .inode = inode,
    ...
    int ret;
    struct btrfs_trans_handle *trans;
    let mut min_size: u64 = btrfs_calc_metadata_size(fs_info, 1);
    let mut lock_start: u64 = round_down(inode.vfs_inode.i_size, fs_info.sectorsize);
    let mut i_size_up: u64 = round_up(inode.vfs_inode.i_size, fs_info.sectorsize);
...
//
    ...
    btrfs_block_rsv_release(fs_info, &rsv, -1, core::ptr::null_mut());
...
//
    inode_init_owner(idmap, inode, core::ptr::null_mut(),
...
//
    ...
    ei.delayed_node = core::ptr::null_mut();
    ei.i_otime_sec = 0;
    ei.i_otime_nsec = 0;
    inode = &ei.vfs_inode;
...
// This io tree sets the valid inode.
    ...
    ei.io_tree.inode = ei;
    ei.file_extent_tree = core::ptr::null_mut();
    mutex_init(&ei.log_mutex);
    spin_lock_init(&ei.ordered_tree_lock);
    ei.ordered_tree = RB_ROOT;
    ei.ordered_tree_last = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_test_destroy_inode(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_free_inode(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_destroy_inode(vfs_inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_drop_inode(inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_destroy_cachep() -> void __cold {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_init_cachep() -> int __init {
    ...
    let mut blocksize: u32 = btrfs_sb(inode.i_sb).sectorsize;
    let mut bi_flags: u32 = BTRFS_I(inode).flags;
    let mut bi_ro_flags: u32 = BTRFS_I(inode).ro_flags;
    ...
    let mut old_ino: u64 = btrfs_ino(BTRFS_I(old_inode));
    let mut new_ino: u64 = btrfs_ino(BTRFS_I(new_inode));
    let mut old_idx: u64 = 0;
    let mut new_idx: u64 = 0;
    int ret;
    int ret2;
    let mut need_abort: bool = false;
    let mut logs_pinned: bool = false;
...
//
    ...
    whiteout_args.inode = core::ptr::null_mut();
    }
    }
    out_fail:
    if (logs_pinned) {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct btrfs_delalloc_work {
    pub inode: *mut inode,
...
#[no_mangle]
unsafe extern "C" fn btrfs_run_delalloc_work(work: *mut btrfs_work) {
    ...
    work.inode = inode;
    btrfs_init_work(&work.work, btrfs_run_delalloc_work, core::ptr::null_mut());
    return work;
    }
//
...
//
    ...
    if (nr_to_write == core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_start_delalloc_snapshot(root: *mut btrfs_root, in_reclaim_context: bool) -> c_int {
    int btrfs_start_delalloc_snapshot(struct btrfs_root *root, bool in_reclaim_context)
    {
    struct btrfs_fs_info *fs_info = root.fs_info;
    if (unlikely(BTRFS_FS_ERROR(fs_info)))
    return -EROFS;
    return start_delalloc_inodes(root, core::ptr::null_mut(), true, in_reclaim_context);
    }
    int btrfs_start_delalloc_roots(struct btrfs_fs_info *fs_info, long nr,
    bool in_reclaim_context)
    {
    long *nr_to_write = nr == LONG_MAX ? core::ptr::null_mut() : &nr;
    struct btrfs_root *root;
    LIST_HEAD(splice);
    int ret;
    if (unlikely(BTRFS_FS_ERROR(fs_info)))
...
// 1 additional item for the inline extent
    ...
    struct btrfs_path *path;
    let mut start: u64 = ins.objectid;
    let mut len: u64 = ins.offset;
    let mut qgroup_released: u64 = 0;
    int ret;
    memset(&stack_fi, 0, sizeof(stack_fi));
...
//
    ...
    struct btrfs_key ins;
    let mut cur_offset: u64 = start;
    let mut clear_offset: u64 = start;
    u64 i_size;
    u64 cur_bytes;
    let mut last_alloc: u64 = (u64)-1;
    let mut ret: c_int = 0;
    let mut own_trans: bool = true;
    let mut end: u64 = start + num_bytes - 1;
    if (trans)
    own_trans = false;
    while (num_bytes > 0) {
...
//
    ...
    if (clear_offset < end)
    btrfs_free_reserved_data_space(BTRFS_I(inode), core::ptr::null_mut(), clear_offset,
    end - clear_offset + 1);
    return ret;
    }
...
//
    ...
    void *tmp;
    ssize_t ret;
    let mut nowait: bool = (iocb.ki_flags & IOCB_NOWAIT);
    path = btrfs_alloc_path();
    if (!path)
    return -ENOMEM;
    path.nowait = nowait;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct btrfs_encoded_read_private {
    pub sync_reads: *mut completion,
    pub uring_ctx: *mut c_void,
    pub pending_refs: refcount_t,
    pub status: blk_status_t,
}

#[no_mangle]
unsafe extern "C" fn btrfs_encoded_read_endio(bbio: *mut btrfs_bio) {
...
//
    ...
    let mut err: c_int = blk_status_to_errno(READ_ONCE(priv.status));
    if (priv.uring_ctx) {
...
//
    ...
    do {
    let mut bytes: usize = min_t(u64, disk_io_size, PAGE_SIZE);
...
// See btrfs_encoded_read_endio() for ordering.
    ...
    disk_io_size, pages, core::ptr::null_mut());
    if (ret)
    goto out;
    btrfs_unlock_extent(io_tree, start, lockend, cached_state);
    btrfs_inode_unlock(inode, BTRFS_ILOCK_SHARED);
...
//
    ...
    em = btrfs_get_extent(inode, core::ptr::null_mut(), start, lockend - start + 1);
    if (IS_ERR(em)) {
    ret = PTR_ERR(em);
    goto out_unlock_extent;
    }
    if (em.disk_bytenr == EXTENT_MAP_INLINE) {
    let mut extent_start: u64 = em.start;
//
...
// Leave inode and extent locked if we need to do a read.
    ...
    struct btrfs_key ins;
    let mut extent_reserved: bool = false;
    struct extent_map *em;
    ssize_t ret;
...
//
    ...
    struct folio *folio;
    let mut bytes: usize = min(min_folio_size, iov_iter_count(from));
    char *kaddr;
...
// Try an inline extent first.
    ...
    if (ret < 0)
    btrfs_qgroup_free_data(inode, data_reserved, start, num_bytes, core::ptr::null_mut());
    out_free_data_space:
//
...
#[no_mangle]
unsafe extern "C" fn btrfs_free_swapfile_pins(inode: *mut inode) {
    static void btrfs_free_swapfile_pins(struct inode *inode)
    {
    struct btrfs_fs_info *fs_info = BTRFS_I(inode).root.fs_info;
    struct btrfs_swapfile_pin *sp;
    struct rb_node *node, *next;
    let mut bg_bytes_released: u64 = 0;
    let mut bg_nr_released: u32 = 0;
    spin_lock(&fs_info.swapfile_pins_lock);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct btrfs_swap_info {
    pub start: u64,
    pub block_start: u64,
    pub block_len: u64,
    pub lowest_ppage: u64,
    pub highest_ppage: u64,
    pub nr_pages: c_ulong,
    pub nr_extents: c_int,
}
...
#[no_mangle]
unsafe extern "C" fn btrfs_swap_deactivate(file: *mut file) {
    ...
    struct extent_io_tree *io_tree = &BTRFS_I(inode).io_tree;
    struct extent_state *cached_state = core::ptr::null_mut();
    struct btrfs_chunk_map *map = core::ptr::null_mut();
    struct btrfs_device *device = core::ptr::null_mut();
    struct btrfs_swap_info bsi = {
    .lowest_ppage = (sector_t)-1ULL,
    };
    struct btrfs_backref_share_check_ctx *backref_ctx = core::ptr::null_mut();
    struct btrfs_path *path = core::ptr::null_mut();
    let mut ret: c_int = 0;
    let mut pinned_bg_nr: u32 = 0;
    u64 isize;
    let mut prev_extent_end: u64 = 0;
    let mut pinned_bg_size: u64 = 0;
//
...
//
#[no_mangle]
pub unsafe extern "C" fn btrfs_assert_inode_range_clean(inode: *mut btrfs_inode, start: u64, end: u64) {
    ...
    ASSERT(ordered == core::ptr::null_mut());
    }
//
