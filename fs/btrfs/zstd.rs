...
pub const ZSTD_BTRFS_MAX_WINDOWLOG: c_int = 17;

pub const ZSTD_BTRFS_DEFAULT_LEVEL: c_int = 3;

pub const ZSTD_BTRFS_MAX_LEVEL: c_int = 15;
// 307s to avoid pathologically clashing with transaction commit

    static zstd_parameters zstd_get_btrfs_parameters(int level,
    size_t src_len)
    {
    let mut params: zstd_parameters = zstd_get_params(level, src_len);
...
pub struct workspace {
    ...
    pub level: c_int,
    pub req_level: c_int,
    pub /: *mut *mut unsigned long last_used; / jiffies,
    pub list: list_head,
    pub lru_list: list_head,
    pub in_buf: zstd_in_buffer,
    pub out_buf: zstd_out_buffer,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct zstd_workspace_manager {
    pub lock: spinlock_t,
    pub lru_list: list_head,
    pub idle_ws: [list_head; ZSTD_BTRFS_MAX_LEVEL],
    pub active_map: c_ulong,
...
#[no_mangle]
pub unsafe extern "C" fn clip_level(level: c_int) -> c_int {
...
unsafe extern "C" fn zstd_reclaim_timer_fn(timer: *mut timer_list) {
    ...
    let mut reclaim_threshold: c_ulong = jiffies - ZSTD_BTRFS_RECLAIM_JIFFIES;
...
//
#[no_mangle]
unsafe extern "C" fn zstd_calc_ws_mem_sizes() {
...
#[no_mangle]
pub unsafe extern "C" fn zstd_alloc_workspace_manager(fs_info: *mut btrfs_fs_info) -> c_int {
    ...
    if (IS_ERR(ws)) {
    btrfs_warn(core::ptr::null_mut(), "cannot preallocate zstd compression workspace");
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn zstd_free_workspace_manager(fs_info: *mut btrfs_fs_info) {
...
//
    ...
    struct list_head *ws;
    struct workspace *workspace;
    let mut i: c_int = clip_level(level);
    ASSERT(zwsm);
    spin_lock_bh(&zwsm.lock);
...
//
#[no_mangle]
pub unsafe extern "C" fn zstd_put_workspace(fs_info: *mut btrfs_fs_info, ws: *mut list_head) {
...
#[no_mangle]
pub unsafe extern "C" fn zstd_free_workspace(ws: *mut list_head) {
...
#[no_mangle]
pub unsafe extern "C" fn zstd_compress_bio(ws: *mut list_head, cb: *mut compressed_bio) -> c_int {
...
// The current folio to read.
    struct folio *in_folio = core::ptr::null_mut();
// The current folio to write to.
    struct folio *out_folio = core::ptr::null_mut();
    let mut tot_in: c_ulong = 0;
    let mut tot_out: c_ulong = 0;
...
// Allocate and map in the output buffer.
    ...
    if (out_folio == core::ptr::null_mut()) {
    ret = -ENOMEM;
    goto out;
    }
...
// Check if we need more input.
    ...
    workspace.in_buf.src = core::ptr::null_mut();
    folio_put(in_folio);
...
#[no_mangle]
pub unsafe extern "C" fn zstd_decompress_bio(ws: *mut list_head, cb: *mut compressed_bio) -> c_int {
    int zstd_decompress_bio(struct list_head *ws, struct compressed_bio *cb)
    {
    struct btrfs_fs_info *fs_info = cb_to_fs_info(cb);
    struct workspace *workspace = list_entry(ws, struct workspace, list);
    struct folio_iter fi;
    let mut srclen: usize = bio_get_size(&cb.bbio.bio);
    zstd_dstream *stream;
    let mut ret: c_int = 0;
    let mut min_folio_size: c_uint = btrfs_min_folio_size(fs_info);
    let mut folio_in_index: c_ulong = 0;
    let mut total_folios_in: c_ulong = DIV_ROUND_UP(srclen, min_folio_size);
    unsigned long buf_start;
    let mut total_out: c_ulong = 0;
