...
//! Source: fs/btrfs/tests/zoned-tests.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct load_zone_info_test_vector {
    pub raid_type: u64,
    pub num_stripes: u64,
    pub alloc_offsets: [u64; 8],
    pub last_alloc: u64,
    pub bg_length: u64,
    pub degraded: bool,
    pub expected_result: c_int,
    pub expected_alloc_offset: u64,
...
pub struct zone_info {
    ...
    {
    struct btrfs_block_group *bg __free(btrfs_free_dummy_block_group) = core::ptr::null_mut();
    struct btrfs_chunk_map *map __free(btrfs_free_chunk_map) = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_test_zoned() -> c_int {
    int btrfs_test_zoned(void)
    {
    struct btrfs_fs_info *fs_info __free(btrfs_free_dummy_fs_info) = core::ptr::null_mut();
    int ret;
