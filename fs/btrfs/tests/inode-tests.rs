...
//! Source: fs/btrfs/tests/inode-tests.c
#![no_std]
...
//
    ...
    let mut value_len: u32 = sizeof(struct btrfs_file_extent_item);
...
#[no_mangle]
unsafe extern "C" fn insert_inode_item_key(root: *mut btrfs_root) {
...
//
#[no_mangle]
unsafe extern "C" fn setup_file_extents(root: *mut btrfs_root, sectorsize: u32) {
    static void setup_file_extents(struct btrfs_root *root, u32 sectorsize)
    {
    let mut slot: c_int = 0;
    let mut disk_bytenr: u64 = SZ_1M;
...
//
    ...
    let mut prealloc_only: static u32 = 0;
    let mut compressed_only: static u32 = 0;
    let mut vacancy_only: static u32 = 0;
#[no_mangle]
unsafe extern "C" fn test_btrfs_get_extent(sectorsize: u32, nodesize: u32) -> noinline int {
...
// First with no extents
    ...
    em = btrfs_get_extent(BTRFS_I(inode), core::ptr::null_mut(), 0, sectorsize);
    if (IS_ERR(em)) {
...
//
    ...
    em = btrfs_get_extent(BTRFS_I(inode), core::ptr::null_mut(), 0, (u64)-1);
    if (IS_ERR(em)) {
    test_err("got an error when we shouldn't have");
    goto out;
    }
...
//
    ...
    btrfs_free_extent_map(em);
// Regular extent
    em = btrfs_get_extent(BTRFS_I(inode), core::ptr::null_mut(), offset, sectorsize);
    if (IS_ERR(em)) {
    test_err("got an error when we shouldn't have");
    goto out;
    }
...
// A hole between regular extents but no hole extent
    em = btrfs_get_extent(BTRFS_I(inode), core::ptr::null_mut(), offset + 6, sectorsize);
    if (IS_ERR(em)) {
    test_err("got an error when we shouldn't have");
    goto out;
    }
    ...
    offset = btrfs_extent_map_end(em);
    btrfs_free_extent_map(em);
    em = btrfs_get_extent(BTRFS_I(inode), core::ptr::null_mut(), offset, SZ_4M);
    if (IS_ERR(em)) {
    test_err("got an error when we shouldn't have");
    goto out;
    }
...
#[no_mangle]
unsafe extern "C" fn test_hole_first(sectorsize: u32, nodesize: u32) -> c_int {
...
//
    ...
    em = btrfs_get_extent(BTRFS_I(inode), core::ptr::null_mut(), 0, 2 * sectorsize);
    if (IS_ERR(em)) {
    test_err("got an error when we shouldn't have");
    goto out;
    }
    ...
    btrfs_free_extent_map(em);
    em = btrfs_get_extent(BTRFS_I(inode), core::ptr::null_mut(), sectorsize, 2 * sectorsize);
    if (IS_ERR(em)) {
    test_err("got an error when we shouldn't have");
    goto out;
    }
...
#[no_mangle]
unsafe extern "C" fn test_extent_accounting(sectorsize: u32, nodesize: u32) -> c_int {
...
// [BTRFS_MAX_EXTENT_SIZE]
    ...
    BTRFS_MAX_EXTENT_SIZE - 1, 0, core::ptr::null_mut());
    if (ret) {
...
// [BTRFS_MAX_EXTENT_SIZE/2][sectorsize HOLE][the rest]
    ...
    EXTENT_DELALLOC | EXTENT_DELALLOC_NEW, core::ptr::null_mut());
    if (ret) {
...
//
    ret = btrfs_set_extent_delalloc(BTRFS_I(inode),
    BTRFS_MAX_EXTENT_SIZE + sectorsize,
    BTRFS_MAX_EXTENT_SIZE + 2 * sectorsize - 1, 0, core::ptr::null_mut());
    if (ret) {
    test_err("btrfs_set_extent_delalloc returned %d", ret);
    goto out;
    }
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_test_inodes(sectorsize: u32, nodesize: u32) -> c_int {
