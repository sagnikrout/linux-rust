...
//! Source: fs/btrfs/tests/chunk-allocation-tests.c
#![no_std]
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct pending_extent_test_case {
...
#[no_mangle]
unsafe extern "C" fn test_find_hole_in_pending(sectorsize: u32, nodesize: u32) -> c_int {
    ...
    let mut hole_start: u64 = test_case.hole_start;
    let mut hole_len: u64 = test_case.hole_len;
    bool found;
    for (int j = 0; j < ARRAY_SIZE(test_case.pending_extents); j++) {
    let mut start: u64 = test_case.pending_extents[j].start;
    let mut len: u64 = test_case.pending_extents[j].len;
    if (!len)
    continue;
    btrfs_set_extent_bit(&device.alloc_state,
    start, start + len - 1,
    CHUNK_ALLOCATED, core::ptr::null_mut());
    }
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct first_pending_test_case {
    pub name: *const c_char,
...
#[no_mangle]
unsafe extern "C" fn test_first_pending_extent(sectorsize: u32, nodesize: u32) -> c_int {
    ...
    let mut start: u64 = test_case.pending_extent.start;
    let mut len: u64 = test_case.pending_extent.len;
...
#[no_mangle]
pub unsafe extern "C" fn btrfs_test_chunk_allocation(sectorsize: u32, nodesize: u32) -> c_int {
