...
//
#[no_mangle]
unsafe extern "C" fn ceph_set_ino_cb(inode: *mut inode, data: *mut c_void) -> c_int {
...
// Mismatch – this should be rare.  Emit a WARN and obtain the correct inode.
    ...
    return ceph_get_inode(sb, vino, core::ptr::null_mut());
    }
//
...
//
    ...
    as_ctx.pagelist = core::ptr::null_mut();
    }
...
//
    ...
    struct inode *inode = ceph_get_inode(parent.i_sb, vino, core::ptr::null_mut());
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut ret: c_int = -ENOTDIR;
    if (IS_ERR(inode))
    return inode;
...
//
    ...
    p = &(*p).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(0: c >) -> else {
    else if (c > 0)
    p = &(*p).rb_right;
    else
    return frag;
...
//
    ...
    let mut c: c_int = ceph_frag_compare(f, frag.frag);
    if (c < 0)
    n = n.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(0: c >) -> else {
...
//
    ...
    let mut t: u32 = ceph_frag_make(0, 0);
    struct ceph_inode_frag *frag;
    unsigned nway, i;
    u32 n;
    if (found)
...
//
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_frag *frag;
    let mut id: u32 = le32_to_cpu(dirinfo.frag);
    let mut mds: c_int = le32_to_cpu(dirinfo.auth);
    let mut ndist: c_int = le32_to_cpu(dirinfo.ndist);
    let mut diri_auth: c_int = -1;
    int i;
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn frag_tree_split_cmp(l: *const c_void, r: *const c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn is_frag_child(f: u32, frag: *mut ceph_inode_frag) -> bool {
    static bool is_frag_child(u32 f, struct ceph_inode_frag *frag)
    {
    if (!frag)
    let mut f: return = = ceph_frag_make(0, 0);
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    struct ceph_inode_frag *frag, *prev_frag = core::ptr::null_mut();
    ...
    if (id != __ceph_choose_frag(ci, id, core::ptr::null_mut(), core::ptr::null_mut()))
    update = true;
    }
    if (!update)
    goto out_unlock;
    ...
    frag_tree_split_cmp, core::ptr::null_mut());
    }
    doutc(cl, "%p %llx.%llx\n", inode, ceph_vinop(inode));
    rb_node = rb_first(&ci.i_fragtree);
    for (i = 0; i < nsplits; i++) {
...
// Set parameters for the netfs library
    ...
    ci.i_symlink = core::ptr::null_mut();
    ci.i_max_bytes = 0;
    ci.i_max_files = 0;
    ci.i_subvolume_id = CEPH_SUBVOLUME_ID_NONE;
    memset(&ci.i_dir_layout, 0, sizeof(ci.i_dir_layout));
    memset(&ci.i_cached_layout, 0, sizeof(ci.i_cached_layout));
    RCU_INIT_POINTER(ci.i_layout.pool_ns, core::ptr::null_mut());
    ci.i_fragtree = RB_ROOT;
    mutex_init(&ci.i_fragtree_mutex);
    ci.i_xattrs.blob = core::ptr::null_mut();
    ci.i_xattrs.prealloc_blob = core::ptr::null_mut();
    ...
    ci.i_snap_realm = core::ptr::null_mut();
    ...
    ci.i_crypt_info = core::ptr::null_mut();
    ci.fscrypt_auth = core::ptr::null_mut();
    ci.fscrypt_auth_len = 0;

    return &ci.netfs.inode;
    }
#[no_mangle]
pub unsafe extern "C" fn ceph_free_inode(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_evict_inode(inode: *mut inode) {
...
//
    ...
    while ((n = rb_first(&ci.i_fragtree)) != core::ptr::null_mut()) {
...
#[no_mangle]
pub unsafe extern "C" fn calc_inode_blocks(size: u64) -> blkcnt_t {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_inode_set_subvolume(inode: *mut inode, subvolume_id: u64) {
...
// subvolume_id should not change once set
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut iatime: timespec64 = inode_get_atime(inode);
    let mut ictime: timespec64 = inode_get_ctime(inode);
    let mut imtime: timespec64 = inode_get_mtime(inode);
...
//
    ...
    struct ceph_buffer *xattr_blob = core::ptr::null_mut();
    struct ceph_buffer *old_blob = core::ptr::null_mut();
    struct ceph_string *pool_ns = core::ptr::null_mut();
    struct ceph_cap *new_cap = core::ptr::null_mut();
    let mut err: c_int = 0;
    let mut wake: bool = false;
    let mut queue_trunc: bool = false;
    let mut new_version: bool = false;
    let mut fill_inline: bool = false;
    let mut mode: umode_t = le32_to_cpu(info.mode);
    let mut rdev: dev_t = le32_to_cpu(info.rdev);
...
// Update change_attribute
    ...
    iinfo.fscrypt_auth = core::ptr::null_mut();
...
// directories have fl_stripe_unit set to zero
    ...
    inode.i_blkbits = CEPH_FSCRYPT_BLOCK_SHIFT;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: le32_to_cpu(info->layout.fl_stripe_unit)) -> else {
...
// be careful with mtime, atime, size
    ...
    let mut size: u64 = le64_to_cpu(info.size);
    ...
    let mut fsize: u64 = __le64_to_cpu(*(__le64 *)iinfo.fscrypt_file);
...
// finally update i_version
    ...
    case S_IFLNK:
    if (!ci.i_symlink) {
    let mut symlen: u32 = iinfo.symlink_len;
    char *sym;
    spin_unlock(&ci.i_ceph_lock);
    if (IS_ENCRYPTED(inode)) {
...
// set dir completion flag?
    ...
    let mut cache_caps: c_int = CEPH_CAP_FILE_CACHE | CEPH_CAP_FILE_LAZYIO;
...
//
    ...
    let mut mask: unsigned = le16_to_cpu(lease.mask);
    let mut duration: long unsigned = le32_to_cpu(lease.duration_ms);
    let mut ttl: long unsigned = from_time + (duration * HZ) / 1000;
    let mut half_ttl: long unsigned = from_time + (duration * HZ / 2) / 1000;
...
// old_lease_session = di->lease_session;
    di.lease_session = core::ptr::null_mut();
    }
    if (!di.lease_session)
    ...
    unsigned long from_time)
    {
    struct ceph_mds_session *old_lease_session = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn splice_dentry(pdn: *mut dentry, in: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_fill_trace(sb: *mut super_block, req: *mut ceph_mds_request) -> c_int {
    int ceph_fill_trace(struct super_block *sb, struct ceph_mds_request *req)
    {
    struct ceph_mds_session *session = req.r_session;
    struct ceph_mds_reply_info_parsed *rinfo = &req.r_reply_info;
    struct inode *in = core::ptr::null_mut();
    struct ceph_vino tvino, dvino;
    struct ceph_fs_client *fsc = ceph_sb_to_fs_client(sb);
    struct ceph_client *cl = fsc.client;
    struct inode *parent_dir = core::ptr::null_mut();
    let mut err: c_int = 0;
...
//
    ...
    err = ceph_fill_inode(parent_dir, core::ptr::null_mut(), &rinfo.diri,
    ...
    err = ceph_fname_to_usr(&fname, core::ptr::null_mut(), &oname, &is_nokey);
    if (err < 0) {
    dput(parent);
...
// Should be filled in by handle_reply
    ...
    ceph_vinop(in));
    req.r_target_inode = core::ptr::null_mut();
...
// parent inode is not locked, be careful
    struct ceph_vino *ptvino = core::ptr::null_mut();
    dvino.ino = le64_to_cpu(rinfo.diri.in.ino);
    dvino.snap = le64_to_cpu(rinfo.diri.in.snapid);
    if (rinfo.head.is_target) {
    tvino.ino = le64_to_cpu(rinfo.targeti.in.ino);
...
//
    ...
    in = ceph_get_inode(req.r_dentry.d_sb, vino, core::ptr::null_mut());
    if (IS_ERR(in)) {
    err = PTR_ERR(in);
    doutc(cl, "badness got %d\n", err);
    continue;
    }
    rc = ceph_fill_inode(in, core::ptr::null_mut(), &rde.inode, core::ptr::null_mut(), session,
...
#[no_mangle]
pub unsafe extern "C" fn ceph_readdir_cache_release(ctl: *mut ceph_readdir_cache_control) {
    void ceph_readdir_cache_release(struct ceph_readdir_cache_control *ctl)
    {
    if (ctl.folio) {
    folio_release_kmap(ctl.folio, ctl.dentries);
    ctl.folio = core::ptr::null_mut();
    }
    }
    ...
    let mut nsize: unsigned = PAGE_SIZE / sizeof(struct dentry*);
    let mut idx: unsigned = ctl.index % nsize;
    let mut pgoff: pgoff_t = ctl.index / nsize;
    if (!ctl.folio || pgoff != ctl.folio.index) {
    ceph_readdir_cache_release(ctl);
    let mut fgf: fgf_t = FGP_LOCK;
    if (idx == 0)
    fgf |= FGP_ACCESSED | FGP_CREAT;
    ctl.folio = __filemap_get_folio(&dir.i_data, pgoff,
    fgf, mapping_gfp_mask(&dir.i_data));
    if (IS_ERR(ctl.folio)) {
    let mut err: c_int = PTR_ERR(ctl.folio);
    ctl.folio = core::ptr::null_mut();
    ctl.index = -1;
    let mut idx: return = = 0 ? err : 0;
    }
...
// i_rwsem, no need to use folio lock
    ...
    struct dentry *dn;
    struct inode *in;
    let mut err: c_int = 0, skipped = 0, ret, i;
    let mut frag: u32 = le32_to_cpu(req.r_args.readdir.frag);
    let mut last_hash: u32 = 0;
    u32 fpos_offset;
    let mut cache_ctl: ceph_readdir_cache_control = {};
...
// FIXME: release caps/leases if error occurs
    ...
    if (rinfo.hash_order) {
    let mut hash: u32 = ceph_frag_value(rde.raw_hash);
    if (hash != last_hash)
    fpos_offset = 2;
    last_hash = hash;
...
// inode
    ...
    in = ceph_get_inode(parent.d_sb, tvino, core::ptr::null_mut());
    if (IS_ERR(in)) {
    ...
    ret = ceph_fill_inode(in, core::ptr::null_mut(), &rde.inode, core::ptr::null_mut(), session,
    -1, &req.r_caps_reservation);
    if (ret < 0) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_inode_set_size(inode: *mut inode, size: loff_t) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_queue_inode_work(inode: *mut inode, work_bit: c_int) {
...
#[no_mangle]
unsafe extern "C" fn ceph_do_invalidate_pages(inode: *mut inode) {
    ...
    if (__ceph_caps_revoking_other(ci, core::ptr::null_mut(), CEPH_CAP_FILE_CACHE))
    check = 1;
    spin_unlock(&ci.i_ceph_lock);
    mutex_unlock(&ci.i_truncate_mutex);
    goto out;
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_do_pending_vmtruncate(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn ceph_inode_work(work: *mut work_struct) {
    static void ceph_inode_work(struct work_struct *work)
    {
...
//
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut boff: c_int = attr.ia_size % CEPH_FSCRYPT_BLOCK_SIZE;
    loff_t pos, orig_pos = round_down(attr.ia_size,
    CEPH_FSCRYPT_BLOCK_SIZE);
    let mut block: u64 = orig_pos >> CEPH_FSCRYPT_BLOCK_SHIFT;
    struct ceph_pagelist *pagelist = core::ptr::null_mut();
    let mut iov: kvec = {0};
    struct iov_iter iter;
    struct page *page = core::ptr::null_mut();
    struct ceph_fscrypt_truncate_size_header header;
    let mut retry_op: c_int = 0;
    let mut len: c_int = CEPH_FSCRYPT_BLOCK_SIZE;
    let mut i_size: loff_t = i_size_read(inode);
    int got, ret, issued;
    u64 objver;
    ret = __ceph_get_caps(inode, core::ptr::null_mut(), CEPH_CAP_FILE_RD, 0, -1, &got);
    if (ret < 0)
    return ret;
    issued = __ceph_caps_issued(ci, core::ptr::null_mut());
    doutc(cl, "size %lld . %lld got cap refs on %s, issued %s\n",
    i_size, attr.ia_size, ceph_cap_string(got),
    ceph_cap_string(issued));
// Try to writeback the dirty pagecaches
    if (issued & (CEPH_CAP_FILE_BUFFER)) {
    let mut lend: loff_t = orig_pos + CEPH_FSCRYPT_BLOCK_SIZE - 1;
...
// Append the last block contents to pagelist
    ...
    let mut isize: loff_t = i_size_read(inode);
    int issued;
    let mut release: c_int = 0, dirtied = 0;
    let mut mask: c_int = 0;
    let mut err: c_int = 0;
    let mut inode_dirty_flags: c_int = 0;
    let mut lock_snap_rwsem: bool = false;
    bool fill_fscrypt;
    int truncate_retry = 20; /* The RMW will take around 50ms */
    struct dentry *dentry;
    char *path;
    let mut do_sync: bool = false;
    dentry = d_find_alias(inode);
    if (!dentry) {
    do_sync = true;
    } else {
    let mut path_info: ceph_path_info = {0};
...
// For none EACCES cases will let the MDS do the mds auth check
    ...
    let mut len: u32 = ceph_fscrypt_auth_len(cia.fscrypt_auth);
...
// It should never be re-set once set
    ...
    cia.fscrypt_auth = core::ptr::null_mut();
    }
    ...
    let mut fsuid: kuid_t = from_vfsuid(idmap, i_user_ns(inode), attr.ia_vfsuid);
    ...
    let mut fsgid: kgid_t = from_vfsgid(idmap, i_user_ns(inode), attr.ia_vfsgid);
...
//
    ...
    dirtied |= CEPH_CAP_AUTH_EXCL;
#[no_mangle]
pub unsafe extern "C" fn if(CEPH_CAP_FILE_EXCL: issued &) -> else {
    else if (issued & CEPH_CAP_FILE_EXCL)
    dirtied |= CEPH_CAP_FILE_EXCL;
#[no_mangle]
pub unsafe extern "C" fn if(CEPH_CAP_XATTR_EXCL: issued &) -> else {
...
//
    ...
    return -EDQUOT;
    err = __ceph_setattr(idmap, inode, attr, core::ptr::null_mut());
    if (err >= 0 && (attr.ia_valid & ATTR_MODE))
    err = posix_acl_chmod(idmap, dentry, attr.ia_mode);
    return err;
    }
#[no_mangle]
pub unsafe extern "C" fn ceph_try_to_choose_auth_mds(inode: *mut inode, mask: c_int) -> c_int {
    int ceph_try_to_choose_auth_mds(struct inode *inode, int mask)
    {
    let mut issued: c_int = ceph_caps_issued(ceph_inode(inode));
//
...
//
    ...
    let mut inline_version: u64 = req.r_reply_info.targeti.inline_version;
    if (inline_version == 0) {
// the reply is supposed to contain inline data
    ...
    struct ceph_mds_client *mdsc = fsc.mdsc;
    struct ceph_mds_request *req;
    let mut mode: c_int = USE_AUTH_MDS;
    int err;
    char *xattr_value;
...
#[no_mangle]
unsafe extern "C" fn statx_to_caps(want: u32, mode: umode_t) -> c_int {
...
//
    ...
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut valid_mask: u32 = STATX_BASIC_STATS;
    let mut err: c_int = 0;
    if (ceph_inode_is_shutdown(inode))
    return -ESTALE;
...
#[no_mangle]
pub unsafe extern "C" fn ceph_inode_shutdown(inode: *mut inode) {
