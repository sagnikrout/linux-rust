...
#[no_mangle]
unsafe extern "C" fn ceph_is_valid_xattr(name: *const c_char) -> bool {
...
pub struct ceph_vxattr {
    pub name: *mut c_char,
    pub /: *mut *mut size_t name_size; / strlen(name) + 1 (for '\0'),
    ssize_t (*getxattr_cb)(struct ceph_inode_info *ci, char *val,
    pub size): usize,
    pub ci): *mut *mut bool (exists_cb)(struct ceph_inode_info,
    pub flags: c_uint,
}

// layouts
#[no_mangle]
unsafe extern "C" fn ceph_vxattrcb_layout_exists(ci: *mut ceph_inode_info) -> bool {
    ...
    rcu_dereference_raw(fl.pool_ns) != core::ptr::null_mut());
    }
    ...
    let mut pool: i64 = ci.i_layout.pool_id;
...
//
#[no_mangle]
pub unsafe extern "C" fn __printf(_arg: 3, _arg: 4) -> static {
    static __printf(3, 4)
#[no_mangle]
pub unsafe extern "C" fn ceph_fmt_xattr(val: *mut c_char, size: usize, fmt: *const c_char, ...) -> c_int {
...
// dir pin
#[no_mangle]
unsafe extern "C" fn ceph_vxattrcb_dir_pin_exists(ci: *mut ceph_inode_info) -> bool {
...
// quotas
#[no_mangle]
unsafe extern "C" fn ceph_vxattrcb_quota_exists(ci: *mut ceph_inode_info) -> bool {
...
// snapshots
#[no_mangle]
unsafe extern "C" fn ceph_vxattrcb_snap_btime_exists(ci: *mut ceph_inode_info) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ceph_vxattrcb_fscrypt_auth_exists(ci: *mut ceph_inode_info) -> bool {
    ...
    { .name = core::ptr::null_mut(), 0 }	/* Required table terminator */
    };
// files
...
pub const MAX_XATTR_VAL_PRINT_LEN: c_int = 256;
    ...
    struct ceph_inode_xattr *xattr = core::ptr::null_mut();
    int c;
    let mut new: c_int = 0;
...
pub unsafe extern "C" fn if(0: c >) -> else {
    ...
    break;
#[no_mangle]
pub unsafe extern "C" fn if(xattr->name_len: name_len <) -> else {
    else if (name_len < xattr.name_len)
    p = &(*p).rb_left;
    else
    p = &(*p).rb_right;
    }
    xattr = core::ptr::null_mut();
    }
    if (update_xattr) {
    let mut err: c_int = 0;
    ...
    err = -EEXIST;
#[no_mangle]
pub unsafe extern "C" fn if(XATTR_REPLACE): !xattr && (flags &) -> else {
...
// newxattr = NULL;
    ...
    struct rb_node *parent = core::ptr::null_mut();
    struct ceph_inode_xattr *xattr = core::ptr::null_mut();
    let mut name_len: c_int = strlen(name);
    int c;
    p = &ci.i_xattrs.index.rb_node;
    while (*p) {
    parent = *p;
...
pub unsafe extern "C" fn if(0: c >) -> else {
    ...
    let mut len: c_int = min(xattr.val_len, MAX_XATTR_VAL_PRINT_LEN);
...
#[no_mangle]
unsafe extern "C" fn __free_xattr(xattr: *mut ceph_inode_xattr) {
...
#[no_mangle]
pub unsafe extern "C" fn __ceph_destroy_xattrs(ci: *mut ceph_inode_info) {
...
#[no_mangle]
unsafe extern "C" fn __build_xattrs(inode: *mut inode) -> c_int {
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    u32 namelen;
    let mut numattr: u32 = 0;
    void *p, *end;
    u32 len;
    const char *name, *val;
    struct ceph_inode_info *ci = ceph_inode(inode);
    u64 xattr_version;
    struct ceph_inode_xattr **xattrs = core::ptr::null_mut();
    let mut err: c_int = 0;
    int i;
    doutc(cl, "len=%d\n",
...
//
    ...
    let mut need: c_int = __get_required_blob_size(ci, 0, 0);
...
#[no_mangle]
pub unsafe extern "C" fn __get_request_mask(in: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_listxattr(dentry: *mut dentry, names: *mut c_char, size: usize) -> isize {
    ssize_t ceph_listxattr(struct dentry *dentry, char *names, size_t size)
    {
    struct inode *inode = d_inode(dentry);
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut len_only: bool = (size == 0);
    u32 namelen;
    int err;
    spin_lock(&ci.i_ceph_lock);
...
// add 1 byte for each xattr due to the null termination
    ...
    let mut op: c_int = CEPH_MDS_OP_SETXATTR;
    int err;
    if (size > 0) {
...
// do request
    ...
    let mut name_len: c_int = strlen(name);
    let mut val_len: c_int = size;
    char *newname = core::ptr::null_mut();
    char *newval = core::ptr::null_mut();
    struct ceph_inode_xattr *xattr = core::ptr::null_mut();
    int required_blob_size;
    let mut check_realm: bool = false;
...
#[no_mangle]
pub unsafe extern "C" fn ceph_security_xattr_wanted(in: *mut inode) -> bool {
    bool ceph_security_xattr_wanted(struct inode *in)
    {
    return in.i_security != core::ptr::null_mut();
    }
#[no_mangle]
pub unsafe extern "C" fn ceph_security_xattr_deadlock(in: *mut inode) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_release_acl_sec_ctx(as_ctx: *mut ceph_acl_sec_ctx) {
