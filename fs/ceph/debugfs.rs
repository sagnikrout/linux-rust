...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ceph_session_feature_desc {
    pub bit: c_uint,
...
#[no_mangle]
unsafe extern "C" fn mdsmap_show(s: *mut seq_file, p: *mut c_void) -> c_int {
    ...
    let mut state: c_int = mdsmap.m_info[i].state;
...
//
#[no_mangle]
unsafe extern "C" fn mdsc_show(s: *mut seq_file, p: *mut c_void) -> c_int {
    ...
    req.r_session.s_mds);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !req->r_request) -> else {
...
#[no_mangle]
unsafe extern "C" fn metrics_file_show(s: *mut seq_file, p: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn metrics_latency_show(s: *mut seq_file, p: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn metrics_size_show(s: *mut seq_file, p: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn metrics_caps_show(s: *mut seq_file, p: *mut c_void) -> c_int {
    static int metrics_caps_show(struct seq_file *s, void *p)
    {
    struct ceph_fs_client *fsc = s.private;
    struct ceph_client_metric *m = &fsc.mdsc.metric;
    let mut nr_caps: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn caps_show_cb(inode: *mut inode, mds: c_int, p: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn caps_show(s: *mut seq_file, p: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn mds_sessions_show(s: *mut seq_file, ptr: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn status_show(s: *mut seq_file, p: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn reset_status_show(s: *mut seq_file, p: *mut c_void) -> c_int {
    static int reset_status_show(struct seq_file *s, void *p)
    {
    struct ceph_fs_client *fsc = s.private;
    struct ceph_mds_client *mdsc = fsc.mdsc;
    struct ceph_client_reset_state *st;
    let mut trigger: u64 = 0, success = 0, failure = 0;
    let mut last_start: c_ulong = 0, last_finish = 0;
    let mut last_errno: c_int = 0;
    let mut phase: enum ceph_client_reset_phase = CEPH_CLIENT_RESET_IDLE;
    let mut drain_timed_out: bool = false;
    let mut sessions_reset: c_int = 0;
    let mut blocked_requests: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn subvolume_metrics_show(s: *mut seq_file, p: *mut c_void) -> c_int {
    static int subvolume_metrics_show(struct seq_file *s, void *p)
    {
    struct ceph_fs_client *fsc = s.private;
    struct ceph_mds_client *mdsc = fsc.mdsc;
    struct ceph_subvol_metric_snapshot *snapshot = core::ptr::null_mut();
    let mut nr: u32 = 0;
    let mut total_sent: u64 = 0;
    let mut nonzero_sends: u64 = 0;
    u32 i;
    if (!mdsc) {
...
#[no_mangle]
unsafe extern "C" fn metric_features_show(s: *mut seq_file, p: *mut c_void) -> c_int {
    static int metric_features_show(struct seq_file *s, void *p)
    {
    struct ceph_fs_client *fsc = s.private;
    struct ceph_mds_client *mdsc = fsc.mdsc;
    let mut session_features: c_ulong = 0;
    let mut have_session: bool = false;
    let mut metric_collect: bool = false;
    let mut subvol_support: bool = false;
    let mut metrics_enabled: bool = false;
    let mut subvol_enabled: bool = false;
    int i;
    if (!mdsc) {
    seq_puts(s, "mds client unavailable\n");
    return 0;
    ...
    let mut set: bool = test_bit(desc.bit, &session_features);
    seq_printf(s, "  %-24s : %s\n", desc.name,
    set ? "yes" : "no");
    }
    return 0;
...
//
#[no_mangle]
unsafe extern "C" fn congestion_kb_set(data: *mut c_void, val: u64) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn congestion_kb_get(data: *mut c_void, val: *mut u64) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_fs_debugfs_cleanup(fsc: *mut ceph_fs_client) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_fs_debugfs_init(fsc: *mut ceph_fs_client) {
