...
#[no_mangle]
pub unsafe extern "C" fn ceph_adjust_quota_realms_count(inode: *mut inode, inc: bool) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_has_realms_with_quotas(inode: *mut inode) -> bool {
...
// lookup inode
    ...
    {
    struct ceph_quotarealm_inode *qri = core::ptr::null_mut();
    ...
    node = &((*node).rb_left);
#[no_mangle]
pub unsafe extern "C" fn if(qri->ino: ino >) -> else {
    else if (ino > qri.ino)
    node = &((*node).rb_right);
    else
    break;
...
// Check if this inode lookup has failed recently
    ...
    if (qri.inode) {
// get caps
    int ret = __ceph_do_getattr(qri.inode, core::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn ceph_cleanup_quotarealms_inodes(mdsc: *mut ceph_mds_client) {
...
//
    ...
    {
    struct ceph_client *cl = mdsc.fsc.client;
    struct ceph_inode_info *ci = core::ptr::null_mut();
...
//
    ...
    in = has_inode ? igrab(realm.inode) : core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn ceph_quota_is_same_realm(old: *mut inode, new: *mut inode) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_quota_is_max_files_exceeded(inode: *mut inode) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_quota_is_max_bytes_exceeded(inode: *mut inode, newsize: loff_t) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_quota_is_max_bytes_approaching(inode: *mut inode, newsize: loff_t) -> bool {
    bool ceph_quota_is_max_bytes_approaching(struct inode *inode, loff_t newsize)
    {
    let mut size: loff_t = ceph_inode(inode).i_reported_size;
    if (!ceph_has_realms_with_quotas(inode))
    return false;
// return immediately if we're decreasing file size
    if (newsize <= size)
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_quota_update_statfs(fsc: *mut ceph_fs_client, buf: *mut kstatfs) -> bool {
    ...
    return false;
    spin_lock(&realm.inodes_with_caps_lock);
    in = realm.inode ? igrab(realm.inode) : core::ptr::null_mut();
    spin_unlock(&realm.inodes_with_caps_lock);
    if (in) {
    ci = ceph_inode(in);
    spin_lock(&ci.i_ceph_lock);
