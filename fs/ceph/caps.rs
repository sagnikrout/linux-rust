...
//
pub const MAX_CAP_STR: c_int = 20;
...
#[no_mangle]
pub unsafe extern "C" fn ceph_caps_init(mdsc: *mut ceph_mds_client) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_caps_finalize(mdsc: *mut ceph_mds_client) {
...
#[no_mangle]
unsafe extern "C" fn __ceph_unreserve_caps(mdsc: *mut ceph_mds_client, nr_caps: c_int) {
...
// first reserve any caps that are already allocated
    ...
    {
    struct ceph_client *cl = mdsc.fsc.client;
    struct ceph_cap *cap = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn ceph_put_cap(mdsc: *mut ceph_mds_client, cap: *mut ceph_cap) {
...
//
    ...
    n = n.rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(cap->mds: mds >) -> else {
    else if (mds > cap.mds)
    n = n.rb_right;
    else
    return cap;
...
//
    ...
    p = &(*p).rb_left;
#[no_mangle]
pub unsafe extern "C" fn if(cap->mds: new->mds >) -> else {
...
// Common issue checks for add_cap, handle_cap_grant.
    ...
    let mut had: unsigned = __ceph_caps_issued(ci, core::ptr::null_mut());
...
//
    ...
    struct ceph_cap *cap;
    let mut mds: c_int = session.s_mds;
    int actual_wanted;
    u32 gen;
    lockdep_assert_held(&ci.i_ceph_lock);
...
//
#[no_mangle]
unsafe extern "C" fn __cap_is_valid(ci: *mut ceph_inode_info, cap: *mut ceph_cap) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_caps_issued(ci: *mut ceph_inode_info, implemented: *mut c_int) -> c_int {
    int __ceph_caps_issued(struct ceph_inode_info *ci, int *implemented)
    {
    struct inode *inode = &ci.netfs.inode;
    struct ceph_client *cl = ceph_inode_to_client(inode);
    let mut have: c_int = ci.i_snap_caps;
    struct ceph_cap *cap;
    struct rb_node *p;
    if (implemented)
// implemented = 0;
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_caps_issued_other(ci: *mut ceph_inode_info, ocap: *mut ceph_cap) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __touch_cap(ci: *mut ceph_inode_info, cap: *mut ceph_cap) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_caps_issued_mask(ci: *mut ceph_inode_info, mask: c_int, touch: c_int) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __ceph_is_any_revoking(ci: *const ceph_inode_info) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn __ceph_caps_used(ci: *mut ceph_inode_info) -> c_int {
...
pub const FMODE_WAIT_BIAS: c_int = 1000;
//
// wanted, by virtue of open file modes
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_caps_file_wanted(ci: *mut ceph_inode_info) -> c_int {
    int __ceph_caps_file_wanted(struct ceph_inode_info *ci)
    {
    let mut PIN_SHIFT: c_int = ffs(CEPH_FILE_MODE_PIN);
    let mut RD_SHIFT: c_int = ffs(CEPH_FILE_MODE_RD);
    let mut WR_SHIFT: c_int = ffs(CEPH_FILE_MODE_WR);
    let mut LAZY_SHIFT: c_int = ffs(CEPH_FILE_MODE_LAZY);
    struct ceph_mount_options *opt =
    ceph_inode_to_fs_client(&ci.netfs.inode).mount_options;
    let mut used_cutoff: c_ulong = jiffies - opt.caps_wanted_delay_max * HZ;
    let mut idle_cutoff: c_ulong = jiffies - opt.caps_wanted_delay_min * HZ;
    if (S_ISDIR(ci.netfs.inode.i_mode)) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_caps_wanted(ci: *mut ceph_inode_info) -> c_int {
    int __ceph_caps_wanted(struct ceph_inode_info *ci)
    {
    let mut w: c_int = __ceph_caps_file_wanted(ci) | __ceph_caps_used(ci);
    if (S_ISDIR(ci.netfs.inode.i_mode)) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_caps_mds_wanted(ci: *mut ceph_inode_info, check: bool) -> c_int {
    int __ceph_caps_mds_wanted(struct ceph_inode_info *ci, bool check)
    {
    struct ceph_cap *cap;
    struct rb_node *p;
    let mut mds_wanted: c_int = 0;
    for (p = rb_first(&ci.i_caps); p; p = rb_next(p)) {
    cap = rb_entry(p, struct ceph_cap, ci_node);
...
#[no_mangle]
pub unsafe extern "C" fn ceph_is_any_caps(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __ceph_remove_cap(ci: *mut ceph_inode_info, cap: *mut ceph_cap, queue_release: bool) {
...
// remove from inode's cap rbtree, and clear auth cap
    ...
    ci.i_auth_cap = core::ptr::null_mut();
...
//
    ...
    ceph_change_snap_realm(&ci.netfs.inode, core::ptr::null_mut());
    ...
    if (ceph_cap_is_removed(cap)) {
    doutc(mdsc.fsc.client, "inode is core::ptr::null_mut()\n");
    return;
    }
    lockdep_assert_held(&ci.i_ceph_lock);
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct cap_msg_args {
    pub session: *mut ceph_mds_session,
    pub follows: u64 ino, cid,,
    pub max_size: u64 flush_tid, oldest_flush_tid, size,,
    pub xattr_version: u64,
    pub change_attr: u64,
    pub xattr_buf: *mut ceph_buffer,
    pub old_xattr_buf: *mut ceph_buffer,
    pub btime: timespec64 atime, mtime, ctime,,
    pub dirty: int op, caps, wanted,,
    pub time_warp_seq: u32 seq, issue_seq, mseq,,
    pub flags: u32,
    pub uid: kuid_t,
    pub gid: kgid_t,
    pub mode: umode_t,
    ...
    pub context: u8 fscrypt_auth[sizeof(struct ceph_fscrypt_auth)]; // for,
}

// Marshal up the cap msg to the MDS
#[no_mangle]
unsafe extern "C" fn encode_cap_msg(msg: *mut ceph_msg, arg: *mut cap_msg_args) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_remove_caps(ci: *mut ceph_inode_info) {
...
//
    ...
    arg.xattr_buf = core::ptr::null_mut();
    arg.old_xattr_buf = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn cap_msg_size(arg: *mut cap_msg_args) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __send_cap(arg: *mut cap_msg_args, ci: *mut ceph_inode_info) {
...
//
    ...
    struct ceph_client *cl = mdsc.fsc.client;
    struct ceph_cap_snap *capsnap;
    let mut oldest_flush_tid: u64 = 0;
    let mut first_tid: u64 = 1, last_tid = 0;
...
// only flush each capsnap once
    ...
    struct ceph_cap_flush *cf = core::ptr::null_mut(), *iter;
    int ret;
    ...
    struct ceph_mds_client *mdsc = ceph_inode_to_fs_client(inode).mdsc;
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_mds_session *session = core::ptr::null_mut();
    let mut need_put: bool = false;
    int mds;
    doutc(cl, "%p %llx.%llx\n", inode, ceph_vinop(inode));
    if (psession)
    session = *psession;
...
//
    ...
    struct inode *inode = &ci.netfs.inode;
    struct ceph_client *cl = ceph_inode_to_client(inode);
    let mut was: c_int = ci.i_dirty_caps;
    let mut dirty: c_int = 0;
    lockdep_assert_held(&ci.i_ceph_lock);
    if (!ci.i_auth_cap) {
    ...
    return core::ptr::null_mut();
    cf.is_capsnap = false;
    cf.ci = core::ptr::null_mut();
    return cf;
    }
#[no_mangle]
pub unsafe extern "C" fn ceph_free_cap_flush(cf: *mut ceph_cap_flush) {
...
#[no_mangle]
unsafe extern "C" fn __get_oldest_flush_tid(mdsc: *mut ceph_mds_client) -> u64 {
    static u64 __get_oldest_flush_tid(struct ceph_mds_client *mdsc)
    {
...
//
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    struct ceph_cap_flush *cf = core::ptr::null_mut();
    int flushing;
    lockdep_assert_held(&ci.i_ceph_lock);
...
//
#[no_mangle]
unsafe extern "C" fn try_nonblocking_invalidate(inode: *mut inode) -> c_int {
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut invalidating_gen: u32 = ci.i_rdcache_gen;
    spin_unlock(&ci.i_ceph_lock);
...
#[no_mangle]
pub unsafe extern "C" fn __ceph_should_report_size(ci: *mut ceph_inode_info) -> bool {
    bool __ceph_should_report_size(struct ceph_inode_info *ci)
    {
    let mut size: loff_t = i_size_read(&ci.netfs.inode);
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_check_caps(ci: *mut ceph_inode_info, flags: c_int) {
    ...
    struct rb_node *p;
    let mut queue_invalidate: bool = false;
    let mut tried_invalidate: bool = false;
    let mut queue_writeback: bool = false;
    struct ceph_mds_session *session = core::ptr::null_mut();
    spin_lock(&ci.i_ceph_lock);
...
//
#[no_mangle]
unsafe extern "C" fn try_flush_caps(inode: *mut inode, ptid: *mut u64) -> c_int {
    static int try_flush_caps(struct inode *inode, u64 *ptid)
    {
    struct ceph_mds_client *mdsc = ceph_sb_to_fs_client(inode.i_sb).mdsc;
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut flushing: c_int = 0;
    let mut flush_tid: u64 = 0, oldest_flush_tid = 0;
    spin_lock(&ci.i_ceph_lock);
    retry_locked:
...
//
#[no_mangle]
unsafe extern "C" fn caps_are_flushed(inode: *mut inode, flush_tid: u64) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn flush_mdlog_and_wait_inode_unsafe_requests(inode: *mut inode) -> c_int {
    static int flush_mdlog_and_wait_inode_unsafe_requests(struct inode *inode)
    {
    struct ceph_mds_client *mdsc = ceph_sb_to_fs_client(inode.i_sb).mdsc;
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    struct ceph_mds_request *req1 = core::ptr::null_mut(), *req2 = core::ptr::null_mut();
    int ret, err = 0;
...
#[no_mangle]
pub unsafe extern "C" fn ceph_fsync(file: *mut file, start: loff_t, end: loff_t, datasync: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_write_inode(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
    ...
    let mut err: c_int = 0;
    int dirty;
    let mut wait: c_int = (wbc.sync_mode == WB_SYNC_ALL && !wbc.for_sync);
    ...
    struct ceph_cap_flush *cf;
    int ret;
    let mut first_tid: u64 = 0;
    let mut last_snap_flush: u64 = 0;
...
// first 8 bits are reserved for CEPH_FILE_MODE_FOO
    ...
    let mut snap_rwsem_locked: bool = false;
...
//
    let mut not: c_int = want & ~(have & need);
    let mut revoking: c_int = implemented & ~have;
    let mut exclude: c_int = revoking & not;
...
// got = need;
    ...
    let mut session_readonly: c_int = false;
    int mds_wanted;
    if (ci.i_auth_cap &&
...
//
#[no_mangle]
unsafe extern "C" fn check_max_size(inode: *mut inode, endoff: loff_t) {
...
#[no_mangle]
pub unsafe extern "C" fn get_used_fmode(caps: c_int) -> c_int {
    static inline int get_used_fmode(int caps)
    {
    let mut fmode: c_int = 0;
...
//
    ...
    let mut ret2: c_int = ceph_wait_on_async_create(inode);
    if (ret2 < 0)
    return ret2;
    }
    if (ret == -EFBIG) {
...
// request needs a retry
    ...
    if (page) {
    let mut uptodate: bool = PageUptodate(page);
    put_page(page);
    if (uptodate)
    break;
    }
...
// page cache
//
    ret = __ceph_do_getattr(inode, core::ptr::null_mut(),
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_get_cap_refs(ci: *mut ceph_inode_info, caps: c_int) {
...
//
    ...
    struct inode *inode = &ci.netfs.inode;
    struct ceph_client *cl = ceph_inode_to_client(inode);
    let mut last: c_int = 0, put = 0, flushsnaps = 0, wake = 0;
    let mut check_flushsnaps: bool = false;
    spin_lock(&ci.i_ceph_lock);
    if (had & CEPH_CAP_PIN)
    --ci.i_pin_ref;
...
//
    ...
    ci.i_head_snapc = core::ptr::null_mut();
    }
// see comment in __ceph_remove_cap()
    if (!__ceph_is_any_real_caps(ci) && ci.i_snap_realm)
    ceph_change_snap_realm(inode, core::ptr::null_mut());
    }
    }
...
// put the ref held by ceph_queue_cap_snap()
    put++;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: __ceph_finish_cap_snap(ci, _arg: capsnap)) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: flushsnaps) -> else {
    else if (flushsnaps)
    ceph_flush_snaps(ci, core::ptr::null_mut());
    break;
...
#[no_mangle]
pub unsafe extern "C" fn ceph_put_cap_refs(ci: *mut ceph_inode_info, had: c_int) {
    void ceph_put_cap_refs(struct ceph_inode_info *ci, int had)
    {
    __ceph_put_cap_refs(ci, had, PUT_CAP_REFS_SYNC);
    }
#[no_mangle]
pub unsafe extern "C" fn ceph_put_cap_refs_async(ci: *mut ceph_inode_info, had: c_int) {
...
//
    ...
    struct inode *inode = &ci.netfs.inode;
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_cap_snap *capsnap = core::ptr::null_mut(), *iter;
    let mut put: c_int = 0;
    let mut last: bool = false;
    let mut flush_snaps: bool = false;
    let mut complete_capsnap: bool = false;
    spin_lock(&ci.i_ceph_lock);
...
//
#[no_mangle]
unsafe extern "C" fn invalidate_aliases(inode: *mut inode) {
    static void invalidate_aliases(struct inode *inode)
    {
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct dentry *dn, *prev = core::ptr::null_mut();
    doutc(cl, "%p %llx.%llx\n", inode, ceph_vinop(inode));
...
//
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut seq: c_int = le32_to_cpu(grant.seq);
    let mut newcaps: c_int = le32_to_cpu(grant.caps);
    int used, wanted, dirty;
    let mut size: u64 = le64_to_cpu(grant.size);
    let mut max_size: u64 = le64_to_cpu(grant.max_size);
    let mut check_caps: c_uchar = 0;
    let mut was_stale: bool = cap.cap_gen < atomic_read(&session.s_cap_gen);
    let mut wake: bool = false;
    let mut writeback: bool = false;
    let mut queue_trunc: bool = false;
    let mut queue_invalidate: bool = false;
    let mut deleted_inode: bool = false;
    let mut fill_inline: bool = false;
    let mut revoke_wait: bool = false;
...
// side effects now are allowed
    ...
    let mut mode: umode_t = le32_to_cpu(grant.mode);
    ...
    grant.xattr_len) {
    let mut len: c_int = le32_to_cpu(grant.xattr_len);
    let mut version: u64 = le64_to_cpu(grant.xattr_version);
...
// file layout may have changed
    let mut old_pool: i64 = ci.i_layout.pool_id;
...
// revocation, grant, or no-op?
    ...
    let mut revoking: c_int = cap.issued & ~newcaps;
...
// re-request max_size if necessary
    ...
    spin_unlock(&ci.i_ceph_lock);
    if (fill_inline)
    ceph_fill_inline_data(inode, core::ptr::null_mut(), extra_info.inline_data,
...
#[no_mangle]
pub unsafe extern "C" fn if(2: check_caps ==) -> else {
...
//
    ...
    LIST_HEAD(to_remove);
    let mut seq: unsigned = le32_to_cpu(m.seq);
    let mut dirty: c_int = le32_to_cpu(m.dirty);
    let mut cleaned: c_int = 0;
    let mut drop: bool = false;
    let mut wake_ci: bool = false;
    let mut wake_mdsc: bool = false;
//
...
//
    ...
    struct ceph_mds_client *mdsc = ceph_sb_to_fs_client(inode.i_sb).mdsc;
    struct ceph_client *cl = mdsc.fsc.client;
    let mut follows: u64 = le64_to_cpu(m.snap_follows);
    struct ceph_cap_snap *capsnap = core::ptr::null_mut(), *iter;
    let mut wake_ci: bool = false;
    let mut wake_mdsc: bool = false;
...
//
    ...
    struct ceph_client *cl = ceph_inode_to_client(inode);
    let mut mds: c_int = session.s_mds;
    let mut seq: c_int = le32_to_cpu(trunc.seq);
    let mut truncate_seq: u32 = le32_to_cpu(trunc.truncate_seq);
    let mut truncate_size: u64 = le64_to_cpu(trunc.truncate_size);
    let mut size: u64 = le64_to_cpu(trunc.size);
    let mut implemented: c_int = 0;
    let mut dirty: c_int = __ceph_caps_dirty(ci);
    let mut issued: c_int = __ceph_caps_issued(ceph_inode(inode), &implemented);
    let mut queue_trunc: bool = false;
    lockdep_assert_held(&ci.i_ceph_lock);
...
//
    ...
    struct ceph_mds_client *mdsc = ceph_inode_to_fs_client(inode).mdsc;
    struct ceph_client *cl = mdsc.fsc.client;
    struct ceph_mds_session *tsession = core::ptr::null_mut();
    struct ceph_cap *cap, *tcap, *new_cap = core::ptr::null_mut();
    struct ceph_inode_info *ci = ceph_inode(inode);
    u64 t_cap_id;
...
// add placeholder for the export target
    let mut flag: c_int = (cap == ci.i_auth_cap) ? CEPH_CAP_FLAG_AUTH : 0;
    tcap = new_cap;
...
// open target session
    ...
    new_cap = ceph_get_cap(mdsc, core::ptr::null_mut());
    } else {
    WARN_ON(1);
    tsession = core::ptr::null_mut();
    target = -1;
    mutex_lock(&session.s_mutex);
    }
    goto retry;
...
//
    ...
    struct ceph_inode_info *ci = ceph_inode(inode);
    struct ceph_client *cl = mdsc.fsc.client;
    struct ceph_cap *cap, *ocap, *new_cap = core::ptr::null_mut();
    let mut mds: c_int = session.s_mds;
    int issued;
    let mut caps: unsigned = le32_to_cpu(im.caps);
    let mut wanted: unsigned = le32_to_cpu(im.wanted);
    let mut seq: unsigned = le32_to_cpu(im.seq);
    let mut mseq: unsigned = le32_to_cpu(im.migrate_seq);
    let mut realmino: u64 = le64_to_cpu(im.realm);
    let mut cap_id: u64 = le64_to_cpu(im.cap_id);
    u64 p_cap_id;
    let mut piseq: u32 = 0;
    let mut pmseq: u32 = 0;
    int peer;
    ...
    ocap = peer >= 0 ? __get_cap_for_mds(ci, peer) : core::ptr::null_mut();
...
//
    ...
    struct ceph_mds_cap_peer *peer = core::ptr::null_mut();
    struct ceph_snap_realm *realm = core::ptr::null_mut();
    int op;
    let mut msg_version: c_int = le16_to_cpu(msg.hdr.version);
    u32 seq, mseq, issue_seq;
    struct ceph_vino vino;
    void *snaptrace;
    size_t snaptrace_len;
    void *p, *end;
    let mut extra_info: cap_extra_info = {};
    bool queue_trunc;
    let mut close_sessions: bool = false;
    let mut do_cap_release: bool = false;
...
//
    if (do_cap_release) {
    cap = ceph_get_cap(mdsc, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_check_delayed_caps(mdsc: *mut ceph_mds_client) -> c_ulong {
    ...
    struct ceph_inode_info *ci;
    struct ceph_mount_options *opt = mdsc.fsc.mount_options;
    let mut delay_max: c_ulong = opt.caps_wanted_delay_max * HZ;
    let mut loop_start: c_ulong = jiffies;
...
//
#[no_mangle]
unsafe extern "C" fn flush_dirty_session_caps(s: *mut ceph_mds_session) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_flush_dirty_caps(mdsc: *mut ceph_mds_client) {
...
//
#[no_mangle]
unsafe extern "C" fn flush_cap_releases(s: *mut ceph_mds_session) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_flush_cap_releases(mdsc: *mut ceph_mds_client) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_get_fmode(ci: *mut ceph_inode_info, fmode: c_int, count: c_int) {
    void ceph_get_fmode(struct ceph_inode_info *ci, int fmode, int count)
    {
    struct ceph_mds_client *mdsc = ceph_sb_to_mdsc(ci.netfs.inode.i_sb);
    let mut bits: c_int = (fmode << 1) | 1;
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_put_fmode(ci: *mut ceph_inode_info, fmode: c_int, count: c_int) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_drop_caps_for_unlink(inode: *mut inode) -> c_int {
    int ceph_drop_caps_for_unlink(struct inode *inode)
    {
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut drop: c_int = CEPH_CAP_LINK_SHARED | CEPH_CAP_LINK_EXCL;
    spin_lock(&ci.i_ceph_lock);
    if (inode.i_nlink == 1) {
...
// only drop unused, clean caps
    ...
    if (cap.issued & drop) {
    let mut wanted: c_int = __ceph_caps_wanted(ci);
...
#[no_mangle]
unsafe extern "C" fn remove_capsnaps(mdsc: *mut ceph_mds_client, inode: *mut inode) -> c_int {
    static int remove_capsnaps(struct ceph_mds_client *mdsc, struct inode *inode)
    {
    struct ceph_inode_info *ci = ceph_inode(inode);
    struct ceph_client *cl = mdsc.fsc.client;
    struct ceph_cap_snap *capsnap;
    let mut capsnap_release: c_int = 0;
    lockdep_assert_held(&ci.i_ceph_lock);
    ...
    __ceph_remove_capsnap(inode, capsnap, core::ptr::null_mut(), core::ptr::null_mut());
    ceph_put_snap_context(capsnap.context);
    ceph_put_cap_snap(capsnap);
    capsnap_release++;
    }
...
#[no_mangle]
pub unsafe extern "C" fn ceph_purge_inode_cap(inode: *mut inode, cap: *mut ceph_cap, invalidate: *mut bool) -> c_int {
    ...
    struct ceph_inode_info *ci = ceph_inode(inode);
    bool is_auth;
    let mut dirty_dropped: bool = false;
    let mut iputs: c_int = 0;
    lockdep_assert_held(&ci.i_ceph_lock);
...
// make further file lock syscall return -EIO
    ...
    ci.i_prealloc_cap_flush = core::ptr::null_mut();
    if (!cf.is_capsnap)
    ceph_free_cap_flush(cf);
    }
