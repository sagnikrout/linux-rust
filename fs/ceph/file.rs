...
#[no_mangle]
unsafe extern "C" fn ceph_flags_sys2wire(mdsc: *mut ceph_mds_client, flags: u32) -> __le32 {
    static __le32 ceph_flags_sys2wire(struct ceph_mds_client *mdsc, u32 flags)
    {
    struct ceph_client *cl = mdsc.fsc.client;
    let mut wire_flags: u32 = 0;
...
//
pub const ITER_GET_BVECS_PAGES: c_int = 64;
    static ssize_t __iter_get_bvecs(struct iov_iter *iter, size_t maxsize,
    struct bio_vec *bvecs)
    {
    let mut size: usize = 0;
    let mut bvec_idx: c_int = 0;
    ...
    let mut len: c_int = min_t(int, bytes, PAGE_SIZE - start);
...
#[no_mangle]
unsafe extern "C" fn put_bvecs(bvecs: *mut bio_vec, num_bvecs: c_int, should_dirty: bool) {
...
//
    ...
    let mut want_auth: c_int = USE_ANY_MDS;
    let mut op: c_int = (flags & O_CREAT) ? CEPH_MDS_OP_CREATE : CEPH_MDS_OP_OPEN;
...
//
#[no_mangle]
unsafe extern "C" fn ceph_init_file(inode: *mut inode, file: *mut file, fmode: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_renew_caps(inode: *mut inode, fmode: c_int) -> c_int {
    ...
    flags = O_RDWR;
#[no_mangle]
pub unsafe extern "C" fn if(CEPH_CAP_FILE_RD: wanted &) -> else {
    else if (wanted & CEPH_CAP_FILE_RD)
    flags = O_RDONLY;
#[no_mangle]
pub unsafe extern "C" fn if(CEPH_CAP_FILE_WR: wanted &) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_open(inode: *mut inode, file: *mut file) -> c_int {
...
//
    ...
    let mut mds_wanted: c_int = __ceph_caps_mds_wanted(ci, true);
    let mut issued: c_int = __ceph_caps_issued(ci, core::ptr::null_mut());
...
// Clone the layout from a synchronous create, if the dir now has Dc caps
    ...
    if ((__ceph_caps_issued(cdst, core::ptr::null_mut()) & CEPH_CAP_DIR_CREATE) &&
...
//
    ...
    let mut got: c_int = 0, want = CEPH_CAP_FILE_EXCL | CEPH_CAP_DIR_CREATE;
    u64 ino;
    spin_lock(&ci.i_ceph_lock);
...
// Any delegated inos?
    ...
    goto no_async;
    if ((__ceph_caps_issued(ci, core::ptr::null_mut()) & want) != want)
    goto no_async;
...
#[no_mangle]
unsafe extern "C" fn restore_deleg_ino(dir: *mut inode, ino: u64) {
    static void restore_deleg_ino(struct inode *dir, u64 ino)
    {
    struct ceph_client *cl = ceph_inode_to_client(dir);
    struct ceph_inode_info *ci = ceph_inode(dir);
    struct ceph_mds_session *s = core::ptr::null_mut();
    spin_lock(&ci.i_ceph_lock);
    if (ci.i_auth_cap)
    s = ceph_get_mds_session(ci.i_auth_cap.session);
    spin_unlock(&ci.i_ceph_lock);
    if (s) {
    let mut err: c_int = ceph_restore_deleg_ino(s, ino);
    if (err)
    pr_warn_client(cl,
...
// MDS changed -- caller must resubmit
    ...
    if (tinode) {
    let mut ino: u64 = ceph_vino(tinode).ino;
    ...
    int ret;
    char xattr_buf[4];
    let mut in: ceph_mds_reply_inode = { };
    let mut iinfo: ceph_mds_reply_info_in = { .in = &in };
    struct ceph_inode_info *ci = ceph_inode(dir);
    struct ceph_dentry_info *di = ceph_dentry(dentry);
...
// lo is private, so pool_ns can't change
    ...
    ret = ceph_fill_inode(inode, core::ptr::null_mut(), &iinfo, core::ptr::null_mut(), req.r_session,
    req.r_fmode, core::ptr::null_mut());
...
//
    ...
    struct ceph_mds_client *mdsc = fsc.mdsc;
    struct ceph_mds_request *req;
    struct inode *new_inode = core::ptr::null_mut();
    struct dentry *dn;
    let mut as_ctx: ceph_acl_sec_ctx = {};
    let mut try_async: bool = ceph_test_mount_opt(fsc, ASYNC_DIROPS);
    int mask;
    int err;
    char *path;
...
//
    ...
    let mut fmode: c_int = ceph_flags_to_mode(flags);
    mask = MAY_READ;
    if (fmode & CEPH_FILE_MODE_WR)
    mask |= MAY_WRITE;
    err = ceph_mds_check_access(mdsc, path, mask);
...
// Hash inode before RPC
    ...
    new_inode = core::ptr::null_mut();
    err = ceph_mdsc_do_request(mdsc, (flags & O_CREAT) ? dir : core::ptr::null_mut(), req);
    if (err == -ENOENT) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_release(inode: *mut inode, file: *mut file) -> c_int {
...
//
    ...
    ssize_t ret;
    let mut off: u64 = *ki_pos;
    let mut len: u64 = iov_iter_count(to);
    let mut i_size: u64 = i_size_read(inode);
    let mut sparse: bool = IS_ENCRYPTED(inode) || ceph_test_mount_opt(fsc, SPARSEREAD);
    let mut objver: u64 = 0;
...
//
    ...
    let mut read_off: u64 = off;
    let mut read_len: u64 = len;
    int extent_cnt;
// determine new offset/length if encrypted
    ...
    CEPH_OSD_OP_READ,
    CEPH_OSD_FLAG_READ,
    core::ptr::null_mut(), ci.i_truncate_seq,
...
#[no_mangle]
pub unsafe extern "C" fn if(-ENOENT: ret ==) -> else {
    else if (ret == -ENOENT)
    ret = 0;
    if (ret < 0) {
    ceph_osdc_put_request(req);
...
// Short read but not EOF? Zero out the remainder.
    ...
    let mut zlen: c_int = min(len - ret, i_size - off - ret);
    let mut zoff: c_int = page_off + ret;
...
// last_objver = objver;
    ...
    return __ceph_sync_read(inode, &iocb.ki_pos, to, retry_op, core::ptr::null_mut());
    }
#[repr(C)]
#[derive(Copy, Clone)]
pub struct ceph_aio_request {
    pub iocb: *mut kiocb,
    pub total_len: usize,
    pub write: bool,
    pub should_dirty: bool,
    pub error: c_int,
    pub osd_reqs: list_head,
    pub num_reqs: unsigned,
    pub pending_reqs: core::sync::atomic::AtomicI32,
    pub mtime: timespec64,
    pub prealloc_cf: *mut ceph_cap_flush,
}

#[repr(C)]
#[derive(Copy, Clone)]
pub struct ceph_aio_work {
    pub work: work_struct,
    pub req: *mut ceph_osd_request,
}
    ...
    int dirty;
    let mut endoff: loff_t = aio_req.iocb.ki_pos + aio_req.total_len;
...
#[no_mangle]
unsafe extern "C" fn ceph_aio_complete_req(req: *mut ceph_osd_request) {
    static void ceph_aio_complete_req(struct ceph_osd_request *req)
    {
    let mut rc: c_int = req.r_result;
    struct inode *inode = req.r_inode;
    struct ceph_aio_request *aio_req = req.r_priv;
    struct ceph_osd_data *osd_data = osd_req_op_extent_osd_data(req, 0);
    struct ceph_osd_req_op *op = &req.r_ops[0];
    struct ceph_client_metric *metric = &ceph_sb_to_mdsc(inode.i_sb).metric;
    let mut len: c_uint = osd_data.bvec_pos.iter.bi_size;
    let mut sparse: bool = (op.op == CEPH_OSD_OP_SPARSE_READ);
    struct ceph_client *cl = ceph_inode_to_client(inode);
...
//
    ...
    let mut endoff: loff_t = aio_req.iocb.ki_pos + rc;
    if (endoff < i_size)
...
#[no_mangle]
unsafe extern "C" fn ceph_aio_retry_work(work: *mut work_struct) {
    static void ceph_aio_retry_work(struct work_struct *work)
    {
    struct ceph_aio_work *aio_work =
    ...
    struct ceph_osd_request *req;
    struct bio_vec *bvecs;
    struct ceph_aio_request *aio_req = core::ptr::null_mut();
    let mut num_pages: c_int = 0;
    int flags;
    let mut ret: c_int = 0;
    let mut mtime: timespec64 = current_time(inode);
    let mut count: usize = iov_iter_count(iter);
    let mut pos: loff_t = iocb.ki_pos;
    let mut write: bool = iov_iter_rw(iter) == WRITE;
    let mut should_dirty: bool = !write && user_backed_iter(iter);
    let mut sparse: bool = ceph_test_mount_opt(fsc, SPARSEREAD);
    ...
    let mut size: u64 = iov_iter_count(iter);
    ssize_t len;
    struct ceph_osd_req_op *op;
    let mut readop: c_int = sparse ? CEPH_OSD_OP_SPARSE_READ : CEPH_OSD_OP_READ;
    int extent_cnt;
    if (write)
...
//
    ...
    int ret;
    let mut check_caps: bool = false;
    let mut mtime: timespec64 = current_time(inode);
    let mut count: usize = iov_iter_count(from);
    ...
    size_t left;
    int n;
    let mut write_pos: u64 = pos;
    let mut write_len: u64 = len;
    u64 objnum, objoff;
    u32 xlen;
    let mut assert_ver: u64 = 0;
    bool rmw;
    bool first, last;
    let mut saved_iter: iov_iter = *from;
    size_t off;
...
// Do we need to preload the pages?
    if (rmw) {
    let mut first_pos: u64 = write_pos;
    let mut last_pos: u64 = (write_pos + write_len) - CEPH_FSCRYPT_BLOCK_SIZE;
    let mut read_len: u64 = CEPH_FSCRYPT_BLOCK_SIZE;
    struct ceph_osd_req_op *op;
...
// Grab assert version. It must be non-zero.
    ...
    let mut plen: usize = min_t(size_t, left, PAGE_SIZE - off);
// copy the data
...
//
#[no_mangle]
unsafe extern "C" fn ceph_read_iter(iocb: *mut kiocb, to: *mut iov_iter) -> isize {
    static ssize_t ceph_read_iter(struct kiocb *iocb, struct iov_iter *to)
    {
    struct file *filp = iocb.ki_filp;
    struct ceph_file_info *fi = filp.private_data;
    let mut len: usize = iov_iter_count(to);
    struct inode *inode = file_inode(filp);
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut direct_lock: bool = iocb.ki_flags & IOCB_DIRECT;
    struct ceph_client *cl = ceph_inode_to_client(inode);
    ssize_t ret;
    let mut want: c_int = 0, got = 0;
    let mut retry_op: c_int = 0, read = 0;
    again:
    ...
    loff_t i_size;
    let mut mask: c_int = CEPH_STAT_CAP_SIZE;
...
//
#[no_mangle]
unsafe extern "C" fn ceph_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    ...
    int err, want = 0, got;
    let mut direct_lock: bool = false;
    u32 map_flags;
    u64 pool_flags;
    loff_t pos;
    let mut limit: loff_t = max(i_size_read(inode), fsc.max_file_size);
    if (ceph_inode_is_shutdown(inode))
    return -ESTALE;
...
//
    if (iocb.ki_flags & IOCB_APPEND) {
    let mut cur_eof: loff_t = i_size_read(inode);
    if (cur_eof != pos) {
    doutc(cl,
...
//
    ...
    let mut max_size: loff_t = ci.i_max_size;
    spin_unlock(&ci.i_ceph_lock);
    if (pos >= max_size) {
    err = -EFBIG;
    goto out_caps;
...
//
#[no_mangle]
unsafe extern "C" fn ceph_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
    ...
    loff_t length)
    {
    let mut nearly: loff_t = round_up(offset, PAGE_SIZE);
    if (offset < nearly) {
    let mut size: loff_t = nearly - offset;
    if (length < size)
    size = length;
    ...
    if (length >= PAGE_SIZE) {
    let mut size: loff_t = round_down(length, PAGE_SIZE);
...
#[no_mangle]
unsafe extern "C" fn ceph_zero_objects(inode: *mut inode, offset: loff_t, length: loff_t) -> c_int {
    static int ceph_zero_objects(struct inode *inode, loff_t offset, loff_t length)
    {
    let mut ret: c_int = 0;
    struct ceph_inode_info *ci = ceph_inode(inode);
    let mut stripe_unit: i32 = ci.i_layout.stripe_unit;
    let mut stripe_count: i32 = ci.i_layout.stripe_count;
    let mut object_size: i32 = ci.i_layout.object_size;
    let mut object_set_size: u64 = (u64) object_size * stripe_count;
    u64 nearly, t;
// round offset up to next period boundary
    ...
    ret = ceph_zero_partial_object(inode, pos, core::ptr::null_mut());
    if (ret < 0)
    return ret;
    pos += stripe_unit;
    }
...
//
    ...
    let mut object_size: u32 = src_ci.i_layout.object_size;
    struct ceph_client *cl = fsc.client;
    int ret;
...
// dst_off += object_size;
    ...
    loff_t size;
    let mut ret: isize = -EIO, bytes;
    u64 src_objnum, dst_objnum, src_objoff, dst_objoff;
    u32 src_objlen, dst_objlen;
    let mut src_got: c_int = 0, dst_got = 0, err, dirty;
