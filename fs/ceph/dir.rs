...
//
#[no_mangle]
unsafe extern "C" fn ceph_d_init(dentry: *mut dentry) -> c_int {
...
//
pub const OFFSET_BITS: c_int = 28;

#[no_mangle]
pub unsafe extern "C" fn ceph_make_fpos(high: unsigned, off: unsigned, hash_order: bool) -> loff_t {
    loff_t ceph_make_fpos(unsigned high, unsigned off, bool hash_order)
    {
    let mut fpos: loff_t = ((loff_t)high << 28) | (loff_t)off;
    if (hash_order)
    fpos |= HASH_ORDER;
    return fpos;
    }
#[no_mangle]
unsafe extern "C" fn is_hash_order(p: loff_t) -> bool {
    static bool is_hash_order(loff_t p)
    {
    return (p & HASH_ORDER) == HASH_ORDER;
    }
#[no_mangle]
unsafe extern "C" fn fpos_frag(p: loff_t) -> unsigned {
    static unsigned fpos_frag(loff_t p)
    {
    return p >> OFFSET_BITS;
    }
#[no_mangle]
unsafe extern "C" fn fpos_hash(p: loff_t) -> unsigned {
    static unsigned fpos_hash(loff_t p)
    {
    return ceph_frag_value(fpos_frag(p));
    }
#[no_mangle]
unsafe extern "C" fn fpos_off(p: loff_t) -> unsigned {
    static unsigned fpos_off(loff_t p)
    {
    return p & OFFSET_MASK;
    }
#[no_mangle]
unsafe extern "C" fn fpos_cmp(l: loff_t, r: loff_t) -> c_int {
    static int fpos_cmp(loff_t l, loff_t r)
    {
    let mut v: c_int = ceph_frag_compare(fpos_frag(l), fpos_frag(r));
    if (v)
    return v;
...
//
    ...
    let mut idx_mask: unsigned = (PAGE_SIZE / sizeof(struct dentry *)) - 1;
    let mut ptr_pos: loff_t = idx * sizeof(struct dentry *);
    let mut ptr_pgoff: pgoff_t = ptr_pos >> PAGE_SHIFT;
    ...
    cache_ctl.folio = core::ptr::null_mut();
...
//
    ...
    struct ceph_client *cl = ceph_inode_to_client(dir);
    struct dentry *dentry, *last = core::ptr::null_mut();
    struct ceph_dentry_info *di;
...
// search start position
    if (ctx.pos > 2) {
    let mut count: u64 = div_u64(i_size_read(dir), sizeof(struct dentry *));
    while (count > 0) {
    let mut step: u64 = count >> 1;
...
// use linear search
    ...
    for (;;) {
    let mut emit_dentry: bool = false;
...
#[no_mangle]
unsafe extern "C" fn need_send_readdir(dfi: *mut ceph_dir_file_info, pos: loff_t) -> bool {
...
#[no_mangle]
unsafe extern "C" fn ceph_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
...
// can we use the dcache?
    ...
    let mut shared_gen: c_int = atomic_read(&ci.i_shared_gen);
...
// discard old result, if any
    ...
    dfi.last_readdir = core::ptr::null_mut();
    }
...
// hints to request -> mds selection code
    ...
    if (dfi.last_name) {
    let mut len: c_int = strlen(dfi.last_name);
...
// keep last_name
    ...
    false);
    kfree(dfi.last_name);
    dfi.last_name = core::ptr::null_mut();
    }
...
#[no_mangle]
unsafe extern "C" fn reset_readdir(dfi: *mut ceph_dir_file_info) {
...
//
#[no_mangle]
unsafe extern "C" fn need_reset_readdir(dfi: *mut ceph_dir_file_info, new_pos: loff_t) -> bool {
...
// dentries are sorted in hash order
    ...
    rinfo = dfi.last_readdir ? &dfi.last_readdir.r_reply_info : core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ceph_dir_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
...
// no trace?
    ...
    dentry = ERR_PTR(err);
#[no_mangle]
pub unsafe extern "C" fn if(req->r_dentry: dentry !=) -> else {
...
#[no_mangle]
unsafe extern "C" fn is_root_ceph_dentry(inode: *mut inode, dentry: *mut dentry) -> bool {
...
//
    ...
    if (IS_ENCRYPTED(dir)) {
    let mut had_key: bool = fscrypt_has_encryption_key(dir);
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_handle_notrace_create(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
//
    ...
    req.r_new_inode = core::ptr::null_mut();
    goto out_req;
    }
    ...
    {
    int err;
    let mut len: c_int = strlen(dest);
    let mut osd_link: fscrypt_str = FSTR_INIT(core::ptr::null_mut(), 0);
    ...
    struct ceph_mds_request *req;
    let mut as_ctx: ceph_acl_sec_ctx = {};
    let mut mode: umode_t = S_IFLNK | 0777;
    int err;
    if (ceph_snap(dir) != CEPH_NOSNAP)
    return -EROFS;
    err = ceph_wait_on_conflict_unlink(dentry);
...
// mkdir .snap/foo is a MKSNAP
    ...
    err = ceph_handle_notrace_create(dir, dentry);
    ret = err ? ERR_PTR(err) : core::ptr::null_mut();
    out_req:
...
#[no_mangle]
unsafe extern "C" fn get_caps_for_async_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
    static int get_caps_for_async_unlink(struct inode *dir, struct dentry *dentry)
    {
    struct ceph_inode_info *ci = ceph_inode(dir);
    struct ceph_dentry_info *di;
    let mut got: c_int = 0, want = CEPH_CAP_FILE_EXCL | CEPH_CAP_DIR_UNLINK;
    spin_lock(&ci.i_ceph_lock);
...
//
#[no_mangle]
unsafe extern "C" fn ceph_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
//
    ...
    struct ceph_client *cl = mdsc.fsc.client;
    struct ceph_mds_request *req;
    let mut op: c_int = CEPH_MDS_OP_RENAME;
    int err;
    if (flags)
    return -EINVAL;
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_dentry_lease_touch(di: *mut ceph_dentry_info) {
...
//
#[no_mangle]
pub unsafe extern "C" fn __ceph_dentry_dir_lease_touch(di: *mut ceph_dentry_info) {
...
#[no_mangle]
unsafe extern "C" fn __dentry_lease_unlist(di: *mut ceph_dentry_info) {
...
pub struct ceph_lease_walk_control {
    pub dir_lease: bool,
    pub expire_dir_lease: bool,
    pub nr_to_scan: c_ulong,
    pub dir_lease_ttl: c_ulong,
}
...
#[no_mangle]
unsafe extern "C" fn __dentry_lease_check(dentry: *const dentry) -> c_int {
    ...
    {
    struct ceph_dentry_info *di = ceph_dentry(dentry);
    let mut ret: c_int = __dir_lease_try_check(dentry);
    if (ret == -EBUSY)
    return KEEP;
    if (ret > 0) {
...
#[no_mangle]
pub unsafe extern "C" fn ceph_trim_dentries(mdsc: *mut ceph_mds_client) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_invalidate_dentry_lease(dentry: *mut dentry) {
...
//
#[no_mangle]
unsafe extern "C" fn __dentry_lease_is_valid(di: *mut ceph_dentry_info) -> bool {
    static bool __dentry_lease_is_valid(struct ceph_dentry_info *di)
    {
...
#[no_mangle]
unsafe extern "C" fn dentry_lease_is_valid(dentry: *mut dentry, flags: c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn __dir_lease_try_check(dentry: *const dentry) -> c_int {
    static int __dir_lease_try_check(const struct dentry *dentry)
    {
    struct ceph_dentry_info *di = ceph_dentry(dentry);
    struct inode *dir;
...
//
#[no_mangle]
unsafe extern "C" fn ceph_d_delete(dentry: *const dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn ceph_d_release(dentry: *mut dentry) {
...
//
#[no_mangle]
unsafe extern "C" fn ceph_d_prune(dentry: *mut dentry) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ceph_dentry_hash(dir: *mut inode, dn: *mut dentry) -> unsigned {
