...
//
#[no_mangle]
unsafe extern "C" fn pipe_clear_nowait(file: *mut file) -> noinline void {
    static noinline void pipe_clear_nowait(struct file *file)
    {
    let mut fmode: fmode_t = READ_ONCE(file.f_mode);
    do {
...
#[no_mangle]
unsafe extern "C" fn wakeup_pipe_readers(pipe: *mut pipe_inode_info) {
...
//
    ...
    {
    let mut spd_pages: c_uint = spd.nr_pages;
    let mut tail: c_uint = pipe.tail;
    let mut head: c_uint = pipe.head;
    let mut ret: isize = 0;
    let mut page_nr: c_int = 0;
    if (!spd_pages)
    return 0;
...
#[no_mangle]
pub unsafe extern "C" fn add_to_pipe(pipe: *mut pipe_inode_info, buf: *mut pipe_buffer) -> isize {
...
//
#[no_mangle]
pub unsafe extern "C" fn splice_grow_spd(pipe: *const pipe_inode_info, spd: *mut splice_pipe_desc) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn splice_shrink_spd(spd: *mut splice_pipe_desc) {
...
#[no_mangle]
unsafe extern "C" fn wakeup_pipe_writers(pipe: *mut pipe_inode_info) {
...
#[no_mangle]
pub unsafe extern "C" fn eat_empty_buffer(pipe: *mut pipe_inode_info) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn splice_from_pipe_next(pipe: *mut pipe_inode_info, sd: *mut splice_desc) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn splice_from_pipe_begin(sd: *mut splice_desc) {
...
//
#[no_mangle]
unsafe extern "C" fn splice_from_pipe_end(pipe: *mut pipe_inode_info, sd: *mut splice_desc) {
...
//
    ...
    let mut nbufs: c_int = pipe.max_usage;
...
// build the vector
    ...
    struct pipe_buffer *buf = pipe_buf(pipe, tail);
    let mut this_len: usize = buf.len;
...
//
    ...
    while (ret > 0) {
    struct pipe_buffer *buf = pipe_buf(pipe, tail);
    let mut seg: usize = min_t(size_t, ret, buf.len);
    buf.offset += seg;
    buf.len -= seg;
    ret -= seg;
    if (!buf.len) {
...
//
#[no_mangle]
unsafe extern "C" fn do_splice_eof(sd: *mut splice_desc) {
...
//
    ...
    while (len) {
    size_t read_len;
    let mut pos: loff_t = sd.pos, prev_pos = pos;
...
#[no_mangle]
unsafe extern "C" fn direct_file_splice_eof(sd: *mut splice_desc) {
...
#[no_mangle]
unsafe extern "C" fn wait_for_space(pipe: *mut pipe_inode_info, flags: unsigned) -> c_int {
...
//
    ...
    struct pipe_inode_info *ipipe;
    struct pipe_inode_info *opipe;
    loff_t offset, *__off_in = core::ptr::null_mut(), *__off_out = core::ptr::null_mut();
    ssize_t ret;
    ipipe = get_pipe_info(in, true);
    opipe = get_pipe_info(out, true);
    if (ipipe) {
    ...
    for (i = 0; i < n; i++) {
    let mut size: c_int = umin(left, PAGE_SIZE - start);
    buf.page = pages[i];
    buf.offset = start;
    buf.len = size;
...
// this one got dropped by add_to_pipe()
    ...
    struct splice_desc *sd)
    {
    let mut n: c_int = copy_page_to_iter(buf.page, buf.offset, sd.len, sd.u.data);
    let mut n: return = = sd.len ? n : -EFAULT;
    }
//
...
//
    ...
    type = ITER_SOURCE;
#[no_mangle]
pub unsafe extern "C" fn if(FMODE_READ: fd_file(f)->f_mode &) -> else {
    ...
    error = 0;
#[no_mangle]
pub unsafe extern "C" fn if(ITER_SOURCE: type ==) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn ipipe_prep(pipe: *mut pipe_inode_info, flags: c_uint) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn opipe_prep(pipe: *mut pipe_inode_info, flags: c_uint) -> c_int {
...
//
    ...
    let mut ret: c_int = 0;
    let mut input_wakeup: bool = false;
    retry:
