...
#[no_mangle]
unsafe extern "C" fn item_depth(item: *mut *mut config_item) -> c_int {
    static int item_depth(struct config_item * item)
    {
    let mut p: *mut config_item = item;
    let mut depth: c_int = 0;
    do { depth++; } while ((p = p.ci_parent) && !configfs_is_root(p));
    return depth;
    }
#[no_mangle]
unsafe extern "C" fn item_path_length(item: *mut *mut config_item) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn fill_item_path(item: *mut *mut config_item, buffer: *mut *mut c_char, length: c_int) {
    static void fill_item_path(struct config_item * item, char * buffer, int length)
    {
    struct config_item * p;
    --length;
    for (p = item; p && !configfs_is_root(p); p = p.ci_parent) {
    let mut cur: c_int = strlen(config_item_name(p));
...
// target = configfs_get_config_item(path.dentry);
    ...
    struct config_item *target_item = core::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn configfs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
