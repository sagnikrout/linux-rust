...
//
pub const SIMPLE_ATTR_SIZE: c_int = 4096;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct configfs_buffer {
    pub count: usize,
    pub pos: loff_t,
    pub page: *mut *mut c_char,
    pub ops: *const configfs_item_operations,
    pub mutex: mutex,
    pub needs_read_fill: c_int,
    pub read_in_progress: bool,
    pub write_in_progress: bool,
    pub bin_buffer: *mut c_char,
    pub bin_buffer_size: c_int,
    pub cb_max_size: c_int,
...
#[no_mangle]
unsafe extern "C" fn fill_read_buffer(file: *mut file, buffer: *mut configfs_buffer) -> c_int {
    static int fill_read_buffer(struct file *file, struct configfs_buffer *buffer)
    {
    struct configfs_fragment *frag = to_frag(file);
    let mut count: isize = -ENOENT;
    if (!buffer.page)
...
#[no_mangle]
unsafe extern "C" fn configfs_read_iter(iocb: *mut kiocb, to: *mut iov_iter) -> isize {
...
#[no_mangle]
unsafe extern "C" fn configfs_bin_read_iter(iocb: *mut kiocb, to: *mut iov_iter) -> isize {
...
// perform first read with buf == NULL to get extent
    down_read(&frag.frag_sem);
    if (!frag.frag_dead)
    len = buffer.bin_attr.read(buffer.item, core::ptr::null_mut(), 0);
    else
    len = -ENOENT;
    up_read(&frag.frag_sem);
    if (len <= 0) {
...
// do not exceed the maximum value
    ...
    if (buffer.bin_buffer == core::ptr::null_mut()) {
    retval = -ENOMEM;
    goto out;
    }
...
// perform second read to fill buffer
    ...
    buffer.bin_buffer = core::ptr::null_mut();
    goto out;
    }
    buffer.needs_read_fill = 0;
    }
...
//
#[no_mangle]
unsafe extern "C" fn configfs_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
...
#[no_mangle]
unsafe extern "C" fn __configfs_open_file(inode: *mut inode, file: *mut file, type: c_int) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn configfs_release(inode: *mut inode, filp: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn configfs_open_file(inode: *mut inode, filp: *mut file) -> c_int {
    static int configfs_open_file(struct inode *inode, struct file *filp)
    {
    return __configfs_open_file(inode, filp, CONFIGFS_ITEM_ATTR);
    }
#[no_mangle]
unsafe extern "C" fn configfs_open_bin_file(inode: *mut inode, filp: *mut file) -> c_int {
    static int configfs_open_bin_file(struct inode *inode, struct file *filp)
    {
    return __configfs_open_file(inode, filp, CONFIGFS_ITEM_BIN_ATTR);
    }
#[no_mangle]
unsafe extern "C" fn configfs_release_bin_file(inode: *mut inode, file: *mut file) -> c_int {
...
// result of ->release() is ignored
    ...
    .llseek		= core::ptr::null_mut(),		/* bin file is not seekable */
...
//
#[no_mangle]
pub unsafe extern "C" fn configfs_create_file(item: *mut *mut config_item, attr: *const *const configfs_attribute) -> c_int {
    ...
    error = configfs_make_dirent(parent_sd, core::ptr::null_mut(), (void *) attr, mode,
...
//
    ...
    struct dentry *dir = item.ci_dentry;
    struct configfs_dirent *parent_sd = dir.d_fsdata;
    let mut mode: umode_t = (bin_attr.cb_attr.ca_mode & S_IALLUGO) | S_IFREG;
    let mut error: c_int = 0;
    inode_lock_nested(dir.d_inode, I_MUTEX_NORMAL);
    error = configfs_make_dirent(parent_sd, core::ptr::null_mut(), (void *) bin_attr, mode,
