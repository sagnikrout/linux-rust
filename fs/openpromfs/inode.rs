...
pub const OPENPROM_ROOT_INO: c_int = 0;
    enum op_inode_type {
    op_inode_node,
    op_inode_prop,
    };
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct op_inode_info {
    pub vfs_inode: inode,
    pub type: enum op_inode_type,
    pub u: union op_inode_data,
}
...
#[no_mangle]
unsafe extern "C" fn is_string(p: *mut c_uchar, len: c_int) -> c_int {
    static int is_string(unsigned char *p, int len)
    {
    int i;
    for (i = 0; i < len; i++) {
    let mut val: c_uchar = p[i];
    if ((i && !val) ||
    (val >= ' ' && val <= '~'))
    continue;
    return 0;
...
#[no_mangle]
unsafe extern "C" fn property_show(f: *mut seq_file, v: *mut c_void) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn property_stop(f: *mut seq_file, v: *mut c_void) {
...
unsafe extern "C" fn property_open(inode: *mut inode, file: *mut file) -> c_int {
    ...
    let mut n: c_int = strlen(node_name);
    if (len == n &&
    ...
    prop = dp.properties;
    while (prop) {
    let mut n: c_int = strlen(prop.name);
...
#[no_mangle]
unsafe extern "C" fn openprom_free_inode(inode: *mut inode) {
...
#[no_mangle]
unsafe extern "C" fn openpromfs_reconfigure(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn openprom_fill_super(s: *mut super_block, fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn openpromfs_get_tree(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn openpromfs_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn op_inode_init_once(data: *mut c_void) {
...
#[no_mangle]
unsafe extern "C" fn init_openprom_fs() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_openprom_fs() -> void __exit {
