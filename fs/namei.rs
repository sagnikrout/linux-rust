...
#[no_mangle]
pub unsafe extern "C" fn filename_init() -> void __init {
    ...
    EMBEDDED_NAME_MAX, core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn free_filename(p: *mut filename) {
    static inline void free_filename(struct filename *p)
    {
    kmem_cache_free(names_cache, p);
    }
#[no_mangle]
pub unsafe extern "C" fn initname(name: *mut filename) {
    static inline void initname(struct filename *name)
    {
    name.aname = core::ptr::null_mut();
    name.refcnt = 1;
    }
#[no_mangle]
unsafe extern "C" fn getname_long(name: *mut filename, filename: *const char __user) -> c_int {
...
//
    ...
    {
    let mut flags: c_int = (uflags & AT_EMPTY_PATH) ? LOOKUP_EMPTY : 0;
...
// empty pathname translates to NULL
    ...
    {
    struct filename *result;
    let mut len: c_int = strlen(filename) + 1;
    char *p;
...
#[no_mangle]
pub unsafe extern "C" fn putname(name: *mut filename) {
...
#[no_mangle]
pub unsafe extern "C" fn delayed_getname(v: *mut delayed_filename, string: *const char __user) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn putname_to_delayed(v: *mut delayed_filename, name: *mut filename) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn dismiss_delayed_filename(v: *mut delayed_filename) {
...
// no ->get_inode_acl() calls in RCU mode...
    ...
    return PTR_ERR(acl);
    if (acl) {
    let mut error: c_int = posix_acl_permission(idmap, inode, acl, mask);
...
//
#[no_mangle]
pub unsafe extern "C" fn no_acl_inode(inode: *mut inode) -> bool {
...
//
    ...
    struct inode *inode, int mask)
    {
    let mut mode: c_uint = inode.i_mode;
    vfsuid_t vfsuid;
//
...
// Do we have ACL's?
    ...
    let mut error: c_int = check_acl(idmap, inode, mask);
    if (error != -EAGAIN)
    return error;
    }
...
//
    if (mask & (mode ^ (mode >> 3))) {
    let mut vfsgid: vfsgid_t = i_gid_into_vfsgid(idmap, inode);
...
//
#[no_mangle]
unsafe extern "C" fn sb_permission(sb: *mut super_block, inode: *mut inode, mask: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn path_get(path: *const path) {
...
//
#[no_mangle]
pub unsafe extern "C" fn path_put(path: *const path) {
...
pub const EMBEDDED_LEVELS: c_int = 2;
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nameidata {
    ...
    pub last: qstr,
    pub root: path,
    pub /: *mut *mut *mut inode inode; / path.dentry.d_inode,
    pub state: unsigned int flags,,
    pub r_seq: unsigned seq, next_seq, m_seq,,
    pub last_type: enum last_type,
    pub depth: unsigned,
    pub total_link_count: c_int,
#[repr(C)]
#[derive(Copy, Clone)]
pub struct saved {
    pub link: path,
    ...
    pub seq: unsigned,
    pub internal: [*mut *mut } stack,; EMBEDDED_LEVELS],
    pub name: *mut filename,
    pub pathname: *const c_char,
    pub saved: *mut nameidata,
    pub root_seq: unsigned,
    pub dfd: c_int,
    pub dir_vfsuid: vfsuid_t,
    pub dir_mode: umode_t,
    pub __randomize_layout: },
pub const ND_ROOT_PRESET: c_int = 1;
pub const ND_ROOT_GRABBED: c_int = 2;
pub const ND_JUMPED: c_int = 4;
#[no_mangle]
unsafe extern "C" fn __set_nameidata(p: *mut nameidata, dfd: c_int, name: *mut filename) {
    static void __set_nameidata(struct nameidata *p, int dfd, struct filename *name)
    {
    pub current->nameidata: *mut *mut nameidata old =,
    pub p->internal: p->stack =,
    pub 0: p->depth =,
    pub dfd: p->dfd =,
    pub name: p->name =,
    pub "": p->pathname = likely(name) ? name->name :,
    pub NULL: p->path.mnt =,
    pub NULL: p->path.dentry =,
    pub 0: p->total_link_count = old ? old->total_link_count :,
    pub old: p->saved =,
    pub p: current->nameidata =,
    }
    static inline void set_nameidata(struct nameidata *p, int dfd, struct filename *name,
    const struct path *root)
    {
    pub name): __set_nameidata(p, dfd,,
    pub 0: p->state =,
    if (unlikely(root)) {
    pub ND_ROOT_PRESET: p->state =,
    pub root: *mut p->root =,
    }
    }
#[no_mangle]
unsafe extern "C" fn restore_nameidata() {
    static void restore_nameidata(void)
    {
    pub now->saved: *mut *mut *mut nameidata now = current->nameidata, old =,
    pub old: current->nameidata =,
    if (old)
    pub now->total_link_count: old->total_link_count =,
    if (now.stack != now.internal)
    }
#[no_mangle]
unsafe extern "C" fn nd_alloc_stack(nd: *mut nameidata) -> bool {
    static bool nd_alloc_stack(struct nameidata *nd)
    {
    pub p: *mut saved,
    p= kmalloc_objs(struct saved, MAXSYMLINKS,
    pub GFP_KERNEL): nd->flags & LOOKUP_RCU ? GFP_ATOMIC :,
    if (unlikely(!p))
    pub false: return,
    pub sizeof(nd->internal)): memcpy(p, nd->internal,,
    pub p: nd->stack =,
    pub true: return,
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn path_connected(mnt: *mut vfsmount, dentry: *mut dentry) -> bool {
    static bool path_connected(struct vfsmount *mnt, struct dentry *dentry)
    {
    pub mnt->mnt_sb: *mut *mut super_block sb =,
// Bind mounts can have disconnected paths
    if (mnt.mnt_root == sb.s_root)
    pub true: return,
    pub mnt->mnt_root): return is_subdir(dentry,,
    }
#[no_mangle]
unsafe extern "C" fn drop_links(nd: *mut nameidata) {
    static void drop_links(struct nameidata *nd)
    {
    pub nd->depth: int i =,
    while (i--) {
    pub i: *mut *mut saved last = nd->stack +,
    }
    }
#[no_mangle]
unsafe extern "C" fn leave_rcu(nd: *mut nameidata) {
    static void leave_rcu(struct nameidata *nd)
    {
    pub ~LOOKUP_RCU: nd->flags &=,
    pub 0: nd->seq = nd->next_seq =,
    }
#[no_mangle]
unsafe extern "C" fn terminate_walk(nd: *mut nameidata) {
    static void terminate_walk(struct nameidata *nd)
    {
    if (unlikely(nd.depth))
    if (!(nd.flags & LOOKUP_RCU)) {
    pub i: c_int,
    pub i++): for (i = 0; i < nd->depth;,
    if (nd.state & ND_ROOT_GRABBED) {
    pub ~ND_ROOT_GRABBED: nd->state &=,
    }
    } else {
    }
    pub 0: nd->depth =,
    pub NULL: nd->path.mnt =,
    pub NULL: nd->path.dentry =,
    }
// path_put is needed afterwards regardless of success or failure
#[no_mangle]
unsafe extern "C" fn __legitimize_path(path: *mut path, seq: unsigned, mseq: unsigned) -> bool {
    static bool __legitimize_path(struct path *path, unsigned seq, unsigned mseq)
    {
    pub mseq): int res = __legitimize_mnt(path->mnt,,
    if (unlikely(res)) {
    if (res > 0)
    pub NULL: path->mnt =,
    pub NULL: path->dentry =,
    pub false: return,
    }
    ...
    pub seq): return !read_seqcount_retry(&path->dentry->d_seq,,
    }
    static inline bool legitimize_path(struct nameidata *nd,
    struct path *path, unsigned seq)
    {
    pub nd->m_seq): return __legitimize_path(path, seq,,
    }
#[no_mangle]
unsafe extern "C" fn legitimize_links(nd: *mut nameidata) -> bool {
    static bool legitimize_links(struct nameidata *nd)
    {
    pub i: c_int,
    pub LOOKUP_CACHED): VFS_BUG_ON(nd->flags &,
    pub {: for (i = 0; i < nd->depth; i++),
    pub i: *mut *mut saved last = nd->stack +,
...
#[no_mangle]
unsafe extern "C" fn legitimize_root(nd: *mut nameidata) -> bool {
...
// Nothing to do if nd->root is zero or is managed by the VFS user.
    ...
    pub true: return,
    pub ND_ROOT_GRABBED: nd->state |=,
    pub nd->root_seq): return legitimize_path(nd, &nd->root,,
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn try_to_unlazy(nd: *mut nameidata) -> bool {
    static bool try_to_unlazy(struct nameidata *nd)
    {
    pub nd->path.dentry: *mut *mut dentry parent =,
    pub LOOKUP_RCU)): VFS_BUG_ON(!(nd->flags &,
    ...
    pub out: goto,
    pub parent->d_inode): BUG_ON(nd->inode !=,
    pub true: return,
    out1:
    pub NULL: nd->path.mnt =,
    pub NULL: nd->path.dentry =,
...
//
#[no_mangle]
unsafe extern "C" fn try_to_unlazy_next(nd: *mut nameidata, dentry: *mut dentry) -> bool {
    ...
    if (unlikely(nd.depth && !legitimize_links(nd)))
    pub out2: goto,
    pub nd->m_seq): res = __legitimize_mnt(nd->path.mnt,,
    if (unlikely(res)) {
    if (res > 0)
    pub out2: goto,
    pub out1: goto,
...
//
    ...
    pub flags): return dentry->d_op->d_revalidate(dir, name, dentry,,
    else
    pub 1: return,
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn complete_walk(nd: *mut nameidata) -> c_int {
    static int complete_walk(struct nameidata *nd)
    {
    pub nd->path.dentry: *mut *mut dentry dentry =,
    pub status: c_int,
    if (nd.flags & LOOKUP_RCU) {
//
...
//
    ...
    pub NULL: nd->root.mnt =,
    pub ~LOOKUP_CACHED: nd->flags &=,
    if (!try_to_unlazy(nd))
    pub -ECHILD: return,
    }
...
//
    ...
    pub 0: return,
    pub nd->flags): status = dentry->d_op->d_weak_revalidate(dentry,,
    if (status > 0)
    pub 0: return,
    if (!status)
    pub -ESTALE: status =,
...
#[no_mangle]
unsafe extern "C" fn set_root(nd: *mut nameidata) -> c_int {
    static int set_root(struct nameidata *nd)
    {
    pub current->fs: *mut *mut fs_fs =,
//
...
//
    ...
    pub -ENOTRECOVERABLE: return,
    if (nd.flags & LOOKUP_RCU) {
    pub seq: unsigned,
    do {
    pub read_seqbegin(&fs->seq): seq =,
    pub fs->root: nd->root =,
    pub __read_seqcount_begin(&nd->root.dentry->d_seq): nd->root_seq =,
    pub seq)): } while (read_seqretry(&fs->seq,,
    } else {
    pub &nd->root): get_fs_root(fs,,
    pub ND_ROOT_GRABBED: nd->state |=,
    }
    pub 0: return,
    }
#[no_mangle]
unsafe extern "C" fn nd_jump_root(nd: *mut nameidata) -> c_int {
...
// Absolute path arguments to path_init() are allowed.
    if (nd.path.mnt != core::ptr::null_mut() && nd.path.mnt != nd.root.mnt)
    pub -EXDEV: return,
    }
    if (!nd.root.mnt) {
    pub set_root(nd): int error =,
    if (unlikely(error))
    pub error: return,
    }
    if (nd.flags & LOOKUP_RCU) {
    pub d: *mut dentry,
    pub nd->root: nd->path =,
    pub nd->path.dentry: d =,
    pub d->d_inode: nd->inode =,
    pub nd->root_seq: nd->seq =,
    if (read_seqcount_retry(&d.d_seq, nd.seq))
    pub -ECHILD: return,
    } else {
    pub nd->root: nd->path =,
    pub nd->path.dentry->d_inode: nd->inode =,
    }
    pub ND_JUMPED: nd->state |=,
    pub 0: return,
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn nd_jump_link(path: *const path) -> c_int {
    int nd_jump_link(const struct path *path)
    {
    pub -ELOOP: int error =,
    pub current->nameidata: *mut *mut nameidata nd =,
...
#[no_mangle]
pub unsafe extern "C" fn put_link(nd: *mut nameidata) {
    static inline void put_link(struct nameidata *nd)
    {
    pub --nd->depth: *mut *mut saved last = nd->stack +,
    if (!(nd.flags & LOOKUP_RCU))
    }
    pub __read_mostly: static int sysctl_protected_symlinks,
    pub __read_mostly: static int sysctl_protected_hardlinks,
    pub __read_mostly: static int sysctl_protected_fifos,
    pub __read_mostly: static int sysctl_protected_regular,
...
#[no_mangle]
unsafe extern "C" fn init_fs_namei_sysctls() -> int __init {
...
//
#[no_mangle]
pub unsafe extern "C" fn may_follow_link(nd: *mut nameidata, inode: *const inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn may_linkat(idmap: *mut mnt_idmap, link: *const path) -> c_int {
...
//
    ...
    {
    let mut dir_mode: umode_t = nd.dir_mode;
    let mut dir_vfsuid: vfsuid_t = nd.dir_vfsuid, i_vfsuid;
...
//
#[no_mangle]
pub unsafe extern "C" fn follow_up(path: *mut path) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn follow_automount(path: *mut path, count: *mut c_int, lookup_flags: unsigned) -> c_int {
...
//
    ...
    let mut need_mntput: bool = false;
    let mut ret: c_int = 0;
...
// jumped = need_mntput;
    ...
    int *count, unsigned lookup_flags)
    {
    let mut flags: unsigned = smp_load_acquire(&path.dentry.d_flags);
// fastpath
...
#[no_mangle]
pub unsafe extern "C" fn follow_down_one(path: *mut path) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn follow_down(path: *mut path, flags: c_uint) -> c_int {
    int follow_down(struct path *path, unsigned int flags)
    {
    struct vfsmount *mnt = path.mnt;
    bool jumped;
    let mut ret: c_int = traverse_mounts(path, &jumped, core::ptr::null_mut(), flags);
    if (path.mnt != mnt)
    mntput(mnt);
    return ret;
    }
...
//
#[no_mangle]
unsafe extern "C" fn __follow_mount_rcu(nd: *mut nameidata, path: *mut path) -> bool {
    static bool __follow_mount_rcu(struct nameidata *nd, struct path *path)
    {
    struct dentry *dentry = path.dentry;
    let mut flags: c_uint = dentry.d_flags;
    if (unlikely(nd.flags & LOOKUP_NO_XDEV))
    return false;
    for (;;) {
//
...
//
    ...
    let mut res: c_int = dentry.d_op.d_manage(path, true);
    if (res)
    let mut res: return = = -EISDIR;
...
// this state.
    ...
    path.dentry = dentry;
    if (nd.flags & LOOKUP_RCU) {
    let mut seq: c_uint = nd.next_seq;
...
//
    ...
    if (dentry) {
    let mut error: c_int = d_revalidate(dir.d_inode, name, dentry, flags);
    if (unlikely(error <= 0)) {
    if (!error)
    d_invalidate(dentry);
    dput(dentry);
...
// Don't go there if it's already dead
    ...
    let mut error: c_int = d_revalidate(inode, name, dentry, flags);
    if (unlikely(error <= 0)) {
    if (!error) {
    d_invalidate(dentry);
    dput(dentry);
...
#[no_mangle]
unsafe extern "C" fn reserve_stack(nd: *mut nameidata, link: *mut path) -> c_int {
...
// unlazy even if we fail to grab the link - cleanup needs it
    let mut grabbed_link: bool = legitimize_path(nd, link, nd.next_seq);
...
// make sure that d_is_symlink from step_into_slowpath() matches the inode
    ...
    if (nd.flags & LOOKUP_RCU) {
    res = get(core::ptr::null_mut(), inode, &last.done);
...
// rare case of legitimate dget_parent()...
    ...
    {
    if (type == LAST_DOTDOT) {
    const char *error = core::ptr::null_mut();
    struct dentry *parent;
    if (!nd.root.mnt) {
...
//
#[no_mangle]
pub unsafe extern "C" fn full_name_hash(salt: *const c_void, name: *const c_char, len: c_uint) -> c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn hashlen_string(salt: *const c_void, name: *const c_char) -> u64 {
    u64 hashlen_string(const void *salt, const char *name)
    {
    let mut a: c_ulong = 0, x = 0, y = (unsigned long)salt;
...
pub unsafe extern "C" fn full_name_hash(salt: *const c_void, name: *const c_char, len: c_uint) -> c_uint {
    unsigned int full_name_hash(const void *salt, const char *name, unsigned int len)
    {
    let mut hash: c_ulong = init_name_hash(salt);
    while (len--)
...
//
    ...
    {
    let mut hash: c_ulong = init_name_hash(nd.path.dentry);
    let mut len: c_ulong = 0, c, last = 0;
    c = (unsigned char)*name;
    do {
    last = (last << 8) + c;
    len++;
...
pub const LAST_WORD_IS_DOT: c_uint = 0x2e;
pub const LAST_WORD_IS_DOTDOT: c_uint = 0x2e2e;
...
//
#[no_mangle]
unsafe extern "C" fn link_path_walk(name: *const c_char, nd: *mut nameidata) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn handle_lookup_down(nd: *mut nameidata) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn path_lookupat(nd: *mut nameidata, flags: unsigned, path: *mut path) -> c_int {
    ...
    (s = lookup_last(nd)) != core::ptr::null_mut())
    ;
    ...
    if (!err) {
// path = nd->path;
    nd.path.mnt = core::ptr::null_mut();
    nd.path.dentry = core::ptr::null_mut();
    }
    terminate_walk(nd);
    return err;
    }
...
// Returns 0 and nd will be valid on success; Returns error, otherwise.
    ...
    {
    const char *s = path_init(nd, flags);
    let mut err: c_int = link_path_walk(s, nd);
    if (!err)
    err = complete_walk(nd);
    if (!err) {
// parent = nd->path;
...
// type = nd.last_type;
    ...
    struct qstr *last, enum last_type *type)
    {
    return __filename_parentat(dfd, name, flags, parent, last, type, core::ptr::null_mut());
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn end_dirop(de: *mut dentry) {
...
#[no_mangle]
pub unsafe extern "C" fn kern_path(name: *const c_char, flags: c_uint, path: *mut path) -> c_int {
    int kern_path(const char *name, unsigned int flags, struct path *path)
    {
    CLASS(filename_kernel, filename)(name);
    return filename_lookup(AT_FDCWD, filename, flags, path, core::ptr::null_mut());
    }
...
//
    ...
    {
    CLASS(filename_kernel, filename)(name);
    let mut root: path = {.mnt = mnt, .dentry = dentry};
// the first argument of filename_lookup() is ignored with root
    return filename_lookup(AT_FDCWD, filename, flags, path, &root);
    }
    EXPORT_SYMBOL(vfs_path_lookup);
#[no_mangle]
pub unsafe extern "C" fn lookup_noperm_common(qname: *mut qstr, base: *mut dentry) -> c_int {
    int lookup_noperm_common(struct qstr *qname, struct dentry *base)
    {
    const char *name = qname.name;
    let mut len: u32 = qname.len;
    ...
    return -EACCES;
    while (len--) {
    let mut c: c_uint = *(const unsigned char *)name++;
    if (c == '/' || c == '\0')
    return -EACCES;
    }
//
...
//
    ...
    let mut err: c_int = base.d_op.d_hash(base, qname);
    if (err < 0)
    return err;
    }
    return 0;
...
//
    ...
    struct qstr *name)
    {
    let mut err: c_int = lookup_one_common(idmap, name, parent);
    if (err)
    return ERR_PTR(err);
...
//
    ...
    struct qstr *name)
    {
    let mut err: c_int = lookup_noperm_common(name, parent);
    if (err)
    return ERR_PTR(err);
    return start_dirop(parent, name, LOOKUP_CREATE);
    }
...
#[no_mangle]
pub unsafe extern "C" fn path_pts(path: *mut path) -> c_int {
...
//
    ...
    return filename_lookup(dfd, filename, flags, path, core::ptr::null_mut());
    }
    EXPORT_SYMBOL(user_path_at);
    int __check_sticky(struct mnt_idmap *idmap, struct inode *dir,
    struct inode *inode)
    {
    let mut fsuid: kuid_t = current_fsuid();
...
#[no_mangle]
unsafe extern "C" fn unlock_rename(p1: *mut dentry, p2: *mut dentry) {
...
//
    ...
    struct dentry *trap;
    struct dentry *d1, *d2;
    let mut target_flags: c_int = LOOKUP_RENAME_TARGET | LOOKUP_CREATE;
    int err;
...
#[no_mangle]
pub unsafe extern "C" fn end_renaming(rd: *mut renamedata) {
...
//
    ...
    let mut error: c_int = may_create_dentry(&nop_mnt_idmap, dir, dentry);
    if (error)
    return error;
    mode &= S_IALLUGO;
    mode |= S_IFREG;
...
#[no_mangle]
pub unsafe extern "C" fn may_open_dev(path: *const path) -> bool {
...
#[no_mangle]
unsafe extern "C" fn handle_truncate(idmap: *mut mnt_idmap, filp: *mut file) -> c_int {
    static int handle_truncate(struct mnt_idmap *idmap, struct file *filp)
    {
    const struct path *path = &filp.f_path;
    struct inode *inode = path.dentry.d_inode;
    let mut error: c_int = get_write_access(inode);
    if (error)
    return error;
...
#[no_mangle]
pub unsafe extern "C" fn open_to_namei_flags(flag: c_int) -> c_int {
    ...
    umode_t mode)
    {
    let mut error: c_int = security_path_mknod(dir, dentry, mode, 0);
    if (error)
    return error;
...
//
    ...
    {
    let mut DENTRY_NOT_SET: *mut dentry const = (void *) -1UL;
...
//
    ...
    {
    let mut delegated_inode: delegated_inode = { };
    struct mnt_idmap *idmap;
...
#[no_mangle]
pub unsafe extern "C" fn trailing_slashes(nd: *mut nameidata) -> bool {
...
// Discard negative dentries. Need inode_lock to do the create
    ...
    {
    let mut open_flag: c_int = op.open_flag;
    struct dentry *dentry;
    const char *res;
    nd.flags |= op.intent;
...
//
    ...
    struct inode *inode;
    int error;
    let mut open_flag: c_int = file.f_flags;
...
//
    ...
    {
    struct path path;
    let mut error: c_int = path_lookupat(nd, flags | LOOKUP_DIRECTORY, &path);
    if (unlikely(error))
    return error;
...
#[no_mangle]
unsafe extern "C" fn do_o_path(nd: *mut nameidata, flags: unsigned, file: *mut file) -> c_int {
    static int do_o_path(struct nameidata *nd, unsigned flags, struct file *file)
    {
    struct path path;
    let mut error: c_int = path_lookupat(nd, flags, &path);
    if (!error) {
    ...
    (s = open_last_lookups(nd, file, op)) != core::ptr::null_mut())
    ;
    if (!error)
    ...
    {
    struct nameidata nd;
    let mut flags: c_int = op.lookup_flags;
    struct file *filp;
    if (IS_ERR(pathname))
    return ERR_CAST(pathname);
    set_nameidata(&nd, dfd, pathname, core::ptr::null_mut());
    ...
    struct qstr last;
    let mut want_dir: bool = lookup_flags & LOOKUP_DIRECTORY;
    let mut reval_flag: c_uint = lookup_flags & LOOKUP_REVAL;
    let mut create_flags: c_uint = LOOKUP_CREATE | LOOKUP_EXCL;
    enum last_type type;
    int error;
...
//
#[no_mangle]
pub unsafe extern "C" fn end_creating_path(path: *const path, dentry: *mut dentry) {
...
// Drop the extra reference
    ...
    error = vfs_create(mnt_idmap(path.mnt), path.dentry, mode, core::ptr::null_mut());
    if (!error)
    error = vfs_open(path, file);
    }
    if (unlikely(error))
...
//
    ...
    {
    let mut is_whiteout: bool = S_ISCHR(mode) && dev == WHITEOUT_DEV;
    let mut error: c_int = may_create_dentry(idmap, dir, dentry);
    if (error)
    return error;
...
#[no_mangle]
unsafe extern "C" fn may_mknod(mode: umode_t) -> c_int {
    ...
    unsigned int dev)
    {
    let mut di: delegated_inode = { };
    struct mnt_idmap *idmap;
    struct dentry *dentry;
    struct path path;
    int error;
    let mut lookup_flags: c_uint = 0;
...
//
    ...
    {
    int error;
    let mut max_links: unsigned = dir.i_sb.s_max_links;
    struct dentry *de;
    error = may_create_dentry(idmap, dir, dentry);
    if (error)
    goto err;
...
#[no_mangle]
pub unsafe extern "C" fn filename_mkdirat(dfd: c_int, name: *mut filename, mode: umode_t) -> c_int {
    int filename_mkdirat(int dfd, struct filename *name, umode_t mode)
    {
    struct dentry *dentry;
    struct path path;
    int error;
    let mut lookup_flags: c_uint = LOOKUP_DIRECTORY;
    let mut delegated_inode: delegated_inode = { };
    retry:
    dentry = filename_create(dfd, name, &path, lookup_flags);
    if (IS_ERR(dentry))
...
//
    ...
    {
    let mut error: c_int = may_delete_dentry(idmap, dir, dentry, true);
    if (error)
    return error;
    if (!dir.i_op.rmdir)
    return -EPERM;
...
#[no_mangle]
pub unsafe extern "C" fn filename_rmdir(dfd: c_int, name: *mut filename) -> c_int {
...
//
    ...
    let mut error: c_int = may_delete_dentry(idmap, dir, dentry, false);
    if (error)
    return error;
    if (!dir.i_op.unlink)
    return -EPERM;
    ...
    error = -EPERM;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: is_local_mountpoint(dentry)) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn filename_unlinkat(dfd: c_int, name: *mut filename) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn filename_symlinkat(from: *mut filename, newdfd: c_int, to: *mut filename) -> c_int {
...
// Make sure we don't allow creating hardlink to an unlinked file
    ...
    error =  -ENOENT;
#[no_mangle]
pub unsafe extern "C" fn if(max_links: max_links && inode->i_nlink >=) -> else {
...
//
    ...
    how |= LOOKUP_FOLLOW;
    retry:
    error = filename_lookup(olddfd, old, how, &old_path, core::ptr::null_mut());
    if (error)
    return error;
...
//
#[no_mangle]
pub unsafe extern "C" fn vfs_rename(rd: *mut renamedata) -> c_int {
    ...
    let mut flags: c_uint = rd.flags;
    let mut is_dir: bool = d_is_dir(old_dentry);
    struct inode *source = old_dentry.d_inode;
    struct inode *target = new_dentry.d_inode;
    let mut new_is_dir: bool = false;
    let mut max_links: unsigned = new_dir.i_sb.s_max_links;
...
//
    ...
    !(flags & RENAME_EXCHANGE) ? target : core::ptr::null_mut(), old_dentry);
    if (flags & RENAME_EXCHANGE) {
    fsnotify_move(new_dir, old_dir, &old_dentry.d_name,
    new_is_dir, core::ptr::null_mut(), new_dentry);
    }
    }
...
#[no_mangle]
pub unsafe extern "C" fn readlink_copy(buffer: *mut char __user, buflen: c_int, link: *const c_char, linklen: c_int) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn vfs_readlink(dentry: *mut dentry, buffer: *mut char __user, buflen: c_int) -> c_int {
...
// get the link contents into pagecache
    ...
    folio = read_mapping_folio(mapping, 0, core::ptr::null_mut());
    if (IS_ERR(folio))
...
//
#[no_mangle]
pub unsafe extern "C" fn page_put_link(arg: *mut c_void) {
...
#[no_mangle]
pub unsafe extern "C" fn page_readlink(dentry: *mut dentry, buffer: *mut char __user, buflen: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn page_symlink(inode: *mut inode, symname: *const c_char, len: c_int) -> c_int {
    int page_symlink(struct inode *inode, const char *symname, int len)
    {
    struct address_space *mapping = inode.i_mapping;
    const struct address_space_operations *aops = mapping.a_ops;
    let mut nofs: bool = !mapping_gfp_constraint(mapping, __GFP_FS);
    struct folio *folio;
    ...
    err = aops.write_begin(core::ptr::null_mut(), mapping, 0, len-1, &folio, &fsdata);
    if (nofs)
    memalloc_nofs_restore(flags);
    if (err)
    goto fail;
    memcpy(folio_address(folio), symname, len - 1);
    err = aops.write_end(core::ptr::null_mut(), mapping, 0, len - 1, len - 1,
    folio, fsdata);
    if (err < 0)
    goto fail;
    if (err < len-1)
