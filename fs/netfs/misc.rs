...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_free_folioq_buffer(fq: *mut folio_queue) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_reset_iter(subreq: *mut netfs_io_subrequest) {
    void netfs_reset_iter(struct netfs_io_subrequest *subreq)
    {
    struct iov_iter *io_iter = &subreq.io_iter;
    let mut remain: usize = subreq.len - subreq.transferred;
    if (io_iter.count > remain)
    iov_iter_advance(io_iter, io_iter.count - remain);
#[no_mangle]
pub unsafe extern "C" fn if(remain: io_iter->count <) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_dirty_folio(mapping: *mut address_space, folio: *mut folio) -> bool {
    bool netfs_dirty_folio(struct address_space *mapping, struct folio *folio)
    {
    struct inode *inode = mapping.host;
    struct netfs_inode *ictx = netfs_inode(inode);
    struct fscache_cookie *cookie = netfs_i_cookie(ictx);
    let mut need_use: bool = false;
    _enter("");
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_unpin_writeback(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
    int netfs_unpin_writeback(struct inode *inode, struct writeback_control *wbc)
    {
    struct fscache_cookie *cookie = netfs_i_cookie(netfs_inode(inode));
    if (wbc.unpinned_netfs_wb)
    fscache_unuse_cookie(cookie, core::ptr::null_mut(), core::ptr::null_mut());
    return 0;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_clear_inode_writeback(inode: *mut inode, aux: *const c_void) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_invalidate_folio(folio: *mut folio, offset: usize, length: usize) {
    void netfs_invalidate_folio(struct folio *folio, size_t offset, size_t length)
    {
    struct netfs_folio *finfo;
    struct inode *inode = folio_inode(folio);
    struct netfs_inode *ctx = netfs_inode(inode);
    let mut flen: usize = folio_size(folio);
    _enter("{%lx},%zx,%zx", folio.index, offset, length);
    if (offset == 0 && length == flen) {
    unsigned long long i_size, remote_i_size, zero_point;
    let mut fpos: c_ulonglong = folio_pos(folio), end;
...
// We have a partially uptodate page from a streaming write.
    let mut fstart: c_uint = finfo.dirty_offset;
    let mut fend: c_uint = fstart + finfo.dirty_len;
    let mut iend: c_uint = offset + length;
    if (offset >= fend)
    return;
    if (iend <= fstart)
    return;
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_release_folio(folio: *mut folio, gfp: gfp_t) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_wake_collector(rreq: *mut netfs_io_request) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_subreq_clear_in_progress(subreq: *mut netfs_io_subrequest) {
...
//
    ...
    {
    let mut need_collect: bool = false, inactive = true, done = true;
...
//
    ...
    trace_netfs_failure(rreq, core::ptr::null_mut(), ret, netfs_fail_short_read);
    ret = -EIO;
    }
    break;
    }
...
#[no_mangle]
pub unsafe extern "C" fn netfs_wait_for_read(rreq: *mut netfs_io_request) -> isize {
    ssize_t netfs_wait_for_read(struct netfs_io_request *rreq)
    {
    return netfs_wait_for_in_progress(rreq, netfs_read_collection);
    }
#[no_mangle]
pub unsafe extern "C" fn netfs_wait_for_write(rreq: *mut netfs_io_request) -> isize {
...
#[no_mangle]
pub unsafe extern "C" fn netfs_wait_for_paused_read(rreq: *mut netfs_io_request) {
    void netfs_wait_for_paused_read(struct netfs_io_request *rreq)
    {
    return netfs_wait_for_pause(rreq, netfs_read_collection);
    }
#[no_mangle]
pub unsafe extern "C" fn netfs_wait_for_paused_write(rreq: *mut netfs_io_request) {
