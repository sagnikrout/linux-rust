...
//! Source: fs/netfs/write_collect.c
#![no_std]
...
pub const NEED_REASSESS: c_uint = 0x02	/* Need to loop round and reassess */;
pub const MADE_PROGRESS: c_uint = 0x04	/* Made progress cleaning up a stream or the folio set */;
pub const NEED_UNLOCK: c_uint = 0x08	/* The pagecache needs unlocking */;
pub const NEED_RETRY: c_uint = 0x10	/* A front op requests retrying */;
pub const SAW_FAILURE: c_uint = 0x20	/* One stream or hit a permanent failure */;
#[no_mangle]
unsafe extern "C" fn netfs_dump_request(rreq: *const netfs_io_request) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_folio_written_back(folio: *mut folio) -> c_int {
    int netfs_folio_written_back(struct folio *folio)
    {
    let mut why: enum netfs_folio_trace = netfs_folio_trace_clear;
    struct inode *inode = folio_inode(folio);
    struct netfs_inode *ictx = netfs_inode(inode);
    struct netfs_folio *finfo;
    struct netfs_group *group = core::ptr::null_mut();
    let mut gcount: c_int = 0;
...
//
    ...
    let mut collected_to: c_ulonglong = wreq.collected_to;
    let mut slot: c_uint = wreq.buffer.first_tail_slot;
...
//
#[no_mangle]
unsafe extern "C" fn netfs_collect_write_results(wreq: *mut netfs_io_request) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_write_collection(wreq: *mut netfs_io_request) -> bool {
    bool netfs_write_collection(struct netfs_io_request *wreq)
    {
    struct netfs_inode *ictx = netfs_inode(wreq.inode);
    size_t transferred;
    let mut transferred_valid: bool = false;
    int s;
...
// As we cleared NETFS_RREQ_IN_PROGRESS, we acquired its ref.
    ...
    if (wreq.iocb) {
    let mut written: usize = min(wreq.transferred, wreq.len);
...
#[no_mangle]
pub unsafe extern "C" fn netfs_write_collection_worker(work: *mut work_struct) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_write_subrequest_terminated(_op: *mut c_void, transferred_or_error: isize) {
