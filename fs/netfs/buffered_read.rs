...
//! Source: fs/netfs/buffered_read.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn netfs_begin_cache_read(rreq: *mut netfs_io_request, ctx: *mut netfs_inode) -> c_int {
...
//
    ...
    let mut rsize: usize = subreq.len;
...
//
    ...
    do {
    struct netfs_io_subrequest *subreq;
    let mut source: enum netfs_io_source = NETFS_SOURCE_UNKNOWN;
    ssize_t slice;
    ...
    let mut zero_point: c_ulonglong = netfs_read_zero_point(rreq.inode);
    let mut zp: c_ulonglong = umin(zero_point, rreq.i_size);
    let mut len: usize = subreq.len;
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_readahead(ractl: *mut readahead_control) {
    void netfs_readahead(struct readahead_control *ractl)
    {
    struct netfs_io_request *rreq;
    struct netfs_inode *ictx = netfs_inode(ractl.mapping.host);
    let mut start: c_ulonglong = readahead_pos(ractl);
    let mut size: usize = readahead_length(ractl);
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn netfs_read_gaps(file: *mut file, folio: *mut folio) -> c_int {
    ...
    struct folio *sink = core::ptr::null_mut();
    struct bio_vec *bvec;
    let mut from: c_uint = finfo.dirty_offset;
    let mut to: c_uint = from + finfo.dirty_len;
    let mut off: c_uint = 0, i = 0;
    let mut flen: usize = folio_size(folio);
    let mut nr_bvec: usize = flen / PAGE_SIZE + 2;
    size_t part;
    int ret;
...
//
    ...
    netfs_read_to_pagecache(rreq, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_read_folio(file: *mut file, folio: *mut folio) -> c_int {
...
//
    ...
    let mut offset: usize = offset_in_folio(folio, pos);
    let mut plen: usize = folio_size(folio);
...
// Allow the netfs (eg. ceph) to flush conflicts.
    ...
    if (ret < 0) {
    trace_netfs_failure(core::ptr::null_mut(), core::ptr::null_mut(), ret, netfs_fail_check_write_begin);
    goto error;
    }
    if (!folio)
    goto retry;
...
//
    ...
    struct address_space *mapping = folio.mapping;
    struct netfs_inode *ctx = netfs_inode(mapping.host);
    let mut start: c_ulonglong = folio_pos(folio);
    let mut flen: usize = folio_size(folio);
    int ret;
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_buffered_read_iter(iocb: *mut kiocb, iter: *mut iov_iter) -> isize {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_file_read_iter(iocb: *mut kiocb, iter: *mut iov_iter) -> isize {
