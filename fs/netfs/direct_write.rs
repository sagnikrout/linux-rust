...
//! Source: fs/netfs/direct_write.c
#![no_std]
...
//
#[no_mangle]
unsafe extern "C" fn netfs_unbuffered_write_done(wreq: *mut netfs_io_request) {
...
// counter.
//
    let mut first: pgoff_t = wreq.start >> PAGE_SHIFT;
    let mut last: pgoff_t = (wreq.start + wreq.transferred - 1) >> PAGE_SHIFT;
...
// As we cleared NETFS_RREQ_IN_PROGRESS, we acquired its ref.
    if (wreq.iocb) {
    let mut written: usize = umin(wreq.transferred, wreq.len);
...
//
#[no_mangle]
unsafe extern "C" fn netfs_unbuffered_write(wreq: *mut netfs_io_request) -> c_int {
    static int netfs_unbuffered_write(struct netfs_io_request *wreq)
    {
    struct netfs_io_subrequest *subreq = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn netfs_unbuffered_write_async(work: *mut work_struct) {
...
//
    ...
    let mut start: c_ulonglong = iocb.ki_pos;
    let mut end: c_ulonglong = start + iov_iter_count(iter);
    ssize_t ret, n;
    let mut len: usize = iov_iter_count(iter);
    let mut async: bool = !is_sync_kiocb(iocb);
    _enter("");
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_unbuffered_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    ...
    let mut end: c_ulonglong = pos + iov_iter_count(from) - 1;
...
//
    ...
    fscache_invalidate(netfs_i_cookie(ictx), core::ptr::null_mut(), i_size_read(inode),
    FSCACHE_INVAL_DIO_WRITE);
    ret = netfs_unbuffered_write_iter_locked(iocb, from, core::ptr::null_mut());
    out:
