...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct netfs_wb_waiter {
    pub /: *mut *mut list_head link; / Link in ictx->wb_queue,
    pub /: *mut *mut *mut task_waiter; / Waiter task; cleared when lock granted,
}
...
//
#[no_mangle]
unsafe extern "C" fn netfs_inode_dio_wait_interruptible(inode: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn netfs_block_o_direct(ictx: *mut netfs_inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_start_io_read(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_end_io_read(inode: *mut inode) {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_start_io_write(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_end_io_write(inode: *mut inode) {
...
// Call with exclusively locked inode->i_rwsem
#[no_mangle]
unsafe extern "C" fn netfs_block_buffered(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_start_io_direct(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_end_io_direct(inode: *mut inode) {
...
//
#[no_mangle]
unsafe extern "C" fn netfs_wb_begin_wait(ictx: *mut netfs_inode) -> bool {
    static bool netfs_wb_begin_wait(struct netfs_inode *ictx)
    {
    let mut waiter: netfs_wb_waiter = {};
...
// Read waiter before accessing inode state.
    if (smp_load_acquire(&waiter.waiter) == core::ptr::null_mut())
    break;
    schedule();
    }
    }
...
// Write inode state before clearing waiter.
    smp_store_release(&waiter.waiter, core::ptr::null_mut());
    wake_up_process(tsk);
    put_task_struct(tsk);
    } else {
