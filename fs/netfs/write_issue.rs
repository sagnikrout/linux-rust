...
//
    ...
    do {
    let mut why: enum netfs_folio_trace = netfs_folio_trace_kill;
    struct netfs_group *group = core::ptr::null_mut();
    struct netfs_folio *finfo = core::ptr::null_mut();
    void *priv;
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_prepare_write_failed(subreq: *mut netfs_io_subrequest) {
...
//
    ...
    {
    let mut size: usize = subreq.len - subreq.transferred;
...
//
    ...
    let mut iter_off: usize = 0;
    let mut fsize: usize = folio_size(folio), flen = fsize, foff = 0;
    let mut fpos: loff_t = folio_pos(folio), i_size;
    let mut to_eof: bool = false, streamw = false;
...
//
    for (;;) {
    ssize_t part;
    let mut lowest_off: usize = ULONG_MAX;
    let mut choose_s: c_int = -1;
...
//
#[no_mangle]
unsafe extern "C" fn netfs_end_issue_write(wreq: *mut netfs_io_request) {
    static void netfs_end_issue_write(struct netfs_io_request *wreq)
    {
    let mut needs_poke: bool = true;
...
//
    ...
    struct netfs_io_request *wreq = core::ptr::null_mut();
    struct folio *folio;
    let mut error: c_int = 0;
    if (!netfs_wb_begin(ictx, wbc.sync_mode == WB_SYNC_NONE))
    return 0;
// Need the first folio to be able to set up the op.
    folio = writeback_iter(mapping, wbc, core::ptr::null_mut(), &error);
    if (!folio)
    goto out;
    wreq = netfs_create_write_req(mapping, core::ptr::null_mut(), folio_pos(folio), NETFS_WRITEBACK);
    if (IS_ERR(wreq)) {
    error = PTR_ERR(wreq);
    goto couldnt_start;
    }
...
// It appears we don't have to handle cyclic writeback wrapping.
    ...
    WARN_ON_ONCE(folio != core::ptr::null_mut());
    } else {
...
//
    ...
    struct netfs_io_stream *stream;
    let mut iter_off: usize = 0;
    let mut fsize: usize = folio_size(folio), flen;
    let mut fpos: loff_t = folio_pos(folio);
    ssize_t ret;
    let mut to_eof: bool = false;
...
//
    ...
    let mut size: usize = iov_iter_count(iter);
    int ret;
...
// The VFS will have undirtied the inode.
    ...
    wreq = netfs_create_write_req(mapping, core::ptr::null_mut(), 0, NETFS_WRITEBACK_SINGLE);
    if (IS_ERR(wreq)) {
    ret = PTR_ERR(wreq);
    goto couldnt_start;
    }
    ...
    let mut part: usize = umin(folioq_folio_size(fq, slot), size);
    _debug("wbiter %lx %llx", folio.index, atomic64_read(&wreq.issued_to));
