...
//! Source: fs/netfs/buffered_write.c
#![no_std]
...
//
    ...
    fscache_update_cookie(ctx.cache, core::ptr::null_mut(), &end);
...
//
    ...
    struct folio *folio = core::ptr::null_mut(), *writethrough = core::ptr::null_mut();
    let mut bdp_flags: c_uint = (iocb.ki_flags & IOCB_NOWAIT) ? BDP_ASYNC : 0;
    let mut written: isize = 0, ret, ret2;
    let mut pos: loff_t = iocb.ki_pos;
    let mut max_chunk: usize = mapping_max_folio_size(mapping);
    let mut maybe_trouble: bool = false;
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_file_write_iter(iocb: *mut kiocb, from: *mut iov_iter) -> isize {
    ...
    if (ret > 0)
    ret = netfs_buffered_write_iter_locked(iocb, from, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn netfs_page_mkwrite(vmf: *mut vm_fault, netfs_group: *mut netfs_group) -> vm_fault_t {
...
// Can we see a streaming write here?
    ...
    folio_detach_private(folio);
#[no_mangle]
pub unsafe extern "C" fn if(NETFS_FOLIO_COPY_TO_CACHE: netfs_group && priv ==) -> else {
    else if (netfs_group && priv == NETFS_FOLIO_COPY_TO_CACHE)
    folio_change_private(folio, netfs_get_group(netfs_group));
#[no_mangle]
pub unsafe extern "C" fn if(!priv: netfs_group &&) -> else {
