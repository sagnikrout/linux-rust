...
//
    ...
    let mut path: path = {.mnt = mnt, .dentry = dir};
    if (nop.get_name)
...
//
    ...
    void *context)
    {
    struct dentry *dentry, *toput = core::ptr::null_mut();
    struct inode *inode;
...
#[no_mangle]
unsafe extern "C" fn dentry_connected(dentry: *mut dentry) -> bool {
...
#[no_mangle]
unsafe extern "C" fn clear_disconnected(dentry: *mut dentry) {
...
pub struct getdents_callback {
    pub ctx: dir_context,
    pub a: *mut *mut *mut char name; / name that was found. It already points to,
    buffer NAME_MAX+1 is size */
    pub /: *mut *mut u64 ino; / the inum we are looking for,
    pub /: *mut *mut int found; / inode matched?,
    pub /: *mut *mut int sequence; / sequence counter,
}
...
//
#[no_mangle]
unsafe extern "C" fn get_name(path: *const path, name: *mut c_char, child: *mut dentry) -> c_int {
    static int get_name(const struct path *path, char *name, struct dentry *child)
    {
...
//
    ...
    buffer.sequence = 0;
    while (1) {
    let mut old_seq: c_int = buffer.sequence;
...
pub const FILEID_INO64_GEN_LEN: c_int = 3;
//
...
//
    ...
    {
    int error;
    struct dentry *p = core::ptr::null_mut();
    struct inode *inode = dentry.d_inode, *parent = core::ptr::null_mut();
