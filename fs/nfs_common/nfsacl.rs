...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfsacl_encode_desc {
    pub desc: xdr_array2_desc,
    pub count: c_uint,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct nfsacl_simple_acl {
    pub acl: posix_acl_hdr,
    pub ace: [posix_acl_entry; 4],
}
...
//
    ...
    {
    let mut entries: c_int = (acl && acl.a_count) ? max_t(int, acl.a_count, 4) : 0;
    struct nfsacl_encode_desc nfsacl_desc = {
    .desc = {
    .elem_size = 12,
...
//
    ...
    int typeflag)
    {
    let mut elem_size: usize = XDR_UNIT * 3;
    let mut entries: u32 = (acl && acl.a_count) ? max_t(int, acl.a_count, 4) : 0;
    struct nfsacl_encode_desc nfsacl_desc = {
    .desc = {
    .elem_size = elem_size,
    .array_len = encode_entries ? entries : 0,
...
#[no_mangle]
pub unsafe extern "C" fn if(uid_gt(a->e_uid: (a->e_tag == ACL_USER) &&, _arg: b->e_uid)) -> else {
    else if ((a.e_tag == ACL_USER) && uid_gt(a.e_uid, b.e_uid))
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(uid_lt(a->e_uid: (a->e_tag == ACL_USER) &&, _arg: b->e_uid)) -> else {
    else if ((a.e_tag == ACL_USER) && uid_lt(a.e_uid, b.e_uid))
    return -1;
#[no_mangle]
pub unsafe extern "C" fn if(gid_gt(a->e_gid: (a->e_tag == ACL_GROUP) &&, _arg: b->e_gid)) -> else {
    else if ((a.e_tag == ACL_GROUP) && gid_gt(a.e_gid, b.e_gid))
    return 1;
#[no_mangle]
pub unsafe extern "C" fn if(gid_lt(a->e_gid: (a->e_tag == ACL_GROUP) &&, _arg: b->e_gid)) -> else {
...
// group_obj = NULL, *mask = NULL;
    ...
    cmp_acl_entry, core::ptr::null_mut());
...
//
    ...
    .desc = {
    .elem_size = 12,
    .xcode = pacl ? xdr_nfsace_decode : core::ptr::null_mut(),
    },
    };
    u32 entries;
    int err;
