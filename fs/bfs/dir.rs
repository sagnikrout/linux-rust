...
#[no_mangle]
unsafe extern "C" fn bfs_readdir(f: *mut file, ctx: *mut dir_context) -> c_int {
    ...
    if (de.ino) {
    let mut size: c_int = strnlen(de.name, BFS_NAMELEN);
    ...
    if (bh) {
    let mut ino: c_ulong = (unsigned long)le16_to_cpu(de.ino);
    brelse(bh);
...
#[no_mangle]
unsafe extern "C" fn bfs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
    ...
    return -EINVAL;
    old_bh = new_bh = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn bfs_add_entry(dir: *mut inode, child: *const qstr, ino: c_int) -> c_int {
    static int bfs_add_entry(struct inode *dir, const struct qstr *child, int ino)
    {
    ...
    struct bfs_dirent **res_dir)
    {
    let mut block: c_ulong = 0, offset = 0;
