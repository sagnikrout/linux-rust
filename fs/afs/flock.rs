...
pub const AFS_LOCK_GRANTED: c_int = 0;
pub const AFS_LOCK_PENDING: c_int = 1;
pub const AFS_LOCK_YOUR_TRY: c_int = 2;
...
#[no_mangle]
pub unsafe extern "C" fn afs_set_lock_state(vnode: *mut afs_vnode, state: enum afs_lock_state) {
...
pub unsafe extern "C" fn afs_lock_may_be_available(vnode: *mut afs_vnode) {
    ...
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_callback_break, 0);
...
//
#[no_mangle]
unsafe extern "C" fn afs_schedule_lock_extension(vnode: *mut afs_vnode) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_lock_op_done(call: *mut afs_call) {
    ...
    if (call.error == 0) {
    spin_lock(&vnode.lock);
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_timestamp, 0);
...
//
#[no_mangle]
unsafe extern "C" fn afs_grant_locks(vnode: *mut afs_vnode) {
    static void afs_grant_locks(struct afs_vnode *vnode)
    {
    struct file_lock *p, *_p;
    let mut exclusive: bool = (vnode.lock_type == AFS_LOCK_WRITE);
...
//
#[no_mangle]
unsafe extern "C" fn afs_next_locker(vnode: *mut afs_vnode, error: c_int) {
    static void afs_next_locker(struct afs_vnode *vnode, int error)
    {
    struct file_lock *p, *_p, *next = core::ptr::null_mut();
    struct key *key = vnode.lock_key;
    let mut type: c_uint = F_RDLCK;
    _enter("");
...
// Select the next locker to hand off to.
    ...
    vnode.lock_key = core::ptr::null_mut();
    key_put(key);
    if (next) {
    ...
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_no_lockers, 0);
    }
    _leave("");
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn afs_kill_lockers_enoent(vnode: *mut afs_vnode) {
...
#[no_mangle]
unsafe extern "C" fn afs_lock_success(op: *mut afs_operation) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_extend_lock(vnode: *mut afs_vnode, key: *mut key) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_release_lock(vnode: *mut afs_vnode, key: *mut key) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_lock_work(work: *mut work_struct) {
    ...
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_work_unlocking, 0);
    spin_unlock(&vnode.lock);
// attempt to release the server lock; if it fails, we just
// wait 5 minutes and it'll expire anyway
    ret = afs_release_lock(vnode, vnode.lock_key);
    if (ret < 0 && vnode.lock_state != AFS_VNODE_LOCK_DELETED) {
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_release_fail,
    ret);
...
//
    ...
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_work_extending, 0);
    spin_unlock(&vnode.lock);
    ret = afs_extend_lock(vnode, key); /* RPC */
    key_put(key);
    if (ret < 0) {
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_extend_fail,
    ret);
...
//
#[no_mangle]
unsafe extern "C" fn afs_defer_unlock(vnode: *mut afs_vnode) {
    ...
    trace_afs_flock_ev(vnode, core::ptr::null_mut(), afs_flock_defer_unlock, 0);
...
//
#[no_mangle]
unsafe extern "C" fn afs_do_setlk(file: *mut file, fl: *mut file_lock) -> c_int {
    static int afs_do_setlk(struct file *file, struct file_lock *fl)
    {
    struct inode *inode = file_inode(file);
    struct afs_vnode *vnode = AFS_FS_I(inode);
    let mut mode: enum afs_flock_mode = AFS_FS_S(inode.i_sb).flock_mode;
    afs_lock_type_t type;
...
//
#[no_mangle]
unsafe extern "C" fn afs_do_unlk(file: *mut file, fl: *mut file_lock) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_do_getlk(file: *mut file, fl: *mut file_lock) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_lock(file: *mut file, cmd: c_int, fl: *mut file_lock) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_flock(file: *mut file, cmd: c_int, fl: *mut file_lock) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_fl_copy_lock(new: *mut file_lock, fl: *mut file_lock) {
    static void afs_fl_copy_lock(struct file_lock *new, struct file_lock *fl)
    {
...
//
#[no_mangle]
unsafe extern "C" fn afs_fl_release_private(fl: *mut file_lock) {
    static void afs_fl_release_private(struct file_lock *fl)
    {
    struct afs_vnode *vnode = AFS_FS_I(file_inode(fl.c.flc_file));
    _enter("");
