...
//
#[no_mangle]
pub unsafe extern "C" fn afs_check_validity(vnode: *const afs_vnode) -> bool {
    bool afs_check_validity(const struct afs_vnode *vnode)
    {
    const struct afs_volume *volume = vnode.volume;
    let mut trace: enum afs_vnode_invalid_trace = afs_vnode_valid_trace;
    let mut cb_expires_at: time64_t = atomic64_read(&vnode.cb_expires_at);
    let mut deadline: time64_t = ktime_get_real_seconds() + 10;
    if (test_bit(AFS_VNODE_DELETED, &vnode.flags))
    return true;
    if (atomic_read(&volume.cb_v_check) != atomic_read(&volume.cb_v_break))
    trace = afs_vnode_invalid_trace_cb_v_break;
#[no_mangle]
pub unsafe extern "C" fn if(AFS_NO_CB_PROMISE: cb_expires_at ==) -> else {
    else if (cb_expires_at == AFS_NO_CB_PROMISE)
    trace = afs_vnode_invalid_trace_no_cb_promise;
#[no_mangle]
pub unsafe extern "C" fn if(deadline: cb_expires_at <=) -> else {
    else if (cb_expires_at <= deadline)
    trace = afs_vnode_invalid_trace_expired;
#[no_mangle]
pub unsafe extern "C" fn if(deadline: volume->cb_expires_at <=) -> else {
    else if (volume.cb_expires_at <= deadline)
    trace = afs_vnode_invalid_trace_vol_expired;
#[no_mangle]
pub unsafe extern "C" fn if(atomic_read(&volume->cb_ro_snapshot): vnode->cb_ro_snapshot !=) -> else {
    else if (vnode.cb_ro_snapshot != atomic_read(&volume.cb_ro_snapshot))
    trace = afs_vnode_invalid_trace_cb_ro_snapshot;
#[no_mangle]
pub unsafe extern "C" fn if(atomic_read(&volume->cb_scrub): vnode->cb_scrub !=) -> else {
    else if (vnode.cb_scrub != atomic_read(&volume.cb_scrub))
    trace = afs_vnode_invalid_trace_cb_scrub;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: test_bit(AFS_VNODE_ZAP_DATA, _arg: &vnode->flags)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn __afs_is_server_excluded(op: *mut afs_operation, volume: *mut afs_volume) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn afs_is_server_excluded(op: *mut afs_operation, volume: *mut afs_volume) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_update_volume_creation_time(op: *mut afs_operation, volume: *mut afs_volume) -> c_int {
    static int afs_update_volume_creation_time(struct afs_operation *op, struct afs_volume *volume)
    {
    unsigned int snap;
    let mut cur: time64_t = volume.creation_time;
    let mut old: time64_t = op.pre_volsync.creation;
    let mut new: time64_t = op.volsync.creation;
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn afs_update_volume_update_time(op: *mut afs_operation, volume: *mut afs_volume) {
    static void afs_update_volume_update_time(struct afs_operation *op, struct afs_volume *volume)
    {
    let mut reason: enum afs_cb_break_reason = afs_cb_break_no_break;
    let mut cur: time64_t = volume.update_time;
    let mut old: time64_t = op.pre_volsync.update;
    let mut new: time64_t = op.volsync.update;
    _enter("%llx,%llx,%llx.%llx", volume.vid, cur, old, new);
    if (cur == TIME64_MIN) {
...
#[no_mangle]
unsafe extern "C" fn afs_update_volume_times(op: *mut afs_operation, volume: *mut afs_volume) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_update_volume_state(op: *mut afs_operation) -> c_int {
    ...
    let mut cb_v_break: c_uint = atomic_read(&volume.cb_v_break);
    let mut cb_v_check: c_uint = atomic_read(&volume.cb_v_check);
    int ret;
    ...
    let mut expires_at: time64_t = cb.expires_at;
...
//
#[no_mangle]
unsafe extern "C" fn afs_zap_data(vnode: *mut afs_vnode) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_validate(vnode: *mut afs_vnode, key: *mut key) -> c_int {
    int afs_validate(struct afs_vnode *vnode, struct key *key)
    {
    struct afs_volume *volume = vnode.volume;
    unsigned int cb_ro_snapshot, cb_scrub;
    let mut deadline: time64_t = ktime_get_real_seconds() + 10;
    let mut zap: bool = false, locked_vol = false;
    int ret;
...
//
    ...
    afs_zap_data(vnode);
#[no_mangle]
pub unsafe extern "C" fn if(_arg: S_ISLNK(vnode->netfs.inode.i_mode)) -> else {
