...
//
#[no_mangle]
unsafe extern "C" fn afs_finished_vl_probe(server: *mut afs_vlserver) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_done_one_vl_probe(server: *mut afs_vlserver, wake_up: bool) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_vlserver_probe_result(call: *mut afs_call) {
    void afs_vlserver_probe_result(struct afs_call *call)
    {
    struct afs_addr_list *alist = call.vl_probe;
    struct afs_vlserver *server = call.vlserver;
    struct afs_address *addr = &alist.addrs[call.probe_index];
    let mut server_index: c_uint = call.server_index;
    let mut rtt_us: c_uint = 0;
    let mut index: c_uint = call.probe_index;
    let mut have_result: bool = false;
...
//
    ...
    struct afs_vlserver *server;
    let mut rtt: c_uint = UINT_MAX, rtt_s;
    let mut have_responders: bool = false;
    let mut pref: c_int = -1, i;
...
// Only wait for servers that have a probe outstanding.
    ...
    for (;;) {
    let mut still_probing: bool = false;
