...
//
    ...
    let mut afs_mntpt_expiry_timeout: static unsigned long = 10 * 60;
...
//
#[no_mangle]
unsafe extern "C" fn afs_mntpt_open(inode: *mut inode, file: *mut file) -> c_int {
    static int afs_mntpt_open(struct inode *inode, struct file *file)
    {
...
//
#[no_mangle]
unsafe extern "C" fn afs_mntpt_set_params(fc: *mut fs_context, mntpt: *mut dentry) -> c_int {
...
// if the directory is a pseudo directory, use the d_name
    let mut size: unsigned = mntpt.d_name.len;
    if (size < 2)
    return -ENOENT;
    p = mntpt.d_name.name;
    ...
    return -ENAMETOOLONG;
    cell = afs_lookup_cell(ctx.net, p, size, core::ptr::null_mut(),
...
// read the contents of the AFS special symlink
    ...
    const char *content;
    let mut size: loff_t = i_size_read(d_inode(mntpt));
    if (src_as.cell)
...
//
#[no_mangle]
unsafe extern "C" fn afs_mntpt_expiry_timed_out(work: *mut work_struct) {
    static void afs_mntpt_expiry_timed_out(struct work_struct *work)
    {
    _enter("");
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_mntpt_kill_timer() {
