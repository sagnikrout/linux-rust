...
//
#[no_mangle]
unsafe extern "C" fn afs_alloc_anon_key(cell: *mut afs_cell) -> c_int {
...
//
    ...
    cell.net.net, core::ptr::null_mut());
    if (IS_ERR(key)) {
...
// act as anonymous user
    if (!cell.anonymous_key)
    return core::ptr::null_mut(); /* Need to allocate */
    _leave(" = {%x} [anon]", key_serial(cell.anonymous_key));
    return key_get(cell.anonymous_key);
    } else {
// act as authorised user
...
//
#[no_mangle]
unsafe extern "C" fn afs_permits_rcu(rcu: *mut rcu_head) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_put_permits(permits: *mut afs_permits) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_clear_permits(vnode: *mut afs_vnode) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_hash_permits(permits: *mut afs_permits) {
    static void afs_hash_permits(struct afs_permits *permits)
    {
    let mut h: c_ulong = permits.nr_permits;
    int i;
    for (i = 0; i < permits.nr_permits; i++) {
...
//
    ...
    {
    struct afs_permits *permits, *xpermits, *replacement, *zap, *new = core::ptr::null_mut();
    let mut caller_access: afs_access_t = scb.status.caller_access;
...
#[no_mangle]
pub unsafe extern "C" fn afs_clean_up_permit_cache() -> void __exit {
