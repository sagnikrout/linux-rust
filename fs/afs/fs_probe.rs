...
//
    ...
    let mut afs_fs_probe_fast_poll_interval: static unsigned int = 30 * HZ;
    let mut afs_fs_probe_slow_poll_interval: static unsigned int = 5 * 60 * HZ;
...
#[no_mangle]
unsafe extern "C" fn afs_endpoint_state_rcu(rcu: *mut rcu_head) {
...
#[no_mangle]
pub unsafe extern "C" fn afs_put_endpoint_state(estate: *mut afs_endpoint_state, where: enum afs_estate_trace) {
    void afs_put_endpoint_state(struct afs_endpoint_state *estate, enum afs_estate_trace where)
    {
    if (estate) {
    let mut server_id: c_uint = estate.server_id, probe_seq = estate.probe_seq;
    bool dead;
    int r;
...
//
    ...
    {
    let mut responded: bool = test_bit(AFS_ESTATE_RESPONDED, &estate.flags);
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_fileserver_probe_result(call: *mut afs_call) {
    ...
    let mut index: c_uint = call.probe_index;
    let mut rtt_us: c_uint = -1, cap0;
    let mut ret: c_int = call.error;
...
//
    ...
    struct afs_addr_list *old_alist = core::ptr::null_mut(), *alist;
    ...
    while (unprobed) {
    let mut index: c_uint = 0, i;
    let mut best_prio: c_int = -1;
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_wait_for_fs_probes(op: *mut afs_operation, states: *mut afs_server_state, intr: bool) -> c_int {
    int afs_wait_for_fs_probes(struct afs_operation *op, struct afs_server_state *states, bool intr)
    {
    struct afs_endpoint_state *estate;
    struct afs_server_list *slist = op.server_list;
    let mut still_probing: bool = true;
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_fs_probe_timer(timer: *mut timer_list) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_dispatch_fs_probe(net: *mut afs_net, server: *mut afs_server) {
...
//
    ...
    write_sequnlock(&net.fs_lock);
    afs_fs_probe_fileserver(net, server, core::ptr::null_mut(), key);
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_probe_fileserver(net: *mut afs_net, server: *mut afs_server) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_fs_probe_dispatcher(work: *mut work_struct) {
    ...
    let mut first_pass: bool = true, set_timer = false;
    if (!net.live) {
    afs_dec_servers_outstanding(net);
    return;
    }
    ...
    again:
    write_seqlock(&net.fs_lock);
    fast = slow = server = core::ptr::null_mut();
    nowj = jiffies;
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_fs_probe_cleanup(net: *mut afs_net) {
