...
#[no_mangle]
pub unsafe extern "C" fn afs_put_serverlist(net: *mut afs_net, slist: *mut afs_server_list) {
...
//
    ...
    let mut type_mask: c_uint = 1 << volume.type;
    let mut use_newrepsites: bool = false;
    let mut ret: c_int = -ENOMEM, nr_servers = 0, newrep = 0, i, j, usable = 0;
...
// Make sure a records exists for each server in the list.
    ...
    let mut newrepsite: bool = vldb.vlsf_flags[i] & AFS_VLSF_NEWREPSITE;
    if (!(vldb.fs_mask[i] & type_mask))
    continue;
    if (vldb.vlsf_flags[i] & AFS_VLSF_DONTUSE)
...
//
    ...
    {
    let mut mask: c_ulong = 1UL << AFS_SE_EXCLUDED;
    int i;
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_attach_volume_to_servers(volume: *mut afs_volume, slist: *mut afs_server_list) {
...
//
    ...
    struct afs_server_list *old)
    {
    let mut n: c_uint = 0, o = 0;
    down_write(&volume.cell.vs_lock);
    while (n < new.nr_servers || o < old.nr_servers) {
    struct afs_server_entry *pn = n < new.nr_servers ? &new.servers[n] : core::ptr::null_mut();
    struct afs_server_entry *po = o < old.nr_servers ? &old.servers[o] : core::ptr::null_mut();
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_detach_volume_from_servers(volume: *mut afs_volume, slist: *mut afs_server_list) {
