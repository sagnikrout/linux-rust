...
//
    ...
    return core::ptr::null_mut(); /* Need a write lock */
...
//
    ...
    struct afs_addr_list *alist = core::ptr::null_mut();
    int ret;
    ret = -ERESTARTSYS;
...
//
    ...
    {
    struct afs_addr_list *alist = core::ptr::null_mut();
    struct afs_server *server, *candidate = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn afs_set_server_timer(server: *mut afs_server, delay_secs: c_uint) {
...
//
    ...
    if (!test_bit(AFS_SERVER_FL_EXPIRED, &server.flags) &&
    READ_ONCE(server.cell.state) < AFS_CELL_REMOVING) {
    let mut unuse_time: time64_t = ktime_get_real_seconds();
...
#[no_mangle]
unsafe extern "C" fn afs_server_rcu(rcu: *mut rcu_head) {
...
#[no_mangle]
unsafe extern "C" fn __afs_put_server(net: *mut afs_net, server: *mut afs_server) {
...
#[no_mangle]
unsafe extern "C" fn afs_give_up_callbacks(net: *mut afs_net, server: *mut afs_server) {
    ...
    afs_fs_give_up_all_callbacks(net, server, &alist.addrs[alist.preferred], core::ptr::null_mut());
    }
//
// Check to see if the server record has expired.
//
#[no_mangle]
unsafe extern "C" fn afs_has_server_expired(server: *const afs_server) -> bool {
...
//
#[no_mangle]
unsafe extern "C" fn afs_remove_server_from_cell(server: *mut afs_server) -> bool {
...
#[no_mangle]
unsafe extern "C" fn afs_server_destroyer(work: *mut work_struct) {
    static void afs_server_destroyer(struct work_struct *work)
    {
...
// Unbind the rxrpc_peer records from the server.
    ...
    if (estate)
    afs_set_peer_appdata(server, estate.addresses, core::ptr::null_mut());
    write_seqlock(&net.fs_lock);
...
#[no_mangle]
unsafe extern "C" fn afs_server_timer(timer: *mut timer_list) {
    static void afs_server_timer(struct timer_list *timer)
    {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_purge_servers(cell: *mut afs_cell) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_wait_for_servers(net: *mut afs_net) {
