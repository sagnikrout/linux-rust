...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct afs_lookup_one_cookie {
    pub ctx: dir_context,
    pub name: qstr,
    pub found: bool,
    pub fid: afs_fid,
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct afs_lookup_cookie {
    pub ctx: dir_context,
    pub name: qstr,
    pub nr_fids: c_ushort,
    pub fids: [afs_fid; 50],
}

#[no_mangle]
unsafe extern "C" fn afs_dir_unuse_cookie(dvnode: *mut afs_vnode, ret: c_int) {
    static void afs_dir_unuse_cookie(struct afs_vnode *dvnode, int ret)
    {
    if (ret == 0) {
    struct afs_vnode_cache_aux aux;
    let mut i_size: loff_t = i_size_read(&dvnode.netfs.inode);
    afs_set_cache_aux(dvnode, &aux);
    fscache_unuse_cookie(afs_vnode_cache(dvnode), &aux, &i_size);
    } else {
    fscache_unuse_cookie(afs_vnode_cache(dvnode), core::ptr::null_mut(), core::ptr::null_mut());
    }
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn afs_dir_dump(dvnode: *mut afs_vnode) {
    static void afs_dir_dump(struct afs_vnode *dvnode)
    {
    struct iov_iter iter;
    let mut i_size: c_ulonglong = i_size_read(&dvnode.netfs.inode);
    ...
    iterate_folioq(&iter, iov_iter_count(&iter), core::ptr::null_mut(), core::ptr::null_mut(),
    afs_dir_dump_step);
    }
//
...
//
#[no_mangle]
unsafe extern "C" fn afs_dir_check(dvnode: *mut afs_vnode) -> c_int {
    ...
    return 0;
    iov_iter_folio_queue(&iter, ITER_SOURCE, dvnode.directory, 0, 0, i_size);
    checked = iterate_folioq(&iter, iov_iter_count(&iter), dvnode, core::ptr::null_mut(),
    afs_dir_check_step);
    if (checked != i_size) {
    afs_dir_dump(dvnode);
    return -EIO;
...
//
#[no_mangle]
unsafe extern "C" fn afs_dir_open(inode: *mut inode, file: *mut file) -> c_int {
    static int afs_dir_open(struct inode *inode, struct file *file)
    {
...
//
#[no_mangle]
unsafe extern "C" fn afs_do_read_single(dvnode: *mut afs_vnode, file: *mut file) -> isize {
...
// Expand the storage.  TODO: Shrink the storage too.
    ...
    let mut cur_size: usize = dvnode.directory_size;
    ret = netfs_alloc_folioq_buffer(core::ptr::null_mut(),
...
//
    ...
    let mut ret2: c_int = afs_dir_check(dvnode);
    if (ret2 < 0)
    ret = ret2;
    }
    }
...
#[no_mangle]
unsafe extern "C" fn afs_read_single(dvnode: *mut afs_vnode, file: *mut file) -> isize {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_read_dir(dvnode: *mut afs_vnode, file: *mut file) -> isize {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct afs_dir_iteration_ctx {
    pub dir_ctx: *mut dir_context,
    pub error: c_int,
}
...
//
#[no_mangle]
unsafe extern "C" fn afs_dir_iterate_contents(dir: *mut inode, dir_ctx: *mut dir_context) -> c_int {
    static int afs_dir_iterate_contents(struct inode *dir, struct dir_context *dir_ctx)
    {
    let mut ctx: afs_dir_iteration_ctx = { .dir_ctx = dir_ctx };
    struct afs_vnode *dvnode = AFS_FS_I(dir);
    struct iov_iter iter;
    let mut i_size: c_ulonglong = i_size_read(dir);
...
//
#[no_mangle]
unsafe extern "C" fn afs_readdir(file: *mut file, ctx: *mut dir_context) -> c_int {
    static int afs_readdir(struct file *file, struct dir_context *ctx)
    {
...
// search the directory
    ret = afs_dir_iterate(dir, &cookie.ctx, core::ptr::null_mut(), _dir_version);
    if (ret < 0) {
    _leave(" = %d [iter]", ret);
    return ret;
    }
...
//
#[no_mangle]
unsafe extern "C" fn afs_do_lookup_success(op: *mut afs_operation) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_server_supports_ibulk(dvnode: *mut afs_vnode) -> bool {
...
//
    ...
    struct inode *inode = core::ptr::null_mut(), *ti;
    let mut data_version: afs_dataversion_t = READ_ONCE(dvnode.status.data_version);
...
// ...then scan linearly from that point for entries to lookup-ahead.
    ...
    afs_dir_iterate(dir, &cookie.ctx, core::ptr::null_mut(), &data_version);
    }
...
// vnodes also.
//
    op = afs_alloc_operation(core::ptr::null_mut(), dvnode.volume);
    if (IS_ERR(op)) {
    ret = PTR_ERR(op);
    goto out;
    }
...
//
    ...
    op.file[1].vnode = core::ptr::null_mut();
    }
    }
...
//
    ...
    ASSERTCMP(d_inode(dentry), ==, core::ptr::null_mut());
    ...
    ret = afs_validate(dvnode, core::ptr::null_mut());
    if (ret < 0) {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: !IS_ERR_OR_NULL(inode)) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn afs_d_revalidate_rcu(dvnode: *mut afs_vnode, dentry: *mut dentry) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_d_delete(dentry: *const dentry) -> c_int {
    static int afs_d_delete(const struct dentry *dentry)
    {
    _enter("%pd", dentry);
...
//
#[no_mangle]
unsafe extern "C" fn afs_d_iput(dentry: *mut dentry, inode: *mut inode) {
    static void afs_d_iput(struct dentry *dentry, struct inode *inode)
    {
    if (dentry.d_flags & DCACHE_NFSFS_RENAMED)
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_d_release(dentry: *mut dentry) {
    void afs_d_release(struct dentry *dentry)
    {
    _enter("%pd", dentry);
    }
#[no_mangle]
pub unsafe extern "C" fn afs_check_for_remote_deletion(op: *mut afs_operation) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_vnode_new_inode(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_create_success(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_create_edit_dir(op: *mut afs_operation) {
    static void afs_create_edit_dir(struct afs_operation *op)
    {
    let mut cres: netfs_cache_resources = {};
    struct afs_vnode_param *dvp = &op.file[0];
    struct afs_vnode_param *vp = &op.file[1];
...
#[no_mangle]
unsafe extern "C" fn afs_create_put(op: *mut afs_operation) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_dir_remove_subdir(dentry: *mut dentry) {
...
#[no_mangle]
unsafe extern "C" fn afs_rmdir_success(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_rmdir_edit_dir(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_rmdir_put(op: *mut afs_operation) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_rmdir(dir: *mut inode, dentry: *mut dentry) -> c_int {
    static int afs_rmdir(struct inode *dir, struct dentry *dentry)
    {
    struct afs_operation *op;
    struct afs_vnode *dvnode = AFS_FS_I(dir), *vnode = core::ptr::null_mut();
    int ret;
    _enter("{%llx:%llu},{%pd}",
...
//
#[no_mangle]
unsafe extern "C" fn afs_dir_remove_link(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_unlink_success(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_unlink_edit_dir(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_unlink_put(op: *mut afs_operation) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
    static int afs_unlink(struct inode *dir, struct dentry *dentry)
    {
    struct afs_operation *op;
    struct afs_vnode *dvnode = AFS_FS_I(dir);
...
//
    ...
    struct afs_operation *op;
    struct afs_vnode *dvnode = AFS_FS_I(dir);
    let mut ret: c_int = -ENAMETOOLONG;
    _enter("{%llx:%llu},{%pd},%ho",
    dvnode.fid.vid, dvnode.fid.vnode, dentry, mode);
    if (dentry.d_name.len >= AFSNAMEMAX)
    goto error;
...
#[no_mangle]
unsafe extern "C" fn afs_link_success(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_link_put(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_symlink_put(op: *mut afs_operation) {
...
//
    ...
    struct afs_vnode *dvnode = AFS_FS_I(dir);
    let mut clen: usize = strlen(content);
    int ret;
    _enter("{%llx:%llu},{%pd},%s",
    dvnode.fid.vid, dvnode.fid.vnode, dentry,
    content);
...
#[no_mangle]
unsafe extern "C" fn afs_rename_success(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_rename_edit_dir(op: *mut afs_operation) {
    static void afs_rename_edit_dir(struct afs_operation *op)
    {
    let mut orig_cres: netfs_cache_resources = {}, new_cres = {};
    ...
    op.rename.rehash = core::ptr::null_mut();
    }
...
#[no_mangle]
pub unsafe extern "C" fn if(0: new_inode->i_nlink >) -> else {
...
#[no_mangle]
unsafe extern "C" fn afs_rename_exchange_edit_dir(op: *mut afs_operation) {
...
#[no_mangle]
unsafe extern "C" fn afs_rename_put(op: *mut afs_operation) {
    ...
    .issue_afs_rpc	= core::ptr::null_mut(),
...
//
    ...
    {
    struct afs_operation *op;
    struct afs_vnode *orig_dvnode, *new_dvnode, *vnode, *new_vnode = core::ptr::null_mut();
    int ret;
...
// Don't allow silly-rename files be moved around.
    ...
    new_dentry);
    op = afs_alloc_operation(core::ptr::null_mut(), orig_dvnode.volume);
    if (IS_ERR(op))
    return PTR_ERR(op);
