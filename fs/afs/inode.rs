...
#[no_mangle]
unsafe extern "C" fn dump_vnode(vnode: *mut afs_vnode, parent_vnode: *mut afs_vnode) -> noinline void {
...
//
#[no_mangle]
unsafe extern "C" fn afs_set_netfs_context(vnode: *mut afs_vnode, is_file: bool) {
...
// Symlinks with a mode of 0644 are actually mountpoints.
    ...
    break;
    default:
    dump_vnode(vnode, op.file[0].vnode != vnode ? op.file[0].vnode : core::ptr::null_mut());
    write_sequnlock(&vnode.cb_lock);
    return afs_protocol_error(core::ptr::null_mut(), afs_eproto_file_type);
    }
...
//
    ...
    struct timespec64 t;
    umode_t mode;
    let mut unexpected_jump: bool = false;
    let mut data_changed: bool = false;
    let mut change_size: bool = vp.set_size;
    _enter("{%llx:%llu.%u} %s",
    vp.fid.vid, vp.fid.vnode, vp.fid.unique,
    op.type ? op.type.name : "???");
    ...
    afs_protocol_error(core::ptr::null_mut(), afs_eproto_bad_status);
    return;
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_vnode_commit_status(op: *mut afs_operation, vp: *mut afs_vnode_param) {
...
#[no_mangle]
unsafe extern "C" fn afs_fetch_status_success(op: *mut afs_operation) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_ilookup5_test_by_fid(inode: *mut inode, opaque: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_iget5_test(inode: *mut inode, opaque: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_iget5_set(inode: *mut inode, opaque: *mut c_void) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_get_inode_cache(vnode: *mut afs_vnode) {
    ...
    vnode.netfs.cache = core::ptr::null_mut();
    return;
    }
...
#[no_mangle]
unsafe extern "C" fn afs_iget5_set_root(inode: *mut inode, opaque: *mut c_void) -> c_int {
...
//
    ...
    inode = iget5_locked(sb, 1, core::ptr::null_mut(), afs_iget5_set_root, core::ptr::null_mut());
    if (!inode) {
    _leave(" = -ENOMEM");
    return ERR_PTR(-ENOMEM);
    }
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_drop_inode(inode: *mut inode) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_evict_inode(inode: *mut inode) {
    ...
    vnode.silly_key = core::ptr::null_mut();
    key_put(vnode.lock_key);
    vnode.lock_key = core::ptr::null_mut();
    _leave("");
    }
#[no_mangle]
unsafe extern "C" fn afs_setattr_success(op: *mut afs_operation) {
    static void afs_setattr_success(struct afs_operation *op)
    {
    struct afs_vnode_param *vp = &op.file[0];
    struct inode *inode = &vp.vnode.netfs.inode;
    let mut old_i_size: loff_t = i_size_read(inode);
    op.setattr.old_i_size = old_i_size;
    afs_vnode_commit_status(op, vp);
// inode->i_size has now been changed.
    if (op.setattr.attr.ia_valid & ATTR_SIZE) {
    let mut size: loff_t = op.setattr.attr.ia_size;
    if (size > old_i_size)
    pagecache_isize_extended(inode, old_i_size, size);
    }
    }
#[no_mangle]
unsafe extern "C" fn afs_setattr_edit_file(op: *mut afs_operation) {
    ...
    if (op.setattr.attr.ia_valid & ATTR_SIZE) {
    let mut size: loff_t = op.setattr.attr.ia_size;
    let mut old: loff_t = op.setattr.old_i_size;
...
// Prevent any new writebacks from starting whilst we do this.
    ...
    let mut size: loff_t = attr.ia_size;
// Wait for any outstanding writes to the server to complete
    let mut from: loff_t = min(size, i_size);
    let mut to: loff_t = max(size, i_size);
...
//
    ...
    afs_file_key(attr.ia_file) : core::ptr::null_mut()),
    vnode.volume);
    if (IS_ERR(op)) {
    ret = PTR_ERR(op);
    goto out_unlock;
