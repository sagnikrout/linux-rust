...
#[no_mangle]
unsafe extern "C" fn afs_put_symlink(symlink: *mut afs_symlink) {
...
#[no_mangle]
unsafe extern "C" fn afs_replace_symlink(vnode: *mut afs_vnode, symlink: *mut afs_symlink) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_invalidate_symlink(vnode: *mut afs_vnode) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_evict_symlink(vnode: *mut afs_vnode) {
    void afs_evict_symlink(struct afs_vnode *vnode)
    {
    struct afs_symlink *old;
    old = rcu_replace_pointer(vnode.symlink, core::ptr::null_mut(), true);
    if (old)
    afs_put_symlink(old);
    }
//
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_init_new_symlink(vnode: *mut afs_vnode, op: *mut afs_operation) {
    void afs_init_new_symlink(struct afs_vnode *vnode, struct afs_operation *op)
    {
    struct afs_symlink *symlink = op.create.symlink;
    let mut dsize: usize = 0;
    let mut size: usize = strlen(symlink.content) + 1;
    char *p;
    ...
    return;
    if (netfs_alloc_folioq_buffer(core::ptr::null_mut(), &vnode.directory, &dsize, size,
...
//
#[no_mangle]
unsafe extern "C" fn afs_do_read_symlink(vnode: *mut afs_vnode) -> isize {
...
//
    ret = netfs_read_single(&vnode.netfs.inode, core::ptr::null_mut(), &iter);
    if (ret >= 0) {
    i_size = ret;
    if (i_size > PAGE_SIZE - 1) {
    trace_afs_file_error(vnode, -EFBIG, afs_file_error_dir_big);
...
#[no_mangle]
unsafe extern "C" fn afs_read_symlink(vnode: *mut afs_vnode) -> isize {
...
#[no_mangle]
unsafe extern "C" fn afs_put_link(arg: *mut c_void) {
...
// RCU pathwalk.
    ...
    set_delayed_call(callback, core::ptr::null_mut(), core::ptr::null_mut());
...
#[no_mangle]
pub unsafe extern "C" fn afs_readlink(dentry: *mut dentry, buffer: *mut char __user, buflen: c_int) -> c_int {
