...
//
    ...
    let mut afs_volume_record_life: static unsigned __read_mostly = 60 * 60;
...
//
    ...
    {
    struct afs_volume *p;
    struct rb_node *parent = core::ptr::null_mut(), **pp;
...
#[no_mangle]
unsafe extern "C" fn afs_remove_volume_from_cell(volume: *mut afs_volume) {
...
//
    ...
    struct afs_vldb_entry *vldb;
    struct afs_volume *volume;
    let mut type_mask: c_ulong = 1UL << params.type;
...
//
#[no_mangle]
unsafe extern "C" fn afs_destroy_volume(work: *mut work_struct) {
    static void afs_destroy_volume(struct work_struct *work)
    {
    ...
    ASSERTCMP(volume.cache, ==, core::ptr::null_mut());
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_try_get_volume(volume: *mut afs_volume, reason: enum afs_volume_trace) -> bool {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_put_volume(volume: *mut afs_volume, reason: enum afs_volume_trace) {
    void afs_put_volume(struct afs_volume *volume, enum afs_volume_trace reason)
    {
    if (volume) {
    let mut debug_id: c_uint = volume.debug_id;
    let mut vid: afs_volid_t = volume.vid;
    bool zero;
    int r;
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_activate_volume(volume: *mut afs_volume) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_deactivate_volume(volume: *mut afs_volume) {
    void afs_deactivate_volume(struct afs_volume *volume)
    {
    _enter("%s", volume.name);

    fscache_relinquish_volume(volume.cache, core::ptr::null_mut(),
    test_bit(AFS_VOLUME_DELETED, &volume.flags));
    volume.cache = core::ptr::null_mut();
...
//
#[no_mangle]
unsafe extern "C" fn afs_update_volume_status(volume: *mut afs_volume, key: *mut key) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_check_volume_status(volume: *mut afs_volume, op: *mut afs_operation) -> c_int {
