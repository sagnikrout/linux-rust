...
//
#[no_mangle]
unsafe extern "C" fn afs_find_contig_bits(block: *mut union afs_xdr_dir_block, nr_slots: c_uint) -> c_int {
...
//
    ...
    core::ptr::null_mut(), &dvnode.directory, &cur_size, blend,
...
// Search the folio queue for the folio containing the block...
    ...
    let mut fsize: usize = folioq_folio_size(fq, s);
    if (blend <= fpos + fsize) {
...
//
    ...
    let mut iter: afs_dir_iter = { .dvnode = vnode };
...
//
    if (!iter.prev_entry) {
    let mut prev_next: __be16 = meta.meta.hashtable[iter.bucket];
    ...
    let mut pb: c_uint = iter.prev_entry / AFS_DIR_SLOTS_PER_BLOCK;
    let mut ps: c_uint = iter.prev_entry % AFS_DIR_SLOTS_PER_BLOCK;
    __be16 prev_next;
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_mkdir_init_dir(dvnode: *mut afs_vnode, parent_dvnode: *mut afs_vnode) {
    ...
    union afs_xdr_dirent *de;
    let mut slot: c_uint = AFS_DIR_RESV_BLOCKS0;
    loff_t i_size;
