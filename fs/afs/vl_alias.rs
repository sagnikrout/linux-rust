...
//
    ...
    let mut a: c_int = 0, b = 0, addr_matches = 0;
    la = rcu_dereference(server_a.endpoint_state).addresses;
    lb = rcu_dereference(server_b.endpoint_state).addresses;
    while (a < la.nr_addrs && b < lb.nr_addrs) {
    let mut pa: c_ulong = (unsigned long)la.addrs[a].peer;
    let mut pb: c_ulong = (unsigned long)lb.addrs[b].peer;
    let mut diff: c_long = pa - pb;
    if (diff < 0) {
    a++;
    } else if (diff > 0) {
    b++;
...
//
    ...
    let mut diff: c_int = memcmp(&server_a.uuid, &server_b.uuid, sizeof(uuid_t));
    if (diff < 0) {
    a++;
    } else if (diff > 0) {
    b++;
...
//
#[no_mangle]
unsafe extern "C" fn afs_compare_cell_roots(cell: *mut afs_cell) -> c_int {
...
//
    ...
    struct afs_cell *p)
    {
    struct afs_volume *volume, *pvol = core::ptr::null_mut();
    int ret;
...
//
#[no_mangle]
unsafe extern "C" fn afs_query_for_alias(cell: *mut afs_cell, key: *mut key) -> c_int {
...
//
    ...
    let mut skipped: bool = false, not_skipped = false;
    int ret;
...
#[no_mangle]
unsafe extern "C" fn yfs_check_canonical_cell_name(cell: *mut afs_cell, key: *mut key) -> c_int {
    ...
    master = afs_lookup_cell(cell.net, cell_name, name_len, core::ptr::null_mut(),
...
#[no_mangle]
unsafe extern "C" fn afs_do_cell_detect_alias(cell: *mut afs_cell, key: *mut key) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_cell_detect_alias(cell: *mut afs_cell, key: *mut key) -> c_int {
