...
//
#[no_mangle]
unsafe extern "C" fn afs_acl_success(op: *mut afs_operation) {
    static void afs_acl_success(struct afs_operation *op)
    {
    afs_vnode_commit_status(op, &op.file[0]);
    }
#[no_mangle]
unsafe extern "C" fn afs_acl_put(op: *mut afs_operation) {
...
//
    ...
    struct afs_acl *acl = core::ptr::null_mut();
    int ret;
    op = afs_alloc_operation(core::ptr::null_mut(), vnode.volume);
    if (IS_ERR(op))
    return -ENOMEM;
...
//
    ...
    struct afs_operation *op;
    struct afs_vnode *vnode = AFS_FS_I(inode);
    struct yfs_acl *yacl = core::ptr::null_mut();
    char buf[16], *data;
    let mut which: c_int = 0, dsize, ret = -ENOMEM;
    if (strcmp(name, "acl") == 0)
    which = 0;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(name, 0: "acl_inherited") ==) -> else {
    else if (strcmp(name, "acl_inherited") == 0)
    which = 1;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(name, 0: "acl_num_cleaned") ==) -> else {
    else if (strcmp(name, "acl_num_cleaned") == 0)
    which = 2;
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(name, 0: "vol_acl") ==) -> else {
...
#[no_mangle]
pub unsafe extern "C" fn if(3: which ==) -> else {
    else if (which == 3)
