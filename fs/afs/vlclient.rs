...
//
#[no_mangle]
unsafe extern "C" fn afs_deliver_vl_get_entry_by_name_u(call: *mut afs_call) -> c_int {
...
// unmarshall the reply once we've received all of it
    ...
    let mut tmp: u32 = ntohl(uvldb.serverFlags[i]);
    int j;
    let mut n: c_int = entry.nr_servers;
...
//
#[no_mangle]
unsafe extern "C" fn afs_deliver_vl_get_addrs_u(call: *mut afs_call) -> c_int {
...
//
    ...
    return ERR_PTR(-ENOMEM);
    call.key = vc.key;
    call.ret_alist = core::ptr::null_mut();
    call.max_lifespan = AFS_VL_MAX_LIFESPAN;
    call.peer = rxrpc_kernel_get_peer(vc.alist.addrs[vc.addr_index].peer);
    call.service_id = vc.server.service_id;
// Marshall the parameters
...
//
#[no_mangle]
unsafe extern "C" fn afs_deliver_vl_get_capabilities(call: *mut afs_call) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn afs_destroy_vl_get_capabilities(call: *mut afs_call) {
...
//
#[no_mangle]
unsafe extern "C" fn afs_deliver_yfsvl_get_endpoints(call: *mut afs_call) -> c_int {
...
//
#[no_mangle]
unsafe extern "C" fn afs_deliver_yfsvl_get_cell_name(call: *mut afs_call) -> c_int {
...
//
    ...
    return ERR_PTR(-ENOMEM);
    call.key = vc.key;
    call.ret_str = core::ptr::null_mut();
    call.max_lifespan = AFS_VL_MAX_LIFESPAN;
    call.peer = rxrpc_kernel_get_peer(vc.alist.addrs[vc.addr_index].peer);
    call.service_id = vc.server.service_id;
// marshall the parameters
