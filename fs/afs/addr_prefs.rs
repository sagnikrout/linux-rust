...
//
#[no_mangle]
unsafe extern "C" fn afs_split_string(pbuf: *mut c_char, strv[]: *mut c_char, maxstrv: c_uint) -> c_int {
    static int afs_split_string(char **pbuf, char *strv[], unsigned int maxstrv)
    {
    let mut count: c_uint = 0;
    char *p = *pbuf;
    maxstrv--; /* Allow for terminal core::ptr::null_mut() */
    for (;;) {
// Skip over spaces
    while (isspace(*p)) {
    if (*p == '\n') {
...
//
#[no_mangle]
unsafe extern "C" fn afs_parse_address(p: *mut c_char, pref: *mut afs_addr_preference) -> c_int {
...
//
    ...
    {
    let mut subnet: c_int = min(a.subnet_mask, b.subnet_mask);
    const __be32 *pa, *pb;
    u32 mask, na, nb;
    int diff;
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_proc_addr_prefs_write(file: *mut file, buf: *mut c_char, size: usize) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(_arg: strcmp(argv[0], 0: "del") ==) -> else {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_get_address_preferences_rcu(net: *mut afs_net, alist: *mut afs_addr_list) {
...
//
#[no_mangle]
pub unsafe extern "C" fn afs_get_address_preferences(net: *mut afs_net, alist: *mut afs_addr_list) {
