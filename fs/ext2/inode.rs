...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_inode_is_fast_symlink(inode: *mut inode) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn ext2_write_failed(mapping: *mut address_space, to: loff_t) {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_evict_inode(inode: *mut *mut inode) {
...
// truncate to 0
    ...
    EXT2_I(inode).i_block_alloc_info = core::ptr::null_mut();
    if (unlikely(rsv))
    kfree(rsv);
    if (want_delete) {
...
#[no_mangle]
pub unsafe extern "C" fn add_chain(p: *mut Indirect, bh: *mut buffer_head, v: *mut __le32) {
...
//
    ...
    {
    let mut ptrs: c_int = EXT2_ADDR_PER_BLOCK(inode.i_sb);
    let mut ptrs_bits: c_int = EXT2_ADDR_PER_BLOCK_BITS(inode.i_sb);
    const long direct_blocks = EXT2_NDIR_BLOCKS,
    indirect_blocks = ptrs,
    double_blocks = (1 << (ptrs_bits * 2));
    let mut n: c_int = 0;
    let mut final: c_int = 0;
    if (i_block < 0) {
...
// i_data is not going away, no lock needed
    add_chain (chain, core::ptr::null_mut(), EXT2_I(inode).i_data + *offsets);
    if (!p.key)
    goto no_block;
    while (--depth) {
...
//
#[no_mangle]
unsafe extern "C" fn ext2_find_near(inode: *mut inode, ind: *mut Indirect) -> ext2_fsblk_t {
...
//
    ...
    let mut current_block: ext2_fsblk_t = 0;
    let mut ret: c_int = 0;
//
...
//
    ...
    {
    let mut blocksize: c_int = inode.i_sb.s_blocksize;
    int i, n = 0;
    let mut err: c_int = 0;
    struct buffer_head *bh;
    int num;
...
//
    ...
    ext2_fsblk_t goal;
    int indirect_blks;
    let mut blocks_to_boundary: c_int = 0;
    int depth;
    struct ext2_inode_info *ei = EXT2_I(inode);
    let mut count: c_int = 0;
    let mut first_block: ext2_fsblk_t = 0;
...
// bno = le32_to_cpu(chain[depth-1].key);
    ...
    {
    let mut max_blocks: unsigned = bh_result.b_size >> inode.i_blkbits;
    let mut new: bool = false, boundary = false;
    u32 bno;
    int ret;
    ...
    let mut first_block: c_ulong = offset >> blkbits;
    let mut max_blocks: c_ulong = (length + (1 << blkbits) - 1) >> blkbits;
    let mut new: bool = false, boundary = false;
    u32 bno;
    int ret;
    let mut create: bool = flags & IOMAP_WRITE;
//
...
#[no_mangle]
unsafe extern "C" fn ext2_read_folio(file: *mut file, folio: *mut folio) -> c_int {
    static int ext2_read_folio(struct file *file, struct folio *folio)
    {
    return mpage_read_folio(folio, ext2_get_block);
    }
#[no_mangle]
unsafe extern "C" fn ext2_readahead(rac: *mut readahead_control) {
...
#[no_mangle]
unsafe extern "C" fn ext2_bmap(mapping: *mut address_space, block: sector_t) -> sector_t {
...
//
#[no_mangle]
pub unsafe extern "C" fn all_zeroes(p: *mut __le32, q: *mut __le32) -> c_int {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_free_data(inode: *mut inode, p: *mut __le32, q: *mut __le32) {
    static inline void ext2_free_data(struct inode *inode, __le32 *p, __le32 *q)
    {
    let mut block_to_free: ext2_fsblk_t = 0, count = 0;
    ext2_fsblk_t nr;
    for ( ; p < q ; p++) {
    nr = le32_to_cpu(*p);
    if (nr) {
...
// accumulate blocks to free if they're contiguous
    ...
    goto free_this;
#[no_mangle]
pub unsafe extern "C" fn if(count: block_to_free == nr -) -> else {
...
//
#[no_mangle]
unsafe extern "C" fn ext2_free_branches(inode: *mut inode, p: *mut __le32, q: *mut __le32, depth: c_int) {
    static void ext2_free_branches(struct inode *inode, __le32 *p, __le32 *q, int depth)
    {
    struct buffer_head * bh;
    ext2_fsblk_t nr;
    if (depth--) {
    let mut addr_per_block: c_int = EXT2_ADDR_PER_BLOCK(inode.i_sb);
    for ( ; p < q ; p++) {
    nr = le32_to_cpu(*p);
    if (!nr)
    continue;
...
#[no_mangle]
unsafe extern "C" fn __ext2_truncate_blocks(inode: *mut inode, offset: loff_t) {
    ...
    n = ext2_block_to_path(inode, iblock, offsets, core::ptr::null_mut());
    if (n == 0)
    return;
//
...
#[no_mangle]
unsafe extern "C" fn ext2_truncate_blocks(inode: *mut inode, offset: loff_t) {
    static void ext2_truncate_blocks(struct inode *inode, loff_t offset)
    {
...
#[no_mangle]
unsafe extern "C" fn ext2_setsize(inode: *mut inode, newsize: loff_t) -> c_int {
...
// p = NULL;
    ...
    gdp = ext2_get_group_desc(sb, block_group, core::ptr::null_mut());
    if (!gdp)
    goto Egdp;
//
...
#[no_mangle]
pub unsafe extern "C" fn ext2_set_inode_flags(inode: *mut inode) {
    void ext2_set_inode_flags(struct inode *inode)
    {
    let mut flags: c_uint = EXT2_I(inode).i_flags;
...
#[no_mangle]
pub unsafe extern "C" fn ext2_set_file_ops(inode: *mut inode) {
...
#[no_mangle]
pub unsafe extern "C" fn ext2_write_inode(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
    int ext2_write_inode(struct inode *inode, struct writeback_control *wbc)
    {
    struct ext2_inode_info *ei = EXT2_I(inode);
    struct super_block *sb = inode.i_sb;
    let mut ino: ino_t = inode.i_ino;
    let mut uid: uid_t = i_uid_read(inode);
    let mut gid: gid_t = i_gid_read(inode);
    struct buffer_head * bh;
    let mut raw_inode: *mut ext2_inode = ext2_get_inode(sb, ino, &bh);
    int n;
    let mut err: c_int = 0;
    if (IS_ERR(raw_inode))
    return -EIO;
...
#[no_mangle]
pub unsafe extern "C" fn ext2_sync_inode_metadata(inode: *mut inode, wbc: *mut writeback_control) -> c_int {
