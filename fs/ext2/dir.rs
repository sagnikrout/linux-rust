...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_rec_len_from_disk(dlen: __le16) -> unsigned {
    static inline unsigned ext2_rec_len_from_disk(__le16 dlen)
    {
    let mut len: unsigned = le16_to_cpu(dlen);
...
#[no_mangle]
pub unsafe extern "C" fn ext2_rec_len_to_disk(len: unsigned) -> __le16 {
...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_chunk_size(inode: *mut inode) -> unsigned {
...
//
    ...
    {
    let mut last_byte: unsigned = inode.i_size;
...
#[no_mangle]
unsafe extern "C" fn ext2_commit_chunk(folio: *mut folio, pos: loff_t, len: unsigned) {
...
#[no_mangle]
unsafe extern "C" fn ext2_check_folio(folio: *mut folio, quiet: c_int, kaddr: *mut c_char) -> bool {
    static bool ext2_check_folio(struct folio *folio, int quiet, char *kaddr)
    {
    struct inode *dir = folio.mapping.host;
    struct super_block *sb = dir.i_sb;
    let mut chunk_size: unsigned = ext2_chunk_size(dir);
    let mut max_inumber: u32 = le32_to_cpu(EXT2_SB(sb).s_es.s_inodes_count);
    unsigned offs, rec_len;
    let mut limit: unsigned = folio_size(folio);
    ext2_dirent *p;
    char *error;
...
//
    ...
    struct folio *folio = read_mapping_folio(mapping, n, core::ptr::null_mut());
    void *kaddr;
    if (IS_ERR(folio))
...
#[no_mangle]
pub unsafe extern "C" fn ext2_set_de_type(de: *mut ext2_dirent, inode: *mut inode) {
    ...
    let mut offset: c_uint = pos & ~PAGE_MASK;
    let mut n: c_ulong = pos >> PAGE_SHIFT;
    let mut npages: c_ulong = dir_pages(inode);
    let mut chunk_mask: unsigned = ~(ext2_chunk_size(inode)-1);
    let mut need_revalidate: bool = !inode_eq_iversion(inode, *(u64 *)file.private_data);
    bool has_filetype;
...
// (u64 *)file->private_data = inode_query_iversion(inode);
    ...
    if (de.inode) {
    let mut d_type: c_uchar = DT_UNKNOWN;
    if (has_filetype)
...
//
    ...
    let mut namelen: c_int = child.len;
    let mut reclen: unsigned = EXT2_DIR_REC_LEN(namelen);
    unsigned long start, n;
    let mut npages: c_ulong = dir_pages(dir);
...
#[no_mangle]
pub unsafe extern "C" fn ext2_inode_by_name(dir: *mut inode, child: *const qstr, ino: *mut ino_t) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext2_prepare_chunk(folio: *mut folio, pos: loff_t, len: unsigned) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext2_handle_dirsync(dir: *mut inode) -> c_int {
    ...
    {
    let mut pos: loff_t = folio_pos(folio) + offset_in_folio(folio, de);
    let mut len: unsigned = ext2_rec_len_from_disk(de.rec_len);
    int err;
    folio_lock(folio);
...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_add_link(dentry: *mut dentry, inode: *mut inode) -> c_int {
    int ext2_add_link (struct dentry *dentry, struct inode *inode)
    {
    struct inode *dir = d_inode(dentry.d_parent);
    const char *name = dentry.d_name.name;
    let mut namelen: c_int = dentry.d_name.len;
    let mut chunk_size: unsigned = ext2_chunk_size(dir);
    let mut reclen: unsigned = EXT2_DIR_REC_LEN(namelen);
...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_delete_entry(dir: *mut ext2_dir_entry_2, folio: *mut folio) -> c_int {
    ...
    char *kaddr;
    loff_t pos;
    ext2_dirent *de, *pde = core::ptr::null_mut();
    int err;
...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_make_empty(inode: *mut inode, parent: *mut inode) -> c_int {
    int ext2_make_empty(struct inode *inode, struct inode *parent)
    {
    struct folio *folio = filemap_grab_folio(inode.i_mapping, 0);
    let mut chunk_size: unsigned = ext2_chunk_size(inode);
    struct ext2_dir_entry_2 * de;
    int err;
    void *kaddr;
    if (IS_ERR(folio))
...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_empty_dir(inode: *mut inode) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext2_dir_open(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext2_dir_release(inode: *mut inode, file: *mut file) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext2_dir_llseek(file: *mut file, offset: loff_t, whence: c_int) -> loff_t {
