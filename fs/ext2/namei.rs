...
#[no_mangle]
pub unsafe extern "C" fn ext2_add_nondir(dentry: *mut dentry, inode: *mut inode) -> c_int {
    static inline int ext2_add_nondir(struct dentry *dentry, struct inode *inode)
    {
    let mut err: c_int = ext2_add_link(dentry, inode);
    if (!err) {
...
//
    ...
    {
    struct inode *inode = ext2_new_inode(dir, mode, core::ptr::null_mut());
    if (IS_ERR(inode))
    return PTR_ERR(inode);
    ext2_set_file_ops(inode);
    mark_inode_dirty(inode);
    ...
    let mut err: c_int = -ENAMETOOLONG;
    let mut l: unsigned = strlen(symname)+1;
    struct inode * inode;
    if (l > sb.s_blocksize)
    goto out;
    err = dquot_initialize(dir);
...
#[no_mangle]
unsafe extern "C" fn ext2_unlink(dir: *mut inode, dentry: *mut dentry) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn ext2_rmdir(dir: *mut *mut inode, dentry: *mut dentry) -> c_int {
    static int ext2_rmdir (struct inode * dir, struct dentry *dentry)
    {
    let mut inode: *mut inode = d_inode(dentry);
    let mut err: c_int = -ENOTEMPTY;
    ...
    unsigned int flags)
    {
    let mut old_inode: *mut inode = d_inode(old_dentry);
    let mut new_inode: *mut inode = d_inode(new_dentry);
    struct folio *dir_folio = core::ptr::null_mut();
