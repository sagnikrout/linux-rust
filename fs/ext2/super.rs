...
//
#[no_mangle]
pub unsafe extern "C" fn ext2_update_dynamic_rev(sb: *mut super_block) {
...
#[no_mangle]
unsafe extern "C" fn ext2_quota_off_umount(sb: *mut super_block) {
...
#[no_mangle]
pub unsafe extern "C" fn ext2_quota_off_umount(sb: *mut super_block) {
    static inline void ext2_quota_off_umount(struct super_block *sb)
    {
    }

#[no_mangle]
unsafe extern "C" fn ext2_put_super(sb: *mut *mut super_block) {
    ...
    ei.i_block_alloc_info = core::ptr::null_mut();
...
#[no_mangle]
unsafe extern "C" fn ext2_free_in_core_inode(inode: *mut inode) {
...
unsafe extern "C" fn init_inodecache() -> int __init {
    ...
    init_once);
    if (ext2_inode_cachep == core::ptr::null_mut())
    return -ENOMEM;
    return 0;
    }
#[no_mangle]
...
#[no_mangle]
unsafe extern "C" fn ext2_show_options(seq: *mut seq_file, root: *mut dentry) -> c_int {
    ...
    let mut def_errors: c_int = le16_to_cpu(es.s_errors);
...
#[no_mangle]
unsafe extern "C" fn ext2_free_fc(fc: *mut fs_context) {
    static void ext2_free_fc(struct fs_context *fc)
    {
    kfree(fc.fs_private);
    }
#[no_mangle]
unsafe extern "C" fn ext2_parse_param(fc: *mut fs_context, param: *mut fs_parameter) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn if(EXT2_ERROR_FS): (sbi->s_mount_state &) -> else {
...
#[no_mangle]
unsafe extern "C" fn ext2_check_descriptors(sb: *mut super_block) -> c_int {
    ...
    struct ext2_group_desc *gdp = ext2_get_group_desc(sb, i, core::ptr::null_mut());
    let mut first_block: ext2_fsblk_t = ext2_group_first_block_no(sb, i);
    let mut last_block: ext2_fsblk_t = ext2_group_last_block_no(sb, i);
...
//
#[no_mangle]
unsafe extern "C" fn ext2_max_size(bits: c_int) -> loff_t {
    static loff_t ext2_max_size(int bits)
    {
    let mut res: loff_t = EXT2_NDIR_BLOCKS;
    int meta_blocks;
...
//
#[no_mangle]
unsafe extern "C" fn ext2_set_options(fc: *mut fs_context, sbi: *mut ext2_sb_info) {
    static void ext2_set_options(struct fs_context *fc, struct ext2_sb_info *sbi)
    {
    struct ext2_fs_context *ctx = fc.fs_private;
    struct ext2_super_block *es = sbi.s_es;
    let mut def_mount_opts: c_ulong = le32_to_cpu(es.s_default_mount_opts);
...
#[no_mangle]
pub unsafe extern "C" fn if(EXT2_ERRORS_CONTINUE: le16_to_cpu(sbi->s_es->s_errors) ==) -> else {
...
#[no_mangle]
unsafe extern "C" fn ext2_fill_super(sb: *mut super_block, fc: *mut fs_context) -> c_int {
    ...
    struct inode *root;
    unsigned long block;
    let mut sb_block: c_ulong = ctx.s_sb_block;
    unsigned long logic_sb_block;
    let mut offset: c_ulong = 0;
    let mut ret: c_long = -ENOMEM;
    let mut blocksize: c_int = BLOCK_SIZE;
    int db_count;
    int i, j;
    __le32 features;
    int err;
...
#[no_mangle]
unsafe extern "C" fn ext2_clear_super_error(sb: *mut super_block) {
...
//
#[no_mangle]
unsafe extern "C" fn ext2_sync_fs(sb: *mut super_block, wait: c_int) -> c_int {
    static int ext2_sync_fs(struct super_block *sb, int wait)
    {
    struct ext2_sb_info *sbi = EXT2_SB(sb);
    struct ext2_super_block *es = EXT2_SB(sb).s_es;
...
#[no_mangle]
unsafe extern "C" fn ext2_freeze(sb: *mut super_block) -> c_int {
    static int ext2_freeze(struct super_block *sb)
    {
    struct ext2_sb_info *sbi = EXT2_SB(sb);
//
...
#[no_mangle]
unsafe extern "C" fn ext2_unfreeze(sb: *mut super_block) -> c_int {
    static int ext2_unfreeze(struct super_block *sb)
    {
...
#[no_mangle]
unsafe extern "C" fn ext2_write_super(sb: *mut super_block) {
    static void ext2_write_super(struct super_block *sb)
    {
    if (!sb_rdonly(sb))
    ext2_sync_fs(sb, 1);
...
#[no_mangle]
unsafe extern "C" fn ext2_reconfigure(fc: *mut fs_context) -> c_int {
    static int ext2_reconfigure(struct fs_context *fc)
    {
    struct ext2_fs_context *ctx = fc.fs_private;
    struct super_block *sb = fc.root.d_sb;
    let mut sbi: *mut ext2_sb_info = EXT2_SB(sb);
    struct ext2_super_block * es;
    struct ext2_mount_options new_opts;
    let mut flags: c_int = fc.sb_flags;
    int err;
    sync_filesystem(sb);
...
#[no_mangle]
unsafe extern "C" fn ext2_statfs(dentry: *mut *mut dentry, buf: *mut *mut kstatfs) -> c_int {
    static int ext2_statfs (struct dentry * dentry, struct kstatfs * buf)
    {
...
#[no_mangle]
pub unsafe extern "C" fn if(le32_to_cpu(es->s_blocks_count): sbi->s_blocks_last !=) -> else {
...
#[no_mangle]
unsafe extern "C" fn ext2_get_tree(fc: *mut fs_context) -> c_int {
...
// we don't have to be afraid of races
    ...
    let mut blk: sector_t = off >> EXT2_BLOCK_SIZE_BITS(sb);
    let mut err: c_int = 0;
...
#[no_mangle]
unsafe extern "C" fn ext2_quota_off(sb: *mut super_block, type: c_int) -> c_int {
    static int ext2_quota_off(struct super_block *sb, int type)
    {
    struct inode *inode = sb_dqopt(sb).files[type];
    int err;
...
#[no_mangle]
unsafe extern "C" fn ext2_init_fs_context(fc: *mut fs_context) -> c_int {
...
#[no_mangle]
unsafe extern "C" fn init_ext2_fs() -> int __init {
...
#[no_mangle]
unsafe extern "C" fn exit_ext2_fs() -> void __exit {
