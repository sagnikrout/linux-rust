...
//
#[no_mangle]
unsafe extern "C" fn verify_dirent_name(name: *const c_char, len: c_int) -> c_int {
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct readdir_callback {
    pub ctx: dir_context,
    pub dirent: *mut *mut old_linux_dirent __user,
    pub result: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct getdents_callback {
    pub ctx: dir_context,
    pub current_dir: *mut *mut linux_dirent __user,
    pub prev_reclen: c_int,
    pub error: c_int,
}
    ...
    unsigned long d_ino;
    let mut reclen: c_int = ALIGN(dirent_size(dirent, namlen + 2), sizeof(long));
    int prev_reclen;
    let mut flags: c_uint = d_type;
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct getdents_callback64 {
    pub ctx: dir_context,
    pub current_dir: *mut *mut linux_dirent64 __user,
    pub prev_reclen: c_int,
    pub error: c_int,
}
    ...
    let mut reclen: c_int = ALIGN(dirent_size(dirent, namlen + 1), sizeof(u64));
    int prev_reclen;
    let mut flags: c_uint = d_type;
    BUILD_BUG_ON(FILLDIR_FLAG_NOINTR & S_DT_MASK);
    d_type &= S_DT_MASK;
...
pub struct compat_old_linux_dirent {
    pub d_ino: compat_ulong_t,
    pub d_offset: compat_ulong_t,
    pub d_namlen: c_ushort,
    pub d_name: [c_char; ],
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_readdir_callback {
    pub ctx: dir_context,
    pub dirent: *mut compat_old_linux_dirent __user,
    pub result: c_int,
}
...
#[repr(C)]
#[derive(Copy, Clone)]
pub struct compat_getdents_callback {
    pub ctx: dir_context,
    pub current_dir: *mut compat_linux_dirent __user,
    pub prev_reclen: c_int,
    pub error: c_int,
}
    ...
    compat_ulong_t d_ino;
    let mut reclen: c_int = ALIGN(dirent_size(dirent, namlen + 2), sizeof(compat_long_t));
    int prev_reclen;
    let mut flags: c_uint = d_type;
    BUILD_BUG_ON(FILLDIR_FLAG_NOINTR & S_DT_MASK);
    d_type &= S_DT_MASK;
